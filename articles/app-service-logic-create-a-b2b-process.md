...
4. 생성된 TPM(거래 업체 관리) 서비스를 구성합니다.
	...
	2. *Components*에서 **파트너** 옵션을 사용하여 **Contoso**라는 새 파트너를  **추가**하고 해당 프로필에서 필수 AS2 ID를 추가합니다.
	3. *Components*에서 **파트너** 옵션을 사용하여 **Northwind**라는 새 파트너를  **추가**하고 해당 프로필에서 필수 AS2 ID를 추가합니다.
	4.  *Components*에서 **규약** 옵션을 사용하여 Contoso와 Northwind 간의 새로운 AS2 규약을 **추가**합니다. 여기서 Northwind는 호스트된 파트너이고 Contoso는 게스트 파트너입니다. 적절한 경우 규약을 생성하는 동안 서명, 암호화, 압축 및 승인을 구성합니다. 인증서를 사용해야 하는 경우 생성된 TPM 서비스를 검색할 때 **인증서** 옵션을 통해 업로드할 수 있습니다.
