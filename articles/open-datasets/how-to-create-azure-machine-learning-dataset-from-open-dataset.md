...
* 패키지를 포함 하는 [Python 용 AZURE MACHINE LEARNING SDK가 설치 되어](/python/api/overview/azure/ml/install?preserve-view=true&view=azure-ml-py) `azureml-datasets` 있습니다.
    ...
    * 사용자 고유의 Python 환경에서 작업 하 고 [다음 지침](/python/api/overview/azure/ml/install?preserve-view=true&view=azure-ml-py)에 따라 SDK를 설치 합니다.
...
작업 영역을 사용 하 여 데이터 집합을 등록 하려면 메서드를 사용 [`register()`](/python/api/azureml-core/azureml.data.abstract_dataset.abstractdataset?view=azure-ml-py#register-workspace--name--description-none--tags-none--create-new-version-false-&preserve-view=true ) 합니다. 
```Python
