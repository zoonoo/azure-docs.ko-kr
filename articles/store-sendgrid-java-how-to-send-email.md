...
이 가이드에서는 Azure에서 SendGrid 전자 메일 서비스로 일반 프로그래밍 작업을 수행하는 방법을 보여 줍니다. 샘플은 Java로 작성되었습니다. **전자 메일 생성**, **전자 메일 보내기**, **첨부 파일 추가**, **필터 사용**, **속성 업데이트** 등의 시나리오를 다룹니다. SendGrid 및 전자 메일 보내기에 대한 자세한 내용은 [다음 단계](#bkmk_NextSteps) 섹션을 참조하십시오.
...
javax.mail 라이브러리를 예를 들어 http://www.oracle.com/technetwork/java/javamail (영문)에서 얻어 코드로 가져옵니다. javax.mail 라이브러리를 사용하여 SMTP를 통해 전자 메일을 보내는 전반적인 프로세스는 다음과 같습니다.
...
5.  javax.mail.Transport 개체를 통해 메시지를 보냅니다. [방법: 전자 메일 보내기](#bkmk_HowToSendEmail) 섹션에 나와 있습니다.
...
SendGrid는 *필터* 사용을 통해 추가 전자 메일 기능을 제공합니다. 클릭 추적, Google 분석, 구독 추적 등을 사용하도록 설정하는 것과 같이 특정 기능을 사용하도록 설정하기 위해 전자 메일 메시지에 추가할 수 있는 설정입니다. 전체 필터 목록은 [필터 설정](http://sendgrid.com/docs/API_Reference/Web_API/filter_settings.html)(영문)을 참조하십시오.
...
SendGrid는 Azure 응용 프로그램에서 추가 SendGrid 기능을 활용하는 데 사용할 수 있는 웹 기반 API를 제공합니다. 자세한 내용은 [SendGrid API 설명서](http://sendgrid.com/docs/API_Reference/index.html)(영문)를 참조하십시오.

다음 단계
---------

SendGrid 전자 메일 서비스에 관한 기본 사항을 익혔으며, 자세한 내용을 보려면 다음 링크를 따라가십시오.

-   Azure 배포에서 SendGrid를 사용하는 방법을 보여 주는 샘플: [Azure 배포에서 Java의 SendGrid를 사용하여 전자 메일을 보내는 방법](/en-us/develop/java/how-to-guides/sendgrid-sample/)
-   SendGrid Java 정보: http://sendgrid.com/docs/Code\_Examples/java.html
-   SendGrid API 설명서: http://sendgrid.com/docs/API\_Reference/index.html
