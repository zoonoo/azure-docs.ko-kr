<properties urlDisplayName="Get Started with Data" pageTitle="데이터 시작(WP8) - Azure 모바일 서비스" metaKeywords="" description="Azure 모바일 서비스 Windows Phone 8 앱에서 데이터를 사용하는 방법에 대해 알아봅니다." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Get started with data in Mobile Services" authors="glenga" solutions="" manager="dwrede" editor="" />
...
이 자습서를 완료하려면 Windows 8에서 실행되는 Visual Studio 2012 Express for Windows Phone 8 및 [Windows Phone 8 SDK]가 필요합니다. 이 자습서를 완료하여 Windows Phone 8.1 앱을 만들려면 Visual Studio 2013 업데이트 2 이상 버전을 사용해야 합니다.
...
2. Visual Studio에서 다운로드한 프로젝트를 열고 MainPage.xaml.cs 파일을 검사합니다.
   	...
   	추가한 **TodoItem** 개체는 메모리 내 **ObservableCollection<TodoItem>**에 저장됩니다.
...
7. 기존 **items** 컬렉션을 정의하는 줄을 주석으로 처리한 후 다음 줄의 주석 처리를 제거합니다.
   	...
   	이 코드를 사용하면 모바일 서비스 인식 바인딩 컬렉션(**items**) 및 SQL 데이터베이스 테이블 **TodoItem**(**todoTable**)에 대한 프록시 클래스가 만들어집니다. 
...
2. 앞에서 한 것처럼 텍스트 상자에 텍스트를 입력하고 **저장**을 클릭합니다.
...
이 자습서에서는 Windows Phone 8 앱이 모바일 서비스의 데이터로 작업할 수 있게 하는 과정의 기본 사항을 설명했습니다. 이제 이 자습서에서 만든 GetStartedWithData 앱을 기반으로 하는 다음 자습서를 완료하세요.
