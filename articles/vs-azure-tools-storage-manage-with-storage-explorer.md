...
Azure Storage 탐색기(미리 보기)는 Windows, macOS 및 Linux에서 Azure Storage 데이터를 손쉽게 사용할 수 있는 독립 실행형 앱입니다. 이 문서에서는 Azure Storage 계정에 연결하고 관리하는 몇 가지 방법을 알아봅니다.

![Microsoft Azure Storage 탐색기(미리 보기)][0]
...
1. 저장소 탐색기(미리 보기)에서 **계정 관리**를 선택하여 **계정 관리 패널**로 이동합니다.
...
저장소 탐색기(미리 보기)를 사용하면 Azure Storage 에뮬레이터를 사용하여 로컬 저장소에서 작업할 수 있습니다. 이 방식에서는 저장소 계정이 Azure Storage 에뮬레이터에서 에뮬레이트되기 때문에 Azure에 저장소 계정을 반드시 배포하지 않고도, Azure Storage 작업을 시뮬레이트할 수 있습니다.
...
1. 저장소 탐색기(미리 보기)의 왼쪽 창에서 **(로컬 및 연결된)** > **Storage 계정** > **(개발)** > **Blob 컨테이너** 노드를 확장합니다.
...
1. 저장소 탐색기(미리 보기)에서 **연결 대화 상자**를 엽니다.
