...
- [로컬 개발 저장소로 작업](#work-with-local-development-storage) - Azure 저장소 에뮬레이터를 사용하여 로컬 저장소를 관리합니다.
...
1. 왼쪽 창에 로그인한 모든 Microsoft 계정이 표시됩니다. 다른 계정에 연결하려면 **계정 추가**를 선택하고 대화 상자를 따라 하나 이상의 활성 Azure 구독에 연결된 Microsoft 계정으로 로그인합니다.
...
1.	저장소 탐색기(미리 보기)에서 **Azure 저장소에 연결**을 선택합니다.

	![][23]

1.	**Azure 저장소에 연결** 대화 상자에서 계정 키(Azure 포털에서 "키 1" 값)를 지정한 후 **다음**을 선택합니다.
...
1.	**Azure 저장소에 연결** 대화 상자에서 연결 문자열을 지정한 후 **다음**을 선택합니다.
...
1.	**Azure 저장소에 연결** 대화 상자에서 SAS URI를 지정한 후 **다음**을 선택합니다.
