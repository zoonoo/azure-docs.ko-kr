<properties 
   pageTitle="리소스 관리자에서 Azure 포털을 사용하여 내부 부하 분산 장치 만들기 시작 | Microsoft Azure"
...
4. **부하 분산 장치 만들기** 블레이드에서 부하 분산 장치의 **이름**을 입력합니다.
5. **구성표** 아래에서 **내부**를 클릭합니다.
...
6. **IP 주소 할당** 아래에서 부하 분산 장치의 IP 주소를 고정으로 할 것인지 유동으로 할 것인지에 따라 **동적** 또는 **정적**을 클릭합니다.
...
4. **간격** 아래에서 응용 프로그램 검색 빈도를 지정합니다.
5. **비정상 임계값** 아래에서 백 엔드 VM을 비정상으로 표시하기 전에 실패하는 시도 횟수를 지정합니다.
5. **확인**을 클릭하여 프로브를 만듭니다.
...
11. **부동 IP(Direct Server Return)** 아래에서 **사용 안 함** 또는 **사용**을 클릭합니다.
12. **확인**을 클릭합니다.
