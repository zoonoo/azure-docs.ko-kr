...
Azure ILB(내부 부하 분산)는 클라우드 서비스 또는 지역 범위의 가상 네트워크 내부에 있는 가상 컴퓨터 간의 부하 분산을 제공합니다. 지역 범위의 가상 네트워크 사용 및 구성에 대한 자세한 내용은 [지역 가상 네트워크](virtual-networks-migrate-to-regional-vnet.md)를 참조하세요. 선호도 그룹에 대해 구성된 기존 가상 네트워크는 ILB를 사용할 수 없습니다.
...
### 2단계: 내부 부하 분산 인스턴스에 끝점 추가

기존 가상 컴퓨터의 경우 다음 명령을 사용하여 내부 부하 분산 인스턴스에 끝점을 추가할 수 있습니다.
...
### 3단계: 새 내부 부하 분산 끝점으로 트래픽을 전송하도록 서버 구성
...
다음 명령은 PARTNER-DBTIER라는 새 내부 부하 분산 인스턴스를 구성하고 3개의 데이터베이스 서버에 해당하는 가상 컴퓨터에 끝점을 추가합니다.
...
그런 후에 Contoso는 다음 명령을 사용하여 PARTNER-DBTIER 내부 부하 분산 인스턴스의 VIP를 확인했습니다.
...
- 인트라넷 웹 클라이언트는 DNS 이름 legalnet.corp.contoso.com을 사용하여 LOB 서버에 연결합니다.

다음 명령은 LEGAL-ILB라는 내부 부하 분산 인스턴스를 만들고 3개의 LOB 서버에 해당하는 가상 컴퓨터에 끝점을 추가합니다.
...
가상 컴퓨터를 만들 때 내부 부하 분산 인스턴스에 추가하려면 New-AzureInternalLoadBalancerConfig 및 New-AzureVMConfig cmdlet을 사용할 수 있습니다.
...
## 클라우드 서비스에 대한 내부 부하 분산 구성하기


내부 부하 분산은 가상 컴퓨터 및 클라우드 서비스를 모두 지원합니다. 지역 가상 네트워크 외부에 있는 클라우드 서비스에 생성된 내부 부하 분산 끝점은 해당 클라우드 서비스 내에서만 액세스할 수 있습니다.

아래 샘플에 나와 있는 것처럼 클라우드 서비스에서 첫 번째 배포를 만드는 동안 내부 부하 분산 구성을 설정해야 합니다.
...
내부 부하 분산 인스턴스에서 끝점인 가상 컴퓨터를 제거하려면 다음 명령을 사용합니다.
...
클라우드 서비스에서 내부 부하 분산 인스턴스를 제거하려면 다음 명령을 사용합니다.
