...
6.  위에서 이동한 대상 공용 IP의 값을 편집 하려면 먼저 리소스 ID를 가져온 다음 복사 하 여 **\<리소스-그룹-이름 >** 파일에 붙여넣어야 합니다.  ID를 가져오려면 [AzPublicIPAddress](https://docs.microsoft.com/powershell/module/az.network/get-azpublicipaddress?view=azps-2.6.0)을 사용 합니다.
...
7.  **\<리소스-그룹-이름 >** 파일에서 공용 IP 외부 ID에 대 한 두 번째 매개 변수에 **DEFAULTVALUE** 대신 **리소스 ID** 를 붙여 넣어 경로를 따옴표로 묶어야 합니다.
...
12. 또한 선택 하는 경우 템플릿의 다른 매개 변수를 변경할 수 있으며 요구 사항에 따라 선택 사항입니다.
    ...
    * **Sku** - **\<리소스 그룹 이름 >. json** 파일에서 **Sku** > **name** 속성을 변경 하 여 구성의 외부 부하 분산 장치에 대 한 sku를 표준에서 기본 또는 기본 또는 표준으로 변경할 수 있습니다.
    ...
    * **부하 분산 규칙** - **\<리소스 그룹 이름 >.** i n i 파일의 **loadBalancingRules** 섹션에 항목을 추가 하거나 제거 하 여 구성에서 부하 분산 규칙을 추가 하거나 제거할 수 있습니다.
    ...
    * **인바운드 nat 규칙** - **\<리소스 그룹 이름 >. Json** 파일의 **loadbalancer.inboundnatrules** 섹션에 항목을 추가 하거나 제거 하 여 부하 분산 장치에 대 한 인바운드 nat 규칙을 추가 하거나 제거할 수 있습니다.
        ...
        ```
        인바운드 NAT 규칙의 추가 또는 제거를 완료 하려면 **\<리소스 그룹 이름 >. json** 파일의 끝에 **유형** 속성으로 해당 규칙을 설치 하거나 제거 해야 합니다.
    ...
    * **아웃 바운드 규칙** - **\<리소스 그룹 이름 >.** i n i 파일에서 **outboundRules** 속성을 편집 하 여 구성에서 아웃 바운드 규칙을 추가 하거나 제거할 수 있습니다.
