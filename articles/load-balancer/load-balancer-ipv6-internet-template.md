<properties
    pageTitle="템플릿을 사용하여 IPv6로 인터넷 연결 부하 분산 솔루션을 배포합니다 | Microsoft Azure"
...
5. "템플릿 편집"을 클릭합니다. (시작 및 종료{ }를 포함하기 위해) 기존 내용을 삭제하고 템플릿 파일의 전체 내용을 복사/붙여넣기한 다음 "저장"을 클릭합니다.
    ...
    > [AZURE.NOTE] Microsoft Internet Explorer를 사용한다면 붙여넣기할 때 Windows 클립보드에 대한 액세스를 허용하도록 요청하는 대화 상자가 나타납니다. "액세스 허용"을 클릭합니다.
...
8. 리소스 그룹 블레이드에서 6단계 때 지정한 리소스 그룹의 이름을 클릭합니다. 배포된 모든 리소스 목록이 표시됩니다. 모든 것이 순조롭다면 "마지막 배포" 아래에 "Succeeded"이라고 표시되어야 합니다. 그렇지 않다면 사용 중인 계정에 필요한 리소스를 만들 권한이 있는지 확인합니다.
    ...
    > [AZURE.NOTE] 6단계를 마친 후 즉시 리소스 그룹을 찾아보기할 경우 "마지막 배포"는 리소스가 배포되는 동안 "배포" 상태를 표시합니다.
...
4. 각 VM에서 IPv6 또는 IPv4에 연결된 인터넷 장치에 아웃 바운드 연결을 시작합니다. 두 경우 모두 대상 장치에 의해 표시된 원본 IP는 부하 분산 장치의 공용 IPv4 또는 IPv6 주소입니다.
