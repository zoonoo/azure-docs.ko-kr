...
Load Balancer는 원본 IP 선호도 배포 모드를 사용하여 구성할 수도 있습니다. 이 배포 모드는 세션 선호도 또는 클라이언트 IP 선호도라고도 합니다. 모드는 2 튜플(원본 ID 및 대상 IP) 또는 3 튜플(원본 ID, 대상 IP 프로토콜 유형) 해시를 사용하여 사용 가능한 서버에 트래픽을 매핑합니다. 원본 IP 선호도를 사용 하면 동일한 클라이언트 컴퓨터에서 시작 된 연결이 동일한 DIP 끝점으로 이동 합니다.
...
2. 부하 분산 장치 개요 블레이드의 **설정**아래에서 **부하 분산 규칙** 을 클릭 합니다.
3. 부하 분산 규칙 블레이드에서 배포 모드를 변경 하려는 부하 분산 규칙을 클릭 합니다.
