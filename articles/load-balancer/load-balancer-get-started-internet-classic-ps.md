---
title: 인터넷 연결 부하 분산 장치 만들기 - Azure PowerShell 클래식
...
다음 예제에서는 "webfarm"이라고 하는 부하 분산 장치 집합을 클라우드 서비스 "mytestcloud"(또는 myctestcloud.cloudapp.net)에 추가하여, 부하 분산 장치의 엔드포인트를 "web1" 및 "web2"라는 가상 머신에 추가합니다. 부하 분산 장치는 포트 80에서 트래픽을 받고 TCP를 사용하여 로컬 엔드포인트(이 경우 포트 80)에서 정의된 가상 머신들 간의 부하를 분산합니다.

### <a name="step-1"></a>1단계

첫 번째 VM "web1"을 위한 부하 분산 엔드포인트 만들기
...
동일한 부하 분산 장치 집합 이름을 사용하여 두 번째 VM "web2"를 위한 다른 엔드포인트 만들기
...
Remove-AzureEndpoint를 사용하여 부하 분산 장치에서 가상 머신 엔드포인트를 제거할 수 있습니다.
...
애플리케이션이 부하 분산 장치 뒤의 서버에 대한 연결을 유지해야 하는 경우 [부하 분산 장치에 대한 유휴 TCP 시간 제한 설정](load-balancer-tcp-idle-timeout.md)에 대해 자세히 이해할 수 있습니다. Azure Load Balancer를 사용하는 경우 유휴 연결 동작에 대해 알아보는 데 도움이 됩니다.
