...
ms.locfileid: "88038259"
---
# <a name="outbound-only-load-balancer-configuration"></a>아웃 바운드 전용 부하 분산 장치 구성
...
2. 화면 왼쪽 상단에서 리소스 만들기를 선택 하 **> 네트워킹 > 가상 네트워크** 를 선택 하거나 검색 상자에서 **가상 네트워크** 를 검색 합니다.

2. **가상 네트워크 만들기**에서 **기본 사항** 탭에 다음 정보를 입력 하거나 선택 합니다.
...
3. **Ip 주소** 탭을 선택 하거나 페이지 맨 아래에 있는 **다음: ip 주소** 단추를 선택 합니다.

4. **IP 주소** 탭에서 다음 정보를 입력 합니다.

    | 설정            | 값                      |
    |--------------------|----------------------------|
    | IPv4 주소 공간 | **10.1.0.0/16** 입력 |

5. **서브넷 이름**에서 **기본**이라는 단어를 선택 합니다.

6. **서브넷 편집**에서 다음 정보를 입력 합니다.
...
9. **BastionHost**아래에서 **사용**을 선택 합니다. 다음 정보를 입력 합니다.

    | 설정            | 값                      |
    |--------------------|----------------------------|
    | 요새 이름 | **MyBastionHost** 입력 |
    | AzureBastionSubnet 주소 공간 | **10.1.1.0/24** 입력 |
    | 공용 IP 주소 | **새로 만들기**를 선택합니다. </br> **이름**에 **myBastionIP**를 입력 합니다. </br> **확인**을 선택합니다. |


8. **검토 + 만들기** 탭을 선택 하거나 **검토 + 만들기** 단추를 선택 합니다.
...
2. **개요** 페이지에서 **연결**, **방호**를 차례로 선택 합니다.

4. VM을 만드는 동안 입력 한 사용자 이름 및 암호를 입력 합니다.
...
- [Azure 방호](https://docs.microsoft.com/azure/bastion/bastion-overview) 에 대해 알아보기
