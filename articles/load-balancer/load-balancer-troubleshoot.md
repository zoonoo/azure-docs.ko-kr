...
이 페이지에서는 일반적인 Azure Load Balancer 질문에 대한 문제 해결 정보를 제공합니다. 부하 분산 장치 연결을 사용할 수 없을 때 가장 일반적인 증상은 다음과 같습니다. 
- 부하 분산 장치 뒤의 VM이 상태 프로브에 응답하지 않습니다. 
- 부하 분산 장치 뒤의 VM이 구성된 포트의 트래픽에 응답하지 않습니다.

## <a name="symptom-vms-behind-the-load-balancer-are-not-responding-to-health-probes"></a>증상: 부하 분산 장치 뒤의 VM이 상태 프로브에 응답하지 않습니다.
백 엔드 서버가 부하 분산 장치 집합에 참여하려면 프로브 검사를 통과해야 합니다. 상태 프로브에 대한 자세한 내용은 [부하 분산 장치 프로브 이해](load-balancer-custom-probe-overview.md)를 참조하세요. 

부하 분산 장치 백 엔드 풀 VM은 다음 이유 중 하나로 인해 프로브에 응답하지 않을 수 있습니다. 
- 부하 분산 장치 백 엔드 풀 VM이 정상적인 상태가 아닙니다. 
- 부하 분산 장치 백 엔드 풀 VM이 프로브 포트에서 수신하지 않습니다. 
- 방화벽 또는 네트워크 보안 그룹이 부하 분산 장치 백 엔드 풀 VM에서 포트를 차단하고 있습니다. 
- 부하 분산 장치의 기타 구성 오류

### <a name="cause-1-load-balancer-backend-pool-vm-is-unhealthy"></a>원인 1: 부하 분산 장치 백 엔드 풀 VM이 정상적인 상태가 아닙니다. 

**유효성 검사 및 해결**

이 문제를 해결하려면 참여하는 VM에 로그인하고 VM 상태가 정상인지와 풀에 있는 다른 VM의 **PsPing** 또는 **TCPing**에 응답할 수 있는지 확인합니다. VM을 정상이 아니거나 프로브에 응답할 수 없으면 부하 분산에 사용하기 전에 문제를 해결하고 VM을 다시 정상 상태로 복구해야 합니다.

### <a name="cause-2-load-balancer-backend-pool-vm-is-not-listening-on-the-probe-port"></a>원인 2: 부하 분산 장치 백 엔드 풀 VM이 프로브 포트에서 수신하지 않습니다.
...
* 또한 VM 또는 서브넷의 NIC에 대해 LB 프로브 또는 트래픽을 허용하는 기본 규칙보다 우선 순위가 더 높은 **모두 거부** 네트워크 보안 그룹 규칙이 있는지 확인합니다(네트워크 보안 그룹은 168.63.129.16의 부하 분산 장치 IP를 허용해야 함). 
...
* 이러한 ping 테스트에서 응답이 수신되지 않으면 다음을 수행합니다.
    ...
    - 네트워크 캡처를 분석하고 ping 쿼리와 관련해서 들어오고 나가는 패킷이 있는지 확인합니다. 
...
* 프로브 형식을 변경하고(예: HTTP에서 TCP로) 네트워크 보안 그룹 ACL의 해당 포트 및 방화벽이 프로브 응답 구성에 문제가 있는지 평가하도록 구성합니다. 상태 프로브 구성에 대한 자세한 내용은 [끝점 부하 분산 상태 프로브 구성](https://blogs.msdn.microsoft.com/mast/2016/01/26/endpoint-load-balancing-heath-probe-configuration-details/)을 참조하세요.

## <a name="symptom-vms-behind-load-balancer-are-not-responding-to-traffic-on-the-configured-data-port"></a>증상: 부하 분산 장치 뒤의 VM이 구성된 데이터 포트의 트래픽에 응답하지 않습니다.

백 엔드 풀 VM이 정상으로 표시되고 상태 프로브에 응답하지만 여전히 부하 분산에 참여하지 않거나 데이터 트래픽에 응답하지 않을 경우 다음 이유 중 하나 때문일 수 있습니다. 
* 부하 분산 장치 백 엔드 풀 VM이 데이터 포트에서 수신하지 않습니다. 
* 네트워크 보안 그룹이 부하 분산 장치 백 엔드 풀 VM에서 포트를 차단하고 있습니다.  
* 동일한 VM 및 NIC에서 부하 분산 장치에 액세스 
* 참여하는 부하 분산 장치 백 엔드 풀 VM에서 인터넷 부하 분산 장치 VIP에 액세스 

### <a name="cause-1-load-balancer-backend-pool-vm-is-not-listening-on-the-data-port"></a>원인 1: 부하 분산 장치 백 엔드 풀 VM이 데이터 포트에서 수신하지 않습니다. 
...
4. 포트가 Listening으로 표시되는 경우 해당 포트의 대상 응용 프로그램에 문제가 있는지 확인합니다. 

### <a name="cause-2-network-security-group-is-blocking-the-port-on-the-load-balancer-backend-pool-vm"></a>원인 2: 네트워크 보안 그룹이 부하 분산 장치 백 엔드 풀 VM에서 포트를 차단하고 있습니다.  
...
### <a name="cause-3-accessing-the-load-balancer-from-the-same-vm-and-network-interface"></a>원인 3: 동일한 VM 및 네트워크 인터페이스에서 부하 분산 장치에 액세스 

부하 분산 장치의 백 엔드 VM에서 호스트된 응용 프로그램이 동일한 네트워크 인터페이스를 통해 동일한 백엔드 VM에서 호스트되는 다른 응용 프로그램에 액세스하려고 하는 경우 지원되지 않는 시나리오이며 실패합니다. 
...
### <a name="cause-4-accessing-the-internet-load-balancer-vip-from-the-participating-load-balancer-backend-pool-vm"></a>원인 4: 참여하는 부하 분산 장치 백 엔드 풀 VM에서 인터넷 부하 분산 장치 VIP에 액세스

ILB VIP가 VNet 내에 구성되어 있고 참가자 백엔드 VM 중 하나가 인터넷 부하 분산 장치 VIP에 액세스하려고 하면 결과는 실패합니다. 이것은 지원되지 않는 시나리오입니다.
...
지원 사례를 열기로 결정한 경우 더 빠른 해결을 위해 다음 정보를 수집합니다. 단일 백 엔드를 선택하여 다음과 같은 테스트를 수행합니다.
