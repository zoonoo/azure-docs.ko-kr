...
Azure Load Balancer는 Azure portal에서 새 Azure 메트릭을 통해 새로운 다차원 메트릭을 제공 하 고 실시간 정보 얻기 진단에 부하 분산 장치 리소스 도와줍니다. 

다양한 표준 Load Balancer 구성에서는 다음 메트릭을 제공합니다.

| 메트릭 | 리소스 종류 | 설명 | 권장 집계 |
| --- | --- | --- | --- |
| 데이터 경로 가용성 (VIP 가용성)| 공용 부하 분산 장치 | 표준 Load Balancer는 지역 내에서 부하 분산 장치 프런트 엔드로, 마지막으로 VM을 지원하는 SDN 스택으로 데이터 경로를 연속적으로 실행합니다. 정상 인스턴스가 남아 있는 한 측정은 애플리케이션 부하가 분산된 트래픽과 동일한 경로를 따릅니다. 고객이 사용하는 데이터 경로의 유효성도 검사합니다. 측정은 애플리케이션에 표시되지 않으며 다른 작업을 방해하지 않습니다.| 평균 |
...
Azure portal에는 특정 리소스에 대 한 부하 분산 장치 리소스 페이지와 Azure Monitor 페이지에서 사용할 수 있는 메트릭 페이지를 통해 부하 분산 장치 메트릭을 노출 합니다. 

표준 Load Balancer 리소스에 대한 메트릭을 보려면:
1. 메트릭 페이지를 이동 하 고 다음 중 하나를 수행 합니다.
...
표준 Load Balancer 리소스에 대 한 데이터 경로 가용성을 가져오려면:
1. 올바른 부하 분산 장치 리소스를 선택했는지 확인합니다. 
2. 에 **메트릭을** 드롭 다운 목록에서 **데이터 경로 가용성**합니다. 
3. **집계** 드롭다운 목록에서 **Avg**를 선택합니다. 
4. 또한 프런트 엔드 IP 주소 또는 필요한 프런트 엔드 IP 주소를 사용 하 여 차원으로 프런트 엔드 포트 또는 프런트 엔드 포트에 필터를 추가 하 고 선택한 차원에 따라 그룹화 합니다.

![VIP 검색](./media/load-balancer-standard-diagnostics/LBMetrics-VIPProbing.png)

*그림: 부하 분산 장치 프런트 엔드 세부 정보를 검색 합니다.*
...
진단 용도로 사용할 수 있습니다 합니다 [상태 프로브 상태와 함께 데이터 경로 가용성 메트릭을](#vipavailabilityandhealthprobes)합니다.
...
1. 선택 합니다 **상태 프로브** 사용 하 여 메트릭 **Avg** 집계 유형입니다. 
...
![데이터 경로 가용성 및 상태 프로브 메트릭을 결합합니다.](./media/load-balancer-standard-diagnostics/lbmetrics-dipnvipavailability-2bnew.png)

*그림: 데이터 경로 가용성 및 상태 프로브 메트릭을 결합합니다.*

차트에는 다음 정보가 표시됩니다.
- Vm을 호스트 하는 인프라에는 사용할 수 없게 하 고 차트 맨 앞에 0%에서 했습니다. 나중에 인프라를 정상 및 Vm에 연결할 수 되었으며 백 엔드에 둘 이상의 VM을 배치 합니다. 이 정보는 100% 이상 기능인 데이터 경로 가용성 (VIP 가용성)에 대 한 파란색으로 표시 됩니다. 
- 자주색 추적에 의해 표시 된 상태 프로브 상태 (DIP 가용성) 차트 맨 앞에 0%를 합니다. 원으로 그려진된 영역은 상태 프로브 상태 (DIP 가용성)의 상태가 양호 하 고 이때 고객의 배포 상태가 여기서 녹색 강조 표시 된 새 흐름을 수락할 수 있습니다.
