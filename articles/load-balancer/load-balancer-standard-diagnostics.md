---
title: 메트릭, 경고 및 리소스 상태를 사용 하는 Azure 표준 Load Balancer 진단
...
ms.locfileid: "69516261"
---
# <a name="standard-load-balancer-diagnostics-with-metrics-alerts-and-resource-health"></a>메트릭, 경고 및 리소스 상태를 사용 하는 표준 Load Balancer 진단
...
3. 필요에 따라 필요한 필터링 및 그룹화를 구성합니다.
    ...
    *그림: 표준 Load Balancer에 대 한 데이터 경로 가용성 메트릭입니다.*
...
차트에는 다음 정보가 표시됩니다.
- Vm을 호스트 하는 인프라를 사용할 수 없으며 차트 시작 부분에 0%가 있습니다. 나중에 인프라가 정상적으로 작동 하 고 Vm에 연결할 수 있으며 백 엔드에 둘 이상의 VM이 배치 되었습니다. 이 정보는 이후 100% 였던 데이터 경로 가용성 (VIP 가용성)에 대 한 파란색 추적으로 표시 됩니다. 
- 자주색 추적으로 표시 되는 상태 프로브 상태 (DIP 가용성)는 차트의 시작 부분에서 0%입니다. 녹색의 원으로 표시 된 영역에는 상태 프로브 상태 (DIP 가용성)가 정상 상태이 고 고객의 배포에서 새 흐름을 허용할 수 있는 위치가 표시 됩니다.
