...
* 스크립트는 공용 Load Balancer 업그레이드만 지원 합니다. 내부 기본 Load Balancer 업그레이드의 경우 아웃 바운드 연결을 원하지 않는 경우 표준 내부 Load Balancer을 만들고 아웃 바운드 연결이 필요한 경우 표준 내부 Load Balancer 및 표준 공용 Load Balancer를 만듭니다.
...
[PowerShell 갤러리](https://www.powershellgallery.com/packages/AzurePublicLBUpgrade/1.0)에서 마이그레이션 스크립트를 다운로드 합니다.
