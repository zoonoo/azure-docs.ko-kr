...
## Azure 클래식과 ARM(Azure 리소스 관리자)의 Azure 부하 분산 장치 이해

클라우드의 모든 리소스에는 인터넷에서 연결할 수 있는 공용 IP 주소가 필요합니다. Microsoft Azure의 클라우드 인프라는 해당 리소스 내에서 라우팅할 수 없는 IP 주소를 사용하며, 공용 IP 주소와 함께 NAT(네트워크 주소 변환)를 사용하여 인터넷과 통신합니다.

Microsoft Azure와 해당 부하 분산 장치 구현에는 다음 두 가지 배포 모델이 있습니다.

 
### Azure 클래식

Azure 클래식은 Microsoft Azure에서 구현된 첫 번째 배포 모델입니다. 이 모델에서는 공용 IP 주소와 FQDN이 클라우드 서비스에 할당되며, 부하 분산 장치를 사용하기 위해 클라우드 서비스 경계 내에 배포된 가상 컴퓨터를 그룹화할 수 있습니다. 부하 분산 장치는 포트 변환을 수행하며 클라우드 서비스에 공용 IP 주소를 활용하여 네트워크 트래픽의 부하를 분산합니다.

클래식 배포 모델에서 포트 변환은 공용 IP 주소의 공용 할당 포트와 특정 가상 컴퓨터로 트래픽을 보내기 위해 할당된 로컬 포트 간의 일대일 관계인 끝점을 사용하여 수행됩니다.

부하 분산은 부하 분산 장치 집합 끝점을 사용하여 수행됩니다. 이러한 끝점은 공용 IP 주소와 부하 분산된 네트워크 트래픽에 대해 응답하는 집합 내 모든 가상 컴퓨터에 할당된 로컬 포트 간에 일대다 관계입니다.

부하 분산 장치가 이 배포 모델에서 사용하는 공용 IP 주소의 도메인 레이블은 `<cloud service name>.cloudapp.net`입니다.

클래식 배포 모델의 부하 분산 장치에 대한 그래픽 표현은 다음과 같습니다. ![해시 기반 부하 분산 장치](./media/load-balancer-overview/asm-lb.png)

### Azure 리소스 관리자
 
ARM(Azure 리소스 관리자)에는 부하 분산 장치를 만드는 데 클라우드 서비스가 필요 없기 때문에 부하 분산 장치의 개념이 변경됩니다.

ARM에서 공용 IP 주소는 자체 리소스이며 도메인 레이블 또는 DNS 이름에 연결할 수 있습니다. 이 경우 공용 IP는 부하 분산 장치 리소스에 연결되므로 부하 분산 장치 규칙, 즉 인바운드 NAT 규칙에서 공용 IP 주소를 부하 분산된 네트워크 트래픽을 수신하는 리소스의 인터넷 끝점으로 사용합니다.
...
가용성 집합은 부하 분산 장치에 가상 컴퓨터를 추가하는 데 사용되는 그룹화 방법입니다. 가용성 집합은 가상 컴퓨터가 동일한 물리적 하드웨어에 상주하지 않도록 보장하며, 물리적 클라우드 인프라와 관련된 오류가 발생한 경우 부하 분산 장치에 항상 부하 분산된 네트워크 트래픽을 수신하는 가상 컴퓨터가 있도록 합니다.

ARM(Azure 리소스 관리자)의 부하 분산 장치에 대한 그래픽 표현은 다음과 같습니다.

![해시 기반 부하 분산 장치](./media/load-balancer-overview/arm-lb.png)
