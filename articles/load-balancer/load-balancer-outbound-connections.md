...
SNAT 포트는 소진될 수 있는 한정된 리소스입니다. 사용 방법을 이해하는 것이 중요합니다. 단일 대상 IP에 대한 흐름 당 하나의 SNAT 포트를 사용합니다. 동일한 대상 IP 주소 및 포트에 대한 여러 흐름의 경우 각 흐름은 단일 SNAT 포트를 사용합니다. 이렇게 하면 흐름은 동일한 대상 IP 주소 및 포트에 대한 동일한 공용 IP 주소에서 시작하는 경우에 고유합니다. 다른 대상 IP 주소 및 포트에 대한 여러 흐름은 각각 단일 SNAT 포트를 공유합니다. 대상 IP 주소 및 포트를 사용하면 흐름은 고유합니다.
...
동일한 대상 IP 주소 및 포트에 대해 많은 아웃바운드 TCP 또는 UDP 연결을 시작할 것인지 알고 있는 경우 실패하는 아웃바운드 연결을 확인하고, 지원 서비스에서 SNAT 포트가 고갈될 것이라는 알림을 받는 경우 몇 가지 일반적인 완화 옵션을 사용할 수 있습니다.  이러한 옵션을 검토하고 시나리오에 가장 적합한 옵션을 결정합니다.  한 가지 이상의 옵션이 이 시나리오를 관리하는 데 도움이 될 수 있습니다.

### <a name="modify-application-to-reuse-connections"></a>연결을 다시 사용하도록 응용 프로그램 수정 
응용 프로그램에서 연결을 다시 사용하여 SNAT에서 사용되는 사용 후 삭제 포트에 대한 수요를 줄일 수 있습니다.  이러한 효과는 연결 재사용이 기본 옵션인 HTTP/1.1 같은 프로토콜에서 특히 두드러집니다.  또한 해당 전송으로 HTTP를 사용하는 다른 프로토콜(예: REST)도 이점을 얻을 수 있습니다.  각 요청에 대해 개별적인 원자성 TCP 연결보다 다시 사용하는 기능이 항상 더 바람직하며, 보다 효율적이고 유용한 TCP 트랜잭션이 구현됩니다.

### <a name="modify-application-to-use-connection-pooling"></a>연결 풀링을 사용하도록 응용 프로그램 수정
응용 프로그램에서 연결 풀링 체계를 사용할 수 있습니다. 이 경우 요청이 고정된 연결 집합에서 내부적으로 분산됩니다(가능한 경우 각 연결이다 시 사용됨).  이 경우 사용 중인 사용 후 삭제 포트의 수가 제한되고 좀 더 예약 가능한 환경이 구축됩니다.  그러면 단일 연결이 작업에 회신하느라 사용 가능하지 않을 때 여러 동시 작업을 허용하여 요청 처리량을 늘릴 수 있습니다.  연결 풀링은 응용 프로그램을 개발하는 데 사용하는 프레임워크 또는 응용 프로그램에 대한 구성 설정에 이미 존재할 수 있습니다.  이 설정을 연결 다시 사용과 함께 사용할 수 있으며, 여러 요청은 TCP 트랜잭션을 효율적으로 사용하여 대기 시간을 단축하고 리소스 활용량을 높이는 이점을 얻으면서, 동일한 대상 IP 주소 및 포트에 대해 고정된 예측 가능한 포트 수를 사용할 수 있습니다.

### <a name="modify-application-to-use-less-aggressive-retry-logic"></a>덜 적극적인 재시도 논리를 사용하도록 응용 프로그램 수정
SNAT에 사용되는 사용 후 삭제 포트가 고갈되거나 응용 프로그램 오류가 발생하면 지연 및 백오프 논리 없는 적극적인 또는 무차별 대입 재시도로 인해 고갈 상태가 발생하거나 지속됩니다. 덜 적극적인 재시도 논리를 사용하여 사용 후 삭제 포트에 대한 수요를 줄일 수 있습니다.   사용 후 삭제 포트에는 4분의 유휴 시간 제한(조정 불가)이 적용되며 재시도가 너무 적극적이면 자체적으로 고갈 상태를 해소할 기회가 없어집니다.  따라서 디자인할 때 응용 프로그램이 트랜잭션을 어떤 방식으로 얼마나 자주 다시 시도하는지 고려하는 것이 중요합니다.

### <a name="assign-an-instance-level-public-ip-to-each-vm"></a>각 VM에 인스턴스 수준 공용 IP 할당
이 옵션을 사용하면 시나리오가 [VM에 대한 인스턴스 수준 공용 IP](#vm-with-an-instance-level-public-ip-address-with-or-without-load-balancer)로 변경됩니다.  각 VM에 사용되는 공용 IP의 모든 사용 후 삭제 포트를 VM에서 사용할 수 있습니다(공용 IP의 사용 후 삭제 포트가 해당 백 엔드 풀과 연결된 모든 VM에서 공유되는 시나리오와 반대임).  IP 주소의 추가 비용 및 많은 수의 개별 IP 주소를 허용 목록에 포함할 때 나타날 수 있는 잠재적 영향 등이 상충되는 고려 사항일 수 있습니다.
...
사용 가능한 SNAT 포트 수가 직접적으로 연결 수로 반영되지는 않습니다. SNAT 포트가 할당된 시기와 방법 및 [이러한 소모성 리소스를 관리하는 방법](#snatexhaust)에 대한 자세한 내용은 이전 섹션을 참조하세요.
