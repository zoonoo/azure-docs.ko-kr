...
이 섹션에서는 가상 네트워크를 만들고, Load Balancer의 백 엔드 풀에 사용되는 가상 머신 2개를 만든 다음, Load Balancer를 테스트하기 위한 IIS를 가상 머신에 설치합니다.
...
표준 Load Balancer는 백 엔드 풀에서 표준 IP 주소를 사용하는 VM만 지원합니다. 이 섹션에서는 이전에 만들어진 표준 Load Balancer의 백 엔드 풀에 추가된 두 가지 다른 영역(*영역 1* 및 *영역 2*)에 표준 공용 IP 주소를 사용하는 두 개의 VM(*myVM1* 및 *myVM2*)을 만듭니다.
...
1. 다음 단계에 따라 표준 SKU 공용 IP 주소 이름이 *myVM2-ip*이고 **가용성 영역** 설정이 **2**이며 모든 기타 설정이 *myVM1*과 같은 두 번째 VM, *myVM2*를 만듭니다. 
...
앱이 실행되는 3개의 모든 VM에서 Load Balancer가 트래픽을 분산하는 것을 확인하기 위해 웹 브라우저를 강제로 새로 고칠 수 있습니다.
