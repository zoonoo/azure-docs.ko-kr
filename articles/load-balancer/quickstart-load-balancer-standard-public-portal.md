...
공용 IP SKU와 부하 분산 장치 SKU는 일치해야 합니다. 표준 부하 분산 장치의 경우 백 엔드 풀에서 표준 IP 주소가 있는 VM을 사용합니다. 

이 섹션에서는 세 개의 다른 영역(**영역 1**, **영역 2** 및 **영역 3**)에서 표준 공용 IP 주소를 사용하여 세 개의 VM(**myVM1**, **myVM2** 및 **myVM3**)을 만듭니다. 
...
8. 1~6단계를 반복하여 **myVM2** 및 **myVM3**에 IIS 및 업데이트된 iisstart.htm 파일을 설치합니다.
