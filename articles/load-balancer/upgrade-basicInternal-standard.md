...
[PowerShell 갤러리에서](https://www.powershellgallery.com/packages/AzureILBUpgrade/1.0)마이그레이션 스크립트를 다운로드합니다.
...
먼저 스크립트가 기본 내부 부하 밸러서에서 마이그레이션된 정확한 구성으로 새 표준 내부 부하 밸러서를 성공적으로 만들수 있는지 다시 한 번 확인합니다. Azure 포털에서 이를 확인할 수 있습니다.

수동 테스트로 표준 로드 밸러저를 통해 소량의 트래픽을 보내야 합니다.
  
다음은 새로 만든 표준 내부 부하 밸런서의 백 엔드 풀에 VM을 추가하는 방법에 대한 몇 가지 시나리오와 각 통합에 대한 권장 사항입니다.

* **기존 VM을 기존 기본 내부 부하 밸랜서의 백 엔드 풀에서 새로 만든 표준 내부 부하 밸랜서의 백 엔드 풀로 이동합니다.**
...
* **새로 생성된 표준 내부 부하 밸러커의 백 엔드 풀에 추가할 새 VM**만들기.
