...
Azure Load Balancer의 유휴 시간 제한 설정은 4 분에서 30 분입니다. 기본적으로 4 분으로 설정 됩니다. 비활성 기간이 시간 제한 값보다 긴 경우 클라이언트와 클라우드 서비스 간의 TCP 또는 HTTP 세션이 유지되지 않을 수 있습니다.
...
설정은 인바운드 연결에 대해서만 작동 합니다. 연결이 끊어지지 않도록 하려면 유휴 시간 제한 설정 보다 작은 간격으로 TCP 연결 유지를 구성 하거나 유휴 시간 제한 값을 늘립니다. 이러한 시나리오를 지원 하기 위해 구성 가능한 유휴 시간 제한에 대 한 지원이 추가 되었습니다.
