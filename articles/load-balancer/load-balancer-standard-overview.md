...
표준 Load Balancer는 현재 모든 Azure 지역에서 사용할 수 있습니다.
...
- [구독 작업 이동](../azure-resource-manager/management/move-resource-group-and-subscription.md)은 표준 SKU LB 및 PIP 리소스에 대해 지원되지 않습니다.
- Vnet 및 기타 Microsoft 플랫폼 서비스를 사용 하지 않는 웹 작업자 역할은 VNet 서비스 및 기타 플랫폼 서비스의 기능을 사용 하는 방법의 부작용으로 인해 내부 표준 Load Balancer 뒤에 있는 인스턴스에서 액세스할 수 있습니다. 해당 서비스 자체 또는 기본 플랫폼이 예고 업이 변경될 수 있기 때문에 여기에 의존하지 말아야 합니다. 내부 표준 Load Balancer만 사용하는 경우 원하면 명시적으로 [아웃 바운드 연결](load-balancer-outbound-connections.md)을 만들어야 한다고 항상 가정해야 합니다.
...
- [Azure Load Balancer](load-balancer-overview.md)에 대해 자세히 알아보세요.
