...
>[!NOTE]
> Azure는 사용자 시나리오를 위한 완전히 관리되는 부하 분산 솔루션 모음을 제공합니다. TLS (전송 계층 보안) 프로토콜 종료 ("SSL 오프 로드") 또는 HTTP/HTTPS 요청당 응용 프로그램 계층 처리를 찾고 [있는 경우 Azure 애플리케이션 게이트웨이의 정의](../application-gateway/overview.md) 를 참조 하세요. 전역 DNS 부하 분산을 찾고 [있는 경우 Traffic Manager 란?](../traffic-manager/traffic-manager-overview.md) 을 참조 하세요. 엔드투엔드 시나리오에서 이러한 솔루션을 조합하여 이점을 얻을 수 있습니다.
>
> Azure 부하 분산 옵션 비교는 [azure의 부하 분산 옵션 개요](https://docs.microsoft.com/azure/architecture/guide/technology-choices/load-balancing-overview)를 참조 하세요.
...
- [Azure Monitor](../azure-monitor/platform/metrics-supported.md#microsoftnetworkloadbalancers)의 진단과 관련된 [지원되는 다차원 메트릭](../monitoring-and-diagnostics/monitoring-overview.md)에 대해 자세히 알아보세요.
