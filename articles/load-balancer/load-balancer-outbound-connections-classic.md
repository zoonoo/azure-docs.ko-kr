...
| 시나리오 | 방법 | 설명 | 웹 작업자 역할 | IaaS | 
| --- | --- | --- | --- | --- |
| [1. 인스턴스 수준 공용 IP 주소가 있는 VM](#ilpip) | SNAT, 포트 가장 사용 안 함 |Azure에서 공용 IP가 할당된 Virtual Machine을 사용합니다. 인스턴스에 있는 모든 삭제 포트를 사용할 수 있습니다. | 아니요 | 예 |
| [2. 부하 분산된 공용 엔드포인트](#publiclbendpoint) | 공용 엔드포인트에 대한 포트 가장(PAT)을 사용하는 SNAT |Azure에서 공용 IP 주소 공용 엔드포인트를 여러 개인 엔드포인트와 공유하며, PAT에 대한 공용 엔드포인트의 사용 후 삭제 포트를 사용합니다. | 예 | 예 |
| [3. 독립 실행형 VM](#defaultsnat) | 포트를 가장하는(PAT) SNAT | Azure에서 자동으로 SNAT에 대한 공용 IP 주소를 지정하고, 이 공용 IP 주소를 전체 배포와 공유하고, PAT에 대한 공용 엔드포인트의 사용 후 삭제 포트를 사용합니다. 이 시나리오는 이전 시나리오의 대체 시나리오입니다. 가시성 및 제어 기능이 필요한 경우에는 권장되지 않습니다. | 예 | 예|
