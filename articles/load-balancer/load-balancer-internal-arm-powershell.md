<properties
   pageTitle="Azure 리소스 관리자를 사용하여 내부 부하 분산 장치 시작 | Microsoft Azure"
   description="Azure 리소스 관리자에 대한 내부 부하 분산 장치 규칙, NAT 규칙 및 프로브를 만드는 방법을 알아봅니다. 내부 부하 분산 장치(ILB) 리소스를 만드는 종단 간 프로세스를 보여 주는 단계별 지침입니다."
...
이 페이지에서는 내부 부하 분산 장치를 만들기 위해 수행되는 개별 작업의 순서를 알아보고 부하 분산 장치를 만들기 위해 수행해야 하는 작업을 자세히 설명합니다.
