...
4. **검토 + 만들기** 선택
    ...
    > [!IMPORTANT]
    > 이 빠른 시작의 나머지 부분에서는 위의 SKU 선택 프로세스 중에 **표준** sku가 선택 되어 있다고 가정 합니다.
...
3. 다음 값을 사용하여 부하 분산 규칙을 구성합니다.
    ...
    | 암시적 아웃 바운드 규칙 만들기 | **아니오**를 선택합니다. 이후 섹션에서 전용 공용 IP를 사용 하 여 아웃 바운드 규칙을 만듭니다. |
...
3. 다음 값을 사용 하 여 아웃 바운드에 대 한 프런트 엔드 IP 구성을 구성 합니다.
    ...
    | 설정 | 값 |
    | ------- | ----- |
    | 속성 | **LoadBalancerFrontEndOutbound**를 입력 합니다. |
    | IP 버전 | **IPv4**를 선택 합니다. |
