...
>[AZURE.NOTE]기본 설정을 사용하여 여러 가상 컴퓨터 또는 서비스 간에 트래픽의 부하를 분산하도록 구성하면 들어오는 트래픽이 임의로 분산됩니다. 세션 선호도를 찾으려는 경우 [부하 분산 장치 배포 모드](load-balancer-distribution-mode.md)를 확인합니다.
...
인터넷 클라이언트가 클라우드 서비스의 공용 IP 주소 및 TCP 포트 443으로 웹 페이지 요청을 보내면 Azure 부하 분산 장치가 해시를 기반으로 하여 부하 분산 집합에 있는 3개의 가상 컴퓨터에 요청을 분산합니다. 부하 분산 알고리즘에 대한 자세한 내용은 [부하 분산 장치 개요 페이지](load-balancer-overview#load-balancer-features)를 참조하세요.
