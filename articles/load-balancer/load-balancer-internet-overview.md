...
> [!NOTE]
> Azure Load Balancer는 기본 설정을 사용하여 여러 가상 컴퓨터 인스턴스 간에 해시 배포 네트워크 트래픽을 제공합니다(해시 배포에 관한 자세한 내용은 [부하 분산 장치 기능](load-balancer-overview.md#load-balancer-features) 참조). 세션 선호도를 찾으려는 경우 [부하 분산 장치 배포 모드](load-balancer-distribution-mode.md)를 확인합니다.
> 
> 
