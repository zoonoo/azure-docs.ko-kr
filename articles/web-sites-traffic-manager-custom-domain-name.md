<properties title="Learn how to configure an Azure website that uses Traffic Manager to use a custom domain name" pageTitle="트래픽 관리자를 사용하는 Azure 웹 사이트의 사용자 지정 도메인 이름 구성" metaKeywords="Microsoft Azure, Microsoft Azure 웹 사이트, 도메인 이름" description="" services="web-sites" documentationCenter="" authors="larryfr, jroth" manager="wpickett" />
...
이 문서에서는 부하 분산을 위해 트래픽 관리자를 사용하는 Azure 웹 사이트에서 사용자 지정 도메인 이름을 사용하는 일반 지침을 제공합니다. 이 문서의 맨 위에 있는 탭에 도메인 등록 기관이 나열되는지 확인하세요. 나열될 경우 등록 기관별 단계를 보려면 해당 탭을 선택하세요.
...
사용자 지정 도메인을 Azure 웹 사이트에 연결하려면 도메인 이름을 구입한 도메인 등록 기관에서 제공하는 도구를 사용하여 DNS 테이블에 사용자 지정 도메인에 대한 새 항목을 추가해야 합니다. DNS 도구를 찾아 사용하려면 다음 단계를 따르세요.
...
5. 각 등록 기관에 대한 구체적인 설명은 다르지만 일반적으로 사용자 지정 도메인 이름(예: **contoso.com**,)*에서* Azure 웹 사이트에 사용하는 트래픽 관리자 도메인 이름(**contoso.trafficmanager.net**)*으로* 매핑합니다.
