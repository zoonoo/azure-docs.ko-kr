...
**저장소 큐** 는 [Azure Storage](https://azure.microsoft.com/services/storage/) 인프라의 일부입니다. 이를 통해 많은 수의 메시지를 저장할 수 있습니다. 전 세계 어디서나 인증된 호출을 통해 HTTP 또는 HTTPS를 사용하여 메시지에 액세스할 수 있습니다. 큐 메시지의 크기는 최대 64KB입니다. 큐는 스토리지 계정의 용량 제한에 도달할 때까지 수백만 개의 메시지를 포함할 수 있습니다. 큐는 비동기적으로 처리할 작업의 백로그를 만드는 데 일반적으로 사용됩니다. 자세한 내용은 [Azure Storage 큐 란?](../storage/queues/storage-queues-introduction.md)을 참조 하세요.

**Service Bus 큐** 는 큐, 게시/구독 및 고급 통합 패턴을 지 원하는 광범위 한 [Azure 메시징](https://azure.microsoft.com/services/service-bus/) 인프라의 일부입니다. 여러 통신 프로토콜, 데이터 계약, 트러스트 도메인 또는 네트워크 환경에 걸쳐 있을 수 있는 응용 프로그램 또는 응용 프로그램 구성 요소를 통합 하도록 설계 되었습니다. Service Bus 큐/항목/구독에 대 한 자세한 내용은 [Service Bus 큐, 토픽 및 구독](service-bus-queues-topics-subscriptions.md)을 참조 하세요.


## <a name="technology-selection-considerations"></a>기술 선택 시 고려 사항
저장소 큐와 Service Bus 큐에는 약간 다른 기능 집합이 있습니다. 특정 솔루션의 요구 사항에 따라 둘 중 하나 또는 둘 다를 선택할 수 있습니다.

지정 된 솔루션의 목적에 맞는 큐 기술을 결정할 때 솔루션 설계자와 개발자는 이러한 권장 사항을 고려해 야 합니다. 

### <a name="consider-using-storage-queues"></a>저장소 큐 사용 고려
솔루션 설계자/개발자라면 다음과 같은 경우 **Storage 큐의 사용을 고려해야** 합니다.

* 응용 프로그램은 큐에 80 기가바이트 이상의 메시지를 저장 해야 합니다.
* 응용 프로그램에서 큐의 메시지 처리에 대 한 진행률을 추적 하려고 합니다. 메시지를 처리 하는 작업자의 작동이 중단 되는 경우에 유용 합니다. 그러면 다른 작업자는이 정보를 사용 하 여 이전 작업자의 남은 위치를 계속 진행할 수 있습니다.
...
* 솔루션은 큐를 폴링할 필요 없이 메시지를 받아야 합니다. Service Bus를 사용 하면 Service Bus에서 지 원하는 TCP 기반 프로토콜을 사용 하 여 긴 폴링 수신 작업을 수행할 수 있습니다.
* 솔루션에서 큐가 보장된 FIFO(선입선출) 순차적 전달을 제공해야 하는 경우.
* 솔루션에서 자동 중복 검색을 지원 해야 합니다.
...
* 큐 기반 지점 간 통신에서 게시-구독 메시징 패턴으로의 최종 마이그레이션을 구상 합니다. 이 패턴은 추가 수신자 (구독자)를 통합할 수 있도록 합니다. 각 수신자는 큐에 전송 된 일부 또는 모든 메시지의 독립적인 복사본을 받습니다. 
* 메시징 솔루션은 추가 인프라 구성 요소를 빌드하지 않고도 "최대 한 번" 배달 보증을 지원 해야 합니다.
* 솔루션은 메시지 일괄 처리를 게시 하 고 사용 해야 합니다.

## <a name="compare-storage-queues-and-service-bus-queues"></a>저장소 큐 및 Service Bus 큐 비교
다음 섹션의 표에서는 큐 기능의 논리적 그룹화를 제공 합니다. 이를 통해 Azure Storage 큐와 Service Bus 큐 모두에서 사용할 수 있는 기능을 한눈에 비교할 수 있습니다.
...
| --- | --- | --- |
| 순서 보장 |**아니요** <br/><br>자세한 내용은 [추가 정보](#additional-information) 섹션의 첫 번째 참고를 참조 하세요.</br> | **예 - 선입선출(FIFO)**<br/><br>( [메시지 세션](message-sessions.md)사용) |
| 전달 보장 |**최소 1회(At-Least-Once)** |PeekLock 수신 모드를 사용 하 여 **최소 한 번** 입니다. 기본값) <br/><br/>**한 번** (ReceiveAndDelete 수신 모드 사용) <br/> <br/> 다양 한 [수신 모드](service-bus-queues-topics-subscriptions.md#receive-modes) 에 대해 자세히 알아보기  |
| 원자성 작업 지원 |**아니요** |**예**<br/><br/> |
| 수신 동작 |**비 차단**<br/><br/>(새 메시지가 없을 경우 즉시 완료) |**제한 시간을 사용 하거나 사용 하지 않고 차단**<br/><br/>(장기 폴링 또는 ["Comet 기술"](https://go.microsoft.com/fwlink/?LinkId=613759) 제공)<br/><br/>**비 차단**<br/><br/>(.NET 관리 API만을 사용하여) |
...
* 저장소 큐의 메시지는 일반적으로 선입 first (선입 first out) 이지만 순서가 잘못 된 경우도 있습니다. 예를 들어 메시지를 처리 하는 동안 클라이언트 응용 프로그램에서 충돌이 발생 하 여 메시지의 표시 제한 시간이 만료 되는 경우입니다. 가시성 시간 제한이 만료되면 다른 작업자가 큐에서 제거할 수 있도록 메시지가 큐에 다시 표시됩니다. 이 시점에서 새로 표시 되는 메시지를 큐에 배치 하 여 다시 큐에서 제거할 수 있습니다.
* Service Bus 큐의 보장된 FIFO 패턴에는 메시징 세션을 사용해야 합니다. **Peek & 잠금** 모드에서 받은 메시지를 처리 하는 동안 응용 프로그램이 충돌 하는 경우 다음에 큐 수신자가 메시징 세션을 수락 하면 메시지의 ttl (time-to-live) 기간이 만료 된 후 실패 한 메시지로 시작 됩니다.
* 저장소 큐는 다음과 같은 표준 큐 시나리오를 지원 하도록 설계 되었습니다.
    - 응용 프로그램 구성 요소를 분리 하 여 확장성 및 오류 허용 오차 향상
    - 부하 평준화
    - 프로세스 워크플로 작성
* 세션 상태를 사용 하 여 세션의 메시지 시퀀스를 처리 하는 과정을 기준으로 응용 프로그램의 상태를 저장 하 고, 정착 받은 메시지 및 세션 상태를 업데이트 하는 트랜잭션을 사용 하 여 세션 상태를 저장 하면 Service Bus 세션의 컨텍스트에서 메시지 처리와 관련 된 불일치를 방지할 수 있습니다. 이러한 종류의 일관성 기능은 다른 공급 업체의 제품에서 *정확히 한 번만* 레이블이 지정 되는 경우도 있습니다. 모든 트랜잭션 오류로 인해 메시지의 다시 배달가 정확 하지 않을 수 있습니다.
...
* Storage 큐는 메시지에 대한 임대를 연장할 수 있는 기능이 포함된 임대 기능을 제공합니다. 이 기능을 사용 하면 작업자 프로세스에서 메시지에 대 한 짧은 임대를 유지할 수 있습니다. 따라서 작업자의 작동이 중단 되 면 다른 작업자가 메시지를 신속 하 게 처리할 수 있습니다. 또한 작업자는 현재 임대 시간 보다 더 오래 처리 해야 하는 경우 메시지에 대 한 임대를 연장할 수 있습니다.
* Storage 큐는 메시지를 큐에 삽입하거나 큐에서 제거할 때 설정할 수 있는 가시성 시간 제한을 제공합니다. 또한 런타임에 다른 임대 값으로 메시지를 업데이트 하 고 동일한 큐의 여러 메시지에서 다른 값을 업데이트할 수 있습니다. Service Bus 잠금 시간 제한은 큐 메타 데이터에 정의 됩니다. 그러나 [RenewLock](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.renewlock#Microsoft_ServiceBus_Messaging_BrokeredMessage_RenewLock) 메서드를 호출 하 여 잠금을 갱신할 수 있습니다.
...
* 저장소 큐의 200-TB 상한 (계정을 가상화 하는 경우 더 많이) 및 무제한 큐와 같은 기능을 사용 하면 SaaS 공급자에 게 이상적인 플랫폼입니다.
...
| 전체 업데이트 |**예** |**예** |
| 서버 측 트랜잭션 로그 |**예** |**아니요** |
| Storage 메트릭 |**예**<br/><br/>**분 메트릭은** 가용성, TPS, API 호출 개수, 오류 수 등에 대 한 실시간 메트릭을 제공 합니다. 모두 실시간으로 분 단위로 집계 되 고 프로덕션에서 발생 한 것부터 몇 분 내에 보고 됩니다. 자세한 내용은 [스토리지 분석 메트릭 정보](/rest/api/storageservices/fileservices/About-Storage-Analytics-Metrics)를 참조하세요. |**예**<br/><br/>([GetQueues](/dotnet/api/microsoft.servicebus.namespacemanager.getqueues#Microsoft_ServiceBus_NamespaceManager_GetQueues)를 호출하여 대량 쿼리) |
...
* 큐 자동 전달을 사용 하면 수천 개의 큐에서 메시지를 수신 응용 프로그램이 메시지를 사용 하는 단일 큐로 autoforward 수 있습니다. 이 메커니즘을 사용하여 보안, 흐름 제어를 달성하고 각 메시지 게시자 사이에 스토리지를 격리할 수 있습니다.
...
* Service Bus 큐는 [배달 못](service-bus-dead-letter-queues.md)한 편지를 지원 합니다. 다음 조건을 충족 하는 메시지를 격리 하는 데 유용할 수 있습니다.
    - 수신 응용 프로그램에서 메시지를 성공적으로 처리할 수 없습니다. 
    - 만료 된 TTL (time-to-live) 속성으로 인해 메시지의 대상에 도달할 수 없습니다. TTL 값은 메시지가 큐에 남아 있는 기간을 지정합니다. Service Bus의 경우 TTL 기간이 만료되면 메시지가 $DeadLetterQueue라는 특별한 큐로 이동됩니다.
...
* 저장소 큐를 사용 하면 큐에 대해 실행 되는 모든 트랜잭션의 상세 로그와 집계 된 메트릭을 가져올 수 있습니다. 이 옵션은 모두 디버깅과 애플리케이션이 Storage 큐를 어떻게 사용하는지 이해하는 데 유용합니다. 응용 프로그램을 성능 조정 하 고 큐 사용 비용을 절감 하는 데에도 유용 합니다.
* Service Bus에서 지 원하는 메시지 세션을 사용 하면 논리 그룹에 속한 메시지를 받는 사람에 게 연결할 수 있습니다. 메시지와 각 수신자 간에 세션 유사 선호도를 만듭니다. 메시지에서 [SessionID](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.sessionid#Microsoft_ServiceBus_Messaging_BrokeredMessage_SessionId) 속성을 설정하면 Service Bus의 이 고급 기능을 사용할 수 있습니다. 수신자가 특정 세션 ID를 수신 대기하고 지정된 세션 식별자를 공유하는 메시지를 수신할 수 있습니다.
* Service Bus 큐의 중복 검색 기능은 [MessageId](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.messageid#Microsoft_ServiceBus_Messaging_BrokeredMessage_MessageId) 속성의 값을 기준으로 큐 또는 토픽으로 전송 된 중복 메시지를 자동으로 제거 합니다.
...
* 파티션은 [프리미엄 계층](service-bus-premium-messaging.md)에서 지원 되지 않습니다. 표준 메시징 계층에서는 1 (기본값), 2, 3, 4 또는 5gb 크기로 Service Bus 큐 및 항목을 만들 수 있습니다. 분할을 사용 하도록 설정 하면 Service Bus는 엔터티의 16 개 복사본 (16 개 파티션)을 만듭니다. 이때 각각 동일한 크기가 지정 됩니다. 따라서 크기가 5gb 인 큐를 만들 경우 16 개의 파티션과 최대 큐 크기는 (5 * 16) = 80 GB가 됩니다.  [Azure Portal][Azure portal]에서 분할 된 큐 또는 토픽의 최대 크기를 확인할 수 있습니다.
* 저장소 큐를 사용 하는 경우 메시지의 내용이 XML 안전 하지 않은 경우 **Base64** 인코딩 되어야 합니다. 메시지를 **Base64** 로 인코딩하면 사용자 페이로드가 64KB가 아니라 최대 48KB가 될 수 있습니다.
* Service Bus 큐의 경우 큐에 저장된 각 메시지가 헤더와 본문의 두 부분으로 구성됩니다. 메시지의 총 크기는 서비스 계층에서 지 원하는 최대 메시지 크기를 초과할 수 없습니다.
* 클라이언트가 TCP 프로토콜을 통해 Service Bus와 통신할 때, 단일 Service Bus 큐에 대한 최대 동시 연결 수가 100으로 제한됩니다. 이 숫자는 보낸 사람과 받는 사람 사이에 공유됩니다. 이 할당량에 도달 하면 추가 연결에 대 한 요청이 거부 되 고 호출 코드에서 예외를 수신 합니다. 이 제한은 REST 기반 API를 사용 하 여 큐에 연결 하는 클라이언트에는 적용 되지 않습니다.
...
### <a name="additional-information"></a>추가 정보
* 각 큐 기술에 대한 모든 요청은 인증되어야 합니다. 익명 액세스 권한이 있는 공개 큐는 지원 되지 않습니다. [SAS](service-bus-sas.md)를 사용하면 쓰기 전용 SAS, 읽기 전용 SAS 또는 모든 권한 SAS를 게시하여 이러한 시나리오에 대응할 수 있습니다.
* 저장소 큐에서 제공 하는 인증 체계에는 대칭 키를 사용 해야 합니다. 이 키는 s h a-256 알고리즘을 사용 하 여 계산 되 고 **Base64** 문자열로 인코딩된 HMAC (해시 기반 MAC(메시지 인증 코드))입니다. 각 프로토콜에 대한 자세한 내용은 [Azure Storage 서비스에 대한 인증](/rest/api/storageservices/fileservices/Authentication-for-the-Azure-Storage-Services)을 참조하세요. Service Bus 큐도 대칭 키를 사용하는 유사한 모델을 지원합니다. 자세한 내용은 [Service Bus에서 공유 액세스 서명 인증](service-bus-sas.md)을 참조하세요.

## <a name="conclusion"></a>결론
두 기술을 보다 심층적으로 이해 하면 사용할 큐 기술과 시기를 보다 적절 하 게 결정할 수 있습니다. Storage 큐 또는 Service Bus 기술을 어느 경우에 사용할 것인지에 대한 결정은 분명히 다양한 요인에 따라 달라집니다. 이러한 요인은 사용 중인 애플리케이션 및 아키텍처의 개별적 요구 사항에 따라 크게 달라질 수 있습니다. 

다음과 같은 이유로 저장소 큐를 선택 하는 것이 좋습니다.

- 응용 프로그램에서 Microsoft Azure의 핵심 기능을 이미 사용 하 고 있는 경우 
- 서비스 간 기본 통신 및 메시징이 필요한 경우 
- 80 GB 보다 클 수 있는 큐가 필요 합니다.

Service Bus 큐는 다음과 같은 다양 한 고급 기능을 제공 합니다. 따라서 하이브리드 응용 프로그램을 빌드하거나 응용 프로그램에서 이러한 기능을 필요로 하는 경우에는이 옵션을 선택 하는 것이 좋습니다.

- [세션](message-sessions.md)
- [트랜잭션](service-bus-transactions.md)
- [중복 검색](duplicate-detection.md)
- [자동 배달 못 한 편지](service-bus-dead-letter-queues.md)
- [지 속성 게시 및 구독 기능](service-bus-queues-topics-subscriptions.md#topics-and-subscriptions) 
