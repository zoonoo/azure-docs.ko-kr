...
3. 기본 네임스페이스와 보조 네임스페이스 간에 페어링을 만들어 ***별칭***을 가져옵니다.
    ...
    >[!NOTE] 
    > [Azure Service Bus 표준 네임 스페이스를 Azure Service Bus Premium으로 마이그레이션한](service-bus-migrate-standard-premium.md)경우 기존 별칭 (예: Service Bus 표준 네임 스페이스 연결 문자열)을 사용 하 여 **PS/CLI** 또는 **REST API**를 통해 재해 복구 구성을 만들어야 합니다.
    >
    >
    > 이는 마이그레이션 중에 Azure Service Bus 표준 네임 스페이스 연결 문자열/n e t Azure Service Bus Premium 네임 스페이스에 대 한 별칭이 되기 때문입니다.
    >
    > 클라이언트 응용 프로그램은이 별칭을 사용 하 여 (예: 표준 네임 스페이스 연결 문자열 Azure Service Bus) 재해 복구 페어링이 설정 된 프리미엄 네임 스페이스에 연결 해야 합니다.
    >
    > 포털을 사용 하 여 재해 복구 구성을 설정 하는 경우 포털은 사용자 로부터이 주의를 추상화 합니다.
