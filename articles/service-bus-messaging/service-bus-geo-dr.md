...
3. 기본 네임스페이스와 보조 네임스페이스 간에 페어링을 만들어 ***별칭***을 가져옵니다.
    ...
    >[!NOTE] 
    > [Azure Service Bus 표준 네임스페이스를 Azure Service Bus Premium으로 마이그레이션한](service-bus-migrate-standard-premium.md)경우 기존 별칭(예: 서비스 버스 표준 네임스페이스 연결 문자열)을 사용하여 **PS/CLI** 또는 **REST API를**통해 재해 복구 구성을 만들어야 합니다.
    >
    >
    > 마이그레이션 하는 동안 Azure Service Bus 표준 네임스페이스 연결 문자열/DNS 이름 자체가 Azure Service Bus Premium 네임스페이스의 별칭이 되기 때문입니다.
    >
    > 클라이언트 응용 프로그램은 이 별칭(예: Azure Service Bus Standard 네임스페이스 연결 문자열)을 사용하여 재해 복구 페어링이 설정된 프리미엄 네임스페이스에 연결해야 합니다.
    >
    > 포털을 사용하여 재해 복구 구성을 설정하는 경우 포털은 이 주의 사항을 추상화합니다.
