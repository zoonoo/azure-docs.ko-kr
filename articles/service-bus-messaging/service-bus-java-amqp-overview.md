---
title: "Java가 포함된 Service Bus AMQP 개요 | Microsoft Docs"
...
Azure Service Bus 클라우드 서비스와 [Windows Server용 Service Bus(Service Bus 1.1)](https://msdn.microsoft.com/library/dn282144.aspx)는 둘 다 AMQP(Advanced Message Queueing Protocol) 1.0을 지원합니다. AMQP를 사용하여 여러 플랫폼 간에 개방형 표준 프로토콜을 사용하는 하이브리드 응용 프로그램을 빌드할 수 있습니다. 다른 언어 및 프레임워크로 빌드된 구성 요소를 사용하며 다른 운영 체제에서 실행되는 응용 프로그램을 생성할 수 있습니다. 이러한 구성 요소는 모두 서비스 버스에 연결할 수 있으며 구조화된 비즈니스 메시지를 효율적이고 완벽하며 원활하게 교환할 수 있습니다.
...
Service Bus로 이러한 라이브러리를 가져오고 사용하는 방법에 대한 자세한 내용은 [Service Bus AMQP 개발자 가이드][Service Bus AMQP 개발자 가이드]를 참조하세요. 자세한 내용으로 연결되는 [다음 단계](service-bus-java-amqp-overview.md#next-steps) 섹션을 참조하세요.
...
* [Service Bus AMQP 개발자 가이드][Service Bus AMQP 개발자 가이드]
* [OASIS AMQP(고급 메시지 큐 프로토콜) 버전 1.0 사양](http://docs.oasis-open.org/amqp/core/v1.0/os/amqp-core-complete-v1.0-os.pdf)

[0]: ./media/service-bus-java-amqp-overview/Example1.png
[Service Bus AMQP 개발자 가이드]: service-bus-amqp-dotnet.md
