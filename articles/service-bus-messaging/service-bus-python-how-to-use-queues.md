...
description: '빠른 시작: Python에서 Azure Service Bus 큐를 사용하는 방법에 대해 알아봅니다.'
...
2. [Azure Portal을 사용하여 Service Bus 큐 만들기](service-bus-quickstart-portal.md) 문서의 단계를 따릅니다.
    ...
    3. **연결 문자열**을 가져옵니다. 
        ...
        > [!NOTE]
        > 이 자습서에서는 Python을 사용하여 Service Bus 네임스페이스에 **큐**를 만들 것입니다. 
1. Python 또는 [Python Azure Service Bus 패키지][Python Azure Service Bus package]를 설치합니다. 자세한 내용은 [Python 설치 가이드](/azure/python/python-sdk-azure-install)를 참조하세요. Service Bus Python SDK에 대한 전체 설명서는 [여기](/python/api/overview/azure/servicebus?view=azure-python)를 참조하세요.

## <a name="create-a-queue"></a>큐 만들기
**ServiceBusClient** 개체를 사용하면 큐로 작업할 수 있습니다. 프로그래밍 방식으로 Service Bus에 액세스하려는 Python 파일의 맨 위쪽에 다음 코드를 추가합니다.

```python
from azure.servicebus import ServiceBusClient
```

다음은 **ServiceBusClient** 개체를 만드는 코드입니다. `<CONNECTION STRING>`을 servicebus connectionstring으로 바꿉니다.

```python
sb_client = ServiceBusClient.from_connection_string('<CONNECTION STRING>')
```

SAS 키 이름과 값은 [Azure Portal][Azure portal] 연결 정보 또는 Visual Studio **속성** 창(이전 섹션에서 보여드린 것처럼 서버 탐색기에서 Service Bus 네임스페이스 선택)에서 확인할 수 있습니다.
...
자세한 내용은 [Azure Service Bus Python 설명서](/python/api/overview/azure/servicebus?view=azure-python)를 참조하세요.

## <a name="send-messages-to-a-queue"></a>큐에 메시지 보내기
Service Bus 큐에 메시지를 보내기 위해 애플리케이션은 `ServiceBusClient` 개체에 대한 `send` 메서드를 호출합니다.
...
`ServiceBusService` 개체의 `get_receiver` 메서드를 사용하여 큐에서 메시지가 수신됩니다.
