...
여기서는 일반 루프로 표시된 것처럼 응용 프로그램이 갑자기 대량의 메시지를 생성하며 다음 메시지를 송신하기 전에 각 송신 작업이 완료될 때까지 대기해야 할 경우, 동기 또는 비동기 API 둘 다 비슷하게 10개 메시지 송신은 확인을 위해 10번의 순차적 전체 왕복이 수행된 후에만 완료됩니다.
