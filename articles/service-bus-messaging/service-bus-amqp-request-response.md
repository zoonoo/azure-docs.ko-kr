---
title: Azure 서비스 버스에서 AMQP 1.0 요청/응답 작업
description: 이 문서에서는 Microsoft Azure 서비스 버스에서 AMQP 요청/응답 기반 작업 목록을 정의합니다.
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:renew-lock`|  
...
|statusDescription|문자열|예|상태에 대한 설명입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:peek-message`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
  
요청 메시지 본문은 다음 항목이 포함된 **맵을** 포함하는 **amqp 값** 섹션으로 구성되어야 합니다.  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|statusCode|int|yes|HTTP 응답 코드 [RFC2616]<br /><br /> 200: OK – 더 많은 메시지가 있음<br /><br /> 204: 콘텐츠 없음 – 더 이상 메시지가 없음|  
|statusDescription|문자열|예|상태에 대한 설명입니다.|  
  
응답 메시지 본문은 다음 항목이 포함된 **맵을** 포함하는 **amqp 값** 섹션으로 구성되어야 합니다.  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:schedule-message`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|session-id|문자열|예|`amqpMessage.Properties.GroupId as string`|  
|파티션 키|문자열|예|`amqpMessage.MessageAnnotations.”x-opt-partition-key"`|
|via-partition-key|문자열|예|`amqpMessage.MessageAnnotations."x-opt-via-partition-key"`|
|message|바이트 배열|yes|AMQP 1.0 실시간 인코딩된 메시지입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:cancel-scheduled-message`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:renew-session-lock`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:set-session-state`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:get-session-state`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:get-message-sessions`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:add-rule`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
**규칙 설명** 맵에는 **SQL 필터와** 상관 관계 **필터가** 상호 배타적인 다음 항목이 포함되어야 합니다.  
...
**상관 관계 필터** 맵에는 다음 항목 중 하나 이상이 포함되어야 합니다.  
  
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|correlation-id|문자열|예||  
|message-id|문자열|예||  
|to|문자열|예||  
|reply-to|문자열|예||  
|label|문자열|예||  
|session-id|문자열|예||  
|reply-to-session-id|문자열|예||  
|content-type|문자열|예||  
...
**sql-rule-action** 맵에는 다음 항목이 포함되어야 합니다.  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:remove-rule`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:enumerate-rules`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:receive-by-sequence-number`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|receiver-settle-mode|ubyte|yes|AMQP 코어 v1.0에 지정된 **수신기 정착** 모드.|  
...
|Key|값 형식|필수|값 내용|  
|---------|----------------|--------------|--------------------|  
|operation(작업)|문자열|yes|`com.microsoft:update-disposition`|  
|`com.microsoft:server-timeout`|uint|예|작업 서버 제한 시간(밀리초)입니다.|  
...
|deadletter-reason|문자열|예|처리 상태가 **일시 중단됨**으로 설정된 경우 설정할 수 있습니다.|  
|deadletter-description|문자열|예|처리 상태가 **일시 중단됨**으로 설정된 경우 설정할 수 있습니다.|  
