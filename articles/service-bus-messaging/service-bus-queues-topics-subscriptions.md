...
Service Bus의 조정된 메시징 기능의 핵심이 되는 메시징 엔터티는 큐, 토픽/구독, 규칙/동작 및 Event Hubs입니다.
...
Event Hubs는 메시지 스트리밍 구문이고 큐 및 토픽과 유사해 보이지만 매우 다양한 특성이 있습니다. 예를 들어 전통적인 조정된 메시징 기능이 스트리밍 기능이 아니기에 이벤트 허브는 메시지 TTL, 효력 상실, 트랜잭션 또는 승인을 제공하지 않습니다. 이벤트 허브는 순서 및 스트림 재생을 분할, 유지하는 것과 같이 다른 스트림 관련 기능을 제공합니다.
