...
JMS의 큐는 위에서 설명한 기존의 Service Bus 큐와 의미상 유사 합니다.
...
JMS의 토픽은 위에서 설명한 기존 Service Bus 항목과 의미상 비교할 수 있습니다.
...
이러한 항목은 위에서 설명한 구독과 의미상 유사 하며 (즉, 항목에 존재 하 고 게시/구독 의미 **체계를 사용**하도록 설정) Java 메시지 서비스 사양은 지정 된 구독에 대 한 **공유, 공유** **되지 않은** **내구성이** 있는 속성의 개념을 소개 합니다.
...
공유 되지 않는 비영구 구독을 만들려면 `JMSConsumer` ' JMSContext 클래스에서 아래 메서드와 같이를 만듭니다. 
