...
6. **일정** 에서 조건에 대 한 시작 날짜와 종료 날짜를 지정 하거나, 특정 요일 (월요일, 화요일 등)을 주 및 시간 중에서 선택 합니다. 
    ...
    자동 크기 조정 설정의 작동 방식, 특히 프로필 또는 조건을 선택 하 고 여러 규칙을 평가 하는 방법에 대해 자세히 알아보려면 [자동 크기 조정 설정 이해](../azure-monitor/autoscale/autoscale-understanding-settings.md)를 참조 하세요.          

    > [!NOTE]
    > - 자동 크기 조정에 대 한 결정을 내릴 때 검토 하는 메트릭은 5-10 분 정도 걸릴 수 있습니다. 급증 워크 로드를 처리 하는 경우 급증 워크 로드를 처리 하는 데 충분 한 메시징 단위가 있도록 규모를 축소 하는 데 더 오랜 기간 (> 10 분)을 확장 하는 기간을 짧게 설정 하는 것이 좋습니다. 
    > 
    > - 용량이 부족 하 여 오류가 표시 되는 경우 (사용할 수 있는 메시징 단위가 없음) microsoft로 지원 티켓을 제기 합니다.  
