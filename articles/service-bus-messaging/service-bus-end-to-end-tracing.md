...
마이크로 서비스 개발의 일반적인 문제 중 하나는 처리에 관련 된 모든 서비스를 통해 클라이언트에서 작업을 추적 하는 기능입니다. 디버깅, 성능 분석, A/B 테스트 및 기타 일반적인 진단 시나리오에 유용합니다.
...
# <a name="azuremessagingservicebus-sdk-latest"></a>[ServiceBus SDK (최신 버전)](#tab/net-standard-sdk-2)
| 속성 이름        | 설명                                                 |
|----------------------|-------------------------------------------------------------|
...
`ServiceBusProcessor` [.Net 용 Azure Messaging Service Bus 클라이언트](/dotnet/api/azure.messaging.servicebus.servicebusprocessor) 클래스는 추적 시스템 또는 클라이언트 코드 부분으로 후크 될 수 있는 추적 계측 지점이 제공 됩니다. 계측을 사용하면 클라이언트 쪽에서 모든 Service Bus 메시징 서비스 호출을 추적할 수 있습니다. (메시지 처리기 패턴)를 사용 [ `ProcessMessageAsync` `ServiceBusProcessor` 하 여](/dotnet/api/azure.messaging.servicebus.servicebusprocessor.processmessageasync) 메시지 처리를 수행 하는 경우 메시지 처리도 계측 됩니다.
...
(메시지 처리기 패턴) [를 사용 `ProcessMessageAsync` `ServiceBusProcessor` 하 여 메시지](/dotnet/api/azure.messaging.servicebus.servicebusprocessor.processmessageasync) 를 처리 하는 경우 메시지 처리도 계측 됩니다. 서비스에서 수행 하는 모든 Service Bus 호출은 자동으로 추적 되 고 다른 원격 분석 항목과 상관 관계가 지정 됩니다. 또는 수동 메시지 처리 추적은 다음 예제를 참조하세요.
...
이 예에서는 처리 된 각 메시지에 대 한 요청 원격 분석이 보고 되며 타임 스탬프, 기간 및 결과 (성공)가 있습니다. 원격 분석에는 상관관계 속성 집합도 있습니다. 또한 메시지 처리 중에 보고된 중첩된 추적 및 예외에 `RequestTelemetry`의 ‘하위’로 표시되는 상관관계 속성을 사용하여 스탬프가 지정됩니다.

메시지를 처리 하는 동안 지원 되는 외부 구성 요소를 호출 하는 경우에도 자동으로 추적 되 고 상관 관계가 지정 됩니다. 수동 추적 및 상관관계는 [Application Insights .NET SDK를 사용하여 사용자 지정 작업 추적](../azure-monitor/app/custom-operations-tracking.md)을 참조하세요.
...
메시지를 받는 시간이 지연 된 것은 아닙니다. 이 시나리오에서는 메시지가 SDK 코드에 대 한 매개 변수로 전달 되기 때문에 메시지를 이미 받았습니다. 그리고 App Insights 로그 (**프로세스**)의 **name** 태그는 현재 외부 이벤트 처리 코드에서 메시지를 처리 하 고 있음을 나타냅니다. 이 문제는 Azure와 관련이 없습니다. 대신 이러한 메트릭은 Service Bus에서 메시지를 이미 수신 했을 때 외부 코드의 효율성을 나타냅니다. 

### <a name="tracking-without-tracing-system"></a>추적 시스템 없이 추적
추적 시스템에서 자동 Service Bus 호출 추적을 지원 하지 않는 경우 추적 시스템 또는 응용 프로그램에 이러한 지원을 추가 하는 방법을 살펴볼 수 있습니다. 이 섹션에서는 Service Bus .NET 클라이언트가 보내는 진단 이벤트에 대해 설명합니다.  
...
DiagnosticSource 이벤트에 대 한 수신기가 없는 경우 계측은 해제 되며 계측 비용이 0으로 유지 됩니다. DiagnosticSource는 수신기에 모든 제어를 제공합니다.
...
- `peer.address` – 정규화 된 네임 스페이스
- `kind` – 생산자, 소비자 또는 클라이언트 중 하나입니다. 생산자는 메시지를 보낼 때, 소비자는 수신 하는 경우, 클라이언트는 정착 때 사용 됩니다.
- `component` – `servicebus`

모든 이벤트에는 ' 엔터티 ' 및 ' 끝점 ' 속성도 있으며 아래 표에서 생략 됩니다.
  * `string Entity` --엔터티 (큐, 항목 등)의 이름입니다.
  * `Uri Endpoint` - Service Bus 엔드포인트 URL

### <a name="instrumented-operations"></a>계측 된 작업
다음은 계측 된 작업의 전체 목록입니다.

| 작업 이름 | 추적된 API |
| -------------- | ----------- | 
| ServiceBusSender. 송신 | ServiceBusSender<br/>ServiceBusSender |
| ServiceBusSender. 일정 | ServiceBusSender<br/>ServiceBusSender | 
| ServiceBusSender. 취소 | ServiceBusSender<br/>ServiceBusSender |
| ServiceBusReceiver. 수신 | ServiceBusReceiver<br/>ServiceBusReceiver |
| ServiceBusReceiver | ServiceBusReceiver |
| ServiceBusReceiver. Peek | ServiceBusReceiver<br/>ServiceBusReceiver |
| ServiceBusReceiver. 중단 | ServiceBusReceiver |
| ServiceBusReceiver. 완료 | ServiceBusReceiver |
| ServiceBusReceiver. 배달 못한 편지 | ServiceBusReceiver |
| ServiceBusReceiver. 지연 |  ServiceBusReceiver |
| ServiceBusReceiver | ServiceBusReceiver |
| ServiceBusSessionReceiver | ServiceBusSessionReceiver |
| ServiceBusSessionReceiver | ServiceBusSessionReceiver |
| ServiceBusSessionReceiver | ServiceBusSessionReceiver |
| ServiceBusProcessor. ProcessMessage | ServiceBusProcessor에 설정 되어 있는 프로세서 콜백입니다. ProcessMessageAsync 속성 |
| ServiceBusSessionProcessor. ProcessSessionMessage | ServiceBusSessionProcessor에 설정 되는 프로세서 콜백입니다. ProcessMessageAsync 속성 |
...
1. `IsEnabled(<OperationName>, string entity, null)`(예: `IsEnabled("ServiceBusSender.Send", "MyQueue1")`). 끝에 ' Start ' 또는 ' Stop '이 없습니다. 특정 작업이나 큐를 필터링하는 데 사용합니다. 콜백 메서드가 반환 되 면 `false` 작업에 대 한 이벤트가 전송 되지 않습니다.
...
2. `IsEnabled(<OperationName>.Start)`(예: `IsEnabled("ServiceBusSender.Send.Start")`). ‘Start’ 이벤트를 실행할지 여부를 확인합니다. 결과는 ' Start ' 이벤트에만 영향을 주지만 추가 계측은이 이벤트에 종속 되지 않습니다.

`IsEnabled`' Stop ' 이벤트가 없습니다.

일부 작업 결과가 예외이면 `IsEnabled("ServiceBusSender.Send.Exception")`이 호출됩니다. ‘Exception’ 이벤트만 구독하고 나머지 계측을 차단할 수 있습니다. 이 경우에도 해당 예외를 처리해야 합니다. 다른 계측은 사용 하지 않도록 설정 되므로 추적 컨텍스트가 소비자에서 생산자로 메시지를 전달 하는 것을 예측할 수 없습니다.

`IsEnabled`를 사용하여 샘플링 전략을 구현할 수도 있습니다. 을 기반으로 하는 샘플링은 `Activity.Id` `Activity.RootId` 모든 타이어에서 일관적인 샘플링을 보장 합니다 (추적 시스템 또는 사용자 고유의 코드에서 전파 되는 경우).

`DiagnosticSource`동일한 소스에 대해 여러 수신기가 있는 경우 하나의 수신기가 이벤트를 수락 하는 것 만으로 충분 하므로를 호출 하는 것은 보장 되지 않습니다 `IsEnabled` .
...
메시지 [처리기 패턴](/dotnet/api/microsoft.azure.servicebus.queueclient.registermessagehandler) 을 사용 하 여 메시지를 처리 하는 경우, 서비스에서 수행 하는 모든 Service Bus 호출은 자동으로 추적 되 고 다른 원격 분석 항목과 상관 관계가 지정 됩니다. 또는 수동 메시지 처리 추적은 다음 예제를 참조하세요.
...
#### <a name="logging-more-properties"></a>추가 속성 로깅

`Activity.Current`는 현재 작업과 해당 부모의 자세한 컨텍스트를 제공합니다. 자세한 내용은 [활동 설명서](https://github.com/dotnet/corefx/blob/master/src/System.Diagnostics.DiagnosticSource/src/ActivityUserGuide.md)를 참조 하세요.
Service Bus 계측은에서 제공 하는 추가 정보를 제공 하 `Activity.Current.Tags` `MessageId` 고, `SessionId` 사용할 수 있을 때마다 제공 합니다.

‘Receive’, ‘Peek’ 및 ‘ReceiveDeferred’ 이벤트를 추적하는 활동에는 `RelatedTo` 태그도 포함될 수 있습니다. 이 태그는 결과로 수신된 메시지의 `Diagnostic-Id` 목록을 포함합니다.
이러한 작업으로 인해 관련 없는 여러 메시지가 수신될 수 있습니다. 또한 `Diagnostic-Id` 작업이 시작 될 때를 알 수 없으므로이 태그를 사용 하 여 ' Receive ' 작업을 ' Process ' 작업과 상호 연결할 수 있습니다. 이 태그는 메시지를 수신하는 데 걸리는 시간을 확인하기 위해 성능 문제를 분석할 때 유용합니다.
...
1. `IsEnabled(<OperationName>, string entity, null)`(예: `IsEnabled("Microsoft.Azure.ServiceBus.Send", "MyQueue1")`). 끝에 ' Start ' 또는 ' Stop '이 없습니다. 특정 작업이나 큐를 필터링하는 데 사용합니다. 콜백 메서드가 반환 되 면 `false` 작업에 대 한 이벤트가 전송 되지 않습니다.

   * ‘Process’ 및 ‘ProcessSession’ 작업의 경우 `IsEnabled(<OperationName>, string entity, Activity activity)` 콜백도 수신됩니다. `activity.Id` 또는 태그 속성을 기준으로 이벤트를 필터링하는 데 사용합니다.
  
2. `IsEnabled(<OperationName>.Start)`(예: `IsEnabled("Microsoft.Azure.ServiceBus.Send.Start")`). ‘Start’ 이벤트를 실행할지 여부를 확인합니다. 결과는 ' Start ' 이벤트에만 영향을 주지만 추가 계측은이 이벤트에 종속 되지 않습니다.

`IsEnabled`' Stop ' 이벤트가 없습니다.

일부 작업 결과가 예외이면 `IsEnabled("Microsoft.Azure.ServiceBus.Exception")`이 호출됩니다. ‘Exception’ 이벤트만 구독하고 나머지 계측을 차단할 수 있습니다. 이 경우에도 해당 예외를 처리해야 합니다. 다른 계측은 사용 하지 않도록 설정 되므로 추적 컨텍스트가 소비자에서 생산자로 메시지를 전달 하는 것을 예측할 수 없습니다.
