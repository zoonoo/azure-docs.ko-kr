---
title: "Service Bus 메시징 개요 | Microsoft Docs"
...
Service Bus WCF Relay는 다양한 이점을 제공하지만 메시지를 보내고 받기 위해 서버와 클라이언트가 동시에 온라인 상태여야 합니다. 이는 일반적으로 요청의 수명이 짧은 HTTP 스타일 통신이나 브라우저, 모바일 응용 프로그램 등과 같이 가끔씩만 연결하는 클라이언트에는 적합하지 않습니다. 조정된 메시징은 분리된 통신을 지원하며, 클라이언트와 서버가 필요할 때 연결하고 비동기 방식으로 작업을 수행할 수 있는 고유한 이점이 있습니다.
