...
- [Azure 서비스 버스 메시징에 대한 일반적인 질문](#general-questions-about-azure-service-bus-messaging)
- [서비스 버스 모범 사례](#service-bus-best-practices)
- [서비스 버스 가격 책정](#service-bus-pricing)
- [서비스 버스 할당량](#service-bus-quotas)
- [구독 및 네임스페이스 관리](#subscription-and-namespace-management)
- [문제 해결](#service-bus-troubleshooting)

## Azure 서비스 버스 메시징에 대한 일반적인 질문

### Azure 서비스 버스 메시징이란?

[Azure 서비스 버스 메시징](service-bus-messaging-overview.md)은 분리된 시스템 간에 데이터를 보낼 수 있도록 하는 비동기 메시징 클라우드 플랫폼입니다. Microsoft에서 서비스로 제공하므로 이를 사용하기 위해 고유한 하드웨어 중 하나를 호스트할 필요는 없습니다.

### 서비스 버스 네임스페이스란?

네임스페이스는 응용 프로그램 내에서 서비스 버스 리소스의 주소를 지정하기 위한 범위 컨테이너를 제공합니다. 네임스페이스를 만드는 작업은 서비스 버스를 사용하는 데 필요하고 서비스 버스를 시작하는 첫 번째 단계 중 하나입니다.

### Azure 서비스 버스 큐란?

[서비스 버스 큐](service-bus-queues-topics-subscriptions.md)는 메시지가 저장되는 엔터티입니다. 큐는 서로 통신해야 하는 여러 응용 프로그램 또는 분산된 응용 프로그램의 여러 부분이 있는 경우에 특히 유용합니다. 큐는 여러 제품(메시지)를 해당 위치에서 받고 보내는 배포 센터와 비슷합니다.
...
일반적인 큐 또는 항목은 단일 메시지 broker에서 처리되며 하나의 메시징 저장소에 저장됩니다. [분할된 큐 또는 토픽](service-bus-partitioning.md)은 여러 메시지 브로커에 의해 처리되고 여러 메시징 저장소에 저장됩니다. 즉, 분할된 큐 또는 항목의 전체 처리량은 단일 메시지 broker 또는 메시징 저장소의 성능으로 제한되지 않습니다. 또한 메시징 스토어가 일시적으로 중단된 경우에도 분할된 큐 또는 항목을 계속 렌더링할 수 없습니다.

분할 엔터티를 사용하는 경우 순서는 보장할 수 없습니다. 파티션을 사용할 수 없는 이벤트에서도 여전히 메세지를 보내고 다른 파티션에서 메시지를 수신할 수 있습니다.

## 서비스 버스 모범 사례

### 일부 Azure 서비스 버스 모범 사례는 무엇인가요?

-   [서비스 버스 조정된 메시징을 사용한 성능 향상에 대한 모범 사례][] - 이 문서에서는 조정된 메시지를 교환할 때 성능을 최적화하는 방법을 설명합니다.

-   [Service Bus 중단 및 재해로부터 응용 프로그램을 보호하는 모범 사례][] - 이 문서에서는 릴레이 끝점, 큐 및 토픽과 활성 및 수동 복제본을 데이터 센터 가동 중단으로부터 보호하는 가장 좋은 방법을 설명합니다.

###메시징 엔터티를 만들기 전에 무엇을 알아야 하나요?   
...
서비스 버스 가격 책정에 대한 전체 내용은 [서비스 버스 가격 책정 세부 정보][Pricing overview]를 참조하세요. 언급된 가격 외에도 응용 프로그램이 프로비전되는 데이터 센터의 외부에서 송신을 위해 연결된 데이터 전송에 요금이 부과됩니다.
...
일반적으로 조정된 엔터티(큐, 토픽 및 구독)에 대해 위에서 설명한 동일한 메서드를 사용하여 릴레이에 대한 청구 가능한 메시지를 계산합니다. 하지만 몇 가지 주목할 차이점이 있습니다.
...
아니요, 서비스 버스는 저장소에 대한 요금을 청구하지 않습니다. 하지만 큐/토픽당 지속될 수 있는 데이터의 최대 크기를 제한하는 할당량이 있습니다. 다음 FAQ를 참조하세요.
...
릴레이에서 동시 **netTcpRelay** 및 **netHttpRelay** 수신기 수를 25개로 제한합니다(**NetOneway** 릴레이에 1개).

#### 네임스페이스 당 동시 릴레이 수신기 수
...
**토픽당 구독 수** – 토픽당 최대 구독 수를 2,000개로 제한합니다. 이 할당량에 도달하면 토픽에 추가 구독을 만들기 위한 후속 요청이 거부됩니다. 이 경우에 만들기 시도가 포털을 통해 또는 클라이언트 코드에서 수행되었는지 여부에 따라 Azure 클래식 포털이 오류 메시지를 표시하거나 호출 클라이언트 코드에서 예외가 발생합니다.

**항목 당 SQL 필터 수** – 항목당 SQL 필터의 최대 수를 2,000개로 제한합니다. 이 할당량에 도달하면 후속 토픽에서 추가 필터의 생성이 거부되며 호출 코드에서 예외를 수신합니다.

**토픽당 상관관계 필터 수** – 토픽당 상관관계 필터의 최대 수는 100,000개로 제한됩니다. 이 할당량에 도달하면 후속 토픽에서 추가 필터의 생성이 거부되며 호출 코드에서 예외를 수신합니다.
...
### Azure 서비스 버스 메시징 API 및 해당 제안된 작업에 의해 생성된 일부 예외는 무엇인가요?
...
[서비스 버스 조정된 메시징을 사용한 성능 향상에 대한 모범 사례]: service-bus-performance-improvements.md
[Service Bus 중단 및 재해로부터 응용 프로그램을 보호하는 모범 사례]: ../service-bus/service-bus-outages-disasters.md
[Pricing overview]: https://azure.microsoft.com/pricing/details/service-bus/
[할당량 개요]: ../service-bus/service-bus-quotas.md
[여기]: ../service-bus/service-bus-powershell-how-to-provision.md#migrate-a-namespace-to-another-azure-subscription
[Exceptions overview]: ../service-bus/service-bus-messaging-exceptions.md
[예외 개요]: ../service-bus/service-bus-messaging-exceptions.md
[공유 액세스 서명]: ../service-bus/service-bus-sas-overview.md
