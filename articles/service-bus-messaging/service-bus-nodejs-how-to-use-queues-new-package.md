...
Service Bus 큐와의 상호 작용은 [ServiceBusClient](/javascript/api/@azure/service-bus/servicebusclient) 클래스를 인스턴스화하여 [QueueClient](/javascript/api/@azure/service-bus/queueclient) 클래스를 인스턴스화하는 데 사용하는 것부터 시작합니다. 큐 클라이언트가 생겼으면 발신자를 만들고, 발신자에서 [send](/javascript/api/@azure/service-bus/sender#send-sendablemessageinfo-) 또는 [sendBatch](/javascript/api/@azure/service-bus/sender#sendbatch-sendablemessageinfo---) 메서드를 사용하여 메시지를 보낼 수 있습니다.
