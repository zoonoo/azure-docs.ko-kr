...
1. 다음 단계를 수행 하 여 키를 만듭니다.
    ...
    1. 키에 대 한 세부 정보를 입력 하 고 **선택** 을 클릭 합니다. 그러면 Microsoft에서 관리 하는 키와 키 (고객 관리 키)를 암호화할 수 있습니다. 


    > [!IMPORTANT]
    > 지역 재해 복구와 함께 고객 관리 키를 사용 하려는 경우이 섹션을 검토 하세요. 
    >
    > 고객 관리 키를 사용 하 여 Microsoft 관리 키의 암호화를 사용 하도록 설정 하려면 지정 된 Azure KeyVault에서 Service Bus ' 관리 id에 대 한 [액세스 정책이](../key-vault/general/secure-your-key-vault.md) 설정 되어 있습니다. 이렇게 하면 Azure Service Bus 네임 스페이스의 Azure KeyVault에 대 한 제어 된 액세스가 보장 됩니다.
    >
    > 다음 이유로 인해:
    > 
    >   * Service Bus 네임 스페이스에 대해 [지역 재해 복구](service-bus-geo-dr.md) 를 이미 사용 하도록 설정 하 고 고객 관리 키를 사용 하도록 설정 하려는 경우 
    >     * 페어링 분리
    >     * 주요 자격 증명 모음에 대 한 기본 및 보조 네임 스페이스에 대 한 관리 되는 id에 대 한 [액세스 정책을 설정](../key-vault/general/assign-access-policy-portal.md) 합니다.
    ...
    >     * 보조 네임 스페이스에 대 한 관리 되는 id에 대 한 [액세스 정책을](../key-vault/general/assign-access-policy-portal.md) 키 자격 증명 모음에 설정 합니다.
