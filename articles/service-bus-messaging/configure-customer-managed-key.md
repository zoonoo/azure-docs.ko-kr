...
2. : **CreateServiceBusPremiumNamespaceParams**라는 템플릿 매개 변수 파일을 만듭니다. 
    ...
    > [!NOTE]
    > 다음 값을 바꿉니다. 
    > - `<ServiceBusNamespaceName>`-Service Bus 네임 스페이스의 이름입니다.
    ...
    ```
3. 다음 PowerShell 명령을 실행 하 여 템플릿을 배포 하 고 premium Service Bus 네임 스페이스를 만듭니다. 그런 다음 나중에 사용할 Service Bus 네임 스페이스의 ID를 검색 합니다. 명령을 실행 하기 전에 `{MyRG}`을 리소스 그룹의 이름으로 바꿉니다.  
