...
두 경우 모두 자연 재해 또는 인재가 문제를 발생시킵니다. 이 문제를 해결하고 메시지를 보낼 수 있는지 확인하려면 [쌍을 이루는 네임스페이스][쌍을 이루는 네임스페이스]를 사용하여 기본 위치가 다시 정상인 동안 메시지를 보조 위치로 보내도록 할 수 있습니다. 자세한 정보는 [Service Bus 가동 중단 및 재해로부터 응용 프로그램을 보호하기 위한 모범 사례][Service Bus 가동 중단 및 재해로부터 응용 프로그램을 보호하기 위한 모범 사례]을 참조하세요.
