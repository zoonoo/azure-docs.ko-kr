...
| `LockToken`                             | lock 토큰은 *보기-잠금* 수신 모드에서 브로커가 보유하는 잠금에 대한 참조입니다. 이 토큰은 [연기](message-deferral.md) API를 통해 잠금을 영구적으로 고정하고, 일반 배달 상태 흐름에서 메시지를 제외하는 데 사용할 수 있습니다. 이 속성은 읽기 전용입니다.                                                                                                                                                               |
