...
이 문서에서는 Azure SQL 데이터 동기화의 알려진 문제를 해결 하는 방법을 설명 합니다. 문제에 대 한 해결 방법이 있으면 여기에 제공 됩니다.
...
- [다음과 같은 메시지가 표시 됩니다. "열 \<열>에 NULL 값을 삽입할 수 없습니다. 열에 null이 허용 되지 않습니다. " 이는 무엇을 의미 하며 어떻게 해결할 수 있나요?](#sync-nulls)
...
### <a name="i-see-this-message-cannot-insert-the-value-null-into-the-column-column-column-does-not-allow-nulls-what-does-this-mean-and-how-can-i-fix-it"></a><a name="sync-nulls"></a>다음과 같은 메시지가 표시 됩니다. "열 \<열>에 NULL 값을 삽입할 수 없습니다. 열에는 Null을 사용할 수 없습니다."라는 오류 메시지를 받았습니다. 이 오류는 어떤 의미이며 오류를 수정할 수 있는 방법은 무엇인가요? 
...
- **해결**. SQL Database 및 SQL Server 데이터베이스가 모두 온라인 상태인지 확인합니다.
...
- **해결**. 온-프레미스 SQL Server 데이터베이스를 등록 취소하려면 데이터베이스를 선택하고 **강제 삭제**를 선택합니다.
...
- **해결**. 사용자 계정에 서비스로 로그온할 수 있는 자격 증명을 부여합니다.
  ...
  1. **시작** > **제어판** >  > **Local Policy** > **User Rights Management****Local Security Policy****Administrative Tools**관리 도구 로컬 보안 정책 로컬 정책 사용자 Rights Management로 이동 합니다. > 
  1. **서비스로 로그온**을 선택합니다.
