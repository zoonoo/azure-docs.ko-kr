...
이 문서에서는 Azure SQL Data Sync에서 알려진 문제를 해결하는 방법을 설명합니다. 문제에 대한 해결 방법이 있는 경우 여기에 제공됩니다.
...
- [클라이언트 에이전트와 연결된 온-프레미스 데이터베이스에 대한 포털 UI에서 동기화가 실패합니다.](#sync-fails)

- [동기화 그룹이 처리 상태에 멈췄습니다.](#sync-stuck)

- [테이블에 잘못된 데이터가 표시됩니다.](#sync-baddata)

- [동기화에 성공한 후 일관성 없는 기본 키 데이터가 표시됨](#sync-pkdata)

- [성능이 크게 저하되는 것을 볼 수 있습니다.](#sync-perf)

- ["열> NULL 값을 열에 \<삽입할 수 없습니다. 열은 null을 허용하지 않습니다." 이것은 무엇을 의미하며 어떻게 해결할 수 있습니까?](#sync-nulls)

- [데이터 동기화는 순환 참조를 어떻게 처리합니까? 즉, 동일한 데이터가 여러 동기화 그룹에서 동기화되고 결과적으로 계속 변경되는 경우입니까?](#sync-circ)
...
- **해결 방법**. SQL 데이터 동기화 서비스를 다시 시작합니다.
    ...
    1. 검색 결과에서 **서비스를**선택합니다.
    1. SQL **데이터 동기화** 서비스를 찾습니다.
    1. 서비스 상태가 **중지된**경우 서비스 이름을 마우스 오른쪽 단추로 클릭한 다음 **시작을**선택합니다.
...
### <a name="i-see-inconsistent-primary-key-data-after-a-successful-sync"></a><a name="sync-pkdata"></a>동기화가 성공한 후 일관되지 않은 기본 키 데이터가 표시됩니다.
...
### <a name="i-see-this-message-cannot-insert-the-value-null-into-the-column-column-column-does-not-allow-nulls-what-does-this-mean-and-how-can-i-fix-it"></a><a name="sync-nulls"></a>"열> NULL 값을 열에 \<삽입할 수 없습니다. 열에는 Null을 사용할 수 없습니다."라는 오류 메시지를 받았습니다. 이 오류는 어떤 의미이며 오류를 수정할 수 있는 방법은 무엇인가요? 
...
- [동기화 그룹을 삭제할 수 없습니다.](#setup-delete)

- [온-프레미스 SQL Server 데이터베이스를 등록 취소할 수 없습니다.](#setup-unreg)

- [시스템 서비스를 시작할 수 있는 권한이 없음](#setup-perms)

- [데이터베이스가 “만료” 상태임](#setup-date)

- [동기화 그룹에 "오래된" 상태가 있습니다.](#setup-date2)

- [에이전트를 제거하거나 중지한 후 3분 이내에 동기화 그룹을 삭제할 수 없습니다.](#setup-delete2)

- [분실되거나 손상된 데이터베이스를 복원하면 어떻게 되나요?](#setup-restore)

### <a name="i-get-a-disk-out-of-space-message"></a><a name="setup-space"></a>"디스크 공간이 부족" 메시지가 나타납니다.
...
### <a name="i-cant-unregister-an-on-premises-sql-server-database"></a><a name="setup-unreg"></a> 온-프레미스 SQL Server 데이터베이스를 등록 취소할 수 없음
...
### <a name="i-dont-have-sufficient-privileges-to-start-system-services"></a><a name="setup-perms"></a>시스템 서비스를 시작하기에 충분한 권한이 없습니다.
...
- **해결 방법**. 사용자 계정에 서비스로 로그온할 수 있는 자격 증명을 부여합니다.
  ...
  1. **제어판** >  **시작** > 패널**관리 도구** > **로컬 보안 정책** > **로컬 정책** > **사용자 권한 관리로**이동합니다.
  1. **서비스로 로그온**을 선택합니다.
  1. **속성** 대화 상자에서 사용자 계정을 추가합니다.
  1. **적용**을 선택한 다음 **확인**을 선택합니다.
  1. 모든 창을 닫습니다.

### <a name="a-database-has-an-out-of-date-status"></a><a name="setup-date"></a>데이터베이스에 "오래된" 상태가 있습니다.
