...
- Azure PowerShell이 설치되어 있어야 합니다. 자세한 내용은 [Azure PowerShell을 시작하기](https://docs.microsoft.com/powershell/azure/get-started-azureps)를 참조하십시오.

> [!NOTE]
> 이 자습서에서는 제한된 미리 보기([Elastic Database 작업](sql-database-elastic-database-client-library.md))에 있는 Azure SQL Database 서비스의 기능을 사용합니다. 이 자습서를 수행 하려면 제목=탄력적 작업 미리 보기와 *함께\@SaaSFeedback microsoft.com* 구독 ID를 제공 합니다. 구독이 활성화되었다는 확인을 받은 후 [최신 시험판 작업 cmdlet을 다운로드하여 설치하세요](https://github.com/jaredmoo/azure-powershell/releases). 이 미리 보기는 제한되어 있으므로 관련 질문이나 지원은 *\@saaSFeedback microsoft.com* 문의하십시오.
...
각 테넌트의 데이터베이스에는 **VenueTypes** 테이블의 장소 유형 집합이 포함됩니다. 각 장소 유형은 장소에서 진행할 수 있는 이벤트의 종류를 정의합니다. 장소 유형은 테넌트 이벤트 앱에 표시된 배경 이미지에 해당합니다.  이 연습에서는 두 개의 추가 장소 유형인 *Motorcycle Racing* 및 *Swimming Club*을 추가하기 위해 모든 데이터베이스에 업데이트를 배포합니다.
...
- **sp\_\_추가 작업은** *참조 데이터 배포라는*작업을 만듭니다.
...
2. SSMS에서 열기 *... 학습 모듈\\스키마\\관리 onlineReindex.sql . \\*

3. 스크립트를 실행하려면 **F5를** 누릅니다.

#### <a name="observe"></a>관찰

*OnlineReindex.sql* 스크립트에서 다음 항목을 살펴봅니다.

* **sp\_\_추가 작업은** 온라인 *재인덱스\_\_PK\_\_VenueTyp 265E44FD7FD4C885라는*새로운 작업을 만듭니다.

* **sp\_\_추가 작업 단계는** 인덱스를 업데이트하는 T-SQL 명령 텍스트를 포함하는 작업 단계를 만듭니다.
...
다음으로 임시 [보고 자습서를](saas-multitenantdb-adhoc-reporting.md) 시도하여 테넌트 데이터베이스 간에 분산 쿼리를 실행하는 방법을 살펴봅습니다.
