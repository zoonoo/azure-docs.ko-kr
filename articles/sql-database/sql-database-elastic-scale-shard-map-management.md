...
1. **Global Shard Map (GSM)**: **shardMapManager**를 만들 때 모든 분할된 데이터베이스 맵 및 매핑용 리포지토리로 사용할 데이터베이스를 지정합니다. 정보를 관리할 특수 테이블 및 저장된 프로시저는 자동으로 생성됩니다. 일반적으로 작은 데이터베이스이며 손쉽게 액세스할 수 있지만 응용 프로그램의 다른 요구 사항에는 사용할 수 없습니다. 테이블은 **\_\_ShardManagement**라는 특수한 스키마에 있습니다.
