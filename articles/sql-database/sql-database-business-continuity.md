...
Azure SQL Database는 비즈니스 프로세스에 영향을 줄 수 있는 오류에 대한 [고가용성](sql-database-high-availability.md) 및 복원력을 제공하는 Azure 클라우드 환경에 맞게 구성되고 최적화된 안정적인 최신 SQL Server 데이터베이스 엔진을 구현한 것입니다. Azure SQL Database의 **비즈니스 연속성**은 비즈니스가 중단, 특히 자사의 컴퓨팅 인프라에서 계속 운영할 수 있도록 하는 메커니즘, 정책 및 절차를 나타냅니다. 대부분의 경우 Azure SQL Database는 클라우드 환경에서 발생할 수 있는 중단 이벤트를 처리하고 비즈니스 프로세스를 계속 실행합니다. 그러나 SQL Database에서 처리할 수 없는 몇 가지 중단 이벤트는 다음과 같습니다.
...
- 데이터 손상 또는 삭제 - 일반적으로 응용 프로그램 버그 또는 사람의 실수로 인해 발생합니다. 이러한 오류는 본질적으로 응용 프로그램에 따라 다르며, 일반적으로 인프라에서 자동으로 검색하거나 완화할 수 없습니다.
- 데이터 센터 중단 - 아마도 자연 재해로 인해 발생했을 수 있습니다. 이 시나리오에는 대체 데이터 센터로의 응용 프로그램 장애 조치가 포함된 일부 수준의 지리적 중복성이 필요합니다.
- 업그레이드 또는 유지 관리 오류 – 응용 프로그램이나 데이터베이스에 대한 계획된 업그레이드 또는 유지 관리 중에 발생하는 예기치 않은 문제로 인해 이전 데이터베이스 상태로 신속하게 롤백해야 할 수 있습니다.
...
다음과 같은 응용 프로그램의 경우 자동화된 백업 및 [특정 시점 복원](sql-database-recovery-using-backups.md#point-in-time-restore)을 비즈니스 연속성 및 복구 메커니즘으로 사용합니다.
...
독립 실행형 데이터베이스 및 탄력적 풀에 대한 응용 프로그램 디자인 고려 사항에 대한 논의는 [클라우드 재해 복구를 위한 응용 프로그램 디자인](sql-database-designing-cloud-solutions-for-disaster-recovery.md) 및 [탄력적 풀 재해 복구 전략](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)을 참조하세요.
