...
- [SQL Database 서버가 삭제되지 않은 경우](sql-database-recovery-using-backups.md#deleted-database-restore)**삭제된 데이터베이스를 삭제된 시점으로 복원**할 수 있습니다.
...
| 여러 데이터베이스를 동시에 장애 조치 (failover)  |     예          |      yes         |
| 사용자가 장애 조치 (failover) 후 연결 문자열을 업데이트 해야 함      |     yes         |      예          |
| 지원 되는 관리 되는 인스턴스                   |     예          |      yes         |
| 주 복제본과 동일한 지역에 있을 수 있습니다.             |     yes         |      예          |
| 여러 복제본                            |     yes         |      예          |
| 읽기 확장 지원                          |     yes         |      yes         |
| &nbsp; | &nbsp; | &nbsp; |
