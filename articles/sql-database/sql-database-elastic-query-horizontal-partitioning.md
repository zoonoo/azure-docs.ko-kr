<properties
    pageTitle="확장된 클라우드 데이터베이스에서 보고 | Microsoft Azure"
...
## 1\.1 데이터베이스 범위 마스터 키 및 자격 증명 만들기 
...
외부 데이터 원본에는 분할 맵을 참조합니다. 그러면 탄력적 쿼리는 외부 데이터 원본과 기본 분할 맵을 사용하여 데이터 계층에 참여하는 데이터베이스를 열거합니다. 탄력적 쿼리를 처리하는 동안 분할 데이터베이스 맵을 읽고 분할 데이터베이스의 데이터에 액세스할 때는 동일한 자격 증명을 사용합니다.
