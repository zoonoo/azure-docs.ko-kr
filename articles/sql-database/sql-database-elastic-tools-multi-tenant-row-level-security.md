...
* **행 수준 보안**을 사용하면 개발자는 쿼리를 실행하는 테넌트에 속하지 않은 행을 필터링하는 보안 정책을 사용하여 여러 테넌트의 데이터를 같은 데이터베이스에 저장할 수 있습니다. 액세스 논리를 응용 프로그램이 아닌 데이터베이스 내부에 중앙 집중화하면 코드 베이스가 성장하기 때문에 응용 프로그램의 유지 관리가 간소화되고 오류 위험이 완화됩니다. RLS를 사용하려면 최신 [Azure SQL 데이터베이스 업데이트(V12)](sql-database-preview-whats-new.md)가 필요합니다.
