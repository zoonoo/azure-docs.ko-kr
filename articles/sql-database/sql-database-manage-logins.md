---
title: 로그인 및 사용자
description: 로그인 및 사용자 계정을 사용 하 여 사용자가 액세스 하도록 사용자를 인증 하 고, 역할 및 명시적 권한을 사용 하 여 로그인 및 사용자가 서버 수준 뿐만 아니라 데이터베이스 내에서 작업을 수행할 수 있는 권한을 부여 하 Azure SQL Database는 방법에 대해 알아봅니다 Synapse.
...
ms.locfileid: "79268913"
---
# <a name="granting-database-access-and-authorization-to-sql-database-and-azure-synapse-analytics-using-logins-and-user-accounts"></a>로그인 및 사용자 계정을 사용 하 여 SQL Database 및 Azure Synapse Analytics에 데이터베이스 액세스 및 권한 부여

로그인 및 사용자 계정을 사용 하 여 Azure SQL Database 및 Azure Synapse Analytics (이전의 Azure SQL Data Warehouse)의 데이터베이스에 대 한 인증 된 액세스를 관리 합니다. [**인증은**](sql-database-security-overview.md#authentication) 사용자를 증명 하는 사용자를 증명 하는 프로세스입니다.

- 로그인은 master 데이터베이스의 개별 계정입니다.
- 사용자 계정은 모든 데이터베이스의 개별 계정이 며 로그인과 연결 될 필요는 없습니다.

> [!IMPORTANT]
> Azure SQL Database 및 Azure Synapse Analytics (이전의 Azure SQL Data Warehouse)의 데이터베이스는이 문서의 나머지 부분에서 Azure SQL Database (간단히 설명)로 통칭 됩니다.

데이터베이스 사용자는 사용자 계정을 사용 하 여 Azure SQL database에 연결 하 고 다음 두 가지 방법 중 하나를 사용 하 여 인증 됩니다.

- 로그인 이름이 나 사용자 계정 이름 및 Azure SQL Database에 저장 된 관련 암호로 구성 된 [SQL 인증](https://docs.microsoft.com/sql/relational-databases/security/choose-an-authentication-mode#connecting-through-sql-server-authentication)입니다.
- Azure Active Directory에 저장 된 로그인 자격 증명을 사용 하는 [Azure Active Directory 인증](sql-database-aad-authentication.md)

Azure SQL database 내에서 데이터에 액세스 하 고 다양 한 작업을 수행할 수 있는 권한 부여는 데이터베이스 역할 및 명시적 권한을 사용 하 여 관리 됩니다. [**권한 부여**](sql-database-security-overview.md#authorization) 는 Azure SQL Database 내에서 사용자에 게 할당 된 사용 권한을 나타내며 사용자가 수행할 수 있는 작업을 결정 합니다. 권한 부여는 사용자 계정의 데이터베이스 [역할 멤버 자격과](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles) [개체 수준 권한](https://docs.microsoft.com/sql/relational-databases/security/permissions-database-engine)으로 제어 됩니다. 사용자에게 필요한 최소한의 권한을 부여하는 것이 가장 좋습니다.
...
첫 번째 Azure SQL Database 배포를 만들 때 해당 로그인에 대 한 관리자 로그인 및 연결 된 암호를 지정 합니다. 이 관리 계정을 **서버 관리자**라고 합니다. Master 및 사용자 데이터베이스에서 다음 로그인 및 사용자 구성은 배포 중에 발생 합니다.
...
이 시점에서 SQL Database은 단일 SQL 로그인 및 사용자 계정을 사용 하는 액세스에 대해서만 구성 됩니다. 전체 또는 부분 관리 권한으로 추가 로그인을 만들려면 배포 모드에 따라 다음과 같은 옵션을 사용할 수 있습니다.
...
- **모든 관리 권한이 있는 Azure Active Directory 관리자 계정 만들기**
  ...
  - [SQL 인증에 Azure Active Directory 인증 사용](sql-database-aad-authentication.md)
...
> [!IMPORTANT]
> Azure AD id에 매핑된 포함 된 사용자를 만들려면 SQL Database의 관리자 인 Azure AD 계정을 사용 하 여 로그인 해야 합니다. 관리 되는 인스턴스에서 `sysadmin` 권한이 있는 SQL 로그인은 Azure AD 로그인 또는 사용자를 만들 수도 있습니다.
...
- [관리 되는 인스턴스 데이터베이스에 대 한 로그인 만들기](https://docs.microsoft.com/t-sql/statements/create-login-transact-sql?view=azuresqldb-mi-current#examples-2)
