...
Azure SQL 데이터베이스 인덱스 관리자는 인덱스를 만들고 삭제하기 위한 권장 사항을 제공할 뿐만 아니라 쿼리를 매개 변수화하고 스키마 문제를 해결하기 위한 권장 사항도 포함하도록 업데이트되었습니다. 이러한 추가 권장 사항이 포함된 인덱스 관리자는 이제 **SQL 데이터베이스 관리자**입니다.

SQL 데이터베이스 관리자는 SQL 데이터베이스의 사용 기록을 분석하여 성능을 평가합니다. 데이터베이스의 일반적인 워크로드를 실행하는 데 가장 적합한 권장 사항을 사용하는 것이 좋습니다.

다음 권장 사항은 V12 서버에 대해 사용할 수 있습니다(V11 서버에는 권장 사항을 사용할 수 없습니다). 현재는 만들기 및 삭제 인덱스 권장 사항이 자동으로 적용되도록 설정할 수 있습니다. 자세한 내용은 아래 [자동 인덱스 관리](#enable-automatic-index-management)를 참조하세요.
...
![추천](./media/sql-database-index-advisor/recommendations.png)
...
권장 사항이 성공적으로 구현된 후(현재는 인덱스 작업 및 쿼리 매개 변수화 권장 사항만) 인덱스 세부 정보 블레이드의 **쿼리 인사이트**를 클릭하여 [Query Performance Insight](sql-database-query-performance.md)를 열고 상위 쿼리의 성능 영향을 확인할 수 있습니다.
