...
SaaS 응용 프로그램을 만들 때 잠재 고객에게 평가판 소프트웨어를 제공하는 것은 일반적입니다. 이 경우 데이터에 대해 다중 테넌트 데이터베이스를 사용하는 것이 비용 효율적입니다. 그러나 잠재 고객이 고객이 되면 단일 테넌트 데이터베이스가 더 나은 성능을 제공하므로 단일 테넌트 데이터베이스가 더 좋습니다. 고객이 평가판 사용 기간 동안 데이터를 만든 경우 [분할-병합 도구](sql-database-elastic-scale-overview-split-and-merge)를 사용하여 데이터를 다중 테넌트에서 새 단일 테넌트 데이터베이스로 이동합니다.

## 다음 단계

클라이언트 라이브러리를 보여 주는 샘플 응용 프로그램은 [탄력적 데이터베이스 도구 시작](sql-database-elastic-scale-get-started.md)을 참조하세요.

분할-병합 도구를 사용하려면 [보안을 구성](sql-database-elastic-scale-split-merge-security-configuration,md)해야 합니다.

탄력적 데이터베이스 풀의 세부 사항을 보려면 [탄력적 데이터베이스 풀의 가격 및 성능 고려 사항](sql-database-elastic-pool-guidance.md)을 참조하거나 [자습서](sql-database-elastic-pool-portal.md)를 사용하여 새로운 풀을 만드세요.
