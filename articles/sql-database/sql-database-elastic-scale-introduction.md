...
* 탄력적 데이터베이스 도구: 이러한 두 도구는 분할된 데이터베이스 솔루션의 개발 및 관리를 간소화합니다. 도구는 [탄력적 데이터베이스 클라이언트 라이브러리](sql-database-elastic-client-overview.md) 및 [탄력적 데이터베이스 분할/병합 도구](sql-database-elastic-scale-overview-split-and-merge.md)입니다. 
...
* [탄력적 데이터베이스 작업](sql-database-elastic-jobs-overview.md)(미리 보기): 작업을 사용하여 많은 수의 Azure SQL 데이터베이스를 관리합니다. **작업**은 미리 정의된 데이터베이스의 집합에 대해 실행되는 멱등원 T-SQL 스크립트입니다. 오류는 자동으로 다시 시도(구성 가능)되고 모든 오류는 디버깅에 대해 기록됩니다. 
...
아래 그래픽은 데이터베이스 컬렉션과 관련된 **탄력적 데이터베이스 기능**을 포함하는 아키텍처를 보여 줍니다.
...
2. **탄력적 데이터베이스 클라이언트 라이브러리**는 분할된 데이터베이스 집합을 관리하는데 사용됩니다.
...
클라우드 응용 프로그램에 대한 탄력성 및 규모 달성은 계산 및 Blob 저장소에 대해 복잡하지 않습니다. 단순히 단위를 더하거나 뺍니다. 하지만 관계형 데이터베이스의 상태 저장 데이터 처리는 과제로 남아 있습니다. 이러한 과제는 다음 두 시나리오에서 가장 눈에 띄게 등장합니다.
...
* 수평적 크기 조정은 [탄력적 데이터베이스 클라이언트 라이브러리](sql-database-elastic-client-overview.md)를 사용하여 관리됩니다.

* 수직적 크기 조정은 서비스 계층을 변경하도록 Azure PowerShell cmdlet을 사용하거나 탄력적 데이터베이스 풀에 데이터베이스를 배치하여 수행됩니다.
...
SaaS 응용 프로그램을 만들 때 잠재 고객에게 평가판 소프트웨어를 제공하는 것은 일반적입니다. 이 경우 데이터에 대해 다중 테넌트 데이터베이스를 사용하는 것이 비용 효율적입니다. 그러나 잠재 고객이 고객이 되면 단일 테넌트 데이터베이스가 더 나은 성능을 제공하므로 단일 테넌트 데이터베이스가 더 좋습니다. 고객이 평가판 사용 기간 동안 데이터를 만든 경우 [분할 병합 도구](sql-database-elastic-scale-overview-split-and-merge)를 사용하여 데이터를 다중 테넌트에서 새 단일 테넌트 데이터베이스로 이동하십시오.

## 다음 단계

클라이언트 라이브러리를 보여 주는 샘플 응용 프로그램은 [탄력적 데이터베이스 도구 시작하기](sql-database-elastic-scale-get-started.md)를 참조하세요.

분할/병합 도구를 사용하려면 [보안을 구성](sql-database-elastic-scale-split-merge-security-configuration,md)해야 합니다.

탄력적 데이터베이스 풀의 세부 사항을 보려면 [탄력적 데이터베이스 풀의 가격 및 성능 고려 사항](sql-database-elastic-pool-guidance.md)을 참조하거나 [자습서](sql-database-elastic-pool-portal.md)를 사용하여 새로운 풀을 만드십시오.
