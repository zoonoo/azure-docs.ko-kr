...
| [인증서 및 비대칭 키](https://docs.microsoft.com/sql/relational-databases/security/sql-server-certificates-and-asymmetric-keys) | 예, `BACKUP` 및 `CREATE` 작업에 대 한 파일 시스템에 액세스 하지 않습니다. | 예, `BACKUP` 및 `CREATE` 작업을 위한 파일 시스템에 대 한 액세스 권한이 없는 경우 [인증서 차이점](sql-database-managed-instance-transact-sql-information.md#certificates)을 참조 하세요. | 
| [변경 데이터 캡처-CDC](https://docs.microsoft.com/sql/relational-databases/track-changes/about-change-data-capture-sql-server) | 아닙니다. | yes |
| [데이터 정렬 - 서버/인스턴스](https://docs.microsoft.com/sql/relational-databases/collations/set-or-change-the-server-collation) | 아니요. 기본 논리 서버 데이터 정렬 @no__t 항상 사용 됩니다. | 예. [인스턴스를 만들](scripts/sql-managed-instance-create-powershell-azure-resource-manager-template.md) 때 설정할 수 있으며 나중에 업데이트할 수 없습니다. |
| [Columnstore 인덱스](https://docs.microsoft.com/sql/relational-databases/indexes/columnstore-indexes-overview) | 예- [프리미엄 계층, 표준 계층-S3 이상, 범용 계층, 중요 비즈니스용 및 HyperScale 계층](https://docs.microsoft.com/sql/relational-databases/indexes/columnstore-indexes-overview) |yes |
| [CLR (공용 언어 런타임)](https://docs.microsoft.com/sql/relational-databases/clr-integration/common-language-runtime-clr-integration-programming-concepts) | 아닙니다. | 예, 하지만 `CREATE ASSEMBLY` 문의 파일 시스템에 액세스할 수 없습니다. [CLR 차이점](sql-database-managed-instance-transact-sql-information.md#clr) 을 참조 하세요. |
...
| [데이터베이스 메일-DbMail](https://docs.microsoft.com/sql/relational-databases/database-mail/database-mail) | 아닙니다. | yes |
...
| [분산된 파티션 뷰](https://docs.microsoft.com/sql/t-sql/statements/create-view-transact-sql#partitioned-views) | 아닙니다. | yes |
...
| [파일 및 파일 그룹](https://docs.microsoft.com/sql/relational-databases/databases/database-files-and-filegroups) | 기본 파일 그룹만 해당 | 예. 파일 경로는 자동으로 할당 되 고 `ALTER DATABASE ADD FILE` [문에서](sql-database-managed-instance-transact-sql-information.md#alter-database-statement)는 파일 위치를 지정할 수 없습니다.  |
...
| [로그인 및 사용자](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/principals-database-engine) | 예. 하지만 `CREATE` 및 @no__t 로그인 문은 모든 옵션을 제공 하지 않습니다 (Windows 및 서버 수준 Azure Active Directory 로그인 없음). `EXECUTE AS LOGIN`은 지원 되지 않습니다. 대신 `EXECUTE AS USER`을 사용 하십시오.  | 예, 몇 가지 [차이점이](sql-database-managed-instance-transact-sql-information.md#logins-and-users)있습니다. Windows 로그인은 지원 되지 않으며 Azure Active Directory 로그인으로 바꾸어야 합니다. |
...
| [시스템 데이터 수정](https://docs.microsoft.com/sql/relational-databases/databases/system-databases) | 아닙니다. | yes |
...
| [PolyBase](https://docs.microsoft.com/sql/relational-databases/polybase/polybase-guide) | 아닙니다. @No__t-0 함수를 사용 하 여 Azure Blob Storage에 저장 된 파일의 데이터를 쿼리할 수 있습니다. | 아닙니다. @No__t-0 함수를 사용 하 여 Azure Blob Storage에 저장 된 파일의 데이터를 쿼리할 수 있습니다. |
| [쿼리 알림](https://docs.microsoft.com/sql/relational-databases/native-client/features/working-with-query-notifications) | 아닙니다. | yes |
...
| [리소스 관리자](https://docs.microsoft.com/sql/relational-databases/resource-governor/resource-governor) | 아닙니다. | yes |
...
| [가용성 영역](/azure/availability-zones/az-overview) | yes | 아닙니다. |
| [Azure Resource Health](/azure/service-health/resource-health-overview) | yes | 아닙니다. |
...
| [하이퍼 확장 아키텍처](sql-database-service-tier-hyperscale.md) | yes | 아닙니다. |
| [장기 백업 보존-LTR](sql-database-long-term-retention.md) | 예, 자동으로 최대 10 년 동안 백업을 유지 합니다. | 아직 없습니다. @No__t-0 [수동 백업을](sql-database-managed-instance-transact-sql-information.md#backup) 임시 해결 방법으로 사용 합니다. |
...
| [SQL 데이터 동기화](sql-database-get-started-sql-data-sync.md) | yes | 아닙니다. |
...
| [QPI (Query Performance Insights)](sql-database-query-performance.md) | yes | 아닙니다. SQL Server Management Studio 및 Azure Data Studio에서 기본 제공 보고서를 사용 합니다. |
...
| VNet 서비스 끝점 | [예](sql-database-vnet-service-endpoint-rule-overview.md) | 아닙니다. |
...
| [SSMS(SQL Server Management Studio)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) | yes | 예 [버전 18.0 이상](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) |
