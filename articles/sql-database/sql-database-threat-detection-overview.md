...
위협 탐지 서비스는 처음 30일 동안 추가 비용이 들지 않는 체험 방식으로 제공되며, 그 이후에는 모든 데이터베이스에 대해 논리 서버 또는 Managed Instance당 매월 15달러의 비용이 부과됩니다.
...
- **잠재적 SQL 삽입**: SQL 삽입에 대해 식별된 애플리케이션 취약성으로 인해 활성 악용이 발생하면 이 경고가 트리거됩니다. 즉 공격자가 취약한 응용 프로그램 코드 또는 저장 프로시저를 사용하여 악성 SQL문을 삽입하려고 하는 것을 의미합니다.
- **비정상적인 위치에서 액세스**: SQL 서버에 대한 액세스 패턴이 변경되면 이 경고가 트리거됩니다. 이 경우 누군가가 비정상적인 지리적 위치에서 SQL 서버에 로그온했습니다. 일부 경우에서 경고는 합법적인 작업(새 응용 프로그램 또는 개발자 유지 관리)을 검색합니다. 다른 경우에서 경고는 악의적인 작업(퇴사 직원, 외부 공격자)을 검색합니다.
- **비정상적인 Azure 데이터 센터에서 액세스**: SQL 서버에 대한 액세스 패턴이 변경되면 이 경고가 트리거됩니다. 이 경우 누군가가 최근 기간 동안 이 서버에 표시된 비정상적인 Azure 데이터 센터에서 SQL 서버에 로그온했습니다. 일부 경우에서 경고는 합법적인 작업(Azure, Power BI, Azure SQL Query Editor의 새 응용 프로그램)을 검색합니다. 다른 경우에서 경고는 Azure 리소스/서비스의 악의적인 작업(퇴사 직원, 외부 공격자)을 검색합니다.
- **알 수 없는 보안 주체에서 액세스**: SQL 서버에 대한 액세스 패턴이 변경되면 이 경고가 트리거됩니다. 이 경우 누군가가 비정상적인 보안 주체(SQL 사용자)를 사용하여 SQL 서버에 로그온했습니다. 일부 경우에서 경고는 합법적인 작업(새 응용 프로그램, 개발자 유지 관리)을 검색합니다. 다른 경우에서 경고는 악의적인 작업(퇴사 직원, 외부 공격자)을 검색합니다.
...
SQL Database 위협 감지는 경고를 [Azure Security Center](https://azure.microsoft.com/services/security-center/)와 통합합니다. Azure Portal의 데이터베이스 및 SQL ATP 블레이드 내에 있는 라이브 SQL 위협 탐지 타일에서는 활성 위협의 상태를 추적합니다.
...
* [단일 데이터베이스 위협 탐지](sql-database-threat-detection.md)에 대한 자세한 정보 
