...
- 임의 지역의 임의 논리 서버에 최신 백업 지점으로 복구된 새 데이터베이스를 만듭니다.
...
데이터베이스를 아무 서비스 계층 또는 계산 크기에 단일 데이터베이스로 복원하거나 탄력적 풀에 복원할 수 있습니다. 논리 서버 또는 데이터베이스를 복원하는 탄력적 풀에 충분한 리소스가 있는지 확인합니다. 완료되면 복원된 데이터베이스는 일반적이고 완벽하게 액세스할 수 있는 온라인 데이터베이스입니다. 복원된 데이터베이스는 서비스 계층 및 계산 크기에 따라 정상 요금이 청구됩니다. 데이터베이스 복원이 완료될 때까지 요금이 발생하지 않습니다.
...
### <a name="recover-to-a-point-in-time-using-azure-portal"></a>Azure Portal을 사용하여 특정 시점으로 복구
...
### <a name="recover-a-deleted-database-using-the-azure-portal"></a>Azure Portal을 사용하여 삭제된 데이터베이스 복구
...
> [!Note]
> Managed Instance에서는 지역 복원을 사용할 수 없습니다.
