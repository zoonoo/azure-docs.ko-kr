...
description: 쿼리 편집기를 사용하여 Azure SQL Database에 대해 Transact-SQL(T-SQL) 쿼리를 실행하는 방법을 알아봅니다.
...
쿼리 편집기는 Azure SQL 데이터베이스 또는 Azure SQL Data Warehouse에 대해 SQL 쿼리를 실행하는 데 사용되는 Azure Portal 도구입니다. 

이 빠른 시작에서는 쿼리 편집기를 사용하여 Azure SQL 데이터베이스에 대해 Transact-SQL(T-SQL) 쿼리를 실행합니다.


## <a name="prerequisites"></a>사전 요구 사항

이 빠른 시작을 완료하려면 AdventureWorksLT 샘플 데이터베이스가 필요합니다. AdventureWorksLT SQL 데이터베이스의 작업 복사본이 없는 경우 다음 빠른 시작에 따라 신속하게 만듭니다.

- [빠른 시작: Azure Portal, PowerShell 또는 Azure CLI를 사용하여 단일 Azure SQL 데이터베이스 만들기](sql-database-single-database-get-started.md) 
...
> [!IMPORTANT]
> 쿼리 편집기는 포트 443 및 1443을 사용하여 통신합니다. 이러한 포트에서 아웃바운드 HTTPS 트래픽을 사용하도록 설정했는지 확인합니다. 또한 데이터베이스 및 데이터 웨어하우스에 액세스하려면 [서버의 허용된 방화벽 규칙에 아웃바운드 IP 주소를 추가](sql-database-server-level-firewall-rule.md)해야 합니다.


## <a name="open-the-sql-database-query-editor"></a>SQL Database 쿼리 편집기 열기

1. [Azure Portal](https://portal.azure.com/)에 로그인하고 쿼리하려는 SQL 데이터베이스를 선택합니다.
...
포털에 로그인되어 있더라도 SQL 데이터베이스에 액세스하려면 자격 증명을 입력해야 합니다. SQL 인증 또는 Azure Active Directory를 사용하여 데이터베이스에 연결할 수 있습니다.
...
Azure Active Directory(Azure AD) 관리자를 구성하면 단일 ID를 사용하여 Azure Portal과 SQL 데이터베이스에 로그인할 수 있습니다. Azure AD를 사용하여 데이터베이스에 연결하려면 다음 단계에 따라 SQL 서버의 Azure AD 관리자를 구성합니다.
...
#### <a name="set-an-active-directory-admin-for-the-database-server"></a>데이터베이스 서버의 Active Directory 관리자 설정

1. Azure Portal에서 SQL 서버를 선택합니다.

2. **SQL 서버** 메뉴에서 **Active Directory 관리자**를 선택합니다.

3. SQL 서버 **Active Directory 관리자** 페이지 도구 모음에서 **관리자 설정**을 선택하고, 사용자나 그룹을 Azure AD 관리자로 선택합니다.
...
6. **SQL Server** 메뉴에서 **SQL 데이터베이스**를 선택하고 SQL 데이터베이스를 선택합니다.

7. **SQL 데이터베이스** 메뉴에서 **쿼리 편집기(미리 보기)** 를 선택합니다. Azure AD 관리자인 경우 **로그인** 페이지의 **Active Directory 인증** 레이블 아래에 로그인되었다는 메시지가 나타납니다. *\<사용자 또는 그룹 ID>* (으)로 **계속** 단추를 선택합니다. 성공적으로 로그인되지 않았다는 내용이 페이지에 표시되면 페이지를 새로 고쳐야 할 수도 있습니다.

## <a name="query-a-sql-database"></a>SQL 데이터베이스 쿼리
...
Azure SQL 데이터베이스에서 지원되는 T-SQL(Transact-SQL)에 대해 자세히 알아보려면 [SQL Database로 마이그레이션 중 Transact-SQL 차이점 해결](sql-database-transact-sql-information.md)을 참조하세요.
