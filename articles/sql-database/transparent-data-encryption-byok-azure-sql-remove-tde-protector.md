...
다음 단계는 지정 된 데이터베이스의 가상 로그 파일 (VLF)에서 아직 사용 중인 TDE 보호기 지문을 확인 하는 방법을 간략하게 설명 합니다. 데이터베이스의 현재 TDE 보호기에 대 한 지문과 데이터베이스 ID를 실행 하 여 찾을 수 있습니다. SELECT [database_id],       [encryption_state], [encryptor_type],/*비대칭 키, AKV, certificate는 서비스 관리 키를 의미*합니다. [sys]의 encryptor_thumbprint]입니다. [dm_database_encryption_keys] 
 
다음 쿼리는 사용 중인 Vlf 및 해당 지문을 반환 합니다. 각 지문이 Azure Key Vault (AKV)에서 다른 키를 참조 합니다. SELECT * FROM dm_db_log_info (database_id) 
