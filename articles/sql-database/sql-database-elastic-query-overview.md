...
* **행 분할 – 분할** (토폴로지 2): 데이터가 수평 확장된 데이터 계층에서 수평으로 배포되기 위해 분할됩니다. 이 방법에서는 스키마가 모든 분할 데이터베이스에서 동일합니다. 이 방법을 "분할"이라고도 합니다. 분할은 (1)탄력적 데이터베이스 도구 라이브러리 또는 (2)자체 분할을 사용하여 수행 및 관리됩니다. 탄력적 쿼리는 여러 분할에서 보고서를 쿼리 또는 컴파일하는 데 사용됩니다. 샤드는 일반적으로 탄력적 풀 내의 데이터베이스입니다. 데이터베이스가 공통 스키마를 공유하는 한 탄력적 풀의 모든 데이터베이스를 한 번에 쿼리하는 효율적인 방법으로 탄력적 쿼리를 생각할 수 있습니다.

> [!NOTE]
> 탄력적 쿼리는 대부분의 처리(필터링, 집계)를 외부 소스 쪽에서 수행할 수 있는 보고 시나리오에 가장 적합합니다. 원격 데이터베이스에서 많은 양의 데이터를 전송하는 ETL 작업에는 적합하지 않습니다. 더 복잡한 쿼리가 있는 과대 보고 워크로드 또는 데이터 웨어하우징 시나리오의 경우 [Azure Synapse 분석을](https://azure.microsoft.com/services/synapse-analytics)사용하는 것도 고려해 보십시오.
>  
...
> [!IMPORTANT]
> 대규모 데이터베이스 집합에 대한 탄력적 쿼리를 성공적으로 실행하도록 하는 것은 쿼리 를 실행하는 동안 각 데이터베이스의 가용성에 크게 의존합니다. 데이터베이스 중 하나를 사용할 수 없는 경우 전체 쿼리가 실패합니다. 한 번에 수백 또는 수천 개의 데이터베이스를 쿼리하려는 경우 클라이언트 응용 프로그램에 재시도 논리가 포함되어 있는지 확인하거나 [Elastic Database Jobs(미리](https://docs.microsoft.com/azure/sql-database/sql-database-job-automation-overview#elastic-database-jobs-preview) 보기)를 활용하고 더 작은 데이터베이스 하위 집합을 쿼리하여 각 쿼리의 결과를 단일 대상으로 통합하는 것이 좋습니다.
...
* nvarchar(max)를 제외하고 LOB 유형(공간 형식 포함)은 외부 테이블 정의에서 지원되지 않습니다. 차선책으로, LOB 형식을 nvarchar(max)로 캐스팅하고, 기준 테이블 대신 뷰에서 외부 테이블을 정의한 다음 다시 쿼리에서 원래의 LOB 형식으로 캐스팅하는 원격 데이터베이스에서 뷰를 만들 수 있습니다.
...
* 탄력적 쿼리는 Azure SQL Database에서만 작동합니다. 온-프레미스 SQL Server 또는 VM의 SQL Server쿼리에는 사용할 수 없습니다.
