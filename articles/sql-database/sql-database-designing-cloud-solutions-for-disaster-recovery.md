<properties
   pageTitle="클라우드 재해 복구 솔루션 - SQL 데이터베이스 지역에서 복제 | Microsoft Azure"
...
# SQL 데이터베이스에서 지역에서 복제를 사용하여 클라우드 재해 복구를 위한 응용 프로그램 디자인

SQL 데이터베이스에서 지역에서 복제를 사용하여 클라우드 재해 복구 솔루션을 포함한 응용 프로그램을 디자인하는 방법을 알아봅니다. 무중단 업무 방식 계획을 위해서는 응용 프로그램 배포 토폴로지, 대상으로 하는 서비스 수준 계약, 트래픽 대기 시간 및 비용을 고려합니다. 이 문서에서 일반적인 응용 프로그램 패턴을 살펴보고 각 옵션의 이점 및 장단점을 설명합니다.

## 디자인 패턴 1: 배치된 데이터베이스와 클라우드 재해 복구에 대한 활성-수동 배포
...
> [AZURE.NOTE] [Azure traffic manager](traffic-manager-overview.md)는 이 문서 전체에서 설명을 위한 용도로만 사용됩니다. 장애 조치(failover) 라우팅 메서드를 지원하는 모든 부하 분산 솔루션을 사용할 수 있습니다.
...
2. 보조 데이터베이스를 호출하여 [데이터베이스 장애 조치(failover)를 시작](https://msdn.microsoft.com/library/azure/dn509573.aspx)합니다.

장애 조치(failover) 후 응용 프로그램은 보조 지역에서 사용자 요청을 처리하지만 주 데이터베이스는 이제 보조 지역에 있으므로 데이터베이스와 함께 배치 상태로 유지됩니다. 다음 다이어그램에서 확인할 수 있습니다. 모든 다이어그램에 실선은 활성 연결을 나타내고 점선은 일시 중단된 연결을 나타내며 중지 기호는 동작 트리거를 나타냅니다.
...
+ 응용 프로그램 및 데이터베이스는 항상 함께 배치되므로 응용 프로그램의 성능은 장애 조치(failover)의 영향을 받지 않습니다.
...
2. 지정된 보조 데이터베이스를 호출하여 [데이터베이스 장애 조치(failover)를 시작합니다](https://msdn.microsoft.com/library/azure/dn509573.aspx).
