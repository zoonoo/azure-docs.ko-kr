---
title: Always Encrypted-Windows 인증서 저장소
...
이 문서에서는 [SSMS (SQL Server Management Studio)](https://msdn.microsoft.com/library/hh213248.aspx)의 [Always Encrypted 마법사](https://msdn.microsoft.com/library/mt459280.aspx) 를 사용 하 여 데이터베이스 암호화를 사용 하 여 SQL 데이터베이스의 중요 한 데이터를 보호 하는 방법을 보여 줍니다. 그뿐 아니라 Windows 인증서 저장소에 암호화 키를 저장하는 방법을 보여 줍니다.
...
* SSMS의 Always Encrypted 마법사를 사용 하 여 [Always Encrypted 키](https://msdn.microsoft.com/library/mt163865.aspx#Anchor_3)를 만듭니다.
...
2. **리소스** > 만들기**데이터 + 저장소** > **SQL Database**를 클릭 합니다.
...
1. **SQL** > database**클리닉** > **데이터베이스 연결 문자열 표시**를 클릭 합니다.
...
1. SSMS를 엽니다. **연결** > **데이터베이스 엔진** 을 클릭 하 여 **서버에 연결** 창이 열려 있지 않으면 엽니다.
...
1. **데이터베이스** > **Clinic**클리닉 > **테이블**을 확장 합니다.
...
**클리닉** > **Security**보안 > **Always Encrypted 키**로 이동 하 여 SSMS에서 키 만들기를 확인할 수 있습니다. 이제 마법사에서 생성한 새 키를 볼 수 있습니다.
