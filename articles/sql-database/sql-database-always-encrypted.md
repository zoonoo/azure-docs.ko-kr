<properties
	pageTitle="SQL 데이터베이스 자습서: 상시 암호화로 중요한 데이터 보호 | Microsoft Azure"
	description="몇 분 만에 SQL 데이터베이스의 중요한 데이터를 보호합니다."
	keywords="sql 데이터베이스 자습서, sql 데이터베이스에 데이터 암호화"	
...
상시 암호화는 서버에서 중요한 미사용 데이터를 보호하는 Azure SQL 데이터베이스 및 SQL Server의 새 암호화 기술입니다. 데이터를 사용 중인 기간 뿐만 아니라 클라이언트와 서버 간에 이동하는 동안에도 중요한 데이터가 데이터베이스 시스템 내부에 일반 텍스트 형식으로 표시되지 않도록 합니다. 키에 액세스할 수 있는 클라이언트 응용 프로그램 또는 앱 서버는 일반 텍스트 데이터에 액세스할 수 있습니다. 자세한 내용은 [상시 암호화(데이터베이스 엔진)](https://msdn.microsoft.com/library/mt163865.aspx)을 참조하세요.

이 자습서에서는 SSMS(SQL Server Management Studio)를 사용하여 단 몇 분 후에 SQL 데이터베이스에 상시 암호화를 구현하여 중요한 데이터를 보호하는 방법을 보여줍니다.
...
- SSMS에서 상시 암호화 마법사를 사용하여 [상시 암호화 키](https://msdn.microsoft.com/library/mt163865.aspx#Anchor_3) 만들기
    - [열 마스터 키(CMK)](https://msdn.microsoft.com/library/mt146393.aspx)를 만듭니다.
    - [열 암호화 키(CEK)](https://msdn.microsoft.com/library/mt146372.aspx)를 만듭니다.
...
- 시작하기 전에 Azure 계정 및 구독. 없는 경우 지금 [무료 평가판](http://azure.microsoft.com/pricing/free-trial/)에 등록하세요.
...
나중에 자습서에서 연결 문자열이 필요하므로 데이터베이스를 만든 후에 새 클리닉 데이터베이스를 찾아서 연결 문자열을 복사합니다.(언제든지 연결 문자열을 얻을 수 있지만 이미 포털에 있으므로 이제 복사하기 쉽습니다)

1. **SQL 데이터베이스** > **클리닉** > **데이터베이스 연결 문자열 표시**를 클릭합니다.
2. **ADO.NET**에 연결 문자열을 복사합니다.
...
2. 서버 이름 및 자격 증명을 입력합니다. 앞에서 복사한 SQL 데이터베이스 블레이드 및 연결 문자열에 서버 이름을 찾아볼 수 있습니다. database.windows.net을 포함하여 전체 서버 이름을 입력합니다.

	![연결 문자열 복사](./media/sql-database-always-encrypted/ssms-connect.png)

3. **새 방화벽 규칙** 창이 열리면 Azure에 로그인하고 SSMS이 새 방화벽 규칙을 만들도록 합니다.


## 테이블 만들기

먼저 환자 데이터를 유지할 테이블을 만듭니다.(처음부터 일반적인 테이블로 다음 섹션에서 암호화를 구성합니다)

1. **데이터베이스**를 확장합니다.
1. **클리닉** 데이터베이스를 마우스 오른쪽 단추로 클릭하고 **새 쿼리**를 클릭합니다.
...
SSMS는 쉽게 열 마스터 키(CMK), 열 암호화 키(CEK) 및 암호화된 열을 설정하여 상시 암호화를 쉽게 구성하는 마법사를 제공합니다.

1. **데이터베이스** > **클리닉** > **테이블**을 확장합니다.
2. 마우스 오른쪽 단추로 **환자** 테이블을 클릭하고 **열 암호화...**를 선택하여 상시 암호화 마법사를 엽니다.

    ![열 암호화](./media/sql-database-always-encrypted/encrypt-columns.png)

3. **열 선택**

    **소개** 페이지에서 **다음**을 클릭하여 암호화하려는 열, [암호화의 형식 및 사용할 열 암호화 키(CEK)](https://msdn.microsoft.com/library/mt459280.aspx#Anchor_2)를 선택하는 **열 선택** 페이지를 엽니다.

    각 환자의 경우 해당 **SSN** 및 **BirthDate** 정보를 암호화하려 합니다. SSN 열은 같음 조회, 조인 및 그룹화를 지원하는 결정적 암호화를 사용합니다. BirthDate 열은 작업을 지원하지 않는 임의의 암호화를 사용합니다.

    SSN 열에 대한 **암호화 형식**을 **결정적**에 대해 선택하고 설정하며 BirthDate 열을 **무작위**에 선택하고 설정하여 **다음**을 클릭합니다.

    ![열 암호화](./media/sql-database-always-encrypted/column-selection.png)

4. **마스터 키 구성**(CMK)

    **마스터 키 구성** 페이지는 열 마스터 키(CMK)를 설치하고 CMK가 저장될 키 저장소 공급자를 선택합니다. 현재 Windows 인증서 저장소, Azure 키 자격 증명 또는 하드웨어 보안 모듈(HSM)에 CMK를 저장할 수 있습니다. 이 자습서에는 Windows 인증서 저장소에 키를 저장하는 방법을 보여줍니다.

    **Windows 인증서 저장소**를 선택하고 **다음**을 클릭했음을 확인합니다.
...
마법사가 완료된 후에 데이터베이스는 상시 암호화에 대해 설정되어 다음이 완료됩니다.

- 만든 열 마스터 키(CMK).
- 만든 열 암호화 키(CEK).
- 암호화에 선택한 열을 구성합니다. (환자 테이블에는 현재 데이터가 없지만 이제 선택된 열의 기존 데이터가 암호화됩니다.)

**클리닉** > **보안** > **상시 암호화 키**를 확장하여 SSMS에서 키 만들기를 확인할 수 있습니다. 이제 마법사에서 생성한 새 키를 볼 수 있습니다.


## 암호화된 데이터로 작동하는 클라이언트 응용 프로그램 만들기

상시 암호화가 설치되었으므로 암호화된 열에서 일부를 삽입 및 선택하는 응용 프로그램 빌드해보겠습니다. 샘플 응용 프로그램을 성공적으로 실행하려면 상시 암호화 마법사를 실행한 동일한 컴퓨터에서 실행해야 합니다. 다른 컴퓨터에서 실행하려면 클라이언트 앱을 실행하는 컴퓨터에 상시 암호화 인증서를 배포해야 합니다.
...
이 섹션에는 단순히 데이터베이스 연결 문자열에서 상시 암호화를 사용하는 방법을 설명합니다. 다음 섹션 **상시 암호화 샘플 콘솔 응용 프로그램**에서 실제로 방금 만든 콘솔 앱을 수정합니다.


상시 암호화를 사용하려면 **열 암호화 설정** 키워드를 연결 문자열에 추가하고 **사용함**으로 설정해야 합니다.

이 연결 문자열에서 직접 설정하거나 [SqlConnectionStringBuilder](https://msdn.microsoft.com/library/system.data.sqlclient.sqlconnectionstringbuilder.aspx)를 사용하여 설정할 수 있습니다. 다음 섹션에서 샘플 응용 프로그램은 **SqlConnectionStringBuilder**를 사용하는 방법을 보여줍니다.
...
**Program.cs**의 내용을 다음과 같이 바꿉니다. Main 메서드 바로 위의 줄에서 전역 connectionString 변수에 대한 연결 문자열을 Azure 포털에서 유효한 연결 문자열로 바꿉니다. 이 코드에 대한 유일한 변경 내용입니다.

이제 작업에서 상시 암호화를 확인하려면 앱을 실행합니다.
...
서버의 실제 데이터가 암호화되었는지 빠른 검사를 위해 SSMS로 환자 데이터를 쉽게 쿼리할 수 있습니다(열 암호화 설정이 아직 사용되지 않는 현재 연결을 사용).

클리닉 데이터베이스에서 다음 쿼리를 실행합니다.
