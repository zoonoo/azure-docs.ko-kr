...
- **하이브리드 데이터 동기화:** 데이터 동기화를 사용 하면 온-프레미스 데이터베이스와 Azure SQL 데이터베이스 간에 데이터를 동기화 된 상태로 유지 하 여 하이브리드 응용 프로그램을 사용할 수 있습니다. 이 기능은 클라우드로 이동하려는 고객에게 표시되고 Azure에 애플리케이션의 일부를 배치할 수 있습니다.
