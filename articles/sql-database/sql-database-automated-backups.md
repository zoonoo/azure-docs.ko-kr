...
낮은 성능 계층으로 다운그레이드 하면 복원 지점의 보존 기간은 현재 데이터베이스 성능 계층의 보존 기간으로 즉시 단축됩니다. 서비스 계층을 업그레이드 한 경우 보존 기간은 데이터베이스를 업그레이드한 후에만 확장이 시작됩니다. 예를 들어 데이터베이스가 P1에서 S3로 다운그레이드한 경우 보존 기간은 35일에서 35일로 즉시 변경되고, 35일 이전의 복원 지점을 더는 사용할 수 없게 됩니다. 이후에 다시 P1으로 업그레이드할 경우 보존 기간은 35일에서 시작하여 35일까지 작성되기 시작합니다.
