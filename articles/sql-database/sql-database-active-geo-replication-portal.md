---
title: '자습서: 포털에서 지역에서 복제 & 장애 조치 (failover)'
description: Azure Portal를 사용 하 여 Azure SQL Database에서 단일 또는 풀링된 데이터베이스에 대 한 지역에서 복제를 구성 하 고 장애 조치를 시작 합니다.
...
1. [Azure Portal](https://portal.azure.com)에서 지역에서 복제에 대해 설정 하려는 데이터베이스를 찾습니다.
...
2. SQL Database 블레이드에서 **모든 설정** > **지역에서 복제**를 선택 합니다.
3. **보조** 목록에서 새 주 데이터베이스로 사용할 데이터베이스를 선택 하 고 **강제 장애 조치 (Failover)** 를 클릭 합니다.
...
* 활성 지역 복제에 대해 자세히 알아보려면 [활성 지역 복제](sql-database-active-geo-replication.md)를 참조 하세요.
* 자동 장애 조치(failover) 그룹에 대해 알아보려면 [자동 장애 조치(failover) 그룹](sql-database-auto-failover-group.md)을 참조하세요.
* 비즈니스 연속성 개요 및 시나리오는 [비즈니스 연속성 개요](sql-database-business-continuity.md)를 참조 하세요.
