...
description: Azure 모니터 로그를 사용하여 Azure SQL 데이터 동기화를 모니터링하는 방법 알아보기
...
문제를 확인하기 위해 더 이상 각 동기화 그룹의 로그를 개별적으로 검색할 필요가 없습니다. 사용자 지정 Azure Monitor 보기를 사용하여 구독의 모든 동기화 그룹을 한 곳에서 모니터링할 수 있습니다. 이 보기는 SQL 데이터 동기화 고객에게 중요한 정보를 표시합니다.
...
Azure Portal이나 PowerShell 또는 REST API를 통해 로그를 수동으로 확인할 필요가 없습니다. [Azure Monitor 로그를](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)사용하면 오류가 발생할 때 확인해야 하는 사람의 전자 메일 주소로 직접 이동하는 경고를 만들 수 있습니다.
...
다음과 같은 작업을 수행하여 SQL Data Sync에 대한 사용자 지정 Azure Monitor 로그 모니터링 솔루션을 1시간 이내에 구현합니다.

세 가지 구성 요소를 구성해야 합니다.

-   SQL 데이터 동기화 로그 데이터를 Azure 모니터 로그에 공급하는 PowerShell 런북입니다.

-   전자 메일 알림에 대한 Azure 모니터 경고입니다.

-   모니터링을 위한 Azure 모니터 보기입니다.
...
Azure 자동화에서 호스팅되는 PowerShell Runbook을 사용하여 SQL 데이터 동기화 로그 데이터를 가져와 Azure 모니터 로그로 보냅니다. 샘플 스크립트가 포함되어 있습니다. 필수 구성 요소로서 Azure Automation 계정이 있어야 합니다. 그런 다음 Runbook을 만들고 실행 일정을 예약해야 합니다. 
...
10. 계정 및 SQL 데이터 동기화 구성에 필요한 대로 변경합니다. (자세한 내용은 샘플 스크립트를 참조하세요.)
    ...
    3.  Azure 모니터는 정보를 기록합니다. 이 정보는 Azure Portal | 설정 | 연결된 원본에서 찾습니다. Azure Monitor 로그로 데이터를 보내는 것에 대한 자세한 내용은 [HTTP 데이터 수집기 API(미리 보기)를 사용하여 Azure Monitor 로그로 데이터 보내기](../azure-monitor/platform/data-collector-api.md)를 참조하십시오.
...
3.  **Runbook에 일정 링크를 선택합니다.**

4.  **새 일람표 작성을 선택합니다.**

5.  **되풀이**를 '되풀이'로 설정하고 원하는 간격을 설정합니다. 스크립트 및 Azure Monitor 로그에서 여기에서 동일한 간격을 사용합니다.
...
## <a name="create-an-azure-monitor-reader-alert-for-email-notifications"></a>전자 메일 알림에 대한 Azure 모니터 판독기 경고 만들기

Azure Monitor 로그를 사용하는 경고를 만들려면 다음 작업을 수행합니다. 전제 조건으로 Azure Monitor 로그가 로그 분석 작업 영역과 연결되어 있어야 합니다.
...
4.  **경고 발생 기준** 아래에서 **메트릭 측정**을 선택합니다.
    ...
    2.  알림을 받기 전에 경과할 임계값을 **다음 값보다 큼** 뒤에 입력합니다. 데이터 동기화에서 일시적인 오류가 예상됩니다. 노이즈를 줄이려면 임계값을 5로 설정합니다.

5.  **작업** 아래에서 **전자 메일 알림**을 "예"로 설정합니다. 원하는 전자 메일 받는 사람을 입력합니다.

6.  **저장**을 클릭합니다. 이제 오류가 발생하면 지정된 받는 사람이 전자 메일 알림을 받습니다.

## <a name="create-an-azure-monitor-view-for-monitoring"></a>모니터링을 위한 Azure 모니터 보기 만들기

이 단계에서는 지정된 모든 동기화 그룹을 시각적으로 모니터링하는 Azure Monitor 뷰를 만듭니다. 이 보기에는 다음과 같은 몇 가지 구성 요소가 포함되어 있습니다.
...
Azure 모니터 보기를 구성하려면 다음 작업을 수행합니다.

1.  로그 분석 작업 영역 홈 페이지에서 왼쪽의 더구를 선택하여 **보기 디자이너를**엽니다.
...
**Azure 모니터 로그:** 사용량에 따라 Azure Monitor 로그와 관련된 비용이 있을 수 있습니다. 체험 계층에서는 매일 500MB의 데이터를 수집할 수 있습니다. 대부분의 경우 이 솔루션은 하루에 500MB 미만으로 데이터를 수집해야 합니다. 사용량을 줄이려면 Runbook에 포함된 실패 전용 필터링을 사용합니다. 하루에 500MB를 초과하여 사용하는 경우 유료 계층으로 업그레이드하여 제한 도달 시 분석이 중지되는 위험을 방지합니다. 자세한 내용은 [Azure 모니터 로그 가격 책정을](https://azure.microsoft.com/pricing/details/log-analytics/)참조하십시오.
