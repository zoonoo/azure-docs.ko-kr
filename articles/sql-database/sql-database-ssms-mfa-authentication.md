---
title: 다단계 AAD 인증 사용
description: Azure SQL 데이터베이스 및 Azure Synapse는 활성 디렉터리 범용 인증을 사용하여 SSMS(SQL Server 관리 스튜디오)의 연결을 지원합니다.
...
---
# <a name="using-multi-factor-aad-authentication-with-azure-sql-database-and-azure-synapse-analytics-ssms-support-for-mfa"></a>Azure SQL 데이터베이스 및 Azure 시냅스 분석(MFA용 SSMS 지원)을 사용하여 다단계 AAD 인증 사용
Azure SQL 데이터베이스 및 Azure Synapse는 *활성 디렉터리 범용 인증을*사용하여 SSMS(SQL Server 관리 스튜디오)의 연결을 지원합니다. 이 문서에서는 다양한 인증 옵션간의 차이점과 범용 인증 사용과 관련된 제한 사항에 대해설명합니다. 
...
이 문서에서 설명하는 모든 기능에 대해서는 적어도 7월 2017년 버전 17.2를 사용하십시오.  가장 최근의 연결 대화 상자는 다음 이미지와 유사해야 합니다.
 
  ![1mfa-universal-connect](./media/sql-database-ssms-mfa-auth/1mfa-universal-connect.png "사용자 이름 상자를 완료합니다.")  

## <a name="the-five-authentication-options"></a>5가지 인증 옵션  

Active Directory 범용 인증은 두 가지 비대화형 인증 방법을 지원합니다.
    - `Active Directory - Password`인증
    - `Active Directory - Integrated`인증

다양한 응용 프로그램(ADO.NET, JDCB, ODC 등)에서 사용할 수 있는 두 가지 비대화형 인증 모델도 있습니다. 이 두 가지 방법은 팝업 대화 상자를 생성하지 않습니다. 
- `Active Directory - Password` 
- `Active Directory - Integrated` 

대화형 메서드는 Azure 다단계 인증(MFA)도 지원하는 것입니다. 
...
Azure AD B2B 시나리오에 대 한 지원 Azure AD 사용자 게스트 사용자(Azure [B2B 공동 작업 이란 참조)는](../active-directory/active-directory-b2b-what-is-azure-ad-b2b.md)현재 Azure AD에서 만든 그룹의 구성원의 일부로만 `CREATE USER` SQL Database 및 Azure Synapse에 연결할 수 있으며 지정된 데이터베이스에서 Transact-SQL 문을 사용하여 수동으로 매핑됩니다. 예를 들어 `steve@gmail.com`이 Azure AD `contosotest`(Azure Ad 도메인 `contosotest.onmicrosoft.com`)에 초대된 경우 Azure AD 그룹(예: `usergroup`)이 `steve@gmail.com` 구성원을 포함하는 Azure AD에 만들어져야 합니다. 그런 다음 이 그룹은 Transact-SQL `CREATE USER [usergroup] FROM EXTERNAL PROVIDER` 문을 실행하여 Azure AD SQL 관리자 또는 Azure AD DBO에 의해 특정 데이터베이스(즉, MyDatabase)에 대해 만들어야 합니다. 데이터베이스 사용자가 만들어지면 `steve@gmail.com` 사용자가 `Active Directory – Universal with MFA support` SSMS 인증 옵션을 사용하여 `MyDatabase`에 로그인할 수 있습니다. 기본적으로 사용자 그룹은 연결 권한만 있으며 추가적인 액세스 권한은 일반적인 방식으로 부여 받아야 합니다. 게스트 사용자인 `steve@gmail.com` 사용자는 확인란을 선택하고 SSMS **연결 속성** 대화 상자에서 `contosotest.onmicrosoft.com` AD 도메인 이름을 추가해야 합니다. **AD 도메인 이름 또는 테넌트 ID** 옵션은 MFA 연결 옵션이 있는 유니버설에서만 지원되며 그 밖의 경우는 회색으로 표시됩니다.

## <a name="universal-authentication-limitations-for-sql-database-and-azure-synapse"></a>SQL 데이터베이스 및 Azure 시냅스에 대한 범용 인증 제한
...
- [SQL 데이터베이스 또는 Azure Synapse를 사용하여 Azure Active Directory 인증 구성 및 관리](sql-database-aad-authentication-configure.md)  
