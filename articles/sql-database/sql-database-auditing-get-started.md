...
5. SQL 데이터베이스에 대한 감사를 구성하는 경우 **감사를 적용하려면 여기를 클릭하세요...** 및 **보안 지원 액세스**를 클릭하고 **필수**를 선택합니다. SQL Server에 대한 감사를 구성하는 경우 두 가지 옵션이 있습니다. (a) #6단계를 수행한 후 서버의 각 SQL 데이터베이스를 탐색하고 이 단계 적용 또는 (2) [연결 문자열의 서버 FDQN 수정](sql-database-auditing-and-dynamic-data-masking-downlevel-clients.md).
