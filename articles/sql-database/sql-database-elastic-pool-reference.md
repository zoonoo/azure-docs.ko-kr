...
- 탄력적 데이터베이스 풀 만들기 및 관리는 [Azure 포털](https://portal.azure.com), [PowerShell](sql-database-elastic-pool-create-powershell.md) 및 .NET 클라이언트 라이브러리(Azure Resource Manager만 해당)를 사용하여 지원합니다. [클래식 포털](https://manage.windowsazure.com/) 및 서비스 관리 명령은 지원되지 않습니다.
