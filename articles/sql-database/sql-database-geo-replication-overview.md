...
>[AZURE.NOTE] **sp_wait_for_database_copy_sync** 프로시저 호출로 인해 발생한 지연은 상당히 클 수 있습니다. 지연 시간은 현재 트랜잭션 로그 길이에 따라 달라지며 이 호출은 전체 로그가 복제될 때까지 반환되지 않습니다. 반드시 필요하지 않는 한 이 프로시저 호출을 피합니다.
