...
### <a name="ParamSniffing"></a>PSP (매개 변수 구분 계획) 문제가 있는 쿼리
...
이 예제에서 `t1.c1`는 `@p1`를 사용 하지만 `t2.c2`는 계속 해 서 리터럴로 GUID를 사용 합니다. 이 경우 `c2`값을 변경 하면 쿼리가 다른 쿼리로 처리 되며 새로운 컴파일이 수행 됩니다. 이 예제에서 컴파일을 줄이려면 GUID도 매개 변수화 합니다.
...
- [Dmv](sql-database-monitoring-with-dmvs.md)를 사용 하 여 리소스 문제 검색:
  ...
  - [누적 된 많은 개별 쿼리는 높은 CPU를 사용 합니다.](sql-database-monitoring-with-dmvs.md#many-individual-queries-that-cumulatively-consume-high-cpu)
  - 
