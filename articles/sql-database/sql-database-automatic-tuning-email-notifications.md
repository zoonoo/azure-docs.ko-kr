...
3. Office 365 통합을 사용하여 메일을 전송하기 위한 작업을 만듭니다.
   ...
   - "**+ 새 단계**"를 선택 하 고 되풀이 흐름 창 내에서 "**작업 추가**"를 선택 합니다.
   - 검색 필드에 "**전자 메일 보내기**"를 입력 하 고 검색 결과에서 "**Office 365 Outlook-전자 메일 보내기**"를 선택 합니다.
...
> [!TIP]
> 자동화된 전자 메일을 다른 받는 사람에게 전송하려면 별도 흐름을 만듭니다. 이러한 추가 흐름에서 "받는 사람" 필드의 받는 사람 전자 메일 주소를 변경 하 고 "제목" 필드에 전자 메일 제목 줄을 변경 합니다. 사용자 지정된 PowerShell 스크립트를 사용하여 Azure Automation에서 새 Runbook을 만들면(예: Azure 구독 ID 변경), 별도의 구독마다 별도의 받는 사람에게 자동 조정 권장 사항에 대한 전자 메일을 전송하는 것과 같이 자동화 시나리오를 추가로 사용자 지정할 수 있습니다.
>
