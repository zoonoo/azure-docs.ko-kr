...
- 온-프레미스 클라이언트 컴퓨터에서 PowerShell 5.1 및 AZ PowerShell 1.4.0 이상요구. 필요한 경우 [Azure PowerShell 모듈 설치](https://docs.microsoft.com/powershell/azure/install-az-ps#install-the-azure-powershell-module) 지침을 참조하세요.
...
2. 이 PowerShell 스크립트를 복사합니다. 이 스크립트는 [Managed Instance 만들기](sql-database-managed-instance-get-started.md) 빠른 시작에서 만든 Managed Instance 가상 네트워크에 VPN 게이트웨이를 연결합니다. 이 스크립트는 Azure PowerShell Az 모듈을 사용하며 Windows 또는 Linux 기반 호스트에 대해 다음을 수행합니다.
...
> [!IMPORTANT]
> PowerShell 스크립트가 완료될 때까지 계속하지 마십시오.
...
4. 온-프레미스 클라이언트 컴퓨터에서 zip 파일에서 파일을 추출한 다음 추출된 파일로 폴더를 엽니다.
...
8. 가상 네트워크를 참조하는 대화 상자에서 **예를** 선택하여 가상 네트워크에 대한 VPN 클라이언트를 설치합니다.

## <a name="connect-to-the-vpn-connection"></a>VPN 연결에 연결

1. 온-프레미스 클라이언트 컴퓨터에서 **네트워크 & 인터넷의** **VPN으로** 이동하여 관리형 인스턴스 가상 네트워크를 선택하여 이 VNet에 대한 연결을 설정합니다. 다음 이미지에서 VNet 이름은 **MyNewVNet**입니다.
...
- 기존 SQL Server 데이터베이스를 온-프레미스에서 관리되는 인스턴스로 복원하려면 [Azure 데이터베이스 마이그레이션 서비스(DMS)를](../dms/tutorial-sql-server-to-managed-instance.md) 사용하여 마이그레이션하거나 [T-SQL RESTORE 명령을](sql-database-managed-instance-get-started-restore.md) 사용하여 데이터베이스 백업 파일에서 복원할 수 있습니다.
