...
* **가치 및 비즈니스 모델**. 응용 프로그램의 테넌트별 수익이&1;달러 미만 등으로 적은 경우에는 격리 요구 사항의 중요성이 낮아지므로 공유 데이터베이스를 사용하는 것이 적절합니다. 테넌트별 수익이 몇 달러 이상이면 테넌트별 데이터베이스 모델이 더 타당합니다. 개발 비용을 줄이는 데에도 도움이 될 수 있습니다.
