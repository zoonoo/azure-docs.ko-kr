...
전체 확장 프로세스 기간은 대부분의 경우에서 5분 미만이 걸립니다. 

[Azure SQL Database 서비스 계층 및 성능 수준](sql-database-service-tiers.md)의 정보를 사용하여 Azure SQL Database에 대해 적절한 서비스 계층 및 성능 수준을 결정합니다.
...
## <a name="change-the-service-tier-and-performance-level-of-your-database"></a>데이터베이스의 서비스 계층 및 성능 수준 변경
규모 확장 또는 축소할 데이터베이스에 대한 SQL 데이터베이스 블레이드를 엽니다.

1. [Azure Portal](https://portal.azure.com)에서 **추가 서비스** > **SQL 데이터베이스**를 클릭합니다.
2. 변경할 데이터베이스를 클릭합니다.
3. **SQL 데이터베이스** 블레이드에서 **가격 책정 계층(배율 DTU)**을 클릭합니다.
...
5. 오른쪽 위에 있는 알림 아이콘(벨)을 클릭합니다.
   ...
   알림 텍스트를 클릭하여 요청의 상태를 볼 수 있는 세부 정보 창을 엽니다.
