...
- [Azure 저장소 계정(클래식)](storage-create-storage-account.md) Blob 컨테이너에서 가져오려는 데이터베이스의 .bacpac 파일
...
3.	SQL Server 블레이드에서 **데이터베이스 가져오기**를 클릭하여 **데이터베이스 가져오기** 블레이드를 엽니다.
...
1.  **데이터베이스 이름**을 입력합니다.
2.  데이터베이스를 가져올 Azure SQL Server에 대한 **서버 관리자 로그인** 및 **암호**를 입력합니다.
1.  **만들기**를 클릭하여 BACPAC에서 데이터베이스를 만듭니다.
...
2.	**모두 찾아보기**를 클릭합니다.
3.	**SQL 데이터베이스**를 클릭하고 새 데이터베이스가 **온라인** 상태인지 확인합니다.
