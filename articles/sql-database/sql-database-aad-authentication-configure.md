...
description: Azure AD를 구성한 후 Azure Active Directory 인증을 사용 하 여 SQL Database, 관리 되는 인스턴스 및 Azure Synapse Analytics에 연결 하는 방법을 알아봅니다.
...
이 문서에서는 azure AD를 만들고 채운 다음 azure ad ( [SQL DB)](sql-database-technical-overview.md), [MI (관리 되는 인스턴스](sql-database-managed-instance.md)) 및 [azure Synapse Analytics (이전의 AZURE SQL DATA WAREHOUSE)](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md)에서 azure SQL Database ad를 사용 하는 방법을 보여 줍니다. 개요는 [Azure Active Directory 인증](sql-database-aad-authentication.md)을 참조하세요.

> [!NOTE]
> 이 문서는 Azure SQL server에 적용 되며 SQL Database 및 Azure Synapse 모두에 적용 됩니다. 간단히 하기 위해 SQL Database와 Azure Synapse를 둘 다 참조할 때 SQL Database 사용 됩니다.
...
Azure AD 인증 방법에 대 한 자세한 내용 및 Azure AD 인증 방법에 대 한 자세한 내용은 다음 문서를 참조 하세요.
...
위의 모든 인증 방법은 SQL DB (단일 데이터베이스 및 데이터베이스 풀), 관리 되는 인스턴스 및 Azure Synapse에 대해 지원 됩니다.
...
5. 관리자 **추가** 페이지에서 사용자를 검색 하 고 관리자가 될 사용자 또는 그룹을 선택한 다음 **선택**을 선택 합니다. Active Directory 관리자 페이지에는 Active Directory의 모든 멤버와 그룹이 표시됩니다. 회색으로 표시된 사용자나 그룹은 Azure AD 관리자로 지원되지 않기 때문에 선택할 수 없습니다. ( [SQL Database 또는 Azure Synapse를 사용 하 여 인증을 위해 Azure Active Directory 인증 사용](sql-database-aad-authentication.md)의 **Azure AD 기능 및 제한 사항** 섹션에서 지원 되는 관리자 목록을 참조 하세요.) RBAC (역할 기반 액세스 제어)는 포털에만 적용 되 고 SQL Server로 전파 되지 않습니다.
...
### <a name="powershell-for-azure-sql-database-and-azure-synapse"></a>Azure SQL Database 및 Azure Synapse 용 PowerShell
...
|[az sql server ad-admin list](/cli/azure/sql/server/ad-admin#az-sql-server-ad-admin-list) | 현재 Azure SQL server 또는 Azure Synapse에 대해 구성 된 Azure Active Directory 관리자에 대 한 정보를 반환 합니다. |
|[az sql server ad-admin update](/cli/azure/sql/server/ad-admin#az-sql-server-ad-admin-update) | Azure SQL server 또는 Azure Synapse에 대 한 Active Directory 관리자를 업데이트 합니다. |
...
> [!NOTE]
> 데이터베이스 사용자(관리자 예외)는 Azure Portal을 사용하여 만들 수 없습니다. RBAC 역할은 SQL Server, SQL Database 또는 Azure Synapse에 전파 되지 않습니다. Azure RBAC 역할은 Azure 리소스 관리에 사용되며 데이터베이스 사용 권한에는 적용되지 않습니다. 예를 들어 **SQL Server 참여자** 역할은 SQL Database 또는 Azure Synapse에 연결할 수 있는 액세스 권한을 부여 하지 않습니다. TRANSACT-SQL 문을 사용하여 데이터베이스에 직접 액세스 권한을 부여해야 합니다.
...
## <a name="connect-to-the-user-database-or-azure-synapse-by-using-ssms-or-ssdt"></a>SSMS 또는 SSDT를 사용 하 여 사용자 데이터베이스 또는 Azure Synapse에 연결  
...
2. **옵션** 단추를 선택하고 **연결 속성** 페이지의 **데이터베이스에 연결** 상자에서 연결하려는 사용자 데이터베이스의 이름을 입력합니다. 자세한 내용은 SSMS 17.x 및 18.x의 연결 속성 간의 차이점에 대 한 [MULTI-FACTOR AAD auth](sql-database-ssms-mfa-authentication.md#azure-ad-domain-name-or-tenant-id-parameter) 문서를 참조 하세요. 
...
Azure AD 클라우드 전용 id 사용자 또는 Azure AD 하이브리드 id를 사용 하는 사용자를 사용 하 여 SQL DB 또는 MI에 인증 하려면이 방법을 사용 합니다. 이 메서드는 해당 Windows 자격 증명을 사용 하려는 사용자를 지원 하지만 로컬 컴퓨터는 도메인에 가입 되지 않습니다 (예: 원격 액세스 사용). 이 경우 Windows 사용자는 도메인 계정 및 암호를 나타낼 수 있으며 SQL DB, MI 또는 Azure Synapse를 인증할 수 있습니다.
...
암호를 대화형으로 요청 하는 MFA (multi-factor authentication)를 사용 하거나 사용 하지 않는 대화형 인증에이 방법을 사용 합니다. 이 메서드는 azure AD 클라우드 전용 id 사용자 또는 azure AD 하이브리드 id를 사용 하는 사용자를 위해 SQL DB, MI 및 Azure Synapse에 인증 하는 데 사용할 수 있습니다.

자세한 내용은 [Azure SQL Database 및 Azure Synapse Analytics에서 MULTI-FACTOR AAD 인증 사용 (MFA에 대 한 SSMS 지원)](sql-database-ssms-mfa-authentication.md)을 참조 하세요.
...
이 인증 방법을 사용 하면 AAD (Azure Active Directory)에서 토큰을 가져와 Azure SQL Database 또는 Azure Synapse에 연결 하기 위해 중간 계층 서비스에서 [JWT (JSON 웹 토큰)](../active-directory/develop/id-tokens.md) 를 가져올 수 있습니다. 이 방법을 사용 하면 인증서 기반 인증을 사용 하 여 서비스 id, 서비스 주체 및 응용 프로그램을 비롯 한 다양 한 응용 프로그램 시나리오를 사용할 수 있습니다 Azure AD 토큰 인증을 사용하려면 다음 네 가지 기본 단계를 완료해야 합니다.
...
- SQL Database의 로그인, 사용자, 데이터베이스 역할 및 권한에 대 한 개요는 [로그인, 사용자, 데이터베이스 역할 및 사용자 계정](sql-database-manage-logins.md)을 참조 하십시오.
