...
description: Azure Active Directory를 구성한 후에 Azure AD 인증을 사용하여 SQL Database, Managed Instance 및 SQL Data Warehouse에 연결하는 방법을 알아봅니다.
...
1. 해당 디렉터리를 데이터베이스를 호스트하는 Azure 구독에서 신뢰할 수 있는 디렉터리로 만들어 Azure Active Directory에 Azure 구독을 연결합니다. 자세한 내용은 [Azure 구독과 Azure AD의 연관 관계](../active-directory/fundamentals/active-directory-how-subscriptions-associated-directory.md)를 참조하세요.
...
> [!IMPORTANT]
> Managed Instance를 프로비전하는 경우에만 다음 단계를 수행합니다. 이 작업은 Azure AD에서 전역/회사 관리자만 실행할 수 있습니다. 다음 단계에서는 디렉터리에서 서로 다른 권한을 가진 사용자에 대해 사용 권한을 부여하는 프로세스를 설명합니다.
...
각 명령에 대한 자세한 내용을 확인하려면 PowerShell 명령 get-help를 사용합니다(예: ``get-help Set-AzSqlServerActiveDirectoryAdministrator``).
...
## <a name="using-an-azure-ad-identity-to-connect-using-ssms-or-ssdt"></a>SSMS 또는 SSDT를 사용하여 연결하는데 Azure AD ID 사용
...
2. **사용자 이름** 상자에 Azure Active Directory 사용자 이름을 **username\@domain.com** 형식으로 입력 합니다. 사용자 이름은 Azure Active Directory의 계정이거나, Azure Active Directory와 페더레이션된 도메인의 계정이어야 합니다.
...
## <a name="using-an-azure-ad-identity-to-connect-from-a-client-application"></a>클라이언트 애플리케이션에서 연결하는데 Azure AD ID 사용
