...
description: "이 방법 가이드에서는 SQL Server Management Studio 및 Transact-SQL을 통해 서버 수준 방화벽 규칙, 데이터베이스 수준 방화벽 규칙, Azure Active Directory 인증, 로그인, 사용자 및 역할을 사용하여 Azure SQL Database 서버와 데이터베이스에 연결하는 액세스 및 제어 권한을 부여하는 방법에 대해 알아봅니다."
keywords: 
...
이 방법 가이드에서는 SQL Server Management Studio를 통해 Azure Active Directory 인증, 로그인, 사용자 및 데이터베이스 역할을 사용하여 Azure SQL Database 서버와 데이터베이스에 대한 액세스 및 권한을 부여하는 방법에 대해 알아봅니다. 이 문서에서 학습할 내용은 다음과 같습니다.
...
**예상 시간**: 이 방법 가이드의 경우 완료하는 데 약 45분 소요됩니다(이미 필수 조건을 충족한다고 가정).
...
* **기본 서버 및 데이터베이스** 이 방법 가이드에 사용된 서버와 두 데이터베이스를 설치 및 구성하려면 **Azure에 배포** 단추를 클릭합니다. 단추를 클릭하면 **템플릿에서 배포** 블레이드가 열리고 새 리소스 그룹이 생성되고 생성할 새 서버에 대한 **관리자 로그인 암호**를 제공합니다.
   ...
   > [!NOTE]
   > SQL Server 인증 관련 방법 가이드인 [SQL 인증, 로그인/사용자 계정, 데이터베이스 역할, 권한, 서버 수준 방화벽 규칙 및 데이터베이스 수준 방화벽 규칙](sql-database-control-access-sql-authentication-get-started.md)의 완료 여부는 선택 사항입니다. 그러나 본 방법 가이드에서 다루지 않는 개념이 포함되어 있습니다. 동일한 컴퓨터에서 동일한 IP 주소로 관련 방법 가이드를 완료하는 경우 서버 수준 및 데이터베이스 수준의 방화벽과 관련된 이 방법 가이드의 절차가 필요하지 않으며 이러한 이유로 선택 사항으로 표시됩니다. 또한 이 방법 가이드의 스크린샷에서는 이 방법 가이드를 완료했다고 가정합니다. 
   >
...
> [!NOTE]
> 이 방법 가이드는 [SQL Database 액세스 및 제어](sql-database-control-access.md), [로그인, 사용자 및 데이터베이스 역할](sql-database-manage-logins.md), [보안 주체](https://msdn.microsoft.com/library/ms181127.aspx), [데이터베이스 역할](https://msdn.microsoft.com/library/ms189121.aspx), [SQL Database 방화벽 규칙](sql-database-firewall-configure.md) 및 [ Azure Active Directory 인증](sql-database-aad-authentication.md) 항목의 내용을 학습하는 데 도움을 줍니다. 
>  
...
방법 가이드의 이 섹션에서는 Azure Portal에서 논리 서버의 보안 구성에 대한 정보를 볼 수 있습니다.
...
> [!NOTE]
> 이 서버의 연결 정보를 검토하려면 [SSMS로 연결](sql-database-connect-query-ssms.md)로 이동합니다. 이 방법 가이드 시리즈의 경우 정규화된 서버 이름은 'sqldbtutorialserver.database.windows.net'입니다.
>
...
방법 가이드의 이 섹션에서는 master 데이터베이스와 사용자 데이터베이스에서 서버 관리자 계정 및 해당 권한에 대한 정보를 봅니다.
...
방법 가이드의 이 섹션에서는 Azure AD 사용자의 UPN(사용자 계정 이름) 또는 Azure AD 그룹의 표시 이름을 기반으로 하는 사용자 계정을 AdventureWorksLT 데이터베이스에 만들고, 해당 사용자의 권한을 public 역할의 구성원으로 테스트하고, 해당 사용자에게 SELECT 권한을 부여한 다음 해당 사용자의 권한을 다시 테스트합니다.

> [!NOTE]
> 데이터베이스 수준 사용자([포함된 사용자](https://msdn.microsoft.com/library/ff929188.aspx))는 나중에 방법 가이드에서 살펴볼 수 있는 기능인 데이터베이스의 이식성을 향상시킵니다.
>
...
> [!NOTE]
> SQL Server 인증에 대한 관련 방법 가이드인 [SQL 인증 및 권한 부여](sql-database-control-access-sql-authentication-get-started.md)에서 해당 절차를 완료했으며 IP 주소가 같은 동일한 컴퓨터를 학습에 사용하는 경우 이 절차를 완료하지 않아도 됩니다.
>

방법 가이드의 이 섹션에서는 다른 IP 주소를 가진 컴퓨터에서 새 사용자 계정을 사용하여 로그인하고, 서버 관리자로 데이터베이스 수준 방화벽 규칙을 만든 후 새로운 이 규칙을 사용하여 성공적으로 로그인하려고 시도합니다. 

> [!NOTE]
> [데이터베이스 수준 방화벽 규칙](sql-database-firewall-configure.md)은 나중에 방법 가이드에서 살펴볼 수 있는 기능인 데이터베이스의 이식성을 향상시킵니다.
>
...
2. **서버에 연결** 창에서 SQL Server 인증을 사용하여 aaduser1@microsoft.com 계정에 연결할 서버 이름과 인증 정보를 입력합니다. 
   ...
   ![방화벽 규칙 1을 사용하지 않고 aaduser1@microsoft.com으로 연결](./media/sql-database-control-access-aad-authentication-get-started/connect_aaduser1_no_rule1.png)

3. **서버에 연결** 대화 상자에서 **옵션**을 클릭하여 연결하려는 데이터베이스를 지정한 다음 **연결 속성** 탭의 **데이터베이스에 연결** 드롭다운 상자에서 **AdventureWorksLT**를 입력합니다.
   
   ![방화벽 규칙 2를 사용하지 않고 aaduser1로 연결](./media/sql-database-control-access-aad-authentication-get-started/connect_aaduser1_no_rule2.png)
...
4. **Connect**를 클릭합니다. SQL Database에 연결하려는 컴퓨터에 데이터베이스 액세스를 허용하는 방화벽 규칙이 없음을 알리는 대화 상자가 표시됩니다. 이전에 방화벽에서 수행한 단계에 따라 두 가지 변형의 대화 상자가 있지만 일반적으로 첫 번째 대화 상자가 표시됩니다.
   ...
   ![방화벽 규칙 3을 사용하지 않고 user1로 연결](./media/sql-database-control-access-aad-authentication-get-started/connect_aaduser1_no_rule3.png)

   ![방화벽 규칙 4를 사용하지 않고 user1로 연결](./media/sql-database-control-access-aad-authentication-get-started/connect_aaduser1_no_rule4.png)
