...
ms.locfileid: "80346438"
---
# <a name="azure-sql-auditing"></a>Azure SQL 감사

Azure SQL [데이터베이스](sql-database-technical-overview.md) 및 [Azure Synapse Analytics에](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) 대한 감사는 데이터베이스 이벤트를 추적하고 Azure 저장소 계정, 로그 분석 작업 영역 또는 이벤트 허브의 감사 로그에 씁니다. 
...
> [!NOTE] 
> 이 항목은 Azure SQL 데이터베이스와 Azure 시냅스 분석 데이터베이스에 모두 적용됩니다. 간단히 하기 위해 SQL 데이터베이스는 Azure SQL 데이터베이스와 Azure 시냅스 분석을 모두 참조할 때 사용됩니다.
...
> [!IMPORTANT]
> - Azure SQL Database 감사는 가용성 & 성능에 최적화되어 있습니다. 매우 높은 작업 동안 Azure SQL Database 작업을 진행할 수 있으며 일부 감사 된 이벤트를 기록 하지 않을 수 있습니다.
   
## <a name="define-server-level-vs-database-level-auditing-policy"></a><a id="subheading-8"></a>서버 수준 및 데이터베이스 수준 감사 정책 정의
...
- 서버에서 활성화하는 것 외에도 데이터베이스 또는 데이터 웨어하우스에서 BLOB 감사를 활성화하는 것은 서버 BLOB 감사 설정을 재정의 또는 변경하지 *않습니다*. 두 감사는 함께 존재합니다. 즉, 데이터베이스는 동시에 두 번 감사됩니다(서버 정책에서 한 번, 데이터베이스 정책에서 한번).
   ...
   > 다음과 같은 경우가 아니면 서버 Blob 감사 및 데이터베이스 Blob 감사를 함께 활성화하지 않아야 합니다.
    > - 특정 데이터베이스에 대해 다른 *저장소 계정,* *보존 기간* 또는 *로그 분석 작업 영역을* 사용하려고 합니다.
...
## <a name="set-up-auditing-for-your-server"></a><a id="subheading-2"></a>서버에 대한 감사 설정
...
5. 감사 로그가 기록될 위치를 구성하는 여러 옵션이 있습니다. Azure 저장소 계정에 로그를 쓰거나, Azure Monitor 로그에서 사용할 수 있는 로그 분석 작업 영역(미리 보기) 또는 이벤트 허브(미리 보기)를 사용하여 소비를 위한 이벤트 허브에 로그를 쓸 수 있습니다. 이러한 옵션을 조합하여 구성할 수 있으며, 감사 로그는 각각에 대해 작성됩니다.
...
#### <a name="log-audits-to-storage-account-behind-vnet-or-firewall"></a>VNet 또는 방화벽 뒤의 저장소 계정에 감사를 기록합니다.

VNet 또는 방화벽 뒤에 있는 Azure Storage 계정에 감사 로그를 작성할 수 있습니다. 특정 지침은 [VNet 및 방화벽 뒤에 있는 저장소 계정에 감사를 작성하십시오.](create-auditing-storage-account-vnet-firewall.md)

#### <a name="remarks"></a>설명

- 모든 스토리지 종류(v1, v2, Blob)가 지원됩니다.
- 모든 스토리지 복제 구성이 지원됩니다.
- 가상 네트워크 및 방화벽 뒤에 있는 저장소가 지원됩니다.
- **프리미엄 스토리지**는 현재 **지원되지 않습니다**.
- **Azure Data Lake 저장소 Gen2 저장소 계정에** 대한 **계층적 네임스페이스는** 현재 **지원되지 않습니다.**
- 일시 중지된 **Azure SQL 데이터 웨어하우스에** 대한 감사는 지원되지 않습니다. 감사를 활성화하려면 데이터 웨어하우스를 다시 시작합니다.
- 보존 기간의 기본값은 0(무제한 보존)입니다. 감사를 위해 저장소 계정을 구성할 때 **저장소 설정에서** **보존(일)** 슬라이더를 이동하여 이 값을 변경할 수 있습니다.
  - 보존 기간을 0(무제한 보존)에서 다른 값으로 변경하는 경우 보존 값변경 후 작성된 로그에만 보존이 적용됩니다(보존이 무제한으로 설정된 기간 동안 작성된 로그는 보존 후 보존이 활성화됩니다).
...
- 감사 로그는 Azure 구독의 Azure Blob 저장소에 **Blob을 추가하기** 위해 작성됩니다.
...
> 일시 중지된 SQL 풀에서 감사를 사용하도록 설정하면 불가능합니다. 이를 사용하려면 SQL 풀을 일시 중지 해제합니다.
...
- Azure [포털](https://portal.azure.com)을 사용합니다.  관련 데이터베이스를 엽니다. 데이터베이스의 **감사** 페이지 맨 위에서 **감사 로그 보기**를 클릭합니다.

    ![감사 로그 보기](./media/sql-database-auditing-get-started/auditing-view-audit-logs.png)

- 그런 다음 로그를 보는 두 가지 방법이 있습니다.
    
    **감사 레코드** 페이지 상단의 **로그 분석을** 클릭하면 시간 범위와 검색 쿼리를 사용자 지정할 수 있는 로그 분석 작업 영역에서 로그 보기가 열립니다.
    
    ![로그 분석 작업 영역에서 열기](./media/sql-database-auditing-get-started/auditing-log-analytics.png)

    **감사 레코드** 페이지 상단의 **대시보드 보기를** 클릭하면 감사 로그 정보를 표시하는 대시보드가 열리며, 여기서 보안 인사이트, 중요한 데이터에 대한 액세스 등을 드릴다운할 수 있습니다. 이 대시보드는 데이터에 대한 보안 통찰력을 얻을 수 있도록 설계되었습니다.
    시간 범위 및 검색 쿼리를 사용자 지정할 수도 있습니다. 
    ![로그 분석 대시보드 보기](media/sql-database-auditing-get-started/auditing-view-dashboard.png)

    ![로그 분석 대시보드](media/sql-database-auditing-get-started/auditing-log-analytics-dashboard.png)

    ![로그 분석 보안 인사이트](media/sql-database-auditing-get-started/auditing-log-analytics-dashboard-data.png)
...
- 또는 Log Analytics 블레이드에서 감사 로그에 액세스할 수도 있습니다. Log Analytics 작업 영역을 열고 **일반** 섹션 아래에서 **로그**를 클릭합니다. *"SQLSecurityAuditEvents" 검색*과 같은 간단한 쿼리를 시작하여 감사 로그를 볼 수 있습니다.
    여기에서 [Azure Monitor 로그를](../log-analytics/log-analytics-log-search.md) 사용하여 감사 로그 데이터에 대한 고급 검색을 실행할 수도 있습니다. Azure Monitor 로그는 통합 검색 및 사용자 지정 대시보드를 사용하여 모든 워크로드 및 서버에서 수백만 개의 레코드를 쉽게 분석할 수 있는 실시간 운영 통찰력을 제공합니다. Azure Monitor 로그 검색 언어 및 명령에 대한 추가 유용한 정보는 [Azure Monitor 로그 검색 참조를](../log-analytics/log-analytics-log-search.md)참조하십시오.
...
- 감사 로그는 설치 중에 선택한 계정에 집계됩니다. [Azure 저장소 탐색기와](https://storageexplorer.com/)같은 도구를 사용하여 감사 로그를 탐색할 수 있습니다. Azure Storage에서 감사 로그는 **sqldbauditlogs**라는 컨테이너 내부에 Blob 파일의 컬렉션으로 저장됩니다. 저장소 폴더의 계층 구조, 명명 규칙 및 로그 형식에 대한 자세한 내용은 [SQL Database 감사 로그 형식을](https://go.microsoft.com/fwlink/?linkid=829599)참조하십시오.
...
- SQL Server Management Studio에서 **감사 파일 병합** 사용(SSMS 17부터 지원):
    1. SSMS 메뉴에서 **파일** > 열기**병합 감사 파일을****선택합니다.** > 
...
- 포털을 통해 또는 [Azure 저장소 탐색기와](https://storageexplorer.com/)같은 도구를 사용하여 Azure Storage Blob 컨테이너에서 로그 파일을 다운로드합니다.
...
## <a name="production-practices"></a><a id="subheading-5"></a>프로덕션 사례

<!--The description in this section refers to preceding screen captures.-->

### <a name=""></a><a id="subheading-6">지역에서 복제된 데이터베이스 감사</a>
...
### <a name=""></a><a id="subheading-6">스토리지 키 다시 생성</a>
...
## <a name="manage-azure-sql-server-and-database-auditing-using-azure-powershell"></a><a id="subheading-7"></a>Azure PowerShell을 사용하여 Azure SQL 서버 및 데이터베이스 감사 관리
...
- [서버 감사 정책 만들기 또는 업데이트(설정-AzSqlServerAudit)](/powershell/module/az.sql/set-azsqlserveraudit)
- [데이터베이스 감사 정책 받기(Get-AzSqlDatabaseAudit)](/powershell/module/az.sql/get-azsqldatabaseaudit)
- [서버 감사 정책 받기(Get-AzSqlServerAudit)](/powershell/module/az.sql/get-azsqlserveraudit)
- [데이터베이스 감사 정책 제거(AzSqlDatabaseAudit 제거)](/powershell/module/az.sql/remove-azsqldatabaseaudit)
- [서버 감사 정책 제거(AzSqlServerAudit 제거)](/powershell/module/az.sql/remove-azsqlserveraudit)

스크립트 예제는 [PowerShell을 사용하여 감사 및 위협 감지 구성](scripts/sql-database-auditing-and-threat-detection-powershell.md)을 참조하세요.

## <a name="manage-azure-sql-server-and-database-auditing-using-rest-api"></a><a id="subheading-8"></a>REST API를 사용하여 Azure SQL Server 및 데이터베이스 감사 관리
...
- [데이터베이스 *확장* 감사 정책 생성 또는 업데이트](/rest/api/sql/database%20extended%20auditing%20settings/createorupdate)
- [서버 *확장* 감사 정책 생성 또는 업데이트](/rest/api/sql/server%20auditing%20settings/createorupdate)
...
## <a name="manage-azure-sql-server-and-database-auditing-using-azure-resource-manager-templates"></a><a id="subheading-9"></a>Azure 리소스 관리자 템플릿을 사용하여 Azure SQL Server 및 데이터베이스 감사 관리
...
> [!NOTE]
> 연결된 샘플은 외부 공용 리포지토리에 있으며 보증 없이 '있는 그대로' 제공되며 Microsoft 지원 프로그램/서비스에서는 지원되지 않습니다.
