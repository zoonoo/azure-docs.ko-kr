...
[Azure PowerShell을 사용하여 SQL 데이터베이스 감사 관리](#manage-auditing) 섹션에 설명된 대로 PowerShell을 사용하여 다양한 형식의 작업 및 작업 그룹에 대한 감사를 구성할 수 있습니다.
...
3. 서버 감사 정책을 설정하는 것을 선호하면 데이터베이스 감사 페이지에서 **서버 설정 보기** 링크를 선택할 수 있습니다. 그런 다음 서버 감사 설정을 보거나 수정할 수 있습니다. 서버 감사 정책은 이 서버의 모든 기존 및 새로 만든 데이터베이스에 적용됩니다.
    ...
    ![탐색 창](./media/sql-database-auditing-get-started/2_auditing_get_started_server_inherit.png)
...
- 보존 기간의 기본값은 0(무제한 보존)입니다. 감사를 위해 저장소 계정을 구성할 때 **저장소 설정에서** **보존(일)** 슬라이더를 이동하여 이 값을 변경할 수 있습니다.
  - 보존 기간을 0(무제한 보존)에서 다른 값으로 변경하는 경우 보존 값변경 후 작성된 로그에만 보존이 적용됩니다(보존이 활성화된 후에도 보존이 무제한으로 설정된 기간 동안 작성된 로그는 보존됩니다).
...
- 서버 또는 데이터베이스 수준 감사 이벤트에 대해 변경할 수 없는 로그 저장소를 구성하려면 [Azure Storage에서 제공하는 지침을](https://docs.microsoft.com/azure/storage/blobs/storage-blob-immutability-policies-manage#enabling-allow-protected-append-blobs-writes) 따릅니다(변경할 수 없는 Blob 저장소를 구성할 때 추가 추가 **부감 허용을** 선택했으십시오).
...
- Azure [포털](https://portal.azure.com)을 사용합니다.  관련 데이터베이스를 엽니다. 데이터베이스의 **감사** 페이지 맨 위에서 **감사 로그 보기**를 클릭합니다.
    ...
    ![탐색 창](./media/sql-database-auditing-get-started/7_auditing_get_started_blob_view_audit_logs.png)
  ...
  - **SQL 삽입 감사 레코드만 표시** 확인란을 선택하여 SQL 삽입 관련 감사 레코드만 볼 수 있습니다.
       ...
       ![탐색 창]( ./media/sql-database-auditing-get-started/8_auditing_get_started_blob_audit_records.png)
...
- SQL Server Management Studio에서 **감사 파일 병합** 사용(SSMS 17부터 지원):
    1. SSMS 메뉴에서 **파일** > 열기**병합 감사 파일을****선택합니다.** > 
        ...
        ![탐색 창](./media/sql-database-auditing-get-started/9_auditing_get_started_ssms_1.png)
...
1. **스토리지 세부 정보**를 엽니다. **스토리지 액세스 키** 상자에서 **보조**를 선택하고 **확인**을 클릭합니다. 그런 다음, 감사 구성 페이지의 맨 위에서 **저장**을 클릭합니다.
    ...
    ![탐색 창](./media/sql-database-auditing-get-started/5_auditing_get_started_storage_key_regeneration.png)
2. 스토리지 구성 패널로 이동하고 기본 액세스 키를 다시 생성합니다.

    ![탐색 창](./media/sql-database-auditing-get-started/6_auditing_get_started_regenerate_key.png)
