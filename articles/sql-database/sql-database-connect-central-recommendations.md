...
자세한 정보는 [포함된 데이터베이스](http://msdn.microsoft.com/library/ff929071.aspx)에서 제공됩니다.
...
- [Azure SQL 데이터베이스 방화벽](sql-database-firewall-configure.md) 에서 포트 1433을 통해 나가는 TCP 통신을 허용해야 합니다.
 - SQL 데이터베이스 서버 또는 개별 데이터베이스에 대해 방화벽 설정을 구성할 수 있습니다.


- *일시적인 오류*를 처리하려면 [*다시 시도* 논리](#TransientFaultsAndRetryLogicGm)를 Azure SQL 데이터베이스와 상호작용하는 클라이언트 프로그램에 추가합니다.
...
#### 풀을 사용하는 경우 throw된 예외


연결 풀링을 사용하는 경우 시간 초과 오류 또는 기타 로그인 오류가 발생하면 예외가 throw됩니다. 다음 5초간 후속 연결 시도가 실패하는 것을 *차단 기간*이라고 합니다.

응용 프로그램에서 차단 기간 내에 연결하려는 경우 첫 번째 예외가 다시 throw됩니다. 차단 기간이 끝난 후 후속 실패로 발생하는 새로운 차단 기간은 이전 차단 기간의 두 배로 지속됩니다.

차단 기간의 최대 기간은 60초입니다.
...
SQL 데이터베이스 서비스에 과도한 작업 부하가 발생하는 경우 Azure 시스템은 서버를 동적으로 다시 구성 하는 기능이 있습니다.

그러나 재구성 중 클라이언트 프로그램의 SQL 데이터베이스에 대한 연결이 손실될 수 있습니다. 이 오류를 *일시적 장애*라고 합니다.

재시도 간에 아마도 6~60초를 기다린 후 클라이언트 프로그램에서 다시 설정을 시도할 수 있습니다. 클라이언트에서 재시도 논리를 제공해야 합니다.
...
SQL 데이터베이스에서 오류가 발생하면 [SqlException](http://msdn.microsoft.com/library/system.data.sqlclient.sqlexception.aspx)이 발생합니다. **sqlException**은 해당 **Number**속성의 숫자 오류 코드를 포함합니다. 오류 코드가 일시적 오류로 나열된 코드인 경우 프로그램에서 호출을 다시 시도해야 합니다.
...
자세한 내용은 - [Azure SQL 데이터베이스 개발: 방법 도움말 항목](http://msdn.microsoft.com/library/azure/ee621787.aspx) - [Azure SQL 데이터베이스에 대한 연결 문제 해결](http://support.microsoft.com/kb/2980233/)을 참조하세요.
...
**일반 샘플:** 프로그래밍 언어, PHP, Python, Node.js 및 c#.NET의 다양한 [코드 샘플](sql-database-develop-quick-start-client-code-samples.md)입니다. 또한, Windows, LInux 및 Mac OS X에서 실행되는 클라이언트에 샘플이 제공됩니다.
