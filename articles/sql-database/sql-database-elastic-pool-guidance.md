...
20개의 데이터베이스간의 DTU 사용률 집계는 그래프 위에 검은 선으로 표시됐습니다. 이는 DTU 사용률 집계가 100 DTU를 초과하지 않음을 보여 주며, 이 기간 동안 20개 데이터베이스가 100 eDTU를 공유할 수 있음을 나타냅니다. 이러한 결과 20X DTUs의 감소와 6x 가격 감소를 비교합니다. 단일 데이터베이스의 S3 퍼포먼스 수준내의 각각의 데이터베이스를 배치하기위해
...
탄력적 데이터베이스 풀의 가격은 풀 DTU와 풀에 있는 데이터베이스 수의 함수입니다. GA 가격 책정에서 풀의 eDTU 단가는 단일 데이터베이스에 대한 DTU 단가보다 3배지만 **많은 데이터베이스가 풀 eDTU를 공유할 수 있으므로 필요한 총 eDTU가 감소하는 경우가 많습니다**. 가격 책정 및 eDTU 공유에서의 이러한 차이가 풀이 제공할 수 있는 가격 절감 가능성의 기초가 됩니다.

<br>

데이터베이스 개수 및 데이터베이스 사용률과 관련된 다음 규칙은 단일 데이터베이스에 대한 성능 수준 사용과 비교하여 탄력적 데이터베이스 풀이 비용 절감을 제공하는 데 도움이 됩니다. 지침 일반 공급 (GA) 가격을 기반으로 합니다. GA 가격은 미리보기 중 50%할인되고, 이 rule of thumb는 비교적 보수적임을 참고합니다.


### 데이터베이스의 최소 수

GA 가격 책정에서 4개 이상의 데이터베이스가 1 eDTU를 공유할 수 있는 경우 탄력적 데이터베이스 풀이 더 비용 효율적인 성능을 제공합니다. 이는 단일 데이터베이스에 대한 성능 수준의 DTU 합계가 풀의 eDTU보다 3배 이상임을 의미합니다. 사용 가능한 크기에 대해서는 [탄력적 데이터베이스 풀과 탄력적 데이터베이스에 대한 eDTU 및 저장소 제한](sql-database-elastic-pool-reference.md#edtu-and-storage-limits-for-elastic-pools-and-elastic-databases)을 참조하세요.


***예제***<br> 100 eDTU 탄력적 데이터베이스 풀이 단일 데이터베이스에 대한 성능 수준 사용보다 비용 효율적이려면 최소 4개의 S3 데이터베이스 또는 36개의 S0 데이터베이스가 필요합니다. (가격 미리보기 참고, 2개의 S3데이터 베이스 또는 17개의 S0데이터베이스의 손익분기점).



### 최대 동시에 최고 데이터베이스

eDTU를 공유하면 풀의 일부 데이터베이스가 단일 데이터베이스에 대한 성능 수준을 사용할 때 사용 가능한 제한까지 eDTU를 동시에 사용할 수 없습니다. 동시에 최대 사용량에 도달하는 데이터베이스 수가 작을수록 더 낮은 풀 eDTU를 설정할 수 있으며 더 비용 효율적이 됩니다. 일반적으로 eDTU 제한까지 동시에 최대 사용량에 도달하는 풀의 데이터베이스 수가 1/3을 초과하면 안 됩니다.

***예제***<br> 200 eDTU 풀에서는 S3 데이터베이스 4개의 비용을 줄이기 위해 최대 2개의 데이터베이스가 동시에 최대 사용률에 도달할 수 있습니다. 이러한 S3 데이터베이스 4개 중 3개 이상이 동시에 최대 사용률에 도달하는 경우 200 eDTU 이상으로 풀 크기를 조정해야 합니다. 풀 크기가 200 eDTU 이상으로 조정된 경우 단일 데이터베이스에 대한 성능 수준보다 낮은 비용을 유지하려면 S3 데이터베이스를 풀에 더 추가해야 합니다.
...
***예제***<br> 최대 사용률이 100 DTU이고 평균 30 DTU 이하를 사용하는 S3 데이터베이스는 탄력적 데이터베이스 풀에서 eDTU를 공유하는 데 적합합니다. 또는 최대 사용률이 20 DTU이고 평균 7 DTU 이하를 사용하는 S1 데이터베이스는 탄력적 데이터베이스 풀에 적합합니다.
...
3. 다음과 같이 풀에 대한 가격을 계산 합니다.
    ...
    풀 가격 = (*풀 eDTU* * *풀 eDTU 단가*) + (*총 DB 수* * *풀 DB 단가*)
...
표준 서비스 계층의 경우 풀에 대해 구성된 1 eDTU마다 1GB의 저장소가 허용됩니다. 예를 들어, 풀 200 Dtu를 구성 된 경우 저장소 한계는 200GB입니다.

다음 표에서는 각 가격 책정 계층에 대한 eDTU당 저장소 양을 보여 줍니다.
...
**STA**<br>STA는 기존 SQL 데이터베이스 서버에서 데이터베이스의 기록 리소스 사용률을 자동으로 평가하고 적절한 탄력적 데이터베이스 풀 구성을 권장하는 [Preview 포털](https://portal.azure.com)의 기본 제공 도구입니다. 자세한 내용은 [탄력적 데이터베이스 풀 가격 책정 계층 권장 사항](sql-database-elastic-pool-portal.md#elastic-database-pool-pricing-tier-recommendations)을 참조하세요.
...
STA는 기존 서버에 탄력적 데이터베이스 풀을 추가할 때 Preview 포털에서 사용할 수 있습니다. 탄력적 데이터베이스 풀의 권장 사항을 해당 서버에 사용할 수 있는 경우 "탄력적 데이터베이스 풀" 생성 페이지에 표시됩니다. 고객은 언제든지 권장 구성을 변경하여 고유한 탄력적 데이터베이스 풀 그룹화를 만들 수 있습니다.
