...
* 최고 사용률과 데이터베이스당 평균 사용률 간에 큰 차이가 있습니다.  
...
데이터베이스 사용률의 최고와 평균 사이의 큰 차이는 장시간 동안 낮은 사용률 그리고 짧은 시간 동안 큰 사용률을 나타냅니다. 이 사용률 패턴은 데이터베이스에서 리소스를 공유 하기에 이상적입니다. 최고 사용률이 평균사용률의 1.5배 이상이 될 때 풀에 대한 데이터베이스를 고려해야 합니다.
...
V12 이전 서버에 대한 임시 크기 조정 추정값 및 다른 서버의 데이터베이스에 대한 크기 조정 추정값을 허용하는 유연한 리소스 사용 평가에 대해서는 [탄력적 풀에 적합한 데이터베이스를 식별하기 위한 PowerShell 스크립트](sql-database-elastic-pool-database-assessment-powershell.md)를 참조하세요.
...
* [탄력적 풀 만들기](sql-database-elastic-pool-create-portal.md)
...
* [탄력적 풀에 적합한 데이터베이스를 식별하기 위한 PowerShell 스크립트](sql-database-elastic-pool-database-assessment-powershell.md)
