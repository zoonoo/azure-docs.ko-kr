...
풀에 더 많은 데이터베이스를 추가할 수 있으므로 비용이 더 크게 절감됩니다. 그러나 응용 프로그램 사용패턴에 따라, 2개의 S3데이터베이스 보다 적은 수로 비용을 절감 할 수 있습니다.
...
**STA**<br>STA는 기존 SQL 데이터베이스 서버에서 데이터베이스의 기록 리소스 사용률을 자동으로 평가하고 적절한 탄력적 데이터베이스 풀 구성을 권장하는 [미리보기 포털](https://portal.azure.com)의 기본 제공 도구입니다. 자세한 내용은 [탄력적 데이터베이스 풀 가격 책정 계층 권장 사항](sql-database-elastic-pool-portal.md#elastic-database-pool-pricing-tier-recommendations)을 참조하세요.
