...
분할된 데이터베이스 환경에는 많은 데이터베이스와 여러 논리 서버로 구성된 잠재적인 데이터베이스가 많이 있습니다. 각 서버에는 많은 데이터베이스가 포함되어 있습니다(단일 테넌트 솔루션에서 테넌트당 하나의 데이터베이스). 분할된 데이터베이스 맵에서 각 데이터베이스를 매핑하여 해당 호출이 올바른 서버 및 데이터베이스로 정확하게 라우팅될 수 있도록 합니다. 데이터베이스는 분할 키에 따라 추적되며 각 분할된 데이터베이스에는 키 값 범위가 할당됩니다. 예를 들어 분할 키는 "D"에서 "F"까지의 고객 이름을 나타낼 수 있습니다. 모든 분할된 데이터베이스의 매핑(즉, 데이터베이스) 및 매핑 범위는 전역의 분할된 데이터베이스 맵에 포함됩니다. 각 데이터베이스에도 분할된 데이터베이스에 포함된 범위 맵이 포함되며 이것을 로컬 분할된 데이터베이스 맵이라고 합니다. LSM은 캐시된 데이터의 유효성을 검사하는 데 사용됩니다. (앱이 분할된 데이터베이스에 연결될 때 빠른 검색을 위해 매핑이 앱과 함께 캐시됩니다. LSM은 이 매핑의 유효성을 검사합니다.)

GSM 및 LSM이 동기화되지 않는 이유는 다음과 같습니다.

1. 범위가 더 이상 사용 중이 아닌 분할된 데이터베이스가 삭제되거나 분할된 데이터베이스의 이름이 변경되어 불일치가 발생했습니다. **분리되어 분할된 매핑**에서 분할된 데이터베이스 결과 삭제. 이름이 바뀐 데이터베이스도 분리되어 분할된 매핑을 발생시킬 수 있습니다. 의도하는 대로 분할된 데이터베이스를 제거하거나 분할된 데이터베이스 위치를 간단히 업데이트할 필요가 있습니다. 
2. 지역 장애 조치(failover) 이벤트가 발생합니다. 계속하려면, 분할된 데이터베이스 맵의 모든 분할된 데이터베이스에 대해 서버 이름, 데이터베이스 이름 및/또는 분할된 데이터베이스 매핑 정보를 업데이트해야 합니다. 지역 장애 조치의 경우 이러한 복구 논리는 장애 조치 워크플로 내에서 자동화됩니다. 복구 작업을 자동화하면 지역 지원 데이터베이스에 대해 원활한 관리 효율성을 제공하며 사람이 직접 작업하지 않아도 됩니다.
