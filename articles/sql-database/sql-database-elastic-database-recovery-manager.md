...
분할된 데이터베이스 환경에는 다양한 데이터베이스 서버가 있습니다. 각 서버는 다시 여러 데이터베이스를 포함하며 다중 테넌트 솔루션에서는 사용자당 하나의 데이터베이스를 포함합니다. 각 데이터베이스를 매핑하여 해당 호출이 올바른 서버 및 데이터베이스로 정확하게 라우팅될 수 있도록 해야 합니다. 데이터베이스는 분할 키에 따라 추적되며 각 서버에는 키 값 범위가 할당됩니다. 예를 들어 분할 키는 "D"에서 "F"까지의 고객 이름을 나타낼 수 있습니다. 모든 서버 및 해당 키 범위의 매핑은 전역 분할된 데이터베이스 맵에 포함됩니다. 각 서버에도 분할된 데이터베이스에 포함된 데이터베이스 맵이 포함되며 이것을 로컬 분할된 데이터베이스 맵이라고 합니다. LSM은 캐시된 데이터의 유효성을 검사하는 데 사용됩니다. (앱이 분할된 데이터베이스에 연결될 때 빠른 검색을 위해 매핑이 앱과 함께 캐시됩니다. LSM은 이 매핑의 유효성을 검사합니다.)

탄력적 데이터베이스 클라이언트 도구 라이브러리와 같은 도구를 사용하여 하나의 분할된 데이터베이스에서 다른 곳으로 데이터를 이동할 수 있습니다. 이동 중에 중단이 발생하는 경우 GSM 및 LSM은 동기화되지 않을 수 있습니다. 다른 원인은 다음과 같습니다.

1. 범위가 더 이상 사용 중이 아닌 분할된 데이터베이스가 삭제되거나 분할된 데이터베이스의 이름이 변경되어 불일치가 발생했습니다. **분리되어 분할된 매핑**에서 분할된 데이터베이스 결과 삭제. 이름이 바뀐 데이터베이스도 분리되어 분할된 매핑을 발생시킬 수 있습니다. 이 경우 분할된 데이터베이스 위치를 업데이트해야 합니다. 
2. 지역 장애 조치(failover) 이벤트가 발생합니다. 계속하려면, 분할된 데이터베이스 맵의 모든 분할된 데이터베이스에 대해 서버 이름, 데이터베이스 이름 및/또는 분할된 데이터베이스 매핑 정보를 업데이트해야 합니다. 지역 장애 조치의 경우 이러한 복구 논리는 장애 조치 워크플로 내에서 자동화됩니다. 
3. 분할된 데이터베이스 또는 ShardMapManager 데이터베이스는 이전 시점으로 복원됩니다. 
 
복구 작업을 자동화하면 지역 지원 데이터베이스에 대해 원활한 관리 효율성을 제공하며 사람이 직접 작업하지 않아도 됩니다. 또한 데이터가 실수록 삭제된 복구 시나리오에도 도움이 됩니다.

Azure SQL 데이터베이스 탄력적 데이터베이스 도구, 지역에서 복제 및 복원에 대한 자세한 내용은 다음을 참조하세요.

* [Azure SQL 데이터베이스에 대한 탄력적 데이터베이스 기능](sql-database-elastic-scale-introduction.md) 
...
* [MappingDifferenceResolution 열거](https://msdn.microsoft.com/library/azure/microsoft.azure.sqldatabase.elasticscale.shardmanagement.recovery.mappingdifferenceresolution.aspx)는 분할된 데이터베이스 매핑 간 차이를 해결하기 위한 메서드를 나타내는 데 사용됩니다. **MappingDifferenceResolution.KeepShardMapping**은 LSM이 정확한 매핑을 포함하므로 분할된 데이터베이스의 매핑이 사용되는 경우에 권장됩니다. 이 방법은 장애 조치 시 일반적인 사례입니다. 이제 분할된 데이터베이스는 새 서버에 상주합니다. 분할된 데이터베이스는 GSM에서 먼저 제거되므로(RecoveryManager.DetachShard 메서드 사용) GSM에 매핑이 더 이상 존재하지 않습니다. 따라서 분할된 매핑을 다시 설정하기 위해 LSM을 사용해야 합니다.
...
1. ShardMapManager에서 복구 관리자를 검색합니다. 
