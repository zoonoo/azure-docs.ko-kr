...
다음 PowerShell 명령을 실행합니다. Replace **subscription-id** with your subscription ID. Also replace **rg-name** with the resource group for your managed instance, and replace **mi-name** with the name of your managed instance.
...
1. Select the **Inbound security rules** tab, and **Add** a rule that has higher priority than the **deny_all_inbound** rule with the following settings: </br> </br>
    ...
    |설정  |제안 값  |설명  |
    |---------|---------|---------|
    |**원본**     |Any IP address or Service tag         |<ul><li>For Azure services like Power BI, select the Azure Cloud Service Tag</li> <li>For your computer or Azure VM, use NAT IP address</li></ul> |
    |**Source port ranges**     |*         |Leave this to * (any) as source ports are usually dynamically allocated and as such, unpredictable |
    |**Destination**     |모두         |Leaving destination as Any to allow traffic into the managed instance subnet |
    |**Destination port ranges**     |3342         |Scope destination port to 3342, which is the managed instance public TDS endpoint |
    |**프로토콜**     |TCP         |Managed instance uses TCP protocol for TDS |
    |**작업**     |허용         |Allow inbound traffic to managed instance through the public endpoint |
    |**우선 순위**     |1300         |Make sure this rule is higher priority than the **deny_all_inbound** rule |
