...
> [AZURE.NOTE] 다음 단계는 사용자가 이미 Azure SQL 논리 인스턴스를 프로비전하고 연결 정보를 알고 있는 경우를 가정합니다.
...
3.	Azure SQL 데이터베이스에서 데이터베이스를 만들려면 로컬 디스크에서 BACPAC 파일을 가져오거나 BACPAC 파일을 업로드한 Azure 저장소 계정 및 컨테이너를 선택합니다.
