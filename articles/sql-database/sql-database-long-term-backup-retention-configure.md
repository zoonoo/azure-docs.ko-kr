...
Azure SQL Database에서 [장기 백업 보존](sql-database-long-term-retention.md) 정책(LTR)을 사용하여 단일 또는 풀린 데이터베이스를 구성하여 최대 10년 동안 별도의 Azure Blob 저장소 컨테이너에 데이터베이스 백업을 자동으로 유지할 수 있습니다. 그런 다음, Azure Portal이나 PowerShell에서 이러한 백업을 사용하여 데이터베이스를 복구할 수 있습니다.

> [!IMPORTANT]
> [Azure SQL 데이터베이스 관리 인스턴스는](sql-database-managed-instance.md) 현재 장기 백업 보존을 지원하지 않습니다.
...
1. Azure Portal에서 SQL Server를 선택한 다음, **백업 관리**를 클릭합니다. 정책 **구성** 탭에서 장기 백업 보존 정책을 설정하거나 수정할 데이터베이스의 확인란을 선택합니다. 데이터베이스 옆의 확인란을 선택하지 않으면 정책에 대한 변경 내용이 해당 데이터베이스에 적용되지 않습니다.  
...
> [!IMPORTANT]
> 장기 백업 보존 정책을 사용하도록 설정하면 첫 번째 백업이 표시되고 복원될 수 있는 데 최대 7일이 걸릴 수 있습니다. LTR 백업 카던스에 대한 자세한 내용은 [장기 백업 보존을](sql-database-long-term-retention.md)참조하십시오.
...
**Get-AzSqlDatabaseLongTerm 보존 백업** 및 **복원-AzSqlDatabase의**경우 다음 역할 중 하나가 필요합니다.

- 구독 소유자 역할 또는
- SQL 서버 기여자 역할 또는
- 다음과 같은 권한이 있는 사용자 지정 역할:

   Microsoft.Sql/위치/장기 보존 백업/읽기 Microsoft.Sql/위치/장기 보존 서버/장기 보존 백업/읽기 Microsoft.Sql/위치/장기 보존 서버/장기 보존 데이터베이스/장기 보존 백업/읽기

**제거-AzSqlDatabaseLongTerm보존 백업의**경우 다음 역할 중 하나가 있어야 합니다.
...
*RBAC* 권한은 구독 또는 *리소스 그룹* 범위에서 부여될 수 있습니다. 그러나 삭제된 서버에 속하는 LTR 백업에 액세스하려면 해당 서버의 *구독* 범위에서 권한을 부여해야 합니다.
...
> [!IMPORTANT]
> 서버가 삭제된 후 LTR 백업에서 복원하려면 서버 구독에 대한 범위가 정해져야 하며 해당 구독이 활성 상태여야 합니다. 또한 선택적 -ResourceGroupName 매개 변수를 생략해야 합니다.
