...
- Azure 파일 공유에 액세스 하기 위해 관리 되는 인스턴스에 대 한 NSG의 보안 규칙에서 포트 445 (TCP 아웃 바운드)이 열립니다. 
...
- 트랜잭션 복제가 구성 된 경우 [활성 지역 복제](sql-database-active-geo-replication.md) 및 [자동 장애 조치 (failover) 그룹](sql-database-auto-failover-group.md) 을 사용 하면 안 됩니다.
...
SQL Server Management Studio를 사용 하 여 관리되는인스턴스에연결하고다음transact-sql(t-sql)코드를실행하여게시자데이터베이스를만듭니다.`sql-mi-pub`
...
SQL Server Management Studio를 사용 하 여 관리되는인스턴스에연결하고다음t-sql코드를실행하여빈구독자데이터베이스를만듭니다.`sql-mi-sub`
...
SQL Server Management Studio를 사용 하 여 관리되는인스턴스에연결하고다음t-sql코드를실행하여배포데이터베이스를구성합니다.`sql-mi-pub` 
...
[리소스 그룹에서 관리 되는 인스턴스 리소스를 삭제](../azure-resource-manager/manage-resources-portal.md#delete-resources) 하 고 리소스 그룹 `SQLMI-Repl`을 삭제 하 여 Azure 리소스를 정리할 수 있습니다. 
