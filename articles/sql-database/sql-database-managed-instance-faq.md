...
버그 및 알려진 문제에 대해서는 [알려진 문제](sql-database-release-notes.md#known-issues)를 참조 하세요.

## <a name="new-features"></a>새 기능

**공개 미리 보기에서 최신 기능과 기능을 어디에서 찾을 수 있나요?**

새 기능 및 미리 보기 기능에 대 한 자세한 내용은 [릴리스 정보](sql-database-release-notes.md?tabs=managed-instance)를 참조 하세요.
...
관리 되는 인스턴스 기본 DNS 영역 *. database.windows.net* 를 변경할 수 있습니다. 
...
관리 되는 인스턴스 제어 평면은 관리 포트를 보호 하는 NSG 규칙을 유지 합니다.
...
IP 주소를 사용 하 여 관리 되는 인스턴스에 연결 하는 것은 지원 되지 않습니다. 관리 되는 인스턴스 호스트 이름은 관리 되는 인스턴스 가상 클러스터 앞의 부하 분산 장치에 매핑됩니다. 하나의 가상 클러스터가 여러 관리 되는 인스턴스를 호스트할 수 있으므로 해당 이름을 지정 하지 않고 연결을 적절 한 관리 되는 인스턴스로 라우팅할 수 없습니다.

관리 되는 인스턴스 가상 클러스터 아키텍처에 대 한 자세한 내용은 [가상 클러스터 연결 아키텍처](sql-database-managed-instance-connectivity-architecture.md#virtual-cluster-connectivity-architecture)를 참조 하세요.
