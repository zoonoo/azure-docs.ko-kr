...
> [!NOTE]
> 포털에서 “쿼리 저장소가 이 데이터베이스에서 올바르게 구성되지 않음” 메시지가 표시되는 경우 [쿼리 저장소 구성 최적화](sql-database-query-performance.md#optimizing-the-query-store-configuration-for-query-performance-insight)를 참조하세요.
>
...
2. 첫 번째 탭에서 최상위 리소스 사용 쿼리의 목록을 검토합니다.
3. 해당하는 세부 정보를 보려면 개별 쿼리를 선택합니다.
4. **지능형 성능** > **성능 권장 사항**을 열고 성능 권장 사항을 사용할 수 있는지 확인합니다. 기본 제공 성능 권장 사항에 대한 자세한 내용은 [SQL Database Advisor](sql-database-advisor.md)를 참조하세요.
5. 슬라이더 또는 확대/축소 아이콘을 사용하여 관찰 간격을 변경합니다.
...
4. 필요에 따라 슬라이더, 확대/축소 단추를 사용하거나 **설정**을 선택하여 쿼리 데이터의 표시 방법을 사용자 지정하거나 다른 시간 범위를 지정합니다.
