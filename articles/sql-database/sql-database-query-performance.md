...
Query Performance Insight를 사용하려면 다음 [역할 기반 액세스 제어](role-based-access-control-configure.md) 권한이 필요합니다.
...
- **설정**을 클릭하여 데이터 표시 방법을 사용자 지정하거나 다른 기간을 표시합니다.
...
1. SQL 데이터베이스를 찾아 **Query Performance Insight**를 클릭합니다. 

    ![쿼리 성능 Insight][1]

    최상위 쿼리 뷰가 열리고 최상위 CPU 사용 쿼리의 목록이 나열됩니다.

1. 세부 정보는 차트 주위를 클릭합니다.<br>위쪽 줄은 데이터베이스에 대한 전체 DTU%를 표시하고 막대는 선택한 쿼리에서 사용된 CPU%를 보여 줍니다. 개별 쿼리를 선택하거나 지워 차트에서 포함하거나 제외합니다.

    ![최상위 쿼리][2]

1. 필요에 따라 **차트 편집**을 클릭하여 CPU 사용 데이터 표시 방법을 사용자 지정하거나 다른 기간을 표시합니다.

## 개별 쿼리 세부 정보 보기

쿼리 세부 정보를 보려면:

1. 최상위 쿼리 목록에서 모든 쿼리를 클릭합니다.<br>세부 정보 보기가 열리고 쿼리 CPU 사용이 시간이 지남에 따라 분리됩니다.
...
쿼리 성능 Insight를 통해 쿼리 작업의 영향 및 데이터베이스 리소스 사용의 관계를 이해할 수 있습니다. 이 기능을 사용하여 최상위 사용 쿼리를 확인하고 문제가 되기 전에 해결할 쿼리를 쉽게 식별할 수 있습니다. 데이터베이스 블레이드의 **Query Performance Insight** 타일을 클릭하여 최상위 리소스(CPU) 소비 쿼리를 확인합니다.
