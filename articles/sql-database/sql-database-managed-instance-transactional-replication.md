...
- 게시자/배포자가 관리 되는 인스턴스에 있고 구독자가이 아닌 경우 포트 1433 (TCP 아웃 바운드)을 열어야 합니다. 포트 1433 **대상 서비스 태그** 의에 대 `allow_linkedserver_outbound` 한 관리 되는 인스턴스 nsg 아웃 바운드 보안 규칙을에서 `virtualnetwork` 로 `internet`변경 해야 할 수도 있습니다. 
...
이 구성에서 하나의 관리 되는 인스턴스는 여러 원본 관리 되는 인스턴스를 제공 하 고 관리 되는 인스턴스, 단일 데이터베이스, 풀링된 데이터베이스 또는 SQL Server에서 하나 이상의 대상에 변경 내용을 배포할 수 있는 다른 관리 되는 인스턴스에 배치 된 배포자에 변경 내용을 게시 합니다.
