...
> - 관리 되는 인스턴스의 게시자 및 배포자 데이터베이스에서 [자동 장애 조치 (failover) 그룹](sql-database-auto-failover-group.md)을 사용 하는 경우 관리 되는 인스턴스 관리자는 [이전 주 데이터베이스의 모든 게시를 삭제 하 고 장애 조치 (failover)가 발생 한 후 새 주 데이터베이스에서 다시 구성](sql-database-managed-instance-transact-sql-information.md#replication)
