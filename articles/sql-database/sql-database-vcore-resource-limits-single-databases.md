...
**참고 1**: Hyperscale은 개별 계산 및 저장소 구성 요소를 포함 하는 다중 계층 아키텍처입니다. [대규모 서비스 계층 아키텍처](sql-database-service-tier-hyperscale.md#distributed-functions-architecture)

**참고 2**: Hyperscale 다중 계층 아키텍처는 여러 수준에서 캐싱을 가집니다. 유효 IOPS는 워크 로드에 따라 달라 집니다.

**참고 3**: 대기 시간은 계산 복제본의 RBPEX SSD 기반 캐시에 있는 데이터의 경우 1-2 밀리초로, 가장 많이 사용 되는 데이터 페이지를 캐시 합니다. 페이지 서버에서 검색 되는 데이터에 대 한 대기 시간이 더 높습니다.
