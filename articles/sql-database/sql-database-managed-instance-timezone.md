---
title: Azure SQL Database Managed Instance 표준 시간대 | Microsoft Docs "
...
ms.locfileid: "68567286"
---
# <a name="time-zones-in-azure-sql-database-managed-instance"></a>Azure SQL Database Managed Instance의 표준 시간대

UTC (협정 세계시)는 클라우드 솔루션의 데이터 계층에 권장 되는 표준 시간대입니다. 또한 Azure SQL Database Managed Instance는 날짜 및 시간 값을 저장 하는 기존 응용 프로그램의 요구를 충족 하 고 특정 표준 시간대의 암시적 컨텍스트를 사용 하 여 날짜 및 시간 함수를 호출 하는 표준 시간대를 선택할 수 있습니다.

[GETDATE ()](https://docs.microsoft.com/sql/t-sql/functions/getdate-transact-sql) 또는 CLR 코드와 같은 t-sql 함수는 인스턴스 수준에서 설정 된 표준 시간대를 관찰 합니다. 또한 SQL Server 에이전트 작업은 인스턴스의 표준 시간대에 따라 일정을 따릅니다.

  >[!NOTE]
  > Managed Instance은 표준 시간대 설정을 지 원하는 Azure SQL Database의 유일한 배포 옵션입니다. 다른 배포 옵션은 항상 UTC를 따릅니다.
UTC가 아닌 표준 시간대에서 날짜 및 시간 정보를 해석 해야 하는 경우 단일 및 풀링된 SQL 데이터베이스에서 [TIME ZONE](https://docs.microsoft.com/sql/t-sql/queries/at-time-zone-transact-sql) 을 사용 합니다.
...
지원 되는 표준 시간대의 이름을 포함 하는 목록은 [time_zone_info](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-time-zone-info-transact-sql) 시스템 뷰를 통해 노출 됩니다.
...
[CURRENT_TIMEZONE](https://docs.microsoft.com/sql/t-sql/functions/current-timezone-transact-sql) 함수는 인스턴스의 표준 시간대에 대 한 표시 이름을 반환 합니다.
...
<del>지정 시간 복원을 수행 하는 경우에 복원 하는 시간은 UTC 시간으로 해석 됩니다. 이 설정은 일광 절약 시간 및 잠재적인 변경으로 인 한 모호성을 방지 합니다.<del>

 >[!WARNING]
  > 현재 동작은 위의 문에 포함 되지 않으며,에 대 한 복원 시간은 자동 데이터베이스 백업이 수행 되는 원본 관리 되는 인스턴스의 표준 시간대에 따라 해석 됩니다. 지정 된 시간을 UTC 시간으로 해석 하기 위해이 동작을 수정 하는 작업을 수행 하 고 있습니다. 자세한 내용은 [알려진 문제](sql-database-managed-instance-timezone.md#known-issues) 를 참조 하세요.
...
장애 조치 (failover) 그룹의 기본 및 보조 인스턴스에서 동일한 표준 시간대를 사용 하는 것은 적용 되지 않지만이를 사용 하는 것이 좋습니다.
  ...
  >[!WARNING]
  > 장애 조치 (failover) 그룹에서 주 및 보조 인스턴스에 대해 동일한 표준 시간대를 사용 하는 것이 좋습니다. 드문 경우 지만 주 인스턴스와 보조 인스턴스 간에 동일한 표준 시간대를 유지 하는 것은 아닙니다. 수동 또는 자동 장애 조치 (failover)의 경우 보조 인스턴스가 원래 표준 시간대를 유지 한다는 것을 이해 하는 것이 중요 합니다.
...
PITR (지정 시간 복원) 작업을 수행 하는 경우 PITR에 대 한 포털 페이지에서 시간이 UTC로 해석 된다는 것을 알 수 있는 경우에도 복원 시간은 자동 데이터베이스 백업이 수행 되는 관리 되는 인스턴스에 설정 된 표준 시간대에 따라 해석 됩니다.

예제:

자동 백업이 수행 되는 인스턴스에 동부 표준시 (UTC-5) 표준 시간대가 설정 되어 있다고 가정 합니다.
지정 시간 복원에 대 한 포털 페이지는 UTC 시간으로 복원 하도록 선택 하는 시간을 제안 합니다.

![포털을 사용 하 여 현지 시간으로 PITR](media/sql-database-managed-instance-timezone/02-pitr-with-nonutc-timezone.png)

그러나에 대 한 복원 시간은 실제로 동부 표준시로 해석 되 고,이 특정 예제 데이터베이스는 UTC 시간이 아닌 오전 9 시 (동부 표준시)에 상태로 복원 됩니다.

지정 시간 복원을 UTC 시간으로 수행 하려면 먼저 원본 인스턴스의 표준 시간대에서 동일한 시간을 계산 하 고 포털 또는 PowerShell/CLI 스크립트에서 해당 시간을 사용 합니다.
...
| 5\. 남부 아메리카 표준시 | (UTC-03:00) 브라질리아 |
| SA 동부 표준시 | (UTC-03:00) 카옌, 포르탈레자 |
| 아르헨티나 표준 시간 | (UTC-03:00) 부에노스아이레스 시 |
...
| 5\. 유럽 표준시 | (UTC+02:00) 키시네프 |
| 시리아 표준시 | (UTC+02:00) 다마스쿠스 |
| 서 부 은행 표준시 | (UTC+02:00) 가자, 헤브론 |
...
| 러시아 시간 영역 3 | (UTC+04:00) 이젭스크, 사마라 |
...
| 개의. 중앙 아시아 표준시 | (UTC+07:00) 노보시비르스크 |
