---
title: CLI 예제 - 그룹 장애 조치(failover) - Azure SQL Database 관리형 인스턴스
description: Azure SQL Database 관리형 인스턴스를 만들어서 장애 조치(failover) 그룹에 추가하고 장애 조치(failover)를 테스트하는 Azure CLI 예제 스크립트입니다.
...
ms.locfileid: "80061858"
---
# <a name="use-cli-to-add-an-azure-sql-database-managed-instance-to-a-failover-group"></a>CLI를 사용하여 Azure SQL Database 관리형 인스턴스를 장애 조치(failover) 그룹에 추가
...
다음 명령을 사용하여 리소스 그룹 및 모든 관련 리소스를 제거합니다.  리소스 그룹은 두 번의 제거 명령어를 통하여 제거합니다. 리소스 그룹을 처음으로 제거하면 관리형 인스턴스와 가상 클러스터가 제거되지만, `az group delete : Long running operation failed with status 'Conflict'.` 오류 메시지와 함께 실패하게 됩니다. az group delete 명령을 한 번 더 실행하여 리소스 그룹뿐 아니라 나머지 리소스를 모두 제거합니다.
...
| [az sql instance-failover-group](/cli/azure/sql/instance-failover-group) | 관리형 인스턴스 장애 조치(failover) 그룹 명령입니다. |
