...
Basic, Standard, Premium 및 Premium RS 서비스 계층의 디자인 목표 중 하나는 데이터베이스가 다른 데이터베이스로부터 격리된 자체 컴퓨터에서 실행되는 것처럼 Azure SQL Database가 동작하도록 하는 것입니다. 리소스 관리는 이러한 동작을 에뮬레이션합니다. 집계 리소스 사용률이 데이터베이스에 할당된 최대 사용 가능한 CPU, 메모리, 로그 I/O 및 데이터 I/O 리소스에 도달하면 리소스 관리가 실행 중인 쿼리를 큐에 대기시키고 리소스가 확보되는 대로 큐에 대기 중인 쿼리에 리소스를 할당합니다.
...
단일 데이터베이스에 대한 데이터베이스 제한은 데이터베이스 서비스 계층 및 성능 수준에 의해 정의됩니다. 다음 표에서는 다양한 성능 수준에서 Basic, Standard, Premium 및 Premium RS 데이터베이스의 특징을 설명합니다.
...
[탄력적 풀](sql-database-elastic-pool.md) 은 풀의 데이터베이스에서 리소스를 공유합니다. 다음 표에서는 Basic, Standard, Premium 및 Premium RS 탄력적 풀의 특징을 설명합니다.
...
| 서버당 데이터베이스 |최대 5000 |서버당 최대 5,000개의 데이터베이스를 허용합니다. |
| 서버당 DTU |45000 |독립 실행형 데이터베이스 및 탄력적 풀을 프로비전하는 데 서버당 45,000개의 DTU가 허용됩니다. 서버당 허용되는 독립 실행형 데이터베이스 및 풀의 총 수는 서버 DTU 수에 의해서만 제한됩니다.  
