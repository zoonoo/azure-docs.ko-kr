...
이 문서에서는 DTU 구매 모델을 사용 하 여 Azure SQL Database 탄력적 풀 및 풀링된 데이터베이스에 대 한 자세한 리소스 제한을 제공 합니다.

단일 데이터베이스에 대 한 DTU 구매 모델 리소스 제한은 [dtu 리소스 제한-단일 데이터베이스](sql-database-vcore-resource-limits-elastic-pools.md)를 참조 하세요. VCore 리소스 제한의 경우 [Vcore 리소스 제한-단일 데이터베이스](sql-database-vcore-resource-limits-single-databases.md) 및 [vcore 리소스 제한-탄력적 풀](sql-database-vcore-resource-limits-elastic-pools.md)을 참조 하세요.
