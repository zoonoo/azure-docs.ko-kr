---
title: Azure SQL Database 기능 제한 | Microsoft Docs
...
기능 제한 사용은 다음과 같이 저장 `sp_add_feature_restriction` 프로시저를 사용 하 여 수행 됩니다.
...
의 `object_class` 값은 `N'User'` 또는가 데이터베이스의 `N'Role'` 사용자 이름 또는 `object_name` 역할 이름 인지 여부를 나타내는 또는 일 수 있습니다.
...
기능 제한 사용 안 함은 다음과 `sp_drop_feature_restriction` 같이 저장 프로시저를 사용 하 여 수행 됩니다.

```sql
EXEC sp_drop_feature_restriction <feature>, <object_class>, <object_name>
```

다음 예에서는 사용자 `MyUser`에 대해 오류 메시지 마스킹을 사용 하지 않도록 설정 합니다.
...
이 `sys.sql_feature_restrictions` 뷰는 데이터베이스에 대해 현재 정의 된 모든 기능 제한을 표시 합니다. 다음과 같은 열이 있습니다.

| 열 이름 | 데이터 형식 | Description |
|-------------|-----------|-------------|
| 클래스       | nvarchar(128) | 제한이 적용 되는 개체의 클래스입니다. |
| 개체(object)      | nvarchar(256) | 제한이 적용 되는 개체의 이름입니다. |
...
웹 응용 프로그램 요청에 대 `id` 한 매개 변수로 전달 된 값이 데이터베이스 쿼리의 $EmpId를 대체 하도록 복사 되 면 공격자는 다음 요청을 수행할 수 있습니다.
...
`sa` 계정을 사용 하는 경우 쿼리는 5 초 추가로 소요 됩니다. `WAITFOR` 데이터베이스`WAITFOR` 에서 기능 제한이 사용 되지 않도록 설정 된 경우이 문은 무시 되 고이 공격을 사용 하 여 정보가 유출 되지 않습니다.