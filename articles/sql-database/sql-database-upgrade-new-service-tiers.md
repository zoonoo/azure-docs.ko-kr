<properties 
	...
	description="이 문서에서는 Azure SQL 데이터베이스 Web 또는 Business 데이터베이스를 새 Azure SQL 서비스 계층/성능 수준으로 업그레이드하는 방법을 설명합니다. SQL 데이터베이스 업그레이드, 서비스 계층 변경, 성능 수준 변경" 
...
Azure SQL Web 및 Business 데이터베이스는 더 이상 사용되지 않으며 [2015년 9월부터 제공되지 않을 예정입니다.](http://msdn.microsoft.com/library/azure/dn741330.aspx) 따라서 이제 기존 Web 또는 Business 데이터베이스를 기본, 표준 또는 프리미엄 서비스 계층으로 업그레이드하는 계획을 시작해야 합니다.
...
여기서 기억해야 할 사항은, Azure SQL 데이터베이스에서 반드시 특정 서비스 계층이나 서비스 수준을 사용해야 하는 것은 아니라는 점입니다. 따라서 데이터베이스의 요구 사항이 변경되면 사용 가능한 서비스 계층과 성능 수준 간에 쉽게 변경이 가능합니다. 실제로 기본, 표준 및 프리미엄 SQL 데이터베이스에서는 시간 단위로 요금이 청구되며 [Azure 포털을 사용하거나 프로그래밍 방식](http://msdn.microsoft.com/library/azure/ff394116.aspx)으로 24시간 동안 각 데이터베이스를 4번 확장 및 축소할 수 있습니다. 즉, 응용 프로그램의 요구 사항과 변화하는 작업량에 따라 데이터베이스의 성능 요구 사항, 기능 집합 및 비용 효율성을 최대화하도록 서비스 계층과 성능 수준을 조정할 수 있습니다. 또한 데이터베이스의 서비스 계층과 성능 수준을 평가하고 변경하는 작업(확장 및 축소)은 예약된 유지 관리 및 성능 튜닝 루틴의 일부분으로 포함해야 하는 지속적인 프로세스입니다. 
...
예를 들어 백업을 유지해야 하는 기간이나 [표준 또는 활성 지역에서 복제](http://msdn.microsoft.com/library/azure/dn783447.aspx) 기능이 필요한지 여부, 필요한 전체 최대 데이터베이스 크기 등을 고려한 다음 이러한 요구 사항에 따라 최소 서비스 계층 항목을 선택합니다.
...
|[Azure SQL 데이터베이스 무중단 업무 방식](http://msdn.microsoft.com/library/azure/hh852669.aspx)|무중단 업무 방식과 각 서비스 계층에서 제공되는 재해 복구 기능(지정 시간 복원, 지역에서 복원, 지역에서 복제)에 대해 자세히 설명합니다.|
...
관리 포털에서 DTU 백분율 사용 방법에 대한 간략한 개요를 파악한 다음 시스템 뷰를 통해 세부 정보를 확인합니다.


### 관리 포털에서 DTU 사용량을 확인하는 방법
관리 포털에서는 기존 Web 또는 Business 데이터베이스의 DTU 사용량을 확인할 수 있습니다.
최신 Azure 포털에서 DTU 정보가 제공됩니다.
...
확인 시 아래 표의 **DTU 백분율** 데이터가 표시되어야 합니다. 이 백분율은 S2(표준) 수준 데이터베이스의 DTU(50)와 비교한 백분율입니다.
...
현재 데이터베이스가 있는 논리 서버의 master 데이터베이스 [sys.resource_stats](http://msdn.microsoft.com/library/azure/dn269979.aspx) 뷰를 통해 Web 및 Business 데이터베이스 리소스 사용량 데이터에 액세스합니다. 리소스 사용 데이터를 성능 수준 한도의 백분율로 표시합니다. 이 보기는 5분 간격으로 최대 최근 14일간의 데이터를 제공합니다.  마스터 데이터베이스에서 다음 쿼리를 실행하여 데이터베이스의 평균 DTU 사용을 검색합니다.
...
Web 및 Business 데이터베이스에서는 개별 데이터베이스에 대해 일정량의 리소스 용량이 예약되지 않습니다. 또한 고객이 Web 또는 Business 데이터베이스에 대해 성능을 확장 또는 축소할 수 있는 메커니즘도 없습니다. 따라서 Web 및 Business 데이터베이스 성능은 매우 느린 수준에서 매우 빠른 수준까지 다양합니다. 이처럼 다양한 성능 범위는 리소스를 공유하는 다중 테넌트 환경 내의 다른 데이터베이스가 특정 시점에 사용하는 리소스의 전반적인 수준에 따라 *unfairly* 달라집니다.  
...
## 4.	더 낮은 성능 수준에 맞게 데이터베이스 작업량 튜닝
데이터베이스의 기존 리소스 사용량 분석 결과 원하는 수준보다 더 고가의 성능 수준으로 업그레이드해야 하는 경우에는 성능을 추가로 튜닝하면 도움이 되는 영역을 파악할 수 있습니다. 

현재 알고 있는 응용 프로그램 세부 정보에 따라 리소스 사용량이 일반적인 작업량에 비해 너무 높다고 생각되면 성능 튜닝을 통해 응용 프로그램을 개선할 수도 있습니다.
...
| 문서 | 설명 |
|:--|:--|
| [Azure SQL 데이터베이스 성능 지침](http://msdn.microsoft.com/library/dn369873.aspx) |  'Tuning Your Application' 섹션에서는 Azure SQL 데이터베이스의 성능 튜닝을 위한 세부 권장 사항 및 기술에 대해 설명합니다.|
...
자세한 내용은 [데이터베이스 서비스 계층 및 성능 수준 변경](http://msdn.microsoft.com/library/dn369872.aspx)을 참조하세요.


## 6.	새 서비스 계층/성능 수준으로의 업그레이드 모니터링
Azure SQL 데이터베이스는 현재 데이터베이스가 있는 논리 서버의 master 데이터베이스 내 sys.dm_operation_status 동적 관리 뷰에서 데이터베이스에 대해 수행하는 CREATE, ALTER, DROP 등의 관리 작업에 대한 정보를 제공합니다. 관련 정보는 [see sys.dm _operation _status 설명서](http://msdn.microsoft.com/library/azure/dn270022.aspx)를 참조하세요. 작업 상태 DMV를 사용하여 데이터베이스의 업그레이드 작업 진행 상태를 확인합니다. 다음 샘플 쿼리는 데이터베이스에서 수행된 모든 관리 작업을 보여 줍니다.
...
### 업그레이드 후 데이터베이스 작업이 리소스 한도에 도달할 경우 수행되는 작업
선택한 서비스 계층/성능 수준에 허용되는 최대 한도(즉, 리소스 사용이 100%임)까지 데이터베이스 작업을 실행하는 데 필요한 리소스를 제공하도록 성능 수준이 보정 및 제어됩니다. 작업이 CPU/데이터 IO/로그 IO 한도 중 하나에서 한도에 도달할 경우 최대 허용 수준에서 계속 리소스를 받지만 쿼리의 대기 시간이 증가할 가능성이 큽니다. 이러한 한도 중 하나에 도달해도 오류가 발생하지는 않으며, 속도 저하가 너무 심해서 쿼리 시간 초과가 시작되지 않는 한 작업 속도만 느려집니다. 허용되는 최대 동시 사용자 세션/요청(작업자 스레드) 제한에 도달하면 [오류 10928 또는 10929](http://msdn.microsoft.com/library/azure/dn338078.aspx)가 표시됩니다.
...
- **경고:** 업그레이드된 데이터베이스의 DTU 사용량이 어느 정도 높은 수준에 도달하면 알림을 보내도록 Azure 관리 포털에서 'Alerts'를 설정합니다. Azure 관리 포털에서 DTU, CPU, IO 및 로그와 같은 다양한 성능 메트릭에 대한 데이터베이스 경고를 설정할 수 있습니다. 

	예를 들어 평균 DTU 백분율 값이 최근 5분 동안 75%를 초과할 경우 "DTU 백분율"에 대해 메일 경고를 설정할 수 있습니다. 경고 알림을 구성하는 방법에 대해 자세히 알아보려면 [방법: Azure에서 경고 알림 받기 및 경고 규칙 관리](http://msdn.microsoft.com/library/azure/dn306638.aspx)를 참조하세요.


- **예약된 성능 수준 업그레이드/다운그레이드:** 응용 프로그램을 사용하는 특정 상황에서 특정 일/주의 특정 시간에만 더 높은 성능이 필요한 경우에는 [Azure 자동화](http://msdn.microsoft.com/library/azure/dn643629.aspx)를 사용하여 계획된 작업으로 데이터베이스를 상위/하위 성능 수준으로 업사이징/다운사이징할 수 있습니다.

	예를 들어 주별 배치/유지 관리 작업 기간 동안 데이터베이스를 상위 성능 수준으로 업그레이드하고 작업이 완료된 후 다운사이징합니다. 이러한 종류의 예약은 데이터 로드, 인덱스 다시 작성 등 리소스를 많이 사용하는 대규모 작업에도 유용합니다. Azure SQL 데이터베이스 청구 모델은 서비스 계층/성능 수준의 시간별 사용에 따라 달라집니다. 이러한 유연성을 통해 예약 또는 계획된 업그레이드를 보다 비용 효율적으로 계획할 수 있습니다.



## 요약
Azure SQL 데이터베이스 서비스는 Web/Business 데이터베이스 작업을 평가하고 업그레이드에 가장 적합한 서비스 계층을 결정하는 원격 분석 데이터 및 도구를 제공합니다. 업그레이드 프로세스는 매우 간단하며, 데이터베이스를 오프라인으로 전환하지 않고도 데이터 손실 없이 수행할 수 있습니다. 업그레이드된 데이터베이스는 새 서비스 계층이 제공하는 예측 가능한 성능과 추가 기능을 활용할 수 있습니다.
