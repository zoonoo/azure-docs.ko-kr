...
데이터베이스의 진단 설정 블레이드에서 먼저 Intelligent Insights 로깅 ("SQLInsights" 로그)을 사용 하도록 설정 하 고 이러한 대상 중 하나로 스트리밍되는 Intelligent Insights 로그 데이터를 구성 하 여 사용을 위한 Azure SQL 분석, Azure 이벤트 허브, Azure Storage 또는 타사 제품의 통합을 수행 합니다.
