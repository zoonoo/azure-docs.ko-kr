...
- 풀에 포함될 경우 유리한 단일 데이터베이스의 경우 Azure의 지능형 엔진이 사용 패턴 기록이 타당하다고 판단되는 경우 탄력적 데이터베이스 풀을 권장합니다. [Azure 포털을 사용하여 탄력적 데이터베이스 풀 모니터링, 관리 및 크기 조정](sql-database-elastic-pool-manage-portal.md)을 참조하세요. 직접 계산을 수행하는 방법에 대한 자세한 내용은 [탄력적 데이터베이스 풀에 대한 가격 및 성능 고려 사항](sql-database-elastic-pool-guidance.md)을 참조하세요.
