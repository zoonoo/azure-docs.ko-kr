...
## <a name="is-the-vcore-based-model-available-to-sql-database-managed-instance"></a>vCore 기반 모델을 SQL Database Managed Instance에서 사용할 수 있나요?
...
## <a name="are-there-any-database-feature-differences-between-the-existing-dtu-based-and-new-vcore-based-service-tiers"></a>기존 DTU 기반 서비스와 새 vCore 기반 서비스 계층의 데이터베이스 기능에 차이가 있나요?
...
## <a name="can-i-convert-a-database-from-a-vcore-based-service-tier-to-a-dtu-based-one"></a>vCore 기반 서비스 계층에서 DTU 기반 서비스 계층으로 데이터베이스를 변환할 수 있나요?
...
## <a name="what-if-a-single-database-is-active-for-less-than-an-hour-or-uses-a-higher-service-tier-for-less-than-an-hour"></a>단일 데이터베이스가 1시간 미만 동안 활성화되거나 더 높은 서비스 계층을 1시간 미만 동안 사용하면 어떻게 되나요?
...
## <a name="how-does-the-vcore-based-usage-show-up-in-my-bill"></a>청구서에 vCore 기반 사용량이 어떻게 표시되나요?
...
## <a name="how-do-i-manage-databases-and-logins"></a>데이터베이스 및 로그인은 어떻게 관리하나요?
...
## <a name="how-do-i-make-sure-only-authorized-ip-addresses-are-allowed-to-access-a-server"></a>권한이 부여된 IP 주소만 서버에 액세스하도록 만들려면 어떻게 해야 하나요?

[방법: SQL Database에서 방화벽 설정 구성](sql-database-configure-firewall-settings.md)을 참조하세요.

## <a name="what-is-an-expected-replication-lag-when-geo-replicating-a-database-between-two-regions-within-the-same-azure-geography"></a>동일한 Azure 지역 내에서 두 지역 간에 데이터베이스를 지역 복제하는 경우 예상되는 복제 지연은 얼마나 되나요?

현재 5초의 RPO를 지원하고 있으며 복제 지연은 지역 보조 데이터베이스가 Azure에서 권장하는 쌍을 이루는 지역에 호스트되고 동일한 서비스 계층인 경우보다 짧았습니다.

## <a name="what-is-an-expected-replication-lag-when-geo-secondary-is-created-in-the-same-region-as-the-primary-database"></a>지역 보조 데이터베이스가 주 데이터베이스와 동일한 지역에 만들어지는 경우 예상되는 복제 지연은 얼마나 되나요?
