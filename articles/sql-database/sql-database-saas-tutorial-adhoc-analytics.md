...
이 자습서에는 임시 분석 데이터베이스를 만들고 모든 테넌트에서 쿼리를 실행합니다. 이러한 쿼리는 Wingtip SaaS 앱의 일일 운영 데이터에 담긴 정보를 추출할 수 있습니다.
...
> * 테넌트 간 쿼리를 활성화하는 각 데이터베이스의 전역 뷰 정보
...
예제 데이터베이스에서는 장소 이름 해시로부터 정수 ID를 계산했지만 어떤 방법으로든 고유한 값을 도입할 수 있습니다. 카탈로그에서 테넌트 키를 만드는 방식과 유사하지만 *adhocanalytics* 데이터베이스에서 카탈로그 키 및 테넌트 ID가 동일해야 한다는 요건은 없습니다.

*뷰*를 점검하고 어떻게 생성되었는지 보려면
...
1. \\Learning Modules\\Operational Analytics\\Adhoc Analytics\\*Deploy-AdhocAnalyticsDB.ps1*을 엽니다.
1. SQL 스크립트에 `$commandText`를 할당하는 섹션으로 아래로 스크롤합니다. 스크립트를 검토하고 다음을 참고합니다.
...
1. 이제 *PowerShell ISE*에서 \\Learning Modules\\Operational Analytics\\Adhoc Analytics\\*Demo-AdhocAnalytics.ps1*를 열고 다음 값을 설정합니다.
...
![adhocanalytics 데이터베이스](media/sql-database-saas-tutorial-adhoc-analytics/adhocanalytics.png)

## <a name="run-ad-hoc-analytics-queries"></a>임시 분석 쿼리 실행

이제 *adhocanalytics* 데이터베이스가 설정되었고 몇 가지 임시 쿼리를 실행합니다.
