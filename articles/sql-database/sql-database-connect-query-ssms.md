...
이 문서에서는 SQL Server Management Studio(SSMS)를 사용하여 Azure SQL 데이터베이스 서버에 연결하고 Transact-SQL(T-SQL) 문을 통해 간단한 쿼리를 수행하는 방법을 보여 줍니다.
...
논리적 서버에 연결한 후 데이터베이스에 연결하고 샘플 쿼리를 실행할 수 있습니다. [Microsoft Azure SQL 데이터베이스 시작](sql-database-get-started.md)의 AdventureWorks 샘플을 사용하여 데이터베이스를 만들지 않은 경우 이 쿼리는 작동하지 않습니다. 자세한 내용을 보려면 다음 단계로 건너뜁니다.
