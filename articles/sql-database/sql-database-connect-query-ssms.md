<properties
	pageTitle="SSMS을 사용하여 SQL 데이터베이스에 연결 | Microsoft Azure"
	description="SQL Server Management Studio(SSMS)를 사용하여Azure SQL 데이터베이스에 연결하는 방법입니다. 그런 다음 Transact-SQL(T-SQL)를 사용하여 샘플 쿼리를 실행합니다."
	metaCanonical=""
...
이 문서에서는 SQL Server Management Studio(SSMS)를 사용하여 Azure SQL 데이터베이스 서버에 연결하고 Transact-SQL 문을 사용하여 간단한 쿼리를 수행하는 방법을 보여 줍니다.
...
4. 서버를 만들 때 설정한 **로그인** 및 **암호**를 입력한 다음 **연결**을 클릭하여 SQL 데이터베이스에 연결합니다.

	![SQL Server Management Studio: Azure SQL 데이터베이스에 연결](./media/sql-database-connect-query-ssms/1-connect.png)

### SQL 데이터베이스에 대한 연결이 실패하는 경우
...
방화벽 설정으로 인해 연결이 실패하는 경우 최신 버전의 SSMS는 질문 후 방화벽 규칙을 만듭니다. 이를 얻으려면 [SSMS 다운로드](https://msdn.microsoft.com/library/mt238290.aspx)를 참조하세요. 이전 버전을 사용하는 경우 IP 주소는 오류 메시지에서 보고되고 서버 방화벽 규칙에 이 IP 주소를 추가해야 합니다. 자세한 내용은 [방법: 방화벽 설정 구성(Azure SQL 데이터베이스)](sql-database-configure-firewall-settings.md)을 참조하세요.

## 예제 쿼리 실행

SQL 데이터베이스에 연결한 후 예제 쿼리를 실행할 수 있습니다. [Microsoft Azure SQL 데이터베이스 시작](sql-database-get-started.md)의 AdventureWorks 샘플을 사용하여 데이터베이스를 만들지 않은 경우 이 쿼리는 작동하지 않습니다. 자세한 내용을 보려면 다음 단계로 건너뜁니다.
