...
> 
> 탄력적 확장에 대한 자세한 내용은 [Azure SQL 데이터베이스 탄력적 확장 항목](sql-database-elastic-scale-documentation-map.md)을 참조하십시오. 확장성, 유연성 및 성능을 최대화하기 위해 사용자 지정 분할 솔루션을 배포하는 것이 좋습니다. 사용자 지정 분할에 대한 자세한 내용은 [Azure SQL 데이터베이스 확장](http://msdn.microsoft.com/library/azure/dn495641.aspx)을 참조하십시오.
...
|40615|16|로그인에서 요청된 서버 '{0}'을(를) 열 수 없습니다. IP 주소가 '{1}'인 클라이언트는 서버에 액세스할 수 없습니다. 액세스할 수 있게 하려면 SQL 데이터 포털을 사용하거나 master 데이터베이스의 sp\_set\_firewall\_rule을(를) 실행하여 해당 IP 주소 또는 주소 범위에 대한 방화벽 규칙을 만드십시오. 이 변경 내용이 적용되려면 최대 5분까지 걸릴 수 있습니다.|
