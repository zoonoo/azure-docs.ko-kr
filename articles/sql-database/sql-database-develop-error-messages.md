...
다음 표는 페더레이션을 사용하여 작업하는 동안 발생할 수 있는 오류를 보여 줍니다.
...
> 
> 탄력적 확장에 대한 자세한 내용은 [Azure SQL 데이터베이스 탄력적인 확장 항목](sql-database-elastic-scale-documentation-map.md)을 참조하세요. 확장성, 유연성 및 성능을 최대화하기 위해 사용자 지정 분할 솔루션을 배포하는 것이 좋습니다. 사용자 지정 분할에 관한 자세한 내용은 [탄력적 데이터베이스 기능 개요](sql-database-elastic-scale-introduction.md)를 참조하세요.


|오류 번호|심각도|설명|해결 방법|
|---:|---:|:---|:---|
|266|16|<statement> 문을 다중 문 트랜잭션 내에서 사용할 수 없습니다|문을 실행하기 전에 `@@trancount`가 연결에서 0인지 여부를 확인하세요.|
|2072|16|데이터베이스 '%. & #x2a; l s'이(가) 없습니다.|`USE FEDERATION`을 실행하기 전에 데이터베이스 문에 대해 `sys.databases`를 확인하세요.|
...
|45005|16|<statement>은(는) 다른 페더레이션 작업이 페더레이션 <federation_name>에서 진행 중이고 ID가 <member_id>인 멤버의 경우 실행될 수 없습니다.|동시 작업이 완료될 때까지 기다리십시오.|
