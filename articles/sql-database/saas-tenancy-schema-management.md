...
![화면](media/saas-tenancy-schema-management/schema-management.png)
...
1. SSMS를 열고 catalog-dpt-\<user\>.database.windows.net server 카탈로그 서버에 연결
1. tenants1-dpt-\<user\>.database.windows.net 테넌트 서버에도 연결
1. *tenants1-dpt-\<user\>* 서버에서 *contosoconcerthall* 데이터베이스로 이동한 뒤 *VenueTypes* 테이블을 쿼리하여 *Motorcycle Racing* 및 *Swimming Club***이** 결과 목록에 없음을 확인합니다.
...
1. SSMS를 열고 catalog-dpt-&lt;User&gt;.database.windows.net 서버에 연결합니다.
...
* [Wingtip Tickets SaaS Database Per Tenant 응용 프로그램 배포를 기반으로 빌드되는 추가 자습서](saas-dbpertenant-wingtip-app-overview.md#sql-database-wingtip-saas-tutorials)
