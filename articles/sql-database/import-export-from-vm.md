...
description: Azure 서비스가 서버에 액세스 하도록 허용 하지 않고 Azure SQL database를 가져오거나 내보냅니다.
...
---
# <a name="import-or-export-an-azure-sql-database-without-allowing-azure-services-to-access-the-server"></a>Azure 서비스가 서버에 액세스 하도록 허용 하지 않고 Azure SQL database 가져오기 또는 내보내기

이 문서에서는 azure SQL server에서 azure *서비스 허용* 을 *OFF* 로 설정 하는 경우 azure sql database를 가져오거나 내보내는 방법을 보여 줍니다. 워크플로는 Azure 가상 머신을 사용 하 여 SqlPackage를 실행 하 고 가져오기 또는 내보내기 작업을 수행 합니다.
...
자세한 내용은 [SqlPackage](https://docs.microsoft.com/sql/tools/sqlpackage)를 참조 하세요.

## <a name="create-a-firewall-rule-to-allow-the-vm-access-to-the-database"></a>데이터베이스에 대 한 VM 액세스를 허용 하는 방화벽 규칙 만들기

가상 컴퓨터의 공용 IP 주소를 SQL Database 서버 방화벽에 추가 합니다.
...
5. **저장**을 선택합니다. SQL Database 서버에서 포트 1433을 여는 가상 머신의 공용 IP 주소에 대 한 서버 수준 IP 방화벽 규칙이 생성 됩니다.
...
이 예에서는 Active Directory 유니버설 인증을 사용 하 여 SqlPackage를 사용 하 여 데이터베이스를 내보내는 방법을 보여 줍니다. 사용자 환경에 해당 하는 값으로 대체 합니다.
...
다음 SqlPackage 명령은 로컬 저장소에서 Azure SQL Database 서버로 **AdventureWorks2017** 데이터베이스를 가져옵니다. **프리미엄** 서비스 계층과 **P6** 서비스 목표로 **myMigratedDatabase**라는 새 데이터베이스를 만듭니다. 이러한 값을 사용자 환경에 적절하게 변경합니다.
...
- 가져온 SQL Database에 연결 하 고 쿼리 하는 방법에 대 한 자세한 내용은 [빠른 시작: Azure SQL Database: SQL Server Management Studio를 사용 하 여 연결 및 데이터 쿼리](sql-database-connect-query-ssms.md)를 참조 하세요.
