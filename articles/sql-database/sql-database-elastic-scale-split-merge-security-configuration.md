...
## <a name="export-ssl-from-store"></a>인증서 저장소에서 SSL 인증서 내보내기
...
## <a name="upload-ssl"></a>클라우드 서비스에 SSL 인증서 업로드

SSL 키 쌍이 포함된 기존 또는 생성된 .PFX 파일을 업로드합니다.

* 개인 키 정보를 보호하는 암호를 입력합니다.

## <a name="update-ssl-in-csfg"></a>서비스 구성 파일에서 SSL 인증서 업데이트

클라우드 서비스에 업로드된 인증서의 지문으로 서비스 구성 파일의 다음 설정에 대한 지문 값을 업데이트합니다.

    <Certificate name="SSL" thumbprint="" thumbprintAlgorithm="sha1" />

## <a name="import-ssl-ca"></a>SSL 인증 기관 가져오기
...
## <a name="create-self-signed-ca"></a>자체 서명된 인증 기관 만들기
...
## <a name="upload-ca-cert"></a>클라우드 서비스에 CA 인증서 업로드

CA 공개 키가 포함된 기존 또는 생성된 .CER 파일과 함께 인증서를 업로드합니다.

## <a name="update-ca-in-csft"></a>서비스 구성 파일의 CA 인증서 업데이트
...
## <a name="issue-client-certs"></a>클라이언트 인증서 발급
...
## <a name="create-pfx-files"></a>클라이언트 인증서용 PFX 파일 만들기
...
## <a name="configure-allowed-client"></a>서비스 구성 파일에서 허용된 클라이언트 구성
...
사용자 지정:
    ...
    MyID.pvk 및 MyID.cer와 암호화 인증서의 파일 이름
...
[Azure 관리 포털](http://manage.windowsazure.com/)에서 다음을 수행합니다.

1. 클라우드 서비스를 선택합니다.
2. 클라우드 서비스를 선택합니다.
3. 최상위 메뉴에서 인증서를 클릭합니다.
4. 아래쪽 메뉴 모음에서 업로드를 클릭합니다.
5. 인증서 파일을 선택합니다.
6. .PFX 파일인 경우 개인 키에 대한 암호를 입력합니다.
7. 완료되면 목록의 새 항목에서 인증서 지문을 복사합니다.

# <a name="other-security"></a> 기타 보안 고려 사항
...
[인증서 구성]:#configuring-certificates
[허용된 IP 주소]:#allowed-ip-addresses
[클라이언트 인증서를 구성하려면]:#configuring-client-certs
[자체 서명된 인증서 만들기]:#create-self-signed-cert
[자체 서명된 SSL 인증서용 PFX 파일 만들기]:#create-pfx-for-self-signed-cert
[클라우드 서비스에 SSL 인증서 업로드]: #upload-ssl
[서비스 구성 파일에서 SSL 인증서 업데이트]: #update-ssl-in-csfg
[SSL 인증 기관 가져오기]: "import-ssl-ca"
[인증서 저장소에서 SSL 인증서 내보내기]: #export-ssl-from-store
[클라이언트 인증서 기반 인증 해제]: #turn-off-client-cert
[자체 서명된 인증 기관 만들기]:#create-self-signed-ca
[클라이언트 인증서 지문 복사]:#copy-client-cert
[클라우드 서비스에 CA 인증서 업로드]:#upload-ca-cert
[서비스 구성 파일의 CA 인증서 업데이트]:#update-ca-in-csft
[클라이언트 인증서 발급]:#issue-client-certs
[클라이언트 인증서용 PFX 파일 만들기]:#create-pfx-files
[클라이언트 인증서 가져오기]:#import-client-cert
[서비스 구성 파일에서 허용된 클라이언트 구성]:#configure-allowed-client
[CA 공개 키 찾기]:#find-ca-public-key
[클라이언트 인증서 해지 확인 구성]:#configure-client-revocation
[서비스 거부 방지]:#denial-of-service-prevention
[인증서를 얻으려면]:#obtain-certificates
[인증서를 생성하는 도구]:#tools
