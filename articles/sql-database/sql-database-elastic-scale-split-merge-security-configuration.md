...
5.    [서비스 구성 파일에 허용 된 클라이언트 구성] (configure ##Clients Allowed #in #the #Service #Configuration File)
...
암호를 입력하고 다음 옵션을 사용하여 인증서를 내보냅니다.: \*네, 개인키를 내보냅니다 \*모든 확장된 속성을 내보냅니다.
...
* 다음 옵션을 사용하여 .PFX 파일로 인증서를 내보냅니다.
    * 예, 개인 키를 내보냅니다.
    * 인증 경로에 있는 인증서 모두를 포함하고 가능하면 모든 *확장된 속성을 내보냅니다.
...
## 클라이언트 인증서 지문 복사
클라이언트 인증서를 발급한 각 개인이 서비스 구성 파일에 추가할 인증서의 지문을 가져오려면 다음 단계를 따라야 합니다.:\*certmger.exe를 실행\* 개인 탭을 눌러 \*인증에 사용될 클라이언트 인증서를 더블 클릭합니다\* 인증대화상자가 열리면, 세부사항 탭을 눌러\* 모두 표시하기\*를 선택합니다. 목록에서 지문을 선택하고 \*지문값을 복사\*\*첫 번째 숫자 앞에 표시되지 않는 유니코드 문자를 삭제합니다\*\* 모든 여백을 삭제합니다.
...
암호를 입력하고 다음 옵션을 사용하여 인증서를 내보냅니다. \*예, 개인키를 내보냅니다 \*모든 확장된 속성을 내보냅니다.\*클라우드 서비스에 인증서를 업로드 할 때 암호가 필요합니다.
...
1. 저장소 위치를 선택합니다.
    ...
    * **현재 사용자** - 현재 사용자 계정으로 실행되는 프로세스만 서비스에 액세스하는 경우
...
[Azure 관리 포털](http://portal.azure.com/)에서.

1. **클라우드 서비스**를 선택합니다.
2. 클라우드 서비스를 선택합니다.
3. 최상위 메뉴에서 **인증서**를 클릭합니다.
4. 아래 표시줄에서 **업로드**를 클릭합니다.
5. 인증서 파일을 선택합니다.
