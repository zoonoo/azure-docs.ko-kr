...
이 자습서에서는 테넌트 데이터베이스의 전체 집합에서 배포된 쿼리를 실행하여 임시 대화형 보고를 사용하도록 설정합니다. 이러한 쿼리는 Wingtip Tickets SaaS 앱의 일일 운영 데이터에 담긴 정보를 추출할 수 있습니다. 이를 위해 추가 분석 데이터베이스를 카탈로그 서버에 배포하고 탄력적 쿼리를 사용하여 배포된 쿼리를 사용하도록 설정합니다.
...
![임시 보고 패턴](media/saas-tenancy-adhoc-analytics/adhocreportingpattern_dbpertenant.png)

SaaS 응용 프로그램을 사용했을 때의 큰 장점 중 하나는 응용 프로그램 작업 및 사용에 대한 통찰력을 확보하기 위해 클라우드 중앙에 저장된 광범위한 테넌트 데이터를 사용할 수 있다는 점입니다. 이러한 정보는 기능 개발, 활용성 향상 및 기타 앱 및 서비스 투자를 유도할 수 있습니다.
...
3. SQL 스크립트를 검토하고 다음 사항을 참고합니다.
   ...
   이전 섹션에서 설명한 전역 뷰를 참조하고 **DISTRIBUTION = SHARDED(VenueId)**로 정의되는 외부 테이블입니다. 각 *VenueId*가 단일 데이터베이스에 매핑되기 때문에 다음 섹션에 표시된 대로 다양한 시나리오의 성능이 향상됩니다.
    ...
    ![외부 테이블 만들기](media/saas-tenancy-adhoc-analytics/external-tables.png)
   ...
   생성되고 채워진 로컬 테이블 *VenueTypes*. 이 참조 데이터 테이블은 모든 테넌트 데이터베이스에서 일반적이므로 여기에서 로컬 테이블로 표시되며 일반 데이터로 채워질 수 있습니다. 이 일부 쿼리의 경우 테넌트 데이터베이스와 *adhocreporting* 데이터베이스 간에 이동되는 데이터의 양을 줄일 수 있습니다.
    ...
    ![테이블 만들기](media/saas-tenancy-adhoc-analytics/create-table.png)
