---
title: Azure SQL 데이터베이스 백업 및 복원 문제
...
4. 새 데이터베이스를 참조하도록 필요에 따라 앱을 조정합니다. [지정 시점으로 데이터베이스 복구](sql-database-recovery-using-backups.md#point-in-time-restore)를 참조하세요.
...
5. 새 데이터베이스를 참조하도록 필요에 따라 앱을 조정합니다. [삭제된 데이터베이스 복구](sql-database-recovery-using-backups.md#deleted-database-restore)를 참조하세요.

## 지역별 데이터 센터 가동 중단에서 복구하려면
표준 및 프리미엄 데이터베이스가 있고 지리적 복제 보조를 설정한 경우 이러한 보조를 사용하여 복구할 수 있습니다. 이렇게 하면 데이터 손실 가능성을 줄이고 데이터베이스를 복원할 수 있습니다. 자세한 내용은 [자동화된 데이터베이스 백업을 사용하여 Azure SQL 데이터베이스 복구](sql-database-disaster-recovery.md)를 참조하세요.
