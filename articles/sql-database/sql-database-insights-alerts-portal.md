...
4. **조건** 섹션에서 **추가**를 클릭 합니다.
  조건 정의를 ![](media/sql-database-insights-alerts-portal/create-rule.png)
5. **신호 논리 구성** 페이지에서 신호를 선택 합니다.
  신호](media/sql-database-insights-alerts-portal/select-signal.png)![선택 합니다.
6. **CPU 백분율**등의 신호를 선택한 후 **신호 논리 구성** 페이지가 나타납니다.
  신호 논리를 구성 ![](media/sql-database-insights-alerts-portal/configure-signal-logic.png)
...
8. **만들기 규칙**에서 기존 **작업 그룹** 을 선택 하거나 새 그룹을 만듭니다. 작업 그룹을 사용 하면 경고 조건이 발생할 때 수행할 작업을 정의할 수 있습니다.
  작업 그룹](media/sql-database-insights-alerts-portal/action-group.png) 정의 ![
