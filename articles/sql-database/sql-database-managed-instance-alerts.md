...
이 문서에서는 Azure Portal를 사용 하 여 Azure SQL Managed Instance 데이터베이스의 데이터베이스에 대 한 경고를 설정 하는 방법을 보여 줍니다. 일부 메트릭 (예: 인스턴스 저장소 크기 또는 CPU 사용량)이 임계값에 도달 하면 경고에서 전자 메일을 보내거나 웹 후크를 호출할 수 있습니다. 이 문서는 또한 경고 기간 설정에 대한 모범 사례를 제공합니다.
