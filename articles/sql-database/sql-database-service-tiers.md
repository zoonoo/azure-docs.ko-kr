...
서비스 계층을 결정하려면 먼저 데이터베이스가 독립 실행형 데이터베이스인지 아니면 탄력적 풀의 일부인지를 확인합니다.

### 독립 실행형 데이터베이스에 대한 서비스 계층 선택

독립 실행형 데이터베이스에 대한 서비스 계층을 결정하려면 먼저 SQL 데이터베이스 버전을 선택하기 위해 필요한 데이터베이스 기능을 확인합니다.
...
탄력적 데이터베이스 풀에 대한 서비스 계층을 결정하려면 먼저 풀에 대한 서비스 계층을 선택하기 위해 필요한 데이터베이스 기능을 확인합니다.
...
풀에 대한 서비스 계층을 결정하면 풀에 대한 성능 수준을 결정할 준비가 되었습니다(eDTU). 실제 경험을 토대로 추측한 다음 [동적으로 강화 또는 축소](sql-database-elastic-pool-manage-portal.md#change-performance-settings-of-a-pool)할 수 있습니다. [DTU 계산기](http://dtucalculator.azurewebsites.net/)를 사용하여 풀 내에서 개별 데이터베이스에 필요한 DTU의 수를 대략적으로 예상하여 풀에 대한 상한값을 설정하도록 도움이 될 수도 있습니다.
