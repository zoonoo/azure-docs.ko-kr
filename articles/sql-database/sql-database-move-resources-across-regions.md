...
1. 서버 수준 감사에 대해 다음을 확인 합니다.
   ...
   - 감사는 대상 서버에서 구성 됩니다. 자세한 내용은 [SQL database 감사 시작](sql-database-auditing.md)을 참조 하세요. 
...
**ReplicationState** 이 인 `2`경우에는 보조 끝점 `<fog-name>.secondary.database.windows.net` 을 사용 하 여 각 데이터베이스 또는 데이터베이스의 하위 집합에 연결 하 고 데이터베이스에 대 한 쿼리를 수행 하 여 연결, 적절 한 보안 구성 및 데이터 복제를 보장 합니다. 
