...
description: 이 페이지에서는 Azure SQL Database의 탄력적 풀에 대한 몇 가지 일반적인 vCore 리소스 제한에 대해 설명합니다.
...
ms.locfileid: "80346277"
---
# <a name="resource-limits-for-elastic-pools-using-the-vcore-purchasing-model"></a>vCore 구매 모델을 사용하는 탄력적 풀의 리소스 제한

이 문서에서는 vCore 구매 모델을 사용하여 Azure SQL Database 탄력적 풀 및 풀린 데이터베이스에 대한 자세한 리소스 제한을 제공합니다.

DTU 구매 모델 제한은 [SQL Database DTU 리소스 제한 - 탄력적 풀을](sql-database-dtu-resource-limits-elastic-pools.md)참조하십시오.
...
|풀 <sup>1당</sup> 최대 B수 B|100|200|500|500|500|500|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS |400|800|1200|1600|2000|2400|
|풀당 최대 로그 비율(MBps)|4.7|9.4|14.1|18.8|23.4|28.1|
|풀당 최대 동시 작업자(요청) <sup>3</sup> |210|420|630|840|1050|1260|
|풀 <sup>3당</sup> 최대 동시 로그인 |210|420|630|840|1050|1260|
...
<sup>1</sup> 추가 고려 사항은 [조밀한 탄력적 풀의 리소스 관리를](sql-database-elastic-pool-resource-management.md) 참조하십시오.

<sup>2</sup> 8KB에서 64KB 사이의 IO 크기에 대한 최대값입니다. 실제 IOPS는 워크로드에 따라 다릅니다. 자세한 내용은 [데이터 IO 거버넌스](sql-database-resource-limits-database-server.md#resource-governance)를 참조하십시오.

<sup>3</sup> 개별 데이터베이스에 대한 최대 동시 작업자(요청)의 경우 [단일 데이터베이스 리소스 제한을](sql-database-vcore-resource-limits-single-databases.md)참조하십시오. 예를 들어 탄력적 풀에서 Gen5를 사용하고 있고 데이터베이스당 최대 vCore가 2로 설정된 경우 최대 동시 작업자 값은 200입니다.  데이터베이스당 최대 vCore가 0.5로 설정된 경우 Gen5에서는 vCore당 최대 100명의 동시 작업자가 있기 때문에 최대 동시 작업자 값은 50입니다. vCore 수가 1개 이하인 다른 데이터베이스당 최대 vCore 설정의 경우 최대 동시 작업자 수가 유사하게 크기 조정됩니다.
...
|풀 <sup>1당</sup> 최대 B수 B|500|500|500|500|500|500|
|Columnstore 지원 여부|yes|yes|yes|yes|yes|yes|
|메모리 내 OLTP 스토리지(GB)|해당 없음|해당 없음|해당 없음|해당 없음|해당 없음|해당 없음|
...
|스토리지 유형|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|
|IO 대기 시간(근사치)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|
|풀 <sup>2당</sup> 최대 데이터 IOPS|2800|3200|3600|4000|6400|9600|
|풀당 최대 로그 비율(MBps)|32.8|37.5|37.5|37.5|37.5|37.5|
...
|풀 <sup>1당</sup> 최대 B수 B|100|200|500|500|500|500|500|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|800|1600|2400|3200|4000|4800|5600|
|풀당 최대 로그 비율(MBps)|9.4|18.8|28.1|37.5|37.5|37.5|37.5|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|210|420|630|840|1050|1260|1470|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|210|420|630|840|1050|1260|1470|
...
|풀 <sup>1당</sup> 최대 B수 B|500|500|500|500|500|500|500|
|Columnstore 지원 여부|yes|yes|yes|yes|yes|yes|yes|
|메모리 내 OLTP 스토리지(GB)|해당 없음|해당 없음|해당 없음|해당 없음|해당 없음|해당 없음|해당 없음|
...
|스토리지 유형|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|프리미엄(원격) 스토리지|
|IO 대기 시간(근사치)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|5-7ms(쓰기)<br>5-10ms(읽기)|
|풀 <sup>2당</sup> 최대 데이터 IOPS |6,400|7,200|8,000|9,600|12,800|16,000|32,000|
|풀당 최대 로그 비율(MBps)|37.5|37.5|37.5|37.5|37.5|37.5|37.5|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|1,680|1890|2100|2520|3,360|4200|8400|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|1,680|1890|2100|2520|3,360|4200|8400|
|최대 동시 세션|30,000|30,000|30,000|30,000|30,000|30,000|30,000|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|16,000|
|풀당 최대 로그 비율(MBps)|37.5|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|3780|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|3780|
|최대 동시 세션|30,000|
...
### <a name="business-critical-service-tier-generation-4-compute-platform-part-1"></a>비즈니스 크리티컬 서비스 계층: 4세대 컴퓨팅 플랫폼(1부)
...
|풀 <sup>1당</sup> 최대 B수 B|50|100|100|100|100|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|9,000|13,500|18,000|22,500|27,000|
|풀당 최대 로그 비율(MBps)|20|30|40|50|60|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|420|630|840|1050|1260|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|420|630|840|1050|1260|
...
|복제본 수|4|4|4|4|4|
|다중 AZ|yes|yes|yes|yes|yes|
|읽기 확장|yes|yes|yes|yes|yes|
...
### <a name="business-critical-service-tier-generation-4-compute-platform-part-2"></a>비즈니스 크리티컬 서비스 계층: 4세대 컴퓨팅 플랫폼(2부)
...
|vCore 수|7|8|9|10|16|24|
|메모리(GB)|49|56|63|70|112|159.5|
|풀 <sup>1당</sup> 최대 B수 B|100|100|100|100|100|100|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|31,500|36,000|40,500|45,000|72,000|96,000|
|풀당 최대 로그 비율(MBps)|70|80|80|80|80|80|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|1470|1,680|1890|2100|3,360|5040|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|1470|1,680|1890|2100|3,360|5040|
|최대 동시 세션|30,000|30,000|30,000|30,000|30,000|30,000|
|데이터베이스별로 최소/최대 탄력적 풀 vcore 선택|0, 0.25, 0.5, 1...7|0, 0.25, 0.5, 1...8|0, 0.25, 0.5, 1...9|0, 0.25, 0.5, 1...10|0, 0.25, 0.5, 1...10, 16|0, 0.25, 0.5, 1...10, 16, 24|
|복제본 수|4|4|4|4|4|4|
...
### <a name="business-critical-service-tier-generation-5-compute-platform-part-1"></a>비즈니스 크리티컬 서비스 계층: 5세대 컴퓨팅 플랫폼(1부)
...
|풀 <sup>1당</sup> 최대 B수 B|50|100|100|100|100|100|
|Columnstore 지원 여부|yes|yes|yes|yes|yes|yes|
...
|스토리지 유형|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|
|IO 대기 시간(근사치)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|
|풀 <sup>2당</sup> 최대 데이터 IOPS|18,000|27,000|36,000|45,000|54,000|63,000|
|풀당 최대 로그 비율(MBps)|60|90|120|120|120|120|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|420|630|840|1050|1260|1470|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|420|630|840|1050|1260|1470|
|최대 동시 세션|30,000|30,000|30,000|30,000|30,000|30,000|
...
### <a name="business-critical-service-tier-generation-5-compute-platform-part-2"></a>비즈니스 크리티컬 서비스 계층: 5세대 컴퓨팅 플랫폼(2부)
...
|vCore 수|16|18|20|24|32|40|80|
|메모리(GB)|83|93.4|103.8|124.6|166.1|207.6|415.2|
|풀 <sup>1당</sup> 최대 B수 B|100|100|100|100|100|100|100|
|Columnstore 지원 여부|yes|yes|yes|yes|yes|yes|yes|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|72,000|81,000|90,000|108,000|144,000|180,000|256,000|
|풀당 최대 로그 비율(MBps)|120|120|120|120|120|120|120|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|1,680|1890|2100|2520|3,360|4200|8400|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|1,680|1890|2100|2520|3,360|4200|8400|
|최대 동시 세션|30,000|30,000|30,000|30,000|30,000|30,000|30,000|
...
|풀 <sup>2당</sup> 최대 데이터 IOPS|200,000|
|풀당 최대 로그 비율(MBps)|333|
|풀당 최대 동시 작업자(요청) <sup>3</sup>|13,440|
|풀당 최대 동시 로그인(요청) <sup>3</sup>|13,440|
|최대 동시 세션|30,000|
