...
## <a name="general-questions"></a>일반 질문
...
복잡 한 쿼리를 사용 하 여 대규모 데이터 분석을 실행 하 고, 100 m b/초 보다 높은 수집 속도를 유지 하거나, PDW (병렬 데이터 웨어하우스), Teradata 또는 다른 대규모 병렬 처리 (MPP) 데이터 웨어하우스를 사용 하는 경우에는 SQL Data Warehouse 수 있습니다. 최상의 선택입니다.
...
아닙니다. RAM을 더 확보하려면 더 큰 컴퓨팅 크기로 업그레이드해야 합니다. 자세한 내용은 [하이퍼스케일 스토리지 및 컴퓨팅 크기](sql-database-vcore-resource-limits-single-databases.md#hyperscale-service-tier-for-provisioned-compute)를 참조하세요.
...
아닙니다. 백업은 저장소 하위 시스템에 의해 관리 되며 저장소 스냅숏을 활용 합니다. 주 서버의 사용자 작업에는 영향을 주지 않습니다.
...
아닙니다. Azure SQL Database에서는 R 및 Python이 지원되지 않습니다.
...
아닙니다. 읽기 전용 의도를 사용 하는 연결은 임의의 읽기 스케일 아웃 복제본으로 리디렉션됩니다.
