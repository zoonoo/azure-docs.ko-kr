...
- 거의 즉시 백업 (Azure Blob storage에 저장 된 파일 스냅숏을 기준) 계산 리소스에서 IO의 영향이 없습니다를 사용 하 여 크기에 관계 없이 데이터베이스  
...
| 문제 | 설명 |
| :---- | :--------- |
| 논리 서버에 대 한 백업 관리 창에 SQL server에서 대규모 데이터베이스를 필터링 할 수는 표시 되지 않습니다.  | 대규모 백업을 관리 하기 위한 별도 메서드가 및 장기 보존 및 백업 보존 설정 시간 점을 적용 되지 않습니다 하 / 무효화 됩니다 같이 합니다. 따라서 하이퍼스케일 데이터베이스가 백업 관리 창에 나타나지 않습니다. |
