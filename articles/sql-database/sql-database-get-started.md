---
title: "Azure Portal: 단일 SQL Database 만들기 및 쿼리 | Microsoft Docs"
...
Azure SQL Database는 정의된 계산 및 저장소 리소스 집합과 함께 논리 서버의 Azure 리소스 그룹 내에 생성됩니다. 다음 단계에 따라서 Adventure Works LT 샘플 데이터를 포함하는 SQL Database를 만듭니다. 

1. Azure Portal의 왼쪽 위에 있는 **새로 만들기** 단추를 클릭합니다.

2. **새** 블레이드에서 **데이터베이스**를 선택하고 **데이터베이스** 블레이드에서 **SQL Database**를 선택합니다.
...
SQL Database 서비스는 외부 응용 프로그램 및 도구가 새 서버 및 데이터베이스로 연결하지 못하도록 하는 방화벽을 생성합니다. 다음 단계에 따라 IP 주소에 대한 SQL Database 서버 수준 방화벽 규칙을 만들어 SQL Database 방화벽을 통한 외부 연결을 사용하도록 설정합니다. 
...
자습서 뒷부분을 위해 이 빠른 시작에서 만든 리소스를 저장하도록 선택할 수 있습니다. 또는 다음 단계에 따라, Azure Portal에서 이 빠른 시작에서 만든 모든 리소스를 삭제할 수 있습니다.
