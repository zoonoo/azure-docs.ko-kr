...
>[AZURE.IMPORTANT]배포가 완료되면 Azure에 새 Azure SQL 서버 및 데이터베이스가 만들어집니다. Azure 외부에서 연결에 대한 SQL Database 방화벽을 열기 위한 서버 방화벽 규칙을 만들 때까지 SQL Server 도구를 사용하여 새 서버 또는 데이터베이스에 연결할 수 없습니다.

[AZURE.INCLUDE [서버 방화벽 규칙 만들기](../../includes/sql-database-create-new-server-firewall-portal.md)]
