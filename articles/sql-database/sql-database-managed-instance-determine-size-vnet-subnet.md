---
title: 관리되는 인스턴스가 VNet/서브넷 크기를 결정합니다.
...
기본적으로 Managed Instance는 서브넷에서 최소 16개의 IP 주소가 필요하고 최대 256개의 IP 주소를 사용할 수 있습니다. 따라서 서브넷 IP 범위를 정의할 때 /28과 /24 사이의 서브넷 마스크를 사용할 수 있습니다. /28(네트워크당 14개의 호스트)의 네트워크 마스크 비트는 단일 범용 또는 비즈니스 크리티컬 배포에 적합한 크기입니다. 마스크 비트 /27(네트워크당 30개 호스트)은 동일한 VNet 내에서 여러 관리되는 인스턴스 배포에 이상적입니다. 마스크 비트 설정 /26 (62 호스트) 및 /24 (254 호스트) 추가 관리되는 인스턴스를 지원 하기 위해 VNet에서 추가 확장 할 수 있습니다.

> [!IMPORTANT]
> IP 주소가 16개인 서브넷 크기는 vCore 크기 변경과 같은 크기 조정 작업이 지원되지 않는 제한된 잠재력을 가진 최소 크기입니다. 접두사 /27 또는 가장 긴 접두사가 있는 서브넷을 선택하는 것이 좋습니다.
...
- 개요는 [관리되는 인스턴스입니다.](sql-database-managed-instance.md)
...
- DNS 문제의 경우 [사용자 지정 DNS 구성](sql-database-managed-instance-custom-dns.md) 을 참조하세요.
