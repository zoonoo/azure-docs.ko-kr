---
title: Managed Instance에 따라 VNet/서브넷 크기가 결정됩니다.
...
기본적으로 Managed Instance는 서브넷에서 최소 16개의 IP 주소가 필요하고 최대 256개의 IP 주소를 사용할 수 있습니다. 결과적으로 서브넷 IP 범위를 정의할 때 /28과 /24 사이의 서브넷 마스크를 사용할 수 있습니다. 네트워크 마스크 비트 /28(네트워크당 14개 호스트)은 단일 범용 또는 중요 비즈니스용 배포에 적합한 크기입니다. 마스크 비트 /27(네트워크당 30개 호스트)은 동일한 VNet 내의 여러 Managed Instance 배포에 적합합니다. 마스크 비트 설정 /26(62개 호스트) 및 /24(254개 호스트)는 추가 Managed Instance를 지원하기 위해 VNet을 추가로 확장할 수 있도록 합니다.

> [!IMPORTANT]
> 16개의 IP 주소를 사용하는 서브넷 크기는 vCore 크기 변경과 같은 크기 조정 작업이 지원되지 않는 제한된 가능성이 있는 최소 크기입니다. 접두사 /27 또는 가장 긴 접두사가 있는 서브넷을 선택하는 것이 좋습니다.
