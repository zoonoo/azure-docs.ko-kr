...
다음 표를 사용하여 **기본** 탭에 필요한 필수 정보를 채웁니다. 관리형 인스턴스를 프로비저닝하기 위한 최소 정보 집합입니다.
...
다음 표를 사용하여 **네트워킹** 탭에 선택적 정보를 채웁니다. 이 정보를 생략하면 포털에서 기본 설정이 적용됩니다.
...
다음 표를 사용하여 **추가 설정** 탭에 선택적 정보를 채웁니다. 이 정보를 생략하면 포털에서 기본 설정이 적용됩니다.
...
1. 알림에서 **배포 진행 중**을 선택하여 관리형 인스턴스 창을 열고 배포 진행 상황을 자세히 모니터링합니다. 

> [!TIP]
> 웹 브라우저를 닫았거나 배포 진행률 화면에서 벗어난 경우에는 다음 단계에 따라 해당 화면을 찾습니다.
> 1. Azure Portal에서 관리형 인스턴스를 배포할 리소스 그룹(**기본** 탭)을 엽니다.
> 2. **배포**를 선택합니다.
> 3. 진행 중인 관리형 인스턴스 배포 작업을 선택합니다.

## <a name="review-resources-and-retrieve-your-host-name"></a>리소스를 검토하고 호스트 이름을 검색합니다.
...
2. 자동으로 생성된 UDR(사용자 정의 경로)을 검토할 경로 테이블을 선택합니다.

   ![경로 테이블](./media/sql-database-managed-instance-get-started/route-table.png)

3. 경로 테이블에서는, 관리형 인스턴스의 가상 네트워크 내에서 트래픽을 라우팅할 항목을 검토합니다. 경로 테이블을 수동으로 만들거나 구성하는 경우에는 경로 테이블에 이러한 항목을 만들어야 합니다.

   ![로컬에 대한 관리형 인스턴스 서브넷의 항목](./media/sql-database-managed-instance-get-started/udr.png)

4. 리소스 그룹으로 돌아가서, 네트워크 보안 그룹을 선택합니다.
...
7. **개요** 탭에서 **호스트** 속성을 찾습니다. 다음 빠른 시작에서 사용할 수 있도록 관리형 인스턴스의 호스트 이름을 복사합니다.

   ![호스트 이름](./media/sql-database-managed-instance-get-started/host-name.png)

   호스트 이름은 FQDN(정규화된 도메인 이름)이라고도 합니다. *your_machine_name.a1b2c3d4e5f6.database.windows.net*과 유사합니다.
