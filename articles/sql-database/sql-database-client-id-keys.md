---
title: 앱을 등록하고 코드에서 SQL 데이터베이스에 연결하기 위한 클라이언트 ID 및 키 가져오기 | Microsoft Docs
...
2. 메뉴에서 **Active Directory**를 찾은 후 선택합니다.
   
    ![AAD][1]
3. 디렉터리를 선택하여 응용 프로그램을 인증하고 해당 **이름**을 클릭합니다.
   
    ![디렉터리][4]
4. 디렉터리 페이지에서 **응용 프로그램**을 클릭합니다.
   
    ![응용 프로그램][5]
5. **추가**를 클릭하여 새 응용 프로그램을 만듭니다.
   
    ![응용 프로그램 추가][6]
6. 앱의 **이름**을 입력하고 **네이티브 클라이언트 응용 프로그램**을 선택합니다.
   
    ![응용 프로그램 추가][7]
7. **리디렉션 URI**를 입력합니다. 실제 끝점일 필요는 없으며 유효한 URI이면 됩니다.
   
    ![응용 프로그램 추가][8]
8. 앱 만들기를 완료하고 **구성**을 클릭한 후 **클라이언트 ID**를 복사합니다(코드에 클라이언트 ID가 필요함).
   
    ![클라이언트 ID 가져오기][9]
9. 페이지 아래로 스크롤하여 **응용 프로그램 추가**를 클릭합니다.
10. **Microsoft 앱**을 선택합니다.
11. **Microsoft Azure Service Management API**를 선택하고 마법사를 완료합니다.
12. **다른 응용 프로그램에 대한 권한** 섹션에서 **Microsoft Azure Service Management API**를 찾은 후 **위임된 권한**을 클릭합니다.
13. **Azure 서비스 관리 액세스...**를 선택합니다.
    
     ![권한][2]
14. 페이지 아래쪽에서 **저장**을 클릭합니다.
...
5. **추가**를 클릭하여 새 응용 프로그램을 만듭니다.
    ...
    ![응용 프로그램 추가][6]
6. 앱의 **이름**을 입력하고 **웹 응용 프로그램 및/또는 웹 API**를 선택합니다.
   
    ![응용 프로그램 추가][10]
7. **로그온 URL** 및 **앱 ID URI**를 제공합니다. 실제 끝점일 필요는 없으며 유효한 URI이면 됩니다.
   
    ![응용 프로그램 추가][11]
8. 앱 만들기를 마치면 **구성**을 클릭합니다.
   
    ![구성][12]
9. **키** 섹션으로 스크롤한 후 **기간 선택** 목록에서 **1년**을 선택합니다. 저장한 후에 키 값이 표시되므로 나중에 돌아와서 키를 복사합니다.
   
    ![키 보존 기간 설정][13]
10. 페이지 아래로 스크롤하여 **응용 프로그램 추가**를 클릭합니다.
11. **Microsoft 앱**을 선택합니다.
12. **Microsoft Azure Service Management API**를 찾아 선택하고 마법사를 완료합니다.
13. **다른 응용 프로그램에 대한 권한** 섹션에서 **Microsoft Azure Service Management API**를 찾은 후 **위임된 권한**을 클릭합니다.
14. **Azure 서비스 관리 액세스...**를 선택합니다.
    
     ![권한][2]
15. 페이지 아래쪽에서 **저장**을 클릭합니다.
16. 저장이 완료되면 클라이언트 ID 및 키를 찾아 저장합니다.
