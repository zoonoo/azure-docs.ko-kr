<properties 
   pageTitle="Visual Studio에서 연결된 서비스를 사용하여 모바일 서비스 추가"
...
1. **연결된 서비스 추가 **대화 상자에서 **Azure 모바일 서비스**를 선택한 다음 **구성** 단추를 선택합니다. 아직 수행하지 않은 경우 Azure에 로그인하도록 메시지가 표시될 수 있습니다.

    ![Azure 모바일 서비스 추가](./media/vs-azure-tools-connected-services-add-mobile-services/IC797636.png)

1. Azure 모바일 서비스 대화 상자에서, 서비스가 있는 경우 기존 모바일 서비스를 선택 합니다. 새 Azure 모바일 서비스를 만들어야 하는 경우 다음 절차에 따라 다음을 수행합니다. 그렇지 않은 경우 다음 단계로 건너뜁니다.
    ...
    2. **모바일 서비스 만들기** 대화 상자에서 JavaScript 백 엔드 모바일 서비스 또는 .NET 백 엔드 모바일 서비스를 **런타임** 드롭다운 목록에서 선택할 수 있습니다. 
        ...
        JavaScript 백 엔드 서비스는 단순하며 강력합니다. JavaScript 백 엔드 모바일 서비스를 만드는 경우 서버쪽 JavaScript 코드가 클라우드에서 저장되지만 서버 탐색기 또는 Azure 관리 포털을 사용하여 서버 스크립트를 편집할 수 있습니다.
    ...
    2. 새 모바일 서비스가 **Azure 모바일 서비스** 대화 상자의 서비스 목록에 나타나야 합니다. 목록에서 새 모바일 서비스를 선택한 다음 **추가** 단추를 선택하여 프로젝트에 서비스를 추가합니다.
