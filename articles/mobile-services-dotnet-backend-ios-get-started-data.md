...
3. [로컬로 서비스 다운로드][로컬로 서비스 다운로드]
4. [모바일 서비스 테스트][모바일 서비스 테스트]
5. [Azure에 모바일 서비스 게시][Azure에 모바일 서비스 게시]
6. [모바일 서비스를 사용하도록 앱 업데이트][모바일 서비스를 사용하도록 앱 업데이트]
7. [모바일 서비스에 대해 앱 테스트][모바일 서비스에 대해 앱 테스트]

이 자습서를 사용하려면 다음이 필요합니다.

+ [모바일 서비스 iOS SDK][모바일 서비스 iOS SDK]와 [XCode 4.5][XCode 4.5] 및 iOS 5.0 이상 버전.
+ Visual Studio 2013([Visual Studio Express for Web][Visual Studio Express for Web] 무료 이용 가능).
...
2. 다운로드한 프로젝트를 Xcode에서 열고 TodoService.m 파일을 검사합니다.
   	...
   	모바일 서비스에서 이 앱이 작동하도록 설정하기 위해 수행해야 하는 단계를 지정하는 **// TODO** 주석이 8개 있습니다.
...
1. Xcode에서 배포 대상 에뮬레이터(iPhone 또는 iPad)를 선택하고 **Run** 단추(또는 Command+R 키)를 눌러 프로젝트를 다시 빌드하고 앱을 시작합니다.
   	...
   	그러면 iOS SDK로 작성된 Azure 모바일 서비스 클라이언트가 실행되어 모바일 서비스에서 항목을 쿼리합니다.

2.  앞에서 한 것처럼 텍스트 상자에 텍스트를 입력한 후 **+** 단추를 클릭합니다.

   	그러면 새 항목이 모바일 서비스에 삽입으로 전송됩니다. 각각의 새 todoItem이 저장되고, Azure 관리 포털에서 모바일 서비스에 대해 전에 구성한 SQL 데이터베이스에서 업데이트됩니다.

3. 변경 사항이 Azure의 데이터베이스에서 유지되었는지 확인하려면 앱을 중지하고 다시 시작합니다.

   	Azure 관리 포털 또는 Visual Studio의 SQL Server 개체 탐색기를 사용하여 데이터베이스를 검토할 수도 있습니다. 다음의 두 단계에서는 [Azure 관리 포털][Azure 관리 포털]을 사용하여 데이터베이스의 변경 사항을 확인합니다.
