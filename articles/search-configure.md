...
1.  **찾아보기** | **모두** | **데이터, 저장소, 캐시 + 백업** | **모두 보기** | **검색 서비스**로 이동한 다음 검색 서비스를 클릭하여 서비스 대시보드를 엽니다.

    ![][7]

2.  서비스 대시보드에는 **속성** 및 **키**에 대한 타일과 리소스 사용을 한눈에 보여 주는 사용 정보가 표시됩니다.
...
1.  호스트와 api-key(소문자)를 해당 서비스에 유효한 값으로 바꾸어 요청 헤더를 지정합니다.
...
2.  요청 본문에 인덱스 정의를 구성하는 필드를 붙여 넣습니다.
...
몇 초 후에 인덱스가 성공적으로 만들어졌음을 나타내는 HTTP 204 응답이 검색 목록에 표시됩니다.

HTTP 504가 표시될 경우 URL이 HTTPS를 지정하는지 확인합니다. HTTP 404가 표시될 경우 구문을 다시 확인합니다. HTTP 400은 api-key에 문제가 있음을 의미합니다(잘못된 키 또는 api-key 지정 방법과 관련된 구문 문제).
...
<!--Image references-->
  ...
  [가격 정보]: http://go.microsoft.com/fwlink/p/?LinkID=509792
  [10]: ./media/search-configure/AzureSearch_Configure1_10_ScaleUp.PNG
  [Telerik에서 무료 다운로드]: http://www.telerik.com/fiddler
