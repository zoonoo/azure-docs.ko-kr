...
1.  [Azure 스토어를 사용하여 New Relic에 등록][Azure 스토어를 사용하여 New Relic에 등록]
2.  [New Relic 모듈 설치][New Relic 모듈 설치]
3.  [모바일 서비스에 New Relic 개발자 분석 사용][모바일 서비스에 New Relic 개발자 분석 사용]
4.  [New Relic 대시보드에서 모바일 서비스 모니터링][New Relic 대시보드에서 모바일 서비스 모니터링]
...
1.  이렇게 하지 않은 경우에는 [소스 제어의 스토어 서버 스크립트][소스 제어의 스토어 서버 스크립트] 자습서에 나온 단계를 따라 모바일 서비스에 대해 소스 제어를 사용하도록 설정하고 리포지토리를 복제하고 [NPM(Node Package Manager)][NPM(Node Package Manager)]을 설치합니다.

2.  로컬 Git 리포지토리의 `.\service` 폴더로 이동한 후 명령 프롬프트에서 다음 명령을 실행합니다.

        npm install newrelic

    NPM은 `\newrelic` 하위 디렉터리에 [New Relic 모듈][New Relic 모듈]을 설치합니다.
...
그런 다음, [관리 포털][관리 포털]에서 모바일 서비스의 New Relic 모니터링을 사용할 수 있도록 설정합니다.
...
2.  잠시 기다렸다가 데이터가 처리된 다음 New Relic 대시보드로 이동합니다.
    ...
    New Relic 구독을 추가 기능으로 구입했으면 [관리 포털][관리 포털]에서 그 구독을 선택하고 **관리**를 클릭합니다.
