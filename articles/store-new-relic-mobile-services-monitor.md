...
1.  [Azure 스토어를 사용하여 New Relic에 등록](#sign-up)
2.  [New Relic 모듈 설치](#install-module)
3.  [모바일 서비스에 New Relic 개발자 분석 사용](#enable-service)
4.  [New Relic 대시보드에서 모바일 서비스 모니터링](#monitor)
...
1.  이렇게 하지 않은 경우에는 [소스 제어의 스토어 서버 스크립트](/en-us/develop/mobile/tutorials/store-scripts-in-source-control/) 자습서에 나온 단계를 따라 모바일 서비스에 대해 소스 제어를 사용하도록 설정하고 리포지토리를 복제하고 [NPM(Node Package Manager)](http://nodejs.org/)을 설치합니다.

2.  로컬 Git 리포지토리의 `.\service` 폴더로 이동한 후 명령 프롬프트에서 다음 명령을 실행합니다.

         npm install newrelic

    NPM은 `\newrelic` 하위 디렉터리에 [New Relic 모듈](https://npmjs.org/package/newrelic)을 설치합니다.
...
그런 다음, [관리 포털](https://manage.windowsazure.com/)에서 모바일 서비스의 New Relic 모니터링을 사용할 수 있도록 설정합니다.

모바일 서비스에 New Relic 개발자 분석 사용
------------------------------------------

1.  [관리 포털](https://manage.windowsazure.com/)에서 모바일 서비스를 선택한 다음 **구성** 탭을 클릭합니다.
...
2.  잠시 기다렸다가 데이터가 처리된 다음 New Relic 대시보드로 이동합니다.
    ...
    New Relic 구독을 추가 기능으로 구입했으면 [관리 포털](https://manage.windowsazure.com/)에서 그 구독을 선택하고 **관리**를 클릭합니다.
...
-   가격 책정 정보는 [Azure 스토어의 New Relic 페이지](/en-us/gallery/store/new-relic/new-relic/)(영문)를 참조하십시오.
-   New Relic을 사용하는 방법에 대한 자세한 내용은 New Relic 설명서의 [응용 프로그램 개요](https://docs.newrelic.com/docs/applications-dashboards/applications-overview)를 참조하십시오.
