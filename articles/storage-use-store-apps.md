...
- [Windows 런타임용 Microsoft Azure Storage Client Library](http://blogs.msdn.com/b/windowsazurestorage/archive/2012/11/05/windows-azure-storage-client-library-for-windows-runtime.aspx) 는 Azure 저장소 작업에 필요한 클래스 라이브러리를 제공합니다.
- [Windows 스토어 앱용 WCF 데이터 서비스 도구](http://www.microsoft.com/ko-kr/download/details.aspx?id=30714) 는 Visual Studio 2012 이상에서 Windows 스토어 앱용 클라이언트 쪽 지원으로 서비스 참조 추가까지 경험하게 해줍니다.
...
그런 다음 **참조**를 마우스 오른쪽 단추로 클릭하고 **참조 추가**를 선택한 후 다운로드한 Windows 런타임용 저장소 클라이언트 라이브러리를 찾아 Azure 저장소 클라이언트 라이브러리에 참조를 추가합니다.

![store-apps-storage-choose-library][store-apps-storage-choose-library]

<h3>Blob 및 큐 서비스로 라이브러리 사용</h3>
...
그런 다음, 페이지에 단추를 추가합니다. 다음 코드를 **Click** 이벤트에 추가하고 [비동기 키워드](http://msdn.microsoft.com/en-US/library/vstudio/hh156513.aspx)로 이벤트 처리기 메서드를 수정합니다.
...
이 코드에서는 두 개의 문자열 변수, 즉 저장소 계정 및 그 계정과 연결된 계정 키의 이름을 나타내는 *accountName* 및 *accountKey*가 있다고 가정합니다.

응용 프로그램을 빌드 및 실행합니다. 단추를 클릭하면 먼저 *container1*이라는 컨테이너가 계정에 있는지 확인하고 없으면 만듭니다.

<h3>테이블 서비스로 라이브러리 사용</h3>
