...
+ 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 <a href="http://www.windowsazure.com/pricing/free-trial/" target="_blank">Azure 무료 평가판</a>을 참조하세요.
...
<!--+  Azure 명령줄 도구를 다운로드하여 로컬 컴퓨터에 설치해야 합니다. 이렇게 하려면 이 항목의 첫 번째 섹션에 있는 지침을 따르세요. 

+ (선택 사항) 명령줄에서 직접 HTTP 요청을 실행할 수 있으려면 cURL 또는 이와 동등한 도구를 사용해야 합니다. cURL은 다양한 플랫폼에서 실행됩니다. <a href=http://go.microsoft.com/fwlink/p/?LinkId=275676 target="_blank">cURL 다운로드 페이지</a>에서 특정 플랫폼에 대한 cURL을 찾아 설치합니다.-->
...
설치를 테스트하려면 명령 프롬프트에 `azure`를 입력합니다. 성공적으로 설치되면 사용 가능한 모든 `azure` 명령 목록이 표시됩니다.
...
게시 설정을 가져온 후에는 보안을 위해 `.publishsettings` 파일을 삭제해야 합니다. 자세한 내용은 [Mac 및 Linux에서 Azure 명령줄 도구를 설치하는 방법]을 참조하세요. 이제 명령줄이나 배치 파일에서 Azure 모바일 서비스를 만들고 관리할 준비가 되었습니다.  
...
> [AZURE.NOTE] 또는 와 함께 <code>-q</code> 매개 변수를 <code>-a</code>지정하지 않을 경우 실행이 일시 중지되고<code>-d</code>SQL 데이터베이스에 대한 삭제 옵션을 선택하라는 메시지가 표시됩니다. 데이터베이스나 서버를 사용하는<code>-a</code> 다른 서비스가 없는 경우  매개 변수만 사용합니다. 그렇지 않으면 <code>-d</code> 매개 변수를 사용하여 삭제되는 모바일 서비스에 속하는 데이터만 삭제합니다.
...
`insert`, `read`, `update`및 `delete` 테이블 작업에 대해 기본 사용 권한 `application`이 있는 새 테이블을 만듭니다. 

다음 명령은 public `read` 권한은 있지만 관리자에게만 `delete` 권한이 부여된 새 테이블을 만듭니다.
...
스크립트 등록에 대한 자세한 내용은 [모바일 서비스 서버 스크립트 참조](영문)를 참조하세요.

<!--<h2><a name="test-service"></a>새 모바일 서비스 테스트</h2>

모바일 서비스의 생성을 자동화하는 경우 cURL 또는 다른 명령줄 요청 생성기를 선택적으로 사용할 수 있습니다. 
...
[를 참조하세요]: http://azure.microsoft.com/documentation/articles/virtual-machines-command-line-tools/#Commands_to_manage_mobile_services
