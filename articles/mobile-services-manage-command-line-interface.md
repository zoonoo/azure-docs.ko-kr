...
-   [새 모바일 서비스 만들기][새 모바일 서비스 만들기]
-   [새 테이블 만들기][새 테이블 만들기]
-   [테이블 작업에 스크립트 등록][테이블 작업에 스크립트 등록]
-   [테이블 나열][테이블 나열]
-   [기존 테이블 삭제][기존 테이블 삭제]
-   [모바일 서비스 나열][모바일 서비스 나열]
-   [기존 모바일 서비스 삭제][기존 모바일 서비스 삭제]
...
-   기존 계정이 있지만 Azure 모바일 서비스 미리 보기를 사용하도록 설정해야 하는 경우 [Azure 미리 보기 기능 사용][Azure 미리 보기 기능 사용]을 참조하십시오.

이 항목에서는 Azure 명령줄 도구가 지원하는 몇 개의 일반적인 관리 작업에 대해 다룹니다. 자세한 내용은 [Azure 명령줄 도구 설명서][Azure 명령줄 도구 설명서]를 참조하십시오.
...
## <a name="install"></a><span class="short-header">도구 설치</span>Azure 명령줄 도구 설치

다음 목록에는 운영 체제에 따른 명령줄 도구 설치 정보가 포함되어 있습니다.

-   **Windows**: [Azure 명령줄 도구 설치 관리자][Azure 명령줄 도구 설치 관리자]를 다운로드합니다. 다운로드한 .msi 파일을 열고 메시지가 표시되면 설치 단계를 완료합니다.
...
## <a name="import-account"></a><span class="short-header">설정 가져오기</span>게시 설정을 다운로드 및 가져오는 방법
...
다음 명령을 실행하여 `.publishsettings` 파일을 가져옵니다. `<path-to-settings-file>`을 해당 `.publishsettings` 파일의 경로로 바꿉니다.
...
게시 설정을 가져온 후 보안을 위해 `.publishsettings` 파일을 삭제해야 합니다. 자세한 내용은 [Mac 및 Linux에서 Azure 명령줄 도구를 설치하는 방법][Mac 및 Linux에서 Azure 명령줄 도구를 설치하는 방법]을 참조하십시오. 이제 명령줄이나 배치 파일에서 Azure 모바일 서비스를 만들고 관리할 준비가 되었습니다.

## <a name="create-service"></a><span class="short-header">서비스 만들기</span>모바일 서비스를 만드는 방법
...
## <a name="list-services"></a><span class="short-header">서비스 나열</span>구독의 기존 모바일 서비스를 나열하는 방법
...
## <a name="delete-service"></a><span class="short-header">서비스 삭제</span>기존 모바일 서비스를 삭제하는 방법
...
<div class="dev-callout"><strong>참고</strong> 
   <p><code data-inline="1">-a</code> 또는 <code data-inline="1">-d</code>와 함께 <code data-inline="1">-q</code> 매개 변수를 지정하지 않을 경우 실행이 일시 중지되고 SQL 데이터베이스에 대한 삭제 옵션을 선택하라는 메시지가 표시됩니다. 데이터베이스나 서버를 사용하는 다른 서비스가 없는 경우 <code data-inline="1">-a</code> 매개 변수만 사용합니다. 그렇지 않으면 <code data-inline="1">-d</code> 매개 변수를 사용하여 삭제되는 모바일 서비스에 속하는 데이터만 삭제합니다.</p>
</div>

## <a name="create-table"></a><span class="short-header">테이블 만들기</span>모바일 서비스에 테이블을 만드는 방법

다음 명령은 지정한 모바일 서비스에 테이블을 만듭니다. 여기서 `<service-name>`은 모바일 서비스 이름이고 `<table-name>`은 만들 테이블의 이름입니다.
...
다음 표는 스크립트 사용 권한 값과 [Azure 관리 포털][Azure 관리 포털]의 사용 권한 값을 비교해서 보여 줍니다.

| 스크립트 값           | 관리 포털 값                        |
|-----------------------|-------------------------------------|
| `public`              | 모든 사람                           |
| `application`(기본값) | 응용 프로그램 키가 있는 모든 사용자 |
| `user`                | 인증된 사용자만                     |
| `admin `              | 스크립트 및 관리자만                |

지정한 테이블이 이미 있을 경우 `mobile table create` 명령이 실패합니다. 자동화 스크립트에서 테이블을 삭제한 후 다시 만들어야 합니다.

## <a name="list-tables"></a><span class="short-header">테이블 나열</span>모바일 서비스의 기존 테이블을 나열하는 방법
...
## <a name="delete-table"></a><span class="short-header">테이블 삭제</span>모바일 서비스에서 기존 테이블을 삭제하는 방법

다음 명령은 모바일 서비스에서 테이블을 삭제합니다. 여기서 `<service-name>`은 모바일 서비스 이름이고 `<table-name>`은 삭제할 테이블의 이름입니다.

        azure mobile table delete <service-name> <table-name> -q

자동화 스크립트에 `-q` 매개 변수를 사용하여 실행을 차단하는 확인 프롬프트를 표시하지 않고 테이블을 삭제합니다.

## <a name="register-script"></a><span class="short-header">스크립트 등록</span>테이블 작업에 스크립트를 등록하는 방법
...
<!--<h2><a name="test-service"></a><span class="short-header">Test the service</span>Test the new mobile service</h2>  When you are automating the creation of your mobile service, you can optionally use cURL or another command-line request generator to   ## <a name="nextsteps"> </a>Next Steps Next steps here.... -->   
  ...
  [테이블 작업에 스크립트 등록]: #register-script
  [테이블 나열]: #list-tables
  ...
  [Azure 미리 보기 기능 사용]: http://azure.microsoft.com/ko-kr/documentation/articles/php-create-account/#enable
  [Azure 명령줄 도구 설명서]: /ko-kr/manage/linux/other-resources/command-line-tools/#Commands_to_manage_mobile_services
  [Azure 명령줄 도구 설치 관리자]: http://go.microsoft.com/fwlink/p?LinkID=275464
  [Azure SDK 설치 관리자]: http://go.microsoft.com/fwlink/p?LinkId=252249
