...
> [AZURE.NOTE] <code>-a</code> 또는 <code>-d</code>와 함께 <code>-q</code> 매개 변수를 지정하지 않을 경우 실행이 일시 중지되고 SQL 데이터베이스에 대한 삭제 옵션을 선택하라는 메시지가 표시됩니다. 데이터베이스나 서버를 사용하는 다른 서비스가 없는 경우에만 <code>-a</code> 매개 변수를 사용합니다. 그렇지 않으면 <code>-d</code> 매개 변수를 사용하여 삭제할 모바일 서비스에 속하는 데이터만 삭제합니다.
