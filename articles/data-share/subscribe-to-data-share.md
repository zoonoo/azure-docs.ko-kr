...
> [!IMPORTANT]
> Azure Data Share를 수락하고 받으려면 먼저 Microsoft.DataShare 리소스 공급자를 등록해야 하며 데이터를 수락하는 스토리지 계정의 소유자여야 합니다. [Azure Data Share의 문제 해결](data-share-troubleshoot.md)에서 설명하는 지침에 따라 데이터 공유 리소스 공급자를 등록하고 자신을 스토리지 계정의 소유자로 추가합니다. 

### <a name="receive-data-into-a-sql-based-source"></a>SQL 기반 원본으로 데이터를 받습니다.

* 데이터 공유 MSI에서 Azure SQL Database 또는 Azure SQL Data Warehouse에 액세스하기 위한 권한. 이 작업은 다음 단계를 통해 수행할 수 있습니다. 
...
이러한 필수 선행 작업이 완료되면 데이터를 SQL Server로 받을 준비가 된 것입니다.
...
> [!IMPORTANT]
> SQL 기반 데이터를 받고 해당 데이터를 SQL 기반 원본으로 받으려는 경우 [데이터 세트 매핑 구성](how-to-configure-mapping.md) 방법 가이드를 참조하여 데이터 세트의 대상으로 SQL Server를 구성하는 방법을 알아봅니다. 
