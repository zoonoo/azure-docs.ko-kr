<properties pageTitle="Microsoft 인증 등록 - 모바일 서비스" metaKeywords="Azure registering application, Azure Microsoft authentication, application authenticate, authenticate mobile services" description="Azure 모바일 서비스 응용 프로그램에서 Microsoft 인증을 위해 등록하는 방법에 대해 알아봅니다." metaCanonical="" disqusComments="0" umbracoNaviHide="1" title="Register your apps to use a Microsoft Account login" authors="glenga" services="mobile-services" documentationCenter="Mobile" manager="dwrede" />
...
>[WACOM.NOTE]공용 Windows 앱에 대한 Microsoft 계정 인증을 구성하거나 Windows 스토어 앱에 대한 Single Sign-On 환경을 제공하려면 [Microsoft 인증을 위해 Windows 스토어 앱 패키지 등록]을 참조하세요.(/ko-kr/documentation/articles/mobile-services-how-to-register-store-app-package-microsoft-authentication)
...
3. **API 설정**을 클릭하고 **리디렉션 URL**에 `https://<mobile_service>.azure-mobile.net/login/microsoftaccount` 값을 제공하고 **저장**을 클릭합니다.
	...
	>[WACOM.NOTE]Visual Studio를 사용하여 Azure에 게시된 .NET 백 엔드 모바일 서비스의 경우 리디렉션 URL은 모바일 서비스 URL에 모바일 서비스를 .NET 서비스로 사용한 _signin-microsoft_ 경로를 추가한 것입니다(예: <code>https://todolist.azure-mobile.net/signin-microsoft</code>).  
