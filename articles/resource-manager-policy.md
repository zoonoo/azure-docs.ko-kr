...
> [!NOTE]
> 현재 정책은 태그, 종류 및 위치를 지원하지 않는 리소스 종류(예: Microsoft.Resources/deployments)를 평가하지 않습니다. 이 지원은 나중에 추가됩니다. 이전 버전과의 호환성 문제를 방지하기 위해 정책을 작성할 때 유형을 명시적으로 지정해야 합니다. 예를 들어 유형을 지정하지 않은 태그 정책이 모든 유형에 적용됩니다. 이 경우 태그를 지원하지 않는 중첩된 리소스가 있고 배포 리소스 종류가 정책 평가에 추가된 경우 나중에 템플릿 배포에 실패할 수 있습니다.
> 
> 
