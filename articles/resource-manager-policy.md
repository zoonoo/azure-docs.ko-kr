...
일반적인 시나리오 중 하나는 비용 청구 용도로 부서별 태그를 요구하는 것입니다. 조직은 적절한 비용 센터가 관련된 경우에만 작업을 허용하는 것이 좋습니다. 다른 경우에는 요청을 거부할 수 있습니다. 이는 수행된 작업에 적절한 비용 센터를 청구하는 데 도움이 될 수 있습니다.
...
**효과:** 조건이 만족될 때의 효과 - 거부 또는 감사를 설명합니다. 감사 효과는 경고 이벤트 서비스 로그를 내보냅니다. 예를 들어 관리자는 누구든지 큰 가상 컴퓨터를 만들면 감사를 지시하는 정책을 만들 수 있습니다. 그런 다음 나중에 이 로그를 검토할 수 있습니다.
...
이 섹션에서는 REST API 및 PowerShell을 사용하여 정책을 만들 수 있는 방법에 대한 세부 정보를 제공합니다.

### REST API를 사용하여 정책 정의 만들기

정책에 대한 REST API를 사용하여 정책을 만들 수 있습니다. REST API를 사용하여 정책을 만들고, 삭제하고, 기존 정책에 관한 정보를 가져올 수 있습니다.

새 정책을 만들려면 다음을 실행합니다.

    PUT https://management.azure.com/subscriptions/{subscription-id}/providers/Microsoft.authorization/policydefinitions/{policyDefinitionName}?api-version={api-version}

다음은 요청 본문이 어떤 모습인지 보여주는 예제입니다.
...
정책 정의를 위에 나오는 예제 중 하나로 정의할 수 있습니다. api-version에는 *2015-10-01-preview*를 사용합니다. 예제 및 더 자세한 세부 정보는 정책에 대한 REST API를 참조하세요.

## 정책 적용

### REST API를 사용하여 정책 할당

REST API를 통해 정책 할당에 대해 원하는 범위에서 정책 정의를 적용할 수 있습니다. REST API를 사용하여 정책 할당을 만들고, 삭제하고, 기존 할당에 관한 정보를 가져올 수 있습니다.

새 정책 할당을 만들려면 다음을 실행합니다.

    PUT https://management.azure.com /subscriptions/{subscription-id}/providers/Microsoft.authorization/policyassignments/{policyAssignmentName}?api-version={api-version}

{policy-assignment}는 정책 할당의 이름입니다. api-version에는 *2015-10-01-preview*를 사용합니다. 예제 및 더 자세한 세부 정보는 정책 할당에 대한 REST API를 참조하세요.
