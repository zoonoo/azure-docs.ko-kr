---
title: 맞춤 정책을 사용하여 Google 계정으로 로그인 설정
...
이 문서에서는 Azure Active Directory B2C(Azure AD B2C)에서 [사용자 지정 정책을](custom-policy-overview.md) 사용하여 Google 계정이 있는 사용자에 대해 로그인을 활성화하는 방법을 보여 주며 있습니다.
...
8. **만들기**를 클릭합니다.
9. **클라이언트 ID** 및 클라이언트 보안 의 값을 **복사합니다.** 테넌트에서 Google을 ID 공급자로 구성하려면 둘 모두가 필요합니다. 클라이언트 암호는 중요한 보안 자격 증명입니다.
...
2. 다음 **ClaimsExchange** 요소를 추가하여 **TargetClaimsExchangeId에**사용한 ID에 대해 동일한 값을 사용해야 합니다.

    ```XML
    <ClaimsExchange Id="GoogleExchange" TechnicalProfileReferenceId="Google-OAuth" />
    ```

    앞에서 만든 기술 프로필의 ID로 **TechnicalProfileReferenceId** 의 값을 업데이트합니다. `Google-OAuth`)을 입력합니다.
