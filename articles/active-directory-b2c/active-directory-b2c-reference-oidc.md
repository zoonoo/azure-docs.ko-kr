...
| id_token_hint| 아니요 | 이전에 발급 된 ID 토큰으로, 로그 아웃 끝점에 클라이언트에 대 한 최종 사용자의 현재 인증 된 세션에 대 한 힌트로 전달 됩니다. 는 Azure AD B2C 응용 프로그램 `post_logout_redirect_uri` 설정에서가 등록 된 회신 URL 인지 확인합니다.`id_token_hint` |
