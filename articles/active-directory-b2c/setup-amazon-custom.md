---
title: Azure Active Directory B2C에서 사용자 지정 정책을 사용하여 Amazon 계정으로 로그인하도록 설정 | Microsoft Docs
...
이 문서에서는 Azure AD(Azure Active Directory) B2C의 [사용자 지정 정책](active-directory-b2c-overview-custom.md)을 사용하여 사용자가 Amazon 계정에서 로그인할 수 있도록 설정하는 방법을 설명합니다.

## <a name="prerequisites"></a>필수 조건

- [사용자 지정 정책 시작](active-directory-b2c-get-started-custom.md)의 단계를 완료합니다.
- Amazon 계정이 아직 없으면 [http://www.amazon.com/](https://www.amazon.com/)에서 만듭니다.

## <a name="register-the-application"></a>응용 프로그램 등록

사용자가 Amazon 계정에서 로그인하도록 설정하려면 Amazon 응용 프로그램을 만들어야 합니다.
...
7. 정책 키의 **이름**을 입력합니다. 예: `AmazonSecret`. `B2C_1A_` 접두사가 키의 이름에 자동으로 추가됩니다.
...
2. **ClaimsProviderSelects** 아래에 다음 요소를 추가 합니다. **TargetClaimsExchangeId** 값은 `AmazonExchange` 등의 적절한 값으로 설정합니다.
...
2. 다음 **ClaimsExchange** 요소를 추가합니다. ID에는 **TargetClaimsExchangeId**에 사용한 것과 같은 값을 사용해야 합니다.
    ...
    **TechnicalProfileReferenceId**의 값을 앞에서 만든 기술 프로필의 ID로 업데이트합니다. 예: `Amazon-OAuth`.
...
5. 응용 프로그램의 이름(예: *testapp1*)을 입력합니다.
...
2. 새 파일을 열고, **TrustFrameworkPolicy**의 **PolicyId** 특성 값을 고유 값으로 업데이트합니다. 예: `SignUpSignInAmazon`.
3. **PublicPolicyUri**의 값을 정책의 URI 바꿉니다. 예: `http://contoso.com/B2C_1A_signup_signin_amazon`
