...
- **네이티브 클라이언트**: 코드가 사용자 쪽 장치에서 실행될 때 인증 중에 사용자 상호 작용이 발생합니다. 이 디바이스는 Android와 같은 네이티브 운영 체제에서 실행되거나 JavaScript와 같은 브라우저에서 실행되는 모바일 응용 프로그램일 수 있습니다.
...
4. 정책 이름(예: *ROPC_Auth*)을 입력한 다음, **응용 프로그램 클레임**을 선택합니다.
...
다음 표의 정보를 사용하여 아래에 표시된 것과 POST 호출을 요청의 본문으로 구성합니다.
