...
- **네이티브 클라이언트**: 코드가 사용자 쪽 장치에서 실행될 때 인증 중에 사용자 상호 작용이 발생합니다. 이 장치는 Android와 같은 네이티브 운영 체제에서 실행되거나 JavaScript와 같은 브라우저에서 실행되는 모바일 응용 프로그램일 수 있습니다.
