...
1. 정책의 기본 파일을 엽니다(예: 예를 들어 <em>`SocialAndLocalAccounts/` **`TrustFrameworkBase.xml`** </em> 합니다. 이 기본 파일은 사용자 지정 정책 시작 팩에 포함 된 정책 파일 중 하나로, 필수 구성 요소, [사용자 지정 정책 시작](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-get-started-custom)에서 가져와야 합니다.
...
Azure AD B2C 사용자 지정 정책을 사용 하 여 URL 경로 또는 쿼리 문자열에 매개 변수를 보낼 수 있습니다. 매개 변수를 HTML 엔드포인트로 전달하면 페이지 콘텐츠를 동적으로 변경할 수 있습니다. 예를 들어 웹 또는 모바일 애플리케이션에서 전달한 매개 변수를 기반으로 Azure AD B2C 등록 또는 로그인 페이지에서 배경 이미지를 변경할 수 있습니다. 매개 변수는 응용 프로그램 ID, 언어 ID 또는 사용자 지정 쿼리 문자열 매개 변수 (예: `campaignId`)와 같은 모든 [클레임 해결](claim-resolver-overview.md)프로그램 일 수 있습니다.

### <a name="sending-query-string-parameters"></a>쿼리 문자열 매개 변수 보내기

쿼리 문자열 매개 변수를 전송 하려면 [신뢰 당사자 정책](relyingparty.md)에서 아래와 같이 `ContentDefinitionParameters` 요소를 추가 합니다.
...
콘텐츠 정의에서 `LoadUri` 값을 `https://<app_name>.azurewebsites.net/home/unified`로 변경 합니다. 사용자 지정 정책 `ContentDefinition`은 다음 코드 조각과 같습니다.
...
Azure AD B2C는 프랑스어 `fr` 언어에 대 한 두 문자 ISO 코드를 보냅니다.
