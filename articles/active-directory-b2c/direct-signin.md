...
Azure AD(Active Directory) B2C를 사용하여 응용 프로그램에 대한 로그인을 설정하는 경우 로그인 이름을 미리 채우거나 Facebook, LinkedIn, Microsoft 계정 같은 특정 소셜 ID 공급자로 직접 로그인할 수 있습니다. 

## <a name="prepopulate-the-sign-in-name"></a>로그인 이름 미리 채우기

로그인 사용자 과정 동안, 신뢰 당사자 응용 프로그램은 특정 사용자 또는 도메인 이름을 대상으로 지정할 수 있습니다. 사용자를 대상으로 지정할 경우 응용 프로그램은 권한 부여 요청에 `login_hint` 쿼리 매개 변수와 사용자 로그인 이름을 지정할 수 있습니다. 사용자는 암호를 제공하기만 하면 되지만, Azure AD B2C는 로그인 이름을 자동으로 채웁니다.
...
Facebook, LinkedIn 또는 Google 등과 같은 소셜 계정을 포함하도록 응용 프로그램에 대한 로그인 과정을 구성한 경우 `domain_hint` 매개 변수를 지정할 수 있습니다. 이 쿼리 매개 변수는 로그인에 사용해야 하는 소셜 ID 공급자에 대한 힌트를 Azure AD B2C에 제공합니다. 예를 들어, 응용 프로그램이 `domain_hint=facebook.com`을 지정하는 경우 로그인을 수행하면 Facebook 로그인 페이지로 직접 이동됩니다.
