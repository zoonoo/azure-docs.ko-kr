...
1. Azure AD B2C 테넌트 내의 왼쪽 탐색 메뉴에서 **추가 서비스**를 클릭합니다.
1. **앱 등록**을 검색하여 엽니다.
...
1. 삭제된 앱을 복원하려는 Azure AD B2C 디렉터리에 대한 전역 관리자로 사이트에 로그인합니다.
1. api-version=1.6인 URL `https://graph.windows.net/{tenantName}.onmicrosoft.com/deletedApplications`에 대해 HTTP GET을 실행합니다. `{tenantName}`을 테넌트 이름으로 바꿔야 합니다. 이 작업은 지난 30일 내에서 삭제된 모든 응용 프로그램을 나열합니다.
1. 목록에서 이름이 ‘b2c-extension-app’으로 시작하는 응용 프로그램을 찾고 해당 `objectid` 속성 값을 복사합니다.
...
> [!NOTE]
> 응용 프로그램이 지난 30일 이내 삭제된 경우에만 복원 가능합니다. 30일이 지난 경우 데이터는 영구적으로 손실됩니다. 추가 지원이 필요한 경우 지원 티켓을 파일로 저장합니다.
