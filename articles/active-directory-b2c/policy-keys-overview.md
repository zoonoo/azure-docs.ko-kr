...
> [!NOTE]
> 현재 정책 키의 구성은 [사용자 지정 정책](active-directory-b2c-get-started-custom.md) 으로만 제한 됩니다.
...
- Openid connect Connect의 잘 알려진 구성 끝점의 키 끝점 (JWKS URI)은 키가 [JwtIssuer 기술 프로필](https://docs.microsoft.com/azure/active-directory-b2c/jwt-issuer-technical-profile)에서 참조 될 때 키 컨테이너에 구성 된 키를 반영 합니다. OIDC 라이브러리를 사용 하는 응용 프로그램은 올바른 키를 사용 하 여 토큰의 유효성을 검사할 수 있도록이 메타 데이터를 자동으로 인출 합니다. 자세한 내용은 항상 최신 토큰 서명 키를 자동으로 가져오는 [Microsoft 인증 라이브러리](https://docs.microsoft.com/azure/active-directory/develop/msal-b2c-overview)를 사용 하는 방법을 알아보세요.

## <a name="policy-key-management"></a>정책 키 관리

키 컨테이너 내에서 현재 활성 키를 가져오려면 Microsoft Graph API [Getactivekey](https://docs.microsoft.com/graph/api/trustframeworkkeyset-getactivekey) 끝점을 사용 합니다.
