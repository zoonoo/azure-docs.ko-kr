<properties
	pageTitle="Azure Active Directory B2C: 토큰, 세션 및 Single Sign-On 구성 | Microsoft Azure"
...
# Azure Active Directory B2C: 토큰, 세션 및 Single Sign-On 구성
...
- **웹앱 세션 수명(분)**: 인증 성공 시 사용자의 브라우저에 저장된 Azure AD B2C 세션 쿠키의 수명입니다.
   ...
   - 최소(포함) = 15분.
   - 최대(포함) = 1440분.
- **웹앱 세션 제한 시간**: 이 스위치가 **절대**로 설정된 경우 사용자는 **웹 앱 세션 수명(분)**에서 지정한 기간이 경과된 후 강제로 다시 인증합니다. 이 스위치가 **롤링**(기본 설정)으로 설정된 경우 사용자는 웹 응용 프로그램에서 지속적으로 활성 상태인 한 로그인 상태로 유지됩니다.
