...
> [AZURE.NOTE]이 빠른 시작이 완벽하게 작동하기 위해 B2C를 사용하여 Azure AD에서 Web API를 보호하는 필수 구성 요소를 포함합니다. .Net 및 node.js 모두에 사용할 수 있도록 만들었습니다. 이 연습에서는 node.js Web API 샘플 구성되었다고 가정합니다. [Node.js 샘플에 대한 Azure Active Directory Web API](active-directory-b2c-devquickstarts-api-node.md`)를 참조하세요.
...
Azure AD B2C에서 모든 사용자 환경은 [**정책**](active-directory-b2c-reference-policies.md)에 의해 정의됩니다. 이 앱은 등록, 로그인 및 Facebook으로 로그인 등 세 가지 ID 환경을 포함합니다. [정책 참조 문서](active-directory-b2c-reference-policies.md#how-to-create-a-sign-up-policy)에서 설명한 대로 각 형식의 정책을 하나씩 만들어야 합니다. 세 가지 정책을 만들 때 다음을 확인합니다.
...
API의 간단한 CRUD 작업뿐만 아니라 원하는 정책을 사용하여 토큰을 가져올 수 있는 `doPolicy` 메서드가 있습니다.
...
3. 정책을 사용하고 이를 URL로 형식을 지정하여 서버로 보내는 메서드 `convertPolicyToDictionary:policy`가 있습니다. 다음으로 이 도우미 메서드를 작성합니다.
4. 마지막으로 큰 `getClaimsWithPolicyClearingCache` 메서드가 있습니다. 작성해야 하는 iOS 용 ADAL에 대한 실제 호출입니다. 나중에 수행합니다.


다음으로 방금 작성한 코드 아래에 `convertPolicyToDictionary` 메서드를 작성합니다.
...
```
이 간단한 코드는 p를 정책에 추가하여 쿼리가 ?p=<policy>처럼 보여야 합니다.

이제 큰 메서드 `getClaimsWithPolicyClearingCache`를 작성합니다. 고유한 섹션을 사용할 만큼 충분히 큽니다.
...
`getTasksList`는 서버에서 작업을 나타내는 배열을 제공합니다. `addTask` 및 `deleteTask`은 후속 동작을 수행하고 성공하면 TRUE 또는 FALSE를 반환합니다.
...
작업 코드를 설명하는 것은 이 연습의 범위 밖이지만 작업 URL을 사용하는 `craftRequest` 메서드 등 사용자가 유의해야 할 점이 있습니다. 이 메서드는 서버에 대해 수신한 액세스 토큰으로 요청을 만들기 위해 사용합니다. 지금 작성해 보겠습니다.
...
동일한 패턴을 따르지만 구현해야 하는 다른(그리고 최종!) 메서드를 도입합니다. `convertTaskToDictionary`은 배열을 사용하고 서버에 전달해야 하는 쿼리 매개 변수를 보다 쉽게 변경하는 사전 개체를 만듭니다. 이 코드는 매우 간단합니다.
