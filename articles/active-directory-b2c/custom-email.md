...
1. `Verification email`와 같은 고유한 템플릿 이름을 입력 하 고 **저장**을 선택 합니다.
...
1. 왼쪽에서 **설정** 을 확장 하 고 **전자 메일 제목**에 `{{subject}}`을 입력 합니다.
1. **템플릿 저장**을 선택 합니다.
...
정책에서 `<BuildingBlocks>`내의 `<ClaimsSchema>` 요소에 다음 클레임 유형을 추가 합니다.
...
`<BuildingBlocks>`내의 클레임 변환 아래에 다음 [Contentdefinition](contentdefinitions.md) 을 추가 하 여 버전 2.0.0 데이터 URI를 참조 합니다.
...
콘텐츠 정의에서 계속 `<BuildingBlocks>`내에 [VerificationControl](display-control-verification.md) 형식의 다음 [DisplayControl](display-controls.md) 를 정책에 추가 합니다.
