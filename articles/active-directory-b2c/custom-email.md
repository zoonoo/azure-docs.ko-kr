...
Azure Active Directory B2C (Azure AD B2C)의 사용자 지정 전자 메일을 사용 하 여 응용 프로그램을 사용 하기 위해 등록 하는 사용자에 게 사용자 지정 전자 메일을 보냅니다. [Displaycontrols](display-controls.md) (현재 미리 보기 상태) 및 타사 전자 메일 공급자를 사용 하 여 사용자 고유의 전자 메일 템플릿과 *의* 주소 및 주체를 사용할 수 있을 뿐만 아니라 지역화 및 사용자 지정 OTP (일회용 암호) 설정을 지원할 수 있습니다.

사용자 지정 전자 메일을 확인 하려면 [SendGrid](https://sendgrid.com) 또는 [SparkPost](https://sparkpost.com)와 같은 타사 전자 메일 공급자, 사용자 지정 REST API 또는 HTTP 기반 메일 공급자 (자체 포함)를 사용 해야 합니다. 이 문서에서는 SendGrid를 사용 하는 솔루션을 설정 하는 방법을 설명 합니다.
...
1. 과 같은 `Verification email` 고유한 템플릿 이름을 입력 한 후 **저장**을 선택 합니다.
1. 새 템플릿 편집을 시작 하려면 **버전 추가**를 선택 합니다.
1. **코드 편집기** 를 선택한 다음 **계속**을 선택 합니다.
1. HTML 편집기에서 다음 HTML 템플릿을 붙여넣거나 사용자 고유의를 사용 합니다. 및 `{{otp}}` `{{email}}` 매개 변수는 일회성 암호 값 및 사용자 전자 메일 주소를 사용 하 여 동적으로 교체 됩니다.
...
1. 왼쪽에서 **설정** 을 확장 하 고 **전자 메일 제목**에를 `{{subject}}`입력 합니다.
1. **템플릿 저장**을 선택 합니다.
1. 뒤로 화살표를 선택 하 여 **트랜잭션 템플릿** 페이지로 돌아갑니다.
1. 이후 단계에서 사용 하기 위해 만든 템플릿의 **ID** 를 기록 합니다. `d-989077fbba9746e89f3f6411f596fb96`)을 입력합니다. [클레임 변환을 추가할](#add-the-claims-transformation)때이 ID를 지정 합니다.

## <a name="add-azure-ad-b2c-claim-types"></a>Azure AD B2C 클레임 유형 추가

정책에서 내의 `<ClaimsSchema>` `<BuildingBlocks>`요소에 다음 클레임 유형을 추가 합니다.
...
내의 `<ClaimsTransformations>` `<BuildingBlocks>`요소에 다음 클레임 변환을 추가 합니다. 클레임 변환 XML에 대 한 다음 업데이트를 수행 합니다.

* InputParameter 값 `template_id` 을 [Create SendGrid template](#create-sendgrid-template)의 이전에 만든 SendGrid 트랜잭션 템플릿의 ID로 업데이트 합니다.
* 주소 값 `from.email` 을 업데이트 합니다. 유효한 전자 메일 주소를 사용 하 여 확인 전자 메일이 스팸으로 표시 되는 것을 방지할 수 있습니다.
...
내의 `<BuildingBlocks>`클레임 변환 아래에서 다음 [contentdefinition](contentdefinitions.md) 을 추가 하 여 버전 2.0.0 데이터 URI를 참조 합니다.
...
1. 사용자의 `email` 주소 클레임 유형을 수집 합니다.
1. 사용자가 사용자에 게 보낸 코드 `verificationCode` 를 사용 하 여 클레임 형식을 제공할 때까지 기다립니다.
...
콘텐츠 정의에서 여전히 내 `<BuildingBlocks>`에 [VerificationControl](display-control-verification.md) 형식의 다음 [DisplayControl](display-controls.md) 를 정책에 추가 합니다.
...
기술 `GenerateOtp` 프로필은 전자 메일 주소에 대 한 코드를 생성 합니다. 기술 `VerifyOtp` 프로필은 전자 메일 주소와 연결 된 코드를 확인 합니다. 형식의 구성과 일회용 암호의 만료를 변경할 수 있습니다. OTP 기술 프로필에 대 한 자세한 내용은 [일회용 암호 기술 프로필 정의](one-time-password-technical-profile.md)를 참조 하세요.
...
마지막 단계에서 사용자가 만든 DisplayControl에 대 한 참조를 추가 합니다. 이전 버전의 `LocalAccountSignUpWithLogonEmail` Azure AD B2C 정책을 사용한 경우 기존 자체 어설션된 기술 프로필을 다음으로 바꿉니다. 이 기술 프로필은 `DisplayClaims` DisplayControl에 대 한 참조와 함께를 사용 합니다.
...
전자 메일을 지역화 하려면 지역화 된 문자열을 SendGrid 또는 전자 메일 공급자에 게 보내야 합니다. 예를 들어 전자 메일의 제목, 본문, 코드 메시지 또는 서명을 지역화 합니다. 이렇게 하려면 [GetLocalizedStringsTransformation](string-transformations.md) 클레임 변환을 사용 하 여 지역화 된 문자열을 클레임 형식으로 복사할 수 있습니다. JSON 페이로드 `GenerateSendGridRequestBody` 를 생성 하는 클레임 변환에서는 지역화 된 문자열을 포함 하는 입력 클레임을 사용 합니다.
...
1. 다음 XML `GenerateSendGridRequestBody` 코드 조각과 함께 입력 클레임을 사용 하도록 클레임 변환을 변경 합니다.
