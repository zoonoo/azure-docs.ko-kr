---
title: 사용자 지정 정책을 사용 하 여 SAML id 공급자로 ADFS 추가
...
- 프라이빗 키가 포함된 인증서 .pfx 파일에 액세스할 수 있는지 확인합니다. 자체 서명된 인증서를 생성하고 Azure AD B2C에 업로드할 수 있습니다. Azure AD B2C는 이 인증서를 사용하여 SAML ID 공급자에 보낸 SAML 요청에 서명합니다. 인증서를 생성 하는 방법에 대 한 자세한 내용은 [서명 인증서 생성](identity-provider-salesforce-custom.md#generate-a-signing-certificate)을 참조 하세요.
...
> [!NOTE]
> Visual Studio code B2C 확장은 "socialIdpUserId"를 사용 합니다. ADFS에도 소셜 정책이 필요 합니다.
>
...
2. 다음 **ClaimsExchange** 요소를 추가합니다. ID에는 **TargetClaimsExchangeId** 에 사용한 것과 같은 값을 사용해야 합니다.
    ...
    **TechnicalProfileReferenceId** 값을 앞에서 만든 기술 프로필의 ID로 업데이트합니다. `Contoso-SAML2`)을 입력합니다.
...
브라우저를 열고 URL로 이동합니다. 올바른 URL을 입력했는지와 XML 메타데이터 파일에 대한 액세스 권한이 있는지 확인합니다. ADFS 관리 스냅인을 사용하여 새 신뢰 당사자 트러스트를 추가하고 설정을 수동으로 구성하려면 페더레이션 서버에서 다음 절차를 수행합니다. 이 절차를 완료 하려면 최소한 **Administrators** 그룹의 구성원 이거나 로컬 컴퓨터에 해당 하는 권한이 있어야 합니다.
