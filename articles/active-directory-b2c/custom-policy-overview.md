...
Azure AD B2C의 사용자 흐름은 위에서 설명한 파일 패턴을 따릅니다. 하지만 Azure Portal는 파일에 대 한 백그라운드 변경을 수행 하는 동안에는 RP 파일만 볼 수 있습니다.

3 가지 유형의 정책 파일이 있지만 세 개의 파일로 제한 되지 않습니다. 각 파일 형식의 파일은 여러 개 있을 수 있습니다. 예를 들어 확장 파일을 변경 하지 않으려는 경우 Extensions2 파일을 만들어 확장 파일을 추가로 확장할 수 있습니다.
...
Id 경험 프레임 워크는 Openid connect Connect, OAuth, SAML 및 REST API 기반 시스템 간 클레임 교환의 경우와 같이 표준 프로토콜 형식에서 엔터티 사이에 신뢰를 오케스트레이션 하는 완전히 구성 가능한 정책 중심의 클라우드 기반 Azure 플랫폼입니다. 이 프레임워크는 HTML 및 CSS를 지원하는 사용자에게 친숙한 화이트 레이블 환경을 만듭니다.
