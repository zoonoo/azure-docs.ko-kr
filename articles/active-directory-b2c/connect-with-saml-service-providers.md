...
1. 인증서 > **작업** > **모든 작업** > **내보내기** 를 선택 합니다.
1.  > **Next** > **Next** **Yes****Yes, export the private key**다음 예를 선택 하 고 개인 키를 내보냅니다 .를 선택 합니다. > 
...
사용자 `SocialAndLocalAccounts\` **`TrustFrameworkExtensions.xml`** 지정 정책 시작 팩에서를 엽니다.
...
1. 아래 표시 `PolicyId` 된 `PublicPolicyUri` `http://tenant-name.onmicrosoft.com/B2C_1A_signup_signin_saml` 대로 정책의 및를 _B2C_1A_signup_signin_saml_ 로 변경 합니다.
...
1. 요소 바로 앞에 `<RelyingParty>` 다음 XML 코드 조각을 추가 합니다. 이 XML은 _Signu폰_ 사용자 경험의 오케스트레이션 단계 번호 7을 덮어씁니다. 시작 팩 `order` 의 다른 폴더에서 시작한 경우 또는 오케스트레이션 단계를 추가 또는 제거 하 여 사용자 경험을 사용자 지정 하는 경우 요소에 있는 숫자 (예: 다른 스타터 pack 폴더의 경우 4의 단계 번호 `LocalAccounts`,에 대 한 6 for `SocialAccounts` 및 9 `SocialAndLocalAccountsWithMfa`)가 지정 된 숫자와 일치 하는지 확인 합니다.
...
1. Azure AD B2C `tenant-name` 테 넌 트의 이름으로 업데이트 합니다.
...
Azure AD B2C policy IDP 메타 데이터는 saml 프로토콜에서 SAML id 공급자의 구성을 노출 하는 데 사용 되는 정보입니다. 메타데이터는 로그인/로그아웃, 인증서, 로그인 방법 등과 같은 서비스의 위치를 정의합니다. Azure AD B2C 정책 메타 데이터는 다음 URL에서 사용할 수 있습니다. 을 `tenant-name` Azure AD B2C 테 넌 트의 이름으로 바꾸고, `policy-name` 을 정책의 이름 (ID)으로 바꿉니다.
...
1. **URI 리디렉션**에서 **웹**을 선택 하 고을 입력 `https://localhost`합니다. 이 값은 나중에 응용 프로그램 등록의 매니페스트에서 수정할 수 있습니다.
1. **등록**을 선택합니다.
...
는 `identifierUris` Azure AD B2C 테 넌 트 내에서 웹 앱을 고유 하 게 식별 하는 사용자 정의 URI를 포함 하는 문자열 컬렉션입니다. 서비스 공급자는 SAML 요청 `Issuer` 요소에서이 값을 설정 해야 합니다.
...
SAML 테스트 응용 프로그램을 사용 하는이 자습서의 경우 다음 값을 사용 `samlMetadataUrl`합니다.
...
메타 데이터 URI를 제공 하지 않는 경우 회신 URL을 명시적으로 지정할 수 있습니다. 이 선택적 속성 `AssertionConsumerServiceUrl` 은 (`SingleSignOnService` 서비스 공급자 메타 데이터의 URL)을 나타내며는 `BindingType` 로 가정 됩니다 `HTTP POST`.
...
SAML 테스트 응용 프로그램을 사용 하는이 자습서의 경우의 `url` `replyUrlsWithType` 속성을 다음 JSON 코드 조각에 표시 된 값으로 설정 합니다.
...
이 선택적 `Logout` 속성은 url (`SingleLogoutService` 신뢰 당사자 메타 데이터의 url)을 나타내며이의 `BindingType` 는로 가정 됩니다 `Http-Redirect`.

SAML 테스트 응용 프로그램을 사용 하는이 자습서의 경우 `logoutUrl` 를로 `https://samltestapp2.azurewebsites.net/logout`설정 된 상태로 둡니다.
...
* **발급자**: 메타 데이터 파일에서 entityID를 사용 합니다.
...
* **Certificate**: 개인 키가 없는 *B2C_1A_SamlIdpCert*입니다. 인증서의 공개 키를 가져오려면:
    ...
    1. `<X509Certificate>` 요소의 값을 복사 합니다.
    1. 텍스트 파일에 붙여 넣습니다.
...
* 이 발급자 URI를 지정 합니다.`https://contoso.onmicrosoft.com/app-name`

**로그인** 을 선택 하면 사용자 로그인 화면이 표시 됩니다. 로그인 할 때 SAML 어설션이 샘플 응용 프로그램으로 다시 발급 됩니다.
...
1. 테 `TenantId` 넌 트 이름 (예: *contoso.b2clogin.com* )과 일치 하도록 업데이트 합니다.
