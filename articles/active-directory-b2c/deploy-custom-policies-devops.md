...
1. PowerShell 스크립트 태스크에 대해 다음 값을 입력 합니다.
    ...
    * **인수:**
        ...
        **인수**에 대해 다음 값을 입력 합니다. 을 `{alias-name}` 이전 섹션에서 지정한 별칭으로 바꿉니다.
...
작업이 성공적으로 완료 되 면 각 사용자 지정 정책 파일에 대해 앞의 단계를 수행 하 여 배포 작업을 추가 합니다. 각 정책 `-PolicyId` 에 `-PathToFile` 대 한 및 인수 값을 수정 합니다.

는 `PolicyId` TrustFrameworkPolicy 노드 내에서 XML 정책 파일의 시작 부분에 있는 값입니다. 예를 들어 다음 `PolicyId` 정책 XML의는 *B2C_1A_TrustFrameworkBase*됩니다.
