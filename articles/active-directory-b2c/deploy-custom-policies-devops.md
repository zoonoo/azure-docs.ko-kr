...
1. *B2CAssets* 폴더에 Azure AD B2C 정책 파일을 추가 합니다. 여기에는 *trustframeworkbase.xml*, *Trustframeworkextensions.xml*, *signuporsignin*, *profileedit .xml*, *passwordreset .xml*및 사용자가 만든 다른 모든 정책이 포함 됩니다. 이후 단계에서 사용할 각 Azure AD B2C 정책 파일의 파일 이름을 기록 합니다 (PowerShell 스크립트 인수로 사용 됨).
1. 리포지토리의 루트 디렉터리에 *Scripts* 라는 폴더를 만들고 자리 표시자 파일 *DeployToB2c.* p s 1로 이름을 지정 합니다. 이 시점에서 파일을 커밋하지 마세요. 이후 단계에서이 작업을 수행 합니다.
1. 다음 PowerShell 스크립트를 *DeployToB2c*에 붙여넣고 파일을 **커밋합니다** . 이 스크립트는 Azure AD에서 토큰을 획득 하 고 Microsoft Graph API를 호출 하 여 *B2CAssets* 폴더 내의 정책을 Azure AD B2C 테 넌 트에 업로드 합니다.
...
1. **파이프라인 변수** 아래에 다음 변수를 추가 하 고 지정 된 대로 값을 설정 합니다.
    ...
    | `tenantId` | `your-b2c-tenant.onmicrosoft.com`. 여기서 *-b2c* 은 Azure AD B2C 테 넌 트의 이름입니다. |
...
1. PowerShell 스크립트 태스크에 대해 다음 값을 입력 합니다.
    ...
    * **유형**: 파일 경로
    * **스크립트 경로**: 줄임표 (***...***)를 선택 하 고 *Scripts* 폴더로 이동한 다음 *DeployToB2C* 파일을 선택 합니다.
    * **인수의**

        **인수**에 대해 다음 값을 입력 합니다. `{alias-name}`를 이전 섹션에서 지정한 별칭으로 바꿉니다.
...
작업이 성공적으로 완료 되 면 각 사용자 지정 정책 파일에 대해 앞의 단계를 수행 하 여 배포 작업을 추가 합니다. 각 정책에 대 한 `-PolicyId` 및 `-PathToFile` 인수 값을 수정 합니다.

`PolicyId`는 TrustFrameworkPolicy 노드 내에서 XML 정책 파일의 시작 부분에 있는 값입니다. 예를 들어 다음 정책 XML의 `PolicyId` *B2C_1A_TrustFrameworkBase*됩니다.
