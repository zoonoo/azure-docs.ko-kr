...
ms.locfileid: "78183519"
---
# <a name="tutorial-add-identity-providers-to-your-applications-in-azure-active-directory-b2c"></a>자습서: Azure Active Directory B2C에서 응용 프로그램에 ID 공급자를 추가합니다.

애플리케이션에서 사용자가 다른 ID 공급자로 로그인하도록 허용할 수 있습니다. ‘ID 공급자’는 애플리케이션에 인증 서비스를 제공하는 동시에 ID 정보를 만들고 유지 관리합니다.** Azure Active Directory B2C(Azure AD B2C)에서 지원하는 ID 공급자를 Azure 포털을 사용하여 [사용자 흐름에](user-flow-overview.md) 추가할 수 있습니다.
...
일반적으로 애플리케이션에서 하나의 ID 공급자만 사용하지만, ID 공급자를 더 추가할 수 있습니다. 이 자습서에서는 Azure AD ID 공급자 및 Facebook ID 공급자를 애플리케이션에 추가하는 방법을 보여 줍니다. 애플리케이션에 두 ID 공급자를 추가하는 것은 선택 사항입니다. 또한 [아마존,](identity-provider-amazon.md) [GitHub,](identity-provider-github.md) [구글,](identity-provider-google.md) [링크드 인,](identity-provider-linkedin.md)마이크로 [소프트,](identity-provider-microsoft-account.md)또는 [트위터와](identity-provider-twitter.md)같은 다른 ID 제공 업체를 추가 할 수 있습니다 .
...
1. 이 응용 프로그램에 **대해서만 이 조직 디렉터리에서 계정** 선택을 수락합니다.
1. 리디렉션 **URI의**경우 **웹** 값을 수락하고 모든 소문자에 다음 URL을 `your-B2C-tenant-name` 입력하여 Azure AD B2C 테넌트의 이름으로 대체합니다.
...
1. **등록을**선택한 다음 이후 단계에서 사용하는 **응용 프로그램(클라이언트) ID를** 기록합니다.
1. 응용 프로그램 메뉴에서 **관리에서** **인증서 & 비밀을**선택한 다음 새 클라이언트 **비밀을**선택합니다.
1. 클라이언트 보안 에 대한 **설명을** 입력합니다. `Azure AD B2C App Secret`)을 입력합니다.
1. 만료 기간을 선택합니다. 이 응용 프로그램의 경우, **1 년의**선택을 수락합니다.
1. **추가를**선택한 다음 이후 단계에서 사용하는 새 클라이언트 보안 의 값을 기록합니다.

### <a name="create-a-facebook-application"></a>Facebook 애플리케이션 만들기

Azure AD B2C의 ID 공급자로 Facebook 계정을 사용하려면 Facebook에서 애플리케이션을 만들어야 합니다. 아직 Facebook 계정이 없는 경우 에서 [https://www.facebook.com/](https://www.facebook.com/)얻을 수 있습니다.
...
1. `Business and Pages` 등의 **범주**를 선택합니다. 이 값은 Facebook에서 요구하지만 Azure AD B2C에서는 사용되지 않습니다.
...
1. **개인 정보 취급 방침 URL**의 URL(예: `http://www.contoso.com/`)을 입력합니다. 개인 정보 보호 정책 URL은 응용 프로그램에 대 한 개인 정보 정보를 제공 하기 위해 유지 하는 페이지입니다.
1. **변경 내용 저장**을 선택합니다.
1. 페이지 상단에 **앱 ID**값을 기록합니다.
1. 앱 **시크릿**옆에서 **표시를** 선택하고 해당 값을 기록합니다. 앱 ID와 앱 시크릿을 모두 사용하여 테넌트의 ID 공급자로 Facebook을 구성합니다. **앱 시크릿은** 안전하게 저장해야 하는 중요한 보안 자격 증명입니다.
1. **제품**옆에 있는 더하기 기호를 선택한 다음 **Facebook 로그인에서** **설정을 선택합니다.**
1. 왼쪽 메뉴의 **Facebook 로그인에서** **설정을**선택합니다.
...
1. Azure AD B2C에서 Facebook 응용 프로그램을 사용할 수 있도록 하려면 페이지 오른쪽 상단의 **상태** 선택기를 클릭하고 **켜서** 응용 프로그램을 공개로 설정한 다음 **확인을**클릭합니다. 이때 상태가 **개발**에서 **라이브**로 변경됩니다.
...
1. (선택 사항) **Domain_hint**에 대한 값을 입력합니다. 예를 들어, *ContosoAD*. [도메인 힌트는](../active-directory/manage-apps/configure-authentication-for-federated-users-portal.md) 응용 프로그램의 인증 요청에 포함된 지시문입니다. 페더레이션된 IdP 로그인 페이지로 사용자를 빠르게 보내는 데 사용할 수 있습니다. 또는 다중 테넌트 애플리케이션에서 테넌트에 대한 브랜딩 Azure AD 로그인 페이지로 사용자를 바로 보내는 데 사용될 수 있습니다.
...
1. **사용자 흐름 실행을**선택한 다음 이전에 추가한 ID 공급자를 통해 로그인합니다.
1. 추가한 다른 ID 공급자에 대해 1-3단계를 반복합니다.

로그인 작업이 성공하면 다음과 유사한 디코딩된 `https://jwt.ms` 토큰을 표시하는 로 리디렉션됩니다.
...
다음으로 응용 프로그램에서 ID 환경의 일부로 사용자에게 표시되는 페이지의 UI를 사용자 지정하는 방법을 알아봅니다.
