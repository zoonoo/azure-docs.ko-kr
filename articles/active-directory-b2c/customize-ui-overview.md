...
중요 한 점은 [CORS를 사용](https://enable-cors.org/server.html)하는 공개적으로 사용 가능한 HTTPS 끝점에서 콘텐츠를 호스트 하는 것입니다. 콘텐츠에 지정하는 경우 절대 URL을 사용해야 합니다.

> [!NOTE]
> HTML 콘텐츠를 만들고, Azure Blob storage에 콘텐츠를 업로드 하 고, CORS를 구성 하는 방법에 대 한 자세한 내용은 UI 사용자 지정 문서의 [사용자 지정 페이지 콘텐츠 연습](custom-policy-ui-customization.md#custom-page-content-walkthrough) 섹션을 참조 하세요.
