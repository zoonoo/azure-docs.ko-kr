...
페이지 UI 사용자 지정 기능을 사용하면 소비자 등록, 로그인 및 프로필 편집 페이지의 모양과 느낌을 사용자 지정할 수 있습니다.([정책](active-directory-b2c-references.policies.md)을 구성하여) 소비자가 Azure AD B2C 서비스에 의해 제공된 응용 프로그램과 페이지 간에 이동하는 경우 일관된 환경을 유지합니다.
...
## 샘플 콘텐츠를 사용하여 페이지 UI 사용자 지정 기능을 테스트하는 데 도움이 되는 도우미 도구

샘플 콘텐츠를 사용하여 페이지 UI 사용자 지정 기능을 사용해 보려면 도우미 도구(출시 예정)를 사용합니다.

## 페이지의 각 형식에서 코어 UI 요소

이 섹션에서 Azure AD B2C가 콘텐츠에 있는 `<div id="api"></div>` 요소로 병합하는 HTML5 조각(페이지의 각 형식에)을 찾습니다. 스타일 시트를 사용하여 이러한 UI 요소를 사용자 지정할 수 있습니다. 이러한 스타일 시트는 `<head>` 조각에서 이러한 페이지 형식에 추가하는 기본 스타일 시트를 재정의할 수 있습니다.

> [AZURE.IMPORTANT]미리 보기를 하는 동안 여러분의 의견에서 학습하고 적용한 대로 변경할 정확한 UI 요소가 필요합니다. 다음 모두에 대한 자세한 내용의 경우 기본 페이지의 소스 코드를 검사합니다.
