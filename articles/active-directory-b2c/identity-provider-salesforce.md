---
title: 사용자 지정 정책을 사용하여 Salesforce SAML 공급자로 로그인 설정
...
2. 다음 **ClaimsExchange** 요소를 추가합니다. **ID** 에는 **TargetClaimsExchangeId** 에 사용한 것과 같은 값을 사용해야 합니다.
    ...
    **TechnicalProfileReferenceId** 값을 앞에서 만든 기술 프로필의 **ID** 로 업데이트합니다. 예를 들어 `salesforce` 또는 `LinkedIn-OAUTH`입니다.
...
2. 새 파일을 열고 **TrustFrameworkPolicy** 의 **PolicyId** 특성 값을 고유 값으로 업데이트합니다. 예들 들어 `SignUpSignInSalesforce`입니다.
