...
| IdTokenAudience | 아니요 | id_token의 대상 그룹입니다. 지정 된 경우 id 공급자가 반환 `aud` 하는 토큰의 클레임이 IdTokenAudience 메타 데이터에 지정 된 것과 동일한 지 여부를 Azure AD B2C 확인 합니다.  |
| METADATA | 예 | Openid connect 잘 알려진 구성 끝점이 라고도 하는 Openid connect Connect id 공급자 구성 문서를 가리키는 URL입니다. URL은 테 넌 트 `{tenant}` 이름으로 대체 되는 식을 포함할 수 있습니다.  |
| authorization_endpoint | 아니요 | Openid connect Connect id 공급자 구성 권한 부여 끝점을 가리키는 URL입니다. Authorization_endpoint 메타 데이터 값은 잘 알려진 Openid connect 구성 `authorization_endpoint` 끝점의 지정 된 보다 우선적으로 적용 됩니다. URL은 테 넌 트 `{tenant}` 이름으로 대체 되는 식을 포함할 수 있습니다. |
| 발급자 | 아니요 | Openid connect Connect id 공급자의 고유 식별자입니다. 발급자 메타 데이터의 값이 Openid connect 잘 알려진 `issuer` 구성 끝점의 지정 된 보다 우선 적용 됩니다.  지정 된 경우 id 공급자에서 반환 `iss` 된 토큰의 클레임이 발급자 메타 데이터에 지정 된 클레임과 동일한 지 여부를 Azure AD B2C 확인 합니다. |
...
다음 설정을 사용 하 여 오류 발생 시 표시 되는 오류 메시지를 구성할 수 있습니다. 메타 데이터는 Openid connect Connect 기술 프로필에서 구성 해야 합니다. 오류 메시지를 [지역화할](localization-string-ids.md#sign-up-or-sign-in-error-messages)수 있습니다.
...
ID 공급자의 리디렉션 URI를 구성할 때 `https://{your-tenant-name}.b2clogin.com/{your-tenant-name}.onmicrosoft.com/oauth2/authresp`를 입력합니다. 을 테 넌 트 `{your-tenant-name}` 의 이름으로 바꾸어야 합니다. 리디렉션 URI는 모두 소문자여야 합니다.
