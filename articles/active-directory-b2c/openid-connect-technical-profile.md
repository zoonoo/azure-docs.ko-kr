...
| end_session_endpoint | 아니요 | End session 끝점의 URL입니다. Authorization_endpoint 메타 데이터 값은 `end_session_endpoint` 잘 알려진 openid connect 구성 끝점의 지정 된 보다 우선적으로 적용 됩니다. |
...
| token_signing_algorithm | 아니요 | **Token_endpoint_auth_method** 메타 데이터가로 설정 된 경우 클라이언트 어설션에 사용 되는 서명 알고리즘입니다 `private_key_jwt` . 가능한 값: `RS256` (기본값) |
...
|ReadBodyClaimsOnIdpRedirect| 아니요| `true`Id 공급자 리디렉션의 응답 본문에서 클레임을 읽으려면로 설정 합니다. 이 메타 데이터는 클레임을 응답 페이로드에 반환 하는 [APPLE ID](identity-provider-apple-id.md)와 함께 사용 됩니다.|
