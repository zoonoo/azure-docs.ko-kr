...
유효성 검사에 실패 한 경우 REST API는 `userMessage` JSON 요소를 사용 하 여 HTTP 409 (충돌)을 반환 해야 합니다. IEF에는 REST API `userMessage` 에서 반환 하는 클레임이 필요 합니다. 유효성 검사에 실패 하면이 클레임은 사용자에 게 문자열로 표시 됩니다.
...
[Restful 기술 프로필](restful-technical-profile.md) 은 고유한 Restful 서비스와의 상호 작용을 지원 합니다. Azure AD B2C는 `InputClaims` 컬렉션의 RESTful 서비스로 데이터를 보내고 `OutputClaims` 컬렉션에서 데이터를 다시 받습니다. **ClaimsProviders** 요소를 찾고 다음과 같이 새 클레임 공급자를 추가 합니다.
...
등록 하는 동안 사용자의 충성도를 얻으려면 사용자가 화면에이 데이터를 입력할 수 있도록 해야 합니다. **LoyaltyId** 출력 클레임을 기존 등록 기술 프로필 섹션의 `OutputClaims` 요소에 추가 하 여 등록 페이지에 추가 합니다. 전체 출력 클레임 목록을 지정 하 여 화면에 클레임이 표시 되는 순서를 제어 합니다.  

을 `REST-ValidateProfile`호출 하는 등록 기술 프로필에 유효성 검사 기술 프로필 참조를 추가 합니다. 새 유효성 검사 기술 프로필이 기본 정책에 정의 된 `<ValidationTechnicalProfiles>` 컬렉션의 맨 위에 추가 됩니다. 이 동작은 유효성 검사가 성공적으로 수행 된 후에만 디렉터리에 계정을 만들도록 Azure AD B2C 이동 한다는 것을 의미 합니다.   
...
프로 모션 코드 클레임을 신뢰 당사자 응용 프로그램에 다시 반환 하려면 출력 클레임을 <em> `SocialAndLocalAccounts/` </em> 파일에 추가 합니다. 출력 클레임은 성공적인 사용자 경험 후에 클레임을 토큰에 추가 하 여 응용 프로그램에 보낼 수 있도록 합니다. 신뢰 당사자 섹션 내에서 기술 프로필 요소를 수정 하 여를 `promoCode` 출력 클레임으로 추가 합니다.
...
1. **사용자 지정 정책 업로드**를 선택 하 고 변경한 정책 파일 ( *Trustframeworkextensions.xml*및 *signu.xml*)을 업로드 합니다. 
