...
테넌트에 게스트 사용자를 초대하는 경우 받는 사람의 이메일 주소와 초대를 설명하는 메시지를 제공합니다. 초대 링크를 클릭하면 **시작** 단추가 선택되고 권한 검토가 수락되는 동의 페이지로 이동됩니다. 전자 메일 주소에 받은 편지함이 연결되어 있지 않으면 초대된 자격 증명을 사용하여 Microsoft 페이지로 이동한 후 동의 페이지로 이동할 수 있습니다. 그러면 사용자는 전자 메일의 링크를 클릭할 때와 동일한 방식으로 초대를 강제로 사용하게 됩니다. 예제: `https://myapps.microsoft.com/B2CTENANTNAME`.
...
소비자 사용자는 Azure AD B2C에서 보호하는 애플리케이션에 로그인할 수 있지만 Azure Portal과 같은 Azure 리소스에 액세스할 수 없습니다. 소비자 사용자는 로컬 계정이나 Facebook 또는 Twitter와 같은 페더레이션된 계정을 사용할 수 있습니다. [등록 또는 로그인 사용자 흐름](../active-directory-b2c/active-directory-b2c-reference-policies.md)을 사용 하거나, Azure AD Graph API를 사용 하거나, Azure Portal를 사용 하 여 소비자 계정을 만듭니다.
...
**Azure AD Graph API 사용**의 [소비자 사용자 계정 만들기](active-directory-b2c-devquickstarts-graph-dotnet.md) 섹션에 포함된 정보를 사용하여 Azure AD B2C 소비자 계정을 만들 수 있습니다. 또한 동일한 문서의 **소비자 사용자 계정 업데이트** 섹션에 포함된 정보를 사용하여 계정 속성을 관리할 수 있습니다.
