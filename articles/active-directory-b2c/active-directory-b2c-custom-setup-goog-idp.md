---
title: 사용자 지정 정책을 사용 하 여 Google 계정으로 로그인 설정
...
이 문서에서는 Azure Active Directory B2C (Azure AD B2C)에서 [사용자 지정 정책을](active-directory-b2c-overview-custom.md) 사용 하 여 Google 계정을 사용 하 여 사용자에 대 한 로그인을 사용 하도록 설정 하는 방법을 보여 줍니다.
...
6. **옵션**으로는 `Manual`를 선택합니다.
7. 정책 키의 **이름**을 입력합니다. 예: `GoogleSecret` `B2C_1A_` 접두사가 키의 이름에 자동으로 추가됩니다.
...
5. 사용자 경험 ID의 이름을 바꿉니다. 예: `SignUpSignInGoogle`
...
2. **ClaimsProviderSelects** 아래에 다음 요소를 추가 합니다. **TargetClaimsExchangeId** 값을 적절한 값(예: `GoogleExchange`)으로 설정합니다.
...
2. **Targetclaimsexchangeid**에 사용한 ID에 동일한 값을 사용 하도록 하는 다음 **claim이상 변경** 요소를 추가 합니다.
    ...
    **TechnicalProfileReferenceId**의 값을 앞에서 만든 기술 프로필의 ID로 업데이트합니다. 예: `Google-OAuth`
...
2. 새 파일을 열고 **TrustFrameworkPolicy**의 **PolicyId** 특성 값을 고유 값으로 업데이트합니다. 예: `SignUpSignInGoogle`
3. **PublicPolicyUri** 값을 정책의 URI로 업데이트합니다. 예를 들어 `http://contoso.com/B2C_1A_signup_signin_google`으로 업데이트할 수 있습니다.
