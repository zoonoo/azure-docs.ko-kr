---
title: Azure Active Directory B2C에서 사용자 지정 정책을 사용 하 여 다중 테 넌 트 Azure AD id 공급자에 대 한 로그인 설정
...
1. 정책 키의 **이름**을 입력합니다. 예를 들어, `AADAppSecret`을 입력합니다.  접두사 `B2C_1A_`은 생성 될 때 키 이름에 자동으로 추가 되므로 다음 섹션의 XML 참조는 *B2C_1A_AADAppSecret*입니다.
...
1. **Client_id** 를 앞에서 등록 한 Azure AD 다중 테 넌 트 응용 프로그램의 응용 프로그램 id로 설정 합니다.
1. **CryptographicKeys**에서 **StorageReferenceId** 의 값을 이전에 만든 정책 키의 이름으로 업데이트 합니다. 예를 들어, `B2C_1A_AADAppSecret`을 입력합니다.
...
1. 브라우저를 열고 테 넌 트에 대 한 Openid connect Connect 메타 데이터 URL로 이동 합니다. **발급자** 개체를 찾아 해당 값을 기록 합니다. 와 유사 `https://sts.windows.net/00000000-0000-0000-0000-000000000000/`하 게 표시 됩니다.
...
5. 사용자 경험 ID의 이름을 바꿉니다. 예를 들어, `SignUpSignInContoso`을 입력합니다.
...
2. 다음 **ClaimsExchange** 요소를 추가합니다. **Id**에는 **TargetClaimsExchangeId**에 사용한 것과 같은 값을 사용해야 합니다.
    ...
    **TechnicalProfileReferenceId** 값을 앞에서 만든 기술 프로필의 **ID**로 업데이트합니다. 예를 들어, `Common-AAD`을 입력합니다.
...
문제를 진단 하는 데 도움이 되도록 정책을 일시적으로 "개발자 모드"에 배치 하 고 Azure 애플리케이션 Insights로 로그를 수집할 수 있습니다. Azure Active Directory B2C에 [대 한 자세한 설명: 로그](active-directory-b2c-troubleshoot-custom.md)수집
