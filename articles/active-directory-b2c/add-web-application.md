---
title: 웹 API 응용 프로그램 추가 - Azure Active Directory B2C | 마이크로 소프트 문서
description: Active Directory B2C 테넌트에 웹 API 응용 프로그램을 추가하는 방법을 알아봅니다.
...
# <a name="add-a-web-api-application-to-your-azure-active-directory-b2c-tenant"></a>Azure Active Directory B2C 테넌트에 웹 API 애플리케이션 추가
 ...
 액세스 토큰을 제공하는 클라이언트 응용 프로그램의 요청을 수락하고 응답할 수 있도록 테넌트에 웹 API 리소스를 등록합니다. 이 문서에서는 Azure Active Directory B2C(Azure AD B2C)에서 웹 API를 등록하는 방법을 보여 주며 이 문서에서는
...
7. **회신 URL**에는 Azure AD B2C에서 애플리케이션이 요청한 토큰을 반환하는 엔드포인트를 입력합니다. 프로덕션 응용 프로그램에서 회신 URL을 `https://localhost:44332`와 같은 값으로 설정할 수 있습니다. 테스트 목적으로 회신 URL을 `https://jwt.ms`으로 설정합니다.
...
1. **리디렉션 URI**에서 **Web**을 선택한 다음, Azure AD B2C에서 애플리케이션이 요청한 토큰을 반환해야 하는 엔드포인트를 입력합니다. 프로덕션 응용 프로그램에서 리디렉션 URI를 와 같은 `https://localhost:5000`끝점으로 설정할 수 있습니다. 개발 또는 테스트 중에 토큰의 `https://jwt.ms`디코딩된 내용을 표시하는 Microsoft 소유 웹 응용 프로그램(토큰의 내용은 브라우저를 떠나지 마십시오)으로 설정할 수 있습니다. 등록된 응용 프로그램에서 언제든지 리디렉션 URI를 추가하고 수정할 수 있습니다.
1. **등록을**선택합니다.
1. 웹 API 코드에 사용할 **응용 프로그램(클라이언트) ID를** 기록합니다.

암시적 권한 부여 흐름(예: JavaScript 기반 단일 페이지 응용 프로그램(SPA)을 구현하는 응용 프로그램이 있는 경우 다음 단계를 수행하여 흐름을 활성화할 수 있습니다.
...
애플리케이션에서 보호된 웹 API를 호출하려면 애플리케이션 사용 권한을 API에 부여해야 합니다. 예를 들어 [자습서: Azure Active Directory B2C에 응용 프로그램을 등록하면](tutorial-register-applications.md) *webapp1이라는* 웹 응용 프로그램이 Azure AD B2C에 등록됩니다. 이 애플리케이션을 사용하여 웹 API를 호출할 수 있습니다.
