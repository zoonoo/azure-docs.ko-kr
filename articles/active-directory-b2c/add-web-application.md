...
1. **리디렉션 URI**에서 **Web**을 선택한 다음, Azure AD B2C에서 애플리케이션이 요청한 토큰을 반환해야 하는 엔드포인트를 입력합니다. 프로덕션 응용 프로그램에서와 같은 `https://localhost:5000`끝점에 리디렉션 URI를 설정할 수 있습니다. 개발 또는 테스트 중에는 토큰의 디코딩된 콘텐츠 `https://jwt.ms`를 표시 하는 Microsoft 소유의 웹 응용 프로그램 (토큰의 내용이 브라우저에서 벗어나면 안 됨)로 설정할 수 있습니다. 언제 든 지 등록 된 응용 프로그램에서 리디렉션 Uri를 추가 하 고 수정할 수 있습니다.
1. **등록**을 선택합니다.
