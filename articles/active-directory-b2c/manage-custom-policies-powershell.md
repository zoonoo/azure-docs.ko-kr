...
`{b2c-tenant-name}`를 Azure AD B2C 테 넌 트의 이름으로 대체 하 여 다음 명령을 실행 합니다. 디렉터리에서 [B2C IEF 정책 관리자](../active-directory/users-groups-roles/directory-assign-admin-roles.md#b2c-ief-policy-administrator) 역할이 할당 된 계정으로 로그인 합니다.
...
정책 콘텐츠를 로컬로 편집 하려면 명령 출력을 `-OutputFilePath` 인수를 사용 하 여 파일에 파이프 한 다음, 즐겨 사용 하는 편집기에서 파일을 엽니다.
...
Azure AD B2C 테 넌 트에 이미 있는 정책 ID를 사용 하 여 `Set-AzureADMSTrustFrameworkPolicy` 명령을 실행 하면 해당 정책의 콘텐츠를 덮어씁니다.
