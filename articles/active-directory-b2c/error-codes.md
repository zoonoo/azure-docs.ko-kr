...
| `AADB2C90006` | 요청에 제공 된{0}리디렉션 URI ' '이 (가) 클라이언트 id '{1}'에 대해 등록 되어 있지 않습니다. |
| `AADB2C90007` | 클라이언트 id ' '과 (와{0}) 연결 된 응용 프로그램에 등록 된 리디렉션 uri가 없습니다. |
| `AADB2C90008` | 요청에 클라이언트 id 매개 변수가 포함 되어 있지 않습니다. |
| `AADB2C90010` | 요청에 범위 매개 변수가 포함 되어 있지 않습니다. |
| `AADB2C90011` | 요청에 제공 된{0}클라이언트 id ' '이 (가) 정책에 등록{1}된 클라이언트 id ' '과 (와) 일치 하지 않습니다. |
| `AADB2C90012` | 요청에 제공{0}된 범위 ' '은 (는) 지원 되지 않습니다. |
| `AADB2C90013` | 요청에 제공 된 요청{0}된 응답 유형 ' '이 (가) 지원 되지 않습니다. |
| `AADB2C90014` | 요청에 제공 된 요청{0}된 응답 모드 ' '이 (가) 지원 되지 않습니다. |
| `AADB2C90016` | 요청 된 클라이언트 어설션 형식 '{0}'이 (가) 필요한 형식 '{1}'과 (와) 일치 하지 않습니다. |
| `AADB2C90017` | 요청에 제공 된 클라이언트 어설션이 잘못 되었습니다.{0} |
| `AADB2C90018` | 요청에 지정 된{0}클라이언트 id ' '이 (가) 테 넌 트{1}' '에 등록 되어 있지 않습니다. |
| `AADB2C90019` | '{1}' 테 넌 트의{0}id가 ' ' 인 키 컨테이너에 올바른 키가 없습니다. 이유: {2}. |
| `AADB2C90021` | 기술 프로필 '{0}'이 (가) ' ' 테 넌{1}트{2}의 정책 ' '에 없습니다. |
| `AADB2C90022` | '{1}' 테 넌 트의 정책 '{0}'에 대 한 메타 데이터를 반환할 수 없습니다. |
| `AADB2C90023` | 프로필 '{0}'에 필수 메타 데이터 키 '{1}'이 (가) 포함 되어 있지 않습니다. |
| `AADB2C90025` | ' '{0}테 넌 트{1}{2}의 정책 ' '에 있는 ' ' 프로필에는 필요한 암호화 키{3}' '이 (가) 포함 되어 있지 않습니다. |
...
| `AADB2C90039` | 요청에 클라이언트 어설션이 포함 되어 있지만 ' ' 테 넌 트{0}{1}의 제공 된 정책 ' '에 RelyingPartyPolicy의 client_secret 누락 되었습니다. |
...
| `AADB2C90044` | 클레임 확인자 '{0}{1}'이 (가) 클레임 ' '을 (를) 지원 하지 않습니다. |
...
| `AADB2C90055` | 요청에 제공{0}된 범위 ' '에서 ' '과 (와)https://example.com/calendar.read같은 리소스를 지정 해야 합니다. |
| `AADB2C90057` | 제공 된 응용 프로그램이 OAuth 암시적 흐름을 허용 하도록 구성 되어 있지 않습니다. |
| `AADB2C90058` | 제공 된 응용 프로그램이 공용 클라이언트를 허용 하도록 구성 되어 있지 않습니다. |
| `AADB2C90067` | 사후 로그 아웃 리디렉션 URI '{0}'의 형식이 잘못 되었습니다. 'https://example.com/return'과 같은 HTTPS 기반 URL을 지정 하거나 native client의 경우 ietf NATIVE client URI ' urn: ietf: wg: oauth: 2.0: oob '를 사용 합니다. |
| `AADB2C90068` | ID가 '{0}' 인 제공 된 응용 프로그램이이 서비스에 적합 하지 않습니다. B2C 포털을 통해 만든 응용 프로그램을 사용 하 고 다시 시도 하세요. |
| `AADB2C90075` | '{0}{1}' 단계에 지정 된 클레임 교환 ' '이 (가) 코드 '{2}' 및 이유가 '{3}' 인 HTTP 오류 응답을 반환 했습니다. |
...
| `AADB2C90083` | 요청에 필요한 매개 변수이 ( {0}가) 없습니다. |
...
| `AADB2C90092` | ' ' 테 넌 트{0}{1}에 대해 ID가 ' ' 인 제공 된 응용 프로그램을 사용할 수 없습니다. 응용 프로그램을 사용 하도록 설정 하 고 다시 시도 하세요. |
| `AADB2C90107` | Openid connect 범위가 요청에 제공{0}되지 않았거나 응용 프로그램에 대해 권한이 없어 id가 ' ' 인 응용 프로그램에서 id 토큰을 가져올 수 없습니다. |
...
| `AADB2C90117` | 요청에 제공{0}된 범위 ' '은 (는) 지원 되지 않습니다. |
| `AADB2C90118` | 사용자가 암호를 잊은 경우 |
| `AADB2C90120` | 요청에 지정 된 최대{0}기간 매개 변수 ' '이 (가) 잘못 되었습니다. 최대 기간은 '{1}'과 ({2}와) ' ' 사이의 정수 여야 합니다. |
...
| `AADB2C90146` | 요청에 제공{0}된 범위 ' '이 (가) 액세스 토큰에 대해 둘 이상의 리소스를 지정 합니다 .이는 지원 되지 않습니다. |
...
| `AADB2C90156` | Multi-factor validation 요청이 '{0}' 오류로 인해 실패 했습니다. |
| `AADB2C90157` | 사용자가 자체 어설션된 단계에 대 한 최대 다시 시도 횟수를 초과 했습니다. |
| `AADB2C90158` | '{0}' 오류로 인해 자체 어설션된 유효성 검사 요청이 실패 했습니다. |
| `AADB2C90159` | '{0}' 오류로 인해 자체 어설션된 확인 요청이 실패 했습니다. |
| `AADB2C90161` | '{0}' (으)로 인해 자체 어설션된 송신 응답이 실패 했습니다. |
| `AADB2C90165` | 상태에서 id가 '{0}' 인 SAML 시작 메시지를 찾을 수 없습니다. |
| `AADB2C90168` | HTTP 리디렉션 요청에 서명 된 요청에 대 한 필수 매개{0}변수 ' '이 (가) 포함 되어 있지 않습니다. |
...
| `AADB2C90188` | SAML 기술 프로필 '{0}'은 (는) ' '의{1}관련 엔터티 URL을 지정 하지만 '{2}' 때문에 메타 데이터를 가져오지 못했습니다. |
| `AADB2C90194` | 전달자 토큰{0}에 대해 지정 된 클레임 ' '이 (가) 사용 가능한 클레임에 없습니다. 사용 가능한 클레임{1}' '. |
...
| `AADB2C90220` | 저장소 식별자가 '{0}{1}' 인 테 넌 트 ' '의 키 컨테이너가 있지만 올바른 인증서를 포함 하지 않습니다. 인증서가 만료 되었거나 인증서가 나중에 활성화 될 수 있습니다 (nbf). |
...
| `AADB2C90242` | SAML 기술 프로필 '{0}'이 (가) '{1}' 이유 때문에 로드할 수 없는 대상 엔터티 CDATA를 지정 합니다. |
...
| `AADB2C90255` | 기술 프로필 '{0}'에 지정 된 클레임 교환이 예상 대로 완료 되지 않았습니다. 처음부터 세션을 시작 해 볼 수 있습니다. |
| `AADB2C90261` | '{1}' 단계에{0}지정 된 클레임 교환 ' '이 (가) 구문 분석할 수 없는 HTTP 오류 응답을 반환 했습니다. |
...
| `AADB2C90276` | 요청은 정책{0}'{1}{2}{3}' 테 넌 트 '{4}'에 대 한 technicalProfile ' '의 제어 설정 ' ': ' '과 (와) 일치 하지 않습니다. |
| `AADB2C90277` | 정책 '{2}'의{0}사용자 경험 '{1}'에 대 한 오케스트레이션 단계 ' '은 (는) 콘텐츠 정의 참조를 포함 하지 않습니다. |
...
| `AADB2C90288` | ' ' 테 넌 트{0}{1}{2}의 새로 고침 토큰 상환에 대해 TechnicalProfile ' '에서 참조 하는 Id가 ' ' 인 userjourney이 정책 '{3}' 또는 해당 기본 정책에 없습니다. |
| `AADB2C90289` | Id 공급자에 연결 하는 동안 오류가 발생 했습니다. 나중에 다시 시도하세요. |
| `AADB2C90296` | 응용 프로그램이 올바르게 구성 되지 않았습니다. 액세스 하려는 사이트의 관리자에 게 문의 하세요. |
| `AADB2C99005` | 요청에 잘못 된 문자 ' '이 ({0}가) 포함 된 잘못 된 범위 매개 변수가 있습니다. |
| `AADB2C99006` | Azure AD B2C에서 앱 id가 '{0}' 인 확장 앱을 찾을 수 없습니다. 자세한 내용은 https://go.microsoft.com/fwlink/?linkid=851224 을 (를) 참조 하세요. |
| `AADB2C99011` | 메타 데이터 값 '{0}'이 (가) 정책 '{1}{2}'의 TechnicalProfile ' '에 지정 되지 않았습니다. |
| `AADB2C99013` | 제공 된 grant_type [{0}] 및 token_type [{1}] 조합은 지원 되지 않습니다. |
| `AADB2C99015` | ' '{0}테 넌 트{2}의{1}정책 ' '에 있는 프로필 ' '에 리소스 소유자 암호 자격 증명 흐름에 필요한 모든 inputclaims 누락 되었습니다. |
