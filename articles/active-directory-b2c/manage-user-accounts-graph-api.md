...
`displayName`은 사용자에 대 한 Azure Portal 사용자 관리에 표시 되는 이름이 고, 액세스 토큰 Azure AD B2C는 응용 프로그램에 반환 됩니다. 이 속성은 필수입니다.
...
로컬 id의 경우 **Passwordprofile** 속성이 필요 하며 사용자의 암호를 포함 합니다. `forceChangePasswordNextSignIn` 속성은 `false`으로 설정 해야 합니다.
...
1. `appSettings` 섹션에서 `your-b2c-tenant`을 테 넌 트의 이름으로 바꾸고,을 관리 응용 프로그램 등록 값으로 `Application (client) ID` 하 고 `Client secret` 합니다 (이 문서의 [관리 응용 프로그램 등록](#register-a-management-application) 섹션 참조).
...
이 샘플 코드는 Microsoft Graph에 액세스 하는 고품질의 효율적이 고 복원 력 있는 응용 프로그램을 간편 하 게 빌드할 수 있도록 설계 된 [MICROSOFT GRAPH SDK](https://docs.microsoft.com/graph/sdks/sdks-overview)를 사용 합니다. 따라서 모든 Microsoft Graph API를 직접 만들 필요가 없습니다.
