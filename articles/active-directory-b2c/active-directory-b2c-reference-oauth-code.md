...
인증 코드를 받았으므로 이제 POST 요청을 `code` 엔드포인트로 전송하여 `/token`를 의도한 리소스에 대한 토큰으로 교환할 수 있습니다. Azure AD B2C에서는 요청에서 해당 범위를 지정 하 여 일반적인 방식으로 [다른 API에 대 한 액세스 토큰을 요청할](active-directory-b2c-access-tokens.md#request-a-token) 수 있습니다.
