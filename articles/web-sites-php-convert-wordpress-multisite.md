<properties urlDisplayName="Convert a WordPress Site to a Multisite" pageTitle="WordPress 사이트를 멀티 사이트로 변환" metaKeywords="WordPress, 멀티 사이트" description="Azure의 갤러리를 통해 만든 기존 WordPress 웹 사이트를 가져와 WordPress 멀티 사이트로 변환하는 방법에 대해 알아봅니다." metaCanonical="" services="web-sites" documentationCenter="PHP" title="Convert a WordPress Site to a Multisite" authors="tomfitz" solutions="" manager="wpickett" editor="" />
...
* [MySQL 및 FTP를 사용하는 PHP 웹 사이트(영문)][website-w-mysql-and-ftp-ftp-setup]

* [MySQL 및 Git를 사용하는 PHP 웹 사이트(영문)][website-w-mysql-and-git-git-setup]
...
하위 도메인과 하위 디렉터리 설정에 대한 자세한 내용은 WordPress Codex의 [멀티 사이트 네트워크 유형][wordpress-codex-types-of-networks](영문) 문서를 참조하세요.
...
[WordPress MU 도메인 매핑][wordpress-plugin-wordpress-mu-domain-mapping](영문) 플러그 인을 사용하면 네트워크에 있는 임의의 사이트에 사용자 지정 도메인을 쉽게 추가할 수 있습니다. 플러그 인의 정상적인 작동을 위해서는 포털과 도메인 등록 기관에서 추가 설정을 해야 합니다.
...
Azure 포털을 통해 웹 사이트로 돌아가서 이번에는 **구성** 탭을 클릭합니다. 이제 **도메인 관리** 단추를 사용할 수 있습니다. 이 단추를 클릭합니다.
...
Azure 웹 사이트에서 CNAME과 A 레코드를 모두 허용하지만 적절한 도메인 매핑을 사용하려면 *반드시* A 레코드를 사용해야 합니다. CNAME은 YOUR_DOMAIN.azurewebsites.net을 사용하여 자동으로 만들어지는 다른 CNAME으로 전달할 수 없습니다.
...
WordPress 멀티 사이트에는 사용자 지정 도메인에 매핑하는 기본 제공 방법이 현재 없습니다. 그러나 매핑 기능을 추가해 주는 [WordPress MU 도메인 매핑][wordpress-plugin-wordpress-mu-domain-mapping](영문)이라는 플러그 인이 있습니다. 사이트의 네트워크 관리 부분에 로그인하여 **WordPress MU 도메인 매핑**(영문) 플러그 인을 설치합니다.
