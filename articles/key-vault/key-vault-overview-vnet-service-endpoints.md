...
> [!IMPORTANT]
> 일단 방화벽 규칙이 적용되면 모든 Key Vault [데이터 평면](../key-vault/key-vault-secure-your-key-vault.md#data-plane-access-control) 작업은 호출자 요청이 허용되는 가상 네트워크 또는 IPV4 주소 범위에서 시작된 경우에만 수행될 수 있습니다. (관리 평면과 데이터 평면에 대해 자세히 알아보려면 [이것](../key-vault/key-vault-secure-your-key-vault.md#management-plane-access-control)을 참조하세요. Azure Portal에서 액세스 키 자격 증명 모음에도 적용됩니다. 사용자가 Azure Portal에서 Key Vault를 찾아볼 수 있지만 해당 클라이언트 컴퓨터가 허용 목록에 없는 경우 키/암호/인증서를 나열하지 못할 수 있습니다. 다른 Azure 서비스의 ‘Key Vault 선택기’도 마찬가지입니다. 방화벽 규칙이 해당 사용자의 클라이언트 컴퓨터를 금지하는 경우 사용자는 Key Vault 목록을 볼 수 있지만 키를 나열할 수는 없습니다.
