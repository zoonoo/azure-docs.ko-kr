...
먼저 로깅을 켭니다. 이 작업을 수행하는 방법에 대한 지침은 [Azure 키 자격 증명 모음 로깅](key-vault-logging.md)을 참조하십시오.
...
일반적으로 키 볼트에 대한 요청은 10초당 2,000개의 요청으로 제한됩니다. Key [Vault 서비스 제한에](key-vault-service-limits.md) 설명된 대로 예외는 키 작업입니다.
...
- 키 볼트 제한이 발생하면 다시 시도할 때 지수 백오프를 사용하도록 요청 코드를 조정합니다. 알고리즘은 여기에 설명되어 [있습니다: 앱을 제한하는 방법](key-vault-ovw-throttling.md#how-to-throttle-your-app-in-response-to-service-limits)
...
한도 증가 요청을 포함한 자세한 지침은 여기에서 찾을 수 있습니다: [키 볼트 제한 지침](key-vault-ovw-throttling.md)
