...
[API 참조 설명서](/python/api/overview/azure/keyvault-certificates-readme?view=azure-python) | [라이브러리 소스 코드](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/keyvault) | [패키지(Python 패키지 인덱스)](https://pypi.org/project/azure-keyvault/)
...
콘솔 창에서 Python용 Azure Key Vault 인증서 라이브러리를 설치합니다.
...
클라우드 기반 .NET 애플리케이션을 인증하는 가장 간단한 방법은 관리 ID를 사용하는 것입니다. 자세한 내용은 [App Service 관리 ID를 사용하여 Azure Key Vault에 액세스](../general/managed-identity.md)를 참조하세요.
...
[az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 명령에 clientId를 전달하여 서비스 사용자에게 권한을 부여하는 키 자격 증명 모음에 대한 액세스 정책을 만듭니다. 서비스 사용자에게 인증서에 대한 get, list 및 create 권한을 부여합니다.
...
Python용 Azure Key Vault 클라이언트 라이브러리를 사용하면 키 및 관련 자산(예: 인증서 및 비밀)을 관리할 수 있습니다. 아래의 코드 샘플에서는 클라이언트 생성, 인증서 생성, 인증서 검색 및 인증서 삭제 방법을 보여줍니다.
...
이제 애플리케이션이 인증되었으므로 자체 서명된 인증서를 키 자격 증명 모음에 배치할 수 있습니다. 
...
[az keyvault certificate show](/cli/azure/keyvault/certificate?view=azure-cli-latest#az-keyvault-certificate-show) 명령을 사용하여 인증서가 설정되었는지 확인할 수 있습니다.
...
이제 이전에 만든 인증서를 검색할 수 있습니다.

```python
retrieved_certificate = client.get_certificate(certificateName)
 ```

이제 인증서가 `retrieved_certificate`로 저장됩니다.

### <a name="delete-a-certificate"></a>인증서 삭제

마지막으로 키 자격 증명 모음에서 인증서를 삭제해 보겠습니다.

```python
client.delete_certificate(certificateName)
```

[az keyvault certificate show](/cli/azure/keyvault/certificate?view=azure-cli-latest#az-keyvault-certificate-show) 명령을 사용하여 인증서가 사라졌는지 확인할 수 있습니다.
