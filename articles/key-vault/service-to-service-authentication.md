...
- "SetProfileEnvironment"를 "True"로 구성 합니다. 자세한 내용은 [여기](https://docs.microsoft.com/iis/configuration/system.applicationhost/applicationpools/add/processmodel#configuration)를 참조 하세요. 
    ...
    - "SetProfileEnvironment"를 검색 합니다. "False"로 설정 되어 있으면 "True"로 변경 합니다. 존재 하지 않는 경우 processModel 요소 (/configuration/system.applicationHost/applicationPools/applicationPoolDefaults/processModel/@setProfileEnvironment)에 특성으로 추가 하 고 "True"로 설정 합니다.
