...
description: 이 자습서를 사용하여 단일 사용자/단일 암호 인증을 사용하는 리소스의 비밀을 자동으로 회전하는 방법을 알아봅니다.
services: key-vault
author: msmbaldwin
manager: rkarlin
tags: rotation
...
ms.locfileid: "80998906"
---
# <a name="automate-the-rotation-of-a-secret-for-resources-that-use-single-usersingle-password-authentication"></a>단일 사용자/암호 인증을 사용하여 리소스의 비밀 회전 자동화

Azure 서비스를 인증하는 가장 좋은 방법은 [관리 ID](managed-identity.md)를 사용하는 것이지만, 이 방법을 사용할 수 없는 시나리오도 있습니다. 이러한 경우 액세스 키 또는 비밀이 사용됩니다. 액세스 키 또는 비밀은 정기적으로 회전해야 합니다.

이 자습서에서는 단일 사용자/단일 암호 인증을 사용하는 데이터베이스 및 서비스의 비밀을 정기적으로 자동 회전하는 방법을 보여줍니다. 특히 이 자습서에서는 Azure Event Grid 알림에 의해 트리거되는 함수를 사용하여 Azure Key Vault에 저장된 SQL 서버 암호를 회전합니다.

![회전 솔루션 다이어그램](./media/rotate1.png)

1. 비밀 만료 30일 전에 Key Vault는 "만료 임박" 이벤트를 Event Grid에 게시합니다.
1. Event Grid에서는 이벤트 구독을 확인하고 HTTP POST를 사용하여 이 이벤트를 구독하는 함수 앱 엔드포인트를 호출합니다.
...
### <a name="rotation-function"></a>회전 함수
이 함수는 이벤트를 사용하여 Key Vault 및 SQL 데이터베이스를 업데이트하는 방법으로 비밀의 회전을 트리거합니다.

#### <a name="function-trigger-event"></a>함수 트리거 이벤트

다음 함수는 이벤트 데이터를 읽고 회전 논리를 실행합니다.
...
다음 회전 메서드는 비밀에서 데이터베이스 정보를 읽고, 새 버전의 비밀을 만들고, 데이터베이스를 새 비밀로 업데이트합니다.
...
만료 날짜가 짧은 비밀을 만들면 `SecretNearExpiry` 이벤트가 즉시 게시되고, 이어서 비밀을 회전하는 함수가 트리거됩니다.

## <a name="test-and-verify"></a>테스트 및 확인
몇 분 후 `sqluser` 비밀이 자동으로 회전됩니다.

비밀이 회전되었는지 확인하려면 **Key Vault** > **비밀**로 이동합니다.
...
[데이터베이스 연결됨] 값이 true인 [생성된 비밀 값]이 표시됩니다.
