...
ms.locfileid: "73464093"
---
# <a name="how-to-receive-and-respond-to-key-vault-notifications-with-azure-event-grid-preview"></a>방법: Azure Event Grid를 사용하여 키 자격 증명 모음 알림 수신 및 응답(미리보기)

현재 미리 보기로 제공되는 Azure Event Grid와 Key Vault를 통합하면 키 자격 증명 모음에 저장된 비밀의 상태가 변경될 때 사용자에게 알릴 수 있습니다. 기능에 대한 개요는 [Azure Event Grid를 사용하여 Key Vault 모니터링](event-grid-overview.md)을 참조하세요.

이 가이드는 Azure Event Grid를 통해 Key Vault 알림을 받는 방법과 Azure Automation을 사용하여 상태 변경에 응답하는 방법을 보여줍니다.
...
Azure Event Grid는 클라우드에 대한 이벤트 서비스입니다. 이 가이드에서는 키 자격 증명 모음에 대한 이벤트를 구독하고 이벤트를 Azure Automation으로 라우팅합니다. 키 자격 증명 모음의 비밀 중 하나가 곧 만료되면 상태 변경에 대해 Event Grid에 알리고 엔드포인트에 HTTP POST가 수행됩니다. 그러면 웹후크가 PowerShell 스크립트의 Azure Automation 실행을 트리거합니다.

![이미지](media/image1.png)

## <a name="create-an-azure-automation-account"></a>Azure Automation 계정 만들기

[Azure Portal](https://portal.azure.com)을 통해 Azure Automation 계정을 만듭니다.

1.  portal.azure.com으로 이동하여 구독에 로그인합니다.

1.  검색 창에 "Automation 계정"을 입력합니다.

1.  검색 창에 있는 드롭다운의 "서비스" 섹션에서 "Automation 계정"을 선택합니다.

1.  추가를 클릭합니다.

    ![](media/image2.png)

1.  "Automation 계정 추가" 블레이드에 필요한 정보를 입력하고 "만들기"를 선택합니다.

## <a name="create-a-runbook"></a>Runbook 만들기

Azure Automation 계정이 준비되면 Runbook을 만듭니다.

![](media/image3.png)

1.  방금 만든 Automation 계정을 선택합니다.

1.  프로세스 자동화 섹션에서 "Runbook"을 선택합니다.

1.  "Runbook 만들기"를 클릭합니다.

1.  Runbook의 이름을 지정하고 Runbook 유형으로 "PowerShell"을 선택합니다.

1.  만든 Runbook을 클릭하고 "편집" 단추를 선택합니다.

1.  테스트 목적으로 다음 코드를 입력하고 "게시" 단추를 클릭합니다. 그러면 받은 POST 요청의 결과가 출력됩니다.
...
이제 웹후크를 만들어서 새로 만든 Runbook을 트리거합니다.
...
1. Webhook의 이름을 지정하고 만료 날짜를 설정하고 **URL을 복사**합니다.

    > [!IMPORTANT] 
    > URL을 만든 후에는 볼 수 없습니다. 이 가이드의 나머지 부분에서 액세스할 수 있는 안전한 위치에 복사본을 저장해야 합니다.

1. "매개 변수 및 실행 설정"을 클릭하고 "확인"을 선택합니다. 매개 변수를 입력하지 마십시오. 그러면 "만들기" 단추가 활성화됩니다.
...
1.  키 자격 증명 모음으로 이동하여 "이벤트" 탭을 선택합니다. 이벤트 탭이 보이지 않으면 [포털의 미리 보기 버전](https://ms.portal.azure.com/?Microsoft_Azure_KeyVault_ShowEvents=true&Microsoft_Azure_EventGrid_publisherPreview=true)을 사용하고 있는지 확인합니다.
...
1.  "이벤트 형식 필터"의 경우 모두 선택된 상태("9개 선택됨")로 둡니다.
...
1.  키 자격 증명 모음의 이벤트 탭으로 이동합니다.

1.  만든 Event Grid 구독을 선택합니다.

1.  메트릭 아래에서 이벤트가 캡처되었는지 확인합니다. 두 가지 이벤트가 필요합니다. SecretNewVersion 및 SecretNearExpiry 이벤트이며, Event Grid가 키 자격 증명 모음에서 비밀의 상태 변경을 캡처했는지 확인합니다.

    ![](media/image11.png)

1.  Azure Automation 계정으로 이동합니다.

1.  "Runbook" 탭을 선택하고 생성한 Runbook을 선택합니다.
...
### <a name="unable-to-create-event-subscription"></a>이벤트 구독을 만들 수 없는 경우

Azure 구독 리소스 공급자에서 Event Grid 및 Key Vault 공급자를 다시 등록합니다. [Azure 리소스 공급자 및 종류](../azure-resource-manager/resource-manager-supported-services.md)를 참조하세요.

## <a name="next-steps"></a>다음 단계

축하합니다! 위의 단계를 모두 수행했다면 이제 키 자격 증명 모음에 저장된 비밀의 상태 변경에 프로그래밍 방식으로 응답할 준비가 되었습니다.

폴링 기반 시스템을 사용하여 키 자격 증명 모음에서 비밀의 상태 변경을 확인한 경우, 이 알림 기능을 사용하여 마이그레이션합니다. 비밀이 만료될 때 프로그래밍 방식으로 비밀을 갱신하는 코드로 Runbook의 테스트 스크립트를 대체할 수도 있습니다.

자세한 정보:

- [Azure Key Vault 개요](key-vault-overview.md)
- [Azure Event Grid 개요](../event-grid/overview.md)
- [Azure Event Grid를 사용하여 Key Vault 모니터링(미리 보기)](event-grid-overview.md)
