...
>
> 이 자습서에서는 최신 Azure CLI 2.0을 사용합니다. 이전(node.js 기반) CLI를 사용하는 지침에 대해서는 [해당 자습서](key-vault-manage-with-cli.md)를 참조하세요.
>
>
...
첫 번째 매개 변수는 리소스 그룹 이름이고, 두 번째 매개 변수 위치입니다. 위치의 경우, `az account list-locations` 명령을 사용하여 이 예에서 대체 위치를 지정하는 방법을 식별합니다. 자세한 정보가 필요한 경우 `az account list-locations -h`를 입력합니다.
...
소프트웨어 보호 키를 만들도록 Azure 키 자격 증명 모음을 사용하려면, `az key create` 명령을 사용하여 다음을 입력합니다.
```
...
* 사용자의 키를 보려면 다음을 입력 합니다. `az keyvault key list --vault-name 'ContosoKeyVault'`
* 사용자의 비밀을 보려면 다음을 입력 합니다. `az keyvault secret list --vault-name 'ContosoKeyVault'`
...
1. Azure 포털에 로그인합니다.
2. 왼쪽에서 **Azure Active Directory**를 클릭한 다음 응용 프로그램을 등록할 디렉터리를 선택합니다. <br> <br> 참고: 사용자 키 자격 증명 모음을 만든 Azure 구독을 포함하는 동일한 디렉터리를 선택해야 합니다. 이 디렉터리를 모르는 경우, **설정**을 클릭하고, 사용자 키 자격 증명 모음을 만든 구독을 식별하고 마지막 열에 표시되는 디렉터리의 이름을 확인합니다.
