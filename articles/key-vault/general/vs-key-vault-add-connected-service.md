...
키 볼트가 Visual Studio에 로그인한 계정과 다른 Microsoft 계정에서 실행 중인 경우(예: 키 볼트가 직장 계정에서 실행중이지만 Visual Studio에서 개인 계정을 사용하고 있음) Program.cs 파일에 오류가 발생하므로 Visual Studio에서 키 볼트에 액세스할 수 없습니다. 이 문제를 해결하려면
...
키 볼트 [개발자 가이드를](developers-guide.md)읽고 키 볼트 개발에 대해 자세히 알아보십시오.
