...
ms.locfileid: "93287438"
---
# <a name="azure-key-vault-basic-concepts"></a>Azure Key Vault 기본 개념

Azure Key Vault는 비밀을 안전 하 게 저장 하 고 액세스 하는 클라우드 서비스입니다. 비밀은 API 키, 암호, 인증서 또는 암호화 키와 같은에 대 한 액세스를 엄격 하 게 제어 하려는 모든 항목입니다. Key Vault 서비스는 자격 증명 모음과 관리 되는 HSM 풀 이라는 두 가지 유형의 컨테이너를 지원 합니다. 자격 증명 모음은 소프트웨어 및 HSM 지원 키, 암호 및 인증서를 저장 하도록 지원 합니다. 관리 되는 HSM 풀은 HSM 지원 키만 지원 합니다. 전체 세부 정보는 [Azure Key Vault REST API 개요](about-keys-secrets-certificates.md) 를 참조 하세요.
...
- **관리 되는 Hsm 관리자** : 관리자 역할이 할당 된 사용자는 관리 되는 hsm 풀을 완전히 제어할 수 있습니다. 다른 사용자에 게 제어 된 액세스를 위임 하는 추가 역할 할당을 만들 수 있습니다.

- **관리 되는 Hsm Crypto 담당자/사용자** : 관리 되는 hsm의 키를 사용 하 여 암호화 작업을 수행 하는 사용자 또는 서비스 사용자에 게 일반적으로 할당 되는 기본 제공 역할입니다. 암호화 사용자는 새 키를 만들 수 있지만 키를 삭제할 수는 없습니다.

- **관리 되는 HSM 암호화 서비스 암호화** : 고객 관리 키를 사용 하 여 미사용 데이터의 암호화를 위한 서비스 계정 관리 서비스 id (예: 저장소 계정)에 일반적으로 할당 되는 기본 제공 역할입니다.
...
- **보안 주체** : azure 보안 주체는 사용자가 만든 앱, 서비스 및 자동화 도구가 특정 Azure 리소스에 액세스 하는 데 사용 하는 보안 id입니다. 특정 역할을 가진 "사용자 id" (사용자 이름 및 암호 또는 인증서)와 긴밀 하 게 제어 된 사용 권한을 고려해 야 합니다. 보안 주체는 일반 사용자 id와 달리 특정 작업을 수행 해야 합니다. 관리 작업을 수행 하는 데 필요한 최소 권한 수준만 부여 하면 보안이 향상 됩니다. 응용 프로그램 또는 서비스에 사용 되는 보안 주체를 **서비스 주체** 라고 합니다.
...
| 수석 보안 책임자(CSO) |"보안 키 관리를 위해 응용 프로그램이 FIPS 140-2 수준 2 또는 FIPS 140-2 Level 3 Hsm을 준수 하는지 알고 싶습니다. <br/><br/>내 조직이 키 수명 주기를 제어하고 키 사용을 모니터링할 수 있는지 확인하고 싶습니다. <br/><br/>그리고 여러 Azure 서비스와 리소스를 사용 하지만 Azure의 단일 위치에서 키를 관리 하려고 합니다. " |√ FIPS 140-2 수준 2 유효성 검사 된 Hsm에 대해 **자격 증명 모음** 을 선택 합니다.<br/>√ FIPS 140-2 수준 3 유효성 검사 Hsm에 대해 **관리 되는 hsm 풀** 을 선택 합니다.<br/><br/>√ Key Vault는 Microsoft에서 키를 보거나 추출할 수 없게 설계되어 있습니다.<br/>√ 키 사용은 거의 실시간으로 기록됩니다.<br/><br/>√ Azure에 얼마나 많은 자격 증명 모음이 있는지, 지원되는 영역 및 사용하는 애플리케이션과 관계 없이 자격 증명 모음은 단일 인터페이스를 제공합니다. |
...
- [관리 되는 HSM 풀을 보호](../managed-hsm/access-control.md) 하는 방법 알아보기
