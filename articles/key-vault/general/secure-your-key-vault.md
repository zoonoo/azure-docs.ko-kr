...
두 평면은 인증을 위해 Azure AD(Azure Active Directory)를 사용합니다. 권한 부여의 경우 관리 평면은 Azure 역할 기반 액세스 제어 (RBAC)를 사용 하 고 데이터 평면은 Key Vault 액세스 정책 및 Azure RBAC (미리 보기)를 사용 합니다.
...
관리 평면에서 Azure RBAC (역할 기반 액세스 제어)를 사용 하 여 호출자가 실행할 수 있는 작업에 권한을 부여 합니다. Azure RBAC 모델에서 각 Azure 구독은 Azure AD의 인스턴스를 포함 합니다. 이 디렉터리에서 사용자, 그룹 및 애플리케이션에 대해 액세스 권한을 부여합니다. Resource Manager 배포 모델을 사용하는 Azure 구독의 리소스를 관리할 수 있는 액세스 권한을 부여합니다.
...
키 자격 증명 모음에 대 한 액세스 정책 Key Vault 설정 하 여 데이터 평면 액세스 권한을 부여할 수 있습니다. 이러한 액세스 정책을 설정하려면 사용자, 그룹 또는 애플리케이션은 해당 Key Vault에 대한 관리 평면에서 `Contributor` 권한이 있어야 합니다.
...
자격 증명 모음 액세스 정책에 대 한 Azure RBAC 권한을 사용 하는 경우의 주요 이점은 중앙화 된 액세스 제어 관리 이며, PIM (Privileged Identity Management)과의 통합입니다. Privileged Identity Management는 중요한 리소스에 대한 과도한, 불필요한 또는 잘못 사용된 액세스 권한의 위험을 완화할 수 있도록 시간 기반 및 승인 기반 역할 활성화를 제공합니다.


## <a name="firewalls-and-virtual-networks"></a>방화벽 및 가상 네트워크

추가 보안 계층의 경우 방화벽 및 가상 네트워크 규칙을 구성할 수 있습니다. 기본적으로 모든 네트워크(인터넷 트래픽 포함)의 트래픽에 대한 액세스를 거부하도록 Key Vault 방화벽 및 가상 네트워크를 구성할 수 있습니다. 특정 Azure 가상 네트워크 및 공용 인터넷 IP 주소 범위의 트래픽에 대한 액세스 권한을 부여하여 애플리케이션에 대한 보안 네트워크 경계를 빌드하도록 할 수 있습니다.
...
| Azure Storage 계정 | 없음 | 키: get, list, wrapKey, unwrapKey <br> | 암호화 서비스 암호화 Key Vault |
