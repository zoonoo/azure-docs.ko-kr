...
키와 같은 key vault 개체를 삭제 하면 서비스는 개체를 삭제 된 상태로 설정 하 여 검색 작업에 액세스할 수 없게 됩니다. 이 상태의 Key Vault 개체는 나열, 복구 또는 강제로/영구 삭제할 수만 있습니다. 개체를 보려면 `-InRemovedState` 여기에 설명 된 대로 [Azure PowerShell 명령에 대해](https://docs.microsoft.com/azure/key-vault/general/soft-delete-powershell#secrets)매개 변수를 사용 해야 합니다.  
