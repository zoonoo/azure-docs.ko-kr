---
title: 모든 Azure Key Vault에 대해 일시 삭제를 사용하도록 설정 | Microsoft Docs
...
ms.locfileid: "97590089"
---
# <a name="soft-delete-will-be-enabled-on-all-key-vaults"></a>모든 키 자격 증명 모음에 대해 사용하도록 설정되는 일시 삭제

> [!WARNING]
> **호환성이 손상되는 변경**: 일시 삭제를 옵트아웃하는 기능은 곧 사용되지 않습니다. Azure Key Vault 사용자와 관리자는 즉시 키 자격 증명 모음에 대해 일시 삭제를 사용하도록 설정해야 합니다.
>
...
> [!Important] 
> **키 자격 증명 모음에 일시 삭제를 설정하기 전에 다음 정보를 주의깊게 검토하세요.**

키 자격 증명 모음 이름은 전역적으로 고유합니다. 키 자격 증명 모음에 저장된 비밀의 이름도 고유합니다. 일시 삭제된 상태의 키 자격 증명 모음 또는 키 자격 증명 모음 개체의 이름은 다시 사용할 수 없습니다. 

**예 #1** 애플리케이션이 'Vault A'라는 키 자격 증명 모음을 프로그래밍 방식으로 만들고 나중에 'Vault A'를 삭제하는 경우. 키 자격 증명 모음이 일시 삭제된 상태로 전환됩니다. 애플리케이션은 키 자격 증명 모음이 일시 삭제된 상태에서 해제될 때까지 'Vault A'라는 이름의 다른 키 자격 증명 모음을 다시 만들 수 없습니다. 

**예 #2** 애플리케이션이 키 자격 증명 모음 A에 `test key`라는 키를 만들고 나중에 자격 증명 모음 A에서 키를 삭제하면 `test key` 개체가 일시 삭제된 상태에서 해제될 때까지 애플리케이션이 키 자격 증명 모음 A에 `test key`라는 새 키를 만들 수 없습니다. 

그러면 먼저 일시 삭제된 상태에서 제거하지 않고 키 자격 증명 모음 개체를 삭제하고 동일한 이름으로 다시 만들려고 시도하는 경우 충돌 오류가 발생할 수 있습니다. 이로 인해 애플리케이션 또는 자동화가 실패할 수 있습니다. 애플리케이션 및 관리에서 아래의 변경 작업을 필요에 따라 수행하기 전에 개발 팀에 문의하세요. 
...
5. "키 자격 증명 모음에서 일시 삭제를 사용하도록 설정해야 함" 정책을 선택합니다.
6. "할당"을 클릭합니다.
7. 범위를 구독으로 설정합니다.
8. 정책의 효과가 "감사"로 설정되어 있는지 확인합니다.
9. "검토 + 만들기"를 선택합니다.
10. 전체 환경 검색을 완료하는 데 최대 24시간이 걸릴 수 있습니다.
11. Azure Policy 블레이드에서 "규정 준수"를 클릭합니다.
12. 적용한 정책을 선택합니다.
...
애플리케이션 논리를 변경할 필요가 없는지 확인합니다. 확인되었으면 모든 키 자격 증명 모음에 대해 일시 삭제를 설정합니다.

### <a name="by-when-do-i-need-to-take-action"></a>언제까지 이 작업을 수행해야 하나요?

애플리케이션에서 영향을 받지 않도록 하려면 가능한 한 빨리 키 자격 증명 모음에 대해 일시 삭제를 설정합니다.
