...
* Key Vault에서 키 (KEK ( *키 교환 키* ) 라고도 함)를 생성 합니다. KEK는 `import` 키 작업만 있는 RSA HSM 키 여야 합니다. Key Vault Premium SKU만 RSA-HSM 키를 지원 합니다.
...
[Az keyvault key create](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-create) 명령을 사용 하 여 키 작업이로 `import`설정 된 KEK를 만듭니다. 다음 명령에서 반환 된`kid`키 식별자 ()를 기록 합니다. `kid` [3 단계의](#step-3-generate-and-prepare-your-key-for-transfer)값을 사용 합니다.
...
BYOK 도구를 다운로드 하 고 설치 하려면 HSM 공급 업체의 설명서를 참조 하세요. HSM 공급 업체의 지침에 따라 대상 키를 생성 한 다음 키 전송 패키지 (BYOK 파일)를 만듭니다. BYOK 도구는 [1 단계](#step-1-generate-a-kek) 와 `kid` [2 단계](#step-2-download-the-kek-public-key) 에서 다운로드 한 kekforbyok. publickey 파일을 사용 하 여 byok 파일에 암호화 된 대상 키를 생성 합니다.
