...
[API 참조 설명서](/python/api/overview/azure/keyvault-keys-readme?view=azure-python) | [라이브러리 소스 코드](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/keyvault) | [패키지(Python 패키지 인덱스)](https://pypi.org/project/azure-keyvault/)
...
콘솔 창에서 Python용 Azure Key Vault 키 라이브러리를 설치합니다.
...
클라우드 기반 애플리케이션을 인증하는 가장 간단한 방법은 관리 ID를 사용하는 것입니다. 자세한 내용은 [Azure Key Vault에 인증](../general/authentication.md)을 참조하세요. 
...
[az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 명령에 clientId를 전달하여 서비스 사용자에게 권한을 부여하는 키 자격 증명 모음에 대한 액세스 정책을 만듭니다. 서비스 사용자에게 키에 대한 get, list 및 create 권한을 부여합니다.
...
Python용 Azure Key Vault 클라이언트 라이브러리를 사용하면 키 및 관련 자산(예: 인증서 및 비밀)을 관리할 수 있습니다. 아래의 코드 샘플에서는 클라이언트 생성, 키 생성, 키 검색 및 삭제 방법을 보여줍니다.
...
이제 애플리케이션이 인증되었으므로 키 자격 증명 모음에 키를 배치할 수 있습니다. 

```python
rsa_key = client.create_rsa_key(myKey,size=2048)
```

[az keyvault key show](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show) 명령을 사용하여 키가 설정되었는지 확인할 수 있습니다.
...
이제 키가 `retrieved_key`로 저장됩니다.

### <a name="delete-a-key"></a>키 삭제

마지막으로 키 자격 증명 모음에서 키를 삭제해 보겠습니다.

```python
client.delete_key(keyName)
```

[az keyvault key show](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show) 명령을 사용하여 키가 사라졌는지 확인할 수 있습니다.
