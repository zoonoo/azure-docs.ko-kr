---
title: 웹 애플리케이션에서 Azure Key Vault 사용 자습서 - Azure Key Vault | Microsoft Docs
...
[Azure Key Vault 시작](key-vault-get-started.md) 단계를 완료하여 암호, 클라이언트 ID, 클라이언트 암호에 대한 URI를 가져오고 애플리케이션을 등록합니다. 웹 응용 프로그램은 자격 증명 모음에 액세스하며 Azure Active Directory에 반드시 등록되어야 합니다. 또한 Key Vault에 대한 액세스 권한이 있어야 합니다. 그렇지 않으면 시작 자습서의 애플리케이션 등록으로 돌아가서 나열된 단계를 반복합니다. Azure Web Apps을 만드는 방법에 대한 자세한 내용은 [Web Apps 개요](../app-service/overview.md)를 참조하세요.
...
## <a id="appstart"></a>애플리케이션 시작 시 비밀 검색
...
.pfx에 대한 종료 날짜와 암호를 메모해 둡니다(예: 2019년 5월 15일 및 MyPassword). 아래 스크립트에서 필요합니다. 
...
먼저 인증서에 액세스하는 코드가 있습니다. 저장소 위치는 LocalMachine이 아닌 CurrentUser입니다. 테스트 인증서를 사용 중이므로 Find 메서드에 'false'를 입력합니다.
