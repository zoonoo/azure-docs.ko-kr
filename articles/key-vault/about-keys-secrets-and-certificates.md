...
- **"하드" 키**: HSM(하드웨어 보안 모듈)에서 처리되는 키입니다. 이러한 키는 Key Vault HSM 보안 권역 중 하나에서 보호됩니다(격리를 유지하기 위해 지역별로 보안 권역이 하나씩 있음). 클라이언트는 소프트 형식으로 또는 호환되는 HSM 디바이스에서 내보내는 방식으로 RSA 또는 EC 키를 가져올 수 있습니다. 클라이언트는 Key Vault에 키를 생성하라고 요청할 수도 있습니다. 이 키 유형은 HSM 키 재료를 운반하기 위해 JWK가 얻는 key_hsm 특성을 추가합니다.
...
-   **ES256** - 곡선 P-256을 사용하여 생성된 SHA-256 메시지 다이제스트 및 키에 대한 ECDSA입니다. 이 알고리즘은 [RFC7518에](https://tools.ietf.org/html/rfc7518)설명되어 있습니다.
-   **ES256K** - 곡선 P-256K를 사용하여 생성된 SHA-256 메시지 다이제스트 및 키에 대한 ECDSA입니다. 이 알고리즘 표준화 보류 중입니다.
-   **ES384** - 곡선 P-384를 사용하여 생성된 SHA-384 메시지 다이제스트 및 키에 대한 ECDSA입니다. 이 알고리즘은 [RFC7518에](https://tools.ietf.org/html/rfc7518)설명되어 있습니다.
-   **ES512** - 곡선 P-521을 사용하여 생성된 SHA-512 메시지 다이제스트 및 키에 대한 ECDSA입니다. 이 알고리즘은 [RFC7518에](https://tools.ietf.org/html/rfc7518)설명되어 있습니다.
...
-   **PS256** - [RFC7518에](https://tools.ietf.org/html/rfc7518)설명된 바와 같이 SHA-256 및 MGF1을 사용하여 SHA-256 및 MGF1을 사용하는 RSASSA-PSS.
-   **PS384** - [RFC7518에](https://tools.ietf.org/html/rfc7518)설명된 바와 같이 SHA-384 및 MGF1을 사용하여 SHA-384 및 MGF1을 사용한다.
-   **PS512** - [RFC7518에](https://tools.ietf.org/html/rfc7518)설명된 바와 같이 SHA-512 및 MGF1을 사용하여 SHA-512 및 MGF1을 사용하는 RSASSA-PSS.
...
- *enabled*: 부울, 선택 사항, 기본값은 **true**입니다. 암호화 작업을 위해 사용 설정 및 사용 가능한 키인지 여부를 지정합니다. *사용 가능한* 특성은 *nbf* 및 *exp.* *nbf와* *exp*간에 작업이 발생하면 *사용 이* 설정되어 있는 경우에만 **true로**설정된 경우에만 허용됩니다. *nbf* / *exp* 창 외부의 작업은 [특정 조건에서](#date-time-controlled-operations)특정 작업 유형을 제외하고 자동으로 허용되지 않습니다.
- *nbf*: IntDate, 선택 사항, 기본값은 지금입니다. *nbf*(이전이 아님) 특성은 [특정 조건](#date-time-controlled-operations)에 따른 특정 작업 유형을 제외하고는 암호화 작업에 키를 사용하지 않아야 하는 시간을 식별합니다. *nbf* 특성을 처리하려면 현재 날짜/시간이 *nbf* 특성에 나열된 날짜/시간 이전이 아닌 시간 이후이거나 같아야 합니다. Key Vault는 클록 오차를 처리하기 위해 일반적으로 약간(몇 분 이내)의 여유를 제공해야 합니다. 이 값은 IntDate 값이 포함된 숫자여야 합니다.  
- *exp*: IntDate, 선택 사항, 기본값은 "영원"입니다. *exp*(만료 시간) 특성은 [특정 조건](#date-time-controlled-operations)에 따른 특정 작업 유형을 제외하고는 암호화 작업에 키를 사용하지 않아야 하는 만료 시간 또는 그 이후를 식별합니다. *exp* 특성을 처리하려면 현재 날짜/시간이 *exp* 특성에 나열된 만료 날짜/시간 이전이어야 합니다. Key Vault는 클록 오차를 처리하기 위해 일반적으로 약간(몇 분 이내)의 여유를 제공해야 합니다. 이 값은 IntDate 값이 포함된 숫자여야 합니다.  
...
*nbf* / *exp* 창 밖에서 아직 유효하지 않고 만료되지 않은 키는 **해독,** **줄 바꿈**해제 및 작업 **확인(403,** 금지됨)에 대해 작동합니다. 아직 유효하지 않은 상태를 사용하는 이유는 프로덕션에서 사용하기 전에 키를 테스트할 수 있도록 하기 위한 것입니다. 만료된 상태를 사용하는 이유는 키가 유효할 때 만들어진 데이터에 대한 복구 작업을 허용하기 위한 것입니다. 또한 Key Vault 정책을 사용하거나 *enabled* 키 특성을 **false**로 업데이트하여 키에 대한 액세스를 사용하지 않도록 설정할 수 있습니다.

데이터 형식에 대한 자세한 내용은 [데이터 형식을](#data-types)참조하십시오.
...
>[!Note]
>태그는 *목록이* 있거나 해당 개체 유형(키, 암호 또는 인증서)에 대한 권한을 *얻는* 경우 호출자가 읽을 수 있습니다.

###  <a name="key-access-control"></a>키 액세스 제어

Key Vault에서 관리하는 키에 대한 액세스 제어는 키 컨테이너 역할을 하는 Key Vault 수준에서 제공됩니다. 키에 대한 액세스 제어 정책은 동일한 키 자격 증명 모음의 비밀에 대한 액세스 제어 정책과 구별됩니다. 사용자는 키를 보관할 하나 이상의 자격 증명 모음을 만들 수 있으며, 시나리오에 따라 키를 적절하게 세분화하고 관리해야 합니다. 키 액세스 제어는 비밀 액세스 제어와 무관합니다.  

자격 증명 모음의 키 액세스 제어 항목에 대해 사용자/서비스 사용자별로 부여할 수 있는 권한은 다음과 같습니다. 이러한 사용 권한은 키 개체에 허용되는 작업을 밀접하게 반영합니다.  키 자격 증명 모음에서 서비스 주체에 대한 액세스 권한을 부여하는 것은 일회성 작업이며 모든 Azure 구독에 대해 동일하게 유지됩니다. 원하는 만큼 인증서를 배포하는 데 사용할 수 있습니다. 
...
-   TLS/SSL 인증서에 대한 다음 인증서 발급자 공급자와 키 Vault 파트너
...
2. 관리자는 TLS/SSL 인증서를 등록(및 갱신)하기 위해 키 자격 증명을 요청합니다.  
...
- [키 볼트 개발자 가이드](/azure/key-vault/key-vault-developers-guide)
