...
- ASP.NET 4.7.1 이상 또는 ASP.NET Core 2.0 이상 웹 프로젝트가 열려 있습니다.
...
1. 만든 Key Vault의 Key Vault 섹션에서 **비밀**을 선택 하 고 **생성/가져오기**를 선택 합니다.
...
1. .NET Core 2의 경우 `Program.cs` 탭을 선택 하 고 Program 클래스의 `BuildWebHost` 정의를 다음과 같이 변경 합니다.
...
1. 그런 다음 *Index.cshtml.cs* 와 같은 페이지 파일 중 하나를 열고 다음 코드를 작성 합니다.
   1. 지시문을 사용 하 여 `Microsoft.Extensions.Configuration`에 대 한 참조를 포함 합니다.
   ...
   1. 런타임에 값을 확인 하려면 메시지에 암호를 표시 하기 위해 `ViewData["Message"]`를 표시 하는 코드를 *cshtml* 파일에 추가 합니다.
...
런타임에 true 값으로 대체 되는 `appSettings` 요소의 더미 값을 web.config 파일에 포함 하도록 구성을 설정할 수 있습니다. 그런 다음 `ConfigurationManager.AppSettings` 데이터 구조를 통해이에 액세스할 수 있습니다.

1. Web.config 파일을 편집 합니다.  AppSettings 태그를 찾고, `configBuilders="AzureKeyVault"`특성을 추가 하 고, 줄을 추가 합니다.

   ```xml
      <add key="mysecret" value="dummy"/>
   ```

1. *HomeController.cs*의 `About` 메서드를 편집 하 여 확인 값을 표시 합니다.
...
Key Vault Visual Studio에 로그인 하는 것과 다른 Microsoft 계정에서 실행 중인 경우 (예: Key Vault 회사 계정에서 실행 중이지만 Visual Studio에서 개인 계정을 사용 하는 경우) Program.cs 파일에 오류가 발생 합니다. Visual Studio에서 Key Vault에 액세스할 수 없습니다. 이 문제를 해결하려면
