...
모든 Communication Services 이벤트의 `subject` 필드는 이벤트의 대상으로 지정된 사용자, 전화 번호 또는 엔터티를 식별합니다. 일반적인 접두사는 단순한 [Event Grid 필터링](../../event-grid/event-filtering.md)을 허용하는 데 사용됩니다.
...
* Azure Event Grid 개념에 대한 소개는 [Event Grid란?](../../event-grid/concepts.md)을 참조하세요.
* Azure Event Grid 시스템 토픽에 대한 소개는 [Azure Event Grid의 시스템 토픽](../../event-grid/system-topics.md)을 참조하세요.
