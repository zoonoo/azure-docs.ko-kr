...
전화를 받으려면 호출 개체에서 `accept` 메서드를 호출합니다. 대리자를 `CallAgent`로 설정합니다.
...
### <a name="set-the-default-microphone-or-speaker"></a>기본 마이크 또는 스피커 설정

디바이스 관리자를 사용하여 통화가 시작될 때 사용되는 기본 디바이스를 설정할 수 있습니다. 스택 기본값을 설정하지 않으면 Communication Services는 OS 기본값으로 대체됩니다.
