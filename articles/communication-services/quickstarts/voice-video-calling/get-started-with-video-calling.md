...
- 호출 클라이언트를 인스턴스화하기 위한 사용자 액세스 토큰을 만듭니다. [사용자 액세스 토큰 만들기 및 관리 방법에 대해 알아봅니다.](https://docs.microsoft.com/azure/communication-services/quickstarts/access-tokens?pivots=programming-language-csharp)
...
이 빠른 시작에서는 Azure Communication 통화 클라이언트 라이브러리 `1.0.0.beta-6`을 사용했습니다. 
...
<USER_ACCESS_TOKEN>을 리소스에 대한 유효한 사용자 액세스 토큰으로 바꿔야 합니다. 사용할 수 있는 토큰이 아직 없는 경우 사용자 액세스 토큰 설명서를 참조하세요. CallClient를 사용하여 전화를 걸거나 받을 수 있는 CommunicationUserCredential로 CallAgent 인스턴스를 초기화합니다. DeviceManager에 액세스하려면 먼저 callAgent 인스턴스를 만들어야 합니다. 그런 다음, `CallClient` 인스턴스에서 `getDeviceManager` 메서드를 사용하여 `DeviceManager`를 가져올 수 있습니다.
...
```  
`LocalVideoStream`을 렌더링하려면 `Renderer`의 새 인스턴스를 만든 다음, 비동기 `createView` 메서드를 사용하여 새 RendererView 인스턴스를 만들어야 합니다. 그러면 모든 UI 요소에 `view.target`을 연결할 수 있습니다. 
...
```
모든 원격 참가자는 통화 인스턴스의 `remoteParticipants` 컬렉션을 통해 사용할 수 있습니다. 추가된 원격 참가자를 구독하려면 현재 통화의 원격 참가자를 구독하고 `remoteParticipantsUpdated` 이벤트를 수신해야 합니다.
...
```
현재 통화의 `remoteParticipants` 컬렉션을 구독하고 `videoStreams` 컬렉션을 검사하여 각 참가자의 스트림을 나열할 수 있습니다. 또한 추가된 원격 참가자를 처리하려면 remoteParticipantsUpdated 이벤트를 구독해야 합니다. 
...
```
`RemoteVideoStream`을 렌더링하려면 `Renderer`의 새 인스턴스를 만든 다음, 비동기 `createView` 메서드를 사용하여 새 `RendererView` 인스턴스를 만들어야 합니다. 그러면 모든 UI 요소에 `view.target`을 연결할 수 있습니다. 
...
- [클라이언트 라이브러리 기능 호출](https://docs.microsoft.com/azure/communication-services/quickstarts/voice-video-calling/calling-client-samples?pivots=platform-web)에 대한 자세한 정보
