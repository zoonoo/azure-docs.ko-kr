...
2. `git clone`
3. **Chat/ClientApp** 폴더로 이동하여 `npm run setup`을 실행합니다.
   ...
   2. 이전 단계를 완료한 후 `npm run setup` 명령을 다시 실행합니다.
4. Azure Portal에서 `Connection String`을 가져옵니다. 연결 문자열에 대한 자세한 내용은 [Azure Communication 리소스 만들기](../quickstarts/create-communication-resource.md)를 참조하세요.
5. `Connection String`을 가져온 후에는 Chat 폴더 아래에 있는 **Chat/appsettings.json** 파일에 연결 문자열을 추가합니다. `ResourceConnectionString` 변수에 연결 문자열을 입력합니다.

### <a name="local-run"></a>로컬 실행

1. Chat 폴더로 이동합니다.
2. Visual Studio에서 `Chat.csproj` 솔루션을 엽니다.
3. `Chat` 프로젝트를 실행합니다.

*브라우저는 localhost:5000(노드가 클라이언트 앱을 배포)에서 열립니다. Internet Explorer에서 앱이 지원되지 않습니다.
...
- [Azure Communication 미리 보기](https://github.com/Azure/communication-preview) - 채팅 웹 SDK에 대한 자세한 정보
