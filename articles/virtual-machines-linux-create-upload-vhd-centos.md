<properties urlDisplayName="Upload a CentOS-based VHD" pageTitle="Azure에서 CentOS 기반 Linux VHD 만들기 및 업로드" metaKeywords="Azure VHD, uploading Linux VHD, CentOS" description="CentOS 기반 Linux 운영 체제가 포함된 Azure VHD(가상 하드 디스크)를 만들고 업로드하는 방법에 대해 알아봅니다." metaCanonical="" services="virtual-machines" documentationCenter="" title="CentOS 기반 Linux 운영 체제가 포함된 가상 하드 디스크 만들기 및 업로드" authors="kathydav" solutions="" manager="timlt" editor="tysonn" />
...
-   [Azure용 CentOS 6.x 가상 컴퓨터 준비][Azure용 CentOS 6.x 가상 컴퓨터 준비]
-   [Azure용 CentOS 7.0 이상 가상 컴퓨터 준비][Azure용 CentOS 7.0 이상 가상 컴퓨터 준비]

## 필수 조건

이 문서에서는 가상 하드 디스크에 CentOS 또는 그와 비슷한 파생 Linux 운영 체제를 이미 설치했다고 가정합니다. Hyper-V와 같은 가상화 솔루션 등의 다양한 도구를 사용하여 .vhd 파일을 만들 수 있습니다. 관련 지침은 [Hyper-V 역할 설치 및 가상 컴퓨터 구성][Hyper-V 역할 설치 및 가상 컴퓨터 구성]을 참조하세요.

**CentOS 설치 참고 사항**

-   새 VHDX 형식은 Azure에서 지원되지 않습니다. Hyper-V 관리자 또는 convert-vhd cmdlet을 사용하여 디스크를 VHD 형식으로 변환할 수 있습니다.

-   Linux 시스템 설치 시에는 LVM(설치 기본값인 경우가 많음)이 아닌 표준 파티션을 사용하는 것이 좋습니다. 이렇게 하면 특히 문제 해결을 위해 OS 디스크를 다른 VM에 연결해야 하는 경우 복제된 VM과 LVM 이름이 충돌하지 않도록 방지합니다. 원하는 경우에는 데이터 디스크에서 LVM 또는 [RAID][RAID]를 사용할 수 있습니다.

-   크기가 큰 VM의 경우 2.6.37 이전 버전 Linux 커널의 버그로 인해 NUMA가 지원되지 않습니다. 이 문제는 주로 업스트림 Red Hat 2.6.32 커널을 사용하는 배포에 영향을 줍니다. Azure Linux 에이전트(waagent)를 수동으로 설치하면 Linux 커널의 GRUB 구성에서 NUMA가 자동으로 사용하지 않도록 설정됩니다. 여기에 대한 자세한 내용은 아래 단계에서 확인할 수 있습니다.
...
8.  **CentOS 6.3에만 해당**: Linux 통합 서비스 드라이버 설치
    ...
    **중요: 이 단계는 CentOS 6.3 이하 버전에만 적용됩니다.** CentOS 6.4 이상 버전에서는 Linux 통합 서비스가 *커널에서 이미 제공됩니다*.
    ...
    a) [Microsoft 다운로드 센터][Microsoft 다운로드 센터]에서 Linux 통합 서비스 드라이버가 포함된 .iso 파일을 다운로드합니다.
...
## <span id="centos7"></span> </a>CentOS 7.0 이상
...
16. Hyper-V 관리자에서 **작업 -\> 종료**를 클릭합니다. 이제 Linux VHD를 Azure에 업로드할 수 있습니다.
  ...
  [Hyper-V 역할 설치 및 가상 컴퓨터 구성]: http://technet.microsoft.com/library/hh846766.aspx
  [RAID]: ../virtual-machines-linux-configure-raid
  [Microsoft 다운로드 센터]: http://www.microsoft.com/ko-kr/download/details.aspx?id=41554
