...
이 자습서에서는 [이전 자습서](app-service-dotnet-create-api-app.md)에서 만든 Web API 프로젝트를 새로운 [API 앱](app-service-api-apps-why-best-platform.md)에 배포합니다. Visual Studio를 사용하여 [Azure 앱 서비스](app-service-value-prop-what-is.md)에서 API 앱 리소스를 만들고 Web API 코드를 Azure API 앱에 배포합니다.

### 다른 배포 옵션

API 앱을 배포할 수 있는 다른 여러 방법이 있습니다. API 앱은 웹 서비스를 호스트하기 위한 추가 기능이 있는 [웹앱](app-service-web-overview.md)으로, [웹앱에 사용할 수 있는 모든 배포 방법은](web-sites-deploy.md) API 앱에도 사용할 수 있습니다. API 앱을 호스트하는 웹앱을 Azure Preview 포털에서는 API 앱 호스트라고 하며, API 앱 호스트 포털 블레이드를 사용하여 배포를 구성할 수 있습니다. API 앱 호스트 블레이드에 대한 자세한 내용은 [API 앱 관리](app-service-api-manage-in-portal.md)를 참조하세요.
