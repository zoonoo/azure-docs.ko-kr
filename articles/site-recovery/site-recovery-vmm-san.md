...
3. **VMM 서버 준비**에서 **등록 키 생성** 파일을 클릭합니다. 이 키는 생성된 날로부터 5일간 유효합니다. 보호하려는 VMM 서버에서 콘솔이 실행되지 않는 경우, VMM 서버가 액세스할 수 있는 안전한 위치에 파일을 다운로드합니다. 예를 들어 공유에 다운로드할 수 있습니다. 다음 사항에 유의하세요.

- 공급자를 처음 설치하는 경우 활성 해당 클러스터의 노드에 설치하고 설치를 완료하여 VMM 서버를 Azure Site Recovery 자격 증명 모음에 등록합니다. 그런 다음 해당 클러스터의 다른 노드에 공급자를 설치합니다.
- 공급자 버전을 업그레이드하는 경우 클러스터의 모든 노드에서 공급자 설치를 실행합니다.

4. **VMM 서버에 설치할 Microsoft Azure Site Recovery 공급자 다운로드**를 클릭하여 최신 버전의 공급자 설치 파일을 받습니다.
5. 원본 및 대상 VMM 서버에서 이 파일을 실행하여 Azure Site Recovery 공급자 설정 마법사를 엽니다.
6. **사전 요구 사항 확인** 페이지에서 공급자 설정을 시작하기 위해 VMM 서비스를 중지하도록 선택합니다. 서비스가 중지되고 설정이 완료될 때 자동으로 다시 시작됩니다. VMM 클러스터에 설치하는 경우 클러스터 역할을 중지해야 합니다.

	![필수 조건](./media/site-recovery-vmm-san/SRSAN_ProviderPrereq.png)

5. **Microsoft 업데이트**에서 업데이트를 선택할 수 있습니다. 이 설정이 사용되면 공급자가 Microsoft 업데이트 정책에 따라 설치됩니다.

공급자가 설치된 후에는 설정을 계속 진행하여 자격 증명 모음에 서버를 등록합니다.

6. VMM 서버에서 실행 중인 공급자를 인터넷에 연결하는 방법을 인터넷 연결 페이지에서 지정합니다. 프록시를 사용하는 것이 아니라, VMM 서버에 구성된 기본 프록시를 사용하거나 사용자 지정 프록시 서버를 사용하도록 선택할 수 있습니다. 다음 사항에 유의하세요.
	...
	- 인터넷 연결을 확인하기 위해 테스트가 실행됩니다. 모든 프록시 오류는 VMM 콘솔에 표시됩니다.

	![인터넷 설정](./media/site-recovery-vmm-san/SRSAN_ProviderProxy.png)

7. **등록 키**에서 Azure 사이트 복구를 다운로드하고 VMM 서버에 복사했다는 것을 선택합니다.
8. 자격 증명 모음에서 VMM 서버를 식별하기 위한 이름을 **서버 이름**에서 지정합니다.

	![서버 등록](./media/site-recovery-vmm-san/SRSAN_ProviderRegKeyServerName.png)

9. **초기 클라우드 메타데이터** 동기화에서 VMM 서버에 있는 모든 클라우드의 메타데이터를 자격 증명 모음과 동기화할 것인지 여부를 선택합니다. 이 작업은 각 서버에서 한 번만 수행해야 합니다. 모든 클라우드를 동기화하지 않는 경우 이 설정을 선택 취소된 상태로 두고 VMM 콘솔의 클라우드 속성에서 각 클라우드를 개별적으로 동기화할 수 있습니다. **데이터 암호화** 옵션은 이 시나리오에서 사용되지 않습니다.

	![서버 등록](./media/site-recovery-vmm-san/SRSAN_ProviderSyncEncrypt.png)

11. 다음 페이지에서 **등록**을 클릭하여 프로세스를 완료합니다. 등록 후에 VMM 서버의 메타데이터가 Azure Site Recovery를 통해 검색됩니다. 서버가 자격 증명 모음의 **서버** 페이지에서 <b>리소스</b> 탭에 표시됩니다. 설치 후에는 VMM 콘솔에서 공급자 설정을 수정합니다.
...
**작업** 탭에서 초기 복제를 비롯하여 보호 사용 작업의 진행 상태를 추적합니다. 보호 완료 작업이 실행된 후에는 가상 컴퓨터가 장애 조치(Failover)를 수행할 준비가 되어 있습니다. 
	![가상 컴퓨터 보호 작업](./media/site-recovery-vmm-san/SRSAN_JobPropertiesTab.png)
...
3.
	![복구 계획 만들기](./media/site-recovery-vmm-san/SRSAN_RPlan.png)
