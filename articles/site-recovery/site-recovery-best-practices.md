...
**공급자/에이전트 연결** | 공급자가 프록시를 통해 사이트 복구 서비스에 연결할 경우 프록시가 사이트 복구 URL에 액세스할 수 있어야 합니다. | 공급자가 프록시를 통해 사이트 복구에 연결할 경우 프록시가 사이트 복구 URL에 액세스할 수 있어야 합니다. | 공급자가 프록시를 통해 사이트 복구에 연결할 경우 프록시가 사이트 복구 URL에 액세스할 수 있어야 합니다.
**인터넷 연결** | VMM 서버 및 Hyper-V 호스트에서 연결합니다. | Hyper-V 호스트에서. | VMM 서버에만 해당.
**네트워크 매핑** | 동일한 Azure 네트워크에서 장애 조치되는 모든 컴퓨터가 자신이 속한 복구 계획에 관계없이 서로 연결할 수 있도록 네트워크 매핑을 설정합니다. 대상 Azure 네트워크에 네트워크 게이트웨이가 있으면 가상 컴퓨터도 온-프레미스 가상 컴퓨터에 연결할 수 있습니다. 동일한 복구 계획에서 장애 조치되는 네트워크 매핑 컴퓨터를 설정하지 않는 경우 연결 가능합니다. | 해당 없음 | 장애 조치(Failover) 후 가상 컴퓨터가 적합한 네트워크에 연결되고 복제본 가상 컴퓨터가 대상 Hyper-V 호스트 서버에 최적으로 배치될 수 있게 네트워크 매핑을 설정합니다. 네트워크 매핑을 구성하지 않으면 장애 조치 후 복제된 컴퓨터가 어떤 VM 네트워크에도 연결되지 않습니다.
**저장소 매핑** | 해당 없음 | 해당 없음 | 장애 조치(Failover) 후 가상 컴퓨터가 저장소에 최적으로 연결되도록 저장소 매핑을 설정할 수 있습니다(기본적으로 복제본 VM은 대상 Hyper-V 서버에 지정된 위치에 저장됨).
**SAN 복제** | 해당 없음 | 해당 없음 | SAN 복제를 통해 두 온-프레미스 VMM 사이트 간을 복제할 경우 기존 SAN 환경을 사용할 수 있습니다. [지원되는 SAN 배열](http://social.technet.microsoft.com/wiki/contents/articles/28317.deploying-azure-site-recovery-with-vmm-and-san-supported-storage-arrays.aspx)을 확인하세요.
**자세한 정보** | [구체적인 배포 필수 조건](site-recovery-vmm-to-azure.md#before-you-start) | [구체적인 배포 필수 조건](site-recovery-hyper-v-site-to-azure.md#before-you-start#before-you-start) | [구체적인 배포 필수 조건](site-recovery-vmm-to-vmm.md#before-you-start)
...
**보호된 컴퓨터** | VMware 가상 컴퓨터 또는 물리적 Windows/Linux 서버가 하나 이상 필요합니다. 배포하는 동안 복제하려는 각 컴퓨터에 모바일 서비스가 설치됩니다. | VMware 가상 컴퓨터 또는 물리적 Windows/Linux 서버가 하나 이상 필요합니다. 배포하는 동안 복제하려는 각 컴퓨터에 통합 에이전트가 설치됩니다.
**연결** | 관리 서버가 프록시를 통해 사이트 복구에 연결할 경우 프록시 서버가 특정 URL에 연결할 수 있어야 합니다. | 구성 서버는 인터넷 액세스가 필요합니다.
**자세한 정보** | [구체적인 배포 필수 조건](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment). | InMage Scout 사용자 설명서를 [다운로드](http://download.microsoft.com/download/E/0/8/E08B3BCE-3631-4CED-8E65-E3E7D252D06D/InMage_Scout_Standard_User_Guide_8.0.1.pdf)합니다.
