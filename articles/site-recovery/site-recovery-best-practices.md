<properties
	pageTitle="Azure Site Recovery 배포 준비"
...
**Hyper-V** | Windows Server 2012 R2 이상을 실행하는 온-프레미스 데이터센터에서 하나 이상의 Hyper-V 호스트 서버 | 최신 업데이트가 설치되고 인터넷에 연결된 Windows Server 2012 이상을 실행하는 원본 및 대상 사이트에서 하나 이상의 Hyper-V 서버<br/><br/> Hyper-V 서버는 VMM 클라우드의 호스트 그룹에 있어야 합니다. | 최신 업데이트가 설치되고 인터넷에 연결된 Windows Server 2012 이상을 실행하는 원본 및 대상 사이트에서 하나 이상의 Hyper-V 서버<br/><br/> Hyper-V 서버는 VMM 클라우드의 호스트 그룹에 있어야 합니다. | 
**가상 컴퓨터** | 원본 Hyper-V 호스트 서버에서 하나 이상의 VM | 원본 VMM 클라우드의 Hyper-V 호스트 서버에서 하나 이상의 VM | 원본 VMM 클라우드의 Hyper-V 호스트 서버에서 하나 이상의 VM. | Azure에 복제하는 VM은 [Azure 가상 컴퓨터 필수 조건](site-recovery-best-practices/#virtual-machines)에 부합해야 합니다.
...
VMware 하이퍼바이저 운영 체제 | 지원되는 운영 체제 실행 | [세부 정보](site-recovery-vmware-to-azure/#before-you-start) 
게스트 운영 체제 | Hyper-V와 Azure 간 복제의 경우 사이트 복구에서는 [Azure에서 지원하는](https://technet.microsoft.com/library/cc794868%28v=ws.10%29.aspx) 모든 운영 체제를 지원합니다. <br/><br/> VMware와 물리적 서버 간 복제의 경우 Windows 및 Linux [필수 구성 요소](site-recovery-vmware-to-azure/#before-you-start)를 확인하세요. | 지원되지 않는 경우 필수 구성 요소 확인이 실패합니다. 
...
- **용량 계획**: Hyper-V VM을 복제하는 경우 [용량 플래너](http://www.microsoft.com/en-in/download/details.aspx?id=39057)를 확인하세요.
