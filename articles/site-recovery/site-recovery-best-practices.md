...
Site Recovery는 온-프레미스 물리적 서버와 가상 컴퓨터를 클라우드(Azure) 또는 보조 데이터센터에 복제하는 것을 오케스트레이션하여 BCDR(비즈니스 연속성 및 재해 복구) 전략에 기여하는 Azure 서비스입니다. 기본 위치에서 중단이 발생하면 보조 위치로 장애 조치하여 앱과 워크로드를 가용 상태로 유지합니다. 기본 위치가 정상 작업 상태로 돌아오면 다시 기본 위치로 돌아갑니다. [Site Recovery란?](site-recovery-overview.md)에서 자세한 내용을 알아보기

## 배포 모델 선택

Azure는 리소스를 만들고 작업하기 위한 두 가지 [배포 모델](../resource-manager-deployment-model.md)로 Azure Resource Manager 모델과 클래식 서비스 관리 모델을 제공합니다. 또한 Azure에는 두 가지 포털이 있는데, 하나는 클래식 배포 모델을 지원하는 [Azure 클래식 포털](https://manage.windowsazure.com/)이고 다른 하나는 두 가지 배포 모델을 모두 지원하는 [Azure 포털](https://ms.portal.azure.com/)입니다.
...
---|---|---|---|---
**VMware VM에서 Azure로** | VMware VM을 Azure 저장소로 복제 | Azure 포털에서 VM은 클래식 또는 Resource Manager 저장소로 장애 조치할 수 있음<br/><br/> [Azure 포털](site-recovery-vmware-to-azure.md)의 배포는 간소화된 배포 경험 및 다양한 기능 이익을 제공합니다. | Azure 클래식 포털에서는 [향상된 모델](site-recovery-vmware-to-azure-classic.md)을 사용하여 배포하고 클래식 Azure 저장소에 장애 조치할 수 있습니다.<br/><br/> 새 배포에 사용하지 않아야 할 구형 모델도 있습니다. | PowerShell은 현재 지원되지 않습니다.
**Hyper-V VM에서 Azure로** | Hyper-V VM에서 Azure 저장소로. VM은 시스템 센터 VMM 클라우드 내에서 관리되거나 VMM 없는 Hyper-V 호스트에 있을 수 있습니다. | Azure 포털에서 VM은 클래식 또는 Resource Manager 저장소로 장애 조치할 수 있습니다.<br/><br/> Azure 포털은 간소화된 배포 환경을 제공합니다. VMM 클라우드에서 Hyper-V VM 복제에 대해 [자세히 알아봅니다](site-recovery-vmm-to-azure.md). (VMM 없이) Hyper-V VM 복제에 대해 [자세히 알아봅니다](site-recovery-hyper-v-site-to-azure.md).| 클래식 Azure 포털에서 VM을 클래식 Azure 저장소로 장애 조치할 수 있습니다. | PowerShell 배포가 지원됩니다.
**물리적 서버에서 Azure로** | 물리적 Windows/Linux 서버에서 Azure 저장소로 복제 | Azure 포털에서 서버는 클래식 또는 Resource Manager 저장소로 장애 조치할 수 있습니다.<br/><br/> [Azure 포털](site-recovery-vmware-to-azure.md)의 배포는 간소화된 배포 환경 및 다양한 기능 이점을 제공합니다. | Azure 클래식 포털에서는 [향상된 모델](site-recovery-vmware-to-azure-classic.md)을 사용하여 배포하고 클래식 Azure 저장소에 장애 조치할 수 있습니다.<br/><br/> 새 배포에 사용하지 않아야 할 구형 모델도 있습니다. | PowerShell 배포는 현재 지원되지 않습니다.
**VMware VM/물리적 서버에서 보조 사이트로* | VMware VM 또는 물리적 Windows/Linux 서버에서 보조 사이트로 복제합니다. InMage Scout 사용자 설명서를 [자세히 알아보고 다운로드](http://download.microsoft.com/download/E/0/8/E08B3BCE-3631-4CED-8E65-E3E7D252D06D/InMage_Scout_Standard_User_Guide_8.0.1.pdf)합니다. | Azure 포털에서 사용할 수 없음 | 클래식 포털에서는 Site Recovery 자격 증명 모음에서 InMage Scout를 다운로드합니다. | PowerShell 배포는 현재 지원되지 않음
...
게스트 운영 체제 | Hyper-V에서 Azure로 복제: Site Recovery에서는 [Azure에서 지원하는](https://technet.microsoft.com/library/cc794868%28v=ws.10%29.aspx) 모든 운영 체제를 지원합니다. <br/><br/> VMware 및 물리적 서버 복제의 경우: Windows 및 Linux [필수 구성 요소](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) 확인 | 지원되지 않는 경우 필수 구성 요소 확인이 실패합니다.
...
- **복구 계획 제한**: Site Recovery는 수천 개의 가상 컴퓨터로 확장할 수 있습니다. 복구 계획은 다 함께 장애 조치되어야 하는 응용 프로그램에 대한 모델로 설계되었으므로 복구 계획에서 컴퓨터의 수를 50으로 제한할 수 있습니다.
...
- **복제 대역폭**: 복제 대역폭이 부족한 경우 다음에 유의하십시오.
	- **Express 경로**: Site Recovery는 Riverbed와 같은 Azure Express 경로 및 WAN 최적화 프로그램과 함께 작동합니다. [여기서](http://blogs.technet.com/b/virtualization/archive/2014/07/20/expressroute-and-azure-site-recovery.aspx) Express 경로에 대해 더 알아볼 수 있습니다.
	- **복제 트래픽**: Site Recovery는 전체 VHD가 아닌 데이터 블록만 사용하여 스마트 초기 복제를 수행합니다. 복제가 진행 중일 때는 변경 내용만 복제됩니다.
...
- **RTO**: Site Recovery를 통해 예상할 수 있는 복제 시간 개체(RTO)를 측정하려면 테스트 장애 조치를 실행하고 Site Recovery 작업을 검토하여 작업 완료까지 시간이 얼마나 걸리는지 분석하는 것이 좋습니다. Azure에 장애 조치하는 경우 최선의 RTO를 위해 Azure 자동화 및 복구 계획과 통합하여 모든 수동 작업을 자동화하는 것이 좋습니다.
- **RPO**: Azure에 복제하는 경우 Site Recovery는 가까운 동기 복구 지점 목표 (RPO)를 지원 합니다. 이때 데이터 센터와 Azure 사이에서 충분한 대역폭을 가정합니다.
