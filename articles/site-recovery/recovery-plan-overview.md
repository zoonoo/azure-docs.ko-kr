...
이 문서에서는 [Azure 사이트](site-recovery-overview.md)복구 에서 복구 계획에 대한 개요를 제공합니다.

복구 계획은 장애 조치(failover)를 목적으로 컴퓨터를 복구 그룹으로 수집합니다. 복구 계획은 장애 조치(failover)할 수 있는 독립적인 작은 단위를 만들어서 체계적인 복구 프로세스를 정의하는 데 도움이 됩니다. 단위는 일반적으로 사용자 환경에서 앱을 나타냅니다.

- 복구 계획은 컴퓨터가 장애 조치(failover)되는 방식 및 장애 조치(failover) 후 시작되는 순서를 정의합니다.
- 복구 계획은 Azure에 대한 장애 조치(failover)에 사용되지만 Azure에서 장애 복구에 사용할 수 없습니다.
- 보호된 인스턴스를 하나의 복구 계획에 최대 100개까지 추가할 수 있습니다.
- 순서, 지침 및 수행할 작업을 추가하여 계획을 사용자 지정할 수 있습니다.
- 계획을 정의한 후에 장애 조치(failover)를 실행할 수 있습니다.
- 여러 복구 계획에서 컴퓨터를 참조할 수 있으며, 후속 계획은 다른 복구 계획을 사용하여 이전에 배포된 경우 컴퓨터의 배포/시작을 건너뜁니다.
...
- 순서가 지정되면 시퀀싱이 사용됩니다. 작업은 응용 프로그램 복구 RTO를 개선하기 위해 적절하게 병렬로 실행됩니다.
...
## <a name="automate-tasks-in-recovery-plans"></a>복구 계획에서 작업 자동화
...
### <a name="run-a-test-failover-on-recovery-plans"></a>복구 계획에서 테스트 장애 조치 실행
...
- 누락된 내용을 발견한 경우 정리를 트리거한 다음 테스트 장애 조치(failover)를 다시 실행합니다. 
...
- 앱 및 해당 종속성은 자주 변경됩니다. 복구 계획이 최신 상태인지 확인하려면 매 분기마다 각 앱에 대해 테스트 장애 조치(failover)를 실행합니다.
...
2계층 워드프레스 앱에 대한 복구 계획에 대한 클릭 장애 조치(failover)를 보여주는 빠른 예제 비디오를 시청하세요.
...
- 장애 조치(failover)를 [실행합니다.](site-recovery-failover.md) 
