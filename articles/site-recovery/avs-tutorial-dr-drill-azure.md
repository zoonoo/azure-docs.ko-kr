---
title: Azure Site Recovery를 사용하여 Azure VMware Solution에서 Azure로 재해 복구 훈련 실행
description: Azure Site Recovery를 사용하여 Azure VMware Solution 프라이빗 클라우드에서 Azure로 재해 복구 훈련을 실행하는 방법을 알아봅니다.
author: Harsha-CS
manager: rochakm
...
이 문서에서는 [Azure Site Recovery](site-recovery-overview.md) 서비스를 사용하여 Azure에서 Azure VMware Solution VM에 대한 재해 복구 훈련을 실행하는 방법을 설명합니다. 훈련은 데이터 손실 없이 복제 전략의 유효성을 검사합니다.


이는 Azure VMware Solution 머신에 대한 재해 복구를 Azure에 설정하는 방법을 보여주는 시리즈 중 네 번째 자습서입니다.
...
- 테스트 네트워크에는 동일한 IP 주소 클래스와 서브넷 범위를 사용해야 합니다.
...
> [장애 조치(Failover) 실행](avs-tutorial-failover.md)
