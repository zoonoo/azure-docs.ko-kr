...
이 문서에서는 사이트 복구 붙정 된 모니터링을 사용 하 여 Azure [사이트 복구를](site-recovery-overview.md)모니터링 하는 방법을 알아봅니다.  다음을 모니터링할 수 있습니다.

- 사이트 복구에서 복제된 컴퓨터의 상태 및 상태
- 컴퓨터의 장애 조치 상태를 테스트합니다.
- 구성 및 복제에 영향을 미치는 문제 및 오류입니다.
- 온-프레미스 서버와 같은 인프라 구성 요소입니다.


## <a name="before-you-start"></a>시작하기 전에

시작하기 전에 [일반적인 모니터링 질문을](monitoring-common-questions.md) 검토할 수 있습니다.

## <a name="monitor-in-the-dashboard"></a>대시보드의 모니터

1. 볼트에서 **개요를 클릭합니다.** 복구 서비스 대시보드는 볼트에 대한 모든 모니터링 정보를 단일 위치에 통합합니다. 사이트 복구 및 Azure 백업 서비스에 대한 페이지가 있으며 사이트 복구 간에 전환할 수 있습니다.

    ![Site Recovery 대시보드](./media/site-recovery-monitor-and-troubleshoot/dashboard.png)

2. 대시보드에서 다른 영역으로 드릴다운합니다. 

    ![Site Recovery 대시보드](./media/site-recovery-monitor-and-troubleshoot/site-recovery-overview-page.png).

3. **복제된 항목에서** **모두 보기를** 클릭하여 볼트의 모든 서버를 확인합니다.
4. 드릴다운하려면 각 섹션의 상태 세부 정보를 클릭합니다.
5. **인프라 보기에서**복제하는 컴퓨터 유형별로 모니터링 정보를 정렬합니다.

## <a name="monitor-replicated-items"></a>복제된 항목 모니터링

**복제된 항목에서**복제가 활성화된 볼트의 모든 컴퓨터의 상태를 모니터링합니다.
...
**장애 조치 테스트 성공에서**볼트의 컴퓨터에 대한 장애 조치 상태를 모니터링합니다.

- 적어도 6개월에 한 번 복제된 컴퓨터에서 테스트 장애 조치(Failover)를 실행하는 것이 좋습니다. 프로덕션 환경을 방해하지 않고 장애 조치(failover)가 예상대로 작동하는지 확인하는 방법입니다. 
...
**구성 문제에서**장애 조치(failover)에 영향을 미칠 수 있는 문제를 모니터링합니다.
...
- 세부 정보를 보려면 링크를 클릭합니다. 특정 컴퓨터에 영향을 미치는 문제의 경우 **대상 구성** 열에서 **주의가 필요합니다.** 세부 정보에는 업데이트 적용 권장 사항이 포함됩니다.
...
## <a name="monitor-errors"></a>오류 모니터링

**오류 요약에서는**볼트의 서버 복제에 영향을 미칠 수 있는 현재 활성 오류 증상을 모니터링하고 영향을 받은 컴퓨터 수를 모니터링합니다.

- 온-프레미스 인프라 구성 요소에 영향을 미치는 오류는 섹션의 시작 부분에 표시됩니다. 예를 들어 온-프레미스 구성 서버 또는 Hyper-V 호스트에서 Azure 사이트 복구 공급자에서 하트비트를 수신하지 않습니다.
- 그런 다음, 복제된 서버에 영향을 주는 복제 오류 증상이 표시됩니다.
- 표 항목은 오류 심각도의 내림차순으로 정렬된 다음, 영향을 받는 컴퓨터 수의 내림차순으로 정렬됩니다.
- 영향을 받는 서버 수는 단일 기본 문제가 여러 컴퓨터에 영향을 미칠 수 있는지 여부를 이해하는 유용한 방법입니다. 예를 들어 네트워크 결함은 Azure로 복제되는 모든 컴퓨터에 잠재적으로 영향을 줄 수 있습니다. 
...
**인프라 보기에서는**복제와 관련된 인프라 구성 요소및 서버와 Azure 서비스 간의 연결 상태를 모니터링합니다.
...
- 인프라 보기를 사용하려면 사용자 환경에서 적절한 복제 시나리오를 선택합니다. 자세한 내용을 보려면 뷰를 드릴다운합니다. 다음 표에서는 표시되는 시나리오를 보여 줍니다.
    ...
    **시나리오** | **상태**  | **보기 사용 가능 여부**
    --- |--- | ---
    **온-프레미스 사이트 간 복제** | 모든 상태 | 예 
    **Azure 지역 간 Azure VM 복제**  | 복제가 사용되도록 설정됨/초기 복제 진행 중 | yes
    **Azure 지역 간 Azure VM 복제** | 장애 조치(Failover)/장애 복구(Failback)됨 | 예   
    **Azure로 VMware 복제** | 복제가 사용되도록 설정됨/초기 복제 진행 중 | yes     
    **Azure로 VMware 복제** | 장애 조치(Failover)/장애 복구(Failback)됨 | 예      
    **Azure로 Hyper-V 복제** | 장애 조치(Failover)/장애 복구(Failback)됨 | 예
...
**복구 계획에서**계획 수를 모니터링하고, 새 계획을 만들고, 기존 계획을 수정합니다.  

## <a name="monitor-jobs"></a>작업 모니터링

**작업에서**사이트 복구 작업의 상태를 모니터링합니다.
...
**복제된 항목에서**복제된 컴퓨터 목록을 가져옵니다. 
    ![Site Recovery 복제된 항목 보기](./media/site-recovery-monitor-and-troubleshoot/site-recovery-virtual-machine-list-view.png)

2. 정보를 보고 필터링할 수 있습니다. 맨 위에 있는 작업 메뉴에서 특정 컴퓨터에 대해 테스트 장애 조치(Failover) 실행 또는 특정 오류 보기 등의 작업을 수행할 수 있습니다.
...
자격 증명 모음 > **모니터링** 섹션에서 **사이트 복구 이벤트를**클릭합니다.
1. **이메일 알림**을 클릭합니다.
...
Azure 모니터를 사용 하 여 사이트 복구 [모니터링에 대해 자세히 알아봅니다.](monitor-log-analytics.md)
