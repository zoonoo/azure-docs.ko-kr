...
6. 컴퓨터를 클릭하여 자세한 내용을 드릴합니다. 세부 정보는 다음과 같습니다. **복제 정보**: 컴퓨터의 현재 상태입니다.
        - **RPO**(복구 지점 목표): 가상 머신에 대한 현재 RPO 및 RPO가 마지막으로 계산된 시간입니다.
        - **복구 지점**: 컴퓨터에 대해 사용 가능한 최신 복구 지점입니다.
        - **장애 조치(Failover) 준비**: 컴퓨터에 대해 테스트 장애 조치(Failover)가 실행되었는지 여부, 컴퓨터에서 실행 중인 에이전트 버전, 구성 문제를 나타냅니다.
        - **오류**: 현재 컴퓨터에서 관찰되는 복제 오류 증상과 가능한 원인/작업 목록입니다.
        - **이벤트**: 컴퓨터에 영향을 주는 최근 이벤트가 시간순으로 나열된 목록입니다. 이벤트는 컴퓨터에 영향을 미친 문제에 대한 기록이지만, 오류 세부 정보는 현재 관찰 가능한 오류 증상을 보여 줍니다.
