...
이 문서에서는 Site Recovery 기본 제공 모니터링을 사용하여 Azure [Site Recovery](site-recovery-overview.md)를 모니터링하는 방법을 알아봅니다.  다음을 모니터링할 수 있습니다.
...
시작하기 전에 [일반적인 모니터링 질문](monitoring-common-questions.md)을 검토할 수 있습니다.

## <a name="monitor-in-the-dashboard"></a>대시보드에서 모니터링

1. 자격 증명 모음에서 **개요** 를 클릭합니다. Recovery Services 대시보드는 자격 증명 모음에 대한 모든 모니터링 정보를 단일 위치에 통합합니다. Site Recovery와 Azure Backup 서비스에 대한 페이지가 모두 있으며 두 페이지 간에 전환할 수 있습니다.

    ![Site Recovery 대시보드](./media/site-recovery-monitor-and-troubleshoot/dashboard.png)

2. 이 대시보드에서 다른 영역으로 드릴다운합니다. 

    ![드릴다운할 수 있는 대시보드의 영역을 보여 주는 스크린샷.](./media/site-recovery-monitor-and-troubleshoot/site-recovery-overview-page.png).

3. **복제된 항목** 에서 **모두 보기** 를 클릭하여 자격 증명 모음에 있는 모든 서버를 확인합니다.
4. 각 섹션에서 상태 세부 정보를 클릭하여 드릴다운합니다.
5. **인프라 보기** 에서 복제하려는 컴퓨터의 유형별로 모니터링 정보를 정렬합니다.

## <a name="monitor-replicated-items"></a>복제된 항목 모니터링

**복제된 항목** 에서 복제를 사용하도록 설정된 자격 증명 모음에 있는 모든 컴퓨터의 상태를 모니터링합니다.
...
**장애 조치(Failover) 테스트 성공** 에서 자격 증명 모음에 있는 컴퓨터의 장애 조치 (failover) 상태를 모니터링합니다.
...
**구성 문제** 에서 장애 조치(failover)를 성공적으로 수행하는 데 영향을 미칠 수 있는 문제를 모니터링합니다.
...
**오류 요약** 에서 자격 증명 모음의 서버 복제에 영향을 줄 수 있는 현재 활성 상태의 오류 증상과 영향을 받는 컴퓨터의 수를 모니터링합니다.
...
**인프라 보기** 에서 복제에 관련된 인프라 구성 요소 및 서버와 Azure 서비스 간의 연결 상태를 모니터링합니다.
...
**복구 계획** 에서 계획의 수를 모니터링하고, 새 계획을 만들고, 기존 계획을 수정합니다.  
...
자격 증명 모음 > **모니터링** 섹션에서 **Site Recovery 이벤트** 를 클릭합니다.
1. **메일 알림** 을 클릭합니다.
...
Azure Monitor로 Site Recovery를 모니터링하는 방법을 [알아봅니다](monitor-log-analytics.md).
