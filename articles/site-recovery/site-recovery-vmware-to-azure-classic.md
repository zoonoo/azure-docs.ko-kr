...
3. **시작하기 전**에서 **구성 서버 및 프로세스 서버 설치**를 선택합니다. 배포 크기에 따라 나중에 추가 프로세스 서버가 필요할 수 있지만 처음으로 이 배포를 설치할 때는 필요하지 않습니다.
...
>[AZURE.WARNING] **글로벌 시간 동기화** 필수 구성 요소 검사에 대한 경고가 표시되는 경우 시스템 클록의 시간이 표준 시간대와 동일한지 확인합니다.
...
10. **설치 위치**에서 이전 파일을 설치하고 캐시를 저장할 위치를 선택합니다. 캐시 드라이브는 600GB 이상의 여유 공간이 있는 것이 좋습니다.
...
3. 다음 설정을 수정할 수 있습니다.
	...
	-  **Azure VM 크기**: 네트워크 어댑터 수가 대상 가상 컴퓨터에 대해 지정하는 크기에 따라 결정됩니다. 크기 및 어댑터에 대해 [자세히 알아봅니다](../virtual-machines/virtual-machines-linux-sizes.md/#size-tables). 다음 사항에 유의하세요.
...
이를 설정하는 방법에 대해 [자세히 알아봅니다](http://social.technet.microsoft.com/wiki/contents/articles/31666.troubleshooting-remote-desktop-connection-after-failover-using-asr.aspx).
...
2. **시작하기 전**에서 **배포 규모 확장을 위해 추가 프로세스 서버 추가**를 선택합니다.
