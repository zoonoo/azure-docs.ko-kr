...
**모범 사례** | **세부 정보**
--- |---
**Usage** | 구성 서버/독립 실행형 프로세스 서버가 목적 으로만 사용 되는지 확인 합니다. 컴퓨터에서 다른 항목을 실행 하지 마세요.
...
![경고][yellow]  | 최근 15 분 동안 CPU 사용률이 80% >. | 1. 새 컴퓨터를 추가 하지 마세요.<br/>2. 프로세스 서버를 사용 하는 Vm 수가 [정의 된 제한](site-recovery-plan-capacity-vmware.md#capacity-considerations)에 부합 하는지 확인 하 고 [추가 프로세스 서버](vmware-azure-set-up-process-server-scale.md)를 설정 하는 것이 좋습니다.<br/>3. [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하려면 아래 지침을 따르세요.
![심각][red] |  최근 15 분 동안 CPU 사용률이 95% >. | 1. 새 컴퓨터를 추가 하지 마세요.<br/>2. 프로세스 서버를 사용 하는 Vm 수가 [정의 된 제한](site-recovery-plan-capacity-vmware.md#capacity-considerations)에 부합 하는지 확인 하 고 [추가 프로세스 서버](vmware-azure-set-up-process-server-scale.md)를 설정 하는 것이 좋습니다.<br/>3. [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하려면 아래 지침을 따르세요.<br/> 4. 문제가 지속 되 면 VMware/물리적 서버 복제에 대 한 [Deployment Planner](https://aka.ms/asr-v2a-deployment-planner) 를 실행 합니다.
![경고][yellow] | 최근 15 분 동안 메모리 사용 > 80%입니다. |  1. 새 컴퓨터를 추가 하지 마세요.<br/>2. 프로세스 서버를 사용 하는 Vm 수가 [정의 된 제한](site-recovery-plan-capacity-vmware.md#capacity-considerations)에 부합 하는지 확인 하 고 [추가 프로세스 서버](vmware-azure-set-up-process-server-scale.md)를 설정 하는 것이 좋습니다.<br/>3. 경고와 관련 된 지침을 따르세요.<br/> 4. 문제가 지속 되 면 아래 지침에 따라 [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하십시오.
![심각][red] | 최근 15 분 동안 메모리 사용 > 95%입니다. | 1. 새 컴퓨터를 추가 하거나 추가 [프로세스 서버](vmware-azure-set-up-process-server-scale.md)를 설정 하는 것을 고려 하지 않습니다.<br/> 2. 경고와 관련 된 지침을 따르세요.<br/> 3. 4. 문제가 계속 되 면 아래 지침에 따라 [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하십시오.<br/> 4. 문제가 지속 되 면 VMware/물리적 서버 복제 문제에 대 한 [Deployment Planner](https://aka.ms/asr-v2a-deployment-planner) 를 실행 합니다.
![경고][yellow] | 마지막 15 분 동안 캐시 폴더의 사용 가능한 공간 < 30%입니다. | 1. 새 컴퓨터를 추가 하지 말고 [추가 프로세스 서버](vmware-azure-set-up-process-server-scale.md)를 설정 하는 것이 좋습니다.<br/>2. 프로세스 서버를 사용 하는 Vm 수가 [지침](site-recovery-plan-capacity-vmware.md#capacity-considerations)에 부합 하는지 확인 합니다.<br/> 3. [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하려면 아래 지침을 따르세요.
![심각][red] |  최근 15 분 동안 사용 가능한 공간 < 25% | 1. 이 문제에 대 한 경고와 관련 된 지침을 따르세요.<br/> 2. 3. [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하려면 아래 지침을 따르세요.<br/> 3. 문제가 지속 되 면 VMware/물리적 서버 복제에 대 한 [Deployment Planner](https://aka.ms/asr-v2a-deployment-planner) 를 실행 합니다.
![심각][red] | 15 분 이상 동안 프로세스 서버에서 하트 비트가 없습니다. Tmansvs 서비스가 구성 서버와 통신 하지 않습니다. | 1) 프로세스 서버가 실행 중인지 확인 합니다.<br/> 2. 프로세스 서버에서 tmassvc가 실행 되 고 있는지 확인 합니다.<br/> 3. [연결 및 복제 문제를 해결](#check-connectivity-and-replication)하려면 아래 지침을 따르세요.
...
## <a name="step-3-check-the-process-server-heartbeat"></a>3단계: 프로세스 서버 하트 비트 확인
...
## <a name="step-4-verify-time-sync-on-source-machine"></a>4단계: 원본 컴퓨터에서 시간 동기화 확인
...
## <a name="step-5-check-anti-virus-software-on-source-machine"></a>5단계: 원본 컴퓨터의 바이러스 백신 소프트웨어를 확인 합니다.
...
**연결** | **세부 정보** | **작업**
--- | --- | ---
**Successful** | 텔넷은 빈 화면을 표시 하 고 프로세스 서버에 연결할 수 있습니다. | 추가 작업이 필요 하지 않습니다.
...
1. 웹 브라우저에서 IP 주소 https://< PS_IP >: < PS_Data_Port >/에 도달 하려고 합니다.
...
## <a name="step-8-check-whether-the-process-server-is-pushing-data"></a>8단계: 프로세스 서버에서 데이터를 푸시하는 지 확인
...
## <a name="step-9-check-the-process-server-connection-to-azure-blob-storage"></a>9단계: Azure blob 저장소에 대 한 프로세스 서버 연결 확인
...
2. Azure Site Recovery 에이전트가 데이터를 보내기 위해 동일한 설정을 사용 해야 합니다.
    ...
    b) **Microsoft Azure Backup**을 열고 **동작** > **변경 속성**을 선택 합니다.
