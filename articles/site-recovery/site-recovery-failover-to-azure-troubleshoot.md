...
1. 가상 **컴퓨터** > **네트워킹으로**이동, 필요한 네트워크 인터페이스의 이름을 클릭합니다.  ![네트워크 인터페이스](media/site-recovery-failover-to-azure-troubleshoot/network-interface.PNG)
...
4. 필수 **설정** > 구성을 클릭하여 새 을**만듭니다.** ![새로 만들기](media/site-recovery-failover-to-azure-troubleshoot/Create-New-Public-IP.png)
...
## <a name="unable-to-open-serial-console-after-failover-of-a-uefi-based-machine-into-azure"></a>UEFI 기반 컴퓨터의 장애 조치 후 Azure로 직렬 콘솔을 열 수 없습니다.

RDP를 사용하여 컴퓨터에 연결할 수 있지만 직렬 콘솔을 열 수 없는 경우 다음 단계를 따르십시오.

* 컴퓨터 OS가 Red Hat 또는 Oracle Linux 7.*/8.0인 경우 루트 권한이 있는 장애 조치 Azure VM에서 다음 명령을 실행합니다. 명령 후 VM을 재부팅합니다.

        grub2-mkconfig -o /boot/efi/EFI/redhat/grub.cfg

* 컴퓨터 OS가 CentOS 7.*인 경우 루트 사용 권한이 있는 장애 조치 Azure VM에서 다음 명령을 실행합니다. 명령 후 VM을 재부팅합니다.
...
일반적으로 문제를 유발하지는 않으며, 계획되지 않은 장애 조치(failover)의 경우 무시할 수 있습니다. 장애 조치(failover)가 계획된 경우 장애 조치 전에 VM이 제대로 종료되었는지 확인하고 보류 중인 복제 데이터를 온-프레미스에서 Azure로 보낼 수 있는 충분한 시간을 제공합니다. 그런 다음, [장애 조치(failover) 화면](site-recovery-failover.md#run-a-failover)의 **최신** 옵션을 사용하여 Azure에서 보류 중인 데이터가 복구 지점으로 처리되어 VM 장애 조치(failover)에 사용되도록 합니다.

## <a name="unable-to-select-the-datastore"></a>데이터 스토어를 선택할 수 없음
...
원본 컴퓨터를 관리하는 vCenter에서 마스터 대상을 수동으로 만듭니다. 데이터스토어는 다음 vCenter 검색 및 패브릭 새로 고침 작업 후에 사용할 수 있습니다.
...
1. 구성 서버 VM에서 명령 프롬프트를 열고 다음 명령을 사용하여 프록시 설정을 확인합니다.
    ...
    고양이 /etc/환경 에코 $http_프록시 에코 $https_proxy 

2. 이전 명령의 출력에 http_proxy 또는 https_proxy 설정이 정의된 것으로 나타나면 다음 방법 중 하나를 사용하여 구성 서버와의 마스터 대상 통신차단을 해제합니다.
   
   - [PsExec 도구를 다운로드하십시오.](https://aka.ms/PsExec)
   - 이 도구를 사용하여 시스템 사용자 컨텍스트에 액세스하고 프록시 주소가 구성되었는지 확인합니다. 
   - 프록시가 구성된 경우 PsExec 도구를 사용하여 시스템 사용자 컨텍스트에서 IE를 엽니다.
  
     **psexec -s -i "%programfiles%\인터넷 익스플로러\iexplore.exe"**

   - 마스터 대상 서버가 구성 서버와 통신할 수 있도록 하려면 다음을 수행합니다.
  
     - Internet Explorer의 프록시 설정을 수정하여 프록시를 통해 마스터 대상 서버 IP 주소를 우회합니다.   
     또는
     - 마스터 대상 서버에서 프록시를 비활성화합니다. 


## <a name="next-steps"></a>다음 단계
- Windows [VM에 대한 RDP 연결](../virtual-machines/windows/troubleshoot-rdp-connection.md) 문제 해결
- [리눅스 VM에 대한 SSH 연결](../virtual-machines/linux/detailed-troubleshoot-ssh-connection.md) 문제 해결
