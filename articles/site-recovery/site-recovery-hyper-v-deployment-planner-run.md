...
description: "이 문서는 Hyper-V에서 Azure로 시나리오에 Azure Site Recovery Deployment Planner를 실행하는 모드를 설명합니다."
...
1.  [VM 목록 가져오기](#get-vm-list-for-profiling-hyper-v-vms)
2.  [프로파일링](#profile-hyper-v-vms)
3.  [보고서 생성](#generate-report)
4.  [처리량 가져오기](#get-throughput)
...
## <a name="get-vm-list-for-profiling-hyper-v-vms"></a>Hyper-V VM을 프로파일링하기 위해 VM 목록 가져오기
...
|-ServerListFile | 프로파일링할 VM이 포함된 서버 목록이 있는 파일입니다. 파일 경로는 절대 경로 또는 상대 경로일 수 있습니다. 이 파일의 각 행에 다음 중 하나를 포함해야 합니다.<ul><li>Hyper-V 호스트 이름 또는 IP 주소</li><li>Hyper-V 클러스터 이름 또는 IP 주소</li></ul><br>예를 들어 "ServerList.txt" 파일은 다음 서버를 포함합니다.<ul><li>Host_1</li><li>10.8.59.27</li><li>Cluster_1</li><li>Host_2</li>|
| -Directory|(선택 사항) 이 작업 중에 생성된 데이터를 저장하기 위한 UNC(범용 명명 규칙) 또는 로컬 디렉터리 경로입니다. 지정하지 않으면 현재 경로 아래에 'ProfiledData'라는 디렉터리가 기본 디렉터리로 사용됩니다.|
|-OutputFile| (선택 사항) 지정된 Hyper-V 서버에서 가져온 VM 목록이 저장된 파일입니다. 이름이 언급되지 않으면 세부 정보는 "VMList.txt"에 저장됩니다.  프로파일링할 필요가 없는 VM을 제거한 후 파일을 사용하여 프로파일링을 시작합니다.|
|-Password|(선택 사항) Hyper-V 호스트에 연결할 암호입니다.   매개 변수로 암호를 지정하지 않으면 나중에 명령을 실행할 때 지정하도록 요구하는 메시지가 표시됩니다.|

### <a name="how-does-getvmlist-discovery-work"></a>GetVMList 검색은 어떻게 작동하나요?
**Hyper-V 클러스터**: Hyper-V 클러스터 이름이 서버 목록 파일에 주어지면 도구가 클러스터의 Hyper-V 노드를 모두 찾아서 각 Hyper-V 호스트에 있는 VM을 가져옵니다.

**Hyper-V 호스트**: Hyper-V 호스트 이름이 주어지면 도구는 우선 해당 이름이 클러스터에 속하는지 확인합니다. 그러한 경우 클러스터에 속하는 노드를 페치합니다. 그런 다음 각 Hyper-V 호스트에서 VM을 가져옵니다. 

수동으로 프로파일링하려는 VM의 이름이나 IP 주소를 파일에 나열하도록 선택할 수도 있습니다.

메모장에서 출력 파일을 연 다음 다른 파일에 프로파일링하려는 모든 VM의 이름(예를 들어 ProfileVMList.txt)을 한 줄에 한 개씩 복사합니다. 이 파일은 프로파일링, 보고서 생성 및 처리량 가져오기와 같은 다른 모든 작업에 대해 도구의 -VMListFile 매개 변수에 대한 입력으로 사용됩니다.

### <a name="example-1-to-store-the-list-of-vms-in-a-file"></a>예제 1: VM 목록을 파일에 저장하려면
```
...
### <a name="example-2-to-store-the-list-of-vms-at-the-default-location-ie--directory-path"></a>예제 2: VM 목록을 기본 위치(예: 디렉터리 경로)에 저장하려면
```
...
* 이 도구는 15초마다 한 번씩 Hyper-V 호스트를 쿼리하여 프로파일링의 정확성을 확인하고 분 단위 성능 카운터 데이터의 평균을 저장합니다.
* 이 도구는 클러스터의 한 노드에서 다른 노드로 VM 마이그레이션과 호스트 내의 저장소 마이그레이션을 원활하게 처리합니다.

### <a name="get-vm-list-to-profile"></a>프로파일링할 VM 목록 확보하기
프로파일링할 VM 목록을 만들려면 [GetVMList](#get-vm-list-for-profiling-hyper-v-vms) 작업을 참조하세요.

프로파일링할 VM 목록이 있으면 프로파일링 모드에서 도구를 실행할 수 있습니다. 

### <a name="command-line-parameters"></a>명령줄 매개 변수
다음 테이블은 프로파일링 모드에서 실행할 도구의 필수 매개 변수 및 선택적 매개 변수 목록을 포함합니다. 이 도구 VMware에서 Azure 및 Hyper-V에서 Azure로 시나리오에 공통적으로 사용됩니다. 다음 매개 변수는 Hyper-V에 적용할 수 있습니다. 
```
...
#### <a name="example-3-profile-vms-for-60-minutes-for-a-quick-test-of-the-tool"></a>예제 3: 도구에 대한 빠른 테스트를 위해 60분 동안 VM 프로파일링
```
...
#### <a name="example-4-profile-vms-for-2-hours-for-a-proof-of-concept"></a>예제 4: 개념 증명을 위해 2시간 동안 VM 프로파일링
```
...
### <a name="command-line-parameters"></a>명령줄 매개 변수
다음 표는 보고서 생성 모드에서 실행할 필수 및 선택적 도구 매개 변수의 목록을 포함하고 있습니다. 이 도구 VMware에서 Azure 및 Hyper-V에서 Azure로 시나리오에 공통적으로 사용됩니다. 다음 매개 변수는 Hyper-V에 적용할 수 있습니다.
```
...
| -User | (선택 사항) Hyper-V 호스트 또는 Hyper-V 클러스터에 연결할 사용자 이름입니다. 사용자에게 관리 액세스 권한이 필요합니다.<br>사용자 및 암호는 보고서에 사용할 디스크 수, 코어 수, NIC 수 등과 같은 VM의 최신 구성 정보를 가져오는 데 사용됩니다. 제공되지 않으면 프로파일링 중에 수집된 구성 정보가 사용됩니다.|
|-Password|(선택 사항) Hyper-V 호스트에 연결할 암호입니다. 지금 지정하지 않으면 나중에 명령을 실행할 때 지정하라는 메시지가 나타납니다.|
| -DesiredRPO | (선택 사항) 원하는 복구 지점 목표(분)입니다. 기본값은 15분입니다.|
| -Bandwidth | (선택 사항) 대역폭(Mbps)입니다. 지정된 대역폭에서 달성할 수 있는 RPO를 계산하는 데 사용하는 매개 변수입니다. |
...
|-TargetRegion|(선택 사항) 복제 대상이 되는 Azure 지역입니다. Azure는 지역마다 비용이 다르기 때문에 특정 대상 Azure 지역으로 보고서를 생성하려면 이 매개 변수를 사용합니다.<br>기본값은 WestUS2 또는 마지막으로 사용된 대상 지역입니다.<br>[지원되는 대상 지역](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-target-regions) 목록을 참조하세요.|
|-OfferId|(선택 사항) 주어진 구독과 연결된 제품입니다. 기본값은 MS-AZR-0003P(종량제)입니다.|
|-Currency|(선택 사항) 생성된 보고서의 비용 표시에 사용되는 통화입니다. 기본값은 미국 달러($) 또는 마지막 사용한 통화입니다.<br>[지원되는 통화](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-currencies) 목록을 참조하세요.|
...
#### <a name="example-3-generate-a-report-with-a-specific-bandwidth-that-you-will-be-provision-for-the-replication"></a>예제 3: 복제를 위해 프로비전할 특정 대역폭으로 보고서 생성
```
...
#### <a name="example-6-generate-a-report-for-5-minutes-rpo"></a>예제 6: 5분 동안의 RPO 보고서 생성
```
...
#### <a name="example-7-generate-a-report-for-south-india-azure-region-with-indian-rupee-and-specific-offer-id"></a>예제 7: 인도 루피 및 특정 제품 ID로 인도 남부 Azure 지역에 대한 보고서 생성
```
...
95번째 백분위수 값을 사용하면 실제 워크로드 특성을 정확히 보여 주며, Azure에서 워크로드를 실행할 때 최상의 성능을 제공합니다. 당사는 사용자가 이 숫자를 변경해야 한다고 예상하지 않습니다. 값을 변경(예를 들어 90번째 백분위수로)하지 않은 경우 기본 폴더의 구성 파일인 ASRDeploymentPlanner.exe.config를 업데이트하고 저장하여 기존 프로파일링된 데이터에 대한 새 보고서를 생성할 수 있습니다.
```
...
* 그 결과 VM에서 변동이 증가하므로 Azure Site Recovery 복제가 일어날 수 있도록 VM을 프리미엄 저장소로 이동해야 합니다.
* 따라서 프리미엄 저장소 계정에 대한 보호를 해제하고 다시 사용하도록 설정해야 합니다.

배포 계획 중에 그리고 기본값이 30%일 때 증가를 계획하는 것이 좋습니다. 사용자는 응용 프로그램 사용 패턴과 증가 예측에 대한 전문가이며 보고서를 생성하는 동안 이 숫자를 해당 계획에 따라 변경할 수 있습니다. 또한 사용자는 여러 증가율 및 동일한 프로파일링된 데이터가 포함된 여러 보고서를 생성하고 가장 적합한 대상 저장소 및 원본 대역폭 권장 사항을 결정할 수 있습니다. 

생성된 Microsoft Excel 보고서에는 다음과 같은 정보가 포함되어 있습니다.

* [온-프레미스 요약](site-recovery-hyper-v-deployment-planner-analyze-report.md#on-premises-summary)
* [권장 사항](site-recovery-hyper-v-deployment-planner-analyze-report.md#recommendations)
* [VM<->저장소 배치](site-recovery-hyper-v-deployment-planner-analyze-report.md#vm-storage-placement-recommendation)]
* [호환되는 VM](site-recovery-hyper-v-deployment-planner-analyze-report.md#compatible-vms)
* [호환되지 않는 VM](site-recovery-hyper-v-deployment-planner-analyze-report.md#incompatible-vms)
* [온-프레미스 Storage 요구 사항](site-recovery-hyper-v-deployment-planner-analyze-report.md#on-premises-storage-requirement)
* [IR 일괄 처리](site-recovery-hyper-v-deployment-planner-analyze-report.md#initial-replication-batching)
* [비용 예측](site-recovery-hyper-v-deployment-planner-cost-estimation.md)

![Deployment Planner 보고서](media/site-recovery-hyper-v-deployment-planner-run/deployment-planner-report-h2a.png)
...
>
>  1. Azure Site Recovery 처리량을 제한하고 있는 네트워크 QoS(서비스 품질)가 있는지 확인합니다.
>
...
* [생성된 보고서 분석](site-recovery-hyper-v-deployment-planner-analyze-report.md)