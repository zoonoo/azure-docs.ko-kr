...
description: "이 문서에서는 Azure Site Recovery 서비스를 사용하여 Azure에 대한 VMware 재해 복구를 위해 기존 구성 서버를 관리하는 방법을 설명합니다."
...
Azure에 대한 VMware VM와 물리적 서버 재해 복구를 위해 [Azure Site Recovery](site-recovery-overview.md) 서비스를 사용할 경우 온-프레미스 구성 서버를 설정합니다. 구성 서버는 온-프레미스 VMware와 Azure 간의 통신을 조정하여 데이터 복제를 관리합니다. 이 문서에서는 배포된 구성 서버를 관리하기 위한 일반적인 작업을 요약합니다.

## <a name="modify-vmware-settings"></a>VMware 설정 수정

구성 서버가 연결하는 VMware 서버에 대한 설정을 수정합니다.

1. 구성 서버를 실행하는 컴퓨터에 로그온합니다.
2. 바탕 화면 바로 가기에서 Azure Site Recovery 구성 관리자를 시작합니다. 또는 **https://configuration-server-name/IP:44315**를 엽니다.
3. **vCenter Server/vSPhere ESXi Server 관리**를 클릭합니다.
    - 다른 VMware 서버를 구성 서버에 연결하려면 **vCenter Server/vSphere ESXi Server 추가**를 클릭하고 서버 세부 정보를 지정합니다.
    - VMware VM의 자동 검색을 위해 VMware 서버에 연결하는 데 사용되는 자격 증명을 업데이트하려면 **편집**을 클릭합니다. 새 자격 증명을 지정하고 **확인**을 클릭합니다.

        ![VMware 수정](./media/site-recovery-vmware-to-azure-manage-configuration-server/modify-vmware-server.png)

## <a name="modify-credentials-for-mobility-service-installation"></a>모바일 서비스 설치에 대한 자격 증명 수정

복제에 대해 사용되는 VMware VM에 모바일 서비스를 자동으로 설치하는 데 사용되는 자격 증명을 수정합니다.

1. 구성 서버를 실행하는 컴퓨터에 로그온합니다.
2. 바탕 화면 바로 가기에서 Azure Site Recovery 구성 관리자를 시작합니다. 또는 **https://configuration-server-name/IP:44315**를 엽니다.
3. **가상 머신 자격 증명 관리**를 클릭하고 새 자격 증명을 지정합니다. 그런 후 **확인**을 클릭하여 설정을 업데이트합니다.

    ![모바일 서비스 자격 증명 수정](./media/site-recovery-vmware-to-azure-manage-configuration-server/modify-mobility-credentials.png)

## <a name="modify-proxy-settings"></a>프록시 설정 수정

Azure에 대한 인터넷 액세스를 위해 구성 서버 컴퓨터에서 사용하는 프록시 설정을 수정합니다. 구성 서버 컴퓨터에서 실행 중인 기본 프로세스 서버 외에도, 추가 프로세스 서버 컴퓨터가 있는 경우 두 컴퓨터의 설정을 수정합니다.

1. 구성 서버를 실행하는 컴퓨터에 로그온합니다.
2. 바탕 화면 바로 가기에서 Azure Site Recovery 구성 관리자를 시작합니다. 또는 **https://configuration-server-name/IP:44315**를 엽니다.
3. **연결 관리**를 클릭하고 프록시 값을 업데이트합니다. 그런 후 **저장**을 클릭하여 설정을 업데이트합니다.

## <a name="add-a-network-adapter"></a>네트워크 어댑터 추가

OVF 템플릿은 단일 네트워크 어댑터를 사용하여 구성 서버 VM을 배포합니다. [VM에 추가 어댑터를 추가](how-to-deploy-configuration-server.md#add-an-additional-adapter)할 수 있으나 자격 증명 모음에 구성 서버를 등록하기 전에 이 작업을 수행해야 합니다.

자격 증명 모음에 구성 서버에 등록한 후 어댑터를 추가해야 할 경우, VM 속성에 어댑터를 추가한 다음, 자격 증명 모음에 서버를 다시 등록해야 합니다.
...
필요한 경우 동일한 자격 증명 모음에 구성 서버를 다시 등록할 수 있습니다. 구성 서버 컴퓨터에서 실행 중인 기본 프로세스 서버 외에도, 추가 프로세스 서버 컴퓨터가 있는 경우 두 컴퓨터를 다시 등록합니다.
  ...
  2. **서버**에서 **등록 키 다운로드**를 클릭합니다. 자격 증명 모음 자격 증명 파일이 다운로드됩니다.
  3. 구성 서버 컴퓨터에 로그온합니다.
  4. **%ProgramData%\ASR\home\svagent\bin**에서 **cspsconfigtool.exe**를 엽니다.
  5. **자격 증명 모음 등록** 탭에서 찾아보기를 클릭하고 다운로드한 자격 증명 모음 자격 증명 파일을 찾습니다.
  6. 필요한 경우 프록시 서버 세부 정보를 제공합니다. 그런 후에 **등록**을 클릭합니다.
...
5. 제거하려는 구성 서버를 클릭합니다. 그런 다음, **세부 정보** 페이지에서 **삭제**를 클릭합니다.

    ![구성 서버 삭제](./media/site-recovery-vmware-to-azure-manage-configuration-server/delete-configuration-server.png)
...
Azure에 대한 [VMware VM](tutorial-vmware-to-azure.md) 및 물리적 서버의 재해 복구를 설정하기 위한 자습서(tutorial-physical-to-azure.md)를 검토하세요.
