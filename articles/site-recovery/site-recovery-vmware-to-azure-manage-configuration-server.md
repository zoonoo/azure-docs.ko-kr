...
3. **가상 머신 자격 증명 관리**를 선택하고 새 자격 증명을 입력합니다. 그런 후 **확인**을 선택하여 설정을 업데이트합니다.
    ...
    ![Mobility Service 자격 증명 수정](./media/site-recovery-vmware-to-azure-manage-configuration-server/modify-mobility-credentials.png)
...
OVF(Open Virtualization Format) 템플릿은 단일 네트워크 어댑터를 사용하여 구성 서버 VM을 배포합니다. [VM에 추가 어댑터를 추가](how-to-deploy-configuration-server.md#add-an-additional-adapter)할 수 있으나 자격 증명 모음에 구성 서버를 등록하기 전에 추가해야 합니다.
...
Azure에 대한 [VMware VM](tutorial-vmware-to-azure.md)의 재해 복구를 설정하기 위한 자습서를 검토하세요.
