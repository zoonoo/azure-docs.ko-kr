...
1. **모든 서비스** > **Recovery Services 자격 증명 모음**에서 이전 자습서에서 준비한 자격 증명 모음의 이름(**ContosoVMVault**)을 클릭합니다.
...
6. **System Center VMM을 사용하고 있습니까?**에서 **아니요**를 선택합니다. 그런 후 **OK**를 클릭합니다.

    ![복제 목표](./media/tutorial-hyper-v-to-azure/replication-goal.png)
...
2. **+Hyper-V 사이트**를 클릭하고 이전 자습서에서 만든 사이트의 이름 **ContosoHyperVSite**를 지정합니다.
3. **+Hyper-V Server**를 클릭합니다.
4. Provider 설치 파일을 다운로드합니다.
5. 자격 증명 모음 등록 키를 다운로드합니다. 공급자 설정을 실행할 때 이 키가 필요합니다. 이 키는 생성된 날로부터 5일간 유효합니다.

    ![Provider 다운로드](./media/tutorial-hyper-v-to-azure/download.png)
...
Azure Site Recovery에서 Hyper-V 서버의 메타데이터가 검색되며 서버가 **Site Recovery 인프라** > **Hyper-V 호스트**에 표시됩니다. 이 작업은 최대 30분까지 걸릴 수 있습니다.
...
4. 이전 자습서에서 복제된 데이터용으로 만든 **contosovmsacct1910171607** 저장소 계정을 선택하고, 장애 조치(failover) 후 Azure VM이 배치되는 **ContosoASRnet** 네트워크를 선택합니다.
