---
title: Windows 서버/시스템 센터 VMM 2012 R2를 Windows 서버 2016-Azure 사이트 복구로 업그레이드
...
ms.locfileid: "73954414"
---
# <a name="upgrade-windows-server-serversystem-center-2012-r2-vmm-to-windows-servervmm-2016"></a>Windows 서버/시스템 센터 2012 R2 VMM을 Windows 서버/VMM 2016으로 업그레이드 
...
- System Center 2012 R2 VMM을 사용하는 경우 
    ...
    - VMM: **VMM 콘솔** -> **설정** -> **일반** -> **데이터베이스 연결에** 대한 데이터베이스 정보 확인
    - System Center Virtual Machine Manager 에이전트 서비스에 사용되는 서비스 계정을 확인합니다.
    - VMM 데이터베이스의 백업이 있는지 확인합니다.
    - 관련된 SCVMM 서버의 데이터베이스 이름을 적어 둡니다. 이 작업은 **VMM 콘솔** -> **설정** -> **일반** -> **데이터베이스 연결로** 이동하여 수행할 수 있습니다.
...
5. SCVMM을 실행하고 **패브릭** 탭에서 각 호스트의 **Refresh** 상태를 확인합니다. 상태는 “주의 필요”로 표시되어야 합니다. 
...
3. SCVMM 콘솔을 실행하고 **패브릭** 탭에서 각 호스트의 상태를 확인합니다. **Refresh** 상태는 “주의 필요”로 표시되어야 합니다.
