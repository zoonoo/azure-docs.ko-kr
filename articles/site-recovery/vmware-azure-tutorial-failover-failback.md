...
이 문서에서는 [Azure Site Recovery](site-recovery-overview.md) 서비스를 사용하여 온-프레미스 VMware VM을 장애 조치(failover)하는 방법을 설명합니다. 
...
> [!NOTE]
> 자습서는 시나리오의 가장 간단한 배포 경로를 보여줍니다. 가능한 경우 기본 옵션을 사용하고 가능한 모든 설정과 경로를 보여주지 않습니다. 장애 조치(failover)에 대해 자세히 알아보려면 [이 문서를 검토](site-recovery-failover.md)합니다.
...
장애 조치(failover)를 실행하기 전에 먼저 VM 속성을 확인하고 해당 VM이 [Azure 요구 사항](vmware-physical-azure-support-matrix.md#replicated-machines)을 준수하는지 확인합니다.
...
3. 장애 조치를 트리거하기 전에 원본 VM을 종료하려고 시도하려면 **장애 조치(failover)를 시작하기 전에 머신을 종료합니다**를 선택합니다. 종료가 실패하더라도 장애 조치는 계속됩니다. **작업** 페이지에서 장애 조치 진행 상황 확인을 수행할 수 있습니다.
...
1. 장애 조치(failover) 후 RDP/SSH를 사용하여 Azure VM에 연결하려면 [이러한 요구 사항을 확인합니다](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover).
...
4. 유효성 검사 후 **커밋**을 클릭하여 장애 조치(failover) 후 VM의 복구 지점을 마무리합니다.
5. 커밋 후 다른 사용 가능한 복구 지점을 모두 삭제합니다. 이렇게 장애 조치(failover)가 완료됩니다.

>[!TIP]
> 장애 조치(failover) 후 연결 문제가 발생하는 경우 이 [문제 해결 가이드](site-recovery-failover-to-azure-troubleshoot.md)를 따릅니다.
