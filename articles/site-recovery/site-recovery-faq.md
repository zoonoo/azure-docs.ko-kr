...
이 문서는 Site Recovery에 대한 질문과 대답을 제공합니다. FAQ를 읽은 후 질문이 있다면 [Azure 복구 서비스 포럼](https://social.msdn.microsoft.com/Forums/azure/home?forum=hypervrecovmgr)에 게시해 주세요.
...
아니요, 이 기능은 지원되지 않습니다. 해결 방법으로 [Azure](site-recovery-vmware-to-azure-classic.md) 또는 [보조 데이터 센터](site-recovery-vmware-to-vmware.md)에 컴퓨터를 물리적 컴퓨터로 복제해야 합니다.
...
아니요. VMM 클라우드에 있는 Hyper-V VM을 복제할 수 있을 뿐 아니라 VMM을 배포하지 않은 환경에서 Hyper-V VM을 복제할 수도 있습니다. [자세히 알아봅니다](site-recovery-hyper-v-site-to-azure.md). 보조 데이터 센터에 복제하려는 경우에는 VMM 클라우드에서 Hyper-V 호스트 서버를 관리해야 합니다.
...
이 시나리오의 경우 VMware vCenter Server, vSphere 하이퍼바이저 및 VMware 도구를 실행 중인 가상 컴퓨터가 필요합니다. 정확한 요구 사항에 대해 [알아봅니다](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment). 물리적 서버에서 Azure로 복사하든 아니면 보조 사이트로 복사하든 관계없이 동일한 제한 사항이 적용됩니다.
...
예. Rest API, PowerShell 또는 Azure SDK를 사용하여 Site Recovery 워크플로를 자동화할 수 있습니다. [PowerShell 및 Azure 리소스 관리자를 사용하여 사이트 복구 배포](site-recovery-deploy-with-powershell-resource-manager.md)에 대해 자세히 알아봅니다.
...
보안 인터넷 연결 또는 사이트 간 VPN(또는 Azure Express 경로)이 있다면 이를 통해 Azure VM에 액세스할 수 있습니다. VPN 연결을 사용할 경우 VM이 있는 Azure 네트워크의 내부 포트와 통신이 이루어집니다. 인터넷을 통한 통신은 VM에 대한 Azure 클라우드 서비스의 공용 끝점에 매핑됩니다.
...
자동이 아닙니다. 포털에서 클릭 한 번으로 장애 조치(Failover)를 시작하거나 [Site Recovery PowerShell cmdlet](https://msdn.microsoft.com/library/dn850420.aspx)을 사용하여 장애 조치(Failover)를 트리거할 수 있습니다. 장애 복구(failback) 작업 역시 Site Recovery 포털에서 간단하게 수행할 수 있습니다. 온-프레미스 Orchestrator 또는 Operations Manager를 사용하여 가상 컴퓨터 오류를 감지하면 SDK를 사용하여 장애 조치(Failover)를 트리거하도록 자동화할 수 있습니다.
