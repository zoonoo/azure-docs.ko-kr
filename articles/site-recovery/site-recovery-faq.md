---
title: Azure 사이트 복구 서비스에 대한 일반적인 질문
description: 이 문서에서는 Azure 사이트 복구에 대한 일반적인 일반적인 질문에 대해 설명합니다.
ms.topic: conceptual
ms.date: 1/24/2020
ms.author: raynew
...
ms.locfileid: "79257681"
---
# <a name="general-questions-about-azure-site-recovery"></a>Azure 사이트 복구에 대한 일반적인 질문

이 문서에서는 Azure 사이트 복구에 대해 자주 묻는 질문을 요약합니다. 특정 시나리오에 대 한 다음 문서를 검토

- [Azure VM 재해 복구에 대한 질문](azure-to-azure-common-questions.md)
- [Azure에 대한 VMware VM 재해 복구에 대한 질문](vmware-azure-common-questions.md)
- [Azure에 대한 하이퍼 V VM 재해 복구에 대한 질문](hyper-v-azure-common-questions.md)
...
아니요, 데이터는 구독의 Azure 저장소에 복제됩니다. 테스트 장애 조치(failover)(DR 드릴) 또는 실제 장애 조치(failover)를 수행하면 Site Recovery가 구독에서 자동으로 가상 머신을 만듭니다.
...
### <a name="where-can-i-find-pricing-information"></a>가격 정보는 어디에서 찾을 수 있습니까?
[사이트 복구 가격](https://azure.microsoft.com/pricing/details/site-recovery/) 세부 정보를 검토합니다.
...
[가격 계산기를](https://aka.ms/asr_pricing_calculator) 사용하여 사이트 복구를 사용하는 동안 비용을 예측할 수 있습니다.

비용에 대한 자세한 추정을 보려면 [VMware](https://aka.ms/siterecovery_deployment_planner) 또는 [Hyper-V에](https://aka.ms/asr-deployment-planner)대한 배포 플래너 도구를 실행하고 [비용 추정 보고서를](https://aka.ms/asr_DP_costreport)사용합니다.


### <a name="managed-disks-are-now-used-to-replicate-vmware-vms-and-physical-servers-do-i-incur-additional-charges-for-the-cache-storage-account-with-managed-disks"></a>이제 관리되는 디스크를 사용하여 VM웨어 VM 및 물리적 서버를 복제할 수 있습니다. 관리 디스크가 있는 캐시 저장소 계정에 추가 요금이 부과됩니까?

아니요, 캐시에 대한 추가 요금은 없습니다. 표준 저장소 계정에 복제할 때 이 캐시 저장소는 동일한 대상 저장소 계정의 일부입니다.
...
예. 모든 보호된 인스턴스에는 처음 31일 동안 Azure Site Recovery 요금이 부과되지 않습니다. 예를 들어 지난 6개월 동안 10개의 인스턴스를 보호하고 11번째 인스턴스를 Azure 사이트 복구에 연결하는 경우 처음 31일 동안 11번째 인스턴스에 대한 요금이 부과되지 않습니다. 처음 10개의 인스턴스는 31일 이상 보호되었기 때문에 Azure 사이트 복구 요금이 계속 부과됩니다.

### <a name="during-the-first-31-days-will-i-incur-any-other-azure-charges"></a>처음 31일 동안 다른 Azure 요금이 부과되나요?

예. 보호된 인스턴스의 처음 31일 동안 사이트 복구가 무료인 경우에도 Azure Storage, 저장소 트랜잭션 및 데이터 전송에 대한 요금이 부과될 수 있습니다. 복구된 가상 머신도 Azure Compute 요금이 발생할 수 있습니다.


### <a name="is-there-a-cost-associated-to-perform-disaster-recovery-drillstest-failover"></a>재해 복구 드릴/테스트 장애 조치(failover)를 수행하는 데 관련된 비용이 있습니까?

DR 드릴에 대한 별도의 비용은 없습니다. 테스트 장애 조치 후 VM이 생성된 후 계산 요금이 부과됩니다.
...
### <a name="how-can-i-enforce-tls-12-on-all-on-premises-azure-site-recovery-components"></a>모든 온-프레미스 Azure 사이트 복구 구성 요소에서 TLS 1.2를 적용하면 어떻게 해야 합니까?
복제된 항목에 설치된 이동성 에이전트는 TLS 1.2에서만 프로세스 서버에 통신합니다. 그러나 구성 서버에서 Azure로, 프로세스 서버에서 Azure로의 통신은 TLS 1.1 또는 1.0에 있을 수 있습니다. [지침에](https://support.microsoft.com/en-us/help/3140245/update-to-enable-tls-1-1-and-tls-1-2-as-default-secure-protocols-in-wi) 따라 모든 구성 서버 및 프로세스 서버에서 TLS 1.2를 적용하십시오.
...
### <a name="is-disaster-recovery-supported-for-azure-vms"></a>Azure VM에서 재해 복구가 지원되고 있습니까?

예. 사이트 복구는 Azure 지역 간의 Azure VM에 대한 재해를 지원합니다. Azure VM 재해 복구에 대한 [일반적인 질문을 검토합니다.](azure-to-azure-common-questions.md)

### <a name="is-disaster-recovery-supported-for-vmware-vms"></a>VMware VM에 재해 복구가 지원되고 있습니까?

예. 사이트 복구는 온-프레미스 VMware VM의 재해 복구를 지원합니다. VMware VM의 재해 복구에 대한 [일반적인 질문을 검토합니다.](vmware-azure-common-questions.md)

### <a name="is-disaster-recovery-supported-for-hyper-v-vms"></a>하이퍼 VM에서 재해 복구가 지원되고 있습니까?
예. 사이트 복구는 온-프레미스 하이퍼-V VM의 재해 복구를 지원합니다. 하이퍼 VM의 재해 복구에 대한 [일반적인 질문을 검토합니다.](hyper-v-azure-common-questions.md)

## <a name="is-disaster-recovery-supported-for-physical-servers"></a>실제 서버에서 재해 복구가 지원되고 있습니까?
예. 사이트 복구는 Windows 및 Linux에서 Azure 또는 보조 사이트로 Windows 및 Linux를 실행하는 온-프레미스 물리적 서버의 재해 복구를 지원합니다. [Azure](vmware-physical-azure-support-matrix.md#replicated-machines)및[보조 사이트에](vmware-physical-secondary-support-matrix.md#replicated-vm-support)대한 재해 복구에 대한 요구 사항에 대해 알아봅니다.
장애 조치 후 물리적 서버는 Azure에서 VM으로 실행됩니다. Azure에서 온-프레미스 물리적 서버로의 장애 복구는 현재 지원되지 않습니다. VMware 가상 컴퓨터로만 장애 조치(fail)할 수 있습니다.
...
Azure 사이트 복구는 공용 끝점을 통해 Azure 저장소 계정 또는 관리 디스크에 데이터를 복제합니다. 사이트 간 VPN을 통해 복제되지 않습니다. 

### <a name="why-cant-i-replicate-over-vpn"></a>VPN을 통해 복제할 수 없는 이유는 무엇인가요?

Azure에 복제하면 복제 트래픽이 Azure Storage의 공용 끝점에 도달합니다. 따라서 공용 인터넷이나 ExpressRoute(Microsoft 피어링 또는 기존 공용 피어링)를 통해서만 복제할 수 있습니다.

### <a name="can-i-use-riverbed-steelheads-for-replication"></a>복제를 위해 리버베드 스틸헤드를 사용할 수 있습니까?

파트너인 Riverbed는 Azure 사이트 복구 작업에 대한 자세한 지침을 제공합니다. 솔루션 [가이드를](https://community.riverbed.com/s/article/DOC-4627)검토합니다.
...
- Azure 사이트 복구는 공용 끝점을 통해 Azure 저장소에 데이터를 복제합니다. 사이트 복구 복제를 위해 ExpressRoute를 사용하려면 [Microsoft 피어링을](../expressroute/expressroute-circuit-peerings.md#microsoftpeering) 설정하거나 기존 [공용 피어링(새](../expressroute/about-public-peering.md) 회로에 대해 더 이상 사용되지 않는)을 사용해야 합니다.
- Microsoft 피어링은 복제에 권장되는 라우팅 도메인입니다.
- 프라이빗 피어링에는 복제가 지원되지 않습니다.
- VMware 컴퓨터 또는 실제 컴퓨터를 보호하는 경우 구성 서버에 대한 [네트워킹 요구 사항도](vmware-azure-configuration-server-requirements.md#network-requirements) 충족해야 합니다. 사이트 복구 복제를 오케스트레이션하려면 구성 서버에서 특정 URL에 대한 연결이 필요합니다. 이 연결에는 ExpressRoute를 사용할 수 없습니다.
...
### <a name="if-i-replicate-to-azure-what-kind-of-storage-account-or-managed-disk-do-i-need"></a>Azure로 복제하는 경우 어떤 종류의 저장소 계정 또는 관리 디스크가 필요합니까?

LRS 또는 GRS 스토리지가 필요합니다. 지역 정전이 발생하거나 주 지역을 복구할 수 없는 경우에 데이터를 복원할 수 있도록 GRS를 사용하는 것이 좋습니다. 계정은 Recovery Services 자격 증명 모음과 동일한 지역에 있어야 합니다. Premium Storage는 Azure Portal에서 Site Recovery를 배포할 때 VMware VM, Hyper-V VM 및 물리적 서버 복제에 지원됩니다. 관리되는 디스크는 LRS만 지원합니다.

### <a name="how-often-can-i-replicate-data"></a>데이터를 얼마나 자주 복제할 수 있나요?
* **하이퍼-V:** 하이퍼 VVM은 30초마다(프리미엄 스토리지 제외), 5분 또는 15분마다 복제할 수 있습니다.
* **Azure VM, VM웨어 VM, 물리적 서버:** 복제 빈도는 여기서 관련이 없습니다. 복제가 계속 됩니다.
...
동적 디스크는 Hyper-V 가상 컴퓨터를 복제할 때와 VMware VM 및 물리적 컴퓨터를 Azure로 복제할 때 지원됩니다. 운영 체제 디스크는 기본 디스크여야 합니다.


### <a name="can-i-throttle-bandwidth-allotted-for-replication-traffic"></a>복제 트래픽에 할당된 대역폭을 제한할 수 있습니까?
예. 대역폭 제한에 대한 자세한 내용은 다음 문서에서 확인할 수 있습니다.
...
### <a name="if-im-failing-over-to-azure-how-do-i-access-the-azure-vms-after-failover"></a>Azure에 장애 조치(failover)를 초과하지 않는 경우 장애 조치 후 Azure VM에 액세스하려면 어떻게 해야 합니까?

보안 인터넷 연결 또는 사이트 간 VPN 또는 Azure ExpressRoute로 Azure VM에 액세스할 수 있습니다. 연결하려면 여러 가지를 준비해야 합니다. [자세히 알아봅니다](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover).
...
Azure는 복원을 위해 디자인되었습니다. 사이트 복구는 Azure SLA에 따라 보조 Azure 데이터 센터로 장애 조치(failover)를 위해 이미 설계되었습니다. 이 상황이 발생하면 사용자가 자격 증명 모음에 대해 선택한 지리적으로 동일한 지역에 메타데이터 및 자격 증명 모음이 남아 있습니다.  
...
자동이 아닙니다. 포털에서 한 번의 클릭으로 장애 조치(failover)를 시작하거나 [사이트 복구 PowerShell을](/powershell/module/az.recoveryservices) 사용하여 장애 조치(failover)를 트리거할 수 있습니다. 장애 복구(failback) 작업은 Site Recovery 포털에서 간단하게 수행할 수 있습니다.
...
### <a name="if-my-on-premises-host-is-not-responding-or-crashed-can-i-fail-back-to-a-different-host"></a>온-프레미스 호스트가 응답하지 않거나 충돌한 경우 다른 호스트로 장애 조치할 수 있습니까?
...
* [하이퍼 V 가상 시스템의 경우](hyper-v-azure-failback.md#fail-back-to-an-alternate-location)
...
### <a name="where-can-i-find-the-release-notesupdate-rollups-of-site-recovery-upgrades"></a>사이트 복구 업그레이드의 릴리스 정보/업데이트 롤업을 찾을 수 있는 위치
