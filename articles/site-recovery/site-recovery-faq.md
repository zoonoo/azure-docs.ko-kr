...
### <a name="what-does-site-recovery-do-to-ensure-data-integrity"></a>데이터 무결성을 보장하기 위해 Site Recovery는 무엇을 하나요?

데이터 무결성을 보장하기 위해 Site Recovery에서 다루는 다양한 측정값이 있습니다. HTTPS 프로토콜을 사용하여 모든 서비스 간에 보안 연결이 설정됩니다. 이렇게 하면 모든 맬웨어 또는 외부 엔터티가 데이터를 조작할 수 없습니다. 또한 체크섬을 사용하는 측정값을 다룹니다. 원본 및 대상 간의 데이터 전송은 상호 간 데이터의 체크섬을 계산하여 실행됩니다. 이렇게 하면 일관된 데이터 전송이 보장됩니다.
...
### <a name="how-can-i-enforce-restricted-access-on-my-storage-accounts-which-are-accessed-by-site-recovery-service-for-readingwriting-replication-data"></a>복제 데이터를 읽고 쓰기 위해 Site Recovery 서비스에서 액세스하는 스토리지 계정에 제한된 액세스를 적용하려면 어떻게 해야 하나요?
*ID* 설정으로 이동하여 Recovery Services 자격 증명 모음의 관리 ID를 켤 수 있습니다. 자격 증명 모음이 Azure Active Directory에 등록되면 스토리지 계정으로 이동하여 자격 증명 모음에 다음과 같은 역할 할당을 제공할 수 있습니다.
...
예, Site Recovery는 Azure 지역 간 Azure VM에 대해 재해 복구를 지원합니다. Azure VM 재해 복구에 대한 [일반적인 질문을 검토하세요](azure-to-azure-common-questions.md). 동일한 대륙의 두 Azure 지역 간에 복제하려면 Azure-Azure DR 제품을 사용하세요. 구성 서버/프로세스 서버 및 ExpressRoute 연결을 설정할 필요가 없습니다.
...
스토리지 계정을 대상 스토리지로 사용하는 것은 Azure Site Recovery에서 지원되지 않습니다. 대신 관리 디스크를 컴퓨터의 대상 스토리지로 사용하는 것이 좋습니다. 관리 디스크는 데이터 복원력을 위해 LRS 유형만 지원합니다.
...
### <a name="can-i-enable-replication-with-app-consistency-in-linux-servers"></a>Linux 서버에서 앱 일관성으로 복제를 사용하도록 설정할 수 있나요? 
예. Linux 운영 체제용 Azure Site Recovery는 앱 일관성을 위한 애플리케이션 사용자 지정 스크립트를 지원합니다. 사전 및 사후 옵션이 포함된 사용자 지정 스크립트는 앱 일관성을 유지하는 동안 Azure Site Recovery 모바일 에이전트에서 사용됩니다. 이 기능을 사용하도록 설정하는 단계는 다음과 같습니다.

1. 컴퓨터에 루트로 로그인합니다.
2. 디렉터리를 Azure Site Recovery 모바일 에이전트 설치 위치로 변경합니다. 기본값은 "/usr/local/ASR"입니다.<br>
    `# cd /usr/local/ASR`
3. 설치 위치에서 디렉터리를 "VX/scripts"로 변경합니다.<br>
    `# cd VX/scripts`
4. 루트 사용자에게 실행 권한이 있는 "customscript.sh" 라는 bash 셸 스크립트를 만듭니다.<br>
    a. 스크립트는 "--pre"및 "--post"(이중 대시 사용) 명령줄 옵션을 지원해야 합니다.<br>
    b. 사전 옵션으로 스크립트를 호출하는 경우 애플리케이션 입력/출력을 고정하고 사후 옵션으로 호출하면 애플리케이션 입력/출력이 재개됩니다.<br>
    다. 샘플 템플릿-<br>
...
5. 사전 및 사후 단계에서 앱 일관성을 요구하는 애플리케이션에 대한 입력/출력을 고정하는 명령과 고정 해제하는 명령을 추가합니다. 이를 지정하는 다른 스크립트를 추가하고 사전 및 사후 옵션을 포함한 "customscript.sh"에서 호출하도록 선택할 수 있습니다.

>[!Note]
>사용자 지정 스크립트를 지원하려면 Site Recovery 에이전트 버전이 9.24 이상이어야 합니다.
...
>[!Note]
>64개 이상의 볼륨이 있는 경우에는 Windows 컴퓨터에서 애플리케이션 일치 복구 지점을 만들지 못합니다.
...
### <a name="what-is-the-difference-between-complete-migration-commit-and-disable-replication"></a>마이그레이션 완료, 커밋 및 복제 사용 안 함 간의 차이점은 무엇인가요?

원본 위치의 컴퓨터가 대상 위치로 장애 조치되면 선택할 수 있는 세 가지 옵션이 있습니다. 세 가지 모두 다른 용도로 사용됩니다.

1.  **마이그레이션 완료** 는 더 이상 원본 위치로 돌아가지 않음을 의미합니다. 대상 지역으로 마이그레이션했으므로 이제 완료되었습니다. 마이그레이션 완료를 클릭하면 내부적으로 커밋과 복제 안 함이 차례로 트리거됩니다. 
2.  **커밋** 은 이 작업이 복제 프로세스의 끝이 아님을 의미합니다. 모든 구성과 함께 복제 항목은 그대로 유지되며 나중에 **다시 보호** 를 눌러 컴퓨터를 원본 지역으로 다시 복제할 수 있습니다. 
3.  **복제 사용 안 함** 은 복제를 사용하지 않도록 설정하고 모든 관련 구성을 제거합니다. 대상 지역에 존재하는 기존 컴퓨터에는 영향을 주지 않습니다.
