...
1. 새 Recovery Services 자격 증명 모음을 만듭니다. 자격 증명 모음 개체를 나중에 사용할 변수에 저장합니다.
   ...
   `Get-AzRecoveryServicesVault` cmdlet을 사용 하 여 자격 증명 모음 개체를 만든 후 검색할 수 있습니다.
...
1. 이 cmdlet는 기본 네트워크와 복구 네트워크 사이에 매핑을 만듭니다. `$PrimaryNetworks`의 첫 번째 요소로 기본 네트워크를 지정 합니다. `$RecoveryNetworks`의 첫 번째 요소로 복구 네트워크를 지정 합니다.
