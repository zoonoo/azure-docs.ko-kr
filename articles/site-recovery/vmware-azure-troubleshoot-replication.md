...
## <a name="step-1-monitor-process-server-health"></a>1 단계: 프로세스 서버 상태 모니터링
...
## <a name="step-2-troubleshoot-connectivity-and-replication-issues"></a>2 단계: 연결 및 복제 문제 해결
...
## <a name="step-3-troubleshoot-source-machines-that-arent-available-for-replication"></a>3 단계: 복제에 사용할 수 없는 원본 컴퓨터 문제 해결

Site Recovery를 사용하여 복제를 사용하도록 설정할 원본 컴퓨터를 선택하려고 하면 다음과 같은 이유 중 하나로 컴퓨터를 계속 사용하지 못할 수 있습니다.

* **동일한 인스턴스 uuid를 사용 하는 두 개의 가상 머신**: vCenter의 두 가상 머신이 동일한 인스턴스 uuid를 사용 하는 경우 구성 서버에서 검색 한 첫 번째 가상 머신이 Azure Portal 표시 됩니다. 이 문제를 해결하려면 두 개의 가상 머신에 동일한 인스턴스 UUID가 없는지 확인합니다. 이 시나리오는 백업 VM이 활성화 되 고 검색 레코드에 기록 되는 인스턴스에서 일반적으로 발생 합니다. 문제를 해결 하려면 [VMware To Azure: 중복 항목 또는 부실 항목을 정리 하는 방법 Azure Site Recovery](https://social.technet.microsoft.com/wiki/contents/articles/32026.asr-vmware-to-azure-how-to-cleanup-duplicatestale-entries.aspx) 을 참조 하세요.
* **잘못 된 vcenter 사용자 자격 증명**: 위치 템플릿 또는 통합 설치 프로그램을 사용 하 여 구성 서버를 설정할 때 올바른 vcenter 자격 증명을 추가 했는지 확인 합니다. 설치 중에 추가한 자격 증명을 확인하려면 [자동 검색에 대한 자격 증명 수정](vmware-azure-manage-configuration-server.md#modify-credentials-for-automatic-discovery)을 참조하세요.
* **vcenter 권한 부족**: vcenter 액세스에 제공 된 권한에 필요한 권한이 없으면 가상 컴퓨터를 검색 하지 못할 수 있습니다. [자동 검색용 계정 준비](vmware-azure-tutorial-prepare-on-premises.md#prepare-an-account-for-automatic-discovery)에 설명된 사용 권한을 vCenter 사용자 계정에 추가해야 합니다.
* **Azure Site Recovery 관리 서버**: Configuration server/scale-out process 서버/마스터 대상 서버 역할 중 하나 이상의 역할에서 가상 컴퓨터를 관리 서버로 사용 하는 경우에는 포털에서 가상 컴퓨터를 선택할 수 없습니다. 관리 서버는 복제할 수 없습니다.
* **이미 Azure Site Recovery 서비스를 통해 보호/장애 조치 (failover**): 가상 컴퓨터가 이미 보호 되 고 있거나 Site Recovery를 통해 장애 조치 (failover) 된 경우에는 가상 컴퓨터를 포털에서 보호 하도록 선택할 수 없습니다. 포털에서 찾고 있는 가상 머신이 다른 사용자 또는 다른 구독을 통해 아직 보호되고 있지 않은지 확인합니다.
* **vcenter가 연결 되지 않음**: vcenter가 연결 된 상태 인지 확인 합니다. 확인하려면 [Recovery Services 가격 증명 모음] > [Site Recovery 인프라] > [구성 서버]로 이동하고 > 해당 구성 서버를 클릭하면 > 연결된 서버의 세부 정보와 함께 블레이드가 오른쪽에 열립니다. VCenter가 연결되었는지 확인합니다. "연결 되지 않음" 상태인 경우에는이 문제를 해결 한 다음 포털에서 [구성 서버를 새로 고칩니다](vmware-azure-manage-configuration-server.md#refresh-configuration-server) . 그러면 포털에 가상 머신이 나열됩니다.
* **ESXi powered**: 가상 머신이 있는 ESXi 호스트가 전원이 꺼진 상태 이면 가상 머신이 나열 되지 않거나 Azure Portal에서 선택할 수 없게 됩니다. ESXi 호스트 전원을 켜고 포털에서 [구성 서버를 새로 고칩니다](vmware-azure-manage-configuration-server.md#refresh-configuration-server). 그러면 포털에 가상 머신이 나열됩니다.
* **다시 부팅 보류**중: 가상 머신에서 다시 부팅이 보류 중인 경우 Azure Portal에서 컴퓨터를 선택할 수 없습니다. 보류 중인 재부팅 활동을 완료하고 [구성 서버를 새로 고쳐야 합니다](vmware-azure-manage-configuration-server.md#refresh-configuration-server). 그러면 포털에 가상 머신이 나열됩니다.
* **Ip를 찾을**수 없음: 가상 머신에 연결 된 유효한 ip 주소가 없으면 Azure Portal에서 컴퓨터를 선택할 수 없습니다. 가상 머신에 유효한 IP 주소를 할당하고 [구성 서버를 새로 고칩니다](vmware-azure-manage-configuration-server.md#refresh-configuration-server). 그러면 포털에 가상 머신이 나열됩니다.

### <a name="troubleshoot-protected-virtual-machines-greyed-out-in-the-portal"></a>포털에서 회색으로 표시 된 보호 된 가상 머신 문제 해결

Site Recovery에서 복제된 가상 머신은 시스템에 중복된 항목이 있으면 Azure Portal에서 사용할 수 없습니다. 오래 된 항목을 삭제 하 고 문제를 해결 하는 방법에 대 한 자세한 내용은 [Azure Site Recovery VMware To Azure: 중복 되거나 오래 된 항목을 정리 하는 방법](https://social.technet.microsoft.com/wiki/contents/articles/32026.asr-vmware-to-azure-how-to-cleanup-duplicatestale-entries.aspx)을 참조 하세요.
...
- 바이러스 백신 소프트웨어가 Azure Site Recovery을 차단 하지 않습니다. Azure Site Recovery에 필요한 폴더 제외 [에 대해 자세히 알아보세요.](vmware-azure-set-up-source.md#azure-site-recovery-folder-exclusions-from-antivirus-program)
...
VSS 설치 오류 동작을 처리 하기 위해 모바일 에이전트 [9.23](vmware-physical-mobility-service-overview.md#mobility-service-agent-version-923-and-higher) & [9.27](site-recovery-whats-new.md#update-rollup-39) 버전에 대 한 기능이 향상 되었습니다. VSS 오류 문제 해결에 대 한 모범 지침은 최신 버전을 확인 하세요.
