...
> [!WARNING]
> 이 문서에는 **레거시 지침**이 포함되어 있습니다. 새 배포에는 사용하지 마십시오. 대신 [이 지침에 따라](site-recovery-vmware-to-azure.md) Azure 포털에서 사이트 복구를 배포하거나, [이 지침을 사용하여](site-recovery-vmware-to-azure-classic.md) 클래식 포털에서 향상된 배포를 구성하세요. 이 문서에 설명된 방법을 사용하여 이미 배포한 경우에는, 클래식 포털에서 향상된 배포로 마이그레이션하는 것이 좋습니다.
> 
> 
...
6. Linux를 실행하는 경우 다음을 수행합니다.
   ...
   4. **tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64**
...
> [!NOTE]
> 등록을 마친 후 마스터 대상 서버가 포털에 나열되기까지 최대 15분이 소요될 수 있습니다. 즉시 업데이트하려면 **구성 서버** 페이지에서 **새로 고침**을 클릭합니다.
> 
> 
...
5. 설치를 완료하려면 서버를 다시 시작해야 할 수 있습니다. **구성 서버** > **서버 정보**에서 프로세스 서버가 성공적으로 등록되었는지 확인합니다.
...
2. 장애 조치(Failover) 후에 Azure에서 컴퓨터에 지정될 이름과 Azure 가상 컴퓨터 크기를 수정할 수 있습니다. 장애 조치(Failover) 후에 컴퓨터가 연결되는 Azure 네트워크를 선택할 수도 있습니다.
   ...
   > [!NOTE]
   > [Migration of networks](../resource-group-move-resources.md) 작업은 Site Recovery를 배포하는 데 사용되는 네트워크에서 같은 구독 내에 있거나 여러 구독에 있는 리소스 그룹에 대해 수행할 수 없습니다.
   > 
   > 
