...
>[!NOTE]
> 현재 Site Recovery은 Windows 운영 체제를 실행 하는 Vm에 대해 Azure Active Directory (AAD)를 사용 하거나 사용 하지 않고 ADE를 지원 합니다. Linux 운영 체제의 경우 AAD 없이 ADE만 지원 합니다. 또한 AAD를 사용 하지 않고 ADE 1.1를 실행 하는 컴퓨터의 경우 Vm은 관리 디스크를 사용 해야 합니다. 관리 되지 않는 디스크가 있는 Vm은 지원 되지 않습니다. ADE 0.1 (AAD)에서 1.1로 전환 하는 경우에는 복제를 사용 하지 않도록 설정 하 고 1.1를 사용 하도록 설정한 후 VM에 대해 복제를 사용 하도록 설정 해야 합니다.
