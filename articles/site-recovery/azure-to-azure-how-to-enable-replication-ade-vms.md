...
Azure Site Recovery는 원본 지역 키 자격 증명 모음에 적어도 읽기 권한과 대상 지역 키 자격 증명 모음 암호를 읽고 대상 지역 키 자격 증명 모음에 복사에 대 한 쓰기 권한이 필요 합니다. 

**원인 1:** 권한이 "GET"는 **원본 지역 키 자격 증명 모음** 키를 읽는 합니다. </br>
**해결 방법:** 이 있는지 여부는 구독에서 관리자는 여부에 관계 없이 key vault에 대 한 get 권한이 중요 합니다.

1. 이 예제에서는 "ContososourceKeyvault"입니다 원본 지역 Key vault로 이동 > **액세스 정책** 
2. 아래 **주체 선택** 예를 들어 사용자 이름을 추가 합니다. "dradmin@contoso.com"
3. 아래 **키 권한** 가져오기를 선택 
4. 아래 **비밀 권한** 가져오기를 선택 
5. 저장 된 액세스 정책

**원인 2:** 에 필요한 권한이 없는 **대상 지역 키 자격 증명 모음** 키를 쓸 수 있습니다. </br>
...
에 필요한 권한을 [대상 키 자격 증명 모음](#required-user-permissions)

**해결 방법:** 로 이동 **Home** > **Keyvaults** > **ContosotargetKeyvault** > **액세스 정책** 및 적절 한 사용 권한을 추가 합니다.
