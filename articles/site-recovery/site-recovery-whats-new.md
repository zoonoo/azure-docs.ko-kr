...
[업데이트 롤업 53](https://support.microsoft.com/topic/update-rollup-53-for-azure-site-recovery-060268ef-5835-bb49-7cbc-e8c1e6c6e12a) 은 다음과 같은 업데이트를 제공 합니다.

**업데이트** | **세부 정보**
--- | ---
**공급자 및 에이전트** | 롤업에 설명 된 대로 Site Recovery 에이전트 및 공급자를 업데이트 합니다.
**문제 해결/향상** | 롤업에 자세히 설명 된 다양 한 수정 사항 및 개선 사항입니다. 
**Azure VM 재해 복구** | 태그에 대 한 복제가 이제 지원 됩니다. 원본 지역의 Azure Vm, 디스크 및 Nic에 추가 된 태그는 대상 지역의 컴퓨터에 복제 됩니다.<br/><br/> 이제 Debian 10이 실행 되는 Azure Vm이 복제에 대해 지원 됩니다.
**Azure에 대 한 VMware v m/물리적 재해 복구** | 업데이트에는 Azure에 대 한 VMware VM 복제에 대 한 로깅 향상 및 향상 된 오류 메시지가 포함 됩니다.<br/><br/> 이제 Debian 10을 실행 하는 VMware Vm 및 물리적 컴퓨터가 복제에 대해 지원 됩니다.
...
[업데이트 롤업 51](https://support.microsoft.com/help/4590304/update-rollup-51-for-azure-site-recovery) 은 다음과 같은 업데이트를 제공 합니다.
...
## <a name="updates-september-2020"></a>업데이트 (9 월 2020)
...
> [!NOTE]
> 업데이트 롤업 48에는 ADE를 사용 하 여 암호화 된 Linux 컴퓨터에 대해 복제를 사용 하도록 설정 하는 알려진 문제가 있습니다. [자세한 정보를 알아보세요](./azure-to-azure-troubleshoot-errors.md#enable-protection-failed-as-the-installer-is-unable-to-find-the-root-disk-error-code-151137).
...
Azure virtual machines는 이제 고객이 관리 하는 키를 사용 하 여 미사용 암호화를 사용 하도록 설정 하는 Vm을 지원 합니다. [자세한 정보를 알아보세요](azure-to-azure-how-to-enable-replication-cmk-disks.md).
...
**기능** | **세부 정보**
--- | ---
새 프로세스 서버 경고 | 새 프로세스 서버 경고가 추가 되었습니다. [자세한 정보를 알아보세요](vmware-physical-azure-monitor-process-server.md).
...
**기능** | **세부 정보**
--- | ---
**추가 된 디스크 복제** | 이미 재해 복구를 사용 하도록 설정 된 Azure VM에 추가 된 데이터 디스크에 대해 복제를 사용 하도록 설정 합니다. [자세한 정보를 알아보세요](azure-to-azure-enable-replication-added-disk.md).
**자동 업데이트** | 재해 복구를 위해 사용 하도록 설정 된 Azure Vm에서 실행 되는 모바일 서비스 확장에 대 한 자동 업데이트를 구성 하는 경우, 이제 Site Recovery에서 만든 기본 계정을 사용 하는 대신 사용할 기존 automation 계정을 선택할 수 있습니다. [자세한 정보를 알아보세요](azure-to-azure-autoupdate.md).
...
**기능** | **세부 정보**
--- | ---
**프로세스 서버 모니터링** | 온-프레미스 VMware Vm 및 물리적 서버의 재해 복구를 위해 향상 된 서버 상태 보고 및 경고와 함께 프로세스 서버 문제를 모니터링 하 고 문제를 해결 합니다. [자세한 정보를 알아보세요](vmware-physical-azure-monitor-process-server.md).
...
[자세한 정보를 알아보세요](azure-vm-disaster-recovery-with-accelerated-networking.md).
...
**스토리지 공간 다이렉트** | Site Recovery는 S2D (저장소 공간 다이렉트)를 사용 하 여 Azure Vm을 지원 합니다.
...
**디렉터리** | 별도의 파티션으로 설정 된 이러한 디렉터리나 동일한 시스템 디스크에 없는 파일 시스템에 대 한 지원이 추가 되었습니다.<br/><br/> /(root),/boot,/usr,/usr/local,/var,/etc
...
업데이트 후 VM을 다시 시작할 필요가 없으며 복제에 영향을 주지 않습니다. [자세한 정보를 알아보세요](azure-to-azure-autoupdate.md).
...
**디스크 암호화 지원** | Azure AD 앱으로 Azure Disk Encryption (ADE)로 암호화 된 Azure Vm의 재해 복구를 위한 지원이 추가 되었습니다. [자세한 정보를 알아보세요](azure-to-azure-how-to-enable-replication-ade-vms.md).
...
**구독 간 재해 복구** | 동일한 Azure Active Directory 테 넌 트 내에서 다른 구독의 Azure Vm을 다른 지역으로 복제 하는 기능을 지원 합니다. [자세한 정보를 알아보세요](https://aka.ms/cross-sub-blog).
