---
title: Azure 사이트 복구를 사용 하 고 하이퍼-V 재해 복구에 대 한 일반적인 질문
...
예. 장애 조치(failover) 동안 Site Recovery가 컴퓨터를 2세대에서 1세대로 변환합니다. 장애 복구 시 컴퓨터가 다시 2세대로 변환됩니다. [자세한 내용을 읽어보십시오.](https://azure.microsoft.com/blog/2015/04/28/disaster-recovery-to-azure-enhanced-and-were-listening/)
...
### <a name="can-we-keep-on-premises-metadata-within-a-geographic-region"></a>온-프레미스 메타데이터를 지리적 지역 내에 유지할 수 있습니까?
...
사이트 복구는 공용 끝점을 통해 온-프레미스에서 Azure 저장소로 데이터를 복제하거나 ExpressRoute Microsoft 피어링을 사용하여 데이터를 복제합니다. 사이트 간 VPN 네트워크를 통한 복제는 지원되지 않습니다.

### <a name="can-i-replicate-to-azure-with-expressroute"></a>ExpressRoute를 사용하여 Azure에 복제할 수 있나요?

예, ExpressRoute를 사용하여 VM을 Azure로 복제할 수 있습니다. 사이트 복구는 공용 끝점을 통해 Azure 저장소 계정에 데이터를 복제하며 사이트 복구 복제에 대한 [Microsoft 피어링을](../expressroute/expressroute-circuit-peerings.md#microsoftpeering) 설정해야 합니다. VM에서 Azure 가상 네트워크로 장애 조치한 후에는 [프라이빗 피어링](../expressroute/expressroute-circuit-peerings.md#privatepeering)을 사용하여 해당 VM에 액세스할 수 있습니다.


### <a name="why-cant-i-replicate-over-vpn"></a>VPN을 통해 복제할 수 없는 이유는 무엇인가요?

Azure에 복제하면 복제 트래픽이 Azure Storage 계정의 공용 끝점에 도달합니다. 따라서 ExpressRoute (Microsoft 피어링)를 통해 공용 인터넷을 통해서만 복제할 수 있으며 VPN이 작동하지 않습니다. 
...
- 장애 조치(Failover)는 두 부분으로 나타미수 있습니다.
...
[장애 조치(Failover)는](site-recovery-failover.md) 자동이 아닙니다. 포털에서 한 번의 클릭으로 장애 조치(failover)를 시작하거나 [PowerShell을](/powershell/module/az.recoveryservices) 사용하여 장애 조치(failover)를 트리거할 수 있습니다.
...
3. 초기 동기화가 완료되면 장애 조치를 완료하도록 선택합니다. 완료된 후 온-프레미스 VM에 로그인하여 모든 것이 예상대로 작동하는지 확인할 수 있습니다. Azure Portal에서 Azure VM이 중지된 것을 확인할 수 있습니다.
...
예, Azure에 장애 조치한 경우 원본을 사용할 수 없을 때 다른 위치로 장애 복구할 수 있습니다. [자세히 알아봅니다](hyper-v-azure-failback.md#fail-back-to-an-alternate-location).
