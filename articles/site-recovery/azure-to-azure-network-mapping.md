...
대상 네트워크는 장애 조치(failover) VNet이 아님 | -대상 IP 주소는 동일한 IP 주소를 사용 하 여 고정 됩니다.<br/><br/>  -동일한 IP 주소가 이미 할당 된 경우 IP 주소는 서브넷 범위의 끝에서 사용할 수 있는 다음 IP 주소입니다.<br/><br/> 예를 들어 원본 고정 IP 주소가 10.0.0.19이 고 장애 조치 (failover) 네트워크가 아닌 네트워크에서 10.0.0.0/24 범위를 사용 하는 경우, 대상 고정 IP 주소가 사용 가능한 경우 10.0.0.19 되 고 그렇지 않은 경우 10.0.0.254가 됩니다.
