---
title: Azure 사이트 복구에서 두 지역 간에 가상 네트워크 매핑
description: Azure 사이트 복구를 사용 하 여 Azure VM 재해 복구에 대 한 두 Azure 지역 간의 가상 네트워크 매핑에 대해 자세히 알아봅니다.
...
이 문서에서는 서로 다른 Azure 지역에 있는 Azure VNet(가상 네트워크)의 두 인스턴스를 매핑하는 방법과 네트워크 간에 IP 주소를 설정하는 방법을 설명합니다. 네트워크 매핑은 복제를 사용하도록 설정하는 시점의 원본 네트워크를 기반으로 대상 네트워크 선택에 대한 기본 동작을 제공합니다.

## <a name="prerequisites"></a>사전 요구 사항

네트워크를 매핑하려면 원본 및 대상 Azure 지역에 [Azure VNet](../virtual-network/virtual-networks-overview.md)이 있어야 합니다. 

## <a name="set-up-network-mapping-manually-optional"></a>네트워크 매핑을 수동으로 설정(선택 사항)
...
- 원본 네트워크에 대해 네트워크 매핑이 이미 발생한 경우 더 많은 VM에 대한 복제를 사용하도록 설정하는 시점에 매핑된 대상 네트워크는 항상 기본값입니다. 드롭다운에서 사용 가능한 다른 옵션을 선택하여 대상 가상 네트워크를 변경하도록 선택할 수 있습니다. 
- 새 복제에 대한 기본 대상 가상 네트워크를 변경하려면 기존 네트워크 매핑을 수정해야 합니다.
- 지역 A에서 지역 B로 네트워크 매핑을 수정하려면 먼저 지역 B에서 지역 A로 네트워크 매핑을 삭제해야 합니다. 역매핑 삭제 후 지역 A에서 영역 B로 네트워크 매핑을 수정한 다음 관련 역매핑을 만듭니다.

>[!NOTE]
>* 네트워크 매핑을 수정하는 것은 새 VM 복제에 대한 기본값만 변경합니다. 기존 복제에 대한 대상 가상 네트워크 선택항목에는 영향을 미치지 않습니다. 
>* 기존 복제에 대한 대상 네트워크를 수정하려면 복제된 항목의 계산 및 네트워크 설정으로 이동합니다.
...
- VM의 **계산 및 네트워크** 설정에서 대상 서브넷을 수정할 수 있습니다.
...
**대상 네트워크** | **세부 정보**
--- | ---
대상 네트워크는 장애 조치(failover) VNet | - 대상 IP 주소는 동일한 IP 주소로 정적입니다. <br/><br/>  - 동일한 IP 주소가 이미 할당된 경우 IP 주소는 서브넷 범위의 끝에서 사용할 수 있는 다음 주소입니다. 예를 들어 원본 IP 주소가 10.0.0.19이고 장애 조치(failover) 네트워크에서 10.0.0.0/24 범위를 사용하는 경우 대상 VM에 할당되는 그 다음 IP 주소는 10.0.0.254입니다.
대상 네트워크는 장애 조치(failover) VNet이 아님 | - 대상 IP 주소는 동일한 IP 주소로 정적입니다.<br/><br/>  - 동일한 IP 주소가 이미 할당된 경우 IP 주소는 서브넷 범위의 끝에서 사용할 수 있는 다음 주소입니다.<br/><br/> 예를 들어 원본 고정 IP 주소가 10.0.0.19이고 장애 조치(failover) 네트워크가 아니고 범위가 10.0.0.0/24인 네트워크에서 장애 조치(failover)되는 경우 대상 고정 IP 주소는 10.0.0.0.19이고(사용 가능한 경우), 그렇지 않으면 10.0.0.254입니다.
