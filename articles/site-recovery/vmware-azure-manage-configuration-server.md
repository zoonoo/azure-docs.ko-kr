...
OVF 템플릿에 제공되는 라이선스는 180일 동안 유효한 평가 라이선스입니다. 중단 없이 사용하려면 제공된 라이선스를 사용하여 Windows를 정품 인증해야 합니다. 라이선스 업데이트는 독립 실행형 키 또는 KMS 표준 키를 통해 수행할 수 있습니다. 지침은 [OS를 실행하기 위한 DISM Windows 명령줄](/windows-hardware/manufacture/desktop/dism-windows-edition-servicing-command-line-options)에서 사용할 수 있습니다. 키를 가져오려면 [KMS 클라이언트 설정](/windows-server/get-started/kmsclientkeys)을 참조하세요.
...
2. **가상 머신 자격 증명 추가** 를 클릭하세요.
    ![가상 머신 자격 증명 추가 링크가 있는 가상 머신 자격 증명 관리 창을 보여 주는 스크린샷](media/vmware-azure-manage-configuration-server/add-mobility-credentials.png)
...
1. 갱신하려면 자격 증명 모음에서 **Site Recovery 인프라** > **구성 서버** 를 엽니다. 원하는 구성 서버를 선택합니다.
...
2. **인증서가 이미 만료된 경우에만 다음 절차를 따르세요.** 구성 서버에 로그인하고, C 드라이브 > 프로그램 데이터 > Site Recovery > 홈 > svsystems > bin으로 이동한 후 "RenewCerts" 실행기 도구를 관리자 권한으로 실행 합니다.
3. PowerShell 실행 창이 팝업되고 인증서 갱신이 트리거됩니다. 이 작업은 최대 15분까지 걸릴 수 있습니다. 갱신이 완료될 때까지 창을 닫지 마세요.
...
3. 업데이트 설치 관리자 파일을 구성 서버에 다운로드합니다.
    ...
    ![업데이트 설치 관리자 파일을 다운로드하기 위해 클릭할 위치를 보여 주는 스크린샷](./media/vmware-azure-manage-configuration-server/update1.png)

4. 두 번 클릭하여 설치 관리자를 실행합니다.
5. 설치 관리자는 컴퓨터에서 실행 중인 현재 버전을 검색합니다. **예** 를 클릭하여 업그레이드를 시작합니다.
6. 업그레이드가 완료되면 서버 구성의 유효성을 검사합니다.

    ![완료된 서버 유효성 검사 구성을 보여 주는 스크린샷](./media/vmware-azure-manage-configuration-server/update3.png)
...
|/BypassProxy|Optional|구성 서버가 프록시 없이 Azure에 연결되도록 지정합니다.||
...
> [!NOTE]
> 구성 서버의 강제 삭제를 위해서는 Remove-AzSiteRecoveryFabric에 **-Force** 옵션을 사용할 수 있습니다.
