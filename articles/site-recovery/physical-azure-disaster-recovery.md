...
이 자습서에서는 Azure에 대한 온-프레미스 물리적 Windows 및 Linux 서버의 재해 복구를 설정하는 방법을 보여 줍니다. 이 자습서에서는 다음을 수행하는 방법을 알아봅니다.
...
5. 자격 증명 모음 등록 키를 다운로드합니다. 통합 설치를 실행할 때 이 키가 필요합니다. 이 키는 생성된 날로부터 5일간 유효합니다.
   ...
   ![설치 파일 및 등록 키를 다운로드 하는 옵션을 보여 주는 스크린샷](./media/physical-azure-disaster-recovery/source-environment.png)
...
3. Site Recovery가 호환되는 Azure Storage 계정 및 네트워크가 하나 이상 있는지 확인합니다.
   ...
   ![대상 환경을 설정 하기 위한 옵션의 스크린샷](./media/physical-azure-disaster-recovery/network-storage.png)
...
5. **애플리케이션 일치 스냅샷 빈도**에서 애플리케이션 일치 스냅샷이 포함된 복구 지점을 만드는 빈도(분)를 지정합니다. **확인** 을 클릭하여 정책을 만듭니다.
    ...
    ![복제 정책을 만드는 옵션의 스크린샷](./media/physical-azure-disaster-recovery/replication-policy.png)
