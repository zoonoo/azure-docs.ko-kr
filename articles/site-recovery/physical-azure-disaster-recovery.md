...
[Azure Site Recovery](site-recovery-overview.md) 서비스는 온-프레미스 컴퓨터 및 Azure vm (virtual machines)의 복제, 장애 조치 (failover) 및 장애 복구를 관리 하 고 오케스트레이션 하 여 재해 복구 전략에 기여 합니다.
...
Microsoft [Azure 계정을](https://azure.microsoft.com/)가져옵니다.
...
[Azure 네트워크](../virtual-network/quick-create-portal.md)를 설정 합니다.
...
등록이 완료 되 면 자격 증명 모음의 **설정** > **서버** 페이지에 구성 서버가 표시 됩니다.
...
1. 새 복제 정책을 만들려면 **Site Recovery 인프라** > **복제** > 정책 **+ 복제 정책**을 클릭 합니다.
...
8. 컴퓨터마다 Azure 네트워크를 선택하려면 **나중에 구성**을 선택합니다. **나중에 구성** 을 선택 하 여 컴퓨터별 Azure 네트워크를 선택 합니다. 
...
12. **복제 사용**을 클릭 합니다.  > **Jobs** **설정**작업 > **Site Recovery 작업**에서 **보호 사용** 작업의 진행률을 추적할 수 있습니다. **보호 완료** 작업이 실행 된 후 컴퓨터는 장애 조치 (failover)를 수행할 준비가 됩니다.
...
[재해 복구 훈련을 실행](tutorial-dr-drill-azure.md)합니다.
