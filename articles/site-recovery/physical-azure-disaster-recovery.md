---
title: Azure 사이트 복구를 사용 하 고 실제 온-프레미스 서버의 재해 복구 설정
...
- 모든 구성 요소에 대한 [지원 요구 사항을](vmware-physical-secondary-support-matrix.md) 검토합니다.
...
마이크로소프트 [Azure 계정을](https://azure.microsoft.com/)가져옵니다.
...
- 컴퓨터를 Azure로 복제하는 데 필요한 [권한을](site-recovery-role-based-linked-access-control.md#permissions-required-to-enable-replication-for-new-virtual-machines) 검토합니다.
...
[Azure 네트워크를](../virtual-network/quick-create-portal.md)설정합니다.
...
[Azure 저장소 계정을](../storage/common/storage-account-create.md)설정합니다.
...
- Windows VM에서는 도메인 계정을 사용하지 않는 경우 로컬 컴퓨터에서 원격 사용자 액세스 제어를 사용하지 않도록 설정합니다. 이렇게 하려면 **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\정책\시스템**아래의 레지스터에서 DWORD 항목 **LocalAccountTokenFilterPolicy를**추가하여 1의 값을 가합니다.
...
2. 리소스 메뉴에서 **사이트 복구** > **준비 인프라** > **보호 목표를 클릭합니다.**
...
구성 서버 컴퓨터에서 시스템 클럭이 [Time Server](https://technet.microsoft.com/windows-server-docs/identity/ad-ds/get-started/windows-time-service/windows-time-service)와 동기화되어 있는지 확인합니다. 서로 일치해야 합니다. 15분 빠르거나 늦은 경우 설치가 실패할 수 있습니다.
...
등록이 완료되면 구성 서버가 볼트의 **설정** > **서버** 페이지에 표시됩니다.

## <a name="set-up-the-target-environment"></a>대상 환경 설정

대상 리소스를 선택하고 확인합니다.

1. **인프라** > **대상**준비를 클릭하고 사용할 Azure 구독을 선택합니다.
2. 대상 배포 모델을 지정합니다.
...
1. 새 복제 정책을 만들려면 **사이트 복구 인프라** > **복제 정책** > **+복제 정책을**클릭합니다.
...
1. 응용 프로그램 > 원본 **복제를****클릭합니다.**
2. **원본**에서 구성 서버를 선택합니다.
3. **컴퓨터 유형에서** **물리적 컴퓨터를**선택합니다.
4. 프로세스 서버(구성 서버)를 선택합니다. 그런 다음 **확인**을 클릭합니다.
5. **Target에서**장애 조치 후 Azure VM을 만들 구독 및 리소스 그룹을 선택합니다. Azure(클래식 또는 리소스 관리)에서 사용할 배포 모델을 선택합니다.
6. 데이터 복제에 사용할 Azure Storage 계정을 선택합니다. 
7. 장애 조치(failover) 후 Azure VM이 생성될 때 연결될 Azure 네트워크 및 서브넷을 선택합니다.
8. 컴퓨터마다 Azure 네트워크를 선택하려면 **나중에 구성**을 선택합니다. **나중에 구성을** 선택하여 컴퓨터당 Azure 네트워크를 선택합니다. 
9. **물리적 컴퓨터**에서 **+물리적 컴퓨터**를 클릭합니다. 이름 및 IP 주소를 지정합니다. 복제하려는 컴퓨터의 운영 체제를 선택합니다. 서버가 검색된 후 나열되는 데 몇 분 정도 걸립니다. 
10. **속성** > **구성 속성에서**프로세스 서버에서 사용할 계정을 선택하여 컴퓨터에 Mobility 서비스를 자동으로 설치합니다.
11. **복제 설정에서** > **복제 설정 구성,** 올바른 복제 정책이 선택되었는지 확인합니다. 
12. **복제 활성화를 클릭합니다.**  >  **설정** > **작업**사이트 복구 작업에서 **보호 활성화** 작업의 진행**률을 추적할**수 있습니다. 보호 **종료** 작업이 실행된 후 컴퓨터가 장애 조치(failover)에 사용할 준비가 된 것입니다.


추가한 서버를 모니터링하려면 구성 서버**마지막 연락처 에서**마지막으로 검색된 시간을 확인할 수 **있습니다.** >  예약된 검색 시간 동안 기다리지 않고 컴퓨터를 추가하려면 구성 서버를 강조 표시하고(클릭하지 않음) **새로 고침**을 클릭합니다.

## <a name="next-steps"></a>다음 단계

[재해 복구 드릴을 실행합니다.](tutorial-dr-drill-azure.md)
