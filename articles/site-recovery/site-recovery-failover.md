...
7. 초기 데이터 동기화가 완료되고 Azure에서 가상 컴퓨터를 종료할 준비가 되었을 때 장애 조치 전에 데이터를 동기화하는 옵션을 선택했다면 **작업** > <planned failover job name> **완전한 장애 조치**에서 의견이나 질문을 게시합니다. 그러면 Azure 컴퓨터가 종료되고 마지막 변경 내용이 온-프레미스 가상 컴퓨터에 전송된 다음 시작됩니다.
