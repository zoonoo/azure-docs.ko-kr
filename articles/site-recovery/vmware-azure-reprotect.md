...
온-프레미스 사이트로 장애 복구 (failback) 하기 전에 Azure에서 프로세스 서버가 필요 합니다.

- 프로세스 서버는 Azure의 보호 된 가상 머신에서 데이터를 수신 하 고 온-프레미스 사이트로 데이터를 보냅니다.
- 프로세스 서버와 보호된 가상 컴퓨터 간에 대기 시간이 짧은 네트워크가 필요합니다. 따라서 Azure에 프로세스 서버를 배포 하는 것이 좋습니다. 프로세스 서버가 복제 가상 머신(Azure에서 장애 조치된 시스템)와 가까울수록 복제 성능이 더 높습니다. 
- 개념 증명을 위해 프라이빗 피어링으로 ExpressRoute 및 온-프레미스 프로세스 서버를 사용할 수 있습니다.
...
7. **확인**을 선택하여 다시 보호를 시작합니다. Azure에서 온-프레미스 사이트로 가상 머신을 복제하는 작업을 시작합니다. **작업** 탭에서 진행률을 추적할 수 있습니다. 다시 보호 성공적으로 완료 되 면 가상 머신이 보호 된 상태로 전환 됩니다.
