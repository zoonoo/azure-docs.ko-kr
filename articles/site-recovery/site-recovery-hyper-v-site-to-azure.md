...
8. Azure VM 이름(대상 이름)이 [Azure 가상 컴퓨터 요구 사항](site-recovery-support-matrix-to-azure.md#failed-over-azure-vm-requirements)을 충족하는지 확인합니다.
...
초기 복제가 완료되고 배포를 테스트한 후에는 필요할 때 장애 조치(failover)를 호출할 수 있습니다. 장애 조치(failover)의 종류와 실행 방법에 대해 [자세히 알아보세요](site-recovery-failover.md).
