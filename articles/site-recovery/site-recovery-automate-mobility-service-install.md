...
# Azure 자동화 DSC와 함께 Site Recovery를 사용하여 Azure에 VMware 가상 컴퓨터 복제
...
## 자동화 DSC를 사용하여 Site Recovery 모바일 서비스 배포
...
- 관리 서버에 등록하기 위해 필요한 암호를 저장할 저장소
 ...
 > [AZURE.NOTE] 각 관리 서버에 대해 고유한 암호가 생성됩니다. 여러 관리 서버를 배포하려는 경우 올바른 암호가 passphrase.txt 파일에 저장되어 있는지 확인해야 합니다.

- 보호할 수 있도록 설정하려는 컴퓨터에 WMF(Windows Management Framework) 5.0 설치(자동화 DSC의 요구 사항)

 > [AZURE.NOTE] WMF 4.0이 설치되어 있는 Windows 컴퓨터에 대해 DSC를 사용하려는 경우 [연결이 끊어진 환경에서 DSC 사용](#연결이 끊어진 환경에서 DSC 사용) 섹션을 참조하세요.
...
>[AZURE.NOTE] 에이전트가 정상적으로 연결되고 올바른 암호를 사용하도록 하려면 실제 관리 서버를 반영하도록 구성의 CSIP를 바꾸세요.
...
## 4단계: 자동화 DSC에 컴퓨터 등록
>[AZURE.NOTE] 이 시나리오를 완료하기 위한 필수 구성 요소 중 하나는 WMF의 최신 버전을 설치하여 Windows 컴퓨터를 업데이트하는 것입니다. [다운로드 센터](https://www.microsoft.com/download/details.aspx?id=50395)에서 사용 중인 플랫폼에 적합한 버전을 다운로드하여 설치할 수 있습니다.
...
모바일 서비스를 설치하면 *SystemDrive*\\ProgramData\\ASRSetupLogs에 자체 로그도 생성됩니다.
...
모바일 서비스 에이전트를 배포한 후 가상 컴퓨터에 대해 [복제를 사용하도록 설정](site-recovery-vmware-to-azure.md#step-6-replicate-applications)할 수 있습니다.
