---
title: Azure Site Recovery로 VMware 재해 복구를 위해 Deployment Planner 보고서 분석
description: 이 문서에서는 Azure Site Recovery를 사용하여 Azure로 VMware 재해를 복구하기 위해 Deployment Planner에서 생성된 보고서를 분석하는 방법을 설명합니다.
...
ms.locfileid: "110577244"
---
# <a name="analyze-the-deployment-planner-report-for-vmware-disaster-recovery-to-azure"></a>Azure로 VMware 재해를 복구하기 위해 Deployment Planner 보고서 분석
...
**호환되는 가상 머신 수**: 필요한 네트워크 대역폭, 필요한 수의 스토리지 계정, Microsoft Azure 코어, 구성 서버 및 추가 프로세스 서버를 계산하는 데 호환되는 VM의 총 수입니다.
...
**호환되는 Virtual Machines 수** 는 Site Recovery를 사용하여 Azure로 보호할 수 있는 Virtual Machines의 수입니다. 필요한 네트워크 대역폭, 스토리지 계정 수, Azure 코어 수, 구성 서버 수 및 추가 프로세스 서버 수를 계산하는 데 호환되는 가상 컴퓨터의 총 수입니다. 모든 호환되는 VM의 세부 정보는 "호환 VM" 섹션에서 사용할 수 있습니다.
...
**달성된 처리량:** GetThroughput 명령을 실행한 서버에서 스토리지 계정이 있는 Microsoft Azure 지역으로의 처리량입니다. 이 처리량 숫자는 구성 서버 또는 프로세스 서버 스토리지 및 네트워크 특성이 도구를 실행한 서버의 해당 항목과 동일하게 유지된다는 조건 하에 Site Recovery를 사용하여 호환되는 VM을 보호할 때 달성할 수 있는 예상되는 수준을 나타냅니다.
...
다음 차트에서는 호환되는 모든 VM을 보호하는 데 필요한 스토리지 계정(표준 및 프리미엄)의 총 수를 보여 줍니다. 각 VM에 사용할 스토리지 계정에 대해 알아보려면 &quot;VM 스토리지 배치&quot; 섹션을 참조하세요. Deployment Planner v2.5를 사용하는 경우 데이터를 Managed Disks에 직접 기록하기 때문에 이 권장 사항은 복제에 필요한 표준 캐시 스토리지 계정의 수만 표시합니다.
...
>[!Note]
>Deployment Planner v 2.5부터는 Managed Disks에 직접 복제되는 머신에 대한 스토리지 배치를 권장합니다.

![VM-스토리지 배치](media/site-recovery-vmware-deployment-planner-analyze-report/vm-storage-placement-v2a.png)

**복제 스토리지 유형**: **배치할 VM** 열에 언급된 해당 가상 컴퓨터를 모두 복제하는 데 사용되는 표준 또는 프리미엄 Managed Disks입니다.

**로그 스토리지 계정 유형**: 모든 복제 로그는 표준 스토리지 계정에 저장됩니다.

**스토리지 계정의 제안된 접두사**: 캐시 스토리지 계정 이름을 지정하는 데 사용할 수 있는 3자로 구성된 접두사입니다. 고유한 접두사를 사용할 수 있지만 도구의 제안은 [스토리지 계정에 대 한 파티션 명명 규칙](/en-in/azure/storage/blobs/storage-performance-checklist)을 따릅니다.

**제안된 로그 계정 이름**: 제안된 접두사를 포함한 후 스토리지 계정 이름입니다. 꺾쇠 괄호(< 및 >) 안의 이름을 사용자 지정 입력으로 바꿉니다.

**배치 요약**: 스토리지 유형별로 보호된 VM에 필요한 디스크의 요약입니다. 총 VN 수, 모든 디스크에서 프로비저닝된 총 크기 및 총 디스크 수를 포함합니다.
...
**VM 호환성**: 값은 **예** 및 **예\* *_입니다. _* 예**\*는 VM이 [프리미엄 SSD](../virtual-machines/disks-types.md)에 적합한 경우에 해당합니다. 여기서 프로파일링된 높은 변동량 또는 IOPS 디스크는 P20 또는 P30 범주에 적합하지만 디스크의 크기 때문에 P10 또는 P20에 낮게 매핑됩니다. 스토리지 계정 크기에 따라 디스크를 매핑할 Premium Storage 디스크 유형이 결정됩니다. 예를 들면 다음과 같습니다.
* 128GB 미만은 P10입니다.
* 128GB ~ 256GB는 P15입니다.
* 256GB ~ 512GB는 P20입니다.
...
**복제를 위해 만든 Asrseeddisk(Managed Disks)** : 복제를 사용하도록 설정할 때 생성되는 디스크의 이름입니다. Azure에 데이터와 해당 스냅샷이 저장됩니다.
...
* 데이터 디스크 크기가 잘못되었거나 OS 디스크 크기가 잘못되었습니다. 지원 제한을 [검토](vmware-physical-azure-support-matrix.md#azure-vm-requirements)하세요. 
...
* 평균 데이터 변동률이 디스크의 평균 I/O 크기에 대해 지원되는 Site Recovery 데이터 변동률 한도(20MB/s)를 초과합니다.
...
* 계산된 스냅샷 스토리지가 지원되는 스냅샷 스토리지 한도(10TB)를 초과합니다.
...
**복제 스토리지 대상** | **평균 원본 디스크 I/O 크기** |**평균 원본 디스크 데이터 변동** | **일일 총 원본 디스크 데이터 변동**
---|---|---|---
