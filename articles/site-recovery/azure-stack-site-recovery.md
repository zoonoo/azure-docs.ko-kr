...
> * **5단계: 복제 를 활성화합니다.** 복제 설정을 구성하고 VM에 대한 복제를 사용하도록 설정합니다. 복제를 사용하도록 설정하면 모바일 서비스가 VM에 설치됩니다. Site Recovery가 VM의 초기 복제를 수행하면 진행 중인 복제가 시작됩니다.
...
- sshd_config 구성 파일에서 SFTP 하위 시스템 및 암호 인증을 사용하도록 설정합니다.
    ...
    2. /etc/ssh/sshd_config 파일에서 **PasswordAuthentication**으로 시작하는 줄을 찾습니다. 줄의 주석을 주석 을 해제하고 값을 **예로**변경합니다.
...
5. 대시보드에서 볼트에 빠르게 액세스하려면 **대시보드에** > 대한 핀을**선택합니다.**
   ...
   새 볼트는 **대시보드** > **모든 리소스**및 기본 복구 서비스 자격 증명 **모음** 페이지에 나타납니다.
...
1. **인프라** > 준비**대상에서**사용할 Azure 구독을 선택합니다.
2. 대상 배포 모델을 지정합니다.
...
5. **앱 일치 스냅샷 빈도**에서 애플리케이션 일치 스냅샷이 만들어지는 빈도를 지정합니다.
    ...
    - 앱 일관된 스냅숏은 VM 내의 앱 데이터의 시점 스냅샷입니다.
...
8. 보호를 위해 선택한 모든 컴퓨터에 네트워크 설정을 적용하려면 **선택한 컴퓨터에 대해 지금 구성을** 선택합니다. 각 컴퓨터에 대해 별도로 Azure 네트워크를 선택하려는 경우 **나중에 구성**을 선택합니다.
...
10. **속성** > **구성 속성에서**프로세스 서버가 컴퓨터에 이동성 서비스를 자동으로 설치하는 데 사용할 계정을 선택합니다.
11. **복제 설정에서** > **복제 설정 구성,** 올바른 복제 정책이 선택되어 있는지 확인합니다.
12. **복제 활성화를 클릭합니다.**
13.  >  **설정** > **작업****사이트 복구 작업에서**보호 **활성화** 작업의 진행 률을 추적합니다. **보호 완료** 작업이 실행된 후에는 컴퓨터가 장애 조치(failover)를 수행할 준비가 되어 있습니다.
...
2. 장애 조치(Failover)는 지정된 복구 지점을 사용하여 데이터를 처리합니다.
    ...
    - **최신 앱 일관성**: 컴퓨터가 최신 앱 일관된 복구 지점으로 장애 로 실패합니다.
