...
이 자습서는 Azure VMware Solution VM에 대한 재해 복구를 Azure에 설정하는 방법을 보여주는 시리즈 중 세 번째 자습서입니다. 이전 자습서에서는 Azure로 재해 복구하기 위한 [Azure VMware Solution 환경을 준비](avs-tutorial-prepare-avs.md)했습니다.
...
> [!div class="checklist"]
>
> * 원본 복제 설정 및 Azure VMware Solution 프라이빗 클라우드에서 Site Recovery 구성 서버 설정
> * 복제 대상 설정 지정
> * 복제 정책을 만듭니다.
> * VMware VM을 복제하도록 설정
...
3. 이 자습서에서는 단일 VM을 복제하는 방법을 보여줍니다. 여러 VM을 배포하는 경우 [Deployment Planner 도구](https://aka.ms/asr-deployment-planner)를 사용해야 합니다. [자세히 알아보세요](site-recovery-deployment-planner.md) .
...
> [재해 복구 훈련 실행](avs-tutorial-dr-drill-azure.md)
