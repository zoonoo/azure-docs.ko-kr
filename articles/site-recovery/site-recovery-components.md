...
Site Recovery는 클래식 포털과 Azure 포털 모두에서 지원됩니다. Azure 클래식 포털에서 기존 서비스 관리 모델을 사용하여 Site Recovery를 지원할 수 있습니다. Azure 포털에서 기존 모델 또는 리소스 모델 배포를 지원할 수 있습니다. Azure 포털을 사용하여 배포하는 방법에 대해 [자세히 읽어 봅니다](site-recovery-overview.md#site-recovery-in-the-azure-portal).
...
## Azure에 복제: VMware 가상 컴퓨터 또는 Windows/Linux 물리적 서버
...
- **Hyper-V 호스트**: 하나 이상의 Windows Server 2012 R2 Hyper-V 호스트 서버가 필요합니다. Site Recovery 배포 중에 다음을 설치합니다.
- **Hyper-V VM**: Hyper-V 호스트 서버에 하나 이상의 VM이 필요합니다. Site Recovery 배포 중에 Hyper-V 호스트 서버에서 Azure Site Recovery 공급자 및 Azure 복구 서비스 에이전트 공급자는 인터넷을 통해 사이트 복구 서비스에서의 복제를 조정 및 오케스트레이션합니다. 에이전트는 HTTPS 443을 통해 데이터 복제 데이터를 처리합니다. 공급자 및 에이전트로부터의 통신은 모두 보호 및 암호화됩니다. Azure 저장소에 복제된 데이터도 암호화됩니다.
...
- Azure 포털 배포에 대한 요구 사항에 대해 [자세히 알아봅니다](site-recovery-hyper-v-site-to-azure.md#azure-prerequisites).
- 클래식 포털 배포에 대한 요구 사항에 대해 [자세히 알아봅니다](site-recovery-hyper-v-site-to-azure-classic.md#azure-prerequisites).



## Azure에 복제: VMM에서 관리하는 Hyper-V VM
...
- Azure 포털 배포에 대한 요구 사항에 대해 [자세히 알아봅니다](site-recovery-vmm-to-azure.md#azure-requirements).
- 클래식 포털 배포에 대한 요구 사항에 대해 [자세히 알아봅니다](site-recovery-vmm-to-azure-classic.md#before-you-start).
...
**Azure 계정**: InMage Scout를 사용하여 이 시나리오를 배포합니다. 이를 얻기 위해서는 Azure 구독이 필요합니다. 사이트 복구 자격 증명 모음을 만든 후에 InMage Scout를 다운로드하고 최신 업데이트를 설치하여 배포를 설정합니다. **프로세스 서버(기본 사이트)**: 캐싱, 압축 및 데이터 최적화를 처리하도록 기본 사이트의 프로세스 서버 구성 요소를 설정합니다. 또한 보호하려는 컴퓨터에 대해 통합 에이전트의 푸시 설치를 처리합니다. **VMware ESX/ESXi 및 vCenter 서버(기본 사이트)**: VMware VM을 보호하는 경우 VMware EXS/ESXi 하이퍼바이저 및 필요에 따라 하이퍼바이저를 관리하는 VMware vCenter 서버가 필요합니다.
...
## 보조 사이트에 복제: VMM에서 관리하는 Hyper-V VM


다음과 같이 Site Recovery를 사용하여 System Center VMM에서 관리하지 않는 Hyper-V VM을 보조 데이터 센터에 복제할 수 있습니다.

- **Azure 포털 사용**-Azure 포털에서 Site Recovery를 배포하는 경우 [자세히 알아봅니다](site-recovery-hyper-v-site-to-azure.md).
...
- Azure 포털에서 배포 요구 사항에 대해 [자세히 알아봅니다](site-recovery-vmm-to-vmm.md#azure-prerequisites).
- - Azure 클래식 포털에서 배포 요구 사항에 대해 [자세히 알아봅니다](site-recovery-vmm-to-vmm-classic.md#before-you-start).




## SAN 복제를 사용하여 보조 사이트에 복제: VMM에서 관리하는 Hyper-V VM
...
배포 요구 사항에 대해 [자세히 알아봅니다](site-recovery-vmm-san.md#before-you-start).
### 온-프레미스
