---
title: "Azure 포털을 사용하여 Azure에 VMM 클라우드의 Hyper-V 가상 컴퓨터 복제 | Microsoft Docs"
...
---
# <a name="replicate-hyperv-virtual-machines-in-vmm-clouds-to-azure-using-the-azure-portal"></a>Azure 포털을 사용하여 Azure에 VMM 클라우드의 Hyper-V 가상 컴퓨터 복제
...
전체 배포를 위해서는 이 문서의 모든 단계를 따르는 것이 좋습니다. 하지만 시간이 부족한 경우 다음 추가 정보가 있는 링크를 통해 빠른 요약을 살펴보세요.
...
|  **Azure 제한 사항** |GRS를 사용하는 경우 로깅을 위해 다른 LRS 계정이 필요합니다.<br/><br/> Azure 포털에서 만든 저장소 계정은 리소스 그룹 간에 이동할 수 없습니다.<br/><br/> 프리미엄 저장소는 지원되지 않습니다. |
|  **VM 복제** |VM은 Azure 필수 조건을 준수해야 함](site-recovery-best-practices.md#azure-virtual-machine-requirements)<br/><br/> |
|  **복제 제한 사항** |고정 IP 주소를 사용하여 Linux를 실행하는 VM을 복제할 수 없습니다.<br/><br/> 특정 디스크를 복제에서 제외할 수 있습니다. |
...
Azure에는 리소스를 만들고 작업하는 데, Azure Resource Manager 및 클래식이라는 두 가지 서로 다른 [배포 모델](../resource-manager-deployment-model

> )이 있습니다. Azure에는 두 가지, 즉 Azure 클래식 포털 및 Azure 포털이 있습니다. 이 문서에서는 Azure 포털에 배포하는 방법을 설명합니다. 
> 
> 

Azure 포털의 Site Recovery는 새 기능을 포함합니다.

* Azure 백업 및 Azure Site Recovery Services가 단일 Recovery Services 자격 증명 모음으로 결합되어 있어서 단일 위치에서 BCDR(무중단 업무 방식 및 재해 복구)을 설정하고 관리할 수 있습니다. 통합 대시보드를 통해 온-프레미스 사이트와 Azure 공용 클라우드의 작업을 모니터링하고 관리할 수 있습니다.
* CSP(클라우드 솔루션 공급자) 프로그램이 포함된 Azure 구독이 있는 사용자는 이제 Azure 포털에서 사이트 복구 작업을 관리할 수 있습니다.
* Azure 포털에서 Azure Resource Manager 저장소 계정에 컴퓨터를 복제할 수 있습니다. 장애 조치(Failover) 시 Site Recovery는 Azure에 Resource Manager 기반 VM을 만듭니다.
...
* [자세히 알아보세요](site-recovery-network-mapping.md) .
...
4. [리소스 그룹을 만들거나](../resource-group-template-deploy-portal.md)기존 그룹을 선택합니다. Azure 지역을 지정합니다. 이 지역에 컴퓨터가 복제됩니다. 지원되는 지역을 확인하려면 [Azure 사이트 복구 가격 정보](https://azure.microsoft.com/pricing/details/site-recovery/)
...
시작에서 복제할 컴퓨터 유형과 복제할 위치를 선택합니다. 온-프레미스 서버, Azure 저장소 계정 및 네트워크를 설정합니다. 복제 정책을 만들고 용량 계획을 수행합니다. 인프라를 갖춘 후에 VM에 복제를 사용하도록 설정합니다. 특정 컴퓨터에 대해 장애 조치(Failover)를 실행하거나 여러 컴퓨터를 장애 조치(Failover)하는 복구 계획을 만들 수 있습니다.
...
3. **서버 추가** 블레이드에서 **System Center VMM 서버**가 **서버 형식**에 표시되고 해당 VMM 서버가 [필수 조건 및 URL 요구 사항](#on-premises-prerequisites)을 만족하는지 확인합니다.
...
#### <a name="commandline-installation-for-the-azure-site-recovery-provider"></a>명령줄로 Azure Site Recovery 공급자 설치
...
### <a name="install-the-azure-recovery-services-agent-on-hyperv-hosts"></a>Hyper-V 호스트에 Azure 복구 서비스 에이전트 설치
...
#### <a name="set-up-internet-proxy-access-to-site-recovery-from-hyperv-hosts"></a>Hyper-V 호스트에서 사이트 복구에 대한 인터넷 프록시 액세스를 설정합니다.
...
7.  **속성** > **속성 구성**에서 선택한 VM의 운영 체제 및 OS 디스크를 선택합니다. 그런 후 **OK**를 클릭합니다. 나중에 추가 속성을 설정할 수 있습니다.
