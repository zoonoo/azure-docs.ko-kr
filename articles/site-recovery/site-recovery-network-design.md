...
관리자가 재해 복구 솔루션을 배포하려는 경우 한 가지 핵심적인 질문은 장애 조치를 완료한 후에 가상 컴퓨터를 연결할 수 있는 방법입니다. ASR는 관리자가 장애 후에 어떤 가상 컴퓨터를 네트워크에 연결할 것인지를 선택하게 해줍니다. 주 사이트는 VMM 서버에 의해 관리되며 이것은 네트워크 매핑을 사용하여 달성됩니다. 자세한 내용은 [네트워크 매핑 준비](site-recovery-network-mapping.md)를 참조하세요.
...
Contoso라는 가상의 기업이 전체 서브넷을 장애 조치(failover)하면서 해당 VM을 복구 위치에 복제할 수 있는 방법을 살펴보겠습니다. 먼저 두 개의 온-프레미스 위치 사이에서 VM을 복제하는 동안 Contoso에서 서브넷을 관리하는 방법을 본 다음, [Azure가 재해 복구 사이트](#failover-to-azure)로 사용될 때 서브넷 장애 복구가 어떻게 동작하는지 알아보겠습니다.
...
![장애 조치 전에](./media/site-recovery-network-design/network-design2.png)
...
![장애 조치 후에](./media/site-recovery-network-design/network-design3.png)

장애 조치(failover) 후

보조 사이트는 온-프레미스이며 VMM 서버를 사용하여 관리한 다음, 특정 가상 컴퓨터에 대해 보호를 활성화하면 ASR에서 다음 워크플로에 따라 네트워킹 리소스를 할당합니다.
...
ASR(Azure Site Recovery)는 Microsoft Azure가 가상 컴퓨터에 대한 재해 복구 사이트로 사용되도록 할 수 있습니다. 이 경우 하나 더 많은 제약조건을 처리해야 합니다.
...
![서브넷 장애 조치 전에](./media/site-recovery-network-design/network-design7.png)
...
![서브넷 장애 조치 후에](./media/site-recovery-network-design/network-design9.png)
...
![다른 IP - 장애 조치 전에](./media/site-recovery-network-design/network-design10.png)
...
![다른 IP - 장애 조치 후에](./media/site-recovery-network-design/network-design11.png)
...
[학습](site-recovery-network-mapping.md) 기본 사이트를 관리 하는 VMM 서버를 사용 하는 경우 사이트 복구 원본 및 대상 네트워크를 매핑하는 방법입니다.
