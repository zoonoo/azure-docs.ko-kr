...
Azure Site Recovery를 위한 필수 조건 목록은 [여기](site-recovery-best-practices.md) 그리고 아래의 문서에 언급되어 있으며, Azure에 복제 및 복구할 때 Azure Site Recovery에 지원되는 구성 및 구성 요소를 요약해서 보여줍니다.
...
**Hyper-V(VMM 없음)** | Windows Server 2016, 최신 업데이트가 포함된 Windows Server 2012 R2
...
아래는 Azure Site Recovery를 사용하는 경우 다양한 배포 시나리오에서 지원되는 복제된 운영 체제를 요약한 테이블입니다. 이 지원은 언급된 OS에서 실행되는 **모든 워크로드에 적용**됩니다.

 **VMware/물리적 서버** | **Hyper-V(VMM 없음)** | **Hyper-V(VMM 포함)**
--- | --- | ---
...
**구성** | **VMware/물리적 서버** | **Hyper-V(VMM 없음)** | **Hyper-V(VMM 포함)**
--- | --- | --- | ---
...
SMB 3.0 | 해당 없음 | 예 | 예
SAN(ISCSI) | 예 | 예 | 예
다중 경로(MPIO)<br></br>테스트 제품: Microsoft DSM, EMC PowerPath 5.7 SP4, EMC PowerPath DSM for CLARiiON | VMware의 경우 예<br/><br/> | 예 | 예
...
**VM 유형** | 1세대<br/><br/> 2세대 - Windows | 기본 OS 디스크 형식이 있는 2세대 VM - VHDX로 포맷된 한 두 개의 데이터 볼륨을 포함하며 300GB 미만이 지원됩니다.<br></br>Linux 2세대 VM은 지원되지 않습니다. [자세히 알아보기](https://azure.microsoft.com/blog/2015/04/28/disaster-recovery-to-azure-enhanced-and-were-listening/)|
...
**모바일 서비스** | 온-프레미스 VMware 서버/물리적 서버 및 Azure/보조 사이트 간 복제 조정<br/><br/> 복제하려는 VMware VM 또는 물리적 서버에 설치  | NA(포털에서 사용 가능) | 해당 없음
**MARS(Microsoft Azure Recovery Services) 에이전트** | Hyper-V VM과 Azure 간 복제 조정<br/><br/> 온-프레미스 Hyper-V 서버에 설치(VMM 서버 존재 여부는 관계 없음) | 최신 에이전트([포털에서 제공](http://aka.ms/latestmarsagent)) |
