---
title: "Azure Site Recovery에서 Hyper-V 가상 머신에 대한 장애 복구 | Microsoft Docs"
...
---
# <a name="failback-in-site-recovery-for-hyper-v-virtual-machines"></a>Site Recovery에서 Hyper-V 가상 머신에 대한 장애 복구
...
기본 위치에서 보조 위치로 장애 조치한 후에는 Site Recovery에서 복제된 가상 머신을 보호하지 않으며 보조 위치가 현재 활성 위치로 작동합니다. 이들 절차를 따라 원래 기본 사이트로 장애 복구합니다. 이 절차는 복구 계획에 대해 계획된 장애 조치를 실행하는 방법을 설명합니다. 또는 **Virtual Machines** 탭에서 단일 Virtual Machines에 대한 장애 조치를 실행할 수 있습니다.
...
3. Azure로부터 장애 복구하는 경우 **데이터 동기화**:에서 설정을 선택합니다.
   ...
   * **장애 조치(failover) 전 데이터 동기화(델타 변경 내용만 동기화)** - 가상 머신을 종료하지 않고 동기화할 때 가상 머신의 가동 중지 시간을 최소화하는 옵션입니다. 다음 단계를 수행합니다.
    ...
    - **장애 조치 중에만 데이터 동기화(전체 다운로드)** - 오랜 시간 동안 Azure에서 실행한 경우 이 옵션을 사용합니다. 대부분의 디스크가 변경되고 체크섬 계산에 시간을 소모하지 않으므로 이 옵션이 더 빠릅니다. 디스크의 다운로드를 수행합니다. 온-프레미스 가상 머신이 삭제되었을 때도 유용합니다.
