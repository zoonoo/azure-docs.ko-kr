...
6. 장애 조치 이전에 데이터를 동기화하는 옵션을 선택한 경우 초기 데이터 동기화를 완료하고 Azure에서 가상 컴퓨터를 종료할 준비가 되었을 때 **작업** > <planned failover job name> **장애 조치(failover) 완료**를 차례로 클릭합니다. 그러면 Azure 컴퓨터가 종료되고, 온-프레미스 가상 컴퓨터로 최신 변경 내용이 전송되며, 온-프레미스 VM이 시작됩니다.
