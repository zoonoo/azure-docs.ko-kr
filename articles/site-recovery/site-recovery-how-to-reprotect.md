...
* *마스터 대상 서버에 저장소 vMotion을 사용할 수 없습니다*. 이 때문에 장애 복구에 오류가 발생할 수 있습니다. 디스크를 사용할 수 없으므로 가상 컴퓨터가 시작되지 않습니다. 이 문제를 방지하려면 vMotion 목록에서 마스터 대상 서버를 제외합니다.
...
> [!TIP]
> 장애 조치 중에 항상 Azure 기반 프로세스 서버를 사용하는 것이 좋습니다. 프로세스 서버가 복제 가상 컴퓨터와 가까울 수록(Azure에서 장애 조치된 시스템) 복제 성능이 더 높습니다. 그러나 개념 증명 또는 데모 중에 POC를 더 빨리 완료하기 위해 개인 피어링을 사용하는 ExpressRoute와 함께 온-프레미스 프로세스 서버를 사용할 수 있습니다.
...
* 가상 컴퓨터가 vCenter 서버의 온-프레미스에 없는 경우 사이트 복구 서비스를 통해 다시 보호하는 중에 새 가상 컴퓨터를 만들어야 합니다. 이 가상 컴퓨터는 마스터 대상을 만드는 ESX 호스트에서 만들어집니다. ESX 호스트를 신중하게 선택하여 원하는 호스트에 장애 복구 가상 컴퓨터를 만듭니다.

* *마스터 대상 서버에 저장소 vMotion을 사용할 수 없습니다*. 이 때문에 장애 복구에 오류가 발생할 수 있습니다. 디스크를 사용할 수 없으므로 가상 컴퓨터가 시작되지 않습니다.

> [!WARNING]
> 마스터 대상에서 저장소 vMotion 게시 다시 보호를 수행하는 경우 마스터 대상에 연결되어 있는 보호된 가상 컴퓨터 디스크가 vMotion의 대상으로 마이그레이션됩니다. 이 작업 후 장애 복구(failback)를 수행하려고 하면 디스크 분리에 실패하고 디스크가 없음이 표시됩니다. 그러면 저장소 계정에서 디스크를 찾기가 매우 어려워집니다. 디스크를 수동으로 찾아 가상 컴퓨터에 연결해야 합니다. 그리고 나면 온-프레미스 가상 컴퓨터를 부팅할 수 있습니다.
...
> [!NOTE]
> 온-프레미스 가상 컴퓨터가 다시 보호 중에 꺼집니다. 복제 중 데이터 일관성을 보장하기 위해서 입니다. 다시 보호가 완료된 후 가상 컴퓨터를 켜지 마십시오.
