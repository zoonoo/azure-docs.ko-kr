...
네트워크 매핑은 VMM 및 사이트 복구 배포 시 중요한 요소입니다. 대상 Hyper-V 호스트 서버에 복제된 가상 컴퓨터를 최적으로 배치하고 장애 조치(Failover) 후 복제된 가상 컴퓨터가 적합한 네트워크에 연결되도록 합니다. 이 문서는 네트워크 매핑을 설명하고 네트워크 매핑 작동 원리를 이해하는 데 도움이 되는 몇 가지 예를 제공합니다.
...
네트워크 매핑 설정 방법은 사이트 복구 배포 시나리오에 따라 다릅니다.



- **온-프레미스 간 VMM 서버**—다음을 수행하도록 원본 VMM 서버의 VM 네트워크와 대상 VMM 서버의 VM 네트워크 사이의 네트워크 매핑을 매핑합니다.
...
- **온-프레미스 VMM 서버와 Azure**—네트워크 매핑은 원본 다음을 수행하도록 VMM 서버의 VM 네트워크와 대상 Azure 네트워크 간을 매핑합니다.
...
VMM 논리 네트워크는 실제 네트워크 인프라의 추상적 뷰를 제공합니다. VM 네트워크는 가상 컴퓨터가 논리 네트워크에 연결할 수 있도록 네트워크 인터페이스를 제공합니다. 논리 네트워크에는 VM 네트워크가 하나 이상 필요합니다. 보호를 위해 클라우드에 가상 컴퓨터를 배치할 때 클라우드와 연결된 논리 네트워크에 연결하는 VM 네트워크에 연결되어야 합니다. 자세한 정보:

- [논리 네트워크(1부)](http://blogs.technet.com/b/scvmm/archive/2013/02/14/networking-in-vmm-2012-sp1-logical-networks-part-i.aspx)
- [VMM 2012 SP1에서 가상 네트워킹](http://blogs.technet.com/b/scvmm/archive/2013/01/08/virtual-networking-in-vmm-2012-sp1.aspx)
...
VMM 클라우드를 예로 든 조직 및 클라우드와 연결된 논리 네트워크에서 설정하는 방법은 다음과 같습니다.
...
이러한 설정으로 몇 가지 가능한 시나리오에서 발생하는 내용을 검토해 보겠습니다.

**시나리오** | **결과**
---|---
장애 조치 후 VM-2의 네트워크 속성에는 변경 사항이 없음 | VM-1에 원본 네트워크에 연결된 상태로 유지됩니다.
VM-2의 네트워크 속성은 장애 조치 후 변경되고 연결이 끊김 | VM-1은 연결이 끊김
VM-2의 네트워크 속성은 장애 조치 후 변경되고 VMNetwork2-시카고에 연결됨 | VMNetwork2-시카고가 매핑되지 않는 경우 VM-1은 연결이 끊김
VMNetwork1-시카고의 네트워크 매핑이 변경됨 | VM-1은 VMNetwork1-시카고에 이제 매핑된 네트워크에 연결됨
