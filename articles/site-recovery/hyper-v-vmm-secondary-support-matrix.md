---
title: Azure 사이트 복구를 사용하여 보조 VMM 사이트에 대한 매트릭스 하이퍼-V 재해 복구 지원
...
이 문서에서는 [Azure 사이트 복구](site-recovery-overview.md) 서비스를 사용하여 VMM(시스템 센터 가상 시스템 관리자) 클라우드에서 관리되는 Hyper-V VM을 보조 사이트로 복제할 때 지원되는 내용을 요약합니다. Hyper-V VM을 Azure에 복제하려면 [이 지원 매트릭스](hyper-v-azure-support-matrix.md)를 검토하세요.
