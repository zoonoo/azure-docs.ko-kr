...
**프로파일링된 Hyper-V 서버의 수**: VM의 보고서가 생성된 Hyper-V 서버의 수입니다. 숫자를 선택하여 Hyper-V 서버의 이름을 확인합니다. 온-프레미스 스토리지 요구 사항 시트가 열려 모든 서버가 해당 스토리지 요구 사항과 함께 표시됩니다. 
...
**호환 가능한 가상 머신 수**: Azure Site Recovery를 사용하여 Azure로 보호할 수 있는 VM 수입니다. 필요한 네트워크 대역폭, 스토리지 계정 수, Azure 코어 수가 계산되는 호환 가능한 총 VM 수입니다. 모든 호환되는 VM의 세부 정보는 "호환 VM" 섹션에서 사용할 수 있습니다.
...
다음 차트에서는 호환되는 모든 VM을 보호하는 데 필요한 스토리지 계정(표준 및 프리미엄)의 총 수를 보여 줍니다. 각 VM에 사용할 저장소 계정에 대해 알아보려면 "VM 저장소 배치" 섹션을 참조하세요.
...
**복제 비용**: 복제 중에 발생하는 비용입니다. 스토리지, 네트워크 및 Site Recovery 라이선스 비용이 포함됩니다. 
...
**제안된 접두사**: 스토리지 계정의 이름을 지정하는 데 사용할 수 있는 3자로 구성된 접두사 제안입니다. 고유한 접두사를 사용할 수 있지만 도구의 제안은 [스토리지 계정에 대 한 파티션 명명 규칙](https://aka.ms/storage-performance-checklist)을 따릅니다.

**제안된 계정 이름**: 제안된 접두사 다음에 포함시킬 스토리지 계정 이름입니다. 꺾쇠 괄호(< 및 >) 안의 이름을 사용자 지정 입력으로 바꿉니다.

**로그 스토리지 계정**: 모든 복제 로그는 표준 스토리지 계정에 저장됩니다. Premium Storage 계정에 복제하는 VM의 경우 로그 스토리지에 대한 추가 표준 스토리지 계정을 설정합니다. 여러 프리미엄 복제 스토리지 계정에서 단일 표준 로그 스토리지 계정을 사용할 수 있습니다. 표준 스토리지 계정에 복제된 VM은 로그에 대해 동일한 스토리지 계정을 사용합니다.
...
프리미엄 스토리지 | 16KB 이상| VM 당 20 MB/s | VM 당 1684 GB
