...
# 1단계: Azure로 Hyper-V 복제를 위한 아키텍처 검토
<a id="step-1-review-the-architecture-for-hyper-v-replication-to-azure" class="xliff"></a>


이 문서에서는 Azure Portal에서 [Azure Site Recovery](site-recovery-overview.md) 서비스를 사용하여 온-프레미스 Hyper-V 가상 컴퓨터(System Center VMM에서 관리되지 않음)를 Azure에 복제할 때 포함된 구성 요소 및 프로세스를 설명합니다.
...
VMM 없이 Hyper-V VM을 Azure에 복제하는 경우 여러 가지 구성 요소가 관련됩니다.
...
**Hyper-V** | Hyper-V 호스트 및 클러스터는 Hyper-V 사이트에 수집됩니다. Azure Site Recovery 공급자와 Recovery Services 에이전트는 각 Hyper-V 호스트에 설치됩니다. | 공급자는 인터넷을 통한 사이트 복구로 복제를 오케스트레이션합니다. Recovery Services 에이전트는 데이터 복제를 처리합니다.<br/><br/> 공급자 및 에이전트로부터의 통신은 모두 보호 및 암호화됩니다. Azure 저장소에 복제된 데이터도 암호화됩니다.
