...
1. 대상 스토리지 계정(주 지역의 원래 스토리지 계정)이 없는 경우 새로 만들어집니다. 할당 된 저장소 계정 이름은 보조 VM에서 사용 하는 저장소 계정의 이름이 며 `asr`접미사가 붙습니다.
...
1. VM이 관리 디스크를 사용 하는 경우에는 `-ASRReplica` 접미사로 원본 디스크의 복사본을 만듭니다. 원래 디스크가 삭제됩니다. `-ASRReplica` 복사본은 복제에 사용 됩니다.
...
1. VM이 관리 디스크를 사용 하는 경우 `-ASRReplica` 접미사로 복제본 디스크가 생성 됩니다. `-ASRReplica` 복사본은 복제에 사용 됩니다.
...
|---|---|
|원본 영역에 1TB standard 디스크가 있는 VM이 1 개 있습니다.<br/>127 GB 데이터만 사용 되며 나머지 디스크는 비어 있습니다.<br/>디스크 유형은 표준 이며 60 S p/S 처리량이 있습니다.<br/>장애 조치 (failover) 후 데이터가 변경 되지 않습니다.| 대략적인 시간: 45 분 ~ 1.5 시간<br/>다시 보호 하는 동안 Site Recovery 127 g b/45 Mb를 사용 하는 모든 데이터의 체크섬 (약 45 분)을 채웁니다.<br/>일부 오버 헤드 시간은 Site Recovery 자동 크기 조정에 약 20-30 분이 소요 됩니다.<br/>송신 요금이 청구 되지 않습니다. |
|원본 영역에 1TB standard 디스크가 있는 VM이 1 개 있습니다.<br/>127 GB 데이터만 사용 되며 나머지 디스크는 비어 있습니다.<br/>디스크 유형은 표준 이며 60 S p/S 처리량이 있습니다.<br/>장애 조치 (failover) 후 45 GB 데이터 변경| 대략적인 시간: 1 시간-2 시간<br/>다시 보호 하는 동안 Site Recovery 127 g b/45 Mb를 사용 하는 모든 데이터의 체크섬 (약 45 분)을 채웁니다.<br/>45 45 GB/45 MBps (약 17 분)의 변경 내용을 적용 하기 위한 전송 시간입니다.<br/>송신 요금은 체크섬이 아닌 45 GB 데이터 변경에 대 한 것입니다. |
