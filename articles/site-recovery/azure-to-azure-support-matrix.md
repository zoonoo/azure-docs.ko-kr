...
유럽 | 영국 서부, 영국 남부, 북유럽, 서유럽, 남아프리카 공화국 서부, 남아프리카 공화국, 노르웨이 동부, 노르웨이 서부
아시아 | 남인도, 인도 중부, 서인도, 동남아시아, 동아시아, 일본 동부, 일본 서부, 한국 중부, 한국
...
국내 재해 복구를 위해 예약된 제한된 지역 |독일 북부 는 독일 서중부, 스위스 서부 는 스위스 북부, 프랑스 남부는 프랑스 중부, UAE 중부 는 UAE 북부 고객을 위해 예약
...
Premium Storage | 지원 안 함 | 표준 저장소 계정은 비용을 최적화하는 데 도움이 되는 캐시 저장소에 사용됩니다.
...
RBAC 정책 | 지원 안 함 | VM의 역할 기반 액세스 제어(RBAC) 정책은 대상 지역의 장애 조치 VM에 복제되지 않습니다.
확장 | 지원 안 함 | 확장은 대상 지역의 장애 조치 VM에 복제되지 않습니다. 장애 조치 후 수동으로 설치해야 합니다.
...
콜드 및 핫 스토리지 | 지원 안 함 | VM 디스크는 콜드 및 핫 스토리지에서 지원되지 않습니다.
스토리지 공간 | 지원됨 |
...
핫 제거 디스크    | 지원 안 함 | VM에서 데이터 디스크를 제거하는 경우 복제를 사용하지 않도록 설정하고 VM에 대해 복제를 다시 활성화해야 합니다.
...
RA-GRS | 지원됨 |
ZRS | 지원 안 함 |
콜드 및 핫 스토리지 | 지원 안 함 | 가상 머신 디스크는 콜드 및 핫 스토리지에서 지원되지 않습니다.
...
여러 IP 주소 | 지원 안 함 | IP 주소가 여러 개인 NIC가 있는 VM을 장애 조치하면 원본 지역의 NIC의 기본 IP 주소만 유지됩니다. 여러 IP 주소를 할당하려면 [복구 계획에](recovery-plan-overview.md) VM을 추가하고 스크립트를 첨부하여 계획에 추가 IP 주소를 할당하거나 장애 조치 후 수동으로 또는 스크립트로 변경할 수 있습니다.
