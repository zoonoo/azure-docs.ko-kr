...
---
# <a name="reprotect-azure-vms-back-to-the-primary-region"></a>Azure VM에서 주 지역으로 다시 보호



>[!NOTE]
>
> Azure VM(가상 머신)에 대한 Site Recovery 복제는 현재 미리 보기로 제공됩니다.


한 Azure 지역에서 다른 지역으로 VM을 [장애 조치](../site-recovery-failover.md)하면 장애 조치된 VM은 보호되지 않은 상태에 있습니다. 이러한 VM을 주 지역으로 다시 가져오려면 먼저 VM을 복제한 다음 다시 장애 조치해야 합니다. 방향에 따른 장애 조치 방법에는 차이점이 없습니다. 마찬가지로, VM 복제를 활성화한 후 다시 보호 사후 장애 조치 또는 장애 복구 간에 차이점은 없습니다.

다시 보호 프로세스를 설명하기 위해 예를 들어 보호된 VM의 주 사이트가 동아시아 지역이고 복구 사이트가 동남 아시아라고 가정합니다. 장애 조치 도중에 VM을 동남 아시아 지역으로 장애 조치합니다. 장애 복구 전에 VM을 동남 아시아에서 동아시아로 다시 복제해야 합니다. 이 문서에서는 다시 보호에 필요한 단계를 설명합니다.

> [!WARNING]
> 가상 컴퓨터를 마이그레이션하고 또 다른 리소스 그룹으로 이동했거나 Azure 가상 컴퓨터를 삭제한 경우 장애 복구를 수행할 수 없습니다.

다시 보호가 완료되고 VM이 복제 중이면 VM에 대한 장애 조치를 시작하여 다시 동아시아 지역으로 가져올 수 있습니다.

## <a name="prerequisites"></a>필수 조건
1. VM이 커밋되었어야 합니다.
2. 대상 사이트(동아시아)가 사용 가능하고 해당 지역에서 새 리소스를 액세스/생성할 수 있어야 합니다.

## <a name="reprotect"></a>다시 보호

기본 설정을 사용하여 VM을 다시 보호하려면 다음 단계를 수행합니다.

1. **자격 증명 모음** > **복제된 항목**에서 장애 조치된 VM을 마우스 오른쪽 단추로 클릭한 다음 **다시 보호**를 선택합니다. 컴퓨터를 클릭하고 명령 단추에서 **다시 보호**를 선택할 수도 있습니다.

  ![다시 보호](./media/site-recovery-how-to-reprotect-azure-to-azure/reprotect.png)

2. 복제 방향 **동남 아시아에서 동아시아로**를 선택했는지 확인합니다.

  ![다시 보호](./media/site-recovery-how-to-reprotect-azure-to-azure/reprotectblade.png)

3. **리소스 그룹, 네트워크, 저장소 및 가용성 집합** 정보를 검토하고 **확인**을 클릭합니다. (신규)가 표시된 리소스가 있는 경우 이러한 리소스는 다시 보호 중에 생성됩니다.

그러면 대상 사이트를 최신 데이터로 시드하는 다시 보호 작업이 트리거되고 이 작업이 완료된 후 동남 아시아로 장애 복구하기 전에 델타를 복제합니다.

### <a name="reprotect-customization"></a>다시 보호 사용자 지정
다시 보호하는 동안 추출 저장소 계정 또는 네트워크를 선택하려면 다시 보호 페이지에 제공된 사용자 지정 옵션을 사용하여 수행할 수 있습니다.
...
|대상 Virtual Network     | 다시 보호하는 동안 네트워크를 변경할 수 없습니다. 네트워크를 변경하려면 네트워크 매핑을 다시 실행합니다.         |
...
가상 컴퓨터가 보호된 상태가 되면 장애 조치를 시작할 수 있습니다. 장애 조치에서는 동아시아 Azure 지역의 가상 컴퓨터를 종료한 다음 동남 아시아 지역 가상 컴퓨터를 만들고 부팅합니다. 이로 인해 응용 프로그램의 가동 중지 시간이 짧습니다. 따라서 응용 프로그램에서 가동 중지 시간을 허용할 수 있는 경우 장애 조치 시간을 선택합니다. 장애 조치를 시작하기 전에 먼저 가상 머신에 대한 테스트 장애 조치를 실행하여 올바르게 작동하는지 확인하는 것이 좋습니다.
