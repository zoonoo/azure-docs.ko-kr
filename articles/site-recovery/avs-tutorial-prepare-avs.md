...
이 자습서는 Azure VMware Solution VM에 대한 재해 복구를 Azure에 설정하는 방법을 보여주는 시리즈 중 두 번째 자습서입니다. 첫 번째 자습서에서는 Azure VMware Solution 재해 복구에 필요한 [Azure 구성 요소를 설정](avs-tutorial-prepare-azure.md)했습니다.
...
[이 시리즈의 첫 번째 자습서](avs-tutorial-prepare-azure.md)에 설명된 대로 Azure가 준비되었는지 확인합니다.
...
1. Azure VMware 솔루션 [소프트웨어 버전](../azure-vmware/concepts-private-clouds-clusters.md#vmware-software-versions)을 확인합니다.
2. [VMware 서버 요구 사항](vmware-physical-azure-support-matrix.md#on-premises-virtualization-servers)을 확인합니다.
3. Linux VM의 경우 파일 시스템 및 스토리지 요구 사항을 [확인](vmware-physical-azure-support-matrix.md#linux-file-systemsguest-storage)합니다. 
...
- 장애 조치 전에 Azure VMware Solution VM에서 시스템 부팅 시 Secure Shell 서비스가 자동으로 시작되도록 설정되어 있는지 확인합니다.
...
Azure VMware Solution 클라우드로 장애 복구(failback)하려는 경우 [장애 복구(failback)에 대한 필수 구성 요소](avs-tutorial-reprotect.md#before-you-begin)가 몇 가지 있습니다. 지금 준비할 수 있지만 꼭 해야 하는 것은 아닙니다. Azure로 장애 조치(failover)한 후에 준비할 수 있습니다.
...
> [재해 복구 설정](avs-tutorial-replication.md)
- 여러 VM을 복제하는 경우에는 [용량을 계획](site-recovery-deployment-planner.md)합니다.
