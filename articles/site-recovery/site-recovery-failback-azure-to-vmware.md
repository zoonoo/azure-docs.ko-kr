...
* 실제 서버를 장애 조치한 경우 장애 복구는 항상 새 VMware VM으로 수행됩니다.
  * 물리적 컴퓨터를 장애 복구(failback)하기 전에 다음 사항에 유의하세요.
    * 보호된 물리적 컴퓨터는 Azure에서 VMware로 다시 장애 조치(failover)된 경우 가상 컴퓨터로 복구됩니다. 보호되고 Azure로 장애 조치된 Windows Server 2008 R2 SP1 컴퓨터는 장애 복구할 수 없습니다.
...
5. *클래식 배포* 네트워크를 선택하면 Azure 갤러리를 통해 새로운 VM을 만들고 그 안에 프로세스 서버를 설치할 것인지에 대한 질문을 받게 됩니다.
   ...
   2.  [여기에 주어진](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)
...
> [!NOTE]
> Linux에 마스터 대상 서버를 설치하려는 경우 다음 절차의 지침을 따릅니다. MT OS에 최소 운영 체제 CentOS 6.6만 사용해야 합니다.
> 
> 
...
3. 보호하려는 원본 컴퓨터가 루트 또는 부팅 장치에 대해 Linux wit Reiser 또는 XFS 파일 시스템을 실행하는 경우 다음과 같이 추가 패키지를 다운로드하고 설치해야 합니다.
   ...
   * # <a name="yum-install-device-mapper-multipath---this-is-required-to-enable-multipath-packages-on-the-mt-server"></a>yum install device-mapper-multipath 이는 MT 서버에서 다중 경로 패키지를 사용하도록 설정하는 데 필요합니다.
...
5. 프로세스 서버에서 CS에 연결할 수 없는 경우 포트 443에서 CS 컴퓨터에 대한 텔넷을 사용하여 프로세스 서버와 CS 간의 연결을 확인할 수 있습니다. 또한 PS 컴퓨터에서 CS를 ping할 수도 있습니다. PS 서버는 CS에 연결된 경우 하트비트가 있어야 합니다.
6. 대체 vCenter로 장애 복구(failback)하려면 검색된 마스터 대상 서버에서 새 vCenter가 검색되어야 합니다. 일반적인 증상으로, 다시 보호 대화 상자에서 데이터 저장소에 액세스하거나 데이터 저장소를 볼 수 없습니다.
7. 물리적 또는 가상 온-프레미스 컴퓨터로 보호된 WS2008R2SP1 컴퓨터는 Azure에서 온-프레미스로 장애 복구할 수 없습니다.
