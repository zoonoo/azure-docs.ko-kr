...
> [!NOTE]
> 이 자습서는 최소 단계로 DR 훈련을 수행하는 데 도움이 됩니다. 네트워킹 고려 사항, 자동화 또는 문제 해결을 포함하여 DR 훈련 수행과 관련된 다양한 측면에 대해 자세히 알아보려면 Azure VM에 대한 ‘방법’에 있는 문서를 참조하세요.
...
- 각 NIC의 원본 네트워킹 구성에 따라 선택적으로 **서브넷, IP 주소, 공용 IP, 네트워크 보안 그룹 또는 내부 Load Balancer**를 지정하여 DR 훈련을 수행하기 전에 컴퓨팅 및 네트워크의 테스트 장애 조치(failover) 설정에서 각 NIC에 연결할 수 있습니다.
...
2. **테스트 장애 조치**에서 장애 조치에 사용할 복구 지점을 선택합니다.
    ...
    - **가장 최근에 처리됨**: VM을 Site Recovery에서 처리된 최신 복구 시점으로 장애 조치(failover)합니다. 타임스탬프가 표시됩니다. 이 옵션을 사용하면 데이터를 처리하는 데 시간을 소비하지 않으므로 낮은 RTO(복구 시간 목표)가 제공됩니다.
   - **최신 앱 일치**: 모든 VM을 최신 앱 일치 복구 시점으로 장애 조치합니다. 타임스탬프가 표시됩니다.
   - **사용자 지정**: 특정 복구 지점으로 장애 조치(failover)합니다. 사용자 지정은 단일 VM을 장애 조치(failover)할 때만 사용할 수 있으며 복구 계획을 사용한 장애 조치(failover)에는 사용할 수 없습니다.

3. 장애 조치가 발생한 후에 보조 지역의 Azure VM을 연결할 대상 Azure 가상 네트워크를 선택합니다.

    > [!NOTE]
    > 복제된 항목에 대해 테스트 장애 조치(failover) 설정이 미리 구성된 경우 Azure 가상 네트워크를 선택하는 드롭다운이 표시되지 않습니다.
