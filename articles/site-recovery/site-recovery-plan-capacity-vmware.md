...
3. 측정된 일일 데이터 변경률을 기준으로 [크기 지침](vmware-azure-set-up-process-server-scale.md#download-installation-file)에 따라 [스케일 아웃 프로세스 서버](site-recovery-plan-capacity-vmware.md#size-recommendations-for-the-process-server)를 배포합니다.
