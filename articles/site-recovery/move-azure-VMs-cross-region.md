...
ms.locfileid: "56875823"
---
# <a name="move-azure-vms-to-another-region"></a>다른 지역으로 Azure VM 이동

기존 VM의 신뢰성 및 가용성 개선, 관리 효율성 개선, 거버넌스 등 다양한 이유로 기존 Azure IaaS 가상 머신을 한 Azure 지역에서 다른 Azure 지역으로 이동하려는 다양한 시나리오가 있습니다. 
...
> [!IMPORTANT]
> 이 문서에서는 Azure VM을 한 Azure 지역에서 다른 Azure 지역으로 이동하는 방법을 안내하며, VM을 가용성 영역으로 이동하여 인프라 가용성을 높여야 하는 요구 사항이 있는 경우 여기 자습서를 참조하세요.
...
4. 원본 네트워킹 레이아웃 및 현재 확인에 사용하는 모든 리소스(부하 분산 장치, NSG, 공용 IP 등)를 식별하고 나열합니다.
...
5. 기본 대상 리소스를 사용해도 되고 미리 만들어 둔 리소스를 사용해도 됩니다.
...
> [!IMPORTANT]
> 이동 후 Site Recovery 복제 요금이 청구되지 않도록 위의 단계를 수행해야 합니다.
