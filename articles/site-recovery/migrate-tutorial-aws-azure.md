...
- 장애 조치(failover)는 데이터를 처리하며 이 데이터에서 Azure VM이 만들어질 수 있습니다. 최신 복구 시점을 선택하는 경우 해당 데이터에서 복구 시점이 만들어집니다.
...
2. **장애 조치(Failover)** 에서 장애 조치할 **복구 시점**을 선택합니다. 최근 복구 시점을 선택하고 장애 조치(failover)를 시작합니다. **작업** 페이지에서 장애 조치 진행 상황 확인을 수행할 수 있습니다.
