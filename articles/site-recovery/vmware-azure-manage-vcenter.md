...
온-프레미스 VMware Vm에 대 한 재해 복구를 설정 하는 경우 Site Recovery 프로세스 서버가 자동으로 Vm을 검색 하 고 필요에 따라 장애 조치 (failover) 할 수 있도록 Site Recovery vCenter Server/vSphere 호스트에 액세스할 수 있어야 합니다. 기본적으로 프로세스 서버는 Site Recovery 구성 서버에서 실행 됩니다. 구성 서버가 다음과 같이 vCenter Server/vSphere 호스트에 연결할 계정을 추가 합니다.

1. 구성 서버 컴퓨터에 로그인 합니다.
2. 바탕 화면 바로 가기를 사용 하 여 구성 서버 도구 (cspsconfigtool .exe)를 엽니다.
3. **계정 관리** 탭에서 **계정 추가**를 클릭 합니다. 

   ![add-account](./media/vmware-azure-manage-vcenter/addaccount.png)
4. 계정 세부 정보를 입력하고 **확인**을 클릭하여 계정을 추가합니다.  계정에는 다음 표에 요약된 권한이 있어야 합니다. 
...
2. **세부 정보** 페이지에서 **vCenter**를 클릭 합니다.
3. **Vcenter 추가**에서 vsphere 호스트 또는 vCenter server의 이름을 지정 합니다.
4. 서버의 IP 주소 또는 FQDN을 지정 합니다.
5. VMware 서버가 다른 포트에서 요청을 수신하도록 구성되지 않은 경우 포트를 443으로 그대로 둡니다.
6. VMware vCenter 또는 vSphere ESXi 서버에 연결 하는 데 사용할 계정을 선택 합니다. 그런 후 **OK**를 클릭합니다.
...
3. 새 계정 정보를 입력 하 고 **확인**을 클릭 합니다. 계정에는 [위에](#account-permissions)나열 된 사용 권한이 필요 합니다.
4. 자격 증명 모음 > **Site Recovery 인프라** > **구성**서버에서 구성 서버를 엽니다.
5. **자세히**에서 **서버 새로 고침**을 클릭 합니다.
6. 서버 새로 고침 작업이 완료 되 면 vCenter Server를 선택 합니다.
7. **요약**에서 **Center Server/vsphere 호스트 계정**에 새로 추가 된 계정을 선택 하 고 **저장**을 클릭 합니다.
...
2. **VCenter 서버**에서 수정 하려는 vCenter Server를 클릭 합니다.
5. **요약**에서 IP 주소와 포트를 업데이트 하 고 변경 내용을 저장 합니다.
...
새 vCenter Server을 사용 하도록 모든 Vm을 마이그레이션하려면 vCenter Server 할당 된 IP 주소만 업데이트 해야 합니다. 중복 된 항목이 있을 수 있으므로 다른 VMware 계정을 추가 하지 마세요. 다음과 같이 주소를 업데이트 합니다.

1. 자격 증명 모음에서 **Site Recovery 인프라** > **구성**서버를 > vCenter Server 추가 된 구성 서버를 설정 합니다.
2. **VCenter servers** 섹션에서 마이그레이션할 vCenter Server를 클릭 합니다.
5. **요약**에서 새 vCenter Server IP 주소를 업데이트 하 고 변경 내용을 저장 합니다.
6. IP 주소가 업데이트 되는 즉시 Site Recovery 새 vCenter Server에서 VM 검색 정보를 수신 하기 시작 합니다. 진행 중인 복제 작업에는 영향을 주지 않습니다.

## <a name="migrate-a-few-vms-to-a-new-server"></a>몇 개의 Vm을 새 서버로 마이그레이션

복제 하는 Vm 중 일부를 새 vCenter 서버로 마이그레이션하려면 다음을 수행 합니다.

1. 구성 서버에 새 vCenter Server를 [추가](#add-vmware-server-to-the-vault) 합니다.
2. 새 서버로 이동 하는 Vm에 대해 [복제를 사용 하지 않도록 설정](site-recovery-manage-registration-and-protection.md#disable-protection-for-a-vmware-vm-or-physical-server-vmware-to-azure) 합니다.
3. VMware에서 Vm을 새 vCenter Server 마이그레이션합니다. 
4. 새 vCenter Server을 선택 하 여 마이그레이션된 Vm에 대 한 복제를 다시 [사용 하도록 설정](vmware-azure-tutorial.md#enable-replication) 합니다.


## <a name="migrate-most-vms-to-a-new-server"></a>새 서버로 대부분의 Vm 마이그레이션
 새 vCenter Server로 마이그레이션하려는 Vm의 수가 원래 vCenter Server에 남아 있는 Vm의 수보다 크면 다음을 수행 합니다.

1. 구성 서버 설정의 vCenter Server에 할당 된 [IP 주소](#modify-the-ip-address-and-port) 를 새 vCenter Server의 주소로 업데이트 합니다.
2. 이전 서버에 남아 있는 몇 개의 Vm에 대해 [복제를 사용 하지 않도록 설정](site-recovery-manage-registration-and-protection.md#disable-protection-for-a-vmware-vm-or-physical-server-vmware-to-azure) 합니다.
3. [이전 vCenter Server](#add-vmware-server-to-the-vault) 와 해당 IP 주소를 구성 서버에 추가 합니다.
4. 이전 서버에 남아 있는 Vm에 대 한 [복제를 다시 사용 하도록 설정](vmware-azure-tutorial.md#enable-replication) 합니다.
 
 ## <a name="next-steps"></a>다음 단계
문제가 발생 하는 경우 vCenter Server 오류 [문제를 해결](vmware-azure-troubleshoot-vcenter-discovery-failures.md) 하십시오.
