...
2. 삭제할 VMM 서버에서 클라우드에 사용된 모든 네트워크 매핑을 삭제합니다. **시스템 센터 VMM** > **네트워크 매핑용** **사이트 복구 인프라에서** > 네트워크 매핑을 마우스 오른쪽 > **삭제합니다.**
3. VMM 서버의 ID를 메모해 둡니다.
4. 제거할 VMM 서버의 클라우드에서 복제 정책을 연결 해제합니다.  **시스템 센터 VMM** >  복제 정책에 대한 **사이트 복구 인프라에서** > 연결된 정책을 두 번 클릭합니다.**Replication Policies** 클라우드를 마우스 오른쪽 단추로 클릭하고 > **연결 해제**를 클릭합니다.
5. VMM 서버 또는 활성 노드를 삭제합니다. **시스템 센터 VMM** > **VMM 서버의**사이트 **복구 인프라에서** >  **서버를**마우스 오른쪽 > 삭제합니다.
6. VMM 서버가 연결 분리 상태인 경우 VMM 서버에서 [정리 스크립트](https://aka.ms/asr-cleanup-script-vmm)를 다운로드하여 실행합니다. 기본(LocalMachine) 범위에 대한 실행 정책을 변경하려면 **관리자로 실행** 옵션을 사용하여 PowerShell을 엽니다. 스크립트에서 제거할 VMM 서버의 ID를 지정합니다. 이 스크립트는 서버에서 등록 및 클라우드 페어링을 제거합니다.
...
2. Hyper-V 사이트에 대한 정책을 연결 해제합니다. **하이퍼 V 사이트** >  복제 정책의 **사이트 복구 인프라에서** > 연결된 정책을 두 번 클릭합니다.**Replication Policies** 사이트를 마우스 오른쪽 단추로 클릭하고 > **연결 해제**를 클릭합니다.
3. Hyper-V 호스트를 삭제합니다. 하이퍼 V 사이트 > **하이퍼 V 호스트에****대 한**사이트 복구 **인프라에서** >  **서버를**마우스 오른쪽 단추로 클릭 > 삭제 합니다.
4. 모든 호스트가 제거되었으면 Hyper-V 사이트를 삭제합니다. **하이퍼 V 사이트** > **하이퍼 V 사이트의**사이트 **복구 인프라에서** >  **사이트를**삭제> 마우스 오른쪽 단추로 클릭합니다.
...
2. **복제 사용 안 함** 페이지에서 다음 옵션 중 하나를 선택합니다.
    - **복제를 사용하지 않도록 설정하고 제거(권장)** - 이 옵션은 Azure 사이트 복구에서 복제된 항목을 제거하고 컴퓨터에 대한 복제가 중지됩니다. 구성 서버에서 복제 구성을 정리하고 이 보호되는 서버에 대한 Site Recovery 청구가 중지됩니다. 이 옵션은 구성 서버가 연결된 상태에 있는 경우에만 사용할 수 있습니다.
...
> [!NOTE]
> VM을 통해 이미 실패했지만 Azure에서 실행 중인 경우 보호 비활성화가 VM을 통해 실패한 것을 제거하거나 영향을 주지 않습니다.
## <a name="disable-protection-for-a-azure-vm-azure-to-azure"></a>Azure VM에 대한 보호를 사용하지 않도록 설정(Azure에서 Azure로)

-  **보호된 항목** > **복제 항목에서**컴퓨터를 마우스 오른쪽 단추로 클릭> **복제 를 사용하지 않도록 설정합니다.**
> [!NOTE]
