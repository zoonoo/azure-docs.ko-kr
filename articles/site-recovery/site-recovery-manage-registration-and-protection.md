...
6. VMM 서버가 연결 되지 않은 상태 이면 VMM 서버에서 아래 정리 스크립트를 다운로드 하 여 실행 합니다. **관리자 권한으로 실행** 옵션을 사용 하 여 PowerShell을 열고 기본 (LocalMachine) 범위에 대 한 실행 정책을 변경 합니다. 스크립트에서 제거할 VMM 서버의 ID를 지정합니다. 이 스크립트는 서버에서 등록 및 클라우드 페어링을 제거합니다.
