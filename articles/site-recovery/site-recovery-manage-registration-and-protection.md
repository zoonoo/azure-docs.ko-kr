...
-  Azure에 연결된 경우 VMM 서버의 등록을 취소하는 것이 좋습니다. 이렇게 하면 온-프레미스 VMM 서버 및 연결된 VMM 서버(삭제하려는 서버의 클라우드에 매핑되는 클라우드를 포함하는 VMM 서버)가 적절하게 정리됩니다. 연결에 영구적인 문제가 있는 경우에만 연결되지 않은 서버를 제거하는 것이 좋습니다.
- 삭제할 때 VMM 서버기 연결되지 않은 경우 정리 작업을 수행하는 스크립트를 수동으로 실행해야 합니다. 이 스크립트는 [Microsoft 갤러리](https://gallery.technet.microsoft.com/scriptcenter/Cleanup-Script-for-Windows-95101439)에서 사용할 수 있습니다. 수동 정리 프로세스를 완료하기 위해 서버의 VMM ID를 기록해 둡니다.
- 클러스터에 배포되는 VMM 서버 등록을 취소해야 하는 경우 다음을 수행합니다.
...
가상 컴퓨터 보호를 중지하려면 다음을 수행합니다.

1. 클라우드 속성의 **가상 컴퓨터** 탭에서 가상 컴퓨터 > **제거**를 선택합니다.
2. **가상 컴퓨터 제거 확인** 페이지에 몇 가지 선택 사항이 있습니다.

	- **가상 컴퓨터 보호 사용 안 함**—이 옵션을 사용하도록 설정하고 저장하면 가상 컴퓨터가 더 이상 Azure Site Recovery를 통해 보호되지 않습니다. 가상 컴퓨터의 보호 설정이 자동으로 정리됩니다.
	- **가상 컴퓨터 관리 중지**—이 옵션을 선택하면 가상 컴퓨터가 Azure Site Recovery 서비스에서만 제거됩니다. 가상 컴퓨터의 온-프레미스 보호 설정은 영향을 받지 않으며 아래 지침을 사용하여 수동으로 정리해야 할 수 있습니다.
...
### 보호 설정을 수동으로 정리(VMware 사이트와 Azure 간)

원본 서버에서 모바일 서비스를 제거합니다.
