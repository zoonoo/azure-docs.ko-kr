...
6. VMM 서버가 연결 분리 상태인 경우 VMM 서버에서 [정리 스크립트](unregister-vmm-server-script.md)를 다운로드하여 실행합니다. 기본(LocalMachine) 범위에 대한 실행 정책을 변경하려면 **관리자 권한으로 실행** 옵션으로 PowerShell을 엽니다. 스크립트에서 제거할 VMM 서버의 ID를 지정합니다. 이 스크립트는 서버에서 등록 및 클라우드 페어링을 제거합니다.
...
> [!NOTE]
> 이미 VM을 장애 조치(failover)했으며 Azure에서 실행 중인 경우 보호를 사용하지 않도록 설정해도 장애 조치(failover)된 VM을 제거하거나 영향을 주지 않습니다.
## <a name="disable-protection-for-a-azure-vm-azure-to-azure"></a>Azure VM에 대한 보호 사용 안 함(Azure에서 Azure로)
