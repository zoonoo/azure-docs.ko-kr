...
2. 삭제할 VMM 서버에서 클라우드에 사용된 모든 네트워크 매핑을 삭제합니다. **System Center VMM** > **네트워크 매핑에**대 한 **Site Recovery 인프라** > 에서 네트워크 매핑을 마우스 오른쪽 단추로 클릭 하 > **삭제**를 클릭 합니다.
3. VMM 서버의 ID를 메모해 둡니다.
4. 제거할 VMM 서버의 클라우드에서 복제 정책을 연결 해제합니다.  **System Center VMM** >  **복제 정책**에 대 한 **Site Recovery 인프라** > 에서 연결 된 정책을 두 번 클릭 합니다. 클라우드를 마우스 오른쪽 단추로 클릭하고 > **연결 해제**를 클릭합니다.
5. VMM 서버 또는 활성 노드를 삭제합니다. **System Center vmm** > **vmm 서버**에 대 한 **Site Recovery 인프라** > 에서 서버를 마우스 오른쪽 단추로 클릭 하 > **삭제**를 클릭 합니다.
...
2. Hyper-V 사이트에 대한 정책을 연결 해제합니다.  >  Hyper-v 사이트**복제 정책****에 대 한** **Site Recovery 인프라** > 에서 연결 된 정책을 두 번 클릭 합니다. 사이트를 마우스 오른쪽 단추로 클릭하고 > **연결 해제**를 클릭합니다.
3. Hyper-V 호스트를 삭제합니다. Hyper-v 사이트 > **hyper-v 호스트****의** **Site Recovery 인프라** > 에서 서버를 마우스 오른쪽 단추로 클릭 하 > **삭제**를 클릭 합니다.
