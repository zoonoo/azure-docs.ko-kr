...
2. [런타임 디버깅][런타임 디버깅]
3. [진단 로그 분석][진단 로그 분석]
4. [클라우드 어셈블리 확인 디버깅][클라우드 어셈블리 확인 디버깅]
5. [Entity Framework 마이그레이션 문제 해결][Entity Framework 마이그레이션 문제 해결]
...
HTTP 트래픽을 전송 및 검사하기 위해서는 아무 HTTP 디버거나 사용할 수 있습니다. [Fiddler][Fiddler]는 이러한 목적으로 개발자가 자주 사용하는 인기 있는 도구입니다. 개발 작업을 쉽게 수행할 수 있도록 모바일 서비스에는 외부 도구를 사용할 필요 없이 모바일 서비스에서 직접 사용할 수 있는 웹 기반 HTTP 디버거("테스트 클라이언트")가 포함되어 있습니다. 모바일 서비스를 로컬로 호스팅할 때는 [][]<http://localhost:59233></a>과 같은 URI로 사용할 수 있으며, 클라우드에서 호스팅할 때는 [][1]<http://todo-list.azure-mobile.net></a> 형식의 URI가 사용됩니다. 다음 단계는 서비스 호스팅 위치에 관계없이 동일하게 작동합니다.
...
2. **F5**를 누르면 프로젝트가 빌드되고 실행됩니다. 시작 페이지에서 **try it out(사용해 보기)**를 선택합니다.
    ...
    > [WACOM.NOTE]
    > 서비스가 로컬로 호스팅되는 경우 링크를 클릭하면 다음 페이지로 이동됩니다. 하지만 클라우드에서 호스팅할 경우에는 자격 증명 집합을 입력하라는 메시지가 표시됩니다. 이 메시지는 인증되지 않은 사용자가 API 및 페이로드에 대한 정보에 액세스할 수 없도록 보장하기 위한 것입니다. 페이지를 표시하려면 **빈 사용자 이름** 및 **응용 프로그램 키**(암호)를 사용해서 로그인해야 합니다. 응용 프로그램 키는 **Azure 관리 포털**에서 모바일 서비스에 대한 **대시보드** 탭으로 이동하고 **키 관리**를 선택하여 확인할 수 있습니다.
    >
    > ![도움말 페이지에 액세스하기 위한 인증 프롬프트][도움말 페이지에 액세스하기 위한 인증 프롬프트]
...
6. 모바일 서비스로부터 전송되는 HTTP 응답을 브라우저 창에서 바로 볼 수 있습니다.
    ...
    ![테스트 클라이언트의 HTTP 응답][테스트 클라이언트의 HTTP 응답]
...
3. 왼쪽에서 **기호** 노드를 선택하고 [][2]<http://srv.symbolsource.org/pdb/Public></a> URI를 사용해서 (SymbolSource)[<http://symbolsource.org>] 서버에 참조를 추가합니다. 모바일 서비스 .NET 백 엔드의 기호는 모든 새 릴리스에 제공됩니다.
...
10. 서비스가 성공적으로 게시되었으면 **서버 탐색기**를 열고 **Microsoft Azure** 및 **모바일 서비스** 노드를 확장합니다. 필요한 경우 로그인합니다.
...
모바일 서비스가 고객의 요청을 처리할 때는 여러 가지 유용한 진단 정보를 생성하고 발생한 모든 예외를 캡처합니다. 이 외에도 [**TableController**][**TableController**]의 [**Services**][**Services**] 속성에서 제공되는 [**Log**][**Log**] 속성을 활용해서 추가 로그로 컨트롤러 코드를 계측할 수 있습니다.

로컬로 디버그할 때는 로그가 Visual Studio **출력** 창에 표시됩니다.

![Visual Studio 출력 창의 로그][Visual Studio 출력 창의 로그]
...
![Visual Studio 서버 탐색기의 로그][Visual Studio 서버 탐색기의 로그]

모바일 서비스에 대한 **로그** 탭의 **Azure 관리 포털**에서도 동일한 로그를 사용할 수 있습니다.

![Azure 관리 포털의 로그][Azure 관리 포털의 로그]

<a name="AssemblyResolution"></a>
## 클라우드 어셈블리 확인 디버깅

모바일 서비스를 Azure에 게시하면 컨트롤러 코드를 호스팅하는 HTTP 파이프라인에 업그레이드 및 패치를 효율적으로 적용할 수 있도록 모바일 서비스 호스팅 환경에서 로드됩니다. 여기에는 [.NET 백 엔드 NuGet 패키지][.NET 백 엔드 NuGet 패키지]에서 참조되는 모든 어셈블리가 포함됩니다. 업무 팀은 이러한 어셈블리의 최신 버전을 사용하기 위해 서비스를 지속적으로 업데이트합니다.
...
![어셈블리 로드 충돌을 나타내는 도움말 페이지][어셈블리 로드 충돌을 나타내는 도움말 페이지]
...
이 문제는 쉽게 수정할 수 있습니다. 필요한 어셈블리의 지원되는 버전으로 되돌리고 서비스를 다시 게시하면 됩니다.

<a name="EFMigrations"></a>
## Entity Framework 마이그레이션 문제 해결

SQL 데이터베이스와 함께 모바일 서비스 .NET 백 엔드를 사용할 때는 사용자가 데이터베이스를 쿼리하고 여기에 개체를 저장할 수 있게 해주는 데이터 액세스 기술로 EF(Entity Framework)가 사용됩니다. EF가 개발자를 위해 처리하는 한 가지 중요한 특징은 코드에 지정된 모델 클래스가 변경될 때 데이터베이스 열(*스키마*)이 변경되는 방식입니다. 이 프로세스를 [Code First 마이그레이션][Code First 마이그레이션]이라고 합니다.

마이그레이션은 복잡할 수 있으며 작업을 성공하기 위해서는 데이터베이스 상태가 EF 모델과 동기화된 상태로 유지되어야 합니다. 모바일 서비스의 마이그레이션 처리 방법과 발생 가능한 오류에 대한 자세한 내용은 [.NET 백 엔드 모바일 서비스에 대한 데이터 모델 변경 방법][.NET 백 엔드 모바일 서비스에 대한 데이터 모델 변경 방법]을 참조하세요.

<!-- IMAGES -->

[HTTP 디버깅]: #HttpDebugging
[런타임 디버깅]: #RuntimeDebugging
[진단 로그 분석]: #Logs
[클라우드 어셈블리 확인 디버깅]: #AssemblyResolution
[Entity Framework 마이그레이션 문제 해결]: #EFMigrations
[Fiddler]: http://www.telerik.com/fiddler
[]: http://localhost:59233
[1]: http://todo-list.azure-mobile.net
[도움말 페이지에 액세스하기 위한 인증 프롬프트]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/6.png
[도움말 페이지]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/7.png
[API의 테스트 페이지]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/8.png
[테스트 클라이언트]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/9.png
[테스트 클라이언트의 HTTP 응답]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/10.png
[기호 로드 구성]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/1.png
[2]: http://srv.symbolsource.org/pdb/Public
[기호 서버 구성]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/2.png
[중단점 도착]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/3.png
[디버그 게시]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/4.png
[디버거 연결]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/5.png
...
[Visual Studio 출력 창의 로그]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/11.png
[Visual Studio 서버 탐색기의 로그]: ./media/mobile-services-dotnet-backend-how-to-troubleshoot/12.png
