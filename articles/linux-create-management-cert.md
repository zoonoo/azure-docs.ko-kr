...
다음 사이트를 방문하여 Azure용 관리 인증서를 쉽게 만들 수 있습니다. <https://windows.azure.com/download/publishprofile.aspx>
...
이 파일은 복구할 수 없으며 손실될 경우 새 관리 인증서를 생성해야 하므로 파일을 안전한 위치에 저장하십시오. 시스템에서 사용할 수 있는 인증서의 총수에는 제한이 있습니다. 이를 확인하려면 이 웹 사이트의 해당 섹션을 참조하십시오. 그런 다음, 이 인증서를 다음과 같은 여러 방법으로 사용할 수 있습니다.
...
추출한 내용을 파일로 복사한 후 사용 가능한 Linux의 Base 64 디코더를 사용하여 디코드해야 합니다.
...
이 도구로 플랫폼에서 만들 수 있는 인증서는 최대 10개로 제한됩니다. 시스템의 임의 위치에 이러한 개인 키를 저장하지 않고 생성했다면 생성한 키를 복구할 방법이 없습니다. 시스템의 인증서 제한에 도달한 경우 사용 중인 인증서를 초기화하거나 이전 Silverlight 포털을 렌더링하여 이러한 작업을 수행할 수 있는 브라우저를 사용하려면 Azure 포럼을 통해 지원을 문의해야 합니다.
...
여기서 xxxxx는 [인코딩한 파일]의 내용이며, 이 내용은 Azure account import (File) 명령으로 Linux Azure 명령줄 도구에 세부 정보를 제공하는 데 사용합니다.
