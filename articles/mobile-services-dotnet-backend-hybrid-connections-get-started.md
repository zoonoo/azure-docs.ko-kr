...
1.  [필수 조건][필수 조건]
2.  [SQL Server Express 설치, TCP/IP 사용 및 SQL Server 데이터베이스 온-프레미스 만들기][SQL Server Express 설치, TCP/IP 사용 및 SQL Server 데이터베이스 온-프레미스 만들기]
3.  [하이브리드 연결 만들기][하이브리드 연결 만들기]
4.  [온-프레미스 하이브리드 연결 관리자를 설치하여 연결 완료][온-프레미스 하이브리드 연결 관리자를 설치하여 연결 완료]
5.  [연결을 사용하도록 모바일 서비스 수정][연결을 사용하도록 모바일 서비스 수정]
...
SQL Server를 구성하는 방법에 대한 자세한 내용은 [SQL Server Express 설치, TCP/IP 사용 및 SQL Server 데이터베이스 온-프레미스 만들기][1]를 참조하세요. 위에서 설명한 조건을 충족하는 구성 및 환경으로 SQL Server를 이미 설치한 경우 건너뛰어서 [SQL Server 데이터베이스 온-프레미스][SQL Server 데이터베이스 온-프레미스]를 시작할 수 있습니다.
...
1.  온-프레미스 컴퓨터에서 [Azure 관리 포털][Azure 관리 포털]에 로그온합니다.
...
3.  **BizTalk 서비스 이름**을 입력하고 **버전**을 선택합니다.
    ...
    ![새로운 BizTalk 서비스 구성][새로운 BizTalk 서비스 구성]
...
2.  **설치 및 구성**을 클릭합니다.
    ...
    ![하이브리드 연결 관리자 설치][하이브리드 연결 관리자 설치]
...
1.  포털의 **모바일 서비스** 탭에서 기존 모바일 서비스를 선택하거나 새로운 서비스를 만듭니다.
    ...
    > [WACOM.NOTE].NET 백 엔드를 사용하여 생성된 서비스를 선택하거나 새 .NET 백 엔드 모바일 서비스를 만들어야 합니다. 새 .NET 백 엔드 모바일 서비스를 만드는 방법에 대한 자세한 내용은 [모바일 서비스 시작하기][모바일 서비스 시작하기]를 참조하세요.

2.  모바일 서비스의 **구성** 탭에서 **하이브리드 연결** 섹션을 찾아 **하이브리드 연결 추가**를 선택합니다.

    ![하이브리드 연결 연계][하이브리드 연결 연계]

3.  BizTalk 서비스 탭에서 방금 만든 하이브리드 연결을 선택하고 **확인**을 누릅니다.

    ![연계된 하이브리드 연결 선택][연계된 하이브리드 연결 선택]
...
1.  **앱 설정**의 **구성** 탭에서 `Server=onPremisesServer,1433;Database=OnPremisesDB;User ID=sa;Password={password}`같은 값과 함께 `onPremisesDatabase`로 명명한 새 앱 설정을 추가합니다.

    ![연결 문자열에 대한 앱 설정][연결 문자열에 대한 앱 설정]

    온-프레미스 데이터베이스의 보안 암호로 `{password}`를 바꿉니다.

2.  **저장**을 눌러 하이브리드 연결과 방금 만든 앱 설정을 저장합니다.

3.  Visual Studio 2013에서 .NET 기반 모바일 서비스를 정의하는 프로젝트를 엽니다.

    .NET 백 엔드 프로젝트를 다운로드하는 방법에 대해서는 [모바일 서비스 시작하기][모바일 서비스 시작하기]를 참조하세요.

4.  솔루션 탐색기에서 **Models** 폴더를 확장하여 *Context.cs*로 끝나는 데이터 모델 파일을 엽니다.

5.  다음 코드 조각과 유사하게 **DbContext** 인스턴스 생성자를 수정합니다.
    ...
    이제 서비스는 Azure의 앱 설정을 정의하는 새 하이브리드 연결 문자열을 사용합니다.

6.  변경 내용을 게시하고 모바일 서비스에 연결된 클라이언트 앱을 사용하여 데이터베이스 변경 내용을 생성하는 몇 가지 작업을 호출합니다.

7.  SQL Server가 실행되는 온-프레미스 컴퓨터에서 SQL Management Studio를 연 다음 개체 탐색기에서 **OnPremisesDB** 데이터베이스와 **테이블**을 확장합니다.

8.  **hybridService1.TodoItems** 테이블을 마우스 오른쪽 단추로 클릭하고 **상위 1000개의 행 선택**을 선택하여 결과를 봅니다.
...
<!-- IMAGES -->
  ...
  [필수 조건]: #Prerequisites
  [SQL Server Express 설치, TCP/IP 사용 및 SQL Server 데이터베이스 온-프레미스 만들기]: #InstallSQL
  [하이브리드 연결 만들기]: #CreateHC
  ...
  [하이브리드 연결 추가]: ./media/mobile-services-dotnet-backend-hybrid-connections-get-started/3.png
  [하이브리드 연결 구성]: ./media/mobile-services-dotnet-backend-hybrid-connections-get-started/4.png
  [하이브리드 연결이 생성됨]: ./media/mobile-services-dotnet-backend-hybrid-connections-get-started/5.png
  [온-프레미스 설치]: ./media/mobile-services-dotnet-backend-hybrid-connections-get-started/5-1.png
