...
|**TunnelDiagnosticLog** | 터널 상태 변경 이벤트를 포함 합니다. 터널 연결/연결 끊기 이벤트는 상태 변경에 대 한 요약 된 이유 (해당 하는 경우)를 포함 합니다. |
...
|***이름** _ | _ *_설명_** |
|---        | ---               |
|**TimeGenerated** | 각 이벤트의 타임 스탬프 (UTC 표준 시간대)입니다.|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-26-set-gateway.png" alt-text="GatewayDiagnosticLog에서 표시 되는 게이트웨이 집합 작업의 예입니다.":::
...
| **인스턴스 \_ s** | 이벤트를 트리거한 게이트웨이 역할 인스턴스입니다. 이는 \_ \_ \_ 게이트웨이의 두 인스턴스 이름에 해당 하는 0 또는 gatewaytenantworker (1)의 gatewaytenantworker 일 수 있습니다 \_ .|
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-16-tunnel-connected.png" alt-text="TunnelDiagnosticLog에 표시 되는 터널 연결 이벤트의 예입니다.":::
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-31-bgp-route.png" alt-text="RouteDiagnosticLog에 표시 되는 BGP 경로 교환 작업의 예입니다.":::
...
|**이벤트** | 문제 해결에 유용한 진단 메시지를 포함 합니다. 일반적으로 키워드를 사용 하 여 시작 하 고 Azure 게이트웨이에서 수행 하는 작업을 참조 합니다. **\[ 송신 \]** 은 azure 게이트웨이에서 보낸 IPSec 패킷으로 인 한 이벤트를 나타냅니다.  **\[ 수신 \]** 은 온-프레미스 장치에서 받은 패킷의 결과로 이벤트를 나타냅니다.  **\[ LOCAL \]** 은 Azure 게이트웨이에서 로컬에서 수행 하는 작업을 나타냅니다. |


RemoteIP, LocalIP 및 이벤트 열이 AzureDiagnostics 데이터베이스의 원래 열 목록에는 없지만 "메시지" 열의 출력을 구문 분석 하 여 분석을 간소화 함으로써 쿼리에 추가 되는 방법을 확인 합니다.
...
- IPsec 터널을 설정 하지 못하면 Azure는 몇 초 마다 재시도를 계속 합니다. 이러한 이유로 "VPN 다운" 문제를 해결 하는 것은 특정 시간이 문제를 재현할 때까지 기다릴 필요가 없기 때문에 IKEdiagnosticLog에서 매우 편리 합니다. 또한 오류는 이론적으로 항상 동일 하 게 수행 되므로 언제 든 지 하나의 "샘플" 협상 실패를 확대할 수 있습니다.

- SA \_ INIT는 피어가이 ipsec 협상에 사용 하려고 하는 ipsec 매개 변수를 포함 합니다. 공식 문서   
[기본 ipsec/IKE 매개 변수](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpn-devices#ipsec) 는 기본 설정으로 Azure 게이트웨이에서 지 원하는 ipsec 매개 변수를 나열 합니다.


## <a name="p2sdiagnosticlog"></a><a name="P2SDiagnosticLog"></a>P2SDiagnosticLog

VPN 진단에 사용할 수 있는 마지막 테이블은 **P2SDiagnosticLog** 입니다. 다음 표에서는 지점과 사이트 간 활동을 추적 합니다.
...
:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-28-p2s-log-event.png" alt-text="P2SDiagnosticLog에 표시 되는 지점 및 사이트 간 연결의 예입니다.":::

또한 클라이언트가 IKEv2 또는 OpenVPN 지점을 통해 사이트에 연결할 때마다이 테이블은 패킷 작업, EAP/RADIUS 대화 및 사용자에의 한 성공/실패 결과를 기록 합니다.

:::image type="content" source="./media/troubleshoot-vpn-with-azure-diagnostics/image-29-eap.png" alt-text="P2SDiagnosticLog에 표시 된 EAP 인증의 예입니다.":::
