...
- 강제 터널링은 Azure 동적 라우팅 VPN 게이트웨이(정적 게이트웨이 아님)가 있는 VNet에 연결되어야 합니다. 가상 네트워크에 연결된 크로스-프레미스 로컬 사이트 사이에서 "기본 사이트"를 설정해야 합니다.

- Express 경로 강제 터널링은 이 메커니즘을 통해 구성되지 않지만 대신 Express 경로 BGP 피어링 세션을 통해 기본 경로를 보급한 고객으로 활성화됩니다. 자세한 내용은 [Express 경로 설명서](https://azure.microsoft.com/documentation/services/expressroute/)를 참조하십시오.

## 강제 터널링 구성 방법

아래의 지침에 따라 가상 네트워크에서 강제 터널링을 지정할 수 있습니다. 구성 단계는 아래의 가상 네트워크 예제에 해당합니다. 이 가상 네트워크인 다중 계층-VNet에는 3개의 서브넷이 있습니다. 4개의 크로스 프레미스 연결:*DefaultSiteHQ* 및 3개의 *분기*가 있는 *프런트 엔드*, *중간 계층* 및 *백 엔드* 서브넷 다음 구성 단계에서는 강제 터널링에 대한 기본 사이트 연결로 *DefaultSiteHQ*를 설정하고 *중간 계층* 및 *백 엔드* 서브넷을 구성하여 강제 터널링을 사용합니다.
