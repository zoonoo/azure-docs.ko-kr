...
2. 해당 패키지를 두 번 클릭하여 설치합니다. SmartScreen 팝업이 표시 되 면 **자세한 정보**를 클릭 하 고 **실행**을 클릭 합니다.
3. 클라이언트 컴퓨터에서 **네트워크 설정** 으로 이동 하 고 **VPN**을 클릭 합니다. VPN 연결에서 연결되는 가상 네트워크의 이름을 표시합니다. 
...
3. 네트워크 **기본 설정** 에서 **네트워크** 대화 상자를 열고 **' + '** 를 클릭 하 여 AZURE 가상 네트워크에 대 한 P2S 연결에 대 한 새 VPN 클라이언트 연결 프로필을 만듭니다.
...
9. **네트워크** 대화 상자에서 **적용**을 클릭하여 모든 변경 내용을 저장합니다. 그런 다음 **연결** 을 클릭 하 여 Azure 가상 네트워크에 대 한 P2S 연결을 시작 합니다.
...
5. 다운로드한 클라이언트 구성 파일에 포함된 **일반** 폴더에서 **VpnSettings.xml** 파일을 엽니다. **VpnServer** 라는 태그를 찾아서 ' azuregateway '로 시작 하 고 '. cloudapp.net '로 끝나는 이름을 복사 합니다.
...
7. 연결의 **클라이언트** 섹션에서 **인증**에 대해 **인증서/프라이빗 키**를 선택합니다. **인증서** 및 **프라이빗 키**에 대해 이전에 만든 인증서 및 프라이빗 키를 선택합니다. **옵션**에서 **내부 IP 주소 요청**을 선택합니다. 그런 다음 **추가**를 클릭 합니다.
