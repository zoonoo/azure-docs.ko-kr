...
이 문서는 Azure 리소스 관리자 배포 모델을 사용하여 만든 VNet 및 VPN 게이트웨이에 적용됩니다. 서비스 관리 배포 모델을 사용하여 강제 터널링을 구성하려는 경우 [강제 터널링 구성](vpn-gateway-about-forced-tunneling.md)을 참조하세요.
...
이 예제에서 가상 네트워크인 "MultiTier-VNet"에는 3개의 서브넷이 있습니다. 4개의 크로스 프레미스 연결:*DefaultSiteHQ* 및 3개의 *분기*가 있는 *프런트 엔드*, *중간 계층* 및 *백 엔드* 서브넷 절차단계에서는 강제 터널링에 대한 기본 사이트 연결로 *DefaultSiteHQ*를 설정하고 *중간 계층* 및 *백 엔드* 서브넷을 구성하여 강제 터널링을 사용합니다.
...
- 웹 플랫폼 설치 관리자를 사용하는 Azure PowerShell cmdlet의 최신 버전입니다. [웹 플랫폼 설치 관리자](http://aka.ms/webpi-azps/)에서 최신 버전을 다운로드하고 설치할 수 있습니다. 이 설명서는 PowerShell 1.0 이상용으로 작성되었습니다. 이 구성에 필요한 cmdlet이 이전 버전에 없습니다. PowerShell 1.0에 대한 자세한 내용은 [Azure PowerShell 1.0 Preview](https://azure.microsoft.com/blog/azps-1-0-pre/)를 참조하세요.
...
6. 중간 계층 및 백 엔드 서브넷에 경로 테이블을 연결합니다.
...
9. 기본 사이트로 게이트웨이를 만듭니다. 이 단계에서 게이트웨이를 만들고 구성하기 때문에 완료되기까지 약간의 시간이 걸리며, 20분 이상 걸리는 경우도 있습니다. 콘솔에서는 몇 개의 cmdlet이 수행되는 것처럼 보이지만 백그라운드로 많은 작업이 진행되고 있습니다. GatewayDefaultSite는 강제 라우팅 구성 작업을 허용하는 cmdlet 매개 변수이므로 사용자는 이 단계를 건너뛰는 것을 원치 않을 것입니다. PowerShell 1.0 이상에서만 사용할 수 있습니다.
