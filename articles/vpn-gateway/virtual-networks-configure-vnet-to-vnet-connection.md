...
이 문서에서는 클래식 배포 모델(서비스 관리라고도 함)을 사용하여 가상 네트워크를 만들어 하나로 연결하는 과정을 안내합니다. 아래 단계에서는 Azure 클래식 포털을 사용하여 VNet 및 게이트웨이를 만들고 PowerShell을 사용하여 VNet-VNet 연결을 구성합니다. 포털에서 VNet-VNet 연결을 구성할 수는 없습니다.
...
가상 네트워크를 다른 가상 네트워크에 연결(VNet 간)하는 것은 가상 네트워크를 온-프레미스 사이트 위치에 연결하는 것과 매우 유사합니다. 두 연결 유형 모두 VPN 게이트웨이를 사용하여 IPsec/IKE를 통한 보안 터널을 제공합니다.

연결하는 VNet은 서로 다른 구독 및 지역에 있을 수 있습니다. VNet 간 통신을 다중 사이트 구성과 통합할 수 있습니다. 이렇게 하면 아래 다이어그램에 표시된 것처럼 크로스-프레미스 연결을 가상 네트워크 간 연결과 결합하는 네트워크 토폴로지를 설정할 수 있습니다.
...
- 크로스-프레미스 연결이 필요한 경우가 아니면 여러 가상 네트워크를 연결할 때 VPN 장치는 필요하지 않습니다.
...
아래 테이블에는 VNet을 정의하는 방법의 예가 나와 있습니다. 아래의 범위는 지침으로만 참고하고, 실제 가상 네트워크에 사용할 범위를 기록해야 합니다. 이 정보는 이후 단계에서 필요합니다.
...
2. 화면 왼쪽 아래에서 **새로 만들기** > **네트워크 서비스** > **가상 네트워크** > **사용자 지정 만들기**를 차례로 클릭하여 구성 마법사를 시작합니다. 마법사를 따라 이동하면서 아래 나열된 값을 각 페이지에 추가합니다.
...
온-프레미스 네트워크에도 연결되는 VNet을 만드는 경우 온-프레미스 네트워크에 사용되는 범위와 겹치지 않는 범위를 선택하는 것이 특히 중요합니다. 해당 경우, 가상 네트워크에 사용할 온-프레미스 네트워크 주소 공간에서 일정 범위의 IP 주소를 지정하는 네트워크 관리자와 조정하기 위해 필요합니다.
...
다음으로, 이전 단계를 반복하여 다른 가상 네트워크를 만듭니다. 나중에 이 두 개의 가상 네트워크를 연결하게 됩니다. 1단계의 [예제 설정](#step1)을 참조할 수 있습니다. VNet이 이미 있으면 이 단계를 수행할 필요가 없습니다. 하지만 IP 주소 범위가 연결하려는 다른 VNet 또는 온-프레미스 네트워크와 겹치지 않는지 확인해야 합니다.

## 4단계 - 로컬 네트워크 사이트 추가

VNet-VNet 구성을 만들 경우 포털의 **로컬 네트워크** 페이지에 표시되는 로컬 네트워크 사이트를 구성해야 합니다. Azure는 각 로컬 네트워크 사이트에 지정된 설정을 사용하여 VNet 간에 트래픽을 라우팅하는 방법을 결정합니다. 각 로컬 네트워크 사이트를 참조하는 데 사용할 이름을 확인합니다. 드롭다운 목록에서 값을 선택할 때 설명이 포함된 이름을 사용하는 것이 가장 좋습니다.

예를 들어 이 연습에서 VNet1은 "VNet2Local" 이름으로 만드는 로컬 네트워크 사이트에 연결됩니다. VNet2Local에 대한 설정에는 VNet2에 대한 주소 접두사와 VNet2 게이트웨이에 대한 공용 IP 주소가 포함됩니다. VNet2는 VNet1에 대한 주소 접두사와 VNet1 게이트웨이에 대한 공용 IP 주소를 포함하는 "VNet1Local"이라는 로컬 네트워크 사이트에 연결됩니다.
...
3. **VPN 장치 IP 주소(선택 사항)**에 유효한 공용 IP 주소를 입력합니다. 일반적으로는 VPN 장치의 실제 외부 IP 주소를 사용합니다. VNet-VNet 구성의 경우 VNet에 대한 게이트웨이에 할당된 공용 IP 주소를 사용합니다. 그러나 게이트웨이를 아직 만들지 않은 경우 자리 표시자로 유효한 공용 IP 주소를 지정합니다. 이 구성에서는 선택 사항이 아니므로 이 항목은 비워두면 안 됩니다. Azure에서 해당하는 게이트웨이 IP 주소를 생성하고 나면 이 설정으로 돌아와서 IP 주소를 구성합니다. 화살표를 클릭하여 다음 화면으로 이동합니다.

4. **주소 지정 페이지**에서 VNet1의 실제 IP 주소 범위와 주소 수를 입력합니다. 이 범위는 앞에서 VNet1에 대해 구성한 범위와 정확하게 일치해야 합니다. Azure는 사용자가 지정한 IP 주소 범위를 사용하여 VNet1으로 예정된 트래픽을 라우팅합니다. 확인 표시를 클릭하여 로컬 네트워크를 만듭니다.
...
각 가상 네트워크에 대한 동적 라우팅 게이트웨이 구성 이 구성은 고정 라우팅 게이트웨이를 지원하지 않습니다. 이전에 구성되었으며 동적 라우팅 게이트웨이가 이미 있는 VNet을 사용하는 경우 이 단계를 수행할 필요가 없습니다. 게이트웨이가 정적인 경우 삭제한 후 동적 라우팅 게이트웨이로 다시 만들어야 합니다. 게이트웨이를 삭제하는 경우 할당된 공용 IP 주소가 해제되므로 돌아가서 새 게이트웨이에 대한 새 공용 IP 주소를 사용하여 로컬 네트워크 및 VPN 장치를 다시 구성해야 합니다.

1. **네트워크** 페이지에서 가상 네트워크의 상태 열이 **생성됨**인지 확인합니다.

2. **이름** 열에서 가상 네트워크의 이름을 클릭합니다. 이 예제에서는 VNet1을 사용합니다.
...
위의 모든 단계를 완료한 후에는 IPsec/IKE 사전 공유 키를 설정합니다. 이 작업은 PowerShell을 사용하여 수행합니다. Azure PowerShell cmdlet 설치에 대한 자세한 내용은 [Azure PowerShell 설치 및 구성 방법](../powershell-install-configure.md)을 참조하세요. 최신 버전의 SM(서비스 관리) cmdlet을 다운로드해야 합니다.

1. Windows PowerShell을 열고 로그인합니다.

		Add-AzureAccount

2. 연결을 만듭니다. 아래 예제에서 공유 키가 정확히 동일한지 확인합니다. 공유 키는 항상 일치해야 합니다.
...
3. 연결이 초기화될 때까지 기다립니다. 게이트웨이가 초기화되면 아래 그림과 같이 게이트웨이가 표시되며 가상 네트워크가 연결됩니다.
