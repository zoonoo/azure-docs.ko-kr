...
이 시나리오에서는 지점 및 사이트 간 연결로 가상 네트워크를 만듭니다. 또한 지침은 이 구성에 필요한 인증서를 생성하는 데도 도움이 됩니다. P2S 연결은 VPN 게이트웨이가 포함된 VNet, 루트 인증서 .cer 파일(공용 키), 클라이언트 인증서 및 클라이언트 쪽 VPN 구성으로 이루어집니다. 
...
VNet용 가상 네트워크 게이트웨이를 구성하고 만듭니다. *-GatewayType*은 **Vpn**이어야 하고 *-VpnType*은 **RouteBased**이어야 합니다. VPN 게이트웨이를 완료하는 데 최대 45분이 걸릴 수 있습니다.
...
지점 및 사이트 간 VPN을 사용하여 VNet에 연결하려면 각 클라이언트에서 VPN 클라이언트 구성 패키지를 설치해야 합니다. 패키지는 VPN 클라이언트를 설치하지 않습니다. 가상 네트워크에 연결하는 데 필요한 설정을 사용하여 기본 Windows VPN 클라이언트를 구성합니다. 지원되는 클라이언트 운영 체제의 목록은 이 문서 끝의 [지점 및 사이트 간 연결 FAQ](#faq)를 참조하세요.
...
1. VNet에 연결하면 P2S 연결을 통해 VM에 연결할 수 있습니다. VM에 연결하려면 가상 컴퓨터의 개인 IP 주소가 필요합니다. 다음 예제를 사용하면 [Get-AzureRmNetworkInterface](https://docs.microsoft.com/powershell/module/azurerm.network/get-azurermnetworkinterface?view=azurermps-3.7.0)를 사용하여 개인 IP 주소를 가져올 수 있습니다. 결과는 모든 리소스 그룹에서 VM 및 해당 개인 IP 주소의 목록을 반환합니다. 
  ...
  ```
2. 다음 명령을 사용하여 가상 컴퓨터와의 원격 데스크톱 세션을 만듭니다. IP 주소를 연결하려는 VM과 연결된 개인 IP 주소로 바꿉니다. 가상 컴퓨터를 만들 때 사용되는 자격 증명을 묻는 메시지가 표시되면 입력합니다. 
...
Azure에 최대 20개의 신뢰할 수 있는 루트 인증서 .cer 파일을 추가할 수 있습니다. 다음 단계를 사용하면 루트 인증서를 추가할 수 있습니다.
