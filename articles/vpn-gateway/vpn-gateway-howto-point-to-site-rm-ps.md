...
description: P2S 및 자체 서명 또는 CA 발급 인증서를 사용하여 Windows 및 macOS 클라이언트를 Azure 가상 네트워크에 안전하게 연결합니다. 이 문서에서는 PowerShell을 사용합니다.
...
---
# <a name="configure-a-point-to-site-vpn-connection-to-a-vnet-using-native-azure-certificate-authentication-powershell"></a>네이티브 Azure 인증서 인증을 사용하여 VNet에 지점 및 사이트 간 연결 구성: PowerShell
...
## <a name="4-create-the-vpn-gateway"></a><a name="creategateway"></a>4. VPN 게이트웨이 만들기
...
## <a name="7-upload-the-root-certificate-public-key-information"></a><a name="upload"></a>7. 루트 인증서 공개 키 정보 업로드
...
이 섹션에서는 컴퓨터가 가상 네트워크 게이트웨이에 연결할 네이티브 클라이언트를 구성합니다. 예를 들어 Windows 컴퓨터에서 VPN 설정으로 이동하면 VPN 연결을 추가할 수 있습니다. 지점 및 사이트 간 연결에는 특정 구성 설정이 필요합니다. 이러한 단계는 VPN 클라이언트가 지점 간 연결을 통해 가상 네트워크에 연결하는 데 필요한 특정 설정을 사용하여 패키지를 생성하는 데 도움이 됩니다.
