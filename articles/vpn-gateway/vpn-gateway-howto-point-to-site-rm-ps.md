...
## <a name="<a-name="declare"></a>part-1---log-in-and-set-variables"></a><a name="declare"></a>1부 - 로그인 및 변수 설정
...
## <a name="part-2---configure-a-vnet"></a>2부 - VNet 구성 
...
## <a name="part-3---add-trusted-certificates"></a>3부 - 신뢰할 수 있는 인증서 추가
...
Azure에 최대 20개의 신뢰할 수 있는 인증서를 추가할 수 있습니다. 공개 키를 가져오려면 인증서를 Base64 인코딩 X.509(.cer) 파일로 내보냅니다. .cer를 내보내는 방법 중의 하나는 **certmgr.msc**를 열고 개인/인증서에서 인증서를 찾는 것입니다. 인증서를 마우스 오른쪽 단추로 클릭하고 개인 키 없이 "Base-64로 인코딩된 X.509(. CER)"로 내보냅니다. .cer 파일로 내보낸 파일 경로를 기록해 둡니다. 이것은 인증서의 Base64 문자열 표현을 가져오는 샘플입니다. 
...
## <a name="part-4---create-the-vpn-gateway"></a>4부 - VPN 게이트웨이 만들기
...
## <a name="part-5---download-the-vpn-client-configuration-package"></a>5부 - VPN 클라이언트 구성 패키지 다운로드
...
## <a name="<a-name="cc"></a>part-6---generate-the-client-certificate"></a><a name="cc"></a>6부 - 클라이언트 인증서 생성
...
## <a name="part-7---install-the-client-certificate"></a>7부 - 클라이언트 인증서 설치
...
## <a name="part-8---connect-to-azure"></a>8부 - Azure에 연결
...
## <a name="part-9---verify-your-connection"></a>9부 - 연결 확인
...
## <a name="to-add-or-remove-a-trusted-root-certificate"></a>신뢰할 수 있는 루트 인증서를 추가 또는 제거하려면
...
## <a name="to-manage-the-list-of-revoked-client-certificates"></a>해지된 클라이언트 인증서 목록을 관리하려면
