...
이 문서는 **클래식 배포 모드**(서비스 관리)를 사용하여 만든 가상 네트워크에 대한 지점 및 사이트 간 VPN 게이트웨이 연결에 적용됩니다. 리소스 관리자를 사용하여 만든 VNet에 대한 지점 및 사이트 간 연결을 구성하려는 경우 [PowerShell을 사용하여 가상 네트워크에 지점 및 사이트 간 연결 구성](vpn-gateway-howto-point-to-site-rm-ps.md)을 참조하세요.
