...
다음 절차는 가상 네트워크에 보안 지점 및 사이트 간 연결을 만드는 단계를 안내합니다. 지점 및 사이트 간 연결을 구성할 때 여러 단계가 필요하긴 하지만 VPN 장치를 구입하거나 구성하지 않고 컴퓨터에서 가상 네트워크까지 보안 연결을 설정하는 것이 좋은 방법입니다.

지점 및 사이트 간 연결에 대한 구성은 가상 네트워크 및 VPN 게이트웨이, 인증에 사용되는 인증서, 가상 네트워크에 연결하는 데 사용되는 VPN 클라이언트 이렇게 3가지 섹션으로 구분됩니다. 이러한 각 구성 순서가 중요하므로, 단계를 건너뛰거나 앞으로 이동하지 마세요.

## 섹션 1 - 가상 네트워크 및 VPN 게이트웨이 만들기

지점 대 사이트간 연결에는 동적 라우팅 게이트웨이를 사용하는 가상 네트워크가 필요합니다. 다음 단계에서 안내하는 프로세스입니다.
...
1. (Preview 포털이 아닌) **Azure 포털**에 로그인합니다.
...
3. 다음 예제의 명령은 컴퓨터의 개인 인증서 저장소에 루트 인증서를 만들어 설치하고 나중에 Azure 포털에 업로드할 *.cer* 파일도 만듭니다.
...
### Azure 포털에 루트 인증서 .cer 파일 업로드

각 루트 인증서에 대한 해당.cer 파일을 Azure에 업로드해야 합니다. 최대 20개의 인증서를 업로드할 수 있습니다.

1. 이전 프로시저에서 루트 인증서를 생성했다면, *.cer* 파일도 생성되었습니다. 이제 Azure 포털에 해당 파일을 업로드합니다. .cer 파일은 루트 인증서의 개인 키를 포함하지 않습니다. 최대 20개의 루트 인증서를 업로드할 수 있습니다.
