...
1. 클라이언트 인증서는 가상 네트워크에 연결하려는 각 컴퓨터에 설치되어야 합니다. 여러 클라이언트 인증서를 만든 다음 내보내야 할 수 있음을 의미합니다. 클라이언트 인증서를 내보내려면 *certmgr.msc*를 사용합니다. 내보낼 클라이언트 인증서를 마우스 오른쪽 단추로 클릭하고 **모든 작업**을 클릭한 다음 **내보내기**를 클릭합니다.
