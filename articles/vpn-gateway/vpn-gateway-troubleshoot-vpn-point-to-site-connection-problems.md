...
Azure VPN 게이트웨이에 루트 인증서 공개 키를 업로드하지 않은 경우 이 문제가 발생합니다. 키가 손상되거나 만료되는 경우에도 이 문제가 발생할 수 있습니다.
...
**가상 네트워크 게이트웨이 &lt;*게이트웨이 이름*&gt;을 저장하지 못했습니다. 인증서 &lt;*인증서 ID*&gt;에 대한 데이터가 유효하지 않습니다.**
...
**가상 네트워크 게이트웨이 &lt;*게이트웨이 이름*&gt;을 저장하지 못했습니다. 리소스 이름 &lt;*업로드하려는 인증서 이름*&gt;이 유효하지 않습니다**.
...
## <a name="azure-vpn-gateway-upgrade-all-p2s-clients-are-unable-to-connect"></a>Azure VPN Gateway 업그레이드: 모든 P2S 클라이언트를 연결할 수 없습니다.
