...
**이 확장 가능한 인증 프로토콜에서 사용할 수 있는 인증서를 찾을 수 없습니다. (오류 798)**
...
## <a name="the-network-connection-between-your-computer-and-the-vpn-server-could-not-be-established-because-the-remote-server-is-not-responding"></a>원격 서버가 응답하지 않아 컴퓨터와 VPN 서버 간의 네트워크 연결을 설정하지 못했습니다.

### <a name="symptom"></a>증상

Windows에서 IKEv2를 사용하여 Azure 가상 네트워크 게이트웨이에 연결하려고 하면 다음과 같은 오류 메시지가 나타납니다.

**원격 서버가 응답하지 않아 컴퓨터와 VPN 서버 간의 네트워크 연결을 설정하지 못했습니다.**

### <a name="cause"></a>원인
 
 Windows 버전에 IKE 조각화를 지원하지 않는 경우 문제가 발생합니다.
...
2. 레지스트리 키 값을 설정합니다. 레지스트리에서 `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\RasMan\ IKEv2\DisableCertReqPayload` REG_DWORD 키를 1로 만들거나 설정합니다.
...
**수신된 메시지가 예기치 않거나 잘못 형식이 지정되었습니다. (오류 0x80090326)**
...
**라우팅 테이블을 업데이트하기 위해 사용자 지정 스크립트가 실패했습니다. (오류 8007026f)**
...
4. **신뢰할 수 있는 루트 인증 기관** 노드를 마우스 오른쪽 단추로 클릭합니다. **모든 작업** > **가져오기를**클릭하고 VPN 클라이언트 구성 패키지에서 추출한 .cer 파일로 이동합니다.
5. 컴퓨터를 다시 시작합니다. 
6. VPN 클라이언트를 설치해봅니다.
...
**가상 네트워크 게이트웨이 &lt;게이트웨이 *이름을*&gt;저장하지 못했습니다. 인증서 &lt; *인증서 ID에* &gt; 대한 데이터가 잘못되었습니다.**
...
**가상 네트워크 게이트웨이 &lt;게이트웨이 *이름을*&gt;저장하지 못했습니다. *업로드하려는* &lt;&gt; 리소스 이름 인증서 이름이 잘못되었습니다.**
...
이 문제를 해결하려면 모든 클라이언트에서 Point to Site 패키지를 다시 다운로드하고 다시 배포합니다.
...
## <a name="vpn-client-error-dialing-vpn-connection-vpn-connection-name-status--vpn-platform-did-not-trigger-connection"></a>VPN 클라이언트 오류: VPN <VPN Connection Name>연결 다이얼링, 상태 = VPN 플랫폼연결을 트리거하지 않음

RasClient의 <User> <VPN Connection Name> 이벤트 뷰어에서 다음과 같은 오류가 표시될 수도 있습니다. 오류 오류가 반환될 때 반환되는 오류 코드는 1460입니다."

### <a name="cause"></a>원인

Azure VPN 클라이언트에는 Windows용 앱 설정에서 "백그라운드 앱" 앱 권한이 활성화되어 있지 않습니다.

### <a name="solution"></a>해결 방법

1. Windows에서 설정 -> 개인 정보 보호 -> 백그라운드 앱으로 이동합니다.
2. "앱이 백그라운드에서 실행되도록" 을 켜기
...
## <a name="vpn-package-installer-doesnt-complete"></a>VPN 패키지 설치 프로그램이 완료되지 않습니다.
