...
>[AZURE.NOTE] 다음 테이블에 나열된 값이 Azure VPN 게이트웨이에서 지원되더라도 현재 Azure VPN 게이트웨이에서 특정 조합을 지정하거나 선택할 방법이 없습니다. 온-프레미스 VPN 장치에서 제약 조건을 지정해야 합니다. 또한 MSS를 1350에 고정해야 합니다.
...
| 2단계 SA(보안 연결) 수명(처리량) | 102,400,000KB | - | | IPsec SA 암호화 및 인증 제품(우선 순위 순서로) | 1. ESP-AES256 2. ESP-AES128 3. ESP-3DES 4. 해당 없음 | *경로 기반 게이트웨이 IPsec SA(보안 연결) 제안 참조*(아래) | | PFS(Perfect Forward Secrecy) | 아니요 | 아니요(*) | | DPD(Dead Peer Detection) | 지원되지 않음 | 지원됨 |
