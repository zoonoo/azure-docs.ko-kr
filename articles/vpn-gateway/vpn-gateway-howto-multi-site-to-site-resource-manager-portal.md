---
title: "Azure Portal을 사용하여 Resource Manager 배포 모델에 대한 가상 네트워크에 사이트 간 연결 다중 VPN Gateway 추가 방법 | Microsoft Docs"
...
> * [클래식 - PowerShell](vpn-gateway-multi-site.md)
> 
> 

이 문서에서는 Azure Portal을 사용하여 기존 연결이 있는 VPN 게이트웨이에 S2S(사이트 간) 연결을 추가하는 과정을 안내합니다. 이러한 유형의 연결을 "다중 사이트" 구성이라고 합니다. 

S2S 연결, 사이트 간 연결 또는 VNet 간 연결이 이미 있는 VNet에 S2S 연결을 추가하려면 이 문서를 사용할 수 있습니다. 연결을 추가하는 데 몇 가지 제한이 있습니다. 구성을 시작하기 전에 확인하려면 이 문서의 [시작 하기 전에](#before) 섹션을 확인합니다. 
...
## <a name="a-namebeforeabefore-you-begin"></a><a name="before"></a>시작하기 전에
다음 사항을 확인합니다.
...
## <a name="a-namepart1apart-1---configure-a-connection"></a><a name="part1"></a>1부 - 연결 구성
...
4. **연결** 블레이드에서 **+추가**를 클릭합니다.
    ...
    ![연결 단추 추가](./media/vpn-gateway-howto-multi-site-to-site-resource-manager-portal/addbutton.png "Add connection button")<br>
5. **연결 추가** 블레이드에서 다음 필드를 작성합니다.
   
   * **이름:** 연결을 만들고자 하는 사이트에 부여하고자 하는 이름입니다.
   * **연결 형식:** **사이트 간(IPSec)**을 선택합니다.
     
     ![연결 블레이드 추가](./media/vpn-gateway-howto-multi-site-to-site-resource-manager-portal/addconnectionblade.png "Add connection blade")<br>

## <a name="a-namepart2apart-2---add-a-local-network-gateway"></a><a name="part2"></a>2부 - 로컬 네트워크 게이트웨이 추가
...
2. **로컬 네트워크 게이트웨이 만들기** 블레이드를 열려면 **새로 만들기**를 클릭합니다.
    ...
    ![로컬 네트워크 게이트웨이 블레이드 만들기](./media/vpn-gateway-howto-multi-site-to-site-resource-manager-portal/createlngblade.png "Create local network gateway")<br>
...
## <a name="a-namepart3apart-3---add-the-shared-key-and-create-the-connection"></a><a name="part3"></a>3부 - 공유 키 추가 및 연결 만들기
...
## <a name="a-namepart4apart-4---verify-the-vpn-connection"></a><a name="part4"></a>4부 - VPN 연결 확인
