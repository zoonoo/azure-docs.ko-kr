...
이 문서에서는 Azure 리소스 관리자 배포 모델을 사용하여 가상 네트워크와 온-프레미스 네트워크에 대한 사이트 간 VPN 연결을 만드는 과정을 안내합니다.
...
위에 있는 탭을 사용하여 배포 모델 및 배포 도구에 대한 문서를 선택할 수 있습니다. 예를 들어 리소스 관리자 모델을 사용하는 대신, Azure 포털에서 클래식 배포 모델을 사용하여 사이트 간 VPN 게이트웨이 연결을 만들려는 경우 **Azure 포털** 탭(위)을 클릭하여 [Azure 포털에서 사이트 간 VPN 연결 만들기](vpn-gateway-site-to-site-create.md)를 엽니다.
...
다음으로, Azure 리소스 관리자 모드로 전환합니다. 당분간 이 모드를 변경하지 않아도 되며 두 모드에 대한 cmdlet이 자동으로 제공됩니다.
...
경우에 따라 로컬 사이트 접두사를 변경합니다. IP 주소 접두사를 수정하기 위해 수행하는 단계는 VPN 게이트웨이 연결을 만들었는지 여부에 따라 달라집니다. [로컬 사이트에 대한 IP 주소 접두사 수정](#modify-ip-address-prefixes-for-a-local-site)을 참조하세요.



## 게이트웨이에 대한 공용 IP 주소 요청
