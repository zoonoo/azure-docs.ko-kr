...
# Azure Resource Manager 및 PowerShell을 사용하여 Azure VPN Gateway와의 활성-활성 S2S VPN 연결 구성
...
### 2단계 - 활성-활성 모드로 TestVNet1용 VPN Gateway 만들기

#### 1\. 공용 IP 주소 및 게이트웨이 IP 구성 만들기
...
#### 3\. 게이트웨이 공용 IP 주소 및 BGP 피어 IP 주소 획득
...
이 단계에서는 "EnableBGP"를 $True로 설정하여 TestVNet1에서 Site5\_1까지의 연결을 만듭니다.

	New-AzureRmVirtualNetworkGatewayConnection -Name $Connection151 -ResourceGroupName $RG1 -VirtualNetworkGateway1 $vnet1gw -LocalNetworkGateway2 $lng5gw1 -Location $Location1 -ConnectionType IPsec -SharedKey 'AzureA1b2C3' -EnableBGP True

#### 3\. 온-프레미스 VPN 장치에 대한 VPN 및 BGP 매개 변수
...
#### 2\. VNet 게이트웨이 및 두 번째 로컬 네트워크 게이트웨이 연결

"EnableBGP"를 $True로 설정하여 TestVNet1에서 Site5\_2까지의 연결을 만듭니다.

	$lng5gw2 = Get-AzureRmLocalNetworkGateway   -Name $LNGName52 -ResourceGroupName $RG5

	New-AzureRmVirtualNetworkGatewayConnection -Name $Connection152 -ResourceGroupName $RG1 -VirtualNetworkGateway1 $vnet1gw -LocalNetworkGateway2 $lng5gw2 -Location $Location1 -ConnectionType IPsec -SharedKey 'AzureA1b2C3' -EnableBGP True

#### 3\. 두 번째 온-프레미스 VPN 장치에 대한 VPN 및 BGP 매개 변수
...
#### 3\. TestVNet2에 대한 활성-활성 VPN Gateway 만들기
...
#### 2\. 공용 IP 주소를 만든 다음 두 번째 게이트웨이 IP 구성을 추가합니다.

	$gwpip2     = New-AzureRmPublicIpAddress -Name $GWIPName2 -ResourceGroupName $RG -Location $location -AllocationMethod Dynamic
	Add-AzureRmVirtualNetworkGatewayIpConfig -VirtualNetworkGateway $gw -Name $GWIPconf2 -Subnet $subnet -PublicIpAddress $gwpip2 

#### 3\. 활성-활성 모드를 사용하도록 설정하고 게이트웨이 업데이트
...
#### 2\. 게이트웨이 IP 구성을 제거하고 활성-활성 모드를 사용하지 않도록 설정
