...
VNet에 대한 게이트웨이 서브넷을 만들어서 VPN 게이트웨이를 구성해야 합니다. 모든 게이트웨이 서브넷이 제대로 작동하려면 이름을 GatewaySubnet으로 지정해야 합니다. 게이트웨이 서브넷에 다른 이름을 지정하지 않도록 해야 하며 VM 등을 게이트웨이 서브넷에 배포하지 마십시오. 게이트웨이 서브넷에 대한 자세한 내용은 VPN 게이트웨이 문서에서 [게이트웨이 서브넷](vpn-gateway-about-vpngateways.md#gwsub)을 참조하세요.
...
#### <a name="connectiontype"></a>연결 유형 정보
...
VPN 장치를 구성한 경험이 필요합니다. VPN 장치에 대한 자세한 내용은 [VPN 장치 정보](vpn-gateway-about-vpn-devices.md)를 참조하세요. VPN 장치 문서에는 검증된 장치, 검증되지 않은 장치의 요구 사항, 각 장치의 장치 구성 문서(사용 가능한 경우)에 대한 링크가 포함되어 있습니다.
...
강제 터널링 구성에 대한 자세한 내용은 [클래식 배포 모델의 강제 터널링 정보](vpn-gateway-about-forced-tunneling.md) 및 [리소스 관리자 배포 모델의 강제 터널링 정보](vpn-gateway-about-forced-tunneling.md)를 참조하세요.
...
연결 토폴로지에 대한 자세한 내용은 [연결 토폴로지](vpn-gateway-topology.md)를 참조하세요.
