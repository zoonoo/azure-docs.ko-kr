...
지점 및 사이트 간 연결을 작동하는 데는 VPN 장치 또는 공용 IP 주소가 필요하지 않습니다. 클라이언트 컴퓨터에서 연결을 시작하여 VPN 연결을 설정합니다. 지점 및 사이트 간 연결에 대한 자세한 내용은 이 문서의 끝에 있는 [지점 및 사이트 간 FAQ](#faq)를 참조하세요.

이 문서에서는 Azure Portal을 사용하여 지점 및 사이트 간 연결로 VNet을 만드는 방법을 안내합니다. 해당 단계는 Resource Manager 배포 모델에 적용됩니다.
...
## <a name="a-nameaddressapart-2---specify-address-space-and-subnets"></a><a name="address"></a>2부 - 주소 공간 및 서브넷 지정
VNet이 만들어지면 여기에 다른 주소 공간 및 서브넷을 추가할 수 있습니다.

[!INCLUDE [vpn-gateway-additional-address-space](../../includes/vpn-gateway-additional-address-space-include.md)]

## <a name="a-namegatewaysubnetapart-3---add-a-gateway-subnet"></a><a name="gatewaysubnet"></a>3부 - 게이트웨이 서브넷 추가
...
## <a name="a-namednsapart-4---specify-a-dns-server-optional"></a><a name="dns"></a>4부 - DNS 서버 지정(선택 사항)
[!INCLUDE [vpn-gateway-add-dns-rm-portal](../../includes/vpn-gateway-add-dns-rm-portal-include.md)]

## <a name="a-namecreategwapart-5---create-a-virtual-network-gateway"></a><a name="creategw"></a>5부 - 가상 네트워크 게이트웨이 만들기
...
## <a name="a-namegeneratecertapart-6---generate-certificates"></a><a name="generatecert"></a>6부 - 인증서 생성
...
### <a name="a-namegetcerastep-1---obtain-the-cer-file-for-the-root-certificate"></a><a name="getcer"></a>1단계 - 루트 인증서용 .cer 파일 가져오기

루트 인증서용 .cer 파일을 가져와야 합니다. 엔터프라이즈 솔루션에서 루트 인증서를 사용 하거나 [makecert를 사용하여 자체 서명된 루트 인증서를 만들](vpn-gateway-certificates-point-to-site.md) 수 있습니다. PowerShell을 사용하여 자체 서명된 인증서를 만들 수 있지만, PowerShell을 사용하여 생성된 인증서는 P2S 연결에 필요한 필드를 포함하지 않습니다.

1. 인증서에서 .cer 파일을 가져오려면 **certmgr.msc**를 열고 루트 인증서를 찾습니다. 자체 서명된 루트 인증서를 마우스 오른쪽 단추로 클릭하고 **모든 작업**을 클릭한 후 **내보내기**를 클릭합니다. **인증서 내보내기 마법사**가 열립니다.
...
### <a name="a-namegenerateclientcertastep-2---generate-a-client-certificate"></a><a name="generateclientcert"></a>2단계 - 클라이언트 인증서 생성
...
### <a name="a-nameexportclientcertastep-3---export-the-client-certificate"></a><a name="exportclientcert"></a>3단계 - 클라이언트 인증서 내보내기
...
## <a name="a-nameaddresspoolapart-7---add-the-client-address-pool"></a><a name="addresspool"></a>7부 - 클라이언트 주소 풀 추가
...
## <a name="a-nameuploadfileapart-8---upload-the-root-certificate-cer-file"></a><a name="uploadfile"></a>8부 - 루트 인증서 .cer 파일 업로드
...
## <a name="a-nameclientconfigapart-9---download-and-install-the-vpn-client-configuration-package"></a><a name="clientconfig"></a>9부 - VPN 클라이언트 구성 패키지 다운로드 및 설치
...
2. 클라이언트에 대한 올바른 패키지를 선택한 후 **다운로드**를 클릭합니다. 64비트 클라이언트인 경우 **AMD64**를 선택합니다. 32비트 클라이언트인 경우 **x86**을 선택합니다.
    ...
    ![VPN 클라이언트 다운로드 2](./media/vpn-gateway-howto-point-to-site-resource-manager-portal/client.png)
...
## <a name="a-nameinstallclientcertapart-10---install-the-client-certificate"></a><a name="installclientcert"></a>10부 - 클라이언트 인증서 설치
...
## <a name="a-nameconnectapart-11---connect-to-azure"></a><a name="connect"></a>11부 - Azure에 연결
...
## <a name="a-nameverifyapart-12---verify-your-connection"></a><a name="verify"></a>12부 - 연결 확인
...
**지점 및 사이트 간 구성** 블레이드에서 해지된 클라이언트 인증서 목록을 관리할 수 있습니다. [신뢰할 수 있는 루트 인증서를 업로드](#uploadfile)하는 데 사용한 블레이드입니다. 인증서 이름 및 지문을 추가한 다음 저장합니다.

## <a name="a-namefaqapoint-to-site-faq"></a><a name="faq"></a>지점 및 사이트 간 FAQ
