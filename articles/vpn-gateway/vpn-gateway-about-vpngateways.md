...
VPN 게이트웨이는 가상 네트워크와 온-프레미스 위치 간에 네트워크 트래픽을 보내는 데 사용되는 설정의 컬렉션입니다. 이 문서의 섹션에서는 VPN 게이트웨이와 관련된 설정을 설명합니다. VPN 게이트웨이는 사이트간, 지점-사이트 및 Express 경로 연결에 사용됩니다. 또한 VPN 게이트웨이는 Azure 내의 여러 가상 네트워크 간(VNet 간)에 트래픽을 보내는 데에도 사용됩니다.

VPN 게이트웨이를 가상 네트워크에 추가하여 연결을 만들 수 있습니다. 각 가상 네트워크에는 하나의 VPN 게이트웨이가 있을 수 있으며 각 연결에 대한 특정 구성 단계가 있습니다. 연결 다이어그램은 [VPN 게이트웨이 연결 토폴로지](vpn-gateway-topology.md)를 참조하세요.
...
아래 표는 게이트웨이 유형과 예상 총 처리량을 보여 줍니다. 이 표는 리소스 관리자 배포 모델과 클래식 배포 모델 모두에 적용됩니다.
...
Resource Manager 배포 모델에 대한 이 예제는 -GatewayType을 *Vpn*으로 지정합니다. 게이트웨이를 만들 때 게이트웨이 유형이 구성에 정확한지 확인해야 합니다.
...
Resource Manager 배포 모델에 대한 이 예제는 `-VpnType`을 *RouteBased*로 지정합니다. 게이트웨이를 만들 때 -VpnType이 구성에 정확한지 확인해야 합니다.
