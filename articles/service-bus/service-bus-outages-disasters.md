...
ACS 가동 중단으로부터 보호하려면 SAS(공유 액세스 서명) 토큰을 사용하십시오. 이 경우 클라이언트는 비밀 키가 포함된 자체 발행 토큰에 서명하는 방식으로 서비스 버스와 직접 인증합니다. 따라서 ACS 호출은 필요하지 않습니다. SAS 토큰에 대한 자세한 내용은 [서비스 버스 인증][]을 참조하세요.

## 메시지 저장소 오류로부터 큐와 항목 보호

분할되지 않은 큐나 항목은 하나의 메시징 저장소에 할당됩니다. 이 메시지 저장소를 사용할 수 없게 되면 해당 큐 또는 항목의 모든 작업이 실패하게 됩니다. 반면에 분할된 큐는 여러 조각으로 구성됩니다. 각 조각은 서로 다른 메시징 저장소에 저장됩니다. 분할된 큐 또는 항목으로 메시지를 보내면 서비스 버스는 메시지를 조각 중 하나에 할당합니다. 해당 메시징 저장소를 사용할 수 없는 경우, 서비스 버스는 가능한 다른 조각에 메시지를 씁니다. 분할된 엔터티에 대한 자세한 내용은 [분할된 메시징 엔터티][]를 참조하세요.
...
## 데이터센터 가동 중단 또는 재해로부터 릴레이 끝점 보호
...
[서비스 버스 조정된 메시지를 사용한 지역 복제][] 샘플을 통해 메시징 엔터티를 능동 복제하는 방법을 볼 수 있습니다.

> [AZURE.NOTE] 능동 복제 방식은 작업의 수가 두 배이므로 그만큼 비용이 많이 소요됩니다.
...
[서비스 버스 조정된 메시지를 사용한 지역 복제][] 샘플을 통해 메시징 엔터티를 수동 복제하는 방법을 볼 수 있습니다.

## 지속형 클라이언트 쪽 큐

응용 프로그램이 서비스 버스 엔터티를 사용할 수 없어도 작동 가능하지만 메시지를 손실해서는 안 되는 경우, 발신자는 지속형 클라이언트 쪽 큐를 사용할 수 있습니다. 이렇게 하면 서비스 버스로 보낼 수 없는 모든 메시지가 로컬로 저장됩니다. 서비스 버스 엔터티를 다시 사용할 수 있게 되면 버퍼링된 모든 메시지가 해당 엔터티로 전송됩니다. [지속형 메시지 발신자][] 샘플에서는 MSMQ를 통해 이러한 큐를 구현합니다. 또는 로컬 디스크에 메시지를 쓸 수도 있습니다.

지속형 클라이언트 쪽 큐는 메시지 순서를 유지하며 서비스 버스 엔터티를 사용할 수 없을 때 예외로부터 클라이언트 응용 프로그램을 보호합니다. 단순 트랜잭션 및 분산 트랜잭션으로도 사용할 수 있습니다.

> [AZURE.NOTE] 이 샘플은 로컬 디스크 또는 MSMQ용 디스크가 저장소 계정에 매핑되고 메시지는 MSMQ와 함께 안정적으로 저장되는 IaaS(서비스 제공 인프라) 시나리오에서 잘 작동합니다. 클라우드 서비스 및 웹 응용 프로그램과 같은 PaaS(Platform as a Service) 시나리오에는 적합하지 않습니다.
