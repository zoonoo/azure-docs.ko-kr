...
Visual Studio 2013 및 무료로 제공되는 Azure SDK for .NET을 사용하면 Microsoft Azure로 하이브리드 클라우드 응용 프로그램을 개발하는 일이 간단합니다. 이 가이드에서는 이전에 Azure를 사용한 경험이 없는 개발자를 대상으로 합니다. 30분 이내에 여러 Azure 리소스를 사용하는 응용 프로그램을 클라우드에서 실행할 수 있습니다.
...
3.  관리 포털의 아래쪽 창에서 **Create**를 클릭합니다. ![][5]

4.  **Add a new namespace** 대화 상자에서 네임스페이스 이름을 입력합니다. 시스템에서 사용 가능한 이름인지 즉시 확인합니다. ![][6]
...
6.  왼쪽의 **응용 프로그램** 탭을 클릭한 후** .NET Framework 4** 또는 **.NET Framework 4.5**가 **대상 프레임워크:** 드롭다운에 표시되는지 확인합니다. 그렇지 않는 경우 드롭다운에서 선택한 후 프로젝트를 다시 로드하라는 메시지가 나타나면 **예**를 클릭합니다.
...
10. 왼쪽에서 **시작 프로젝트**를 클릭합니다. 오른쪽에서 **여러 개의 시작 프로젝트**를 클릭합니다. **ProductsServer**, **ProductsPortal.Azure** 및 **ProductsPortal**이 이 순서대로 나타나고 **ProductsServer** 및 **ProductsPortal.Azure**의 동작으로 **시작**이 설정되어 있고 **ProductsPortal**의 동작으로 **없음**이 설정되어 있는지 확인합니다. 예:
