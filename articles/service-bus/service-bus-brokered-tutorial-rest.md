...
1. **공유 액세스 키 생성기** 섹션에서 **RootManageSharedAccessKey** 정책과 연결된 **기본 키**를 메모해 두거나 클립보드로 복사합니다. 나중에 이 자습서에서 이 값을 사용하게 됩니다.
...
- 서비스 버스에서 구독 규칙을 포함하여 모든 큐, 항목 및 구독 정보를 검색합니다.

- 큐, 항목 및 구독 리소스를 삭제합니다.

서비스가 REST 스타일 웹 서비스이므로 전체 교환이 문자열과 관련이 있기 때문에 특수 형식이 관련되지 않습니다. 즉, Visual Studio 프로젝트가 서비스 버스 라이브러리를 참조하지 않아야 합니다.

1단계에서 서비스 네임스페이스와 자격 증명을 구한 후 기본 Visual Studio 콘솔 응용 프로그램을 만듭니다.
