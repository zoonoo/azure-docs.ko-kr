...
큐 및 항목과 같은 메시징 엔터티를 포함하는 서비스 버스 조정된 메시징은 클라우드 범위에서 엔터프라이즈 메시징 기능을 풍부한 게시-구독 의미 체계에 결합합니다. 서비스 버스 조정된 메시징은 정교한 여러 클라우드 솔루션의 통신 백본으로 사용됩니다.

서비스 버스 메시징의 *프리미엄* 계층을 도입하면서 업무용 응용 프로그램에 대한 확장성, 성능 및 가용성에 대한 일반적인 고객 요구가 해결되었습니다. 기능 집합은 거의 동일하지만 이러한 Azure 서비스 버스 메시징의 두 계층은 다른 용도로 사용될 수 있게 고안되었습니다.
...
**Azure 서비스 버스 프리미엄 메시징**은 각 고객의 작업이 따로 실행되도록 CPU 및 메모리 계층에서 리소스 격리를 제공합니다. 이 리소스 컨테이너를 *메시징 단위*라고 합니다. 각 프리미엄 네임스페이스에는 하나 이상의 메시징 단위가 할당됩니다. 각 서비스 버스 프리미엄 네임스페이스에 대해 1, 2 또는 4 메시징 단위를 구입할 수 있습니다. 요금은 24시간 단위 또는 일별 요금으로 부과되더라도 단일 작업 또는 엔터티가 여러 메시징 단위에 걸쳐 있을 수 있고 메시징 단위 수를 변경할 수도 있습니다. 그 결과, 서비스 버스 기반 솔루션에 대해 예측 가능하고 반복 가능한 성능이 구현됩니다.

이로 인해 예측 가능성 및 가용성도 높아질 뿐 아니라 속도도 더 빨라집니다. Azure 서비스 버스 프리미엄 메시징은 [Azure 이벤트 허브](http://azure.microsoft.com/services/event-hubs/)에 도입된 저장소 엔진 위에 구축됩니다. 프리미엄 메시징을 사용할 경우 표준 계층보다 최고 성능이 훨씬 더 빠릅니다.
