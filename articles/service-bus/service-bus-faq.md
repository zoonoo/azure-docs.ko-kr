...
[릴레이 서비스](service-bus-relay-overview.md)는 투명하게 호스팅하고 어디에서나 WCF 서비스에 액세스하는 기능을 제공합니다. 즉, 이 프로그램을 사용하면 Azure 데이터 센터와 온-프레미스 엔터프라이즈 환경 모두에서 실행되는 하이브리드 응용 프로그램을 활성화할 수 있습니다.
...
-   [서비스 버스 중단 및 재해로부터 응용 프로그램을 보호하는 모범 사례][] - 이 문서에서는 릴레이 끝점, 큐 및 토픽과 활성 및 수동 복제본을 데이터 센터 가동 중단으로부터 보호하는 가장 좋은 방법을 설명합니다.
...
서비스 버스 제한 및 할당량 목록은 [할당량 개요][]를 참조하세요.
...
토픽/큐(영구 저장소 지원 엔터티)는 서비스 네임스페이스에 최대 10,000개로 제한합니다. 이 할당량에 도달하면 서비스 네임스페이스에 새 항목/큐를 만들기 위한 후속 요청이 거부됩니다. 이 경우에 만들기 시도가 포털을 통해 또는 클라이언트 코드에서 수행되었는지 여부에 따라 Azure 클래식 포털이 오류 메시지를 표시하거나 호출 클라이언트 코드에서 예외가 발생합니다.
...
[서비스 버스 메시징 예외][Exceptions overview] 문서에서는 권장된 조치를 사용하여 몇 가지 예외를 설명합니다.
