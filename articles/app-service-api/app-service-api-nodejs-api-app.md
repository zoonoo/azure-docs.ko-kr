---
title: Azure 앱 서비스의 Node.js API 앱 | Microsoft Docs
...
4. 스캐폴드된 코드가 들어 있는 폴더(이 경우는 *ContactList* 하위 폴더)로 이동합니다.
...
5. **앱 서비스 계획/위치**를 클릭한 다음 **새로 만들기**를 클릭합니다.
...
2. Postman 또는 Fiddler(또는 사용 중인 웹 브라우저)와 같은 REST API 클라이언트를 사용하여 연락처 API 호출 URL(API 앱의 `/contacts` 끝점)을 입력합니다. URL은 `https://{your API app name}.azurewebsites.net/contacts`입니다.
