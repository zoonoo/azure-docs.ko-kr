<properties 
	...
	keywords="앱 서비스, Azure 앱 서비스, 인증, Azure 인증, API, 인증 API"
...
웹앱에 추가하는 코드는 [클라이언트 흐름](../app-service/app-service-authentication-overview.md#client-flow) 인증이라고 하는 프로세스를 구현합니다. 다음 다이어그램에서는 AAD 액세스 토큰을 가져오고 API 응용 프로그램 (Zumo) 토큰에 대해 교환하기 위한 과정을 보여줍니다.

![](./media/app-service-api-authentication-client-flow/clientflow.png)

API 앱에 대한 인증에서 API 앱 게이트웨이의 역할에 익숙하지 못한 경우 [API 앱 및 모바일 앱에 대한 인증](../app-service/app-service-authentication-overview.md)을 참조하세요.
...
1. API 앱 프로젝트를 만들거나 다운로드합니다.
	...
	* 프로젝트를 만들려면 [API 앱 만들기](app-service-dotnet-create-api-app.md)지침에 따릅니다.

	* 프로젝트를 다운로드하려면 [ContactsList GitHub 리포지토리](https://github.com/tdykstra/ContactsList)에서 프로젝트를 가져옵니다.
...
2. Azure에서 새 API 앱에 웹 API 프로젝트 배포
	...
	[Azure 미리 보기 포털]에서, API 앱에 대한 **API 정의** 블레이드에서는 배포한 웹 API의 Get 및 Post 메서드를 표시합니다.
...
이 섹션에서는 AAD 인증을 위해 설정하는 웹 프로젝트를 다운로드 및 구성합니다. 프로젝트에는 로그인된 사용자에 대한 클레임 정보를 표시하는 **정보** 페이지가 있습니다.
