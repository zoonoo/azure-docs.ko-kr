...
- [하이브리드 연결 관리자를 설치] (](.. / app-service-logic/app-service-logic-hybrid-connection-manager.md). (기본 제공 커넥터 API 앱에 적용됩니다.) 하이브리드 연결 관리자는 SQL Server 또는 SAP와 같이 온-프레미스 시스템에 연결 기능을 제공합니다. 이 하이브리드 연결은 Azure 리소스와 온-프레미스 리소스 간의 보안을 연결하고 제어하기 위해 Azure 서비스 버스를 사용합니다.
...
이 문서에서는 Azure 포털을 사용하여 API 앱 관리 작업을 수행하는 방법을 보여 주었습니다.
