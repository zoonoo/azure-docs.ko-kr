---
title: "App Service에서 API Apps 및 ASP.NET 시작 | Microsoft Docs"
...
5. **솔루션 탐색기**의 ToDoListDataAPI 프로젝트에서 *App_Start\SwaggerConfig.cs* 파일을 연 후에 174 줄로 스크롤을 내리고 다음 코드의 주석 처리를 제거합니다.
...
2. **웹 게시** 마법사의 **프로필** 단계에서 **Microsoft Azure App Service**를 클릭합니다.
...
4. App Service 대화 상자에서 사용할 Azure **구독**을 선택하고 **새로 만들기**를 클릭합니다.
   
    ![앱 서비스 대화 상자에서 새로 만들기 클릭](./media/app-service-api-dotnet-get-started/clicknew.png)
   
    **App Service 만들기** 대화 상자의 **호스팅** 탭이 나타납니다.
...
7. **App Service 계획** 드롭다운 옆의 **새로 만들기** 단추를 클릭합니다.
...
11. **App Service 계획 구성** 대화 상자에서 **확인**을 클릭합니다.
    
    ![앱 서비스 계획 구성에서 확인 클릭](./media/app-service-api-dotnet-get-started/configasp.png)
12. **App Service 만들기** 대화 상자에서 **만들기**를 클릭합니다.
...
4. **App Service** 대화 상자에서 이 자습서에 사용하는 리소스 그룹을 확장하고 API 앱을 선택한 다음 **확인**을 클릭합니다.
   ...
   > [!TIP]
   > 코드 생성에 대한 메타데이터를 가져오는 다른 방법은 찾아보기 대화 상자를 사용하는 대신 URL을 직접 입력하는 것입니다. 또는 Azure에 배포하기 전에 클라이언트 코드를 생성하려는 경우 로컬로 Web API 프로젝트를 실행하고, Swagger JSON 파일을 제공하는 URL로 이동한 다음, 파일을 저장하여 **기존 Swagger 메타데이터 파일 선택** 을 사용합니다.
   > 
   > 
...
6. ToDoListAPI 프로젝트에서 *Controllers\ToDoListController.cs*를 열고 생성된 클라이언트를 사용하여 API를 호출하는 40 줄의 코드를 표시합니다.
...
## <a name="create-an-api-app-to-host-the-middle-tier"></a>중간 계층을 호스트하는 API 앱 만들기
...
4. **App Service 만들기** 대화 상자의 **호스팅** 탭에서 기본 **API 앱 이름**을 허용하거나 *azurewebsites.net* 도메인에 고유한 이름을 입력합니다.
...
## <a name="configure-the-middle-tier-to-call-the-data-tier"></a>중간 계층을 구성하여 데이터 계층 호출
...
Azure 앱 서비스에서 ASP.NET API 앱이 실행 중인 경우 문제 해결을 단순화하는 Visual Studio 기능에 대한 자세한 내용을 확인할 수 있습니다. 로깅, 원격 디버깅 등에 대한 정보는 [Visual Studio에서 Azure App Service 앱 문제 해결](../app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md)을 참조하세요.
