...
이 문서에서는 Azure 앱 서비스의 API 앱에 대한 인증 및 권한 부여를 처리하는 옵션에 대해 설명합니다.

다음 다이어그램에서는 앱 서비스 인증의 몇 가지 주요 특징을 보여 줍니다.

* 들어오는 API 요청을 전처리하여 사용자 고유의 코드에서 얼마나 많은 인증이 작동하도록 할지에 대한 여러 옵션을 제공합니다. 
* 여기서는 Azure Active Directory, Facebook, Google, Twitter 및 Microsoft 계정의 5가지 인증 공급자를 지원합니다.
* 최종 사용자 및 서비스 주체 인증 모두에 대해 작동합니다. 
...
앱 서비스는 익명 HTTP 요청이 API 앱에 도달하지 못하도록 하고 API 앱에 도달하기 전에 토큰이 있는 요청을 인증하도록 할 수 있습니다. API 앱에 대해 세 가지 옵션 중 하나를 구성할 수 있습니다.

1. 인증된 요청만 API 앱에 도달하도록 허용합니다.

	브라우저에서 익명 요청을 받으면 앱 서비스가 로그온 페이지로 리디렉션합니다.

	사용할 인증 공급자(Google, Twitter 등)를 사전에 알고 있는 경우 작동하며 앱 서비스가 로그온 프로세스를 처리하도록 구성할 수 있습니다. 다른 방법으로는 앱 서비스가 익명 요청을 리디렉션할 고유한 URL을 지정할 수 있습니다. 그런 다음 사용자가 다양한 인증 공급자를 제공할 수 있습니다.
...
앱 서비스가 HTTP 요청의 인증 헤더에서 JWT 토큰을 전달하고 임의 언어 또는 프레임워크로 작성된 코드는 해당 토큰에서 필요한 정보를 얻을 수 있습니다. 또한 앱 서비스는 다음과 같은 몇 가지 특수 헤더를 설정하여 가장 일반적으로 사용되는 클레임에 보다 쉽게 액세스할 수 있도록 합니다.
...
Azure 앱 서비스의 인증 및 권한 부여 서비스에 대한 자세한 내용은 [앱 서비스 인증/권한 부여 확장](/blog/announcing-app-service-authentication-authorization/)을 참조하세요.

## 다음 단계

이 문서에서는 앱 서비스 API 앱의 인증 및 권한 부여 기능을 설명했습니다.

ASP.NET 및 API 앱에 대한 자습서의 시작하기 순서를 진행하는 경우 다음 자습서인 [앱 서비스 API 앱에서 사용자 인증](app-service-api-dotnet-user-principal-auth.md)에서 해당 기능을 사용해보세요.
