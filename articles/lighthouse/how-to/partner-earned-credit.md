...
Azure Resource manager 템플릿을 통해 고객을 온 보 딩 하는 경우 다음 프로세스를 사용 하 여 파트너 ID를 연결 하 고 파트너의 획득 크레딧을 사용 하도록 설정 합니다. 이러한 단계를 완료 하려면 [MPN 파트너 ID](/partner-center/partner-center-account-setup#locate-your-mpn-id) 를 알고 있어야 합니다.

간단히 하기 위해 테 넌 트에 서비스 주체 계정을 만들고, MPN ID와 연결한 다음, [PEC에 적격 한 Azure 기본 제공 역할](https://docs.microsoft.com/partner-center/azure-roles-perms-pec)을 사용 하 여 등록 한 모든 고객에 게 액세스 권한을 부여 하는 것이 좋습니다.
...
1. 해당 서비스 주체 계정을 사용 하 여 관리 하는 테 넌 트의 [파트너 ID에 연결](../../cost-management-billing/manage/link-partner-id.md#link-to-a-partner-id) 합니다. 이 작업은 한 번만 수행 하면 됩니다.
1. [Azure Resource Manager 템플릿을 사용 하 여 고객을](onboard-customer.md)등록 하는 경우 pec [에 적격 한 Azure 기본 제공 역할이](https://docs.microsoft.com/partner-center/azure-roles-perms-pec)있는 사용자로 pec Automation 계정을 포함 하는 권한 부여를 포함 해야 합니다.
...
이미 고객을 등록 하는 경우 다른 배포를 수행 하 여 PEC Automation 계정 서비스 주체를 추가 하는 것을 원하지 않을 수 있습니다. 대신, 해당 고객 테 넌 트의 작업에 대 한 액세스 권한이 이미 있는 사용자 계정과 MPN ID를 연결할 수 있습니다. 계정에는 [PEC에 적합 한 Azure 기본 제공 역할이](https://docs.microsoft.com/partner-center/azure-roles-perms-pec)부여 되어 있어야 합니다.

계정이 관리 되는 테 넌 트의 [파트너 ID에 연결](../../cost-management-billing/manage/link-partner-id.md#link-to-a-partner-id) 된 후에는 해당 고객에 대 한 PEC를 받을 수 있습니다.
...
- [Microsoft 파트너 네트워크](/partner-center/mpn-overview)에 조인 하는 방법에 대해 알아봅니다.
