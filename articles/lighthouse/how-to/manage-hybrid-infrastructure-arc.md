...
[Azure Arc Enabled Kubernetes](../../azure-arc/kubernetes/overview.md) 를 통해 고객은 azure 내부 또는 외부에서 Kubernetes 클러스터를 연결 하 고 구성할 수 있습니다. Kubernetes 클러스터가 Azure Arc에 연결 되 면 Azure Resource Manager ID와 관리 되는 id를 사용 하 여 Azure Portal에 표시 됩니다. 클러스터는 표준 Azure 구독에 연결되고, 리소스 그룹에 위치하며, 다른 Azure 리소스와 마찬가지로 태그를 받을 수 있습니다.

이 항목에서는 관리 되는 모든 고객 테 넌 트의 가시성으로 고객의 하이브리드 환경을 관리 하는 확장 가능한 방법으로 서비스 공급자가 Azure Arc 사용 서버 및 Azure Arc 사용 Kubernetes를 사용 하는 방법에 대 한 개요를 제공 합니다.
