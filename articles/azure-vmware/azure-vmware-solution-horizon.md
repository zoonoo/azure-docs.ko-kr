...
>[!NOTE]
>이 문서에서는 VMware 수평 제품에 대해 집중적으로 설명 합니다. 이는 제품 이름이 수평 7에서 마감일로 변경 되기 전에 이전에는 수평 7로 알려져 있습니다. 수평은 몇 가지 공유 구성 요소가 있지만 Azure의 수평 클라우드와는 다른 솔루션입니다. Azure VMware 솔루션의 주요 장점에는 보다 간단한 크기 조정 방법과 VMware Cloud Foundation 관리가 Azure Portal에 통합 되어 있습니다.

[VMware 수평](https://www.vmware.com/products/horizon.html)®은 데이터 센터에서 실행 되 고 간단 하 고 중앙 집중식 관리를 제공 하는 가상 데스크톱 및 응용 프로그램 플랫폼입니다. 어디서 든 모든 장치에서 최종 사용자에 게 가상 데스크톱 및 응용 프로그램을 제공 합니다. 수평을 통해 Windows 가상 데스크톱, Linux 가상 데스크톱, RDS (원격 데스크톱 서버) 호스트 된 응용 프로그램, 데스크톱 및 물리적 컴퓨터에 대 한 연결을 만들고 broker 할 수 있습니다.
...
:::image type="content" source="media/horizon/difference-horizon-azure-vmware-solution-horizon-cloud-azure.png" alt-text="Azure의 Azure VMware 솔루션 및 수평 클라우드의 수평 간 차이" border="false":::
...
수평 클라우드 Pod 아키텍처 (CPA)를 사용 하 여 온-프레미스 데이터 센터와 Azure 데이터 센터를 상호 연결 하는 경우 하이브리드 클라우드 환경에서 수평을 배포할 수 있습니다. CPA는 일반적으로 배포를 강화 하 고, 하이브리드 클라우드를 구축 하 고, 비즈니스 연속성 및 재해 복구를 위한 중복성을 제공 하는 데 사용 됩니다. VMware 수평 비즈니스 연속성 지침에 대 한 자세한 내용은 [기존 수평 7 환경 확장](https://techzone.vmware.com/resource/business-continuity-vmware-horizon#_Toc41650874)을 참조 하세요.
...
각 Azure private cloud/SDDC은 4000 데스크톱 또는 앱 세션을 처리할 수 있으며 다음을 가정 합니다.

* 작업 트래픽이 Loginvsi 등과 작업 작업자 프로필의 트래픽과 일치 합니다.
...
단일 수평 pod는 가장 간단한 전달 배포 시나리오입니다. 이 예에서는 미국 동부 지역에 하나의 수평 pod만 배포 하려는 경우를 결정 합니다. 각 사설 클라우드/a s/SDDC은 약 4000 데스크톱 세션의 트래픽을 처리 하는 것으로 예상 되므로 최대 수평 pod 크기를 배포 하기 위해 최대 3 개의 사설 클라우드/SDDCs 배포를 계획할 수 있습니다.

따라서이 예제에서는 Azure Virtual Network에 배포 된 수평 인프라 Vm (가상 머신)과 함께 워크 로드 및 데이터 요구 사항에 따라 수평 pod 당 12000 세션에 도달할 수 있습니다. Azure Virtual Network에 대 한 각 사설 클라우드 및 SDDC 간 연결은 사설 클라우드 간의 동-서 트래픽이 필요 하지 않도록 Express 경로 빠른 경로입니다.
...
이 기본 예제에서는 VPN 또는 Express 경로 회로를 통해 Azure Virtual Network의 AD 도메인 컨트롤러를 온-프레미스 Active Directory에 연결할 수 있습니다.

설명 된 기본 예제에 대 한 변형은 온-프레미스 리소스에 대 한 연결을 지 원하는 것입니다. 이는 데스크톱에 액세스 하 여 가상 데스크톱 응용 프로그램 트래픽을 생성 하거나 CPA를 사용 하 여 온-프레미스 수평 pod에 연결 하는 사용자 일 수 있습니다.

다음 다이어그램에서는이 작업을 수행 하는 방법을 보여 줍니다.회사 네트워크를 Azure Virtual Network에 연결 하려면 Express 경로가 필요 합니다.또한 SDDC에서 Express 경로 및 온-프레미스 리소스에 연결할 수 있도록 하는 Global Reach을 사용 하 여 회사 네트워크를 각 사설 클라우드/SDDCs와 연결 해야 합니다.

:::image type="content" source="media/horizon/connect-corporate-network-azure-virtual-network.png" alt-text="Azure의 Azure VMware 솔루션 및 수평 클라우드의 수평 간 차이" border="false":::

### <a name="multiple-horizon-pods-on-azure-vmware-solution-across-multiple-regions"></a>여러 지역에서 Azure VMware 솔루션의 여러 수평 pod

다른 수평 pod 예의 경우 여러 Pod에 걸친 수평 확장을 보여 주는 예를 살펴보겠습니다.이 예제에서는 두 개의 다른 지역에 두 개의 수평 pod를 배포 하 고 CPA를 사용 하 여 페더레이션 합니다.네트워크 구성은 앞의 예제와 유사 하며, 일부 추가 지역 간 링크를 포함 합니다. 

CPA 페더레이션의 일부인 수평 연결 서버가 관리 하는 모든 데스크톱에 연결할 수 있도록 각 지역의 Azure Virtual Network을 다른 지역의 사설 클라우드/SDDCs에 연결 해야 합니다.이 구성에 사설 클라우드/SDDCs를 추가 하면 전체 24000 세션으로 확장할 수 있습니다. 

이 예제에서는 여러 지역을 표시 하지만 동일한 지역에 두 개의 수평 pod를 배포 하려는 경우에도 동일한 원칙이 적용 됩니다. 두 번째 수평 pod가 *별도의 Azure Virtual Network*에 배포 되었는지 확인 해야 합니다.마지막으로, 이전 단일 Pod 예제에서와 같이 고객 Express 경로 및 Global Reach를 사용 하 여 회사 네트워크 및 온-프레미스 Pod를이 다중 Pod/지역 예제에 연결할 수 있습니다.

:::image type="content" source="media/horizon/multiple-horizon-pod-azure-vmware-solution.png" alt-text="Azure의 Azure VMware 솔루션 및 수평 클라우드의 수평 간 차이" border="false":::

## <a name="size-azure-vmware-solution-hosts-for-horizon-deployments"></a>수평 배포를 위한 Azure VMware 솔루션 호스트 크기 조정 

Azure vmware 솔루션 호스트 인스턴스가 표준화 되기 때문에 Azure VMware 솔루션에서 실행 되는 호스트에 대 한 수평 크기 조정 방법은 수평 온-프레미스 보다 간단 합니다. 정확한 호스트 크기 조정은 VDI 요구 사항을 지 원하는 데 필요한 호스트 수를 결정 하는 데 도움이 되며 데스크톱 당 비용을 결정 하는 데 중요 합니다.
...
일반적으로 VDI 배포는 CPU 또는 RAM 제약을 받습니다. 이러한 요소는 호스트 크기를 결정 합니다. Loginvsi 등과 지식 근로자의 작업 부하에 대 한 다음 예제를 사용 하 여 성능 테스트로 유효성을 검사 합니다.
...
재해 복구 사용 사례와 마찬가지로 Azure VMware 솔루션 및 온-프레미스에 두 가지를 모두 배포 하는 경우에는 수평 세계 구독 라이선스를 선택 합니다. 유니버설 라이선스는 온-프레미스 배포에 대 한 vSphere 라이선스를 포함 하기 때문에 비용이 더 많이 듭니다.
...
\$위의 예제에서 2000-데스크톱 배포에 대 한 인프라 VM은 매월 사용자 당 0.36에 해당 합니다. 이 예제에서는 6 월 2020을 기준으로 미국 동부 Azure 인스턴스 가격 책정을 사용 합니다. 가격은 지역, 선택한 옵션 및 타이밍에 따라 달라질 수 있습니다.
