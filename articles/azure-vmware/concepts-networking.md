...
Azure virtual network를 Azure VMware 솔루션 사설 클라우드 구현과 상호 연결할 수 있습니다. Azure VMware 솔루션 사설 클라우드를 관리 하 고, 사설 클라우드에서 워크 로드를 사용 하 고, 다른 Azure 서비스에 액세스할 수 있습니다.

아래 다이어그램은 사설 클라우드 배포 시 설정 된 기본 네트워크 상호 연결과를 보여 줍니다. Azure의 가상 네트워크와 사설 클라우드의 논리적 네트워킹을 보여 줍니다. 이러한 연결은 Azure VMware 솔루션 서비스의 일부인 백 엔드 Express 경로를 통해 설정 됩니다. 상호 연결과는 다음과 같은 주요 사용 사례를 충족 합니다.

- Azure 구독의 Vm에서 액세스할 수 있는 vCenter server 및 NSX manager에 대 한 인바운드 액세스입니다.
- 사설 클라우드의 Vm에서 Azure 서비스로의 아웃 바운드 액세스
- 사설 클라우드에서 실행 되는 워크 로드에 대 한 인바운드 액세스입니다.
...
완전히 상호 연결 된 시나리오에서는 Azure 가상 네트워크 및 온-프레미스에서 Azure VMware 솔루션에 액세스할 수 있습니다. 이 구현은 이전 섹션에서 설명 하는 기본 구현의 확장입니다. Express 경로 회로는 온-프레미스에서 azure의 Azure VMware 솔루션 사설 클라우드로 연결 하는 데 필요 합니다.

아래 다이어그램은 온-프레미스에서 사설 클라우드로 상호 연결과를 보여 주며,이를 통해 다음과 같은 사용 사례를 사용할 수 있습니다.

- 온-프레미스와 Azure VMware 솔루션 간의 핫/콜드 vCenter vMotion.
...
사설 클라우드에 대 한 전체 상호 연결과 경우 Express 경로 Global Reach를 사용 하도록 설정 하 고 Azure Portal의 Global Reach에 대 한 인증 키 및 개인 피어 링 ID를 요청 해야 합니다. 권한 부여 키 및 피어 링 ID는 구독의 Express 경로 회로와 사설 클라우드의 Express 경로 회로 간에 Global Reach를 설정 하는 데 사용 됩니다. 연결 되 면 두 개의 Express 경로 회로에서 온-프레미스 환경 간의 네트워크 트래픽을 사설 클라우드로 라우팅합니다. 이 절차에 대 한 자세한 내용은 [사설 클라우드로 express 경로 Global Reach 피어 링을 만드는 자습서](tutorial-expressroute-global-reach-private-cloud.md)를 참조 하세요.
...
이제 Azure VMware 솔루션 네트워크 및 상호 연결과 개념을 살펴보았습니다. 다음에 대해 알아볼 수 있습니다.
