---
title: Azure VMware 솔루션 (AVS)-Oracle RAC 용 AVS 사설 클라우드 최적화
...
---
# <a name="optimize-your-avs-private-cloud-for-installing-oracle-rac"></a>Oracle RAC 설치를 위한 AVS 사설 클라우드 최적화

AVS 사설 클라우드 환경에 Oracle RAC (Real Application 클러스터)를 배포할 수 있습니다. 이 가이드에서는 새 클러스터를 배포 하 고 Oracle RAC 솔루션에 대 한 VM을 최적화 하는 방법을 설명 합니다. 이 항목의 단계를 완료 한 후 Oracle RAC를 설치 하 고 구성할 수 있습니다.
...
다음 섹션에서는 Oracle RAC에 대해 AVS 사설 클라우드 환경을 설정 하는 방법을 설명 합니다.

1. 디스크 구성 모범 사례
2. AVS 사설 클라우드 vSphere 클러스터 배포
...
|-------------------------------------------|-----------------------------------------------|-------------|
| OS                                        | 운영 체제 디스크                         | 아닙니다.          |
| 그리드에                                      | Oracle 그리드 소프트웨어의 설치 위치     | 아닙니다.          |
| DATABASE                                  | Oracle 데이터베이스 소프트웨어의 설치 위치 | 아닙니다.          |
| ORAHOME                                   | Oracle 데이터베이스 이진 파일의 기본 위치    | 아닙니다.          |
...
## <a name="deploy-avs-private-cloud-vsphere-cluster"></a>AVS 사설 클라우드 vSphere 클러스터 배포

AVS 사설 클라우드에 vSphere 클러스터를 배포 하려면 다음 프로세스를 수행 합니다.

1. AVS 포털에서 [Avs 사설 클라우드를 만듭니다](create-private-cloud.md). AVS는 새로 만든 AVS 사설 클라우드에 ' cloudowner ' 라는 기본 vCenter 사용자를 만듭니다. 기본 AVS 사설 클라우드 사용자 및 사용 권한 모델에 대 한 자세한 내용은 [Avs 사설 클라우드 권한 모델 알아보기](learn-private-cloud-permissions.md)를 참조 하세요. 이 단계에서는 AVS 사설 클라우드에 대 한 기본 관리 클러스터를 만듭니다.

2. AVS 포털에서 새 클러스터를 사용 하 여 [Avs 사설 클라우드를 확장](expand-private-cloud.md) 합니다. 이 클러스터는 Oracle RAC를 배포 하는 데 사용 됩니다. 원하는 내결함성 (3 개 노드 이상)에 따라 노드 수를 선택 합니다.

## <a name="set-up-networking-for-oracle-rac"></a>Oracle RAC에 대 한 네트워킹 설정

1. AVS 사설 클라우드에서 Oracle 공용 네트워크용으로 하나, Oracle 개인 네트워크에 대해 하나, 적절 한 서브넷 CIDRs를 할당 하는 [vlan 두 개를 만듭니다](create-vlan-subnet.md).
2. Vlan을 만든 후에는 [AVS 사설 클라우드 vCenter에서 분산 포트 그룹](create-vlan-subnet.md#use-vlan-information-to-set-up-a-distributed-port-group-in-vsphere)을 만듭니다.
3. Oracle 환경을 위한 관리 클러스터에서 [DHCP 및 DNS 서버 가상 머신을](dns-dhcp-setup.md) 설정 합니다.
4. AVS 사설 클라우드에 설치 된 [dns 서버에서 dns 전달을 구성](on-premises-dns-setup.md#create-a-conditional-forwarder) 합니다.
...
1. AVS 사설 클라우드의 [vSphere 클라이언트에 로그인](https://docs.azure.cloudsimple.com/vsphere-access) 합니다.
...
6. **Vsan** > **가용성** 섹션에서 사이트 재해 허용 범위에 대해 **없음** 을 선택 합니다. 허용 되는 오류에 대해 원하는 FTT의 **RAID 미러링** 옵션을 선택 합니다.
    vSAN 설정 ![](media/oracle-rac-storage-wizard-vsan.png)합니다.
...
1. AVS 포털에서 cloudowner 사용자의 [권한을 에스컬레이션](escalate-private-cloud-privileges.md) 합니다.
2. AVS 사설 클라우드의 [vSphere 클라이언트에 로그인](https://docs.azure.cloudsimple.com/vsphere-access) 합니다.
3. VSphere 클라이언트에서 Oracle Vm이 배포 되는 클러스터를 선택 하 고 **구성**을 클릭 합니다.
4. 구성 아래에서 **v m/호스트 그룹**을 선택 합니다.
5. 페이지 맨 아래에 있는 **+** 를 사용하여 응용 프로그램에 Single Sign-On 할 수 있습니다.
...
8. 페이지 맨 아래에 있는 **+** 를 사용하여 응용 프로그램에 Single Sign-On 할 수 있습니다.
