---
title: Azure VMware 솔루션 (AVS)-온-프레미스 워크 로드에 대 한 재해 사이트로 AVS 사설 클라우드 사용
description: 온-프레미스 VMware 워크 로드에 대 한 재해 복구 사이트로 AVS 사설 클라우드를 설정 하는 방법을 설명 합니다.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/20/2019
ms.topic: article
...
재해 발생 시 비즈니스 연속성을 제공 하기 위해 온-프레미스 응용 프로그램에 대 한 복구 사이트로 AVS를 설정할 수 있습니다. 복구 솔루션은 Zerto 가상 복제를 복제 및 오케스트레이션 플랫폼으로 기반으로 합니다. 중요 한 인프라 및 응용 프로그램 가상 머신은 온-프레미스 vCenter에서 AVS 사설 클라우드로 지속적으로 복제할 수 있습니다. 장애 조치 (failover) 테스트를 위해 AVS 사설 클라우드를 사용 하 고 재해 발생 시 응용 프로그램의 가용성을 보장할 수 있습니다. 다른 위치의 복구 사이트로 보호 되는 기본 사이트로 AVS 사설 클라우드를 설정 하는 것과 유사한 접근 방식을 사용할 수 있습니다.
...
* 전 세계 지리적 복원 력을 위해 AVS가 배포 되는 Azure 위치를 활용할 수 있습니다.
* 에서는 배포 비용과 DR에 대 한 총 소유 비용을 줄일 수 있는 옵션을 제공 합니다.

이 솔루션을 사용 하려면 다음을 수행 해야 합니다.

* AVS 사설 클라우드에서 Zerto를 설치, 구성 및 관리 합니다.
* AVS 사설 클라우드가 보호 된 사이트인 경우 Zerto에 대 한 고유한 라이선스를 제공 합니다. 라이선스를 위해 온-프레미스 사이트와 함께 AVS 사이트에서 실행 되는 Zerto을 연결할 수 있습니다.
...
다음 섹션에서는 Zerto 가상 복제를 사용 하 여 AVS 사설 클라우드에서 DR 솔루션을 배포 하는 방법을 설명 합니다.

1. [필수 구성 요소](#prerequisites)
2. [AVS 사설 클라우드의 선택적 구성](#optional-configuration-on-your-avs-private-cloud)
...
4. AVS 사설 클라우드에 Windows server를 설치 합니다. 서버는 Zerto Virtual Manager를 설치 하는 데 사용 됩니다.
5. [AVS 권한을 에스컬레이션](escalate-private-cloud-privileges.md)합니다.
6. Zerto Virtual Manager의 서비스 계정으로 사용할 관리 역할을 사용 하 여 AVS 사설 클라우드 vCenter에 새 사용자를 만듭니다.

### <a name="optional-configuration-on-your-avs-private-cloud"></a>AVS 사설 클라우드의 선택적 구성

1. 온-프레미스 환경에서 Vm에 대 한 대상 리소스 풀로 사용할 수 있도록 AVS 사설 클라우드 vCenter에서 하나 이상의 리소스 풀을 만듭니다.
2. 온-프레미스 환경에서 Vm의 대상 폴더로 사용할 하나 이상의 폴더를 AVS 사설 클라우드 vCenter에 만듭니다.
...
5. [DHCP 및 DNS 서버](dns-dhcp-setup.md) 를 설치 하거나, AVS 사설 클라우드 환경에서 Active Directory 도메인 컨트롤러를 사용 합니다.

### <a name="set-up-zvm-and-vra-on-your-avs-private-cloud"></a>AVS 사설 클라우드에서 ZVM 및 VRA 설정

1. AVS 사설 클라우드의 Windows server에 ZVM (Zerto Virtual Manager)을 설치 합니다.
2. 이전 단계에서 만든 서비스 계정을 사용 하 여 ZVM에 로그인 합니다.
3. Zerto 가상 관리자에 대 한 라이선스를 설정 합니다.
4. AVS 사설 클라우드의 ESXi 호스트에 VRA (Zerto Virtual Replication 어플라이언스)를 설치 합니다.
5. AVS 사설 클라우드 ZVM을 온-프레미스 ZVM과 페어링 합니다.
...
3. AVS 사설 클라우드로 복구 사이트를 선택 하 고, 기본 복구 서버를 AVS 사설 클라우드 클러스터로 선택 하거나, 만든 리소스 그룹을 선택 합니다. AVS 사설 클라우드의 복구 데이터 저장소에 대해 **Vsandatastore** 저장소를 선택 합니다.
