---
title: Azure VMware 솔루션 (AVS)-VMware Site Recovery Manager를 사용 하 여 재해 복구 사이트로 AVS 사설 클라우드 설정
...
온-프레미스 VMware 워크 로드를 위한 DR (재해 복구) 사이트로 AVS 사설 클라우드를 사용할 수 있습니다.

DR 솔루션은 vSphere 복제 및 SRM (VMware Site Recovery Manager)을 기반으로 합니다. 온-프레미스 복구 사이트에서 보호 하는 기본 사이트로 AVS 사설 클라우드를 사용 하도록 설정 하기 위해 유사한 접근 방식을 사용할 수 있습니다.
...
AVS 솔루션을 사용 하려면 다음을 수행 해야 합니다.

* AVS 사설 클라우드에서 vSphere 복제 및 SRM을 설치, 구성 및 관리 합니다.
* AVS 사설 클라우드가 보호 된 사이트인 경우 SRM에 대 한 사용자 고유의 라이선스를 제공 합니다. 복구 사이트로 사용 되는 경우에는 AVS 사이트에 대 한 추가 SRM 라이선스가 필요 하지 않습니다.
...
AVS 사설 클라우드 및 온-프레미스 환경과 호환 되는 모든 버전의 vRA 및 SRM을 사용할 수 있습니다. 이 가이드의 예제에서는 vRA 6.5 및 SRM 6.5을 사용 합니다. 이러한 버전은 AVS에서 지 원하는 vSphere 6.5와 호환 됩니다.

## <a name="deploy-the-solution"></a>솔루션 배포

다음 섹션에서는 AVS 사설 클라우드에서 SRM을 사용 하 여 DR 솔루션을 배포 하는 방법을 설명 합니다.
...
3. [사용자 환경에 대 한 AVS 사설 클라우드 만들기](#create-an-avs-private-cloud-for-your-environment)
4. [SRM 솔루션에 대 한 AVS 사설 클라우드 네트워킹 설정](#set-up-avs-private-cloud-networking-for-the-srm-solution)
5. [온-프레미스 네트워크와 AVS 사설 클라우드 간에 사이트 간 VPN 연결을 설정 하 고 필요한 포트를 엽니다.](#set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports)
6. [AVS 사설 클라우드에서 인프라 서비스 설정](#set-up-infrastructure-services-in-your-avs-private-cloud)
7. [온-프레미스 환경에서 vSphere 복제 어플라이언스 설치](#install-vsphere-replication-appliance-in-your-on-premises-environment)
8. [AVS 사설 클라우드 환경에 vSphere 복제 어플라이언스 설치](#install-vsphere-replication-appliance-in-your-avs-private-cloud-environment)
9. [온-프레미스 환경에 SRM 서버 설치](#install-srm-server-in-your-on-premises-environment)
10. [AVS 사설 클라우드에 SRM 서버 설치](#install-srm-server-in-your-avs-private-cloud)
...
* 동일한 버전의 SRM은 AVS 사설 클라우드와 온-프레미스 환경에 배포 해야 합니다.
* 동일한 버전의 vSphere 복제를 AVS 사설 클라우드와 온-프레미스 환경에 배포 해야 합니다.
* AVS 사설 클라우드 및 온-프레미스 환경의 플랫폼 서비스 컨트롤러 (PSC) 버전이 호환 되어야 합니다.
* AVS 사설 클라우드 및 온-프레미스 환경의 vCenter 버전이 호환 되어야 합니다.
...
AVS 사설 클라우드에서 vCenter 및 PSC 버전을 확인 하려면 AVS 포털을 엽니다. **리소스**로 이동 하 여 AVS 사설 클라우드를 선택 하 고 **Vsphere 관리 네트워크** 탭을 클릭 합니다.

![AVS (vCenter & PSC)에서 AVS 사설 클라우드의 버전](media/srm-resources.png)
...
3. AVS sizer 도구를 사용 하 여 온-프레미스 환경을 보호 하기 위해 DR 사이트에서 필요한 리소스를 예상할 수 있습니다.

### <a name="create-an-avs-private-cloud-for-your-environment"></a>사용자 환경에 대 한 AVS 사설 클라우드 만들기

Avs [사설 클라우드 만들기](create-private-cloud.md)의 지침 및 크기 조정 권장 사항에 따라 avs 포털에서 Avs 사설 클라우드를 만듭니다.

### <a name="set-up-avs-private-cloud-networking-for-the-srm-solution"></a>SRM 솔루션에 대 한 AVS 사설 클라우드 네트워킹 설정

AVS 포털에 액세스 하 여 SRM 솔루션에 대 한 AVS 사설 클라우드 네트워킹을 설정 합니다.

SRM 솔루션 네트워크에 대 한 VLAN을 만들고 서브넷 CIDR에 할당 합니다. 자세한 내용은 [vlan/서브넷 만들기 및 관리](create-vlan-subnet.md)를 참조 하세요.

### <a name="set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports"></a>온-프레미스 네트워크와 AVS 사설 클라우드 간에 사이트 간 VPN 연결을 설정 하 고 필요한 포트를 엽니다.

온-프레미스 네트워크와 AVS 사설 클라우드 사이에서 사이트 간 연결을 설정 합니다. 자세한 지침은 [AVS 사설 클라우드에 대 한 VPN 연결 구성](set-up-vpn.md)을 참조 하세요.

### <a name="set-up-infrastructure-services-in-your-avs-private-cloud"></a>AVS 사설 클라우드에서 인프라 서비스 설정

작업 및 도구를 쉽게 관리할 수 있도록 AVS 사설 클라우드에서 인프라 서비스를 구성 합니다.

다음 중 하나를 수행 하려는 경우 [Azure AD를 Azure AD를 사용 하 여 vCenter의 vCenter에 대 한 id 공급자로 사용](azure-ad.md) 에 설명 된 대로 외부 id 공급자를 추가할 수 있습니다.

* AVS 사설 클라우드의 온-프레미스 AD (Active Directory)에서 사용자를 식별 합니다.
* 모든 사용자에 대해 AVS 사설 클라우드에서 AD를 설정 합니다.
* Azure AD를 사용 합니다.
...
## <a name="install-vsphere-replication-appliance-in-your-avs-private-cloud-environment"></a>AVS 사설 클라우드 환경에 vSphere 복제 어플라이언스 설치

시작 하기 전에 다음이 있는지 확인 합니다.

* 온-프레미스 환경의 서브넷에서 AVS 사설 클라우드의 관리 서브넷으로의 IP 연결
* 온-프레미스 vSphere 환경에 있는 복제 서브넷에서 AVS 사설 클라우드의 SRM 솔루션 서브넷으로의 IP 연결

자세한 지침은 [AVS 사설 클라우드에 대 한 VPN 연결 구성](set-up-vpn.md)을 참조 하세요. 이러한 단계는 온-프레미스 설치와 비슷합니다.

AVS는 vRA 및 SRM을 설치 하는 동안 IP 주소 대신 Fqdn을 사용 하는 것을 권장 합니다. AVS 사설 클라우드에서 vCenter 및 PSC의 FQDN을 확인 하려면 AVS 포털을 엽니다. **리소스**로 이동 하 여 AVS 사설 클라우드를 선택 하 고 **Vsphere 관리 네트워크** 탭을 클릭 합니다.

![AVS 사설 클라우드에서 vCenter/PSC의 FQDN 찾기](media/srm-resources.png)

AVS를 사용 하려면 기본 ' cloudowner ' 사용자를 사용 하 여 vRA 및 SRM을 설치 하지 말고 새 사용자를 만들어야 합니다. 이 작업은 AVS 사설 클라우드 vCenter 환경의 가동 시간 및 가용성을 보장 하기 위해 수행 됩니다. 그러나 AVS 사설 Cloud vCenter의 기본 cloudowner 사용자는 관리자 권한으로 새 사용자를 만들 수 있는 충분 한 권한이 없습니다.

VRA 및 SRM을 설치 하기 전에 cloudowner 사용자의 vCenter 권한을 에스컬레이션 하 고 vCenter SSO 도메인에서 관리 권한이 있는 사용자를 만들어야 합니다. 기본 AVS 사설 클라우드 사용자 및 사용 권한 모델에 대 한 자세한 내용은 [Avs 사설 클라우드 권한 모델 알아보기](learn-private-cloud-permissions.md)를 참조 하세요.
...
6. AVS 사설 클라우드 사이트에서 vCenter Single Sign-on을 사용 하 여 AVS 사설 클라우드 vRA를 등록 합니다.
...
#### <a name="vcenter-ui-create-a-user-in-the-avs-private-cloud-for-vra-and-srm-installation"></a>vCenter UI: vRA 및 SRM 설치를 위한 AVS 사설 클라우드에서 사용자 만들기

AVS 포털에서 권한을 에스컬레이션 한 후 cloudowner 사용자 자격 증명을 사용 하 여 vCenter에 로그인 합니다.

VCenter에서 `srm-soln-admin`새 사용자를 만들고 vCenter의 administrators 그룹에 추가 합니다.
Cloudowner 사용자로 vCenter에서 로그 아웃 하 고 *srm-* 사용자로 로그인 합니다.

#### <a name="avs-portal-configure-firewall-rules-for-vra"></a>AVS 포털: vRA에 대 한 방화벽 규칙 구성
...
* vSphere 복제 어플라이언스는 온-프레미스 및 AVS 사설 클라우드 환경에 설치 됩니다.
...
### <a name="install-srm-server-in-your-avs-private-cloud"></a>AVS 사설 클라우드에 SRM 서버 설치
...
다음 단계에서는 AVS 사설 클라우드 SRM 설치에 대해 설명 합니다.

1. [vCenter UI: SRM 설치](#vcenter-ui-install-srm)
2. [AVS 포털: SRM에 대 한 방화벽 규칙 구성](#avs-portal-configure-firewall-rules-for-srm)
3. [vCenter UI: SRM 구성](#vcenter-ui-configure-srm)
4. [AVS 포털: 권한 상승](#avs-portal-de-escalate-privileges)
...
#### <a name="avs-portal-configure-firewall-rules-for-srm"></a>AVS 포털: SRM에 대 한 방화벽 규칙 구성

다음 간의 통신을 허용 하도록 [방화벽 테이블 및 규칙 설정](firewall.md) 에 설명 된 대로 방화벽 규칙을 구성 합니다.

AVS 서버와 AVS 사설 클라우드의 vCenter/PSC
두 사이트의 SRM 서버
...
AVS를 AVS 사설 클라우드에 설치한 후에 VMware Site Recovery Manager 설치 및 구성 가이드의 섹션에 설명 된 대로 다음 작업을 수행 합니다. SRM 6.5의 경우 [Site Recovery Manager를 설치](https://docs.vmware.com/en/Site-Recovery-Manager/6.5/com.vmware.srm.install_config.doc/GUID-437E1B65-A17B-4B4B-BA5B-C667C90FA418.html)하는 VMware 문서에서 지침을 사용할 수 있습니다.
...
#### <a name="avs-portal-de-escalate-privileges"></a>AVS 포털: 권한 상승
...
AVS 사설 클라우드 환경에서 vSphere 복제 및 SRM 소프트웨어를 완전히 제어할 수 있으며 필요한 소프트웨어 수명 주기 관리를 수행할 것으로 예상 됩니다. VSphere 복제 또는 SRM을 업데이트 하거나 업그레이드 하기 전에 새 버전의 소프트웨어가 AVS 사설 클라우드 vCenter 및 PSC와 호환 되는지 확인 합니다.

> [!NOTE]
> AVS는 현재 관리 되는 DR 서비스를 제공 하기 위한 옵션을 탐색 합니다. 

## <a name="multiple-replication-configuration"></a>다중 복제 구성

 [배열 기반 복제와 vSphere 복제 기술은 동시에 SRM과 함께 사용할 수 있습니다](https://blogs.vmware.com/virtualblocks/2017/06/22/srm-array-based-replication-vs-vsphere-replication) . 그러나 별도의 Vm 집합에 적용 해야 합니다. 지정 된 VM은 배열 기반 복제 또는 vSphere 복제를 통해 보호 될 수 있지만 둘 다로 보호할 수는 없습니다. 또한 여러 보호 된 사이트에 대 한 복구 사이트로 AVS 사이트를 구성할 수 있습니다. 다중 사이트 구성에 대 한 자세한 내용은 [SRM 다중 사이트 옵션](https://blogs.vmware.com/virtualblocks/2016/07/28/srm-multisite/) 을 참조 하세요.
