---
title: Azure VMware 솔루션 (AVS)-AVS 사설 클라우드에 대 한 DNS 구성
description: 온-프레미스 워크스테이션에서 AVS 사설 클라우드의 vCenter 서버에 액세스 하기 위해 DNS 이름 확인을 설정 하는 방법을 설명 합니다.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
온-프레미스 워크스테이션에서 AVS 사설 클라우드의 vCenter 서버에 액세스 하려면 IP 주소 뿐만 아니라 호스트 이름을 사용 하 여 vCenter 서버 주소를 지정할 수 있도록 DNS 주소 확인을 구성 해야 합니다.

## <a name="obtain-the-ip-address-of-the-dns-server-for-your-avs-private-cloud"></a>AVS 사설 클라우드에 대 한 DNS 서버의 IP 주소를 가져옵니다.

1. [AVS 포털](access-cloudsimple-portal.md)에 로그인 합니다.

2. **리소스** > **avs 사설** 클라우드로 이동 하 고 연결 하려는 avs 사설 클라우드를 선택 합니다.

3. AVS 사설 클라우드의 **요약** 페이지에 있는 **기본 정보**에서 avs 사설 클라우드 DNS 서버 IP 주소를 복사 합니다.

    ![AVS 사설 클라우드 DNS 서버](media/private-cloud-dns-server.png)
...
6. Avs 포털에서 가져온 AVS 사설 클라우드에 대 한 DNS 서버의 IP 주소를 입력 합니다.
...
3. DNS 도메인 및 dns 서버의 IP 주소를 AVS 사설 클라우드에 입력 하 고 **확인**을 클릭 합니다.
