...
사설 클라우드를 만들기 전에 노드를 프로 비전 해야 합니다.  노드에 프로 비전 하는 방법에 대 한 자세한 내용은 [CloudSimple을 사용한 VMware 솔루션에 대 한 노드 프로 비전-Azure](create-nodes.md) 문서를 참조 하세요.
...
1. **리소스** 페이지에서 **새 사설 클라우드**를 클릭 합니다.

    ![사설 클라우드 만들기-시작 방법](media/create-pc-button.png)

2. 사설 클라우드 리소스를 호스트할 위치를 선택 합니다.

3. 사설 클라우드에 대해 프로 비전 된 CS28 또는 CS36 노드 형식을 선택 합니다. 후자 옵션은 최대 계산 및 메모리 용량을 포함 합니다.

4. 사설 클라우드의 노드 수를 선택 합니다. [프로 비전](create-nodes.md)된 최대 노드 수를 선택할 수 있습니다.

    ![사설 클라우드-기본 설정 만들기](media/create-private-cloud-basic-info.png)

5. **다음: 고급 옵션**입니다.

6. VSphere/Vsphere 서브넷의 CIDR 범위를 입력 합니다. CIDR 범위가 온-프레미스 또는 기타 Azure 서브넷 (가상 네트워크) 또는 게이트웨이 서브넷과 겹치지 않는지 확인 합니다.  Azure 가상 네트워크에 정의 된 CIDR 범위를 사용 하지 마세요.
    ...
    **CIDR 범위 옵션:** /24,/23,/22 또는/21. /24 CIDR 범위는 최대 9 개의 노드를 지원 하 고/23 CIDR 범위는 최대 41 노드를 지원 하며,/22 및/21 CIDR 범위는 최대 64 노드 (사설 클라우드의 최대 노드 수)를 지원 합니다.
    ...
    > [!CAUTION]
    > VSphere/Vsphere CIDR 범위의 IP 주소는 사설 클라우드 인프라에서 사용 하도록 예약 되어 있습니다.  가상 머신에서이 범위의 IP 주소를 사용 하지 마십시오.

7. **다음: 검토 하 고**만듭니다.

8. 설정을 검토 합니다. 설정을 변경 해야 하는 경우에는 **이전**을 클릭 합니다.

9. **만들기**를 클릭합니다.

만들기를 클릭 하면 사설 클라우드 프로 비전이 시작 됩니다.  CloudSimple 포털의 [작업](https://docs.azure.cloudsimple.com/activity/#tasks) 페이지에서 진행률을 모니터링할 수 있습니다.  프로 비전은 30 분에서 2 시간까지 걸릴 수 있습니다.  프로 비전이 완료 되 면 전자 메일을 받게 됩니다.

## <a name="next-steps"></a>다음 단계

* [사설 클라우드 확장](expand-private-cloud.md)