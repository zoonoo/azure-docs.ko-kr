---
title: Azure VMware 솔루션 (AVS)-AVS 사설 클라우드에 대 한 vSAN 암호화 구성
description: Azure 가상 네트워크에서 실행 되는 키 관리 서버에서 AVS 사설 클라우드를 사용할 수 있도록 vSAN 소프트웨어 암호화 기능을 구성 하는 방법을 설명 합니다.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
---
# <a name="configure-vsan-encryption-for-avs-private-cloud"></a>AVS 사설 클라우드에 대 한 vSAN 암호화 구성

Azure 가상 네트워크에서 실행 되는 키 관리 서버에서 AVS 사설 클라우드를 사용할 수 있도록 vSAN 소프트웨어 암호화 기능을 구성할 수 있습니다.
...
* Azure 가상 네트워크에서 실행 되는 타사 KMS 도구를 사용 하 여 AVS 사설 클라우드에서 vSAN 암호화를 구성 하 고 관리 합니다.

## <a name="kms-deployment-scenario"></a>KMS 배포 시나리오

KMS 서버 클러스터는 Azure virtual network에서 실행 되며, 구성 된 Azure Express 경로 연결을 통해 AVS 사설 Cloud vCenter에서 IP에 연결할 수 있습니다.
...
2. [AVS 포털: Express 경로 피어 링 정보 가져오기](#avs-portal-obtain-expressroute-peering-information)
3. [Azure Portal: 가상 네트워크를 AVS 사설 클라우드에 연결](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
...
* AVS 사설 클라우드가 이미 만들어졌습니다.

### <a name="avs-portal-obtain-expressroute-peering-information"></a>AVS 포털: Express 경로 피어 링 정보 가져오기

설치를 계속 하려면 Express 경로에 대 한 인증 키 및 피어 회로 URI와 Azure 구독에 대 한 액세스 권한이 필요 합니다. 이 정보는 AVS 포털의 Virtual Network 연결 페이지에서 사용할 수 있습니다. 자세한 지침은 [AVS 사설 클라우드에 대 한 가상 네트워크 연결 설정](virtual-network-connection.md)을 참조 하세요. 정보를 얻는 데 문제가 있는 경우 [지원 요청](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest)을 여세요.
...
2. [포털을 사용 하 여 가상 네트워크를 express 경로 회로에 연결](../expressroute/expressroute-howto-linkvnet-portal-resource-manager.md)의 지침에 따라 가상 네트워크를 AVS express 회로에 연결 합니다.
3. AVS의 환영 전자 메일에서 받은 AVS Express 경로 회로 정보를 사용 하 여 가상 네트워크를 Azure의 AVS Express 경로 회로에 연결 합니다.
...
6. SSH 셸에서 메시지가 표시 되 면 `No`를 선택 하 여 노드를 초기 KeyControl 노드로 설정 합니다.
7. 이 절차의 3-5 단계를 반복 하 고 기존 클러스터에 추가 하 라는 메시지가 표시 되 면 `Yes`를 선택 하 여 추가 KeyControl 노드를 추가 합니다.
