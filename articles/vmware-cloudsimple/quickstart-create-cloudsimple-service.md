...
ms.locfileid: "69574559"
---
# <a name="quickstart---create-cloudsimple-service"></a>빠른 시작-CloudSimple service 만들기

시작 하려면 Azure Portal에서 CloudSimple로 Azure VMware 솔루션을 만듭니다.

CloudSimple 서비스를 사용 하면 CloudSimple로 Azure VMware 솔루션을 사용할 수 있습니다.  서비스를 만들면 노드를 구매 하 고, 노드를 예약 하 고, 사설 클라우드를 만들 수 있습니다.  CloudSimple 서비스를 사용할 수 있는 각 Azure 지역에 CloudSimple 서비스를 추가 합니다.  서비스는 CloudSimple로 Azure VMware 솔루션의에 지 네트워크를 정의 합니다.  이 edge 네트워크는 VPN, Express 경로 및 사설 클라우드에 대 한 인터넷 연결을 포함 하는 서비스에 사용 됩니다.
...
## <a name="enable-microsoftvmwarecloudsimple-resource-provider"></a>VMwareCloudSimple 리소스 공급자를 사용 하도록 설정

CloudSimple service에 대 한 리소스 공급자를 사용 하도록 설정 하려면 다음 단계를 수행 합니다.

1. **모든 서비스**를 선택합니다.
2. **구독**을 검색 하 고 선택 합니다.

    ![구독 선택](media/cloudsimple-service-select-subscriptions.png)

3. CloudSimple 서비스를 사용 하도록 설정 하려는 구독을 선택 합니다.
4. 구독에 대 한 **리소스 공급자** 를 클릭 합니다.
5. **VMwareCloudSimple** 를 사용 하 여 리소스 공급자를 필터링 합니다.
6. **VMwareCloudSimple** 리소스 공급자를 선택 하 고 **등록**을 클릭 합니다.
...
5. CloudSimple 노드를 구매 하려는 구독을 선택 합니다.
