---
title: Azure VMware 솔루션 (AVS)-Veeam을 사용 하 여 AVS 사설 클라우드의 워크 로드 가상 머신 백업
description: Veeam B & R 9.5을 사용 하 여 Azure 기반 AVS 사설 클라우드에서 실행 되는 가상 머신을 백업 하는 방법을 설명 합니다.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
---
# <a name="back-up-workload-vms-on-avs-private-cloud-using-veeam-br"></a>Veeam B & R을 사용 하 여 AVS 사설 클라우드에서 워크 로드 Vm 백업

이 가이드에서는 Veeam B & R 9.5을 사용 하 여 Azure 기반 AVS 사설 클라우드에서 실행 되는 가상 머신을 백업 하는 방법을 설명 합니다.
...
장기 백업 및 보관을 위해 Azure를 활용 하 여 백업 리포지토리 및 저장소 대상을 제공할 수 있습니다. AVS 사설 클라우드의 Vm과 Azure의 백업 리포지토리 간의 모든 백업 네트워크 트래픽은 높은 대역폭의 짧은 대기 시간 링크를 통해 이동 합니다. 지역 간 복제 트래픽이 내부 Azure 후면판 네트워크를 통해 이동 하 여 사용자의 대역폭 비용이 줄어듭니다.

**기본 배포**

백업할 때 30tb 미만인 환경의 경우에는 다음 구성을 권장 합니다.

* Veeam 백업 서버 및 프록시 서버를 AVS 사설 클라우드의 동일한 VM에 설치 합니다.
...
백업 하는 데 30tb를 초과 하는 환경에서는 다음과 같은 구성을 사용 하는 것이 좋습니다.

* Veeam에 권장 된 대로 vSAN 클러스터의 노드당 하나의 프록시 서버.
* 빠른 복원을 위해 5 일간의 데이터를 캐시 하는 AVS 사설 클라우드의 Windows 기반 주 백업 리포지토리입니다.
...
## <a name="requirements-for-veeam-solution-on-avs"></a>AVS의 Veeam 솔루션에 대 한 요구 사항

Veeam 솔루션을 사용 하려면 다음을 수행 해야 합니다.

* 사용자 고유의 Veeam 라이선스를 제공 합니다.
* Veeam을 배포 하 고 관리 하 여 AVS 사설 클라우드에서 실행 되는 워크 로드를 백업 합니다.
...
## <a name="install-and-configure-veeam-backups-in-your-avs-private-cloud"></a>AVS 사설 클라우드에 Veeam 백업 설치 및 구성

다음 섹션에서는 AVS 사설 클라우드에 대해 Veeam 백업 솔루션을 설치 하 고 구성 하는 방법을 설명 합니다.

배포 프로세스는 다음 단계로 구성 됩니다.

1. [vCenter UI: AVS 사설 클라우드에서 인프라 서비스 설정](#vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud)
2. [AVS 포털: Veeam에 대 한 AVS 사설 클라우드 네트워킹 설정](#avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam)
3. [AVS 포털: 권한 상승](#avs-private-cloud-escalate-privileges-for-cloudowner)
4. [Azure Portal: 가상 네트워크를 AVS 사설 클라우드에 연결](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
5. [Azure Portal: Azure에서 백업 리포지토리 만들기](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
6. [Azure Portal: 장기적인 데이터 보존을 위해 Azure blob storage 구성](#configure-azure-blob-storage-for-long-term-data-retention)
7. [AVS (AVS Private Cloud)의 vCenter UI: Veeam B & R 설치](#vcenter-console-of-avs-private-cloud-install-veeam-br)
8. [Veeam 콘솔: 복구 소프트웨어 & Veeam 백업 구성](#veeam-console-install-veeam-backup-and-recovery-software)
9. [AVS 포털: Veeam 액세스 및 에스컬레이션 권한 설정](#avs-portal-set-up-veeam-access-and-de-escalate-privileges)
...
* 구독의 Azure 가상 네트워크
* Azure Storage 계정
* AVS 포털을 사용 하 여 만든 [Avs 사설 클라우드](create-private-cloud.md) .  
...
* Veeam 9.5 u3 설치 가능 미디어 (ISO)를 AVS 사설 클라우드의 vSAN 데이터 저장소에 업로드

### <a name="vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud"></a>vCenter UI: AVS 사설 클라우드에서 인프라 서비스 설정

작업 및 도구를 쉽게 관리할 수 있도록 AVS 사설 클라우드에서 인프라 서비스를 구성 합니다.

* 다음 중 하나가 적용 되는 경우 [Active Directory 사용 하도록 vCenter id 원본 설정](set-vcenter-identity.md) 에 설명 된 대로 외부 id 공급자를 추가할 수 있습니다.
  * AVS 사설 클라우드의 온-프레미스 AD (Active Directory)에서 사용자를 식별 하려고 합니다.
  * 모든 사용자에 대해 AVS 사설 클라우드에서 AD를 설정 하려고 합니다.
...
### <a name="avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam"></a>AVS 사설 클라우드: Veeam에 대 한 AVS 사설 클라우드 네트워킹 설정

Veeam 솔루션에 대 한 AVS 사설 클라우드 네트워킹을 설정 하려면 AVS 포털에 액세스 합니다.
...
기본적으로 AVS는 1Gbps Express 경로 링크를 제공 합니다. 환경 크기가 큰 경우 더 높은 대역폭 연결이 필요할 수 있습니다. 더 높은 대역폭 연결에 대 한 자세한 내용은 Azure 지원에 문의 하세요.

설치를 계속 하려면 인증 키 및 피어 회로 URI와 Azure 구독에 대 한 액세스 권한이 필요 합니다. 이 정보는 AVS 포털의 Virtual Network 연결 페이지에서 사용할 수 있습니다. 지침은 [Azure virtual network에 대 한 피어 링 정보를 AVS 연결로 가져오기](virtual-network-connection.md)를 참조 하세요. 정보를 얻는 데 문제가 있는 경우 [지원 담당자에 게 문의 하세요](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest).

### <a name="avs-private-cloud-escalate-privileges-for-cloudowner"></a>AVS 사설 클라우드: **cloudowner** 에 대 한 권한 상승
...
### <a name="azure-portal-connect-your-virtual-network-to-the-avs-private-cloud"></a>Azure Portal: 가상 네트워크를 AVS 사설 클라우드에 연결

Express 경로를 [사용 하 여 Azure Virtual Network 연결](azure-expressroute-connection.md)의 지침에 따라 가상 네트워크를 AVS 사설 클라우드에 연결 합니다.
...
7. 서버를 무인 사설 클라우드에서 실행 중인 Veeam 백업 서버에 대 한 NFS 탑재 지점으로 제공 합니다. 지침은 [CentOS 6에서 NFS 탑재를 설정 하는 방법](https://www.digitalocean.com/community/tutorials/how-to-set-up-an-nfs-mount-on-centos-6)에 대 한 디지털 해양 문서를 참조 하세요. Veeam 백업 서버에서 백업 리포지토리를 구성할 때이 NFS 공유 이름을 사용 합니다.
...
### <a name="vcenter-console-of-avs-private-cloud-install-veeam-br"></a>AVS (AVS Private Cloud)의 vCenter 콘솔: Veeam B & R 설치

AVS 사설 클라우드에서 vCenter에 액세스 하 여 Veeam 서비스 계정을 만들고, Veeam B & R 9.5을 설치 하 고, 서비스 계정을 사용 하 여 Veeam을 구성 합니다.
...
1. VMware vSphere를 관리 되는 서버 환경으로 추가 합니다. 메시지가 표시 되 면 AVS 사설 클라우드의 VCenter 콘솔을 시작할 때 만든 Veeam 서비스 계정의 자격 증명을 제공 합니다 [. Veeam B & R을 설치](#vcenter-console-of-avs-private-cloud-install-veeam-br)합니다.
...
### <a name="avs-portal-set-up-veeam-access-and-de-escalate-privileges"></a>AVS 포털: Veeam 액세스 및 에스컬레이션 권한 설정
...
### <a name="avs-references"></a>AVS 참조
...
* [권한 상승](escalate-privileges.md)
* [방화벽 테이블 및 규칙 설정](firewall.md)
* [AVS 사설 클라우드 권한](learn-private-cloud-permissions.md)
