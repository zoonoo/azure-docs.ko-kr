...
5. AzCopy를 사용 하 여 Azure Blob 저장소에서 관리 디스크로 데이터를 다운로드 합니다.  명령 구문: `azcopy copy "https://<storage-account-name>.blob.core.windows.net/<container-name>/*" "<local-directory-path>/"`.  @No__t_0를 Azure storage 계정 이름으로 바꾸고 Data Box를 통해 복사 된 데이터를 보관 하는 컨테이너로 `<container-name>` 합니다.
...
8. @No__t_0 파일을 편집 하 여 NFS 공유에 액세스할 클라이언트 IP 주소에 대 한 사용 권한을 할당 합니다.
...
9. @No__t_0 명령을 사용 하 여 NFS 공유를 내보냅니다.

10. @No__t_0 명령을 사용 하 여 NFS 커널 서버를 다시 시작 합니다.
