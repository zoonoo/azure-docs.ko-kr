---
title: Azure VMware 솔루션 (AVS)의 Vlan 및 서브넷
description: AVS 사설 클라우드의 Vlan 및 서브넷에 대해 알아보기
author: sharaths-cs
ms.author: dikamath
ms.date: 08/15/2019
ms.topic: article
...
AVS는 AVS 서비스가 배포 되는 지역 당 네트워크를 제공 합니다. 네트워크는 기본적으로 라우팅이 활성화 된 단일 TCP 계층 3 주소 공간입니다. 이 지역에서 만든 모든 AVS 사설 클라우드 및 서브넷은 추가 구성 없이 서로 통신할 수 있습니다. Vlan을 사용 하 여 vCenter에서 분산 포트 그룹을 만들 수 있습니다.

![AVS 네트워크 토폴로지](media/cloudsimple-network-topology.png)

## <a name="vlans"></a>VLAN

각 AVS 사설 클라우드에 대해 VLAN (계층 2 네트워크)이 생성 됩니다. 계층 2 트래픽은 avs 사설 클라우드의 경계 내에서 유지 되므로, AVS 사설 클라우드 내에서 로컬 트래픽을 격리할 수 있습니다. AVS 사설 클라우드에서 만든 VLAN을 사용 하 여 해당 AVS 사설 클라우드에만 분산 포트 그룹을 만들 수 있습니다. Avs 사설 클라우드에서 만들어진 VLAN은 AVS 사설 클라우드의 호스트에 연결 된 모든 스위치에서 자동으로 구성 됩니다.
...
모든 서브넷은 기본적으로 서로 통신할 수 있으므로, AVS 사설 클라우드 간의 라우팅에 대 한 구성 오버 헤드를 줄일 수 있습니다. 동일한 지역에 있는 여러 Pc의 동-서 데이터는 동일한 계층 3 네트워크에 유지 되며 지역 내의 로컬 네트워크 인프라를 통해 전송 됩니다. 지역에서 AVS 사설 클라우드 간의 통신에는 송신이 필요 하지 않습니다. 이 접근 방식을 사용 하면 다른 AVS 사설 클라우드에 다른 워크 로드를 배포할 때 WAN/송신 성능 저하가 발생 하지 않습니다.

## <a name="vspherevsan-subnets-cidr-range"></a>vSphere/Vsphere 서브넷 CIDR 범위

AVS 사설 클라우드는 vCenter 서버에서 관리 하는 격리 된 VMware stack (ESXi 호스트, vCenter, vSAN 및 NSX) 환경으로 만들어집니다. 관리 구성 요소는 vSphere/Vsphere 서브넷 CIDR에 대해 선택 된 네트워크에 배포 됩니다. 네트워크 CIDR 범위는 배포 하는 동안 다른 서브넷으로 나뉩니다.
...
VSphere/Vsphere 서브넷 CIDR 범위 크기를 선택 하면 AVS 사설 클라우드의 크기에 영향을 줍니다. 다음 표에서는 vSphere/Vsphere 서브넷 CIDR의 크기에 따라 가질 수 있는 최대 노드 수를 보여 줍니다.
...
### <a name="management-subnets-created-on-an-avs-private-cloud"></a>AVS 사설 클라우드에서 만든 관리 서브넷

다음 관리 서브넷은 AVS 사설 클라우드를 만들 때 생성 됩니다.
