...
CloudSimple은 사용자 이름 `cloudowner@cloudsimple.local`를 사용 하 여 기본 vCenter 사용자 계정을 할당 합니다. 시작 하려면 추가 계정 설정이 필요 하지 않습니다.  CloudSimple은 일반적으로 일반적인 작업을 수행 하는 데 필요한 권한을 관리자에 게 할당 합니다.  온-프레미스 active directory 또는 Azure AD를 사설 클라우드의 [추가 id 원본](set-vcenter-identity.md) 으로 설정 합니다.
