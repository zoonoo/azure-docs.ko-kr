...
* VMware 클라우드 환경의 주문형 셀프 서비스 프로 비전. 요청 시 용량을 추가 하 고 제거 하는 기능입니다.
...
* VPN, 공용 IP 및 인터넷 게이트웨이를 포함 하는 네트워크 서비스에 대 한 경계를 입력 합니다. 이러한 서비스는 Azure에서 실행 되며 Azure의 보안 및 DDoS 보호를 수행 합니다.
* 비용을 절감 하기 위한 용량 예약.
* Azure 및 온-프레미스에 대 한 고속 및 대기 시간이 짧은 연결
* 고객이 통합 된 방식으로 Azure 서비스를 사용 하 고 공용 클라우드의이 고유한 "VMware 클라우드" 아키텍처를 활용 하는 솔루션 아키텍처입니다. Azure 서비스에는 Azure AD, 저장소, 응용 프로그램 게이트웨이 및 기타가 포함 됩니다.
* 사용자에 게 완전히 전담 되며 다른 고객의 인프라와 물리적으로 격리 된 인프라입니다.
...
* **운영 연속성**. CloudSimple은 VMware 플랫폼에 대 한 기본 액세스를 제공 합니다. CloudSimple 아키텍처는 기존 사용자와 호환 됩니다.
    ...
    * 규정 준수 도구
    * 프로세스
* 재 학습 **하지 않습니다**. VMware 플랫폼 호환성을 사용 하면 기존 기술과 지식을 사용할 수 있습니다.
...
* **새 하이브리드 플랫폼**. 이 서비스는 Azure의 나머지 부분에 대 한 고속 및 짧은 대기 시간 액세스를 가능 하 게 합니다. 또한 CloudSimple management를 사용 하면 동일한 UI 및 API를 사용 하 여 VMware 가상 머신 및 나머지 Azure에 대 한 통합 된 관리를 수행할 수 있습니다. 개발 팀은 일관 된 통합 방식으로 공용 및 개인 플랫폼을 활용할 수 있습니다.
...
* **요청 시 확장**합니다. 새 개발 환경 또는 계절 용량 버스트와 같은 예기치 않은 요구를 충족 하기 위해 클라우드로 확장 합니다. 필요할 때 새 용량을 쉽게 만들고 필요할 때만 유지할 수 있습니다. 온-프레미스와 클라우드 둘 다에서 동일한 아키텍처 및 정책을 사용 하 여 선불 투자를 줄이고 프로 비전 속도를 가속화 하 고 복잡성을 줄입니다.
* **Azure 클라우드의 재해 복구 및 가상 데스크톱**. Azure 클라우드에서 데이터, 앱 및 데스크톱에 대 한 원격 액세스를 설정 합니다. 고대역폭 연결을 사용 하는 경우 데이터를 신속 하 게 업로드 하 고 다운로드 하 여 인시던트를 복구 합니다. 대기 시간이 짧은 네트워크는 사용자가 데스크톱 앱에서 요구 하는 빠른 응답 시간을 제공 합니다. CloudSimple을 사용 하면 CloudSimple 포털 및 익숙한 VMware 도구를 사용 하 여 클라우드의 모든 정책과 네트워킹을 쉽게 복제할 수 있습니다. 복구 및 복제의 용이성은 DR 및 VDI 구현을 만들고 관리 하는 작업 및 위험을 크게 줄여 줍니다.
* **고성능 응용 프로그램 및 데이터베이스** CloudSimple은 가장 까다로운 VMware 워크 로드를 실행 하도록 설계 된 하이퍼 수렴 형 아키텍처를 제공 합니다. Oracle, Microsoft SQL server, 미들웨어 시스템 및 고성능의 SQL 데이터베이스를 실행 합니다. 온-프레미스, Azure의 VMware 및 Azure 개인 워크 로드에서 성능을 저하 시 키 지 않고 하이브리드 앱을 실행할 수 있도록 하는 고속 25gbps 네트워크 연결을 사용 하 여 클라우드를 고유한 데이터 센터로 경험해 보세요.
