---
title: CloudSimple의 VMware 솔루션에 대 한 노드 개요-Azure
...
ms.locfileid: "68812583"
---
# <a name="cloudsimple-nodes-overview"></a>CloudSimple 노드 개요

노드는 다음과 같습니다.

* VMware ESXi 하이퍼바이저가 설치 된 전용 완전 계산 호스트  
* 사설 클라우드를 만들기 위해 프로 비전 하거나 예약할 수 있는 컴퓨팅 단위  
* CloudSimple 서비스를 사용할 수 있는 지역에서 프로 비전 하거나 예약할 수 있습니다.

노드는 사설 클라우드의 구성 요소입니다.  사설 클라우드를 만들려면 동일한 SKU의 노드가 세 개 이상 필요 합니다.  사설 클라우드를 확장 하려면 노드를 더 추가 합니다.  기존 클러스터에 노드를 추가할 수 있습니다. 또는 Azure Portal에서 노드를 프로 비전 하 고 CloudSimple 서비스와 연결 하 여 새 클러스터를 만들 수 있습니다.  프로 비전 된 모든 노드는 CloudSimple 서비스 아래에 표시 됩니다.  CloudSimple 포털의 프로 비전 된 노드에서 사설 클라우드를 만듭니다.

## <a name="provisioned-nodes"></a>프로 비전 된 노드

프로 비전 된 노드는 종 량 제 용량을 제공 합니다. 노드를 프로 비전 하면 주문형 VMware 클러스터를 신속 하 게 확장할 수 있습니다. 필요에 따라 노드를 추가 하거나 프로 비전 된 노드를 삭제 하 여 VMware 클러스터를 확장할 수 있습니다. 프로 비전 된 노드는 매달 요금이 청구 되 고 프로 비전 되는 구독에 대 한 요금이 청구 됩니다.
...
프로 비전 또는 예약에 사용할 수 있는 형식 노드는 다음과 같습니다.
