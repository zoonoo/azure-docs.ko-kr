...
[가상 컴퓨터에 원격 디버깅 사용][가상 컴퓨터에 원격 디버깅 사용]

## <a name="cloudservice"></a>클라우드 서비스에 원격 디버깅 사용

1.  빌드 에이전트에서 [Azure에 대한 명령줄 빌드][Azure에 대한 명령줄 빌드]에 간략히 설명된 Azure에 대한 초기 환경을 설정합니다.
2.  패키지에 원격 디버그 런타임(msvsmon.exe)이 필요하므로, [Visual Studio 2013용 원격 도구][Visual Studio 2013용 원격 도구](또는 Visual Studio 2012를 사용할 경우 [Visual Studio 2012 업데이트 4용 원격 도구][Visual Studio 2012 업데이트 4용 원격 도구])를 설치합니다. 또는 Visual Studio를 설치한 시스템에서 원격 디버그 이진을 복사할 수 있습니다.
3.  [Azure용 서비스 인증서 만들기][Azure용 서비스 인증서 만들기]에 간략히 설명된 인증서를 만듭니다. .pfx 및 RDP 인증서 지문을 유지하고 대상 클라우드 서비스에 인증서를 업로드합니다.
...
1.  Azure 가상 컴퓨터를 만듭니다. [Windows Server를 실행하는 가상 컴퓨터 만들기][Windows Server를 실행하는 가상 컴퓨터 만들기] 또는 [Visual Studio에서 Azure 가상 컴퓨터 만들기][Visual Studio에서 Azure 가상 컴퓨터 만들기]를 참조하세요.
2.  [Azure 포털 페이지][Azure 포털 페이지]에서, 가상 컴퓨터 대시보드를 보고 가상 컴퓨터의 "RDP 인증서 지문"을 확인합니다. 이는 확장 구성에서 ServerThumbprint 값에 사용됩니다.
3.  [Azure용 서비스 인증서 만들기][Azure용 서비스 인증서 만들기]에 간략히 설명된 대로 클라이언트 인증서를 만듭니다(.pfx 및 RDP 인증서 지문 유지).
4.  Microsoft 다운로드 세터에서 [Azure Powershell][Azure Powershell](버전 0.7.4 이상)을 설치합니다.
5.  다음 스크립트를 실행하여 RemoteDebug 확장을 사용합니다. 개인 데이터를 사용자의 데이터(예: 구독 이름, 서비스 이름 및 지문)로 바꿉니다. 참고: 이 스크립트는 Visual Studio 2013용으로 구성된 것입니다. Visual Studio 2012를 사용하는 경우 ReferenceName 및 ExtensionName으로 "RemoteDebugVS2013"을 사용합니다.
