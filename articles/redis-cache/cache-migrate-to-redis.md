...
>[AZURE.NOTE] Azure Redis Cache를 사용하려면 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 체험](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=redis_cache_hero)을 참조하세요.
...
Visual Studio에서 개발된 .NET 응용 프로그램은 StackExchange.Redis 캐시 클라이언트를 사용하여 해당 캐시에 액세스할 수 있습니다. StackExchange.Redis NuGet 패키지를 사용하여 Visual Studio에서 클라이언트 응용 프로그램을 구성하려면 **솔루션 탐색기**에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **NuGet 패키지 관리**를 선택합니다.

![Azure Redis Cache 관리 NuGet 패키지](./media/cache-migrate-to-redis/IC729541.png)

**온라인 검색** 텍스트 상자에 **StackExchange.Redis**를 입력하여 결과에서 선택한 후 **설치**를 클릭합니다.

![Azure Redis Cache StackExchange.Redis NuGet 패키지](./media/cache-migrate-to-redis/IC746253.png)
