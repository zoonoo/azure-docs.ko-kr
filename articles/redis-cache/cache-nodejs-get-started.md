---
title: Node.js에서 Azure Redis Cache를 사용하는 방법에 대한 빠른 시작 | Microsoft Docs
description: 이 빠른 시작에서는 Node.js 및 node_redis와 함께 Azure Redis Cache를 사용하는 방법을 알아봅니다.
...
ms.locfileid: "38700785"
---
# <a name="quickstart-how-to-use-azure-redis-cache-with-nodejs"></a>빠른 시작: Node.js와 함께 Azure Redis Cache를 사용하는 방법
...
[node_redis](https://github.com/mranney/node_redis)의 최신 빌드는 SSL을 사용하여 Azure Redis Cache에 연결을 지원합니다. 다음 예제에서는 6380 SSL 끝점을 사용하여 Azure Redis Cache에 연결하는 방법을 보여줍니다. 
...
다음 예제 JavaScript를 파일에 추가합니다. 이 코드는 캐시 호스트 이름 및 키 환경 변수를 사용하여 Azure Redis Cache 인스턴스에 연결하는 방법을 보여줍니다. 코드는 또한 캐시에 문자열 값을 저장하고 검색합니다. 또한 `PING` 및 `CLIENT LIST` 명령을 실행했습니다. [node_redis](https://github.com/mranney/node_redis) 클라이언트가 포함된 Redis 사용에 관한 더 많은 예는 [http://redis.js.org/](http://redis.js.org/)를 참조하세요.
...
이 빠른 시작에서는 Node.js 응용 프로그램에서 Azure Redis Cache를 사용하는 방법을 알아보았습니다. ASP.NET 웹앱에서 Redist Cache를 사용하려면 다음 빠른 시작으로 계속 진행하세요.
