---
title: Java와 함께 Azure Redis Cache를 사용하는 방법에 대한 빠른 시작 | Microsoft Docs
description: 이 빠른 시작에서는 Azure Redis Cache를 사용하는 새 Java 앱을 만듭니다.
...
ms.locfileid: "44055280"
---
# <a name="quickstart-how-to-use-azure-redis-cache-with-java"></a>빠른 시작: Java와 함께 Azure Redis Cache를 사용하는 방법


Azure Redis Cache는 Microsoft에서 관리하는 전용 Redis Cache에 액세스할 수 있게 합니다. Microsoft Azure 내의 모든 응용 프로그램에서 캐시에 액세스할 수 있습니다.

이 문서에서는 Java용 [Jedis](https://github.com/xetorthio/jedis) Redis Cache 클라이언트를 사용하여 Azure Redis Cache를 시작하는 방법을 보여줍니다.
...
이 코드는 캐시 호스트 이름 및 키 환경 변수를 사용하여 Azure Redis Cache 인스턴스에 연결하는 방법을 보여줍니다. 코드는 또한 캐시에 문자열 값을 저장하고 검색합니다. 또한 `PING` 및 `CLIENT LIST` 명령을 실행했습니다. 
...
이 빠른 시작에서는 Java 응용 프로그램에서 Azure Redis Cache를 사용하는 방법을 알아보았습니다. ASP.NET 웹앱에서 Redist Cache를 사용하려면 다음 빠른 시작으로 계속 진행하세요.
