...
-	ARM 템플릿을 사용하여 응용 프로그램에 대한 Azure 리소스를 프로비전하는 방법.
...
### .NET용 Azure SDK 포함 Visual Studio 2015
...
3. `Team.cs` 파일의 맨 위에 있는 `using` 문을 다음과 같은 Using 문으로 바꿉니다.
...
3.  다음 연결 문자열을 `connectionStrings` 섹션에 추가합니다. 연결 문자열의 이름은 `TeamContext`인 Entity Framework 데이터베이스 컨텍스트 클래스의 이름과 일치해야 합니다.
...
5. **모델 클래스** 드롭다운 목록에서 **팀(ContosoTeamStats.Models)**을 선택합니다. **데이터 컨텍스트 클래스** 드롭다운 목록에서 **TeamContext(ContosoTeamStats.Models)**를 선택합니다. **컨트롤러** 이름 텍스트 상자에 `TeamsController`를 입력합니다(자동으로 채워지지 않은 경우). **추가**를 클릭하여 컨트롤러 클래스를 만들고 기본 보기를 추가합니다.

    ![컨트롤러 구성][cache-configure-controller]

4. **솔루션 탐색기** **Global.asax**를 확장하고 **Global.asax.cs**를 두 번 클릭하여 엽니다.

    ![Global.asax.cs][cache-global-asax]

5. 파일의 맨 위에 있는 다음 Using 문 두 개를 다른 Using 문 아래에 추가합니다.
...
6. 다음 코드 줄을 `Application_Start` 메서드의 끝에 추가합니다.


	    Database.SetInitializer<TeamContext>(new TeamInitializer());


7. **솔루션 탐색기**에서 `App_Start`를 확장하고 `RouteConfig.cs`를 두 번 클릭합니다.

    ![RouteConfig.cs][cache-RouteConfig-cs]

8. 다음 예제와 같이 `RegisterRoutes` 메서드의 다음 코드에 있는 `controller = "Home"`을 `controller = "Teams"`로 바꿉니다.
...
2. 검색 텍스트 상자에 **StackExchange.Redis**를 입력하고 결과에서 원하는 버전을 선택한 다음 **설치**를 클릭합니다.
    ...
    NuGet 패키지는 클라이언트 응용 프로그램이 StackExchange.Redis 캐시 클라이언트를 사용하여 Azure Redis Cache에 액세스하는 데 필요한 어셈블리 참조를 다운로드하고 추가합니다. 강력한 이름의 **StackExchange.Redis** 클라이언트 라이브러리 버전을 사용하려면 **StackExchange.Redis.StrongName**을 선택하고 그렇지 않으면 **StackExchange.Redis**를 선택합니다.

3. **솔루션 탐색기**에서 **Controllers** 폴더를 확장하고 **TeamsController.cs**를 두 번 클릭하여 엽니다.
...
1. 컴퓨터에 `WebAppPlusCacheAppSecrets.config`라는 파일을 만들고 이 파일을 다른 위치에서 체크인하기로 결정한 경우 샘플 응용 프로그램의 소스 코드에서 체크인하지 않을 위치에 배치합니다. 이 예제에서 `AppSettingsSecrets.config` 파일은 `C:\AppSecrets\WebAppPlusCacheAppSecrets.config`에 위치합니다.
...
### 캐시 또는 데이터베이스에서 결과를 반환하도록 TeamsController 클래스 업데이트
...
### 캐시로 작업하도록 Create, Edit 및 Delete 메서드 업데이트
...
### 캐시로 작업하도록 팀 인덱스 보기 업데이트

1. **솔루션 탐색기**에서 **Views** 폴더, **Teams** 폴더를 차례로 확장한 다음 **Index.cshtml**을 두 번 클릭합니다.
...
3. **Index.cshtml** 파일의 아래쪽으로 스크롤하고 `tr` 요소를 파일의 마지막 테이블에서 마지막 행이 되도록 추가합니다.
...
[![Azure에 배포][deploybutton]](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2F201-web-app-redis-cache-sql-database%2Fazuredeploy.json)로 바꿉니다.
...
![Azure에 배포][cache-deploy-to-azure-step-1]
...
![배포 시작됨][cache-deployment-started]
...
3. Azure 리소스를 만들 때 사용한 구독을 선택하고 리소스가 포함된 리소스 그룹을 확장하고 원하는 웹앱을 선택하고 **확인**을 클릭합니다. **Azure에 배포** 단추를 사용한 경우 웹앱 이름은 **webSite**로 시작하고 그 다음에 일부 추가 문자를 덧붙입니다.
...
## 응용 프로그램을 마쳤으면 리소스를 삭제합니다.

샘플 자습서 응용 프로그램을 마쳤을 때 비용 및 리소스를 절감하기 위해 사용한 Azure 리소스를 삭제할 수 있습니다. [Azure 리소스](#provision-the-azure-resources) 섹션의 **Azure에 배포** 단추를 사용했고 모든 리소스를 같은 리소스 그룹에 포함시킨 경우 리소스 그룹을 삭제하여 한 번의 작업으로 이들을 모두 삭제할 수 있습니다.
...
사용할 캐시를 선택하거나 만든 후 Azure 포털에서 캐시를 찾아가고 캐시에 대해 [호스트 이름](cache-configure.md#properties) 및 [액세스 키](cache-configure.md#access-keys)를 검색합니다. 지침은 [Redis Cache 설정 구성](cache-configure.md#configure-redis-cache-settings)을 참조하세요.
