...
1. Azure Redis Cache와 클라이언트 응용 프로그램이 Azure에서 동일한 지역에 있는지 확인합니다. 예를 들어 캐시는 미국 동부에 있는 반면에 클라이언트는 미국 서부에 있고 요청이 `synctimeout` 간격 내에 완료되지 않을 때 시간 초과가 일어날 수 있거나 또는 로컬 개발 컴퓨터에서 디버깅을 할 때 시관 초과가 일어날 수 있습니다.
