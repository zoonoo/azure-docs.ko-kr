...
>[AZURE.IMPORTANT] 클라이언트 연결이 논리 오류 또는 클라이언트 코드의 버그 때문에 모두 소비되는 경우 Redis 노드가 다시 온라인 상태가 되면 StackExchange.Redis가 자동으로 다시 연결됩니다. 기본 문제가 해결되지 않으면 클라이언트 연결은 계속 소비됩니다.
