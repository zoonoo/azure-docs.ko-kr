---
title: .NET Core 앱에서 Azure Redis Cache를 사용하는 방법에 대한 빠른 시작 | Microsoft Docs
description: 이 빠른 시작에서는 .NET Core 앱에서 Azure Redis Cache에 액세스하는 방법을 알아봅니다.
...
다음 멤버를 *Program.cs*의 `Program` 클래스에 추가합니다. 다음 코드에서는 구성을 초기화하여 Redis Cache 연결 문자열에 대한 사용자 비밀에 액세스합니다.
...
Azure Redis Cache 연결은 `ConnectionMultiplexer` 클래스로 관리됩니다. 이 클래스는 클라이언트 응용 프로그램 전체에서 공유하고 다시 사용해야 합니다. 각 작업에 대해 새 연결을 만들지 마세요. 
...
Azure Redis Cache에는 Redis 캐시 내에서 데이터를 논리적으로 구분하는 데 사용할 수 있는 여러 개의 구성 가능한 데이터베이스(기본 16개)가 있습니다. 이 코드에서는 기본 데이터베이스인 DB 0에 연결합니다. 자세한 내용은 [Redis 데이터베이스란?](cache-faq.md#what-are-redis-databases) 및 [기본 Redis 서버 구성](cache-configure.md#default-redis-server-configuration)을 참조하세요.
