...
* ASP.NET에서 web.config의 `<processModel>` 구성 요소에서 ["minIoThreads" 구성 설정]["minIoThreads" 구성 설정]을 사용합니다. Azure 웹 사이트 내에서 실행하는 경우 이 설정은 구성 옵션을 통해 노출되지 않습니다. 그러나 global.asax.cs의 Application\_Start 메서드에서 프로그래밍 방식(아래 참조)으로 설정할 수 있어야 합니다.
