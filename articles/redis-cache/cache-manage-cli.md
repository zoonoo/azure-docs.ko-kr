...
-	다음 명령 중 하나를 실행하기 전에 `azure config mode arm` 명령을 실행하여 Azure CLI를 리소스 관리자 모드로 전환합니다. 자세한 내용은 [Azure 리소스 관리자 모드 설정](../xplat-cli-azure-resource-manager.md#setting-the-azure-resource-manager-mode)을 참조하세요.
