...
다음 예제는 Azure에 호스트된 웹 페이지에서 Twilio를 사용하여 전화를 거는 방법을 보여 줍니다. 다음 스크린샷과 같이 결과 응용 프로그램에서 사용자에게 전화 통화 값을 묻는 메시지가 표시됩니다.

![Twilio 및 Java를 사용하는 Azure 통화 양식][twilio_java]

이 항목에서 코드를 사용하려면 다음을 수행해야 합니다.

1. Twilio 계정 및 인증 토큰을 받습니다. Twilio를 시작하려면 에서 [https://www.twilio.com/pricing][twilio_pricing]가격을 평가합니다. 에서 [https://www.twilio.com/try-twilio][try_twilio]등록할 수 있습니다. Twilio에서 제공하는 API에 대한 [https://www.twilio.com/api][twilio_api]자세한 내용은 을 참조하십시오.
2. Twilio JAR를 가져옵니다. 에서 [https://github.com/twilio/twilio-java][twilio_java_github]GitHub 소스를 다운로드하고 사용자 고유의 JAR을 만들거나 종속성 유무에 관계없이 미리 빌드된 JAR을 다운로드할 수 있습니다.
...
전화 걸기와 함께, makecall.jsp는 Twilio 엔드포인트, API 버전 및 통화 상태를 표시합니다. 다음 스크린샷을 예로 들 수 있습니다.
...
* makecall.jsp에서 값을 하드 코딩하는 대신, **RoleEnvironment.getConfigurationSettings** 를 사용하여 배포 구성 설정에서 Twilio 계정 ID 및 인증 토큰을 검색할 수 있습니다. **RoleEnvironment** 클래스에 대한 자세한 내용은 [JSP의 Azure 서비스 런타임 라이브러리 사용을][azure_runtime_jsp]참조하십시오.
* makecall.jsp 코드는 Url **변수에** Twilio가 제공한 URL을 [https://twimlets.com/message][twimlet_message_url]할당합니다. 이 URL은 Twilio에 통화를 진행하는 방법을 알리는 TwiML(Twilio Markup Language) 응답을 제공합니다. 예를 들어 반환되는 TwiML에는 호출 ** &lt;&gt; ** 받는 사람에게 텍스트가 음성으로 전달되는 Say 동사가 포함될 수 있습니다. Twilio 제공 URL을 사용하는 대신, 고유한 서비스를 빌드하여 Twilio의 요청에 응답할 수 있습니다. 자세한 내용은 [Java에서 음성 및 SMS 기능을 위해 Twilio를 사용하는 방법][howto_twilio_voice_sms_java]을 참조하세요. TwiML에 대한 자세한 내용은 [https://www.twilio.com/docs/api/twiml][twiml]에서 찾을 수 있으며 ** &lt;Say&gt; ** 및 기타 Twilio 동사에 대한 자세한 내용은 에서 [https://www.twilio.com/docs/api/twiml/say][twilio_say]찾을 수 있습니다.
* 에서 Twilio 보안 [https://www.twilio.com/docs/security][twilio_docs_security]지침을 읽어보십시오.

Twilio에 대한 자세한 [https://www.twilio.com/docs][twilio_docs]내용은 을 참조하십시오.
