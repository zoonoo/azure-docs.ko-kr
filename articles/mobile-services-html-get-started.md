...
<p>이 자습서에서는 Azure 모바일 서비스를 사용하는 HTML 앱에 클라우드 기반 백 엔드 서비스를 추가하는 방법에 대해 설명합니다. 이 자습서에서는 새 모바일 서비스와 새 모바일 서비스에 앱 데이터를 저장하는 간단한 <em>To do list</em> 앱을 둘 다 만듭니다. 오른쪽에 있는 클립을 클릭하여 이 자습서의 동영상 버전을 볼 수 있습니다.</p>
</div>
...
2. 퀵 스타트 탭에서 **플랫폼 선택** 아래의 **Windows**를 클릭하고 **새 HTML 응용 프로그램 만들기**를 확장합니다.
...
4. Under **앱 다운로드 및 실행** 아래에서 **다운로드**를 클릭합니다. 

  	모바일 서비스와 연결되는 샘플 _할 일 모음_ 응용 프로그램용 웹 사이트 파일이 다운로드됩니다. 압축 파일을 로컬 컴퓨터에 저장하고 저장 위치를 적어둡니다.

5. **구성** 탭의 **크로스-원본 자원 공유(CORS)** 아래에 있는 **호스트 이름의 요청 허용** 목록에 `localhost`가 이미 나열되어 있는지 확인합니다. 그렇지 않으면 **호스트 이름** 필드에 `localhost`를 입력한 후 **저장**을 클릭합니다.

  	![][9]

	<div class="dev-callout"><b>참고</b>
		<p>localhost 이외의 웹 서버에 퀵 스타트 앱을 배포하는 경우, 웹 서버 호스트 이름을 <strong>호스트 이름의 요청 허용</strong> 목록에 추가해야 합니다. 자세한 내용은 <a href="http://msdn.microsoft.com/ko-kr/library/windowsazure/dn155871.aspx" target="_blank">크로스-원본 자원 공유</a>를 참조하세요.</p>
	</div>
...
1. 압축된 프로젝트 파일을 저장한 위치로 이동하고, 컴퓨터에 파일의 압축을 풀고, **server** 하위 폴더에서 다음 명령 파일 중 하나를 실행합니다.
...
3. 앱에서 **Enter new task**(새 작업 입력)에 _자습서 완료_과 같은 의미 있는 텍스트를 입력하고 **추가**를 클릭합니다.
