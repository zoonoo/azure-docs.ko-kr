...
Docker 지원을 추가할 때, `.UseUrls(Environment.GetEnvironmentVariable("ASPNETCORE_URLS"))`를 WebHostBuilder()에 추가해야 합니다. `Main()` 함수나 새 클래스 WebHostBuilder를 `Program.cs`에서 찾을 수 없는 경우, 경고가 표시됩니다. docker 컨테이너 내에서 실행 시 localhost 너머에서 들어오는 트래픽을 Kestrel이 수신하도록 설정하는데 `.UseUrls()`가 필요합니다. 완료 시 일반적인 코드는 다음과 같이 표시됩니다.
...
`/c/Users/Public` 디렉터리의 내용을 보려면 웜홀 디렉터리로 변경합니다.
...
> [AZURE.NOTE] Linux VM에서 작업할 때, 컨테이너 파일 시스템은 대/소문자를 구분합니다.

내용을 볼 수 없는 경우 다음을 시도합니다.

**윈도우용 Docke 베타**
- 시스템 트레이에 `moby` 아이콘이 있는지 살피고, 아이콘이 흰색이고 기능 중인지 확인하여 Windows용 Docker 데스크톱 앱이 실행 중인지 확인합니다.
- 시스템 트레이에 있는 `moby` 아이콘을 마우스 오른쪽 단추로 클릭하여, 설정을 선택하고 **공유 드라이브 관리...**를 클릭하여 볼륨 매핑이 구성되어 있는지 확인합니다.
...
Microsoft Edge 브라우저를 사용하는 경우 Edge에서 IP 주소를 보안되지 않은 상태로 간주하므로 사이트가 열리지 않을 수 있습니다. 이 문제를 해결하려면 다음 단계를 수행합니다.
...
브라우저 열기 실패는 `docker-compose-up` 중 오류일 수 있습니다. 오류를 확인하려면 다음 단계를 수행합니다.
...
1. `-noexit` 매개 변수를 추가하면 이제 줄이 다음과 유사합니다. 이 코드는 PowerShell을 계속 열려 있게 하여 오류를 확인할 수 있습니다.
