...
**데이터 분리:** 고객 데이터는 Operational Insights 서비스 전체에서 각 구성 요소에 논리적으로 별도로 유지됩니다. 모든 데이터에는 조직별로 태그가 지정됩니다. 이 태그는 데이터 수명 주기 동안 유지되며 서비스의 각 계층에서 적용됩니다.
...
## 데이터 흐름 보안
다음 다이어그램에서는 회사의 정보 흐름 및 궁극적으로 Operational Insights에서 볼 수 있도록 Operational Insights로 이동할 때 보안이 유지되는 방식을 보여 줍니다. 각 단계에 대한 자세한 내용은 다이어그램 뒤에 나옵니다.
...
Operational Insights 계정은 데이터를 수집, 집계, 분석 및 제공하는 데 사용됩니다. Operational Insights 계정은 주로 데이터를 분할하는 데 사용되며, 각 Operational Insights 계정은 고유합니다. 예를 들어 하나의 Operational Insights 계정을 사용하여 프로덕션 데이터를 관리하고 다른 계정을 사용하여 테스트 데이터를 관리할 수 있습니다. 관리자는 계정을 사용하여 데이터에 대한 사용자 액세스를 제어할 수 있습니다. 각 Operational Insights 계정에는 여러 사용자 계정이 연결될 수 있으며, 각 사용자 계정에는 여러 Operational Insights 계정이 있을 수 있습니다.

구성 마법사가 완료되면 각 Operations Manager 관리 그룹에서 Operational Insights 서비스와의 연결을 설정합니다. 그런 다음 컴퓨터 추가 마법사를 사용하여 관리 그룹에서 서비스로 데이터를 보낼 수 있는 컴퓨터를 선택합니다.

두 유형의 에이전트 모두 Operational Insights에 대한 데이터를 수집합니다. 수집되는 데이터 형식은 사용하는 솔루션 유형에 따라 다릅니다. 솔루션은 미리 정의된 보기, 로그 검색 쿼리, 데이터 수집 규칙 및 처리 논리의 모음입니다. Operational Insights 관리자만 Operational Insights를 사용하여 솔루션을 가져올 수 있습니다. 가져온 솔루션은 Operations Manager 관리 서버(사용하는 경우)로 이동한 후 선택한 Operations Manager 에이전트로 이동합니다. 그런 다음 에이전트에서 데이터를 수집합니다.

다음 표에는 Operational Insights에서 사용할 수 있는 솔루션 및 각 솔루션이 수집하는 데이터 형식이 나와 있습니다.
...
<table border="1" cellspacing="4" cellpadding="4">
    ...
    <tr align="left" valign="top">
		<td>구성 평가</td>
		<td>구성 데이터, 메타데이터 및 상태 데이터</td>
    </tr>
    <tr align="left" valign="top">
		<td>용량 계획</td>
		<td>성능 데이터, 메타데이터 및 상태 데이터</td>
    </tr>
	<tr align="left" valign="top">
		<td>맬웨어 방지</td>
		<td>구성 데이터, 메타데이터 및 상태 데이터</td>
    ...
    <tr align="left" valign="top">
		<td>변경 내용 추적</td>
		<td>소프트웨어 인벤토리 및 Windows 서비스 메타데이터</td>
    </tr>
    <tr align="left" valign="top">
		<td>SQL 및 Active Directory 평가</td>
		<td>WMI 데이터, 레지스트리 데이터, 성능 데이터 및 SQL Server 동적 관리 보기 결과</td>
    </tr>
    </tbody>
    </table>
...
<table border="1" cellspacing="4" cellpadding="4">
	...
	<tr align="left" valign="top">
		<td>이벤트</td>
		<td>EventId, EventOriginalID, BaseManagedEntityInternalId, RuleId, PublisherId, PublisherName, FullNumber, Number, Category, ChannelLevel, LoggingComputer, EventData, EventParameters, TimeGenerated, TimeAdded<p><b>참고:</b> 사용자 지정 필드가 있는 이벤트를 Windows 이벤트 로그에 기록하면 Operational Insights에서 수집합니다. </td>
    </tr>
	    <tr align="left" valign="top">
		<td>Metadata</td>
