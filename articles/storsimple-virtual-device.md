...
- 가상 장치의 경우, [Azure에서 가상 네트워크를 구성합니다](https://msdn.microsoft.com/library/azure/jj156074.aspx). 
- 자신의 DNS 서버 이름을 지정하는 대신 Azure에서 제공되는 기본 DNS 서버를 사용할 수 있습니다. 
...
- [Azure 가상 컴퓨터](https://msdn.microsoft.com/library/azure/jj156003.aspx)(호스트 서버)를 가상 네트워크에서 만듭니다. 이 서버는 다음 요구 사항을 충족해야 합니다. 							
...
- 가상 장치에 대해 호스트 서버가 될 VM에 대해 [액세스 제어 레코드](https://msdn.microsoft.com/library/1747f56e-858a-4cfe-a020-949d7db23b8b#sec02)를 추가합니다.

- 가상 장치와 동일한 지역에 [저장소 계정](https://msdn.microsoft.com/library/1747f56e-858a-4cfe-a020-949d7db23b8b#sec01)이 있는지 확인합니다. 다른 영역의 저장소 계정으로 성능이 저하될 수 있습니다.
...
이러한 절차를 수행하기 전에 [가상 장치에 대한 필수 구성 요소](https://msdn.microsoft.com/library/dn772572.aspx)를 충족하는지 확인합니다.

이 절차를 완료하면 [가상 장치를 작업할](https://msdn.microsoft.com/library/dn772527.aspx) 준비가 된 것입니다.
...
- 가상 장치에 저장된 데이터를 Microsoft 데이터 센터에서 호스팅함을 이해했음을 표시하려면 확인 표시를 클릭합니다. 물리적 장치만 사용하는 경우, 암호화 키는 사용자 장치와 함께 유지되므로 Microsoft는 해독할 수 없습니다. 가상 장치를 사용하면 Microsoft Azure에 암호화 키와 암호 해독 키 모두가 저장됩니다. 자세한 내용은 [가상 장치를 사용하기 위한 보안 고려 사항](https://msdn.microsoft.com/library/dn772561.aspx)을 참조하세요.
...
1. **StorSimple 가상 장치**를 사용자 장치로 선택하고 두 번 클릭하여 빠른 시작에 액세스합니다.

- **장치 설치 완료**를 클릭합니다. 그러면 장치 구성 마법사가 시작됩니다.

- **서비스 데이터 암호화 키**를 제공한 공간에 입력합니다.

- 가상 장치의 초기 구성 및 등록을 완료하려면 확인 표시를 클릭합니다. 스냅숏 관리자 암호 및 장치 관리자 암호는 기본값으로 미리 구성되며 장치가 등록된 후 변경해야 합니다.

### 장치 구성 설정 수정

다음 섹션에서는 StorSimple 가상 장치에 대한 구성에 필요한 장치 구성 설정을 설명합니다.
...
다음 단계를 수행하여 StorSimple 가상 장치를 사용하는 경우 StorSimple 스냅숏 관리자를 구성합니다.
...
다음 단계에 따라 StorSimple 가상 장치에 대한 장치 관리자 암호를 구성합니다.
...
- [볼륨 컨테이너](https://msdn.microsoft.com/library/dn757817.aspx)

- [볼륨](https://msdn.microsoft.com/library/dn772417.aspx)

- [백업 정책](https://msdn.microsoft.com/library/dn772382.aspx)
...
- **자동 업데이트** – 자동 업데이트를 켜거나 끌 수 있습니다. 자동 업데이트를 설정하면 가상 장치가 자동으로 종료되고 다시 시작되어(필요한 경우) 출시된 업데이트를 적용합니다. 가상 장치에는 단일 인터페이스만 있기 때문에 업데이트 적용 시 약간의 서비스가 중단됨을 의미합니다.
...
[Windows PowerShell을 사용하여 원격으로 연결](https://msdn.microsoft.com/library/dn772393.aspx)의 절차에 따라 가상 장치에 대한 원격 서비스를 설정해야 합니다.
...
DR(재해 복구)는 StorSimple 가상 장치가 설계된 주요 시나리오 중 하나입니다. 이 시나리오에서는 물리적 StorSimple 장치 또는 전체 데이터 센터를 사용하지 못할 수 있습니다. 다행스럽게도 가상 장치를 사용하여 다른 위치에서 작업을 복원할 수 있습니다. DR 중, 원본 장치의 볼륨 컨테이너는 소유권을 변경하고 가상 장치로 전송됩니다. DR에 대한 필수 구성 요소는 가상 장치가 작성되고 구성되었는지, 볼륨 컨테이너 내의 모든 볼륨을 오프라인으로 전환했는지 및 볼륨 컨테이너에 연결된 클라우드 스냅숏이 있는지입니다.
...
가상 장치를 삭제하거나 종료하는 경우 StorSimple Manager 서비스의 장치 페이지에 **오프라인**으로 표시됩니다. 가상 장치에서 만든 백업도 삭제하려는 경우, 장치에 따라 삭제하거나 비활성화하도록 선택할 수 있습니다. 자세한 내용은 [비활성화](https://msdn.microsoft.com/library/33b7811b-36ba-4609-b165-0796ad456435#BKMK_acis_deactivate)를 참조하세요.
...
- **삭제**를 클릭하고 모든 가상 컴퓨터 디스크를 삭제합니다. <!--HONumber=52-->
