...
-   [Azure 모바일 서비스 SDK 버전 1.3.0-alpha3(이상)][Azure 모바일 서비스 SDK 버전 1.3.0-alpha3(이상)]
-   [Azure 모바일 서비스 SQLite 저장소 버전 1.0.0-alpha2(이상)][Azure 모바일 서비스 SQLite 저장소 버전 1.0.0-alpha2(이상)]

> [WACOM.NOTE] 아래의 지침에서는 Xamarin 확장이 포함된 Visual Studio 2012 이상을 사용 중이라고 가정합니다. OS X에서 Xamarin Studio를 사용할 경우 대부분의 지침이 동일하지만 시험판 모바일 서비스 NeGet 패키지를 프로젝트에 쉽게 추가할 수 있도록 [Xamarin용 NuGet 추가 기능][Xamarin용 NuGet 추가 기능]을 설치해야 합니다.
...
2.  `todoTable` 멤버 유형을 `IMobileServiceTable`에서 `IMobileServicesSyncTable`로 변경
...
4.  `QSTodoService`에 대한 생성자에서 두 번째 코드 행으로 `SQLitePCL.CurrentPlatform.Init()`에 대한 호출을 추가합니다.
...
사용자가 새로 고침 제스처를 수행할 때 새로운 `SyncAsync` 메서드를 호출하도록 `QSTodoListViewController`를 수정합니다.

1.  `todoService` 초기화 다음에 `ViewDidLoad()`에서 `InitializeStoreAsync`에 대한 호출을 추가합니다.
...
2.  `RefreshAsync` 호출 전에 `SyncAsync`를 호출하도록 `AddRefreshControl` 메서드를 수정합니다.
