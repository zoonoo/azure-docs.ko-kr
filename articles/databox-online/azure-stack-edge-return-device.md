---
title: Azure Stack Edge 장치 반환 또는 교체 | Microsoft Docs
description: Azure Stack Edge 장치를 반환 하거나 교체 하는 방법을 설명 합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "82569356"
---
# <a name="return-or-replace-your-azure-stack-edge-device"></a>Azure Stack Edge 장치 반환 또는 교체

이 문서에서는 데이터를 초기화 하 고 Azure Stack Edge 장치를 반환 하는 방법을 설명 합니다. 장치를 반환한 후에는 장치와 연결 된 리소스를 삭제 하거나 교체 장치를 주문할 수도 있습니다.
...
3. 확인 메시지가 표시 되 면 경고를 검토 하 고 **예** 를 선택 하 여 계속 합니다.
    ...
    ![다시 설정 확인](media/azure-stack-edge-return-device/device-reset-2.png)  

다시 설정 하면 장치 데이터 디스크에서 데이터가 지워집니다. 이 프로세스는 장치에 있는 데이터의 양에 따라 약 30-40 분이 소요 됩니다.

또는 장치의 PowerShell 인터페이스에 연결 하 고 `Reset-HcsAppliance` cmdlet을 사용 하 여 데이터 디스크에서 데이터를 지웁니다. 자세한 내용은 [장치 다시 설정](azure-stack-edge-connect-powershell-interface.md#reset-your-device)을 참조 하세요.
...
1. 장치를 반환할지를 나타내는 Microsoft 지원 지원 티켓을 엽니다. 문제 유형을 Azure Stack에 **지 하드웨어**로 선택 합니다.
...
6. 해당 지역 운송업체에서 픽업하도록 예약합니다. 장치를 미국에서 반환 하는 경우에는 캐리어가 UPS 또는 FedEx 수 있습니다. UPS를 사용 하 여 픽업을 예약 하려면:
    ...
    픽업을 예약 하는 대신 가장 가까운 드롭다운 위치에 Azure Stack 가장자리를 놓을 수도 있습니다.
...
2. 교체 장치에 대 한 [새 리소스를 만듭니다](azure-stack-edge-deploy-prep.md#create-a-new-resource) . **Azure Stack Edge 장치**에 대 한 확인란을 선택 했는지 확인 합니다. 
3. 교체 장치를 받은 후에는 새 리소스에 대해 교체 장치를 [설치](azure-stack-edge-deploy-install.md) 하 고 [활성화](azure-stack-edge-deploy-connect-setup-activate.md) 합니다.
