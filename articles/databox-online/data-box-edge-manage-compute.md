...
이벤트는 작업을 수행하려는 클라우드 환경 또는 디바이스에서 발생하는 상황입니다. 예를 들어, 공유에서 파일이 만들어지는 것도 이벤트에 해당합니다. 트리거는 해당 이벤트에 대한 응답입니다. 트리거는 이벤트가 발생할 때마다 함수가 실행되도록 할 수 있습니다. Data Box Edge의 경우 트리거는 파일 이벤트 또는 일정에 대한 응답일 수 있습니다.
