---
title: Azure Portal에서 Azure Stack Edge Pro R 디바이스에 대한 시간, 디바이스, 업데이트 설정을 설정하는 자습서 | Microsoft Docs
description: Azure Stack Edge Pro R을 배포하기 위한 자습서에서는 물리적 디바이스에 대한 디바이스, 업데이트, 시간 설정을 설정하는 방법을 안내합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "106059618"
---
# <a name="tutorial-configure-the-device-settings-for-azure-stack-edge-pro-r"></a>자습서: Azure Stack Edge Pro R에 대한 디바이스 설정 구성

이 자습서에서는 Azure Stack Edge Pro R 디바이스에 대한 디바이스 관련 설정을 구성하는 방법을 설명합니다. 로컬 웹 UI를 통해 디바이스 이름, 업데이트 서버 및 시간 서버를 설정할 수 있습니다.
...
Azure Stack Edge Pro R 디바이스에서 디바이스 관련 설정을 구성하기 전에 다음 사항을 확인합니다.

* 물리적 디바이스:

    - [Azure Stack Edge Pro R 설치](azure-stack-edge-pro-r-deploy-install.md)에서 설명한 대로 물리적 디바이스를 설치했습니다.
    - [자습서: Azure Stack Edge Pro R에 대한 네트워크 구성](azure-stack-edge-pro-r-deploy-configure-network-compute-web-proxy.md)에 설명된 대로 네트워크를 구성하고 디바이스에서 컴퓨팅 네트워크를 사용하도록 설정하고 구성했습니다.
...
1. **디바이스** 페이지에서 다음 단계를 수행합니다.
    ...
    3. 구성된 디바이스 설정의 유효성을 검사하고 설정을 적용하려면 **적용** 을 선택합니다.
        ...
        ![로컬 웹 UI "디바이스" 페이지 1](./media/azure-stack-edge-pro-r-deploy-set-up-device-update-time/device-2.png)
        ...
        ![로컬 웹 UI "디바이스" 페이지 2](./media/azure-stack-edge-pro-r-deploy-set-up-device-update-time/device-3.png)

    4. 디바이스 이름 및 DNS 도메인이 변경되면 SMB 엔드포인트가 생성됩니다.  

    5. 설정이 적용된 후 **다음: 업데이트 서버** 를 선택합니다.

        ![로컬 웹 UI "디바이스" 페이지 3](./media/azure-stack-edge-pro-r-deploy-set-up-device-update-time/device-4.png)
...
1. 이제 **업데이트** 페이지에서 디바이스에 대한 업데이트를 다운로드할 위치를 구성할 수 있습니다.  
    ...
    - **Microsoft 업데이트 서버** 에서 직접 업데이트를 가져올 수 있습니다.
        ...
        ![로컬 웹 UI "업데이트 서버" 페이지](./media/azure-stack-edge-pro-r-deploy-set-up-device-update-time/update-2.png)
...
1. **시간** 페이지에서 디바이스의 표준 시간대와 기본 및 보조 NTP 서버를 선택할 수 있습니다.  
    ...
    4. 구성된 시간 설정의 유효성을 검사하고 설정을 적용하려면 **적용** 을 선택합니다.
        ...
        ![로컬 웹 UI "시간" 페이지](./media/azure-stack-edge-pro-r-deploy-set-up-device-update-time/time-2.png)
...
Azure Stack Edge Pro R 디바이스의 인증서를 구성하는 방법을 알아보려면 다음을 참조하세요.

> [!div class="nextstepaction"]
> [인증서 구성](./azure-stack-edge-pro-r-deploy-configure-certificates-vpn-encryption.md)
