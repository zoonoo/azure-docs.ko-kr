...
1. [배포 검사 목록](azure-stack-edge-gpu-deploy-checklist.md)에 필요한 정보를 수집 합니다. 이전 장치 구성에서 저장 한 정보를 사용할 수 있습니다. 
...
1. 이전 장치에 사용한 것과 동일한 IP 주소를 사용 하 여 네트워크를 구성 합니다. 동일한 IP 주소를 사용 하면 사용자 환경에서 사용 되는 클라이언트 컴퓨터에 미치는 영향이 최소화 됩니다. [네트워크 설정을 구성](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)하는 방법을 참조 하세요.
1. 이전 장치와 동일한 장치 이름 및 DNS 도메인을 할당 합니다. 이렇게 하면 클라이언트는 동일한 장치 이름을 사용 하 여 새 장치와 통신할 수 있습니다. [장치 설정을 구성](azure-stack-edge-gpu-deploy-set-up-device-update-time.md)하는 방법을 참조 하세요.
...
1. Azure에서 [공유 데이터를 새로 고칩니다](azure-stack-edge-j-series-manage-shares.md#refresh-shares) . 그러면 기존 컨테이너에서 공유로 모든 클라우드 데이터를 가져옵니다.
