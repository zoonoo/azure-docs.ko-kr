...
### <a name="url-patterns-for-compute-for-azure-government"></a>Azure Government용 컴퓨팅 URL 패턴
...
- **컨테이너 관련 사항** - 다음 사항을 고려하십시오.
    ...
    - 컨테이너 사용 공간이 무엇인가요? 컨테이너에서 소비하는 메모리, 저장소 및 CPU의 양은 얼마인가요?
    - 워크로드에 있는 컨테이너는 몇 개인가요? 여러 개의 경량 컨테이너를 사용하거나 리소스를 많이 사용하는 컨테이너를 사용할 수 있습니다.
    - 컨테이너에 할당되는 리소스와 컨테이너가 소비하는 리소스(사용 공간)는 무엇인가요?
    - 컨테이너는 몇 개의 계층을 공유하나요? 컨테이너 이미지는 계층의 스택 안에 구성되는 파일 번들입니다. 컨테이너 이미지의 경우 리소스 소비량을 계산하기 위한 계층 수와 각 크기를 결정합니다.
