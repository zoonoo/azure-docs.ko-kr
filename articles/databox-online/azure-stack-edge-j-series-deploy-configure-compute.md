---
title: GPU가 있는 Azure Stack Edge Pro의 컴퓨팅 기능을 사용한 데이터를 필터링하고 분석하기 위한 자습서 | Microsoft Docs
description: Azure Stack Edge Pro GPU 디바이스에 컴퓨팅 역할을 구성하고 이 역할을 사용하여 데이터를 변환한 후에 Azure로 보내는 방법을 알아봅니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
3. **IoT Edge 서비스 만들기** 에서 IoT Hub 리소스에 대한 설정을 입력합니다.
...
1. 다음 단계에 따라 디바이스에 Edge 공유를 추가합니다.
    ...
    로컬 NFS 공유를 만든 경우 다음 원격 동기화(`rsync`) 명령 옵션을 사용하여 파일을 공유에 복사합니다.
...
1. 파일 탐색기에서, 이전에 만든 Edge 로컬 및 Edge 공유에 모두 연결합니다.
    ...
    ![데이터 변환 확인 - 1](./media/azure-stack-edge-j-series-deploy-configure-compute/verify-data-2.png) 
 
1. 로컬 공유에 데이터를 추가합니다.

    ![데이터 변환 확인 - 2](./media/azure-stack-edge-j-series-deploy-configure-compute/verify-data-3.png) 
 
   데이터가 클라우드 공유로 이동합니다.

    ![데이터 변환 확인 - 3](./media/azure-stack-edge-j-series-deploy-configure-compute/verify-data-4.png)  
