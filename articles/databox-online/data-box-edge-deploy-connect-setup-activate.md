...
description: Data Box Edge 배포에 대한 세 번째 자습서에서는 물리적 장치에 연결하고, 설정하고, 활성화하는 방법을 안내합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
이 자습서에는 로컬 웹 UI를 사용하여 Data Box Edge 장치에 연결하고, 설정하고, 활성화하는 방법을 설명합니다. 
...
* Data Box Edge 장치를 관리하기 위해 만든 Data Box Edge 서비스의 활성화 키를 갖고 있습니다. 자세한 내용은 [Azure Data Box Edge 배포 준비](data-box-edge-deploy-prep.md)를 참조하세요.

## <a name="connect-to-the-local-web-ui-setup"></a>로컬 웹 UI 설정에 연결 

1. 고정 IP 주소가 192.168.100.5이고 서브넷이 255.255.255.0인 Edge 장치에 연결하는 데 사용하는 컴퓨터에서 이더넷 어댑터를 구성합니다.
2. 장치의 포트 1에 컴퓨터를 연결합니다. 
3. 브라우저 창을 열고 https://192.168.100.10에서 장치의 로컬 웹 UI에 액세스합니다. 이 작업은 장치를 켠 후 몇 분 정도 걸릴 수 있습니다. 
...
2. 장치의 웹 UI에 로그인합니다. 기본 암호는 *Password1*입니다. 
...
## <a name="set-up-and-activate-the-physical-device"></a>물리적 장치 설정 및 활성화
 
1. 대시보드에서, 물리적 장치를 구성하고 Data Box Edge 서비스에 등록하는 데 필요한 다양한 설정으로 이동할 수 있습니다. **장치 이름**, **네트워크 설정**, **웹 프록시 설정** 및 **시간 설정**은 선택 사항입니다. 유일한 필수 설정은 **클라우드 설정**입니다.
...
3. (선택 사항) **네트워크 설정**을 구성합니다. 물리적 장치에는 6개의 네트워크 인터페이스가 표시됩니다. 포트 1 및 포트 2는 1Gbps 네트워크 인터페이스입니다. 포트 3, 포트 4, 포트 5 및 포트 6은 모두 25Gbps 네트워크 인터페이스입니다. 포트 1은 관리 전용 포트로 자동으로 구성되지만 포트 2 및 포트 6은 모두 데이터 포트입니다. 아래 표시된 **네트워크 설정** 페이지입니다.
...
6. **클라우드 설정** 페이지에서, Azure Portal의 Data Box Edge 서비스를 사용하여 장치를 활성화합니다.
    ...
    3. 장치가 성공적으로 활성화되면 연결 모드 옵션이 표시됩니다. 부분적으로 연결이 끊어짐 또는 연결 끊김 모드에서 장치를 사용해야 하는 경우 이러한 설정이 구성됩니다. 

        ![](./media/data-box-edge-deploy-connect-setup-activate/set-up-activate-7.png)    

장치 설정이 완료되었습니다. 이제 장치에서 공유를 추가할 수 있습니다.
