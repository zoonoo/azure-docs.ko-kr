...
Data Box Gateway가 만들어진 후에는 적절한 가상 디바이스 이미지를 다운로드하여 호스트 시스템에 가상 디바이스를 프로비전합니다. 가상 디바이스 이미지는 운영 체제 별로 구분되며 Azure Portal에서 리소스의 **빠른 시작** 블레이드에서 다운로드할 수 있습니다.
...
Data Box Gateway 리소스가 가동하여 실행되면 활성화 키를 얻어야 합니다. 이 키는 Data Box Gateway 디바이스를 활성화하고 리소스와 연결하는 데 사용됩니다.

활성화 키는 Data Box Gateway 리소스와 함께 활성화해야 하는 모든 Data Box Gateway 디바이스를 등록하는 데 사용됩니다. 이제 Azure Portal에서 이 키를 얻을 수 있습니다.
