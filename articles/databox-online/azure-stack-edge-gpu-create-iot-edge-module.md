---
title: Azure Stack Edge Pro with GPU용 C# IoT Edge 모듈 | Microsoft Docs
description: Azure Stack Edge Pro GPU 디바이스에 배포될 수 있는 C# IoT Edge 모듈을 개발하는 방법을 알아봅니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
Azure Stack Edge Pro로 Azure IoT Edge 모듈을 사용하여 데이터를 Azure로 이동할 수 있습니다. 이 문서에서 사용되는 모듈은 로컬 공유에서 Azure Stack Edge Pro 디바이스의 클라우드 공유로 파일을 복사하는 논리를 구현합니다.
...
![Azure Stack Edge Pro에서의 Azure IoT Edge 모듈 작동 원리](./media/azure-stack-edge-gpu-create-iot-edge-module/how-module-works-1.png)
...
- 실행 중인 Azure Stack Edge Pro 디바이스입니다.
    ...
    자세한 내용은 Azure Stack Edge Pro의 [컴퓨팅 구성](azure-stack-edge-j-series-deploy-configure-compute.md#configure-compute)을 참조하세요.<!--Update link?-->
...
3. 다음을 제공합니다.
   ...
   6. SKU를 **기본** 으로 설정합니다.
      ...
      ![컨테이너 레지스트리 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-container-registry-1.png)
 
4. **만들기** 를 선택합니다.
5. 컨테이너 레지스트리를 만든 후에는 해당 레지스트리를 찾고 **액세스 키** 를 선택합니다.

    ![선택키 가져오기](./media/azure-stack-edge-gpu-create-iot-edge-module/get-access-keys-1.png)
...
3. 명령 팔레트에서 **Azure IoT Edge: 새 IoT Edge 솔루션** 명령을 입력하고 실행합니다. 명령 팔레트에서 다음 정보를 제공하여 솔루션을 만듭니다.
    ...
    2. 솔루션에 대한 이름을 제공하거나 기본 **EdgeSolution** 을 그대로 적용합니다.
        ...
        ![새 솔루션 1 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-1.png)

    3. **C# 모듈** 을 모듈 템플릿으로 선택합니다.
    4. 기본 모듈 이름을 할당하려는 이름으로 바꿉니다. 이 예제의 경우 **FileCopyModule** 입니다.
    
        ![새 솔루션 2 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. 이전 섹션에서 만든 컨테이너 레지스트리를 첫 번째 모듈에 대한 이미지 리포지토리로 지정합니다. **localhost:5000** 을 복사한 로그인 서버 값으로 바꿉니다.
        ...
        ![새 솔루션 3 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-3.png)

4. **파일 > 폴더 열기** 로 이동합니다.

    ![새 솔루션 4 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-4.png)

5. 앞에서 만든 **EdgeSolution** 폴더를 찾고 가리킵니다. VS Code 창은 최상위 5개의 구성 요소로 IoT Edge 솔루션 작업 영역을 로드합니다. 이 문서에서는`.vscode` 폴더, **.gitignore** 파일, **.env** 파일, deployment.template.json**을 편집하지 않습니다.
    
    수정하는 유일한 구성 요소는 모듈 폴더입니다. 이 폴더에는 모듈에 대한 C# 코드 및 컨테이너 이미지로 모듈을 빌드하는 Docker 파일이 있습니다.

    ![새 솔루션 5 만들기](./media/azure-stack-edge-gpu-create-iot-edge-module/create-new-solution-5.png)
...
6. **PipeMessage 메서드** 용 코드를 제거하고, **FileCopy** 용 코드를 삽입합니다.
...
2. 통합 터미널에 다음 명령을 입력하여 Docker에 로그인합니다.
    ...
    ![IoT Edge 솔루션 빌드 및 푸시](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. 자격 증명이 제공되면 Azure 컨테이너 레지스트리에 모듈 이미지를 푸시할 수 있습니다. VS Code 탐색기에서 **module.json** 파일을 마우스 오른쪽 단추로 클릭하고 **IoT Edge 솔루션 빌드 및 푸시** 를 선택합니다.
    ...
    ![IoT Edge 솔루션 빌드 및 푸시 2](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    ![플랫폼 선택](./media/azure-stack-edge-gpu-create-iot-edge-module/select-platform.png)
...
Azure Stack Edge Pro에서 이 모듈을 배포하고 실행하려면 [모듈 추가](azure-stack-edge-j-series-deploy-configure-compute.md#add-a-module) 단계를 참조하세요.<!--Update link?-->
