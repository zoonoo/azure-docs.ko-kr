...
`Read all directory objects`하려면 Active Directory 테 넌 트에서 `User` 액세스 권한이 있어야 합니다. `Read all directory objects`권한이 없으므로 게스트 사용자가 될 수 없습니다. 게스트인 경우 활성화 키 생성, Data Box Edge 디바이스에 대한 공유 만들기, 사용자 만들기 등의 작업이 모두 실패합니다.
