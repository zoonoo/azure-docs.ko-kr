...
1. [Azure Portal](https://portal.azure.com/)에서 **모든 리소스**로 이동한 다음, Data Box Edge 리소스를 검색합니다.
    
1. 필터링된 리소스 목록에서 Data Box Edge 리소스를 선택합니다.

1. 왼쪽 창에서 **개요**를 선택한 다음, **공유 추가**를 선택합니다.
   
   ![공유 추가](./media/data-box-edge-deploy-add-shares/click-add-share.png)

1. **공유 추가** 창에서 다음 절차를 수행합니다.
    ...
    다. 공유가 저장될 스토리지 계정을 제공합니다.  
    컨테이너가 아직 없으면 스토리지 계정에 새로 만든 공유 이름으로 만들어집니다. 컨테이너가 이미 있으면 해당 컨테이너가 사용됩니다. 
    
    d. **스토리지 서비스** 드롭다운 목록에서 **블록 Blob**, **페이지 Blob** 또는 **파일**을 선택합니다.  
    선택하는 서비스 유형은 Azure에서 사용하려는 데이터 형식에 따라 달라집니다. 이 예제에서는 데이터를 Blob 블록으로 Azure에 저장하므로 **블록 Blob**을 선택합니다. [페이지 Blob]을 선택하는 경우 데이터가 512바이트로 정렬되어 있는지 확인합니다. 예를 들어 VHDX는 항상 512바이트로 정렬됩니다.
   
    e. SMB 공유 또는 NFS 공유를 만들었는지 여부에 따라 다음 단계 중 하나를 수행합니다. 
...
1. **만들기**를 선택하여 공유를 만듭니다. 
    
    공유 만들기가 진행 중이라는 알림이 표시됩니다. 공유가 지정된 설정으로 만들어지면 **공유** 섹션이 새 공유 정보로 업데이트됩니다. 
...
1. 이 작업을 요청하는 메시지가 표시되면 공유에 대한 암호를 입력합니다.  
...
1. 키보드에서 Windows+R을 선택합니다. 

1. **실행** 창에서 `\\<device IP address>`를 지정한 다음, **확인**을 선택합니다.  
