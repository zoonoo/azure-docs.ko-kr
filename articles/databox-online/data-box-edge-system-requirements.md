...
- 다음과 같이 장치의 PowerShell 인터페이스를 통해 사용할 수 있는 모니터링 명령입니다.
    ...
    - `dkrdbe stats`컨테이너 리소스 사용 통계의 라이브 스트림을 가져옵니다. 명령은 CPU, 메모리 사용량, 메모리 제한 및 네트워크 IO 메트릭을 지원 합니다.
    - `dkrdbe system df`사용 된 디스크 공간의 양에 대 한 정보를 가져옵니다. 
    - `dkrdbe image prune`사용 하지 않는 이미지를 정리 하 고 공간을 확보 합니다.
    - `dkrdbe ps --size`실행 중인 컨테이너의 대략적인 크기를 표시 합니다. 
