---
title: Azure 노트북 미리 보기를 사용 하 여 프로젝트와 데이터 가져오기 및 내보내기
description: 외부 원본에서 Azure Notebooks 미리 보기 프로젝트로 데이터를 가져오는 방법과 프로젝트에서 데이터를 내보내는 방법에 대해 알아봅니다.
ms.topic: how-to
ms.date: 12/04/2018
...
---
# <a name="work-with-data-files-in-azure-notebooks-preview-projects"></a>Azure 노트북 미리 보기 프로젝트에서 데이터 파일로 작업
...
1. **업로드** 명령을 선택하고, **URL에서** 또는 **컴퓨터에서** 중에 선택하고, 가져오려는 데이터에 대한 필수 세부 정보를 제공합니다.
   ...
   - **URL에서**: **파일 URL** 필드의 소스 주소와 파일 **이름** 필드의 프로젝트에서 전자 필기장에 할당할 파일 이름을 입력합니다. **+ 파일 추가**를 선택하여 업로드 목록에 URL을 추가합니다. URL이 더 있으면 이 프로세스를 반복한 다음, **완료**를 선택합니다.

     ![URL 팝업에서 업로드](media/quickstarts/upload-from-url-popup.png)

   - **컴퓨터에서**: 파일을 팝업으로 끌어 놓거나 **파일 선택을**선택한 다음 가져올 데이터 파일을 찾아서 선택합니다. 파일을 열고 데이터를 구문 분석하는 것은 Notebook의 코드에 달려 있으므로 파일 수, 파일 형식, 파일 형식을 원하는 대로 끌어서 놓거나 선택할 수 있습니다.
...
1. 실행 중인 전자 필기장 내에서 **파일** > **업로드** 명령을 선택합니다.
...
1. 나타나는 **업로드 상태** 팝업의 드롭다운 목록에서 **대상 폴더**를 선택합니다.
    ...
    - 세션 폴더*~/* (): 현재 전자 필기장 세션에 파일을 업로드하지만 프로젝트에서 파일을 만들지 않습니다. 세션 폴더는 피어-프로젝트 폴더이지만, 세션이 종료된 후에는 유지되지 않습니다. 코드에서 세션 파일에 액세스하려면 파일 이름의 접두사로 상대 경로 *../* 를 붙입니다.
