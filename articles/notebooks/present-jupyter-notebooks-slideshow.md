---
title: Azure Notebooks 미리 보기에 Jupyter 노트북을 슬라이드쇼로 제공
description: Jupyter 노트북에서 슬라이드쇼 모드로 셀을 구성 하는 방법에 대해 알아보고 확장 확장을 사용 하 여 슬라이드 쇼를 표시 합니다.
ms.topic: how-to
ms.date: 12/04/2018
...
ms.locfileid: "85834049"
---
# <a name="run-a-notebook-slideshow-in-azure-notebooks-preview"></a>Azure Notebooks 미리 보기에서 노트북 슬라이드 쇼 실행
...
1. 각 셀에 대해 5개 형식 중 하나를 선택합니다.
    ...
    | Fragment | 셀 콘텐츠는 아래쪽 탐색 화살표를 사용하는 경우 이전 슬라이드 또는 하위 슬라이드의 컨텍스트에서 나타납니다(조각은 위쪽 화살표를 사용하는 경우 제거됨). 코드 셀이 있는 조각을 사용해 그 코드를 슬라이드에 표시되도록 하거나 여러 조각을 사용하여 텍스트 글머리 기호가 하나씩 표시되도록 할 수 있습니다(다음 섹션의 예제 참조). 조각은 현재 슬라이드에서 빌드되므로, 과도한 조각은 브라우저 창 아래쪽에 나타나지 않습니다. |
    | 건너뛰기 | 셀이 슬라이드 쇼에 표시되지 않습니다. |
...
1. 코드 셀에서 재생 단추를 사용하여 코드를 실행하면 출력이 슬라이드에 표시됩니다.
    ...
    > [!Tip]
    > 슬라이드 쇼에서 셀 출력은 셀의 일부로 간주됩니다. Notebook 또는 슬라이드 쇼 보기에서 셀을 실행하는 경우 출력은 다른 보기에서도 나타납니다. 출력을 지우려면 현재 **셀**  >  **출력**  >  **clear** 명령 (현재 셀의 경우) 또는 **셀**  >  **모든 출력**  >  **지우기** (모든 셀의 경우)를 사용 합니다.
