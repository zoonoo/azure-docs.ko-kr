...
스마트 계약은 종종 외부 시스템 및 디바이스와 통합해야 하는 비즈니스 워크플로를 나타냅니다.
...
메시징 통합은 대화식 로그인이 불가능하거나 바람직하지 않은 시스템, 서비스 및 디바이스와의 상호 작용을 용이하게 합니다. 메시징 통합은 분산 원장에서 해당 트랜잭션이 실행되도록 요청하는 메시지와 트랜잭션이 발생할 때 해당 원장에서 노출하는 이벤트의 두 가지 메시지 유형에 중점을 둡니다.
...
Azure 플랫폼에서 디바이스와의 통합은 일반적으로 IoT Hub를 통해 수행됩니다. IoT Hub는 콘텐츠를 기반으로 메시지를 라우팅하는 기능을 제공하며 위에 설명된 기능 유형을 지원합니다.
