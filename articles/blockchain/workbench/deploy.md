---
title: Azure 블록체인 워크벤치 미리 보기 배포
description: Azure 블록 체인 워크벤치 미리 보기를 배포하는 방법
ms.date: 01/08/2020
ms.topic: article
ms.reviewer: brendal
...
---
# <a name="deploy-azure-blockchain-workbench-preview"></a>Azure 블록체인 워크벤치 미리 보기 배포

Azure 블록 체인 워크벤치 미리 보기는 Azure 마켓플레이스의 솔루션 템플릿을 사용하여 배포됩니다. 템플릿은 블록체인 애플리케이션을 만드는 데 필요한 구성 요소의 배포를 간소화합니다. 배포되면 Blockchain Workbench는 사용자와 블록체인 애플리케이션을 만들고 관리할 수 있는 클라이언트 앱에 대한 액세스를 제공합니다.
...
* Azure Key Vault
* Service Bus
* SQL 데이터베이스(표준 S0) + SQL 논리 서버
* Azure 저장소 계정(표준 LRS)
* 용량이 1인 가상 시스템 스케일 세트
* 가상 네트워크 리소스 그룹(로드 밸런서, 네트워크 보안 그룹, 공용 IP 주소, 가상 네트워크 사용)
* Azure 블록 체인 서비스. 이전 블록 체인 워크 벤치 배포를 사용하는 경우 Azure 블록 체인 워크 벤치를 다시 배포하여 Azure 블록 체인 서비스를 사용하는 것이 좋습니다.
...
1. **블록 체인** > **Azure 블록 체인 워크 벤치 (미리보기)를 선택합니다.**
    ...
    | 데이터베이스 및 블록체인 암호 | 배포의 일부로 생성된 데이터베이스에 대한 액세스에 사용할 암호를 지정합니다. 암호는 다음 네 가지 요구 사항 중 세 가지 를 충족해야 합니다: 길이는 12& 72자, 1 소문자, 1 대문자, 1 숫자 및 숫자 기호(#),\`퍼센트(%), 쉼표(%), 쉼표(), 별(*), 따옴표(),double quote("), single quote("), 대시(-) 및 반열(;) 사이에 있어야 합니다. |
...
1. **고급 성능**에서 새 블록체인 네트워크를 만들지 아니면 기존 인증 증명 블록체인 네트워크를 사용할지를 선택합니다.
    ...
    *새 만들기* 옵션은 기본 기본 sku를 사용하여 Azure 블록 체인 서비스 쿼럼 원장을 배포합니다.

    ![새 블록체인 네트워크에 대한 고급 설정](media/deploy/advanced-blockchain-settings-new.png)

    | 설정 | 설명  |
    |---------|--------------|
    | Azure 블록 체인 서비스 가격 책정 계층 | 블록 체인 워크 벤치에 사용되는 **기본** 또는 **표준** Azure 블록 체인 서비스 계층을 선택하십시오. |
    | Azure Active Directory 설정 | **나중에 추가**를 선택합니다.</br>참고: [Azure AD를 미리 구성](#azure-ad-configuration)하도록 선택했거나 재배포하는 경우 *지금 추가*하도록 선택합니다. |
    | VM 선택 | 블록 체인 네트워크에 대한 기본 스토리지 성능 및 VM 크기를 선택하십시오. Azure 무료 계층 같은 낮은 서비스 한도로 구독하는 경우 *표준 DS1 v2*와 같은 작은 크기의 VM을 선택합니다. |
...
## <a name="blockchain-workbench-web-url"></a>블록체인 워크벤치 웹 URL
...
1. 형식이 **App Service**인 리소스가 두 개 있습니다. "-api" **App Service** *접미사가 없는* 앱 서비스 형식의 리소스를 선택합니다.

    ![App Service 목록](media/deploy/resource-group-list.png)

1. 앱 서비스 **개요에서** **배포된** 블록체인 워크벤치에 웹 URL을 나타내는 URL 값을 복사합니다.
...
1. 클라우드 셸 PowerShell 환경에서 명령을 붙여넣기하고 실행합니다.
...
Azure 블록 체인 워크벤치 배포를 완료했습니다. 배포 사용을 시작하려면 [다음 단계를](#next-steps) 참조하세요.
...
Blockchain Workbench 배포에는 Azure AD 애플리케이션의 등록이 필요합니다. Azure AD(Azure Active Directory) 테넌트에서 앱을 등록해야 합니다. 기존 테넌트를 사용하거나 새 테넌트를 만들 수 있습니다. 기존 Azure AD 테넌트를 사용하는 경우 Azure AD 테넌트 내에서 애플리케이션을 등록하고, Graph API 권한을 부여하고, 게스트 액세스를 허용할 수 있는 충분한 권한이 필요합니다. 기존 Azure AD 테넌트에 충분한 사용 권한이 없는 경우 새 테넌트를 만듭니다.


1. [Azure 포털에](https://portal.azure.com)로그인합니다.
1. 오른쪽 위 모서리에서 사용자의 계정을 선택하고 원하는 Azure AD 테넌트로 전환합니다. 테넌트는 Azure Blockchain Workbench가 배포되고 응용 프로그램을 등록할 수 있는 충분한 권한이 있는 구독 관리자의 테넌트여야 합니다.
1. 왼쪽 탐색 창에서 **Azure Active Directory** 서비스를 선택합니다. **앱 등록 을** > 선택**새로운 등록.**

    ![앱 등록](media/deploy/app-registration.png)

1. 표시 **이름을** 제공하고 **이 조직 디렉터리에서 계정만**선택합니다.

    ![앱 등록 만들기](media/deploy/app-registration-create.png)

1. Azure AD 응용 프로그램을 등록하려면 **레지스터를** 선택합니다.
...
1. 등록한 응용 프로그램의 경우 **관리** 섹션에서 **매니페스트를** 선택합니다.
1. 다음으로 매니페스트의 **appRoles** 섹션을 업데이트합니다. 제공된 `"appRoles": []` JSON으로 바꿉습니다. **id** 필드에 대한 값을 사용자가 생성한 GUID로 바꾸었는지 확인합니다. 
...
1. 블록체인 *API* 앱 등록에서 **API 권한을 선택합니다.** 기본적으로 그래프 API **User.Read** 권한이 추가됩니다.
1. Workbench 응용 프로그램은 사용자의 기본 프로필 정보에 대한 읽기 액세스가 필요합니다. *구성된 사용 권한에서* **권한 추가를**선택합니다. **마이크로소프트 API에서** **마이크로소프트 그래프를**선택합니다.
1. Workbench 응용 프로그램은 인증된 사용자 자격 증명을 사용하므로 **위임된 권한을 선택합니다.**
1. *사용자* 범주에서 **User.ReadBasic.All** 권한을 선택합니다.

    ![Microsoft 그래프 User.ReadBasic.All 위임된 권한을 추가하는 것을 보여주는 Azure AD 앱 등록 구성](media/deploy/add-graph-user-permission.png)

    **권한 추가**를 선택합니다.

1. *구성된 사용 권한에서*도메인에 대한 **관리자 동의를** 선택한 다음 확인 프롬프트에 **대해 예를** 선택합니다.
...
1. 등록한 응용 프로그램의 경우 **개요를**선택합니다.
1. 나중에 배포하는 동안 사용할 수 위해 **응용 프로그램 ID** 값을 복사하여 저장합니다.
...
1. 왼쪽 탐색 창에서 **Azure Active Directory** 서비스를 선택합니다. **앱 등록을 선택합니다.**
1. 필수 구성 요소 섹션에 등록한 Azure AD 클라이언트 애플리케이션을 선택합니다.
1. **인증**을 선택합니다.
1. 블록 체인 워크 벤치 웹 URL 섹션에서 검색 한 Azure 블록 체인 워크 벤치 배포의 기본 [웹 URL을](#blockchain-workbench-web-url) 지정합니다. 회신 URL은 `https://`를 접두사로 사용합니다. 예를 들어 `https://myblockchain2-7v75.azurewebsites.net`

    ![인증 회신 URL](media/deploy/configure-reply-url.png)

1. 고급 **설정** 섹션에서 **액세스 토큰** 및 **ID 토큰을 확인합니다.**
