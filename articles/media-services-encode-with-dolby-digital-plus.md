...
Dolby Digital Plus는 Azure 미디어 인코더에서 지원됩니다. **Azure 미디어 인코더**에 대한 참조를 가져오려면 [미디어 프로세서 가져오기](../media-services-get-media-processor) 항목을 참조하세요.

###<a id="configure_preset"></a>Azure 미디어 인코더 설정 구성

Azure 미디어 인코더에서 사용할 인코딩 설정을 구성하면 기억하기 쉬운 문자열로 표현된 여러 가지 미리 정의된 사전 설정이 있습니다. Dolby Digital Plus 인코더는 풍부한 컨트롤 배열을 제공합니다. 자세한 내용은 [<DolbyDigitalPlusAudioProfile>](https://msdn.microsoft.com/library/azure/dn296500.aspx)을 참조하세요. 따라서 이 코덱을 사용하는 미리 작성된 문자열 사전 설정은 없습니다. XML 파일에서 원하는 인코더 설정을 지정하고 이 데이터를 제출해야 합니다. with your Task as shown in the following code example:
...
인코딩 작업을 만들려면 이 XML 사전 설정을 **Azure 미디어 인코더**로 전달해야 합니다. 해당 내용은 [이](../media-services-dotnet-encode-asset) 항목([여기]에 설명된 대로 미리 정의된 사전 설정 문자열 대신에만 전체 XML 사전 설정 전달(#configure_preset))을 참조하세요.
...
Azure 미디어 인코더는 DPLM SDK를 사용하여 입력 오디오에서 대화 음의 강도를 측정하고 올바른 DialogNormalization 값을 설정할 수 있습니다. 이 기능은 오디오가 Dolby Digital Plus로 인코딩되는 경우에만 사용됩니다. DPLM은 사전 설정 구성 파일에서 <DolbyDigitalPlusAudioProfile> 요소의 자식인 <LoudnessMetering> 요소를 사용하여 구성합니다. 다음 샘플 사전 설정에서는 DPLM을 구성하는 방법을 보여 줍니다.
