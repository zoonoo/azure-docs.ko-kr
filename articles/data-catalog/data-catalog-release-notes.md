...
**Azure Data Catalog** 포털에서 "Power BI Desktop에서 열기" 옵션을 사용할 때 Power BI Desktop 애플리케이션에서 다음 두 가지 문제 중 하나가 발생할 수 있습니다.
...
이 ADC 릴리스 날짜 현재, 최신 [windows용 Teradata ODBC 드라이버(버전 15.10)](https://downloads.teradata.com/download/connectivity/odbc-driver/windows) 는 Office 2013과 호환되지만 Office 2016과는 호환되지 않습니다.
...
**원인 1: Active Directory Federation Services 구성** 데이터 원본 등록 도구는 Active Directory에 대 한 사용자 로그온의 유효성을 검사 하 여 폼 인증을 사용 합니다. 로그온이 성공하려면 Active Directory 관리자가 전역 인증 정책에서 폼 인증을 사용하도록 설정해야 합니다.
...
**원인 2: 네트워크 프록시 구성** 회사 네트워크에서 프록시 서버를 사용 하는 경우 등록 도구 못할 프록시를 통해 Azure Active Directory에 연결 합니다. 도구의 구성 파일을 편집하고 이 섹션을 파일에 추가하여 사용자가 등록 도구를 확인할 수 있습니다.
