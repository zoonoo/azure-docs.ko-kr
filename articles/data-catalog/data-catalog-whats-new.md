...
2016년 2월 19일 주를 기준으로 다음과 같은 기능이 **Azure 데이터 카탈로그**에 추가되었습니다.

- **Azure 데이터 카탈로그** 데이터 소스 등록 도구의 데이터 소스 선택 환경이 새로 디자인되었습니다. 사용자가 **Azure 데이터 카탈로그**에서 지원하는 데이터 소스를 보다 쉽게 찾아서 선택할 수 있도록 데이터 소스 등록 도구가 업데이트되었습니다.
- **Azure 데이터 카탈로그** 포털 및 데이터 소스 등록 도구에서 10개의 추가 언어가 지원됩니다. **Azure 데이터 카탈로그** 환경이 이제 영어 이외에 독일어, 스페인어, 프랑스어, 이탈리아어, 일본어, 한국어, 포르투갈어(브라질), 러시아어, 중국어 간체 및 중국어 번체로 제공됩니다. **Azure 데이터 카탈로그** 사용자 환경이 Windows 또는 사용자의 웹 브라우저에 설정된 언어 기본 설정에 따라 지역화됩니다.
- 비즈니스 연속성 및 재해 복구를 위해 **Azure 데이터 카탈로그** 데이터에 대한 지역에서 복제가 지원됩니다. 이제 데이터 소스 메타데이터 및 크라우드소싱 주석을 포함하여 모든 **Azure 데이터 카탈로그** 내용이 두 Azure 지역 간에 무료로 복제됩니다. Azure 지역은 500마일 이상 떨어진 지역이 사전에 쌍을 이루며, [BCDR(비즈니스 연속성 및 재해 복구): Azure 쌍을 이루는 지역](../best-practices-availability-paired-regions.md)에 설명된 대로 매핑을 따릅니다. 
...
- **Azure 데이터 카탈로그** 포털에서 데이터 자산을 고정하거나 고정 해제할 수 있습니다. 사용자는 쉽게 다시 검색하고 다시 사용하기 위해 데이터 자산을 고정하도록 선택할 수 있습니다.
- **Azure 데이터 카탈로그** 포털의 홈 페이지가 새로 디자인되었습니다. 새 홈페이지에는 카탈로그 전체의 활동에 대한 정보뿐만 아니라 최근에 게시된 자산, 고정된 자산 및 저장된 검색 등 현재 사용자 활동에 대한 정보가 포함되어 있습니다.
- **Azure 데이터 카탈로그** 포털에서 영구 사용자 설정이 지원됩니다. 표 또는 타일 보기, 페이지당 결과 수 및 적중 항목 강조 표시 설정 또는 해제 등 사용자 환경 설정이 사용자 세션 간에 유지됩니다.
