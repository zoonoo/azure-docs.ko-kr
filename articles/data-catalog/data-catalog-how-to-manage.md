<properties
   pageTitle="데이터 자산을 관리하는 방법 | Microsoft Azure"
...
데이터 카탈로그는 사용자가 데이터 자체에 액세스할 권한을 주지 않습니다. 데이터 원본의 소유자가 데이터 액세스를 제어합니다. 데이터 카탈로그를 사용하면 사용자가 데이터 원본을 검색하고 카탈로그에 등록된 소스와 관련된 메타데이터를 볼 수 있습니다.

그러나 데이터 원본을 특정 사용자 또는 특정 그룹의 구성원에게만 표시해야 할 상황이 있을 수 있습니다. 이러한 시나리오의 경우 데이터 카탈로그를 사용하면 사용자가 카탈로그 내에서 등록된 데이터 자산의 소유권을 가져온 다음 소유한 자산의 표시 여부를 제어할 수 있습니다.

> [AZURE.NOTE] 이 문서에서 설명한 기능은 Azure Data Catalog의 표준 버전에서만 사용할 수 있습니다. 무료 버전에서는 소유권 및 데이터 자산의 표시를 제한하기 위한 기능을 제공하지 않습니다.
...
> [AZURE.NOTE] 데이터 카탈로그의 소유권은 카탈로그에 저장된 메타데이터에 영향을 줍니다. 기본 데이터 원본에 대한 권한을 부여하지 않습니다.
...
### <a name="adding-owners-and-co-owners"></a>소유자 및 공동 소유자 추가
데이터 자산을 이미 소유한 경우 사용자는 소유권을 그냥 가져올 수 없습니다. 기존 소유자에 의해 공동 소유자로 추가되어야 합니다. 소유자는 추가 사용자 또는 보안 그룹을 공동 소유자로 추가할 수 있습니다.

> [AZURE.NOTE] 소유한 데이터 자산에 둘 이상의 개인이 소유자인 것이 좋습니다.

### <a name="removing-owners"></a>소유자 제거
자산 소유자가 공동 소유자를 추가할 수 있는 것처럼 자산 소유자는 공동 소유자를 제거할 수 있습니다.

자산 소유자가 자신을 소유자에서 제거하는 경우 자산을 더 이상 관리할 수 없습니다. 자산 소유자가 자신을 소유자에서 제거하고 다른 공동 소유자가 없는 경우 자산은 소유하지 않은 상태로 변환됩니다.
...
> [AZURE.NOTE] 가능한 경우에는 개별 사용자가 아닌 보안 그룹에 자산 소유권 및 표시 여부 권한을 할당해야 합니다.
