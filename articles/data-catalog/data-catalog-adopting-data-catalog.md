...
---
# Azure Data Catalog 채택을 위한 접근 방식 및 프로세스
...
**Azure Data Catalog**가 해결하는 비즈니스 문제 중 하나는 **데이터 생산자**를 **데이터 소비자**에 연결하는 것입니다. 기업 데이터 원본에 관한 정보의 중앙 저장소 역할을 함으로써 이 작업을 수행합니다. David는 데이터 카탈로그를 사용하여 Adventure Works 및 SQL Server 데이터 원본을 등록합니다. 이 데이터 원본을 검색하는 사용자의 크라우드소싱을 사용 하면 자신이 검색한 데이터를 사용할 뿐만 아니라 데이터에 관한 자신의 의견을 공유할 수 있습니다. 예를 들어 Nancy는 카탈로그를 검색하여 데이터 원본을 검색하며 데이터에 관한 자신의 전문화된 지식을 공유합니다.  이제 조직의 다른 사람은 데이터 카탈로그를 검색하여 공유되는 지식에서 이익을 얻습니다.
