...
**Azure Data Catalog** 를 사용하면 Adventure Works 재무 팀이 주요 데이터 원본에 대해 협업하여 모든 팀원이 필요한 데이터를 쉽게 찾아서 사용하고 지식을 팀 전체와 공유할 수 있습니다.
...
파일럿 프로젝트를 선택할 때 기존 비즈니스 문제를 해결하는 가장 매력적인 시나리오를 가진 팀을 선택합니다. 예를 들어 비즈니스 분석자는 SQL 서버 데이터베이스에서 보고서를 작성합니다. 문제는 이 분석자가 여러 동료에게 말한 후에만 데이터 원본을 인식하게 되었다는 것입니다. 마지막으로, 많은 시간을 소비하여 사용할 데이터 원본을 찾은 후 각 데이터 원본에 대한 설명이 포함된 Excel 통합 문서를 발견했습니다. Excel 통합 문서는 분석자에게 필요한 테이블을 적절히 설명하지만 이러한 데이터 원본이 **Azure Data Catalog** 에 등록되고 주석이 추가되어 있었다면 빨리 발견했을 것입니다.
...
**Azure Data Catalog** 가 해결하는 비즈니스 문제 중 하나는 **데이터 생산자** 를 **데이터 소비자** 에 연결하는 것입니다. 기업 데이터 원본에 관한 정보의 중앙 저장소 역할을 함으로써 이 작업을 수행합니다. David는 데이터 카탈로그를 사용하여 Adventure Works 및 SQL Server 데이터 원본을 등록합니다. 이 데이터 원본을 검색하는 사용자의 크라우드소싱을 사용하면 사용자가 검색한 데이터를 사용할 뿐만 아니라 데이터에 관한 의견을 공유할 수 있습니다. 예를 들어 Nancy는 카탈로그를 검색하여 데이터 원본을 검색하며 데이터에 관한 자신의 전문화된 지식을 공유합니다.  이제 조직의 다른 사람은 데이터 카탈로그를 검색하여 공유되는 지식에서 이익을 얻습니다.
...
예를 들어 비즈니스 분석가 Nancy는 SQL Server 데이터베이스의 데이터를 사용하여 보고서를 만듭니다. 문제는 이 분석자가 여러 동료에게 말한 후에만 데이터 원본을 인식하게 되었다는 것입니다. **Azure Data Catalog** 와 같은 중앙 위치에 등록되고 주석을 추가했다면 이러한 데이터 원본을 빨리 찾을 수 있었을 것입니다.
