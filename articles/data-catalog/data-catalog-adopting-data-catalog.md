...
**Azure 데이터 카탈로그는** Adventure Works 재무 팀이 주요 데이터 원본에서 공동 작업을 수행할 수 있도록 지원하므로 모든 팀 구성원이 필요한 데이터를 쉽게 찾고 사용할 수 있으며 팀 전체와 지식을 공유할 수 있습니다.
...
파일럿 프로젝트를 선택할 때 기존 비즈니스 문제를 해결하는 가장 매력적인 시나리오를 가진 팀을 선택합니다. 예를 들어 비즈니스 분석자는 SQL 서버 데이터베이스에서 보고서를 작성합니다. 문제는 여러 동료와 이야기 한 후에만 데이터 원본을 알게되었다는 것입니다. 마지막으로 사용할 데이터 원본을 찾는 데 시간을 낭비한 후 각 데이터 원본에 대한 설명이 포함된 Excel 통합 문서에 대해 알아보았습니다. Excel 통합 문서에서 필요한 테이블을 적절하게 설명하지만 **Azure Data Catalog에**등록되고 추가된 경우 이러한 데이터 원본을 신속하게 찾았을 것입니다.
...
**Azure Data Catalog**가 해결하는 비즈니스 문제 중 하나는 **데이터 생산자**를 **데이터 소비자**에 연결하는 것입니다. 기업 데이터 원본에 관한 정보의 중앙 저장소 역할을 함으로써 이 작업을 수행합니다. David는 데이터 카탈로그를 사용하여 Adventure Works 및 SQL Server 데이터 원본을 등록합니다. 이 데이터 원본을 발견한 모든 사용자를 크라우드소싱하면 발견한 데이터를 사용하는 것 외에도 데이터에 대한 자신의 의견을 공유할 수 있습니다. 예를 들어 Nancy는 카탈로그를 검색하여 데이터 원본을 검색하며 데이터에 관한 자신의 전문화된 지식을 공유합니다.  이제 조직의 다른 사람은 데이터 카탈로그를 검색하여 공유되는 지식에서 이익을 얻습니다.
...
예를 들어 비즈니스 분석가 Nancy는 SQL Server 데이터베이스의 데이터를 사용하여 보고서를 만듭니다. 문제는 여러 동료와 이야기 한 후에만 데이터 원본을 알게되었다는 것입니다. **Azure 데이터 카탈로그와**같은 중앙 위치에 등록되고 추가된 경우 이러한 데이터 원본을 신속하게 찾았을 것입니다.
...
* **실패할 수 있도록** 하십시오 - 파일럿이 원하는 결과를 얻지 못하는 경우, 변경해야 할 영역을 다시 평가하고 식별합니다 - 더 큰 범위로 이동하기 전에 파일럿의 문제를 해결합니다.
