...
Azure Data Catalog는 기업 데이터 원본의 등록 시스템 및 검색 역할을 하는 완전히 관리되는 클라우드 서비스입니다. 즉, 데이터 카탈로그는 사람들이 데이터 원본을 검색, 이해 및 사용하는 데 도움이 됩니다. 이를 통해 조직은 기존 데이터에서 더 많은 가치를 얻을 수 있습니다. 데이터 카탈로그를 사용하여 데이터 원본이 등록되면 해당 메타데이터는 서비스로 인덱싱되어 사용자가 쉽게 검색하여 필요한 데이터를 검색할 수 있습니다.
...
검색 결과를 볼 때 히트 강조 표시를 사용하도록 설정한 경우에도 데이터 자산이 포함된 이유가 항상 명확하지 않을 수 있습니다. 모든 속성은 기본적으로 검색되므로 열 수준 속성과 일치로 인해 데이터 자산이 반환될 수 있습니다. 또한 여러 사용자가 등록된 데이터 자산에 자신의 태그와 설명을 추가할 수 있으므로 모든 메타데이터가 검색 결과 목록에 표시되는 것은 아닙니다.
