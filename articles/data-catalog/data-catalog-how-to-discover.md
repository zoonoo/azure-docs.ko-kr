...
Azure Data Catalog는 기업 데이터 원본의 등록 시스템 및 검색 역할을 하는 완전히 관리되는 클라우드 서비스입니다. 즉, Data Catalog는 사용자가 데이터 원본을 검색 하 고 이해 하 고 사용 하는 데 도움이 됩니다. 이를 통해 조직은 기존 데이터에서 더 많은 가치를 얻을 수 있습니다. 데이터 카탈로그를 사용하여 데이터 원본이 등록되면 해당 메타데이터는 서비스로 인덱싱되어 사용자가 쉽게 검색하여 필요한 데이터를 검색할 수 있습니다.
...
검색 결과를 볼 때 적중 항목 강조 표시를 사용 하는 경우에도 데이터 자산이 포함 되는 이유가 항상 명확 하지는 않을 수 있습니다. 모든 속성은 기본적으로 검색되므로 열 수준 속성과 일치로 인해 데이터 자산이 반환될 수 있습니다. 여러 사용자가 고유한 태그와 설명을 사용 하 여 등록 된 데이터 자산에 주석을 달 수 있으므로 모든 메타 데이터는 검색 결과 목록에 표시 되지 않습니다.
