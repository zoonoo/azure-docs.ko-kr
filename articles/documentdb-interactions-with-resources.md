...
마지막으로, 위 예제의 저장 프로시저를 실행하려면 저장 프로시저 리소스 (/dbs/_rid-db/colls/_rid-coll/sprocs/sproc1)의 URI에 대해 POST를 실행해야 합니다. 이 내용은 다음 코드에 나와 있습니다.  
...
1.	두 클라이언트가 최신 리소스 버전([if-match] 요청 헤더를 통해 지정됨)을 사용하여 리소스에서 변형되는 요청을 동시에 실행할 경우(PUT/ DELETE 동사를 통해) DocumentDB 데이터베이스 엔진이 트랜잭션 동시성 제어를 적용합니다.
2.	클라이언트가 이전 리소스 버전([if-match] 요청 헤더를 통해 지정됨)을 제공할 경우 요청이 거부됩니다.  
...
1.	**게이트웨이 모드:** 클라이언트가 논리적 주소를 물리적 주소로 변환하거나 라우팅 세부 정보에 액세스할 수 없습니다. 단순히 논리적 URI를 다루며 RESTful 방식으로 리소스 모델을 탐색합니다. 클라이언트가 논리적 URI로 요청을 실행하면 에지 컴퓨터가 논리적 URI를 복제본의 물리적 주소로 변환합니다. 복제본은 리소스를 관리하고 요청을 전달합니다. 라우팅 테이블을 캐싱하고 주기적으로 새로 고치는 에지 컴퓨터가 있으면 라우팅이 매우 효율적입니다.
2.	**직접 연결 모드:** 클라이언트가 해당 프로세스 공간에서 직접 라우팅 테이블을 관리하고 주기적으로 새로 고칩니다. 클라이언트는 복제본에 직접 연결하여 에지 컴퓨터를 무시할 수 있습니다.
...
<table width="300">
    <tbody>
        ...
        <tr>
            ...
            <td width="264" valign="top">
                <p>
                    응용 프로그램이 논리적 URI를 사용하여 에지 노드에 직접 연결합니다. 이 경우 추가 네트워크 홉이 발생합니다.
                </p>
            </td>
            <td width="150" valign="top">
                <p>
        ...
        <tr>
            ...
            <td width="264" valign="top">
                <p>
                    응용 프로그램이 라우팅 테이블에 직접 액세스하고 클라이언트 쪽 라우팅을 수행하여 복제본에 직접 연결할 수 있습니다.
                </p>
            </td>
            <td width="150" valign="top">
                <p>
...
##다음 단계
REST API를 사용하여 리소스에 대한 작업을 수행하는 방법에 대한 자세한 내용은 [Azure DocumentDB REST API 참조](https://msdn.microsoft.com/library/azure/dn781481.aspx)(영문)를 살펴보세요.

##참조
-   [Azure DocumentDB REST API 참조](https://msdn.microsoft.com/library/azure/dn781481.aspx)(영문)
-	REST [http://en.wikipedia.org/wiki/Representational_state_transfer](http://en.wikipedia.org/wiki/Representational_state_transfer)(영문)
-	JSON 사양 [http://www.ietf.org/rfc/rfc4627.txt](http://www.ietf.org/rfc/rfc4627.txt)(영문)
-	HTTP 사양 [http://www.w3.org/Protocols/rfc2616/rfc2616.html](http://www.w3.org/Protocols/rfc2616/rfc2616.html)(영문)
-	엔터티 태그 [http://en.wikipedia.org/wiki/HTTP_ETag](http://en.wikipedia.org/wiki/HTTP_ETag)(영문)
