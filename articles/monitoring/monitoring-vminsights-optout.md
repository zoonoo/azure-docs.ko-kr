...
VM용 Azure Monitor를 온보딩하기 전에 작업 영역에서 Windows 또는 Linux 기반 VM에 [사용하도록 설정된 성능 카운터를 수집](monitoring-vminsights-onboard.md?toc=/azure/azure-monitor/toc.json#performance-counters-enabled)하지 않는 경우 [여기](../log-analytics/log-analytics-data-sources-performance-counters.md?toc=/azure/azure-monitor/toc.json#configuring-performance-counters)에 설명된 단계를 따라 Linux 및 Windows에 대한 해당 규칙을 사용하지 않도록 설정해야 합니다.
