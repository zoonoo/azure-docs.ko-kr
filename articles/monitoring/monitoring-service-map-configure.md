...
이 문서에서는 서비스 맵 및 에이전트 등록에 대해 자세히 설명합니다. 서비스 맵 사용에 대한 자세한 내용은 [Azure에서 서비스 맵 솔루션 사용]( monitoring-service-map.md)을 참조하세요.
...
| 연결된 원본 | 지원됨 | 설명 |
|:--|:--|:--|
| Windows 에이전트 | yes | 서비스 맵은 Windows 컴퓨터에서 데이터를 분석하고 수집합니다. <br><br>[Windows용 Log Analytics 에이전트](../log-analytics/log-analytics-concept-hybrid.md) 외에도 Windows 에이전트에는 Microsoft Dependency Agent가 필요합니다. 운영 체제 버전의 전체 목록은 [지원되는 운영 체제](#supported-operating-systems)를 참조하세요. |
| Linux 에이전트 | yes | 서비스 맵은 Linux 컴퓨터에서 데이터를 분석하고 수집합니다. <br><br>[Linux용 Log Analytics 에이전트](../log-analytics/log-analytics-concept-hybrid.md) 외에도 Linux 에이전트에는 Microsoft Dependency Agent가 필요합니다. 운영 체제 버전의 전체 목록은 [지원되는 운영 체제](#supported-operating-systems)를 참조하세요. |
...
1.  [Log Analytics 에이전트를 통해 하이브리드 환경에서 데이터 수집](../log-analytics/log-analytics-concept-hybrid.md)에 설명된 방법 중 하나를 수행하여 Log Analytics 에이전트를 설치합니다.
...
결과에서 다양한 이벤트를 얻었나요? 최근 데이터인가요? 그렇다면 Log Analytics 에이전트가 올바르게 작동하고 Log Analytics와 통신하고 있습니다. 그렇지 않으면 [Windows용 Log Analytics 에이전트 문제 해결](https://support.microsoft.com/help/3126513/how-to-troubleshoot-monitoring-onboarding-issues) 또는 [Linux용 Log Analytics 에이전트 문제 해결](../log-analytics/log-analytics-agent-linux-support.md)을 참조하여 서버의 에이전트를 확인합니다.
...
- 서비스 맵이 배포 및 구성된 후 [서비스 맵을 사용]( monitoring-service-map.md)하는 방법을 알아봅니다.
