...
5.  로드를 기준으로 인스턴스 수가 자동으로 조정되도록 하려면 **Autoscale Mode** 아래에서 **성능**을 선택합니다. 이때 Azure 포털 Preview에서 **일정**을 선택할 수 없습니다.
...
![CPU 비율이 있는 크기 조정 블레이드](./media/insights-how-to-scale/Insights_ScalePartBladeOn.png)
...
선택하는 각 규칙의 옵션은 다음과 같습니다. - 조건 - 보다 큼 또는 보다 작음 - Threshold - 동작을 트리거하기 위해 이 메트릭이 통과해야 하는 수치 - Over Past - 이 메트릭의 평균을 계산하는 범위 시간(분) - Scale up or down by - 크기 조정 작업의 크기 - Cool down - 이전 크기 조정 작업 이후에 이 규칙이 다시 크기 조정하도록 대기해야 하는 시간
