...
description: Azure Database for MySQL 보안 기준은 Azure 보안 벤치 마크에 지정 된 보안 권장 사항을 구현 하기 위한 절차 지침과 리소스를 제공 합니다.
author: msmbaldwin
ms.service: mysql
ms.topic: conceptual
ms.date: 09/02/2020
...
- [Azure Database for MySQL에 대 한 서비스 태그 사용 이해](concepts-data-access-and-security-vnet.md#terminology-and-description)
...
태그를 사용 하 여 모든 리소스를 만들고 태그가 지정 되지 않은 기존 리소스를 알리기 위해 태그 지정과 관련 된 기본 제공 Azure Policy 정의 (예: **태그 및 해당 값 필요** )를 사용 합니다.
...
- [Azure Database for MySQL에 대해 Advanced Threat Protection을 사용 하도록 설정 하는 방법 (미리 보기)](howto-database-threat-protection-portal.md)
...
- [Azure Database for MySQL 리소스 공급자 작업 이해](../role-based-access-control/resource-provider-operations.md#microsoftdbformysql)

- [Azure Database for MySQL에 대 한 액세스 관리 이해](concepts-security.md#access-management)
...
- [Azure Database for MySQL에 대 한 추가 계정을 만드는 방법](howto-create-users.md)
...
- [Azure Database for MySQL 인증에 Azure Active Directory 사용](howto-configure-sign-in-azure-ad-authentication.md)
...
- [Azure Database for MySQL에 대 한 데이터 반출을 완화 하는 방법](concepts-data-access-security-private-link.md#data-exfiltration-prevention)
...
- [Azure Database for MySQL에 대 한 전송에서 암호화를 구성 하는 방법](concepts-ssl-connection-security.md)
...
- [Azure Database for MySQL 용 SQL을 사용 하 여 사용자 액세스를 구성 하는 방법](howto-create-users.md)
...
- [Azure Database for MySQL에 대 한 미사용 암호화 이해](concepts-security.md)

- [Azure Database for MySQL에 대 한 고객 관리 키를 구성 하는 방법](concepts-data-encryption-mysql.md)
...
**지침**: Azure Database for MySQL 및 관련 리소스를 보호 하는 Azure Security Center의 권장 사항을 따릅니다.

Microsoft는 Azure Database for MySQL을 지 원하는 기본 시스템에서 취약성 관리를 수행 합니다.
...
**지침**: Azure 리소스 그래프를 사용 하 여 구독 내의 모든 리소스 (Azure Database for MySQL 인스턴스 포함)를 쿼리하고 검색 합니다. 테넌트에서 적절한 권한(읽기)이 있는지 확인하고, 모든 Azure 구독 및 구독 내의 리소스를 열거할 수 있습니다.
...
**지침**: Azure Policy로 Azure Database for MySQL 인스턴스에 대한 표준 보안 구성을 정의하고 구현합니다. **Microsoft DBforMySQL** 네임 스페이스의 Azure Policy 별칭을 사용 하 여 Azure Database for MySQL 인스턴스의 네트워크 구성을 감사 하거나 적용 하는 사용자 지정 정책을 만듭니다. 다음과 같이 Azure Database for MySQL 인스턴스와 관련 된 기본 제공 정책 정의를 사용할 수도 있습니다.
...
**지침**: **Microsoft DBforMySQL** 네임 스페이스의 Azure Policy 별칭을 사용 하 여 시스템 구성을 경고, 감사 및 적용 하기 위한 사용자 지정 정책을 만듭니다. Azure Database for MySQL 인스턴스 및 관련 리소스에 대한 구성을 자동으로 적용하려면 Azure Policy [감사], [거부] 및 [없는 경우 배포]를 사용합니다.
...
- [Azure Database for MySQL에 대 한 백업 이해](concepts-backup.md)
...
- [Azure Database for MySQL에 대 한 백업 이해](howto-restore-server-portal.md) 
...
- [Azure Database for MySQL에서의 백업 및 복원 이해](concepts-backup.md)
