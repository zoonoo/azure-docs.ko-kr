...
- 주 데이터베이스 엔진 버전 간에 자동화된 마이그레이션은 현재 지원되지 않습니다. 다음의 주 버전으로 업그레이드하려는 경우 새 엔진 버전을 사용하여 생성된 서버에 주 버전을 [덤프 및 복원](./concepts-migrate-dump-restore.md)합니다.
