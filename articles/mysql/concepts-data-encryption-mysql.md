...
* [일시 삭제]를 사용 하도록 설정 합니다 (. /key-vault/general/soft-delete-overview.md)은 키 자격 증명 모음에서 보존 기간이 **90 일로** 설정 된 경우 실수로 인 한 키 또는 Key Vault 삭제가 발생 하는 경우 데이터 손실을 방지 하기 위한 기능입니다. 일시 삭제 된 리소스는 보존 기간이 명시적으로 <= 90 일로 설정 되지 않는 한 기본적으로 90 일 동안 보존 됩니다. 복구 및 제거 작업에는 키 자격 증명 모음 액세스 정책과 연결된 고유 권한이 있습니다. 일시 삭제 기능은 기본적으로 해제되어 있지만, PowerShell 또는 Azure CLI를 통해 사용하도록 설정할 수 있습니다(Azure Portal에서는 사용하도록 설정할 수 없음).
...
* 키가 *사용* 상태여야 합니다.
* 키에는 보존 기간이 **90 일로** 설정 된 [일시 삭제가](../key-vault/general/soft-delete-overview.md) 있어야 합니다.
* Kay는 [제거 보호를 사용 하도록 설정](../key-vault/general/soft-delete-overview.md#purge-protection)해야 합니다.
