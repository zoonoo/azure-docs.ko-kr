...
description: 고객 관리 키를 사용 하 여 데이터 암호화 Azure Database for MySQL
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
* 암호화 키에 대 한 투명도, 세부적인 제어 및 관리 향상 
...
Key Vault에 저장 된 고객의 관리 키를 사용 하 여 Azure Database for MySQL 암호화 되 면 로컬 또는 지역 복원 작업이 나 읽기 복제본을 통해 새로 생성 된 서버 복사본도 동일한 고객의 관리 키를 사용 하 여 암호화 됩니다. 그러나 암호화를 위해 새로운 고객의 관리 키를 반영 하도록 변경할 수 있습니다. 고객 관리 키가 변경 되 면 서버의 이전 백업에서 최신 키를 사용 하기 시작 합니다.
...
[Azure Portal](howto-data-encryption-portal.md)를 사용 하 여 Azure Database for MySQL에 대해 고객이 관리 하는 키를 사용 하 여 데이터 암호화를 설정 하는 방법을 알아봅니다.
