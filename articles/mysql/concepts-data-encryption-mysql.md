...
* 우발적인 키 (또는 Key Vault) 삭제가 발생 하는 경우 데이터 손실을 방지 하기 위해 보존 기간이 **90 일로** 설정 된 key vault에서 [일시 삭제](../key-vault/general/soft-delete-overview.md) 기능을 사용 하도록 설정 합니다. 일시 삭제 된 리소스는 보존 기간이 명시적으로 <= 90 일로 설정 되지 않는 한 기본적으로 90 일 동안 보존 됩니다. 복구 및 제거 작업에는 키 자격 증명 모음 액세스 정책과 연결된 고유 권한이 있습니다. 일시 삭제 기능은 기본적으로 해제되어 있지만, PowerShell 또는 Azure CLI를 통해 사용하도록 설정할 수 있습니다(Azure Portal에서는 사용하도록 설정할 수 없음).
* 보존 기간이 **90 일로** 설정 된 주요 자격 증명 모음에서 [보호 제거](../key-vault/general/soft-delete-overview.md#purge-protection) 기능을 사용 하도록 설정 합니다. 일시 삭제를 사용 하도록 설정한 후에만 제거 보호를 사용 하도록 설정할 수 있습니다. Azure CLI 또는 PowerShell을 통해 설정할 수 있습니다. 보호 제거를 설정 하는 경우 보존 기간이 지날 때까지 삭제 된 상태의 자격 증명 모음 또는 개체를 제거할 수 없습니다. 일시 삭제 된 자격 증명 모음 및 개체는 계속 복구할 수 있으므로 보존 정책이 적용 됩니다. 
...
* 키 활성화 날짜(설정된 경우)는 과거의 날짜와 시간이어야 합니다. 만료 날짜가 설정 되지 않았습니다.
* 키가 *사용* 상태여야 합니다.
* 키에는 보존 기간이 **90 일로** 설정 된 [일시 삭제가](../key-vault/general/soft-delete-overview.md) 있어야 합니다. 이는 필수 키 특성 recoveryLevel "복구 가능"을 암시적으로 설정 합니다. 보존이 < 90 일로 설정 된 경우 recoveryLevel: "CustomizedRecoverable"는 요구 사항이 아니므로 보존 기간을 설정 해야 합니다 .이는 **90 일로** 설정 됩니다.
* 키에 보호 해제가 [설정](../key-vault/general/soft-delete-overview.md#purge-protection)되어 있어야 합니다.
...
[Azure Portal](howto-data-encryption-portal.md) 및 [Azure CLI](howto-data-encryption-cli.md)를 사용 하 여 Azure database for MySQL에 대 한 고객 관리 키를 사용 하 여 데이터 암호화를 설정 하는 방법에 대해 알아봅니다.
