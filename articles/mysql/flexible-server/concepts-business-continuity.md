...
| **프리미엄 파일 공유** | 데이터베이스 파일은 자동 데이터 복구 기능을 사용 하 여 가용성 영역 내에 저장 된 세 개의 복제본 복사본을 사용 하 여 데이터 중복성을 제공 하는 안정적인 내구성이 있는 안정적인 Azure premium 파일 공유에 저장 됩니다. 자세한 내용은 [프리미엄 파일 공유](../../storage/files/storage-how-to-create-file-share.md) 를 참조 하세요. | 가용성 영역 내에 저장 된 데이터 |
