...
또한 복원 작업이 완료된 후 복원 작업 후에 기본값으로 재설정되고 (주 서버에서 복사되지 않는) 두 개의 서버 매개 변수가 있습니다.
*   time_zone - 이 값은 기본값 **SYSTEM** 으로 설정됩니다
*   event_scheduler - event_scheduler는 복원된 서버에서 **OFF** 로 설정됩니다

[서버 매개 변수](howto-server-parameters.md)를 다시 구성하여 주 서버에서 값을 복사하고 복원된 서버에서 설정해야 합니다.
...
1. 포털의 왼쪽 상단 모서리에서 **리소스 만들기** 단추(+)를 선택합니다. **데이터베이스** > **Azure Database for MySQL** 을 차례로 선택합니다.
   ...
   :::image type="content" source="./media/howto-restore-server-portal/1_navigate-to-mysql.png" alt-text="Azure Database for MySQL로 이동합니다.":::
 
2. 새 서버의 구독, 리소스 그룹 및 이름을 입력합니다. 

3. **백업** 을 **데이터 원본** 으로 선택합니다. 이 작업은 지역 중복 백업을 사용하도록 설정된 서버 목록 드롭다운을 로드합니다.
...
5. 복원할 원본 서버를 선택합니다.
   ...
   :::image type="content" source="./media/howto-restore-server-portal/5-select-backup.png" alt-text="백업을 선택합니다.":::

6. 서버는 **vCore** 의 수, **백업 보존 기간**, **백업 중복 옵션**, **엔진 버전**, 그리고 **관리자 자격 증명** 의 값을 기본값으로 표시합니다. **계속** 을 선택합니다. 
...
7. 나머지 양식은 기본 설정으로 작성합니다. **위치** 를 선택할 수 있습니다.
    ...
    위치를 선택한 후 **서버 구성** 을 선택하여 **컴퓨팅 세대**(선택한 지역에서 사용 가능한 경우), **vCore** 의 수, **백업 보존 기간**, 그리고 **백업 중복 옵션** 을 업데이트할 수 있습니다. **가격 책정 계층**(기본, 범용 또는 메모리 최적화) 또는 **스토리지** 크기는 복원하는 동안 변경할 수 없습니다.
