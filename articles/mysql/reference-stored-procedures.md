---
title: 관리 저장 프로시저 - MySQL용 Azure 데이터베이스
description: MySQL용 Azure 데이터베이스에서 데이터 인 복제를 구성하고, 시간대를 설정하고, 쿼리를 죽이는 데 유용한 저장 프로시저를 알아봅니다.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
---
# <a name="azure-database-for-mysql-management-stored-procedures"></a>MySQL 관리 저장 프로시저에 대한 Azure 데이터베이스

MySQL 서버용 Azure 데이터베이스에서 저장 프로시저를 사용하여 MySQL 서버를 관리할 수 있습니다. 여기에는 서버의 연결, 쿼리 관리 및 데이터 입력 복제 설정이 포함됩니다.  
...
MySQL용 Azure 데이터베이스에서 마스터와 복제본 간에 데이터 인 복제를 설정하려면 [데이터 인 복제를 구성하는 방법을](howto-data-in-replication.md)참조하십시오.

## <a name="other-stored-procedures"></a>기타 저장 프로시저

MySQL에 대한 Azure 데이터베이스에서 서버를 관리하는 다음 저장 프로시저를 사용할 수 있습니다.
