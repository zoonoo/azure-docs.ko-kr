---
title: Azure Database for MySQL의 느린 쿼리 로그
description: Azure Database for MySQL에서 사용할 수 있는 느린 쿼리 로그와 다양한 로깅 수준을 사용하도록 설정할 수 있게 해주는 매개 변수를 설명합니다.
author: savjani
ms.author: pariks
ms.service: mysql
ms.topic: conceptual
...
서버에서 [쿼리 저장소](concepts-query-store.md)를 사용하도록 설정하면 느린 쿼리 로그에 “`CALL mysql.az_procedure_collect_wait_stats (900, 30);`”와 같은 쿼리가 로그될 수 있습니다. 이는 쿼리 저장소 기능이 쿼리에 대한 통계를 수집함에 따른 정상적인 동작입니다. 
...
> 오랜 기간 동안 느린 쿼리를 로깅할 계획인 경우 `log_output`을 “None”으로 설정하기를 권합니다. “File”로 설정된 경우 해당 로그는 로컬 서버 스토리지에 기록되고, MySQL 성능에 영향을 줄 수 있습니다. 
...
Azure Database for MySQL에서 느린 쿼리 로그에 액세스하는 두 가지 옵션은 로컬 서버 스토리지와 Azure Monitor 진단 로그입니다. 이는 `log_output` 매개 변수를 사용하여 설정됩니다.

로컬 서버 스토리지의 경우 Azure Portal 또는 Azure CLI를 사용하여 느린 쿼리 로그를 나열하고 다운로드 할 수 있습니다. Azure Portal에서 Azure Portal의 서버로 이동합니다. **모니터링** 머리글 아레에서 **서버 로그** 페이지를 선택합니다. Azure CLI에 대한 자세한 내용은 [Azure CLI를 사용한 느린 쿼리 로그 구성 및 액세스](howto-configure-server-logs-in-cli.md)를 참조하세요. 
...
Azure Database for MySQL은 Azure Monitor 진단 로그와 통합됩니다. MySQL 서버에서 느린 쿼리 로그를 사용하도록 설정하고 나면 Azure Monitor 로그, Event Hubs 또는 Azure Storage로 로그를 내보내도록 선택할 수 있습니다. 진단 로그를 사용하도록 설정하는 방법에 대한 자세한 내용은 [진단 로그 설명서](../azure-monitor/essentials/platform-logs-overview.md)의 방법 섹션을 참조하세요.
...
- [Azure Portal에서 느린 쿼리 로그를 구성하는 방법](howto-configure-server-logs-in-portal.md)
