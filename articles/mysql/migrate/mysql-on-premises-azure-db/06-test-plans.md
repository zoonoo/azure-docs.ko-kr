---
title: MySQL 온-프레미스에서 Azure Database for MySQL로의 마이그레이션 가이드 Test Plans
...
ms.locfileid: "112082953"
---
# <a name="mysql-on-premises-to-azure-database-for-mysql-migration-guide-test-plans"></a>MySQL 온-프레미스에서 Azure Database for MySQL로의 마이그레이션 가이드 Test Plans
...
테스트:
  ...
  - 마이그레이션된 데이터베이스가 온-프레미스 테이블과 일치하는지 확인합니다(동일한 레코드 수 및 쿼리 결과).
...
WWI는 태평양 표준시로 오후 10시에 시작되고 오전 2시에 종료되는 마이그레이션 주말 및 기간을 확인했습니다. 마이그레이션이 모든 테스트를 통과한 상태로 오전 2시 목표(4시간 가동 시간 목표) 전에 완료되지 않은 경우 롤백 절차가 시작되었습니다. 이러한 문제는 후속 마이그레이션 시도를 위해 문서화되었습니다. 모든 마이그레이션 기간은 적절한 비즈니스 타임라인에 따라 앞으로 당겨지고 다시 예약되었습니다.
...
다음 쿼리를 사용하여 다른 모든 데이터베이스 개체가 고려되는지 확인합니다. 이러한 쿼리는 테이블 행 수를 계산할 수 있지만 특정 데이터베이스 개체의 버전을 고려하지 않을 수 있습니다. 예를 들어 저장 프로시저가 있어도 마이그레이션의 시작과 종료 중간에 변경될 수 있습니다. 마이그레이션 중에 데이터베이스 개체 개발을 중지하는 것이 좋습니다.
