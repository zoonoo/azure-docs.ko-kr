...
**log_output** "파일"로 구성된 경우 서버의 로컬 저장소에서 직접 로그에 액세스할 수 있습니다. 서버에 사용할 수 있는 느린 쿼리 로그 파일을 나열하려면 [az mysql 서버 로그 목록 명령을 실행합니다.](/cli/azure/mysql/server-logs#az-mysql-server-logs-list)
...
**log_output** "파일"로 구성된 경우 [az mysql 서버](/cli/azure/mysql/server-logs#az-mysql-server-logs-download) 로그 다운로드 명령을 사용하여 서버에서 개별 로그 파일을 다운로드할 수 있습니다.
