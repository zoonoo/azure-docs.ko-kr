...
| `query_digest_truncated` | bit| YES| 쿼리 텍스트가 잘린 지 여부를 나타냅니다. 쿼리가 1kb 보다 길면 값은 예입니다.|
...
| `sum_timer_wait` | double| YES| 이 쿼리의 전체 실행 시간 (간격)|
| `avg_timer_wait` | double| YES| 이 쿼리에 대 한 평균 실행 시간 (간격)|
...
- 대기 통계는 샘플을 사용 하 여 이벤트의 비율을 캡처합니다. 매개 변수 `query_store_wait_sampling_frequency`를 사용 하 여 빈도를 수정할 수 있습니다.
