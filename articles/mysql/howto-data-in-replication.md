...
2. 동일한 사용자 계정 및 해당 권한 만들기
   ...
   사용자 계정이 원본 서버에서 복제 서버로 복제 되지 않습니다. 사용자에 게 복제 서버에 대 한 액세스 권한을 제공 하려면 새로 만든 Azure Database for MySQL 서버에서 모든 계정 및 해당 권한을 수동으로 만들어야 합니다.
...
다음 단계에서는 데이터 내부 복제를 위해 다른 클라우드 공급자가 호스팅하는 가상 머신 또는 데이터베이스 서비스에서 온-프레미스 호스팅 MySQL 서버를 준비하고 구성합니다. 이 서버는 데이터 내 복제의 "원본"입니다.

1. 계속 하기 전에 [원본 서버 요구 사항을](concepts-data-in-replication.md#requirements) 검토 하십시오.

2. 원본 서버에서 포트 3306에 대 한 인바운드 및 아웃 바운드 트래픽을 모두 허용 하는지 확인 하 **고, DNS** 에 공개적으로 액세스할 수 있거나 FQDN (정규화 된 도메인 이름)이 있는지 확인 합니다.
   ...
   1. MySQL 명령줄 등의 도구를 사용 하 여 Azure Database for MySQL에 로그인 합니다.
   ...
   4. Ping 유틸리티에서 다음 명령을 실행 하 여 IP 주소를 가져옵니다.
...
5. 새 복제 역할 만들기 및 권한 설정
   ...
   다음 명령에서 새로 만든 복제 역할은 원본 자체를 호스트 하는 컴퓨터 뿐 아니라 모든 컴퓨터에서 원본에 액세스할 수 있습니다. 이 작업은 create user 명령에 “syncuser@’%’”를 지정하여 수행합니다. [계정 이름 지정](https://dev.mysql.com/doc/refman/5.7/en/account-names.html)에 대한 자세한 내용은 MySQL 설명서를 참조하세요.
   ...
   *SSL 없이 복제*
   ...
   모든 연결에 SSL이 필요 하지 않은 경우 다음 명령을 사용 하 여 사용자를 만듭니다.
   ...
   **MySQL Workbench**
   ...
   MySQL 워크 벤치에서 복제 역할을 만들려면 **관리** 패널에서 **사용자 및 권한** 패널을 연 다음 **계정 추가** 를 선택 합니다.
   ...
   **관리 역할** 패널을 선택한 다음 **글로벌 권한** 목록에서 **복제 슬레이브** 를 선택 합니다. 그런 다음, **적용** 을 선택 하 여 복제 역할을 만듭니다.

   :::image type="content" source="./media/howto-data-in-replication/replicationslave.png" alt-text="복제 슬레이브":::

6. 원본 서버를 읽기 전용 모드로 설정
...
7. 이진 로그 파일 이름 및 오프셋 가져오기
   ...
   결과는 다음과 유사 하 게 표시 됩니다. 이후 단계에서 사용 되므로 이진 파일 이름을 기록해 두어야 합니다.
...
1. Azure Database for MySQL에 복제 하려는 데이터베이스 및 테이블을 결정 하 고 원본 서버에서 덤프를 수행 합니다.
    ...
    mysqldump를 사용하여 마스터 서버에서 데이터베이스를 덤프할 수 있습니다. 자세한 내용은 [덤프 및 복원](concepts-migrate-dump-restore.md)을 참조하세요. MySQL 라이브러리 및 테스트 라이브러리를 덤프 하는 것은 필요 하지 않습니다.
...
1. 원본 서버를 설정 합니다.
   ...
   - master_ssl_ca: CA 인증서의 컨텍스트입니다. SSL을 사용하지 않는 경우 빈 문자열을 전달합니다.
     ...
     이 매개 변수를 변수로 전달 하는 것이 좋습니다. 자세한 내용은 다음 예를 참조하세요.
...
4. 복제 상태를 확인 합니다.
   ...
   및의 상태가 `Slave_IO_Running` `Slave_SQL_Running` "yes"이 고 값 `Seconds_Behind_Master` 이 "0" 이면 복제가 정상적으로 작동 합니다. `Seconds_Behind_Master`는 복제본이 얼마나 지연되었는지를 나타냅니다. 값이 "0"이 아니면 복제본이 업데이트를 처리 하 고 있음을 의미 합니다.
...
복제 오류를 건너뛰고 복제를 계속 하려면 다음 저장 프로시저를 사용 합니다.
