---
title: 백업 및 복원 - MySQL용 Azure 데이터베이스
...
MySQL용 Azure 데이터베이스는 데이터 파일및 트랜잭션 로그의 백업을 수행합니다. 지원되는 최대 저장소 크기에 따라 전체 및 차등 백업(최대 4TB 저장소 서버) 또는 스냅샷 백업(최대 16TB 최대 저장소 서버)을 사용합니다. 이러한 백업을 사용하면 서버를 구성된 백업 보존 기간 내의 특정 시점으로 복원할 수 있습니다. 기본 백업 보존 기간은 7일입니다. 선택적으로 최대 35일까지 [구성할](howto-restore-server-portal.md#set-backup-configuration) 수 있습니다. 모든 백업은 AES 256비트 암호화를 사용하여 암호화됩니다.

이러한 백업 파일은 사용자가 노출하지 않으며 내보낼 수 없습니다. 이러한 백업은 MySQL용 Azure 데이터베이스의 복원 작업에만 사용할 수 있습니다. [mysqldump을](concepts-migrate-dump-restore.md) 사용하여 데이터베이스를 복사할 수 있습니다.
...
MySQL용 Azure 데이터베이스에서 복원을 수행하면 원래 서버의 백업에서 새 서버가 생성되고 서버에 포함된 모든 데이터베이스가 복원됩니다.

사용할 수 있는 두 가지 유형의 복원이 있습니다.

- **시점 복원은** 백업 중복 옵션에서 사용할 수 있으며 전체 및 트랜잭션 로그 백업의 조합을 사용하여 원래 서버와 동일한 지역에 새 서버를 만듭니다.
- **지리적 복원은** 서버를 지리적 중복 저장소로 구성한 경우에만 사용할 수 있으며 가장 최근의 백업을 활용하여 서버를 다른 지역으로 복원할 수 있습니다.
...
지역 중복 백업을 위해 서버를 구성한 경우 서비스를 사용할 수 있는 다른 Azure 지역으로 서버를 복원할 수 있습니다. 최대 4TB의 저장소를 지원하는 서버는 지리적 으로 쌍을 이루는 지역 또는 최대 16TB의 저장소를 지원하는 모든 지역으로 복원할 수 있습니다. 최대 16TB의 저장소를 지원하는 서버의 경우 16TB 서버를 지원하는 모든 지역에서 지리적 백업을 복원할 수 있습니다. 지원되는 지역 목록에 [대한 MySQL 가격 책정 계층에](concepts-pricing-tiers.md) 대한 Azure 데이터베이스 검토

지역 복원은 서버가 호스팅되는 지역에 사고가 발생하여 서버를 사용할 수 없는 경우에 대비한 기본 복구 옵션입니다. 지역에서 발생한 대규모 사고로 인해 데이터베이스 애플리케이션을 사용할 수 없는 경우 지역 중복 백업에서 다른 지역에 있는 서버로 서버를 복원할 수 있습니다. 지리적 복원은 서버의 최신 백업을 활용합니다. 백업을 수행할 때와 다른 지역으로 복제할 때 사이에 지연이 있습니다. 재해가 발생한 경우 최대 1시간 동안의 데이터가 손실되므로 이 지연은 최대 1시간일 수 있습니다.
...
- 사용자가 연결할 수 있도록 적절한 VNet 규칙이 마련되어 있는지 확인합니다. 이러한 규칙은 원래 서버에서 복사되지 않습니다.
...
- Azure 포털을 사용하여 한 시점까지 복원하려면 [Azure 포털을 사용하여 서버를 한 시점으로 복원하는 것을 참조하세요.](howto-restore-server-portal.md)
- Azure CLI를 사용하여 한 시점으로 복원하려면 [CLI를 사용하여 서버를 한 시점으로 복원을 참조하세요.](howto-restore-server-cli.md)
