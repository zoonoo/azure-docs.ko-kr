...
`az extension add --name rdbms-vnet` 명령을 사용하여 Azure Database for MySQL VNet 서비스 엔드포인트에 대한 CLI 확장을 설치합니다. 
```azurecli-interactive
...
az extension list
```
명령의 출력은 설치된 모든 확장을 나열합니다. Azure Database for MySQL CLI 확장은 다음과 같습니다.
