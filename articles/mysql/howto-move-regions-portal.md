...
기존 Azure Database for MySQL 서버를 한 지역에서 다른 지역으로 이동 하는 다양 한 시나리오가 있습니다. 예를 들어 재해 복구 계획의 일부로 프로덕션 서버를 다른 지역으로 이동 하는 것이 좋습니다.

Azure Database for MySQL [지역 간 읽기 복제본](concepts-read-replicas.md#cross-region-replication) 을 사용 하 여 다른 지역으로 이동 하는 것을 완료할 수 있습니다. 이렇게 하려면 먼저 대상 지역에서 읽기 복제본을 만듭니다. 그런 다음 읽기 복제본 서버에 대 한 복제를 중지 하 여 읽기 및 쓰기 트래픽을 모두 수락 하는 독립 실행형 서버로 만듭니다. 
...
- Azure Database for MySQL 원본 서버가 이동 하려는 Azure 지역에 있는지 확인 합니다.
...
1. 원본 서버로 사용 하려는 기존 Azure Database for MySQL 서버를 선택 합니다. 이 작업은 **개요** 페이지를 엽니다.
1. 메뉴의 **설정** 아래에서 **복제**를 선택합니다.
1. **복제본 추가**를 선택합니다.
1. 복제본 서버의 이름을 입력합니다.
1. 복제본 서버의 위치를 선택합니다. 기본 위치는 마스터 서버의 위치와 같습니다. 복제본을 배포할 대상 위치를 선택 했는지 확인 합니다.
1. **확인**을 선택하여 복제본 만들기를 확인합니다. 복제본을 만드는 동안 원본 서버에서 복제본으로 데이터가 복사 됩니다. 생성 시간은 원본 서버의 크기에 비례하여 몇 분 이상 지속 될 수 있습니다.

>[!NOTE]
> 복제본을 만들 때 마스터 서버의 VNet 서비스 끝점은 상속 되지 않습니다. 이러한 규칙은 복제본에 대해 별도로 설정해야 합니다.
...
1. 복제본을 만든 후 Azure Database for MySQL 원본 서버를 찾아 선택 합니다. 
1. 메뉴의 **설정** 아래에서 **복제**를 선택합니다.
1. 복제 서버를 선택 합니다.
1. **복제 중지**를 선택합니다.
...
원본 Azure Database for MySQL 서버를 삭제 하는 것이 좋습니다. 이렇게 하려면 다음 단계를 수행합니다.
...
이 자습서에서는 Azure Portal를 사용 하 여 Azure Database for MySQL 서버를 한 지역에서 다른 지역으로 이동한 다음 불필요 한 소스 리소스를 정리 했습니다. 
