...
3. **가상 컴퓨터 이미지 선택** 대화 상자에서 **JDK 7 Windows Server 2012**를 선택합니다. **JDK 6 Windows Server 2012**는 JDK 7에서 실행할 준비가 되지 않은 레거시 응용 프로그램이 있으면 사용 가능합니다.
4. **다음**을 클릭합니다.
...
5. **인바운드 규칙**을 선택한 후 **새 규칙**을 클릭합니다.![새 인바운드 규칙][NewIBRule]
6. **규칙 유형**의 경우 **포트**를 선택한 후 **다음**을 클릭합니다.![새 인바운드 규칙 포트][NewRulePort]
7. **프로토콜 및 포트** 화면에서 **TCP**를 선택하고 **특정 로컬 포트**를 **8080**으로 지정한 후 **다음**을 클릭합니다. ![새 인바운드 규칙][NewRuleProtocol]
8. **작업** 화면에서 **연결 허용**을 선택한 후 **다음**을 클릭합니다.![새 인바운드 규칙 동작][NewRuleAction]
9. **프로필** 화면에서 **도메인**, **개인** 및 **공용**이 선택되어 있는지 확인한 후 **다음**을 클릭합니다.![새 인바운드 규칙 프로필][NewRuleProfile]
10. **이름** 화면에서 규칙의 이름(예: **HttpIn**)(규칙 이름이 끝점 이름과 일치할 필요는 없음)을 지정한 후 **마침**을 클릭합니다.![새 인바운드 규칙 이름][NewRuleName]
...
## 응용 프로그램 수명 주기 고려 사항
* 고유한 WAR(웹 응용 프로그램 보관 파일)을 만들어 **webapps** 폴더에 추가할 수 있습니다. 예를 들어, 기본 JSP(Java 서비스 페이지) 동적 웹 프로젝트를 만들어 WAR 파일로 내보내고, 가상 컴퓨터의 Apache Tomcat **webapps** 폴더로 WAR 파일을 복사한 후 브라우저에서 실행합니다.
...
## 다음 단계
Java 응용 프로그램에 포함시킬만한 다른 서비스(예: Azure 저장소, 서비스 버스, SQL 데이터베이스)에 대한 정보는 [Java 개발자 센터](http://azure.microsoft.com/develop/java/)에 제공되는 정보를 참고하세요.
