<properties
 pageTitle="Azure에서 HPC 팩 클러스터에 작업 제출 | Microsoft Azure"
...
* **HPC 팩 설치 미디어** - [Microsoft Download Center](http://go.microsoft.com/fwlink/?LinkId=328024)에서 최신 버전의 HPC 팩(HPC 팩 2012 R2)용 무료 설치 패키지를 사용할 수 있습니다. 헤드 노드 VM에 설치된 HPC 팩과 동일한 버전의 HPC 팩을 선택해야 합니다.

* **클라이언트 컴퓨터** - HPC 팩 클라이언트 유틸리티를 실행할 수 있는 Windows 또는 Windows Server 클라이언트 컴퓨터가 필요합니다([시스템 요구 사항](https://technet.microsoft.com/library/dn535781.aspx) 참조). HPC 팩 웹 포털 또는 REST API를 사용하여 작업을 제출하려는 경우 사용자가 선택한 클라이언트 컴퓨터를 사용할 수 있습니다.
...
>[AZURE.TIP][HPC 팩 IaaS 배포 스크립트](virtual-machines-hpcpack-cluster-powershell-script.md)와 같은 자동화된 방법을 사용하여 클러스터를 만드는 경우 웹 구성 요소를 배포의 일부로 설치 및 구성할 수 있습니다.
...
4. 인증서를 선택하라는 메시지가 표시되면 헤드 노드의 공용 DNS 이름에 해당하는 인증서를 선택합니다(CN=&lt;*HeadNodeDnsName*&gt;.cloudapp.net).
...
[Microsoft Download Center](http://go.microsoft.com/fwlink/?LinkId=328024)에서 클라이언트 컴퓨터로 HPC 팩 설치 파일의 호환 버전을 다운로드하고(이미 다운로드하지 않은 경우) HPC 팩 클라이언트 유틸리티의 설치 옵션을 선택합니다.
...
3. [1단계: 헤드 노드에 웹 구성 요소 설치 및 구성](#step-1:-install-and-configure-the-web-components-on-the-head-node)에서 HPC 팩 웹 구성 요소에 구성한 인증서를 찾습니다(예: 명명된 &lt;*HeadNodeDnsName*&gt;.cloudapp.net).

4. 인증서를 마우스 오른쪽 단추로 클릭하고 **모든 작업**을 선택한 후 **내보내기**를 클릭합니다.

5. 인증서 내보내기 마법사에서 **다음**을 클릭하고 **예, 개인 키를 내보냅니다.**를 선택합니다.
...
3. 인증서 관리자에서 **인증서 - 현재 사용자**를 확장하고 **신뢰할 수 있는 루트 인증 기관**을 확장합니다. 그런 다음 마우스 오른쪽 단추로 **인증서**를 클릭하고 **모든 작업**을 클릭한 다음 **가져오기**를 클릭합니다.

4. 인증서 가져오기 마법사에서 **다음**을 클릭하고 헤드 노드에서 내보낸 인증서를 가져오는 단계를 따릅니다.
...
구성을 확인하려면 HPC 팩 클라이언트 유틸리티를 실행하고 있는 온-프레미스 컴퓨터를 사용하여 Azure의 클러스터에 작업을 실행해 봅니다. 예를 들어 HPC 팩 GUI 도구 또는 명령줄 명령을 사용하여 클러스터로 작업을 제출할 수 있습니다. 웹 기반 포털을 사용하여 작업을 제출할 수도 있습니다.


**클라이언트 컴퓨터에서 작업 제출 명령을 실행하려면**


1. 클라이언트 컴퓨터에서 명령 프롬프트를 시작합니다.

2. 샘플 명령을 입력합니다. 예를 들어 클러스터의 모든 작업을 나열하려면 다음을 입력합니다.
...
3. 메시지가 표시되면 구성되어 있는 HPC 클러스터 관리자 또는 다른 클러스터 사용자의 사용자 이름(&lt;DomainName&gt;&lt;UserName&gt; 형식)과 암호를 입력합니다. 추가 작업에 대해 자격 증명을 저장할 수도 있습니다.
...
1. 작업을 제출할 때 클라이언트 컴퓨터에 클러스터 사용자의 도메인 자격 증명을 저장하지 않은 경우 자격 증명 관리자에서 자격 증명을 추가할 수 있습니다.
    ...
    c. 인터넷 주소 https://&lt;*HeadNodeDnsName*&gt;.cloudapp.net/HpcScheduler를 지정하고 구성되어 있는 HPC 클러스터 관리자 또는 다른 클러스터 사용자의 사용자 이름(&lt;DomainName&gt;&lt;UserName&gt; 형식)과 암호를 입력합니다.

2. 클라이언트 컴퓨터에서 HPC 작업 관리자를 시작합니다.

3. **헤드 노드 선택** 대화 상자에서 Azure의 헤드 노드에 https://&lt;*HeadNodeDnsName*&gt;.cloudapp.net의 형식으로 URL을 입력합니다.

    HPC 작업 관리자가 열리고 헤드 노드의 작업 목록이 표시됩니다.

**헤드 노드에서 웹 기반 작업 포털을 사용하려면**

1. 클라이언트 컴퓨터에서 웹 브라우저를 시작하고 주소로 ```
    https://HeadNodeDnsName.cloudapp.net/HpcPortal
    ```을 입력합니다.
2. 보안 대화 상자가 나타나면 HPC 클러스터 관리자의 도메인 자격 증명을 입력합니다. (다른 역할의 다른 클러스터 사용자를 추가할 수도 있습니다. 자세한 내용은 [클러스터 사용자 관리](https://technet.microsoft.com/library/ff919335.aspx)를 참조하세요.)
...
* Linux 클라이언트에서 작업을 제출하려는 경우 [HPC 팩 SDK](https://www.microsoft.com/download/details.aspx?id=47756)의 Python 샘플을 참조하세요.
