---
title: "VM에서 Oracle GoldenGate 구성 | Microsoft Docs"
...
---
# <a name="configuring-oracle-goldengate-for-azure"></a>Azure에 대한 Oracle GoldenGate 구성
...
* Windows Server에서 Oracle Enterprise Edition을 사용하여 Azure에서 두 VM(가상 컴퓨터)을 만들었습니다. 영구적인 개인 IP 주소를 통해 서로 액세스할 수 있도록 하려면 [동일한 클라우드 서비스](virtual-machines-linux-load-balance.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) 및 동일한 [가상 네트워크](https://azure.microsoft.com/documentation/services/virtual-network/)에 가상 컴퓨터가 있어야 합니다.
...
> [!IMPORTANT]
> 이 자습서에서는 다음 소프트웨어 구성에 대해 설정 및 테스트되었습니다.
> 
...
## <a name="1-setup-database-on-site-a-and-site-b"></a>1. 사이트 A와 사이트 B에서 데이터베이스를 설치 합니다.
...
다음으로 %ORACLE\_HOME%\\database 폴더에 있는 INIT\<DatabaseSID\>.ORA 파일을 찾아서 INITTEST.ora에 다음과 같은 데이터베이스 매개 변수를 추가합니다.
...
### <a name="perform-initial-data-load"></a>초기 데이터 로드를 수행 합니다.
...
## <a name="2-prepare-site-a-and-site-b-for-database-replication"></a>2. 사이트 A와 사이트 B의 데이터베이스 복제를 준비합니다.
...
## <a name="3-create-all-necessary-objects-to-support-ddl-replication"></a>3. DDL 복제를 지원 하기 위해 필요한 모든 개체를 만듭니다.
이 섹션에는 DDL복제를 지원하는 모든 필요한 개체를 만들기 위해 필요한 스크립트를 나열 합니다. 사이트 A와 사이트 B  모두에 이 섹션에 지정된 스크립트를 실행 해야 합니다.

Windows 명령 프롬프트를 열고 C:\\OracleGG와 같은 Oracle GoldenGate 폴더로 이동 합니다. SQL\*Plus 명령 프롬프트를 **SYSDBA**와 같은 데이터 베이스 관리자 권한을 사용하여 사이트 A와 사이트 B에서 시작합니다.
...
## <a name="4-configure-goldengate-manager-on-site-a-and-site-b"></a>4. 사이트 A와 사이트 B에서 GoldenGate 관리자를 구성합니다.
...
## <a name="5-create-extract-group-and-data-pump-processes-on-site-a-and-site-b"></a>5. 사이트 A와 사이트 B에서 추출 그룹 및 데이터 펌프 프로세스를 생성합니다.
### <a name="create-extract-and-data-pump-processes-on-site-a"></a>사이트 A에서 추출 및 데이터 펌프 프로세스를 만듭니다.
...
### <a name="create-a-goldengate-checkpoint-table-on-site-b"></a>사이트 B에서 GoldenGate 검사점 테이블을 만듭니다.
...
### <a name="add-replicat-on-site-b"></a>사이트 B에서 REPLICAT을 추가합니다.
...
### <a name="create-extract-and-data-pump-processes-on-site-b"></a>사이트 B에서 추출 및 데이터 펌프 프로세스를 만듭니다.
...
### <a name="create-a-goldengate-checkpoint-table-on-site-a"></a>사이트 A에서 GoldenGate 검사점 테이블을 만듭니다.
...
### <a name="add-replicat-on-site-a"></a>사이트 A에서 REPLICAT을 추가합니다.
...
### <a name="add-trandata-on-site-a-and-site-b"></a>사이트 A와 사이트 B에서 trandata를 추가합니다.
...
### <a name="start-extract-and-data-pump-processes-on-site-a"></a>사이트 A에서 추출 및 데이터 펌프 프로세스를 시작 합니다.
...
### <a name="start-extract-and-data-pump-processes-on-site-b"></a>사이트 B에서 추출 및 데이터 펌프 프로세스를 시작 합니다.
...
### <a name="start-replicat-process-on-site-a"></a>사이트 A에서 REPLICAT 프로세스를 시작합니다.
...
### <a name="start-replicat-process-on-site-b"></a>사이트 B에서 REPLICAT 프로세스를 시작합니다.
...
## <a name="6-verify-the-bi-directional-replication-process"></a>6. 양방향 복제 프로세스를 확인 합니다.
