...
1. 구성된 관리자 계정(**\DomainAdmin**) 및 암호(**Contoso!0000**)로 로그인합니다.
...
1. 주 도메인 컨트롤러(**ad-primary-dc**)에 대해 RDP 파일을 시작하고 구성된 관리자 계정(**BUILTIN\DomainAdmin**) 및 암호(**Contoso!0000**)를 사용하여 VM에 로그인합니다.
...
1.  **Active Directory 관리 센터** select **corp(로컬)** 을 선택합니다.
...
1. **CORP\Install** 사용자를 입력한 후 **확인**을 클릭합니다. 자격 증명에 대한 메시지가 표시되면 **DomainAdmin** 계정과 **Contoso!0000** 암호를 사용합니다.
...
1. **할 및 기능 추가 마법사**에서 **기능** 페이지가 표시될 때까지 **다음**을 클릭합니다.
...
1. **서버 역할** 페이지에서 **sysadmin**을 선택합니다. 그런 다음 **확인**을 클릭합니다. 로그인이 생성되면 **로그인** in **로그인**을 참조하세요.
...
1. **백업** 유형에서 **트랜잭션 로그**를 선택합니다. **대상** 파일 경로를 앞에서 지정한 경로로 설정된 상태를 유지하고 **확인**을 클릭합니다. 백업 작업이 완료되면 **확인** 을 한 번 더 클릭합니다.
...
1. **IP 주소**에서 **고정 IP 주소**를 클릭하고 Azure Portal에서 **sqlLB** 부하 분산 장치에 사용된 주소와 동일한 주소로 고정 IP 주소를 설정합니다. 이 동일한 IP 주소를 PowerShell 스크립트에서 `$ILBIP` 변수에 사용합니다. 
