...
Azure Resource Manager에서 가상 컴퓨터에 대한 지원을 발표한 이후 거의 1년이 지났습니다. 여기에서는 Azure Resource Manager의 개선 사항 및 추가 기능에 대해 자세히 설명합니다. 또한 가상 네트워크 사이트-사이트 게이트웨이를 사용하여 구독에서 두 가지 배포 모델의 리소스를 연결하고 사용하는 최적의 방법에 대해서도 설명합니다. 이 문서에서는 Microsoft가 어떻게 클래식에서 Resource Manager로 IaaS(Infrastructure as a Service) 리소스를 마이그레이션할 수 있도록 지원하는지 설명합니다.

## 마이그레이션 목표

새 릴리스 모델을 사용하여 리소스 그룹에서 관련된 서비스를 배포, 관리 및 모니터링할 수 있습니다. Resource Manager는 템플릿을 사용하여 복잡한 응용 프로그램을 배포할 수 있도록 지원하며 VM 확장을 사용하여 가상 컴퓨터를 구성하고 액세스 관리와 태깅을 통합합니다. 또한 가용성 집합에 가상 컴퓨터에 대해 확장성 있는 병렬 배포를 포함합니다. 뿐만 아니라 새로운 모델에서는 계산, 네트워크, 저장소의 수명 주기를 독립적으로 관리할 수 있으며 마지막으로 가상 네트워크에 가상 컴퓨터를 적용하여 보안 구현을 기본적으로 중요시합니다.

클래식 배포 모델의 거의 모든 기능이 Azure Resource Manager의 계산, 네트워크 및 저장소에 대해 지원됩니다. Azure Resource Manager에서 이와 같은 새로운 기능을 제공하고 배포 기반이 확대됨에 따라 고객이 클래식 배포 모델에서 기존 배포를 마이그레이션할 수 있도록 지원하고자 합니다.

>[AZURE.NOTE] 마이그레이션 서비스의 공개 미리 보기 중에는 Azure 구독에서 비프로덕션 워크로드만 마이그레이션하는 것이 좋습니다.

## 마이그레이션 후 자동화 및 도구 변경

클래식 모델에서 Resource Manager 모델로 리소스를 마이그레이션하는 과정의 일부로, 기존 자동화 또는 도구를 업데이트하여 마이그레이션 후에도 계속 작동되도록 해야 합니다.

## 클래식에서 Resource Manager로 IaaS 리소스를 마이그레이션하는 작업의 의미
...
>[AZURE.NOTE] 일부 마이그레이션 시나리오에서는 가상 컴퓨터를 중지, 할당 취소 및 다시 시작합니다. 이 경우 데이터 평면 가동이 잠시 중지됩니다.

## 지원되는 마이그레이션 범위

공개 미리 보기 중에는 기본적으로 계산과 네트워크를 대상으로 하는 두 가지 마이그레이션 범위를 제공합니다. 원활한 마이그레이션을 위해 클래식 저장소 계정에 리소스 관리자 VM용 디스크를 포함할 수 있도록 했습니다.

### 가상 컴퓨터 마이그레이션(가상 네트워크가 아님)

Resource Manager 배포 모델에서는 기본적으로 응용 프로그램 보안이 적용되어 있습니다. 모든 VM은 Resource Manager 모델의 가상 네트워크에 있어야 합니다. 따라서 마이그레이션 중에 VM을 다시 시작합니다(`Stop`, `Deallocate`, `Start`). 가상 네트워크에는 다음과 같은 두 가지 옵션이 있습니다.
...
이 범위에서는 대부분의 VM 구성에 대해 클래식 배포 모델과 Resource Manager 배포 모델 간에 메타데이터만 마이그레이션합니다. 기본 VM은 동일한 네트워크의 동일한 하드웨어에서 동일한 저장소로 실행됩니다. 따라서 클래식에서 Resource Manager로 메타데이터를 마이그레이션할 경우에는 마이그레이션 중 특정 시간 동안 관리 평면 작업이 허용되지 않을 수 있습니다. 그러나 데이터 평면은 계속 작동합니다. 즉, 마이그레이션 중 VM(클래식) 상에서 실행되는 응용 프로그램에 가동 중지 시간이 발생하지 않습니다.

현재는 다음 구성이 지원되지 않습니다. 향후 이에 대한 지원을 추가할 경우 이 구성의 일부 VM에서 가동 중지 시간(VM 작업 중지, 할당 취소, 다시 시작 진행)이 발생할 수 있습니다.
...
저장소 계정 마이그레이션은 이 공개 미리 보기에 지원되지 않습니다.

원활한 마이그레이션을 위해 클래식 저장소 계정에 Resource Manager VM을 배포하는 기능을 제공하고 있습니다. 이 기능을 사용하면 계산 및 네트워크 리소스를 저장소 계정과 상관없이 마이그레이션할 수 있으며 이러한 방식이 바람직합니다.
...
다음 기능은 공개 미리 보기에 지원되지 않습니다. 이러한 설정을 제거하고 VM을 마이그레이션한 다음 Resource Manager 배포 모델에서 설정을 다시 사용하도록 지정할 수 있습니다(선택 사항).
...
- 마이그레이션하려는 리소스가 지원되지 않는 기능이나 구성을 사용하지 않도록 확인합니다. 대부분의 경우에는 플랫폼이 이러한 문제를 감지하고 오류를 throw합니다.
...
- 지금은 프로덕션 리소스를 마이그레이션하지 않도록 합니다.
...
마이그레이션 워크플로는 다음과 같습니다. Microsoft는 공개 미리 보기를 발표하면서 REST API, PowerShell, Azure CLI를 통한 마이그레이션 트리거 지원을 추가했습니다.

![마이그레이션 워크플로를 보여주는 스크린 샷](./media/virtual-machines-windows-migration-classic-resource-manager/migration-workflow.png)

>[AZURE.NOTE] 다음 섹션에서 설명하는 모든 작업은 idempotent 상태입니다. 지원되지 않는 기능 또는 구성 오류 이외의 문제가 발생하는 경우 준비, 중단 또는 커밋 작업을 다시 시도하는 것이 좋습니다. 그러면 플랫폼에서 작업을 다시 시도합니다.

### 준비

준비 작업은 마이그레이션 프로세스의 첫 번째 단계입니다. 이 단계의 목표는 클래식에서 Resource Manager 리소스로의 IaaS 리소스 전환을 시뮬레이션하고 사용자가 볼 수 있도록 나란히 표시하는 것입니다.
...
* 리소스를 마이그레이션할 수 없을 경우 플랫폼에 마이그레이션이 지원되지 않는 이유가 나열됩니다.
...
문제가 발생할 경우 언제든지 마이그레이션을 중단하고 클래식 배포 모델로 돌아갈 수 있습니다. 그럴 경우 클래식 배포 모델에서 해당 VM에서 정상 작업을 재개할 수 있도록 리소스에 대한 관리 평면 작업이 시작됩니다.

### 중단

중단 단계에서는 변경 사항을 클래식 배포 모델로 되돌리고 마이그레이션을 중지할 수 있습니다(선택 사항). 커밋 작업을 트리거하면 이 작업을 실행할 수 없습니다.
...
이 작업이 실패할 경우 이 작업을 몇 번 더 시도하는 것이 좋습니다. 계속 실패할 경우 지원 티켓을 만들거나 [VM 포럼](https://social.msdn.microsoft.com/Forums/azure/ko-KR/home?forum=WAVirtualMachinesforWindows)에서 ClassicIaaSMigration 태그를 사용하여 포럼 게시물을 작성할 수 있습니다.
...
Microsoft는 기존 클래식 API와 리소스 모델을 중단할 계획이 없으며 보다 편리한 마이그레이션을 위해 Resource Manager 배포 모델에서 사용할 수 있는 고급 기능을 고려하고 있습니다. 리소스 관리자의 IaaS로 구현된 [몇 가지 개선 사항](virtual-machines-windows-compare-deployment-models.md)을 살펴볼 것을 권장합니다.
...
마이그레이션하는 리소스의 수에 따라 달라집니다. 소규모 배포(몇 십 대의 VM)의 경우 전체 마이그레이션은 1시간 미만이 소요됩니다. 대규모 배포(수백 대의 VM)의 경우 마이그레이션에 몇 시간이 걸릴 수 있습니다. 서비스가 공개 미리 보기에 제공되므로 개발 또는 테스트 구독에서 실행하여 영향을 평가해 보는 것이 좋습니다.
...
최근 리소스 관리자에서 VM의 Azure Site Recovery 및 백업 지원이 추가되었습니다. Microsoft는 Resource Manager로 VM을 마이그레이션하는 기능을 지원하기 위해 노력하고 있습니다. 현재 해당 기능을 사용하고 있는 경우에는 마이그레이션을 수행하지 않는 것이 좋습니다.

**내 구독 또는 리소스에서 마이그레이션이 가능한지 확인할 수 있나요?**

현재 준비 작업은 마이그레이션을 준비 중인 리소스에 대한 유효성 검사를 의미하지 않습니다. 플랫폼 지원 마이그레이션 옵션에서 마이그레이션을 준비하는 첫 번째 단계는 리소스를 마이그레이션할 수 있는지 여부를 확인하는 것입니다. 유효성 검사가 실패할 경우 리소스에 대해 아무 작업도 실행되지 않습니다.
