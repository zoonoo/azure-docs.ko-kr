...
## Azure 가상 컴퓨터
Azure 가상 컴퓨터를 사용하면 다양한 컴퓨팅 솔루션을 민첩하게 배포할 수 있습니다. 이미지 갤러리에서 Windows 또는 Linux 가상 컴퓨터 또는 나날이 다양해지는 파트너의 목록 중 하나에서 사용자 지정으로 만든 가상 컴퓨터를 배포합니다. 거의 모든 운영 체제에서 워크로드 및 언어를 배포할 수 있습니다. 그래도 원하는 내용이 표시되지 않나요? 걱정하지 마세요. 온-프레미스에서 고유한 이미지를 가져올 수 있습니다.
...
4. SLA(서비스 수준 계약)를 충족하는 방법을 포함하여 Linux 및 Azure에 대한 자세한 내용입니다. **법적 문서를 싫어하는 경우라도 이 문서를 참고합니다**.
