...
Microsoft HPC 팩에서는 MPI 응용 프로그램을 포함한 다양한 대규모 HPC 및 병렬 응용 프로그램을 Microsoft Azure 가상 컴퓨터의 클러스터에서 실행하는 기능을 제공합니다. Microsoft HPC 팩 2012 R2부터는 HPC 팩에서 HPC 팩 클러스터에 배포된 Linux 계산 노드 VM에서 Linux HPC 응용 프로그램의 실행도 지원합니다. HPC 팩으로 Linux 계산 노드 사용에 대한 소개는 [Azure에서 HPC 팩 클러스터의 Linux 계산 노드 시작](virtual-machines-linux-cluster-hpcpack.md)을 참조하세요.


## 필수 조건

* **Linux 계산 노드가 포함된 HPC 팩 클러스터** - Azure 마켓플레이스에서 Azure PowerShell 스크립트 및 HPC 팩 이미지를 사용하여 Azure에서 Linux 계산 노드와 함께 HPC 팩 클러스터를 배포하는 필수 조건 및 단계는 [Azure에서 HPC 팩 클러스터의 Linux 계산 노드 시작](virtual-machines-linux-cluster-hpcpack.md)을 참조하세요.
...
4.	On the **작업 세부 정보** 페이지의 **작업 리소스** 아래에서 리소스 유형을 **노드**로 선택하고 **최소**를 3으로 설정합니다. 이 예에서는 Linux 노드 3개에 작업을 실행하며 각 노드는 코어 4개를 포함합니다.
