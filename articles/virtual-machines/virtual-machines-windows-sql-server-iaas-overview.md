...
### 데이터 백업
Blob 저장소를 사용하여 Azure에서 직접 데이터베이스 백업을 저장합니다. 자세한 내용은 [SQL Server 백업 및 복원에 Azure 저장소 사용](../sql-database/storage-use-storage-sql-server-backup-restore.md)을 참조하세요. SQL VM에 대해 잘 작동되지만 온-프레미스 SQL Server 데이터베이스에 대해서도 작동합니다. 백업 및 복원 옵션에 대한 개요는 [Azure 가상 컴퓨터에서 SQL Server 백업 및 복원](virtual-machines-windows-sql-backup-recovery.md)을 참조하세요.

또한 Azure VM에서는 SQL Server에 대한 [자동화된 백업](virtual-machines-windows-sql-automated-backup.md) 및 [자동화된 패치 적용](virtual-machines-windows-sql-automated-patching.md)을 활용할 수 있습니다.
