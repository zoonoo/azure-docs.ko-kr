---
title: 유지 관리 알림
description: Azure에서 실행 중인 가상 시스템에 대한 유지 관리 알림 개요입니다.
author: shants123
...
Azure에서는 가상 머신에 대한 호스트 인프라의 안정성, 성능 및 보안을 향상시키기 위해 주기적으로 업데이트를 수행합니다. 업데이트란 호스팅 환경의 패치 적용 또는 하드웨어의 업그레이드 및 서비스 해제와 같은 변경 내용을 말합니다. 이러한 업데이트의 대부분은 호스팅된 가상 시스템에 영향을 주지 않고 완료됩니다. 그러나 업데이트가 다음 항목에 영향을 미치는 경우가 있습니다.

- 유지 관리를 다시 부팅하지 않아도 되는 경우 Azure에서는 호스트를 업데이트하는 동안 바로 마이그레이션을 사용하여 VM을 일시 중지합니다. 이러한 형식의 유지 관리 작업은 오류 도메인에 의해 오류 도메인에 적용됩니다. 경고 상태 신호를 받으면 진행이 중지됩니다.

- 유지 관리를 다시 부팅해야 하는 경우 유지 관리가 계획된 시기에 대해 알림을 받을 수 있습니다. 당신은 당신이 당신을 위해 작동 할 때, 당신이 유지 보수를 직접 시작할 수있는 약 35 일의 시간 기간이 주어집니다.


다시 부팅해야 하는 계획된 유지 관리는 웨이브에서 예약됩니다. 각 웨이브는 범위(지역)이 다릅니다.

- 웨이브는 고객에게 알림을 보내면서 시작합니다. 기본적으로 알림은 서비스 관리자 및 공동 관리자에게 전송됩니다. [활동 로그 경고를](../service-health/alerts-activity-log-service-notifications.md)사용하여 이메일, SMS 및 웹후크와 같은 받는 사람 및 메시징 옵션을 더 추가할 수 있습니다.  
- 알림이 꺼지면 *셀프 서비스 창을* 사용할 수 있습니다. 이 기간 동안 영향을 받는 가상 컴퓨터를 쿼리하고 사용자 고유의 일정 요구에 따라 유지 관리를 시작할 수 있습니다. 셀프 서비스 기간은 일반적으로 약 35일입니다.
...
**가용성 집합을**사용하는 배포에는 셀프 서비스 유지 관리를 사용하지 않는 것이 좋습니다. 가용성 집합은 이미 한 번에 하나의 업데이트 도메인만 업데이트됩니다. 

- Azure에서 유지 관리를 트리거할 수 있습니다. 재부팅이 필요한 유지 관리를 위해 업데이트 도메인에 의해 업데이트 도메인이 수행됩니다. 업데이트 도메인이 반드시 유지 관리를 순차적으로 수신할 필요는 없으며 업데이트 도메인 간에 30분 동안 일시 중지됩니다. 
- 일부 용량(1 업데이트 도메인)의 일시적인 손실이 우려되는 경우 유지 관리 기간 동안 인스턴스를 추가할 수 있습니다. 
...
- 두 업데이트 도메인(UD) 간에 VM 복구 시간이 30분 이상 필요합니다. 업데이트 도메인 사이의 시간을 제어하려면 한 번에 하나의 UD(업데이트 도메인)에서 VM 유지 관리를 트리거해야 합니다.
...
**Q: 가용성 집합을 사용하여 고가용성에 대한 권장 사항을 따르는 경우 안전한가요?**
...
고가용성에 대한 자세한 내용은 Azure 의 [가상 컴퓨터 가용성을](./linux/availability.md)참조하십시오.

**Q: 계획된 유지 관리에 관한 알림은 어떻게 받나요?**

**A:** 계획된 유지 관리 주기는 하나 이상의 Azure 지역에 예약을 설정하는 것에서 출발합니다. 곧 구독 관리자에게 이메일 알림이 전송됩니다(구독당 하나의 이메일). 이 알림에 대한 추가 채널과 받는 사람은 활동 로그 경고를 통해 구성할 수 있습니다. 계획된 유지 관리가 이미 예약된 지역에 가상 머신을 배포하는 경우 알림이 전달되지 않으므로 VM의 유지 관리 상태를 확인해야 합니다.

**Q: 포털, Powershell 또는 CLI에서 계획된 유지 관리의 표시가 표시되지 않습니다. 무슨 문제 있나요?**
...
**A:** 이제 가상 시스템 스케일 집합에 대해 계획된 유지 관리를 사용할 수 있습니다. 셀프 서비스 유지 관리를 시작하는 방법에 대한 지침은 가상 시스템 규모 집합 문서에 [대한 계획된 유지 관리를](../virtual-machine-scale-sets/virtual-machine-scale-sets-maintenance-notifications.md) 참조하십시오.
...
[Azure CLI,](maintenance-notifications-cli.md) [Azure PowerShell](maintenance-notifications-powershell.md) 또는 [포털을](maintenance-notifications-portal.md)사용하여 계획된 유지 관리를 처리할 수 있습니다.
