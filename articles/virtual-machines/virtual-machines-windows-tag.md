...
이 문서에서는 Azure Resource Manager를 통해 Azure의 Windows 가상 컴퓨터에 태그를 지정하는 다양한 방법에 대해 설명합니다. 태그는 리소스 또는 리소스 그룹에 직접 배치할 수 있는 사용자 정의 키/값 쌍입니다. Azure는 현재 리소스 및 리소스 그룹당 최대 15개의 태그를 지원합니다. 태그를 만들 때 리소스에 배치하거나 기존 리소스에 추가할 수 있습니다. 태그는 Azure Resource Manager를 통해 생성된 리소스에 대해서만 지원됩니다. Linux 가상 컴퓨터에 태그를 지정하려는 경우 [Azure에서 Linux 가상 컴퓨터에 태그를 지정하는 방법](virtual-machines-linux-tag.md)을 참조하세요.
