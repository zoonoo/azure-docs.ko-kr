---
title: "Windows VM(가상 컴퓨터)에서 SAP NetWeaver - 배포 가이드 | Microsoft Docs"
description: "Windows VM(가상 컴퓨터)에서 SAP NetWeaver - 배포 가이드"
...
---
# <a name="sap-netweaver-on-windows-virtual-machines-vms--deployment-guide"></a>Windows VM(가상 컴퓨터)에서 SAP NetWeaver - 배포 가이드
...
* [Windows VM(가상 컴퓨터)에서 SAP NetWeaver - 배포 가이드(이 문서)][deployment-guide]
...
위의 템플릿을 사용하려는 경우 OS 디스크에 VM 에이전트가 이미 설치되어 있어야 합니다. 그렇지 않으면 배포에 실패합니다. 이 문서의 [Azure VM 에이전트 다운로드, 설치 및 사용][deployment-guide-4.4] 챕터에 설명된 대로 VM에 VM 에이전트를 다운로드하고 설치합니다.
...
###### <a name="issue"></a>문제
Windows 서비스 'AzureEnhancedMonitoring'이 존재하지 않습니다. Azperflib.exe: azperlib.exe 출력 시 [아래 그림][deployment-guide-figure-14]와 같이 오류가 발생합니다.
