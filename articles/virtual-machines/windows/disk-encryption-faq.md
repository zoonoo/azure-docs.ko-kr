---
title: FAQ-Windows Vm에 대 한 Azure Disk Encryption
...
- [지원 되지 않는 Windows 시나리오](disk-encryption-windows.md#unsupported-scenarios)에서 호출 되는 시나리오를 사용 하는 경우 [고객 관리 키를 사용 하 여 서버 쪽 암호화](disk-encryption.md)를 고려 합니다. 
...
Azure Disk Encryption는 다음과 같이 Windows 버전에 따라 BitLocker의 암호화 방법을 선택 합니다.

| Windows 버전                 | 버전 | 암호화 방법        |
|----------------------------------|--------|--------------------------|
| Windows Server 2012, Windows 10 이상  | >= 1511 |XTS-AES 256 비트           |
| Windows Server 2012, Windows 8, 8.1, 10 | < 1511 |AES 256 비트 *              |
| Windows Server 2008R2            |        |AES 256 비트 (디퓨저 포함) |
