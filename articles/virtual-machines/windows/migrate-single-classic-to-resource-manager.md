...
Managed Disks를 사용할 수 있는 위치를 선택합니다. Premium Storage에서 지원하는 Managed Disks로 마이그레이션하는 경우에도 해당 지역에서 Premium Storage를 사용할 수 있는지 확인합니다. 사용 가능한 위치에 대한 최신 정보는 [지역별 Azure 서비스](https://azure.microsoft.com/regions/#services)를 참조하세요.

### <a name="vm-sizes"></a>VM 크기

Premium Storage를 사용하는 Managed Disks로 마이그레이션하는 경우 VM 크기를 VM이 위치한 지역에서 제공하는 Premium Storage 지원 가능 크기로 업데이트해야 합니다. Premium Storage를 사용할 수 있는 VM 크기를 검토합니다. Azure VM 크기 사양은 [가상 머신의 크기](sizes.md)에 나열되어 있습니다.
...
VM에서 사용할 수 있는 Premium Storage에는 7가지 유형이 있으며 각각에는 특정 IOP 및 처리량 한도가 있습니다. 용량, 성능, 확장성 및 최대 로드 측면에서 애플리케이션의 필요에 따라 VM에 대한 프리미엄 디스크 유형을 선택할 때 이 제한을 고려합니다.
...
VM에서 사용할 수 있는 표준 디스크에는 7가지 유형이 있습니다. 각각은 용량이 다르지만 동일한 IOPS 및 처리량이 제한됩니다. 애플리케이션의 용량 요구 사항에 따라 표준 Managed Disks의 종류를 선택합니다.
...
[Managed Disks에 대한 가격 책정](https://azure.microsoft.com/pricing/details/managed-disks/)을 검토합니다. 프리미엄 Managed Disks의 가격은 프리미엄 관리되지 않는 디스크와 같습니다. 하지만 표준 Managed Disks의 가격은 표준 관리되지 않는 디스크와 다릅니다.
...
클래식 VM에서 VHD를 사용하여 관리되는 OS 디스크를 만듭니다. $osVhdUri 매개 변수에 OS VHD의 전체 URI를 제공했는지 확인합니다. 또한 마이그레이션하는 디스크의 종류(프리미엄 또는 표준)에 따라 **-AccountType**을 **Premium_LRS** 또는 **Standard_LRS**로 입력합니다.
...
데이터 VHD 파일에서 관리되는 데이터 디스크를 만들고 새 VM에 추가합니다.
...
공용 IP, 가상 네트워크 및 NIC를 설정하여 새 VM을 만듭니다.
