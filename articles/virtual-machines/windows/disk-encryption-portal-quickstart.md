...
Azure Portal을 통해 Azure VM(가상 머신)을 만들 수 있습니다. Azure Portal은 VM 및 관련 리소스를 만드는 브라우저 기반 사용자 인터페이스를 제공합니다. 이 빠른 시작에서는 Azure Portal을 사용하여 Ubuntu 18.04 LTS를 실행하는 Windows VM(가상 머신)을 배포하고, 암호화 키 스토리지용 키 자격 증명 모음을 만든 다음, VM을 암호화합니다.
...
1. **Key Vault** 화면에서 새 Key Vault에 대한 고유한 이름을 선택합니다.

    > [!Important]
    > 각 Key Vault마다 고유한 이름이 있어야 합니다. 다음 예제에서는 이름이 *myADEKV*인 Key Vault를 만들지만 사용자는 자신만의 이름을 지정해야 합니다.

1. **구독**을 선택합니다.
1.  **리소스 그룹**에서 **새로 만들기**를 선택합니다. 팝업에서 리소스 그룹의 이름에 *myResourceGroup*을 입력한 다음, **확인**을 선택합니다. 

    ![리소스 그룹 만들기 화면](../media/disk-encryption/portal-qs-keyvaultcreation.png)

1. **위치** 풀 다운 메뉴에서 **미국 동부**를 선택합니다.
1. 다른 옵션은 기본값으로 그대로 둡니다.
1. 새 화면으로 이동하는 “액세스 정책”을 선택합니다.
1. “볼륨 암호화를 위해 Azure Disk Encryption에 대한 액세스 사용” 옆에 있는 확인란을 선택합니다.

    ![리소스 그룹 만들기 화면](../media/disk-encryption/portal-qs-keyvault-enable-encryption.png)

1. 액세스 정책 화면 아래쪽에서 “검토 + 만들기”를 클릭합니다.
1. 검토한 후 “만들기”를 클릭합니다.
...
1. **관리자 계정**에서 **암호**를 선택합니다. 사용자 이름 및 암호를 입력합니다.
    ![리소스 그룹 만들기 화면](../media/disk-encryption/portal-qs-windows-vm-creation.png)
