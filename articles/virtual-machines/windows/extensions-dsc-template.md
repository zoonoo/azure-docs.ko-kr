...
description: "Azure에서 Desired State Configuration 확장에 대한 Resource Manager 템플릿 정의"
...
이 문서에서는 [Desired State Configuration 확장 처리기](extensions-dsc-overview.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)를 위한 Azure Resource Manager 템플릿에 대해 설명합니다. 

*참고: 약간 다른 스키마 예가 있을 수 있습니다. *
*2016년 10월에 스키마가 변경되었습니다. *
*자세한 내용은 이 페이지의*
*[이전 형식에서 업데이트](##Updating-from-the-Previous-Format)* 섹션에 나와 있습니다.

## <a name="template-example-for-a-windows-vm"></a>Windows VM의 템플릿 예제

다음 코드 조각은 템플릿의 리소스 섹션에 대한 코드입니다.
DSC 확장은 [VirtualMachineExtension 클래스](https://docs.microsoft.com/en-us/dotnet/api/microsoft.azure.management.compute.models.virtualmachineextension?view=azure-dotnet.)에서 설명한 대로 기본 확장 속성을 상속합니다.
...
DSC 확장은 [VirtualMachineScaleSetExtension 클래스](https://docs.microsoft.com/en-us/dotnet/api/microsoft.azure.management.compute.models.virtualmachinescalesetextension?view=azure-dotnet)에서 설명한 대로 기본 확장 속성을 상속합니다.
...
*기본 구성 스크립트에서 사용할 수 있는 인수 목록은*
*아래의*
*[기본 구성 스크립트](##Default-Configuration-Script)* 섹션을 참조하세요.
...
| settings.configuration.url |string |DSC 구성 zip 파일을 다운로드할 URL 위치를 지정합니다. 제공된 URL에서 액세스를 위해 SAS 토큰을 요구하는 경우 protectedSettings.configurationUrlSasToken 속성을 SAS 토큰 값으로 설정해야 합니다. settings.configuration.script 및/또는 settings.configuration.function이 정의된 경우 이 속성이 필요합니다. 이러한 속성에 대한 값이 없는 경우 확장은 기본 구성 스크립트를 호출하여 제공해야 할 LCM 메타데이터 및 인수를 설정합니다. |
| settings.configuration.script |string |DSC 구성의 정의를 포함하는 스크립트의 파일 이름을 지정합니다. 이 스크립트는 configuration.url 속성에 지정된 URL에서 다운로드된 zip 파일의 루트 폴더에 있어야 합니다. settings.configuration.url 및/또는 settings.configuration.script가 정의된 경우 이 속성이 필요합니다. 이러한 속성에 대한 값이 없는 경우 확장은 기본 구성 스크립트를 호출하여 적용해야 할 LCM 메타데이터 및 인수를 설정합니다. |
| settings.configuration.function |string |DSC 구성의 이름을 지정합니다. 명명된 구성은 configuration.script에 정의된 스크립트에 있어야 합니다. settings.configuration.url 및/또는 settings.configuration.function이 정의된 경우 이 속성이 필요합니다. 이러한 속성에 대한 값이 없는 경우 확장은 기본 구성 스크립트를 호출하여 제공해야 할 LCM 메타데이터 및 인수를 설정합니다. |
| settings.configurationArguments |컬렉션 |DSC 구성을 전달하려는 매개 변수를 정의합니다. 이 속성은 암호화되지 않습니다. |
| settings.configurationData.url |string |DSC 구성에 대한 입력으로 사용할 구성 데이터(.psd1) 파일을 다운로드할 URL을 지정합니다. 제공된 URL에서 액세스를 위해 SAS 토큰을 요구하는 경우 protectedSettings.configurationDataUrlSasToken 속성을 SAS 토큰 값으로 설정해야 합니다. |
...
이러한 값에 대한 자세한 내용은 설명서 페이지 [로컬 구성 관리자 기본 설정](https://docs.microsoft.com/en-us/powershell/dsc/metaconfig#basic-settings)을 참조하세요.
아래 표의 LCM 속성만 DSC 확장 기본 구성 스크립트를 사용하여 구성할 수 있습니다.

| 속성 이름 | 형식 | 설명 |
| --- | --- | --- |
| settings.configurationArguments.RegistrationKey |securestring |필수 속성입니다. 노드에서 Azure Automation 서비스에 등록하는 데 PowerShell 자격 증명 개체의 암호로 사용되는 키를 지정합니다. 이 값은 Automation 계정에 대해 listkeys 메서드를 사용하여 자동으로 검색할 수 있으며 보호된 설정으로 보호해야 합니다. |
| settings.configurationArguments.RegistrationUrl |string |필수 속성입니다. 노드가 등록하려는 Azure Automation 끝점의 Url을 지정합니다. 이 값은 Automation 계정에 대해 참조 메서드를 사용하여 자동으로 검색할 수 있습니다. |
| settings.configurationArguments.NodeConfigurationName |string |필수 속성입니다. 노드를 할당할 Azure Automation 계정의 노드 구성을 지정합니다. |
| settings.configurationArguments.ConfigurationMode |string |로컬 구성 관리자에 대한 모드를 지정합니다. 유효한 옵션은 "ApplyOnly", "ApplyandMonitor", 및 "ApplyandAutoCorrect"입니다.  기본값은 "ApplyandMonitor"입니다. |
...
| settings.configurationArguments.RebootNodeIfNeeded | 부울 | DSC 작업에서 요청할 경우 노드를 자동으로 다시 부팅할 수 있는지 여부를 지정합니다.  기본값은 False입니다. |
...
| settings.configurationArguments.AllowModuleOverwrite | 부울 | LCM이 노드의 기존 모듈을 덮어쓸지 여부를 지정합니다.  기본값은 False입니다. |
...
다음 예는 DSC 확장의 기본 동작으로, 로컬 구성 관리자에 메타데이터 설정을 제공하고 Azure Automation DSC 서비스에 등록하는 것입니다.
구성 인수는 필수이며 LCM 메타데이터를 설정하기 위해 기본 구성 스크립트에 전달됩니다.
...
## <a name="example-using-configuration-script-in-azure-storage"></a>Azure Storage의 구성 스크립트 사용 예
...
[Azure DSC 확장에 Virtual Machine Scale Sets 사용](../../virtual-machine-scale-sets/virtual-machine-scale-sets-dsc.md)에서 DSC 및 가상 머신 확장 집합에 대해 알아봅니다.
