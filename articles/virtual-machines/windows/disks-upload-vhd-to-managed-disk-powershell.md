---
title: Azure PowerShell을 사용하여 Azure에 vhd 업로드
...
---
# <a name="upload-a-vhd-to-azure-using-azure-powershell"></a>Azure PowerShell을 사용하여 Azure에 vhd 업로드
...
Azure에서 IaaS VM에 대한 백업 솔루션을 제공하는 경우 직접 업로드를 사용하여 관리되는 디스크에 대한 고객 백업을 복원하는 것이 좋습니다. Azure 외부에 있는 컴퓨터에서 VHD를 업로드하는 경우 로컬 대역폭에 따라 속도가 달라집니다. Azure VM을 사용하는 경우 대역폭은 표준 HDD와 동일합니다.
...
업로드를 위한 빈 표준 HDD를 만들기 전에 업로드하려는 vhd의 바이트로 파일 크기가 필요합니다. 예제 코드는 당신을 위해 그것을 얻을 것 이다 하지만, 그것을 직접 사용할 수 있습니다. `$vhdSizeBytes = (Get-Item "<fullFilePathHere>").length` 이 값은 **-UploadSizeInBytes** 매개 변수를 지정할 때 사용됩니다.
