...
1. [Azure Portal](https://portal.azure.com/) 로 이동 하 여 VM에 연결 합니다. **가상 머신**을 검색하여 선택합니다.
2. 목록에서 가상 머신을 선택합니다.
...
5. 다운로드한 RDP 파일을 열고 메시지가 표시되면 **연결**을 선택합니다. `.rdp` 파일은 알 수 없는 게시자가 제공한 파일이라는 경고가 표시됩니다. 이는 예상된 결과입니다. **원격 데스크톱 연결** 창에서 **연결**을 선택하여 계속합니다.
