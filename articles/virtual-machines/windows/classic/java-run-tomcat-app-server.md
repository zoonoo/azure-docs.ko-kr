...
이 가이드를 완료하면 Azure에서 실행되는 가상 컴퓨터를 만들고 Java 응용 프로그램 서버에서 실행하도록 구성하는 방법을 이해할 수 있게 됩니다. 다음 작업을 배우고 수행합니다.

* Java 개발 키트(JDK)가 이미 설치된 가상 컴퓨터를 만드는 방법
* 가상 컴퓨터에 원격으로 로그인하는 방법
* 가상 컴퓨터에 Java 응용 프로그램 서버(Apache Tomcat)를 설치하는 방법
* 가상 컴퓨터의 끝점을 만드는 방법
* 방화벽에서 응용 프로그램 서버용 포트를 여는 방법

설치를 완료하면 Tomcat이 가상 컴퓨터에서 실행됩니다.
...
5. **기본 사항** 블레이드에서 다음을 수행합니다.
   1. 가상 컴퓨터의 이름을 지정합니다.
   2. **사용자 이름** 필드에 관리자의 이름을 입력합니다. 이 이름과 다음 필드에 있는 연결된 암호를 기억해 두세요. 가상 컴퓨터에 원격으로 로그인할 때 필요합니다.
...
5. **Connect**를 클릭합니다.
6. 가상 컴퓨터에 연결해야 한다는 메시지에 응답합니다. 일반적으로 연결 세부 정보가 포함된 .rdp 파일을 열거나 저장해야 합니다. .rdp 파일의 첫 줄 끝 부분으로 url:port를 복사한 다음 원격 로그인 응용 프로그램에 붙여 넣어야 할 수 있습니다.
...
이 자습서에서는 설치할 Java 응용 프로그램 서버로 Tomcat을 사용합니다.

1. 가상 컴퓨터에 로그인하는 경우 [Apache Tomcat](http://tomcat.apache.org/download-80.cgi)에 대한 브라우저 세션을 엽니다.
...
가상 컴퓨터에서 명령 프롬프트를 열고 **net&nbsp;start&nbsp;Tomcat8** 명령을 실행하여 Tomcat을 수동으로 시작할 수 있습니다.

Tomcat이 실행되면 가상 컴퓨터의 브라우저에서 URL <http://localhost:8080/>을 입력하여 Tomcat에 액세스할 수 있습니다.
...
이제 외부 브라우저에서 Tomcat 웹 사이트를 볼 수 있어야 합니다. 브라우저의 주소 창에서 폼의 URL을 입력  **http://*프로그램\_DNS\_이름*. cloudapp.net**, 여기서 ***프로그램\_DNS\_이름*** 가상 컴퓨터를 만들 때 지정 된 DNS 이름입니다.
...
Azure Storage, Service Bus, SQL Database 등 Java 응용 프로그램에 포함할 기타 서비스에 대해 자세히 알아볼 수 있습니다. [Java 개발자 센터](https://azure.microsoft.com/develop/java/)에서 제공되는 정보를 확인하세요.
