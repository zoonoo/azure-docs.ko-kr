...
Azure Backup 서비스에서 백업 작업을 시작하면 백업 확장을 트리거하여 특정 시점 스냅샷을 만듭니다. Azure Backup 서비스는 [VMSnapshot](../extensions/vmsnapshot-windows.md) 확장을 사용합니다. VM을 실행하는 경우 확장은 첫 번째 VM 백업 중에 설치됩니다. VM이 실행되고 있지 않을 경우 Backup 서비스가 기본 스토리지의 스냅샷을 생성합니다(VM이 중지되었을 때는 애플리케이션 쓰기가 수행되지 않음).
...
1. 목록에서 백업할 VM을 선택합니다.
1. VM 블레이드의 **작업** 섹션에서 **백업**을 클릭합니다. **백업 사용** 블레이드가 열립니다.
1. **Recovery Services 자격 증명 모음**에서 **새로 만들기**를 클릭하고 새 자격 증명 모음의 이름을 제공합니다. 새 자격 증명 모음이 가상 머신과 동일한 리소스 그룹과 위치에 만들어집니다.
1. **백업 정책 선택**에서 기본값인 **(신규) DailyPolicy**를 그대로 유지한 다음, **백업 사용**을 클릭합니다.
1. 초기 복구 지점을 만들려면 **Backup** 블레이드에서 **지금 Backup**을 클릭합니다.
1. **지금 백업** 블레이드에서 달력 모양 아이콘을 클릭하고, 달력 컨트롤을 사용하여 복원 지점을 유지할 기간을 선택하고, **확인**을 클릭합니다.
1. VM의 **백업** 블레이드에 완료된 복원 지점의 수가 표시됩니다.
...
1. 로컬 컴퓨터에서 브라우저를 새로 고쳐서 기본 IIS 페이지에 대한 이미지가 사라졌는지 확인합니다.

    ![기본 IIS 웹 페이지](./media/tutorial-backup-vms/iis-broken.png)

1. 로컬 컴퓨터에서 새 탭을 열고 [Azure Portal](https://portal.azure.com)로 이동합니다.
1. 왼쪽 메뉴에서 **가상 머신**을 선택하고 목록에서 VM 형식을 선택합니다.
1. VM 블레이드의 **작업** 섹션에서 **백업**을 클릭합니다. **Backup** 블레이드가 열립니다. 
1. 블레이드 위쪽의 메뉴에서 **파일 복구**를 선택합니다. **파일 복구** 블레이드가 열립니다.
1. **1단계: 복구 지점 선택**의 드롭다운에서 복구 지점을 선택합니다.
1. **2단계: 스크립트를 다운로드하여 파일 찾아보기 및 복구**에서 **실행 파일 다운로드** 단추를 클릭합니다. 파일의 암호를 복사하여 안전한 곳에 저장합니다.
1. 로컬 컴퓨터에서 **파일 탐색기**를 열고 **다운로드** 폴더로 이동하여 다운로드한 .exe 파일을 복사합니다. 파일 이름은 VM 이름을 앞에 붙이게 됩니다. 
1. VM에서(RDP 연결을 사용하여) .exe 파일을 VM의 바탕 화면에 붙여넣습니다. 
1. VM의 바탕 화면으로 이동한 다음 .exe를 두 번 클릭합니다. 명령 프롬프트가 시작됩니다. 프로그램에서 복구 지점을 액세스할 수 있는 파일 공유로 탑재합니다. 공유 만들기가 끝나면 **q**를 입력하여 명령 프롬프트를 닫습니다.
