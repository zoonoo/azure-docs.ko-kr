...
이 문서에서는 Azure의 [가상 컴퓨터 SQL Server](virtual-machines-windows-sql-server-iaas-overview.md) 에 대 한 가격 책정 지침을 제공 합니다. 비용에 영향을 미치는 다양한 옵션이 있고 비용과 비즈니스 요구 사항 간에 균형을 이루는 적합한 이미지를 선택해야 합니다.
...
**사용 당 SQL Server 라이선스** ( **종 량**제)를 지불 하는 것은 Azure VM을 실행 하는 초당 비용에 SQL Server 라이선스 비용이 포함 됨을 의미 합니다. [Windows](https://azure.microsoft.com/pricing/details/virtual-machines/windows) 또는 [Linux](https://azure.microsoft.com/pricing/details/virtual-machines/linux)에 대한 Azure VM 가격 페이지에서 다른 SQL Server 버전(Web, Standard, Enterprise)에 대한 가격을 확인할 수 있습니다.

SQL Server의 모든 버전(2012 SP3 ~ 2017)에 대한 비용은 동일합니다. 초당 라이선싱 비용은 VM vCPU 수에 따라 달라집니다.

사용당 SQL Server 라이선스 지급이 권장되는 경우는 다음과 같습니다.

- **임시 또는 정기 작업**입니다. 예: 매주 월요일의 비즈니스 분석 또는 1년에 몇 개월 동안 이벤트를 지원해야 하는 앱.

- **수명이 나 소수 자릿수가 알려지지 않은 작업**입니다. 예: 몇 개월 내에 필요하지 않을 수 있거나 수요에 따라 더 많은 또는 더 적은 컴퓨팅 기능이 필요할 수 있는 앱.

이러한 종 량 제 이미지 중 하나를 사용 하 여 SQL Server 2017 Azure VM을 만들려면 다음 링크를 참조 하세요.
...
> [!NOTE]
> 이제 라이선스 모델을 종량제 모델과 BYOL(사용자 라이선스 필요) 간에 변경할 수 있습니다. 자세한 내용은 [SQL VM에 대 한 라이선스 모델을 변경 하는 방법](virtual-machines-windows-sql-ahb.md)을 참조 하세요. 

## <a name="bring-your-own-license-byol"></a><a id="byol"></a>BYOL (사용자 라이선스 사용)
...
- **수명 및 소수 자릿수가 알려진 작업**입니다. 예: 1년 내내 필요하고 수요가 예측된 앱.

SQL Server VM와 함께 BYOL을 사용 하려면 SQL Server Standard 또는 엔터프라이즈 및 [소프트웨어 보증](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default.aspx#tab=1)에 대 한 라이선스가 있어야 합니다 .이는 일부 볼륨 라이선싱 프로그램을 통해 필수 옵션이 며 다른 사용자와 함께 구매할 수도 있습니다. 볼륨 라이선싱을 통해 제공된 가격 수준은 SQL Server에 대한 계약 유형과 수량 및/또는 약정에 따라 달라집니다. 하지만 일반적으로 연속 프로덕션 작업에 대한 사용자 라이선스 필요에는 다음과 같은 이점이 있습니다.

| BYOL 이점 | Description |
|-----|-----|
| **비용 절감** | [Azure 하이브리드 혜택](https://azure.microsoft.com/pricing/hybrid-benefit/) 는 최대 55% 절감 액을 제공 합니다. 자세한 내용은 [Switch license model](virtual-machines-windows-sql-ahb.md) 항목을 참조 하세요. |
