...
사용량에`PAYG`따라 지불 하려면 종 량 제 ()으로 SQL Server 라이선스 유형을 제공 하 고 Azure 하이브리드 혜택`AHUB`, 사용자 라이선스를 사용 하거나 재해 복구 (`DR`)를 사용 하 여 [무료 DR 복제본 라이선스](virtual-machines-windows-sql-high-availability-dr.md#free-dr-replica-in-azure)를 활성화 합니다.
...
`AHUB` `PAYG`, `DR` 또는을 **sqlLicenseType** `SQL2008-WS2008` 으로 지정 하거나 `SQL2008R2-WS2008` **sqlImageOffer**로 지정 합니다. 

Windows Server 2008 인스턴스에 SQL Server 2008 또는 2008 R2를 등록 하려면 다음 Az CLI 또는 PowerShell 코드 조각을 사용 합니다. 
