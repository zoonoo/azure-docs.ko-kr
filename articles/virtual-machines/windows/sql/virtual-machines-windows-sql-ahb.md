...
라이선스 모델 간에 전환하는 기능은 새 SQL VM 리소스 공급자(Microsoft.SqlVirtualMachine)에서 제공하는 기능입니다. 현재는 라이선스 모델을 전환하려면 먼저 새 공급자를 구독에 등록한 다음, 새 SQL VM 리소스 공급자에 기존 VM을 등록해야 합니다. SQL VM 리소스 공급자를 이용하려면 SQL IaaS 확장도 설치해야 합니다. 이렇게 하면 배포된 VM을 VHD에 등록할 수 있습니다. 자세한 내용은 [SQL IaaS 확장](virtual-machines-windows-sql-server-agent-extension.md)을 참조하세요. 
...
다음 코드 조각은 먼저 새 SQL 리소스 공급자를 구독에 등록한 다음, 새 리소스 공급자에 기존 SQL VM을 등록합니다. 
...
```
  ...
  >[!NOTE]
  > 라이선스 간에 전환하려면 새 SQL VM 리소스 공급자를 사용하고 있어야 합니다. SQL VM을 새 공급자에 등록하기 전에 이러한 명령을 실행하려고 하면 다음 오류가 발생할 수 있습니다. `Get-AzureRmResource : The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. The property 'sqlServerLicenseType' cannot be found on this object. Verify that the property exists and can be set. ` 이 오류가 표시되면 [새 리소스 공급자에 SQL VM을 등록](#register-existing-SQL-vm-with-new-resource-provider)하세요. 
...
```
  ...
  >[!NOTE]
  >라이선스 간에 전환하려면 새 SQL VM 리소스 공급자를 사용하고 있어야 합니다. SQL VM을 새 공급자에 등록하기 전에 이러한 명령을 실행하려고 하면 다음 오류가 발생할 수 있습니다. `The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. ` 이 오류가 표시되면 [새 리소스 공급자에 SQL VM을 등록](#register-existing-SQL-vm-with-new-resource-provider)하세요. 
