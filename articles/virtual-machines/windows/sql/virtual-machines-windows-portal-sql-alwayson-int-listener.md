---
title: 가용성 그룹 & 로드 밸러커(Azure portal) 구성
...
---
# <a name="configure-a-load-balancer-for-an-availability-group-on-azure-sql-server-vms"></a>Azure SQL Server VM의 가용성 그룹에 대한 로드 밸런서 구성
...
* [Azure VM(GUI)의 가용성 그룹에서 항상 구성](virtual-machines-windows-portal-sql-availability-group-tutorial.md)   
...
5. **부하 분산 장치 만들기** 대화 상자에서 다음과 같이 부하 분산 장치를 구성합니다.
   ...
   | **유형** |**내부**: 대부분의 구현에서는 내부 부하 분산 장치를 사용하며, 동일한 가상 네트워크 내에 있는 애플리케이션은 가용성 그룹에 연결할 수 있습니다.  </br> **외부**: 애플리케이션이 공용 인터넷 연결을 통해 가용성 그룹에 연결할 수 있습니다. |
...
3. **프로브 추가** 블레이드에서 프로브를 구성합니다. 다음 값을 사용하여 프로브를 구성합니다.
   ...
   | **프로토콜** |**Tcp** |
   | **Port** |사용 가능한 모든 포트를 사용할 수 있습니다. 예를 들어 *59999*입니다. |
   | **간격** |*5* |
   | **비정상 임계값** |*2* |
...
3. **부하 분산 규칙 추가** 블레이드에서 부하 분산 규칙을 구성합니다. 다음 설정을 사용합니다. 
   ...
   | **프로토콜** |**Tcp** |
   | **Port** |*1433* |
   | **백 엔드 포트** |*1433*. 이 규칙은 **부동 IP (직접 서버 반환)를**사용하기 때문에이 값은 무시됩니다. |
...
2. **AlwaysOn 고가용성** > **그룹** > **가용성 그룹 수신기로**이동합니다.  
...
2. **sqlcmd** 유틸리티를 사용하여 연결을 테스트합니다. 예를 들어 다음 스크립트는 Windows 인증을 사용하여 수신기를 통해 기본 복제본에 대한 **sqlcmd** 연결을 설정합니다.
...
7. 다음 설정을 사용하여 상태 검색을 추가합니다.
   ...
   |**이름** |프로브를 식별하는 이름입니다.
   |**프로토콜** |TCP
   |**Port** |사용하지 않는 TCP 포트를 모든 가상 머신에서 사용할 수 있어야 합니다. 다른 용도로는 사용할 수 없습니다. 두 수신기가 동일한 프로브 포트를 사용할 수 없습니다. 
...
10. 다음 설정을 사용하여 새 부하 분산 규칙을 구성합니다.
    ...
    |**프로토콜** |TCP
    |**Port** |SQL Server 인스턴스에서 사용하는 포트를 사용합니다. 변경하지 않을 경우 기본 인스턴스는 포트 1433을 사용합니다. 
...
2. [클러스터 관리자에서 클라이언트 액세스 포인트를 추가합니다.](#addcap)
...
1. 다음 설정을 사용하여 부하 분산 규칙을 만듭니다.
   ...
   |**프로토콜** |TCP
   |**Port** |5022 - [분산 가용성 그룹 엔드포인트 수신기](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/configure-distributed-availability-groups)의 포트입니다.</br> 사용 가능한 모든 포트일 수 있습니다.  
...
- [다른 지역의 Azure 가상 컴퓨터에서 SQL Server Always On 가용성 그룹 구성](virtual-machines-windows-portal-sql-availability-group-dr.md)
