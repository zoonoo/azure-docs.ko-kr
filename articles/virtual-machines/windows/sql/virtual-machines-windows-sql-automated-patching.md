...
> * [고전적인](../sqlclassic/virtual-machines-windows-classic-sql-automated-patching.md)

자동화된 패치는 SQL Server를 실행하는 Azure Virtual Machine에 대한 유지 관리 기간을 설정합니다. 이 유지 관리 기간 동안만 자동화된 업데이트를 설치할 수 있습니다. SQL Server의 경우 이 제한을 통해 시스템 업데이트 및 관련 재시작 작업이 데이터베이스에 가장 적절한 시간에 수행됩니다. 

> [!IMPORTANT]
> **중요** 또는 **중요로** 표시된 Windows 및 SQL Server 업데이트만 설치됩니다. **중요** 또는 **중요로** 표시되지 않은 서비스 팩 및 누적 업데이트와 같은 다른 SQL Server 업데이트는 수동으로 설치해야 합니다. 
...
SQL **Server 설정** 탭에서 자동 패치 에서 **구성 변경을** **선택합니다.** 다음 Azure 포털 스크린샷은 **SQL 자동화된 패치** 블레이드를 보여 줍니다.
...
기존 SQL Server 가상 시스템의 경우 [SQL 가상 컴퓨터 리소스를](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource) 열고 **설정에서** **패치를** 선택합니다. 
...
다음 예제에서는 PowerShell을 사용하여 기존 SQL Server VM에 대해 자동화된 패치를 구성합니다. **New-AzVMSqlServer자동 패칭구성** 명령은 자동 업데이트를 위한 새 유지 관리 기간을 구성합니다.

    $vmname = "vmname"
    $resourcegroupname = "resourcegroupname"
    $aps = New-AzVMSqlServerAutoPatchingConfig -Enable -DayOfWeek "Thursday" -MaintenanceWindowStartingHour 11 -MaintenanceWindowDuration 120  -PatchCategory "Important"
s 세트 AzVMSqlServerExtension -자동 패치설정 $aps -VMName $vmname -ResourceGroupName $resourcegroupname
...
| **PatchCategory** |이 매개 변수에 대한 유일한 가능한 설정은 **중요**. 이렇게 하면 중요로 표시된 Windows 업데이트가 설치되고, 이 범주에 포함되지 않는 모든 SQL Server 업데이트는 설치되지 않습니다. |

SQL Server IaaS 에이전트를 설치하고 구성하는 데는 몇 분 정도 걸릴 수 있습니다.

자동 패치를 사용하지 않으려면 **New-AzVMSqlServer자동 패칭구성에** **대한 -Enable** 매개 변수 없이 동일한 스크립트를 실행합니다. **-Enable** 매개 변수가 없는 경우 기능을 해제하는 명령을 신호로 보냅니다.
