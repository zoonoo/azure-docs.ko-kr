...
| **리뷰 + 만들기** | [요약 검토](#4-review--create) |

## <a name="1-configure-basic-settings"></a>1. 기본 설정 구성


에 **기본 사항** 탭에서 다음 정보를 제공 합니다.
...
* 아래 **인스턴스 세부 정보**:
    ...
    1. 에 대 한 위치를 선택 하면 **지역**합니다. 
    1. 이 가이드에서는 둡니다 **가용성 옵션** 로 설정 _필수 인프라 중복 되지 않은_합니다. 가용성 옵션에 대 한 자세한 내용을 참조 하세요 [Azure 지역 및 가용성](../../windows/regions-and-availability.md)합니다. 
    1. 에 **이미지** 목록에서 _체험 용 SQL Server 라이선스: Windows Server 2016의 SQL Server 2017 Developer_ 이미지를 선택합니다.  
    1. 하기로 **크기를 변경** 에 대 한 합니다 **크기** 가상 컴퓨터를 선택 합니다 **A2 기본** 제공. 모든 예기치 않은 요금을 방지 하기 위해 완료 한 후에 리소스를 정리 해야 합니다. 프로덕션 워크로드의 경우 [Azure Virtual Machines의 SQL Server에 대한 성능 모범 사례](virtual-machines-windows-sql-performance.md)에서 권장하는 컴퓨터 크기 및 구성을 참조하세요.
...
* 아래 **인바운드 포트 규칙**, 선택 **선택한 포트를 허용** 선택한 후 **RDP (3389)** 드롭다운 목록에서. 
...
| 매개 변수 | 설명 | 예제 |
| --- | --- | --- |
| **주요 자격 증명 URL** |주요 자격 증명 모음의 위치입니다. |https:\//contosokeyvault.vault.azure.net/ |
| **사용자 이름** |Azure Active Directory 서비스 주체 이름. 이 이름을 클라이언트 ID라고도 합니다. |fde2b411-33d5-4e11-af04eb07b669ccf2 |
| **보안 주체 암호** |Azure Active Directory 서비스 주체 암호입니다. 이 암호를 클라이언트 암호라고도 합니다. |9VTJSQwzlFepD8XODnzy8n2V01Jd8dAjwm/azF1XDKM= |
...
에 **검토 + 만들기** 탭, 요약 정보를 검토 및 선택 **만들기** SQL Server, 리소스 그룹 및이 VM에 대해 지정 된 리소스를 만드는 합니다.
