...
로그인에 성공하면 기본 구독의 구독 이름 및 ID를 포함하는 일부 정보가 화면에 표시됩니다. 이는 다른 구독으로 변경하지 않는 경우 이 자습서에 대한 리소스가 만들어지는 구독입니다. 여러 구독이 있는 경우 다음 cmdlet을 실행하여 모든 구독 목록을 반환합니다.

```PowerShell
Get-AzureRmSubscription
```
다른 구독으로 변경하려면 대상 SubscriptionName을 사용하여 다음 명령을 실행합니다.
...
다음 변수를 사용하여 가상 컴퓨터에 사용할 이미지를 정의합니다. 이 예제에서는 SQL Server 2016 Developer Edition 이미지를 사용합니다. Developer Edition은 테스트 및 개발을 위해 무료로 사용이 허가되며 VM을 실행하는 요금만 지불하면 됩니다.
...
Get-AzureRmVMImageSku 명령을 사용하여 제품에 사용 가능한 Sku를 확인할 수 있습니다. 다음 명령을 사용하여 **SQL2016SP1-WS2016** 제품에 사용 가능한 모든 SKU를 확인할 수 있습니다.
