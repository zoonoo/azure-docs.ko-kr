...
1. [볼륨을 만듭니다](https://technet.microsoft.com/windows-server-docs/storage/storage-spaces/hyper-converged-solution-using-storage-spaces-direct#step-36-create-volumes).
   ...
   스토리지 공간 다이렉트 사용 하도록 설정 하면 저장소 풀이 자동으로 만들어집니다. 이제 볼륨을 만들 준비가 되었습니다. PowerShell cmdlet `New-Volume` 은 볼륨 생성 프로세스를 자동화 합니다. 이 프로세스에는 형식을 지정 하 고, 클러스터에 볼륨을 추가 하 고, CSV (클러스터 공유 볼륨)를 만듭니다. 이 예제에서는 800 기가바이트 (GB) CSV를 만듭니다.
