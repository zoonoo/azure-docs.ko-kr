...
- 디스크에 이 기능을 사용하도록 설정하면 비활성화할 수 없습니다.
    이 방법을 해결해야 하는 경우 모든 데이터를 고객 관리 키를 사용하지 않는 완전히 다른 관리 디스크에 [복사해야](disks-upload-vhd-to-managed-disk-powershell.md#copy-a-managed-disk) 합니다.
- 크기 2080의 ["소프트" 및 "하드" RSA 키만](../../key-vault/keys/about-keys.md) 지원되며 다른 키나 크기는 지원되지 않습니다.
...
1. 최신 [Azure PowerShell 버전을](/powershell/azure/install-az-ps)설치하였고 Connect-AzAccount를 사용하여 Azure 계정에 로그인했는지 확인합니다.
