...
- SQL Server
	...
	-	SQL 서버 서비스 계정 이름: sqlservice (기본값)
...
- SharePoint Server
	...
	-	호스트 이름 접두사: 초기 구성 중에 지정됩니다.
	-	크기: A2(기본값)
	-	SharePoint 팜 계정 이름: sp_farm (기본값)
...
-	SQL Server
	-	가상 컴퓨터 이미지: Windows Server 2012 R2의 SQL Server 2014 RTM Enterprise
	-	호스트 이름 접두사: 초기 구성 중에 지정됩니다.
	-	크기: SQL 서버용 A5 (기본값), 파일 공유 감시용 A0 (기본값)
	-	데이터베이스 액세스 계정 이름: 초기 구성 중에 지정됩니다.
	-	데이터베이스 액세스 계정 암호: 초기 구성 중에 지정됩니다.
	-	SQL 서버 서비스 계정 이름: sqlservice (기본값)
	-	SQL 서버 서비스 계정 암호: 초기 구성 중에 지정됩니다.
...
Azure 미리보기 포털의 SharePoint 서버 팜 기능은 서비스 관리에서 가상 컴퓨터를 만듭니다. 리소스 관리자에서, SharePoint 서버2013 팜을 만들려면 [Azure 리소스 관리자 템플릿을 사용하여 SharePoint 팜 배포](virtual-machines-workload-template-sharepoint.md)를 참조하세요.
