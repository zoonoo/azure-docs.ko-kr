...
# Linux 기반 Azure 가상 컴퓨터에 Secure Shell 연결 문제 해결

Linux 기반 Azure 가상 컴퓨터에 연결하려고 할 때 다양한 이유로 인해 SSH(Secure Shell) 오류가 발생할 수 있습니다. 이 문서는 문제를 찾아 해결하는 데 도움이 됩니다.
...
이 섹션에는 일반적인 SSH 연결 문제에 대한 빠른 해결 방법이 나와 있습니다.

### 클래식 배포 모델을 사용하여 만든 가상 컴퓨터 문제 해결

클래식 배포 모델을 사용하여 만든 가상 컴퓨터의 보다 일반적인 SSH 연결 오류를 해결하려면 다음 단계를 시도합니다. 각 단계 후 가상 컴퓨터에 다시 연결을 시도합니다.

- [Azure 포털](https://portal.azure.com)에서 원격 액세스를 다시 설정합니다. Azure 포털에서 **찾아보기** > **가상 컴퓨터(클래식)** > *Linux 가상 컴퓨터* > **원격 액세스 다시 설정**을 클릭합니다.

- 가상 컴퓨터를 다시 시작합니다. [Azure 포털](https://portal.azure.com)에서 **찾아보기** > **가상 컴퓨터(클래식)** > *Linux 가상 컴퓨터* > **다시 시작**을 클릭합니다.

	또는

	[Azure 클래식 포털](https://manage.windowsazure.com)에서 **가상 컴퓨터** > **인스턴스** > **다시 시작**을 선택합니다.

- 새 Azure 노드로 가상 컴퓨터를 다시 배포합니다. 이 작업을 수행하는 방법에 대한 자세한 내용은 [새 Azure 노드로 가상 컴퓨터 다시 배포](virtual-machines-windows-redeploy-to-new-node.md)를 참조하세요.
...
- [Linux 기반 가상 컴퓨터의 암호 또는 SSH를 다시 설정하는 방법](virtual-machines-linux-classic-reset-access.md)의 지침을 따르세요.
	- 암호 또는 SSH 키를 재설정합니다.
	- 새 _sudo_ 사용자 계정을 만듭니다.
	- SSH 구성을 재설정합니다.

- VM 리소스 상태에 플랫폼 문제가 있는지 확인합니다.<br> **찾아보기** > **가상 컴퓨터(클래식)** > *Linux 가상 컴퓨터* > **설정** > **상태 검사**를 선택합니다.


### 리소스 관리자 배포 모델을 사용하여 만든 가상 컴퓨터 문제 해결

Azure Resource Manager 배포 모델을 사용하여 만든 가상 컴퓨터의 일반적인 SSH 문제를 해결하려면 다음 단계를 시도합니다.

#### SSH 연결 다시 설정
Azure CLI를 사용하여 [Microsoft Azure Linux 에이전트](virtual-machines-linux-agent-user-guide.md) 버전 2.0.5 이상이 설치되어 있는지 확인합니다.

아직 설치되지 않은 경우 [Azure CLI를 설치하고 Azure 구독에 연결](../xplat-cli-install.md)합니다. `azure login` 명령을 사용하여 로그인합니다. 리소스 관리자 모드에 있는지 확인합니다. ```
	azure config mode arm
	```

다음 방법 중 하나를 사용하여 SSH 연결을 다시 설정할 수 있습니다.

**방법 1:** 다음 예제와 같이 `vm reset-access` 명령을 사용합니다. 그러면 가상 컴퓨터에 `VMAccessForLinux` 확장이 설치됩니다.
...
**방법 2:** 다음과 같은 내용으로 PrivateConf.json이라는 파일을 만듭니다.
...
**방법 1:** `vm reset-access` 명령을 실행하여 SSH 자격 증명을 설정합니다.
...
그런 다음 방금 설명한 파일 이름(PrivateConf.json)을 사용하여 Linux 확장을 실행합니다.

	```
	$azure vm extension set "testRG" "testVM" VMAccessForLinux Microsoft.OSTCExtensions "1.2" --private-config-path PrivateConf.json
	```

다른 문제 해결 접근 방식을 시도하려는 경우 [Linux 기반 가상 컴퓨터의 암호 또는 SSH를 재설정하는 방법](virtual-machines-linux-classic-reset-access.md)의 접근 방식과 유사한 단계를 따를 수 있습니다. 리소스 관리자 모드에 대한 Azure CLI 지침을 수정해야 합니다.


## 자세한 SSH 오류 문제 해결

SSH 클라이언트가 가상 컴퓨터의 SSH 서비스에 도달할 수 없는 데에는 여러 원인이 있습니다. 다음 다이어그램은 관련된 구성 요소를 보여 줍니다.

![SSH 서비스의 구성 요소를 보여주는 다이어그램](./media/virtual-machines-linux-troubleshoot-ssh-connection/ssh-tshoot1.png)

다음 단계를 참조하여 문제의 발생지를 격리하고 해결 방법을 찾을 수 있습니다.

### 준비 단계 수행

먼저 포털에 있는 가상 컴퓨터의 상태를 확인합니다.

[Azure 클래식 포털](https://manage.windowsazure.com)에서 클래식 배포 모델을 사용하여 만든 가상 컴퓨터의 경우:

1. **가상 컴퓨터** > *VM 이름*을 선택합니다.
2. VM의 **대시보드**를 선택하여 상태를 확인합니다.
3. **모니터**를 선택하여 계산, 저장소 및 네트워크 리소스에 대한 최근 활동을 봅니다.
4. **끝점**을 선택하여 SSH 트래픽의 끝점이 있는지 확인합니다.

[Azure portal](https://portal.azure.com)에서:

1. 클래식 배포 모델을 사용하여 만든 가상 컴퓨터의 경우 **찾아보기** > **가상 컴퓨터(클래식)** > *VM 이름*을 선택합니다.

	또는

	리소스 관리자 모델을 사용하여 만든 가상 컴퓨터의 경우 **찾아보기** > **가상 컴퓨터** > *VM 이름*을 선택합니다.

	가상 컴퓨터에 대한 상태 창에 **실행 중**이 표시됩니다. 아래로 스크롤하여 계산, 저장소 및 네트워크 리소스에 대한 최근 활동을 표시합니다.

2. **설정**을 선택하여 끝점, IP 주소 및 기타 설정을 확인합니다.

	리소스 관리자를 사용하여 만든 가상 컴퓨터에서 끝점을 식별하려면 [네트워크 보안 그룹](../virtual-network/virtual-networks-nsg.md)이 정의되어 있는지 확인합니다. 또한 규칙이 네트워크 보안 그룹에 적용되어 있고 서브넷에서 참조되고 있는지 확인합니다.
...
![SSH 클라이언트 컴퓨터 구성 요소를 강조하는 다이어그램](./media/virtual-machines-linux-troubleshoot-ssh-connection/ssh-tshoot2.png)
...
> [AZURE.NOTE] 이 발생지는 클래식 배포 모델을 사용하여 만든 가상 컴퓨터에만 적용됩니다. 리소스 관리자를 사용하여 만든 가상 컴퓨터의 경우 [발생지 4: 네트워크 보안 그룹](#nsg)으로 건너뜁니다.
...
동일한 가상 네트워크에 다른 VM이 없는 경우 새 가상 컴퓨터를 손쉽게 만들 수 있습니다. 자세한 내용은 [CLI를 사용하여 Azure에서 Linux VM 만들기](virtual-machines-linux-quick-create-cli.md)를 참조하세요. 테스트를 마치면 추가한 VM을 삭제합니다.

동일한 가상 네트워크의 VM에 SSH 연결을 만들 수 있는 경우 다음을 확인합니다.

- **대상 VM의 SSH 트래픽에 대한 끝점 구성.** 끝점의 개인 TCP 포트는 VM에서 SSH 서비스가 수신 대기 중인 TCP 포트와 일치해야 합니다. 기본 포트는 22입니다. 리소스 관리자 배포 모델을 사용하여 만든 VM의 경우 SSH TCP 포트 번호를 확인하려면 Azure 포털에서 **찾아보기** > **가상 컴퓨터(v2)** > *VM 이름* > **설정** > **끝점**으로 이동합니다.
...
문제의 발생지인 끝점을 제거하려면 현재 끝점을 제거하고 새 끝점을 만든 다음 SSH 이름(공용 및 개인 포트 번호에 TCP 포트 22)을 지정합니다. 자세한 내용은 [Azure의 가상 컴퓨터에 끝점 설정](virtual-machines-windows-classic-setup-endpoints.md)을 참조하세요.
...
![Linux 기반 Azure 가상 컴퓨터를 강조하는 다이어그램](./media/virtual-machines-linux-troubleshoot-ssh-connection/ssh-tshoot5.png)

아직 그렇게 하지 않은 경우 가상 컴퓨터에서 [Linux 기반 가상 컴퓨터의 암호 또는 SSH를 다시 설정하는 방법](virtual-machines-linux-classic-reset-access.md)의 지침을 따르세요.
...
- 리소스 관리자 배포 모델을 사용하여 만든 가상 컴퓨터의 문제 해결 방법에 대한 자세한 내용은 다음을 참조하세요.
	- [Windows 기반 Azure 가상 컴퓨터에 대한 Windows 원격 데스크톱 연결 문제 해결](virtual-machines-windows-troubleshoot-rdp-connection.md)
