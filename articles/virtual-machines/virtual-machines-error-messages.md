---
title: "Azure Virtual Machine 오류 메시지"
description: 
...
---
# <a name="azure-virtual-machine-error-messages"></a>Azure Virtual Machine 오류 메시지
이 문서에서는 Azure VM(Virtual Machine)을 관리할 때 발생하는 일반적인 오류 코드 및 메시지를 설명합니다.  

>[!NOTE]
>오류 메시지 피드백은 이 페이지에서 의견을 남겨주시거나 #azerrormessage 태그를 사용하여 [Azure 피드백](https://feedback.azure.com/forums/216843-virtual-machines)을 통해 의견을 남겨주세요.
>
>
...
오류 응답은 상태 코드 및 오류 개체를 항상 포함합니다. 각 오류 개체에는 항상 오류 코드 및 메시지가 포함됩니다. 템플릿을 통해 VM 배포를 수행한 경우 오류 개체는 오류 코드 및 메시지의 내부 수준이 포함된 세부 정보 섹션도 포함합니다. 일반적으로 오류 메시지의 내부 수준은 루트 실패입니다. 


## <a name="common-virtual-machine-management-error"></a>일반적인 가상 컴퓨터 관리 오류

이 섹션에서는 가상 컴퓨터를 관리하는 경우에 일반적인 오류 메시지를 나열합니다.
...
|  AttachDiskWhileBeingDetached  |  현재 디스크가 분리되어 있으므로 데이터 디스크 '{0}'을 VM '{1\}'에 연결할 수 없습니다. 디스크가 완전히 분리될 때까지 잠시 기다렸다가 다시 시도하세요.  |
...
|  ConflictingUserInput  |  디스크 '{0}'은 VM '{1}'에서 이미 소유한 디스크로 연결될 수 없습니다.  |
...
|  DiskBlobAlreadyInUseByAnotherDisk  |  Blob {0}은 VM '{1\}'에 속한 다른 디스크에서 이미 사용 중입니다. 디스크 참조 정보에 대한 Blob 메타데이터를 검사할 수 있습니다.  |
...
|  InternalOperationError  |  저장소 계정 '{0}'을 확인할 수 없습니다. 이 계정이 계산 리소스와 동일한 위치에 있는 저장소 리소스 공급자를 통해 생성되었는지 확인하세요.  |
|  InternalOperationError  |  {0} 목표 검색 태스크에 실패했습니다.  |
|  InternalOperationError  |  VM '{0}'의 네트워크 프로필을 검사하는 동안 오류가 발생했습니다.  |
|  InvalidAccountType  |  AccountType {0}은 올바르지 않습니다.  |
|  InvalidParameter  |  매개 변수 {0}의 값이 잘못되었습니다.  |
|  InvalidParameter  |  지정된 관리자 암호가 허용되지 않습니다.  |
|  InvalidParameter  |  "제공된 암호의 길이는 {0}~{1}자 사이여야 하고 최소한 다음의 암호 복잡성 요구 사항 중 적어도 {2}를 충족해야 합니다. <ol><li> 대문자를 포함합니다.</li><li>소문자를 포함합니다.</li><li>숫자를 포함합니다.</li><li>특수 문자를 포함합니다.</li></ol>  |
...
|  InvalidParameter  |  이름이 '{0}'인 디스크가 이미 동일한 LUN {1}을 사용합니다.  |
|  InvalidParameter  |  '{0}'이라는 디스크는 이미 존재합니다.  |
|  InvalidParameter  |  지정된 이미지 참조에 이미 정의된 디스크에 대해 사용자 이미지 재정의를 지정할 수 없습니다.  |
|  InvalidParameter  |  이름이 '{0}'인 디스크가 이미 동일한 VHD URL {1}을 사용합니다.  |
|  InvalidParameter  |  지정된 장애 도메인 수 {0}은(는) {1}~{2} 범위에 있어야 합니다.  |
|  InvalidParameter  |  라이선스 유형 {0}이 올바르지 않습니다. 올바른 라이선스 유형: Windows_Client 또는 Windows_Server(대/소문자 구분)  |
|  InvalidParameter  |  Linux 호스트 이름은 {0}자를 초과하거나 {1} 문자를 포함할 수 없습니다.  |
|  InvalidParameter  |  Linux 프로비전 에이전트에 있는 알려진 문제로 인해 SSH 공개 키에 대한 대상 경로가 현재 기본값 {0}으로 제한됩니다.  |
...
|  InvalidParameter  |  LUN {0}에 디스크가 이미 있습니다.  |
|  InvalidParameter  |  현재 가용성 집합이 할당된 클러스터에서 요청한 크기 {0}을 사용할 수 없기 때문에 VM을 만들 수 없습니다. 사용 가능한 크기는 {1}입니다. https://aka.ms/azure-resizevm에서 전략의 크기를 조정하는 VM을 참고합니다.  |
|  InvalidParameter  |  요청한 VM 크기 {0}을 현재 지역에서 사용할 수 없습니다. 현재 지역에서 사용할 수 있는 크기는 {1}입니다. https://aka.ms/azure-regions에서 각 지역에서 사용 가능한 VM 크기에 대해 자세히 알아봅니다.  |
|  InvalidParameter  |  요청한 VM 크기 {0}을 현재 지역에서 사용할 수 없습니다. https://aka.ms/azure-regions에서 각 지역에서 사용 가능한 VM 크기에 대해 자세히 알아봅니다.  |
|  InvalidParameter  |  Windows 관리자 사용자 이름은 길이가 {0}자 이하이거나 마침표(.)로 끝나거나 문자 {1}을 포함할 수 없습니다.  |
|  InvalidParameter  |  Windows 컴퓨터 이름은 길이가 {0}자 이하이거나 전체가 숫자이거나 문자 {1}을 포함할 수 없습니다.  |
...
|  NotFound  |  가용성 집합 {0}을 찾을 수 없습니다.  |
|  NotFound  |  요청에 지정된 원본 가상 컴퓨터 '{0}'은 이 Azure 위치에 존재하지 않습니다.  |
|  NotFound  |  ID {0}을(를) 갖는 테넌트를 찾을 수 없습니다.  |
|  NotFound  |  {0} 이미지를 찾을 수 없습니다.  |
|  NotSupported  |  라이선스 형식이 {0}이지만 이미지 Blob {1}이 온-프레미스에서 제공되지 않았습니다.  |
...
|  OperationNotAllowed  |  캡처 작업은 Blob 기반 디스크를 사용하는 가상 컴퓨터에서만 지원됩니다. '이미지' 리소스 API를 사용하여 관리되는 가상 컴퓨터에서 이미지를 만드세요.  |
...
|  OperationNotAllowed  |  VM '{1}'이 생성되었으므로 작업 '{0}'은 이 VM에서 허용되지 않습니다.  |
...
|  OperationNotAllowed  |  VM '{1}'의 할당이 취소되지 않았으므로 작업 '{0}'은 이 VM에서 허용되지 않습니다.  |
...
|  OperationNotAllowed  |  진행 중인 작업이 가상 컴퓨터의 전원 상태를 {0}에서 {1}로 변경하고 있습니다. 일정 시간이 지난 후 {2} 작업을 수행하세요.  |
|  OperationNotAllowed  |  VM을 추가하거나 업데이트할 수 없습니다. 기존 할당 단위에서 요청된 VM 크기 {0}를 사용하지 못할 수도 있습니다. https://aka.ms/azure-resizevm에서 전략의 크기를 조정하는 VM을 참고합니다.  |
|  OperationNotAllowed  |  현재 가용성 집합이 할당된 클러스터에서 요청한 크기 {0}을 사용할 수 없기 때문에 VM의 크기를 조정할 수 없습니다. 사용 가능한 크기는 {1}입니다. https://aka.ms/azure-resizevm에서 전략의 크기를 조정하는 VM을 참고합니다.  |
|  OperationNotAllowed  |  현재 VM이 할당된 클러스터에서 요청한 크기 {0}을 사용할 수 없기 때문에 VM의 크기를 조정할 수 없습니다. VM의 크기를 {1}로 조정하려면 Azure Portal에서 중지 작업인 할당을 취소하고 크기 조정 작업을 다시 시도하세요. https://aka.ms/azure-resizevm에서 전략의 크기를 조정하는 VM을 참고합니다.  |
...
|  OSProvisioningClientError  |  SSH 호스트 키를 생성하지 못했습니다. 오류 세부 정보: {0} 이 문제를 해결하려면 Linux 에이전트가 올바르게 설정되었는지 확인합니다. <ul><li>https://azure.microsoft.com/documentation/articles/virtual-machines-linux-agent-user-guide/에 있는 지침을 확인할 수 있습니다. </li></ul> |
...
|  StorageAccountLocationMismatch  |  저장소 계정 '{0}'을 확인할 수 없습니다. 이 계정이 계산 리소스와 동일한 위치에 있는 저장소 리소스 공급자를 통해 생성되었는지 확인하세요.  |
|  StorageAccountNotFound  |  저장소 계정 {0}을 찾을 수 없습니다. 저장소 계정이 삭제되지 않았고 VM과 동일한 Azure 위치에 속하는지 확인하세요.  |
...
|  TargetDiskBlobAlreadyExists  |  Blob {0}은 이미 있습니다. 다른 Blob URI를 제공하여 새로운 빈 데이터 디스크 '{1}'을 만드세요.  |
|  TargetDiskBlobAlreadyExists  |  대상 이미지 Blob {0}이 이미 있고 VHD Blob을 덮어쓰는 플래그가 설정되어 있지 않아서 캡처 작업을 완료할 수 없습니다. Blob을 삭제하거나 VHD Blob을 덮어쓰는 플래그를 설정하고 다시 시도하세요.  |
...
|  VHDSizeInvalid  |  Blob {2}를 사용하는 디스크 '{1}'에 {0}이라는 지정된 디스크 크기 값이 잘못되었습니다. 디스크 크기는 {3}에서 {4} 사이여야 합니다.  |
|  VMAgentStatusCommunicationError  |  VM '{0}'이 VM 에이전트 또는 확장의 상태를 보고하지 않았습니다. VM에 실행 중인 VM 에이전트가 있고 VM이 Azure Storage에 아웃바운드 연결을 설정할 수 있는지 확인하세요.  |
...
|  VMMarketplaceInvalidInput  |  Marketplace 이미지로부터 가상 컴퓨터를 만들려면 요청에 계획 정보가 있어야 합니다. OS 디스크 이름은 {0}입니다.  |
|  VMNotFound  |  VM '{0}'을 찾을 수 없습니다.  |
...
|  VMStartTimedOut  |  VM '{0}'이 할당된 시간에 시작하지 못했습니다. VM을 성공적으로 시작할 수 있습니다. 나중에 전원 상태를 확인하세요.  |
