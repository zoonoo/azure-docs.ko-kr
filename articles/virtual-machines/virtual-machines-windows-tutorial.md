...
이 자습서에서는 리소스 관리자 배포 모델을 사용하여 가상 컴퓨터를 만듭니다. 서비스 관리 API를 기반으로 하는 기존 배포 모델 대신 이 방법을 사용하는 것이 좋습니다. 리소스 관리자에 대한 자세한 내용은 [Azure 리소스 관리자 개요](resource-group-overview.md)를 참조하세요. 가상 컴퓨터에 리소스 관리자를 사용하는 것의 장점을 알아보려면 [Azure 리소스 관리자에 통합된 Azure 계산, 네트워크 및 저장소 공급자](virtual-machines-azurerm-versus-azuresm.md)를 참조하세요.
