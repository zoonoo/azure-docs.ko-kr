...
- N 시리즈 VM은 디스크에 지원되는 Azure Storage의 유형이 다양합니다. NC 및 NV VM은 표준 디스크 스토리지(HDD)로 백업되는 VM 디스크만 지원합니다. 다른 모든 GPU Vm은 Standard 디스크 저장소 및 Premium 디스크 저장소 (SSD)에 의해 지원 되는 VM 디스크를 지원 합니다.
