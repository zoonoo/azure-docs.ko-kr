...
갤러리에서 다른 많은 SQL Server 가상 컴퓨터 이미지 중 하나를 설치할 수도 있습니다. 이러한 이미지에는 VM 가격에 포함된 SQL Server의 라이선스가 포함되어 있습니다. 자세한 내용은 [Azure에서 SQL Server 가상 컴퓨터 프로비전][provisionsqlvm]을 참조하세요.
...
## 데이터 마이그레이션
SQL Server를 실행하는 Azure VM으로 데이터를 마이그레이션할 수 있는 여러 가지 방법이 있습니다. 먼저 SQL Server Management Studio에서 Azure 포털, PowerShell 자동화 또는 배포 마법사를 사용하여 SQL Server 가상 컴퓨터를 프로비전합니다. 최적화된 SQL Server 이미지에는 해당 가격 모델의 라이선스가 포함되어 있지만 사용자가 소유한 라이선스를 사용하여 SQL Server를 설치할 수도 있습니다. 데이터를 마이그레이션하기 위해서는 배포 마법사를 사용하거나 데이터 디스크를 대상 가상 컴퓨터로 마이그레이션하는 등 몇 가지 옵션이 있습니다. 자세한 내용은 [Azure 가상 컴퓨터의 SQL Server로 마이그레이션할 준비하기][migratesql]를 참조하세요.
...
[Azure 가상 컴퓨터의 SQL Server 관련 기술 문서][technicalarticles]
