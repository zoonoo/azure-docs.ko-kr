...
1. 로컬 컴퓨터의 PowerShell 창에서 Azure 모듈을 가져오고, 게시 설정 파일을 사용자 컴퓨터에 다운로드한 다음 다운로드 게시 설정을 가져와 PowerShell 세션을 Azure 구독에 연결합니다.
...
1. 구성 파일을 가져와서 가상 네트워크를 만듭니다.

		Set-AzureVNetConfig `
			-ConfigurationPath $networkConfigPath

	구성 파일은 다음 XML 문서를 포함합니다. 간단히 말해, **ContosoAG**라는 선호도 그룹에서 **ContosoNET**이라는 가상 네트워크를 지정하고 주소 공간은 **10.10.0.0/16**이며 두 서브넷(프론트 서브넷 **10.10.1.0/24**와 백 서브넷 **10.10.2.0/24**)이 있습니다. 프런트 서브넷은 Microsoft SharePoint와 같은 클라이언트 응용 프로그램을 배치하는 곳이며 백 서브넷에는 SQL Server VM을 배치합니다. **$affinityGroupName** 및 **$virtualNetworkName** 변수를 앞에서 변경할 경우 아래의 해당하는 이름도 함께 변경해야 합니다.
...
1. 만든 선호도 그룹과 연결되는 저장소 계정을 만들고 해당 계정을 구독에서 현재 저장소 계정으로 설정합니다.
...
1. **DCPROMO.EXE** 명령을 실행하여 M 드라이브의 데이터 디렉터리로 **corp.contoso.com** 도메인을 설정합니다.
...
1. 원격 데스크톱 파일을 실행하여 DC 서버에 다시 연결합니다. 이번에는 **CORP\\Administrator**로 로그인합니다.

1. 관리자 모드에서 PowerShell 창을 열고 다음 명령을 사용하여 Active Directory PowerShell 모듈을 가져옵니다.

		Import-Module ActiveDirectory

1. 다음 명령을 실행하여 도메인에 3명의 사용자를 추가합니다.
...
1. 이후 다음 명령을 실행하여 도메인에서 컴퓨터 개체를 만들 권한을 **CORP\\Install**에 부여합니다.
...
1. 다음 파이프 명령을 실행하여 이름이 **ContosoQuorum**인 WSFC 클러스터의 첫 번째 VM을 만듭니다.
...
- (ContosoSQL1 및 ContosoSQL2만) 기본 데이터베이스에서 **CORP\\Install**을 **sysadmin** 역할로 추가해야 합니다.
...
이제 배포할 준비가 되었습니다. **ContosoQuorum**부터 다음 단계를 수행합니다.
...
1. 계속하기 전에 SQL Server 설치가 자동 초기화 작업 실행을 마칠 때까지 기다립니다.

1. 관리자 모드에서 PowerShell 창을 엽니다.

1. Windows 장애 조치 클러스터링 기능을 설치합니다.

		Import-Module ServerManager
		Add-WindowsFeature Failover-Clustering

1. **CORP\\Install**을 로컬 관리자로 추가합니다.

		net localgroup administrators "CORP\Install" /Add

1. ContosoQuorum에서 로그아웃합니다. 이제 이 서버는 작업을 마쳤습니다.
...
1. SQL Server PowerShell 공급자를 가져옵니다.

		Set-ExecutionPolicy -Execution RemoteSigned -Force
		Import-Module -Name "sqlps" -DisableNameChecking

1. 기본 SQL Server 인스턴스에 sysadmin 역할로 **CORP\\Install**을 추가합니다.
...
1. SQL Server의 원격 액세스를 위해 방화벽을 엽니다.
...
이제 가용성 그룹을 구성할 준비가 되었습니다. SQL Server PowerShell 공급자를 사용하여 **ContosoSQL1**에서 모든 작업을 수행하게 됩니다.
...
1. ContosoSQL1의 SQL Server 서비스 계정을 CORP\\SQLSvc1로 변경합니다.
...
1. 마지막으로, 가용성 그룹을 만들고 보조 복제본을 가용성 그룹에 연결합니다.
