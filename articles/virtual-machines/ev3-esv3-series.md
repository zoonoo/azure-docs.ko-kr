...
Ev3 및 Esv3 시리즈는 하이퍼 스레드 구성에서 Intel® Xeon® 8171M 2.1 GHz (Skylake) 또는 Intel® Xeon® E5-2673 v4 2.3 GHz (Broadwell) 프로세서를 제공 하며, 대부분의 범용 워크 로드에 대해 더 나은 가치를 제공 하 고, Ev3를 대부분의 다른 클라우드의 범용 Vm과 맞춥니다.  하이퍼 스레딩으로 이동하기 위해 디스크 및 네트워크 제한이 코어 단위로 조정되는 동안 메모리가 확장되었습니다(7GiB/vCPU에서 8GiB/vCPU로). Ev3는 D/Dv2 제품군의 상위 메모리 VM 크기에 대한 후속 시리즈입니다.
