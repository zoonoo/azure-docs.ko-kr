...
시작하려면 원하는 텍스트 편집기(Visual Studio Code/Sublime/Vim/etc)에서 \_terraform_azure101.tf_라는 파일을 만듭니다. Terraform이 폴더 이름을 매개 변수로 사용하고 폴더에 있는 모든 스크립트가 실행되기 때문에 파일의 정확한 이름은 중요하지 않습니다. 다음 코드를 새 파일에 붙여 넣습니다.
...
```
스크립트가 저장된 폴더에 `terraformscripts`이 있다고 가정합니다. `plan` Terraform 명령을 사용하여 스크립트에 정의된 리소스를 살펴봅니다. 이 명령을 Terraform에서 저장한 상태 정보와 비교한 후 Azure에서 실제 리소스를 생성하지 _않고_ 계획된 실행을 출력합니다. 
...
## <a name="complete-the-terraform-script"></a>Terraform 스크립트 완료
