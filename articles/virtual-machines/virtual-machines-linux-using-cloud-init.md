...
이 문서에서는 cloud-init 스크립트를 만들어 호스트 이름을 설정하고 설치된 패키지를 업데이트하며 사용자 계정을 관리합니다. 그런 다음 [Azure CLI](../xplat-cli-install.md)를 사용하여 Linux VM이 생성되는 동안 cloud-init 스크립트를 실행합니다.
...
Azure에서 VM을 생성할 때 cloud-init 스크립트를 시작하려면 Azure CLI `--custom-data` 스위치를 사용하여 cloud-init 파일을 지정합니다.

참고: 이 문서는 cloud-init 파일에 대해 `--custom-data` 스위치를 사용하는 방법에 대해 설명하지만 이 스위치를 사용하여 임의 코드 또는 파일을 전달할 수도 있습니다. Linux VM이 이러한 파일에서 어떤 작업을 해야 하는지 이미 알고 있는 경우에는 작업이 자동으로 실행됩니다.
...
#### `cloud_config_hostname.txt`라는 예제 cloud-init 스크립트입니다.
...
#### Debian 제품군에 대한 예제 cloud-init 스크립트 `cloud_config_apt_upgrade.txt`
...
새 Linux VM이 부팅되면 `apt-get`을 통해 설치된 모든 패키지가 자동으로 업데이트됩니다.
...
새 Linux VM에서 가장 먼저 해야 할 작업 중 하나는 자신의 사용자를 추가해야 하며, 그렇지 않을 경우 `root`를 사용하지 않아야 합니다. 이와 같이 할 경우 암호 없는 보안 SSH 로그인에 대한 해당 사용자의 `~/.ssh/authorized_keys` 파일에 SSH 공개 키를 추가한 다음 보안을 강화할 수 있습니다.

#### Debian 제품군에 대한 예제 cloud-init 스크립트 `cloud_config_add_users.txt`
