...
* **모바일 서비스**는 복제하려는 모든 표준 VM에 배포되는 구성 요소입니다. 표준 VM에 기록된 데이터를 캡처하고 프로세스 서버에 전달합니다. 복제 된 [컴퓨터 필수 구성 요소](../../site-recovery/vmware-walkthrough-overview.md)를 참조 하세요.
...
2. **리소스** > **Management**관리 > **백업** 만들기 및 **Site Recovery (OMS)** 를 선택 합니다. 또는 **찾아보기** > **Recovery Services 자격 증명 모음** > **추가**를 선택할 수 있습니다. 
...
2. **Recovery Services 자격 증명 모음** > **설정** > **Site Recovery** > **1 단계: 인프라** > 준비**보호 목표**로 이동 합니다.
...
3. 구성 서버로 사용 중인 VM에서 통합 설치 프로그램을 실행하여 구성 서버 및 프로세스 서버를 설치합니다. [스크린샷을 따라](../../site-recovery/vmware-walkthrough-overview.md) 설치를 완료할 수 있습니다. 이 마이그레이션 시나리오에 지정된 단계에 다음 스크린샷을 참조할 수 있습니다.
   ...
   1. **시작 하기 전에**에서 **구성 서버 및 프로세스 서버 설치**를 선택 합니다.
...
**인프라** > 준비**대상**을 선택 하 고 장애 조치 (failover) 후 vm에 사용 하려는 배포 모델을 지정 합니다. 시나리오에 따라 **클래식** 또는 **리소스 관리자**를 선택할 수 있습니다.
...
2. 다음과 같이 복제를 사용합니다.
   1. **응용 프로그램** > **원본**복제를 선택 합니다. 처음으로 복제를 활성화한 후 자격 증명 모음에서 **+복제**를 선택하여 추가 컴퓨터에 대해 복제를 활성화합니다.
   2. 1 단계에서 **원본** 을 프로세스 서버로 설정 합니다.
...
Azure Storage 환경을 디자인할 때 가용성 집합의 각 VM에 대해 별도의 스토리지 계정을 사용하는 것이 좋습니다. 저장소 계층의 모범 사례에 따라 [각 가용성 집합에 대해 여러 저장소 계정을 사용](../linux/manage-availability.md)하는 것이 좋습니다. 여러 스토리지 계정에 VM 디스크를 배포하면 스토리지의 가용성이 향상되고 Azure Storage 인프라 전반에 걸쳐 I/O가 배포됩니다.
...
복제가 완료 되었는지 확인 하려면 Site Recovery 인스턴스를 선택한 다음 **설정** > **복제 된 항목**을 선택 합니다. 복제 프로세스의 상태 및 백분율이 표시됩니다. 
...
*YOUR_FAILOVER_PLAN_NAME* **설정** > **작업** > 에서 테스트 장애 조치 (failover)의 상태를 볼 수 있습니다. 창에서는 단계의 분석 및 성공/실패 결과가 표시됩니다. 어느 단계에서 테스트 장애 조치(failover)가 실패하는 경우 단계를 선택하여 오류 메시지를 확인합니다. 
