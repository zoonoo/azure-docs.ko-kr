...
1. 조직에 Azure Resource Manager가 필요한 이유는 무엇입니까?  마이그레이션에 대한 비즈니스상 이유는 무엇입니까?
2. Azure Resource Manager에 대한 기술적인 이유는 무엇입니까?  추가로 활용하려는 Azure 서비스는 무엇입니까?(있는 경우)
3. 마이그레이션에 포함되는 응용 프로그램(또는 가상 컴퓨터 집합)은 무엇입니까?
4. 마이그레이션 API로 지원되는 시나리오는 무엇입니까?  [지원되지 않는 기능 및 구성](migration-classic-resource-manager-overview.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#unsupported-features-and-configurations)을 검토하세요.
5. 운영 팀이 이제 클래식 및 Azure Resource Manager에서 응용 프로그램/VM을 지원합니까?
6. Azure Resource Manager에서는 VM 배포, 관리, 모니터링 및 보고 프로세스를 어떻게 변경합니까?(변경할 수 있는 경우)  배포 스크립트를 업데이트해야 합니까?
7. 이해 관계자(최종 사용자, 응용 프로그램 소유자 및 인프라 소유자)에게 알릴 통신 계획은 무엇입니까?
8. 환경의 복잡성에 따라 최종 사용자와 응용 프로그램 소유자가 응용 프로그램을 사용할 수 없는 유지 관리 기간이 있어야 합니까?  그렇다면 얼마 동안?
9. 이해 관계자가 Azure Resource Manager에 정통하고 숙련되었는지 확인하기 위한 학습 계획은 무엇입니까?
10. 마이그레이션을 위한 프로그램 관리 또는 프로젝트 관리 계획은 무엇입니까?
11. Azure Resource Manager 마이그레이션 및 기타 관련 기술 로드맵의 일정은 무엇입니까?  최적으로 정렬되어 있습니까?
...
- **Azure Resource Manager API 제한 한도** - 충분히 큰 환경이 있는 경우 (예: 한 VNET에 VM이 400개 이상) Azure Resource Manager에서 쓰기에 대한 기본 API 제한 한도(현재 `1200 writes/hour`)에 도달할 수 있습니다. 마이그레이션을 시작하기 전에 지원 티켓을 늘려 구독 한도를 높여야 합니다.
