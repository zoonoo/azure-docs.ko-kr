...
마이그레이션에 사용되는 단계를 진행하려면 GitHub의 [DB2onAzure](http://aka.ms/db2onazure) 리포지토리에서 설치 스크립트를 참조하세요. 이러한 스크립트는 일반적인 중간 크기의 OLTP(온라인 트랜잭션 처리) 워크로드용 아키텍처를 기준으로 합니다.

## <a name="get-started"></a>시작하기

이 아키텍처를 배포하려면 GitHub의 [DB2onAzure](http://aka.ms/db2onazure) 리포지토리에 있는 deploy.sh 스크립트를 다운로드하여 실행합니다.
...
> [!NOTE]
> 샘플 지시 파일인 DB2server.rsp는 GitHub의 [DB2onAzure](http://aka.ms/db2onazure) 리포지토리에 포함되어 있습니다. 이 파일을 사용하는 경우 사용자 환경에서 사용할 수 있도록 편집해야 합니다.
...
이러한 문제 및 기타 알려진 문제에 대한 자세한 내용은 [DB2onAzure](http://aka.ms/DB2onAzure) 리포지토리에서 kb.md 파일을 참조하세요.
