...
다음 명령을 사용하여 `<resourceGroupName>` 리소스 그룹을 지역에 만듭니다. `<resourceGroupName>`을 선택한 이름으로 바꿉니다. 이 자습서에서는 `East US 2`를 사용합니다. 자세한 내용은 이 [빠른 시작](../quick-create-cli.md)을 참조하세요.
...
1. 모든 노드에서 기존 클러스터 구성을 제거합니다. 다음 명령 실행:
...
1. 다음 명령을 사용하여 `ag_cluster` 리소스를 사용 가능한 `ag1` 그룹에 만듭니다.
...
1. IP 주소와 AG 리소스가 동일한 노드에서 실행되고 있는지 확인하려면 공동 배치 제약 조건을 구성해야 합니다. 다음 명령 실행:
