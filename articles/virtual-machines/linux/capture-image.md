---
title: Azure CLI를 사용하여 Linux VM 이미지 캡처
...
**Azure VM 이미지 빌더(공개 미리 보기)** 서비스를 사용하여 사용자 지정 이미지를 빌드하거나, 도구를 학습하거나, 파이프라인을 설정하여 이미지 구성을 제공하면 이미지 빌더가 이미지를 만들 수 있습니다. 자세한 내용은 [Azure VM 이미지 빌더를 시작하십시오.](https://docs.microsoft.com/azure/virtual-machines/linux/image-builder-overview)
...
## <a name="prefer-a-tutorial-instead"></a>대신 자습서를 선호?

이 문서에 대한 간소화된 버전 및 Azure의 VM에 대한 테스트, 평가 또는 학습에 대해 알아보려면 [CLI를 사용하여 Azure VM의 사용자 지정 이미지 만들기](tutorial-custom-images.md)를 참조하세요.  그렇지 않으면 전체 그림을 얻으려면 여기에서 계속 읽으십시오.


## <a name="step-1-deprovision-the-vm"></a>1단계: VM 프로비전 해제
Azure VM 에이전트를 사용하여 VM의 프로비전을 해제하여 머신별 파일 및 데이터를 삭제합니다. 원본 Linux VM에서 `-deprovision+user` 매개 변수와 함께 `waagent` 명령을 사용합니다. 자세한 내용은 Azure [Linux 에이전트 사용 설명서를](../extensions/agent-linux.md)참조하십시오.
...
4. 명령이 완료되면 **종료**를 입력하여 SSH 클라이언트를 닫습니다.  이 시점에서 VM이 계속 실행됩니다.
...
1. [az vm deallocate](/cli/azure/vm)로 프로비전 해제한 VM의 할당을 취소합니다. 다음 예제에서는 *myResourceGroup이라는*리소스 그룹에서 *myVM이라는* VM을 할당 합니다.  
    ...
    계속하기 전에 VM이 완전히 할당 될 때까지 기다립니다. 이 작업을 완료하는 데 몇 분 정도 걸릴 수 있습니다.  할당 할당 중에 VM이 종료됩니다.
...
이 명령은 VM 이미지를 설명하는 JSON을 반환합니다. 나중에 참조할 수 위해 이 출력을 저장합니다.
