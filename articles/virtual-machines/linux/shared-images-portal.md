...
이 문서의 예제를 완료하려면 일반화된 VM의 기존 관리되는 이미지 또는 특수 VM의 스냅숏이 있어야 합니다. [자습서: Azure PowerShell을 사용하여 Azure VM의 사용자 지정 이미지를 만들어](tutorial-custom-images.md) 관리되는 이미지를 만들거나 특수 VM에 대한 [스냅숏을 만들](../windows/snapshot-copy-managed-disk.md) 수 있습니다. 관리되는 이미지와 스냅숏 모두 데이터 디스크 크기가 1TB를 초과할 수 없습니다.
...
1. **관리자 계정에서**소스 VM이 일반화된 경우 **사용자 이름** 및 **SSH 공개 키를**입력합니다. 원본 VM이 특수화된 경우 원본 VM의 정보가 사용되므로 이러한 옵션이 회색으로 표시됩니다.
1. **공용 인바운드 포트에서**VM에 대한 원격 액세스를 허용하려면 **선택한 포트 허용을** 선택한 다음 드롭다운에서 **SSH(22)를** 선택합니다. VM에 대한 원격 액세스를 허용하지 않으려면 **공용 인바운드 포트에**대해 **없음을** 선택하지 않은 상태로 둡니다.
