...
이 자습서에서는 가상 네트워크에 여러 VM(가상 컴퓨터)을 만드는 방법을 알아보고 이러한 컴퓨터 간에 네트워크 연결을 구성합니다. 완료되면 SSH 연결의 경우는 포트 22, HTTP 연결의 경우는 포트 80을 통해 인터넷에서 '프런트 엔드' VM에 액세스할 수 있습니다. MySQL 데이터베이스가 있는 '백 엔드' VM은 격리되며, 포트 3306에서만 프런트 엔드 VM에서 액세스할 수 있습니다.
...
다른 Azure 리소스를 만들려면 먼저 az group create를 사용하여 리소스 그룹을 만들어야 합니다. 다음 예제에서는 `westus` 위치에 `myRGNetwork`이라는 리소스 그룹을 만듭니다.

```azurecli
az group create --name myRGNetwork --location westus
```

Azure CLI를 사용하여 가상 컴퓨터를 만들 때 필요한 네트워크 리소스가 동시에 자동으로 만들어집니다. [az vm create](https://docs.microsoft.com/cli/azure/vm#create)를 사용하여 `myFrontendVM` 및 지원 네트워크 리소스를 만듭니다.
...
[az vm open-port](https://docs.microsoft.com/cli/azure/vm#open-port)를 사용하여 `myFrontendVM`에서 포트 80을 엽니다.
...
VM의 내부 통신 또한 NSG를 사용하여 구성할 수 있습니다. 이 섹션에서는 네트워크에서 추가 서브넷을 만들고 NSG를 서브넷에 할당하여 `myFrontendVM`에서 포트 3306의 `myBackendVM`에 연결할 수 있도록 하는 방법을 알아봅니다. 그러면 서브넷이 만들어질 때 VM에 할당됩니다.

[az network nsg create](https://docs.microsoft.com/cli/azure/network/nsg#create)를 사용하여 `myBackendNSG`라는 새 네트워크 보안 그룹을 추가합니다. 
...
포트를 설정하여 `myFrontendVM` 및 `myBackendVM`이 VNet에서 서로 통신할 수 있도록 합니다. `az network rule create`를 사용하여 `myVMSubnet`에서만 `myBackendSubnet`으로 트래픽을 보낼 수 있도록 허용하는 NSG 규칙을 추가합니다.
...
[az network vnet subnet create](https://docs.microsoft.com/cli/azure/network/vnet/subnet#create)를 사용하여 `myBackEndSubnet`을 `myFrontendVMVNet`에 추가합니다.
...
`myBackendSubnet` 및 `az vm create`를 사용하여 `myBackendVM`을 만듭니다.
...
백 엔드 VM은 공개적으로 액세스하기 위한 것이 아닙니다. 이 섹션에서는 SSH를 사용하여 `myFrontendVM`에 로그인 한 다음 SSH를 사용하여 `myFrontendVM`에서 `myBackendVM`에 로그인하는 방법을 알아봅니다.

예제 IP 주소를 `myFrontendVM`의 공용 IP 주소로 바꿉니다.

```bash
scp ~/.ssh/id_rsa 40.68.254.142:~/.ssh/id_rsa
```

`myFrontendVM`과의 SSH 연결을 만듭니다. 예제 IP 주소를 `myFrontendVM`의 공용 IP 주소로 바꿉니다.
...
응용 프로그램을 `myBackendVM`에 설치하는 방법을 보여 주기 위해 MySQL이 설치되지만 이 자습서에서 실제로 사용되지는 않습니다.
