...
### <a name="bkmk_RunningLinuxCLI"> </a>Azure CLI를 사용 하 여 기존 또는 실행 중인 Linux VM에서 암호화 사용 
...
### <a name="bkmk_RunningLinuxPSH"></a> PowerShell을 사용 하 여 기존 또는 실행 중인 Linux VM에서 암호화 사용
...
EncryptFormatAll 매개 변수는 Linux 데이터 디스크가 암호화 되는 시간을 줄입니다. 특정 기준을 충족 하는 파티션은 현재 파일 시스템으로 포맷 됩니다. 그런 다음 명령 실행 전의 위치에 다시 탑재 합니다. 조건을 충족 하는 데이터 디스크를 제외 하려는 경우 명령을 실행 하기 전에 분리할 수 있습니다.
