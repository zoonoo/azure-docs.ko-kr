...
Lsv2 시리즈 가상 머신(VM)의 설계는 AMD EPYC™ 7551 프로세서를 최대화하여 프로세서, 메모리, NVMe 장치 및 VM 간에 최상의 성능을 제공합니다. Linux의 파트너와 협력하여 Lsv2 시리즈 성능에 최적화되어 있으며 현재 다음을 포함하는 여러 빌드를 사용할 수 있는 Azure Marketplace가 있습니다.
...
## <a name="tips-to-maximize-performance"></a>성능을 극대화하기 위한 팁

* 워크로드에 대한 사용자 지정 Linux GuestOS를 업로드하는 경우 가속 네트워킹은 기본적으로 **꺼져** 있습니다. 가속 네트워킹을 사용하려는 경우 최상의 성능을 위해 VM 생성 시 활성화합니다.
...
* **성능에 대한 rq_affinity 조정해야 합니까?**  
   rq_affinity 설정은 절대 최대 입력/출력 연산(IOPS)을 사용할 때 사소한 조정입니다. 다른 모든 것이 잘 작동하면 rq_affinity 0으로 설정하여 차이를 만드는지 확인하십시오.

* **blk_mq 설정을 변경해야 하나요?**  
   RHEL/CentOS 7.x는 NVMe 장치에 대해 blk-mq를 자동으로 사용합니다. 구성을 변경하거나 설정하지 않습니다. scsi_mod.use_blk_mq 설정은 SCSI전용이며 NVMe 장치가 게스트 VM에서 SCSI 장치로 표시되었기 때문에 Lsv2 미리 보기 중에 사용되었습니다. 현재 NVMe 장치는 NVMe 장치로 표시되므로 SCSI blk-mq 설정은 관련이 없습니다.

* **"fio"를 변경해야 하나요?**  
   L64v2 및 L80v2 VM 크기의 'fio'와 같은 성능 측정 도구를 사용하여 최대 IOPS를 얻으려면 각 NVMe 장치에서 "rq_affinity"을 0으로 설정합니다.  예를 들어 이 명령줄은 L80v2 VM의 모든 10개의 NVMe 장치에 대해 "rq_affinity"을 0으로 설정합니다.

   ```console
   for i in `seq 0 9`; do echo 0 >/sys/block/nvme${i}n1/queue/rq_affinity; done
   ```

   또한 분할이 없고 파일 시스템, RAID 0 구성 등이 없는 각 원시 NVMe 장치에 I/O가 직접 수행될 때 최상의 성능을 얻을 수 있습니다. 테스트 세션을 시작하기 전에 각 NVMe 장치에서 실행하여 `blkdiscard` 구성이 알려진 새 상태/정리 상태인지 확인합니다.
