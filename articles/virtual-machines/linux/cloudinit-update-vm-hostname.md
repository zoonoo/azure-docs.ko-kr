...
기본적으로 호스트 이름은 Azure에서 새 가상 머신을 만들 때 VM 이름과 동일합니다.  Azure에서 [az vm create](/cli/azure/vm#create)를 사용하여 VM을 만들 때 cloud-init 스크립트를 실행하여 이 기본 호스트 이름을 변경하려면 cloud-init 파일을 `--custom-data` 스위치로 지정합니다.  
