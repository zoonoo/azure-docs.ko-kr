---
title: Azure에서 OpenShift Container Platform 자체 관리 Marketplace 제품 배포 | Microsoft Docs
description: Azure에서 OpenShift Container Platform 자체 관리 Marketplace 제품을 배포 합니다.
...
ms.locfileid: "70091920"
---
# <a name="configure-prerequisites"></a>필수 조건 구성

Marketplace 제품을 사용 하 여 Azure에서 자체 관리 되는 OpenShift 컨테이너 플랫폼 클러스터를 배포 하기 전에 몇 가지 필수 구성 요소를 구성 해야 합니다.  Ssh 키 (암호 제외), Azure key vault, key vault 암호 및 서비스 주체를 만드는 방법에 대 한 자세한 내용은 [Openshift 필수 조건](https://docs.microsoft.com/azure/virtual-machines/linux/openshift-prerequisites) 문서를 참조 하세요.

 
## <a name="deploy-using-the-marketplace-offer"></a>Marketplace 제품을 사용 하 여 배포

자체 관리 되는 OpenShift Container Platform 클러스터를 Azure에 배포 하는 가장 간단한 방법은 [Azure Marketplace 제품](https://azuremarketplace.microsoft.com/marketplace/apps/redhat.openshift-container-platform?tab=Overview)을 사용 하는 것입니다.
...
- **노드**: 노드 수 (1 ~ 007e; 9)와 인스턴스 형식은 구성 가능 합니다.
- **디스크 유형**: Managed Disks가 사용됩니다.
- **네트워킹**: 신규 또는 기존 네트워크 및 사용자 지정 CIDR 범위에 대 한 지원.
- **CNS**: CNS를 사용하도록 설정할 수 있습니다.
- **메트릭**: Hawkular 메트릭을 사용 하도록 설정할 수 있습니다.
- **로깅**: EFK 로깅은 사용 하도록 설정할 수 있습니다.
- **Azure 클라우드 공급 기업**: 기본적으로 사용 하도록 설정 되어 있으므로 사용 하지 않도록 설정할 수 있습니다.
...
목록에서 **Red Hat OpenShift Container Platform이 자동으로 관리 되** 는 결과 페이지가 열립니다. 
