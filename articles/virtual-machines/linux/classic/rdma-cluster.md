...
ms.locfileid: "38990730"
---
# <a name="set-up-a-linux-rdma-cluster-to-run-mpi-applications"></a>MPI 응용 프로그램을 실행하도록 Linux RDMA 클러스터 설정

Azure에서 [고성능 계산 VM 크기](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)를 사용하여 MPI(Message Passing Interface) 응용 프로그램을 병렬로 실행하도록 Linux RDMA 클러스터를 설정하는 방법을 알아봅니다. 이 문서는 클러스터에서 Intel MPI를 실행하도록 Linux HPC 이미지를 준비하기 위한 단계를 제공합니다. 준비가 끝나면 이 이미지와 [RDMA 지원 Azure VM 크기](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#rdma-capable-instances) 중 하나를 사용하여 VM 클러스터를 배포합니다. RDMA(원격 직접 메모리 액세스) 기술을 기반으로 하는 짧은 대기 시간, 높은 처리량의 네트워크에서 효율적으로 통신하는 MPI 응용 프로그램을 실행하려면 클러스터를 사용합니다.

> [!IMPORTANT]
> Azure에는 리소스를 만들고 사용하기 위한 별도의 두 가지 배포 모델, 즉 [Azure Resource Manager](../../../azure-resource-manager/resource-manager-deployment-model.md)와 클래식 모델이 있습니다. 이 문서에서는 클래식 배포 모델 사용에 대해 설명합니다. 새로운 배포는 대부분 리소스 관리자 모델을 사용하는 것이 좋습니다.

## <a name="cluster-deployment-options"></a>클러스터 배포 옵션
다음은 작업 스케줄러의 유무에 관계 없이 Linux RDMA 클러스터를 만드는 데 사용할 수 있는 다양한 방법입니다.
...
다음 단계에서는 Azure CLI를 사용하여 Azure Marketplace에서 SLES(SUSE Linux Enterprise Server) 12 HPC VM을 배포하고, 사용자 지정하고, 사용자 지정 VM 이미지를 만드는 방법을 보여 줍니다. 그런 다음 이 이미지를 사용하여 RDMA 지원 VM 클러스터 배포를 스크립팅할 수 있습니다.
...
* **VM 크기 가용성**: Azure 지역에서 H 시리즈 또는 기타 RDMA 지원 VM 크기의 가용성은 [지역별로 제공되는 제품](https://azure.microsoft.com/regions/services/)을 확인하세요.
...
[RDMA 지원 Azure VM 크기](../sizes-hpc.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#rdma-capable-instances)에 대한 배포 고려 사항도 검토하세요.

### <a name="provision-an-sles-12-hpc-vm"></a>SLES 12 HPC VM 프로비전
...
Azure에서 공개적으로 사용할 수 있는 SLES 12 HPC 이미지를 보려면 셸 환경에서 **grep**를 지원한다고 가정하고 다음과 같은 명령을 실행합니다.

    azure vm image list | grep "suse.*hpc"

다음과 같은 명령을 실행하여 SLES 12 SP3 HPC 이미지로 RDMA 지원 VM을 프로비전합니다.
...
* 새 클라우드 서비스는 해당 위치로 지정된 Azure 지역에 만들어집니다. 선택한 VM 크기를 사용할 수 있는 위치를 지정합니다(예: “미국 서부”).
* SUSE 우선 순위 지원(추가 요금 발생)의 경우 SLES 12 이미지 이름은 현재 이름에 `priority`가 있는 옵션 중 하나일 수 있습니다. 
...
* **업데이트** - zypper를 사용하여 업데이트를 설치합니다. NFS 유틸리티를 설치하려고 할 수도 있습니다.
  ...
  > [!IMPORTANT]
  > SLES 12 HPC VM에서 Linux RDMA 드라이버에 문제가 발생할 수 있으므로 커널 업데이트를 적용하지 않는 것이 좋습니다.
  >
  >
* **Intel MPI**: 다음 명령을 실행하여 VM에서 Intel MPI 설치를 완료합니다.
...
* **메모리 잠금** - MPI 코드로 RDMA에 사용 가능한 메모리를 잠그려면 /etc/security/limits.conf 파일에서 다음 설정을 추가하거나 변경합니다. 이 파일을 편집하려면 루트 액세스가 필요합니다.
  ...
  > [!NOTE]
  > 테스트를 위해 memlock을 무제한으로 설정할 수도 있습니다. 예: `* hard memlock unlimited` 자세한 내용은 [가장 잘 알려진 잠금 메모리 크기 설정 방법](https://software.intel.com/en-us/blogs/2014/12/16/best-known-methods-for-setting-locked-memory-size)(영문)을 참조하세요.
  >
  >
* **SLES VM용 SSH 키** - SSH 키를 생성하여 MPI 작업을 실행할 때 SLES 클러스터의 계산 노드 간에 사용자 계정에 대한 신뢰를 설정합니다. CentOS 기반 HPC VM을 배포한 경우 이 단계를 수행하지 않습니다. 이미지를 캡처하고 클러스터를 배포한 후에 클러스터 노드 간에 암호 없는 SSH 트러스트를 설정하려면 이 문서의 뒷부분에 나오는 지침을 참조하세요.
  ...
  SSH 키를 만들려면 `ssh-keygen` 명령을 실행합니다. 입력하라는 메시지가 표시되면 **Enter** 키를 선택하여 암호를 설정하지 않고 기본 위치에 키를 생성합니다.
...
CentOS 기반 HPC 클러스터에서 사용자 기반 인증을 사용하기 위한 샘플 스크립트 `user_authentication.sh`는 다음과 같습니다.
...
다음 단계를 사용하여 이 스크립트를 실행합니다. 이 스크립트를 수정하거나 다른 메서드를 사용하여 클러스터 계산 노드 간에 암호 없는 SSH 인증을 설정할 수 있습니다.
...
계산 노드 중 하나에서 MPI 명령을 실행하여 MPI가 제대로 설치되어 있고 적어도 두 개의 계산 노드 간에 통신할 수 있는지 확인합니다. 다음 **mpirun** 명령은 두 개의 노드에서 **hostname** 명령을 실행합니다. `-hosts` 매개 변수의 경우 Azure VNet에서 두 노드의 IP 주소를 전달합니다(예: 10.32.0.4,10.32.0.5).
...
```
### <a name="sample-mpi-run-script"></a>샘플 MPI 실행 스크립트
MPI 응용 프로그램을 실행하는 데 필요한 변수를 구성하는 샘플 Bash 스크립트는 다음과 같습니다. Intel MPI 설치를 위해 필요에 따라 경로를 `mpivars.sh`로 변경합니다.
...
호스트 파일의 형식은 다음과 같습니다. 클러스터의 각 노드에 대해 한 줄을 추가합니다. 앞에서 정의한 가상 네트워크에서 DNS 이름이 아니라 개인 IP 주소를 지정합니다. 예를 들어 IP 주소가 10.32.0.4 및 10.32.0.4인 두 호스트의 경우 파일에 다음이 포함됩니다.
