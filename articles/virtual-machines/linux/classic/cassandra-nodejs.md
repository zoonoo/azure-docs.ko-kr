...
Microsoft Azure는 운영 체제, 애플리케이션 서버, 메시징 미들웨어뿐 아니라 상용 및 오픈 소스 모델의 SQL 및 NoSQL 데이터베이스를 포함하는 Microsoft 및 타사 소프트웨어를 실행하는 개방형 클라우드 플랫폼입니다. Azure를 비롯한 공용 클라우드에 복원 서비스를 빌드하려면 애플리케이션 서버 및 스토리지 계층 둘 다의 신중한 계획과 세밀한 아키텍처가 필요합니다. Cassandra의 분산 저장소 아키텍처는 클러스터 오류에 대한 내결함성이 있는 고가용성 시스템 빌드에 도움이 됩니다. Cassandra는 cassandra.apache.org의 Apache Software Foundation에서 유지 관리하는 클라우드 규모의 NoSQL 데이터베이스입니다. Cassandra는 Java로 작성됩니다. 따라서 Windows 및 Linux 플랫폼에서 모두 실행됩니다.
