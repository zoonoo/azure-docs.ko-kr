...
Azure의 엔드포인트는 공용 및 개인 포트와 함께 TCP 또는 UDP 프로토콜로 구성됩니다. 개인 포트는 가상 머신에서 서비스가 수신 대기하는 포트입니다. 공용 포트는 외부에서 Azure 클라우드 서비스가 들어오는 인터넷 기반 수신 트래픽을 수신 대기하는 포트입니다.  

8080 TCP 포트는 Tomcat에서 수신 대기하는 데 사용하는 기본 포트 번호입니다. Azure 엔드포인트에서 이 포트를 열면 사용자와 다른 인터넷 클라이언트가 Tomcat 페이지에 액세스할 수 있습니다.  

1. 포털에서 **찾아보기** > **가상 머신**를 차례로 클릭한 다음 만든 가상 머신을 클릭합니다.  
   ![가상 머신 디렉터리의 스크린샷][5]
2. 가상 머신에 엔드포인트를 추가하려면 **엔드포인트** 확인란을 클릭합니다.
   ![엔드포인트 확인란을 보여 주는 스크린샷][6]
3. **추가**를 클릭합니다.  
...
4. **확인** 을 클릭하여 가상 머신에 엔드포인트를 추가합니다.
...
Tomcat은 Java로 작성되었습니다. 완벽하게 지원되는 Java 런타임을 얻는 방법은 [Azure 지원 JDK](https://aka.ms/azure-jdks)를 참조하세요. 자체 릴리스를 사용해도 되지만, 이 문서의 나머지 부분에서는 Azure에서 지원하는 버전을 사용합니다.


#### <a name="install-azure-supported-jdk"></a>Azure 지원 JDK 설치

[Azul Zulu Enteprise for Azure](https://www.azul.com/downloads/azure-only/zulu/#apt-repo) 웹 사이트에 설명된 `apt-get` 설치 지침을 따릅니다.
...
#### <a name="possible-root-cause"></a>가능한 근본 원인   
  ...
  * Tomcat 수신 대기 포트는 Tomcat 트래픽에 대한 가상 머신 엔드포인트의 개인 포트와 다릅니다.  
     ...
     공용 포트 및 개인 포트 엔드포인트 설정을 확인하고, 개인 포트가 Tomcat 수신 대기 포트와 같은지 확인합니다. 가상 머신의 엔드포인트 구성에 대한 지침은 이 문서의 "1단계: 이미지 만들기" 섹션을 참조하세요.  
...
#### <a name="solution"></a>해결 방법
  ...
  * Tomcat 수신 대기 포트가 가상 머신의 트래픽에 대한 엔드포인트의 개인 포트와 다른 경우 개인 포트를 Tomcat 수신 대기 포트와 동일하게 변경해야 합니다.   
