...
* Linux 시스템을 설치할 때 LVM이 아닌 표준 파티션을 사용 하는 *것이 좋습니다* (종종 대부분의 설치에 대 한 기본값). 이렇게 하면 특히 문제 해결을 위해 OS 디스크를 다른 VM에 연결해야 하는 경우 복제된 VM과 LVM 이름이 충돌하지 않도록 방지합니다. 데이터 디스크에서 [LVM](configure-lvm.md) 또는 [RAID](configure-raid.md)를 사용할 수 있습니다.
