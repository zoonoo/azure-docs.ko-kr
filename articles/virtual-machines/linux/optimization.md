...
**/etc/waagent.conf** 파일에서 두 개의 항목을 찾아 수정합니다. 전용 스왑 파일의 존재 여부 및 스왑 파일의 크기를 제어합니다. 확인해야 하는 매개 변수는 `ResourceDisk.EnableSwap` 및 `ResourceDisk.SwapSizeMB`입니다. 

적절히 설정된 디스크 및 탑재된 스왑 파일을 사용하도록 설정하려면 매개 변수의 설정이 다음과 같은지 확인합니다.
