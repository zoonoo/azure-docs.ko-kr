...
Azure에서 Linux VM(가상 컴퓨터)에서 OS(운영 체제)에 대한 기본 가상 하드 디스크 크기는 일반적으로 30GB입니다. [데이터 디스크를 추가](add-disk.md)하여 추가 저장소 공간을 제공할 수 있으나, 기존 데이터 디스크를 확장할 수도 있습니다. 이 문서에서는 Azure CLI 2.0을 사용하여 Linux VM에서 Managed Disks를 확장하는 방법을 설명합니다. [Azure CLI 1.0](expand-disks-nodejs.md)을 사용하여 관리되지 않는 OS 디스크를 확장할 수도 있습니다.
...
4. 적절한 자격 증명을 사용하여 VM에 SSH합니다. [az vm show](/cli/azure/vm#show)를 사용하여 VM의 공용 IP 주소를 가져올 수 있습니다.

    ```azurecli
    az vm show --resource-group myResourceGroup --name myVM -d --query [publicIps] --o tsv
    ```

5. 확장된 디스크를 사용하려면 기본 파티션 및 파일 시스템을 확장해야 합니다.
...
5. 파티션 크기를 조정했으면 `e2fsck`를 사용하여 파티션 일관성을 확인합니다.

    ```bash
    sudo e2fsck -f /dev/sdc1
    ```

6. 이제 파일 시스템 크기를 `resize2fs`로 조정합니다.

    ```bash
    sudo resize2fs /dev/sdc1
    ```

7. `/datadrive`와 같이 파티션을 원하는 위치로 탑재합니다.

    ```bash
    sudo mount /dev/sdc1 /datadrive
    ```

8. OS 디스크 크기를 조정했는지 확인하려면 `df -h`을 사용합니다. 다음 예제 출력에서는 데이터 드라이브(*/dev/sdc1*)가 200GB임을 보여 줍니다.
