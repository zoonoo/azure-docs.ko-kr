...
기존 RHEL VM 이미지, 예를 들어 RHEL 7.4(이미지 URN: `RedHat:RHEL:7.4:7.4.2018010506`)를 사용하는 경우 2018년 11월 21일에 만료된 SSL 클라이언트 인증서로 인해 RHUI에 연결 문제가 발생합니다. 이 문제를 해결하려면 다음 명령을 사용하여 VM에서 RHUI 클라이언트 패키지 업데이트

```bash
sudo yum update -y --disablerepo=* --enablerepo=rhui-microsoft-* rhui-azure-rhel7
```

또는 `sudo yum update`를 실행하면 다른 리포지토리에 대해 참조하는 "만료된 SSL 인증서" 오류에 관계 없이 이 패키지를 업데이트할 수도 있습니다. 업데이트를 따라 다른 RHUI 리포지토리에 대한 일반 연결을 복원해야 합니다.
