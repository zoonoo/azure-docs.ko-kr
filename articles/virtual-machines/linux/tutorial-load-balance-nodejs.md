...
# <a name="build-a-load-balanced-highly-available-application-on-linux-virtual-machines-in-azure"></a>Azure의 Linux 가상 컴퓨터에서 부하 분산된 고가용성 응용 프로그램 빌드
...
**cloud-init**는 VM 사용자 지정을 위해 널리 사용되는 방법입니다. **cloud-init**를 사용하여 패키지를 설치하고 파일을 쓸 수 있습니다. 초기 배포 동안 **cloud-init**가 실행될 경우 앱을 실행하기 위한 추가 단계가 필요하지 않습니다. 부하 분산 장치는 VM이 배포를 완료하고 앱을 실행하게 되면 트래픽을 분산하기 시작합니다. **cloud-init** 사용 방법에 대한 자세한 내용은 [cloud-init를 사용하여 생성 중인 Linux VM 사용자 지정](using-cloud-init.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)을 참조하세요.
...
![Node.js 앱 실행](./media/tutorial-load-balance-nodejs/running-nodejs-app.png)
...
이 자습서는 개별 Azure 리소스를 사용하여 고가용성 응용 프로그램 인프라를 구축합니다. 또한 Virtual Machine Scale Sets를 사용하여, 앱을 실행하는 VM의 규모(수)를 자동으로 확장 또는 축소할 수 있습니다. 다음 자습서에서 계속 설명 - [Virtual Machine Scale Sets를 사용하여 Linux에 항상 사용 가능한 응용 프로그램 만들기](tutorial-convert-to-vmss.md)

이 자습서에 도입된 고가용성 기능 중 일부에 대한 자세한 내용은 다음 정보를 참조하세요.

- [Linux 가상 컴퓨터의 가용성 관리](../windows/manage-availability.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
- [Azure CLI 샘플 스크립트](../windows/cli-samples.md)
