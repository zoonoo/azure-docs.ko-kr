...
Azure 갤러리 또는 마켓플레이스의 Linux 이미지는 다양한 파트너에서 제공되며, 여러 Linux 커뮤니티와의 작업을 통해 좀 더 다양한 인증 배포 목록을 추가합니다. 그러는 동안 이 갤러리에서 사용할 수 없는 배포의 경우, [이 페이지](classic/create-upload-vhd.md?toc=%2fazure%2fvirtual-machines%2flinux%2fclassic%2ftoc.json)의 지침에 따라 자신만의 Linux를 언제든지 가져올 수 있습니다.
...
Hyper-V 및 Azure용 LIS(Linux 통합 서비스) 드라이버는 Microsoft가 업스트림 Linux 커널로 직접 제공되는 커널 모듈입니다.  LIS 드라이버는 기본적으로 분포의 커널로 빌드되거나 이전 RHEL/CentOS 기반 분포는 [여기](http://go.microsoft.com/fwlink/?LinkID=403033&clcid=0x409)에서 별도의 다운로드로 사용 가능합니다.  LIS 드라이버에 대한 자세한 내용은 [이 문서](create-upload-generic.md#linux-kernel-requirements) 를 참조하세요.
