...
`cloud_init_modules`의 `/etc/cloud/cloud.cfg` 섹션을 업데이트하여 다음 모듈을 포함합니다.
```bash
- disk_setup
- mounts
```
...
다음 줄을 사용 하 여 선택한 편집기를 사용 하 `/etc/cloud/cloud.cfg.d/91-azure_datasource.cfg` 새 파일을 만들어 azure Linux 에이전트에 대 한 데이터 원본으로 Azure만 허용 합니다.
