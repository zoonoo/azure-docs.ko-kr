...
1. Key Vault에 이름을 지정합니다.  Azure 전반의 모든 Key Vault에는 고유한 이름이 있어야 합니다.
1. **액세스 정책** 탭에서 **볼륨 암호화를 위한 Azure Disk Encryption** 상자를 확인합니다.

    ![디스크 및 암호화 선택](../media/disk-encryption/portal-qs-keyvault-enable.png)

1. **검토 + 만들기**를 선택합니다.  
1. Key Vault가 유효성 검사를 통과한 후 **만들기**를 선택합니다. 그러면 **Azure Key Vault에서 키 선택** 화면으로 돌아갑니다.
