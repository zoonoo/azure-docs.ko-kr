...
다음 스크립트는 [ManagedBy](managed-disks-overview.md) 속성 값을 검사하여 연결되지 않은 **관리 디스크**를 찾습니다. 관리 디스크가 VM에 연결되어 있으면 **ManagedBy** 속성에 해당 VM의 리소스 ID가 포함됩니다. 관리 디스크가 연결되어 있지 않으면 **ManagedBy** 속성이 null입니다. 스크립트는 Azure 구독의 모든 관리 디스크를 검사합니다. 스크립트에서 **ManagedBy** 속성이 null로 설정된 관리 디스크를 찾을 경우 디스크가 연결되어 있지 않다고 결정합니다.
