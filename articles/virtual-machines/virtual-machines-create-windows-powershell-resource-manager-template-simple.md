...
착수하기 전에 Azure, Windows PowerShell 및 Azure PowerShell을 구성하고 사용할 준비가 되었는지 확인하려면 다음 지침을 따릅니다.
...
다음 예제에 대한 명령을 실행하고 Azure 배포 이름, 리소스 그룹 이름 및 Azure 데이터센터 위치를 제공합니다. **New-AzureResourceGroupDeployment** 명령을 실행하면 JSON 파일의 "parameters" 섹션에 매개 변수 값을 제공하라는 메시지가 표시됩니다. 매개 변수 값을 모두 지정하면 명령에서 리소스 그룹과 가상 컴퓨터를 만듭니다.
...
[Azure PowerShell 및 Azure 서비스 관리자를 사용하여 Windows 가상 컴퓨터 만들기](virtual-machines-create-windows-powershell-service-manager.md)
