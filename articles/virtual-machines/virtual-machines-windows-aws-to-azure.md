...
VHD를 업로드하여 AWS(Amazon Web Services) EC2 인스턴스를 Azure로 마이그레이션할 수 있습니다. 같은 이미지로 Azure에서 여러 VM을 만들려면 먼저 VM을 일반화한 후 일반화된 VHD를 로컬 디렉터리로 내보내야 합니다. VHD가 업로드되면 저장소에 [Managed Disks](../storage/storage-managed-disks-overview.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)를 사용하는 새 Azure VM을 만들 수 있습니다. Azure Managed Disks를 사용하면 Azure IaaS VM의 저장소 계정을 관리할 필요가 없습니다. 필요한 디스크의 유형(프리미엄 또는 표준)과 크기만 지정하면 Azure가 알아서 디스크를 만들고 관리합니다. 

이 프로세스를 시작하기 전에 [Managed Disks로 마이그레이션하기 위한 계획](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)을 검토하세요.
...
## <a name="export-the-vhd-from-an-ec2-instance"></a>EC2 인스턴스에서 VHD 내보내기
...
아직 PowerShell을 설치하지 않은 경우 [Azure PowerShell 설치 및 구성 방법](/powershell/azureps-cmdlets-docs)을 참조하세요.
...
업로드된 VHD를 사용하여 관리 디스크 또는 새 VM을 만들려면 나중에 사용할 수 있도록 **대상 URI**를 저장해 두세요.
...
## <a name="create-a-managed-image-from-the-uploaded-vhd"></a>업로드된 VHD로 관리 이미지 만들기 
...
먼저 이미지에 대한 기본 정보를 수집하고 이미지의 변수를 만들어야 합니다. 이 예제에서는 **미국 중서부** 위치의 **myResourceGroup** 리소스 그룹에 있는 **myImage**라는 관리 VM 이미지를 사용합니다. 
...
## <a name="get-the-credentials-for-the-vm"></a>VM 자격 증명 가져오기

다음 cmdlet을 사용하면 VM에 원격으로 액세스하기 위한 로컬 관리자 계정으로 사용할 새 사용자 이름 및 암호를 입력할 수 있는 창이 열립니다. 
...
1. VM 이름 및 컴퓨터 이름에 대한 변수를 만듭니다. 이 예에서는 VM 이름을 **myVM**으로 설정하고 컴퓨터 이름을 **myComputer**로 설정합니다.

    ```powershell
    $vmName = "myVM"
    $computerName = "myComputer"
    ```
2. 가상 컴퓨터의 크기를 설정합니다. 이 예에서는 **Standard_DS1_v2** 크기의 VM을 만듭니다. 자세한 내용은 [VM 크기](https://azure.microsoft.com/documentation/articles/virtual-machines-windows-sizes/) 설명서를 참조하세요.
...
## <a name="set-the-vm-image-as-source-image-for-the-new-vm"></a>새 VM의 원본 이미지로 VM 이미지 설정

관리 VM 이미지의 ID를 사용하여 원본 이미지를 설정합니다.
...
저장소 유형(PremiumLRS 또는 StandardLRS) 및 OS 디스크의 크기를 입력합니다. 이 예에서는 계정 유형을 **PremiumLRS**로, 디스크 크기를 **128GB**로, 디스크 캐싱을 **ReadWrite**로 설정합니다.
