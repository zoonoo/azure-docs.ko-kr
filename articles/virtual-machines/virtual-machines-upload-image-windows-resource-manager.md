...
이 문서에서는 Windows 기반 운영 체제에서 VHD(가상 하드 디스크)를 업로드하는 방법을 보여 주므로 이를 통해 리소스 관리자 배포 모델을 사용하여 새 Windows 가상 컴퓨터를 만들 수 있습니다. Microsoft Azure의 디스크 및 VHD에 대한 자세한 내용은 [가상 컴퓨터용 디스크 및 VHD에 대하여](virtual-machines-disks-vhds.md)를 참조하세요.
...
3. **시스템 준비 도구** 대화 상자에서 다음을 수행합니다.
	...
	![Sysprep 시작](./media/virtual-machines-upload-image-windows-resource-manager/sysprepgeneral.png)
...
4. 새 저장소 계정을 만들려면 **추가**를 클릭하고 다음 정보를 입력합니다.
	...
	5. **만들기**를 클릭합니다. 이제 계정이 **저장소 계정** 패널에 나타납니다.
		...
		![저장소 계정 세부 정보 입력](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_storage_account.png)
	...
	6. 이 단계와 다음 단계에서는 저장소 계정에서 Blob 컨테이너를 만드는 방법을 보여 줍니다. 이는 선택 사항으로, 이미지를 업로드하기 위한 PowerShell 명령으로도 이미지에 대한 새 Blob 컨테이너를 만들 수 있기 때문입니다. 직접 만들고 싶지 않으면 [VM 이미지 업로드](#uploadvm) 섹션으로 진행합니다. 그렇지 않으면 **서비스** 타일에서 **Blob**을 클릭합니다.
		...
		![Blob 서비스](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_blob.png)

	7. Blob 패널이 표시되면 **+ 컨테이너**를 클릭하여 새 Blob 저장소 컨테이너를 만듭니다. 컨테이너의 이름 및 액세스 유형을 입력합니다.

		![새 Blob 만들기](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_container.png)
...
2. [Add-AzureRmVhd](https://msdn.microsoft.com/library/mt603554.aspx)를 사용하여 일반화된 Azure VHD를 저장소 계정에 추가합니다.
	...
	위치: - **StorageAccountURL**은 저장소 계정에 대한 URL입니다. 일반적인 형식은 다음과 같습니다. `https://YourStorageAccountName.blob.core.windows.net`. **YourStorageAccountName** 대신 기존 또는 새 저장소 계정 이름을 사용해야 합니다. **BlobContainer**는 이미지를 저장하려는 Blob 컨테이너입니다. cmdlet이 이 이름의 기존 Blob 컨테이너를 찾을 수 없는 경우 새 Blob 컨테이너를 만듭니다. **TargetVHDName**은 이미지를 저장할 이름입니다. **LocalPathOfVHDFile**은 로컬 컴퓨터에서 .vhd 파일의 이름 및 전체 경로입니다.
...
Azure PowerShell을 사용하여 새 가상 컴퓨터를 관리하려면 [Azure 리소스 관리자 및 PowerShell을 사용하여 가상 컴퓨터 관리](virtual-machines-deploy-rmtemplates-powershell.md)를 참조하세요.
