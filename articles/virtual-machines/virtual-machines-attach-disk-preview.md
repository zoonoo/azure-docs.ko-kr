<properties
	pageTitle="Azure Preview 포털에서 데이터 디스크를 연결하는 방법"
	description="Azure Preview 포털에서 새 및 기존 데이터 디스크를 연결하는 방법 | Microsoft Azure"
...
3.	Azure가 디스크를 만들고 가상 컴퓨터에 연결한 후 새 디스크가 **데이터 디스크** 아래의 가상 컴퓨터의 디스크 설정에 나열됩니다.

## 옵션 2: 기존 디스크 연결

1.	**디스크** 블레이드에서 **기존 연결**을 클릭합니다.

2.	**기존 디스크 연결** 아래에서 **VHD 파일**을 클릭합니다.

	![기존 디스크 연결](./media/virtual-machines-attach-disk-preview/attach-existing.png)

3.	**저장소 계정** 아래에서 계정 및 .vhd 파일을 보관하는 컨테이너를 선택합니다.

	![VHD 위치 찾기](./media/virtual-machines-attach-disk-preview/find-storage-container.png)

4.	.vhd 파일을 업로드합니다.

5.	**기존 디스크 연결** 아래에서 방금 선택한 파일이 **VHD 파일** 아래에 나열됩니다. **확인**을 클릭합니다.

6.	Azure가 디스크를 가상 컴퓨터에 연결한 후 해당 디스크가 **데이터 디스크** 아래의 가상 컴퓨터의 디스크 설정에 나열됩니다.
