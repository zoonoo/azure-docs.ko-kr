...
이 문서에서는 Azure CLI를 사용하여 간단한 LAMP 응용 프로그램을 Ubuntu에 배포합니다.

## 필수 조건

다음 예에서는 먼저 Ubuntu 14.04를 실행하는 두 Azure VM을 만듭니다. VM은 *script-vm* 및 *lamp-vm*이라고 불립니다. VM을 만드는 경우 고유한 이름을 사용하세요. 하나는 CLI 명령을 실행하고 다른 하나는 LAMP 앱을 배포하는 데 사용합니다.
...
다음 스크립트 예제는 Ubuntu에 LAMP 스택을 설치하고(MySQL 자동 설치를 설정하는 작업도 수행함) 간단한 PHP 파일을 쓴 다음 Apache를 시작합니다.
...
원격 VM에서 로그 파일을 통해 사용자 지정 스크립트가 잘 실행되는지 확인할 수 있습니다. 다음 명령을 사용하여 *lamp-vm*에 대해 SSH를 실행하고 로그 파일의 끝부분을 확인합니다.

    cd /var/log/azure/Microsoft.OSTCExtensions.CustomScriptForLinux/1.3.0.0/
    tail -f extension.log

CustomScript 확장 프로그램을 실행한 후에 정보를 위해 만든 PHP 페이지를 찾아볼 수 있습니다. 이 문서의 예제에 나와 있는 PHP 페이지는 **http://lamp-vm.cloudapp.net/phpinfo.php*입니다.
