...
`docker-compose 1.4.1`와 같은 출력이 표시됩니다.
...
이제 `http://localhost:8080`을 탐색하여 VM에서 직접 WordPress에 연결할 수 있습니다. 인터넷을 통해 VM에 연결하려는 경우, 먼저 공용 포트 80을 개인 포트 8080에 매핑하는 VM에 대한 HTTP 끝점을 구성합니다. 예를 들어 Azure 서비스 관리 배포 시 다음 Azure CLI 명령을 실행합니다.
...
이제 WordPress 시작 화면이 표시되면 이 화면에서 설치를 완료하고 응용 프로그램을 시작할 수 있습니다.
