...
원하는 편집기에서 파일을 열고 다음과 같이 표시되도록 경로에서 /../를 제거하여 “cookbook\_path”를 수정합니다.
...
이제 knife.rb 파일이 다음과 유사하게 표시됩니다.
...
이제 ChefDK(Chef Development Kit)를 다운로드 및 설치하여 Chef Workstation을 설치합니다.
...
모든 것이 올바르게 구성되었는지 확인하려면 다음을 실행합니다.
...
이 단계에서는 로컬 컴퓨터에서 만든 Cookbook을 복사하여 Chef Hosted Server에 업로드합니다. 업로드가 완료되면 policy 탭 아래에 Cookbook이 표시됩니다.
...
명령 프롬프트로 돌아와 다음을 수행합니다.

![][10]

배포가 완료되면 포트 80을 통해 웹 서비스에 연결할 수 있어야 합니다. knife azure 명령으로 VM을 프로비전할 때 이 포트를 이미 열었기 때문입니다. 이 VM은 클라우드 서비스에 있는 유일한 VM이므로 클라우드 서비스 url과 연결해 보겠습니다.
