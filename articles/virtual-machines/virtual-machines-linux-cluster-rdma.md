<properties
 pageTitle="MPI 응용 프로그램을 실행하도록 Linux RDMA 클러스터 설정 | Microsoft Azure"
 description="RDMA를 사용하여 MPI 응용 프로그램을 실행하기 위해 크기가 A8 또는 A9인 VM의 Linux 클러스터를 만드는 방법에 대해 알아봅니다."
...
>[AZURE.NOTE]Azure Linux RDMA는 현재 SUSE Linux Enterprise Server 12(SLES 12)에서 실행되는 Intel MPI Library 버전 5.0에서 지원됩니다.
>
...
* **HPC 팩** - Azure에서 Microsoft HPC 팩 클러스터를 만들고 지원되는 Linux 배포판(HPC Pack 2012 R2 업데이트 2부터 지원)을 실행하는 계산 노드를 추가할 수 있습니다. 일부 Linux 노드는 RDMA 네트워크에 액세스하도록 구성할 수 있습니다. 시작하려면 [HPC 팩 설명서](http://go.microsoft.com/fwlink/?LinkId=617894)를 참조하세요.

* **Azure CLI 스크립트** - Mac, Linux 및 Windows용 [Azure 명령줄 인터페이스](../xplat-cli.md)(CLI)에서 사용자 고유의 스크립트를 작성하여 가상 네트워크 및 기타 Linux 클러스터를 만드는 데 필요한 구성 요소를 배포할 수 있습니다. Azure 서비스 관리(asm) 모드의 CLI는 계산 노드를 순차적으로 배포하므로 많은 계산 노드를 배포하는 경우 배포를 완료하는 데 몇 분 정도 걸릴 수 있습니다. 예제는 이 문서의 나머지 단계를 참조하세요.

* **Azure 리소스 관리자 템플릿** - 간단한 Azure 리소스 관리자 JSON 템플릿 파일을 만들고 Azure CLI arm-mode 명령을 실행하거나 Azure Preview 포털을 사용하여 여러 A8 및 A9 Linux VM을 배포할 수 있으며, 가상 네트워크, 고정 IP 주소, DNS 설정 및 그 밖에 RDMA 네트워크를 활용하고 MPI 작업을 실행할 수 있는 계산 클러스터를 만드는 데 필요한 리소스를 정의할 수 있습니다. [사용자 고유의 템플릿을 만들거나](../resource-group-authoring-templates.md) [Azure 빠른 시작 템플릿 페이지](https://azure.microsoft.com/documentation/templates/)에서 Microsoft 또는 커뮤니티가 참여한 템플릿을 확인하여 원하는 솔루션을 배포할 수 있습니다. 리소스 관리자 템플릿은 일반적으로 Linux 클러스터를 배포하는 가장 빠르고 안정적인 방법을 제공합니다.
...
* **클라이언트 컴퓨터** - Azure와 통신할 Mac, Linux 또는 Windows 기반 클라이언트가 필요합니다.
...
* **Azure CLI** - Azure CLI를 [설치](../xplat-cli-install.md)하고 클라이언트 컴퓨터에서 Azure 구독을 사용하도록 [구성](../xplat-cli-connect.md)합니다.


### SLES 12 VM 프로비전

Azure CLI에서 Azure에 로그인 한 후 `azure config list` 명령을 실행하여 CLI가 Azure 서비스 관리(asm) 모드에 있는지 확인합니다. 그렇지 않은 경우 다음 명령을 실행하여 모드를 설정합니다.
...
* 외부 SSH 포트 번호(이 예제의 경우 10004)는 유효한 모든 포트 번호이고, 내부 SSH 포트 번호는 22로 설정됩니다.
...
VM 프로비전이 완료되면 VM의 외부 IP 주소(또는 DNS 이름) 및 구성한 외부 포트 번호를 사용하여 VM에 SSH 연결한 후 VM을 사용자 지정합니다. 연결에 대한 자세한 내용은 [Linux를 실행하는 가상 컴퓨터에 로그온하는 방법](virtual-machines-linux-how-to-log-on.md)을 참조하세요.

>[AZURE.NOTE]Microsoft Azure에서는 Linux VM에 대한 루트 액세스를 제공하지 않습니다. 사용자로 연결된 경우 관리 액세스 권한을 얻으려면 `sudo –s`를 사용하면 됩니다.

**업데이트** - **zypper** 및 NFS 유틸리티를 사용하여 업데이트를 설치할 수 있습니다.
...
**메모리 잠금** - MPI 코드를 사용하여 RDMA에 사용 가능한 메모리를 잠그려면 /etc/security/limits.conf 파일에서 다음 설정을 추가하거나 변경해야 합니다.
...
**SSH 키** - 클러스터의 모든 계산 노드 간에 이 VM을 만드는 데 사용한 사용자 이름 및 암호에 대한 트러스트를 설정합니다. 다음 명령을 사용하여 SSH 키를 만듭니다.

```
$ ssh-keygen
```

기본 위치에 공개 키를 저장하고 입력한 암호를 기억합니다.
...
또는 다음과 같이 클러스터의 각 VM에 대한 개인 네트워크 IP 주소를 나열할 수 있습니다.
...
>[AZURE.NOTE]`StrictHostKeyChecking no`를 구성하면 위와 같이 특정 IP 주소 또는 범위를 지정하지 않은 경우 잠재적인 보안 위험이 발생할 수 있습니다.

**응용 프로그램** - 이미지를 캡처하기 전에 이 VM에 필요한 응용 프로그램을 복사하거나 다른 사용자 지정을 수행합니다.

### 이미지 캡처

이미지를 캡처하려면 먼저 Linux VM에서 다음 명령을 실행합니다.
...
다음 스크립트를 사용자 환경에 적절한 값으로 수정하여 클라이언트 컴퓨터에서 실행합니다. ASM 배포 방법에서는 VM을 순차적으로 배포하려면 이 스크립트에 제안된 8대의 A9 VM을 배포하는 데 몇 분 정도 걸립니다.
...
Azure Linux RDMA에서 MPI 응용 프로그램을 실행하려면 Intel MPI에 특정한 특정 환경 변수를 구성해야 합니다. 변수를 구성하고 응용 프로그램을 실행하는 샘플 스크립트는 다음과 같습니다.
...
호스트 파일의 형식은 다음과 같습니다. 클러스터의 각 노드에 대해 한 줄을 추가합니다. DNS 이름이 아니라 이전에 정의된 VNet에서 개인 IP 주소를 지정합니다.
