...
새 VM을 만드는데 특수화된 VHD를 그대로 사용하려는 경우 다음 단계가 완료되었는지 확인합니다. 
  * Managed Disks를 사용하려는 경우 [Managed Disks로 마이그레이션 계획 수립](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)을 검토합니다.
...
선택 사항: 추가 Managed Disks를 데이터 디스크로 연결합니다. 이 옵션은 사용자가 [관리되는 데이터 디스크 만들기](virtual-machines-windows-create-managed-disk-ps.md)에 따라 관리되는 데이터 디스크를 만들었다고 가정합니다. 
