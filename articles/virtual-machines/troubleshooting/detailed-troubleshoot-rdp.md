...
## <a name="source-1-remote-desktop-client-computer"></a>발생 지 1: 원격 데스크톱 클라이언트 컴퓨터
...
## <a name="source-2-organization-intranet-edge-device"></a>원본 2: 조직 인트라넷 에지 디바이스
...
> [!NOTE]
> 리소스 관리자에서 만든 virtual machines 건너뜁니다 [발생 지 4: 네트워크 보안 그룹](#source-4-network-security-groups)합니다.
...
* 대상 VM의 원격 데스크톱 트래픽에 대 한 끝점 구성: 끝점의 개인 TCP 포트는 VM의 원격 데스크톱 서비스가 수신 대기 중인 TCP 포트와 일치 해야 합니다 (기본값인 3389).
* 대상 VM의 원격 데스크톱 트래픽 끝점에 대 한 ACL. Acl을 사용 하면 트래픽을 허용 또는 거부 들어오는 원본 IP 주소에 따라 인터넷에서 지정할 수 있습니다. ACL이 잘못 구성될 경우 엔드포인트에 원격 데스크톱 트래픽이 들어오지 못할 수 있습니다. ACL을 확인하고 프록시 또는 다른 에지 서버의 공용 IP 주소에서 들어오는 트래픽이 허용되어 있는지 확인하세요. 자세한 내용은 [네트워크 ACL(Access Control 목록)이란?](../../virtual-network/virtual-networks-acl.md)
...
## <a name="source-5-windows-based-azure-vm"></a>발생 지 5: Windows 기반 Azure VM
...
[Azure 가상 머신에서 실행 중인 애플리케이션에 대한 액세스 문제 해결](../linux/troubleshoot-app-connection.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
