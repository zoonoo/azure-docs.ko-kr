...
* 기본 [문제 해결 가이드에서 일반적인 RDP 오류에 대한 빠른 수정 단계를](troubleshoot-rdp-connection.md#quick-troubleshooting-steps)따릅니다.
...
* [조직 인트라넷 에지 디바이스](#source-2-organization-intranet-edge-device)
...
* 대상 VM에서 원격 데스크톱 트래픽 엔드포인트에 대한 ACL: ACL은 인터넷에서 들어오는 트래픽을 원본 IP 주소에 따라 허용 또는 거부하도록 지정하는 데 사용됩니다. ACL이 잘못 구성될 경우 엔드포인트에 원격 데스크톱 트래픽이 들어오지 못할 수 있습니다. ACL을 확인하고 프록시 또는 다른 에지 서버의 공용 IP 주소에서 들어오는 트래픽이 허용되어 있는지 확인하세요. 자세한 내용은 [ACL(네트워크 액세스 제어 목록)이란 무엇입니까?](../../virtual-network/virtual-networks-acl.md)

엔드포인트가 문제의 발생지인지 확인하려면 현재 엔드포인트를 제거하고 새 엔드포인트를 만든 후 외부 포트 번호에 49152-65535 범위의 임의 포트를 선택합니다. 자세한 내용은 [가상 시스템에 끝점을 설정하는 방법을](../windows/classic/setup-endpoints.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fclassic%2ftoc.json)참조하십시오.
...
[IP 흐름 확인을](../../network-watcher/network-watcher-check-ip-flow-verify-portal.md) 사용하여 네트워크 보안 그룹의 규칙이 가상 컴퓨터의 트래픽을 차단하고 있는지 확인합니다. 효과적인 보안 그룹 규칙을 검토하여 인바운드 "허용" NSG 규칙이 있는지와 해당 규칙이 RDP 포트(기본값: 3389)에 우선적으로 사용되도록 설정되어 있는지 확인합니다. 자세한 내용은 [효과적인 보안 규칙을 사용하여 VM 트래픽 흐름 문제를 해결합니다.](../../virtual-network/diagnose-network-traffic-filter-problem.md)

## <a name="source-5-windows-based-azure-vm"></a>발생지 5: Windows 기반 Azure VM
![](./media/detailed-troubleshoot-rdp/tshootrdp_5.png)

[이 문서의](../windows/reset-rdp.md)지침을 따르십시오. 이 문서는 가상 머신에서 원격 데스크톱 서비스를 다시 설정합니다.
