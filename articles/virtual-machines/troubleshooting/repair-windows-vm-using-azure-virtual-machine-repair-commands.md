---
title: Azure 가상 시스템 복구 명령을 사용하여 Windows VM 복구 | 마이크로 소프트 문서
description: 이 문서에서는 Azure VM 복구 명령을 사용하여 디스크를 다른 Windows VM에 연결하여 오류를 수정한 다음 원래 VM을 다시 빌드하는 방법을 자세히 설명합니다.
...
Azure의 Windows 가상 컴퓨터(VM)에 부팅 또는 디스크 오류가 발생하면 디스크 자체에서 완화작업을 수행해야 할 수 있습니다. 일반적인 예로는 애플리케이션 업데이트가 실패하여 VM이 성공적으로 부팅되지 않는 경우입니다. 이 문서에서는 Azure VM 복구 명령을 사용하여 디스크를 다른 Windows VM에 연결하여 오류를 수정한 다음 원래 VM을 다시 빌드하는 방법을 자세히 설명합니다.
...
2. az 확장 추가/업데이트를 실행합니다.
3. az vm 복구 만들기를 실행합니다.
4. az vm 복구 실행을 실행합니다.
5. az vm 복구 복원을 실행합니다.
...
> [!NOTE]
> * 스크립트를 실행하려면 VM(포트 443)의 아웃바운드 연결이 필요합니다.
...
1. Azure Cloud Shell 시작
   ...
   클라우드 셸을 열려면 코드 블록의 오른쪽 위 모서리에서 **시도를** 선택합니다. 을 방문하여 [https://shell.azure.com](https://shell.azure.com)별도의 브라우저 탭에서 클라우드 셸을 열 수도 있습니다.
...
4. `az vm repair run`을 실행합니다. 이 명령은 복구 VM을 통해 연결된 디스크에서 지정된 복구 스크립트를 실행합니다.
