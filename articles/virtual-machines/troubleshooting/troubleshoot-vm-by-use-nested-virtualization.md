---
title: Azure에서 중첩된 가상화를 사용하여 잘못된 Azure VM 문제 해결 | 마이크로 소프트 문서
...
---
# <a name="troubleshoot-a-faulty-azure-vm-by-using-nested-virtualization-in-azure"></a>Azure에서 중첩된 가상화를 사용하여 잘못된 Azure VM 문제 해결

이 문서에서는 Microsoft Azure에서 중첩된 가상화 환경을 만드는 방법을 보여 주므로 문제 해결을 위해 Hyper-V 호스트(Rescue VM)에 결함이 있는 VM의 디스크를 탑재할 수 있습니다.

## <a name="prerequisites"></a>사전 요구 사항

결함이 있는 VM을 탑재하려면 Rescue VM에서 결함이 있는 VM과 동일한 유형의 저장소 계정(표준 또는 프리미엄)을 사용해야 합니다.
...
1.  새 복구 VM을 만듭니다.
    ...
    -  결함이 있는 VM과 동일한 위치, 저장소 계정 및 리소스 그룹입니다.

    -  결함이 있는 VM(표준 또는 프리미엄)과 동일한 저장소 유형을 선택합니다.

2.  복구 VM을 만든 후 복구 VM에 데스크톱을 원격 연결합니다.

3.  서버 관리자에서 **Manage** > **역할 및 기능 추가**관리를 선택합니다.

4.  **설치 유형** 섹션에서 **역할 기반 또는 기능 기반 설치**를 선택합니다.

5.  **대상 서버 선택** 섹션에서 복구 VM이 선택되어 있는지 확인합니다.

6.  **하이퍼-V 역할** > **추가 피처를**선택합니다.
...
## <a name="step-2-create-the-faulty-vm-on-the-rescue-vms-hyper-v-server"></a>2단계: 레스큐 VM의 하이퍼 V 서버에서 결함이 있는 VM 만들기

1.  문제가 있는 VM의 OS 디스크에 대한 [스냅숏 디스크를 만든](troubleshoot-recovery-disks-portal-windows.md#take-a-snapshot-of-the-os-disk) 다음 스냅숏 디스크를 recuse VM에 연결합니다.

2.  구조 VM에 원격 데스크톱.

3.  디스크 관리(diskmgmt.msc)를 엽니다. 결함이 있는 VM의 디스크가 **오프라인으로**설정되어 있는지 확인합니다.

4.  Hyper-V 관리자 열기: **서버 관리자**에서 **Hyper-V 역할**을 선택합니다. 서버를 마우스 오른쪽 단추로 클릭한 다음 **Hyper-V 관리자**를 선택합니다.

5.  하이퍼 V 관리자에서 구조 VM을 마우스 오른쪽 단추로 클릭한 다음 **New** > **Virtual Machine** > **새**가상 컴퓨터를 다음에 선택합니다.
...
14. **물리적 하드 디스크에서**Azure VM에 연결한 결함이 있는 VM의 디스크를 선택합니다. 디스크가 나열되지 않으면 디스크 관리를 사용하여 디스크가 오프라인으로 설정되어 있는지 확인합니다.
...
## <a name="step-3-replace-the-os-disk-used-by-the-faulty-vm"></a>3단계: 결함이 있는 VM에서 사용하는 OS 디스크 교체

1.  VM을 온라인으로 다시 가져온 후 Hyper-V 관리자에서 VM을 종료합니다.

2.  [복구된 OS 디스크를 마운트 해제하고 분리합니다.](troubleshoot-recovery-disks-portal-windows.md#unmount-and-detach-original-virtual-hard-disk
)
3.  [VM에서 사용하는 OS 디스크를 복구된 OS 디스크로 교체합니다.](troubleshoot-recovery-disks-portal-windows.md#swap-the-os-disk-for-the-vm
)
