---
title: VM 시작은 다음 상태에서 중지되었습니다. "Windows가 준비 중입니다. 컴퓨터를 끄지 마세요." (Azure) | Microsoft Docs
description: VM 시작이 "Windows가 준비 중입니다. 컴퓨터를 끄지 마세요."에서 중지되는 문제를 해결하는 단계를 소개합니다.
...
이 문서에서는 VM(가상 머신)이 시작 중에 “Windows가 준비 중입니다. 컴퓨터를 끄지 마세요." 단계에서 중지되는 문제를 해결하도록 도와줍니다.

## <a name="symptoms"></a>증상

**부트 진단**을 사용하여 VM의 스크린샷을 확인하면 운영 체제가 완전히 시작되지 않은 것을 알 수 있습니다. 또한 VM에서 **“Windows가 준비 중입니다. 컴퓨터를 끄지 마세요.”** 메시지가 표시됩니다.

![메시지 예제](./media/troubleshoot-vm-configure-update-boot/message1.png)

![메시지 예제](./media/troubleshoot-vm-configure-update-boot/message2.png)

## <a name="cause"></a>원인

일반적으로 이 문제는 구성이 변경된 후 서버가 마지막 다시 부팅을 수행하는 동안 발생합니다. 구성 변경은 Windows 업데이트로 인해 또는 서버 역할/기능 변경으로 인해 초기화될 수 있습니다. Windows 업데이트의 경우, 업데이트의 크기가 크면 운영 체제에 변경 내용을 다시 구성하는 데 더 많은 시간이 걸립니다.
...
VM에 암호화된 VM이 있는지 확인합니다. 이렇게 하려면 다음 단계를 수행하세요.

1. 포털에서 VM을 열고 디스크를 찾습니다.

2. 암호화 설정 여부를 나타내는 “암호화” 열이 표시됩니다.

OS 디스크가 암호화되어 있으면 암호화된 디스크의 잠금을 해제합니다. 이렇게 하려면 다음 단계를 수행하세요.

1. 영향을 받는 VM과 동일한 리소스 그룹, 저장소 계정 및 위치에 있는 복구 VM을 만듭니다.

2. Azure Portal에서 영향을 받는 VM을 삭제하고 디스크를 유지합니다.
...
6. Base64로 인코딩된 값을 바이트로 변환하고 출력을 파일에 씁니다. 
    ...
    > [!Note]
    > USB 잠금 해제 옵션을 사용하는 경우 BEK 파일 이름이 원래 BEK GUID와 일치해야 합니다. 또한 다음 단계를 수행하려면 먼저 C 드라이브에 “BEK”라는 폴더를 만들어야 합니다.
...
7. PC에서 BEK 파일이 만들어지면 해당 파일을 잠근 OS 디스크가 연결된 복구 VM으로 복사합니다. BEK 파일 위치를 사용하여 다음을 실행합니다.

    ```Powershell
    manage-bde -status F:
    manage-bde -unlock F: -rk C:\BEKFILENAME.BEK
    ```
    일부 시나리오에서는 **경우에 따라** 이 명령으로 디스크 암호를 해독하는 작업도 필요할 수 있습니다.
    ...
    > [!Note]
    > 여기서는 암호화할 디스크가 문자 F:에 있다고 간주합니다.

8. 로그를 수집해야 할 경우 경로 **드라이브 문자: \Windows\System32\winevt\Logs**로 이동할 수 있습니다.

9. 복구 컴퓨터에서 드라이브를 분리합니다.

### <a name="create-a-snapshot"></a>스냅숏 만들기

스냅숏을 만들려면 [디스크 스냅숏](..\windows\snapshot-copy-managed-disk.md)의 단계를 따릅니다.

## <a name="collect-an-os-memory-dump"></a>OS 메모리 덤프 수집

구성 중에 VM이 중단되면 [OS 덤프 수집](troubleshoot-common-blue-screen-error.md#collect-memory-dump-file) 섹션의 단계를 사용하여 OS 덤프를 수집합니다.

## <a name="contact-microsoft-support"></a>Microsoft 지원에 문의

덤프 파일을 수집한 후 [Microsoft 지원 서비스](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade)에 문의하여 근본 원인을 분석합니다.


## <a name="rebuild-the-vm-using-powershell"></a>PowerShell을 사용하여 VM 다시 작성

메모리 덤프 파일을 수집한 후 다음 단계를 사용하여 VM을 다시 작성해야 합니다.
