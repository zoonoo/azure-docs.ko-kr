...
제목: Azure CLI에서 Linux 문제 해결 VM 사용 | Microsoft Docs 설명: Azure CLI 서비스를 사용하여 OS 디스크를 복구 VM에 연결하여 Linux VM 문제 해결 방법 알아보기: virtual-machines-linux documentationCenter: '' authors: genlin manager: jeconnoc editor: ''
...
VM에 연결하는 데 문제가 있는 경우 [Azure VM에 SSH 연결 문제 해결](troubleshoot-ssh-connection.md)을 참조하세요. VM에서 실행 중인 응용 프로그램에 액세스하는 데 문제가 있는 경우 [Linux VM에서 응용 프로그램 연결 문제 해결](troubleshoot-app-connection.md)을 참조하세요.
