...
5. **VM Resource Health를 확인**합니다. 이 문제 해결 단계에서는 Azure 플랫폼에 VM 연결에 영향을 줄 수 있는 알려진 문제가 없는지 확인합니다.
   
    Azure Portal에서 VM을 선택합니다. 설정 창을 아래로 스크롤하여 목록 아래쪽의 **지원 + 문제 해결** 섹션으로 이동 합니다. **리소스 상태** 단추를 클릭 합니다. 정상 VM은 **사용 가능**으로 보고합니다.
...
6. **사용자 자격 증명 다시 설정**. 이 문제 해결 단계에서는 자격 증명이 확실하지 않거나 잊어버린 경우 로컬 관리자 계정에서 암호를 다시 설정합니다.  VM에 로그인하면 해당 사용자의 암호를 다시 설정해야 합니다.
    ...
    Azure Portal에서 VM을 선택합니다. 설정 창을 아래로 스크롤하여 목록 아래쪽의 **지원 + 문제 해결** 섹션으로 이동 합니다. **암호 다시 설정** 단추를 클릭 합니다. **모드**를 **암호 다시 설정**으로 지정한 다음 사용자 이름 및 새 암호를 입력합니다. 마지막으로 **업데이트** 단추를 클릭합니다.
   
    ![Azure Portal에서 사용자 자격 증명 다시 설정](./media/troubleshoot-rdp-connection/reset-password.png)
7. **VM을 다시 시작**합니다. 이 문제 해결 단계에서는 VM 자체의 기본 문제를 해결할 수 있습니다.
...
8. **VM을 다시 배포**합니다. 이 문제 해결 단계에서는 Azure 내의 다른 호스트에 VM을 다시 배포하여 기본 플랫폼 또는 네트워킹 문제를 해결합니다.
   
    Azure Portal에서 VM을 선택합니다. 설정 창을 아래로 스크롤하여 목록 아래쪽의 **지원 + 문제 해결** 섹션으로 이동 합니다. **다시 배포** 단추를 클릭한 다음 **다시 배포**를 클릭합니다.
...
3. **사용자 자격 증명 다시 설정**. 이 문제 해결 단계에서는 자격 증명이 확실하지 않거나 잊어버린 경우 사용자가 지정하는 로컬 관리자 계정에서 암호를 다시 설정합니다.
    ...
    ```
4. **VM을 다시 시작**합니다. 이 문제 해결 단계에서는 VM 자체의 기본 문제를 해결할 수 있습니다.
    ...
    ```
5. **VM을 다시 배포**합니다. 이 문제 해결 단계에서는 Azure 내의 다른 호스트에 VM을 다시 배포하여 기본 플랫폼 또는 네트워킹 문제를 해결합니다.
...
4. **VM Resource Health를 확인**합니다. 이 문제 해결 단계에서는 Azure 플랫폼에 VM 연결에 영향을 줄 수 있는 알려진 문제가 없는지 확인합니다.
   
    Azure Portal에서 VM을 선택합니다. 설정 창을 아래로 스크롤하여 목록 아래쪽의 **지원 + 문제 해결** 섹션으로 이동 합니다. **리소스 상태** 단추를 클릭합니다. 정상 VM은 **사용 가능**으로 보고합니다.
...
5. **사용자 자격 증명 다시 설정**. 이 문제 해결 단계에서는 자격 증명이 확실하지 않거나 잊어버린 경우 사용자가 지정하는 로컬 관리자 계정에서 암호를 다시 설정합니다.  VM에 로그인하면 해당 사용자의 암호를 다시 설정해야 합니다.
    ...
    Azure Portal에서 VM을 선택합니다. 설정 창을 아래로 스크롤하여 목록 아래쪽의 **지원 + 문제 해결** 섹션으로 이동 합니다. **암호 다시 설정** 단추를 클릭 합니다. 사용자 이름 및 새 암호를 입력합니다. 마지막으로 **저장** 단추를 클릭 합니다.
   
    ![Azure Portal에서 사용자 자격 증명 다시 설정](./media/troubleshoot-rdp-connection/classic-reset-password.png)
6. **VM을 다시 시작**합니다. 이 문제 해결 단계에서는 VM 자체의 기본 문제를 해결할 수 있습니다.
...
* [라이선스를 제공 하는 데 사용할 수 있는 원격 데스크톱 라이선스 서버가 없으므로 원격 세션의 연결을 끊었습니다](troubleshoot-specific-rdp-errors.md#rdplicense).
* [원격 데스크톱이 컴퓨터 "name"을 찾을 수 없습니다](troubleshoot-specific-rdp-errors.md#rdpname).
* [인증 오류가 발생했습니다. 로컬 보안 기관에 연결할 수 없습니다.](troubleshoot-specific-rdp-errors.md#rdpauth)
* [Windows 보안 오류: 자격 증명이 작동 하지 않습니다](troubleshoot-specific-rdp-errors.md#wincred).
