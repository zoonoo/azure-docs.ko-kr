...
SQL Server를 실행하는 두 대의 가상 컴퓨터가 있습니다. 한 가상 컴퓨터는 가용성 그룹의 주 데이터베이스 복제본을 포함하며 두 번째 가상 컴퓨터는 보조(백업) 복제본을 포함합니다. 고가용성을 지원하기 위해 백업이 있습니다. 추가 가상 컴퓨터는 클러스터 주 노드용입니다.
...
다음 절차를 두 번 사용하여(SQL 서버를 실행하는 각 가상 컴퓨터에 한 번씩) 다른 데이터 디스크를 추가하고 새 볼륨에 폴더를 만듭니다.

### 빈 디스크를 초기화하고 폴더를 추가하려면
...
10. 완료되면 **닫기**를 클릭합니다.
11. Windows PowerShell 프롬프트에서 다음 명령을 실행합니다.
	- md f:\\Data
	- md f:\\Log
	- md f:\\Backup

다음 절차를 두 번(SQL Server를 실행하는 각 가상 컴퓨터에 대해 한 번씩) 사용하여 새 데이터베이스와 계정 및 권한에 F: 드라이브를 사용하도록 구성합니다.
...
9.	**로그인 이름**에 *도메인*\\sqladmin를 입력합니다. 여기서 *도메인*은 [2단계](virtual-machines-windows-ps-lob-ph2.md)에서 만든 sqladmin 계정의 도메인 이름입니다. 
10.	**페이지 선택**에서 **서버 역할**, **sysadmin**, **확인**을 차례로 클릭합니다.
11.	SQL Server 2014 Management Studio를 닫습니다.

다음 절차를 두 번(각 SQL Server에 대해 한 번씩) 수행하여 sqladmin 계정을 사용한 원격 데스크톱 연결을 허용합니다.
...
원하는 원격 데스크톱 클라이언트를 사용하여 클러스터 주 노드 서버 가상 컴퓨터에 대한 원격 데스크톱 연결을 만듭니다. 인트라넷 DNS 또는 컴퓨터 이름 및 로컬 관리자 계정의 자격 증명을 사용합니다.

Windows PowerShell에서 적합한 AD DS 도메인을 클러스터 주 노드 서버와 연결합니다.
...
다시 시작된 후 로컬 관리자 권한이 있는 계정과 다시 연결합니다.

## Windows Server 장애 조치 클러스터링 클러스터 만들기
...
- SQL Server를 실행하는 주 가상 컴퓨터
- SQL Server를 실행하는 보조 가상 컴퓨터
- 클러스터 주 노드

장애 조치(failover) 클러스터에는 VM이 3개 이상 필요합니다. 두 컴퓨터는 SQL Server를 호스팅하는데, 이 때 주 컴퓨터에 장애가 발생하더라도 데이터 손실이 없도록 하기 위해 보조 가상 컴퓨터는 동기 보조 복제본입니다. 세 번째 가상 컴퓨터는 SQL Server를 호스팅하지 않아도 됩니다. 클러스터 주 노드 기능은 WSFC에서 쿼럼을 제공합니다. WSFC 클러스터는 쿼럼을 사용하여 상태를 모니터링하므로 WSFC 클러스터가 온라인 상태로 유지되도록 항상 주 노드가 있어야 합니다. 클러스터에 컴퓨터가 두 개뿐인데 그 중 하나에 오류가 발생하면 주 노드가 없어지게 됩니다. 자세한 내용은 [WSFC 쿼럼 모드 및 응답 구성(SQL Server)](http://msdn.microsoft.com/library/hh270280.aspx)을 참조하세요.

두 SQL Server 가상 컴퓨터 및 클러스터 주 노드에 대해 관리자 수준의 Windows PowerShell 프롬프트에서 다음 명령을 실행합니다.
...
1.	[2단계](virtual-machines-windows-ps-lob-ph2.md)에서 만든 sqladmin 계정으로 주 SQL Server 가상 컴퓨터에 로그온합니다.
...
8.	sqladmin 계정으로 보조 SQL Server 가상 컴퓨터에 로그온한 다음 2-7단계를 반복합니다. 
