...
Azure 가상 컴퓨터에서 실행중인 응용 프로그램에 액세스 할 수 없는 경우, 이 문서는 문제의 원인을 축출하고 해당 문제를 수정하기 위한 체계적인 방법을 설명합니다.

> [AZURE.NOTE]Azure 가상 머신에 연결에 대한 도움말은, [Windows 기반 Azure 가상 컴퓨터에 대한 원격 데스크톱 연결 문제 해결](virtual-machines-troubleshoot-remote-desktop-connections.md) 또는 [Linux 기반 Azure 가상 컴퓨터에 대한 보안 셸(SSH)연결 문제 해결](virtual-machines-troubleshoot-ssh-connections.md)을 참조하세요.
...
응용 프로그램에 액세스할 수 있는 경우 [2 단계](#step2)로 이동하세요.
...
Windows 및 Linux 기반 가상 컴퓨터 모두에서, 활성 수신 포트를 표시하기 위해 **netstat -a** 명령어를 사용하세요. 응용 프로그램이 수신해야 할 예상되는 포트에 대한 출력을 검토하세요 응용 프로그램을 다시 시작하거나 필요에 따라 예상되는 포트를 사용하도록 구성하세요
...
응용 프로그램에 액세스 할 수 있는 경우, [3 단계](#step3)로 이동합니다.
...
Windows 기반 가상 컴퓨터에서, 방화벽 규칙이 사용자의 응용프로그램의 인바운드 및 아웃 바운드 트래픽을 제외할지 여부를 확인하려면 고급 보안이 포함된 Windows 방화벽을 사용하세요.

## <a id="step3"></a> 3 단계: 가상 네트워크 외부에 있지만 사용자의 컴퓨터와 동일한 네트워크에 연결되지 않은 컴퓨터에서 사용자가 응용 프로그램에 액세스 할 수 있습니까?
...
- 가상 컴퓨터에 대한 끝점 구성은 서비스 관리에서 만든 가상 컴퓨터에 대해, 수신 트래픽을 허용하며, 특히 프로토콜 (TCP 또는 UDP) 및 공용 및 개인 포트 번호가 해당됩니다. 자세한 내용은 [가상 컴퓨터로 끝점을 설정하는 방법](virtual-machines-set-up-endpoints.md)을 참조하세요.
- 서비스 관리에서 만든 가상 컴퓨터에 대해, 끝점상의 액세스 제어 목록은 인트라넷에서 들어오는 트래픽을 막지 않습니다. 자세한 내용은 [가상 컴퓨터로 끝점을 설정하는 방법](virtual-machines-set-up-endpoints.md)을 참조하세요.
- 가상 컴퓨터에 대한 끝점 구성은 리소스 관리자에서 만든 가상 컴퓨터에 대하여, 수신 트래픽을 허용하며, 특히 프로토콜 (TCP 또는 UDP) 및 공용 및 개인 포트 번호가 해당됩니다. 
- 네트워크 보안 그룹은 인바운드 요청 및 아웃 바운드 요청 트래픽을 허용합니다. 자세한 내용은 [네트워크 보안 그룹(NSG)은 무엇입니까?](virtual-networks-nsg.md)를 참조하세요.

가상 컴퓨터 또는 끝점이 부하 분산 집합의 구성원인 경우:

- 프로브 프로토콜(TCP 또는 UDP) 및 포트 번호가 올바른지 확인합니다.
- 프로브 프로토콜 및 포트가 부하 분산 집합 프로토콜 및 포트와 다른 경우:
	- 응용 프로그램이 프로브 프로토콜(TCP 또는 UDP) 및 포트 번호(가상 컴퓨터에서 **netstat –a**를 사용합니다)에서 수신 중인지 확인합니다.
	- 대상 가상 컴퓨터의 호스트 방화벽이 인바운드 프로브 요청 및 아웃 바운드 프로브 응답 트래픽을 허용합니다.

응용 프로그램에 액세스 할 수 있는 경우, 인터넷 가장자리 장치가 다음을 허용하는지 확인합니다.
...
이 문서에서 1 단계에서 3 단계까지 실행하였지만, 문제를 수정을 위해 추가적인 도움이 필요한 경우, 다음을 수행할 수 있습니다.
...
- Azure 지원 인시던트 제출 [Azure 지원 사이트](http://azure.microsoft.com/support/options/)로 이동, **기술 및 대금 청구 지원** 아래의 **지원받기**를 클릭하세요.
