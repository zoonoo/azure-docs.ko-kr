<properties
   ...
   description="클래식 배포 모델 및 Azure CLI를 사용하여 여러 VM 배포를 만드는 방법을 알아봅니다."
...
이 스크립트는 두 개의 데이터 디스크가 연결되어 있는 클라우드 서비스 **servercs**에 VM 1개(**servervm**)를 만들고 클라우드 서비스 **workercs**에 VM 2개(**clientvm1, clientvm2**)를 만듭니다. 두 클라우드 서비스는 모두 VNET **samplevnet**에 배치됩니다. **servercs** 클라우드 서비스에는 외부 연결용 끝점도 구성되어 있습니다.
