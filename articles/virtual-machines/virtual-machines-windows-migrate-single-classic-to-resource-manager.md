---
title: "클래식 배포 모델에서 Azure Managed Disks로 단일 VM 마이그레이션 | Microsoft Docs"
...
# <a name="migrate-a-single-azure-vm-from-classic-to-managed-disks"></a>클래식에서 Managed Disks로 단일 Azure VM 마이그레이션 
...
3.  마이그레이션하는 지역에서 사용할 수 있는 정확한 VM 크기를 결정합니다. VM 크기는 현재 포함하고 있는 데이터 디스크 수를 지원할 만큼 충분히 커야 합니다. 예를 들어,&4;개의 데이터 디스크가 있는 경우 VM에는 두 개 이상의 코어가 있어야 합니다. 또한 처리 능력, 메모리 및 네트워크 대역폭 요구 사항을 고려합니다.
