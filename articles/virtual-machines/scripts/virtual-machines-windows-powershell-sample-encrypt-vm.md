---
title: Azure PowerShell 스크립트 샘플 - Windows VM 암호화
...
| [New-AzKeyVault](/powershell/module/az.keyvault/new-azkeyvault) | 암호화 키와 같은 보안 데이터를 저장하기 위한 Azure Key Vault를 만듭니다. |
| [Add-AzKeyVaultKey](/powershell/module/az.keyvault/add-azkeyvaultkey) | Key Vault에서 암호화 키를 만듭니다. |
| [New-AzADServicePrincipal](/powershell/module/az.resources/new-azadserviceprincipal) | 암호화 키를 안전하게 인증하고 암호화 키에 대한 액세스를 제어하기 위한 Azure Active Directory 서비스 주체를 만듭니다. |
...
| [Get-AzKeyVault](/powershell/module/az.keyvault/get-azkeyvault) | Key Vault에 대한 필수 정보를 가져옵니다. |
| [Set-AzVMDiskEncryptionExtension](/powershell/module/az.compute/set-azvmdiskencryptionextension) | 서비스 주체 자격 증명 및 암호화 키를 사용하여 VM에 대해 암호화를 사용하도록 설정합니다. |
| [Get-AzVmDiskEncryptionStatus](/powershell/module/az.compute/get-azvmdiskencryptionstatus) | VM 암호화 프로세스의 상태를 표시합니다. |
