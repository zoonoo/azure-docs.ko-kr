...
Azure 인프라 서비스에서 SQL Server AlwaysOn 가용성 그룹을 사용하여 인트라넷 전용 SharePoint 2013 팜을 배포하는 이 단계에서는 온-프레미스 네트워크로 VPN 또는 Express 경로 연결을 통해 인증 트래픽을 보내는 대신 Azure 가상 네트워크에서 SharePoint 팜 리소스에 대한 클라이언트 웹 요청을 인증할 수 있도록 Azure 가상 네트워크에서 도메인 컨트롤러 두 개를 구성합니다.
...
PowerShell 명령의 다음 블록을 사용하여 두 도메인 컨트롤러용 가상 컴퓨터를 만듭니다. < and > 문자를 제거하고 변수의 값을 지정합니다. 이 PowerShell 명령 집합은 다음 표의 값을 사용합니다.
...
### <a id="logon"></a>원격 데스크톱 연결을 사용하여 가상 컴퓨터에 로그온

1.	Azure 관리 포털의 왼쪽 패널에서 **가상 컴퓨터**를 클릭합니다.
2.	VM에 연결하려면 VM 이름 옆의 **상태** 열에서 **실행**을 클릭합니다.
3.	페이지 아래쪽의 명령 모음에서 **연결**을 클릭합니다.
4.	관리 포털에서 .rdp 파일을 검색 중이라는 알림이 표시됩니다. **확인**을 클릭합니다.
...
8.	**사용자 이름**에 VM의 이름 및 VM과 함께 만든 로컬 관리자 계정(로컬 컴퓨터 계정)의 사용자 이름을 입력합니다. 이때 다음 형식을 사용합니다. 
...
5.	서버 및 디스크 선택에서 **디스크 2**를 클릭하고 **다음**을 클릭합니다. 메시지가 표시되면 확인을 클릭합니다.
...
2.	Windows PowerShell 명령 프롬프트에서 **ping** 명령을 사용하여 조직 네트워크의 리소스 이름과 IP 주소에 대해 ping을 실행합니다.
...
- sqlservice: SQL 인스턴스를 실행할 수 있는 사용자 계정입니다.
...
5.	대화 상자에서 **고급 기능 보기 및 클릭**을 클릭합니다. 이 옵션을 사용하면 AD 개체에 대해 속성 창에서 숨겨진 컨테이너와 탭을 모두 표시할 수 있습니다.
...
1.	Azure 관리 포털의 왼쪽 창에서 **네트워크**를 클릭하고 가상 네트워크의 이름(표 V - 항목 1 - 값 열)을 클릭합니다.
2.	**Configure**를 클릭합니다.
...
6.	Azure 관리 포털의 왼쪽 창에서 **가상 컴퓨터**를 클릭한 다음 첫 번째 도메인 컨트롤러 이름 옆에 있는 **상태** 열을 클릭합니다.
...
다음으로는 Azure 가상 네트워크의 서버가 로컬 도메인 컨트롤러를 사용하도록 AD 복제 사이트를 만듭니다. sp_install 계정을 사용하여 주 도메인 컨트롤러에 로그온한 다음 관리자 수준 Windows PowerShell 명령 프롬프트에서 다음 명령을 실행합니다.
