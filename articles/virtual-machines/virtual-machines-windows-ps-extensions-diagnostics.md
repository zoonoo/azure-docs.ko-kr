...
Azure 리소스 관리자 배포 모델을 통해 Windows VM을 만드는 동안 리소스 관리자 템플릿에 확장 구성을 추가하여 진단 확장을 사용하도록 설정할 수 있습니다. [Azure Resource Manager를 사용하여 Windows 가상 컴퓨터와 모니터링 및 진단 기능 만들기](virtual-machines-windows-extensions-diagnostics-template.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)를 참조하세요.
...
* *Metrics* 요소의 **resourceID** 특성을 VM의 리소스 ID로 업데이트해야 합니다.
  ...
  * 예를 들어 VM이 실행 중인 구독의 구독 ID가 **11111111-1111-1111-1111-111111111111**이고, 리소스 그룹의 리소스 그룹 이름이 **MyResourceGroup**이고, VM 이름이 **MyWindowsVM**일 경우 *resourceID* 값은 다음과 같습니다.
      ...
      ```
  * 성능 카운터와 메트릭 구성을 기반으로 메트릭을 생성하는 방법에 대한 자세한 내용은 [Azure Diagnostics metrics table in storage(저장소의 Azure 진단 메트릭 테이블)](virtual-machines-windows-extensions-diagnostics-template.md#wadmetrics-tables-in-storage)을 참조하세요.
