...
##Azure에서 Oracle 데이터베이스 가상 컴퓨터를 만드는 방법

1.	[Azure 포털](https://ms.portal.azure.com/)에 로그인합니다.

2.	**Marketplace**를 클릭하고 **계산**을 클릭한 다음 검색 상자에 **Oracle**을 입력합니다.

3.	사용할 수 있는 Oracle 데이터베이스 이미지- **버전 11g, 버전 12c, Standard Edition, Enterprise Edition 또는 가장 많이 사용하는 옵션이나 고급 옵션 번들** 중 하나를 선택합니다. 선택하는 이미지에 관한 정보(예: 최소 권장 크기)를 검토하고 **다음**을 클릭합니다.
...
12. **만들기**를 클릭하면 가상 컴퓨터를 만드는 과정이 시작됩니다. 가상 컴퓨터가 **실행** 상태가 되면 다음 단계를 진행합니다.


##Azure에서 Oracle 데이터베이스 가상 컴퓨터를 사용하여 데이터베이스를 만드는 방법
...
7.	**Windows 시작**을 클릭한 다음 **Database Configuration Assistant**를 입력합니다. **Database Configuration Assistant** 아이콘을 클릭합니다.

8.	**Database Configuration Assistant** 마법사에서 각 대화 상자 단계마다 필요한 값을 제공합니다.

	1.	**1단계:** **데이터베이스 만들기**를 클릭하고 **다음**을 클릭합니다.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image5.png)

	2. **2단계:** **글로벌 데이터베이스 이름**에 대한 값을 입력합니다. **관리 암호**에 대한 값을 입력 및 확인합니다. 이 암호는 Oracle 데이터베이스 **시스템** 사용자를 위한 것입니다. **컨테이너 데이터베이스로 만들기** 선택을 취소합니다. **다음**을 클릭합니다.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image6.png)

	3. **3단계:** 필수 구성 요소 확인은 **4단계**에 앞서 자동으로 진행됩니다.
	
	4. **4단계:** **데이터베이스 만들기 - 요약** 옵션을 검토하고 **마침**을 클릭합니다.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image7.png)
	5. **5단계:** **진행률 페이지**는 데이터베이스 만들기 상태를 보고합니다.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image8.png)
	6. 데이터베이스가 만들어지면 **암호 관리** 대화 상자를 사용하는 옵션이 생깁니다. 요구 사항에 필요한 경우 암호 설정을 수정한 다음 대화 상자를 닫아 **Database Configuration Assistant** 마법사를 종료합니다.

##설치된 데이터베이스를 확인하는 방법

1.	가상 컴퓨터에 로그인된 상태에서 **Windows 시작**을 클릭하고 **SQL Plus**를 입력하여 SQL Plus 명령 프롬프트를 시작합니다. **SQL Plus** 아이콘을 클릭합니다.

2.	프롬프트가 나타나면 **시스템**의 사용자 이름 및 Oracle 데이터베이스를 만들 때 지정한 암호를 사용하여 로그인합니다.
...
(`lsnrctl status`을(를) 실행하여 수신기의 상태를 확인할 수 있습니다. 수신기를 종료하려는 경우 `lsnrctl stop`을(를) 실행합니다.)

### 가상 컴퓨터의 방화벽에서 1521 포트 열기

1.	가상 컴퓨터에 로그인되어 있는 상태입니다. **Windows 시작**을 클릭하고 **고급 보안이 포함된 Windows 방화벽**를 입력한 다음 **고급 보안이 포함된 Windows 방화벽** 아이콘을 클릭합니다. 이렇게 하면 **고급 보안이 포함된 Windows 방화벽** 관리 콘솔이 열립니다.
...
4.	**프로토콜 및 포트**는 **TCP**를 선택하고 **특정 로컬 포트**를 선택하여 포트에 대해 **1521**을 입력하고 다음을 클릭합니다.
...
8.	끝점의 이름을 지정합니다.
	...
	2. 공용 포트에 **1521**을 사용합니다.
	
	3. 개인 포트에 **1521**을 사용합니다.
...
**Oracle Database with Popular Options bundle**은 Oracle Database Enterprise Edition의 라이선스가 포함된 인스턴스 및 [Partitioning](http://www.oracle.com/us/products/database/options/partitioning/overview/index.html), [Active Data Guard](http://www.oracle.com/us/products/database/options/active-data-guard/overview/index.html), [Oracle Tuning Pack for Database](http://docs.oracle.com/html/A86647_01/tun_ovw.htm), [Oracle Diagnostics Pack for Database](http://docs.oracle.com/cd/B28359_01/license.111/b28287/options.htm#CIHIHDDJ) 및 [Oracle Lifecycle Management Pack for Database](http://www.oracle.com/technetwork/oem/lifecycle-mgmt-495331.html)를 포함합니다.

**Oracle Database with Advanced Options bundle**은 가장 많이 사용하는 옵션 번들에 있는 모든 옵션의 라이선스가 포함된 인스턴스를 비롯하여 [Advanced Compression](http://www.oracle.com/us/products/database/options/advanced-compression/overview/index.html), [Advanced Security](http://www.oracle.com/us/products/database/options/advanced-security/overview/index.html), [Label Security](http://www.oracle.com/us/products/database/options/label-security/overview/index.html), [Database Vault](http://www.oracle.com/us/products/database/options/database-vault/overview/index.html), [Advanced Analytics](http://www.oracle.com/us/products/database/options/advanced-analytics/overview/index.html), [OLAP](http://docs.oracle.com/cd/E11882_01/license.112/e47877/options.htm#CIHGDEEF), [Spatial and Graph](http://docs.oracle.com/cd/E11882_01/license.112/e47877/options.htm#CIHGDEEF), [In-Memory Database Cache](http://www.oracle.com/technetwork/products/timesten/overview/timesten-imdb-cache-101293.html), [Data Masking Pack](http://docs.oracle.com/cd/E11882_01/license.112/e47877/options.htm#CHDGEEBB), 및 Oracle Test Data Management Pack(Data Masking Pack의 일부)을 포함합니다.
