...
Oracle은 Oracle Linux 기반 가상 컴퓨터 이미지의 Azure에서 Oracle DB 12.1 Standard 및 Enterprise Edition을 실행하도록 지원합니다.  Azure에서 Oracle DB의 프로덕션 워크로드에 대한 최상의 성능을 위해 VM 이미지 크기를 적절히 조정하고 Premium Storage에서 지원되는 관리 디스크를 사용해야 합니다. Oracle에서 게시한 VM 이미지를 사용하여 Azure에서 Oracle DB를 신속하게 실행하는 방법에 대한 지침은 [Oracle DB 빠른 시작 연습](oracle-database-quick-create.md)을 참조하세요.
...
Oracle RAC는 온-프레미스 다중 노드 클러스터 구성에서 단일 노드의 오류를 완화하도록 고안되었습니다.  이 기능은 하이퍼스케일(hyper-scale) 공용 클라우드 환경에 네이티브가 아닌 두 가지 온-프레미스 기술인 네트워크 멀티캐스트 및 공유 디스크에 의존합니다. Azure에서 Oracle RAC를 배포해야 하는 경우 이러한 기술을 에뮬레이트하는 [FlashGrid 같은](https://www.flashgrid.io/oracle-rac-in-azure/) 회사에서 만든 타사 솔루션이 있습니다. 
...
Azure에 HA 및 DR 솔루션이 설계되어 있음에도 불구하고 백업 전략을 사용하여 데이터베이스를 복원할 수 있습니다.  일관된 백업 설정에 대한 기본 절차를 안내하는 [Oracle 데이터베이스 백업 및 복구](oracle-backup-recovery.md) 자습서가 있습니다.
