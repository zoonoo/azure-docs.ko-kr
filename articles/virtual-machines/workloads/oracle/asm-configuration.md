...
# <a name="set-up-oracle-asm-on-an-azue-linux-virtual-machine"></a>Azure Linux 가상 컴퓨터에 Oracle ASM 설정 
...
10. 루트, oracle 및 눈금 사용자에 대한 암호를 변경합니다. (암호는 설치하는 동안 나중에 사용합니다.)
...
7.  사용 가능한 스왑 공간을 확인합니다. Oracle Grid Infrastructure를 설치하려면 적어도 6GB의 스왑 공간이 필요합니다.

    ```bash
    # swapon -s
    ```

    스왑 공간이 6GB 미만인 경우 다음 명령을 실행하여 스왑 공간을 추가할 수 있습니다.
...
X11을 실행하는 클라이언트 및 VM을 준비하려면 다음 단계를 완료합니다. 

1.  루트로 로그인하고 /etc/ssh/ssh_config 파일을 편집합니다. Forwardx11에 대한 설정을 `yes`로 변경합니다.
...
2.  Windows 컴퓨터에 PuTTY 및 Xming을 다운로드합니다.

  * [PuTTY 다운로드](http://www.putty.org/)
  * [Xming 다운로드](https://xming.en.softonic.com/)

3.  PuTTY를 PuTTY 폴더(예: C:\Program Files\PuTTY)에 설치한 후 puttygen.exe(PuTTY 키 생성기)를 실행합니다.

4.  PuTTY 키 생성기에서,

  1.  키를 생성하려면 **생성** 단추를 선택합니다. 
  2.  키의 콘텐츠를 복사(Ctrl + C)합니다.
  3.  **개인 키 저장** 단추를 선택합니다. 
  4.  표시되는 경고를 무시하고 **확인**을 선택합니다.

  ![PuTTY 키 생성기 페이지의 스크린샷](./media/asm-configuration/puttykeygen.png)
...
6.  authorized_keys라는 파일을 만듭니다. 키의 콘텐츠를 이 파일에 붙여넣은 다음 파일을 저장합니다.

  > [!NOTE]
  > 키에는 문자열 `ssh-rsa`가 포함되어야 합니다. 또한 키의 콘텐츠는 한 줄 텍스트여야 합니다.
  >  

7.  PuTTY를 시작합니다. **카테고리** 창에서 **연결** > **SSH** > **인증**으로 이동합니다. **인증에 대한 개인 키 파일** 상자에서 이전에 생성한 키를 찾아봅니다.

  ![개인 키 설정 페이지의 스크린샷](./media/asm-configuration/setprivatekey.png)

9.  **카테고리** 창에서 **연결** > **SSH** > **X11**로 이동합니다. **X11 전달을 사용하도록 설정** 상자를 선택합니다.

  ![X11 사용 페이지의 스크린샷](./media/asm-configuration/enablex11.png)

10. **카테고리** 창에서 **세션**으로 이동합니다. 호스트 정보를 입력한 다음 **열기**를 선택합니다.

  ![세션 페이지의 스크린샷](./media/asm-configuration/puttysession.png)
...
2. **설치 옵션 선택** 페이지에서 **독립 실행형 서버에 대한 Oracle Grid Infrastructure 설치 및 구성**을 선택합니다.
  ...
  ![설치 관리자 설치 옵션 선택 페이지의 스크린샷](./media/asm-configuration/install01.png)

3. **제품 언어 선택** 페이지에서 **영어** 또는 원하는 언어를 선택합니다.

  ![설치 관리자 제품 언어 선택 페이지의 스크린샷](./media/asm-configuration/install02.png)

4. **ASM 디스크 그룹 만들기** 페이지에서,
  1.  디스크 그룹에 사용할 이름을 입력합니다.
  2.  **중복**에서 **외부**를 선택합니다.
  3.  **할당 단위 크기**에서 **4**를 선택합니다.
  4.  **디스크 추가**에서 **ORCLASMSP**를 선택합니다.

  ![설치 관리자 ASM 디스크 그룹 만들기 페이지의 스크린샷](./media/asm-configuration/install03.png)

5. **ASM 암호 지정** 페이지에서 **이러한 계정에 대해 동일한 암호 사용** 옵션을 선택하고 암호를 입력합니다.

  ![설치 관리자 ASM 암호 지정 페이지의 스크린샷](./media/asm-configuration/install04.png)

6. (선택 사항) **관리 옵션 지정** 페이지에서 **EM(Enterprise Manager) 클라우드 컨트롤에 등록** 상자를 선택합니다.

  ![설치 관리자 관리 옵션 지정 페이지의 스크린샷](./media/asm-configuration/install05.png)

7. **권한있는 운영 체제 그룹** 페이지에서 기본 설정을 사용합니다.

  ![설치 관리자 권한있는 운영 체제 그룹 페이지의 스크린샷](./media/asm-configuration/install06.png)

8. **설치 위치 지정** 페이지에서 기본 설정을 사용합니다.

  ![설치 관리자 설치 위치 지정 페이지의 스크린샷](./media/asm-configuration/install07.png)

9. **인벤토리 만들기** 페이지에서 폴더 위치를 입력하거나 찾아봅니다.

  ![설치 관리자 인벤토리 만들기 페이지의 스크린샷](./media/asm-configuration/install08.png)

10. **루트 스크립트 실행 구성** 페이지에서 **구성 스크립트 자동으로 실행** 상자를 선택합니다. 그런 다음 **"root" 사용자 자격 증명 사용** 옵션을 선택하고 루트 사용자 암호를 입력합니다.

  ![설치 관리자 루트 스크립트 실행 구성 페이지의 스크린샷](./media/asm-configuration/install09.png)

11. **필수 구성 요소 검사 수행** 페이지에서 **수정 및 다시 확인**을 선택합니다.

  ![설치 관리자 필수 구성 요소 검사 수행 페이지의 스크린샷](./media/asm-configuration/install10.png)

12. **스크립트 수정** 페이지에서 **확인**을 선택합니다.

  ![설치 관리자 스크립트 수정 페이지의 스크린샷](./media/asm-configuration/install11.png)

13. **요약** 페이지에서 설정 선택을 검토한 다음 **설치**를 선택합니다.

  ![설치 관리자 요약 페이지의 스크린샷](./media/asm-configuration/install12.png)

14. 경고 대화 상자가 나타납니다. **예**를 선택하여 계속합니다.

  ![경고 대화 상자의 스크린샷](./media/asm-configuration/install14.png)

15. **마침** 페이지에서 **닫기**를 선택하여 설치를 완료합니다.

  ![설치 관리자 마침 페이지의 스크린샷](./media/asm-configuration/install16.png)
...
2.  **ASM 구성: 디스크 그룹** 페이지에서 **만들기** 단추를 선택한 다음 **고급 옵션 표시**를 선택합니다.

  ![ASM 구성: 디스크 그룹 페이지의 스크린샷](./media/asm-configuration/asm01.png)

3.  **디스크 그룹 만들기** 페이지에서,

  1.  디스크 그룹 이름을 입력합니다.
  2.  **멤버 디스크 선택**에서 **ORCL_DATA** 및 **ORCL_DATA1**을 선택합니다.
  3.  **할당 단위 크기**에서 **4**를 선택합니다. 

  ![디스크 그룹 만들기 페이지의 스크린샷](./media/asm-configuration/asm02.png)

4.  **ASM 구성: 디스크 그룹** 페이지에서 **만들기** 단추를 선택한 다음 **고급 옵션 표시**를 선택합니다.

  ![ASM 구성: 디스크 그룹 페이지의 스크린샷](./media/asm-configuration/asm03.png)

5.  **디스크 그룹 만들기** 페이지에서,

  1.  디스크 그룹 이름을 입력합니다.
  2.  **중복**에서 **외부**를 선택합니다.
  3.  **멤버 디스크 선택**에서 **ORCL_FRA**를 선택합니다.
  4.  **할당 단위 크기**에서 **4**를 선택합니다.

  ![디스크 그룹 만들기 페이지의 스크린샷](./media/asm-configuration/asm04.png)

6.  **끝내기**를 선택하여 ASM 구성 도우미를 닫습니다.

  ![끝내기 단추가 있는 ASM 구성: 디스크 그룹 페이지의 스크린샷](./media/asm-configuration/asm05.png)
...
2.  **데이터베이스 작업** 페이지에서 **데이터베이스 만들기**를 선택합니다.
  ...
  ![데이터베이스 작업 페이지의 스크린샷](./media/asm-configuration/createdb01.png)
  
3. **생성 모드** 페이지에서,

    1.  데이터베이스에 사용할 이름을 입력합니다. 
    2.  **저장소 형식**의 경우 **ASM(자동 저장소 관리)**을 선택합니다.
    3.  **데이터베이스 파일 위치**의 경우 사용할 폴더를 찾아봅니다.
    4.  **빠른 복구 영역**의 경우 사용할 폴더를 찾아봅니다.

  ![생성 모드 페이지의 스크린샷](./media/asm-configuration/createdb02.png)

4.  **요약** 페이지에서 설정 선택을 검토한 다음 **마침**을 선택하여 데이터베이스를 만듭니다.

  ![요약 페이지의 스크린샷](./media/asm-configuration/createdb03.png)

5.   (선택 사항) **마침** 페이지에서 암호를 변경하려면 **암호 관리**를 선택합니다.

  ![마침 페이지의 스크린샷](./media/asm-configuration/createdb04.png)
