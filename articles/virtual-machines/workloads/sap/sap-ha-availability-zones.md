...
ms.locfileid: "55746220"
---
# <a name="sap-workload-configurations-with-azure-availability-zones"></a>Azure 가용성 영역을 사용하는 SAP 워크로드 구성

Azure가 SAP 워크플로의 전반적 가용성을 향상시키기 위해 제공하는 고가용성 기능 중 하나는 [Azure 가용성 영역](https://docs.microsoft.com/azure/availability-zones/az-overview)입니다. 가용성 영역을 다른 [Azure 지역](https://azure.microsoft.com/global-infrastructure/regions/)에서 이미 사용할 수 있습니다. 앞으로 더 많은 지역이 이러한 기능을 지원할 예정입니다. 

이 그림에 표시된 대로 SAP 고가용성의 기본 아키텍처를 설명할 수 있습니다.

![표준 HA 구성](./media/sap-ha-availability-zones/standard-ha-config.png)

SAP 애플리케이션 계층은 하나의 Azure [가용성 집합](https://docs.microsoft.com/azure/virtual-machines/windows/manage-availability)에서 배포됩니다. SAP Central Services의 고가용성을 위해, 별도 가용성 집합에 두 개의 VM을 배포하고 Windows 장애 조치(failover) 클러스터 서비스 또는 Pacemaker(Linux)를 사용하여 인프라 또는 소프트웨어 문제가 발생할 경우 자동 장애 조치(failover)를 허용하는 고가용성 프레임워크를 배포합니다. 이러한 배포를 설명하는 설명서는 다음과 같습니다.
...
비슷한 아키텍처가 SAP NetWeaver, S/4HANA 또는 Hybris 시스템의 DBMS 계층에 적용됩니다. 장애 조치(failover) 클러스터 솔루션을 사용하여 액티브/패시브 모드에서 DBMS 계층을 배포합니다. 이 솔루션은 DBMS 특정 장애 조치(failover) 프레임워크, Windows 장애 조치(failover) 클러스터 서비스 또는 Pacemaker를 사용하여 인프라 또는 소프트웨어 장애가 발생할 때 장애 조치(failover) 활동을 시작합니다. 

Azure 가용성 영역을 사용하여 동일한 아키텍처를 배포하려면 설명된 아키텍처를 변경해야 합니다. 이러한 변경 내용은 이 문서의 다른 부분에서 설명합니다.
...
- Azure 가용성 영역은 한 Azure 지역 내의 다른 영역 간 특정 거리의 어떤 보증도 제공하지 않습니다.
- Azure 가용성 영역은 이상적인 DR 솔루션이 아닙니다. 엄청난 자연 재해가 일부 지역을 광범위하게 손상시키는 경우(전력 인프라의 심각한 손상 포함) 다른 지역 간 거리가 충분히 크지 않아서 적절한 DR 솔루션으로 인정받지 못할 수 있습니다.
- 서로 다른 Azure 지역 내에서 다양한 Azure 가용성 영역 간의 네트워크 대기 시간은 Azure 지역마다 달라질 수 있습니다. 하나의 영역에서 활성 DBMS VM으로의 네트워크 대기 시간은 비즈니스 프로세스 영향에서 여전히 허용 가능하므로 다른 영역에서 배포된 SAP 애플리케이션 계층을 실행할 수 있는 경우가 있습니다. 그렇지만, 일부 Azure 지역에서 하나의 영역에 있는 활성 DBMS VM과 다른 영역에 있는 SAP 애플리케이션 인스턴스 간의 대기 시간이 너무 방해가 되며 SAP 비즈니스 프로세스에 대해 허용되지 않을 수 있는 시나리오가 있습니다. 결과적으로 배포 아키텍처는 영역 간 대기 시간이 너무 높은 경우 애플리케이션에 대한 활성/활성 아키텍처 또는 능동/수동 아키텍처와 달라야 합니다.
- Azure 가용성 영역을 사용할 수 있는 구성을 결정합니다. 네트워크 대기 시간을 기준으로, 여러 영역 간에 측정을 수행합니다. 네트워크 대기 시간은 다음과 같이 두 개의 다른 영역에서 중요한 역할을 합니다.
    - 동기 복제를 설정해야 하는 두 DBMS 인스턴스 간의 대기 시간. 네트워크 대기 시간이 길수록 워크로드의 확장성에 영향을 미칠 가능성이 높아집니다.
    - 활성 DBMS 인스턴스와 같은 영역에서 SAP 대화 상자 인스턴스를 실행하는 VM과 다른 영역의 유사한 VM 간의 네트워크 대기 시간 차이. 이 차이가 높을수록, DBMS가 있는 동일한 영역에서 실행되는지 또는 다른 영역에서 실행되는지에 따라 비즈니스 프로세스 및 일괄 처리 작업의 런타임에 미치는 영향이 높아집니다.


Azure 기능 사용의 경우, 영역 간에 Azure VM을 배포하고 동일한 Azure 지역 내의 여러 다른 가용성 영역에서 장애 조치(failover) 솔루션을 설정할 때 사용할 수 있는 기능이 약간 제한됩니다. 이러한 제한 사항은 다음과 같습니다.

- [Azure Managed Disks](https://azure.microsoft.com/services/managed-disks/) 사용은 Azure 가용성 영역으로 배포할 때 반드시 필요합니다. 
- 물리적 영역에 대한 영역 열거형 매핑은 Azure 구독을 기준으로 고정됩니다. 다른 구독을 사용하여 SAP 시스템을 배포하는 경우 각 구독에 대해 적절한 영역을 별도로 정의해야 합니다.
...
## <a name="ideal-zone-combination"></a>이상적인 영역 조합
가용성 영역을 활용하는 방법을 결정하기 위해서는 다음과 같은 정보를 제공하는 조사를 수행해야 합니다.
...
- 선택한 가용성 영역에서 배포해야 하는 VM 유형을 선택한 두 영역에서 사용할 수 있는지 여부 특히 M 시리즈 가상 머신의 경우, 세 영역 중 두 곳에서만 다른 M 시리즈 VM SKU를 사용할 수 있는 상황이 발생할 수 있습니다.
...
여러 다른 영역의 VM SKU 측정값 및 가용성에 따라, 다음과 같은 사항을 결정해야 합니다.

- DBMS 계층의 이상적인 영역을 정의합니다.
- 한 영역 내부 또는 여러 영역 간의 네트워크 대기 시간 차이에 따라, 1개, 2개 또는 3개의 모든 영역에서 활성 SAP 애플리케이션 계층을 배포할 수 있는지 여부를 물어봅니다.
- (뒷부분 참조)는 애플리케이션 관점에서 액티브/패시브 구성을 배포할지 또는 액티브/액티브 구성을 배포할지를 물어봅니다(뒷부분 참조).
...
> [!IMPORTANT]
> 수행하는 측정 및 의사 결정은 이러한 측정을 수행할 때 사용한 Azure 구독에 대해 유효합니다. 영역의 구독 종속 열거형 매핑은 구독마다 달라질 수 있으므로 다른 구독을 사용할 때는 측정을 반복해서 수행해야 합니다.


> [!IMPORTANT]
> 위에 따라 수행한 측정이 [가용성 영역](https://docs.microsoft.com/azure/availability-zones/az-overview)을 지원하는 모든 Azure 지역에서 다른 결과를 나타낼 수 있습니다. 네트워크 대기 시간에 대한 요구 사항은 달라지지 않더라도, 영역 간 네트워크 대기 시간이 다를 수 있으므로 Azure 지역마다 다른 배포 전략을 적용해야 할 수 있습니다. 일부 Azure 지역에서는 3개의 다른 영역 간 네트워크 대기 시간이 크게 다를 수 있습니다. 반면에 다른 지역에서는 3개의 다른 영역 간 네트워크 대기 시간이 좀 더 균일할 수 있습니다. 1밀리초~2밀리초의 네트워크 대기 시간이 **항상** 존재한다는 주장은 **잘못된** 것입니다. Azure 지역의 가용성 영역 간 네트워크 대기 시간은 일반화할 수 없습니다.
...
이러한 구성을 고려하려면 지역에서 사용자의 워크로드와 동기 DBMS 복제에 적합한 영역 간 네트워크 대기 시간을 제공하는 2개의 가용성 영역을 찾아야 합니다. 또한 선택한 영역 내의 네트워크 대기 시간과 영역 간 네트워크 대기 시간 간의 델타를 너무 크게 유지하지 않으려고 합니다. 후자의 이유라면, 작업이 DBMS 서버가 있는 영역 내에서 실행되는지 또는 영역에 걸쳐 수행되는지에 따라, 비즈니스 프로세스 또는 일괄 처리 프로세스의 런타임 변화가 크지 않은 것을 원할 것입니다. 약간의 차이 정도는 허용될 수 있습니다.
...
- 몇 가지 중요한 비즈니스 트랜잭션 및/또는 작업에 대한 런타임 일관성 유지 특정 일괄 처리 작업 및 사용자를 SAP 일괄 처리 서버 그룹, 로그온 그룹 또는 RFC 그룹을 사용하여 활성 DBMS 인스턴스가 있는 영역의 특정 애플리케이션 인스턴스에 직접 보내려고 할 수 있습니다. 그러나 영역 장애 조치(failover)의 경우 이러한 그룹을 나머지 영역에 있는 대화 상자 인스턴스로 수동으로 이동해야 합니다. 
- 애플리케이션 인스턴스의 부분을 배포한 영역의 서비스가 중단될 경우, 즉시 이전 리소스 용량으로 회복될 수 있도록 각 서비스 영역에 일부 유휴 대화 상자 인스턴스를 배포할지 여부를 결정합니다.


## <a name="activepassive-deployment"></a>액티브/패시브 배포
한 영역 내의 네트워크 대기 시간과 영역 간 네트워크 트래픽 간에 적합한 델타를 찾을 수 없는 경우 배포할 수 있는 아키텍처는 SAP 애플리케이션 계층의 관점에서 액티브/패시브 특성을 갖습니다. 완전한 애플리케이션 계층을 배포하고, 활성 DBMS 인스턴스 및 활성 SAP Central Services 인스턴스를 실행하려고 하는 ‘활성’ 영역을 정의합니다. 이러한 구성을 사용할 경우 작업이 활성 DBMS 인스턴스와 동일한 영역에서 실행되는지 여부에 따라 좌우되는 비즈니스 트랜잭션 및 일괄 처리 작업에서 런타임 차이가 크지 않은지 확인해야 합니다.
...
- Azure 가용성 영역에서 가용성 집합을 배포할 수 없습니다. 따라서 이러한 경우에는 애플리케이션 계층에 대해 하나의 업데이트 및 장애 도메인을 구축합니다. 이러한 도메인은 하나의 영역에만 배포되기 때문입니다. 이 구성은 Azure 가용성 집합에 애플리케이션 계층을 배포할 것으로 예상되는 참조 아키텍처에 비해 약간 단점이 있습니다.
...
Microsoft는 특정 Azure 지역에서 여러 Azure 가용성 영역을 호스트하는 시설 간의 지리적 거리에 대한 정보를 제공하고 있지 않지만, 일부 고객은 **R**(복구) **P**(지점)**O**(목표)를 제공할 수 있는 조합된 HA 및 DR 구성을 위해 영역을 이용하고 있습니다. 즉, 재해 복구 경우에도 커밋된 데이터베이스 트랜잭션을 손실하지 않게 됩니다. 
