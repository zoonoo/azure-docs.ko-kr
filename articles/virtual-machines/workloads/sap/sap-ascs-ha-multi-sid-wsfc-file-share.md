---
title: "Azure에서 Windows Server 장애 조치(failover) 클러스터링 및 파일 공유를 사용하는 SAP (A)SCS 인스턴스 다중 SID 고가용성 | Microsoft Docs"
...
# <a name="sap-ascs-instance-multi-sid-high-availability-with-windows-server-failover-clustering-and-file-share-on-azure"></a>Azure에서 Windows Server 장애 조치(failover) 클러스터링 및 파일 공유를 사용하는 SAP (A)SCS 인스턴스 다중 SID 고가용성
...
SAP를 배포한 경우 내부 부하 분산 장치를 사용하여 SAP ASCS/SCS에 대한 Windows 클러스터 구성을 만들 수 있습니다.
...
![그림 1: 두 클러스터에 배포된 SAP (A)SCS 인스턴스 및 SOFS][sap-ha-guide-figure-8007]

_**그림 1:** 두 클러스터에 배포된 SAP (A)SCS 인스턴스 및 SOFS_
...
## <a name="sap-ascs-multi-sid-architecture-with-file-share"></a>파일 공유를 사용하는 SAP (A)SCS 다중 SID HA 아키텍처
...
추가 **&lt;SID2&gt;** SAP 시스템을 설치하는 방법은 한 <SID> 시스템을 설치하는 방법과 동일합니다. (A)SCS 클러스터 및 파일 공유 SOFS 클러스터에서 두 개의 추가 준비 단계가 필요합니다.

## <a name="infrastructure-preparation-for-sap-multi-sid-scenario"></a>SAP 다중 SID 시나리오를 위한 인프라 준비

### <a name="infrastructure-preparation-on-domain-controller"></a>도메인 컨트롤러에서 인프라 준비

예를 들어 &lt;SID2&gt; = PR2를 사용하여 **&lt;Domain&gt;\SAP_&lt;SID2&gt;_GlobalAdmin** 도메인 그룹을 만듭니다. 도메인 그룹 이름은 <Domain>\SAP_PR2_GlobalAdmin입니다.

### <a name="infrastructure-preparation-on-ascs-cluster"></a>(A)SCS 클러스터에서 인프라 준비

기존 (A)SCS 클러스터에서 두 번째 SAP &lt;SID&gt;에 대한 인프라를 준비해야 합니다.
...
### <a name="infrastructure-preparation-on-sofs-cluster-using-existing-sap-global-host"></a>기존 SAP 전역 호스트를 사용하여 SOFS 클러스터에서 인프라 준비

첫 번째 SAP <SID1> 시스템의 기존 **&lt;SAPGLOBALHost&gt;** 및 **Volume1**을 다시 사용할 수 있습니다.

![그림 3: 다중 SID SOFS 동일한 SAP 전역 호스트 이름][sap-ha-guide-figure-8014]

_**그림 3:** 다중 SID SOFS 동일한 SAP 전역 호스트 이름_

> [!IMPORTANT]
>두 번째 **SAP &lt;SID2&gt;** 시스템의 경우 동일한 Volume1 및 동일한 **&lt;SAPGlobalHost&gt;** 네트워크 이름이 사용됩니다.
>동일한 **&lt;SAPGlobalHost&gt;** 네트워크 이름을 다시 사용하고 싶고 서로 다른 SAP 시스템에 대해 **SAPMNT** 공유 이름을 고정한 경우 강제로 동일한 **Volume1**이 사용됩니다.
>
><SID2> 전역 호스트의 파일 경로는 C:\ClusterStorage\\**Volume1**\usr\sap\<SID2>\SYS\입니다.
>

SAP 전역 호스트 **..\SYS\..** 폴더를 &lt;SID2&gt; 시스템에 대한 SOFS 클러스터에 준비해야 합니다.

다음 PowerShell 스크립트를 실행하여 &lt;SID2&gt; 인스턴스에 대한 SAP 전역 호스트를 준비합니다.
...
### <a name="infrastructure-preparation-on-sofs-cluster-using-different-sap-global-host"></a>다른 SAP 전역 호스트를 사용하여 SOFS 클러스터에서 인프라 준비

두 번째 SOFS를 구성할 수 있습니다. 예를 들어 두 번째 **&lt;SID2&gt;**에 대해 **&lt;SAPGlobalHost2&gt;** 및 다른 **Voulme2**를 사용하여 두 번째 SOFS 클러스터 역할을 구성할 수 있습니다.

![그림 4: 다중 SID SOFS 동일한 SAP 전역 호스트 이름 2][sap-ha-guide-figure-8015]

_**그림 4:** 다중 SID SOFS 동일한 SAP 전역 호스트 이름 2_

다음 PowerShell 스크립트 실행하여 &lt;SAPGlobalHost2&gt;로 두 번째 SOFS 역할을 만듭니다.
...
![그림 5: 다중 SID SOFS 동일한 SAP 전역 호스트 이름 2][sap-ha-guide-figure-8016]

_**그림 5:** 장애 조치(Failover) 클러스터 관리자의 두 번째 **Volume2**_

두 번째 **&lt;SID2&gt;**에 대한 SAP 전역 폴더를 만들고 파일 보안을 설정합니다.
...
**&lt;SAPGlobalHost2&gt;** 호스트 이름을 사용하는 **Volume2**에 SAPMNT 파일 공유를 만들려면 두 번째 SAP **&lt;SID2&gt;**에 대해 장애 조치(Failover) 클러스터 관리자에서 다음 마법사를 시작합니다.
...
![그림 8: sapglobalhost2를 선택하고 Volume2에 경로 지정][sap-ha-guide-figure-8019]

_**그림 8:** **sapglobalhost2**를 선택하고 Volume2에 경로 지정_


![그림 9: 파일 공유 이름을 sapmnt로 설정][sap-ha-guide-figure-8020]
...
다음에 대해 "모든 권한"을 파일 및 sapmnt 공유로 설정해야 합니다.
* **SAP_&lt;SID&gt;_GlobalAdmin** 도메인 사용자 그룹
* (A)SCS 클러스터 노드 **ascs-1$** 및 **ascs-2$**의 컴퓨터 개체

![그림 11: 사용자 그룹 및 컴퓨터 계정에 대한 모든 권한 설정][sap-ha-guide-figure-8022]

_**그림 11:** 사용자 그룹 및 컴퓨터 계정에 대한 모든 권한 설정_

![그림 12: 만들기 클릭][sap-ha-guide-figure-8023]

_**그림 12:** 만들기 클릭_

![그림 13: 두 번째 sapmnt가 sapglobal2 호스트에 바인딩되고 Volume2 생성][sap-ha-guide-figure-8024]

_**그림 13:** **두 번째 sapmnt**가 **sapglobal2** 호스트에 바인딩되고 **Volume2** 생성_

## <a name="sap-netweaver-multi-sid-installation"></a>SAP NetWeaver 다중 SID 설치

### <a name="sap-ltsid2gt-ascs-and-ers-instances-installation"></a>SAP &lt;SID2&gt; (A)SCS 및 ERS 인스턴스 설치
