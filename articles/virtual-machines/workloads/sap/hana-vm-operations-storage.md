...
Linux에는 몇 가지 다른 I/O 일정 예약 모드가 있습니다. Linux 공급 업체 및 SAP를 통한 일반적인 권장 사항은 디스크 볼륨에 대 한 i/o scheduler 모드를 **mq-마감일이** 나 **kyber** 모드에서 **noop** (비 multiqueue)로 다시 구성 하거나 (multiqueue) 모드에 대해 **없음** 을 다시 구성 하는 것입니다. 자세한 내용은 [SAP Note #1984787](https://launchpad.support.sap.com/#/notes/1984787)에서 참조 하세요. 
...
- 가상 컴퓨터의 <b>sid</b>Adm 및 그룹 id `sapsys` 에 대 한 사용자 ID는 Azure NetApp Files의 구성과 일치 해야 합니다. 

> [!IMPORTANT]
> SAP HANA 워크 로드의 경우 짧은 대기 시간이 중요 합니다. Microsoft 담당자와 협력 하 여 가상 머신과 Azure NetApp Files 볼륨이 근접 하 게 배포 되었는지 확인 합니다.  

> [!IMPORTANT]
> <b>Sid</b>Adm의 사용자 ID와 가상 컴퓨터와 Azure netapp 구성 사이에 `sapsys` 그룹 id가 일치 하지 않는 경우 가상 컴퓨터에 탑재 된 Azure netapp 볼륨의 파일에 대 한 사용 권한이로 `nobody`표시 됩니다. Azure NetApp Files에 대 한 [새 시스템을 온 보 딩](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbRxjSlHBUxkJBjmARn57skvdUQlJaV0ZBOE1PUkhOVk40WjZZQVJXRzI2RC4u) 할 때 <b>sid</b>ADM에 올바른 `sapsys`사용자 ID와에 대 한 그룹 id를 지정 해야 합니다.
...
| 볼륨 | Size<br /> Premium Storage 계층 | Size<br /> Ultra Storage 계층 | 지원 되는 NFS 프로토콜 |
| --- | --- | --- |
...
> [!TIP]
> 볼륨을 요구 `unmount` 하지 않고 Azure NetApp Files 볼륨을 동적으로 크기를 조정 하 여 가상 컴퓨터를 중지 하거나 SAP HANA을 중지할 수 있습니다. 이를 통해 응용 프로그램에 예상 된 처리량과 예측할 수 없는 처리량 요구를 모두 충족할 수 있습니다.
