...
위의 명령은 Windows 기반 VM을 배포 합니다. 이 VM이 성공적으로 배포 되 면 근접 배치 그룹의 데이터 센터 범위가 Azure 지역 내에서 정의 됩니다. 이전 명령에 표시 된 것과 같이 근접 배치 그룹을 참조 하는 모든 후속 VM 배포는 동일한 Azure 데이터 센터에 배포 됩니다. 해당 데이터 센터에 배치 된 하드웨어에서 VM 유형을 호스트 하 고 해당 VM 유형에 대 한 용량을 사용할 수 있는 경우에 해당 합니다.
