...
* 관리되는 호스트에 로그인하는 경우 파일 시스템이 마운트 해제되지 않도록 차단해야 합니다.  
  Linux 가상 컴퓨터에 로그인하여 작업 디렉토리를 마운트 지점의 디렉토리(예: /usr/sap/AH1/ASCS00/exe)로 변경하는 경우 볼륨을 해제할 수 없으며 재배치 또는 준비 해제가 실패합니다.

* SUSE SLES 리눅스 가상 머신에서 CLOUD_NETCONFIG_MANAGE 않도록 설정하십시오. 자세한 내용은 [SUSE KB 7023633을](https://www.suse.com/support/kb/doc/?id=7023633)참조하십시오.
...
Azure 커넥터는 Azure 리소스 관리자 API를 사용하여 Azure 리소스를 관리합니다. SAP LaMa는 서비스 주체 또는 관리되는 ID를 사용하여 이 API에 대해 인증할 수 있습니다. SAP LaMa가 Azure VM에서 실행 중인 경우 장에서 설명한 대로 관리되는 [ID를 사용하여 관리되는 ID를 사용하여 Azure API에 대한 액세스 권한을 얻는](lama-installation.md#af65832e-6469-4d69-9db5-0ed09eac126d)것이 좋습니다. 서비스 주체를 사용하려면 서비스 보안 주체 사용 의 단계를 [수행하여 Azure API에 대한 액세스 권한을 얻습니다.](lama-installation.md#913c222a-3754-487f-9c89-983c82da641e)

### <a name="use-a-service-principal-to-get-access-to-the-azure-api"></a><a name="913c222a-3754-487f-9c89-983c82da641e"></a>서비스 보안 주체를 사용하여 Azure API에 대한 액세스 권한 얻기

Azure 커넥터는 서비스 보안 주체를 사용하여 Microsoft Azure에 대해 권한을 부여할 수 있습니다. 다음 단계에 따라 SAP LaMa(Landscape Management)에 대한 서비스 주체를 생성합니다.
...
1. 새 앱을 선택하고 설정 탭에서 인증서 & 비밀을 클릭합니다.
1. 새 클라이언트 비밀 만들기, 새 키에 대한 설명을 입력하고, 비밀이 만료될 시기를 선택하고 저장을 클릭합니다.
...
### <a name="use-a-managed-identity-to-get-access-to-the-azure-api"></a><a name="af65832e-6469-4d69-9db5-0ed09eac126d"></a>관리되는 ID를 사용하여 Azure API에 대한 액세스 권한 얻기

관리되는 ID를 사용하려면 SAP LaMa 인스턴스가 시스템 또는 사용자에게 할당된 ID가 있는 Azure VM에서 실행되어야 합니다. 관리되는 ID에 대한 자세한 내용은 [Azure 리소스에 대한 관리 ID란 무엇입니까?](../../../active-directory/managed-identities-azure-resources/overview.md) 및 [Azure 포털을 사용하여 VM에서 Azure 리소스에 대한 관리되는 ID 구성을](../../../active-directory/managed-identities-azure-resources/qs-configure-portal-windows-vm.md)참조하십시오.

관리되는 ID에는 기본적으로 Azure 리소스에 액세스할 수 있는 권한이 없습니다. 액세스 권한을 부여해야 합니다.

1. [https://editor.swagger.io](https://portal.azure.com) 으로 이동합니다.
1. 리소스 그룹 블레이드를 엽니다.
1. 사용하려는 리소스 그룹을 선택합니다.
1. 액세스 제어(IAM) 클릭
1. 역할 할당 추가 -> 추가를 클릭합니다.
1. 기여자 역할을 선택합니다.
1. '액세스 할당'에 대한 '가상 컴퓨터'를 선택합니다.
1. SAP 라마 인스턴스가 실행 중인 가상 컴퓨터를 선택합니다.
1. 저장을 클릭합니다.
1. SAP LaMa에서 사용하려는 모든 리소스 그룹에 대한 단계를 반복합니다.

SAP LaMa Azure 커넥터 구성에서 '관리되는 ID 사용'을 선택하여 관리되는 ID를 사용하도록 설정합니다. 시스템에 할당된 ID를 사용하려면 사용자 이름 필드를 비워 두어야 합니다. 사용자 할당된 ID를 사용하려면 사용자 이름 필드에 할당된 ID ID를 입력합니다.

### <a name="create-a-new-connector-in-sap-lama"></a>SAP 라마에 새 커넥터 만들기
...
* 사용자 이름: 서비스 주체 응용 프로그램 ID 또는 가상 시스템의 ID를 할당한 사용자의 ID입니다. 자세한 내용은 [시스템 또는 사용자 할당된 ID 사용] 을 참조하십시오.
* 암호: 서비스 주체 키/암호. 시스템 또는 사용자가 할당한 ID를 사용하는 경우 이 필드를 비워 둘 수 있습니다.
* URL: 기본값 `https://management.azure.com/` 유지
* Monitoring Interval (Seconds)(모니터링 간격(초)): 300 이상이어야 합니다.
* 관리되는 ID 사용: SAP LaMa는 시스템 또는 사용자에게 할당된 ID를 사용하여 Azure API에 대해 인증할 수 있습니다. 이 [가이드에서 Azure API에 액세스하려면 관리되는 ID 사용](lama-installation.md#af65832e-6469-4d69-9db5-0ed09eac126d) 장을 참조하십시오.
...
* 비용을 절감하기 위해 저장소 유형을 변경: Azure 어댑터가 디스크를 사용하지 않을 때 비용을 절감하기 위해 관리디스크의 저장소 유형을 변경해야 하는 경우 이 설정을 사용하도록 설정합니다. SAP 인스턴스 구성에서 참조되는 데이터 디스크의 경우 어댑터는 인스턴스 준비 해제 중에 디스크 유형을 표준 저장소로 변경하고 인스턴스를 준비하는 동안 원래 저장소 유형으로 돌아갑니다. SAP LaMa에서 가상 컴퓨터를 중지하면 어댑터가 OS 디스크를 포함한 연결된 모든 디스크의 저장소 유형을 표준 저장소로 변경합니다. SAP LaMa에서 가상 컴퓨터를 시작하면 어댑터가 저장소 유형을 원래 저장소 유형으로 다시 변경합니다.
...
#### <a name="install-sap-netweaver-ascs-for-sap-hana-using-azure-managed-disks"></a>Azure 관리 디스크를 사용하여 SAP HANA용 SAP NetWeaver ASCS 설치
...
#### <a name="install-sap-netweaver-ascs-for-sap-hana-on-azure-netappfiles-anf-beta"></a>Azure NetAppFiles (ANF) 베타에 SAP HANA용 SAP 넷위버 ASCS 설치

> [!NOTE]
> 이 기능은 아직 GA도 아닙니다. 자세한 내용은 SAP Note [2815988(미리] 보기 고객에게만 표시)을 참조하십시오.
구성 요소 BC-VCM-LVM-HYPERV에서 SAP 인시던트를 열고 Azure NetApp 파일 미리 보기용 라마 스토리지 어댑터 에 가입요청

ANF는 Azure에 대해 NFS를 제공합니다. SAP LaMa의 맥락에서 이것은 ABAP 중앙 서비스(ASCS) 인스턴스의 생성과 응용 프로그램 서버의 후속 설치를 단순화합니다. 이전에는 ASCS 인스턴스도 NFS 서버역할을 해야 했으며, acosprep/nfs_paths 매개 변수를 SAP 호스트 에이전트의 host_profile 추가해야 했습니다.

#### <a name="anf-is-currently-available-in-these-regions"></a>ANF는 현재 다음 리전에서 사용할 수 있습니다.

오스트레일리아 동부, 미국 중부, 미국 동부, 미국 동부 2, 북유럽, 미국 중남부, 서유럽 및 미국 서부 2.

#### <a name="network-requirements"></a>네트워크 요구 사항

ANF에는 SAP 서버와 동일한 VNET의 일부여야 하는 위임된 서브넷이 필요합니다. 이러한 구성에 대한 예는 다음과 같습니다.
이 화면에는 VNET 및 첫 번째 서브넷의 생성이 표시됩니다.

![SAP 라마는 Azure ANF용 가상 네트워크를 만듭니다. ](media/lama/sap-lama-createvn-50.png)

다음 단계에서는 Microsoft.NetApp/볼륨에 대해 위임된 서브넷을 만듭니다.

![SAP 라마 추가 위임된 서브넷 ](media/lama/sap-lama-addsubnet-50.png)

![서브넷의 SAP 라마 목록 ](media/lama/sap-lama-subnets.png)

이제 Azure 포털 내에서 NetApp 계정을 만들어야 합니다.

![SAP 라마는 넷 앱 계정을 만들 ](media/lama/sap-lama-create-netappaccount-50.png)

![SAP 라마 넷앱 계정 생성 ](media/lama/sap-lama-netappaccount.png)

NetApp 계정 내에서 용량 풀은 각 풀의 디스크 크기와 유형을 지정합니다.

![SAP 라마, NetApp 용량 풀 생성 ](media/lama/sap-lama-capacitypool-50.png)

![SAP 라마 넷앱 용량 풀 생성 ](media/lama/sap-lama-capacitypool-list.png)

이제 NFS 볼륨을 정의할 수 있습니다. 한 풀에 여러 시스템에 대한 볼륨이 있기 때문에 자체 설명 명명 체계를 선택해야 합니다. SID를 추가하면 관련 볼륨을 함께 그룹화하는 데 도움이 됩니다. ASCS 및 AS 인스턴스의 경우 다음과 같은 마운트가 필요합니다: *\</sapmnt/\>SID,* */usr/sap/SID,\<\>* 및 */home/sid\<\>adm*. *선택적으로, /usr/sap/trans는* 적어도 하나의 풍경의 모든 시스템에서 사용되는 중앙 전송 디렉토리에 필요합니다.

> [!NOTE]
> 베타 단계에서 볼륨의 이름은 구독 내에서 고유해야 합니다.

![SAP 라마는 볼륨 1을 만듭니다. ](media/lama/sap-lama-createvolume-80.png)

![SAP 라마는 볼륨 2를 만듭니다. ](media/lama/sap-lama-createvolume2-80.png)

![SAP 라마는 볼륨 3을 만듭니다. ](media/lama/sap-lama-createvolume3-80.png)

이러한 단계는 다른 볼륨에도 반복되어야 합니다.

![생성된 볼륨의 SAP 라마 목록 ](media/lama/sap-lama-volumes.png)

이제 이러한 볼륨은 SAP SWPM을 사용하여 초기 설치가 수행되는 시스템에 장착해야 합니다.

먼저 마운트 포인트를 만들어야 합니다. 이 경우 SID는 AN1이므로 다음 명령을 실행해야 합니다.
...
```
마운트 명령은 포털에서 파생될 수도 있습니다. 로컬 마운트 지점을 조정해야 합니다.

df-h 명령을 사용하여 확인합니다.

![SAP 라마 마운트 포인트 OS 레벨 ](media/lama/sap-lama-mounts.png)

이제 SWPM을 가진 설치를 수행해야 합니다.

하나 이상의 AS 인스턴스에 대해 동일한 단계를 수행해야 합니다.

성공적으로 설치한 후 SAP LaMa 내에서 시스템을 검색해야 합니다.

탑재 점은 ASCS 및 AS 인스턴스에 대해 다음과 같아야 합니다.

![라마의 SAP 라마 마운트 ](media/lama/sap-lama-ascs.png) 포인트(예시입니다. IP 주소와 내보내기 경로는 이전에 사용된 주소와 다릅니다.
