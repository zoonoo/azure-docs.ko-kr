...
Azure 가상 머신에서 실행되는 SAP HANA에 대한 백업 가이드에서는 Azure 관련 항목만 설명합니다. 일반적인 SAP HANA 백업 관련 항목은 SAP HANA 설명서를 확인 하세요. 사용자는 주요 데이터베이스 백업 전략, 이유 및 동기를 사용 하 여 사운드와 유효한 백업 전략을 파악 하 고, 백업 절차, 백업 보존 기간 및 복원에 대 한 요구 사항을 알고 있어야 합니다. 여기서.
...
표시 되는 첫 번째 시나리오는 Azure Backup 서비스가 SAP HANA `backint` 인터페이스를 사용 하 여 SAP HANA 데이터베이스에서 스트리밍 백업을 수행 하는 시나리오입니다. 또는 Azure Backup 서비스에 대 한 보다 일반적인 기능을 사용 하 여 응용 프로그램 일치 디스크 스냅숏을 만들고이를 Azure Backup 서비스로 전송 합니다.
