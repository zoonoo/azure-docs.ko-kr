...
다음 섹션에서, 그림에서 “default”라고 불리는 서브넷의 IP 주소 범위를 **Azure VM 서브넷 IP 주소 범위**라고 합니다. 게이트웨이 서브넷의 IP 주소 범위를 **가상 네트워크 게이트웨이 서브넷 IP 주소 범위**라고 합니다. 

위의 두 그림에서 **가상 네트워크 주소 공간**은 **Azure VM 서브넷 IP 주소 범위** 및 **가상 네트워크 게이트웨이 서브넷 IP 주소 범위**를 모두 포함합니다. 

IP 주소 범위를 절약하거나 구체적으로 지정해야 하는 경우 가상 네트워크의 **가상 네트워크 주소 공간**을 각 서브넷에 사용되는 특정 범위로 제한할 수 있습니다. 가상 네트워크의 **가상 네트워크 주소 공간**을 여기 표시된 것처럼 여러 특정 범위로 정의할 수 있습니다.

![두 공간이 있는 Azure Virtual Network 주소 공간](./media/hana-overview-connectivity/image3-azure-vnet-address-space_alternate.png)

이 경우 **가상 네트워크 주소 공간**에는 두 개의 공간이 정의되어 있습니다. 이 두 공간은 **Azure VM 서브넷 IP 주소 범위** 및 **가상 네트워크 게이트웨이 서브넷 IP 주소 범위**에 대해 정의된 IP 주소 범위와 동일합니다.
...
여러 VM 서브넷 및 연속되지 않은 주소 범위를 사용할 수 있습니다. 이러한 주소 범위는 가상 네트워크의 **가상 네트워크 주소 공간**에 포함되어야 합니다. 이 주소 공간은 집계된 형태이거나 VM 서브넷 및 게이트웨이 서브넷의 정확한 범위 목록일 수 있습니다.
...
- **가상 네트워크 주소 공간**은 Azure VM 서브넷 IP 주소 범위 및 가상 네트워크 게이트웨이 서브넷 IP 주소 범위에 대한 범위를 다루는 더 큰 범위일 수 있습니다.
...
- 주소 공간은 Azure에서 HANA 대규모 인스턴스 단위로 트래픽을 허용 또는 차단하기 위해 HANA 대규모 인스턴스 쪽의 필터로 사용됩니다. Azure Virtual Network의 BGP 라우팅 정보와 HANA 대규모 인스턴스 측에서 필터링을 위해 구성된 IP 주소 범위가 일치하는지 확인합니다. 일치하지 않으면 연결 문제가 발생할 수 있습니다.
...
- **가상 네트워크 주소 공간:** 앞서 소개한 것처럼 **가상 네트워크 주소 공간**은 SAP HANA 대규모 인스턴스 환경에 연결되는 Azure Virtual Network의 주소 공간 매개 변수에 할당한(또는 할당 계획) IP 주소 범위입니다.

 이 주소 공간 매개 변수는 이전 그림과 같이 Azure VM 서브넷 범위와 Azure 게이트웨이 서브넷 범위로 구성된 여러 줄 값으로 지정하는 것이 좋습니다. 이 범위는 온-프레미스, 서버 IP 풀 또는 ER-P2P 주소 범위와 겹치면 안 됩니다. 
 
이러한 IP 주소 범위는 어떻게 가져오나요? 

회사 네트워크 팀 또는 서비스 공급자는 네트워크 내에서 사용되지 않는 하나 이상의 IP 주소 범위를 제공해야 합니다. 예를 들어 Azure VM 서브넷이 10.0.1.0/24이고 Azure 게이트웨이 서브넷이 10.0.2.0/28이면, Azure Virtual Network 주소 공간은 10.0.1.0/24 및 10.0.2.0/28로 두 줄이 되도록 하는 것이 좋습니다. 

주소 공간 값을 집계할 수 있지만 서브넷 범위와 일치시키는 것이 좋습니다. 이렇게 하면 나중에 네트워크의 다른 위치에 있는 더 큰 주소 공간 내에 사용하지 않은 IP 주소 범위를 실수로 다시 사용하는 문제를 방지할 수 있습니다. **가상 네트워크 주소 공간은 초기 배포를 요청할 때 Microsoft에 제출해야 하는 IP 주소 범위입니다.**

- **Azure VM 서브넷 IP 주소 범위:** 앞에서 설명한 것처럼 SAP HANA 대규모 인스턴스 환경에 연결되는 Azure Virtual Network의 Azure Virtual Network 서브넷 매개 변수에 할당한(또는 할당 계획) IP 주소 범위입니다. 이 IP 주소 범위는 IP 주소를 Azure VM에 할당하는 데 사용됩니다. 이 범위를 벗어난 IP 주소 범위가 SAP HANA 큰 인스턴스 환경에 연결되도록 허용됩니다. 필요한 경우 여러 Azure VM 서브넷을 사용할 수 있습니다. 각 Azure VM 서브넷에 대해 /24 CIDR 블록이 권장됩니다. 이 주소 범위는 Azure Virtual Network 주소 공간에 사용된 값에 포함되어야 합니다. 
...
- **서버 IP 풀 주소 범위:** 이 IP 주소 범위는 개별 IP 주소를 HANA 대규모 인스턴스 서버에 할당하는 데 사용됩니다. 권장되는 서브넷 크기는 /24 CIDR 블록입니다. 필요한 경우 64개 IP 주소만큼 더 작은 수도 있습니다. 이 범위에서 처음 30개 IP 주소는 Microsoft에서 사용하도록 예약됩니다. 범위의 크기를 선택할 때 이러한 사실을 고려해야 합니다. 이 범위는 온-프레미스 또는 기타 Azure IP 주소와 겹치면 안 됩니다. 이 IP 주소 범위는 어떻게 가져오나요? 회사 네트워크 팀 또는 서비스 공급자는 네트워크 내에서 현재 사용되지 않는 IP 주소 범위를 제공해야 합니다. Azure(대규모 인스턴스)에서 SAP HANA에 필요한 특정 IP 주소를 할당하는 데 사용할 /24(권장) 고유 CIDR 블록 **이 범위는 초기 배포를 요청할 때 Microsoft에 제출해야 하는 IP 주소 범위입니다.**
 
앞서 설명한 IP 주소 범위를 정의하고 계획해야 하지만 모든 주소 범위를 Microsoft에 전송할 필요는 없습니다. Microsoft에 이름을 지정해야 하는 IP 주소 범위는 다음과 같습니다.
...
다음은 구성하고 궁극적으로 Microsoft에 제공해야 하는 다양한 범위 및 몇 가지 범위 예입니다. Azure Virtual Network 주소 공간의 값은 첫 번째 예에서는 집계되지 않지만 첫 번째 Azure VM 서브넷 IP 주소 범위와 가상 네트워크 게이트웨이 서브넷 IP 주소 범위의 범위에서 정의됩니다. 
...
위 예제에서 볼 수 있듯이 Azure Virtual Network의 주소 공간을 정의한 두 개의 작은 범위 대신 4096개의 IP 주소를 포함하는 하나의 더 큰 범위를 포함합니다. 주소 공간의 그러한 큰 정의는 다소 큰 범위를 사용하지 않고 남겨 둡니다. 가상 네트워크 주소 공간 값이 BGP 경로 전파에 사용되므로 온-프레미스 또는 네트워크의 다른 곳에서 사용하지 않는 범위를 사용하면 라우팅 문제가 발생할 수 있습니다. 
