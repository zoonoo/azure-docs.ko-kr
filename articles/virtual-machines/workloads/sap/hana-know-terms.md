...
- **SAP 환경**: IT 환경에서 전체 SAP 자산을 나타냅니다. SAP 지형에는 모든 프로덕션 및 비프로덕션 환경이 포함됩니다.
...
- **프레미스 간**: VM이 온-프레미스 데이터 센터와 Azure 간에 사이트 간, 다중 사이트 또는 ExpressRoute 방식으로 연결된 Azure 구독에 배포되는 시나리오를 설명합니다. 공통 Azure 설명서에서 이러한 종류의 배포를 크로스-프레미스 시나리오라고도 합니다. 이러한 연결은 온-프레미스 도메인, 온-프레미스 Azure Active Directory/OpenLDAP 및 온-프레미스 DNS를 Azure로 확장하기 위한 것입니다. 온-프레미스 배경은 Azure 구독의 Azure 자산으로 확장됩니다. 이 확장을 사용하면 VM이 온-프레미스 도메인에 속할 수 있습니다. 
   ...
   온-프레미스 도메인의 도메인 사용자는 서버에 액세스하고, 이러한 VM에서 서비스(예: DBMS 서비스)를 실행할 수 있습니다. 온-프레미스에 배포된 VM과 Azure에 배포된 VM 간의 통신 및 이름 확인이 가능합니다. 이 시나리오는 대부분의 SAP 자산이 배포되는 전형적인 방식입니다. 자세한 내용은 [Azure VPN 게이트웨이를](../../../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) 참조하고 [Azure 포털을 사용하여 사이트 간 연결이 있는 가상 네트워크 만들기를](../../../vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)참조하십시오.
...
- **스탬프**: HANA 대형 인스턴스의 Microsoft 내부 배포 크기를 정의합니다. HANA 대형 인스턴스 장치를 배포하려면 컴퓨팅, 네트워크 및 스토리지 랙으로 구성된 HANA 대형 인스턴스 스탬프를 데이터 센터 위치에 배포해야 합니다. 이러한 배포를 HANA 대형 인스턴스 스탬프 또는 개정 4(아래 참조)에서 **대형 인스턴스 행의** 대체 용어를 사용합니다.
- **개정**: HANA 대형 인스턴스 스탬프에 대한 두 가지 스탬프 개정이 있습니다. 이러한 아키텍처및 Azure 가상 시스템 호스트에 대한 근접성은 다릅니다.
    - "개정 3"(Rev 3): 2016년 중반부터 배포된 오리지널 디자인입니다.
    - "개정 4"(Rev 4): Azure 가상 시스템 호스트에 근접하고 Azure VM과 HANA 대형 인스턴스 단위 간의 네트워크 대기 시간이 더 짧은 새로운 디자인입니다. 
