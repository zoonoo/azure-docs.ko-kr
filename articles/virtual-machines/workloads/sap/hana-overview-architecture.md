...
문서는 Azure(큰 인스턴스)에서 새 SAP HANA에 SAP를 배포하는 데 도움이 되는 정보를 제공하는 다섯 부분으로 된 아키텍처 및 기술적 배포 가이드입니다. 포괄적인 내용은 다루지 않으며 SAP 솔루션 설정과 관련된 구체적인 정보는 제공하지 않습니다. 대신, 초기 배포 및 진행 중인 작업에 도움이 되는 유용한 정보를 제공합니다. SAP HANA 설치와 관련된 SAP 설명서(또는 해당 항목을 다루는 여러 SAP 지원 참고 사항) 대신 이 문서를 사용하지 마세요. 또한 Azure(큰 인스턴스)에서 SAP HANA를 설치하는 방법도 제공합니다.
...
HANA 큰 인스턴스 스탬프의 전체 인프라 스택이 단일 고객 용도로 단독으로 할당되지 않는지 확인해야 합니다. 이 사실은 Azure에 배포된 네트워크 패브릭을 통해 연결된 계산 및 저장소 리소스 랙에 적용됩니다. HANA 큰 인스턴스 인프라는 Azure와 마찬가지로 네트워크 격리를 통해 서로 격리된 서로 다른 고객 &quot;테넌트&quot;를 배포합니다. 따라서 HANA 큰 인스턴스의 이러한 배포는 서로 방해하지 않으며 서로를 볼 수도 없습니다. 청구 측면에서 큰 인스턴스 스탬프에 배포된 테넌트가 하나의 Azure 구독에 할당됩니다. 그러나 네트워크 측면에서 그것은 동일한 Azure 등록 내에서 다른 Azure 구독의 Azure Vnet에서 액세스할 수 있습니다. 또한 동일한 Azure 지역의 또 다른 Azure 구독을 사용하여 배포할 경우 별도의 HANA 큰 인스턴스 테넌트를 요청하기로 선택할 수 있습니다.
...
- Azure와 달리 Azure(큰 인스턴스) 서버에서 SAP HANA는 특정 고객이 전용으로 사용할 수 있습니다. 서버를 다시 부팅하거나 종료해도 운영 체제 및 SAP HANA가 다른 서버에 배포되지 않습니다. (Type I 클래스 SKU의 경우 유일한 예외 사항은 서버에서 문제가 발생할 경우 다른 서버에서 재배포를 수행해야 한다는 점입니다.)
...
여러 고객이 SAP HANA 큰 인스턴스 스탬프에서 배포를 받으며 각 고객은 자신의 고유한 VLAN에 배포하여 서로 보호됩니다. HANA 큰 인스턴스를 Azure Virtual Network(VNet)에 연결하기 위해 네트워킹 구성 요소는 HANA 큰 인스턴스 단위의 테넌트를 테넌트 Azure 구독의 Azure VNet에 격리된 방식으로 연결합니다. 
