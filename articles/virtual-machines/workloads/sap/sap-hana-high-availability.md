---
title: SLES의 Azure VM에서 SAP HANA의 고가용성 | 마이크로 소프트 문서
...
* SAP 주 [1944799SAP] 응용 프로그램에 대 한 SUSE 리눅스 엔터프라이즈 서버에 대 한 SAP HANA 지침.
...
1. 다음 매개 변수를 입력합니다.
    ...
    - **스택 유형**: (이 매개 변수는 수렴 된 템플릿을 사용하는 경우에만 적용됩니다.) SAP NetWeaver 스택 유형을 선택합니다.
    - **OS 유형**: Linux 배포판 중 하나를 선택합니다. 이 예제에서는 **SLES 12를**선택합니다.
    - **DB 유형**: **HANA**를 선택합니다.
    ...
    - **서브넷 ID**: VM을 정의한 서브넷이 있는 기존 VNet에 VM을 배포하려면 해당 하위 넷의 ID 이름을 지정합니다. ID는 일반적으로 **/subscriptions/구독\<ID>/resourceGroups/리소스\<그룹 이름>/공급자/Microsoft.Network/가상 네트워크/가상\<네트워크\<이름>/서브넷/서브넷 이름>** 같습니다.
...
1. **[2]** 두 번째 노드에서 시스템 복제를 구성합니다.
   ...
   두 번째 노드를 등록하여 시스템 복제를 시작합니다. <하나시드\>adm으로 다음 명령을 실행 :
