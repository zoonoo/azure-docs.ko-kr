...
5. [Azure NetApp Files에 대 한 NFS 볼륨 만들기](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes)의 지침에 따라 Azure NetApp Files 볼륨을 배포 합니다.  
   ...
   볼륨을 배포할 때 **nfsv 4.1** 버전을 선택 해야 합니다. 지정 된 Azure NetApp Files [서브넷](https://docs.microsoft.com/rest/api/virtualnetwork/subnets)에 볼륨을 배포 합니다. 
...
3. **[A]** Azure NetApp Files에 대 한 통신이 저장소 네트워크 인터페이스를 통해 이동 하도록 네트워크 경로를 추가 합니다.  
   ...
   이 예제에서는 `Networkmanager`를 사용 하 여 추가 네트워크 경로를 구성 합니다. 다음 지침에서는 저장소 네트워크 인터페이스가 `eth1`된 것으로 가정 합니다.  
   먼저 장치 `eth1`의 연결 이름을 결정 합니다. 이 예제에서는 장치 `eth1`의 연결 이름이 `Wired connection 1`됩니다.  
   ...
   그런 다음 `eth1`를 통해 Azure NetApp Files 위임 된 네트워크에 대 한 추가 경로를 구성 합니다.  
...
3. **[A]** [Netapp AFF Systems with nfs 구성 가이드의 SAP HANA](https://www.netapp.com/us/media/tr-4435.pdf)에 설명 된 대로 nfs를 사용 하 여 Azure netapp에서 SAP HANA를 실행 하기 위해 OS를 준비 합니다. NetApp 구성 설정에 대 한 */etc/sysctl.d/netapp-hana.conf* 구성 파일을 만듭니다.  
...
5. **[A]** [NFS를 사용 하는 Netapp AFF 시스템에서 SAP HANA 구성 가이드](https://www.netapp.com/us/media/tr-4435.pdf)에서 권장 하는 대로 sunrpc 설정을 조정 합니다.  
...
6. **[A]** HANA 구성에 대 한 red Hat.
    ...
    > [!NOTE]
    > HANA 2.0 SP04을 설치 하는 경우 SAP HANA을 설치 하려면 먼저 SAP note [2593824]에 설명 된 대로 패키지 `compat-sap-c++-7`를 설치 해야 합니다. 
