...
이 가이드에서는 NW1 및 NW2라는 두 개의 SAP 시스템에 사용되는 고가용성 NFS 서버를 설정하는 방법에 대해 설명합니다. 예제에서 리소스 이름 (예: 가상 머신, 가상 네트워크)은 [SAP 파일 서버 템플릿을][template-file-server] 리소스 접두사 **prod**와 함께 사용 한다고 가정 합니다.
...
* 프로브 포트
  * NW1의 경우 포트 61000
  * NW2의 경우 포트 61001
* Loadbalancing 규칙 (기본 부하 분산 장치를 사용 하는 경우)
  * NW1의 경우 2049 TCP
  * NW1의 경우 2049 UDP
  * NW2의 경우 2049 TCP
  * NW2의 경우 2049 UDP
...
1. Load Balancer (내부)를 만듭니다. [표준 부하 분산 장치](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-overview)를 권장 합니다.  
   1. 표준 부하 분산 장치를 만들려면 다음 지침을 따르세요.
      ...
      1. 부하 분산 규칙
         ...
         1. 이전에 만든 프런트 엔드 IP 주소, 백 엔드 풀 및 상태 프로브 (예: **n w 1**)를 선택 합니다. **n w 1-백 엔드** 및 **n w 1-hp**)
         1. **HA 포트**를 선택 합니다.
