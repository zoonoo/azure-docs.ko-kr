---
title: SLES에서 Azure NetApp 파일로 대기를 사용하는 SAP HANA 확장 | 마이크로 소프트 문서
description: SAP 응용 프로그램에 대 한 Azure NetApp 파일 사용 SUSE 리눅스 엔터프라이즈 서버에서 SAP NetWeaver에 대 한 고가용성 가이드
...
이 문서에서는 공유 저장소 볼륨에 [Azure NetApp 파일을](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-introduction/) 사용하여 Azure 가상 시스템(VM)에서 대기가 있는 확장 구성에서 가용성이 높은 SAP HANA 시스템을 배포하는 방법에 대해 설명합니다.  

예제 구성, 설치 명령 등에서 HANA 인스턴스는 **03이고** HANA 시스템 ID는 **HN1입니다.** 이 예제는 SAP 12 SP4용 HANA 2.0 SP4 및 SUSE Linux 엔터프라이즈 서버를 기반으로 합니다. 
...
* SAP 참고 [2205917]: SAP 응용 프로그램에 대한 SUSE 리눅스 엔터프라이즈 서버에 대한 권장 OS 설정을 포함
* SAP 참고 [1944799]: SAP 응용 프로그램에 대한 SUSE 리눅스 엔터프라이즈 서버에 대한 SAP 지침을 포함
...
* SAP 참고 [1984787]: SUSE 리눅스 엔터프라이즈 서버 12에 대한 일반 정보가 포함되어 있습니다.
...
5. Azure NetApp 파일에 [대한 NFS 볼륨 만들기의](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-create-volumes)지침에 따라 Azure NetApp 파일 볼륨을 배포합니다.  
   ...
   볼륨을 배포할 때 **NFSv4.1** 버전을 선택해야 합니다. 현재 NFSv4.1에 액세스하려면 추가 허용 목록이 필요합니다. 지정된 Azure NetApp 파일 [서브넷에](https://docs.microsoft.com/rest/api/virtualnetwork/subnets)볼륨을 배포합니다. 
   
   Azure NetApp 파일 리소스 및 Azure VM은 동일한 Azure 가상 네트워크 또는 피어있는 Azure 가상 네트워크에 있어야 합니다. 예를 들어 **HN1**-data-mnt0001, **HN1**-log-mnt00001 등은 볼륨 이름과**nfs://10.23.1.5/ HN1**-data-mnt00001, nfs://10.23.1.4/**HN1**-log-mnt00001 등Azure NetApp 파일 볼륨의 파일 경로입니다.  

   * 볼륨 **HN1**-데이터-mnt00001(nfs://10.23.1.5/**HN1**-데이터-mnt00001)
   * 볼륨 **HN1**-데이터-mnt00002(nfs://10.23.1.6/**HN1**-데이터-mnt00002)
   * 볼륨 **HN1**-log-mnt00001 (nfs://10.23.1.4/**HN1**-log-mnt00001)
   * 볼륨 **HN1**-log-mnt00002 (nfs://10.23.1.6/**HN1**-log-mnt00002)
   * 볼륨 **HN1**- 공유 (nfs://10.23.1.4/**HN1**공유)
   
   이 예제에서는 각 HANA 데이터 및 로그 볼륨에 대해 별도의 Azure NetApp 파일 볼륨을 사용했습니다. 더 작거나 비생산적인 시스템에서 보다 비용 최적화된 구성을 위해 모든 데이터 마운트와 모든 로그를 단일 볼륨에 배치할 수 있습니다.  

### <a name="important-considerations"></a>중요 고려 사항

SUSE 고가용성 아키텍처에서 SAP NetWeaver용 Azure NetApp 파일을 만들 때 다음과 같은 중요한 사항을 알고 있어야 합니다.
...
- 1MB I/O 크기로 초당 250MB(MB/s)의 /hana/log에서 읽기 쓰기를 활성화합니다.  
- 16MB 및 64MB I/O 크기에 대해 /hana/data에 대해 최소 400MB/s의 읽기 작업을 활성화합니다.  
- 16MB 및 64MB I/O 크기의 /hana/데이터에 대해 최소 250MB/s의 쓰기 작업을 활성화합니다. 
...
2. 다음 단계를 수행하여 세 가지 가상**컴퓨터(hanadb1,** **hanadb2,** **hanadb3)를**만듭니다.  
   ...
   a. SAP HANA에 지원되는 Azure 갤러리에서 SLES4SAP 이미지를 사용합니다. 이 예제에서는 SLES4SAP 12 SP4 이미지를 사용했습니다.  
...
2. **[A]** 의도하지 않은 호스트 이름 변경을 방지하기 위해 스토리지용 네트워크 인터페이스에 대한 DHCP 및 클라우드 구성 설정을 변경합니다.  

    다음 지침은 저장소 네트워크 인터페이스가 `eth1`있는 것으로 가정합니다. 
...
2. **[A]** Azure NetApp 파일에 대한 통신이 저장소 네트워크 인터페이스를 통해 전달되도록 네트워크 경로를 추가합니다.  
...
4. **[A]** Azure 구성 설정에 대한 Microsoft와 구성 파일 */etc/sysctl.d/ms-az.conf를* 만듭니다.  
...
5. **[A]** SAP HANA 그룹과 사용자를 수동으로 만듭니다. 그룹 수액 및 사용자 **hn1**광고의 경우 온보딩 중에 제공되는 동일한 아이디로 설정해야 합니다. (이 예제에서는 아이디가 **1001로**설정됩니다.) 아이디가 올바르게 설정되지 않으면 볼륨에 액세스할 수 없습니다. 그룹 수액 및 사용자 계정 **hn1**adm 및 sapadm에 대한 ID는 모든 가상 컴퓨터에서 동일해야 합니다.  
...
1. **[1]** SAP HANA 2.0 설치 및 업데이트 가이드의 지침에 따라 [SAP HANA를 설치합니다.](https://help.sap.com/viewer/2c1988d620e04368aa4103bf26f17727/2.0.04/en-US/7eb0167eb35e4e2885415205b8383584.html) 이 예제에서는 마스터, 작업자 1개 및 대기 노드 1개를 사용하는 SAP HANA 스케일 아웃을 설치합니다.  
   ...
   b. 프롬프트에서 다음 값을 입력합니다.
     ...
     * **루트 사용자 암호의**경우 : 루트 사용자의 암호를 입력합니다.
