---
title: "SAP (A)SCS 인스턴스에 Windows 장애 조치(failover) 클러스터 및 공유 디스크를 사용하여 SAP HA를 위한 Azure 인프라 준비 | Microsoft Docs"
description: "SAP (A)SCS 인스턴스에 Windows 장애 조치(failover) 클러스터 및 공유 디스크를 사용하여 SAP HA를 위한 Azure 인프라 준비"
...
---
# <a name="azure-infrastructure-preparation-for-sap-ha-using-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance"></a>SAP (A)SCS 인스턴스에 Windows 장애 조치(failover) 클러스터 및 공유 디스크를 사용하여 SAP HA를 위한 Azure 인프라 준비
...
이 문서에서는 SAP (A)SCS 인스턴스를 클러스터링하는 옵션으로 **클러스터 공유 디스크**를 사용하여 **WSFC(Windows 장애 조치(failover) 클러스터)**에 고가용성 SAP 시스템을 설치하고 구성하는 데 필요한 Azure 인프라 준비 단계를 설명합니다.

## <a name="prerequisites"></a>필수 조건

설치를 시작하기 전에 다음 문서를 검토해야 합니다.

* [아키텍처 가이드 - **클러스터 공유 디스크**][를 사용하여 SAP (A)SCS 인스턴스를 **Windows 장애 조치(failover) 클러스터**에 클러스터링sap-high-availability-guide-wsfc-shared-disk]

* [**SAP (A)SCS** 인스턴스에 **Windows 장애 조치(failover) 클러스터** 및 **공유 디스크**를 사용하여 SAP HA를 위한 Azure 인프라 준비][sap-high-availability-infrastructure-wsfc-shared-disk]
...
## <a name="c87a8d3f-b1dc-4d2f-b23c-da4b72977489"></a> 프로덕션 환경에서 사용하기 위해 회사 네트워크 연결(크로스-프레미스)을 사용하여 Virtual Machines 배포
...
데이터베이스 다중 SID 템플릿을 설정하려면 [데이터베이스 다중 SID 템플릿][sap-templates-3-tier-multisid-db-marketplace-image] 또는 [관리 디스크를 사용하는 데이터베이스 다중 SID 템플릿][sap-templates-3-tier-multisid-db-marketplace-image-md]에서 다음 매개 변수 값을 입력합니다.
  ...
  -  **OS 종류**. 가상 컴퓨터의 운영 체제를 선택합니다.
  -  **데이터베이스 유형**. 클러스터에 설치하려는 데이터베이스의 유형을 선택합니다. Microsoft SQL Server를 설치하려는 경우 **SQL**을 선택합니다. 가상 컴퓨터에 SAP HANA를 설치하려는 경우 **HANA**를 선택합니다. 올바른 운영 체제 종류, 즉, SQL에는 **Windows**를 선택하고 HANA에는 Linux 배포를 선택합니다. 가상 컴퓨터에 연결되는 Azure Load Balancer는 선택한 다음 데이터베이스 형식을 지원하도록 구성됩니다.
...
## <a name="84c019fe-8c58-4dac-9e54-173efd4b2c30"></a> SAP 가상 컴퓨터의 고정 IP 주소 설정
...
2.  **IP 주소** 블레이드의 **할당** 아래에서 **고정**을 선택합니다. **IP 주소** 상자에 사용할 IP 주소를 입력합니다.
  ...
  ![그림 3: 각 가상 컴퓨터의 네트워크 카드에 대한 고정 IP 주소 설정][sap-ha-guide-figure-3002]
...
## <a name="7a8f3e9b-0624-4051-9e41-b73fff816a9e"></a> Azure 내부 부하 분산 장치의 Azure 고정 IP 주소 설정
...
## <a name="f19bd997-154d-4583-a46e-7f5a69d0153c"></a> Azure 내부 부하 분산 장치의 기본 ASCS/SCS 부하 분산 규칙
...
### <a name="fe0bd8b5-2b43-45e3-8295-80bee5415716"></a> Azure 내부 부하 분산 장치의 기본 ASCS/SCS 부하 분산 규칙 변경
...
![그림 7: 도메인에 가상 컴퓨터 추가][sap-ha-guide-figure-3006]

_**그림 7:** 도메인에 가상 컴퓨터 추가_

## <a name="661035b2-4d0f-4d31-86f8-dc0a50d78158"></a> SAP ASCS/SCS 인스턴스의 두 클러스터 노드에 대한 레지스트리 항목 추가
...
## <a name="0d67f090-7928-43e0-8772-5ccbf8f59aab"></a> SAP ASCS/SCS 인스턴스의 Windows Server 장애 조치(Failover) 클러스터링 클러스터 설정
...
2.  장애 조치 클러스터 관리자를 사용하여 장애 조치 클러스터를 설정합니다. 장애 조치 클러스터 관리자에서 **클러스터 만들기**를 선택한 다음 첫 번째 클러스터 노드, 노드 A의 이름만 추가합니다. 아직은 두 번째 노드를 추가하지 마세요. 두 번째 노드는 이후 단계에서 추가할 것입니다.

  ![그림 8: 첫 번째 클러스터 노드의 서버 또는 가상 컴퓨터 이름 추가][sap-ha-guide-figure-3007]
...
6.  클러스터 가상 호스트 이름을 온라인으로 가져옵니다.
  ...
  ![그림 15: 올바른 IP 주소를 사용하여 작동 및 실행하는 클러스터 코어 서비스][sap-ha-guide-figure-3014]

  _**그림 15:** 올바른 IP 주소를 사용하여 작동 및 실행하는 클러스터 코어 서비스_
...
3.  목록에 클러스터 이름 개체를 추가하려면 **추가**를 선택합니다. 위의 그림에서 보여 주는 방법 외에도 필터를 변경하여 컴퓨터 개체를 확인합니다.
...
4.  **그림: 컴퓨터를 포함하도록 개체 형식 변경**에 나와 있는 것처럼 클러스터 이름 개체를 입력합니다. 레코드가 이미 생성되었기 때문에 **그림: 클러스터 이름 개체의 공유를 위한 권한 할당**에 나와 있는 것처럼 권한을 변경할 수 있습니다.
...
5.  원하는 변경 내용을 선택하고 **다음**을 선택합니다. 다음 그림과 같이 클러스터 구성을 다시 구성해야 합니다.  

  ![그림 28: 클러스터를 다시 구성했는지 확인][sap-ha-guide-figure-3027]

  _**그림 28:** 클러스터를 다시 구성했는지 확인_

Windows 장애 조치(failover) 클러스터를 성공적으로 설치한 후 장애 조치(failover) 검색이 Azure의 상태에 맞게 조정되도록 일부 임계값을 변경해야 합니다. 변경할 매개 변수는 [장애 조치(failover) 클러스터 네트워크 임계값 조정][tuning-failover-cluster-network-thresholds] 블로그에 설명되어 있습니다. ASCS/SCS에 대한 Windows 클러스터 구성을 빌드하는 2개의 VM이 동일한 서브넷에 있다고 가정할 경우 다음 매개 변수를 다음과 같은 값으로 변경해야 합니다.
...
### <a name="5c8e5482-841e-45e1-a89d-a05c0907c868"></a> SAP ASCS/SCS 클러스터 공유 디스크용 SIOS DataKeeper Cluster Edition 설치
...
- 다음 그림과 같이 Windows에서 역할 및 기능 추가 마법사를 사용합니다.
...
1.  DataKeeper 관리 및 구성 도구를 시작한 다음 **서버 연결**을 선택합니다. (이 옵션은 빨간색 원으로 표시되어 있습니다.)
...
5.  대상 노드의 이름, TCP/IP 주소 및 디스크 볼륨을 정의합니다.
  ...
  ![그림 41: 현재 대상 노드에 해당하는 노드의 기본 데이터 정의][sap-ha-guide-figure-3041]

  _**그림 41:** 현재 대상 노드에 해당하는 노드의 기본 데이터 정의_
...
8.  복제 작업에 의해 복제되는 볼륨을 Windows Server 장애 조치 클러스터링 클러스터 구성에 공유 디스크로 나타낼지 여부를 정의합니다. SAP ASCS/SCS 구성의 경우 Windows 클러스터가 복제된 볼륨을 클러스터 볼륨으로 사용할 수 있는 공유 디스크로 인식하도록 **예**를 선택합니다.
  ...
  이제 다음 그림과 같이 장애 조치(Failover) 클러스터 관리자에서 디스크를 DataKeeper 디스크로 표시합니다.
...
* [SAP (A)SCS 인스턴스에 **Windows 장애 조치(failover) 클러스터** 및 **공유 디스크**를 사용하여 SAP NetWeaver HA 설치][sap-high-availability-installation-wsfc-shared-disk]
