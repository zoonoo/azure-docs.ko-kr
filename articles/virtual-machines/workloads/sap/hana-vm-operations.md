...
- [SAP 관리 가이드](https://help.sap.com/viewer/6b94445c94ae495c83a19646e7c3fd56/2.0.02/330e5550b09d4f0f8b6cceb14a64cd22.html)
...
- [Azure 가상 시스템](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-manage-vm)
- [Azure 네트워킹 및 가상 네트워크](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-virtual-network)
- [Azure 저장소](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-manage-disks)
...
>[!IMPORTANT]
> M208xx_v2 VM을 사용하려면 Azure VM 이미지 갤러리에서 Linux 이미지를 선택해야 합니다. 세부 정보를 읽으려면 [메모리 최적화 된 가상 컴퓨터 크기를 읽으십시오.](../../mv2-series.md)
> 


### <a name="storage-configuration-for-sap-hana"></a>SAP HANA용 스토리지 구성
Azure에서 SAP HANA와 함께 사용할 저장소 구성 및 저장소 유형에 대 한 문서 [SAP HANA Azure 가상 시스템 저장소 구성을 읽으십시오.](./hana-vm-operations-storage.md)
...
그러나 지속되고 있는 배포의 경우 Azure에서 가상 데이터 센터 네트워크 아키텍처를 만들어야 합니다. 이 아키텍처는 온-프레미스에 연결되는 Azure VNet 게이트웨이를 별도의 Azure VNet으로 분리하는 것이 좋습니다. 이 별도의 VNet은 온-프레미스 또는 인터넷에 남는 모든 트래픽을 호스트해야 합니다. 이 방법을 사용하면 이 별도의 허브 VNet에서 Azure의 가상 데이터 센터로 들어가는 감사 및 로깅 트래픽을 위한 소프트웨어를 배포할 수 있습니다. 따라서 Azure 배포로 들어오고 나가는 트래픽과 관련된 모든 소프트웨어 및 구성을 하나의 VNet이 호스트하도록 해야 합니다.
...
OLAP 확장 또는 S/4HANA 확장 에 대해 인증된 Azure VM 유형을 확인하려면 [SAP HANA 하드웨어 디렉토리를](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)확인하십시오. '클러스터링' 열의 체크 표시는 확장 지원을 나타냅니다. 응용 프로그램 유형은 OLAP 확장 또는 S/4HANA 확장이 지원되는지 여부를 나타냅니다. 각 VM에 대해 확장으로 인증된 노드에 대한 자세한 내용은 SAP HANA 하드웨어 디렉토리에 나열된 특정 VM SKU의 항목에 대한 세부 정보를 확인하십시오.

Azure VM에서 확장 구성을 배포하기 위한 최소 OS 릴리스는 SAP HANA 하드웨어 디렉토리에 나열된 특정 VM SKU의 항목에 대한 세부 정보를 확인합니다. n노드 OLAP 스케일아웃 구성 중 하나의 노드가 마스터 노드로 작동합니다. 인증 한도까지의 다른 노드는 작업자 노드역할을 합니다. 추가 대기 노드는 인증된 노드 수에 포함되지 않습니다.

>[!NOTE]
> 대기 노드가 있는 SAP HANA의 Azure VM 확장 배포는 [Azure NetApp 파일](https://azure.microsoft.com/services/netapp/) 저장소를 통해서만 가능합니다. 다른 SAP HANA 인증 Azure 저장소는 SAP HANA 대기 노드의 구성을 허용하지 않습니다.
>

/hana/shared의 경우 [Azure NetApp 파일의](https://azure.microsoft.com/services/netapp/)사용도 권장합니다. 

확장 구성의 단일 노드에 대한 일반적인 기본 설계는 다음과 같습니다.

![단일 노드의 스케일 아웃 기본 사항](media/hana-vm-operations/scale-out-basics-anf-shared.PNG)

SAP HANA 스케일 아웃에 대한 VM 노드의 기본 구성은 다음과 같습니다.

- **/hana/shared의**경우 Azure NetApp 파일을 통해 제공되는 기본 NFS 서비스를 사용합니다. 
- 다른 모든 디스크 볼륨은 다른 노드 간에 공유되지 않으며 NFS를 기반으로 하지 않습니다. 이 문서의 후반부에서 공유되지 않은 **/hana/data** 및 **/hana/log를** 사용하여 확장 된 HANA 설치를 위한 설치 구성 및 단계가 추가로 제공됩니다. 사용할 수 있는 HANA 인증 저장소의 경우 [SAP HANA Azure 가상 시스템 저장소 구성을 확인하십시오.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)


볼륨 또는 디스크 크기를 조정하면 작업자 노드 수에 따라 필요한 크기에 대한 문서 [SAP HANA TDI 저장소 요구 사항을](https://www.sap.com/documents/2015/03/74cdb554-5a7c-0010-82c7-eda71af511fa.html)확인해야 합니다. 이 문서에서 는 볼륨의 필요한 용량을 얻기 위해 적용해야 하는 수식을 릴리스합니다.

확장 형 SAP HANA VM에 대한 단일 노드 구성의 그래픽에 표시되는 다른 디자인 기준은 VNet 또는 서브넷 구성이 더 좋습니다. SAP는 HANA 노드 간 통신으로부터 트래픽 발생 클라이언트/애플리케이션을 분리할 것을 권장합니다. 그래픽에서와 같이 이는 두 개의 다른 vNIC를 VM에 연결하여 달성됩니다. 두 vNIC는 서로 다른 서브넷에 있고, 서로 다른 두 IP 주소를 가집니다. 그런 다음, NSG 또는 사용자 정의 경로를 사용하여 라우팅 규칙을 통해 트래픽 흐름을 제어합니다.
...
>[!NOTE]
>SAP는 이 문서에 설명된 대로 네트워크 트래픽을 클라이언트/응용 프로그램 측및 노드 내 트래픽으로 분리하는 것이 좋습니다. 따라서 마지막 그래픽에 표시된 대로 아키텍처를 배치하는 것이 좋습니다. 또한 보안 및 규정 준수 팀에 권장 사항에서 벗어난 요구 사항을 문의하십시오. 
>
...
- 새 Azure VNet 인프라 배포 또는 적응
- ANF를 기반으로 Azure 관리 형 프리미엄 스토리지, 울트라 디스크 볼륨 및 /또는 NFS 볼륨을 사용하여 새 VM 배포
...
- Azure Premium Storage 또는 Ultra 디스크 저장소를 /hana/data 및 /hana/log의 공유되지 않는 디스크와 함께 사용하는 경우 global.ini 파일을 변경하고 global.ini 파일에 'basepath_shared = 아니오'라는 매개 변수를 추가해야 합니다. 이 매개 변수를 사용하면 SAP HANA가 노드 간에 '공유' **/hana/data** 및 **/hana/로그** 볼륨 없이 확장에서 실행할 수 있습니다. 자세한 내용은 [SAP 정보 #2080991](https://launchpad.support.sap.com/#/notes/2080991)에 설명되어 있습니다. /hana/data 및 /hana/log에 대해 ANF를 기반으로 NFS 볼륨을 사용하는 경우 이 변경이 필요하지 않습니다.
- global.ini 매개 변수가 결국 변경된 후 SAP HANA 인스턴스를 다시 시작합니다.
...
SUSE Linux에서 대기 노드가 있는 SAP HANA 스케일 아웃 시스템을 설정하는 세부 사항은 [SUSE Linux 엔터프라이즈 서버에서 Azure NetApp 파일을 사용하여 Azure VM에서 대기 노드가 있는 SAP HANA 스케일 아웃 시스템 배포에](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-suse)자세히 설명되어 있습니다. Red Hat에 대한 동등한 설명서는 [Red Hat 엔터프라이즈 Linux에서 Azure NetApp 파일을 사용하여 Azure VM에서 대기 노드가 있는 SAP HANA 스케일 아웃 시스템 배포](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-scale-out-standby-netapp-files-rhel)문서에서 찾을 수 있습니다. 


## <a name="sap-hana-dynamic-tiering-20-for-azure-virtual-machines"></a>Azure 가상 머신에 대한 SAP HANA Dynamic Tiering 2.0

Microsoft Azure에서는 Azure M 시리즈 VM에 대한 SAP HANA 인증 외에도 SAP HANA Dynamic Tiering 2.0을 지원합니다(아래의 SAP HANA Dynamic Tiering 설명서 링크 참조). 예를 들어 Azure Virtual Machine 내에서 SAP HANA Cockpit을 통해 제품을 설치하거나 운영하는 데에는 차이가 없지만 공식적인 Azure 지원에 필수적인 몇 가지 중요한 항목이 있습니다. 이러한 중요 사항은 아래에 설명되어 있습니다. 이 문서 전체에서 "DT 2.0"이라는 약어는 동적 계층화 2.0이라는 전체 이름 대신 사용됩니다.
...
DT 2.0 모범 사례 지침에 따라 디스크 IO 처리량은 물리적 코어당 50MB/초 이상이어야 합니다. DT 2.0에서 지원되는 두 개의 Azure VM 유형에 대한 사양을 살펴보면 VM의 최대 디스크 IO 처리량 제한은 다음과 같습니다.
...
로그 볼륨의 크기와 관련하여 권장되는 시작점은 데이터 크기의 15%로 추론됩니다. 로그 볼륨은 비용 및 처리량 요구 사항에 따라 다양한 Azure 디스크 유형을 사용하여 만들 수 있습니다. 로그 볼륨의 경우 높은 I/O 처리량이 필요합니다.  VM 유형 M64-32ms를 사용하는 경우 쓰기 [가속기를](https://docs.microsoft.com/azure/virtual-machines/linux/how-to-enable-write-accelerator)사용하도록 설정하는 것이 필수적입니다. Azure Write Accelerator는 트랜잭션 로그에 대한 최적 디스크 쓰기 대기 시간을 제공합니다(M 시리즈에서만 사용 가능). VM 유형별 최대 디스크 수와 같이 고려해야 할 몇 가지 항목도 있습니다. Write Accelerator에 대한 자세한 정보는 [여기](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)를 참조할 수 있습니다.
...
- [SAP HANA 스토리지 스냅샷 벤치마크](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-backup-storage-snapshots)
...
SAPRouter는 Jumpbox VM이 아니라 별도의 VM에 설치해야 합니다. 별도의 VM에는 고정 IP 주소가 있어야 합니다. SAProuter를 SAP에서 호스트하는 SAProuter에 연결하려면 SAP에 IP 주소를 문의하세요. SAP에서 호스팅하는 SAprouter는 VM에 설치하는 SAprouter 인스턴스의 해당 라우터입니다. SAP의 IP 주소를 사용하여 SAprouter 인스턴스를 구성합니다. 구성 설정에서 필수 포트는 TCP 포트 3299뿐입니다.
...
SUSE Linux 엔터프라이즈 서버 또는 Red Hat을 실행하는 경우 STONITH 장치를 사용하여 페이스메이커 클러스터를 설정할 수 있습니다. 이 디바이스를 사용하면 HANA 시스템 복제를 통한 동기 복제와 자동 장애 조치(failover)를 사용하는 SAP HANA 구성을 설정할 수 있습니다. 자세한 내용은 '다음 단계' 섹션에 나와 있습니다.
