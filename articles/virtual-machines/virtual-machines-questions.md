...
- Linux 이미지에 대해, 관리 포털을 사용하는 경우, ‘azureuser’가 기본 사용자 이름으로 부여되며 가상 컴퓨터를 만들기 위한 방법으로 ‘빨리 만들기' 대신 ‘갤러리에서"를 사용하여 이 이름을 변경할 수 있습니다. ‘갤러리에서'를 사용하면 로그인을 위해 암호, SSH 키를 사용할 것인지 둘 다 사용할 것인지를 결정할 수 있습니다. 사용자 계정은 권한이 있는 명령을 실행하기 위한 ‘sudo’ 액세스를 할 수 있는 권한이 없는 사용자입니다. ‘root’ 계정은 비활성화 됩니다.
