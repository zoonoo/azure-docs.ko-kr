...
> [!NOTE] 
> 사용 약관에 동의하게 되면 미리 보기를 사용할 수 있습니다. 자세한 내용은 [Microsoft Azure Preview에 대한 Microsoft Azure 추가 사용 약관](https://azure.microsoft.com/en-us/support/legal/preview-supplemental-terms/)을 참조하세요.
>
...
사용자 논리를 테스트하려면 Azure Portal, API, CLI 또는 PowerShell을 사용하여 예약된 이벤트가 발생하는 작업을 시작할 수 있습니다. 가상 컴퓨터를 다시 시작하면 `Reboot`와 같은 이벤트 형식으로 예정된 이벤트가 발생합니다. 가상 컴퓨터를 다시 배포하면 `Redeploy`와 같은 이벤트 형식으로 예정된 이벤트가 발생합니다.
예약된 이벤트를 사용하면 정상적으로 응용 프로그램을 종료하는 데 더 많은 시간이 걸리기 때문에 두 경우 모두 사용자 시작 작업을 완료하는 데에도 더 많은 시간이 걸립니다. 
...
| 리소스| 이 이벤트가 영향을 주는 리소스 목록입니다. 최대 하나의 [업데이트 도메인](windows/manage-availability.md)에 있는 컴퓨터를 포함하지만 UD의 모든 컴퓨터를 포함할 수는 없습니다. <br><br> 예제: <br><ul><li> ["FrontEnd_IN_0", "BackEnd_IN_0"] |
...
다음 샘플은 메타데이터 서비스와 통신하는 단순한 클라이언트의 샘플입니다
...
예약된 이벤트는 다음 데이터 구조를 사용하여 나타낼 수 있습니다.
...
- [인스턴스 메타데이터 서비스](virtual-machines-instancemetadataservice-overview.md)에서 사용 가능한 API에 대해 자세히 알아봅니다.
- [Azure에서 Windows 가상 컴퓨터에 대한 계획된 유지 관리](windows/planned-maintenance.md)에 대해 알아봅니다.
- [Azure에서 Linux 가상 컴퓨터에 대한 계획된 유지 관리](linux/planned-maintenance.md)에 대해 알아봅니다.
