...
1. 설치 시스템에서 액세스할 수 있는 위치에 kickstart 파일을 보관합니다.
2. Hyper-V 관리자에서 새 VM을 만듭니다. **가상 하드 디스크 연결** 페이지에서 **나중에 가상 하드 디스크 연결**을 선택하고 새 가상 컴퓨터 마법사를 완료합니다.
3. VM 설정을 엽니다.
...
4. VM을 시작합니다. 설치 가이드가 나타나면 **Tab** 키를 눌러서 부팅 옵션을 구성합니다.
5. 부팅 옵션 마지막에 `inst.ks=<the location of the kickstart file>`을 입력하고 **Enter** 키를 누릅니다.
6. 설치가 완료될 때까지 기다립니다. 완료되면 VM가 자동으로 종료됩니다. 이제 Linux VHD를 Azure에 업로드할 수 있습니다.
