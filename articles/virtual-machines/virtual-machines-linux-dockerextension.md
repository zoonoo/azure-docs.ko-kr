...
- 앱의 프로토타입을 신속하게 만들려는 경우 또는 Docker Machine에 대해 이미 알고 사용하고 있는 경우 [Docker Machine Azure 드라이버를 사용](./virtual-machines-linux-docker-machine.md)하여 Azure 내 Docker 호스트를 스핀업할 수 있습니다.
- 역할 기반 액세스 제어, 진단, 기록 및 제어된 배포가 포함된 Docker 환경을 재현 가능한 Azure-템플릿-정의 방식으로 배포할 때 더 자세히 제어하려는 경우 Azure Resource Manager와 함께 Azure Docker VM 확장을 사용하는 것이 가장 좋습니다.
...
Azure Docker VM 확장은 Linux VM에 Docker를 설치하고 자동으로 구성해 줍니다. Resource Manager 템플릿을 사용함으로써 환경을 일관적 방식으로 다시 배포할 수 있습니다. Azure Docker VM 확장은 단순히 Docker Machine을 사용하거나 Docker 호스트를 직접 만드는 경우에 비해 추가적인 제어 기능을 제공하므로 더욱 견고한 개발자 또는 운영 환경에 적합합니다.

Azure Resource Manager를 사용하여 Docer 호스트, 저장소, RBAC(역할 기반 액세스 제어), 진단과 같은 환경의 전체 구조를 정의하는 템플릿을 만들고 배포할 수 있습니다. [Resource Manager와 템플릿에 대해 자세히 읽어보고](../resource-group-overview.md) 몇 가지 이점을 더 잘 이해하십시오. 단순히 Docker Machine을 사용하는 경우에 비해 Resource Manager 템플릿을 사용하는 이점은 추가 Docker 호스트, 저장소, 액세스 제어 등을 정의하고 향우 필요할 때 배포를 재현할 수 있다는 점입니다.
...
배포가 완료되면 배포 중 입력한 DNS 이름을 사용하여 새 Docker 호스트에 SSH를 연결합니다. Docker 도구가 이미 설치되어 있으므로 nginx 컨테이너를 스핀업해 보겠습니다.
