...
3. **가용성 집합 만들기** 블레이드에서 해당 집합에 대한 정보를 입력합니다.
   ...
   * **업데이트 도메인** - 5개의 업데이트 도메인이 기본적으로 할당되며 1~20개 사이로 설정할 수 있습니다. 업데이트 도메인은 동시에 다시 부팅할 수 있는 가상 컴퓨터 그룹과 기본 물리적 하드웨어를 나타냅니다. 예를 들어&5;개의 업데이트 도메인을 지정하며 단일 가용성 집합 내에&5;개를 넘는 가상 컴퓨터를 구성한 경우&6;번째 가상 컴퓨터는 동일한 업데이트 도메인에 첫 번째 가상 컴퓨터로 배치되고,&7;번째 가상 컴퓨터는 동일한 UD에 두 번째 가상 컴퓨터로 배치되는 식입니다. 다시 부팅 순서는 순차적이지 않을 수 있지만 한 번에 하나의 업데이트 도메인만 다시 부팅합니다.
...
자세한 내용은 [New-AzureRmAvailabilitySet](/powershell/new-azurermavailabilityset)를 참조하세요.
