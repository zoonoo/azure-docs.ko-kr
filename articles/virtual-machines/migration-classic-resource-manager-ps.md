...
원하는 경우 [Azure CLI](migration-classic-resource-manager-cli.md)를 사용 하 여 리소스를 마이그레이션할 수도 있습니다.
...
3. 적절 한 구독 항목을 찾은 다음 **내 역할** 필드를 확인 합니다. 공동 관리자의 경우이 값은 _계정 관리자_ 여야 합니다.
...
* **옵션 1: Vm을 플랫폼에서 만든 가상 네트워크로 마이그레이션합니다.**
    ...
    다음 명령은 마이그레이션을 차단하는 경고와 오류를 표시합니다. 유효성 검사 메시지에 오류 유형 메시지가 없는 경우 준비 단계로 이동할 수 있습니다.
...
* **옵션 2: 리소스 관리자 배포 모델에서 기존 가상 네트워크로 마이그레이션**
    ...
    다음 명령은 마이그레이션을 차단하는 경고와 오류를 표시합니다. 유효성 검사 메시지에 오류가 없는 경우 다음 준비 단계를 진행할 수 있습니다.
...
> [!NOTE]
> 가상 컴퓨터의 VHD (OS 및 데이터) 파일을 사용 하 여 Managed Disks를 사용 하 여 새 리소스 관리자 가상 컴퓨터를 만들어 클래식 배포 모델을 사용 하 여 만든 [단일 가상 컴퓨터를 마이그레이션합니다](./windows/create-vm-specialized-portal.md) .
<br>
