...
![Azure 가상 네트워크](./media/virtual-machines-windows-network-overview/vnetoverview.png)
...
| Azure CLI 2.0 | **--public-ip-address** 매개 변수와 함께 [az network nic create](https://docs.microsoft.com/cli/azure/network/nic#create)를 사용하여 이전에 만든 공용 IP 주소의 식별자를 제공합니다. |
| [Azure CLI](../virtual-network/virtual-network-deploy-multinic-arm-cli.md) | **--public-ip-address** 매개 변수와 함께 **azure network nic create**를 사용하여 이전에 만든 공용 IP 주소의 식별자를 제공합니다. |
...
| Azure CLI 2.0 | Dynamic 또는 Static인 **--allocation-method** 매개 변수와 함께 [az network public-ip create](https://docs.microsoft.com/cli/azure/network/public-ip#create)를 사용합니다. |
| [Azure CLI](../virtual-network/virtual-network-deploy-static-pip-arm-cli.md) | Dynamic 또는 Static인 **--allocation-method** 매개 변수와 함께 **azure network public-ip create**를 사용합니다. |
...
| Azure CLI 2.0 | 서브넷과 VNet은 동시에 만들어집니다. [az network vnet create](https://docs.microsoft.com/cli/azure/network/vnet#create)에 서브넷 이름으로 **--subnet-name** 매개 변수를 제공합니다. |
| [Azure CLI](../virtual-network/virtual-networks-create-vnet-arm-cli.md) | **azure network vnet create**에 서브넷 이름으로 **-n** 매개 변수를 제공합니다. |
...
| [Azure CLI](../virtual-network/virtual-networks-create-nsg-arm-cli.md) | **azure network nsg create**를 사용하여 NSG를 처음으로 만듭니다. **azure network nsg rule create**를 사용하여 규칙을 NSG에 추가합니다. **azure network vnet subnet update**를 사용하여 NSG를 서브넷에 추가합니다. |
...
| [Azure CLI](../load-balancer/load-balancer-get-started-internet-arm-cli.md) | **azure network lb create**를 사용하여 초기 부하 분산 장치 구성을 만듭니다. **azure network lb frontend-ip create**를 사용하여 이전에 만든 공용 IP 주소를 추가합니다. **azure network lb address-pool create**를 사용하여 백 엔드 주소 풀의 구성을 추가합니다. **azure network lb inbound-nat-rule create**를 사용하여 NAT 규칙을 추가합니다. **azure network lb rule create**를 사용하여 부하 분산 장치 규칙을 추가합니다. **azure network lb probe create**를 사용하여 프로브를 추가합니다. |
| [템플릿](../load-balancer/load-balancer-get-started-internet-arm-template.md) | 템플릿을 사용하여 부하 분산 장치를 배포하기 위한 지침으로 [부하 분산 장치의&2;개 VM 및 LB에 NAT 규칙 구성](https://github.com/Azure/azure-quickstart-templates/tree/master/201-2-vms-loadbalancer-natrules)(영문)을 사용합니다. |
...
| [Azure CLI](../load-balancer/load-balancer-get-started-ilb-arm-cli.md) | **azure network lb create** 명령을 사용하여 초기 부하 분산 장치 구성을 만듭니다. **--private-ip-address** 매개 변수와 함께 **azure network lb frontend-ip create**를 사용하여 개인 IP 주소를 정의합니다. **azure network lb address-pool create**를 사용하여 백 엔드 주소 풀의 구성을 추가합니다. **azure network lb inbound-nat-rule create**를 사용하여 NAT 규칙을 추가합니다. **azure network lb rule create**를 사용하여 부하 분산 장치 규칙을 추가합니다. **azure network lb probe create**를 사용하여 프로브를 추가합니다.|
| [템플릿](../load-balancer/load-balancer-get-started-ilb-arm-template.md) | 템플릿을 사용하여 부하 분산 장치를 배포하기 위한 지침으로 [부하 분산 장치의&2;개 VM 및 LB에 NAT 규칙 구성](https://github.com/Azure/azure-quickstart-templates/tree/master/201-2-vms-internal-load-balancer)(영문)을 사용합니다. |
...
| [템플릿](virtual-machines-windows-ps-template.md) | 템플릿을 사용하여 VM을 배포하기 위한 지침으로 [매우 간단한 Windows VM 배포](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-simple-windows)(영문)를 사용합니다. |
