...
동일한 클라우드 서비스 또는 가상 네트워크에 다른 가상 컴퓨터가 없는 경우 [Azure에서 Windows를 실행 중인 가상 컴퓨터 만들기](virtual-machines-windows-hero-tutorial.md)의 단계를 사용하여 새 가상 컴퓨터를 만들 수 있습니다. 테스트가 완료된 후에 별도의 가상 컴퓨터를 삭제합니다.
...
[Azure IaaS(Windows) 진단 패키지](https://home.diagnostics.support.microsoft.com/SelfHelp?knowledgebaseArticleFilter=2976864)를 사용하여 오류가 Azure 가상 컴퓨터 자체 때문인지 확인합니다. 이 진단 패키지로 **Azure VM에 대한 RDP 연결(다시 부팅 필요)** 문제를 해결할 수 없는 경우 [이 문서](virtual-machines-windows-reset-rdp.md)의 지침에 따라 가상 컴퓨터에서 원격 데스크톱 서비스를 다시 설정합니다. 그러면
