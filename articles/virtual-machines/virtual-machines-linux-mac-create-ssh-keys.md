...
## <a name="quick-command-listing"></a>빠른 명령 나열

다음 명령 예제에서 &lt;와 &gt; 사이의 값을 사용자 환경의 값으로 바꿉니다.  모든 SSH 키가 해당 디렉터리에서 생성되도록 `cd ~/.ssh/` 디렉터리를 변경하여 시작합니다.
...
azure_fedora_id_rsa에 대한 암호를 입력합니다.
...
이 명령은 2048비트 RSA를 사용하여 암호 보안된(암호화된) SSH 키 쌍을 만들고 쉽게 식별할 수 있도록 주석 처리됩니다.  모든 SSH 키가 해당 디렉터리에서 생성되도록 `cd ~/.ssh/` 디렉터리를 변경하여 시작합니다.
...
## <a name="walkthrough-of-ssh-keygen"></a>ssh-keygen 연습

자세히 설명한 각 단계:  `~/.ssh` 디렉터리로 변경하여 시작하고 `ssh-keygen`을 실행합니다.
...
### <a name="example-`~/.ssh/config`-file:"></a>예제 `~/.ssh/config` 파일:
