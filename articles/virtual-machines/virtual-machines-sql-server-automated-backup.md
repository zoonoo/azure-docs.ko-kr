<properties
	...
	description="리소스 관리자 배포 모델을 사용하여 Azure 가상 컴퓨터에서 실행 중인 SQL Server에 대한 자동화된 백업 기능에 대해 설명합니다."
...
다음 표에서는 자동화된 백업에 대해 구성할 수 있는 옵션을 설명합니다. 실제 구성 단계는 Azure Preview 포털 또는 Azure Windows PowerShell 명령 사용 여부에 따라 달라집니다.
...
## Azure Preview 포털에서 자동화된 백업 구성

Azure Preview 포털을 사용하여 새 SQL Server 2014 가상 컴퓨터를 만들 때 자동화된 백업을 구성할 수 있습니다.
...
다음 Azure Preview 포털 스크린샷에는 **선택적 구성** | **SQL 자동화된 백업** 아래에 이러한 옵션이 나와 있습니다.

![Azure Preview 포털에서 SQL 자동 백업 구성](./media/virtual-machines-sql-server-automated-backup/IC778483.jpg)
...
![Azure Preview 포털에서 자동 백업 구성](./media/virtual-machines-sql-server-automated-backup/IC792133.jpg)

>[AZURE.NOTE]처음으로 자동화된 백업을 사용 설정하면 Azure에서 백그라운드로 SQL Server IaaS 에이전트를 구성합니다. 이 시간 동안에는 Azure Preview 포털에 자동화된 백업이 구성되었다고 표시되지 않습니다. 에이전트가 설치 및 구성될 때까지 몇 분 정도 기다리세요. 그 후 Azure Preview 포털에 새로운 설정이 반영됩니다.
