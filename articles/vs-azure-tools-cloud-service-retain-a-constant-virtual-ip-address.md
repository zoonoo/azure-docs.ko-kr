...
---
# <a name="how-to-retain-a-constant-virtual-ip-address-for-an-azure-cloud-service"></a>Azure 클라우드 서비스에 대한 일정한 가상 IP 주소를 유지하는 방법
...
1. **Azure 응용 프로그램 게시** 대화 상자에서 배포할 구독을 선택하고 필요한 경우 로그인한 후 **다음**을 선택합니다.

    ![Azure 응용 프로그램 게시: 로그인](./media/vs-azure-tools-cloud-service-retain-a-constant-virtual-ip-address/azure-publish-signin.png)

1. **일반 설정** 탭에서 배포 중인 클라우드 서비스 이름을 확인하고 **환경**, **빌드 구성** 및 **서비스 구성**이 모두 올바른지 확인합니다.

    ![Azure 응용 프로그램 게시: 일반 설정](./media/vs-azure-tools-cloud-service-retain-a-constant-virtual-ip-address/azure-publish-common-settings.png)
...
1. **고급 설정** 탭에서 저장소 계정과 배포 레이블이 올바른지 확인하고 **실패 시 배포 삭제** 확인란이 선택 취소되었고 **배포 업데이트** 확인란이 선택되었는지 확인합니다. **배포 업데이트** 확인란을 선택하면 응용 프로그램을 다시 게시할 때 배포가 삭제되지 않고 VIP가 손실되지 않았는지 확인할 수 있습니다. **실패 시 배포 삭제**확인란을 선택 취소하면 배포 중에 오류가 발생할 경우 VIP가 손실되지 않았는지 확인할 수 있습니다.
    ...
    ![Azure 응용 프로그램 게시 - 고급 설정](./media/vs-azure-tools-cloud-service-retain-a-constant-virtual-ip-address/azure-publish-advanced-settings.png)

1. (선택 사항) 역할을 업데이트하는 방법을 자세히 지정하려면 **배포 업데이트** 옆의 **설정**을 선택합니다. **증분 업데이트** 또는 **동시 업데이트**를 선택합니다. **증분 업데이트**를 선택하면 응용 프로그램의 각 인스턴스가 하나씩 차례로 업데이트되므로 해당 응용 프로그램을 항상 사용할 수 있습니다. **동시 업데이트**를 선택하면 응용 프로그램의 모든 인스턴스가 동시에 업데이트됩니다. 동시 업데이트는 신속하게 진행되지만 업데이트 과정 중에 서비스를 사용할 수 없습니다. 완료되면 **다음**을 선택합니다.

    ![Azure 응용 프로그램 게시 - 배포 업데이트 설정](./media/vs-azure-tools-cloud-service-retain-a-constant-virtual-ip-address/azure-publish-deployment-update-settings.png)

1. **Azure 응용 프로그램 게시** 대화 상자로 돌아가면 **요약** 페이지가 표시될 때까지 **다음**을 선택합니다. 설정을 확인한 다음 **게시**를 선택합니다.
   
    ![Azure 응용 프로그램 게시 - 요약](./media/vs-azure-tools-cloud-service-retain-a-constant-virtual-ip-address/azure-publish-summary.png)
