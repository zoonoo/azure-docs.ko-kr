...
- 앱 서비스 계획과 해당 리소스 그룹에 있는 모든 기타 앱 서비스 리소스를 이미 앱 서비스 리소스가 없는 새 리소스 그룹으로 이동합니다. 즉, 앱 서비스 계획에 연결되지 않은 앱 서비스 리소스를 이동하는 것도 의미합니다. 
- 앱을 다른 리소스 그룹으로 이동하지만 모든 앱 서비스는 원래 리소스 그룹에 유지합니다.

원래 리소스 그룹에 Application Insights 리소스도 포함하는 경우 Application Insights가 현재 이동 작업을 지원하지 않으므로 해당 리소스를 이동할 수 없습니다. 앱 서비스 앱을 이동할 때 Application Insights 리소스를 포함 하는 경우 전체 이동 작업이 실패 합니다. 하지만, Application Insights와 앱 서비스 계획은 제대로 기능하기 위해 해당 앱에 대해 앱과 같은 리소스 그룹에 상주하지 않아도 됩니다.

예를 들어 리소스 그룹에 다음 사항이 포함된 경우:

- **계획-a** 및 **app-insights-a**와 연결된 **웹-a**
- **계획-b** 및 **app-insights-b**와 연결된 **웹-b**

옵션은 다음과 같습니다.

- **웹-a**, **계획-a**, **웹-b** 및 **계획-b** 이동
- **웹-a** 및 **웹-b** 이동
- **웹-a** 이동
- **웹-b** 이동
...
- **웹-a**가 **웹-그룹**에 상주하는 경우
- **계획-a**가 **계획-그룹**에 상주하는 경우
- **웹-a** 및 **계획-a**가 **통합-그룹**에 상주하길 원하는 경우

이 이동을 수행하려면 다음 순서로 두 개의 별도 이동 작업을 수행하세요.

1. **웹-a**를 **계획-그룹**으로 이동
2. **웹-a** 및 **계획-a**를 **통합-그룹**으로 이동
...
- 이동은 포털 또는 클래식 이동에 대한 별도의 REST API를 통해서만 요청할 수 있습니다. 클래식 리소스를 새 구독으로 이동할 경우 표준 Resource Manager 이동 명령은 작동하지 않습니다. 포털 또는 REST API를 사용하는 단계는 아래 섹션에 표시되어 있습니다.

## 포털을 사용하여 리소스 이동

포털을 통해 일부 리소스를 이동할 수 있습니다. 그러나 이동 작업을 지원하는 모든 리소스 공급자가 포털을 통한 이동 기능을 제공하는 것은 아닙니다.

리소스를 이동하려면 리소스를 선택하고 **이동** 단추를 선택합니다.

![리소스 이동](./media/resource-group-move-resources/move-resources.png)

리소스 이동 시 대상 구독 및 리소스 그룹을 지정합니다. 리소스와 함께 이동해야 하는 다른 리소스가 있는 경우 나열됩니다.
...
리소스를 새 리소스 그룹(구독 제외)으로 이동하는 옵션의 경우 이동하려는 리소스를 선택하세요.

![이동할 리소스 선택](./media/resource-group-move-resources/select-resource.png)

**속성**을 선택합니다.

![속성 선택](./media/resource-group-move-resources/select-properties.png)

이 리소스 형식에 사용할 수 있는 경우 **리소스 그룹 변경**을 선택합니다.

![리소스 그룹 변경](./media/resource-group-move-resources/change-resource-group.png)

이동할 리소스와 어떤 리소스 그룹으로 이동할지 선택할 수 있습니다.

![리소스 이동](./media/resource-group-move-resources/select-group.png)

클래식 모델을 통해 배포된 리소스를 새 리소스 그룹으로 이동하는 경우 리소스 그룹 옆의 편집 아이콘을 사용할 수 있습니다.

![클래식 리소스 이동](./media/resource-group-move-resources/edit-rg-icon.png)

[클래식 배포 제한 사항](#classic-deployment-limitations)을 염두에 두고 이동할 리소스를 선택합니다. **확인**을 선택하여 이동을 시작합니다.

 ![클래식 리소스 선택](./media/resource-group-move-resources/select-classic-resources.png)
 
 클래식 모델을 통해 배포된 리소스를 새 구독으로 동하는 경우 구독 의 편집 아이콘을 사용할 수 있습니다.
 
 ![새 구독으로 이동](./media/resource-group-move-resources/edit-subscription-icon.png)
 
 이동에 모든 클래식 리소스가 자동으로 선택됩니다.
...
하지만, **새 구독으로 클래식 리소스**를 이동하려면 다른 REST 작업을 사용해야 합니다. 구독을 클래식 리소스의 교차 구독 이동에서 원본 또는 대상 구독으로 참여할 수 있는지 확인하려면 다음 작업을 사용하세요.
