...
이 항목에서는 새 구독 또는 동일한 구독의 새 리소스 그룹으로 리소스를 이동하는 방법을 보여 줍니다. 새 구독을 이동할 때는 구독이 원본 구독과 동일한 [Active Directory 테넌트](active-directory/active-directory-howto-tenant.md)에 있어야 합니다. 포털, PowerShell, Azure CLI 또는 REST API를 사용하여 리소스를 이동할 수 있습니다. 이 항목의 이동 작업은 Azure 지원의 도움 없이도 사용 가능합니다.
...
1. 서비스는 리소스 이동 기능을 사용하도록 설정해야 합니다. [리소스 이동을 사용하는 서비스](#services-that-enable-move)에 대한 자세한 내용은 아래 목록을 참조하세요.
...
3. 앱 서비스 앱을 이동하는 경우 [앱 서비스 제한](#app-service-limitations)을 먼저 검토해야 합니다.
4. Recovery Services와 연결된 리소스를 이동하는 경우 [Recovery Services 제한 사항](#recovery-services-limitations)을 먼저 검토해야 합니다.
...
현재, 웹앱에 외부에서 구매하여 앱에 업로드한 SSL 인증서가 포함된 경우 웹앱을 이동하기 전에 인증서를 삭제해야 합니다. 예를 들어, 다음 단계를 수행해야 합니다.

1. 웹앱에서 업로드한 인증서 삭제
2. 웹앱 이동
3. 웹앱에 인증서 업로드

웹앱에서 App Service Certificate를 사용하는 경우 웹앱 및 인증서를 새 리소스 그룹으로 문제 없이 이동할 수 있습니다. App Service Certificate를 새 구독으로 이동하려면 Azure 지원에 문의하세요.
...
다른 리소스 그룹 또는 구독에 기존 리소스를 이동하려면 **azure resource move** 명령을 사용합니다. 이동할 리소스에 대한 리소스 ID를 제공해야 합니다. 다음 명령을 사용하여 리소스를 가져올 수 있습니다.
