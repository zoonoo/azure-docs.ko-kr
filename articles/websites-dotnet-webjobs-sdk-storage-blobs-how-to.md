<properties 
	pageTitle="WebJobs SDK를 사용하여 Azure Blob 저장소로 작업하는 방법" 
...
-   [Blob가 만들어지거나 업데이트될 때 함수를 트리거하는 방법](#trigger)
	- 확장명을 포함하는 Blob 이름에 대한 단일 자리 표시자
	- Blob 이름과 확장명에 대한 별도의 자리 표시자
-   [BlobTrigger가 작동하는 유형](#types)
-   [문자열에 바인딩하여 텍스트 Blob 콘텐츠 가져오기](#string)
-   [ICloudBlobStreamBinder를 사용하여 serialize된 Blob 콘텐츠 가져오기](#icbsb)
-   [포이즌 Blob를 처리하는 방법](#poison)
-   [Blob 폴링 알고리즘](#polling)
-   [Blob 수신 확인](#receipts)
-   [큐 문서에서 다루는 관련 항목](#queues)
-   [다음 단계](#nextsteps)
...
* `CloudBlockBlob`
* `CloudPageBlob`
* [ICloudBlobStreamBinder]에 의해 deserialize된 다른 유형(#icbsb) 
...
최대 다시 시도 횟수는 구성 가능합니다. 동일한 [MaxDequeueCount](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#configqueue) 설정이 포이즌 Blob 처리와 포이즌 큐 메시지 처리에 사용됩니다. 
...
* Blob에 대해 호출된 함수("*{WebJob name}*.Functions.*{Function name}*", 예: "WebJob1.Functions.CopyBlob")
* 컨테이너 이름
