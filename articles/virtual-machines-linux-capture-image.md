<properties urlDisplayName="Capture an image" pageTitle="Linux를 실행하는 가상 컴퓨터의 이미지 캡처" metaKeywords="Azure Linux vm, Linux vm" description="Linux를 실행하는 Azure VM(가상 컴퓨터)의 이미지를 캡처하는 방법에 대해 알아봅니다. " metaCanonical="" services="virtual-machines" documentationCenter="" title="Linux를 실행하는 가상 컴퓨터의 이미지를 캡처하는 방법" authors="kathydav" solutions="" manager="timlt" editor="tysonn" />
...
1.  명령 모음에서 **연결**을 클릭하여 가상 컴퓨터에 연결합니다. 자세한 내용은 [Linux를 실행하는 가상 컴퓨터에 로그온하는 방법][Linux를 실행하는 가상 컴퓨터에 로그온하는 방법]을 참조하세요.
