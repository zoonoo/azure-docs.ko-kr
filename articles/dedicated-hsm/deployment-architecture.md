...
각 지역에는 두 개의 독립적인 데이터 센터나 두 개 이상의 독립 된 가용성 영역에 배포 된 HSM 랙이 있습니다. 예를 들어 남부 동아시아는 세 개의 가용성 영역을 포함 하 고 미국 동부 2는 2 개 있습니다. 유럽, 아시아 및 미국에서 전용 HSM 서비스를 제공 하는 총 8 개 지역이 있으며 새 지역에 새 HSM 랙이 추가 됨에 따라 변경 됩니다. Azure 지역에 대한 자세한 내용은 공식 [Azure 지역 정보](https://azure.microsoft.com/global-infrastructure/regions/)를 참조하세요.
...
배포 아키텍처가 결정 되 면 해당 아키텍처를 구현 하는 대부분의 구성 작업은 Thales에서 제공 됩니다. 여기에는 디바이스 구성뿐 아니라 애플리케이션 통합 시나리오가 포함됩니다. 자세한 내용은 [Thales 고객 지원](https://supportportal.gemalto.com/csm/) 포털을 사용 하 여 관리 및 구성 가이드를 다운로드 하세요. Microsoft 파트너 사이트에는 다양한 통합 가이드가 있습니다.
