...
description: 기본 정보, 상호 운용성, 고가용성 및 지원과 같은 Azure 전용 하드웨어 보안 모듈에 대한 일반적인 질문과 답변을 찾아볼 수 있습니다.
...
### <a name="q-what-is-the-azure-dedicated-hsm-offering"></a>Q: Azure 전용 HSM 제품이란 무엇인가요?

Azure Dedicated HSM은 고객의 가상 네트워크에 직접 연결된 Azure 데이터 센터에서 호스팅되는 HSM을 제공하는 클라우드 기반 서비스입니다. 이러한 HSM은 전용 [Thales Luna 7 HSM](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms) 네트워크 어플라이언스입니다. 고객의 개인 IP 주소 공간에 직접 배포되며, Microsoft는 HSM의 암호화 기능에 액세스할 수 없습니다. 이러한 디바이스에 대한 완전한 관리 및 암호화 제어 권한은 고객에게만 있습니다. 고객은 디바이스 관리에 대한 책임이 있으며, 디바이스에서 전체 활동 로그를 직접 가져올 수 있습니다. Dedicated HSM을 사용하면 고객이 FIPS 140-2 수준 3, HIPAA, PCI-DSS, eIDAS 및 기타 등등의 규정 준수/규제 요구 사항을 충족할 수 있습니다.

### <a name="q-what-hardware-is-used-for-dedicated-hsm"></a>Q: 전용 HSM에 사용되는 하드웨어는 무엇인가요?

Microsoft는 Azure 전용 HSM 서비스 제공을 위해 Thales와 제휴 관계를 맺고 있습니다. 사용되는 특정 디바이스는 [Thales Luna 7 HSM 모델 A790](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms)입니다. 이 디바이스는 [FIPS 140-2 수준 3](https://csrc.nist.gov/publications/detail/fips/140/2/final) 검증 펌웨어를 제공할 뿐만 아니라 10개의 파티션을 통해 짧은 대기 시간, 고성능 및 대용량도 제공합니다. 
...
일단 Microsoft에서 프로비전하면 Thales는 HSM 디바이스용 소프트웨어를 모두 제공합니다. 이 소프트웨어는 [Thales 고객 지원 포털](https://supportportal.thalesgroup.com/csm)에서 사용할 수 있습니다. 전용 HSM 서비스를 사용하는 고객은 Thales 지원에 등록해야 하며, 관련 소프트웨어에 대한 액세스 및 다운로드를 가능하게 하는 고객 ID를 가지고 있어야 합니다. 지원되는 클라이언트 소프트웨어는 FIPS 140-2 수준 3 검증 펌웨어 버전 7.0.3과 호환되는 버전 7.2입니다. 

### <a name="q-what-extra-costs-may-be-incurred-with-dedicated-hsm-service"></a>Q: 전용 HSM 서비스로 발생할 수 있는 추가 비용은 무엇인가요?

전용 HSM 서비스를 사용할 때 추가 비용이 발생하는 항목은 다음과 같습니다. 
* 전용 HSM 서비스에 전용 온-프레미스 백업 디바이스를 사용하는 것이 가능하지만, 이렇게 하면 추가 비용이 발생하며 Thales에서 직접 구입해야 합니다.
* 전용 HSM은 10개 파티션 라이선스로 제공됩니다. 고객에게 추가 파티션이 필요하면 Thales에서 직접 구입하는 추가 라이선스에 대한 추가 비용이 발생합니다.
* 전용 HSM을 사용하려면 디바이스 구성을 위한 가상 머신과 같은 리소스 및 네트워킹 인프라(VNET, VPN 게이트웨이 등)가 필요합니다. 이러한 추가 리소스는 추가 비용을 발생시키며, 전용 HSM 서비스 가격 책정에 포함되지 않습니다.

### <a name="q-does-azure-dedicated-hsm-offer-password-based-and-ped-based-authentication"></a>Q: Azure 전용 HSM에서 암호 기반 및 PED 기반 인증이 제공되나요?
...
### <a name="q-does-azure-dedicated-hsm-support-payment-pineft-features"></a>Q: Azure 전용 HSM에서 결제(PIN/EFT) 기능이 지원되나요?
...
### <a name="q-which-azure-regions-is-dedicated-hsm-available-in"></a>Q: 전용 HSM을 사용할 수 있는 Azure 지역은 어디인가요?

2019년 3월 기준으로 전용 HSM은 아래 나열된 14개 지역에서 제공됩니다. 추가 지역이 예정되어 있으며, 자세한 정보가 필요하면 Microsoft 계정 담당자에게 문의하시기 바랍니다.
...
Thales에서 제공한 HSM 클라이언트 도구/SDK/소프트웨어를 사용하여 애플리케이션에서 암호화 작업을 수행합니다. 이 소프트웨어는 [Thales 고객 지원 포털](https://supportportal.thalesgroup.com/csm)에서 사용할 수 있습니다. 전용 HSM 서비스를 사용하는 고객은 Thales 지원에 등록해야 하며, 관련 소프트웨어에 대한 액세스 및 다운로드를 가능하게 하는 고객 ID를 가지고 있어야 합니다.
...
예, 온-프레미스 Thales Luna 7 HSM이 있으면 가능합니다. 여러 가지 방법이 있습니다. [Thales HSM 설명서](https://thalesdocs.com/gphsm/luna/7.2/docs/network/Content/Home_network.htm)를 참조하세요.
...
Azure 전용 HSM은 [Thales Luna 7 HSM 모델 A790](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms) 디바이스를 사용하며 암호 기반 인증을 지원합니다.
...
* 키리스 TLS용 애플리케이션 게이트웨이 또는 트래픽 관리자
...
### <a name="q-does-the-customer-get-full-exclusive-control-over-the-hsms-with-dedicated-"></a>Q: 고객이 전용 HSM을 통해 HSM을 독점적으로 완벽하게 제어하나요?
...
### <a name="q-what-is-the-tenant-admin-account-microsoft-uses-i-am-used-to-the-admin-user-being-admin-on-thales-luna-hsms"></a>Q: Microsoft에서 사용되는 “테넌트 관리자” 계정은 무엇인가요? Thales Luna HSM에서 사용하던 “관리자” 계정과 어떻게 다른가요?

HSM 디바이스는 일반적인 기본 암호가 포함된 관리자라는 기본 사용자와 함께 제공됩니다. Microsoft는 고객이 프로비전하도록 디바이스가 풀에 대기 중인 상태에서 기본 암호를 사용하는 것을 선호하지 않았습니다. 이것은 Microsoft의 엄격한 보안 요구 사항을 위배하는 것입니다. 이러한 이유로 Microsoft는 프로비저닝을 수행할 때 폐기되는 강력한 암호를 설정했습니다. 또한 프로비전할 때 “테넌트 관리자”라는 관리자 역할의 새 사용자를 만듭니다. 이 사용자는 기본 암호를 갖습니다. 새로 프로비전된 디바이스에 처음 로그인할 때 고객은 첫 번째 작업으로 이 암호를 변경해야 합니다. 이러한 프로세스는 높은 보안 수준을 보장하며 고객에 대한 단독 관리 제어라는 약속을 지킬 수 있게 해줍니다. “테넌트 관리자” 사용자는 고객이 해당 계정을 사용하길 원하는 경우 관리자 사용자의 암호를 재설정하기 위해 사용될 수 있습니다. 
...
다른 펌웨어 버전의 특정 기능이 필요한 경우 고객은 소프트웨어/펌웨어 업그레이드를 포함한 모든 관리 권한을 갖습니다. 항목을 변경하기 전 HSMRequest@microsoft.com에 연락하여 업그레이드 관련 사항을 Microsoft에 문의하세요.  
...
### <a name="q-is-it-possible-to-configure-high-availability-in-the-same-region-or-across-multiple-regions"></a>Q: 동일한 지역에 또는 여러 지역 간에 고가용성을 구성하는 것이 가능한가요?

예. 고가용성 구성 및 설정은 Thales에서 제공되는 HSM 클라이언트 소프트웨어에서 수행됩니다. 동일한 지역 또는 지역 간에 있는 동일하거나 다른 VNET의 HSM 또는 사이트 간 VPN이나 지점 간 VPN을 사용하여 VNET에 연결된 온-프레미스 HSM은 동일한 고가용성 구성에 추가할 수 있습니다. 이렇게 하면 역할과 같은 특정 구성 항목이 아니라 주요 자료만 동기화됩니다.

### <a name="q-can-i-add-hsms-from-my-on-premises-network-to-a-high-availability-group-with-azure-dedicated-hsm"></a>Q: Azure 전용 HSM을 사용하여 온-프레미스 네트워크의 HSM을 고가용성 그룹에 추가할 수 있나요?

예. [Thales Luna 7 HSM](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms)에 대한 고가용성 요구 사항을 충족해야 합니다.

### <a name="q-can-i-add-luna-56-hsms-from-on-premises-networks-to-a-high-availability-group-with-azure-dedicated-hsm"></a>Q: Azure 전용 HSM을 사용하여 온-프레미스 네트워크의 Luna 5/6 HSM을 고가용성 그룹에 추가할 수 있나요?
...
전용 HSM 서비스에는 [Thales Luna 7 HSM](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms) 어플라이언스가 사용됩니다. 이러한 디바이스는 물리적 및 논리적 변조 탐지를 지원합니다. 변조 이벤트가 발생하면 HSM이 자동으로 초기화됩니다.
...
### <a name="q-how-do-i-get-the-client-software-documentation-and-access-to-integration-guidance-for-the-thales-luna-7-hsm"></a>Q: Thales Luna 7 HSM에 대한 클라이언트 소프트웨어, 설명서 및 통합 가이드 액세스는 어떻게 얻을 수 있나요?

서비스를 등록하면 Thales 고객 지원 포털 등록에 사용할 수 있는 Thales 고객 ID가 제공됩니다. 이를 통해 모든 소프트웨어 및 설명서에 액세스할 수 있고 Thales에 직접 지원을 요청할 수도 있습니다.

### <a name="q-if-there-is-a-security-vulnerability-found-and-a-patch-is-released-by-thales-who-is-responsible-for-upgradingpatching-osfirmware"></a>Q: 보안 취약성이 있고 Thales에서 패치를 릴리스한 경우 OS/펌웨어 업그레이드/패치 작업은 누가 수행해야 하나요?

Microsoft는 고객에게 할당된 HSM에 연결할 수 없습니다. 따라서 고객이 HSM을 업그레이드하고 패치해야 합니다.

### <a name="q-what-if-i-need-to-reboot-my-hsm"></a>Q: HSM을 다시 부팅해야 하면 어떻게 해야 하나요?
...
예, 전용 HSM은 [FIPS 140-2 수준 3](https://csrc.nist.gov/publications/detail/fips/140/2/final) 유효성 검사가 완료된 Thales Luna 7 HSM을 프로비전합니다. 
...
예. 전용 HSM 서비스는 [FIPS 140-2 수준 3](https://csrc.nist.gov/publications/detail/fips/140/2/final) 유효성 검사가 완료된 [Thales Luna 7 HSM 모델 A790](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms) 어플라이언스를 프로비전합니다.
...
### <a name="q-how-does-a-customer-ensure-that-when-an-hsm-is-deprovisioned-all-the-key-material-is-wiped-out"></a>Q: 고객이 HSM을 프로비저닝 해제할 때 모든 키 자료가 초기화되도록 하려면 어떻게 해야 하나요?
...
사용된 [Thales Luna 7 HSM 모델 A790](https://cpl.thalesgroup.com/encryption/hardware-security-modules/network-hsms)에는 서비스 비용으로 10개 파티션에 대한 라이선스가 포함됩니다. 디바이스에는 100개 파티션 제한이 있으며, 이 제한까지 파티션을 추가하면 추가 라이선스 비용이 발생할 수 있으며, 디바이스에 새 라이선스 파일을 설치해야 합니다.

### <a name="q-how-many-keys-can-be-supported-in-dedicated-hsm"></a>Q: Dedicated HSM에서 지원할 수 있는 키의 수는 어떻게 되나요?

최대 키 수는 사용 가능한 메모리에 따라 달라집니다. 사용 중인 Thales Luna 7 모델 A790에는 32MB 메모리가 포함됩니다. 비대칭 키를 사용하는 경우 키 쌍에 다음 숫자를 적용할 수도 있습니다.
