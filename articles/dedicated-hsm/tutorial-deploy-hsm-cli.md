...
또한 위의 명령에서 DNS 이름 대신 VM의 IP 주소를 사용할 수도 있습니다. 이 명령이 성공하면 경우 암호를 묻는 메시지가 표시되고, 암호를 입력해야 합니다. 가상 머신에 로그온되면 포털에서 HSM과 연결된 네트워크 인터페이스 리소스에 대해 확인된 사설 IP 주소를 사용하여 HSM에 로그인할 수 있습니다.
...
위의 스크린샷에서 "HSM1_HSMnic" 또는 "HSM2_HSMnic"를 클릭하면 적절한 사설 IP 주소가 표시됩니다. 그렇지 않을 경우 위에서 사용된 `az resource show` 명령으로 올바른 IP 주소를 확인할 수 있습니다. 
