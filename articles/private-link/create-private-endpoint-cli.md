...
SQL Server ID는 다음 단계에서 SQL Server ID를 사용할 ```/subscriptions/subscriptionId/resourceGroups/myResourceGroup/providers/Microsoft.Sql/servers/myserver.```와 비슷합니다. 
...
이 섹션에서는 개인 끝점을 사용 하 여 VM에서 SQL Database 서버에 연결 합니다.
 ...
 2. Nslookup myserver.database.windows.net  입력 하면 다음과 유사한 메시지가 표시 됩니다. 
