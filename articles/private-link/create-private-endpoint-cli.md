...
az sql server create 명령을 사용하여 SQL Database Server를 만듭니다. SQL Server의 이름은 Azure 전체에서 고유해야 하므로 괄호 안의 자리 표시자 값을 사용자 고유의 값으로 바꿉니다. 
...
SQL Server ID는  ```/subscriptions/subscriptionId/resourceGroups/myResourceGroup/providers/Microsoft.Sql/servers/myserver.```와 비슷합니다. 이 SQL Server ID는 다음 단계에서 사용합니다. 

## <a name="create-the-private-endpoint"></a>프라이빗 엔드포인트 만들기 
Virtual Network에서 SQL Database 서버에 대한 프라이빗 엔드포인트를 만듭니다. 
```azurecli-interactive
...
SQL Database 서버 도메인에 대한 프라이빗 DNS 영역을 만들고, Virtual Network와의 연결 링크를 만듭니다. 
```azurecli-interactive
...
이 섹션에서는 프라이빗 엔드포인트를 사용하여 VM에서 SQL Database Server에 연결합니다.

 1. *myVM*의 원격 데스크톱에서 PowerShell을 엽니다.
 2. nslookup myserver.database.windows.net을 입력합니다.  다음과 비슷한 메시지가 표시됩니다. 
...
```
 ...
 4. [서버에 연결]에서 다음 정보를 입력하거나 선택합니다. 서버 유형: [데이터베이스 엔진]을 선택합니다.
 서버 이름: myserver.database.windows.net을 선택합니다. 사용자 이름: 만드는 중에 제공된 사용자 이름을 입력합니다.
