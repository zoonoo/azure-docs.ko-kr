<properties pageTitle="Azure 미디어 서비스 REST API 시작하기" description="이 자습서에서는 Azure 미디어 서비스(AMS) REST API를 사용하여 주문형 비디오(VoD) 콘텐츠 제공 응용 프로그램을 구현하는 단계를 안내합니다." services="media-services" documentationCenter="" authors="Juliako" manager="dwrede" />
...
# 주문형 비디오는 미디어 서비스 REST API와 함께 제공됩니다. 

[AZURE.INCLUDE [media-services-selector-get-started](../includes/media-services-selector-get-started.md)]


>[AZURE.NOTE]
> 이 자습서를 완료하려면 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 <a href="http://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A8A8397B5" target="_blank">Azure 무료 평가판</a>.

이 자습서에서는 Azure 미디어 서비스(AMS) REST API를 사용하여 주문형 비디오(VoD) 콘텐츠 제공 응용 프로그램을 구현하는 단계를 안내합니다. 
...
이 자습서에서는 Azure 관리 포털 및 미디어 서비스 REST API를 사용하여 사용하여 다음 작업을 수행합니다.     


1.  [포털을 사용하여 미디어 서비스 계정 만들기](#create_ams).
1.  [REST API를 통해 미디어 서비스 계정에 연결](#connect).
1.  [새 자산을 만들고 REST API를 통해 비디오 파일 업로드](#upload).
1.  [REST API를 통해 스트리밍 단위 구성](#configure_streaming_units).
2.  [REST API를 통해 적응 비트 전송률 MP4 파일 집합으로 소스 파일 인코딩](#encode).
1.  [REST API를 통해 인코딩된 자산 제공 정책 구성](#configure_delivery_method).
1.  [REST API를 통해 자산을 게시하고 스트리밍 및 점진적으로 URL 다운로드](#publish_get_urls). 
1.  [콘텐츠 재생](#play). 

## 필수 조건
미디어 서비스 REST API를 사용하여 개발을 시작하려면 다음 필수 조건이 필요합니다.

- 미디어 서비스 REST API를 사용하여 개발하는 방법을 이해합니다. 자세한 내용은 [미디어 서비스 REST 개요](http://msdn.microsoft.com/library/azure/hh973616.aspx)를 참조하세요.
...
1. [관리 포털][]에서 **새로 만들기**를 클릭하고 **미디어 서비스**를 클릭한 다음 **빠른 생성**을 클릭합니다.
...
Azure 미디어 서비스에 액세스하려면 두 가지가 필요합니다. Azure 액세스 제어 서비스(ACS)에서 제공하는 액세스 토큰과 미디어 서비스 자체 URI입니다. 이러한 요청을 만들 때는 올바른 헤더 값을 지정하고 미디어 서비스를 호출할 때 액세스 토큰을 올바르게 전달하기만 하면 원하는 방법을 사용할 수 있습니다.
...
2. 미디어 서비스 URI에 연결합니다. 
	...
	>[AZURE.NOTE]
	https://media.windows.net에 연결하면 다른 미디어 서비스 URI를 지정하는 301 리디렉션을 받게 됩니다. 새 URI에 대한 후속 호출을 확인해야 합니다.
	> 
	> ODATA API 메타데이터 설명을 포함하는 HTTP/1.1 200 응답을 받을 수도 있습니다.
3. 새 URL에 후속 API 호출을 게시합니다. 
	
	예를 들어, 연결을 시도하면 다음을 받습니다.
...
다음 예제에서는 응답 본문에 액세스 토큰을 포함하는 HTTP 응답을 보여줍니다.
...
미디어 서비스의 루트 URI는 https://media.windows.net/입니다. 처음에는 이 URI에 연결해야 하고 응답에서 301 리디렉션을 받게 되면 새 URI에 후속 호출을 확인해야 합니다. 또한 요청에서 자동 리디렉션/팔로우 논리를 사용하지 마십시오. HTTP 동사와 요청 본문은 새 URI로 전달되지 않습니다.
...
## <a id="upload"></a>새 자산을 만들고 REST API를 통해 비디오 파일 업로드
...
**HTTP 요청**
	...
	Authorization: Bearer http%3a%2f%2fschemas.xmlsoap.org%2fws%2f2005%2f05%2fidentity%2fclaims%2fnameidentifier=amstestaccount001&urn%3aSubscriptionId=f7f09258-6753-4ca2-b1ae-193798e2c9d8&http%3a%2f%2fschemas.microsoft.com%2faccesscontrolservice%2f2010%2f07%2fclaims%2fidentityprovider=https%3a%2f%2fwamsprodglobal001acs.accesscontrol.windows.net%2f&Audience=urn%3aWindowsAzureMediaServices&ExpiresOn=1421662918&Issuer=https%3a%2f%2fwamsprodglobal001acs.accesscontrol.windows.net%2f&HMACSHA256=utmoXXbm9Q7j4tW1yJuMVA3egRiQy5FPygwadkmPeaY%3d
	x-ms-version: 2.8
	호스트: wamsbayclus001rest-hs.cloudapp.net
...
- 콘텐츠를 배달하는 출발점이 될 스트리밍 끝점에 하나 이상의 주문형 스트리밍 단위를 구성합니다(이 섹션에서 설명).
...
새 주문형 스트리밍 단위를 할당하는 작업은 완료하는 데 20분 정도 걸립니다. 작업 상태를 확인하려면 **작업** 메서드를 사용하여 작업 Id를 지정합니다. 응답에서 작업 Id는 **조정** 요청으로 반환됩니다.
...
각 작업을 수행하려는 처리 유형에 따라 하나 이상의 작업을 가질 수 있습니다. REST API를 통해 다음 두 가지 방법 중 하나로 작업 및 관련된 작업을 만들 수 있습니다. 작업은 작업 엔터티의 작업 탐색 속성을 통해 떠는 OData 일괄 처리를 통해 인라인으로 정의할 수 있습니다. 미디어 서비스 SDK 는 일괄 처리를 사용하지만 이 항목에 있는 코드 예제 가독성의 경우 작업은 인라인으로 정의됩니다. 일괄 처리에 대한 정보는 [Open Data Protocol(OData) 일괄 처리](http://www.odata.org/documentation/odata-version-3-0/batch-processing/)를 참조하십시오.

다음 예제에서는 특정 해상도와 품질로 비디오를 인코딩하기 위해 하나의 작업 집합으로 작업을 만들어 게시하는 방법을 보여줍니다. 다음 설명서 섹션은 Azure 미디어 프로세서에서 지원하는 모든 [작업 미리 설정](http://msdn.microsoft.com/library/azure/dn619392.aspx) 목록을 포함합니다.  
...
- TaskBody 정의에서 <outputAsset>에 대한 각 내부 값은 <inputAsset> JobInputAsset(값) 또는 JobOutputAsset(값)으로 설정해야 합니다.
...
## <a id="publish_get_urls"></a>REST API를 통해 자산을 게시하고 스트리밍 및 점진적으로 URL 다운로드
...
미디어 콘텐츠를 다운로드하거나 스트리밍하기 전에 먼저 읽기 권한이 포함된 AccessPolicy를 정의하고 클라이언트에 대해 사용하도록 설정하려는 배달 메커니즘 유형을 지정하는 적절한 로케이터 엔터티를 만듭니다. 사용할 수 있는 속성에 대한 자세한 내용은 [AccessPolicy 엔터티 속성](https://msdn.microsoft.com/ko-kr/library/azure/hh974297.aspx#accesspolicy_properties)을 참조하세요.
...
>[AZURE.NOTE]
이 기본 워크플로는 자산을 통합할 때 파일을 업로드하는 것과 같습니다(이 항목의 앞부분에서 설명함). 또한 파일을 업로드하는 것과 마찬가지로 사용자(또는 클라이언트)가 파일에 즉시 액세스해야 하는 경우 StartTime 값을 현재 시간에서 5분 전으로 설정해야 합니다. 이 동작은 클라이언트와 미디어 서비스 사이에 시간차가 있을 수 있기 때문에 필요합니다. StartTime 값은 다음 DateTime 형식 이어야 합니다. YYYY-m M-DDTHH:mm:ssZ (예: "2014-05-23T17:53:50Z").
...
### 콘텐츠 스트리밍을 위한 주문형 원본 URL 만들기


다음 코드에서는 원본 URL 로케이터를 만드는 방법을 보여줍니다.
...
HLS를 테스트하려면 iOS 또는 Safari 장치나 [3ivx hls 플레이어](http://apps.microsoft.com/windows/ko-kr/app/3ivx-hls-player/f79ce7d0-2993-4658-bc4e-83dc182a0614)를 사용합니다. 
