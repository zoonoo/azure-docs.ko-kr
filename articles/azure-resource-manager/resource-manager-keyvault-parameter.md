...
이제 템플릿을 배포하고 매개 변수 파일을 전달합니다. Azure CLI의 경우 
...
키 자격 증명 모음 비밀에 대한 리소스 ID를 동적으로 생성하려면, 비밀이 필요한 리소스를 중첩된 템플릿으로 이동해야 합니다. 마스터 템플릿에서, 중첩된 템플릿을 추가하고 동적으로 생성된 리소스 ID를 포함하는 매개 변수에 전달합니다. 중첩된 템플릿은 외부 URI를 통해 사용할 수 있어야 합니다. 이 문서의 나머지 부분에서는 저장소 계정에 위의 템플릿을 추가하고 URI를 통해 사용할 수 있다고 가정합니다.`https://<storage-name>.blob.core.windows.net/templatecontainer/sqlserver.json`
