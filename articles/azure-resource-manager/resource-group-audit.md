...
3. 여러 조건을 선택하여 표시되는 작업의 수를 제한할 수 있습니다. 예를 들어 다음 이미지는 지난 달에 특정 사용자 또는 애플리케이션이 수행한 작업을 표시하도록 변경된 **시간 간격** 및 **이벤트를 시작한 사람** 필드를 보여줍니다. 쿼리 결과를 확인하려면 **적용** 을 선택합니다.

    ![필터 옵션 설정](./media/resource-group-audit/set-filter.png)

4. 나중에 쿼리를 다시 실행해야 하는 경우 **현재 필터 고정**을 선택하고 이름을 지정합니다.
