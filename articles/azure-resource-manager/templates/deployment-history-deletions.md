...
**자동 삭제는 리소스 그룹 및 구독 배포에 대해 지원됩니다. 현재 [관리 그룹](deploy-to-management-group.md) 및 [테넌트](deploy-to-tenant.md) 배포에 대한 기록에서 배포는 자동으로 삭제되지 않습니다.**
...
:::image type="content" border="false" source="./media/deployment-history-deletions/deployment-history.png" alt-text="배포 기록 삭제의 다이어그램":::

> [!IMPORTANT]
> 리소스 그룹이 이미 800개 제한에 도달하면 다음 배포는 오류로 인해 실패합니다. 자동 삭제 프로세스가 즉시 시작됩니다. 잠시 기다리면 배포를 다시 시도할 수 있습니다.

배포 외에도 [가상 작업](./deploy-what-if.md)을 실행하거나 배포의 유효성을 검사할 때 삭제를 트리거합니다.
