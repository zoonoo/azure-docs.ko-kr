...
템플릿 내에서 dependsOn 요소를 사용하면 하나의 리소스를 하나 이상의 리소스에 종속된 것으로 정의할 수 있습니다. 값은 문자열의 JSON 배열로, 각 문자열은 리소스 이름입니다. 배열에는 [조건부로 배포](conditional-resource-deployment.md)되는 리소스가 포함 될 수 있습니다. 조건부 리소스가 배포 되지 않은 경우 Azure Resource Manager은 필요한 종속성에서 자동으로 제거 합니다.
