...
템플릿 사양은 다음과 같은 이점을 제공 합니다.

* 템플릿 사양에 표준 ARM 템플릿을 사용 합니다.
* SAS 토큰이 아닌 Azure RBAC를 통해 액세스를 관리 합니다.
* 사용자는 템플릿에 대 한 쓰기 권한이 없어도 템플릿 사양을 배포할 수 있습니다.
* 템플릿 사양을 PowerShell 스크립트 또는 DevOps 파이프라인과 같은 기존 배포 프로세스에 통합할 수 있습니다.

템플릿 사양을 사용 하면 정식 템플릿을 만들고 조직의 팀과 공유할 수 있습니다. 템플릿 사양은 배포에 Azure Resource Manager 사용할 수 있지만 올바른 권한이 없는 사용자는 액세스할 수 없기 때문에 안전 합니다. 사용자는 템플릿을 배포 하기 위한 템플릿 사양에 대 한 읽기 권한만 필요 하므로 다른 사용자가 수정할 수 없도록 템플릿을 공유할 수 있습니다.

현재 GitHub 리포지토리 또는 저장소 계정에 템플릿이 있는 경우 템플릿을 공유 하 고 사용 하려고 할 때 몇 가지 문제가 발생 합니다. 템플릿을 배포 하려면 템플릿을 공개적으로 액세스 가능 하도록 설정 하거나 SAS 토큰을 사용 하 여 액세스를 관리 해야 합니다. 이 제한을 해결 하기 위해 사용자는 원래 템플릿에서 분기 하는 로컬 복사본을 만들 수 있습니다. 템플릿 사양은 템플릿 공유를 단순화 합니다.
...
다음 형식으로 URL을 열어 템플릿 사양을 배포할 수도 있습니다.
