...
매개 변수를 사용하는 방법에 대한 예제는 [ARM 템플릿의 매개 변수](template-parameters.md)를 참조하세요.
...
변수를 사용하는 방법에 대한 예제는 [ARM 템플릿의 변수](template-variables.md)를 참조하세요.
...
사용자 지정 함수를 사용하는 방법에 대한 예제는 [ARM 템플릿의 사용자 정의 함수](template-user-defined-functions.md)를 참조하세요.
...
출력을 사용하는 방법에 대한 예제는 [ARM 템플릿의 출력](template-outputs.md)을 참조하세요.
...
인라인 주석의 경우 `//` 또는 `/* ... */` 중 하나를 사용할 수 있지만, 이 구문이 모든 도구에서 작동하지는 않습니다. 이 주석 스타일을 추가하는 경우 사용하는 도구가 반드시 인라인 JSON 주석을 지원해야 합니다.

> [!NOTE]
> 버전이 2.3.0 이하인 Azure CLI를 사용하여 주석이 있는 템플릿을 배포하려면 `--handle-extended-json-format` 스위치를 사용해야 합니다.
...
> [!NOTE]
> 버전이 2.3.0 이하인 Azure CLI를 사용하여 다중 선 줄 문자열이 있는 템플릿을 배포하려면 `--handle-extended-json-format` 스위치를 사용해야 합니다.
...
* 템플릿을 만드는 방법에 대한 권장 사항은 [Azure 템플릿 모범 사례](template-best-practices.md)를 참조하세요.
