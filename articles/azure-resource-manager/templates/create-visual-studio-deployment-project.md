...
1. Visual Studio는 웹앱에 대한 리소스 그룹 배포 프로젝트를 만듭니다. 프로젝트에 대한 파일을 보려면 배포 프로젝트의 노드를 확인합니다.
    ...
    ![노드 표시](./media/create-visual-studio-deployment-project/show-items.png)
...
1. Visual Studio 편집기는 Resource Manager 템플릿 편집에 도움이 되는 도구를 제공합니다. **JSON 개요** 창을 통해 템플릿에 정의된 요소를 쉽게 볼 수 있습니다.
   ...
   ![JSON 개요 표시](./media/create-visual-studio-deployment-project/show-json-outline.png)
...
1. 추가된 리소스 뿐만 아니라 형식 스토리지 계정에 대한 매개 변수 및 스토리지 계정 이름에 대한 변수입니다.

   ![개요 표시](./media/create-visual-studio-deployment-project/show-new-items.png)

1. 스토리지 계정의 유형에 대한 매개 변수는 허용되는 유형 및 기본 유형을 사용하여 미리 정의됩니다. 이러한 값을 유지하거나 시나리오에 대해 편집할 수 있습니다. 아무도 이 템플릿을 통해 **Premium_LRS** 스토리지 계정을 배포하지 못하게 하려면 허용된 형식에서 제거해야 합니다.
...
1. 배포 프로젝트 노드의 바로 가기 메뉴에서 새 **배포를** > **선택합니다.**
...
1. 배포된 리소스가 모두 표시됩니다. 스토리지 계정의 이름은 해당 리소스를 추가할 때 지정한 것과 일치하지 않은지 확인합니다. 스토리지 계정은 고유해야 합니다. 고유한 이름을 만들기 위해 템플릿은 자동으로 사용자가 제공한 이름에 문자의 문자열을 추가합니다.

    ![리소스 표시](./media/create-visual-studio-deployment-project/show-deployed-resources.png)

## <a name="add-code-to-project"></a>프로젝트에 코드 추가

이 시점에서 앱에 대한 인프라를 배포했지만 프로젝트와 함께 배포된 실제 코드가 없습니다.

1. Visual Studio 솔루션에 프로젝트를 추가합니다. 솔루션을 마우스 오른쪽 단추로 클릭하고**새 프로젝트** **추가를** > 선택합니다.
...
1. 배포가 완료된 후에 포털에서 웹앱을 선택합니다. URL을 선택하여 사이트를 찾습니다.
   ...
   ![사이트 찾아보기](./media/create-visual-studio-deployment-project/browse-site.png)

1. 기본 ASP.NET 앱을 성공적으로 배포했습니다.

   ![배포된 앱 표시](./media/create-visual-studio-deployment-project/show-deployed-app.png)
...
1. 상단 메뉴에서 **리소스 그룹 삭제를** 선택합니다.

## <a name="next-steps"></a>다음 단계

이 문서에서는 Visual Studio를 사용하여 템플릿을 만들고 배포하는 방법을 배웠습니다. 템플릿 개발에 대해 자세히 알아보려면 새로운 초보자용 자습서 시리즈를 참조하세요.
