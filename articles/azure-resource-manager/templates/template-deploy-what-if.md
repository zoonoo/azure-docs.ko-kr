...
PowerShell에서이를 사용 하려면 PowerShell Core (6.x 또는 4.x)가 있어야 합니다. PowerShell 5.x 이전 버전인 경우 [powershell의 버전을 업데이트](/powershell/scripting/install/installing-powershell)합니다.

PowerShell 버전이 올바른지 확인 한 후 PowerShell 갤러리에서 Az. Resources 모듈의 미리 보기 버전을 설치 합니다.
...
템플릿을 배포 하기 전에 변경 내용에 대 한 미리 보기를 보려면 `-Whatif` switch 매개 변수를 deployment 명령에 추가 합니다.
