...
템플릿에서 location 이라는 매개 변수를 포함 해야 합니다. 이 매개 변수를 사용 하 여 템플릿에서 리소스의 위치를 설정 합니다. 기본 템플릿 (azuredeploy.json 또는 on mainTemplate.js)에서이 매개 변수는 기본적으로 리소스 그룹 위치로 사용할 수 있습니다. 연결 된 템플릿이나 중첩 된 템플릿에서 location 매개 변수에는 기본 위치를 사용할 수 없습니다.

템플릿의 사용자는 제한 된 영역을 사용할 수 있습니다. 리소스 위치를 하드 코드 하는 경우 사용자는 해당 지역에서 리소스를 만들지 못하도록 차단할 수 있습니다. 리소스 위치를로 설정한 경우에도 사용자가 차단 될 수 있습니다 `"[resourceGroup().location]"` . 리소스 그룹이 다른 사용자가 액세스할 수 없는 지역에 만들어졌을 수 있습니다. 이러한 사용자는 템플릿 사용을 차단 합니다.

기본적으로 리소스 그룹 위치에 대 한 위치 매개 변수를 제공 하 여 사용자는 편리한 경우에도 다른 위치를 지정할 수 있습니다.
...
대신 리소스 그룹 위치로 기본 설정 되는 매개 변수를 만들지만 사용자가 다른 값을 제공할 수 있도록 합니다. 다음 예제에서는 템플릿이 주 템플릿으로 사용 되는 경우이 테스트를 **통과** 합니다.
...
그러나 앞의 예제를 연결 된 템플릿으로 사용 하는 경우 테스트가 **실패** 합니다. 연결 된 템플릿으로 사용 하는 경우 기본값을 제거 합니다.
