...
식 내에서 구문은 템플릿 내에서 사용 하기 위해 리소스 관리자 제공 하는 함수 중 하나를 호출 `resourceGroup()` 합니다. 이 경우 [resourceGroup](resource-group-template-functions-resource.md#resourcegroup) 함수입니다. JavaScript에서와 마찬가지로 함수 호출은 `functionName(arg1,arg2,arg3)`과 같이 형식이 지정됩니다. `.location` 구문은 해당 함수에서 반환 하는 개체에서 속성 하나를 검색 합니다.
