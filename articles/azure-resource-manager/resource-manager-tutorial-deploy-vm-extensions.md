...
[Azure 가상 머신 확장](../virtual-machines/extensions/features-windows.md)을 사용하여 Azure VM에서 배포 후 구성 및 자동화 작업을 수행하는 방법을 알아봅니다. Azure VM에서 여러 다양한 VM 확장을 사용할 수 있습니다. 이 자습서에서는 Resource Manager 템플릿의 사용자 지정 스크립트 확장을 배포하여 Windows VM에서 PowerShell 스크립트를 실행합니다.  이 스크립트는 VM에 웹 서버를 설치합니다.
...
이 리소스 정의에 대한 자세한 정보가 필요한 경우 [확장 참조](https://docs.microsoft.com/azure/templates/microsoft.compute/virtualmachines/extensions)를 참조하세요. 다음은 중요한 요소입니다.
...
* **dependsOn**: 가상 머신을 만든 후 확장 리소스를 만들어야 합니다.
* **fileUris**: 스크립트 파일이 저장되는 위치입니다. 제공된 위치를 사용하지 않으려면 값을 업데이트해야 합니다.
* **commandToExecute**: 스크립트를 호출하는 명령입니다.  
...
포털에서 VM을 선택하고, VM 개요에서 IP 주소 오른쪽에 있는 **복사하려면 클릭** 단추를 사용하여 복사한 후 브라우저 탭에 붙여넣습니다. 기본 IIS 시작 페이지가 열리는데, 다음과 비슷한 모습입니다.

![Azure Resource Manager가 vm 확장 고객 스크립트 IIS 웹 서버 배포](./media/resource-manager-tutorial-deploy-vm-extensions/resource-manager-template-deploy-extensions-customer-script-web-server.png)
