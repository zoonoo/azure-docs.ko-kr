...
description: Azure 리소스 관리자를 사용하여 클래식 배포 리소스를 새 리소스 그룹 또는 구독으로 이동합니다.
ms.topic: conceptual
ms.date: 07/09/2019
...
---
# <a name="move-guidance-for-classic-deployment-model-resources"></a>클래식 배포 모델 리소스에 대한 지침 이동

클래식 모델을 통해 배포된 리소스를 이동하는 단계는 구독 내에서 리소스를 이동하는지 새 구독으로 이동하는지 여부에 따라 다릅니다.

## <a name="move-in-the-same-subscription"></a>동일한 구독으로 이동
...
클래식 리소스를 동일한 구독 내의 새 리소스 그룹으로 이동하려면 포털, Azure PowerShell, Azure CLI 또는 REST API를 통해 [표준 이동 작업을](../move-resource-group-and-subscription.md) 사용합니다. Resource Manager 리소스 이동을 위해 사용하는 동일한 작업을 사용합니다.
...
클래식 리소스를 이동하는 데 문제가 있는 경우 [지원](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/overview)팀에 문의하십시오.
