...
리소스 관리자를 추가할 때 모든 리소스는 기본 리소스 그룹에 소급 추가되었습니다. 지금 클래식 배포를 통해 리소스를 만드는 경우 배포 시 해당 리소스 그룹을 지정하지 않았더라도 해당 서비스의 기본 리소스 그룹 내에서 리소스가 자동으로 만들어집니다. 그러나 리소스 그룹 내에 만 있다고 해서 리소스가 리소스 관리자 모델로 변환된 것은 아닙니다.

## <a name="understand-support-for-the-models"></a>모델에 대한 지원 이해

다음의 세 가지 시나리오를 알고 있어야 합니다.

1. 클라우드 서비스는 리소스 관리자 배포 모델을 지원하지 않습니다.
...
어떤 경우에는 Resource Manager 명령으로 클래식 배포를 통해 만들어지는 리소스에 대한 정보를 검색하거나, 클래식 리소스를 다른 리소스 그룹으로 이동하는 것과 같은 관리 작업을 수행할 수 있습니다. 그러나 이러한 경우 형식이 리소스 관리자 작업을 지원한다는 인상을 주어서는 안 됩니다. 예를 들어 클래식 배포로 만든 가상 컴퓨터가 포함된 리소스 그룹이 있다고 가정합니다. 다음 Resource Manager PowerShell 명령을 실행하는 경우
...
* 가상 시스템은 네트워크 리소스 공급자(필수)에 정의된 특정 네트워크 인터페이스 카드와 Compute 리소스 공급자에 정의된 가용성 집합(선택 사항)을 참조합니다.
* 네트워크 인터페이스 카드는 가상 시스템의 할당된 IP 주소(필수), 가상 컴퓨터용 가상 네트워크의 서브넷(필수) 및 네트워크 보안 그룹(선택 사항)을 참조합니다.
* 가상 네트워크의 서브넷은 NSG(선택)를 참조합니다.
* 로드 밸런서 인스턴스는 가상 시스템의 네트워크 인터페이스 카드(선택 사항)를 포함하고 로드 밸런서 공용 또는 개인 IP 주소(선택 사항)를 참조하는 IP 주소의 백 엔드 풀을 참조합니다.
...
* 가상 머신 호스팅(컴퓨팅)을 위한 컨테이너 역할을 하는 데 필요한 클라우드 서비스. 가상 컴퓨터는 네트워크 인터페이스 카드와 Azure에서 할당한 IP 주소와 함께 자동으로 제공됩니다. 또한 클라우드 서비스는 외부 부하 분산 장치 인스턴스, 공용 IP 주소, 기본 엔드포인트를 포함하여 Windows 기반의 가상 머신을 위한 원격 데스크톱 및 원격 PowerShell 트래픽 및 Linux 기반의 가상 머신을 위한 SSH(Secure Shell) 트래픽을 허용합니다.
* 운영 체제, 임시 및 추가 데이터 디스크(저장소)를 포함하여 가상 시스템에 대한 가상 하드 디스크를 저장하는 필수 저장소 계정입니다.
* 서브넷 구조를 만들고 가상 시스템이 있는 서브넷(네트워크)을 선택할 수 있는 추가 컨테이너 역할을 하는 선택적 가상 네트워크입니다.
...
| Virtual Network |가상 머신의 가상 네트워크는 선택 사항입니다. 포함된 경우 리소스 관리자를 통해 가상 네트워크를 배포할 수 없습니다. |가상 머신에 Resource Manager에서 배포한 가상 네트워크가 필요합니다. |
| Storage 계정 |가상 시스템에는 운영 체제, 임시 및 추가 데이터 디스크에 대한 가상 하드 디스크를 저장하는 저장소 계정이 필요합니다. |가상 머신에 Blob Storage에서 해당 디스크를 저장하는 스토리지 계정이 필요합니다. |
...
| 선호도 그룹 |선호도 그룹은 Virtual Network를 만드는 데 필요했습니다. 그러나 지역 가상 네트워크가 도입되면서 더 이상 필요하지 않았습니다. |간단히 말해, Azure 리소스 관리자를 통해 표시되는 API에는 선호도 그룹 개념이 존재하지 않습니다. |
...
| 네트워크 인터페이스 |기본 및 보조 네트워크 인터페이스와 해당 속성이 가상 머신의 네트워크 구성으로 정의되었습니다. |네트워크 인터페이스는 Microsoft.Network 공급자가 표시하는 리소스입니다. 네트워크 인터페이스의 수명 주기는 가상 시스템에 연결되지 않습니다. 네트워크 인터페이스는 가상 머신의 할당된 IP 주소(필수), 가상 머신에 대한 가상 네트워크의 서브넷(필수) 및 네트워크 보안 그룹(선택)을 참조합니다. |
...
리소스를 클래식 배포에서 리소스 관리자 배포로 마이그레이션할 준비가 된 경우 다음을 참조하십시오.
...
이 구성은 지원되지 않습니다. 리소스 관리자를 사용하여 가상 컴퓨터를 클래식 배포를 사용하여 만든 가상 네트워크에 배포할 수 없습니다.

**Resource Manager를 사용하면 클래식 배포 모델을 통해 만든 사용자 이미지에서 가상 머신을 만들 수 있나요?**

이 구성은 지원되지 않습니다. 그러나 클래식 배포 모델을 사용하여 만든 저장소 계정에서 가상 하드 디스크 파일을 복사하여 Resource Manager를 통해 만든 새 계정에 추가할 수 있습니다.
...
포괄적인 시작 템플릿 집합은 Azure [리소스 관리자 빠른 시작 템플릿에서](https://azure.microsoft.com/documentation/templates/)찾을 수 있습니다.
