...
description: Azure PowerShell을 사용하여 Azure 리소스 관리자를 통해 리소스 그룹을 관리합니다. 리소스 그룹을 만들고, 나열하고, 삭제하는 방법을 보여 주면 됩니다.
author: mumian
ms.topic: conceptual
ms.date: 02/11/2019
ms.author: jgao
...
---
# <a name="manage-azure-resource-manager-resource-groups-by-using-azure-powershell"></a>Azure PowerShell을 사용하여 Azure 리소스 관리자 리소스 그룹 관리

Azure [리소스 관리자와](overview.md) 함께 Azure PowerShell을 사용하여 Azure 리소스 그룹을 관리하는 방법에 대해 알아봅니다. Azure 리소스 를 관리하기 위해 [Azure PowerShell을 사용하여 Azure 리소스 관리를](manage-resources-powershell.md)참조하십시오.
...
다음 PowerShell 스크립트는 리소스 그룹을 만들고 리소스 그룹을 표시합니다.
...
다음 PowerShell 스크립트는 구독 아래에 리소스 그룹을 나열합니다.
...
[기존 리소스 그룹에 리소스 배포를 참조합니다.](manage-resources-powershell.md#deploy-resources-to-an-existing-resource-group)

리소스 그룹 배포의 유효성을 검사하려면 [테스트-AzResourceGroup 배포](https://docs.microsoft.com/powershell/module/Az.Resources/Test-AzResourceGroupDeployment?view=azps-1.3.0)를 참조하십시오.
...
리소스 그룹 및 리소스에 태그를 적용하여 논리적으로 자산을 구성할 수 있습니다. 자세한 내용은 [태그 사용을 사용하여 Azure 리소스를 구성합니다.](tag-resources.md#powershell)

## <a name="export-resource-groups-to-templates"></a>리소스 그룹을 템플릿으로 내보내기

리소스 그룹을 설정한 후 리소스 그룹에 대한 리소스 관리자 템플릿을 볼 수 있습니다. 템플릿을 내보내면 다음과 같은 두 가지 이점이 있습니다.

- 템플릿에 전체 인프라가 포함되어 있으므로 솔루션의 향후 배포를 자동화합니다.
...
하나의 리소스를 내보내려면 해당 리소스 ID를 전달합니다.
...
두 개 이상의 리소스를 내보내려면 배열에 리소스 를 전달합니다.
...
템플릿을 내보낼 때 템플릿에서 매개 변수가 사용되는지 여부를 지정할 수 있습니다. 기본적으로 리소스 이름에 대한 매개 변수는 포함되지만 기본값은 없습니다. 배포 하는 동안 해당 매개 변수 값을 전달 해야 합니다.
...
템플릿을 `-IncludeParameterDefaultValue` 내보낼 때 매개 변수를 사용하는 경우 템플릿 매개 변수에는 현재 값으로 설정된 기본값이 포함됩니다. 이 기본값을 사용하거나 다른 값을 전달하여 기본값을 덮어쓸 수 있습니다.
...
템플릿을 `-SkipResourceNameParameterization` 내보낼 때 매개 변수를 사용하는 경우 리소스 이름에 대한 매개 변수는 템플릿에 포함되지 않습니다. 대신 리소스 이름은 리소스에 직접 현재 값으로 설정됩니다. 배포 하는 동안 이름을 사용자 지정할 수 없습니다.
...
[RBAC(역할 기반 액세스 제어)](../../role-based-access-control/overview.md)는 Azure에서 리소스에 대한 액세스를 관리하는 방법입니다. 자세한 내용은 [RBAC 및 Azure PowerShell을 사용하여 액세스 관리를](../../role-based-access-control/role-assignments-powershell.md)참조하십시오.
