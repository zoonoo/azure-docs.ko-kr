...
"프록시, 캐시" 리소스 **끝점** 을 구현 하는 **끝점** 은 Azure의 새 API에 대 한 요청 및 응답을 처리 해야 합니다. 이 경우 **resourceType** 은, 및에 대 한 새 AZURE 리소스 API를 생성 하 `PUT` `GET` 고 `DELETE` `GET` 모든 기존 리소스를 검색 하는 것 뿐만 아니라 단일 리소스에 대해 CRUD를 수행 합니다.

> [!NOTE]
> Azure API는 요청 메서드인 및를 생성 `PUT` `GET` `DELETE` 하지만 캐시 **끝점** 은 및만 처리 하면 됩니다 `PUT` `DELETE` .
> **끝점** 에서도 구현 하는 것이 좋습니다 `GET` .
...
- 사용자 지정 리소스 공급자는 `name` `type` `id` 요청에 대 한, 및 필드를 덮어씁니다.
- 사용자 지정 리소스 공급자는 `properties` 캐시 끝점에 대 한 개체 아래 에서만 필드를 반환 합니다.
...
`name` `id` `type` 사용자 지정 리소스 공급자는 사용자 지정 리소스에 대해, 및 필드가 자동으로 생성 됩니다.
