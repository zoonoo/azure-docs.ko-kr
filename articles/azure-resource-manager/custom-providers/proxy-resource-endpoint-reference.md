...
"프록시" 리소스 **끝점** 을 구현 하는 **끝점** 은 Azure에서 새 API에 대 한 요청 및 응답을 처리 해야 합니다. 이 경우 **resourceType** 은 `PUT`, `GET`및 `DELETE`에 대 한 새 AZURE 리소스 API를 생성 하 여 단일 리소스에 대 한 CRUD를 수행 하 고 모든 기존 리소스를 검색 `GET` 합니다.
