...
1. 들어오는 POST 요청을 기록 하 고를 반환 `200 OK`하는 공용 HTTPS 끝점을 실행 합니다.
...
> [!NOTE]
> 현재 응용 프로그램 정의 속성 `notificationEndpoints` 의에서 끝점을 하나만 제공할 수 있습니다.
...
1. 웹 후크 URI 위에 다음과 같이 쿼리 매개 변수를 제공 합니다 (예:\:https/. 각 알림에서 쿼리 매개 변수가 `sig` 예상 값 `Guid`을 포함 하는지 확인 합니다.
...
관리 되는 응용 프로그램 알림 서비스 `200 OK` 에는 웹 후크 끝점에서 알림으로의 응답이 필요 합니다. 웹 후크 끝점이 500 보다 크거나 같은 HTTP 오류 코드를 반환 하거나, 429 오류 코드를 반환 하는 경우 또는 끝점에 일시적으로 연결할 수 없는 경우 알림 서비스가 다시 시도 됩니다. 웹 후크 끝점을 10 시간 이내에 사용할 수 없게 되 면 알림 메시지가 삭제 되 고 다시 시도가 중지 됩니다.
