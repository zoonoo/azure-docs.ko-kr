...
클래식 모델을 통해 배포 된 리소스를 이동 하는 단계는 구독 내의 리소스를 이동 하는지 아니면 새 구독으로 이동 하는지에 따라 달라 집니다.
...
동일한 구독 내에서 새 리소스 그룹으로 클래식 리소스를 이동 하려면 포털, Azure PowerShell Azure CLI 또는 REST API를 통해 [표준 이동 작업](../resource-group-move-resources.md) 을 사용 합니다. Resource Manager 리소스 이동을 위해 사용하는 동일한 작업을 사용합니다.
...
클래식 리소스를 이동 하는 데 문제가 있는 경우 [지원](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/overview)담당자에 게 문의 하세요.
