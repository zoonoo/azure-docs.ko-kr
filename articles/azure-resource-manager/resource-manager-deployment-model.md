...
2. 가상 컴퓨터, 저장소 계정 및 가상 네트워크는 Resource Manager 및 클래식 배포 모델을 모두 지원합니다.
...
* 가상 컴퓨터는 SRP(저장소 리소스 공급자)에서 Blob 저장소(필수)에 디스크를 저장하도록 정의된 특정 Storage 계정에 따라 달라집니다.
* 가상 컴퓨터는 NRP(네트워크 리소스 공급)(필수)에서 정의된 특정 NIC 및 CRP(Compute 리소스 공급자)(선택)에서 정의된 가용성 집합을 참조합니다.
...
| 항목 | 클래식 | 리소스 관리자 |
| --- | --- | --- |
| Virtual Machines용 클라우드 서비스 |클라우드 서비스는 플랫폼 가용성 및 부하 분산이 필요한 가상 컴퓨터를 유지하는 컨테이너였습니다. |클라우드 서비스는 더 이상 새 모델을 사용하여 가상 컴퓨터를 만드는 데 필요한 개체가 아닙니다. |
| Virtual Network |가상 컴퓨터의 가상 네트워크는 선택 사항입니다. 포함되는 경우 Resource Manager에서 가상 네트워크를 배포할 수 없습니다. |가상 컴퓨터에 Resource Manager에서 배포한 가상 네트워크가 필요합니다. |
| Storage 계정 |가상 컴퓨터에 운영 체제, 임시 및 추가 데이터 디스크용 VHD를 저장하는 데 필요한 저장소 계정이 필요합니다. |가상 컴퓨터에 blob 저장소에서 해당 디스크를 저장하는 저장소 계정이 필요합니다. |
...
| 부하 분산 |클라우드 서비스를 만들면 배포된 Virtual Machines에 대한 암시적 부하 분산 장치가 제공됩니다. |부하 분산 장치는 Microsoft.Network 공급자가 표시하는 리소스입니다. 부하 분산이 필요한 Virtual Machines의 기본 네트워크 인터페이스는 부하 분산 장치를 참조해야 합니다. 부하 분산 장치는 내부 또는 외부에 있을 수 있습니다. 부하 분산 장치 인스턴스는 가상 컴퓨터의 NIC(옵션)를 포함하는 IP 주소의 백 엔드 풀을 참조하며 로드 분산 장치 공용 또는 개인 IP 주소(옵션)를 참조합니다. [자세한 정보](../virtual-network/resource-groups-networking.md) |
...
**클래식 배포를 사용하여 만든 가상 네트워크에 Resource Manager를 사용하여 만든 가상 컴퓨터를 배포할 수 있나요?**

이 구성은 지원되지 않습니다. Resource Manager를 사용하면 클래식 배포를 통해 만든 가상 네트워크에 가상 컴퓨터를 배포할 수 없습니다.

**Resource Manager를 사용하면 클래식 배포 모델을 통해 만든 사용자 이미지에서 가상 컴퓨터를 만들 수 있나요?**
