<properties
	...
	keywords="azure 앱 서비스,앱 서비스,모바일 앱,모바일 백 엔드,saas,azure saas,엔터프라이즈 모바일"
	documentationCenter=""
...
1. **Azure 클래식 포털**의 [Active Directory] 섹션에서 테넌트를 선택합니다. 앱 서비스용으로 만든 웹 응용 프로그램으로 이동합니다.
...
SharePoint를 호출하려면 모바일 앱이 연결할 끝점을 지정해야 합니다. 또한 앱 서비스의 ID를 입증할 수 있어야 합니다. 이러한 작업은 클라이언트 ID 및 클라이언트 암호 쌍을 사용해서 수행됩니다. 앱 서비스에 대한 클라이언트 ID는 AAD 로그인 설정 중에 이미 가져와서 저장했습니다. 이러한 ID는 민감한 자격 증명이므로 코드에 일반 텍스트로 저장해서는 안 됩니다. 대신 모바일 앱 백 엔드에 대한 응용 프로그램 설정으로 이러한 값을 설정합니다.
...
3. [Azure 포털](https://portal.azure.com/)의 모바일 앱 백 엔드에서 **모든 설정** > **응용 프로그램 설정**을 클릭하고 **앱 설정**까지 아래로 스크롤합니다. 여기에서는 필요한 자격 증명을 참조할 수 있도록 키-값 쌍을 제공할 수 있습니다.
