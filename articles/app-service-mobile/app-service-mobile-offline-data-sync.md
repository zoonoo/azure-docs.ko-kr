...
Windows Phone 또는 Microsoft Store에서 SQLite 기반 구현을 사용하려면 SQLite 확장을 설치해야 합니다. 자세한 내용은 [유니버설 Windows 플랫폼: 오프라인 동기화 사용 을]참조하십시오. 안드로이드와 iOS는 장치 운영 체제 자체에서 SQLite 버전과 함께 제공되므로 자체 버전의 SQLite를 참조 할 필요가 없습니다.
...
로컬 저장소는 [.NET 클라이언트 SDK에서와] `IMobileServicesSyncContext.InitializeAsync(localstore)` 같은 초기화 방법을 사용하여 동기화 컨텍스트와 연결됩니다.

## <a name="how-offline-synchronization-works"></a><a name="how-sync-works"></a>오프라인 암호 동기화 작동 방법
...
* **증분 동기화**: 가져오기 작업의 첫 번째 매개 변수는 클라이언트에만 사용되는 *쿼리 이름* 입니다. null이 아닌 쿼리 이름을 사용하는 경우 Azure Mobile SDK는 *증분 동기화를*수행합니다. 끌어오기 작업이 결과 집합을 반환할 `updatedAt` 때마다 해당 결과 집합의 최신 타임스탬프가 SDK 로컬 시스템 테이블에 저장됩니다. 후속 끌어오기 작업은 해당 timestamp 이후의 레코드만 검색합니다.
