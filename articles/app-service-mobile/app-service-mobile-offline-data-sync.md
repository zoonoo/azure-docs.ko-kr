...
오프라인 사용을 지원하려면 앱은 *(.NET 클라이언트 SDK) 또는* (iOS 클라이언트)과 같은 `IMobileServiceSyncTable`동기화 테이블`MSSyncTable` API를 대신 사용해야 합니다. 같은 CRUD 작업 모두(만들기, 읽기, 업데이트, 삭제)가 *로컬 저장소*에 읽거나 쓰는 것을 제외하고 동기화 테이블 API에 대해 작동합니다. 모든 동기화 테이블 작업을 수행하려면 먼저 로컬 저장소를 초기화해야 합니다.
...
Windows Phone 또는 Microsoft Store에서 SQLite 기반 구현을 사용하려면 SQLite 확장을 설치해야 합니다. 자세한 내용은 [Universal Windows Platform: 오프라인 동기화 사용]을 참조 하세요. Android 및 iOS는 장치 운영 체제 자체에서 SQLite 버전을 제공 하므로 자신의 SQLite 버전을 참조할 필요가 없습니다.
...
로컬 저장소는 `IMobileServicesSyncContext.InitializeAsync(localstore)`.NET 클라이언트 SDK[.NET 클라이언트 SDK]와 같은 초기화 메서드를 사용하여 동기화 컨텍스트와 연결됩니다.
...
* **증분 동기화**: 가져오기 작업의 첫 번째 매개 변수는 클라이언트에만 사용되는 *쿼리 이름* 입니다. Null이 아닌 쿼리 이름을 사용 하는 경우 Azure 모바일 SDK는 *증분 동기화*를 수행 합니다. 끌어오기 작업에서 결과 집합을 반환할 때마다 해당 결과 집합에서 최신 `updatedAt` 타임 스탬프가 SDK 로컬 시스템 테이블에 저장 됩니다. 후속 끌어오기 작업은 해당 timestamp 이후의 레코드만 검색합니다.
...
* **제거**: `IMobileServiceSyncTable.PurgeAsync`를 사용하여 로컬 저장소의 내용을 지울 수 있습니다.
