...
> [!NOTE]
> 이 항목은 웹, 모바일 및 API 앱을 다루는 통합된 [앱 서비스 인증/권한 부여](../app-service/app-service-authentication-overview.md) 항목으로 마이그레이션됩니다.
> 
> 

앱 서비스 인증/권한 부여는 응용 프로그램이 앱 백 엔드에 필요한 코드 변경 없이 사용자를 로그인할 수 있도록 만드는 기능입니다. 응용 프로그램을 보호하고 사용자 단위당 데이터로 작업하는 쉬운 방법을 제공합니다.

앱 서비스는 페더레이션된 ID를 사용하며 여기서 타사 **ID 공급자** ("IDP")는 계정을 저장하고 사용자를 인증하며 응용 프로그램은 자체 ID 대신 이 ID를 사용합니다. App Service는 기본적으로 *Azure Active Directory*, *Facebook*, *Google*, *Microsoft 계정* 및 *Twitter*와 같은 다섯 가지 ID 공급자를 지원합니다. 또한 다른 ID 공급자 또는 사용자 고유의 사용자 지정 ID 솔루션을 통합하여 앱에 이 지원을 확장할 수 있습니다.
