...
1. `ToDoActivity.cs`의 위쪽에서 `applicationURL` 및 `gatewayURL`의 초기화를 변경하여 잘못된 URL을 가리킵니다.
...
2. `ToDoActivity.SyncAsync`을 업데이트하므로 오프라인 상태를 가정하여 `MobileServicePushFailedException`가 발생하고 무시됩니다.
...
3. 앱을 빌드 및 실행합니다. 이름 확인예외를 보고하는 대화 상자를 받으면 닫습니다. 새 할 일 항목을 추가하고 `MobileServicePushFailedException`이 대화 상자를 표시하지 않고 처리되기 때문에 앱이 연결된 것처럼 동작한다고 알립니다.
...
1. `ToDoActivity.cs`를 엽니다. `applicationURL` 및 `gatewayURL`를 수정하여 올바른 URL을 가리킵니다.
