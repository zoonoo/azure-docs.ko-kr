...
3. SQLite는 네이티브 라이브러리이며 **x86**, **x64** 또는 **ARM**과 같은 플랫폼 특정 아키텍처를 선택해야 합니다. **임의 CPU**는 지원되지 않습니다. 솔루션 탐색기의 맨 위에 있는 솔루션을 클릭하고 프로세서 아키텍처 드롭다운 상자를 테스트하려는 지원되는 설정 중 하나로 변경합니다.

    >[AZURE.NOTE] Visual Studio 2015를 사용하는 경우 솔루션을 마우스 오른쪽 단추로 클릭한 다음 **속성**을 클릭하여 Windows 및 Windows Phone 프로젝트에 대한 플랫폼을 설정하는 구성 관리자를 엽니다.

    ![][13]


4. 솔루션 탐색기에서 Windows 8.1 런타임 및 Windows Phone 8.1 플랫폼 프로젝트에 대한 **참조**를 마우스 오른쪽 단추로 클릭합니다. **확장** 섹션에 있는 SQLite에 대한 참조가 있는지 확인합니다.

    >[AZURE.NOTE] Visual Studio 2015를 사용하는 경우 Windows 8.1 런타임 및 Windows Phone 8.1 플랫폼 프로젝트에 대한 **참조** 노드를 마우스 오른쪽 단추로 클릭하고 **참조 추가**를 클릭하여 참조 관리자를 엽니다.
