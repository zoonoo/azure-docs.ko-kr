<properties urlDisplayName="Enable Remote Desktop" pageTitle="클라우드 서비스에 원격 데스크톱 사용(Node.js)" metaKeywords="Azure Node.js 원격 액세스, Azure Node.js 원격 연결, Azure Node.js VM 액세스, Azure Node.js 가상 컴퓨터 액세스" description="Azure Node.js 응용 프로그램을 호스트하는 가상 컴퓨터에 대해 원격 데스크톱 액세스를 사용하도록 설정하는 방법에 대해 알아봅니다. " metaCanonical="" services="cloud-services" documentationCenter="nodejs" title="Enabling Remote Desktop in Azure" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
2.  디렉터리를 서비스 디렉터리로 변경하고 **Enable-AzureServiceProjectRemoteDesktop**을 입력한 다음 클라우드의 역할 인스턴스로 인증할 때 사용할 사용자 이름과 암호를 입력합니다.
...
2.  **인스턴스**를 클릭한 다음 **프로덕션**이나 **스테이징**을 클릭하여 해당 서비스의 인스턴스를 확인합니다. 인스턴스를 선택한 후 페이지 아래쪽에서 **연결**을 클릭합니다.
