...
**-DownloadLogs** 매개 변수를 지정하면 이 cmdlet은 로그 파일의 위치만 반환합니다. 

**-DownloadLogs** 매개 변수는 지정하고 출력 디렉터리(**-Output **매개 변수)를 지정하지 않는 경우 로그 파일이 기본 **문서** 폴더로 다운로드됩니다. 

**-DownloadLogs** 매개 변수와 함께 출력 폴더(**-Output**)를 지정하는 경우에는 로그 파일이 지정한 폴더로 다운로드됩니다. 
...
파이프라인을 만들면 데이터 처리가 수행되는 기간을 지정할 수 있습니다. 파이프라인의 활성 기간을 지정하면 각 ADF 테이블에 대해 정의된 가용성 속성을 기준으로 데이터 조각이 처리되는 기간이 정의됩니다.
