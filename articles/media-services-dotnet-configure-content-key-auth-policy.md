...
현재 다음 스트리밍 형식을 암호화할 수 있습니다. HLS, MPEG DASH 및 부드러운 스트리밍. HDS 스트리밍 형식 또는 점진적 다운로드는 암호화할 수 없습니다.

미디어 서비스에서 자산을 암호화하려는 경우 암호화 키(**CommonEncryption** 또는 **EnvelopeEncryption**)를 자산([여기]에 설명되어 있음(media-services-dotnet-create-contentkey.md))에 연결하고 해당 키에 대해 권한 부여 정책(이 문서에서 설명)도 구성해야 합니다. 
...
- 자산은 다중 비트 전송률 MP4 또는 다중 비트 전송률 부드러운 스트리밍 파일 집합을 포함해야 합니다. 자세한 내용은 [자산 인코딩]을 참조하세요(media-services-encode-asset.md).  
...
##다음 단계
콘텐츠 키의 권한 부여 정책을 구성했으므로 [자산 배달 정책을 구성하는 방법](media-services-dotnet-configure-asset-delivery-policy.md) 항목으로 이동합니다.
