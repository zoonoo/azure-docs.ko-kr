...
## <a name="stopstart-an-azure-database-for-mariadb-preview"></a>Azure Database for MariaDB 중지/시작 (미리 보기)
Azure Database for MariaDB 사용 하지 않을 때 서버를 **중지** 하 고 작업을 다시 시작할 때 서버를 **시작** 하는 기능을 제공 합니다. 이는 기본적으로 데이터베이스 서버에 대 한 비용을 절감 하 고 사용 하는 경우에만 리소스에 대 한 비용을 지불 합니다. 이는 개발-테스트 워크 로드에 대해 매우 중요 하 고 하루 중에만 서버를 사용 하는 경우에도 더욱 중요 합니다. 서버를 중지 하면 모든 활성 연결이 삭제 됩니다. 나중에 서버를 다시 온라인 상태로 전환 하려는 경우 [Azure Portal](../mysql/how-to-stop-start-server.md) 또는 [CLI](../mysql/how-to-stop-start-server.md)를 사용할 수 있습니다.
...
서버를 중지 하는 동안 서버에서 관리 작업을 수행할 수 없습니다. 서버에서 구성 설정을 변경 하려면 [서버를 시작](../mysql/how-to-stop-start-server.md)해야 합니다.
