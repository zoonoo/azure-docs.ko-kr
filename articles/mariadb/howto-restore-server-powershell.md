...
서버를 만드는 동안 이러한 값을 설정 하는 방법에 대 한 자세한 내용은 [PowerShell을 사용 하 여 Azure Database for MariaDB 서버 만들기](quickstart-create-mariadb-server-database-using-azure-powershell.md)를 참조 하세요.
...
서버를 지리적으로 중복 백업 하도록 구성한 경우 기존 서버의 백업에서 새 서버를 만들 수 있습니다. 이 새 서버는 Azure Database for MariaDB를 사용할 수 있는 모든 지역에서 만들 수 있습니다.

지역 중복 백업을 사용 하 여 서버를 만들려면 `Restore-AzMariaDbServer` **UseGeoRestore** 매개 변수와 함께 명령을 사용 합니다.
