...
* 지역별 가용성: Azure Database for MariaDB는 미국 서 부 2, 프랑스 중부, 아랍에미리트 북부 및 독일 중부에서 사용할 수 있습니다. 그러나 쌍을 이루는 지역에는 사용할 수 없습니다.
...
만들 때 복제본은 마스터 서버의 방화벽 규칙을 상속 합니다. 그런 다음 이러한 규칙은 마스터 서버와는 독립적입니다.
...
복제본을 만들 때 방화벽 규칙 및 매개 변수 설정은 마스터 서버에서 복제본으로 상속 됩니다. 그런 다음 복제본의 규칙은 독립적입니다.

### <a name="stopped-replicas"></a>중지된 복제본

마스터 서버와 읽기 복제본 간의 복제를 중지 하면 중지 된 복제본이 읽기와 쓰기를 모두 수락 하는 독립 실행형 서버가 됩니다. 독립 실행형 서버를 다시 복제본으로 만들 수 없습니다.
