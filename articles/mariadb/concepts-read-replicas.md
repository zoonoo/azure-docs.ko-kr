---
title: Azure Database for MariaDB에서에서 읽기 복제본
description: 이 문서에서는 Azure database for MariaDB 읽기 복제본을 설명합니다.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
ms.locfileid: "67079367"
---
# <a name="read-replicas-in-azure-database-for-mariadb"></a>Azure Database for MariaDB에서에서 읽기 복제본

읽기 복제본 기능을 사용 하면 읽기 전용 서버에는 Azure Database for MariaDB 서버에서에서 데이터를 복제할 수 있습니다. 마스터와 동일한 지역 내에서 최대 5 개의 복제본으로 마스터 서버에서 복제할 수 있습니다. 복제본 전역 트랜잭션 ID (GTID)를 사용 하 여 MariaDB 엔진의 이진 로그 (binlog) 파일 위치 기반 복제 기술을 사용 하 여 비동기로 업데이트 됩니다. binlog 복제에 대해 자세히 알아보려면 [binlog 복제 개요](https://mariadb.com/kb/en/library/replication-overview/)를 참조하세요.

> [!IMPORTANT]
> 동일한 지역 복제본이 현재 공개 미리 보기를 읽습니다.

복제본은 일반 비슷합니다 Azure Database for MariaDB 서버를 관리 하는 새 서버입니다. 읽기 복제본의 경우, vCore 및 스토리지에 프로비저닝된 컴퓨팅에 대한 비용이 GB/월 단위로 청구됩니다.

GTID 복제에 대 한 자세한 내용은 참조 하십시오 합니다 [MariaDB 복제 설명서](https://mariadb.com/kb/en/library/gtid/)합니다.
...
비동기 복제를 사용 하는 읽기 복제본 기능. 이 기능은 동기식 복제 시나리오를 위한 것이 아닙니다. 마스터와 복제본 간에는 측정 가능한 지연이 발생합니다. 복제본의 데이터는 결과적으로 마스터의 데이터와 일치하게 됩니다. 이러한 지연 시간을 수용할 수 있는 워크로드에 이 기능을 사용합니다.
...
복제본 만들기 워크플로 시작 하면를 빈 Azure Database for MariaDB 서버 생성 됩니다. 새 서버는 마스터 서버에 있는 데이터로 채워집니다. 생성 시간은 마스터의 데이터 양과 지난 주 전체 백업 이후의 시간에 따라 달라집니다. 시간은 몇 분에서 몇 시간까지 걸릴 수 있습니다.
...
일반 Azure Database for MariaDB 서버에서와 해당 호스트 이름 및 유효한 사용자 계정을 사용 하 여 복제본에도 연결할 수 있습니다. 관리자 사용자 이름 **myadmin**을 사용한 **myreplica**라는 이름의 서버의 경우, mysql CLI를 사용하여 복제본에 연결할 수 있습니다.
...
Azure Database for MariaDB는 제공 된 **복제 지연 시간 (초)에서** Azure Monitor에서 메트릭. 이 메트릭은 복제본에만 사용할 수 있습니다.

이 메트릭을 사용 하 여 계산 되는 `seconds_behind_master` MariaDB의에서 사용 가능한 메트릭 `SHOW SLAVE STATUS` 명령입니다.
...
읽기 복제본을 새 Azure Database for MariaDB 서버 만들어집니다. 기존 서버를 복제본으로 만들 수 없습니다. 다른 읽기 복제본의 복제본은 만들 수 없습니다.

### <a name="replica-configuration"></a>복제본 구성

복제본은 마스터와 같은 서버 구성을 사용하여 생성됩니다. 복제본이 만들어지면 몇 가지 설정을 변경할 수 있습니다 하지 독립적으로 마스터 서버에서: 계산 세대, Vcore, 저장소, 백업 보존 기간 및 MariaDB 엔진 버전입니다. 가격 책정도 기본 계층에서 다른 계층으로 또는 다른 계층에서 기본 계층으로 변경하는 경우 이외의 다른 방식으로 독립적으로 변경할 수 있습니다.
...
- 메모리 내 테이블이 있으면 복제본이 동기화되지 않을 수 있기 때문입니다. MariaDB 복제 기술의 제한 됩니다.
