---
title: 비즈니스 연속성 - MariaDB용 Azure 데이터베이스
description: MariaDB 서비스에 Azure Database를 사용할 때 비즈니스 연속성(시점 복원, 데이터 센터 중단, 지역 복원)에 대해 알아봅니다.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
---
# <a name="understand-business-continuity-in-azure-database-for-mariadb"></a>MariaDB용 Azure 데이터베이스의 비즈니스 연속성 이해

이 문서에서는 MariaDB용 Azure 데이터베이스가 비즈니스 연속성 및 재해 복구에 대해 제공하는 기능에 대해 설명합니다. 데이터 손실을 유발하거나 데이터베이스 및 애플리케이션을 사용할 수 없게 될 수 있는 중단 이벤트로부터 복구하는 옵션에 대해 알아봅니다. 사용자 또는 애플리케이션 오류가 데이터 무결성에 영향을 주거나, Azure 지역에 가동 중단이 발생하거나, 애플리케이션에 유지 관리가 필요할 때 수행할 작업을 알아봅니다.
