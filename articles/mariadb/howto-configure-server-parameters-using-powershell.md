...
description: 이 문서에서는 PowerShell을 사용하여 Azure Database for MariaDB에서 서비스 매개 변수를 구성하는 방법을 설명합니다.
author: savjani
ms.author: pariks
ms.service: mariadb
...
ms.locfileid: "98662460"
---
# <a name="configure-server-parameters-in-azure-database-for-mariadb-using-powershell"></a>PowerShell을 사용하여 Azure Database for MariaDB에서 서버 매개 변수 구성

PowerShell을 사용하여 Azure Database for MariaDB 서버에 대한 구성 매개 변수를 나열, 표시 및 업데이트할 수 있습니다. 엔진 구성의 하위 집합은 서버 수준에서 노출되고 수정할 수 있습니다.
...
서버의 수정 가능한 모든 매개 변수와 해당 값을 나열하려면 `Get-AzMariaDbConfiguration` cmdlet을 실행합니다.

다음 예에는 **myresourcegroup** 리소스 그룹의 **mydemoserver** 서버에 대한 서버 구성 매개 변수가 나열되어 있습니다.
...
서버의 특정 구성 매개 변수에 대한 세부 정보를 표시하려면 `Get-AzMariaDbConfiguration` cmdlet을 실행하고 **Name** 매개 변수를 지정합니다.
...
또한 특정 서버 구성 매개 변수의 값을 수정할 수 있습니다. 그러면 MariaDB 서버 엔진에 대한 기본 구성 값이 업데이트됩니다. 구성을 업데이트하려면 `Update-AzMariaDbConfiguration` cmdlet을 사용합니다.
...
> [PowerShell을 사용하여 Azure Database for MariaDB 서버에서 스토리지 자동 확장](howto-auto-grow-storage-powershell.md).