...
ms.locfileid: "52833842"
---
# <a name="high-availability-concepts-in-azure-database-for-mariadb"></a>Azure Database for MariaDB의 고가용성 개념
Azure Database for MariaDB 서비스는 높은 수준의 가용성을 보장합니다. 재정적으로 지원되는 SLA(서비스 수준 계약)는 일반 공급 시 99.99%입니다. 이 서비스를 사용할 때는 응용 프로그램 작동 중단 시간이 거의 없습니다.
...
MariaDB 데이터베이스 애플리케이션은 끊어진 연결과 실패한 트랜잭션을 감지하고 재시도하도록 빌드되었다는 점이 중요합니다. 응용 프로그램이 재시도할 때 응용 프로그램의 연결은 새로 생성된 인스턴스로 투명하게 리디렉션되며 이 인스턴스는 실패한 인스턴스를 대신합니다.
