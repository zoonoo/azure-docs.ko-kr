<properties pageTitle="Azure WebJobs SDK 정의" metaKeywords="Azure Azure WebJobs SDK, Azure storage, Azure queues, Azure tables" description="Azure WebJobs SDK에 대해 소개하고 SDK의 정의, SDK가 유용한 일반적인 시나리오 및 코드 샘플에 대해 설명합니다." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Azure WebJobs SDK 정의" authors="tdykstra" solutions="" manager="wpickett" editor="jimbe" />
...
-   [시나리오][시나리오]
-   [코드 샘플][코드 샘플]
-   [WebJobs 외부에서 WebJobs SDK 사용][WebJobs 외부에서 WebJobs SDK 사용]
-   [WebJobs SDK를 사용하여 함수 호출][WebJobs SDK를 사용하여 함수 호출]
-   [다음 단계][다음 단계]

## <span id="overview"></span></a>개요

[WebJobs][WebJobs]는 웹 사이트와 동일한 컨텍스트에서 프로그램이나 스크립트를 실행할 수 있도록 하는 Azure 웹 사이트의 기능입니다. WebJobs SDK는 WebJob으로 실행되며 Azure 저장소 큐, Blob 및 테이블과 서비스 버스 큐에 사용 가능한 코드를 작성하는 작업을 간소화하기 위한 도구입니다.
...
-   백그라운드 스레드에서 실행할 기타 장기 실행 작업(예: [전자 메일 전송][전자 메일 전송])

## <span id="code"></span></a> 코드 샘플
...
## <span id="workerrole"></span></a>WebJobs 외부에서 WebJobs SDK 사용
...
-   자세한 내용은 [함수를 수동으로 호출하는 방법][함수를 수동으로 호출하는 방법] 및 [로그를 작성하는 방법][로그를 작성하는 방법]을 참조하세요.

## <span id="nextsteps"></span></a>다음 단계

WebJobs SDK에 대한 자세한 내용은 [Azure WebJobs 권장 리소스][Azure WebJobs 권장 리소스]를 참조하세요.
  ...
  [다음 단계]: #nextsteps
  [WebJobs]: /ko-kr/documentation/articles/web-sites-create-web-jobs/
  [WebJobs SDK 시작 자습서]: ../websites-dotnet-webjobs-sdk-get-started/
  [전자 메일 전송]: https://github.com/victorhurdugaci/AzureWebJobsSamples/tree/master/SendEmailOnFailure
  ...
  [함수를 수동으로 호출하는 방법]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#manual
  [로그를 작성하는 방법]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs
