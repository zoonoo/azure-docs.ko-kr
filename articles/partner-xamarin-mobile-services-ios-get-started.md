...
이 자습서를 완료하려면 XCode 4.5 및 iOS 5.0 이상 버전이 필요하며, OS X의 경우 [Xamarin Studio]가 추가로 필요하고 Windows의 경우 Visual Studio용 Xamarin Visual Studio 플러그 인이 추가로 필요합니다.
...
2. 빠른 시작 탭에서 **플랫폼 선택** 아래의 **Xamarin.iOS**을 클릭하고 **새 Xamarin.iOS 앱 만들기**를 확장합니다.
...
3. 아직 수행하지 않은 경우 [Xcode] v4.4 이상 버전 및 [Xamarin Studio]를 다운로드하여 설치합니다.

4. **TodoItems 테이블 만들기**를 클릭하여 앱 데이터를 저장할 테이블을 만듭니다.

5. **앱 다운로드 및 실행**에서 **다운로드**를 클릭합니다. 

	그러면 모바일 서비스에 연결되고 Xamarin.iOS용 Azure 모바일 서비스 구성 요소를 참조하는 샘플 _할 일 모음_ 응용 프로그램용 프로젝트가 다운로드됩니다. 압축된 프로젝트 파일을 로컬 컴퓨터에 저장하고 저장한 위치를 기록해 둡니다.
