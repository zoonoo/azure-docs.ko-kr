...
| 1 | 전역을 제공 합니다. | ☐ |  |
| 3 | 주문 어플라이언스. | ☐ |  |
| 4 | 네트워크에서 서브넷 목록을 준비 합니다. | ☐ |  |
| 5 | 프로덕션 네트워크의 VLAN 목록을 제공 합니다. | ☐ |  |
| 6 | 네트워크의 스위치 모델 목록을 제공 합니다. | ☐ |  |
| 7 | 산업 장비의 공급 업체 및 프로토콜 목록을 제공 합니다. | ☐ |  |
| 8 | 센서에 대 한 네트워크 세부 정보 (IP 주소, 서브넷, D-GW, DNS)를 제공 합니다. | ☐ |  |
| 9 | 필요한 방화벽 규칙 및 액세스 목록을 만듭니다. | ☐ |  |
| 10 | 포트 모니터링에 대 한 스위치에 스패닝 포트를 만들거나 원하는 대로 네트워크 탭을 구성 합니다. | ☐ |  |
| 11 | 센서 어플라이언스에 대 한 랙 공간을 준비 합니다. | ☐ |  |
| 12 | 직원을 위한 워크스테이션을 준비 합니다. | ☐ |  |
| 13 | IoT 랙 장치용 Defender에 대 한 키보드, 모니터 및 마우스를 제공 합니다. | ☐ |  |
