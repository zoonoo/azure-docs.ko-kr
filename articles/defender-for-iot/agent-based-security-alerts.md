...
이 문서에서는 IoT 장치에서 트리거될 수 있는 기본 제공 경고 목록을 찾을 수 있습니다.
...
| 이름 | Severity | 데이터 원본 | Description | 제안 된 수정 단계 |
|--|--|--|--|--|
| **높은** 심각도 |  |  |  |
| 이진 명령줄 | 높은 | 클래식 Defender-마이크로 에이전트 | 명령줄에서 호출 되거나 실행 되는 LA Linux 이진 파일이 검색 되었습니다. 이 프로세스는 합법적인 작업 일 수도 있고 장치가 손상 되었다는 표시 일 수도 있습니다. | 명령을 실행 한 사용자를 사용 하 여 명령을 검토 하 고 장치에서 실행 될 것으로 예상 되는 것이 무엇 인지 확인 합니다. 그렇지 않은 경우 정보 보안 팀에 게 경고를 에스컬레이션 합니다. |
| 방화벽 사용 안 함 | 높은 | 클래식 Defender-마이크로 에이전트 | 호스트에서 호스트 방화벽을 조작할 수 있습니다. 악의적인 행위자는 데이터를 탈취 하기 위해 온-호스트 방화벽을 사용 하지 않도록 설정 하는 경우가 많습니다. | 명령을 실행 한 사용자를 검토 하 여이 장치가 장치에서 정상적인 예상 작업 인지 확인 합니다. 그렇지 않은 경우 정보 보안 팀에 게 경고를 에스컬레이션 합니다. |
| 포트 전달 검색 | 높은 | 클래식 Defender-마이크로 에이전트 | 검색 된 외부 IP 주소에 대 한 포트 전달의 시작입니다. | 장치에서 볼 수 있는 합법적인 작업 인 경우 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| Auditd 로깅을 사용 하지 않도록 설정할 수 있습니다. | 높은 | 클래식 Defender-마이크로 에이전트 | Linux Auditd 시스템은 시스템에서 보안과 관련 된 정보를 추적 하는 방법을 제공 합니다. 시스템은 가능한 한 시스템에서 발생 하는 이벤트에 대 한 많은 정보를 기록 합니다. 이 정보는 중요 업무용 환경에서 보안 정책을 위반 하는 사용자 및 수행한 작업을 확인 하는 데 중요 합니다. Auditd 로깅을 사용 하지 않도록 설정 하면 시스템에서 사용 되는 보안 정책의 위반을 발견할 수 없습니다. | 비즈니스 상의 이유로 합법적인 활동 인지는 장치 소유자에 게 문의 하세요. 그렇지 않은 경우이 이벤트는 악의적인 행위자에 의해 활동을 숨기는 것일 수 있습니다. 정보 보안 팀에 즉시 인시던트를 전달 합니다. |
| 역방향 셸 | 높은 | 클래식 Defender-마이크로 에이전트 | 장치에서 호스트 데이터를 분석 하면 잠재적인 역방향 셸이 검색 되었습니다. 역방향 셸은 종종 악성 행위자가 제어 하는 컴퓨터로 다시 호출 하는 손상 된 컴퓨터를 가져오는 데 사용 됩니다. | 장치에서 볼 수 있는 합법적인 작업 인 경우 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| Bruteforce 시도 성공 | 높은 | 클래식 Defender-마이크로 에이전트 | 여러 번 실패 한 로그인 시도를 확인 하 고 성공한 로그인을 확인 했습니다. 무차별 암호 대입 공격이 장치에서 성공 했을 수 있습니다. | SSH 무차별 암호 대입 경고 및 장치에 대 한 작업을 검토 합니다. <br>작업이 악성 인 경우:<br> 손상 된 계정에 대 한 암호 재설정을 롤아웃 합니다.<br> 맬웨어에 대 한 장치를 조사 하 고 재구성 합니다 (있는 경우). |
| 로컬 로그인 성공 | 높은 | 클래식 Defender-마이크로 에이전트 | 장치에 대 한 로컬 로그인에 성공 했습니다. | 로그인 한 사용자가 인증 된 파티 인지 확인 합니다. |
| 웹 셸 | 높은 | 클래식 Defender-마이크로 에이전트 | 가능한 웹 셸이 검색 되었습니다. 악의적인 행위자는 일반적으로 손상 된 컴퓨터에 웹 셸을 업로드 하 여 지 속성을 얻거나 추가 익스플로잇을 얻습니다. | 장치에서 볼 수 있는 합법적인 작업 인 경우 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| **중간** 심각도 |  |  |  |
| 일반적인 Linux 봇과 유사한 동작이 감지됨 | 중간 | 클래식 Defender-마이크로 에이전트 | 일반적으로 일반적인 Linux 봇 넷 연결 된 프로세스의 실행이 검색 되었습니다. | 장치에서 볼 수 있는 합법적인 작업 인 경우 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| Fairware 랜섬웨어와 유사한 동작이 감지됨 | 중간 | 클래식 Defender-마이크로 에이전트 | 호스트 데이터의 분석을 사용 하 여 검색 된 의심 스러운 위치에 적용 된 rm-rf 명령 실행. Rm-rf는 파일을 재귀적으로 삭제 하므로 일반적으로 개별 폴더 에서만 사용 됩니다. 이 경우 많은 양의 데이터를 제거할 수 있는 위치에서 사용 됩니다. Fairware 랜섬웨어는 이 폴더에서 rm-rf 명령을 실행하는 것으로 알려져 있습니다. | 장치에서 볼 수 있는 합법적인 활동 이었던 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| 랜 섬 웨어 감지와 유사한 동작 | 중간 | 클래식 Defender-마이크로 에이전트 | 사용자가 시스템이 나 개인 파일에 액세스 하지 못하도록 하는 알려진 랜 섬 웨어와 유사한 파일의 실행으로, 액세스 권한을 다시 얻기 위해 ransom 지불을 요구할 수 있습니다. | 장치에서 볼 수 있는 합법적인 작업 인 경우 명령을 실행 한 사용자를 검토 합니다. 그렇지 않은 경우 경고를 정보 보안 팀으로 에스컬레이션 합니다. |
| 암호화 동전 마이너 컨테이너 이미지가 검색 됨 | 중간 | 클래식 Defender-마이크로 에이전트 | 알려진 디지털 통화 마이닝 이미지를 실행 하는 컨테이너를 검색 합니다. | 1 .이 동작이 의도 되지 않은 경우 관련 컨테이너 이미지를 삭제 합니다.<br> 2. 안전 하지 않은 TCP 소켓을 통해 Docker 디먼에 액세스할 수 없는지 확인 합니다.<br> 3. 정보 보안 팀에 게 경고를 에스컬레이션 합니다. |
...
| 로컬 사용자 삭제 검색 됨 | 낮음 | 클래식 Defender-마이크로 에이전트 | 검색 된 로컬 사용자를 삭제 합니다. 악의적인 행위자가 합법적인 사용자에 대 한 액세스를 거부 하거나 작업 기록을 삭제 하려고 시도 하는 경우가 일반적입니다. | 변경 내용이 영향을 받는 사용자에 게 필요한 권한과 일치 하는지 확인 합니다. 변경 내용이 일치 하지 않으면 정보 보안 팀으로 에스컬레이션 합니다. |
