---
title: 보안 에이전트 구성
description: Iot 보안 서비스용 Defender에 사용할 IoT 보안 에이전트에 대해 Defender를 구성 하는 방법에 대해 알아봅니다.
...
ms.locfileid: "103495779"
---
# <a name="tutorial-configure-security-agents"></a>자습서: 보안 에이전트 구성

이 문서에서는 IoT 보안 에이전트의 Defender에 대해 설명 하 고이를 변경 및 구성 하는 방법에 대해 자세히 설명 합니다.
...
IoT 용 Defender 보안 에이전트는 IoT 장치에서 데이터를 수집 하 고 보안 작업을 수행 하 여 검색 된 취약점을 완화 합니다. 보안 에이전트 구성은 사용자 지정할 수 있는 모듈 쌍 속성 집합을 사용 하 여 제어할 수 있습니다. 일반적으로 이러한 속성에 대 한 보조 업데이트는 자주 발생 하지 않습니다.

Defender for IoT의 보안 에이전트 쌍 구성 개체는 JSON 형식 개체입니다. 구성 개체는 에이전트의 동작을 제어 하기 위해 정의할 수 있는 제어 가능한 속성 집합입니다.

이러한 구성을 통해 필요한 각 시나리오에 대 한 에이전트를 사용자 지정할 수 있습니다. 예를 들어 일부 이벤트를 자동으로 제외 하거나 이러한 속성을 구성 하 여 전원 소비를 최소 수준으로 유지할 수 있습니다.

IoT 용 Defender 보안 에이전트 구성 [스키마](https://aka.ms/iot-security-github-module-schema) 를 사용 하 여 변경 합니다.

## <a name="configuration-objects"></a>구성 개체

모든 IoT 보안 에이전트에 대 한 모든 Defender와 관련 된 속성은 **azureiotsecurity** 모듈의 desired 속성 섹션에 있는 에이전트 구성 개체에 있습니다.
...
IoT 용 Defender 보안 에이전트는 **azureiotsecurity** 모듈 쌍 id의 보고 된 속성 섹션 내에서 현재 구성을 보고 합니다.
...
1. Defender-마이크로 에이전트에서 변경 하려는 속성을 편집 합니다.
...
다음 표에서는 IoT 보안 에이전트에 대 한 Defender의 제어 가능 속성을 포함 합니다.
...
|----------|-|---------|----|----|
|진단 이벤트|eventPriorityDiagnostic| 꺼짐| 거짓| 에이전트 관련 진단 이벤트입니다. 자세한 정보 로깅에이 이벤트를 사용 합니다.|
...
|수신 대기 포트|eventPriorityListeningPorts |높은 |참 |장치에서 열려 있는 모든 수신 포트의 스냅숏입니다.|
...
|로컬 사용자| eventPriorityLocalUsers |높은 |참|시스템 내에서 등록 된 로컬 사용자의 스냅숏입니다. |
|로그인|  eventPriorityLogin |높은|거짓|로그인 이벤트를 장치 (로컬 및 원격 로그인)로 감사 합니다.|
...
- [IoT 권장 사항에 대 한 Defender 이해](concept-recommendations.md)
- [IoT 경고 용 Defender 살펴보기](concept-security-alerts.md)
