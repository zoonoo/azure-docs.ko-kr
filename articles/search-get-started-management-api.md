...
자습서를 완료하는 과정에서 두 가지 서비스, Azure 검색 및 Azure AD(Active Directory)가 프로비전됩니다. 또한 Azure에서 클라이언트 응용 프로그램과 리소스 관리자 끝점 간에 트러스트를 설정하는 AD 응용 프로그램을 만듭니다.

이 자습서를 완료하려면 Azure 계정이 있어야 합니다.


<h2 id="download">샘플 응용 프로그램 다운로드</h2>
...
<h3>이 서비스에 대한 새 AD 응용 프로그램 만들기</h3>
...
11. 사용 권한으로 아래로 스크롤하고 **Microsoft Azure 관리 API**를 선택합니다. 이 단계에서 필요한 액세스 수준과 함께 클라이언트 응용 프로그램이 액세스해야 하는 API(이 경우 Azure 리소스 관리자 API)를 지정합니다.
...
<h3>등록 및 구독 값과 함께 샘플 응용 프로그램 로드</h3>
...
아직 [Codeplex(linkTBD)에서 샘플 응용 프로그램을 다운로드]하지 않았으면 이 단계를 위해 다운로드해야 합니다.
