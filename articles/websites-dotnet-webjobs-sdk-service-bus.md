<properties 
	pageTitle="WebJobs SDK를 사용하여 Azure 서비스 버스로 작업하는 방법" 
...
**참고:** WebJobs SDK를 사용하지 않는 응용 프로그램에서 큐 메시지를 만드는 경우 [BrokeredMessage.ContentType](http://msdn.microsoft.com/ko-kr/library/microsoft.servicebus.messaging.brokeredmessage.contenttype.aspx)을 "text/plain"으로 설정해야 합니다.
...
POCO 유형 매개 변수의 경우 함수가 종료되면 큐 메시지가 항상 만들어집니다. 매개 변수가 null인 경우에는 SDK에서 메시지가 수신되고 deserializ될 때 null을 반환하는 큐 메시지를 만듭니다. 다른 유형의 경우 매개 변수가 null이면 큐 메시지가 만들어지지 않습니다.
...
서비스 버스에 특정하지 않은 WebJobs SDK 시나리오에 대한 자세한 내용은 [WebJobs SDK를 사용하여 Azure 큐 저장소로 작업하는 방법](../websites-dotnet-webjobs-sdk-storage-queues-how-to/). 을 참조하세요.
