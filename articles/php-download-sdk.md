...
Azure용 PHP 클라이언트 라이브러리를 사용하는 방법에 대한 내용은 [Blob 서비스 사용 방법][blob-service], [테이블 서비스 사용 방법][table-service] 및 [큐 서비스 사용 방법][queue-service]을 참조하십시오.
...
Azure CLI는 Azure 웹 사이트 및 Azure 가상 컴퓨터와 같은 Azure 서비스를 배포 및 관리하기 위한 명령 집합입니다. 다음 목록에서는 운영 체제에 따른 Azure CLI 설치 방법에 대해 설명합니다.
...
* **Linux**: 최신 버전의 [Node.js][nodejs-org]를 설치한 후([패키지 관리자를 통해 Node.js 설치][install-node-linux] 참조) 다음 명령을 실행합니다.
	...
	> [AZURE.NOTE]이 명령은 상승된 권한으로 실행해야 할 수도 있습니다.`sudo npm install azure-cli -g`


Azure CLI을 사용하는 방법에 대한 정보는 [Mac, Linux 및 Windows용 Azure 명령줄 인터페이스 사용 방법][azure cli]을 참조하십시오.
