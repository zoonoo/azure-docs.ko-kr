...
2. **도메인 선택** 드롭다운 메뉴에서 **육지**옵션을 선택 합니다.
3. **사용량 선택** 드롭다운 메뉴에서 **교육**옵션을 선택 합니다.
4. **국가 선택** 드롭다운 메뉴에서 국가를 선택 합니다.
5. 등록 **을 선택 하** 여 등록 프로세스를 완료 합니다.
...
**사례 1**: 설치 관리자에서 원하는 구독 내에 Azure active directory 응용 프로그램 등록을 만들 수 있는 권한이 있다고 가정 합니다. 등록 하려면 포털에 로그인 하 고 **Azure active directory** > **앱 등록** > **새 등록**으로 이동 합니다.
...
1. 등록 [스크립트](https://aka.ms/FarmBeatsAADScript)를 가져옵니다.
...
6. Cloud Shell에서 아래 명령을 입력 하 여 스크립트를 실행 합니다.
...
1. **Azure Portal** 에 로그인 하 고 오른쪽 위 모서리에서 계정을 선택 하 고 Microsoft Azure FarmBeats를 배포 하려는 Azure AD 테 넌 트로 전환 합니다.
2. **검색/마켓플레이스** 또는 **리소스 만들기**를 선택 합니다. 제품 세부 정보를 보려면 **FarmBeats** 를 입력 합니다. 다음 단계를 진행 하기 전에이 페이지에서 aka.ms 하이퍼링크를 통해 사용할 수 있는 가이드를 다운로드 합니다.
3. **만들기** 를 선택 하 고 다음 정보를 입력 합니다.

   - 구독 이름을 입력 합니다.
   - 기존 리소스 그룹 이름 (빈 리소스 그룹에만 해당)을 입력 하거나 Azure FarmBeats를 배포 하기 위한 새 리소스 그룹을 만듭니다. 이후 단계에서 입력. json 파일 내에 입력할이 리소스 그룹을 기록해 둡니다.
   - Azure FarmBeats를 설치 하려는 지역을 입력 합니다. FarmBeats는 현재 미국 중부, 유럽 서부, 미국 동부 2, 서유럽, 미국 서 부, 동남 아시아, 미국 동부, 오스트레일리아 동부, 미국 서 부 2 지역에 설치 될 수 있습니다.
4. **확인**을 선택 하면 사용 약관 페이지로 리디렉션됩니다. 표준 마켓플레이스 용어를 검토 하거나 하이퍼링크를 선택 하 여 사용 약관을 검토 합니다.
5. **닫기**를 선택 하 고 "동의 함" 확인란을 선택 하 고 **만들기**를 선택 합니다.
6. 이제 marketplace에서 Azure FarmBeats의 EULA (최종 사용자 사용권 계약)에 성공적으로 서명 했습니다. 배포를 계속 하려면이 가이드의 다음 단계를 수행 합니다.

### <a name="prepare-inputjson-file"></a>Input. Json 파일 준비

이 파일은 Azure Cloud Shell 하는 입력 파일 이며 업로드 하기 전에이 파일 내에 값이 지정 된 매개 변수는 배포 중에 메시지가 표시 되지 않으므로 시간을 절약할 수 있습니다.  
...
|"datahubResourceGroup"  | 데이터 허브 리소스의 리소스 그룹 이름입니다.|
|"datahubLocation" | 데이터 허브 리소스를 저장할 위치입니다.|
|"acceleratorWebsiteName"  |데이터 허브의 이름을 위한 고유한 URL 접두사
Swagger 웹 사이트. 기본값은 데이터 허브 리소스 그룹 이름입니다. 기본값을 계속 하려면 enter 키를 누릅니다.|
|"acceleratorResourceGroup"  | 데이터 허브 리소스의 리소스 그룹 이름입니다. |
|"acceleratorLocation"  | 데이터 허브 리소스를 저장할 위치
"acceleratorWebsiteName"  | 액셀러레이터 웹 사이트의 이름을 위한 고유한 URL 접두사입니다. 기본값은 accelerator입니다. 기본값을 계속 하려면 enter 키를 누릅니다.|
|''sentinelUsername'' | 로그인 할 사용자 이름: https://scihub.copernicus.eu/dhus/#/self-registration.|
|"sentinelPassword"  | 로그인 할 암호: https://scihub.copernicus.eu/dhus/#/self-registration.|
|"farmbeatsAppId"  | 팀 Azure FarmBeats에서 공유할 값입니다.  |
|"farmbeatsPassword"  | 팀 Azure FarmBeats에서 공유할 값입니다.|
|"notificationEmailAddress"  | 데이터 허브 내에서 구성 하는 경고에 대 한 알림을 받을 전자 메일 주소입니다.|
|"upda" aadAppClientId ""  |[**선택 사항**] Azure AD 앱이 이미 있는 경우에만 Input. Json에 포함 될 매개 변수입니다.  -True/False. 처음 설치 하는 경우 False이 고 업그레이드 시나리오의 경우 True입니다.|
...
Marketplace 워크플로의 일부로 하나의 리소스 그룹을 만들고 최종 사용자 사용권 계약에 서명 했습니다. 그러면 실제 배포의 일부로 다시 검토할 수 있습니다. 배포는 Bash 환경을 사용 하 여 Azure Cloud Shell (브라우저 기반 명령줄)를 통해 수행 됩니다.  

> [!NOTE]
> 비활성 Cloud Shell 세션은 20 분 후에 만료 됩니다. 이 시간 내에 배포를 완료 해 보세요.

1. **Azure** portal에 로그인 하 고 원하는 구독 및 AD 테 넌 트를 선택 합니다.
2. **Azure** portal의 위쪽 탐색 모음에서 **Cloud Shell** 를 시작 합니다.

   ![프로젝트 팜 비트](./media/prepare-for-deployment/navigation-bar-1.png)

3. 구독을 선택하여 스토리지 계정 및 Microsoft Azure Files 공유를 만듭니다.
4. **스토리지 만들기**를 선택합니다.
5. 셸 창 왼쪽에서 환경 드롭다운 (Bash)을 선택 합니다.

   ![프로젝트 팜 비트](./media/prepare-for-deployment/bash-1-1.png)

### <a name="deployment-scenario-1"></a>배포 시나리오 1

설치 관리자가 Azure AD를 만듭니다 (AD 테 넌 트 읽기 권한 보유).  

1. [Input. json](https://aka.ms/PPInputJsonTemplate) 템플릿을 다운로드 합니다. Input. json 파일에 Azure 애플리케이션 클라이언트 ID와 암호를 포함 하 고 저장 합니다.
2. 다운로드 한 파일을 메모장에서 열고 값을 입력 하 여 파일을 채웁니다.
    ...
    > [!NOTE]
    > 이 단계를 건너뛸 수 있으며 런타임에 입력에 프롬프트가 표시 됩니다.

3. 파일을 저장 하 고 로컬 컴퓨터에 있는 경로를 적어 둡니다.  
4. Azure Cloud Shell으로 이동 하 고 인증을 완료 한 후 업로드 (아래 이미지의 강조 표시 된 아이콘 참조)를 선택 하 고 입력. json 파일을 Cloud Shell 저장소에 업로드 합니다. 설치 관리자가 Azure AD 앱 등록을 만들고 구성 하므로 json 내에서 Azure AD 매개 변수를 전달할 필요가 없습니다.

   ![프로젝트 팜 비트](./media/prepare-for-deployment/bash-2-1.png)

5. "배포 명령"을 Cloud Shell에 입력 하거나 붙여 넣습니다. 입력 경로를 수정 해야 합니다. Json 파일을 입력 하 고 enter 키를 누릅니다.  

    ```
    wget -N -O farmbeats-installer.sh https://aka.ms/FB_1.2.0 && bash farmbeats-installer.sh> /home/dummyuser/input.json

    ```
   이 스크립트는 모든 종속성을 자동으로 다운로드 하 고 배포자를 작성 합니다. 그런 다음 Azure FarmBeats EULA (최종 사용자 사용권 계약)에 동의 하 라는 메시지가 표시 됩니다.

   - 동의 하면 ' Y '를 입력 하 고 다음 단계를 진행 합니다.
   - 조건에 동의 하지 않는 경우 ' N '을 입력 하면 배포가 종료 됩니다.

   그런 다음 배포에 대 한 액세스 토큰을 입력 하 라는 메시지가 표시 됩니다. 생성 된 코드를 복사 하 고 Azure 자격 증명을 사용 하 여 https://microsoft.com/devicelogin 로그인 합니다.

   > [!NOTE]
   > 코드는 60 분 후에 만료 됩니다. 만료 되 면 배포 명령을 다시 입력 하 여 다시 시작할 수 있습니다.

6. 다음 프롬프트에서 센티널 계정 암호를 입력 합니다.
7. 이제 설치 관리자는 유효성을 검사 하 고 배포를 시작 합니다 .이는 약 20 분 정도 걸릴 수 있습니다.
8. 배포가 성공적으로 완료 되 면 아래 출력 링크를 받게 됩니다.
    - **데이터 허브 URL**: Swagger 링크를 통해 Azure FarmBeats api를 사용해 보세요.
    - **ACCELERATOR URL**: Azure FarmBeats 스마트 팜 가속기를 탐색 하기 위한 사용자 인터페이스입니다.
    - **배포자 로그 파일**-배포 중에 생성 되는 로그 파일입니다. 문제를 해결 하는 데 사용할 수 있습니다.

    - 동의 하면 ' Y '를 입력 하 고 다음 단계를 계속 합니다.
    - 조건에 동의 하지 않고 배포가 종료 되는 경우 ' N '을 입력 합니다.

   그런 다음 배포에 대 한 액세스 토큰을 입력 하 라는 메시지가 표시 됩니다. 생성 된 코드를 복사 하 고 Azure 자격 증명을 사용 하 여 https://microsoft.com/devicelogin에 로그인 합니다.

   > [!NOTE]
    > 이러한 정보를 적어 두고 나중에 사용할 배포 로그 파일 경로를 유지 합니다.


### <a name="deployment-scenario-2"></a>배포 시나리오 2

기존 Azure Active Directory 앱 등록을 사용 하 여를 배포 합니다.

1. [입력 json](https://aka.ms/PPInputJsonTemplate) 다운로드 AZURE 애플리케이션 클라이언트 ID와 암호를 입력. json에 포함 하 고 저장 합니다.
...
4. *배포 명령을* Cloud Shell에 입력 하거나 붙여 넣습니다. 입력 경로를 수정 해야 합니다. Json 파일을 입력 하 고 enter 키를 누릅니다.
...
5. 이 스크립트는 모든 종속성을 자동으로 다운로드 하 고 배포자를 작성 합니다.
6. Azure FarmBeats EULA (최종 사용자 사용권 계약)를 읽고 동의 하 라는 메시지가 표시 됩니다.

   - 동의 하면 ' Y '를 입력 하 고 다음 단계를 계속 합니다.
   - 조건에 동의 하지 않고 배포가 종료 되는 경우 ' N '을 입력 합니다.

7. 배포에 대 한 액세스 토큰을 입력 하 라는 메시지가 표시 됩니다. 생성 된 코드를 복사 하 고 Azure 자격 증명을 사용 하 여 https://microsoft.com/devicelogin에 로그인 합니다.
8. 이제 설치 관리자에서 리소스 만들기의 유효성을 검사 하 고 시작 합니다 .이는 약 20 분 정도 걸릴 수 있습니다. 이 시간 동안 Cloud Shell에서 세션을 활성 상태로 유지 합니다.
9. 배포가 성공적으로 완료 되 면 아래 출력 링크를 받게 됩니다.
   - **데이터 허브 URL**: Swagger 링크 FarmBeats api를 시도 합니다.
   - **ACCELERATOR URL**: FarmBeats 스마트 팜 가속기를 탐색 하기 위한 사용자 인터페이스입니다.
   - **배포자 로그 파일**: 배포 하는 동안 로그를 저장 하 고 문제 해결에 사용할 수 있습니다.
...
데이터 허브 설치가 완료 되 면 다음 형식으로 Swagger 인터페이스를 통해 Azure FarmBeats Api에 액세스 하는 URL을 받게 됩니다. https://\<원하는 datahub-웹 사이트-이름 >.
...
가속기 설치가 완료 되 면 FarmBeats 사용자 인터페이스에 대 한 액세스 URL을 https://\<액셀러레이터-웹 사이트 이름 >.
...
3. 저장소 계정 및 Azure Files 파일 공유를 만들 구독을 선택 합니다.
4. **스토리지 만들기**를 선택합니다.
5. 셸 (Bash)의 왼쪽에 있는 드롭다운에서 환경을 선택 합니다.
6. 필요한 경우에만 입력 된 json 파일을 변경 하 고 Azure Cloud Shell 업로드 합니다. 예를 들어 수신 하려는 알림에 대 한 전자 메일 주소를 업데이트할 수 있습니다.
7. 로컬 컴퓨터의 입력. json 파일 경로를 기록해 둡니다.
8. Azure Cloud Shell로 이동 합니다. 성공적으로 인증 되 면 업로드 단추 (아래 이미지의 강조 표시 된 아이콘 참조)를 선택 하 고 입력. json 파일을 Cloud Shell 저장소에 업로드 합니다.

   ![프로젝트 팜 비트](./media/prepare-for-deployment/bash-2-1.png)

9. **배포 명령을** Cloud Shell에 입력 하거나 붙여 넣습니다. 입력 경로를 수정 해야 합니다. Json 파일을 입력 하 고 enter 키를 누릅니다.
...
10. 설치 관리자는 런타임에 필요한 입력을 자동으로 표시 합니다.

    - 배포에 대 한 액세스 토큰을 입력 합니다. 생성 된 코드를 복사 하 고 Azure 자격 증명을 사용 하 여 https://microsoft.com/devicelogin에 로그인 합니다.
...
11. 이제 설치 관리자는 리소스의 유효성을 검사 하 고 리소스 만들기를 시작 합니다 .이는 약 20 분 정도 걸릴 수 있습니다.
12. 배포가 성공적으로 완료 되 면 아래 출력 링크를 받게 됩니다.
    ...
    > [!NOTE]
    > 위의 링크를 적어 두고 나중에 사용 하기 위해 배포 로그 파일 경로를 편리 하 게 유지 합니다.

## <a name="uninstall"></a>제거

현재 설치 관리자를 사용 하 여 FarmBeats의 자동 제거를 지원 하지 않습니다. 데이터 허브 또는 가속기를 제거 하려면 Azure Portal에서 이러한 구성 요소가 설치 된 리소스 그룹을 삭제 하거나 리소스를 수동으로 삭제 합니다.

예를 들어 두 개의 다른 리소스 그룹에 데이터 허브 및 가속기를 배포한 경우 다음과 같이 해당 리소스 그룹을 삭제 합니다.

1. Azure Portal에 로그인합니다.
2. 오른쪽 위 모서리에서 사용자의 계정을 선택 하 고 Microsoft FarmBeats를 배포 하려는 Azure AD 테 넌 트로 전환 합니다.
...
4. 해당 리소스 그룹 이름을 선택합니다. 이름을 다시 입력 하 여 다시 확인 하 고 삭제를 클릭 하 여 리소스 그룹 및 해당 하는 모든 기본 리소스를 제거 합니다.
