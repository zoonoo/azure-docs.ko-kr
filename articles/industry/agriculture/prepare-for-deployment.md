...
|S.No  |리소스 이름  |Azure FarmBeats Component  |
|---------|---------|---------|
...
|6 |      EventHub Namespace    |  데이터 허브       |
|7  |    Azure Data Factory V2       |     Data hub/ Accelerator      |
|8  |Batch 계정    |데이터 허브   |
|9  |       Storage 계정     |  Data hub/ Accelerator        |
|10  |    논리 앱        |     데이터 허브      |
|11  |    API connection        |     데이터 허브      |
|12|      App Service      |  Data hub/Accelerator       |
|13 |    앱 서비스 계획        |     Data hub/ Accelerator      |
|14 |Azure Maps account     |액셀러레이터    |
...
https://scihub.copernicus.eu/dhus/#/self-registration 로 이동합니다. In the registration page, provide a first name, last name, username, password, and email.
...
**Case 2**: This method is the preferred step when you don't have enough rights to create and configure an Azure AD app registration within your subscription. Request your  admin to use the [custom script](https://aka.ms/FarmBeatsAADScript), which will help IT admin automatically generate and configure the Azure AD app registration on the Azure portal. As an output to running this custom script using PowerShell environment the IT admin needs to share an Azure Active Directory Application Client ID and password secret with you. 이러한 값을 기록해 둡니다.
...
4. First-time users will be prompted to select a subscription to create a storage account and Microsoft Azure Files share. **스토리지 만들기**를 선택합니다.
...
The steps for upgrade are similar to the first-time installation. 다음 단계를 수행하세요.
...
4. 해당 리소스 그룹 이름을 선택합니다. Type in the name again to double-check, and select Delete to remove the resource group, and all its underlying resources.
