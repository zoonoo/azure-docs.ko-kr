...
- **센서** 센서 센서 센서는 값을 기록 하는 실제 센서에 해당 합니다. / 센서는 일반적으로 장치 ID를 사용 하 여 장치에 연결 됩니다.
  ...
  이름  | 리소스를 식별 하는 이름입니다. 예: 모델 이름/제품 이름 |
...
ParentDeviceId | 이 장치가 연결 된 부모 장치의 ID입니다. 예제: 게이트웨이에 연결 된 노드입니다. 노드가 parentDeviceID를 게이트웨이로 포함 합니다. |
  이름  | 리소스를 식별 하는 이름입니다.  장치 파트너는 장치 파트너 측의 장치 이름과 일치 하는 이름을 보내야 합니다. 장치 이름이 장치 파트너 쪽에서 사용자 정의 이면 동일한 사용자 정의 이름을 FarmBeats에 전파 해야 합니다.  |
  설명  | 의미 있는 설명 제공  |
  속성  |제조업체의 추가 속성  |
