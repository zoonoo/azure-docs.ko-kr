...
파트너는이 구성 요소를 사용 하 여 FarmBeats와 통합 되어 API를 사용 하 고 고객 장치 데이터 및 원격 분석을 FarmBeats Datahub로 보내는 센서를 개발할 수 있습니다. FarmBeats Accelerator를 사용 하 여 데이터를 시각화 합니다. 데이터를 사용 하 여 데이터를 fusion 하 고 기계 학습/인공 지능 모델을 빌드할 수 있습니다.

## <a name="link-a-farmbeats-account"></a>FarmBeats 계정 연결

장치 또는 센서를 구입 하 고 배포한 후에는 장치 파트너의 SaaS (software as a service) 포털에서 장치 데이터 및 원격 분석에 액세스할 수 있습니다. 장치 파트너를 사용 하 여 Azure에서 FarmBeats 인스턴스에 계정을 연결할 수 있습니다. 사용자 또는 시스템 통합자는 다음 자격 증명을 입력 해야 합니다.
...
기존 FarmBeats 통합의 연결을 해제할 수 있습니다. FarmBeats 연결을 해제 하면 데이터 허브에 생성 된 모든 장치 또는 센서 메타 데이터를 삭제할 필요가 없습니다. 연결 끊기는 다음을 수행 합니다.
...
클라이언트 암호 또는 연결 문자열이 변경 되는 경우 FarmBeats 통합 설정을 편집할 수 있습니다. 이 경우 다음 필드만 편집할 수 있습니다.
...
전송 된 마지막 원격 분석의 타임 스탬프를 볼 수 있습니다 .이 타임 스탬프는 **전송 된 원격**분석에서 찾을 수 있습니다. 최신 원격 분석이 성공적으로 FarmBeats에 전송 된 시간입니다.
...
- /**센서**: 센서는 값을 기록 하는 실제 센서에 해당 합니다. 센서는 일반적으로 장치 ID를 사용 하 여 장치에 연결 됩니다.
  ...
  **디바이스** |  |
  ...
  name  | 리소스를 식별 하는 이름입니다. 장치 파트너는 장치 파트너 측의 장치 이름과 일치 하는 이름을 보내야 합니다. 장치 이름이 장치 파트너 쪽에서 사용자 정의 된 경우 동일한 사용자 정의 이름을 FarmBeats에 전파 해야 합니다.  |
  설명  | 의미 있는 설명을 제공 합니다.  |
  properties  |제조업체의 추가 속성입니다.  |
  **SensorModel** |  |
...
지정 된 FarmBeats 인스턴스에서 장치 데이터 또는 원격 분석을 받을 수 없는 경우 장치 파트너는 지원 및 문제 해결 메커니즘을 제공 해야 합니다.
...
장치가 장치 또는 센서 메타 데이터 또는 장치 파트너 시스템의 데이터 통합 또는 원격 분석 데이터 흐름에 영향을 주는 경우 알림을 받게 됩니다. 오류를 해결 하는 메커니즘을 설계 하 고 구현 해야 합니다.
