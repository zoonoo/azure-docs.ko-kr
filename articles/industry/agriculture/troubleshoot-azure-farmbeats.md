...
**시정 조치** 이 장치에 대해 보고 간격이 구성되지 않았습니다. 보고 간격을 설정하려면 장치 제조업체에 문의하십시오. 
...
2. 특정 장치를 삭제합니다.  
    ...
    > 파트너는 장치 또는 센서를 삭제할 권한이 없습니다. 관리자만 삭제할 수 있습니다.
...
**시정 조치** 이 문제는 데이터 파이프라인의 일시적인 오류로 인해 발생할 수 있습니다. 작업을 다시 만듭니다. 오류가 지속되면 오류 메시지/로그와 함께 문의하십시오.
...
**시정 조치** 역할 할당을 추가하려는 전자 메일 ID를 확인합니다. 전자 메일 ID는 Active Directory에서 해당 사용자에 대해 등록된 ID와 정확히 일치해야 합니다. 오류가 지속되면 오류 메시지/로그와 함께 문의하십시오.

### <a name="unable-to-log-in-to-accelerator"></a>액셀러레이터에 로그인할 수 없음

**메시지**: "오류: 서비스를 호출할 권한이 없습니다. 관리자에게 권한을 요청하십시오."

**시정 조치** 관리자에게 FarmBeats 배포에 액세스할 수 있는 권한을 부여하도록 요청합니다. 이 작업은 역할 할당 API의 POST를 수행하거나 가속기의 **설정** 창에서 액세스 제어를 통해 수행할 수 있습니다.  

이미 액세스 권한이 부여되어 이 오류에 직면한 경우 페이지를 새로 고쳐 다시 시도하십시오. 오류가 지속되면 오류 메시지/로그와 함께 문의하십시오.
...
**시정 조치** 이 오류는 페이지를 너무 오래 유휴 상태로 두면 발생합니다. 페이지를 새로 고칩니다. 오류가 지속되면 오류 메시지/로그와 함께 문의하십시오.

**문제**: FarmBeats 가속기는 FarmBeatsDeploy를 업그레이드 한 후에도 최신 버전을 표시하지 않습니다.

**시정 조치** 이 오류는 브라우저에서 서비스 작업자 지속성 으로 인해 발생 합니다. 다음을 수행합니다.
...
- 아래 단계를 사용하여 올바른 사용자 이름/암호로 FarmBeats를 업데이트하고 작업을 다시 시도합니다.

  **센티넬 사용자 이름 업데이트**

    1. [Azure 포털](https://portal.azure.com)에 로그인합니다.
    2. **검색** 상자에서 FarmBeats Datahub 리소스 그룹을 검색합니다.
    3. 저장소 계정 저장소***** > **컨테이너** > **일괄 준비 파일** > **to_vm** > **config.ini**
    4. **편집** 선택
  ...
  **센티넬 암호 업데이트**
    ...
    4. 설정에서 액세스 정책 선택
    5. **액세스 추가 정책** 선택
    6. 템플릿에서 구성하기 위해 **비밀 관리를** 사용하고 보안 주체에 자신을 추가합니다.
    7. **에 추가를**선택한 다음 **액세스 정책** 페이지에서 **저장을** 선택합니다.
    8. **설정에서** **비밀** 선택
    9. **센티넬 암호** 선택
    10. 값의 새 버전을 만들고 활성화합니다.
...
3. 실패한 작업을 다시 실행하거나 5~7일 의 날짜 범위에서 위성 인덱스 작업을 실행한 다음 작업이 성공했는지 확인합니다.  
...
2. 실패한 작업을 다시 실행하거나 5~7일 의 날짜 범위에서 위성 인덱스 작업을 실행한 다음 작업이 성공했는지 확인합니다.
...
1. 다른 날짜 범위에 대한 작업을 다시 실행하고 위성 인덱스의 NDVI 값이 0.3보다 작은지 확인합니다.
