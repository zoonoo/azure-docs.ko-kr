<properties pageTitle="Windows Phone 클라이언트에서 사용자 지정 API 호출 - 모바일 서비스" description="사용자 지정 API를 정의한 다음 Azure 모바일 서비스를 사용하는 Windows Phone 앱에서 이를 호출하는 방법에 대해 알아봅니다." services="mobile-services" documentationCenter="windows" authors="ggailey777" manager="dwrede" editor=""/>
...
이 자습서는 간단한 TodoList 앱인 GetStartedWithData 샘플을 기반으로 합니다. 이 자습서를 시작하려면 먼저 [기존 앱에 모바일 서비스 추가](/ko-kr/documentation/articles/mobile-services-windows-phone-get-started-data/)를 완료해야 합니다.
