...
## <a name="1-attestation-provider"></a>1. 증명 공급자

### <a name="11-create-an-attestation-provider"></a>1.1 증명 공급자 만들기

#### <a name="111-to-configure-the-provider-with-unsigned-policies"></a>1.1.1 서명되지 않은 정책을 사용하여 공급자를 구성하려면
...
#### <a name="112-to-configure-the-provider-with-signed-policies"></a>1.1.2 서명된 정책을 사용하여 공급자를 구성하려면
...
5.  [증명 공급자 만들기] 페이지에서 다음 정보를 제공합니다.
    ...
    e. **정책 서명자 인증서 파일**: 정책 서명 인증서를 사용하여 증명 공급자를 구성하려면 인증서 파일을 업로드합니다. [여기](./policy-signer-examples.md)에 있는 예제를 참조하세요. 
6.  필요한 입력이 제공되었으면 **검토 + 만들기** 를 클릭합니다.
7.  유효성 검사 문제가 있는 경우 수정하고, **만들기** 를 클릭합니다.

### <a name="12-view-attestation-provider"></a>1.2 증명 공급자 보기

1.  Azure Portal 메뉴 또는 홈페이지에서 **모든 리소스** 를 선택합니다.
2.  필터 상자에서 증명 공급자 이름을 입력하고 선택합니다.

### <a name="13-delete-attestation-provider"></a>1.3 증명 공급자 삭제
...
## <a name="2-attestation-policy-signers"></a>2. 증명 정책 서명자

### <a name="21-view-policy-signer-certificates"></a>2.1 정책 서명자 인증서 보기
...
### <a name="22-add-policy-signer-certificate"></a>2.2 정책 서명자 인증서 추가
...
6.  정책 서명자 인증서 파일을 업로드하고, **추가** 를 클릭합니다. [여기](./policy-signer-examples.md)에 있는 예제를 참조하세요.

### <a name="23-delete-policy-signer-certificate"></a>2.3 정책 서명자 인증서 삭제
...
6.  정책 서명자 인증서 파일을 업로드하고, **삭제** 를 클릭합니다. [여기](./policy-signer-examples.md)에 있는 예제를 참조하세요. 

## <a name="3-attestation-policy"></a>3. 증명 정책

### <a name="31-view-attestation-policy"></a>3.1 증명 정책 보기
...
### <a name="32-configure-attestation-policy"></a>3.2 증명 정책 구성

#### <a name="321-when-attestation-provider-is-created-without-policy-signing-requirement"></a>3.2.1 정책 서명 요구 사항을 사용하지 않고 증명 공급자를 만든 경우
...
8.  정책 콘텐츠가 포함된 정책 파일을 **서명되지 않은/서명된 JWT** 형식으로 업로드하고, **저장** 을 클릭합니다. [여기](./policy-examples.md)에 있는 예제를 참조하세요.
...
8.  콘텐츠가 포함된 정책 파일을 **텍스트** 형식으로 업로드하거나, 텍스트 영역에서 정책 콘텐츠를 입력하고 **저장** 을 클릭합니다. [여기](./policy-examples.md)에 있는 예제를 참조하세요.

    파일 업로드 옵션의 경우 정책 미리 보기는 텍스트 형식으로 표시되며 이를 편집할 수 없습니다.

8.  **새로 고침** 을 클릭하여 구성된 정책을 확인합니다.

#### <a name="322-when-attestation-provider-is-created-with-policy-signing-requirement"></a>3.2.2 정책 서명 요구 사항을 사용하여 증명 공급자를 만든 경우
...
7.  정책 파일을 **서명된 JWT 형식** 으로 업로드하고, **저장** 을 클릭합니다. [여기](./policy-examples.md)에 있는 예제를 참조하세요.
