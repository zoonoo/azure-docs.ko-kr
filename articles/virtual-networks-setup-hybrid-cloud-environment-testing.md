...
RRAS1의 관리자 수준 Windows PowerShell 명령 프롬프트에서 다음 명령을 실행합니다. 이러한 명령을 실행하기 전에 변수 값을 작성하고 < 및 > 문자를 제거합니다. 네트워크 어댑터의 현재 이름은 **Get-NetAdapter** 명령 표시에서 가져올 수 있습니다.
...
2.	가상 네트워크 세부 정보 페이지에서 **이름**에 **TestVNET**을 입력합니다.
3.	**위치**에서 현재 위치의 적절한 데이터 센터를 선택합니다.
4.	다음 화살표를 클릭합니다.
5.	DNS 서버 및 VPN 연결 페이지의 **DNS 서버**에서 **이름 선택 또는 입력**에 **DC1**을 입력하고 **IP 주소에** **10.0.0.1**을 입력한 다음 **사이트 간 VPN 구성**을 선택합니다.
...
8.	사이트 간 연결 페이지에서 다음을 수행합니다.
	...
	- **주소 공간**의 **시작 IP** 열에 **10.0.0.0**을 입력합니다. **CIDR(주소 수)**에서 **/8**을 선택하고 **주소 공간 추가**를 클릭합니다.
9.	다음 화살표를 클릭합니다.
...
이제 [Azure PowerShell을 설치 및 구성하는 방법](install-configure-powershell.md)의 지침을 사용하여 로컬 컴퓨터에 Azure PowerShell을 설치합니다.

그런 다음 TestVNET 가상 네트워크에 대한 새 클라우드 서비스를 만듭니다. 고유한 이름을 선택해야 합니다. 예를 들어 이름을 TestVNET-*UniqueSequence*로 지정할 수 있습니다(여기서 *UniqueSequence*는 조직의 약어). 예를 들어 조직의 이름이 Tailspin Toys인 경우 클라우드 서비스 이름을 TestVNET-Tailspin으로 지정할 수 있습니다.
...
1.	로컬 컴퓨터의 Azure 관리 포털에서 왼쪽 창에 있는 **네트워크**를 클릭한 다음 TestVNET의 **상태** 열이 **생성됨**으로 설정되어 있는지 확인합니다.
2.	**TestVNET**을 클릭합니다. 대시보드 페이지에 **게이트웨이가 만들어지지 않음** 상태가 표시되어야 합니다.
3.	작업 표시줄에서 **게이트웨이 만들기**를 클릭한 다음 **동적 라우팅**을 클릭합니다. 메시지가 나타나면 **예**를 클릭합니다. 게이트웨이가 완료되고 해당 상태가 **연결 중**으로 변경될 때까지 기다립니다. 몇 분 정도 걸릴 수 있습니다.
4.	대시보드 페이지에서 **게이트웨이 IP 주소**를 기록해 둡니다. 이는 TestVNET 가상 네트워크에 대한 Azure VPN 게이트웨이의 공용 IP 주소입니다. RRAS1을 구성하려면 이 IP 주소가 필요합니다.
5.	작업 표시줄에서 **키 관리**를 클릭한 다음 키 옆의 복사 아이콘을 클릭하여 클립보드에 복사합니다. 이 키를 문서에 붙여 넣고 저장합니다. RRAS1을 구성하려면 이 키 값이 필요합니다. 
...
이제 로컬 컴퓨터에서 Azure 관리 포털로 이동하여 TestVNET 가상 네트워크의 상태가 **연결됨**으로 표시될 때까지 기다립니다.

그런 다음 인터넷 위치에 대한 변환된 트래픽을 지원하도록 RRAS1을 구성합니다. RRAS1에서 다음을 수행합니다.

1.	시작 화면에서 **rras**를 입력하고 **라우팅 및 원격 액세스**를 클릭합니다. 
2.	콘솔 트리에서 서버 이름을 열고 **IPv4**를 클릭합니다.
3.	**일반**을 마우스 오른쪽 단추로 클릭한 후 **새 라우팅 프로토콜**을 클릭합니다.
4.	**NAT**를 클릭한 후 **확인**을 클릭합니다.
5.	콘솔 트리에서 **NAT**를 마우스 오른쪽 단추로 클릭하고 **새 인터페이스**, **Corpnet**을 차례로 클릭한 후 **확인**을 두 번 클릭합니다.
6.	**NAT**를 마우스 오른쪽 단추로 클릭하고 **새 인터페이스**, **인터넷**, **확인**을 차례로 클릭합니다.
7.	**NAT** 탭에서 **공용 인터페이스를 인터넷에 연결**을 클릭하고 **이 인터페이스에 NAT 사용**을 선택한 후 **확인**을 클릭합니다.
...
인터페이스 이름이 Ethernet이 아닌 경우 **Get-NetAdapter** 명령을 사용하여 인터페이스 이름을 확인합니다.
...
1.	Azure 관리 포털의 왼쪽 창에서 **가상 컴퓨터**를 클릭한 다음 DC2에 대한 **상태** 열에서 **실행 중**을 클릭합니다.
2.	작업 표시줄에서 **연결**을 클릭합니다. 
3.	DC2.rdp를 열라는 메시지가 나타나면 **열기**를 클릭합니다.
...
1.	Azure 관리 포털의 왼쪽 창에서 **네트워크**를 클릭한 후 **TestVNET**을 클릭합니다.
2.	**구성**을 클릭합니다.
3.	**DNS 서버**에서 **10.0.0.1** 항목을 제거합니다.
4.	**DNS 서버**에서 **DC2**가 포함된 항목을 이름으로 추가하고, **192.168.0.4**를 IP 주소로 추가합니다. 
...
1.	로컬 컴퓨터의 Azure 관리 포털 왼쪽 창에서 **네트워크**, **TestVNET**, **대시보드**를 차례로 클릭합니다.
2.	작업 표시줄에서 **게이트웨이 삭제**를 클릭합니다. 메시지가 나타나면 **예**를 클릭합니다. 게이트웨이가 삭제되고 해당 상태가 **게이트웨이가 만들어지지 않았습니다.**로 변경될 때까지 기다립니다.

게이트웨이를 삭제한 경우 테스트 환경을 복원하려면 먼저 새 게이트웨이를 만들어야 합니다.

1.	로컬 컴퓨터의 Azure 관리 포털 왼쪽 창에서 **네트워크**, **TestVNET**을 차례로 클릭합니다. 대시보드 페이지에 **게이트웨이가 만들어지지 않았습니다.** 상태가 표시되어야 합니다.
2.	작업 표시줄에서 **게이트웨이 만들기**를 클릭한 다음 **동적 라우팅**을 클릭합니다. 메시지가 나타나면 **예**를 클릭합니다. 게이트웨이가 완료되고 해당 상태가 **연결 중**으로 변경될 때까지 기다립니다. 몇 분 정도 걸릴 수 있습니다.
3.	대시보드 페이지에서 **게이트웨이 IP 주소**를 기록해 둡니다. 이는 TestVNET 가상 네트워크에 대한 Azure VPN 게이트웨이의 새 공용 IP 주소입니다. RRAS1을 다시 구성하려면 이 IP 주소가 필요합니다.
4.	작업 표시줄에서 **키 관리**를 클릭한 다음 키 옆의 복사 아이콘을 클릭하여 클립보드에 복사합니다. 이 키 값을 문서에 붙여 넣고 저장합니다. RRAS1을 다시 구성하려면 이 키 값이 필요합니다. 
