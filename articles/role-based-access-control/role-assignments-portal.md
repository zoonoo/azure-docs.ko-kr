...
역할 정의는 역할 할당에 사용되는 권한 컬렉션입니다. Azure에는 70개가 넘는 [기본 제공 역할](built-in-roles.md)이 있습니다. 다음 단계를 수행하여 포털에서 역할을 나열합니다.
...
액세스 권한을 관리할 때 누가, 어떤 수준에서, 어떤 액세스 권한을 갖고 있는지 확인하고 싶은 경우가 있습니다. 역할 할당을 나열하면 액세스 권한이 나열됩니다. 다음 단계에 따라 사용자에 대한 액세스를 나열하고 서로 다른 범위에서 액세스를 나열합니다.
...
1. 리소스 그룹을 선택하고 **액세스 제어(IAM)** 를 선택합니다.
   ...
   ID 및 액세스 관리라고도 하는 액세스 제어(IAM) 블레이드에서 이 리소스 그룹에 액세스할 수 있는 사용자를 볼 수 있습니다. 일부 역할의 범위는 **이 리소스**이지만 나머지는 다른 범위에서 **(상속)** 됩니다. 액세스는 리소스 그룹에 특별히 할당되거나 부모 구독에 할당된 내용에서 상속됩니다.
...
1. **액세스 제어(IAM)** 를 선택합니다.
    ...
    액세스 제어(IAM) 블레이드에서 이 구독 및 해당 역할에 대한 액세스 권한이 있는 사용자를 볼 수 있습니다.

    ![구독의 액세스 제어(IAM) 블레이드](./media/role-assignments-portal/subscription-access-control.png)
...
1. 선택한 관리 그룹에 대해 **(세부 정보)** 를 선택합니다.

    ![관리 그룹](./media/role-assignments-portal/management-groups-list.png)

1. **액세스 제어(IAM)** 를 선택합니다.

    액세스 제어(IAM) 블레이드에서 이 관리 그룹 및 해당 역할에 대한 액세스 권한이 있는 사용자를 볼 수 있습니다.

    ![관리 그룹의 액세스 제어(IAM) 블레이드](./media/role-assignments-portal/management-groups-access-control.png)
...
1. **액세스 제어(IAM)** 를 선택하면 구독 범위의 현재 역할 할당 목록을 볼 수 있습니다.
   ...
   ![관리 그룹의 액세스 제어(IAM) 블레이드](./media/role-assignments-portal/grant-management-groups-access-control.png)

1. **추가**를 선택하여 **권한 추가** 창을 엽니다.

   역할을 할당할 수 있는 권한이 없는 경우 **추가** 옵션이 표시되지 않습니다.

   ![권한 추가 창](./media/role-assignments-portal/add-permissions-management-groups.png)
...
1. 제거할 역할 할당이 있는 관리 그룹, 구독, 리소스 그룹 또는 리소스의 **액세스 제어(IAM)** 블레이드를 엽니다.
