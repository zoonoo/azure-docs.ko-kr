...
-   Windows 8에서 실행되는 [Windows Phone 8 SDK](http://go.microsoft.com/fwlink/p/?LinkID=268374)
-   [Azure 계정](http://www.windowsazure.com/ko-kr/pricing/free-trial/)
-   이 자습서는 모바일 서비스 quickstart를 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [모바일 서비스 시작하기](/en-us/develop/mobile/tutorials/get-started-wp8)를 완료해야 합니다.
...
1.  Visual Studio에서 [모바일 서비스 시작](/en-us/develop/mobile/tutorials/get-started-wp8) 자습서에서 다운로드한 TodoList 프로젝트를 엽니다.
...
3.  Visual Studio 솔루션 탐색기에서 MainPage.xaml.cs를 열고 다음 `using` 지시문을 추가합니다.
...
6.  왼쪽 에뮬레이터에서 마지막 TodoItem의 `text`를 **Test Write 1**로 업데이트한 다음 `LostFocus` 이벤트 처리기가 데이터베이스를 업데이트하도록 다른 텍스트 상자를 클릭합니다. 예제는 아래 스크린샷을 참조하십시오.
...
이 자습서에서는 모바일 서비스에서 데이터 작업 시 발생하는 쓰기 충돌을 처리하도록 Windows Phone 8 앱을 설정하는 방법을 알아보았습니다. 이제 다음 자습서 중 하나에서 데이터 시리즈를 완료하십시오.
...
[Azure Account]: http://www.windowsazure.com/ko-kr/pricing/free-trial/
