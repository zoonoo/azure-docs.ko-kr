...
템플릿 내에서 dependsOn 요소는 하나의 리소스를 하나 이상의 리소스에 종속된 것으로 정의할 수 있는 기능을 제공합니다. 값은 리소스 이름의 쉼표로 구분된 목록일 수 있습니다.

다음 예제에서는 부하 분산 장치, 가상 네트워크 및 여러 저장소 계정을 만드는 루프에 종속된 가상 컴퓨터 크기 집합을 보여 줍니다. 이러한 다른 리소스는 아래에 표시되어 있지 않지만 템플릿 내 다른 곳에 존재해야 합니다.
...
리소스와 copy 루프를 통해 만들어진 리소스 사이의 종속성을 정의해야 하는 경우(위 그림 참조) dependsOn 요소를 루프의 이름으로 설정할 수 있습니다. 예제는 [Azure 리소스 관리자에서 리소스의 여러 인스턴스 만들기](resource-group-create-multiple.md)를 참조하세요.
