...
2. **새로 만들기**를 선택합니다.
3. 블레이드 만들기에서 **계산** > **Azure 마켓플레이스**를 선택합니다.
4. 마켓플레이스 블레이드에서 **API 앱**을 선택하고 검색 창에서 SAP에 대해 검색합니다.
	
	![SAP Connector API App][2]	
5. Microsoft에서 게시한 **SAP 커넥터**를 선택합니다.
6. SAP 커넥터 블레이드에서 **만들기**를 선택합니다.
7. 열리는 새 블레이드에서 다음을 입력합니다.
	1. **위치** - 커넥터를 배포하려는 지리적 위치를 선택합니다.
	...
	7. **패키지 설정**
		- **서버 이름** - SAP 서버 이름을 입력합니다. 예제: "SAPserver" 또는 "SAPserver.mydomain.com"입니다.
		...
		- **온-프레미스** - SAP 서버가 방화벽 뒤의 온-프레미스인지 여부를 입력합니다. TRUE로 설정한 경우 SAP 서버에 액세스할 수 있는 서버에 수신기 에이전트를 설치해야 합니다. API 앱 통계 페이지로 이동하고 'Hybrid Connection'을 선택하여 에이전트를 설치할 수 있습니다.
...
## 하이브리드 수신기 설치
**찾아보기** > **API 앱** > *커넥터 이름*를 통해 만든 SAP 커넥터로 이동합니다.
...
커넥터 블레이드에서 하이브리드 연결 상태가  *Connected*인 것인지 확인할 수 있습니다.
