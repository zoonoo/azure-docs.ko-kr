...
* 더 오래 또는 오프라인 보고에 사용하도록 메트릭을 **보관**합니다. 리소스에 대한 진단 설정을 구성할 때 메트릭을 blob 저장소로 라우팅할 수 있습니다.
* 메트릭을 이벤트 허브로 **스트리밍한 다음** Azure Stream Analytics나 사용자 지정 앱으로 라우팅하여 거의 실시간에 가까운 분석이 가능합니다. 진단 설정을 사용할 수 있습니다.
* 모든 메트릭을 **Route** OMS Log Analytics로 라우팅하여 리소스로부터 받은 메트릭 데이터에 대한 인스턴스 분석, 검색 및 사용자 지정 경고를 잠금 해제합니다.
* 새로운 Azure Monitor REST API를 통해 메트릭을 **사용합니다**.
* PowerShell Cmdlet 또는 Cross-Platform REST API를 사용하여 메트릭을 **쿼리**합니다.
...
Azure Monitor REST API 사용에 대한 자세한 연습은 [Azure Monitor REST API 연습](../monitoring-and-diagnostics/monitoring-rest-api-walkthrough.md)을 참조하세요.
...
Resource Manager 템플릿, [PowerShell](../monitoring-and-diagnostics/insights-powershell-samples.md), [CLI](../monitoring-and-diagnostics/insights-cli-samples.md) 또는 [REST API](https://msdn.microsoft.com/library/dn931943.aspx)를 통해 이 항목을 구성할 수 있습니다.
...
### Autoscale
일부 Azure 리소스에서는 워크로드 처리를 위해 규모를 확대/축소하는 여러 인스턴스를 지원합니다. 자동 크기 조정은 앱 서비스(웹 앱), 가상 컴퓨터 규모 집합(VMSS) 및 기존 Cloud Services에 적용됩니다. 특정 메트릭이 지정한 임계값을 초과할 때 규모를 확대하거나 축소하도록 자동 크기 조정 규칙을 구성할 수 있습니다. 자세한 내용은 [자동 크기 조정 개요](../monitoring-and-diagnostics/monitoring-overview-autoscale.md)를 참조하세요.
...
모든 지원되는 서비스 및 메트릭의 상세 목록은 [Azure Monitor 메트릭 - 리소스 유형별 지원 메트릭](../monitoring-and-diagnostics/monitoring-supported-metrics.md)에서 확인할 수 있습니다.

## 다음 단계
이 문서에 있는 링크를 참조하세요. 또한 다음을 학습할 수 있습니다.

* [자동 크기 조정에 대한 공통 메트릭](../monitoring-and-diagnostics/insights-autoscale-common-metrics.md) 정보
* [경고 규칙을 만드는](../monitoring-and-diagnostics/insights-alerts-portal.md) 방법
