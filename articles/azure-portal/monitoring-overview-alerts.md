...
* **OMS 로그 분석** - 진단 로그를 Log Analytics로 라우팅할 수 있습니다. OMS에서는 메트릭, 로그 및 기타 경고 유형을 지원합니다. 자세한 내용은 [Log Analytics의 경고](../log-analytics/log-analytics-alerts.md)를 참조하세요.
* **Azure Monitor** - Azure Monitor에서는 메트릭 값과 활동 로그 이벤트 모두를 기반으로 한 경고가 가능합니다. Azure Monitor에는 [Azure Insights REST API](https://msdn.microsoft.com/library/dn931943.aspx)가 포함됩니다. [Azure 포털, PowerShell 또는 명령줄 인터페이스를 사용하여 경고 만들기](../monitoring-and-diagnostics/insights-alerts-portal.md)도 참조하세요.

## 경고 작업
트리거되면 다음을 수행하도록 경고를 구성할 수 있습니다.

* 서비스 관리자, 공동 관리자 및/또는 사용자가 지정한 추가 이메일 주소로 이메일 알림을 보냅니다.
* 추가 자동화 작업을 시작할 수 있게 webhook 호출
  
  ![경고를 설명합니다.](./media/monitoring-overview-alerts/AlertsOverviewResource3.png)

## 다음 단계
다음을 통해 경고 규칙에 대한 정보를 확인하고 구성할 수 있습니다.
