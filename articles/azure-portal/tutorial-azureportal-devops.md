...
Azure 플랫폼은 유연한 DevOps 워크플로를 담고 있습니다. 이 자습서에서는 Azure Portal의 기능을 활용하여 실행 중인 응용 프로그램을 개발, 테스트, 배포, 문제 해결, 모니터링 및 관리하는 방법을 알아봅니다. 이 자습서는 다음에 중점을 둡니다.
...
[Azure App Service](https://azure.microsoft.com/services/app-service/)를 사용하여 웹앱을 만들고 이 자습서의 뒷부분에서 사용할 수 있습니다. 실행 중인 Azure 환경에 소스 코드 리포지토리의 지속적인 배포를 처음부터 사용할 수 있습니다.
...
12. 리포지토리에 변경 내용을 커밋한 후에 포털 알림 영역에서 새 배포 시작을 확인합니다. 리포지토리에 커밋한 후에 변경 내용을 신속하게 표시하지 않으면 동기화를 클릭합니다.
    ...
    ![image12][image12]
13. 이 시점에서 앱 서비스에 대한 페이지를 사용해보고 로드하는 경우 403 오류가 나타날 수 있습니다. 이 예제에서는 index.htm 또는 default.html과 같은 파일 등 페이지에 일반적인 기본 문서가 설정되지 않았기 때문입니다. Azure Portal의 도구를 사용하여 이 문제를 신속하게 해결할 수 있습니다.  Azure Portal에서 설정 &gt; 응용 프로그램 설정을 차례로 선택합니다.
...
15. 필요에 따라 브라우저 URL을 새로 고쳐서 필요한 변경 내용을 확인할 수 있습니다. 이 경우에 간단한 텍스트가 페이지를 채웁니다. 리포지토리에 추가 변경 사항이 있을 때마다 새 자동 배포가 발생합니다.
    ...
    Azure Portal을 사용하여 연속 배포를 사용하면 쉽습니다. 더 복잡한 릴리스 파이프라인을 구축하고 기존 원본 제어와 연속 통합 시스템을 사용하여 다른 여러 가지 기술을 사용하여 Azure에 배포할 수도 있습니다(예: 자동화된 구축 및 릴리스 관리 시스템 활용).

## <a name="develop-and-test-an-app"></a>앱 개발 및 테스트
다음으로, 코드 기본을 변경하고 이러한 변경 내용을 신속하게 배포합니다. 또한 웹앱에 대한 일부 성능 테스트를 설정합니다.

1. Azure Portal의 탐색 창에서 App Service를 선택하고 해당 App Services를 찾습니다.
   
   ![image16][image16]
2. 도구를 클릭합니다.
   
   ![image17][image17]
3. 도구에서 개발 범주를 확인합니다. 여러 가지 유용한 도구를 사용하여 Azure Portal에서 나가지 않고 앱으로 작업할 수 있습니다. 콘솔을 클릭합니다.
...
15. 테스트가 실행되기 시작하면 상태를 모니터링할 수 있습니다.
    ...
    ![image33][image33]
16. 이 예제에서는 제한된 데이터를 분석하도록 작은 테스트를 실행하기도 하지만 다양한 메트릭을 확인할 뿐만 아니라 이 보기에서 테스트를 다시 실행할 수 있습니다. Azure Portal을 통해 웹 성능 테스트를 쉽게 만들고, 실행 및 분석할 수 있습니다. 아래 스크린샷은 성능 데이터를 표시합니다.
...
1. Azure Portal에서 웹앱에 대한 도구를 선택합니다.
...
5. 적절한 로그를 선택하여 잠재적인 문제를 진단할 수 있습니다. 로깅을 사용하도록 설정하여 HTTP 로그 등 사용 가능한 데이터 옵션을 모두 확인해야 합니다.
   ...
   ![image43][image43]
6. 더 많은 데이터를 보려면 추가 로깅을 사용해야 합니다. Azure Portal에서 웹앱으로 이동하고 설정을 선택합니다.
...
11. HTML 파일 로그를 클릭하여 추가로 조사할 풍부한 브라우저 기반 보고서를 생성합니다.
    ...
    ![image49][image49]
12. 앱에 대한 Azure Portal의 도구 섹션으로 다시 이동합니다. 도구 섹션으로 스크롤한 다음 프로세스 탐색기를 선택합니다.
    
    ![image50][image50]
13. 프로세스 탐색기를 선택하여 프로세스를 실행하는 방법에 대한 세부 정보를 볼 수 있습니다. 아래에서 프로세스를 자세히 살펴보고 나서 Azure Portal에서 모든 프로세스를 종료할 수 있습니다.
...
14. 왼쪽의 설정 블레이드로 다시 이동합니다. 새 지원 요청을 클릭합니다.
    ...
    ![image53][image53]
15. 오른쪽에 블레이드에서 문제에 대한 세부 정보를 채우고 연락처 정보를 입력하고 나서 진단 데이터를 업로드할 수 있습니다. Azure Portal을 통해 Microsoft로 작업하면 원활한 환경을 지원할 수 있습니다.
    
    ![image54][image54]
    
    ![image55][image55]
    
    Azure Portal 도움말은 강력하고 친숙한 도구 환경을 제공하여 실행 중인 응용 프로그램을 모니터링하고 문제를 해결하는 데 도움을 줍니다. 프로세스 재활용, 다양한 데이터 컬렉션 사용 및 사용 안 함, Microsoft 전문 지원과 통합 등의 작업을 수행하여 신속하게 조치를 취할 수도 있습니다.
...
11. 오른쪽의 블레이드에서 App Service 인증을 선택합니다. 인기 있는 공급자를 통해 구성할 수 있는 다양한 옵션을 확인합니다.
     ...
     ![image66][image66]
12. 선택한 공급자를 선택하고 범위에 대한 옵션을 확인합니다. 앱 ID 및 앱 암호를 제공하고 앱에 Facebook 인증을 빠르게 설정할 수 있습니다. Azure Portal을 통해 앱에 대한 턴키 솔루션으로 인증을 활성화할 수 있습니다.
    
     ![image67][image67]
13. 설정 블레이드로 다시 이동하고 리소스 관리 범주에서 사용자를 선택합니다.
    
     ![image68][image68]
14. 오른쪽의 블레이드에서 역할 및 사용자를 추가하기 위한 다양한 옵션을 검사합니다. Azure Portal을 사용하면 응용 프로그램에서 RBAC(역할 기반 액세스 제어)를 쉽게 제어할 수 있습니다.
