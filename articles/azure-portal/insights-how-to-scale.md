---
title: 수동 또는 자동으로 인스턴스 개수 조정 | Microsoft Docs
...
1. 슬라이더를 사용하여 수동으로 **인스턴스** 수를 조정할 수 있습니다.
2. **저장**을 클릭하면 거의 즉시 해당 인스턴스 수로 크기가 조정됩니다.
...
> [!NOTE]
> 자동 크기 조정은 선택한 **표준 시간대**에 대한 일광 절약 규칙을 사용합니다. 그러나 일광 절약 시간제 동안 UTC 오프셋은 일광 절약 UTC 오프셋이 아니라 기본 표준 시간대 오프셋을 표시합니다.
> 
> 

1. **확인**을 클릭합니다.
2. 이제 두 번째 프로필 중에 적용할 규칙을 추가해야 합니다. **규칙 추가**를 클릭한 다음 기본 프로필과 동일한 규칙을 구성할 수 있습니다. ![근무 외 시간에 규칙 추가](./media/insights-how-to-scale/Insights_RuleOffWork.png)
3. 규모 확장 및 규모 감축에 대한 규칙을 모두 만들어야 합니다. 그렇지 않으면 프로필 중에 인스턴스 개수가 증가(또는 감소)하기만 합니다.
4. 끝으로, **저장**을 클릭합니다.
