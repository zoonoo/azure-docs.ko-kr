...
**참고**
이 자습서는 Xamarin.iOS 앱에서 모바일 서비스를 통해 Azure를 사용하여 데이터를 저장하고 검색할 수 있는 방법을 더욱 잘 이해할 수 있도록 돕기 위한 것입니다. 이 항목에서는 모바일 서비스 퀵 스타트에서 완료한 다수의 단계를 순서대로 안내합니다. 모바일 서비스를 처음 사용하는 경우 먼저 [모바일 서비스 시작][모바일 서비스 시작] 자습서를 완료하는 것이 좋습니다.

</div>

이 자습서에서는 다음 기본 단계를 단계별로 안내합니다.

1.  [Xamarin.iOS 앱 프로젝트 다운로드][Xamarin.iOS 앱 프로젝트 다운로드]
...
이 자습서에는 [Azure 모바일 서비스 구성 요소][Azure 모바일 서비스 구성 요소], [XCode 5.0][XCode 5.0], [Xamarin.iOS] 및 iOS 5.0 이상 버전이 필요합니다.
...
이 자습서는 Xamarin.iOS 앱인 [GetStartedWithData 앱][Xamarin.iOS 앱 프로젝트 다운로드]을 기반으로 합니다. 이 앱의 UI는 모바일 서비스 Xamarin.iOS 빠른 시작에서 생성된 앱과 동일합니다. 단, 추가된 항목이 로컬에서 메모리에 저장된다는 점은 예외입니다.

1.  [GetStartedWithData 앱][Xamarin.iOS 앱 프로젝트 다운로드]을 다운로드합니다.
...
[완성된 예제 프로젝트][Xamarin.iOS 앱 프로젝트 다운로드]를 다운로드합니다. 고유한 Azure 설정으로 **applicationURL** 및 **applicationKey** 변수를 업데이트해야 합니다.
