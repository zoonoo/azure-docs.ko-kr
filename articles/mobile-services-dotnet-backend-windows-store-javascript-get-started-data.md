...
>[AZURE.NOTE]이 항목에서는 Windows 스토어 프로젝트에 Azure 모바일 서비스를 추가하는 방법을 보여 줍니다. Visual Studio 2013 도구를 사용하여 동일한 .NET 백 엔드 모바일 서비스를 범용 Windows 앱 프로젝트에 추가할 수 있습니다. 자세한 내용은 이 자습서의 [범용 Windows 앱 버전](/ko-kr/documentation/articles/mobile-services-dotnet-backend-windows-universal-javascript-get-started-data)을 참조하세요.
...
* 활성 Azure 계정. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 평가판](http://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A0E0E5C02&amp;returnurl=http%3A%2F%2Fazure.microsoft.com%2Fko-kr%2Fdocumentation%2Farticles%2Fmobile-services-dotnet-backend-windows-store-javascript-get-started-data%2F)을 참조하세요.
...
5. 앱에서 **Insert a TodoItem** 아래의 상자에 일부 텍스트를 입력하고 **저장**을 클릭합니다.
...
4. 방금 다운로드한 개인화된 서비스 시작 프로젝트의 압축을 풉니다. 압축 파일에 있던 폴더를 데이터 시작 솔루션 파일(.sln)이 있는 **JavaScript** 디렉터리로 복사합니다. 이렇게 하면 NuGet 패키지 관리자에서 모든 패키지의 동기화를 더욱 쉽게 유지할 수 있습니다. 
...
4. Visual Studio의 솔루션 탐색기에서 **js** 폴더를 확장하고 default.js를 엽니다. 복사한 코드 조각을  `app.onactivated` 이벤트 처리기에서  `todoItems` 변수에 대한 정의 바로 앞에 붙여 넣습니다. 이 코드 조각에서는 응용 프로그램 키를 사용하여 Azure에서 모바일 서비스에 연결하기 위한 생성자가 주석 처리되어 있습니다. 다음 단계에서는 이 주석 처리를 제거합니다.
...
6. Visual Studio의 솔루션 탐색기에서 default.html 파일을 엽니다. 파일 맨 위에 MobileServices.js의 새로운 WinJS 스크립트 참조를 추가합니다. 그런 다음 파일을 저장합니다.
