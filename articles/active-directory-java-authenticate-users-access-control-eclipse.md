<properties urlDisplayName="Access Control" pageTitle="액세스 제어 사용 방법(Java) - Azure 기능 가이드" metaKeywords="" description="Learn how to develop and use Access Control with Java in Azure." metaCanonical="" services="active-directory" documentationCenter="Java" title="How to Authenticate Web Users with Azure Access Control Service Using Eclipse" videoId="" scriptId="" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" />
...
대부분의 개발자는 ID 전문가가 아니며 일반적으로
응용 프로그램 및 서비스에 대한 인증 및 권한 부여 메커니즘을
개발하는 데 시간을 들이려 하지 않습니다. ACS는 복잡한 인증 논리를
코드에 팩터링할 필요 없이 웹 응용 프로그램 및 서비스에 액세스해야 하는
사용자를 간편하게 인증할 수 있는
Azure 서비스입니다.
...
Azure ACS는 온-프레미스 또는 클라우드에서 실행되는
응용 프로그램을 위한 인증 메커니즘을 만드는 일관된 접근 방법인
클레임 기반 ID 원칙에 따라 구축되었습니다. 클레임 기반 ID는
응용 프로그램 및 서비스가 해당 조직 내부, 조직 외부 및
인터넷에서 사용자에 대해 필요한 ID 정보를 획득하는 공통된 방법을
제공합니다.

이 가이드의 작업을 완료하려면 다음 개념을 이해해야
합니다.

**클라이언트** - 이 방법 가이드의 컨텍스트에서 웹 응용 프로그램에 대한
액세스 권한을 얻으려고 하는 브라우저입니다.

**RP(신뢰 당사자) 응용 프로그램** - RP 응용 프로그램은 특정 외부 기관으로 인증을
아웃소싱하는 웹 사이트 또는 서비스입니다. ID
표현으로는 RP가 해당 기관을 신뢰한다고 합니다. 이 가이드에서는
ACS를 신뢰하도록 응용 프로그램을 구성하는 방법을 설명합니다.

**토큰** - 토큰은 사용자가 성공적으로 인증되었을 때
발급되는 보안 데이터 컬렉션입니다. 토큰에는 인증된 사용자의 특성인 *클레임* 집합이 포함되어 있습니다. 클레임은
사용자의 이름, 사용자가 속한 역할에 대한 식별자, 사용자의 연령
등을 나타낼 수 있습니다. 토큰은 일반적으로 디지털로 서명됩니다. 즉,
항상 발급자에게 다시 제공되며 해당 콘텐츠를 무단으로 변경할 수
없습니다. 사용자는 RP 응용 프로그램이 신뢰하는 기관에서 발급한
유효한 토큰을 제공하여 RP 응용 프로그램에 대한 액세스 권한을 얻습니다.

**IP(ID 공급자)** - 사용자 ID를 인증하고
보안 토큰을 발급하는 기관입니다. 토큰을 실제로 발급하는
작업은 STS(보안 토큰 서비스)라는 특수한 서비스를
통해 구현됩니다. IP의 일반적인 예에는 Windows Live ID,
Facebook, 비즈니스 사용자 리포지토리(예: Active Directory) 등이 포함됩니다.
ACS가 IP를 신뢰하도록 구성되면 시스템은
해당 IP에서 발급한 토큰의 유효성을 검사합니다. ACS는 한 번에 여러 IP를 신뢰할 수 있습니다.
즉, 응용 프로그램이 ACS를 신뢰하는 경우 사용자 대신
ACS가 신뢰하는 모든 IP에서 인증된 모든 사용자에게
응용 프로그램을 제공할 수 있습니다.

**FP(페더레이션 공급자)** - IP는 사용자에 대한 직접적인 정보를 보유하고
자격 증명을 사용하여 인증하며, 사용자 관련 정보에 대한
클레임을 발급합니다. FP(페더레이션 공급자)는 다른 종류의
기관입니다. 즉, 사용자를 직접 인증하는 대신 하나의 RP와
하나 이상의 IP 간의 매개 역할 및 중개자 인증 역할을
합니다. IP와 FP는 보안 토큰을 발급하므로 둘 다
STS(보안 토큰 서비스)를 사용합니다. ACS는 하나의 FP입니다.

**ACS 규칙 엔진** - 신뢰할 수 있는 ID에서 들어오는
토큰을 RP에서 사용할 수 있는 토큰으로 전환하는 데 사용되는 논리는
단순한 클레임 변환 규칙 형태로 코딩됩니다. ACS는 RP에 대해 지정한
변환 논리의 적용을 관리하는 규칙 엔진을
제공합니다.

**ACS 네임 스페이스** - 네임스페이스는 설정을 구성하는 데 사용하는
ACS의 최상위 파티션입니다. 네임스페이스는 신뢰하는 IP 목록,
제공하려는 RP 응용 프로그램, 규칙 엔진이 들어오는 토큰을
처리하는 데 사용하는 규칙 등을 보유합니다. 네임스페이스는
응용 프로그램 및 개발자가 기능 수행을 위해 ACS를 얻는 데 사용하는
다양한 끝점을 노출합니다.

다음 그림에서는 웹 응용 프로그램에서 ACS 인증이 작동하는 방식을
보여 줍니다.
...
7.  ACS가 클라이언트를 RP로 리디렉션합니다. 클라이언트는 ACS가 발급한 새 보안 토큰을 RP로 보냅니다. RP가 ACS에서 발급한 보안 토큰의 서명과 이 토큰의 클레임을 확인한 후 원래 요청된 페이지를 반환합니다.
...
Azure에서ACS(액세스 제어 서비스) 사용을 시작하려면
ACS 네임스페이스를 만들어야 합니다. 네임스페이스는 응용 프로그램 내에서
ACS 리소스의 주소를 지정하기 위한 고유 범위를 제공합니다.
...
5. 네임스페이스가 사용되는 지역을 선택합니다. 응용 프로그램을 배포할 지역을 사용하는 것이 가장 좋습니다.
...
이 작업에서는 인증을 위해 RP 응용 프로그램에 사용할 IP를
추가합니다. 데모 목적으로 이 작업은 Windows Live를
IP로 추가하는 방법을 보여 주지만 ACS 관리 포털에 나열된
아무 IP를 사용해도 됩니다.
...
현재 Windows Live ID가 ACS 네임스페이스에 대한 IP로 사용하도록 설정되어 있습니다. 이제
Java 웹 응용 프로그램(나중에 만들 예정)을 RP로 지정합니다.

## <a name="add-RP"></a>신뢰 당사자 응용 프로그램 추가

이 작업에서는 Java 웹 응용 프로그램을 유효한
RP 응용 프로그램으로 인식하도록 ACS를 구성합니다.
...
이제 Azure 계산 에뮬레이터(http://localhost:8080/)에서 실행될 때 Java 웹 응용 프로그램이
 ACS 네임스페이스의 RP가 되도록 성공적으로 구성했습니다. 다음에는
ACS가 RP에 대한 클레임을 처리하는 데 사용하는 규칙을 만듭니다.

## <a name="create-rules"></a>규칙 만들기

이 작업에서는 클레임에서 IP에서 RP로 전달되는 방식을 구동하는
규칙을 정의합니다. 이 가이드의 목적에 맞게 입력 클레임 형식 및 값을
필터링하거나 수정하지 않고 출력 토큰에 직접 복사하도록
간단히 ACS를 구성합니다.
...
다음으로 응용 프로그램 통합 페이지에서 정보를 검토하고
ACS를 사용하도록 Java 웹 응용 프로그램을
구성하는 데 필요한 URI를 복사합니다.

## <a name="review-app-int"></a>응용 프로그램 통합 페이지 검토

ACS 관리 포털의 응용 프로그램 통합 페이지에서 Java 웹
응용 프로그램(RP 응용 프로그램)을 ACS와 작동하도록 구성하는 데
필요한 모든 정보와 코드를 찾을 수 있습니다. 페더레이션
인증을 위해 Java 웹 응용 프로그램을 구성할 때
이 정보가 필요합니다.
...
1. Eclipse 내의 메뉴에서 **File**, **New**, **Dynamic Web Project**를 차례로 클릭합니다. (**File**, **New**를 클릭한 후 사용 가능한 프로젝트로 **Dynamic Web Project**가 나열되지 않는 경우 다음을 수행합니다. **File**, **New**, **Project**를 차례로 클릭하고 **Web**을 확장한 후 **Dynamic Web Project**, **Next**를 차례로 클릭합니다.) 이 자습서에서는 프로젝트의 이름을 **MyACSHelloWorld**로 지정합니다. (이 이름을 사용했는지 확인하세요. 이 자습서의 이후 단계에서는 WAR 파일의 이름이 MyACSHelloWorld라고 가정합니다.) 화면이 다음과 유사하게 나타납니다.
...
4. **Azure Access Control Services Filter (by MS Open Tech)**를 클릭한 후 **Next**를 클릭합니다. **Azure Access Control Services Filter** 대화 상자가 표시됩니다.  **Location** 필드에는 Eclipse를 설치한 위치에 따라 다른 경로가 포함될 수 있으며 소프트웨어 업데이트에 따라 버전 번호도 다를 수 있습니다.)

    ![Add ACS Filter library][add_acs_filter_lib]

5. 관리 포털의 **로그인 페이지 통합** 페이지가 열려 있는 브라우저를 사용하여 **옵션 1: Link to an ACS-hosted login page** 필드에 나열된 URL을 복사하여 Eclipse 대화 상자의 **ACS Authentication Endpoint** 필드에 붙여 넣습니다.
6. 관리 포털의 **신뢰 당사자 응용 프로그램 편집** 페이지가 열려 있는 브라우저를 사용하여 **영역** 필드에 나열된 URL을 복사하여 Eclipse 대화 상자의 **Relying Party Realm** 필드에 붙여 넣습니다.
...
15. Eclipse에서 **Publish to Azure Cloud** 단추를 클릭합니다. [Eclipse에서 Azure용 Hello World 응용 프로그램 만들기](http://msdn.microsoft.com/ko-kr/library/windowsazure/hh690944.aspx)(영문) 항목의 **To deploy your application to Azure** 섹션에서 수행한 것과 유사하게 프롬프트에 응답합니다. 
...
4. **Add Component** 대화 상자 내에서 다음을 수행합니다.
    1. **Import** 섹션에서 다음을 수행합니다.
        ...
        2. **Method**에 대해 **copy**를 선택합니다.
    2. **As Name**의 경우 텍스트 상자를 클릭하고 기본 이름을 그대로 사용합니다.
    3. **Deploy** 섹션에서 다음을 수행합니다.
        1. **Method**에 대해 **copy**를 선택합니다.
        2. For **To directory**의 경우 **%JAVA_HOME%**를 입력합니다.
