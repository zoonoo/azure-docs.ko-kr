...
보여 줍니다. 샘플은 PHP로 작성되었으며 [Azure SDK for PHP][download-sdk]를 사용합니다. 여기서 다루는 시나리오에는 **토픽 및 구독 만들기**, **구독 필터 만들기**, **토픽에 메시지 보내기**, **구독에서 메시지 받기**, **토픽 및 구독 삭제** 등이 포함됩니다.
...
아래 예제에서는 자동 로더 파일을 포함하고 **ServiceBusService** 클래스를 참조하는 방법을 보여 줍니다.
...
**ServiceBusRestProxy** 클래스를 통해 서비스 버스 토픽에 대한 관리 작업을 수행할 수 있습니다. **ServiceBusRestProxy** 개체는 관리에 필요한 토큰 사용 권한을 캡슐화하는 적합한 연결 문자열을 사용한 **ServicesBuilder::createServiceBusService** 팩터리 메서드를 통해 구성됩니다.

아래 예에서는 **ServiceBusRestProxy**를 인스턴스화하고 **ServiceBusRestProxy->createTopic**을 호출하여 `MySBNamespace` 서비스 네임스페이스 내에 `mytopic`이라는 토픽을 만드는 방법을 보여 줍니다.
	...
	> [AZURE.NOTE]
	> <b>ServiceBusRestProxy</b> 개체의 <b>listTopics</b> 메서드를 사용하여 서비스 네임스페이스 내에 지정된 이름의 토픽이 이미 있는지 확인할 수 있습니다.

## 구독을 만드는 방법

토픽 구독도 **ServiceBusRestProxy->createSubscription** 개체를 사용하여 만듭니다. 구독에는 이름이 지정되며, 구독의 가상 큐에 전달되는 메시지 집합을 제한하는 선택적 필터가 있을 수 있습니다.

### 기본(MatchAll) 필터를 사용하여 구독 만들기

**MatchAll** 필터는 새 구독을 만들 때 필터를 지정하지 않은 경우 사용되는 기본 필터입니다. **MatchAll** 필터를 사용하면 토픽에 게시된 모든 메시지가 구독의 가상 큐에 배치됩니다. 다음 예제에서는 'mysubscription' 구독을 만들고 기본 **MatchAll** 필터를 사용합니다.
...
토픽으로 전송된 메시지 중 특정 토픽 구독 내에 표시되어야 하는 메시지의 범위를 지정하는 필터를 설정할 수도 있습니다. 구독에서 지원하는 가장 유연한 유형의 필터는 SQL92 하위 집합을 구현하는 **SqlFilter**입니다. SQL 필터는 토픽에 게시된 메시지의 속성에 적용됩니다. SqlFilter에 대한 자세한 내용은 [SqlFilter.SqlExpression 속성 ][sqlfilter]를 참조하세요.

	> [AZURE.NOTE]
	> 구독에 대한 각 규칙은 들어오는 메시지를 독립적으로 처리하여 해당 결과 메시지를 구독에 추가합니다. 또한 각 새 구독에는 토픽의 모든 메시지를 구독에 추가하는 필터가 포함된 기본 <b>규칙</b>이 있습니다. 필터와 일치하는 메시지만 받으려면 기본 규칙을 제거해야 합니다. <b>ServiceBusRestProxy->deleteRule</b> 메서드를 사용하여 기본 규칙을 제거할 수 있습니다.

아래 예제에서는 사용자 지정 **MessageNumber** 속성이 3보다 큰 메시지만 선택하는 **SqlFilter**가 포함된 "HighMessages"라는 구독을 만듭니다. 사용자 지정 속성을 메시지에 추가하는 방법에 대한 자세한 내용은 [방법: 토픽에 메시지 보내기](#SendMessage) 를 참조하세요.
...
위 코드에서는 추가 네임스페이스인 `WindowsAzure\ServiceBus\Models\SubscriptionInfo`를 사용해야 합니다.
...
이제 `mytopic` 토픽으로 전송하는 메시지는 항상 `mysubscription` 구독에 가입된 수신기로 배달되며 메시지 내용에 따라 선택적으로 "HighMessages" 및 "LowMessages" 구독에 가입된 수신기로 배달됩니다.

## 방법: 토픽에 메시지 보내기

서비스 버스 토픽에 메시지를 보내기 위해 응용 프로그램은 **ServiceBusRestProxy->sendTopicMessage** 메서드를 호출합니다. 아래 코드는 `mytopic`
토픽(위에서 `MySBNamespace` 서비스 네임스페이스 내에 만듬)에 메시지를 보내는 방법을 보여 줍니다.
...
서비스 버스 토픽으로 전송되는 메시지는 **BrokeredMessage** 클래스 인스턴스입니다. **BrokeredMessage** 개체에는 **getLabel**, **getTimeToLive**, **setLabel**, **setTimeToLive** 등의 표준 속성 집합 및 메서드와 응용 프로그램별 사용자 지정 속성을 저장하는 데 사용할 수 있는 속성이 포함되어 있습니다. 다음 예제에서는 앞에서 만든 `mytopic` 토픽에 5개의 테스트 메시지를 보내는 방법을 보여 줍니다. **setProperty** 메서드는 각 메시지에 사용자 지정 속성(`MessageNumber`)을 추가하는 데 사용됩니다. 각 메시지에서 `MessageNumber` 속성 값이 어떻게 달라지는지 살펴봅니다. 위의 [방법: 구독 만들기](#CreateSubscription) 섹션에 나와 있는 것처럼 이 값을 사용하여 메시지를 받는 구독을 결정할 수 있습니다.
...
**ReceiveAndDelete** 모드를 사용하는 경우 수신은 1단계 작업입니다. 즉, 서비스 버스가 구독의 메시지에 대한 읽기 요청을 받으면 메시지를 사용 중인 것으로 표시하고 응용 프로그램에 반환합니다. **ReceiveAndDelete** 가장 단순한 모델이며, 응용 프로그램이 실패 이벤트 시 메시지를 처리하지 않아도 안전한
시나리오에서 효과적입니다. 이 모드를 파악하려면 소비자가 수신 요청을 실행했는데 요청이 처리되기 전에 응용 프로그램 작동이 중지되는 시나리오를 고려할 수 있습니다. 서비스 버스는 메시지를 이용되는 것으로 표시하기 때문에 응용 프로그램이 다시 시작되고 메시지 소비를 다시 시작할 경우 크래시 전에 소비된 메시지가 누락됩니다.
...
항목 [큐, 토픽 및 구독][]에서 자세한 내용을 확인할 수 있습니다.

[download-sdk]: http://go.microsoft.com/fwlink/?LinkId=252473
[서비스 버스 토픽 및 구독 정의]: #bkmk_WhatAreSvcBusTopics
[서비스 네임스페이스 만들기]: #bkmk_CreateSvcNamespace
[네임스페이스에 대한 기본 관리 자격 증명 얻기]: #bkmk_ObtainDefaultMngmntCredentials
