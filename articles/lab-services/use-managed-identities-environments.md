...
1. PUT Https 메서드를 수행 하 여 다음 예제 `ServiceRunner` 와 비슷한 랩에 새 리소스를 추가 합니다. Service runner 리소스는 DevTest Labs에서 관리 되는 id를 관리 하 고 제어 하는 프록시 리소스입니다. Service runner 이름은 유효한 이름일 수 있지만 관리 되는 id 리소스의 이름을 사용 하는 것이 좋습니다. 
