...
이 템플릿을 포함 하는 리포지토리의 폴더에는 `nestedtemplates` **nestone. Json** 및 **nestone. json**파일의 하위 폴더가 있습니다. **Azuredeploy json**에서 템플릿의 URI는 아티팩트 위치, 중첩 된 템플릿 폴더, 중첩 된 템플릿 파일 이름을 사용 하 여 빌드됩니다. 마찬가지로 매개 변수에 대 한 URI는 아티팩트 위치, 중첩 된 템플릿 폴더 및 중첩 된 템플릿에 대 한 매개 변수 파일을 사용 하 여 빌드됩니다. 
