...
이 문서에서는 Azure Lab Services를 사용 하 여 NLP (자연어 처리)의 심층 학습에 초점을 맞춘 랩을 설정 하는 방법을 보여 줍니다. NLP (자연어 처리)는 번역, 음성 인식 및 기타 언어 이해 기능을 갖춘 컴퓨터를 사용할 수 있도록 하는 AI (인공 지능) 형태입니다.  

NLP 클래스를 사용 하는 학생은 Linux VM (가상 머신)을 사용 하 여 작성 된 인간 언어 분석에 사용 되는 심층 학습 모델을 개발 하는 신경망 알고리즘을 적용 하는 방법을 알아봅니다. 
...
| 랩 계정 설정 | 지침 |
| ----------- | ------------ |  
| Marketplace 이미지 | 랩 계정 내에서 사용할 Ubuntu (Linux 용 Data Science Virtual Machine) 이미지를 사용 하도록 설정 합니다.  지침은 다음 문서를 참조하세요. [랩 작성자가 사용할 수 있는 marketplace 이미지를 지정](tutorial-setup-lab-account.md#specify-marketplace-images-available-to-lab-creators)합니다. | 
