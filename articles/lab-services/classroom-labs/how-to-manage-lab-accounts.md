...
6. 페이지 맨 아래에서 **다음: 태그** 를 선택 하 여 **태그** 탭으로 전환 합니다. 랩 계정에 연결 하려는 태그를 추가 합니다. 태그는 여러 리소스 및 리소스 그룹에 동일한 태그를 적용 하 여 리소스를 범주화 하 고 통합 된 청구를 볼 수 있는 이름/값 쌍입니다. 자세한 내용은 [태그를 사용하여 Azure 리소스 구성](../../azure-resource-manager/management/tag-resources.md)을 참조하세요.
