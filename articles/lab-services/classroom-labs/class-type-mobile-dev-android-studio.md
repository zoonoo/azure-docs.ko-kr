...
Android Sdk가 있는 경우 Android 용 Visual Studio 에뮬레이터를 알리기 위해 특정 레지스트리 키를 설정 해야 합니다.  필요한 레지스트리 키를 설정 하려면 아래 스크립트를 실행 합니다.  아래 PowerShell 스크립트는 Android Sdk의 기본 설치 위치를 가정 합니다.  다른 위치에 Android Sdk를 설치한 경우 스크립트를 실행 하기 전에 `$androidSdkPath`의 값을 수정 합니다.
...
25 개의 학생 \* (20 개의 예약 + 10 할당량) 시간 * 55 랩 단위 * 0.01 USD/시간 = 412.5 USD
