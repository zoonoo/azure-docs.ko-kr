...
랩의 Vm이 특정 시간에 자동으로 시작/중지 되도록 랩에 대 한 예약 된 이벤트를 만듭니다. 이전에 지정한 사용자 할당량은 예약 된 시간 외에 각 사용자에 게 할당 된 추가 시간입니다. 

1. **일정** 페이지로 전환 하 고 도구 모음에서 **예약 된 이벤트 추가** 를 선택 합니다. 

    ![일정 페이지의 일정 추가 단추](../media/how-to-create-schedules/add-schedule-button.png)
2. **표준** 이 **이벤트 유형**으로 선택 되어 있는지 확인 합니다. Vm의 시작 시간만 지정 하려면 **시작만** 을 선택 합니다. Vm에 대 한 중지 시간만 지정 하려면 **중지만** 을 선택 합니다. 
7. **반복** 섹션에서 현재 일정을 선택 합니다. 

    ![일정 페이지의 일정 추가 단추](../media/how-to-create-schedules/select-current-schedule.png)
5. **반복** 대화 상자에서 다음 단계를 수행 합니다.
    1. **반복** 필드에 대해 **매주가 설정 되어 있는지 확인** 합니다. 
    3. **시작 날짜**를 지정 합니다.
    4. Vm을 시작할 **시작 시간** 을 지정 합니다.
    5. Vm을 종료할 **중지 시간** 을 지정 합니다. 
    6. 지정한 시작 및 중지 시간에 대 한 **표준 시간대** 를 지정 합니다. 
    2. 일정을 적용할 날짜를 선택 합니다. 다음 예에서는 월요일-목요일이 선택 됩니다. 
    8. **저장**을 선택합니다. 

        ![반복 일정 설정](../media/how-to-create-schedules/set-repeat-schedule.png)

3. 이제 **예약 된 이벤트 추가** 페이지에서 **메모 (선택 사항)** 에 대해 설명 또는 일정에 대 한 메모를 입력 합니다. 
4. **예약 된 이벤트 추가** 페이지에서 **저장**을 선택 합니다. 
