...
4. **가상 머신 사양 선택** 페이지에서 다음 단계를 수행합니다.
    1. 랩에서 만드는 VM(가상 머신)의 **크기**를 선택합니다. 현재 **소규모**, **중간**를 **medium (가상화)** , **큰**, 및 **GPU** 크기는 사용할 수 있습니다. 자세한 내용은 참조는 [VM 크기](#vm-sizes) 섹션입니다.
...
5. **자격 증명 설정** 페이지에서 랩의 모든 VM에 대한 기본 자격 증명을 지정합니다. 
    ...
    3. 사용 안 함 **동일한 암호를 사용 하 여 모든 가상 머신에 대해** 학생에 게 자신의 암호를 설정 하기를 원하는 경우 옵션입니다. 이 단계는 **선택 사항**입니다. 

        교사 랩에서 모든 Vm에 대해 동일한 암호를 사용 하거나 해당 Vm에 대 한 암호를 설정 하는 학생을 허용 하도록 선택할 수 있습니다. 기본적으로 모든 Windows 및 Linux 이미지 Ubuntu 제외 하 고이 설정을 사용 합니다. 선택 하면 **Ubuntu** 이 설정은 VM를 사용할 수 없으므로 학생 들에 게 처음 로그인 할 때 암호를 설정 하 라는 메시지가 나타납니다.
    1. **만들기**를 선택합니다. 
...
8. 다음 단계는 이 자습서의 선택적 단계입니다. 
    ...
    1. 선택 **암호 재설정** VM에 대 한 암호를 재설정할 수 있습니다. 
...
| 크기 | 코어 수 | RAM | 설명 | 
| ---- | ----- | --- | ----------- | 
| 작음 | 2 | 3.5 GB | 이 크기는 중소 규모 데이터베이스 작은 트래픽이 낮은 웹 서버, 웹 브라우저를 열고 명령줄에 가장 적합 합니다. |
| 중간 | 4 | 7 GB | 이 크기는 관계형 데이터베이스, 메모리 내 캐싱 및 분석에 가장 적합 | 
| 중간 (중첩 된 가상화 사용) | 4 | 16GB | 이 크기는 관계형 데이터베이스, 메모리 내 캐싱 및 분석에 가장 적합 합니다. 이 크기는 또한 중첩 된 가상화를 지원합니다. <p>이 크기 시나리오에서 사용할 수 있는 각 학생 여러 Vm이 필요 합니다. 교사 중첩 된 가상화를 사용 하 여 가상 머신 내에서 몇 가지 중첩 하는 작은 크기 가상 머신을 설정 하 수 있습니다. </p> |
| 큰 | 8 | 32GB | 이 크기는 더 빠른 Cpu, 더 뛰어난 로컬 디스크 성능, 대규모 데이터베이스, 대용량 메모리 캐시 해야 하는 응용 프로그램에 가장 적합 합니다. 이 크기에도 중첩 된 가상화 지원 |  
| GPU | 12 | 112GB | 이 크기는 계산 집약적 이며 그래픽 집약적인 시각화 워크 로드에 가장 적합 | 
