...
- 구독과 연결 된 AD (Azure Active Directory) 테 넌 트의 사용자에 게 Azure Lab Services에 대 한 액세스 권한을 부여할 수 있습니다. 사용자를 랩 계정 소유자 \ 기여자 또는 교실 랩 작성자로 추가할 수 있습니다.
...
공유 이미지 갤러리는 랩 계정에 연결 되 고 이미지를 저장 하기 위한 중앙 리포지토리로 사용 됩니다. 교육자가 교실 랩의 템플릿 가상 머신 (VM)에서 저장 하도록 선택 하면 이미지는 갤러리에 저장 됩니다. 교육자가 템플릿 VM을 변경 하 고 저장할 때마다 이전 버전을 유지 하면서 이미지의 새 버전이 저장 됩니다.

강사는 새로운 교실 랩을 만들 때 공유 이미지 갤러리에서 이미지 버전을 게시할 수 있습니다. 갤러리에서 여러 버전의 이미지를 저장할 수 있지만 교사는 랩 생성 중에 최신 버전만 선택할 수 있습니다.
...
- **템플릿 VM 이미지의 버전을 저장 하 고 관리할 수**있습니다.
    ...
    사용자 지정 이미지를 만들거나 공용 Marketplace 갤러리에서 이미지를 변경 (소프트웨어, 구성 등) 할 때 유용 합니다.  예를 들어, 교육자는 서로 다른 기타 \tools를 설치 해야 하는 경우가 많습니다. 학습자는 이러한 필수 구성 요소를 직접 설치 하도록 요구 하는 대신 다른 버전의 템플릿 VM 이미지를 공유 이미지 갤러리에 저장할 수 있습니다. 이러한 이미지 버전은 새 교실 랩을 만들 때 사용할 수 있습니다.
- **교실 랩에서 템플릿 VM 이미지를 Sharing\reuse 수 있습니다**.

    이를 통해 새로운 교실 랩을 만들 때마다 이미지를 처음부터 구성할 필요가 없습니다. 예를 들어 동일한 이미지를 필요로 하는 여러 클래스를 제공 하는 경우이 이미지를 한 번만 만들고 공유 이미지 갤러리에 저장 하 여 교실 랩에서 공유할 수 있도록 해야 합니다.
- **복제를 통해 이미지 가용성을 보장**합니다.

    교실 랩에서 공유 이미지 갤러리에 저장 하면 이미지가 동일한 지리 내의 다른 지역에 자동으로 복제 됩니다. 지역에 대 한 가동 중단이 발생 한 경우 다른 지역에서 이미지 복제본을 사용 하 여 교실 랩에서 템플릿 VM을 게시할 때 영향을 받지 않습니다. 또한 다양 한 복제본을 사용 하도록 분산 하 여 다중 VM 게시 시나리오의 성능을 향상 시킬 수 있습니다.
...
| 리소스 유형 | 역할 | 제안된 패턴 | 예시 |
| ------------- | ---- | ----------------- | -------- | 
| 리소스 그룹 | 하나 이상의 랩 계정 및 하나 이상의 공유 이미지 갤러리를 포함 합니다. | \<조직 약식 이름\>-\<환경\>-rg<ul><li>**조직 약식 이름** 리소스 그룹이 지 원하는 조직의 이름을 식별 합니다.</li><li>**환경** 에서 테스트 또는 프로덕션과 같은 리소스에 대 한 환경을 식별 합니다.</li><li>**Rg** 는 리소스 종류: 리소스 그룹을 나타냅니다.</li></ul> | a n y i n y-rg<br/>테스트-rg-rg<br/>a n y i n a d-prod-rg |
| 랩 계정 | 하나 이상의 랩 포함 | \<조직 약식 이름\>-\<환경\>-la<ul><li>**조직 약식 이름** 리소스 그룹이 지 원하는 조직의 이름을 식별 합니다.</li><li>**환경** 에서 테스트 또는 프로덕션과 같은 리소스에 대 한 환경을 식별 합니다.</li><li>**La** 는 리소스 유형: 랩 계정을 나타냅니다.</li></ul> | a n y i n y-la<br/>mathdeptlabs-la<br/>sciencedeptlabs-la<br/>sciencedeptlabs-prod-la |
...
## <a name="regions-or-locations"></a>영역 또는 위치
Azure Lab Services 리소스를 설정 하는 경우 리소스를 호스팅할 데이터 센터의 지역 또는 위치를 제공 해야 합니다. 다음은 지역 \location이 Lab Services 배포에 사용 되는 다음 리소스에 영향을 주는 방법에 대 한 자세한 내용입니다.
...
- **랩 계정 또는 교실 랩**
    ...
    랩 계정의 위치는이 리소스에 대 한 지역을 나타냅니다. 랩 계정에서 만든 교실 랩을 동일한 지리 내의 모든 지역에 배포할 수 있습니다. 랩의 Vm을 배포 하는 특정 지역은 해당 지역에서 사용 가능한 용량에 따라 자동으로 선택 됩니다.  
    관리자가 교실 랩의 위치를 선택 하도록 허용 하는 경우 랩을 만들 때 선택할 수 있는 위치가 사용 가능한 지역 용량을 기반으로 합니다.

    또한 교실 랩의 위치는 선택할 수 있는 VM 계산 크기를 결정 합니다. 특정 계산 크기는 특정 위치 내 에서만 사용할 수 있습니다.
- **공유 이미지 갤러리**

    지역은 대상 지역에 자동으로 복제 되기 전에 첫 번째 이미지 버전이 저장 된 원본 지역을 나타냅니다.
    
일반적인 규칙은 리소스의 위치를 사용자에 게 가장 가까운 것으로 설정 하는 것입니다. 교실 랩에서는 학생 들에 게 가장 가까운 교실 랩을 만드는 것을 의미 합니다. 학생 들이 전 세계에 있는 온라인 과정의 경우 최고의 판단을 사용 하 여 중앙에 있는 교실 랩을 만들어야 합니다. 또는 학생의 지역에 따라 클래스를 여러 교실 랩으로 분할 합니다.
...
| 크기 | 사양 | 권장 사용 |
| ---- | ----- | ------------- |
| 작음| <ul><li>2개 코어</li><li>3.5GB RAM</li></ul> | 이 크기는 명령줄, 웹 브라우저 열기, 트래픽이 적은 웹 서버, 중소 규모의 데이터베이스에 가장 적합 합니다. |
| 중간 | <ul><li>4 코어</li><li>7GB RAM</li></ul> | 이 크기는 관계형 데이터베이스, 메모리 내 캐싱 및 분석에 가장 적합 합니다. |
| 중간 (중첩 된 가상화) | <ul><li>4 코어</li><li>16GB RAM</li></ul> | 이 크기는 관계형 데이터베이스, 메모리 내 캐싱 및 분석에 가장 적합 합니다.  이 크기는 중첩 된 가상화도 지원 합니다. |
| 큼 | <ul><li>8개 코어</li><li>32 GB RAM</li></ul> | 이 크기는 더 빠른 Cpu, 더 뛰어난 로컬 디스크 성능, 큰 데이터베이스, 큰 메모리 캐시를 필요로 하는 응용 프로그램에 가장 적합 합니다.  이 크기는 중첩 된 가상화도 지원 합니다. |
| 소형 GPU (시각화) | <ul><li>6 코어</li><li>56 GB RAM</li> | 이 크기는 OpenGL 및 DirectX와 같은 프레임 워크를 사용 하는 원격 시각화, 스트리밍, 게임, 인코딩에 가장 적합 합니다. |
| 소형 GPU (계산) | <ul><li>6 코어</li><li>56 GB RAM</li></ul> |이 크기는 인공 지능 및 심층 학습 같은 컴퓨터 집약적 응용 프로그램에 가장 적합 합니다. |
| 중형 GPU (시각화) | <ul><li>12 코어</li><li>112 GB RAM</li></ul> | 이 크기는 OpenGL 및 DirectX와 같은 프레임 워크를 사용 하는 원격 시각화, 스트리밍, 게임, 인코딩에 가장 적합 합니다. |

## <a name="manage-identity"></a>Id 관리
랩 계정 관리자에 게는 다음과 같은 두 가지 유형의 역할이 있습니다.

- **소유자**

    **소유자** 역할에 할당 된 관리자는 다른 사용자에 게 랩 계정에 대 한 액세스 권한을 부여 하 고 랩 작성자를 추가할 수 있는 권한을 포함 하 여 랩 계정에 대 한 모든 권한을 갖습니다. 기본적으로 랩 계정을 만드는 관리자는 소유자로 추가 됩니다.
- **기여자**

    참가자 역할이 할당 된 관리자는 랩 계정 설정을 변경할 수 있지만 다른 사용자에 게 액세스 권한을 부여할 수는 없습니다. 또한 랩 작성자를 추가할 수 있습니다.

공유 이미지 갤러리를 랩 계정에 연결 하면 관리자 및 랩 작성자 모두에 게 액세스 권한이 자동으로 부여 되므로 갤러리에서 이미지를 보고 저장할 수 있습니다. 
