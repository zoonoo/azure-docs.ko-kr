...
2. 목록에서 특정 또는 모든 사용자를 선택합니다. 특정 사용자를 선택하려면 목록 첫 번째 열의 확인란을 선택하세요. 모든 사용자를 선택하려면 첫 번째 열의 제목 앞에 있는 확인란(**이름**)을 선택하거나 목록에 있는 모든 사용자에 대한 확인란을 모두 선택하세요. 상태를 볼 수는 **초대 상태** 이 목록에 있습니다.  모든 학생 들에 대 한 초대 상태로 다음 그림과에서 **초대를 보내지 않음**합니다. 

    ![학생 선택](../media/tutorial-setup-classroom-lab/select-students.png)
1. 선택 된 **전자 메일 아이콘 (봉투)** 행 (또는) 선택 중 하나로 **초대를 보낼** 도구 모음에서. 또한 학생 이름을 전자 메일 아이콘을 보려면 목록에서 마우스를 가리키면 수 있습니다. 
...
4. **이메일로 등록 링크 보내기** 페이지에서 다음 단계를 수행하세요. 
    ...
    2. **이메일로 등록 링크 보내기** 페이지에서 **보내기**를 선택합니다. 초대 변경의 상태를 확인할 **초대장 보내기** 하 고 **보낸 초대**합니다. 
        
        ![초대 전송](../media/tutorial-setup-classroom-lab/invitations-sent.png)
