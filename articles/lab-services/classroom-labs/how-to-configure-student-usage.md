...
## <a name="share-registration-link-with-students"></a>학생 들과 등록 링크 공유
...
Azure Lab Services를 통해 교사는 다른 전자 메일 클라이언트를 사용 하지 않고도 전체 또는 선택한 학생에 게 랩 초대를 전자 메일로 보낼 수 있습니다. 교사는 목록의 개별 학생 들에 게 각 학생의 전자 메일 아이콘을 표시 하거나 학생을 하나 이상 선택 하 고 도구 모음에서 **초대 보내기** 를 사용할 수 있습니다. 이 기능은 교사가 추가한 등록 링크와 메시지 (있는 경우)가 포함 된 전자 메일을 보냅니다. 초대를 보낸 후에는 교사가 이미 등록 링크를 받은 학생 및 보낸 날짜를 추적할 수 있도록 초대 상태가 **초대로 변경** 됩니다.
...
4. 학생 들이 클래스에 등록할 수 있도록 **등록 링크** 를 학생 들과 공유 합니다. 
...
3. **사용자당 할당량** 페이지에서 각 사용자(학생)에게 제공하려는 시간을 지정합니다. 
    1. **사용자당 총 랩 시간 수** 사용자는 **예약된 시간 외에도** 설정된 시간(이 필드에서 지정된) 동안 VM을 사용할 수 있습니다. 이 옵션을 선택하는 경우 텍스트 상자에 **시간**을 입력합니다. 
        ...
        ![사용자당 시간 수](../media/how-to-configure-student-usage/number-of-hours-per-user.png)을 선택합니다. 
...
> [!IMPORTANT]
> 학생에 게 등록 링크를 보내기 전에 0 할당량 시간을 선택 하거나 랩에 대 한 할당량 시간을 지정 하는 경우 클래스에 대 한 일정을 설정 해야 합니다.
>
