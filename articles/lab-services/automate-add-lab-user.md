...
가장 먼저 해야 할 일은 리소스에 대 한 형식이 리소스 그룹 `Microsoft.Authorization/roleAssignments` 에 대 한 것과는 다른 것입니다.  대신 리소스 형식이 패턴 `{provider-namespace}/{resource-type}/providers/roleAssignments`을 따릅니다. 이 경우 리소스 형식은이 됩니다 `Microsoft.DevTestLab/labs/providers/roleAssignments`.

역할 할당 이름은 전역적으로 고유 해야 합니다.  할당의 이름에는 패턴이 `{labName}/Microsoft.Authorization/{newGuid}`사용 됩니다. 는 `newGuid` 템플릿에 대 한 매개 변수 값입니다. 역할 할당 이름이 고유한 지 확인 합니다. Guid를 만들기 위한 템플릿 함수가 없으므로 GUID 생성기 도구를 사용 하 여 직접 GUID를 생성 해야 합니다.  
...
역할 할당 자체는 세 가지 속성을 정의 합니다. `roleDefinitionId`, `principalId`및 `scope`가 필요 합니다.
...
구독 ID는 템플릿 함수를 사용 `subscription().subscriptionId` 하 여 가져옵니다.  
...
역할 ID는 variables 섹션에서 정의 되 고 이름이로 `devTestLabUserRoleId`지정 됩니다. 템플릿에서 역할 ID는 111111111-0000-0000-11111111111111111로 설정 됩니다. 
...
### <a name="scope"></a>범위
범위 역할 할당을 적용 해야 하는 리소스 또는 리소스 그룹을 지정 합니다. 리소스의 경우 범위는 형식입니다 `/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/{provider-namespace}/{resource-type}/{resource-name}`. 템플릿은 `subscription().subscriptionId` 함수를 사용 하 여 파트를 채우고 `subscription-id` `resourceGroup().name` 템플릿 함수를 사용 하 여 `resource-group-name` 파트를 채웁니다. 이러한 함수를 사용 하면 역할을 할당 하는 랩이 현재 구독에 존재 하 고 템플릿 배포가 수행 되는 동일한 리소스 그룹에 있어야 합니다. 마지막 부분인 `resource-name`는 랩의 이름입니다. 이 값은이 예제에서 템플릿 매개 변수를 통해 수신 됩니다. 
...
권한을 부여할 리소스 `ResourceName`를 지정 하려면 `ResourceType` `ResourceGroup` `scope` 매개 변수를 사용 하 여, 또는를 조합 하 여 지정할 수 있습니다. 사용 되는 매개 변수 조합을 사용 하 여 cmdlet에 대 한 충분 한 정보를 제공 하 여 Active Directory 개체 (사용자, 그룹 또는 서비스 사용자), 범위 (리소스 그룹 또는 리소스), 역할 정의를 고유 하 게 식별 합니다.
...
액세스 권한이 부여 되는 개체는 `objectId`, `signInName`, `spn` 매개 변수를 통해 지정할 수 있습니다. 개체에 `scope` 대 한 액세스 권한을 부여 하는 랩을 url 또는 `resource-name`, `resource-type`및 `resource-group` 매개 변수의 조합으로 식별할 수 있습니다.
