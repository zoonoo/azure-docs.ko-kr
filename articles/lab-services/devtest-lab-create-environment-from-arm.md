...
- 마스터 템플릿 파일의 이름을 *azuredeploy*로 표시 해야 합니다. 
  
- 매개 변수 파일에 정의 된 매개 변수 값을 사용 하려면 매개 변수 파일의 이름을 *azuredeploy. json*으로 지정 해야 합니다.
...
- 메타 데이터를 정의 하 여 다음과 같이 *metadata. json*파일에 템플릿 표시 이름 및 설명을 지정할 수 있습니다.
...
1. **추가** 창에서 환경 사용자에 게 표시할 **환경 이름** 값을 입력 합니다. 
   ...
   Azure Resource Manager 템플릿은 나머지 입력 필드를 정의 합니다. *Azuredeploy. parameter. json* 파일이 기본값을 정의 하는 경우 입력 필드에 해당 값이 표시 됩니다. 
...
1. 다음 샘플 PowerShell 스크립트를 하드 드라이브에 *deployenv.* p s 1로 저장 합니다. 
...
Azure CLI를 사용 하 여 리소스 관리자 템플릿으로 리소스를 배포할 수도 있습니다. 자세한 내용은 [Resource Manager 템플릿과 Azure CLI로 리소스 배포](../azure-resource-manager/resource-group-template-deploy-cli.md)를 참조하세요.
...
- 리소스 관리자 템플릿은 대부분의 기존 리소스를 참조할 수 없습니다. 새 리소스를 만들 수만 있습니다. 리소스 관리자 템플릿이 기존 리소스를 참조 하는 DevTest Labs 외부에서 사용 하는 경우 DevTest Labs에서 사용할 수 없습니다. 단, 기존 가상 네트워크를 참조할 수 있는 경우는 예외입니다. 
