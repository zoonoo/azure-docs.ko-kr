...
다음은 랩의 서브넷을 연결 하는 간단한 환경 템플릿입니다. 이 샘플에서 매개 변수 `DTLSubnetId` 는 랩이 존재 하는 서브넷의 ID를 나타냅니다. DevTest Labs에서 랩 서브넷 `$(LabSubnetId)`의 ID로 자동으로 해결 되는에 할당 됩니다. 이 정의에서 VM의 **네트워크 인터페이스** 에 대 한 **서브넷** 속성은 동일한 서브넷에 `DTLSubnetId` 조인 되도록로 설정 됩니다. 
