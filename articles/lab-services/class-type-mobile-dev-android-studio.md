...
3. vs_emulatorsetup.exe 다운로드 되 면 실행 파일을 실행 합니다.
...
10. **Android Studio 설치 완료** 페이지에서  **Finish**를 클릭합니다.
...
17. **구성 요소 다운로드 중** 페이지에서 모든 구성 요소가 다운로드 될 때까지 기다립니다.  **Finish**를 클릭합니다.
...
Android Sdk가 있는 경우 Android 용 Visual Studio 에뮬레이터를 알리기 위해 특정 레지스트리 키를 설정 해야 합니다.  필요한 레지스트리 키를 설정 하려면 아래 스크립트를 실행 합니다.  아래 PowerShell 스크립트는 Android Sdk의 기본 설치 위치를 가정 합니다.  다른 위치에 Android Sdk를 설치한 경우 `$androidSdkPath` 스크립트를 실행 하기 전에에 대 한 값을 수정 합니다.
