...
CSV 텍스트 파일은 쉼표로 구분 된 CSV () 테이블 형식 데이터 (숫자 및 텍스트)를 저장 하는 데 사용 됩니다. 스프레드시트 등의 열 필드에 정보를 저장 하는 대신 CSV 파일은 쉼표로 구분 된 정보를 저장 합니다. CSV 파일의 각 줄에는 동일한 수의 쉼표로 구분 된 "필드"가 있습니다. Excel을 사용 하 여 CSV 파일을 쉽게 만들고 편집할 수 있습니다.
