...
- [사용자 지정 이미지를 저장 하 고 여러 랩에 배포](image-factory-save-distribute-custom-images.md)
...
정리 단계를 구성 하기 전에 DevTest Labs에 보존할 기록 이미지 수를 정의 합니다. [Azure DevOps에서 이미지 팩터리 실행](image-factory-set-up-devops-lab.md) 문서를 실행 한 후에는 다양 한 빌드 변수를 구성 했습니다. 그 중 하나가 **ImageRetention**되었습니다. 이 변수는 `1`로 설정 합니다. 즉, DevTest Labs에서 사용자 지정 이미지의 기록을 유지 관리 하지 않습니다. 최신 분산 이미지만 사용할 수 있습니다. 이 변수를 `2`변경 하는 경우 최신 분산 이미지와 이전 이미지를 합한 값이 유지 됩니다. 이 값을 설정 하 여 DevTest Labs에서 유지 하려는 기록 이미지의 수를 정의할 수 있습니다.
