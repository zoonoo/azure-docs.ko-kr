...
1. 이미지에서 만든 실제 랩 환경에서 Hyper-v VM으로 시작 합니다.
    ...
    1. 일반적으로 VM을 이미지 합니다.
1. [VM에 연결 하 고 Azure에 대해 준비](../virtual-machines/windows/prepare-for-upload-vhd-image.md)합니다.
    1. [Azure에 대한 Windows 구성 설정](../virtual-machines/windows/prepare-for-upload-vhd-image.md#set-windows-configurations-for-azure)
    1. [VM 연결을 보장 하는 데 필요한 최소한의 Windows 서비스를 확인 합니다.](../virtual-machines/windows/prepare-for-upload-vhd-image.md#check-the-windows-services)
    1. [원격 데스크톱 레지스트리 설정 업데이트](../virtual-machines/windows/prepare-for-upload-vhd-image.md#update-remote-desktop-registry-settings)
    1. [Windows 방화벽 규칙 구성](../virtual-machines/windows/prepare-for-upload-vhd-image.md#configure-windows-firewall-rules)
    1. Windows 업데이트 설치
    1. [여기에 표시 된 대로 Azure VM 에이전트 및 추가 구성을 설치 합니다.](../virtual-machines/windows/prepare-for-upload-vhd-image.md#complete-the-recommended-configurations) 
    
    위의 단계에서는 특수 이미지를 만듭니다. 일반화 된 이미지를 만드는 경우 [SysPrep](../virtual-machines/windows/prepare-for-upload-vhd-image.md#determine-when-to-use-sysprep)도 실행 해야 합니다. <br/>
...
1. VHD를 Azure에 업로드 하 여 관리 디스크를 만듭니다.
    1. [Azure에 VHD 업로드 또는 다른 지역으로 관리 디스크 복사](../virtual-machines/windows/disks-upload-vhd-to-managed-disk-powershell.md)에 설명 된 대로 명령줄에서 Storage 탐색기 또는 AzCopy를 사용할 수 있습니다.        
    컴퓨터가 절전 또는 잠금으로 전환 되 면 업로드 프로세스가 중단 되 고 실패할 수 있습니다.
    1. 이 단계의 결과는 이제 Azure Portal에서 볼 수 있는 관리 디스크를 갖게 됩니다. 
        Azure Portal의 "Size\Performance" 탭을 사용 하 여 디스크 크기를 선택할 수 있습니다. 앞서 언급 했 듯이 크기는 128 > 하지 않아야 합니다.
1. 관리 디스크의 스냅숏을 만듭니다.
    [포털 또는 powershell을 사용 하 여 스냅숏 만들기](../virtual-machines/windows/snapshot-copy-managed-disk.md)에 설명 된 대로 powershell에서 Azure Portal를 사용 하거나 Storage 탐색기 내에서이 작업을 수행할 수 있습니다.
...
1. Azure Lab Services에서 랩을 만들고 공유 이미지 갤러리에서 사용자 지정 이미지를 선택 합니다.
    ...
    원래 Hyper-v VM에 OS가 설치 된 후 디스크를 확장 한 경우 할당 되지 않은 디스크 공간을 사용 하려면 Windows에서 C 드라이브를 확장 해야 합니다. 이렇게 하려면 랩을 만든 후에 템플릿 VM에 로그인 한 다음, [기본 볼륨 확장](/windows-server/storage/disk-management/extend-a-basic-volume)에 표시 되는 것과 유사한 단계를 수행 합니다. UI를 통해 PowerShell을 사용 하는 것 외에도이 작업을 수행할 수 있는 옵션이 있습니다.
