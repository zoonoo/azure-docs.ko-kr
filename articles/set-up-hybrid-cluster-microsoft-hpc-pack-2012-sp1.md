...
이 자습서는 이전에 계산 클러스터나 HPC 팩을 사용한 경험이 없다고 가정합니다. 데모를 위해 하이브리드 계산 클러스터를 신속하게 배포하도록 도와주는 역할만 합니다. 하이브리드 HPC 팩 클러스터를 프로덕션 환경에 대규모로 배포하는 단계와 고려 사항은 [자세한 지침][]을 참조하십시오.

> [WACOM.NOTE] Azure는 각 작업에 적합한 [다양한 크기][]를 계산 리소스에 제공합니다. 예를 들어 A8 및 A9 인스턴스는 특정 HPC 응용 프로그램에 필요한 낮은 대기 시간, 높은 처리량의 응용 프로그램 네트워크 액세스와 고성능을 결합합니다. 자세한 내용은 [A8 및 A9 계산 집약적인 인스턴스 정보][]를 참조하세요.
...
> [WACOM.NOTE] 이 자습서를 완료하려면 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 계정 만들기][](영문)를 참조하세요.

또한 이 자습서에는 다음이 필요합니다.

-   Windows Server 2012 R2 또는 Windows Server 2012 버전이 실행되는 온-프레미스 컴퓨터. 이 컴퓨터는 HPC 클러스터의 헤드 노드입니다. Windows Server가 아직 실행되지 않는 경우 [평가판][]을 다운로드하고 설치할 수 있습니다.
...
-   무료로 사용할 수 있는 HPC Pack 2012 R2용 설치 파일. 전체 설치 패키지를 [다운로드][]하고 헤드 노드 컴퓨터나 네트워크 위치에 파일을 복사합니다. Windows Server 설치와 동일한 언어의 설치 파일을 선택합니다.
...
[관리 포털][]을 사용하여 Azure 구독으로 다음 단계를 수행합니다. 이러한 단계는 나중에 온-프레미스 헤드 노드에서 Azure 노드를 배포하는 데 필요합니다.
...
-   Azure 저장소 계정 만들기
    ...
    > [WACOM.NOTE]또한 나중에 필요한 Azure 구독 ID를 기록해 둡니다. Azure [계정 정보][]에서 확인할 수 있습니다.
...
> [WACOM.NOTE]성능을 위해 동일한 지역에 클라우드 서비스와 저장소 계정을 만듭니다.
...
10. Create Node Template Wizard에서 다음을 수행합니다.
    ...
    c. **Provide Subscription Information** 페이지에서 Azure [계정 정보][]에 제공된 Azure 구독 ID를 입력합니다. **Management certificate**에서 **Browse**를 클릭하고 **Default HPC Azure Management**를 선택합니다. 그런 후 **다음**을 클릭합니다.

    ![노드 템플릿][1]

    d. **Provide Service Information** 페이지에서 이전 단계를 통해 만든 클라우드 서비스와 저장소 계정을 선택합니다. 그런 후 **다음**을 클릭합니다.
...
3.  **Specify New Nodes** 페이지에서 이전에 만든 Azure 노드 템플릿(기본적으로 **Default AzureNode Template**)을 선택합니다. 크기가 **Small**인 노드 **2**개를 지정하고 **Next**를 클릭합니다.
    ...
    사용 가능한 가상 컴퓨터 크기에 대한 자세한 내용은 [Azure를 위한 가상 컴퓨터 및 클라우드 서비스 크기][]를 참조하십시오.
...
4.  역할 인스턴스가 실행되고 있는지 확인하려면 [관리 포털][]에서 **클라우드 서비스**를 클릭하고 클라우드 서비스의 이름을 클릭한 후 **인스턴스**를 클릭합니다.
...
4.  역할 인스턴스가 Azure에서 더 이상 실행되지 않는지 확인하려면 [관리 포털][]에서 **클라우드 서비스**를 클릭하고 클라우드 서비스의 이름을 클릭한 후 **인스턴스**를 클릭합니다. 프로덕션 환경에는 인스턴스가 배포되지 않습니다.
...
-   [HPC Pack 2012 R2 및 HPC Pack 2012][]
-   [Microsoft HPC 팩을 사용하여 Azure로 버스트][자세한 지침]
-   [Azure 노드에 응용 프로그램 배포][]
-   [Azure VM의 Microsoft HPC 팩][]

  [하이브리드 HPC 클러스터]: ./media/set-up-hybrid-cluster-microsoft-hpc-pack-2012-sp1/hybrid_cluster_overview.png
  [자세한 지침]: http://go.microsoft.com/fwlink/p/?LinkID=200493
  [다양한 크기]: http://go.microsoft.com/fwlink/p/?LinkId=389844
  [A8 및 A9 계산 집약적인 인스턴스 정보]: http://go.microsoft.com/fwlink/p/?Linkid=328042
  [필수 조건]: #BKMK_Prereq
  ...
  [평가판]: http://technet.microsoft.com/evalcenter/dn205286.aspx
  [다운로드]: http://go.microsoft.com/fwlink/p/?linkid=389557
  ...
  [계정 정보]: [https://account.windowsazure.com/Subscriptions
  ...
  [Azure를 위한 가상 컴퓨터 및 클라우드 서비스 크기]: http://msdn.microsoft.com/library/windowsazure/dn197896.aspx
