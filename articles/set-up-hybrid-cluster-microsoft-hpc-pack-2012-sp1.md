...
4. **Microsoft Software User Agreement** 페이지에서 **다음**를 클릭합니다.
...
1. 헤드 노드에서 HPC Cluster Manager를 시작합니다. **헤드 노드 선택** 대화 상자가 표시되면 **로컬 컴퓨터**를 클릭합니다. **배포 할 일 모음**가 나타납니다.

2. **필수 배포 작업**에서 **네트워크 구성**를 클릭합니다.

	![Configure Network][config_hpc2]

3. 네트워크 구성 마법사에서 **모든 노드가 엔터프라이즈 네트워크에만 있음**(토폴로지 5)를 선택합니다.
...
4. 마법사의 나머지 페이지에서 **다음**를 클릭하여 기본값을 적용합니다. 그런 다음 **검토** 탭에서 **구성**를 클릭하여 네트워크 구성을 완료합니다.

5. **배포 할 일 모음**에서 **설치 자격 증명 제공**를 클릭합니다. 

6. **설치 자격 증명** 대화 상자에서 HPC 팩을 설치하는 데 사용한 도메인 계정의 자격 증명을 입력합니다. 그런 후 **확인**를 클릭합니다.

	![Installation Credentials][config_hpc6]

	>[WACOM.NOTE]HPC 팩 서비스는 설치 자격 증명을 사용하여 온-프레미스 계산 노드만 배포합니다.
	
7. **배포 할 일 모음**에서 **새 노드의 명명 구성**를 클릭합니다. 

8. **노드 명명 시리즈 지정** 대화 상자에서 기본 이름 지정 시리즈를 적용하고 **확인**를 클릭합니다.

	![Node Naming][config_hpc8]

	>[WACOM.NOTE]이 이름 지정 시리즈는 도메인에 가입된 계산 노드의 이름만 생성합니다. Azure 노드의 이름은 자동으로 지정됩니다.
	  
9. **배포 할 일 모음**에서 **노드 템플릿 만들기**을 클릭합니다. 노드 템플릿을 사용하여 클러스터에 Azure 노드를 추가합니다.

10. 노드 템플릿 만들기 마법사에서 다음을 수행합니다.

	a. **노드 템플릿 유형 선택** 페이지에서 **Azure 노드 템플릿**을 클릭한 후 **다음**를 클릭합니다.

	![Node Template][config_hpc10]

	b. **다음**를 클릭하여 기본 템플릿 이름을 적용합니다.

	c. **구독 정보 제공** 페이지에서 Azure <a href="[https://account.windowsazure.com/Subscriptions">계정 정보</a>에 제공된 Azure 구독 ID를 입력합니다. **관리 인증서**에서 **찾아보기**를 클릭하고 **Default HPC Azure Management**를 선택합니다. 그런 후 **다음**를 클릭합니다.

	![Node Template][config_hpc12]

	d. **서비스 정보 제공** 페이지에서 이전 단계에 만든 클라우드 서비스와 저장소 계정을 선택합니다. 그런 후 **다음**를 클릭합니다.

	![Node Template][config_hpc13]

	e. 마법사의 나머지 페이지에서 **다음**를 클릭하여 기본값을 적용합니다. **검토** 탭에서 **만들기**를 클릭하여 노드 템플릿을 만듭니다.
...
1. HPC Cluster Manager의 **노드 관리**에 있는 **작업** 창에서 **를 클릭합니다**를 클릭합니다. 

	![Add Node][add_node1]

2. Add Node Wizard의 **배포 방법 선택** 페이지에서 **Azure 노드 추가**를 클릭한 후 **다음**를 클릭합니다.

	![Add Azure Node][add_node1_1]

3. **새 노드 지정** 페이지에서 이전에 만든 Azure 노드 템플릿(기본적으로 **기본 Azure 노드 템플릿**)을 선택합니다. 크기가 **작음**인 노드 **2**개를 지정하고 **다음**를 클릭합니다.
...
4. **노드 추가 마법사 완료** 페이지에서 **마침**를 클릭합니다. 
...
1.	HPC Cluster Manager의 **노드 관리**에서 노드 중 하나 또는 둘 다 클릭하고 **작업** 창에서 **시작**를 클릭합니다. 

	![Start Nodes][add_node4]

2. **Azure 노드 시작** 대화 상자에서 **시작**를 클릭합니다.
...
2. **새 매개 변수 스윕 작업** 대화 상자의 **명령줄**에 `set /a *+*`를 입력합니다(표시되는 기본 명령줄을 덮어씀). 나머지 설정은 기본값을 그대로 두고 **제출**를 클릭하여 작업을 제출합니다.
...
1. HPC Cluster Manager의 **노드 관리** 에서 Azure 노드를 둘 다 선택합니다. **작업** 창에서 **중지**을 클릭합니다. 

	![Stop Nodes][stop_node1]

2. **Azure 노드 중지** 대화 상자에서 **중지**을 클릭합니다.

	![Stop Nodes][stop_node2]

3. 노드가 **중지 중** 상태로 전환됩니다. 몇 분 후에 HPC Cluster Manager에서 노드가 **배포되지 않음**로 표시됩니다.
