<properties 
	pageTitle="Microsoft HPC 팩을 사용하여 하이브리드 계산 클러스터 설정" 
...
이 자습서는 이전에 계산 클러스터나 HPC 팩을 사용한 경험이 없다고 가정합니다. 데모를 위해 하이브리드 계산 클러스터를 신속하게 배포하도록 도와주는 역할만 합니다. 하이브리드 HPC 팩 클러스터를 프로덕션 환경에 대규모로 배포하는 단계와 고려 사항은 [자세한 지침](http://go.microsoft.com/fwlink/p/?LinkID=200493)을 참조하세요. Azure에서 전체 HPC 팩 클러스터를 설정하려면 [Azure VM의 Microsoft HPC 팩](http://go.microsoft.com/fwlink/p/?linkid=330375)을 참조하세요. 

>[AZURE.NOTE] Azure는 각 작업에 적합한 [다양한 크기](http://go.microsoft.com/fwlink/p/?LinkId=389844)를 계산 리소스에 제공합니다. 예를 들어 A8 및 A9 인스턴스는 특정 HPC 응용 프로그램에 필요한 낮은 대기 시간, 높은 처리량의 응용 프로그램 네트워크 액세스와 고성능을 결합합니다. 자세한 내용은 [A8 및 A9 계산 집약적인 인스턴스 정보](http://go.microsoft.com/fwlink/p/?Linkid=328042)를 참조하세요. 
...
* Windows Server 2012 R2 또는 Windows Server 2012 버전을 실행 중인 온-프레미스 컴퓨터. 이 컴퓨터는 HPC 클러스터의 헤드 노드가 됩니다. Windows Server가 아직 실행되지 않는 경우 [평가판](http://technet.microsoft.com/evalcenter/dn205286.aspx)을 다운로드하고 설치할 수 있습니다.
...
* 무료로 사용할 수 있는 HPC Pack 2012 R2용 설치 파일. 최신 버전은 HPC Pack 2012 R2 업데이트 1입니다. [전체 설치 패키지를](http://go.microsoft.com/fwlink/p/?linkid=328024) 다운로드하고 헤드 노드 컴퓨터나 네트워크 위치에 파일을 복사합니다. Windows Server 설치와 동일한 언어의 설치 파일을 선택합니다.
...
3. **HPC Pack 2012 R2 설치** 화면에서 **새 설치 또는 기존 설치에 새 기능 추가**를 클릭합니다.

	![HPC Pack 2012 Setup][install_hpc1]

4. **Microsoft 소프트웨어 사용자 계약 페이지**에서 **다음**을 클릭합니다.

5. **설치 종류 선택** 페이지에서 **헤드 노드를 만들어 새 HPC 클러스터 만들기**를 클릭한 후 **다음**을 클릭합니다.

	![Select Installation Type][install_hpc2]

6. 마법사에서 여러 개의 사전 설치 테스트를 실행합니다. 테스트를 모두 통과한 경우 **설치 규칙** 페이지에서 **다음**을 클릭합니다. 그렇지 않으면 제공된 정보를 검토하고 필요에 따라 환경을 변경합니다. 다시 테스트를 실행하거나 필요한 경우 Installation Wizard를 다시 시작합니다. 

	![Installation Rules][install_hpc3]

7. **HPC DB 구성** 페이지에서 모든 HPC 데이터베이스에 대해 **헤드 노드**가 선택되었는지 확인하고 **다음**을 클릭합니다.

	![DB Configuration][install_hpc4]

8. 마법사의 나머지 페이지에서 기본 선택을 적용합니다. **필수 구성 요소 설치** 페이지에서 **설치**를 클릭합니다.

	![Install][install_hpc6]

9. 설치가 완료되면 **HPC 클러스터 관리자 시작**을 선택 취소하고 **마침**을 클릭합니다. 이후 단계에서 HPC Cluster Manager를 시작하여 헤드 노드 구성을 완료할 것입니다.
...
1. 헤드 노드에서 HPC Cluster Manager를 시작합니다. **헤드 노드 선택** 대화 상자가 표시되면 **로컬 컴퓨터**를 클릭합니다. **배포 할 일 모음**이 나타납니다.

2. **필수 배포 작업**에서 **네트워크 구성**을 클릭합니다.

	![Configure Network][config_hpc2]

3. 네트워크 구성 마법사에서 **모든 노드가 엔터프라이즈 네트워크에만 있음**(토폴로지 5)을 선택합니다.
...
4. 마법사의 나머지 페이지에서 **다음**을 클릭하여 기본값을 적용합니다. **검토** 탭에서 **구성**을 클릭하여 네트워크 구성을 완료합니다.

5. **배포 할 일 모음**에서 **설치 자격 증명 제공**을 클릭합니다. 

6. **설치 자격 증명** 대화 상자에서 HPC 팩을 설치하는 데 사용한 도메인 계정의 자격 증명을 입력합니다. 그런 후 **확인**을 클릭합니다.

	![Installation Credentials][config_hpc6]

	>[AZURE.NOTE]HPC 팩 서비스는 설치 자격 증명을 사용하여 온-프레미스 계산 노드만 배포합니다.
	
7. **배포 할 일 모음**에서 **새 노드의 명명 구성**을 클릭합니다. 

8. **노드 명명 시리즈 지정** 대화 상자에서 기본 이름 지정 시리즈를 적용하고 **확인**을 클릭합니다.

	![Node Naming][config_hpc8]

	>[AZURE.NOTE]이 이름 지정 시리즈는 도메인에 가입된 계산 노드의 이름만 생성합니다. Azure 노드의 이름은 자동으로 지정됩니다.
	  
9. **배포 할 일 모음**에서 **노드 템플릿 만들기**를 클릭합니다. 노드 템플릿을 사용하여 클러스터에 Azure 노드를 추가합니다.

10. 노드 템플릿 만들기 마법사에서 다음을 수행합니다.
	...
	d. **서비스 정보 제공** 페이지에서 이전 단계를 통해 만든 클라우드 서비스와 저장소 계정을 선택합니다. 그런 후 **다음**을 클릭합니다.

	![Node Template][config_hpc13]

	e. 마법사의 나머지 페이지에서 **다음**을 클릭하여 기본값을 적용합니다. **검토** 탭에서 **만들기**를 클릭하여 노드 템플릿을 만듭니다.
...
1. HPC Cluster Manager의 **노드 관리**에 있는 **작업** 창에서 **노드 추가**를 클릭합니다. 

	![Add Node][add_node1]

2. 노드 추가 마법사의 **배포 방법 선택** 페이지에서 **Azure 노드 추가**를 클릭한 후 **다음**을 클릭합니다.

	![Add Azure Node][add_node1_1]

3. **새 노드 지정** 페이지에서 이전에 만든 Azure 노드 템플릿(기본적으로 **기본 Azure 노드 템플릿**)을 선택합니다. 크기가 **작음**인 노드 **2**개를 지정하고 **다음**을 클릭합니다.

	![Specify Nodes][add_node2]

	사용 가능한 가상 컴퓨터 크기에 대한 자세한 내용은 [Azure를 위한 가상 컴퓨터 및 클라우드 서비스 크기](http://msdn.microsoft.com/library/windowsazure/dn197896.aspx)를 참조하세요.

4. **노드 추가 마법사 완료** 페이지에서 **마침**을 클릭합니다. 
...
1.	HPC Cluster Manager의 **노드 관리**에서 노드 중 하나 또는 둘 다 클릭하고 **작업** 창에서 **시작**을 클릭합니다. 

	![Start Nodes][add_node4]

2. **Azure 노드 시작** 대화 상자에서 **시작**을 클릭합니다.

	![Start Nodes][add_node5]
 
	노드가 **프로비전** 상태로 전환됩니다. 프로비전 로그를 보고 프로비전 진행 상태를 추적할 수 있습니다.

	![Provision Nodes][add_node6]

3. 몇 분 후에 Azure 노드가 프로비전을 완료하고 **오프라인** 상태로 전환됩니다. 이 상태에서는 역할 인스턴스가 실행되지만 클러스터 작업을 수락하지 않습니다.
...
5. 클러스터 작업을 실행하기 위해 Azure 노드를 온라인 상태로 전환하려면 노드를 선택하고 마우스 오른쪽 단추를 클릭한 후 **온라인 상태로 전환**을 클릭합니다.
	...
	HPC Cluster Manager에서 노드가 **온라인** 상태로 표시됩니다.
...
1. HPC Cluster Manager의 **작업 관리**에 있는 **작업** 창에서 **새 매개 변수 스윕 작업**을 클릭합니다.

	![New Job][test_job1]

2. **새 매개 변수 스윕 작업** 대화 상자의 **명령줄**에 `set /a *+*`를 입력합니다(표시되는 기본 명령줄을 덮어씀). 나머지 설정은 기본값을 그대로 두고 **제출**을 클릭하여 작업을 제출합니다.

	![Parametric Sweep][param_sweep1]

3. 작업이 완료되면 **내 스윕 작업** 작업을 두 번 클릭합니다.

4. **작업 보기**를 클릭한 후 하위 작업을 클릭하여 해당 하위 작업의 계산된 출력을 표시합니다.

	![Task Results][view_job361]

5. 하위 작업에 대한 계산을 수행한 노드를 보려면 **할당된 노드**를 클릭합니다. 클러스터에서 다른 노드 이름이 표시할 수도 있습니다.
...
1. HPC Cluster Manager의 **노드 관리**에서 Azure 노드를 둘 다 선택합니다. **작업** 창에서 **중지**를 클릭합니다. 

	![Stop Nodes][stop_node1]

2. **Azure 노드 중지** 대화 상자에서 **중지**를 클릭합니다.

	![Stop Nodes][stop_node2]

3. 노드가 **중지 중** 상태로 전환됩니다. 몇 분 후에 HPC Cluster Manager에서 노드가 **배포되지 않음**으로 표시됩니다.
