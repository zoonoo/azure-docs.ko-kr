...
[이 자습서](../custom-event-quickstart-portal.md)를 수행 하 여 클라우드에서 event grid 토픽 및 구독을 만듭니다. 자습서의 뒷부분에서 사용할 새로 만든 토픽의 `topicURL`, `sasKey`및 `topicName`를 적어둡니다.

예를 들어 미국 서 부에 `testegcloudtopic` 라는 토픽을 만든 경우 값은 다음과 같습니다.
