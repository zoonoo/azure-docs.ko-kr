...
Event Grid json 페이로드의 모든 속성에 필터를 지정할 수 있습니다. 이러한 필터는 각각의 외부 조건과 선택적인 내부 `OR` 조건이 있는 `AND` 조건 집합으로 모델링 됩니다. 각 `AND` 조건에 대해 다음 값을 지정 합니다.

* `OperatorType`-비교의 유형입니다.
* `Key`-필터를 적용할 속성의 json 경로입니다.
* `Value`-필터가 실행 되는 데 사용할 참조 값 (또는) `Values`-필터가 실행 되는 참조 값의 집합입니다.
...
앞에서 제공 된 json 예제에서 `AdvancedFilters`은 배열입니다. 각 `AdvancedFilter` 배열 요소를 `AND` 조건으로 간주 합니다.

여러 값 (예: `NumberIn`, `NumberNotIn`, `StringIn`등)을 지 원하는 연산자의 경우 각 값은 `OR` 조건으로 처리 됩니다. 따라서 `StringBeginsWith("a", "b", "c")` `a` 또는 `b` 또는 `c`로 시작 하는 임의의 문자열 값과 일치 합니다.

> [!CAUTION]
> NOT 연산자-`NumberNotIn` 및 `StringNotIn`는 `Values` 필드에 지정 된 각 값의 및 조건으로 동작 합니다.
>
...
`Key` 속성은 잘 알려진 최상위 속성 이거나 여러 점이 있는 json 경로 일 수 있습니다. 여기서 각 점은 중첩 된 json 개체의 단계별 실행을 의미 합니다.
...
Event Grid는 페이로드에 봉투 (envelope) 스키마를 적용 하지 않으므로 사용자 지정 이벤트 스키마의 `Key`에 대 한 제한은 없습니다.
