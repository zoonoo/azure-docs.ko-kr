---
title: Azure Event Grid에서 이벤트 도메인을 사용하여 대규모 토픽 집합을 관리하고 토픽에 이벤트를 게시하는 방법
description: Azure Event Grid에서 이벤트 도메인을 사용하여 토픽을 생성 및 관리하고 토픽에 이벤트를 게시하는 방법을 보여줍니다.
services: event-grid
author: banisadr
...
이벤트 도메인 만들기는 [Azure CLI 2.0](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)용 `eventgrid` 확장을 통해 수행할 수 있습니다. 도메인을 만든 후에는 대규모 토픽 집합을 관리하는 데 사용할 수 있습니다.
...
`endpoint` 및 `id`를 기록해 둡니다. 도메인 및 게시 이벤트를 관리하는 데 필요합니다.
...
도메인에서 토픽을 구독하는 방법은 다른 Azure 리소스를 구독하는 방법과 동일합니다.
...
제공된 리소스 ID는 앞에서 도메인을 만들 때 반환된 것과 동일한 ID입니다. 구독할 토픽을 지정하려면 리소스 ID의 끝에 `/topics/<my-topic>`을 추가합니다.

도메인의 모든 이벤트를 수신하는 도메인 범위 이벤트 구독을 만들려면 토픽을 지정하지 말고 도메인을 `resource-id`로 제공하세요(예: `/subscriptions/<sub-id>/resourceGroups/<my-resource-group>/providers/Microsoft.EventGrid/domains/<my-domain-name>`).
...
다음 명령은 `foo` 토픽에만 이벤트 구독을 만들고 삭제하도록 `alice@contoso.com`을 제한합니다.
