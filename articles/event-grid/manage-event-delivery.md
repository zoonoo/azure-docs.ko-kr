---
title: Azure Event Grid 구독에 대한 전송 설정 관리
description: Event Grid에 대한 이벤트 전송 옵션을 사용자 지정하는 방법을 설명합니다.
services: event-grid
author: tfitzmac
manager: timlt
...
Event Grid 구독을 만들 때 Event Grid에서 이벤트 전송을 시도할 기간의 값을 설정할 수 있습니다. 기본적으로, Event Grid는 24시간(1440분) 동안 최대 30회 시도합니다. Event Grid 구독에 대해 이러한 값 중 하나를 설정할 수 있습니다.
...
Event Grid는 모든 재시도 횟수를 시도했거나 전송이 성공할 수 없음을 나타내는 오류 메시지를 수신하는 경우, 이벤트를 배달 못한 편지 위치로 보냅니다. 예를 들어 Event Grid에서 이벤트를 전송할 때 잘못된 형식 오류를 수신할 경우, 해당 이벤트를 배달 못한 편지 위치로 즉시 보냅니다.
