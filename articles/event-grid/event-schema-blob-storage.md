...
이러한 이벤트는 클라이언트가 Blob REST Api를 호출 하 여 blob을 만들거나 바꾸거나 삭제할 때 트리거됩니다.
 ...
 |이벤트 이름 |설명|
 |----------|-----------|
 |**Microsoft.Storage.BlobCreated** |Blob을 만들거나 바꿀 때 트리거됩니다. <br>특히이 이벤트는 클라이언트가 Blob REST API에서 사용할 수 있는 `PutBlob`, `PutBlockList` 또는 `CopyBlob` 작업을 사용 하는 경우 트리거됩니다.   |
...
> 이러한 이벤트는 공개 미리 보기 상태 이며 **미국 서 부 2** 및 미국 **서 부** 지역 에서만 사용할 수 있습니다.
 ...
 |이벤트 이름|설명|
 |----------|-----------|
 |**Microsoft.Storage.BlobCreated** | Blob을 만들거나 바꿀 때 트리거됩니다. <br>특히이 이벤트는 클라이언트가 Azure Data Lake Storage Gen2 REST API에서 사용할 수 있는 `CreateFile` 및 `FlushWithClose` 작업을 사용 하는 경우 트리거됩니다. |
 ...
 |**Microsoft.Storage.BlobRenamed**|Blob의 이름을 바꾸면 트리거됩니다. <br>특히이 이벤트는 Azure Data Lake Storage Gen2 REST API에서 사용할 수 있는 `RenameFile` 작업을 클라이언트에서 사용할 때 트리거됩니다.|
 |**Microsoft.Storage.DirectoryCreated**|디렉터리를 만들 때 트리거됩니다. <br>특히이 이벤트는 Azure Data Lake Storage Gen2 REST API에서 사용할 수 있는 `CreateDirectory` 작업을 클라이언트에서 사용할 때 트리거됩니다.|
 |**Microsoft.Storage.DirectoryRenamed**|디렉터리의 이름을 바꾸면 트리거됩니다. <br>특히이 이벤트는 Azure Data Lake Storage Gen2 REST API에서 사용할 수 있는 `RenameDirectory` 작업을 클라이언트에서 사용할 때 트리거됩니다.|
 |**Microsoft.Storage.DirectoryDeleted**|디렉터리가 삭제 되 면 트리거됩니다. <br>특히이 이벤트는 Azure Data Lake Storage Gen2 REST API에서 사용할 수 있는 `DeleteDirectory` 작업을 클라이언트에서 사용할 때 트리거됩니다.|
...
* @No__t-0 키가-1 @no__t의 값으로 설정 되어 있습니다.

* @No__t-0 키는 문자열 `CreateFile` 또는 `FlushWithClose`로 설정 됩니다.

* @No__t-0 키는 데이터 집합에 포함 됩니다.
...
* @No__t-0 키는 문자열 `DeleteFile`로 설정 됩니다.

* @No__t-0 키는-1 @no__t 경로를 포함 합니다.
...
| sourceUrl |string | 작업 이전에 존재 하는 파일의 url입니다. 예를 들어 파일의 이름을 바꾸면 이름 바꾸기 작업 전의 원래 파일 이름에 대 한 url이 `sourceUrl`으로 포함 됩니다. <br>계층 네임 스페이스가 있는 blob storage 계정에서 트리거되는 이벤트에 대해서만 나타납니다. |
| url | string | Blob에 대한 경로입니다. <br>클라이언트에서 Blob REST API 사용 하는 경우 url의 구조는 *@no__t -1storage-account-name\>.blob.core.windows.net/\<container-name @ no__t-4 @ no__t-5 @ no__t-6file-name @ no__t-7*입니다. <br>클라이언트에서 Data Lake Storage REST API를 사용 하는 경우 url의 구조는 다음과 같습니다. *@no__t -1storage-account-name\>.dfs.core.windows.net/@no__t--name @ no__t-4 @ no__t @ no__t-6file-name @ no__t-7*. |
| recursive | string | 모든 자식 디렉터리에서 작업을 수행 하려면-0을 @no__t 합니다. 그렇지 않으면-1 @no__t 합니다. <br>계층 네임 스페이스가 있는 blob storage 계정에서 트리거되는 이벤트에 대해서만 나타납니다. |
