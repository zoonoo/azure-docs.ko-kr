...
Azure에서 함수를 이벤트 처리기로 사용 하려면 다음 방법 중 하나를 수행 합니다. 

-   [Event Grid 트리거](../azure-functions/functions-bindings-event-grid-trigger.md)를 사용 합니다.  **Azure Function** 을 **끝점 형식** 으로 지정 합니다. 그런 다음 함수 앱 및 이벤트를 처리할 함수를 지정 합니다. 
-   [HTTP 트리거](../azure-functions/functions-bindings-http-webhook.md)를 사용 합니다.  **웹 후크** 를 **끝점 형식** 으로 지정 합니다. 그런 다음 이벤트를 처리할 함수에 대 한 URL을 지정 합니다. 
...
-   Event Grid는 함수가 이벤트를 처리할 수 있는 인지 비율에 따라 Event Grid 이벤트에 의해 트리거되는 함수에 이벤트가 전달 되는 속도를 자동으로 조정 합니다. 이 요율 일치 기능은 함수에서 이벤트 처리 속도로 이벤트를 처리할 수 없는 경우 발생 하는 배달 오류를 시간에 따라 averts 수 있습니다. 높은 처리량으로 효율성을 향상 시키려면 이벤트 구독에서 일괄 처리를 사용 하도록 설정 합니다. 자세한 내용은 [일괄 처리 사용](#enable-batching)을 참조 하세요.
    ...
    > [!NOTE]
    > 현재 **CloudEvents** 스키마에서 이벤트를 전달 하는 경우 함수 앱에 대 한 Event Grid 트리거를 사용할 수 없습니다. 대신, HTTP 트리거를 사용합니다.
