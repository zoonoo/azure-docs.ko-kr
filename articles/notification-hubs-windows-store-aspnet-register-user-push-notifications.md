...
1.  Visual Studio 2012에서 [알림 허브를 통해 사용자에게 알림](/en-us/manage/services/notification-hubs/notify-users-aspnet) 필수 자습서를 완료할 때 만든 프로젝트에서 app.xaml.cs 파일을 엽니다.
...
4.  왼쪽 창에서 **온라인** 범주를 선택하고, `json.net`을 검색하고, **Json.NET** 패키지에서 **설치**를 클릭한 다음, 라이선스 계약에 동의합니다.
...
1.  솔루션 탐색기에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **추가**를 클릭한 후 **클래스**를 클릭하고 `LocalStorageManager`를 입력한 후 **추가**를 클릭합니다.
...
2.  새 LocalStorageManager.cs 프로젝트 파일을 열고 다음 **using** 문을 추가합니다.

         using Windows.Storage;

3.  **LocalStorageManager** 클래스 정의를 다음 코드로 바꿉니다.
...
4.  MainPage.xaml 프로젝트 파일을 열고 루트 **Grid** 요소를 다음 XAML 코드로 바꿉니다.
...
5.  MainPage.xaml.cs 파일로 돌아가서 다음 **using** 지시문을 추가합니다.
