...
높은 수준의 보안을 보장 하기 위해 Notification Hubs은 2020 년 4 월 30 일에 TLS 버전 1.0 및 1.1에 대 한 지원을 사용 하지 않도록 설정 합니다. 이러한 이전 프로토콜은 약한 암호화를 제공 하며, 비스 트 및 POODLE 공격에 취약 합니다. Android 버전 5 이상 또는 iOS 버전 5 이상을 실행 하는 장치에 배포 된 응용 프로그램은 이러한 변경의 영향을 받지 않습니다. 이러한 운영 체제는 TLS 1.2을 지원 하 고 클라이언트와 서버는 상호 지원 되는 최고 버전의를 협상 합니다. 연결 시의 프로토콜입니다.
