...
이 가이드에서는 [Node.js](https://nodejs.org) 응용 프로그램에서 Azure 알림 허브를 사용하여 푸시 알림을 직접 보내는 방법을 보여 줍니다.
...
2. **Notification Hubs**를 선택한 다음 샘플로 사용하려는 허브를 찾습니다. 새 알림 허브를 만드는 데 도움이 필요한 경우 [Windows 스토어 시작 자습서를](notification-hubs-windows-store-dotnet-get-started-wns-push-notification.md) 참조할 수 있습니다.
3. **설정**을 선택합니다.
...
탑재하중 형식에 대한 자세한 내용은 [페이로드 설명서를](https://distriqt.github.io/ANE-PushNotifications/m.FCM-GCM%20Payload)참조하십시오.
...
- **TargetName** -  `toast` 알림 메시지 알림의 대상 이름입니다. `token` - 타일 알림 메시지인 경우.
