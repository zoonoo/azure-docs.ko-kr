---
title: "Azure Notification Hubs를 사용하여 속보 보내기(유니버설 Windows 플랫폼)"
...
이 항목에서는 Azure 알림 허브를 사용하여 Windows 스토어 또는 Windows Phone 8.1(비 Silverlight) 앱에 속보 알림을 브로드캐스트하는 방법을 보여 줍니다. Windows Phone 8.1 Silverlight를 대상으로 하는 경우 [Windows Phone](notification-hubs-windows-phone-push-xplat-segmented-mpns-notification.md) 버전을 참조하세요. 

이 프로세스를 완료한 후에는 관심 있는 속보 범주를 등록하고 해당 카테고리에 대해서만 푸시 알림을 받아볼 수 있습니다. 이 시나리오는 관심사를 지정한 사용자 그룹에게 알림을 보내는 많은 앱(예: RSS 리더, 음악 팬용 앱)에 일반적입니다. 

알림 허브에서 등록을 만들 때 하나 이상의 *태그*를 포함하여 브로드캐스트 시나리오를 사용할 수 있습니다. 태그에 알림이 전송되면 태그에 대해 등록된 모든 장치에서 알림을 받게 됩니다. 태그는 단순히 문자열이므로 사전에 설정할 필요가 없습니다. 태그에 대한 자세한 내용은 [Notification Hubs 라우팅 및 태그 식](notification-hubs-tags-segment-push-message.md)을 참조하세요.
...
이 항목은 [Notification Hubs 시작][get-started]에서 만든 앱을 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [Notification Hubs 시작하기][get-started]를 완료해야 합니다.
...
3. 다음 코드를 새 **Notifications** 클래스에 추가합니다.
    ...
    여러 템플릿을 등록하려 할 수 있으므로(예: 토스트 알림용 1개, 타일용 1개) 템플릿 이름(예: "simpleWNSTemplateExample")도 함께 제공합니다. 업데이트하거나 삭제할 수 있는 템플릿 이름을 지정합니다.
...
1. Visual Studio에서 **F5**를 눌러 앱을 컴파일 및 시작합니다.  
...
이 문서에서는 범주별로 속보를 브로드캐스트하는 방법에 대해 알아보았습니다. 이제 다른 고급 Notification Hubs 시나리오를 다루는 다음 자습서 중 하나를 완료해 보세요.

* [Notification Hubs를 사용하여 지역화된 속보 브로드캐스트] 이 자습서에서는 지역화된 알림을 보내도록 속보 앱을 확장하는 방법에 대해 논의합니다.
