...
이 자습서에서는 Azure Notification Hubs를 사용하여 UWP(유니버설 Windows 플랫폼) 애플리케이션을 실행하는 특정 Windows 디바이스에 알림을 푸시하는 방법을 알아봅니다. 이 자습서를 완료한 후에는 관심 있는 속보 범주를 등록하고 해당 카테고리에 대해서만 푸시 알림을 받아볼 수 있습니다.
...
3. 다음 코드를 새 `Notifications` 클래스에 복사합니다.
...
5. `MainPage.xaml.cs` 파일에서 다음 줄을 추가합니다.

    ```csharp
    using Windows.UI.Popups;
    ```

6. `MainPage.xaml.cs` 파일에서 다음 메서드를 추가합니다.
...
1. 범주 기반 구독에 `notifications` 클래스를 사용하려면 App.xaml.cs 파일을 연 다음, `InitNotificationsAsync` 메서드를 업데이트합니다.
    ...
    이 프로세스는 앱이 시작될 때 로컬 스토리지에서 범주를 검색하고, 이러한 범주에 대한 등록을 요청하게 합니다. `InitNotificationsAsync` 메서드는 [Notification Hubs 시작][get-started] 자습서의 일부로 만들어졌습니다.
2. `MainPage.xaml.cs` 프로젝트 파일에서 다음 코드를 `OnNavigatedTo` 메서드에 추가합니다.
...
이제 앱이 완료되었습니다. 이제 사용자가 범주 선택을 변경할 때 알림 허브에 등록하는 데 사용한 디바이스 로컬 스토리지에 범주 집합을 저장할 수 있습니다. 다음 섹션에서는 범주 알림을 이 앱에 보낼 수 있는 백 엔드를 정의합니다.
