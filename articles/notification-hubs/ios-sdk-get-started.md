...
2. **Keychain 액세스** 를 선택하고, **인증서 도우미** 를 확장한 다음, **인증 기관에서 인증서 요청** 을 선택합니다.
   ...
   :::image type="content" source="media/ios-sdk-get-started/image1.png" alt-text="인증 기관에서 인증서 요청 메뉴 옵션을 강조 표시하는 스크린샷.":::
...
3. **사용자 이메일 주소** 를 선택하고, **일반 이름** 값을 입력하고, **디스크에 저장됨** 을 지정했는지 확인한 다음, **계속** 을 선택합니다. **CA 이메일 주소** 는 필요하지 않으므로 비워둡니다.

   :::image type="content" source="media/ios-sdk-get-started/image2.png" alt-text="필요한 인증서 정보":::

4. **다른 이름으로 저장** 에 CSR 파일의 이름을 입력하고, **어디** 에서 위치를 선택한 다음, **저장** 을 선택합니다.
...
- [토큰 기반 인증](notification-hubs-push-notification-http2-token-authentication.md)에 사용할 수 있는 **.p8** 파일을 만듭니다(보다 새로운 접근 방법).
...
5. 다운로드한 푸시 인증서 **aps\_development.cer** 을 두 번 클릭합니다. 이 작업은 다음 이미지에 보이는 것처럼 새 인증서를 Keychain에 설치합니다.
...
8. [Visual Studio Code](https://code.visualstudio.com/)와 같이 선택한 적합한 애플리케이션에서 .p8 인증서를 연 다음, 키 값을 기록해 둡니다. 이는 **-----BEGIN PRIVATE KEY-----** 및 **-----END PRIVATE KEY-----** 사이의 값입니다.
...
3. 다음으로, **App ID** 드롭다운 목록에서 만든 앱 ID를 선택한 다음, **계속** 을 선택합니다.
...
8. 마지막으로, **프로비저닝 프로필 이름** 에서 프로필 이름을 선택한 다음, **생성** 을 선택합니다.

   :::image type="content" source="media/ios-sdk-get-started/image17.png" alt-text="프로비저닝 프로필 이름 선택":::

9. 새 프로비저닝 프로필이 만들어지면 **다운로드** 를 선택합니다. 저장된 위치를 기억합니다.
...
2. 왼쪽 메뉴에서 **모든 서비스** 를 선택하고, **모바일** 섹션에서 **Notification Hubs** 를 선택합니다. 서비스 이름 옆의 별표 아이콘을 선택하여 서비스를 왼쪽 메뉴의 **즐겨찾기** 섹션에 추가합니다. **Notification Hubs** 를 **즐겨찾기** 에 추가한 후 선택합니다.
...
**Notification Services** 에서 **Apple(APNS)** 을 선택한 다음, 이전에 [Notification Hubs에 대한 인증서 만들기](#create-a-certificate-for-notification-hubs) 섹션에서 선택한 접근 방식에 따라 적절한 단계를 수행합니다.
