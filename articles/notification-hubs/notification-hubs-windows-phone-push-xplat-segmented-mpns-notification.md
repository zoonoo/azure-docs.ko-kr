...
1. `MainPage.xaml` 파일을 연 다음, `TitlePanel` 및 `ContentPanel`이라는 `Grid` 요소를 다음 코드로 바꿉니다.
    ...
    ```
2. `Notifications`라는 클래스를 프로젝트에 추가합니다. `public` 한정자를 클래스 정의에 추가합니다. 그런 다음, 다음 `using` 문을 새 파일에 추가합니다.
...
5. `MainPage.xaml.cs`에서 다음 줄을 추가합니다.
