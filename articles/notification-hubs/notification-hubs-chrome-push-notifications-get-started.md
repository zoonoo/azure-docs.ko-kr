...
이 문서에서는 Azure Notification Hubs를 사용하여 Google Chrome 브라우저의 컨텍스트 내에서 표시되는 Chrome 앱에 푸시 알림을 보내는 방법을 보여줍니다. 이 자습서에서는 [GCM(Google Cloud Messaging)](https://developers.google.com/cloud-messaging/)을 사용하여 푸시 알림을 받는 Chrome 앱을 만듭니다. 
...
2. 만든 프로젝트의 **프로젝트** 페이지에서 **프로젝트 번호**를 적어 둡니다. 이 프로젝트 번호를 Chrome 앱에서 GCM에 등록하기 위한 **GCM 보낸 사람 ID** 로 사용합니다.
...
5. 서버 **API 키**값을 적어 둡니다. 이 값을 사용하여 다음 섹션에서 알림 허브를 구성하여 GCM에 푸시 알림을 보낼 수 있습니다.
...
6.   **설정** 페이지에서 **알림 서비스**를 선택한 다음, **Google(GCM)**을 선택합니다. API 키를 입력하고 저장합니다.
...
[Chrome App GCM 샘플]을 기반으로 하는 다음 샘플에서는 권장되는 방식으로 Chrome 앱을 만듭니다. 이 섹션에서는 Azure Notification Hubs에만 적용되는 단계를 중점적으로 설명합니다. 
...
1. 폴더를 만들고 이름을 `ChromePushApp`로 지정합니다. 이름은 임의입니다. 이름을 달리 지정할 경우 필요한 코드 세그먼트에서 경로를 대체해야 합니다.
...
3. `manifest.json` 파일을 만듭니다. 모든 Chrome 앱은 앱 메타데이터 및 사용자가 설치하는 경우 앱에 부여된 권한을 모두 포함하는 매니페스트 파일에서 지원됩니다.
    ...
    이 Chrome 앱이 GCM에서 푸시 알림을 받을 수 있도록 지정하는 `permissions` 요소를 확인합니다. 이 요소는 Chrome 앱이 등록을 위한 REST 호출을 수행하는 Azure Notification Hubs URI도 지정해야 합니다.
    또한 샘플 앱에는 원본 GCM 샘플에서 다시 사용되는 소스에 들어 있는 `gcm_128.png`라는 아이콘 파일도 사용됩니다. [아이콘 조건](https://developer.chrome.com/apps/manifest/icons)에 맞는 이미지로 대체할 수 있습니다.
...
6. 이 단계의 코드로 `register.js`라는 파일을 만듭니다. 이 파일은 `register.html`의 스크립트를 지정합니다. Chrome 앱은 인라인 실행을 허용하지 않으므로 UI에 대해 별도의 지원 스크립트를 만들어야 합니다.
    ...
    스크립트에는 다음과 같은 주요 매개 변수가 있습니다.
   
   * **window.onload** 는 UI에서 두 단추의 단추 클릭 이벤트를 정의합니다. 첫 번째 단추 클릭 이벤트 처리기는 GCM에 등록되고 다른 단추는 GCM에 등록한 후에 반환되는 등록 ID를 사용하여 Azure Notification Hubs에 등록됩니다.
   ...
   * **sendNHRegistrationRequest**는 Azure Notification Hubs에 HTTP REST 호출을 수행하는 함수입니다.
   * **registrationPayload** 는 등록 XML 페이로드를 정의합니다. 자세한 내용은 [등록 NH REST API 만들기]를 참조하세요. 여기서는 GCM에서 받은 정보를 사용하여 등록 ID를 업데이트해야 합니다.
   * **client**는 응용 프로그램이 HTTP POST 요청을 수행하는 데 사용하는 **XMLHttpRequest** 인스턴스입니다. `sasToken`으로 `Authorization` 헤더를 업데이트합니다. 이 호출이 정상적으로 완료되면 이 Chrome 앱 인스턴스가 Azure Notification Hubs에 등록됩니다.
...
3. Chrome 앱이 오류 없이 만들어지면 Chrome 앱이 다음과 같이 표시됩니다.
...
> [!NOTE]
> 공용 <a href="http://msdn.microsoft.com/library/windowsazure/dn223264.aspx">REST 인터페이스</a>를 통해 백 엔드에서 Notification Hubs를 사용하여 푸시 알림을 보낼 수 있습니다. 더 많은 플랫폼 간 예제는 [설명서 포털](https://azure.microsoft.com/documentation/services/notification-hubs/)을 참조하세요.
> 
> 
...
2. **도구** 메뉴에서 **NuGet 패키지 관리자**, **패키지 관리자 콘솔**을 차례로 클릭합니다. 맨 아래 창에 패키지 관리자 콘솔이 표시됩니다.
