...
4. 그런 다음 이 페이지에서 **创建应用**(**응용 프로그램 만들기**)를 클릭합니다.

    ![Baidu 응용 프로그램 만들기](./media/notification-hubs-baidu-get-started/BaiduCreateApplication.png)

5. 다음 페이지에서 创建新应用(**새 응용 프로그램 만들기**)를 클릭합니다.
   
    ![Baidu 새 응용 프로그램 만들기](./media/notification-hubs-baidu-get-started/BaiduCreateNewApplication.png)

6. 응용 프로그램 이름을 입력하고 创建(**만들기**)를 클릭합니다.
...
&emsp;&emsp;![Azure Notification Hubs - Baidu 비밀](./media/notification-hubs-baidu-get-started/NotificationHubBaiduConfigure.png)
...
2.  응용 프로그램 이름을 입력하고 Minimum Required SDK 버전이 API 16: Android 4.1인지 확인합니다. **또한 패키지 이름(应用包名)이 Baidu 클라우드 푸시 포털과 동일한지 확인하세요.**
...
3.  다음을 클릭하고 만들기 작업 창이 표시될 때까지 마법사를 계속 진행합니다. 빈 작업이 선택되었는지 확인하고 마침을 선택하여 새로운 Android 응용 프로그램을 만듭니다.
...
5.  그런 다음 Azure Notification Hubs 라이브러리를 추가합니다. 앱의 `Build.Gradle` 파일에서 종속성 섹션에 다음 줄을 추가합니다.
    ...
    목록 충돌을 방지하기 위해 **Manifest.xml**에 다음 코드를,
...
6.  [Baidu 푸시 Android SDK]를 다운로드하고 압축을 풉니다. libs 폴더에 `pushservice-x.y.z jar` 파일을 복사합니다. 그런 다음 Android 응용 프로그램의 `src/main/jniLibs`(새 폴더 만들기) 폴더에 `.so` 파일을 복사합니다.

    ![Azure Notification Hubs - Baidu SDK Libs](./media/notification-hubs-baidu-get-started/BaiduSDKLib.png)

7. libs 폴더의 pushervice-x.y.z.jar 파일을 마우스 오른쪽 단추로 클릭하고, 이 lib 프로젝트를 프로젝트로 포함하도록 라이브러리로 추가를 클릭합니다.

    ![Azure Notification Hubs - Baidu 라이브러리로 추가](./media/notification-hubs-baidu-get-started/BaiduAddAsALib.jpg)

8. Android 프로젝트의 **AndroidManifest.xml** 파일을 열고 Baidu SDK에 필요한 권한을 추가합니다. **패키지 이름으로 `YOURPACKAGENAME`을 대체합니다**.
