...
이 자습서를 시작하기 전에 응용 프로그램 이름을 예약하고 Azure 알림 허브를 만들고 나서 해당 응용 프로그램에 연결해야 합니다. [알림 허브 시작(Windows 스토어)](notification-hubs-windows-store-dotnet-get-started.md), 특히 [Windows 스토어용 앱 등록](notification-hubs-windows-store-dotnet-get-started.md#register) 및 [알림 허브 구성](notification-hubs-windows-store-dotnet-get-started.md#configure-hub) 섹션에 설명된 단계를 따르세요. 특히 알림 허브의 **구성** 탭에서 포털의 **패키지 SID** 및 **클라이언트 암호** 값을 입력했는지 확인합니다. 이 구성 절차는 [알림 허브 구성] 섹션에서 설명합니다(notification-hubs-windows-store-dotnet-get-started.md#configure-hub). 이는 중요한 단계입니다. 포털의 자격 증명이 선택한 앱 이름에 대해 지정된 자격 증명과 일치하지 않으면 푸시 알림이 실패합니다.
