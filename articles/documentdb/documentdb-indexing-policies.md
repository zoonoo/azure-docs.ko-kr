...
- DocumentDB는 어떻게 기본적으로 모든 속성의 인덱싱을 지원합니까?
- 인덱싱에서 포함하거나 제외할 속성을 어떻게 재정의합니까?
- 최종 업데이트에 대한 인덱스를 어떻게 구성합니까?
- Order By 또는 범위 쿼리를 수행하는 인덱싱을 어떻게 구성합니까?
...
예를 들어 위의 예에서 JSON 속성 {"headquarters": "Belgium"} 속성은 속성 경로 /"headquarters"/"Belgium"에 해당합니다. JSON 배열 {"exports": [{"city": “Moscow"}, {"city": Athens"}]}은 경로 /"exports"/0/"city"/"Moscow" 및 /"exports"/1/"city"/"Athens"에 해당합니다.
...
다음 샘플에서는 DocumentDB REST API를 사용하여 컬렉션을 만드는 동안 사용자 지정 인덱싱 정책을 설정하는 방법을 보여 줍니다. 이 샘플은 경로, 인덱스 유형 및 자릿수를 기준으로 표현된 인덱싱 정책을 나타냅니다.
...
>[AZURE.NOTE]인덱싱 정책에 대한 JSON 스키마는 REST API 버전 2015-06-03의 출시로 변경되어 문자열에 대한 범위 인덱스를 지원합니다. .NET SDK 1.2.0과 Java, Python, 및 Node.js SDK 1.1.0은 새 정책 스키마를 지원합니다. 이전 SDK는 REST API 버전 2015-04-08를 사용하고 인덱싱 정책의 이전 스키마를 지원합니다.
>
>컬렉션의 인덱싱 정책은 컬렉션을 만들 때 지정해야 합니다. 컬렉션을 만든 후에는 인덱싱 정책을 수정할 수 없지만 DocumentDB의 이후 릴리스에서는 이러한 수정 기능이 지원될 예정입니다.
>
>기본적으로 DocumentDB는 해시 인덱스를 사용하여 문서 내에서 모든 경로 일관되게 인덱싱합니다. 내부 타임스탬프(_ts) 경로는 범위 인덱스와 함께 저장됩니다.
...
모든 경로의 값에 대한 인덱스 정밀도는 1과 8 사이이거나 -1로 설정하여 경로가 필요한 최대 정밀도를 이용해야 합니다. 각 데이터 형식은 인덱스의 배열로 각 데이터 유형(문자열 및 숫자)에 하나씩, 각각의 정밀도를 지정하여 구성될 수 있습니다.

또한 지원되는 인덱스 유형은 해시와 범위 두 종류입니다. **해시** 인덱스 유형을 선택하면 같음 쿼리를 효율적으로 수행할 수 있습니다. 대부분의 사용 사례에서는 기본값 3바이트보다 큰 자릿수의 해시 인덱스가 불필요합니다.

**범위** 인덱스 유형을 선택하면 **Order By** 쿼리 뿐만 아니라 범위 쿼리(>, <, >=, <=, != 사용)를 수행할 수 있습니다. 또한 Order By 쿼리도 기본적으로 필요한 결과의 총 순서를 보장하기 위해 최대 인덱스 정밀도(-1)로 범위 인덱스를 작성합니다.

값의 큰 범위가 포함된 경로의 경우 6바이트와 같은 큰 자릿수를 사용하는 것이 좋습니다. 큰 자릿수 범위 인덱스가 필요한 일반적인 사용 사례는 Epoch 시간으로 저장되는 타임스탬프입니다.

해당 사용 사례에 Order BY 및/또는 효율적인 범위 쿼리가 불필요한 경우 해시 인덱스의 기본값은 저장소와 성능 간의 균형을 가장 잘 조정합니다. order by 또는 쿼리 범위를 지원하려면 사용자 지정/기본이 아닌 인덱스 정책을 지정해야 합니다.
...
<table border="0" cellspacing="0" cellpadding="0">
    <tbody>
        ...
        <tr>
            ...
            <td valign="top">
                <p>
                    ["a", "b", "c"]와 같은 스칼라의 배열에 대한 반복 및 JOIN 쿼리를 제공하는데 필요한 인덱스 경로입니다.
                </p>
                <p>
        ...
        <tr>
            ...
            <td valign="top">
                <p>
                    [{subprop: "a"}, {subprop: "b"}]와 같은 개체의 배열에 대한 반복 및 JOIN 쿼리를 제공하는데 필요한 인덱스 경로입니다.
                </p>
                <p>
...
다음 예제에서는 범위 인덱싱을 사용하고 사용자 지정 자릿수 값이 20 바이트인 특정 경로를 구성합니다.
...
DocumentDB는 쿼리가 Order By를 사용하지만 최대 전체 자릿수로 쿼리된 경로에 대한 범위 인덱스가 없는 경우 오류를 반환합니다. 범위 인덱스(모든 정밀도)가 없는 경우 >=와 같은 범위 연산자를 사용하여 쿼리에 오류가 반환되지만 인덱스에서 제공될 수 있는 다른 필터가 있는 경우 제공될 수 있습니다. REST API의 x-ms-documentdb-enable-scans 헤더를 사용하거나 .NET SDK의 EnableScanInQuery 옵션을 사용하여 범위 인덱스 없이 범위 쿼리를 수행할 수 있습니다.

마찬가지로 경로는 인덱싱에서 완전히 제외될 수 있습니다. 다음 예제는 "*" 와일드 카드를 사용하는 인덱싱에서 문서(하위 트리라고도 함)의 전체 섹션을 제외하는 방법을 보여줍니다.
...
## 인덱싱 정책 사양의 변경 내용
인덱싱 정책에 대한 스키마에서 변경 내용이 2015년 7월 7일에 REST API 버전 2015-06-03에서 도입되었습니다. SDK 버전에서 해당 클래스는 스키마와 일치하도록 새로 구현됩니다.

다음과 같은 변경 내용은 JSON 사양에서 구현됩니다.

- 정책 인덱싱이 문자열에 대한 범위 인덱스 지원
- 각 경로는 각 데이터 형식에 하나씩 여러 인덱스 정의가 존재
- 인덱싱 정확도가 숫자에 대해 1-8, 문자열에 대해 1-100 및 -1(최대 전체 자릿수) 지 원
- 경로 세그먼트는 각 경로에서 벗어나기 위해 큰 따옴표를 사용할 필요가 없습니다. 예를 들어 /"title"/ 대신 /title/?에 경로를 추가할 수 있습니다.
- "모든 경로"를 나타내는 루트 경로는 /* (in addition to /)으로 표시

.NET SDK 또는 이전의 버전 1.1.0로 작성된 사용자 지정 인덱싱 정책을 사용하여 컬렉션을 프로비전하는 코드가 있으면 SDK 버전 1.2.0로 이동하기 위해서 응용 프로그램 코드를 변경하여 이러한 변경 내용을 처리합니다. 인덱싱 정책을 구성하는 코드 또는 이전 SDK 버전을 사용하려는 계획이 없는 경우 변경해야 할 사항은 없습니다.

API 버전 2015-06-03 및 이전 버전 2015-04-08 간의 차이를 보여주는 샘플은 다음과 같습니다.
...
인덱스 정책 관리 예제 및 DocumentDB의 쿼리 언어에 대한 자세한 내용을 보려면 아래 링크를 따르십시오.
