...
-	**원자성 트랜잭션:** DocumentDB에서는 단일 저장 프로시저 또는 트리거 내에서 수행되는 데이터베이스 작업이 원자성이 되도록 보장합니다. 이렇게 하면 응용 프로그램이 관련된 작업을 단일 일괄 처리로 조합하여 모든 작업이 성공하거나 아무 작업도 성공하지 않도록 할 수 있습니다. 

-	**성능:** JSON이 Javascript 언어 유형 시스템에 기본적으로 매핑되고 DocumentDB에서 기본 저장소 단위이기도 하기 때문에 버퍼 풀에서 JSON 문서의 느슨한 구체화와 같은 다양한 최적화를 수행할 수 있으며, 필요에 따라 실행 코드에서 사용할 수 있습니다. 데이터베이스에 비즈니스 논리를 제공할 경우에는 다양한 성능 이점이 있습니다.
...
-	**캡슐화:** 저장 프로시저를 사용해서 비즈니스 논리를 한 위치에 그룹화할 수 있습니다. 여기에는 다음 두 가지 이점이 있습니다.
...
[Azure DocumentDB SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)를 탐색해서 추가 저장 프로시저, 트리거 및 UDF를 만듭니다.
