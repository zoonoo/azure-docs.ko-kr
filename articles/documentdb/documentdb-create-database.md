...
1. **DocumentDB(NoSQL)** 블레이드에서 DocumentDB NoSQL 데이터베이스를 추가할 계정을 선택합니다. 나열된 계정이 없는 경우 [DocumentDB 계정을 만들](documentdb-create-account.md)어야 합니다.
2. **DocumentDB 계정** 블레이드에서 **데이터베이스 추가**를 클릭합니다.
...
3. **데이터베이스 추가** 블레이드에서 새 데이터베이스의 ID를 입력합니다. 이름의 유효성이 검사되면 **ID** 상자에 녹색 확인 표시가 나타납니다. 그런 후 **OK**를 클릭합니다.
...
4. 이제 **DocumentDB 계정** 블레이드의 **데이터베이스** 렌즈에 새 데이터베이스가 표시됩니다.
