...
이 문서에서는 자동 인덱싱이 DocumentDB에서 작동하는 방식을 소개하며 "스키마를 알 수 없는 인덱싱와 [2015년의 VLDB 회의](http://www.vldb.org/2015/)에 발표된 "Azure DocumentDB로 스키마를 알 수 없는 인덱싱" 백서에서 발췌됩니다. 이 문서를 읽은 후 다음 질문에 답할 수 있습니다.

- DocumentDB는 JSON 문서에서 스키마를 유추하는 방법
- DocumentDB가 서로 다른 문서 간에 인덱스를 빌드하는 방법
- DocumentDB가 대규모로 자동 인덱싱을 수행하는 방법

##<a id="HowDocumentDBIndexingWorks"></a> DocumentDB 인덱싱 작동 방법

[Microsoft Azure DocumentDB](http://azure.microsoft.com/services/documentdb/)는 JSON 용으로 작성된 트루 스키마 무료 데이터베이스 용도입니다. 대규모 데이터 인덱스는 모든 스키마 또는 보조 인덱스 정의를 예상하거나 필요로 하지 않습니다. DocumentDB를 사용하는 응용 프로그램 데이터 모델을 신속하게 정의하고 반복할 수 있습니다. 컬렉션에 문서를 추가하면 DocumentDB에서 자동으로 모든 문서 속성을 인덱싱하므로 쿼리에 이러한 속성을 사용할 수 있습니다. 자동 인덱싱을 사용하면 스키마 또는 보조 인덱스에 대한 걱정 없이 완전히 임의의 스키마에 속하는 문서를 저장할 수 있습니다.
...
>[AZURE.NOTE]JSON은 자체 설명, 즉, 각 문서에는 스키마(메타데이터)와 데이터를 모두 포함하기 때문에, 예를 들어 `{"locationId", 5, "city": "Moscow"}`가 두 속성, `locationId` 및 `city`가 있고 숫자 및 문자열 속성 값을 갖습니다. DocumentDB는 문서 스키마를 유추하고 삽입 또는 교체 시 스키마나 보조 인덱스를 정의하지 않고도 인덱스할 수 있습니다.
...
- 위의 예제에서의 JSON 속성 `{"headquarters": "Belgium"}` 속성은 path/headquarters/Belgium에 해당됩니다.
- JSON 배열 `{"exports": [{"city": “Moscow"}`, `{"city": Athens"}]}`는 `/exports/[]/city/Moscow` 및 `/exports/[]/city/Athens`에 해당됩니다.
...
- 이제 인덱싱의 작동 방식을 알아보았으므로, [DocumentDB SQL로 쿼리](documentdb-sql-query.md)하는 방법에 대해 알아봅니다.
- DocumentDB 인덱스를 사용자 지정하는 방법은 [여기](documentdb-indexing-policies.md)에서 알아봅니다.
