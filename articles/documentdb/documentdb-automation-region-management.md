...
명령 프롬프트에서 `azure --version`을 입력하면 0.10.4 이상을 이미 설치했는지 여부를 확인할 수 있습니다. 이 단계에서 Microsoft Azure CLI 1.0 데이터 수집에 참여하라는 메시지가 표시될 수 있습니다. 그러면 y를 선택하여 옵트인하거나 n을 선택하여 옵트아웃할 수 있습니다.
...
여기서 설명한 대화형 로그인 방법 외에 사용 가능한 추가 Azure CLI 1.0 로그인 방법이 있습니다. 다른 방법에 대한 자세한 내용 및 여러 구독 처리에 대한 자세한 내용은 [Azure 명령줄 인터페이스(Azure CLI 1.0)에서 Azure 구독에 연결](../xplat-cli-connect.md)을 참조하세요.

### <a name="switch-to-azure-cli-10-resource-group-mode"></a>Azure CLI 1.0 리소스 그룹 모드로 전환

기본적으로 Azure CLI 1.0은 서비스 관리 모드(**asm** 모드)로 시작됩니다. 다음을 입력하여 리소스 그룹 모드로 전환합니다.
