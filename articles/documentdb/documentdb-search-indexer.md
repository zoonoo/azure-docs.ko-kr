---
title: "인덱서를 사용해서 Azure Cosmos DB를 Azure Search에 연결 | Microsoft Docs"
description: "이 문서에서는 Azure Cosmos DB를 데이터 원본으로 지정하여 Azure Search 인덱서를 사용하는 방법을 보여 줍니다."
services: cosmosdb
documentationcenter: 
author: mimig1
manager: jhubbard
...
---
# <a name="connecting-azure-cosmos-db-with-azure-search-using-indexers"></a>인덱서를 사용해서 Azure Cosmos DB를 Azure Search에 연결
Cosmos DB 데이터에 대한 뛰어난 검색 환경을 구현하려는 경우 Cosmos DB에 Azure Search 인덱서를 사용합니다. 이 문서에서는 인덱싱 인프라를 유지 관리하기 위해 어떠한 코드도 작성할 필요 없이 Azure Cosmos DB를 Azure Search와 통합하는 방법을 보여 줍니다.

이를 설정하려면 [Azure Search 계정을 설정](../search/search-create-service-portal.md)하고(표준 검색으로 업그레이드할 필요 없음), [Azure Search REST API](https://msdn.microsoft.com/library/azure/dn798935.aspx)를 호출해서 Cosmos DB **데이터 원본** 및 이 데이터 원본에 대한 **인덱서**를 만들어야 합니다.
...
* **컨테이너**:
  ...
  * **이름**: 필수입니다. 인덱싱할 DocumentDB API 컬렉션의 ID를 지정합니다.
...
데이터 변경 감지 정책의 목적은 변경된 데이터 항목을 효율적으로 식별하는 것입니다. 현재까지 지원되는 유일한 정책은 Cosmos DB에서 제공하는 마지막으로 수정된 타임스탬프 속성인 `_ts`를 사용하는 `High Water Mark` 정책입니다. 이는 다음과 같이 지정됩니다.
...
### <a id="LeveagingQueries"></a>쿼리 활용
변경 및 삭제된 문서를 캡처하는 것뿐만 아니라 DocumentDB API 쿼리를 사용하여 중첩된 속성을 평면화하고, 배열을 해제하고, json 속성을 프로젝션하고, 인덱싱할 데이터를 필터링할 수 있습니다. 인덱싱할 데이터를 조작하면 Azure Search 인덱서의 성능을 높일 수 있습니다.
...
* Cosmos DB에 대한 자세한 내용은 [Azure Cosmos DB 서비스 페이지](https://azure.microsoft.com/services/documentdb/)를 참조하세요.
