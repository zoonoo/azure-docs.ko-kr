...
![DocumentDB 계층적 리소스 모델][1] **단일 데이터베이스 계정 아래의 계층적 리소스 모델**
...
|리소스 |설명
|-----------|-----------
|데이터베이스 계정 |데이터베이스 계정은 데이터베이스 집합 및 고정된 양의 첨부 파일(미리 보기 기능)용 blob 저장소에 연결됩니다. Azure 구독을 사용하여 데이터베이스 계정을 하나 이상 만들 수 있습니다. 모든 Standard 데이터베이스 계정에는 최소 용량의 S1 컬렉션 하나가 할당됩니다. 자세한 내용은 [가격 책정 페이지](https://azure.microsoft.com/pricing/details/documentdb/)를 참조하세요.
...
|컬렉션 |컬렉션은 JSON 문서 및 관련 JavaScript 응용 프로그램 논리의 컨테이너입니다. 컬렉션은 해당 컬렉션과 연관된 성능 수준에 따라 비용이 결정되는 청구 가능 엔터티입니다. 성능 수준(S1, S2 및 S3)은 10GB의 저장소와 고정된 양의 처리량을 제공합니다. 성능 수준에 대한 자세한 내용은 [성능 페이지](documentdb-performance-levels.md)를 참조하세요.
...
|\_self 값 |설명|-------------------|-----------|/dbs |단일 데이터베이스 계정 아래의 데이터베이스 피드|/dbs/{\_rid-db} |고유한 id 속성 값이 {\_rid-db}인 데이터베이스|/dbs/{\_rid-db}/colls/ |단일 데이터베이스 아래의 컬렉션 피드|/dbs/{\_rid-db}/colls/{\_rid-coll} |고유한 id 속성 값이 {\_rid-coll}인 컬렉션|/dbs/{\_rid-db}/users/ |단일 데이터베이스 아래의 사용자 피드|/dbs/{\_rid-db}/users/{\_rid-user} |고유한 id 속성 값이 {\_rid-user}인 사용자|/dbs/{\_rid-db}/users/{\_rid-user}/permissions |단일 데이터베이스 아래의 사용 권한 피드|/dbs/{\_rid-db}/users/{\_rid-user}/permissions/{\_rid-permission} |고유한 id 속성 값이 {\_rid-permission}인 사용 권한
...
## 데이터베이스 계정
Azure 구독을 사용하여 DocumentDB 데이터베이스 계정을 하나 이상 프로비전할 수 있습니다. 각 Standard 계층 데이터베이스 계정에는 최소 용량의 S1 컬렉션 하나가 제공됩니다.
...
![데이터베이스 계정 및 컬렉션 계층적 모델][2] **데이터베이스는 사용자 및 컬렉션의 논리적 컨테이너입니다.**
...
## 컬렉션
DocumentDB 컬렉션은 JSON 문서의 컨테이너입니다. 컬렉션은 배율, 트랜잭션 및 쿼리 단위이기도 합니다. 컬렉션을 더 추가하면 DocumentDB 데이터베이스를 확장할 수 있습니다. 응용 프로그램을 확장해야 하는 경우 각각 해당 성능 계층에 따라 SSD 지원 저장소 및 고정된 처리량과 연결된 컬렉션을 추가할 수 있습니다.
 
### 탄력적 SSD 지원 문서 저장소
컬렉션은 본질적으로 탄력적입니다. 문서를 추가하거나 제거하면 자동으로 확장 및 축소됩니다. DocumentDB의 자사 사용에서는 규모가 각각 몇 기가바이트에서 테라바이트에 이르는 단일 데이터베이스 내의 수천 개 컬렉션으로 테스트했지만 DocumentDB는 현재 지정된 컬렉션의 탄력성을 10GB로 제한합니다.
...
컬렉션을 만드는 동안에만 인덱싱 정책을 구성할 수 있습니다. 컬렉션을 만든 후에는 정책을 업데이트할 수 없습니다.
