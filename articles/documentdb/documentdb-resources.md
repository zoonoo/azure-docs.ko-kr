...
DocumentDB에서 관리하는 데이터베이스 엔터티를 **리소스**라고 부릅니다. 각 리소스는 논리적 URI를 통해 고유하게 식별됩니다. 표준 HTTP 동사, request/response 헤더 및 상태 코드를 사용해서 리소스를 사용할 수 있습니다. 
...
>[AZURE.NOTE] 또한 DocumentDB는 통신 모델이 RESTful이며 [.NET 클라이언트 SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)를 통해 사용할 수 있는 효율성 높은 TCP 프로토콜을 제공합니다.

![][1]  
**단일 데이터베이스 계정 아래의 계층적 리소스 모델**   

리소스 작업을 시작하려면 Azure 구독을 사용해서 [DocumentDB 데이터베이스 계정을 만들어야](documentdb-create-account.md) 합니다. 데이터베이스는 일련의 **데이터베이스** 집합으로 구성되며, 각 데이터베이스에는 여러 개의 **컬렉션**이 포함되며, 이러한 각 컬렉션에는 또한 **저장 프로시저, 트리거, UDF, 문서** 및 관련 **첨부 파일**이 포함됩니다. 또한 데이터베이스에는 **사용자**가 연관될 수 있으며, 이러한 각 사용자는 컬렉션, 저장 프로시저, 트리거, UDF, 문서 또는 첨부 파일에 액세스할 수 있는 **권한** 집합을 갖고 있습니다. 데이터베이스, 사용자, 사용 권한 및 컬렉션은 잘 알려진 스키마가 있는 시스템 정의 리소스인 반면 문서 및 첨부 파일에는 임의의 사용자 정의 JSON 콘텐츠가 포함됩니다.  
...
|_self 값	|설명
|-------------------|-----------
|/dbs	|데이터베이스 계정의 데이터베이스 피드입니다.
|/dbs/{_rid-db}	|{_rid-db} 값을 가진 고유한 id 속성을 포함하는 데이터베이스입니다.
|/dbs/{_rid-db}/colls/	|데이터베이스의 컬렉션 피드입니다.
|/dbs/{_rid-db}/colls/{_rid-coll}	|{_rid-coll} 값을 가진 고유 id 속성을 포함하는 컬렉션입니다.
|/dbs/{_rid-db}/users/	|데이터베이스의 사용자 피드입니다. 
|/dbs/{_rid-db}/users/{_rid-user}	|{_rid-user} 값을 가진 고유 id 속성을 포함하는 사용자입니다.
|/dbs/{_rid-db}/users/{_rid-user}/permissions	|데이터베이스의 사용 권한 피드입니다. 
|/dbs/{_rid-db}/users/{_rid-user}/permissions/{_rid-permission}	|{_rid-permission} 값을 가진 고유 id 속성을 포함하는 사용 권한입니다.  
  
리소스에는 리소스의 id 속성을 통해 노출되는 고유한 사용자 정의 이름도 있습니다. id는 특정 상위 리소스 컨텍스트 내에서 고유한 최대 256자로 된 사용자 정의 문자열입니다. 예를 들어 지정된 컬렉션 내에서 모든 문서의 id 속성 값은 고유하지만 컬렉션 간에는 고유하지 않을 수 있습니다. 마찬가지로 지정된 사용자에 대한 모든 사용 권한의 id 속성 값은 고유하지만 사용자 간에는 고유하지 않을 수 있습니다. _rid 속성은 리소스의 주소 지정 가능 _self 링크를 생성하는 데 사용됩니다.   
...
##데이터베이스 계정
Azure 구독을 사용하여 DocumentDB 데이터베이스 계정을 하나 이상 프로비전할 수 있습니다. 데이터베이스 계정의 경우 SSD 지원 문서 저장소 및 다음 다이어그램에 표시된 것처럼 CU(용량 단위) 기준의 처리량의 누적형 단위를 구입할 수 있습니다.    

![][2] 

응용 프로그램 규모와 성능 요구에 따라 증분 방식으로 CU를 추가하거나 제거할 수 있습니다. 각 CU에 탄력적 컬렉션 집합, 프로비전된 SSD 지원 문서 저장소, 프로비전된 처리량이 있습니다. CU과 관련해서 프로비전된 저장소 및 처리량은 데이터베이스 계정으로 다양한 데이터베이스에 만드는 DocumentDB 컬렉션에 분산됩니다. 데이터베이스 계정 아래에 프로비전된 용량은 계정 내에 존재하거나 만들어진 모든 데이터베이스 및 컬렉션에 사용할 수 있습니다. 데이터베이스 계정의 크기에 대한 실질적인 규모 한도는 없습니다. 용량 단위를 개수에 제한 없이 점차 추가할 수 있으며 제품 제한이 적용됩니다. CU 내에서 관리되는 리소스는 분할을 통해 확장되며 고가용성을 위해 복제됩니다. 

Azure 포털([http://portal.azure.com/](http://portal.azure.com/))을 통해 [DocumentDB 데이터베이스 계정을 만들고 관리](documentdb-create-account.md)할 수 있습니다. 데이터베이스 계정을 만들고 관리하려면 관리 액세스 권한이 필요하며 Azure 구독으로만 수행할 수 있습니다. 
...
<p>이 속성은 <i>사용자 정의 리소스에만 적용됩니다</i>. 모든 시스템 정의 리소스는 강력한 일관성으로 읽기/쿼리를 지원하도록 구성됩니다.</p></td>
</tr>
...
<td valign="top" ><p>데이터베이스 계정 아래에 프로비전된 총 용량 단위 수입니다.</p></td>
</tr>

<tr>
<td valign="top" ><p>CapacityUnitsConsumed(READ)</p></td>
<td valign="top" ><p>데이터베이스 계정 아래에서 현재 사용된 총 용량 단위 수입니다.</p></td>
</tr>

<tr>
<td valign="top" ><p>ProvisionedDocumentStorageInMB(READ)</p></td>
<td valign="top" ><p>데이터베이스 계정 아래의 모든 용량 단위에 프로비전된 총 문서 저장소 크기(MB)입니다.</p></td>
</tr>

<tr>
<td valign="top" ><p>ReservedDocumentStorageInMB(READ)</p></td>
<td valign="top" ><p>데이터베이스 계정 아래의 모든 용량 단위에 예약된 총 문서 저장소 크기(MB)입니다. 컬렉션에 문서가 포함된 경우 문서 저장소가 예약되었다고 합니다. </p></td>
</tr>

<tr>
<td valign="top" ><p>ConsumedDocumentStorageInMB(READ)</p></td>
<td valign="top" ><p>데이터베이스 계정 아래의 모든 용량 단위에서 실제로 사용된 총 문서 저장소 크기(MB)입니다. 데이터베이스 계정 아래의 다양한 컬렉션에 있는 실제 문서 및 인덱스가 결합된 크기입니다.</p></td>
</tr>
...
데이터베이스 내에 컬렉션을 개수에 제한 없이 만들 수 있으며 구매한 용량 단위 크기와 제품이 적용됩니다. 용량 단위 구매를 통해 프로비전된 SSD 지원 저장소 및 처리량은 데이터베이스 계정의 데이터베이스 아래 컬렉션에 분산될 수 있습니다.   
...
##컬렉션
DocumentDB 컬렉션은 JSON 문서의 컨테이너입니다. 컬렉션은 배율, 트랜잭션 및 쿼리 단위이기도 합니다. 컬렉션을 더 추가하면 DocumentDB 데이터베이스를 확장할 수 있습니다. 응용 프로그램을 더 확장해야 하는 경우 SSD 지원 문서 저장소(및 처리량)을 더 프로비전하고 데이터베이스 계정 아래의 하나 이상 데이터베이스 컬렉션에 분산할 수 있습니다.
 
###탄력적 SSD 지원 문서 저장소
컬렉션은 본질적으로 탄력적입니다. 문서를 추가하거나 제거하면 자동으로 확장 및 축소됩니다. DocumentDB의 자사 사용에서는 규모가 각각 몇 기가바이트에서 테라바이트에 이르는 단일 데이터베이스 내의 수천 개 컬렉션으로 테스트했지만 DocumentDB의 표준 미리 보기 제품은 현재 지정된 컬렉션의 탄력성을 10GB로 제한합니다. 
...
DocumentDB 미리 보기 릴리스에서는 컬렉션을 만드는 동안에만 인덱싱 정책을 구성할 수 있습니다. 컬렉션을 만든 후에는 정책을 업데이트할 수 없습니다.

###컬렉션 쿼리
컬렉션 내의 문서는 임의 스키마를 포함할 수 있으며, 스키마 또는 보조 인덱스를 미리 제공하지 않고 컬렉션 내의 문서를 쿼리할 수 있습니다. JavaScript 기반 UDF를 통해 풍부한 계층적 관계형 연산자와 확장성을 제공하는 [DocumentDB SQL 쿼리 언어](https://msdn.microsoft.com/library/azure/dn782250.aspx)를 사용하여 컬렉션을 쿼리할 수 있습니다. JSON 문법을 통해 JSON 문서를 트리로 모델링하고 레이블을 트리 노드로 사용할 수 있습니다. 이 기능은 DocumentDB의 자동 인덱싱 기술과 DocumentDB의 SQL 쿼리 언어 둘 다에서 사용됩니다. DocumentDB 쿼리 언어는 다음 세 가지 주요 측면으로 구성됩니다.   
...
DocumentDB 쿼리 모델은 기능, 효율성 및 간결성 간의 균형을 이루려고 합니다. DocumentDB의 데이터베이스 엔진은 기본적으로 SQL 쿼리 문을 컴파일하고 실행합니다. [Azure DocumentDB REST API](https://msdn.microsoft.com/library/azure/dn781481.aspx) 또는 [클라이언트 SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)를 사용하여 컬렉션을 쿼리할 수 있습니다. .NET SDK에는 LINQ 공급자가 포함되어 있습니다. 이후 릴리스에서는 클라이언트 쪽 [DocumentDB JavaScript SDK](https://github.com/Azure/azure-documentdb-js) 및 서버 쪽 저장 프로시저와 트리거 둘 다에서 사용할 수 있는 기본 밑줄 매핑이 제공될 예정입니다.   
...
컬렉션을 만든 후 [Azure DocumentDB REST API](https://msdn.microsoft.com/library/azure/dn781481.aspx) 또는 [클라이언트 SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx)를 사용하여 저장 프로시저, 트리거 및 UDF를 컬렉션에 등록할 수 있습니다. 등록이 완료되면 참조하고 실행할 수 있습니다. JavaScript로만 작성된 다음 저장 프로시저는 두 개의 인수(책 이름 및 저자 이름)를 사용하며 새 문서를 만들고 문서를 쿼리한 다음 업데이트합니다. 모든 작업은 암시적 ACID 트랜잭션을 활용하여 수행됩니다. 실행 중 언제든지 JavaScript 예외가 발생하면 전체 트랜잭션이 중단됩니다.
...
클라이언트는 HTTP POST를 통한 트랜잭션 실행을 위해 위의 JavaScript 논리를 데이터베이스에 "전달"할 수 있습니다. HTTP 메서드 사용에 대한 자세한 내용은 [DocumentDB 리소스와 RESTful 상호 작용](documentdb-interactions-with-resources.md)을 참조하세요. 
...
진정한 개방형 데이터베이스 서비스인 DocumentDB는 직렬화된 데이터 형식(예: 날짜/시간) 또는 JSON 문서에 대한 특정 인코딩을 고안하지 않습니다. DocumentDB는 다양한 문서 간의 관계를 분류하기 위한 특별한 JSON 규칙이 필요 없습니다. DocumentDB의 SQL 쿼리 언어에서 특별한 주석 없이 문서를 쿼리 및 프로젝션하는 강력한 계층적 관계형 쿼리 연산자를 제공하거나 고유 속성을 사용하여 문서 간의 관계를 분류해야 합니다.  
 
다른 모든 리소스와 마찬가지로, REST API 또는 클라이언트 SDK를 사용하여 문서를 쉽게 만들고, 바꾸고, 삭제하고, 읽고, 열거하고, 쿼리할 수 있습니다. 문서를 삭제하면 중첩된 모든 첨부 파일에 해당하는 할당량이 즉시 해제됩니다. 문서의 읽기 일관성 수준은 데이터베이스 계정의 일관성 정책을 따릅니다. 응용 프로그램의 데이터 일관성 요구 사항에 따라 요청 단위로 이 정책을 재정의할 수 있습니다. 문서를 쿼리할 때 읽기 일관성은 컬렉션에 설정된 인덱싱 모드를 따릅니다. "일관성"의 경우 계정의 일관성 정책을 따릅니다. 
...
예제에서는 ID를 사용하여 리소스 계층 구조를 전달합니다. 고유한 리소스 ID를 사용하여 REST API를 통해 리소스에 액세스합니다. 
...
##권한
액세스 제어 관점에서 데이터베이스 계정, 데이터베이스, 사용자, 사용 권한 등의 리소스는 관리 권한이 필요하기 때문에 *administrative* 리소스로 간주됩니다. 반면, 컬렉션, 문서, 첨부 파일, 저장 프로시저, 트리거 및 UDF를 비롯한 리소스는 지정된 데이터베이스 아래로 범위가 지정되며 *application resources*로 간주됩니다. 권한 부여 모델은 두 가지 리소스 유형 및 각 유형에 액세스하는 역할(즉, 관리자 및 사용자)에 해당하는 두 가지 *access keys* 유형인 *master key* 및 *resource key*를 정의합니다. 마스터 키는 데이터베이스 계정의 일부이며 데이터베이스 계정을 프로비전하는 개발자(또는 관리자)에게 제공됩니다. 이 마스터 키는 관리 리소스 및 응용 프로그램 리소스 둘 다에 대한 액세스 권한 부여에 사용될 수 있다는 점에서 관리자 의미 체계를 갖습니다. 반면, 리소스 키는 *specific* 응용 프로그램 리소스에 대한 액세스를 허용하는 세분화된 액세스 키입니다. 따라서 데이터베이스 사용자와 특정 리소스(예: 컬렉션, 문서, 첨부 파일, 저장 프로시저, 트리거 또는 UDF)에 대한 이 사용자의 사용 권한 간 관계를 캡처합니다.   
...
##다음 단계
[DocumentDB 리소스와의 RESTful 상호 작용](documentdb-interactions-with-resources.md)에서 HTTP 명령을 사용하여 리소스를 사용하는 방법을 알아봅니다.
