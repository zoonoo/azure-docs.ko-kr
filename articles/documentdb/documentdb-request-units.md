...
---
# <a name="request-units-in-documentdb"></a>DocumentDB의 요청 단위
...
[Azure DocumentDB](https://azure.microsoft.com/services/documentdb/)는 JSON 문서에 대한 완전히 관리되고 확장 가능한 NoSQL 데이터베이스 서비스입니다. DocumentDB를 사용하면 가상 컴퓨터를 임대하거나, 소프트웨어를 배포하거나, 데이터베이스를 모니터링할 필요가 없습니다. 세계적 수준의 가용성, 성능 및 데이터 보호를 제공하기 위해 Microsoft 엔지니어가 DocumentDB를 작동하고 지속적으로 모니터링합니다. DocumentDB의 데이터는 탄력적이고 항상 사용 가능한 컨테이너인 컬렉션 내에 저장됩니다. 컬렉션에 대한 CPU, 메모리 및 IOP와 같은 하드웨어 리소스를 고찰 및 관리하는 대신 초당 요청 기준으로 처리량을 예약할 수 있습니다. DocumentDB는 프로비전된 요청 수를 제공하기 위해 프로비전, 투명한 분할 및 컬렉션의 크기 조정을 자동으로 관리합니다. 

DocumentDB는 읽기, 쓰기, 쿼리 및 저장된 프로시저 실행에 대해 다양한 API를 지원합니다. 모든 요청 값이 같지 않으므로 요청을 처리하는 데 필요한 계산의 양에 기반하여 정규화된 양의 **요청 단위**가 할당됩니다. 작업에 대한 요청 단위 수는 결정적이며 응답 헤더를 통해 DocumentDB의 모든 작업에 사용된 요청 단위 수를 추적할 수 있습니다.

DocumentDB의 각 컬렉션은 처리량으로 예약되고 요청 단위를 기준으로 표현될 수도 있습니다. 초당 수백 요청 단위부터 수백만 요청 단위까지 초당 100RU 블록으로 표현됩니다. 프로비전된 처리량은 변화하는 처리 요구에 맞고 응용 프로그램의 패턴에 액세스하는 컬렉션의 수명 동안 조정할 수 있습니다. 
...
Aravind Ramachandran이 DocumentDB의 요청 단위 및 예측 가능한 성능을 설명하는 다음 동영상을 시청하는 것으로 시작하는 것이 좋습니다.
...
> [!NOTE]
> 파티션 키는 논리적 경계이며 실제 경계가 아닙니다. 따라서 특정 파티션 키 값의 수를 제한할 필요가 없습니다. 사실 DocumentDB에 더 많은 부하 분산 옵션이 있으므로 보다 더 적게 특정 파티션 키 값을 갖는 것이 좋습니다.
...
DocumentDB는 처리량의 예약 모델에서 작동합니다. 즉, 능동적으로 *사용된* 처리량이 얼마가 됐든, 해당 컬렉션에 대해 *예약된* 처리량에 따라 요금이 청구됩니다. 응용 프로그램의 부하, 데이터 및 사용 패턴이 변하면 그에 따라 DocumentDB SDK를 통해 또는 [Azure Portal](https://portal.azure.com)을 사용하여 예약된 RU 양을 간단하게 늘리거나 줄일 수 있습니다.
...
> [!NOTE]
> 1KB 문서에 대한 한 개 요청 단위의 기초는 문서의 self 링크 또는 ID에 의한 간단한 GET에 해당합니다.
> 
> 
...
이 도구에는 제공한 샘플 문서에 기반하여 데이터 저장소 필요를 예측하는 지원이 포함됩니다.
...
1. 하나 이상의 대표 JSON 문서를 업로드합니다.
    ...
    ![요청 단위 계산기에 문서 업로드][2]
2. 데이터 저장소 요구 사항을 예측하려면 저장할 문서의 총수를 입력합니다.
3. 문서 수를 입력하여 필요한 작업을 만들고 읽고 업데이트하고 삭제합니다(초 단위 별로). 문서 업데이트 작업의 요청 단위 요금을 예측하려면 일반적인 필드 업데이트가 포함된 위의 1단계에서 샘플 문서의 복사본을 업로드합니다.  예를 들어 문서 업데이트가 일반적으로 lastLogin 및 userVisits라는 두 가지 속성을 수정하는 경우 샘플 문서를 복사하고 해당 두 가지 속성의 값을 업데이트한 다음 복사된 문서를 업로드합니다.
...
> [!NOTE]
> 인덱싱된 속성과 크기 및 수가 많이 다른 문서 유형이 있는 경우 일반 문서의 각 *유형* 을 도구에 업로드한 다음 결과를 계산합니다.
> 
> 

### <a name="use-the-documentdb-request-charge-response-header"></a>DocumentDB 요청 요금 응답 헤더를 사용합니다.
...
4. 일반적이고 공통적인 문서 쿼리의 요청 단위 요금을 기록합니다.
...
> [!NOTE]
> 문서가 DocumentDB에서 축소되기 때문에 위에서 시스템이 계산한 문서 크기가 1KB보다 약간 작습니다.
> 
> 
