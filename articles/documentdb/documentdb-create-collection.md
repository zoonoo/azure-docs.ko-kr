---
title: "Azure Cosmos DB 데이터베이스 및 컬렉션 만들기 | Microsoft Docs"
description: "클라우드 기반 문서 데이터베이스인 Azure Cosmos DB용 온라인 서비스 포털을 사용하여 데이터베이스 및 컨테이너를 만드는 방법을 알아봅니다. 지금 무료 평가판을 사용해보세요."
services: cosmosdb
author: mimig1
manager: jhubbard
editor: monicar
...
---
# <a name="how-to-create-an-azure-cosmos-db-collection-and-database-using-the-azure-portal"></a>Azure Portal을 사용하여 Azure Cosmos DB 컬렉션 및 데이터베이스를 만드는 방법
Microsoft Azure Cosmos DB를 사용하려면 [ 계정](documentdb-create-account.md), 데이터베이스, 컬렉션 및 문서가 있어야 합니다. 이 항목에서는 Azure Portal에서 Azure Cosmos DB 컬렉션을 만드는 방법을 설명합니다.

컬렉션이 무엇인지 모르시겠습니까? [Azure Cosmos DB 컬렉션이란?](#what-is-a-documentdb-collection)을 참조하세요.
...
1. [Azure Portal](https://portal.azure.com/)의 이동 표시줄에서 **Azure Cosmos DB**를 클릭한 다음 **Azure Cosmos DB** 블레이드에서 컬렉션을 추가할 계정을 선택합니다. 나열된 계정이 없는 경우 [DocumentDB 계정을 만들](documentdb-create-account.md)어야 합니다.

   ![이동 표시줄에서 Azure Cosmos DB 계정을, Azure Cosmos DB 계정 블레이드에서 계정을, Azure Cosmos DB 계정 블레이드와 데이터베이스 렌즈에서 데이터베이스를 강조 표시한 스크린샷](./media/documentdb-create-collection/docdb-database-creation-1-2.png)

   이동 표시줄에 **Azure Cosmos DB**가 표시되지 않으면 **추가 서비스**를 클릭한 다음 **Azure Cosmos DB**를 클릭합니다. 나열된 계정이 없는 경우 [DocumentDB 계정을 만들](documentdb-create-account.md)어야 합니다.
2. 선택한 계정의 **Azure Cosmos DB 계정** 블레이드에서 **컬렉션 추가**를 클릭합니다.

    ![이동 표시줄에서 Azure Cosmos DB 계정을, Azure Cosmos DB 계정 블레이드에서 계정을, Azure Cosmos DB 계정 블레이드와 데이터베이스 렌즈에서 데이터베이스를 강조 표시한 스크린샷](./media/documentdb-create-collection/docdb-database-creation-3.png)
3. **컬렉션 추가** 블레이드의 **컬렉션 ID** 상자에 새로운 컬렉션에 대한 ID를 입력합니다. 컬렉션 이름은 1~255자 사이여야 하며 `/ \ # ?` 또는 후행 공백을 포함할 수 없습니다. 이름의 유효성이 검사되면 ID 상자에 녹색 확인 표시가 나타납니다.
    ...
    ![데이터베이스 블레이드의 컬렉션 추가 단추, 컬렉션 추가 블레이드의 설정 및 확인 단추를 강조 표시하는 스크린샷 - Azure Cosmos DB용 Azure Portal - NoSQL JSON 데이터베이스용 클라우드 기반 데이터베이스 작성자](./media/documentdb-create-collection/docdb-collection-creation-5-8.png)
4. 기본적으로 **스토리지 용량**은 파티션된 컬렉션을 처리하도록 **무제한**으로 설정됩니다.
    ...
    여러 파티션에서 무제한 저장소를 처리하도록 확장하고 최소 처리량 수준이 2,500RU/s인 [분할된 컬렉션](documentdb-partition-data.md#single-partition-and-partitioned-collections)을 원하는 경우 저장소 용량을 **무제한**으로 설정합니다.

    10GB 또는 250GB 이외의 양을 프로비저닝하려는 경우 저장소 용량을 **사용자 지정**으로 설정합니다. Azure Cosmos DB의 규모는 거의 무제한이므로, 지원 요청에 요청하는 저장소 크기 및 처리량 값을 포함해 주세요.

6. **파티션 키** 상자에 컬렉션에 대한 파티션 키를 입력합니다. 분할 컬렉션에 필요하며 단일 파티션 컬렉션에는 선택 사항입니다. 올바른 파티션 키를 선택하는 것은 성능이 뛰어난 컬렉션을 만드는 데 중요합니다. 파티션 키의 선택에 대한 자세한 내용은 [분할 설계](../cosmos-db/partition-data.md#designing-for-partitioning)를 참조하세요.
...
9. 이제 **개요** 블레이드의 **컬렉션** 렌즈에 새 컬렉션이 표시됩니다.
    ...
    ![데이터베이스 블레이드의 새 컬렉션 스크린샷 - Azure Cosmos DB용 Azure Portal - NoSQL JSON 데이터베이스용 클라우드 기반 데이터베이스 작성자](./media/documentdb-create-collection/docdb-collection-creation-9.png)
...
## <a name="what-is-an-azure-cosmos-db-collection"></a>Azure Cosmos DB 컬렉션이란?
...
컬렉션은 관계형 데이터베이스의 테이블과 동일하지 않습니다. 컬렉션은 스키마를 적용하지 않으며 사실상 Azure Cosmos DB는 스키마를 적용하지 않는 스키마 없는 데이터베이스입니다. 따라서 동일한 컬렉션에서도 다양한 스키마를 사용해서 여러 유형의 문서를 저장할 수 있습니다. 테이블에서와 같은 방식으로 컬렉션을 사용해서 단일 유형의 개체를 저장할 수도 있습니다. 모델 효율성은 쿼리 및 트랜잭션에 데이터가 표시되는 방법에 따라서만 결정됩니다.

## <a name="other-ways-to-create-an-azure-cosmos-db-collection"></a>Azure Cosmos DB 컬렉션을 만드는 다른 방법
