---
title: Azure 빠른 시작 - Azure Portal을 사용하여 고가용성 응용 프로그램을 위한 Front Door 프로필 만들기
description: 이 빠른 시작 문서에는 고가용성, 고성능 글로벌 웹 응용 프로그램을 위한 Front Door를 만드는 방법을 설명합니다.
services: front-door
documentationcenter: ''
author: sharad4u
editor: ''
...
---
# <a name="quickstart-create-a-front-door-for-a-highly-available-global-web-application"></a>빠른 시작: 고가용성 글로벌 웹 응용 프로그램을 위한 Front Door 만들기

이 빠른 시작에서는 글로벌 웹 응용 프로그램에 고가용성 및 고성능을 제공하는 Front Door 프로필을 만드는 방법을 설명합니다. 

이 빠른 시작에 설명된 시나리오에는 서로 다른 Azure 지역에서 실행되는 두 개의 웹 응용 프로그램 인스턴스가 포함되어 있습니다. 동등한 [가중치 및 동일한 우선 순위 백 엔드](front-door-routing-methods.md)를 기반으로 응용 프로그램을 실행하는 가장 가까운 사이트 백 엔드에 사용자 트래픽을 보내는 Front Door 구성이 만들어집니다. Front Door는 지속적으로 웹 응용 프로그램을 모니터링하다가 가장 가까운 사이트를 사용할 수 없게 되면 그 다음 가용 백업 사이트로 자동 장애 조치(failover)합니다.
...
다음으로, 응용 프로그램이 어디에 상주하는지 Front Door가 알 수 있도록 백 엔드 풀에 응용 프로그램 백 엔드를 구성해야 합니다. 
...
### <a name="view-front-door-handle-application-failover"></a>Front Door 핸들 응용 프로그램 장애 조치(failover) 보기
...
더 이상 필요하지 않은 리소스 그룹, 웹 응용 프로그램 및 모든 관련 리소스를 삭제합니다.
