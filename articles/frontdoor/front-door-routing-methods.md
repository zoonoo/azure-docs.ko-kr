...
* **세션 선호도:** 사용자 세션이 아직 활성 상태이 고 백 엔드 인스턴스가 상태 프로브에 따라 계속 정상으로 보고 되는 경우 사용자의 후속 요청이 동일한 백 엔드로 전송 되도록 하려면 프런트 엔드 호스트 또는 도메인에 대 한 세션 선호도를 구성할 수 있습니다. 
...
| 사용할 수 있는 백 엔드 | Priority | 대기 시간 신호(상태 프로브 기반) | 가중치 |
|-------------| ----------- | ----------- | ----------- |
