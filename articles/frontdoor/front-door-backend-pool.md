...
ms.locfileid: "58123694"
---
# <a name="backends-and-backend-pools-in-azure-front-door-service"></a>Azure Front Door Service의 백 엔드 및 백 엔드 풀
이 문서에서는 애플리케이션 배포를 Front Door와 매핑하는 방법에 대한 다양한 개념을 설명합니다. 설명할는 다양 한 용어 앞에 응용 프로그램 백 엔드 주위 도어 구성을 의미 합니다.
...
1. **경로**: 백 엔드 풀의 모든 백 엔드에 대 한 프로브 요청 하는 URL 경로에 보내집니다. 예를 들어, 백 엔드 중 하나가 `contoso-westus.azurewebsites.net`이고 경로가 `/probe/test.aspx`로 설정되는 경우 프로토콜이 HTTP로 설정되었다는 가정하에, Front Door 환경은 http://contoso-westus.azurewebsites.net/probe/test.aspx로 상태 프로브 요청을 보냅니다. 
2. **프로토콜**: HTTP 또는 HTTPS 프로토콜을 통해 백 첫 번째 관문에서 상태 프로브 요청을 보낼지 여부를 정의 합니다.
3. **간격 (초)**: 이 필드는 프로브를 전송할 각 프런트 도어 환경의 간격, 즉 백을 상태 프로브 빈도 정의 합니다. 참고 - 더 빠른 장애 조치(Failover)를 원할 이 필드를 더 낮은 값으로 설정합니다. 그러나 이 값이 낮을수록 백 엔드가 수신하는 상태 프로브 용량이 더 많아집니다. Front Door가 백 엔드에서 생성하는 프로브 용량 크기를 이해하려면 예제를 사용하세요. 간격이 30초로 설정되어 있고, 전역적으로 약 90개의 Front Door 환경 또는 POP가 있다고 가정합니다. 이 경우 각 백 엔드는 초당 약 3~5개의 프로브 요청을 받습니다.
...
1. **샘플 크기**: 이 속성을 백 엔드 상태 평가 대 한 고려해 보아야 상태 프로브의 얼마나 많은 샘플을 식별 합니다.
2. **성공적으로 샘플 크기**: 이 속성을 크기인' 샘플' 위에서 설명 했 듯이 얼마나 많은 샘플에서는 필요가 정상으로 백 엔드를 호출 하는 성공에 대 한 확인을 정의 합니다. 
...
3. **대기 시간 민감도 (추가 대기 시간)**: 대기 시간 민감도 필드 프런트 도어 대기 시간 측정 또는 가장 가까운 백 엔드에 요청 전달 측면에서 민감도 범위 내에 있는 백 엔드에 요청을 보낼 것인지 여부를 정의 합니다. 자세한 내용을 보려면 Front Door에 대한 [최소 대기 시간 기반 라우팅 방법](front-door-routing-methods.md#latency)을 읽어보세요.
...
1. **백 엔드 호스트 유형**: 추가 하려는 리소스의 형식입니다. Front Door는 앱 서비스, 클라우드 서비스 또는 저장소에서 애플리케이션 백 엔드를 자동으로 검색하도록 지원합니다. Azure 또는 비 Azure 백 엔드의 다른 리소스를 원할 경우 ‘사용자 지정 호스트’를 선택합니다. 참고 - 구성 중에 API는 백 엔드를 Front Door 환경에서 액세스할 수 있는지 여부를 검증하지 않으며, 대신 사용자가 Front Door 환경에서 백 엔드에 연결할 수 있는지를 확인해야 합니다. 
2. **구독 및 백 엔드 호스트 이름**: 선택 하지 않은 경우 백 엔드 호스트에 대 한 ' 사용자 지정 호스트' 형식을 선택한 다음 아래로 범위 및 사용자 인터페이스에서 적절 한 구독 및 해당 백 엔드 호스트 이름을 선택 하 여 백 엔드를 선택 해야 합니다.
3. **백 엔드 호스트 헤더**: 각 요청에 대 한 백 엔드에 전송 하는 호스트 헤더 값입니다. 자세한 내용은 [백 엔드 호스트 헤더](#hostheader)를 읽어보세요.
...
대부분의 응용 프로그램 백 엔드(예: Web Apps, Blob Storage 및 Cloud Services)에서는 호스트 헤더가 백 엔드의 도메인과 일치해야 합니다. 그러나 백 엔드에 라우팅하는 프런트 엔드 호스트 www와 같은 다른 호스트 이름을 갖습니다\.contoso.azurefd.net 합니다. 설정하는 백 엔드에서 호스트 헤더가 백 엔드의 호스트 이름과 일치해야 할 경우 ‘백 엔드 호스트 헤더’도 백 엔드의 호스트 이름을 갖는지 확인해야 합니다.
