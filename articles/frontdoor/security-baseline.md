...
**지침** : 해당 사항 없음 Azure Front 도어가 개인 네트워크에 배포 하거나 보안을 유지 하도록 설계 되지 않았습니다 .이 제어는 네트워크 연결을 설명 하기 위한 것 이며 적용 되지 않습니다.

**Azure Security Center 모니터링** : 해당 없음

**책임** : 설정 되지 않았습니다. 작업 항목에 값을 제공 하십시오.

### <a name="ns-3-establish-private-network-access-to-azure-services"></a>NS-3: Azure 서비스에 대 한 개인 네트워크 액세스 설정

**지침** : 해당 없음, Azure Front 도어가 개인 네트워크 액세스를 위해 가상 네트워크에 배포 하거나 보호 하도록 설계 되지 않았습니다.
...
**지침** : 암호화를 사용 하 여 외부 및 공용 네트워크에서 트래픽을 보호 하 고 데이터 보호에 매우 중요 합니다. 또한 다음 작업도 수행해야 합니다.

- 액세스 제어 사용

- 공격자가 데이터를 쉽게 읽거나 수정할 수 없도록 암호화를 사용 하 여 ' 대역 외 ' 공격 (예: 트래픽 캡처)에 대해 전송 중인 데이터를 보호 합니다.
- HTTP 트래픽에 대 한 Azure 리소스에 연결 하는 클라이언트에서 TLS v 1.2 이상을 협상할 수 있는지 확인 합니다.
- 암호화 되지 않은 프로토콜 대신 SSH (Linux) 또는 RDP/TLS (Windows의 경우)를 사용 하 여 원격 관리를 사용 합니다.

- 사용 되지 않는 SSL/TLS/SSH 버전, 프로토콜 및 약한 암호화 사용 안 함

기본적으로 Azure는 Azure 데이터 센터 간의 데이터 트래픽에 대 한 전송 암호화에 데이터를 제공 합니다. 
...
**지침** : 해당 사항 없음 Azure Front 도어는 수명 주기 관리 프로세스에서 자산의 보안을 보장 하는 데 사용할 수 없습니다. 높은 영향을 미치는 자산의 네트워크 구성과 특성을 유지 관리 하는 것은 고객의 책임입니다. 
