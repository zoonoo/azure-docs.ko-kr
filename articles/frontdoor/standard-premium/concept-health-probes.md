---
title: Azure 전면 도어 표준/프리미엄 (미리 보기) 상태 프로브 모니터링
description: 이 문서는 Azure Front 문이 백 엔드의 상태를 모니터링 하는 방법을 이해 하는 데 도움이 됩니다.
services: frontdoor
author: duongau
...
* **가져오기:** GET 메서드는 요청 URI로 식별 되는 모든 정보 (엔터티 형식)를 검색 함을 의미 합니다.
* **헤드:** 서버는 응답에서 메시지 본문을 반환 해서는 안 된다는 점을 제외 하 고 HEAD 메서드는 GET과 동일 합니다. 새 Front 도어 프로필의 경우 기본적으로 프로브 방법이 HEAD로 설정 됩니다.
...
1. 백 엔드 풀의 정상 백 엔드 집합의 경우, 프런트 도어는 각 백 엔드에 대 한 대기 시간 (왕복 시간)을 추가로 측정 하 고 유지 관리 합니다.
...
백 엔드 풀에 하나의 백 엔드가 있거나 백 엔드 풀에서 하나의 백 엔드가 활성 상태인 경우 상태 프로브를 사용 하지 않도록 설정할 수 있습니다. 이렇게 하면 응용 프로그램 백 엔드에 대 한 부하를 줄일 수 있습니다.
