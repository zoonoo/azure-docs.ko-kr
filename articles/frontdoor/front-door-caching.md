...
---
# <a name="caching-with-azure-front-door"></a>Azure 정문으로 캐싱
다음 문서는 캐싱을 허용하고 회람 규칙을 사용하는 Front Door의 동작을 설명합니다. Front Door는 최신 CDN(콘텐츠 전송 네트워크)이므로 동적 사이트 가속 및 부하 분산과 함께 다른 CDN과 마찬가지로 캐싱 동작도 지원합니다.

## <a name="delivery-of-large-files"></a>대용량 파일 전달
Azure 정문 문은 파일 크기에 대한 한도 없이 대용량 파일을 제공합니다. Front Door는 개체 청크라는 기술을 사용합니다. 대용량 파일이 요청되면 Front Door는 백엔드에서 더 작은 파일 조각을 검색합니다. 전체 또는 바이트 범위의 파일을 요청 받은 후 Front Door 환경은 백엔드에서 8MB 청크의 파일을 요청합니다.
...
- [Gzip (GNU 지퍼)](https://en.wikipedia.org/wiki/Gzip)
- [브로틀리 (것)로](https://en.wikipedia.org/wiki/Brotli)
...
Front Door를 사용하면 쿼리 문자열이 포함된 웹 요청에 대해 파일이 캐시되는 방식을 제어할 수 있습니다. 쿼리 문자열이 있는 웹 요청에서 쿼리 문자열은 물음표(?) 다음에 나오는 요청 부분입니다. 쿼리 문자열은 필드 이름 및 해당 값이 등호(=)로 구분된 하나 이상의 키-값 쌍을 포함할 수 있습니다. 각 키-값 쌍은 앰퍼샌드(&)로 구분됩니다. `http://www.contoso.com/content.mov?field1=value1&field2=value2`)을 입력합니다. 요청의 쿼리 문자열에 둘 이상의 키-값 쌍이 있는 경우 해당 순서는 중요하지 않습니다.
...
1. **단일 경로 제거**: 파일 확장자(예: /pictures/strasbourg.png)와 함께 자산의 전체 경로(프로토콜 및 도메인 제외)를 지정하여 개별 자산을 제거합니다.
2. **와일드 카드 제거**: 별표(\*)를 와일드 카드로 사용할 수 있습니다. 경로에 /가\* 있는 끝점 아래의 모든 폴더, 하위 폴더 및 파일을 제거하거나 폴더 다음에\*/,/pictures/\*.
...
캐시 제어: 개인, 캐시 제어: 캐시 없음 및 캐시 제어: 저장소 없음과 같이 응답이 캐시되지 않는다는 것을 나타내는 캐시 제어 응답 헤더는 적용되지 않습니다. 그러나 여러 요청이 동일한 URL에 대해 POP에서 이동 중인 경우 해당 응답을 공유할 수도 있습니다. 캐시 컨트롤이 없는 경우 기본 동작은 AFD가 X가 1~3일 사이에 임의로 선택되는 X 시간 동안 리소스를 캐시한다는 것입니다.
