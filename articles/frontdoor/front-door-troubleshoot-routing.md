...
이 증상은 프런트 도어에서 요청을 수신 하는 백 엔드가 시간 제한 구성 (기본값은 30 초)을 초과 하거나이 제한 시간 값을 초과 하 여 Front 문의 요청에 응답을 보내는 경우에 발생 합니다. 
...
- 앞 도어를 만들었지만 도메인 또는 프런트 엔드 호스트에 대 한 요청이 HTTP 400 상태 코드를 반환 합니다.
...
## <a name="front-door-is-not-redirecting-http-to-https"></a>프런트 도어는 HTTP를 HTTPS로 리디렉션하는 것이 아닙니다.
...
이 동작은 Front 문에 대 한 라우팅 규칙을 올바르게 구성 하지 않은 경우에 발생할 수 있습니다. 기본적으로 현재 구성은 구체적이 아니며 충돌 하는 규칙이 있을 수 있습니다.
...
- 백 엔드는 공용 백 엔드가 아니므로 전방 도어에 표시 되지 않습니다.
- 백 엔드가 잘못 구성 되어 프런트 도어가 잘못 된 요청을 전송 합니다. 즉, 백 엔드에서 HTTP만 허용 하지만, 프런트 도어가 HTTPS 요청을 전달 하는 데 HTTPS를 허용 하도록 선택 하지 않았습니다.
...
3. 회람 규칙 설정 확인
    - 해당 프런트 엔드 호스트 이름에서 백 엔드 풀로 라우팅되어야 하는 회람 규칙으로 이동합니다. 허용되는 프로토콜이 올바르게 구성되었는지, 그렇지 않다면 요청 전달 시 Front Door에서 사용할 프로토콜이 올바르게 구성되었는지 확인하세요. 허용 되는 _프로토콜_ 필드에 따라 앞 도어를 수락 해야 하는 요청과 _전달 프로토콜_ 에 따라 프런트 도어에서 백 엔드에 요청을 전달 하는 데 사용할 프로토콜을 결정 합니다.
