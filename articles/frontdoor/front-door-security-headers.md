...
다음 예제에서는 규칙 엔진 구성이 연결된 경로에 정의된 경로와 일치하는 모든 들어오는 요청에 콘텐츠 보안 정책 헤더를 추가하는 방법을 보여 줍니다. 여기서는 신뢰할 수 있는 사이트, **https://apis.contoso.com** 의 스크립트만 허용하여 애플리케이션에서 실행합니다.
...
4. 헤더 이름: **콘텐츠 보안 정책**을 추가하고 이 헤더에 허용되는 값을 정의합니다. 이 시나리오에서는 *"script-src 'self' https://apis.contoso.com."* 을 선택합니다.
