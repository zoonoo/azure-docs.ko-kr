...
4. 기존 스크립트를 다음 함수로 바꾼 후 **저장**을 클릭합니다.
    ...
    > [AZURE.NOTE] **지우기**를 클릭한 다음 **저장**을 클릭하여 **스크립트** 탭에서 등록된 스크립트를 제거할 수 있습니다.
...
4. **TodoListViewController.cs**에서 **OnAdd** 메서드를 찾습니다. 반환된 <code>index</code> 가 <code>-1</code>이 아니도록 메서드를 업데이트합니다(-1일 경우 **InsertTodoItemAsync**의 예외 처리에서 반환됨). 이 경우 <code>TableView</code>.에 새 행을 추가하지 않으려고 합니다.
...
이 자습서를 완료했으므로 이제 데이터 시리즈의 마지막 자습서인 [페이징을 사용하여 쿼리 구체화]를 계속 진행합니다.
