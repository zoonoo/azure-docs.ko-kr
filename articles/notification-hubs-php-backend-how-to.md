...
-   선택한 모바일 플랫폼에 대한 [시작 자습서][시작 자습서]에 따라 PHP에서 백 엔드 부분을 구현합니다.

## 클라이언트 인터페이스

기본 클라이언트 인터페이스에서는 [.NET 알림 허브 SDK][.NET 알림 허브 SDK]에서 제공되는 것과 같은 메서드를 제공할 수 있습니다. 이 인터페이스를 사용하여 현재 이 사이트에서 사용할 수 있고 인터넷의 커뮤니티에서 제공되는 모든 자습서와 샘플을 직접 변환할 수 있습니다.

[PHP REST 래퍼 샘플][PHP REST 래퍼 샘플]에서 사용할 수 있는 모든 코드를 찾을 수 있습니다.
...
아직 하지 않았으면 백 엔드를 구현해야 하는 [시작 자습서][시작 자습서]의 마지막 섹션까지 수행하세요.
또한 [PHP REST 래퍼 샘플][PHP REST 래퍼 샘플]의 코드를 사용하려면 [자습서 완료][자습서 완료] 섹션으로 바로 이동하세요.

전체 REST 래퍼를 구현하는 방법에 대한 자세한 내용은 [MSDN][MSDN]을 참조하세요. 이 섹션에서는 알림 허브 REST 끝점에 액세스하는 데 필요한 기본 단계의 PHP 구현에 대해 설명합니다.
...
보안 토큰 만들기에 대한 자세한 내용은 [여기][여기]를 참조하세요.
...
모든 사용할 수 있는 옵션은 [알림 허브 REST API 설명서][알림 허브 REST API 설명서] 및 특정 알림 플랫폼의 형식을 참조하세요.
...
알림 허브를 초기화합니다([시작 자습서][시작 자습서]에 설명된 대로 연결 문자열 및 허브 이름 대체).
 $hub = new NotificationHub("connection string", "hubname");
...
-   위의 모든 코드가 포함된 전체 [PHP REST 래퍼 샘플][PHP REST 래퍼 샘플]을 다운로드합니다.
