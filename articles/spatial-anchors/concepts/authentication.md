---
title: Azure 공간 앵커에 대 한 인증 및 권한 부여 | Microsoft Docs
...
Azure Active Directory 사용자를 대상으로 하는 응용 프로그램의 경우 다음 설명서에 설명 된 대로 ADAL 라이브러리를 사용 하 여 얻을 수 있는 사용자에 대해 Azure AD 토큰을 사용 하 [https://docs.microsoft.com/azure/active-directory/develop/v1-overview](../../active-directory/develop/v1-overview.md)는 것이 좋습니다 .이에 대해서는 다음 문서에 설명 된 단계를 수행 해야 합니다. 다음을 포함 하는 "빠른 시작":
...
여기서는 앱이 자체 메커니즘을 사용 하는 것으로 가정 합니다 (예: Microsoft 계정, PlayFab, Facebook, Google ID, 사용자 지정 사용자 이름/암호 등) 백 엔드 서비스에 인증 합니다. 사용자가 백 엔드 서비스에 인증 되 면 해당 서비스는 Azure AD 토큰을 검색 하 고, Azure 공간 앵커에 대 한 액세스 토큰에 대해 교환 하 고, 클라이언트 응용 프로그램에 다시 반환할 수 있습니다.

Azure AD 액세스 토큰은 다음 설명서 [https://docs.microsoft.com/azure/active-directory/develop/v1-overview](../../active-directory/develop/v1-overview.md)에 설명 된 대로 ADAL 라이브러리를 사용 하 여 검색 됩니다. "빠른 시작"에 나열 된 단계를 따라야 합니다. 여기에는 다음이 포함 됩니다.
...
1.  Azure Portal 구성:
    1.  Azure AD에서 응용 프로그램을 등록 합니다.
        ...
        4.  해당 응용 프로그램에서 **설정**을 클릭 한 다음 **키** 탭을 선택 합니다. 키 이름을 입력 하 고, 기간을 선택 하 고, **저장**을 누릅니다. 웹 서비스의 코드에 포함 해야 하므로 해당 시간에 표시 되는 키 값을 저장 해야 합니다.
