...
## <a name="register"></a><span class="short-header">앱 등록</span>푸시 알림을 위한 앱 등록
...
2.  **Description**에 앱 이름을 입력하고 **Bundle Identifier** 값을 입력한 후 "App Services" 섹션에서 "Push Notifications" 옵션을 선택하고 **Continue**를 클릭합니다.
    ...
    **Submit**를 클릭하면 아래와 같은 **Registration complete** 화면이 표시됩니다. **Done**을 클릭합니다.
    ...
    <div class="dev-callout"><b>참고</b>
<p><b>MobileServices.Quickstart</b> 외의 <strong>Bundle Identifier</strong> 값을 제공하려는 경우 Xcode 프로젝트에서도 번들 식별자 값을 업데이트해야 합니다.</p>
 </div>
...
4.  **Edit**를 클릭하고 화면 맨 아래로 스크롤한 후 **Development Push SSL Certificate** 섹션 아래의 **Create Certificate...**를 클릭합니다.
    ...
    <div class="dev-callout"><b>참고</b>
<p>이 자습서에서는 개발 인증서를 사용합니다. 프로덕션 인증서를 등록할 때에도 동일한 프로세스가 사용됩니다. 모바일 서비스로 인증서를 업로드할 때 동일한 인증서 유형을 설정해야 합니다.</p>
</div>
...
6.  포털에서 인증서가 생성되면 **Download**를 클릭한 후 **Done**을 클릭합니다.
    ...
    <div class="dev-callout"><b>참고</b>
<p>기본적으로 다운로드된 개발 증명서 파일은 이름이 <strong>aps_development.cer</strong>로 지정됩니다.</p>
</div>
...
7.  다운로드한 푸시 인증서 **aps\_development.cer**을 두 번 클릭합니다.
    ...
    > [WACOM.NOTE]
    > 인증서의 이름은 다를 수 있지만 **Apple Development iOS Push Notification Services:**가 앞에 옵니다.
