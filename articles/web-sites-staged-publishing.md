...
-   [웹 사이트에 배포 슬롯 추가][웹 사이트에 배포 슬롯 추가]
-   [배포 슬롯의 구성 정보][배포 슬롯의 구성 정보]
-   [배포 슬롯 교환][배포 슬롯 교환]
-   [프로덕션 사이트를 준비 사이트로 롤백][프로덕션 사이트를 준비 사이트로 롤백]
-   [사이트 슬롯 삭제][사이트 슬롯 삭제]
-   [사이트 슬롯용 Azure PowerShell cmdlet][사이트 슬롯용 Azure PowerShell cmdlet]
-   [사이트 슬롯용 Azure 플랫폼 간 명령줄 인터페이스(xplat-cli) 명령][사이트 슬롯용 Azure 플랫폼 간 명령줄 인터페이스(xplat-cli) 명령]
...
3.  웹 사이트 목록에서 배포 슬롯에 표시할 웹 사이트 이름의 왼쪽에 있는 표시를 확장합니다. 웹 사이트 이름 뒤에 배포 슬롯 이름이 표시됩니다.

    ![배포 슬롯이 있는 사이트 목록][배포 슬롯이 있는 사이트 목록]

4.  배포 사이트 슬롯의 이름을 클릭하면 다른 웹 사이트와 마찬가지로 일련의 탭이 있는 페이지가 열립니다. ***웹-사이트-이름*(*배포-슬롯-이름*)**이 포털 페이지의 맨 위에 나타나서 현재 배포 사이트 슬롯을 보는 중임을 알립니다.

    ![배포 슬롯 제목][배포 슬롯 제목]

5.  대시보드 뷰에서 사이트 URL을 클릭합니다. 배포 슬롯은 고유의 호스트 이름을 가지고 있고 라이브 사이트이기도 합니다. 배포 슬롯에 대한 공용 액세스를 제한하려면 [Azure 웹 사이트 – 비 프로덕션 배포 슬롯에 대한 웹 액세스 차단][Azure 웹 사이트 – 비 프로덕션 배포 슬롯에 대한 웹 액세스 차단](영문)을 참조하십시오.

    -   

콘텐츠가 없습니다. 다른 리포지토리 분기 또는 아예 다른 리포지토리로부터 슬롯에 배포할 수 있습니다. 슬롯의 구성을 변경할 수도 있습니다. 게시 프로필을 사용하거나 콘텐츠 업데이트를 위해 배포 슬롯에 연결된 배포 자격 증명을 사용합니다. 예를 들어 [git를 사용하여 이 슬롯에 게시][git를 사용하여 이 슬롯에 게시]할 수 있습니다.
...
2.  Swap Deployments 대화 상자가 나타납니다. 대화 상자에서 소스가 되어야 할 사이트 슬롯 및 대상이 되어야 할 사이트를 선택할 수 있습니다.
    ...
    ![Swap Deployments 대화 상자][Swap Deployments 대화 상자]
...
-   기본적으로 배포 슬롯(사이트)은 프로덕션 슬롯(사이트)과 동일한 리소스를 공유하고 동일한 VM에서 실행됩니다. 준비 슬롯에 스트레스 테스트를 실행하는 경우 프로덕션 환경에 비교 가능한 스트레스 로드가 발생합니다.
    ...
    > [WACOM.NOTE] 특별히 [Azure Preview 포털][Azure Preview 포털]에서는 프로덕션이 아닌 슬롯을 다른 웹 호스팅 계획으로 일시적으로 전환함으로써 프로덕션 슬롯에 미칠 수 있는 영향을 방지할 수 있습니다. 테스트 슬롯을 프로덕션으로 교환하기 전에 테스트 및 프로덕션 슬롯이 다시 한 번 같은 웹 호스팅 계획을 공유해야 합니다.
...
-   Azure PowerShell을 설치 및 구성하는 방법과 Azure 구독에 Azure PowerShell을 인증하는 방법에 대한 자세한 내용은 [Azure PowerShell 설치 및 구성 방법][Azure PowerShell 설치 및 구성 방법](영문)을 참조하십시오.
...
-   xplat-cli 설치 및 구성 지침과 xplat-cli를 Azure 구독에 연결하는 방법에 대한 자세한 내용은 [Azure 플랫폼 간 명령줄 인터페이스 설치 및 구성][Azure 플랫폼 간 명령줄 인터페이스 설치 및 구성](영문)을 참조하십시오.
...
[Azure 웹 사이트 – 비 프로덕션 배포 슬롯에 대한 웹 액세스 차단][Azure 웹 사이트 – 비 프로덕션 배포 슬롯에 대한 웹 액세스 차단]

[Microsoft Azure 무료 평가판][Microsoft Azure 무료 평가판]
...
<!-- IMAGES -->
  ...
  [git를 사용하여 이 슬롯에 게시]: http://azure.microsoft.com/ko-kr/documentation/articles/web-sites-publish-source-control/
  ...
  [2]: ./media/web-sites-staged-publishing/DeleteStagingSiteButton.png
  ...
  [Azure 플랫폼 간 명령줄 인터페이스 설치 및 구성]: http://www.windowsazure.com/ko-kr/documentation/articles/xplat-cli
