---
title: Azure DDoS Protection 기본 모범 사례
description: DDoS 보호를 사용하는 보안 모범 사례를 알아봅니다.
...
예를 들어 공격 위험이 증가하면 애플리케이션의 크기(*노출 영역*)도 커집니다. 부하 분산 장치([Azure Load Balancer](../load-balancer/quickstart-load-balancer-standard-public-portal.md) 및 [Azure Application Gateway](../application-gateway/application-gateway-create-probe-portal.md))에 필요하지 않은 공개된 IP 주소 공간 및 수신 대기 포트를 종료하기 위해 승인 목록을 사용하여 노출 영역을 줄일 수 있습니다. [NSG(네트워크 보안 그룹)](../virtual-network/network-security-groups-overview.md)는 공격 노출을 줄이기 위한 또 다른 방법입니다.
