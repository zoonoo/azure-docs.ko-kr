...
기존에는 Azure Relay 애플리케이션에서 두 가지 권한 부여 모델 중 하나를 선택하여 사용할 수 있었습니다. 그중 하나는 Relay 서비스에서 직접 제공되는 [SAS(공유 액세스 서명)](../service-bus-messaging/service-bus-sas.md) 토큰 모델이고, 다른 하나는 페더레이션 모델입니다. 페더레이션 모델에서는 권한 부여 규칙 관리 작업이 [Azure Active Directory](/azure/active-directory/) ACS(Access Control Service)를 통해 내부에서 관리되며, 원하는 기능에 대한 액세스 권한을 부여하기 위해 ACS에서 가져온 토큰이 Relay로 전달됩니다.

ACS 권한 부여 모델은 [SAS 권한 부여](../service-bus-messaging/service-bus-authentication-and-authorization.md)가 기본 모델로 사용되면서 이미 오래전에 교체되었으며 현재는 모든 설명서, 지침 및 샘플에서 SAS만 사용되고 있습니다. 또한 ACS와 쌍으로 지정된 새 Relay 네임스페이스도 더 이상 만들 수 없습니다.
...
1.  **기본값 변경 안 됨**. [SharedSecretTokenProvider](/dotnet/api/microsoft.servicebus.sharedsecrettokenprovider) 개체를 사용하여 자동 생성된 **소유자** 서비스 ID와 해당 비밀 키를 Relay 네임스페이스와 쌍으로 설정된 ACS 네임스페이스용으로 전달하고 새 규칙은 추가하지 않는 일부 고객이 사용하는 시나리오입니다.
...
애플리케이션이 단순 규칙을 포함하는 사용자 지정 서비스 ID를 사용하는 경우, 특정 Relay에 대한 액세스 제어 기능을 제공하기 위해 ACS 서비스 ID를 만들었다면 마이그레이션을 손쉽게 수행할 수 있습니다. 이 시나리오는 대개 각 Relay를 테넌트 사이트나 지점에 대한 브리지로 사용하며 해당 특정 사이트용으로 서비스 ID를 만드는 SaaS 스타일 솔루션에서 사용되는 경우가 많습니다. 이 경우에는 개별 서비스 ID를 Relay에서 직접 공유 액세스 서명 규칙으로 마이그레이션할 수 있습니다. 서비스 ID 이름은 SAS 규칙 이름으로, 서비스 ID 키는 SAS 규칙 키로 설정될 수 있습니다. 그리고 나면 SAS 규칙의 권한이 엔터티에 해당하는 각 ACS 규칙과 동일하게 구성됩니다.
...
Azure Relay 인증에 대한 자세한 내용은 다음 항목을 참조하세요.

* [Azure Relay 인증 및 권한 부여](relay-authentication-and-authorization.md)
* [공유 액세스 서명을 사용한 Service Bus 인증](../service-bus-messaging/service-bus-sas.md)
