...
1. 이제 Visual Studio 편집기에서 *HomeController.cs* 파일을 열고 네임스페이스 정의를 다음 코드로 바꿉니다. 을 `yourServiceNamespace` 서비스 네임 스페이스의 이름으로,을 SAS 키로 바꾸어야 `yourKey` 합니다. 이 코드를 사용 하면 클라이언트에서 온-프레미스 서비스를 호출 하 여 호출 결과를 반환할 수 있습니다.
