...
컨테이너를 만듭니다. 이 메서드는 컨테이너가 없으면(그리고 **true**를 반환하면) 컨테이너를 만듭니다. 컨테이너가 있으면 **false**를 반환합니다. **createIfNotExist** 대신 **create** 메서드(컨테이너가 있으면 오류를 반환함)를 사용할 수 있습니다.
...
**MakeHTMLPage**라는 도우미 함수를 호출하여 지금 Azure 저장소 계정에 있는 Blob에 대한 원본 집합으로 **&lt;image\>** 요소가 포함된 기본 HTML 페이지를 만듭니다. **MakeHTMLPage**에 대한 코드는 이 항목의 뒷부분에서 다룹니다.
...
업로드된 Blob 목록에서 반복합니다. Blob이 Azure 저장소 계정에 있는 경우 각 Blob에 대해 HTML 페이지에 Blob의 URI에 보낸 **src** 특성이 있는 **&lt;img\>** 요소를 만듭니다. 이 샘플에서는 이미지를 하나만 추가했지만 더 추가하면 이 코드는 모든 이미지를 반복합니다.
