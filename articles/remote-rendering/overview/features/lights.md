...
모든 광원 형식은 추상 기본 클래스 `LightComponent` 에서 파생 되 고 다음 속성을 공유 합니다.
...
* **길이:** 및 `Radius` 가 `Length` 모두 0이 아니면 빛은 튜브 광원의 역할을 합니다. Neon 튜브를 시뮬레이션 하는 데 사용할 수 있습니다.

* **AttenuationCutoff:** 왼쪽에서 (0, 0) 조명의 감쇠는 해당 `Intensity`에 따라 다릅니다. 그러나 사용자 지정 최소/최대 거리를 제공할 수 있습니다 .이 거리는 조명의 농도가 0으로 선형적으로 조정 됩니다. 이 기능을 사용 하 여 특정 조명의 영향을 더 작은 범위에 적용할 수 있습니다.
...
`SpotLightComponent` 는와 유사 `PointLightComponent` 하지만 조명이 원뿔의 모양으로 제한 됩니다. 원뿔의 방향은 *소유자 엔터티의 음수 z 축*에 의해 정의 됩니다.
