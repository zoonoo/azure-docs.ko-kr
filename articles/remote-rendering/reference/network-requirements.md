...
* 네트워크에 경쟁 하는 트래픽이 없는 것으로 가정 하 여 Azure 원격 렌더링의 단일 사용자 세션에 대해 최소 **40 mbps** 및 **5 mbps** 의 인터넷 연결을 지원 해야 합니다. 환경을 개선 하기 위해 더 높은 요금을 권장 합니다. 
* **Wi-fi** 는 짧은 대기 시간, 높은 대역폭 및 안정적인 연결을 지원 하기 때문에 권장 되는 네트워크 유형입니다. 일부 모바일 네트워크에서는 잘못 된 환경을 유발할 수 있는 지터를 소개 합니다. 
* **5 ghz Wi-Fi 대역** 을 사용 하면 일반적으로 2.4 ghz Wi-Fi 대역 보다 더 나은 결과를 얻을 수 있지만 둘 다 작동 해야 합니다.
* 인접 한 다른 Wi-Fi 네트워크가 있는 경우 이러한 다른 네트워크에서 사용 하는 Wi-Fi 채널을 사용 하지 마십시오. [WifiInfoView](https://www.nirsoft.net/utils/wifi_information_view.html) 와 같은 네트워크 검색 도구를 사용 하 여 Wi-Fi 네트워크에서 사용 하는 채널이 경쟁 하는 트래픽을 사용 하는지 확인할 수 있습니다.
* 반복기 또는 powerline 전달 **Wi-Fi 사용 하지 않는** 것이 좋습니다.
* 동일한 Wi-Fi 네트워크에서 비디오 또는 게임 스트리밍 같은 **대역폭에 강한 트래픽을 방지** 합니다.
* 동일한 액세스 지점에 여러 장치가 있는 경우에는 요구 사항이 확장 됩니다. 환경에서 여러 액세스 지점이 있는 경우 액세스 지점의 장치 부하를 분산 하 여 균등 하 게 분산 됩니다.
...
1. **Azure 데이터 센터에 대 한 대기 시간을 측정 하는 www.azurespeed.com와 같은 도구를 사용** 합니다. 가장 가까운 Azure 원격 렌더링에서 지원 되는 Azure 데이터 센터 ( [지원 되는 지역](regions.md)참조)를 선택 하 고 **대기 시간 테스트** 를 실행 합니다. 표시 되는 숫자의 변형이 있는 경우 안정화 될 때 까지의 결과를 제공 합니다.
   * Azure 원격 렌더링의 **최소 요구 사항** : 대기 시간은 지속적으로 80 밀리초 미만 이어야 합니다.
   * Azure 원격 렌더링에 **권장** : 대기 시간은 지속적으로 40 밀리초 미만 이어야 합니다.
