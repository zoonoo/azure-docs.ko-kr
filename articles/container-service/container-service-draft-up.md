...
5. Draft를 구성하여 레지스트리를 사용하고 만든 각 Helm 차트에 대한 하위 도메인을 만듭니다. Draft을 구성하려면 다음이 필요합니다.
  - Azure Container Registry 이름(이 예제에서는 `draftacs`)
  - 레지스트리 키 또는 암호(`az acr credential show -n $acrname --output tsv --query "passwords[0].value"` 명령 사용)
  - Kubernetes 수신 외부 IP 주소(여기서는 `13.64.108.240`)에 매핑하도록 구성된 루트 배포 도메인

  이러한 값을 사용하여 구성 JSON 문자열(`{"username":"<user>","password":"<secret>","email":"email@example.com"}`)의 base 64로 인코딩된 값을 만들 수 있습니다. 값을 인코딩하는 한 가지 방법은 다음과 같습니다(그러나 이 예제의 값을 사용자 고유의 값으로 바꿈).
      ```bash
      acrname="draftacs"
  ...
  인코딩되지 않은 결과를 표시하려면 `echo $authtoken | base64 -D`를 입력하여 JSON 문자열이 올바른지 확인할 수 있습니다.
  이제 이 명령 및 `-set` 옵션에 대한 구성 인수로 Draft를 초기화합니다.
      ```bash
      draft init --set registry.url=$acrname.azurecr.io,registry.org=$acrname,registry.authtoken=$authtoken,basedomain=squillace.io
      ```
      > [!NOTE]
      > `basedomain` 값은 사용자가 제어하고 수신 외부 IP를 가리키도록 구성한 기본 배포 도메인입니다.
