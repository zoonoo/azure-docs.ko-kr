<properties
   pageTitle="ACS 앱에 공용 액세스를 사용하도록 설정 | Microsoft Azure"
...
# Azure Container Service 응용 프로그램에 공용 액세스를 사용하도록 설정
...
3. 에이전트 부하 분산 장치(**XXXX-agent-lb-XXXX**와 유사하게 이름 지정됨)를 선택합니다.
    ...
    ![Azure Container Service 부하 분산 장치](media/container-service-dcos-agents/agent-load-balancer.png)

4. **프로브**를 클릭한 후 **추가**를 클릭합니다.

    ![Azure Container Service 부하 분산 장치 프로브](media/container-service-dcos-agents/add-probe.png)
...
6. 에이전트 부하 분산 장치 속성으로 돌아가 **부하 분산 규칙**을 클릭한 후 **추가**를 클릭합니다.
    ...
    ![Azure Container Service 부하 분산 장치 규칙](media/container-service-dcos-agents/add-balancer-rule.png)
...
7. 부하 분산 장치 양식을 입력하고 **확인**을 클릭합니다.
    ...
    | 세션 지속성 | 세션 기간 동안 클라이언트의 트래픽을 처리하는 방식을 결정합니다.<br><br>**없음**: 동일한 클라이언트에서 오는 후속 요청은 어떤 컨테이너로든 처리할 수 있습니다.<br>**클라이언트 IP**: 동일한 클라이언트 IP에서 오는 후속 요청은 동일한 컨테이너로 처리합니다.<br>**클라이언트 IP 및 프로토콜**: 동일한 클라이언트 IP 및 프로토콜 조합의 후속 요청은 동일한 컨테이너로 처리합니다. |
...
3. **공용** 에이전트 네트워크 보안 그룹을 선택합니다(이름은 **XXXX-agent-public-nsg-XXXX**와 유사).
    ...
    ![Azure Container Service 네트워크 보안 그룹](media/container-service-dcos-agents/agent-nsg.png)

4. **인바운드 보안 규칙**을 선택한 후 **추가**를 선택합니다.

    ![Azure Container Service 네트워크 보안 그룹 규칙](media/container-service-dcos-agents/add-firewall-rule.png)
