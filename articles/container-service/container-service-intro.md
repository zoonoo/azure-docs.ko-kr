...
---
# <a name="azure-container-service-introduction"></a>Azure 컨테이너 서비스 소개
...
기본적으로 Azure Container Service에서 실행 중인 DC/OS에는 워크로드를 예약하기 위한 Marathon 오케스트레이션 플랫폼이 포함되어 있습니다. 그러나 ACS의 DC/OS 배포에는 서비스에 추가할 수 있는 서비스의 Mesosphere Universe가 포함되어 있으며 여기에는 Spark, Hadoop, Cassandra 등이 포함됩니다.
...
Kubernetes는 자동화된 컨테이너 배포, 컨테이너화된 응용 프로그램의 확장 및 관리를 위한 인기있는 오픈 소스 프로덕션 등급 컨테이너 오케스트레이터 도구입니다. Kubernetes는 오픈 소스 솔루션이며 오픈 소스 커뮤니티에서 구동하므로 Azure Container Service에서 원활하게 실행되며 이 서비스에서 컨테이너를 대규모로 배포하는 데 사용할 수 있습니다.

![Kubernetes를 사용하도록 구성된 Azure Container Service(에이전트 및 마스터 표시)](media/acs-intro/kubernetes.png)
