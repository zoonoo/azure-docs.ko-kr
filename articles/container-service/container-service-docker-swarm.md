...
Docker Swarm에 새 컨테이너를 만들려면 `docker run` 명령을 사용합니다. 이 예제는 `yeasy/simple-web` 이미지에서 컨테이너를 만듭니다.
...
![실제 방문 결과](media/real-visit.jpg)

## 여러 컨테이너 배포

여러 컨테이너가 Docker Swarm 클러스터에서 시작되기 때문에 `docker ps` 명령을 사용하여 컨테이너가 실행되고 있는 호스트를 확인할 수 있습니다. 이 예제에서는 세 개의 컨테이너가 세 개의 Swarm 에이전트 고르게 분포되어 있습니다.
...
Docker Compose를 사용하여 여러 컨테이너의 배포 및 구성을 자동화할 수 있습니다. 그러려면 SSH(Secure Shell) 터널이 생성되어 있고 DOCKER\_HOST 변수가 설정되었는지 확인합니다.
