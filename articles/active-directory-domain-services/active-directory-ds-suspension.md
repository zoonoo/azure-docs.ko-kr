...
ms.locfileid: "39126096"
---
# <a name="suspended-domains"></a>일시 중단된 도메인
Azure AD Domain Services가 관리되는 도메인을 오랫동안 서비스할 수 없는 경우, 관리되는 도메인이 일시 중단된 상태로 전환됩니다. 이 문서에서는 관리되는 도메인이 일시 중단된 이유와 일시 중단된 도메인을 수정하는 방법을 설명합니다.
...
앞의 그림은 Azure AD Domain Services 관리되는 도메인의 가능한 상태를 간략하게 설명합니다.
...
관리자가 작업을 수행해야 하는 문제가 하나 이상 있는 경우, 관리되는 도메인은 **주의 필요** 상태입니다. 이 상태에 있는 관리되는 도메인의 상태 페이지에는 하나 이상의 경고가 나열됩니다. 예를 들어, 가상 네트워크에 대해 제한적인 NSG를 구성한 경우, Microsoft에서 관리되는 도메인을 업데이트 및 모니터링하지 못할 수 있습니다. 이 잘못된 구성으로 인해 경고가 생성되고 관리되는 도메인이 ‘주의 필요’ 상태로 전환됩니다.

각 경고에 일련의 해결 단계가 있습니다. 일부 경고는 일시적이며, 서비스에서 자동으로 해결됩니다. 이 경고에 해당하는 해결 단계의 지침에 따라 다른 일부 경고를 해결할 수 있습니다. 몇 가지 중요한 경고를 해결하려면 Microsoft 지원에 문의해야 합니다.

자세한 내용은 [관리되는 도메인에서 경고의 문제를 해결하는 방법](active-directory-ds-troubleshoot-alerts.md)을 참조하세요.

**예상할 수 있는 작업:**

관리되는 도메인의 도메인 컨트롤러에 연결할 수 없는 경우가 있습니다(예: 잘못된 네트워크 구성이 있는 경우). Microsoft는 이 상태의 관리되는 도메인이 정기적으로 모니터링, 패치, 업데이트 또는 백업될 것을 보장할 수 없습니다.
...
* 관리되는 도메인에 영향을 주는 경고를 해결하면 관리되는 도메인을 ‘실행’ 상태로 복원할 수 있습니다.
* Microsoft에서 도메인 컨트롤러에 연결할 수 없는 구성 문제가 있을 경우, 중요한 경고가 트리거됩니다. 이러한 경고가 15일 이내에 해결되지 않으면 관리되는 도메인이 ‘일시 중단됨’ 상태로 전환됩니다.


### <a name="suspended-state"></a>‘일시 중지됨’ 상태
관리되는 도메인이 **일시 중단됨** 상태로 전환되는 이유는 다음과 같습니다.
* 15일 이내에 해결되지 않은 중요한 경고가 하나 이상 있습니다. 중요한 경고는 Azure AD Domain Services에 필요한 리소스에 대한 액세스를 차단하는 잘못된 구성으로 인해 발생할 수 있습니다.
    * 예를 들어, 관리되는 도메인에 대한 경고 [AADDS104: 네트워크 오류](active-directory-ds-troubleshoot-nsg.md)가 15일 이내에 해결되지 않은 경우입니다.
...
* 관리되는 도메인을 ‘일시 중단됨’ 상태로 전환하는 경고를 해결한 다음, 고객 지원팀에 문의하세요.
...
* 관리되는 도메인을 복원할 수 없으며, Azure AD Domain Services를 사용하려면 새 관리되는 도메인을 만들어야 합니다.
* 삭제된 후에는 관리되는 도메인에 대한 요금이 청구되지 않습니다.


## <a name="how-do-you-know-if-your-managed-domain-is-suspended"></a>관리되는 도메인이 일시 중단되었는지 확인하려면 어떻게 하나요?
Azure Portal의 Azure AD Domain Services 상태 페이지에 도메인이 일시 중단되었다고 선언하는 [경고](active-directory-ds-troubleshoot-alerts.md)가 표시됩니다. 도메인 상태도 “일시 중단됨”으로 표시됩니다.
...
2. 관리되는 도메인을 클릭합니다.
3. 왼쪽 탐색에서 **상태**를 클릭합니다.
4. 경고를 클릭합니다. 경고 ID는 일시 중단의 원인에 따라 AADDS503 또는 AADDS504입니다.
5. 경고에 제공된 해결 링크를 클릭하고 단계에 따라 경고를 해결합니다.
