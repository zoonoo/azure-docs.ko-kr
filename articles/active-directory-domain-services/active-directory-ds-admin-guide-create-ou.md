...
기본적으로 사용자 지정 OU를 만든 사용자('AAD DC 관리자' 그룹의 구성원)는 OU에 대해 관리자 권한(모든 권한)이 부여됩니다. 사용자는 계속해서 원하는 대로 다른 사용자에게 또는 'AAD DC 관리자' 그룹에 권한을 부여할 수 있습니다. 사용자 다음 스크린샷에 표시 된 대로 'bob@domainservicespreview.onmicrosoft.com' 새 'MyCustomOU' 조직 구성 단위를 만든 사람 것에 대 한 모든 권한을 부여 됩니다.
