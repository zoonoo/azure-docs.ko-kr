...
### 클라우드 전용 테넌트에 암호 동기화 사용
조직이 클라우드 전용 Azure AD 테넌트인 경우 Azure AD 도메인 서비스를 사용해야 하는 사용자는 암호를 변경해야 합니다. 이 단계를 수행하면 Kerberos 및 NTLM 인증을 위해 Azure AD 도메인 서비스에 필요한 자격 증명 해시가 Azure AD에서 생성됩니다. Azure AD 도메인 서비스를 사용해야 하는 테넌트의 모든 사용자에 대한 암호를 만료시키거나, 암호를 변경하도록 이러한 최종 사용자에게 지시할 수 있습니다.
...
사용자가 암호를 변경한 후 새 암호가 즉시 Azure AD 도메인 서비스에 동기화됩니다. 암호 동기화가 완료된 후 사용자가 새로 변경된 암호를 사용하여 도메인에 로그인할 수 있습니다.
...
도메인에 가입한 컴퓨터에 Azure AD Connect의 GA 릴리스를 설치해야 합니다. Azure AD Connect 설치의 기존 인스턴스가 있는 경우 Azure AD Connect GA 빌드를 사용하도록 업데이트해야 합니다.

  [Azure AD Connect 다운로드 - GA 릴리스](http://download.microsoft.com/download/B/0/0/B00291D0-5A83-4DE7-86F5-980BC00DE05A/AzureADConnect.msi)

> [AZURE.WARNING]Azure AD 테넌트에 동기화할 레거시 암호 자격 증명(NTLM 및 Kerberos 인증에 필요)을 사용하도록 설정하려면 Azure AD Connect의 GA 릴리스를 설치해야 합니다. 이 기능은 Azure AD Connect의 이전 릴리스에서 사용할 수 없습니다.
