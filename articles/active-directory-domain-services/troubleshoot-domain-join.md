...
1. Azure AD DS 관리 되는 도메인의 도메인 이름 (예:)을 사용 하 여 도메인 `ping aaddscontoso.com`에 ping을 시도 합니다.
    * Ping 응답이 실패할 경우 포털의 개요 페이지에 표시 되는 도메인의 IP 주소를 ping 하 여 Azure AD DS 관리 되는 도메인 (예:)에 대 `ping 10.0.0.4`한 IP 주소를 ping 해 봅니다.
