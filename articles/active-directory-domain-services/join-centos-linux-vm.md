...
* 관리 되는 도메인이 아닌 다른 서브넷에 VM을 배포 합니다.
...
Vm을 관리 되는 도메인에 가입 하려면 VM에 몇 가지 추가 패키지가 필요 합니다. 이러한 패키지를 설치 하 고 구성 하려면 다음을 사용 하 여 도메인 가입 도구를 업데이트 하 고 설치 합니다 `yum` .
