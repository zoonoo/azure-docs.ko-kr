---
title: RHEL VM을 Azure AD 도메인 서비스에 가입 | 마이크로 소프트 문서
description: Red Hat 엔터프라이즈 Linux 가상 컴퓨터를 Azure AD 도메인 서비스 관리 도메인에 구성하고 가입하는 방법을 알아봅니다.
...
이 문서에서는 Red Hat 엔터프라이즈 Linux(RHEL) VM을 Azure AD DS 관리 도메인에 가입하는 방법을 보여 주며 있습니다.
...
## <a name="create-and-connect-to-a-rhel-linux-vm"></a>RHEL 리눅스 VM 생성 및 연결

Azure에 기존 RHEL Linux VM이 있는 경우 SSH를 사용하여 연결한 다음 다음 단계로 계속하여 [VM 구성을 시작합니다.](#configure-the-hosts-file)

RHEL Linux VM을 만들어야 하거나 이 문서에서 사용할 테스트 VM을 만들려면 다음 방법 중 하나를 사용할 수 있습니다.
...
VM은 Azure AD DS 관리 도메인에 VM을 조인하기 위해 몇 가지 추가 패키지가 필요합니다. 이러한 패키지를 설치하고 구성하려면 을 사용하여 `yum`도메인 조인 도구를 업데이트하고 설치합니다. RHEL 7.x와 RHEL 6.x 간에는 몇 가지 차이점이 있으므로 이 문서의 나머지 섹션에서 배포판 버전에 적합한 명령을 사용하십시오.
...
필요한 패키지가 VM에 설치되었으므로 VM을 Azure AD DS 관리 도메인에 조인합니다. 다시, RHEL 배포판 버전에 대 한 적절 한 단계를 사용 합니다.
...
1. `adcli info` 명령을 사용하여 Azure AD DS 관리 도메인을 검색합니다. 다음 예제는 *ADDDSCONTOSO.COM*영역을 검색합니다. 모든 대문자에서 고유한 Azure AD DS 관리 도메인 이름을 지정합니다.
...
1. 먼저 명령을 사용하여 도메인에 `adcli join` 가입하면 이 명령은 컴퓨터를 인증하는 키탭도 만듭니다. Azure AD DS 관리 도메인의 일부인 사용자 계정을 사용합니다.

    ```console
    sudo adcli join aaddscontoso.com -U contosoadmin
    ```

1. 이제 `aaddscontoso.com` Active `/ect/krb5.conf` Directory `/etc/sssd/sssd.conf` 도메인을 사용하도록 파일을 구성하고 만듭니다.
   고유한 `AADDSCONTOSO.COM` 도메인 이름으로 대체되었는지 확인합니다.

    편집기와 `/ect/krb5.conf` 함께 파일을 엽니다.

    ```console
    sudo vi /etc/krb5.conf
    ```

    다음 `krb5.conf` 샘플과 일치하도록 파일을 업데이트합니다.
   ...
   `/etc/sssd/sssd.conf` 파일 만들기:
    
    ```console
    sudo vi /etc/sssd/sssd.conf
    ```

    다음 `sssd.conf` 샘플과 일치하도록 파일을 업데이트합니다.
...
1. 사용 `/etc/sssd/sssd.conf` 권한이 600이고 루트 사용자가 소유하고 있는지 확인합니다.
...
1. AD Linux 통합에 대해 VM에 지시하는 데 사용합니다. `authconfig`

    ```console
    sudo authconfig --enablesssd --enablesssdauth --update
    ```

1. sssd 서비스를 시작하고 활성화하십시오.
...
이제 다음을 사용하여 사용자 AD 정보를 쿼리할 수 있는지 확인합니다.`getent`
...
1. 변경 내용을 적용하고 사용자가 암호를 사용하여 로그인할 수 있도록 하려면 RHEL 배포판 버전에 대한 SSH 서비스를 다시 시작하십시오.
...
RHEL VM에서 *AAD DC Administrators* 그룹 관리 권한의 구성원에게 권한을 부여하려면 */etc/sudoers*에 항목을 추가합니다. 추가되면 *AAD DC 관리자* 그룹의 구성원은 `sudo` RHEL VM에서 명령을 사용할 수 있습니다.
...
1. 본체에서 새 SSH 연결을 만듭니다. rhel.aaddscontoso.com 와 같은 VM의 주소를 `ssh -l` 입력하는 등의 `contosoadmin@aaddscontoso.com` 명령을 사용하여 관리되는 도메인에 속한 도메인 계정을 *사용합니다.* Azure 클라우드 셸을 사용하는 경우 내부 DNS 이름이 아닌 VM의 공용 IP 주소를 사용합니다.
