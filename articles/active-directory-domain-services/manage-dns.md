...
---
# <a name="administer-dns-and-create-conditional-forwarders-in-an-azure-ad-domain-services-managed-domain"></a>Azure AD Domain Services 관리 되는 도메인에서 DNS를 관리 하 고 조건부 전달자를 만듭니다.
...
하이브리드 환경에서 온-프레미스 AD DS 환경과 같은 다른 DNS 네임 스페이스에서 구성 된 DNS 영역 및 레코드는 Azure AD DS에 동기화 되지 않습니다. 다른 DNS 네임 스페이스의 명명 된 리소스를 해결 하려면 환경에서 기존 DNS 서버를 가리키는 조건부 전달자를 만들고 사용 합니다.
...
1. **Dns 서버에 연결** 대화 상자에서 **다음 컴퓨터**를 선택 하 고 관리 되는 도메인의 DNS 도메인 이름 (예: *aaddscontoso.com*)을 입력 합니다.
    ...
    ![DNS 콘솔에서 Azure AD DS 관리 되는 도메인에 연결](./media/manage-dns/connect-dns-server.png)
...
Azure AD DS DNS 영역에는 관리 되는 도메인에 대 한 영역 및 레코드만 포함 되어야 합니다. 다른 DNS 네임 스페이스의 명명 된 리소스를 확인 하기 위해 Azure AD DS에 추가 영역을 만들지 마세요. 대신 Azure AD DS 관리 되는 도메인의 조건부 전달자를 사용 하 여 DNS 서버에 해당 리소스에 대 한 주소를 확인 하기 위해 이동할 위치를 알려 줍니다.

조건부 전달자는 쿼리를 전달할 dns 도메인 (예: *contoso.com*)을 정의 하는 데 사용할 수 있는 dns 서버의 구성 옵션입니다. 로컬 DNS 서버에서 해당 도메인의 레코드에 대 한 쿼리를 확인 하는 대신 DNS 쿼리는 해당 도메인에 대해 구성 된 DNS에 전달 됩니다. 이 구성을 사용 하면 Azure AD DS 관리 되는 도메인에 중복 레코드를 사용 하 여 해당 리소스를 반영 하는 로컬 DNS 영역을 만들지 않으므로 올바른 DNS 레코드가 반환 되는지 확인할 수 있습니다.

Azure AD DS 관리 되는 도메인에서 조건부 전달자를 만들려면 다음 단계를 완료 합니다.

1. *Aaddscontoso.com*와 같은 AZURE AD DS DNS 영역을 선택 합니다.
...
이제 Azure AD DS 관리 되는 도메인에 연결 된 Vm의 다른 네임 스페이스에 있는 리소스의 이름 확인이 올바르게 확인 되어야 합니다. 조건부 전달자에 구성 된 DNS 도메인에 대 한 쿼리는 관련 DNS 서버로 전달 됩니다.
