...
아닙니다. 단일 Azure AD 디렉터리에 Azure AD Domain Services에서 제공하는 단일 관리되는 도메인만 만들 수 있습니다.
...
클래식 가상 네트워크는 새 배포에 대해 지원 되지 않습니다. 클래식 가상 네트워크에 배포 된 기존의 관리 되는 도메인은 계속 지원 됩니다.
...
예. Azure Resource Manager 가상 네트워크에서 Azure AD Domain Services를 사용할 수 있습니다. 클래식 Azure 가상 네트워크는 새 관리 되는 도메인을 만들 때 더 이상 지원 되지 않습니다.
...
예,이 기능은 미리 보기 상태입니다. 자세한 내용은 [클래식 가상 네트워크 모델에서 리소스 관리자로 Azure AD Domain Services 마이그레이션 (미리 보기)](migrate-from-classic-vnet.md)을 참조 하세요.
...
아닙니다. NTLM 또는 Kerberos를 통해 사용자를 인증 하려면 사용자 계정의 암호 해시에 액세스 해야 Azure AD Domain Services. 페더레이션된 디렉터리에서 암호 해시는 Azure AD 디렉터리에 저장 되지 않습니다. 따라서 Azure AD Domain Services는 이러한 Azure AD 디렉터리에서 작동 하지 않습니다.
...
아니요, 현재는 템플릿을 사용 하 Azure AD Domain Services를 사용 하도록 설정할 수 없습니다. 스크립팅된 방법은 [PowerShell을 사용 하 여 Azure AD Domain Services를 사용 하도록 설정 하는 방법](powershell-create-instance.md)을 참조 하세요.
...
아닙니다. 스키마는 관리되는 도메인에 대해 Microsoft에서 관리합니다. Azure AD Domain Services에서 스키마 확장을 지원 하지 않습니다.
...
아닙니다. Azure AD Domain Services 관리 되는 도메인을 사용 하도록 설정 하면 관리 되는 도메인을 삭제할 때까지 선택한 가상 네트워크 내에서 서비스를 사용할 수 있습니다. 서비스를 일시 중지할 수 있는 방법은 없습니다. 관리되는 도메인을 삭제할 때까지 시간 기준으로 계속 청구됩니다.
...
아닙니다. Azure AD Domain Services은 현재 지역 중복 배포 모델을 제공 하지 않습니다. Itin은 Azure 지역의 단일 가상 네트워크로 제한 됩니다. 여러 Azure 지역을 사용하려면 Azure IaaS VM에서 Active Directory 도메인 컨트롤러를 실행해야 합니다. 아키텍처 지침은 [여기](https://docs.microsoft.com/azure/architecture/reference-architectures/identity/adds-extend-domain)에서 찾을 수 있습니다.
...
아닙니다. Azure AD Domain Services는 종 량 제 Azure 서비스 이며 EMS의 일부가 아닙니다. Azure AD Domain Services은 모든 버전의 Azure AD (무료 및 프리미엄)에서 사용할 수 있습니다. 사용량에 따라 시간 단위로 요금이 청구 됩니다.
