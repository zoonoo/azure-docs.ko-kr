...
클래식 가상 네트워크는 새 배포에 대해 지원 되지 않습니다. 클래식 가상 네트워크에 배포 된 기존의 관리 되는 도메인은 계속 지원 됩니다. [클래식 가상 네트워크 모델에서 리소스 관리자 (미리 보기)로 Azure AD Domain Services을 마이그레이션할](migrate-from-classic-vnet.md)수도 있습니다.
...
아닙니다. Azure AD Domain Services은 현재 지역 중복 배포 모델을 제공 하지 않습니다. Azure 지역의 단일 가상 네트워크로 제한 됩니다. 여러 Azure 지역을 사용하려면 Azure IaaS VM에서 Active Directory 도메인 컨트롤러를 실행해야 합니다. 아키텍처 지침은 [온-프레미스 Active Directory 도메인을 Azure로 확장](https://docs.microsoft.com/azure/architecture/reference-architectures/identity/adds-extend-domain)을 참조 하세요.
