...
7. **도메인 서비스의 DNS 도메인 이름**을 지정합니다.
   ...
   - 또한 사용자 지정 도메인 이름을 이 목록에 직접 입력하여 추가할 수 있습니다.
     ...
     > [AZURE.WARNING] 지정한 도메인 이름의 도메인 접두사(예: 'contoso.com' 도메인 이름에서 'contoso')는 15자 이내로 합니다. 15자 보다 긴 도메인 접두사로 Azure AD 도메인 서비스 도메인을 만들 수 없습니다.
...
8. 다음 단계에서는 Azure AD 도메인 서비스를 사용할 수 있는 가상 네트워크를 선택합니다. **해당 가상 네트워크에 도메인 서비스 연결** 드롭다운에 방금 만든 가상 네트워크를 선택합니다.
   ...
   - Azure AD 도메인 서비스를 지원하지 않는 영역에 속한 가상 네트워크는 드롭 다운 목록에 표시되지 않습니다.
   - 마찬가지로, Azure Resource Manager를 사용하여 만든 가상 네트워크(ARM 기반 가상 네트워크)는 드롭 다운 목록에 표시되지 않습니다. ARM 기반 가상 네트워크가 현재 Azure AD 도메인 서비스에서 지원되지 않기 때문입니다.

9. 관리되는 도메인에서 선택한 DNS 도메인 이름은 가상 네트워크에 존재하지 않도록 합니다. 이는 다음과 같은 시나리오 중 하나에서 발생할 수 있습니다.
...
10. 위의 옵션을 선택하는 작업이 완료되면 페이지의 맨 아래에 있는 작업창에서 **저장**을 클릭하여 Azure AD 도메인 서비스를 사용합니다.

11. Azure AD 도메인 서비스를 디렉터리에 사용하는 동안 페이지는 '보류 중...' 상태를 표시합니다.
...
12. 20-30분 후에 **구성** 페이지의 **IP 주소** 필드 중 가상 네트워크에서 사용할 수 있는 도메인 서비스가 있는 첫 번째 IP 주소를 확인합니다.
...
13. 높은 가용성이 도메인에 대해 작동할 때 페이지에 표시되는 두 개의 IP 주소를 확인합니다. Azure AD 도메인 서비스를 선택된 가상 네트워크에서 사용할 수 있는 IP 주소가 있습니다. 이러한 IP 주소 적어두어 가상 네트워크에 대한 DNS 설정을 업데이트하도록 합니다. 이 단계는 도메인 가입과 같은 작업을 위해 도메인에 연결할 가상 네트워크에 가상 컴퓨터를 사용할 수 있습니다.
