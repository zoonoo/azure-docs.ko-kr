...
Azure Logic Apps 및 파일 시스템 커넥터를 사용하면 다음과 같이 온-프레미스 파일 공유에 파일을 만들어서 관리하는 자동화된 작업 및 워크플로를 만들 수 있습니다.
...
- 파일 콘텐츠 및 메타데이터를 가져옵니다.
  ...
  > [!IMPORTANT]
  > 파일 시스템 커넥터는 현재 Windows 운영 체제의 Windows 파일 시스템만 지원합니다.  

이 문서에서는 [Dropbox에 업로드된 파일을 파일 공유에 복사한 후 이메일 보내기] 예제 시나리오에서 설명한 것처럼, 온-프레미스 파일 시스템에 연결하는 방법을 보여줍니다. 온-프레미스 시스템에 안전하게 연결하고 액세스할 수 있도록 논리 앱은 [온-프레미스 데이터 게이트웨이](../logic-apps/logic-apps-gateway-connection.md)를 사용합니다. 논리 앱을 처음 접하는 경우 [Azure Logic Apps란?](../logic-apps/logic-apps-overview.md)을 검토합니다. 커넥터 관련 기술 정보는 [파일 시스템 커넥터 참조](/connectors/filesystem/)를 참조하세요.
...
* [Dropbox 계정](https://www.dropbox.com/) - 무료로 등록할 수 있습니다. 논리 앱과 Dropbox 계정 간 연결을 만들려면 계정 자격 증명이 필요합니다.

* 사용하려는 파일 시스템이 있는 컴퓨터에 대한 액세스 권한. 예를 들어 파일 시스템과 동일한 컴퓨터에 데이터 게이트웨이를 설치하는 경우 해당 컴퓨터의 계정 자격 증명이 필요합니다.

* Office 365 Outlook, Outlook.com, Gmail 등 Logic Apps에서 지원되는 공급자의 이메일 계정. 다른 공급자에 대한 내용은 [여기서 커넥터 목록을 검토하세요](/connectors/). 이 논리 앱은 회사 또는 학교 계정을 사용합니다. 다른 이메일 계정을 사용하는 경우 전체 단계는 동일하지만 UI가 약간 다를 수 있습니다.
...
1. 트리거 아래에서 **다음 단계** 를 선택합니다. 검색 상자에 "파일 시스템"을 필터로 입력합니다. 작업 목록에서 **파일 만들기** 작업을 선택합니다.
...
1. 파일 시스템에 아직 연결하지 않은 경우 연결을 생성하라는 메시지가 표시됩니다.
   ...
   | **인증 유형** | 예 | <*auth-type*> | 파일 시스템에서 사용되는 인증 유형입니다(**Windows**). |
   | **사용자 이름** | 예 | <*domain*>\\<*username*> <p>또는 <p><*local-computer*>\\<*username*> | 파일 시스템 폴더가 있는 컴퓨터의 사용자 이름입니다. <p>파일 시스템 폴더가 온-프레미스 데이터 게이트웨이와 동일한 컴퓨터에 있는 경우 <*local-computer*>\\<*username*>을 사용할 수 있습니다. |
...
커넥터의 Swagger 파일에 설명된 트리거, 작업, 제한 등 이 커넥터에 대한 자세한 기술 정보는 [커넥터의 참조 페이지](/connectors/fileconnector/)에서 확인할 수 있습니다.
