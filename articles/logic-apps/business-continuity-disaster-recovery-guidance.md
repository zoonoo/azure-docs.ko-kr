...
Azure Logic Apps는 논리 앱이 다른 앱, 서비스, 시스템 및 기타 리소스 (예: Azure Storage 계정, SQL Server 데이터베이스, Office 365 Outlook 전자 메일 계정 등)로 작업 하는 데 사용할 수 있는 [수백 개의 관리 되는 커넥터와 기본 제공 트리거 및 작업](../connectors/apis-list.md) 을 제공 합니다. 논리 앱에서 이러한 리소스에 액세스 해야 하는 경우 이러한 리소스에 대 한 액세스를 인증 하는 연결을 만듭니다. 각 연결은 특정 위치에 있는 별도의 Azure 리소스 이며 다른 위치의 리소스에서 사용할 수 없습니다.
...
| *능동-수동* | 주 논리 앱 인스턴스는 전체 워크 로드를 적극적으로 처리 하는 반면, 보조 인스턴스는 수동 (비활성화 또는 비활성)입니다. 보조는 중단 또는 실패로 인해 주 복제본을 사용할 수 없거나 작동 하지 않는 신호를 대기 하 고 작업을 활성 인스턴스로 사용 합니다. |
