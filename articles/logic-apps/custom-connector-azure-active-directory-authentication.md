...
이 자습서는 Azure Resource Manager API를 예로 들어 설명합니다. Azure Resource Manager는 데이터베이스, 가상 머신 및 웹앱과 같이 Azure에 구축한 솔루션의 구성 요소를 관리하는 데 도움이 됩니다. Azure Resource Manager의 사용자 지정 커넥터는 워크플로에서 Azure 리소스를 관리하려는 경우에 유용할 수 있습니다. 자세한 내용은 [Azure 리소스 관리자 개요](../azure-resource-manager/resource-group-overview.md)를 참조하세요.
...
1. [Azure Portal](https://portal.azure.com)에 로그인합니다. Azure Active Directory 테넌트가 둘 이상인 경우 사용자 이름 아래 디렉터리를 확인하여 올바른 디렉터리에 로그인했는지 확인합니다. 
...
2. 현재 디렉터리를 볼 수 있도록 기본 Azure 메뉴에서 **Azure Active Directory**를 선택합니다.
   ...
   > [!TIP]
   > Azure 기본 메뉴에 **Azure Active Directory**가 표시되지 않으면 **All services**를 선택합니다. **필터** 상자에 "Azure Active Directory"를 필터로 입력한 다음 **Azure Active Directory**를 선택합니다.
