...
이 섹션에는 Azure Logic Apps 서비스에 대 한 인바운드 IP 주소만 나열 됩니다. 보안 규칙을 만들 때 복잡성을 줄이기 위해 각 지역에 대 한 인바운드 Logic Apps IP 주소 접두사를 지정 하는 대신 [서비스 태그](../virtual-network/service-tags-overview.md) **LogicAppsManagement**를 선택적으로 사용할 수 있습니다. 이 태그는 Logic Apps 서비스를 사용할 수 있는 지역에서 작동 합니다. Azure Government 경우 [Azure Government 인바운드 IP 주소](#azure-government-inbound)를 참조 하세요.
...
이 섹션에서는 Azure Logic Apps 서비스 및 관리 되는 커넥터에 대 한 아웃 바운드 IP 주소를 나열 합니다. 보안 규칙을 만들 때 복잡성을 줄이기 위해 각 지역에 대해 아웃 바운드 Logic Apps IP 주소 접두사를 지정 하는 대신 [서비스 태그](../virtual-network/service-tags-overview.md) **LogicApps**를 선택적으로 사용할 수 있습니다. 이 태그는 Logic Apps 서비스를 사용할 수 있는 지역에서 작동 합니다. 관리 되는 커넥터의 경우 IP 주소를 사용 합니다. Azure Government 경우 [Azure Government-아웃 바운드 IP 주소](#azure-government-outbound)를 참조 하세요.
