...
1. 응답 동작의 **Body** 속성에서 트리거의 상대 경로에 지정한 매개 변수를 나타내는 토큰을 포함 합니다.
   ...
   **본문** 속성에 후행 공백을 사용 `Postal Code: ` 하 여을 입력 합니다. 표시 되는 동적 콘텐츠 목록에서 **postalCode** 토큰을 선택 합니다.
...
HTTP 끝점을 만든 후에는 끝점의 전체 URL에 HTTP `POST` 요청을 전송 하 여 논리 앱을 트리거할 수 있습니다. Logic Apps는 직접 액세스 엔드포인트에 대한 기본 제공 지원을 포함합니다.
