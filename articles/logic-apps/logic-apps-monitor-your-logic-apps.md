---
title: 상태를 확인 하 고, 로깅을 설정 하 고, 경고를 가져옵니다.
description: 상태를 모니터링 하 고, 로깅을 설정 하 고, Azure Logic Apps에서 오류 및 문제를 진단 하기 위한 경고를 설정 합니다.
services: logic-apps
...
오류 또는 가능한 다른 문제에 대한 알림을 받으려면 [경고](#add-azure-alerts)를 설정합니다. 예를 들어 "한 시간에 5개 이상의 실행이 실패하는 경우"를 검색하는 경고를 만들 수 있습니다. [Azure Diagnostics 이벤트 설정 및 속성](#diagnostic-event-properties)을 사용하여 프로그래밍 방식으로 모니터링, 추적 및 로깅을 설정할 수도 있습니다.
...
런타임 세부 정보 및 이벤트를 사용 하 여 보다 다양 한 디버깅을 위해 [Azure Monitor 로그](../log-analytics/log-analytics-overview.md)를 사용 하 여 진단 로깅을 설정할 수 있습니다. Azure Monitor은 클라우드 및 온-프레미스 환경을 모니터링 하 여 가용성과 성능을 유지 하는 데 도움이 되는 Azure의 서비스입니다. 
...
설정하려는 옵션에 따라 먼저 [Azure Storage 계정을 만들](../storage/common/storage-create-storage-account.md)거나 [Azure 이벤트 허브를 만들](../event-hubs/event-hubs-create.md)어야 합니다. 그런 다음 진단 데이터를 전송하려는 위치에 대한 옵션을 선택합니다.

![Azure Storage 계정 또는 이벤트 허브로 데이터 보내기](./media/logic-apps-monitor-your-logic-apps/storage-account-event-hubs.png)
...
[Azure Monitor 로그](../log-analytics/log-analytics-overview.md)를 사용 하지 않고 경고를 설정 하려면 다음 단계를 수행 합니다. 고급 경고 조건 및 작업에 대 한 자세한 내용은 [Azure Monitor 로그를 설정](#azure-diagnostics) 합니다.
...
* `trackedProperties`: 진단 데이터에서 입력 또는 출력을 추적하기 위해 추적된 속성을 논리 앱의 JSON 정의에서 작업에 추가할 수 있습니다. 추적된 속성으로 단일 작업의 입력 및 출력만 추적할 수 있지만 이벤트의 `correlation` 속성을 사용하여 실행에 있는 작업 간에 상호 연결할 수 있습니다.
  ...
  ```
  **변수 초기화** 작업을 사용 하는 또 다른 예는 다음과 같습니다. 이 예제에서는 레코드가 아닌 배열이 아니라 작업의 입력에서 추적 된 속성을 추가 합니다.  
