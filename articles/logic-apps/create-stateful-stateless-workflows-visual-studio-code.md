...
1. [저장소 계정의 연결 문자열을 찾아 복사 합니다](../storage/common/storage-account-keys-manage.md?tabs=azure-portal#view-account-access-keys). 예를 들면 다음과 같습니다.
...
1. 워크플로에 대 한 프로젝트 폴더를 확장 합니다. 파일의 바로 가기 메뉴에서 **workflow.js** 을 열고 **디자이너에서 열기** 를 선택 합니다.
   ...
   * **Macos**: 다음 해결 방법을 시도해 보세요. 실패 한 경우 [문제 및 오류 해결](#troubleshooting)을 참조 하세요.
     ...
     1. 프로젝트에서 파일 **의local.settings.js** 를 엽니다 .이 파일은 프로젝트의 루트 폴더와 **워크플로-designtime** 폴더에서 찾을 수 있습니다.
...
로컬 디버깅 세션을 처음 시작 하거나 디버깅 하지 않고 워크플로를 실행 하는 경우 Logic Apps 런타임은 워크플로를 서비스 끝점에 등록 하 고 웹 후크 작업을 알리기 위해 해당 끝점을 구독 합니다. 다음 번에 워크플로를 실행 하면 구독 등록이 로컬 저장소에 이미 있기 때문에 런타임이 등록 되거나 재구독 되지 않습니다.
...
Application Insights 열리면 논리 앱에 대 한 다양 한 메트릭을 검토할 수 있습니다. 자세한 내용은 [Azure Logic Apps 실행-Application Insights으로 모니터링-1 부](https://techcommunity.microsoft.com/t5/integrations-on-azure/azure-logic-apps-running-anywhere-monitor-with-application/ba-p/1877849)를 참조 하세요.
...
* [Docker란 무엇인가요?](/dotnet/architecture/microservices/container-docker-introduction/docker-defined)
...
* Docker 컨테이너를 빌드할 때 사용 하는 .NET 워크플로에 대 한 Docker 파일

   예를 들어이 샘플 Docker 파일은 상태 저장 워크플로를 사용 하 여 논리 앱을 배포 합니다. 이 파일은 Azure Portal에 논리 앱을 게시 하는 데 사용 된 Azure Storage 계정에 대 한 연결 문자열 및 액세스 키를 지정 합니다.
...
### <a name="build-and-publish-your-app"></a>앱 빌드 및 게시

1. 논리 앱의 프로젝트를 로컬로 빌드하려면 명령줄 프롬프트를 열고 다음 명령을 실행 합니다.

   `dotnet build -c release`

   자세한 내용은 [dotnet 빌드](/dotnet/core/tools/dotnet-build/) 참조 페이지를 참조 하세요.

1. 다음 명령을 실행 하 여 호스팅 환경에 배포 하는 데 사용할 폴더에 프로젝트 빌드를 게시 합니다.

   `dotnet publish -c release`

   자세한 내용은 [dotnet publish](/dotnet/core/tools/dotnet-publish/) 참조 페이지를 참조 하세요.

### <a name="access-to-your-storage-account"></a>저장소 계정에 대 한 액세스

Docker 컨테이너를 빌드하고 실행 하기 전에 저장소 계정에 대 한 액세스 키가 포함 된 연결 문자열을 가져와야 합니다.

1. Azure Portal 저장소 계정 메뉴의 **설정** 에서 **액세스 키** 를 선택 합니다. 

   ![저장소 계정 액세스 키와 연결 문자열이 복사 된 Azure Portal를 보여 주는 스크린샷](./media/create-stateful-stateless-workflows-visual-studio-code/find-storage-account-connection-string.png)

1. **연결 문자열** 에서 저장소 계정의 연결 문자열을 복사 합니다. 연결 문자열은 다음 샘플과 유사 합니다.

   `DefaultEndpointsProtocol=https;AccountName=fabrikamstorageacct;AccountKey={access-key};EndpointSuffix=core.windows.net`

   자세한 내용은 [저장소 계정 키 관리](../storage/common/storage-account-keys-manage.md?tabs=azure-portal#view-account-access-keys)를 참조 하세요.

1. 안전한 곳에 연결 문자열을 저장 합니다. 논리 앱 프로젝트에서이 문자열을 파일 **의 두local.settings.js** 에 모두 추가 해야 합니다. 또한 Docker 파일에이 문자열을 추가 해야 합니다.
...
요청 트리거에 대 한 콜백 URL을 가져오려면 다음 요청을 보냅니다.
...
<*마스터 키*> 값은 `AzureWebJobsStorage` 이 섹션의 값을 찾을 수 있는 **Azure-webjobs/{deployment-name}/host.js** 파일에서에 대해 설정한 Azure Storage 계정에 정의 됩니다.
...
### <a name="opening-designer-fails-with-error-workflow-design-time-could-not-be-started"></a>"워크플로 디자인 시간을 시작할 수 없습니다." 라는 오류로 인해 디자이너를 열지 못했습니다.
