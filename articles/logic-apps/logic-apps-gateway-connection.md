...
* 온-프레미스 데이터 게이트웨이를 설치할 때 사용한 것 [과 동일한 azure 계정 및 azure 구독이](../logic-apps/logic-apps-gateway-install.md#requirements) 있습니다.

* 이전에 게이트웨이 설치를 Azure의 다른 게이트웨이 리소스에 연결 하지 않았습니다.

  게이트웨이 리소스를 만들 때 게이트웨이 설치를 선택 하 여 게이트웨이 리소스와 연결 합니다. 게이트웨이 리소스를 만들 때에는 이미 연결 된 게이트웨이 설치를 선택할 수 없습니다.
...
1. **연결 게이트웨이 만들기**에서 게이트웨이 리소스에 대해이 정보를 제공 합니다. 완료되면 **만들기**를 선택합니다.
   ...
   ![온-프레미스 데이터 게이트웨이를 만들기 위해 세부 정보 제공](./media/logic-apps-gateway-connection/gateway-details.png)
...
1. **게이트웨이의**경우 만든 게이트웨이 리소스를 선택 합니다.
