...
여기에서 원하는 작업을 찾지 못하면 Azure Logic Apps에서 제공하는 다양한 [데이터 조작 함수를](../logic-apps/workflow-definition-language-functions-reference.md) 탐색해 보십시오.
...
여러 입력에서 JSON 개체와 같은 단일 출력을 생성하려면 **[작성** 작업]을 사용할 수 있습니다. 입력에는 정수, 부울, 배열, JSON 개체 및 Azure Logic Apps에서 지원하는 다른 원시 형식(예: 이진 및 XML)과 같은 다양한 형식이 있습니다. 그런 다음, **작성** 작업 이후에 수행하는 작업에서 출력을 사용할 수 있습니다. **작성** 작업을 사용하면 논리 앱의 워크플로를 작성하는 동안 동일한 입력을 반복적으로 입력하지 않도록 할 수도 있습니다.
...
1. 출력을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다. 
   ...
   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.
     ...
     !["작성" 동작에 대해 "새 단계"를 선택합니다.](./media/logic-apps-perform-data-operations/add-compose-operation-action.png)
...
1. **입력** 상자에서 출력을 만들기 위해 원하는 입력을 제공합니다.
   ...
   !["컴포지션" 동작에 대한 완성된 예제](./media/logic-apps-perform-data-operations/finished-compose-action.png)
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **작성** 작업 아래에서 **출력**을 선택합니다.
   ...
   !["컴포지션" 작업에 대한 "출력" 필드](./media/logic-apps-perform-data-operations/send-email-compose-action.png)

1. 이제 논리 앱을 수동으로 실행합니다. 디자이너 도구 모음에서 **실행을**선택합니다.
...
배열에서 자바스크립트 개체 표기(JSON) 개체의 속성과 값이 있는 CSV(쉼표 구분값) 테이블을 만들려면 **CSV 테이블 만들기** 작업을 사용합니다. 그런 다음, **CSV 테이블 만들기** 작업 이후에 수행하는 작업에서 결과 테이블을 사용할 수 있습니다.
...
1. CSV 테이블을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다. 
   ...
   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.
     ...
     !["CSV 테이블 만들기" 작업에 대해 "새 단계" 선택](./media/logic-apps-perform-data-operations/add-create-table-action.png)
...
1. **원본** 상자에서 테이블을 만들기 위해 원하는 배열 또는 식을 제공합니다.
   ...
   !["CSV 테이블 만들기" 작업에 대한 예제 완료](./media/logic-apps-perform-data-operations/finished-create-csv-table-action.png)

1. 논리 앱을 저장합니다. 디자이너 도구 모음에서 **저장**을 선택합니다.

### <a name="customize-table-format"></a>테이블 형식 사용자 지정

기본적으로 **Columns** 속성은 배열 항목을 기반으로 테이블 열을 자동으로 만들도록 설정됩니다. 사용자 지정 헤더 및 값을 지정하려면 다음 단계를 따르십시오.

1. **열** 목록을 열고 사용자 **지정**을 선택합니다.

1. **Header** 속성에서 대신 사용할 사용자 지정 헤더 텍스트를 지정합니다.

1. **Value** 속성에서 대신 사용할 사용자 지정 값을 지정합니다.

배열에서 값을 반환하려면 **CSV 테이블 만들기** 작업과 함께 [ `item()` 함수를](../logic-apps/workflow-definition-language-functions-reference.md#item) 사용할 수 있습니다. `For_each` 루프에서 이 [ `items()` 함수를](../logic-apps/workflow-definition-language-functions-reference.md#items)사용할 수 있습니다.

예를 들어 배열의 속성 이름이 아닌 속성 값만 있는 테이블 열을 원한다고 가정합니다. 이러한 값만 반환하려면 디자이너 보기 또는 코드 보기에서 작업하기 위한 다음 단계를 따르십시오. 이 예제에서 반환하는 결과는 다음과 같습니다.
...
작업에서 **헤더** 열을 비워 두십시오. **Value** 열의 각 행에서 원하는 각 배열 속성을 참조합니다. **Value** 아래의 각 행은 지정된 배열 속성에 대한 모든 값을 반환하고 테이블의 열이 됩니다.

1. **Value**에서 원하는 각 행에서 동적 콘텐츠 목록이 표시되도록 편집 상자 내부를 클릭합니다.

1. 동적 콘텐츠 목록에서 **식**을 선택합니다.

1. 식 편집기에서 원하는 배열 속성 값을 지정하는 이 식을 입력하고 **확인을**선택합니다.
   ...
   * `item()?['Product_ID']`
   ...
   !["CSV 테이블 만들기"에 대한 "설명" 참조](./media/logic-apps-perform-data-operations/csv-table-expression.png)

1. 원하는 각 배열 속성에 대해 이전 단계를 반복합니다. 작업이 완료되면 작업은 다음과 같습니다.

   !["항목()" 함수 "CSV 테이블 만들기"](./media/logic-apps-perform-data-operations/finished-csv-expression.png)

1. 식을 보다 설명적인 버전으로 확인하려면 코드 보기로 전환하고 디자이너 보기로 다시 전환한 다음 축소된 작업을 다시 엽니다.

   **이제 CSV 테이블 만들기** 작업이 다음과 같습니다.
...
작업의 JSON 정의에서 `columns` 배열 내에서 `header` 속성을 빈 문자열로 설정합니다. 각 `value` 속성에 대해 원하는 각 배열 속성을 참조합니다.

1. 디자이너 도구 모음에서 **코드 보기를 선택합니다.**

1. 코드 편집기에서 작업의 `columns` 배열에서 원하는 배열 `header` 값의 `value` 각 열에 대해 빈 속성과 이 식을 추가합니다.
...
1. 디자이너 보기로 다시 전환하고 축소된 작업을 다시 엽니다.
   ...
   **이제 CSV 테이블 만들기** 작업이 이 예제와 같이 나타나고 식이 더 설명적인 버전으로 확인되었습니다.
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **CSV 테이블 만들기** 작업 아래에서 **출력**을 선택합니다. 
   ...
   이 예제에서는 Office 365 Outlook **전자 메일 보내기** 작업을 사용 하 고 전자 메일 본문에 **출력** 필드를 포함 합니다.
...
배열에서 JSON(JavaScript 개체 표기) 개체의 속성과 값이 있는 HTML 테이블을 만들려면 **HTML 테이블 만들기** 작업을 사용합니다. 그런 다음, **HTML 테이블 만들기** 작업 이후에 수행하는 작업에서 결과 테이블을 사용할 수 있습니다.
...
1. HTML 테이블을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다.
   ...
   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.
     ...
     !["HTML 테이블 만들기" 작업에 대해 "새 단계" 선택](./media/logic-apps-perform-data-operations/add-create-table-action.png)
...
1. **원본** 상자에서 테이블을 만들기 위해 원하는 배열 또는 식을 제공합니다.
   ...
   !["HTML 테이블 만들기"에 대한 완성된 예제](./media/logic-apps-perform-data-operations/finished-create-html-table-action.png)
...
배열에서 값을 반환하려면 **HTML 테이블 만들기** 작업과 함께 [ `item()` 함수를](../logic-apps/workflow-definition-language-functions-reference.md#item) 사용할 수 있습니다. `For_each` 루프에서 이 [ `items()` 함수를](../logic-apps/workflow-definition-language-functions-reference.md#items)사용할 수 있습니다.
...
작업에서 **헤더** 열을 비워 두십시오. **Value** 열의 각 행에서 원하는 각 배열 속성을 참조합니다. **Value** 아래의 각 행은 지정된 속성에 대한 모든 값을 반환하고 테이블의 열이 됩니다.
...
1. 식 편집기에서 원하는 배열 속성 값을 지정하는 이 식을 입력하고 **확인을**선택합니다.
   ...
   * `item()?['Product_ID']`
   ...
   !["HTML 테이블 만들기" 작업의 속성 참조 해제](./media/logic-apps-perform-data-operations/html-table-expression.png)
...
1. 디자이너 보기로 다시 전환하고 축소된 작업을 다시 엽니다.
   ...
   이제 **HTML 테이블 만들기** 작업이 이 예제와 같이 나타나고 식이 더 설명적인 버전으로 확인되었습니다.
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **HTML 테이블 만들기** 작업 아래에서 **출력**을 선택합니다. 
   ...
   !["HTML 테이블 만들기" "출력" 필드](./media/logic-apps-perform-data-operations/send-email-create-html-table-action.png)
...
1. 이제 논리 앱을 수동으로 실행합니다. 디자이너 도구 모음에서 **실행을**선택합니다.
   ...
   !["HTML 테이블 만들기" 결과가 있는 전자 메일](./media/logic-apps-perform-data-operations/create-html-table-email-results.png)

<a name="filter-array-action"></a>

## <a name="filter-array-action"></a>배열 필터링 작업

기존 배열에서 특정 조건을 충족하는 항목이 있는 더 작은 배열을 만들려면 **Filter 배열** 작업을 사용합니다. 그런 다음, **배열 필터링** 작업 이후에 수행하는 작업에서 필터링된 배열을 사용할 수 있습니다.
...
> 
> HTTP 끝점을 호출하고 JSON 응답을 받는 경우 **JSON** 응답을 사용하여 JSON 응답을 처리합니다. 
> 그렇지 않으면 **필터 배열** 동작은 JSON 페이로드의 구조가 아닌 응답 본문만 읽을 수 있습니다.
...
1. [Azure Portal](https://portal.azure.com) 또는 Visual Studio의 논리 앱 디자이너에서 논리 앱을 엽니다.
   ...
   !["필터 배열" 작업에 대 한 샘플 논리 응용 프로그램 시작](./media/logic-apps-perform-data-operations/sample-starting-logic-app-filter-array-action.png)

1. 필터링된 배열을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다. 

   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.

     !["필터 배열" 작업에 대해 "새 단계" 선택](./media/logic-apps-perform-data-operations/add-filter-array-action.png)
...
1. 조건에 대해 비교할 배열 항목을 지정하고, 비교 연산자를 선택하고, 비교 값을 지정합니다.
   ...
   이 예제에서는 `item()` 배열의 각 항목에 액세스하는 함수를 사용하고 **Filter 배열** 작업은 값이 하나보다 큰 배열 항목을 검색합니다.

   !["필터 배열" 작업에 대한 완성된 예제](./media/logic-apps-perform-data-operations/finished-filter-array-action.png)
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **표현식**을 선택합니다. **배열 필터링** 작업에서 배열 출력을 가져오려면 **배열 필터링** 작업 이름이 포함된 다음 식을 입력합니다.

   `@actionBody('Filter_array')`

   이 예제에서는 Office 365 Outlook **전자 메일 보내기** 작업을 사용 하 고 전자 메일 본문에 **actionBody ('Filter_array')** 표현식의 출력을 포함 합니다.

   !["필터 배열" 동작에서 작업 출력](./media/logic-apps-perform-data-operations/send-email-filter-array-action.png)
...
배열의 모든 항목이 있는 문자열을 만들고 해당 항목을 특정 구분 기호 문자로 구분하려면 **Join** 작업을 사용합니다. 그런 다음, **조인** 작업 이후에 수행하는 작업에서 문자열을 사용할 수 있습니다.
...
1. 배열에서 문자열을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다.
   ...
   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.
     ...
     !["조인" 작업에 대한 S선택 "새 단계"](./media/logic-apps-perform-data-operations/new-step-add-join-action.png)

   * 단계 간에 작업을 추가하려면 마우스를 연결 화살표 위로 이동하여 더하기 기호()가**+** 나타납니다. 더하기 기호를 선택한 다음 **작업 추가를**선택합니다.

1. 검색 상자에서 필터로 `join`을 입력합니다. 작업 목록에서 **조인** 작업을 선택합니다.

   !['가입' 작업 선택](./media/logic-apps-perform-data-operations/select-join-operation-action.png)
...
JSON(JavaScript 개체 표기형) 콘텐츠의 속성을 참조하거나 액세스하려면 **사용자** 친화적인 필드 또는 토큰을 만들 수 있습니다. 이렇게 하면 논리 앱에 대한 입력을 지정할 때 동적 콘텐츠 목록에서 해당 속성을 선택할 수 있습니다. 이 작업의 경우 JSON 콘텐츠 또는 페이로드 샘플에서 JSON 스키마를 제공하거나 생성할 수 있습니다.
...
1. [Azure Portal](https://portal.azure.com) 또는 Visual Studio의 논리 앱 디자이너에서 논리 앱을 엽니다.
   ...
   !["JSON 분석" 작업에 대한 샘플 논리 앱 시작](./media/logic-apps-perform-data-operations/sample-starting-logic-app-parse-json-action.png)

1. JSON 콘텐츠를 구문 분석하려는 논리 앱에서 다음 단계 중 하나를 수행합니다.

   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.

     !["JSON 분석" 작업에 대해 "새 단계" 선택](./media/logic-apps-perform-data-operations/add-parse-json-action.png)

   * 단계 간에 작업을 추가하려면 마우스를 연결 화살표 위로 이동하여 더하기 기호()가**+** 나타납니다. 더하기 기호를 선택한 다음 **작업 추가를**선택합니다.

1. 검색 상자에서 필터로 `parse json`을 입력합니다. 작업 목록에서 **JSON 분석** 작업을 선택합니다.
...
1. 구문 분석하려는 JSON 콘텐츠를 설명하는 JSON 스키마를 입력합니다.
   ...
   1. **샘플 JSON 페이로드를 입력하거나 붙여넣기**에서 JSON 콘텐츠를 제공한 다음 **완료를 선택합니다.**
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **JSON 구문 분석** 작업 아래에서 구문 분석된 JSON 콘텐츠로부터 속성을 선택할 수 있습니다.
   ...
   이 예제에서는 Office 365 Outlook **전자 메일 보내기** 작업을 사용 하 고 **이름,** **이름**및 **전자 메일** 필드를 전자 메일 본문에 포함 합니다.

   !["이메일 보내기" 작업의 JSON 속성](./media/logic-apps-perform-data-operations/send-email-parse-json-action.png)

   완료된 이메일 작업은 다음과 같습니다.

   ![이메일 작업에 대한 완성된 예제](./media/logic-apps-perform-data-operations/send-email-parse-json-action-2.png)

1. 이제 논리 앱을 수동으로 실행합니다. 디자이너 도구 모음에서 **실행을**선택합니다. 

   사용한 이메일 커넥터에 따라 다음과 같은 결과가 표시됩니다.

   !["JSON 분석" 작업 결과가 있는 이메일](./media/logic-apps-perform-data-operations/parse-json-email-results.png)

<a name="select-action"></a>

## <a name="select-action"></a>작업 선택

기존 배열의 값으로 만들어진 JSON 개체가 있는 배열을 만들려면 **Select** 작업을 사용합니다. 예를 들어 각 JSON 개체에 필요한 속성과 원본 배열의 값을 이러한 속성에 매핑하는 방법을 지정하여 정수 배열의 각 값에 대한 JSON 개체를 만들 수 있습니다. 그리고 이러한 JSON 개체의 구성 요소는 변경할 수 있지만, 출력 배열에는 항상 원본 배열과 동일한 수의 항목이 있습니다.
...
1. 배열을 만들려는 논리 앱에서 다음 단계 중 하나를 수행합니다. 
   ...
   * 마지막 단계에서 작업을 추가하려면 **새 단계를**선택합니다.
     ...
     !["선택" 작업에 대해 "새 단계"를 선택합니다.](./media/logic-apps-perform-data-operations/add-select-operation-action.png)

   * 단계 간에 작업을 추가하려면 마우스를 연결 화살표 위로 이동하여 더하기 기호()가**+** 나타납니다. 더하기 기호를 선택한 다음 **작업 추가를**선택합니다.

1. **작업 선택** 아래에서 **기본 제공**을 선택합니다. 검색 상자에서 필터로 `select`을 입력합니다. 작업 목록에서 **작업 선택을** 선택합니다.
...
1. **맵** 상자의 왼쪽 열에서 원본 배열의 각 값을 할당하려는 속성 이름을 제공합니다. 오른쪽 열에서 속성을 할당하려는 값을 나타내는 식을 지정합니다.
   ...
   이 예제에서는 "Product_ID"을 속성 이름으로 지정하여 각 배열 항목에 액세스하는 `item()` 식의 함수를 사용하여 정수 배열의 각 값을 할당합니다. 

   ![배열을 만드는 JSON 개체 속성 및 값 지정](./media/logic-apps-perform-data-operations/configure-select-action-2.png)

   완료된 작업은 다음과 같습니다.

   !["선택" 작업에 대한 완성된 예제](./media/logic-apps-perform-data-operations/finished-select-action.png)
...
1. 해당 작업에서 결과를 표시하려는 아무 위치를 클릭합니다. 동적 콘텐츠 목록이 열리면 **표현식**을 선택합니다. **선택** 작업에서 배열 출력을 가져오려면 **선택** 작업 이름이 포함된 다음 식을 입력합니다.

   `@actionBody('Select')`

   이 예제에서는 Office 365 Outlook **전자 메일 보내기** 작업을 `@actionBody('Select')` 사용 하 고 전자 메일 본문에 식의 출력을 포함 합니다.

   !["선택" 작업에서 작업 출력](./media/logic-apps-perform-data-operations/send-email-select-action.png)
