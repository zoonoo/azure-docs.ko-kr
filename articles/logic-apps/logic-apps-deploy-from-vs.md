---
title: "Visual Studio에서 Azure Logic Apps 빌드 | Microsoft Docs"
...
[Azure Portal](https://portal.azure.com/)에서 Logic Apps을 디자인하고 관리하는 훌륭한 방법을 제공하지만 Visual Studio에서도 논리 앱을 디자인하고 배포할 수 있습니다.  Logic Apps은 풍부한 Visual Studio 도구 집합과 함께 제공되어 디자이너를 사용하여 논리 앱을 빌드하고, 모든 배포 및 자동화 템플릿을 구성하며, 모든 환경에 배포할 수 있습니다.  
...
설치가 완료되면 Azure 리소스 그룹 프로젝트를 논리 앱 디자이너와 함께 사용할 수 있습니다.

## <a name="create-a-project"></a>프로젝트 만들기
1. **파일** 메뉴로 이동하여 **새로 만들기** >  **프로젝트**를 선택합니다(또는 **추가**로 이동한 다음 **새 프로젝트**를 선택하여 기존 솔루션에 추가 가능).  ![파일 메뉴](./media/logic-apps-deploy-from-vs/filemenu.png)
...
3. **논리 앱** 템플릿을 선택합니다. 이렇게 하면 빈 논리 앱 배포 템플릿을 생성하여 시작됩니다.
    ![Azure 템플릿 선택](./media/logic-apps-deploy-from-vs/selectazuretemplate1.png)
4. **템플릿**을 선택했으면 **확인**을 누릅니다.
   
    이제 논리 앱 프로젝트가 솔루션에 추가됩니다. 솔루션 탐색기에 배포 파일이 표시되어야 합니다.  
   
    ![배포](./media/logic-apps-deploy-from-vs/deployment.png)
...
2. 구독, 리소스 그룹 및 배포 템플릿의 위치를 선택합니다.
   * 논리 앱을 디자인하면 디자인하는 동안 속성을 쿼리하는 **API 연결** 리소스가 만들어집니다.  선택한 리소스 그룹은 디자인 시간 동안 이러한 연결을 만드는 데 사용됩니다.  Azure Portal로 이동한 후 **API 연결**을 검색하여 모든 API 연결을 보거나 수정할 수 있습니다.
...
4. 이제 논리 앱을 만들어 디자인할 수 있으며 변경 내용은 배포 템플릿에서 업데이트됩니다.
    ![Visual Studio의 디자이너](./media/logic-apps-deploy-from-vs/designer_in_vs.png)

논리 앱이 작동하는 데 필요한 모든 연결을 위해 `Microsoft.Web/connections` 리소스가 리소스 파일에 추가됩니다.  이러한 연결 속성은 배포할 때 설정할 수 있으며 배포한 후 Azure Portal의 **API 연결** 에서 관리할 수 있습니다.
...
1. 솔루션 탐색기에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **배포** > **새 배포...**
    ![새 배포](./media/logic-apps-deploy-from-vs/newdeployment.png)로 이동합니다.
2. Azure 구독에 로그인하라는 메시지가 표시됩니다. 
3. 이제 논리 앱을 배포하려는 리소스 그룹의 세부 정보를 선택해야 합니다. 
    ![리소스 그룹에 배포](./media/logic-apps-deploy-from-vs/deploytoresourcegroup.png)
...
5. 배포의 상태가 **출력** 창(**Azure 프로비전**을 선택해야 할 수 있음)에 표시됩니다. 
    ![출력](./media/logic-apps-deploy-from-vs/output.png)

나중에 소스 제어에서 논리 앱을 수정하고 새 버전을 배포하기 위해 Visual Studio를 사용할 수 있습니다. 

> [!NOTE]
> Azure Portal에서 정의를 직접 수정한 경우 다음에 Visual Studio에서 변경 내용을 배포하면 해당 변경 내용이 덮어쓰여집니다.
> 
> 

## <a name="adding-a-logic-app-to-an-existing-resource-group-project"></a>논리 앱을 기존 리소스 그룹 프로젝트에 추가
기존 리소스 그룹 프로젝트를 사용하는 경우 JSON 개요 창을 통해 기존 프로젝트에 논리 앱을 추가하거나 이전에 만든 논리 앱과 함께 또 다른 논리 앱을 추가할 수 있습니다.
1. `<template>.json` 파일을 엽니다.
2. JSON 개요 창을 엽니다.  JSON 개요 창은 **보기** > **다른 창** > **JSON 개요**에서 찾을 수 있습니다.
3. JSON 개요 창의 맨 위에 있는 리소스 추가 단추를 클릭하거나 **리소스**를 마우스 오른쪽 단추로 클릭하고 **새 리소스 추가**를 선택하여 템플릿 파일에 리소스를 추가할 수 있습니다.

    ![Json 개요](./media/logic-apps-deploy-from-vs/jsonoutline.png)
    
4. **리소스 추가** 대화 상자에서 **논리 앱**을 찾아 선택한 후 이름을 지정하고 **추가**를 선택합니다.
