...
| `staticResults` | 예 | 이러한 작업에 정적 결과가 사용 되는 경우 작업에서 모의 출력으로 반환 하는 하나 이상의 정적 결과에 대 한 정의입니다. 각 작업 정의에서 특성은 `runtimeConfiguration.staticResult.name` 내부에서 해당 하는 정의를 참조 합니다 `staticResults` . 자세한 내용은 [정적 결과](#static-results)를 참조 하세요. |
...
| <*허용 되는 매개 변수-값 배열*> | 예 | 배열 | 매개 변수를 수용할 수 있는 값으로 배열 |
