...
1. 논리 앱 메뉴의 **설정**에서 **ID**를 선택합니다. **저장**시 **시스템 할당 됨** > **을** > 선택 합니다. Azure에서 확인 메시지가 표시 되 면 **예**를 선택 합니다.
   ...
   | 속성 | 값 | Description |
   |----------|-------|-------------|
   | **개체 ID** | <*id-리소스 ID*> | Azure AD 테 넌 트에서 논리 앱의 시스템 할당 id를 나타내는 GUID (Globally Unique Identifier)입니다. |
   ||||
...
논리 앱과 같은 Azure 리소스 만들기 및 배포를 자동화 하기 위해 [Azure Resource Manager 템플릿을](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md)사용할 수 있습니다. 템플릿에서 논리 앱에 대 한 시스템 할당 관리 id를 사용 하도록 설정 하려면 템플릿에서 `identity` 개체 및 `type` 자식 속성을 논리 앱의 리소스 정의에 추가 합니다. 예를 들면 다음과 같습니다.
...
Azure에서 논리 앱 리소스 정의를 만들 때 개체 `identity` 는 다음과 같은 추가 속성을 가져옵니다.
...
|-----------------|-------|-------------|
| `principalId` | <*보안 주체 ID*> | Azure AD 테 넌 트에서 논리 앱을 나타내는 관리 되는 id에 대 한 서비스 주체 개체의 GUID (Globally Unique Identifier)입니다. 이 GUID는 때때로 "개체 ID" 또는 `objectID`로 표시 됩니다. |
| `tenantId` | <*Azure-AD-테 넌 트-ID*> | 논리 앱이 현재 멤버로 속해 있는 Azure AD 테 넌 트를 나타내는 GUID (Globally Unique Identifier)입니다. Azure AD 테넌트 내부에서 서비스 주체는 논리 앱 인스턴스와 이름이 같습니다. |
||||
...
1. [Azure Portal](https://portal.azure.com)의 검색 상자에 모든 페이지에서를 입력 `managed identities`하 고 **관리 되는 id**를 선택 합니다.
...
1. 관리 id에 대 한 정보를 제공 하 고 **만들기**를 선택 합니다. 예를 들면 다음과 같습니다.
   ...
   | 속성 | 필수 | 값 | Description |
   |----------|----------|-------|-------------|
   | **리소스 이름** | 예 | <*사용자 할당 id-이름*> | 사용자에 게 할당 된 id를 제공 하는 이름입니다. 이 예에서는 "Fabrikam-사용자 할당-id"를 사용 합니다. |
   | **구독** | 예 | <*Azure-구독-이름*> | 사용할 Azure 구독의 이름입니다. |
   | **리소스 그룹** | 예 | <*Azure-리소스 그룹-이름*> | 사용할 리소스 그룹의 이름입니다. 새 그룹을 만들거나 기존 그룹을 선택 합니다. 이 예제에서는 이름이 "fabrikam-관리-RG" 인 새 그룹을 만듭니다. |
   | **위치** | 예 | <*Azure-지역*> | 리소스에 대 한 정보를 저장할 Azure 지역입니다. 이 예제에서는 “미국 서부”를 사용합니다. |
   |||||
...
* `type` 속성이로 설정 된 `identity` 개체입니다.`UserAssigned`

* 및 속성이 있는 다른 자식 개체인 id의 리소스 ID를 지정 하는 자식 `userAssignedIdentities` 개체입니다. `clientId` `principalId`

이 예제에서는 HTTP PUT 요청에 대 한 논리 앱 리소스 정의를 보여 주고 매개 변수가 `identity` 없는 개체를 포함 합니다. PUT 요청 및 후속 GET 작업에 대 한 응답에도이 `identity` 개체가 있습니다.
...
| Property(JSON) | 값 | Description |
|-----------------|-------|-------------|
| `principalId` | <*보안 주체 ID*> | Azure AD 테 넌 트에서 사용자 할당 관리 id의 GUID (Globally Unique Identifier)입니다. |
| `clientId` | <*클라이언트 ID*> | 런타임에 호출에 사용 되는 논리 앱의 새 id에 대 한 GUID (Globally Unique Identifier)입니다. |
||||

템플릿에 관리 id의 리소스 정의도 포함 되어 있으면 개체를 `identity` 매개 변수화 할 수 있습니다. 이 예제에서는 자식 `userAssignedIdentities` 개체가 템플릿의 `userAssignedIdentity` `variables` 섹션에서 정의한 변수를 참조 하는 방법을 보여 줍니다. 이 변수는 사용자 할당 id에 대 한 리소스 ID를 참조 합니다.
...
| Property(JSON) | 값 | Description |
|-----------------|-------|-------------|
| `tenantId` | <*Azure-AD-테 넌 트-ID*> | 사용자가 할당 한 id가 현재 멤버로 속해 있는 Azure AD 테 넌 트를 나타내는 GUID (Globally Unique Identifier)입니다. Azure AD 테 넌 트 내에서 서비스 주체는 사용자 할당 id 이름과 동일한 이름을 갖습니다. |
| `principalId` | <*보안 주체 ID*> | Azure AD 테 넌 트에서 사용자 할당 관리 id의 GUID (Globally Unique Identifier)입니다. |
| `clientId` | <*클라이언트 ID*> | 런타임에 호출에 사용 되는 논리 앱의 새 id에 대 한 GUID (Globally Unique Identifier)입니다. |
||||
...
1. 리소스 메뉴에서 **액세스 제어 (IAM)** > **역할 할당** 을 선택 하 여 해당 리소스에 대 한 현재 역할 할당을 검토할 수 있습니다. 도구 모음에서 **추가** > **역할 할당**추가를 선택 합니다.
...
1. 아직 수행 하지 않은 경우 [관리 되는 id를 지 원하는 트리거 또는 작업](logic-apps-securing-a-logic-app.md#managed-identity-authentication)을 추가 합니다.
   ...
   | 속성 | 필수 | Description |
   |----------|----------|-------------|
   | **방법이** | 예 | 실행 하려는 작업에서 사용 하는 HTTP 메서드입니다. |
   ...
   | **헤더** | 아니요 | 필요한 모든 헤더 값 (예: 콘텐츠 형식)을 보내는 요청에 포함 하려고 합니다. |
   | **쿼리** | 아니요 | 특정 작업에 대 한 매개 변수 또는 실행 하려는 작업의 API 버전 등 요청에 포함 하려는 모든 쿼리 매개 변수 |
   ...
   |----------|----------|---------------|-------------|
   | **방법이** | 예 | `PUT`| 스냅숏 Blob 작업에서 사용 하는 HTTP 메서드입니다. |
   | **URI** | 예 | `https://{storage-account-name}.blob.core.windows.net/{blob-container-name}/{folder-name-if-any}/{blob-file-name-with-extension}` | 이 구문을 사용 하는 Azure 전역 (공용) 환경의 Azure Blob Storage 파일에 대 한 리소스 ID입니다. |
   | **헤더** | 예, Azure Storage | `x-ms-blob-type` = `BlockBlob` <p>`x-ms-version` = `2019-02-02` | Azure Storage `x-ms-blob-type` 작업 `x-ms-version` 에 필요한 및 헤더 값입니다. <p><p>**중요**: Azure Storage에 대 한 나가는 HTTP 트리거와 작업 요청에서 헤더에는 실행 `x-ms-version` 하려는 작업의 속성 및 API 버전이 필요 합니다. <p>자세한 내용은 다음 항목을 참조하세요. <p><p>- [요청 헤더-스냅숏 Blob](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob#request) <br>- [Azure Storage services에 대 한 버전 관리](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#specifying-service-versions-in-requests) |
...
1. 일부 트리거 및 동작에서 대상 리소스 ID를 설정 하는 데에도 **대상 속성이 표시** 됩니다. 대상 **그룹** 속성을 [대상 리소스 또는 서비스에 대 한 리소스 ID](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication)로 설정 합니다. 그렇지 않은 경우 기본적으로 **대상** 속성은 Azure Resource Manager에 `https://management.azure.com/` 대 한 리소스 id를 사용 하 여 리소스 id를 사용 합니다.
   ...
   이 예에서는 **대상** 속성을로 `https://storage.azure.com/` 설정 하 여 인증에 사용 되는 액세스 토큰이 모든 저장소 계정에 대해 유효한 지 확인 합니다. 그러나 특정 저장소 계정에 대 한 루트 서비스 URL `https://fabrikamstorageaccount.blob.core.windows.net`을 지정할 수도 있습니다.
...
1. 논리 앱 메뉴의 **설정**에서 **id**를 선택 하 고 id에 대 한 단계를 따릅니다.
   ...
   * **저장**시 **시스템 할당 됨** > **을** > 선택 합니다. Azure에서 확인 메시지가 표시 되 면 **예**를 선택 합니다.
...
Azure Resource Manager 템플릿을 사용 하 여 논리 앱의 관리 되는 id를 만든 경우 `identity` 개체의 `type` 자식 속성을로 `None`설정 합니다. 시스템 관리 id의 경우이 작업은 또한 Azure AD에서 보안 주체 ID를 삭제 합니다.
