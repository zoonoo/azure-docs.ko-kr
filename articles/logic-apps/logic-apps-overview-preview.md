...
* SaaS (Software as a Service) 및 PaaS (Platform as a Service) 앱 및 서비스에 대해 [400 개 이상의 커넥터](/connectors/connector-reference/connector-reference-logicapps-connectors) 에서 논리 앱 및 해당 워크플로를 만들고 온-프레미스 시스템을 위한 커넥터를 만듭니다.
  ...
  * 이제 일부 관리 되는 커넥터를 기본 제공 버전으로 사용할 수 있습니다 .이 버전은 Azure Logic Apps Preview 런타임에 기본적으로 실행 되는 요청 트리거 및 HTTP 작업과 같은 기본 제공 트리거와 작업과 유사 하 게 실행 됩니다. 예를 들어 이러한 새 기본 제공 커넥터에는 Azure Service Bus, Azure Event Hubs, SQL Server 및 MQ가 포함 됩니다.

    > [!NOTE]
    > 기본 제공 SQL Server 커넥터의 경우 **쿼리 실행** 작업만 [온-프레미스 데이터 게이트웨이](logic-apps-gateway-connection.md)를 요구 하지 않고 Azure 가상 네트워크에 직접 연결할 수 있습니다.
