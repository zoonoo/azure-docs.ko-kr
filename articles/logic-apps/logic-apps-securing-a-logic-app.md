...
1. **트리거의 ip 범위** 상자가 표시 되 면 트리거에서 허용 하는 ip 주소 범위를 지정 합니다. 올바른 IP 범위는 *x.x.x.x/x* 또는 *x.x.x.x-x.x.x.x* 형식을 사용합니다.
   ...
   예를 들어 논리 앱을 HTTP 작업을 통해 중첩 된 논리 앱 으로만 호출할 수 있도록 하려면 **특정 IP 범위** 옵션 ( **다른 Logic Apps** 옵션은 아님)을 사용 하 고 부모 논리 앱의 [아웃 바운드 IP 주소](../logic-apps/logic-apps-limits-and-config.md#outbound)를 입력 합니다.

   그러나 기본 제공 [Azure Logic Apps 작업](../logic-apps/logic-apps-http-endpoint.md)을 통해 논리 앱을 중첩 된 논리 앱 으로만 호출할 수 있도록 하려면 **다른 Logic Apps** 옵션만 선택 합니다. 이 옵션은 논리 앱 리소스에 빈 배열을 작성 하며, 다른 "부모" 논리 앱의 호출만 기본 제공 **Azure Logic Apps** 작업을 통해 중첩 된 논리 앱을 트리거할 수 있어야 합니다.
