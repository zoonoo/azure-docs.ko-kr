...
1. 정적 결과를 설정 하려는 작업에서 다음 단계를 수행 합니다. 
   ...
   1. **정적 결과 사용**을 선택 합니다. 필수 (*) 속성의 경우 작업의 응답에 대해 반환 하려는 모의 출력 값을 지정 합니다.
      ...
      JavaScript Object Notation (JSON) 형식으로 모의 데이터를 입력 하려면 **Json 모드로 전환** (!["Json 모드로 전환"](./media/test-logic-apps-mock-data-static-results/switch-to-json-mode-button.png))을 선택 합니다.
...
1. 저장할 준비가 되 면 **완료**를 선택 합니다.
   ...
   작업의 오른쪽 위 모서리에서 제목 표시줄에는 테스트 반쯤 아이콘 (정적 결과](./media/test-logic-apps-mock-data-static-results/static-results-test-beaker-icon.png)에 대 한![아이콘)이 표시 되며,이는 정적 결과를 사용 하도록 설정 했음을 나타냅니다.
...
1. **정적 결과** 상자가 열리면 다음 단계 중 하나를 선택 합니다.
   ...
   * 전체 JSON 개체를 붙여넣으려면 **Json 모드로 전환** (!["Json 모드로 전환"](./media/test-logic-apps-mock-data-static-results/switch-to-json-mode-button.png))을 선택 합니다.
...
1. 모두 마쳤으면 **완료**를 선택합니다. 또는 디자이너로 돌아가려면 **편집기 모드 전환** (!["편집기 모드 전환"](./media/test-logic-apps-mock-data-static-results/switch-editor-mode-button.png))을 선택 합니다.
