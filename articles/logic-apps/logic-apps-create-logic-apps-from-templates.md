---
title: 미리 빌드된 템플릿을 사용하여 논리 앱 워크플로를 더 빠르게 생성
description: Azure Logic Apps에서 제공하는 미리 빌드된 템플릿을 사용하여 논리 앱 워크플로를 신속하게 빌드합니다.
services: logic-apps
...
1. 아직 로그인하지 않은 경우 Azure [포털](https://portal.azure.com "Azure portal")에 로그인합니다.

2. 기본 Azure 메뉴에서 **리소스** > **엔터프라이즈 통합** > **논리 앱**만들기를 선택합니다.
...
3. 이 이미지의 테이블에서 설정을 사용하여 논리 앱을 만듭니다.
   ...
   | 설정 | 값 | 설명 | 
   | ------- | ----- | ----------- | 
   | **이름** | *당신의 논리 - 응용 프로그램 이름* | 고유한 논리 앱 이름을 제공합니다. | 
   ...
   | **Log Analytics** | **끄기**(기본값) 또는 **켜기** | Azure Monitor 로그를 사용하여 논리 앱에 대한 [진단 로깅을](../logic-apps/monitor-logic-apps-log-analytics.md) [설정합니다.](../log-analytics/log-analytics-overview.md) Log Analytics 작업 영역이 이미 있어야 합니다. | 
   |||| 
...
5. **템플릿**에 대한 소개 비디오 및 공통 트리거는 넘어갑니다. 미리 빌드된 템플릿을 선택합니다. 예를 들어:
...
1. Azure [포털에서](https://portal.azure.com "Azure portal")로직 앱 디자이너에서 논리 앱을 찾아 엽니다.

2. 디자이너 도구 모음에서 **템플릿**을 선택합니다. 이 작업은 저장되지 않은 변경 내용을 삭제하므로 계속하려는지 확인할 수 있도록 경고 메시지가 표시됩니다. 확인하려면 **확인**을 선택합니다. 예를 들어:

   !["템플릿" 선택](./media/logic-apps-create-logic-apps-from-templates/logic-app-update-existing-with-template.png)

3. **템플릿**에 대한 소개 비디오 및 공통 트리거는 넘어갑니다. 미리 빌드된 템플릿을 선택합니다. 예를 들어:
