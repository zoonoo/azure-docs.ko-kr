---
title: 코드 조각 추가 및 실행-Azure Logic Apps
...
* 트리거를 포함 하 여 코드 조각을 추가 하려는 논리 앱입니다. 논리 앱이 없는 경우 빠른 시작을 참조 [하세요. 첫 번째 논리 앱 만들기](../logic-apps/quickstart-create-first-logic-app-workflow.md)를 검토하세요.

   이 항목의 예제 논리 앱에서는 다음 Office 365 Outlook 트리거를 사용 합니다. **새 전자 메일이 도착했을 때**
...
1. **작업 선택**아래의 검색 상자에 "인라인 코드"를 필터로 입력 합니다. 작업 목록에서 다음 작업을 선택합니다. **JavaScript 코드 실행**
...
1. **코드** 상자에서 샘플 코드를 삭제 하 고 실행 하려는 코드를 입력 합니다. 메서드 시그니처를 정의 하지 않고 메서드 내에 배치 하는 코드를 작성 합니다. 
   ...
   **본문** 토큰을 선택 하면 인라인 코드 작업에서 토큰 `workflowContext` 을 전자 메일의 `Body` 속성 값을 참조 하는 개체로 확인 합니다.

   ![결과 선택](./media/logic-apps-add-run-inline-code/inline-code-example-select-outputs.png)

   코드 상자에서 **코드** 조각이 읽기 `workflowContext` 전용 개체를 입력으로 사용할 수 있습니다. 이 개체에는 워크플로의 트리거와 이전 작업의 결과에 대 한 코드 액세스를 제공 하는 하위 속성이 있습니다.
   자세한 내용은이 항목의 뒷부분에 나오는 다음 섹션을 참조 하십시오. [코드에 대 한 참조 트리거 및 작업 결과](#workflowcontext)입니다.
...
개체 `workflowContext` `actions`는, `trigger`및 하위속성을포함하는이구조체를가집니다.`workflow`
...
이 항목의 예제 `workflowContext` 에서 개체에는 코드에서 액세스할 수 있는 다음과 같은 속성이 있습니다.
...
1. 디자이너 도구 모음에서 **코드 보기**를 선택 하 고 작업 이름에 `actions` 대 한 특성 내에서 검색 합니다.
   ...
   예 `Send_approval_email_` 를 들어는 **승인 전자 메일 보내기** 작업의 JSON 이름입니다.
