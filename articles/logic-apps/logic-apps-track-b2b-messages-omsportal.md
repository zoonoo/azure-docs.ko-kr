...
* 통합 계정. [통합 계정](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md)을 만들고 로깅을 사용하도록 설정할 수 있습니다. [여기](logic-apps-monitor-b2b-message.md)에서 단계를 찾을 수 있습니다.
* 논리 앱. [논리 앱](../logic-apps/logic-apps-create-a-logic-app.md)을 만들고 로깅을 사용하도록 설정할 수 있습니다. [여기](../logic-apps/logic-apps-monitor-your-logic-apps.md#azure-diagnostics-and-alerts)에서 단계를 찾을 수 있습니다.

## <a name="adding-logic-apps-b2b-solution-to-oms-portal"></a>OMS 포털에 논리 앱 B2B 솔루션 추가

1. 포털에서 **추가 서비스**를 선택하고 **log analytics**를 검색한 다음 **Log Analytics**를 선택합니다 .  
...
3. **OMS 포털**을 선택하고 OMS 포털 홈 페이지를 엽니다.   
![OMS 포털 찾아보기](media/logic-apps-track-b2b-messages-omsportal/omsportalpage.png)
...
6. **추가**를 클릭하여 **Logic Apps B2B 메시지**를 홈 페이지에 추가합니다.  
![추가 선택](media/logic-apps-track-b2b-messages-omsportal/omshomepage3.png)

7. 홈 페이지로 이동하여 **Logic Apps B2B 메시지**를 확인합니다 .  
...
1. 메시지 프로세스를 게시합니다. 홈 페이지가 메시지 수로 업데이트됩니다.   
![홈 페이지를 선택합니다.](media/logic-apps-track-b2b-messages-omsportal/omshomepage6.png)

2. 홈 페이지에서 **Logic Apps B2B 메시지**를 선택하면 AS2 및 X12 메시지 상태를 보여 줍니다.  데이터는 지난&1;일을 기반으로 합니다.
![논리 앱 B2B 메시지 선택](media/logic-apps-track-b2b-messages-omsportal/omshomepage5.png)



3. 상태별로 AS2 또는 X12 메시지를 선택하면 메시지 목록으로 이동합니다.   
![AS2 메시지 상태 선택](media/logic-apps-track-b2b-messages-omsportal/as2messagelist.png)

| 속성 | 설명 |
| --- | --- |
| 보낸 사람 | 수신 설정에 구성된 게스트 파트너 또는 AS2 규약의 송신 설정에 구성된 호스트 파트너 |
| 받는 사람 | 수신 설정에 구성된 호스트 파트너 또는 AS2 규약의 송신 설정에 구성된 게스트 파트너 |
| 논리 앱 | AS2 작업이 구성된 Logic App |
| 가동 상태 | AS2 메시지 상태. 성공 = 양호한 AS2 메시지를 받거나 보내고, MDN을 구성하지 않았습니다. 성공 = 양호한 AS2 메시지를 받거나 보내고, MDN을 구성하고, MDN을 받거나 보냈습니다. 실패 = 잘못된 AS2 메시지를 받고 MDN을 구성하지 않았습니다. 보류 중 = 양호한 AS2 메시지와 MDN을 를 받거나 보냈습니다. 기능 Ack를 구성하고 기다리고 있습니다. |
| Ack | MDN 메시지 상태 |
| 방향 | AS2 메시지 방향 |
...
![X12 메시지 상태 선택](media/logic-apps-track-b2b-messages-omsportal/x12messagelist.png)
...
| 받는 사람 | 수신 설정에 구성된 호스트 파트너 또는 AS2 규약의 송신 설정에 구성된 게스트 파트너 |
| 논리 앱 | AS2 작업이 구성된 Logic App |
| 가동 상태 | X12 메시지 상태 성공 = 양호한 X12 메시지를 받거나 보내고, 기능 Ack를 구성하지 않았습니다. 성공 = 양호한 X12 메시지를 받거나 보내고, 기능 Ack를 구성하고, 기능 Ack를 받거나 보냈습니다. 실패 = 잘못된 X12 메시지를 받거나 보냈습니다. 보류 중 = 양호한 X12 메시지를 받거나 보내고, 기능 Ack를 구성하고, 기능 Ack를 기다리고 있습니다. |
| Ack | 기능 Ack(997) 상태.  수락됨 = 긍정적인 기능 Ack를 받거나 보냈습니다. 거부됨 = 부정적인 기능 Ack를 받거나 보냈습니다. 보류 중 = 기능 Ack를 기다렸지만 받지 못했습니다. 보류 중 = 기능 Ack를 생성했지만 파트너에게 보낼 수 없습니다. |
| 방향 | X12 메시지 방향 |
| 상관관계 ID | Logic App 내의 모든 트리거와 작업을 상호 연결하는 ID |
...
4. AS2 또는 X12 메시지 목록에서 행을 선택하면 로그 검색으로 이동합니다.  로그 검색을 수행하면 동일한 **실행 ID**를 갖는 모든 작업이 나열됩니다. 
...
1. 로그 검색에서 쿼리를 작성하고 **저장**을 선택합니다.  [여기](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md) 는 쿼리를 단계입니다. ![홈 페이지 선택](media/logic-apps-track-b2b-messages-omsportal/logsearchaddquery.png)
...
* 로그 검색에서 **즐겨찾기**를 선택하여 저장된 쿼리를 봅니다.  제공된 쿼리 결과 중 하나 선택 ![홈 페이지 선택](media/logic-apps-track-b2b-messages-omsportal/logsearchaddquery5.png)
