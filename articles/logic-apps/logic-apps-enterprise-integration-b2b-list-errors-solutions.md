...
| 사용자 동작 | 동의한 비즈니스 ID로 통합 계정에 규약을 추가해야 합니다. </br>비즈니스 ID는 입력 메시지 ID와 일치해야 합니다. |  
|   |   |
...
| 사용자 동작 | 규약에 대해 구성된 AS2-From 또는 AS2-To가 잘못되었습니다. </br>AS2 메시지 헤더의 AS2 ID가 규약 구성과 일치하도록 AS2 메시지의 "AS2-From" 또는 "AS2-To" 헤더나 규약을 수정합니다. |
|   |   |     
...
| 사용자 동작 | AS2-From, AS2-To 또는 두 헤더가 포함되지 않은 AS2 메시지를 받았습니다. </br> AS2 메시지 AS2-From 및 AS2-To 헤더를 확인하고 규약 구성에 따라 수정합니다. |
|  |  | 
...
| 사용자 동작 | 메시지 본문이 포함되지 않은 AS2 메시지를 받았습니다. |
|  |  | 
...
| 사용자 동작 | 파트너에게 보내기 전에 AS2Message에 @base64ToBinary를 추가합니다. |
|||
...
|---|---|
| 오류 설명 |  [처리됨/오류: 암호 해독 실패] | 
| 사용자 동작 | 파트너에게 보내기 전에 MDN에 @base64ToBinary를 추가합니다. | 
|||
...
| 사용자 동작 | 서명을 위한 올바른 인증서로 AS2 규약 설정을 구성합니다. |
|  |  | 
...
| 사용자 동작 | 선행 공백 및 후행 공백을 허용하도록 구성할 규약 설정입니다. </br>선행 공백 및 후행 공백을 허용하도록 규약 설정을 편집합니다. |
|   |   |
...
| 사용자 동작 | 이 오류는 받은 메시지에 중복 컨트롤 번호가 있음을 나타냅니다. </br>컨트롤 번호를 수정하고 메시지를 다시 보냅니다. |
|   |   |
...
| 사용자 동작 | 이 오류는 입력 메시지에 본문이 포함되어 있지 않음을 나타냅니다. |
|   |   | 
