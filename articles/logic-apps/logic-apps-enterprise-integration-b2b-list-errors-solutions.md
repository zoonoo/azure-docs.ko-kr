---
title: B2B 시나리오의 일반적인 오류 및 문제에 대한 해결 방법
description: Azure Logic Apps에서 B2B 시나리오 문제를 해결할 때 일반적인 오류 및 문제에 대한 해결 방법 찾기
services: logic-apps
...
**사용자 작업**: 동의한 비즈니스 ID로 통합 계정에 규약을 추가해야 합니다. 비즈니스 ID는 입력 메시지 ID와 일치해야 합니다.
...
**사용자 작업**: 규약에 대해 구성된 AS2-From 또는 AS2-To가 잘못되었습니다. AS2 메시지 헤더의 AS2 ID가 규약 구성과 일치하도록 AS2 메시지의 "AS2-From" 또는 "AS2-To" 헤더나 규약을 수정합니다.
...
**사용자 작업**: AS2-From, AS2-To 또는 두 헤더가 포함되지 않은 AS2 메시지를 받았습니다. AS2 메시지 AS2-From 및 AS2-To 헤더를 확인하고 규약 구성에 따라 수정합니다.

### <a name="missing-as2-message-body-and-headers"></a>누락된 AS2 메시지 본문 및 헤더    

**오류 설명**: 요청 콘텐츠가 null이거나 비어 있습니다.

**사용자 작업**: 메시지 본문이 포함되지 않은 AS2 메시지를 받았습니다.

### <a name="as2-message-decryption-failure"></a>AS2 메시지 암호 해독 실패

**오류 설명**: [처리됨/오류: 암호 해독 실패]

**사용자 작업**: 파트너에게 보내기 전에 AS2Message에 @base64ToBinary를 추가합니다.
...
**사용자 작업**: 파트너에게 보내기 전에 MDN에 @base64ToBinary를 추가합니다.
...
**오류 설명**: AS2 파티에 대해 서명 인증서가 구성되지 않았습니다. AS2-From: partner1 AS2-To: partner2

**사용자 작업**: 서명을 위한 올바른 인증서로 AS2 규약 설정을 구성합니다.
...
**사용자 작업**: 선행 공백 및 후행 공백을 허용하도록 구성할 규약 설정입니다. 선행 공백 및 후행 공백을 허용하도록 규약 설정을 편집합니다.
...
**오류 설명**: 컨트롤 번호가 중복되었습니다.

**사용자 작업**: 이 오류는 받은 메시지에 중복 컨트롤 번호가 있음을 나타냅니다. 컨트롤 번호를 수정하고 메시지를 다시 보냅니다.
...
**사용자 작업**: 규약 설정에서 스키마를 구성합니다.

### <a name="incorrect-schema-in-the-agreement"></a>규약에 잘못된 스키마가 있음

**오류 설명**: 메시지에 알 수 없는 문서 유형이 있으며, 규약에 구성된 기존 스키마로 확인되지 않았습니다.

**사용자 작업**: 규약 설정에서 올바른 스키마를 구성합니다.
...
**사용자 작업**: 이 오류는 입력 메시지에 본문이 포함되어 있지 않음을 나타냅니다.
