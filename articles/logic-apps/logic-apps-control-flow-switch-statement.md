...
1. 이 예제에서는 샘플 워크플로 끝에 switch 문을 추가합니다. 마지막 단계를 수행한 후 **새 단계**를 선택합니다.
   ...
   단계 사이에 switch 문을 추가하려면 switch 문을 추가하려는 화살표 위로 포인터를 이동합니다. 표시 되는 **더하기 기호** (**+**)를 선택 하 고 **작업 추가**를 선택 합니다.
...
1. 해당 케이스에 다음 작업을 추가합니다.
   ...
   | 케이스 2 | **내용을** | 다른 승인자에게 RSS 항목이 거부되었음을 알리는 Outlook **이메일 보내기** 작업을 추가합니다. |
   | 기본값 | 없음 | 아무런 작업도 수행할 필요가 없습니다. 이 예에서는 **SelectedOption**에 두 가지 옵션만 있으므로 **기본** 케이스는 비어 있습니다. |
   |||
