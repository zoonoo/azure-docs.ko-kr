...
1. **설치됨** 아래에서 **Visual C#** 또는 **Visual Basic**을 선택합니다. 그런 다음 **클라우드** > **Azure 리소스 그룹**을 선택 합니다.
   ...
   1. **새 프로젝트 만들기** 상자에서 시각적 개체 C# 또는 Visual Basic에 대 한 **Azure 리소스 그룹** 프로젝트 템플릿을 선택 하 고 **다음**을 선택 합니다.
...
JavaScript, Python, F#, PowerShell, Batch 또는 Bash를 사용 하 여 함수 프로젝트 및 함수를 만들려면 [Azure Functions Core Tools 작업](../azure-functions/functions-run-local.md)의 단계를 따르세요. 솔루션 내에서를 사용 C# 하 여 Azure 함수를 개발 하려면 C# [.net 클래스 라이브러리를 함수 앱로 게시](https://blogs.msdn.microsoft.com/appserviceteam/2017/03/16/publishing-a-net-class-library-as-a-function-app/)의 단계를 수행 하 여 클래스 라이브러리를 사용 합니다.
