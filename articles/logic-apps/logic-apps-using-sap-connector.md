...
ms.locfileid: "99252613"
---
# <a name="connect-to-sap-systems-from-azure-logic-apps"></a>Azure Logic Apps에서 SAP 시스템에 연결

이 문서에서는 [sap 커넥터](https://docs.microsoft.com/connectors/sap/)를 사용 하 여 LOGIC APPS에서 sap 리소스에 액세스 하는 방법을 설명 합니다.
...
SAP 커넥터는 Logic Apps 트리거에 대 한 Azure의 [비동기 요청-회신 패턴](/azure/architecture/patterns/async-request-reply.md) 을 지원 합니다. 이 패턴을 사용 하면 기본 동기 요청-회신 패턴으로 인해 실패 한 성공적인 요청을 만들 수 있습니다. 
...
게이트웨이 앱의 설정에서의 .zip 파일로 [게이트웨이의 모든 구성 및 서비스 로그를 내보낼](https://docs.microsoft.com/data-integration/gateway/service-gateway-tshoot#collect-logs-from-the-on-premises-data-gateway-app) 수 있습니다.
...
필요에 따라 고급 사용자는 ETW 이벤트를 직접 캡처할 수 있습니다. 그런 다음 [Event Hubs에서 Azure 진단 데이터를 사용](https://docs.microsoft.com/azure/azure-monitor/platform/diagnostics-extension-stream-event-hubs) 하거나 데이터를 [수집 하 여 로그를 Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/platform/diagnostics-extension-logs)수 있습니다. 자세한 내용은 [데이터 수집 및 저장에 대 한 모범 사례](https://docs.microsoft.com/azure/architecture/best-practices/monitoring#collecting-and-storing-data)를 참조 하세요. [Perfview](https://github.com/Microsoft/perfview/blob/master/README.md) 를 사용 하 여 결과 ETL 파일 작업을 수행 하거나 프로그램을 직접 작성할 수 있습니다. 이 연습에서는 PerfView를 사용 합니다.
...
[Send idoc ( `SendIDoc` )](https://docs.microsoft.com/connectors/sap/#send-idoc-(preview)) 작업은 IDOC 메시지를 SAP 서버에 보냅니다.
