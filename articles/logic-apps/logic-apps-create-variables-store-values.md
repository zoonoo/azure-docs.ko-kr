...
이 문서에서는 논리 앱에 값을 저장 하는 데 사용 하는 변수를 만들고 작업 하는 방법을 보여 줍니다. 예를 들어 변수를 통해 루프가 실행 되는 횟수를 추적할 수 있습니다. 배열을 반복 하거나 특정 항목의 배열을 검사 하려면 변수를 사용 하 여 각 배열 항목의 인덱스 번호를 참조할 수 있습니다.
...
* 변수에 다른 값을 할당합니다.
* 변수의 값을 문자열 또는 배열의 마지막 항목으로 삽입 하거나 *추가* 합니다.

변수는 해당 변수를 만드는 논리 앱 인스턴스 내에서만 존재하고 전역입니다. 또한 논리 앱 인스턴스 내의 모든 루프 반복에서 지속됩니다. 변수를 참조 하는 경우 작업의 출력을 참조 하는 일반적인 방법인 작업 이름이 아닌 토큰으로 변수 이름을 사용 합니다.
...
디자이너에서 코드 뷰 편집기로 전환 하는 경우 다음은 **변수 초기화** 작업을 논리 앱 정의에 표시 하는 방법입니다 (JSON (JavaScript Object Notation) 형식).
...
> [!NOTE]
> **변수 초기화** 작업에는 `variables` 배열로 구조화 된 섹션이 있지만 작업에서 한 번에 하나의 변수만 만들 수 있습니다. 각 새 변수에는 개별 **변수 초기화** 작업이 필요 합니다.
...
예를 들어이 식은 함수를 사용 하 여 [이 문서에서 이전에 만든](#append-value) 배열 변수에서 항목을 가져옵니다 `variables()` . `string()`함수는 다음과 같이 문자열 형식으로 변수의 내용을 반환 합니다.`"1, 2, 3, red"`
...
상수 값으로 *변수를 늘리거나 늘리려면 논리* 앱에 **변수 증가** 작업을 추가 합니다. 이 작업은 정수 및 float 변수에서만 작동합니다.

1. 논리 앱 디자이너에서 기존 변수를 증가 시키려는 단계 아래에서 **새 단계**를 선택 합니다. 
...
디자이너에서 코드 뷰 편집기로 전환 하는 경우 다음은 JSON 형식인 논리 앱 정의 내에 **증분 변수** 작업을 표시 하는 방법입니다.
...
1. [**변수 초기화** 작업](#create-variable)을 추가합니다. 시작 값이 0 인 이라는 정수 변수를 만듭니다 `Count` .

   ![“변수 초기화” 작업 추가](./media/logic-apps-create-variables-store-values/initialize-variable.png)

1. 각 첨부 파일을 순환 하려면 *for each* 루프를 추가 합니다.

   1. **변수 초기화** 작업에서 **새 단계**를 선택 합니다.

   1. **작업 선택** 아래에서 **기본 제공**을 선택합니다. 검색 상자에 `for each` 검색 필터로를 입력 하 고 **각각에 대해**를 선택 합니다.
...
1. 루프에서 **이전 단계에서 출력 선택** 상자 내부를 클릭합니다. 동적 콘텐츠 목록이 나타나면 **첨부 파일**을 선택합니다.
   ...
   **Attachments** 속성은 전자 메일 첨부 파일을 포함 하는 배열을 사용자의 루프로 전달 합니다.

1. **For each** 루프에서 **작업 추가**를 선택 합니다.
...
1. 검색 상자에 “변수 증가”를 필터로 입력합니다. 작업 목록에서 **증가 변수**를 선택 합니다.
   ...
   > [!NOTE]
   > **증분 변수** 동작이 루프 내에 표시 되는지 확인 합니다. 작업이 루프 외부에 표시되는 경우 작업을 루프로 끌어옵니다.
...
1. 논리 앱을 사용할 수 없는 경우 논리 앱 메뉴에서 **개요**를 선택 합니다. 도구 모음에서 **사용**을 선택 합니다.

1. 논리 앱 디자이너 도구 모음에서 **실행**을 선택 합니다. 이 단계에서는 논리 앱을 수동으로 시작합니다.
...
디자이너에서 코드 뷰 편집기로 전환 하는 경우 **for each** 루프가 JSON 형식의 논리 앱 정의 내에서 **증가 변수** 작업과 함께 표시 되는 방법입니다.
...
상수 값으로 *변수를 줄이거나 줄이려면 변수* 를 [증가](#increment-value) 시키기 위한 단계를 수행 합니다. 단, 변수 **감소** 작업을 대신 찾아 선택 합니다. 이 작업은 정수 및 float 변수에서만 작동합니다.
...
| 속성 | 필수 | 값 |  Description |
|----------|----------|-------|--------------|
| **이름** | 예 | <*변수 이름*> | 변경할 변수의 이름입니다. |
| **값** | 예 | <*새-값*> | 변수에 할당할 값입니다. 둘 다 데이터 형식이 같아야 합니다. |
||||| 
...
>
> 1. 루프의 오른쪽 위 모서리에서 줄임표 (**...**) 단추를 선택한 다음 **설정**을 선택 합니다.
> 
> 2. **동시성 제어** 아래에서 **기본값 재정의** 설정을 **켬**으로 변경합니다.
>
> 3. **병렬도** 슬라이더를 **1**로 끕니다.

디자이너에서 코드 뷰 편집기로 전환 하는 경우 다음은 **변수 설정** 동작이 JSON 형식인 논리 앱 정의 내에 표시 되는 방법입니다. 이 예에서는 `Count` 변수의 현재 값을 다른 값으로 변경 합니다.
...
|----------|----------|-------|--------------|
| **이름** | 예 | <*변수 이름*> | 변경할 변수의 이름입니다. |
| **값** | 예 | <*추가 값*> | 추가하려는 값으로, 임의 형식일 수 있습니다. |
|||||
