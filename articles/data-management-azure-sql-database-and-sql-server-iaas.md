...
**Microsoft Azure SQL 데이터베이스(Azure SQL 데이터베이스)**는  *Platform as a Service (PaaS)* 산업 부문으로 분류되는 관계형 DaaS(Database-as-a-Service)입니다. Azure SQL 데이터베이스는 Microsoft에서 소유하고 호스트하고 유지 관리하는 표준화된 하드웨어 및 소프트웨어를 기반으로 구축됩니다. SQL 데이터베이스에서는 기본 제공 기능을 사용하여 서비스를 직접 개발할 수 있습니다. SQL 데이터베이스를 사용할 경우 필요한 추가 성능에 맞게 강화 및 규모 확장하는 옵션으로 사용량에 따라 지불합니다.

**Azure VM(가상 컴퓨터)의 SQL Server**는  *Infrastructure as a Service (IaaS)* 산업 부문으로 분류되며 클라우드의 가상 컴퓨터 내에서 SQL Server를 실행할 수 있도록 지원합니다. Azure SQL 데이터베이스와 마찬가지로 Microsoft에서 소유하고 호스트하고 유지 관리하는 표준화된 하드웨어를 기반으로 구축됩니다. VM의 SQL Server를 사용할 경우 고유한 SQL Server 라이선스를 Azure로 가져오거나 Azure 포털에 미리 구성된 SQL Server 이미지 중 하나를 사용할 수 있습니다.
...
<tr>
   ...
   <td valign="middle">
      ...
      <li type=round>Azure 저장소의 <a href="http://msdn.microsoft.com/library/jj919148.aspx">백업</a> 또는 <a href="http://msdn.microsoft.com/library/azure/jj870962.aspx">Azure VM의 AlwaysOn 복제본을 사용하여 온-프레미스 SQL Server 응용 프로그램에 대한 재해 복구</a>
...
<tr>
   ...
   <td valign="middle"><ul><li type=round>Azure SQL 데이터베이스는 내결함성 인프라 기능을 기본적으로 제공할 뿐 아니라 지정 시간 복원, 지역 복원 및 지역에서 복제 등의 다양한 기능을 제공하여 비즈니스 연속성을 향상합니다. 자세한 내용은 <a href="http://msdn.microsoft.com/library/azure/hh852669.aspx">Azure SQL 데이터베이스 비즈니스 연속성을</a>참조하세요.</ul></td>
   <td valign="middle"><ul><li type=round>Azure VM의 SQL Server를 사용하면 데이터베이스의 특정 요구에 맞게 고가용성 및 재해 복구 솔루션을 설정할 수 있습니다. 따라서 시스템을 응용 프로그램에 최적화할 수 있습니다. 필요한 경우 장애 조치(Failover)를 직접 테스트하고 실행할 수 있습니다. 자세한 내용은 <a href="http://msdn.microsoft.com/library/azure/jj870962.aspx">Azure 가상 컴퓨터의 SQL Server에 대한 고가용성 및 재해 복구를</a>참조하세요.</ul></td>
...
<tr>
   ...
   <td valign="middle"><ul>
      <li type=round>Azure VM의 SQL Server를 사용하면 응용 프로그램을 일부는 클라우드에서, 일부는 온-프레미스에서 실행할 수 있습니다. 예를 들어 온-프레미스 네트워크 및 도메인의 Active Directory를 <a href="http://msdn.microsoft.com/library/azure/gg433091.aspx">Azure 네트워크 서비스를 통해</a>클라우드로 확장할 수 있습니다. 또한 Azure 기능의 <a href="http://msdn.microsoft.com/library/dn385720.aspx">SQL Server 데이터 파일을 사용하여</a>Azure 저장소에 온-프레미스 데이터 파일을 저장할 수 있습니다. 자세한 내용은 <a href="http://msdn.microsoft.com/library/dn606154.aspx">SQL Server 2014 하이브리드 클라우드 소개를</a>참조하세요.
      <li type=round>Azure  저장소의  <a href="http://msdn.microsoft.com/library/jj919148.aspx">백업</a> 또는 <a href="http://msdn.microsoft.com/library/azure/jj870962.aspx">Azure VM의 AlwaysOn 복제본을 사용하여 온-프레미스 SQL Server 응용 프로그램에 대한 재해 복구를</a>지원합니다.
      </ul></td>
...
**중요 정보:** 현재 Azure SQL 데이터베이스는 SQL Server의 일부 기능을 지원하지 않습니다. 자세한 비교 정보는 [Azure SQL 데이터베이스 지침 및 제한 사항](http://msdn.microsoft.com/library/azure/ff394102.aspx)을 참조하세요. 기존 데이터베이스를 Azure SQL 데이터베이스로 이동하려는 경우 데이터베이스를 다시 디자인하는 데 추가 비용이 필요할 수 있습니다. Azure SQL 데이터베이스는 Microsoft의 PaaS(Platform as-a-Service) 제품입니다. 기존 온-프레미스 SQL Server 응용 프로그램을 Azure SQL 데이터베이스로 마이그레이션할 때 응용 프로그램을 업데이트하여 PaaS(Platform-as-a-Service) 제품의 모든 이점을 누리는 것이 좋습니다. 예를 들어 응용 프로그램 계층에서 [Azure 웹 사이트](http://azure.microsoft.com/documentation/services/websites/) 또는 [Azure 클라우드 서비스](http://azure.microsoft.com/services/cloud-services/)를 사용하여 비용 혜택을 늘리세요. 또한 다른 Azure SQL 데이터베이스 서비스 계층에 대해 응용 프로그램 유효성 검사를 하고 응용 프로그램 요구 사항에 가장 적합한 계층을 확인하세요. 이러한 작업은 더 나은 성능 결과를 얻고 비용을 최소화하는 데 도움이 됩니다. 자세한 내용은 [Azure SQL 데이터베이스 서비스 계층 및 성능 수준](http://msdn.microsoft.com/library/azure/dn741336.aspx)을 참조하세요.
...
- 새로운 클라우드 기반 응용 프로그램을 빌드하는 경우 또는 기존 SQL Server 데이터베이스를 Azure로 마이그레이션하려고 하는데 데이터베이스에서 Azure SQL 데이터베이스에서 지원되지 않는 기능은 사용하지 않는 경우. 자세한 내용은 참조 [Azure SQL 데이터베이스 Transact-SQL 참조](http://msdn.microsoft.com/library/azure/ee336281.aspx)를 참조하세요. 이 접근 방식을 사용하면 클라우드 서비스를 완전히 관리할 수 있고 빠른 출시 시간이 보장됩니다.
...
**기술 검토자:** Conor Cunningham

**테크니컬 리뷰어:** Joanne Marone (Hodgins), Karthika Raman, Lindsey Allen, Lori Clark, Luis Carlos Vargas Herring, Nosheen Syed Wajahatulla Hussain, Pravin Mittal, Shawn Bice, Silvano Coriani, Tony Petrossian, Tracy Daugherty.
