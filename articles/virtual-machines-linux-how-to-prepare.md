...
Azure Linux 에이전트(Waagent)는 NetworkManager와 호환되지 않습니다. 네트워킹 구성은 ifcfg-eth0 파일을 사용해야 하며, ifup/ifdown 스크립트를 통해 제어할 수 있어야 합니다. Waagent는 NetworkManager 패키지가 검색되는 경우 설치가 거부됩니다.

NUMA는 2.6.37 이하 Linux 커널 버전의 버그 때문에 지원되지 않습니다. waagent의 설치 프로그램은 Linux 커널 명령줄의 GRUB 구성에서 NUMA를 사용하지 않도록 자동으로 설정합니다.
...
6.  다음 명령을 실행하여 네트워크 서비스를 사용하도록 설정합니다.

        chkconfig network on

7.  CentOS 6.2 또는 6.3: Linux 통합 서비스 드라이버 설치
...
9.  /etc/yum.repos.d/CentOS-Base.repo 파일을 다음 텍스트로 바꿉니다.
...
10. /etc/yum.conf에 다음 줄을 추가합니다.

        http_caching=packages
        exclude=kernel*

11. "/etc/yum/pluginconf.d/fastestmirror.conf" 파일을 편집하여 yum 모듈 "fastestmirror"를 사용하지 않도록 설정하고 [main] 아래에 다음을 입력합니다.

        set enabled=0

12. 다음 명령을 실행하여 현재 yum 메타데이터를 지웁니다.

        yum clean all

13. CentOS 6.2 및 6.3의 경우 다음 명령을 실행하여 실행 중인 VM 커널을 업데이트합니다.
...
14. 다음 매개 변수를 포함하려면 Grub의 커널 부팅 줄을 수정합니다. 이렇게 하면 모든 콘솔 메시지가 첫 번째 직렬 포트로 전송되므로 Azure 지원에서 문제를 디버깅하는 데에도 도움이 될 수 있습니다.

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300

15. 커널에 탑재된 모든 SCSI 장치에 300초 이상의 I/O 시간 제한이 포함되었는지 확인합니다.
...
5.  Ubuntu는 시스템 작동 중지 후 사용자 입력을 위한 Grub 프롬프트를 기다립니다. 이를 예방하려면 다음 단계를 완료하십시오.
    ...
    d) update-grub를 실행합니다.

6.  다음 매개 변수를 포함하려면 Grub 또는 Grub2의 커널 부팅 줄을 수정합니다. 이렇게 하면 모든 콘솔 메시지가 첫 번째 직렬 포트로 전송되므로 Azure 지원에서 문제를 디버깅하는 데에도 도움이 될 수 있습니다.
