...
2. 응용 프로그램 프록시가 커넥터에 연결
3. 커넥터가 대상 응용 프로그램에 연결
...
응용 프로그램 프록시는 테넌트 위치에 따라 사용자의 인스턴스 위치를 선택합니다. 커넥터를 설치할 위치를 결정하여 네트워크 트래픽의 대기 시간 특정을 정의할 수 있습니다.
...
커넥터는 Azure 및 응용 프로그램과 통신해야 하므로 커넥터의 배치는 이러한 두 연결의 대기 시간에 영향을 줍니다. 커넥터의 배치를 평가할 때 다음을 고려합니다.

* Single Sign-On에 대해 KCD(Kerberos 제한 위임)을 사용하려는 경우 커넥터에서 데이터 센터에 대한 가시성을 확보해야 합니다. 
* 확실하지 않은 경우 응용 프로그램에 더 가깝게 커넥터를 설치합니다.

### <a name="general-approach-to-minimize-latency"></a>대기 시간을 최소화하는 일반적인 방법

각 네트워크 연결을 최적화하여 종단 간 트래픽을 시도하고 대기 시간을 최소화할 수 있습니다. 각 연결은 다음을 통해 최적화할 수 있습니다.
...
사용자와 Azure AD 응용 프로그램 프록시 간 연결을 제어하기 위해 수행할 수 있는 방법은 거의 없습니다. 홈 네트워크, 커피숍 또는 다른 국가에서 앱에 액세스할 수도 있습니다. 대신 응용 프로그램 프록시에서 커넥터로, 다시 앱으로의 연결을 최적화할 수 있습니다. 사용자 환경에서 다음과 같은 패턴을 통합하는 것이 좋습니다.

### <a name="pattern-1-put-the-connector-close-to-the-application"></a>패턴 1: 커넥터를 응용 프로그램 가까이에 배치

커넥터를 고객 네트워크의 대상 응용 프로그램 가까이에 배치합니다. 이 패턴은 커넥터가 도메인 컨트롤러에 대한 시야를 필요로 할 수 있다는 장점이 있습니다. 이 방법은 대부분의 시나리오에 충분하므로 대부분의 고객이 이 패턴을 따릅니다. 이 패턴을 패턴 2와 조합하여 응용 프로그램 프록시와 커넥터 사이의 트래픽을 최적화할 수 있습니다.
...
다른 위치 및 네트워크에 있는 대상 앱에 [커넥터 그룹](active-directory-application-proxy-connectors.md)을 사용하는 것이 좋습니다. 이 옵션을 고려하는 경우 [Azure AD Domain Services](../active-directory-domain-services/active-directory-ds-overview.md)를 참조하세요.
...
- **홉 1**: 사용자 - Azure AD 응용 프로그램 프록시
- **홉 2**: Azure AD 응용 프로그램 프록시 - 커넥터
- **홉 3**: 커넥터 - 대상 응용 프로그램 
