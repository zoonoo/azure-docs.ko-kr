...
> | 그룹 초과 지표 | `groups:src1` | 길이는 제한되지 않으나 토큰에 너무 큰 토큰 요청의 경우 사용자의 전체 그룹 목록에 대한 링크가 포함됩니다. SAML의 경우 `groups` 클레임 대신 새 클레임으로 추가됩니다. <br><br> **참고**: <br> Azure AD Graph API는 Microsoft Graph API로 바뀝니다. 해당 엔드포인트에 관한 자세한 내용은 [user: getMemberObjects](https://docs.microsoft.com/graph/api/user-getmemberobjects)를 참조하세요. | `<Attribute Name=" http://schemas.microsoft.com/claims/groups.link">`<br>`<AttributeValue>https://graph.windows.net/{tenantID}/users/{userID}/getMemberObjects<AttributeValue>` |
...
* Microsoft Graph API를 사용한 토큰 수명 정책 관리에 관한 자세한 내용은 [Azure AD 정책 리소스 개요](/graph/api/resources/policy)를 참조하세요.
