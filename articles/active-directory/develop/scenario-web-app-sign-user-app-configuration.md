...
- 예를 들어 국가`Instance`클라우드에서 앱을 실행 하려는 경우 클라우드 인스턴스 ()
- 테 넌 트 ID ()의`TenantId`대상입니다.
- Azure Portal에서 복사한 응용`ClientId`프로그램의 클라이언트 ID ()입니다.

경우에 따라 `Authority`응용 프로그램을 매개 변수가 있는 수 있습니다 .이는 및 `Instance` `TenantId`의 연결입니다.
...
ASP.NET Core 다른 파일 ([properties\launchSettings.json](https://github.com/Azure-Samples/active-directory-aspnetcore-webapp-openidconnect-v2/blob/bc564d68179c36546770bf4d6264ce72009bc65a/1-WebApp-OIDC/1-1-MyOrg/Properties/launchSettings.json#L6-L7))에는 응용 프로그램 및 다양`applicationUrl`한 프로필에 대 한 URL ()`sslPort`과 TLS/SSL 포트 ()가 포함 되어 있습니다.
...
Azure Portal에서 응용 프로그램에 대 한 **인증** 페이지에 등록 해야 하는 회신 uri는 이러한 url과 일치 해야 합니다. 위의 두 구성 파일은 `https://localhost:44321/signin-oidc`입니다. 이유는 `applicationUrl` 가 `http://localhost:3110`이지만 (44321) `sslPort` 가 지정 된 이유입니다. `CallbackPath`는 `/signin-oidc`에 `appsettings.json`정의 된 대로입니다.
...
Java에서 구성은 아래 `src/main/resources`에 있는 [응용 프로그램. 속성](https://github.com/Azure-Samples/ms-identity-java-webapp/blob/d55ee4ac0ce2c43378f2c99fd6e6856d41bdf144/src/main/resources/application.properties) 파일에 있습니다.
...
Azure Portal에서 응용 프로그램에 대 한 **인증** 페이지에 등록 해야 하는 회신 uri는 응용 프로그램에서 정의 하는 `redirectUri` 인스턴스와 일치 해야 합니다. 즉, 및 `http://localhost:8080/msal4jsample/secure/aad` `http://localhost:8080/msal4jsample/graph/me`여야 합니다.
...
웹 앱 (및 web Api)에서 컨트롤러 또는 컨트롤러 작업에 대 한 `[Authorize]` 특성이 있으므로 응용 프로그램은 보호 됩니다. ASP.NET Core 이 특성은 사용자가 인증 되었는지 확인 합니다. 응용 프로그램을 초기화 하는 코드는 *Startup.cs* 파일에 있습니다.

Microsoft id 플랫폼 (이전의 Azure AD v2.0)으로 인증을 추가 하려면 다음 코드를 추가 해야 합니다. 코드의 주석은 설명이 필요 하지 않습니다.

> [!NOTE]
> Visual Studio 내에서 또는 또는 `dotnet new mvc --auth SingleAuth` `dotnet new webapp --auth SingleAuth`를 사용 하 여 기본 ASP.NET Core 웹 프로젝트를 사용 하 여 프로젝트를 시작 하는 경우 다음과 같은 코드가 `services.AddAuthentication(AzureADDefaults.AuthenticationScheme).AddAzureAD(options => Configuration.Bind("AzureAd", options));`표시 됩니다.
> 
...
2. 에서 `ConfigureServices` `AddSignIn` 및 `AddMicrosoftIdentityUI` 메서드를 사용 하도록 코드를 업데이트 합니다.
...
3. Startup.cs의 `Configure` 메서드에서를 *Startup.cs*호출 하 여 인증을 사용 하도록 설정 합니다.`app.UseAuthentication();`
...
위의 코드에서 다음을 수행 합니다.
- 확장 `AddSignIn` 메서드는 **Microsoft. Identity. Web**에 정의 되어 있습니다. 메서드
  - 인증 서비스를 추가 합니다.
...
- 구성 개체 외에도를 호출할 `AddSignIn`때 구성 섹션의 이름을 지정할 수 있습니다. 기본적으로 `AzureAd`입니다.

- `AddSignIn`고급 시나리오에 대 한 다른 매개 변수가 있습니다. 예를 들어 추적 Openid connect Connect 미들웨어 이벤트는 인증이 작동 하지 않는 경우 웹 응용 프로그램의 문제를 해결 하는 데 도움이 될 수 있습니다. 선택적 매개 변수 `subscribeToOpenIdConnectMiddlewareDiagnosticsEvents` 를로 `true` 설정 하면 HTTP 응답에서 사용자 id로 진행 되는 ASP.NET Core 미들웨어 집합에서 정보를 처리 하는 방법을 보여 줍니다 `HttpContext.User`.

- 확장 `AddMicrosoftIdentityUI` 메서드는 **Microsoft. Identity. system.web**. 로그 아웃을 처리 하는 기본 컨트롤러를 제공 합니다.
...
Java 샘플에서는 스프링 프레임 워크를 사용 합니다. 각 HTTP 응답을 가로채는 필터를 구현 하므로 응용 프로그램은 보호 됩니다. Java 웹 앱에 대 한 빠른 시작에서이 필터 `AuthFilter` 는 `src/main/java/com/microsoft/azure/msalwebsample/AuthFilter.java`에 있습니다.
