...
- 토큰 액세스를 위한 새 모델:
  - ADAL은 서버를 나타내는를 통해 토큰에 대 한 액세스를 제공 합니다 `AuthenticationContext` . MSAL은 클라이언트를 나타내는를 통해 토큰에 대 한 액세스를 제공 합니다 `PublicClientApplication` . 클라이언트 개발자는 `PublicClientApplication` 와 상호 작용 해야 하는 모든 기관에 대해 새 인스턴스를 만들 필요가 없습니다. 구성이 하나만 `PublicClientApplication` 필요 합니다.
...
- MSAL은 및 호출에 대 한 매개 변수 개체를 지원 `AcquireToken` `AcquireTokenSilent` 합니다.
...
현재 ADAL을 사용 하 고 있고 증분 동의를 사용할 필요가 없는 경우 MSAL을 사용 하 여 시작 하는 가장 간단한 방법은 `acquireToken` 새 개체를 사용 하 여 요청을 수행 하 `AcquireTokenParameter` 고 리소스 ID 값을 설정 하는 것입니다.
...
MSAL을 사용 하는 경우를 인스턴스화합니다 `PublicClientApplication` . 이 개체는 앱 id를 모델링 하 고 하나 이상의 기관에 요청을 수행 하는 데 사용 됩니다. 이 개체를 사용 하 여 클라이언트 id, 리디렉션 URI, 기본 인증, 장치 브라우저 사용 여부, 로그 수준 등을 구성 합니다.

JSON을 사용 하 여이 개체를 선언적으로 구성할 수 있습니다 .이 개체는 APK 내의 리소스로 파일 또는 저장소로 제공 합니다.

이 개체는 단일 항목이 아니지만 내부적으로 `Executors` 대화형 및 자동 요청에 대해 shared를 사용 합니다.
...
ADAL에서는 `AuthenticationResult` `UserInfo` 인증 된 계정에 대 한 정보를 검색 하는 데 사용 되는 개체를 제공 합니다. 사람이 나 소프트웨어 에이전트를 의미 하는 "사용자" 라는 용어는 여러 계정이 포함 된 단일 사용자 (사용자 또는 소프트웨어 에이전트의 경우)를 지원 한다는 것을 알리는 방식으로 적용 되었습니다.
...
계정이 표시 되는 각 테 넌 트의 계정에 대 한 클레임에 액세스 하는 앱이 있는 경우 `IAccount` 개체를로 캐스팅할 수 있습니다 `IMultiTenantAccount` . 이 인터페이스는 `ITenantProfiles` 테 넌 트 ID로 키가 지정 된 맵을 제공 합니다 .이 맵을 사용 하면 현재 계정에 상대적인 토큰을 요청한 각 테 넌 트의 계정에 속하는 클레임에 액세스할 수 있습니다.

및의 루트에 있는 클레임은 `IAccount` `IMultiTenantAccount` 항상 홈 테 넌 트의 클레임을 포함 합니다. 홈 테 넌 트 내에서 토큰에 대 한 요청을 아직 수행 하지 않은 경우이 컬렉션은 비어 있습니다.
