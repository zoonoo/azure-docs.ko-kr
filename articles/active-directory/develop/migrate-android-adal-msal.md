...
- 토큰 액세스를 위한 새 모델:
  - ADAL은 `AuthenticationContext`서버를 나타내는를 통해 토큰에 대 한 액세스를 제공 합니다. MSAL은 클라이언트를 나타내는 `PublicClientApplication`를 통해 토큰에 대 한 액세스를 제공 합니다. 클라이언트 개발자는와 상호 작용 해야 하 `PublicClientApplication` 는 모든 기관에 대해 새 인스턴스를 만들 필요가 없습니다. `PublicClientApplication` 구성이 하나만 필요 합니다.
...
- 토큰 API 변경:
  - ADAL에서는 `AcquireToken()` 먼저 자동 요청을 수행 합니다. 실패 하면 대화형 요청이 발생 합니다. 이 동작으로 인해 일부 개발자가에 `AcquireToken`만 의존 하 여 때때로 사용자가 예기치 않게 자격 증명을 입력 하 라는 메시지가 표시 됩니다. MSAL을 사용 하려면 사용자에 게 UI 프롬프트가 표시 될 때 개발자가 의도적인 것을 요구 합니다.
...
- MSAL은 및 `AcquireTokenSilent` 호출에 `AcquireToken` 대 한 매개 변수 개체를 지원 합니다.
...
현재 ADAL을 사용 하 고 있고 증분 동의를 사용할 필요가 없는 경우 MSAL을 사용 하 여 시작 하는 가장 간단한 방법은 `acquireToken` 새 `AcquireTokenParameter` 개체를 사용 하 여 요청을 수행 하 고 리소스 ID 값을 설정 하는 것입니다.
...
- `Scope`: 권한 부여를 요청 하 고 액세스 토큰을 받을 범위 목록입니다.
...
MSAL을 사용 하는 `PublicClientApplication`경우를 인스턴스화합니다. 이 개체는 앱 id를 모델링 하 고 하나 이상의 기관에 요청을 수행 하는 데 사용 됩니다. 이 개체를 사용 하 여 클라이언트 id, 리디렉션 URI, 기본 인증, 장치 브라우저 사용 여부, 로그 수준 등을 구성 합니다.

JSON을 사용 하 여이 개체를 선언적으로 구성할 수 있습니다 .이 개체는 APK 내의 리소스로 파일 또는 저장소로 제공 합니다.

이 개체는 단일 항목이 아니지만 내부적으로 대화형 및 자동 `Executors` 요청에 대해 shared를 사용 합니다.

### <a name="business-to-business"></a>비즈니스 간

ADAL에서는에서 액세스 토큰을 요청 하는 모든 조직에 별도의 인스턴스가 필요 합니다 `AuthenticationContext`. MSAL에서는 더 이상 필요 하지 않습니다. 자동 또는 대화형 요청의 일부로 토큰을 요청 하려는 기관을 지정할 수 있습니다.
...
Microsoft에 알려지지 않은 기관을 사용 하려고 시도 하 고 구성에 포함 되지 않은 경우를 받게 됩니다 `UnknownAuthorityException`.
...
ADAL에서는 `AuthenticationResult` 인증 된 계정 `UserInfo` 에 대 한 정보를 검색 하는 데 사용 되는 개체를 제공 합니다. 사람이 나 소프트웨어 에이전트를 의미 하는 "사용자" 라는 용어는 여러 계정이 포함 된 단일 사용자 (사용자 또는 소프트웨어 에이전트의 경우)를 지원 한다는 것을 알리는 방식으로 적용 되었습니다.
...
Microsoft id 플랫폼은 금융 기관과 차이가 있는 경우, Microsoft id 플랫폼은 사용자가 한 계정 및 연결 된 자격 증명을 사용 하 여 여러 개인 및 조직에 속한 리소스에 액세스할 수 있도록 하는 프레임 워크를 제공 합니다. 이는 한 은행에서 발행 한 카드를 다른 금융 기관에서 사용할 수 있는 것과 같습니다. 이는 문제의 모든 조직이 Microsoft id 플랫폼을 사용 하 여 여러 조직에서 하나의 계정을 사용할 수 있기 때문에 작동 합니다. 아래 예를 살펴보세요.
...
호출이 성공적 `acquireToken` 으로 수행 되 면 이후 `IAccount` `acquireTokenSilent` 요청에서 사용할 수 있는 개체에 대 한 참조가 표시 됩니다.

### <a name="imultitenantaccount"></a>IMultiTenantAccount

계정이 표시 되는 각 테 넌 트의 계정에 대 한 클레임에 액세스 하는 앱이 있는 경우 개체를로 `IAccount` `IMultiTenantAccount`캐스팅할 수 있습니다. 이 인터페이스는 테 넌 트 `ITenantProfiles`ID로 키가 지정 된 맵을 제공 합니다 .이 맵을 사용 하면 현재 계정에 상대적인 토큰을 요청한 각 테 넌 트의 계정에 속하는 클레임에 액세스할 수 있습니다.

`IAccount` 및 `IMultiTenantAccount` 의 루트에 있는 클레임은 항상 홈 테 넌 트의 클레임을 포함 합니다. 홈 테 넌 트 내에서 토큰에 대 한 요청을 아직 수행 하지 않은 경우이 컬렉션은 비어 있습니다.
...
ADAL에는 `ADALError` 열거형 값을 검색 하기 위한 메서드 `AuthenticationException`를 포함 하는 예외 형식이 하나 있습니다.
