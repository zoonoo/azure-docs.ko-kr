...
**IP(ID 공급자)** - IP는 사용자 ID를 인증하고 보안 토큰을 발급하는 기관입니다. 토큰 발급의 실제 작업은 STS(보안 토큰 서비스)라는 특수 서비스를 통해 구현됩니다. IP의 일반적인 예로는 Windows Live ID, Facebook, 비즈니스 사용자 리포지토리(예: Active Directory) 등이 있습니다.
ACS가 IP를 신뢰하도록 구성된 경우 시스템은 해당 IP에서 발급된 토큰을 수락하고 유효성을 검사합니다. ACS는 한 번에 여러 IP를 신뢰할 수 있으며, 이는 응용 프로그램이 ACS를 신뢰하는 경우 사용자를 대신하여 ACS가 신뢰하는 모든 IP의 인증된 모든 사용자에게 응용 프로그램을 즉시 제공할 수 있음을 의미합니다.
...
* Azure 구독(<http://www.microsoft.com/windowsazure/offers/>에서 획득 가능)
...
Azure에서 ACS(Access Control Service) 사용을 시작하려면 ACS 네임스페이스를 만들어야 합니다. 네임스페이스는 응용 프로그램 내에서 ACS 리소스의 주소를 지정하기 위한 고유 범위를 제공합니다.

1. [Azure 관리 포털][Azure Management Portal]에 로그인합니다.
2. **Active Directory**를 클릭합니다. 
3. 새 Access Control 네임스페이스를 만들려면 **새로 만들기**, **App Services**, **Access Control**, **빠른 생성**을 차례로 클릭합니다. 
...
## <a name="upload-a-certificate-to-your-acs-namespace"></a>ACS 네임스페이스에 인증서 업로드
...
## <a name="review-the-application-integration-page"></a>응용 프로그램 통합 페이지 검토
...
3. **새 JSP 파일** 대화 상자에서 파일의 이름을 **index.jsp**로 지정합니다. 다음에 표시된 것처럼 상위 폴더를 MyACSHelloWorld/WebContent로 유지합니다.
...
## <a name="add-the-acs-filter-library-to-your-application"></a>응용 프로그램에 ACS Filter 라이브러리 추가
...
## <a name="deploy-to-the-compute-emulator"></a>계산 에뮬레이터에 배포
...
7. 브라우저에서 <http://localhost:8080/MyACSHelloWorld/>(또는 **HTTPS 연결 필요**를 선택한 경우 <https://localhost:8080/MyACSHelloWorld/>)를 열어 응용 프로그램을 실행합니다. Windows Live ID 로그인 메시지가 표시되어야 하며, 그런 다음 신뢰 당사자 응용 프로그램에 대해 지정된 반환 URL로 이동되어야 합니다.
8. 응용 프로그램 확인을 마쳤으면 **Reset Azure Emulator** 단추를 클릭합니다.
...
7. 관리 포털의 **로그인 페이지 통합** 페이지가 열려 있는 브라우저를 사용하여 **옵션 1: ACS 호스트된 로그인 페이지에 연결** 필드에 나열된 URL을 복사하여 Eclipse 대화 상자의 **ACS Authentication Endpoint** 필드에 붙여넣습니다.
