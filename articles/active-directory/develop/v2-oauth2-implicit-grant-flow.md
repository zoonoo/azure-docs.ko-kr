...
> [!IMPORTANT]
> ID 토큰을 성공적으로 요청 하려면 **암시적 권한 부여** 에서 **액세스 토큰** 및 **ID 토큰** 을 선택 하 여 [Azure Portal 앱 등록](https://go.microsoft.com/fwlink/?linkid=2083908) 페이지의 앱 등록에서 암시적 부여 흐름이 올바르게 설정 되어 있어야 합니다. 여기서. 사용 하도록 설정 되지 않은 경우 `unsupported_response` 오류가 반환 됩니다. **입력 매개 변수 ' response_type '에 대해 제공 된 값이이 클라이언트에 허용 되지 않습니다. 필요한 값은 ' 코드 '입니다** .
