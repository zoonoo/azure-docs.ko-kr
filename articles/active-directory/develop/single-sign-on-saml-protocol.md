...
`RequestedAuthnContext` 요소는 원하는 인증 방법을 지정합니다. Azure AD로 전송되는 `AuthnRequest` 요소에서는 선택 사항입니다. Azure AD는 `urn:oasis:names:tc:SAML:2.0:ac:classes:Password`와 같은 `AuthnContextClassRef` 값을 지원 합니다.
