...
**앱 추가**를 클릭하여 이름을 지정합니다. 포털에서는 나중에 코드에서 사용할 전역적으로 고유한 애플리케이션 ID를 앱에 할당합니다. 앱이 API(예: Office, Azure 또는 사용자의 고유한 웹 API)를 호출하는 액세스 토큰이 필요한 서버 측 구성 요소를 포함한 경우, **애플리케이션 암호** 또한 만들어야 합니다.
