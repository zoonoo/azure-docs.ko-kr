...
앱이 사용자 이름/암호를 사용 하는 경우에는 응용 프로그램에 대 한 리디렉션 URI를 등록할 필요가 없습니다. 실제로이 흐름은 Microsoft identity platform v2.0 끝점으로의 왕복을 수행 하며 응용 프로그램은 특정 URI에서 다시 호출 되지 않습니다. 그러나 응용 프로그램이 공용 클라이언트 응용 프로그램 임을 표현 해야 합니다. 이 구성은 응용 프로그램에 대 한 **인증** 섹션으로 이동 하 고, **고급 설정** 하위 섹션에서 **예**를 선택 하 고, 질문을 **응용 프로그램을 공용 클라이언트로 간주** ( **기본 클라이언트 유형** 단락)
