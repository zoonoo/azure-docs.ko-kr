---
title: Azure AD B2C (Android 용 Microsoft 인증 라이브러리) | Microsoft
...
앱에 대 한 구성 파일은 두 `authorities`을 선언 합니다. 각 정책에 대해 하나씩입니다. 각 기관의 `type` 속성은-1 @no__t 됩니다.
...
@No__t-0은 앱 구성에 등록 되어야 하 고, [인증 코드 부여 흐름](https://docs.microsoft.com/en-us/azure/active-directory-b2c/active-directory-b2c-reference-oauth-code)동안 리디렉션을 지원 하기 위해 `AndroidManifest.xml` 에서도 지원 됩니다.

## <a name="initialize-ipublicclientapplication"></a>IPublicClientApplication 초기화

`IPublicClientApplication`은 응용 프로그램 구성을 비동기적으로 구문 분석할 수 있도록 팩터리 메서드에 의해 생성 됩니다.
...
MSAL을 사용 하 여 토큰을 자동으로 획득 하려면 `AcquireTokenSilentParameters` 인스턴스를 빌드하여 `acquireTokenSilentAsync` 메서드에 제공 합니다. @No__t-0 메서드와 달리 토큰을 자동으로 얻으려면 `authority`을 지정 해야 합니다.
...
B2C의 정책은 별도의 인증 기관으로 표시 되기 때문에 기본값 이외의 정책을 호출 하는 것은 `acquireToken` 또는 `acquireTokenSilent` 매개 변수를 생성할 때 `fromAuthority` 절을 지정 하 여 수행 됩니다.  예를 들어 다음과 같은 가치를 제공해야 합니다.
...
대신 오류 코드 `AADB2C90118`이 앱으로 반환 됩니다. 앱은 암호를 재설정 하는 특정 사용자 흐름을 실행 하 여이 오류 코드를 처리 해야 합니다.

암호 다시 설정 오류 코드를 catch 하기 위해 @no__t 내에서 다음 구현을 사용할 수 있습니다.
...
토큰 획득이 성공 하면 @no__t 0 개체가 반환 됩니다. 여기에는 액세스 토큰, 사용자 클레임 및 메타 데이터가 포함 됩니다.
...
IdToken에서 반환 된 클레임은 MSAL이 아니라 STS (보안 토큰 서비스)에 의해 채워집니다. 사용 된 IdP (id 공급자)에 따라 일부 클레임이 없을 수도 있습니다. 일부 IdPs는 현재 @no__t 0 클레임을 제공 하지 않습니다. 이 클레임은 캐싱에 대해 MSAL에서 사용 되기 때문에 자리 표시자 값 `MISSING FROM THE TOKEN RESPONSE`이 대신 사용 됩니다. B2C IdToken 클레임에 대 한 자세한 내용은 [Azure Active Directory B2C의 토큰 개요](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-reference-tokens#claims)를 참조 하세요.

## <a name="managing-accounts-and-policies"></a>계정 및 정책 관리

B2C는 각 정책을 별도의 기관으로 처리 합니다. 따라서 각 정책에서 반환 된 액세스 토큰, 새로 고침 토큰 및 ID 토큰은 서로 교환할 수 없습니다. 즉, 각 정책은 다른 정책을 호출 하는 데 사용할 수 없는 토큰을 가진 별도의 `IAccount` 개체를 반환 합니다.

각 정책은 각 사용자의 캐시에 `IAccount`을 추가 합니다. 사용자가 응용 프로그램에 로그인 하 고 두 개의 정책을 호출 하는 경우 두 개의 @no__t 0이 됩니다. 캐시에서이 사용자를 제거 하려면 각 정책에 대해 `removeAccount()`을 호출 해야 합니다.

@No__t-0으로 정책에 대 한 토큰을 갱신 하는 경우 정책에 대 한 이전 호출에서 반환 된 것과 동일한 `IAccount`을 `AcquireTokenSilentParameters`로 제공 합니다. 다른 정책에서 반환 된 계정을 제공 하면 오류가 발생 합니다.
