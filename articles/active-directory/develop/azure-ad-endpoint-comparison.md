...
* v1.0과 v2.0 엔드포인트 둘 다 테넌트별 엔드포인트(`https://login.microsoftonline.com/{TenantId_or_Name}`)를 가리키도록 구성된 *[단일 테넌트](single-and-multi-tenant-apps.md)* 또는 *다중 테넌트* 애플리케이션으로 구성된 애플리케이션에 대해 Azure AD 디렉터리의 *[게스트 사용자](https://docs.microsoft.com/azure/active-directory/b2b/what-is-b2b)* 가 로그인할 수 있습니다.
...
v2.0 엔드포인트를 사용하는 애플리케이션의 경우 동일한 OAuth 2.0 인증 요청은 다음과 같습니다.
...
Microsoft ID 플랫폼과 통합되는 애플리케이션을 빌드하는 경우, v2.0 엔드포인트 및 인증 프로토콜이 사용자 요구를 충족하는지 여부를 판단해야 합니다. v1.0 엔드포인트 및 플랫폼은 여전히 완벽하게 지원되며 어떤 면에서는 v2.0보다 더 풍부한 기능을 제공합니다. 그러나 v2.0은 개발자에게 [상당한 혜택을 소개](azure-ad-endpoint-comparison.md)합니다.
