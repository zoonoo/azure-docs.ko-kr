...
- 이 가이드에서 앱을 빌드하려면 XCode 버전 11.x 이상이 필요합니다. [Mac App Store](https://geo.itunes.apple.com/us/app/xcode/id497799835?mt=12 "XCode 다운로드 URL")에서 XCode를 다운로드할 수 있습니다.
...
7. 화면 위쪽에 있는 **새 환경 사용해 보기**를 클릭하여 새 앱 등록 환경을 연 다음, **+새 등록** >  **+ 플랫폼 추가** > **iOS/macOS**를 클릭합니다.
...
8. `Configure`를 클릭하고 **MSAL 구성** 페이지에 나타나는 **MSAL 구성**을 저장합니다. 그러면 나중에 앱을 구성할 때 이 구성을 입력할 수 있습니다.  **Done**을 클릭합니다.
...
위에서 수정해야 하는 값은 [애플리케이션 ID](./developer-glossary.md#application-id-client-id)`kClientID`에 할당된 값뿐입니다. 이 값은 이 자습서의 시작 부분에서 애플리케이션을 Azure Portal에 등록하는 단계에서 저장한 MSAL 구성 데이터의 일부입니다.
