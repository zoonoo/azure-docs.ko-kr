...
description: 유니버설 Windows 플랫폼(XAML) 응용 프로그램이 액세스 토큰을 가져와서 Azure Active Directory v2.0 엔드포인트로 보호된 API를 호출할 수 있는 방법을 알아봅니다.
...
이 빠른 시작에는 UWP(유니버설 Windows 플랫폼) 응용 프로그램이 개인 또는 회사 및 학교 계정으로 사용자를 로그인하고, 액세스 토큰을 가져오고, Microsoft Graph API를 호출할 수 있는 방법을 보여 주는 코드 샘플이 포함되어 있습니다.

![이 빠른 시작에서 생성된 샘플 앱의 작동 방식](media/quickstart-v2-uwp/uwp-intro.png)
...
> 이 빠른 시작의 코드 샘플이 작동하려면 리디렉션 URL을 **urn:ietf:wg:oauth:2.0:oob**로 추가해야 합니다.
...
1. **App.Xaml.cs**를 편집하고 `private static string ClientId`로 시작하는 줄을 다음으로 바꿉니다.
...
이 빠른 시작에 대한 개요는 다음과 같습니다.

### <a name="msalnet"></a>MSAL.NET

MSAL([Microsoft.Identity.Client](https://www.nuget.org/packages/Microsoft.Identity.Client))은 사용자를 로그인하고 Microsoft Azure Active Directory로 보호된 API 액세스에 사용하는 토큰을 요청하는 데 사용되는 라이브러리입니다. Visual Studio의 *패키지 관리자 콘솔*에서 다음 명령을 실행하여 설치할 수 있습니다.
...
아래 줄을 추가하여 MSAL에 대한 참조를 추가할 수 있습니다.

```csharp
using Microsoft.Identity.Client;
```

그런 다음, 아래 줄을 사용하여 MSAL을 초기화합니다.
...
#### <a name="get-a-user-token-interactively"></a>대화형으로 사용자 토큰 가져오기
 ...
 일부 상황에서는 사용자가 팝업 창을 통해 Azure Active Directory v2 엔드포인트를 조작하여 자격 증명의 유효성을 검사하거나 동의를 제공해야 합니다. 일부 사례:
...
사용자가 리소스에 액세스해야 할 때마다 자격 증명의 유효성을 검사할 필요가 없도록 하려고 합니다. 대부분은 사용자 개입 없이 토큰 가져오기 및 갱신을 자동으로 처리하려고 합니다. `AcquireTokenSilentAsync` 메서드는 초기 `AcquireTokenAsync` 메서드 다음에 보호된 리소스 액세스에 사용하는 토큰을 가져오는 데 일반적으로 사용되는 메서드입니다.
