...
> [!NOTE]
> X5T 헤더는 인증서의 해시를 사용 하 여 기본 64 문자열로 변환 하 여 계산 해야 합니다. 에서 C# 이를 수행 하는 코드는 `System.Convert.ToBase64String(cert.GetCertHash());`합니다.

[Microsoft id 플랫폼을 사용 하는 코드 샘플 .Net Core 디먼 콘솔 응용](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2) 프로그램은 응용 프로그램에서 인증에 자체 자격 증명을 사용 하는 방법을 보여 줍니다. 또한 [ Powershell 명령을 사용하여 ](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2/tree/master/1-Call-MSGraph#optional-use-the-automation-script)자체 서명된 인증서를 만드는`New-SelfSignedCertificate` 방법을 보여줍니다. [앱 만들기 스크립트](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2/blob/master/1-Call-MSGraph/AppCreationScripts-withCert/AppCreationScripts.md)를 활용 및 사용하여 인증서를 만들고, 지문 등을 계산할 수도 있습니다.
