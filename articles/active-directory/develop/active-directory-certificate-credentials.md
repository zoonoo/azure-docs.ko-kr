...
Microsoft ID 플랫폼을 사용하면 응용 프로그램이 인증을 위해 자체 자격 증명을 사용할 수 있습니다(예: [OAuth 2.0 클라이언트 자격 증명 부여 flowv2.0](v2-oauth2-client-creds-grant-flow.md) 및 [Of-Of flow).](v2-oauth2-on-behalf-of-flow.md)

애플리케이션이 인증에 사용할 수 있는 자격 증명의 한 가지 형태는 애플리케이션에서 소유한 인증서로 서명된 JWT(JSON Web Token) 어설션입니다.

## <a name="assertion-format"></a>어설션 형식
Microsoft ID 플랫폼 어설션을 계산하려면 원하는 언어로 여러 [JSON 웹 토큰](https://jwt.ms/) 라이브러리 중 하나를 사용할 수 있습니다. 토큰에 의해 전달되는 정보는 다음과 같습니다.
...
| 매개 변수 |  설명 |
| --- | --- |
| `aud` | 청중 : ** https://login.microsoftonline.com/ *tenant_Id*/ oauth2 / 토큰이어야합니다.** |
...
다음 방법 중 다음 방법을 사용하여 Azure 포털을 통해 Microsoft ID 플랫폼의 클라이언트 응용 프로그램과 인증서 자격 증명을 연결할 수 있습니다.

### <a name="uploading-the-certificate-file"></a>인증서 파일 업로드

클라이언트 애플리케이션에 대한 Azure 앱 등록에서:
1. **인증서 및 비밀**을 선택합니다.
2. 인증서 **업로드를** 클릭하고 업로드할 인증서 파일을 선택합니다.
3. **추가**를 클릭합니다.
  인증서가 업로드되면 지문, 시작 날짜 및 만료 값이 표시됩니다.
...
클라이언트 애플리케이션에 대한 Azure 앱 등록에서:
1. 응용 프로그램 매니페스트를 열려면 **매니페스트를** 선택합니다.
2. 다음 스키마를 사용해서 *keyCredentials* 속성을 새 인증서 정보로 바꿉니다.
   ...
   ```
3. 편집 된 편집 을 응용 프로그램 매니페스트에 저장 한 다음 매니페스트를 Microsoft ID 플랫폼에 업로드 합니다.
...
> [!NOTE]
> 인증서의 해시를 사용하여 기본 64 문자열로 변환하여 X5T 헤더를 계산해야 합니다. C#에서 이 작업을 수행하는 `System.Convert.ToBase64String(cert.GetCertHash());`코드는 .

Microsoft ID 플랫폼을 사용하는 코드 샘플 [.NET Core 데몬 콘솔 응용 프로그램은](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2) 응용 프로그램이 인증을 위해 자체 자격 증명을 사용하는 방법을 보여 주며 있습니다. 또한 `New-SelfSignedCertificate` Powershell 명령을 사용하여 [자체 서명된 인증서를 만드는](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2/tree/master/1-Call-MSGraph#optional-use-the-automation-script) 방법을 보여줍니다. [앱 만들기 스크립트](https://github.com/Azure-Samples/active-directory-dotnetcore-daemon-v2/blob/master/1-Call-MSGraph/AppCreationScripts-withCert/AppCreationScripts.md)를 활용 및 사용하여 인증서를 만들고, 지문 등을 계산할 수도 있습니다.
