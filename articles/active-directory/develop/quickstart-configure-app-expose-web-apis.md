---
title: 웹 API를 공개하는 응용 프로그램 구성 | Azure
description: 응용 프로그램이 새 권한/범위와 역할을 공개하여 해당 응용 프로그램을 클라이언트 응용 프로그램이 사용할 수 있게 구성하는 방법을 살펴봅니다.
...
---
# <a name="quickstart-configure-an-application-to-expose-web-apis-preview"></a>빠른 시작: 웹 API를 공개하는 응용 프로그램 구성(미리 보기)

웹 API를 개발하고 [권한/범위](developer-glossary.md#scopes) 및 [역할](developer-glossary.md#roles)을 공개하여 클라이언트 응용 프로그램에서 사용할 수 있게 만들 수 있습니다. 올바르게 구성된 웹 API는 Graph API 및 Office 365 API와 같은 다른 Microsoft 웹 API와 마찬가지로 사용 가능합니다.
...
1. 새 범위를 공개하는 데 사용할 방법, UI 또는 응용 프로그램 매니페스트를 선택합니다.
...
1. **응용 프로그램 ID URI**를 설정하지 않은 경우 입력하라는 메시지가 표시됩니다. 응용 프로그램 ID URI를 입력하거나, 제공된 항목을 사용한 다음, **저장 및 계속**을 선택합니다.
...
## <a name="expose-a-new-scope-or-role-through-the-application-manifest"></a>응용 프로그램 매니페스트를 통해 새 범위 또는 역할 공개
...
응용 프로그램 매니페스트를 통해 새 범위 또는 역할을 공개하려면
...
1. 완료되면 **저장**을 클릭합니다. 이제 Web API가 디렉토리에 있는 다른 응용 프로그램에 의해 사용되도록 구성되었습니다.
1. [웹 API가 다른 응용 프로그램에 공개되는지 확인](#verify-the-web-api-is-exposed-to-other-applications)하기 위한 단계를 따릅니다.

## <a name="verify-the-web-api-is-exposed-to-other-applications"></a>웹 API가 다른 응용 프로그램에 공개되는지 확인합니다.

1. Azure AD 테넌트로 돌아가서 **앱 등록**을 다시 선택한 다음, 구성하려는 클라이언트 응용 프로그램을 찾아 선택합니다.
