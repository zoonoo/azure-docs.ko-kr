---
title: 웹 API에 액세스하는 응용 프로그램 구성 | Azure
description: 리디렉션 URI, 자격 증명 또는 웹 API에 대한 액세스 권한을 포함하도록 Microsoft 플랫폼에 등록된 응용 프로그램을 구성하는 방법을 알아봅니다.
...
---
# <a name="quickstart-configure-a-client-application-to-access-web-apis-preview"></a>빠른 시작: 웹 API에 액세스하는 클라이언트 응용 프로그램 구성(미리 보기)
...
또한 클라이언트가 리소스 응용 프로그램(예: Microsoft Graph API)에서 공개한 웹 API에 액세스하기 전에 동의 프레임워크는 클라이언트에서 요청된 권한에 기반하여 필요한 권한 부여를 얻도록 합니다. 기본적으로 모든 응용 프로그램은 Microsoft Graph API에서 사용 권한을 선택할 수 있습니다. [Graph API "로그인 및 읽기 사용자 프로필" 권한](https://developer.microsoft.com/graph/docs/concepts/permissions_reference#user-permissions)은 기본적으로 선택됩니다. 원하는 웹 API 각각에서 [두 가지 형식의 사용 권한](developer-glossary.md#permissions) 중에 선택할 수 있습니다.

* **응용 프로그램 권한** -클라이언트 응용 프로그램이 직접 웹 API에 액세스해야 합니다(사용자 컨텍스트 없음). 이 유형의 권한은 관리자의 동의가 필요하며 공용(데스크톱 및 모바일) 클라이언트 응용 프로그램에 대해 사용할 수 없습니다.
...
* 지원되는 [권한 및 동의](v2-permissions-and-consent.md)를 살펴봅니다. 여기서는 다른 사용자 또는 응용 프로그램에서 사용해야 하는 응용 프로그램을 빌드하는 경우를 이해하는 것이 중요합니다.
* 응용 프로그램이 등록된 테넌트가 있습니다.
  * 앱이 등록되지 않았다면 [Microsoft ID 플랫폼에 응용 프로그램을 등록하는 방법](quickstart-register-app.md)을 살펴봅니다.
...
1. 구성하려는 응용 프로그램을 찾아 선택합니다. 앱을 선택하면 볼 **개요** 또는 기본 등록 페이지가 나타납니다.
1. 웹 API에 액세스하도록 응용 프로그램을 구성하는 단계를 따릅니다. 
    * [리디렉션 URL을 응용 프로그램에 추가](#add-redirect-uris-to-your-application)
    * [웹 응용 프로그램에 자격 증명 추가](#add-credentials-to-your-web-application)
    * [웹 API에 액세스 권한 추가](#add-permissions-to-access-web-apis)

## <a name="add-redirect-uris-to-your-application"></a>응용 프로그램에 리디렉션 URI 추가
...
리디렉션 URI를 응용 프로그램에 추가하려면

1. 앱의 **개요** 페이지에서 **인증** 섹션을 선택합니다.

1. 웹 및 공용 클라이언트 응용 프로그램에 대한 사용자 지정 리디렉션 URI를 추가하려면 다음 단계를 따릅니다.

    1. **리디렉션 URI** 섹션을 찾습니다.
    1. 빌드하는 응용 프로그램 유형을 **웹** 또는 **공용 클라이언트(모바일 및 데스크톱)** 로 선택합니다.
    1. 응용 프로그램의 리디렉션 URI를 입력합니다.
        * 웹 응용 프로그램의 경우 응용 프로그램의 기준 URL을 제공합니다. 예를 들어 http://localhost:31544은 로컬 머신에서 실행 중인 웹 응용 프로그램의 URL일 수 있습니다. 사용자는 이 URL을 사용하여 웹 클라이언트 응용 프로그램에 로그인합니다.
        * 공용 응용 프로그램의 경우 Azure AD에서 토큰 응답을 반환하는 데 사용하는 URI를 제공합니다. 응용 프로그램에 고유하게 해당되는 값을 입력합니다(예: https://MyFirstApp).

1. 공용 클라이언트(모바일, 데스크톱)에 대해 제안된 리디렉션 URI에서 선택하려면 다음 단계를 따릅니다.

    1. **공용 클라이언트(모바일, 데스크톱)에 대해 제안된 리디렉션 URI** 섹션을 찾습니다.
    1. 확인란을 사용하여 응용 프로그램에 적합한 리디렉션 URI를 선택합니다.

## <a name="add-credentials-to-your-web-application"></a>웹 응용 프로그램에 자격 증명 추가
...
1. API를 선택한 후에는 **요청된 API 권한** 페이지가 표시됩니다. API가 위임된 권한과 응용 프로그램 권한 모두를 공개할 경우 응용 프로그램에 필요한 권한 유형을 선택합니다.
