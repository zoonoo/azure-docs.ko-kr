---
title: 클라이언트 응용 프로그램 초기화 (.NET 용 Microsoft 인증 라이브러리) | Microsoft
...
응용 프로그램 빌더를 사용 하는 코드 조각에서는 여러 `.With` 메서드를 한정자로 적용할 수 있습니다 ( `.WithCertificate` 예: 및 `.WithRedirectUri`). 
...
|`.WithDebugLoggingCallback()` | 호출 되 `Debug.Write` 는 경우 응용 프로그램은 단순히 디버깅 추적을 사용 하도록 설정 합니다. 자세한 내용은 [로깅](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/wiki/logging) 을 참조 하세요.|
...
|매개 변수 | Description|
|--------- | --------- |
|`.WithIosKeychainSecurityGroup()` | **Xamarin.ios만**해당: 캐시 지 속성을 위한 iOS 키 체인 보안 그룹을 설정 합니다.|
