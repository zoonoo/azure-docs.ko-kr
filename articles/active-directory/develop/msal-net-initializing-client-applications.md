...
이 문서에서는 .NET 용 Microsoft Authentication Library (MSAL.NET)를 사용 하 여 공용 클라이언트 및 기밀 클라이언트 응용 프로그램을 초기화 하는 방법을 설명 합니다.  클라이언트 응용 프로그램 유형에 대 한 자세한 내용은 [공용 클라이언트 및 기밀 클라이언트 응용 프로그램](msal-client-applications.md)을 참조 하세요.
...
[API 참조 설명서](/dotnet/api/microsoft.identity.client)  |  [NuGet](https://www.nuget.org/packages/Microsoft.Identity.Client/)  |  의 패키지 [라이브러리 소스 코드](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet)  |  [코드 샘플](sample-v2-code.md)
...
동일한 방식으로 다음 코드는 `https://myapp.azurewebsites.net` Microsoft Azure 공용 클라우드의 사용자에서 작업 및 학교 계정 또는 개인 Microsoft 계정으로 토큰을 처리 하는 기밀 응용 프로그램 (에 있는 웹 앱)을 인스턴스화합니다. 응용 프로그램은 클라이언트 암호를 공유 하 여 id 공급자로 식별 됩니다.
...
기밀 클라이언트 응용 프로그램에는 동일한 종류의 패턴이 적용 됩니다. `.WithXXX`한정자 (여기서는 인증서)를 사용 하 여 다른 매개 변수를 추가할 수도 있습니다.
...
|`.WithExtraQueryParameters(IDictionary<string,string> eqp)` | 모든 인증 요청에서 전송 될 응용 프로그램 수준 추가 쿼리 매개 변수를 설정 합니다. 이는 동일한를 사용 하 여 각 토큰 획득 방법 수준에서 재정의할 수 `.WithExtraQueryParameters pattern` 있습니다.|
...
클라이언트 응용 프로그램을 초기화 한 후 다음 작업은 사용자 로그인, 권한 있는 API 액세스 또는 둘 다에 대 한 지원을 추가 하는 것입니다.

응용 프로그램 시나리오 설명서에서는 사용자에 게 로그인 하 고 액세스 토큰을 획득 하 여 해당 사용자를 대신 하 여 API에 액세스 하는 지침을 제공 합니다.

- [사용자 로그인 및 로그 아웃 하는 웹 앱](scenario-web-app-sign-user-sign-in.md)
- [웹 Api를 호출 하는 웹 앱: 토큰 획득](scenario-web-app-call-api-acquire-token.md)
