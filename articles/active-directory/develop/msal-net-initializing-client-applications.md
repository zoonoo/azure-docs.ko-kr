...
동일한 방식으로, 다음 코드는 기밀 응용 프로그램 (`https://myapp.azurewebsites.net`에 있는 웹 앱)을 Microsoft Azure 공용 클라우드의 사용자 (회사 및 학교 계정) 또는 개인 Microsoft 계정에 대 한 토큰을 처리 합니다. 응용 프로그램은 클라이언트 암호를 공유 하 여 id 공급자로 식별 됩니다.
...
응용 프로그램 빌더를 사용 하는 코드 조각에서는 여러 `.With` 메서드를 한정자 (예: `.WithCertificate` 및 `.WithRedirectUri`)로 적용할 수 있습니다. 
