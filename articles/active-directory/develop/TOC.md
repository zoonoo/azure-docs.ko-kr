# 개요
## [Azure AD와 통합](active-directory-how-to-integrate.md)
## [인증 시나리오](active-directory-authentication-scenarios.md)
## [그래프 API](active-directory-graph-api.md)
## [인증 라이브러리](active-directory-authentication-libraries.md)
## [인증 프로토콜](active-directory-authentication-protocols.md)
## [앱 및 서비스 주체 개체](active-directory-application-objects.md)
## [개발자 가이드](active-directory-developers-guide.md)
## [OAuth2 암시적 허용](active-directory-dev-understanding-oauth2-implicit-grant.md)
## [Azure AD의 응용 프로그램](active-directory-how-applications-are-added.md)

# 시작
## 모바일 및 PC 앱
### [iOS](active-directory-devquickstarts-ios.md)
### [Android](active-directory-devquickstarts-android.md)
### [Windows Phone](active-directory-devquickstarts-windowsphone.md)
### [.Net](active-directory-devquickstarts-dotnet.md)
### [Windows 스토어](active-directory-devquickstarts-windowsstore.md)
### [Xamarin](active-directory-devquickstarts-xamarin.md)
### [Cordova](active-directory-devquickstarts-cordova.md)
### [Oauth](active-directory-protocols-oauth-code.md)
## 웹 앱
### [.NET](active-directory-devquickstarts-webapp-dotnet.md)
### [Java](active-directory-devquickstarts-webapp-java.md)
### [AngularJS](active-directory-devquickstarts-angular.md)
### [NodeJS](active-directory-devquickstarts-openidconnect-nodejs.md)
### [OpenID Connect](active-directory-protocols-openid-connect-code.md)
### [Java 명령줄](active-directory-devquickstarts-headless-java.md)
## Web API
### [Web API 및 .NET](active-directory-devquickstarts-webapi-dotnet.md)
### [NodeJS](active-directory-devquickstarts-webapi-nodejs.md)
## [그래프 API](active-directory-graph-api-quickstart.md)

# 방법
## [테넌트 가져오기](active-directory-howto-tenant.md)
## [모든 Azure AD 사용자 로그인](active-directory-devhowto-multi-tenant-overview.md)
## [앱 통합](active-directory-integrating-applications.md)
## SSO 사용
### [Android](active-directory-sso-android.md)
### [iOS](active-directory-sso-ios.md)
## [AppSource 인증하기](active-directory-devhowto-appsource-certified.md)
## [갤러리에서 앱 나열](active-directory-app-gallery-listing.md)
## [앱 매니페스트 이해](active-directory-application-manifest.md)
## [웹 사용자 인증](active-directory-java-authenticate-users-access-control-eclipse.md)
## SAML 작업
### [Access Control에서 반환되는 SAML 보기](active-directory-java-view-saml-returned-by-access-control.md)
### [Azure AD 및 SAML](active-directory-saml-protocol-reference.md)
### [차트 사용자 지정](active-directory-saml-claims-customization.md)
### [SAML 기반 SSO 디버그](active-directory-saml-debugging.md)
## [서명 키 롤오버 사용](active-directory-signing-key-rollover.md)


## V2 끝점 사용
### [개요](active-directory-appmodel-v2-overview.md)
### [V2의 다른 점은 무엇입니까?](active-directory-v2-compare.md)
### [인증 라이브러리](active-directory-v2-libraries.md)
### [앱 형식](active-directory-v2-flows.md)
### [제한 사항](active-directory-v2-limitations.md)
### [범위](active-directory-v2-scopes.md)
### [등록 포털](active-directory-v2-registration-portal.md)
### [토큰 참조](active-directory-v2-tokens.md)
### 프로토콜
#### [OAuth 2.0 클라이언트 자격 증명](active-directory-v2-protocols-oauth-client-creds.md)
#### [OAuth 2.0 및 OpenID](active-directory-v2-protocols.md)
#### [최근 업데이트](active-directory-v2-preview-oidc-changes.md)
### 모바일 앱
#### [iOS](active-directory-v2-devquickstarts-ios.md)
#### [Windows Phone](active-directory-v2-devquickstarts-wpf.md)
#### [Android](active-directory-v2-devquickstarts-android.md)
### 웹앱 및 API
#### [NodeJS로 AngularJS](active-directory-v2-devquickstarts-angular-node.md)
#### [.NET으로 AngularJS](active-directory-v2-devquickstarts-angular-dotnet.md)
#### [.NET 웹앱](active-directory-v2-devquickstarts-dotnet-web.md)
#### [NodeJS 웹앱](active-directory-v2-devquickstarts-node-web.md)
#### [.NET API](active-directory-v2-devquickstarts-dotnet-api.md)
#### [NodeJS API](active-directory-v2-devquickstarts-node-api.md)
#### [웹 API 호출](active-directory-v2-devquickstarts-webapp-webapi-dotnet.md)
### 프로토콜을 사용하여 직접 통합
#### [앱 등록](active-directory-v2-app-registration.md)
#### [OpenID Connect](active-directory-v2-protocols-oidc.md)
#### [암시적 흐름](active-directory-v2-protocols-implicit.md)
#### [OAuth 2.0 인증 코드](active-directory-v2-protocols-oauth-code.md)
#### [OAuth 2.0 클라이언트 자격 증명](active-directory-v2-protocols-oauth-client-creds.md)
#### [서비스-서비스 호출](active-directory-protocols-oauth-service-to-service.md)

## Visual Studio 작업
### [.NET 시작](vs-active-directory-dotnet-getting-started.md)
### [내 MVC 프로젝트가 어떻게 되었습니까?](vs-active-directory-dotnet-what-happened.md)
### [인증 검색 중 오류 발생](vs-active-directory-error.md)
### [웹 WebApi 시작](vs-active-directory-webapi-getting-started.md)
### [내 WebApi 프로젝트가 어떻게 되었습니까?](vs-active-directory-webapi-what-happened.md)


# 참조
## [용어](active-directory-dev-glossary.md)
## [브랜딩 지침](active-directory-branding-guidelines.md)
## [코드 샘플](active-directory-code-samples.md)
## [페더레이션 메타데이터](active-directory-federation-metadata.md)
## [Single Sign-On](active-directory-single-sign-on-protocol-reference.md)
## [Single Sign-Out](active-directory-single-sign-out-protocol-reference.md)
## [토큰 및 클레임](active-directory-token-and-claims.md)


<!--HONumber=Jan17_HO3-->


