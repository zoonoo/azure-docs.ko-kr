...
description: Microsoft ID 플랫폼에서 지원하는 OAuth 2.0 및 OpenID Connect 프로토콜에 대한 가이드입니다.
...
ms.locfileid: "98755744"
---
# <a name="oauth-20-and-openid-connect-protocols-on-the-microsoft-identity-platform"></a>Microsoft ID 플랫폼의 OAuth 2.0 및 OpenID Connect 프로토콜

IaaS(Identity as a Service)용 Microsoft ID 플랫폼 엔드포인트는 업계 표준 프로토콜 OIDC(OpenID Connect) 및 OAuth 2.0을 사용하여 인증 및 권한 부여를 구현합니다. 서비스는 표준을 준수하지만 이러한 프로토콜의 두 구현 간에는 약간의 차이가 있을 수 있습니다. 여기에 있는 정보는 [오픈 소스 라이브러리](reference-v2-libraries.md) 중 하나를 사용하는 대신 HTTP 요청을 직접 전송 및 처리하여 코드를 작성하거나 타사 오픈 소스 라이브러리를 사용하도록 선택한 경우에 유용합니다.
...
* **권한 부여 서버** 는 Microsoft ID 플랫폼이며 사용자 ID를 확인하고 리소스에 대한 액세스 권한을 부여 및 해지하고, 토큰을 발급합니다. 권한 부여 서버는 ID 공급자라고도 하며 사용자 정보, 해당 액세스 및 흐름의 요소 간 트러스트 관계와 관련된 모든 사항을 안전하게 처리합니다.
* **리소스 소유자** 는 일반적으로 최종 사용자입니다. 데이터를 소유하는 당사자이며 클라이언트가 해당 데이터 또는 리소스에 액세스하도록 허용할 권한이 있습니다.
...
> [!TIP]
> Azure AD에 등록된 모든 앱은 개인 계정에 로그인하지 않아도 Microsoft ID 플랫폼을 사용할 수 있습니다.  이러한 방식으로 애플리케이션을 다시 만들지 않고도 기존 애플리케이션을 Microsoft ID 플랫폼 및 [MSAL](reference-v2-libraries.md)로 마이그레이션할 수 있습니다.

## <a name="tokens"></a>토큰

OAuth 2.0과 OpenID Connect는 일반적으로 [JWT(JSON Web Token)](https://tools.ietf.org/html/rfc7519)로 표현되는 **전달자 토큰** 을 광범위하게 활용합니다. 전달자 토큰은 보호된 리소스에 대한 "전달자" 액세스 권한을 부여하는 간단한 보안 토큰입니다. 이러한 점에서 토큰의 복사본을 가져오는 것은 무엇이든 “전달자”입니다. 당사자는 전달자 토큰을 받으려면 먼저 Microsoft ID 플랫폼을 사용하여 인증해야 하지만 전송 및 스토리지에서 토큰 보안을 유지하는 데 필요한 단계를 취하지 않으면 의도하지 않은 당사자가 토큰을 가로채서 사용할 수 있습니다. 일부 보안 토큰에는 권한 없는 당사자가 사용하는 것을 방지하는 기본 제공 메커니즘이 있지만, 전달자 토큰에는 이러한 메커니즘이 없으므로 전송 계층 보안(HTTPS)과 같은 보안 채널에서 전달자 토큰을 전송해야 합니다. 전달자 토큰이 보안되지 않은 상태로 전송되는 경우, 악의적 사용자가 메시지 가로채기(man-in-the-middle) 공격으로 토큰을 획득하고 보호된 리소스에 무단으로 액세스하는 데 이 토큰을 사용할 수 있습니다. 나중에 사용하기 위해 전달자 토큰을 저장하거나 캐싱할 때도 동일한 보안 원칙이 적용됩니다. 항상 앱이 안전한 방식으로 전달자 토큰을 전송하고 저장하도록 합니다. 전달자 토큰의 보안 고려 사항을 자세히 알아보려면 [RFC 6750 Section 5](https://tools.ietf.org/html/rfc6750)를 참조하세요.

OAuth 2.0/OIDC에서는 주로 세 가지 유형의 토큰이 사용됩니다.

* [액세스 토큰](access-tokens.md) - 리소스 서버가 클라이언트에서 받는 토큰으로 클라이언트에 부여된 권한이 포함되어 있습니다.  
* [ID 토큰](id-tokens.md) - 클라이언트가 권한 부여 서버에서 받는 토큰으로 사용자를 로그인하고 해당 사용자에 대한 기본 정보를 가져오는 데 사용됩니다.
* 새로 고침 토큰 - 클라이언트가 시간 경과에 따라 새로운 액세스 권한을 얻고 ID 토큰을 가져오는 데 사용됩니다.  해당 토큰은 불투명 문자열이며 권한 부여 서버에서만 인식할 수 있습니다.

## <a name="protocols"></a>프로토콜

일부 예제 요청을 확인할 준비가 되었다면 아래 프로토콜 문서 중 하나를 시작합니다. 각각 특정 인증 시나리오에 해당합니다. 사용자에게 적합한 흐름을 결정하는 데 도움이 필요하면 [Microsoft ID 플랫폼으로 빌드할 수 있는 앱 형식](v2-app-types.md)을 확인하세요.

* [OAuth 2.0으로 모바일, 네이티브, 웹 애플리케이션 빌드](v2-oauth2-auth-code-flow.md)
* [OpenID Connect로 사용자 로그인](v2-protocols-oidc.md)
...
* [OAuth 2.0 암시적 흐름으로 단일 페이지 앱 빌드](v2-oauth2-implicit-grant-flow.md)
