...
description: 모든 Azure Active Directory 테넌트에서 사용자를 로그인할 수 있는 다중 테넌트 응용 프로그램을 빌드하는 방법을 보여줍니다.
...
테넌트 관리자는 일반 사용자가 애플리케이션에 동의하는 기능을 사용하지 않도록 설정할 수 있습니다. 이 기능이 사용되지 않는 경우 테넌트에서 애플리케이션을 사용하려면 항상 관리자 동의가 필요합니다. 최종 사용자 동의를 사용하지 않도록 설정된 응용 프로그램을 테스트하려면 **엔터프라이즈 응용 프로그램** 아래 **[사용자 설정](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/UserSettings/menuId/)** 섹션의 [Azure Portal][AZURE-portal]에서 구성 스위치를 찾을 수 있습니다.
...
응용 프로그램에 관리자 동의가 필요하고 관리자가 `prompt=admin_consent` 매개 변수를 보내지 않고 로그인하는 경우, 관리자가 응용 프로그램에 동의할 때 **자신의 사용자 계정에만** 적용됩니다. 일반 사용자는 여전히 애플리케이션에 로그인하거나 동의할 수 없습니다. 이 기능은 다른 사용자에게 액세스를 허용하기 전에 응용 프로그램을 탐색하는 기능을 테넌트 관리자에게 부여하고자 할 때 유용합니다.
