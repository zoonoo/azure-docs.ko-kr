...
테넌트 관리자는 일반 사용자가 애플리케이션에 동의하는 기능을 사용하지 않도록 설정할 수 있습니다. 이 기능이 사용되지 않는 경우 테넌트에서 응용 프로그램을 사용하려면 항상 관리자 동의가 필요합니다. 최종 사용자 동의를 사용하지 않도록 설정된 응용 프로그램을 테스트하려면 **엔터프라이즈 응용 프로그램** 아래 **[사용자 설정](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/UserSettings/menuId/)** 섹션의 [Azure Portal][AZURE-portal]에서 구성 스위치를 찾을 수 있습니다.

관리 동의가 필요하지 않은 사용 권한을 요청하는 애플리케이션에서 `prompt=admin_consent` 매개 변수를 사용할 수도 있습니다. 예를 들어 응용 프로그램에 테넌트 관리자가 한 번 “등록”한 환경이 필요하고, 해당 시점에서 다른 사용자에게 동의를 요구하지 않는 경우가 있습니다.
