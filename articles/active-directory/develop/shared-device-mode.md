---
title: Android 장치에 대 한 공유 장치 모드 | Microsoft
...
소매 연결, 비행 crew 구성원 및 현장 서비스 작업자와 같은 firstline 작업자는 일반적으로 공유 모바일 장치를 사용 하 여 작업을 수행 합니다. 공유 장치에서 고객 및 비즈니스 데이터에 액세스 하기 위해 암호 또는 pin 번호 공유를 시작 하는 경우 문제가 됩니다.
...
MSAL (Microsoft Authentication Library SDK)을 사용 하 여 작성 된 응용 프로그램은 단일 계정 또는 여러 계정을 관리할 수 있습니다. 자세한 내용은 [단일 계정 모드 또는 여러 계정 모드](https://docs.microsoft.com/azure/active-directory/develop/single-multi-account)를 참조 하세요. 앱에서 사용할 수 있는 Microsoft id 플랫폼 기능은 응용 프로그램이 단일 계정 모드에서 실행 되는지 또는 여러 계정 모드로 실행 되는지에 따라 달라 집니다.
...
앱이 실행 중인 장치 유형에 따라 동작을 변경 하도록 할 수도 있습니다. `ISingleAccountPublicClientApplication.isSharedDevice()`를 사용 하 여 단일 계정 모드에서 실행할 시기를 결정할 수 있습니다.
