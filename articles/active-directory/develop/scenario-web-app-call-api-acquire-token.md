---
title: Web Api를 호출 하는 웹 앱에서 토큰 가져오기-Microsoft identity platform | Microsoft
...
- 토큰 캐시를 사용 하 여 web API에 대 한 토큰을 가져옵니다. 이 토큰을 가져오려면 `AcquireTokenSilent`를 호출 합니다.
...
`ITokenAcquisition` 서비스는 종속성 주입을 통해 ASP.NET에 의해 삽입 됩니다.
...
`getAuthResultBySilentFlow`를 호출 하려고 합니다. 사용자가 더 많은 범위에 동의 해야 하는 경우 코드는 `MsalInteractionRequiredException`를 처리 하 여 사용자를 시도 합니다.
