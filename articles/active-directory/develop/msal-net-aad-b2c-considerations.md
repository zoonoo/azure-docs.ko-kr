---
title: Azure AD B2C (Microsoft Authentication Library for .NET) | Microsoft
...
사용할 `https://login.microsoftonline.com/tfp/{tenant}/{policyName}` 기관은 다음과 같습니다.

- `tenant`Azure AD B2C 테 넌 트의 이름입니다. 
- `policyName`적용할 정책의 이름 (예를 들어 로그인/등록의 경우 "b2c_1_susi")입니다.

Azure AD B2C의 현재 지침은 기관으로를 사용 `b2clogin.com` 하는 것입니다. `$"https://{your-tenant-name}.b2clogin.com/tfp/{your-tenant-ID}/{policyname}"` )을 입력합니다. 자세한 내용은이 [설명서](/azure/active-directory-b2c/b2clogin)를 참조 하세요.
...
정책을 적용 하는 경우 (예를 들어 최종 사용자가 프로필을 편집 하거나 암호를 다시 설정 하는 등 `AcquireTokenInteractive`)은 현재를 호출 하 여 수행 됩니다. 이러한 두 정책의 경우 반환 된 토큰/인증 결과를 사용 하지 않습니다.

## <a name="special-case-of-editprofile-and-resetpassword-policies"></a>EditProfile 및 ResetPassword 정책의 특수 한 경우

최종 사용자가 소셜 id를 사용 하 여 로그인 한 다음 해당 프로필을 편집 하 여 Azure AD B2C EditProfile 정책을 적용 하려는 환경을 제공 하려는 경우. 이 작업을 수행 하는 방법은 사용자가 `AcquireTokenInteractive` 이미 로그인 했으므로 계정 선택 대화 상자가 표시 되지 않도록 해당 정책 `Prompt.NoPrompt` 에 대 한 특정 기관 및 프롬프트를로 설정 하 여를 호출 하는 것입니다.
