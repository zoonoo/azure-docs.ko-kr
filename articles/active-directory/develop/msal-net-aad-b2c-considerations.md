...
- 정책을 적용 하려는 경우 `AcquireTokenInteractive` `authority` 매개 변수를 포함 하는의 재정의를 호출 해야 합니다.
...
사용할 `https://{azureADB2CHostname}/tfp/{tenant}/{policyName}` 기관은 다음과 같습니다.

- `azureADB2CHostname`는 Azure AD B2C 테 넌 트와 호스트 (예: `{your-tenant-name}.b2clogin.com`)의 이름입니다.
- `tenant`Azure AD B2C 테 넌 트의 전체 이름 (예: `{your-tenant-name}.onmicrosoft.com`) 또는 테 넌 트의 GUID입니다.
- `policyName`적용할 정책 또는 사용자 흐름의 이름 (예를 들어, 등록/로그인에 대 한 "b2c_1_susi").
...
- `policy`이전 문자열 중 하나입니다 (예를 들어 `PolicySignUpSignIn`).
- `ParentActivityOrWindow`는 Android (작업)에 필요 하며, windows의 windows 및 iOS의 UIViewController와 같이 부모 UI를 지 원하는 다른 플랫폼의 경우 선택 사항입니다. [UI 대화 상자에서](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/wiki/Acquiring-tokens-interactively#withparentactivityorwindow)자세한 내용을 참조 하세요.
...
을 호출 `AcquireTokenInteractive`하 여 정책 또는 사용자 흐름 (예: 최종 사용자가 프로필을 편집 하거나 암호를 다시 설정 하도록 허용)을 적용 하는 것이 현재 수행 됩니다. 이러한 두 정책의 경우 반환 된 토큰/인증 결과를 사용 하지 않습니다.

## <a name="special-case-of-editprofile-and-resetpassword-policies"></a>EditProfile 및 ResetPassword 정책의 특수 한 경우

최종 사용자가 소셜 id를 사용 하 여 로그인 한 다음 프로필을 편집 하는 환경을 제공 하려는 경우 Azure AD B2C 프로필 편집 정책을 적용 합니다. 이 작업을 수행 하는 방법은 해당 `AcquireTokenInteractive` 정책에 대 한 특정 권한을 사용 하 여를 호출 하 고 `Prompt.NoPrompt` , 사용자가 이미 로그인 되어 있고 활성 쿠키 세션이 있으므로 계정 선택 대화 상자가 표시 되지 않도록 하는 프롬프트를로 설정 하는 것입니다.
...
`IPublicClientApplication`에는 메서드가 포함 되어 있습니다.
```csharp
...
또는 [B2C 사용자 지정 정책을](https://aka.ms/ief)사용 `tid` 하는 경우 응용 프로그램에 추가 클레임을 반환 하는 기능을 제공 하므로 클레임을 사용할 수 있습니다. [클레임 변환](/azure/active-directory-b2c/claims-transformation-technical-profile) 에 대 한 자세한 정보
