...
이 빠른 시작에서는 JavaScript SPA(단일 페이지 응용 프로그램)에서 개인 계정, 회사 및 학교 계정에 로그인하고, 액세스 토큰을 가져오고, Microsoft Graph API를 호출할 수 있는 방법을 보여주는 코드 샘플을 사용하는 방법을 알아보겠습니다.
...
* [웹 서버(예: Node.js)에 대한 주요 프로젝트 파일 다운로드](https://github.com/Azure-Samples/active-directory-javascript-graphapi-v2/archive/core.zip)
...
> [!div renderon="docs"]
> `msalconfig.js`를 편집하고 `Enter_the_Application_Id_here`를 방금 등록한 앱의 응용 프로그램 ID로 바꿉니다. *개요* 페이지에서 *응용 프로그램 ID*를 찾을 수 있습니다.

> [!div class="sxs-lookup" renderon="portal"]
> `msalconfig.js`를 편집하고 msalconfig으로 바꿉니다.
...
```
> [!NOTE]
> [Visual Studio 2017](https://visualstudio.microsoft.com/downloads/)를 사용하는 경우 리디렉션 URI은 코드 샘플의 프로젝트에 구성된 대로 `http://localhost:30662/`로 설정됩니다. [Node.js](https://nodejs.org/en/download/) 또는 다른 웹 서버를 사용하는 경우 리디렉션 URI를 `http://localhost:30662/`로 설정한 다음, 이 포트에서 수신 대기를 시작하도록 서버를 구성합니다.
>

#### <a name="step-4-run-the-project"></a>4단계: 프로젝트 실행

Visual Studio를 사용하는 경우 **F5** 키를 눌러 프로젝트를 실행합니다.
...
Node.js를 사용하는 경우 명령줄에서는 프로젝트의 디렉터리에서 다음을 실행하여 서버를 시작할 수 있습니다.
 ...
 node server.js
 ```
웹 브라우저를 열고 `http://localhost:30662/`로 이동합니다. **Microsoft Graph API 호출** 단추를 클릭하여 로그인하기 시작합니다.
...
> |`loginCallBack`| 인증이 앱으로 다시 리디렉션된 후에 호출되는 콜백 메서드|
> |`redirectUri`  |Azure AD로 인증한 후에 다시 사용자를 리디렉션하는 URL|
...
> |Where  |  |
> |---------|---------|
> | `graphAPIScopes`   | (선택 사항) 로그인 시 사용자 동의를 요청하는 범위가 포함됩니다(예: Microsoft Graph의 경우 `[ "user.read" ]` 또는 사용자 지정 Web API의 경우 `[ "api://<Application ID>/access_as_user" ]`). |

> [!TIP]
> 또는 `loginPopup` 메서드를 사용하여 사용자에게 로그인하라는 팝업 창을 표시할 수 있습니다.
...
> |Where  |  |
> |---------|---------|
> | `graphAPIScopes`   | API에 대한 액세스 토큰에서 반환되도록 요청되는 범위가 포함됩니다(예: Microsoft Graph의 경우 `[ "user.read" ]` 또는 사용자 지정 Web API의 경우 `[ "api://<Application ID>/access_as_user" ]`). |
...
`acquireTokenRedirect(scope)`를 호출하면 사용자가 Azure AD v2.0 엔드포인트(또는 팝업 창의 `acquireTokenPopup(scope)` 결과)로 리디렉션되며, 여기에서 사용자는 해당 자격 증명을 확인하거나 필수 리소스에 동의하거나, 2단계 인증을 완료하여 상호 작용해야 합니다.
