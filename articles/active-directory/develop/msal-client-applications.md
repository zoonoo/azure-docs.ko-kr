...
> [!NOTE]
> MSAL.js에는 공용 및 기밀 클라이언트 앱이 분리 되지 않습니다.  MSAL.js은 클라이언트 앱을 사용자 에이전트 기반 앱으로 나타내며, 클라이언트 코드가 웹 브라우저와 같은 사용자 에이전트에서 실행 되는 공용 클라이언트입니다. 브라우저 컨텍스트는 공개적으로 액세스할 수 있기 때문에 이러한 클라이언트는 비밀을 저장 하지 않습니다.
