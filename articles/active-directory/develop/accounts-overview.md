...
---
# <a name="accounts--tenant-profiles-android"></a>계정 & 테 넌 트 프로필 (Android)

이 문서에서는 Microsoft id 플랫폼에 `account`이 무엇 인지에 대 한 개요를 제공 합니다.
...
- 계정에는 기관 또는 레코드 시스템의 원본이 있습니다. 계정이 만들어지고 해당 계정과 연결 된 자격 증명이 저장 된 시스템입니다. Microsoft id 플랫폼과 같은 다중 테 넌 트 시스템에서 레코드 시스템은 계정이 만들어진 @no__t 0입니다. 이 테 넌 트를 `home tenant`이 라고도 합니다.
...
> [!NOTE]
> MSAL은 Microsoft 계정 시스템 (Live, MSA)을 Microsoft id 플랫폼 내의 다른 테 넌 트로 처리 합니다. Microsoft 계정 테 넌 트의 테 넌 트 id는 `9188040d-6c67-4c5b-b112-36a304b66dad`입니다.
...
- 계정 `bob@contoso.com`은 온-프레미스 Windows Server Active Directory (원본 온-프레미스 시스템 레코드)에 생성 됩니다.
- 계정 `tom@live.com`은 Microsoft 계정 테 넌 트에 생성 됩니다.
- `bob@contoso.com`은 다음 Azure Active Directory 테 넌 트에 있는 하나 이상의 리소스에 액세스할 수 있습니다.
  - contoso.com (레코드의 클라우드 시스템-온-프레미스 시스템에 연결 된 레코드의 클라우드 시스템)
  - fabrikam.com
  - woodgrovebank.com
  - @No__t-0에 대 한 테 넌 트 프로필은 이러한 각 테 넌 트에 있습니다.
- `tom@live.com`은 다음 Microsoft 테 넌 트의 리소스에 액세스할 수 있습니다.
  - contoso.com
  - fabrikam.com
  - @No__t-0에 대 한 테 넌 트 프로필은 이러한 각 테 넌 트에 있습니다.
- 다른 테 넌 트의 Tom 및 Bob에 대 한 정보는 레코드 시스템의 정보에 따라 다를 수 있습니다. 작업 제목, 사무실 위치 등의 특성에 따라 달라질 수 있습니다. 각 조직 (Azure Active Directory 테 넌 트) 내에 있는 그룹 및/또는 역할의 멤버일 수 있습니다. @No__t-0 테 넌 트 프로필로이 정보를 참조 합니다.

다이어그램에서 bob@contoso.com 및 tom@live.com은 다른 Azure Active Directory 테 넌 트의 리소스에 액세스할 수 있습니다. 자세한 내용은 [Azure Portal에서 B2B 공동 작업 사용자 추가 Azure Active Directory](https://docs.microsoft.com/azure/active-directory/b2b/add-users-administrator)를 참조 하세요.
...
ID 토큰에는 클레임 목록이 포함 되어 있습니다. `Claims`은 계정에 대 한 이름/값 쌍 이며 요청을 만드는 데 사용 됩니다.
...
계정 개체에 노출 되는 클레임은 항상 계정에 대 한 ' home 테 넌 트 '/{authority}의 클레임입니다. 해당 계정이 홈 테 넌 트에 대 한 토큰을 요청 하는 데 사용 되지 않은 경우 MSAL은 계정 개체를 통해 클레임을 제공할 수 없습니다.  예를 들어 다음과 같은 가치를 제공해야 합니다.
...
> [!TIP]
> Id_token에 추가 클레임을 포함 하려면 @no__t에서 선택적 클레임 설명서를 참조 하세요. Azure AD 앱에 선택적 클레임 제공 @ no__t-0

### <a name="access-tenant-profile-claims"></a>테 넌 트 프로필 클레임 액세스

다른 테 넌 트에 표시 되는 계정에 대 한 클레임에 액세스 하려면 먼저 계정 개체를 `IMultiTenantAccount`으로 캐스팅 해야 합니다. 모든 계정은 다중 테 넌 트 일 수 있지만 MSAL을 통해 사용할 수 있는 테 넌 트 프로필 수는 현재 계정을 사용 하 여 토큰을 요청한 테 넌 트를 기반으로 합니다.  예를 들어 다음과 같은 가치를 제공해야 합니다.
...
즉, MSAL의 경우 다른 B2C 정책을 사용 하 여 토큰을 획득 하는 경우 이러한 토큰은 각각 고유한 식별자를 가진 별도의 계정으로 취급 됩니다. @No__t-0을 사용 하 여 토큰을 요청 하는 계정을 사용 하려는 경우 토큰 요청에 사용 하는 정책과 일치 하는 계정 목록에서 계정을 선택 해야 합니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
