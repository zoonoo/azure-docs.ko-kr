...
2. Azure AD 테넌트에 새 애플리케이션을 만들고 결과 프롬프트에서 **네이티브 클라이언트 애플리케이션**을 선택합니다.
  * **이름**은 사용자에게 응용 프로그램을 설명합니다.
  * **리디렉션 URI** 값으로 `http://TodoListClient/`를 입력합니다.

3. 등록을 완료한 후에는 Azure AD가 사용자 앱에 고유한 애플리케이션 ID를 할당합니다. 이 값은 다음 단계에서 필요하므로 응용 프로그램 페이지에서 복사해 둡니다.
