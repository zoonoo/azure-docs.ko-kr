---
title: Active Directory 연결된 서비스(Visual Studio) 사용
...
Azure AD(Azure Active Directory)를 사용하여 ASP.NET MVC 웹 애플리케이션을 위한 SSO(Single Sign-on) 또는 웹 API 서비스의 Active Directory 인증을 지원할 수 있습니다. Azure AD 인증을 사용하면 사용자가 Azure Active Directory의 자신의 계정을 사용하여 웹 애플리케이션에 연결할 수 있습니다. 웹 API에서 사용하는 Azure AD 인증의 장점으로 웹 애플리케이션에서 API를 표시하는 경우 데이터 보안이 향상되는 것을 들 수 있습니다. Azure AD를 사용하면, 자체 계정 및 사용자 관리를 사용하는 별도 인증 시스템을 관리할 필요가 없습니다.
...
- **Visual Studio 2015** 이상 [지금 Visual Studio를 다운로드하세요](https://aka.ms/vsdownload?utm_source=mscom&utm_campaign=msdocs).
...
1. **Single Sign-On** 페이지의 **도메인** 드롭다운 목록에서 도메인을 선택합니다. 이 목록에는 Visual Studio의 계정 설정 대화 상자(**파일 > 계정 설정...** )에 나열된 계정으로 액세스할 수 있는 모든 도메인이 포함되어 있습니다. 원하는 도메인을 찾지 못한 경우 `mydomain.onmicrosoft.com` 같은 도메인 이름을 입력할 수도 있습니다. Azure Active Directory 앱을 만들거나 기존 Azure Active Directory 앱의 설정을 사용하는 옵션을 선택할 수 있습니다. 완료되면 **다음** 을 선택합니다.
