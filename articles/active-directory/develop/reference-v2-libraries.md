...
ms.locfileid: "77611947"
---
# <a name="microsoft-identity-platform-authentication-libraries"></a>Microsoft ID 플랫폼 인증 라이브러리

[Microsoft ID 플랫폼 엔드포인트는](active-directory-v2-compare.md) 업계 표준 OAuth 2.0 및 OpenID Connect 1.0 프로토콜을 지원합니다. MSAL(Microsoft 인증 라이브러리)은 Microsoft ID 플랫폼 끝점에서 작동하도록 설계되었습니다. OAuth 2.0 및 OpenID Connect 1.0을 지원하는 오픈 소스 라이브러리를 사용할 수도 있습니다.

SDL(보안 개발 수명 주기) 방법을 따르는 프로토콜 도메인 전문가가 작성한 라이브러리를 사용하는 것이 좋습니다. 이러한 방법론에는 [Microsoft에서 따르는 방법이][Microsoft-SDL]포함됩니다. 프로토콜에 대한 코드를 직접 사용하는 경우 Microsoft SDL과 같은 방법을 따라야 합니다. 각 프로토콜의 표준 사양의 보안 고려 사항에 주의를 기울이십시오.

> [!NOTE]
> Azure Active 디렉터리 인증 라이브러리(ADAL)를 찾고 계십니까? [ADAL 라이브러리 가이드를](../azuread-dev/active-directory-authentication-libraries.md)확인하십시오.

## <a name="types-of-libraries"></a>라이브러리 유형

Microsoft ID 플랫폼 끝점은 두 가지 유형의 라이브러리에서 작동합니다.

* **클라이언트 라이브러리**: 네이티브 클라이언트와 서버는 클라이언트 라이브러리를 사용하여 Microsoft Graph와 같은 리소스를 호출하기 위한 액세스 토큰을 획득합니다.
...
* **호환**: Microsoft는 기본 시나리오에서 이러한 라이브러리를 테스트했으며 Microsoft ID 플랫폼 끝점에서 작동한다는 것을 확인했습니다. Microsoft는 이러한 라이브러리에 대한 수정 프로그램을 제공하지 않으며 이러한 라이브러리에 대한 검토를 수행하지 않았습니다. 문제 및 기능 요청은 라이브러리의 오픈 소스 프로젝트로 리디렉션되어야 합니다.

Microsoft ID 플랫폼 끝점에서 작동하는 라이브러리 목록은 다음 섹션을 참조하십시오.
...
| 아이폰 OS & 맥 OS | MSAL iOS 및 맥OS | [GitHub](https://github.com/AzureAD/microsoft-authentication-library-for-objc) |[GitHub](https://github.com/AzureAD/microsoft-authentication-library-for-objc) | [iOS 앱,](https://github.com/Azure-Samples/ms-identity-mobile-apple-swift-objc) [맥OS 앱](https://github.com/Azure-Samples/ms-identity-macOS-swift-objc) | [참조](https://azuread.github.io/microsoft-authentication-library-for-objc/index.html)  | [개념 문서](msal-overview.md) | |
|![Android/Java](media/sample-v2-code/logo_Android.png) | MSAL 안드로이드 | [중앙 리포지토리](https://repo1.maven.org/maven2/com/microsoft/identity/client/msal/) |[GitHub](https://github.com/AzureAD/microsoft-authentication-library-for-android) | [안드로이드 앱](quickstart-v2-android.md) | [JavaDocs](https://javadoc.io/doc/com.microsoft.identity.client/msal) | [개념 문서](msal-overview.md) |[로드맵](https://github.com/AzureAD/microsoft-authentication-library-for-android/wiki/Roadmap)
...
## <a name="microsoft-supported-libraries-by-os--language"></a>OS / 언어에 의해 마이크로 소프트 지원 라이브러리

지원되는 운영 체제와 언어의 경우 매핑은 다음과 같은 것입니다.

|             | Windows    | Linux      | macOS      | iOS | Android    |
|-------------|------------|------------|------------|------------|------------|
| ![JavaScript](media/sample-v2-code/logo_js.png)  |  MSAL.js | MSAL.js | MSAL.js | MSAL.js |  MSAL.js |
| <img alt="C#" src="../../cognitive-services/speech-service/media/index/logo_csharp.svg" width="64px" height="64px" /> | ASP.NET, ASP.NET 코어, MSAL.Net (.NET FW, 코어, UWP)| ASP.NET 코어, MSAL.Net (.NET 코어) | ASP.NET 코어, MSAL.Net (맥 OS)       | MSAL.Net (자마린.iOS) | MSAL.Net (자마린.안드로이드)|
| Swift <br> Objective-C |            |            | [iOS 및 macOS용 MSAL](msal-overview.md) | [iOS 및 macOS용 MSAL](msal-overview.md) |            |
| ![Java](media/sample-v2-code/logo_java.png) Java | msal4j | msal4j | msal4j | | MSAL 안드로이드 |
| ![Python](media/sample-v2-code/logo_python.png) Python | MSAL Python | MSAL Python | MSAL Python |
| ![Node.js](media/sample-v2-code/logo_nodejs.png) Node.JS | 여권.노드 | 여권.노드 | 여권.노드 |

[지원되는 플랫폼 및 언어별 시나리오](authentication-flows-app-scenarios.md#scenarios-and-supported-platforms-and-languages) 참조
...
| ![Java](media/sample-v2-code/logo_java.png) | [글루오픈ID 커넥트 라이브러리](https://github.com/GluuFederation/oxAuth) | [버전 3.0.2](https://github.com/GluuFederation/oxAuth/releases/tag/3.0.2) | [글루오픈ID 커넥트 라이브러리](https://github.com/GluuFederation/oxAuth) | |
| ![Python](media/sample-v2-code/logo_python.png) | [요청 -오아울리브](https://github.com/requests/requests-oauthlib) | [버전 1.2.0](https://github.com/requests/requests-oauthlib/releases/tag/v1.2.0) | [요청 -오아울리브](https://github.com/requests/requests-oauthlib) | |
| ![Node.js](media/sample-v2-code/logo_nodejs.png) | [오픈 리드 클라이언트](https://github.com/panva/node-openid-client) | [버전 2.4.5](https://github.com/panva/node-openid-client/releases/tag/v2.4.5) | [오픈 리드 클라이언트](https://github.com/panva/node-openid-client) | |
...
| ![Ruby](media/sample-v2-code/logo_ruby.png) |[OmniAuth](https://github.com/omniauth/omniauth/wiki) |옴니아우트: 1.3.1<br />옴니아우트-오트2: 1.4.0 |[OmniAuth](https://github.com/omniauth/omniauth)<br />[OmniAuth OAuth2](https://github.com/intridea/omniauth-oauth2) |  |
| 아이폰 OS, 맥 OS, & 안드로이드  | [네이티브 앱 인증 반응](https://github.com/FormidableLabs/react-native-app-auth) | [버전 4.2.0](https://github.com/FormidableLabs/react-native-app-auth/releases/tag/v4.2.0) | [네이티브 앱 인증 반응](https://github.com/FormidableLabs/react-native-app-auth) | |

표준을 준수하는 모든 라이브러리의 경우 Microsoft ID 플랫폼 끝점을 사용할 수 있습니다. 지원을 위해 어디로 가야 할지 아는 것이 중요합니다.

* 라이브러리 코드에서 문제 및 새 기능 요청은 라이브러리 소유자에게 문의합니다.
* 서비스 측 프로토콜 구현에서 문제 및 새 기능 요청은 Microsoft로 문의합니다.
* 프로토콜에 표시할 추가 기능에 대한 [기능 요청을 제출합니다.](https://feedback.azure.com/forums/169401-azure-active-directory)
* Microsoft ID 플랫폼 끝점이 OAuth 2.0 또는 OpenID Connect 1.0을 준수하지 않는 문제가 있는 경우 [지원 요청을 만듭니다.](https://docs.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request)

## <a name="related-content"></a>관련 콘텐츠

Microsoft ID 플랫폼 끝점에 대한 자세한 내용은 [Microsoft ID 플랫폼 개요를][AAD-App-Model-V2-Overview]참조하십시오.
