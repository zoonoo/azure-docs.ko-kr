...
Broker support is enabled on a per-PublicClientApplication basis. 기본적으로 사용하지 않도록 설정되어 있습니다. Use the `WithBroker()` parameter (set to true by default) when you create the PublicClientApplication through the PublicClientApplicationBuilder.
