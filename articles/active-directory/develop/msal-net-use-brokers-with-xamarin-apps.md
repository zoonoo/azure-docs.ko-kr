...
description: Microsoft Authenticator 및 .NET 용 Microsoft 인증 라이브러리 (MSAL.NET)를 사용할 수 있는 Xamarin iOS 응용 프로그램을 설정 하는 방법에 대해 알아봅니다. 또한 .NET 용 Azure AD 인증 Library (ADAL.NET)에서 MSAL.NET (Microsoft Authentication Library for .NET)로 마이그레이션하는 방법에 대해 알아봅니다.
author: jmprieur
manager: CelesteDG
...
시스템 브라우저 및 Android 11 지원에 대 한 응용 프로그램을 구성 하는 방법에 대 한 자세한 내용은 [시스템 브라우저 지원에 대 한 android 매니페스트 업데이트](msal-net-xamarin-android-considerations.md#update-the-android-manifest-for-system-webview-support)를 참조 하세요.
...
- **로그** -조정 된 인증에 문제가 발생 하는 경우 broker의 로그를 보면 원인을 진단 하는 데 도움이 될 수 있습니다.
  - Microsoft Authenticator 로그 가져오기:
    ...
    1. **사용자 의견 보내기**  >  를 선택 **하는 데 문제가 있나요?** 를 선택 합니다.
    1. **수행** 하려는 작업에서 옵션을 선택 하 고 설명을 추가 합니다.
    1. 로그를 보내려면 앱의 오른쪽 위 모퉁이에 있는 화살표를 선택 합니다.

    로그를 보낸 후에 인시던트 ID가 대화 상자에 표시 됩니다. 인시던트 ID를 기록 하 고 지원을 요청할 때 포함 합니다.

  - Intune 회사 포털 로그 가져오기:

    1. 앱의 왼쪽 위 모퉁이에 있는 메뉴 단추를 선택 합니다.
    1. **도움말**  >  **전자 메일 지원** 을 선택 합니다.
    1. 로그를 보내려면 **로그만 업로드** 를 선택 합니다.
