...
ms.locfileid: "39596503"
---
# <a name="azure-active-directory-v20-tokens-reference"></a>Azure Active Directory v2.0 토큰 참조
Azure AD(Azure Active Directory) v2.0 엔드포인트는 각 [인증 흐름](v2-app-types.md)에서 여러 형식의 보안 토큰을 내보냅니다. 이 참조에서는 각 토큰 유형의 형식, 보안 특성 및 내용을 설명합니다.

> [!NOTE]
> v2.0 엔드포인트에서는 일부 Azure Active Directory 시나리오 및 기능만 지원합니다.  v2.0 끝점을 사용해야 하는지 확인하려면 [v2.0 제한 사항](active-directory-v2-limitations.md)을 참조하세요.
>
>

## <a name="types-of-tokens"></a>토큰 형식
v2.0 끝점은 액세스 토큰 및 새로 고침 토큰을 사용하는 [OAuth 2.0 인증 프로토콜](active-directory-v2-protocols.md)을 지원합니다. 또한 [OpenID Connect](active-directory-v2-protocols.md)를 통한 인증 및 로그인을 지원합니다. OpenID Connect는 세 번째 형식의 토큰인 ID 토큰을 도입합니다. 이러한 토큰은 각각 *전달자* 토큰으로 표시됩니다.
...
샘플 ID 토큰 이외에 다음 섹션에도 ID 토큰의 클레임에 대한 자세한 내용이 나와 있습니다. ID 토큰의 클레임은 특정 순서로 반환되지 않습니다. 또한 언제든지 새 클레임을 ID 토큰에 도입할 수 있습니다. 샘플 ID 토큰 이외에 다음 섹션에도 ID 토큰의 클레임에 대한 자세한 내용이 나와 있습니다. 다음 목록에는 앱이 현재 안정적으로 해석할 수 있는 클레임이 포함되어 있습니다. 자세한 내용은 [OpenID Connect 사양](http://openid.net/specs/openid-connect-core-1_0.html)에서 확인할 수 있습니다.
...
새로 고침 토큰은 영구적으로 앱에 완전히 불투명합니다. 새로 고침 토큰은 Azure AD v2.0 끝점에서 발급되며 Azure AD v2.0 끝점에서만 검사되고 해석됩니다. 또한 새로 고침 토큰은 수명이 길지만 일정 기간 지속될 것으로 예상하도록 앱을 작성해서는 안 됩니다. 다양한 이유로 언제든지 새로 고침 토큰이 무효화될 수 있기 때문입니다. 자세한 내용은 [토큰 해지](v1-id-and-access-tokens.md#token-revocation)를 참조하세요. 앱에서 새로 고침 토큰이 유효한지 확인하는 유일한 방법은 v2.0 끝점에 대한 토큰 요청을 수행하여 교환하는 것입니다.
