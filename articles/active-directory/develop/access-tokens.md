...
| `preferred_username` | 문자열 | 사용자를 나타내는 기본 사용자 이름입니다. 메일 주소, 전화 번호 또는 지정된 형식이 없는 일반 사용자 이름일 수 있습니다. 해당 값은 변경 가능하며 시간이 지남에 따라 변경될 수 있습니다. Mutable 이므로이 값 권한 부여 결정을 내릴 수 하지 사용 해야 합니다.  사용자 이름 힌트에 사용할 수 있습니다. `profile` 범위는이 클레임을 수신 해야 합니다. |
| `name` | 문자열 | 사람이 인식할 수 있으며 토큰의 주체를 식별하는 값을 제공합니다. 값을 고유 하 게 보장 되지 않습니다 하 고는 변경할 수 표시 용도로 사용 하도록 설계 했습니다. `profile` 범위는이 클레임을 수신 해야 합니다. |
...
| `unique_name` | 문자열 | v1.0 토큰에만 제공됩니다. 토큰의 주체를 식별하는, 사람이 인식할 수 있는 값을 제공합니다. 이 값은 테 넌 트 내에서 고유 하 게 보장 되지 않습니다 및 표시 용도로 사용 해야 합니다. |
