...
| 클레임 | 형식 | Description |
|-----|--------|-------------|
| `aud` | 문자열, 앱 ID URI, GUID | 토큰의 의도된 수신자(대상)를 식별합니다.  API는 이 값의 유효성을 검사하고 값이 일치하지 않을 경우 토큰을 거부해야 합니다. v2.0 토큰에서 이는 항상 API의 클라이언트 ID이고, v1.0 토큰에서는 클라이언트가 토큰을 요청한 방법에 따라 요청에 사용되는 클라이언트 ID 또는 리소스 URI가 될 수 있습니다.|
