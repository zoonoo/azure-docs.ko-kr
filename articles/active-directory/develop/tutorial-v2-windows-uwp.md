...
이 가이드의 끝에서 애플리케이션은 개인 계정을 사용하여 보호된 API를 호출합니다. outlook.com, live.com 및 기타를 예로 듭니다. 또한 응용 프로그램은 Azure Active Directory가 있는 모든 회사 또는 조직의 회사 및 학교 계정을 호출합니다.
...
![이 자습서에서 생성된 샘플 앱의 작동 방식 표시](./media/tutorial-v2-windows-uwp/uwp-intro-updated.png)
...
`AcquireTokenAsync` 메서드를 호출하면 사용자에게 로그인하라는 창이 표시됩니다. 처음으로 보호된 리소스에 액세스할 때 애플리케이션에서는 사용자가 일반적으로 대화형으로 로그인하도록 요청합니다. 토큰을 획득하는 자동 작업에 실패한 경우 로그인해야 할 수도 있습니다. 사용자의 암호가 만료된 경우를 예로 듭니다.
