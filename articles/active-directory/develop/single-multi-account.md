...
클래스를 사용 하면 한 `SingleAccountPublicClientApplication` 번에 하나의 계정만 로그인 할 수 있는 MSAL 기반 앱을 만들 수 있습니다. `SingleAccountPublicClientApplication` 와 `PublicClientApplication` 의 차이점은 다음과 같습니다.
...
다음 의사 코드에서는를 사용 하는 방법을 보여 줍니다 `SingleAccountPublicClientApplication` .
...
클래스는 여러 계정을 동시에 로그인 할 수 있도록 하 `MultipleAccountPublicClientApplication` 는 MSAL 기반 응용 프로그램을 만드는 데 사용 됩니다. 다음과 같이 계정을 가져오고 추가 하 고 제거할 수 있습니다.

### <a name="add-an-account"></a>계정 추가

한 번 이상 호출 하 여 응용 프로그램에서 하나 이상의 계정을 사용 `acquireToken` 합니다.  
...
계정 식별자를 사용 하 여를 호출 하 여 계정을 제거 `removeAccount` 합니다.

Broker를 사용 하도록 앱을 구성 하 고 장치에 broker를 설치 하는 경우를 호출할 때 broker에서 계정이 제거 되지 않습니다 `removeAccount` .  클라이언트와 연결 된 토큰만 제거 됩니다.
