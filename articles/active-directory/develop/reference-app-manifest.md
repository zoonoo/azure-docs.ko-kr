...
| `availableToOtherTenants` | Boolean | 응용 프로그램; 다른 테 넌 트와 공유 될 경우 true 그렇지 않으면 false입니다. <br><br> _참고: 이 바뀝니다 `signInAudience` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | |
...
| `displayName` | String | 앱의 표시 이름입니다. <br><br> _참고: 이 바뀝니다 `name` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | `"MyRegisteredApp"` |
...
| `homepage` | String | 응용 프로그램 홈페이지의 URL입니다. <br><br> _참고: 이 바뀝니다 `signInUrl` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | `"https://MyRegisteredApp"` |
| `objectId` | String | 디렉터리의 앱에 대한 고유 식별자입니다. <br><br> _참고: 이 바뀝니다 `id` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | 이 응용 프로그램이 공용 클라이언트 (예: 모바일 장치에서 실행 중인 설치 된 응용 프로그램) 인지 여부를 지정 합니다. <br><br> _참고: 이 바뀝니다 `allowPublicClient` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | |
| `publisherDomain` | String | 응용 프로그램에 대 한 확인 된 게시자 도메인입니다. 읽기 전용입니다. | https://www.contoso.com |
| `replyUrls` | 문자열 배열 | 이 다중값 속성은 Azure AD가 토큰을 반환할 때 대상으로 허용하는 등록된 redirect_uri 값 목록을 포함합니다. <br><br> _참고: 이 바뀝니다 `replyUrlsWithType` 에 [앱 등록 (미리 보기)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) 발생 합니다._ | |
...
응용 프로그램 매니페스트를 Azure AD에서 기본 응용 프로그램 모델의 스키마를 나타냅니다. 기본 스키마가 진화 매니페스트 편집기에서 새 스키마를 반영 하도록 업데이트 됩니다. 결과적으로, 응용 프로그램 매니페스트에 표시 하는 새 특성을 볼 수 있습니다. 드물긴 하지만 기존 특성 구문 또는 의미 체계 변경 나타날 수 있습니다 또는 이전에 존재 하는 특성을 더 이상 지원 되지 않습니다 하는 것이 있습니다. 새 특성을 예를 들어 표시 됩니다는 [앱 등록 (미리 보기)](https://developer.microsoft.com/graph/blogs/new-app-registration/) 앱 등록 (GA) 환경에서 다른 이름으로 알려진 합니다.
...
1. 나중에 사용할 소스 리포지토리에 매니페스트를 저장 해야 하는 워크플로, 경우에 표시 한 리포지토리에 저장 된 매니페스트를 기준 주소 다시 지정는 것이 좋습니다 합니다 **앱 등록 (미리 보기)** 경험 합니다.
