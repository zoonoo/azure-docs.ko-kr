---
title: Microsoft ID 플랫폼에 등록된 응용 프로그램에서 지원하는 계정 수정 | Azure
description: Microsoft ID 플랫폼에 등록된 응용 프로그램을 구성하여 응용 프로그램에 액세스할 수 있는 계정 또는 사용자를 변경합니다.
...
---
# <a name="quickstart-modify-the-accounts-supported-by-an-application-preview"></a>빠른 시작: 응용 프로그램에서 지원되는 계정 수정(미리 보기)

Microsoft ID 플랫폼에서 응용 프로그램을 등록하는 경우 조직 내의 사용자만 해당 응용 프로그램에 액세스할 수 있도록 제한하는 것이 좋습니다. 또는 응용 프로그램을 외부 조직의 사용자나, 꼭 조직의 구성원은 아닌 사용자가 액세스할 수 있게 할 수 있습니다.
...
1. 다음 단계에 따라 [다른 계정을 지원하기 위해 응용 프로그램을 변경](#change-the-application-registration-to-support-different-accounts)합니다.
1. 단일 페이지 응용 프로그램인 경우 [OAuth 2.0 암시적 허용](#enable-oauth-20-implicit-grant-for-single-page-applications)을 사용합니다.
...
1. 앱 **개요** 페이지에서 **인증** 섹션을 선택하고 **지원되는 계정 유형**에서 선택한 값을 변경합니다.
    * 업무용(LOB) 응용 프로그램을 빌드하는 경우 **이 디렉터리의 계정에만 해당**을 선택합니다. 이 옵션은 응용 프로그램이 디렉터리에 등록되지 않은 경우에는 사용할 수 없습니다.
...
사용자가 승인하면 이 동일한 인증 프로토콜을 사용하여 클라이언트와 응용 프로그램에 대해 구성된 다른 웹 API 리소스 간의 보안 호출을 위해 토큰을 가져올 수 있습니다. 암시적 허용에 관한 자세한 내용을 확인하고 자신의 응용 프로그램 시나리오에 적절한지 판단하려면 Azure AD [v1.0](v1-oauth2-implicit-grant-flow.md) 및 [v2.0](v2-oauth2-implicit-grant-flow.md)의 OAuth 2.0 암시적 허용 흐름에 대해 알아보세요.

기본적으로 응용 프로그램에 대해 OAuth 2.0 암시적 허용이 사용되지 않도록 설정됩니다. 아래 개요의 단계에 따라 응용 프로그램에 대해 OAuth 2.0 암시적 허용을 사용할 수 있습니다.
