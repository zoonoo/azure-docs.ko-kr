...
ms.locfileid: "68693998"
---
# <a name="permissions-and-consent-in-the-microsoft-identity-platform-endpoint"></a>Microsoft id 플랫폼 끝점에서 사용 권한 및 동의
...
Openid connect Connect의 Microsoft identity platform 구현에는 `openid`특정 리소스에 적용 되지 않는 몇 가지 잘 정의 된 범위 `profile`(, `email`, 및 `offline_access`)가 있습니다. `address` 및 `phone` OpenID Connect 범위는 지원되지 않습니다.

### <a name="openid"></a>openid

앱이 [OpenID Connect](active-directory-v2-protocols.md)를 사용하여 로그인을 수행하는 경우 `openid` 범위를 요청해야 합니다. `openid` 범위는 작업 계정 동의 페이지에 "로그인" 권한으로 표시되고 Microsoft 계정 동의 페이지에 "Microsoft 계정을 사용하여 프로필 보기 및 앱과 서비스에 연결" 권한으로 표시됩니다. 이 사용 권한을 통해 앱은 `sub` 클레임 형식으로 사용자에 대한 고유 식별자를 받을 수 있습니다. 또한 앱이 UserInfo 엔드포인트에 액세스할 수 있도록 해줍니다. Microsoft id 플랫폼 토큰 끝점에서 범위를사용하여인증을위해앱에서사용할수있는id토큰을얻을수있습니다.`openid`
...
관리자 동의는 범위 매개 변수를 인정하지 않으므로 요청되는 사용 권한을 애플리케이션 등록에서 정적으로 정의해야 합니다. 일반적으로 지정 된 응용 프로그램에 대해 정적으로 정의 된 사용 권한은 동적/증분으로 요청 하는 사용 권한의 상위 집합 인지 확인 하는 것이 좋습니다.
...
`/.default` 범위를 사용 하 여 v2.0 끝점에서 Microsoft id 플랫폼 끝점으로 앱을 마이그레이션할 수 있습니다. 이것은 애플리케이션 등록 시 구성된 정적 사용 권한 목록을 참조하는 모든 애플리케이션에 대한 기본 제공 범위입니다. `scope` 값 `https://graph.microsoft.com/.default`는 기본적으로 v1.0 엔드포인트 `resource=https://graph.microsoft.com`과 같습니다. 즉, Azure Portal에서 애플리케이션이 등록된 Microsoft Graph의 범위를 사용하여 토큰을 요청합니다.

기본 범위는 모든 OAuth 2.0 흐름에서 사용할 수 있지만, [흐름](v2-oauth2-on-behalf-of-flow.md) 및 [클라이언트 자격 증명 흐름](v2-oauth2-client-creds-grant-flow.md)에서 필요 합니다.  
