...
ROPC 흐름은 단일 요청으로, 클라이언트 id와 사용자의 자격 증명을 IDP에 보낸 다음 반환 되는 토큰을 받습니다. 이렇게 하려면 먼저 클라이언트에서 사용자의 이메일 주소(UPN) 및 암호를 요청해야 합니다. 요청이 성공하는 즉시 클라이언트는 메모리에서 사용자의 자격 증명을 안전하게 해제해야 합니다. 이를 저장하면 안 됩니다.
