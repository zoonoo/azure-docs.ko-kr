...
사용자 할당을 사용하는 애플리케이션을 만드는 방법에는 두 가지가 있습니다. 한 방법에는 **전역 관리자** 역할이 필요 하지만 다른 방법에는 이 역할이 필요하지 않습니다.

### <a name="enterprise-applications-requires-the-global-administrator-role"></a>엔터프라이즈 애플리케이션(전역 관리자 역할 필요)
...
1. 목록에서 사용자 또는 보안 그룹을 할당할 애플리케이션을 선택합니다. 
    창 상단에 있는 필터를 사용하여 특정 애플리케이션을 검색합니다.
1. 애플리케이션 **개요** 페이지의 **관리** 에서 **속성** 을 선택합니다.
1. **사용자 할당이 필요합니까?** 설정을 찾아 **예** 로 설정합니다. 이 옵션을 **예** 로 설정하면 먼저 테넌트의 사용자를 이 애플리케이션에 할당해야 합니다. 할당하지 않으면 이 애플리케이션에 로그인할 수 없습니다.
1. **저장** 을 선택합니다.
...
1. 관리할 앱을 만들거나 선택합니다. 이 애플리케이션의 **소유자** 여야 합니다.
1. 애플리케이션 **개요** 페이지의 **기본 정보** 섹션에서 **로컬 디렉터리에서 관리되는 애플리케이션** 링크를 선택합니다.
1. **관리** 에서 **속성** 을 선택합니다.
1. **사용자 할당이 필요합니까?** 설정을 찾아 **예** 로 설정합니다. 이 옵션을 **예** 로 설정하면 먼저 테넌트의 사용자를 이 애플리케이션에 할당해야 합니다. 할당하지 않으면 이 애플리케이션에 로그인할 수 없습니다.
1. **저장** 을 선택합니다.
...
1. (선택 사항) 애플리케이션에서 앱 역할을 정의한 경우 **역할 선택** 옵션을 사용하여 선택한 사용자 및 그룹을 애플리케이션의 역할 중 하나에 할당할 수 있습니다. 
1. **할당** 을 선택하여 앱에 사용자와 및 그룹의 할당을 완료합니다. 
