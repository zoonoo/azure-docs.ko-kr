...
description: Safari 브라우저에서 JavaScript 용 Microsoft Authentication Library (MSAL.js)를 사용할 때 발생 하는 문제에 대해 알아봅니다.
...
---
# <a name="known-issues-on-safari-browser-with-msaljs"></a>MSAL.js 있는 Safari 브라우저의 알려진 문제 
...
MSAL.js는 숨겨진 Iframe을 사용 하 여 호출의 일부로 자동 토큰 획득 및 갱신을 수행 `acquireTokenSilent` 합니다. 자동 토큰 요청은 Azure AD에서 설정 된 쿠키가 나타내는 인증 된 사용자 세션에 액세스할 수 있는 Iframe을 사용 합니다. ITP 2.0를 사용 하 여 이러한 쿠키에 대 한 액세스를 차단 하면 MSAL.js 자동으로 토큰을 획득 하 고 갱신할 수 없으므로 `acquireTokenSilent` 오류가 발생 합니다.
...
`acquireTokenSilent`사용자에 게 로그인 하 라는 메시지를 표시 하는 대화형 획득 토큰 호출로 오류를 처리 해야 합니다.
반복 해 서 로그인을 방지 하기 위해 구현할 수 있는 방법은 오류를 처리 하 `acquireTokenSilent` 고 사용자에 게 Safari에서 ITP 설정을 사용 하지 않도록 설정 하는 옵션을 제공 하 여 대화형 호출을 진행 하는 것입니다. 설정을 사용 하지 않도록 설정 하면 후속 자동 토큰 갱신에 성공 해야 합니다.
