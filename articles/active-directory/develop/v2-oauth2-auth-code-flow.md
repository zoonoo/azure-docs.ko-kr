...
## <a name="setup-required-for-single-page-apps"></a>단일 페이지 앱에 필요한 설정

단일 페이지 애플리케이션에 대한 인증 코드 흐름에는 몇 가지 추가 설정이 필요합니다.  [애플리케이션을 만드는](howto-create-service-principal-portal.md) 동안 앱에 대한 리디렉션 URI를 `spa` 리디렉션 URI로 표시해야 합니다. 이렇게 하면 로그인 서버에서 앱에 대한 CORS(원본 간 리소스 공유)를 허용합니다.  XHR을 사용하여 코드를 사용하는 데 필요합니다.
...
> [!NOTE]
> 단일 페이지 앱에는 '단일 페이지 애플리케이션' 클라이언트 유형에 대해서만 원본 간 토큰 상환이 허용됨을 나타내는 `invalid_request` 오류가 표시될 수 있습니다.  이것은 토큰을 요청하는 데 사용되는 리디렉션 URI가 `spa` 리디렉션 URI로 표시되지 않았음을 나타냅니다.  이 흐름을 사용하도록 설정하는 방법에 대해서는 [애플리케이션 등록 단계](#setup-required-for-single-page-apps)를 검토합니다.
