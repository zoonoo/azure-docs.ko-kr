...
> [!NOTE]
> 로그인 사용자 이름과 암호가 일치 하지 않으면 로그인이 실패 하 고 위험 검색이 수행 되지 않습니다. 익숙하지 않은 위치에서의 로그인 위험 검색은이 성공적 이면 로그인에 대해서만 트리거됩니다.
...
알 수 없는 위치는 공격자가 도난당한 ID를 사용할 수 있다는 확실한 표시를 제공할 수 있습니다. 가양성은 사용자가 이동하거나, 새 디바이스를 사용하거나, 새 VPN을 사용할 때 발생할 수 있습니다. 이러한 가양성의 결과로이 이벤트 유형의 위험 수준은 **보통**입니다.
