...
|50005|사용자가 현재 조건부 액세스 정책을 통해 지원 되지 않는 플랫폼에서 장치에 로그인 하려고 했습니다.|
...
|50042|쌍으로 된 식별자를 생성 하는 데 필요한 솔트가 원칙에 없습니다. 테넌트 관리자에게 문의하세요.|
...
|50078|제공 된 다단계 인증이 만료 되었습니다 .에 액세스 하려면 multi-factor authentication을 새로 고쳐야 합니다.|
...
|50194|응용 프로그램 ' {appId} ' ({appName})이 (가) 다중 테 넌 트 응용 프로그램으로 구성 되어 있지 않습니다. ' {Time} ' 후에 생성 된 이러한 응용 프로그램에는/scommon 끝점을 사용할 수 없습니다. 테 넌 트 별 끝점을 사용 하거나 응용 프로그램을 다중 테 넌 트로 구성 합니다.|
...
|90094| 앱에서 로그인 한 사용자가 동의할 수 없는 권한을 요청 했으며 사용자가 차단 되었습니다. |
|90095| 앱에서 로그인 한 사용자가 동의할 수 없는 권한을 요청 했으며 사용자에 게 [관리자 동의 요청](../manage-apps/configure-admin-consent-workflow.md) 양식이 표시 되었습니다. |
|130500|사용자 자격 증명 정책으로 인해 전화 로그인이 차단 되었습니다.|
...
|500014|리소스 ' {identifier} '이 (가) 비활성화 되었습니다.|
...
|500172|' {Issuer} '에 의해 발급 된 인증서 ' {name} '이 (가) 잘못 되었습니다. 현재 시간: ' {curTime} '. Certificate NotBefore: ' {startTime} '. 인증서 NotAfter: ' {endTime} '.|
|501291|클라이언트 앱이 Mam 앱 이며 장치가 등록 되지 않고 broker를 사용 하 여 요청을 보냅니다. 앱에 액세스 하기 전에 장치를 등록 하려면 작업 장소 조인을 수행 해야 합니다.|
|530003|이 리소스에 액세스 하려면 장치를 관리 해야 합니다.|
...
|700051|응용 프로그램에 대해 ' token ' Response_type 사용 하도록 설정 되어 있지 않습니다. 응용 프로그램에서 response_type ' token '이 응용 프로그램에 대해 사용 하도록 설정 되지 않아 응용 프로그램에서 지원 되지 않는 응답 형식을 요청 했습니다. 응용 프로그램 소유자는 Azure Portal로 이동 하거나 MS Graph를 호출 하 여 암시적 액세스 토큰 부여를 사용 하도록 설정 해야 합니다.|
|900432|기밀 클라이언트는 클라우드 간 요청에서 지원 되지 않습니다.|
|5000811|SAML 토큰 서명을 확인할 수 없습니다. 서명 키 식별자가 유효한 등록 키와 일치 하지 않습니다.|
|7000215|잘못 된 클라이언트 암호를 제공 했습니다.|
