---
title: "Azure 가상 머신에 온-프레미스 Active Directory 도메인용 복제본 도메인 컨트롤러 설치 | Microsoft Docs"
description: "Azure 가상 네트워크의 Azure VM(가상 머신)에 온-프레미스 Active Directory 도메인용 복제본 DC를 설치하는 방법입니다."
...
이 문서에서는 Azure 가상 네트워크의 Azure VM(가상 머신)에 온-프레미스 Active Directory 도메인의 복제본 DC로 사용할 추가 DC(도메인 컨트롤러)를 설치하는 방법을 설명합니다. [Azure 가상 네트워크에 Windows Server Active Directory 포리스트를 설치](active-directory-new-forest-virtual-machine.md)할 수도 있습니다. Azure 가상 네트워크에 AD DS(Active Directory Domain Services)를 설치하는 방법은 [Azure Virtual Machines에 Windows Server Active Directory 배포에 대한 지침](https://msdn.microsoft.com/library/azure/jj156090.aspx)을 참조하세요.
...
DC 역할을 호스트할 VM을 만들려면 필요에 따라 [Azure Portal을 사용하여 Windows 가상 머신 만들기](../virtual-machines/windows/quick-create-portal.md)의 단계를 반복합니다. 내결함성과 중복성을 제공하려면 가상 DC를 두 개 이상 배포하세요. Azure 가상 네트워크에 비슷하게 구성된 DC가 두 개 이상 있는 경우 내결함성을 향상시키기 위해 해당 DC를 실행하는 VM을 가용성 집합에 배치할 수 있습니다.
...
2. 관리할 가상 네트워크를 연 다음 [가상 네트워크에 대한 DNS 서버 IP 주소를 다시 구성](../virtual-network/manage-virtual-network.md#change-dns-servers)하여 온-프레미스 DNS 서버의 IP 주소가 아닌 복제본 DC에 할당된 정적 IP 주소를 사용합니다.
