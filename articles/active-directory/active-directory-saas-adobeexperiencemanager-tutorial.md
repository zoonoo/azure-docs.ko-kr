...
- 사용자가 해당 Azure AD 계정으로 Adobe Experience Manager에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
4. 검색 상자에 **Adobe Experience Manager**를 입력하고 결과 창에서 **Adobe Experience Manager**를 선택한 다음 **추가** 단추를 클릭하여 응용 프로그램을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Adobe Experience Manager 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Adobe Experience Manager의 관련 사용자 간에 연결 관계가 형성되어야 합니다.

Adobe Experience Manager에서 Azure AD의 **사용자 이름** 값을 **Username** 값으로 할당하여 연결 관계를 설정합니다.

Adobe Experience Manager에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
1. Azure Portal의 **Adobe Experience Manager** 응용 프로그램 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
3. **Adobe Experience Manager 도메인 및 URL** 섹션에서 **IdP** 모드로 앱을 구성하려는 경우 다음 단계를 수행합니다.
    ...
    a. **식별자** 텍스트 상자에 AEM 서버에 대해서도 정의한 고유한 값을 입력합니다. 

    b. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://<AEM Server Url>/saml_login`
...
4. **SP** 시작 모드에서 응용 프로그램을 구성하려면 고급 URL 설정 표시를 확인하고 다음 단계를 수행합니다.
    ...
    **로그온 URL** 텍스트 상자에 Adobe Experience Manager 서버 URL을 입력합니다. 
...
6. Adobe Experience Manager 구성 섹션에서 Adobe Experience Manager 구성을 클릭하여 로그온 구성 창을 엽니다. 빠른 참조 섹션에서 **SAML 로그온 서비스 URL**, **SAML 엔터티 ID** 및 **로그아웃 ID**를 복사합니다.
...
9. **설정** -> **보안** -> **사용자**를 선택합니다.
    ...
    ![Single Sign-On 구성 저장 단추](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_adobeexperiencemanager_user.png)
...
11. **계정 설정** -> **TrustStore 만들기/관리**를 선택합니다.
...
12. **CER 파일의 인증서 추가** 단추에서 **인증서 파일 선택**을 클릭합니다. Azure Portal에서 다운로드한 인증서 파일로 이동하고 선택합니다.
...
16. 관리 화면으로 돌아가 **설정** -> **작업** -> **웹 콘솔**을 선택합니다.
...
17. 구성 페이지가 열립니다.
    ...
    ![Single Sign-On 구성 저장 단추](./media/active-directory-saas-adobeexperiencemanager-tutorial/tutorial_adobeexperiencemanager_admin2.png)

18. **Adobe Granite SAML 2.0 인증 처리기**를 찾고 **추가** 아이콘을 클릭합니다.
...
19. 이 페이지에서 다음 작업을 수행합니다.
    ...
    a. **경로** 텍스트 상자에 **/**를 입력합니다.

    b. Azure Portal에서 복사한 **SAML 로그온 서비스 URL** 값을 **IDP URL** 텍스트 상자에 붙여넣습니다.

    c. TrustStore에서 추가한 **인증서 별칭** 값을 **IDP 인증서 별칭** 텍스트 상자에 입력합니다.

    d. Azure Portal에서 구성한 고유한 **SAML 엔터티 ID** 값을 **보안 제공 엔터티 ID** 텍스트 상자에 입력합니다.

    e. Azure Portal에서 구성한 **회신 URL** 값을 **Assertion Consumer Service URL** 텍스트 상자에 입력합니다.

    f. 키 저장소에서 설정한 **암호**를 **키 저장소의 암호** 텍스트 상자에 입력합니다.

    g. 사례와 관련된 **이름 ID** 또는 다른 사용자 ID를 **사용자 특성 ID** 텍스트 상자에 입력합니다.

    h. **CRX 사용자 자동 작성**을 선택합니다.

    i. Azure Portal에서 가져온 **로그아웃 URL** 값을 **로그아웃 URL** 텍스트 상자에 입력합니다.
...
액세스 패널에서 Adobe Experience Manager 타일을 클릭하면 Adobe Experience Manager 응용 프로그램에 자동으로 로그온됩니다.
