...
8. **SAML Config(SAML 구성)** 섹션에서 **Enable SAML login(SAML 로그인 사용)**을 선택하고 아래 이미지의 설명에 따라 세부 정보를 입력합니다. 
    ...
    b. Azure AD에서 복사한 **Single Sign-On 서비스 URL**을 **URL** 섹션에 붙여 넣습니다.

    c. Azure Portal에서 다운로드한 메타데이터(XML)에서 **X509Certificate** 값을 복사하여 **x509 Certificate(X509 인증서)** 섹션에 붙여 넣습니다.
...
이 섹션에서는 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트합니다. 액세스 패널에서 LCVista 타일을 클릭하면 조직 로그온 페이지로 리디렉션됩니다. 로그인이 성공한 후 LCVista 응용 프로그램에 로그온됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://msdn.microsoft.com/library/dn308586)를 참조하세요.
