<properties
   pageTitle="Azure에서 Office 365 구독의 디렉터리 관리"
   description="Azure Active Directory 및 관리 포털을 사용하여 Office 365 구독 계정 디렉터리 관리"
...
이 문서에서는 Azure 관리 포털 내에서 Office 365 구독에 대해 만들어진 디렉터리를 관리하는 방법을 설명합니다. 이 작업을 완료하는 단계는 Azure에 대한 구독이 이미 있는지 여부는 따라 다릅니다. Azure 관리 포털에 로그인하려면 Azure 구독의 서비스 관리자 또는 공동 관리자여야 합니다.
...
Azure 구독을 완료한 후 Azure 관리 포털에 로그인하고 Azure 서비스에 액세스할 수 있습니다. Office 365 사용자를 인증하는 동일한 디렉터리를 관리하기 위해 Active Directory 확장을 클릭합니다.
...
1.	Azure 관리 포털에 msmith@hotmail.com으로 로그인합니다.
2.	**새로 만들기** > **앱 서비스** > **Active Directory** > **디렉터리 ** > **사용자 지정 만들기**를 클릭합니다.
3.	기존 디렉터리 사용을 클릭하고 **지금 로그아웃** 확인란을 선택합니다.
4.	Contoso.onmicrosoft.com의 전역 관리자로 Azure 관리 포털에 로그인합니다(예: msmith@contoso.com)).
5.	**Azure로 Contoso 디렉터리를 사용할까요?**라는 메시지가 나타나면 **계속**을 클릭합니다.
6.	**지금 로그아웃**을 클릭합니다.
7.	관리 포털에 msmith@hotmail.com으로 로그인합니다. Contoso 디렉터리와 기본 디렉터리가 Active Directory 확장에 표시됩니다.

다음 단계가 완료되면 msmith@hotmail.com은 Contoso 디렉터리의 전역 관리자가 됩니다.

##전역 관리자로 리소스를 관리하려면
이제 John Doe는 Azure 관리 포털에 로그인하고 msmith@hotmail.com의 Azure 구독과 연결된 웹 사이트 및 데이터베이스 리소스를 관리해야 한다고 가정해 봅니다. 이를 위해 Michael Smith는 다음 추가 단계를 완료해야 합니다.

1.	Azure 구독의 서비스 관리자 계정을 사용하여 관리 포털에 로그인합니다(이 예에서는 msmith@hotmail.com)).
