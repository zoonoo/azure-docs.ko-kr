...
이 문서에서는 Office 365 구독에 대해 만들어진 디렉터리를 관리하는 방법을 설명하지만 Azure 클래식 포털을 사용하겠습니다. Azure 클래식 포털에 로그인하려면 Azure 구독의 서비스 관리자 또는 공동 관리자여야 합니다. Azure 구독이 아직 업는 경우

1. Office 365에 로그인하는 데 사용하는 회사 또는 학교 계정을 사용하여 Azure에 등록할 수 있습니다.

![전자 메일 주소 입력란](./media/active-directory-manage-o365-subscription/AAD_O365_01.png)

Azure에 해당하는 구독을 찾을 수 없으나 **Azure에 등록**을 클릭하면 Office 365 계정의 관련 정보가 등록 양식에 미리 채워집니다. 기본적으로 동일한 계정이 서비스 관리자 역할에 할당됩니다.

![Azure 구독에 등록하기 위한 링크](./media/active-directory-manage-o365-subscription/AAD_O365_02.png)
...
1.	msmith@hotmail.com와 같이 [Azure 클래식 포털](https://manage.windowsazure.com)에 로그인합니다.
...
## 전역 관리자로 리소스를 관리하려면
이제 Jane Doe는 Azure 클래식 포털에 로그인하고 msmith@hotmail.com의 Azure 구독과 연결된 웹 사이트 및 데이터베이스 리소스를 관리해야 한다고 가정해 봅니다. 이렇게 하려면 Michael Smith는 다음 추가 단계를 완료해야 합니다.
...
3.	Jane Doe를 구독의 공동 관리자로 추가합니다. 이를 위해 **설정** > **관리자**를 클릭하고 구독을 선택한 후 **추가**를 선택하고 ****JohnDoe@Contoso.com**을 입력합니다.
