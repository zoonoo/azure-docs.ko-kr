...
이 문서에서는 Azure 클래식 포털을 사용하여 Office 365 구독에 대해 만들어진 디렉터리를 관리하는 방법을 설명합니다. Azure 클래식 포털에 로그인하려면 Azure 구독의 서비스 관리자 또는 공동 관리자여야 합니다. Azure 구독이 아직 업는 경우:
...
Azure에 대한 해당 구독을 찾을 수 없다는 메시지가 표시됩니다. **Azure에 등록**을 클릭하는 경우 Office 365 계정의 관련 정보가 등록 양식에 미리 채워집니다. 기본적으로 동일한 계정이 서비스 관리자 역할에 할당됩니다.
...
## 전역 관리자로 리소스를 관리하려면
이제 Jane Doe는 msmith@hotmail.com의 Azure 구독과 연결된 웹 사이트 및 데이터베이스 리소스를 관리해야 한다고 가정해 봅니다. 이렇게 하려면 Michael Smith는 다음 추가 단계를 완료해야 합니다.
