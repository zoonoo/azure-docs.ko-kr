...
이 자습서에서는 ADP eTime과 Azure AD(Azure Active Directory)를 통합하는 방법을 보여 줍니다. ADP eTime을 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.
...
7. 결과 창에서 **ADP eTime**을 선택하고 **완료**를 클릭하여 응용 프로그램을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 ADP eTime 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 ADP eTime의 관련 사용자 간에 연결이 형성되어야 합니다. 이 연결 관계는 Azure AD의 **사용자 이름** 값을 ADP eTime의 **Username** 값으로 할당하여 설정합니다.
...
2. **ADP eTime에 대한 사용자 로그온 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    a. **회신 URL** 텍스트 상자에 다음 패턴을 사용하여 사용자가 ADP eTime 응용 프로그램에 로그온하는 데 사용할 URL을 입력합니다`https://<server name>.adp.com/affwebservices/public/saml2assertionconsumer`.
...
이 섹션의 목적은 Britta Simon에게 ADP eTime에 대한 액세스 권한을 부여하여 Azure Single Sign-On을 사용할 수 있도록 하는 것입니다.
...
이 섹션은 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트하기 위한 것입니다. 액세스 패널에서 ADP eTime 타일을 클릭하면 ADP eTime 응용 프로그램에 자동으로 로그온됩니다.
