...
---
# <a name="tutorial-configure-workday-for-inbound-synchronization"></a>자습서: 인바운드 동기화를 위한 Workday 구성

이 자습서는 Workday에서 Microsoft Azure AD로 사람들을 가져오기 위해 Workday 및 Microsoft Azure AD에서 수행해야 하는 단계를 설명하기 위한 것입니다.    

>[!NOTE]
>Azure AD(Active Directory) Premium은 Azure AD의 전 세계 인스턴스를 사용하여 중국의 고객에게 제공됩니다. 중국의 21Vianet에서 운영하는 Microsoft Azure 서비스는 현재 Azure AD Premium을 지원하지 않습니다.    
> 
> 
...
## <a name="enable-the-application-integration-for-workday"></a>Workday에 응용 프로그램 통합 사용
...
3. 응용 프로그램 보기를 열려면 디렉터리 보기의 최상위 메뉴에서 **응용 프로그램** 을 클릭합니다.    
   ...
   ![응용 프로그램](./media/active-directory-saas-inbound-synchronization-tutorial/IC700994.png "응용 프로그램")  
 ...
 * 사용자의 세션이 너무 빨리 시간 초과되지 않도록 **세션 시간 초과 분**을 기본값인 0으로 둡니다.    
   ...
   ![통합 시스템 사용자 만들기](./media/active-directory-saas-inbound-synchronization-tutorial/IC750980.png "통합 시스템 사용자 만들기")  
...
2. 보안 그룹 만들기 작업을 완료합니다.  멤버를 명시적으로 추가할 보안 그룹을 만들려면 테넌트 보안 그룹의 유형에서 통합 시스템 보안 그룹 - 제한되지 않음을 선택합니다.     
   ...
   ![보안 그룹 만들기](./media/active-directory-saas-inbound-synchronization-tutorial/IC750982.png "보안 그룹 만들기")  
...
2. 이름으로 새 통합 보안 그룹을 검색하고 선택합니다.    
   ...
   ![보안 그룹 편집](./media/active-directory-saas-inbound-synchronization-tutorial/IC750984.png "보안 그룹 편집")  
3. 새 보안 그룹에 새 통합 시스템 사용자 할당       
   
   ![시스템 보안 그룹](./media/active-directory-saas-inbound-synchronization-tutorial/IC750985.png "시스템 보안 그룹")  
...
2. 시스템을 검색하여 시스템 기능 영역을 선택합니다.  확인이라는 레이블이 지정된 단추를 클릭합니다.     
   ...
   ![도메인 보안 정책](./media/active-directory-saas-inbound-synchronization-tutorial/IC750987.png "도메인 보안 정책")  
3. 시스템 기능 영역에 대한 보안 정책 목록에서 보안 관리를 확장하고,도메인 보안 정책, 외부 계정 프로비저닝을 선택합니다.     
   
   ![도메인 보안 정책](./media/active-directory-saas-inbound-synchronization-tutorial/IC750988.png "도메인 보안 정책")  
4. 사용 권한 편집 단추를 클릭한 다음, 사용 권한 편집 화면에서 Get 및 Put 통합 사용 권한이 있는 보안 그룹의 목록에 새 보안 그룹을 추가합니다.     
   
   ![사용 권한 편집](./media/active-directory-saas-inbound-synchronization-tutorial/IC750989.png "사용 권한 편집")  
5. 1단계를 반복하여 기능 영역을 선택하는 화면으로 돌아간 다음, 이번에는 인력 관리를 검색하고 인력 관리 기능 영역을 선택한 다음 확인이라는 레이블이 지정된 단추를 클릭합니다.    
   
   ![도메인 보안 정책](./media/active-directory-saas-inbound-synchronization-tutorial/IC750990.png "도메인 보안 정책")  
...
2. 감사 목적에 대한 메모를 입력하고 확인이라는 레이블이 지정된 단추를 클릭하여 보류 중인 보안 정책 변경 내용의 활성화 작업을 시작합니다.      
   ...
   ![보류 중인 보안 활성화](./media/active-directory-saas-inbound-synchronization-tutorial/IC750993.png "보류 중인 보안 활성화")  
3. 다음 화면에서 확인이라는 레이블이 지정된 확인란을 선택하고 확인이라는 레이블이 지정된 단추를 클릭하여 작업을 완료합니다.     
   
   ![보류 중인 보안 활성화](./media/active-directory-saas-inbound-synchronization-tutorial/IC750994.png "보류 중인 보안 활성화")  

## <a name="configure-user-import-in-microsoft-azure-ad"></a>Microsoft Azure AD에서 사용자 가져오기 구성
 ...
 * **Workday 관리자 이름** 텍스트 상자에 [통합 시스템 사용자 만들기](https://msdn.microsoft.com/library/azure/Dn762434.aspx#BKMK_CreateUser) 섹션에서 만든 사용자의 이름을 입력합니다.    
 * **Workday 관리자 암호** 텍스트 상자에 [통합 시스템 사용자 만들기](https://msdn.microsoft.com/library/azure/Dn762434.aspx#BKMK_CreateUser) 섹션에서 만든 사용자의 암호를 입력합니다.    
 * **Workday 테넌트 URL** 텍스트 상자에 Workday 테넌트의 URL을 입력합니다.    
3. **연결 테스트** 페이지에서 **테스트 시작**을 클릭하여 연결을 확인하고 **다음**을 클릭합니다.    
   
   ![연결 테스트](./media/active-directory-saas-inbound-synchronization-tutorial/IC750996.png "연결 테스트")  
4. **프로비저닝 옵션** 페이지에서 **다음**을 클릭합니다.    
   
   ![프로비저닝 옵션](./media/active-directory-saas-inbound-synchronization-tutorial/IC750997.png "프로비저닝 옵션")  
