...
* 사용자가 해당 Azure AD 계정으로 Tableau Server에 자동으로 로그온(SSO)되도록 설정할 수 있습니다.
...
SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Tableau Server 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 Tableau Server의 관련 사용자 간에 연결이 설정되어야 합니다.
...
3. **[Tableau Server 테스트 사용자 만들기](#creating-a-tableauserver-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Tableau Server에 만듭니다.
4. **[Azure AD 테스트 사용자 할당](#assigning-the-azure-ad-test-user)** - Britta Simon이 Azure AD Single Sign-on을 사용할 수 있도록 합니다.
5. **[Single Sign-On 테스트](#testing-single-sign-on)** - 구성이 작동하는지 확인합니다.

### <a name="configure-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성
이 섹션은 Azure 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 Tableau Server 응용 프로그램에서 SSO를 구성하는 방법을 설명하기 위한 것입니다.
...
**Tableau Server에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**
...
2. **SAML 토큰 특성** 대화 상자에서 다음 단계를 수행합니다.
   ...
   2. **특성 이름** 텍스트 상자에 **username**을 입력합니다.
   3. **특성 값** 목록에서 **user.displayname**을 선택합니다.
   4. **완료**를 클릭합니다.    

3. 위쪽 메뉴에서 **빠른 시작**을 클릭합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-tableauserver-tutorial/tutorial_general_83.png)  
4. 페이지 맨 아래에 있는 **Configure single sign-on** to open the **Single Sign-On 구성** 대화 상자를 클릭합니다.
   
    ![Single Sign-On 구성][6] 
5. **Tableau Server에 대한 사용자 로그인 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_03.png) 
6. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_04.png) 

   1. **로그인 URL** 텍스트 상자에 Tableau Server의 URL을 입력합니다. 
   2. **식별자 상자**에서 다음을 복사합니다.
   3. **다음**을 클릭합니다.

7. **Tableau Server에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
...
8. 응용 프로그램에 대해 구성된 SSO를 가져오려면 관리자 권한으로 Tableau Server 테넌트에 로그온해야 합니다.
   ...
   1. Tableau Server 구성에서 **SAML** 탭을 클릭합니다.
  
    ![Single Sign-On 구성](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_001.png) 
   2. **Single Sign-On에 SAML 사용**확인란을 선택합니다.
   3. Azure 클래식 포털에서 다운로드한 페더레이션 메타데이터 파일을 찾은 다음 **SAML Idp 메타데이터 파일**에 업로드합니다.
   4. Tableau Server 반환 URL - Tableau Server 사용자가 액세스하는 URL예: http://tableau_server 입니다. http://localhost 를 사용하지 않는 것이 좋습니다. 후행 슬래시가 있는 URL(예: http://tableau_server/)은 지원되지 않습니다. **Tableau Server 반환 URL**을 복사하여 3단계에 표시된 대로 Azure AD **로그온 URL** 텍스트 상자에 붙여 넣습니다.
   5. SAML 엔터티 ID - 엔터티 ID는 IdP에 대한 Tableau Server 설치를 고유하게 식별합니다. 원하는 경우 여기에 Tableau Server URL을 다시 입력할 수 있지만 반드시 Tableau Server URL을 입력해야 하는 것은 아닙니다. **SAML 엔터티 ID**를 복사하여 3단계에 표시된 대로 Azure AD **식별자** 텍스트 상자에 붙여 넣습니다.
   6. **메타데이터 파일 내보내기** 를 클릭하여 텍스트 편집기 응용 프로그램에서 엽니다. Http Post 및 인덱스 0이 포함된 어설션 소비자 서비스 URL을 찾아서 복사합니다. 이제 3단계에 표시된 대로 Azure AD **회신 URL** 텍스트 상자에 붙여 넣습니다. 
   7. Tableau Server 구성 페이지에서 **확인** 단추를 클릭합니다.
...
### <a name="create-a-tableau-server-test-user"></a>Tableau Server 테스트 사용자 만들기
이 섹션은 Tableau Server에서 Britta Simon이라는 사용자를 만들기 위한 것입니다. Tableau Server의 모든 사용자를 프로비전해야 합니다. 

사용자의 사용자 이름은 Azure AD에서 구성한 사용자 지정 특성 **username**의 값과 일치해야 합니다. 올바른 매핑을 사용해야 통합 시 [Azure AD Single Sign-On 구성](#configuring-azure-ad-single-single-sign-on)이 작동합니다.
...
이 섹션은 Britta Simon에게 Tableau Server에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
