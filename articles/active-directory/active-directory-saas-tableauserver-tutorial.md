...
2. 페이지 맨 아래에 있는 **Configure single sign-on** to open the **Single Sign-On 구성** 대화 상자를 클릭합니다.
   
    ![Single Sign-On 구성][6] 
3. **Tableau Server에 대한 사용자 로그인 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_03.png) 
4. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
...
1. **Tableau Server에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
...
1. 응용 프로그램에 대해 구성된 SSO를 가져오려면 관리자 권한으로 Tableau Server 테넌트에 로그온해야 합니다.
    ...
    d. Tableau Server 반환 URL - Tableau Server 사용자가 액세스하는 URL(예: http://tableau_server)입니다. Http://localhost를 사용하지 않는 것이 좋습니다. 후행 슬래시가 있는 URL(예: http://tableau_server/)은 지원되지 않습니다. **Tableau Server 반환 URL**을 복사하여 3단계에 표시된 대로 Azure AD **로그온 URL** 텍스트 상자에 붙여 넣습니다.
    ...
    > [AZURE.NOTE] Tableau Server에서 SAML을 구성하는 데 도움이 필요한 경우 [SAML 구성](http://onlinehelp.tableau.com/current/server/en-us/config_saml.htm) 
