...
3. **Reward Gateway 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 실제 식별자 및 회신 URL로 해당 값을 업데이트합니다. 이러한 값을 얻으려면 [Reward Gateway 지원 팀](mailto:clientsupport@rewardgateway.com)에 문의하세요.
...
6. **Reward Gateway** 쪽에서 Single Sign-On을 구성하려면 다운로드한 **메타데이터 XML**을 [Reward Gateway 지원 팀](mailto:clientsupport@rewardgateway.com)에 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
