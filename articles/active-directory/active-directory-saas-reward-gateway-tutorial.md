...
## <a name="adding-reward-gateway-from-the-gallery"></a>갤러리에서 Reward Gateway 추가
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Reward Gateway 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Reward Gateway의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Reward Gateway의 **Username** 값으로 할당하여 설정합니다.

Reward Gateway에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 Reward Gateway 응용 프로그램에서 Single Sign-On을 구성합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    a. **식별자 텍스트** 상자에 다음 패턴을 사용하여 사용자가 Reward Gateway 응용 프로그램에 로그인하는 데 사용할 URL을 입력합니다. 
...
1. **Reward Gateway에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
...
2. 응용 프로그램에 대해 구성된 SSO를 얻으려면 Reward Gateway [지원팀](mailTo:clientsupport@rewardgateway.com)에 문의하고 다음을 제공하세요.
...
5. **이 사용자에 대한 정보 입력** 대화 상자 페이지에서  ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-reward-gateway-tutorial/create_aaduser_05.png) 단계를 수행합니다. 
    ...
    c. **다음**을 클릭합니다.
6. **사용자 프로필** 대화 상자 페이지에서 ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-reward-gateway-tutorial/create_aaduser_06.png) 단계를 수행합니다. 
