...
이 자습서는 Azure 및 UserVoice의 통합을 보여주기 위한 것입니다.  
...
### <a name="to-enable-the-application-integration-for-uservoice-perform-the-following-steps"></a>UserVoice에 응용 프로그램 통합을 사용하도록 설정하려면
...
이 섹션은 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD의 계정으로 UserVoice에 인증할 수 있게 하는 방법을 간략하게 설명하기 위한 것입니다.  
UserVoice에 대한 Single Sign-On을 구성하려면 인증서의 지문(thumbprint) 값을 검색해야 합니다.  
...
8. **사용자 인증 편집** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    b. Azure 클래식 포털의 **UserVoice에 대한 Single Sign-On 구성** 대화 상자 페이지에서 **원격 로그인 URL** 값을 복사한 다음 **SSO 원격 로그인** 텍스트 상자에 붙여 넣습니다.

    c. Azure 클래식 포털의 **UserVoice에 대한 Single Sign-On 구성** 대화 상자 페이지에서 **원격 로그아웃 URL** 값을 복사한 다음 **SSO 원격 로그아웃** 텍스트 상자에 붙여 넣습니다.
 
    ㄹ. 내보낸 인증서에서 **지문** 값을 복사한 다음 **현재 인증서 SHA1 지문** 텍스트 상자에 붙여 넣습니다.  
...
### <a name="to-assign-users-to-uservoice-perform-the-following-steps"></a>UserVoice에 사용자를 할당하려면 다음 단계를 수행합니다.
