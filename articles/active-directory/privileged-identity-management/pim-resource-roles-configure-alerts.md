---
title: 권한 있는 ID 관리 - Azure Active Directory에서 Azure 리소스 역할에 대한 보안 경고 구성 | 마이크로 소프트 문서
...
---
# <a name="configure-security-alerts-for-azure-resource-roles-in-privileged-identity-management"></a>권한 있는 ID 관리에서 Azure 리소스 역할에 대한 보안 경고 구성

권한 있는 ID 관리(PIM)는 Azure Active Directory(Azure AD) 조직에 의심스럽거나 안전하지 않은 활동이 있을 때 경고를 생성합니다. 경고가 트리거될 때 경고 페이지에 표시됩니다.

![Azure 리소스 - 경고 페이지 목록 경고, 위험 수준 및 개수](media/pim-resource-roles-configure-alerts/rbac-alerts-page.png)

## <a name="review-alerts"></a>경고 검토

경고를 선택하여 수정 지침과 함께 경고를 트리거한 사용자 또는 역할을 나열하는 보고서를 확인합니다.

![마지막 검색 시간, 설명, 완화 단계, 유형, 심각도, 보안 영향 및 다음 번 방지 방법을 보여 주었던 경고 보고서](media/pim-resource-roles-configure-alerts/rbac-alert-info.png)
