...
**절** 은 각 사용자의 특성을 평가하여 어떤 사용자가 범위 지정 필터를 통과할 수 있는지를 파악합니다. 예를 들어, 사용자의 '상태' 특성이 뉴욕이어야 하는 절을 가졌기 때문에 뉴욕의 사용자만 응용 프로그램에 프로비전됩니다.
...
지정 범위가 여러 개 정의된 경우, 각 사용자는 응용 프로그램에 액세스하려면 최소 한 개의 지정 범위를 통과해야 합니다. 그러나 각 범위 그룹 내에서 사용자는 해당 특정 범위 그룹을 통과하는 모든 절을 통과해야 합니다. 

즉, 범위 그룹이 함께 OR 처리되고 있으며 절들은 함께 AND 처리됨으로써 그 안에 있다고 생각할 수 있습니다. 예를 들어, 다음의 지정 범위 필터를 고려하세요.
