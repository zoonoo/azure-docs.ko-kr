...
하지만에 대 한 모든 사용 가능한 권한 부여 컨트롤을 선택할 수 있습니다 합니다 **다른 클라이언트** 조건는, 최종 사용자 환경을 항상 동일한 액세스를 차단 합니다.

레거시 인증에 사용 하 여 차단 하는 경우는 **다른 클라이언트** 조건을 설정할 수도 있습니다 장치 플랫폼 및 위치 조건입니다. 예를 들어 모바일 디바이스에 대한 레거시 인증만 차단하려면 다음을 선택하여 **디바이스 플랫폼** 조건을 설정합니다.
