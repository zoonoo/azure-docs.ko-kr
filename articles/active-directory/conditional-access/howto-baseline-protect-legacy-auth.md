---
title: 기준 정책 레거시 인증 차단 (미리 보기)-Azure Active Directory
description: 레거시 인증 프로토콜을 차단 하는 조건부 액세스 정책
...
오늘날 대부분의 손상 된 로그인 시도는 대부분 레거시 인증에서 제공 됩니다. 레거시 인증은 MFA (multi-factor authentication)를 지원 하지 않습니다. 디렉터리에서 MFA 정책을 사용 하도록 설정한 경우에도 잘못 된 행위자는 레거시 프로토콜을 사용 하 여 인증 하 고 MFA를 우회할 수 있습니다.

레거시 프로토콜에서 수행 하는 악의적인 인증 요청 으로부터 계정을 보호 하는 가장 좋은 방법은 이러한 시도를 모두 차단 하는 것입니다. 환경을 보다 쉽게 보호할 수 있도록 하기 위해 레거시 인증을 차단 하는이 기준 정책을 만들었습니다.

**레거시 인증 차단** 은 레거시 프로토콜에서 수행 된 모든 인증 요청을 차단 하는 [기준 정책](concept-baseline-protection.md) 입니다. 최신 인증을 사용 하 여 모든 사용자에 게 성공적으로 로그인 해야 합니다. 다른 기준 정책과 함께 사용 하는 경우 레거시 프로토콜에서 들어오는 모든 요청은 차단 되 고 필요할 때마다 MFA에 모든 사용자가 필요 합니다. 이 정책은 Exchange ActiveSync를 차단 하지 않습니다.
...
이러한 로그는 레거시 인증에 종속 된 사용자 및 레거시 프로토콜을 사용 하 여 인증 요청을 수행 하는 응용 프로그램을 표시 합니다. 이러한 로그에 표시 되지 않고 레거시 인증을 사용 하지 않는 사용자의 경우 조건부 액세스 정책을 구현 하거나 기준 정책 사용: 해당 사용자에 대해서만 **레거시 인증 차단** 을 선택 합니다.
...
온-프레미스에서 최신 인증을 사용 하려면 먼저 필수 구성 요소를 충족 하는지 확인 하세요.
이제 온-프레미스에서 최신 인증을 사용할 준비가 되었습니다.
...
정책 **기준 정책: 레거시 인증 차단 (미리 보기** )은 미리 구성 되어 있으며 Azure Portal의 조건부 액세스 블레이드로 이동 하면 위쪽에 표시 됩니다. 이 설정은 로그인이 성공한 후에만 적용 되므로 사용자가 계속 해 서 레거시 인증을 사용 하는 옵션을 사용할 수 있습니다.

이 정책을 사용 하도록 설정 하 고 조직을 보호 하려면:
