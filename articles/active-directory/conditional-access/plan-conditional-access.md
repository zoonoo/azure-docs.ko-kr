---
title: Azure Active Directory에서 조건부 액세스 정책을 계획 | Microsoft Docs
description: 이 문서에서는 Azure Active Directory 조건부 액세스 정책을 계획 하는 방법을 알아봅니다.
...
조건부 액세스 배포를 계획 하는 것이 조직에서 앱 및 리소스에 대 한 필수 액세스 전략을 달성 해야 중요 합니다. 대부분의 사용자가 선택한 조건에 대 한 액세스를 차단 하거나 부여 하는 데 필요한 다양 한 정책 디자인 배포 계획 단계 중 시간을 보냅니다. 이 문서에서는 안전 하 고 효과적인 조건부 액세스 정책을 구현 하기 위해 취해야 하는 단계를 설명 합니다. 시작 하기 전에 이해 해야 하는 방법 [조건부 액세스](overview.md) 방식과 때 사용 해야 합니다.

## <a name="what-you-should-know"></a>알아야 할 사항

조건부 액세스에서 조직의 앱 및 독립 실행형 기능 대신 리소스에 대 한 액세스를 제어할 수 있는 프레임 워크도 간주 합니다. 따라서 일부 조건부 액세스 설정을 구성할 수 있는 추가적인 기능이 필요 합니다. 예를 들어 특정 [로그인 위험 수준](../identity-protection/howto-sign-in-risk-policy.md#what-is-the-sign-in-risk-policy)에 응답하는 정책을 구성할 수 있습니다. 그러나 로그인 위험 수준을 기반으로 하는 정책의 경우 [Azure Active Directory ID 보호](../identity-protection/overview.md)를 사용하도록 설정해야 합니다.

추가 기능이 필요한 경우 관련 라이선스를 얻어야 할 수도 있습니다. 예를 들어, 조건부 액세스는 Azure AD Premium P1 기능, id 보호는 Azure AD Premium P2 라이선스가 필요 합니다.

조건부 액세스 정책의 두 종류가 있습니다: 기본 및 표준입니다. A [기준 정책](baseline-protection.md) 미리 정의 된 조건부 액세스 정책입니다. 기준 수준 이상의 보안을 유지하는 것이 이 정책의 목표입니다. 기준 정책. 기준 정책은 모든 Azure AD 버전에서 사용할 수 있으며 제한된 사용자 지정 옵션만 제공합니다. 시나리오에 유연성이 더 필요한 경우 기준 정책을 사용하지 않도록 설정하고 사용자 지정 표준 정책에서 요구 사항을 구현합니다.

표준 조건부 액세스 정책에서 모든 비즈니스 요구 사항에 맞게 정책을 조정 설정을 사용자 지정할 수 있습니다. 표준 정책에는 Azure AD Premium P1 라이선스가 필요합니다.

## <a name="draft-policies"></a>초안 정책

Azure Active Directory 조건부 액세스를 사용 하면 새로운 수준으로 클라우드 앱의 보호를 가져올 수 있습니다. 이 새로운 수준에서는 정적 액세스 구성 대신 동적 정책 평가를 기준으로 클라우드 앱에 액세스할 수 있습니다. 응답을 정의 하는 조건부 액세스 정책을 사용 하 여 (**이렇게**)는 액세스 조건 (**이 경우**).

![이유 및 응답](./media/plan-conditional-access/10.png)

이 계획 모델을 사용 하 여 구현 하려는 모든 조건부 액세스 정책을 정의 합니다. 계획 연습은 다음을 수행합니다.

- 각 정책에 대한 응답 및 조건의 개요 작성에 도움이 됩니다.
- 조직에 잘 문서화 된 조건부 액세스 정책 카탈로그에 대 한 결과입니다. 

카탈로그를 사용하여 정책 구현이 조직의 비즈니스 요구 사항을 반영하는지 여부를 평가할 수 있습니다. 

조직에 대 한 조건부 액세스 정책을 만들려면 다음 예제에서는 서식 파일을 사용 합니다.
...
최소한 **조건**은 클라우드 앱(**what**)에 액세스를 시도하는 보안 주체(**who**)를 정의합니다. 필요한 경우 액세스 시도가 수행되는 **방법**을 포함할 수도 있습니다. 조건부 액세스의 요소를 정의 하는 누가, 방법과, 조건 이라고 합니다. 자세한 내용은 참조 하세요. [Azure Active Directory 조건부 액세스의 조건 이란?](conditions.md) 

**수행할 작업**을 사용하여 액세스 조건에 대한 정책의 응답을 정의합니다. 응답에서 추가 요구 사항(예: MFA(다단계 인증))을 통해 액세스 권한을 부여하거나 차단합니다. 전체 개요를 참조 하세요. [Azure Active Directory 조건부 액세스의 제어 액세스 이란?](controls.md)  
...
이 시점에서 정책의 명명 표준을 결정하는 것이 좋습니다. 이름 지정 표준은 정책을 찾고 Azure 관리 포털에서 열지 않고도 해당 용도를 파악하는 데 도움이 됩니다. 표시할 정책의 이름을 지정 합니다.
...
이름을 사용 하면 조건부 액세스 구현에 대 한 개요를 유지 하도록 도와줍니다, 하는 동안 시퀀스 번호는 대화의 정책을 참조 해야 하는 경우에 유용 합니다. 예를 들어 휴대폰에서을 관리자를 이야기 하면 문제를 해결 하기 위해 EM063 정책을 열려면 해당 요청할 수 있습니다.
...
예를 들어, 이름이이 정책에서 MFA 중단이 발생 하는 경우 사용 하도록 설정 해야 4 첫 번째 정책이 임을 나타냅니다.

`EM01 - ENABLE IN EMERGENCY, MFA Disruption[1/4] - Exchange SharePoint: Require hybrid Azure AD join For VIP users`

## <a name="plan-policies"></a>정책 계획

조건부 액세스 정책 솔루션을 계획할 때 다음 결과 달성 하는 정책을 만들어야 하는지 여부를 평가 합니다. 
...
사용자의 로그인 환경을 간소화하기 위해 사용자가 사용자 이름 및 암호를 사용하여 클라우드 앱에 로그인하도록 허용할 수 있습니다. 그러나 일반적으로 보다 강력한 형태의 계정 확인을 요구하는 것이 좋은 몇 가지 시나리오가 있습니다. 조건부 액세스 정책을 사용 하 여 특정 시나리오에 대 한 MFA에 대 한 요구 사항을 제한할 수 있습니다. 
...
조건부 액세스 정책을 사용 하 여 잠재적으로 손상 된 id로 로그인 하려면 자동화 된 응답을 구현할 수 있습니다. 계정이 손상되었을 확률은 위험 수준 형태로 표시됩니다. ID 보호를 통해 계산되는 두 가지 위험 수준(로그인 위험 및 사용자 위험)이 있습니다. 로그인 위험에 대한 응답을 구현하려면 다음 두 가지 옵션이 있습니다.

- [로그인 위험 조건](conditions.md#sign-in-risk) 조건부 액세스 정책
...
클라우드 리소스에 액세스하도록 지원되는 디바이스의 범위가 넓어지면 사용자의 생산성을 개선할 수 있습니다. 반대로 환경에서 특정 리소스에 알려지지 않은 보호 수준을 가진 디바이스가 액세스하지 않도록 하는 것이 좋습니다. 영향을 받는 리소스의 경우 사용자가 관리 디바이스를 사용해서만 액세스할 수 있도록 해야 합니다. 자세한 내용은 [조건부 액세스를 사용 하 여 클라우드 앱 액세스를 위해 관리 되는 장치를 요구 하는 방법](require-managed-devices.md)합니다. 

### <a name="require-approved-client-apps"></a>승인된 클라이언트 앱 필요

BYOD(Bring Your Own Device) 시나리오에 대해 결정해야 하는 첫 번째 사항 중 하나는 전체 디바이스를 관리해야 하는지 아니면 디바이스의 데이터만 관리해야 하는지 여부입니다. 직원은 개인 및 회사 작업에 대해 모바일 디바이스를 사용합니다. 직원이 생산적이 될 수 있도록 하는 반면 데이터 손실을 방지하려고 합니다. Azure Active Directory (Azure AD) 조건부 액세스를 사용 하 여 회사 데이터를 보호할 수 있는 승인 된 클라이언트 앱에 클라우드 앱에 대 한 액세스를 제한할 수 있습니다. 자세한 내용은 [조건부 액세스를 사용 하 여 클라우드 앱 액세스를 위해 승인 된 클라이언트 앱을 요구 하는 방법](app-based-conditional-access.md)합니다.

### <a name="block-legacy-authentication"></a>레거시 인증 차단

Azure AD는 레거시 인증을 포함하여 가장 널리 사용되는 몇 가지 인증 및 권한 부여 프로토콜을 지원합니다. 레거시 인증을 사용하는 애플리케이션에서 테넌트의 리소스에 액세스하지 못하도록 방지하려면 어떻게 해야 할까요? 방금 조건부 액세스 정책을 사용 하 여 차단 하는 것이 좋습니다. 필요한 경우 특정 사용자 및 특정 네트워크 위치만 레거시 인증을 기반으로 하는 애플리케이션을 사용하도록 허용합니다. 자세한 내용은 [조건부 액세스를 사용 하 여 Azure AD에 대 한 레거시 인증을 차단 하는 방법](block-legacy-authentication.md)합니다.
...
조건부 액세스 정책을 관리 하는 것은 수동 작업입니다. Azure portal에서 단일 중앙 위치인 조건부 액세스 페이지에서 조건부 액세스 정책을 관리할 수 있습니다. 조건부 액세스 페이지 하나 진입점은는 **보안** 섹션을 **Active Directory** 탐색 창입니다. 

![조건부 액세스](media/plan-conditional-access/03.png)

조건부 액세스 정책 만들기를 참조 하는 방법에 자세히 알아보려면 싶다면 [Azure Active Directory 조건부 액세스를 사용 하 여 특정 앱에 대 한 MFA 필요](app-based-mfa.md)합니다. 이 빠른 시작은 다음을 수행하는 데 도움이 됩니다.

- 사용자 인터페이스를 파악합니다.
- 조건부 액세스의 작동 원리의 첫 인상을 합니다. 

### <a name="evaluate-a-simulated-sign-in"></a>시뮬레이션된 로그인 평가

조건부 액세스 정책을 구성한 경우에 이제 예상 대로 작동 하는지 확인 하려면 것입니다. 첫 번째 단계로, 조건부 액세스를 사용 하 여 [경우에 어떻게 정책 도구](what-if-tool.md) 시뮬레이션의 테스트 사용자를 로그인 합니다. 이 시뮬레이션은 이 로그인이 정책에 미치는 영향을 평가하고, 시뮬레이션 보고서를 생성합니다.

>[!NOTE]
> 시뮬레이션된을 실행 하는 조건부 액세스 정책에 미치는 인상을 제공 하는 동안 실행 된 실제 테스트를 대체 하지는 않습니다.

### <a name="test-your-policy"></a>정책 테스트

테스트 계획에 따라 테스트 사례를 실행합니다. 이 단계에서는 테스트 사용자에 대해 각 정책의 엔드투엔드 테스트를 실행하여 각 정책이 올바르게 작동하는지 확인합니다. 위에서 만든 시나리오를 사용하여 각 테스트를 실행합니다.

정책의 제외 기준을 테스트하는 것이 중요합니다. 예를 들어 MFA를 요구하는 정책에서 사용자 또는 그룹을 제외할 수 있습니다. 제외 된 사용자는 메시지가 표시 되 면 MFA에 대 한 다른 정책의 조합이 해당 사용자에 대 한 MFA를 요구할 수 있으므로 테스트 합니다.
