...
- [**도메인 기반**] (#구성 도메인 기반 필터링): 이 필터링 형식을 사용하여 Azure AD Connect 동기화의 Source AD Connector 속성을 관리할 수 있습니다. 이 형식을 사용하여 Azure AD로 동기화할 수 있는 도메인을 선택할 수 있습니다.

- [**조직 구성 단위 기반 필터링 구성**] (#구성-조직 구성 단위 – 기반 필터링): 이 필터링 형식을 사용하여 Azure AD Connect 동기화의 Source AD Connector의 속성을 관리할 수 있습니다. 이 필터링 형식을 사용하여 Azure AD로 동기화할 수 있는 OU를 선택할 수 있습니다.

- [**특성 기반**](#configure-attribute-based-filtering): 이 필터링 메서드를 사용하여 특성 기반 필터를 지정할 수 있습니다. 이를 통해 클라우드로 동기화할 개체를 제어할 수 있습니다.

> [AZURE.NOTE]필터 구성을 변경하고 암호 동기화를 사용하려면 구성을 완료한 다음 모든 암호를 전체 동기화 트리거 해야 합니다. 암호 전체 동기화 트리거 하는 방법에 대한 과정은 [모든 암호의 전체 동기화 트리거](active-directory-aadconnectsync-implement-password-synchronization.md#trigger-a-full-sync-of-all-passwords)를 참조하십시오.

## 도메인 기반 필터링 구성

이 섹션에서는 도메인 필터를 구성하기 위해 수행해야 하는 단계를 제공합니다.

Azure AD Connect를 설치한 후 포리스트 내에 도메인을 추가 및 삭제했다면 필터링 구성을 업데이트해야 합니다.
...
7. **디렉터리 파티션 선택** 목록에서 동기화하려는 파티션만 선택되어 있는지 확인합니다. <br> 동기화 프로세스에서 도메인을 제거하려면 해당 도메인의 확인란 선택을 해제합니다.
...
디렉터리 파티션 목록에서 파티션을 **제거**한 경우에 해당 파티션을 참조하는 모든 실행 프로필 단계도 제거되었는지 확인해야 합니다.
...
파티션을 디렉터리 파티션 목록에 **추가**한 경우 파티션에 대한 실행 프로필 단계가 위의 목록에 있는 각 실행 프로필에서 사용 가능한지 확인해야 합니다.
...
### 새 도메인 또는 제거된 도메인에 대한 필터를 변경합니다.
포리스트에 새 도메인을 추가 또는 포리스트에서 도메인을 제거하는 경우, 도메인 필터링 구성을 필히 업데이트해야 합니다.
...
7. 페이지 위쪽의 **새로고침** 단추를 클릭합니다.

8. **디렉터리 파티션 선택**목록에서 새 도메인을 선택합니다.

9. **속성** 대화 상자를 닫으려면 확인을 클릭합니다.

동일한 커넥터가 여전히 선택되어있는지 확인합니다. 이제 커넥터의 실행 프로필을 구성해야 합니다.
...
10. 대화 상자에 대한 **실행 프로필 구성**을 열려면 **작업** 메뉴에서 **실행 프로필 구성**을 클릭합니다.

11. **커넥터 실행 프로필** 목록에서 구성하려는 실행 프로필을 선택합니다.

12. **실행 프로필 구성** 대화 상자를 열려면 **새 단계**를 클릭합니다.

13. 단계 형식 목록에 있는 **구성 단계** 페이지에서 단계 형식을 선택하고 **다음**을 클릭합니다.

14. **파티션** 목록에 있는 **커넥터 구성** 페이지에서 도메인 필터에 추가한 파티션의 이름을 선택합니다.

15. **실행 프로필 구성** 대화 상자를 닫으려면 **마침**을 클릭합니다.

16. 대화 상자에 대한 **실행 프로필 구성**을 닫으려면 **확인**을 클릭합니다.

17. 모든 실행 프로필에 이 단계를 반복합니다.
...
8. **속성** 대화 상자를 닫으려면 확인을 클릭합니다. 도메인이 제거되었다는 메시지가 팝업되고 해당 구성은 정리됩니다.
...
6. **변환 추가**를 클릭하고 **FlowType**을 **식**으로 선택하고 **대상 특성**을 **sourceObjectType**으로 선택합니다. **원본**에 다음 식을 입력합니다.<br>`IIF(IsNullOrEmpty([department]),NULL,IIF([department]<>”Sales”,”DoNotSync”,NULL))`
