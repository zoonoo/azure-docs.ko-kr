...
ms.locfileid: "36231341"
---
# <a name="unexpected-error-when-performing-consent-to-an-application"></a>응용 프로그램에 대한 동의를 수행할 때 예기치 않은 오류

이 문서에서는 응용 프로그램에 대한 동의 프로세스 도중 발생할 수 있는 오류에 대해 설명합니다. 오류 메시지가 포함되지 않는 예기치 않은 동의 프롬프트의 문제를 해결하려는 경우 [Azure AD 인증 시나리오](https://docs.microsoft.com/azure/active-directory/develop/active-directory-authentication-scenarios)를 참조하세요.

Azure Active Directory와 통합되는 많은 응용 프로그램을 작동시키기 위해 다른 리소스에 대한 사용 권한이 필요합니다. 또한 이러한 리소스가 Azure Active Directory와 통합되면 종종 일반적인 동의 프레임워크를 사용하여 액세스하기 위한 사용 권한을 요청합니다. 일반적으로 응용 프로그램을 처음 사용할 때 발생하지만 이후에 응용 프로그램을 사용할 때도 발생할 수 있는 동의 확인 프롬프트가 표시됩니다.

응용 프로그램에 필요한 사용 권한에 대해 동의한 사용자에게는 특정 조건이 true여야 합니다. 이러한 조건이 충족되지 않으면 발생할 수 있는 오류는 다음과 같습니다.
...
사용자가 동의하려는 응용 프로그램이 조직의 디렉터리(테넌트)에서 찾을 수 없는 리소스 응용 프로그램에 액세스하는 사용 권한을 요청하는 경우에 이러한 오류가 발생합니다. 이 상황이 발생할 수 있는 몇 가지 이유는 다음과 같습니다.
...
-   대상 리소스 응용 프로그램을 나타내는 서비스 주체는 조직에서 존재하지 않거나 이전에 있었지만 제거되었습니다. 이 문제를 해결하려면 클라이언트 응용 프로그램이 이에 대한 권한을 요청할 수 있도록 조직에서 리소스 응용 프로그램의 서비스 주체를 제공해야 합니다. 서비스 주체는 다음을 포함하여 응용 프로그램의 유형에 따라 여러 가지 방법으로 프로비전할 수 있습니다.

    -   리소스 응용 프로그램의 구독 구입(게시된 Microsoft 응용 프로그램)

    -   리소스 응용 프로그램에 대한 동의

    -   Azure Portal을 통해 응용 프로그램 권한 부여
