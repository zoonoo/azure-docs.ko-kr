...
3. 새 프로젝트가 프로젝트 메뉴에서 선택되어 있는지 확인합니다. 그런 다음, **APIs 및 Services**에서 **OAuth 동의 화면**을 선택합니다.

4. **외부**를 선택한 다음, **만들기**를 선택합니다. 
5. **OAuth 동의 화면**에서 **애플리케이션 이름**을 입력합니다. (다른 설정은 변경하지 말고 그대로 둡니다.)
