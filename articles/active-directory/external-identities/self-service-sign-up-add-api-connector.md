...
인증서가 만료 되는 경우에 대 한 미리 알림 경고를 설정 하는 것이 좋습니다. 기존 API 커넥터에 새 인증서를 업로드 하려면 **모든 api 커넥터** 에서 api 커넥터를 선택 하 고 **새 커넥터 업로드** 를 클릭 합니다. 만료 되지 않고 시작 날짜를 지난 가장 최근에 업로드 된 인증서는 Azure Active Directory에 의해 자동으로 사용 됩니다.
