...
1. 최소 4gb RAM 및 .NET 4.7.1 + runtime을 사용 하 여 Windows Server 2012 R2 이상을 실행 하는 도메인에 가입 된 호스트 서버 id 

2. 서버와 Azure AD 사이에 방화벽이 있는 경우 다음 항목을 구성합니다.
   - 에이전트가 다음 포트를 통해 Azure AD에 대 한 *아웃 바운드* 요청을 수행할 수 있는지 확인 합니다.

     | 포트 번호 | 사용 방법 |
     | --- | --- |
     | **80** | SSL 인증서의 유효성을 검사하는 동안 CRL(인증서 해지 목록) 다운로드 |
     | **443** | 서비스와의 모든 아웃바운드 통신 처리 |
     | **8080**(선택 사항) | 에이전트는 포트 8080을 통해 10 분 마다 상태를 보고 합니다 (포트 443를 사용할 수 없는 경우). 이 상태는 Azure AD 포털에 표시됩니다. 포트 8080은 사용자 로그인에 사용되지 _않습니다_. |
   ...
   - 초기 등록을 위해 에이전트에 **login.windows.net** 및 **login.microsoftonline.com** 에 대 한 액세스 권한이 필요 합니다. 이러한 URL에 대한 방화벽도 엽니다.
...
## <a name="configure-azure-ad-connect-cloud-provisioning"></a>클라우드 프로 비전 Azure AD Connect 구성
 프로 비전을 구성 하려면 다음 단계를 사용 합니다.

1.  Azure AD 포털에 로그인 합니다.
2.  **Azure Active Directory** 클릭
3.  **Azure AD Connect** 클릭
4.  **프로 비전 관리 (미리 보기)** 
![](media/how-to-configure/manage1.png)를 선택 합니다.
5.  **새 구성**
를 클릭 ![](media/tutorial-single-forest/configure1.png)
7.  구성 화면에서 **알림 전자 메일**을 입력 하 고 선택기를 **사용** 으로 이동한 후 **저장**을 클릭 합니다.
![](media/tutorial-single-forest/configure2.png)
1.  이제 구성 상태가 **정상**이어야 합니다.
![](media/how-to-configure/manage4.png)

## <a name="verify-users-are-created-and-synchronization-is-occurring"></a>사용자가 생성되고 동기화가 수행되는지 확인
이제 온-프레미스 디렉터리에 있던 사용자가 동기화 되었으며 이제 Azure AD 테 넌 트에 존재 하는지 확인 합니다.  이 작업을 완료하는 데 몇 시간이 걸릴 수 있습니다.  사용자가 동기화되었는지 확인하려면 다음을 수행합니다.
