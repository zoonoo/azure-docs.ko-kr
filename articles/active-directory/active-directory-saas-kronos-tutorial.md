...
* 사용자가 해당 Azure AD 계정으로 Kronos SSO(Single Sign-On)에 자동으로 로그온되도록 설정할 수 있습니다.
...
## <a name="add-kronos-from-the-gallery"></a>갤러리에서 Kronos 추가
...
이 섹션에서는 "Britta Simon"이라는 테스트 사용자를 기반으로 Kronos에서 Azure AD SSO를 구성하고 테스트합니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Kronos 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Kronos의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Kronos의 **Username** 값으로 할당하여 설정합니다.

Kronos에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.

1. **[Azure AD Single Sign-On 구성](#configuring-azure-ad-single-single-sign-on)** - 사용자가 이 기능을 사용할 수 있도록 합니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)** - Britta Simon으로 Azure AD Single Sign-On을 테스트하는 데 사용합니다.
3. **[Kronos 테스트 사용자 만들기](#creating-an-kronos-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Kronos에 만듭니다.
4. **[Azure AD 테스트 사용자 할당](#assigning-the-azure-ad-test-user)** - Britta Simon이 Azure AD Single Sign-on을 사용할 수 있도록 합니다.
5. **[Single Sign-On 테스트](#testing-single-sign-on)** - 구성이 작동하는지 확인합니다.

### <a name="configure-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성
이 섹션에서는 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 Kronos 응용 프로그램에서 Single Sign-On을 구성합니다.

Kronos 응용 프로그램은 특정 형식의 SAML 어설션이 필요합니다. 응용 프로그램에 매핑되는 올바른 사용자 식별자를 식별하려면 Kronos 팀과 먼저 작업해 보세요. 

또한 이 매핑에 사용하려는 특성에 대한 Kronos 팀의 지침을 수행하세요. Microsoft는 사용자 식별자로 **"NameIdentifier"** 특성을 사용하도록 권장합니다. 응용 프로그램의 **"특성"** 탭에서 이 특성의 값을 관리할 수 있습니다. 

다음 스크린샷은 이에 대한 예제를 보여 줍니다. 여기에서는 **ExtractMailPrefix** 함수와 함께 **userprincipalname** 특성을 사용하여 고유한 사용자 ID를 제공하는 nameidentifier 클레임을 매핑했습니다. 성공적인 SAML 응답으로 Kronos 응용 프로그램에 전송됩니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
  ...
  1. 식별자 텍스트 상자에 `https://<company name>.kronos.net/`
  2. 회신 URL에 `https://<company name>.kronos.net/wfc/navigator/logonWithUID`

1. **Kronos의 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-kronos-tutorial/tutorial_kronos_05.png) 
  1. **메타데이터 다운로드**를 클릭하고 파일을 컴퓨터에 저장합니다. 
  2. **다음**을 클릭합니다.

2. 응용 프로그램에 구성된 SSO를 사용하려면 Kronos 계정 관리자에게 문의하고 SSO를 구성하기 위해 적절한 채널을 사용하도록 도움을 줄 수 있습니다. 전자 메일을 보내고 다운로드한 메타데이터 파일을 연결해야 합니다.
...
### <a name="create-an-kronos-test-user"></a>Kronos 테스트 사용자 만들기
이 섹션에서는 Kronos에서 Britta Simon이라는 사용자를 만듭니다. Kronos 응용 프로그램에서는 SSO를 수행하기 전에 모든 사용자를 프로비전해야 합니다. 

Kronos 고객 지원 담당자와 함께 모든 사용자를 응용 프로그램에 프로비전하세요. 

>[!NOTE]
>사용자를 수동으로 만들거나 일괄 처리하려면 Kronos 지원 팀에 문의해야 합니다. 
> 

### <a name="assign-the-azure-ad-test-user"></a>Azure AD 테스트 사용자 할당
이 섹션에서는 Azure SSO를 사용할 수 있도록 Britta Simon에게 Kronos에 대한 액세스 권한을 부여합니다.
