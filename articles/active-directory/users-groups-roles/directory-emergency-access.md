---
title: 비상 액세스 관리자 계정 관리 - Azure AD | 마이크로 소프트 문서
description: 이 문서에서는 Azure Active Directory(Azure AD) 조직에서 실수로 잠기지 않도록 긴급 액세스 계정을 사용하는 방법을 설명합니다.
...
ms.locfileid: "74028138"
---
# <a name="manage-emergency-access-accounts-in-azure-ad"></a>Azure AD에서 응급 액세스 계정 관리

다른 사용자의 계정을 관리자로 로그인하거나 활성화할 수 없으므로 Azure Active Directory(Azure AD) 조직에서 실수로 잠기지 않도록 하는 것이 중요합니다. 조직에서 두 개 이상의 *긴급 액세스 계정을* 만들어 실수로 관리 액세스가 부족하여 영향을 줄일 수 있습니다.

응급 액세스 계정에는 높은 권한이 부여되고 특정 개인에게 할당되지 않습니다. 긴급 액세스 계정은 일반 관리 계정을 사용할 수 없는 비상 사태 또는 "유리 파손" 시나리오로 제한됩니다. 긴급 계정 사용을 절대적으로 필요한 시간으로만 제한하는 목표를 유지하는 것이 좋습니다.

이 문서에서는 Azure AD에서 응급 액세스 계정을 관리하는 지침을 제공합니다.

## <a name="why-use-an-emergency-access-account"></a>긴급 액세스 계정을 사용하는 이유
...
## <a name="create-emergency-access-accounts"></a>비상 액세스 계정 만들기
...
- 응급 액세스 계정에 사용되는 인증 메커니즘은 다른 응급 액세스 계정을 비롯한 기타 관리 계정에 사용되는 메커니즘과 구분되어야 합니다.  예를 들어 온-프레미스 MFA를 통해 일반 관리자 로그인이 수행되는 경우에는 Azure MFA가 다른 메커니즘이 될 수 있습니다.  그러나 Azure MFA가 관리 계정에 대한 인증의 기본 부분인 경우 타사 MFA 공급자와 조건부 액세스를 사용하는 것과 같은 다른 접근 방식을 고려하십시오.
...
하지만 응급 액세스 계정 중 하나 이상은 응급 계정이 아닌 기타 계정과 MFA 메커니즘이 달라야 합니다. 여기에는 타사 MFA 솔루션이 포함됩니다. Azure AD 및 기타 연결된 소프트웨어에 [대한 모든 관리자에 대해](../authentication/howto-mfa-userstates.md) 서비스(SaaS) 앱으로 다단계 인증을 요구하는 조건부 액세스 정책이 있는 경우 이 요구 사항에서 긴급 액세스 계정을 제외하고 대신 다른 메커니즘을 구성해야 합니다. 또한 계정에 사용자별 MFA 정책이 없도록 해야 합니다.

### <a name="exclude-at-least-one-account-from-conditional-access-policies"></a>조건부 액세스 정책에서 하나 이상의 계정 제외

응급 상황에서 정책으로 인해 액세스 권한이 차단되어 문제를 해결하지 못하는 경우는 없어야 합니다. 모든 조건부 액세스 정책에서 하나 이상의 긴급 액세스 계정을 제외해야 합니다. [기준 정책](../conditional-access/baseline-protection.md)을 사용하도록 설정한 경우에는 응급 액세스 계정을 제외해야 합니다.
...
조직은 긴급 계정에서 로그인 및 로그 활동을 모니터링하고 다른 관리자에게 알림을 트리거해야 합니다. 브레이크 글라스 계정의 활동을 모니터링할 때 이러한 계정이 테스트 또는 실제 비상 사태에만 사용되는지 확인할 수 있습니다. Azure Log Analytics를 사용하여 로그인 로그를 모니터링하고 유리 계정이 로그인할 때마다 관리자에게 이메일 및 SMS 경고를 트리거할 수 있습니다.

### <a name="prerequisites"></a>사전 요구 사항

1. [Azure AD 로그인 로그를](https://docs.microsoft.com/azure/active-directory/reports-monitoring/howto-integrate-activity-logs-with-log-analytics) Azure 모니터로 보냅니다.

### <a name="obtain-object-ids-of-the-break-glass-accounts"></a>브레이크 글래스 계정의 개체 ID 구하기

1. 사용자 관리자 역할에 할당된 계정으로 [Azure 포털에](https://portal.azure.com) 로그인합니다.
1. **Azure Active 디렉터리** > **사용자를**선택합니다.
1. 브레이크 글라스 계정을 검색하고 사용자 이름을 선택합니다.
1. 나중에 사용할 수 있도록 Object ID 특성을 복사하여 저장합니다.
1. 두 번째 브레이크 글라스 계정에 대해 이전 단계를 반복합니다.

### <a name="create-an-alert-rule"></a>경고 규칙 만들기

1. Azure 모니터의 모니터링 기여자 역할에 할당된 계정으로 Azure [포털에](https://portal.azure.com) 로그인합니다.
1. **모든 서비스를**"검색에서 "로그 분석"을 입력한 다음 **로그 분석 작업 영역을**선택합니다.
1. 작업 영역을 선택합니다.
1. 작업 영역에서**새 경고 규칙**경고를 **선택합니다.** > 
    1. **리소스에서**구독이 경고 규칙을 연결하려는 구독인지 확인합니다.
    1. **조건에서** **추가를**선택합니다.
    1. **신호 이름**에서 사용자 **지정 로그 검색을** 선택합니다.
    1. **검색 쿼리에서**다음 쿼리를 입력하고 두 개의 나누기 유리 계정의 개체 ID를 삽입합니다.
        > [!NOTE]
        > 포함하려는 각 추가 브레이크 유리 계정에 대해 쿼리에 다른 "또는 UserId == "ObjectGuid"를 추가합니다.

        ![브레이크 글래스 계정의 개체 ID를 경고 규칙에 추가합니다.](./media/directory-emergency-access/query-image1.png)
    ...
    1. **을 기준으로 평가된** **기간(분)에서** 쿼리를 실행할 기간에 대한 기간(분)과 쿼리를 실행할 **빈도(분)를** 선택합니다. 빈도는 기간보다 적거나 같아야 합니다.

        ![경고 논리](./media/directory-emergency-access/alert-image2.png)

    1. **완료를 선택합니다.** 이제 이 경고의 예상 월별 비용을 볼 수 있습니다.
1. 경고로 알림을 받을 사용자의 작업 그룹을 선택합니다. 하나를 만들려면 [작업 그룹 만들기를](#create-an-action-group)참조하십시오.
1. 작업 그룹의 구성원에게 전송된 전자 메일 알림을 사용자 지정하려면 **작업 사용자 지정**에서 작업을 선택합니다.
1. **경고 세부 정보에서**경고 규칙 이름을 지정하고 선택적 설명을 추가합니다.
1. 이벤트의 **심각도 수준을** 설정합니다. **치명타(Sev 0)로**설정하는 것이 좋습니다.
1. **생성 시 사용 규칙에서** **'예'로**설정된 상태로 둡니다.
1. 잠시 동안 경고를 끄려면 경고 **억제** 확인란을 선택하고 다시 경고하기 전에 대기 기간을 입력한 다음 **저장을**선택합니다.
1. **경고 규칙 만들기**를 클릭합니다.
...
1. 작업 그룹 이름과 짧은 이름을 입력합니다.
1. 구독 및 리소스 그룹을 확인합니다.
1. 작업 유형에서 **전자 메일/SMS/푸시/음성을**선택합니다.
1. 글로벌 관리자 알림 과 같은 작업 이름을 **입력합니다.**
1. 작업 **유형을** **이메일/SMS/푸시/음성으로**선택합니다.
1. **세부 정보 편집을** 선택하여 구성할 알림 방법을 선택하고 필요한 연락처 정보를 입력한 다음 **확인을** 선택하여 세부 정보를 저장합니다.
1. 트리거할 작업을 추가합니다.
1. **확인**을 선택합니다.

## <a name="validate-accounts-regularly"></a>계정 의 정기적인 유효성 검사

직원에게 비상 액세스 계정을 사용하도록 교육하고 비상 액세스 계정의 유효성을 검사하는 경우 최소한 정기적으로 다음 단계를 수행합니다.
