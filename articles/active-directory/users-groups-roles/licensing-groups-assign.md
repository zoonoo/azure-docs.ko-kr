...
이 문서에서는 사용자 그룹에 제품 라이선스를 할당하고 Azure Active Directory(Azure AD)에서 제품 라이선스가 올바르게 라이선스되었는지 확인하는 방법을 안내합니다.
...
1. 라이선스 관리자 계정으로 [**Azure AD 관리자 센터에**](https://aad.portal.azure.com) 로그인합니다. 라이선스를 관리하려면 계정이 라이선스 관리자, 사용자 관리자 또는 글로벌 관리자여야 합니다.

1. **라이선스를** 선택하여 테넌트의 모든 라이선스 제품을 보고 관리할 수 있는 페이지를 엽니다.

1. **모든 제품에서**제품 이름을 선택하여 Office 365 엔터프라이즈 E5 및 엔터프라이즈 이동성 + 보안 E3를 모두 선택합니다. 과제를 시작하려면 페이지 상단에서 **할당을** 선택합니다.

   ![라이선스 를 할당할 제품 선택](./media/licensing-groups-assign/licenses-all-products-assign.png)
  
1. 라이선스 **할당** 페이지에서 사용자 **및 그룹을** 선택하여 사용자 및 그룹 목록을 엽니다.

1. 사용자 또는 그룹을 선택한 다음 페이지 하단의 **선택** 단추를 사용하여 선택 사항을 확인합니다.

1. 라이선스 **할당** 페이지에서 이전에 선택한 두 제품에 포함된 모든 서비스 계획을 표시하는 **할당 옵션을**클릭합니다. **Yammer 엔터프라이즈를** 찾아 **끄면** 제품 라이센스에서 해당 서비스를 사용하지 않도록 설정합니다. **라이센스 옵션**하단에서 **확인을** 클릭하여 확인합니다.

   ![라이센스에 대한 서비스 계획 선택](./media/licensing-groups-assign/assignment-options.png)
  
1. 과제를 완료하려면 라이센스 **할당** 페이지에서 페이지 하단의 **할당을** 클릭합니다.

1. 오른쪽 위 모서리에 프로세스의 상태와 결과를 보여 주는 알림이 표시됩니다. 그룹에 대한 할당을 완료할 수 없으면(예를 들어 그룹의 기존 라이선스 때문에) 알림을 클릭하여 실패 세부 정보를 확인합니다.

그룹에 라이선스를 할당할 때 Azure AD는 해당 그룹의 모든 기존 구성원을 처리합니다. 이 프로세스는 그룹의 크기에 따라 다소 시간이 걸릴 수 있습니다. 다음 단계에서는 프로세스가 완료되었는지와 문제 해결을 위해 추가적인 관리가 필요한지를 확인하는 방법을 설명합니다.

## <a name="step-2-verify-that-the-initial-assignment-has-finished"></a>2단계: 초기 할당이 완료되었는지 확인

1. Azure **활성 디렉터리** > **그룹으로**이동합니다. 라이선스가 할당된 그룹을 선택합니다.

1. 그룹 페이지에서 **라이선스를 선택합니다.** 그러면 사용자에게 라이선스가 완전히 할당되었는지, 확인해야 할 오류가 있는지 신속하게 확인합니다. 다음과 같은 정보가 제공됩니다.

   - 현재 그룹에 할당된 서비스 라이센스입니다. 사용하도록 설정된 특정 서비스를 표시하고 변경을 수행할 항목을 선택합니다.

   - 변경 내용이 처리중이거나 모든 사용자 구성원에 대한 처리가 완료된 경우 사용할 수 있는 최신 라이선스 변경의 상태 업데이트입니다.

   - 오류 상태에 있는 사용자 라이선스 할당에 대한 정보입니다.

   ![라이선스 오류 및 라이센스 상태](./media/licensing-groups-assign/assignment-errors.png)

1. **Azure Active Directory** > **사용자 및 그룹** > *이름* > **감사 로그에서**라이선스 처리에 대한 자세한 정보를 확인합니다. 다음 활동을 확인하십시오.
...
1. Azure **Active Directory** > **그룹으로**이동하여 라이선스가 할당된 그룹을 찾습니다.
1. 그룹 페이지에서 **라이선스를 선택합니다.** 페이지 상단의 알림에는 라이선스를 할당할 수 없는 사용자가 10개 있음을 보여 주는 것입니다. 이 그룹에 대한 라이선스 오류 상태의 모든 사용자 목록을 보려면 이 그룹을 엽니다.
...
1. 사용자의 라이선스 페이지를 열 사용자를 **선택합니다.** 이 페이지에는 현재 사용자에게 할당된 모든 라이선스가 표시됩니다. 이 예에서는 사용자에게 **키오스크 사용자** 그룹에서 상속된 Office 365 Enterprise E1 라이선스가 있습니다. 이 라이선스는 시스템에서 **HR Department** 그룹에 적용하려고 하는 E3 라이선스와 충돌합니다. 결과적으로 해당 그룹의 어떠한 라이선스도 사용자에게 할당되지 않았습니다.

   ![사용자에 대한 모든 라이선스 충돌 보기](./media/licensing-groups-assign/user-licence-conflicting-service-plans.png)

1. 이 충돌을 해결하기 위해 **키오스크 사용자** 그룹에서 해당 사용자를 제거합니다. Azure AD에서 변경 내용을 처리한 후에 **HR Department** 라이선스가 올바르게 할당됩니다.

## <a name="next-steps"></a>다음 단계

그룹을 사용하는 라이선스 할당에 대한 기능 집합에 대해 자세히 알아보려면 다음 문서를 참조하십시오.
