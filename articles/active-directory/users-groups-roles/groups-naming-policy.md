---
title: Azure Active Directory에서 그룹 이름 지정 정책 적용 | 마이크로 소프트 문서
description: Azure Active Directory에서 Office 365 그룹에 대 한 이름 지정 정책을 설정 하는 방법
...
ms.locfileid: "79497885"
---
# <a name="enforce-a-naming-policy-on-office-365-groups-in-azure-active-directory"></a>Azure Active Directory에서 Office 365 그룹에 이름 지정 정책을 적용합니다.
...
> [!IMPORTANT]
> Office 365 그룹에 대한 Azure AD 이름 지정 정책을 사용하려면 하나 이상의 Office 365 그룹의 구성원인 각 고유 사용자에 대해 Azure Active Directory Premium P1 라이선스 또는 Azure AD Basic EDU 라이선스를 소유하되 반드시 할당할 필요는 없습니다.

명명 정책은 그룹을 만드는 데 적용되거나 워크로드(예: Outlook, Microsoft Team, SharePoint, Exchange 또는 Planner)에서 만든 그룹을 편집하는 데 적용됩니다. 이 정책은 그룹 이름 및 그룹 별칭 둘 다에 적용됩니다. Azure AD에서 이름 지정 정책을 설정하고 기존 Exchange 그룹 이름 지정 정책이 있는 경우 조직에 Azure AD 명명 정책이 적용됩니다.

그룹 이름 지정 정책이 구성되면 최종 사용자가 만든 새 Office 365 그룹에 정책이 적용됩니다. 명명 정책은 전역 관리자 또는 사용자 관리자와 같은 특정 디렉터리 역할에는 적용되지 않습니다(그룹 이름 지정 정책에서 제외된 역할의 전체 목록은 아래 참조). 기존 Office 365 그룹의 경우 구성 시 정책이 즉시 적용되지 않습니다. 그룹 소유자가 이러한 그룹에 대한 그룹 이름을 편집하면 이름 지정 정책이 적용됩니다.

## <a name="naming-policy-features"></a>명명 정책 기능

다음 두 가지 방법으로 그룹에 대한 명명 정책을 적용할 수 있습니다.
...
명명 정책을 구성하려면 folowing 역할 중 하나가 필요합니다.
- 전역 관리자
- 그룹 관리자
- 사용자 관리자
...
## <a name="configure-naming-policy-in-azure-portal"></a>Azure 포털에서 이름 지정 정책 구성

1. 그룹 관리자 계정으로 [Azure AD 관리자 센터에](https://aad.portal.azure.com) 로그인합니다.
1. **그룹**을 선택한 다음, **명명 정책**을 선택하여 명명 정책 페이지를 엽니다.

    ![관리 센터에서 명명 정책 페이지 열기](./media/groups-naming-policy/policy.png)

### <a name="view-or-edit-the-prefix-suffix-naming-policy"></a>접두사 접미사 이름 지정 정책 보기 또는 편집
...
1. 새 정책이 적용될 변경 내용을 저장하려면 **저장**을 선택합니다.

### <a name="edit-custom-blocked-words"></a>사용자 지정 차단 단어 편집
...
3. AzureADPreview의 최신 버전을 설치합니다.
   ...
   신뢰할 수 없는 리포지토리에 액세스하라는 메시지가 표시되면 **Y를**입력합니다. 새 모듈을 설치하는 데 몇 분 정도 걸릴 수 있습니다.
...
1. 다음 예제와 같이 새 정책이 적용될 설정을 저장합니다.
...
## <a name="export-or-import-custom-blocked-words"></a>사용자 지정 차단 단어 내보내기 또는 가져오기

자세한 내용은 [그룹 설정을 구성하기 위한 Azure Active Directory cmdlet](groups-settings-cmdlets.md)문서를 참조하십시오.
...
## <a name="experience-across-office-365-apps"></a>Office 365 앱 전반의 경험
...
Microsoft 365 관리 센터 | Microsoft 365 관리 센터는 이름 지정 정책을 준수합니다. 사용자가 그룹 이름을 작성하거나 편집할 때 명명 정책이 자동으로 적용되며 사용자 정의 차단 단어를 입력하면 적절한 오류가 표시됩니다. Microsoft 365 관리 센터는 아직 이름 지정 정책의 미리 보기를 표시 하지 않습니다 및 사용자가 그룹 이름을 입력할 때 사용자 지정 차단 된 단어 오류를 반환 하지 않습니다.
