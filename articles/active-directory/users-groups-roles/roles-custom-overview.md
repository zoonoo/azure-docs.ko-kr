...
Azure AD 역할 기반 액세스 제어는 Azure AD의 공개 미리 보기 기능이 며 유료 Azure AD 라이선스 계획과 함께 사용할 수 있습니다. 미리 보기에 대 한 자세한 내용은 [Microsoft Azure 미리 보기의 추가 사용 약관](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)을 참조 하세요.
...
### <a name="how-azure-ad-determines-if-a-user-has-access-to-a-resource"></a>Azure AD에서 사용자에 게 리소스에 대 한 액세스 권한이 있는지 확인 하는 방법

관리 리소스에 대 한 액세스 권한이 있는지 확인 하기 위해 Azure AD에서 사용 하는 대략적인 단계는 다음과 같습니다. 이 정보를 사용 하 여 액세스 문제를 해결할 수 있습니다.

1. 사용자 (또는 서비스 주체)가 Microsoft Graph 또는 Azure AD Graph 끝점에 대 한 토큰을 가져옵니다.

1. 사용자는 발급 된 토큰을 사용 하 여 Microsoft Graph 또는 Azure AD Graph를 통해 Azure Active Directory (Azure AD)에 대 한 API 호출을 수행 합니다.

1. 환경에 따라 Azure AD는 다음 작업 중 하나를 수행 합니다.

    - 사용자의 액세스 토큰에 있는 [wids 클레임](https://docs.microsoft.com/azure/active-directory/develop/access-tokens) 을 기준으로 사용자의 역할 멤버 자격을 평가 합니다.
    - 직접 또는 그룹 멤버 자격을 통해 사용자에 게 적용 되는 모든 역할 할당을 작업을 수행할 리소스에 검색 합니다.

1. Azure AD는 API 호출의 작업이 사용자가이 리소스에 대해 보유 한 역할에 포함 되는지 여부를 결정 합니다.
