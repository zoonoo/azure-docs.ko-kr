...
2. 페이지의 오른쪽 위 모서리에 있는 사용자 계정을 클릭하여 Azure AD 테넌트를 선택합니다.
3. 왼쪽 탐색 창에서 **Azure Active Directory**를 클릭합니다.
4. **앱 등록**을 클릭하고 **추가**를 클릭합니다.
5. 응용 프로그램의 이름(예: "TodoListService")을 입력하고 응용 프로그램 종류로 “웹 응용 프로그램 및/또는 Web API”를 선택합니다.
6. 로그온 URL에 대해서는 샘플의 기준 URL(예: `https://localhost:8080`)을 입력합니다.
7. 등록이 끝나면 Azure AD는 응용 프로그램에 고유한 클라이언트 식별자인 응용 프로그램 ID를 할당합니다. 이 값은 다음 섹션에서 필요하므로 응용 프로그램 페이지에서 복사해둡니다.
8. 설정 페이지에서 AAD가 토큰을 반환할 위치인 **회신 URL**을 구성합니다. 이 샘플의 기본 위치는 `https://localhost:44326/`

* Adal.js는 OAuth 암시적 흐름을 사용하여 Azure AD와 통신합니다.  다음을 수행하여 응용 프로그램에 대한 암시적 흐름을 사용하도록 설정해야 합니다.
  * 응용 프로그램 페이지에서 **매니페스트**를 클릭하여 매니페스트 편집기를 열고 `oauth2AllowImplicitFlow` 속성을 찾습니다. 해당 값을 `true`로 설정합니다.
  * 매니페스트를 저장합니다.
