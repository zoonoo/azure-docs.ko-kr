...
이 항목에서는 셀프 서비스 등록 프로세스(바이럴 등록이라고도 함) 및 DNS 도메인 이름을 인수하는 방법을 설명합니다.
...
사용자의 셀프 서비스 등록 후 관리되지 않는 테넌트를 인수하는 중이므로 도메인 소유권을 확인하고 싶거나 관리되는 기존 테넌트에 새 도메인을 추가할 수 있습니다. 예를 들어 contoso.com이라는 이름의 도메인을 가지고 있으며 contoso.co.uk 또는 contoso.uk라는 이름의 새 도메인을 추가하려고 합니다.

## 도메인 인수란?  

이 섹션에서는 도메인 소유권을 확인하는 방법을 다룹니다.

### 도메인 유효성 검사란 무엇이며 사용해야 하는 이유는?

Azure AD를 사용하여 테넌트에서 작업을 수행하려면 DNS 도메인의 소유권이 있어야 합니다. 도메인의 유효성 검사를 사용하면 테넌트를 요구하거나 셀프 서비스 테넌트를 관리되는 테넌트로 승격시키거나 관리되는 기존 테넌트에 병합할 수 있습니다.

## 도메인 유효성 검사 예

테넌트의 DNS 인수를 수행하는 데는 다음 두 가지 방법이 있습니다.

+ 내부 인수(예: 관리자가 셀프 서비스와 관리되지 않는 테넌트를 검색하여 관리되는 테넌트로 바꾸려고 함)
+ 외부 인수(예: 관리자가 새 도메인을 관리되는 테넌트에 추가하려고 함)

### 내부 인수 - 셀프 서비스, 관리되지 않는 테넌트를 관리되는 테넌트로 승격

내부 인수를 수행하면 관리되지 않는 테넌트가 관리되는 테넌트로 변환됩니다. DNS 도메인 이름 유효성 검사를 완료해야 합니다. DNS 영역에 MX 레코드 또는 TXT 레코드를 만드는 경우 다음을 수행합니다.

+ 도메인 소유 확인
+ 관리되는 테넌트로 생성
+ 테넌트의 전역 관리자가 됨

Bellows College의 IT 관리자가 셀프 서비스 제공 사항에 등록한 교내 사용자를 찾았다고 가정해 보겠습니다. IT 관리자는 BellowsCollege.com이란 DNS 이름의 등록된 소유자로서 Azure에서 DNS 이름의 소유권을 확인한 다음 관리되지 않는 테넌트를 인수할 수 있습니다. 그러면 이 테넌트는 관리되는 테넌트가 되며 IT 관리자에게 BellowsCollege.com 디렉터리에 대한 전역 관리자 역할이 할당됩니다.

### 외부 인수 - 셀프 서비스 테넌트를 관리되는 기존 테넌트에 병합
...
## DNS 도메인 이름 인수를 수행하는 방법

도메인 유효성 검사를 수행하는 방법으로 다음과 같은 몇 가지 옵션이 있습니다(원하는 경우 인수 수행).

1.  Azure 관리 포털

	도메인을 추가하면 인수가 트리거됩니다. 해당 도메인에 대한 테넌트가 이미 존재하면 외부 인수를 수행할 수 있습니다.

	자격 증명을 사용하여 Azure 포털에 로그인합니다. 기존 테넌트로 이동한 다음 **도메인 추가**로 이동합니다.
...
다음 순서도는 테넌트와 셀프 서비스 등록에 대한 이러한 매개 변수 및 그 결과 조건의 다양한 모든 조합을 설명합니다.
