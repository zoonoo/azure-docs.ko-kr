...
Zscaler Two에서 Azure AD의 **사용자 이름** 값을 **Username** 값으로 할당하여 연결 관계를 설정합니다.
...
4. **[Zscaler Two 테스트 사용자 만들기](#creating-a-zscaler-two-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Zscaler Two에 만듭니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 Zscaler Two 응용 프로그램에서 Single Sign-On을 구성합니다.

**Zscaler Two에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **Zscaler Two** 응용 프로그램 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
3. **Zscaler Two 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_url.png)

   [로그온 URL] 텍스트 상자에 사용자가 ZScaler Two 응용 프로그램에 로그인하는 데 사용하는 URL을 입력합니다.
...
6. **Zscaler Two 구성** 섹션에서 **Zscaler Two 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-zscaler-two-tutorial/tutorial_zscalertwo_configure.png) 

7. 다른 웹 브라우저 창에서 Zscaler Two 회사 사이트에 관리자로 로그인합니다.
...
11. **SAML Single Sign-On 매개 변수 구성** 대화 상자 페이지에서 다음 단계를 수행하고 **완료**를 클릭합니다.
    ...
    c. **Zscaler pem**을 클릭하여 다운로드한 인증서를 업로드합니다.
...
5. 프록시 서버 섹션에서 다음 단계를 수행합니다.   
    ...
    b. 주소 텍스트 상자에 **gateway.zscalertwo.net**을 입력합니다.

    c. 포트 텍스트 상자에 **80**을 입력합니다.
...
Azure AD 사용자가 ZScaler Two에 로그인할 수 있도록 하려면 ZScaler Two로 프로비전되어야 합니다. ZScaler Two의 경우, 수동으로 프로비전합니다.

### <a name="to-configure-user-provisioning-perform-the-following-steps"></a>사용자 프로비저닝을 구성하려면

1. **Zscaler Two** 테넌트에 로그인합니다.
...
3. **사용자 관리**를 클릭합니다.   
     ...
     ![추가](./media/active-directory-saas-zscaler-two-tutorial/ic781036.png "추가")
...
> [!NOTE]
> 다른 ZScaler Two 사용자 계정 생성 도구 또는 ZScaler에서 제공한 API를 사용하여 Azure AD 사용자 계정을 프로비전할 수 있습니다.
...
**Zscaler Two에 Britta Simon을 할당하려면 다음 단계를 수행합니다.**

1. Azure Portal에서 응용 프로그램 보기를 연 다음 디렉터리 보기로 이동하고 **엔터프라이즈 응용 프로그램**으로 이동한 후 **모든 응용 프로그램**을 클릭합니다.

    ![사용자 할당][201] 

2. 응용 프로그램 목록에서 **Zscaler Two**를 선택합니다.
...
액세스 패널에서 Zscaler Two 타일을 클릭하면 Zscaler Two 응용 프로그램에 자동으로 로그온됩니다.
