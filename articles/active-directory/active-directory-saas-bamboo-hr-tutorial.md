...
3. **[BambooHR 테스트 사용자 만들기](#creating-a-bamboohr-test-user)** - Britta Simon의 Azure AD 표현과 연결되는 해당 사용자를 BambooHR에 만듭니다.
...
3. **BambooHR 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<company>.bamboohr.com`
    
    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. 이 값을 실제 로그온 URL로 업데이트합니다. 이 값을 얻으려면 [BambooHR 클라이언트 지원팀](https://www.bamboohr.com/contact.php)에 문의하세요. 
...
7. 홈페이지에서 다음 단계를 수행합니다.
    ...
    b. 왼쪽의 앱 메뉴에서 **Single Sign-on**을 클릭합니다.
...
8. **SAML Single Sign-On** 섹션에서 다음 단계를 수행합니다.
    ...
    a. **SAML Single Sign-On 서비스 URL** 값을 **SSO 로그인 URL** 텍스트 상자에 붙여 넣습니다.
      
    b. Azure Portal에서 다운로드한 Base-64로 인코딩된 인증서를 메모장에서 열고, 콘텐츠를 클립보드에 복사한 다음, **X.509 인증서** 텍스트 상자에 붙여 넣습니다.
