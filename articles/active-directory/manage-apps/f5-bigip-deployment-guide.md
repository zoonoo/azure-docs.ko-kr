...
7. **기본** 메뉴를 단계별로 실행 하 고 다음 설정을 사용 합니다.
 ...
 | **인스턴스 세부 정보**|  |
 |VM 이름| 예제 큰 IP-VM |
 |Azure 지역 | 빅 IP VM에 대 한 대상 Azure 지역 |
...
7. 안정적인 NTP 원본을 지정 하 고 **추가**, **업데이트** 를 차례로 선택 합니다. 예, `time.windows.com`
...
9. 세 개의 드롭다운 목록에서 암호 없이 가져온 와일드 카드 인증서를 선택 하 고 완료 **추가** 를 선택  >  **Finished** 합니다.
