...
애플리케이션 프록시를 성공적으로 배포하려면 커넥터가 하나 이상 필요하지만, 복원력을 높이기 위해 두 개 이상을 사용하는 것이 좋습니다. Windows Server 2012 R2 이상을 실행 하는 컴퓨터에 커넥터를 설치 합니다. 커넥터는 애플리케이션 프록시 서비스 및 게시하는 온-프레미스 애플리케이션과 통신해야 합니다.
...
예상된 트래픽 볼륨을 처리하기 위해 커넥터 간에 충분한 용량을 계획했는지 확인하는 것이 중요합니다. 고가용성 및 확장성을 제공 하기 위해 커넥터 그룹에는 두 개 이상의 커넥터가 있는 것이 좋습니다. 3 개의 커넥터가 있는 경우 언제 든 지 컴퓨터를 서비스 해야 할 수 있습니다.
...
커넥터는 애플리케이션 프록시 서비스로 요청을 전송할 수 있는 네트워크 어느 곳에나 설치할 수 있습니다. 중요한 것은 커넥터를 실행 중인 컴퓨터에 앱에 대한 액세스가 있는 것입니다. 회사 네트워크 내부 또는 클라우드에서 실행되는 가상 머신에 커넥터를 설치할 수 있습니다. 커넥터는 완충 영역 (DMZ)이 라고도 하는 경계 네트워크 내에서 실행할 수 있지만, 네트워크가 안전 하 게 유지 되도록 모든 트래픽이 아웃 바운드 되기 때문에 반드시 필요한 것은 아닙니다.
...
커넥터 성능은 CPU 및 네트워킹에 의해 한정됩니다. TLS 암호화 및 암호 해독에는 CPU 성능이 필요 하지만, 네트워킹은 Azure에서 응용 프로그램 및 온라인 서비스에 대 한 빠른 연결을 제공 하는 데 중요 합니다.
...
- **도메인 컨트롤러**: 커넥터가 Kerberos 제한 위임을 사용 하 여 SSO (Single Sign-On)를 수행 하는 경우 백 엔드에 요청을 보내기 전에 도메인 컨트롤러에 연결 합니다. 커넥터에는 Kerberos 티켓 캐시가 있지만 사용량이 많은 환경에서는 도메인 컨트롤러의 응답성이 성능에 영향을 줄 수 있습니다. 이 문제는 Azure에서 실행되지만 온-프레미스 도메인 컨트롤러와 통신하는 커넥터에서 좀 더 자주 발생합니다.
...
서비스 창에서 서비스 상태를 검사할 수 있습니다. 커넥터는 두 개의 Windows 서비스(실제 커넥터와 업데이터)로 구성됩니다. 둘 다 항상 실행되어야 합니다.
 ...
 ![예: Azure AD 서비스 로컬을 보여 주는 서비스 창](./media/application-proxy-connectors/aad-connector-services.png)
