...
1. 디렉터리에 대한 관리 사용자 이름 및 암호를 포함하는 Windows PowerShell 자격 증명 개체 `$cred`를 만듭니다. 다음 명령을 실행 하 *\<username\>* 고 및를 바꿉니다 *\<password\>* .
...
3. 다음 Windows PowerShell 명령을 실행 하 고를 \<tenant GUID\> 디렉터리 ID로 바꿉니다.
