...
1.  내부 URL이 애플리케이션의 모든 관련 링크를 포함하는 루트인지 확인합니다. 이렇게 하면 모든 링크를 동일한 애플리케이션 내에 게시된 콘텐츠로 해결할 수 있습니다.
    ...
    내부 URL을 변경하지만 사용자의 방문 페이지를 변경하지 않으려는 경우 홈 페이지 URL을 이전에 게시된 내부 URL로 변경합니다. 이 작업은 "Azure Active Directory"-&gt; 앱 등록&gt; - 응용&gt; 프로그램 선택 - 브랜딩으로 이동하여 수행할 수 있습니다. 브랜딩 섹션에는 원하는 방문 페이지로 조정할 수 있는 "홈 페이지 URL" 필드가 표시됩니다. 여전히 레거시 앱 등록을 사용하는 경우 속성 탭에 "홈 페이지 URL" 세부 정보가 표시됩니다. 
    
    > [!IMPORTANT]
    > 위의 내용을 변경하려면 Azure AD에서 응용 프로그램 개체를 수정할 수 있는 권한이 필요합니다. 사용자에게 Azure AD의 응용 프로그램 수정 권한을 부여하는 응용 프로그램 [관리자](../users-groups-roles/roles-delegate-app-roles.md#assign-built-in-application-admin-roles) 역할을 할당해야 합니다.
    >
