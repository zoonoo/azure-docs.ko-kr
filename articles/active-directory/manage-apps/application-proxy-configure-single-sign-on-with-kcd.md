...
이 다이어그램은 사용자가 IWA를 사용하는 온-프레미스 응용 프로그램에 액세스하려고 할 때의 흐름을 설명합니다.
...
4. 응용 프로그램 프록시는 토큰의 유효성을 검사하고 UPN(사용자 주체 이름)을 검색한 다음 커넥터가 UPN및 서비스 주체 이름(SPN)을 이중으로 인증된 보안 채널을 통해 가져옵니다.
5. 커넥터는 온프레미스 AD와 Kerberos 제한 위임(KCD) 협상을 수행하여 사용자를 가장하여 Kerberos 토큰을 응용 프로그램에 가져옵니다.
...
1. Active 디렉터리에서 **도구** > **사용자 및 컴퓨터로**이동합니다.
2. 커넥터를 실행하는 서버를 선택합니다.
3. 오른쪽 단추를 클릭하고 **속성** > **위임을**선택합니다.
...
2. 웹 `principalsallowedtodelegateto` 응용 프로그램의 서비스 계정(컴퓨터 또는 전용 도메인 사용자 계정)의 속성을 사용하여 응용 프로그램 프록시(커넥터)에서 Kerberos 인증 위임을 활성화합니다. 응용 프로그램 서버가 컨텍스트에서 `webserviceaccount` 실행중이며 위임 서버는 `connectorcomputeraccount`입니다. 아래 명령을 도메인 컨트롤러(Windows Server 2012 R2 이상 실행)에서 `webserviceaccount`실행합니다. 두 계정에 대해 플랫 이름(UPN이 아닌 이름)을 사용합니다.

   의 `webserviceaccount` 컴퓨터 계정인 경우 다음 명령을 사용합니다.
   ...
   의 `webserviceaccount` 사용자 계정인 경우 다음 명령을 사용합니다.
...
애플리케이션 프록시는 사용자가 클라우드 및 온-프레미스에서 정확히 동일한 ID를 사용한다고 가정합니다. 그러나 일부 환경에서는 회사 정책이나 응용 프로그램 종속성으로 인해 조직에서 로그인에 대체 IT를 사용해야 할 수 있습니다. 이러한 경우 단일 사인온에 KCD를 계속 사용할 수 있습니다. 각 애플리케이션에 **위임된 로그인 ID**를 구성하여 Single Sign-On을 수행할 때 어떤 ID를 사용해야 하는지 지정합니다.  
...
* 온프레미스 및 클라우드에서 서로 다른 별칭을 사용합니다. 예제: joe-johns@contoso.com 및 joej@contoso.com  
...
SSO 프로세스에 오류가 있으면 [문제 해결에](application-proxy-back-end-kerberos-constrained-delegation-how-to.md)설명된 대로 커넥터 컴퓨터 이벤트 로그에 나타납니다.
