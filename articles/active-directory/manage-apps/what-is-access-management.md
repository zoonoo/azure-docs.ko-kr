...
* **그룹 기반 할당(Azure AD 프리미엄 P1 또는 P2 필요)** 디렉터리 글로벌 관리자 권한이 있는 IT 관리자는 응용 프로그램에 그룹을 할당할 수 있습니다. 특정 사용자의 액세스는 애플리케이션에 액세스하려고 할 때 그룹의 멤버인지 여부로 결정됩니다. 즉, 관리자는 "할당된 그룹의 현재 멤버는 애플리케이션에 액세스한다"는 내용의 할당 규칙을 효율적으로 만들 수 있습니다. 이 할당 옵션을 사용하여 관리자는 [특성 기반 동적 그룹](../fundamentals/active-directory-groups-create-azure-portal.md), 외부 시스템 그룹(예: 온-프레미스 Active Directory 또는 Workday), 관리자 관리 또는 셀프 서비스 관리된 그룹을 포함하는 Azure AD 그룹 관리 옵션 중 하나에서 혜택을 줄 수 있습니다. 단일 그룹은 쉽게 여러 앱에 할당할 수 있으며 할당 선호도가 있는 애플리케이션은 할당 규칙을 공유하여 전반적인 관리 복잡성을 줄일 수 있습니다. 현재 중첩된 그룹 구성원은 이번 애플리케이션에 대한 그룹 기반 할당에 지원되지 않습니다.

이러한 두 가지 할당 모드 관리자를 사용하면 바람직한 할당 관리 방법을 달성할 수 있습니다.

### <a name="requiring-user-assignment-for-an-app"></a>앱에 대한 사용자 할당 필요

특정 유형의 응용 [프로그램에서는 사용자를 응용 프로그램에 할당하도록 요구할 수](assign-user-or-group-access-portal.md#configure-an-application-to-require-user-assignment)있습니다. 이렇게 하면 응용 프로그램에 명시적으로 할당한 사용자를 제외한 모든 사용자가 로그인하지 못하게 됩니다. 다음 유형의 응용 프로그램은 이 옵션을 지원합니다.

* SAML 기반 인증을 통해 페더레이션된 단일 사인온(SSO)을 위해 구성된 응용 프로그램
* Azure Active Directory 사전 인증을 사용하는 응용 프로그램 프록시 응용 프로그램
* 사용자 또는 관리자가 해당 응용 프로그램에 동의한 후 OAuth 2.0 / OpenID 연결 인증을 사용하는 Azure AD 응용 프로그램 플랫폼에 빌드된 응용 프로그램입니다. 특정 엔터프라이즈 응용 프로그램은 로그인할 수 있는 사람을 추가로 제어할 수 있습니다.

사용자 할당이 *필요하지 않은*경우 할당되지 않은 사용자는 내 앱 액세스 패널에 앱을 볼 수 없지만 응용 프로그램 자체에 계속 로그인할 수 있습니다(SP 시작 사인온이라고도 함) 또는 응용 프로그램의 **속성** 페이지(IDP 시작 사인온이라고도 함)에서 사용자 **액세스 URL을** 사용할 수 있습니다.

일부 응용 프로그램의 경우 사용자 할당을 요구하는 옵션은 응용 프로그램의 속성에서 사용할 수 없습니다. 이러한 경우 PowerShell을 사용하여 서비스 주체에서 appRoleAssignmentRequired 속성을 설정할 수 있습니다.

### <a name="determining-the-user-experience-for-accessing-apps"></a>앱 액세스에 대한 사용자 환경 결정

Azure AD는 조직의 최종 사용자에게 [응용 프로그램을 배포하는 몇 가지 사용자 지정 가능한 방법을](end-user-experiences.md) 제공합니다.

* Azure AD 내 앱 액세스 패널
* Office 365 애플리케이션 실행 프로그램
* 페더레이션 앱에 직접 로그온(서비스 홍보)
* 페더레이션된 앱, 암호로 보호된 앱 또는 기존 앱에 대한 딥 링크

엔터프라이즈 앱에 할당된 사용자가 액세스 패널 및 Office 365 응용 프로그램 시작 프로그램에서 볼 수 있는지 여부를 확인할 수 있습니다.

## <a name="example-complex-application-assignment-with-azure-ad"></a>예: Azure AD를 사용 하 고 복잡 한 응용 프로그램 할당
...
관리자는 [Azure AD 조건부 액세스를](../active-directory-conditional-access-azure-portal.md) 사용하여 특정 역할에 대한 액세스 정책을 설정할 수 있습니다. 이러한 정책은 기업 환경 외부 및 Multi-Factor Authentication 또는 디바이스 요구에 액세스가 허용되는지 여부를 포함하여 다양한 경우에 액세스를 달성할 수 있습니다.

## <a name="access-to-microsoft-applications"></a>Microsoft 응용 프로그램에 대한 액세스
...
- Microsoft 또는 제3자가 누구나 자유롭게 게시하는 응용 프로그램의 경우 사용자는 사용자 [동의를](configure-user-consent.md)통해 액세스 권한을 부여받을 수 있습니다. 즉, Azure AD 작업 또는 학교 계정으로 응용 프로그램에 로그인하고 계정의 일부 제한된 데이터 집합에 액세스할 수 있습니다.
- Microsoft 또는 제3자가 누구나 자유롭게 게시하는 응용 프로그램의 경우 [사용자는 관리자 동의를](manage-consent-requests.md)통해 액세스 권한을 부여받을 수도 있습니다. 즉, 관리자는 전역 관리자 계정 사용하여 애플리케이션에 로그인하고 조직의 모든 사용자에게 액세스 권한을 부여하도록 조직의 모든 사용자가 애플리케이션을 사용할 수 있게 결정했습니다.

일부 응용 프로그램은 이러한 메서드를 결합합니다. 예를 들어, 특정 Microsoft 응용 프로그램은 Office 365 구독의 일부이지만 여전히 동의가 필요합니다.

사용자는 Office 365 포털을 통해 Office 365 응용 프로그램에 액세스할 수 있습니다. 디렉터리의 **사용자 설정에서** [Office 365 가시성 토글을](hide-application-from-user-portal.md) 사용하여 내 앱 액세스 패널에서 Office 365 응용 프로그램을 표시하거나 숨길 수도 있습니다. 

엔터프라이즈 앱과 마찬가지로 Azure 포털을 통해 특정 Microsoft 응용 프로그램에 사용자를 할당하거나 PowerShell을 사용하여 포털 옵션을 사용할 수 없는 경우 [사용자를 할당할](assign-user-or-group-access-portal.md) 수 있습니다.
