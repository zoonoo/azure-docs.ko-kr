...
이전 빠른 시작에서는 Azure AD 테넌트에 애플리케이션을 추가했습니다. 애플리케이션을 추가하면 Azure AD 테넌트는 해당 애플리케이션이 앱의 ID 공급자라는 것을 알 수 있습니다. 이제 앱의 몇 가지 속성을 구성하겠습니다.
...
1. Azure AD Portal에서 **엔터프라이즈 애플리케이션**을 선택한 다음, 구성하려는 애플리케이션을 찾아 선택합니다.
2. [관리] 섹션에서 **속성**을 선택하여 편집할 속성 창을 엽니다.
    ![속성 화면과 편집 가능한 앱 속성 표시](media/add-application-portal/edit-properties.png)
3. 구성에 사용할 수 있는 옵션을 잠시 살펴봅니다.
    ...
    - **사용자가 볼 수 있습니까?** 는 앱에 할당된 사용자가 액세스 패널(https://myapps.microsoft.com) 및 O365 앱 시작 관리자(Office 365 또는 Microsoft 365 웹 사이트의 왼쪽 위에 있는 와플 메뉴)에서 앱을 볼 수 있는지 여부를 결정합니다.
...
2. Azure AD Portal에서 **엔터프라이즈 애플리케이션**을 선택한 다음, 구성하려는 애플리케이션을 찾아 선택합니다.
3. [관리] 섹션에서 **속성**을 선택하여 편집할 속성 창을 엽니다. 
4. 아이콘을 선택하여 로고를 업로드합니다.
5. 완료되면 **저장**을 선택합니다. 
    ![앱의 [속성] 페이지에서 로고를 변경하는 방법 표시](media/add-application-portal/change-logo.png)
