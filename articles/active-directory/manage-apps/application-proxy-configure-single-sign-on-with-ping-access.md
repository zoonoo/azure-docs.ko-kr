...
1. **엔터프라이즈 응용 프로그램**  >  **새 응용**프로그램  >  **온-프레미스 응용 프로그램 추가**를 선택 합니다. **사용자 고유의 온-프레미스 응용 프로그램 추가** 페이지가 나타납니다.
...
1. 새 응용 프로그램에 대 한 정보를 사용 하 여 필수 필드를 작성 합니다. 설정에 대 한 아래 지침을 사용 합니다.
   ...
   1. **내부 url**: 회사 네트워크에 있을 때 일반적으로 앱의 로그인 페이지로 이동 하는 URL을 제공 합니다. 이 시나리오의 경우 커넥터는 응용 프로그램의 front 페이지로 연결 액세스 프록시를 처리 해야 합니다. `https://<host name of your PingAccess server>:<port>` 형식을 사용합니다. 포트는 기본적으로 3000이지만 PingAccess에서 구성할 수 있습니다.
      ...
      > [!WARNING]
      > 이러한 유형의 Single Sign-On에서 내부 URL은를 사용 해야 `https` 하며 사용할 수 없습니다 `http` .
...
1. 응용 프로그램 사이드바에서 사용자 **및 그룹**  >  **사용자**사용자  >  **및 그룹 추가 ( \<Number> 선택 됨)** 를 선택 합니다. 사용자가 선택할 수 있는 사용자 및 그룹의 목록이 표시 됩니다.
...
**필드를 업데이트 합니다 `acceptMappedClaims` .**
...
1. 필드를 검색 하 `acceptMappedClaims` 고 값을로 변경 `True` 합니다.
1. **저장**을 선택합니다.
...
이 시나리오의 연결에 대 한 자세한 단계는 Ping Id 설명서를 참조 하세요. Ping Id 웹 사이트에서 [Microsoft Azure AD 응용 프로그램 프록시를 사용 하 여 게시 된 응용 프로그램을 보호 하려면 AZURE AD에 대 한 연결 액세스 구성](https://support.pingidentity.com/s/document-item?bundleId=pingaccess-52&topicId=agents/azure/pa_c_PAAzureSolutionOverview.html) 의 지침에 따라 [최신 버전의 웹 액세스](https://www.pingidentity.com/en/lp/azure-download.html?)를 다운로드 합니다.

이러한 단계를 통해 액세스를 설치 하 고, 연결 액세스 계정 (아직 없는 경우)을 설정할 수 있습니다. 그런 다음 Azure AD Openid connect Connect (OIDC) 연결을 만들려면 Azure AD 포털에서 복사한 **디렉터리 (테 넌 트) ID** 값을 사용 하 여 토큰 공급자를 설정 합니다. 그런 다음, **응용 프로그램 (클라이언트) ID** 및 값을 사용 하 여 웹 세션을 만듭니다 `PingAccess key` . 그런 다음 ID 매핑을 설정하고 가상 호스트, 사이트 및 애플리케이션을 만들 수 있습니다.
