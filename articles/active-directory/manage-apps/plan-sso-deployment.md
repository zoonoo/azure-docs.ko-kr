...
- **암호 기반 single sign-on** 사용자는 처음으로 사용자 이름 및 암호를 사용 하 여 응용 프로그램에 로그인 합니다. 첫 번째 로그온 후에는 Azure AD가 사용자 이름 및 암호를 애플리케이션에 제공합니다. 암호 기반 Single Sign-On을 사용하면 웹 브라우저 확장 또는 모바일 앱을 사용하여 안전하게 애플리케이션 암호를 저장하고 재생할 수 있습니다. 이 옵션은 응용 프로그램에서 제공 하는 기존 로그인 프로세스를 활용 하 여 관리자가 암호를 관리할 수 있도록 하 고 사용자가 암호를 알 필요가 없습니다.
...
인증 방법을 선택하려면 시간, 기존 인프라, 복잡성 및 선택을 구현하는 데 드는 비용을 고려해야 합니다. 이러한 요소는 조직마다 다르며 시간이 흐름에 따라 변경될 수 있습니다. 특정 시나리오와 가장 일치 하는 항목을 선택 해야 합니다. 자세한 내용은 [Azure Active Directory 하이브리드 id 솔루션에 적합 한 인증 방법 선택](https://docs.microsoft.com/azure/security/azure-ad-choose-authn)을 참조 하세요.
