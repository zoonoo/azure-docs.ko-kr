...
이 문서에서는 HRD (홈 영역 검색) 정책을 사용 하 여 페더레이션된 사용자에 대 한 Azure Active Directory 인증 동작을 구성 하는 방법을 소개 합니다.  자동 가속을 사용 하 여 사용자 이름 입력 화면을 건너뛰고 자동으로 사용자를 페더레이션 로그인 끝점으로 전달 하는 방법을 다룹니다.  자동 가속을 더 이상 구성 하지 않는 것이 좋습니다 .이는 FIDO 및 방해 공동 작업과 같은 더 강력한 인증 방법을 사용할 수 없기 때문입니다.

## <a name="home-realm-discovery"></a>홈 영역 검색

HRD (홈 영역 검색)은 사용자가 로그인 시 인증 해야 하는 id 공급자 ("IdP")를 결정 하는 데 Azure AD (Azure Active Directory)를 허용 하는 프로세스입니다.  사용자가 Azure AD 테넌트에 로그인하여 리소스 또는 Azure AD 일반 로그인 페이지에 액세스할 때 사용자는 UPN(사용자 이름)을 입력합니다. Azure AD는 이 기능을 사용하여 사용자가 로그인해야 하는 위치를 검색합니다.

사용자는 인증할 다음 id 공급자 중 하나로 이동 됩니다.

- 사용자의 홈 테넌트(사용자가 액세스하려고 하는 리소스와 같은 테넌트일 수 있음)

- Microsoft 계정  사용자는 인증을 위해 소비자 계정을 사용 하는 리소스 테 넌 트의 게스트입니다.
...
>[!NOTE]
>자동 가속화를 사용 하도록 응용 프로그램을 구성 하는 경우 사용자는 관리 되는 자격 증명 (예: FIDO)을 사용할 수 없고 게스트 사용자가 로그인 할 수 없습니다. 인증을 위해 페더레이션된 IdP로 사용자를 바로 보내는 경우 Azure Active Directory 로그인 페이지로 다시 돌아갈 방법이 없습니다. 다른 테넌트 또는 Microsoft 계정과 같은 외부 IdP로 이동해야 할 수도 있는 게스트 사용자는 홈 영역 검색 단계가 생략되므로 해당 애플리케이션에 로그인할 수 없습니다.  

페더레이션된 IdP에 대 한 자동 가속을 제어 하는 방법에는 세 가지가 있습니다.

- 응용 프로그램에 대 한 인증 요청에 [도메인 힌트](#domain-hints) 를 사용 합니다.
- [자동 가속을 강제로 적용](#home-realm-discovery-policy-for-auto-acceleration)하도록 홈 영역 검색 정책을 구성 합니다.
- 특정 응용 프로그램 또는 특정 도메인의 [도메인 힌트를 무시](prevent-domain-hints-with-home-realm-discovery.md) 하도록 홈 영역 검색 정책을 구성 합니다.
...
**OPEN ID Connect**: 쿼리 문자열 domain_hint = contoso.com입니다.

기본적으로 Azure AD는 다음 두 조건에 **모두** 해당 하는 경우 도메인에 대해 구성 된 IdP로 로그인을 리디렉션합니다.

- 도메인 힌트는 응용 프로그램의 인증 요청에 **포함 되어 있습니다** .
- 테 넌 트가 해당 도메인과 페더레이션 됩니다.

도메인 힌트가 확인 된 페더레이션된 도메인을 참조 하지 않는 경우 무시 됩니다.
...
>[!NOTE]
>도메인 힌트가 인증 요청에 포함 되어 있으며 적용 [해야](#home-realm-discovery-policy-to-prevent-auto-acceleration)하는 경우 해당 상태는 hrd 정책의 응용 프로그램에 대해 설정 된 자동 가속화를 재정의 합니다.

### <a name="home-realm-discovery-policy-for-auto-acceleration"></a>자동 가속에 대한 홈 영역 검색 정책

일부 애플리케이션은 인증 요청을 구성하는 방법을 제공하지 않습니다. 이 경우 도메인 힌트를 사용하여 자동 가속을 제어할 수는 없습니다. 자동 가속화는 동일한 동작을 얻기 위해 홈 영역 검색 정책을 통해 구성할 수 있습니다.  
 ...
 **PreferredDomain** 이 지정된 경우 테넌트에 대한 확인된 페더레이션된 도메인이 일치해야 합니다. 응용 프로그램의 모든 사용자가 해당 도메인에 로그인 할 수 있어야 합니다. 페더레이션 도메인에서 로그인 할 수 없는 사용자는 트래핑 되며 로그인을 완료할 수 없습니다.
...
또한 두 개의 테 넌 트 수준 HRD 옵션이 있습니다 (위에 표시 되지 않음).

- **AlternateIdLogin** 은 선택 사항입니다.  사용 하도록 설정 하면 사용자가 Azure AD 로그인 페이지에서 [UPN 대신 전자 메일 주소를 사용 하 여 로그인 할 수 있습니다](../authentication/howto-authentication-use-email-signin.md) .  대체 Id는 페더레이션 IDP 자동 가속화 되지 않는 사용자를 사용 합니다.

- **Domainhintpolicy** 는 [도메인 힌트가 사용자를 페더레이션 도메인으로 자동 가속화 *하지 못하도록*](prevent-domain-hints-with-home-realm-discovery.md)하는 선택적 복합 개체입니다. 이 테 넌 트 전체 설정은 도메인 힌트를 보내는 응용 프로그램이 사용자가 클라우드 관리 자격 증명을 사용 하 여 로그인 할 수 없도록 하는 데 사용 됩니다.

### <a name="priority-and-evaluation-of-hrd-policies"></a>HRD 정책의 우선 순위 및 평가

HRD 정책을 만들어 특정 조직 및 서비스 주체에 할당할 수 있습니다. 즉, 여러 정책을 특정 응용 프로그램에 적용할 수 있으므로 Azure AD가 우선 순위를 결정 해야 합니다. 규칙 집합은 적용 되는 HRD 정책 (다 수의 적용 됨)을 결정 합니다.

- 도메인 힌트가 인증 요청에 있는 경우 테 넌 트의 HRD 정책 (테 넌 트 기본값으로 설정 된 정책)을 확인 하 여 [도메인 힌트를 무시 해야](prevent-domain-hints-with-home-realm-discovery.md)하는지 확인 합니다. 도메인 힌트가 허용 되는 경우 도메인 힌트에 지정 된 동작이 사용 됩니다.
