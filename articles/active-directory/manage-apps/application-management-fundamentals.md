...
| 사용자에 게 My Apps 액세스 패널 배포 | `https://myapps.microsoft.com`의 [액세스 패널](end-user-experiences.md) 은 사용자에 게 할당 된 클라우드 기반 응용 프로그램에 대 한 단일 진입점을 제공 하는 웹 기반 포털입니다. 그룹 관리 및 셀프 서비스 암호 재설정과 같은 추가 기능을 추가 하면 사용자가 액세스 패널에서 해당 기능을 찾을 수 있습니다. [액세스 패널 배포 계획](access-panel-deployment-plan.md)을 참조 하세요.
