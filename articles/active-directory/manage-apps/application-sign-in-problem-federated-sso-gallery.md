---
title: 페더레이션된 단일 사인온 갤러리 앱에 로그인하는 문제 | 마이크로 소프트 문서
...
*오류 AADSTS70001: 식별자 'https:\//contoso.com'이 있는 응용 프로그램이 디렉터리에서 찾을 수 없습니다.*
...
SAML 요청의 `AssertionConsumerServiceURL` 값이 Azure AD에 구성된 회신 URL 값 또는 패턴과 일치하지 않습니다. SAML 요청의 값은 `AssertionConsumerServiceURL` 오류에 표시되는 URL입니다.

**해상도**

SAML `AssertionConsumerServiceURL` 요청의 값이 Azure AD에 구성된 회신 URL 값과 일치하는지 확인합니다. 내 앱 보안 브라우저 확장과 함께 Azure 포털에서 [테스트 환경을](../azuread-dev/howto-v1-debug-saml-sso-issues.md) 사용하는 경우 다음 단계를 수동으로 수행할 필요가 없습니다.
...
1.  애플리케이션이 로드되면 **기본 SAML 구성**을 엽니다. SAML 요청의 값과 일치하도록 회신 `AssertionConsumerServiceURL` URL 텍스트 상자의 값을 확인하거나 업데이트합니다.    
...
*오류 AADSTS50105: 로그인한 사용자\@'브라이언 contoso.com'은 응용 프로그램의 역할에 할당되지 않습니다.*
...
1.  응용 프로그램 목록에서 사용자를 할당할 응용 프로그램을 선택합니다.

1.  응용 프로그램이 로드되면 응용 프로그램의 왼쪽 탐색 메뉴에서 **사용자 및 그룹을** 선택합니다.
...
1. 목록에서 **사용자** 위로 마우스를 **가져가확인란을**표시합니다. 사용자의 프로필 사진 또는 로고 옆에 있는 확인란을 클릭하여 **선택한** 목록에 사용자를 추가합니다.

1. **선택 사항:** **두 명 이상의 사용자를 추가하려면**이름 또는 전자 메일 주소 검색 상자에 다른 전체 이름 또는 전자 **메일** 주소를 입력하고 확인란을 클릭하여 **선택한** 목록에 사용자를 추가합니다.

1. 사용자 선택을 마치면 **선택** 단추를 클릭하여 응용 프로그램에 할당할 사용자 및 그룹 목록에 사용자를 추가합니다.

1. **선택 사항:** **과제 추가** 창에서 **역할 선택** 선택을 클릭하여 선택한 사용자에게 할당할 역할을 선택합니다.

1. 선택한 사용자에게 응용 프로그램을 할당하려면 **할당** 단추를 클릭합니다.

짧은 시간이 지나면 선택한 사용자가 솔루션 설명 섹션에 설명된 방법을 사용하여 이러한 응용 프로그램을 시작할 수 있습니다.
...
1. SAML 요청을 캡처합니다. SAML 요청을 캡처하는 방법을 알아보려면 [Azure AD의 응용 프로그램에 SAML 기반 단일 사인온을 디버깅하는 방법을](../azuread-dev/howto-v1-debug-saml-sso-issues.md) 설명합니다.
...
1. SAML 서명 인증서 섹션에서 **새 인증서 만들기를** **선택합니다.**

1. 만료 날짜를 선택한 다음 **저장을**클릭합니다.

1. 활성 인증서를 재정의하려면 **새 인증서를 활성으로 확인합니다.** 그런 다음, 창 맨 위에 있는 **저장**을 클릭하고 롤오버 인증서를 활성화하도록 수락합니다.
...
응용 프로그램에 전송된 SAML 특성 클레임을 사용자 지정하는 방법을 알아보려면 [Azure Active Directory의 클레임 매핑을](../develop/active-directory-claims-mapping.md)참조하십시오.
