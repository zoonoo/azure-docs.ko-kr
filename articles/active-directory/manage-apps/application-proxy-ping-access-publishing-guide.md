---
title: Azure AD 응용 프로그램 프록시에 대 한 액세스 권한이 있는 헤더 기반 인증
...
Azure ad (Azure Active Directory) 응용 프로그램 프록시는 Azure AD 고객이 더 많은 응용 프로그램에 액세스할 수 있도록 Azure 액세스와 파트너 관계를 맺고 있습니다. 공동 액세스는 통합 [헤더 기반 Single Sign-On](application-proxy-configure-single-sign-on-with-headers.md)이외의 다른 옵션을 제공 합니다.
...
1. 새 응용 프로그램에 대 한 정보를 사용 하 여 필수 필드를 작성 합니다. 설정에 대 한 아래 지침을 사용 합니다.
   ...
   1. **내부 url**: 회사 네트워크에 있을 때 일반적으로 앱의 로그인 페이지로 이동 하는 URL을 제공 합니다. 이 시나리오의 경우 커넥터는 응용 프로그램의 front 페이지로 연결 액세스 프록시를 처리 해야 합니다. `https://<host name of your PingAccess server>:<port>` 형식을 사용합니다. 포트는 기본적으로 3000이지만 PingAccess에서 구성할 수 있습니다.
      ...
      > [!WARNING]
      > 이러한 유형의 Single Sign-On에서 내부 URL은를 사용 해야 `https` 하며 사용할 수 없습니다 `http` . 또한 응용 프로그램을 구성 하는 경우에는 응용 프로그램 간 차이점을 유지 하기 위해 앱 프록시가 허용 하는 것과 동일한 내부 URL을 포함 하지 않는 응용 프로그램을 구성할 때 제약 조건이 있습니다.
...
AzureAD에 없는 특성에 대 한 [클레임 매핑 정책 (미리 보기)](../develop/active-directory-claims-mapping.md#claims-mapping-policy-properties) 입니다. 클레임 매핑을 사용 하면 ADFS 또는 사용자 개체에 의해 지원 되는 사용자 지정 클레임을 더 추가 하 여 오래 된 온-프레미스 앱을 클라우드로 마이그레이션할 수 있습니다.
...
- [Microsoft Azure AD 응용 프로그램 프록시를 사용 하 여 게시 된 응용 프로그램을 보호 하기 위해 Azure AD에 대 한 연결 액세스](https://docs.pingidentity.com/bundle/pingaccess-60/page/jep1564006742933.html)
