---
title: 응용 프로그램 프록시 앱에 단일 사인온을 구성하는 방법
...
SSO를 구성하려면 먼저 애플리케이션이 Azure Active Directory를 통해 사전 인증할 수 있도록 구성되어 있는지 확인합니다. 이 구성을 수행하려면 **Azure Active Directory**  - &gt; **엔터프라이즈 응용 프로그램으로**  - &gt; 이동하여 **모든 응용 프로그램**  - &gt; 응용 ** - &gt; 프로그램 프록시로**이동합니다. 이 페이지에서 "사전 인증" 필드가 표시되고 "Azure Active Directory"로 설정되어 있는지 확인합니다. 
...
-   **SAML 단일 사인온**: SAML 단일 사인온을 사용하면 Azure AD는 사용자의 Azure AD 계정을 사용하여 응용 프로그램에 인증합니다. Azure AD는 연결 프로토콜을 통해 애플리케이션에 로그온 정보를 통신합니다. SAML 기반 Single Sign-On을 사용하면 SAML 클레임에서 정의하는 규칙에 따라 사용자를 특정 애플리케이션 역할에 매핑할 수 있습니다. SAML 단일 사인온 설정에 대한 자세한 내용은 [응용 프로그램 프록시를 통해 단일 사인온에 대한 SAML을](application-proxy-configure-single-sign-on-on-premises-apps.md)참조하십시오.

이러한 각 옵션은 "엔터프라이즈 애플리케이션"에서 애플리케이션으로 이동하여 왼쪽 메뉴에서 **Single Sign-On** 페이지를 열면 찾을 수 있습니다. 응용 프로그램이 이전 포털에서 만들어진 경우 이러한 옵션이 모두 표시되지 않을 수 있습니다.
...
- [응용 프로그램 프록시를 사용 하 고 단일 사인온에 대 한 SAML](application-proxy-configure-single-sign-on-on-premises-apps.md)입니다.
