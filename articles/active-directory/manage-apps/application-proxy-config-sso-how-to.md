...
SSO를 구성하려면 먼저 애플리케이션이 Azure Active Directory를 통해 사전 인증할 수 있도록 구성되어 있는지 확인합니다. 이 구성을 수행 하려면 **Azure Active Directory** -&gt; **Enterprise applications** -&gt; 응용 프로그램 -응용 프로그램 **프록시**&gt; 응용 프로그램-&gt; **모든 응용** 프로그램로 이동 합니다. 이 페이지에서 "사전 인증" 필드가 표시되고 "Azure Active Directory"로 설정되어 있는지 확인합니다. 
