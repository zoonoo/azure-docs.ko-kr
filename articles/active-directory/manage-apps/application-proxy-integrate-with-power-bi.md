---
title: Azure AD 응용 프로그램 프록시를 사용 하 여 Power BI에 대 한 원격 액세스 사용 Microsoft Docs
...
-L 옵션과 함께 Setspn 명령을 실행하여 SPN이 추가되었는지 확인할 수 있습니다. 이 명령에 대해 자세히 알아보려면 [Setspn](https://social.technet.microsoft.com/wiki/contents/articles/717.service-principal-names-spn-setspn-syntax.aspx)을 참조 하세요.
...
자세한 내용은 [Reporting Services 구성 파일](https://msdn.microsoft.com/library/bb630448.aspx) 수정 및 [보고서 서버에서 Windows 인증 구성](https://msdn.microsoft.com/library/cc281253.aspx)을 참조 하세요.
...
4. 위임 설정을 **지정 된 서비스에 대 한 위임용 으로만이 컴퓨터 트러스트**로 설정 합니다. 그런 다음 **모든 인증 프로토콜 사용**을 선택 합니다.
...
1. 다음 설정을 사용 하 여 응용 프로그램 프록시를 통해 보고서 서비스를 게시 합니다. 응용 프로그램 프록시를 통해 응용 프로그램을 게시 하는 방법에 대 한 단계별 지침은 [Azure AD 응용 프로그램 프록시를 사용 하 여 응용 프로그램 게시](application-proxy-add-on-premises-application.md#add-an-on-premises-app-to-azure-ad)를 참조 하세요.
   - **내부 URL**: 회사 네트워크에서 커넥터에 연결할 수 있는 보고서 서버의 URL을 입력 합니다. 커넥터가 설치 된 서버에서이 URL에 연결할 수 있는지 확인 합니다. 응용 프로그램 프록시를 통해 게시 되지 않은 하위 경로 ( `https://servername/` `https://servername/reports/` 예: 및 `https://servername/reportserver/`)와 관련 된 문제를 방지 하기 위해와 같은 최상위 도메인을 사용 하는 것이 가장 좋습니다.
     > [!NOTE]
...
2. 앱이 게시된 후에는 다음 단계에 따라 Single Sign-On 설정을 구성합니다.
   ...
   a. 포털의 응용 프로그램 페이지에서 **Single sign-on**을 선택 합니다.

   b. **Single Sign-on 모드**의 경우 **Windows 통합 인증**을 선택 합니다.

   c. **내부 애플리케이션 SPN**을 이전에 설정한 값으로 설정합니다.  

   d. 커넥터에서 사용자를 대신 하 여 사용할 **위임 된 로그인 id** 를 선택 합니다. 자세한 내용은 [다른 온-프레미스 및 클라우드 id로 작업](application-proxy-configure-single-sign-on-with-kcd.md#working-with-different-on-premises-and-cloud-identities)을 참조 하세요.
...
1. AzureActiveDirectory 한 다음 **앱 등록**으로 이동 합니다.
