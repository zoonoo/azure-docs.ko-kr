...
description: 응용 프로그램 ID (AppId), 이름 (DisplayName), 외부 URL (ExternalUrl), 내부 URL (InternalUrl) 및 인증 유형과 함께 모든 Azure Active Directory (Azure AD) 응용 프로그램 프록시 응용 프로그램을 나열 하는 PowerShell 예제입니다. ExternalAuthenticationType).
...
이 PowerShell 스크립트 예제는 응용 프로그램 ID (AppId), 이름 (DisplayName), 외부 URL (ExternalUrl), 내부 URL (InternalUrl)을 포함 하 여 모든 Azure Active Directory (Azure AD) 응용 프로그램 프록시 응용 프로그램에 대 한 정보를 나열 합니다. 인증 유형 (ExternalAuthenticationType).
