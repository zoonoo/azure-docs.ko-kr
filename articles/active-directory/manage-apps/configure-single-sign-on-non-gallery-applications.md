...
이 문서는 Microsoft identity 플랫폼 앱 갤러리에 없는 응용 프로그램에 대 한 single sign on 구성 관리자를 사용 하도록 설정 하는 기능에 대 한 *코드를 작성 하지 않고도*합니다.
...
- [Office 365 앱 시작 관리자](https://blogs.office.com/2014/10/16/organize-office-365-new-app-launcher-2/) 또는 [Azure AD 액세스 패널](what-is-single-sign-on.md#linked-sign-on)에서 애플리케이션에 대한 링크를 추가하는 기능
...
Microsoft Id 플랫폼은 응용 프로그램을 등록 하는 두 가지 메커니즘을 제공 합니다.

와 같은 최신 프로토콜을 사용 하는 응용 프로그램 [OpenId Connect/OAuth](https://docs.microsoft.com/azure/active-directory/develop/active-directory-v2-protocols) 사용 하 여 등록은 해당 사용자를 인증 하는 [응용 프로그램 등록 포털](https://docs.microsoft.com/azure/active-directory/develop/quickstart-v2-register-an-app)합니다.

다른 모든 유형의 활용 하 여 응용 프로그램의 등록 [인증 메커니즘을 지원](what-is-single-sign-on.md) 같은 합니다 [SAML](https://docs.microsoft.com/azure/active-directory/develop/single-sign-on-saml-protocol) 프로토콜을 사용 하 여 합니다 **엔터프라이즈 응용 프로그램** 블레이드 Microsoft Id 플랫폼을 사용 하 여 자신을 연결 합니다.

앱 통합 템플릿을 사용 하 여 응용 프로그램에 연결 하 여 Microsoft id 플랫폼 관리자 계정을 사용 하 여 Azure portal에 로그인 합니다. **Active Directory &gt; 엔터프라이즈 애플리케이션 &gt; 새 애플리케이션 &gt; 비갤러리 애플리케이션** 섹션으로 이동하여 **추가**, **갤러리에서 애플리케이션 추가**를 차례로 선택합니다.
...
> [!TIP]
> 모범 사례로 애플리케이션 갤러리에 애플리케이션이 이미 존재하는지 확인하려면 검색 기능을 사용합니다. 앱이 검색되었고 설명에 "Single Sign-On"이 포함되어 있는 경우에는 페더레이션된 Single Sign-On이 애플리케이션에 이미 지원됩니다.
...
- **로그온 URL (SP에서 시작한만)** – 사용자가이 응용 프로그램에 로그인 하는 위치입니다. 경우 응용 프로그램은 서비스 공급자는 Azure AD를 인증 하 고 사용자를 로그인에 필요한 리디렉션을 수행 사용자가이 URL을 탐색, 다음 서비스 공급자가 시작한 single sign-on을 수행 하도록 구성 됩니다. 이 필드가 채워지면 Azure AD는 이 URL을 사용하여 Office 365 및 Azure AD 액세스 패널에서 애플리케이션을 시작합니다. 이 필드가 생략 되 면 Azure AD id 공급자를 대신 수행 됩니다-Office 365, Azure AD 액세스 패널 또는 Azure AD single sign-on URL (대시보드 탭에서 복사할 수 있습니다)에서 앱을 실행 하는 경우 로그온 시작 합니다.
...
자세한 내용은 참조 하세요. [Microsoft id 플랫폼 (Azure AD)을 지 원하는 SAML 2.0 인증 요청 및 응답](https://docs.microsoft.com/azure/active-directory/develop/active-directory-single-sign-on-protocol-reference?/?WT.mc_id=DOC_AAD_How_to_Debug_SAML)
...
- 응용 프로그램에 Microsoft id 플랫폼에 저장 된 사용자 이름 (즉, 사용자 계정 이름) 이외의 NameIdentifier 클레임 요구 하는 방식으로 배포 되었습니다.
...
자세한 내용은 [페더레이션된 single sign-on의 Microsoft id 플랫폼에 대 한 인증서 관리](manage-certificates-for-federated-single-sign-on.md)합니다.
...
SAML 애플리케이션을 테스트하기 전에 Azure AD로 애플리케이션을 설정하고 애플리케이션에 사용자 또는 그룹을 할당해야 합니다. SAML 응용 프로그램을 테스트 하려면 참조 [SAML 기반 single sign Microsoft id 플랫폼의 응용 프로그램에 디버그 하는 방법을](../develop/howto-v1-debug-saml-sso-issues.md)합니다.
...
- [(Azure Active Directory 개발자를 위한 Microsoft id 플랫폼](https://aka.ms/aaddev)
