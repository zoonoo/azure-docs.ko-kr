...
* Windows 서비스 콘솔을 열고 **Microsoft AAD 애플리케이션 프록시 커넥터** 서비스가 활성화되어 있고 실행 중인지 확인합니다. 또한 다음 그림에 표시된 것처럼 애플리케이션 프록시 서비스 속성 페이지에서 확인할 수도 있습니다.  
...
| 이 회사 앱에 액세스할 수 없습니다. 이 애플리케이션에 액세스할 수 있는 권한이 없습니다. 권한 부여에 실패했습니다. 이 애플리케이션에 액세스할 수 있는 사용자를 할당할 수 있는지 확인합니다. | 사용자는 자신의 회사 계정 대신 Microsoft 계정 로그인을 사용 하는 경우 게시 된 앱에 액세스 하려고 할 때이 오류가 발생할 수 있습니다. 게스트 사용자에게 이 오류가 발생할 수도 있습니다. Microsoft 계정 사용자 및 게스트가 IWA 애플리케이션에 액세스할 수 없습니다. 사용자가 게시된 애플리케이션의 도메인과 일치하는 기업 계정을 사용하여 로그인하는지 확인합니다.<br><br>이 애플리케이션에 사용자를 할당하지 않았을 수 있습니다. **애플리케이션** 탭으로 이동하고 **사용자 및 그룹**에서 이 사용자나 사용자 그룹을 이 애플리케이션에 할당합니다. |
| 현재 이 회사 앱에 액세스할 수 없습니다. 나중에 다시 시도하세요. 커넥터 제한 시간이 초과되었습니다. | 사용자는 온-프레미스 쪽에서이 응용 프로그램에 대 한 제대로 정의 되지 않은 경우 게시 된 앱에 액세스 하려고 할 때이 오류가 발생할 수 있습니다. 온-프레미스 컴퓨터에서이 백 엔드 응용 프로그램에 대해 정의 된 사용자가 있는 적절 한 권한이 있는지 확인 합니다. |
| 이 회사 앱에 액세스할 수 없습니다. 이 애플리케이션에 액세스할 수 있는 권한이 없습니다. 권한 부여에 실패했습니다. 사용자에게 Azure Active Directory Premium 또는 Basic에 대한 라이선스가 있는지 확인합니다. | 사용자는 구독자의 관리자가 Premium/Basic 라이선스를 사용 하 여 명시적으로 할당 되지 않은 경우 게시 된 앱에 액세스 하려고 할 때이 오류가 발생할 수 있습니다. 구독자의 Active Directory **라이선스** 탭으로 이동하고 이 사용자나 사용자 그룹에 Premium 또는 Basic 라이선스가 할당되어 있는지 확인합니다. |
| 지정된 된 호스트 이름 사용 하 여 서버를 찾을 수 없습니다. | 사용자는 응용 프로그램의 사용자 지정 도메인을 올바르게 구성 되지 않은 경우 게시할 앱에 액세스 하려고 할 때이 오류가 발생할 수 있습니다. 도메인에 대 한 인증서를 업로드 하 고 단계를 수행 하 여 DNS 레코드를 올바르게 구성 했는지 [Azure AD 응용 프로그램 프록시에서 사용자 지정 도메인 작업](application-proxy-configure-custom-domain.md) |
