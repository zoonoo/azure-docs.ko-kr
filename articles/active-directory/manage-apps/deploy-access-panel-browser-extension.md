...
이 자습서에서는 그룹 정책을 사용하여 사용자의 컴퓨터에 Internet Explorer용 액세스 패널 확장을 원격 설치하는 방법을 보여줍니다. 이 확장은 [암호 기반 Single Sign-On](sso-options.md#password-based-sso)을 사용하여 구성된 앱에 로그인하는 Internet Explorer 사용자에게 필요합니다.
...
1. 설치 관리자 패키지를 공유의 원하는 위치에 복사합니다.
    ...
    ![.Msi 파일을 공유에 복사 합니다.](./media/deploy-access-panel-browser-extension/copy-package.png)
...
1. OU를 선택한 후에 마우스 오른쪽 단추로 클릭하고 **이 도메인에서 GPO를 만들고 여기에 연결...** 을 선택합니다.
    ...
    ![새 GPO 만들기 옵션을 보여 주는 스크린샷](./media/deploy-access-panel-browser-extension/create-gpo.png)

1. **새 GPO** 프롬프트에 새 그룹 정책 개체의 이름을 입력합니다.
1. 만든 그룹 정책 개체를 마우스 오른쪽 단추로 클릭하고 **편집**을 선택합니다.

## <a name="step-3-assign-the-installation-package"></a>3 단계: 설치 패키지 할당
...
## <a name="step-4-auto-enable-the-extension-for-internet-explorer"></a>4 단계: Internet Explorer 용 확장 자동 사용
...
1. **추가 기능 목록**을 마우스 오른쪽 단추로 클릭하고 **편집**을 선택합니다.
    ...
    !["추가 기능 목록"을 마우스 오른쪽 단추로 클릭 하 고 "편집"을 선택 합니다.](./media/deploy-access-panel-browser-extension/edit-add-on-list.png)
...
1. **내용 표시** 창에서 다음 단계를 수행합니다.
   ...
   1. **확인**을 클릭하여 **내용 표시** 창을 닫습니다.
      ...
      ![이전 단계에서 지정한 대로 값을 입력 합니다.](./media/deploy-access-panel-browser-extension/show-contents.png)
...
## <a name="step-5-optional-disable-remember-password-prompt"></a>5 단계 (선택 사항): "암호 기억을" 프롬프트를 사용 하지 않도록 설정

사용자가 액세스 패널 확장을 사용하여 웹 사이트에 로그인하면 Internet Explorer에 “암호를 저장하시겠습니까?”라고 묻는 프롬프트가 다음과 같이 표시될 수 있습니다.

!["암호를 저장 하 시겠습니까?"를 표시 합니다. 요청할](./media/deploy-access-panel-browser-extension/remember-password-prompt.png)
...
1. 이름이 **양식의 사용자 이름과 암호에 자동 완성 기능 사용**인 설정을 찾습니다.
   ...
   > 이전 버전의 Active Directory에는 이 설정의 이름이 **Do not allow auto-complete to save passwords**(자동 완성이 암호를 저장하도록 허용 안 함)으로 표시될 수 있습니다. 해당 설정에 대한 구성은 이 자습서에 설명되어 있는 설정마다 다릅니다.
    ...
    ![사용자 설정에서이를 확인 해야 합니다.](./media/deploy-access-panel-browser-extension/disable-auto-complete.png)

1. 위 설정을 마우스 오른쪽 단추로 클릭하고 **편집**을 선택합니다.
1. **양식의 사용자 이름과 암호에 자동 완성 기능 사용** 창에서 **사용 안함**을 선택합니다.

    ![자동 완성 기능 설정에 대해 "사용 안 함" 옵션을 선택 합니다.](./media/deploy-access-panel-browser-extension/disable-passwords.png)
...
## <a name="step-6-testing-the-deployment"></a>6 단계: 배포 테스트
...
1. 이 컴퓨터에서 그룹 정책 변경을 완전히 업데이트 하기 위해 몇 번의 로그인이 필요할 수 있습니다. 업데이트를 강제 실행하려면 **명령 프롬프트** 창을 열고 터미널 창을 열고 `gpupdate /force` 명령을 실행합니다.
...
1. **추가 기능 관리** 창에서 **액세스 패널 확장**이 설치되었으며 그 **상태**가 **사용**으로 설정되었는지 확인합니다.
   ...
   ![액세스 패널 확장이 설치 되어 있고 사용 하도록 설정 되어 있는지 확인](./media/deploy-access-panel-browser-extension/verify-install.png)
