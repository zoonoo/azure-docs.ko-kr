...
1. 다음을 포함하여 애플리케이션 갤러리에서 모든 애플리케이션을 추가하는 경우:
   ...
   1. **AZURE Ad 갤러리 응용 프로그램** – azure ad와 Single Sign-On 하기 위해 사전 통합 된 응용 프로그램입니다.
   ...
   4. **비 갤러리 응용 프로그램** – 사용자 고유의 응용 프로그램을 가져옵니다. 원하는 모든 웹 링크 또는 사용자 이름 및 암호 필드를 렌더링하는 모든 애플리케이션은 SAML 또는 OpenID Connect 프로토콜을 지원하거나 Single Sign-On에 대해 Azure AD와 통합하려는 SCIM을 지원합니다.
