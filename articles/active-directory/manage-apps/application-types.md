...
1. 다음을 포함하여 애플리케이션 갤러리에서 모든 애플리케이션을 추가하는 경우:
   ...
   1. **Azure AD 갤러리 응용 프로그램** - Azure AD를 사용 하 여 단일 사인온에 대 한 사전 통합 된 응용 프로그램
   ...
   4. **갤러리가 아닌 응용 프로그램** - 자신의 응용 프로그램을 가져와! 원하는 모든 웹 링크 또는 사용자 이름 및 암호 필드를 렌더링하는 모든 애플리케이션은 SAML 또는 OpenID Connect 프로토콜을 지원하거나 Single Sign-On에 대해 Azure AD와 통합하려는 SCIM을 지원합니다.
...
전체 디렉터리에 대한 사용자 동의를 비활성화하면 모든 애플리케이션에 대한 최종 사용자 동의를 방지합니다. 관리자는 여전히 사용자를 대신하여 동의할 수 있습니다. 애플리케이션 동의 및 동의하거나 동의하지 않을 수 있는 이유에 대해 자세히 알아보려면 [사용자 및 관리자 동의 이해하기](https://docs.microsoft.com/azure/active-directory/develop/active-directory-devhowto-multi-tenant-overview)를 참조하세요.
