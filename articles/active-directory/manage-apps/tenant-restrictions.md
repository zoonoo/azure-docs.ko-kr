...
1. **AZURE ad**: `Restrict-Access-To-Tenants: <permitted tenant list>` 있는 경우 azure ad는 허용 되는 테 넌 트에 대해서만 보안 토큰을 발급 합니다.

2. **온-프레미스 프록시 서버 인프라**:이 인프라는 SSL (SSL(Secure Sockets Layer)) 검사를 지 원하는 프록시 장치입니다. 허용 되는 테 넌 트 목록을 포함 하는 헤더를 Azure AD로 향하는 트래픽에 삽입 하도록 프록시를 구성 해야 합니다.
...
다음 다이어그램은 고급 트래픽 흐름을 보여 줍니다. 테 넌 트 제한에는 Office 365 클라우드 서비스가 아닌 Azure AD에 대 한 트래픽에 대 한 SSL 검사만 필요 합니다. Azure AD에 대 한 인증을 위한 트래픽 볼륨이 Exchange Online 및 SharePoint Online과 같은 SaaS 응용 프로그램에 대 한 트래픽 볼륨과 일반적으로 훨씬 낮으므로 이러한 구분은 중요 합니다.
...
- 클라이언트는 SSL 통신을 위해 프록시에서 제공한 인증서 체인을 신뢰해야 합니다. 예를 들어 내부 [PKI (공개 키 인프라)](/windows/desktop/seccertenroll/public-key-infrastructure) 의 인증서를 사용 하는 경우 내부 발급 루트 인증 기관 인증서를 신뢰할 수 있어야 합니다.
...
3. 사용자 지정 규칙을 사용하여 *Restrict-Access-To-Tenants* 및 *Restrict-Access-Context* 머리글을 삽입하도록 Fiddler를 구성합니다.
   ...
   2. `OnBeforeRequest` 함수의 시작 부분에 다음 줄을 추가 합니다. 테 넌 트 도메인\> \<테 넌 트에 등록 된 도메인 (예: `contoso.onmicrosoft.com`)으로 바꿉니다. \<directory ID\>를 테넌트의 Azure AD GUID 식별자로 바꿉니다.
