<properties pageTitle="자습서: Intacct와 Azure Active Directory 통합 | Microsoft Azure" description="Azure Active Directory에서 Intacct를 사용하여 Single Sign-On, 자동화된 프로비저닝 등을 사용하도록 설정하는 방법을 알아봅니다." services="active-directory" authors="MarkusVi"  documentationCenter="na" manager="stevenpo"/>
<tags ms.service="active-directory" ms.devlang="na" ms.topic="article" ms.tgt_pltfrm="na" ms.workload="identity" ms.date="08/01/2015" ms.author="markvi" />
#자습서: Intacct와 Azure Active Directory 통합
>[AZURE.TIP]사용자 의견을 보내려면 [여기](http://go.microsoft.com/fwlink/?LinkId=528190)를 클릭하십시오.
  
이 자습서는 Azure와 Intacct의 통합을 보여주기 위한 것입니다. 이 자습서에 설명된 시나리오에서는 사용자에게 이미 다음 항목이 있다고 가정합니다.

-   유효한 Azure 구독
-   Intacct 테넌트
  
이 자습서를 완료하면 Intacct에 할당한 Azure AD 사용자가 Intacct 회사 사이트(서비스 공급자가 제공한 로그온)에서나 [액세스 패널 소개](https://msdn.microsoft.com/library/dn308586)를 사용하여 응용 프로그램에 Single Sign-On으로 로그인할 수 있습니다.
...
이 섹션에서는 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD의 계정으로 Intacct에 인증할 수 있게 하는 방법을 간략하게 설명합니다. 이 절차의 일부로 base-64로 인코딩된 인증서 파일을 만들어야 합니다. 이 절차를 잘 모르는 경우 [이진 인증서를 텍스트 파일로 변환하는 방법](http://youtu.be/PlgrzUZ-Y1o)을 참조하십시오.

###Single Sign-On을 구성하려면 다음 단계를 수행합니다.

1.  Azure AD 포털의 **Intacct** 응용 프로그램 통합 페이지에서 **Single Sign-On 구성**을 클릭하여 **Single Sign-On 구성** 대화 상자를 엽니다.

    ![Single Sign-On 구성](./media/active-directory-saas-intacct-tutorial/IC790033.png "Single Sign-On 구성")

2.  **Intacct에 대한 사용자 로그온 방법을 선택하십시오.** 페이지에서 **Microsoft Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.

    ![Single Sign-On 구성](./media/active-directory-saas-intacct-tutorial/IC790034.png "Single Sign-On 구성")

3.  **앱 URL 구성** 페이지의 **Intacct Sign-On URL** 텍스트 상자에 다음 패턴 "**https://Intacct.com/company*"을 사용하여 URL을 입력하고 **다음**을 클릭합니다.
...
8.  **Single Sign-On** 섹션에서 다음 단계를 수행합니다.
    ...
    3.  Azure 포털의 **Intacct에 Single Sign-On 구성** 대화 상자 페이지에서 **발급자 URL** 값을 복사한 다음 **발급자 URL** 텍스트 상자에 붙여넣습니다.
    4.  Azure 포털의 **Intacct에 Single Sign-On 구성** 대화 상자 페이지에서 **원격 로그인 URL** 값을 복사한 다음 **로그인 URL** 텍스트 상자에 붙여넣습니다.
...
Azure AD 사용자가 Intacct에 로그인할 수 있도록 하려면 Intacct로 프로비저닝되어야 합니다. Intacct의 경우 프로비저닝은 수동 작업입니다.
...
4.  **사용자 정보** 섹션에서 다음 단계를 수행합니다.
    ...
    3.  **저장**을 클릭합니다.
		...
		>[AZURE.NOTE]AAD 계정 보유자에게 전자 메일이 발송되며 여기에 포함된 링크를 클릭하여 계정을 확인하면 계정이 활성화됩니다.

>[AZURE.NOTE]다른 Intacct 사용자 계정 생성 도구 또는 Intacct가 제공한 API를 사용하여 AAD 사용자 계정을 프로비저닝할 수 있습니다.
