---
title: 2 단계 확인 방법 및 설정 변경-Azure Active Directory
description: 추가 보안 인증 페이지에서 회사 또는 학교 계정에 대 한 보안 확인 방법 및 설정을 변경 하는 방법에 대해 알아봅니다.
...
- 전화 번호와 같은 보안 확인 방법 세부 정보

- Authenticator 앱 설정 또는 인증자 앱에서 장치 삭제

## <a name="using-the-additional-security-verification-page"></a>추가 보안 인증 페이지 사용

조직에서 2 단계 인증을 설정 하 고 관리 하는 방법에 대 한 특정 단계를 제공한 경우에는 먼저 해당 지침을 따라야 합니다. 그렇지 않으면 [추가 보안 인증](https://docs.microsoft.com/azure/active-directory/user-help/multi-factor-authentication-end-user-first-time) 페이지에서 보안 확인 방법 설정을 가져올 수 있습니다.

>[!Note]
>화면에 표시 되는 내용이이 문서에서 설명 하는 내용과 일치 하지 않는 경우 관리자가 **보안 정보 (미리 보기)** 환경을 사용 하도록 설정 했거나 조직에서 사용자 고유의 사용자 지정 포털을 제공 했음을 의미 합니다. 새 보안 정보 환경에 대 한 자세한 내용은 [보안 정보 (미리 보기) 개요](user-help-security-info-overview.md)를 참조 하세요. 조직의 사용자 지정 포털에 대 한 자세한 내용은 조직의 기술 지원팀에 문의 해야 합니다.

### <a name="to-get-to-the-additional-security-verification-page"></a>추가 보안 인증 페이지를 가져오려면

[추가 보안 인증 페이지](https://account.activedirectory.windowsazure.com/proofup.aspx?proofup=1)에 대 한이 링크를 따를 수 있습니다.
...
다음 단계를 수행 하 여 **추가 보안 확인** 페이지로 이동할 수도 있습니다.
...
>[!Note]
>**추가 보안 인증** 페이지의 **앱 암호** 섹션을 사용 하는 방법에 대 한 자세한 내용은 [2 단계 인증을 위해 앱 암호 관리](multi-factor-authentication-end-user-app-passwords.md)를 참조 하세요. 앱 암호는 2 단계 인증을 지원 하지 않는 앱에만 사용 해야 합니다.
...
1. **추가 보안 인증** 페이지의 **기본 설정 옵션** 목록에서 사용할 방법을 선택 합니다. 모든 옵션이 표시 되지만, 조직에서 사용할 수 있는 옵션만 선택할 수 있습니다.

    - **앱을 통해 알림**: 사용자에 게 대기 중인 인증 프롬프트가 있음을 인증자 앱을 통해 알려 드리겠습니다.

    - **내 인증 전화 통화**: 모바일 장치에서 사용자의 정보를 확인 하도록 요청 하는 전화를 받게 됩니다.

    - **내 인증 전화로 텍스트 코드**: 모바일 장치에서 문자 메시지의 일부로 확인 코드를 받게 됩니다. 회사 또는 학교 계정에 대 한 확인 프롬프트에이 코드를 입력 해야 합니다.

    - **사무실 전화 통화**: 사무실 전화를 통해 사용자의 정보를 확인 하도록 요청 하는 전화를 받게 됩니다.

    - **앱에서 확인 코드 사용**: authenticator 앱을 사용 하 여 회사 또는 학교 계정의 프롬프트에 입력 하는 확인 코드를 가져옵니다.
...
1. **대체 인증 전화** 옵션 옆의 상자를 선택 하 고 기본 장치에 액세스할 수 없는 경우 문자 메시지 또는 전화 통화를 받을 수 있는 보조 전화 번호를 입력 합니다.
...
Microsoft Authenticator 앱에서 회사 또는 학교 계정을 이미 설정한 경우에는 다시 실행할 필요가 없습니다.

1. **추가 보안 확인** 페이지의 **응답 방법** 섹션에서 **인증자 앱 설정**을 선택 합니다.
...
1. 모바일 장치를 사용 하 여 QR 코드를 스캔 하는 등 화면의 지시에 **따라 다음을 선택 합니다**.
...
조직 설정에 따라 브라우저에서 2 단계 인증을 수행할 때 **X 일 동안 다시 묻지 않음** 이라는 확인란이 표시 될 수 있습니다. 이 옵션을 선택 하 여 2 단계 인증 프롬프트를 중지 한 후 장치를 분실 하거나 장치가 손상 될 가능성이 있는 경우 계정을 보호 하기 위해 2 단계 확인 메시지를 다시 설정 해야 합니다. 모든 장치에 대 한 프롬프트를 동시에 켜야 합니다. 아쉽게도 특정 장치에 대해서만 프롬프트를 다시 켤 수는 없습니다.

### <a name="to-turn-two-factor-verification-prompts-back-on-for-your-devices"></a>장치에 대해 2 단계 확인 메시지를 다시 설정 하려면

[ **추가 보안 인증** 페이지](#to-get-to-the-additional-security-verification-page)에서 **이전에 신뢰할 수 있는 장치에서 multi-factor authentication 복원**을 선택 합니다. 다음 번에 장치에서 로그인 하면 2 단계 인증을 수행 하 라는 메시지가 표시 됩니다.
...
- [2 단계 인증을 사용 하 여 로그인 하는 방법](multi-factor-authentication-end-user-signin.md)

- [2 단계 확인 시 발생 하는 일반적인 문제 해결](multi-factor-authentication-end-user-troubleshoot.md)
