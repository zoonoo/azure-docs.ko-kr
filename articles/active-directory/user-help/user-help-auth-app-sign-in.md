...
- **이미 이 앱을 2단계 인증에 사용 중인 경우.** 이 앱과 2단계 인증을 이미 사용하고 있는 경우 계정 이름 옆에 있는 드롭다운 화살표를 선택한 다음, **휴대폰 로그인 활성화**를 선택하면 됩니다.
...
- **계정 타일을 탭**하면 계정이 계정 전체 화면 보기로 열립니다. **휴대폰 로그인 활성화됨**이 표시되면 암호 없이 로그인하도록 완전히 설정된 것입니다. **휴대폰 로그인 활성화**가 표시되면 탭하여 휴대폰 로그인을 켭니다.
- **이미 이 앱을 2단계 인증에 사용 중인 경우** 계정 타일을 탭하면 계정의 전체 화면 보기로 확장됩니다. 그런 다음, **휴대폰 로그인 활성화**를 탭하여 휴대폰 로그인을 켭니다.
- 앱의 **계정** 화면에서 **회사 또는 학교 계정을 찾을 수 없는 경우** 앱에 계정을 아직 추가하지 않은 것입니다. [회사 또는 학교 계정 추가] 문서의 단계를 수행하여 회사 또는 학교 계정을 추가하세요.

> [!NOTE]
> iOS의 Authenticator 앱에서는 디바이스 등록과 인증서 기반 인증 조합을 지원하지 않습니다. 사용자는 로그인하기 전에 Authenticator 설정을 통해 수동으로 디바이스를 등록하는 방법을 사용해야 합니다.
...
- **이 아이콘 ![설정을 보여주는 아이콘](media/user-help-auth-app-sign-in/icon.png)이 있는 경우** 이 아이콘이 개인 Microsoft 계정 이름 옆에 표시되면 해당 계정에서 휴대폰 로그인을 이미 설정했다는 뜻입니다. 앱 외부에서 인증 요청에 대한 알림을 받을 수 있도록 계정에 대한 푸시 알림을 추가하라는 메시지가 표시될 수 있습니다.
- **2단계 인증에 대한 앱을 사용하고 있는 경우** 이 앱과 2단계 인증을 이미 사용하고 있는 경우 계정 이름 옆에 있는 드롭다운 화살표를 선택한 다음, **휴대폰 로그인 활성화**를 선택하면 됩니다.
- **회사 또는 학교 계정을 찾을 수 없는 경우** 앱의 **계정** 화면에서 계정을 찾을 수 없는 경우 앱에 계정을 아직 추가하지 않은 것을 의미합니다. [개인 Microsoft 계정 추가](user-help-auth-app-add-personal-ms-account.md) 문서의 단계에 따라 계정을 추가하세요.
...
표준 2단계 인증 방법을 사용하려면 로그인하려는 디바이스에 사용자 이름과 암호를 입력해야 합니다. 그런 다음, Microsoft Authenticator 앱을 열고 계정에 대해 임의로 생성된 코드를 **코드 입력** 상자에 입력합니다. Android 디바이스는 **계정** 화면에서 이 확인 코드를 찾을 수 있습니다. iOS 디바이스는 계정 유형에 따라 **계정** 화면 또는 계정의 전체 화면 보기에서 이 확인 코드를 찾을 수 있습니다. Microsoft Authenticator 앱에 계정을 추가하는 프로세스의 일환으로 계정에 대해 2단계 인증을 설정합니다.
