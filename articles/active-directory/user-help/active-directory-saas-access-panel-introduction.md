...
MyApps 포털에는 최소한 JavaScript를 지원하고 CSS를 사용하도록 설정된 브라우저가 필요합니다. 암호 기반 SSO(Single Sign-On)를 통해 애플리케이션에 로그인하려면 브라우저에 MyApps 포털 확장이 설치되어 있어야 합니다. 암호 기반 SSO에 구성된 응용 프로그램을 선택할 때 확장이 자동으로 다운로드됩니다.
...
응용 프로그램 타일을 처음으로 선택하면 Internet Explorer 또는 Chrome용 암호 SSO 플러그인을 설치하라는 메시지가 표시됩니다. 설치 시 웹 브라우저를 다시 시작해야 할 수도 있습니다. MyApps 포털로 돌아와서 애플리케이션 타일을 다시 선택하면 애플리케이션에 대한 사용자 이름과 및 암호를 묻는 메시지가 표시됩니다. 사용자 이름과 암호를 입력하면 자격 증명이 Azure AD의 해당 계정에 안전하게 저장되고 연결됩니다.
...
응용 프로그램에 SSO를 구성하기 위해 Azure Portal에서는 기존 Single Sign-On이라는 세 번째 옵션을 제공합니다. 이 옵션을 사용하면 관리자가 애플리케이션에 대한 링크를 만들어 선택한 사용자의 MyApps 포털에 배치할 수 있습니다.

예를 들어 애플리케이션이 AD FS 2.0을 사용하여 사용자를 인증하도록 구성되어 있는 경우 관리자는 기존 Single Sign-On 옵션을 사용하여 MyApps 포털에 해당 애플리케이션에 대한 링크를 만들 수 있습니다. 이 링크에 액세스하면 AD FS 2.0 또는 응용 프로그램에서 제공하는 기존 SSO 솔루션을 통해 인증됩니다.
