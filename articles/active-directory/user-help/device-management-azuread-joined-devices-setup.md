---
title: Azure Active Directory 연결 디바이스 설정 | Microsoft Docs
description: Azure Active Directory 연결 디바이스를 설정하는 방법을 알아봅니다.
...
---
# <a name="set-up-azure-active-directory-joined-devices"></a>Azure Active Directory 연결 디바이스 설정

Azure AD(Active Directory)의 디바이스 관리를 사용하면 보안 및 규정 준수에 대한 표준을 충족하는 디바이스에서 사용자 리소스에 액세스할 수 있습니다. 자세한 내용은 [Azure Active Directory의 디바이스 관리 소개](../device-management-introduction.md)를 참조하세요.

회사 소유 Windows 10 디바이스가 Azure AD에서 제어되도록 하려면 Azure AD 조인 디바이스를 구성합니다. 이 항목에서는 관련 단계를 제공합니다. 


## <a name="prerequisites"></a>필수 조건

Windows 10 디바이스를 조인하려면 디바이스를 등록할 수 있도록 디바이스 등록 서비스를 구성해야 합니다. Azure AD 테넌트에 디바이스를 조인할 수 있는 권한이 필요한 것 외에도, 구성된 디바이스 최대 수보다 적은 수의 디바이스를 등록해야 합니다. 자세한 내용은 [디바이스 설정 구성](../devices/device-management-azure-portal.md#configure-device-settings)을 참조하세요.
...
- Windows에서는 Azure AD에서 조직의 디렉터리에 디바이스를 조인합니다.
...
- 페더레이션 사용자인 경우 ID 공급자가 WS-Fed 및 WS-Trust 사용자 이름/암호 엔드포인트를 지원해야 합니다. 버전 1.3 또는 2005일 수 있습니다. Azure AD에 디바이스를 조인하고 암호를 사용하여 디바이스에 로그온하려면 이 프로토콜 지원이 필요합니다. 
...
이 섹션에서는 Windows 10 디바이스를 Azure AD에 조인하는 단계를 제공합니다. 성공적으로 조인된 디바이스는 **\<Azure AD\>에 연결됨**으로 표시됩니다.
...
성공적으로 조인된 디바이스에서 **AzureAdJoined**는 **예**입니다.

![연결됨](./media/device-management-azuread-joined-devices-setup/14.png)

Azure AD 포털에서 디바이스 설정을 검토할 수도 있습니다.

![연결됨](./media/device-management-azuread-joined-devices-setup/15.png)

자세한 내용은 [디바이스 찾기](../devices/device-management-azure-portal.md#locate-devices)를 참조하세요.
