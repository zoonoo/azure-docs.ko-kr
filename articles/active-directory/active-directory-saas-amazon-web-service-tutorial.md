...
### <a name="to-add-amazon-web-services-aws-from-the-gallery-perform-the-following-steps"></a>갤러리에서 AWS(Amazon Web Services)를 추가하려면 다음 단계를 수행합니다.
...
이 섹션에서는 "Britta Simon"이라는 테스트 사용자를 기반으로 하여 AWS(Amazon Web Services)에서 Azure AD Single Sign-On을 구성하고 테스트하는 방법을 보여 줍니다.
...
이 섹션에서는 Azure 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 AWS(Amazon Web Services) 응용 프로그램에서 Single Sign-On을 구성합니다.  
AWS(Amazon Web Services) 응용 프로그램에는 특정 형식의 SAML 어설션이 필요하므로 사용자 지정 특성 매핑을 **SAML 토큰 특성** 구성에 추가해야 합니다. 다음 스크린샷은 이에 대한 예제를 보여 줍니다.
...
22. 사용자 특성 추가 대화 상자에서 다음 단계를 수행합니다. 
    ...
    a. **특성 이름** 텍스트 상자에서 **https://aws.amazon.com/SAML/Attributes/Role**을 입력합니다.
    
    b. **특성 값** 텍스트 상자에서 **[역할 ARN 값],[신뢰할 수 있는 엔터티 ARN 값]**을 입력합니다.
...
24. 사용자 특성 추가 대화 상자에서 다음 단계를 수행합니다. 
    ...
    ![Single Sign-On 구성][25]
     ...
     a. **특성 이름** 텍스트 상자에서 **https://aws.amazon.com/SAML/Attributes/RoleSessionName**을 입력합니다.

     b. **특성 값** 텍스트 상자의 드롭다운 목록에서 **user.userprincipalname**을 입력하거나 선택합니다.
...
이 섹션에서는 Single Sign-On을 사용할 수 있도록 Britta Simon에게 AWS(Amazon Web Services)에 대한 액세스 권한을 부여합니다.
