...
- 사용자가 해당 Azure AD 계정으로 WORKS MOBILE에 자동으로 로그온(SSO)되도록 설정할 수 있습니다.
...
## <a name="add-works-mobile-from-the-gallery"></a>갤러리에서 WORKS MOBILE 추가
...
이 섹션에서는 "Britta Simon"이라는 테스트 사용자를 기반으로 WORKS MOBILE에서 Azure AD SSO를 구성하고 테스트합니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 WORKS MOBILE 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 WORKS MOBILE의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 WORKS MOBILE의 **Username** 값으로 할당하여 설정합니다.

WORKS MOBILE에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션에서는 Azure 관리 포털에서 Azure AD SSO를 사용하도록 설정하고 WORKS MOBILE 응용 프로그램에서 SSO를 구성합니다.

**WORKS MOBILE에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**
...
3. **WORKS MOBILE 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
  ...
  1. **로그온 URL** 텍스트 상자에서 다음 패턴 `https://<your-subdomain>.worksmobile.com/jp/myservice`을 사용하여 URL을 입력합니다.
  2. **식별자** 텍스트 상자에 해당 값으로 `worksmobile.com`을 입력합니다.
...
10. 응용 프로그램에 대해 SSO를 구성하려면 [WORKS MOBILE 지원 팀](mailto:dl_ssoinfo@worksmobile.com)에 문의한 후 다운로드한 **인증서 파일**, **SAML Single Sign-on 서비스 URL**, **SAML 엔터티 ID**, **로그아웃 URL**을 제공합니다.
...
### <a name="create-a-works-mobile-test-user"></a>WORKS MOBILE 테스트 사용자 만들기
...
이 섹션에서는 Azure SSO를 사용할 수 있도록 Britta Simon에게 WORKS MOBILE에 대한 액세스 권한을 부여합니다.
