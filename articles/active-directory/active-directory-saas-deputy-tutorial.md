<properties
	pageTitle="자습서: Deputy와 Azure Active Directory 통합 | Microsoft Azure"
...
# 자습서: Deputy와 Azure Active Directory 통합
...
2. **Deputy에 대한 사용자 로그인 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행하고 **다음**을 클릭합니다.
    ...
    b. **회신 URL** 텍스트 상자에 다음 `https://<your-subdomain>.<region>.deputy.com/exec/devapp/samlacs` 패턴으로 URL을 입력합니다.
...
4. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 **"고급 설정 표시(선택 사항)"**를 클릭하고 **로그온 URL**을 입력한 후 **다음**을 클릭합니다.
	...
	a. **로그인 URL** 텍스트 상자에서 다음과 같은 패턴을 사용하여 URL을 입력합니다. `https://<your-subdomain>.<region>.deputy.com`

	b. **Next**를 클릭합니다.

	> [AZURE.NOTE] Deputy 지역 접미사는 선택 사항이거나 au | na | eu |as |la |af |an |ent-au |ent-na |ent-eu |ent-as | ent-la | ent-af | ent-an 중 하나를 사용해야 합니다.
...
6. 브라우저를 열고 다음 URL로 이동합니다. https://(your-subdomain).deputy.com/exec/config/system_config **보안 설정**으로 이동하고 **편집**을 클릭합니다.
...
8. 이 **보안 설정** 페이지에서 아래 단계를 수행합니다.
	...
	b. Base64로 인코딩된 인증서를 메모장에서 열고, 내용을 클립보드에 복사한 다음 **OpenSSL 인증서** 텍스트 상자에 붙여넣습니다.

	c. SAM SSO URL 텍스트 상자에 `https://<your subdomain>.deputy.com/exec/devapp/samlacs?dpLoginTo=<saml sso url>`를 입력합니다.
	
	d. SAM SSO URL 텍스트 상자에서 `<your subdomain>`를 하위 도메인으로 바꿉니다.

	e. SAM SSO URL 텍스트 상자에서 `<saml sso url>`을 Azure 클래식 포털에서 복사한 SAML SSO URL로 바꿉니다.
...
3.  **구성원 추가** 단추를 클릭하고 **단일 구성원 추가**를 클릭합니다.
    ...
    ![구성원 추가](./media/active-directory-saas-deputy-tutorial/tutorial_deputy_002.png "구성원 추가")

4.  다음 단계를 수행하고 **저장 및 초대**를 클릭합니다.

    ![새 사용자](./media/active-directory-saas-deputy-tutorial/tutorial_deputy_003.png "새 사용자")

    a. **이름** 텍스트 상자에 **Britta** 및 **Simon**을 입력합니다.

    b. **전자 메일** 텍스트 상자에 프로비전하려는 Azure AD 계정의 전자 메일 주소를 입력합니다.

    c. **작업 위치** 텍스트 상자에 비즈니스 이름을 입력합니다.

	d. **저장 및 초대** 단추를 클릭합니다.

	>[AZURE.NOTE]AAD 계정 보유자에게 이메일이 발송되며 여기에 포함된 링크를 클릭하여 계정을 확인하면 계정이 활성화됩니다. 다른 Deputy 사용자 계정 생성 도구 또는 Deputy가 제공한 API를 사용하여 AAD 사용자 계정을 프로비전할 수 있습니다.
