...
사용자가 OAuth 2.0, OpenID Connect, SAML 또는 WS-Federation을 사용하는 페더레이션된 응용 프로그램에 액세스할 때 액세스 규칙이 평가됩니다. 또한 액세스 규칙은 Auth 2.0 및 OpenID Connect가 새로고침 토큰을 사용하여 액세스 토큰을 확보할 때 평가됩니다. 새로 고침 토큰을 사용할 때 정책 평가에 실패하면 invalid\_grant 오류가 반환됩니다. 이 오류는 사용자가 클라이언트에 다시 인증해야 함을 나타냅니다. 다단계 인증을 제공하도록 페더레이션 서비스 구성
