...
![로그인 위험 정책](./media/overview/1014.png "로그인 위험 정책")

Azure AD ID 보호를 사용하면 다음을 사용하여 위험한 로그인을 완화하도록 관리할 수 있습니다.

* 정책이 적용되는 사용자 및 그룹 설정:

    ![로그인 위험 정책](./media/overview/1015.png "로그인 위험 정책")
* 정책을 트리거하는 로그인 위험 수준 임계값 설정(낮음, 보통 또는 높음):

    ![로그인 위험 정책](./media/overview/1016.png "로그인 위험 정책")
* 정책이 트리거할 때 적용될 컨트롤 설정:  

    ![로그인 위험 정책](./media/overview/1017.png "로그인 위험 정책")
* 정책 상태 전환:

    ![MFA 등록](./media/overview/403.png "MFA 등록")
* 활성화하기 전에 변경의 영향 검토 및 평가:

    ![로그인 위험 정책](./media/overview/1018.png "로그인 위험 정책")
...
- **Azure AD ID 보호** 블레이드의 **구성** 섹션에서 **로그인 위험 정책**을 클릭합니다.
    ...
    ![사용자 위험 정책](./media/overview/1014.png "사용자 위험 정책")



## <a name="users-flagged-for-risk"></a>위험에 대한 플래그가 지정된 사용자

사용자에 대해 Azure Active Directory에서 검색된 모든 활성 [위험 이벤트](../reports-monitoring/concept-risk-events.md)는 사용자 위험이라는 논리적 개념을 파생시킵니다. 위험 플래그가 지정된 사용자는 손상되었을 수 있는 사용자 계정에 대한 표시기입니다.

![위험에 대한 플래그가 지정된 사용자](./media/overview/1200.png)
...
![사용자 위험](./media/overview/1031.png "사용자 위험")
...
1. **Azure AD ID 보호** 블레이드의 **조사** 아래에 있는 **위험 이벤트**를 클릭합니다.
    ...
    ![암호 수동 다시 설정](./media/overview/1002.png "암호 수동 다시 설정")
2. **위험 이벤트** 목록에서 위험을 클릭합니다.

    ![암호 수동 다시 설정](./media/overview/1003.png "암호 수동 다시 설정")
3. 위험 블레이드에서 사용자를 마우스 오른쪽 단추로 클릭합니다.

    ![암호 수동 다시 설정](./media/overview/1004.png "암호 수동 다시 설정")
...
![정책](./media/overview/1005.png "정책")

**관련 구성 대화 상자를 열려면**

1. **Azure AD ID 보호** 블레이드에서 **위험 플래그가 지정된 사용자**를 클릭합니다.

    ![암호 수동 다시 설정](./media/overview/1006.png "암호 수동 다시 설정")
2. 사용자 목록에서 하나 이상의 위험 이벤트와 사용자를 선택합니다.

    ![암호 수동 다시 설정](./media/overview/1007.png "암호 수동 다시 설정")
3. 사용자 블레이드에서 **암호 재설정**을 클릭합니다.

    ![암호 수동 다시 설정](./media/overview/1008.png "암호 수동 다시 설정")
...
![사용자 위험 정책](./media/overview/1009.png "사용자 위험 정책")

Azure AD ID 보호를 사용하면 다음을 사용하여 위험에 플래그가 지정된 사용자의 완화 및 수정을 관리할 수 있습니다.

* 정책이 적용되는 사용자 및 그룹 설정:

    ![사용자 위험 정책](./media/overview/1010.png "사용자 위험 정책")
* 정책을 트리거하는 사용자 위험 수준 임계값 설정(낮음, 보통 또는 높음):

    ![사용자 위험 정책](./media/overview/1011.png "사용자 위험 정책")
* 정책이 트리거할 때 적용될 컨트롤 설정:

    ![사용자 위험 정책](./media/overview/1012.png "사용자 위험 정책")
* 정책 상태 전환:

    ![사용자 위험 정책](./media/overview/403.png "MFA 등록")
* 활성화하기 전에 변경의 영향 검토 및 평가:

    ![사용자 위험 정책](./media/overview/1013.png "사용자 위험 정책")
...
![사용자 위험 정책](./media/overview/1019.png "사용자 위험 정책")
...
* 정책이 적용되는 사용자 및 그룹 설정:
    ...
    ![MFA 정책](./media/overview/1020.png "MFA 정책")
* 정책이 트리거할 때 적용될 컨트롤 설정:  

    ![MFA 정책](./media/overview/1021.png "MFA 정책")
* 정책 상태 전환:

    ![MFA 정책](./media/overview/403.png "MFA 정책")
* 현재 등록 상태 보기:

    ![MFA 정책](./media/overview/1022.png "MFA 정책")
...
- **Azure AD ID 보호** 블레이드의 **구성** 섹션에서 **다단계 인증 등록**을 클릭합니다.
    ...
    ![MFA 정책](./media/overview/1019.png "MFA 정책")
...
* [Azure Active Directory ID 보호에서 검색하는 취약성](vulnerabilities.md)

* [Azure Active Directory 위험 이벤트](../active-directory-identity-protection-risk-events.md)

* [Azure Active Directory ID 보호 알림](notifications.md)

* [Azure Active Directory ID 보호 플레이 북](playbook.md)

* [Azure Active Directory ID 보호 용어집](glossary.md)

* [Azure AD ID 보호를 사용하는 로그인 환경](flows.md)

* [Azure Active Directory ID 보호 - 사용자를 차단 해제하는 방법](howto-unblock-user.md)

* [Azure Active Directory ID 보호 및 Microsoft Graph 시작](graph-get-started.md)
