---
title: Azure Active 디렉터리 ID 보호 보안 개요
description: 보안 개요를 통해 조직의 보안 상태를 파악하는 방법을 알아봅니다.
...
Azure 포털의 [보안 개요를](https://aka.ms/IdentityProtectionRefresh) 통해 조직의 보안 상태를 파악할 수 있습니다. 잠재적 공격을 식별하고 정책의 효과를 이해하는 데 도움이 됩니다.
...
이 차트는 선택된 기간에 탐지된 위험한 로그인 수를 보여 줍니다. 로그인 위험 유형(실시간 또는 집계) 또는 로그인 위험 수준(낮음, 중간, 높음)을 기준으로 이 차트의 보기를 필터링할 수 있습니다. 보호되지 않는 로그인은 MFA가 요청되지 않은 성공한 실시간 위험 로그인입니다. (참고: 오프라인 검색으로 인해 위험한 로그인은 로그인 위험 정책에 의해 실시간으로 보호될 수 없습니다.) UTC 날짜 증분을 가리키면 해당 날짜에 탐지된 위험한 로그인 수가 표시됩니다. 이 차트를 클릭하면 ‘위험한 로그인’ 보고서로 이동됩니다.
...
'보호되지 않은 위험한 로그인' 타일은 조건부 액세스 정책, ID 보호 위험 정책 또는 사용자당 MFA에 의해 차단되지 않았거나 MFA에 의해 문제가 되지 않은 성공적이고 실시간 위험한 로그인의 지난 주 수를 보여 주며, 성공했으며 MFA가 요청되지 않은 잠재적으로 손상된 로그인입니다. 나중에 해당 로그인을 보호하려면 로그인 위험 정책을 적용합니다. ‘보호되지 않는 위험한 로그인’ 타일을 클릭하면 로그인 위험 정책 구성 블레이드로 리디렉션됩니다. 이 블레이드에서 지정된 위험 수준으로 로그인할 경우 MFA를 요구하도록 로그인 위험 정책을 구성할 수 있습니다.
...
ID 보안 점수는 보안 상태를 측정하고 업계 패턴과 비교합니다. '아이덴티티 보안 점수(미리 보기)' 타일을 클릭하면 보안 태세 개선에 대해 자세히 알아볼 수 있는 '아이덴티티 보안 점수' 블레이드로 리디렉션됩니다.
