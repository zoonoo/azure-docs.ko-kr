---
title: Azure 활성 디렉터리 ID 보호를 위한 Microsoft 그래프 API
description: Azure Active Directory에서 Microsoft 그래프 위험 검색 및 관련 정보를 쿼리하는 방법 알아보기
...
Microsoft Graph는 Microsoft의 통합된 API 엔드포인트이며 [Azure Active Directory ID 보호](../active-directory-identityprotection.md) API의 시작점입니다. 위험한 사용자 및 로그인에 대한 정보를 노출하는 API는 4가지가 있습니다. 첫 번째 API인 **riskDetection을**사용하면 Microsoft 그래프에서 연결된 위험 검색 및 검색에 대한 관련 정보 모두 목록을 쿼리할 수 있습니다. 두 번째 API인 **riskyUsers**를 사용하면 위험으로 검색된 사용자 ID 보호에 대한 정보에 관하여 Microsoft Graph를 쿼리할 수 있습니다. 세 번째 API인 **signIn**을 사용하면 위험 상태, 세부 정보 및 수준과 관련된 특정 속성을 사용하여 Azure AD 로그인의 정보에 대한 Microsoft Graph를 쿼리할 수 있습니다. 네 번째 API인 **identityRiskEvents를**사용하면 Microsoft 그래프에서 위험 검색 및 관련 정보 목록을 쿼리할 수 [있습니다.](../reports-monitoring/concept-risk-events.md) identityRiskEvents API는 2020년 1월 10일에 더 이상 사용되지 않습니다. 대신 **위험 탐지 API를** 사용하는 것이 좋습니다. 이 문서에서는 Microsoft Graph에 연결 및 이러한 API 쿼리를 사용하여 시작합니다. 자세한 소개, 전체 설명서 및 Graph Explorer에 대한 액세스는 [Microsoft Graph 사이트](https://graph.microsoft.io/) 또는 이러한 API에 대한 특정 참조 설명서를 참조하세요.

* [위험 탐지 API](/graph/api/resources/riskdetection?view=graph-rest-beta)
...
* [identityRiskEvents API는](/graph/api/resources/identityriskevent?view=graph-rest-beta) *2020년 1월 10일 더 이상 사용되지* 않습니다.
...
1. **만들기** 페이지에서 다음 단계를 수행합니다.
   ...
   1. **이름** 텍스트 상자에서 응용 프로그램의 이름을 입력합니다(예: Azure AD 위험 감지 API 응용 프로그램).

   1. **유형**으로 **웹 애플리케이션 및/또는 Web API**를 선택합니다.

   1. 로그인 **URL** 텍스트 상자에 을 `http://localhost`입력합니다.
...
1. **키** 페이지에서 다음 단계를 수행합니다.
   ...
   1. 키 **설명** 텍스트 상자에서 설명을 입력합니다(예: *Azure AD 위험 감지).*
   1. **기간**으로 **1년**을 선택합니다.
   1. **저장**을 클릭합니다.
...
## <a name="authenticate-to-microsoft-graph-and-query-the-identity-risk-detections-api"></a>Microsoft 그래프인증 및 ID 위험 검색 API 쿼리
...
응답이 성공하면 OData JSON 형식의 ID 위험 검색 및 관련 데이터의 모음으로, 적합한 방식으로 구문 분석및 처리될 수 있습니다.
...
### <a name="get-all-of-the-offline-risk-detections-riskdetection-api"></a>모든 오프라인 위험 탐지(위험 감지 API) 받기

ID 보호 로그인 위험 정책을 사용하면 위험이 실시간으로 감지될 때 조건을 적용할 수 있습니다. 그러나 오프라인에서 검색된 검색은 어떨까요? 오프라인에서 발생한 검색과 로그인 위험 정책을 트리거하지 않은 검색을 이해하려면 riskDetection API를 쿼리할 수 있습니다.
...
ID 보호 위험 기반 정책이 조직에 미치는 영향을 이해하려면 위험한 로그인 정책에 의해 트리거되는 MFA 챌린지를 성공적으로 통과한 모든 사용자를 쿼리할 수 있습니다. 이 정보는 위험에서 잘못 감지했을 수 있는 사용자 ID 보호 및 AI에서 위험하다고 판단하는 작업을 수행할 수 있는 합법적인 사용자를 이해하는 데 도움이 될 수 있습니다.
...
이제 ID 위험 검색을 쿼리하고 적합하다고 보이는 데이터를 사용할 수 있습니다.
...
- [Azure Active 디렉터리 ID 보호에서 검색된 위험 검색 유형](../reports-monitoring/concept-risk-events.md)
