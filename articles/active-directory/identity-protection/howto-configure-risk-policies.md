...
Azure AD는 잠재적으로 손상 된 id에 대 한 지표로 표시 되는 위험 검색을 검색 합니다. 위험 정책을 구성하여 탐지 결과에 대한 자동화된 응답을 정의할 수 있습니다.

- 로그인 위험 정책을 사용 하면 사용자가 로그인 하는 동안 검색 된 실시간 위험 검색에 대 한 응답을 구성할 수 있습니다. 
...
사용자가 로그인 위험 정책에 의해 트리거되는 MFA 프롬프트를 성공적으로 완료 하면 로그인이 합법적인 사용자 로부터 시작 된 것으로 Id 보호에 피드백을 제공 합니다. 따라서 MFA 프롬프트를 트리거한 로그인 위험 검색은 자동으로 닫히고 Id 보호로 인해이 이벤트가 사용자의 권한 상승에 영향을 주지 않습니다. 로그인 위험 정책을 사용 하도록 설정 하면 사용자가 MFA를 묻는 메시지가 표시 되 면 자동으로 수정 하 여 관련 된 위험한 로그인을 자동으로 닫을 수 있으므로 위험한 로그인 noisiness 줄일 수 있습니다.
...
- 정책이 적용되는 사용자 및 그룹:
   ...
   ![사용자 및 그룹](./media/howto-configure-risk-policies/11.png)
...
- 로그인 위험 수준에 도달할 때 적용할 액세스 유형:  
   ...
   ![액세스](./media/howto-configure-risk-policies/13.png)
...
Azure AD는 사용자의 각 로그인을 분석합니다. 분석 목표는 로그인과 함께 발생하는 의심스러운 작업을 감지하는 것입니다. Azure AD에서 시스템이 검색할 수 있는 의심 스러운 작업을 위험 감지 라고도 합니다. 일부 위험 검색은 실시간으로 검색 될 수 있지만 더 많은 시간이 필요한 위험 검색도 있습니다. 예를 들어, 비정상적인 위치로 불가능한 이동을 감지하기 위해 시스템은 사용자의 일반 동작을 학습하기 위해 14일의 초기 학습 기간을 필요로 합니다. 검색 된 위험 검색을 해결 하는 몇 가지 옵션이 있습니다. 예를 들어, 개별 위험 검색을 수동으로 해결 하거나 로그인 위험 또는 사용자 위험 조건부 액세스 정책을 사용 하 여 해결할 수 있습니다.

사용자에 대해 검색 되 고 해결 되지 않은 모든 위험 검색은 활성 위험 검색 이라고 합니다. 사용자와 연결 된 활성 위험 검색을 사용자 위험 이라고 합니다. 사용자 위험에 따라 Azure AD는 사용자가 손상되었을 확률(낮음, 보통, 높음)을 계산합니다. 이 확률을 사용자 위험 수준이라고 합니다.
...
- 정책이 적용되는 사용자 및 그룹:
   ...
   ![사용자 및 그룹](./media/howto-configure-risk-policies/111.png)
...
- 로그인 위험 수준에 도달할 때 적용할 액세스 유형:  
   ...
   ![액세스](./media/howto-configure-risk-policies/113.png)
...
![차단](./media/howto-configure-risk-policies/116.png)

로그인 차단:

* 영향을 받는 사용자에 대 한 새 사용자 위험 검색을 생성 하지 않습니다.
* 관리자가 사용자의 id에 영향을 주는 위험 검색을 수동으로 재구성 하 고 보안 상태로 복원할 수 있습니다.
