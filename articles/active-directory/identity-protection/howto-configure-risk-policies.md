...
사용자가 로그인 위험 정책에 의해 트리거되는 MFA 프롬프트를 성공적으로 완료 하면 로그인이 합법적인 사용자 로부터 시작 된 것으로 Id 보호에 피드백을 제공 합니다. 따라서 MFA 프롬프트를 트리거한 로그인 위험 이벤트는 자동으로 닫히고 Id 보호로 인해이 이벤트가 사용자 위험에 노출 되지 않습니다. 로그인 위험 정책을 사용 하도록 설정 하면 사용자가 MFA를 묻는 메시지가 표시 되 면 자동으로 수정 하 여 관련 된 위험한 로그인을 자동으로 닫을 수 있으므로 위험한 로그인 noisiness 줄일 수 있습니다.
