...
동일한 사용자에 대 한 두 개의 로그인이 검색 되는 경우 (하나 이상의 로그인이 비정상적인 로그인 위치에서 발생 하 고) 로그인 사이의 시간이이 두 항목 사이에서 실제로 이동 하는 데 걸리는 최소 시간 보다 짧은 경우에 발생 하는 위험 검색 위치도.  

### <a name="investigation"></a>조사
수정 또는 완화 단계가 필요한 지 여부를 결정 하기 위한 작업, 로그 및 기타 관련 정보를 검토 하 고, id가 손상 되었는지 파악 하 고, 손상 된 경우를 이해 하는 프로세스입니다. id가 사용 되었습니다.
...
위험 검색 시 전역 관리자, 대금 청구 관리자, 서비스 관리자, 사용자 관리자, 암호 등 Azure Active Directory에 있는 하나 이상의 리소스에 대 한 영구 또는 임시 관리 권한이 있는 사용자입니다. 관리자나. 
...
### <a name="risk-detection-type"></a>위험 검색 유형
