<properties
	pageTitle="조직에서 비즈니스용 Microsoft Windows Hello 사용 | Microsoft Azure"
...
# 조직에서 비즈니스용 Microsoft Windows Hello 사용

Windows 10 도메인에 가입된 장치와 Azure AD(Azure Active Directory)를 연결한 후에 조직에서 비즈니스용 Microsoft Windows Hello를 사용하도록 설정하려면 다음을 수행합니다.

## System Center Configuration Manager 현재 분기 배포 
비즈니스용 Windows Hello 키에 따라 사용자 인증서를 배포하려면 다음이 필요합니다.

- **System Center Configuration Manager 현재 분기** 버전 1606 이상을 설치해야 합니다. 자세한 내용은 [System Center Configuration Manager용 문서](https://technet.microsoft.com/library/mt346023.aspx) 및 [System Center Configuration Manager 팀 블로그](http://blogs.technet.com/b/configmgrteam/archive/2015/09/23/now-available-update-for-system-center-config-manager-tp3.aspx)를 참조하세요.
- **PKI(공개 키 인프라)**: 사용자 인증서를 사용하여 비즈니스용 Microsoft Windows Hello를 사용하려면 PKI가 제 위치에 있어야 합니다. PKI가 아직 없거나 사용자 인증서에 사용하지 않으려면 다음을 수행할 수 있습니다.
...
## Active Directory에서 그룹 정책을 통해 비즈니스용 Microsoft Windows Hello 구성

 Windows Server Active Directory에서 그룹 정책을 사용하여 Windows에 대한 사용자 로그인에서 비즈니스용 Windows Hello 사용자 자격 증명을 프로비전하도록 Windows 10 도메인에 가입된 장치를 구성할 수 있습니다.
...
## System Center Configuration Manager를 사용하여 비즈니스용 Windows Hello 구성

System Center Configuration Manager를 사용해야만 Windows 10 버전 1607에 대해 비즈니스용 Windows Hello 정책을 배포할 수 있습니다. Windows 10 컴퓨터의 버전을 조회하는 경우 Windows 10 컴퓨터에서 winver를 시작하세요.
...
2. **지원되는 플랫폼** 대화 상자에서 이 비즈니스용 Windows Hello 프로필로 프로비전된 플랫폼을 선택하고 **다음**을 클릭합니다.

	![비즈니스용 Windows Hello 구성](./media/active-directory-azureadjoin-passport-deployment/04.png)


2. **설정** 대화 상자에서 다음 단계를 수행합니다.
...
2. **요약** 대화 상자에서 **다음**을 클릭합니다.

2. **완료** 대화 상자에서 **닫기**를 클릭합니다.


2. 위쪽의 도구 모음에서 **배포**를 클릭합니다.

	![비즈니스용 Windows Hello 구성](./media/active-directory-azureadjoin-passport-deployment/06.png)



## 인증서 프로필을 구성하여 Configuration Manager에서 비즈니스용 Windows Hello 등록 인증서 사용
