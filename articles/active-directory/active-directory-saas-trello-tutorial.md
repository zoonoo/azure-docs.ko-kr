...
* 사용자가 해당 Azure AD 계정으로 Trello에 자동으로 로그온(SSO)되도록 설정할 수 있습니다.
...
SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Trello 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Trello의 관련 사용자 간에 연결 관계가 설정되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Trello의 **Username** 값으로 할당하여 설정합니다. Trello에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
2. **SAML 토큰 특성** 대화 상자에서 아래 테이블의 각 행에 대해 다음 단계를 수행합니다.
   ...
   3. **특성 값** 목록에서 해당 행에 표시된 값을 선택합니다.
...
4. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
   ...
   1. 회신 URL 텍스트 상자에 다음 `https://trello.com/auth/saml/consume/<enterprise>` 패턴을 사용하여 URL을 입력합니다.
   2. **다음**을 클릭합니다.
...
5. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 **"고급 설정 표시(선택 사항)"**를 클릭하고 **로그온 URL**을 입력합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-trello-tutorial/tutorial_trello_08.png)
   
   1. **로그온 URL** 텍스트 상자에서 다음 패턴 `https://trello.com/auth/saml/consume/<enterprise>`을 사용하여 URL을 입력합니다.
   2. **다음**을 클릭합니다.

6. **Trello에서 Single Sign-On 구성** 페이지에서 **인증서 다운로드**를 클릭하여 컴퓨터에 파일을 저장합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-trello-tutorial/tutorial_trello_09.png)

7. 응용 프로그램에 구성된 SSO를 가져오려면 [Trello Enterprise SSO 구성](https://trello.com/sso-configuration) 페이지로 이동하여 Trello 팀에 로그온 URL을 보내고 다운로드한 인증서를 첨부합니다.
