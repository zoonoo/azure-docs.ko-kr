...
사용자가 암호 동기화 범위 내에 있을경우, 클라우드 계정 암호는 “\*Never Expire\*”로 설정됩니다. 즉, 사용자의 암호가 온-프레미스 환경에서 만료되어도 만료된 암호를 사용하여 클라우드 서비스에 계속 로그인할 수 있습니다.
...
이벤트에 영향을 받는 개체는 해당 앵커 및 DN 값으로 식별됩니다. 앵커 값에 해당하는**ImmutableId**값은 Get MsoUser cmdlet에 의해 사용자에게 반환됩니다.

개체 식별자 외에도**이벤트 ID 656**는 온-프레미스 Active Directory에서 사용자의 암호가 변경된 날짜를 제공합니다.
