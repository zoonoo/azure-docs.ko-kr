...
3. **그룹** 페이지에서 필요한 정보를 입력합니다.
    ...
    - **멤버 자격 유형(필수).** 미리 정의된 멤버 자격 유형을 선택합니다. 다음 내용이 포함됩니다.
        ...
        - **동적 장치.** 동적 그룹 규칙을 사용하여 장치를 자동으로 추가하고 제거할 수 있습니다. 장치의 특성이 변경되면 시스템은 디렉터리의 동적 그룹 규칙을 확인하여 장치가 규칙 요구 사항을 충족하는지(추가됨), 아니면 더 이상 규칙 요구 사항을 충족하지 않는지(제거됨) 확인합니다.

        >[!Important]
        >장치 또는 사용자 중 하나의 동적 그룹만 만들 수 있습니다. 또한 장치 소유자의 특성을 기반으로 장치 그룹을 만들 수는 없습니다. 장치 멤버 자격 규칙은 장치 특성만 참조할 수 있습니다. 사용자 및 장치의 동적 그룹을 만드는 방법에 대한 자세한 내용은 [동적 그룹 만들기 및 상태 확인](../users-groups-roles/groups-create-rule.md)을 참조하세요.
