...
1. **Azure Active Directory**를 검색하고 선택합니다.
    ...
    ![그룹이 표시된 Azure AD 페이지](media/active-directory-groups-create-azure-portal/search-active-directory.png)
...
1. **새 그룹** 페이지에서 필요한 정보를 입력합니다.

    ![예제 정보로 채워진 새 그룹 페이지](media/active-directory-groups-create-azure-portal/new-group-blade.png)

   - **그룹 유형(필수).** 미리 정의된 그룹 유형을 선택합니다. 여기에는 **보안**이 포함됩니다. 사용자 그룹의 공유 리소스에 대한 멤버 및 컴퓨터 액세스를 관리하는 데 사용됩니다. 예를 들어 특정 보안 정책의 보안 그룹을 만들 수 있습니다. 이렇게 하면 각 멤버에 개별적으로 권한을 추가할 필요 없이 한 번에 모든 멤버에게 권한 집합을 제공할 수 있습니다. 리소스의 액세스 관리 방법에 대한 자세한 내용은 [Azure Active Directory 그룹을 사용하여 리소스에 대한 액세스 관리](active-directory-manage-groups.md)를 참조하세요.
   ...
   - **그룹 이메일 주소(필수).** 그룹의 이메일 주소를 추가하거나 자동으로 채워진 이메일 주소를 유지합니다.

   - **그룹 설명.** 그룹에 선택적 설명을 추가합니다.

   - **멤버 자격 유형(필수).** 미리 정의된 멤버 자격 유형을 선택합니다. 여기에는 **할당됨**이 포함됩니다. 특정 사용자를 이 그룹의 멤버로서, 고유한 권한을 가지도록 추가할 수 있습니다. 이 문서에서는 이 옵션을 사용합니다.
