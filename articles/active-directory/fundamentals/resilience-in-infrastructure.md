...
호출의 구성 요소 중 하나라도 실패 하면 인증 시스템에 대 한 모든 호출이 중단 될 수 있습니다. 기본 구성 요소 오류로 인해 인증이 중단 되 면 사용자는 응용 프로그램에 액세스 하지 않습니다. 따라서 이러한 호출에서 인증 호출 수와 종속성 수를 줄이는 것이 복원 력에 중요 합니다. 응용 프로그램 개발자는 토큰을 요청 하는 빈도에 대 한 일부 제어를 어설션할 수 있습니다. 예를 들어 가능한 경우 응용 프로그램에 대해 Azure AD 관리 Id를 사용 하 고 있는지 확인 하기 위해 개발자와 협력 합니다. 
