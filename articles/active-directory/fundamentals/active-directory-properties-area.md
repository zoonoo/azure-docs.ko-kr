...
이 문서에서는 테넌트 관리자가 Azure 포털을 통해 조직의 Azure Active Directory(Azure AD) 테넌트에 개인 정보 관련 정보를 추가하는 방법을 설명합니다.

내부 직원과 외부 게스트가 정책을 검토할 수 있도록 글로벌 개인 정보 보호 연락처와 조직의 개인 정보 취급 방침을 모두 추가하는 것이 좋습니다. 개인정보처리방침이 고유하게 생성되고 각 비즈니스에 맞게 조정되기 때문에 도움말은 변호사에게 문의하는 것이 좋습니다.
...
3.    직원의 개인 정보를 추가합니다.
    ...
    - **개인정보처리방침 URL** 조직에서 내부 및 외부 게스트의 데이터 개인 정보를 처리하는 방법을 설명하는 조직의 문서에 대한 링크를 입력합니다.

        >[!Important]
        >개인 정보 취급 방침 또는 개인 정보 보호 연락처를 포함하지 않는 경우 외부 숙박 고객에게 **검토 권한** 상자에 텍스트가 ** < _your org name_ **표시되어> 조직 이름은 검토할 수 있는 약관에 대한 링크를 제공하지 않았습니다. 예를 들어 게스트 사용자는 B2B 협업을 통해 조직에 액세스하는 초대를 받을 때 이 메시지를 확인하게 됩니다.
