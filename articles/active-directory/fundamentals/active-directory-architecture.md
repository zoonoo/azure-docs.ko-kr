...
데이터 계층에는 읽기 쓰기 기능을 제공하는 몇 가지 프런트 엔드 서비스가 있습니다. 아래 다이어그램은 지리적으로 분산된 데이터 센터 전체에서 단일 디렉터리 파티션의 구성 요소가 어떻게 전달되는지 보여 주며,

  ![단일 디렉토리 파티션 다이어그램](./media/active-directory-architecture/active-directory-architecture.png)
...
모든 디렉터리 *읽기는* 서로 다른 지역에 물리적으로 위치한 데이터 센터에 있는 *보조 복제본에서*서비스됩니다. 데이터가 비동기적으로 복제되는 경우 보조 복제본이 많이 생성됩니다. 인증 요청과 같은 디렉터리 읽기는 고객과 가까운 데이터 센터에서 서비스됩니다. 보조 복제본은 읽기 확장성을 담당합니다.
...
가용성(또는 가동 중단)은 시스템의 기능을 정의하여 방해 받지 않고 실행됩니다. Azure AD의 고가용성의 핵심은 서비스가 지리적으로 분산된 여러 데이터 센터에서 트래픽을 빠르게 이동할 수 있다는 것입니다. 각 데이터 센터는 독립적이며, 이를 통해 상호 연관되지 않은 실패 모드를 사용할 수 있습니다. 이러한 고가용성 설계를 통해 Azure AD는 유지 관리 활동에 대한 가동 중지 시간이 필요하지 않습니다.
...
쓰기는 승인되기 전에 적어도 두 개의 데이터 센터에 지속적으로 커밋됩니다. 이 문제는 먼저 주 에 쓰기를 커밋한 다음 적어도 하나의 다른 데이터 센터에 쓰기를 즉시 복제하여 발생합니다. 이 쓰기 작업은 주 를 호스팅하는 데이터 센터의 잠재적 치명적인 손실이 데이터 손실을 초래하지 않도록 합니다.
...
Azure AD의 복제본은 전 세계에 걸쳐 위치한 데이터 센터에 저장됩니다. 자세한 내용은 [Azure 글로벌 인프라를](https://azure.microsoft.com/global-infrastructure/)참조하십시오.

Azure AD는 다음과 같은 특성을 가진 데이터 센터 에서 작동합니다.

* Authentication, Graph 및 기타 AD 서비스는 게이트웨이 서비스에 포함됩니다. 게이트웨이는 이러한 서비스의 부하 분산을 관리합니다. 또한 트랜잭션 상태 프로브에서 비정상적인 서버가 검색되면 자동으로 장애 조치(failover)합니다. 이러한 상태 프로브를 기반으로 게이트웨이는 트래픽을 정상 데이터 센터로 동적으로 라우팅합니다.
* *읽기의*경우 디렉터리에는 여러 데이터 센터에서 작동하는 활성 활성 구성에서 보조 복제본 및 해당 프런트 엔드 서비스가 있습니다. 전체 데이터 센터가 실패하면 트래픽이 자동으로 다른 데이터 센터로 라우팅됩니다.
 **쓰기의*경우 디렉터리(마스터) 복제본은 계획된(새 주 복제본이 이전 주 복제본에 동기화) 또는 긴급 장애 조치 절차를 통해 데이터 센터 전체에서 장애 조치됩니다. 데이터 내구성은 두 개 이상의 데이터 센터에 대한 커밋을 복제하여 달성됩니다.
...
Azure AD의 Microsoft 그래프 API를 사용하는 응용 프로그램 쓰기는 읽기-쓰기 일관성을 위해 선호도를 유지에서 디렉터리 복제본에 추상화됩니다. Microsoft Graph API 서비스는 읽기에 사용되는 보조 복제본과 친화성이 있는 논리 세션을 유지 관리합니다. 선호도는 서비스가 보조 복제 데이터베이스 데이터 센터의 분산 캐시를 사용하여 캐시하는 "복제 토큰"에 캡처됩니다. 이 토큰은 동일한 논리 세션의 후속 작업에 사용됩니다. 동일한 논리 세션을 계속 사용하려면 후속 요청을 동일한 Azure AD 데이터 센터로 라우팅해야 합니다. 디렉터리 클라이언트 요청이 여러 Azure AD 데이터 센터로 라우팅되는 경우 논리 세션을 계속할 수 없습니다. 이 경우 클라이언트에는 독립적인 읽기-쓰기 구성이 있는 여러 논리 세션이 있습니다.

 >[!NOTE]
 >쓰기는 논리 세션의 읽기가 실행된 보조 복제본으로 즉시 복제됩니다.

#### <a name="backup-protection"></a>Backup 보호
