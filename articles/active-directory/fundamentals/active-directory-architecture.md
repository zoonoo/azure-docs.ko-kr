...
데이터 계층에는 읽기 쓰기 기능을 제공하는 몇 가지 프런트 엔드 서비스가 있습니다. 아래 다이어그램은 단일 디렉터리 파티션의 구성 요소가 지리적으로 분산 된 데이터 센터에서 배달 되는 방법을 보여 줍니다.
...
모든 디렉터리 *읽기* 는 여러 지리적 위치에 물리적으로 배치 된 데이터 센터에 있는 *보조 복제본*에서 처리 됩니다. 데이터가 비동기적으로 복제되는 경우 보조 복제본이 많이 생성됩니다. 인증 요청과 같은 디렉터리 읽기는 고객과 가까운 데이터 센터에서 서비스 됩니다. 보조 복제본은 읽기 확장성을 담당합니다.
...
가용성(또는 가동 중단)은 시스템의 기능을 정의하여 방해 받지 않고 실행됩니다. Azure AD의 고가용성에 대 한 핵심은 서비스에서 여러 지리적으로 분산 된 데이터 센터 간에 트래픽을 빠르게 이동할 수 있다는 것입니다. 각 데이터 센터는 독립적 이므로 상관 관계가 지정 되지 않은 오류 모드를 사용할 수 있습니다. 이 고가용성 설계를 통해 Azure AD는 유지 관리 작업에 대해 가동 중지 시간이 필요 하지 않습니다.
...
Azure AD의 복제본은 전 세계에 걸쳐 위치한 데이터 센터에 저장됩니다. 자세한 내용은 [Azure 글로벌 인프라](https://azure.microsoft.com/global-infrastructure/)를 참조 하세요.

Azure AD는 다음과 같은 특징을 가진 데이터 센터에서 작동 합니다.
...
* *읽기*의 경우 디렉터리에는 여러 데이터 센터에서 작동 하는 활성-활성 구성의 보조 복제본과 해당 프런트 엔드 서비스가 있습니다. 전체 데이터 센터의 장애가 발생 하는 경우 트래픽은 자동으로 다른 데이터 센터로 라우팅됩니다.
 \* *쓰기*의 경우 디렉터리는 계획 된 (새 주 복제본을 이전 주 복제본으로 동기화) 또는 응급 장애 조치 (failover)를 통해 데이터 센터에서 주 (마스터) 복제본을 장애 조치 (failover) 합니다. 데이터 내구성은 두 개 이상의 데이터 센터에 커밋을 복제 하 여 이루어집니다.
...
Azure AD의 Graph API를 사용하는 애플리케이션은 읽기 쓰기 일관성을 위해 유지 관리 선호도에서 디렉터리 복제본으로 추상화됩니다. Azure AD Graph 서비스는 읽기에 사용 되는 보조 복제본에 대 한 선호도를 가진 논리적 세션을 유지 관리 합니다. 선호도는 graph 서비스가 보조 복제본 데이터 센터의 분산 캐시를 사용 하 여 캐시 하는 "복제본 토큰"에 캡처됩니다. 이 토큰은 동일한 논리 세션의 후속 작업에 사용됩니다. 동일한 논리 세션을 계속 사용 하려면 후속 요청을 동일한 Azure AD datacenter로 라우팅해야 합니다. 디렉터리 클라이언트 요청이 여러 Azure AD 데이터 센터로 라우팅되는 경우 논리 세션을 계속할 수 없습니다. 이 경우 클라이언트에는 독립적인 읽기/쓰기 일관성 있는 여러 논리 세션이 있습니다.

 >[!NOTE]
 >쓰기는 논리 세션의 읽기가 실행된 보조 복제본으로 즉시 복제됩니다.

#### <a name="backup-protection"></a>백업 보호
