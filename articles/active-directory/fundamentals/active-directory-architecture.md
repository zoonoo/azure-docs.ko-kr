...
데이터 계층에는 읽기 쓰기 기능을 제공하는 몇 가지 프런트 엔드 서비스가 있습니다. 아래 다이어그램에서는 지리적으로 분산된 된 데이터 센터 전체에 걸쳐 단일 디렉터리 파티션의 구성 요소는 배달 하는 방법을 보여 줍니다. 
...
모든 디렉터리 *읽습니다* 에서 처리 됩니다 *보조 복제본*를 물리적으로 여러 지역에 걸쳐 위치한 데이터 센터는 합니다. 데이터가 비동기적으로 복제되는 경우 보조 복제본이 많이 생성됩니다. 인증 요청을 같은 디렉터리 읽기는 고객에 가까운 데이터 센터에서 처리 됩니다. 보조 복제본은 읽기 확장성을 담당합니다.
...
가용성(또는 가동 중단)은 시스템의 기능을 정의하여 방해 받지 않고 실행됩니다. Azure AD의 고가용성을 키는 서비스 지리적으로 분산 된 여러 데이터 센터에서 트래픽을 이동할 수 있습니다 신속 하 게 합니다. 취소 상관 관계가 지정 된 오류 모드를 사용 하도록 설정 하는 각 데이터 센터는 독립적입니다. Azure AD는이 고가용성 설계를 통해 유지 관리 작업에 대 한 가동 중지 시간 없이 필요합니다.
...
쓰기는 승인 되기 전에 두 개 이상의 데이터 센터에 영구적으로 커밋됩니다. 첫 번째 커밋 주 서버, 쓰기 및 다음 즉시 쓰기를 하나 이상의 다른 데이터 센터에 복제 하 여 발생 합니다. 이 쓰기 작업을 통해 잠재적인 데이터 손실을 주 데이터베이스를 호스팅하는 데이터 센터의 치명적인 손실을 발생 하지 않습니다.
...
Azure AD의 복제본은 전 세계에 걸쳐 위치한 데이터 센터에 저장됩니다. 자세한 내용은 [Azure 글로벌 인프라](https://azure.microsoft.com/global-infrastructure/)합니다.

Azure AD는 다음 특성을 사용 하 여 데이터 센터에서 작동합니다.

 * Authentication, Graph 및 기타 AD 서비스는 게이트웨이 서비스에 포함됩니다. 게이트웨이는 이러한 서비스의 부하 분산을 관리합니다. 또한 트랜잭션 상태 프로브에서 비정상적인 서버가 검색되면 자동으로 장애 조치(failover)합니다. 이러한 상태 프로브에 따라 게이트웨이에 동적으로 트래픽을 라우팅합니다 정상 데이터 센터에 있습니다.
 * 에 대 한 *읽습니다*, 여러 데이터 센터에서 운영 하는 활성-활성 구성에서 보조 복제본 및 해당 프런트 엔드 서비스를에 하는 디렉터리입니다. 전체 데이터 센터의 오류가 발생 트래픽은 다른 데이터 센터에 자동으로 라우팅됩니다 됩니다.
 *  에 대 한 *씁니다*, 디렉터리는 주 복제본 장애 조치 (마스터) 데이터 센터를 통해 계획 (새 주 복제본은 이전 주 복제본에 동기화 됨) 또는 응급 장애 조치 절차입니다. 데이터 지 속성은 두 개 이상의 데이터 센터에 커밋을 복제 하 여 수행 됩니다.
...
Azure AD의 Graph API를 사용하는 애플리케이션은 읽기 쓰기 일관성을 위해 유지 관리 선호도에서 디렉터리 복제본으로 추상화됩니다. Azure AD Graph 서비스 선호도 논리적 세션을 유지 관리; 읽기에 사용 되는 보조 복제본으로 선호도는 graph 서비스는 보조 복제본 데이터 센터에 분산된 된 캐시를 사용 하 여 캐시 하는 "복제본 토큰"에 캡처됩니다. 이 토큰은 동일한 논리 세션의 후속 작업에 사용됩니다. 동일한 논리 세션을 사용 하 여 계속 하려면 후속 요청을 동일한 Azure AD 데이터 센터에 라우팅해야 합니다. 해당 디렉터리 클라이언트에서 요청 하는 여러 Azure AD 데이터 센터;에 라우팅되는 경우 논리적 세션을 계속할 수 아닙니다. 이 경우 클라이언트에 관계 없이 읽기 / 쓰기 일관성을 포함 하는 여러 논리 세션입니다.
