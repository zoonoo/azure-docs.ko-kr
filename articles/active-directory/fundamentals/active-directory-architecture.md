 ...
 *  *쓰기*의 경우 디렉터리는 계획 된 (새 주 복제본이 이전 주 복제본으로 동기화 됨) 또는 응급 장애 조치 (failover)를 통해 데이터 센터에서 주 (마스터) 복제본을 장애 조치 합니다. 데이터 내구성은 두 개 이상의 데이터 센터에 커밋을 복제 하 여 이루어집니다.
