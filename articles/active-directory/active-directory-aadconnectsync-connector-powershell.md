...
PowerShell 커넥터를 사용하면 동기화 서비스를 Windows PowerShell 기반 응용 프로그램 프로그래밍 인터페이스(API)를 제공하는 외부 시스템과 통합할 수 있습니다. 커넥터는 호출 기반 ECMA2(extensible connectivity management agent 2) 프레임 워크 및 Windows PowerShell의 기능 간에 브리지를 제공합니다. ECMA 프레임 워크에 대한 자세한 내용은 [Extensible Connectivity 2.2 관리 에이전트 참조](https://msdn.microsoft.com/library/windows/desktop/hh859557.aspx)를 참조하세요.
...
동기화 서비스에서 Windows PowerShell 커넥터를 만들려면 동기화 서비스에서 요청하는 단계를 실행하는 일련의 Windows PowerShell 스크립트를 제공해야 합니다. 데이터 원본에 따라 필요한 기능에 연결하고 구현해야 하는 스크립트가 달라집니다. 이 섹션은 필요할 때 뿐만 아니라 구현할 수 있는 스크립트 각각을 간략하게 설명합니다.

Windows PowerShell 커넥터는 동기화 서비스 데이터베이스 내에서 스크립트를 각각 저장하도록 설계됩니다. 파일 시스템에 저장된 스크립트를 실행할 수 있지만 각 스크립트의 본문을 직접 커넥터의 구성에 삽입하는 것이 훨씬 쉽습니다.
...
다음으로 원격 시스템에 연결하기 위한 구성 매개 변수를 제공할 수 있습니다. 이러한 매개 변수는 동기화 서비스에 의해 안전하게 저장되고 커넥터를 실행할 때 Windows PowerShell 스크립트를 사용할 수 있도록 만들어 집니다.
...
커넥터는 관리자가 구성에서 공유 Windows PowerShell 모듈을 저장하도록 합니다. 커넥터가 스크립트를 실행할 때 Windows PowerShell 모듈을 파일 시스템에 추출하므로 각 스크립트에서 가져올 수 있습니다.
...
유효성 검사 스크립트는 관리자가 제공한 커넥터 구성 매개 변수가 유효한지 확인하는 데 사용될 수 있는 선택적 Windows PowerShell 스크립트입니다. 서버 및 연결 자격 증명 유효성 검사 뿐만 아니라 연결 매개 변수는 유효성 검사 스크립트의 일반적인 사용입니다. 유효성 검사 스크립트는 다음과 같은 탭 뒤에 호출되고 대화 상자에서 수정됩니다.
...
스키마 검색 스크립트는 필수입니다. 이 스크립트는 특성 흐름 규칙을 구성할 때 동기화 서비스에서 사용할 개체 형식, 특성 및 특성 제약 조건을 반환합니다. 스키마 검색 스크립트는 커넥터를 만드는 동안 실행되고 커넥터의 스키마를 채우며 이후에 동기화 서비스 관리자에서 스키마 새로 고침 기능에서 수행됩니다.
...
지금까지 설명한 표준 구성 설정 외에도 커넥터의 인스턴스에 지정된 추가 사용자 지정 구성 설정을 정의할 수 있습니다. 이러한 매개 변수는 파티션, 커넥터에서 지정되거나 단계 수준을 실행하고 관련 Windows PowerShell 스크립트에서 액세스할 수 있습니다. 사용자 지정 구성 설정은 일반 텍스트 형식으로 동기화 서비스 데이터베이스에 저장하거나 암호화될 수 있습니다. 동기화 서비스는 자동으로 필요할 때 안전한 구성 설정을 암호화 및 해독합니다.
...
관리 에이전트 설계자의 기능 탭은 커넥터의 동작 및 기능을 정의합니다. 커넥터를 만들면 이 탭에서 선택한 내용을 수정할 수 없습니다. 다음 테이블에서 각 기능 설정을 나열합니다.
...
관리 에이전트 설계자에서 글로벌 매개 변수 탭을 사용하면 관리자가 연결 탭에 정의된 사용자 지정 구성 설정에 대한 전역 값 뿐만 아니라 커넥터에서 실행되는 각 Windows PowerShell 스크립트를 구성할 수 있습니다.

**파티션 검색**

파티션은 하나의 공유 스키마 내에서 별도의 네임스페이스입니다. 예를 들어 Active Directory에서 모든 도메인은 한 포리스트 내의 파티션입니다. 파티션은 작업 가져오기 및 내보내기에 대한 논리적 그룹화입니다. 가져오기 및 내보내기에는 파티션이 컨텍스트로 존재하고 모든 작업은 이 컨텍스트에서 발생해야 합니다. 파티션은 LDAP의 계층 구조를 나타내야 합니다. 파티션의 고유 이름은 파티션의 범위 내에서 반환된 개체가 모두 있는지 확인하기 위해 가져오기에 사용됩니다. 파티션 고유 이름은 메타버스에서 커넥터 공간까지 프로비전하는 동안 사용되어 내보내는 동안 연결되어야 하는 파티션 개체를 결정합니다.
...
계층 구조 검색 스크립트는 고유 이름 스타일 기능이 LDAP인 경우에 사용됩니다. 관리자가 가져오기 또는 내보내기에 대한 범위 안팎에 있는 컨테이너의 집합을 찾고 선택하는 데 스크립트를 사용할 수 있습니다. 스크립트는 스크립트에 제공된 루트 노드의 직접 하위 항목인 노드의 목록을 제공해야 합니다.
...
가져오기 시작 스크립트는 가져오기 실행 단계의 시작 부분에서 실행됩니다. 이 단계에서는 원본 시스템에 대한 연결을 설정할 수 있으며 연결된 시스템에서 데이터를 가져오기 전에 모든 준비 단계를 수행할 수 있습니다.
...
가져오기 실행을 끝낼 때 최종 가져오기 스크립트가 실행됩니다. 이 스크립트는 필요한 정리 작업을 수행합니다.(예: 시스템에 대한 연결 닫기, 오류에 대한 응답 등)
...
내보내기 시작 스크립트는 내보내기 실행 단계의 시작 부분에서 실행됩니다. 이 단계에서는 원본 시스템에 대한 연결을 설정할 수 있으며 연결된 시스템에서 데이터를 내보내기 전에 모든 준비 단계를 수행할 수 있습니다.
...
동기화 서비스는 보류 중인 내보내기를 모두 처리하는 데 필요한 횟수 만큼 데이터 내보내기 스크립트를 호출합니다. 커넥터 공간에 커넥터의 페이지 크기, 참조 특성 또는 암호의 존재 보다 많은 보류 중인 내보내기가 있는지 여부에 따라 데이터 내보내기 스크립트는 동일한 개체에서 가능한 여러 번 호출될 수 있습니다.
...
아래 샘플 코드는 파이프라인에 PutExportEntriesResults 개체를 반환하는 방법을 보여줍니다. `Write-Output (New-Object Microsoft.MetadirectoryServices.PutExportEntriesResults)`

**최종 내보내기**

내보내기 실행을 끝낼 때 최종 내보내기 스크립트가 실행됩니다. 이 스크립트는 필요한 정리 작업을 수행합니다.(예: 시스템에 대한 연결 닫기, 오류에 대한 응답 등)
