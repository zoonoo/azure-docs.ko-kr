...
* 사용자가 해당 Azure AD 계정으로 StatusPage SSO(Single Sign-on)에 자동으로 로그온되도록 설정할 수 있습니다.
...
* Azure AD 구독
* StatusPage SSO(Single Sign-on)가 설정된 구독
...
5. **수행할 작업** 대화 상자에서 **갤러리에서 응용 프로그램 추가**를 클릭합니다.
    ...
    ![응용 프로그램][4]
6. 검색 상자에 **StatusPage**를 입력합니다.
   
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_01.png)
7. 결과 창에서 **StatusPage**를 선택하고 **완료**를 클릭하여 응용 프로그램을 추가합니다.
   
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_02.png)

## <a name="configure-and-test-azure-ad-sso"></a>Azure AD SSO 구성 및 테스트
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 StatusPage에서 Azure AD SSO를 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 StatusPage 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 StatusPage의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 StatusPage의 **Username** 값으로 할당하여 설정합니다.
...
이 섹션은 Azure 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 StatusPage 응용 프로그램에서 Single Sign-On을 구성하는 방법을 설명하기 위한 것입니다. 

**StatusPage에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**

1. Azure 클래식 포털의 **StatusPage** 응용 프로그램 통합 페이지에서 **Single Sign-On 구성**을 클릭하여 **Single Sign-On 구성** 대화 상자를 엽니다.
   
    ![Single Sign-On 구성][6] 
2. **StatusPage에 대한 사용자 로그온 방법 선택 페이지**에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_03.png) 
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
   ...
   > 
  1. 메타데이터에서 발급자 값을 복사한 다음 **식별자** 텍스트 상자에 붙여넣습니다.
  2. 메타데이터에서 회신 URL을 복사한 다음 **회신 URL** 텍스트 상자에 붙여넣습니다.
  3. **다음**을 클릭합니다.

4. **StatusPage에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_05.png)   
  1. **인증서 다운로드**를 클릭하고 파일을 컴퓨터에 저장합니다.
  2. **다음**을 클릭합니다.
5. 다른 웹 브라우저 창에서 StatusPage 회사 사이트에 관리자로 로그인합니다.
6. 주 도구 모음에서 **계정 관리**를 클릭합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_06.png) 
7. **Single Sign-On** 탭을 클릭합니다. 
   
    ![Single Sign-On 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_07.png) 
8. SSO 설정 페이지에서 다음 단계를 수행합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_08.png)  
  1. Azure 클래식 포털의 **StatusPage에서 Single Sign-On 구성** 대화 상자 페이지에서 **Single Sign-On 서비스 URL** 값을 복사한 다음 **SSO 테넌트 URL** 텍스트 상자에 붙여넣습니다. 
  2. 다운로드된 인증서를 메모장에서 열고, 내용을 복사한 다음 전체 인증서를 **인증서** 텍스트 상자에 붙여넣습니다. 
  3. **Save**를 클릭합니다.
9. Azure 클래식 포털에서 Single Sign-On 구성 확인을 선택하고 **다음**을 클릭합니다. 
...
1. **Azure 클래식 포털**의 왼쪽 탐색 창에서 **Active Directory**를 클릭합니다.
    ...
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/create_aaduser_09.png)  
2. **디렉터리** 목록에서 디렉터리 통합을 사용하도록 설정할 디렉터리를 선택합니다.
...
5. **이 사용자에 대한 정보 입력** 대화 상자 페이지에서 다음 단계를 수행합니다. 
    ...
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/create_aaduser_05.png) 
  1. 사용자 유형에서 조직의 새 사용자를 선택합니다.  
  2. 사용자 이름 **텍스트 상자**에 **BrittaSimon**을 입력합니다.
  3. **다음**을 클릭합니다.
6. **사용자 프로필** 대화 상자 페이지에서 다음 단계를 수행합니다. 
   
   ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/create_aaduser_06.png)  
...
7. **임시 암호 가져오기** 대화 상자 페이지에서 **만들기**를 클릭합니다.
    ...
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/create_aaduser_07.png) 
8. **임시 암호 가져오기** 대화 상자 페이지에서 다음 단계를 수행합니다.
   
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-statuspage-tutorial/create_aaduser_08.png) 
  1. **새 암호** 값을 적어둡니다.  
  2. 페이지 맨 아래에 있는 **완료**을 참조하세요.   

### <a name="create-a-statuspage-test-user"></a>StatusPage 테스트 사용자 만들기
이 섹션은 StatusPage에서 Britta Simon이라는 사용자를 만들기 위한 것입니다.

StatusPage는 적시에 프로비전을 지원합니다. 이미 [Azure AD Single Sign-On 구성](#configuring-azure-ad-single-single-sign-on)에서 사용하도록 설정했습니다.
...
이 섹션은 Britta Simon에게 StatusPage에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
...
2. 응용 프로그램 목록에서 **StatusPage**를 선택합니다.
    ...
    ![Single Sign-On 구성](./media/active-directory-saas-statuspage-tutorial/tutorial_statuspage_50.png) 
