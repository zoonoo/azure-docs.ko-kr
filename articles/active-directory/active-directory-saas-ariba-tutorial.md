...
* 사용자가 해당 Azure AD 계정으로 Ariba SSO(Single Sign-on)에 자동으로 로그온되도록 설정할 수 있습니다.
...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Ariba에서 Azure AD SSO를 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Ariba 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 Ariba의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Ariba의 **Username** 값으로 할당하여 설정합니다.

Ariba에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션은 Azure 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 Ariba 응용 프로그램에서 SSO를 구성하는 방법을 설명하기 위한 것입니다.

**Ariba에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
  ...
  1. 로그인 URL 텍스트 상자에 **“https://<companyname>.sourcing.ariba.com"** 또는 **"https://<CompanyName>.supplier.ariba.com”**의 패턴을 사용하여 사용자가 Ariba 응용 프로그램에 로그인하는 데 사용할 URL을 입력합니다.
  2. **다음**을 클릭합니다.

4. **Ariba의 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
...
### <a name="create-a-ariba-test-user"></a>Ariba 테스트 사용자 만들기
