---
title: 'Azure AD 연결: Office 365 리소스에 대 한 기본 설정 데이터 위치 구성'
...
> [!IMPORTANT]
> Multi-Geo는 현재 활성 엔터프라이즈 계약및 최소 500개의 Office 365 서비스 구독을 보유한 고객에게 제공됩니다. 자세한 내용은 Microsoft 담당자에게 문의하세요.
>
>

Office 365의 모든 지오 목록은 데이터가 [있는 위치에서](https://aka.ms/datamaps)찾을 수 있습니다.
...
기본적으로 **preferredDataLocation**은 동기화가 허용되지 않습니다. 이 기능은 대규모 조직을 대상으로 합니다. Windows Server 2019의 Active Directory 스키마에는 **msDS-preferredData** 특성이 있습니다. Active Directory 스키마를 업데이트하지 않았고 그렇게 할 수 없는 경우 사용자에 대한 Office 365 지리적을 보유하는 특성을 식별해야 합니다. 이러한 특성은 각 조직마다 다를 수 있습니다.
...
* Active Directory 스키마를 2019로 업그레이드하지 않은 경우 소스 특성으로 사용할 온-프레미스 Active Directory 특성을 결정합니다. **단일 값 문자열** 형식이어야 합니다.
...
Active Directory 스키마를 2019로 업데이트하고 스키마 확장 전에 Connect가 설치된 경우 Connect 스키마 캐시에 업데이트된 스키마가 없습니다. 그런 다음 마법사에서 스키마를 새로 고쳐 UI에 표시해야 합니다.

1. 데스크톱에서 Azure AD Connect 마법사를 시작합니다.
2. **디렉터리 스키마 새로 고침** 옵션을 선택하고 다음 을 **클릭합니다.**
3. Azure AD 자격 증명을 입력하고 **다음**을 클릭합니다.
4. **디렉터리 스키마 새로 고침** 페이지에서 모든 포리스트를 선택하고 **다음을**클릭합니다.
5. 완료되면 마법사를 닫습니다.

![연결 마법사에서 디렉토리 스키마 새로 고침 화면](./media/how-to-connect-sync-feature-preferreddatalocation/preferreddatalocation-refreshschema.png)

## <a name="step-3-add-the-source-attribute-to-the-on-premises-active-directory-connector-schema"></a>3단계: 온-프레미스 Active Directory 커넥터 스키마에 소스 특성 추가
**이 단계는 Connect 버전 1.3.21 이상을 실행하는 경우에만 필요합니다. 1.4.18 이상인 경우 5단계로 건너뜁니다.**  
모든 Azure AD 특성이 온-프레미스 Active Directory 커넥터 공간으로 가져오는 것은 아닙니다. 기본적으로 동기화되지 않은 특성을 사용하도록 선택한 경우 해당 특성을 가져와야 합니다. 가져온 특성 목록에 원본 특성을 추가하려면 다음을 수행합니다.
...
## <a name="step-4-add-preferreddatalocation-to-the-azure-ad-connector-schema"></a>4단계: Azure AD 커넥터 스키마에 **기본 설정데이터 위치** 추가
**이 단계는 Connect 버전 1.3.21 이상을 실행하는 경우에만 필요합니다. 1.4.18 이상인 경우 5단계로 건너뜁니다.**  
기본적으로 기본 **설정DataLocation** 특성은 Azure AD 커넥터 공간으로 가져오지 않습니다. 가져온 속성 목록에 추가하려면 다음을 수행합니다.

1. Synchronization Service Manager에 있는 **커넥터** 탭을 선택합니다.
2. Azure AD 커넥터를 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다.
3. 팝업 대화 상자에서 **특성 선택** 탭으로 이동합니다.
...
5. 모든 개체를 포함하도록 **범위 지정 필터를** 비워 두십시오. Azure AD Connect 배포에 따라 범위 지정 필터를 조정해야 할 수도 있습니다.
...
아웃바운드 동기화 규칙은 특성 값이 메타버스에서 Azure AD의 **preferred데이터 위치** 특성으로 흐를 수 있도록 허용합니다.
...
5. **범위 지정 필터** 탭으로 이동하여 다음 두 절을 사용하여 단일 범위 지정 필터 그룹을 추가합니다.
    ...
    범위 지정 필터는 이 아웃바운드 동기화 규칙이 적용되는 Azure AD 개체를 결정합니다. 이 예제에서는 "기본에서 Azure AD로 – 사용자 ID" OOB(기본) 동기화 규칙에서 동일한 범위 지정 필터를 사용합니다. 온-프레미스 Active Directory에서 동기화되지 않은 **사용자** 개체에 동기화 규칙이 적용되지 않도록 합니다. Azure AD Connect 배포에 따라 범위 지정 필터를 조정해야 할 수도 있습니다.
...
3. 기존 **사용자** 개체에서 동기화 규칙 변경 내용을 확인합니다.
...
5. Azure AD로의 **보류 중인 내보내기**를 확인합니다.
   ...
   1. **Azure AD 커넥터를**마우스 오른쪽 단추로 클릭하고 **커넥터 공간 검색을**선택합니다.
   2. **커넥터 공간 검색** 대화 상자에서:
...
## <a name="step-8-re-enable-sync-scheduler"></a>8단계: 동기화 스케줄러 다시 사용
