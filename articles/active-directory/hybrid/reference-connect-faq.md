...
**Q: Azure AD Connect 서버를 강화하여 보안 공격 표면을 줄이려면 어떻게 해야 합니까?**

Azure AD Connect 서버를 강화하여 IT 환경의 이 중요한 구성 요소에 대한 보안 공격 표면을 줄이는 것이 좋습니다.  아래 권장 사항을 따르면 조직의 보안 위험이 줄어듭니다.

* 도메인 에 가입된 서버에 Azure AD Connect 배포 및 도메인 관리자 또는 기타 엄격하게 제어되는 보안 그룹에 대한 관리 액세스를 제한합니다.
...
**Q: Azure AD Connect가 두 도메인에서 Azure AD로동기화를 지원합니까?**  
예, 이 시나리오는 지원됩니다. 여러 [도메인을](how-to-connect-install-multiple-domains.md)참조하십시오.
 
**Q: Azure AD Connect에서 동일한 Active Directory 도메인에 대해 여러 커넥터를 사용할 수 있습니까?**  
아니요, 동일한 AD 도메인에 여러 커넥터를 사용할 수 없습니다. 

**Q: Azure AD Connect 데이터베이스를 로컬 데이터베이스에서 원격 SQL Server 인스턴스로 이동할 수 있습니까?**   
예, 이 작업을 수행하는 방법에 대한 일반적인 지침은 다음과 같습니다. 현재 문서를 더 자세히 준비하고 있습니다.
1. LocalDB ADSync 데이터베이스를 백업합니다.
이 작업을 수행하는 가장 간단한 방법은 Azure AD Connect와 동일한 컴퓨터에 설치된 SQL Server Management Studio를 사용하는 것입니다. *(LocalDb)에 연결합니다.\ADSync*, 그런 다음 ADSync 데이터베이스를 백업합니다.
...
3. 기존 [원격 SQL 데이터베이스](how-to-connect-install-existing-database.md)에 대해 Azure AD Connect를 설치합니다.
   문서에서는 로컬 SQL 데이터베이스를 사용하여 마이그레이션하는 방법을 보여 줍니다. 원격 SQL 데이터베이스를 사용하여 마이그레이션하는 경우, 이 프로세스의 5단계에서 Windows 동기화 서비스가 실행될 기존 서비스 계정도 입력해야 합니다. 다음은 이 동기화 엔진 서비스 계정에 대한 설명입니다.
      ...
      **기존 서비스 계정 사용**: 기본적으로 Azure AD Connect는 가상 서비스 계정을 사용하여 동기화 서비스를 사용합니다. 원격 SQL Server 인스턴스를 사용하거나 인증이 필요한 프록시를 사용하는 경우, 도메인의 관리 서비스 계정 또는 서비스 계정을 사용하고 암호를 알고 있어야 합니다. 이러한 경우에 사용할 계정을 입력합니다. 설치를 실행하는 사용자가 서비스 계정에 대한 로그인 자격 증명을 만들 수 있도록 SQL의 시스템 관리자인지 확인합니다. 자세한 내용은 [Azure AD Connect 계정 및 사용 권한을](reference-connect-accounts-permissions.md#adsync-service-account)참조하십시오. 
...
**Q: 이 분야의 모범 사례는 무엇입니까?**  

다음은 엔지니어링, 지원 및 컨설턴트가 수년에 걸쳐 개발한 몇 가지 모범 사례를 제시하는 정보 문서입니다.  이 글은 빠르게 참조할 수 있는 글머리 기호 목록에 표시됩니다.  이 목록은 포괄적이려고 시도하지만 아직 목록에 포함되지 않은 추가 모범 사례가 있을 수 있습니다.

- 전체 SQL을 사용하는 경우 로컬 과 원격 상태로 유지되어야 합니다.
    - 홉 감소
    - 문제 해결이 용이
    - 복잡성 감소
    - SQL에 리소스를 지정하고 Azure AD Connect 및 OS에 대한 오버헤드를 허용해야 합니다.
- 가능한 경우 프록시를 우회할 수 없는 경우 시간 초과 값이 5분보다 큰지 확인해야 합니다.
- 프록시가 필요한 경우 machine.config 파일에 프록시를 추가해야 합니다.
- 로컬 SQL 작업 및 유지 관리 및 Azure AD Connect에 미치는 영향( 특히 다시 인덱싱)
- DNS가 외부에서 해결할 수 있는지 확인
- 실제 [서버를](how-to-connect-install-prerequisites.md#hardware-requirements-for-azure-ad-connect) 사용하든 가상 서버를 사용하든 권장 사항에 따라 서버 사양이 권장되는지 확인합니다.
- 가상 서버를 사용하는 경우 필요한 리소스가 전용인지 확인합니다.
- 디스크 및 디스크 구성이 SQL Server의 모범 사례를 충족하는지 확인합니다.
- 모니터링을 위한 Azure AD 연결 상태 설치 및 구성
- Azure AD Connect에 기본 제공된 삭제 임계값을 사용합니다.
- 릴리스 업데이트를 신중하게 검토하여 추가될 수 있는 모든 변경 내용과 새 특성에 대비할 수 있습니다.
- 모든 백업
    - 백업 키
    - 백업 동기화 규칙
    - 백업 서버 구성
    - SQL Database 백업
- SQL VSS Writer 없이 SQL을 백업하는 제3자 백업 에이전트가 없는지 확인합니다(제3자 스냅숏이 있는 가상 서버에서 일반적임)
- 복잡성을 추가할 때 사용되는 사용자 지정 동기화 규칙의 양 제한
- Azure AD 연결 서버를 계층 0 서버로 처리
- 영향과 올바른 비즈니스 동인을 잘 이해하지 못하고 클라우드 동기화 규칙을 수정할 수 있습니다.
- Azure AD Connect 및 Azure AD 연결 상태를 지원하기 위해 올바른 URL 및 방화벽 포트가 열려 있는지 확인합니다.
- 클라우드 필터링 특성을 활용하여 가상 개체 문제 해결 및 방지
- 스테이징 서버를 사용하면 서버 간의 일관성을 위해 Azure AD Connect 구성 문서 작성기를 사용하고 있는지 확인합니다.
- 스테이징 서버는 별도의 데이터 센터에 있어야 합니다(물리적 위치
- 스테이징 서버는 고가용성 솔루션이 아니지만 여러 스테이징 서버를 가질 수 있습니다.
- "지연" 스테이징 서버를 도입하면 오류 발생 시 일부 잠재적인 가동 중지 시간을 줄일 수 있습니다.
- 스테이징 서버의 모든 업그레이드를 먼저 테스트하고 유효성 검사
- 스테이징 서버로 전환하기 전에 항상 내보내기의 유효성을 검사합니다.  전체 가져오기 및 전체 동기화를 위한 스테이징 서버를 활용하여 비즈니스 영향 감소
- Azure AD Connect 서버 간에 버전 일관성을 최대한 유지 

**Q: Azure AD Connect에서 작업 그룹 컴퓨터에서 Azure AD 커넥터 계정을 만들도록 허용할 수 있습니까?**
아니요.  Azure AD Connect가 Azure AD 커넥터 계정을 자동으로 만들 수 있도록 하려면 컴퓨터가 도메인에 가입되어 있어야 합니다.  

## <a name="network"></a>네트워크
**Q: 방화벽, 네트워크 장치 또는 네트워크에서 연결을 계속 유지할 수 있는 시간을 제한하는 다른 장치가 있습니다. Azure AD Connect를 사용할 때 클라이언트 측 시간 초과 임계값은 어떻게 해야 합니까?**  
...
**Q: I 다중 포리스트 환경을 가지며 두 포리스트 간의 네트워크가 NAT(네트워크 주소 변환)를 사용하고 있습니다. 이러한 두 포리스트 간에 Azure AD Connect를 사용하는 것이 지원되고 있습니까?**</br>
...
**Q: Office 365 의존 파티에 대해 "자동으로 연결 파티 업데이트"가 설정되어 있습니다. 토큰 서명 인증서가 자동으로 롤오버될 때 조치를 취해야 합니까?**  
...
**Q: 차세대 암호화(NGC) 동기화 규칙이 FIPS 지원 컴퓨터에서 지원되나요?**  
아니요.  지원되지 않습니다.

**Q. Azure 포털에서 동기화된 장치(예: HAADJ)를 사용하지 않도록 설정한 경우 다시 활성화되는 이유는 무엇입니까?**<br>
동기화된 장치는 온-프레미스에서 작성되거나 마스터될 수 있습니다. 동기화된 장치가 온-프레미스에서 활성화된 경우 관리자가 이전에 사용하지 않도록 설정한 경우에도 Azure Portal에서 동기화된 장치를 다시 사용할 수 있습니다. 동기화된 장치를 사용하지 않도록 설정하려면 온-프레미스 Active Directory를 사용하여 컴퓨터 계정을 사용하지 않도록 설정합니다.

**Q. 동기화된 사용자를 위해 Office 365 또는 Azure AD 포털에서 사용자 로그인을 차단하는 경우 다시 로그인할 때 차단해제된 이유는 무엇입니까?**<br>
동기화된 사용자는 온-프레미스에서 작성되거나 마스터될 수 있습니다. 계정이 온-프레미스에서 활성화되어 있는 경우 관리자가 배치한 로그인 차단을 해제할 수 있습니다.
...
**Q: Azure 로그인 페이지에는 이전에 성공적으로 로그인한 사용자의 사용자 이름이 표시됩니다. 이 동작을 해제할 수 있습니까?**  
현재 자동 완성 태그를 포함하여 **암호** 입력 필드의 HTML 특성을 수정하는 것은 지원되지 않습니다. 현재 **암호** 필드에 특성을 추가할 수 있는 사용자 지정 JavaScript를 허용하는 기능을 개발하고 있습니다.
...
**Q: 자동 업그레이드가 더 이상 작동하지 않으며 새 버전을 설치해야 한다는 이메일을 받았습니다. 왜 이렇게 해야 합니까?**  
...
**Q: 자동 업그레이드를 다시 활성화하기 위해 최신 버전으로 업그레이드하라는 이메일을 받았습니다. 버전 1.1.654.0을 사용하고 있습니다. 업그레이드해야 하나요?**  
...
**Q: 자동 업그레이드를 다시 활성화하기 위해 최신 버전으로 업그레이드하라는 이메일을 받았습니다. 자동 업그레이드를 활성화하기 위해 PowerShell을 사용한 경우에도 최신 버전을 설치해야 합니까?**  
예, 1.1.750.0 버전 이상으로 업그레이드해야 합니다. PowerShell을 사용하여 자동 업그레이드 서비스를 사용하도록 설정하더라도 1.1.750.0 이전 버전에서 발견된 자동 업그레이드 문제가 완화되지 않습니다.

**Q: 최신 버전으로 업그레이드하고 싶지만 Azure AD Connect를 설치한 사람이 누구인지 확실하지 않으며 사용자 이름과 암호가 없습니다. 우리는 이것입니까?**
...
서버에 설치된 Azure AD Connect 버전을 확인하려면 제어판으로 이동하여 다음과 같이 **프로그램** > **및 기능을**선택하여 Microsoft Azure AD Connect의 설치된 버전을 조회합니다.
...
**Q: 이미 작년에 Azure AD Connect의 최신 버전으로 업그레이드했습니다. 다시 업그레이드해야 합니까?**  
...
**Q: Azure AD Connect로 업그레이드했다고 생각하지만 Office 포털에서 DirSync를 계속 언급합니다. 왜 그럴까요?**  
Office 팀은 Office 포털 업데이트에서 현재 제품 이름을 반영하도록 노력하고 있습니다. 사용 중인 동기화 도구는 반영되지 않습니다.

**Q: 자동 업그레이드 상태는 "일시 중단됨"이라고 표시됩니다. 일시 중단되는 이유는 무엇입니까? 활성화해야 합니까?**  
...
**Q: 우리 회사는 엄격한 변경 관리 요구 사항을 가지고 있으며, 밀려난 시기를 제어하고 싶습니다. 자동 업그레이드가 시작되는 시기를 제어할 수 있습니까?**  
아니요, 현재 이러한 기능은 없습니다. 이 기능은 이후 릴리스에서 평가되고 있습니다.

**Q: 자동 업그레이드에 실패한 경우 이메일을 받게 됩니까? 성공했다는 것을 어떻게 알 수 있습니까?**  
...
**Q: Azure AD Connect의 최신 버전으로 업그레이드할 때 어떤 위험이 있는지 잘 모르겠습니다. 업그레이드를 도와주러 전화할 수 있습니까?**  
...
**Q: 동기화 단계 오류 이후에 이벤트 6311 및 6401이 발생하는 이유는 무엇입니까?**

이벤트 6311 - **서버가 콜백을 수행하는 동안 예기치 않은 오류가 발생하고** 6401 - **관리 에이전트 컨트롤러가 예기치 않은 오류가 발생한** 경우 동기화 단계 오류가 발생한 후 항상 기록됩니다. 이러한 오류를 해결하려면 동기화 단계 오류를 정리해야 합니다.  자세한 내용은 [Azure AD Connect 동기화를](tshoot-connect-objectsync.md) 통해 개체 동기화 및 문제 해결 [중 오류 해결을](tshoot-connect-sync-errors.md) 참조하세요.
