...
- 응용 프로그램 (예: `https://myapps.microsoft.com/contoso.com`)에서 테 넌 트를 식별 하는 `domain_hint` (Openid connect Connect) 또는 SAML (`whr`) 매개 변수를 전달 하거나, Azure AD 로그인 요청에서 사용자를 식별 하는 `login_hint` 매개 변수를 전달 하는 경우 사용자가 없는 상태에서 사용자가 자동으로 로그인 됩니다. 사용자 이름 또는 암호를 입력 합니다.
- 또한 응용 프로그램 (예: `https://contoso.sharepoint.com`)이 azure ad의 공통 끝점 (즉, `https://login.microsoftonline.com/common/<...>`) 대신 테 넌 @no__t @no__t 트로 설정 된 Azure AD의 끝점에 로그인 요청을 전송 하는 경우 자동 로그온 환경을 사용할 수 있습니다.
...
| --- | --- |--- | --- | --- | -- 
|Windows 10|예\*|예|예|예\*\*\*|해당 사항 없음
|Windows 8.1|예\*|해당 사항 없음|예|예\*\*\*|해당 사항 없음
|Windows 8|예\*|해당 사항 없음|예|예\*\*\*|해당 사항 없음
|Windows 7|예\*|해당 사항 없음|예|예\*\*\*|해당 사항 없음
|Windows Server 2012 R2 이상|예\*\*|해당 사항 없음|예|예\*\*\*|해당 사항 없음
|Mac OS X|해당 사항 없음|해당 사항 없음|예\*\*\*|예\*\*\*|예\*\*\*
