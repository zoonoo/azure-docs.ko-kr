...
- 응용 프로그램 ( `https://myapps.microsoft.com/contoso.com`예:)이 테 넌 트 `domain_hint` 를 식별 하는 ( `whr` openid connect Connect) 또는 (SAML) 매개 변수를 `login_hint` 전달 하거나 Azure AD 로그인 요청에서 사용자를 식별 하는 매개 변수를 전달 하면 사용자 이름 또는 암호를 입력 하지 않고 자동으로 로그인 됩니다.
- 응용 프로그램 (예 `https://contoso.sharepoint.com`:)이 azure ad의 공통 끝점 (즉, `https://login.microsoftonline.com/contoso.com/<..>` `https://login.microsoftonline.com/<tenant_ID>/<..>` `https://login.microsoftonline.com/common/<...>`) 대신 테 넌 트로 설정 된 azure ad의 끝점에 로그인 요청을 전송 하는 경우에도 자동 로그온 환경을 사용할 수 있습니다.
...
- [**기술 심층**](how-to-connect-sso-how-it-works.md) 분석-이 기능의 작동 방식을 이해 합니다.
