...
이전에는 하나 이상의 유효한 인증서를 포함 하는 모든 컴퓨터를 Azure AD에 동기화 하 고 1.4 버전부터 Azure AD Connect 시작 Azure AD Connect 하기 전에 동기화 엔진은 하이브리드 Azure ad 조인 인증서를 식별할 수 있으며, 올바른 하이브리드 Azure AD 조인 인증서가 없는 경우에는 컴퓨터 개체가 Azure AD로 동기화 되지 않도록 ' cloudfilter ' 됩니다.
