...
* `IsPresent([isCriticalSystemObject])`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 기본 제공 관리자 계정과 같은 Active Directory의 많은 기본 개체가 동기화되지 않도록 합니다.
* `IsPresent([sAMAccountName]) = False`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. sAMAccountName 특성이 없는 사용자 개체가 동기화되지 않도록 합니다. 이 경우는 실질적으로 NT4에서 업그레이드된 도메인에 발생합니다.
* `Left([sAMAccountName], 4) = "AAD_"`, `Left([sAMAccountName], 5) = "MSOL_"`에 적용되지 않습니다. Azure AD Connect 동기화 및 이전 버전에서 사용한 서비스 계정을 동기화하지 않습니다.
...
* Exchange Online에서 작동하지 않는 개체를 동기화하지 않습니다.
  ...
  * 사서함 계획
  * 검색 사서함
* `CBool(InStr(DNComponent(CRef([dn]),1),"\\0ACNF:")>0)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 복제 피해 개체를 동기화하지 않습니다.

다음 특성 규칙이 적용됩니다.

* `sourceAnchor <- IIF([msExchRecipientTypeDetails]=2,NULL,..)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. sourceAnchor 특성은 연결된 사서함에서 제공되지 않습니다. 실제 계정에서 연결된 사서함을 찾은 경우 실제 계정이 나중에 조인된다고 가정합니다.
...
* 연락처는 메일을 사용할 수 있어야 합니다. 다음 규칙을 통해 확인합니다.
  * `IsPresent([proxyAddresses]) = True)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. proxyAddresses 특성을 채워야 합니다.
  * 기본 전자 메일 주소는 proxyAddresses 특성 또는 메일 특성에서 찾을 수 있습니다. \@의 존재는 콘텐츠가 전자 메일 주소인지 확인하는 데 사용됩니다. 이 두 가지 규칙 중 하나가 True로 평가되어야 합니다.
    * `(Contains([proxyAddresses], "SMTP:") > 0) && (InStr(Item([proxyAddresses], Contains([proxyAddresses], "SMTP:")), "@") > 0))`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. "SMTP:"가 있는 항목이 있나요? 있다면 문자열에서 \@을 찾을 수 있나요?
    * `(IsPresent([mail]) = True && (InStr([mail], "@") > 0)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 메일 특성이 채워지나요? 그리고 채워진다면 문자열에서 \@을 찾을 수 있나요?

다음 연락처 개체는 Azure AD에 동기화되지 **않습니다** .

* `IsPresent([isCriticalSystemObject])`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 중요로 표시된 연락처 개체가 동기화되지 않도록 합니다. 기본 구성을 사용하지 않아야 합니다.
* `((InStr([displayName], "(MSOL)") > 0) && (CBool([msExchHideFromAddressLists])))`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다.
* `(Left([mailNickname], 4) = "CAS_" && (InStr([mailNickname], "}") > 0))`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 해당 개체는 Exchange Online에서 작동하지 않습니다.
* `CBool(InStr(DNComponent(CRef([dn]),1),"\\0ACNF:")>0)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 복제 피해 개체를 동기화하지 않습니다.
...
* `IsPresent([isCriticalSystemObject])`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 기본 제공 관리자 그룹과 같은 Active Directory의 많은 기본 개체가 동기화되지 않도록 합니다.
* `[sAMAccountName] = "MSOL_AD_Sync_RichCoexistence"`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. DirSync에서 사용하는 레거시 그룹입니다.
* `BitAnd([msExchRecipientTypeDetails],&amp;H40000000)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 역할 그룹입니다.
* `CBool(InStr(DNComponent(CRef([dn]),1),"\\0ACNF:")>0)`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. 복제 피해 개체를 동기화하지 않습니다.
...
* `userCertificate ISNOTNULL`에 대한 답변에 설명되어 있는 단계를 성공적으로 완료하면 활성화됩니다. Windows 10 컴퓨터만이 이 특성을 채웁니다. 이 특성의 값을 가진 컴퓨터 개체를 모두 동기화합니다.
...
SRE는 리소스 키트 도구 이며 Azure AD Connect 동기화와 함께 설치 됩니다. 이를 시작 하려면 ADSyncAdmins 그룹의 멤버 여야 합니다. 시작할 때 다음과 같이 표시됩니다.
...
이 동기화 규칙이 암호 동기화에 사용 되는 것을 확인할 수도 있습니다. 사용자가이 동기화 규칙에 대해 범위 내에 있는 경우 암호는 온-프레미스에서 클라우드로 동기화 됩니다 (암호 동기화 기능을 사용 하도록 설정 했다고 가정).
...
조인 규칙의 콘텐츠는 설치 마법사에서 선택한 일치 옵션에 따라 달라집니다. 인바운드 규칙의 경우, 평가는 원본 커넥터 공간에서 개체와 함께 시작되며 조인 규칙의 각 그룹이 순서대로 평가됩니다. 원본 개체가 조인 규칙 중 하나를 사용하는 메타버스로의 개체 하나와 정확히 일치하는 것으로 평가되는 경우, 개체는 조인됩니다. 모든 규칙이 평가되며 일치 항목이 없는 경우, 설명 페이지의 링크 형식이 사용됩니다. 이 구성이 **프로 비전**으로 설정 된 경우 조인 조건에 하나 이상의 특성이 있는 경우 (값이 있는 경우) 대상의 메타 버스에 새 개체가 만들어집니다. 메타버스에 새 개체를 프로비전하는 것은 개체를 메타버스에 **프로젝트** 하는 것이라고도 합니다.
