...
>[!NOTE] 
> 백 엔드 서비스에서 동기화 요청을 처리하는 데 문제가 있는 경우 이 서비스는 테넌트의 관리 연락처 전자 메일 주소(es)로 오류 세부 정보가 있는 알림 이메일을 보냅니다. 경우에 따라 이러한 메시지의 양이 엄청나게 커서 이러한 메시지를 보내는 방식을 바꾸고 있다는 고객의 피드백을 들었습니다. 
>
> 모든 동기화 오류에 대 한 메시지를 보낼 때마다 발생 하는 대신 백 엔드 서비스가 반환 하는 모든 오류의 일일 다이제스트를 보냅니다. 이를 통해 고객은 이러한 오류를 보다 효율적으로 처리하고 중복 오류 메시지 수를 줄일 수 있습니다.
>
> 이 변경 사항은 2020년 1월 15일에 시행될 예정입니다.
