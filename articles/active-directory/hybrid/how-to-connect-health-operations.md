...
1. Azure Portal에서 Azure AD Connect Health를 검색합니다.
2. **동기화 오류** 선택
3. **알림 설정** 을 선택합니다.
5. 메일 알림 스위치에서 **켜기** 를 선택합니다.
6. 모든 전역 관리자가 메일 알림을 수신하도록 하려는 경우 확인란을 선택합니다.
7. 다른 메일 주소로 메일 알림을 받으려는 경우 **추가 메일 받는 사람** 상자에 주소를 지정합니다. 이 목록에서 메일 주소를 제거하려면 항목을 마우스 오른쪽 단추로 클릭하고 **삭제** 를 선택합니다.
8. 변경을 완료하려면 **저장** 을 클릭합니다. 변경 내용은 저장한 후에만 적용됩니다.
