...
중복 특성 복원력은 Microsoft의 동기화 도구 중 하나를 실행할 때 **UserPrincipalName** 및 SMTP **프록시주소** 충돌로 인한 마찰을 제거하는 Azure Active Directory의 기능입니다.
...
특성 복원력 프로세스는 UPN 및 SMTP **프록시주소** 값만 처리합니다.

**프록시 주소**, Azure Active Directory와 같이 특성이 필요하지 않은 경우 충돌 특성을 격리하고 개체 만들기 또는 업데이트를 진행합니다.
...
현재 중복 속성 충돌로 인해 이러한 오류가 있는 개체를 식별하는 두 가지 방법인 Azure Active Directory PowerShell 및 [Microsoft 365 관리 센터가](https://admin.microsoft.com)있습니다. 향후 보고에 기반한 추가 포털로 확장할 계획이 있습니다.
...
1. [모두 보기](#see-all)
2. [속성 유형별](#by-property-type)
3. [값 충돌](#by-conflicting-value)
4. [문자열 검색 사용](#using-a-string-search)
5. 정렬
6. [제한된 수량 또는 전부](#in-a-limited-quantity-or-all)
...
1. **MaxResults \<int>** 사용하여 쿼리를 특정 값 수로 제한할 수 있습니다.
...
Microsoft 365 관리 센터에서 디렉터리 동기화 오류를 볼 수 있습니다. Microsoft 365 관리 센터의 보고서에는 이러한 오류가 있는 **사용자** 개체만 표시됩니다. **그룹** 및 **연락처** 간의 충돌에 대한 정보는 표시하지 않습니다.

![활성 사용자](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/1234.png "활성 사용자")

Microsoft 365 관리 센터에서 디렉터리 동기화 오류를 보는 방법에 대한 지침은 [Office 365의 디렉터리 동기화 오류 식별을](https://support.office.com/article/Identify-directory-synchronization-errors-in-Office-365-b4fc07a5-97ea-4ca6-9692-108acab74067)참조하십시오.
...
1. 특정 특성 구성이 있는 개체는 격리되는 중복 특성과 달리 내보내기 오류가 계속 수신됩니다.  
   예를 들어:
    ...
    a. 새 사용자는 **\@조 contoso.com** 및 프록시 주소 **smtp의\@** UPN으로 AD에서 만들어집니다:조 contoso.com
   
    b. 이 개체의 속성은 프록시 주소가 **\@SMTP:Joe contoso.com**기존 그룹과 충돌합니다.
...
2. UPN 충돌에 대한 자세한 오류 메시지는 해당 UPN을 변경하고 격리한 사용자에 대한 잘못된 displayName을 보여 줍니다. 예를 들어:
    ...
    a. **사용자 A는** **UPN = 사용자\@contoso.com.**
   
    b. **사용자 B는** **UPN =\@사용자 contoso.com**.
   
    다. **사용자 B의** UPN이 **\@User1234 contoso.onmicrosoft.com** 변경되고 **\@사용자 contoso.com** **DirSync프로비저닝 오류에**추가됩니다.
   
    d. **사용자 B에** 대한 오류 메시지는 **사용자 A가** 이미 **사용자\@contoso.com** UPN으로 가지고 있지만 **사용자 B의** 자체 표시 이름을 표시해야 합니다.
...
[https://aka.ms/duplicateattributeresiliency](https://aka.ms/duplicateattributeresiliency)을 가리킨다.
