...
현재 중복 된 속성 충돌, Azure Active Directory PowerShell 및 [Microsoft 365 관리 센터](https://admin.microsoft.com)때문에 이러한 오류가 발생 한 개체를 식별 하는 두 가지 방법이 있습니다. 향후 보고에 기반한 추가 포털로 확장할 계획이 있습니다.
...
1. **MaxResults \<Int >** 를 사용 하 여 쿼리를 특정 값 수로 제한할 수 있습니다.
...
Microsoft 365 관리 센터에서 디렉터리 동기화 오류를 확인 하는 방법에 대 한 지침은 [Office 365에서 디렉터리 동기화 오류 확인](https://support.office.com/article/Identify-directory-synchronization-errors-in-Office-365-b4fc07a5-97ea-4ca6-9692-108acab74067)을 참조 하세요.
...
2. UPN 충돌에 대한 자세한 오류 메시지는 해당 UPN을 변경하고 격리한 사용자에 대한 잘못된 displayName을 보여 줍니다. 다음은 그 예입니다.
    ...
    a. **사용자 A는** **UPN = user\@contoso.com**를 사용 하 여 먼저 동기화 됩니다.
   
    b. **사용자 B** 는 **UPN = User\@contoso.com**다음에 동기화 하려고 시도 합니다.
   
    다. **사용자 B** UPN은 **User1234\@contoso.onmicrosoft.com** 로 변경 되 고 **사용자\@contoso.com** 는 **DirSyncProvisioningErrors**에 추가 됩니다.
   
    d. 사용자 **b** 에 대 한 오류 메시지는 사용자 **A** 가 이미 UPN으로 **사용자\@Contoso.com** 있음을 표시 하지만 **사용자 b의** 고유한 displayName을 표시 합니다.
