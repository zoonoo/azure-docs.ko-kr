...
중복 특성 복원력은 Microsoft의 동기화 도구 중 하나를 실행하는 경우 **UserPrincipalName** 및 SMTP **ProxyAddress** 의 충돌로 발생하는 마찰을 제거하는 Azure Active Directory의 기능입니다.
...
중복 속성 충돌로 인해 이러한 오류가 있는 개체를 식별하는 방법은 현재 PowerShell Azure Active Directory 및 [Microsoft 365 관리 센터](https://admin.microsoft.com)의 두 가지가 있습니다. 향후 보고에 기반한 추가 포털로 확장할 계획이 있습니다.
...
Microsoft 365 관리 센터에서 디렉터리 동기화 오류를 볼 수 있습니다. Microsoft 365 관리 센터의 보고서는 이러한 오류가 있는 **사용자** 개체만을 표시합니다. **그룹** 및 **연락처** 간의 충돌에 대한 정보는 표시하지 않습니다.

![Microsoft 365 관리 센터의 디렉터리 동기화 오류를 보여 주는 스크린샷](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/1234.png "활성 사용자")

Microsoft 365 관리 센터에서 디렉터리 동기화 오류를 보는 방법에 대한 지침은 [365에서 디렉터리 동기화 오류 확인](https://support.office.com/article/Identify-directory-synchronization-errors-in-Office-365-b4fc07a5-97ea-4ca6-9692-108acab74067)을 참조하세요.
...
ProxyAddress 충돌에 대한 메일 알림의 예제는 다음과 같습니다.  
    ![ProxyAddress 충돌에 대한 메일 알림 예제를 보여 주는 스크린샷](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/6.png "활성 사용자")  
...
1. 특정 특성 구성이 있는 개체는 격리되는 중복 특성과 달리 내보내기 오류가 계속 수신됩니다.  
   예를 들면 다음과 같습니다.
    ...
    a. **Joe\@contoso.com** 및 ProxyAddress **smtp:Joe\@contoso.com** 의 UPN을 사용하여 AD에서 새 사용자가 만들어집니다.
...
2. UPN 충돌에 대한 자세한 오류 메시지는 해당 UPN을 변경하고 격리한 사용자에 대한 잘못된 displayName을 보여 줍니다. 예를 들면 다음과 같습니다.
    ...
    a. **사용자 A** 는 먼저 **UPN = User\@contoso.com** 으로 동기화됩니다.
   
    b. **사용자 B** 는 다음에 **UPN = User\@contoso.com** 으로 동기화하려고 시도합니다.
   
    다. **사용자 B** 의 UPN은 **User1234\@contoso.onmicrosoft.com** 으로 변경되고 **User\@contoso.com** 이 **DirSyncProvisioningErrors** 에 추가됩니다.
...
[https://aka.ms/duplicateattributeresiliency]()를 가리켜야 합니다.
