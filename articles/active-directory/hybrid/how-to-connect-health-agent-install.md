...
| 아웃바운드 트래픽에 대한 TLS 검사가 필터링되거나 비활성화됨 | 네트워크 계층에서 아웃바운드 트래픽에 대한 TLS 검사 또는 종료가 있는 경우 에이전트 등록 단계 또는 데이터 업로드 작업이 실패할 수 있습니다. [TLS 검사 를 설정하는 방법에](https://technet.microsoft.com/library/ee796230.aspx) 대해 자세히 알아보기 |
...
* AD FS용 Azure AD Connect Health 사용 시작
    ...
    * [설치 지침을 참조하십시오.](#installing-the-azure-ad-connect-health-agent-for-ad-fs)
...
* AD DS용 Azure AD Connect Health 사용 시작
    ...
    * [설치 지침을 참조하십시오.](#installing-the-azure-ad-connect-health-agent-for-ad-ds)
...
![AD FS 감사 로그](./media/how-to-connect-health-agent-install/adfsaudit.png)

> [!WARNING]
> 그룹 정책은 AD FS 감사를 사용하지 않도록 설정할 수 있습니다. AD FS 감사를 사용하지 않도록 설정하면 로그인 활동에 대한 사용량 현황 분석을 사용할 수 없습니다. AD FS 감사를 사용하지 않도록 설정하는 그룹 정책이 없는지 확인해야 합니다.
>
...
![동기화에 대한 Azure AD Connect Health 확인](./media/how-to-connect-health-agent-install/services.png)

> [!NOTE]
> Azure AD Connect Health를 사용하려면 Azure AD Premium이 필요합니다. Azure AD Premium이 없으면 Azure Portal에서 구성을 완료할 수 없습니다. 자세한 내용은 [요구 사항 페이지](how-to-connect-health-agent-install.md#requirements)를 참조하세요.
>
>
...
1. Azure AD에서 암호를 사용하여 사용자 계정을 만듭니다.
2. 포털을 통해 Azure AD Connect Health에서 이 로컬 AAD 계정에 대한 **소유자** 역할을 할당합니다. [여기에](how-to-connect-health-operations.md#manage-access-with-role-based-access-control)단계를 따르십시오. 모든 서비스 인스턴스에 역할을 할당합니다. 
3. 설치를 위해 로컬 도메인 컨트롤러에서 .exe MSI 파일을 다운로드합니다.
4. 등록할 다음 스크립트를 실행합니다. 매개 변수를 만든 새 사용자 계정과 해당 암호로 바꿉습니다. 
...
1. 완료되면 다음 중 하나 이상을 수행하여 로컬 계정에 대한 액세스를 제거할 수 있습니다. 
    * AAD 연결 건강에 대한 로컬 계정의 역할 할당 제거
    * 로컬 계정의 암호를 회전합니다. 
    * AAD 로컬 계정 사용 안 함
    * AAD 로컬 계정 삭제  
...
* 기본적으로 전역 관리자는 에이전트 등록을 수행할 권한이 있습니다. 기타 낮은 권한의 ID가 이 단계를 수행하도록 허용할 수도 있습니다. [역할 기반 Access Control](how-to-connect-health-operations.md#manage-access-with-role-based-access-control)에 대해 자세히 알아보세요.
...
> 다시 시작 서비스 AzureADConnectHealth*
>
>
...
> [!NOTE]
> 연결 도구를 사용하려면 먼저 에이전트 등록을 완료해야 합니다. 에이전트 등록을 완료할 수 없는 경우 Azure AD Connect Health에 대한 [요구 사항](how-to-connect-health-agent-install.md#requirements)을 모두 충족했는지 확인합니다. 이 연결 테스트는 에이전트를 등록하는 동안 기본적으로 수행됩니다.
>
>
