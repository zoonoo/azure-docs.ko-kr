...
>
>사용된 스크립트는 Azure AD Connect를 설치하기 전에 일반 Active Directory 환경을 만듭니다.  그러한 스크립트는 모든 자습서와 관련이 있습니다.
>
> 이 자습서에 사용된 PowerShell 스크립트의 복사본은 Github [페이지](https://github.com/billmath/tutorial-phs)에서 확인할 수 있습니다.

## <a name="create-a-virtual-machine"></a>가상 머신 만들기
하이브리드 ID 환경을 시작하고 실행하기 위해 첫 번째로 수행해야 하는 작업은 온-프레미스 Active Directory 서버로 사용할 가상 머신을 만드는 것입니다.  

>[!NOTE]
>호스트 컴퓨터의 PowerShell에서 스크립트를 실행한 적이 없다면 스크립트를 실행하기 전에 `Set-ExecutionPolicy remotesigned`를 실행하고 PowerShell에서 yes를 입력해야 합니다.
...
2. 시작 단추를 클릭합니다.
3.  ‘CD 또는 DVD에서 부팅하려면 아무 키나 누르세요’라는 메시지가 표시됩니다. 아무 키나 눌러서 계속 진행합니다.
4. Windows Server 시작 화면에서 언어를 선택하고 **다음**을 클릭합니다.
5. **지금 설치**를 클릭합니다.
6. 라이선스 키를 입력하고 **다음**을 클릭합니다.
7. **동의함 확인란을 선택하고 **다음**을 클릭합니다.
8. **사용자 지정: Windows만 설치(고급)** 를 선택합니다.
9. **다음**을 누릅니다
10. 설치가 완료되고 나면 가상 머신을 다시 시작하고 로그인한 후 Windows 업데이트를 실행하여 VM이 최신 버전인지 확인합니다.  최신 업데이트를 설치합니다.

## <a name="install-active-directory-pre-requisites"></a>Active Directory 필수 구성 요소 설치
가상 머신을 만들었으므로 이제 Active Directory를 설치하기 전에 몇 가지 작업을 수행해야 합니다.  즉, 가상 머신의 이름을 바꾸고 고정 IP 주소 및 DNS 정보를 설정해야 하며 원격 서버 관리 도구를 설치해야 합니다.   다음을 수행합니다.
...
VM을 만들고 이름을 바꿨으며 고정 IP 주소를 설정했으므로 계속해서 Active Directory Domain Services를 설치하고 구성할 수 있습니다.  다음을 수행합니다.
...
Active Directory 환경을 만들었으므로 테스트 계정이 필요합니다.  이 계정은 온-프레미스 AD 환경에 생성된 다음 Azure AD에 동기화됩니다.  다음을 수행합니다.
...
이제 클라우드로 사용자를 동기화할 수 있도록 Azure AD 테넌트를 만들어야 합니다.  새 Azure AD 테넌트를 만들려면 다음 단계를 수행합니다.
...
## <a name="create-a-global-administrator-in-azure-ad"></a>Azure AD에서 전역 관리자 만들기
이제 Azure AD 테넌트가 있으므로 전역 관리자 계정을 만들겠습니다.  이 계정은 Azure AD Connect를 설치하는 동안 Azure AD Connector 계정을 만드는 데 사용됩니다.  Azure AD Connector 계정은 Azure AD에 정보를 쓰는 데 사용됩니다.   전역 관리자 계정을 만들려면 다음을 수행합니다.

1.  **관리**에서 **사용자**를 선택합니다.</br>
![만들기](media/tutorial-password-hash-sync/gadmin1.png)</br>
2.  **모든 사용자**, **+ 새 사용자**를 차례로 선택합니다.
3.  이 사용자에 사용할 이름 및 사용자 이름을 입력합니다. 이 사용자가 테넌트의 전역 관리자입니다. **디렉터리 역할**도 **전역 관리자**로 변경해야 합니다. 임시 암호를 표시할 수도 있습니다. 작업이 완료되면 **만들기**를 선택합니다.</br>
![만들기](media/tutorial-password-hash-sync/gadmin2.png)</br>
4. 이 작업이 완료되면 새 웹 브라우저를 열고 새 전역 관리자 계정 및 임시 암호를 사용하여 myapps.microsoft.com에 로그인합니다.
5. 전역 관리자의 암호를 쉽게 기억할 수 있는 암호로 변경합니다.

## <a name="add-the-custom-domain-name-to-your-directory"></a>디렉터리에 사용자 지정 도메인 이름 추가
테넌트와 전역 관리자를 만들었으므로 Azure에서 확인할 수 있도록 사용자 지정 도메인을 추가해야 합니다.  다음을 수행합니다.
...
4. **사용자 지정 도메인 이름**의 상자에 사용자 지정 도메인 이름을 입력하고 **도메인 추가**를 클릭합니다.
5. 사용자 지정 도메인 이름 화면에는 TXT 또는 MX 정보가 제공됩니다.  도메인의 도메인 등록 기관 DNS 정보에 이 정보를 추가해합니야 다.  이렇게 하려면 사용 중인 도메인 등록 기관으로 이동하여 도메인의 DNS 설정에 TXT 또는 MX 정보를 입력합니다.  이렇게 하면 Azure에서 도메인을 확인할 수 있습니다.  Azure에서 도메인을 확인하려면 최대 24시간이 걸릴 수 있습니다.  자세한 내용은 [사용자 지정 도메인 추가](../../active-directory/fundamentals/add-custom-domain.md) 설명서를 참조하세요.</br>
![페더레이션](media/tutorial-federation/custom2.png)</br>
6. 도메인을 확인하려면 확인 단추를 클릭합니다.</br>
![페더레이션](media/tutorial-federation/custom3.png)</br>

## <a name="download-and-install-azure-ad-connect"></a>Azure AD Connect 다운로드 및 설치
이제 Azure AD Connect를 다운로드하여 설치합니다.  Azure AD Connect를 설치한 후에 빠른 설치를 통해 실행할 것입니다.  다음을 수행합니다.
...
1. Azure AD에 연결 화면에서 앞에서 만든 전역 관리자의 사용자 이름과 암호를 입력하고 **다음**을 클릭합니다.
...
7. 사용자 및 장치 필터링 화면에서 **다음**을 클릭합니다.
...
1. 웹 응용 프로그램 프록시 서버 화면에서 **다음**을 클릭합니다.
...
6. 설치가 완료된 후 Synchronization Service Manager 또는 Synchronization Rule Editor를 사용하기 전에 로그아웃했다가 다시 로그인합니다.


## <a name="verify-users-are-created-and-synchronization-is-occurring"></a>사용자가 생성되고 동기화가 수행되는지 확인
이제 온-프레미스 디렉터리의 사용자가 동기화되어 Azure AD 테넌트에 포함되어 있는지를 확인합니다.  이 작업을 완료하려면 몇 시간 정도 걸릴 수 있습니다.  사용자가 동기화되었는지 확인하려면 다음을 수행합니다.


1. [Azure Portal](https://portal.azure.com)로 이동하여 Azure 구독이 있는 계정으로 로그인합니다.
2. 왼쪽에서 **Azure Active Directory**를 선택합니다.
3. **관리**에서 **사용자**를 선택합니다.
4. 테넌트에 새 사용자가 표시되는지 확인합니다. ![동기화](media/tutorial-password-hash-sync/synch1.png)
...
이제 Azure가 제공하는 기능을 테스트하고 숙지하는 데 사용할 수 있는 하이브리드 ID 환경이 설정되었습니다.
