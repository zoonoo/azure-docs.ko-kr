...
* 편집 및 이동: 컴퓨터 구성 > 정책 > 관리 템플릿은 Windows 구성 요소 > 장치 등록을 >.

>[!NOTE]
>2012R2의 경우 정책 설정이 **컴퓨터 구성 > 정책 > 관리 템플릿 > Windows 구성 요소 > 작업 공간 연결 > 클라이언트 컴퓨터에 자동으로 작업 공간 연결**에 있습니다.

* 이 설정 사용: 도메인에 가입한 컴퓨터를 장치로 등록합니다.
* 적용하고 [확인]을 클릭합니다.
* GPO를 원하는 위치(조직 구성 단위, 보안 그룹 또는 모든 디바이스의 도메인)에 연결합니다.

## <a name="2-configure-network-with-device-registration-endpoints"></a>2. 장치 등록 끝점으로 네트워크 구성
...
## <a name="3-implement-wpad-for-windows-10-devices"></a>3. 윈도우에 대 한 WPAD 구현 10 장치
조직이 아웃바운드 프록시를 통해 인터넷에 액세스하는 경우 Windows 10 컴퓨터에서 Azure AD에 등록할 수 있게 WPAD(웹 프록시 자동 검색)를 구현합니다.

## <a name="4-configure-the-scp-in-any-forests-that-were-not-configured-by-azure-ad-connect"></a>4. Azure AD Connect에서 구성하지 않은 포리스트에서 SCP 구성 
...
## <a name="5-configure-any-federation-service-that-was-not-configured-by-azure-ad-connect"></a>5. Azure AD Connect에서 구성하지 않은 페더레이션 서비스 구성
...
## <a name="6-enable-azure-ad-seamless-sso-for-windows-down-level-devices"></a>6. Windows 하위 수준 장치에 대한 Azure AD 원활한 SSO 사용
...
## <a name="8-add-azure-ad-endpoint-to-windows-down-level-devices"></a>8. Windows 하위 수준 장치에 Azure AD 끝점을 추가합니다.
...
## <a name="9-install-microsoft-workplace-join-on-windows-down-level-devices"></a>9. 윈도우 다운 레벨 장치에 마이크로 소프트 워크 플레이스 가입 을 설치
...
## <a name="10-configure-group-policy-to-allow-device-registration"></a>10. 장치 등록을 허용하도록 그룹 정책 구성
