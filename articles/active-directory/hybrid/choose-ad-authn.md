...
3. 계정 만료, 사용 안 함 계정, 암호 만료, 계정 잠금 및 로그인 시간과 같은 사용자 수준 Active Directory 보안 정책을 적용 해야 하는 경우에는 Azure AD에 일부 온-프레미스 구성 요소가 필요 합니다.
...
5. 사용자에 게 *유출 된 자격 증명* 보고서를 제공 하기 위해 선택한 로그인 방법에 관계 없이 암호 해시 동기화가 Azure AD ID 보호 됩니다. 조직에서는 기본 로그인 메서드가 실패 하 고 실패 이벤트 이전에 구성 된 경우 암호 해시 동기화로 장애 조치 (failover) 할 수 있습니다.
...
배포 단계는 [통과 인증 구현](../../active-directory/hybrid/how-to-connect-pta.md) 을 참조 하세요.
...
* **고급 시나리오**. 페더레이션 인증 솔루션은 Azure AD에서 기본적으로 지원 하지 않는 인증 요구 사항이 있는 경우에 필요 합니다. 자세한 내용은 [올바른 로그인 옵션 선택](https://blogs.msdn.microsoft.com/samueld/2017/06/13/choosing-the-right-sign-in-option-to-connect-to-azure-ad-office-365/)을 참조하세요. 고려할 일반적인 요구 사항은 다음과 같습니다.
  ...
  * UPN (사용자 계정 이름) 대신에 sAMAccountName (예:)을 사용 하 여 로그인 user@domain.com합니다.
...
* 중복성을 위해 두 개의 에이전트를 사용 하는 통과 인증의 에이전트 요구 사항:
...
|TLS/SSL 인증서 요구 사항이 있나요?|아니요|예|예|
...
|조건부 액세스 옵션은 무엇 인가요?|[Azure AD 조건부 액세스, Azure AD Premium](../../active-directory/conditional-access/overview.md)|[Azure AD 조건부 액세스, Azure AD Premium](../../active-directory/conditional-access/overview.md)|[Azure AD 조건부 액세스, Azure AD Premium](../../active-directory/conditional-access/overview.md)<br><br>[AD FS 클레임 규칙](https://adfshelp.microsoft.com/AadTrustClaims/ClaimsGenerator)|
...
2. **온-프레미스 중단에서 생존**.  사이버 공격이나 재해로 인한 온-프레미스 중단은 브랜드 이미지 손상부터 해당 공격을 처리할 수 없을 정도의 조직 마비에 이르기까지 막대한 영향을 줄 수 있습니다. 최근 대부분의 조직에서는 온-프레미스 서버 작동이 중단 된 대상 랜 섬 웨어를 비롯 한 맬웨어 공격을 교착 상태가 발생 했습니다. 고객이 이러한 종류의 공격을 처리할 수 있도록 지원하면서 Microsoft는 두 가지 범주의 조직이 있음을 알았습니다.
   ...
   * 이전에 페더레이션 또는 통과 인증을 통해 암호 해시 동기화를 설정한 조직은 자신의 기본 인증 방법을 변경 하 여 암호 해시 동기화를 사용 합니다. 몇 시간 만에 다시 온라인 상태가 되었습니다. Office 365를 통한 이메일 액세스를 사용하여 문제를 해결하고 다른 클라우드 기반 워크로드에 액세스할 수 있었습니다.
