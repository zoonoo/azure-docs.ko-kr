---
title: Azure AD 하이브리드 ID 솔루션에 대한 인증
...
ID는 IT 보안의 새로운 제어 국이므로 인증은 새로운 클라우드 세계에 대한 조직의 액세스 가드입니다. 조직에서는 보안을 강화하고 클라우드 앱을 침입자들로부터 안전하게 보호하는 ID 제어 평면을 필요로 합니다.

> [!NOTE]
> 인증 방법을 변경하려면 계획, 테스트 및 가동 중지 시간이 필요합니다. [단계적 롤아웃은](https://docs.microsoft.com/azure/active-directory/hybrid/how-to-connect-staged-rollout) 페더레이션에서 클라우드 인증으로 테스트하고 점진적으로 마이그레이션할 수 있는 좋은 방법입니다.
...
**Azure AD 암호 해시 동기화**. Azure AD에서 온-프레미스 디렉터리 개체에 대한 인증을 사용하는 가장 간단한 방법입니다. 사용자는 추가 인프라를 배포하지 않고도 온-프레미스에서 사용하는 것과 동일한 사용자 이름과 암호를 사용할 수 있습니다. ID 보호 및 [Azure AD 도메인 서비스와](../../active-directory-domain-services/active-directory-ds-getting-started-password-sync.md)같은 Azure AD의 일부 프리미엄 기능은 선택한 인증 방법에 관계없이 암호 해시 동기화가 필요합니다.
...
5. Azure AD ID 보호에는 사용자에게 *유출된 자격 증명* 보고서를 제공하기 위해 선택한 로그인 방법에 관계없이 암호 해시 동기화가 필요합니다. 조직은 기본 로그인 메서드가 실패하고 실패 이벤트 전에 구성된 경우 암호 해시 동기화로 장애 조치할 수 있습니다.
...
* **고급 시나리오**. 조직에서 원하는 경우, Azure AD Premium P2가 있으면 손실된 자격 증명 보고서 등의 Azure AD ID 보호 보고서에서 ID의 정보를 사용할 수 있습니다. 비즈니스용 Windows Hello에는 [암호 해시 동기화를 사용할 때 특정 요구 사항이](https://docs.microsoft.com/windows/access-protection/hello-for-business/hello-identity-verification)있습니다. [Azure AD 도메인 서비스는](../../active-directory-domain-services/active-directory-ds-getting-started-password-sync.md) 관리되는 도메인에서 회사 자격 증명을 사용하여 사용자를 프로비전하기 위해 암호 해시 동기화가 필요합니다.

    암호 해시 동기화를 사용하여 다단계 인증이 필요한 조직은 Azure AD 다단계 인증 또는 [조건부 액세스 사용자 지정 컨트롤을](../../active-directory/conditional-access/controls.md#custom-controls-preview)사용해야 합니다. 이러한 조직은 페더레이션에 의존하는 타사 또는 온-프레미스 다단계 인증 방법을 사용할 수 없습니다.

> [!NOTE]
> Azure AD 조건부 액세스에는 [Azure AD 프리미엄 P1](https://azure.microsoft.com/pricing/details/active-directory/) 라이선스가 필요합니다.
...
> [!NOTE]
> 암호 만료 및 계정 잠김 상태는 현재, Azure AD Connect를 사용하여 Azure AD에 동기화되지 않습니다. 사용자의 암호를 변경하고 *다음 로그온 플래그에서 암호를 변경해야 하도록* 설정하면 사용자가 암호를 변경할 때까지 암호 해시가 Azure AD에 Azure AD에 동기화되지 않습니다.
...
* **고급 시나리오**. 통과 인증은 로그인 시 온-프레미스 계정 정책을 적용합니다. 예를 들어 온-프레미스 사용자의 계정 상태가 비활성화되거나 잠기거나 [암호가 만료되거나](../../active-directory/hybrid/how-to-connect-pta-faq.md#what-happens-if-my-users-password-has-expired-and-they-try-to-sign-in-by-using-pass-through-authentication) 로그인 시도가 사용자가 로그인할 수 있는 시간 외에 는 로그온 시도가 있을 때 액세스가 거부됩니다.

    통과 인증을 통해 다단계 인증이 필요한 조직은 MFA(Azure 다단계 인증) 또는 [조건부 액세스 사용자 지정 컨트롤을](../../active-directory/conditional-access/controls.md#custom-controls-preview)사용해야 합니다. 이러한 조직은 페더레이션에 의존하는 타사 또는 온-프레미스 다단계 인증 방법을 사용할 수 없습니다. ID 보호의 유출된 자격 증명 보고서 등과 같은 고급 기능을 사용하려면 통과 인증 선택 여부에 관계없이 암호 해시 동기화를 배포해야 합니다.
...
* **고려 사항**. 특정 온-프레미스 오류 때문에 에이전트에서 사용자 자격 증명의 유효성을 검사할 수 없는 경우 암호 해시 동기화를 통과 인증에 대한 백업 인증 방법으로 사용할 수 있습니다. 암호 해시 동기화에 장애 조치가 자동으로 발생하지 않으며 Azure AD Connect를 사용하여 로그온 메서드를 수동으로 전환해야 합니다.

    대체 ID 지원을 포함하여 통과 인증에 대한 기타 고려 사항은 [질문과 대답](../../active-directory/hybrid/how-to-connect-pta-faq.md)을 참조하세요.

배포 단계에 대한 [통과 인증 구현을](../../active-directory/hybrid/how-to-connect-pta.md) 참조하십시오.
...
* **고급 시나리오**. 페더레이션 인증 솔루션은 일반적으로 고객에게 Azure AD에서 기본적으로 지원되지 않는 인증 요구 사항이 있는 경우 필요합니다. 자세한 내용은 [올바른 로그인 옵션 선택](https://blogs.msdn.microsoft.com/samueld/2017/06/13/choosing-the-right-sign-in-option-to-connect-to-azure-ad-office-365/)을 참조하세요. 고려할 일반적인 요구 사항은 다음과 같습니다.
  ...
  * 온-프레미스 MFA 서버 또는 페더레이션된 ID 공급자가 필요한 타사 다단계 공급자입니다.
  * 타사 인증 솔루션을 사용하는 인증. [Azure AD 페더레이션 호환성 목록](../../active-directory/hybrid/how-to-connect-fed-compatibility.md)을 참조하세요.
  * 로그인하려면 예를 들어 사용자 주체 이름(UPN) 대신 도메인\사용자 이름과 같은 sAMAccountName이 user@domain.com필요합니다.
...
* 이중화를 위해 두 에이전트를 사용하는 통과 인증의 에이전트 요구 사항:
...
|프로비저닝 시스템(Azure AD Connect) 이외의 온-프레미스 서버 요구 사항은 무엇인가요?|None|각 추가 인증 에이전트마다 서버 1개|둘 이상의 AD FS 서버<br><br>경계/DMZ 네트워크에 둘 이상의 WAP 서버|
|프로비저닝 시스템 이외의 온-프레미스 인터넷 및 네트워킹 요구 사항은 무엇인가요?|None|인증 에이전트를 실행하는 서버의[아웃바운드 인터넷 액세스](../../active-directory/hybrid/how-to-connect-pta-quick-start.md)|경계에 있는 WAP 서버에 대한 [인바운드 인터넷 액세스](https://docs.microsoft.com/windows-server/identity/ad-fs/overview/ad-fs-requirements)<br><br>경계에 있는 WAP 서버에서 AD FS 서버로의 인바운드 네트워크 액세스<br><br>네트워크 부하 분산|
|TLS/SSL 인증서 요구 사항이 있습니까?|예|예|yes|
...
|지원되는 로그인 유형은 무엇인가요?|UserPrincipalName + 암호<br><br>[원활한 SSO를](../../active-directory/hybrid/how-to-connect-sso.md) 사용하여 Windows 통합 인증<br><br>[대체 로그인 ID](../../active-directory/hybrid/how-to-connect-install-custom.md)|UserPrincipalName + 암호<br><br>[원활한 SSO를](../../active-directory/hybrid/how-to-connect-sso.md) 사용하여 Windows 통합 인증<br><br>[대체 로그인 ID](../../active-directory/hybrid/how-to-connect-pta-faq.md)|UserPrincipalName + 암호<br><br>sAMAccountName + 암호<br><br>Windows 통합 인증<br><br>[인증서 및 스마트 카드 인증](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configure-user-certificate-authentication)<br><br>[대체 로그인 ID](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configuring-alternate-login-id)|
...
|다단계 인증 옵션은 무엇인가요?|[Azure MFA](https://docs.microsoft.com/azure/multi-factor-authentication/)<br><br>[조건부 액세스가 있는 사용자 지정 컨트롤*](../../active-directory/conditional-access/controls.md)|[Azure MFA](https://docs.microsoft.com/azure/multi-factor-authentication/)<br><br>[조건부 액세스가 있는 사용자 지정 컨트롤*](../../active-directory/conditional-access/controls.md)|[Azure MFA](https://docs.microsoft.com/azure/multi-factor-authentication/)<br><br>[Azure MFA 서버](../../active-directory/authentication/howto-mfaserver-deploy.md)<br><br>[타사 MFA](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/configure-additional-authentication-methods-for-ad-fs)<br><br>[조건부 액세스가 있는 사용자 지정 컨트롤*](../../active-directory/conditional-access/controls.md)|
...
|조건부 액세스 옵션은 무엇입니까?|[Azure AD 프리미엄을 사용 하 여 Azure AD 조건부 액세스](../../active-directory/conditional-access/overview.md)|[Azure AD 프리미엄을 사용 하 여 Azure AD 조건부 액세스](../../active-directory/conditional-access/overview.md)|[Azure AD 프리미엄을 사용 하 여 Azure AD 조건부 액세스](../../active-directory/conditional-access/overview.md)<br><br>[AD FS 클레임 규칙](https://adfshelp.microsoft.com/AadTrustClaims/ClaimsGenerator)|
|레거시 프로토콜 차단이 지원되나요?|[예](../../active-directory/conditional-access/overview.md)|[예](../../active-directory/conditional-access/overview.md)|[예](https://docs.microsoft.com/windows-server/identity/ad-fs/operations/access-control-policies-w2k12)|
...
2. **온-프레미스 중단에서 생존**.  사이버 공격이나 재해로 인한 온-프레미스 중단은 브랜드 이미지 손상부터 해당 공격을 처리할 수 없을 정도의 조직 마비에 이르기까지 막대한 영향을 줄 수 있습니다. 최근에는 많은 조직이 대상 랜섬웨어를 포함한 맬웨어 공격의 희생자로 인해 온-프레미스 서버가 다운되었습니다. 고객이 이러한 종류의 공격을 처리할 수 있도록 지원하면서 Microsoft는 두 가지 범주의 조직이 있음을 알았습니다.
   ...
   * 이전에 암호 해시 동기화를 사용하도록 설정하지 않은 조직은 통신을 위해 신뢰할 수 없는 외부 소비자 전자 메일 시스템에 의존하여 문제를 해결해야 했습니다. 이러한 경우 사용자가 클라우드 기반 앱에 다시 로그인하기 전에 온-프레미스 ID 인프라를 복원하는 데 몇 주가 걸렸습니다.

3. **ID 보호**. Azure AD Premium P2가 있는 Azure AD Identity Protection은 클라우드에서 사용자를 보호하는 가장 좋은 방법 중 하나입니다. Microsoft는 악의적인 행위자가 판매하고 다크 웹에서 사용할 수 있도록 만드는 사용자 및 암호 목록에 대해 인터넷을 지속적으로 검사합니다. Azure AD는 이 정보를 사용하여 사용자 조직의 사용자 이름 및 암호가 훼손되었는지 확인할 수 있습니다. 따라서 페더레이션 또는 통과 인증 여부에 관계없이 사용하는 인증 방법에 관계없이 암호 해시 동기화를 사용하도록 설정하는 것이 중요합니다. 유출된 자격 증명은 보고서로 제공되므로 사용자가 유출된 암호로 로그인하려고 할 때 이 정보를 사용하여 해당 사용자를 차단하거나 유출된 암호를 변경하도록 할 수 있습니다.
...
비즈니스 요구 사항이 솔루션을 배포하려는 노력과 로그인 프로세스에 대한 사용자의 경험이 비즈니스 요구 사항을 해결합니까? 또한 조직에 각 인증 방법의 고급 시나리오 및 비즈니스 연속성 기능이 필요한지 여부도 평가해야 합니다. 마지막으로, 각 인증 방법에서 고려할 사항을 평가하여 선택한 방법을 구현하지 못하도록 하는 요소가 있는지 확인해야 합니다.
...
페더레이션 인증에서 클라우드 인증으로 마이그레이션을 고려하는 경우 [로그인 방법 변경](../../active-directory/hybrid/plan-connect-user-signin.md)에 대해 자세히 알아보세요. 마이그레이션을 계획하고 구현하는 데 도움이 되면 [이러한 프로젝트 배포 계획을](https://aka.ms/deploymentplans) 사용하거나 새 [단계적 롤아웃](../../active-directory/hybrid/how-to-connect-staged-rollout.md) 기능을 사용하여 페더레이션된 사용자를 단계적 접근 방식으로 클라우드 인증을 사용하도록 마이그레이션하는 것이 좋습니다.
