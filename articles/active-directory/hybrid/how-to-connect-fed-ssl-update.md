---
title: Azure AD 연결 - AD FS 팜에 대한 TLS/SSL 인증서 업데이트 | 마이크로 소프트 문서
description: 이 문서에서는 Azure AD Connect를 사용하여 AD FS 팜의 TLS/SSL 인증서를 업데이트하는 단계를 자세히 설명합니다.
...
---
# <a name="update-the-tlsssl-certificate-for-an-active-directory-federation-services-ad-fs-farm"></a>AD FS(활성 디렉터리 페더레이션 서비스) 팜에 대한 TLS/SSL 인증서 업데이트

## <a name="overview"></a>개요
이 문서에서는 Azure AD Connect를 사용하여 AD FS(활성 디렉터리 페더레이션 서비스) 팜에 대한 TLS/SSL 인증서를 업데이트하는 방법에 대해 설명합니다. Azure AD Connect 도구를 사용하여 선택한 사용자 로그인 방법이 AD FS가 아닌 경우에도 AD FS 팜에 대한 TLS/SSL 인증서를 쉽게 업데이트할 수 있습니다.

모든 페더레이션 및 WAP(웹 응용 프로그램 프록시) 서버에서 AD FS 팜에 대한 TLS/SSL 인증서를 업데이트하는 전체 작업을 다음 세 단계로 수행할 수 있습니다.
...
![TLS 작업 업데이트](./media/how-to-connect-fed-ssl-update/updatessltask.png)
...
표시된 서버 목록은 서버를 추가하거나 제거하여 AD FS 팜의 현재 구성을 반영하도록 수정할 수 있습니다. 서버 정보가 제공되는 즉시 Azure AD Connect는 연결 및 현재 TLS/SSL 인증서 상태를 표시합니다.
...
AD FS 팜 서버에 대한 정보를 확인한 후 Azure AD Connect는 새 TLS/SSL 인증서를 요청합니다. 설치를 계속하려면 암호로 보호된 PFX 인증서를 제공합니다.
...
다음 단계에서는 TLS/SSL 인증서를 업데이트해야 하는 서버를 선택합니다. 오프라인 상태인 서버는 업데이트 대상으로 선택할 수 없습니다.
...
* **새 AD FS TLS/SSL 인증서에 대한 인증서의 주이름은 무엇입니까?**
...
* **새 TLS/SSL 인증서로 팜 서버의 하위 집합을 업데이트할 수 있습니까?**
...
* **이전 실행에서 서버를 제거했지만 여전히 오프라인으로 표시되고 AD FS 서버 페이지에 나열됩니다. 오프라인 서버를 제거한 후에도 오프라인 서버가 남아 있는 이유는 무엇입니까?**
