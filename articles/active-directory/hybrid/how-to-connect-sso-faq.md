...
**Q: 원활한 SSO는 [마이크로소프트 Azure 독일 클라우드와](https://www.microsoft.de/cloud-deutschland) [마이크로소프트 Azure 정부 클라우드에서](https://azure.microsoft.com/features/gov/)사용할 수 있습니까?**

아니요. Seamless SSO는 Azure AD의 전 세계 인스턴스에서만 사용할 수 있습니다.

**Q: 원활한 SSO의 `domain_hint` `login_hint` 어떤 응용 프로그램 또는 매개 변수 기능을 활용합니까?**
...
또한 응용 프로그램이 테넌터로 설정된 Azure AD의 끝점에 로그인 요청을 보내는 경우 자동 사인온 환경을\/받게 됩니다 <\/> tenant_ID login.microsoftonline.com/<login.microsoftonline.com/contoso.com/<..>. > - Azure AD의 일반적인 끝점 대신 -\/즉, https: /login.microsoftonline.com/common/<...>. 이러한 형식의 로그인을 요청하는 애플리케이션의 부분 목록이 아래에 나열됩니다.
...
**Q: 원활한 SSO가 대신 사용자 `Alternate ID` 이름으로 `userPrincipalName`지원합니까?**
...
**Q: [Azure AD 조인에서](../active-directory-azureadjoin-overview.md) 제공하는 단일 사인온 환경과 원활한 SSO의 차이점은 무엇입니까?**
...
**Q: AD FS를 사용하지 않고 Azure AD에 Windows 이외의 10 장치를 등록하려고 합니다. 대신 원활한 SSO를 사용할 수 있습니까?**

예, 이 시나리오에는 [작업 공간 연결 클라이언트](https://www.microsoft.com/download/details.aspx?id=53554) 버전 2.1 이상이 필요합니다.

**Q: 컴퓨터 계정의 Kerberos 암호 해독 키를 `AZUREADSSOACC` 롤오버하면 어떻게 해야 합니까?**
...
Azure AD Connect를 실행 중인 온-프레미스 서버에서 다음 단계를 따릅니다.
   ...
   **2단계. 각 AD 포리스트에서 Kerberos 암호 해독 키를 설정하여 업데이트합니다.**
...
**Q: 원활한 SSO를 비활성화하면 어떻게 해야 합니까?**

   **1단계. 테넌트에서 기능 사용 안 함**

   **옵션 A: Azure AD Connect를 사용하여 사용하지 않도록 설정**
   ...
   **옵션 B: PowerShell을 사용하여 사용하지 않도록 설정**
   ...
   **3단계. 나열된 각 `AZUREADSSOACCT` AD 포리스트에서 컴퓨터 계정을 수동으로 삭제합니다.**

## <a name="next-steps"></a>다음 단계

- [**빠른 시작**](how-to-connect-sso-quick-start.md) - Azure AD 원활한 SSO를 시작하고 실행합니다.
