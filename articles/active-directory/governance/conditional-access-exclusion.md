...
description: Azure Active Directory(Azure AD) 액세스 검토를 사용하여 조건부 액세스 정책에서 제외된 사용자를 관리하는 방법을 알아봅니다.
...
IT 관리자는 [Azure AD 조건부 액세스를](../conditional-access/overview.md) 사용하여 사용자가 MFA(다단계 인증)를 사용하여 인증하거나 신뢰할 수 있는 네트워크 또는 장치에서 로그인하도록 요구할 수 있습니다. 배포 계획 중에 일부 사용자가 이러한 일부 요구 사항을 충족할 수 없다는 점을 알게 되었습니다. 예를 들어, 내부 네트워크의 일부가 아닌 원격 사무실에서 작업하는 사용자가 있거나 지원되지 않는 이전 전화를 사용하는 경영진이 있습니다. 비즈니스에서는 이러한 사용자가 로그인하여 작업을 수행할 수 있도록 요구하므로 조건부 액세스 정책에서 제외됩니다.

또 다른 예로 조건부 Access에서 [명명된 위치를](../conditional-access/location-condition.md) 사용하여 사용자가 테넌트에 액세스하도록 허용하지 않으려는 카운티 및 지역 집합을 구성할 수 있습니다.

![조건부 액세스의 명명된 위치](./media/conditional-access-exclusion/named-locations.png)

그러나 경우에 따라 사용자가 이러한 차단된 국가/지역에서 로그인해야 하는 정당한 이유가 있을 수 있습니다. 예를 들어 사용자가 출장이나 개인적인 여행 중일 수도 있습니다. 이 예에서 이러한 국가/지역을 차단하는 조건부 액세스 정책에는 정책에서 제외된 사용자를 위한 전용 클라우드 보안 그룹이 있을 수 있습니다. 여행하는 동안 액세스해야 하는 사용자는 [Azure AD 셀프 서비스 그룹 관리](../users-groups-roles/groups-self-service-management.md)를 사용하여 그룹에 자신을 추가할 수 있습니다.

또 다른 예로 대부분의 사용자에 대한 레거시 인증을 차단하는 조건부 액세스 정책이 있을 [수 있습니다.](https://cloudblogs.microsoft.com/enterprisemobility/2018/06/07/azure-ad-conditional-access-support-for-blocking-legacy-auth-is-in-public-preview/) 보안 태세를 개선하기 위해 테넌트에서 레거시 프로토콜의 사용을 차단하는 것이 좋습니다. 그러나 일부 사용자가 반드시 레거시 인증 방법을 사용하여 Office 2010 또는 POP/IMAP/SMTP 기반 클라이언트를 통해 리소스에 액세스해야 하는 경우 이러한 사용자를 레거시 인증 방법을 차단하는 정책에서 제외할 수 있습니다.

## <a name="why-are-exclusions-challenging"></a>제외하기 어려운 이유는?

Azure AD에서 조건부 액세스 정책의 범위를 사용자 집합으로 범위를 정할 수 있습니다. Azure AD 역할, 개별 사용자 또는 사용자 게스트를 선택하여 이러한 사용자 중 일부를 제외할 수도 있습니다. 이러한 제외를 구성하는 경우 정책 의도는 해당 사용자에게 적용될 수 없다는 점에 유의하세요. 이러한 제외를 개별 사용자의 목록으로 구성하거나 레거시 온-프레미스 보안 그룹을 통해 구성한 경우 이 제외 목록의 가시성(사용자가 존재 여부를 모를 수 있음) 및 IT 관리자의 제어(사용자가 보안 그룹을 조인하여 정책을 무시할 수 있음)를 제한합니다. 또한 일회성 제외로 한정된 사용자가 더 이상 이 제한을 필요로 하지 않거나 적합하지 않을 수도 있습니다.
...
그러나 Azure AD 그룹을 사용하여 제외를 조건부 액세스 정책으로 구성하는 경우 액세스 검토를 보정 제어로 사용하여 가시성을 높이고 예외가 있는 사용자 수를 줄일 수 있습니다.

## <a name="how-to-create-an-exclusion-group-in-a-conditional-access-policy"></a>조건부 액세스 정책에서 제외 그룹을 만드는 방법

다음 단계에 따라 새 Azure AD 그룹 및 해당 그룹에 적용되지 않는 조건부 액세스 정책을 만듭니다.
...
1. 이 제외 그룹의 일부인 사용자를 선택한 다음, **만들기**를 클릭합니다.
    ...
    ![Azure 활성 디렉터리에서 새 그룹 창](./media/conditional-access-exclusion/new-group.png)
...
1. 왼쪽 탐색에서 **Azure Active Directory를** 클릭한 다음 **조건부 액세스를** 클릭하여 **정책** 블레이드를 엽니다.
...
조건부 액세스 정책에서 제외를 관리하기 위해 액세스 검토를 사용할 수 있는 두 가지 예를 살펴보겠습니다.

## <a name="example-1-access-review-for-users-accessing-from-blocked-countriesregions"></a>예 1: 차단된 국가/지역에서 액세스하는 사용자에 대한 액세스 검토

특정 국가/지역의 액세스를 차단하는 조건부 액세스 정책이 있다고 가정해 보겠습니다. 여기에는 정책에서 제외된 그룹이 포함됩니다. 그룹의 멤버를 검토하는 권장되는 액세스 검토는 다음과 같습니다.

> [!NOTE]
> 액세스 검토를 만들려면 글로벌 관리자 또는 사용자 관리자 역할이 필요합니다.
...
4. 각 사용자는 이러한 차단된 국가/지역에서 액세스해야 하므로 그룹의 구성원이어야 한다는 것을 스스로 증명해야 합니다.

5. 사용자가 검토 요청에 응답하지 않으면 그룹에서 자동으로 제거되므로 이러한 국가/지역으로 이동하는 동안 테넌트에 더 이상 액세스할 수 없습니다.

6. 사용자가 액세스 검토의 시작 및 완료를 알 수 있도록 메일 알림을 사용합니다.

    ![예 1과 같은 액세스 검토 창 만들기](./media/conditional-access-exclusion/create-access-review-1.png)

## <a name="example-2-access-review-for-users-accessing-with-legacy-authentication"></a>예제 2: 레거시 인증을 사용하여 액세스하는 사용자에 대한 액세스 검토

레거시 인증 및 이전 클라이언트 버전을 사용하는 사용자의 액세스를 차단하는 조건부 액세스 정책이 있다고 가정해 보겠습니다. 여기에는 정책에서 제외된 그룹이 포함됩니다. 그룹의 멤버를 검토하는 권장되는 액세스 검토는 다음과 같습니다.
...
6. 사용자가 액세스 검토의 시작 및 완료를 알 수 있도록 메일 알림을 사용합니다.
    ...
    ![예를 들어 2에 대한 액세스 검토 창 만들기](./media/conditional-access-exclusion/create-access-review-2.png)
...
이제 모든 것이 제자리에 있고, 그룹화되고, 조건부 액세스 정책 및 액세스 리뷰가 되었으므로 이러한 리뷰의 결과를 모니터링하고 추적할 차례입니다.
...
1. **결과**를 클릭하여 목록에 유지되도록 승인된 사용자 및 제거된 사용자를 확인합니다.
    ...
    ![액세스 리뷰 결과에 승인된 사람 표시](./media/conditional-access-exclusion/access-reviews-results.png)
