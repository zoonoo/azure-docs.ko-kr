...
- **자동화를 사용할 수 없는 경우:** 보안 그룹 또는 Office 365 그룹에서 동적 멤버 자격에 대한 규칙을 만들 수 있습니다. 하지만, HR 데이터가 Azure AD에 없거나 사용자가 그룹을 떠난 뒤에도 대체자를 교육시키기 위해 액세스 권한이 필요한 경우 어떻게 해야 하나요? 해당 그룹에서 검토를 만들어서 계속 액세스 권한이 필요한 사용자가 계속 액세스할 수 있도록 합니다.
...
액세스 검토를 만들려면 다음 단계를 수행합니다.

1. 액세스 검토를 관리하고 전역 관리자 또는 사용자 관리자로 로그인하려면 [Azure Portal](https://portal.azure.com)에 로그인합니다.

1. **Azure Active Directory**를 검색하고 선택합니다.

      ![Azure Active Directory에 대한 Azure Portal 검색](media/access-reviews-overview/search-azure-active-directory.png)

1. **ID 거버넌스**를 선택합니다.

1. 시작 페이지에서 **액세스 검토 만들기** 버튼을 클릭합니다.

   ![액세스 검토 시작 페이지](./media/access-reviews-overview/access-reviews-overview-create-access-reviews.png) 

### <a name="creating-access-review-on-a-group-that-can-be-assigned-to-azure-ad-role"></a>Azure AD 역할에 할당할 수 있는 그룹에 대한 액세스 검토 만들기
액세스 검토의 최신 버전인 경우(검토자는 기본적으로 **내 액세스**로 이동) 전역 관리자만 역할 할당 가능 그룹에서 액세스 검토를 만들 수 있습니다. 그러나 이전 버전의 액세스 검토를 사용하는 경우(검토자는 기본적으로 **액세스 패널**로 이동) 전역 관리자와 사용자 관리자 모두 역할 할당 가능 그룹에서 액세스 검토를 만들 수 있습니다.  

2020년 8월 1일 모든 고객에게 새로운 환경이 제공될 예정될 예정이지만 더 빨리 업그레이드하려는 경우, [Azure AD 액세스 검토 - 내 액세스 등록의 업데이트된 검토자 경험](https://forms.microsoft.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbR5dv-S62099HtxdeKIcgO-NUOFJaRDFDWUpHRk8zQ1BWVU1MMTcyQ1FFUi4u)에서 요청하세요.

[Azure AD 역할에 그룹을 할당하는 방법에 대해 자세히 알아보세요](https://go.microsoft.com/fwlink/?linkid=2103037).

## <a name="learn-about-access-reviews"></a>액세스 검토에 대해 자세히 알아보기
...
Azure AD Premium P2 라이선스는 다음 작업에 필요하지 **않습니다**.

- 전역 관리자 또는 사용자 관리자 역할을 가진 사용자는 액세스 검토를 설정하거나, 설정을 구성하거나, 검토에서 나온 결정 사항을 적용하는 데 라이선스가 필요하지 않습니다.
...
다음은 보유해야 하는 라이선스 수를 결정하는 데 도움이 되는 몇 가지 예제 라이선스 시나리오입니다.
...
| 관리자가 50명의 멤버 사용자와 25명의 게스트 사용자가 있는 그룹 C의 액세스 검토를 만듭니다. 자체적으로 검토합니다. | 자체 검토자인 각 사용자용 라이선스 50개<br/>(게스트 사용자는 5명당 1개 비율로 필요합니다.) | 50 |
| 관리자가 6명의 멤버 사용자와 108명의 게스트 사용자가 있는 그룹 D의 액세스 검토를 만듭니다. 자체적으로 검토합니다. | 자체 검토자인 각 사용자용 라이선스 6개 + 108명의 게스트 사용자 모두를 위한 추가 라이선스 16개(5명당 1개 비율로 필요) 게스트 사용자용 라이선스 6개(6\*5 = 30) 나머지 게스트 사용자(108-6\*5 = 78명)를 위해 78/5 = 16개의 추가 라이선스가 필요합니다. 따라서 총 6 + 16 = 22개의 라이선스가 필요합니다. | 22 |
