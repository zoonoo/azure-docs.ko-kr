...
[PIM (Azure AD Privileged Identity Management)](../privileged-identity-management/pim-configure.md) 은 azure AD, azure 및 기타 Microsoft Online Services에서 리소스에 대 한 액세스 권한을 보호 하도록 조정 된 추가 컨트롤을 제공 합니다.  Multi-factor authentication 및 조건부 액세스 외에도 Azure AD PIM에서 제공 하는 just-in-time 액세스 및 역할 변경 경고 기능은 회사의 리소스 (디렉터리, Office 365 및 Azure 리소스 역할)를 보호 하는 데 도움이 되는 포괄적인 거 버 넌 스 컨트롤 집합을 제공 합니다. 다른 양식의 액세스를 사용하는 경우와 마찬가지로 조직은 액세스 검토를 사용하여 관리자 역할의 모든 사용자에 대해 정기 액세스 재인증을 구성할 수 있습니다.
...
- [Azure AD 액세스 검토 란 무엇 인가요?](access-reviews-overview.md)
