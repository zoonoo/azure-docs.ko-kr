...
|[중소기업](#mid) | 원본 값의 부분 문자열을 반환합니다. 부분 문자열은 원본 문자열에서 문자 중 일부만 포함하는 문자열입니다.|
|[NormalizeDiacritics](#normalizediacritics)|하나의 문자열 인수가 필요합니다. 문자열을 반환하지만 해당하는 비분음 부호 문자로 대체된 분음 부호 문자를 포함합니다.|
|[나타내지](#not) |**source** 의 부울 값을 대칭 이동합니다. **source** 값이 "*True*"이면 "*False*"가 반환됩니다. False인 경우에는 "*True*"가 반환됩니다.| 
