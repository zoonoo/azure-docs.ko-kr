...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    a. **식별자** 텍스트 상자에서 `https://<server name>.globalview.adp.com/federate2` 또는 `https://<server name>.globalview.adp.com/federate` 패턴 중 하나를 사용하여 ADP GlobalView 응용 프로그램을 식별하는 데 사용할 URL을 입력합니다.


    b. **회신 URL** 텍스트 상자에서 `https://<server name>.globalview.adp.com/federate2/sp/ACS.saml2` 또는 `https://<server name>.globalview.adp.com/federate/sp/ACS.saml2` 패턴 중 하나를 사용하여 ADP GlobalView 응용 프로그램에 대한 응답을 게시하기 위해 Azure AD에서 사용할 URL을 입력합니다.
...
2. 응용 프로그램에 대해 구성된 SSO를 얻으려면 ADP GlobalView 지원 팀에 문의하고 다음을 제공하십시오. 
   ...
   * **Single Sign-Out Service URL**
    ...
    > [AZURE.NOTE] **ADP GlobalView** 팀이 인스턴스를 구성한 후 **RelayState** 값을 가져옵니다. 아래에서 언급하는 단계에 따라 인스턴스를 구성합니다. 이 구성을 완료한 후에 통합을 테스트할 수 있습니다. 따라서 이는 응용 프로그램 통합을 작동하기 위한 중요한 구성입니다.

1. Azure AD에서 RelayState 값을 구성하려면 다음 단계를 수행합니다. 
    ...
    j. 설정을 저장합니다.
2. Azure 클래식 포털에서 Single Sign-On 구성 확인을 선택하고 **다음**을 클릭합니다.
