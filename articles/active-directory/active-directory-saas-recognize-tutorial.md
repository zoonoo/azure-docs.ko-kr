...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Recognize에서 Azure AD Single Sign-On을 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Recognize 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 Recognize의 관련 사용자 간에 연결이 형성되어야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 Recognize 응용 프로그램에서 Single Sign-On을 구성합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
    ...
    a. **로그온 URL** 텍스트 상자에서 `https://recognizeapp.com/<your-domain>/saml/sso` 패턴을 사용하여 URL을 입력합니다.
   
    b. **식별자** 텍스트 상자에 `https://recognizeapp.com/<your-domain>/saml/metadata` 패턴으로 URL을 입력합니다.
...
8. **SSO 설정** 섹션에서 다음 단계를 수행합니다.
    ...
    b. **IDP 엔터티 ID** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **발급자 URL** 값을 저장합니다.
   
    c. **SSO 대상 URL** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **Single Sign-On 서비스 URL** 값을 입력합니다.
   
    d. **SLO 대상 URL** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **Single Sign-Out 서비스 URL** 값을 입력합니다.
   
    e. 다운로드된 인증서 파일을 메모장에서 열고, 내용을 클립보드에 복사한 다음 전체 인증서를 **인증서** 텍스트 상자에 붙여넣습니다. 
...
4. **사용자 동기화** 섹션에서 다음 단계를 수행합니다.
   ...
   ![새 사용자](./media/active-directory-saas-recognize-tutorial/tutorial_recognize_005.png "New User")
   
   a. **동기화 사용**에서 **켜기**를 선택합니다.
   
   b. **동기화 공급자 선택**에서 **Microsoft/Office 365**를 선택합니다.
   
   c. **사용자 동기화 실행**을 클릭합니다.
