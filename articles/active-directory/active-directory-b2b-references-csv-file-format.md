...
>[AZURE.NOTE] 레이블의 첫 번째 행(예: Email, DisplayName 등)은 CSV 파일을 성공적으로 구문 분석하기 위해 필요합니다. 철자는 아래 지정된 필드와 일치해야 합니다. 이러한 레이블은 그 아래의 콘텐츠를 식별합니다. 필요하지 않은 선택적 필드에 대한 레이블은 CSV 파일에서 제거할 수 있습니다. 전체 열을 비워 둘 수 있습니다.

## 필수 필드: <br/>
**Email:** 초대한 사용자의 메일 주소입니다. <br/> **DisplayName:** 초대한 사용자에 대한 표시 이름(일반적으로 성 및 이름)입니다.<br/>


## 선택적 필드: <br/>

**InvitationText:** 앱 브랜딩 뒤와 사용 링크 앞의 초대 메일 텍스트를 사용자 지정합니다.<br/> **InvitedToApplications:** 사용자를 할당할 회사 응용 프로그램에 대한 AppID입니다. AppID는 사용자를 추가하려는 그룹에 대한 `Get-MsolServicePrincipal | fl DisplayName, AppPrincipalId`<br/> **InvitedToGroups:** ObjectID를 호출하여 PowerShell에서 검색할 수 있습니다. ObjectID는 초대 수락 후 초대한 사용자를 연결하기 위해 `Get-MsolGroup | fl DisplayName, ObjectId`<br/> **InviteRedirectURL:** URL을 호출하여 PowerShell에서 검색할 수 있습니다. 회사 관련 URL(예: [*contoso.my.salesforce.com*](http://contoso.my.salesforce.com/))이어야 합니다. 이 선택적 필드를 지정하지 않으면 초대한 사용자가 선택한 회사 앱을 탐색할 수 있는 앱 액세스 패널로 연결됩니다. 앱 액세스 패널 URL의 형식은 `https://account.activedirectory.windowsazure.com/applications/default.aspx?tenantId=<TenantID>`입니다.<br/> **CcEmailAddress**: 메일로 보낸 초대를 복사할 메일 주소입니다. CcEmailAddress 필드를 사용할 경우 이 초대는 메일 확인 사용자 또는 테넌트 만들기에 사용할 수 없습니다.<br/> **Language:** 초대 메일 및 사용 환경에 사용하는 언어로, 지정되지 않은 경우 기본값으로 "en"(영어)이 사용됩니다. 지원되는 다른 10가지 언어 코드는 다음과 같습니다.<br/>
1. de: 독일어<br/>
2. es: 스페인어<br/>
3. fr: 프랑스어<br/>
4. it: 이탈리아어<br/>
