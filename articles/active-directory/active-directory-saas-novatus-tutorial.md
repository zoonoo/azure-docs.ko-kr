...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    a. 로그온 URL 텍스트 상자에 **“https://sso.novatuscontracts.com/companyname”**의 패턴을 사용하여 사용자가 Novatus 응용 프로그램에 로그온하는 데 사용할 URL을 입력합니다. **companyname** 을 실제 이름으로 교체해야 하는 일반 이름을 참조하는 경우


1. **Novatus의 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
    ...
    b. **다음**을 클릭합니다.
2. 응용 프로그램에 대해 SSO를 구성하려면 jvinci@novatusinc.com. 다운로드한 인증서 파일을 메일에 첨부하고 해당하는 쪽에 SSO를 설정하기 위해 Novatus 팀과 메타데이터 URL(엔터티 ID, SSO 로그인 URL 및 로그아웃 URL)을 공유합니다.
