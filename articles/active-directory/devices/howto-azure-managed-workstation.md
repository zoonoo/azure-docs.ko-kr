...
| Microsoft Autopilot를 사용 하 여 배포 |   |   | yes  | yes | yes | yes |
| Intune에 의해서만 설치 된 앱 |   |   |   | yes | yes |yes |
| 승인 된 목록으로 제한 된 Url |   |   |   | yes | yes |yes |
...
1. 다음을 입력합니다.
   ...
   * **제한 된 관리자** - **디렉터리 역할** 을 선택 하 고 **Intune 관리자** 역할을 선택 합니다.
...
1. **장치 설정** > **Azure Active Directory** > **장치** 로 이동 합니다.
...
1. **다음**을 선택합니다.
1. **선택한 그룹** > **할당 > ** **할당을** 선택 합니다. **포함할 그룹 선택**에서 **워크스테이션 보안**을 선택 합니다.
1. **다음**을 선택합니다.
...
1. **Windows DEFENDER ATP** > **Microsoft Intune** > **장치 준수** 로 이동 합니다.
...
여기의 지침에 따라 보안 워크스테이션을 배포 했습니다. 그러나 추가 컨트롤을 고려해 야 합니다. 다음은 그 예입니다.
...
1. **앱 패키지 파일**의 추출 된 위치에서 `GoogleChromeStandaloneEnterprise64.msi` 파일을 선택 하 고 **확인**을 선택 합니다.
...
1. **Azure Portal** > **Microsoft Intune** > **조건부 액세스 정책** > **새 정책**으로 이동 합니다.
1. 정책의 **이름을** 제공 합니다.
1. **사용자 및 그룹** > 선택 하 여 **사용자 및 그룹 선택** 
1.  > **디렉터리 역할** **포함** 을 선택 하 > 전역 관리자, 권한 있는 역할 관리자, 권한 있는 인증 관리자, 보안 관리자, 준수 관리자, 조건부 액세스 관리자, 응용 프로그램 관리자, 클라우드 응용 프로그램 관리자, Intune 서비스 관리자 > 역할을 선택 합니다.
...
Microsoft Scripting Center의 [Setdesktopbackground.](https://gallery.technet.microsoft.com/scriptcenter/Set-Desktop-Image-using-5430c9fb/) p s 1 스크립트를 통해 Windows는 시작 시이 [무료 일반 배경 이미지](https://i.imgur.com/OAJ28zO.png) 를 로드할 수 있습니다.
...
1. 장치를 등록 하려면 다음 정보가 필요 합니다.
   * **일련 번호** -장치 섀시에 있습니다.
   * **Windows 제품 ID** -windows 설정 메뉴의 **시스템** ** > 에서** 찾을 수 있습니다.
   * [WindowsAutoPilotInfo](https://aka.ms/Autopilotshell) 를 실행 하 여 장치 등록에 필요한 모든 정보가 포함 된 CSV 해시 파일을 가져올 수 있습니다.
   
     `Get-WindowsAutoPilotInfo – outputfile device1.csv`를 실행 하 여 Intune으로 가져올 수 있는 CSV 파일로 정보를 출력 합니다.
