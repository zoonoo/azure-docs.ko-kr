...
* **장치 ID**: PRT는 특정 장치에서 사용자에 게 발급 됩니다. 장치 ID 클레임 `deviceID` 은 사용자가 PRT를 발급 한 장치를 결정 합니다. 이 클레임은 나중에 PRT를 통해 가져온 토큰에 발급 됩니다. 장치 ID 클레임은 장치 상태 또는 규정 준수에 따라 조건부 액세스에 대 한 권한 부여를 결정 하는 데 사용 됩니다.
...
* **AZURE AD 등록 장치**: 사용자가 Windows 10 장치에 보조 회사 계정을 추가할 때 PRT가 발급 됩니다. 사용자는 두 가지 방법으로 Windows 10에 계정을 추가할 수 있습니다.  
   ...
   * **설정** > **Accounts**계정의 > 계정 추가**회사 또는 학교** > **연결** 액세스
