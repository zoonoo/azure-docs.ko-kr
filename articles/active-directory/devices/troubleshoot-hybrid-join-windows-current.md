---
title: 하이브리드 Azure Active Directory 연결 된 장치 문제 해결-Azure Active Directory
...
### <a name="step-3-find-the-phase-in-which-join-failed-and-the-errorcode"></a>3단계: 조인이 실패 한 단계와 errorcode를 찾습니다.
...
### <a name="step-4-check-for-possible-causes-and-resolutions-from-the-lists-below"></a>4단계: 아래 목록에서 가능한 원인 및 해결 방법을 확인 하십시오.
...
- **DSREG_AUTOJOIN_DISC_FAILED** (0x801c0021/-2145648607)
   - 원인: 일반 검색 오류입니다. DRS에서 검색 메타 데이터를 가져오지 못했습니다.
   - 해결 방법: 아래에서 하위 오류를 찾아 자세히 조사 합니다.
- **DSREG_AUTOJOIN_DISC_WAIT_TIMEOUT**  (0x801c001f/-2145648609)
   - 원인: 검색을 수행 하는 동안 작업 시간이 초과 되었습니다.
...
- **DSREG_AUTOJOIN_USERREALM_DISCOVERY_FAILED** (0x801c0021/-2145648611)
   - 원인: 일반 영역 검색 오류입니다. STS에서 도메인 유형 (관리/페더레이션)을 확인 하지 못했습니다. 
   - 해결 방법: 아래에서 하위 오류를 찾아 자세히 조사 합니다.
...
- **WININET_E_DECODING_FAILED** (0x80072f8f가/-2147012721)
   ...
   - 해결 방법: 네트워크 프록시가 서버 응답을 방해 하 고 수정 하 고 있지 않은지 확인 합니다.

###### <a name="http-errors"></a>HTTP 오류

- **DSREG_DISCOVERY_TENANT_NOT_FOUND** (0x801c003a/-2145648582)
   - 원인: 잘못 된 테 넌 트 ID로 구성 된 SCP 개체입니다. 또는 테 넌 트에서 활성 구독을 찾을 수 없습니다.
   - 해결 방법: SCP 개체가 올바른 Azure AD 테 넌 트 ID 및 활성 구독으로 구성 되어 있는지 또는 테 넌 트에 있는지 확인 하세요.
...
- **ERROR_ADAL_INTERNET_CANNOT_CONNECT** (0xcaa82efd/-894947587)
   - 원인: 에 `https://login.microsoftonline.com` 연결 하지 못했습니다.
   - 해결 방법: 에 대 한 네트워크 `https://login.microsoftonline.com`연결을 확인 합니다.

##### <a name="other-errors"></a>기타 오류

- **ERROR_ADAL_SERVER_ERROR_INVALID_GRANT** (0xcaa20003/-895352829)
   - 원인: 온-프레미스 id 공급자의 SAML 토큰은 Azure AD에서 허용 되지 않았습니다.
...
- **DSREG_E_DIRECTORY_FAILURE** (0x801c03f2/-2145647630)
   - 원인: DRS에서 오류 응답을 수신 했습니다. 오류 코드: DirectoryError
   - 해결 방법: 가능한 원인 및 해결 방법에 대해서는 서버 오류 코드를 참조 하십시오.
- **DSREG_E_DEVICE_AUTHENTICATION_ERROR** (0x801c0002/-2145648638)
   - 원인: DRS에서 오류 응답을 수신 했습니다. 오류 코드: "AuthenticationError" 및 ErrorSubCode 코드는 "DeviceNotFound"이 아닙니다. 
   - 해결 방법: 가능한 원인 및 해결 방법에 대해서는 서버 오류 코드를 참조 하십시오.
...
- **TPM_E_NOTFIPS** (0x80280036/-2144862154)
   - 원인: FIPS 모드의 TPM은 현재 지원 되지 않습니다.
...
- **WININET_E_TIMEOUT** (0x80072ee2/-2147012894)
   - 원인: DRS에서 장치를 등록 하려고 시도 하는 일반 네트워크 시간 초과
   - 해결 방법: 에 대 한 네트워크 `https://enterpriseregistration.windows.net`연결을 확인 합니다.
- **WININET_E_NAME_NOT_RESOLVED** (0x80072ee7/-2147012889)
   - 원인: 서버 이름이 나 주소를 확인할 수 없습니다.
   - 해결 방법: 에 대 한 네트워크 `https://enterpriseregistration.windows.net`연결을 확인 합니다. 호스트 이름에 대 한 DNS 확인이 장치에서 n/w와 정확한 지 확인 합니다.
- **WININET_E_CONNECTION_ABORTED** (0x80072efe/-2147012866)
   - 원인: 서버와의 연결이 비정상적으로 종료 되었습니다.
   - 해결 방법: 잠시 후 다시 시도 하거나 안정적인 다른 네트워크 위치에서 조인 해 보세요.
...
| 서버 오류 코드 | 서버 오류 메시지 | 가능한 이유 | 해결 방법 |
| --- | --- | --- | --- |
| DirectoryError | AADSTS90002: 테 <UUID> 넌 트를 찾을 수 없습니다. 이 오류는 테 넌 트에 대 한 활성 구독이 없는 경우에 발생할 수 있습니다. 구독 관리자에 게 문의 하세요. | SCP 개체의 테 넌 트 ID가 잘못 되었습니다. | SCP 개체가 올바른 Azure AD 테 넌 트 ID 및 활성 구독으로 구성 되 고 테 넌 트에 표시 되는지 확인 합니다. |
...
### <a name="step-5-collect-logs-and-contact-microsoft-support"></a>5단계: 로그 및 연락처 Microsoft 지원 수집

여기에서 공용 [ https://1drv.ms/u/s 스크립트를 가져옵니다. AkyTjQ17vtfagYkZ6VJzPg78e3o7PQ]( https://1drv.ms/u/s!AkyTjQ17vtfagYkZ6VJzPg78e3o7PQ)
