...
이 자습서에서는 Azure AD(Azure Active Directory)와 Autotask Workplace를 통합하는 방법에 대해 알아봅니다.

Autotask Workplace와 Azure AD를 통합하면 다음과 같은 이점이 제공됩니다.

- Azure AD에서는 Autotask Workplace에 대한 액세스 권한이 있는 사용자를 제어할 수 있습니다.
- 사용자가 해당 Azure AD 계정으로 Autotask Workplace에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
Autotask Workplace와 Azure AD 통합을 구성하려면 다음 항목이 필요합니다.
...
1. 갤러리에서 Autotask Workplace 추가
2. Azure AD Single Sign-on 구성 및 테스트

## <a name="adding-autotask-workplace-from-the-gallery"></a>갤러리에서 Autotask Workplace 추가
Autotask Workplace의 Azure AD 통합을 구성하려면 갤러리의 Autotask Workplace를 관리되는 SaaS 앱 목록에 추가해야 합니다.

**갤러리에서 Autotask Workplace를 추가하려면 다음 단계를 수행합니다.**
...
Autotask Workplace에서 Azure AD의 **사용자 이름** 값을 **Username** 값으로 할당하여 링크 관계를 설정합니다.
...
**Autotask Workplace에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.**
...
3. **Autotask Workplace 도메인 및 URL** 섹션에서 **IDP** 시작 모드로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
    ...
    나. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://<subdomain>.awp.autotask.net/singlesignon/saml/SSO`
...
4. **SP** 시작 모드에서 응용 프로그램을 구성하려면 **고급 URL 설정**을 확인하고 다음 단계를 수행합니다.
    ...
    **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<subdomain>.awp.autotask.net/loginsso`
    ...
    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 값을 실제 식별자, 회신 URL 및 로그온 URL로 업데이트합니다. 이러한 값을 얻으려면 [Autotask Workplace 클라이언트 지원팀](https://awp.autotask.net/help/Content/0_HOME/Support_for_End_Clients.htm)에 문의하세요. 
...
액세스 패널에서 Autotask Workplace 타일을 클릭하면 Autotask Workplace 응용 프로그램에 자동으로 로그온됩니다.
