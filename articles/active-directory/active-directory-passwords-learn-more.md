...
아래 테이블에서는 동기화 기능의 버전에 따라 지원되는 시나리오를 설명합니다.  일반적으로 암호 쓰기 저장 기능을 사용하려면 최신 버전의 [Azure AD Connect](connect/active-directory-aadconnect.md#install-azure-ad-connect) 를 설치하는 것이 좋습니다.
...
> [!NOTE]
> 사무실 전화는 등록 포털에 나타나지 않는데, 사용자가 현재 디렉터리에서 이 속성을 편집할 수 없기 때문입니다.
>
>
...
#### <a name="data-accessible-with-azure-ad-powershell"></a>Azure AD PowerShell을 통해 액세스할 수 있는 데이터
...
##### <a name="alternate-email"></a>대체 전자 메일
```
Connect-MsolService
...
##### <a name="authentication-phone"></a>인증 전화
```
Connect-MsolService
Get-MsolUser -UserPrincipalName user@domain.com | select -Expand StrongAuthenticationUserDetails | select PhoneNumber
```

##### <a name="authentication-email"></a>인증 전자 메일
```
Connect-MsolService
