...
3. **웹 응용 프로그램 풀 – SharePoint – 80**을 선택합니다. 옵션은 기본적으로 웹 풀에서 SSL을 사용하는 경우 또는 웹 풀의 이름에 따라 약간 다를 수 있습니다.
  ...
  ![서비스 계정 구성 옵션](./media/application-proxy-remote-sharepoint/service-web-application.png)
...
2. **응용 프로그램 관리**로 이동하고 **웹 응용 프로그램 관리**를 선택하고 SharePoint 사이트를 선택합니다. 이 예제에서는 **SharePoint – 80**입니다.
  ...
  ![SharePoint 사이트 선택](./media/application-proxy-remote-sharepoint/manage-web-applications.png)
...
6. Windows 통합 인증 필드에 대한 드롭다운 상자에서 **협상(Kerberos)**이 선택되었는지 확인합니다.
  ...
  ![인증 편집 대화 상자](./media/application-proxy-remote-sharepoint/service-edit-authentication.png)
...
4. 위임 설정이 **지정된 서비스에 대한 위임용으로만 이 컴퓨터 트러스트**로 설정되어 있는지 확인합니다. 그런 다음 **모든 인증 프로토콜 사용**을 선택합니다.
  ...
  ![위임 설정](./media/application-proxy-remote-sharepoint/delegation-box.png)

5. **추가** 단추를 클릭한 후 **사용자 또는 컴퓨터**를 클릭하고 서비스 계정을 찾습니다.

  ![서비스 계정에 대한 SPN 추가](./media/application-proxy-remote-sharepoint/users-computers.png)
...
1. 다음 설정을 사용하여 SharePoint 사이트를 게시합니다. 단계별 지침은 [Azure AD 응용 프로그램 프록시를 사용하여 응용 프로그램 게시](application-proxy-publish-azure-portal.md)를 참조하세요. 
   ...
   ![SharePoint를 응용 프로그램으로 게시](./media/application-proxy-remote-sharepoint/publish-app.png)
...
2. 앱이 게시된 후에는 다음 단계에 따라 Single Sign-On 설정을 구성합니다.
   ...
   ![SSO용 통합 Windows 인증 구성](./media/application-proxy-remote-sharepoint/configure-iwa.png)
...
2. **시스템 설정** 아래에서 **대체 액세스 매핑 구성**을 선택합니다. 대체 액세스 매핑 상자가 열립니다.
  ...
  ![대체 액세스 매핑 상자](./media/application-proxy-remote-sharepoint/alternate-access1.png)
