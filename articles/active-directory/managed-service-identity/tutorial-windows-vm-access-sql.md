...
> * Azure AD에서 그룹을 만들고 VM의 시스템 할당 관리 ID를 그룹의 구성원으로 만들기
...
1.  Azure AD에서 그룹을 만들고 VM의 시스템 할당 관리 ID를 그룹의 구성원으로 만듭니다.
2.  SQL Server에 대한 Azure AD 인증을 사용하도록 설정합니다.
3.  Azure AD 그룹을 나타내는 데이터베이스에 **포함된 사용자**를 만듭니다.

> [!NOTE]
> 일반적으로 VM의 시스템 할당 관리 ID에 직접 매핑되는 포함된 사용자를 만듭니다.  현재 Azure SQL은 포함된 사용자에 매핑될 VM의 시스템 할당 관리 ID를 나타내는 Azure AD 서비스 주체를 허용하지 않습니다.  지원되는 해결 방법으로, VM의 시스템 할당 관리 ID를 Azure AD 그룹의 구성원으로 만든 다음, 해당 그룹을 나타내는 데이터베이스에 포함된 사용자를 만듭니다.


## <a name="create-a-group-in-azure-ad-and-make-the-vms-system-assigned-managed-identity-a-member-of-the-group"></a>Azure AD에서 그룹을 만들고 VM의 시스템 할당 관리 ID를 그룹의 구성원으로 만들기
...
다음으로, 그룹에 VM의 시스템 할당 관리 ID를 추가합니다.  Azure PowerShell을 사용하여 알 수 있는 시스템 할당 관리 ID의 **ObjectId**가 필요합니다.  먼저 [Azure Powershell](https://docs.microsoft.com/powershell/azure/install-azurerm-ps)을 다운로드합니다. 그런 후 `Connect-AzureRmAccount`를 사용하여 로그인하고 다음 명령을 실행하여 다음을 수행합니다.
- 여러 구독이 있는 경우 세션 컨텍스트가 원하는 Azure 구독으로 설정되어 있는지 확인합니다.
- Azure 구독의 사용 가능한 리소스를 나열하고 올바른 리소스 그룹 및 VM 이름을 확인합니다.
- 적절한 `<RESOURCE-GROUP>` 및 `<VM-NAME>` 값을 사용하여 VM의 시스템 할당 관리 ID 속성을 가져옵니다.
...
출력은 다음과 같으며 VM의 시스템 할당 관리 ID의 서비스 주체 개체 ID를 검사합니다.
```powershell
...
이제 그룹에 VM의 시스템 할당 관리 ID를 추가합니다.  Azure AD PowerShell을 사용하면 그룹에 서비스 주체만 추가할 수 있습니다.  다음 명령을 실행합니다.
```powershell
...
그룹을 만들고 그룹 구성원 자격에 VM의 시스템 할당 관리 ID를 추가했으므로 다음 단계를 사용하여 [SQL Server에 대해 Azure AD 인증을 구성](/azure/sql-database/sql-database-aad-authentication-configure#provision-an-azure-active-directory-administrator-for-your-azure-sql-server)할 수 있습니다.
