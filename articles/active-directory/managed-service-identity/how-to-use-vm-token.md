...
이 문서에서는 토큰 획득을 위한 다양한 코드 및 스크립트 예제뿐만 아니라 토큰 만료 및 HTTP 오류를 처리하는 등 중요한 항목에 대한 지침을 제공합니다. VM 확장 엔드포인트는 향후에 사용이 중지되므로 IMDS 엔드포인트와 함께 관리 서비스 ID를 사용하는 것이 좋습니다.
...
> [!IMPORTANT]
> - 관리 ID의 보안 경계는 리소스입니다. MSI 사용 Virtual Machine에서 실행되는 모든 코드/스크립트는 토큰을 요청하고 검색할 수 있습니다. 

## <a name="overview"></a>개요

클라이언트 응용 프로그램은 지정된 리소스에 액세스하기 위해 MSI [앱 전용 액세스 토큰](../develop/active-directory-dev-glossary.md#access-token)을 요청할 수 있습니다. 토큰은 [MSI 서비스 주체에 기반](overview.md#how-does-it-work)합니다. 따라서 고유한 서비스 주체 하에서 액세스 토큰을 가져오기 위해 클라이언트를 등록하지 않아도 됩니다. 토큰은 [서비스 간 호출 요청 클라이언트 자격 증명](../develop/active-directory-protocols-oauth-service-to-service.md)에서 전달자 토큰으로 사용하기에 적합합니다.
...
MSI IMDS(인스턴스 메타데이터 서비스) 엔드포인트를 사용하는 샘플 요청 *(권장됨)*:
...
코드에서 토큰을 캐시하는 경우 토큰이 만료되었음을 리소스가 나타내는 시나리오를 처리하도록 준비해야 합니다. 

참고: IMDS MSI 하위 시스템은 토큰을 캐시하지 않으므로 다음과 같은 경우에만 Azure AD 결과를 실시간으로 호출합니다.
...
| 서비스의 5xx 일시적 오류입니다. | MSI 하위 시스템 또는 Azure Active Directory에서 일시적 오류를 반환했습니다. | 1초 이상 대기한 후에 다시 시도하는 것이 안전합니다.  너무 빨리 또는 너무 자주 다시 시도하는 경우 IMDS 및/또는 Azure AD는 속도 제한 오류(429)를 반환할 수 있습니다.|
...
|           | unauthorized_client | 클라이언트에는 이 메서드를 사용하여 액세스 토큰을 요청할 권한이 없습니다. | 로컬 루프백을 사용하여 확장을 호출하지 않은 요청에 의해 발생하거나 MSI를 올바르게 구성하지 않은 VM에서 발생합니다. VM을 구성하는 데 도움이 필요한 경우 [Azure Portal을 사용하여 VM MSI(관리 서비스 ID) 구성](qs-configure-portal-windows-vm.md)을 참조하세요. |
...
| 500 내부 서버 오류 | 알 수 없음 | Active directory에서 토큰을 검색하지 못했습니다. 자세한 내용은 *\<파일 경로\>* 에서 로그를 참조하세요. | MSI가 VM에서 설정되어 있는지 확인합니다. VM을 구성하는 데 도움이 필요한 경우 [Azure Portal을 사용하여 VM MSI(관리 서비스 ID) 구성](qs-configure-portal-windows-vm.md)을 참조하세요.<br><br>또한 HTTP GET 요청 URI의 형식, 특히 쿼리 문자열에서 지정된 리소스 URI가 올바르게 지정되었는지 확인합니다. 예제는 [이전 REST 섹션](#rest)에서 "샘플 요청"을 참조하세요. 또는 서비스 및 각 리소스 ID의 목록은 [Azure AD 인증을 지원하는 Azure 서비스](overview.md#azure-services-that-support-azure-ad-authentication)를 참조하세요.
