...
3. **로컬 추적 시작**을 선택합니다.
   ...
   ![네트워크 캡처 시작](./media/application-proxy-working-with-proxy-servers/start-local-trace.png)

3. Azure AD 응용 프로그램 프록시 커넥터 서비스를 시작합니다.
4. 네트워크 캡처를 중지합니다.

   ![네트워크 캡처 중지](./media/application-proxy-working-with-proxy-servers/stop-trace.png)
