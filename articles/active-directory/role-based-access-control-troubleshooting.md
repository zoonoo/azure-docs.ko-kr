...
이 문서에서는 Azure 포털의 몇 가지 역할을 사용하는 경우의 결과를 소개합니다. 모든 리소스 유형을 다루기 위해 포함된 3가지 일반적인 역할이 있습니다.
...
1. 시작, 중지 등의 명령
2. 일반 구성, 규모 설정, 백업 설정, 모니터링 설정 등의 설정 변경
...
1. 다음 항목을 사용하려면 가상 컴퓨터에 대한 **쓰기** 권한이 필요합니다.  
    ...
    * 디스크
    * 확장
2. 다음 항목을 사용하려면 가상 컴퓨터와 가상 컴퓨터가 속한 **리소스 그룹**(도메인 이름 포함) 둘 다에 대한 **쓰기** 권한이 필요합니다.  
    * 가용성 집합
    * 부하 분산된 집합
    * 경고 규칙
