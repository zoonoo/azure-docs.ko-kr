...
* 사용자가 해당 Azure AD 계정으로 BetterWorks SSO(Single Sign-On)에 자동으로 로그온되도록 설정할 수 있습니다.
...
2. **SAML 토큰 특성** 대화 상자에서 아래 테이블의 각 행에 대해 다음 단계를 수행합니다.
    ...
    3. **특성 값** 목록에서 해당 행에 대해 표시되는 SAML 토큰 ID를 입력합니다.
    4. 페이지 맨 아래에 있는 **완료**
...
3. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
   ...
   1. **식별자** 텍스트 상자에 `https://app.betterworks.com/saml2/metadata/` 패턴으로 URL을 입력합니다.
   2. **회신 URL** 텍스트 상자에 `https://app.betterworks.com/saml2/acs/` 패턴으로 URL을 입력합니다.
   3. 페이지 맨 아래에 있는 **다음**
...
1. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
    ...
    1. **고급 설정을 표시합니다(선택 사항).**를 선택합니다.
    2. **로그온 URL** 텍스트 상자에 `https://app.betterworks.com` 패턴을 사용하여 사용자가 BetterWorks 응용 프로그램에 로그온하는 데 사용할 URL을 입력합니다.
    3. **다음**을 클릭합니다.
...
### <a name="create-a-betterworks-test-user"></a>BetterWorks 테스트 사용자 만들기
...
이 섹션은 Britta Simon에게 BetterWorks에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
