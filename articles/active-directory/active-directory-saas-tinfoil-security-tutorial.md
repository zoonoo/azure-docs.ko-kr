...
이 자습서는 Azure와 Tinfoil Security의 통합을 보여 주기 위한 것입니다.  
...
### <a name="to-enable-the-application-integration-for-tinfoil-security-perform-the-following-steps"></a>Tinfoil Security에 응용 프로그램 통합을 사용하도록 설정하려면 다음 단계를 수행합니다.
...
이 섹션은 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD의 계정으로 Tinfoil Security에 인증할 수 있게 하는 방법을 간략하게 설명하기 위한 것입니다.  
Tinfoil Security에 대한 Single Sign-On을 구성하려면 인증서의 지문(thumbprint) 값을 검색해야 합니다.  
...
3. **앱 URL 구성** 페이지에 있는 **Tinfoil Security 회신 URL** 텍스트 상자에 자신의 Tinfoil Security 어설션 소비자 서비스(ACS) URL(예: "*https://www.tinfoilsecurity.com/saml/consume*")을 입력하고 **다음**을 클릭합니다.
    ...
    > [!NOTE]
    > Tinfoil Security 메타데이터(https://www.tinfoilsecurity.com/saml/metadata)에서 ACS URL을 가져올 수 있어야 합니다.
    > 
    > 
...
8. **Single Sign-On** 구성 페이지에서 다음 단계를 수행합니다.
    ...
    c. Azure 클래식 포털의 **Tinfoil Security에서 Single Sign-On 구성** 대화 상자 페이지에서 **SAML SSO URL** 값을 복사한 다음 **SAML 게시 URL** 텍스트 상자에 붙여 넣습니다.
   
    ㄹ. 내보낸 인증서에서 **지문** 값을 복사한 다음 **SAML 인증서 지문** 텍스트 상자에 붙여 넣습니다.  
...
11. 필요한 특성 매핑을 추가하려면 다음 단계를 수행합니다.
    ...
    c. **특성 값** 텍스트 상자에 이전 섹션에서 복사한 계정 ID 값을 붙여 넣습니다.
...
Azure AD 사용자가 Tinfoil Security에 로그인할 수 있게 하려면 Tinfoil Security로 프로비전되어야 합니다.  
Tinfoil Security의 경우 프로비전은 수동 작업입니다.

### <a name="to-get-a-user-provisioned-perform-the-following-steps"></a>사용자 프로비전을 구성하려면 다음 단계를 따르십시오.
...
> [!NOTE]
> 다른 Tinfoil Security 사용자 계정 생성 도구 또는 Tinfoil Security가 제공한 API를 사용하여 AAD 사용자 계정을 프로비전할 수 있습니다.
> 
> 
...
### <a name="to-assign-users-to-tinfoil-security-perform-the-following-steps"></a>Tinfoil Security에 사용자를 할당하려면 다음 단계를 수행합니다.
