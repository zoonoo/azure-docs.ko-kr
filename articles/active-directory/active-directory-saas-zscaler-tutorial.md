...
### <a name="to-enable-the-application-integration-for-zscaler-perform-the-following-steps"></a>Zscaler에 응용 프로그램 통합을 사용 하려면 다음 단계를 수행 합니다.
...
이 섹션의 목적은 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD에서 자신의 계정으로 Zscaler에 인증할 수 있게 하는 방법을 간략하게 설명하는 것입니다.  
이 절차의 일부로 Zscaler에 인증서를 업로드 해야 합니다.
...
3. **Zscaler Sign In URL** 텍스트 상자의 **앱 URL 구성**페이지에서 Zscaler에서 얻은 로그인 URL을 입력한 후, **다음**을 클릭합니다. 
    ...
    > [!NOTE]
    > 로그인 URL을 모를 경우 Zscaler 지원팀에 문의하세요.
    > 
    > 
...
Azure AD 사용자가 ZScaler에 로그인할 수 있도록 하려면 사용자 계정이 ZScaler로 프로비전되어야 합니다.  
...
### <a name="to-assign-users-to-zscaler-perform-the-following-steps"></a>Zscaler에 사용자를 할당하려면 다음 단계를 수행합니다.
