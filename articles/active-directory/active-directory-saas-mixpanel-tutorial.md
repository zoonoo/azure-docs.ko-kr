...
* 사용자가 해당 Azure AD 계정으로 Mixpanel SSO(Single Sign-On)에 자동으로 로그인하도록 설정할 수 있습니다.
...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Mixpanel에서 Azure AD SSO를 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Mixpanel 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자 및 Mixpanel의 관련 사용자 간에 연결이 형성되어야 합니다.
...
이 섹션은 Azure 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 Mixpanel 응용 프로그램에서 SSO를 구성하는 방법을 설명하기 위한 것입니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
  ...
  1. **로그온 URL** 텍스트 상자에 다음 **“https://mixpanel.com/login/”** 패턴을 사용하여 사용자가 Mixpanel 응용 프로그램에 로그온하는 데 사용할 URL을 입력합니다.

    >[!NOTE]
    >로그인 자격 증명을 설정하려면 [https://mixpanel.com/register/](https://mixpanel.com/register/)에 등록하고 테넌트에 대한 SSO 설정을 사용할 수 있도록 하려면 [Mixpanel 지원팀](mailto:support@Mixpanel.com)에 문의하십시오. Mixpanel 지원팀에서 필요한 경우 로그인 URL 값을 가져올 수도 있습니다.
    >
...
### <a name="create-a-mixpanel-test-user"></a>Mixpanel 테스트 사용자 만들기
...
이 섹션은 Britta Simon에게 Mixpanel에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
