...
description: Azure AD Multi-Factor Authentication의 NPS 확장 문제 해결에 대 한 도움말 보기
...
ms.locfileid: "94839032"
---
# <a name="resolve-error-messages-from-the-nps-extension-for-azure-ad-multi-factor-authentication"></a>Azure AD Multi-Factor Authentication의 NPS 확장에서 오류 메시지를 확인 합니다.

Azure AD Multi-Factor Authentication에 대 한 NPS 확장에 오류가 발생 하는 경우이 문서를 사용 하 여 해결 속도를 더 빠르게 달성 하세요. Nps 확장 로그는 이벤트 뷰어의 **사용자 지정 보기**  >  **서버 역할**  >  **네트워크 정책 및** nps 확장이 설치 된 서버의 액세스 서비스에서 찾을 수 있습니다.
...
| **HTTPS_COMMUNICATION_ERROR** | NPS 서버가 Azure AD MFA에서 응답을 받을 수 없습니다. 방화벽이 https://adnotifications.windowsazure.com과의 트래픽을 위해 양방향으로 열려 있는지 확인합니다. |
...
| **REQUEST_MISSING_CODE** | NPS 서버와 NAS 서버 간의 암호 암호화 프로토콜이 사용 중인 보조 인증 방법을 지원하는지 확인합니다. **PAP** 는 클라우드에서 AZURE AD MFA의 모든 인증 방법을 지원 합니다. 전화 통화, 단방향 문자 메시지, 모바일 앱 알림 및 모바일 앱 확인 코드입니다. **CHAPV2** 및 **EAP** 는 전화 통화 및 모바일 앱 알림을 지원합니다. |
...
| **InvalidSession** | 지정된 세션이 유효하지 않거나 만료되었을 수 있습니다. | 세션이 완료되는 데 3분 넘게 걸렸습니다. 사용자가 인증 요청을 시작하고 3분 이내에 확인 코드를 입력하는지 또는 앱 알림 인증 요청에 응답하는지 확인합니다. 그래도 문제가 해결 되지 않으면 클라이언트, NAS 서버, NPS 서버 및 Azure AD MFA 끝점 사이에 네트워크 대기 시간이 없는지 확인 합니다.  |
...
[AZURE AD MFA Nps 확장 상태 검사 스크립트](/samples/azure-samples/azure-mfa-nps-extension-health-check/azure-mfa-nps-extension-health-check/) 는 nps 확장 문제를 해결할 때 기본 상태 검사를 수행 합니다. 스크립트를 실행 하 고 옵션 3을 선택 합니다.
