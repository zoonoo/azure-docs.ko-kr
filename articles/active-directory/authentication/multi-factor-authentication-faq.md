...
* [Azure 다단계 인증 서버는 사용자 데이터를 어떻게 처리합니까?](#how-does-azure-multi-factor-authentication-server-handle-user-data)
* [사용자에게 SMS 메시지를 보내는 데 사용되는 SMS 단축 코드는 무엇입니까?](#what-sms-short-codes-are-used-for-sending-sms-messages-to-my-users)
...
캐나다에서는 다음과 같은 SMS 단축 코드를 사용합니다.

* *759731*
* *673801*

동일한 번호로 일관된 SMS 또는 음성 기반 다단계 인증 프롬프트 배달을 보장할 수 없습니다. 당사는 SMS 제공 가능성을 개선하기 위해 경로를 조정할 때 언제든지 단축 코드를 추가하거나 제거할 수 있습니다. 미국 및 캐나다 이외의 국가 또는 지역에 대해서는 단축 코드를 지원하지 않습니다.

## <a name="billing"></a>결제

대부분의 청구 질문은 [다단계 인증 가격 책정 페이지](https://azure.microsoft.com/pricing/details/multi-factor-authentication/) 또는 Azure [다단계 인증 버전 및 소비 계획에](concept-mfa-licensing.md)대한 설명서를 참조하여 답변할 수 있습니다.

* [인증에 사용되는 전화 통화 및 문자 메시지 전송에 대한 요금이 조직에 청구되나요?](#is-my-organization-charged-for-sending-the-phone-calls-and-text-messages-that-are-used-for-authentication)
* [사용자별 청구 모델이 모든 사용 가능한 사용자 또는 2단계 인증을 수행한 사용자에게만 요금이 부과됩니까?](#does-the-per-user-billing-model-charge-me-for-all-enabled-users-or-just-the-ones-that-performed-two-step-verification)
* [다단계 인증 청구는 어떻게 작동합니까?](#how-does-multi-factor-authentication-billing-work)
* [Azure 다단계 인증의 무료 버전이 있습니까?](#is-there-a-free-version-of-azure-multi-factor-authentication)
* [조직에서 언제든지 사용자별 및 인증별 소비 청구 모델 간에 전환할 수 있습니까?](#can-my-organization-switch-between-per-user-and-per-authentication-consumption-billing-models-at-any-time)
* [조직에서 언제든지 소비 기반 청구 및 구독(라이선스 기반 모델) 간에 전환할 수 있습니까?](#can-my-organization-switch-between-consumption-based-billing-and-subscriptions-a-license-based-model-at-any-time)
* [조직에서 Azure 다단계 인증을 사용하려면 ID를 사용하고 동기화해야 합니까?](#does-my-organization-have-to-use-and-synchronize-identities-to-use-azure-multi-factor-authentication)
...
아니요, Azure 다단계 인증을 통해 사용자에게 전송된 개별 전화 통화 또는 문자 메시지에 대해서는 요금이 부과되지 않습니다. 인증별 MFA 공급자를 사용하는 경우 각 인증에 대한 요금이 청구되지만 사용된 방법에 대해서는 청구되지 않습니다.
...
사용자 단위 또는 인증 단위 MFA 공급자를 만들 때 해당 조직의 Azure 구독이 사용량을 기준으로 매월 청구됩니다. 이 청구 모델은 Azure가 가상 컴퓨터 및 웹 앱 사용에 대해 청구하는 방법과 유사합니다.
...
Azure AD 프리 티어에서 보안 기본값을 활성화할 수 있습니다. 보안 기본값을 사용하면 모든 사용자가 Microsoft 인증자 앱을 사용하여 다단계 인증을 사용할 수 있습니다. 보안 기본값으로 문자 메시지 또는 전화 확인을 사용할 수 없으며 Microsoft 인증자 앱만 사용할 수 있습니다.

자세한 내용은 [보안 기본값이란 무엇입니까?](../fundamentals/concept-fundamentals-security-defaults.md)
...
MFA 공급자가 Azure AD 테넌트에 *연결되지 않았거나* 새 MFA 공급자를 다른 Azure AD 테넌트에 연결하는 경우 사용자 설정 및 구성 옵션이 전송되지 않습니다. 또한 새 MFA 공급자를 통해 생성된 활성화 자격 증명을 사용하여 기존 Azure MFA 서버를 다시 활성화해야 합니다. MFA 서버를 새 MFA 공급자에 연결하기 위해 다시 활성화해도 전화 및 문자 메시지 인증에는 영향을 미치지 않지만 모바일 앱을 다시 활성화할 때까지 모바일 앱 알림이 모든 사용자에 대해 작동 중지됩니다.
...
디렉터리에 *인증별* Azure 다단계 인증 공급자가 있는 경우 MFA 공급자가 구독에 연결되어 있는 한 각 인증에 대한 요금이 항상 청구됩니다. 사용자에게 MFA 라이선스를 할당할 수 있지만 사용자의 MFA 라이선스 할당 여부에 관계없이 2단계 인증 요청마다 요금이 계속 청구됩니다.
...
조직에서 사용량 기반 청구 모델을 사용하는 경우 Azure Active Directory는 선택 사항이며 필수가 아닙니다. MFA 공급자가 Azure AD 테넌트에 연결되지 않은 경우 Azure 다단계 인증 서버온-프레미스에만 배포할 수 있습니다.
...
* [휴대폰에서 응답을 받지 못하면 사용자에게 어떻게 해야 하나요?](#what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone)
* [사용자 중 한 명이 계정에 로그인할 수 없는 경우 어떻게 해야 하나요?](#what-should-i-do-if-one-of-my-users-cant-get-in-to-their-account)
* [사용자 중 한 명이 앱 암호를 사용하는 휴대폰을 분실한 경우 어떻게 해야 하나요?](#what-should-i-do-if-one-of-my-users-loses-a-phone-that-is-using-app-passwords)
* [사용자가 비브라우저 앱에 로그인할 수 없는 경우 어떻게 해야 합니까?](#what-if-a-user-cant-sign-in-to-non-browser-apps)
* [내 사용자는 때때로 문자 메시지 또는 확인 시간을 받지 못한다고 말합니다.](#my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out)
* [시스템이 시간 중지되기 전에 사용자가 문자 메시지에서 확인 코드를 입력해야 하는 시간을 변경할 수 있습니까?](#can-i-change-the-amount-of-time-my-users-have-to-enter-the-verification-code-from-a-text-message-before-the-system-times-out)
* [Azure 다단계 인증 서버에서 하드웨어 토큰을 사용할 수 있습니까?](#can-i-use-hardware-tokens-with-azure-multi-factor-authentication-server)
* [Azure 다단계 인증 서버를 사용하여 터미널 서비스를 보호할 수 있습니까?](#can-i-use-azure-multi-factor-authentication-server-to-secure-terminal-services)
* [MFA 서버에서 발신자 ID를 구성했지만 사용자는 여전히 익명 호출자로부터 다단계 인증 호출을 받습니다.](#i-configured-caller-id-in-mfa-server-but-my-users-still-receive-multi-factor-authentication-calls-from-an-anonymous-caller)
* [사용자에게 보안 정보를 등록하라는 메시지가 표시되는 이유는 무엇입니까?](#why-are-my-users-being-prompted-to-register-their-security-information)

### <a name="what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone"></a>휴대폰에서 응답을 받지 못하면 사용자에게 어떻게 해야 하나요?

사용자가 인증을 위해 전화 또는 SMS를 받기 위해 5분 만에 최대 5번 시도하도록 합니다. Microsoft는 여러 공급자를 통해 전화를 걸고 SMS 메시지를 전송합니다. 이 접근 방식이 작동하지 않으면 지원 사례를 열어 문제를 추가로 해결하십시오.

타사 보안 앱은 확인 코드 문자 메시지 또는 전화 통화를 차단할 수도 있습니다. 타사 보안 앱을 사용하는 경우 보호를 사용하지 않도록 설정한 다음 다른 MFA 확인 코드를 보내달라.

위의 단계가 작동하지 않으면 사용자가 둘 이상의 확인 방법에 대해 구성되어 있는지 확인합니다. 다시 로그인을 시도하지만 로그인 페이지에서 다른 확인 방법을 선택합니다.

자세한 내용은 최종 [사용자 문제 해결 가이드를](../user-help/multi-factor-authentication-end-user-troubleshoot.md)참조하십시오.
...
조직에 레거시 클라이언트가 없는 경우 사용자가 앱 암호를 만들도록 허용해서는 안 됩니다.

> [!NOTE]
> **Office 2013 클라이언트에 대한 최신 인증**
>
> 앱 암호는 최신 인증을 지원하는 않는 앱에만 필요합니다. Office 2013 클라이언트는 최신 인증 프로토콜을 지원하지만 구성되어야 합니다. 최신 인증은 Office 2013에 대한 2015년 3월 또는 이후 업데이트를 실행하는 모든 고객에게 제공됩니다. 자세한 내용은 Office [365 최신 인증](https://www.microsoft.com/microsoft-365/blog/2015/11/19/updated-office-365-modern-authentication-public-preview/)업데이트 블로그 게시물을 참조하세요.

### <a name="my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out"></a>내 사용자는 때때로 문자 메시지 또는 확인 시간을 받지 못한다고 말합니다.

서비스의 안정성에 영향을 줄 수 있는 제어할 수 없는 요소가 있기 때문에 SMS 메시지 배달이 보장되지 않습니다. 이러한 요인은 대상 국가 또는 지역, 휴대 전화 캐리어, 및 신호 강도를 포함한다.

타사 보안 앱은 확인 코드 문자 메시지 또는 전화 통화를 차단할 수도 있습니다. 타사 보안 앱을 사용하는 경우 보호를 사용하지 않도록 설정한 다음 다른 MFA 확인 코드를 보내달라.

사용자가 안정적으로 문자 메시지를 받는 데 문제가 있는 경우 대신 Microsoft 인증자 앱 또는 전화 통화 방법을 사용하도록 지시합니다. Microsoft 인증자는 셀룰러 및 Wi-Fi 연결을 통해 알림을 받을 수 있습니다. 또한 디바이스에 신호가 전혀 없는 경우에도 모바일 앱은 인증 코드를 생성할 수 있습니다. 마이크로소프트 인증자 애플 리 케이 션은 [안 드 로이드에](https://go.microsoft.com/fwlink/?Linkid=825072)사용할 수 있습니다., [iOS,](https://go.microsoft.com/fwlink/?Linkid=825073)그리고 [윈도우 폰](https://www.microsoft.com/p/microsoft-authenticator/9nblgggzmcj6).
...
2. *pfsvc_pendingSmsTimeoutSeconds라는* **DWORD** 레지스트리 키를 만들고 Azure MFA Server에서 일회용 암호를 저장할 시간을 초 단위로 설정합니다.
...
클라우드에서 Azure MFA를 사용 하 여 단방향 SMS (AD FS 어댑터 또는 네트워크 정책 서버 확장 포함) 시간 시간 설정을 구성할 수 없습니다. Azure AD는 180초 동안 확인 코드를 저장합니다.
...
Azure 다단계 인증 서버를 사용하는 경우 타사 OPEN 인증(OATH) 시간 기반 토타임 암호(TOTP) 토큰을 가져온 다음 2단계 인증에 사용할 수 있습니다.
...
예. 하지만 Windows Server 2012 R2 이상을 사용하는 경우 원격 데스크톱 게이트웨이(RD 게이트웨이)를 사용하여 터미널 서비스를 보호할 수 있습니다.
...
경우에 따라 Multi-Factor Authentication 호출이 공용 전화망을 통해 이루어진 경우 발신자 번호를 지원하지 않는 통신 회사를 통해 라우팅됩니다. 이 이동통신사 동작으로 인해 다단계 인증 시스템이 항상 전송하더라도 발신자 ID는 보장되지 않습니다.
...
- 사용자가 Azure AD(Azure AD Join 포함)에 장치를 등록하고 있으며, 조직에장치 등록을 위해 MFA가 필요하지만 사용자가 이전에 MFA에 등록하지 않았습니다.
...
* [사용자가 모바일 앱 알림을 사용할 때 "인증 요청이 활성화된 계정에 대한 것이 아님" 오류 메시지가 표시되면 어떻게 해야 합니까?](#what-should-users-do-if-they-see-an-authentication-request-is-not-for-an-activated-account-error-message-when-using-mobile-app-notifications)
* [사용자가 비 브라우저 응용 프로그램에 로그인할 때 0x800434D4D4L 오류 메시지가 표시되면 어떻게 해야 합니까?](#what-should-users-do-if-they-see-a-0x800434d4l-error-message-when-signing-in-to-a-non-browser-application)

### <a name="what-should-users-do-if-they-see-an-authentication-request-is-not-for-an-activated-account-error-message-when-using-mobile-app-notifications"></a>사용자가 모바일 앱 알림을 사용할 때 "인증 요청이 활성화된 계정에 대한 것이 아님" 오류 메시지가 표시되면 어떻게 해야 합니까?

사용자에게 다음 절차를 완료하여 Microsoft 인증자에서 계정을 제거한 다음 다시 추가하도록 요청합니다.

1. Azure [포털 프로필로](https://account.activedirectory.windowsazure.com/profile/) 이동하여 조직 계정으로 로그인합니다.
2. **추가 보안 인증**을 선택합니다.
3. Microsoft 인증자 앱에서 기존 계정을 제거합니다.
4. **구성을**클릭한 다음 지침에 따라 Microsoft 인증기를 다시 구성합니다.
...
*0x800434D4L* 오류는 로컬 컴퓨터에 설치된 비브라우저 응용 프로그램에 로그인하려고 할 때 발생하며 2단계 확인이 필요한 계정에서는 작동하지 않습니다.
...
여기서 질문에 대한 답변이 없는 경우 다음 지원 옵션을 사용할 수 있습니다.

* [Microsoft 지원 기술 자료](https://support.microsoft.com)에서 일반적인 기술 문제에 대한 솔루션을 검색합니다.
* 커뮤니티에서 기술적인 질문과 답변을 검색및 검색하거나 Azure [Active Directory Q&A](https://docs.microsoft.com/answers/topics/azure-active-directory.html).
* [Azure 다단계 인증 서버 지원을](https://support.microsoft.com/oas/default.aspx?prid=14947)통해 Microsoft 전문가에게 문의하십시오. 문의하는 경우 가능한 문제에 대한 많은 정보를 제공해주시면 도움이 됩니다. 오류를 발견한 페이지, 특정 오류 코드, 특정 세션 ID 및 오류를 발견한 사용자의 ID를 포함하는 정보를 제공해 주시면 됩니다.
* 기존 PhoneFactor 고객이고 암호 재설정에 대한 질문이 있거나 도움이 필요한 경우 [phonefactorsupport@microsoft.com](mailto:phonefactorsupport@microsoft.com) 전자 메일 주소를 사용하여 지원 사례를 엽니다.
