---
title: 휴대폰용 Microsoft Authenticator 앱 - Azure AD | Microsoft Docs
...
2. **Authenticator 앱** 옆에 있는 확인란을 선택한 후 **구성**을 선택합니다.
    ...
    ![보안 확인 설정 화면의 구성 단추](./media/microsoft-authenticator-app-how-to/azureauthe.png)

    QR 코드가 있는 화면이 표시됩니다.

    ![QR 코드를 제공하는 화면](./media/microsoft-authenticator-app-how-to/barcode2.png)
...
5. 앱에 그 아래 6자리 코드가 있는 계정 이름이 표시되면 완료된 것입니다.
    ...
    ![계정 화면](./media/microsoft-authenticator-app-how-to/accounts.png)
...
2. **구성**을 선택합니다.
    ...
    ![QR 코드와 URL을 제공하는 화면](./media/microsoft-authenticator-app-how-to/barcode2.png)
3. Microsoft Authenticator 앱을 엽니다. **계정** 화면에서 **+** 를 선택한 다음 회사 또는 학교 계정을 추가할지 지정합니다.

4. 스캐너에서 **수동으로 코드 입력**을 선택합니다.

    ![QR 코드를 스캔하는 화면](./media/microsoft-authenticator-app-how-to/scan2.png)
5. 앱에서 해당 상자에 코드와 URL을 입력한 후 **마침**을 선택합니다.

    ![코드와 URL을 입력하기 위한 화면](./media/microsoft-authenticator-app-how-to/manual.png)

6. 앱에 그 아래 6자리 코드가 있는 계정 이름이 표시되면 완료된 것입니다.

    ![계정 화면](./media/microsoft-authenticator-app-how-to/accounts.png)

### <a name="add-an-account-to-the-app-using-your-devices-fingerprint-or-facial-recognition-capabilities"></a>디바이스의 지문이나 안면 인식 기능을 사용하여 앱에 계정 추가
조직에서는 인증 시도를 완료하기 위해 PIN을 요구할 수 있습니다. Microsoft Authenticator 앱에서는 PIN 대신 디바이스의 지문이나 안면 인식 기능을 사용할 수 있습니다. 앱에서 최초 확인 시 이 기능을 설정하기 위해 Touch ID(iOS) 또는 지문 인식을 대신 사용하는 옵션이 표시됩니다. 

Microsoft Authenticator에 Touch ID를 설정하려면 PIN을 사용하여 일반 확인 시도를 완료해야 합니다. Microsoft Authenticator가 자동으로 Touch ID를 지원하는 디바이스에 대해 해당 기능을 설정합니다. 

![Touch ID 설정 확인](./media/microsoft-authenticator-app-how-to/touchid1.png)

해당 위치 다음부터 로그인 확인이 필요한 경우 받은 푸시 알림을 선택하고 PIN을 입력하는 대신 지문을 스캔합니다.

![푸시 알림](./media/microsoft-authenticator-app-how-to/touchid2.png)
