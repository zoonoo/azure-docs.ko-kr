---
title: IIS 인증 및 Azure MFA 서버 - Azure 활성 디렉터리
...
2. 양식 기반 탭을 **클릭합니다.**
3. **추가**를 클릭합니다.
4. 사용자 이름, 암호 및 도메인 변수를 자동으로 검색하려면 자동 `https://localhost/contoso/auth/login.aspx`구성 양식 기반 웹 사이트 대화 상자 내의 로그인 URL(예: )을 입력하고 **확인을**클릭합니다.
...
8. 올바른 요청 양식을 선택합니다. 대부분의 웹 응용 프로그램에 대해 **POST 또는 GET으로** 설정됩니다.
...
2. **HTTP** 탭을 클릭합니다.
3. **추가**를 클릭합니다.
4. 기본 URL 대화 상자에서 HTTP 인증이 수행되는 웹 사이트의 <http://localhost/owa>URL을 입력하고(예:) 응용 프로그램 이름(선택 사항)을 제공합니다. 애플리케이션 이름이 Azure Multi-Factor Authentication 보고서에 나타나며 SMS 또는 모바일 앱 인증 메시지 내에 표시될 수 있습니다.
...
1. IIS 6에서 실행 중인 경우 **ISAPI** 탭을 클릭합니다. 웹 응용 프로그램이 실행 중인 웹 사이트(예: 기본 웹 사이트)에서 해당 사이트에 대한 Azure 다단계 인증 ISAPI 필터 플러그인을 사용하도록 설정합니다.
2. IIS 7 이상에서 실행되는 경우 **기본 모듈** 탭을 클릭합니다.
3. 화면 상단의 **IIS 인증 활성화** 상자를 클릭합니다. 이제 Azure Multi-Factor Authentication이 선택된 IIS 애플리케이션을 보호합니다. 사용자를 서버에 가져왔는지 확인합니다.
...
4. 허용되는 IP 주소, IP 주소 범위 또는 서브넷을 입력합니다. 서브넷을 입력하는 경우 해당 네트워크 마스크를 선택하고 **확인**을 클릭합니다.
