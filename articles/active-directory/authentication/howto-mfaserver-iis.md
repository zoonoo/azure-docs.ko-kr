...
2. **양식 기반** 탭을 클릭 합니다.
3. **추가**를 클릭합니다.
4. 사용자 이름, 암호 및 도메인 변수를 자동으로 검색 하려면 양식 기반 웹 사이트 `https://localhost/contoso/auth/login.aspx`자동 구성 대화 상자 내에서 로그인 URL (예:)을 입력 하 고 **확인**을 클릭 합니다.
...
8. 올바른 요청 양식을 선택합니다. 대부분의 웹 응용 프로그램에 대해 **POST 또는 GET** 으로 설정 됩니다.
...
2. **HTTP** 탭을 클릭 합니다.
3. **추가**를 클릭합니다.
4. [기준 URL 추가] 대화 상자에서 HTTP 인증이 수행 되는 웹 사이트의 URL (예 <http://localhost/owa>:)을 입력 하 고 응용 프로그램 이름 (선택 사항)을 제공 합니다. 애플리케이션 이름이 Azure Multi-Factor Authentication 보고서에 나타나며 SMS 또는 모바일 앱 인증 메시지 내에 표시될 수 있습니다.
...
1. IIS 6에서 실행 하는 경우 **isapi** 탭을 클릭 합니다. 웹 응용 프로그램이 실행 중인 웹 사이트 (예: 기본 웹 사이트)를 선택 하 여 해당 사이트에 대 한 AZURE Multi-Factor Authentication ISAPI 필터 플러그 인을 사용 하도록 설정 합니다.
2. IIS 7 이상에서 실행 하는 경우 **네이티브 모듈** 탭을 클릭 합니다. 원하는 수준에서 IIS 플러그 인을 사용 하도록 설정 하려면 서버, 웹 사이트 또는 응용 프로그램을 선택 합니다.
3. 화면 위쪽에서 **IIS 인증 사용** 상자를 클릭 합니다. 이제 Azure Multi-Factor Authentication이 선택된 IIS 애플리케이션을 보호합니다. 사용자를 서버에 가져왔는지 확인합니다.
...
2. **추가**를 클릭합니다.
3. 신뢰할 수 있는 ip 추가 대화 상자가 나타나면 **단일 ip**, **Ip 범위**또는 **서브넷** 라디오 단추를 선택 합니다.
4. 허용 해야 하는 ip 주소 또는 서브넷의 범위를 입력 합니다. 서브넷을 입력하는 경우 해당 네트워크 마스크를 선택하고 **확인**을 클릭합니다.
