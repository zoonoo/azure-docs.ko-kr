...
* Azure MFA 서버 업그레이드(하위 서버 다음 주 서버)
* 사용자 포털 인스턴스 업그레이드
...
2. C:\Program Files\Multi-Factor Authentication Server\Data\PhoneFactor.pfdata(기본 설치 위치로 가정)에 있는 MFA 서버 데이터 파일의 백업을 주 MFA 서버에 만듭니다.
...
4. 각 MFA 서버에서 새 설치 관리자를 실행합니다. 하위 서버는 주 서버에 의해 복제되는 이전 데이터 파일을 읽을 수 있으므로 먼저 업그레이드합니다.
...
7. 모든 하위 서버에서 이 단계를 반복합니다. 하위 서버 중 하나를 새 주 서버로 승격한 다음 이전 주 서버를 업그레이드합니다.
...
> [!NOTE]
> Azure MFA Server 8.0 이전 버전에서 8.0 이상 버전으로 업그레이드할 때 업그레이드 후 모바일 앱 웹 서비스를 제거할 수 있습니다
