...
에 있는 IP 주소에서 요청이 들어오면 `IP_WHITELIST` 2 단계 인증을 건너뜁니다. IP 목록은 RADIUS 요청의 *ratNASIPAddress* 특성에 제공 된 ip 주소와 비교 됩니다. RatNASIPAddress 특성 없이 RADIUS 요청이 발생 하는 경우 경고가 기록 됩니다. "IP_WHITE_LIST_WARNING:: IP 허용 목록는 RADIUS 요청 NasIpAddress 특성에 원본 IP가 없으므로 무시 됩니다.
