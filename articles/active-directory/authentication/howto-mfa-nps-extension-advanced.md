...
허용 목록 IP를 구성 하려면로 이동 `HKLM\SOFTWARE\Microsoft\AzureMfa` 및 다음 레지스트리 값을 구성 합니다.
...
에 있는 IP 주소에서 요청 제공 하는 경우는 `IP_WHITELIST`, 2 단계 확인을 건너뜁니다. IP 목록에 제공 된 IP 주소와 비교할 합니다 *ratNASIPAddress* RADIUS 요청의 특성입니다. RADIUS 요청이 ratNASIPAddress 특성 없이 수행되면 다음과 같은 경고가 기록됩니다. "RADIUS 요청의 NasIpAddress 특성에 원본 IP가 없으므로 P_WHITE_LIST_WARNING::IP 인증 허용 목록이 무시됩니다."
