...
|감사 전용 패스 (사용자 이름으로 인해 실패 함)| 10016, 30024| 10017, 30023|

위 표에서 “결합된 정책”을 참조하는 사례는 Microsoft 금지 암호 목록과 고객 금지 암호 목록의 토큰을 하나 이상 포함하는 사용자 암호가 발견된 경우를 가리킵니다.

위의 표에서 "사용자 이름"을 참조 하는 사례는 사용자의 암호가 사용자의 계정 이름 및/또는 사용자의 이름 중 하나를 포함 하는 것으로 확인 된 경우를 참조 합니다. 이러한 시나리오에서는 정책이 적용으로 설정 된 경우 사용자의 암호가 거부 되거나 정책이 감사 모드에 있는 경우에는 통과 됩니다.
