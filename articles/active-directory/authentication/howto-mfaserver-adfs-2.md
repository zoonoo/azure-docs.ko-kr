---
title: AD FS 2.0 - Azure 활성 디렉터리에서 Azure MFA 서버 사용
...
1. Azure 다단계 인증 서버에서 왼쪽 메뉴에서 **IIS 인증** 아이콘을 클릭합니다.
2. 양식 기반 탭을 **클릭합니다.**
3. **추가**를 클릭합니다.

   ![MFA 서버 IIS 인증 창](./media/howto-mfaserver-adfs-2/setup1.png)

4. 사용자 이름, 암호 및 도메인 변수를 자동으로 검색하려면 자동 `https://sso.contoso.com/adfs/ls`구성 양식 기반 웹 사이트 대화 상자 내의 로그인 URL(예: )을 입력하고 **확인을**클릭합니다.
...
7. 양식 기반 웹 사이트 추가 대화 상자에서 URL 제출 필드(예: `https://sso.contoso.com/adfs/ls`)의 AD FS 로그인 페이지의 URL을 입력하고 응용 프로그램 이름(선택 사항)을 입력합니다. 애플리케이션 이름이 Azure Multi-Factor Authentication 보고서에 나타나며 SMS 또는 모바일 앱 인증 메시지 내에 표시될 수 있습니다.
8. 요청 형식을 **게시 또는 GET으로**설정합니다.
...
12. AD FS 프록시 서버는 도메인에 연결될 가능성이 낮으므로 LDAP를 사용하여 사용자 가져오기 및 사전 인증을 위해 도메인 컨트롤러에 연결할 수 있습니다. 고급 양식 기반 웹 사이트 대화 상자에서 **기본 인증** 탭을 클릭하고 사전 인증 유형에 대한 **LDAP 바인딩을** 선택합니다.
...
2. 설정 탭에서 특정 **LDAP 구성** 라디오 사용 단추를 선택합니다.
...
![회사 설정을 구성하는 레지스트리 편집기](./media/howto-mfaserver-adfs-2/reg.png)
...
1. Azure 다단계 인증 서버 내에서 왼쪽 메뉴에서 **IIS 인증** 아이콘을 클릭합니다.
2. **HTTP** 탭을 클릭합니다.
3. **추가**를 클릭합니다.
4. 기본 URL 대화 상자에서 HTTP 인증이 수행되는 `https://sso.domain.com/adfs/ls/auth/integrated`AD FS 웹 사이트의 URL을 기본 URL 필드에 입력합니다. 그런 다음 애플리케이션 이름을 입력합니다(선택 사항). 애플리케이션 이름이 Azure Multi-Factor Authentication 보고서에 나타나며 SMS 또는 모바일 앱 인증 메시지 내에 표시될 수 있습니다.
...
Active Directory에서 서버로 사용자를 가져왔는지 확인합니다. 해당 위치에서 웹 사이트에 로그인할 때 2단계 인증이 필요하지 않도록 내부 IP 주소를 허용하려면 신뢰할 수 있는 IP 섹션을 참조하십시오.
...
2. **추가를 클릭합니다...** 단추를 선택합니다.
3. 신뢰할 수 있는 IP 추가 대화 상자가 나타나면 **단일 IP**, **IP 범위** 또는 **서브넷** 라디오 단추를 선택합니다.
4. 허용되는 IP 주소, IP 주소 범위 또는 서브넷을 입력합니다. 서브넷을 입력하는 경우 적합한 넷마스크를 선택하고 **확인** 단추를 클릭합니다.

![신뢰할 수 있는 IP를 MFA 서버에 구성](./media/howto-mfaserver-adfs-2/trusted.png)
