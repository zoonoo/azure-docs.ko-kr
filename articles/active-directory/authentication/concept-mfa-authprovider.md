...
Azure Multi-factor Authentication 공급자는 정식 버전의 Azure MFA에서 제공하는 기능을 활용하는 데 사용됩니다. **Azure MFA, Azure AD Premium 또는 Azure AD Premium/Azure MFA가 포함된 번들을 통해 라이선스를 갖고 있지 않은** 사용자를 위한 기능입니다. Azure MFA 및 Azure AD Premium은 기본적으로 Azure MFA의 전체 버전을 포함하고 있습니다.
...
현재 Multi-Factor Auth 공급자가 Azure AD 디렉터리(Azure AD 테넌트라고도 함)에 연결된 경우 MFA 공급자를 안전하게 삭제하고 동일한 Azure AD 테넌트에 연결된 새 공급자를 만들 수 있습니다. 또는 MFA가 설정된 모든 사용자를 처리할 만큼 충분한 Azure AD Premium 또는 Azure MFA 라이선스가 포함된 MFA, Azure AD Premium 또는 번들을 구입한 경우 MFA 공급자를 모두 삭제해도 됩니다.
