...
셀프 서비스 암호 재설정 사용자는 암호 재설정 포털에서 사용할 수 있는 "관리자에게 문의" 링크가 있습니다. 사용자가 이 링크를 선택하면 다음 두 가지 중 하나를 수행합니다.

* 기본 상태로 남아 있는 경우:
   * 전자 메일은 관리자에게 전송되며 사용자의 암호를 변경하는 데 도움을 요청합니다. 아래 [샘플 이메일을](#sample-email) 참조하십시오.
* 사용자 지정하는 경우:
   * 관리자가 지정한 웹 페이지 또는 전자 메일 주소로 사용자를 보내 도움을 요청합니다.
...
![관리자에게 보낸 전자 메일 재설정 에 대한 샘플 요청][Contact]
...
* 모든 최종 사용자에게 셀프 서비스 암호 재설정 사용 이 옵션은 **암호 재설정** > **속성**아래에 있습니다. 사용자가 자신의 암호를 재설정할 수 없도록 하려면 빈 그룹에 대한 액세스 범위를 지정하면 됩니다. *이 옵션은 권장하지 않습니다.*
...
**Azure Active 디렉터리** > **속성**에서 디렉터리 이름 특성을 변경할 수 있습니다. 여기에서 포털과 자동 커뮤니케이션에 사용할 조직 이름을 입력할 수 있습니다. 이 옵션은 다음과 같은 자동 메일에 표시됩니다.
...
[Contact]: ./media/concept-sspr-customization/sspr-contact-admin.png "암호 이메일 재설정 예제를 보려면 관리자에게 문의하십시오."
