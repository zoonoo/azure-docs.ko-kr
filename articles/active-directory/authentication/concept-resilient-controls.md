---
title: 탄력적인 액세스 제어 관리 전략 만들기 - Azure AD
...
* 사용자별 MFA가 아닌 조건부 액세스(CA)를 사용하여 MFA를 구현합니다.
* CA(조건부 액세스) 컨트롤을 여러 개 사용하여 사용자 잠금을 완화합니다.
...
* **관리자 잠금:** 관리자는 테넌트 또는 서비스를 관리할 수 없습니다.
* **사용자 잠금:** 사용자는 앱 또는 리소스에 액세스할 수 없습니다.
...
### <a name="mitigating-user-lockout"></a>사용자 잠금 완화
 ...
 사용자 잠금의 위험을 완화하려면 여러 컨트롤이 있는 조건부 액세스 정책을 사용하여 사용자가 앱 및 리소스에 액세스하는 방법을 선택할 수 있습니다. 예를 들어 사용자가 MFA로 로그인 **또는** 관리 디바이스에서 로그인 **또는** 회사 네트워크에서 로그인 중에서 선택할 수 있도록 하여 액세스 제어 중 하나를 사용할 수 없는 경우 계속 작업할 수 있는 다른 옵션이 사용자에게 제공됩니다.

#### <a name="microsoft-recommendations"></a>Microsoft 추천 사항

조직의 기존 조건부 액세스 정책에 다음 액세스 컨트롤을 통합합니다.
...
* 정책 1: 대상 그룹 외부의 사용자에 대한 액세스 차단
  * 사용자 및 그룹: 모든 사용자를 포함합니다. AppUsers, CoreAdmins 및 EmergencyAccess 제외
  * 클라우드 앱: 모든 앱 포함
  * 조건: (없음)
  * 부여 제어: 차단
* 정책 2: MFA 또는 신뢰할 수 있는 장치가 필요한 AppUsers에 대한 액세스 권한을 부여합니다.
  * 사용자 및 그룹: AppUsers포함. CoreAdmins 및 EmergencyAccess 제외
  * 클라우드 앱: 모든 앱 포함
  * 조건: (없음)
  * 권한 부여 제어: 액세스 권한 부여, 다단계 인증 필요, 장치가 호환되어야 합니다. 여러 컨트롤의 경우: 선택한 컨트롤 중 하나를 요구합니다.
...
1. 미션 크리티컬 앱미리 결정: 위험/보안 이력이 낮은 경우에도 액세스 권한을 부여해야 하는 앱은 무엇입니까? 이러한 애플리케이션의 목록을 작성하고, 액세스 제어가 모두 사라지더라도 이러한 애플리케이션이 계속 실행되어야 한다는 것에 대해 다른 모든 이해 관계자(비즈니스, 보안, 법률, 리더십)가 동의해야 합니다. 결과적으로 다음과 같은 범주로 분류될 수 있습니다.
...
긴급 조건부 액세스 정책은 Azure MFA, 타사 MFA, 위험 기반 또는 장치 기반 제어를 생략하는 **비활성화된 정책입니다.** 그런 다음, 조직에서 대응 계획을 활성화하도록 결정하면 관리자는 정책을 사용하도록 설정하고 일반 제어 기반 정책을 사용하지 않도록 설정할 수 있습니다.
...
* 중단 시 정책을 쉽게 찾을 수 있도록 해당 정책의 이름을 지정합니다. 정책 이름에 포함되는 요소는 다음과 같습니다.
  * 정책에 대한 *레이블 번호*
  * 표시할 텍스트 - 이 정책은 응급 상황에만 적용됩니다. 예: **비상 시 사용**
  * 적용되는 *중단*. 예: **MFA 중단 중**
...
다음 예제: **예제 A - 긴급 복구 CA 정책은 미션 크리티컬 한 공동 작업 앱에 대한 액세스를 복원하는**일반적인 회사 비상 사태입니다. 이 시나리오에서 조직은 일반적으로 모든 Exchange Online 및 SharePoint Online 액세스에서 MFA를 요구하며, 이 경우 중단되면 고객에 대한 MFA 공급자(Azure MFA, 온-프레미스 MFA 공급자 또는 타사 MFA)가 중단된 것입니다. 이 정책은 특정 대상 사용자가 신뢰할 수 있는 회사 네트워크에서 해당 애플리케이션에 액세스할 때만 신뢰할 수 있는 Windows 디바이스에서 이러한 애플리케이션에 액세스할 수 있도록 하여 이러한 중단을 완화합니다. 또한 응급 계정과 핵심 관리자도 이러한 제한에서 제외됩니다. 그러면 대상으로 지정된 사용자는 Exchange Online 및 SharePoint Online에 액세스할 수 있지만, 다른 사용자는 중단으로 인해 애플리케이션에 액세스할 수 없습니다. 이 예제에서는 대상 사용자가 있는 **CorpNetwork** 네트워크 위치와 **ContingencyAccess** 보안 그룹, 핵심 관리자가 있는 **CoreAdmins** 그룹 및 응급 액세스 계정이 있는 **EmergencyAccess** 그룹이 필요합니다. 긴급 상황에는 원하는 액세스를 제공하는 4가지 정책이 필요합니다. 

**예제 A - 중요 업무용 협업 앱에 대한 액세스를 복원하는 대응 CA 정책:**

* 정책 1: 교환 및 공유점을 위해 도메인 에 가입된 장치가 필요합니다.
  * 이름: EM001 - 비상 시 사용: MFA 중단[1/4] - Exchange SharePoint - 하이브리드 Azure AD 조인 필요
  * 사용자 및 그룹: 비상 사태 액세스를 포함합니다. CoreAdmins 및 EmergencyAccess 제외
  * 클라우드 앱: 온라인 및 공유점 온라인 교환
  * 조건: 모든
  * 권한 부여 제어: 도메인 가입 필요
  * 상태: 사용 안 함
* 정책 2: Windows 이외의 블록 플랫폼
  * 이름: EM002 - 비상 시 사용: MFA 중단[2/4] - Exchange SharePoint - Windows를 제외한 액세스 차단
  * 사용자 및 그룹: 모든 사용자를 포함합니다. CoreAdmins 및 EmergencyAccess 제외
  * 클라우드 앱: 온라인 및 공유점 온라인 교환
  * 조건: 장치 플랫폼에 모든 플랫폼 포함, Windows 제외
  * 부여 제어: 차단
  * 상태: 사용 안 함
* 정책 3: CorpNetwork 이외의 네트워크 차단
  * 이름: EM003 - 비상 시 사용: MFA 중단[3/4] - Exchange SharePoint - 회사 네트워크를 제외한 액세스 차단
  * 사용자 및 그룹: 모든 사용자를 포함합니다. CoreAdmins 및 EmergencyAccess 제외
  * 클라우드 앱: 온라인 및 공유점 온라인 교환
  * 조건: 위치 포함 모든 위치, CorpNetwork 제외
  * 부여 제어: 차단
  * 상태: 사용 안 함
* 정책 4: EAS 를 명시적으로 차단
  * 이름: EM004 - 비상 시 사용: MFA 중단[4/4] - 교환 - 모든 사용자를 위한 EAS 차단
  * 사용자 및 그룹: 모든 사용자 포함
  * 클라우드 앱: 온라인 교환 포함
  * 조건: 클라이언트 앱: 활성 동기화 교환
  * 부여 제어: 차단
  * 상태: 사용 안 함
...
2. 정책 1 사용: 제외 그룹에 없는 도메인 조인 장치의 사용자가 Exchange Online 및 SharePoint Online에 액세스할 수 있는지 확인합니다. 제외 그룹에 속한 사용자가 모든 디바이스에서 SharePoint Online 및 Exchange에 액세스할 수 있는지 확인합니다.
3. 정책 활성화 2: 제외 그룹에 없는 사용자가 모바일 장치에서 SharePoint Online 및 Exchange Online에 접속할 수 없는지 확인합니다. 제외 그룹에 속한 사용자가 모든 디바이스(Windows/iOS/Android)에서 SharePoint 및 Exchange에 액세스할 수 있는지 확인합니다.
4. 정책 3 사용: 제외 그룹에 없는 사용자가 도메인에 가입한 컴퓨터가 있더라도 회사 네트워크에서 SharePoint 및 Exchange에 액세스할 수 없는지 확인합니다. 제외 그룹에 속한 사용자가 모든 네트워크에서 SharePoint 및 Exchange에 액세스할 수 있는지 확인합니다.
5. 정책 사용 4: 모든 사용자가 모바일 장치의 기본 메일 응용 프로그램에서 Exchange Online을 받을 수 없는지 확인합니다.
...
* 정책 1: 영업 구성 팀에 포함되지 않은 모든 사람을 차단합니다.
  * 이름: EM001 - 비상 시 사용: 장치 규정 준수 중단[1/2] - Salesforce - SalesforceCongncy를 제외한 모든 사용자 차단
  * 사용자 및 그룹: 모든 사용자를 포함합니다. SalesAdmins 및 SalesforceContingency 제외
  * 클라우드 앱: 세일즈포스.
  * 조건: 없음
  * 부여 제어: 차단
  * 상태: 사용 안 함
* 정책 2: 모바일 이외의 플랫폼에서 영업 팀을 차단(공격 의 표면적 을 줄이기 위해)
  * 이름: EM002 - 비상 시 사용: 장치 규정 준수 중단[2/2] - Salesforce - iOS 및 Android를 제외한 모든 플랫폼 차단
  * 사용자 및 그룹: 영업 인력유지를 포함합니다. SalesAdmins 제외
  * 클라우드 앱: 세일즈포스
  * 조건: 장치 플랫폼에 모든 플랫폼 포함, iOS 및 Android 제외
  * 부여 제어: 차단
  * 상태: 사용 안 함
...
2. 정책 1 사용: SalesConency 외부의 사용자가 Salesforce에 액세스할 수 없는지 확인합니다. SalesAdmins 및 SalesforceContingency에 속한 사용자가 Salesforce에 액세스할 수 있는지 확인합니다.
3. 정책 2 사용: SalesContingency 그룹의 사용자가 Windows/Mac 랩톱에서 Salesforce에 액세스할 수 없지만 모바일 장치에서 계속 액세스할 수 있는지 확인합니다. SalesAdmin이 모든 디바이스에서 Salesforce에 계속 액세스할 수 있는지 확인합니다.
...
4. 이 기간 동안 [보고된 모든 위험 탐지를 심사합니다.](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-sign-ins)
...
5. 의심스러운 활동이 중단된 후 [보고된 모든 위험 탐지를](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-sign-ins) 계속 심사합니다.
...
## <a name="emergency-options"></a>응급 옵션
 ...
 응급 상황이 발생하고 조직에서 이전에 완화 또는 긴급 복구 계획을 구현하지 않은 경우 이미 조건부 액세스 정책을 사용하여 MFA를 적용하는 경우 [사용자 잠금](#contingencies-for-user-lockout) 섹션의 권장 사항을 따릅니다.
...
>[!IMPORTANT]
 > 신뢰할 수 있는 IP 주소를 확장하여 액세스를 차단 해제하면 IP 주소(예: 불가능한 이동 또는 익숙하지 않은 위치)와 관련된 위험 검색이 생성되지 않습니다.
...
* [Azure Active Directory에서 명명된 위치 구성](https://docs.microsoft.com/azure/active-directory/reports-monitoring/quickstart-configure-named-locations)
...
* [Azure Active 디렉터리 조건부 액세스의 조건은 무엇입니까?](https://docs.microsoft.com/azure/active-directory/conditional-access/conditions)
* [Azure Active 디렉터리 조건부 액세스의 액세스 컨트롤이란 무엇입니까?](https://docs.microsoft.com/azure/active-directory/conditional-access/controls)
