...
* **Q: 내 사용자가 자신의 암호 재설정 데이터를 등록할 수 있습니까?**
  ...
  > **A:** 예로. 암호 재설정이 사용되고 라이선스가 부여된 경우 사용자는 암호 재설정 등록 포털(https://aka.ms/ssprsetup))로 이동하여 인증 정보를 등록할 수 있습니다. 사용자는 액세스 패널 ( https://myapps.microsoft.com) ) 을 통해 등록할 수도 있습니다. 액세스 패널을 통해 등록하려면 프로필 사진을 선택하고 **프로필**을 선택한 다음 **암호 재설정 등록** 옵션을 사용해야 합니다.
  >
  >
...
* **Q: 내 사용자가 다른 사용자들이 볼 수 없도록 데이터를 등록할 수 있나요?**
  ...
  > **A:** 예로. 사용자가 암호 재설정 등록 포털을 사용하여 데이터를 등록한 경우 데이터가 글로벌 관리자 및 사용자에게만 표시되는 프라이빗 인증 필드에 저장됩니다.
  >
  >
...
* **Q: 암호 재설정에서 지원되는 언어는 무엇입니까?**
  ...
  >
  >
* **Q: 내 디렉터리의 구성 탭에서 조직 브랜드 항목을 설정 하는 경우 암호 재설정의 어느 부분에서 브랜드를 설정 하나요?**

  > **A:** 암호 재설정 포털에는 조직 로고가 표시되며, 사용자 지정 메일 또는 URL을 가리키도록 “관리자에게 문의” 링크를 구성할 수 있습니다. 암호 재설정으로 전송 되는 모든 전자 메일에는 메일 본문에 조직의 로고, 색 및 이름이 포함 되며, 해당 특정 이름에 대 한 설정에서 사용자 지정 됩니다.
  >
  >
* **Q: 암호 재설정으로 이동할 수 있는 위치에 대해 사용자에게 어떻게 교육할 수 있습니까?**

  > **A:** [SSPR 배포](howto-sspr-deployment.md#plan-communications) 문서에서 제안 중 일부를 사용해 보세요.
  >
  >
...
* **Q: 사용자가 암호를 재설정할 때 로컬 Active Directory 계정 잠금 해제가 지원되나요?**
  ...
  > **A:** 예로. 사용자가 암호를 다시 설정 하는 경우 암호 쓰기 저장이 Azure AD Connect를 통해 배포 된 경우 해당 사용자의 계정이 암호를 다시 설정할 때 자동으로 잠금 해제 됩니다.
  >
  >
* **Q: 암호 재설정을 내 사용자의 데스크톱 로그인 환경으로 직접 통합 하려면 어떻게 해야 하나요?**

  > **A:** 사용자가 Azure AD Premium 고객인 경우, 추가 비용 없이 Microsoft Identity Manager를 설치하고 온-프레미스 암호 재설정 솔루션을 배포할 수 있습니다.
  >
  >
* **Q: 로캘에 따라 다른 보안 질문을 설정할 수 있습니까?**
...
* **Q: 사용자는 동일한 본인 확인 질문을 두 번 이상 등록할 수 있나요?**
  ...
  >
  >
* **Q: 등록 및 재설정을 위한 보안 질문의 최소 제한을 설정할 수 있습니까?**
...
* **Q: 내 사용자는 어디서 자신의 암호를 변경해야 합니까?**
  ...
  >
  >
* **Q: 온-프레미스 암호가 만료 되 면 사용자에 게 Office 포털에서 사용자에 게 알릴 수 있나요?**
...
* **Q: 데이터가 암호 관리 보고서에 표시 되는 데 얼마나 걸립니까?**

  > **A:** 데이터는 5~10분 내에 암호 관리 보고서에 표시됩니다. 최대 한 시간이 걸리는 경우도 있습니다.
  >
  >
* **Q: 암호 관리 보고서를 필터링 하려면 어떻게 해야 하나요?**
...
* **Q: 암호 관리 보고서에 최대 몇 행을 표시할 수 있습니까?**
  ...
  > **A:** 예로. UI에 표시되거나 다운로드되었는지 여부와 관계없이 암호 관리 보고서에는 최대 75,000개 행이 표시됩니다.
  >
  >
* **Q: 암호 재설정 또는 등록 보고 데이터에 액세스하는 API가 있습니까?**

  > **A:** 예로. 암호 재설정 보고 데이터에 액세스 하는 방법을 알아보려면 [Azure Log Analytics REST API 참조](/rest/api/loganalytics/)를 참조 하세요.
  >
  >

## <a name="password-writeback"></a>비밀번호 쓰기 저장

* **Q: 비밀 번호 쓰기 저장은 백그라운드에서 어떻게 작동 하나요?**
...
* **Q: 온-프레미스 계정이 온-프레미스 Active Directory 암호 정책에 의해 제한되는 경우 암호를 변경할 때 SSPR이 이 정책을 준수하나요?**
  ...
  >
  >
* **Q: 비밀 번호 쓰기 저장이 작동 하는 계정 유형은 무엇입니까?**

  > **A:** 페더레이션되고 암호 해시 동기화된 통과 인증 사용자를 비롯하여 온-프레미스 Active Directory에서 Azure AD로 동기화되는 사용자 계정에 대한 비밀번호 쓰기 저장이 작동합니다.
  >
  >
* **Q: 비밀 번호 쓰기 저장은 내 도메인의 암호 정책을 적용 하나요?**

  > **A:** 예로. 비밀번호 쓰기 저장은 암호 사용 기간, 기록, 복잡성, 필터 및 관리자가 로컬 도메인에서 암호에 적용한 그 밖의 제한 사항을 이행합니다.
  >
  >
