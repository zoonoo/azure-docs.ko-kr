...
description: 조직에서 등록 하 고 암호 재설정을 수행 하는 데 사용 하는 인증 방법에 대 한 개요입니다.
...
**Azure Multi-Factor Authentication 사용할 수** 있는 사용자, 암호 없는 **인증** 을 지 원하는 사용자 또는 **셀프 서비스 암호 재설정을 수행할** 수 있는 사용자 또는 사용자 등록 정보 목록을 미리 필터링 할 수 있는 사용자를 클릭 합니다.

- **Azure Multi-Factor Authentication 사용할 수 있는 사용자** 는 azure AD에서 MFA를 사용할 수 있는 사용자의 분석을 보여 줍니다. 사용자가 강력한 인증 방법에 대해 등록 되 고 정책에서 MFA를 수행 하는 데 사용할 수 있도록 설정 된 경우 사용자가 사용할 수 있는 것으로 간주 됩니다. 이 수는 Azure AD 외부에서 MFA에 등록 된 사용자를 반영 하지 않습니다. 
- 암호 없는 **인증을 사용할 수 있는 사용자** 는 암호 없이 로그인 할 수 있는 사용자의 분석을 보여 줍니다. 여기에는 FIDO2에 등록 된 사용자, 비즈니스용 Windows Hello 및 Microsoft Authenticator 앱을 사용 하 여 암호 없는 휴대폰 로그인이 포함 됩니다. 
- **셀프 서비스 암호 재설정을 수행할 수 있는 사용자** 는 셀프 서비스 암호 재설정을 수행할 수 있는 사용자의 분석을 보여 줍니다. 사용자는 조직의 SSPR 정책을 충족 하 고 SSPR를 수행할 수 있는 충분 한 방법으로 등록 된 경우 SSPR 수 있는 것으로 간주 됩니다. 
...
**인증 방법에의 한 최근 등록 방법** 인증 방법에 따라 성공한 인증 방법 등록 수를 표시 합니다. 인증 방법을 클릭 하 여 해당 메서드에 대 한 최근 등록 이벤트를 확인 합니다.

![최근에 등록 된 스크린샷](media/how-to-authentication-methods-usage-insights/recently-registered.png)

## <a name="usage-details"></a>사용량 세부 정보

**사용 현황** 보고서에는 사용자가 로그인 하 고 암호를 재설정 하는 데 사용 하는 인증 방법이 표시 됩니다.

![사용량 페이지의 스크린샷](media/how-to-authentication-methods-usage-insights/usage-page.png)

**인증 요구 사항** 에의 한 로그인은 Azure AD에서 multi-factor authentication 및 multi-factor authentication을 수행 하는 데 필요한 성공적인 사용자 대화형 로그인 수를 표시 합니다. 이는 MFA가 타사 MFA 공급자에 의해 적용 된 로그인을 반영 하지 않습니다.

![인증 요구 사항에의 한 로그인의 스크린샷](media/how-to-authentication-methods-usage-insights/sign-ins-protected.png)

**인증 방법으로 로그인** 은 사용한 인증 방법에 따라 사용자 대화형 로그인 (성공 및 실패)의 수를 표시 합니다. 토큰의 클레임에 의해 인증 요구 사항이 충족 된 로그인은 포함 되지 않습니다.
...
임시 액세스 통과 (탭) 등록은 짧은 시간 동안만 유효 하므로 보고서의 등록 탭에 반영 되지 않습니다.
