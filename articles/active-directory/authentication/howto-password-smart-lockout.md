...
스마트 잠금은 사용자의 암호를 추측하거나 무차별 대입 방법을 사용하여 입력하려는 악의적인 행위자가 잠그는 데 도움을 줍니다. 이 기능은 유효한 사용자로부터 오는 로그인을 인식하고 이를 공격자 및 기타 알 수 없는 원본 중 하나와 다르게 취급합니다. 스마트 잠금은 사용자가 계정에 계속 액세스하여 생산성을 유지할 수 있게 해주면서도 공격자를 차단합니다.
...
스마트 잠금을 사용한다고 해서 정품 사용자가 절대로 잠기지 않는다는 보장은 없습니다. 스마트 잠금이 사용자 계정을 잠글 때, 우리는 진짜 사용자를 잠그지 않도록 최선을 다합니다. 잠금 서비스는 악의적인 사용자가 정품 사용자 계정에 액세스할 수 없도록 하려고 합니다.  
