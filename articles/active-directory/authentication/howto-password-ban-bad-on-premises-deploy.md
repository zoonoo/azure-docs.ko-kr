...
description: Azure AD 암호 보호 미리 보기를 배포하여 온-프레미스에서 잘못된 암호를 사용할 수 없도록 금지
...
이제 [Windows Server Active Directory에 Azure AD 암호 보호를 적용하는 방법](concept-password-ban-bad-on-premises.md)을 알아보았으므로 다음 단계는 배포를 계획하고 실행하는 것입니다.
...
* Azure AD 암호 보호 프록시 서비스가 설치될 모든 머신에서 Windows Server 2012 R2 이상을 실행해야 합니다.
* 도메인 컨트롤러를 포함하여 Azure AD 암호 보호 구성 요소가 설치된 모든 머신에 Universal C 런타임이 설치되어야 합니다.
...
* 각 도메인의 하나 이상의 도메인 컨트롤러와 Azure AD 암호 보호 프록시 서비스를 호스트하는 하나 이상의 서버 간에 네트워크 연결이 있어야 합니다. 이 연결을 통해 도메인 컨트롤러가 프록시 서비스의 RPC 엔드포인트 매퍼 포트(135) 및 RPC 서버 포트에 액세스할 수 있어야 합니다. RPC 서버 포트는 기본적으로 동적 RPC 포트이지만 정적 포트를 사용하도록 구성될 수 있습니다(아래 참조).
* Azure AD 암호 보호 프록시 서비스를 호스트하는 모든 머신에 다음 엔드포인트에 대한 네트워크 액세스 권한이 있어야 합니다.
...
1. Azure AD 암호 보호 프록시 서비스를 호스트할 서버를 하나 이상 선택합니다.
   * 이러한 각 서비스는 단일 포리스트에 대한 암호 정책만 제공할 수 있으며, 호스트 머신은 해당 포리스트의 도메인(루트 및 자식 도메인이 둘 다 지원됨)에 가입되어야 합니다. Azure AD 암호 보호 프록시 서비스에서 임무를 수행하려면 포리스트의 각 도메인에 있는 하나 이상의 DC와 Azure AD 암호 보호 프록시 머신 간에 네트워크 연결이 있어야 합니다.
   * 테스트를 위해 Azure AD 암호 보호 프록시 서비스를 도메인 컨트롤러에 설치하고 실행할 수 있습니다. 단점은 도메인 컨트롤러에 인터넷 연결이 필요하므로 보안 문제가 발생할 수 있다는 것입니다. 이 구성은 테스트 목적으로만 사용하는 것이 좋습니다.
   * 중복성을 위해 최소한 두 개의 프록시 서버를 사용하는 것이 좋습니다. [고가용성 참조](howto-password-ban-bad-on-premises-deploy.md#high-availability)

2. AzureADPasswordProtectionProxySetup.msi MSI 패키지를 사용하여 Azure AD 암호 보호 프록시 서비스를 설치합니다.
   * 소프트웨어가 설치되면 다시 부팅할 필요가 없습니다. 소프트웨어 설치는 표준 MSI 절차(예: `msiexec.exe /i AzureADPasswordProtectionProxySetup.msi /quiet /qn`)를 사용하여 자동화할 수 있습니다.

      > [!NOTE]
      > AzureADPasswordProtectionProxySetup.msi MSI 패키지를 설치하기 전에 Windows 방화벽 서비스가 실행 중이어야 합니다. 그렇지 않으면 설치 오류가 발생합니다. Windows 방화벽이 실행되지 않도록 구성된 경우 해결 방법은 설치 프로세스 중에 Windows 방화벽 서비스를 일시적으로 사용하도록 설정하고 시작하는 것입니다. 프록시 소프트웨어는 설치 후 Windows 방화벽 소프트웨어에 특별히 의존하지 않습니다. 타사 방화벽을 사용 중인 경우에도 배포 요구 사항에 맞게 구성되어 있어야 합니다(포트 135 및 프록시 RPC 서버 포트에 대한 동적 또는 정적 인바운드 액세스 허용). [배포 요구 사항 참조](howto-password-ban-bad-on-premises-deploy.md#deployment-requirements)
...
4. 프록시를 등록합니다.
   * 3단계가 완료되면 Azure AD 암호 보호 프록시 서비스가 머신에서 실행 중이지만 Azure AD와 통신하는 데 필요한 자격 증명이 아직 없습니다. `Register-AzureADPasswordProtectionProxy` PowerShell cmdlet을 사용하여 해당 기능을 사용하도록 설정하려면 Azure AD에 등록해야 합니다. 이 cmdlet을 사용하려면 Azure 테넌트에 대한 전역 관리자 자격 증명과 포리스트 루트 도메인의 온-프레미스 Active Directory 도메인 관리자 권한이 필요합니다. 일단 지정된 프록시 서비스에 대해 성공하면 `Register-AzureADPasswordProtectionProxy`의 추가 호출이 계속 성공하지만 필요한 것은 아닙니다.
      ...
      * 대화형 인증 모드:
         ...
         ```
         > [!NOTE]
         > 이 모드는 Server Core 운영 체제에서 작동하지 않습니다. 대신, 아래에 설명된 대체 인증 모드 중 하나를 사용합니다.

         > [!NOTE]
         > Internet Explorer 보안 강화 구성이 사용되는 경우 이 모드는 실패할 수 있습니다. 해결 방법은 IESC를 사용하지 않고, 프록시를 다시 등록한 다음, IESC를 다시 사용하는 것입니다.
      ...
      * 디바이스 코드 인증 모드:
         ...
         그런 다음, 다른 디바이스에서 표시된 지침에 따라 인증을 완료할 수 있습니다.
      ...
      * 자동(암호 기반) 인증 모드:
         ...
         > [!NOTE]
         > 어떤 이유로든 인증에 MFA가 필요한 경우 이 모드는 실패합니다. 이 경우 이전 두 모드 중 하나를 사용하여 MFA 기반 인증을 수행합니다.

      이후 기능을 위해 예약된 -ForestCredential 매개 변수는 현재 지정할 필요가 없습니다.

   > [!NOTE]
   > Azure AD 암호 보호 프록시 서비스의 등록은 서비스 수명 중의 일회성 단계로 요구됩니다. 프록시 서비스는 이 시점부터 다른 필요한 유지 관리 기능을 자동으로 수행합니다. 지정된 프록시에 대해 성공하면 ‘Register-AzureADPasswordProtectionProxy’의 추가 호출이 계속 성공하지만 필요한 것은 아닙니다.
...
5. 포리스트를 등록합니다.
   * 온-프레미스 Active Directory 포리스트는 `Register-AzureADPasswordProtectionForest` PowerShell cmdlet을 사용하여 Azure와 통신하는 데 필요한 자격 증명으로 초기화해야 합니다. 이 cmdlet을 사용하려면 Azure 테넌트에 대한 전역 관리자 자격 증명과 포리스트 루트 도메인의 온-프레미스 Active Directory 도메인 관리자 권한이 필요합니다. 이 단계는 포리스트마다 한 번씩 실행됩니다.
      ...
      Register-AzureADPasswordProtectionForest cmdlet은 다음과 같은 세 가지 인증 모드를 지원합니다.
      ...
      * 자동(암호 기반) 인증 모드:
         ...
         > [!NOTE]
         > 인증에 MFA가 필요한 경우 이 모드는 실패합니다. 이 경우 이전 두 모드 중 하나를 사용하여 MFA 기반 인증을 수행합니다.

      위의 예제는 현재 로그인한 사용자가 루트 도메인에 대한 Active Directory 도메인 관리자이기도 한 경우에만 성공합니다. 이 경우가 아니면 -ForestCredential 매개 변수를 통해 대체 도메인 자격 증명을 제공할 수 있습니다.

   > [!NOTE]
   > 사용자 환경에 여러 프록시 서버가 설치되어 있는 경우, 어떤 프록시 서버를 사용하여 포리스트를 등록하는지는 중요하지 않습니다.
...
6. HTTP 프록시를 통해 통신하도록 Azure AD 암호 보호 프록시 서비스를 구성합니다.

   사용자 환경에서 특정 HTTP 프록시를 사용하여 Azure와 통신해야 하는 경우 다음과 같이 수행할 수 있습니다.

   다음과 같은 콘텐츠가 포함된 `proxyservice.exe.config` 파일을 `%ProgramFiles%\Azure AD Password Protection Proxy\Service` 폴더에 만듭니다.
   ...
   HTTP 프록시에 인증이 필요한 경우 다음과 같이 useDefaultCredentials 태그를 추가합니다.
   ...
   두 경우 모두, `http://yourhttpproxy.com:8080`을 특정 HTTP 프록시 서버의 주소 및 포트로 바꿉니다.

   HTTP 프록시가 권한 부여 정책으로 구성된 경우 Azure AD 암호 보호 프록시 서비스를 호스트하는 머신의 Active Directory 컴퓨터 계정에 액세스 권한을 부여해야 합니다.

   `proxyservice.exe.config` 파일을 만들거나 업데이트한 후 Azure AD 암호 보호 프록시 서비스를 중지하고 다시 시작해야 합니다.

   Azure AD 암호 보호 프록시 서비스에서는 특정 자격 증명을 사용하여 HTTP 프록시에 연결할 수 없습니다.

7. 선택 사항: 특정 포트에서 수신 대기하도록 Azure AD 암호 보호 프록시 서비스를 구성합니다.
   * TCP를 통한 RPC는 도메인 컨트롤러의 Azure AD 암호 보호 DC 에이전트 소프트웨어에서 Azure AD 암호 보호 프록시 서비스와 통신하는 데 사용됩니다. 기본적으로 Azure AD 암호 보호 프록시 서비스는 사용 가능한 모든 동적 RPC 엔드포인트에서 수신 대기합니다. 네트워킹 토폴로지 또는 방화벽 요구 사항에 따라 필요한 경우 특정 TCP 포트에서 수신 대기하도록 서비스를 대신 구성할 수 있습니다.
...
### <a name="install-the-azure-ad-password-protection-dc-agent-service"></a>Azure AD 암호 보호 DC 에이전트 서비스 설치
   ...
   소프트웨어 설치는 표준 MSI 절차를 사용하여 자동화할 수 있습니다. 예를 들면 다음과 같습니다.

   `msiexec.exe /i AzureADPasswordProtectionDCAgent.msi /quiet /qn`

   > [!WARNING]
   > 위의 msiexec 명령 예제를 사용하면 즉시 다시 부팅됩니다. `/norestart` 플래그를 지정하여 이를 방지할 수 있습니다.
...
Azure AD 암호 보호를 여러 포리스트에 배포하기 위한 추가 요구 사항은 없습니다. 각 포리스트는 단일 포리스트 배포 섹션에서 설명한 대로 독립적으로 구성됩니다. Azure AD 암호 보호 프록시는 해당 프록시가 연결된 포리스트의 도메인 컨트롤러만 지원할 수 있습니다. 지정된 포리스트의 Azure AD 암호 보호 소프트웨어는 Active Directory 신뢰 구성과 관계없이 다른 포리스트에 배포된 Azure AD 암호 보호 소프트웨어를 인식하지 못합니다.
...
Azure AD 암호 보호의 고가용성 보장과 관련된 주요 문제는 포리스트의 도메인 컨트롤러가 Azure에서 새 정책 또는 다른 데이터를 다운로드하려고 할 때 프록시 서버를 사용할 수 있는 가용성에 대한 것입니다. 각 DC 에이전트에서 호출할 프록시 서버를 결정할 때 간단한 라운드 로빈 방식 알고리즘을 사용하고, 응답하지 않는 프록시 서버는 건너뜁니다. 정상적인 복제(디렉터리 및 sysvol 상태 둘 다)를 사용하는 완전히 연결된 Active Directory 배포의 경우 대체로 두 개의 프록시 서버만 있으면 가용성을 제공할 수 있으므로 새 정책 및 기타 데이터의 적시 다운로드가 가능합니다. 추가 프록시 서버를 원하는 대로 배포할 수 있습니다.
