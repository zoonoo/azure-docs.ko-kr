...
* Azure AD 암호 보호 DC 에이전트 서비스는 DC 에이전트 암호 필터 dll에서 암호 유효성 검사 요청을 받아서, 이 요청을 로컬에서 현재 사용 가능한 암호 정책을 사용하여 처리하고, 결과(pass\fail)를 반환합니다. 이 서비스는 새로운 버전의 암호 정책을 검색하기 위해 Azure AD 암호 보호 프록시 서비스를 주기적으로(한 시간에 한 번) 호출하는 작업을 담당합니다. Azure AD 암호 보호 DC 에이전트 서비스와 Azure AD 암호 보호 프록시 서비스 간의 통신은 TCP를 통한 RPC(원격 프로시저 호출)를 사용하여 처리됩니다. 검색이 되면 새 정책은 sysvol 폴더에 저장되며 이 곳에서 다른 도메인 컨트롤러에 복제할 수 있습니다. 또한 DC 에이전트 서비스는 다른 도메인 컨트롤러에서 새 암호 정책을 작성한 경우 sysvol 폴더에 대한 변경을 모니터링하며, 적절한 최신 정책을 이미 사용할 수 있으면 새 정책 다운로드 요청을 건너뛸 수 있습니다.
