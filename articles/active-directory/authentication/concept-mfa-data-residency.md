---
title: Azure AD Multi-Factor Authentication 데이터 상주
description: Azure AD Multi-Factor Authentication 개인 및 조직 데이터에 대해 알아보고, 사용자와 사용자 및 해당 국가/지역에 남아 있는 데이터에 대해 알아봅니다.
...
클라우드 기반 Azure AD Multi-Factor Authentication 및 Azure Multi-Factor Authentication 서버는 몇 가지 개인 데이터와 조직 데이터를 처리 하 고 저장 합니다. 이 문서에서는 저장되는 데이터 및 위치를 간략하게 설명합니다.

Azure AD Multi-Factor Authentication 서비스에는 미국, 유럽 및 아시아 태평양의 데이터 센터가 있습니다. 다음 활동은 명시 된 경우를 제외 하 고는 지역 데이터 센터에서 제외 됩니다.
...
## <a name="personal-data-stored-by-azure-ad-multi-factor-authentication"></a>Azure AD Multi-Factor Authentication에 저장 된 개인 데이터
...
Azure AD Multi-Factor Authentication는 사용자 이름, 전화 번호 또는 IP 주소와 같은 개인 데이터를 기록 하지 않지만 사용자에 대 한 Multi-Factor Authentication 시도를 식별 하는 *Userobjectid* 는 있습니다. 로그 데이터는 30일 동안 저장됩니다.
...
> [!IMPORTANT]
> 2019년 7월 1일부터 Microsoft는 더 이상 새 배포를 위한 Multi-Factor Authentication 서버를 제공하지 않습니다. 사용자에 게 multi-factor authentication을 요구 하려는 새 고객은 클라우드 기반 Azure AD Multi-Factor Authentication를 사용 해야 합니다. 7월 1일 이전에 Multi-Factor Authentication 서버를 활성화한 기존 고객은 종전과 같이 최신 버전 및 이후 업데이트를 다운로드하고 활성화 자격 증명을 생성할 수 있습니다.
...
## <a name="organizational-data-stored-by-azure-ad-multi-factor-authentication"></a>Azure AD Multi-Factor Authentication에서 저장 한 조직 데이터
...
로그 정보가 저장되는 위치는 처리되는 지역에 따라 다릅니다. 대부분의 지역에는 네이티브 Azure AD Multi-Factor Authentication 기능이 있으므로 로그 데이터는 Multi-Factor Authentication 요청을 처리 하는 동일한 지역에 저장 됩니다. 네이티브 Azure AD Multi-Factor Authentication 지원 하지 않는 지역에서 이러한 서비스는 미국 또는 유럽 지역에서 서비스 되며, 로그 데이터는 Multi-Factor Authentication 요청을 처리 하는 동일한 지역에 저장 됩니다.
...
클라우드 기반 Azure AD Multi-Factor Authentication 및 Azure Multi-Factor Authentication 서버에서 수집 하는 사용자 정보에 대 한 자세한 내용은 [AZURE ad Multi-Factor Authentication 사용자 데이터 컬렉션](howto-mfa-reporting-datacollection.md)을 참조 하세요.
