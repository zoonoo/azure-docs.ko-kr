---
title: Azure MFA - Azure Active Directory에 대한 액세스 및 사용 보고서
...
| 무시된 사용자 기록 | Azure AD > 보안 > MFA > 일회성 우회 | 사용자의 Multi-Factor Authentication을 바이패스하는 요청 기록을 제공합니다. |
...
3. 보려는 보고서를 선택합니다.
   ...
   ![Azure 포털의 MFA 서버 서버 상태 보고서](./media/howto-mfa-reporting/report.png)
...
이 데이터는 Azure [포털](https://portal.azure.com) 및 [보고 API를](../reports-monitoring/concept-reporting-api.md)통해 사용할 수 있습니다.

![Azure 포털의 Azure AD 로그인 보고서](./media/howto-mfa-reporting/sign-in-report.png)

### <a name="sign-ins-report-structure"></a>로그인 보고서 구조

MFA의 로그인 활동 보고서를 통해 다음 정보에 액세스 할 수 있습니다.

**MFA 필요:** 로그인에 MFA가 필요한지 여부. 사용자별 MFA, 조건부 액세스 또는 기타 이유로 인해 MFA가 필요할 수 있습니다. 가능한 값은 **예** 또는 **아니요**입니다.
...
**조건부 액세스** 다음을 포함하여 로그인 시도에 영향을 주는 조건부 액세스 정책에 대한 정보를 찾습니다.
...
먼저 [MSOnline V1 PowerShell 모듈이](https://docs.microsoft.com/powershell/azure/active-directory/overview?view=azureadps-1.0) 설치되어 있는지 확인합니다.

다음에 나오는 PowerShell을 사용하여 MFA에 등록한 사용자를 식별합니다. 이 명령 집합은 이러한 계정이 Azure AD에 대해 인증할 수 없으므로 비활성화된 사용자를 제외합니다.
...
다음에 나오는 PowerShell을 사용하여 MFA에 등록하지 않은 사용자를 식별합니다. 이 명령 집합은 이러한 계정이 Azure AD에 대해 인증할 수 없으므로 비활성화된 사용자를 제외합니다.
...
## <a name="possible-results-in-activity-reports"></a>활동 보고서의 가능한 결과

다음 표는 다운로드한 버전의 다단계 인증 활동 보고서를 사용하여 다단계 인증 문제를 해결하는 데 사용할 수 있습니다. Azure 포털에 직접 나타나지 않습니다.

| 통화 결과 | 설명 | 광범위한 설명 |
| --- | --- | --- |
| SUCCESS_WITH_PIN | PIN이 입력됨 | 사용자가 PIN을 입력했습니다.  인증이 성공하면 사용자가 올바른 PIN을 입력한 것이며, 인증이 거부되면 잘못된 PIN을 입력했거나 사용자가 표준 모드로 설정된 것입니다. |
| SUCCESS_NO_PIN | 만 # 입력 | PIN 모드로 설정된 사용자의 인증이 거부되는 경우 사용자가 PIN을 입력하지 않고 #만 입력했음을 의미합니다.   표준 모드로 설정된 사용자가 인증에 성공하면 사용자가 표준 모드에서는 올바른 동작인 #만 입력했음을 의미합니다. |
...
| SUCCESS_USED_CACHE | 사용된 캐시 | 구성된 캐시 기간 내에 동일한 사용자 이름에 대한 이전 인증이 성공적으로 발생했기 때문에 다단계 인증 호출 없이 인증이 성공했습니다. |
| SUCCESS_BYPASSED_AUTH | 바이패스된 인증 | 사용자가 시작한 원타임 바이패스를 사용하여 인증에 성공했습니다.  바이패스에 대한 자세한 내용은 바이패스된 사용자 기록 보고서를 참조하십시오. |
| SUCCESS_USED_IP_BASED_CACHE | 사용된 IP 기반 캐시 | 구성된 캐시 기간 내에 동일한 사용자 이름, 인증 유형, 응용 프로그램 이름 및 IP에 대한 이전 인증이 성공적으로 인증되었기 때문에 다단계 인증 호출 없이 인증이 성공했습니다. |
| SUCCESS_USED_APP_BASED_CACHE | 사용된 앱 기반 캐시 | 구성된 캐시 기간 내에 동일한 사용자 이름, 인증 유형 및 응용 프로그램 이름에 대한 이전 인증이 성공적으로 성공한 이후 다단계 인증 호출 없이 인증이 성공했습니다. |
| SUCCESS_INVALID_INPUT | 잘못된 폰 입력 | 폰에서 보낸 응답이 올바르지 않습니다. 팩스 기기 나 모뎀에서 온 것일 수도 있고 사용자가 PIN의 일부로 *를 입력했을 수 있습니다. |
| SUCCESS_USER_BLOCKED | 사용자가 차단됨 | 사용자의 전화 번호가 차단됩니다. 차단된 번호는 인증 호출 중에 사용자가 시작하거나 Azure 포털을 사용하는 관리자가 시작할 수 있습니다. <br> 참고: 차단된 번호는 사기 경고의 부산물이기도 합니다. |
| SUCCESS_SMS_AUTHENTICATED | 문자 메시지 인증됨 | 양방향 문자 메시지의 경우 사용자가 OTP(일회성 암호) + PIN을 포함하여 올바로 회신했습니다. |
| SUCCESS_SMS_SENT | 문자 메시지 전송됨 | 문자 메시지의 경우 OTP(일회성 암호)가 포함된 문자 메시지가 성공적으로 전송되었습니다. 사용자는 인증을 완료하기 위해 응용 프로그램에 OTP 또는 OTP + PIN을 입력합니다. |
...
| FAILED_PHONE_BUSY | 이미 진행 중인 인증 | Multi-Factor Authentication에서 해당 사용자에 대한 인증을 처리하고 있습니다. 이는 종종 동일한 사인온 중에 여러 인증 요청을 보내는 RADIUS 클라이언트에서 발생합니다. |
| CONFIG_ISSUE | 폰에 연결할 수 없음 | 전화를 시도했지만 배치할 수 없거나 응답되지 않았습니다. 여기에는 통화 중 신호, 빠른 통화 신호(연결이 끊긴), 트라이 톤(더 이상 서비스 중 이없음), 울리는 동안 시간 중지 등이 포함됩니다. |
| FAILED_INVALID_PHONENUMBER | 잘못된 전화 번호 형식 | 전화 번호 형식이 잘못되었습니다. 전화 번호는 숫자여야 하며 국가 코드 +1(캐나다 & 미국)의 경우 10자리여야 합니다. |
...
| FAILED_SERVER_ERROR | 전화 걸 수 없음 | 다단계 인증 서비스가 호출을 배치할 수 없습니다. |
| FAILED_SMS_NOT_SENT | 문자 메시지를 보낼 수 없음 | 문자 메시지를 보낼 수 없습니다.  인증이 거부됩니다. |
| FAILED_SMS_OTP_INCORRECT | 문자 메시지의 OTP가 잘못됨 | 사용자가 받은 문자 메시지에서 잘못된 일회용 암호(OTP)를 입력했습니다.  인증이 거부됩니다. |
| FAILED_SMS_OTP_PIN_INCORRECT | 문자 메시지의 OTP + PIN이 잘못됨 | 사용자가 잘못된 일회용 암호(OTP) 및/또는 잘못된 사용자 PIN을 입력했습니다.  인증이 거부됩니다. |
| FAILED_SMS_MAX_OTP_RETRY_REACHED | 최대 문자 메시지 OTP 시도 초과 | 사용자가 OTP(일회용 암호) 시도의 최대 횟수를 초과했습니다. |
...
| FAILED_OATH_CODE_PIN_INCORRECT | 선서 코드 + PIN 올바르지 않습니다. | 사용자가 잘못된 OATH 코드 및/또는 잘못된 사용자 PIN을 입력했습니다.   인증이 거부됩니다. |
...
| FAILED_OATH_TOKEN_TIMEOUT | OATH 코드 결과 시간 시간 | 사용자가 OATH 코드를 입력하는 데 시간이 너무 오래 걸렸고 다단계 인증 시도가 이미 시간 만료되었습니다. |
| FAILED_SECURITY_QUESTIONS_TIMEOUT | 보안 질문 결과 시간 시간 | 사용자가 보안 질문에 대한 답변을 입력하는 데 시간이 너무 오래 걸렸으며 다단계 인증 시도가 이미 시간 만료되었습니다. |
| FAILED_AUTH_RESULT_TIMEOUT | 인증 결과 시간 시간 | 사용자가 다단계 인증 시도를 완료하는 데 시간이 너무 오래 걸렸습니다. |
| FAILED_AUTHENTICATION_THROTTLED | 인증 제한 | 다단계 인증 시도는 서비스에 의해 제한되었습니다. |
