---
title: Passwordless 보안 키 온-프레미스 리소스에 로그인 (미리 보기)-Azure Active Directory
description: 온-프레미스 리소스에 대해 암호 없는 보안 키 로그인 사용 (미리 보기)
...
ms.locfileid: "73164862"
---
# <a name="enable-passwordless-security-key-sign-in-to-on-premises-resources-preview"></a>온-프레미스 리소스에 대해 암호 없는 보안 키 로그인 사용 (미리 보기)
...
온-프레미스 Active Directory에 Azure AD Kerberos 서버 개체가 만들어지고 Azure Active Directory에 안전 하 게 게시 됩니다. 개체가 실제 서버와 연결 되어 있지 않습니다. Azure Active Directory에서 Active Directory 도메인에 대 한 Kerberos 티켓 부여 티켓 (Tgt)을 생성 하는 데 사용할 수 있는 리소스입니다.

![Azure AD에서 TGT 및 PRT 가져오기 및 AD DS](./media/howto-authentication-passwordless-on-premises/fido2-tgt-exchange-process.png)
...
이 문서의 단계를 완료 하기 전에 조직에서 [Windows 10 장치에 대 한 암호 없는 보안 키 서명 사용 (미리 보기)](howto-authentication-passwordless-security-key.md) 문서의 단계를 완료 해야 합니다.

조직은 다음과 같은 소프트웨어 요구 사항을 충족 해야 합니다.

- 장치에서 Windows 10 Insider Build 18945 이상을 실행 해야 합니다.
- 최신 버전의 [Azure AD Connect](../hybrid/how-to-connect-install-roadmap.md#install-azure-ad-connect) 로 업그레이드
- 온-프레미스 인증 요청 부하를 처리 하는 Windows Server 2016/2019 도메인 컨트롤러를 완전히 패치 했습니다.

### <a name="supported-scenarios"></a>지원되는 시나리오

- 시나리오는 두 가지 모두에 대해 SSO (Single Sign-On)를 지원 합니다.
   - 클라우드 리소스 (예: Office 365 및 기타 SAML 사용 가능 응용 프로그램)
   - 온-프레미스 리소스 및 웹 사이트에 대 한 Windows 통합 인증 (IIS 인증을 요구 하는 웹 사이트 및 SharePoint 사이트 및 NTLM 인증을 사용 하는 리소스 포함)

### <a name="unsupported-scenarios"></a>지원되지 않는 시나리오

지원 되지 않는 시나리오는 다음과 같습니다.

- AD DS (Windows Server Active Directory Domain Services) 도메인 가입 (온-프레미스 전용 장치) 배포는 **지원 되지 않습니다**.
...
관리자는 Azure AD Connect 서버의 PowerShell 도구를 사용 하 여 온-프레미스 디렉터리에 Azure AD Kerberos 서버 개체를 만듭니다. Azure AD 사용자를 포함 하는 조직의 각 도메인 및 포리스트에서 이러한 단계를 실행 해야 합니다.

1. 최신 버전의 Azure AD Connect로 업그레이드 합니다. 이 지침에서는 하이브리드 환경을 지원 하도록 Azure AD Connect를 이미 구성 했다고 가정 합니다.
1. Azure AD Connect 서버에서 관리자 권한 PowerShell 프롬프트를 열고 C:\Program Files\Microsoft Azure Active Directory Connect\AzureADKerberos\로 이동 합니다.
...
> [!NOTE]
> 다음 예제의 "contoso.corp.com"을 온-프레미스 Active Directory 도메인 이름으로 바꿉니다.
...
이 명령은 Azure AD Kerberos 서버의 속성을 출력 합니다. 속성을 검토 하 여 모든 것이 올바른 순서 인지 확인할 수 있습니다.

| 자산 | 설명 |
| --- | --- |
| ID | AD 도메인 컨트롤러 개체의 고유 ID입니다. 이 ID는 "슬롯" 또는 "분기 ID" 라고도 합니다. |
| DomainDnsName | Active Directory 도메인 DNS 도메인 이름입니다. |
| 컴퓨터 계정 | Azure AD Kerberos 서버 개체 (DC)의 컴퓨터 계정 개체입니다. |
| UserAccount | Azure AD Kerberos 서버 TGT 암호화 키를 보유 하는 사용 하지 않도록 설정 된 사용자 계정 개체입니다. 이 계정의 DN은 다음과 같습니다. <br> CN = krbtgt_AzureAD, CN = Users, < 도메인-DN > |
| KeyVersion | Azure AD Kerberos 서버 TGT 암호화 키의 키 버전입니다. 이 버전은 키를 만들 때 할당 됩니다. 그런 다음 키를 회전할 때마다 버전이 증가 합니다. 증분은 복제 메타 데이터를 기반으로 하므로 1 보다 클 수 있습니다. 예를 들어 초기 KeyVersion은 192272 일 수 있습니다. 키를 처음 회전할 때 버전은 212621로 이동 될 수 있습니다. 확인 해야 하는 중요 한 점은 온-프레미스 개체의 KeyVersion과 클라우드 개체의 CloudKeyVersion이 동일한 지 확인 하는 것입니다. |
| KeyUpdatedOn | Azure AD Kerberos 서버 TGT 암호화 키가 업데이트/생성 된 날짜와 시간입니다. |
| KeyUpdatedFrom | Azure AD Kerberos 서버 TGT 암호화 키가 마지막으로 업데이트 된 도메인 컨트롤러입니다. |
| CloudId | Azure AD 개체의 ID입니다. 위의 ID와 일치 해야 합니다. |
| CloudDomainDnsName | Azure AD 개체의 DomainDnsName입니다. 위의 DomainDnsName 일치 해야 합니다. |
| CloudKeyVersion | Azure AD 개체의 KeyVersion입니다. 위의 KeyVersion과 일치 해야 합니다. |
| CloudKeyUpdatedOn | Azure AD 개체의 KeyUpdatedOn입니다. 위의 KeyUpdatedOn 일치 해야 합니다. |
...
예를 들어 조직에 contoso.com 및 fabrikam.com 라는 두 도메인이 있는 Active Directory 포리스트가 있습니다. Azure AD에서 전체 포리스트에 대해 Kerberos TGT (허용 티켓)를 발급 하도록 선택 하는 경우 Azure AD에는 두 개의 KerberosDomain 개체가 있습니다. Contoso.com 및 fabrikam.com에 대 한 KerberosDomain 개체 1 개 Active Directory 포리스트가 여러 개인 경우 각 포리스트의 각 도메인에 대해 하나의 KerberosDomain 개체를 갖게 됩니다.

Azure AD 사용자를 포함 하는 조직의 각 도메인 및 포리스트에서 [Kerberos 서버 개체 만들기](#create-kerberos-server-object) 의 단계를 실행 해야 합니다.

## <a name="known-behavior"></a>알려진 동작

암호가 만료 되 면 FIDO를 사용 하 여 로그온이 차단 됩니다. 사용자가 자신의 암호를 다시 설정 하 여 FIDO를 사용 하 여 로그인 할 수 있습니다.
...
하이브리드 Azure AD 조인 컴퓨터를 새로 설치 하는 경우 도메인 가입 및 다시 시작 후에 암호를 사용 하 여 로그인 하 고 FIDO를 사용 하 여 로그인 할 수 있기 전에 정책이 동기화 될 때까지 기다려야 합니다.

- 명령 창에 `dsregcmd /status`를 입력 하 여 현재 상태를 확인 하 고 `AzureAdJoined` 및 `DomainJoined` 모두 `YES`를 표시 하는지 확인 합니다.
- 이러한 지연은 도메인 가입 장치에 대 한 알려진 제한 이며 FIDO 특정 하지 않습니다.
...
리소스 요청을 처리할 때까지 충분 한 도메인 컨트롤러가 패치 되었는지 확인 합니다. 기능을 실행 하는 도메인 컨트롤러를 볼 수 있는지 확인 하려면 nltest/dsgetdc: contoso/keylist/kdc의 출력을 확인 합니다.
