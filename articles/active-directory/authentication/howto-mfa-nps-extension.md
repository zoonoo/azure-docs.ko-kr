...
1. RADIUS 클라이언트(VPN, Netscaler 서버 또는 기타)와 NPS 서버 간에 사용되는 암호 암호화 알고리즘입니다.
   ...
   - **CHAPV2** 및 **EAP**는 전화 통화 및 모바일 앱 알림을 지원합니다.
      ...
      >
      > 또한 VPN 클라이언트 UX가 입력 필드를 지원하고 네트워크 액세스 정책을 구성한 경우 인증이 성공할 수 있지만 네트워크 정책에 구성된 RADIUS 특성은 RRAS 서버나 VPN 클라이언트와 같은 네트워크 액세스 장치에적용되지 않습니다. 따라서 VPN 클라이언트는 원하는 것보다 더 많은 액세스 권한을 갖거나 액세스 하지 않을 수 있습니다.
      >
