...
| 31034| ServiceBusListenerError| 이 이벤트는 테 넌 트의 Service Bus 수신기에 연결 하는 동안 오류가 발생 했음을 나타냅니다. 오류 메시지에 "원격 인증서가 유효 하지 않습니다."가 포함 된 경우 Azure AD Connect 서버에 [AZURE TLS 인증서 변경](../../security/fundamentals/tls-certificate-changes.md)에 설명 된 대로 필요한 모든 루트 ca가 있는지 확인 합니다. |
