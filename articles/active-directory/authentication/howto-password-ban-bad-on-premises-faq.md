...
**Q: 온-프레미스 public이 아닌 클라우드에서 지원 되는 Azure AD 암호 보호 인지 확인 합니다.**

아니요 - 온-프레미스 Azure AD 암호 보호는 공용 클라우드에서만 지원됩니다. 비공용 클라우드가 지원되는 날짜는 발표되지 않았습니다.

**Q: Azure AD 암호 보호의 이점 내 온-프레미스 사용자의 하위 집합에 적용할 수는 방법**
...
[DFSR sysvol 복제 마이그레이션에 대 한 사례](https://blogs.technet.microsoft.com/askds/2010/04/22/the-case-for-migrating-sysvol-to-dfsr)

[완료 되 면 FRS 용 Nigh](https://blogs.technet.microsoft.com/filecab/2014/06/25/the-end-is-nigh-for-frs)

**Q: 기능에서 도메인 sysvol 공유에 얼마나 많은 공간이 필요 합니까?**
...
**Q: DC 에이전트 소프트웨어를 업그레이드 하거나 설치 다시 부팅이 필요한 이유**

이 요구 사항은 핵심 Windows 동작으로 인해 발생할 수 있습니다.

**Q: 특정 프록시 서버를 사용 하도록 DC 에이전트를 구성 하는 방식으로?**

아니요. 프록시 서버는 상태 비저장 서버이므로 어떤 프록시 서버를 사용하는지는 중요하지 않습니다.

**Q: Azure AD Connect와 같은 다른 서비스와 함께 Azure AD 암호 보호 프록시 서비스를 배포 해도 입니까?**
...
**Q: 고려해 야 내 도메인 컨트롤러에서이 기능을 배포에서 성능 저하에 대 한?**
...
**Q: 도메인의 일부 DC에서만 Azure AD 암호 보호를 테스트하려고 합니다. 해당 특정 Dc를 사용할 사용자 암호 변경 내용을 적용할 수는?**
...
**Q: Azure AD 암호 보호 DC 에이전트 서비스에만 주 도메인 컨트롤러 (PDC)를 설치 하는 경우 도메인의 다른 모든 도메인 컨트롤러도 보호 됩니다?**
...
**Q: Azure AD 암호 보호를 위한 System Center Operations Manager 관리 팩을 사용할 수는?**
...
[피싱 보호 가이드-Part 15 전자 메일: Microsoft Azure AD 암호 보호 서비스 구현 (온-프레미스에 대 한 너무!)](https://blogs.technet.microsoft.com/cloudready/2018/10/14/email-phishing-protection-guide-part-15-implement-the-microsoft-azure-ad-password-protection-service-for-on-premises-too/)

[Azure AD 암호 보호 및 스마트 잠금은 이제 공개 미리 보기로 제공!](https://techcommunity.microsoft.com/t5/Azure-Active-Directory-Identity/Azure-AD-Password-Protection-and-Smart-Lockout-are-now-in-Public/ba-p/245423#M529)
