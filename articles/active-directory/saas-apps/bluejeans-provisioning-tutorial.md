...
> [!div class="checklist"]
> * BlueJeans에서 사용자 만들기
> * BlueJeans에서 더 이상 액세스할 필요가 없는 사용자 제거
> * Azure AD와 BlueJeans 간에 사용자 특성을 동기화된 상태로 유지
> * BlueJeans에 대한 [Single Sign-On](./bluejeans-tutorial.md)(권장)
...
3. [Azure AD와 BlueJeans 간에 매핑](../app-provisioning/customize-application-attributes.md)할 데이터를 결정합니다.

## <a name="step-2-configure-bluejeans-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD에서 프로비저닝을 지원하도록 BlueJeans 구성

1. BlueJeans 관리 콘솔에 로그인합니다. 그룹 설정 > 보안으로 이동합니다.
2. **Single Sign On** 및 **지금 구성** 을 선택합니다.

    ![now](./media/bluejeans-provisioning-tutorial/configure.png)

3. **프로비전 및 통합** 창에서 **IDP를 통해 사용자 계정 만들기 및 관리** 를 선택하고 **토큰 생성** 을 클릭합니다.

    ![generate](./media/bluejeans-provisioning-tutorial/token.png)
    
4. 토큰을 복사하고 저장합니다. 
5. BlueJeans 테넌트 URL은 `https://api.bluejeans.com/v2/scim`입니다. 이전 단계의 **테넌트 URL** 및 **비밀 토큰** 은 Azure Portal에서 BlueJeans 애플리케이션의 프로비저닝 탭에 입력됩니다.

## <a name="step-3-add-bluejeans-from-the-azure-ad-application-gallery"></a>3단계: Azure AD 애플리케이션 갤러리에서 BlueJeans 추가

Azure AD 애플리케이션 갤러리에서 BlueJeans를 추가하여 BlueJeans에 대한 프로비저닝 관리를 시작합니다. 이전에 SSO에 대해 BlueJeans를 설정한 경우 동일한 애플리케이션을 사용할 수 있습니다. 그러나 처음 통합을 테스트하는 경우 별도의 앱을 만드는 것이 좋습니다. [여기](../manage-apps/add-application-portal.md)를 클릭하여 갤러리에서 애플리케이션을 추가하는 방법에 대해 자세히 알아봅니다.
...
* BlueJeans에 사용자를 할당할 때 **기본 액세스** 이외의 역할을 선택해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 역할을 더 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-bluejeans"></a>5단계. BlueJeans에 대한 자동 사용자 프로비저닝 구성
...
5. **관리자 자격 증명** 섹션 아래에 2단계에서 검색한 BlueJeans 테넌트 URL 및 비밀 토큰을 입력합니다. **연결 테스트** 를 클릭하여 Azure AD가 BlueJeans에 연결할 수 있는지 확인합니다. 연결에 실패하면 BlueJeans 계정에 관리자 권한이 있는지 확인하고 다시 시도합니다.

    ![토큰](common/provisioning-testconnection-tenanturltoken.png)


6. **알림 이메일** 필드에 프로비저닝 오류 알림을 받을 사람의 이메일 주소를 입력하고, **오류가 발생할 경우 이메일 알림 보내기** 확인란을 선택합니다.
...
9. **특성 매핑** 섹션에서는 Azure AD에서 BlueJeans로 동기화된 사용자 특성을 검토합니다. **일치** 속성으로 선택한 특성은 업데이트 작업 시 BlueJeans의 사용자 계정을 일치시키는 데 사용됩니다. [일치하는 대상 특성](../app-provisioning/customize-application-attributes.md)을 변경하는 경우 BlueJeans API에서 해당 특성에 따라 사용자 필터링을 지원하는지 확인해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
12. **설정** 섹션의 **범위** 에서 원하는 값을 선택하여 BlueJeans에 프로비저닝하려는 사용자를 정의합니다.
