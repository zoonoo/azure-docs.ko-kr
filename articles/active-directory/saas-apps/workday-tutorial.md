...
1. 검색 상자에 **Workday**를 입력하고 결과 패널에서 **Workday**를 선택한 후 **추가** 단추를 클릭하여 응용 프로그램을 추가합니다.
...
1. **[Workday 테스트 사용자 만들기](#create-a-workday-test-user)** - Britta Simon의 Azure AD 표현과 연결되는 대응 사용자를 Workday에 만듭니다.
...
1. **Workday 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    나. **식별자** 텍스트 상자에 URL `http://www.workday.com`를 입력합니다.
...
1. **Workday 구성** 섹션에서 **Workday 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.
...
1. **리디렉션 URL** 섹션에서 다음 단계를 수행합니다.
   
    ![리디렉션 URL](./media/workday-tutorial/IC7829581.png "리디렉션 URL")
   
    a. **행 추가**를 클릭합니다.
   
    나. **로그인 리디렉션 URL** 텍스트 상자 및 **모바일 리디렉션 URL** 텍스트 상자에서 Azure Portal의 **Workday 도메인 및 URL** 섹션에 입력한 **로그온 URL**을 입력합니다.
...
1. **x509 공개 키 보기** 섹션에서 다음 단계를 수행합니다. 
    ...
    나. **유효 시작** 텍스트 상자에 인증서의 유효 시작 특성 값을 입력합니다.
...
1. 다음 단계를 수행합니다. 
    ...
    나. **SP에서 시작한 인증 요청을 Deflate하지 않음**을 선택합니다.
...
1. 응용 프로그램 목록에서 **Workday**를 선택합니다.
