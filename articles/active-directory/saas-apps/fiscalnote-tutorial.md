...
1. **[FiscalNote 테스트 사용자 만들기](#create-a-fiscalnote-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 FiscalNote에 만듭니다.
...
1. **FiscalNote 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
...
1. FiscalNote 응용 프로그램은 특정 서식에서 SAML 어설션을 예상하며 이는 SAML 토큰 특성 구성에 사용자 할당 특성 매핑을 추가합니다. 이 응용 프로그램에 대해 다음 클레임을 구성합니다. 응용 프로그램 통합 페이지의 **"사용자 특성"** 섹션에서 이러한 특성의 값을 관리할 수 있습니다.
...
1. **FiscalNote 구성** 섹션에서 **FiscalNote 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![FiscalNote 구성](./media/fiscalnote-tutorial/tutorial_fiscalnote_configure.png) 

1. **FiscalNote** 쪽에서 Single Sign-On을 구성하려면 다운로드한 **인증서(원시), 로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 [FiscalNote 지원 팀](mailto:support@fiscalnote.com)에 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
