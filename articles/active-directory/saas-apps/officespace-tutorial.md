...
* 사용자가 해당 Azure AD 계정으로 OfficeSpace Software에 자동으로 로그인(Single Sign-On)되도록 설정할 수 있습니다.
...
4. 검색 상자에 **OfficeSpace Software**를 입력하고 결과 패널에서 **OfficeSpace Software**를 선택한 후 **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD 사용자와 OfficeSpace Software의 관련 사용자 간에 연결 관계를 설정해야 합니다.

OfficeSpace Software에서 Azure AD Single Sign-on을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.

1. **[Azure AD Single Sign-On 구성](#configure-azure-ad-single-sign-on)** - 사용자가 이 기능을 사용할 수 있도록 합니다.
2. **[OfficeSpace Software Single Sign-On 구성](#configure-officespace-software-single-sign-on)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
3. **[Azure AD 테스트 사용자 만들기](#create-an-azure-ad-test-user)** - Britta Simon으로 Azure AD Single Sign-On을 테스트하는 데 사용합니다.
4. **[Azure AD 테스트 사용자 할당](#assign-the-azure-ad-test-user)** - Britta Simon이 Azure AD Single Sign-on을 사용할 수 있도록 합니다.
5. **[OfficeSpace Software 테스트 사용자 만들기](#create-officespace-software-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 OfficeSpace Software에 만듭니다.
...
1. [Azure Portal](https://portal.azure.com/)의 **OfficeSpace Software** 애플리케이션 통합 페이지에서 **Single Sign-On**을 선택합니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![OfficeSpace Software 도메인 및 URL Single Sign-On 정보](common/sp-identifier.png)

    a. **로그온 URL** 텍스트 상자에 다음 패턴을 사용하여 URL을 입력합니다. `https://<company name>.officespacesoftware.com/users/sign_in/saml`

    b. **식별자(엔터티 ID)** 텍스트 상자에 다음 패턴을 사용하여 URL을 입력합니다. `<company name>.officespacesoftware.com`
...
5. OfficeSpace Software 애플리케이션은 특정 서식에서 SAML 어설션을 예상하며 이는 SAML 토큰 특성 구성에 사용자 지정 특성 매핑을 추가합니다. 다음 스크린샷에서는 **nameidentifier**가 **user.userprincipalname**과 매핑되는 기본 특성 목록을 보여줍니다. OfficeSpace Software 애플리케이션에서는 **nameidentifier**가 **user.mail**과 매핑되므로 특성 매핑을 변경하기 위해 **편집** 아이콘을 클릭하여 특성 매핑을 편집해야 합니다.

    ![이미지](common/edit-attribute.png)

6. 위에서 언급한 특성 외에도, OfficeSpace Software 애플리케이션에는 SAML 응답에서 다시 전달되어야 하는 몇 가지 특성이 추가로 필요합니다. **사용자 특성** 대화 상자의 **사용자 클레임** 섹션에서 다음 단계를 수행하여 아래 표와 같은 SAML 토큰 특성을 추가합니다.
...
9. **OfficeSpace Software 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
### <a name="configure-officespace-software-single-sign-on"></a>OfficeSpace Software Single Sign-On 구성
...
1. Azure Portal에서 **엔터프라이즈 애플리케이션**, **모든 애플리케이션**, **OfficeSpace Software**를 차례로 선택합니다.
...
이 섹션에서는 Britta Simon이라는 사용자를 OfficeSpace Software에서 만듭니다. OfficeSpace Software는 Just-In-Time 사용자 프로비저닝을 지원하며, 기본적으로 사용하도록 설정되어 있습니다. 이 섹션에 작업 항목이 없습니다. OfficeSpace Software에 사용자가 아직 없는 경우 인증 후에 새 사용자가 만들어집니다.
...
액세스 패널에서 OfficeSpace Software 타일을 클릭하면 SSO를 설정한 OfficeSpace Software에 자동으로 로그온됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
