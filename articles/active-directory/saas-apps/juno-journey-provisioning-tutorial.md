---
title: '자습서: Azure Active Directory을 사용 하 여 자동 사용자 프로 비전을 위한 Juno 과정 구성 | Microsoft Docs'
description: 사용자 계정을 Azure AD에서 Juno 여행으로 자동으로 프로 비전 및 프로 비전 해제 하는 방법을 알아봅니다.
...
---
# <a name="tutorial-configure-juno-journey-for-automatic-user-provisioning"></a>자습서: 자동 사용자 프로 비전을 위한 Juno 경험 구성

이 자습서에서는 자동 사용자 프로 비전을 구성 하기 위해 Juno 경험과 Azure Active Directory (Azure AD)에서 수행 해야 하는 단계를 설명 합니다. 구성 된 경우 azure AD는 Azure AD 프로 비전 서비스를 사용 하 여 사용자 및 그룹을 [Juno](https://www.junojourney.com/) 으로 프로 비전 하 고 프로 비전 해제 합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../manage-apps/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원 되는 기능
> [!div class="checklist"]
> * Juno 여행에서 사용자 만들기
> * 더 이상 액세스할 필요가 없는 경우 Juno에서 사용자 제거
> * Azure AD와 Juno 여행 간에 사용자 특성을 동기화 상태로 유지
> * Juno에 대 [한 Single sign-on](https://docs.microsoft.com/azure/active-directory/saas-apps/juno-journey-tutorial) (권장)
...
*  [Juno 여행 테 넌 트](https://www.junojourney.com/getstartedwithjuno).
*  관리자 권한으로 Juno의 사용자 계정
...
3. [AZURE AD와 Juno 여행 간에 매핑할](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)데이터를 결정 합니다. 

## <a name="step-2-configure-juno-journey-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD를 사용 하 여 프로 비전을 지원 하도록 Juno 경험 구성

1. **비밀 토큰** 및 **테 넌 트 URL** 의 경우 Juno 여행 지원 support@the-juno.com팀에 문의 하세요. 이 값은 Azure Portal에서 Juno 여행 응용 프로그램의 프로 비전 탭에 각각 **비밀 토큰** 및 **테 넌 트 URL** 필드에 입력 됩니다. 

## <a name="step-3-add-juno-journey-from-the-azure-ad-application-gallery"></a>3단계: Azure AD 응용 프로그램 갤러리에서 Juno 여행 추가

Azure AD 응용 프로그램 갤러리에서 Juno 경험을 추가 하 여 Juno 여행에 대 한 프로 비전 관리를 시작 합니다. 이전에 SSO에 대 한 Juno 경험을 설정 했다면 동일한 응용 프로그램을 사용할 수 있습니다. 그러나 처음에 통합을 테스트할 때 별도의 앱을 만드는 것이 좋습니다. [여기](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)에서 갤러리에서 응용 프로그램을 추가 하는 방법에 대해 자세히 알아보세요. 
...
* Juno 경험에 사용자 및 그룹을 할당할 때 **기본 액세스**외의 다른 역할을 선택 해야 합니다. 기본 액세스 역할이 있는 사용자는 프로 비전에서 제외 되 고 프로 비전 로그에 효과적으로 부여 되지 않은 것으로 표시 됩니다. 응용 프로그램에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [응용 프로그램 매니페스트를 업데이트](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps) 하 여 역할을 더 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-juno-journey"></a>5단계. Juno 여행에 대 한 자동 사용자 프로 비전 구성 

이 섹션에서는 azure ad의 사용자 및/또는 그룹 할당에 따라 TestApp의 사용자 및/또는 그룹을 만들고, 업데이트 하 고, 비활성화 하도록 Azure AD 프로 비전 서비스를 구성 하는 단계를 안내 합니다.

### <a name="to-configure-automatic-user-provisioning-for-juno-journey-in-azure-ad"></a>Azure AD에서 Juno에 대 한 자동 사용자 프로 비전을 구성 하려면:
...
2. 애플리케이션 목록에서 **Juno Journey**를 선택합니다.
    ...
    ![응용 프로그램 목록의 Juno 여행 링크](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션에서 이전에 **테 넌 트 url**에서 검색 된 테 넌 트 url 값을 입력 합니다. **비밀 토큰**에서 이전에 검색 된 비밀 토큰 값을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 Juno 여행에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Juno 경험 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 합니다.
...
8. **매핑** 섹션 아래에서 **Juno 여행에 사용자 Azure Active Directory 동기화를**선택 합니다.

9. **특성 매핑** 섹션에서 Azure AD에서 Juno로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 Juno의 사용자 계정을 일치 시키는 데 사용 됩니다. 일치 하는 [대상 특성](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)을 변경 하도록 선택 하는 경우 JUNO 여행 API가 해당 특성에 따라 사용자 필터링을 지원 하는지 확인 해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
11. Juno 여행에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 **설정 하려면 설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 Juno 과정에 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
