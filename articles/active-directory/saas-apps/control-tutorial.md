...
description: Azure Active Directory 및 Control 간에 Single Sign-On을 구성하는 방법에 대해 알아봅니다.
...
ms.locfileid: "65874051"
---
# <a name="tutorial-integrate-control-with-azure-active-directory"></a>자습서: Control과 Azure Active Directory 통합

이 자습서에서는 Azure AD(Azure Active Directory)와 Control을 통합하는 방법에 대해 알아봅니다. Azure AD와 Control을 통합하는 경우 다음을 수행할 수 있습니다.

* Azure AD에서는 Control에 대한 액세스 권한이 있는 사용자를 제어합니다.
...
1. **기본 SAML 구성** 페이지에서 다음 필드의 값을 입력합니다.
    ...
    **로그인 URL** 텍스트 상자에서 `https://<SUBDOMAIN>.continuity.net/auth/saml` 패턴을 사용하여 URL을 입력합니다.
    ...
    > [!Note]
    > 이 값은 실제 값이 아닙니다. 이 값을 실제 로그온 URL로 업데이트합니다. 값을 얻으려면 [Control 클라이언트 지원 팀](mailto:help@continuity.net)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.
...
1. **Control 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
**Control** 쪽에서 Single Sign-On을 구성하려면 **지문 값**과 Azure Portal에서 복사한 해당 URL을 [Control 지원 팀](mailto:help@continuity.net)으로 전송해야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
...
이 섹션에서는 Control에서 Britta Simon이라는 사용자를 만듭니다. Control 플랫폼에서 사용자를 추가하려면  [Control 지원 팀](mailto:help@continuity.net)에 문의하세요. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.
