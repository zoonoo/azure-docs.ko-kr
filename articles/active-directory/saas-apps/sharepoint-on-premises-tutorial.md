...
5. **[SharePoint 온-프레미스 테스트 사용자에 대한 액세스 권한 부여](#grant-access-to-sharepoint-on-premises-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 SharePoint 온-프레미스에 만듭니다.
...
Azure AD에 로그인하고 SharePoint에 액세스할 사용자는 애플리케이션에 대한 액세스 권한이 부여되어야 합니다. 다음 단계를 사용하여 웹 애플리케이션에 액세스하는 사용 권한을 설정합니다.

1. 중앙 관리에서 **애플리케이션 관리**를 클릭합니다.

2. **애플리케이션 관리** 페이지의 **웹 애플리케이션** 섹션에서 **웹 애플리케이션 관리**를 클릭합니다.

3. 적합한 웹 애플리케이션을 클릭한 다음, **사용자 정책**을 클릭합니다.

4. 웹 애플리케이션에 대한 정책에서 **사용자 추가**를 클릭합니다.
...
6. **웹 애플리케이션에 대한 정책** 대화 상자의 **사용자 선택** 섹션에서 **찾아보기** 아이콘을 클릭합니다.

7. **찾기** 텍스트 상자에 Azure AD에서 SharePoint 온-프레미스 애플리케이션을 구성한 **UPN(사용자 계정 이름)** 값을 입력하고 **검색**을 클릭합니다. </br>예: *brittasimon@contoso.com*.
