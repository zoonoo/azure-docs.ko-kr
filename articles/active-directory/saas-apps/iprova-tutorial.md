...
5. **[iProva 테스트 사용자 만들기](#create-iprova-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 iProva에 만듭니다.
...
이 섹션에서는 iProva에서 몇 가지 중요한 정보를 검색합니다.
이 정보는 Azure AD Single Sign-On을 구성하는 데 필요합니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    a. **식별자** 필드를 **iProva SAML2 정보 페이지**(다른 브라우저 탭에 아직 열려 있음)의 **EntityID** 레이블 뒤에 표시된 값으로 채웁니다

    b. **Reply-URL** 필드를 **iProva SAML2 정보 페이지**(다른 브라우저 탭에 아직 열려 있음)의 **Reply URL** 레이블 뒤에 표시된 값으로 채웁니다.

    다. **Sign-on URL** 필드를 **iProva SAML2 정보 페이지**(다른 브라우저 탭에 아직 열려 있음)의 **Sign-on URL** 레이블 뒤에 표시된 값으로 채웁니다.

    ![iProva 도메인 및 URL Single Sign-On 정보](common/sp-identifier-reply.png)

5. iProva 애플리케이션에는 특정 형식의 SAML 어설션이 필요합니다. 이 애플리케이션에 대해 다음 클레임을 구성합니다. 응용 프로그램 통합 페이지의 **사용자 특성** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. **SAML로 Single Sign-On 설정** 페이지에서 **편집** 단추를 클릭하여 **사용자 특성** 대화 상자를 엽니다.
...
6. **사용자 특성** 대화 상자의 **사용자 클레임** 섹션에서 위의 이미지에 표시된 것과 같이 SAML 토큰 특성을 구성하고 다음 단계를 수행합니다.
    ...
    다. **네임스페이스** 목록에서 해당 행에 대해 표시된 네임스페이스 값을 입력합니다.
...
8. **Setup**(설정) 단추를 클릭합니다.

9. **다음** 단추를 클릭합니다.

10. 페더레이션 데이터를 URL에서 다운로드할지 아니면 파일에서 업로드할지 묻는 메시지가 iProva에 표시됩니다. **from URL**(URL에서) 옵션을 선택합니다.

    ![Azure AD 메타데이터 다운로드](media/iprova-tutorial/iprova-download-metadata.png)

11. **Configure Azure AD single sign-on** 챕터의 마지막 단계에서 저장한 메타데이터 URL을 붙여넣습니다.

12. 화살표 모양의 단추를 클릭하여 Azure AD에서 메타데이터를 다운로드합니다.

13. 다운로드가 완료되면 확인 메시지, **Valid Federation Data file downloaded**(유효한 페더레이션 데이터 파일이 다운로드됨)가 표시됩니다

14. **다음** 단추를 클릭합니다.

15. **Test login**(로그인 테스트) 옵션은 건너뛰고 **Next**(다음) 단추를 클릭합니다.

16. **Claim to use** 드롭다운에서 **windowsaccountname**을 선택합니다.

17. **마침** 단추를 클릭합니다.

18. 이제 **Edit general settings**(일반 설정 편집) 화면으로 돌아갑니다. 페이지 맨 아래까지 **아래로 스크롤**하고 **OK**를 클릭하여 구성을 저장합니다.
...
3. 사용자 속성에서 다음 단계를 수행합니다.
    ...
    a. **이름** 필드에 **BrittaSimon**과 같은 입력합니다.
  
    b. **사용자 이름** 필드에 **yourname@yourcompanydomain.extension**을 입력합니다.  
...
4. **사용자 및 사용자 그룹** 패널에서 **사용자**를 클릭합니다.

5. **추가** 단추를 클릭합니다.

6. **Username** 필드에 **brittasimon@yourcompanydomain.extension**을 입력합니다.  
    예를 들어 BrittaSimon@contoso.com

7. **전체 이름** 필드에 **BrittaSimon**과 같은 전체 이름을 입력합니다.

8. **암호 없음(Single Sign-On 사용)** 옵션을 선택합니다.

9. **이메일 주소** 필드에 **yourname@yourcompanydomain.extension**(예: BrittaSimon@contoso.com)을 입력합니다.

10. 페이지 끝까지 아래로 스크롤하여 **마침** 단추를 클릭합니다.
