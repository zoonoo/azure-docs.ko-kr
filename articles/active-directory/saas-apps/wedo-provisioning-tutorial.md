...
이 자습서에서는 자동 사용자 프로비전을 구성하기 위해 WEDO 및 Azure AD(Azure Active Directory)에서 수행해야 하는 단계를 설명합니다. 구성된 경우 Azure AD는 Azure AD 프로비전 서비스를 사용하여 사용자 및 그룹을 [WEDO](https://www.wedo.com/) 로 자동으로 프로비전 및 프로비전 해제합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../app-provisioning/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원되는 기능
> [!div class="checklist"]
> * WEDO에서 사용자를 만듭니다.
> * WEDO에서 더 이상 액세스할 필요가 없는 사용자 제거.
> * Azure AD와 WEDO 간에 사용자 특성을 동기화된 상태로 유지.
> * [Single sign-on](wedo-tutorial.md) 설정을 WEDO에 적용(권장).
...
3. [Azure AD와 WEDO 간에 매핑](../app-provisioning/customize-application-attributes.md)할 데이터를 결정합니다. 

## <a name="step-2-configure-wedo-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD에서 프로비전을 지원하도록 WEDO 구성

* WEDO 지원팀에 [info@wedo.swiss](mailto:info@wedo.swiss) 연락하여 **테넌트 URL** 및 **비밀 토큰** 을 가져옵니다. 해당 값들은 Azure Portal의 WEDO 응용 프로그램 프로비전 탭에서 테넌트 URL *필드 및 비밀 토큰 *필드에 입력됩니다.

## <a name="step-3-add-wedo-from-the-azure-ad-application-gallery"></a>3단계: Azure AD 응용 프로그램 갤러리에서 WEDO 추가

Azure AD 응용 프로그램 갤러리에서 WEDO를 추가하여 WEDO의 프로비전 관리를 시작합니다. 이전에 SSO용 WEDO를 설정했다면 동일한 응용 프로그램을 사용할 수 있습니다. 그러나 처음 통합을 테스트하는 경우 별도의 앱을 만드는 것이 좋습니다. [여기](../manage-apps/add-application-portal.md)를 클릭하여 갤러리에서 애플리케이션을 추가하는 방법에 대해 자세히 알아봅니다. 
...
* 사용자 및 그룹을 WEDO에 할당할 때 **기본 액세스** 가 아닌 다른 역할을 선택해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 역할을 더 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-wedo"></a>5단계. WEDO에 대한 자동 사용자 프로비전 구성 

이 섹션에서는 Azure AD의 사용자 및/또는 그룹 할당에 따라 TestApp에서 사용자 및/또는 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-wedo-in-azure-ad"></a>Azure AD에서 WEDO에 대한 자동 사용자 프로비전을 구성하려면 다음을 수행합니다.
...
5. **관리자 자격 증명** 섹션에서 WEDO 테넌트 URL 및 비밀 토큰을 입력합니다. **연결 테스트** 를 클릭하여 Azure AD가 WEDO에 연결할 수 있는지 확인합니다. 연결에 실패하면 WEDO 계정에 관리자 권한이 있는지 확인하고 다시 시도하세요.
...
8. **매핑** 섹션에서 **Azure Active Directory 사용자를 WEDO에 동기화** 를 선택합니다.

9. Azure AD에서 WEDO로 동기화되는 사용자 특성을 **특성 매핑** 섹션에서 검토합니다. **일치** 속성으로 선택한 특성은 업데이트 작업 시 WEDO의 사용자 계정을 일치시키는 데 사용됩니다. [일치하는 대상 특성](../app-provisioning/customize-application-attributes.md)을 변경하도록 선택하는 경우 WEDO API가 해당 특성에 따라 사용자 필터링을 지원하는지 확인해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
13. WEDO에 대한 Azure AD 프로비전 서비스를 사용하도록 설정하려면 **프로비전 상태** 를 **On** 으로 **설정** 섹션에서 설정합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

14. **범위** 를 **설정** 섹션에서 원하는 값으로 선택해 WEDO에 프로비전하려는 사용자 및/또는 그룹을 정의합니다.
