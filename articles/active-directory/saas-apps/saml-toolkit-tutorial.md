---
title: '자습서: Azure AD SAML Toolkit와 Azure Active Directory 통합 | Microsoft Docs'
...
---
# <a name="tutorial-integrate-azure-ad-saml-toolkit-with-azure-active-directory"></a>자습서: Azure Active Directory와 Azure AD SAML Toolkit 통합
...
이 자습서에서는 테스트 환경에서 Azure AD SSO를 구성하고 테스트합니다. Azure AD SAML Toolkit에서 **SP** 시작 SSO를 지원합니다.
...
2. **[Azure AD SAML Toolkit SSO 구성](#configure-azure-ad-saml-toolkit-sso)** - 애플리케이션 쪽에서 SSO 설정을 구성합니다.
...
5. **[Azure AD SAML Toolkit 테스트 사용자 만들기](#create-azure-ad-saml-toolkit-test-user)** - B.Simon의 Azure AD 표현과 연결된 해당 사용자를 Azure AD SAML Toolkit에 만듭니다.
...
1. [Azure Portal](https://portal.azure.com/)의 **Azure AD SAML Toolkit** 애플리케이션 통합 페이지에서 **관리** 섹션을 찾은 다음, **Single Sign-On**을 선택합니다.
...
1. **기본 SAML 구성** 페이지에서 다음 필드에 대한 값을 입력합니다.
    ...
    1. **로그온 URL** 텍스트 상자에 다음 URL을 입력합니다. `https://samltoolkit.azurewebsites.net/`

    1. **식별자(엔터티 ID)** 텍스트 상자에서 `https://samltoolkit.azurewebsites.net` URL을 입력합니다.

    1. **회신 URL** 텍스트 상자에서 `https://samltoolkit.azurewebsites.net/SAML/Consume` URL을 입력합니다.
...
1. **SAML SSO 구성** 페이지에서 다음 단계를 수행합니다.
    ...
    1. **파일 선택**을 클릭하고 Azure Portal에서 다운로드한 **페더레이션 메타데이터 XML** 파일을 업로드합니다.
...
액세스 패널에서 Azure AD SAML Toolkit 타일을 선택하면 SSO를 설정한 Azure AD SAML Toolkit에 자동으로 로그인됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
