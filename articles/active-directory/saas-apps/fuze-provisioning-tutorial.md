...
> [!TIP]
> [Fuze single sign-on 자습서](fuze-tutorial.md)에 제공 된 지침에 따라 Fuze에 대해 SAML 기반 Single Sign-On를 사용 하도록 선택할 수도 있습니다. Single Sign-On과 자동 사용자 프로비저닝은 서로 보완적이지만, 별개로 구성할 수 있습니다.
...
5. **관리자 자격 증명** 섹션 아래에 있는 **테 넌 트 URL**에 `https://api.fuze.com/scim/v2`을 입력 합니다. **비밀 토큰**의 Fuze 담당자 로부터 이전에 검색 된 **Scim 인증 토큰** 값을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 Fuze에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Fuze 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 합니다.
