...
**Azure AD 응용 프로그램 갤러리에서 Cisco Spark를 추가 하려면 다음 단계를 수행 합니다.**
...
4. 검색 상자에서 **Cisco Spark**를 입력하고, 결과 패널에서 **Cisco Spark**를 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.

    ![결과 목록의 Cisco Cloud](common/search-new-app.png)
...
2. 애플리케이션 목록에서 **Cisco Spark**를 선택합니다.

    ![애플리케이션 목록의 Cisco Spark 링크](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션 아래에 **테넌트 URL** 및 Cisco Spark 계정의 **비밀 토큰**을 입력합니다.
    ...
    ![Cisco Spark 프로비전](./media/cisco-spark-provisioning-tutorial/secrettoken1.png)
...
6. Cisco Spark 계정의 **테넌트 ID** 와 **비밀 토큰**은 관리자 계정으로 [Cisco Spark 개발자 사이트](https://developer.webex.com/)에 로그인하면 찾을 수 있습니다. 로그인한 후에
...
7. 5단계에 표시된 필드를 채우면 **연결 테스트**를 클릭하여 Azure AD가 Cisco Spark에 연결할 수 있는지 확인합니다. 연결이 실패하면 Cisco Spark 계정에 관리자 권한이 있는지 확인한 후 다시 시도합니다.
