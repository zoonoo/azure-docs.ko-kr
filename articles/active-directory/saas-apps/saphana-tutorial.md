...
1. 검색 상자에 **SAP HANA**를 입력합니다. 그런 다음 결과 창에서 **SAP HANA**를 선택합니다. 마지막으로 **추가** 단추를 선택하여 애플리케이션을 추가합니다. 

    ![새 애플리케이션](./media/saphana-tutorial/tutorial_saphana_addfromgallery.png)
...
1. [Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user) - Britta Simon으로 Azure AD Single Sign-On을 테스트하는 데 사용합니다.
1. [SAP HANA 테스트 사용자 만들기](#creating-a-sap-hana-test-user) - Britta Simon의 Azure AD 표현과 연결되는 대응 사용자를 SAP HANA에 만듭니다.
...
1. Azure Portal의 **SAP HANA** 애플리케이션 통합 페이지에서 **Single Sign-On**을 선택합니다.

    ![Single Sign-On 구성][4]

1. **Single Sign-On** 대화 상자의 **SAML 기반 로그인** 아래에서 **모드**를 선택합니다.
 
    ![Single Sign-On 대화 상자](./media/saphana-tutorial/tutorial_saphana_samlbase.png)

1. **SAP HANA 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
...
1. SAP HANA 애플리케이션에는 특정 형식의 SAML 어설션이 필요합니다. 다음 스크린샷은 이 형식에 대한 예제를 보여 줍니다. 

    여기서는 **user.mail**의 **ExtractMailPrefix()** 함수를 이용하여 **사용자 ID**를 매핑했습니다. 이렇게 하면은 고유한 사용자 ID인 사용자의 전자 메일 접두사 값을 얻을 수 있습니다. 이 사용자 ID는 응답이 성공할 때마다 SAP HANA 애플리케이션에 전송됩니다.

    ![Single Sign-On 구성](./media/saphana-tutorial/attribute.png)

1. **Single Sign-On** 대화 상자의 **사용자 특성** 섹션에서 다음 단계를 수행합니다.
...
1. SAP HANA 쪽에 Single Sign-On을 구성하려면 해당 HTTPS 끝점으로 이동하여 **HANA XSA 웹 콘솔**에 로그인합니다.

    > [!NOTE]
    > 기본 구성에서는 URL이 요청을 로그온 화면으로 리디렉션하며, 인증된 SAP HANA 데이터베이스 사용자의 자격 증명이 필요합니다. 로그인하는 사용자는 SAML 관리 작업을 수행할 권한이 있어야 합니다.

1. XSA 웹 인터페이스에서 **SAML ID 공급자**로 이동합니다. 여기에서 화면 아래쪽의 **+** 단추를 선택하여 **ID 공급자 정보 추가** 창을 표시합니다. 다음 단계를 수행합니다.
...
1. **저장**을 선택하여 SAML ID 공급자의 세부 정보를 저장하고 알려진 SAML IDP 목록에 새 SAML IDP를 추가합니다.

    ![저장 단추](./media/saphana-tutorial/sap4.png)

1. HANA Studio **Configuration**(구성) 탭의 시스템 속성 내에서 **saml**을 기준으로 설정을 필터링합니다. 그런 다음 **assertion_timeout**을 **10초**에서 **120초**로 조정합니다.
...
1. **SAML** 왼쪽의 보이지 않는 확인란을 선택한 다음 **구성** 링크를 선택합니다.

1. **추가**를 선택하여 SAML IDP를 추가합니다.  적절한 SAML IDP를 선택한 다음 **확인**을 선택합니다.

1. **외부 ID**(이 경우, BrittaSimon)를 추가하거나 **모두**를 선택합니다. 그런 다음 **확인**을 선택합니다.
...
1. 테스트를 위해 모든 **XS** 역할을 사용자에게 할당합니다.
...
1. Azure Portal에서 응용 프로그램 보기를 엽니다. 디렉터리 보기로 이동하여 **엔터프라이즈 애플리케이션**으로 이동합니다. **모든 응용 프로그램**을 선택합니다.
...
액세스 패널에서 SAP HANA 타일을 선택하면 SAP HANA 애플리케이션에 자동으로 로그온됩니다.
