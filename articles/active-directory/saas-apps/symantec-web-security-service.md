---
title: '자습서: Azure Active Directory를 사용 하 여 자동 사용자 프로 비전을 위한 Symantec WSS (Web Security Service) 구성 Microsoft Docs'
...
이 자습서에서는 사용자 및/또는 그룹을 Symantec 웹 보안에 자동으로 프로 비전 및 프로 비전 해제 하도록 Azure AD를 구성 하기 위해 Symantec WSS (Web Security Service) 및 Azure Active Directory (Azure AD)에서 수행 하는 단계를 설명 합니다. 서비스 (WSS).
...
1. [Symantec 웹 보안 서비스 관리 콘솔](https://portal.threatpulse.com/login.jsp)에 로그인 합니다. **솔루션** > **서비스**로 이동 합니다.
...
> [!TIP]
> [SYMANTEC wss (web Security service) single sign-on 자습서](symantec-tutorial.md)에 제공 된 지침에 따라 symantec Wss (Web security service)에 대해 SAML 기반 single sign-on을 사용 하도록 선택할 수도 있습니다. Single sign-on은 자동 사용자 프로 비전과 독립적으로 구성할 수 있습니다. 단,이 두 가지 기능은 서로 보완적입니다.
