...
description: 디지털 프로세스 자동화를 위해 Azure AD에서 Bizagi Studio로 사용자 계정을 자동으로 프로 비전 및 프로 비전 해제 하는 방법을 알아봅니다.
...
이 자습서에서는 자동 사용자 프로 비전을 구성 하기 위해 Bizagi Studio for Digital Process Automation and Azure Active Directory (Azure AD)에서 수행 해야 하는 단계를 설명 합니다. 구성 된 경우 azure ad는 Azure AD 프로 비전 서비스를 사용 하 여 자동으로 사용자 및 그룹을 [Bizagi Studio](https://www.bizagi.com/) 로 프로 비전 하 고 프로 비전 합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../manage-apps/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원되는 기능
> [!div class="checklist"]
> * 디지털 프로세스 자동화를 위해 Bizagi Studio에서 사용자 만들기
> * 더 이상 액세스할 필요가 없는 경우 Bizagi Studio에서 사용자를 제거 하 여 디지털 프로세스 자동화
...
## <a name="step-2-configure-bizagi-studio-for-digital-process-automation-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD를 사용 하 여 프로 비전을 지원 하도록 디지털 프로세스 자동화에 대 한 Bizagi Studio 구성


1. **관리자 권한이** 있는 사용자로 회사 포털에 로그인 합니다.

2. **관리자 > 보안-> Oauth 2 응용 프로그램** 으로 이동 합니다.

   ![설정](media/bizagi-studio-for-digital-process-automation-provisioning-tutorial/admin.png)

3. 추가 단추를 클릭 합니다.
4. 권한 부여 유형에서 **전달자 토큰** 을 선택 합니다. 허용 되는 범위에서 **API** 및 **사용자 동기화** 를 선택 하 고 저장을 클릭 합니다.

   ![api](media/bizagi-studio-for-digital-process-automation-provisioning-tutorial/token.png)

5. **클라이언트 암호** 를 복사 하 고 저장 합니다. 이 값은 Azure Portal의 Digital Process Automation 용 Bizagi Studio Automation 응용 프로그램의 프로 비전 탭에 있는 **비밀 토큰** 필드에 입력 됩니다.

   ![token](media/bizagi-studio-for-digital-process-automation-provisioning-tutorial/secret.png)

## <a name="step-3-add-bizagi-studio-for-digital-process-automation-from-the-azure-ad-application-gallery"></a>3단계: Azure AD 응용 프로그램 갤러리에서 디지털 프로세스 자동화에 대 한 Bizagi Studio 추가

Azure AD 응용 프로그램 갤러리에서 디지털 프로세스 자동화 용 Bizagi Studio를 추가 하 여 디지털 프로세스 자동화를 위해 Bizagi Studio로 프로 비전 관리를 시작 합니다. 이전에 SSO에 대 한 디지털 프로세스 자동화를 위해 Bizagi Studio를 설치한 경우 동일한 응용 프로그램을 사용할 수 있습니다. 그러나 처음 통합을 테스트하는 경우 별도의 앱을 만드는 것이 좋습니다. [여기](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)를 클릭하여 갤러리에서 애플리케이션을 추가하는 방법에 대해 자세히 알아봅니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-bizagi-studio-for-digital-process-automation"></a>5단계. 디지털 프로세스 자동화를 위해 Bizagi Studio에 자동 사용자 프로 비전 구성 

이 섹션에서는 Azure AD의 사용자 및/또는 그룹 할당에 따라 TestApp에서 사용자 및/또는 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-bizagi-studio-for-digital-process-automation-in-azure-ad"></a>Azure AD에서 디지털 프로세스 자동화에 대 한 Bizagi Studio 자동 사용자 프로 비전을 구성 하려면:
...
2. 응용 프로그램 목록에서 **Bizagi Studio For Digital Process Automation** 을 선택 합니다.
    ...
    ![응용 프로그램 목록의 Bizagi Studio for Digital Process Automation 링크](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션 아래에서 디지털 프로세스 자동화 테 넌 트 URL 및 비밀 토큰에 대 한 Bizagi Studio를 입력 합니다. 

      * **테 넌 트 URL:** 다음 구조를 사용 하 여 Bizagi SCIM 끝점을 입력 <Your_Bizagi_Project>/scim/v2/예: `https://my-company.bizagi.com/scim/v2/`

      * **비밀 토큰:** 이 값은 2 단계에서 이전에 검색 됩니다.

      **연결 테스트** 를 클릭 하 여 Azure AD가 디지털 프로세스 자동화를 위해 Bizagi Studio에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Bizagi Studio for Digital Process Automation 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
...
9. **특성 매핑** 섹션에서 Azure AD에서 디지털 프로세스 자동화를 위해 Bizagi Studio로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 디지털 프로세스 자동화에 대 한 Bizagi Studio의 사용자 계정을 일치 시키는 데 사용 됩니다. 일치 하는 [대상 특성](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)을 변경 하도록 선택 하는 경우 Bizagi Studio For Digital PROCESS Automation API에서 해당 특성에 따라 사용자 필터링을 지원 하는지 확인 해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
11. 디지털 프로세스 자동화를 위해 Bizagi Studio에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 설정 하려면 **설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 Bizagi Studio for Digital Process Automation에 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
