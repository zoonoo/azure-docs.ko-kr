...
* Azure AD에서 IamIP Patent Platform에 액세스할 수 있는 사용자를 제어합니다.
...
* IamIP Patent Platform SSO(Single Sign-On)를 사용하도록 설정된 구독

## <a name="scenario-description"></a>시나리오 설명

이 자습서에서는 테스트 환경에서 Azure AD SSO를 구성하고 테스트합니다.

* IamIP Patent Platform에서 **SP 및 IDP** 시작 SSO를 지원합니다.
* IamIP Patent Platform이 구성되면 세션 제어를 적용하여 조직의 중요한 데이터의 반출 및 반입을 실시간으로 보호할 수 있습니다. 세션 제어는 조건부 액세스에서 확장됩니다. [Microsoft Cloud App Security를 사용하여 세션 제어를 적용하는 방법을 알아봅니다](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app).


## <a name="adding-iamip-patent-platform-from-the-gallery"></a>갤러리에서 IamIP Patent Platform 추가
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-iamip-patent-platform"></a>IamIP Patent Platform에 대한 Azure AD Single Sign-On 구성 및 테스트

**B.Simon**이라는 테스트 사용자를 사용하여 IamIP Patent Platform에서 Azure AD SSO를 구성하고 테스트합니다. SSO가 작동하려면 Azure AD 사용자와 IamIP Patent Platform의 관련 사용자 간에 연결 관계를 설정해야 합니다.

IamIP Patent Platform에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료합니다.
...
1. **[IamIP Patent Platform SSO 구성](#configure-iamip-patent-platform-sso)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
    * **[IamIP Patent Platform 테스트 사용자 만들기](#create-iamip-patent-platform-test-user)** - B.Simon의 Azure AD 표현과 연결된 해당 사용자를 IamIP Patent Platform에 만듭니다.
...
1. [Azure Portal](https://portal.azure.com/)의 **IamIP Patent Platform** 애플리케이션 통합 페이지에서 **관리** 섹션을 찾고, **Single Sign-On**을 선택합니다.
...
1. **SP** 시작 모드에서 애플리케이션을 구성하려면 **추가 URL 설정**를 클릭하고 다음 단계를 수행합니다.
    ...
    **로그온 URL** 텍스트 상자에 `https://patents.iamip.com/login-user` URL을 입력합니다.
...
1. **IamIP Patent Platform 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
이 섹션에서는 Azure Single Sign-On을 사용할 수 있도록 B.Simon에게 IamIP Patent Platform에 대한 액세스 권한을 부여합니다.
...
**IamIP Patent Platform** 쪽에서 Single Sign-On을 구성하려면 Azure Portal에서 다운로드한 **인증서(원시)** 와 적절히 복사한 URL을 [IamIP Patent Platform 지원 팀](mailto:info@iamip.com)에 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.

### <a name="create-iamip-patent-platform-test-user"></a>IamIP Patent Platform 테스트 사용자 만들기

이 섹션에서는 IamIP Patent Platform에서 B.Simon이라는 사용자를 만듭니다.  [IamIP Patent Platform 지원 팀](mailto:info@iamip.com)과 협력하여 사용자를 IamIP Patent Platform 플랫폼에 추가합니다. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.

## <a name="test-sso"></a>SSO 테스트 

이 섹션에서는 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트합니다.

액세스 패널에서 IamIP Patent Platform 타일을 클릭하면 SSO를 설정한 IamIP Patent Platform에 자동으로 로그인됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
