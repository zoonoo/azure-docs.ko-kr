...
ms.locfileid: "59272498"
---
# <a name="tutorial-azure-active-directory-integration-with-tas"></a>자습서: TAS와 Azure Active Directory 통합

이 자습서에서는 Azure AD(Azure Active Directory)와 TAS를 통합하는 방법에 대해 알아봅니다.
TAS를 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.

* Azure AD에서 TAS에 액세스할 수 있는 사용자를 제어할 수 있습니다.
* 사용자가 해당 Azure AD 계정으로 TAS에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
* TAS Single Sign-On을 사용하도록 설정된 구독
...
## <a name="adding-tas-from-the-gallery"></a>갤러리에서 TAS 추가

TAS가 Azure AD에 통합되도록 구성하려면 갤러리의 TAS를 관리형 SaaS 앱 목록에 추가해야 합니다.

**갤러리에서 TAS를 추가하려면 다음 단계를 수행합니다.**
...
4. 검색 상자에서 **TAS**를 입력하고, 결과 패널에서 **TAS**를 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.

     ![결과 목록의 TAS](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성 및 테스트

이 섹션에서는 **Britta Simon**이라는 테스트 사용자를 기반으로 하여 TAS에서 Azure AD Single Sign-On을 구성하고 테스트합니다.
Single Sign-On이 작동하려면 Azure AD 사용자와 TAS의 관련 사용자 간에 연결 관계를 설정해야 합니다.

TAS에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.

1. **[Azure AD Single Sign-On 구성](#configure-azure-ad-single-sign-on)** - 사용자가 이 기능을 사용할 수 있도록 합니다.
2. **[TAS Single Sign-On 구성](#configure-tas-single-sign-on)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
3. **[Azure AD 테스트 사용자 만들기](#create-an-azure-ad-test-user)** - Britta Simon으로 Azure AD Single Sign-On을 테스트하는 데 사용합니다.
4. **[Azure AD 테스트 사용자 할당](#assign-the-azure-ad-test-user)** - Britta Simon이 Azure AD Single Sign-on을 사용할 수 있도록 합니다.
5. **[TAS 테스트 사용자 만들기](#create-tas-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 TAS에 만듭니다.
...
TAS에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.

1. [Azure Portal](https://portal.azure.com/)의 **TAS** 애플리케이션 통합 페이지에서 **Single Sign-On**을 선택합니다.
...
4. **IDP** 시작 모드에서 애플리케이션을 구성하려면 **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![TAS 도메인 및 URL Single Sign-On 정보](common/idp-intiated.png)

    a. **식별자** 텍스트 상자에서 다음 패턴을 사용하여 URL을 입력합니다. `https://taseu.combtas.com/<DOMAIN>`

    b. **회신 URL** 텍스트 상자에 다음 패턴을 사용하여 URL을 입력합니다. `https://taseu.combtas.com/<ENVIRONMENTNAME>/AssertionService.aspx`
...
### <a name="configure-tas-single-sign-on"></a>TAS Single Sign-On 구성
...
2. 메뉴의 왼쪽에서 **Settings**(설정)를 클릭하고, **Administrator**(관리자)로 이동한 다음, **Manage Single sign on**(Single Sign-On 관리)을 클릭합니다.
    ...
    ![TAS 구성](./media/tas-tutorial/configure01.png)

3. **Manage Single sign on**(Single Sign-On 관리) 페이지에서 다음 단계를 수행합니다.

    ![TAS 구성](./media/tas-tutorial/configure02.png)
    ...
    d. Azure Portal에서 다운로드한 base-64로 인코딩된 인증서를 메모장에서 열고, 해당 콘텐츠를 복사한 다음,  **Enter Certification** (인증 입력) 상자에 붙여넣습니다.
...
2. 애플리케이션 목록에서 **TAS**를 선택합니다.

    ![애플리케이션 목록의 TAS 링크](common/all-applications.png)
...
액세스 패널에서 TAS 타일을 클릭하면 SSO를 설정한 TAS에 자동으로 로그인됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
