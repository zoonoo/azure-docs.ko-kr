---
title: '자습서: Azure Active Directory를 사용 하 고 자동 사용자 프로비저닝에 대 한 깜박임 구성 | 마이크로 소프트 문서'
description: Azure Active Directory를 구성하여 사용자 계정을 깜박임에 자동으로 프로비전하고 프로비전 해제하도록 구성하는 방법을 알아봅니다.
...
* Azure AD 테넌트
* [깜박임 테넌트](https://joinblink.com/pricing)
* 관리자 권한이 있는 깜박임의 사용자 계정입니다.
...
* 단일 Azure AD 사용자가 자동 사용자 프로비전 구성을 테스트하기 위해 Blink에 할당되는 것이 좋습니다. 추가 사용자 및/또는 그룹은 나중에 할당할 수도 있습니다.

* 사용자를 Blink에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램 별 역할(사용 가능한 경우)을 선택해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비저닝에서 제외됩니다.
...
## <a name="add-blink-from-the-gallery"></a>갤러리에서 깜박임 추가

Azure AD를 사용하여 자동 사용자 프로비전을 위해 Blink를 구성하기 전에 Azure AD 응용 프로그램 갤러리에서 깜박임을 관리되는 SaaS 응용 프로그램 목록에 추가해야 합니다.

**Azure AD 응용 프로그램 갤러리에서 깜박임을 추가하려면 다음 단계를 수행합니다.**
...
4. 검색 상자에서 **깜박임을**입력하고 결과 패널에서 **깜박임을** 선택한 다음 **추가** 단추를 클릭하여 응용 프로그램을 추가합니다.
...
2. 애플리케이션 목록에서 **Blink**를 선택합니다.
    ...
    ![응용 프로그램 목록의 깜박임 링크](common/all-applications.png)
...
5. 관리자 **자격 증명** 섹션에서 `https://api.joinblink.com/scim` **테넌트 URL에**입력합니다. **비밀** **토큰의** 앞에서 검색한 SCIM 인증 토큰 값을 입력합니다. Azure AD가 깜박임에 연결할 수 있는지 확인하려면 **테스트 연결을** 클릭합니다. 연결이 실패하면 Blink 계정에 관리자 권한이 있는지 확인하고 다시 시도하십시오.
...
8. **매핑** 섹션에서 **Azure Active Directory 사용자 동기화를 선택하여 깜박입니다.**

    ![사용자 매핑 깜박임](media/blink-provisioning-tutorial/User_mappings.png)
...
11. 깜박임에 대한 Azure AD 프로비저닝 서비스를 사용하려면 **설정** 섹션에서 **프로비저닝 상태를** **켜기로** 변경합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **Scope에서** 원하는 값을 선택하여 깜박임에 프로비전할 사용자를 정의합니다.
