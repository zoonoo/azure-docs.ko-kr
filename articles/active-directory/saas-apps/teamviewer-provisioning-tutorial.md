...
이 자습서에서는 자동 사용자 프로비저닝을 구성하기 위해 TeamViewer 및 Azure AD(Azure Active Directory)에서 수행해야 하는 단계를 설명합니다. 구성된 경우 Azure AD는 Azure AD 프로비저닝 서비스를 사용하여 사용자 및 그룹을 [TeamViewer](https://www.teamviewer.com/buy-now/)로 자동으로 프로비저닝 및 프로비저닝 해제합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../app-provisioning/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원되는 기능
> [!div class="checklist"]
> * TeamViewer에서 사용자 만들기
> * TeamViewer에서 더 이상 액세스할 필요가 없는 사용자 제거
> * Azure AD와 TeamViewer 간에 사용자 특성을 동기화된 상태로 유지
> * TeamViewer에 대한 [Single Sign-On](./teamviewer-tutorial.md)(권장)
...
3. [Azure AD와 TeamViewer 간에 매핑](../app-provisioning/customize-application-attributes.md)할 데이터를 결정합니다. 

## <a name="step-2-configure-teamviewer-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD에서 프로비저닝을 지원하도록 TeamViewer 구성

1. [TeamViewer 관리 콘솔](https://login.teamviewer.com)에 로그인합니다. **프로필 편집** 으로 이동합니다.

    ![TeamViewer 관리자 콘솔](./media/teamviewer-provisioning-tutorial/admin.png)

2.  **앱** 으로 이동합니다. **스크립트 토큰 만들기** 를 클릭합니다.

    ![TeamViewer 토큰 만들기](./media/teamviewer-provisioning-tutorial/createtoken.png)

3.  스크립트 토큰의 이름을 제공합니다. **저장** 단추를 클릭합니다.

    ![TeamViewer 토큰 이름](./media/teamviewer-provisioning-tutorial/tokenname.png)

4. **토큰** 을 복사하고 **확인** 을 클릭합니다. 이 값은 Azure Portal에서 TeamViewer 애플리케이션의 **비밀 토큰** 필드에 입력됩니다.

    ![TeamViewer 토큰](./media/teamviewer-provisioning-tutorial/token.png)

## <a name="step-3-add-teamviewer-from-the-azure-ad-application-gallery"></a>3단계: Azure AD 애플리케이션 갤러리에서 TeamViewer 추가

Azure AD 애플리케이션 갤러리에서 TeamViewer를 추가하여 TeamViewer에 대한 프로비저닝 관리를 시작합니다. 이전에 SSO에 대해 TeamViewer 설정한 경우 동일한 애플리케이션을 사용할 수 있습니다. 그러나 처음 통합을 테스트하는 경우 별도의 앱을 만드는 것이 좋습니다. [여기](../manage-apps/add-application-portal.md)를 클릭하여 갤러리에서 애플리케이션을 추가하는 방법에 대해 자세히 알아봅니다. 
...
* 사용자 및 그룹을 TeamViewer에 할당하는 경우 **기본 액세스** 이외의 역할을 선택해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 역할을 더 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-teamviewer"></a>5단계. TeamViewer에 대한 자동 사용자 프로비저닝 구성 

이 섹션에서는 Azure AD의 사용자 및/또는 그룹 할당에 따라 TestApp에서 사용자 및/또는 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-teamviewer-in-azure-ad"></a>Azure AD에서 TeamViewer에 대한 자동 사용자 프로비저닝을 구성하려면 다음을 수행합니다.
...
2. 애플리케이션 목록에서 **TeamViewer** 를 선택합니다.
    ...
    ![애플리케이션 목록의 TeamViewer 링크](common/all-applications.png)
...
8. **매핑** 섹션에서 **Azure Active Directory 사용자를 TeamViewer에 동기화** 를 선택합니다.

9. **특성-매핑** 섹션에서 Azure AD에서 TeamViewer로 동기화되는 사용자 특성을 검토합니다. **일치** 속성으로 선택한 특성은 업데이트 작업 시 TeamViewer의 사용자 계정을 일치시키는 데 사용됩니다. [일치하는 대상 특성](../app-provisioning/customize-application-attributes.md)을 변경하는 경우 TeamViewer API에서 해당 특성에 따라 사용자 필터링을 지원하는지 확인해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
11. TeamViewer에 대한 Azure AD 프로비저닝 서비스를 사용하도록 설정하려면 **설정** 섹션에서 **프로비전 상태** 를 **켜기** 로 변경합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택하여 TeamViewer에 프로비저닝하려는 사용자 및/또는 그룹을 정의합니다.
