...
4. 검색 상자에 **Zscaler**를 입력하고 결과 패널에서 **Zscaler**를 선택한 후 **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
    ...
    ![결과 목록의 Zscaler](./media/zscaler-tutorial/tutorial_zscaler_addfromgallery.png)
...
3. **[Zscaler 테스트 사용자 만들기](#creating-a-zscaler-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Zscaler에 만듭니다.
4. **[Azure AD 테스트 사용자 할당](#assigning-the-azure-ad-test-user)** - Britta Simon이 Azure AD Single Sign-on을 사용할 수 있도록 합니다.
5. **[Single Sign-On 테스트](#testing-single-sign-on)** - 구성이 작동하는지 확인합니다.

### <a name="configuring-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성

이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 Zscaler 애플리케이션에서 Single Sign-On을 구성합니다.

**Zscaler에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **Zscaler** 애플리케이션 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![Zscaler 도메인 및 URL Single Sign-On 정보](./media/zscaler-tutorial/tutorial_zscaler_url.png)

    **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<companyname>.zscaler.net`

    > [!NOTE] 
    > 이 값은 실제 값이 아닙니다. 이 값을 실제 로그온 URL로 업데이트합니다. 이 값을 얻으려면 [Zscaler 클라이언트 지원 팀](https://www.zscaler.com/company/contact)에 문의하세요.

5. Zscaler 애플리케이션에는 특정 형식의 SAML 어설션이 필요합니다. 이 애플리케이션에 대해 다음 클레임을 구성합니다. 애플리케이션 통합 페이지의 **사용자 특성 및 클레임** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. **SAML로 Single Sign-On 설정 페이지**에서 **편집** 단추를 클릭하여 **사용자 특성 및 클레임** 대화 상자를 엽니다.

    ![특성 링크](./media/zscaler-tutorial/tutorial_zscaler_attribute.png)
...
7. **SAML 서명 인증서** 페이지의 **SAML 서명 인증서** 섹션에서 **다운로드**를 클릭하고 **인증서(Base64)** 다운로드한 다음, 컴퓨터에 인증서 파일을 저장합니다.
    ...
    ![인증서 다운로드 링크](./media/zscaler-tutorial/tutorial_zscaler_certificate.png) 

8. **Zscaler 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
    ...
    ![Zscaler 구성](common/configuresection.png)

9. 다른 웹 브라우저 창에서 ZScaler 회사 사이트에 관리자로 로그인합니다.
...
이 섹션은 Zscaler에서 Britta Simon이라는 사용자를 만들기 위한 것입니다. Zscaler는 적시에 프로비전을 지원하며 기본적으로 사용하도록 설정합니다. 이 섹션에 작업 항목이 없습니다. 새 사용자가 아직 존재하지 않는 경우 Zscaler에 액세스하는 동안 만들어질 수 있습니다.
>[!Note]
>사용자를 수동으로 만들어야 하는 경우  [Zscaler 지원 팀](https://www.zscaler.com/company/contact)에 문의하세요.
...
5. **사용자 및 그룹** 대화 상자의 목록에서 **Britta Simon** 등의 사용자를 선택하고 화면 맨 아래에서 **선택** 단추를 클릭합니다.
...
액세스 패널에서 Zscaler 타일을 클릭하면 Zscaler 애플리케이션에 자동으로 로그온됩니다.
