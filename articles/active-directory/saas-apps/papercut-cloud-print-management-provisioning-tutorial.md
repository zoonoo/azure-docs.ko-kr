---
title: '자습서: Azure Active Directory을 사용 하 여 자동 사용자 프로 비전을 위한 용지 자르기 클라우드 인쇄 관리 구성 Microsoft Docs'
description: Azure AD에서 사용자 계정을 자동으로 프로 비전 및 프로 비전 해제 하 여 클라우드 인쇄 관리를 제거 하는 방법을 알아봅니다.
...
이 자습서에서는 자동 사용자 프로 비전을 구성 하기 위해 용지 잘라내기 클라우드 인쇄 관리 및 Azure Active Directory (Azure AD)에서 수행 해야 하는 단계를 설명 합니다. 구성 된 경우 Azure AD는 사용자와 그룹을 자동으로 프로 비전 하 고 프로 비전 하 여 Azure AD 프로 비전 서비스를 사용 하 여 [클라우드 인쇄 관리를 잘라냅니다](https://www.papercut.com/products/papercut-pocket/) . 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../app-provisioning/user-provisioning.md)를 참조하세요.

## <a name="capabilities-supported"></a>지원되는 기능

> [!div class="checklist"]
> * 용지에서 사용자 만들기 클라우드 인쇄 관리
...
3. [AZURE AD와 용지 잘라내기 클라우드 인쇄 관리 간에 매핑할](../app-provisioning/customize-application-attributes.md)데이터를 결정 합니다.
...
4. 설치 되 면 추가 정보 페이지에 **테 넌 트 URL** 및 **암호 토큰이** 표시 됩니다. 이러한 값은 \* \* Azure Portal의 용지 잘라내기 클라우드 인쇄 관리 응용 프로그램의 프로 비전 탭에 있는 테 넌 트 URL 필드 및 비밀 토큰 필드에 입력 됩니다.
...
* 사용자 및 그룹을 용지에 할당 하 여 클라우드 인쇄 관리를 잘라낼 때 **기본 액세스** 이외의 다른 역할을 선택 해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 역할을 더 추가할 수 있습니다.
...
2. 응용 프로그램 목록에서 **용지 잘라내기 클라우드 인쇄 관리** 를 선택 합니다.
...
5. **관리자 자격 증명** 섹션 아래에서 용지 잘라내기 클라우드 인쇄 관리 테 넌 트 URL 및 비밀 토큰을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 용지 잘라내기 클라우드 인쇄 관리에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 용지 잘라내기 클라우드 인쇄 관리 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
...
11. 용지 잘림 클라우드 인쇄 관리에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 **설정 하려면 설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 용지에서 클라우드 인쇄 관리를 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
