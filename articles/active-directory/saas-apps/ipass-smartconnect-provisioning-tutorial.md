---
title: '자습서: Azure Active Directory을 사용 하 여 자동 사용자 프로 비전을 위한 iPass SmartConnect 구성 Microsoft Docs'
...
이 자습서에서는 사용자 및/또는 그룹을 iPass SmartConnect로 자동으로 프로 비전 및 프로 비전 해제 하도록 Azure AD를 구성 하기 위해 iPass SmartConnect 및 Azure Active Directory (Azure AD)에서 수행 하는 단계를 보여 줍니다.
...
자동 사용자 프로비저닝을 구성 하 고 사용 하도록 설정 하기 전에 iPass SmartConnect에 대 한 액세스가 필요한 Azure AD의 사용자 및/또는 그룹을 결정 해야 합니다. 일단 결정 되 면 다음 지침에 따라 이러한 사용자 및/또는 그룹을 iPass SmartConnect에 할당할 수 있습니다.
* [엔터프라이즈 앱에 사용자 또는 그룹 할당](../manage-apps/assign-user-or-group-access-portal.md)

## <a name="important-tips-for-assigning-users-to-ipass-smartconnect"></a>IPass SmartConnect에 사용자를 할당 하기 위한 주요 팁

* 자동 사용자 프로 비전 구성을 테스트 하려면 단일 Azure AD 사용자를 iPass SmartConnect에 할당 하는 것이 좋습니다. 추가 사용자 및/또는 그룹은 나중에 할당할 수도 있습니다.

* 사용자를 iPass SmartConnect에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할 (사용 가능한 경우)을 선택 해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비전에서 제외됩니다.

## <a name="setup-ipass-smartconnect-for-provisioning"></a>프로 비전을 위한 iPass SmartConnect 설정
...
Azure AD를 사용한 자동 사용자 프로 비전을 위해 iPass SmartConnect를 구성 하려면 Azure AD 응용 프로그램 갤러리의 iPass SmartConnect를 관리 되는 SaaS 응용 프로그램 목록에 추가 해야 합니다.

**Azure AD 응용 프로그램 갤러리에서 iPass SmartConnect를 추가 하려면 다음 단계를 수행 합니다.**
...
4. 검색 상자에 **Ipass smartconnect** 를 입력 하 고 결과 패널에서 **ipass smartconnect** 를 선택한 다음 **추가** 단추를 클릭 하 여 응용 프로그램을 추가 합니다.

    ![결과 목록의 iPass SmartConnect](common/search-new-app.png)

## <a name="configuring-automatic-user-provisioning-to-ipass-smartconnect"></a>IPass SmartConnect에 자동 사용자 프로 비전 구성 
...
5. **관리자 자격 증명** 섹션 아래에 `https://openmobile.ipass.com/moservices/scim/v1` 있는 **테 넌 트 URL** 에 입력 합니다. 이전에 **비밀 토큰** 에서 검색 된 전달자 토큰을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 Ipass smartconnect에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 iPass SmartConnect 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
...
9. **특성 매핑** 섹션에서 Azure AD에서 Ipass smartconnect로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 Ipass smartconnect의 사용자 계정을 일치 시키는 데 사용 됩니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
    ...
    :::image type="content" source="media/ipass-smartconnect-provisioning-tutorial/userattribute.png" alt-text="특성 매핑 페이지의 스크린샷 테이블에 Azure Active Directory 및 iPass SmartConnect 특성과 일치 하는 우선 순위가 나열 됩니다." border="false":::
...
11. IPass SmartConnect에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 **설정 하려면 설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 smartconnect에 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
...
이 작업은 **설정** 의 **범위** 섹션에 정의된 모든 사용자 및/또는 그룹의 초기 동기화를 시작합니다. 초기 동기화는 Azure AD 프로비전 서비스가 실행되는 동안 약 40분마다 발생하는 후속 동기화보다 더 많은 시간이 걸립니다. **동기화 세부 정보** 섹션을 사용 하 여 진행률을 모니터링 하 고 프로 비전 활동 보고서에 대 한 링크를 따라 Ipass smartconnect에서 Azure AD 프로 비전 서비스에서 수행 하는 모든 작업을 설명 합니다.

Azure AD 프로비저닝 로그를 읽는 방법에 대한 자세한 내용은 [자동 사용자 계정 프로비저닝에 대한 보고](../app-provisioning/check-status-user-account-provisioning.md)를 참조하세요.

## <a name="connector-limitations"></a>커넥터 제한 사항

* iPass SmartConnect는 해당 도메인이 iPass SmartConnect 관리 콘솔에 등록 된 사용자 이름을 허용 합니다.  
