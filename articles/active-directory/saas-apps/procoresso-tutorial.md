...
2. 도구 상자 드롭다운에서 **관리** 를 클릭하여 SSO 설정 페이지를 엽니다.
    ...
    ![스크린샷은 디렉터리가 선택된 Procore 회사 사이트를 보여줍니다.](./media/procoresso-tutorial/procore_tool_admin.png)

3. 아래 설명에 따라 값을 상자에 붙여 넣습니다.

    ![스크린샷은 사용자 추가 대화 상자를 보여줍니다.](./media/procoresso-tutorial/procore_setting_admin.png) 
...
액세스 패널에서 Procore SSO 타일을 클릭하면 SSO를 설정한 Procore SSO에 자동으로 로그인되어야 합니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](../user-help/my-apps-portal-end-user-access.md)를 참조하세요.
