...
1. 결과 패널에서 **PolicyStat**를 선택하고 **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 PolicyStat 애플리케이션에서 Single Sign-On을 구성합니다.

**PolicyStat에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **PolicyStat** 애플리케이션 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
1. 이 섹션은 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD의 계정으로 PolicyStat에 인증할 수 있게 하는 방법을 간략하게 설명하기 위한 것입니다.
    ...
    PolicyStat 애플리케이션에는 특정 형식의 SAML 어설션이 필요하므로, **SAML 토큰 특성** 구성에 사용자 지정 특성 매핑을 추가해야 합니다.  
...
1. 애플리케이션 목록에서 **PolicyStat**를 선택합니다.
...
액세스 패널에서 PolicyStat 타일을 클릭하면 PolicyStat 애플리케이션에 자동으로 로그온됩니다.
