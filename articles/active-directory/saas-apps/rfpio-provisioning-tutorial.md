---
title: '자습서: Azure Active Directory를 사용 하 여 자동 사용자 프로 비전을 위한 RFPIO 구성 Microsoft Docs'
...
ms.locfileid: "68737811"
---
# <a name="tutorial-configure-rfpio-for-automatic-user-provisioning"></a>자습서: 자동 사용자 프로 비전을 위한 RFPIO 구성

이 자습서에서는 사용자 및/또는 그룹을 RFPIO로 자동으로 프로 비전 및 프로 비전 해제 하도록 Azure AD를 구성 하기 위해 RFPIO and Azure Active Directory (Azure AD)에서 수행 하는 단계를 보여 줍니다.
...
* Azure AD 테넌트.
* [RFPIO 테 넌 트](https://www.rfpio.com/product/)입니다.
...
## <a name="important-tips-for-assigning-users-to-rfpio"></a>RFPIO에 사용자를 할당 하기 위한 주요 팁

* 자동 사용자 프로 비전 구성을 테스트 하기 위해 단일 Azure AD 사용자를 RFPIO에 할당 하는 것이 좋습니다. 추가 사용자 및/또는 그룹은 나중에 할당할 수도 있습니다.

* 사용자를 RFPIO에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할 (사용 가능한 경우)을 선택 해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비전에서 제외됩니다.

## <a name="setup-rfpio-for-provisioning"></a>프로 비전을 위한 RFPIO 설정
...
Azure AD를 사용한 자동 사용자 프로 비전을 위해 RFPIO를 구성 하려면 Azure AD 응용 프로그램 갤러리의 RFPIO를 관리 되는 SaaS 응용 프로그램 목록에 추가 해야 합니다.
...
## <a name="configuring-automatic-user-provisioning-to-rfpio"></a>RFPIO에 자동 사용자 프로 비전 구성 

이 섹션에서는 azure ad의 사용자 및/또는 그룹 할당에 따라 RFPIO에서 사용자 및/또는 그룹을 만들고, 업데이트 하 고, 사용 해제 하도록 Azure AD 프로 비전 서비스를 구성 하는 단계를 안내 합니다.

> [!TIP]
> [RFPIO single sign-on 자습서](rfpio-tutorial.md)에 제공 된 지침에 따라 RFPIO에 대해 SAML 기반 single sign-on을 사용 하도록 선택할 수도 있습니다. Single Sign-On과 자동 사용자 프로비저닝은 서로 보완적이지만, 별개로 구성할 수 있습니다.
...
5. **관리자 자격 증명** 섹션 아래에 있는 `https://<RFPIO tenant instance>.rfpio.com/rfpserver/scim/v2 ` **테 넌 트 URL**에 입력 합니다. 예제 값은 `https://Azure-test1.rfpio.com/rfpserver/scim/v2`입니다. **비밀 토큰**에서 이전에 검색 한 **Scim API 토큰** 값을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 RFPIO에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 RFPIO 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 합니다.
...
9. **특성 매핑** 섹션에서 Azure AD에서 RFPIO로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 RFPIO의 사용자 계정을 일치 시키는 데 사용 됩니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
11. RFPIO에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 **설정 하려면 설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로 비전 상태 전환 설정](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 RFPIO에 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
...
* RFPIO는 현재 그룹 프로 비전을 지원 하지 않습니다.
