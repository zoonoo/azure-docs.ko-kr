...
6. **편집** 아이콘을 클릭하여 **이름 식별자 값** 을 편집합니다.
    ...
    !["편집" 아이콘이 선택된 "사용자 특성 및 클레임" 페이지를 보여주는 스크린샷.](media/sapbusinessbydesign-tutorial/mail-prefix1.png)
...
3. **새 ID 공급자** 를 클릭하고 Azure Portal에서 다운로드한 메타데이터 XML 파일을 선택합니다. 시스템은 메타데이터를 가져와서 필수 서명 인증서 및 암호화 인증서를 자동으로 업로드합니다.
    ...
    !["ID 공급자" 탭이 선택되고 "새 ID 공급자" 작업이 선택된 "Single Sign-On 구성" 페이지를 보여주는 스크린샷.](./media/sapbusinessbydesign-tutorial/tutorial_sapbusinessbydesign_54.png)
...
7. **내 시스템** 탭을 클릭합니다.
    ...
    !["내 시스템" 탭이 선택된 "Single Sign-On 구성" 페이지를 보여주는 스크린샷.](./media/sapbusinessbydesign-tutorial/tutorial_sapbusinessbydesign_52.png)

8. **Azure AD 로그온 URL** 텍스트 상자에 Azure Portal에서 복사한 **로그인 URL** 값을 붙여넣습니다.

    ![Single Sign-on 구성](./media/sapbusinessbydesign-tutorial/tutorial_sapbusinessbydesign_53.png)
...
액세스 패널에서 SAP Business ByDesign 타일을 클릭하면 SSO를 설정한 SAP Business ByDesign에 자동으로 로그인됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](../user-help/my-apps-portal-end-user-access.md)를 참조하세요.
