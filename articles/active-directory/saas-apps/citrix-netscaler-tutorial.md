---
title: '자습서: Citrix ADC와 Azure Active Directory Single Sign-On 통합(Kerberos 기반 인증) | Microsoft Docs'
description: Kerberos 기반 인증을 사용하여 Azure Active Directory와 Citrix ADC 간에 SSO(Single Sign-On)를 구성하는 방법을 알아봅니다.
...
* [Citrix ADC에 대한 Kerberos 기반 인증](#publish-the-web-server)

* [Citrix ADC에 대한 헤더 기반 인증](header-citrix-netscaler-tutorial.md#publish-the-web-server)
...
- [Kerberos 기반 인증을 위한 Citrix ADC SSO 구성](#publish-the-web-server)

- [헤더 기반 인증을 위한 Citrix ADC SSO 구성](header-citrix-netscaler-tutorial.md#publish-the-web-server)
...
1. **추가** 를 선택합니다.
    ...
    ![Citrix ADC 구성 - Services 창](./media/citrix-netscaler-tutorial/web01.png)
...
1. **확인** 을 선택합니다.
    ...
    ![Citrix ADC 구성 - Basic Settings 창](./media/citrix-netscaler-tutorial/load01.png)
...
1. **Services and Service Groups(서비스 및 서비스 그룹)** 창에서 **No Load Balancing Virtual Server Service Binding(부하 분산 가상 서버 서비스 바인딩 없음)** 을 선택합니다.
   ...
   ![Citrix ADC 구성 - Load Balancing Virtual Server Service Binding 창](./media/citrix-netscaler-tutorial/bind01.png)

1. 다음 스크린샷과 같이 설정을 확인한 다음, **닫기** 를 선택합니다.

   ![Citrix ADC 구성 - 가상 서버 서비스 바인딩 확인](./media/citrix-netscaler-tutorial/bind02.png)
...
1. **인증서** 아래에서 **서버 인증서 없음** 을 선택합니다.
   ...
   ![Citrix ADC 구성 - 서버 인증서 창](./media/citrix-netscaler-tutorial/bind03.png)

1. 다음 스크린샷과 같이 설정을 확인한 다음, **닫기** 를 선택합니다.

   ![Citrix ADC 구성 - 인증서 확인](./media/citrix-netscaler-tutorial/bind04.png)
...
1. **정책 바인딩** 창에서 인증 정책을 선택한 다음, **바인딩** 를 선택합니다.
    ...
    ![Citrix ADC 구성 - Policy Binding 창](./media/citrix-netscaler-tutorial/virtual02.png)
...
1. **바인딩** 을 선택합니다.
    ...
    ![Citrix ADC 구성 - Load Balancing Virtual Server Binding 창](./media/citrix-netscaler-tutorial/virtual04.png)
...
1. 변경 내용을 확인하려면 브라우저에서 애플리케이션 URL로 이동합니다. 이전에 확인한 인증되지 않은 액세스 대신 테넌트 로그인 페이지가 표시됩니다.
    ...
    ![Citrix ADC 구성 - 웹 브라우저의 로그인 페이지](./media/citrix-netscaler-tutorial/virtual05.png)

## <a name="configure-citrix-adc-sso-for-kerberos-based-authentication"></a>Kerberos 기반 인증을 위한 Citrix ADC SSO 구성
...
1. 사용자 계정을 만듭니다(이 예제의 경우, _AppDelegation_ 사용).
    ...
    ![Citrix ADC 구성 - Properties 창](./media/citrix-netscaler-tutorial/kerberos01.png)
...
1. 다음 스크린샷에 표시된 것처럼 웹 서버에 대한 위임을 구성합니다.
    ...
    ![Citrix ADC 구성 - Properties 창의 Delegation](./media/citrix-netscaler-tutorial/kerberos02.png)

    > [!NOTE]
    > 스크린샷 예제에서 WIA(Windows 통합 인증) 사이트를 실행하는 내부 웹 서버 이름은 _CWEB2_ 입니다.

### <a name="citrix-adc-aaa-kcd-kerberos-delegation-accounts"></a>Citrix ADC AAA KCD(Kerberos 위임 계정)

Citrix ADC AAA KCD 계정을 구성하려면 다음을 수행합니다.
...
1. **확인** 을 선택합니다.
    ...
    ![Citrix ADC 구성 - Configure KCD Accoun 창](./media/citrix-netscaler-tutorial/kerberos03.png)
...
1. **확인** 을 선택합니다.
    ...
    ![Citrix ADC 구성 - Configure Traffic Profile 창](./media/citrix-netscaler-tutorial/kerberos04.png)
...
1. **확인** 을 선택합니다.
    ...
    ![Citrix ADC 구성 - Configure Traffic Policy 창](./media/citrix-netscaler-tutorial/kerberos05.png)
...
1. **부하 분산 가상 서버** 창의 **고급 설정** 아래에서 **정책** 을 선택합니다. NetScaler 인스턴스에 대해 구성된 모든 정책이 목록에 표시됩니다.
    ...
    ![Citrix ADC 구성 - Load Balancing Virtual Server 창](./media/citrix-netscaler-tutorial/kerberos06.png)

    ![Citrix ADC 구성 - Policies 대화 상자](./media/citrix-netscaler-tutorial/kerberos07.png)

1.  이 가상 서버에 바인딩하려는 정책 이름 옆의 확인란을 선택합니다.
 
    ![Citrix ADC 구성 - Load Balancing Virtual Server Traffic Policy Binding 창](./media/citrix-netscaler-tutorial/kerberos09.png)
...
1. **형식 선택** 대화 상자에서
    ...
    ![Citrix ADC 구성 - Choose Type 창](./media/citrix-netscaler-tutorial/kerberos08.png)

1. 정책이 바인딩되면 **완료** 를 선택합니다.
 
    ![Citrix ADC 구성 - Policies 창](./media/citrix-netscaler-tutorial/kerberos10.png)

1. WIA 웹 사이트를 사용하여 바인딩을 테스트합니다.

    ![Citrix ADC 구성 - 웹 브라우저의 테스트 페이지](./media/citrix-netscaler-tutorial/kerberos11.png)    
