...
* Azure AD에서 Citrix NetScaler에 대한 액세스 권한이 있는 사용자를 제어할 수 있습니다.
* 사용자가 자신의 Azure AD 계정으로 Citrix NetScaler에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
* Citrix NetScaler Single Sign-On이 설정된 구독
...
이 섹션에서는 **Britta Simon**이라는 테스트 사용자를 기반으로 Citrix NetScaler에서 Azure AD Single Sign-On을 구성하고 테스트합니다.
...
1. 다른 웹 브라우저 창에서 Citrix NetScaler 테넌트에 관리자로 로그인합니다.
...
이 섹션에서는 Citrix NetScaler에서 Britta Simon이라는 사용자를 만듭니다. Citrix NetScaler는 기본적으로 사용하도록 설정되는 Just-In-Time 사용자 프로비저닝을 지원합니다. 이 섹션에 작업 항목이 없습니다. Citrix NetScaler에 사용자가 아직 없는 경우 인증 후에 새 사용자가 만들어집니다.
