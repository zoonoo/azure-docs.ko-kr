---
title: '자습서: Azure Active Directory로 자동 사용자 프로비저닝을 위한 Cisco 구성 | Microsoft Docs'
...
ms.locfileid: "42144280"
---
# <a name="tutorial-configure-cisco-webex-for-automatic-user-provisioning"></a>자습서: 자동 사용자 프로비저닝을 위한 Cisco Webex 구성


이 자습서에서는 사용자를 Cisco Webex에 자동으로 프로비전 및 프로비전 해제하도록 Azure AD(Azure Active Directory)를 구성하기 위해 Cisco Webex 및 Azure AD에서 수행하는 단계를 보여 줍니다.
...
> [!NOTE]
> Azure AD 프로비전 통합에는 Cisco Webex 팀에서 사용할 수 있는 [Cisco Webex 웹 서비스](https://developer.webex.com/getting-started.html)가 필요합니다.

## <a name="adding-cisco-webex-from-the-gallery"></a>갤러리에서 Cisco Webex 추가
Azure AD를 사용하여 사용자를 자동으로 프로비전하도록 Cisco Webex를 구성하기 전에, 먼저 Cisco Webex를 Azure AD 응용 프로그램 갤러리에서 관리되는 SaaS 응용 프로그램 목록에 추가해야 합니다.

**Azure AD 응용 프로그램 갤러리에서 Cisco Webex를 추가하려면 다음 단계를 수행합니다.**
...
3. Cisco Webex를 추가하려면 대화 상자의 위쪽에서 **새 응용 프로그램** 단추를 클릭합니다.

    ![새 응용 프로그램 단추][3]

4. 검색 상자에 **Cisco Webex**를 입력합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/AppSearch.png)

5. 결과 패널에서 **Cisco Webex**를 선택한 다음, **추가** 단추를 클릭하여 SaaS 응용 프로그램의 목록에 Cisco Webex를 추가합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/AppSearchResults.png)

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/AppCreation.png)
...
*   사용자를 Cisco Webex에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할(사용 가능한 경우)을 선택해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비전에서 제외됩니다.
...
2. SaaS 응용 프로그램 목록에서 Cisco Webex를 선택합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/Successcenter2.png)

3. **프로비전** 탭을 선택합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/ProvisioningTab.png)

4. **프로비전 모드**를 **자동**으로 설정합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/ProvisioningCredentials.png)

5. **관리자 자격 증명** 섹션 아래에서 Cisco Webex 계정의 **테넌트 URL** 및 **비밀 토큰**을 입력합니다.

    *   **테넌트 URL** 필드에서 테넌트에 대한 Cisco Webex SCIM API URL을 채웁니다. 이 URL은 `https://api.webex.com/v1/scim/[Tenant ID]/` 형식이며, 여기서 `[Tenant ID]`는 6단계에서 설명한 대로 영숫자 문자열입니다.

    *   **비밀 토큰** 필드에서 6단계에서 설명한 대로 비밀 토큰을 채웁니다.

1. Cisco Webex 계정에 대한 **테넌트 ID** 및 **비밀 토큰**은 [Cisco Webex 개발자 사이트](https://developer.webex.com/)에 관리자 계정으로 로그인하여 찾을 수 있습니다. 로그인되면 다음을 수행합니다.
    ...
    * [인증 섹션](https://developer.webex.com/getting-started.html#authentication)
    ![Cisco Webex 인증 토큰](./media/cisco-webex-provisioning-tutorial/SecretToken.png)까지 아래로 스크롤합니다.
    ...
    * [Get My Own Details page](https://developer.webex.com/endpoint-people-me-get.html)(내 세부 정보 가져오기 페이지)로 이동합니다.
        ...
        * "Bearer"라는 단어 뒤에 공백을 입력한 다음, 비밀 토큰을 ![Cisco Webex 인증 토큰](./media/cisco-webex-provisioning-tutorial/GetMyDetails.png) 권한 부여 필드에 붙여넣습니다.
        * [실행]을 클릭합니다.
    * 오른쪽의 응답 텍스트에서 **테넌트 ID**가 "orgId"로 표시됩니다.
...
1. 5단계에 표시된 필드를 채우면 **연결 테스트**를 클릭하여 Azure AD에서 Clarizen에 연결할 수 있는지 확인합니다. 연결이 실패하면 Cisco Webex 계정에 관리자 권한이 있는지 확인하고 다시 시도합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/TestConnection.png)

8. **알림 메일** 필드에 프로비저닝 오류 알림을 받을 개인 또는 그룹의 메일 주소를 입력하고, **오류가 발생할 경우, 메일 알림 보내기** 확인란을 선택합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/EmailNotification.png)
...
13. Cisco Webex에 Azure AD 프로비전 서비스를 사용하도록 설정하려면 **설정** 섹션에서 **프로비전 상태**를 **켜기**로 변경합니다.
    ...
    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/ProvisioningStatus.png)

14. **설정**의 **범위** 섹션에서 원하는 값을 선택하여 Cisco Webex에 프로비전하려는 사용자 및/또는 그룹을 정의합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/SyncScope.png)

15. 프로비전할 준비가 되면 **저장**을 클릭합니다.

    ![Cisco Webex 프로비전](./media/cisco-webex-provisioning-tutorial/Save.png)
...
* Cisco Webex는 현재 Cisco의 EFT(초기 필드 테스트) 단계에 있습니다. 자세한 내용은 [Cisco의 지원 팀](https://www.webex.co.in/support/support-overview.html)에 문의하세요. 
