---
title: '자습서: Azure Active Directory로 자동 사용자 프로비저닝을 위한 Olfeo SAAS 구성 | Microsoft Docs'
description: 사용자 계정을 Azure AD에서 Olfeo SAAS로 자동으로 프로비저닝 및 프로비저닝을 해제하는 방법을 알아봅니다.
...
> * Olfeo SAAS에서 더 이상 액세스할 필요가 없는 사용자 제거
> * Azure AD와 Olfeo SAAS 간에 사용자 특성을 동기화된 상태로 유지
> * Olfeo SAAS에서 그룹 및 그룹 구성원 프로비저닝
> * Olfeo SAAS에 대한 [Single sign-on](olfeo-saas-tutorial.md)(권장)
...
* [Olfeo SAAS 테넌트](https://www.olfeo.com/).
* 관리자 권한이 있는 Olfeo SAAS 사용자 계정
...
1. [Azure AD와 Olfeo SAAS 간에 매핑](../app-provisioning/customize-application-attributes.md)할 데이터를 결정합니다.

## <a name="step-2-configure-olfeo-saas-to-support-provisioning-with-azure-ad"></a>2단계. Azure AD에서 프로비저닝을 지원하도록 Olfeo SAAS 구성

1. Olfeo SAAS 관리 콘솔에 로그인합니다. 
1. **구성 > Annuaires** 로 이동합니다.
1. 새 디렉터리를 만들고 이름을 지정합니다.
1. **Azure** 공급자를 선택한 다음 **Cr�er** 를 클릭하여 새 디렉터리를 저장합니다. 
1. **동기화** 탭으로 이동하여 **테넌트 URL** 및 **Jeton 비밀** 을 확인합니다. 이러한 값은 Azure Portal에 있는 Olfeo SAAS 애플리케이션에 대한 프로비저닝 탭의 **테넌트 URL** 및 **비밀 토큰** 필드에 입력됩니다.
...
* 사용자 및 그룹을 Olfeo SAAS에 할당할 때 **기본 액세스** 이외의 역할을 선택해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 더 많은 역할을 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-olfeo-saas"></a>5단계. Olfeo SAAS에 대한 자동 사용자 프로비저닝 구성 

이 섹션에서는 Azure AD의 사용자 및 그룹 할당에 따라 Olfeo SAAS 앱에서 사용자 및 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-olfeo-saas-in-azure-ad"></a>Azure AD에서 Olfeo SAAS에 대한 자동 사용자 프로비저닝을 구성하려면 다음을 수행합니다.
...
1. 애플리케이션 목록에서 **Olfeo SAAS** 를 선택합니다.
    ...
    ![애플리케이션 목록의 Olfeo SAAS 연결](common/all-applications.png)
...
1. **관리자 자격 증명** 섹션에 Olfeo SAAS **테넌트 URL** 및 **비밀 토큰** 정보를 입력합니다. **연결 테스트** 를 선택하여 Azure AD에서 Olfeo SAAS에 연결할 수 있는지 확인합니다. 연결에 실패하면 Olfeo SAAS 계정에 관리자 권한이 있는지 확인하고 다시 시도합니다.
...
1. **매핑** 섹션에서 **Azure Active Directory 사용자를 Olfeo SAAS에 동기화** 를 선택합니다.

1. **특성 매핑** 섹션에서는 Azure AD에서 Olfeo SAAS로 동기화된 사용자 특성을 검토합니다. **일치** 속성으로 선택한 특성은 업데이트 작업 시 Olfeo SAAS의 사용자 계정을 일치시키는 데 사용됩니다. [일치하는 대상 특성](../app-provisioning/customize-application-attributes.md)을 변경하는 경우 Olfeo SAAS API에서 해당 특성에 따라 사용자 필터링을 지원하는지 확인해야 합니다. 변경 내용을 커밋하려면 **저장** 을 선택합니다.
...
1. **매핑** 섹션에서 **Azure Active Directory 그룹을 Olfeo SAAS로 동기화** 를 선택합니다.

1. **특성-매핑** 섹션에서, Azure AD에서 Olfeo SAAS로 동기화되는 그룹 특성을 검토합니다. **일치** 속성으로 선택된 특성은 업데이트 작업 시 Olfeo SAAS의 그룹을 일치시키는 데 사용됩니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
1. Olfeo SAAS에 대한 Azure AD 프로비저닝 서비스를 사용하도록 설정하려면 **설정** 섹션에서 **프로비저닝 상태** 를 **켜기** 로 변경합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

1. **설정** 의 **범위** 섹션에서 원하는 값을 선택하여 Olfeo SAAS에 프로비저닝하려는 사용자 또는 그룹을 정의합니다.
