...
이 자습서에서는 Azure AD(Azure Active Directory)와 GitHub Enterprise Managed User를 통합하는 방법에 대해 알아봅니다. Azure AD와 GitHub Enterprise Managed User를 통합하면 다음을 수행할 수 있습니다.
...
**B.Simon** 이라는 테스트 사용자를 사용하여 GitHub Enterprise Managed User에서 Azure AD SSO를 구성하고 테스트합니다. SSO가 작동하려면 Azure AD 사용자와 GitHub Enterprise Managed User의 관련 사용자 간에 연결 관계를 설정해야 합니다.
...
1. **[GitHub Enterprise Managed User SSO 구성](#configure-github-enterprise-managed-user-sso)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
    1. **[GitHub Enterprise Managed User 테스트 사용자 만들기](#create-github-enterprise-managed-user-test-user)** - B.Simon의 Azure AD 표현과 연결된 해당 사용자를 GitHub Enterprise Managed User에 만듭니다.
...
1. **IDP** 섹션에서 애플리케이션을 구성하려면 **기본 SAML 구성** 섹션에서 다음 필드 값을 입력합니다.
    ...
    a. **식별자** 텍스트 상자에서 `https://github.com/enterprise-managed/<ENTITY>` 패턴을 사용하여 URL을 입력합니다.
...
1. **SP** 시작 모드에서 애플리케이션을 구성하려면 **추가 URL 설정** 를 클릭하고 다음 단계를 수행합니다.
    ...
    **로그인 URL** 텍스트 상자에서 `https://github.com/enterprises/<ENTITY>/sso` 패턴을 사용하여 URL을 입력합니다.
    ...
    > [!NOTE]
    > 이러한 값은 실제 값이 아닙니다. 실제 식별자, 회신 URL 및 로그온 URL을 사용하여 이러한 값을 업데이트합니다. 이러한 값을 얻으려면 [GitHub Enterprise Managed User 클라이언트 지원 팀](mailto:support@github.com)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.

1. **SAML로 Single Sign-On 설정** 페이지의 **SAML 서명 인증서** 섹션에서 **인증서(Base64)** 를 찾은 후 **다운로드** 를 선택하여 인증서를 다운로드하고 컴퓨터에 저장합니다.

    ![인증서 다운로드 링크](common/certificatebase64.png)

1. **GitHub Enterprise Managed User 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
이 섹션에서는 B.Simon에게 GitHub Enterprise Managed User에 대한 액세스 권한을 부여하여 해당 사용자가 Azure Single Sign-On을 사용하도록 설정합니다.
...
**GitHub Enterprise Managed User** 쪽에서 Single Sign-On을 구성하려면 다운로드한 **인증서(Base64)** 및 Azure Portal에서 복사한 적절한 URL을 [GitHub Enterprise Managed User 지원 팀](mailto:support@github.com)에 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.

### <a name="create-github-enterprise-managed-user-test-user"></a>GitHub Enterprise Managed User 테스트 사용자 만들기
...
* Azure Portal에서 **이 애플리케이션 테스트** 를 클릭합니다. 그러면 로그인 흐름을 시작할 수 있는 GitHub Enterprise Managed User 로그온 URL로 리디렉션됩니다.  

* GitHub Enterprise Managed User 로그온 URL로 직접 이동하여 해당 위치에서 로그인 흐름을 시작합니다.
