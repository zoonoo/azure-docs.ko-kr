---
title: '자습서: Azure Active Directory를 사용 하 여 자동 사용자 프로 비전을 위한 BitaBIZ 구성 Microsoft Docs'
...
* Azure AD 테넌트.
* [BitaBIZ 테 넌 트](https://bitabiz.dk/en/price/)입니다.
...
> [!TIP]
> [BitaBIZ single sign-on 자습서](BitaBIZ-tutorial.md)에 제공 된 지침에 따라 BitaBIZ에 대해 SAML 기반 single sign-on을 사용 하도록 선택할 수도 있습니다. Single sign-on은 자동 사용자 프로 비전과 독립적으로 구성 될 수 있지만,이 두 가지 기능은 서로 보완 됩니다.
...
5. 관리자 자격 증명 섹션 아래에서 먼저 테 넌 트 URL 및 암호 토큰에서 검색 된 **Scim 프로 비전 끝점 url** 및 **전달자 토큰** 값을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 BitaBIZ에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 BitaBIZ 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 합니다.
