...
1. 메타데이터 인증서는 나중에 설정 프로세스에서 사용할 F5(Kerberos)로 가져와야 합니다. **시스템 > 인증서 관리 > 트래픽 인증서 관리 >> SSL 인증서 목록**으로 차례로 이동합니다. 오른쪽 모서리의 **가져오기**를 클릭합니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure01.png) 

1. 또한 호스트 이름(`Kerbapp.superdemo.live`)에 대한 **SSL 인증서**도 필요합니다. 이 예제에서는 와일드카드 인증서를 사용했습니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure02.png) 
 
1. **F5 BIG-IP 클릭 액세스 > 단계별 구성 > 페더레이션 > SAML 서비스 공급자**로 차례로 이동합니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure03.png) 

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure04.png)

1. **엔터티 ID**를 지정합니다(Azure AD 애플리케이션 구성에서 구성한 것과 동일).

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure05.png) 

1. 새 가상 서버를 만들고 **대상 주소**를 지정합니다. 이전에 업로드한 **와일드카드 인증서**(또는 애플리케이션을 위해 업로드한 **인증서**) 및 **연결된 프라이빗 키**를 선택합니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure06.png)

1. 구성 **메타데이터**를 업로드하고, 새 **SAML IDP 커넥터 이름**을 지정합니다. 또한 이전에 업로드한 페더레이션 인증서를 지정해야 합니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure07.png)  

1. **새로 만들기**를 사용하여 백 엔드 앱 풀을 만들고, 백 엔드 애플리케이션 서버의 **IP 주소**를 지정합니다.
 
    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure08.png)

1. **Single Sign-On 설정**에서 **Kerberos**를 선택하고 **고급 설정**을 선택합니다. `user@domain.suffix`에서 요청을 만들어야 합니다.

1. **사용자 이름 원본**에서 `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`을 지정합니다. 변수 및 값의 전체 목록은 부록을 참조하세요.
계정 이름은 생성된 F5 위임 계정(F5 설명서 확인)입니다.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure09.png)   

1. **엔드포인트 확인 속성** 설명서 세부 정보는 [F5(Kerberos) 클라이언트 지원 팀](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)에 문의하세요.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure10.png) 

1. **세션 관리 속성** 설명서 세부 정보는 [F5(Kerberos) 클라이언트 지원 팀](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)에 문의하세요.

    ![F5(Kerberos) 구성](./media/kerbf5-tutorial/configure11.png) 
 
1. **요약 검토**를 수행하고 **배포**를 클릭합니다.
