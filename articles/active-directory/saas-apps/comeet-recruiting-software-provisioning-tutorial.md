---
title: '자습서: Azure Active Directory로 자동 사용자 프로 비전에 대 한 Comeet Recruiting 소프트웨어 구성 | Microsoft Docs'
description: 자동으로 프로 비전 하 고 Comeet Recruiting Software에 사용자 계정을 프로 비전 해제 하도록 Azure Active Directory를 구성 하는 방법에 알아봅니다.
...
이 자습서의 목적은 자동으로 프로 비전 하 고 사용자 및/또는 그룹 Comeet Recruiting 소프트웨어를 프로 비전 해제 하도록 Azure AD를 구성 하려면 Comeet Recruiting 소프트웨어 및 Azure Active Directory (Azure AD)에서 수행 하는 단계를 보여 주기 위해입니다.
...
* Azure AD 테넌트
* [Comeet Recruiting 소프트웨어 테 넌 트](https://www.comeet.co/)
* 관리자 권한이 있는 Comeet Recruiting Software의 사용자 계정.

## <a name="add-comeet-recruiting-software-from-the-gallery"></a>갤러리에서 Comeet Recruiting Software를 추가 합니다.

Azure AD를 사용 하 여 자동 사용자 프로 비전을 위한 Comeet Recruiting 소프트웨어를 구성 하기 전에 Azure AD 응용 프로그램 갤러리에서 관리 되는 SaaS 응용 프로그램의 목록에 Comeet 채용 소프트웨어를 추가 해야 합니다.
...
4. 검색 상자에 입력 **Comeet 채용 소프트웨어**를 선택 **Comeet 채용 소프트웨어** 클릭 한 다음 확인 하 고 결과 패널에서는 **추가** 단추는 응용 프로그램을 추가 합니다.

    ![결과 목록의 Comeet Recruiting Software](common/search-new-app.png)

## <a name="assigning-users-to-comeet-recruiting-software"></a>Comeet Recruiting 소프트웨어에 사용자 할당
...
를 구성 하 고 자동 사용자 프로 비전을 사용 하도록 설정 하기 전에 Comeet Recruiting 소프트웨어에 대 한 액세스를 해야 하는 사용자 및/또는 Azure AD의 그룹 결정 해야 합니다. 결정 했으면 할당할 수 있습니다 이러한 사용자 및/또는 그룹 Comeet Recruiting 소프트웨어 여기의 지침에 따라.

* [엔터프라이즈 앱에 사용자 또는 그룹 할당](../manage-apps/assign-user-or-group-access-portal.md)

### <a name="important-tips-for-assigning-users-to-comeet-recruiting-software"></a>Comeet Recruiting 소프트웨어에 사용자를 할당 하기 위한 주요 팁

* 것이 좋습니다 단일 Comeet Recruiting 소프트웨어에 자동 사용자 프로 비전 구성을 테스트 하려면 Azure AD 사용자를 할당 합니다. 추가 사용자 및/또는 그룹은 나중에 할당할 수도 있습니다.

* 사용자에 Comeet 채용 소프트웨어를 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할 (사용 가능한 경우)를 선택 해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비전에서 제외됩니다.

## <a name="configuring-automatic-user-provisioning-to-comeet-recruiting-software"></a>Comeet Recruiting 소프트웨어에 자동 사용자 프로비저닝 구성 

Azure AD 사용자 및/또는 그룹 할당 기준으로이 섹션에서는 지침을 만들려면 Azure AD 프로 비전 서비스를 구성 하는 단계를 통해 업데이트 및 사용자 및/또는 Comeet Recruiting 소프트웨어에서 그룹을 사용 하지 않도록 설정 합니다.

> [!TIP]
> 제공한 지침에 따라 SAML 기반 single sign-on 사용 Comeet Recruiting 소프트웨어용 수도 있습니다, 합니다 [Comeet 채용 Software single sign-on 자습서](comeetrecruitingsoftware-tutorial.md)합니다. Single Sign-On과 자동 사용자 프로비저닝은 서로 보완적이지만, 별개로 구성할 수 있습니다.
...
5. 아래를 **관리자 자격 증명** 섹션에서 입력 합니다 **테 넌 트 URL** 및 **비밀 토큰** 6 단계에에서 설명 된 대로 Comeet Recruiting 소프트웨어의 계정.
...
13. Comeet Recruiting 소프트웨어에 대 한 서비스를 프로 비전 하는 Azure AD를 사용 하도록 설정 하려면 변경를 **프로 비전 상태** 에 **에** 에 **설정** 섹션입니다.

    ![프로 비전 상태에서 전환](common/provisioning-toggle-on.png)

14. 원하는 값을 선택 하 여 Comeet 채용 소프트웨어를 프로 비전 하려는 그룹 및/또는 사용자 정의 **범위** 에 **설정** 섹션입니다.
...
이 작업은 **설정**의 **범위** 섹션에 정의된 모든 사용자 및/또는 그룹의 초기 동기화를 시작합니다. 초기 동기화는 Azure AD 프로비전 서비스가 실행되는 동안 약 40분마다 발생하는 후속 동기화보다 더 많은 시간이 걸립니다. 사용할 수는 **동기화 세부 정보** 진행률을 모니터링 하 고 링크를 프로 비전 활동 보고서를 프로 비전 서비스 Comeet Recruiting 소프트웨어에서 Azure AD에서 수행한 모든 작업을 설명 하는 섹션입니다.

Azure AD 프로비저닝 로그를 읽는 방법에 대한 자세한 내용은 [자동 사용자 계정 프로비저닝에 대한 보고](../manage-apps/check-status-user-account-provisioning.md)를 참조하세요.

## <a name="connector-limitations"></a>커넥터 제한 사항

* Comeet Recruiting 소프트웨어 현재 그룹을 지원 하지 않습니다.
