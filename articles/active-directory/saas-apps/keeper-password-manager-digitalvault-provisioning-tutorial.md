---
title: '자습서: Azure Active Directory로 자동 사용자 프로 비전에 대 한 Keeper Password Manager & Digital Vault 구성 | Microsoft Docs'
description: 자동으로 프로 비전 하 고 Keeper Password Manager & Digital Vault에 대 한 사용자 계정을 프로 비전 해제 하도록 Azure Active Directory를 구성 하는 방법에 알아봅니다.
...
Keeper Password Manager & Digital Vault 및 Azure Active Directory (Azure AD) Azure AD를 구성 하려면 자동으로 프로 비전 하 고 사용자 및/또는 그룹 Keeper Password로 프로 비전 해제를 수행 하는 단계를 보여 주기 위해이 자습서의 목표는 Manager & Digital Vault입니다.
...
* Azure AD 테넌트
* [Keeper Password Manager & Digital Vault 테 넌 트](https://keepersecurity.com/pricing.html?t=e)
* 관리자 권한으로 Keeper Password Manager & Digital Vault에서 사용자 계정.

## <a name="add-keeper-password-manager--digital-vault-from-the-gallery"></a>갤러리에서 Keeper Password Manager & Digital Vault 추가

Keeper Password Manager & Digital Vault와 Azure AD 자동 사용자 프로 비전을 구성 하기 전에 Azure AD 응용 프로그램 갤러리의 Keeper Password Manager & Digital Vault를 관리 되는 SaaS 응용 프로그램의 목록에 추가 해야 합니다.

**Azure AD 응용 프로그램 갤러리에서 Keeper Password Manager & Digital Vault를 추가 하려면 다음 단계를 수행 합니다.**
...
4. 검색 상자에서 입력 **Keeper Password Manager & Digital Vault**를 선택 **Keeper Password Manager & Digital Vault** 클릭 한 다음 확인 하 고 결과 패널에서는 **추가**단추는 응용 프로그램을 추가 합니다.
...
를 구성 하 고 자동 사용자 프로 비전을 사용 하도록 설정 하기 전에 Keeper Password Manager & Digital Vault에 대 한 액세스를 해야 하는 사용자 및/또는 Azure AD의 그룹 결정 해야 합니다. 결정 했으면 할당할 수 있습니다 이러한 사용자 및/또는 그룹 Keeper Password Manager & Digital Vault 여기의 지침에 따라.

* [엔터프라이즈 앱에 사용자 또는 그룹 할당](../manage-apps/assign-user-or-group-access-portal.md)

### <a name="important-tips-for-assigning-users-to-keeper-password-manager--digital-vault"></a>Keeper Password Manager & Digital Vault에 사용자를 할당 하기 위한 주요 팁

* 것이 좋습니다 단일 Keeper Password Manager & Digital Vault에 자동 사용자 프로 비전 구성을 테스트 하려면 Azure AD 사용자를 할당 합니다. 추가 사용자 및/또는 그룹은 나중에 할당할 수도 있습니다.

* 사용자를 Keeper Password Manager & Digital Vault에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할 (사용 가능한 경우)를 선택 해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로비전에서 제외됩니다.

## <a name="configuring-automatic-user-provisioning-to-keeper-password-manager--digital-vault"></a>Keeper Password Manager & Digital Vault에 자동 사용자 프로비저닝 구성 

이 섹션에서는 안내 만들기, 업데이트 및 사용자 및/또는 그룹에서 Keeper Password Manager를 사용 하지 않도록 설정 하려면 Azure AD 프로 비전 서비스를 구성 하는 단계 & Digital Vault에서 Azure AD 사용자 및/또는 그룹 할당에 기반 합니다.

> [!TIP]
> 제공한 지침에 따라 SAML 기반 single sign-on 사용 Keeper Password Manager & Digital Vault 수도 있습니다, 합니다 [Keeper Password Manager & Digital Vault single sign-on 자습서](keeperpasswordmanager-tutorial.md)합니다. Single Sign-On과 자동 사용자 프로비저닝은 서로 보완적이지만, 별개로 구성할 수 있습니다.
...
5. 아래는 **관리자 자격 증명** 섹션에서 입력 합니다 **테 넌 트 URL** 및 **비밀 토큰** 에 Keeper Password Manager & Digital Vault 계정 6 단계에에서 설명 된 대로의 합니다.
...
15. Keeper Password Manager & Digital Vault에 대 한 서비스를 프로 비전 하는 Azure AD를 사용 하도록 설정 하려면 변경 합니다 **프로 비전 상태** 에 **에** 에 **설정** 섹션.

    ![프로 비전 상태에서 전환](common/provisioning-toggle-on.png)

16. Keeper Password Manager & Digital Vault에 프로 비전에서 원하는 값을 선택 하 여 사용자 및/또는 하려는 그룹을 정의 **범위** 에 **설정** 섹션입니다.
...
이 작업은 **설정**의 **범위** 섹션에 정의된 모든 사용자 및/또는 그룹의 초기 동기화를 시작합니다. 초기 동기화는 Azure AD 프로비전 서비스가 실행되는 동안 약 40분마다 발생하는 후속 동기화보다 더 많은 시간이 걸립니다. 사용할 수는 **동기화 세부 정보** 진행률을 모니터링 하 고 링크를 프로 비전 활동 보고서는 Azure AD 프로 비전 서비스에서 Keeper Password Manager에서 수행한 모든 작업을 설명 하는 섹션 & 디지털 자격 증명 모음입니다.
