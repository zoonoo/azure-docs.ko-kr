...
4. **[Tableau Server 테스트 사용자 만들기](#create-tableau-server-test-user)** - 사용자의 Azure AD 표시와 연결되어있는 Cisco Umbrella의 Britta Simon에 해당하는 사용자를 만듭니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 Tableau Server 애플리케이션에서 Single Sign-On을 구성합니다.

**Tableau Server에서 Azure AD Single Sign-on을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **Tableau Server** 애플리케이션 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
3. Tableau Server 애플리케이션에서는 사용자 지정 클레임 **username**을 아래와 같이 정의해야 합니다. 이것이 고유한 사용자 ID 클레임 대신 사용자 ID로 사용되고 있습니다. 애플리케이션 통합 페이지의 **사용자 특성 및 클레임** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. **편집** 단추를 클릭하여 **사용자 특성 및 클레임** 대화 상자를 엽니다.
...
1. 애플리케이션에 대해 구성된 SSO를 가져오려면 관리자 권한으로 Tableau Server 테넌트에 로그온해야 합니다.
...
3. **구성** 페이지에서 다음 단계를 수행합니다.
    ...
    다. Tableau Server 반환 URL - Tableau Server 사용자가 액세스하는 URL(예: <http://tableau_server>)입니다. `http://localhost`을 사용하는 것은 권장되지 않습니다. 후행 슬래시가 있는 URL을 사용 하 여 (예를 들어 `http://tableau_server/`) 지원 되지 않습니다. **Tableau Server 반환 URL**을 복사하여 **Tableau Server 도메인 및 URL** 섹션의 Azure AD **로그온 URL** 텍스트 상자에 붙여넣습니다.
    ...
    e. **XML 메타데이터 파일 다운로드**를 클릭하고 텍스트 편집기 애플리케이션에서 엽니다. Http Post 및 인덱스 0이 포함된 어설션 소비자 서비스 URL을 찾아서 복사합니다. **Tableau Server 도메인 및 URL** 섹션의 Azure AD **응답 URL** 텍스트 상자에 붙여넣습니다.
    ...
    >Tableau Server에서 SAML을 구성하는 데 도움이 필요한 경우 [SAML 구성](https://onlinehelp.tableau.com/v2018.2/server/en-us/saml_config_steps_tsm_ui.htm) 문서를 참조하세요.
...
액세스 패널에서 Tableau Server 타일을 클릭하면 Tableau Server 애플리케이션에 자동으로 로그온됩니다.
