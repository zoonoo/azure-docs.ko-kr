...
* 사용자가 자신의 Azure AD 계정으로 ZPA(Zscaler Private Access)에 자동으로 로그인(Single Sign-On)되도록 설정할 수 있습니다.
...
4. 검색 상자에 **ZPA(Zscaler Private Access)** 를 입력하고 결과 패널에서 **ZPA(Zscaler Private Access)** 를 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD 사용자와 ZPA(Zscaler Private Access)의 관련 사용자 간에 연결이 형성되어야 합니다.

ZPA(Zscaler Private Access)에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.

1. **[Azure AD Single Sign-On 구성](#configure-azure-ad-single-sign-on)** - 사용자가 이 기능을 사용할 수 있도록 합니다.
2. **[ZPA(Zscaler Private Access) Single Sign-On 구성](#configure-zscaler-private-access-zpa-single-sign-on)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
...
1. [Azure Portal](https://portal.azure.com/)의 **ZPA(Zscaler Private Access)** 애플리케이션 통합 페이지에서 **Single Sign-On**을 선택합니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![ZPA(Zscaler Private Access) 도메인 및 URL Single Sign-On 정보](common/sp-identifier.png)

    a. **로그온 URL** 텍스트 상자에서 `https://samlsp.private.zscaler.com/auth/login?domain=<your-domain-name>` 패턴을 사용하는 URL을 입력합니다.

    b. **식별자(엔터티 ID)** 텍스트 상자에서 `https://samlsp.private.zscaler.com/auth/metadata` URL을 입력합니다.
...
6. **ZPA(Zscaler Private Access) 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
1. Azure Portal에서 **엔터프라이즈 애플리케이션**, **모든 애플리케이션**, **ZPA(Zscaler Private Access)** 를 차례로 선택합니다.
...
액세스 패널에서 ZPA(Zscaler Private Access) 타일을 클릭하면 SSO를 설정한 ZPA(Zscaler Private Access)에 자동으로 로그인되어야 합니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
