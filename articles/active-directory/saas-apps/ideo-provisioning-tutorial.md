...
* 사용자를 IDEO에 할당할 때 할당 대화 상자에서 유효한 응용 프로그램별 역할 (사용 가능한 경우)을 선택 해야 합니다. **기본 액세스** 역할이 있는 사용자는 프로 비전에서 제외 됩니다.
...
1. **비밀 토큰** 문의 IDEO 지원 팀에 문의 productsupport@ideo.com하세요. 이 값은 Azure Portal IDEO 응용 프로그램의 프로 비전 탭에 있는 **비밀 토큰** 필드에 입력 됩니다. 
...
5. **관리자 자격 증명** 섹션 아래에 있는 `https://profile.ideo.com/api/scim/v2` **테 넌 트 URL**에 입력 합니다. **비밀 토큰**의 IDEO 지원 팀에서 검색 한 값을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 IDEO에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 IDEO 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 합니다.
