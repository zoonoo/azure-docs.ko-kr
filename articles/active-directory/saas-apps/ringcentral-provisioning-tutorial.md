...
5. **관리자 자격 증명** 섹션 아래의 **테넌트 URL** 에서 `https://platform.ringcentral.com/scim/v2`를 입력합니다. 이전에 검색한 **SCIM Authentication Token(SCIM 인증 토큰)** 값을 **비밀 토큰** 에 입력합니다. **연결 테스트** 를 클릭하여 Azure AD가 RingCentral에 연결할 수 있는지 확인합니다. 연결에 실패하면 RingCentral 계정에 관리자 권한이 있는지 확인하고 다시 시도합니다.
