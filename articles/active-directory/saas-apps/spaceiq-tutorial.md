...
1. 검색 상자에 **SpaceIQ**를 입력하고, 결과 패널에서 **SpaceIQ**를 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 SpaceIQ 애플리케이션에서 Single Sign-On을 구성합니다.

**SpaceIQ에서 Azure AD Single Sign-on을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **SpaceIQ** 애플리케이션 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
1. **SpaceIQ 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    b. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://api.spaceiq.com/saml/<instanceid>/callback`
...
1. **SSO** 대화 상자에서 다음 단계를 수행합니다.
    ...
    a. **SAML 발급자 URL** 상자에 Azure AD 애플리케이션 구성 창에서 복사한 **SAML 엔터티 ID** 값을 붙여넣습니다.
    
    b. **SAML 콜백 엔드포인트 URL(읽기 전용)** 값을 복사하여 SpaceIQ **도메인 및 URL** 섹션 아래, Azure Portal의 **회신 URL** 상자에 값을 붙여넣습니다.
...
1. 애플리케이션 목록에서 **SpaceIQ**를 선택합니다.
    ...
    ![애플리케이션 목록의 SpaceIQ 링크](./media/spaceiq-tutorial/tutorial_spaceiq_app.png)  
...
액세스 패널에서 SpaceIQ 타일을 클릭하면 SpaceIQ 애플리케이션에 자동으로 로그온됩니다.
