...
1. **[Grovo 테스트 사용자 만들기](#create-a-grovo-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Grovo에 만듭니다.
...
1. Grovo 애플리케이션은 특정 형식의 SAML 어설션이 필요합니다. 이 애플리케이션에 대해 다음 클레임을 구성합니다. 애플리케이션 통합 페이지의 **"사용자 특성"** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. **user.mail**을 사용하여 **사용자 ID**를 매핑하고, 아래 스크린샷과 같이 다른 특성을 구성합니다.
...
1. **Grovo 구성** 섹션에서 **Grovo 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Grovo 구성](./media/grovo-tutorial/tutorial_grovo_configure.png) 

1. 다른 웹 브라우저 창에서 Grovo에 관리자 권한으로 로그인합니다.

1. **관리** > **통합**으로 차례로 이동합니다.
 
    ![Grovo 구성](./media/grovo-tutorial/tutorial_grovo_admin.png) 

1. **SP Initiated SAML 2.0**(SP가 시작한 SAML 2.0) 섹션 아래에서 **설정**을 클릭합니다.

    ![Grovo 구성](./media/grovo-tutorial/tutorial_grovo_setup.png)

1. **SP Initiated SAML 2.0** 팝업 창에서 다음 단계를 수행합니다.
