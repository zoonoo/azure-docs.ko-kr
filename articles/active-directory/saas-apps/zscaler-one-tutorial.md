---
title: '자습서: Zscaler One과 Azure Active Directory 통합| Microsoft Docs'
...
4. 검색 상자에 **Zscaler One**을 입력하고 결과 패널에서 **Zscaler One**을 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
    ...
    ![결과 목록의 Zscaler One](./media/zscaler-one-tutorial/tutorial_zscalerone_addfromgallery.png)
...
3. **[Zscaler One 테스트 사용자 만들기](#creating-a-zscaler-one-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Zscaler One에 만듭니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![Zscaler One 도메인 및 URL Single Sign-On 정보](./media/zscaler-one-tutorial/tutorial_zscalerone_url.png)
...
5. Zscaler One 애플리케이션에는 특정 형식의 SAML 어설션이 필요합니다. 이 애플리케이션에 대해 다음 클레임을 구성합니다. 애플리케이션 통합 페이지의 **사용자 특성 및 클레임** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. **SAML로 Single Sign-On 설정 페이지**에서 **편집** 단추를 클릭하여 **사용자 특성 및 클레임** 대화 상자를 엽니다.

    ![특성 링크](./media/zscaler-one-tutorial/tutorial_zscalerone_attribute.png)
...
7. **SAML 서명 인증서** 페이지의 **SAML 서명 인증서** 섹션에서 **다운로드**를 클릭하고 **인증서(Base64)** 다운로드한 다음, 컴퓨터에 인증서 파일을 저장합니다.
    ...
    ![인증서 다운로드 링크](./media/zscaler-one-tutorial/tutorial_zscalerone_certificate.png) 
...
8. **Zscaler One 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
    ...
    ![Zscaler One 구성](common/configuresection.png)

9. 다른 웹 브라우저 창에서 관리자 권한으로 Zscaler One 회사 사이트에 로그인합니다.

10. **관리 > 인증 > 인증 설정**으로 이동하고 다음 단계를 수행합니다.
   
    ![관리](./media/zscaler-one-tutorial/ic800206.png "관리")
...
12. **사용자 인증 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    ![관리](./media/zscaler-one-tutorial/ic800207.png)
...
1. **연결** 탭을 클릭합니다.   
     ...
     ![연결](./media/zscaler-one-tutorial/ic769493.png "연결")
...
이 섹션은 Zscaler One에서 Britta Simon이라는 사용자를 만들기 위한 것입니다. Zscaler One은 적시에 프로비전을 지원하며 기본적으로 사용하도록 설정합니다. 이 섹션에 작업 항목이 없습니다. 새 사용자가 아직 존재하지 않는 경우 Zscaler One에 액세스하는 동안 만들어질 수 있습니다.
>[!Note]
>사용자를 수동으로 만들어야 하는 경우  [Zscaler One 지원 팀](https://www.zscaler.com/company/contact)에 문의하세요.
...
5. **사용자 및 그룹** 대화 상자의 목록에서 **Britta Simon** 등의 사용자를 선택한 다음, 화면 맨 아래에서 **선택** 단추를 클릭합니다.
    ...
    ![이미지](./media/zscaler-one-tutorial/tutorial_zscalerone_users.png)

6. **역할 선택** 대화 상자의 목록에서 적절한 사용자 역할을 선택한 다음, 화면 맨 아래에서 **선택** 단추를 클릭합니다.

    ![이미지](./media/zscaler-one-tutorial/tutorial_zscalerone_roles.png)

7. **할당 추가** 대화 상자에서 **할당** 단추를 선택합니다.

    ![이미지](./media/zscaler-one-tutorial/tutorial_zscalerone_assign.png)
