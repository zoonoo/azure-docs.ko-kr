...
5. **관리자 자격 증명** 섹션 아래에서 PureCloud by를 입력 하 여 **테 넌 트 Url** 및 **비밀 토큰** 필드에 Oauth 토큰을 입력 합니다. API URL은 [PureCloud 개발자 센터](https://developer.mypurecloud.com/api/rest/index.html)에서 PureCloud `{{API Url}}/api/v2/scim/v2`지역에 대 한 api url을 사용 하 여 구성 됩니다. **연결 테스트** 를 클릭 하 여 Azure AD가 PureCloud에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Genesys 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
