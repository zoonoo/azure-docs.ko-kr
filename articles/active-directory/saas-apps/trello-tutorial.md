...
1. **[Trello 테스트 사용자 만들기](#create-a-trello-test-user)** - Azure AD 표현과 연결된 Trello의 Britta Simon에 해당하는 사용자를 만듭니다.
...
1. **Trello 도메인 및 URL** 섹션에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
...
1. **SAML 서명 인증서** 섹션에서 **인증서(Base64)** 를 클릭한 후 컴퓨터에 메타데이터 파일을 저장합니다.
...
1. **Trello 구성** 섹션에서 **Trello 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Trello 구성](./media/trello-tutorial/tutorial_trello_configure.png) 

1. **Trello** 쪽에서 Single Sign-On을 구성하려면 [Trello 엔터프라이즈 SSO 구성](https://trello.com/sso-configuration) 페이지로 이동하여 다운로드한 **인증서(Base64)** 및 **SAML Single Sign-On 서비스 URL**을 [Trello 지원 팀](mailto:support@trello.com)에 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
