...
* 사용자가 자신의 Azure AD 계정으로 AnswerHub에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
* AnswerHub Single Sign-On을 사용하도록 설정된 구독
...
4. 검색 상자에 **AnswerHub**를 입력하고 결과 패널에서 **AnswerHub**를 선택한 다음, **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD 사용자와 AnswerHub의 관련 사용자 간에 연결 관계를 설정해야 합니다.
...
5. **[AnswerHub 테스트 사용자 만들기](#create-answerhub-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 AnswerHub에 만듭니다.
...
4. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    ![AnswerHub 도메인 및 URL Single Sign-On 정보](common/sp-identifier.png)

    a. **로그온 URL** 텍스트 상자에서 `https://<company>.answerhub.com` 패턴을 사용하는 URL을 입력합니다.

    b. **식별자(엔터티 ID)** 텍스트 상자에서 `https://<company>.answerhub.com` 패턴을 사용하는 URL을 입력합니다.

    > [!NOTE]
    > 이러한 값은 실제 값이 아닙니다. 실제 로그온 URL 및 식별자로 이러한 값을 업데이트합니다. 이러한 값을 가져오려면 [AnswerHub 클라이언트 지원팀](mailto:success@answerhub.com)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.
...
1. 다른 웹 브라우저 창에서 AnswerHub 회사 사이트에 관리자로 로그인합니다.
...
4. 탐색 창의 왼쪽에 있는 **소셜 설정** 섹션에서 **SAML 설정**을 클릭합니다.

5. **IDP 구성** 탭을 클릭합니다.

6. **IDP 구성** 탭에서 다음 단계를 수행합니다.

    ![SAML 설정](./media/answerhub-tutorial/ic785172.png "SAML 설정")  
  
    a. **IDP 로그인 URL** 텍스트 상자에, Azure Portal에서 복사한 **로그인 URL**을 붙여넣습니다.
  
    b. Azure Portal에서 복사한 **로그아웃 URL**을 **IDP 로그인 URL** 텍스트 상자에 붙여 넣습니다.

    다. **사용자 특성** 섹션의 Azure Portal에서 선택한 것과 동일한 사용자 ID 값을 **IDP 이름 식별자 형식**에 입력합니다.
  
    d. **키 및 인증서**를 클릭합니다.

7. **키와 인증서** 탭에서 다음 단계를 수행합니다.
...
2. 애플리케이션 목록에서 **AnswerHub**를 선택합니다.
    ...
    ![애플리케이션 목록의 AnswerHub 링크](common/all-applications.png)
...
4. 왼쪽 탐색 창의 **사용자 관리** 섹션에서 **사용자 만들기 또는 가져오기**를 클릭한 다음, **사용자 및 그룹**을 클릭합니다.
...
액세스 패널에서 AnswerHub 타일을 클릭하면 SSO를 설정한 AnswerHub에 자동으로 로그인됩니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)를 참조하세요.
