...
![다이어그램은 I D P 시작 D s O를 통해 3 개의 W 계정에 연결 된 W S 앱이 있는 Azure A D를 보여줍니다.](./media/aws-multi-accounts-tutorial/amazonwebservice.png)
...
6. **사용자 특성** 대화 상자의 **사용자 클레임** 섹션에서 위의 이미지에 표시된 것과 같이 SAML 토큰 특성을 구성하고 다음 단계를 수행합니다.
    ...
    ![새 클레임 추가 및 저장 이라는 사용자 클레임이 스크린샷으로 표시 됩니다.](common/new-save-attribute.png)

    ![이 단계에서 설명 하는 값을 입력할 수 있는 관리 사용자 클레임을 보여 주는 스크린샷](common/new-attribute-details.png)
...
1. 검색 창에서 **관리자 액세스** 를 검색 하 고 관리자 **액세스** 확인란을 선택한 후 **다음: 태그**를 클릭 합니다.
    ...
    ![정책 이름으로 선택한 관리자 액세스를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/administrator-access.png)

1. **태그 추가 (선택 사항)** 섹션에서 다음 단계를 수행 합니다.

    ![키 값 쌍을 추가할 수 있는 태그 추가 창이 스크린샷으로 표시 됩니다.](./media/aws-multi-accounts-tutorial/config2.png)
...
1. 아래 강조 표시 된 것 처럼 EC2 속성 또는 IAM 대시보드에서 해당 AWS 계정에 대 한 계정 ID를 적어 두세요.
    ...
    ![계정 I D가 W S 창에 표시 되는 위치를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/aws-accountid.png)
...
1. 이전에 만든 IAM 역할과 동일한 이름으로 새 그룹을 만들고 이러한 새 그룹의 **개체 id** 를 적어둡니다.
    ...
    ![개요 창에서 계정 I D를 입력 하는 위치를 보여 주는 스크린샷 ](./media/aws-multi-accounts-tutorial/copy-objectids.png)

1. 현재 AWS 계정에서 로그아웃하고 Azure AD에 Single Sign-On을 구성할 다른 계정으로 로그인합니다.

1. 계정에서 역할이 모두 만들어지면 해당 계정의 **역할** 목록에 나타납니다.

    ![스크린샷에는 역할 이름, 설명 및 신뢰할 수 있는 엔터티가 있는 역할 목록이 표시 됩니다.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-listofroles.png)
...
1. 역할을 클릭하여 **역할 ARN** 및 **신뢰할 수 있는 엔터티** 값을 복사합니다. Azure AD에서 만들어야 하는 모든 역할에 대해 이 값이 필요합니다.
    ...
    ![트러스트 관계 탭이 선택 된 요약 창을 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-role-summary.png)
...
1. 다른 창에서 [Microsoft Graph 탐색기](https://developer.microsoft.com/graph/graph-explorer) 를 엽니다.
    ...
    ![시작, 베타 및 실행 쿼리를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/graph-explorer-new1.png)
    ...
    ![스크린 샷에서는 쿼리를 사용 하 여 서비스 주체 개체를 가져오는 방법을 보여 줍니다.](./media/aws-multi-accounts-tutorial/graph-explorer-new2.png)

    g. 서비스 사용자 개체에서 appRoles 속성을 추출합니다.

    ![서비스 사용자 개체의 세부 정보를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/graph-explorer-new3.png)
    ...
    ![Patch 메서드가 선택 된 그래프 탐색기를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/graph-explorer-new11.png)
...
1. 그룹을 만든 후에는 그룹을 선택하여 애플리케이션에 할당합니다.
    ...
    ![사용자 및 그룹이 사용자 및 그룹 창을 열기 위해 선택한 사용자 및 그룹에 할당 추가가 스크린샷으로 표시 됩니다.](./media/aws-multi-accounts-tutorial/graph-explorer-new5.png)
...
1. 그룹에 역할을 할당하려면 역할을 선택하고 페이지 아래의 **할당** 단추를 클릭합니다.
    ...
    ![그룹 하나를 선택 하 여 추가 할당을 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/graph-explorer-new6.png)
...
![역할을 선택할 수 있는 W S 응용 프로그램 페이지를 보여 주는 스크린샷](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-test-screen.png)

SAML 응답에서도 클레임으로 전달된 역할을 확인할 수 있습니다.

![스크린샷에 특성 값이 있는 SAML 응답의 일부가 표시 됩니다.](./media/aws-multi-accounts-tutorial/tutorial-amazonwebservices-test-saml.png)
