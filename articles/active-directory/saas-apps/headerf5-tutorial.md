...
1. 메타데이터 인증서는 나중에 설정 프로세스에서 사용할 F5(헤더 기반)로 가져와야 합니다. **System > Certificate Management > Traffic Certificate Management >> SSL Certificate List**(시스템 > 인증서 관리 > 트래픽 인증서 관리 >> SSL 인증서 목록)로 차례로 이동합니다. 오른쪽 모서리의 **Import**(가져오기)를 클릭합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure12.png)
 
1. 또한 호스트 이름(`headerapp.superdemo.live`)에 대한 **SSL Certificate**(SSL 인증서)도 필요합니다. 이 예에서는 와일드카드 인증서를 사용했습니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure13.png)

1. **F5 (Header Based) BIG-IP Click Access > Guided Configuration > Federation > SAML Service Provider**(F5(헤더 기반) BIG-IP 클릭 액세스 > 단계별 구성 > 페더레이션 > SAML 서비스 공급자)로 차례로 이동합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure01.png)

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure02.png)
 
1. **Entity ID**(엔터티 ID)를 지정합니다(Azure AD 애플리케이션 구성에서 구성한 것과 동일).

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure03.png) 

1. 새 가상 서버를 만들고 **Destination Address**(대상 주소)를 지정합니다. **Redirect Port**(리디렉션 포트)는 선택 사항입니다. 이전에 업로드한 **Wild Card Certificate**(와일드카드 인증서)(또는 애플리케이션을 위해 업로드한 **Cert**(인증서)) 및 **Associated Private Key**(연결된 프라이빗 키)를 선택합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure04.png) 

1. 구성 **메타데이터**를 업로드하고, 새 **Name for SAML IDP Connector**(SAML IDP 커넥터 이름)를 지정합니다. 또한 이전에 업로드한 페더레이션 인증서를 지정해야 합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure05.png)
 
1. **Create New**(새로 만들기)를 사용하여 백 엔드 애플리케이션 풀을 만들고, 백 엔드 애플리케이션 서버의 **IP 주소**를 지정합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure06.png)

1. Single Sign-On 아래에서 **HTTP header-based**(HTTP 헤더 기반)를 선택합니다. 애플리케이션에 따라 다른 헤더를 추가할 수 있습니다. SAML 세션 변수 목록은 부록에서 확인할 수 있습니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure07.png) 

1. **Endpoint Check Properties**(엔드포인트 확인 속성) 설명서 세부 정보는 [F5(헤더 기반) 클라이언트 지원 팀](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)에 문의합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure08.png)

1. **Session Management Properties**(세션 관리 속성) 설명서 세부 정보는 [F5(헤더 기반) 클라이언트 지원 팀](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)에 문의합니다.

    ![F5(헤더 기반) 구성](./media/headerf5-tutorial/configure09.png)

1. **Review Summary**(요약 검토)를 수행하고, **Deploy**(배포)를 클릭합니다.
