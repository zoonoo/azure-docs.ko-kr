...
1. 검색 상자에 **Rollbar**를 입력하고 결과 패널에서 **Rollbar**를 선택한 후 **추가** 단추를 클릭하여 애플리케이션을 추가합니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 Rollbar 애플리케이션에서 Single Sign-On을 구성합니다.

**Rollbar에서 Azure AD Single Sign-On을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **Rollbar** 애플리케이션 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
1. **Rollbar 도메인 및 URL** 섹션에서 **IDP** 시작 모드로 애플리케이션을 구성하려는 경우 다음 단계를 수행합니다.
    ...
    b. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://rollbar.com/<accountname>/saml/sso/azure/`
...
1. 텍스트 상자에 같은 사용자의 이름을 입력 **brittasimon\@contoso.com** 누릅니다 **추가/초대**합니다.
...
1. 애플리케이션 목록에서 **Rollbar**를 선택합니다.
    ...
    ![애플리케이션 목록의 Rollbar 링크](./media/rollbar-tutorial/tutorial_rollbar_app.png)  
...
액세스 패널에서 Rollbar 타일을 클릭하면 Rollbar 애플리케이션에 자동으로 로그온됩니다.
