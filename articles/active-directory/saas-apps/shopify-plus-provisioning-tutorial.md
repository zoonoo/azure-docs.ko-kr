---
title: '자습서: Azure Active Directory을 사용 하 여 자동 사용자 프로 비전을 위한 Shopify Plus 구성 Microsoft Docs'
description: 사용자 계정을 Azure AD에서 Shopify Plus로 자동으로 프로 비전 및 프로 비전 해제 하는 방법을 알아봅니다.
...
ms.locfileid: "96907559"
---
# <a name="tutorial-configure-shopify-plus-for-automatic-user-provisioning"></a>자습서: 자동 사용자 프로 비전을 위한 Shopify Plus 구성

이 자습서에서는 자동 사용자 프로 비전을 구성 하기 위해 Shopify Plus와 Azure Active Directory (Azure AD)에서 수행 해야 하는 단계를 설명 합니다. 구성 된 경우 azure ad는 Azure AD 프로 비전 서비스를 사용 하 여 사용자 및 그룹을 [Shopify Plus](https://www.shopify.com) 에 자동으로 프로 비전 및 프로 비전 해제 합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../manage-apps/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원되는 기능
> [!div class="checklist"]
> * Shopify Plus에서 사용자 만들기
> * Shopify Plus에서 더 이상 액세스할 필요가 없는 사용자를 제거 합니다.
> * Azure AD와 Shopify Plus 간에 사용자 특성을 동기화 상태로 유지
> * Shopify Plus에 대 [한 Single sign-on](https://docs.microsoft.com/azure/active-directory/saas-apps/shopify-plus-tutorial) (권장)
...
* 도메인을 확인 하 고 SAML 구성을 만듭니다. 확인 된 도메인과 연결 된 사용자만 관리할 수 있습니다.
...
3. [AZURE AD와 Shopify Plus 간에 매핑할](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)데이터를 결정 합니다. 

## <a name="step-2-configure-shopify-plus-to-support-provisioning-with-azure-ad"></a>2단계. Shopify Plus를 구성 하 여 Azure AD로 프로 비전 지원

1. [Shopify Plus 조직 관리자](https://shopify.plus )에 로그인 합니다. **사용자 > 보안** 으로 이동 합니다.

2. **Scim 통합** 섹션으로 이동 하 여 **API 토큰 생성** 을 클릭 합니다.

3. 생성 된 토큰을 복사 하 고 저장 합니다. 이 값은 Azure Portal Shopify Plus 응용 프로그램의 프로 비전 탭에 있는 **비밀 토큰** 필드에 입력 됩니다.
...
Azure AD 응용 프로그램 갤러리에서 Shopify Plus를 추가 하 여 Shopify Plus에 프로 비전 관리를 시작 합니다. 이전에 Shopify Plus for SSO를 설치한 경우 동일한 응용 프로그램을 사용할 수 있습니다. 그러나 처음 통합을 테스트하는 경우 별도의 앱을 만드는 것이 좋습니다. [여기](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)를 클릭하여 갤러리에서 애플리케이션을 추가하는 방법에 대해 자세히 알아봅니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-shopify-plus"></a>5단계. Shopify Plus에 자동 사용자 프로 비전 구성 

이 섹션에서는 Azure AD의 사용자 및/또는 그룹 할당에 따라 TestApp에서 사용자 및/또는 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-shopify-plus-in-azure-ad"></a>Azure AD에서 Shopify Plus에 대해 자동 사용자 프로 비전을 구성 하려면:
...
2. 애플리케이션 목록에서 **Shopify Plus** 를 선택합니다.
    ...
    ![응용 프로그램 목록의 Shopify Plus 링크](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션 아래에서 Shopify 및 테 넌 트 URL 및 암호 토큰을 입력 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 Shopify Plus에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Shopify Plus 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
...
8. **매핑** 섹션에서 **사용자 Azure Active Directory Shopify Plus에 동기화를** 선택 합니다.

9. **특성 매핑** 섹션에서 Azure AD에서 Shopify Plus로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 Shopify Plus의 사용자 계정을 일치 시키는 데 사용 됩니다. 일치 하는 [대상 특성](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)을 변경 하도록 선택 하는 경우 SHOPIFY Plus API가 해당 특성에 따라 사용자 필터링을 지원 하는지 확인 해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
...
11. Shopify Plus에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 **설정 하려면 설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 Shopify에 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
