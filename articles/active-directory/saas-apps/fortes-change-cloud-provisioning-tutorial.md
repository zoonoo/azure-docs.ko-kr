---
title: '자습서: Azure Active Directory을 사용 하 여 자동 사용자 프로 비전을 위한 Fortes 전달자 변경 클라우드 구성 Microsoft Docs'
description: Azure AD에서 사용자 계정을 자동으로 프로 비전 및 프로 비전 해제 하는 방법에 대해 알아봅니다 변경 클라우드.
...
이 자습서에서는 자동 사용자 프로 비전을 구성 하기 위해 Fortes 전달자 변경 클라우드와 Azure Active Directory (Azure AD)에서 수행 해야 하는 단계를 설명 합니다. 구성 된 경우 azure AD는 Azure AD 프로 비전 서비스를 사용 하 여 사용자 및 그룹을 자동으로 프로 비전 하 고 [클라우드로 변경](https://fortesglobal.com/) 합니다. 이 서비스의 기능, 작동 방법 및 질문과 대답에 대한 중요한 내용은 [Azure Active Directory를 사용하여 SaaS 애플리케이션의 사용자를 자동으로 프로비저닝 및 프로비저닝 해제](../app-provisioning/user-provisioning.md)를 참조하세요. 


## <a name="capabilities-supported"></a>지원되는 기능
> [!div class="checklist"]
> * Fortes 전달자 변경 클라우드에서 사용자 만들기
> * 더 이상 액세스 하지 않아도 되는 경우 변경 클라우드의 Fortes 전달자에서 사용자 제거
...
* 클라우드 테 넌 트를 변경 하는 Fortes 기술입니다.
* Fortes 전달자의 사용자 계정은 관리자 권한으로 클라우드를 변경 합니다.
...
3. [AZURE AD와 Fortes 전달자 변경 클라우드 간에 매핑할](../app-provisioning/customize-application-attributes.md)데이터를 결정 합니다. 
...
* 사용자 및 그룹을 Fortes 전달자 변경 클라우드에 할당할 때 **기본 액세스** 이외의 다른 역할을 선택 해야 합니다. 기본 액세스 역할이 있는 사용자는 프로비저닝에서 제외되고 프로비저닝 로그에 실질적으로 권한을 부여받지 않은 것으로 표시됩니다. 애플리케이션에서 사용할 수 있는 유일한 역할이 기본 액세스 역할인 경우에는 [애플리케이션 매니페스트를 업데이트](../develop/howto-add-app-roles-in-azure-ad-apps.md)하여 역할을 더 추가할 수 있습니다. 
...
## <a name="step-5-configure-automatic-user-provisioning-to-fortes-change-cloud"></a>5단계. Fortes 전달자 변경 클라우드에 자동 사용자 프로 비전 구성 

이 섹션에서는 Azure AD의 사용자 및/또는 그룹 할당에 따라 TestApp에서 사용자 및/또는 그룹을 만들고, 업데이트하고, 사용 해제하도록 Azure AD 프로비저닝 서비스를 구성하는 단계를 안내합니다.

### <a name="to-configure-automatic-user-provisioning-for-fortes-change-cloud-in-azure-ad"></a>Azure AD의 Fortes 전달자 변경 클라우드에 대해 자동 사용자 프로 비전을 구성 하려면:
...
2. 애플리케이션 목록에서 **Fortes Change Cloud** 를 선택합니다.
    ...
    ![응용 프로그램 목록의 Fortes 전달자 클라우드 링크 변경](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션 아래에서 fortes 전달자에 클라우드 테 넌 트 URL 및 암호 토큰을 변경 합니다. **연결 테스트** 를 클릭 하 여 Azure AD가 Fortes 전달자 변경 클라우드에 연결할 수 있는지 확인 합니다. 연결에 실패 하면 Fortes 전달자 변경 클라우드 계정에 관리자 권한이 있는지 확인 하 고 다시 시도 하세요.
...
8. **매핑** 섹션 아래에서 **Azure Active Directory 사용자를 Fortes 전달자 변경 클라우드로 동기화를** 선택 합니다.

9. **특성 매핑** 섹션에서 Azure AD에서 Fortes 전달자 변경 클라우드로 동기화 되는 사용자 특성을 검토 합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 기술 전달자 변경 클라우드의 사용자 계정을 일치 시키는 데 사용 됩니다. 일치 하는 [대상 특성](../app-provisioning/customize-application-attributes.md)을 변경 하도록 선택 하는 경우 fortes 지원 변경 클라우드 API가 해당 특성에 따라 사용자 필터링을 지원 하는지 확인 해야 합니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
   ...
   |externalId|String|
   |urn: ietf: params: scim: 스키마: 확장: fcc: 2.0: 사용자: 관리자|부울|
   |urn: ietf: params: scim: 스키마: 확장: fcc: 2.0: 사용자: loginDisabled|부울|
...
11. Azure AD 프로 비전 서비스에 대 한 Azure AD 프로 비전 서비스를 사용 하도록 설정 하려면 **설정** 섹션에서 **프로 비전 상태** 를 **켜기** 로 변경 합니다.

    ![프로비전 상태 켜기로 전환](common/provisioning-toggle-on.png)

12. **설정** 섹션의 **범위** 에서 원하는 값을 선택 하 여 클라우드 변경 클라우드로 프로 비전 하려는 사용자 및/또는 그룹을 정의 합니다.
