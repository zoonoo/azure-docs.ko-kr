...
1. 검색 상자에 **Fidelity NetBenefits**를 입력하고 결과 패널에서 **Fidelity NetBenefits**를 선택하고 **추가** 단추를 클릭하여 응용 프로그램을 추가합니다.
...
1. **[Fidelity NetBenefits 테스트 사용자 만들기](#create-a-fidelity-netbenefits-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Fidelity NetBenefits에 만듭니다.
...
1. **Fidelity NetBenefits 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
...
1. Fidelity NetBenefits 응용 프로그램은 특정 형식의 SAML 어설션이 필요합니다. **사용자 ID**를 **user.userprincipalname**과 매핑하였습니다. 이것을 **employeeid** 또는 조직에 **사용자 ID**로 적용할 수 있는 다른 클레임과 매핑할 수 있습니다. 다음 스크린샷은 이에 대한 예제를 보여 줍니다.
...
1. **Fidelity NetBenefits 구성** 섹션에서 **Fidelity NetBenefits 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Fidelity NetBenefits 구성](./media/fidelitynetbenefits-tutorial/tutorial_fidelitynetbenefits_configure.png) 

1. **Fidelity NetBenefits** 쪽에서 Single Sign-On을 구성하려면 다운로드한 **메타데이터 XML**, **SAML Single Sign-On 서비스 URL** 및 **SAML 엔터티 ID**를 [Fidelity NetBenefits 지원 팀](mailto:SSOMaintenance@fmr.com)으로 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
...
1. 응용프로그램 목록에서 **Fidelity NetBenefits**를 선택합니다.
