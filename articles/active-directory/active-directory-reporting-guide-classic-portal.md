---
title: "Azure Active Directory 클래식 포털 보고 가이드 | Microsoft Docs"
description: "Azure 클래식 포털에 대한 모든 Azure Active Directory Reporting 문서가 포함된 가이드"
services: active-directory
documentationcenter: 
author: dhanyahk
manager: femila
ms.assetid: b60d7f22-f271-4148-a3c8-b0ed0eccd106
ms.service: active-directory
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: na
ms.workload: identity
ms.date: 01/19/2017
ms.author: femila;markvi
translationtype: Human Translation
ms.sourcegitcommit: be986fd7bb1745dcf43a1066dfabc1e1c699ab4c
ms.openlocfilehash: 87a563f1e89fa1801edca557e1ef7cfdb6a9de4e


---
# <a name="azure-active-directory-classic-portal-reporting-guide"></a>Azure Active Directory 클래식 포털 보고 가이드
> [!div class="op_single_selector"]
> * [Azure 포털](active-directory-reporting-guide.md)
> * [Azure 클래식 포털](active-directory-reporting-guide-classic-portal.md)
> 
> 

* [Azure Active Directory Reporting 대기 시간](active-directory-reporting-latencies.md)
* [액세스 및 사용 보고서 보기](active-directory-view-access-usage-reports.md)
* [Azure Active Directory Reporting 알림](active-directory-reporting-notifications.md)

**보고서 이해**

* [비정상적인 로그인 작업](active-directory-reporting-irregular-sign-in-activity.md)
* [여러 번의 실패 후 로그인](active-directory-reporting-sign-ins-after-multiple-failures.md)
* [의심스러운 활동을 포함하는 IP 주소의 로그인](active-directory-reporting-sign-ins-from-ip-addresses-with-suspicious-activity.md)
* [여러 지역에서의 로그인](active-directory-reporting-sign-ins-from-multiple-geographies.md)
* [감염 가능성이 있는 장치에서의 로그인](active-directory-reporting-sign-ins-from-possibly-infected-devices.md)




<!--HONumber=Jan17_HO3-->


