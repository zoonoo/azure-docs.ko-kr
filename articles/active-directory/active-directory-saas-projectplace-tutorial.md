...
Projectplace를 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.

- Projectplace에 대한 액세스 권한이 있는 사용자를 Azure AD에서 제어할 수 있습니다.
- 사용자가 해당 Azure AD 계정으로 Projectplace에 자동으로 로그온(Single Sign-on)되도록 설정할 수 있습니다.
...
Projectplace와 Azure AD 통합을 구성하려면 다음 항목이 필요합니다.
...
**갤러리에서 Projectplace를 추가하려면 다음 단계를 수행합니다.**
...
이 섹션에서는 "Britta Simon"이라는 테스트 사용자를 기반으로 Projectplace에서 Azure AD Single Sign-On을 구성하고 테스트합니다.

Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Projectplace 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Projectplace의 관련 사용자 간에 연결이 형성되어야 합니다.

Projectplace에서 Azure AD의 **사용자 이름** 값을 **Username** 값으로 할당하여 링크 관계를 설정합니다.
...
이 섹션에서는 Azure Portal에서 Azure AD Single Sign-On을 사용하도록 설정하고 Projectplace 응용 프로그램에서 Single Sign-On을 구성합니다.

**Projectplace에서 Azure AD Single Sign-on을 구성하려면 다음 단계를 수행합니다.**

1. Azure Portal의 **Projectplace** 응용 프로그램 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
3. **Projectplace 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<company>.projectplace.com`
...
### <a name="creating-a-projectplace-test-user"></a>Projectplace 테스트 사용자 만들기
...
2. **사람**으로 이동한 다음 **멤버**를 클릭합니다.
    ...
    ![사람](./media/active-directory-saas-projectplace-tutorial/ic790228.png "사람")
...
4. **멤버 추가** 섹션에서 다음 단계를 수행합니다.
    ...
    a. **새 멤버** 텍스트 상자에서 프로비전할 유효한 AAD 계정의 이메일 주소를 관련 텍스트 상자에 입력합니다.
...
이 섹션에서는 Azure Single Sign-On을 사용할 수 있도록 Britta Simon에게 Projectplace에 대한 액세스 권한을 부여합니다.

![사용자 할당][200] 

**Britta Simon을 Projectplace에 할당하려면 다음 단계를 수행합니다.**
...
액세스 패널에서 Projectplace 타일을 클릭하면 Projectplace 응용 프로그램에 자동으로 로그온됩니다.
