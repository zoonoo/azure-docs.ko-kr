---
title: B2B - Azure AD에 대한 ID 공급자로 Google 추가
...
Google과의 페더레이션을 설정하면 초대된 사용자가 MSA(Microsoft 계정)를 만들지 않고도 자신의 Gmail 계정으로 공유 앱 및 리소스에 로그인할 수 있습니다. 

> [!NOTE]
> Google 페더레이션은 Gmail 사용자를 위해 특별히 설계되었습니다. G Suite 도메인을 사용하여 페더레이션하려면 [직접 페더레이션 기능을](direct-federation.md)사용합니다.
...
팀은 모든 기기에서 Google 게스트 사용자를 완벽하게 지원합니다. Google 사용자는 와 같은 `https://teams.microsoft.com`공통 끝점에서 팀에 로그인할 수 있습니다.

다른 애플리케이션의 공통 끝점은 Google 사용자를 지원하지 않을 수 있습니다. Google 게스트 사용자는 테넌트 정보가 포함된 링크를 사용하여 로그인해야 합니다. 예를 들면 다음과 같습니다.
  * `https://myapps.microsoft.com/?tenantid=<your tenant id>`
  * `https://portal.azure.com/<your tenant id>`
  * `https://myapps.microsoft.com/<your verified domain>.onmicrosoft.com`

   Google 게스트 사용자가 `https://myapps.microsoft.com` '또는'과 `https://portal.azure.com`같은 링크를 사용하려고 하면 오류가 발생합니다.

이 `https://myapps.microsoft.com/signin/Twitter/<application ID?tenantId=<your tenant ID>`링크에 테넌트 정보가 포함되어 있는 한 Google 게스트 사용자에게 응용 프로그램 또는 리소스에 대한 직접 링크를 제공할 수도 있습니다. 
...
3. 새 프로젝트가 프로젝트 메뉴에서 선택되어 있는지 확인합니다. 그런 다음 **API & 서비스에서** **OAuth 동의 화면을 선택합니다.**

4. **외부**을 선택한 다음 을 **선택합니다.** 
5. **OAuth 동의 화면에**응용 **프로그램 이름을**입력합니다. (다른 설정은 변경하지 말고 그대로 둡니다.)

   ![구글 OAuth 동의 화면 옵션을 보여주는 스크린 샷](media/google-federation/google-oauth-consent-screen.png)

6. **권한 있는 도메인** 섹션으로 스크롤하고 microsoftonline.com을 입력합니다.

   ![인증된 도메인 섹션을 보여주는 스크린샷](media/google-federation/google-oauth-authorized-domains.png)

7. **저장**을 선택합니다.

8. **자격 증명 을 선택합니다.** **자격 증명 만들기** 메뉴에서 **OAuth 클라이언트 ID**를 선택합니다.

   ![Google API를 보여주는 스크린샷은 자격 증명 옵션을 만듭니다.](media/google-federation/google-api-credentials.png)
...
9. **애플리케이션 유형**에서 **웹 애플리케이션**을 선택한 다음, **권한이 부여된 리디렉션 URI**에서 다음 URI를 입력합니다.
   ...
   ![공인 리디렉션 URIs 섹션을 보여주는 스크린샷](media/google-federation/google-create-oauth-client-id.png)

10. **만들기**를 선택합니다. 클라이언트 ID 및 클라이언트 비밀을 복사합니다. Azure AD 포털에서 ID 공급자를 추가할 때 사용하게 됩니다.

   ![OAuth 클라이언트 ID 및 클라이언트 비밀을 보여주는 스크린샷](media/google-federation/google-auth-client-id-secret.png)
...
2. **조직 관계**를 선택합니다.
3. **ID 공급자를**선택합니다.
4. **Google** 줄에서 컨텍스트 메뉴 **(...**) 를 선택한 다음 **삭제를**선택합니다. 
   
   ![소셜 ID 공급자에 대한 삭제 옵션을 보여주는 스크린샷](media/google-federation/google-social-identity-providers.png)
