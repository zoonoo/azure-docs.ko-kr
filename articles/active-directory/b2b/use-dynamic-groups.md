---
title: 동적 그룹 및 B2B 공동 작업 - Azure Active Directory | 마이크로 소프트 문서
...
Azure AD(Azure Active Directory)에 대한 보안 그룹 구성원의 동적 구성은 [Azure Portal](https://portal.azure.com)에서 사용할 수 있습니다. 관리자는 사용자 특성(예: userType, 부서 또는 국가/지역)을 기반으로 Azure AD에서 만든 그룹을 채우도록 규칙을 설정할 수 있습니다. 특성에 따라 구성원을 보안 그룹에 자동으로 추가하거나 보안 그룹에서 제거할 수 있습니다. 이러한 그룹은 SharePoint 사이트, 문서 등의 애플리케이션 또는 클라우드 리소스에 대한 액세스 권한을 제공하고 구성원에게 라이선스를 할당할 수 있습니다. [Azure Active Directory의 전용 그룹](../active-directory-accessmanagement-dedicated-groups.md)에서 동적 그룹에 대해 자세히 알아보세요.
...
1. 테넌트에서 글로벌 관리자 또는 사용자 관리자 역할을 할당한 계정으로 [Azure 포털에](https://portal.azure.com) 로그인합니다.
1. **Azure 활성 디렉터리**를 선택합니다.
2. **관리**에서 **그룹을**선택한 다음 **새 그룹을**선택합니다.
1. 새 **그룹** 페이지에서 **그룹 유형**아래에서 **보안을**선택합니다. 새 그룹의 **그룹 이름** 및 **그룹 설명**을 입력합니다. 
2. **멤버 자격 유형에서** **동적 사용자**를 선택한 다음 동적 **쿼리 추가를**선택합니다. 
4. 규칙 **구문** 텍스트 상자 위에서 **편집을**선택합니다. 규칙 **구문 편집** 페이지에서 텍스트 상자에 다음 표현식을 입력합니다.
...
1.  **저장**을 선택합니다. 이제 새 동적 그룹에는 B2B 게스트 사용자와 구성원 사용자가 포함됩니다.


1. 그룹을 만들려면 **새 그룹** 페이지에서 **만들기를** 선택합니다.

## <a name="creating-a-group-of-members-only"></a>구성원 그룹 만들기만

그룹이 게스트 사용자를 제외하고 테넌트의 구성원만 포함하려면 위에서 설명한 대로 동적 그룹을 만드지만 **규칙 구문** 상자에 다음 표현식을 입력합니다.

```
(user.objectId -ne null) and (user.userType -eq "Member")
```

다음 이미지는 멤버만 포함하고 게스트를 제외하도록 수정된 동적 그룹에 대한 규칙 구문을 보여 주며,

![사용자 유형이 구성원과 동일한 규칙 표시](media/use-dynamic-groups/all-member-user-rule-syntax.png)

## <a name="creating-a-group-of-guests-only"></a>게스트 그룹 만들기

정책(예: Azure AD 조건부 액세스 정책)을 적용할 수 있도록 게스트 사용자만 포함된 새 동적 그룹을 만드는 것이 유용할 수도 있습니다. 위에서 설명한 대로 동적 그룹을 만들지만 **규칙 구문** 상자에 다음 식을 입력합니다.

```
(user.objectId -ne null) and (user.userType -eq "Guest")
```

다음 이미지는 게스트만 포함하고 구성원 사용자를 제외하도록 수정된 동적 그룹에 대한 규칙 구문을 보여 주며,
...
- [B2B 공동 작업 사용자를 위한 조건부 액세스](conditional-access.md)
