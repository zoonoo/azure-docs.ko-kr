...
description: Azure AD B2B 공동 작업을 통해 클라우드 B2B 사용자에게 온프레미스 앱에 대한 액세스를 제공하는 방법을 보여 줍니다.
...
- 애플리케이션 프록시를 인증하는 동안 MFA가 사용되도록 게스트 사용자에 대한 MFA(Multi-Factor Authentication)를 설정할 수 있습니다. 자세한 내용은 [B2B 공동 작업 사용자에 대한 조건부 액세스를](conditional-access.md)참조하십시오.
- 클라우드 B2B 사용자에서 수행되는 모든 후원, 액세스 검토, 계정 확인 등은 온-프레미스 사용자에게 적용됩니다. 예를 들어 클라우드 사용자가 라이프사이클 관리 정책을 통해 삭제된 경우 온-프레미스 사용자는 MIM Sync 또는 Azure AD Connect 동기화를 통해 삭제됩니다. 자세한 내용은 [Azure AD 액세스 검토를 사용하는 게스트 액세스 관리를](../governance/manage-guest-access-with-access-reviews.md)참조하십시오.
