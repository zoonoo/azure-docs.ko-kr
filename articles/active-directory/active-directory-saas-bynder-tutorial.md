...
* 사용자가 해당 Azure AD 계정으로 Bynder SSO(Single Sign-on)에 자동으로 로그온되도록 설정할 수 있습니다.
...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Bynder에서 Microsoft Azure AD SSO를 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Bynder 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 Bynder의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Bynder의 **Username** 값으로 할당하여 설정합니다.

Bynder에서 Microsoft Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션에서는 클래식 포털에서 Microsoft Azure AD SSO를 사용하도록 설정하고 Bynder 응용 프로그램에서 SSO를 구성합니다.

**Bynder에서 Microsoft Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**
...
3. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행하고 **다음**을 클릭합니다.
  ...
  1. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://<company name>.getbynder.com/sso/SAML/authenticate/`
  2. **다음**을 클릭합니다.
4. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 **"고급 설정 표시(선택 사항)"**를 클릭하고 **로그온 URL**을 입력한 후 **다음**을 클릭합니다.
  ...
  1. **로그온 URL** 텍스트 상자에서 다음 패턴 `https://<company name>.getbynder.com/login/`을 사용하여 URL을 입력합니다.
  2. **다음**을 클릭합니다.
  
   >[!NOTE]
   >이 자습서의 로그온 URL 값은 자리 표시자일 뿐입니다. 작업 환경에 대한 실제 값을 가져오려면 Bynder에 문의하세요.
   >
...
이 섹션은 Britta Simon에게 Bynder에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
