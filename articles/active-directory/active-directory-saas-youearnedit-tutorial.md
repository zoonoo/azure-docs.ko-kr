<properties
	pageTitle="자습서: YouEarnedIt과 Azure Active Directory 통합 | Microsoft Azure"
...
# 자습서: Azure Active Directory와 YouEarnedIt 통합
...
5. **이 사용자에 대한 정보 입력** 대화 상자 페이지에서 다음 단계를 수행합니다. ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-youearnedit-tutorial/create_aaduser_05.png)
...
6.  **사용자 프로필** 대화 상자 페이지에서 다음 단계를 수행합니다. ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-youearnedit-tutorial/create_aaduser_06.png)
...
### YouEarnedIt 테스트 사용자 만들기

이 섹션에서는 YouEarnedIt에서 Britta Simon이라는 사용자를 만듭니다. YouEarnedIt 플랫폼에서 사용자를 추가하려면 YouEarnedIt 지원 팀에 문의하세요.

> [AZURE.NOTE]  YouEarnedIt은 ID 공급자가 NameID 특성의 EmailAddress 또는 UserName을 제공할 것으로 예상합니다. 해당 UserName 또는 EmailAddress가 데이터베이스 내에 없거나 정확히 일치하지 않을 경우 인증이 실패합니다. 이 경우 SSO 통합 전에 YouEarnedIt 시스템으로 계정을 가져와야 합니다(일반적으로 API 또는 CSV 가져오기를 통해).
