<properties
	pageTitle="자습서: Front와 Azure Active Directory 통합 | Microsoft Azure"
...
# 자습서: Front와 Azure Active Directory 통합
...
7. 결과 창에서 **Front**를 선택하고 **완료**를 클릭하여 응용 프로그램을 추가합니다.

	![갤러리에서 앱 선택](./media/active-directory-saas-front-tutorial/tutorial_front_0001.png)

##  Azure AD Single Sign-on 구성 및 테스트
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Front에서 Azure AD Single Sign-On을 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.

Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Front 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 Front의 관련 사용자 간에 연결이 형성되어야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 Front 응용 프로그램에서 Single Sign-On을 구성합니다.
...
2. **Front에 대한 사용자 로그인 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행하고 **다음**을 클릭합니다.
	...
	a. **식별자** 텍스트 상자에 다음 패턴으로 URL을 입력합니다. `https://<company name>.frontapp.com`

    b. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다. `https://<company name>.frontapp.com/sso/saml/callback`
...
4. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 **"고급 설정 표시(선택 사항)"**를 클릭하고 **로그온 URL**을 입력한 후 **다음**을 클릭합니다.
	...
	a. **로그인 URL** 텍스트 상자에서 다음과 같은 패턴을 사용하여 URL을 입력합니다. `https://<company name>.frontapp.com`

	b. **다음**를 클릭합니다.

	> [AZURE.NOTE] 이러한 값은 실제 값이 아닙니다. 실제 로그온 URL, 식별자 및 회신 URL로 값을 업데이트해야 합니다. 이러한 값을 가져오려면 **12단계**에서 자세한 내용을 참조하거나 [support@frontapp.com](emailTo:support@frontapp.com)을 통해 Front에 문의하면 됩니다.
...
7. **설정(왼쪽 세로 막대의 맨 아래에 있는 톱니바퀴 아이콘) > 기본 설정**으로 이동합니다.
...
10. **진입점** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **Single Sign-on 서비스 URL** 값을 입력합니다.
...
이 섹션의 목적은 Britta Simon에게 Front에 대한 액세스 권한을 부여하여 Azure Single Sign-On을 사용할 수 있도록 하는 것입니다.
