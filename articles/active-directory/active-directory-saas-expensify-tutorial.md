...
* 사용자가 해당 Azure AD 계정으로 Expensify SSO(Single Sign-On)에 자동으로 로그온되도록 설정할 수 있습니다.
...
7. 결과 창에서 **Expensify**를 선택한 다음 **완료**를 클릭하여 응용 프로그램을 추가합니다.
...
SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 Expensify 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 Expensify의 관련 사용자 간에 연결이 설정되어야 합니다.  

이 연결 관계는 Azure AD의 **사용자 이름** 값을 Expensify의 **Username** 값으로 할당하여 설정합니다.

Expensify에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD SSO를 사용하도록 설정하고 Expensify 응용 프로그램에서 Single Sign-On을 구성합니다.

**Expensify에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**

1. 클래식 포털의 **Expensify** 응용 프로그램 통합 페이지에서 **Single Sign-On 구성**을 클릭하여 **Single Sign-On 구성** 대화 상자를 엽니다.
   
    ![Single Sign-on 구성][6] 
2. **Expensify에 대한 사용자 로그온 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택한 후 **다음**을 클릭합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
  ...
  1. 로그인 URL 텍스트 상자에 다음 **“https://www.expensify.com/authentication/saml/login”**패턴을 사용하여 사용자가 Expensify 응용 프로그램에 로그인하는 데 사용할 URL을 입력합니다.
4. **Expensify에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
  ...
  2. **다음**을 클릭합니다.
5. Expensify에서 SSO를 사용하도록 설정하려면 먼저 응용 프로그램에서 **도메인 컨트롤** 을 사용하도록 설정해야 합니다. 응용 프로그램에서 도메인 컨트롤을 사용할 수 있습니다. 단계는 [여기](http://help.expensify.com/domain-control)에 나열됩니다. 추가 지원은 [help@expensify.com](mailto:help@expensify.com)을 참조하세요. 도메인 컨트롤을 사용하도록 설정한 후에는 다음 단계를 따릅니다.
   
    ![Single Sign-on 구성](./media/active-directory-saas-expensify-tutorial/tutorial_expensify_51.png) 
  1. Expensify 응용 프로그램에 로그인합니다.
  2. 위쪽에 도구 모음에서 **관리자**를 클릭합니다.
  3. 왼쪽 패널에서 **도메인 컨트롤**을 클릭합니다.
  4. 확인된 도메인 이름을 클릭합니다.
  5. 왼쪽 패널에서 **SAML**을 클릭한 다음 **사용**을 선택합니다.
  6. Azure AD에서 다운로드한 페더레이션 메타데이터를 열고 콘텐츠를 복사하여 **ID 공급자 메타데이터** 텍스트 상자에 붙여 넣습니다.
...
>[!NOTE]
>수동으로 사용자를 만들어야 하는 경우 Expensify 지원 팀에 문의해야 합니다. 
> 
