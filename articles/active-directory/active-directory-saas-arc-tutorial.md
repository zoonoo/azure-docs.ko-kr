...
4. 검색 상자에서 **Arc Publishing - SSO**를 입력하고, 결과 패널에서 **Arc Publishing - SSO**를 선택한 다음, **추가** 단추를 클릭하여 응용 프로그램을 추가합니다.

    ![결과 목록의 Arc Publishing - SSO](./media/active-directory-saas-arc-tutorial/tutorial_arc_addfromgallery.png)
...
3. **[Arc Publishing - SSO 테스트 사용자 만들기](#create-an-arc-publishing---sso-test-user)** - Britta Simon의 Azure AD 표현과 연결된 해당 사용자를 Arc Publishing - SSO에 만듭니다.
...
2. **Single Sign-On** 대화 상자에서 **모드**를 **SAML 기반 로그온**으로 선택하여 Single Sign-On을 사용하도록 설정합니다.
    ...
    ![Single Sign-On 대화 상자](./media/active-directory-saas-arc-tutorial/tutorial_arc_samlbase.png)

3. **Arc Publishing - SSO 도메인 및 URL** 섹션에서 응용 프로그램을 **IDP** 시작 모드로 구성하려는 경우 다음 단계를 수행합니다.

    ![Arc Publishing - SSO 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-arc-tutorial/tutorial_arc_url.png)

    a. **식별자** 텍스트 상자에서 `https://www.okta.com/saml2/service-provider/<Unique ID>` 패턴을 사용하여 URL을 입력합니다.

    나. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://arcpublishing-<Customer>.okta.com/sso/saml2/<Unique ID>`

4. **SP** 시작 모드에서 응용 프로그램을 구성하려면 **고급 URL 설정 표시**를 확인하고 다음 단계를 수행합니다.

    ![Arc Publishing - SSO 도메인 및 URL Single Sign-On 정보](./media/active-directory-saas-arc-tutorial/tutorial_arc_url1.png)
...
5. Arc Publishing - SSO 응용 프로그램에는 특정 형식의 SAML 어설션이 필요합니다. 이 응용 프로그램에 대해 다음 클레임을 구성합니다. 응용 프로그램 통합 페이지의 **사용자 특성** 섹션에서 이러한 특성의 값을 관리할 수 있습니다. 다음 스크린샷은 이에 대한 예제를 보여 줍니다.
...
6. **Single sign-on** 대화 상자의 **사용자 특성** 섹션에서 위의 이미지에 표시된 것과 같이 SAML 토큰 특성을 구성하고 다음 단계를 수행합니다.
    ...
    > [!NOTE]
    > 여기서 **groups** 특성은 **user.assignedroles**와 매핑됩니다. 이는 Azure AD에서 만든 사용자 지정 역할로서 응용 프로그램에서 그룹 이름을 다시 매핑합니다. Azure AD에서 사용자 지정 역할을 만드는 방법에 대한 자세한 지침은 [여기](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-enterprise-app-role-management)서 확인할 수 있습니다. 

7. **SAML 서명 인증서** 섹션에서 **인증서(Base64)** 를 클릭한 후 컴퓨터에 인증서 파일을 저장합니다.

    ![인증서 다운로드 링크](./media/active-directory-saas-arc-tutorial/tutorial_arc_certificate.png) 

8. **저장** 단추를 클릭합니다.

    ![Single Sign-On 구성 저장 단추](./media/active-directory-saas-arc-tutorial/tutorial_general_400.png)
    
9. **Arc Publishing - SSO 구성** 섹션에서 **Arc Publishing - SSO**를 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Arc Publishing - SSO 구성](./media/active-directory-saas-arc-tutorial/tutorial_arc_configure.png) 

10. **Arc Publishing - SSO** 쪽에서 Single Sign-On을 구성하려면 다운로드한 **인증서(Base64), 로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 [Arc Publishing - SSO 지원 팀](mailto:inf@washpost.com)으로 보내야 합니다. 이렇게 설정하면 SAML SSO 연결이 양쪽에서 제대로 설정됩니다.
...
1. Azure Portal의 왼쪽 창에서 **Azure Active Directory** 단추를 클릭합니다.
    ...
    ![Azure Active Directory 단추](./media/active-directory-saas-arc-tutorial/create_aaduser_01.png)

2. 사용자 목록을 표시하려면 **사용자 및 그룹**으로 이동한 후 **모든 사용자**를 클릭합니다.

    !["사용자 및 그룹" 및 "모든 사용자" 링크](./media/active-directory-saas-arc-tutorial/create_aaduser_02.png)

3. **사용자** 대화 상자를 열려면 **모든 사용자** 대화 상자 위쪽에서 **추가**를 클릭합니다.

    ![추가 단추](./media/active-directory-saas-arc-tutorial/create_aaduser_03.png)

4. **사용자** 대화 상자에서 다음 단계를 수행합니다.

    ![사용자 대화 상자](./media/active-directory-saas-arc-tutorial/create_aaduser_04.png)
...
2. 응용 프로그램 목록에서 **Arc Publishing - SSO**를 선택합니다.

    ![응용 프로그램 목록의 Arc Publishing - SSO 링크](./media/active-directory-saas-arc-tutorial/tutorial_arc_app.png)  
