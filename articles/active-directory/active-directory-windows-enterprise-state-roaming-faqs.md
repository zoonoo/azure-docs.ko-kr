...
- **언어 기본 설정**: 키보드 레이아웃, 시스템 언어, dateand 시간 등 
...
Windows 10에서 장치의 기본 계정만 설정 동기화에 사용할 수 있습니다. 예외에 대한 내용은 Windows 8의 Microsoft 계정 설정 동기화에서 Windows 10의 Azure AD 설정 동기화로 업그레이드하려면 어떻게 해야 하나요?를 참조하세요.

데이터는 장치의 서로 다른 사용자 계정 간에 절대로 혼합되지 않습니다. 설정 동기화에 대한 두 가지 규칙이 있습니다. 첫째, Windows 설정은 항상 기본 계정을 사용하여 로밍합니다. 둘째, 앱 데이터는 앱을 획득하는 데 사용된 계정을 사용하여 태그가 지정됩니다. 기본 계정을 사용하여 태그가 지정된 앱만 동기화 됩니다. 앱 소유권 태그 지정은 Windows 스토어를 통해 앱이 설치될 때 또는 MDM(모바일 장치 관리)을 통해 앱이 사이드 로드될 때 결정됩니다.
...
>[AZURE.NOTE]  
Microsoft 계정을 도메인 계정에 연결하고 모든 사용자 데이터를 Microsoft 계정과 동기화(예: “연결된 Microsoft 계정 및 Active Directory” 기능을 통해 Microsoft 계정 로밍)하는 기능은 연결된 Active Directory/Azure AD 환경에 조인되는 Windows 10 장치에서 제거되었습니다.
...
회사 장치에 개인 데이터를 저장하는 사용자는 Windows OS 및 응용 프로그램 데이터가 Azure AD와 동기화된다는 사실에 주의하셔야 합니다. 이는 다음과 같은 의미입니다. 개인용 Microsoft 계정 사용자의 설정이 조직용 Azure AD 계정의 설정과 “점점 멀어집니다”. 이제는 Microsoft 계정과 Azure AD 설정 동기화에서 서로 다른 계정을 사용하기 때문입니다. 연결된 Microsoft 계정을 통해 이전에 동기화된 Wi-Fi 암호, 웹 자격 증명, Internet Explorer 즐겨찾기 및 탭 등의 개인 데이터가 Azure AD를 통해 동기화됩니다.


## Microsoft 계정과 Azure AD 엔터프라이즈 상태 로밍의 상호 운용성이 어떻게 됩니까? 
Windows 10 2015 릴리스에서 엔터프라이즈 상태 로밍은 한 번에 한 계정에만 지원됩니다. 사용자가 조직용 Azure AD 계정을 사용하여 Windows에 로그인하면 모든 데이터가 Azure AD를 통해 동기화됩니다. 사용자가 개인용 Microsoft 계정을 사용하여 Windows에 로그인하면 모든 데이터가 Microsoft 계정을 통해 동기화됩니다. 앱 라이선스의 소유자가 기본 계정인 경우 앱 데이터는 장치의 기본 로그인 계정을 사용하여 로밍됩니다. 소유자가 보조 계정인 앱은 Windows 10에서 앱 데이터를 동기화하지 않습니다.
...
## 클래식 Windows 응용 프로그램의 설정 로밍 옵션으로 무엇이 있습니까?
로밍은 범용 Windows 앱에만 작동합니다. 클래식 Windows 응용 프로그램에서 로밍을 활성화하는 두 가지 옵션이 있습니다.

- Project Centennial을 사용하면 클래식 Windows 응용 프로그램을 범용 Windows 앱으로 간편하게 변환할 수 있습니다. 이때 앱 개발자가 Azure AD 앱 데이터 로밍을 활용하려면 최소한의 코드 변경 작업이 필요합니다. Win32 앱 데이터 로밍을 활성화하려는 경우 이 방법을 추천합니다. 
- [UE-V(사용자 경험 가상화)](https://technet.microsoft.com/library/dn458947.aspx) 템플릿 생성기를 사용하면 사용자 지정 설정 템플릿을 만들어서 앱이 실행될 때마다 앱에 이 템플릿을 적용할 수 있습니다. 이 옵션은 앱 개발자가 앱 코드를 변경할 필요가 없지만 Windows 10 2015 릴리스에서는 Microsoft Desktop Optimization Package를 구입한 고객만 온-프레미스 Active Directory 로밍이 가능하도록 UE-V가 제한됩니다. 앞으로 Microsoft는 UE-V를 Windows와 긴밀하게 통합하고 Azure AD 클라우드를 통해 설정을 로밍할 수 있도록 UE-V를 확장하는 방법을 연구할 것입니다. 

## 동기화된 설정 및 데이터를 온-프레미스에 저장할 수 있나요?
엔터프라이즈 상태 로밍은 설정 데이터를 Azure 클라우드에 저장하도록 설계되었습니다. UE-V는 Windows 10용 온-프레미스 로밍 솔루션을 제공합니다.

## 로밍되는 데이터를 관리하는 사람은 누구입니까?
테넌트 관리자가 데이터를 관리하고, 데이터는 Azure 데이터 센터에 저장됩니다. 모든 사용자 데이터는 Azure RMS(Azure Rights Management)를 사용하여 암호화된 상태로 전송되고 클라우드에 보관됩니다. 이는 사용자 자격 증명 같은 민감한 특정 데이터만 장치에서 내보내기 전에 암호화하는 Microsoft 계정 기반 설정 동기화에 비해 향상된 기능입니다.

Microsoft는 고객 데이터를 보호하기 위해 최선을 다하고 있습니다. 엔터프라이즈 사용자의 설정 데이터는 Windows 10 장치에서 나갈 때마다 Azure RMS를 통해 암호화되므로 다른 사용자가 이 데이터를 읽을 수 없습니다. 조직에서 Azure RMS에 대한 유료 구독을 보유한 경우 문서 추적 및 해제, 민감한 정보가 포함된 전자 메일 자동 보호, 사용자 고유 키("BYOK(Bring Your Own Key"라고도 함) 관리 등 다른 Azure RMS 기능을 사용할 수 있습니다. 이러한 기능 및 Azure RMS의 작동 방식에 대한 자세한 내용은 [Azure Rights Management란?](https://technet.microsoft.com/jj585026.aspx)을 참조하세요.

## 특정 앱 또는 설정에 대한 동기화를 관리할 수 있나요?
Windows 10에는 개별 앱의 로밍을 비활성화하는 MDM 또는 그룹 정책 설정이 없습니다. 테넌트 관리자는 관리되는 장치의 모든 앱에 대해 앱 데이터 동기화를 비활성화할 수 있지만 앱당 또는 앱 내부 수준에서 더욱 정교하게 제어하는 방법은 없습니다.
...
Microsoft에서는 사용자 프로필 로밍, UE-V, Azure AD 로밍을 포함하여 몇 가지 설정 로밍 솔루션을 제공하고 있습니다. 조직에서 데이터를 클라우드로 이동할 준비가 덜 되었거나 불편하게 생각하는 경우에는 기본 로밍 기술로 UE-V를 사용하시기를 권장합니다. 조직에서 클래식 Windows 응용 프로그램에 대한 로밍 지원이 필요하지만 클라우드로 이동하고 싶은 경우에는 엔터프라이즈 설정 동기화와 UE-V를 모두 사용하시기를 권장합니다. UE-V와 엔터프라이즈 설정 동기화는 매우 비슷한 기술이지만 상호 배타적인 관계는 아니며, 현재는 조직에서 사용자에게 필요한 로밍 서비스를 제공할 수 있도록 서로를 보완해 주는 관계에 있습니다. 엔터프라이즈 설정 동기화와 UE-V를 모두 사용할 경우 다음 규칙이 적용됩니다.

- 엔터프라이즈 상태 로밍은 장치의 기본 로밍 에이전트입니다. UE-V는 “Win32 갭”을 보완하기 위해 사용됩니다. 
- Windows 설정 및 최신 UWP 앱 데이터는 이미 엔터프라이즈 상태 로밍을 통해 로밍되고 있으므로 둘에 대한 UE-V 로밍을 비활성화해야 합니다. 

## 조직에서 한동안 로밍을 사용하다가 Azure RMS를 구입하면 어떻게 되나요? 
조직에서 Azure RMS 무료 구독을 통해 Windows 10에서 이미 로밍을 사용 중인 경우에는 유료 Azure RMS 구독을 구입해도 로밍 기능에 영향을 주지 않으며 IT 관리자가 구성을 변경할 필요가 없습니다.
...
- Internet Explorer의 **즐겨찾기** 동기화에 문제가 있습니다. 더 많은 문제가 발생하지 않도록 IE 즐겨찾기 동기화가 일시적으로 비활성화됩니다. 이 문제는 추후에 있을 Windows 10 업데이트에서 해결될 것입니다.
