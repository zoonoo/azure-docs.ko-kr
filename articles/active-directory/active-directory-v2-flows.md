<properties
	pageTitle="앱 모델 v2.0 앱 유형 | Microsoft Azure"
	description="Azure AD 앱 모델 v2.0 공개 미리 보기에서 지원되는 앱 형식 및 시나리오입니다."
...
1. 앱이 로그인을 위해 최종 사용자를 v2.0 끝점으로 보냅니다.
2. 사용자가 사용자 이름 및 암호나 다른 정보를 입력하여 인증합니다.
3. 사용자가 앱이 요청하는 사용 권한을 부여하여 앱이 대신 작업을 수행할 수 있는 권한을 부여합니다.
...
이러한 일곱 단계는 각각 빌드하는 앱 형식에 따라 약간씩 다르며, 자동으로 세부 정보를 처리하는 오픈 소스 라이브러리가 있습니다. [사용 권한, 동의 및 범위](active-directory-v2-scopes.md)에 대해 자세히 알아보거나, 계속해서 몇 가지 구체적인 예를 살펴보세요.

## 웹앱
브라우저를 통해 액세스되는 웹앱(.NET, PHP, Java, Ruby, Python, Node 등)의 경우 v2.0 앱 모델에서 [OpenID Connect](active-directory-v2-protocols.md#openid-connect-sign-in-flow)를 사용한 사용자 로그인을 지원합니다. OpenID Connect에서는 웹앱이 사용자 ID를 확인하고 클레임 형태로 사용자 정보를 제공하는 보안 토큰인 `id_token`을 받습니다.
...
간단한 로그인뿐 아니라 웹 서버 앱은 REST API와 같은 다른 일부 웹 서비스에 액세스해야 할 수도 있습니다. 이 경우 웹 서버 앱은 [OAuth 2.0 인증 코드 흐름](active-directory-v2-protocols.md#oauth2-authorization-code-flow)을 사용하여 결합된 OpenID Connect 및 OAuth 2.0 흐름에 참여할 수 있습니다. 이 시나리오는 아래의 [Web API 섹션](#web-apis)과 [WebApp-WebAPI 시작 항목](active-directory-v2-devquickstarts-webapp-webapi-dotnet.md)에서 설명합니다.

## Web API
v2.0 앱 모델을 사용하여 앱의 RESTful Web API와 같은 웹 서비스의 보안을 유지할 수 있습니다. Id\_token 및 세션 쿠키 대신 Web API는 OAuth 2.0 access\_token을 사용하여 데이터 보안을 유지하고 들어오는 요청을 인증합니다. Web API 호출자는 HTTP 요청의 인증 헤더에 access\_token을 추가합니다.
...
Web API는 웹 서버 앱, 데스크톱 및 모바일 앱, 단일 페이지 앱, 서버 쪽 데몬 및 다른 Web API까지 포함하여 모든 유형의 앱에서 access\_token을 받을 수 있습니다. 한 예로 Web API를 호출하는 웹 서버 앱에 대한 전체 흐름을 살펴보겠습니다.
...
이 흐름은 현재 v2.0 앱 모델에서 지원되지 않으므로 대화형 사용자 로그인 흐름이 발생한 후에만 앱이 토큰을 가져올 수 있습니다. 조만간 클라이언트 자격 증명 흐름이 추가될 예정입니다. 일반 공급 Azure AD 서비스에서 클라이언트 자격 증명 흐름을 확인하려면 [GitHub의 디먼 샘플](https://github.com/AzureADSamples/Daemon-DotNet)을 참조하세요.
