...
Microsoft Azure AD(Active Directory) 응용 프로그램 프록시를 사용하도록 설정한 후에 원격 사용자가 개인 네트워크 외부에서 액세스할 수 있도록 온-프레미스 응용 프로그램을 게시할 수 있습니다.

이 문서에서는 로컬 네트워크에 실행 중인 응용 프로그램을 게시하는 단계를 안내하고 네트워크 외부에서 안전한 원격 액세스를 제공합니다. 응용 프로그램 프록시를 설정하거나 커넥터를 설치하지 않은 경우 계속하기 전에 [Azure 포털에서 응용 프로그램 프록시 사용](active-directory-application-proxy-enable.md)의 단계를 수행합니다.

Azure AD 응용 프로그램 프록시를 처음 사용하는 경우 응용 프로그램을 게시하기 전에 개인 네트워크에서 웹 사이트를 게시하여 커넥터를 테스트하는 것이 좋습니다.
...
5. 응용 프로그램에 대한 다음과 같은 정보를 제공합니다.
	...
	- **사전 인증 메서드**: 응용 프로그램 프록시가 사용자에게 응용 프로그램에 대한 액세스를 제공하기 전에 해당 사용자를 확인하는 방법입니다. 드롭다운 메뉴에서 옵션 중 하나를 선택합니다.
...
사용자가 게시된 응용 프로그램에 액세스하기 위해서는 개별적으로 또는 그룹에 할당되어야 합니다. 사전 인증을 필요로 하는 앱의 경우 이렇게 하면 앱을 사용할 권한이 부여됩니다. 사전 인증을 요구하지 않는 앱의 경우 사용자에게 권한이 필요하지는 않지만 여전히 해당 응용 프로그램 목록에 표시되도록 앱에 할당되어야 합니다.
...
2. 디렉터리에서 특정 그룹을 검색 하거나 모든 사용자를 표시합니다. 확인 표시를 클릭하면 결과를 표시합니다.
