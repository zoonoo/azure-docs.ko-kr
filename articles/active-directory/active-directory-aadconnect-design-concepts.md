...
- 길이는 60자 미만이어야 합니다.
- & #92;! # $ % & * + / = ? ^ & #96; { } | ~ < > () '; : , [ ] " @ 기호와 같은 특수 문자는 포함할 수 없습니다.
- 전역적으로 고유해야 합니다.
- 문자열, 정수 또는 이진이어야 합니다.

선택된 sourceAnchor가 문자열 형식이 아닌 경우, Azure AD Connect는 특성 값에 특수 문자가 나타나지 않도록 Base64로 인코딩합니다. ADFS 이외의 다른 페더레이션 서버를 사용할 경우, 그 서버가 특성을 Base64로 인코딩할 수 있는지 확인하십시오.

SourceAnchor 특성은 대소문자를 구분합니다. "JohnDoe"의 값은 "johndoe"와 다릅니다.

단일 포리스트 온-프레미스가 있다면 사용해야할 특성은 **objectGUID**입니다. Azure AD Connect에서 사용되는 express 설정을 사용할 때 사용되는 특성이기도 하며 DirSync가 사용하는 특성이기도 합니다.

여러 포리스트가 있고 사용자를 포리스트 간 및 동일한 포리스트 내의 도메인 간에 이동하지 않을 경우에도 **objectGUID**는 사용하기 좋은 특성입니다.

포리스트와 도메인 간에 사용자를 이동하지 않는 경우 변경되지 않는 특성을 찾아야 합니다. 일반적으로 사용되는 특성에는 **employeeID**를 포함합니다. **sAMAccountName**처럼 문자를 포함하는 특성을 고려하는 경우, 특성 값의 대소문자를 변경할 수 있는 방법이 없는지 확인합니다. 나쁜 속성은 사용자의 이름의 해당 속성을 포함하여 사용할 수 없습니다. 결혼 또는 이혼 해서 이름을 바꿔야하는 경우에도 이 특성에서는 허용되지 않습니다. 이와 같은 또 하나의 이유는**userPrincipalName**, **mail**, and **targetAddress** 같은 특성은 AZure AD Connect 설치 마법사에서 선택조차 할 수 없기 때문입니다. 이러한 특성은 sourceAnchor에서 허용하지 않는 @-문자도 포함하고 있습니다.

부적합한 특성을 사용하려는 경우에는 가상 값을 도입해야 합니다. 예를 들어, GUID처럼 보이는 것을 포함할 수 있는 특성은 적합합니다. 개체가 생성되는 동안 새 GUID는 생성되고 사용자에게 표시됩니다. 개체를 이동할 때, 이 값의 콘텐츠도 복사했는지 확인합니다.
...
- 다른 Azure AD Connect 서버를 설치한 경우 이전에 사용했던 동일한 sourceAnchor 특성을 선택해야만 합니다. 이전부터 DirSync를 사용하고 Azure AD Connect로 이동한 경우, Dirsync에서 사용했던 **objectGUID**를 사용해야 합니다.
