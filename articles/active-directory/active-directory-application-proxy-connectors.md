...
- 사이트의 여러 데이터 센터가 서로 연결되어 있으며 데이터 센터 내에서 가능한 한 많은 트래픽을 유지하려고 하고 일반적으로 비용이 많이 들고 대기 시간이 긴 데이터 센터 간 링크를 이용하지 않으려는 경우. 각 데이터 센터에서 커넥터를 배포하여 데이터 센터 내의 응용 프로그램만 처리하고 귀중한 데이터 센터 간 링크를 절약할 수 있으며 사용자에게 완전히 투명한 환경을 제공할 수 있습니다.
...
- [응용 프로그램 프록시에서 발생하는 문제 해결](active-directory-application-proxy-troubleshoot.md)
