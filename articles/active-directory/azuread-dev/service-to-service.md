...
- OAuth 2.0 On-Behalf-Of 초안 사양으로 빌드된 웹 API를 호출해야 하는 서버 애플리케이션(예: 웹 API)
    ...
    이 시나리오에서는 사용자가 네이티브 애플리케이션에 대해 인증했으며 이 네이티브 애플리케이션이 웹 API를 호출해야 한다고 가정합니다. Azure AD는 웹 API를 호출하는 JWT 액세스 토큰을 발급합니다. 웹 API가 다른 다운스트림 웹 API를 호출해야 하는 경우 On-Behalf-Of 흐름을 사용하여 사용자의 ID를 위임하고 두 번째 계층 웹 API에 대해 인증할 수 있습니다.
...
1. HTTPS를 통해 웹 애플리케이션이 반환된 JWT 액세스 토큰을 사용하여 웹 API에 대한 요청의 권한 부여 헤더에 “전달자”를 지정한 JWT 문자열을 추가합니다. 그러면 웹 API에서 JWT 토큰의 유효성을 검사하여 유효성 검사가 성공하면 원하는 리소스를 반환합니다.
...
웹 API 시나리오에 대한 디먼 또는 서버 애플리케이션에 대한 코드 샘플은 [웹 API에 대한 서버 또는 디먼 애플리케이션](sample-v1-code.md#daemon-applications-accessing-web-apis-with-the-applications-identity)을 참조하세요.

## <a name="app-registration"></a>앱 등록

* 단일 테넌트 - 애플리케이션 ID 및 위임된 사용자 ID의 경우 모두, 디먼 또는 서버 애플리케이션을 Azure AD의 동일한 디렉터리에 등록해야 합니다. 여러 권한을 노출하도록 웹 API를 구성할 수 있으며 이는 해당 리소스에 대한 디먼 또는 서버의 액세스를 제한하는 데 사용됩니다. 위임된 사용자 ID 형식을 사용 중인 경우 서버 애플리케이션이 원하는 권한을 선택해야 합니다. 애플리케이션 등록에 대한 **API 권한** 페이지에서 **사용 권한 추가** 를 선택하고 API 패밀리를 선택한 다음 **위임된 권한** 을 선택하여 사용 권한을 선택합니다. 이 단계는 애플리케이션 ID 형식을 사용 중일 때는 필요 없습니다.
