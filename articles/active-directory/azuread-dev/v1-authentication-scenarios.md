---
title: 개발자를 위한 Azure AD(v1.0) | Azure
description: 앱 모델, API, 프로비저닝 및 가장 일반적인 인증 시나리오와 같은 개발자(v1.0)에 대한 Azure AD에 대한 인증 기본 사항에 대해 알아봅니다.
...
개발자를 위한 Azure Active Directory(v1.0) (Azure AD)는 OAuth 2.0 및 OpenID Connect와 같은 업계 표준 프로토콜과 오픈 소스 라이브러리를 지원하여 서비스로 ID를 제공하여 응용 프로그램 개발자에 대한 인증을 단순화합니다. 다양한 플랫폼에 대한 도움을 받아 코딩을 신속하게 시작할 수 있습니다.
...
* Azure AD는 ID 공급자입니다. ID 공급자는 조직의 디렉터리에 있는 사용자 및 응용 프로그램의 ID를 확인하고 해당 사용자 및 응용 프로그램의 성공적인 인증시 보안 토큰을 발급합니다.
* Azure AD에 대한 인증을 아웃소싱하려는 응용 프로그램은 Azure Active Directory(Azure AD)에 등록되어야 합니다. Azure AD는 앱을 디렉터리에 등록하고 고유하게 식별합니다.
...
* 사용자가 인증되면 응용 프로그램은 인증이 성공했는지 확인하기 위해 사용자의 보안 토큰의 유효성을 검사해야 합니다. 애플리케이션이 수행해야 하는 것을 보여주는 빠른 시작, 자습서 및 코드 샘플을 다양한 언어 및 프레임워크에서 찾을 수 있습니다.
...
* 인증 프로세스에 대한 요청의 흐름 및 응답은 OAuth 2.0, OpenID Connect, WS-Federation 또는 SAML 2.0과 같이 사용하는 인증 프로토콜에 의해 결정됩니다. 프로토콜에 대한 자세한 내용은 설명서의 **인증 프로토콜에 > 개념** 섹션을 참조하십시오.
...
|-----|----------|----------|--------------|---------------|--------------------|
|[권한 부여 코드 흐름](v1-protocols-oauth-code.md) | | x | x | x | x|  
|[암시적 흐름](v1-oauth2-implicit-grant-flow.md) | | x        | x    |      |                    |
|[하이브리드 OIDC 흐름](v1-protocols-openid-connect-code.md#get-access-tokens)| | x  | |          |            x   |
|[토큰 사용 새로 고침](v1-protocols-oauth-code.md#refreshing-the-access-tokens) | 토큰 새로 고침 | x | x | x| |
|[흐름을 대신하여](v1-oauth2-on-behalf-of-flow.md) | 액세스 토큰| x| x| x| |
...
1. Azure AD는 테넌트 A의 응용 프로그램 개체를 테넌트 B에서 서비스 주체를 만들기 위한 청사진으로 사용합니다.
1. 사용자가 요청된 토큰 수신
...
아래 표에서는 Azure AD에서 내보내는 각 클레임 유형에 대해 간략하게 설명합니다. 자세한 내용은 Azure AD에서 발급한 [액세스 토큰](../develop/access-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) 및 [ID 토큰을](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) 참조하세요.
