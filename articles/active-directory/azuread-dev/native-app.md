...
1. 네이티브 애플리케이션이 브라우저 팝업을 사용하여 Azure AD의 권한 부여 엔드포인트에 요청합니다. 이 요청에는 Azure Portal에 표시된 네이티브 애플리케이션의 애플리케이션 ID와 리디렉션 URI 및 웹 API에 대한 애플리케이션 ID URI가 포함됩니다. 사용자가 아직 로그인하지 않은 경우에는 로그인하라는 메시지가 다시 표시됩니다.
1. Azure AD가 사용자를 인증합니다. 다중 테넌트 애플리케이션이고 애플리케이션을 사용하는 데 동의가 필요한 경우에는 사용자가 아직 동의하지 않았다면 동의가 필요합니다. 동의한 다음 인증에 성공하면 Azure AD는 인증 코드 응답을 클라이언트 애플리케이션의 리디렉션 URI에 발급합니다.
1. Azure AD가 인증 코드 응답을 리디렉션 URI에 발급하면 클라이언트 애플리케이션이 브라우저 조작을 중지하고 응답에서 인증 코드를 추출합니다. 해당 인증 코드를 사용하여 클라이언트 애플리케이션은 인증 코드, 클라이언트 애플리케이션에 대한 정보(애플리케이션 ID 및 리디렉션 URI), 원하는 리소스(웹 API에 대한 애플리케이션 ID URI) 등이 포함된 요청을 Azure AD의 토큰 엔드포인트에 보냅니다.
...
1. HTTPS를 통해 클라이언트 애플리케이션이 반환된 JWT 액세스 토큰을 사용하여 웹 API에 대한 요청의 권한 부여 헤더에 “전달자”를 지정한 JWT 문자열을 추가합니다. 그러면 웹 API에서 JWT 토큰의 유효성을 검사하여 유효성 검사가 성공하면 원하는 리소스를 반환합니다.
...
* 단일 테넌트 - 네이티브 애플리케이션과 웹 API를 Azure AD의 동일한 디렉터리에 등록해야 합니다. 여러 권한을 노출하도록 웹 API를 구성할 수 있으며 이는 해당 리소스에 대한 네이티브 애플리케이션의 액세스를 제한하는 데 사용됩니다. 그러면 클라이언트 애플리케이션에서 Azure Portal의 “다른 애플리케이션에 대한 사용 권한” 드롭다운 메뉴에서 원하는 권한을 선택합니다.
