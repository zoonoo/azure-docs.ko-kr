...
* 사용자가 해당 Azure AD 계정으로 HireVue SSO(Single Sign-On)에 자동으로 로그온되도록 설정할 수 있습니다.
...
* Azure AD 구독
* HireVue SSO(Single Sign-On)가 설정된 구독
...
이 섹션에서는 "Britta Simon"이라는 테스트 사용자를 기반으로 HireVue에서 Azure AD SSO를 구성하고 테스트합니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 HireVue 사용자가 누구인지 알고 있어야 합니다. 즉, Azure AD 사용자와 HireVue의 관련 사용자 간에 연결이 형성되어야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 HireVue 응용 프로그램에서 SSO를 구성합니다.

**HireVue에서 Azure AD SSO를 구성하려면 다음 단계를 수행합니다.**
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행합니다.
  ...
  1. **로그온 URL** 텍스트 상자에 다음 패턴을 사용하여 사용자가 ICIMS 응용 프로그램에 로그온하는 데 사용할 URL을 입력합니다. 

    | Environment | URL |
    |---|---|
    | 프로덕션 | `https://<company name>.hirevue.com` |
    | 스테이징| `https://<company name>.stghv.com` |
  2. **식별자** 텍스트 상자에 다음 패턴으로 URN을 입력합니다.
...
4. **HireVue의 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
   
    ![Single Sign-On 구성](./media/active-directory-saas-hirevue-tutorial/tutorial_hirevue_05.png)  
  1. **인증서 다운로드**를 클릭하고 파일을 컴퓨터에 저장합니다.
  2. **다음**을 클릭합니다.
5. 응용 프로그램에 대해 구성된 SSO를 얻으려면 HireVue 지원 팀( [samlsupport@hirevue.com](mailTo:samlsupport@hirevue.com) )에 전자 메일로 문의하고 다음을 제공하세요.
...
5. **이 사용자에 대한 정보 입력** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-hirevue-tutorial/create_aaduser_05.png) 
  1. 사용자 유형에서 조직의 새 사용자를 선택합니다.
  2. 사용자 이름 **텍스트 상자**에 **BrittaSimon**을 입력합니다.
  3. **다음**을 클릭합니다.
6. **사용자 프로필** 대화 상자 페이지에서 다음 단계를 수행합니다.

   ![Azure AD 테스트 사용자 만들기](./media/active-directory-saas-hirevue-tutorial/create_aaduser_06.png) 
...
### <a name="create-an-hirevue-test-user"></a>HireVue 테스트 사용자 만들기
이 섹션에서는 HireVue에서 Britta Simon이라는 사용자를 만듭니다. HireVue 플랫폼에서 사용자를 추가하려면 HireVue 지원 팀에 문의하세요.

### <a name="assign-the-azure-ad-test-user"></a>Azure AD 테스트 사용자 할당
이 섹션에서는 Azure SSO를 사용할 수 있도록 Britta Simon에게 HireVue에 대한 액세스 권한을 부여합니다.
