...
3. **iLMS 도메인 및 URL** 섹션에서 **IDP** 시작 모드로 응용 프로그램을 구성하려는 경우 다음 단계를 수행합니다.
    ...
    b. **회신 URL** 텍스트 상자에 iLMS 관리 포털에 있는 SAML 설정의 **서비스 공급자** 섹션에서 복사한 다음과 같은 패턴의 **끝점(URL)** 값을 붙여 넣습니다. `https://www.inspiredlms.com/Login/<instanceName>/consumer.aspx`
...
10. **Settings(설정)** 탭에서 **SSO:SAML**을 클릭하여 SAML 설정을 열고 다음 단계를 수행합니다.
    ...
    c. Azure Portal에서 다운로드한 **메타데이터** 파일을 **SAML 서명 인증서** 섹션에서 선택합니다.
...
3. **“Register User”(사용자 등록)** 페이지에서 다음 단계를 수행합니다.
    ...
    c. **Email ID(전자 메일 ID)** 텍스트 상자에 Britta Simon 계정의 전자 메일 주소를 입력합니다.
