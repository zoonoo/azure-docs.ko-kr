...
>[!NOTE]
>이 기능은 현재 포털을 통해 제공되는 클레임 사용자 지정을 바꾸고 대체합니다([여기](https://docs.microsoft.com/azure/active-directory/develop/active-directory-saml-claims-customization)에 문서화됨). 동일한 응용 프로그램에서 이 문서에 자세히 설명된 Graph/Powershell 방법뿐 아니라 포털을 사용하여 클레임을 사용자 지정하는 경우 해당 응용 프로그램에 대해 발급된 토큰이 포털의 구성을 무시합니다.
이 문서에 설명된 방법을 통해 수행된 구성은 포털에 반영되지 않습니다.

이 기능은 테넌트 관리자가 테넌트의 특정 응용 프로그램에 대한 토큰에 내보내지는 클레임을 사용자 지정하는 데 사용됩니다. 클레임 매핑 정책을 사용하여 다음을 수행할 수 있습니다.
...
>[!NOTE]
>이 기능은 현재 공개 미리 보기로 제공되고 있습니다. 변경 내용을 되돌리거나 제거할 준비를 해야 합니다. 이 기능은 공개 미리 보기 동안 모든 Azure Active Directory 구독에서 사용할 수 있습니다. 그러나 기능이 일반 공급되면 일부 기능에는 Azure Active Directory Premium 구독이 필요할 수도 있습니다.

## <a name="claims-mapping-policy-type"></a>클레임 매핑 정책 유형
Azure AD에서 정책 개체는 개별 응용 프로그램 또는 조직의 모든 응용 프로그램에 적용되는 규칙 집합을 나타냅니다. 각 정책 유형마다 할당된 개체에 적용되는 속성 집합을 포함하는 고유한 구조가 있습니다.

클레임 매핑 정책은 특정 응용 프로그램에 대해 발급된 토큰에 내보내지는 클레임을 수정하는 정책 개체 유형입니다.
...
### <a name="table-1---jwt-restricted-claim-set"></a>표 1 - JWT 제한 클레임 집합
|클레임 형식(이름)|
| ----- |
|_claim_names|
|_claim_sources|
...
클레임 매핑 정책의 속성은 내보내지는 클레임과 데이터가 소싱되는 위치를 제어하는 데 사용됩니다. 정책을 설정하지 않으면 시스템은 핵심 클레임 집합, 기본 클레임 집합 및 응용 프로그램이 수신하도록 옵트인한 선택적 클레임이 포함된 토큰을 발급합니다.
...
>[!NOTE] 
>핵심 클레임 집합의 클레임은 이 속성의 설정값에 관계없이 모든 토큰에 포함됩니다. 
...
**데이터 형식:** 하나 이상의 클레임 스키마 항목을 포함하는 JSON BLOB(형식은 샘플 클레임 매핑 정책 섹션 참조)

**요약:** 이 속성은 기본 클레임 집합(IncludeBasicClaimSet가 True로 설정된 경우) 및 핵심 클레임 집합뿐 아니라 정책의 영향을 받는 토큰에 포함될 클레임을 정의하는 데 사용됩니다.
이 속성에 정의된 각 클레임 스키마 항목에 대해 데이터가 제공되는 위치(값 또는 원본/ID 쌍) 및 데이터가 내보내지는 클레임(클레임 형식)의 두 가지 정보를 설정해야 합니다.
...
- "company": 클레임의 데이터가 리소스 테넌트에 대한 회사 개체의 속성입니다.
- "transformation": 클레임의 데이터가 클레임 변환에서 제공됩니다. 아래의 클레임 변환 섹션을 참조하세요. 

원본이 변환인 경우 TransformationID 요소도 이 클레임 정의에 포함되어야 합니다.

ID 요소는 클레임의 값을 제공할 원본의 속성을 식별합니다. 아래 표에는 각 원본 값에 유효한 ID 값이 나와 있습니다.
...
- JwtClaimTypeshould에는 JWT로 내보낼 클레임 이름이 포함되어야 합니다.
- SamlClaimType에는 SAML 토큰으로 내보낼 클레임 URI가 포함되어야 합니다.

>[!NOTE]
>제한된 클레임 집합에 있는 클레임의 이름 및 URI는 클레임 형식 요소에 사용할 수 없습니다. 아래 예외 및 제한 사항 섹션을 참조하세요.
...
**데이터 형식:** 하나 이상의 변환 항목을 포함하는 JSON BLOB(형식은 샘플 클레임 매핑 정책 섹션 참조)

**요약:** 이 속성은 클레임 스키마에 지정된 클레임에 대한 출력 데이터를 생성하기 위해 원본 데이터에 일반 변환을 적용하는 데 사용됩니다.
변환 항목 요소

**ID:** ID 요소는 클레임 스키마 항목의 TransformationID에서 이 변환 항목을 참조하는 데 사용됩니다. 이 값은 이 정책 내에서 각 변환 항목에 고유해야 합니다.

**TransformationMethod:** TransformationMethod 요소는 클레임에 대한 데이터를 생성하기 위해 수행할 작업을 식별합니다.

선택한 방법에 따라, 입/출력 집합이 예상됩니다. 이러한 값은 InputClaims, InputParameters 및 OutputClaims 요소를 사용하여 정의됩니다.

### <a name="table-4---transformation-methods-and-expected-inputsoutputs"></a>표 4 - 변환 메서드 및 예상 입/출력
|TransformationMethod|예상 입력|예상 출력|설명|
|-----|-----|-----|-----|
|Join|string1, string2, 구분 기호|outputClaim|구분 기호를 사용하여 입력 문자열을 조인합니다. 예: string1:"foo@bar.com" , sring2:"sandbox" , separator:"."은 outputClaim:"foo@bar.com.sandbox"를 생성합니다.|
...
**InputClaims:** InputClaims 요소를 사용하여 클레임 스키마 항목에서 변환에 데이터를 전달할 수 있습니다. 두 개의 특성 ClaimTypeReferenceId 및 TransformationClaimType이 있습니다.

- ClaimTypeReferenceId가 클레임 스키마 항목의 ID 요소와 조인되어 적절한 입력 클레임을 찾습니다. 
- TransformationClaimType은 이 입력에 고유 이름을 지정하는 데 사용됩니다. 이 이름은 변환 방법에 대한 예상 입력 중 하나와 일치해야 합니다.

**InputParameters:** InputParameters 요소는 변환에 상수 값을 전달하는 데 사용됩니다. 두 개의 특성, Value 및 ID가 있습니다.
Value는 전달할 실제 상수 값입니다.
ID는 이 입력에 고유 이름을 지정하는 데 사용됩니다. 이 이름은 변환 방법에 대한 예상 입력 중 하나와 일치해야 합니다.

**OutputClaims:** OutputClaims 요소는 변환에 의해 생성된 데이터를 보관하는 데 사용되며 클레임 스키마 항목에 연결합니다. 두 개의 특성 ClaimTypeReferenceId 및 TransformationClaimType이 있습니다.

ClaimTypeReferenceId가 클레임 스키마 항목의 ID와 조인되어 적절한 출력 클레임을 찾습니다. TransformationClaimType은 이 출력에 고유 이름을 지정하는 데 사용됩니다. 이 이름은 변환 방법에 대한 예상 출력 중 하나와 일치해야 합니다.

### <a name="exceptions-and-restrictions"></a>예외 및 제한 사항

**SAML NameID 및 UPN:** NameID 및 UPN 값을 소싱하는 특성과 허용되는 클레임 변환은 제한됩니다.

### <a name="table-5---attributes-allowed-as-data-source-for-saml-nameid"></a>표 5 - SAML NameID에 대한 데이터 원본으로 허용되는 특성
|원본|ID|설명|
|-----|-----|-----|
|사용자|mail|메일 주소|
|사용자|userprincipalname|사용자 계정 이름|
...
클레임 매핑 정책이 게스트 사용자에게 적용되지 않습니다. 게스트 사용자가 해당 서비스에 클레임 매핑 정책이 적용된 응용 프로그램에 액세스하려고 하면 기본 토큰이 발급됩니다(정책이 적용되지 않음).
...
특정 서비스 주체에 대해 토큰에 내보내지는 클레임을 사용자 지정할 수 있는 경우 Azure AD에서 많은 시나리오가 가능합니다. 이 섹션에서는 클레임 매핑 정책 형식을 사용하는 방법을 이해하는 데 도움이 되는 몇 가지 일반적인 시나리오를 안내합니다.
...
이 예제에서는 연결된 서비스 주체에 발급된 토큰에 EmployeeID 및 TenantCountry를 추가하는 정책을 만듭니다. EmployeeID는 SAML 토큰 및 JWT 둘 다에서 이름 클레임 형식으로 내보내집니다. TenantCountry는 SAML 토큰 및 JWT 둘 다에서 국가 클레임 형식으로 내보내집니다. 이 예제에서는 토큰에 기본 클레임 집합도 계속 포함하도록 선택합니다.
...
2.  서비스 주체에게 정책을 할당합니다. 서비스 주체의 ObjectId도 가져와야 합니다. 
    ...
    Add-AzureADServicePrincipalPolicy -Id <ObjectId of the ServicePrincipal> -RefObjectId <ObjectId of the Policy>
    ```
#### <a name="example-create-and-assign-a-policy-utilizing-a-claims-transformation-in-tokens-issued-to-a-service-principal"></a>예제: 서비스 주체에 발급된 토큰에서 클레임 변환을 이용하는 정책을 만들고 할당합니다.
이 예제에서는 연결된 서비스 주체에 발급된 JWT에 사용자 지정 클레임 “JoinedData”를 내보내는 정책을 만듭니다. 이 클레임에는 사용자 개체의 extensionattribute1 특성에 저장된 데이터와 ".sandbox"를 조인하여 만든 값이 포함됩니다. 이 예제에서는 토큰에서 기본 클레임 집합도 계속 제외하도록 선택합니다.
