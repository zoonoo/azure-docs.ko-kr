...
8. Single Sign-On 섹션에서 다음 단계를 수행 합니다.
    ...
    b. Azure 클래식 포털의 **TalentLMS에서 Single Sign-On 구성** 대화 상자 페이지에서 **ID 공급자 ID** 값을 복사한 다음 **ID 공급자(IdP)** 텍스트 상자에 붙여 넣습니다.
   
    c. 내보낸 인증서에서 **지문** 값을 복사한 다음 **인증서 지문** 텍스트 상자에 붙여넣습니다.
    ...
    ㄹ. Azure 클래식 포털의 **TalentLMS에서 Single Sign-On 구성** 대화 상자 페이지에서 **원격 로그인 URL** 값을 복사한 다음 **원격 로그인 URL** 텍스트 상자에 붙여 넣습니다.
   
    e. Azure 클래식 포털의 **TalentLMS에서 Single Sign-On 구성** 대화 상자 페이지에서 **원격 로그아웃 URL** 값을 복사한 다음 **원격 로그아웃 URL** 텍스트 상자에 붙여 넣습니다.
   
    f. **대상 ID** 텍스트 상자에 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name**을 입력합니다.
   
    g. **이름** 텍스트 상자에 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname**을 입력합니다.
   
    h. **성** 텍스트 상자에 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname**을 입력합니다.
   
    i. **전자 메일** 텍스트 상자에 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress**를 입력합니다.
...
3. **사용자 추가** 대화 상자 페이지에서 다음 단계를 수행합니다.
    ...
    a. Azure AD 사용자 계정의 관련 특성 값을 다음 텍스트 상자에 입력합니다. **이름**, **성**, **메일 주소**를 입력합니다.
