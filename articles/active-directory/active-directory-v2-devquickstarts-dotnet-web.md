...
-	`Startup.cs` 프로젝트에 "OWIN 시작 클래스"를 추가하고, 프로젝트를 마우스 오른쪽 단추로 클릭한 다음 **추가** --> **새 항목** --> "OWIN"을 검색합니다. OWIN 미들웨어는 앱이 시작되면 `Configuration(...)` 메서드를 호출합니다.
...
참조를 위해 완성된 샘플(사용자 구성 값 제외)은 [여기에서 .zip으로 다운로드](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-OpenIdConnect-DotNet/archive/complete.zip)하거나 GitHub에서 복제할 수 있습니다.
