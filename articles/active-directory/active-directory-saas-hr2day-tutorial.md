...
HR2day by Merces에서 Azure AD의 **사용자 이름** 값을 **Username** 값으로 할당하여 링크 관계를 설정합니다.

HR2day by Merces에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.

1. **[Azure AD Single Sign-On 구성](#configuring-azure-ad-single-sign-on)** - 사용자가 이 기능을 사용할 수 있도록 합니다.
2. **[Azure AD 테스트 사용자 만들기](#creating-an-azure-ad-test-user)** - Britta Simon으로 Azure AD Single Sign-On을 테스트하는 데 사용합니다.
3. **[HR2day by Merces 테스트 사용자 만들기](#creating-an-hr2day-by-merces-test-user)** - Britta Simon의 Azure AD 표현과 연결되는 대응 사용자를 HR2day by Merces에 만듭니다.
...
1. Azure Portal의 **HR2day by Merces** 응용 프로그램 통합 페이지에서 **Single Sign-On**을 클릭합니다.
...
3. **HR2day by Merces 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-hr2day-tutorial/tutorial_hr2daybymerces_url.png)

    a. **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<tenantname>.force.com/<instancename>`

    b. **식별자** 텍스트 상자에서 `https://hr2day.force.com/<companyname>` 패턴을 사용하여 URL을 입력합니다.
...
5. 이 섹션은 사용자가 SAML 프로토콜 기반 페더레이션을 사용하여 Azure AD의 계정으로 HR2day by Merces에 인증할 수 있게 하는 방법을 간략하게 설명하기 위한 것입니다.
...
6. **Single Sign-On** 대화 상자의 **사용자 특성** 섹션에서 이미지에 표시된 것과 같이 SAML 토큰 특성을 구성하고 다음 단계를 수행합니다.
    ...
    b. **이름** 텍스트 상자에 **“ATTR_LOGINCLAIM”**을 입력합니다.

    c. **값** 목록에서 **Join()**을 선택합니다.

    d. **String1** 목록에서 **user.mail**을 선택합니다.

    e. **String2** 목록에서 HR2day 팀이 제공하는 **고유 식별자**를 입력합니다.
...
8. **HR2day by Merces 구성** 섹션에서 **HR2day by Merces 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.

    ![Single Sign-on 구성](./media/active-directory-saas-hr2day-tutorial/tutorial_hr2daybymerces_configure.png) 

9. 응용 프로그램에 SSO를 구성하려면 [HR2day by Merces 클라이언트 지원 팀](mailTo:servicedesk@merces.nl)에 문의하고 다운로드한 **인증서(Base64)** 파일을 메일에 첨부합니다. 또한 SSO 통합을 위해 구성할 수 있도록 **로그아웃 URL, SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 제공하세요.
...
### <a name="creating-an-hr2day-by-merces-test-user"></a>HR2day by Merces 테스트 사용자 만들기

이 섹션의 목적은 HR2day by Merces에서 Britta Simon이라는 사용자를 만드는 것입니다. HR2day by Merces 계정에 사용자를 추가하려면 [HR2day by Merces 클라이언트 지원 팀](mailto:servicedesk@merces.nl)에 문의하세요. 

> [!NOTE]
> 사용자를 수동으로 생성해야 하는 경우 [HR2day by Merces 클라이언트 지원 팀](mailto:servicedesk@merces.nl)에 문의해야 합니다.
