...
## <a name="select-your-custom-domain"></a>사용자 지정 도메인 선택
...
4. 외부 URL이 https인 경우 Azure에서 응용 프로그램의 URL에 대한 유효성을 검사할 수 있도록 인증서를 업로드하라는 메시지가 표시됩니다. 응용 프로그램의 외부 URL과 일치하는 와일드 카드 인증서를 업로드할 수도 있습니다. 이 도메인은 [Azure 확인된 도메인](https://msdn.microsoft.com/library/azure/jj151788.aspx)목록 안에 있어야 합니다. Azure는 응용 프로그램에 대한 외부 URL과 일치하는 응용 프로그램 또는 와일드카드 인증서의 도메인 URL에 대한 인증서가 있어야 합니다.
...
**Q: 이미 업로드한 인증서를 다시 업로드하지 않고 선택할 수 있나요?**  
...
**Q: 새 도메인을 등록한다면 이 도메인을 사용할 수 있나요?**  
A: 예, 도메인 목록은 테넌트의 확인된 도메인 목록에서 공급됩니다.  

**Q: 인증서가 만료되면 어떻게 되나요?**  
A: 응용 프로그램 구성 페이지의 인증서 섹션에서 경고가 표시됩니다. 사용자가 응용 프로그램에 액세스하려고 하면 보안 경고가 표시됩니다.  

**Q: 지정된 앱에 대한 인증서를 바꾸려면 어떻게 해야 하나요?**  
A: 응용 프로그램 구성 페이지에서 새 인증서를 업로드합니다.  

**Q: 인증서를 삭제하고 바꿀 수 있나요?**  
A: 새 인증서를 업로드할 때 이전 인증서가 다른 응용 프로그램에서 사용 중이지 않으면 자동으로 삭제됩니다.  
...
**Q: 내 테넌트에 대한 모든 인증서를 볼 수 있는 곳이 있나요?**  
