...
Q: 이미 업로드한 인증서를 다시 업로드하지 않고 선택할 수 있습니까? A: 이전에 업로드된 인증서는 자동으로 응용 프로그램에 바인딩되며 응용 프로그램의 호스트 이름과 정확히 일치하는 하나의 인증서가 있습니다. ... Q: 인증서를 어떻게 추가하며 내보낸 인증서를 어떤 형식으로 업로드해야 하나요? A: 응용 프로그램 구성 페이지에서 인증서를 업로드해야 합니다. 인증서는 PFX 파일이어야 합니다. ... Q: ECC 인증서를 사용할 수 있나요? A: 서명 메서드에 명시적 제한은 없습니다. ... Q: SAN 인증서를 사용할 수 있나요? A: 예. ... Q: 와일드카드 인증서를 사용할 수 있나요? A: 예. ... Q: 각 응용 프로그램에서 다른 인증서를 사용할 수 있나요? A: 예, 두 응용 프로그램이 같은 외부 호스트를 공유하지 않는다면 가능합니다. … Q: 새 도메인을 등록한다면 이 도메인을 사용할 수 있습니까? A: 예, 도메인 목록은 테넌트의 확인된 도메인 목록에서 공급됩니다. ... Q: 인증서가 만료되면 어떻게 되나요? A: 응용 프로그램 구성 페이지의 인증서 섹션에 경고가 표시됩니다. 사용자가 응용 프로그램에 액세스하려는 경우 보안 경고가 팝업됩니다. ... Q: 지정된 앱에 대한 인증서를 바꾸려면 어떻게 해야 하나요? A: 응용 프로그램 구성 페이지에서 새 인증서를 업로드합니다. ... Q: 인증서를 삭제하고 바꿀 수 있나요? A: 새 인증서를 업로드할 때 기존 인증서가 다른 응용 프로그램에서 사용 중이지 않으면 자동으로 삭제됩니다. ... Q: 인증서가 해지되면 어떻게 되나요? A: 인증서에 대해 해지 검사는 수행되지 않습니다. 사용자가 응용 프로그램에 액세스하려고 할 때 브라우저에 따라서 보안 경고가 표시될 수 있습니다. … Q: 자체 서명된 인증서를 사용할 수 있나요? A: 예, 자체 서명된 인증서가 허용됩니다. 개인 인증 기관을 사용하는 경우 인증서에 대한 CDP(인증서 해지 지점 배포 지점)는 공용이어야 합니다. ... Q: 내 테넌트에 대한 모든 인증서를 볼 수 있는 곳이 있습니까? A: 현재 버전에서는 지원되지 않습니다.
