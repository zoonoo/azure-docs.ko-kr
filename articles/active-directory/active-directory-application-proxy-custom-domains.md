...
Azure Active Directory 응용 프로그램 프록시를 통해 응용 프로그램을 게시할 때 사용자가 원격으로 작업 중일 때 이동하도록 외부 URL을 만듭니다. 이 URL은 기본 도메인 *yourtenant-msappproxy.net*을 가져옵니다. 자신의 도메인 이름을 사용하려는 경우 응용 프로그램에 대한 사용자 지정 도메인을 구성합니다. 
...
- 모든 응용 프로그램이 동일한 내부 및 외부 URL을 갖는 경우 다른 응용 프로그램을 가리키는 하나의 응용 프로그램의 링크는 회사 네트워크 외부에서 계속해서 작동합니다. 
...
5. 비활성화되었던 **인증서** 필드가 활성화됩니다. 이 필드를 선택합니다. 
   ...
   이 필드가 비활성화로 유지되는 경우 아마도 인증서가 이미 해당 도메인에 대해 업로드된 것을 의미합니다. 

6. PFX 인증서를 업로드하고 인증서의 암호를 입력합니다. 
7. **저장**을 선택하여 변경 내용을 저장합니다. 
8. msappproxy.net 도메인에 새 외부 URL을 리디렉션하는 DNS 레코드를 추가합니다. 
...
인증서 서명 메서드에 대한 제한은 없습니다. ECC, SAN 및 다른 일반적인 인증서 형식이 모두 지원됩니다. 와일드카드 인증서를 사용할 수도 있습니다. 와일드카드 인증서를 사용하는 경우 와일드카드가 원하는 외부 URL과 일치하는지 확인합니다. 자체 서명된 인증서도 허용됩니다. 개인 인증 기관을 사용하는 경우 인증서에 대한 CDP(인증서 해지 지점 배포 지점)는 공용이어야 합니다.

### <a name="changing-the-domain"></a>도메인 변경
모든 확인된 도메인은 응용 프로그램에 대한 외부 URL 드롭다운 목록에 나타납니다. 도메인을 변경하려면 응용 프로그램에 대한 해당 필드를 업데이트합니다. 연결된 인증서가 없는 도메인을 선택하는 경우 5-7단계를 수행하여 인증서를 추가합니다. 원하는 도메인이 목록에 없는 경우 [확인된 도메인으로 추가](active-directory-domains-add-azure-portal.md)합니다. 그런 다음 새 외부 URL에서 리디렉션할 DNS 레코드를 업데이트해야 합니다. 
