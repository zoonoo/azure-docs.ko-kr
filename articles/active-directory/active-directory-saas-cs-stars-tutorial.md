...
이 자습서에서는 CS Stars와 Azure AD(Azure Active Directory)를 통합하는 방법을 보여 줍니다. CS Stars를 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.
...
6. 검색 상자에 **CS Stars**를 입력합니다.

	![응용 프로그램][5]

7. 결과 창에서 **CS Stars**를 선택하고 **완료**를 클릭하여 응용 프로그램을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 CS Stars 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 CS Stars의 관련 사용자 간에 연결이 형성되어야 합니다. 이 연결 관계는 Azure AD의 **사용자 이름** 값을 CS Stars의 **Username** 값으로 할당하여 설정합니다.
...
이 섹션은 Azure 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 CS Stars에서 Single Sign-On을 구성하는 방법을 설명하기 위한 것입니다.

**CS Stars에서 Azure AD Single Sign-on을 구성하려면 다음 단계를 수행합니다.**

1. Azure 클래식 포털의 **CS Stars** 응용 프로그램 통합 페이지에서 **Single Sign-On 구성**을 클릭하여 **Single Sign-On 구성** 대화 상자를 엽니다.

	![Single Sign-On 구성][6]

2. **CS Stars에 대한 사용자 로그온 방법 선택** 페이지에서 **Azure AD Single Sign-On**을 선택하고 **다음**을 클릭합니다.
...
4. **CS Stars에서 Single Sign-On 구성** 페이지에서 **메타데이터 다운로드**를 클릭한 다음 컴퓨터에 로컬로 메타데이터 파일을 저장합니다.
...
CS Stars에서 만든 사용자를 가져오려면 Marsh ClearSight 담당자에게 문의해야 합니다.


### Azure AD 테스트 사용자 할당

이 섹션의 목적은 Britta Simon에게 CS Stars에 대한 액세스 권한을 부여하여 Single Sign-On을 사용할 수 있도록 하는 것입니다.
...
이 섹션은 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트하기 위한 것입니다. 액세스 패널에서 CS Stars 타일을 클릭하면 CS Stars 응용 프로그램에 자동으로 로그온됩니다.
