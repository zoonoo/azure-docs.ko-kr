...
[Azure 클래식 포털][AZURE-CLASSIC-PORTAL]을 통해 응용 프로그램 매니페스트라고 하는 JSON 파일 표현을 다운로드 및 업로드하여 응용 프로그램의 ID 구성을 관리할 수 있습니다. 어떤 실제 파일도 디렉터리에 저장되지 않습니다. 응용 프로그램 매니페스트는 Azure AD Graph API 응용 프로그램 엔터티의 단순한 HTTP GET 작업이며 업로드는 응용 프로그램 엔터티의 HTTP PATCH 작업입니다.
...
1. [Azure 클래식 포털][AZURE-CLASSIC-PORTAL]로 이동하고 서비스 관리자 또는 공동 관리자 권한이 있는 계정으로 로그인합니다.
...
4. 응용 프로그램의 기본 페이지를 선택했으므로 페이지의 아래쪽에서(1) "매니페스트 관리" 기능을 확인합니다. 이 링크를 클릭하면 JSON 매니페스트 파일을 업로드하거나 다운로드하 라는 메시지가 표시됩니다. "매니페스트 다운로드"(2)를 클릭합니다. "매니페스트 다운로드"(3)를 클릭하면 바로 뒤에 확인하라는 메시지가 표시된 다운로드 확인 대화 상자가 나타납니다. 그러면 파일을 로컬로 열거나 저장합니다(4).
   
    ![매니페스트 관리, 다운로드 옵션][MANAGE-MANIFEST-DOWNLOAD]
   
    ![메타데이터 다운로드][DOWNLOAD-MANIFEST]
5. 이 예에서 파일을 로컬에서 저장하면 이를 편집기에서 열고 JSON을 변경하고 다시 저장할 수 있습니다. Visual Studio JSON 편집기 내에서 JSON 구조체 모양은 다음과 같습니다. 앞에서 언급한 대로 [응용 프로그램 엔터티][APPLICATION-ENTITY]의 스키마를 따릅니다.
