...
---
# <a name="tutorial-azure-active-directory-integration-with-moveit-transfer"></a>자습서: Azure Active Directory와 MOVEit Transfer 통합
...
* 사용자가 해당 Azure AD 계정으로 MOVEit Transfer SSO(Single Sign-On)에 자동으로 로그온되도록 설정할 수 있습니다.
...
## <a name="adding-moveit-transfer-from-the-gallery"></a>갤러리에서 MOVEit Transfer 추가
...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 MOVEit Transfer에서 Azure AD SSO를 구성하고 테스트하는 방법을 보여 주기 위한 것입니다.

SSO가 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 MOVEit Transfer 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 MOVEit Transfer의 관련 사용자 간에 연결이 형성되어야 합니다.

이 연결 관계는 Azure AD의 **사용자 이름** 값을 MOVEit Transfer의 **Username** 값으로 할당하여 설정합니다.

MOVEit Transfer에서 Azure AD SSO를 구성하고 테스트하려면 다음 구성 요소를 완료해야 합니다.
...
이 섹션에서는 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 MOVEit Transfer 응용 프로그램에서 SSO를 구성합니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.
  ...
  1. **로그온 URL** 텍스트 상자에 고유한 도메인으로 로그인 URL을 입력합니다.
  2. **식별자** 텍스트 상자에 엔터티 ID URL을 입력합니다.
  3. **회신 URL** 텍스트 상자에 활성화된 어설션 소비자 인터페이스 URL을 입력합니다.
  4. **다음**을 클릭합니다.
...
8. 메타데이터 URL 링크를 클릭하여 메타데이터 문서를 다운로드합니다.
  ...
  * **EntityID**가&3;단계의 **식별자**와 일치하는지 확인합니다.
  * **AssertionConsumerService** 위치 URL이&3;단계의 **회신 URL**과 일치하는지 확인합니다.
...
12. **페더레이션 ID 공급자 사용자 설정 편집** 페이지에서 다음 작업을 수행하고 **저장**을 클릭합니다.   
  1. **로그인 이름**으로 **SAML NameID**를 선택합니다. 
  2. **전체 이름**으로 **기타**를 선택하고 **특성 이름** 텍스트 상자에 http://schemas.microsoft.com/identity/claims/displayname 값을 넣습니다. 
  3. **전자 메일**로 **기타**를 선택하고 **특성 이름** 텍스트 상자에 http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress 값을 넣습니다. 
  4. **SignOn에서 계정 자동 만들기**로 **예**를 선택합니다. 
  5. **저장** 단추를 클릭합니다.
...
### <a name="create-a-moveit-transfer-test-user"></a>MOVEit Transfer 테스트 사용자 만들기
...
이 섹션은 Britta Simon에게 MOVEit Transfer에 대한 액세스 권한을 부여하여 Azure SSO를 사용할 수 있도록 하기 위한 것입니다.
