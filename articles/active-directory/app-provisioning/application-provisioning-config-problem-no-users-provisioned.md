...
프로비저닝 로그는 프로비저닝 범위에 있는 할당된 사용자에 대해 Azure AD 쿼리, 해당 사용자의 존재에 대한 대상 앱 쿼리, 사용자 개체 간 비교 를 포함하여 프로비저닝 서비스에서 수행하는 모든 작업을 기록합니다. 시스템에 있습니다. 그런 다음 비교를 기반으로 대상 시스템에서 사용자 계정을 추가, 업데이트 또는 비활성화합니다.
