...
- **없음** - 대상 특성이 수정되지 않습니다. 그러나 대상 특성이 비어 있는 경우에는 사용자가 지정한 기본값으로 채워집니다.
...
- **일치 우선 순위** – 여러 일치 특성을 설정할 수 있습니다. 여러 개가 있는 경우이 필드에 정의 된 순서 대로 평가 됩니다. 일치 항목이 발견되는 즉시 더 이상 일치 특성을 평가하지 않습니다. 일치 하는 특성을 원하는 만큼 설정할 수 있지만, 일치 하는 특성으로 사용 하는 특성이 실제로 고유 하 고 특성을 일치 해야 하는지 여부를 고려해 야 합니다. 일반적으로 고객의 구성에는 1 또는 2 개의 일치 하는 특성이 있습니다. 
- **이 매핑 적용**
...
- **기본 키** -특성이 대상 개체의 스키마에서 기본 키 필드로 정의 되는지 여부입니다.
- **필수?** -대상 응용 프로그램 또는 시스템에서 특성을 채워야 하는지 여부입니다.
- **다중 값?** -특성에서 다중 값을 지원 하는지 여부입니다.
...
SCIM 응용 프로그램의 경우 특성 이름은 아래 예제에 표시 된 패턴을 따라야 합니다. "CustomExtensionName" 및 "CustomAttribute"는 응용 프로그램의 요구 사항에 따라 사용자 지정할 수 있습니다. 예를 들면 다음과 같습니다.  
 * urn: ietf: params: scim: 스키마: 확장: CustomExtensionName: 2.0: User: CustomAttribute 
 * urn: ietf: params: scim: 스키마: 확장: 2.0: CustomExtensionName: CustomAttribute  
 * urn: ietf: params: scim: 스키마: extension: CustomExtensionName: 2.0: CustomAttributeName: value
...
- Is소프트 Deleted 특성은 종종 응용 프로그램에 대 한 기본 매핑의 일부입니다. Issoft Deleted는 네 가지 시나리오 중 하나에 해당할 수 있습니다. 응용 프로그램에서 할당 되지 않아서 사용자가 범위를 벗어난 경우, 사용자가 범위 지정 필터를 충족 하지 않거나, 사용자가 Azure AD에서 일시 삭제 되었거나, 사용자에 대해 AccountEnabled 속성이 false로 설정 되어 있기 때문입니다. 특성 매핑에서 Is소프트 삭제 된 특성을 제거 하는 것은 권장 되지 않습니다.
- Azure AD 프로 비전 서비스는 null 값 프로 비전을 지원 하지 않습니다.
- 이러한 기본 키 (일반적으로 "ID")는 특성 매핑에서 대상 특성으로 포함 되어서는 안 됩니다. 
- 일반적으로 역할 특성은 직접 매핑 대신 식을 사용 하 여 매핑해야 합니다. 역할 매핑에 대 한 자세한 내용은 위의 섹션을 참조 하세요. 
