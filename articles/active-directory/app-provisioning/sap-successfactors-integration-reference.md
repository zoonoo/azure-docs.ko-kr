...
* SuccessFactors to 온-프레미스 Active Directory 사용자 프로 비전
* SuccessFactors 사용자 프로 비전을 Azure Active Directory
* SuccessFactors 쓰기 저장

이 문서에서는 통합이 작동 하는 방식과 여러 HR 시나리오에 대 한 프로 비전 동작을 사용자 지정 하는 방법을 설명 합니다. 

## <a name="establishing-connectivity"></a>연결 설정 
Azure AD 프로 비전 엔진은 기본 인증을 사용 하 여 Employee Central OData API 끝점에 연결 합니다. SuccessFactors 프로 비전 앱을 설정할 때 *관리자 자격 증명* 섹션에서 *테 넌 트 url* 매개 변수를 사용 하 여 [API 데이터 센터 URL](https://apps.support.sap.com/sap/support/knowledge/en/2215682)을 구성 합니다. 
...
* Azure 공용 클라우드의 [최신 IP 범위](https://www.microsoft.com/download/details.aspx?id=56519) 다운로드 
* 파일을 열고 **AzureActiveDirectory** 및 **AzureActiveDirectoryDomainServices** 태그를 검색 합니다. 

  >[!div class="mx-imgBorder"] 
  >![Azure AD IP 범위](media/sap-successfactors-integration-reference/azure-active-directory-ip-range.png)

* *AddressPrefixes* 요소 내에 나열 된 모든 ip 주소 범위를 복사 하 고 범위를 사용 하 여 ip 주소 제한 목록을 작성 합니다.
* CIDR 값을 IP 범위로 변환 합니다.  
* SuccessFactors 관리 포털에 로그인 하 여 허용 목록에 IP 범위를 추가 합니다. SAP [support note 2253200](https://apps.support.sap.com/sap/support/knowledge/en/2253200)을 참조 하세요. 이제이 도구에 [IP 범위를 입력할](https://answers.sap.com/questions/12882263/whitelisting-sap-cloud-platform-ip-address-range-i.html) 수 있습니다. 
...
| 10 | 기타 부서                           | employmentNav/jobInfoNav/departmentNav | 부서 또는 departmentId 특성만 매핑되는 경우에만 |
...
| 13 | 구분                             | employmentNav/jobInfoNav/divisionNav  | 나누기 또는 divisionId 특성이 매핑되는 경우에만 |
| 14 | 코드 코드                              | employmentNav/jobInfoNav/jobCodeNav  | JobCode 또는 jobCodeId 특성이 매핑된 경우에만 |
| 15 | FOPayGrade                             | employmentNav/jobInfoNav/payGradeNav  | PayGrade 특성만 매핑되는 경우에만 |
| 16 | 위치                             | employmentNav/jobInfoNav/locationNav  | Location 특성만 매핑되는 경우에만 |
| 17 | FOCorporateAddressDEFLT                | employmentNav/jobInfoNav/addressNavDEFLT  | 매핑에 다음 특성 중 하나가 포함 된 경우: officeLocationZipCode |
| 18 | 이벤트 이유                          | employmentNav/jobInfoNav/eventReasonNav  | EventReason 특성이 매핑된 경우에만 |
| 19 | EmpGlobalAssignment                    | employmentNav/empGlobalAssignmentNav | AssignmentType가 매핑된 경우에만 |
| 20 | EmploymentType 선택 목록                | employmentNav/jobInfoNav/employmentTypeNav | EmploymentType가 매핑된 경우에만 |
| 21 | EmployeeClass 선택 목록                 | employmentNav/jobInfoNav/employeeClassNav | EmployeeClass가 매핑된 경우에만 |
| 22 | EmplStatus 선택 목록                    | employmentNav/jobInfoNav/emplStatusNav | EmplStatus가 매핑된 경우에만 |
| 23 | AssignmentType 선택 목록                | employmentNav/empGlobalAssignmentNav/assignmentTypeNav | AssignmentType가 매핑된 경우에만 |

## <a name="how-full-sync-works"></a>전체 동기화 작동 방법
특성 매핑을 기반으로 전체 동기화 중에 Azure AD 프로 비전 서비스는 다음 "GET" OData API 쿼리를 전송 하 여 모든 활성 사용자의 유효 데이터를 가져옵니다. 
...
특정 *personIdExternal*에 대해 OData api 끝점에서 반환 된 데이터의 유효성을 검사 하려면 api 데이터 센터 서버 URL을 사용 하 여 아래 Api 쿼리의 *SuccessFactorsAPIEndpoint* 를 업데이트 하 고 [postman](https://www.postman.com/downloads/) 과 같은 도구를 사용 하 여 쿼리를 호출 합니다. 
...
전체 동기화 후에 Azure AD 프로 비전 서비스는 *Lastexecutiontimestamp* 를 유지 관리 하 고이를 사용 하 여 증분 변경 내용을 검색 하는 델타 쿼리를 만듭니다. *LastModifiedDateTime*, SuccessFactors, *endDate*및 *latestTerminationDate*와 같은 각 엔터티에 있는 타임 스탬프 특성은 변경 내용이 *lastexecutiontimestamp* 와 *CurrentExecutionTime*사이에 있는지 *여부를 확인*하기 위해 평가 됩니다. 예 이면 항목 변경 내용이 적용 되는 것으로 간주 되 고 동기화에 대해 처리 됩니다. 
...
* **엔터프라이즈 응용 프로그램**  ->  **SuccessFactors 앱**  ->  **프로비저닝**  ->  **프로 비전**  ->  **특성 매핑 편집 페이지**로 이동 합니다.
* 아래로 스크롤하고 **고급 옵션 표시**를 클릭 합니다.
* **SuccessFactors에 대 한 특성 목록 편집**을 클릭 합니다. 
...
* 이 보기의 **API 식** 열에는 커넥터에서 사용 하는 JSONPath 식이 표시 됩니다.
  >[!div class="mx-imgBorder"] 
  >![API 식](media/sap-successfactors-integration-reference/jsonpath-api-expressions.png#lightbox)  
* 기존 JSONPath 값을 편집 하거나 유효한 JSONPath 식으로 새 특성을 스키마에 추가할 수 있습니다. 
...
기본 Azure AD SuccessFactors 프로 비전 앱 스키마는 [90 개 이상의 미리 정의 된 특성과](sap-successfactors-attribute-reference.md)함께 제공 됩니다. 프로 비전 스키마에 더 많은 기본 SuccessFactors 특성을 추가 하려면 아래 나열 된 단계를 사용 합니다. 

* 아래 OData 쿼리를 사용 하 여 Employee Central에서 유효한 테스트 사용자에 대 한 데이터를 검색 합니다. 
...
* 특성과 연결 된 Employee Central 엔터티를 확인 합니다.
...
* 특성과 연결 된 JSON 경로를 생성 하 고이 새 특성을 SuccessFactors 특성 목록에 추가 합니다. 
...
* SuccessFactors 특성 목록을 편집 하 여 *empJobNavCustomString35*라는 새 특성을 추가 합니다.
* 이 특성에 대 한 JSONPath API 식을 다음과 같이 설정 합니다.`$.employmentNav.results[0].jobInfoNav.results[0].customString35`
* Azure Portal에서 매핑 변경을 저장 하 고 다시 로드 합니다.  
* 특성 매핑 블레이드에서 *empJobNavCustomString35* 을 *physicalDeliveryOfficeName*에 매핑합니다.
* 매핑을 저장 합니다.
...
작업자 변환은 기존 정규 직원을 계약자로 변환 하거나 그 반대로 변환 하는 프로세스입니다. 이 시나리오에서 Employee Central은 동일한 *Person* 엔터티에 대해 새 *사용자* 엔터티와 함께 새 *EmpEmployment* 엔터티를 추가 합니다. 이전 *EmpEmployment* 엔터티 아래에 중첩 된 *사용자* 엔터티는 null로 설정 됩니다. 변환이 발생할 때 새 고용 데이터가 표시 되도록이 시나리오를 처리 하려면 아래 나열 된 단계를 사용 하 여 프로 비전 앱 스키마를 대량 업데이트할 수 있습니다.  

* SuccessFactors 프로비저닝 앱의 특성 매핑 블레이드를 엽니다. 
* 아래로 스크롤하고 **고급 옵션 표시**를 클릭 합니다.
* **여기에서 스키마 검토** 링크를 클릭 하 여 스키마 편집기를 엽니다. 
  >![검토-스키마](media/sap-successfactors-integration-reference/review-schema.png#lightbox)
* 편집 하기 전에 **다운로드** 링크를 클릭 하 여 스키마 복사본을 저장 합니다. 
  >![다운로드-스키마](media/sap-successfactors-integration-reference/download-schema.png#lightbox)
* 스키마 편집기에서 Ctrl + H 키를 눌러 찾기-바꾸기 컨트롤을 엽니다.
* 찾기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다.`$.employmentNav.results[0]`
* 바꾸기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다 `$.employmentNav.results[?(@.userNav != null)]` . 연산자 주위의 공백은 `!=` JSONPath 식을 성공적으로 처리 하는 데 중요 합니다. 
  >![찾기-바꾸기-변환](media/sap-successfactors-integration-reference/find-replace-conversion-scenario.png#lightbox)
* "모두 바꾸기" 옵션을 클릭 하 여 스키마를 업데이트 합니다. 
* 스키마를 저장합니다. 
* 위의 프로세스는 다음과 같이 모든 JSONPath 식을 업데이트 합니다. 
...
HR 프로세스에서 옵션 1을 사용 하는 경우 프로 비전 스키마를 변경할 필요가 없습니다. HR 프로세스에서 옵션 2를 사용 하는 경우 Employee Central은 동일한 *Person* 엔터티에 대해 새 *사용자* 엔터티와 함께 새 *EmpEmployment* 엔터티를 추가 합니다. 변환 시나리오와 달리 이전 *EmpEmployment* 엔터티는 *사용자* 엔터티를 유지 하며 null로 설정 되지 않습니다. 
...
* 스키마 편집기에서 Ctrl + H 키를 눌러 찾기-바꾸기 컨트롤을 엽니다.
* 찾기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다.`$.employmentNav.results[0]`
* 바꾸기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다 `$.employmentNav.results[-1:]` . 이 JSONPath 식은 최신 *EmpEmployment* 레코드를 반환 합니다.   
* "모두 바꾸기" 옵션을 클릭 하 여 스키마를 업데이트 합니다. 
* 스키마를 저장합니다. 
* 위의 프로세스는 다음과 같이 모든 JSONPath 식을 업데이트 합니다. 
  * 이전 JSONPath:`$.employmentNav.results[0].jobInfoNav.results[0].departmentNav.name_localized`
...
* 스키마 편집기에서 Ctrl + H 키를 눌러 찾기-바꾸기 컨트롤을 엽니다.
* 찾기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다.`$.employmentNav.results[0]`
* 바꾸기 텍스트 상자에서 값을 복사 하 여 붙여넣습니다 `$.employmentNav.results[?(@.assignmentClass == 'ST')]` . 
* "모두 바꾸기" 옵션을 클릭 하 여 스키마를 업데이트 합니다. 
* 스키마를 저장합니다. 
* 위의 프로세스는 다음과 같이 모든 JSONPath 식을 업데이트 합니다. 
  * 이전 JSONPath:`$.employmentNav.results[0].jobInfoNav.results[0].departmentNav.name_localized`
  * 새 JSONPath:`$.employmentNav.results[?(@.assignmentClass == 'ST')].jobInfoNav.results[0].departmentNav.name_localized`
* 앱의 특성 매핑 블레이드를 다시 로드 합니다. 
* 아래로 스크롤하고 **고급 옵션 표시**를 클릭 합니다.
* **SuccessFactors에 대 한 특성 목록 편집**을 클릭 합니다.
* 새 특성을 추가 하 여 전역 할당 데이터를 인출 합니다. 예를 들어 전역 할당 프로필에 연결 된 부서 이름을 페치 하려는 경우 JSONPath 식이로 설정 된 **globalAssignmentDepartment** 특성을 추가할 수 있습니다 `$.employmentNav.results[?(@.assignmentClass == 'GA')].jobInfoNav.results[0].departmentNav.name_localized` . 
* 이제 두 부서 값을 Active Directory 특성으로 이동 하거나 식 매핑을 사용 하 여 선택적으로 값을 전달할 수 있습니다. 예: 아래 식은 AD *부서* 특성의 값을 *globalAssignmentDepartment* 로 설정 하 고, 그렇지 않은 경우 값을 표준 할당과 연결 된 *학과* 로 설정 합니다. 
...
* 아래로 스크롤하고 **고급 옵션 표시**를 클릭 합니다.
* **SuccessFactors에 대 한 특성 목록 편집**을 클릭 합니다.
* 작업 1 및 작업 2와 연결 된 부서를 풀 려 한다고 가정해 보겠습니다. 미리 정의 된 특성 *부서* 는 첫 번째 작업에 대 한 학과의 가치를 이미 페치합니다. *SecondJobDepartment* 라는 새 특성을 정의 하 고 JSONPath 식을로 설정할 수 있습니다.`$.employmentNav.results[1].jobInfoNav.results[0].departmentNav.name_localized`
* 이제 두 부서 값을 Active Directory 특성으로 이동 하거나 식 매핑을 사용 하 여 선택적으로 값을 전달할 수 있습니다. 
* 매핑을 저장 합니다. 
* 프로 비전을 다시 시작 합니다. 
