...
| Azure Active Directory 그룹 | urn: ietf: params: scim: 스키마: core: 2.0: 그룹 |
| --- | --- |
| displayName |displayName |
| mail |emails[type eq "work"].value |
...
* [SCIM 프로토콜의 섹션 3.4.2](https://tools.ietf.org/html/rfc7644#section-3.4.2)사용자 또는 그룹 쿼리를 지원 합니다.  기본적으로 사용자 `id` 는을 통해 검색 되 고 `username` 및 `externalid`에 의해 쿼리 되며 그룹은에서 `displayName`쿼리 됩니다.  
...
* `id`는 모든 리소스에 대 한 필수 속성입니다. 리소스를 반환 하는 모든 응답은 멤버가 0 인 경우 `ListResponse` 를 제외 하 고 각 리소스에이 속성이 있는지 확인 해야 합니다.
* 쿼리/필터 요청에 대 한 응답은 항상 이어야 `ListResponse`합니다.
* 그룹은 선택 사항 이지만 SCIM 구현이 패치 요청을 지 원하는 경우에만 지원 됩니다.
* 패치 응답에 전체 리소스를 포함 하는 것은 필요 하지 않습니다.
* Microsoft Azure AD는 다음 연산자만 사용 합니다.  
    - `eq`
    - `and`
* 에 `op` https://tools.ietf.org/html/rfc7644#section-3.5.2정의 된 대로 scim의 구조 요소에 대 한 대/소문자를 구분 하는 일치를 요구 하지 않습니다. Azure AD는 ' op ' 값을, `Add` `Replace`및 `Remove`로 내보냅니다.
...
![사용자 프로 비전 및 프로 비전 해제 시퀀스를 표시 합니다.](media/use-scim-to-provision-users-and-groups/scim-figure-4.png)<br/>
...
![그룹 프로 비전 및 프로 비전 해제 시퀀스를 표시 합니다.](media/use-scim-to-provision-users-and-groups/scim-figure-5.png)<br/>
...
* 또는 `userName` `email[type eq "work"]` 특성을 통해 사용자를 쿼리할 수 있습니다.  
...
###### <a name="response-user-found"></a><a name="response-1"></a>응답 (사용자 찾음)
*HTTP/1.1 200 확인*
```json
{
    "schemas": ["urn:ietf:params:scim:schemas:core:2.0:User"],
...
이제 스키마를 설계 하 고 Azure AD SCIM 구현을 이해 했으므로 SCIM 끝점 개발을 시작할 수 있습니다. 처음부터 시작 하 여 구현을 완전히 빌드하는 대신 SCIM 커뮤니티에서 게시 한 여러 오픈 소스 SCIM 라이브러리를 사용할 수 있습니다.

Azure AD 프로 비전 팀에서 게시 한 오픈 소스 .NET Core [참조 코드](https://aka.ms/SCIMReferenceCode) 는 개발을 시작할 수 있는 리소스 중 하나입니다. SCIM 끝점을 빌드한 후에는 테스트 해 볼 수 있습니다. 참조 코드의 일부로 제공 되는 [postman 테스트](https://github.com/AzureAD/SCIMReferenceCode/wiki/Test-Your-SCIM-Endpoint) 컬렉션을 사용 하거나 [위에서](https://docs.microsoft.com/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups#user-operations)제공 된 샘플 요청/응답을 통해 실행할 수 있습니다.  

   > [!Note]
   > 이 참조 코드는 SCIM 끝점 빌드를 시작 하는 데 도움을 주기 위한 것 이며 "있는 그대로" 제공 됩니다. 커뮤니티의 기여는 코드를 작성 하 고 유지 관리 하는 데 도움이 됩니다.

이 솔루션은 두 개의 프로젝트 인 _microsoft. scim_ _. webhostsample_로 구성 됩니다.

_MICROSOFT scim_ 프로젝트는 scim 사양을 준수 하는 웹 서비스의 구성 요소를 정의 하는 라이브러리입니다. 이 클래스는 _Microsoft. SCIM. IProvider_를 선언 합니다. 요청은 공급자의 메서드에 대 한 호출로 변환 되며, id 저장소에서 작동 하도록 프로그래밍 됩니다.

![분석: 공급자의 메서드에 대 한 호출로 변환 된 요청](media/use-scim-to-provision-users-and-groups/scim-figure-3.png)

Microsoft. n a m a. n a m a. n o w s _Hostsample_ 프로젝트는 _빈_ 템플릿을 기반으로 하는 Visual Studio ASP.NET Core 웹 응용 프로그램입니다. 이를 통해 샘플 코드를 독립 실행형으로 배포 하거나 컨테이너에 호스트 하거나 인터넷 정보 서비스 내에 배포할 수 있습니다. 또한 메모리의 클래스를 샘플 id 저장소로 유지 하는 _Microsoft_ 의 클래스를 구현 합니다.
...
이 .NET Core SDK에는 개발 중에 사용할 수 있는 HTTPS 개발 인증서가 포함 되어 있습니다 .이 인증서는 첫 실행 환경의 일부로 설치 됩니다. ASP.NET Core 웹 응용 프로그램을 실행 하는 방법에 따라 다른 포트를 수신 대기 합니다.

* Microsoft. SCIM. WebHostSample:https://localhost:5001
* IIS Express:https://localhost:44359/

의 HTTPS에 대 한 자세한 내용은 다음 링크를 사용 하세요. [ASP.NET Core에서 Https 적용](https://docs.microsoft.com/aspnet/core/security/enforcing-ssl) 을 ASP.NET Core

### <a name="handling-endpoint-authentication"></a>엔드포인트 인증 처리

Azure Active Directory에서 요청은 OAuth 2.0 전달자 토큰을 포함합니다. 요청을 받는 모든 서비스는 예상 Azure Active Directory 테 넌 트에 대해 Azure Active Directory 되는 발급자를 인증 해야 합니다.
...
전달자 토큰은 제공 된 [postman 테스트](https://github.com/AzureAD/SCIMReferenceCode/wiki/Test-Your-SCIM-Endpoint) 를 사용 하 고 localhost를 사용 하 여 로컬 디버깅을 수행 하는 데에도 필요 합니다. 이 샘플 코드에서는 ASP.NET Core 환경을 사용 하 여 개발 단계 중에 인증 옵션을 변경 하 고 자체 서명 된 토큰 사용을 사용 하도록 설정 합니다.

ASP.NET Core에서 여러 환경에 대 한 자세한 내용은 다음 링크를 사용 하세요. [의 여러 환경 사용 ASP.NET Core](
https://docs.microsoft.com/aspnet/core/fundamentals/environments)

다음 코드는 사용자 지정 키로 서명 된 전달자 토큰을 사용 하 여 서비스의 끝점에 대 한 요청이 인증 됨을 적용 합니다.
...
토큰 컨트롤러에 GET 요청을 보내 유효한 전달자 토큰을 가져옵니다. 메서드 _GenerateJSONWebToken_ 는 개발용으로 구성 된 매개 변수와 일치 하는 토큰을 만듭니다.
...
샘플 코드에서 요청은 서비스 공급자의 QueryAsync 메서드에 대 한 호출로 변환 됩니다. 해당 메서드의 서명은 다음과 같습니다. 
...
예제 쿼리에서 externalId 특성에 대해 지정 된 값을 가진 사용자에 대해 QueryAsync 메서드에 전달 된 인수의 값은 다음과 같습니다.
...
사용자의 mailNickname 특성 값과 일치 하는 externalId 특성 값이 있는 사용자에 대 한 웹 서비스에 대 한 쿼리에 대 한 응답이 사용자를 반환 하지 않는 경우 Azure Active Directory는 Azure Active Directory에서 해당 서비스에 해당 하는 사용자를 프로 비전 하도록 요청 합니다.  다음은 그러한 요청의 예제입니다. 
...
샘플 코드에서 요청은 서비스 공급자의 CreateAsync 메서드에 대 한 호출로 변환 됩니다. 해당 메서드의 서명은 다음과 같습니다. 
...
사용자를 프로 비전 하는 요청에서 리소스 인수의 값은 가진 core2enterpriseuser 라이브러리에 정의 된 Microsoft. n a m e. n a m e.  사용자를 프로 비전 하는 요청이 성공 하면 메서드를 구현할 때 가진 core2enterpriseuser 클래스의 인스턴스가 반환 됩니다 .이 클래스는 식별자 속성의 값을 새로 프로 비전 된 사용자의 고유 식별자로 설정 합니다.  
...
샘플 코드에서 요청은 서비스 공급자의 RetrieveAsync 메서드에 대 한 호출로 변환 됩니다. 해당 메서드의 서명은 다음과 같습니다. 
...
참조 특성을 업데이트 하는 경우 Azure Active Directory 서비스를 쿼리하여 서비스에 의해 제어 되는 id 저장소에 있는 참조 특성의 현재 값이 Azure Active Directory의 해당 특성 값과 이미 일치 하는지 여부를 확인 합니다. 사용자의 경우 현재 값이 이 방식으로 쿼리된 유일한 특성은 관리자 특성입니다. 사용자 개체의 관리자 특성에 현재 특정 값이 있는지 여부를 확인 하는 요청의 예는 다음과 같습니다. 샘플 코드에서 요청은 서비스 공급자의 QueryAsync 메서드에 대 한 호출로 변환 됩니다. 매개 변수 인수의 값으로 제공되는 개체의 속성 값은 다음과 같습니다. 
...
샘플 코드에서 요청은 서비스 공급자의 UpdateAsync 메서드에 대 한 호출로 변환 됩니다. 해당 메서드의 서명은 다음과 같습니다. 
...
샘플 코드에서 요청은 서비스 공급자의 DeleteAsync 메서드에 대 한 호출로 변환 됩니다. 해당 메서드의 서명은 다음과 같습니다. 
...
4. 응용 프로그램의 이름을 입력 하 고 **추가** 를 선택 하 여 앱 개체를 만듭니다. 새 앱이 엔터프라이즈 응용 프로그램 목록에 추가 되 고 앱 관리 화면으로 열립니다.
   ...
   ![Azure AD 응용 프로그램 갤러리를 보여 주는 스크린샷](media/use-scim-to-provision-users-and-groups/scim-figure-2a.png)<br/>
...
* **소셜 미디어.** Twitter, Facebook 또는 LinkedIn 같은 소셜 미디어를 활용 하 여 고객에 대 한 통합을 홍보 하세요. 게시물을 제거할 수 @AzureAD 있도록를 포함 해야 합니다. [예: Imprivata Twitter Post](https://twitter.com/azuread/status/1123964502909779968)
