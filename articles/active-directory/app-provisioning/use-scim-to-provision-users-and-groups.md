...
예제를 포함 하 여 SCIM 끝점을 빌드하는 방법에 대 한 지침은 [샘플 SCIM 끝점 개발](use-scim-to-build-users-and-groups-endpoints.md)을 참조 하세요.

Azure AD 프로 비전 팀에서 게시 한 오픈 소스 .NET Core [참조 코드 예제](https://aka.ms/SCIMReferenceCode) 는 개발을 시작할 수 있는 리소스 중 하나입니다. SCIM 엔드포인트를 빌드하면 테스트를 수행할 수 있습니다. 참조 코드의 일부로 제공된 [postman 테스트](https://github.com/AzureAD/SCIMReferenceCode/wiki/Test-Your-SCIM-Endpoint) 컬렉션을 사용하거나 [위의](#user-operations)에서 제공된 샘플 요청/응답을 실행할 수 있습니다.  
...
3. **+ 새 응용 프로그램**  >  **+ 사용자 고유의 응용 프로그램 만들기** 를 선택 합니다.
4. 응용 프로그램의 이름을 입력 하 고 "*갤러리에서 찾을 수 없는 다른 응용 프로그램 통합*" 옵션을 선택한 다음 **추가** 를 선택 하 여 앱 개체를 만듭니다. 새 앱이 엔터프라이즈 애플리케이션 목록에 추가되고 해당 앱 관리 화면이 열립니다.

   ![Azure AD 응용 프로그램 갤러리 ](media/use-scim-to-provision-users-and-groups/scim-figure-2b-1.png)
    *azure ad 응용 프로그램 갤러리* 를 보여 주는 스크린샷

   > [!NOTE]
   > 이전 앱 갤러리 환경을 사용 하는 경우 아래 화면 가이드를 따르세요.
   
   ![Azure AD 이전 앱 갤러리 환경 ](media/use-scim-to-provision-users-and-groups/scim-figure-2a.png)
    *azure ad 이전 앱 갤러리* 환경을 보여 주는 스크린샷
