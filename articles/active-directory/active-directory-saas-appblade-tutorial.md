...
이 자습서에서는 AppBlade와 Azure AD(Azure Active Directory)를 통합하는 방법을 보여 줍니다. AppBlade를 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.
...
7. 결과 창에서 **AppBlade**를 선택하고 **완료**를 클릭하여 응용 프로그램을 추가합니다.
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 AppBlade 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 AppBlade의 관련 사용자 간에 연결이 형성되어야 합니다. 이 연결 관계는 Azure AD의 **사용자 이름** 값을 AppBlade의 **Username** 값으로 할당하여 설정합니다.
...
4. **AppBlade에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행합니다.
...
> [AZURE.NOTE] 수동으로 사용자를 만들어야 하는 경우 AppBlade 지원 팀에 문의해야 합니다.
...
이 섹션은 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트하기 위한 것입니다. 액세스 패널에서 AppBlade 타일을 클릭하면 AppBlade 응용 프로그램에 자동으로 로그인됩니다.
