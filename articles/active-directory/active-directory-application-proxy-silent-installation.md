<properties
	pageTitle="Azure AD 응용 프로그램 프록시 커넥터를 자동으로 설치하는 방법"
...
사용자 인터페이스를 사용하도록 설정되지 않은 Windows Server 또는 여러 서버에 설치 스크립트를 보낼 수 있습니다. 이 항목에서는 무인 설치를 사용하도록 설정하고 Azure AD 응용 프로그램 프록시 커넥터를 설치 및 등록하는 Windows PowerShell 스크립트를 만드는 방법에 대해 설명합니다.

## 액세스 사용
응용 프로그램 프록시는 네트워크 내부에서 커넥터라고 불리는 간단한 Windows Server 서비스를 설치하여 사용합니다. 응용 프로그램 프록시 커넥터가 작동하려면 전역 관리자 및 암호를 사용하여 Azure AD 디렉터리에 등록되어야 합니다. 일반적으로 이러한 등록 정보는 커넥터 설치 중에 팝업 대화 상자에서 입력됩니다. 또는 Windows PowerShell을 사용하여 자격 증명 개체를 만들고 자격 증명 개체가 만드는 토큰을 사용하여 등록 정보를 입력하거나, 고유한 토큰을 만들고 이 토큰을 사용하여 등록 정보를 입력할 수 있습니다.
...
다음 방법 중 하나를 사용하여 이 작업을 수행할 수 있습니다. - Windows PowerShell 자격 증명 개체를 사용하여 커넥터 등록 - 오프라인으로 만든 토큰을 사용하여 커넥터 등록
...
2. 	**C:\\Program Files\\Microsoft AAD App Proxy Connector**로 이동하고 Windows PowerShell에서 **Register Connector.PS1** 파일을 실행합니다.
3. 다음을 실행하여, 만든 PowerShell 자격 증명 개체를 사용해 커넥터 등록 사용자 이름 및 암호를 스크립트에 입력합니다. 여기서 $cred는 만든 PowerShell 자격 증명 개체의 이름입니다.
...
1. AuthenticationContext 클래스를 사용하여 오프라인 토큰을 만듭니다. 예를 들면 다음과 같습니다.
...
3. 다음 Windows PowerShell 명령을 실행합니다. 여기서 SecureToken은 위에서 만든 토큰의 이름입니다. <br> `RegisterConnector.ps1 -modulePath "C:\Program Files\Microsoft AAD App Proxy Connector\Modules" -moduleName "AppProxyPSModule" -Authenticationmode Token -Token $SecureToken -TenantId <tenant GUID>`
