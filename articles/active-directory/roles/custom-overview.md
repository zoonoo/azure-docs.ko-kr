...
ms.locfileid: "93378502"
---
# <a name="custom-administrator-roles-in-azure-active-directory-preview"></a>Azure Active Directory의 사용자 지정 관리자 역할

이 문서에서는 역할 기반 액세스 제어 및 리소스 범위를 사용하는 Azure AD(Azure Active Directory)의 Azure AD 사용자 지정 역할에 대해 알아봅니다. 사용자 지정 Azure AD 역할은 사용자 고유의 사용자 지정 역할을 만들고 구성할 수 있도록 [기본 제공 역할](permissions-reference.md)의 기본 권한을 노출합니다. 이 접근 방식을 사용하면 필요할 때 기본 제공 역할보다 세분화된 방식으로 액세스 권한을 부여할 수 있습니다. Azure AD 사용자 지정 역할의 첫 번째 릴리스에는 앱 등록을 관리하는 권한 할당을 위한 역할을 만들 수 있는 기능이 포함되어 있습니다. 엔터프라이즈 애플리케이션, 사용자, 디바이스 등의 조직 리소스에 대한 권한이 앞으로 더 추가될 예정입니다.  
...
Azure AD 기본 제공 및 사용자 지정 역할은 [Azure RBAC(Azure 역할 기반 액세스 제어)](../../active-directory-b2c/overview.md)와 유사한 방식으로 작동합니다. [두 역할 기반 액세스 제어 시스템 간의 차이점](../../role-based-access-control/rbac-and-directory-admin-roles.md)은 Azure RBAC는 Azure Resource Management를 사용하는 가상 머신 또는 스토리지와 같은 Azure 리소스에 대한 액세스를 제어하고, Azure AD 사용자 지정 역할은 Graph API를 사용하여 Azure AD 리소스에 대한 액세스를 제어한다는 것입니다. 두 시스템 모두 역할 정의 및 역할 할당이라는 개념을 활용합니다. Azure AD RBAC 권한은 Azure RBAC 역할에 포함될 수 없으며 그 반대의 경우도 마찬가지입니다.
