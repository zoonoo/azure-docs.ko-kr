...
- 사용자 프로필의 개별 할당
   ...
   1. **관리 단위**를 선택 합니다. **관리 단위에 할당** 을 선택 하 고 사용자를 할당할 관리 단위를 선택 하 여 하나 이상의 관리 단위에 사용자를 할당할 수 있습니다.
       ...
       ![추가를 선택 하 고 관리 단위의 이름을 입력 합니다.](./media/admin-units-add-manage-users/assign-users-individually.png)
...
- 관리 단위의 개별 할당
   ...
   1. **모든 사용자** 를 선택한 후 **구성원 추가** 를 선택 하 여 **구성원 추가** 창에서 관리 단위에 할당할 사용자를 하나 이상 선택 합니다.
        ...
        ![관리 단위를 선택한 다음, 구성원 추가](./media/admin-units-add-manage-users/assign-to-admin-unit.png)
...
- 대량 할당
   ...
   1. **모든 사용자**열기  >  **.csv 파일에서 멤버를 추가**합니다. 그런 다음 CSV (쉼표로 구분 된 값) 템플릿을 다운로드 하 고 파일을 편집할 수 있습니다. 형식은 단순 하 고 각 줄에 단일 사용자 계정 이름을 추가 해야 합니다. 파일이 준비 되 면 적절 한 위치에 저장 한 후이 단계의 일부로 업로드 합니다.
    ...
    ![관리 단위에 사용자 대량 할당](./media/admin-units-add-manage-users/bulk-assign-to-admin-unit.png)
...
1. **관리 단위** 를 선택 하 여 사용자가 할당 된 관리 단위 목록을 표시 합니다.
   ...
   ![사용자의 관리 단위 나열](./media/admin-units-add-manage-users/list-user-admin-units.png)
...
![사용자 프로필에서 관리 단위에서 사용자 제거](./media/admin-units-add-manage-users/user-remove-admin-units.png)

사용자를 제거 하려는 관리 단위를 선택 하 여 **Azure AD**  >  **관리 단위** 에서 사용자를 제거할 수도 있습니다. 사용자를 선택 하 고 **구성원 제거**를 선택 합니다.
  
![관리 단위 수준에서 사용자 제거](./media/admin-units-add-manage-users/admin-units-remove-user.png)
...
![관리 단위에서 대량 사용자 제거를 위해 CSV 파일 편집](./media/admin-units-add-manage-users/bulk-user-entries.png)

파일에 항목을 저장 한 후 파일을 업로드 하 고 **제출**을 선택 합니다.

![대량 업로드 파일 제출](./media/admin-units-add-manage-users/bulk-user-remove.png)
