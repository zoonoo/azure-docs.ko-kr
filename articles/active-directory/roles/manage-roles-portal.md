...
1. 각 행의 오른쪽에 있는 줄임표 (...)를 선택 하 여 역할에 대 한 사용 권한을 확인 합니다. 역할에 할당 된 사용자를 보려면 역할을 선택 합니다. 다음 그림과 다른 내용이 표시 되는 경우 [권한 있는 역할에 대 한 할당 보기](#view-assignments-for-privileged-roles) 의 메모를 읽고 PRIVILEGED IDENTITY MANAGEMENT (PIM)에 있는지 여부를 확인 합니다.
    ...
    ![Azure AD 포털의 역할 목록](./media/manage-roles-portal/view-roles-in-azure-active-directory.png)
...
![관리자 역할의 멤버 목록](./media/manage-roles-portal/member-list.png)
...
!["전역 관리자-설명" 페이지를 보여 주는 스크린샷](./media/manage-roles-portal/role-description.png)
...
![역할에 대 한 모든 할당 다운로드](./media/manage-roles-portal/download-role-assignments.png)
...
1. 역할을 선택 하 여 해당 할당을 확인 합니다.
    ...
    !["PIM에서 관리" 작업이 선택 된 "사용자 관리자-할당" 페이지를 보여 주는 스크린샷](./media/manage-roles-portal/member-list.png)
...
1. 할당 **추가** 를 선택 하 고 할당 하려는 역할을 선택 합니다. 추가 관리 기능을 위해 **PIM에서 관리**를 선택할 수 있습니다. 다음 그림과 다른 내용이 표시 되는 경우 [권한 있는 역할에 대 한 할당 보기](#view-assignments-for-privileged-roles) 에서 참고 사항을 읽어 PIM에 있는지 확인 합니다.
    ...
    ![관리 역할에 대한 권한 목록](./media/manage-roles-portal/directory-role-select-role.png)
