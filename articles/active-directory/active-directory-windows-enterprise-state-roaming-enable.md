...
엔터프라이즈 상태 로밍을 사용하도록 설정하면 조직에 Azure Rights Management에 대한 제한된 무료 구독 라이선스가 자동으로 부여됩니다. 이 무료 구독은 엔터프라이즈 설정 데이터의 암호화 및 암호 해독과 엔터프라이즈 상태 로밍 서비스와 동기화된 응용 프로그램 데이터로 제한됩니다. Azure Rights Management의 모든 기능을 사용하려면 유료 구독이 필요합니다.

프리미엄 Azure AD 구독을 얻은 후 다음 단계에 따라 엔터프라이즈 상태 로밍을 사용하도록 설정합니다.
...
## 엔터프라이즈 상태 로밍 관리
Azure AD 전역 관리자는 Azure 클래식 포털에서 엔터프라이즈 상태 로밍을 활성화 및 비활성화할 수 있습니다. ![](./media/active-directory-enterprise-state-roaming/active-directory-enterprise-state-roaming-manage.png)

전역 관리자는 특정 보안 그룹으로 설정 동기화를 제한할 수 있습니다.

또한 전역 관리자는 Active Directory 인스턴스 **사용자** 목록에서 특정 사용자를 선택하고 **장치** 탭을 클릭한 다음 **설정 및 엔터프라이즈 앱 데이터를 동기화하는 장치** 보기를 선택하여 사용자별 장치 동기화 상태 보고서를 볼 수도 있습니다. ![](./media/active-directory-enterprise-state-roaming/active-directory-enterprise-state-roaming-device-sync-settings.png)
