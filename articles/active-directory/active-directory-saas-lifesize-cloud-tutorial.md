...
3. **Lifesize Cloud 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    b. **식별자** 텍스트 상자에서 `https://login.lifesizecloud.com/<companyname>` 패턴을 사용하여 URL을 입력합니다.
...
6. **Lifesize Cloud 구성** 섹션에서 **Lifesize Cloud 구성**을 클릭하여 **로그온 구성** 창을 엽니다. **빠른 참조 섹션**에서 **SAML 엔터티 ID 및 SAML Single Sign-On 서비스 URL**을 복사합니다.
...
10. 이제 SSO 구성 UI에서 다음 값을 구성합니다.    
    ...
    c. Azure Portal에서 다운로드한 base-64로 인코딩된 인증서를 메모장에서 열고, 콘텐츠를 클립보드에 복사한 다음, **X.509 인증서** 텍스트 상자에 붙여넣습니다.
  
    d. 이름에 대한 SAML 특성 매핑 텍스트 상자에 값을 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname**으로 입력합니다.
    
    e. **성**에 대한 SAML 특성 매핑 텍스트 상자에 값을 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname**으로 입력합니다.
    
    f. **전자 메일**에 대한 SAML 특성 매핑 텍스트 상자에 값을 **http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress**로 입력합니다.
