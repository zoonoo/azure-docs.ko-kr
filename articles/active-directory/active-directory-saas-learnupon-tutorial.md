...
이 자습서에서는 LearnUpon과 Azure AD(Azure Active Directory)를 통합하는 방법을 보여 줍니다. LearnUpon을 Azure AD와 통합하면 다음과 같은 이점이 제공됩니다.
...
Single Sign-On이 작동하려면 Azure AD에서 Azure AD 사용자에 해당하는 LearnUpon 사용자가 누군지 알고 있어야 합니다. 즉, Azure AD 사용자와 LearnUpon의 관련 사용자 간에 연결이 형성되어야 합니다. 이 연결 관계는 Azure AD의 **사용자 이름** 값을 LearnUpon의 **Username** 값으로 할당하여 설정합니다.
...
이 섹션은 Azure 클래식 포털에서 Azure AD Single Sign-On을 사용하도록 설정하고 LearnUpon 응용 프로그램에서 Single Sign-On을 구성하는 방법을 설명하기 위한 것입니다.
...
5. **일반 설정** 섹션에서 다음 단계를 수행합니다.
	...
	f. **공급자 위치 식별** 텍스트 상자에서 Azure 클래식 포털 로그인 화면에서 업로드된 아이콘을 클릭할 경우 사용자가 전송되는 위치를 나타내는 값을 입력합니다.

	g.Azure 클래식 포털에서 **Single Sign-Out 서비스 URL**을 복사한 다음 **로그아웃 URL** 텍스트 상자에 붙여 넣습니다.
...
이 섹션의 목적은 Britta Simon에게 LearnUpon에 대한 액세스 권한을 부여하여 Azure Single Sign-On을 사용할 수 있도록 하는 것입니다.
...
이 섹션은 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트하기 위한 것입니다. 액세스 패널에서 LearnUpon 타일을 클릭하면 LearnUpon 응용 프로그램에 자동으로 로그온됩니다.
