<properties
	pageTitle="자습서: Wizergos Productivity Software와 Azure Active Directory 통합 | Microsoft Azure"
...
# 자습서: Wizergos Productivity Software와 Azure Active Directory 통합 
...
## 갤러리에서 Wizergos Productivity Software 추가
...
이 섹션은 "Britta Simon"이라는 테스트 사용자를 기반으로 Wizergos Productivity Software에서 Azure AD Single Sign-on을 구성하고 테스트하는 방법을 보여 주기 위해 작성되었습니다.
...
3. **앱 설정 구성** 대화 상자 페이지에서 **IDP 시작 모드**로 응용 프로그램을 구성하려는 경우 다음 단계를 수행하고 **다음**을 클릭합니다.
	...
	a. **식별자** 텍스트 상자에 `https://www.wizergos.net`를 입력합니다.

    b. **회신 URL** 텍스트 상자에서 `https://www.wizergos.net/register_users/saml2/`을(를) 입력합니다.
...
4. **앱 설정 구성** 대화 상자 페이지에서 **SP 시작 모드**로 응용 프로그램을 구성하려는 경우 **"고급 설정 표시(선택 사항)"**를 클릭하고 **로그온 URL**을 입력한 후 **다음**을 클릭합니다.
	...
	a. **로그온 URL** 텍스트 상자에 `https://www.wizergos.net`를 입력합니다.

	b. **다음**를 클릭합니다.

5. **Wizergos Productivity Software에서 Single Sign-On 구성** 페이지에서 다음 단계를 수행하고 **다음**을 클릭합니다.

	![Single Sign-On 구성](./media/active-directory-saas-wizergosproductivitysoftware-tutorial/tutorial_wizergosproductivitysoftware_06.png)

    a. **인증서 다운로드**를 클릭하고 파일을 컴퓨터에 저장합니다.

    b. **Next**를 클릭합니다.

6. 다른 웹 브라우저 창에서 Wizergos Productivity Software 테넌트에 관리자로 로그인합니다.

7. 햄버거 메뉴에서 **관리자**를 선택합니다.
...
8. 왼쪽 메뉴의 관리자 페이지에서 **인증**을 선택하고 **Azure AD**를 클릭합니다.
...
9. **인증** 섹션에서 다음 단계를 수행합니다.
	...
	a. Azure AD에서 다운로드한 인증서를 열고 인증서의 지문을 복사합니다. 그런 다음 **인증서 지문**의 텍스트 상자에 붙여 넣습니다.

	![앱 쪽에서 Single Sign-On 구성](./media/active-directory-saas-wizergosproductivitysoftware-tutorial/tutorial_wizergosproductivitysoftware_004.png)

	b. **발급자 URL** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사에서 나온 **발급자 URL** 값을 저장합니다.

	c. **Signle Sign-On URL** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **Single Sign-on 서비스 URL** 값을 입력합니다.

	d. **Signle Sign-Out URL** 텍스트 상자에 Azure AD 응용 프로그램 구성 마법사의 **Single Sign-Out 서비스 URL** 값을 입력합니다.

	e. **저장** 단추를 클릭합니다.

	> [AZURE.NOTE] 인증서 지문은 사이에 공백을 포함하면 안 됩니다.
...
### Wizergos Productivity Software 테스트 사용자 만들기
...
이 섹션의 목적은 Britta Simon에게 Wizergos Productivity Software에 대한 액세스 권한을 부여하여 Single Sign-On을 사용할 수 있도록 하는 것입니다.
