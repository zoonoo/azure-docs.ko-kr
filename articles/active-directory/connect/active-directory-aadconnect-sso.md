...
먼저 사용자는 SharePoint 온라인과 같이 트러스트 토큰이 Azure AD에서 발행된 리소스에 액세스하려고 시도합니다.  그런 다음, SharePoint 온라인은 Azure AD로 인증하도록사용자를 리디렉션합니다. 조직에 대해 Single sign-on이 활성화된 경우 사용자는 Azure AD를 구축할 수 있도록 사용자 이름을 제공합니다. 조직에 Single sign-on이 활성화된 경우 다음과 같은 상황이 발생합니다.
...
>[!NOTE]
>SSO를 구성하기 위해 Azure AD Connect가 포트 9090(TCP)의 *.msappproxy.net과 통신할 수 있어야 합니다. 구성 중에만 필요하며 최종 사용자에 의한 인증 중에는 사용되지 않습니다.

## <a name="ensuring-clients-sign-in-automatically"></a>클라이언트 로그인 자동으로 확인
기본적으로 브라우저는 인트라넷 영역에서와 같이 URL이 정의되어 있지 않으면 웹 서버에 자격 증명을 보내려는 시도하지 않습니다.  일반적으로 브라우저는 URL을 보면 적합한 영역을 계산할 수 있습니다.  예를 들어 URL이 http://intranet/이면 URL이 인트라넷 영역으로 매핑되기 때문에 브라우저는 자격 증명을 자동으로 보냅니다.  하지만 URL에 http://intranet.contoso.com/과 같이 마침표가 포함되어 있으면 서버는 자격 증명을 자동으로 보내지 않고 다른 인터넷 사이트를 처리하듯 URL을 처리합니다.
