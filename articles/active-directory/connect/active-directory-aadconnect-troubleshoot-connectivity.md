...
첫째, [**machine.config**](active-directory-aadconnect-prerequisites.md#connectivity) 가 올바르게 구성되었는지 확인해야 합니다.  
...
설치 마법사가 Azure AD 연결에 성공했지만 암호 자체를 확인할 수 없는 경우 이 ![badpassword](./media/active-directory-aadconnect-troubleshoot-connectivity/badpassword.png)가 표시됩니다.
...
![proxy407](./media/active-directory-aadconnect-troubleshoot-connectivity/invokewebrequest407.png)와 같은 오류가 표시됩니다.
...
* https://dc.services.visualstudio.com을 호출하게 됩니다. 설치가 성공하는 데 프록시에 이 URL이 열려 있을 필요는 없으므로 무시해도 됩니다.
* DNS 확인에서 microsoftonline.com이 아닌 DNS 이름 공간 nsatc.net 및 기타 네임스페이스에 있어야 하는 실제 호스트가 나열됩니다. 그러나 실제 서버 이름에 대한 웹 서비스를 요청하지 않으므로 이 호스트를 프록시에 추가할 필요가 없습니다.
...
잘못된 사용자 이름 또는 암호 자세한 내용은 [암호를 확인할 수 없음](#the-password-cannot-be-verified) 를 참조하세요.
...
* 이 오류가 표시되면 [netsh](active-directory-aadconnect-prerequisites.md#connectivity) 에서 프록시 구성을 살펴보고 구성이 올바른지 확인합니다.
