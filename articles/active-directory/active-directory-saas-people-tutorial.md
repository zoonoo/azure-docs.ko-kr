...
3. **People 도메인 및 URL** 섹션에서 다음 단계를 수행합니다.
    ...
    a. **로그온 URL** 텍스트 상자에서 다음 패턴으로 URL을 입력합니다. `https://<company name>.peoplehr.com/`

    b. **식별자** 텍스트 상자에서 `https://www.peoplehr.com` 패턴을 사용하여 URL을 입력합니다.

    c. **회신 URL** 텍스트 상자에 다음 패턴으로 URL을 입력합니다.`https://<company name>.peoplehr.net/Pages/Saml/ConsumeAzureAD.aspx`

    > [!NOTE] 
    > 이러한 값은 실제 값이 아닙니다. 이러한 값을 실제 식별자, 회신 URL 및 로그온 URL로 업데이트합니다. 이러한 값을 얻으려면 [People 클라이언트 지원 팀](mailto:customerservices@peoplehr.com)에 문의하세요.
