---
title: PowerShell - Azure AD를 사용하여 가상 시스템 규모 집합에서 관리되는 ID 구성
...
1. [새 AzVmssConfig](/powershell/module/az.compute/new-azvmssconfig) cmdlet 참조 문서의 *예제 1을* 참조하여 시스템 할당된 관리 ID가 있는 가상 시스템 스케일 집합을 만듭니다.  `-IdentityType SystemAssigned` 매개 변수를 `New-AzVmssConfig` cmdlet에 추가:
...
2. 먼저 [`Get-AzVmss`](/powershell/module/az.compute/get-azvmss) cmdlet을 사용하여 가상 시스템 크기 집합 속성을 검색합니다. 그런 다음, 시스템 할당 관리 ID를 사용하도록 설정하려면 [Update-AzVmss](/powershell/module/az.compute/update-azvmss) cmdlet에서 `-IdentityType` 스위치를 사용합니다.
