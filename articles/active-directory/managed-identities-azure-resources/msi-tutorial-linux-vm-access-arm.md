...
[az vm assign-identity](/cli/azure/vm#az-vm-assign-identity)를 사용하여 Linux VM에 사용자 할당 관리 ID를 할당합니다. `<RESOURCE GROUP>` 및 `<VM NAME>` 매개 변수 값을 원하는 값으로 바꾸세요. 이전 단계에서 반환된 `id` 속성을 `--identities` 매개 변수 값에 사용합니다.
