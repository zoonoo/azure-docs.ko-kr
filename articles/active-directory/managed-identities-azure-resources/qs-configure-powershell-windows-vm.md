...
1. `Get-AzVM` cmdlet을 사용하여 VM 속성을 검색합니다. 그런 다음, 시스템 할당 관리 ID를 사용하도록 설정하려면 [Update-AzVM](/powershell/module/az.compute/update-azvm) cmdlet에서 `-IdentityType` 스위치를 사용합니다.
...
1. 그룹의 `ObjectID`(반환된 값의 `Id` 필드에 지정되어 있음)를 검색하여 적어 둡니다.

   ```azurepowershell-interactive
   Get-AzADGroup -searchstring "myGroup"
   ```

1. VM 서비스 주체를 그룹에 추가합니다.
...
1. `Get-AzVM` cmdlet을 사용하여 VM 속성을 검색하고 `-IdentityType` 매개 변수를 `UserAssigned`로 설정합니다.
...
1. [New-AzUserAssignedIdentity](/powershell/module/az.managedserviceidentity/new-azuserassignedidentity) cmdlet을 사용하여 사용자 할당 관리 ID를 만듭니다.  다음 단계에서 필요하므로 출력에서 `Id`를 메모해 둡니다.
   ...
   ```
1. `Get-AzVM` cmdlet을 사용하여 VM 속성을 검색합니다. 그런 다음, Azure VM에 사용자 할당 관리 ID를 할당하고 [Update-AzVM](/powershell/module/az.compute/update-azvm) cmdlet에서 `-IdentityType` 및 `-IdentityID` 스위치를 사용합니다.  `-IdentityId` 매개 변수의 값은 이전 단계에서 메모해 둔 `Id`입니다.  `<VM NAME>`, `<SUBSCRIPTION ID>`, `<RESROURCE GROUP>` 및 `<USER ASSIGNED IDENTITY NAME>`을 사용자 고유의 값으로 바꿉니다.
