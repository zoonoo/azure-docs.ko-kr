---
title: "Azure Backup 에이전트(MARS 에이전트) 문제 해결| Microsoft Docs"
...
아래 지정된 권장 조치를 참조하여 Azure Backup 에이전트의 구성이나 등록 중에 표시되는 오류를 해결합니다.
...
## <a name="error-the-activation-did-not-complete-successfully-the-current-operation-failed-due-to-an-internal-service-error-0x1fc07"></a>오류: 정품 인증이 완료되지 않았습니다. 내부 서비스 오류 [0x1FC07]로 인해 현재 작업이 실패했습니다.

| 오류 세부 정보 | 가능한 원인 | 권장 조치 |
| ---     | ---     | ---    |          
| **오류** </br><ol><li>*정품 인증이 완료되지 않았습니다. 내부 서비스 오류 [0x1FC07]로 인해 현재 작업이 실패했습니다. 잠시 후 작업을 다시 시도하세요. 문제가 지속되면 Microsoft 지원에 문의하세요.* <li>*오류 34506. 이 컴퓨터에 저장된 암호화의 암호가 제대로 구성되어 있지 않습니다.* | <li> 스크래치 폴더가 공간이 부족한 볼륨에 있습니다. <li> 스크래치 폴더가 다른 위치로 잘못 이동되었습니다. <li> OnlineBackup.KEK 파일이 없습니다. | <li>스크래치 폴더 또는 캐시 위치를 백업 데이터 전체 크기의 5~10%에 달하는 여유 공간이 있는 볼륨으로 이동합니다. 캐시 위치를 올바르게 이동하려면 [이 문서](https://docs.microsoft.com/azure/backup/backup-azure-file-folder-backup-faq#backup)에 언급된 단계를 참조합니다.<li> OnlineBackup.KEK 파일이 있는지 확인합니다. <br>*스크래치 폴더 또는 캐시 위치 경로의 기본 위치는 C:\Program Files\Microsoft Azure Recovery Services Agent\Scratch입니다.*
...
* [Azure Backup 에이전트로 Windows 서버 백업](tutorial-backup-windows-server-to-azure.md)에서 자세한 내용을 참조하세요.
