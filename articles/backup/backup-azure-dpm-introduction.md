---
title: 워크로드를 백업하기 위해 DPM 서버 준비
description: 이 문서에서는 Azure 백업 서비스를 사용하여 Azure에 대한 DPM(시스템 센터 데이터 보호 관리자) 백업을 준비하는 방법을 알아봅니다.
ms.topic: conceptual
ms.date: 01/30/2019
...
[시스템 센터 DPM은](https://docs.microsoft.com/system-center/dpm/dpm-overview) 파일 및 응용 프로그램 데이터를 백업합니다. DPM 은 Azure Backup과 다음과 같이 상호작용합니다.
...
구성 요소 | DPM 서버에Windows PowerShell 및 .NET 프레임워크 4.5가 설치되어 있어야 합니다.
...
로컬 스토리지 | 백업하려는 각 컴퓨터에는 백업되는 데이터 크기의 5% 이상을 포함하는 로컬 무료 저장소가 있어야 합니다. 예를 들어 100GB 데이터를 백업하는 경우 스크래치 위치에 최소 5GB의 여유 공간이 필요합니다.
자격 증명 모음 스토리지 | Azure Backup 자격 증명 모음에 백업할 수 있는 데이터 양에는 제한이 없지만 데이터 원본(예: 가상 컴퓨터 또는 데이터베이스)의 크기는 54,400GB를 초과해서는 안 됩니다.
Azure ExpressRoute | Azure ExpressRoute개인 또는 Microsoft 피어링으로 구성된 경우 데이터를 Azure에 백업하는 데 사용할 수 없습니다.<br/><br/> Azure ExpressRoute공용 피어링으로 구성된 경우 데이터를 Azure에 백업하는 데 사용할 수 있습니다.<br/><br/> **참고:** 새 회로의 경우 공용 피어링이 더 이상 사용되지 않습니다.
...
자격 증명을 얻으려면 Azure 포털에서 보안 채널을 통해 자격 증명 자격 증명 파일을 다운로드합니다.
...
- DPM 서버에서 액세스할 수 있는 위치에 파일이 저장되어 있는지 확인합니다. 파일 공유/SMB에 저장된 경우 액세스 권한을 확인합니다.
...
4. **속성** > **백업 자격 증명에서** **다운로드를**클릭합니다. 포털은 자격 증명 모음 이름과 현재 날짜를 조합하여 자격 증명 모음의 자격 증명 파일을 생성하고 다운로드할 수 있도록 제공합니다.
...
1. DPM 관리자 콘솔 > **관리**에서 **온라인**을 클릭합니다. **등록을**선택합니다. 서버 등록 마법사가 열립니다.
...
서버가 자격 증명 모음에 성공적으로 등록되면 이제 Microsoft Azure에 백업을 시작할 준비가 완료된 것입니다. DPM 콘솔에서 Azure에 대한 워크로드를 백업하도록 보호 그룹을 구성해야 합니다. 보호 그룹을 배포하는 [방법을 알아봅니다.](https://docs.microsoft.com/system-center/dpm/create-dpm-protection-groups?view=sc-dpm-2019)
...
설치 응용 프로그램에서 액세스할 수 있는 위치에서 자격 증명 자격 증명 파일을 사용할 수 있는지 확인합니다. 액세스 관련 오류가 발생할 경우 이 컴퓨터의 임시 위치에 자격 증명 모음 자격 증명 파일을 복사하고 작업을 다시 시도합니다.
