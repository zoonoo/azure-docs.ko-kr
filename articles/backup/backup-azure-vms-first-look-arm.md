...
Azure Vm을 백업 하기 위해 Azure Backup는 컴퓨터에서 실행 되는 VM 에이전트에 확장을 설치 합니다. Azure Marketplace 이미지에서 VM을 만든 경우 에이전트가 실행됩니다. 예를 들어 사용자 지정 VM을 만들거나 온-프레미스에서 컴퓨터를 마이그레이션하는 경우에는 에이전트를 수동으로 설치 해야 할 수도 있습니다.
...
1. 백업을 즉시 실행 하려면 VM 메뉴에서 지금 **백업 백업을 선택**  >  합니다.
