...
일시 삭제를 사용 하지 않도록 설정 하면 모든 유형의 작업에 대해 기능을 사용할 수 없다는 점을 기억해 야 합니다. 예를 들어 동일한 자격 증명 모음에서 가상 컴퓨터에 대해 사용 하도록 설정 된 상태로 유지 하면서 SQL server 또는 SAP HANA Db에 대해서만 일시 삭제를 사용 하지 않도록 설정할 수는 없습니다. 세부적인 제어를 위해 별도의 자격 증명 모음을 만들 수 있습니다.
