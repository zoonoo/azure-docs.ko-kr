...
| 논리 디스크(실제 디스크)--%평균 Disk Sec Read or Write |• 0.001ms ~ 0.015ms = 정상</br>• 0.015ms ~ 0.025ms = 경고 또는 모니터링</br>• 0.026ms 이상 = 위험 또는 사양을 벗어남 |
...
## 원인: 다른 프로세스 또는 바이러스 백신 소프트웨어가 Azure 백업을 방해함
...
## 원인: 백업 에이전트가 Azure 가상 컴퓨터에서 실행되고 있음
