...
6.  명령 프롬프트에서 시스템 상태 복원을 실행할 수도 있습니다. 이렇게 하려면 복구할 컴퓨터에서 Windows Server 백업을 시작합니다. 버전 식별자를 확인하려면 명령 프롬프트에 다음을 입력합니다. ```wbadmin get versions -backuptarget \<servername\sharename\>```
