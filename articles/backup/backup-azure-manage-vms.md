...
백업 항목이 보존 정책으로 다시 보호되거나 데이터 삭제로 보호 중지될 때까지 복구 지점은 영원히 보존됩니다. 다시 보호하는 경우, 관련된 새 정책에 따라 복구 지점의 보존이 결정됩니다. 마찬가지로 백업 중지를 수행하지 않고 데이터 원본을 삭제하면 새 백업이 실패하고 이전 복구 지점이 보존 정책에 따라 만료되지만, 데이터 삭제를 사용하여 백업 중지를 수행할 때까지 마지막 복구 지점이 항상 유지됩니다.
