<properties
	pageTitle="클래식 배포 모델을 사용하여 Windows 서버 또는 클라이언트 백업 | Microsoft Azure"
...
# 클래식 배포 모델을 사용하여 Windows 서버 또는 클라이언트 백업
...
>[AZURE.IMPORTANT] Azure는 리소스를 만들고 작업하기 위한 두 가지 배포 모델로 리소스 관리자와 클래식을 제공합니다. 이 문서에서는 클래식 배포 모델 사용에 대해 설명합니다. 새로운 배포는 대부분 리소스 관리자 모델을 사용하는 것이 좋습니다.
