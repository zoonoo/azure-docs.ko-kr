...
데이터베이스 대신 백업 데이터를 파일로 복원 하려면 **파일로 복원**을 선택 합니다. 지정 된 경로에 파일을 덤프 한 후에는 이러한 파일을 데이터베이스로 복원 하려는 모든 SAP HANA 컴퓨터로 가져올 수 있습니다. 이러한 파일을 컴퓨터로 이동할 수 있으므로 이제 구독과 지역 간에 데이터를 복원할 수 있습니다.
...
1. **서버의 대상 경로**에서 2 단계에서 선택한 서버에 폴더 경로를 입력 합니다. 서비스에서 필요한 모든 백업 파일을 덤프 하는 위치입니다.
    ...
    * JSON 메타 데이터 파일 (관련 된 각 백업 파일에 해당)
    ...
    일반적으로 대상 경로로 지정 된 경우 네트워크 공유 경로 또는 탑재 된 Azure 파일 공유의 경로를 사용 하면 동일한 네트워크에 있거나 동일한 Azure 파일 공유에 탑재 된 다른 컴퓨터에서 이러한 파일에 쉽게 액세스할 수 있습니다.

    >[!NOTE]
    >대상으로 등록 된 VM에 탑재 된 Azure 파일 공유에서 데이터베이스 백업 파일을 복원 하려면 루트 계정에 Azure 파일 공유에 대 한 읽기/쓰기 권한이 있어야 합니다.
...
1. 선택한 복원 지점 유형 (지정**시간** 또는 **전체 & 차등**)에 따라 대상 경로에 하나 이상의 폴더가 생성 된 것을 볼 수 있습니다. 이라는 폴더 중 하나에는 `Data_<date and time of restore>` 전체 백업과 차등 백업이 포함 되 고 다른 폴더에는 `Log` 로그 백업이 포함 됩니다.
1. 복원 된 파일을 데이터베이스로 복원 하려는 SAP HANA 서버로 이동 합니다.
1. 그러고 나서 다음 단계를 수행합니다.
    1. 다음 명령을 사용 하 여 백업 파일이 저장 되는 폴더/디렉터리에 대 한 사용 권한을 설정 합니다.

        ```bash
        chown -R <SID>adm:sapsys <directory>
        ```

    1. 다음 명령 집합을 실행 합니다.`<SID>adm`

        ```bash
        su - <sid>adm
        ```

    1. 복원에 사용할 카탈로그 파일을 생성 합니다. 전체 백업에 대 한 JSON 메타 데이터 파일에서 **BackupId** 를 추출 합니다 .이 파일은 나중에 복원 작업에서 사용 됩니다. 전체 및 로그 백업이 다른 폴더에 있는지 확인 하 고 이러한 폴더에서 카탈로그 파일 및 JSON 메타 데이터 파일을 삭제 하십시오.
        ...
        * `<DataFileDir>`-전체 백업을 포함 하는 폴더
        * `<LogFilesDir>`-로그 백업을 포함 하는 폴더
        * `<PathToPlaceCatalogFile>`-카탈로그 파일이 생성 된 폴더를 배치 해야 합니다.

    1. HANA Studio를 통해 새로 생성 된 카탈로그 파일을 사용 하 여 복원 하거나 새로 생성 된 카탈로그를 사용 하 여 HDBSQL 복원 쿼리를 실행 합니다. HDBSQL 쿼리는 아래에 나열 되어 있습니다.

    * 특정 시점으로 복원 하려면 다음을 수행 합니다.

        새 복원 된 데이터베이스를 만드는 경우 HDBSQL 명령을 실행 하 여 새 데이터베이스를 만든 `<DatabaseName>` 다음 복원에 대 한 데이터베이스를 중지 합니다. 그러나 기존 데이터베이스만 복원 하는 경우에는 HDBSQL 명령을 실행 하 여 데이터베이스를 중지 합니다.

        그런 후 다음 명령을 실행 하 여 데이터베이스를 복원 합니다.

        ```hdbsql
        RECOVER DATABASE FOR <DatabaseName> UNTIL TIMESTAMP '<TimeStamp>' CLEAR LOG USING SOURCE '<DatabaseName@HostName>'  USING CATALOG PATH ('<PathToGeneratedCatalogInStep3>') USING LOG PATH (' <LogFileDir>') USING DATA PATH ('<DataFileDir>') USING BACKUP_ID <BackupIdFromJsonFile> CHECK ACCESS USING FILE
        ```

        * `<DatabaseName>`-복원 하려는 새 데이터베이스 또는 기존 데이터베이스의 이름
        * `<Timestamp>`-지정 시간 복원의 정확한 타임 스탬프
        * `<DatabaseName@HostName>`-복원에 사용 되는 백업이 있는 데이터베이스의 이름 및이 데이터베이스가 상주 하는 **호스트** /SAP HANA 서버 이름입니다. `USING SOURCE <DatabaseName@HostName>`옵션은 복원에 사용 되는 데이터 백업이 대상 SAP HANA 컴퓨터와 다른 SID 또는 이름을 가진 데이터베이스 임을 지정 합니다. 따라서 백업이 수행 되는 동일한 HANA 서버에서 수행 되는 복원에 대해 지정할 필요가 없습니다.
        * `<PathToGeneratedCatalogInStep3>`- **C 단계** 에서 생성 된 카탈로그 파일의 경로
        * `<DataFileDir>`-전체 백업을 포함 하는 폴더
        * `<LogFilesDir>`-로그 백업을 포함 하는 폴더
        * `<BackupIdFromJsonFile>`- **C 단계** 에서 추출 된 **BackupId**

    * 특정 전체 또는 차등 백업으로 복원 하려면 다음을 수행 합니다.
