---
title: Azure 백업 서버를 사용하여 워크로드 백업
description: 이 문서에서는 MABS(Microsoft Azure 백업 서버)를 사용하여 워크로드를 보호하고 백업하기 위해 환경을 준비하는 방법을 알아봅니다.
ms.topic: conceptual
ms.date: 11/13/2018
...
> 에 적용: MABS v3. (MABS v2는 더 이상 지원되지 않습니다. MABS v3 보다 이전 버전을 사용 하는 경우 최신 버전으로 업그레이드 하십시오.)
...
> [!NOTE]
> Azure Backup Server는 이제 VMware VM을 보호하고 개선된 보안 기능을 제공할 수 있습니다. 아래 섹션에 설명된 대로 제품 및 최신 Azure Backup 에이전트를 설치합니다. Azure Backup Server를 사용하여 VMware 서버를 백업하는 방법에 대한 자세한 내용은 [Azure Backup Server를 사용하여 VMware 서버 백업](backup-azure-backup-server-vmware.md) 문서를 참조하세요. 보안 기능에 대한 자세한 내용은 [Azure 백업 보안 기능 설명서를](backup-azure-security-feature.md)참조하십시오.
>
>

Azure VM에 배포된 MABS는 Azure에서 VM을 백업할 수 있지만 백업 작업을 활성화하려면 동일한 도메인에 있어야 합니다. Azure VM을 백업하는 프로세스는 온-프레미스에서 VM을 백업하는 것과 동일하게 유지되지만 Azure에서 MABS를 배포하는 데는 몇 가지 제한사항이 있습니다. 제한에 대한 자세한 내용은 [DPM을 Azure 가상 컴퓨터로](https://docs.microsoft.com/system-center/dpm/install-dpm?view=sc-dpm-1807#setup-prerequisites) 참조하십시오.

> [!NOTE]
> Azure에는 리소스 를 만들고 작업하기 위한 두 가지 배포 모델( [리소스 관리자 및 클래식.](../azure-resource-manager/management/deployment-models.md) 이 문서에서는 리소스 관리자 모델을 사용하여 배포된 VM을 복원하기 위한 정보 및 절차를 제공합니다.
>
>
...
Azure 백업 서버를 실행하기 위한 서버를 선택할 때는 Windows Server 2016 데이터 센터 또는 Windows Server 2019 데이터 센터의 갤러리 이미지로 시작하는 것이 좋습니다. [Azure 포털에서 첫 번째 Windows 가상 머신 만들기](../virtual-machines/virtual-machines-windows-hero-tutorial.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)문서는 Azure를 사용한 경험이 없는 경우 Azure에서 권장된 가상 머신 시작에 대한 자습서를 제공합니다. 서버 가상 머신(VM)에 권장되는 최소 요구 사항은 4개의 코어와 8GB RAM이 있는 Standard_A4_v2 이어야 합니다.
...
Azure에서 기본 서버를 실행하지 않을 경우 Hyper-V VM, VMware VM 또는 실제 호스트에서 서버를 실행할 수 있습니다. 서버 하드웨어에 권장되는 최소 요구 사항은 두 개의 코어와 8GB RAM입니다. 지원되는 운영 체제는 다음 표에 나열되어 있습니다.
...
스토리지 복제 옵션을 사용하면 지역 중복 스토리지와 로컬 중복 스토리지 중에서 선택할 수 있습니다. 기본적으로 Recovery Services 자격 증명 모음은 지역 중복 스토리지를 사용합니다. 이 자격 증명 모음이 기본 자격 증명 모음인 경우 스토리지 옵션을 지역 중복 스토리지 상태로 둡니다. 오래 지속되지 않는 저렴한 옵션을 원하는 경우에는 로컬 중복 스토리지를 선택합니다. [Azure Storage 복제 개요에서](../storage/common/storage-redundancy.md) [지리적 중복](../storage/common/storage-redundancy-grs.md) 및 [로컬 중복](../storage/common/storage-redundancy-lrs.md) 저장소 옵션에 대해 자세히 알아보기.
...
5. **Backup 목표** 블레이드의 **작업이 실행되는 위치** 메뉴에서 **온-프레미스**를 선택합니다.
    ...
    백업할 **내용에서** 드롭다운 메뉴를 사용하여 보호할 워크로드를 선택한 다음 **확인을**클릭합니다.
...
7. 모든 파일을 선택하고 **다음**을 클릭합니다. Microsoft Azure Backup 다운로드 페이지에서 들어오는 모든 파일을 다운로드하고 모든 파일을 동일한 폴더에 배치합니다.
    ...
    모든 파일의 다운로드 크기가 3G> 때문에 10Mbps 다운로드 링크에서 다운로드를 완료하는 데 최대 60분이 걸릴 수 있습니다.
...
> [!WARNING]
> 설치 파일을 추출하려면 최소 4GB의 여유 공간이 필요합니다.
>
>
...
2. 시작 화면에서 **다음** 버튼을 클릭합니다. 그러면 *필수 구성 요소 확인* 섹션으로 이동합니다. 이 화면에서 **확인**을 클릭하여 Azure Back Server에 대한 하드웨어 및 소프트웨어 필수 구성 요소가 충족되었는지 확인합니다. 모든 필수 조건이 성공적으로 충족되면 컴퓨터가 요구 사항을 만족한다는 것을 나타내는 메시지가 표시됩니다. **다음** 단추를 클릭합니다.

    ![Azure Backup 서버 - 시작 및 필수 조건 확인](./media/backup-azure-microsoft-azure-backup/prereq/prereq-screen2.png)
3. Azure 백업 서버 설치 패키지에는 필요한 적절한 SQL Server 이비너리와 함께 제공됩니다. 새 Azure 백업 서버 설치를 시작할 때 **이 설치를 통해 SQL Server의 새 인스턴스 설치** 옵션을 선택하고 확인 및 **설치** 단추를 클릭합니다. 필수 구성 요소가 성공적으로 설치되면 **다음**을 클릭합니다.

    >[!NOTE]
    >사용자 고유의 SQL 서버를 사용하려는 경우 지원되는 SQL Server 버전은 2016년 및 2017년 SQL Server 2014 SP1 이상입니다.  모든 SQL Server 버전은 Standard 또는 Enterprise 64비트여야 합니다.
    >Azure Backup 서버는 원격 SQL Server 인스턴스에서 작동하지 않습니다. Azure Backup 서버에서 사용되는 인스턴스는 로컬이어야 합니다. MABS에 기존 SQL 서버를 사용하는 경우 MABS 설정은 SQL 서버의 *명명된 인스턴스만* 사용합니다.

    ![Azure Backup 서버 - SQL 확인](./media/backup-azure-microsoft-azure-backup/sql/01.png)

    컴퓨터를 다시 시작하라는 권장 사항과 함께 실패가 발생한 경우 작업을 수행하고 **다시 확인**을 클릭합니다. SQL 구성 문제가 있는 경우 SQL 지침에 따라 SQL을 다시 구성하고 기존 SQL 인스턴스를 사용하여 MABS를 설치/업그레이드하도록 다시 시도합니다.
   ...
   **수동 구성**
    ...
    **SQL 2017을 사용하여 SSRS 구성**
    ...
    SQL 2017의 고유한 인스턴스를 사용하는 경우 SSRS를 수동으로 구성해야 합니다. SSRS 구성 후 SSRS의 *IsInitialized* 속성이 *True*로 설정되었는지 확인합니다. True로 설정하면 MABS는 SSRS가 이미 구성되어 있다고 가정하고 SSRS 구성을 건너뜁니다.

    SSRS 구성에 대해 다음 값을 사용합니다.
    * 서비스 계정: '기본 제공 계정 사용'은 네트워크 서비스여야 합니다.
    * 웹 서비스 URL: '가상 디렉터리'는 SQLInstanceName> ReportServer_\<있어야 합니다.
    * 데이터베이스: 데이터베이스 이름은 보고서\<서버 $ SQLInstanceName>
    * 웹 포털 URL: '가상 디렉터리'는 SQLInstanceName> Reports_\<합니다.
    ...
    > [!NOTE]
    > MABS의 데이터베이스로 사용되는 SQL Server에 대한 라이선스는 Microsoft 온라인 서비스 약관(OST)의 적용을 받습니다. [Microsoft Online Services Terms](https://www.microsoft.com/licensing/product-licensing/products) OST에 따르면 MABS와 함께 번들로 제공되는 SQL Server는 MABS의 데이터베이스로만 사용할 수 있습니다.
...
8. 설치는 단계적으로 발생합니다. 첫 번째 단계에서는 Microsoft Azure 복구 서비스 에이전트가 서버에 설치됩니다. 또한 마법사가 인터넷 연결을 확인합니다. 인터넷 연결을 사용할 수 있으면 설치를 계속 진행할 수 있고 그렇지 않으면 프록시 세부 정보를 제공하여 인터넷에 연결해야 합니다.
...
1. 백업 서버 관리자 콘솔에서 **관리** > **에이전트를**선택합니다.
...
스토리지를 유지하면서 MABS를 새 서버로 이동해야 하는 경우 단계는 다음과 같습니다. 모든 데이터가 Modern Backup Storage에 있는 경우에만 수행할 수 있습니다.
  ...
  > [!IMPORTANT]
  >
  > * 새 서버 이름은 원래 Azure Backup Server 인스턴스와 같은 이름이어야 합니다. 이전 저장소 풀 및 MABS 데이터베이스(DPMDB)를 사용하여 복구 지점을 유지하려는 경우 새 Azure Backup Server 인스턴스의 이름을 변경할 수 없습니다.
  > * MABS 데이터베이스(DPMDB)의 백업이 있어야 합니다. 데이터베이스를 복원해야 합니다.

1. 표시 창에서 보호 에이전트를 업데이트할 클라이언트 컴퓨터를 선택합니다.
2. 원래 Azure 백업 서버를 종료하거나 와이어를 해제합니다.
3. 활성 디렉터리에서 컴퓨터 계정을 다시 설정합니다.
4. 새 컴퓨터에 Server 2016을 설치하고 원래 Azure Backup Server와 동일한 컴퓨터 이름으로 지정합니다.
5. 도메인 조인
6. Azure 백업 서버 V3 이상 설치(이전 서버에서 MABS 저장소 풀 디스크 이동 및 가져오기)
7. 1단계에서 가져온 DPMDB를 복원합니다.
8. 스토리지를 원래 백업 서버에서 새 서버로 연결합니다.
9. SQL에서 DPMDB 복원
10. 새 서버의 관리 명령줄에서 Microsoft Azure Backup 설치 위치 및 bin 폴더로 cd

    경로 예: C:\windows\system32>cd "c:\\프로그램 파일\Microsoft Azure 백업\DPM\DPM\빈\"

11. Azure 백업에 DPMSYNC -SYNC 실행

    이전 디스크를 이동하는 대신 DPM 저장소 풀에 새 디스크를 추가한 경우 DPMSYNC -Reallocatereplica를 실행합니다.
...
이와 동시에 Azure 구독은 정상 상태여야 합니다. 구독 상태를 알아보고 관리하려면 [구독 포털에](https://account.windowsazure.com/Subscriptions)로그인합니다.
...
Azure에 대한 액세스를 방해하는 방화벽이나 프록시가 있는 경우 방화벽/프록시 프로필에서 다음 도메인 주소를 허용해야 합니다.
...
ExpressRoute Microsoft 피어링을 사용하는 경우 다음 서비스/지역을 선택하십시오.

* Azure Active Directory (12076:5060)
* 마이크로소프트 Azure 리전(복구 서비스 볼트의 위치에 따라)
* Azure 저장소(복구 서비스 자격 증명 모음의 위치에 따라)
...
Azure 구독을 *만료됨* 또는 *프로비전 해제됨* 상태에서 *활성* 상태로 전환할 수 있습니다. 그러나 상태가 *활성*상태가 아닌 동안 제품 동작에 몇 가지 영향을 미칩니다.
...
[DPM에 대한 환경 준비에](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-2019)대한 자세한 정보는 여기에서 확인할 수 있습니다. 또한 여기에는 Azure Backup 서버를 배포 및 사용하는 데 지원되는 구성에 대한 정보도 포함되어 있습니다. 다양한 작업을 수행하는 데 일련의 [PowerShell cmdlet](https://docs.microsoft.com/powershell/module/dataprotectionmanager/?view=systemcenter-ps-2016)을 사용할 수 있습니다.
