...
## 개념
Azure 백업 설명서에 [Azure IaaS VM 백업이 소개되어](backup-azure-vms-introduction.md) 있습니다. VM, 필수 조건 및 제한 사항을 백업해야 이유에 대한 필수 정보가 설명되어 있습니다.
...
## 설정 및 등록
시작하려면 다음과 같은 **Switch-azuremode** commandlet를 사용함으로써 *AzureResourceManager* 모드로 전환하여 Azure 백업 commandlet를 사용합니다.
...
**New-AzureBackupVault** commandlet을 사용하여 새 백업 저장소를 만들 수 있습니다. 백업 저장소는 ARM 리소스이므로 리소스 그룹 내에 배치해야 합니다. 승격된 Azure PowerShell 콘솔에서 다음 명령을 실행합니다.
...
**Get-AzureBackupVault** commandlet을 사용하여 지정된 구독의 모든 백업 저장소 목록을 가져올 수 있습니다.

> [AZURE.NOTE]백업 저장소 개체를 변수에 저장하면 편리합니다. 저장소 개체는 많은 Azure 백업 commandlet에 대한 입력으로서 필요합니다.


### VM 등록
Azure 백업으로 백업을 구성하는 첫 번째 단계는 Azure 백업 저장소에 사용자 컴퓨터 또는 VM을 등록하는 것입니다. **Register-AzureRMBackupContainer** commandlet는 Azure IaaS 가상 컴퓨터의 입력된 정보를 받아 지정된 저장소에 등록합니다. 등록 작업은 Azure 가상 컴퓨터를 백업 저장소와 연결하고 백업 수명 주기를 통해 VM을 추적합니다.
...
### 보호 정책 만들기
VM의 백업을 시작하기 위한 새 보호 정책을 만들지 않아도 됩니다. 저장소는 빠르게 보호하는 데 사용할 수 있는 ‘기본 정책’이 적용된 다음, 나중에 올바른 세부 정보로 편집할 수 있습니다. 다음과 같은 **Get-AzureRMBackupProtectionPolicy** commandlet를 사용하여 저장소에서 사용할 수 있는 정책 목록을 얻을 수 있습니다.
...
백업 정책은 하나 이상의 보존 정책과 연관됩니다. 보존 정책은 Azure 백업의 복구 지점을 얼마나 오래 유지할지를 정의합니다. **New-AzureRMBackupRetentionPolicy** commandlet는 보존 정책 정보를 담고 있는 PowerShell 개체를 만듭니다. 이러한 보존 정책 개체는 *New-AzureRMBackupProtectionPolicy* commandlet에 대한 입력으로 사용되거나 *Enable-AzureRMBackupProtection* commandlet와 함께 직접 사용됩니다.

백업 정책은 항목의 백업 수행 시점과 빈도를 정의합니다. **New-AzureRMBackupProtectionPolicy** commandlet는 백업 정책 정보를 담고 있는 PowerShell 개체를 만듭니다. 백업 정책은 *Enable-AzureRMBackupProtection* commandlet에 대한 입력으로 사용됩니다.
...
### 초기 백업
백업 일정은 항목에 대한 전체 초기 복사와 후속 백업에 대한 증분 복사 작업을 처리합니다. 그러나 초기 백업을 강제로 특정 시간 또는 즉시 수행하려는 경우 다음과 같은 **Backup-AzureRMBackupItem** commandlet를 사용합니다.
...
진행 중인 작업의 최신 상태를 가져오려면 **Get-AzureRMBackupJob** commandlet를 사용합니다.
...
불필요한 추가 코드이며, 완료하기 위해 이들 작업을 폴링하지 않고 **Wait-AzureRMBackupJob** commandlet를 더욱 간단히 사용할 수 있습니다. 스크립트를 사용하면 작업이 완료되거나 지정된 시간 제한 값에 도달할 때까지 commandlet는 실행을 일시 중지합니다.
...
올바른 백업 항목을 식별하는 PowerShell 개체를 가져오려면 해당 저장소에 있는 컨테이너에서 시작하고 개체 계층 구조를 따라 내려가는 방식으로 작업해야 합니다. VM을 나타내는 컨테이너를 선택하려면 **Get-AzureRMBackupContainer** commandlet와 **Get AzureRMBackupItem** commandlet로 향하는 파이프를 사용합니다.
...
이제 **Get-AzureRMBackupRecoveryPoint** commandlet를 사용하는 백업 항목에 대한 모든 복구 지점을 나열하고 복원할 복구 지점을 선택할 수 있습니다. 일반적으로 사용자는 목록에서 가장 최근의 *AppConsistent* 지점을 선택합니다.
...
복원 작업이 완료되면 **Get-AzureRMBackupJobDetails** commandlet를 사용하여 복원 작업의 세부 정보를 얻을 수 있습니다. *ErrorDetails* 속성은 VM을 다시 빌드하는 데 필요한 정보를 갖게 됩니다.
