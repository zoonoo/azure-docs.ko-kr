...
> * **MAB 에이전트** - 최소 에이전트 버전 2.0.9052. 이러한 기능을 사용하도록 설정한 후에는 암호 변경, 데이터 삭제로 백업 중지와 같은 중요한 작업을 수행하기 위해 이 에이전트 버전으로 업그레이드 해야 합니다. <br/>
> * **Azure Backup Server** - Azure Backup Server 업데이트 1이 있는 최소 MAB 에이전트 버전 2.0.9052 <br/>
> * **DPM** - DPM에 대해서는 이러한 기능을 사용하도록 설정하지 마세요. 이러한 기능은 향후 UR에서 곧 제공될 예정이므로 사용하도록 설정해도 기존 기능에는 영향을 미치지 않습니다. <br/>
> * **IaaS VM 백업** - IaaS VM 백업에 대해서는 이러한 기능을 사용하도록 설정하지 마세요. 이러한 기능은 아직 IaaS VM 백업에 사용할 수 없으므로 사용하도록 설정해도 IaaS VM 백업에 영향을 미치지 않습니다.
> * 일단 사용하도록 설정한 경우 모든 Azure Recovery Services 에이전트(MARS) 컴퓨터 및 Azure Backup Server에 대한 보안 기능을 자격 증명 모음에 등록합니다. <br/>
> * 이 기능의 설정은 일회성 작업이며 사용하도록 설정한 후에는 사용하지 않도록 설정할 수 없습니다. <br/>
> * 이 기능은 Recovery Services 자격 증명 모음에만 사용할 수 있습니다.
> 
> 
...
1. 백업이 발생하는 컴퓨터를 계속 사용할 수 있는 경우 MARS의 [동일한 컴퓨터로 데이터 복구](backup-azure-restore-windows-server.md#recover-data-to-the-same-machine)를 사용하여 모든 이전 복구 지점에서 복구합니다.
2. 위에서 언급한 컴퓨터를 사용할 수 없는 경우 [다른 컴퓨터로 복구](backup-azure-restore-windows-server.md#recover-to-an-alternate-machine)를 사용하여 다른 MARS 컴퓨터를 통해 이 데이터를 가져옵니다.
