...
1. Azure Backup Server 관리 콘솔의 **복구** 탭에서 화면 왼쪽 상단에 있는 **' 외부 DPM 추가 '** 를 선택 합니다.
...
5. 적절 한 항목을 마우스 오른쪽 단추로 클릭 하 고 **복구**를 선택 합니다.

    ![외부 DPM 복구](./media/backup-azure-alternate-dpm-server/recover.png)
6. **복구 선택 사항**을 검토합니다. 복구될 백업 복사본의 날짜와 시간, 백업 복사본이 만들어진 원본을 확인합니다. 선택이 올바르지 않으면 **취소** 를 선택 하 여 복구 탭으로 다시 이동 하 여 적절 한 복구 지점을 선택 합니다. 선택이 올바르면 **다음**을 선택 합니다.
...
9. **요약** 화면에 지금까지 선택한 옵션이 나열됩니다. **복구**를 선택 하면 데이터가 적절 한 온-프레미스 위치에 복구 됩니다.
...
10. DPM 서버의 **복구** 탭에서 **외부 dpm 지우기** 를 선택 하 여 외부 dpm 서버에 대 한 보기를 제거할 수 있습니다.
