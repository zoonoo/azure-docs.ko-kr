...
1. **ConfigureSharePoint.exe 실행** - 이 도구는 SharePoint VSS 기록기 서비스 \(WSS\)를 구성하고 보호 에이전트에 SharePoint 팜에 대한 자격 증명을 제공합니다. 보호 에이전트를 배포한 후에는 프런트 엔드 웹 서버의 `<MABS Installation Path\>\bin` 폴더\-에서 ConfigureSharePoint.exe 파일을 찾을 수 있습니다.  여러 WFE 서버가 있는 경우 그 중 하나에만 설치해야 합니다. 다음과 같이 실행합니다.
    ...
    * 팜 관리자 자격 증명을 입력 합니다. 이 계정은 WFE 서버에서 로컬 관리자 그룹의 구성원 이어야 합니다. 팜 관리자가 로컬 관리자가 아닌 경우, WFE 서버에 다음 권한을 부여 합니다.
        ...
        * MABS 폴더 \(%Program Files%\\Data Protection Manager\\DPM\)에 대한 모든 권한을 WSS\_Admin\_WPG 그룹에 부여합니다.
            -A

        * MABS 레지스트리 키 \(HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Microsoft Data Protection Manager\)에 WSS\_Admin\_WPG 그룹 읽기 액세스 권한을 부여합니다.
...
1. **데이터 보호 방법 선택**에서 단기 및 장기\- 백업을 처리하는 방법을 지정합니다. 단기\- 백업은 항상 디스크에 먼저 기록되고 Azure Backup을 사용하여 \(단기 또는 장기\- 동안\) 디스크에서 Azure 클라우드로 백업하는 옵션이 있습니다.
...
2. **알림 게시**를 사용하도록 설정한 후에는 사용자 조치가 필요할 수 있는 모든 기존 MABS 경고가 **MABS 경고** 이벤트 로그에 게시됩니다. Operations Manager 에이전트가 MABS 서버에 설치되면 이러한 경고를 Operations Manager에 게시하며 새로운 경고가 생성되면 계속해서 콘솔을 업데이트합니다.
