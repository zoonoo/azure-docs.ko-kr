...
> [!NOTE]
> Azure에는 리소스를 만들고 작업하기 위한 두 가지 배포 모델인 [리소스 관리자와 클래식](../resource-manager-deployment-model.md) 모델이 있습니다. 복구 서비스 자격 증명 모음으로 Resource Manager 배포 VM과 클래식 VM을 보호할 수 있습니다. 클래식 배포 모델 VM 작업에 대한 자세한 내용은 [Azure 가상 컴퓨터 백업](backup-azure-vms.md)을 참조하세요.
> 
> 
...
4. **백업 작업** 블레이드에서 모든 작업의 상태를 볼 수 있습니다.
   ...
   > [!NOTE]
   > 백업 작업의 일부로 Azure 백업 서비스는 각 가상 컴퓨터에서 백업 확장에 대한 명령을 발행하여 모든 쓰기를 플러시하고 일관된 스냅숏을 찍습니다.
   > 
   > 
