...
1. Azure에 전송하도록 준비하려는 탑재된 디스크에 대한 드라이브 문자를 후행 콜론 없이 입력합니다. 메시지가 표시될 때 드라이브의 서식에 대한 확인을 제공합니다.
...
1. 디스크를 도구가 제공한 주소로 배송하고 나중에 참조할 수 있도록 추적 번호를 유지합니다.<br/>
2. 도구가 표시한 링크로 이동하면 **오프라인 백업 시작** 워크플로에서 지정한 Azure 저장소 계정이 표시됩니다. 여기서 저장소 계정의 **가져오기/내보내기** 탭에서 새로 생성된 가져오기 작업을 볼 수 있습니다.
   
   ![생성된 가져오기 작업](./media/backup-azure-backup-import-export/ImportJobCreated.png)<br/>
3. 페이지 아래쪽의 **배송 정보**를 클릭하여 다음 화면에 표시된 것처럼 사용자의 연락처 세부 정보를 업데이트합니다. Microsoft는 가져오기 작업이 완료된 후 이 정보를 사용하여 사용자의 디스크를 배송합니다.
   
   ![연락처 정보](./media/backup-azure-backup-import-export/contactInfoAddition.PNG)<br/>
4. 다음 화면에서 배송 세부 정보를 입력합니다. Azure 데이터 센터에 배송한 디스크에 해당하는 **배달 배송업체** 및 **추적 번호** 세부 정보를 제공합니다.
