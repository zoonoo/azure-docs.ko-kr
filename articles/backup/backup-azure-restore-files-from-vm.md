...
백업된 Azure VM에 여러 디스크에 걸쳐 있는 볼륨(스팬 볼륨 및 스트라이프 볼륨) 및/또는 내결함성 볼륨(미러 볼륨 및 RAID-5 볼륨)이 있는 경우 동일한 VM에서 실행 가능한 스크립트를 실행할 수 없습니다. 대신, 호환되는 운영 체제로 다른 컴퓨터에서 실행 가능한 스크립트를 실행합니다.
