...
**복원 옵션** | **세부 정보**
--- | ---
**새 VM 만들기** | 복원 지점에서 기본 VM을 빠르게 만들고, 준비하고, 실행할 수 있습니다.<br/><br/> VM에 대 한 이름을 지정을 선택 하 고 리소스 그룹 가상 네트워크 (VNet)는 표시 됩니다 하 고 복원된 된 VM에 대 한 저장소 계정을 지정 합니다.
**디스크 복원** | 새 VM을 만드는 데 사용할 수 있는 VM 디스크를 복원합니다.<br/><br/> Azure Backup은 VM을 사용자 지정하고 만드는 데 도움이 되는 템플릿을 제공합니다. <br/><br> 복원 작업은 다운로드하여 사용자 지정 VM 설정을 지정하고 VM을 만드는 데 사용할 수 있는 템플릿을 생성합니다.<br/><br/> 디스크를 지정 하는 저장소 계정에 복사 됩니다.<br/><br/> 또는 디스크를 기존 VM에 연결하거나 PowerShell을 사용하여 새 VM을 만들 수 있습니다.<br/><br/> 이 옵션은 VM을 사용자 지정하거나, 백업 시 존재하지 않았던 구성 설정을 추가하거나, 템플릿 또는 PowerShell을 사용하여 구성해야 하는 설정을 추가하려는 경우에 유용합니다.
**기존 항목 바꾸기** | 디스크를 복원하고 이 디스크를 사용하여 기존 VM의 디스크를 바꿀 수 있습니다.<br/><br/> 현재 VM이 있어야 합니다. VM이 삭제된 경우에는 이 옵션을 사용할 수 없습니다.<br/><br/> Azure Backup 디스크를 교체 하기 전에 기존 VM의 스냅숏을 생성 하 고 지정 된 스테이징 위치에 저장 합니다. VM에 연결 하는 기존 디스크를 선택한 복원 지점으로 바뀝니다.<br/><br/> 스냅숏은 자격 증명 모음에 복사 하 고 보존 정책에 따라 유지 됩니다. <br/><br/> 기존 항목 바꾸기는 암호화되지 않은 관리 VM에 대해 지원됩니다. 관리되지 않는 디스크, [일반화된 VM](https://docs.microsoft.com/azure/virtual-machines/windows/capture-image-resource) 또는 [사용자 지정 이미지를 사용하여 만든](https://azure.microsoft.com/resources/videos/create-a-custom-virtual-machine-image-in-azure-resource-manager-with-powershell/) VM에 대해서는 지원되지 않습니다.<br/><br/> 현재 VM보다 많거나 적은 디스크가 복원 지점에 있는 경우 복원 지점의 디스크 수에는 VM 구성만 반영됩니다.<br/><br/>
...
저장소 계정에 대 한 일부 세부 정보:

- **VM 만들기**: 새 VM을 만들 때 VM은 지정 하는 저장소 계정에 배치 됩니다.
- **디스크 복원**: 디스크를 복원 하는 경우 디스크를 지정 하는 저장소 계정에 복사 됩니다. 복원 작업 다운로드 하 고 사용 하 여 사용자 지정 VM 설정을 지정할 수 있는 템플릿을 생성 합니다. 이 템플릿은 지정된 된 저장소 계정에 배치 됩니다.
- **디스크 교체**: 기존 VM에서 디스크를 교체 하는 경우 Azure 백업은 디스크를 교체 하기 전에 기존 VM의 스냅숏을 사용 합니다. 스냅숏은 지정할 (저장소 계정) 스테이징 위치에 저장 됩니다. 이 저장소 계정은 복원 과정에서 스냅숏을 일시적으로 저장 되 고 나중에 쉽게 제거할 수 있습니다는 이렇게 하려면 새 계정을 만드는 것이 좋습니다.
- **저장소 계정 위치** : 저장소 계정 자격 증명 모음과 동일한 지역에 있어야 합니다. 이러한 계정에만 표시 됩니다. 위치에 저장소 계정이 없는 경우 새로 만들려면 해야 합니다.
- **저장소 유형** : Blob 저장소는 지원 되지 않습니다.
- **저장소 중복**: ZRS(영역 중복 스토리지)는 지원되지 않습니다. 계정에 대 한 복제 및 중복 정보는 계정 이름 뒤에 오는 괄호 안에 표시 됩니다. 
- **Premium storage**:
    - 프리미엄이 아닌 Vm을 복원 하는 경우에 프리미엄 저장소 계정은 지원 되지 않습니다.
    - 관리 되는 Vm을 복원 하는 중, 하는 경우에 프리미엄 저장소 계정 네트워크 규칙을 사용 하 여 구성 지원 되지 않습니다.
...
5. **저장소 위치**, VM에 대 한 저장소 계정을 지정 합니다. [자세히 알아보기](#storage-accounts).
...
3. **스테이징 위치**, 복원 프로세스 중 현재 관리 되는 디스크의 스냅숏을 저장할 위치를 지정 합니다. [자세히 알아보기](#storage-accounts).
