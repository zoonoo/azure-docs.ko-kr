...
- **VM 만들기**: 새 VM을 만들 때 VM은 사용자가 지정 하는 저장소 계정에 배치 됩니다.
- **복원 디스크**: 디스크를 복원 하면 지정한 저장소 계정에 디스크가 복사 됩니다. 복원 작업은 다운로드 하 여 사용자 지정 VM 설정을 지정 하는 데 사용할 수 있는 템플릿을 생성 합니다. 이 템플릿은 지정 된 저장소 계정에 배치 됩니다.
...
- **저장소 중복성**: ZRS(영역 중복 스토리지)는 지원되지 않습니다. 계정에 대 한 복제 및 중복성 정보는 계정 이름 뒤에 괄호 안에 표시 됩니다. 
- **Premium storage**:
...
**특수 네트워크 구성으로 VM 복원** | 특수 네트워크 구성에는 내부 또는 외부 부하 분산을 사용하거나, 여러 NICS를 사용하거나, 예약된 여러 IP 주소를 사용하는 VM이 포함되어 있습니다. [디스크 복원 옵션](#restore-disks)을 사용하여 이러한 VM을 복원합니다. 이 옵션은 지정 된 저장소 계정에 Vhd의 복사본을 만들고, 다음에 따라 [내부](https://azure.microsoft.com/documentation/articles/load-balancer-internal-getstarted/) 또는 [외부](https://azure.microsoft.com/documentation/articles/load-balancer-internet-getstarted/) 부하 분산 장치, [다중 NIC](../virtual-machines/windows/multiple-nics.md)또는 [여러 예약 된 IP 주소](../virtual-network/virtual-network-multiple-ip-addresses-powershell.md)를 사용 하 여 VM을 만들 수 있습니다. 구성.
