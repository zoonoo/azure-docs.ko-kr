...
4. SQL Server 인스턴스에서 **작업 관리자**를 엽니다. **AzureWLBackupCoordinatorSvc** 서비스를 다시 시작합니다.<br/> <br/>
 이 방법을 사용 하면 백업 응용 프로그램에서 많은 리소스를 사용 하는 경우 SQL Server [Resource Governor](https://docs.microsoft.com/sql/relational-databases/resource-governor/resource-governor?view=sql-server-2017) 는 들어오는 응용 프로그램 요청이 사용할 수 있는 CPU, 물리적 IO 및 메모리 양에 대 한 제한을 지정 하는 보다 일반적인 방법입니다.
...
SQL 제한 사항에 따라 보조 복제본에 대 한 전체 백업만 복사를 실행할 수 있습니다. 그러나 전체 백업은 허용 되지 않습니다.
...
이름 다시 지정 된 데이터베이스는 새 데이터베이스로 처리 됩니다. 따라서 서비스는 데이터베이스를 찾을 수 없고 백업에 실패 한 것 처럼이 상황을 처리 합니다.
...
[Autoprotected 인스턴스에 추가](backup-sql-server-database-azure-vms.md#enable-auto-protection) 하는 데이터베이스는 보호 된 항목 아래에 즉시 표시 되지 않을 수 있습니다. 일반적으로 검색이 8시간마다 실행되기 때문입니다. 그러나 다음 그림에 표시 된 것 처럼 **Db 복구**를 선택 하 여 수동으로 검색을 실행 하는 경우 새 데이터베이스를 즉시 검색 하 고 보호할 수 있습니다.
