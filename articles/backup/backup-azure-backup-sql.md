...
이 문서는 Azure 백업을 사용한 SQL Server 데이터베이스 백업의 구성 단계를 안내합니다.
...
1. DPM 서버에서 새 **보호 그룹**을 만들어 SQL Server 데이터베이스에 대한 새로운 백업 정책을 구성합니다. **보호** 작업 영역을 클릭합니다.

2. **새로 만들기**를 클릭하여 새 보호 그룹을 만듭니다.
...
8. **다음**를 클릭합니다.
    ...
    DPM에서는 초기 백업 복사본을 만들기 위한 데이터 원본(SQL Server 데이터베이스)당 하나의 볼륨을 만듭니다. 이 방법을 사용하여 논리 디스크 관리자(LDM)는 DPM을 제한하여 최대 300개의 데이터 원본(SQL Server 데이터베이스)을 보호합니다. 이를 방지하기 위해 DPM은 여러 데이터 원본에 단일 볼륨을 사용하는 다른 방법을 구현했습니다. **DPM 저장소 풀에 데이터 배치**를 사용하여 이를 사용하도록 설정합니다. 이 방법을 사용하여 DPM는 최대 2000개의 SQL 데이터베이스를 보호할 수 있습니다.

    **볼륨 자동 증가**를 선택하는 경우 프로덕션 데이터가 증가함에 따라 DPM은 백업 볼륨 증가를 처리할 수 있습니다. **볼륨 자동 증가**의 선택을 해제한 경우 보호 그룹의 데이터 원본을 백업하는 데 사용되는 백업 저장소를 제한합니다.
...
9. 관리자는 해당 초기 백업을 수동으로 전송하도록 선택하여(오프 네트워크) 네트워크를 통한 대역폭 정체를 방지합니다. 또한 초기 전송이 발생할 수 있는 시간을 구성할 수 있습니다. **다음**을 클릭합니다.
    ...
    초기 백업 복사본은 프로덕션 서버(SQL Server 컴퓨터)에서 DPM 서버로 전체 데이터 원본(SQL Server 데이터베이스)을 전송해야 합니다. 해당 데이터는 때로 매우 크고 네트워크를 통해 전송하면 대역폭을 초과할 수 있습니다. 따라서 관리자는 대역폭 정체를 방지하기 위해 해당 초기 백업을 **수동으로** 전송하거나 네트워크를 통해 **자동으로** 발생시킬지를 선택합니다. 또한 관리자가 **네트워크**를 선택하면 지정된 시간에 초기 백업 복사본을 **지금** 또는 **나중에** 만들지를 선택합니다.

    초기 백업을 완료하면 백업의 나머지 부분은 일반적으로 매우 작고 네트워크에서 전송되는 초기 백업 복사본의 위쪽 있는 증분 백업입니다.

10. 일관성 확인을 실행하려는 경우 선택하고 **다음**을 클릭합니다.

    ![일관성 확인](./media/backup-azure-backup-sql/pg-consistent.png)

    DPM은 일관성 확인을 수행하여 백업 시점의 무결성을 확인합니다. 프로덕션 서버(이 시나리오에서 SQL Server 컴퓨터)에서 백업 파일의 체크섬 및 DPM에서 해당 파일에 대한 백업 데이터를 계산합니다. 충돌이 있을 경우 DPM에서 백업된 파일이 손상되었음을 가정합니다. DPM은 체크섬 불일치에 해당하는 블록을 전송하여 백업된 데이터를 균형있게 조정합니다. 일관성 확인이 성능 집약적인 작업이기에 관리자를 자동으로 예약 또는 실행되도록 하는 옵션으로 제공합니다.
...
13. 보존 정책 일정을 선택합니다. 보존 정책이 작동하는 방법에 대한 자세한 내용은 [Azure 백업을 사용하여 테이프 인프라 대체 문서](backup-azure-backup-cloud-as-tape.md)에서 제공됩니다.
...
3. 드롭다운에서 **온라인 보호**를 선택하고 **확인**을 클릭합니다. Azure에서 복구 지점 만들기가 시작됩니다.

    ![복구 지점 만들기](./media/backup-azure-backup-sql/sqlbackup-azure.png)

4. 다음 그림에 그려진 것처럼 진행 중인 작업을 찾을 수 있는 작업 영역 **모니터링**에서 작업 진행률을 볼 수 있습니다.
...
3. DPM에서는 복구 지점에 대한 세부 정보를 보여줍니다. **다음**을 클릭합니다. 복구 형식 **SQL Server의 원본 인스턴스에 복구**을 선택합니다. 데이터베이스를 덮어씁니다. **다음**을 클릭합니다.
...
5. **요약** 화면에서 지금까지 제공하는 모든 복구 구성을 표시합니다. **복구**를 클릭합니다.
    ...
    복구가 완료되면 복원된 데이터베이스 복사본은 응용 프로그램과 일치됩니다.
