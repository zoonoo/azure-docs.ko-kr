...
Azure Backup에서 생성 되는 기본 스냅숏은 유지 되므로 저장소에 복구 지점의 종료와 관련 된 비용이 있을 수 있습니다. 복구 지점이 유지 되는 경우에는 나중에 파일 공유를 복원할 수 있다는 이점도 있습니다. 복구 지점을 유지하는 비용에 대한 자세한 내용은 [가격 책정 세부 정보](https://azure.microsoft.com/pricing/details/backup/)를 참조하세요. 모든 복구 지점은 삭제 하기로 결정 한 경우에는 파일 공유를 복원할 수 없습니다.
