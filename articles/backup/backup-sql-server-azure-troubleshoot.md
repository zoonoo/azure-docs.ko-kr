...
복원 작업 중에 데이터베이스 파일의 매핑이 포함 된 JSON 파일을 대상 복원 경로에 배치 하 여 대상 복원 파일 경로를 재정의할 수 있습니다. `database_name.json` 파일을 만들고 *C:\Program Files\Azure 워크 로드 Backup\bin\plugins\SQL*위치에 배치 합니다.
