...
1. 포털에서 자격 증명 모음을 열고 **개요** 섹션에서 **+백업** 을 선택합니다.

    ![백업 창](./media/backup-azure-vms-encryption/select-backup.png)

1. **백업 목표** > **워크로드가 실행되는 위치** 에서 **Azure** 를 선택합니다.
1. **백업할 항목** 에서 **가상 머신** 을 선택합니다. 그런 다음, **백업** 을 선택합니다.

      ![시나리오 창](./media/backup-azure-vms-encryption/select-backup-goal-one.png)

1. **백업 정책** > **백업 정책 선택** 에서 자격 증명 모음과 연결하려는 정책을 선택합니다. 그런 다음, **확인** 을 선택합니다.
...
- **게스트 사용자** 인 경우 키 자격 증명 모음에 액세스하는 Azure Backup에 대한 권한을 제공해야 합니다.
...
2. 다음 작업 중 하나를 수행하여 가상 머신 인스턴스를 다시 만듭니다.
    ...
    2. PowerShell을 사용하여 복원된 디스크에서 새 VM을 만듭니다. [자세히 알아봅니다](backup-azure-vms-automation.md#create-a-vm-from-restored-disks).
