...
5. 다음에 설명된 대로 **VMSnapshotScriptPluginConfig.json** 을 구성합니다.
    ...
    - **continueBackupOnFailure**: Azure Backup을 사전 스크립트 또는 사후 스크립트가 실패하는 경우 파일 시스템 일관성/충돌 일관성 백업으로 대체하려는 경우 이 값을 **true** 로 설정합니다. 이 값을 **false** 로 설정하면 스크립트 오류가 발생하는 경우 백업에 실패합니다(이 설정과 관련 없는 충돌 일관성 백업으로 대체하는 단일 디스크 VM의 경우 제외). **continueBackupOnFailure** 값이 false로 설정된 경우 백업이 실패하면 서비스의 재시도 논리에 따라 백업 작업을 다시 시도합니다(규정된 시도 횟수).
...
6. 이제 스크립트 프레임워크가 구성되었습니다. VM 백업이 이미 구성된 경우 다음 백업 시 스크립트가 호출되고 애플리케이션 일치 백업이 트리거됩니다. VM 백업이 구성되지 않은 경우 [Recovery Services 자격 증명 모음에 Azure 가상 머신 백업](./backup-azure-vms-first-look-arm.md)을 사용하여 구성합니다.
