...
POST URI에 `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}`및 `{fabricName}` 매개 변수가 있습니다. 이 예제에서 다른 매개 변수에 대 한 값은 다음과 같습니다.

- `{fabricName}` *Azure*

- *azurefilesvault* `{vaultName}`

- *azurefiles* `{vaultresourceGroupName}`
