...
GET URI `{subscriptionId}` `{vaultName}`에는, `{vaultresourceGroupName}` 매개 변수가 있습니다. 이 예제 `{vaultName}` 에서는 "testvault"이 고 `{vaultresourceGroupName}` 는 "testVaultRG"입니다. 모든 필수 매개 변수가 URI에서 지정되므로 별도 요청 본문이 필요 없습니다.
...
패치 URI에는 `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}` 매개 변수가 있습니다. 이 예제 `{vaultName}` 에서는 "testvault"이 고 `{vaultresourceGroupName}` 는 "testVaultRG"입니다. URI를 위의 값으로 바꾸면 URI가 다음과 같이 표시 됩니다.
