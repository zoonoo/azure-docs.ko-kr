...
7. Exchange Server 데이터베이스의 무결성을 확인하려는 경우 **Eseutil 실행하여 데이터 무결성 확인** 옵션을 선택합니다.
    ...
    이 옵션을 선택하면 DPM 서버에서 백업 일관성 검사가 실행되어 Exchange 서버에서 **eseutil** 명령을 실행하여 생성되는 I/O 트래픽을 방지합니다.
...
9. **복사 Backup**에 대한 데이터베이스를 선택하고 **다음**을 클릭합니다.
   ...
   > [!NOTE]
   > 데이터베이스의 DAG 복사본 하나 이상에 대해 "전체 백업"을 선택하지 않으면 로그가 잘리지 않습니다.
   >
   >
...
15. **Azure Backup**에 대한 일정을 정의하고 **다음**을 클릭합니다. 다음은 그 예입니다.
    ...
    > [!NOTE]
    > 온라인 복구 지점은 Express 전체 복구 지점을 기반으로 합니다. 따라서 express 전체 복구 지점에 대해 지정된 시간 이후에 온라인 복구 지점을 예약해야 합니다.
    >
    >
