...
| 응용 프로그램 일치 백업 | Windows에서 응용 프로그램 일치 백업은 복원 시 수정이 필요 없으므로 복구 시간 목표를 단축하고 고객이 보다 신속하게 실행 중 상태로 돌아갈 수 있도록 해 줍니다. |
...
| Azure IaaS VM 백업 | <li>Windows/Linux에 대한 네이티브 백업<li>특정 에이전트 설치 필요 없음<li>백업 인프라가 필요 없는 패브릭 수준 백업<li>Azure 백업 자격 증명 모음 사용 | <li>하루에 한 번 백업/디스크 수준 복원<li>온-프레미스 백업 불가능 | VM<br>개별 디스크 |
...
| Azure IaaS VM 백업 | 서비스 예정 | 서비스 예정 - V2 Linux VM <br><br>(파일 시스템 수준 일관성) |
...
#### 증분 백업
모든 구성 요소는 대상 저장소(디스크, 테이프, 백업 자격 증명 모음)에 관계 없이 증분 백업을 지원합니다. 증분 백업을 사용하면 저장소와 시간이 효율적으로 사용됩니다. 증분 백업은 마지막 백업 이후 변경된 내용만 대상 저장소로 전송합니다.
...
데이터를 백업 서버(DPM 또는 백업 서버)로 보내서 보호하려는 고객의 경우 주 서버에서 백업 서버로 전송되는 트래픽을 압축하여 대역폭을 절약할 수 있습니다.

#### 네트워크 제한
Azure 백업 에이전트는 데이터를 전송하는 동안 네트워크 대역폭이 사용되는 방식을 제어할 수 있는 제한 기능을 제공합니다. 근무 시간에 데이터를 백업해야 하는데 백업 프로세스가 다른 인터넷 트래픽을 방해하지 말아야 할 때 유용한 기능입니다. 데이터 전송 제한은 백업 및 복원 작업에 적용됩니다.
...
Azure 백업은 데이터를 온-프레미스 및 클라우드에 백업합니다. Azure Site Recovery는 가상 컴퓨터 및 실제 서버의 복제, 장애 조치 및 복구를 조정합니다. 중단 상태가 발생할 때 재해 복구 솔루션이 데이터를 안전하게 유지 및 복구할 수 있어야 하고(백업), *동시에* 워크로드를 사용하고 액세스할 수 있어야 하므로(사이트 복구) 두 서비스가 모두 중요합니다.
...
이러한 자습서에 따라 신속하게 백업할 수 있으므로 데이터를 백업하기 위한 가장 직접적인 경로만 보여 줍니다. 수행하려는 백업 유형에 대 한 자세한 내용은 다음을 참조하세요.
