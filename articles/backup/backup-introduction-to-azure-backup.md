<properties
	...
	description="이 문서에서는 고객이 데이터를 Azure에 백업할 수 있게 해주는 Azure 백업 서비스를 간략하게 설명합니다."
	services="backup"
	documentationCenter=""
	authors="prvijay"
	manager="shreeshd"
...
# Azure 백업 소개
이 문서에서는 고객이 온-프레미스 데이터를 Azure에 백업할 수 있게 해주는 Microsoft 클라우드 통합 백업 솔루션에 대해 간략히 소개합니다.

## Azure 백업이란?
Azure 백업은 온-프레미스 데이터를 Azure에 백업할 수 있게 해주는 다중 테넌트 Azure 서비스입니다. 기존의 온-프레미스 또는 오프사이트 백업 솔루션을 신뢰할 수 있고 안전하며 가격 경쟁력이 있는 클라우드 기반 제품으로 대체합니다. Azure 백업은 확장성, 내구성 및 고가용성을 제공하는 최고 수준의 인프라를 기반으로 하여 빌드되었습니다. 이 솔루션을 사용하면 SCDPM(System Center Data Protection Manager) 서버, Windows 서버 또는 Windows 클라이언트 컴퓨터에서 데이터와 응용 프로그램을 백업할 수 있습니다. Azure 백업 및 SCDPM은 Microsoft 클라우드 통합 백업 솔루션을 구성하는 기본 기술입니다.
...
## Azure에 백업할 수 있는 응용 프로그램 및 작업
SCDPM과 결합할 경우 Azure 백업은 다음을 백업할 수 있습니다.

+ 엔터프라이즈 클라이언트, 서버 컴퓨터의 파일 및 폴더
