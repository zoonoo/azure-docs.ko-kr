...
* SATA 드라이브는 준비 위치에서 SATA 드라이브로 백업 데이터를 복사 하는 컴퓨터 ( *복사 컴퓨터*라고 함)에 연결 되어 있어야 합니다. 복사 컴퓨터에서 BitLocker가 사용 하도록 설정 되어 있는지 확인 합니다.
...
* 이전 명령은 `C:\Program Files\Microsoft Azure Recovery Services Agent\Scratch\MicrosoftBackupProvider\OfflineApplicationParams_<Storageaccountname>.xml`파일을 만듭니다.
...
1. 소유 하는 **응용 프로그램** 탭에서 표시 이름 형식이 `AzureOfflineBackup _<Azure User Id`인 응용 프로그램을 찾습니다.
...
1. 인증서 > 관리 **& 암호** > **인증서 업로드**를 선택 합니다. 이전 단계에서 내보낸 인증서를 업로드 합니다.
...
1. **Cloudbackupprovider**를 마우스 오른쪽 단추로 클릭 하 고 `AzureADAppCertThumbprint_<Azure User Id>`이름을 사용 하 여 새 문자열 값을 추가 합니다.
    ...
    >* 레지스트리 경로 `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Azure Backup\DbgSettings\OnlineBackup; Name: CurrentUserId;`로 이동 합니다.
...
1. 추적 번호 정보가 있으면 가져오기 작업이 완료 될 때까지 대기 중인 원본 컴퓨터로 이동 합니다. *Azureofflinebackupdiskprep* 유틸리티 디렉터리를 현재 디렉터리로 사용 하 여 관리자 권한 명령 프롬프트에서 다음 명령을 실행 합니다.
   ...
   `*.\AzureOfflineBackupDiskPrep.exe*  u:  s:<*Staging Location Path*>   p:<*Path to AzurePublishSettingsFile*>`
    ...
    | s:&lt;*스테이징 위치 경로*&gt; | 이 필수 입력은 명령이 원본 컴퓨터에서 실행 되지 않을 때 사용 됩니다. "오프 라인 백업 시작" 섹션의 워크플로에 입력 한 스테이징 위치에 대 한 경로를 제공 하는 데 사용 됩니다. |
    | p:&lt;*PublishSettingsFile에 대한 경로*&gt; | 이 필수 입력은 명령이 원본 컴퓨터에서 실행 되지 않을 때 사용 됩니다. "오프 라인 백업 시작" 섹션의 워크플로에 입력 한 Azure 게시 설정 파일의 경로를 제공 하는 데 사용 됩니다. |
