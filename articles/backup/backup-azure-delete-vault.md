...
1. 포털에서 **Recovery Services 자격 증명 모음** 으로 이동한 다음 **백업 항목** 으로 이동 합니다. 그런 다음 **백업 관리 유형** 목록에서 클라우드의 보호 된 항목을 선택 합니다 (예: azure Virtual Machines, Azure Storage [Azure Files service] 또는 azure Virtual Machines에서 SQL Server).

    ![백업 유형을 선택합니다.](./media/backup-azure-delete-vault/azure-storage-selected.png)

2. 범주에 대 한 모든 항목의 목록이 표시 됩니다. 마우스 오른쪽 단추를 클릭 하 여 백업 항목을 선택 합니다. 백업 항목이 보호 되었는지 여부에 따라 메뉴에 **백업 중지** 창이 나 **백업 데이터 삭제** 창이 표시 됩니다.
