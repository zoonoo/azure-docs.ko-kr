...
* **암호화된 VM 백업** - 암호화된 Azure VM은 Azure Backup을 사용하여 백업됩니다. 암호화된 Azure VM을 백업하는 방법에 대한 자세한 내용은 [PowerShell을 사용하여 Azure VM의 백업 및 복원](backup-azure-vms-automation.md) 관리 문서를 참조하십시오.
...
위에서 생성된 JSON 파일을 사용하여 비밀 이름 및 값을 가져오고 set secret cmdlet에 제공하여 해당 비밀(BEK)을 다시 키 자격 증명 모음에 지정합니다.**BEK 및 KEK를 사용하여 VM이 암호화된**경우 이러한 cmdlet을 사용합니다.
...
> [!NOTE]
>
> * $secretname 값은 $encryptionObject.OsDiskKeyAndSecretDetails.SecretUrl의 출력을 참조하여 얻을 수 있습니다.SecretUrl / 예 : https://keyvaultname.vault.azure.net/secrets/B3284AAA-DAAA-4AAA-B393-60CAA848AAAA/xx000000xx0849999f3xx30000003163 출력 비밀 URL은 및 비밀 이름은 B3284AAA-DAAA-4AAA-B393-60CA848AAAAA
> * 태그의 값 DiskEncryptionKeyFileName 비밀 이름과 동일 합니다.
>
>
...
키 및 비밀을 키 자격 증명 모음으로 다시 복원한 후 [PowerShell을 사용하여 Azure VM의 백업 및 복원을](backup-azure-vms-automation.md#create-a-vm-from-restored-disks) 관리하여 복원된 디스크, 키 및 비밀에서 암호화된 VM을 만드는 문서를 참조하십시오.
