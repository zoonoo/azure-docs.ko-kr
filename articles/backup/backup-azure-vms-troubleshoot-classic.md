...
| 백업 | VM이 더 이상 존재하지 않아 작업을 수행할 수 없습니다. | 백업용으로 구성된 VM이 삭제되었기 때문에 백업을 수행할 수 없습니다. 백업 항목 보기(설정 > 백업 항목 > 항목 이름 클릭)로 이동한 후 백업 중지를 클릭하여 추가 백업을 중지하세요. 백업 데이터 보존 옵션을 선택하여 데이터를 보존할 수 있습니다. 나중에 백업 다시 시작을 클릭하여 이 가상 컴퓨터에 대한 보호를 다시 시작할 수 있습니다.|
