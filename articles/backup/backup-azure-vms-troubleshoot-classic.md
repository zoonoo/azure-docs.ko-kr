...
| 백업 | 내부 오류가 발생하여 백업하지 못했습니다. 몇 분 후에 작업을 다시 시도하세요. 문제가 지속되면 Microsoft 지원에 문의하세요. | VM 저장소에 액세스하는 데 일시적인 문제가 있는지 확인합니다. [Azure 상태](https://azure.microsoft.com/status/)를 확인하여 지역의 계산/저장소/네트워크와 관련하여 진행 중인 문제가 있는지 확인합니다. 문제가 완화되면 백업을 다시 시도합니다. |
