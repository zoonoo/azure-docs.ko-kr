<properties
   pageTitle="소개: Windows 서버 또는 클라이언트를 Azure에 백업하는 방법 | Microsoft Azure"
   description="간단한 단계를 통해 Azure 백업을 사용하여 Windows 서버를 백업하는 방법에 대해 알아보기"
   services="backup"
   documentationCenter=""
   authors="Jim-Parker"
   manager="jwhit"
...
# Windows 서버 또는 클라이언트를 Azure에 백업

간단한 몇 단계를 통해 Windows 컴퓨터(Windows 클라이언트 또는 Windows Server)를 Azure에 백업할 수 있습니다. 아래 4단계를 완료하면 다음을 수행합니다.

- Azure 구독(필요한 경우)을 설정합니다.
- 백업 자격 증명 모음을 만들고 필요한 구성 요소를 다운로드합니다.
- 해당 구성 요소를 설치하고 등록하여 Windows 서버 또는 클라이언트를 준비합니다.
- 데이터를 백업합니다.
...
## 3단계: Windows Server 또는 클라이언트에 Azure 백업 에이전트 설치
...
1. mmc 스냅인(**Microsoft Azure 복구 서비스 에이전트 시작** 확인란을 선택된 상태로 둔 경우 자동으로 열림)에서 **백업 일정**을 클릭합니다.
...
9. mmc 스냅인에서 **지금 백업**을 클릭하여 네트워크를 통한 초기 시드 작업을 완료합니다.
