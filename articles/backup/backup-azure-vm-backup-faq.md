...
### <a name="i-enabled-lock-on-resource-group-created-by-azure-backup-service-ie-azurebackuprg_geo_number-will-my-backups-continue-to-work"></a>Azure Backup 서비스에서 만든 리소스 그룹에 대해 잠금을 사용 하도록 설정 했습니다 (즉, `AzureBackupRG_<geo>_<number>`)는 백업이 계속 작동 하나요?
...
예약 된 백업은 예약 된 백업 시간의 2 시간 이내에 트리거됩니다. 예를 들어 100 Vm의 백업 시작 시간이 2:00 오전 10 시에 예약 된 경우 최대 4:00 AM의 모든 100 Vm은 백업 작업을 진행 합니다. 가동 중단 및 다시 시작/다시 시도로 인해 예약 된 백업이 일시 중지 된 경우이 예약 된 2 시간 동안 백업을 시작할 수 있습니다.

### <a name="what-is-the-minimum-allowed-retention-range-for-daily-backup-point"></a>매일 백업 지점에 대해 허용 되는 최소 보존 범위는 얼마 인가요?

Azure 가상 머신 백업 정책은 최대 9999 일의 최소 보존 범위 7 일을 지원 합니다. 7 일 이내에 기존 VM 백업 정책을 수정 하려면 7 일의 최소 보존 범위를 만족 하는 업데이트가 필요 합니다.

### <a name="can-i-backup-or-restore-selective-disks-attached-to-a-vm"></a>VM에 연결 된 선택적 디스크를 백업 하거나 복원할 수 있나요?
...
3. 동일한 자격 증명 모음 또는 새 자격 증명 모음에서 백업을 다시 사용 하도록 설정 합니다.

이동 작업 전에 만든 사용 가능한 복원 지점에서 VM을 복원할 수 있습니다.

### <a name="is-there-a-limit-on-number-of-vms-that-can-beassociated-with-a-same-backup-policy"></a>동일한 백업 정책에 연결할 수 있는 Vm 수에 제한이 있나요?
