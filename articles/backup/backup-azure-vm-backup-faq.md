...
* 아직 보호되지 않음 - VM 블레이드로 이동하고 블레이드의 설정 메뉴에서 백업 상태를 확인하여 VM의 백업 상태를 확인할 수 있습니다. [VM의 백업 상태를 확인하는 방법](backup-azure-vms-first-look-arm.md#configure-the-backup-job-from-the-vm-management-blade)에 대해 자세히 알아보세요.
* VM과 동일한 지역에 속함
...
복원된 VM에 대해 빠른 만들기 옵션을 사용하는 방법으로 Azure 전체 VM 복원을 생각해 볼 수 있습니다. VM 복원 옵션은 VM 만들기의 일부로 만드는 리소스의 고유성을 위해 디스크 이름, 디스크에서 사용되는 컨테이너, 공용 IP 주소, 네트워크 인터페이스 이름 등을 변경합니다. 또한 가용성 집합에 VM을 추가하지 않습니다. 

복원 디스크를 사용하여 다음을 수행합니다.
* 백업 구성에서 크기를 변경하는 것처럼 특정 시점의 구성에서 만든 VM을 사용자 지정합니다.
...
VM에 새 정책을 적용하면 새 정책의 일정 및 보존이 수행됩니다. 보존 기간을 늘리면 기존 복구 지점이 새 정책에 따라 유지되도록 표시됩니다. 보존 기간을 줄이면 다음 정리 작업에서 정리(prune) 표시되고 삭제됩니다. 
