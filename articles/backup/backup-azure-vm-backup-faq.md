...
Azure 백업이 키 자격 증명 모음에 액세스할 수 있는 권한을 제공합니다. [Azure Backup PowerShell](backup-azure-vms-automation.md) 설명서의 **백업 사용** 섹션에 설명된 대로 PowerShell에서 권한을 지정합니다.
...
예. **스냅숏 촬영** 상태에서 백업 작업을 취소할 수 있습니다. 스냅샷에서 데이터 전송이 진행 중인 경우 작업을 취소할 수 없습니다.

### <a name="i-enabled-a-lock-on-the-resource-group-created-by-azure-backup-service-for-example-azurebackuprg_geo_number-will-my-backups-continue-to-work"></a>Azure 백업 서비스(예: `AzureBackupRG_<geo>_<number>`)에서 만든 리소스 그룹에 대한 잠금을 활성화했습니다. 내 백업이 계속 작동하나요?

Azure Backup Service에서 만든 리소스 그룹을 잠그면 최대 18개의 복원 지점제한이 있으므로 백업이 실패하기 시작합니다.

잠금을 제거하고 해당 리소스 그룹에서 복원 지점 컬렉션을 지우면 향후 백업이 성공적으로 실행됩니다. [다음 단계에 따라](backup-azure-troubleshoot-vm-backup-fails-snapshot-timeout.md#clean-up-restore-point-collection-from-azure-portal) 복원 지점 컬렉션을 제거합니다.
...
Azure Backup은 RPO가 15분인 SAP HANA 데이터베이스에 대한 스트리밍 백업 솔루션을 제공합니다. SAP HANA의 네이티브 API를 활용하는 기본 백업 지원을 제공하기 위해 SAP에서 백인트 인증을 받았습니다. Azure [VM에서 SAP HANA 데이터베이스 백업에 대해](https://docs.microsoft.com/azure/backup/sap-hana-db-about)자세히 알아봅니다.
...
예약된 백업은 예약된 백업 시간보다 2시간 이내에 트리거됩니다. 예를 들어 100개의 VM에 2:00 AM으로 예약된 백업 시작 시간이 있는 경우 늦어도 오전 4:00까지 모든 100개의 VM은 백업 작업이 진행 중입니다. 가동 중단으로 인해 예약된 백업이 일시 중지되고 다시 시작되거나 다시 시도된 경우 백업은 예약된 2시간 기간 외부에서 시작할 수 있습니다.

### <a name="what-is-the-minimum-allowed-retention-range-for-a-daily-backup-point"></a>일일 백업 지점에 허용되는 최소 보존 범위는 얼마입니까?

Azure 가상 시스템 백업 정책은 7일에서 9999일까지의 최소 보존 범위를 지원합니다. 7일 미만의 기존 VM 백업 정책을 수정하려면 최소 보존 범위를 7일 동안 충족하도록 업데이트해야 합니다.

### <a name="what-happens-if-i-change-the-case-of-the-name-of-my-vm-or-my-vm-resource-group"></a>VM 또는 VM 리소스 그룹의 이름의 대/소문자를 변경하면 어떻게 되나요?

VM 또는 VM 리소스 그룹의 대/소문자(상위 또는 하위)를 변경하면 백업 항목 이름의 대/소행이 변경되지 않습니다. 그러나 이 문제는 Azure 백업 동작으로 예상됩니다. 대/소문자 변경은 백업 항목에 나타나지 않지만 백 엔드에서 업데이트됩니다.

### <a name="can-i-back-up-or-restore-selective-disks-attached-to-a-vm"></a>VM에 연결된 선택적 디스크를 백업하거나 복원할 수 있습니까?

Azure Backup은 이제 Azure 가상 시스템 백업 솔루션을 사용하여 선택적 디스크 백업 및 복원을 지원합니다.

오늘날 Azure Backup은 가상 머신 백업 솔루션을 사용하여 VM의 모든 디스크(운영 체제 및 데이터)를 함께 백업할 수 있도록 지원합니다. 제외 디스크 기능을 사용하면 VM의 여러 데이터 디스크에서 하나 또는 몇 개를 백업할 수 있습니다. 이를 통해 백업 및 복원 요구 사항에 대한 효율적이고 비용 효율적인 솔루션을 제공합니다. 각 복구 지점에는 백업 작업에 포함된 디스크 데이터가 포함되어 있으므로 복원 작업 중에 지정된 복구 지점에서 디스크 하위 집합을 복원할 수 있습니다. 이는 스냅샷과 볼트모두에서 복원하는 데 적용됩니다.
...
복원 프로세스는 동일하게 유지됩니다. VM에 관리되지 않는 디스크가 있는 시점인 경우 [디스크를 관리되지 않는 것으로 복원할](tutorial-restore-disk.md#unmanaged-disks-restore)수 있습니다. VM에 디스크가 관리된 경우 [디스크를 관리 디스크로 복원할](tutorial-restore-disk.md#managed-disk-restore)수 있습니다. 그런 다음 [해당 디스크에서 VM을 만들](tutorial-restore-disk.md#create-a-vm-from-the-restored-disk)수 있습니다.
...
예. VM을 삭제하더라도 볼트의 해당 백업 항목으로 이동하여 복구 지점에서 복원할 수 있습니다.

### <a name="how-do-i-restore-a-vm-to-the-same-availability-sets"></a>VM을 동일한 가용성 집합으로 복원하려면 어떻게 해야 합니까?

관리되는 디스크 Azure VM의 경우 관리되는 디스크로 복원하는 동안 템플릿에 옵션을 제공하여 가용성 집합으로 복원할 수 있습니다. 이 템플릿에 **가용성 집합**이라는 입력 매개 변수가 있습니다.
...
암호화된 VM의 키 자격 증명 모음 설정을 변경한 후에도 백업은 새 세부 정보 집합으로 계속 작동합니다. 그러나 변경 하기 전에 복구 지점에서 복원 한 후 VM을 만들 수 있습니다 전에 키 자격 증명 모음에 있는 비밀을 복원 해야 합니다. 자세한 내용은 이 [문서를](https://docs.microsoft.com/azure/backup/backup-azure-restore-key-secret)참조하십시오.

비밀/키 롤오버와 같은 작업은 이 단계가 필요하지 않으며 복원 후 동일한 KeyVault를 사용할 수 있습니다.
...
예. 도메인 컨트롤러와의 관계가 끊어지면 VM이 복원된 VM에 액세스할 수 있습니다. 자세한 내용은 이 [문서를](https://docs.microsoft.com/azure/backup/backup-azure-arm-restore-vms#post-restore-steps) 참조하세요.
...
### <a name="is-there-a-limit-on-number-of-vms-that-can-beassociated-with-the-same-backup-policy"></a>동일한 백업 정책과 연결할 수 있는 VM 수에 제한이 있습니까?
