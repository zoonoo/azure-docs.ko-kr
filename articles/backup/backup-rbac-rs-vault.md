...
> [!IMPORTANT]
> Azure Backup에서 제공하는 역할은 Azure 포털또는 REST API 또는 복구 서비스 볼트 PowerShell 또는 CLI cmdlet을 통해 수행할 수 있는 작업으로 제한됩니다. Azure 백업 에이전트 클라이언트 UI, System Center Data Protection Manager UI 또는 Azure Backup Server UI에서 실행되는 작업은 이러한 역할의 제어를 받지 않습니다.

Azure Backup은 백업 관리 작업을 제어하는 세 가지 기본 제공 역할을 제공합니다. [Azure RBAC 기본 제공 역할](../role-based-access-control/built-in-roles.md)에 대해 알아보기

* [백업 기여자](../role-based-access-control/built-in-roles.md#backup-contributor) - 이 역할에는 복구 서비스 자격 증명 모음을 삭제하고 다른 사람에게 액세스 권한을 부여하는 경우를 제외하고 백업을 만들고 관리할 수 있는 모든 권한이 있습니다. 모든 백업 관리 작업을 수행할 수 있는 백업 관리 관리자 역할로 생각하시면 됩니다.
...
더 많은 제어를 위해 사용자 고유의 역할을 정의하려는 경우 Azure RBAC에서 [사용자 지정 역할을 빌드하는](../role-based-access-control/custom-roles.md) 방법을 참조하세요.
...
다음 표는 Azure File 공유 작업을 수행하는 데 필요한 백업 관리 작업 및 해당 역할을 캡처합니다.

| 관리 작업 | 역할 필수 | 리소스 |
| --- | --- | --- |
| Azure 파일 공유 백업 사용 | Backup 참가자 |Recovery Services 자격 증명 모음 |
| |스토리지 계정 | 기여자 저장소 계정 리소스 |
| VM의 주문형 백업 | Backup 운영자 | Recovery Services 자격 증명 모음 |
| 파일 공유 복원 | Backup 운영자 | Recovery Services 자격 증명 모음 |
| | Storage 계정 참가자 | 복원 소스 및 대상 파일 공유가 있는 저장소 계정 리소스 |
