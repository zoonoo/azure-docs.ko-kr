...
* 동일한 클라우드 서비스에서 4개가 넘는 클래식 VM을 동시에 백업하도록 예약하지 마세요. 동일한 클라우드 서비스에서 여러 VM을 백업하려면 백업 시작 시간에 한 시간의 시차를 두는 것이 좋습니다.
* 리소스 관리자가 배포한 40개가 넘는 VM을 동시에 백업하도록 예약하지 마세요.
* 백업 서비스가 고객 저장소 계정에서 백업 또는 복구 서비스 자격 증명 모음으로 데이터를 전송하기 위해 IOPS를 사용하도록 VM 예약은 사용량이 적은 시간 동안 백업합니다.
* 정책이 서로 다른 저장소 계정에 분산된 VM을 해결하는지 확인합니다. 권장 단일 저장소 계정에서 20개 이하의 총 디스크를 하나의 정책으로 보호하는 것이 좋습니다. 저장소 계정에 20개가 넘는 디스크가 있는 경우 해당 VM을 여러 정책에 분산하여 백업 프로세스의 전송 단계에서 필요한 IOPS를 얻습니다.
