...
> 2. Azure Backup은 저장소 수준의 스냅숏을 생성하고 해당 스냅숏을 자격 증명 모음으로 전송합니다. 백업 작업이 완료될 때까지 저장소 계정 키를 변경하지 마세요.
>
...
Azure Backup은 백업 워크플로 및 환경을 제어하기 위한 스크립팅 프레임워크를 제공합니다. 애플리케이션 일치 Linux VM 백업을 유지하려면 스크립팅 프레임워크를 사용하여 사용자 지정 사전 스크립트 및 사후 스크립트를 만듭니다. VM 스냅숏을 생성하기 전에 사전 스크립트를 호출하고 VM 스냅숏 작업이 완료되면 사후 스크립트를 호출합니다. 자세한 내용은 [애플리케이션 일치 Linux VM 백업](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent) 문서를 참조하세요.
...
성능에 영향을 주는 추가 요소는 **백업 일정**입니다. 모든 VM이 동시에 백업하도록 정책을 구성한 경우 트래픽 정체가 발생합니다. 백업 프로세스는 동시에 모든 디스크를 백업하려고 합니다. 백업 트래픽을 줄이려면 여러 VM이 겹치지 않게 하루 중 서로 다른 시간대에 백업되도록 하는 것입니다.

## <a name="capacity-planning"></a>용량 계획
[VM 백업 용량 계획 Excel 스프레드시트](https://gallery.technet.microsoft.com/Azure-Backup-Storage-a46d7e33) 를 다운로드하여 디스크 및 백업 일정 선택의 영향을 확인합니다.
...
* 큐 대기 시간입니다. Backup 서비스는 여러 고객의 작업을 동시에 처리하므로 스냅숏 데이터가 Recovery Services 자격 증명 모음에 즉시 복사되지 않을 수 있습니다. 최대 부하 시간에 백업이 처리되기까지 최대 8시간이 걸릴 수 있습니다. 그러나 일별 백업 정책의 경우 총 VM 백업 시간은 24시간 미만입니다.
24시간 미만이라는 전체 백업 시간이 증분 백업에는 유효하지만 첫 번째 백업에는 유효하지 않을 수 있습니다. 첫 번째 백업 시간은 비례하며, 데이터 크기 및 백업 수행 시간에 따라 24시간을 초과할 수 있습니다.
...
복원 작업은 두 가지 기본 작업으로 구성됩니다(볼트에서 선택한 고객 저장소 계정으로 데이터를 다시 복사 및 가상 시스템 만들기). 자격 증명 모음에서 데이터를 복사하는 데필 요한 시간은 백업이 Azure에 저장되는 위치와 고객 저장소 계정의 위치에 따라 다릅니다. 데이터를 복사하는 데 걸리는 시간은 다음에 따라 다릅니다.
...
모든 가상 머신에 대한 백업을 구성하는 동안 다음 사례를 따르는 것이 좋습니다.

* 동일한 클라우드 서비스에서 10개가 넘는 클래식 VM에 대한 백업을 동시에 예약하지 마세요. 동일한 클라우드 서비스에서 여러 VM을 백업하려면 백업 시작 시간에 한 시간의 시차를 두는 것이 좋습니다.
...
* 백업에 사용하도록 설정된 Linux VM의 Python 버전이 2.7 이상인지 확인합니다.

### <a name="best-practices-for-vms-with-unmanaged-disks"></a>관리되지 않는 디스크를 사용하는 VM에 대한 모범 사례

다음 권장 사항은 관리되지 않는 디스크를 사용하는 VM에만 적용됩니다. VM이 Managed Disks를 사용하는 경우 Backup 서비스가 모든 저장소 관리 작업을 처리합니다.

* 여러 저장소 계정에 걸쳐 있는 VM에 백업 정책을 적용해야 합니다. 권장 단일 저장소 계정에서 20개 이하의 총 디스크를 동일한 백업 일정으로 보호하는 것이 좋습니다. 저장소 계정에 20개가 넘는 디스크가 있는 경우 해당 VM을 여러 정책에 분산하여 백업 프로세스의 전송 단계에서 필요한 IOPS를 얻습니다.
