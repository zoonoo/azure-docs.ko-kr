...
Azure Backup은 백업 워크플로 및 환경을 제어하기 위한 스크립팅 프레임워크를 제공합니다. 응용 프로그램 일치 Linux VM 백업을 유지하려면 스크립팅 프레임워크를 사용하여 사용자 지정 사전 스크립트 및 사후 스크립트를 만듭니다. VM 스냅숏을 생성하기 전에 사전 스크립트를 호출하고 VM 스냅숏 작업이 완료되면 사후 스크립트를 호출합니다. 자세한 내용은 [응용 프로그램 일치 Linux VM 백업](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent) 문서를 참조하세요.
