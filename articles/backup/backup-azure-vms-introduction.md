...
### 디스크 수
백업 프로세스는 많은 메모리를 소모하며 최대한 빨리 백업을 완료하기 위해 가능한 한 많은 리소스를 사용하려고 합니다. 그러나 모든 I/O 작업은 초당 60MB의 제한이 있는 *단일 Blob의 목표 처리량*에 의해 제한됩니다. 백업 프로세스의 속도를 높이기 위해, VM의 각 디스크 백업을 *병렬로* 시도합니다. 따라서 VM에 4개의 디스크가 있는 경우, Azure 백업은 4개의 디스크를 모두 병렬로 백업하게 됩니다. 그렇기 때문에 고객 저장소 계정으로부터 백업 트래픽이 있는지 판단하는 가장 중요한 단일 요소는 저장소 계정으로부터 백업되고 있는 **디스크 수**입니다.
...
## 모범 사례
가상 컴퓨터에 대한 백업을 구성하는 동안 다음 모범 사례를 따르는 것이 좋습니다.

- 동시에 백업할 같은 클라우드 서비스의 클래식 VM은 4개를 초과하여 예약하지 마세요. 백업을 위해 동일한 클라우드 서비스에서 더 많은 VM을 구성하려면 백업 일정에 한 시간의 시차를 두는 것이 좋습니다. 
- 동시에 백업할 리소스 관리자 VM은 40개를 초과하여 예약하지 마세요.
- 백업 서비스가 고객 저장소 계정에서 백업 자격 증명 모음으로 데이터를 전송하기 위한 IOPS를 확보할 수 있도록, VM에 대한 백업은 사용량이 적은 시간으로 예약하세요. 
- 정책 내에서 VM이 다양한 저장소 계정으로 분산되도록 해야 합니다. VM으로부터 단일 저장소 계정에 저장된 총 디스크의 수가 20개를 초과하면, 백업의 전송 단계에 필요한 IOPS를 확보하도록 VM을 다양한 백업 일정으로 분산하는 것이 좋습니다.
- 프리미엄 저장소에서 실행되는 VM을 동일한 저장소 계정에 복원하면, 이 작업이 백업 작업으로 들어가면서 백업에 사용할 수 있는 IOPS의 수를 감소시킬 수 있기 때문에 좋지 않습니다. 
- 만족스러운 백업 시간 성능을 확보하려면, 각각의 프리미엄 VM을 다른 프리미엄 저장소 계정에 유지하는 것이 좋습니다. 
