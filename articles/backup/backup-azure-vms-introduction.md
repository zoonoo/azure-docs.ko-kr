...
이 문서에서는 성능 및 리소스를 제안하여 VM 백업 인프라를 계획할 수 있도록 합니다. 또한 백업 서비스의 핵심 요소를 정의합니다. 이러한 측면은 아키텍처, 용량 계획 및 예약을 결정하는 데 중요한 요인이 될 수 있습니다. [환경을 준비](backup-azure-vms-prepare.md)했다면 이 단계는 [VM 백업](backup-azure-vms.md)을 시작하기 전의 다음 단계입니다. Azure 가상 컴퓨터에 대한 자세한 내용은 [가상 컴퓨터 설명서](https://azure.microsoft.com/documentation/services/virtual-machines/)를 참조하세요.
...
| --- | --- | --- |
| 응용 프로그램 일관성 |예 |다음을 보장하므로 Microsoft 워크로드에 이상적인 일관성 유형입니다.<ol><li> VM이 *부팅*됩니다. <li>*손상이 없습니다*. <li>*데이터 손실*이 없습니다.<li> 데이터는 백업 시 VSS를 사용하는 응용 프로그램을 포함하여 데이터를 사용하는 응용 프로그램과 일치됩니다.</ol> 대부분의 Microsoft 워크로드에는 데이터 일관성과 관련된 워크로드 관련 동작을 수행하는 VSS 작성자가 있습니다. 예를 들어 Microsoft SQL Server에는 트랜잭션 로그 파일에 대한 쓰기를 확인하고 데이터베이스가 제대로 수행되었는지 확인하는 VSS 작성자가 있습니다.<br><br> Azure VM 백업의 경우, 응용 프로그램에 일관된 복구 지점이 있다는 것은 백업 확장이 VSS 워크플로를 적용하고 VM 스냅숏을 촬영하기 전에 *제대로* 완료할 수 있었음을 의미합니다. 기본적으로 Azure VM에서 모든 응용 프로그램의 VSS 작성자도 적용되었음을 의미합니다.<br><br>[VSS 기본 사항](http://blogs.technet.com/b/josebda/archive/2007/10/10/the-basics-of-the-volume-shadow-copy-service-vss.aspx)(영문) 및 [작동 방법](https://technet.microsoft.com/library/cc785914%28v=ws.10%29.aspx)(영문)에서 자세히 알아 보세요. |
...
백업 데이터가 저장소 계정에서 복사되면 저장소 계정의 초당 입력/출력 작업(IOPS) 및 송신(또는 처리량) 메트릭에 가산됩니다. 이와 동시에 가상 컴퓨터가 실행되고 IOPS와 처리량을 사용합니다. 목표는 총 트래픽(백업 및 가상 컴퓨터)이 저장소 계정 제한을 초과하지 않도록 하는 것입니다.

### <a name="number-of-disks"></a>디스크 수
백업 프로세스에서는 백업 작업을 가능한 한 빨리 완료하려고 합니다. 이렇게 하면 가능한 많은 리소스를 소모합니다. 그러나 모든 I/O 작업은 초당 60MB의 제한이 있는 *단일 Blob의 목표 처리량*에 의해 제한됩니다. 해당 속도를 최대화하기 위해 백업 프로세스에서는 각 VM의 디스크를 *병렬로*백업하려고 합니다. 따라서 VM에&4;개의 디스크가 있는 경우, Azure 백업은&4;개의 디스크를 모두 병렬로 백업하게 됩니다. 그렇기 때문에 고객 저장소 계정이 있는 백업 트래픽을 결정하는 가장 중요한 요소는 고객 저장소 계정에서 백업된 **디스크 수** 입니다.
...
백업은 자격 증명 모음에 스냅숏 만들기 및 스냅숏 전송과 같은 두 단계로 구성됩니다. 자격 증명 모음에 데이터를 전송하는 두 번째 단계에서 백업에 사용할 저장소를 최적화하려면 이전 스냅숏에서 증분 변경 내용을 전송하면 됩니다. 이를 위해 블록의 체크섬을 계산하고 블록이 변경되는 경우 자격 증명 모음에 이 블록을 보내도록 식별할 수 있습니다. 여기에서는 다시 블록을 추가로 드릴다운하여 데이터 전송량을 최소화할 수 있는지 확인하고 변경된 모든 블록을 병합하고 자격 증명 모음에 보냅니다. 일부 레거시 응용 프로그램의 경우 응용 프로그램에 의한 쓰기가 작고 조각난 쓰기이기 때문에 저장소에 대해 최적이 아닙니다. 따라서 이러한 응용 프로그램에서 작성된 데이터를 처리하는 추가 시간을 사용해야 합니다. VM 내에서 실행되는 응용 프로그램의 경우 Azure에서 권장되는 응용 프로그램 쓰기 블록은 최소 8KB입니다. 응용 프로그램이 8KB보다 작은 블록을 사용하는 경우 백업 성능은 Azure에서 권장하는 것보다 많은 영향을 받습니다. [Azure Storage에서 최적의 성능을 위한 응용 프로그램 조정](../storage/storage-premium-storage-performance.md)을 살펴보고 백업 성능을 향상시키기 위한 최적의 방식으로 작성하도록 응용 프로그램을 조정할 수 있는지 확인하겠습니다. 문서에서 Premium storage에 대해 설명하지만 표준 저장소에서 실행되는 디스크에도 적용됩니다.
...
## <a name="how-are-protected-instances-calculated"></a>보호된 인스턴스를 계산하는 방법
