...
관리되지 않는 디스크를 사용하여 가상 머신에 대한 백업을 구성하는 동안 다음 사례를 따르는 것이 좋습니다.

> [!Note]
> 저장소 계정의 변경 또는 관리를 권장하는 다음 사례은 관리되지 않는 디스크를 사용하는 VM에만 적용됩니다. Managed Disks를 사용하는 경우 Azure는 저장소와 관련된 모든 관리 작업을 관리합니다.
> 

* 동일한 클라우드 서비스에서 10개가 넘는 클래식 VM을 동시에 백업하도록 예약하지 마세요. 동일한 클라우드 서비스에서 여러 VM을 백업하려면 백업 시작 시간에 한 시간의 시차를 두는 것이 좋습니다.
* 100개 이상의 VM을 단일 자격 증명 모음에서 동시에 백업하도록 예약하지 마세요. 
