...
* MABS는 MABS 서버와 동일한 도메인 또는 자식 도메인 및 트러스트 된 도메인에서 작업을 보호할 수 있습니다. NTLM 또는 인증서 인증을 사용 하 여 작업 그룹 및 신뢰할 수 없는 도메인에서 다음과 같은 작업을 보호할 수도 있습니다.
  ...
  * Hyper-V
  ...
  이러한 작업은 단일 서버 또는 클러스터 구성에서 실행할 수 있음 트러스트 된 도메인에 없는 작업을 보호 하려면 지원 되는 항목 및 필요한 인증에 대 한 정확한 내용은 [작업 그룹 및 신뢰할 수 없는 도메인의 컴퓨터 준비](/system-center/dpm/prepare-environment-for-dpm) 를 참조 하세요.
