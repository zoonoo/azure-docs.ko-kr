...
1. **공유 복원**을 선택 하면 **복원** 창이 열립니다. 복원 작업을 수행 하는 데 사용할 복원 지점을 선택 하려면 **복원 지점** 텍스트 상자 아래에서 링크 **선택** 텍스트를 선택 합니다.

    ![선택을 선택 하 여 복원 지점을 선택 합니다.](./media/restore-afs/select-restore-point.png)
...
1. **복원 지점 컨텍스트 선택** 창이 오른쪽에 열리고 선택한 파일 공유에 사용할 수 있는 복원 지점이 나열 됩니다. 복원 작업을 수행 하는 데 사용할 복원 지점을 선택 하 고 **확인**을 선택 합니다.
    ...
    >[!NOTE]
    >기본적으로 **복원 지점 선택** 창에는 지난 30 일 동안의 복원 지점이 나열 됩니다. 특정 기간 동안 만들어진 복원 지점에 대해 확인 하려면 적절 한 **시작 시간과** **종료 시간** 을 선택 하 여 범위를 지정 하 고 **새로 고침** 단추를 선택 합니다.
...
1. **파일 복구**를 선택 하면 **복원** 창이 열립니다. 복원 작업을 수행 하는 데 사용할 복원 지점을 선택 하려면 **복원 지점** 텍스트 상자 아래에서 링크 **선택** 텍스트를 선택 합니다.

    ![선택 링크를 선택 하 여 복원 지점을 선택 합니다.](./media/restore-afs/select-restore-point.png)
...
1. 복원 하려는 파일 또는 폴더를 선택 하려면 **파일 추가** 단추를 선택 합니다. 그러면 오른쪽에 있는 상황에 맞는 창이 열리고 복원 하도록 선택한 파일 공유 복구 지점의 콘텐츠가 표시 됩니다.
...
1. **폴더 이름** 상자에서 복원 된 내용이 포함 된 대상 파일 공유에 만들려는 폴더 이름을 지정 합니다.
1. 충돌이 발생 했을 때 건너뛸지 아니면 덮어쓸지를 선택 합니다.
1. 복원 하려는 파일 또는 폴더를 선택 하려면 **파일 추가** 단추를 선택 합니다. 그러면 복원 하도록 선택한 파일 공유 복구 지점의 내용이 표시 되는 상황에 맞는 창이 열립니다.
