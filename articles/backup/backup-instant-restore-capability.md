...
기본적으로 스냅숏은 2 일 동안 보존 됩니다. 이 기능을 사용하면 복원 시간을 줄여 이러한 스냅숏에서 복원 작업을 수행할 수 있습니다. Recovery Services 데이터에서 관리 되는 디스크 만들어지고, 변환 및 사용자가 관리 되는 디스크에 대 한 관리 되지 않는 디스크 시나리오에 대 한 사용자의 저장소 계정 자격 증명 모음에서 다시 데이터를 복사 하는 데 필요한 시간이 줄어듭니다.
...
모든 사용자를 모두 **공용 지역** 인스턴트 복원을 수행 하려면 업그레이드 되었습니다.
