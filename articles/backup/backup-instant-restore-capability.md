...
* 7일 동안 디스크와 함께 스냅숏을 보존하여 백업 및 복원 시간을 줄입니다.
...
또는 자격 증명 모음의 **속성** 페이지로 이동하여 **VM 백업 스택** 아래에 **업그레이드** 옵션을 표시할 수 있습니다.
...
새 모델에서는 스냅숏(계층 1)을 삭제하지 않은 경우 복원 지점(계층 2) 삭제를 허용하지 않습니다. 현재는 스냅숏(계층 1) 삭제에 대해 7일의 보존 기간을 지원하므로 7일 미만의 복원 지점(계층 2) 보존 기간은 적용되지 않습니다. 복원 지점(계층 2) 보존 기간을 7일 이상으로 예약하는 것이 좋습니다.
...
복구 지점에 스냅숏이 있고 사용 가능한 최신 RP인 경우 다음에 성공한 백업이 있을 때까지 보존됩니다. 이는 VM의 문제로 인해 모든 백업이 실패하는 경우에도 항상 하나 이상의 최신 RP가 있도록 요구하는 현재 설계된 GC 정책을 따른 것입니다. 정상적인 시나리오에서는 만료 후 최대 48시간 이내에 RP가 정리됩니다.
