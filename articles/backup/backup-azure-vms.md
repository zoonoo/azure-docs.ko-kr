...
4. **등록 항목** 팝업에서 등록하려는 가상 컴퓨터를 선택합니다. 동일한 이름을 가진 가상 컴퓨터가 두 개 이상 있는 경우 클라우드 서비스를 사용하여 가상 컴퓨터를 구분합니다.
...
4. 이렇게 하면 보호할 가상 컴퓨터를 선택할 수 있는 **항목 보호** 마법사를 불러 옵니다. 동일한 이름을 가진 가상 컴퓨터가 두 개 이상 있는 경우 클라우드 서비스를 사용하여 가상 컴퓨터를 구분합니다.
...
| 백업 | 스냅숏 VM 하위 작업이 시간 초과되었습니다. 몇 분 후에 작업을 다시 시도하세요. 문제가 지속되면 Microsoft 지원에 문의하세요. | 이 오류는 VM 에이전트에 문제가 있거나 Azure 인프라에 대한 네트워크 액세스가 어떤 방식으로든 차단된 경우에 발생합니다. <ul><li>[VM 에이전트 문제 디버그](#Troubleshooting-vm-agent-related-issues)에 대한 자세한 정보 <li>[네트워킹 문제 디버그](#troubleshooting-networking-issues)에 대한 자세한 정보 </ul> |
