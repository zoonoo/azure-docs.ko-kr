...
Backup Server v2를 Windows Server 2016 또는 Windows Server 2012 R2에 설치할 수 있습니다. System Center 2016 Data Protection Manager Modern Backup Storage 같은 새로운 기능을 활용하려면 Backup Server v2를 Windows Server 2016에 설치해야 합니다. Backup Server v2로 업그레이드하거나 이 버전을 설치하기 전에 [installation prerequisites](https://docs.microsoft.com/system-center/dpm/install-dpm#setup-prerequisites)(설치 필수 구성 요소)를 읽어 보세요.

> [!NOTE]
> Azure Backup Server에는 System Center Data Protection Manager와 동일한 코드베이스가 있습니다. Backup Server v1은 Data Protection Manager 2012 R2에 해당합니다. Backup Server v2는 Data Protection Manager 2016에 해당합니다. 이 문서에서는 때때로 Data Protection Manager 설명서를 참조합니다.
>
>
...
백업 저장소 효율성을 개선하기 위해 Backup Server v2에서는 볼륨에 대한 지원을 추가합니다. Backup Server v1 및 Backup Server v2는 모두 디스크를 지원합니다.
...
볼륨을 Backup Server에 추가할 때 볼륨에 이름을 지정할 수 있습니다. 이름을 지정할 볼륨의 **식별 이름** 열을 선택합니다. 필요한 경우 나중에 이름을 변경할 수 있습니다. PowerShell을 사용하여 볼륨의 이름을 추가하거나 변경할 수도 있습니다.
...
Backup Server의 저장소 풀에서 볼륨 속성을 업데이트하려면 PowerShell cmdlet **Update-DPMDiskStorage**를 사용합니다.
...
5. **그룹 구성원 선택** 페이지의 **사용 가능한 구성원** 상자에서 보호 에이전트가 있는 구성원이 나열됩니다. 이 예제에서는 볼륨 D:\ 및 E:\,를 선택한 다음, **선택한 구성원**에 추가합니다. **다음**을 선택합니다.

  ![그룹 구성원 선택 페이지](./media/backup-mabs-upgrade-to-v2/create-a-protection-group-3.png)

6. **데이터 보호 방법 선택** 페이지에서 **보호 그룹 이름**을 입력하고, 보호 방법을 선택하고 나서, **다음**을 선택합니다. 단기 보호가 필요하면 **디스크** 백업 방법을 선택합니다.
...
8. **디스크 저장소 할당 검토** 페이지에서 데이터 원본 크기, 프로비전할 공간에 대한 값 및 대상 저장소 볼륨을 포함한 선택한 데이터 원본에 대한 세부 정보를 검토합니다.
...
Backup Server v2로 업그레이드하거나 설치한 다음, 운영 체제를 Windows Server 2016으로 업그레이드한 후 Modern Backup Storage를 사용하도록 보호 그룹을 업데이트합니다. 기본적으로 보호 그룹은 변경되지 않습니다. 보호 그룹은 처음에 설정된 대로 계속 작동합니다. 
...
3. 사용 가능한 디스크 목록에서 추가할 디스크를 선택합니다. **추가**를 선택한 다음, **확인**을 선택합니다.
...
다음 섹션에서는 연결된 클라이언트 컴퓨터 및 연결되지 않은 클라이언트 컴퓨터에 대한 보호 에이전트를 업데이트하는 방법을 설명합니다.
...
2. 표시 창에서 보호 에이전트를 업데이트할 클라이언트 컴퓨터를 선택합니다.
  ...
  > [!NOTE]
  > **에이전트 업데이트** 열에는 각 보호된 컴퓨터에 보호 에이전트 업데이트를 사용할 수 있는 시기가 표시됩니다. 업데이트가 사용 가능한 경우 외에는 보호된 컴퓨터가 선택된 경우 **작업** 창에서 **업데이트** 작업을 사용할 수 없습니다.
...
4. 네트워크에 연결되지 않은 클라이언트 컴퓨터의 경우 컴퓨터가 네트워크에 연결될 때까지 **에이전트 상태** 열에는 **업데이트 보류 중** 상태가 표시됩니다.
  ...
  클라이언트 컴퓨터가 네트워크에 연결된 경우 클라이언트 컴퓨터에 대한 **에이전트 업데이트** 열에는 **업데이트 중** 상태가 표시됩니다.
  
### <a name="move-legacy-protection-groups-from-the-old-version-and-sync-the-new-version-with-azure"></a>이전 버전에서 레거시 보호 그룹 이동 및 새 버전과 Azure 동기화

Azure Backup Server와 OS가 모두 업데이트되면 Modern Backup Storage를 사용하여 새 데이터 원본을 보호할 준비가 된 것입니다. 이미 보호되는 데이터 원본은 Azure Backup Server(레거시)에 있으므로 계속 보호됩니다. 새 보호 그룹은 모두 Modern Backup Storage를 사용합니다.

보호의 레거시 모드에서 Modern Backup Storage로 데이터 원본을 마이그레이션하려면 다음을 수행합니다.

1.  Data Protection Manager 저장소 풀에 새 볼륨을 추가합니다. 또한 식별 이름을 할당하고 데이터 원본 태그를 선택할 수 있습니다.

2. 레거시 모드의 각 데이터 원본의 경우 데이터 원본의 보호를 중지합니다. 그런 다음, **보호된 데이터 보존**을 선택합니다.  이렇게 하면 마이그레이션 후 이전 복구 지점을 복구할 수 있게 됩니다.

3. 새 보호 그룹을 만듭니다. 그런 다음, 새 형식을 사용하여 저장하려는 데이터 원본을 선택합니다.

  Data Protection Manager는 레거시 백업 저장소에서 Modern Backup Storage 볼륨으로 로컬로 복제본 복사를 저장합니다.
    > [!NOTE] 
    > 복사본을 만들면 복구 후 작업으로 표시됩니다.

  그런 다음, 모든 새로운 동기화 및 복구 지점이 Modern Backup Storage에 저장됩니다.

  이전 복구 지점은 만료되는 대로 정리됩니다. 이전 복구 지점이 삭제되면 디스크 공간이 확보됩니다.

  모든 레거시 볼륨이 이전 저장소에서 삭제되면 Azure Backup에서 디스크를 제거할 수 있습니다. 그런 다음, 시스템에서 디스크를 제거할 수 있습니다.

4. Data Protection Manager 데이터베이스의 백업을 만듭니다.
...
5. 원래 Azure Backup Server 인스턴스를 종료하거나 서버를 오프라인으로 전환합니다.

6. Active Directory에서 머신 계정을 다시 설정합니다.

7. 새 머신에서 Windows Server 2016을 설치합니다. 서버 이름의 경우 원래 Azure Backup Server 인스턴스와 같은 머신 이름을 사용합니다.

8. 도메인에 조인합니다.

9. Azure Backup Server v2를 설치합니다. (이전 서버에서 Data Protection Manager 저장소 풀 디스크를 제거하고 새 서버로 가져옵니다.)

10. 4단계에서 만든 Data Protection Manager 데이터베이스를 복원합니다.

11. 저장소를 원래 백업 서버에서 새 서버로 연결합니다.

12. SQL Server에서 Data Protection Manager 데이터베이스를 복원합니다.

13. 새 서버의 관리자 명령 줄을 사용에서 `cd`를 사용하여 Microsoft Azure Backup 설치 위치 및 bin 폴더로 이동합니다.  

  예제:  
  C:\windows\system32>cd "c:\Program Files\Microsoft Azure Backup\DPM\DPM\bin\에서 Azure Backup

14. `DPMSYNC -SYNC`을 실행합니다.
  
  > [!NOTE]
  > 이전 버전을 이동하는 대신 *새* 디스크를 Data Protection Manager 저장소 풀에 추가한 경우 `DPMSYNC -Reallocatereplica`를 실행합니다.

## <a name="new-powershell-cmdlets-in-backup-server-v2"></a>Backup Server v2의 새로운 PowerShell cmdlet
