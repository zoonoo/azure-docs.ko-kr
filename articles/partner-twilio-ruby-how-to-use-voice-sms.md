<properties urlDisplayName="Twilio Voice/SMS Service" pageTitle="음성 및 SMS에 Twilio 사용(Ruby) - Azure" metaKeywords="Azure Ruby Twilio, Azure 전화 통화, Azure 전화 통화, Azure twilio, Azure SMS, Azure SMS, Azure 음성 통화, azure 음성 통화, Azure 문자 메시지, Azure 문자 메시지" description="Learn how to make a phone call and send a SMS message with the Twilio API service on Azure. Code samples written in Ruby." metaCanonical="" services="" documentationCenter="Ruby" title="How to Use Twilio for Voice and SMS Capabilities in PHP" authors="MicrosoftHelp@twilio.com" solutions="" manager="twilio" editor="" />
...
## <a id="Pricing"></a>Twilio 가격 책정 및 특별 행사
...
먼저, 새 Ruby 웹 응용 프로그램의 호스트 역할을 할 [새 Azure Linux VM을 설정합니다][azure_vm_setup](영문). Rails 앱을 만드는 것과 관련된 단계는 무시하고 VM 설정만 수행하세요. 외부 포트 80과 내부 포트 5000으로 끝점을 만들어야 합니다.
...
브라우저에서 `http://yourdomain.cloudapp.net/make_call`을 열면 Twilio API에 대한 호출이 트리거되어 전화 통화가 걸립니다. `client.account.calls.create`의 첫 번째 두 매개 변수 즉, 통화가 `from`(걸려오는) 번호 및 통화가 `to`(걸리는 대상) 번호는 따로 설명이 필요 없습니다. 
...
* [Twilio 사용 방법 및 예제 코드(영문)] [twilio_howtos]
