...
특정 계정 또는 구독에 대해 구매 주문 번호를 입력할 수도 있습니다. 보고서는 기본적으로 계층에 설정된 최하위 구매 주문 번호를 사용합니다. 즉, 구독 구매 주문 번호를 입력하지 않으면 계정 구매 주문 번호가 사용 되고, 계정 구매 주문을 입력하지 않으면 부서 구매 주문이 사용됩니다.
...
**시작 날짜가 2018년 5월 1일 이전인 직접 등록**: 직접 EA(Enterprise Azure) 고객은 Azure 서비스에 대한 연간 청구 기간으로 설정됩니다(Marketplace 서비스 제외). 청구 기간은 연주기일을 기준으로 합니다. 연주기일은 계약이 발효된 날짜입니다. 첫 번째 Azure Service 초과분 청구서를 받으려면 MC('현금 약정 금액') 임계값의 150%를 초과해야 합니다.  총 서비스 사용량이 MC 임계값의 150%를 초과하면 연주기일을 기준으로 분기별 청구 기간으로 자동 변환됩니다.  MC 임계값의 150%를 초과하지 않으면 등록은 연간 청구 기간으로 유지되고, 초과분 청구서는 약정 연도 종료 시 발송됩니다.

**시작 날짜가 2018년 5월 1일 이후인 직접 등록**: 고객의 Azure 사용량 및 별도 청구 사용량 청구서는 월별 청구 기간을 기준으로 합니다.  Azure 현금 약정 금액에 포함되지 않는 요금은 초과분으로 결제해야 합니다.  

**2018년 5월 1일 이전에 시작된 등록을 사용하여 간접 등록**: 간접 EA(Enterprise Azure) 고객은 분기별 청구 기간으로 설정됩니다.  CP(채널 파트너)는 고객에게 직접 요금을 청구하게 됩니다.  

**시작 날짜가 2018년 5월 1일 이후인 간접 등록**: 시작 날짜가 2018년 5월 1일 이후인 간접 계약은 매월 요금이 청구됩니다.  
...
두 개의 개별 청구서가 단일 청구서로 결합되었기 때문에 2019년 4월 청구 기간부터는 고객에게 단일 Azure 청구서가 발송됩니다. (이 변경 내용은 호주, 일본 또는 싱가포르 고객에게는 영향을 주지 않습니다.) 통합 청구서로 전환하는 과정에는 부분 Marketplace 청구서를 받게 됩니다. 이 마지막 개별 청구서에는 청구 업데이트 날짜 이전의 Marketplace 요금이 표시됩니다. 해당 날짜 이후의 Marketplace 요금은 Azure 청구서에 표시됩니다. 전환 기간이 지나면 모든 Azure 및 Marketplace 요금이 통합 청구서에 표시됩니다.  
...
1. 이 줄에는 등록에 적용된 크레딧과 간단한 설명이 표시됩니다. 예를 들어 서비스 수준 계약 크레딧이라는 설명이 표시될 수 있습니다.
...
Excel은 모든 필드를 '일반' 텍스트로 가져오며, 숫자는 수학적 표준: "1,000.00"에서 구분되는 것으로 가정합니다.  따라서 천 단위 구분 기호로 마침표(.)를 사용하고 소수 자릿수 구분 기호로 쉼표(,)를 사용하는 유럽 통화 "1.000,00"은 올바르게 표시되지 않습니다. 이는 영역별 언어 설정에 따라 달라질 수 있습니다.
...
- **월 요금제:** 사용량/액세스를 기준으로 매월 요금이 청구됩니다.
...
새 계측 방법은 2016년 1월 27일~28일 사이에 적용됩니다. 이 전환 기간 동안 배포용 .csv 다운로드에는 다음과 같은 두 개의 품목이 표시됩니다. 하나는 A1 미터에서 내보낸 사용량을 알리는 품목이고, 다른 하나는 배포 크기에 해당하는 새 전용 미터에서 내보낸 사용량을 알리는 항목입니다.
...
Azure 마켓플레이스에 제공되는 일부 타사 재판매인 서비스는 이제 EA(기업계약) 현금 약정 금액 잔액을 사용합니다. 이전에는 이러한 서비스가 EA 현금 약정 금액과 별도로 청구되었으며 별도의 청구서가 발송되었습니다. Marketplace에서 이러한 서비스에 EA 현금 약정 금액을 사용함에 따라 고객 구매 및 결제 관리를 간소화할 수 있습니다. 현재 현금 약정 금액을 사용하는 전체 서비스 목록은 [Azure 웹 사이트](https://azure.microsoft.com/updates/azure-marketplace-third-party-reseller-services-now-use-azure-monetary-commitment/?WT.mc_id=azurebg_email_Trans_33771_1695_Release_Direct_Tier2_March14)를 참조하세요.
...
- **사용량 기준:** 제품은 트랜잭션 단위로 측정됩니다.  예를 들어 가상 머신은 시간 단위로 요금이 청구되고, Bing API 검색은 검색 건수를 기준으로 요금이 청구됩니다.
- **비-사용량 기준:** 사용량에 관계 없는 일회성 요금 또는 매달 반복되는 요금입니다.
...
- **범용**: "Microsoft Azure 엔터프라이즈" 콘텐츠 팩 평가판 제품("제안")의 확장된 Power BI Pro는 제품 기간 동안 기존의 적격 사용자가 사용할 수 있으며, 특정 Power BI 콘텐츠 팩을 사용하여 Microsoft Azure 사용과 관련된 특정 인사이트에 액세스할 수 있습니다.
- **자격**: EA(기업계약)에 포함되는 사용자는 조직의 Microsoft Azure 청구, 서비스, 서비스 및/또는 비용 관리와 관련된 직무를 갖고 있는 경우 제품에 참여할 수 있습니다.
- **제외**:
  ...
  - 기간: 제품은 2017년 6월 1일에 시작하여 2018년 5월 31일에 종료됩니다.  이 12개월 동안 언제든지 제품에 참여할 수 있지만, 제품에 참여한 시간에 관계없이 모든 사용자의 제품은 2018년 5월 31일에 종료됩니다.
...
리소스 태그는 Azure Portal에서 관리됩니다. [https://portal.azure.com](https://portal.azure.com)에서 Azure 구독 팀에 문의할 수 있습니다. 지원 요청을 신청하는 방법에 대한 [이 링크](https://docs.microsoft.com/azure/azure-supportability/how-to-create-azure-support-request)의 단계를 따르세요.
...
**총 원시 단위:** 자세한 사용량 보고서에 표시되는 소비된 수량입니다.
**단위당 MOCP 리소스:** 업스트림 사용 시스템은 각 서비스의 사용량을 서로 다른 단위로 내보냅니다. (사전 설정) **단위당 사용량:** EA 측정 단위입니다. (사전 설정) **가격:** Azure EA Portal의 단가입니다.
**총 비용:** 자세한 사용량 보고서의 확장된 비용이거나 Azure EA Portal의 약정 사용량 + 초과분입니다.
