...
자세한 사용 현황 CSV 파일은 청구 기간 및 일별 사용 현황별로 청구 요금을 보여 줍니다. 파일을 다운로드 하거나 보려면 [Azure 청구 청구서 및 일별 사용 현황 데이터 가져오기](billing-download-azure-invoice-daily-usage-date.md)를 참조 하세요.
...
청구서의 **사용 요금** 섹션에는 청구 기간 동안 사용 된 각 미터의 총 값이 표시 됩니다. 예를 들어 다음 이미지는 Azure Scheduler 서비스에 대 한 사용 요금을 보여 줍니다.
...
요금의 일별 분석 결과를 보려면 CSV 파일의 **일일 사용량** 섹션으로 이동 합니다. *미터 범주의* *스케줄러* 를 필터링 합니다. 미터가 사용된 날짜와 사용량을 확인할 수 있습니다. 비교를 위해 *리소스* 및 *리소스 그룹* 정보도 표시 됩니다. *사용* 된 값은 청구서에 표시 되는 값에 추가 되어야 합니다.
...
### <a name="option-2-compare-the-usage-and-costs-in-the-azure-portal"></a>옵션 2: Azure Portal 사용량 및 비용 비교
...
1. 요금을 표시 하는 행을 선택 하 여 일별 비용 분석을 표시 합니다.
...
Azure는 리소스 요금을 기준으로 직접 청구하지 않습니다. 리소스 요금은 하나 이상의 미터를 사용하여 계산됩니다. 미터는 수명 내내 리소스 사용량을 추적 하는 데 사용 됩니다. 이러한 미터는 청구서를 계산하는 데 사용됩니다.

예를 들어 가상 머신과 같은 단일 Azure 리소스를 만드는 경우 하나 이상의 미터 인스턴스가 생성됩니다. 미터는 시간 경과에 따른 리소스 사용량을 추적 하는 데 사용 됩니다. 각 미터는 Azure에서 청구서를 계산하는 데 사용하는 사용량 레코드를 내보냅니다.
...
VM을 만들 때 각 미터는 사용 레코드를 내보내기 시작 합니다. 이 사용량과 미터의 요금은 Azure 계량 시스템에서 추적됩니다.

## <a name="payment"></a>청구서 결제

신용 카드 카드를 지불 방법으로 설정 하면 청구 기간이 끝난 후 10 일 이내에 자동으로 지불 요금이 청구 됩니다. 신용 카드 명세서에는 **MSFT Azure** 항목으로 표시됩니다.

청구 되는 신용 카드를 변경 하려면 [Azure에 대 한 신용 카드 추가, 업데이트 또는 제거](billing-how-to-change-credit-card.md)를 참조 하세요.

[청구서 결제](billing-how-to-pay-by-invoice.md)의 경우 청구서 맨 아래에 기재된 위치로 결제 금액을 송금합니다.

지불 상태를 확인 하려면 [지원 티켓을 만듭니다](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade).
