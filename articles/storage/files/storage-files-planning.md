...
프리미엄 파일 공유 (미리 보기)는 고정된 GiB/P s/처리량 비율에 따라 프로 비전 됩니다. 공유에 프로비전되는 각 GiB당 IOPS 1개, 초당 0.1MiB의 처리량이 공유당 최대 한도까지 지급됩니다. 허용되는 최소 프로비저닝 용량은 100GiB(최소 IOPS/처리량)입니다. 공유 크기 모든 시간과 감소에서 언제 든 지 늘릴 수 있지만 마지막 증가 이후 24 시간을 줄일 수 있습니다.
...
모든 공유는 100 MiB 초당 100 개 이상의 IOPS 및 대상 처리량까지 버스트할 수 있습니다. 공유 1 GiB 단위로 프로 비전 되어야 합니다. 최소 크기가 100gib를 다음 크기 101 GIB 등.

> [!TIP]
> 기준 IOPS 100 + 1 = * GiB 프로 비전 합니다. (최대 최대 100,000 개의 IOPS).
>
...
|용량 (GiB) | 기준 IOPS | 버스트 한도 | 송신 (초) | 수신 (초) |
|---------|---------|---------|---------|---------|
...
크레딧은 IOPS 기준선 아래에 파일 공유에 대 한 트래픽을 때마다 버스트 버킷에 누적 됩니다. 예를 들어 100 GiB 공유 100 기준 IOPS에 있습니다. 공유에서 실제 트래픽을 특정 1 초 간격에 대 한 40 IOPS 되었으면 60 사용 되지 않는 IOPS 버스트 버킷에 크레딧 됩니다. 이 크레딧은 다음 됩니다 나중에 작업 기준 IOPs를 초과 하는 경우.

> [!TIP]
> 버스트 제한 버킷 크기인 Baseline_IOPS = * 2 * 3600 합니다.
...
- 크레딧 또는 기준 없습니다 중 하나는 0부터 남은 IOPS 사용 중입니다.

전체 해당 버스트 버킷의 학점 수를 사용 하 여 새 파일 공유 시작 합니다.
