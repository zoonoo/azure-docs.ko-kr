...
"먼저 storage-preview 확장을 추가한 다음 아래 예제의 코드를 실행해야 합니다. 예제의 자리 표시자 값은 실제 값으로 바꾸세요. Azure CLI 확장 추가에 대한 자세한 내용은 이 문서를 참조하세요.”
...
이제 SMB를 통한 Azure AD 인증을 올바르게 사용하도록 설정했으며, 파일 공유 액세스 권한을 제공하는 사용자 지정 역할을 Azure AD ID에 할당했습니다. 추가 사용자에게 파일 공유 액세스 권한을 부여하려면 2단계에 나와 있는 지침을 따르세요.
