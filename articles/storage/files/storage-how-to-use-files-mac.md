...
2. **"이동" 메뉴에서 "서버에 연결"을 선택**: 필수 구성 에서 UNC 경로를 사용하여 시작`\\`이중 `smb://` 백슬래시 ()`\`및 기타 모든 백슬래시 () 전달 슬래시 ()로`/`변환합니다. 링크는 다음과 같습니다. !["서버에 연결" 대화 상자](./media/storage-how-to-use-files-mac/mount-via-finder-2.png)
