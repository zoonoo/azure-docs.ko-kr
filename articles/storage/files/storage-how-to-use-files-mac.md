...
2. **"이동" 메뉴에서 "서버에 연결"을 선택**합니다. 필수 구성 요소에서 UNC 경로를 사용 하 여 시작 이중 백슬래시 (`\\`)를 `smb://`으로 변환 하 고 다른 모든 백슬래시 (`\`)는 슬래시 (`/`)를 전달 합니다. 링크는 다음과 같습니다. !["서버에 연결" 대화 상자](./media/storage-how-to-use-files-mac/mount-via-finder-2.png)
