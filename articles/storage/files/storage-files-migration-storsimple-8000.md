...
### <a name="expose-the-storsimple-8020-volumes-to-the-vm"></a>VM에 StorSimple 8020 볼륨 노출
...
1. [클라우드 어플라이언스에 대 한 개인 IP 가져오기](../../storsimple/storsimple-8000-cloud-appliance-u2.md#get-private-ip-for-the-cloud-appliance)
...
DFS를 배포 하는 경우 DFN 네임 스페이스를 새 서버 폴더 위치로 가리킬 수 있습니다. DFS (제어 되) 배포가 없고 8100 8600 어플라이언스를 Windows 서버와 로컬로 연결 하는 경우 해당 서버를 도메인에서 사용 하지 않고 도메인에 AFS로 새 Windows Server를 도메인에 가입 하 고 이전 서버와 동일한 서버 이름을 지정 합니다. 및 동일한 공유 이름을 통해 새 서버로의 이동은 사용자, 그룹 정책 또는 스크립트에 대해 투명 하 게 유지 됩니다.
