...
Azure 파일 공유 (미리 보기)에 대해 온-프레미스 Active Directory Domain Services 인증을 사용 하도록 설정 하는 방법을 알아보려면 [azure 파일 공유에 대해 SMB를 통한 온-프레미스 Active Directory Domain Services 인증 사용](storage-files-identity-auth-active-directory-enable.md)을 참조 하세요.
...
- **온-프레미스 Active Directory Domain Services (AD DS)**
    ...
    온-프레미스 Active Directory Domain Services (AD DS)와 Azure Files (미리 보기)를 통합 하면 디렉터리 데이터를 저장 하는 동안 네트워크 사용자 및 관리자가 사용할 수 있도록 하는 방법을 제공 합니다. 보안은 디렉터리의 개체에 대 한 로그온 인증 및 액세스 제어를 통해 AD DS와 통합 됩니다. 관리자는 단일 네트워크 로그온을 통해 네트워크에서 디렉터리 데이터 및 조직을 관리할 수 있으며, 권한 있는 네트워크 사용자는 네트워크의 모든 위치에서 리소스에 액세스할 수 있습니다. AD DS은 일반적으로 온-프레미스 환경에서 기업에 의해 채택 되 고 AD DS 자격 증명은 액세스 제어를 위한 id로 사용 됩니다. 자세한 내용은 [Active Directory Domain Services 개요](https://docs.microsoft.com/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview)를 참조 하세요.
...
다음 표에는 Azure AD DS 및 온-프레미스 AD DS (미리 보기)에 대해 지원 되는 Azure 파일 공유 인증 시나리오가 요약 되어 있습니다. Azure Files와 통합 하기 위해 클라이언트 환경에 채택 된 도메인 서비스를 선택 하는 것이 좋습니다. 온-프레미스 또는 온-프레미스에 장치가 도메인에 가입 되어 있는 AD DS (미리 보기)를 이미 설치한 경우 Azure 파일 공유 인증에 대해 AD DS (미리 보기)를 활용 하도록 선택 해야 합니다. 마찬가지로 GA (Azure AD DS)를 이미 채택한 경우 Azure 파일 공유 인증에이를 사용 해야 합니다.


|Azure AD DS 인증  | 온-프레미스 AD DS (미리 보기) 인증  |
|---------|---------|
|Azure AD DS에 가입 된 Windows 컴퓨터는 SMB를 통해 Azure AD 자격 증명을 사용 하 여 Azure 파일 공유에 액세스할 수 있습니다.     |온-프레미스 AD DS에 가입 된 Windows 컴퓨터는 SMB를 통해 Azure AD에 동기화 되는 온-프레미스 Active Directory 자격 증명을 사용 하 여 Azure 파일 공유에 액세스할 수 있습니다.         |

### <a name="unsupported-scenarios"></a>지원되지 않는 시나리오

- Azure AD DS와 온-프레미스 AD DS 인증은 컴퓨터 계정에 대 한 인증을 지원 하지 않습니다. 대신 서비스 로그온 계정을 사용 하는 것을 고려할 수 있습니다.
- Azure AD DS 인증은 Azure AD에 가입 된 장치에 대 한 인증을 지원 하지 않습니다.
...
Azure 파일 공유는 Azure AD DS 또는 온-프레미스 AD DS (미리 보기)와 통합 하기 위해 Kerberos 인증을 지원 합니다. Azure 파일 공유에서 인증을 사용 하도록 설정 하려면 먼저 도메인 환경을 설정 해야 합니다. Azure AD DS 인증의 경우 파일 데이터에 액세스 하려는 Vm에 대 한 Azure AD Domain Services 및 도메인 가입을 사용 하도록 설정 해야 합니다. 도메인에 가입 된 VM은 Azure AD DS와 동일한 VNET (가상 네트워크)에 상주해 야 합니다. 마찬가지로 온-프레미스 AD DS (미리 보기) 인증의 경우 도메인 컨트롤러를 설정 하 고 컴퓨터 또는 Vm을 도메인에 가입 시켜야 합니다.

VM에서 실행 되는 응용 프로그램과 연결 된 id가 Azure 파일 공유의 데이터에 액세스 하려고 하면 요청이 Azure AD DS에 전송 되어 id를 인증 합니다. 인증에 성공 하면 Azure AD DS는 Kerberos 토큰을 반환 합니다. 응용 프로그램은 Kerberos 토큰을 포함 하는 요청을 보내고, Azure 파일 공유는 해당 토큰을 사용 하 여 요청에 권한을 부여 합니다. Azure 파일 공유는 토큰만 수신 하 고 Azure AD DS 자격 증명을 유지 하지 않습니다. 온-프레미스 AD DS 인증은 AD DS Kerberos 토큰을 제공 하는 비슷한 방식으로 작동 합니다.
...
새 및 기존 저장소 계정의 Azure 파일 공유에 대해 Azure AD DS 또는 온-프레미스 AD DS (미리 보기)를 사용 하 여 id 기반 인증을 사용 하도록 설정할 수 있습니다. 저장소 계정의 파일 액세스 인증에는 하나의 도메인 서비스만 사용할 수 있으며,이는 계정의 모든 파일 공유에 적용 됩니다. Azure AD DS을 사용 하 여 인증을 위한 파일 공유 설정에 대 한 자세한 지침은 다른 문서에서 온-프레미스 AD DS (미리 보기)에 대 한 [Azure Active Directory Domain Services Azure Files 인증 사용](storage-files-identity-auth-active-directory-domain-service-enable.md) 에 대 한 지침을 참조 하세요. [azure 파일 공유에 대해 SMB를 통한 온-프레미스 Active Directory Domain Services 인증을 사용 하도록](storage-files-identity-auth-active-directory-enable.md)설정 합니다.

### <a name="configure-share-level-permissions-for-azure-files"></a>Azure Files에 대한 공유 수준 권한 구성

Azure AD DS 또는 온-프레미스 AD DS (미리 보기) 인증을 사용 하는 경우 기본 제공 RBAC 역할을 사용 하거나 Azure AD id에 대 한 사용자 지정 역할을 구성 하 고 저장소 계정의 모든 파일 공유에 대 한 액세스 권한을 할당할 수 있습니다. 할당 된 사용 권한을 통해 부여 된 id는 루트 디렉터리 뿐만 아니라 공유에 대해서만 액세스 권한을 얻을 수 있습니다. Azure 파일 공유에 대 한 디렉터리 또는 파일 수준 권한을 별도로 구성 해야 합니다.
