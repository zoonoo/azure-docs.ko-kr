...
이 문서에서는 Windows 클라이언트에서 연결할 때 Microsoft Azure Files와 관련하여 발생하는 일반적인 문제를 보여 줍니다. 또한 이러한 문제의 가능한 원인과 해결 방법을 제공합니다. 이 문서의 문제 해결 단계 외에도 [AzFileDiagnostics를](https://gallery.technet.microsoft.com/Troubleshooting-tool-for-a9fa1fe5) 사용하여 Windows 클라이언트 환경에 올바른 필수 구성 조건이 있는지 확인할 수도 있습니다. AzFileDiagnostics는 이 문서에서 설명하는 대부분의 현상을 자동으로 감지하고 최적의 성능을 얻도록 환경을 설정하는 데 도움이 됩니다. 이 정보는 Azure Files 공유 연결/매핑/탑재 관련 문제에 도움이 되는 단계를 제공하는 [Azure Files 공유 문제 해결사](https://support.microsoft.com/help/4022301/troubleshooter-for-azure-files-shares)에서도 찾을 수 있습니다.
...
### <a name="cause-3-share-level-permissions-are-incorrect-when-using-identity-based-authentication"></a>원인 3: ID 기반 인증을 사용할 때 공유 수준 사용 권한이 올바르지 않습니다.

사용자가 AD(Active Directory) 또는 Azure Active Directory 도메인 서비스(Azure AD DS) 인증을 사용하여 Azure 파일 공유에 액세스하는 경우 공유 수준 사용 권한이 올바르지 않은 경우 "액세스가 거부됨" 오류로 파일 공유에 대한 액세스가 실패합니다. 
...
### <a name="cause-1-port-445-is-blocked"></a>원인 1: 포트 445가 차단됨
...
cmdlet을 `Test-NetConnection` 사용하려면 Azure PowerShell 모듈을 설치해야 하며 자세한 내용은 [Azure PowerShell 설치 모듈을](/powershell/azure/install-Az-ps) 참조하십시오. 잊지 말고 `<your-storage-account-name>` 및 `<your-resource-group-name>`을 스토리지 계정과 관련된 이름으로 바꿔야 합니다.
...
Azure 파일 동기화는 온-프레미스 Windows Server를 Azure 파일 공유의 빠른 캐시로 변환할 수 있습니다. SMB, NFS 및 FTPS를 포함하여 로컬로 데이터에 액세스하기 위해 Windows Server에서 사용할 수 있는 모든 프로토콜을 사용할 수 있습니다. Azure File Sync는 포트 443에서 작동하므로 포트 445가 차단된 클라이언트에서 Azure 파일에 액세스하는 해결 방법을 사용할 수 있습니다. [Azure 파일 동기화를 설정하는 방법에 대해 알아봅니다.](https://docs.microsoft.com/azure/storage/files/storage-sync-files-extend-servers)

#### <a name="solution-2---use-vpn"></a>해결 방법 2 - VPN 사용
특정 저장소 계정에 VPN을 설정하면 트래픽이 인터넷을 통해 가아닌 보안 터널을 통과합니다. Windows에서 Azure 파일에 [액세스하려면 VPN을 설정하는 방법에](storage-files-configure-p2s-vpn-windows.md) 따라 설명합니다.

#### <a name="solution-3---unblock-port-445-with-help-of-your-ispit-admin"></a>해결 방법 3 - ISP/IT 관리자의 도움을 받아 포트 445의 차단 해제
IT 부서 또는 ISP와 협력하여 Azure IP 범위에 대한 포트 445 [아웃바운드를 엽니다.](https://www.microsoft.com/download/details.aspx?id=41653)
...
Azure 파일은 SMB 외에도 REST도 지원합니다. REST 액세스는 포트 443(표준 tcp)에서 작동합니다. 풍부한 UI 환경을 활성화하는 REST API를 사용하여 작성된 다양한 도구가 있습니다. [저장소 탐색기는](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer?tabs=windows) 그 중 하나입니다. [저장소 탐색기를 다운로드 및 설치하고](https://azure.microsoft.com/features/storage-explorer/) Azure Files에서 지원하는 파일 공유에 연결합니다. 또한 사용자 REST API인 [PowerShell을](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-powershell) 사용할 수도 있습니다.

### <a name="cause-2-ntlmv1-is-enabled"></a>원인 2: NTLMv1이 사용 설정되어 있습니다.
...
파일을 삭제하려고 하면 다음과 같은 오류가 발생할 수 있습니다.
...
## <a name="no-folder-with-a-drive-letter-in-my-computer-or-this-pc"></a>"내 컴퓨터" 또는 "이 PC"에 드라이브 문자가 있는 폴더가 없습니다.
...
- cmdkey 명령을 사용하여 자격 증명 관리자에 자격 증명을 추가합니다. 대화형 로그인을 통해 또는 을 사용하여 `runas`서비스 계정 컨텍스트 아래의 명령줄에서 이 작업을 수행합니다.
...
## <a name="error-aaddstenantnotfound-in-enabling-azure-active-directory-domain-service-aad-ds-authentication-for-azure-files-unable-to-locate-active-tenants-with-tenant-id-aad-tenant-id"></a>오류 AadsTenantNotAzure 파일에 대한 Azure Active Directory 도메인 서비스(AAD DS) 인증을 사용하도록 설정하지 않음 "테넌트 Id aad-테넌트-id를 사용하여 활성 테넌트를 찾을 수 없음"
...
## <a name="error-system-error-1359-has-occurred-an-internal-error-received-over-smb-access-to-file-shares-with-azure-active-directory-domain-service-aad-ds-authentication-enabled"></a>시스템 오류 1359가 발생했습니다. Azure Active Directory 도메인 서비스(AAD DS) 인증을 사용하도록 설정한 파일 공유에 대한 SMB 액세스를 통해 받은 내부 오류'
