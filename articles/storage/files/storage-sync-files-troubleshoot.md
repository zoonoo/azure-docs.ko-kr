...
1. [Azure 저장소 포럼](https://social.msdn.microsoft.com/forums/azure/home?forum=windowsazuredata).
...
<a id="agent-installation-failures"></a>**에이전트 설치 오류 문제 해결**  
...
<a id="agent-installation-on-DC"></a>**Active Directory 도메인 컨트롤러에서 에이전트 설치가 실패했습니다.**  
...
<a id="parameter-is-incorrect"></a>**Windows Server 2012 R2에서 볼륨에 액세스하는 데 오류가 발생했습니다: 매개 변수가 올바르지 않습니다.**  
Windows Server 2012 R2에서 서버 끝점을 만든 후 볼륨에 액세스할 때 다음과 같은 오류가 발생합니다.

드라이브 레터:\ 액세스할 수 없습니다.  
매개 변수가 올바르지 않습니다.

해결하려면 Windows Server 2012 R2의 최신 업데이트를 설치하고 서버를 다시 시작합니다.

<a id="server-registration-missing-subscriptions"></a>**서버 등록에 모든 Azure 구독이 나열되지는 않습니다.**  
ServerRegistration.exe를 사용하여 서버를 등록할 때 Azure 구독 드롭다운을 클릭하면 구독이 누락됩니다.

이 문제는 ServerRegistration.exe현재 다중 테넌트 환경을 지원하지 않기 때문에 발생합니다. 이 문제는 향후 Azure 파일 동기화 에이전트 업데이트에서 해결될 예정입니다.
...
<a id="server-registration-prerequisites"></a>**서버 등록에는 "필수 구성 조건이 없습니다" 라는 메시지가 표시됩니다.**  
이 메시지는 Az 또는 AzureRM PowerShell 모듈이 PowerShell 5.1에 설치되어 있지 않은 경우에 나타납니다. 

> [!Note]  
> 서버등록.exe는 PowerShell 6.x를 지원하지 않습니다. PowerShell 6.x에서 레지스터-AzStorageSyncServer cmdlet을 사용하여 서버를 등록할 수 있습니다.

PowerShell 5.1에 Az 또는 AzureRM 모듈을 설치하려면 다음 단계를 수행하십시오.

1. 상승된 명령 프롬프트에서 **전원 셸을** 입력하고 enter를 적중시합니다.
2. 다음 설명서를 따라 최신 Az 또는 AzureRM 모듈을 설치합니다.
    - [Az 모듈(.NET 4.7.2 필요)](https://go.microsoft.com/fwlink/?linkid=2062890)
...
<a id="server-already-registered"></a>**서버 등록에는 "이 서버가 이미 등록되었습니다" 라는 메시지가 표시됩니다.** 
...
<a id="web-site-not-trusted"></a>**서버를 등록할 때 수많은 "신뢰할 수 없는 웹 사이트" 응답이 표시됩니다. 왜?**  
...
<a id="server-registration-missing"></a>**서버가 Azure 포털의 등록된 서버 아래에 나열되지 않습니다.**  
...
2. Azure 파일 공유를 마우스 오른쪽 단추로 클릭한 다음 **메타데이터 편집을**선택합니다.
3. **동기화 서비스**를 마우스 오른쪽 단추로 클릭한 다음 **에서 삭제를**선택합니다.

<a id="cloud-endpoint-authfailed"></a>**"AuthorizationFailed" 오류로 인해 클라우드 엔드포인트를 만드는 데 실패했습니다.**  
이 오류는 사용자 계정에 클라우드 끝점을 만들 수 있는 충분한 권한이 없는 경우에 발생합니다. 
...
1. Azure 포털에서 **리소스 그룹을**선택합니다.
2. 스토리지 계정이 있는 리소스 그룹을 선택하고 **IAM(Access Control)** 를 선택합니다.
3. **역할 할당** 탭을 선택합니다.
4. 사용자 **Role** 계정의 역할(예: 소유자 또는 기여자)을 선택합니다.
5. 리소스 **공급자** 목록에서 **Microsoft 권한 부여를**선택합니다. 
    * **역할 할당에는** **읽기** 및 **쓰기** 권한이 있어야 합니다.
    * **역할 정의에는** **읽기** 및 **쓰기** 권한이 있어야 합니다.

<a id="-2134375898"></a>**"MgmtServerJobFailed"(오류 코드: -2134375898 또는 0x80c80226)와 함께 서버 끝점 생성이 실패합니다.**  
...
<a id="-2147024894"></a>**"MgmtServerJobFailed"(오류 코드: -2147024894 또는 0x8007002)와 함께 서버 끝점 생성이 실패합니다.**  
지정된 서버 엔드포인트 경로가 잘못된 경우 이 오류가 발생합니다. 지정된 서버 엔드포인트 경로가 로컬로 연결된 NTFS 볼륨인지 확인합니다. Azure 파일 동기화는 매핑된 드라이브를 서버 끝점 경로로 지원하지 않습니다.

<a id="-2134375640"></a>**"MgmtServerJobFailed"(오류 코드: -2134375640 또는 0x80c80328)와 함께 서버 끝점 생성이 실패합니다.**  
지정된 서버 끝점 경로가 NTFS 볼륨이 아닌 경우 이 오류가 발생합니다. 지정된 서버 엔드포인트 경로가 로컬로 연결된 NTFS 볼륨인지 확인합니다. Azure 파일 동기화는 매핑된 드라이브를 서버 끝점 경로로 지원하지 않습니다.

<a id="-2134347507"></a>**"MgmtServerJobFailed"(오류 코드: -2134347507 또는 0x80c8710d)와 함께 서버 끝점 생성이 실패합니다.**  
...
2. 시스템 계정에서 실행 중인 명령 프롬프트를 실행하려면 상승된 명령 프롬프트에서 다음 명령을 **실행합니다.**
3. 시스템 계정으로 실행되는 명령 프롬프트에서 다음 명령을 입력하고 Enter 키를 누릅니다.   
    **cd /d "드라이브 문자:\시스템 볼륨 정보"**  
    **컴팩트 /u/s**

<a id="-2134376345"></a>**"MgmtServerJobFailed"(오류 코드: -2134376345 또는 0x80C80067)와 함께 서버 끝점 생성이 실패합니다.**  
서버당 서버 엔드포인트 수 제한에 도달한 경우에 이 오류가 발생합니다. Azure 파일 동기화는 현재 서버당 최대 30개의 서버 엔드포인트를 지원합니다. 자세한 내용은 [Azure 파일 동기화 축척 대상을](https://docs.microsoft.com/azure/storage/files/storage-files-scale-targets#azure-file-sync-scale-targets)참조하십시오.

<a id="-2134376427"></a>**"MgmtServerJobFailed"(오류 코드: -2134376427 또는 0x80c80015)**  
이 오류는 다른 서버 엔드포인트에서 지정된 서버 엔드포인트 경로를 이미 동기화하고 있는 경우에 발생합니다. Azure 파일 동기화는 동일한 디렉터리 또는 볼륨을 동기화하는 여러 서버 엔드포인트를 지원하지 않습니다.

<a id="-2160590967"></a>**"MgmtServerJobFailed"(오류 코드: -2160590967 또는 0x80c80077)와 함께 서버 끝점 생성이 실패합니다.**  
이 오류는 서버 끝점 경로에 분리된 계층화 파일이 포함된 경우 발생합니다. 서버 끝점이 최근에 제거된 경우 분리된 계층화 된 파일 정리가 완료될 때까지 기다립니다. 고아 계층화 된 파일 정리가 시작되면 이벤트 ID 6662가 원격 분석 이벤트 로그에 기록됩니다. 고아 계층화 된 파일 정리가 완료 되 고 서버 끝점을 경로를 사용 하 여 다시 만들 수 있습니다 일단 이벤트 ID 6661 기록 됩니다. Event ID 6661이 기록된 후 서버 끝점 생성에 실패하면 서버 끝점 섹션을 [삭제한 후 계층화](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) 파일에 설명된 단계를 수행하여 분리된 계층화 된 파일을 제거합니다.

<a id="-2134347757"></a>**"MgmtServerJobExpired"(오류 코드: -2134347757 또는 0x80c87013)**  
이 오류는 서버가 오프라인 상태이거나 네트워크에 연결되어 있지 않은 경우 발생합니다. 서버를 더 이상 사용할 수 없는 경우 Azure Portal에서 서버의 등록을 취소하여 서버 엔드포인트를 삭제합니다. 서버 엔드포인트를 삭제하려면 [Azure 파일 동기화에서 서버 등록 취소](storage-sync-files-server-registration.md#unregister-the-server-with-storage-sync-service)에 설명된 단계를 따릅니다.
...
```
<a id="server-endpoint-noactivity"></a>**서버 끝점에 "활동 없음" 또는 "보류 중" 상태가 있고 등록된 서버 블레이드의 서버 상태가 "오프라인으로 나타납니다"**  

이 문제는 저장소 동기화 모니터 프로세스(AzureStorageSyncMonitor.exe)가 실행중이아니거나 서버가 Azure 파일 동기화 서비스에 액세스할 수 없는 경우에 발생할 수 있습니다.

포털에서 "오프라인으로 나타납니다"로 표시되는 서버에서 원격 분석 이벤트 로그(응용 프로그램 및 서비스 아래에 위치)의 이벤트 ID 9301을 확인하여 서버가 Azure File Sync 서비스에 액세스할 수 없는 이유를 확인합니다. 

- **GetNextJob 상태로 완료** 된 경우: 0 기록, 서버 Azure 파일 동기화 서비스와 통신할 수 있습니다. 
    - 서버에서 작업 관리자를 열고 Storage 동기화 모니터링 (AzureStorageSyncMonitor.exe) 프로세스가 실행 중인지 확인합니다. 프로세스가 실행되지 않으면 먼저 서버를 다시 시작합니다. 서버를 다시 시작해도 문제가 해결되지 않으면 최신 Azure 파일 동기화 [에이전트 버전](https://docs.microsoft.com/azure/storage/files/storage-files-release-notes)으로 업그레이드합니다. 

- **GetNextJob 상태로 완료 된 경우: -2134347756** 기록, 서버 방화벽 또는 프록시로 인해 Azure 파일 동기화 서비스와 통신할 수 없습니다. 
    - 서버가 방화벽 뒤에 있는 경우 포트 443 아웃 바운드가 허용되는지 확인합니다. 방화벽이 특정 도메인으로 트래픽을 제한하는 경우 방화벽 [설명서에](https://docs.microsoft.com/azure/storage/files/storage-sync-files-firewall-and-proxy#firewall) 나열된 도메인에 액세스할 수 있는지 확인합니다.
    - 서버가 프록시 뒤에 있는 경우 프록시 [설명서의](https://docs.microsoft.com/azure/storage/files/storage-sync-files-firewall-and-proxy#proxy)단계에 따라 컴퓨터 전체 또는 앱별 프록시 설정을 구성합니다.
    - 테스트 스토리지싱크네트워크 연결 cmdlet을 사용하여 서비스 끝점에 대한 네트워크 연결을 확인합니다. 자세한 내용은 [서비스 끝점에 대한 네트워크 연결 테스트를](https://docs.microsoft.com/azure/storage/files/storage-sync-files-firewall-and-proxy#test-network-connectivity-to-service-endpoints)참조하십시오.

- **GetNextJob 상태로 완료 된 경우: -2134347764** 기록, 서버 만료 또는 삭제 된 인증서로 인해 Azure 파일 동기화 서비스와 통신할 수 없습니다.  
    - 서버에서 다음 PowerShell 명령을 실행하여 인증에 사용되는 인증서를 재설정합니다.
    ```powershell
    Reset-AzStorageSyncServerCertificate -ResourceGroupName <string> -StorageSyncServiceName <string>
    ```
<a id="endpoint-noactivity-sync"></a>**서버 끝점에 "활동 없음"의 상태가 있고 등록된 서버 블레이드의 서버 상태가 "온라인"입니다.**  
...
서버 끝점은 버그 또는 시스템 리소스 부족으로 인해 몇 시간 동안 동기화 작업을 기록하지 않을 수 있습니다. 최신 Azure 파일 동기화 [에이전트 버전이](https://docs.microsoft.com/azure/storage/files/storage-files-release-notes) 설치되어 있는지 확인합니다. 문제가 지속되면 지원 요청을 엽니다.
...
<a id="serverendpoint-pending"></a>**서버 끝점 상태가 몇 시간 동안 보류 중입니다.**  
...
2. 서버가 클라우드를 통해 완전히 최신 상태가 되고 어느 방향으로든 동기화할 변경 내용이 없으면 빈 동기화 세션이 표시됩니다. 이러한 이벤트는 모든 SyncFileCount, SyncDirCount, SyncTombcountCount 및 SyncSizeBytes)가 0인 업로드 및 다운로드 이벤트로 표시되므로 동기화할 것이 없습니다. 이러한 빈 동기화 세션은 항상 동기화할 새로운 것이 있기 때문에 이탈이 많은 서버에서는 발생하지 않을 수 있습니다. 동기화 활동이 없으면 30분마다 발생해야 합니다. 
...
> [!Note]  
> FileSyncErrorsReport.ps1 스크립트가 "파일 오류를 찾지 못했습니다"를 반환하거나 동기화 그룹에 대한 항목별 오류를 나열하지 않은 경우 원인은 다음과 같은 것입니다.
>
>- 원인 1: 마지막으로 완료된 동기화 세션에 항목별 오류가 없습니다. 포털이 동기화되지 않는 파일 0을 표시하도록 곧 업데이트되어야 합니다. 
>   - 원격 분석 이벤트 로그에서 [이벤트 ID 9102를](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=server%2Cazure-portal#broken-sync) 확인하여 PerItemErrorCount가 0인지 확인합니다. 
>
>- 원인 2: 항목당 오류가 너무 많아서 서버에서 ItemResults 이벤트 로그온이 더 이상 이 동기화 그룹에 대한 오류를 포함하지 않습니다.
>   - 이 문제를 방지하려면 ItemResults 이벤트 로그 크기를 늘립니다. ItemResults 이벤트 로그는 이벤트 뷰어의 "응용 프로그램 및 서비스 로그\Microsoft\FileSync\Agent"에서 찾을 수 있습니다. 
...
|---------|-------------------|--------------|-------|-------------|
| 0x80070043 | -2147942467 | ERROR_BAD_NET_NAME | 서버의 계층화된 파일에 액세스할 수 없습니다. 이 문제는 서버 엔드포인트를 삭제하기 전에 계층화된 파일이 회수되지 않은 경우에 발생합니다. | 이 문제를 해결하려면 [서버 끝점을 삭제한 후 서버에서 계층화 된 파일에 액세스 할 수 없음을](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint)참조하십시오. |
| 0x80c80207 | -2134375929 | ECS_E_SYNC_CONSTRAINT_CONFLICT | 종속 폴더가 아직 동기화되지 않았기 때문에 파일 또는 디렉터리 변경 을 아직 동기화할 수 없습니다. 이 항목은 종속 변경 내용이 동기화된 후 동기화됩니다. | 아무 조치도 취할 필요가 없습니다. 오류가 며칠 동안 지속되면 FileSyncErrors.ps1 PowerShell 스크립트를 사용하여 종속 폴더가 아직 동기화되지 않은 이유를 확인합니다. |
| 0x80c80284 | -2134375804 | ECS_E_SYNC_CONSTRAINT_CONFLICT_SESSION_FAILED | 종속 폴더가 아직 동기화되지 않았고 동기화 세션이 실패했기 때문에 파일 또는 디렉터리 변경을 아직 동기화할 수 없습니다. 이 항목은 종속 변경 내용이 동기화된 후 동기화됩니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 동기화 세션 오류를 조사합니다. |
...
| 0x80c80018 | -2134376424 | ECS_E_SYNC_FILE_IN_USE | 사용 중이므로 파일을 동기화할 수 없습니다. 파일이 더 이상 사용되지 않을 때 동기화됩니다. | 아무 조치도 취할 필요가 없습니다. Azure 파일 동기화는 핸들이 열려 있는 파일을 동기화하기 위해 하루 한 번 서버에 임시 VSS 스냅샷을 만듭니다. |
| 0x80c8031d | -2134375651 | ECS_E_CONCURRENCY_CHECK_FAILED | 파일이 변경되었지만 동기화를 통해 변경이 아직 검색되지 않았습니다. 이 변경이 감지되면 동기화가 복구됩니다. | 아무 조치도 취할 필요가 없습니다. |
| 0x80070002 | -2147024894 | ERROR_FILE_NOT_FOUND | 파일이 삭제되었으며 동기화가 변경 된 것을 인식하지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 변경 검색에서 파일이 삭제된 것을 감지하면 동기화가 이 오류 로깅을 중지합니다. |
| 0x80070003 | -2147942403 | ERROR_PATH_NOT_FOUND | 항목이 대상에서 이미 삭제되었고 동기화가 변경 된 것을 인식하지 못하기 때문에 파일 또는 디렉터리 삭제를 동기화 할 수 없습니다. | 아무 조치도 취할 필요가 없습니다. 대상에서 변경 검색이 실행되고 항목이 삭제된 항목을 동기화하면 동기화가 이 오류로깅을 중지합니다. |
| 0x80c80205 | -2134375931 | ECS_E_SYNC_ITEM_SKIP | 파일 또는 디렉터리가 건너뛰었지만 다음 동기화 세션 중에 동기화됩니다. 항목을 다운로드할 때 이 오류가 보고되면 파일 또는 디렉터리 이름이 유효하지 않을 수 있습니다. | 파일을 업로드할 때 이 오류가 보고되는 경우 작업이 필요하지 않습니다. 파일을 다운로드할 때 오류가 보고되면 해당 파일 또는 디렉터리 이름을 바꿉니다. 자세한 내용은 [지원되지 않는 문자 처리](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#handling-unsupported-characters)를 참조하세요. |
| 0x800700B7 | -2147024713 | ERROR_ALREADY_EXISTS | 항목이 대상에 이미 있고 동기화가 변경 된 것을 인식하지 못하기 때문에 파일 또는 디렉터리 만들기를 동기화 할 수 없습니다. | 아무 조치도 취할 필요가 없습니다. 대상에서 변경 검색이 실행되고 동기화가 이 새 항목을 인식하면 동기화가 이 오류 로깅을 중지합니다. |
...
| 0x80c8027C | -2134375812 | ECS_E_ACCESS_DENIED_EFS | 이 파일은 NTFS EFS와 같이 지원되지 않는 솔루션에 의해 암호화됩니다. | 파일의 암호를 해독하고 지원되는 암호화 솔루션을 사용합니다. 지원 솔루션의 목록은 계획 가이드의 [암호화 솔루션](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#encryption)을 참조하세요. |
| 0x80c80283 | -2160591491 | ECS_E_ACCESS_DENIED_DFSRRO | 파일은 DFS-R 읽기 전용 복제 폴더에 있습니다. | 파일은 DFS-R 읽기 전용 복제 폴더에 있습니다. Azure 파일 동기화는 DFS-R 읽기 전용 복제 폴더에서 서버 엔드포인트를 지원하지 않습니다. 자세한 내용은 [계획 가이드를](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#distributed-file-system-dfs) 참조하십시오. |
| 0x80070005 | -2147024891 | ERROR_ACCESS_DENIED | 파일에 삭제 보류 중인 상태가 있습니다. | 아무 조치도 취할 필요가 없습니다. 열려 있는 모든 파일 핸들이 닫히면 파일이 삭제됩니다. |
| 0x80c86044 | -2134351804 | ECS_E_AZURE_AUTHORIZATION_FAILED | 저장소 계정의 방화벽 및 가상 네트워크 설정이 활성화되어 있고 서버가 저장소 계정에 액세스할 수 없으므로 파일을 동기화할 수 없습니다. | 배포 가이드의 방화벽 및 가상 네트워크 설정 [구성](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide?tabs=azure-portal#configure-firewall-and-virtual-network-settings) 섹션에 설명된 단계에 따라 서버 IP 주소 또는 가상 네트워크를 추가합니다. |
| 0x80c80243 | -2134375869 | ECS_E_SECURITY_DESCRIPTOR_SIZE_TOO_LARGE | 보안 설명자 크기가 64 KiB 제한을 초과하므로 파일을 동기화할 수 없습니다. | 이 문제를 해결하려면 파일의 ACE(액세스 제어 항목)를 제거하여 보안 설명자 크기를 줄입니다. |
| 0x8000ffff | -2147418113 | E_UNEXPECTED | 예기치 않은 오류로 인해 파일을 동기화할 수 없습니다. | 며칠 동안 오류가 지속되면 지원 사례를 열어 주십시오. |
| 0x80070020 | -2147024864 | ERROR_SHARING_VIOLATION | 사용 중이므로 파일을 동기화할 수 없습니다. 파일이 더 이상 사용되지 않을 때 동기화됩니다. | 아무 조치도 취할 필요가 없습니다. |
| 0x80c80017 | -2134376425 | ECS_E_SYNC_OPLOCK_BROKEN | 동기화 중에 파일이 변경되었기 때문에 다시 동기화해야 합니다. | 아무 조치도 취할 필요가 없습니다. |
| 0x80070017 | -2147024873 | ERROR_CRC | CRC 오류로 인해 파일을 동기화할 수 없습니다. 이 오류는 서버 끝점을 삭제하기 전에 계층화 된 파일이 회수되지 않았거나 파일이 손상된 경우 발생할 수 있습니다. | 이 문제를 해결하려면 [서버 끝점을 삭제한 후 서버에서 계층화 된 파일에 액세스 할 수 없도록하여](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) 분리 된 계층화 된 파일을 제거하십시오. oprhaned 계층화 된 파일을 제거 한 후 오류가 계속 발생하면 볼륨에서 [chkdsk를](https://docs.microsoft.com/windows-server/administration/windows-commands/chkdsk) 실행합니다. |
| 0x80c80200 | -2134375936 | ECS_E_SYNC_CONFLICT_NAME_EXISTS | 최대 충돌 파일 수에 도달했기 때문에 파일을 동기화할 수 없습니다. Azure 파일 동기화는 파일당 100개의 충돌 파일을 지원합니다. 파일 충돌에 대한 자세한 내용은 Azure 파일 동기화 [FAQ를](https://docs.microsoft.com/azure/storage/files/storage-files-faq#afs-conflict-resolution)참조하십시오. | 이 문제를 해결하려면 충돌 파일 수를 줄입니다. 충돌 파일 수가 100개 미만이면 파일이 동기화됩니다. |

#### <a name="handling-unsupported-characters"></a>지원되지 않는 처리 문자
**FileSyncErrors.ps1** PowerShell 스크립트가 지원되지 않는 문자(오류 코드 0x8007007b 또는 0x80c80255)로 인해 오류가 발생하는 경우 해당 파일 이름에서 오류의 문자를 제거하거나 이름을 바꿉니다. 이러한 문자는 대부분 표준 시각적 개체 인코딩이 없으므로 PowerShell이 이러한 문자를 물음표 또는 빈 사각형으로 인쇄할 수 있습니다. [평가 도구](storage-sync-files-planning.md#evaluation-cmdlet)는 지원되지 않는 문자를 식별하는 데 사용될 수 있습니다.
...
동기화 세션은 다시 시작되거나 업데이트되는 서버, VSS 스냅숏 등 여러 가지 이유로 실패할 수 있습니다. 이 오류는 후속 조치가 필요한 것처럼 보이지만 몇 시간 동안 지속되지 않는 한 이 오류를 무시하는 것이 안전합니다.

<a id="-2147012889"></a>**서비스와의 연결을 설정할 수 없습니다.**    
...
<a id="-2134364043"></a>**변경 검색이 사후 복원을 완료할 때까지 동기화가 차단됩니다.**  
...
사용자가 조치할 필요는 없습니다. Azure Backup을 사용하여 파일 또는 파일 공유(클라우드 끝점)가 복원되면 Azure 파일 공유에서 변경 검색이 완료될 때까지 동기화가 차단됩니다. 변경 검색은 복원이 완료되는 즉시 실행되며 기간은 파일 공유의 파일 수를 기반으로 합니다

<a id="-2147216747"></a>**동기화 데이터베이스가 언로드되었기 때문에 동기화가 실패했습니다.**  
...
<a id="-2134351804"></a>**요청이 이 작업을 수행할 권한이 없기 때문에 동기화가 실패했습니다.**  
...
<a id="-2134364022"></a><a id="storage-unknown-error"></a>**저장소 계정에 액세스하는 동안 알 수 없는 오류가 발생했습니다.**  
...
<a id="-2134364014"></a>**저장소 계정이 잠겨 동기화가 실패했습니다.**  
...
저장소 계정에 읽기 전용 [리소스 잠금이](https://docs.microsoft.com/azure/azure-resource-manager/management/lock-resources)있기 때문에 이 오류가 발생합니다. 이 문제를 해결하려면 저장소 계정의 읽기 전용 리소스 잠금을 제거합니다. 
...
이 오류는 Azure File Sync에서 사용하는 내부 데이터베이스에 문제가 있을 때 발생합니다. 이 문제가 발생하면 지원 요청을 만들고 이 문제를 해결하는 데 도움이 되는 연락을 드릴 것입니다.
...
서버에 설치된 Azure 파일 동기화 에이전트 버전이 지원되지 않으면 이 오류가 발생합니다. 이 문제를 해결하려면 [지원되는 에이전트 버전으로]( https://docs.microsoft.com/azure/storage/files/storage-files-release-notes#supported-versions) [업그레이드합니다.]( https://docs.microsoft.com/azure/storage/files/storage-files-release-notes#upgrade-paths)
...
5. **파일** 공유 목록을 보려면 파일을 선택합니다.
6. 클라우드 엔드포인트가 참조하는 Azure 파일 공유에 대한 행의 끝에서 세 점을 클릭합니다.
7. **사용량**이 **할당량** 미만인지 확인합니다. 대체 할당량이 지정되지 않은 경우 할당량은 [Azure 파일 공유의 최대 크기와](storage-files-scale-targets.md)일치합니다.
...
이 오류는 스토리지 계정 방화벽 때문에 또는 스토리지 계정이 가상 네트워크에 속하기 때문에 Azure 파일 공유에 액세스할 수 없을 때 발생합니다. 저장소 계정의 방화벽 및 가상 네트워크 설정이 올바르게 구성되었는지 확인합니다. 자세한 내용은 [방화벽 및 가상 네트워크 설정 구성을](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide?tabs=azure-portal#configure-firewall-and-virtual-network-settings)참조하십시오. 
...
<a id="-2146762487"></a>**서버가 보안 연결을 설정하지 못했습니다. 클라우드 서비스에서 예기치 않은 인증서를 받았습니다.**  
...
이 오류는 조직에서 TLS 종료 프록시를 사용 하거나 악의적인 엔터티가 서버와 Azure File Sync 서비스 간의 트래픽을 가로채는 경우에 발생할 수 있습니다. 조직에서 TLS 종료 프록시를 사용하고 있기 때문에 예상되는 경우 레지스트리 재정의를 사용하여 인증서 확인을 건너뜁니다.
...
이 레지스트리 값을 설정하 여 Azure File Sync 에이전트는 서버와 클라우드 서비스 간에 데이터를 전송할 때 로컬로 신뢰할 수 있는 TLS/SSL 인증서를 수락합니다.

<a id="-2147012894"></a>**서비스와의 연결을 설정할 수 없습니다.**  
...
이 오류는 일반적으로 서버 시간이 올바르지 않기 때문에 발생합니다. 서버가 가상 컴퓨터에서 실행 중인 경우 호스트의 시간이 올바른지 확인합니다.

<a id="-2134364040"></a>**인증서 만료로 인해 동기화가 실패했습니다.**  
...
<a id="-2134375896"></a>**인증 인증서를 찾을 수 없습니다 인해 동기화가 실패했습니다.**  
...
<a id="-2134364039"></a>**인증 ID를 찾을 수 없습니다 인해 동기화가 실패했습니다.**  
...
이 오류는 Azure 파일 공유에 이미 존재하는 콘텐츠로 클라우드 끝점이 만들어졌기 때문에 발생합니다. Azure File Sync는 서버 끝점이 초기 동기화를 진행하도록 허용하기 전에 모든 콘텐츠에 대해 Azure 파일 공유를 검사해야 합니다.
...
<a id="-2134376373"></a>**현재 서비스를 사용할 수 없습니다.**  
...
<a id="-2146233088"></a>**예외로 인해 동기화가 실패했습니다.**  
...
이 오류는 예외로 인해 동기화가 실패한 경우에 발생합니다. 오류가 몇 시간 동안 지속되면 지원 요청을 만드십시오.

<a id="-2134364045"></a>**저장소 계정이 다른 지역으로 초과 실패했기 때문에 동기화가 실패했습니다.**  
...
<a id="-2134364024"></a>**Azure Active Directory 테넌트의 변경으로 인해 동기화가 실패했습니다.**  
...
이 오류는 저장소 계정에서 방화벽 및 가상 네트워크 설정을 사용하도록 설정하고 "신뢰할 수 있는 Microsoft 서비스가 이 저장소 계정에 액세스할 수 있도록 허용" 예외가 선택되지 않은 경우에 발생합니다. 이 문제를 해결하려면 배포 가이드의 [방화벽 및 가상 네트워크 설정 구성](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide?tabs=azure-portal#configure-firewall-and-virtual-network-settings) 섹션에 설명된 단계를 수행합니다.

<a id="-2147024891"></a>**시스템 볼륨 정보 폴더에 대한 사용 권한이 올바르지 않아 동기화가 실패했습니다.**  
...
이 오류는 NT AUTHORITY\SYSTEM 계정에 서버 엔드포인트가 있는 볼륨의 시스템 볼륨 정보 폴더에 대한 권한이 없는 경우에 발생할 수 있습니다. 개별 파일이 ERROR_ACCESS_DENIED 동기화되지 않는 경우 [파일/디렉터리 동기화 오류당 문제 해결](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#troubleshooting-per-filedirectory-sync-errors) 섹션에 설명된 단계를 수행합니다.

이 문제를 해결하려면 다음 단계를 수행합니다.

1. [PsExec](https://docs.microsoft.com/sysinternals/downloads/psexec) 도구를 다운로드합니다.
2. 시스템 계정을 사용 하 여 명령 프롬프트를 실행 하려면 상승 된 명령 프롬프트에서 다음 명령을 실행: **PsExec.exe -i-s-d cmd** 
...
<a id="-2134375810"></a>**Azure 파일 공유가 삭제되고 다시 생성되었기 때문에 동기화가 실패했습니다.**  
...
1. 동기화 그룹의 모든 서버 끝점을 삭제합니다.
2. 클라우드 끝점을 삭제합니다. 
3. 동기화 그룹을 삭제합니다.
4. 서버 끝점에서 클라우드 계층을 사용하도록 설정한 경우 서버 끝점 섹션을 [삭제한 후 계층화](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint) 파일에 설명된 단계를 수행하여 서버에서 분리된 계층화 된 파일을 삭제합니다.
5. 동기화 그룹을 다시 만듭니다.

<a id="-2145844941"></a>**HTTP 요청이 리디렉션되었기 때문에 동기화가 실패했습니다.**  
...
Azure 파일 동기화 HTTP 리디렉션 (3xx 상태 코드)를 지원 하지 않습니다 때문에이 오류가 발생 합니다. 이 문제를 해결하려면 프록시 서버 또는 네트워크 장치에서 HTTP 리디렉션을 사용하지 않도록 설정합니다.

<a id="-2134364027"></a>**오프라인 데이터 전송 중에 시간 시간이 발생했지만 아직 진행 중입니다.**  
...
이 오류는 데이터 수집 작업이 시간 초과를 초과할 때 발생합니다. 동기화가 진행 중인 경우 이 오류는 무시할 수 있습니다(AppliedItemCount가 0보다 큽니다). [현재 동기화 세션의 진행 률을 모니터링하려면 어떻게](#how-do-i-monitor-the-progress-of-a-current-sync-session)해야 합니까?
...
# <a name="portal"></a>[포털](#tab/azure-portal)
1. 왼쪽 목차의 **개요를** 클릭하여 기본 저장소 계정 페이지로 돌아갑니다.
2. **파일** 공유 목록을 보려면 파일을 선택합니다.
...
1. **Microsoft.StorageSync** 또는 **하이브리드 파일 동기화 서비스(이전** 응용 프로그램 이름)가 **판독기 및 데이터 액세스** 역할과 함께 목록에 나타나는지 확인합니다. 

    ![저장소 계정의 액세스 제어 탭에서 하이브리드 파일 동기화 서비스 주체의 스크린샷](media/storage-sync-files-troubleshoot/file-share-inaccessible-3.png)

    **Microsoft.StorageSync** 또는 **하이브리드 파일 동기화 서비스가** 목록에 나타나지 않으면 다음 단계를 수행합니다.

    - **추가**를 클릭합니다.
    - **역할** 필드에서 **읽기 권한자 및 데이터 액세스**를 선택합니다.
    - **선택** 필드에서 **Microsoft.StorageSync를**입력하고 역할을 선택하고 **저장을**클릭합니다.
...
> [!Important]  
> 파일 화면은 Azure 파일 동기화에서 지원되지 않는 문자 만들기를 차단하는 데만 사용해야 합니다. 다른 시나리오에서 파일 화면이 사용되는 경우 동기화는 Azure 파일 공유에서 서버로 파일을 계속 다운로드하려고 시도하며 파일 화면으로 인해 차단되어 높은 데이터 송신이 발생합니다. 
...
- 클라우드 스토리지 오류
    ...
    - *액세스할 수 없는 스토리지 계정* - 일반적으로 동기화 계정의 클라우드 엔드포인트인 Azure 파일 공유가 있는 동안 스토리지 계정을 삭제하면 이 오류가 발생합니다. 
- 서버 오류 
...
- 이벤트 ID 9003은 서버 엔드포인트에 대한 오류 분포를 제공합니다. 예를 들어 총 오류 수, 오류 코드 등 오류 코드당 하나의 이벤트가 기록됩니다.
...
- 이벤트 ID 9006은 서버 엔드포인트에 대한 회수 오류 분포를 제공합니다. 예를 들어 총 실패한 요청, 오류 코드 등 오류 코드당 하나의 이벤트가 기록됩니다.
...
> [!NOTE]
> 파일이 계층화에 실패하면 이벤트 ID 9003이 원격 분석 이벤트 로그에 1시간에 한 번 기록됩니다(오류 코드 하나당 이벤트 하나가 기록). [계층화 오류 및 수정](#tiering-errors-and-remediation) 섹션을 확인하여 오류 코드에 대한 수정 단계가 나열되어 있는지 확인합니다.

### <a name="tiering-errors-and-remediation"></a>계층화 오류 및 수정

| HRESULT | HRESULT(10진) | 오류 문자열 | 문제 | 재구성 |
|---------|-------------------|--------------|-------|-------------|
| 0x80c86043 | -2134351805 | ECS_E_GHOSTING_FILE_IN_USE | 사용 중이기 때문에 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 파일이 더 이상 사용되지 않을 때 계층화됩니다. |
| 0x80c80241 | -2134375871 | ECS_E_GHOSTING_EXCLUDED_BY_SYNC | 파일이 동기화에 의해 제외되어 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 동기화 제외 목록에 있는 파일은 계층화할 수 없습니다. |
| 0x80c86042 | -2134351806 | ECS_E_GHOSTING_FILE_NOT_FOUND | 파일이 서버에서 찾을 수 없기 때문에 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 서버에 파일이 있는지 확인합니다. |
| 0x80c83053 | -2134364077 | ECS_E_CREATE_SV_FILE_DELETED | Azure 파일 공유에서 파일이 삭제되었기 때문에 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 다음 다운로드 동기화 세션이 실행될 때 서버에서 파일을 삭제해야 합니다. |
| 0x80c8600e | -2134351858 | ECS_E_AZURE_SERVER_BUSY | 네트워크 문제로 인해 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 Azure 파일 공유에 대한 네트워크 연결을 확인합니다. |
| 0x80072ee7 | -2147012889 | WININET_E_NAME_NOT_RESOLVED | 네트워크 문제로 인해 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 Azure 파일 공유에 대한 네트워크 연결을 확인합니다. |
| 0x80070005 | -2147024891 | ERROR_ACCESS_DENIED | 액세스 거부 오류로 인해 파일이 계층에 실패했습니다. 이 오류는 파일이 DFS-R 읽기 전용 복제 폴더에 있는 경우 발생할 수 있습니다. | Azure 파일 동기화는 DFS-R 읽기 전용 복제 폴더에서 서버 엔드포인트를 지원하지 않습니다. 자세한 내용은 [계획 가이드를](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#distributed-file-system-dfs) 참조하십시오. |
| 0x80072efe | -2147012866 | WININET_E_CONNECTION_ABORTED | 네트워크 문제로 인해 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 Azure 파일 공유에 대한 네트워크 연결을 확인합니다. |
| 0x80c80261 | -2134375839 | ECS_E_GHOSTING_MIN_FILE_SIZE | 파일 크기가 지원되는 크기보다 작기 때문에 파일이 계층화되지 못했습니다. | 에이전트 버전이 9.0보다 작으면 지원되는 최소 파일 크기는 64kb입니다. 에이전트 버전이 9.0이고 최신 인 경우 지원되는 최소 파일 크기는 파일 시스템 클러스터 크기(이중 파일 시스템 클러스터 크기)를 기반으로 합니다. 예를 들어 파일 시스템 클러스터 크기가 4kb인 경우 최소 파일 크기는 8kb입니다. |
| 0x80c83007 | -2134364153 | ECS_E_STORAGE_ERROR | Azure 저장소 문제로 인해 파일이 계층화되지 못했습니다. | 오류가 지속되면 지원 요청을 엽니다. |
| 0x800703e3 | -2147023901 | ERROR_OPERATION_ABORTED | 파일이 동시에 회수되었기 때문에 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 리콜이 완료되고 파일이 더 이상 사용되지 않을 때 파일이 계층화됩니다. |
| 0x80c80264 | -2134375836 | ECS_E_GHOSTING_FILE_NOT_SYNCED | Azure 파일 공유에 동기화되지 않았기 때문에 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 파일이 Azure 파일 공유에 동기화되면 계층화됩니다. |
| 0x80070001 | -2147942401 | ERROR_INVALID_FUNCTION | 클라우드 계층화 필터 드라이버(storagesync.sys)가 실행되지 않아 파일이 계층화되지 못했습니다. | 이 문제를 해결 하려면 상승 된 명령 프롬프트를 열고 다음 명령을 실행 합니다.`fltmc load storagesync`<br>fltmc 명령을 실행할 때 storagesync 필터 드라이버가 로드되지 않으면 Azure 파일 동기화 에이전트를 제거하고 서버를 다시 시작하고 Azure File Sync 에이전트를 다시 설치합니다. |
| 0x80070070 | -2147024784 | ERROR_DISK_FULL | 서버 끝점이 있는 볼륨의 디스크 공간이 부족하여 파일이 계층화되지 못했습니다. | 이 문제를 해결하려면 서버 끝점이 있는 볼륨에서 최소 100MB의 디스크 공간을 확보합니다. |
| 0x80070490 | -2147023728 | ERROR_NOT_FOUND | Azure 파일 공유에 동기화되지 않았기 때문에 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 파일이 Azure 파일 공유에 동기화되면 계층화됩니다. |
| 0x80c80262 | -2134375838 | ECS_E_GHOSTING_UNSUPPORTED_RP | 지원되지 않는 리패시드 포인트이기 때문에 파일이 계층화되지 못했습니다. | 파일이 데이터 중복 제거 회수 지점인 경우 [계획 가이드의](https://docs.microsoft.com/azure/storage/files/storage-sync-files-planning#data-deduplication) 단계를 수행하여 데이터 중복 제거 지원을 사용하도록 설정합니다. 데이터 중복 제거 이외의 재분석 점이 있는 파일은 지원되지 않으며 계층화되지 않습니다.  |
| 0x80c83052 | -2134364078 | ECS_E_CREATE_SV_STREAM_ID_MISMATCH | 파일이 수정되었기 때문에 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 수정된 파일이 Azure 파일 공유에 동기화되면 파일이 계층화됩니다. |
| 0x80c80269 | -2134375831 | ECS_E_GHOSTING_REPLICA_NOT_FOUND | Azure 파일 공유에 동기화되지 않았기 때문에 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 파일이 Azure 파일 공유에 동기화되면 계층화됩니다. |
| 0x80072ee2 | -2147012894 | WININET_E_TIMEOUT | 네트워크 문제로 인해 파일이 계층화되지 못했습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 지속되면 Azure 파일 공유에 대한 네트워크 연결을 확인합니다. |
| 0x80c80017 | -2134376425 | ECS_E_SYNC_OPLOCK_BROKEN | 파일이 수정되었기 때문에 계층화에 실패했습니다. | 아무 조치도 취할 필요가 없습니다. 수정된 파일이 Azure 파일 공유에 동기화되면 파일이 계층화됩니다. |
| 0x800705aa | -2147023446 | ERROR_NO_SYSTEM_RESOURCES | 시스템 리소스가 부족하여 파일이 계층에 실패했습니다. | 오류가 지속되면 시스템 리소스를 모두 소모하는 응용 프로그램 또는 커널 모드 드라이버를 조사합니다. |
...
> [!NOTE]
> 파일이 회수에 실패하면 이벤트 ID 9006이 원격 분석 이벤트 로그에 1시간에 한 번 기록됩니다(오류 코드 하나당 이벤트 하나가 기록). 리콜 [오류 및 수정](#recall-errors-and-remediation) 섹션을 확인하여 오류 코드에 대한 수정 단계가 나열되어 있는지 확인합니다.

### <a name="recall-errors-and-remediation"></a>리콜 오류 및 수정

| HRESULT | HRESULT(10진) | 오류 문자열 | 문제 | 재구성 |
|---------|-------------------|--------------|-------|-------------|
| 0x80070079 | -2147942521 | ERROR_SEM_TIMEOUT | I/O 시간 부족으로 인해 파일을 회수하지 못했습니다. 이 문제는 서버 리소스 제약 조건, 네트워크 연결 불량 또는 Azure 저장소 문제(예: 제한)와 같은 여러 가지 이유로 발생할 수 있습니다. | 아무 조치도 취할 필요가 없습니다. 오류가 몇 시간 동안 지속되면 지원 사례를 열어 주세요. |
| 0x80070036 | -2147024842 | ERROR_NETWORK_BUSY | 네트워크 문제로 인해 파일을 회수하지 못했습니다.  | 오류가 지속되면 Azure 파일 공유에 대한 네트워크 연결을 확인합니다. |
| 0x80c80037 | -2134376393 | ECS_E_SYNC_SHARE_NOT_FOUND | 서버 끝점이 삭제되었기 때문에 파일을 회수하지 못했습니다. | 이 문제를 해결하려면 [서버 끝점을 삭제한 후 서버에서 계층화 된 파일에 액세스 할 수 없음을](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#tiered-files-are-not-accessible-on-the-server-after-deleting-a-server-endpoint)참조하십시오. |
| 0x80070005 | -2147024891 | ERROR_ACCESS_DENIED | 액세스 거부 오류로 인해 파일을 회수하지 못했습니다. 저장소 계정의 방화벽 및 가상 네트워크 설정이 활성화되어 있고 서버가 저장소 계정에 액세스할 수 없는 경우 이 문제가 발생할 수 있습니다. | 이 문제를 해결하려면 배포 가이드의 방화벽 및 가상 네트워크 [설정 구성](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide?tabs=azure-portal#configure-firewall-and-virtual-network-settings) 섹션에 설명된 단계에 따라 서버 IP 주소 또는 가상 네트워크를 추가합니다. |
| 0x80c86002 | -2134351870 | ECS_E_AZURE_RESOURCE_NOT_FOUND | Azure 파일 공유에서 액세스할 수 없기 때문에 파일을 회수하지 못했습니다. | 이 문제를 해결하려면 Azure 파일 공유에 파일이 있는지 확인합니다. Azure 파일 공유에 파일이 있는 경우 최신 Azure 파일 동기화 [에이전트 버전으로](https://docs.microsoft.com/azure/storage/files/storage-files-release-notes#supported-versions)업그레이드합니다. |
| 0x80c8305f | -2134364065 | ECS_E_EXTERNAL_STORAGE_ACCOUNT_AUTHORIZATION_FAILED | 저장소 계정에 대한 권한 부여 실패로 인해 파일을 회수하지 못했습니다. | 이 문제를 해결하려면 [Azure File Sync에서 저장소 계정에 액세스할 수 있는지](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#troubleshoot-rbac)확인합니다. |
| 0x80c86030 | -2134351824 | ECS_E_AZURE_FILE_SHARE_NOT_FOUND | Azure 파일 공유에 액세스할 수 없기 때문에 파일을 회수하지 못했습니다. | 파일 공유가 존재하고 액세스할 수 있는지 확인합니다. 파일 공유가 삭제되고 다시 생성된 경우 Azure [파일 공유가 삭제되고 다시 생성된](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#-2134375810) 섹션으로 인해 동기화 실패에 대해 설명된 단계를 수행하여 동기화 그룹을 삭제하고 다시 만듭니다. |
| 0x800705aa | -2147023446 | ERROR_NO_SYSTEM_RESOURCES | 시스템 리소스가 부족하여 파일을 회수하지 못했습니다. | 오류가 지속되면 시스템 리소스를 모두 소모하는 응용 프로그램 또는 커널 모드 드라이버를 조사합니다. |
| 0x800700e | -2147024882 | ERROR_OUTOFMEMORY | 이 파일은 메모리가 부족하여 회수하지 못했습니다. | 오류가 지속되면 메모리 부족 상태를 일으키는 응용 프로그램 또는 커널 모드 드라이버를 조사합니다. |
| 0x80070070 | -2147024784 | ERROR_DISK_FULL | 디스크 공간이 부족하여 파일을 회수하지 못했습니다. | 이 문제를 해결하려면 파일을 다른 볼륨으로 이동하여 볼륨의 공간을 확보하거나, 볼륨 크기를 늘리거나, Invoke-StorageSyncCloudTiering cmdlet을 사용하여 파일을 계층화하도록 합니다. |
...
서버 끝점을 삭제하기 전에 파일을 회수하지 않으면 서버의 계층화된 파일에 액세스할 수 없게 됩니다.

계층화 된 파일에 액세스할 수 없는 경우 기록된 오류
- 파일을 동기화할 때 오류 코드 -21479424467(0x80070043 - ERROR_BAD_NET_NAME)이 ItemResults 이벤트 로그에 기록됩니다.
- 파일을 리콜할 때 오류 코드 -2134376393(0x80c80037 - ECS_E_SYNC_SHARE_NOT_FOUND)이 RecallResults 이벤트 로그에 기록됩니다.
...
위의 조건이 충족되지 않으면 서버에서 계층화된 파일이 이제 분리되기 때문에 액세스를 복원하지 못할 수 있습니다. 아래 의 지침에 따라 분리된 계층화 파일을 제거하십시오.

**참고**
- 서버에서 계층화된 파일에 액세스할 수 없는 경우에도 Azure 파일 공유에 직접 액세스하는 경우에도 전체 파일에 액세스할 수 있어야 합니다.
- 나중에 분리된 계층화 된 파일을 방지하려면 서버 끝점을 삭제할 때 [서버 끝점 제거에](https://docs.microsoft.com/azure/storage/files/storage-sync-files-server-endpoint#remove-a-server-endpoint) 설명된 단계를 따릅니다.

<a id="get-orphaned"></a>**분리계층 파일 목록을 가져오는 방법** 

1. Azure 파일 동기화 에이전트 버전 v5.1 이상이 설치된지 확인합니다.
2. 다음 PowerShell 명령을 실행하여 분리된 계층화 파일을 나열합니다.
```powershell
Import-Module "C:\Program Files\Azure\StorageSyncAgent\StorageSync.Management.ServerCmdlets.dll"
$orphanFiles = Get-StorageSyncOrphanedTieredFiles -path <server endpoint path>
$orphanFiles.OrphanedTieredFiles > OrphanTieredFiles.txt
```
3. 파일이 삭제 된 후 백업에서 복원해야하는 경우에 OrphanTieredFiles.txt 출력 파일을 저장합니다.

<a id="remove-orphaned"></a>**분리계층 파일을 제거하는 방법** 

*옵션 1: 분리계층 파일 삭제*

이 옵션은 Windows Server에서 분리된 계층화 된 파일을 삭제하지만 30 일 후에 레크리에이션으로 인해 서버 끝점이 있거나 다른 동기화 그룹에 연결되어있는 경우 서버 끝점을 제거해야합니다. 서버 끝점을 다시 생성하기 전에 Windows Server 또는 Azure 파일 공유에서 파일이 업데이트되면 파일 충돌이 발생합니다.

1. Azure 파일 동기화 에이전트 버전 v5.1 이상이 설치된지 확인합니다.
2. Azure 파일 공유 및 서버 끝점 위치를 백업합니다.
3. 서버 끝점 [제거에](https://docs.microsoft.com/azure/storage/files/storage-sync-files-server-endpoint#remove-a-server-endpoint)설명된 단계에 따라 동기화 그룹의 서버 끝점(있는 경우)을 제거합니다.

> [!Warning]  
> 제거-StorageSyncOrphanedTieredFiles cmdlet을 사용하기 전에 서버 끝점이 제거되지 않으면 서버에서 분리된 계층화 된 파일을 삭제하면 Azure 파일 공유의 전체 파일이 삭제됩니다. 

4. 다음 PowerShell 명령을 실행하여 분리된 계층화 파일을 나열합니다.
...
$orphanFiles.OrphanedTieredFiles > OrphanTieredFiles.txt
```
5. 파일이 삭제 된 후 백업에서 복원해야하는 경우에 OrphanTieredFiles.txt 출력 파일을 저장합니다.
6. 다음 PowerShell 명령을 실행하여 분리된 계층화 된 파일을 삭제합니다.
...
```
**참고** 
- Azure 파일 공유에 동기화되지 않은 서버에서 수정된 계층화 파일은 삭제됩니다.
- 고아가 아닌 액세스할 수 있는 계층화된 파일은 삭제되지 않습니다.
- 계층화되지 않은 파일은 서버에 남아 있습니다.

7. 선택 사항: 3단계에서 삭제된 경우 서버 끝점을 다시 만듭니다.

*옵션 2: Azure 파일 공유를 마운트하고 서버에서 분리된 파일을 로컬로 복사합니다.*

이 옵션은 서버 끝점을 제거할 필요가 없지만 전체 파일을 로컬로 복사할 수 있는 충분한 디스크 공간이 필요합니다.

1. 계층화 된 파일이 분리된 Windows 서버에 Azure 파일 공유를 [마운트합니다.](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows)
2. 다음 PowerShell 명령을 실행하여 분리된 계층화 파일을 나열합니다.
```powershell
Import-Module "C:\Program Files\Azure\StorageSyncAgent\StorageSync.Management.ServerCmdlets.dll"
$orphanFiles = Get-StorageSyncOrphanedTieredFiles -path <server endpoint path>
$orphanFiles.OrphanedTieredFiles > OrphanTieredFiles.txt
```
3. OrphanTieredFiles.txt 출력 파일을 사용하여 서버에서 분리계층 파일을 식별합니다.
4. Azure 파일 공유에서 Windows Server로 전체 파일을 복사하여 분리된 계층화 된 파일을 덮어씁니다.

### <a name="how-to-troubleshoot-files-unexpectedly-recalled-on-a-server"></a>서버에서 예기치 않게 회수된 파일 문제를 해결하는 방법  
...
1. AFSDiag 출력이 저장될 디렉터리를 만듭니다(예: C:\output).
    > [!NOTE]
    >AFSDiag는 로그를 수집하기 전에 출력 디렉토리의 모든 콘텐츠를 삭제합니다. 데이터를 포함하지 않는 출력 위치를 지정합니다.
...
- [Linux에서 Azure 파일 문제 해결](storage-troubleshoot-linux-file-connection-problems.md)
