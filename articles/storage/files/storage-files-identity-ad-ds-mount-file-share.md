...
- 이전에 저장소 계정 키를 사용 하 여 파일 공유를 탑재 한 클라이언트에서 파일 공유를 탑재 하는 경우 공유의 연결을 끊고, 저장소 계정 키의 영구 자격 증명을 제거 하 고, 현재 인증에 AD DS 자격 증명을 사용 하 고 있는지 확인 합니다. 저장소 계정 키를 사용 하 여 탑재 된 공유를 지우는 지침은 [FAQ 페이지](./storage-files-faq.md#ad-ds--azure-ad-ds-authentication)를 참조 하세요.
