...
## <a name="general"></a>일반
* **Azure Files란?**  
    Azure Files는 산업 표준 [SMB(서버 메시지 블록) 프로토콜](https://msdn.microsoft.com/library/windows/desktop/aa365233.aspx)(일반 인터넷 파일 시스템 또는 CIFS라고도 함)을 통해 액세스할 수 있는, 클라우드에서 완전히 관리되는 파일 공유를 제공합니다. Azure File 공유는 Windows, Linux 및 macOS의 클라우드 또는 온-프레미스 배포를 통해 동시에 탑재될 수 있습니다.

* **Azure Files가 유용한 이유는 무엇인가요?**  
...
* **데이터를 위해 Azure 파일 공유 및 Azure Blob Storage를 사용하는 이유는 각각 무엇인가요?**  
...
* **Azure 파일 공유 및 Azure 디스크를 사용하는 이유는 각각 무엇인가요?**  
...
* **Azure Files에서 지원되는 저장소 중복 옵션은 어느 것인가요?**  
...
* **실제로 Azure Files에 *x* 기능이 추가되는 것을 보고 싶습니다. 추가해줄 수 있나요?**  
...
* **현재 어떤 지역에서 Azure File Sync(미리 보기)가 지원되나요?**  
...
* **SMB 또는 포털을 통해 Azure 파일 공유에서 직접 파일을 만들었습니다. 파일이 동기화 그룹의 서버와 동기화되는 데 시간이 얼마나 걸리나요?** 
    Azure Portal, SMB 또는 REST를 사용하여 Azure 파일 공유에서 파일을 만들거나 수정할 경우 변경 내용을 동기화 그룹의 서버와 동기화하는 데 최대 24시간이 걸릴 수 있습니다.

* **같은 파일이 두 서버에서 거의 동시에 변경되면 어떻게 되나요?**  
...
* **서버를 등록할 때 "웹 사이트를 신뢰할 수 없습니다." 메시지가 여러 번 표시되는 이유는 무엇인가요?**  
...
* **GRS(지역 중복 저장소)가 Azure File Sync에서 지원되나요?**  
...
* **사용하려는 파일이 계층화되어 있습니다. 로컬로 사용하기 위해 디스크로 회수하려면 어떻게 해야 하나요?**  
...
* **파일 또는 폴더가 Azure File Sync에 의해 제외됩니다. 이유는 무엇인가요?**
...
* **indows Server 2008 R2, Linux 또는 NAS 장치에서 Azure File Sync를 사용하려고 합니다. Azure File Sync가 지원되나요?**
...
* **Azure Files에서 Active Directory 기반 인증 및 액세스 제어를 지원하나요?**  
    Azure Files는 현재 AD 기반 인증이나 ACL을 지원하지 않지만 현재 관련 작업이 진행되고 있습니다. 현재 AD 기반 인증 및 액세스 제어가 지원되지 않는 문제를 해결할 수 있는 다음과 같은 2가지 방법이 있습니다.
...
* **Azure 파일 공유가 미사용 암호화되도록 하려면 어떻게 해야 하나요?**  
    암호화를 사용하도록 설정하기 위해 수행해야 하는 작업은 없습니다. 현재 [서버 쪽 암호화](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)가 기본적으로 모든 공용 지역 및 국가 클라우드의 Azure Files에 대해 사용하도록 설정되어 있습니다. 

* **웹 브라우저를 통해 특정 파일에 대한 액세스를 제공하려면 어떻게 해야 하나요?**  
...
* **Azure Files에 연결하거나 온-프레미스에서 Azure Files에 사용하려면 Azure ExpressRoute를 사용해야 하나요?**  
...
* **내 Azure 파일 공유를 백업하려면 어떻게 하나요?**  
    실수로 삭제하지 않도록 보호하기 위해 주기적인 공유 스냅숏(미리 보기)을 사용할 수 있습니다. 탑재된 파일 공유를 백업할 수 있는 AzCopy, RoboCopy 또는 타사 백업 도구를 사용할 수 있습니다.
...
* **개별 파일의 공유 스냅숏을 만들 수 있나요?**
    공유 스냅숏은 파일 공유 수준에서 만들어집니다. 파일 공유 스냅숏에서 개별 파일을 복원할 수 있지만 파일 수준 공유 스냅숏을 만들 수 없습니다. 그러나 공유 수준 공유 스냅숏을 작성했으며 특정 파일이 변경된 공유 스냅숏을 나열하려면 Windows 탑재 공유의 "이전 버전" 환경에서 이 작업을 수행할 수 있습니다.
...
* **공유 스냅숏이 지역 중복 스냅샷인가요?**
    공유 스냅숏은 해당 Azure 파일 공유와 동일한 중복성을 갖게 됩니다. 계정에 대해 GRS(지역 중복 저장소)를 선택한 경우 공유 스냅숏이 페어링된 지역에도 중복해서 저장됩니다.
...
* **Azure VM과 파일 공유 사이의 네트워크 트래픽은 구독에 청구되는 외부 대역폭으로 간주되나요?**  
...
* **스냅숏 공유 비용은 얼마나 되나요?**
     ...
     공유 스냅숏은 기본적으로 증분합니다. 기본 공유 스냅숏은 공유 자체입니다. 모든 후속 공유 스냅숏은 증분하며, 이전 공유 스냅숏과의 차이만 저장합니다. 변경된 콘텐츠에 대해서만 요금이 청구됩니다. 100GB의 데이터 공유가 있으나 마지막 공유 스냅숏 후에 5GB만 변경된 경우 해당 공유 스냅숏은 추가로 5GB만 사용하게 되며 105GB에 대한 요금만 청구됩니다. 트랜잭션 및 표준 송신 요금에 대한 자세한 내용은 [가격 책정 페이지](https://azure.microsoft.com/pricing/details/storage/files/)를 참조하세요.
...
* **Azure Files에 파일의 압축을 풀려고 하면 성능이 느려졌습니다. 어떻게 해야 하나요?**  
    많은 파일을 Azure Files로 전송하려면 네트워크 전송을 위해 최적화된 도구인 AzCopy(Windows, Linux/Unix 용 미리 보기) 또는 Azure PowerShell을 사용하는 것이 좋습니다.

* **Windows Server 2012 R2 또는 Windows 8.1에 Azure 파일 공유를 탑재했으며 성능이 느려졌습니다. 이유는 무엇인가요?**  
...
* **장애 조치(Failover) 클러스터에 대한 “파일 공유 감시”는 Azure Files의 사용 사례 중 하나인가요?**  
...
* **포함된 공유를 지정할 수 있습니까, 다시 말해서 공유 밑에 공유를 지정할 수 있습니까?**  
    아니요. 파일 공유는 마운트할 수 있는 가상 드라이버이므로 포함된 공유는 지원되지 않습니다.

* **IBM MQ에서 Azure Files를 사용하고 있습니다.**  
