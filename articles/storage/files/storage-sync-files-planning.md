...
ms.locfileid: "82176093"
---
# <a name="planning-for-an-azure-file-sync-deployment"></a>Azure 파일 동기화 배포에 대한 계획

:::row:::
    :::column:::
        [![인터뷰 및 데모 소개 Azure File Sync-클릭 하 여 재생 합니다.](./media/storage-sync-files-planning/azure-file-sync-interview-video-snapshot.png)](https://www.youtube.com/watch?v=nfWLO7F52-s)
    :::column-end:::
    :::column:::
        Azure File Sync은 온-프레미스 Windows Server 또는 클라우드 VM에서 많은 Azure 파일 공유를 캐시 하는 데 사용할 수 있는 서비스입니다. 
        
        이 문서에서는 개념과 기능을 Azure File Sync 하는 방법을 소개 합니다. Azure File Sync에 대해 잘 알고 있으면 [Azure File Sync 배포 가이드](storage-sync-files-deployment-guide.md) 에 따라이 서비스를 사용해 보십시오.        
    :::column-end:::
:::row-end:::

파일은 [Azure 파일 공유](storage-files-introduction.md)의 클라우드에 저장 됩니다. Azure 파일 공유는 이러한 서버를 사용 하지 않는 Azure 파일 공유 (SMB)를 직접 탑재 하거나 Azure File Sync를 사용 하 여 온-프레미스에서 Azure 파일 공유를 캐시 하는 두 가지 방법으로 사용할 수 있습니다. 배포를 계획할 때 고려해 야 하는 사항을 변경 하는 배포 옵션을 선택 합니다. 
...
Windows Server에서 동기화 기능을 사용 하도록 설정 하려면 Azure File Sync 다운로드할 수 있는 에이전트를 설치 해야 합니다. Azure File Sync 에이전트는 두 가지 주요 구성 요소인 `FileSyncSvc.exe`, 즉 서버 끝점의 변경 내용을 모니터링 하 고 동기화 세션을 시작 하는 백그라운드 Windows 서비스와 `StorageSync.sys`클라우드 계층화 및 빠른 재해 복구를 지 원하는 파일 시스템 필터를 제공 합니다.  
...
|---------|----------------|------------------------------|
| 시작 | Datacenter, Standard 및 IoT | 전체 및 코어 |
...
\*1억 개 이상의 파일 & 디렉터리를 동기화 하는 것은 현재 권장 되지 않습니다. 이는 테스트 된 임계값을 기반으로 하는 소프트 제한입니다. 자세한 내용은 [Azure Files 확장성 및 성능 목표](storage-files-scale-targets.md#azure-file-sync-scale-targets)를 참조 하세요.
...
Azure File Sync은 동기화를 설정 하는 것 외에는 특별 한 설정 없이 표준 AD 기반 id로 작동 합니다. Azure File Sync를 사용 하는 경우 대부분의 액세스는 Azure 파일 공유 대신 Azure File Sync 캐싱 서버를 통과 하는 것이 일반적입니다. 서버 끝점은 Windows Server에 있고 Windows Server는 오랜 시간 동안 AD 및 Windows 스타일 Acl을 지원 하므로 저장소 동기화 서비스에 등록 된 Windows 파일 서버가 도메인에 가입 되어 있는지 확인 하는 것 외에는 아무 작업도 필요 하지 않습니다. Azure File Sync는 Azure 파일 공유의 파일에 Acl을 저장 하 고 모든 서버 끝점에 복제 합니다.
...
BitLocker와 유사 하 게 작동 하는 타사 제품 (NTFS 볼륨 아래에 있음)은 Azure File Sync를 사용 하 여 완전히 투명 하 게 작동 해야 합니다. 
...
Azure File Sync ntfs EFS (NTFS 암호화 파일 시스템) 또는 파일 시스템 위에 있지만 파일의 데이터 스트림 아래에 있는 타사 암호화 솔루션과 상호 운용 되지 않습니다. 

### <a name="encryption-in-transit"></a>전송 중 암호화

> [!NOTE]
> Azure File Sync 서비스는 8 월 2020에 TLS 1.0 및 1.1에 대 한 지원을 제거 합니다. 지원 되는 모든 Azure File Sync 에이전트 버전은 기본적으로 TLS 1.2를 이미 사용 하 고 있습니다. 서버에서 TLS 1.2를 사용 하지 않도록 설정 했거나 프록시가 사용 되는 경우 이전 버전의 TLS를 사용 하는 것이 발생할 수 있습니다. 프록시를 사용 하는 경우 프록시 구성을 확인 하는 것이 좋습니다. 5/1/2020 이후 추가 된 서비스 지역은 TLS 1.2만 지원 하 고 TLS 1.0에 대 한 지원 및 1.1은 8 월 2020 일에 기존 지역에서 제거 됩니다. Azure File Sync  자세한 내용은 [문제 해결 가이드](storage-sync-files-troubleshoot.md#tls-12-required-for-azure-file-sync)를 참조 하세요.
...
- 새 파일 서버에만 서버 끝점을 만들고를 사용 하 여 `robocopy`이전 파일 공유에서로 데이터를 복사 합니다. 새 서버에 있는 파일 공유의 토폴로지 (각 볼륨에 있는 공유의 수, 각 볼륨의 사용 가능한 시간 등)에 따라, 이전 서버에서 온-프레미스 데이터 센터 내의 새 서버에 대 한 데이터를 `robocopy` Azure로 이동 하는 Azure File Sync 것 보다 더 빠르게 완료 될 것으로 예상 되는 추가 저장소를 임시로 프로 비전 해야 할 수 있습니다.
