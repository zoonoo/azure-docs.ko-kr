...
File REST 프토콜을 직접 작업하는 것이 가능하지만(즉, REST HTTP 호출 직접 생성), File REST 프로토콜을 사용하는 가장 일반적인 방법은 Azure CLI 또는 [AzureRM PowerShell 모듈](storage-how-to-use-files-powershell.md), 또는 Azure Storage SDK를 사용하는 것입니다. 이들 모두 선택한 스크립팅/프로그래밍 언어에서 File REST 프로토콜에 좋은 래퍼를 제공합니다.  
