...
Azure 가상 네트워크 게이트웨이는 온-프레미스 Windows 머신에서 VPN 연결을 초기화하는 데 필요한 구성 파일이 포함된 다운로드 가능한 패키지를 만듭니다. Windows 10/Windows Server 2016 이상의 [Always On VPN](https://docs.microsoft.com/windows-server/remote/remote-access/vpn/always-on-vpn/) 기능을 사용하여 VPN 연결을 구성하겠습니다. 이 패키지에는 원하는 경우 레거시 Windows VPN 클라이언트를 구성할 수 있는 실행 가능 패키지도 포함되어 있습니다. 이 가이드에서는 레거시 Windows VPN 클라이언트가 아닌 Always On VPN을 사용합니다. Always On VPN 클라이언트는 최종 사용자가 머신에 대한 관리자 권한이 없어도 Azure VPN에서 연결/연결 해제할 수 있기 때문입니다. 
