...
### <a name="create-the-storage-sync-service-private-endpoint"></a>저장소 동기화 서비스 개인 끝점 만들기
> [!Important]  
...
Azure Policy는 조직의 표준을 적용 하 고 대규모로 이러한 표준에 대 한 준수를 평가 하는 데 도움이 됩니다. Azure Files 및 Azure 파일 동기화는 배포를 모니터링 하 고 자동화 하는 데 도움이 되는 몇 가지 유용한 감사 및 수정 네트워크 정책을 노출 합니다.

정책은 사용자 환경을 감사 하 고 저장소 계정 또는 저장소 동기화 서비스가 정의 된 동작에서 분기 하는 경우 사용자에 게 알립니다. 예를 들어 공용 끝점을 사용 하지 않도록 설정 하는 정책이 설정 된 경우 공용 끝점을 사용 합니다. 정책 수정/배포는 추가 단계를 수행 하 고 리소스 (예: 저장소 동기화 서비스)를 사전에 수정 하거나 정책에 맞게 리소스 (예: 개인 끝점)를 배포 합니다.

Azure Files 및 Azure 파일 동기화에 대해 다음과 같은 미리 정의 된 정책을 사용할 수 있습니다.

| 작업 | 서비스 | 조건 | 정책 이름 |
|-|-|-|-|
| 감사 | Azure 파일 | 저장소 계정의 공용 끝점이 사용 됩니다. 자세한 내용은 [저장소 계정 공용 끝점에 대 한 액세스 사용 안 함](#disable-access-to-the-storage-account-public-endpoint) 을 참조 하세요. | 스토리지 계정은 네트워크 액세스를 제한해야 함 |
| 감사 | Azure 파일 동기화 | 저장소 동기화 서비스의 공용 끝점을 사용할 수 있습니다. 자세한 내용은 [저장소 동기화 서비스 공용 끝점에 대 한 액세스 사용 안 함](#disable-access-to-the-storage-sync-service-public-endpoint) 을 참조 하세요. | Azure 파일 동기화에 대 한 공용 네트워크 액세스를 사용 하지 않도록 설정 해야 합니다. |
| 감사 | Azure 파일 | 저장소 계정에는 하나 이상의 개인 끝점이 필요 합니다. 자세한 내용은 [저장소 계정 개인 끝점 만들기](#create-the-storage-account-private-endpoint) 를 참조 하세요. | 스토리지 계정은 프라이빗 링크 연결을 사용해야 함 |
| 감사 | Azure 파일 동기화 | 저장소 동기화 서비스에는 하나 이상의 개인 끝점이 필요 합니다. 자세한 내용은 [저장소 동기화 서비스 개인 끝점 만들기](#create-the-storage-sync-service-private-endpoint) 를 참조 하세요. | Azure 파일 동기화 개인 링크를 사용 해야 합니다. |
| 수정 | Azure 파일 동기화 | 저장소 동기화 서비스의 공용 끝점을 사용 하지 않도록 설정 합니다. | 수정-공용 네트워크 액세스를 사용 하지 않도록 Azure 파일 동기화 구성 |
| 배포 | Azure 파일 동기화 | 저장소 동기화 서비스에 대 한 개인 끝점을 배포 합니다. | 개인 끝점을 사용 하 여 Azure 파일 동기화 구성 |
| 배포 | Azure 파일 동기화 | Privatelink.afs.azure.net DNS 영역에 A 레코드를 배포 합니다. | 사설 DNS 영역을 사용 하도록 Azure 파일 동기화 구성 |

### <a name="set-up-a-private-endpoint-deployment-policy"></a>개인 끝점 배포 정책 설정
개인 끝점 배포 정책을 설정 하려면 [Azure Portal](https://portal.azure.com/)로 이동 하 고 **정책을** 검색 합니다. Azure Policy 센터는 최고 결과 여야 합니다.   >  정책 센터의 목차에서 제작 **정의** 로 이동 합니다. 결과 **정의** 창에는 모든 Azure 서비스에 대해 미리 정의 된 정책이 포함 됩니다. 특정 정책을 찾으려면 범주 필터에서 **저장소** 범주를 선택 하거나 **개인 끝점을 사용 하 여 Azure 파일 동기화 구성** 을 검색 합니다. ... **를 선택 하** 고 **할당** 을 선택 하 여 정의에서 새 정책을 만듭니다.

**정책 할당** 마법사의 **기본 사항** 블레이드를 사용 하 여 범위, 리소스 또는 리소스 그룹 제외 목록을 설정 하 고 정책을 구분 하는 데 도움이 되는 친숙 한 이름을 지정할 수 있습니다. 정책이 작동 하기 위해 이러한 작업을 수정할 필요는 없지만 수정 하려는 경우에는 가능 합니다. **다음** 을 선택 하 여 **매개 변수** 페이지로 이동 합니다. 

**매개 변수** 블레이드에서 **privateEndpointSubnetId** 드롭다운 목록 옆 **에 있는 ...** 를 선택 하 여 저장소 동기화 서비스 리소스의 개인 끝점을 배포할 가상 네트워크 및 서브넷을 선택 합니다. 결과 마법사는 구독에서 사용 가능한 가상 네트워크를 로드 하는 데 몇 초 정도 걸릴 수 있습니다. 사용자 환경에 적합 한 가상 네트워크/서브넷을 선택 하 고 **선택** 을 클릭 합니다. **다음** 을 선택 하 여 **재구성** 블레이드로 이동 합니다.

개인 끝점 없이 저장소 동기화 서비스를 식별 하는 경우 개인 끝점을 배포 하려면 **수정** 페이지에서 **수정 작업 만들기** 를 선택 해야 합니다. 마지막으로 **검토 + 만들기** 를 선택 하 여 정책 할당을 검토 **하 고 만들기를 선택** 합니다.

결과 정책 할당은 주기적으로 실행 되며 생성 된 후 즉시 실행 되지 않을 수 있습니다.
