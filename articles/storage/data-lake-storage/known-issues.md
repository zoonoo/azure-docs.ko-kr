...
Blob Storage API는 아직 Azure Data Lake Storage Gen 2 계정에 지원되지 않습니다.
...
관리되지 않는VM(가상 머신) 디스크는 이러한 API에 따라 달라집니다. 따라서 Storage 계정에서 계층 구조 네임스페이스를 사용하려면 계층 구조 네임스페이스를 설정하지 않은 Storage 계정에 관리되지 않는 VM 디스크를 배치하는 것이 좋습니다.

## <a name="azure-storage-explorer"></a>Azure Storage 탐색기

Storage 탐색기의 일부 기능은 Azure Data Lake Storage Gen2 파일 시스템에서 아직 작동하지 않습니다. 이러한 제한 사항은 Azure Portal에 표시되는 버전뿐만 아니라 Azure Storage 탐색기의 [독립 실행형 버전](https://azure.microsoft.com/features/storage-explorer/)에도 적용됩니다.
...
Azure Databricks, HDInsight, Azure Data Factory와 같은 서비스는 Azure AD(Azure Active Directory) OAuth 전달자 토큰 인증을 사용하여 통합되지 않습니다.

## <a name="access-control-lists-acl"></a>ACL(액세스 제어 목록)

디렉터리 및 파일 수준 ACL(액세스 제어 목록)은 관리하기가 어렵습니다. 해당 액세스 제어 목록을 가져오고 설정하는 데 사용할 수 있는 UI 기반 도구가 없습니다.
...
Azure Data Lake Storage Gen2 계정의 파일 시스템 개체에 역할 기반 액세스 제어를 적용할 수 없습니다.

## <a name="sql-data-warehouse-polybase"></a>SQL Data Warehouse PolyBase

Azure Storage 계정에서 Storage 방화벽을 사용하는 경우 SQL Data Warehouse [Polybase](https://docs.microsoft.com/sql/relational-databases/polybase/polybase-guide?view=sql-server-2017)는 해당 계정에 액세스할 수 없습니다.
