...
* [Net use에 성공했지만 Windows 탐색기에 탑재된 Azure 파일 공유가 표시되지 않음](#netuse)
...
## <a name="error-53-when-you-try-to-mount-or-unmount-an-azure-file-share"></a>Azure 파일 공유를 탑재 또는 탑재 해제할 때 “오류 53” 발생
...
Azure 파일 공유를 탑재할 때 수신되는 "시스템 오류 53"은 Azure Files 데이터 센터에 대한 포트 445 아웃바운드 통신이 차단될 경우 발생할 수 있습니다. 포트 445에서 시작되는 액세스를 허용하거나 거부하는 ISP에 대한 요약을 확인하려면 [여기](http://social.technet.microsoft.com/wiki/contents/articles/32346.azure-summary-of-isps-that-allow-disallow-access-from-port-445.aspx)를 클릭하세요.
...
클라이언트에서 NTLMv1 통신이 사용될 경우 "시스템 오류 53"이 수신될 수도 있습니다. NTLMv1을 사용하도록 설정하면 클라이언트 보안이 약화됩니다. 따라서 Azure Files에 대한 통신이 차단됩니다. 이것이 오류의 원인인지 확인하려면 다음 레지스트리 하위 키가 3 값으로 설정되어 있는지 확인합니다.
...
## <a name="net-use-was-successful-but-dont-see-the-azure-file-share-mounted-in-windows-explorer"></a>Net use에 성공했지만 Windows 탐색기에 탑재된 Azure 파일 공유가 표시되지 않음
### <a name="cause"></a>원인
기본적으로 Windows 탐색기는 관리자 권한으로 실행되지 않습니다. 관리자 명령 프롬프트에서 **net use**를 실행할 경우 네트워크 드라이브를 "관리자 권한으로" 매핑합니다. 매핑된 드라이브는 사용자 중심이므로 다른 사용자 계정으로 탑재될 경우 로그인된 사용자 계정에 드라이브가 표시되지 않습니다.
...
이러한 지침을 따른 후 "시스템 오류 1312가 발생했습니다. 지정된 로그온 세션이 없습니다. 이미 종료된 것 같습니다." 메시지가 시스템/네트워크 서비스 계정에 대해 **net use**를 실행할 때 수신될 수 있습니다. 이 문제가 발생하면 **net use**에 전달되는 사용자 이름에 도메인 정보(예: "[저장소 계정 이름].file.core.windows.net")가 포함되는지 확인합니다.
...
## <a name="mount-error-115-when-you-try-to-mount-azure-files-on-the-linux-vm"></a>Linux VM에 Azure Files을 탑재할 때 “탑재 오류 115” 발생
### <a name="cause"></a>원인
Linux 배포는 아직 SMB 3.0의 암호화 기능을 지원하지 않습니다. 일부 배포에서는 사용자가 SMB 3.0을 사용하여 Azure Files를 탑재할 경우 기능 누락으로 인해 “115” 오류 메시지를 수신할 수 있습니다.

### <a name="solution"></a>해결 방법
사용되는 Linux SMB 클라이언트가 암호화를 지원하지 않을 경우 File Storage 계정과 같은 데이터 센터의 Linux VM에서 SMB 2.1을 사용하여 Azure Files를 탑재합니다.
