...
> [!CAUTION]
> 지정 시간 복원은 블록 blob 에서만 처리 되는 작업에 대 한 복원을 지원 합니다. 컨테이너에서 작업 하는 모든 작업은 복원할 수 없습니다. 예를 들어 [컨테이너 삭제](/rest/api/storageservices/delete-container) 작업을 호출 하 여 저장소 계정에서 컨테이너를 삭제 하는 경우 해당 컨테이너는 지정 시간 복원 작업을 사용 하 여 복원할 수 없습니다. 전체 컨테이너를 삭제 하는 대신 나중에 복원할 수 있는 경우 개별 blob을 삭제 합니다.
