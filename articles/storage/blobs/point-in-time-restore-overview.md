---
title: 블록 blob에 대 한 지정 시간 복원 (미리 보기)
...
ms.locfileid: "83195209"
---
# <a name="point-in-time-restore-for-block-blobs-preview"></a>블록 blob에 대 한 지정 시간 복원 (미리 보기)
...
지정 시간 복원을 시작 하려면 [Blob 범위 복원](/rest/api/storagerp/storageaccounts/restoreblobranges) 작업을 호출 하 고 UTC 시간으로 복원 지점을 지정 합니다. 복원 하는 데 사용할 수 있는 사전순의 컨테이너 및 blob 이름을 지정 하거나, 범위를 생략 하 여 저장소 계정의 모든 컨테이너를 복원할 수 있습니다. **Blob 범위 복원** 작업은 작업을 고유 하 게 식별 하는 복원 ID를 반환 합니다.
...
복원 작업을 요청 하는 경우 Azure Storage은 작업 기간 동안 복원 되는 범위의 blob에 대 한 데이터 작업을 차단 합니다. 읽기, 쓰기 및 삭제 작업은 기본 위치에서 차단 됩니다. 저장소 계정이 지리적으로 복제 되는 경우 보조 위치에서 읽기 작업은 복원 작업 중에 진행할 수 있습니다.

한 번에 하나의 복원 작업만 저장소 계정에서 실행할 수 있습니다. 복원 작업은 진행 중인 동안에는 취소할 수 없지만 첫 번째 작업을 실행 취소 하기 위해 두 번째 복원 작업을 수행할 수 있습니다.

지정 시간 복원 상태를 확인 하려면 **Blob 범위 복원** 작업에서 반환 된 복원 ID를 사용 하 여 **복원 상태 가져오기** 작업을 호출 합니다.
...
지정 시간 복원을 사용 하기 전에 저장소 계정에 이러한 기능을 사용 하도록 설정 합니다. 변경 피드 및 blob 버전 관리 미리 보기를 사용 하도록 설정 하기 전에 등록 해야 합니다.
...
- 보관 계층의 blob 복원은 지원 되지 않습니다. 예를 들어 핫 계층의 blob을 2 일 전에 보관 계층으로 이동 하 고 복원 작업이 3 일 전 지점으로 복원 되 면 blob이 핫 계층으로 복원 되지 않습니다.
...
미리 보기에 등록 하려면 Azure PowerShell에서 다음 명령을 실행 합니다.
...
## <a name="ask-questions-or-provide-feedback"></a>질문 하기 또는 피드백 제공

지정 시간 복원 미리 보기에 대해 문의 하거나 피드백을 제공 하려면 Microsoft에 문의 하세요 pitrdiscussion@microsoft.com .

## <a name="next-steps"></a>다음 단계

- [블록 blob에 대 한 지정 시간 복원 설정 및 관리 (미리 보기)](point-in-time-restore-manage.md)
- [Azure Blob Storage의 변경 피드 지원 (미리 보기)](storage-blob-change-feed.md)
