...
description: Azure HDInsight 및 Azure Data Lake Storage Gen2를 사용하여 Spark의 성능을 튜닝하기 위한 지침을 파악합니다.
services: storage
author: normesta
...
ms.locfileid: "95912810"
---
# <a name="tune-performance-spark-hdinsight--azure-data-lake-storage-gen2"></a>성능 튜닝: Spark, HDInsight 및 Azure Data Lake Storage Gen2
...
* **Azure Data Lake Storage Gen2 계정**. 계정을 만드는 방법에 대한 지침은 [빠른 시작: Azure Data Lake Storage Gen2 스토리지 계정 만들기](../common/storage-account-create.md)를 참조하세요.
...
* **Data Lake Storage Gen2에서 Spark 클러스터 실행**.  자세한 내용은 [HDInsight Spark 클러스터를 사용하여 Data Lake Storage Gen2의 데이터 분석](../../hdinsight/spark/apache-spark-use-with-data-lake-store.md)을 참조하세요.
...
- 가상 코어 수 = (클러스터의 노드 수 * 노드의 물리적 코어 수 * 2)
- CPU 제약 조건 = (총 가상 코어 수 / 실행기당 코어 수) / 앱 수

**num-executors 설정** – num-executors 매개 변수는 메모리 제약 조건 및 CPU 제약 조건 중 최소값을 사용하여 결정됩니다. 

실행기 수 = Min(총 가상 코어 수 / 실행기당 코어 수, 사용 가능한 YARN 메모리 / 실행기 메모리)
...
- YARN 코어 수 = 클러스터의 노드 수 * 노드당 코어 수 * 2
- YARN 코어 수 = 8개의 노드 * D14당 8개의 코어 * 2 = 128
- CPU 제약 조건 = (총 YARN 코어 수 /실행기당 코어 수) / 앱 수
- CPU 제약 조건 = (128 / 4) / 2
- CPU 제약 조건 = 16

**num-executors 설정**

- 실행기 수 = Min(메모리 제약 조건, CPU 제약 조건)
- 실행기 수 = Min(16, 16)
- 실행기 수 = 16