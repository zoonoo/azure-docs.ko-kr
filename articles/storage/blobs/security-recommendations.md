...
| Azure 키 볼트로 계정 액세스 키 보호 | Azure AD를 사용하여 Azure 저장소에 대한 요청을 승인하는 것이 좋습니다. 그러나 공유 키 인증을 사용해야 하는 경우 Azure Key Vault를 사용하여 계정 키를 보호합니다. 런타임에 키 자격 증명 모음에서 키를 응용 프로그램에 저장하는 대신 검색할 수 있습니다. Azure 키 자격 증명 모음에 대한 자세한 내용은 [Azure 키 자격 증명 모음 개요를](../../key-vault/key-vault-overview.md)참조하십시오. | - |
