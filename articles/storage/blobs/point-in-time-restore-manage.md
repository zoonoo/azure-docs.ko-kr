...
description: 지정 시간 복원 (미리 보기)을 사용 하 여 블록 blob 집합을 이전 상태로 복원 하는 방법에 대해 알아봅니다.
services: storage
author: tamram
ms.service: storage
ms.topic: how-to
...
ms.locfileid: "90068512"
---
# <a name="enable-and-manage-point-in-time-restore-for-block-blobs-preview"></a>블록 Blob에 대한 지정 시간 복원 설정 및 관리(미리 보기)

특정 시점 복원 (미리 보기)을 사용 하 여 블록 blob 집합을 이전 상태로 복원할 수 있습니다. 이 문서에서는 PowerShell을 사용하여 스토리지 계정에 대한 지정 시점 복원을 사용하도록 설정하는 방법을 설명합니다. PowerShell을 사용하여 복원 작업을 수행하는 방법도 보여줍니다.
...
> [!IMPORTANT]
> 지정 시간 복원 미리 보기는 비프로덕션 용도로만 사용해야 합니다.
...
PowerShell을 사용 하 여 지정 시간 복원을 구성 하려면 먼저 Az. Storage preview module version 1.14.1이 이상 버전의 preview 모듈을 설치 합니다. Az. Storage 모듈의 다른 버전을 제거 합니다.

PowerShellGet의 버전 2.2.4.1 이상을 설치 했는지 확인 합니다. 현재 설치 되어 있는 버전을 확인 하려면 다음 명령을 실행 합니다.

```powershell
Get-InstalledModule PowerShellGet
```

다음으로 Az. Storage preview 모듈을 설치 합니다. 다음 명령은 Az. Storage 모듈의 [2.5.2-preview](https://www.powershellgallery.com/packages/Az.Storage/2.5.2-preview) 버전을 설치 합니다.

```powershell
Install-Module -Name Az.Storage -RequiredVersion 2.5.2-preview -AllowPrerelease
```

Azure PowerShell를 설치 하는 방법에 대 한 자세한 내용은 [Powershellget 설치](/powershell/scripting/gallery/installing-psget) 및 [powershellget을 사용 하 여 Azure PowerShell 설치](/powershell/azure/install-az-ps)를 참조 하세요.
...
블록 blob만 복원 됩니다. 페이지 blob 및 추가 blob은 복원 작업에 포함 되지 않습니다. 추가 blob과 관련 된 제한 사항에 대 한 자세한 내용은 [알려진 문제](#known-issues)를 참조 하세요.
...
PowerShell을 사용 하 여 저장소 계정의 모든 컨테이너와 blob을 복원 하려면 **AzStorageBlobRange** 명령을 호출 하 고 매개 변수를 생략 합니다 `-BlobRestoreRange` . 다음 예는 스토리지 계정의 컨테이너를 현재 시점 12시간 전의 상태로 복원합니다.
...
단일 범위의 blob을 복원 하려면 **AzStorageBlobRange** 명령을 호출 하 고 매개 변수에 대해 사전순으로 컨테이너 및 blob 이름을 지정 `-BlobRestoreRange` 합니다. 예를 들어 *sample-container*라는 단일 컨테이너에 Blob을 복원하려면 *sample-container*로 시작하고 *sample-container1*로 끝나는 범위를 지정하면 됩니다. 시작 및 끝 범위에 명명된 컨테이너가 존재할 필요는 없습니다. 범위의 끝은 제외되기 때문에 스토리지 계정에 *sample-container1*이라는 컨테이너가 포함되어 있어도 *sample-container*라는 컨테이너만 복원됩니다.
...
여러 블록 Blob 범위를 복원하려면 `-BlobRestoreRange` 매개 변수에 범위 배열을 지정합니다. 다음 예에서는 *container1* 및 *container4*의 전체 콘텐츠를 복원 하는 두 범위를 지정 합니다.
...
### <a name="restore-block-blobs-asynchronously-with-powershell"></a>PowerShell을 사용 하 여 비동기적으로 블록 blob 복원
...
추가 blob이 있는 복원 작업의 하위 집합에 대 한 복원 작업은 실패 합니다. 계정에 추가 blob이 있는 경우 미리 보기 중에 특정 시점 복원을 수행 하지 않는 것이 좋습니다.
