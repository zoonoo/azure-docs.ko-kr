...
마지막 Blob 계층 변경 시간은 **액세스 계층 변경 시간** Blob 속성을 통해 노출됩니다. **액세스 계층 변경 시간은** blob 수준 속성으로, 기본 계정 계층이 변경 될 때 업데이트 되지 않습니다. 계정 속성과 blob 속성은 구분 됩니다. 계정의 기본 액세스 계층이 변경 될 때마다 저장소 계정의 모든 blob에 대 한 **액세스 계층 변경 시간** 을 업데이트 하는 데 비용이 많이 듭니다.

핫 또는 쿨 계층의 blob을 덮어쓸 때 새로 만든 blob은 새 blob 액세스 계층이 생성 시 명시적으로 설정 되지 않으면 덮어쓴 blob의 계층을 상속 합니다. Blob이 보관 계층에 있는 경우 덮어쓸 수 없으므로 동일한 blob을 업로드 하는 것은이 시나리오에서 허용 되지 않습니다.
