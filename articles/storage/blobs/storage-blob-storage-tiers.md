...
마지막 Blob 계층 변경 시간은 **액세스 계층 변경 시간** Blob 속성을 통해 노출됩니다. Blob이 보관 계층에 있는 경우 덮어쓸 수 없으므로 동일한 blob을 업로드 하는 것은이 시나리오에서 허용 되지 않습니다. 핫 또는 쿨 계층의 blob을 덮어쓸 때 새 blob은 덮어쓴 blob의 계층을 상속 합니다.
...
1. 설정에서 **구성** 을 클릭 하 여 계정 구성을 확인 하 고 변경 합니다.
...
- **트랜잭션 비용**: 계층의 증가가 증가할수록 모든 계층에 대해 트랜잭션 당 요금이 부과 됩니다.
