...
Azure Storage에서는 다양한 가격 책정 및 기능을 지원하는 세 가지 고유한 계정 옵션을 제공합니다. 사용자가 해당 응용 프로그램에 가장 적합한 옵션을 확인하기 위해 이러한 차이점을 고려해야 합니다.  세 가지 다른 옵션은 다음과 같습니다.

* **범용 v2(GPv2)** 계정은 모든 최신 기능 및 지원 Blob, 파일, 큐 및 테이블을 제공합니다. 현재 이러한 최신 기능에는 Blob 수준 계층화, 보관 저장소, 개략적인 계정 제한 및 저장소 이벤트가 포함됩니다. 가격 책정은 가장 낮은 GB 가격 및 업계에서 경쟁력 있는 트랜잭션 가격을 제공하도록 설계되었습니다.

* **Blob Storage** 계정은 블록 Blob에 모든 최신 기능을 제공하지만 블록 Blob만을 지원합니다.  가격 책정은 범용 v2와 비슷합니다. 대부분의 사용자가 Blob Storage 계정보다 범용 v2를 사용하는 것이 좋습니다.
...
Blob Storage 계정은 계정 수준에서 **액세스 계층** 특성을 노출하며, 기본 저장소 계정 계층을 **핫** 또는 **쿨**로 지정합니다. 기본 저장소 계정 계층은 Blob 수준에서 명시적 계층 집합이 없는 모든 Blob에 적용됩니다. 데이터의 사용 패턴에 변화가 있으면 언제든 이 저장소 계층 간을 전환할 수 있습니다. **보관 계층**은 Blob 수준에서만 적용할 수 있습니다.
...
**Blob Storage 계정**은 GPv2와 동일한 블록 Blob 기능을 지원하지만 블록 Blob에 대해서만 지원됩니다. 고객은 Blob Storage 계정과 GPv2 간에 가격 책정 차이를 검토하고 GPv2로 업그레이드하는 것이 좋습니다. 이 업그레이드는 취소할 수 없습니다.

> [!NOTE]
> Blob Storage 계정은 블록 및 추가 Blob만을 지원하고 페이지 Blob은 지원하지 않습니다.

## <a name="general-purpose-v1"></a>범용 v1
**범용 v1(GPv1)**은 가장 오래된 저장소 계정이며 클래식 배포 모델에서 사용할 수 있는 유일한 종류의 계정입니다. 쿨 및 보관 저장소과 같은 기능은 GPv1에서 사용할 수 없습니다. GPv1의 GB 저장소 비용은 일반적으로 더 높지만, GPv2 또는 Blob Storage 계정보다 트랜잭션 비용이 낮습니다.
...
블록 또는 연결 Blob Storage만 필요한 응용 프로그램의 경우 GPv2 저장소 계정을 사용하여 차별화된 계층형 저장소 가격 책정 모델을 활용하는 것이 좋습니다. 그러나 다음과 같이 GPv1 저장소 계정을 사용하도록 권장한 특정 상황에서 해당 기능을 사용할 수 없을 수 있습니다.

* 여전히 클래식 배포 모델을 사용해야 합니다. Blob Storage 계정은 Azure Resource Manager 배포 모델을 통해서만 사용할 수 있습니다.

* GPv1보다 GPv2 및 Blob Storage 계정에서 많은 비용이 드는 대량의 트랜잭션이나 지역 복제 대역폭을 사용하고, GB 저장소의 비용을 절감할 수 있는 저장소가 충분하지 않습니다.
...
> [!NOTE]
> Blob Storage 계정은 현재 모든 Azure 지역에서 지원됩니다.
...
* GPv2 저장소 계정을 만드는 방법
* GPv1 또는 Blob Storage 계정을 GPv2 저장소 계정으로 변환하는 방법
...
5. 계정 종류 드롭다운 목록에서 **범용 v2**를 선택합니다.
    ...
    저장소 계정 유형을 선택하는 위치입니다. 범용 저장소에서 계층화된 저장소를 사용할 수 없습니다. Blob Storage 형식 계정에서만 사용할 수 있습니다.     
...
6. 저장소 계정의 복제 옵션을 **LRS**, **GRS** 또는 **RA-GRS**로 선택합니다. 기본값은 **RA-GRS**입니다.
    ...
    Azure Storage 복제 옵션에 대한 자세한 내용은 아래의 [Azure Storage 복제](../common/storage-redundancy.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)를 확인하세요.
...
3. 설정 블레이드에서 **구성**을 클릭합니다.

4. 계정 종류 아래에서 **업그레이드**를 클릭합니다.

5. 확인을 위해 오른쪽에 새 블레이드가 표시됩니다. 확인 업그레이드에서 계정 이름을 입력합니다. 
...
기존 저장소 계정을 모니터링하고 이 데이터를 수집하기 위해 로깅을 수행하고 저장소 계정에 대한 메트릭 데이터를 제공하는 Azure 저장소 분석을 사용할 수 있습니다. 저장소 분석은 GPv1, GPv2 및 Blob Storage 계정 형식의 Storage 서비스에 대한 요청과 관련하여 집계된 트랜잭션 통계 및 용량 데이터를 포함하는 메트릭을 저장할 수 있습니다. 이 데이터는 동일한 저장소 계정에서 잘 알려진 테이블에 저장됩니다.
...
> [!NOTE]
> Blob Storage 계정은 해당 계정의 메트릭 데이터를 저장하고 액세스하는 경우에만 테이블 서비스 끝점을 노출합니다. GPv1 ZRS 저장소 계정은 메트릭 데이터를 지원하지 않습니다.
...
> [!NOTE]
> 블록 및 추가 Blob 데이터와 함께 페이지 Blob과 가상 머신 디스크, 또는 큐, 파일이나 테이블을 저장한 범용 저장소 계정이 있는 경우 이 예측 프로세스는 적용되지 않습니다. 즉, 용량 데이터가 다른 형식의 블록 Blob과 다르지 않고 기타 데이터 형식에 대한 용량 데이터를 제공하지 않기 때문입니다. 이러한 형식을 사용하는 경우 가장 최근의 청구서에서 수량을 보는 것이 대체 방법입니다.
...
Blob Storage 계정은 저장 전용 블록 및 추가 Blob에 맞게 특별히 설정됩니다. 테이블, 큐, 파일 및 디스크는 물론 Blob도 저장할 수 있는 기존의 범용 저장소 계정은 Blob Storage 계정으로 변환할 수 없습니다. 저장소 계층을 사용하려면, 새로운 Blob Storage 계정을 만들어서 기존 데이터를 새로 만든 계정으로 마이그레이션해야 합니다.

다음 방법을 사용하여 기존 데이터를 온-프레미스 저장소 장치, 타사 클라우드 저장소 공급자 또는 Azure의 기존 범용 저장소 계정에서 Blob Storage 계정으로 마이그레이션할 수 있습니다.

### <a name="azcopy"></a>AzCopy

AzCopy는 Azure Storage의 데이터를 고속으로 복사하기 위해 설계된 Windows 명령줄 유틸리티입니다. AzCopy를 사용하여 기존 범용 저장소 계정의 데이터를 Blob Storage 계정으로 복사하거나, 온-프레미스 저장소 장치의 데이터를 Blob Storage 계정에 업로드할 수 있습니다.
...
.NET용 Azure Storage 데이터 이동 라이브러리는 AzCopy를 구동하는 핵심 데이터 이동 프레임워크를 기반으로 합니다. 이 라이브러리는 AzCopy와 유사하게 성능이 높고 안정적이며 사용이 간편한 데이터 전송 작업을 제공합니다. 이 라이브러리를 사용하면 AzCopy의 외부 인스턴스를 실행 및 모니터링하지 않고도 응용 프로그램에서 기본적으로 AzCopy가 제공하는 기능을 완벽히 활용할 수 있습니다.

자세한 내용은 [.NET용 Azure Storage 데이터 이동 라이브러리](https://github.com/Azure/azure-storage-net-data-movement)

### <a name="rest-api-or-client-library"></a>REST API 또는 클라이언트 라이브러리

Azure 클라이언트 라이브러리 또는 Azure Storage 서비스 REST API 중 하나를 통해 데이터를 Blob Storage 계정으로 마이그레이션하는 사용자 지정 응용 프로그램을 만들 수 있습니다. Azure Storage는 NET, Java, C++, Node.JS, PHP, Ruby, Python 등, 여러 언어와 플랫폼을 위한 다양한 클라이언트 라이브러리를 제공합니다. 이 클라이언트 라이브러리는 재시도 논리, 로깅, 병렬 업로드와 같은 고급 기능을 제공합니다. HTTP/HTTPS 요청이 가능한 모든 언어로 호출할 수 있는 REST API에 대해 바로 개발할 수도 있습니다.

자세한 내용은 [Azure Blob Storage 시작](storage-dotnet-how-to-use-blobs.md)을 참조하세요.
...
**Blob Storage 계정의 저장소 계층을 얼마나 자주 변경할 수 있나요?**
...
GPv2 및 Blob Storage 계정인 핫 저장소 계층의 Blob에는 GPv1 저장소 계정의 Blob과 동일한 대기 시간이 있습니다. 쿨 저장소 계층의 Blob에는 핫 계층의 Blob과 유사한 대기 시간(밀리초 단위)이 있습니다. 보관 저장소 계층의 Blob에는 몇 시간의 대기 시간이 있습니다.
...
GPv2 저장소 계정은 GPv1 저장소 계정과 매우 유사하며, 높은 내구성 및 가용성, 확장성, 성능 및 보안을 비롯한 Azure Storage의 모든 주요 기능을 지원합니다. Blob Storage 계정 고유의 특징 및 제한 사항과 위에서 설명한 저장소 계층을 제외하고 GPv2 또는 Blob Storage로 업그레이드하는 경우 나머지는 모두 같습니다.

## <a name="next-steps"></a>다음 단계

### <a name="evaluate-blob-storage-accounts"></a>Blob Storage 계정 평가

[지역별 Blob Storage 계정의 가용성 확인](https://azure.microsoft.com/regions/#services)
...
[지역별 Blob Storage 가격 확인](https://azure.microsoft.com/pricing/details/storage/)
...
[Azure Blob Storage 시작](storage-dotnet-how-to-use-blobs.md)
