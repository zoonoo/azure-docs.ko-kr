...
> [!IMPORTANT]
> 이 `storage-preview` 문서에서 설명 하는 확장은 현재 공개 미리 보기 상태입니다.

[Sample](https://github.com/Azure/azure-cli-extensions/tree/master/src/storage-preview#adls-gen2-support) | [Gen1 샘플에 대 한 Gen2 매핑](https://github.com/Azure/azure-cli-extensions/tree/master/src/storage-preview#mapping-from-adls-gen1-to-adls-gen2) | [피드백 제공](https://github.com/Azure/azure-cli-extensions/issues)
...
2. 다음 명령을 사용 하 여 설치 된 Azure CLI 버전이 `2.0.67` 이상 인지 확인 합니다.
   ...
   ```azurecli
    az --version
   ```
   Azure CLI 버전이 보다 `2.0.67`낮은 경우 최신 버전을 설치 합니다. [Azure CLI 설치](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)를 참조하세요.
...
1. Azure CLI를 로컬로 사용 하는 경우 로그인 명령을 실행 합니다.
   ...
   그렇지 않으면에서 [https://aka.ms/devicelogin](https://aka.ms/devicelogin) 브라우저 페이지를 열고 터미널에 표시 된 인증 코드를 입력 합니다. 그런 다음 브라우저에서 계정 자격 증명으로 로그인 합니다.
...
이 예에서는 라는 계정에 `my-directory` 있는 라는 `my-file-system` 파일 시스템에 라는 디렉터리를 추가 `mystorageaccount`합니다.
...
`az storage blob show` 명령을 사용 하 여 디렉터리의 속성을 콘솔에 인쇄할 수 있습니다.
...
`az storage blob directory move` 명령을 사용 하 여 디렉터리 이름을 바꾸거나 이동 합니다.

이 예에서는 이름 `my-directory` 에서 이름 `my-new-directory`으로 디렉터리의 이름을 바꿉니다.
...
이 예에서는 라는 `my-directory` 디렉터리가 `my-file-system` 파일 시스템에 있는지 여부를 나타냅니다. 
...
이 예제에서는 라는 디렉터리에서 `upload.txt` 라는 파일을 다운로드 `my-directory`합니다. 
...
이 예제에서는 라는 `my-directory` `my-file-system` `mystorageaccount`저장소 계정의 파일 시스템에 있는 디렉터리의 콘텐츠를 나열 합니다. 
...
이 예제에서는 라는 디렉터리에 `upload.txt` 라는 파일을 업로드 `my-directory`합니다. 
...
이 예에서는 이름 `my-file.txt` 에서 이름 `my-file-renamed.txt`으로 파일의 이름을 바꿉니다.
...
명령을 사용 하 여 디렉터리의 ACL을 가져옵니다. **directory** `az storage blob directory access show`
...
명령을 사용 하 여 파일에 대 한 액세스 권한을 가져옵니다. **file** `az storage blob access show` 
...
매개 변수를 `-permissions` acl의 약식 형식으로 설정 하 여 디렉터리 또는 파일의 acl을 업데이트 합니다.
...
이 예제에서는 디렉터리 디렉터리에 대 한 사용자 정의 메타 `my-directory` 데이터를 추가 합니다.
...
이 예에서는 라는 `my-directory`디렉터리에 대 한 모든 사용자 정의 메타 데이터를 보여 줍니다.
