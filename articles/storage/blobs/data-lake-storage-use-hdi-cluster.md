...
* **데이터 스토리지 비용:** 컴퓨팅 클러스터의 비용이 Azure 스토리지의 비용보다 높으므로 데이터를 네이티브 HDFS에 오랫동안 저장하는 경우 Azure 스토리지에 저장하는 것보다 비용이 많이 듭니다. 또한 컴퓨팅 클러스터를 생성할 때마다 데이터를 다시 로드할 필요가 없기 때문에 데이터 로드 비용도 절약됩니다.
