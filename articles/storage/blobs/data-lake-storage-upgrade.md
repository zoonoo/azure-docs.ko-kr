...
| **한 번 복사 및 증분 복사** | Azure 데이터 팩터리                                                                                                    | 관리형 클라우드 서비스                                                                                                                | 데이터와 Acl은 현재를 통해 복사할 수 있습니다. ADF에서 증분 복사를 지원하려면 데이터를 시계열 방식으로 구성해야 합니다. 증분 복사 간의 가장 짧은 간격은 [15분](https://docs.microsoft.com/azure/data-factory/how-to-create-tumbling-window-trigger)입니다. |
...
여기서 주요 작업은 접두사가 `adl://` 인 URI를 `abfss://` 접두사가 있는 URI로 변환 하는 것입니다.
...
Data Lake Storage Gen2 파일에 액세스 하기 위한 URI 체계에 대 한 자세한 내용은 [여기](../../hdinsight/hdinsight-hadoop-use-data-lake-storage-gen2.md) 에 자세히 설명 되어 있지만,이는 `abfss://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.windows.net/<PATH>`입니다.
