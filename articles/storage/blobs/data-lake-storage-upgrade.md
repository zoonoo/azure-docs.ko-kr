...
다음 표에서는 사용자 지정 애플리케이션에 사용할 수 있는 API 세트에 대해 설명합니다. 좀 더 명확 하 게 하기 위해 이러한 API 집합을 관리 Api 및 파일 시스템 Api의 2 가지 형식으로 구분 했습니다.

관리 Api는 계정을 관리 하는 데 도움이 되 고 파일 시스템 Api는 파일 및 폴더에 대해 작업을 수행 하는 데 도움이 됩니다.
...
| PowerShell-관리 및 파일 시스템 | [링크](https://docs.microsoft.com/powershell/module/az.datalakestore)                                                                                                                                                                                                                        | 관리 – 지원 되지 않는 파일 시스템- *아직 사용할 수 없음*        | 관리 – *지금 사용 가능 -* [링크](https://docs.microsoft.com/powershell/module/az.storage) <br><br>파일 시스템- *아직 사용할 수 없음* |
...
| 소비          | [Power BI Desktop](https://docs.microsoft.com/azure/data-lake-store/data-lake-store-power-bi)                                                                           | *아직 사용할 수 없음*                                                                                                                                                          | *아직 사용할 수 없음*                                                                                                                             |
...
여기서 주요 작업은 `adl://` `abfss://` 접두사가 있는 uri로의 접두사가 있는 uri를 변환 하는 것입니다.
...
Data Lake Storage Gen2 파일에 액세스 하기 위한 URI 체계에 대 한 자세한 설명은 [여기](../../hdinsight/hdinsight-hadoop-use-data-lake-storage-gen2.md) 에 자세히 `abfss://<FILE_SYSTEM_NAME>@<ACCOUNT_NAME>.dfs.core.windows.net/<PATH>`설명 되어 있습니다.
...
전환 작업이 완료되면 마지막 단계로 철저히 확인해야 합니다. 여기에는 데이터가 안정적으로 복사 되었는지 확인 하 고, Acl이 올바르게 설정 되었는지 확인 하 고, 종단 간 파이프라인이 올바르게 작동 하는지 확인 하는 작업이 포함 되지만이에 제한 되지 않습니다. 확인이 완료되면 이제 이전 파이프라인을 해제하고, 원본 Data Lake Storage Gen1 계정을 삭제하고, Data Lake Storage Gen2 기반 솔루션을 최대한 활용할 수 있습니다.
