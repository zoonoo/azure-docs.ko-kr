---
title: '성능 튜닝: Spark, HDInsight 및 Azure Data Lake Storage Gen2 | Microsoft Docs'
description: Azure HDInsight 클러스터 및 Azure Data Lake Storage Gen2에서 Azure Storm 토폴로지의 성능을 튜닝하기 위한 지침을 파악합니다.
author: normesta
...
---
# <a name="tune-performance-storm-hdinsight--azure-data-lake-storage-gen2"></a>성능 튜닝: Spark, HDInsight 및 Azure Data Lake Storage Gen2
...
* **Azure Data Lake Storage Gen2 계정**. 계정을 만드는 방법에 대한 지침은 [빠른 시작: 분석용 스토리지 계정 만들기](../common/storage-account-create.md)를 참조하세요.
...
* **Data Lake Storage Gen2에서 Storm 클러스터 실행**. 자세한 내용은 [HDInsight의 Storm](../../hdinsight/storm/apache-storm-overview.md)을 참조하세요.
...
* **태스크 수** 각 Bolt는 단일 스레드로 실행됩니다. Bolt당 추가 태스크는 어떠한 동시성도 제공하지 않습니다. 이익이 되는 경우는 튜플을 승인하는 프로세스가 Bolt 실행 시간의 상당 부분을 차지하는 경우 뿐입니다. Bolt에서 승인을 보내기 전에 많은 튜플을 더 크게 추가하여 그룹화하는 것이 좋습니다. 즉, 대부분의 경우에 여러 태스크에는 추가적인 이점이 없습니다.
...
* **시간 초과된 튜플 수가 많음** 토폴로지에서 각 노드를 확인하여 병목 현상이 발생한 위치를 확인합니다. 가장 일반적인 원인은 Bolt가 Spout을 따라갈 수 없기 때문입니다. 이렇게 하면 튜플이 처리될 때까지 대기하는 동안 내부 버퍼를 방해합니다. 시간 초과 값을 늘리거나 보류 중인 최대 Spout을 줄이세요.
