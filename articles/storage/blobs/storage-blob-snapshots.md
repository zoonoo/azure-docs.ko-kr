...
[CreateSnapshotAsync][dotnet_CreateSnapshotAsync]를 호출할 때 조건이 충족 될 때만 스냅숏이 만들어지도록 액세스 조건을 지정할 수 있습니다. 액세스 조건을 지정 하려면 [Accesscondition][dotnet_AccessCondition] 매개 변수를 사용 합니다. 지정 된 조건이 충족 되지 않으면 스냅숏이 생성 되지 않으며 Blob service는 상태 코드 [HTTPStatusCode][dotnet_HTTPStatusCode]를 반환 합니다. Httpstatuscode.preconditionfailed.
