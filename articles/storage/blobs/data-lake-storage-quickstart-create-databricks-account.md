...
1. SQL 문을 실행하여 샘플 JSON 데이터 파일인 **small_radio_json.json** 의 데이터를 사용하여 임시 테이블을 만듭니다. 다음 코드 조각에서 자리 표시자 값을 컨테이너 이름 및 스토리지 계정 이름으로 대체합니다. 앞에서 만든 노트북을 사용하여 노트북의 새 코드 셀에 코드 조각을 붙여넣은 다음, SHIFT+ENTER를 누릅니다.
    ...
    `%sql` 언어 magic 명령을 사용하면 노트북이 다른 유형인 경우에도 노트북에서 SQL 코드를 실행할 수 있습니다. 자세한 내용은 [노트북에서 언어 혼합](/azure/databricks/notebooks/notebooks-use#mix-languages)을 참조하세요.
...
클러스터를 수동으로 종료하지 않으면 클러스터를 만드는 중에 **Terminate after \_\_ minutes of inactivity**(비활성 )_. 이 옵션을 설정하면 클러스터가 지정된 시간 동안 비활성 상태가 된 후에 클러스터가 중지됩니다.
...
- 다른 데이터 원본에서 Azure Databricks로 데이터를 가져오는 방법에 대해 알아보려면 [Spark 데이터 원본](/azure/databricks/data/data-sources/)을 참조하세요.

- Azure Databricks 작업 영역에서 Azure Data Lake Storage Gen2에 액세스하는 다른 방법에 대해 알아보려면 [Azure Data Lake Storage Gen2](/azure/databricks/data/data-sources/azure/azure-datalake-gen2)를 참조하세요.