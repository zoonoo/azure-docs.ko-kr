...
기본적으로 목록 작업은 한 번에 최대 5,000개의 결과를 반환합니다. 좀 더 작은 결과 세트를 반환하려면 **ListBlobs** 메서드 중 하나를 호출할 때 `maxresults` 매개 변수에 대한 0이 아닌 값을 제공합니다.

목록 작업이 5,000개 이상의 Blob을 반환하거나 목록 작업이 스토리지 계정에서 컨테이너의 하위 세트를 반환하도록 `maxresults`의 값을 지정한 경우 Azure Storage는 Blob 목록과 함께 *연속 토큰*을 반환합니다. 연속 토큰은 Azure Storage에서 다음 결과 세트를 검색하는 데 사용할 수 있는 불투명 값입니다.
...
결과와 함께 Blob 메타데이터를 반환하려면 [BlobListingDetails](/dotnet/api/microsoft.azure.storage.blob.bloblistingdetails) 열거에 대한 **Metadata** 값을 지정합니다. Azure Storage에는 반환된 각 Blob이 있는 메타데이터가 포함되어 있으므로 Blob 메타데이터를 검색하기 위해 이 컨텍스트에서 **FetchAttributes** 메서드 중 하나를 호출할 필요가 없습니다.
