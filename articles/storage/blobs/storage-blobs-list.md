...
기본적으로 목록 작업은 한 번에 최대 5000 개의 결과를 반환 합니다. 더 작은 결과 집합을 반환 하려면 **Listblobs** 메서드 중 하나를 호출할 `maxresults` 때 매개 변수에 0이 아닌 값을 제공 합니다.

목록 작업에서 5000 개 이상의 blob을 반환 하거나 목록 작업이 저장소 계정에서 컨테이너의 하위 `maxresults` 집합을 반환 하도록 값을 지정한 경우에는 Azure Storage blob 목록과 함께 *연속 토큰* 을 반환 합니다. 연속 토큰은 Azure Storage에서 다음 결과 집합을 검색 하는 데 사용할 수 있는 불투명 값입니다.
...
Blob을 계층적으로 나열 하려면 목록 `useFlatBlobListing` 메서드의 매개 변수를 **false**로 설정 합니다.
