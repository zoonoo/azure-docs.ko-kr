---
title: '성능 튜닝: MapReduce, HDInsight 및 Azure Data Lake Storage Gen2 | Microsoft Docs'
description: Azure Data Lake Storage Gen2에서 Map Reduce 작업의 성능을 튜닝하기 위한 지침을 파악합니다.
author: normesta
...
ms.locfileid: "95912929"
---
# <a name="tune-performance-mapreduce-hdinsight--azure-data-lake-storage-gen2"></a>성능 튜닝: MapReduce, HDInsight 및 Azure Data Lake Storage Gen2
...
**Mapreduce.map.memory / Mapreduce.reduce.memory** 맵 및/또는 리듀스 태스크에 필요한 메모리 양에 따라 이 숫자를 조정해야 합니다  mapreduce.map.memory 및 mapreduce.reduce.memory에 대한 기본 값은 Yarn 구성을 통해 Ambari에서 볼 수 있습니다.  Ambari에서 YARN으로 이동하여 Configs 탭을 봅니다. YARN 메모리가 표시됩니다.  
...
YARN 컨테이너 수 = 총 YARN 메모리 / mapreduce.map.memory
...
클러스터의 총 메모리는 D14 = 768GB에 대한 8개 노드 * 96GB YARN 메모리입니다.

**4단계: YARN 컨테이너 수 결정**

YARN 컨테이너 수 = 사용 가능한 메모리 768GB / 메모리 3GB =   256
