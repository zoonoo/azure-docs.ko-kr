---
title: '조정 성능: MapReduce, HDInsight & Azure 데이터 레이크 스토리지 Gen2 | 마이크로 소프트 문서'
...
ms.locfileid: "74327919"
---
# <a name="tune-performance-mapreduce-hdinsight--azure-data-lake-storage-gen2"></a>조정 성능: MapReduce, HDInsight & Azure 데이터 레이크 스토리지 Gen2
...
* Data Lake Storage Gen2 계정에 대한 액세스 권한이 있는 **Azure HDInsight 클러스터**. [Azure HDInsight 클러스터를 사용하여 Azure 데이터 레이크 스토리지 Gen2 사용](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-use-data-lake-storage-gen2) 참조
...
**Mapreduce.map.memory / Mapreduce.reduce.memory** 맵 및/또는 리듀스 태스크에 필요한 메모리 양에 따라 이 숫자를 조정해야 합니다  mapreduce.map.memory 및 mapreduce.reduce.memory에 대한 기본 값은 Yarn 구성을 통해 Ambari에서 볼 수 있습니다.  Ambari에서 YARN으로 이동하여 구성 표시기 탭을 봅니다.  YARN 메모리가 표시됩니다.  
...
**2단계: mapreduce.map.map.memory/mapreduce.memory.reduce.memory 설정**
...
mapreduce.job.maps/mapreduce.job.reduces를 튜닝하려면 사용 가능한 YARN 메모리 양을 고려해야 합니다.  이 정보는 Ambari에 제공됩니다.  YARN으로 이동하여 구성 설정 탭을 봅니다.  YARN 메모리가 이 창에 표시됩니다.  YARN 메모리에 클러스터에 있는 노드 수를 곱하여 총 YARN 메모리를 얻습니다.
