...
5. **Azure Slave Plugin** 확인란을 선택합니다.
...
## 구독 프로필로 Azure 슬레이브 플러그인을 구성하려면
...
이는 구독 프로필에서 찾을 수 있습니다. 구독 프로필의 사본이 없다면 [구독 사이트](https://manage.windowsazure.com/publishsettings/Index?SchemaVersion=2.0)에서 다운로드할 수 있습니다. 다음은 구독 프로필의 예입니다.
...
## Azure 슬레이브 플러그인용 가상 컴퓨터 템플릿을 설정하려면
...
5. **Name** 상자에서 Azure 클라우드 서비스 이름을 입력합니다. 입력한 이름이 기존 클라우드 서비스를 참조하는 경우, 가상 컴퓨터는 해당 서비스를 프로비전합니다. 그렇지 않은 경우 Azure는 새로운 서비스를 만듭니다.

6. **Description** 상자에 만드는 템플릿을 설명하는 텍스트를 입력합니다. 이는 레코드만을 위한 것이며 가상 컴퓨터 프로비저닝에는 사용되지 않습니다.
7. **Labels** 상자는 만드는 템플릿을 식별하는 데 사용되며, 추후 Jenkins 작업을 만들 때 참조에 사용됩니다. 용도에 맞게 사용하기 위해 이 상자에 **linux**를 입력합니다.
8. **Region** 목록에서 가상 컴퓨터를 만들 영역을 클릭합니다.
9. **Virtual Machine Size** 목록에서 적절한 크기를 클릭합니다.
10. **Storage Account Name** 상자에서 가상 컴퓨터를 만들 저장소 계정을 지정합니다. 사용할 클라우드 서비스와 동일한 하위 지역에 있는지 확인하십시오. 새 저장소를 만들려는 경우 이 상자를 비워둘 수 있습니다.
11. 보존 시간은 Jenkins가 유휴 슬레이브를 삭제하기까지의 시간을 분 단위로 지정합니다. 기본값인 60으로 둡니다. 슬레이브가 유휴 상태일 때 삭제하는 대신 종료하도록 선택할 수도 있습니다. 이를 수행하려면 **Shutdown Only (Do Not Delete) After Retention Time** 확인란을 선택합니다.
12. **Usage** 목록에서 슬레이브 노드가 사용되는 경우에 적절한 조건을 클릭합니다. 지금은 **Utilize this node as much as possible**을 클릭합니다.
...
13. **Image Family or Id** 상자에서 가상 컴퓨터에 설치할 시스템 이미지를 지정해야 합니다. 이미지 제품군 목록에서 선택하거나 사용자 지정 이미지를 지정할 수 있습니다.
...
14. **Launch Method** 목록에서 **SSH**를 클릭합니다.
15. 아래 스크립트를 복사하여 **Init script** 상자에 붙여넣습니다.
...
16. **Username** 및 **Password** 상자에 가상 컴퓨터에서 만들어질 관리자 계정에 대한 기본 설정값을 입력합니다.
17. **Verify Template**을 클릭하여 지정한 매개 변수가 유효한지 확인합니다.
18. **Save**를 클릭합니다.


## Azure의 슬레이브 노드에서 실행되는 Jenkins 작업을 만들려면
...
3. 프로젝트 형식에 대해서는 **Freestyle project**를 클릭합니다.
4. **Ok**를 클릭합니다.
5. 작업 구성 페이지에서 **Restrict where this project can be run**을 선택합니다.
6. **Label Expression** 상자에 **linux**를 입력합니다. 이전 섹션에서는 **linux**라고 이름을 지정한 슬레이브 템플릿을 만들어 여기에 지정하였습니다.
...
9. **Save**를 클릭합니다.
10. Jenkins 대시보드에서 방금 만든 작업 위로 마우스를 이동하고 드롭다운 화살표를 클릭하여 작업 옵션을 표시합니다.
