...
Microsoft Azure Storage 데이터 이동 라이브러리는 Azure Storage Blob 및 파일의 고성능 업로드, 다운로드 및 복사를 위해 설계된 플랫폼 간 오픈 소스 라이브러리입니다. 이 라이브러리는 [AzCopy](storage-use-azcopy.md)를 지원하는 핵심 데이터 이동 프레임워크입니다. 데이터 이동 라이브러리는 기존의 [.NET Azure Storage 클라이언트 라이브러리](storage-dotnet-how-to-use-blobs.md)에서 사용할 수 없는 편리한 메서드를 제공합니다. 여기에는 병렬 작업 수를 설정하고, 전송 진행률을 추적하며, 취소된 전송을 손쉽게 다시 시작하는 등의 기능이 포함됩니다.  
...
> [!NOTE]
> 이 가이드는 [Azure 저장소](https://azure.microsoft.com/services/storage/)에 대해 잘 알고 있다는 것을 가정합니다. 그렇지 않은 경우 [Azure Storage 소개](storage-introduction.md) 설명서가 도움이 됩니다. [저장소 계정을 만들어](storage-create-storage-account.md#create-a-storage-account) 데이터 이동 라이브러리를 통해 시작해야 한다는 것이 가장 중요합니다.
> 
> 
...
4. Visual Studio Code 마켓플레이스에서 [C# 확장](https://marketplace.visualstudio.com/items?itemName=ms-vscode.csharp)을 설치합니다. Visual Studio Code를 다시 시작합니다. 
...
7. Visual Studio Code를 사용하면 .NET Core 응용 프로그램을 디버그할 수 있습니다. `F5` 키를 눌러 응용 프로그램을 실행하고 설정이 작동하는지 확인합니다. 콘솔에 "Hello World!"가  표시되어야 합니다. 
...
`F5` 키를 눌러 응용 프로그램을 실행합니다. [Microsoft Azure Storage 탐색기](http://storageexplorer.com/)로 저장소 계정을 확인하여 업로드를 수행했는지 확인할 수 있습니다.
...
데이터 이동 라이브러리에서 한 번에 파일 하나씩만 전송할 수 있다면 실망할 것입니다. 다행히도 그렇지는 않습니다. 데이터 이동 라이브러리는 파일 디렉터리와 모든 하위 디렉터리를 전송할 수 있는 기능을 제공합니다. 이렇게 수행할 수 있는 몇 가지 코드를 추가해 보겠습니다.
...
이 메서드와 단일 파일 업로드 메서드 간에는 몇 가지 차이점이 있습니다. 지금 이전에 만든 `TransferManager.UploadDirectoryAsync`와 `getDirectoryTransferContext` 메서드를 사용하고 있습니다. 또한 업로드 작업에 `options` 값도 제공하므로 업로드 시 하위 디렉터리를 포함하도록 나타낼 수 있습니다. 
...
이 시작 자습서에서 Azure Storage와 상호 작용하고 Windows, Linux 및 MacOS에서 실행되는 응용 프로그램을 만들었습니다. 여기서는 Blob Storage에 초점을 맞추었습니다. 그러나 이와 동일한 정보를 File Storage에도 적용할 수 있습니다. 자세한 내용은 [Azure Storage 데이터 이동 라이브러리 참조 설명서](https://azure.github.io/azure-storage-net-data-movement)를 참조하세요.
