...
AzCopy는 CSV가 아닌 JSON 가져오기만 지원합니다.
...
`/XO` 및 `/XN` 매개 변수를 통해 이전 또는 최신 소스 리소스를 각각 복사 작업에서 제외할 수 있습니다. 원본 또는 대상이 테이블인 경우 지원되지 않습니다. 대상에 없는 소스 리소스만 복사하려는 경우 AzCopy 명령에 두 매개 변수를 지정할 수 있습니다.
