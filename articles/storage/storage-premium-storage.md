...
**프리미엄 저장소 디스크**: Azure 프리미엄 저장소는DS 또는 GS 시리즈 Azure VM에 연결할 수 있는 VM 디스크를 지원합니다. 프리미엄 저장소를 사용할 때 P10(128GiB), P20(512GiB), P30(1024GiB)의 세 가지 디스크 크기 중에 선택할 수 있으며, 각각 성능 사양이 다릅니다. 응용 프로그램 요구 사항에 따라 이러한 디스크 중 하나 이상을 DS 또는 GS 시리즈 VM에 연결할 수 있습니다. 사양에 대한 자세한 내용은 [프리미엄 저장소 확장성 및 성능 목표](#premium-storage-scalability-and-performance-targets)에 대한 다음 섹션에서 설명하겠습니다.
...
Azure는 저장소 계정을 운영 체제(OS) 및 데이터 디스크의 컨테이너로 사용합니다. Azure DS 또는GS VM을 만들고 Azure 프리미엄 저장소 계정을 선택하는 경우 운영 체제와 데이터 디스크가 해당 저장소 계정에 저장됩니다.
...
**운영 체제 디스크**: 표준 저장소 계정 또는 프리미엄 저장소 계정에서 호스팅된 운영 체제(OS) 디스크를 사용하도록 DS 시리즈 및 GS 시리즈 Azure 가상 컴퓨터를 구성할 수 있습니다. 부팅에 대해서만 OS 디스크를 사용하는 경우에 OS 디스크 기반 표준 저장소 사용을 고려할 수 있습니다. 이렇게 하면 부팅 후 프리미엄 저장소와 유사한 비용 혜택 및 유사한 성능 결과가 제공됩니다. 부팅이 아닌 OS 디스크에서 추가 작업을 수행하는 경우 더 나은 성능 결과를 제공하므로 프리미엄 저장소를 사용합니다. 예를 들어 응용프로그램이 OS 디스크를 읽고/쓰는 경우 OS 디스크 기반 프리미엄 저장소를 사용하면 VM에 더 나은 성능을 제공합니다.

**데이터 디스크**: 동일한 DS 시리즈 VM 또는 GS 시리즈 VM에 프리미엄 및 표준 저장소 디스크를 모두 사용할 수 있습니다. 프리미엄 저장소를 사용하여 DS 또는 GS 시리즈 VM을 프로비전할 수 있으며 여러 영구 데이터 디스크를 VM에 연결할 수 있습니다. 필요한 경우, 볼륨의 성능과 용량을 늘리도록 디스크에 걸쳐 스트라이핑 할 수 있습니다.
...
> [AZURE.NOTE] 앞서 이 문서의 [DS 및 GS 시리즈 VM](#ds-and-gs-series-vms) 섹션에서 설명한 대로 VM에 디스크 트래픽을 운용하기에 충분한 대역폭이 있는지 확인합니다. 충분하지 않은 경우, 디스크 처리량 및 IOPS가 이전 표에서 언급된 디스크 한도보다는 VM 한도에 기반한 값보다 더 낮게 제한됩니다.
...
- **캐시 적중 수**: 캐시 적중 수는 디스크의 할당된 IOPS/처리량으로 제한되지 않습니다. 예를 들어 DS 시리즈 VM 또는 GS 시리즈 VM에서 캐시가 ReadOnly로 설정된 데이터 디스크를 사용하는 경우 캐시에서 제공하는 읽기는 프리미엄 저장소 디스크 제한의 대상이 아닙니다. 따라서 워크로드가 대부분 읽기인 경우 디스크에서 상당히 높은 처리량을 얻을 수 있습니다. 캐시는 VM 크기에 기반한 VM 단계에서 별도의 IOPS/처리량으로 제한됩니다. DS 시리즈 VM은 대략 캐시 및 로컬 SSD IO에 대해 코어당 4000 IOPS 및 33MB/초여야 합니다. GS 시리즈 VM의 캐시 및 로컬 SSD IO에 대한 제한은 코어당 5000 IOPS 및 50MB/초입니다.
...
프리미엄 저장소를 사용하려면 GS 또는 DS 시리즈 VM을 만들어야 합니다. [Azure 포털에서 Windows 가상 컴퓨터 만들기](../virtual-machines/virtual-machines-windows-tutorial.md)의 단계에 따라 새 DS 또는 GS 가상 컴퓨터를 만듭니다.

#### III. Azure 포털을 통해 프리미엄 저장소 데이터 디스크 연결

1. Azure 포털에서 새로운 또는 기존 DS 또는 GS VM을 찾습니다.
...
VM에 추가 디스크 공간을 사용하려면 콘솔 창에서 다음 PowerShell cmdlet을 실행하여 생성 후 기존 DS 시리즈 VM 또는 GS 시리즈 VM에 새 데이터 디스크를 연결합니다.
...
1. **프리미엄 및 표준 데이터 디스크를 모두 DS 또는 GS 시리즈 VM에 연결할 수 있나요?**

	예. 프리미엄 및 표준 데이터 디스크를 모두 DS 또는 GS 시리즈 VM에 연결할 수 있습니다.

2. **프리미엄 및 표준 데이터 디스크를 모두 D 또는 G 시리즈 VM에 연결할 수 있나요?**

	아니요. DS 또는 GS 시리즈가 아닌 모든 VM에는 표준 데이터 디스크만 연결할 수 있습니다.
...
5. **내 DS 또는 GS 시리즈 VM에 대한 부팅 진단은 어디에 저장할 수 있나요?**

	표준 저장소 계정을 만들어서 DS 또는 GS 시리즈 VM의 부팅 진단을 저장할 수 있습니다.
...
7. **DS 또는 GS 시리즈 VM의 로컬 SSD는 무엇입니까?**

	로컬 SSD는 DS 또는 GS 시리즈 VM에 포함되어 있는 임시 저장소입니다. 이 임시 저장소에 대한 추가 비용은 없습니다. 이 임시 저장소나 로컬 SSD는 Azure Blob 저장소에 보존되지 않으므로 응용 프로그램 데이터를 저장하는 용도로 사용하지 않는 것이 좋습니다.
