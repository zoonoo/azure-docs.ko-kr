...
이 문서에서는 Visual Studio의 **연결된 서비스 추가** 대화 상자를 사용하여 ASP.NET 5 프로젝트에서 Azure 저장소 계정을 만들거나 참조한 후 Visual Studio에서 Azure 테이블 저장소를 사용하는 방법을 설명합니다. **연결된 서비스 추가** 작업은 프로젝트의 Azure 저장소에 액세스하는 데 적합한 NuGet 패키지를 설치하고 프로젝트 구성 파일에 저장소 계정에 대한 연결 문자열을 추가합니다.

시작하려면 먼저 저장소 계정에서 Azure 큐를 만들어야 합니다. Visual Studio **서버 탐색기**에서 큐를 만드는 방법을 보여 드리겠습니다. 원하는 경우 코드에서 큐를 만드는 방법도 보여 드리겠습니다.

또한 큐 메시지 추가, 수정, 읽기 및 제거와 같은 기본 큐 작업을 수행하는 방법을 보여 드립니다. 샘플은 C# 코드로 작성되었으며 Azure Storage Client Library for .NET을 사용합니다. ASP.NET에 대한 자세한 내용은 [ASP.NET(영문)](http://www.asp.net)을 참조하세요.
...
자세한 내용은 [.NET에서 큐 저장소를 사용하는 방법](storage-dotnet-how-to-use-queues.md/ ".NET에서 큐 저장소를 사용하는 방법(영문)")(영문)을 참조하세요.
...
Visual Studio **서버 탐색기**를 사용하는 대신 코드에서 Azure 큐를 만들려면 `CreateIfNotExistsAsync()` 호출을 추가하면 됩니다.
