...
[.NET용 Azure 저장소 클라이언트 라이브러리는](/dotnet/api/overview/azure/storage?view=azure-dotnet) Azure Storage에 업로드하기 전에 클라이언트 응용 프로그램 내의 데이터를 암호화하고 클라이언트에 다운로드하는 동안 데이터를 해독하는 것을 지원합니다. 라이브러리 또한 스토리지 계정 키 관리를 위해 [Azure Key Vault](https://azure.microsoft.com/services/key-vault/)와의 통합을 지원합니다.
...
암호화된 Blob을 다운로드하려면 **DownloadTo**/**BlobReadStream** 편의 방법을 사용하여 전체 Blob의 콘텐츠를 검색해야 합니다. 래핑된 CEK는 IV (blob 메타 데이터로 저장된 경우)와 함께 암호해독되고 사용되어 지며 해독된 데이터가 사용자에게 돌아갑니다.
