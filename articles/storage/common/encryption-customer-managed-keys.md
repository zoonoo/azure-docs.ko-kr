...
고객 관리 키를 사용 또는 비활성화하거나, 키 버전을 업데이트하거나, 다른 키를 지정하여 Azure Storage 암호화에 사용되는 키를 수정할 때 루트 키의 암호화가 변경되지만 Azure Storage 계정의 데이터는 변경되지 않습니다. 다시 암호화해야 합니다.
