---
title: Azure Portal를 사용 하 여 RBAC를 사용 하 여 blob에 대 한 Azure AD 액세스 권한 및 큐 데이터 관리-Azure Storage | Microsoft Docs
description: Azure Portal에서 RBAC (역할 기반 액세스 제어)를 사용 하 여 컨테이너 및 큐에 대 한 액세스 권한을 보안 주체에 게 할당 합니다. Azure Storage는 Azure AD를 통해 인증에 대 한 기본 제공 및 사용자 지정 RBAC 역할을 지원 합니다.
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
1. 적절 한 Azure Storage RBAC 역할을 할당 하 여 Azure AD 보안 주체에 대 한 액세스 권한을 부여 합니다.
...
1. **Save**를 클릭합니다. 역할을 할당받은 ID가 해당 역할에 따라 나열되어 표시됩니다. 예를 들어 다음 이미지에서는 추가된 사용자에게 이제 *sample-container*라는 컨테이너의 데이터에 대한 읽기 권한이 있음을 보여줍니다.
...
사용자가 Azure Portal blob에 액세스할 수 있어야 하는 경우에는 저장소 계정 수준에서 해당 사용자에 게 추가 RBAC 역할 인 [읽기 권한자](../../role-based-access-control/built-in-roles.md#reader) 역할을 할당 합니다. **읽기 권한자** 역할은 사용자가 저장소 계정 리소스를 볼 수 있지만 수정할 수는 없도록 허용 하는 Azure Resource Manager 역할입니다. Azure Storage 데이터에 대 한 읽기 권한은 제공 하지 않고 계정 관리 리소스만 제공 합니다.
