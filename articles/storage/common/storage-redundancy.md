...
| ___ 저장소 계정 유형에서 사용 가능 |GPv1, GPv2, Blob |GPv2 |GPv1, GPv2, Blob |GPv1, GPv2, Blob
...
[Azure Portal](https://portal.azure.com/), [Azure Powershell](storage-powershell-guide-full.md), [Azure CLI](https://docs.microsoft.com/en-us/cli/azure/install-azure-cli?view=azure-cli-latest) 또는 여러 [Azure 클라이언트 라이브러리](https://docs.microsoft.com/en-us/azure/index?view=azure-dotnet#pivot=sdkstools) 중 하나를 사용하여 저장소 계정의 복제 전략을 변경할 수 있습니다. 저장소 계정의 복제 유형을 변경해도 가동 중지 시간이 발생하지 않습니다.

   > [!NOTE]
   > 현재는 포털 또는 API를 사용하여 계정을 ZRS로 변환할 수 없습니다. 하지만 ZRS의 일반 공급 후에 LRS, GRS, RA-GRS에서 ZRS로의 마이그레이션을 지원할 계획입니다. 자세한 내용은 [ZRS(영역 중복 저장소)](storage-redundancy-zrs.md)를 참조하세요.
