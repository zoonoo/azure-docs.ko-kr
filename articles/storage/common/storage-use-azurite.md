...
이 설치 방법을 사용 하려면 [node.js 버전 8.0 이상이](https://nodejs.org) 설치 되어 있어야 합니다. Npm (node Package Manager)는 모든 node.js 설치에 포함 된 패키지 관리 도구입니다. Node.js를 설치한 후 다음 `npm` 명령을 실행 하 여 Azurite를 설치 합니다.
...
이 명령은 Azurite에 게 *c:\azurite*의 특정 디렉터리에 모든 데이터를 저장 하도록 지시 합니다. 이 `--location` 옵션을 생략 하면 현재 작업 디렉터리가 사용 됩니다.
...
**선택 사항** -또는 `-h` `--help` 스위치를 사용 하 여 명령줄 도움말을 가져옵니다.
...
**선택 사항** -기본적으로 Azurite는 로컬 서버로 127.0.0.1을 수신 합니다. `--blobHost` 스위치를 사용 하 여 요구 사항에 대 한 주소를 설정 합니다.
...
**선택 사항** -기본적으로 Azurite는 로컬 서버로 127.0.0.1을 수신 합니다. `--queueHost` 스위치를 사용 하 여 요구 사항에 대 한 주소를 설정 합니다.
...
**선택 사항** -Azurite은 실행 중에 데이터를 로컬 디스크에 저장 합니다. `-l` 또는 `--location` 스위치를 사용 하 여 경로를 작업 영역 위치로 지정 합니다. 기본적으로 현재 프로세스 작업 디렉터리가 사용 됩니다. 소문자 ' l '을 적어둡니다.
...
**선택 사항** -기본적으로 액세스 로그는 콘솔 창에 표시 됩니다. 또는 `-s` `--silent` 스위치를 사용 하 여 액세스 로그를 표시 하지 않습니다.
...
**선택 사항** -디버그 로그에는 모든 요청 및 예외 스택 추적에 대 한 자세한 정보가 포함 됩니다. `-d` 또는 `--debug` 스위치에 올바른 로컬 파일 경로를 제공 하 여 디버그 로그를 사용 하도록 설정 합니다.
...
**선택 사항** -기본적으로 Azurite는 strict 모드를 적용 하 여 지원 되지 않는 요청 헤더와 매개 변수를 차단 합니다. 또는 `-L` `--loose` 스위치를 사용 하 여 strict 모드를 사용 하지 않도록 설정 합니다. 대문자 ' L '을 확인 합니다.
...
**선택 사항** -또는 `-v` `--version` 스위치를 사용 하 여 설치 된 Azurite 버전 번호를 표시 합니다.
...
PEM `--cert` 파일에 대해가 제공 되는 경우 해당 `--key` 스위치를 제공 해야 합니다.

```console
azurite --cert path/server.pem --key path/key.pem
```

PFX `--cert` 파일에 대해가 제공 되는 경우 해당 `--pwd` 스위치를 제공 해야 합니다.
...
**선택 사항** -스위치를 `--oauth` 사용 하 여 Azurite에 대 한 OAuth 인증을 사용 하도록 설정 합니다.
...
> [!NOTE]
> OAuth에는 HTTPS 끝점이 필요 합니다. 스위치와 함께 스위치를 `--cert` `--oauth` 제공 하 여 HTTPS를 사용 하도록 설정 해야 합니다.

Azurite는 `--oauth` 스위치에 매개 변수를 `basic` 지정 하 여 기본 인증을 지원 합니다. Azurite는 들어오는 전달자 토큰의 유효성을 검사 하 고 발급자, 대상 그룹 및 만료를 확인 하는 등의 기본 인증을 수행 합니다. Azurite는 토큰 서명 또는 사용 권한을 확인 하지 않습니다.
...
Azure Sdk를 사용 하는 경우 `--oauth basic and --cert --key/--pwd` 옵션을 사용 하 여 Azurite를 시작 합니다.
...
Azurite는 다음과 같은 형식으로 환경 변수를 `AZURITE_ACCOUNTS` 설정 하 여 사용자 지정 저장소 계정 이름 및 `account1:key1[:key2];account2:key1[:key2];...`키를 지원 합니다.
...
> [!NOTE]
> 사용자 지정 `devstoreaccount1` 저장소 계정을 설정 하는 경우 기본 저장소 계정이 사용 하지 않도록 설정 됩니다.
...
자체 서명 된 `dotnet dev-certs` 인증서를 생성 하는 데를 사용한 경우에는 다음 연결 문자열을 사용 합니다.
...
기본적으로 Storage 탐색기 자체 서명 된 인증서를 사용 하는 HTTPS 끝점은 열리지 않습니다. HTTPS를 사용 하 여 Azurite를 실행 하는 경우 자체 서명 된 인증서를 사용할 가능성이 높습니다. Storage 탐색기**에서 ssl 인증서** ->  **편집** -> **인증서 가져오기** 대화 상자를 통해 ssl 인증서를 가져옵니다.
...
Azurite는 읽기 액세스 지역 중복 복제 (RA-GRS)를 지원 합니다. 저장소 리소스의 경우 계정 이름에를 추가 `-secondary` 하 여 보조 위치에 액세스 합니다. 예를 들어 Azurite의 읽기 전용 보조 데이터베이스를 사용 하 여 blob에 액세스 하기 위해 다음 주소를 사용할 수 있습니다.
