...
번호 페이지 Blob은 계정의 저장소 계층을 유추하지만 가격 책정 또는 가용성에 영향을 주지 않습니다. 페이지 Blob의 액세스 계층을 핫, 쿨 또는 아카이브로 변경할 수 없습니다. Blob 계층 설정 작업은 프리미엄 저장소 계정의 페이지 Blob에서 허용되지만 프리미엄 페이지 Blob의 허용되는 크기, IOPS 및 대역폭만을 결정합니다. 자세한 내용은 [Blob 계층 설정](https://docs.microsoft.com/en-us/rest/api/storageservices/set-blob-tier)을 참조하세요.
