...
* [**GPv2(범용 v2)** 계정](#general-purpose-v2-accounts)
* [**GPv1(범용 v1)** 계정](#general-purpose-v1-accounts)
...
범용 v2(GPv2) 계정은 Blob, 파일, 큐 및 테이블에 대한 모든 최신 기능을 지원하는 저장소 계정입니다. GPv2 계정은 GPv1(범용 v1) 및 Blob 저장소 계정에서 지원하는 모든 API, 서비스 및 기능을 지원합니다. 모든 저장소 계정 유형에서 제공하는 것과 동일한 내구성, 가용성, 확장성 및 성능을 유지합니다. GPv2 계정에 대한 가격 책정은 기가바이트당 가장 낮은 가격 및 업계에서 경쟁력 있는 트랜잭션 가격을 제공하도록 설계되었습니다.

Azure Portal, PowerShell 또는 Azure CLI를 사용하여 GPv1 또는 Blob Storage 계정을 GPv2 계정으로 업그레이드할 수 있습니다. 

GPv2 저장소 계정의 블록 Blob의 경우 사용 패턴에 따라 계정 수준에서 핫 또는 쿨 저장소 액세스 계층을 선택하고 Blob 수준에서 핫, 쿨 또는 보관 액세스 계층을 선택할 수 있습니다. 핫, 쿨 및 보관 저장소 계층에 자주 액세스하고, 드물게 액세스하고 거의 액세스하지 않는 데이터를 저장하여 저장소 및 트랜잭션 비용을 최적화합니다. 

GPv2 저장소 계정은 계정 수준에서 **액세스 계층** 특성을 노출하며, 기본 저장소 계정 계층을 **핫** 또는 **쿨**로 지정합니다. 기본 저장소 계정 계층은 Blob 수준에서 명시적 계층 집합이 없는 모든 Blob에 적용됩니다. 데이터의 사용 패턴에 변화가 있으면 언제든 이 저장소 계층 간을 전환할 수 있습니다. **보관** 계층은 Blob 수준에서만 적용할 수 있습니다.
...
사용자는 언제든지 Azure Portal, PowerShell 또는 Azure CLI를 사용하여 GPv1 또는 Blob Storage 계정을 GPv2 계정으로 업그레이드할 수 있습니다. 이 변경 내용은 되돌릴 수 없으며 다른 계정 유형 변경은 허용되지 않습니다. 기존 저장소 계정을 평가하는 데 대한 자세한 내용은 [GPv2 저장소 계정 평가 및 마이그레이션](#evaluating-and-migrating-to-gpv2-storage-accounts) 섹션을 참조하세요.
* [Azure Portal을 사용하여 GPv2로 업그레이드](#upgrade-with-azure-portal)
* [PowerShell을 사용하여 GPv2로 업그레이드](#upgrade-with-powershell)
* [Azure CLI를 사용하여 GPv2로 업그레이드](#upgrade-with-azure-cli)
...
PowerShell을 사용하여 GPv1 또는 Blob Storage 계정을 GPv2 계정으로 업그레이드하려면 먼저 최신 버전의 **AzureRm.Storage** 모듈을 사용하도록 PowerShell을 업데이트합니다. PowerShell 설치에 대한 자세한 내용은 [Azure PowerShell 설치 및 구성](https://docs.microsoft.com/powershell/azure/install-azurerm-ps)을 참조하세요. 그런 다음 리소스 그룹의 이름 및 저장소 계정을 대체하여 계정을 업그레이드하도록 다음 명령을 호출합니다.
...
Azure CLI를 사용하여 GPv1 또는 Blob Storage 계정을 GPv2 계정으로 업그레이드하려면 먼저 최신 버전의 Azure CLI를 설치합니다. CLI 설치에 대한 자세한 내용은 [Azure CLI 2.0 설치](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)를 참조하세요. 그런 다음 리소스 그룹의 이름 및 저장소 계정을 대체하여 계정을 업그레이드하도록 다음 명령을 호출합니다.
...
최신 블록 또는 연결 BLOB 기능이 필요한 응용 프로그램의 경우 GPv2 저장소 계정을 사용하여 차별화된 계층형 저장소 가격 책정 모델을 활용하는 것이 좋습니다. 그러나 다음과 같은 특정 시나리오에서 GPv1을 사용하는 것이 좋습니다.

* 여전히 클래식 배포 모델을 사용해야 합니다. GPv2 및 Blob Storage 계정은 Azure Resource Manager 배포 모델을 통해서만 사용할 수 있습니다.
...
* [GPv2 또는 Blob Storage 계정에서 계정 계층을 설정하는 방법](#change-the-storage-tier-of-a-gpv2-storage-account-using-the-azure-portal)
* [GPv2 또는 Blob Storage 계정에서 Blob 계층을 설정하는 방법](#change-the-storage-tier-of-a-blob-using-the-azure-portal)
...
2. 저장소 계정으로 이동하려면 **모든 리소스**를 선택한 다음, 저장소 계정을 선택합니다.
...
이 섹션은 사용자가 GPv1 저장소 계정에서 GPv2 저장소 계정을 사용하는 과정을 원활하게 전환할 수 있도록 합니다. 두 가지 사용자 시나리오가 있습니다.
...
두 경우에 첫 번째 우선 순위는 GPv2 저장소 계정에 저장된 데이터를 저장하고 액세스하고 운영하는 비용을 산정하고 현재 비용과 비교하는 것입니다.
...
* 저장소 액세스 패턴(작업 및 데이터 전송)
    - 읽어오는 데이터(송신)의 양과 저장소 계정에 기록되는 데이터(수신)의 양 
    - 저장소 계정의 데이터에 대해 발생하는 작업의 수
    - 데이터를 처리하는 작업 (읽기 및 쓰기)의 종류
...
> [!NOTE]
> 블록 및 추가 Blob 데이터와 함께 페이지 Blob과 가상 머신 디스크, 또는 큐, 파일이나 테이블이 저장된 범용 저장소 계정이 있는 경우 이 예측 프로세스는 적용되지 않습니다. 용량 데이터는 다른 형식의 블록 Blob와 다르지 않으므로 기타 데이터 형식에 대한 용량 데이터를 제공하지 않습니다. 이러한 형식을 사용하는 경우 가장 최근의 청구서에서 수량을 보는 것이 대체 방법입니다.
...
Blob Storage 계정에 대한 데이터 액세스 비용을 예상하려면 트랜잭션을 두 그룹으로 세분화해야 합니다.
...
**GPv2 또는 Blob Storage 계정의 저장소 계층을 얼마나 자주 변경할 수 있나요?**
...
아니요. Blob 저장소 계정은 블록 및 추가 Blob만 지원하고 페이지 Blob은 지원하지 않습니다. Azure 가상 머신 디스크는 페이지 Blob에 의해 지원되며, 결과적으로 Blob 저장소 계정은 가상 머신 디스크를 저장하는 데 사용될 수 없습니다. 하지만 가상 컴퓨터 디스크의 백업을 Blob 저장소 계정의 블록 Blob으로 저장하는 것은 가능합니다. Blob 저장소 계정 대신 GPv2를 사용하도록 고려해야 하는 여러 가지 이유 중 하나입니다.

**GPv2 저장소 계정에서 페이지 Blob을 계층으로 지정할 수 있나요?**

아니요. 페이지 Blob은 계정의 저장소 계층을 유추하지만 가격 책정 또는 가용성에 영향을 주지 않습니다. 페이지 Blob의 액세스 계층을 핫, 쿨 또는 아카이브로 변경할 수 없습니다. Blob 계층 설정 작업은 프리미엄 저장소 계정의 페이지 Blob에서 허용되지만 프리미엄 페이지 Blob의 허용되는 크기, IOPS 및 대역폭만을 결정합니다. 자세한 내용은 [Blob 계층 설정](https://docs.microsoft.com/en-us/rest/api/storageservices/set-blob-tier)을 참조하세요.
