...
가상 컴퓨터 디스크 트래픽(탑재 및 분리 작업 및 디스크 IO 포함)은 네트워크 규칙의 영향을 받지 **않습니다**.  미리 보기 중에 관리되지 않는 디스크의 백업은 보호된 저장소 계정에 대해 지원되지 않습니다.  페이지 blob(Virtual Machine 디스크에 사용)에 대한 REST 액세스는 네트워크 규칙에 의해 보호됩니다.
