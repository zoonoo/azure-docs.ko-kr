---
title: 공유 키 권한 부여를 사용 하 고 REST API 작업 호출
titleSuffix: Azure Storage
description: Azure 저장소 REST API를 사용하여 공유 키 권한 부여를 사용하여 Blob 저장소에 요청을 합니다.
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
---
# <a name="call-rest-api-operations-with-shared-key-authorization"></a>공유 키 권한 부여를 사용 하 고 REST API 작업 호출

이 문서에서는 권한 부여 헤더를 형성하는 방법을 포함하여 Azure 저장소 REST API를 호출하는 방법을 보여 주며, 여기에는 REST에 대해 아무것도 모르고 REST 호출하는 방법을 전혀 모르는 개발자의 관점에서 작성되었습니다. REST 작업을 호출하는 방법을 배운 후 이 지식을 활용하여 다른 Azure Storage REST 작업을 사용할 수 있습니다.

## <a name="prerequisites"></a>사전 요구 사항

샘플 응용 프로그램에는 저장소 계정에 대한 Blob 컨테이너가 나열됩니다. 이 문서의 코드를 사용해 보려면 다음 항목이 필요합니다.
...
[git을](https://git-scm.com/) 사용하여 응용 프로그램의 복사본을 개발 환경에 다운로드합니다.
...
## <a name="about-rest"></a>REST에 대해

REST는 *표현 상태 전송을*의미합니다. 구체적인 정의는 [Wikipedia](https://en.wikipedia.org/wiki/Representational_state_transfer)를 확인하세요.

REST는 HTTP/HTTPS와 같은 인터넷 프로토콜을 통해 서비스와 상호 작용할 수 있는 아키텍처입니다. REST는 서버 또는 클라이언트에서 실행되는 소프트웨어와 는 독립적입니다. REST API는 HTTP/HTTPS를 지원하는 모든 플랫폼에서 호출할 수 있습니다. Mac, Windows, Linux, Android 휴대폰 또는 태블릿, iPhone, iPod, 웹 사이트에서 실행되는 애플리케이션을 작성할 수 있으며 이 모든 플랫폼에 동일한 REST API를 사용할 수 있습니다.

REST API에 대한 호출은 클라이언트가 만든 요청과 서비스에서 반환되는 응답으로 구성됩니다. 요청에서 호출할 작업, 수행할 리소스, 쿼리 매개 변수 및 헤더에 대한 정보와 호출된 작업에 따라 데이터 페이로드가 포함된 URL을 보냅니다. 서비스의 응답에는 상태 코드, 응답 헤더 집합 및 호출된 작업에 따라 데이터 페이로드가 포함됩니다.
...
## <a name="list-containers-operation"></a>컨테이너 작업 목록

ListContainers 작업에 대한 참조를 [검토합니다.](/rest/api/storageservices/List-Containers2) 이 정보는 요청 및 응답에서 일부 필드의 출처를 이해하는 데 도움이 됩니다.

**요청 메서드**: GET. 이 동사는 요청 개체의 속성으로 지정되는 HTTP 메서드입니다. 호출하는 API에 따라 이 동사의 다른 값으로 HEAD, PUT 및 DELETE가 포함됩니다.

**URI**요청 `https://myaccount.blob.core.windows.net/?comp=list`: .요청 URI는 Blob 저장소 계정 끝점 `http://myaccount.blob.core.windows.net` 및 `/?comp=list`리소스 문자열에서 만들어집니다.
...
응답 헤더 **:** 여기에는 *콘텐츠 유형이* [포함됩니다.](/rest/api/storageservices/List-Containers2#response-headers) *x-ms-request-id는*전달한 요청 ID입니다. 사용되는 Blob 서비스의 버전을 나타내는 *x-ms 버전;* 및 UTC에 있는 *날짜이며*요청이 이루어진 시간을 알려줍니다.
...
프로덕션 환경에서 실행할 때 보안을 위해 항상 HTTP 대신 HTTPS를 사용합니다. 이 연습에서는 목적을 달성하기 위해 HTTP를 사용할 것이므로 요청 및 응답 데이터를 볼 수 있습니다. 실제 REST 호출에서 요청 및 응답 정보를 보려면 [Fiddler](https://www.telerik.com/fiddler) 또는 유사한 애플리케이션을 다운로드합니다. Visual Studio 솔루션에서 저장소 계정 이름과 키는 클래스에서 하드 코딩됩니다. ListContainersAsyncREST 메서드는 저장소 계정 이름 및 저장소 계정 키를 REST 요청의 다양 한 구성 요소를 만드는 데 사용 되는 메서드에 전달 합니다. 실제 애플리케이션에서 스토리지 계정 이름과 키는 구성 파일이나 환경 변수에 상주하거나 Azure Key Vault에서 검색할 수 있습니다.

샘플 프로젝트에서 권한 부여 헤더를 만들기 위한 코드는 별도의 클래스에 있습니다. 아이디어는 전체 클래스를 가져 와서 자신의 솔루션에 추가하고 "있는 것처럼"사용할 수 있다는 것입니다. 인증 헤더 코드는 Azure Storage에 대한 대부분의 REST API 호출에 사용할 수 있습니다.
...
및 `x-ms-date` `x-ms-version`에 대한 요청 헤더를 추가합니다. 코드의 이 위치는 호출에 필요한 추가 요청 헤더를 추가하는 위치이기도 합니다. 이 예에는 추가 헤더가 없습니다. 추가 헤더를 전달하는 API의 예로는 컨테이너 ACL 설정 작업이 있습니다. 이 API 호출은 "x-ms-blob-public-access"라는 헤더와 액세스 수준에 대한 값을 추가합니다.
...
요청을 생성했으니 SendAsync 메서드를 호출하여 Azure Storage로 보낼 수 있습니다. 응답 상태 코드의 값이 200이면 작업이 성공했는지 확인합니다. 다음으로 응답을 구문 분석합니다. 이 예에서는 컨테이너의 XML 목록을 가져옵니다. 요청을 만들고 실행한 다음, 컨테이너 목록에 대한 응답을 검사하려면 GetRESTRequest 메서드를 호출하는 코드를 살펴보겠습니다.
...
**응답 본문(XML):** 컨테이너 목록 작업의 경우 컨테이너 목록 및 해당 속성이 표시됩니다.
...
> [!TIP]
> 이제 Azure Storage는 Blob 및 큐에 대한 Azure Active Directory(Azure AD) 통합을 지원합니다. Azure AD는 Azure Storage에 대한 요청에 권한을 부여하는 훨씬 간단한 환경을 제공합니다. Rest 작업을 승인하기 위해 Azure AD를 사용하는 방법에 대한 자세한 내용은 [Azure Active Directory](/rest/api/storageservices/authorize-with-azure-active-directory)에 대한 권한 부여 를 참조하십시오. Azure 저장소와의 Azure AD 통합에 대한 개요는 [Azure Active Directory를 사용하여 Azure 저장소에 대한 액세스 인증을](storage-auth-aad.md)참조하세요.

[Azure Storage에 대한 요청을 인증하는](/rest/api/storageservices/authorize-requests-to-azure-storage)방법을 개념적으로 설명하는 문서가 있습니다.
...
서명 필드는 요청에서 생성되어 SHA256 알고리즘을 사용하여 계산된 후 Base64 인코딩을 사용하여 인코딩되는 해시 기반 메시지 인증 코드(HMAC)입니다. 무슨 말인지 이해하셨나요? (거기에 매달려, 당신은 아직 *정식화* 된 단어를 듣지 못했습니다.)
...
CanonicalizedHeaders 및 CanonicalizedResource란 무엇인가요? 좋은 질문입니다. 정식화의 의미가 무엇일까요? Microsoft Word에서는 이 단어를 인지하지도 못합니다. [위키백과가 정식화에 대해 말하는](https://en.wikipedia.org/wiki/Canonicalization)내용: *컴퓨터 과학에서 정식화(때로는 표준화 또는 정규화)는 두 개 이상의 가능한 표현을 "표준", "정상" 또는 정식 형식으로 변환하는 프로세스입니다.* 다시 말해서, 항목 목록(예: 정식화 헤더인 경우에는 헤더)을 가져와서 필요한 형식으로 표준화하는 것입니다. 기본적으로 Microsoft에서 형식을 결정하면 개발자가 그에 맞춰야 합니다.

인증 헤더를 만드는 데 필요한 두 정식화 필드부터 알아보겠습니다.

### <a name="canonicalized-headers"></a>표준화된 헤더
...
### <a name="canonicalized-resource"></a>표준화된 리소스
...
쿼리 매개 변수가 있는 경우 이 예제에는 해당 매개 변수도 포함됩니다. 다음은 값이 여러 개인 추가 쿼리 매개 변수 및 쿼리 매개 변수를 처리하는 코드입니다. 모든 REST API에 대해 작동하도록 이 코드를 빌드하고 있다는 것을 기억하십시오. ListContainers 메서드가 모든 가능성을 필요로 하지 않더라도 모든 가능성을 포함하려고 합니다.
...
이 코드를 실행하면 결과 MessageSignature가 다음과 같습니다.
...
여기에는 저장소 서비스 REST API호출 요청을 만들 수 있는 클래스를 구성하는 데 필요한 모든 것이 포함됩니다.

## <a name="example-list-blobs"></a>예: Blob 목록

컨테이너 *컨테이너-1에*대 한 Blob 목록 작업을 호출 하는 코드를 변경 하는 방법을 살펴보겠습니다. 이 코드는 컨테이너를 나열하는 코드와 거의 동일하며 URI인 유일한 차이점및 응답을 구문 분석하는 방법뿐입니다.
...
이 문서에서는 Blob 저장소 REST API에 대 한 요청을 만드는 방법을 배웠습니다. 요청을 통해 컨테이너 목록 또는 컨테이너의 Blob 목록을 검색할 수 있습니다. REST API 호출에 대한 권한 부여 서명을 만드는 방법과 REST 요청에서 이를 사용하는 방법을 배웠습니다. 마지막으로 응답을 검사하는 방법을 배웠습니다.
