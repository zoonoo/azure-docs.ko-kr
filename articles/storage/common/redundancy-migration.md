...
PowerShell을 사용 하 여 저장소 계정에 대 한 중복성 옵션을 변경 하려면 [AzStorageAccount](/powershell/module/az.storage/set-azstorageaccount) 명령을 호출 하 고 `-SkuName` 매개 변수를 지정 합니다.
...
Azure CLI를 사용 하 여 저장소 계정에 대 한 중복성 옵션을 변경 하려면 [az storage account update](/cli/azure/storage/account#az-storage-account-update) 명령을 호출 하 고 `--sku` 매개 변수를 지정 합니다.
...
4. **문제** 섹션에서 다음 값을 지정합니다.
    ...
    - **세부 정보**: **세부** 정보 상자에 추가 세부 정보를 입력 합니다. 예를 들어 \_\_ 지역의 [LRS, GRS]에서 ZRS로 마이그레이션하려는 것입니다.
5. **다음**을 선택합니다.
