...
모든 저장소 계정에는 지정된 기간 동안 개체에 99.999999999% 이상의 내구성을 제공하도록 디자인된 **LRS(로컬 중복 저장소)**이 포함됩니다(11 9의 경우). 즉, 여러 데이터 복사본이 저장소 계정을 설정할 때 지정된 데이터 센터의 Azure Storage에서 관리됩니다. 변경 내용이 커밋되는 경우 성공을 반환하기 전에 다른 모든 복사본이 업데이트됩니다. 즉, 복제본은 항상 동기화됩니다. 또한 복사본이 별도의 오류 도메인 및 업그레이드 도메인에 위치합니다. 즉, 데이터를 보유하는 저장소 노드에 오류가 발생하거나 업데이트되기 위해 오프라인으로 전환되는 경우에도 데이터를 사용할 수 있습니다.

**LRS(로컬 중복 저장소)**

위에서 설명한 대로 LRS를 사용하면 단일 데이터 센터에 여러 데이터 복사본이 있게 됩니다. 저장소 노드에 오류가 발생하거나 업데이트되기 위해 오프라인으로 전환되는 경우 데이터를 사용할 수 없는 문제를 해결하지만 전체 데이터 센터를 사용할 수 없는 경우에는 해결할 수 없습니다.

**ZRS(영역 중복 저장소)**

ZRS(영역 중복 저장소)는 다른 일련의 데이터 복사본뿐만 아니라 데이터의 로컬 복사본을 유지하여 지정된 기간 동안 개체에 99.9999999999% 이상의 내구성을 제공하도록 디자인되었습니다(12 9의 경우). 복사본의 두 번째 집합은 하나 또는 두 개의 지역 내에서 데이터 센터 간에 비동기적으로 복제됩니다. ZRS는 범용 저장소 계정에서 블록 Blob에만 사용할 수 있습니다. 또한 저장소 계정을 만들고 ZRS를 선택하면 다른 복제 유형으로 또는 그 반대로 변환할 수 없습니다.
