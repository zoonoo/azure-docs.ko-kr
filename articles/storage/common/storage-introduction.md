...
현재는 Active Directory 기반 인증과 ACL(액세스 제어 목록)이 지원되지 않지만 향후 지원할 예정입니다. 파일 공유 액세스에 대한 인증을 제공하기 위해 스토리지 계정 자격 증명이 사용됩니다. 즉, 공유가 탑재된 모든 사용자는 공유에 대한 전체 읽기/쓰기 액세스 권한을 갖습니다.
...
## <a name="securing-access-to-storage-accounts"></a>저장소 계정에 대한 액세스 보안
...
- **Azure 파일에 대한 SMB에 대한 Azure AD 권한 부여(미리 보기).** Azure Files는 Azure Active Directory 도메인 서비스를 통해 SMB(서버 메시지 블록)에 대한 ID 기반 권한 부여를 지원합니다. 도메인 가입 Windows VM(가상 머신)은 Azure AD 자격 증명을 사용하여 Azure 파일 공유에 액세스할 수 있습니다. 자세한 내용은 [Azure 파일용 SMB에 대한 Azure Active Directory 권한 부여 개요(미리 보기)를](../files/storage-files-active-directory-overview.md)참조하십시오.
- **공유 키를 가진 권한 부여.** 공유 키 권한 부여를 사용하는 Azure 저장소 Blob, 큐 및 테이블 서비스 및 Azure Files 지원 권한 부여는 저장소 계정 액세스 키를 사용하여 서명된 모든 요청과 함께 헤더를 전달합니다. 자세한 내용은 [공유 키를 사용하여 권한 부여](https://docs.microsoft.com/rest/api/storageservices/authorize-with-shared-key)를 참조하세요.
...
- **컨테이너 및 Blob에 대한 익명 액세스.** 컨테이너 와 해당 Blob을 공개적으로 사용할 수 있습니다. 컨테이너 또는 Blob이 공용임을 지정하면 누구나 익명으로 읽을 수 있습니다. 인증이 필요하지 않습니다. 자세한 내용은 [컨테이너 및 Blob에 대한 익명 읽기 액세스 관리를 참조하십시오.](../blobs/storage-manage-access-to-resources.md)

## <a name="encryption"></a>암호화

Storage 서비스에 사용할 수 있는 기본적인 종류의 두 가지 암호화가 있습니다. 보안 및 암호화에 대한 자세한 내용은 [Azure Storage 보안 가이드](../blobs/security-recommendations.md)를 참조하세요.

### <a name="encryption-at-rest"></a>휴지 상태의 암호화

Azure Storage 암호화는 조직 보안 및 규정 준수 약정을 충족하기 위해 데이터를 보호하고 보호합니다. Azure Storage는 저장소 계정에 보관하기 전에 모든 데이터를 자동으로 암호화하고 검색하기 전에 암호를 해독합니다. 암호화, 암호 해독 및 키 관리 프로세스는 사용자에게 완전히 투명합니다. 고객은 Azure 키 자격 증명 모음을 사용하여 자신의 키를 관리하도록 선택할 수도 있습니다. 자세한 내용은 [미사용 데이터에 대한 Azure 저장소 암호화를](storage-service-encryption.md)참조하십시오.
...
데이터의 내구성을 보장하기 위해 Azure Storage는 데이터의 여러 복사본을 저장합니다. 스토리지 계정을 설정할 때 중복 옵션을 선택합니다.
...
Azure Storage를 사용하여 가동하고 실행하려면 [스토리지 계정 만들기](storage-account-create.md)를 참조하세요.
