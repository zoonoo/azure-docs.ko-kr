...
> [!NOTE]
> AzCopy는 파일의 md5 해시 코드를 자동으로 계산 하 고 저장 하지 않습니다. AzCopy는이 작업을 수행 하려면 각 복사 명령에 `--put-md5` 플래그를 추가 합니다. 이렇게 하면 blob이 다운로드 될 때 AzCopy는 다운로드 된 데이터에 대 한 MD5 해시를 계산 하 고 blob의 `Content-md5` 속성에 저장 된 MD5 해시가 계산 된 해시와 일치 하는지 확인 합니다.
...
> [!NOTE]
> AzCopy은 기본적으로 데이터를 블록 blob에 업로드 합니다. 추가 Blob 또는 페이지 Blob으로 파일을 업로드 하려면-0 @no__t 플래그를 사용 합니다.
...
> [!NOTE]
> Blob의 @no__t 속성 값에 해시가 포함 된 경우 AzCopy는 다운로드 된 데이터에 대 한 MD5 해시를 계산 하 고 blob의 `Content-md5` 속성에 저장 된 MD5 해시가 계산 된 해시와 일치 하는지 확인 합니다. 이러한 값이 일치 하지 않으면 복사 명령에 `--check-md5=NoCheck` 또는 `--check-md5=LogOnly`을 추가 하 여이 동작을 재정의 하지 않는 한 다운로드가 실패 합니다.
...
이 예제에서 AzCopy는 `https://mystorageaccount.blob.core.windows.net/mycontainer/FileDirectory/photos` 디렉터리와 `https://mystorageaccount.blob.core.windows.net/mycontainer/FileDirectory/documents/myFile.txt` 파일을 전송 합니다. @No__t-0 옵션을 포함 하 여 `https://mystorageaccount.blob.core.windows.net/mycontainer/FileDirectory/photos` 디렉터리의 모든 파일을 전송 해야 합니다.
...
>-  프리미엄 블록 blob 저장소 계정은 액세스 계층을 지원 하지 않습니다. @No__t-0을 `false`로 설정 하 여 복사 작업에서 blob의 액세스 계층을 생략 합니다 (예: `--s2s-preserve-access-tier=false`).
...
- [자습서: AzCopy @ no__t를 사용 하 여 온-프레미스 데이터를 클라우드 저장소로 마이그레이션-0
