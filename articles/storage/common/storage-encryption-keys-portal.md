...
고객이 관리 하는 키를 사용 하도록 설정 하면 저장소 계정에 연결할 키를 지정할 수 있습니다. 또한 사용자가 자동으로 사용자가 관리 하는 키를 회전 해야 Azure Storage 하는지 또는 키를 수동으로 회전할지 여부를 지정할 수 있습니다.

### <a name="specify-a-key-from-a-key-vault"></a>Key Vault에서 키 지정

키 자격 증명 모음에서 고객 관리 키를 선택 하면 키의 자동 회전이 자동으로 사용 하도록 설정 됩니다. 키 버전을 수동으로 관리 하려면 키 URI를 대신 지정 하 고 키 버전을 포함 합니다. 자세한 내용은 [URI로 키 지정](#specify-a-key-as-a-uri)을 참조 하세요.
...
키 URI를 지정 하는 경우에는 키 버전을 생략 하 여 고객 관리 키의 자동 회전을 사용 하도록 설정 합니다. 키 URI에 키 버전을 포함 하는 경우 자동 회전이 활성화 되지 않으므로 사용자가 직접 키 버전을 관리 해야 합니다. 키 버전을 업데이트 하는 방법에 대 한 자세한 내용은 [수동으로 키 버전 업데이트](#manually-update-the-key-version)를 참조 하세요.
...
1. 복사한 URI를 **키 uri** 필드에 붙여넣습니다. 자동 회전을 사용 하려면 URI에서 키 버전을 생략 합니다.
...
키를 지정한 후에는 Azure Portal 자동 키 회전이 사용 되는지 여부를 표시 하 고 현재 암호화에 사용 중인 키 버전을 표시 합니다.

:::image type="content" source="media/storage-encryption-keys-portal/portal-auto-rotation-enabled.png" alt-text="고객 관리 키 사용 설정의 자동 회전을 보여 주는 스크린샷":::

## <a name="manually-update-the-key-version"></a>수동으로 키 버전 업데이트

기본적으로 Azure Storage는 이전 섹션에 설명 된 대로 자동으로 고객 관리 키를 회전 합니다. 키 버전을 직접 관리 하도록 선택 하는 경우 새 버전의 키를 만들 때마다 저장소 계정에 지정 된 키 버전을 업데이트 해야 합니다.
