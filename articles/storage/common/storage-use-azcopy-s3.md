...
또한 AzCopy는 파일에 대 한 복사를 수행 하므로 이름 충돌을 확인 하 고 해결 하려고 시도 합니다. 예를 들어 이름이 `bucket-name` 및 `bucket.name`인 버킷이 있는 경우 AzCopy은 first에서로 `bucket-name` 이름이 지정 `bucket.name` 된 버킷을 확인 한 다음로 `bucket-name-2`변환 합니다.
