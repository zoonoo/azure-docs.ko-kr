---
title: 지리 중복 스토리지를 사용하여 가용성이 높은 애플리케이션 설계
titleSuffix: Azure Storage
description: 읽기 액세스 지리적 중복 저장소를 사용하여 가동 중단을 처리할 수 있을 만큼 유연할 수 있는 고가용성 응용 프로그램을 설계하는 방법을 알아봅니다.
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
---
# <a name="designing-highly-available-applications-using-read-access-geo-redundant-storage"></a>읽기 액세스 지리적 중복 스토리지를 사용하여 가용성이 높은 응용 프로그램 설계

Azure Storage와 같은 클라우드 기반 인프라의 일반적인 기능은 애플리케이션 호스팅을 위해 항상 사용할 수 있는 플랫폼을 제공하는 것입니다. 클라우드 기반 애플리케이션 개발자는 사용자에게 항상 사용 가능한 애플리케이션을 제공하기 위해 이러한 플랫폼을 활용할 방법을 신중하게 고려해야 합니다. 이 문서에서는 개발자가 Azure의 지리적 중복 복제 옵션 중 하나를 사용하여 Azure Storage 응용 프로그램을 가용성을 높이도록 하는 방법에 대해 중점합니다.

지리적 중복 복제용으로 구성된 저장소 계정은 기본 리전에서 동기적으로 복제된 다음 수백 마일 떨어진 보조 지역으로 비동기적으로 복제됩니다. Azure Storage는 두 가지 유형의 지역 중복 복제를 제공합니다.

* [GZRS(지리 영역 중복 저장소)(미리 보기)는](storage-redundancy.md) 고가용성과 최대 내구성이 모두 필요한 시나리오에 대한 복제를 제공합니다. 데이터는 ZRS(영역 중복 저장소)를 사용하여 기본 지역의 세 Azure 가용성 영역에서 동기적으로 복제된 다음 보조 지역으로 비동기적으로 복제됩니다. 보조 지역의 데이터에 대한 읽기 액세스를 위해 읽기 액세스 지리적 영역 중복 저장소(RA-GZRS)를 사용하도록 설정합니다.
* [GRS(지역 중복 저장소)는](storage-redundancy.md) 지역 간 복제를 제공하여 지역 중단을 방지합니다. 데이터는 LRS(로컬 중복 저장소)를 사용하여 기본 영역에서 3번 동기적으로 복제된 다음 보조 영역에 비동기적으로 복제됩니다. 보조 지역의 데이터에 대한 읽기 액세스를 위해 읽기 액세스 지리적 중복 저장소(RA-GRS)를 사용하도록 설정합니다.

이 문서에서는 기본 지역에서 중단을 처리하도록 응용 프로그램을 디자인하는 방법을 보여 줍니다. 기본 지역을 사용할 수 없게 되면 응용 프로그램은 보조 지역에 대해 읽기 작업을 수행하도록 조정할 수 있습니다. 시작하기 전에 저장소 계정이 RA-GRS 또는 RA-GZRS용으로 구성되어 있는지 확인합니다.
...
## <a name="application-design-considerations-when-reading-from-the-secondary"></a>보조 에서 읽을 때 응용 프로그램 설계 고려 사항
...
RA-GRS 또는 RA-GZRS용 응용 프로그램을 디자인할 때 다음 핵심 사항을 염두에 두십시오.
...
* 저장소 클라이언트 라이브러리를 사용하여 기본 또는 보조 지역에서 데이터를 읽고 쓸 수 있습니다. 주 지역에 대한 읽기 요청의 시간이 초과되면 읽기 요청을 보조 지역에 자동으로 리디렉션할 수도 있습니다.
...
> [!NOTE]
> GZRS/RA-GZRS를 지원하는 지역에서는 고객 관리 계정 장애 조치(미리 보기)를 아직 사용할 수 없으므로 고객은 현재 GZRS 및 RA-GZRS 계정으로 계정 장애 조치 이벤트를 관리할 수 없습니다. 미리 보기 중에 Microsoft는 GZRS/RA-GZRS 계정에 영향을 미치는 장애 조치 이벤트를 관리합니다.
...
예를 들어 고객이 업데이트를 성공적으로 제출하지만 업데이트를 보조 지역으로 전파하기 전에 기본 지역이 실패했다고 가정하겠습니다. 고객이 데이터를 다시 읽어 달라고 요청하면 업데이트된 데이터 대신 보조 지역에서 오래된 데이터를 수신합니다. 애플리케이션을 설계할 때 이런 경우를 허용할지 결정하고, 허용한다면 고객에게 어떤 메시지를 표시할지 결정해야 합니다. 
...
기본 리전에서 중단을 효과적으로 준비하려면 실패한 읽기 요청과 실패한 업데이트 요청(삽입, 업데이트 및 삭제를 의미하는 업데이트)을 모두 처리할 수 있어야 합니다. 기본 영역이 실패하면 읽기 요청을 보조 지역으로 리디렉션할 수 있습니다. 하지만 보조 저장소가 읽기 전용이기 때문에 업데이트 요청은 보조 저장소로 리디렉션될 수 없습니다. 이런 이유로 읽기 전용 모드에서 애플리케이션을 설계해야 합니다.
...
3. 다른 지역의 스토리지 계정에 업데이트를 작성할 수 있습니다. 그런 다음 기본 데이터 센터가 다시 온라인이 되면 데이터의 구조에 따라 업데이트를 기본 데이터에 병합하는 방법이 있습니다. 예를 들어 이름에 날짜/시간 스탬프가 있는 별도의 파일을 만드는 경우 파일을 주 지역에 다시 복사할 수 있습니다. 이러한 방식은 로깅 및 iOT 데이터와 같은 워크로드에 적합합니다.

## <a name="handling-retries"></a>다시 시도 처리

Azure Storage 클라이언트 라이브러리를 사용하면 다시 시도할 수 있는 오류를 확인할 수 있습니다. 예를 들어 404 오류(리소스를 찾을 수 없는 리소스)는 다시 시도하면 성공할 가능성이 없으므로 다시 시도되지 않습니다. 반면에 500 오류는 서버 오류이므로 다시 시도할 수 있으며 일시적인 문제일 수 있습니다. 자세한 내용은 .NET 스토리지 클라이언트 라이브러리에서 [ExponentialRetry 클래스에 대한 오픈 소스 코드](https://github.com/Azure/azure-storage-net/blob/87b84b3d5ee884c7adc10e494e2c7060956515d0/Lib/Common/RetryPolicies/ExponentialRetry.cs)를 참조하세요. (ShouldRetry 메서드를 찾아보세요.)

### <a name="read-requests"></a>읽기 요청

읽기 요청은 기본 스토리지에 문제가 있으면 보조 스토리지로 리디렉션될 수 있습니다. [결과적으로 일치하는 데이터 사용](#using-eventually-consistent-data)에 언급된 바와 같이 애플리케이션에서 잠재적으로 부실 데이터를 읽는 것이 허용됩니다. 저장소 클라이언트 라이브러리를 사용하여 보조 데이터베이스의 데이터에 액세스하는 경우 **LocationMode** 속성에 대한 값을 다음 중 하나로 설정하여 읽기 요청의 재시도 동작을 지정할 수 있습니다.
...
**LocationMode를** **PrimaryThenSecondary로**설정하면 다시 시도할 수 있는 오류로 기본 끝점에 대한 초기 읽기 요청이 실패하면 클라이언트가 자동으로 보조 끝점에 다른 읽기 요청을 만듭니다. 오류가 서버 시간 초과 오류이면 클라이언트는 서버로부터 재시도 가능한 오류를 수신하기 전에 시간 제한이 만료될 때까지 대기해야 합니다.
...
지리적 중복 저장소는 기본 에서 보조 지역으로 트랜잭션을 복제하여 작동합니다. 복제 프로세스는 보조 지역의 데이터가 *결과적으로 일치*하도록 보장합니다. 즉, 주 지역의 모든 트랜잭션이 보조 지역에 언젠가는 표시되지만 트랜잭션이 표시되기 전에 지연이 있을 수 있고 트랜잭션이 보조 지역에 도착하는 순서가 주 지역에 원래 적용된 순서와 같다는 보장이 없습니다. 트랜잭션이 보조 지역에 순서가 바뀌어 도착하면 서비스가 이를 처리할 때까지 보조 지역의 데이터가 불일치 상태인 것으로 생각할 수 *있습니다*.

다음 표에서는 직원의 세부 정보를 업데이트하여 *관리자* 역할의 구성원으로 만들 때 발생할 수 있는 예를 보여 주며 있습니다. 이 예제의 경우 **직원** 엔터티를 업데이트하고 총 관리자 수로 **관리자 역할** 엔터티를 업데이트해야 합니다. 업데이트가 보조 지역에서 어떻게 다른 순서로 적용되는지 살펴보겠습니다.
...
마지막 동기화 시간을 확인하는 방법을 알아보려면 [저장소 계정의 마지막 동기화 시간 속성 확인을](last-sync-time-get.md)참조하십시오.
...
* 마지막 동기화 시간 속성이 설정되는 방법의 다른 예를 포함하여 보조 지역에서 읽는 방법에 대한 자세한 내용은 [Azure 저장소 중복 옵션 및 읽기 액세스 지리적 중복 저장소](https://blogs.msdn.microsoft.com/windowsazurestorage/2013/12/11/windows-azure-storage-redundancy-options-and-read-access-geo-redundant-storage/)를 참조하십시오.

* 기본 끝점과 보조 끝점 간에 스위치를 앞뒤로 전환하는 방법을 보여 줄 전체 샘플은 [RA-GRS 저장소가 있는 회로 차단기 패턴을 사용하여 Azure 샘플을](https://github.com/Azure-Samples/storage-dotnet-circuit-breaker-pattern-ha-apps-using-ra-grs)참조하십시오.
