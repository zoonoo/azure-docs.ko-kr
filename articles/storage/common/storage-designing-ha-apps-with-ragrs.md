...
Azure Storage와 같은 클라우드 기반 인프라의 일반적인 기능은 애플리케이션 호스팅을 위해 항상 사용할 수 있는 플랫폼을 제공하는 것입니다. 클라우드 기반 애플리케이션 개발자는 사용자에게 항상 사용 가능한 애플리케이션을 제공하기 위해 이러한 플랫폼을 활용할 방법을 신중하게 고려해야 합니다. 이 문서에서는 개발자가 Azure의 지역 중복 복제 옵션 중 하나를 사용 하 여 Azure Storage 응용 프로그램을 항상 사용할 수 있도록 하는 방법에 중점을 둘 수 있습니다.
...
> [!NOTE]
> 고객이 관리 하는 계정 장애 조치 (failover) (미리 보기)는 GZRS/RA-GZRS를 지 원하는 지역에서 아직 사용할 수 없으므로, 고객은 현재 GZRS 및 RA-GZRS 계정을 사용 하 여 계정 장애 조치 (failover) 이벤트 미리 보기 중에 Microsoft는 GZRS/RA GZRS 계정에 영향을 주는 장애 조치 (failover) 이벤트를 관리 합니다.
