...
예를 들어 고객이 업데이트를 성공적으로 제출하지만 업데이트를 보조 지역으로 전파하기 전에 기본 지역이 실패했다고 가정하겠습니다. 고객 데이터를 다시 읽어 오도록 요청을 하는 경우 업데이트 된 데이터 대신 보조 지역의 부실 데이터를 받을. 애플리케이션을 설계할 때 이런 경우를 허용할지 결정하고, 허용한다면 고객에게 어떤 메시지를 표시할지 결정해야 합니다. 
...
다음 표에서 멤버인 있도록 직원의 세부 정보를 업데이트 하는 경우 발생할 수 있는 예가 나와 합니다 *관리자* 역할입니다. 이 예제의 경우 **직원** 엔터티를 업데이트하고 총 관리자 수로 **관리자 역할** 엔터티를 업데이트해야 합니다. 업데이트가 보조 지역에서 어떻게 다른 순서로 적용되는지 살펴보겠습니다.
...
보조 데이터베이스에 데이터를 마지막으로 쓴 시기를 결정 하는 마지막 동기화 시간을 검색 하려면 PowerShell 또는 Azure CLI를 사용할 수 있습니다.

### <a name="powershell"></a>PowerShell

PowerShell을 사용 하 여 저장소 계정에 대 한 마지막 동기화 시간을 가져오려면 확인 저장소 계정의 **GeoReplicationStats.LastSyncTime** 속성입니다. 자리 표시자 값을 고유한 값으로 대체 해야 합니다.
...
Azure CLI를 사용 하 여 저장소 계정에 대 한 마지막 동기화 시간을 가져오려면 확인 저장소 계정의 **geoReplicationStats.lastSyncTime** 속성입니다. 사용 된 `--expand` 속성 값을 반환 하는 매개 변수 아래에 중첩 **geoReplicationStats**합니다. 자리 표시자 값을 고유한 값으로 대체 해야 합니다.
