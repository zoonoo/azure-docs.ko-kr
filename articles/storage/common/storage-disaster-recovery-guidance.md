...
- **파일:** [Azure Backup](../../backup/azure-file-share-backup-overview.md) 를 사용 하 여 파일 공유를 백업 합니다. 또한 실수로 인 한 파일 공유 삭제 로부터 보호 하기 위해 [일시 삭제](../files/storage-files-prevent-file-share-deletion.md) 를 사용 하도록 설정 합니다. GRS를 사용할 수 없을 때 지리적 중복성을 위해 [AzCopy](./storage-use-azcopy-v10.md) 또는 [Azure PowerShell](/powershell/module/az.storage/) 를 사용 하 여 다른 지역의 다른 저장소 계정에 파일을 복사 합니다.
...
Azure VM(Virtual Machines)은 계정 장애 조치(failover)의 일부로 장애 조치(failover)되지 않습니다. 주 지역을 사용할 수 없으며 보조 지역으로 장애 조치(failover)할 경우 장애 조치(failover) 후에 VM을 다시 만들어야 합니다. 또한 계정 장애 조치 (failover)와 관련 된 잠재적인 데이터 손실이 있습니다. Azure의 가상 머신과 관련 하 여 다음과 같은 고가용성 및 [재해 복구](../../virtual-machines/backup-recovery.md) 지침을 [사용](../../virtual-machines/availability.md) 하는 것이 좋습니다.
