...
* 애플리케이션에서 확장성 목표 중 하나를 초과해야 하는 경우에는 여러 스토리지 계정을 만들고 해당 계정으로 애플리케이션 데이터를 분할해야 합니다. 이러한 패턴을 사용하는 경우에는 향후 부하 분산을 위해 스토리지 계정을 더 추가할 수 있도록 애플리케이션을 디자인해야 합니다. 작성 당시 각 Azure 구독 (Azure Resource Manager 모델을 사용 하 여 배포) 하는 경우 지역당 최대 250 개의 저장소 계정이 있습니다.  또한 Storage 계정에는 저장하거나 전송하는 데이터 또는 수행하는 트랜잭션과 관련된 사용 요금 외의 기타 비용은 없습니다.
...
Azure Storage는 범위 기반 파티션 구성표를 사용하여 시스템을 확장하고 부하를 분산합니다. 파티션 키 (계정 + 컨테이너 blob)는 파티션에 데이터 범위 및 이러한 범위는 부하 분산 된 시스템에서 사용 됩니다. 즉, 어휘 순서와 같은 명명 규칙 (예를 들어 *mypayroll*를 *myperformance*합니다 *myemployees*등) 또는 타임 스탬프를 사용 하 여 ( *log20160101*, *log20160102*, *log20160102*등)을 잠재적으로 배치에 있는 것 같은 파티션 서버까지 파티션에 자체는 행위는 부하 분산 작업 작은 범위로 분할할 수를 분할합니다. 예를 들어, 컨테이너 내의 모든 blob이이 blob의 부하를 추가 파티션 범위의 다시 분산을 필요로 할 때까지 단일 서버를 통해 제공할 수 있습니다. 마찬가지로, 이름이 어휘 순서로 정렬된 가벼운 부하의 계정 그룹도 하나 또는 전체 계정의 부하에서 여러 파티션 서버로 분할을 요구할 때까지 단일 서버가 지원할 수 있습니다. 각 부하 분산 작업은 작업 중 스토리지 호출의 대기 시간에 영향을 미칠 수 있습니다. 부하 분산 작업이 시작 되 고 파티션 키 범위를 다시 분산 될 때까지 단일 파티션 서버의 확장성 파티션에 트래픽의 급증을 처리 하는 시스템의 기능 제한 됩니다.

이러한 작업의 빈도를 줄이려면 몇 가지 모범 사례를 따르십시오.  

* 가능 하면 처리량이 높은 블록 Blob (HTBB)를 활성화 하려면 Put Blob 또는 블록 크기가 클수록 (4mib 표준 계정에 대 한 보다 크고 256 3:n2}kib/ 프리미엄 계정에 대 한 보다 큰)를 사용 합니다. HTBB 제공 상위 파티션 이름을 지정 하 여 영향을 받지 않는 성능 수집 합니다.
* 계정, 컨테이너, Blob, 테이블, 큐에 사용하는 명명 규칙을 자세히 확인합니다. 사용자의 요구에 가장 적합 한 해시 함수를 사용 하는 3 자릿수 해시를 사용 하 여 계정, 컨테이너 또는 blob 이름 앞에 추가 하는 것이 좋습니다.  
* 타임스탬프 또는 숫자 식별자를 사용하여 데이터를 정리할 경우 추가만 가능한(또는 앞에만 추가할 수 있는) 트래픽 패턴을 사용하지 않아야 합니다. 이러한 패턴은 범위 기반 파티셔닝 시스템에 적합하지 않으며 모든 트래픽이 단일 파티션으로 이동하여 시스템이 부하 분산을 효과적으로 수행하지 못할 수 있습니다. 예를 들어 일상적 작업에서 같은 타임 스탬프는 blob 개체를 사용 하는 경우 *yyyymmdd*, 다음 일일 작업에 대 한 모든 트래픽이 단일 분할 서버에 의해 제공 되는 단일 개체로 이동 됩니다. Blob당 한계와 파티션당 한계가 요구 사항을 충족하는지 확인하고 필요에 따라 이 작업을 여러 Blob으로 나누는 것을 고려해 보십시오. 마찬가지로 테이블에 시계열 데이터를 저장하는 경우 모든 트래픽이 키 네임스페이스의 마지막 부분으로 전송될 수 있습니다. 접두사 ID는 3 자릿수 해시를 사용 하 여 또는 타임 스탬프의 경우와 같은 시간의 초 부분을 접두사로 타임 스탬프 또는 숫자 Id를 사용 해야 하는 경우 *ssyyyymmdd*합니다. 열거 및 쿼리 작업을 일상적으로 수행하는 경우 쿼리 수를 제한하는 해시 함수를 선택합니다. 다른 경우에는 임의의 접두사로도 충분할 수 있습니다.  
* Azure Storage에 사용 된 파티션 구성표에 대 한 자세한 내용은 참조 하세요. [Azure Storage: 강력한 일관성과 함께 항상 사용 가능한 클라우드 스토리지 서비스](https://sigops.org/sosp/sosp11/current/2011-Cascais/printable/11-calder.pdf) 문서를 참조하세요.
...
#### <a name="subheading10"></a>비동기 작업을 사용 하 여 동기 코드를 사용 하는 경우 최소 스레드 수를 늘리려면

이 코드는 스레드 풀의 스레드 최소 수가 증가 합니다.  
...
#### <a name="subheading11"></a>.NET 4.5 및 더 높은 가비지 수집이 활용
...
#### <a name="subheading14"></a>제한 및 서버 작업 중 오류
...
#### <a name="subheading15"></a>비-재시도 가능한 오류
...
에 대 한 검증 된 사례 외에도 [모든 서비스](#allservices) 앞에서 설명한 Blob service에만 적용 다음과 같은 검증 합니다.  
...
* Node.js: 요청 옵션 또는 Blob service에 parallelOperationThreadCount를 사용 합니다.
...
#### <a name="entities-per-second-storage-account"></a>(저장소 계정) 초당 엔터티 수
...
#### <a name="entities-per-second-partition"></a>엔터티 / 초 (파티션)
...
##### <a name="limiting-the-amount-of-data-returned"></a>반환 되는 데이터 양을 제한 합니다.
...
##### <a name="subheading37"></a>데이터 계열을 단일 엔터티에 저장
...
사용할 수는 **업데이트 메시지** 작업 메시지의 상태 정보를 업데이트 하거나 표시 안 함 시간 제한을 늘립니다. 이 강력한 상태인 동안 각 기억 **업데이트 메시지** 작업 확장성 목표를 합산 합니다. 그러나 각 작업 단계가 완료되면 작업을 다음 큐에 순서대로 전달하는 워크플로보다는 UpdateMessage 작업이 훨씬 더 효율적인 방식일 수 있습니다. 사용 하는 **업데이트 메시지** 작업 메시지에 작업 상태를 저장 하 고 다음 단계를 완료 될 때마다 작업의 다음 단계에 대 한 메시지를 requeuing 대신 작업을 계속 응용 프로그램을 허용 합니다.  
