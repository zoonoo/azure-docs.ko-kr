...
Storage 계정에 대한 Storage 서비스 암호화를 사용하거나 사용하지 않도록 설정하려면 [Azure Portal](https://portal.azure.com) 에 로그인한 후 Storage 계정을 선택합니다. 설정 블레이드에서 이 스크린샷에 표시된 것처럼 Blob 서비스 섹션을 찾은 후 암호화를 클릭합니다.
...
Storage 계정 수준에서 Storage 서비스 암호화를 사용할 수 있습니다. 사용하도록 설정하면 고객이 암호화할 서비스를 선택합니다. 다음과 같은 고객 시나리오가 지원됩니다.
...
A: Azure Storage 팀은 기본적으로 Azure Storage(Blob, 파일, 테이블 및 큐 저장소)에 작성된 모든 데이터 및 모든 저장소 계정(Azure Resource Manager 및 클래식 저장소 계정), 신규 및 기존 계정에 Microsoft 관리 키를 사용하여 암호화를 활성화하는 프로세스를 진행 중입니다.
