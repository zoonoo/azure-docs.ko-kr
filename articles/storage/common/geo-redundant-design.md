---
title: 지리적 중복성을 사용하여 고가용성 애플리케이션 설계
...
지역 중복 저장소는 주 데이터베이스에서 보조 지역으로 트랜잭션을 복제 하는 방식으로 작동 합니다. 이 복제 프로세스는 보조 지역의 데이터가 *결과적으로 일치* 하도록 보장 합니다. 즉, 주 지역의 모든 트랜잭션이 보조 지역에 언젠가는 표시되지만 트랜잭션이 표시되기 전에 지연이 있을 수 있고 트랜잭션이 보조 지역에 도착하는 순서가 주 지역에 원래 적용된 순서와 같다는 보장이 없습니다. 트랜잭션이 보조 지역에 순서가 바뀌어 도착하면 서비스가 이를 처리할 때까지 보조 지역의 데이터가 불일치 상태인 것으로 생각할 수 *있습니다*.
