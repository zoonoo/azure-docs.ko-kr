...
| 속성 | Description |
|:--- |:---|
|**time** | 저장소에서 요청을 받은 UTC (Universal Time) 시간입니다. 예를 들어 `2018/11/08 21:09:36.6900118`을 참조하십시오.|
...
|**operationVersion** | 요청을 만들 때 지정한 저장소 서비스 버전입니다. 이 값은 **x-y** 헤더의 값과 동일 합니다. 예를 들어 `2017-04-17`을 참조하십시오.|
|**schemaVersion** | 로그의 스키마 버전입니다. 예를 들어 `1.0`을 참조하십시오.|
|**statusCode** | 요청의 HTTP 상태 코드입니다. 요청이 중단 되 면이 값을로 설정할 수 있습니다 `Unknown` . <br> 예: `206` |
...
|**durationMs** | 요청 된 작업을 수행 하는 데 걸리는 총 시간 (밀리초)입니다. 여기에는 들어오는 요청을 읽고 요청자에 게 응답을 보내는 시간이 포함 됩니다. 예를 들어 `12`을 참조하십시오.|
|**callerIpAddress** | 요청자의 IP 주소(포트 번호 포함)입니다. 예를 들어 `192.100.0.102:4362`을 참조하십시오. |
|**correlationId** | 리소스에서 로그를 상호 연결 하는 데 사용 되는 ID입니다. 예를 들어 `b99ba45e-a01e-0042-4ea6-772bbb000000`을 참조하십시오. |
|**location** | 저장소 계정의 위치입니다. 예를 들어 `North Europe`을 참조하십시오. |
|**protocol**|작업에 사용 되는 프로토콜입니다. 예: `HTTP` ,, `HTTPS` `SMB` 또는`NFS`|
| **uri** | 요청 된 uri (Uniform resource identifier)입니다. 예를 들어 `http://myaccountname.blob.core.windows.net/cont1/blobname?timeout=10`을 참조하십시오. |
...
|**권한 부여/roleAssignmentId** | 역할 할당 ID입니다. 예를 들어 `4e2521b7-13be-4363-aeda-111111111111`을 참조하십시오.|
|**권한 부여/roleDefinitionId** | 역할 정의 ID입니다. 예를 들어 `ba92f5b4-2d11-453d-a403-111111111111"`을 참조하십시오.|
|**보안 주체/id** | 보안 주체의 ID입니다. 예를 들어 `a4711f3a-254f-4cfb-8a2d-111111111111`을 참조하십시오.|
|**보안 주체/유형** | 보안 주체의 유형입니다. 예를 들어 `ServicePrincipal`을 참조하십시오. |
|**요청자/appID** | 요청자로 사용 되는 Open Authorization (OAuth) 응용 프로그램 ID입니다. <br> 예를 들어 `d3f7d5fe-e64a-4e4e-871d-333333333333`을 참조하십시오.|
|**요청자/대상** | 요청의 OAuth 대상 그룹입니다. 예를 들어 `https://storage.azure.com`을 참조하십시오. |
|**요청자/objectId** | 요청자의 OAuth 개체 ID입니다. Kerberos 인증의 경우는 Kerberos 인증 사용자의 개체 식별자를 나타냅니다. 예를 들어 `0e0bf547-55e5-465c-91b7-2873712b249c`을 참조하십시오. |
|**요청자/tenantId** | Id의 OAuth 테 넌 트 ID입니다. 예를 들어 `72f988bf-86f1-41af-91ab-222222222222`을 참조하십시오.|
|**요청자/tokenIssuer** | OAuth 토큰 발급자입니다. 예를 들어 `https://sts.windows.net/72f988bf-86f1-41af-91ab-222222222222/`을 참조하십시오.|
|**요청자/upn** | 요청자의 UPN (사용자 계정 이름)입니다. 예를 들어 `someone@contoso.com`을 참조하십시오. |
...
| 속성 | 설명 |
|:--- |:---|
|**accountName** | 스토리지 계정 이름입니다. 예를 들어 `mystorageaccount`을 참조하십시오.  |
|**requestUrl** | 요청 된 URL입니다. 예를 들어 `http://mystorageaccount.blob.core.windows.net/cont1/blobname?timeout=10`을 참조하십시오.|
|**userAgentHeader** | **사용자 에이전트 헤더** 값 (따옴표)입니다. 예를 들어 `WA-Storage/6.2.0 (.NET CLR 4.0.30319.42000; Win32NT 6.2.9200.0)`을 참조하십시오.|
|**referrerHeader** | 참조 **페이지 헤더 값** 입니다. 예를 들어 `http://contoso.com/about.html`을 참조하십시오.|
|**clientRequestId** | 요청의 **x-y (요청 id)** 헤더 값입니다. 예를 들어 `360b66a6-ad4f-4c4a-84a4-0ad7cb44f7a6`을 참조하십시오. |
|**etag** | 반환된 개체의 ETag 식별자로 따옴표로 묶여 있습니다. 예를 들어 `0x8D101F7E4B662C4`을 참조하십시오.  |
|**Azurestoragelog.serverlatencyms** | 요청 된 작업을 수행 하는 데 걸린 총 시간 (밀리초)입니다. 이 값에는 네트워크 대기 시간 (들어오는 요청을 읽고 요청자에 게 응답을 보내는 시간)이 포함 되지 않습니다. 예를 들어 `22`을 참조하십시오. |
|**serviceType** | 이 요청과 연관 된 서비스입니다. 예를 들면 `blob`, `table`, `files`, `queue` 등입니다. |
|**operationCount** | 요청에 관련 된 각 로그 된 작업의 수입니다. 이 개수는의 인덱스로 시작 `0` 합니다. 일부 요청에는 blob 복사 요청과 같은 두 개 이상의 작업이 필요 합니다. 대부분의 요청은 하나의 작업만 수행 합니다. 예를 들어 `1`을 참조하십시오. |
|**requestHeaderSize** | 표시 된 요청 헤더의 크기 (바이트)입니다. 예를 들어 `578`을 참조하십시오. <br>요청이 실패할 경우이 값은 비어 있을 수 있습니다. |
|**requestBodySize** | 저장소 서비스에서 읽은 요청 패킷의 크기 (바이트)입니다. <br> 예를 들어 `0`을 참조하십시오. <br>요청이 실패할 경우이 값은 비어 있을 수 있습니다.  |
|**responseHeaderSize** | 표시 되는 응답 헤더의 크기 (바이트)입니다. 예를 들어 `216`을 참조하십시오. <br>요청이 실패할 경우이 값은 비어 있을 수 있습니다.  |
|**responseBodySize** | 저장소 서비스에서 쓰는 응답 패킷의 바이트 크기입니다. 요청이 실패할 경우 이 값은 비어 있을 수 있습니다. 예를 들어 `216`을 참조하십시오.  |
|**requestMd5** | 요청에서 **콘텐츠-md5** 헤더 또는 **x-y** 헤더의 값입니다. 이 필드에 지정된 MD5 해시 값은 요청의 콘텐츠를 나타냅니다. 예를 들어 `788815fd0198be0d275ad329cafd1830`을 참조하십시오. <br>이 필드는 비어 있을 수 있습니다.  |
|**serverMd5** | 저장소 서비스에서 계산 된 MD5 해시의 값입니다. 예를 들어 `3228b3cf1069a5489b298446321f8521`을 참조하십시오. <br>이 필드는 비어 있을 수 있습니다.  |
|**lastModifiedTime** | 반환 된 개체에 대 한 LMT (마지막 수정 시간)입니다.  예를 들어 `Tuesday, 09-Aug-11 21:13:26 GMT`을 참조하십시오. <br>여러 개체를 반환할 수 있는 작업의 경우 이 필드가 비어 있습니다. |
|**conditionsUsed** | 조건을 나타내는 키-값 쌍의 세미콜론으로 구분 된 목록입니다. 조건은 다음 중 하나일 수 있습니다. <li> If-Modified-Since <li> If-Unmodified-Since <li> If-Match <li> If-None-Match  <br> 예를 들어 `If-Modified-Since=Friday, 05-Aug-11 19:11:54 GMT`을 참조하십시오. |
...
|**tlsVersion** | 요청 연결에 사용 되는 TLS 버전입니다. 예를 들어 `TLS 1.2`을 참조하십시오. |
