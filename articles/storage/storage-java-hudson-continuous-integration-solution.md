...
3. **Microsoft Azure Storage Account Configuration** 섹션에서:
    ...
    b. 역시 Azure 포털에서 얻을 수 있는 저장소 계정 키를 입력합니다.

    c. 공용 Azure 클라우드를 사용 중이면 **Blob Service Endpoint URL**에 기본값을 사용합니다. 다른 Azure 클라우드를 사용 중이면 Azure 포털에서 저장소 계정에 대해 지정된 끝점을 사용합니다.
...
14. 작업이 성공적으로 완료되었을 때 공용 Blob을 열어 빌드 아티팩트를 검사할 수 있습니다.
    ...
    a. [Azure 포털](https://manage.windowsazure.com)에 로그인합니다.
...
- **URL 형식**: Blob에 다음 URL 형식을 사용하여 주소를 지정할 수 있습니다.
    ...
    (위 형식은 공용 Azure 클라우드에 적용됩니다. 다른 Azure 클라우드를 사용 중이면 Azure 포털 내의 끝점을 사용하여 URL 끝점을 확인합니다.)
