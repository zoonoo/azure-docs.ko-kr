...
컨테이너를 새로 작성하는 경우 `result`이(가) true입니다. 컨테이너가 이미 있는 경우 `result`이(가) false입니다. `response`에는 컨테이너의 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) 정보와 같은 작업 관련 정보가 포함됩니다.
