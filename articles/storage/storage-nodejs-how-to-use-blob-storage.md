...
이 가이드에서는 Azure Blob 서비스를 사용하여 일반 시나리오를 수행하는 방법을 보여 줍니다. 샘플은 Node.js API를 사용하여 작성되었습니다. Blob **업로드**, **나열**, **다운로드** 및 **삭제** 시나리오를 다룹니다.
...
새 컨테이너를 만들려면 **createContainerIfNotExists**를 사용합니다. 다음에서는 'mycontainer'라는 이름의 새 컨테이너를 만듭니다.
...
컨테이너가 만들어지면, `result`true가 됩니다. 컨테이너가 이미 있을 경우는 `result`false가 됩니다. `response`컨테이너의 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) 정보와 같은 작업 관련 정보가 포함됩니다.
...
에는 각 Blob을 설명하는 개체의 배열인 `entries`컬렉션이 `result`포함됩니다. Blob을 모두 반환할 수 없는 경우에는 `result`에서 추가 항목 검색에 두 번째 매개 변수로 사용할 수 있는 `continuationToken`을 제공합니다.
...
에는 **ETag** 정보를 비롯한 Blob 관련 정보가 `result`포함됩니다.
...
ETag 조건은 선택적인 매개 `options.accessConditions`변수를 사용하여 설정할 수 있습니다. 아래 예제에서는 Blob이 이미 `etagToMatch`에 포함된 ETag 값이 있는 경우에만 **test.txt** 파일을 업로드합니다.
