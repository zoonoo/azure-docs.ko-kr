...
> [!NOTE]
> 이미 Azure에서 실행되는 Linux VM이나 Azure 마켓플레이스의 이미지에서 만든 새 Linux VM의 경우 OS 디스크의 암호화가 현재 지원되지 않습니다. Linux VM의 OS 볼륨 암호화는 온-프레미스에서 암호화된 후 Azure로 업로드된 VM에 대해서만 지원됩니다. 이 제한은 OS 디스크에만 적용됩니다. Linux VM에 대한 데이터 볼륨 암호화가 지원됩니다.
> 
> 
...
* [Azure 리소스 관리자](../azure-resource-manager/resource-group-overview.md) 모델을 사용하여 만든 IaaS VM에서 암호화 사용
