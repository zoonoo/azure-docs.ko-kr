...
description: 읽기 및 쓰기 작업을 효율적으로 지원하도록 Azure Storage 테이블 서비스를 디자인하기 위한 지침을 이해합니다.
services: storage
ms.service: storage
author: tamram
ms.author: tamram
...
* ***쿼리에서 PartitionKey와 RowKey를 모두 지정합니다.** _ 이와 같은 ‘지점 쿼리’는 가장 효율적인 테이블 서비스 쿼리입니다.  
...
* ***복합 키 값을 사용합니다.** _ 보유한 키는 _ *PartitionKey**와 **RowKey** 뿐입니다. 예를 들어 복합 키 값을 사용하여 엔터티에 대한 대체 키 액세스 경로를 사용하도록 설정할 수 있습니다.  
