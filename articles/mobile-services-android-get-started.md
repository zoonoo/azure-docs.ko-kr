...
5. 이제 앱을 다운로드합니다.
	...
	- 이전 버전에서는 이전 버전의 SDK를 사용합니다. 이를 사용하려면 **앱 다운로드 및 실행** 아래에서 **다운로드**를 클릭합니다. 모바일 서비스에 연결된 샘플 _To do list_ 응용 프로그램에 대한 프로젝트가 다운로드됩니다. 프로젝트 파일은 압축되므로 해당 위치로 이동한 후 컴퓨터에 파일 압축을 풉니다.
...
5. 앱에서 _Complete the tutorial_ 등의 의미 있는 텍스트를 입력하고 **추가**를 클릭합니다.
