...
이 토픽에서는 Azure와 호환되는 SSH 키를 생성하는 단계를 설명합니다.
...
4.  Linux 가상 컴퓨터를 만드는 동안 `myCert.pem`을 업로드합니다. 프로비저닝 프로세스에서 가상 컴퓨터의 지정된 사용자에 대한 `authorized_keys` 파일에 이 인증서의 공개 키를 자동으로 설치합니다.
...
모든 Linux 가상 컴퓨터는 표준 포트와 다를 수 있는 특정 포트에서 SSH로 프로비전됩니다.

1.  관리 포털에서 Linux 가상 컴퓨터에 연결하는 데 사용할 포트를 찾습니다.
...
1.  다음 위치에서 msysgit를 다운로드하여 설치합니다. [][]<http://msysgit.github.com/></a>
2.  설치된 디렉터리(예: c:\\msysgit\\msys.exe)에서 `msys`를 실행합니다.
3.  `cd bin`을 입력하여 `bin` 디렉터리로 변경합니다.

### Windows용 GitHub 사용

1.  다음 위치에서 Windows용 GitHub를 다운로드하여 설치합니다. [][1]<http://windows.github.com/></a>
2.  시작 메뉴 \> 모든 프로그램 \> GitHub, Inc에서 Git 셸을 실행합니다.

### cygwin 사용

1.  다음 위치에서 Cygwin을 다운로드하여 설치합니다. [][2]<http://cygwin.com/></a>
...
4.  표시되는 질문에 대답합니다.
5.  두 개의 파일, 즉 `myPrivateKey.key` 및 `myCert.pem`.
...
1.  다음 위치에서 puttygen을 다운로드하여 설치합니다. [][3]<http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html></a>
2.  `puttygen.exe`을 실행합니다.
3.  메뉴: 파일 \> 개인 키 로드를 클릭합니다.
4.  이름이 `myPrivateKey.key`인 개인 키를 찾습니다. **모든 파일(\*.*)**을 표시하도록 파일 필터를 변경해야 합니다.
5.  **열기**를 클릭합니다. 다음과 같은 프롬프트가 표시됩니다.

    ![linuxgoodforeignkey][linuxgoodforeignkey]

6.  **확인**을 클릭합니다.
7.  아래 스크린샷에서 강조 표시된 **개인 키 저장**을 클릭합니다.
...
1.  다음 위치에서 putty를 다운로드하여 설치합니다. [][3]<http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html></a>
2.  putty.exe를 실행합니다.
3.  관리 포털의 IP를 사용하여 호스트 이름을 채웁니다.

    ![linuxputtyconfig][linuxputtyconfig]

4.  **열기**를 선택하기 전에 연결 \> SSH \> 인증 탭을 클릭하여 키를 선택합니다. 채울 필드는 아래 스크린샷을 참조하세요.
