...
Python API를 사용하여 작성되었습니다. **테이블 만들기 및
...
**TableService** 개체를 사용하면 테이블 서비스로 작업할 수 있습니다. 
다음 코드는 **TableService** 개체를 만듭니다. 
...
사전을 만듭니다. 모든 엔터티에 대해
**PartitionKey** 및 **RowKey**를 지정해야 합니다. 이 두 키는
...
실패합니다. 엔터티가 이미 있는지
여부에 관계없이 엔터티를 저장하려면 **insert\_or\_replace_entity**를 사용합니다. 
...
**TableService**에서 **begin\_batch** 메서드를 사용한 다음
평상시처럼 여러 작업을 호출합니다. 일괄 제출하려면
...
*프로젝션*이라는 이 기술로 대역폭을 줄이고
특히 큰 엔터티에서 쿼리 성능을 높일 수 있습니다. **select**
...
*다음 코드 조각은 클라우드 저장소 서비스에 대해서만 작동하며
저장소 에뮬레이터에서 지원되지
않습니다.*
