...
이 가이드에서는 Node.js 응용 프로그램에서 서비스 버스 토픽과 구독을 사용하는 방법을 보여 줍니다. 여기서 다루는 시나리오에는 **토픽 및 구독 만들기, 구독 필터 만들기, 토픽에 메시지 보내기,** **구독에서 메시지 받기,** **토픽 및 구독 삭제** 등이 포함됩니다. 토픽 및 구독에 대한 자세한 내용은 [다음 단계](#nextsteps) 섹션을 참조하십시오.
...
Azure 모듈은 Azure 서비스 버스에 연결하는 데 필요한 정보를 얻기 위해 환경 변수 AZURE\_SERVICEBUS\_NAMESPACE 및 AZURE\_SERVICEBUS\_ACCESS\_KEY를 읽습니다. 이러한 환경 변수를 설정하지 않은 경우 **createServiceBusService** 호출 시 계정 정보를 지정해야 합니다.
