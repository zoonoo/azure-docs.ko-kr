<properties urlDisplayName="Service Bus Topics" pageTitle="서비스 버스 토픽을 사용하는 방법(Node.js) - Azure" metaKeywords="Get started Azure Service Bus topics, Get Started Service Bus topics, Azure publish subscribe messaging, Azure messaging topics and subscriptions, Service Bus topic Node.js" description="Azure에서 서비스 버스 토픽 및 구독을 사용하는 방법에 대해 알아봅니다. 코드 샘플은 Node.js 응용 프로그램용으로 작성되었습니다." metaCanonical="" services="service-bus" documentationCenter="nodejs" title="How to Use Service Bus Topics/Subscriptions" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
**ServiceBusService** 개체를 사용하면 토픽으로 작업할 수 있습니다. 다음 코드는 **ServiceBusService** 개체를 만듭니다. 이 코드를 **server.js** 파일의 위쪽, Azure 모듈을 가져오기 위한 문 뒤에 추가합니다.

    var serviceBusService = azure.createServiceBusService();

**ServiceBusService** 개체에서 **createTopicIfNotExists**를 호출하면
지정한 토픽이 반환되거나(있는 경우) 지정한 이름의 새 토픽이 만들어집니다. 다음 코드는 **createTopicIfNotExists**를 사용하여 'MyTopic'이라는 토픽을 만들거나 이 토픽에 연결합니다.
...
**createServiceBusService**는
메시지 TTL(Time to Live) 또는 최대 토픽 크기와 같은 기본 토픽 설정을 재정의할 수 있는
추가 옵션도 지원합니다. 다음은
최대 토픽 크기를 5GB, TTL(Time to Live)을 1분으로 설정하는 예제입니다.
...
##<a name="create-subscription"></a> 구독을 만드는 방법

토픽 구독은 **ServiceBusService** 개체로도 생성됩니다. 구독에는 이름이 지정되며, 구독의 가상 큐에 전달되는 메시지 집합을 제한하는 선택적 필터가 있을 수 있습니다.

<div class="dev-callout">
<strong>참고</strong>
<p>구독은 영구적이며, 구독 자체 또는 구독과 연결된 토픽이 삭제될 때까지 계속 유지됩니다. 응용 프로그램에 구독을 만들기 위한 논리가 포함된 경우, <strong>getSubscription</strong> 메서드를 사용하여 구독이 이미 있는지를 먼저 확인해야 합니다.</p>
</div>
...
> 기본 필터는 모든 새로운 구독에 자동으로 적용되므로 먼저 기본 필터를 제거해야 합니다. 그렇지 않으면 <strong>MatchAll</strong> 이 사용자가 지정하는 기타 필터를 재정의합니다. 기본 규칙을 제거하려면 <strong>deleteRule</strong> 메서드( <strong>ServiceBusService</strong> 개체)를 사용합니다.
...
마찬가지로, 다음 예제에서는
'LowMessages'라는 구독을 만들며, **messagenumber** 속성이 3보다 작거나 같은 메시지만 선택하는
**SqlFilter**를 사용합니다.
...
##<a name="send-messages"></a> 토픽에 메시지를 보내는 방법
...
##<a name="receive-messages"></a> 구독에서 메시지를 받는 방법
...
##<a name="delete"></a> 토픽과 구독을 삭제하는 방법
...
-   GitHub에서 [Azure SDK for Node] 리포지토리를 방문하세요.
  ...
  [WebMatrix를 사용한 웹 사이트]: /ko-kr/develop/nodejs/tutorials/web-site-with-webmatrix/
  ...
  [저장소를 사용한 Node.js 웹 응용 프로그램]: /ko-kr/develop/nodejs/tutorials/web-site-with-storage/
