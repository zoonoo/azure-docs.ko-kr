...
이 문서에서는 Azure 방호를 사용 하 여 Azure 가상 네트워크에서 Windows 가상 머신 확장 집합 인스턴스에 안전 하 고 원활 하 게 RDP 하는 방법을 보여 줍니다. Azure Portal에서 직접 가상 머신 확장 집합 인스턴스에 연결할 수 있습니다. Azure Bastion을 사용하면 VM에 클라이언트, 에이전트 또는 추가 소프트웨어가 필요하지 않습니다. Azure 방호에 대 한 자세한 내용은 [개요](bastion-overview.md)를 참조 하세요.
