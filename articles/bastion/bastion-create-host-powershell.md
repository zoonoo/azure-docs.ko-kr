---
title: Azure Powershell을 사용하여 요새 호스트 만들기 | 마이크로 소프트 문서
description: 이 문서에서는 Azure 요새 호스트를 만드는 방법을 알아봅니다.
services: bastion
author: cherylmc
ms.service: bastion
ms.topic: conceptual
...
이 문서에서는 PowerShell을 사용하여 Azure 요새 호스트를 만드는 방법을 보여 주며 있습니다. 가상 네트워크에서 Azure Bastion 서비스를 프로비전하면 동일한 가상 네트워크의 모든 VM에서 원활한 RDP/SSH 환경을 사용할 수 있습니다. Azure Bastion 배포는 구독/계정 또는 가상 머신이 아닌 가상 네트워크별로 수행됩니다.

선택적으로 Azure [포털을](bastion-create-host-portal.md)사용하여 Azure 요새 호스트를 만들 수 있습니다.
...
이 섹션에서는 Azure PowerShell을 사용하여 새 Azure 요새 리소스를 만드는 데 도움이 됩니다.
