<properties title="Tutorial: Move and process log files using Azure Data Factory" pageTitle="Azure 데이터 팩터리를 사용하여 로그 파일 이동 및 처리" description="This advanced tutorial describes a near real-world scenario and implements the scenario using Azure Data Factory service." metaKeywords=""  services="data-factory" solutions=""  documentationCenter="" authors="spelluru" manager="jhubbard" editor="monicar" />
...
2.	이 자습서를 수행하려면 Azure 구독이 있어야 합니다. 구독을 얻는 방법에 대한 자세한 내용은 [구매 옵션] [azure-purchase-options], [회원 제안][azure-member-offers] 또는 [무료 평가판][azure-free-trial]을 참조하세요.
...
4.	[ADFWalkthrough][adfwalkthrough-download] 파일을 **C:\ADFWalkthrough** 폴더로 다운로드합니다. 이때 다음과 같이 **폴더 구조를 유지**합니다.
	...
	- **AzureEnvironmentSetup.ps1:** Azure 환경을 설정하는 PowerShell 스크립트입니다.
	- **uploadSampleDataAndScripts.ps1:** 이 스크립트는 샘플 데이터 및 스크립트를 Azure에 업로드합니다.
5. 다음과 같은 Azure 리소스를 만들었는지 확인합니다.			
	- Azure 저장소 계정
	- Azure SQL 데이터베이스
	- Azure HDInsight 클러스터 버전 3.1 이상	
7. Azure 리소스를 만들었으면 이러한 각 리소스에 연결하는 데 필요한 정보를 알고 있는지 확인합니다.
 	- **Azure 저장소 계정** - 계정 이름 및 계정 키  
	- **Azure SQL 데이터베이스** - 서버, 데이터베이스, 사용자 이름 및 암호
	- **Azure HDInsight 클러스터** - HDInsight 클러스터 이름, 사용자 이름, 암호 및 이 클러스터와 연결된 Azure 저장소의 계정 이름과 계정 키.  
 8. **Azure PowerShell**을 시작하고 **C:\ADFWalkthrough**로 이동하여 준비 스크립트 **\AzureEnvironmentSetup.ps1**을 실행합니다.
 
		스크립트를 실행하려면 구독 이름이 필요하고 Microsoft 계정이 구독과 연결되어 있어야 합니다.

		  Azure PowerShell을 시작할 때마다 위의 스크립트를 사용하여 Azure 환경을 설정해야 합니다. 이 구성은 세션이 바뀌면 유지되지 않으며 여러 세션을 동시에 사용하는 경우에도 따로따로 실행해야 합니다.

		또는 **Add-AzureAccount** cmdlet을 사용하여 Azure에 로그인하고 **Select-AzureSubscription** cmdlet을 사용하여 구독을 선택할 수도 있습니다(여러 구독이 있는 경우).
...
3. [3단계: 연결된 서비스 만들기](#MainStep3). 이 단계에서는 다음과 같은 연결된 서비스를 만듭니다. 
...
1. 즐겨 사용하는 편집기에서 **uploadSampleDataAndScripts.ps1** 파일을 **C:\ADFWalkthrough** 폴더(또는 압축을 푼 파일이 포함된 폴더)에서 열고 강조 표시된 내용을 클러스터 정보로 바꾼 다음 파일을 저장합니다.
	...
	> [WACOM.NOTE] 이 스크립트를 사용하려면 컴퓨터에 sqlcmd 유틸리티가 설치되어 있어야 합니다. SQL Server를 설치한 경우에는 이 유틸리티가 이미 설치되어 있습니다. 그렇지 않은 경우 유틸리티를 [다운로드][sqlcmd-install]하고 설치합니다. 
	> 또는 C:\ADFWalkthrough\Scripts 폴더의 파일을 사용하여 pig/hive 스크립트와 샘플 파일을 Blob 저장소의 adfwalkthrough 컨테이너로 업로드하고 MarketingCamapaigns Azure SQL 데이터베이스에 MarketingCampaignEffectiveness 테이블을 만들 수도 있습니다.   
...
3. **Azure PowerShell**을 시작합니다. 
4. 샘플의 압축을 푼 위치로 이동합니다(예: **C:\ADFWalkthrough**)
5. **uploadSampleDataAndScripts.ps1을 실행합니다.** 
...
5. **새 데이터 팩터리** 블레이드에서 **LogProcessingFactory**를 **이름**에 입력합니다.

	![Data Factory Blade][image-data-factory-tutorial-new-datafactory-blade]

6. Azure 리소스 그룹 **ADF**를 아직 만들지 않은 경우 다음을 수행합니다.
	1. **리소스 그룹 이름**을 클릭하고 **새 리소스 그룹 만들기**를 클릭합니다.
	
		![Resource Group Blade][image-data-factory-tutorial-resourcegroup-blade]
	2. **리소스 그룹 만들기** 블레이드에서 **ADF**를 리소스 그룹 이름으로 입력하고 **확인**을 클릭합니다.
	
		![Create Resource Group][image-data-factory-tutorial-create-resourcegroup]
7. **리소스 그룹 이름**에서 **ADF**를 선택합니다.  
8.	**새 데이터 팩터리** 블레이드에서는 **Add to Startboard**가 기본적으로 선택되어 있습니다. 선택된 경우 데이터 팩터리에 대한 링크가 시작 보드(Azure 미리 보기 포털에 로그인할 때 표시되는 페이지)에 추가됩니다.
...
10.	데이터 팩터리가 만들어지면 **LogProcessingFactory**라는 제목의 **데이터 팩터리** 블레이드가 표시됩니다.
	...
	- 왼쪽에서 **찾아보기**를 클릭하고, **모두**를 클릭하고, **데이터 팩터리**를 클릭한 다음 데이터 팩터리를 클릭합니다.
...
2. **연결된 서비스** 블레이드의 명령 모음에서 **+ Data Store**를 클릭합니다.	

	![Linked Services - Add Store][image-data-factory-tutorial-linkedservices-add-datstore]

3. **새 데이터 저장소** 블레이드에서 **StorageLinkedService**를 **이름**에 입력하고 **TYPE (settings required)**을 클릭하고 **Azure 저장소 계정**을 선택합니다.
...
5. 새 데이터 저장소 블레이드에서 **확인**을 클릭하면 **연결된 서비스** 블레이드의 **데이터 저장소** 목록에 **StorageLinkedService**가 표시됩니다. 왼쪽의 **알림** 허브에서 메시지를 확인합니다.
...
7. 연결된 서비스 블레이드에서 목록에 **StorageLinkedService** 및 **HDInsightStorageLinkedService**가 모두 표시되는지 확인합니다.
8. **연결된 서비스** 블레이드의 명령 모음에서 **Add (+) Data Store**를 클릭합니다.
9. 이름에 **AzureSqlLinkedService**를 입력합니다.
10. **TYPE (settings required)**을 클릭하고 **Azure SQL 데이터베이스**를 선택합니다.
11. 이제 **새 데이터 저장소** 블레이드에 다음과 같은 추가 필드가 표시됩니다. Azure SQL 데이터베이스 **서버**의 이름, **데이터베이스** 이름, **사용자 이름** 및 **암호**를 입력하고 **확인**을 클릭합니다.
	1. **MarketingCampaigns**를 **데이터베이스**에 입력합니다. 이 Azure SQL 데이터베이스는 1단계에서 실행한 스크립트에 의해 생성되었습니다. 이 데이터베이스가 스크립트에 의해 실제로 생성되었는지 확인해야 합니다(오류가 발생한 경우).
		
 		![Azure SQL Settings][image-data-factory-tutorial-azuresql-settings]

		Azure 관리 포털에서 이러한 값을 가지려면 MarketingCampaigns 데이터베이스에 대한 SQL 데이터베이스 연결 문자열 보기를 클릭합니다.

		![Azure SQL Database Connection String][image-data-factory-tutorial-azuresql-database-connection-string]

12. 만든 3개의 데이터 저장소 **StorageLinkedService**, **HDInsightStorageLinkedService** 및 **AzureSqlLinkedService**가 모두 표시되는지 확인합니다.
13. 연결된 서비스를 또 하나 만들어야 하지만, 이 서비스는 계산 서비스 특히, **Azure HDInsight 클러스터**여야 합니다. 아직은 포털에서 계산 연결된 서비스를 만들 수 없습니다. 따라서 이 연결된 서비스를 만들려면 Azure PowerShell을 사용해야 합니다. 
14. **Azure PowerShell**로 전환하거나(이미 연 경우) **Azure PowerShell**을 시작합니다.
...
17. 즐겨 사용하는 편집기에서 **HDInsightLinkedService.json**을 열고, 강조 표시된 내용을 클러스터 정보로 바꾸고 파일을 저장합니다.
...
20. 아래와 같이 **연결된 서비스** 블레이드에 4개의 연결된 서비스가 모두 표시되어야 합니다. 연결된 서비스가 열려 있지 않은 경우 **LogProcessingFactory**의 **데이터 팩터리** 페이지에서 연결된 서비스를 클릭합니다. 연결된 서비스 블레이드가 새로 고쳐질 때까지 몇 초 정도 걸릴 수 있습니다.
...
4. **Azure 미리 보기 포털**에서 **LogProcessingFactory**의 **DATA FACTORY** 블레이드에서 **데이터 집합**을 클릭하고 데이터 집합이 모두 표시되는지 확인합니다(테이블은 사각형 데이터 집합). 
...
2.	즐겨 사용하는 편집기에서 **PartitionGameLogsPipeline.json**을 열고 강조 표시된 내용을 데이터 저장소 계정 정보로 바꾸고 파일을 저장합니다.
...
파이프라인의 활성 기간을 지정하려면 Set-AzureDataFactoryPipelineActivePeriod cmdlet을 사용할 수 있습니다. 이 연습에서는 샘플 데이터가 5월 1일에서 5월 5일까지이므로 2014-05-01을 StartDateTime으로 사용합니다. EndDateTime은 선택 사항입니다.
...
11. **Azure 미리 보기 포털**에서 **LogProcessingFactory**의 **DATA FACTORY** 블레이드에서 파이프라인 이름이 아니라 **파이프라인** 타일을 클릭하면 만든 파이프라인이 표시됩니다.
...
1.	LogProcessingFactory에 대한 데이터 팩터리 블레이드가 열려 있지 않은 경우 다음 중 하나를 수행할 수 있습니다.
	1.	**시작 보드**에서 **LogProcessingFactory**를 클릭합니다. 데이터 팩터리를 만들 때 **Add to Startboard** 옵션이 자동으로 선택되어 있었습니다.
	...
	3. **찾아보기** 블레이드에서 **데이터 팩터리**를 선택하고**데이터 팩터리** 블레이드에서 **LogProcessingFactory**를 선택합니다.
...
6. **RawGameEventsTable**의 테이블 블레이드에 조각이 모두 표시됩니다. 다음 스크린샷에서는 모든 조각이 **준비** 상태이고 문제 조각이 없습니다. 즉, 데이터를 처리할 준비가 되었습니다.	
...
9. **Produced datasets** 블레이드에서 **PartitionedGameEvents** 테이블을 클릭합니다. 
10.	모든 조각의 **상태**가 **Ready**로 설정되어 있는지 확인합니다. 
11.	**Ready** 상태인 조각 중 하나를 클릭하여 해당 조각의 **데이터 조각** 블레이드를 표시합니다.
...
12.	**데이터 조각** 블레이드의 **Activity Runs** 목록에서 실행을 클릭합니다. 해당 조각의 Activity Run 블레이드가 표시됩니다. 다음과 같은 **ACTIVITY RUN DETAILS** 블레이드가 표시됩니다.
...
**(권장)** [파이프라인에서 온-프레미스 데이터를 사용할 수 있도록 설정][useonpremisesdatasources] 문서를 검토하고 온-프레미스 SQL Server에서 Azure Blob 저장소로 데이터를 이동하는 파이프라인을 만드는 연습을 실습합니다.
...
1. [1단계: 데이터 관리 게이트웨이 만들기](#OnPremStep1). 데이터 관리 게이트웨이  는 클라우드에서 조직의 온-프레미스 데이터 원본에 액세스할 수 있게 해주는 클라이언트 에이전트입니다. 이 게이트웨이를 사용하여 온-프레미스 SQL Server와 Azure 데이터 저장소 간에 데이터를 전송할 수 있습니다.	

	온-프레미스 SQL Server 데이터베이스를 Azure 데이터 팩터리에 연결된 서비스로 추가하기 전에 회사 환경에 게이트웨이를 2개 이상 설치하고 Azure 데이터 팩터리에 등록해야 합니다.

2. [2단계: 온-프레미스 SQL Server에 대한 연결된 서비스 만들기](#OnPremStep2). 이 단계에서는 먼저 온-프레미스 SQL Server 컴퓨터에서 데이터베이스와 테이블을 만든 다음 연결된 서비스 **OnPremSqlLinkedService**를 만듭니다.  
3. [3단계: 테이블 및 파이프라인 만들기](#OnPremStep3). 이 단계에서는 **MarketingCampaignEffectivenessOnPremSQLTable** 테이블과 **EgressDataToOnPremPipeline** 파이프라인을 만듭니다. 

4. [4단계: 파이프라인을 모니터링하고 결과 보기](#OnPremStep4). 이 단계에서는 Azure 포털을 사용하여 파이프라인, 테이블 및 데이터 조각을 모니터링합니다.


### <a name="OnPremStep1"></a>1단계: 데이터 관리 게이트웨이 만들기
...
1.	논리 데이터 게이트웨이를 만듭니다. **Azure 미리 보기 포털**의 **데이터 팩터리** 블레이드에서 **연결된 서비스**를 클릭합니다.
2.	명령 모음에서 **Add (+) Data Gateway**를 클릭합니다.  
3.	**새 데이터 게이트웨이** 블레이드에서 **만들기**를 클릭합니다.
4.	**만들기** 블레이드에서 **MyGateway**를 데이터 게이트웨이 **이름**으로 입력합니다.
5.	**PICK A REGION**을 클릭하고 필요한 경우 지역을 변경합니다. 
6.	**만들기** 블레이드에서 **확인**을 클릭합니다. 
7.	**구성** 블레이드가 표시됩니다. 
8.	**구성** 블레이드에서 **Install directly on this computer**를 클릭합니다. 그러면 게이트웨이가 컴퓨터에 다운로드되어 설치 및 구성되고 서비스에 등록됩니다. 포털의 이 논리 게이트웨이에 연결할 기존 게이트웨이가 컴퓨터에 설치되어 있는 경우 데이터 관리 게이트웨이 구성 관리자(미리 보기) 도구를 사용하여 이 블레이드에서 키를 사용하여 기존 게이트웨이를 다시 등록합니다.

	![Data Management Gateway Configuration Manager][image-data-factory-datamanagementgateway-configuration-manager]

9. **확인**을 클릭하여 **구성** 블레이드를 닫고 **확인**을 클릭하여 **만들기** 블레이드를 닫습니다. **연결된 서비스** 블레이드에서 **MyGateway**의 상태가 **GOOD**으로 변경될 때까지 기다립니다. 또한 **데이터 관리 게이트웨이 구성 관리자(미리 보기)** 도구를 시작하여 게이트웨이 이름의 포털의 이름과 일치하고 **상태**가 **Registered**인지 확인할 수도 있습니다. 연결된 서비스를 닫았다가 다시 열여야 최신 상태를 표시할 수 있습니다. 화면이 최신 상태로 새로 고쳐지려면 몇 분 정도 걸릴 수 있습니다.	

### <a name="OnPremStep2"></a>2단계: 온-프레미스 SQL Server에 대한 연결된 서비스 만들기

이 단계에서는 먼저 온-프레미스 SQL Server 컴퓨터에서 필요한 데이터베이스와 테이블을 만든 다음 연결된 서비스를 만듭니다.

#### 온-프레미스 데이터베이스 및 테이블 준비

먼저 SQL Server 데이터베이스, 테이블, 사용자 정의 형식 및 저장 프로시저를 만들어야 합니다. 이들은 **MarketingCampaignEffectiveness** 결과를 Azure Blob에서 SQL Server 데이터베이스로 이동하는 데 사용됩니다.
...
4.	**prepareOnPremDatabase&Table.ps1을 실행합니다.** **(& 큰따옴표 또는 아래와 같이)**.
...
1.	**Azure 미리 보기 포털**에서 **LogProcessingFactory**의 **데이터 팩터리** 블레이드에서 **연결된 서비스** 타일을 클릭합니다.
2.	**연결된 서비스** 블레이드에서 **Add (+) Data Store**를 클릭합니다.
3.	**새 데이터 저장소** 블레이드에서 **OnPremSqlLinkedService**를 **이름**에 입력합니다. 
4.	**Type (Settings required)**을 클릭하고 **SQL Server**를 선택합니다. 이제 **새 데이터 저장소** 블레이드에 **데이터 게이트웨이**, **서버**, **데이터베이스** 및 **자격 증명** 설정이 표시됩니다. 
5.	**DATA GATEWAY (configure required settings)**를 클릭하고 앞에서 만든 **MyGateway**를 선택합니다. 
6.	**MarketingCampaigns** 데이터베이스를 호스트하는 데이터베이스 서버의 **이름**을 입력합니다. 
7.	데이터베이스에 **MarketingCampaigns**를 입력합니다. 
8.	**자격 증명**을 클릭합니다. 
9.	**자격 증명** 블레이드에서 **Click here to set credentials securely**를 클릭합니다.
10.	One-Click 응용 프로그램이 처음으로 설치되고 **Setting Credentials **대화 상자가 시작됩니다. 
11.	**Setting Credentials** 대화 상자에서 **사용자 이름** 및 **암호**를 입력하고 **확인**을 클릭합니다. 대화 상자가 닫힐 때까지 기다립니다. 
12.	**새 데이터 저장소** 블레이드에서 **확인**을 클릭합니다. 
13.	**연결된 서비스** 블레이드에서 **OnPremSqlLinkedService**가 목록에 표시되고 연결된 서비스의 **상태**가 **Good**인지 확인합니다.

### <a name="OnPremStep3"></a>3단계: 테이블 및 파이프라인 만들기
...
2. **Set-AzureDataFactoryPipelineActivePeriod** cmdlet을 사용하여 **EgressDataToOnPremPipeline**의 활성 기간을 설정합니다.
...
### <a name="OnPremStep4"></a>4단계: 파이프라인을 모니터링하고 결과 보기

이제 [6단계: 테이블 및 파이프라인 모니터링](#MainStep6)  에 설명된 단계를 동일하게 이용하여 새 온-프레미스 ADF 테이블에 대한 새 파이프라인과 데이터 조각을 모니터링할 수 있습니다.
 
**MarketingCampaignEffectivenessOnPremSQLTable** 테이블의 조각 상태가 Ready로 변경되면 조각의 파이프라인 실행이 완료된 것입니다. 결과를 보려면 SQL Server의 **MarketingCampaigns** 데이터베이스에서 **MarketingCampaignEffectiveness** 테이블을 쿼리합니다.
 
축하합니다. 온-프레미스 데이터를 사용하는 연습을 성공적으로 마쳤습니다. 
 
## 참고 항목

문서 | 설명
------ | ---------------
[파이프라인에서 온-프레미스 데이터를 사용할 수 있도록 설정][useonpremisesdatasources] | 이 문서의 연습에서는 온-프레미스 SQL Server 데이터베이스에서 Azure Blob으로 데이터를 복사하는 방법을 보여 줍니다.
[데이터 팩터리에서 Pig 및 Hive 사용][usepigandhive] | 이 문서의 연습에서는 HDInsight 작업을 사용하여 hive/pig 스크립트로 입력 데이터를 처리하고 출력 데이터를 생성하는 방법을 보여 줍니다. 
...
[Azure 데이터 팩터리 Cmdlet 참조][cmdlet-reference] | 이 참조 콘텐츠에는 모든 데이터 팩터리 cmdlet에 대한 내용이 자세히 나와 있습니다. 
