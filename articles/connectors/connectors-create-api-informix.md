...
논리 앱 내에서 Informix 데이터베이스의 테이블을 나열 하는 작업을 추가할 수 있습니다. 이 작업은 `CALL SYSIBM.SQLTABLES`와 같은 Informix 스키마 문을 처리 하도록 커넥터에 지시 합니다.
...
| database |필수 사항입니다. DRDA 관계형 데이터베이스 이름(RDBNAM)을 나타내는 문자열 값이 허용됩니다. IBM Informix 데이터베이스 이름(dbname)으로 인식되는 데이터베이스인 Informix에는 128바이트 문자열이 허용됩니다. |
| authentication |선택 사항입니다. 목록 항목 값, 즉 기본 또는 Windows (kerberos)가 허용됩니다. |
| username |필수 사항입니다. 문자열 값을 허용합니다. |
| 암호 |필수 사항입니다. 문자열 값을 허용합니다. |
| 게이트웨이 |필수 사항입니다. 스토리지 그룹의 Logic Apps에 정의된 온-프레미스 데이터 게이트웨이를 나타내는 목록 항목 값이 허용됩니다. |
...
### <a name="create-a-logic-app"></a>논리 앱 만들기
1. **Azure 시작 보드**에서 **+** (더하기 기호), **웹 + 모바일**, **논리 앱**을 차례로 선택합니다.
2. **이름** (예: "**InformixgetRows**"), **구독**, **리소스 그룹**, **위치**및 **App Service 계획**을 입력 합니다. **대시보드에 고정**을 선택하고 **만들기**를 선택합니다.
...
Informix 테이블에서 한 행을 추가 하는 논리 앱 작업을 만들 수 있습니다. 이 작업은 `INSERT INTO AREA (AREAID, AREADESC, REGIONID) VALUES ('99999', 'Area 99999', 102)`와 같은 Informix INSERT 문을 처리 하도록 커넥터에 지시 합니다.
...
Informix 테이블에서 한 행을 인출 하는 논리 앱 작업을 만들 수 있습니다. 이 작업을 수행 하면 커넥터에서 `SELECT FROM AREA WHERE AREAID = '99999'`와 같은 Informix SELECT WHERE 문을 처리 하도록 지시 합니다.
...
Informix 테이블에서 한 행을 변경 하는 논리 앱 작업을 만들 수 있습니다. 이 작업은 `UPDATE AREA SET AREAID = '99999', AREADESC = 'Area 99999', REGIONID = 102)`와 같은 Informix UPDATE 문을 처리 하도록 커넥터에 지시 합니다.
...
Informix 테이블에서 한 행을 제거 하는 논리 앱 작업을 만들 수 있습니다. 이 작업은 `DELETE FROM AREA WHERE AREAID = '99999'`와 같은 Informix DELETE 문을 처리 하도록 커넥터에 지시 합니다.
...
[Create a logic app를 참조하세요. [API 목록](apis-list.md)에서 Logic Apps의 사용 가능한 다른 커넥터를 확인하세요.
