...
1. 되풀이에 대한 간격 및 빈도를 설정합니다. 이 예제에서는 매주 워크플로를 실행하도록 이러한 속성을 설정합니다.
   ...
   | 속성 | 필수 | JSON 이름 | 유형 | 설명 |
   |----------|----------|-----------|------|-------------|
   | **간격은** | 예 | interval | 정수 | 빈도에 따라 워크플로가 얼마나 자주 실행되는지를 설명하는 양의 정수입니다. 다음은 최소 및 최대 간격입니다. <p>- 시간: 1-12,000시간 </br>- 분: 1-72,000분 </br>- 초: 1-9,999,999초<p>예를 들어 간격이 6이 고 빈도가 "Hour" 이면 되풀이 간격은 6 시간 마다입니다. |
   | **빈도** | 예 | frequency | 문자열 | 되풀이 시간 단위: **초**, **분**또는 **시간** |
   ||||||
