---
title: SFTP 계정-Azure Logic Apps에 연결
...
SFTP 커넥터 처리 된 파일만 *50MB 또는 더 적게* 지원 하지 않습니다 [메시지 청크](../logic-apps/logic-apps-handle-large-messages.md)합니다. 더 큰 파일을 사용 합니다 [SFTP-SSH 커넥터](../connectors/connectors-sftp-ssh.md)합니다. SFTP 커넥터와 SFTP-SSH connector 사이의 차이점에 대 한 검토 [비교 SFTP-SSH 및 SFTP](../connectors/connectors-sftp-ssh.md#comparison) 문서의 SFTP-SSH 합니다.
...
* 논리 앱에서 SFTP 계정에 액세스하도록 하는 SFTP 서버 주소 및 계정 자격 증명 [SSH(Secure Shell)](https://www.ssh.com/ssh/protocol/) 프로토콜을 사용하려면 SSH 개인 키 및 SSH 개인 키 암호에 대한 액세스도 필요합니다.
  ...
  > [!NOTE]
  >
  > SFTP 커넥터는 다음과 같은 프라이빗 키 형식을 지원합니다. OpenSSH, ssh.com 및 PuTTY와 같은 개인 키 형식을 지원합니다.
  >
...
## <a name="how-sftp-triggers-work"></a>SFTP 작업 트리거 하는 방법

SFTP는 SFTP 파일 시스템에서 마지막 폴링 이후 변경 된 모든 파일에 대 한 확인 하 여 작업을 트리거합니다. 일부 도구를 통해 파일을 변경하는 경우 타임스탬프를 유지할 수 있습니다. 이러한 경우 트리거가 작동할 수 있도록 이 기능을 사용하지 않도록 설정해야 합니다. 아래에는 몇 가지 일반적인 설정이 나와 있습니다.
...
### <a name="sftp-trigger-when-a-file-is-added-or-modified"></a>SFTP 트리거: 파일이 추가되거나 수정된 경우
