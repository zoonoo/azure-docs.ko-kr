...
DB2 데이터베이스 테이블에서 한 개의 레코드를 페치하려면 논리 앱에서 **행 가져오기** 작업을 사용합니다. 이 작업은 `SELECT WHERE`와 같은 DB2 `SELECT FROM AREA WHERE AREAID = '99999'` 문을 실행합니다.
...
DB2 데이터베이스 테이블에서 모든 레코드를 페치하려면 논리 앱에서 **행 가져오기** 작업을 사용합니다. 이 작업은 `SELECT`와 같은 DB2 `SELECT * FROM AREA` 문을 실행합니다.
...
DB2 데이터베이스 테이블에 단일 레코드를 추가하려면 논리 앱에서 **행 삽입** 작업을 사용합니다. 이 작업은 `INSERT`와 같은 DB2 `INSERT INTO AREA (AREAID, AREADESC, REGIONID) VALUES ('99999', 'Area 99999', 102)` 문을 실행합니다.
...
DB2 데이터베이스 테이블에서 단일 레코드를 업데이트하려면 논리 앱에서 **행 업데이트** 작업을 사용합니다. 이 작업은 `UPDATE`와 같은 DB2 `UPDATE AREA SET AREAID = '99999', AREADESC = 'Updated 99999', REGIONID = 102)` 문을 실행합니다.
...
DB2 데이터베이스 테이블에서 단일 레코드를 삭제하려면 논리 앱에서 **행 삭제** 작업을 사용합니다. 이 작업은 `DELETE`와 같은 DB2 `DELETE FROM AREA WHERE AREAID = '99999'` 문을 실행합니다.
