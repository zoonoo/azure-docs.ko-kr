...
description: Azure Logic Apps용 기본 제공, 관리되는 API, 온-프레미스, 통합 계정 및 엔터프라이즈 커넥터를 사용하여 워크플로를 자동화합니다.
services: logic-apps
...
ms.locfileid: "37096614"
---
# <a name="connectors-for-azure-logic-apps"></a>Azure Logic Apps용 커넥터

Azure Logic Apps를 사용하여 자동화된 워크플로를 만드는 경우 커넥터가 중요한 역할을 수행합니다. 이미 만들어 사용하고 있는 데이터로 작업을 수행할 수 있도록, 논리 앱에서 커넥터를 사용하여 온-프레미스 및 클라우드 앱에 대한 기능을 확장합니다. Logic Apps는 200개 이상의 커넥터를 제공하지만, 이 문서에서는 수천 개의 앱 및 수백만 개의 실행에서 데이터와 정보를 처리하는 데 성공적으로 사용되는 매우 일반적이며 인기 있는 커넥터에 대해 설명합니다.
커넥터는 기본 제공 커넥터 또는 관리되는 커넥터로 사용할 수 있습니다. 

* [**기본 제공 커넥터**](#built-ins): 이러한 기본 제공 작업 및 트리거를 사용하면 논리 앱을 만들 수 있습니다. 이 논리 앱에서는 사용자 지정 일정에 따라 실행되고, 다른 엔드포인트와 통신하고, 요청을 받고 이에 응답하며, Azure 함수, Azure API Apps(Web Apps), Azure API Management를 통해 관리되고 게시되는 사용자 고유 API 및 요청을 받을 수 있는 중첩된 논리 앱을 호출합니다. 또한 논리 앱의 워크플로를 구성 및 제어하고, 데이터를 사용하는 데 도움이 되는 기본 제공 작업을 사용할 수도 있습니다.

* **관리되는 커넥터**: 이러한 커넥터는 다른 서비스와 시스템에 액세스하기 위한 트리거와 작업을 제공합니다. 일부 커넥터에서는 Azure Logic Apps에서 관리되는 연결을 먼저 만들어야 합니다. 관리되는 커넥터는 다음과 같은 그룹으로 구성됩니다.
...
Swagger 설명으로 정의되는 각 커넥터의 트리거 및 작업에 대한 기술 정보는 [커넥터 세부 정보](/connectors/)를 참조하세요. 비용 정보는 [Logic Apps 가격 세부 정보](https://azure.microsoft.com/pricing/details/logic-apps/) 및 [Logic Apps 가격 책정 모델](../logic-apps/logic-apps-pricing.md)을 참조하세요. 
...
|   |   | 
|---|---| 
| ![기본 제공 아이콘][data-operations-icon]<br/>**데이터 작업** | 데이터를 사용하여 수행하는 작업: <p>- **작성**: 다양한 형식의 여러 입력에서 하나의 출력을 만듭니다. <br>- **CSV 테이블 만들기**: JSON 개체를 사용하여 배열에서 CSV(쉼표로 구분된 값) 테이블을 만듭니다. <br>- **HTML 테이블 만들기**: JSON 개체를 사용하여 배열에서 HTML 테이블을 만듭니다. <br>- **배열 필터링**: 조건을 충족하는 다른 배열의 항목에서 배열을 만듭니다. <br>- **조인**: 배열의 모든 항목에서 문자열을 만들고, 지정한 구분 기호를 사용하여 해당 항목을 구분합니다. <br>- **JSON 구문 분석**: JSON 콘텐츠의 속성 및 해당 값에서 사용자에게 친숙한 토큰을 만들어 워크플로에서 이러한 속성을 사용할 수 있습니다. <br>- **선택**: 다른 배열의 항목이나 값을 변환하고 해당 항목을 지정한 속성에 매핑하여 JSON 개체가 포함된 배열을 만듭니다. | 
...
## <a name="more-about-triggers-and-actions"></a>트리거 및 작업에 대한 추가 정보

일부 커넥터에서는 특정 이벤트가 발생할 때 논리 앱에 알리는 *트리거*를 제공합니다. 따라서 이러한 이벤트가 발생하면 트리거에서 논리 앱의 인스턴스를 만들고 실행합니다. 예를 들어 FTP 커넥터는 파일을 업데이트할 때 논리 앱을 시작하는 "파일이 추가되거나 수정된 경우" 트리거를 제공합니다. 

Logic Apps에서 제공하는 이러한 종류의 트리거는 다음과 같습니다.  

* *폴링 트리거*: 이러한 트리거는 지정한 빈도로 서비스를 폴링하고 새 데이터를 확인합니다. 

  새 데이터를 사용할 수 있게 되면 논리 앱의 새 인스턴스가 만들어지고 입력으로 전달된 데이터를 사용하여 실행됩니다. 

* *푸시 트리거*: 이러한 트리거는 엔드포인트에서 새 데이터를 받거나 이벤트가 발생하여 논리 앱의 새 인스턴스를 만들고 실행합니다.

* *되풀이 트리거*: 이 트리거는 지정한 일정에 따라 논리 앱의 인스턴스를 만들고 실행합니다.

또한 커넥터는 논리 앱의 작업 흐름에서 작업을 수행하는 *작업*도 제공합니다. 예를 들어 논리 앱에서 데이터를 읽고, 논리 앱의 나중 단계에서 이 데이터를 사용할 수 있습니다. 더 구체적으로, 논리 앱에서 SQL 데이터베이스로부터 고객 데이터를 찾고, 나중에 논리 앱의 워크플로에서 이 데이터를 처리할 수 있습니다. 

트리거 및 작업에 대한 자세한 내용은 [커넥터 개요](connectors-overview.md)를 참조하세요. 
