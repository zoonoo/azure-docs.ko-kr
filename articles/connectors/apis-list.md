...
* [**기본**](#built-ins)제공: 기본 제공 트리거 및 작업은 기본적으로 Azure Logic Apps에서 실행 되므로 연결을 만들 필요 없이 논리 앱에 대해 이러한 작업을 수행 하는 데 도움이 됩니다.
...
| [![HTTP 기본 제공 커넥터 ][http-icon]<br> **http**][http-doc] | HTTP에 대한 트리거 및 작업을 사용하여 HTTP 또는 HTTPS 엔드포인트를 호출합니다. 다른 HTTP 기본 제공 트리거 및 작업에는 [http + Swagger 기본 제공 커넥터][http-swagger-doc] 및 [http + Webhook][http-webhook-doc]가 포함 됩니다. |
| [![기본 제공 커넥터 ][http-request-icon]<br> **요청** 요청][http-request-doc] | - 논리 앱을 다른 앱이나 서비스에서 호출할 수 있게 하고, Event Grid 리소스 이벤트를 트리거하거나, **요청** 트리거를 사용하여 Azure Security Center 경고에 대한 응답을 트리거합니다. <br>- **응답** 작업을 사용하여 앱이나 서비스에 응답을 보냅니다. |
...
| [![Azure 앱 Services 기본 제공 커넥터 ][azure-app-services-icon]<br> **Azure 앱 <br> 서비스**][azure-app-services-doc] | Azure App Service에서 호스팅되는 Azure API Apps 또는 Web Apps를 호출합니다. Swagger가 포함되는 경우 이러한 앱에서 정의된 트리거 및 작업은 다른 모든 고급 트리거 및 작업처럼 나타납니다. |
| [![Azure Logic Apps 기본 제공 커넥터 ][azure-logic-apps-icon]<br> **Azure 논리 <br> 앱**][nested-logic-app-doc] | **요청** 트리거로 시작 하는 다른 논리 앱을 호출 합니다. |
|||
...
| [![][for-each-icon]<br> 각 기본 제공 작업에 대해][for-each-doc] | 배열의 모든 항목에 대해 동일한 작업을 수행합니다. |
| [![기본 제공 작업 ][scope-icon]<br> **범위** 범위][scope-doc] | 범위에 속한 작업의 실행이 완료되면 해당 상태를 가져오는 *범위* 로 작업을 그룹화합니다. |
| [![기본 제공 동작 ][switch-icon]<br> **스위치** 전환][switch-doc] | 기본 사례를 제외하고는 고유한 값이 할당된 *사례* 로 작업을 그룹화합니다. 할당된 값이 식, 개체 또는 토큰의 결과와 일치하는 사례만 실행합니다. 일치하는 사례가 없는 경우 기본 사례를 실행합니다. |
| [![기본 ][terminate-icon]<br>  제공 작업 종료 종료][terminate-doc] | 활발히 실행 중인 논리 앱 워크플로를 중지합니다. |
| [![다음까지 기본 제공 작업 ][until-icon]<br>  까지][until-doc] | 지정된 조건이 true이거나 일부 상태가 변경될 때까지 작업을 반복합니다. |
|||
...
| [![변수 기본 제공 작업 ][variables-icon]<br> **변수**][variables-doc] | 변수를 사용하여 수행하는 작업: <p>- **배열 변수에 추가**: 값을 변수로 저장되는 배열의 마지막 항목으로 삽입합니다. <br>- **문자열 변수에 추가**: 값을 변수로 저장되는 문자열의 마지막 문자로 삽입합니다. <br>- **변수 감소**: 변수를 상수 값만큼 줄입니다. <br>- **변수 증가**: 변수를 상수 값만큼 늘립니다. <br>- **변수 초기화**: 변수를 만들고 해당 데이터 형식과 초기 값을 선언합니다. <br>- **변수 설정**: 기존 변수에 다른 값을 할당합니다. |
|||
...
| [![][azure-blob-storage-icon]<br>**Azure Blob <br> Storage** Azure Blob Storage 관리 되는 커넥터][azure-blob-storage-doc] | 스토리지 계정에 연결하여 Blob 콘텐츠를 만들고 관리할 수 있습니다. |
...
| [![FTP 관리 커넥터 ][ftp-icon]<br> **ftp**][ftp-doc] | 파일 및 폴더에 대 한 작업을 수행할 수 있도록 인터넷에서 액세스할 수 있는 FTP 서버에 연결 합니다. |
| [![파일 시스템 관리 커넥터 ][file-system-icon]<br> **파일 <br> 시스템**][file-system-doc] | 파일을 만들고 관리할 수 있도록 온-프레미스 파일 공유에 연결 합니다. |
...
| [![Azure Event Grid managed connector ][azure-event-grid-icon]<br> **Azure Event** <br> **Grid**][azure-event-grid-doc] | Azure 리소스 또는 타사 리소스가 변경되는 경우와 같이 Event Grid에서 게시한 이벤트를 모니터링합니다. |
| [![Salesforce 관리 커넥터 ][salesforce-icon]<br> **salesforce**][salesforce-doc] | 레코드, 작업, 개체 등의 항목을 만들고 관리할 수 있도록 Salesforce 계정에 연결 합니다. |
|||
...
다음은 온-프레미스 시스템의 데이터 및 리소스에 액세스 하기 위해 Logic Apps 제공 하는 일반적으로 *사용 되는* 표준 커넥터입니다. 온-프레미스 커넥터 목록은 [지원 되는 데이터 원본](../logic-apps/logic-apps-gateway-connection.md#supported-connections)을 참조 하세요.
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![파일 시스템 커넥터 ][file-system-icon]<br> **파일 <br> 시스템**][file-system-doc]
    :::column-end:::
    :::column:::
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![SharePoint Server 커넥터 ][sharepoint-server-icon]<br> **sharepoint <br> 서버**][sharepoint-server-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![AS2 인코딩 동작 ][as2-icon]<br> **as2 <br> 인코딩**][as2-doc]
    :::column-end:::
    :::column:::
...
:::row:::
    :::column:::
        [![플랫 파일 디코딩 동작 ][flat-file-decode-icon]<br> **플랫 파일 <br> 디코딩**][flat-file-decode-doc]
    :::column-end:::
    :::column:::
        [![플랫 파일 인코딩 동작 ][flat-file-encode-icon]<br> **플랫 파일 <br> 인코딩**][flat-file-encode-doc]
    :::column-end:::
    :::column:::
        [![통합 계정 작업 ][integration-account-icon]<br> **통합 <br> 계정**][integration-account-doc]
    :::column-end:::
    :::column:::
        [![액체 변형 작업 ][liquid-icon]<br> **액체** <br> **변환**][json-liquid-transform-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
        [![X12 디코딩 동작 ][x12-icon]<br> **x12 <br> 디코딩**][x12-decode-doc]
    :::column-end:::
    :::column:::
        [![X12 인코딩 동작 ][x12-icon]<br> **x12 <br> 인코딩**][x12-encode-doc]
    :::column-end:::
    :::column:::
        [![Xml 변환 작업 ][xml-transform-icon]<br> **xml** <br> **변환**][xml-transform-doc]
    :::column-end:::
    :::column:::
        [![XML 유효성 검사 동작 ][xml-validate-icon]<br> **xml <br> 유효성 검사**][xml-validate-doc]
    :::column-end:::
:::row-end:::
...
:::row:::
    ...
    :::column-end:::
    :::column:::
        [![][azure-automation-icon]<br>**Azure <br> Automation** Azure Automation ISE 커넥터][azure-automation-doc]
    :::column-end:::
    :::column:::
        [![][azure-blob-storage-icon]<br>**Azure Blob <br> Storage** Azure Blob Storage ISE 커넥터][azure-blob-storage-doc]
    :::column-end:::
    :::column:::
        [![][azure-cosmos-db-icon]<br>**Azure Cosmos <br> DB** Azure Cosmos DB ISE 커넥터][azure-cosmos-db-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
    ...
    :::column-end:::
    :::column:::
        [![Azure Key Vault ISE 커넥터 ][azure-key-vault-icon]<br> **Azure 주요 <br> 자격 증명 모음**][azure-key-vault-doc]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
        [![Azure Monitor 로그 ISE 커넥터 ][azure-monitor-logs-icon]<br> **Azure Monitor <br> 로그**][azure-monitor-logs-doc]
    :::column-end:::
    :::column:::
...
:::row:::
    :::column:::
        [![Azure 큐 ISE 커넥터 ][azure-queues-icon]<br> **azure <br> 큐**][azure-queues-doc]
    :::column-end:::
    :::column:::
        [![EDIFACT ISE 커넥터 ][edifact-icon]<br> **edifact**][edifact-doc]
    :::column-end:::
    :::column:::
        [![파일 시스템 ISE 커넥터 ][file-system-icon]<br> **파일 <br> 시스템**][file-system-doc]
    :::column-end:::
    :::column:::
...
되풀이 [트리거와](../connectors/connectors-native-recurrence.md)같이 기본적으로 Azure Logic Apps에서 실행 되는 되풀이 기본 제공 트리거의 동작은 SQL connector 트리거와 같은 연결을 먼저 만들어야 하는 되풀이 연결 기반 트리거의 동작과 다릅니다.

그러나 두 종류의 트리거에서 모두 되풀이에서 특정 시작 날짜와 시간을 지정 하지 않는 경우에는 트리거의 되풀이 설정에도 불구 하 고 논리 앱을 저장 하거나 배포할 때 첫 번째 되풀이가 즉시 실행 됩니다. 이 동작을 방지 하려면 첫 번째 되풀이를 실행할 시작 날짜와 시간을 제공 합니다.

<a name="recurrence-built-in"></a>

### <a name="recurrence-for-built-in-triggers"></a>기본 제공 트리거의 되풀이

반복 되는 기본 제공 트리거는 사용자가 지정 하는 표준 시간대를 포함 하 여 사용자가 설정한 일정을 적용 합니다. 그러나 되풀이에서 이후 되풀이를 실행 하는 특정 시간과 같은 다른 고급 일정 옵션을 지정 하지 않는 경우 이러한 되풀이는 마지막 트리거 실행을 기반으로 합니다. 결과적으로, 해당 되풀이에 대 한 시작 시간은 저장소 호출 중의 대기 시간과 같은 요소로 인해 드리프트 될 수 있습니다. 또한 표준 시간대를 선택 하지 않은 경우에는 트리거가 실행 될 때 DST (일광 절약 시간)에 영향을 줄 수 있습니다. 예를 들어 dst가 시작 될 때 시작 시간을 1 시간 앞으로 이동 하 고, DST가 종료 되 면 1 시간 뒤로 이동 합니다.

논리 앱이 지정 된 시작 시간에 실행 되 고 되풀이를 누락 하지 않도록 하려면 (특히 빈도가 며칠 이상 일 때) 다음 해결 방법을 시도해 보세요.

* 논리 앱이 지정 된 시작 시간에 실행 되도록 표준 시간대를 선택 해야 합니다. 그렇지 않으면 트리거가 실행 될 때 dst가 영향을 받을 수 있습니다. 예를 들어 dst가 시작 될 때 시작 시간을 1 시간 앞으로 이동 하 고, DST가 종료 되 면 1 시간 뒤로

  작업을 예약할 때 Logic Apps는 메시지를 큐에 배치 하 고 마지막 작업이 실행 된 UTC 시간과 다음 작업이 실행 되도록 예약 된 UTC 시간을 기준으로 메시지를 사용할 수 있는 시기를 지정 합니다. 표준 시간대를 지정 하 여 논리 앱에 대 한 UTC 시간도 계절 시간 변경에 대 한 카운터로 이동 됩니다. 그러나 시간이 이동 하면 windows에서 문제가 발생할 수 있습니다. 자세한 내용 및 예제는 일광 절약 [시간 및 표준 시간에 대 한 되풀이](../logic-apps/concepts-schedule-automated-recurring-tasks-workflows.md#daylight-saving-standard-time)를 참조 하세요.

* 되풀이 트리거를 사용 하 여 되풀이의 시작 날짜와 시간을 지정 하 고, **해당 시간에** 명명 된 속성을 사용 하 여 이후 되풀이를 실행 하는 특정 시간 및 **요일** 및 **주** 빈도에 대해서만 사용할 수 있는 시간 **(분)** 을 제공 합니다.
...
SQL 또는 SFTP-SSH와 같은 되풀이 연결 기반 트리거에서 일정은 실행을 제어 하는 유일한 드라이버는 아니며 표준 시간대는 초기 시작 시간만 결정 합니다. 후속 실행은 되풀이 일정, 마지막 트리거 실행 *및* 런타임 시 예기치 않은 동작을 발생 시킬 수 있는 기타 요인에 따라 달라 집니다. 예를 들면 다음과 같습니다.

* 트리거가 더 많은 데이터를 포함 하는 서버에 액세스 하는지 여부를 나타냅니다 .이는 트리거가 즉시 인출 하려고 시도 합니다.

* 트리거가 발생 하는 오류 또는 다시 시도

* 저장소를 호출 하는 동안 대기 시간입니다.

* DST (일광 절약 시간)가 시작 되 고 끝날 때 지정 된 일정을 유지 하지 않습니다.

* 다음 실행 시간이 발생할 때 영향을 줄 수 있는 다른 요소

이러한 문제를 해결 하거나 해결 하려면 다음 해결 방법을 사용 하십시오.

* DST가 적용 될 때 되풀이 시간이 이동 하지 않도록 하려면 논리 앱이 예상 시간에 계속 실행 되도록 되풀이를 수동으로 조정 합니다. 그렇지 않으면 dst가 시작 될 때 시작 시간이 1 시간 뒤로 이동 하 고, DST가 종료 되 면 1 시간 뒤로 이동 합니다.

* 되풀이 트리거를 사용 하 여 표준 시간대, 시작 날짜 및 *시간을 지정할 수 있으며,* **이러한 시간에** 명명 된 속성을 사용 하 여 이후 되풀이가 실행 되는 특정 시간 및 **요일** 및 **주** 빈도에 대해서만 사용할 수 있는 시간 **(분)** 을 지정할 수 있습니다. 그러나 시간이 이동 하면 windows에서 여전히 문제가 발생 하는 경우도 있습니다. 자세한 내용 및 예제는 일광 절약 [시간 및 표준 시간에 대 한 되풀이](../logic-apps/concepts-schedule-automated-recurring-tasks-workflows.md#daylight-saving-standard-time)를 참조 하세요.

* 되풀이를 누락 하지 않으려면 되풀이 트리거 대신 [슬라이딩 윈도우 트리거](../connectors/connectors-native-sliding-window.md)를 사용 합니다.
...
[azure-sql-data-warehouse-doc]: /connectors/sqldw/ "Azure Synapse Analytics에 연결 하 여 데이터를 볼 수 있도록 합니다."
