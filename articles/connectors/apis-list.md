...
Azure Logic Apps를 사용하여 자동화된 워크플로를 만드는 경우 커넥터가 중요한 역할을 수행합니다. 이미 만들어 사용하고 있는 데이터로 작업을 수행할 수 있도록, 논리 앱에서 커넥터를 사용하여 온-프레미스 및 클라우드 앱에 대한 기능을 확장합니다. 

Logic Apps는 [200개 이상의 커넥터](https://docs.microsoft.com/connectors)를 제공하지만, 이 문서에서는 수천 개의 앱 및 수백만 개의 실행에서 데이터와 정보를 처리하는 데 성공적으로 사용되는 매우 일반적이며 인기 있는 커넥터에 대해 설명합니다. 커넥터는 기본 제공 커넥터 또는 관리되는 커넥터로 사용할 수 있습니다. 

> [!NOTE]
> 커넥터의 전체 목록과 작업, 트리거 및 제한과 같은 각 커넥터의 참조 정보는 [커넥터 개요](https://docs.microsoft.com/connectors)에서 전체 목록을 확인할 수 있습니다.
