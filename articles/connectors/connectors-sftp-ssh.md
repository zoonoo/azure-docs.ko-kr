---
title: SSH-Azure Logic Apps를 사용 하 여 SFTP 서버에 연결
...
SFTP-SSH 커넥터와 SFTP 커넥터 간 차이점에 대 한 검토 합니다 [비교 SFTP-SSH 및 SFTP](#comparison) 이 항목의 뒷부분에 나오는 섹션입니다.

## <a name="limits"></a>제한

* 기본적으로 SFTP-SSH 작업 읽거나 쓸 수 있는 파일 *1GB 이하로* 있지만 에서만 *15MB* 한 번에 청크 합니다. 처리할 파일 15MB SFTP-SSH 작업 지원 보다 큰 [메시지 청크](../logic-apps/logic-apps-handle-large-messages.md), 파일 복사 작업을 제외 하 고 15MB 파일만 처리할 수 있는 합니다. 합니다 **파일 콘텐츠 가져오기** 작업 메시지 청크 암시적으로 사용 합니다. 

* SFTP-SSH 트리거 청크를 지원 하지 않습니다. 트리거 15MB 있는 파일만 선택 파일 콘텐츠를 요청할 경우 하거나 축소 합니다. 파일을 가져오려면 15 MB 보다 큰이 패턴을 대신 수행 합니다.

  * 트리거 사용 하 여 SFTP-SSH와 같은 파일 속성을 반환 하는 **파일이 추가 되거나 (속성만)를 수정할 때**합니다.

  * 사용 하 여 트리거는 SFTP-SSH를 수행 **파일 콘텐츠 가져오기** 전체 파일을 읽고 메시지 청크를 암시적으로 사용 하는 작업입니다.
...
* 기본적으로 SFTP-SSH 작업 읽거나 쓸 수 있는 파일 *1GB 이하로* 있지만 에서만 *15MB* 한 번에 청크 합니다. 처리할 15 MB 보다 큰 파일을 SFTP-SSH 작업을 사용할 수 있습니다 [메시지 청크](../logic-apps/logic-apps-handle-large-messages.md)합니다. 그러나 파일 복사 작업이 해당 작업에서 메시지 청크를 지원 하지 않으므로 15MB 파일만 지원 합니다. SFTP-SSH 트리거 청크를 지원 하지 않습니다.
...
## <a name="how-sftp-ssh-triggers-work"></a>SFTP-SSH 트리거 작동 하는 방법

SFTP-SSH 트리거 SFTP 파일 시스템에서 마지막 폴링 이후 변경 된 모든 파일에 대 한 확인 하 여 작동 합니다. 일부 도구를 통해 파일을 변경하는 경우 타임스탬프를 유지할 수 있습니다. 이러한 경우 트리거가 작동할 수 있도록 이 기능을 사용하지 않도록 설정해야 합니다. 아래에는 몇 가지 일반적인 설정이 나와 있습니다.
