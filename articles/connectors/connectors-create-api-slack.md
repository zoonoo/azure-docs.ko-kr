...
| 텍스트 |string |yes |쿼리 |없음 |보낼 메시지의 텍스트입니다. 서식 지정 옵션을 보려면 https://api.slack.com/docs/formatting. |
| username |string |no |쿼리 |없음 |봇의 이름입니다. |
| as\_user |부울 |no |쿼리 |없음 |봇 대신 인증된 사용자로 메시지를 게시하려면 true를 전달합니다. |
| parse |string |no |쿼리 |없음 |메시지를 처리하는 방법을 변경합니다. 자세한 내용은 https://api.slack.com/docs/formatting. |
