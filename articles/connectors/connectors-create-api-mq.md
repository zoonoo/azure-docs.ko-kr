...
MQ 커넥터는 온-프레미스 또는 Azure에서 MQ 서버에 저장 된 메시지를 보내고 검색 합니다. 이 커넥터는 TCP/IP 네트워크를 통해 원격 IBM MQ 서버와 통신하는 Microsoft MQ 클라이언트를 포함합니다. 이 문서는 MQ 커넥터를 사용하는 시작 가이드를 제공합니다. 먼저 큐에서 단일 메시지를 검색한 다음, 다른 작업을 시도할 수 있습니다.

MQ 커넥터에는 이러한 작업이 포함 되지만 트리거는 제공 되지 않습니다.

- MQ 서버에서 메시지를 삭제 하지 않고 단일 메시지를 찾습니다.
- MQ 서버에서 메시지를 삭제 하지 않고 메시지 일괄 처리를 찾습니다.
- 단일 메시지를 받고 MQ 서버에서 메시지를 삭제 합니다.
- 메시지 일괄 처리를 받고 MQ 서버에서 메시지를 삭제 합니다.
- 단일 메시지를 MQ 서버에 보냅니다.
...
* 온-프레미스 MQ 서버를 사용 하는 경우 네트워크 내의 서버에 [온-프레미스 데이터 게이트웨이를 설치](../logic-apps/logic-apps-gateway-install.md) 해야 합니다.

  > [!NOTE]
  > MQ 서버를 공개적으로 사용할 수 있거나 Azure 내에서 사용할 수 있는 경우 데이터 게이트웨이를 사용할 필요가 없습니다.

  * MQ 커넥터가 작동 하려면 온-프레미스 데이터 게이트웨이를 설치 하는 서버에도 .NET Framework 4.6가 설치 되어 있어야 합니다.
  
  * 온-프레미스 데이터 게이트웨이를 설치한 후에는 MQ 커넥터가 온-프레미스 MQ 서버에 액세스 하는 데 사용 하는 온 [-프레미스 데이터 게이트웨이에 대 한 Azure 게이트웨이 리소스도 만들어야](../logic-apps/logic-apps-gateway-connection.md) 합니다.

* MQ 커넥터를 사용 하려는 논리 앱입니다. MQ 커넥터에는 트리거가 없으므로 먼저 논리 앱에 트리거를 추가해야 합니다. 예를 들어 [되풀이 트리거](../connectors/connectors-native-recurrence.md)를 사용할 수 있습니다. 논리 앱을 처음 접하는 경우 [첫 번째 논리 앱을 만들기 위한 빠른 시작](../logic-apps/quickstart-create-first-logic-app-workflow.md)을 사용해 보세요.

## <a name="limitations"></a>제한 사항

MQ 커넥터는 메시지의 **형식** 필드를 지원 하거나 사용 하지 않으며 문자 집합 변환을 수행 하지 않습니다. 커넥터는 메시지 필드에 표시 되는 모든 데이터를 JSON 메시지에 넣고 메시지를 함께 보냅니다.
...
1. MQ 서버에 대한 연결 정보를 입력합니다.
   ...
   * TLS (전송 계층 보안) 또는 SSL(Secure Sockets Layer) (SSL)을 사용 하려면 **Ssl 사용** 을 선택 합니다.
...
커넥터의 Swagger 파일에 설명 된 작업 및 제한과 같은 기술 세부 정보는 [커넥터의 참조 페이지를 참조](/connectors/mq/)하세요.
