...
ms.locfileid: "103495235"
---
# <a name="connect-to-an-ibm-mq-server-from-azure-logic-apps"></a>Azure Logic Apps에서 IBM MQ 서버에 연결

MQ 커넥터는 MQ 서버 온-프레미스 또는 Azure에 저장된 메시지를 전송하고 검색합니다. 이 커넥터는 TCP/IP 네트워크를 통해 원격 IBM MQ 서버와 통신하는 Microsoft MQ 클라이언트를 포함합니다. 이 문서는 MQ 커넥터를 사용하는 시작 가이드를 제공합니다. 먼저 큐에서 단일 메시지를 검색한 다음, 다른 작업을 시도할 수 있습니다.

MQ 커넥터에는 다음 작업이 포함되지만 트리거는 제공되지 않습니다.

- MQ 서버에서 메시지를 삭제하지 않고 단일 메시지를 찾아봅니다.
- MQ 서버에서 메시지를 삭제하지 않고 일괄 처리 메시지를 찾아봅니다.
- MQ 서버에서 단일 메시지를 수신하고 메시지를 삭제합니다.
- MQ 서버에서 일괄 처리 메시지를 수신하고 메시지를 삭제합니다.
...
* 온-프레미스 MQ 서버를 사용하는 경우 네트워크 내의 서버에서 [온-프레미스 데이터 게이트웨이를 설치](../logic-apps/logic-apps-gateway-install.md)해야 합니다.

  > [!NOTE]
  > MQ 서버를 공개적으로 사용할 수 있거나 Azure 내에서 사용할 수 있는 경우 데이터 게이트웨이를 사용할 필요가 없습니다.

  * MQ 커넥터가 작동하려면 온-프레미스 데이터 게이트웨이를 설치하는 서버에도 .NET Framework 4.6이 설치되어 있어야 합니다.
  
  * 온-프레미스 데이터 게이트웨이를 설치한 후에는 MQ 커넥터가 온-프레미스 MQ 서버에 액세스하는 데 사용하는 [온-프레미스 데이터 게이트웨이에 대한 Azure 게이트웨이 리소스도 만들어야 합니다](../logic-apps/logic-apps-gateway-connection.md).

* MQ 커넥터를 사용하려는 논리 앱입니다. MQ 커넥터에는 트리거가 없으므로 먼저 논리 앱에 트리거를 추가해야 합니다. 예를 들어, [되풀이 트리거](../connectors/connectors-native-recurrence.md)를 사용할 수 있습니다. 논리 앱을 처음 접하는 경우 [첫 번째 논리 앱을 만들기 위한 빠른 시작](../logic-apps/quickstart-create-first-logic-app-workflow.md)을 사용해 보세요.

## <a name="limitations"></a>제한 사항

MQ 커넥터는 메시지의 **형식** 필드를 지원하거나 사용하지 않으며 문자 집합 변환을 수행하지 않습니다. 커넥터는 메시지 필드에 표시되는 모든 데이터를 JSON 메시지에 넣고 메시지를 함께 보냅니다.
...
1. MQ 서버에 대한 연결 정보를 입력합니다.
   ...
   * TLS(전송 계층 보안) 또는 SSL(Secure Sockets Layer)을 사용하려면 **SSL 사용** 을 선택합니다.
...
* MQ 서버를 사용하려면 TLS/SSL 연결에 사용하려는 암호화 사양을 정의해야 합니다. 그러나 .NET의 SslStream에서는 암호화 사양의 순서를 지정하는 것을 허용하지 않습니다. 이 제한을 해결하기 위해 커넥터가 TLS/SSL 협상에서 보내는 도구 모음의 첫 번째 암호화 사양과 일치하도록 MQ 서버 구성을 변경할 수 있습니다.
...
커넥터의 Swagger 파일에 나오는 작업 및 제한에 대한 기술 정보는 [커넥터의 참조 페이지](/connectors/mq/)를 검토하세요.
