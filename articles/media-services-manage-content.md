...
1.  [관리 포털][]에서 **미디어 서비스**를 클릭한 후 미디어 서비스 계정 이름을 클릭합니다.
2.  콘텐츠 페이지를 선택합니다.
...
4.  Azure 미디어 인코더 대화 사자에서 일반 또는 고급 인코딩 기본 설정 중 하나를 선택합니다.
    ...
    **고급 기본 설정**
    ...
    -   [Azure 미디어 인코더에 대한 작업 기본 설정 문자열][] 항목에서는 고급 기본 설정 목록에 포함된 각 기본 설정의 의미를 설명합니다.

    ![EncoderDialog][]

    현재 포털에서는 미디어 인코더가 지원하는 일부 인코딩 형식을 지원하지 않습니다. 또한 미디어 자산 암호화/암호 해독을 지원하지 않습니다. 이러한 작업은 프로그래밍 방식으로 수행할 수 있습니다. 자세한 내용은 [Media Services SDK for .NET을 사용하여 응용 프로그램 빌드][] 및 [Azure 미디어 인코더에 대한 작업 기본 설정 문자열][]을 참조하세요.
