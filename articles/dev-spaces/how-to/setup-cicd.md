...
개발 공간을 만든 후에는 호스트 접미사를 확인 해야 합니다. @No__t-0 명령을 사용 하 여 Azure Dev Spaces 수신 컨트롤러의 호스트 접미사를 표시 합니다.
...
1. 원본으로 **github** 를 선택 하 고, 필요한 경우 github 계정에 대 한 권한을 부여 하 고, _개발 공간_ 샘플 응용 프로그램 리포지토리의 분기 버전에서 _azds_updates_ 분기를 선택 합니다.
1. 템플릿으로 **구성을 코드**또는 **yaml**로 선택 합니다.
1. 이제 빌드 파이프라인의 구성 페이지가 표시됩니다. 위에서 설명한 것 처럼 **...** 단추를 사용 하 여 **yaml 파일 경로의** 언어별 경로로 이동 합니다. 예를 들어, `samples/dotnetcore/getting-started/azure-pipelines.dotnet.yml`을 입력합니다.
1. [ **변수** ] 탭으로 이동 합니다.
...
1. **찾아보기** 를 클릭 하 고 프로젝트에서 `samples/release.json`을 선택 합니다.
...
1. 연결 형식에서 **Azure Resource Manager** 가 선택 되어 있는지 확인 **합니다.** 빨간색으로 강조 표시 된 세 개의 드롭다운 컨트롤이 표시 됩니다. ![ 릴리스 정의 설정 @ no__t-1
...
배포 후에 *webfrontend*의 _dev_ 버전은 `http://dev.webfrontend.fedcba098.eus.azds.io`와 같은 공용 URL을 통해 액세스할 수 있습니다. @No__t-0 명령을 실행 하 여이 URL을 찾을 수 있습니다. 
