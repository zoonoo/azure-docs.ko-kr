...
Azure Dev Spaces은 Azure Dev Spaces 클라이언트 도구에서 생성 된 투구 차트에 비밀을 저장 하기 위한 두 가지 권장 되는 옵션을 제공 합니다 .이 옵션은 azds 파일에 있으며, 직접 인라인 합니다. values.yaml에 비밀을 저장하지 않는 것이 좋습니다. 이 문서에 정의 된 클라이언트 도구에 의해 생성 된 투구 차트를 사용 하는 두 가지 방법 중 하나를 사용 하 여 사용자 고유의 투구 차트를 만드는 경우 투구 차트를 직접 사용 하 여 비밀을 관리 하 고 저장할 수 있습니다.
