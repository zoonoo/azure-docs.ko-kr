...
서비스의 업데이트된 버전을 배포하려면 프로젝트의 파일을 업데이트하고 *.NET Core 시작(AZDS)* 을 다시 실행하면 됩니다. 다음은 그 예입니다.
...
서비스가 디버깅 모드에서 실행 중인 경우 `Controllers/HomeController.cs`의 22줄을 업데이트합니다. 다음은 그 예입니다.
