<properties title="Azure Notification Hubs Notify Users" pageTitle="Azure 알림 허브 사용자에게 알림" metaKeywords="Azure 푸시 알림, Azure 알림 허브" description="Learn how to send secure push notifications in Azure. Code samples written in Objective-C using the .NET API." documentationCenter="Mobile" metaCanonical="" disqusComments="1" umbracoNaviHide="0" authors="yuaxu" manager="dwrede" />
...
<div class="dev-center-tutorial-selector sublanding">
    	<a href="/ko-kr/documentation/articles/notification-hubs-windows-dotnet-notify-users/" title="Windows Universal">Windows 범용</a><a href="/ko-kr/documentation/articles/notification-hubs-/" title="iOS" class="current">iOS</a>
...
> [AZURE.NOTE] 이 자습서에서는 [알림 허브 시작(iOS)](http://azure.microsoft.com/ko-kr/documentation/articles/notification-hubs-ios-get-started/)에 설명된 대로 알림 허브를 만들고 구성했다고 가정합니다. 이 자습서는 [보안 푸시(iOS)](http://azure.microsoft.com/ko-kr/documentation/articles/notification-hubs-aspnet-backend-ios-secure-push/) 자습서의 필수 조건이기도 합니다.


## 알림 허브 만들기 및 구성

[알림 허브 시작(iOS) 시작](http://azure.microsoft.com/ko-kr/documentation/articles/notification-hubs-ios-get-started/)의 1~5 섹션을 수행하세요. iOS 장치 프로비저닝에 대한 추가 리소스는 [Big Nerd Ranch](http://www.bignerdranch.com/we-teach/how-to-prepare/ios-device-provisioning.html)(영문)의 가이드를 확인하세요.

[WACOM.INCLUDE [notification-hubs-aspnet-backend-notifyusers](../includes/notification-hubs-aspnet-backend-notifyusers.md)]

## iOS 앱 수정

1. [알림 허브 시작(iOS)](http://azure.microsoft.com/ko-kr/documentation/articles/notification-hubs-ios-get-started/)의 1~5 섹션에 따라 만든 Single Page 보기 앱을 엽니다.

> [AZURE.NOTE] 이 섹션에서는 빈 조직 이름을 사용하여 프로젝트를 구성했다고 가정합니다. 그렇지 않으면 모든 클래스 이름 앞에 조직 이름을 추가해야 합니다.

2. Main.storyboard의 개체 라이브러리에서 다음 구성 요소를 추가합니다.
	+ 자리 표시자 텍스트 **Username**이 포함된 UITextField
	+ 자리 표시자 텍스트 **Password**가 포함된 UITextField, Attributes Inspector의 Textfield Return Key에서 **Secure** 옵션 선택
	+ **1. 로그인**으로 레이블이 지정된 UIButton, Attributes Inspector의 Control, Content에서 **Enabled** 옵션 선택 해제
	+ **2. 푸시 보내기**로 레이블이 지정된 UIButton, **Enabled** 옵션 선택 해제
...
4. 먼저 백 엔드와 상호 연결되는 RegisterClient 클래스를 만듭니다. NSObject에서 RegisterClient inheriting이라는 Objective-C 클래스를 만듭니다. 그런 다음 RegisterClient.h 인터페이스 섹션에 다음 코드를 추가합니다.
...
6. RegisterClient.m 구현 섹션에 다음 코드를 추가하고 *{backend endpoint}* 자리 표시자를 이전 섹션에서 앱 백 엔드를 배포하는 데 사용한 대상 URL로 대체합니다.
	...
	위의 코드는 NSURLSession를 사용하여 앱 백 엔드에 대한 REST 호출을 수행하고 NSUserDefaults를 사용하여 알림 허브에서 반환된 registrationId를 로컬로 저장하는, 지침 문서 [앱 백 엔드에서 등록](http://msdn.microsoft.com/ko-kr/library/dn743807.aspx)에서 설명된 논리를 구현합니다.

	이 클래스가 제대로 작동하려면 **authorizationHeader** 속성을 설정해야 합니다. 이 속성은 로그인 후에 **ViewController** 클래스에 의해 설정됩니다.

7. ViewController.h에서 장치 토큰 및 참조에 대한 다음 선언을 RegisterClient 인스턴스에 추가합니다.

		@property (strong, nonatomic) NSData* deviceToken;
		@property (strong, nonatomic) RegisterClient* registerClient;

8. ViewController.m에서 ViewController 클래스를 UITextFieldDelegate로 만듭니다. 그런 다음 private 메서드 선언을 추가합니다.
...
> [AZURE.NOTE] 다음 조각은 보안 인증 체계가 아닙니다. **createAndSetAuthenticationHeaderWithUsername:AndPassword:** 구현을 레지스터 클라이언트 클래스(예: OAuth, Active Directory)에서 사용할 인증 토큰을 생성하는 특정 인증 메커니즘으로 대체해야 합니다.
...
9. 그리고 나서 ViewController.m의 구현 섹션에서 다음 코드를 추가합니다.
	...
	장치 토큰 설정이 어떻게 로그인 단추를 활성화하는지에 주목하세요. 이는 로그인 작업 중에 보기 컨트롤러가 푸시 알림을 앱 백 엔드에 등록하기 때문입니다. 따라서 장치 토큰이 제대로 설정될 때까지 로그인 작업이 액세스되지 않도록 합니다. 장치 토큰이 설정된 후에 로그인 작업이 액세스되는 한 푸시 등록에서 로그인을 분리할 수 있습니다.

10. ViewController.m에서 백 엔드 끝점에 대한 상수를 추가하고 다음 조각을 사용하여 UIButton에 대한 작업 메서드를 구현합니다. 자리 표시자 백 엔드 끝점을 백 엔드에 사용한 대상 URL로 바꿉니다.
...
12. 이제 **AppDelegate.m**에서 **application:didRegisterForPushNotificationWithDeviceToken:** 메서드의 모든 콘텐츠를 제거하고 다음으로 바꿔 보기 컨트롤러에 APN에서 검색된 최신 장치 토큰이 포함되도록 합니다.
...
4. **푸시 보내기**를 클릭하고 홈 단추를 누릅니다. 잠시 후에 푸시 알림이 표시됩니다.
