...
## <a id="sqlserver-factory"></a>온-프레미스 SQL Server를 Azure Data Factory에 연결하는 동안 데이터 관리 게이트웨이 사용
...
| 적용 가능한 기술 | 일반 |
| 특성              | StorageType - 파일 |
| 참조              | [Azure File Storage](../storage/storage-files-introduction.md), [Windows에 Azure 파일 공유를 탑재하는 방법](../storage/storage-file-how-to-use-files-windows.md) |
| 단계 | Azure File Storage는 REST API를 사용하는 경우 HTTPS를 지원하지만, VM에 연결된 SMB 파일 공유로 사용되는 경우가 더 일반적입니다. SMB 2.1은 암호화를 지원하지 않으므로 Azure의 동일한 지역 내에서만 연결이 허용됩니다. 그러나 SMB 3.0은 암호화를 지원하고, Windows Server 2016, Windows Server 2012 R2, Windows 8.1 및 Windows 10에서 사용할 수 있으며 지역 간 액세스 및 데스크톱에 대한 액세스도 허용합니다. |
