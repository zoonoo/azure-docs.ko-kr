...
4. Azure에서는 VM 서비스 모델을 업데이트하고 IaaS VM은 암호가 해독되었다고 표시합니다. VM의 콘텐츠는 더 이상 휴지 상태로 암호화되지 않습니다.
5. 암호화 사용 안 함 작업은 Windows에 대한 BitLocker 암호화 키 및 Linux에 대한 암호인 고객 키 자격 증명 모음 또는 암호화 키 자료를 삭제하지 않습니다.
...
> [AZURE.NOTE] AAD 인증서 기반 인증은 현재 Linux VM에서 지원되지 않습니다.
...
> [AZURE.NOTE] 블록 장치 이름(예: /dev/sdb1)을 지정하는 대신, UUID=...를 사용하여 /etc/fstab에서 데이터 드라이브를 지정해야 합니다. 암호화하는 동안 드라이브의 순서는 VM에서 변경됩니다. VM이 블록 장치의 특정 순서에 의존하는 경우 암호화 후 탑재하지 못합니다.

3\. SSH 세션을 로그아웃합니다.

4\. OS를 암호화하려면 [암호화 사용 시](#enable-encryption-on-existing-or-running-iaas-linux-vm-in-azure) volumeType을 "All" 또는 "OS"로 지정합니다.

> [AZURE.NOTE] `systemd` 서비스로 실행되지 않는 모든 사용자 공간 프로세스는 `SIGKILL`로 중지됩니다. VM이 다시 부팅됩니다. 실행 중인 VM에서 OS 디스크 암호화를 사용할 경우 VM의 가동 중지 시간을 계획하세요.
