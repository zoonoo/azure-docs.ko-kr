...
네트워크 액세스 제어는 가상 네트워크 내의 특정 장치 또는 서브넷 간 연결을 제한하는 기능입니다. 네트워크 액세스 제어의 목표는 가상 머신 및 서비스에 대한 액세스를 승인 된 사용자 및 디바이스로 제한하는 것입니다. 액세스 제어는 가상 머신 또는 서비스 간 연결을 허용하거나 거부할지 결정에 따라 이루어집니다.
...
* 성능 향상. 여러 장치에서 연결을 부하 분산시키면 단일 장치에서 모든 프로세스를 처리하지 않아도 됩니다. 대신, 콘텐츠 제공에 대한 처리 및 메모리 요구는 여러 디바이스 전반에서 분산됩니다.
