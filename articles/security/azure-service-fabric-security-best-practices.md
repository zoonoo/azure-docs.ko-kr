...
1. 키 자격 증명 모음 전용 리소스 그룹을 만듭니다.
    ...
    자체 리소스 그룹에 키 자격 증명 모음을 배치하는 것이 좋습니다. 이렇게 하면 저장소, 계산 또는 클러스터를 포함하는 그룹 등의 다른 리소스 그룹을 제거하는 경우 키와 암호가 분실되는 것을 방지할 수 있습니다. 사용자 Key Vault를 포함하는 리소스 그룹은 해당 그룹을 사용하는 클러스터와 동일한 지역에 있어야 합니다.
