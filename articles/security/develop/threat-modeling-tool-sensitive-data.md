...
| **데이터베이스 백업** | <ul><li>[동적 데이터 마스킹을 구현하여 권한 없는 사용자에 대한 중요한 데이터 노출 제한](#dynamic-users)</li><li>[암호가 솔트된 해시 형식으로 저장되었는지 확인](#salted-hash)</li><li>[데이터베이스 열의 중요 한 데이터가 암호화 되어 있는지 확인](#db-encrypted)</li><li>[TDE(데이터베이스 수준 암호화)가 활성화되어 있는지 확인](#tde-enabled)</li><li>[데이터베이스 백업이 암호화되었는지 확인](#backup)</li></ul> | 
...
## <a id="api-browser"></a>Web API와 관련된 중요한 데이터가 브라우저의 스토리지에 저장되지 않았는지 확인
...
## <a id="pii-phones"></a>휴대폰 로컬 스토리지에 기록된 중요한 PII 데이터 암호화
