...
| **권한 높이기** | 권한 없는 사용자가 권한 있는 액세스를 얻게 되면 전체 시스템을 손상시키거나 파괴할 수 있게 됩니다. 권한 상승 위협은 공격자가 모든 시스템 방어망을 효과적으로 침투하고 신뢰할 수 있는 시스템 자체의 일부가 되는 상황을 야기하므로 실제로 굉장히 위험합니다. |

## <a name="next-steps"></a>다음 단계

**[위협 모델링 도구 해결 방법](threat-modeling-tool-mitigations.md)** 으로 진행하여 Azure를 사용하여 이러한 위협을 완화할 수 있는 다양한 방법을 알아봅니다.