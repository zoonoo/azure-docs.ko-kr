...
샘플 앱은 네트워크에서 들어오고 나가는 전송 중인 데이터에 대해 종단 간 TLS/SSL 암호화를 사용 합니다. 게이트웨이는 자체 서명 된 인증서로 구성 됩니다.
> [!IMPORTANT]
...
포털에 로그인 하기 위해 샘플 앱은 리소스에 대 한 액세스가 할당 된 Azure Active Directory (Azure AD) 관리자에 대 한 Multi-Factor Authentication를 사용 합니다.
...
- 개발 컴퓨터에 [Azure CLI](/cli/azure/install-azure-cli) 합니다.
...
3. Scripts 폴더에는 사용 중인 플랫폼 (Windows 또는 Linux)과 관련 된 파일이 있습니다. Azure CLI 이미 설치 되어 있으면 명령 프롬프트에서이 Azure CLI 명령을 실행 하 여 Azure 계정에 로그인 합니다.
...
Scripts 폴더에는 저장 된 함수를 `functions.sql` 실행할 때 저장 된 함수를 만드는 PL/pgsql 코드를 포함 하는 파일이 있습니다. 이 파일을 실행 하면 입력이 매개 변수화 SQL 삽입이 제한 됩니다.
...
PostgreSQL에 대 한 TLS 및 CA (인증 기관) 확인을 설정 하는 방법에 대 한 자세한 내용은 [Azure Database for PostgreSQL에서 tls 연결 구성](/azure/postgresql/concepts-ssl-connection-security)을 참조 하세요.
...
2. [OpenSSL를 컴퓨터에 다운로드 하 여 설치](/azure/postgresql/concepts-ssl-connection-security)합니다.
3. 인증서 파일 디코딩:
...
PostgreSQL에 대 한 TLS 보안을 구성 하는 방법에 대 한 자세한 내용은 [Tls 연결 보안 구성](/azure/postgresql/concepts-ssl-connection-security)을 참조 하세요.

#### <a name="deploy-azure-web-apps-on-linux"></a>Linux에서 Azure Web Apps 배포

Azure에서 Python, Ruby, c #, Java와 같이 널리 사용 되는 언어에 대해 미리 작성 된 컨테이너 및 이미지 집합을 제공 하므로 Azure App Service 위에 Linux 서비스를 쉽게 빌드할 수 있습니다. 또한 Azure는 거의 모든 프로그래밍 언어를 Azure App Service 플랫폼에서 실행할 수 있는 사용자 지정 컨테이너를 지원 합니다.
...
위의 Dockerfile은 Azure Container Registry에서 호스팅되는 컨테이너를 빌드하는 데 사용 됩니다 `mcr.microsoft.com/samples/basic-linux-app`.
...
해당 개요 페이지에서 게이트웨이의 IP 주소를 검색할 수 있습니다. **Ip 주소 CIDR** 탭에서 ip 주소를 형식으로 입력 `<GATEWAY_IP_ADDRESS>/32`합니다.
...
4. 등록 된 앱과 해당 정보를 표시 하는 화면이 표시 됩니다. Azure Key Vault 인스턴스에이 정보를 추가 해야 합니다.
   ...
   4. 이전에 만든 Azure AD 앱의 **인증서 & 암호** 탭으로 이동 하 고 다음 스크린샷에 표시 된 것 처럼 **새 클라이언트 암호**를 선택 합니다. 만료 날짜를 설정 하 고 생성 된 값을 복사한 다음 Key Vault에 저장 `CLIENTSECRET`합니다.
...
비밀을 Key Vault 하 고 응용 프로그램이 암호 및 데이터베이스에 액세스할 수 있게 되 면 게이트웨이의 응용 프로그램 URL (https:\//GATEWAY_HASH. p s e. n e t. n e t. n e t. n e t)을 통해 응용 프로그램 서비스에 연결할 수 있습니다 .이는 블레이드에서 가져올 수 있습니다.
...
2. 보안 범주 아래에서 조건부 액세스를 선택 합니다. 다음 화면이 표시됩니다.
...
4. **액세스 제어**에서 **허용** 탭을 선택한 후 **다단계 인증 필요** 및 기타 설정을 선택 합니다.
   ...
   ![MFA 필요](./media/secure-web-app/ad-mfa-conditional-add.png)
...
응용 프로그램 종속성을 찾아 수정 하려면 [OWASP의 종속성 검사](https://owasp.org/www-project-dependency-check/)를 사용 하면 됩니다.
