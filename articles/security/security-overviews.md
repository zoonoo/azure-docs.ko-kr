---
title: Azure 보안 기술 개요 | Microsoft Docs
description: 이 문서에서는 다양한 Azure 리소스에 적합한 Azure 보안 기술 개요 목록을 제공합니다.
services: azure-security
documentationcenter: na
author: TomShinder
manager: barbkess
editor: TomSh
ms.assetid: ''
ms.service: security
ms.devlang: na
ms.topic: conceptual
ms.tgt_pltfrm: na
ms.workload: na
ms.date: 2/16/2018
ms.author: tomsh
ms.openlocfilehash: 853dd89f4df59338a4f520106bff70d67cbab2a3
ms.sourcegitcommit: 41ca82b5f95d2e07b0c7f9025b912daf0ab21909
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 06/13/2019
ms.locfileid: "60597645"
---
# <a name="azure-security-technical-overviews"></a>Azure 보안 기술 개요


* [Azure 데이터베이스 보안 개요](azure-database-security-overview.md)
* [Azure 암호화 개요](security-azure-encryption-overview.md)
* [Azure ID 관리 보안 개요](security-identity-management-overview.md)
* [Azure 네트워크 보안 개요](security-network-overview.md)
* [Azure 운영 보안 개요](azure-operational-security-overview.md)
* [Azure 보안 관리 및 모니터링 개요](security-management-and-monitoring-overview.md)
* [Azure Service Fabric 보안 개요](azure-service-fabric-security-overview.md)
* [Azure Storage 보안 개요](security-storage-overview.md)
* [Azure 가상 머신 보안 개요](security-virtual-machines-overview.md)
* [사물 인터넷 보안 아키텍처](https://docs.microsoft.com/azure/iot-suite/iot-security-architecture#security-in-iot) 
* [Azure 보안 소개](azure-security.md)
* [Microsoft Azure 로그 통합 소개](security-azure-log-integration-overview.md)
* [Azure에서 사물 인터넷 보호](security-internet-of-things-overview.md)
* [PaaS 배포 보안](security-paas-deployments.md)
* [Azure Security Center란?](https://docs.microsoft.com/azure/security-center/security-center-intro?toc=%2fazure%2fsecurity%2ftoc.json) 
