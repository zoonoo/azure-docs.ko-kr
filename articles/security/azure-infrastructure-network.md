...
이러한 디바이스는 L2 트래픽에 대한 집계 지점 역할을 합니다. L2 패브릭에 대한 배포 계층이며, 많은 양의 트래픽을 처리할 수 있습니다. 이러한 디바이스는 트래픽을 집계하기 때문에 802.1q 기능과 포트 집계 및 10GE와 같은 고대역폭 기술이 필요합니다.
...
Azure는 자동화된 감지 및 완화를 제공하는 DCR(데이터 센터 라우터)에서 A10 네트워크 디바이스를 사용합니다. A10 솔루션은 Azure 네트워크 모니터링을 사용하여 흐름 패킷을 샘플링하고 공격이 있는지 확인합니다. 공격이 감지되면 A10 디바이스는 공격을 완화하기 위해 제거됩니다. 그런 후에야 깨끗해진 트래픽이 DCR에서 Azure 데이터 센터로 직접 허용됩니다. Microsoft는 A10 솔루션을 사용하여 Azure 네트워크 인프라를 보호합니다.
