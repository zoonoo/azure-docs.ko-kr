...
2. 다음으로 구독에 대한 읽기 권한자 액세스 권한을 2단계에서 만든 서비스 주체에 할당하는 명령을 실행합니다. SubscriptionID를 지정하지 않으면 이 명령은 사용자가 액세스 권한을 갖고 있는 모든 구독에 서비스 주체 읽기 관리자 역할을 할당하려고 시도합니다. </br></br>
