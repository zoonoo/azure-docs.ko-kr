...
이 문서의 권장 사항은 Azure AD 테넌트의 ID 보안 구성 평가를 자동화한 [ID 보안 점수](../../active-directory/fundamentals/identity-secure-score.md)와 일치합니다. 조직은 Azure AD 포털의 ID 보안 점수 페이지를 사용하여 현재 보안 구성의 차이를 찾아 현재 Microsoft [보안 모범 사례를](identity-management-best-practices.md) 따르도록 할 수 있습니다. [보안 점수] 페이지의 각 권장 사항을 구현하면 점수가 올라가고 진행 상황을 추적할 수 있으며, 비슷한 규모의 조직 또는 산업과 구현을 비교할 수 있습니다.

![ID 보안 점수](./media/steps-secure-identity/azure-ad-sec-steps0.png)

> [!NOTE]
> 여기에 설명된 대부분의 기능은 Azure AD Premium 구독이 필요하지만 일부는 무료입니다. 자세한 내용은 [Azure Active 디렉터리 가격 책정](https://azure.microsoft.com/pricing/details/active-directory/) 및 Azure [AD 배포 검사 목록을](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-deployment-checklist-p2) 검토하십시오.
...
공격자가 권한 있는 계정의 제어권을 확보하면 엄청난 피해가 발생할 수 있으므로 이러한 계정을 보호하는 것이 무엇보다도 중요합니다. [Azure AD 보안 기본값](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md) 또는 [조건부 액세스를](../../active-directory/conditional-access/plan-conditional-access.md)사용하여 조직의 모든 관리자에 대해 Azure [다단계](../../active-directory/authentication/multi-factor-authentication.md) 인증(MFA)을 활성화하고 필요합니다. MFA를 구현하지 않았다면, 지금 구현하세요! 이 문제는 중요합니다.
...
### <a name="make-sure-your-organization-uses-strong-authentication"></a>조직에서 강력한 인증을 사용하는지 확인
...
기본 ID 보안 수준을 쉽게 활성화하려면 [Azure AD 보안 기본값](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md)에서 한 번의 클릭으로 인에이블먼트를 사용할 수 있습니다. 보안 기본값은 테넌트의 모든 사용자에 대해 Azure MFA를 적용하고 레거시 프로토콜 테넌트 전체에서 로그인을 차단합니다.
...
2. 만료 규칙을 사용하지 않도록 설정하여 사용자가 **Spring2019와** 같은 암호를 쉽게 추측할 수 있도록 유도합니다!
...
* 온-프레미스 중단(예: 랜섬웨어 공격)의 경우 [암호 해시 동기화를 사용하여 클라우드 인증을](choose-ad-authn.md)사용하는 것으로 전환할 수 있습니다. 이 백업 인증 방법을 사용하면 Office 365를 포함하여 Azure Active Directory를 사용하여 인증하도록 구성된 앱에 계속 액세스할 수 있습니다. 이 경우 IT 직원은 온-프레미스 중단이 해결될 때까지 개인 전자 메일 계정에 의존하여 데이터를 공유할 필요가 없습니다.
...
자체 레거시 메서드를 사용하여 Azure AD로 인증하고 회사 데이터에 액세스하는 앱은 조직에 또 다른 위험을 제기합니다. 레거시 인증을 사용한 앱으로는 POP3, IMAP4 또는 SMTP 클라이언트가 있습니다. 레거시 인증 앱은 사용자를 대신하여 인증하고 Azure AD가 고급 보안 평가를 수행하지 못하도록 방지합니다. 대안적인 최신 인증은 다단계 인증 및 조건부 액세스를 지원하기 때문에 보안 위험을 줄일 수 있습니다. 다음과 같은 세 가지 작업이 권장됩니다.
...
3. Azure AD Premium이 있는 경우 [조건부 액세스 정책을](../../active-directory/conditional-access/overview.md) 사용하여 레거시 인증을 차단하고, 그렇지 않으면 [Azure AD 보안 기본값을](../../active-directory/fundamentals/concept-fundamentals-security-defaults.md)사용합니다.

### <a name="block-invalid-authentication-entry-points"></a>잘못된 인증 진입점 차단

위반이 발생할 때 가상 위반 사고방식을 사용하여 손상된 사용자 자격 증명의 영향을 줄여야 합니다. 사용자 환경의 각 앱의 경우 권한이 있는 그룹, 네트워크, 디바이스 및 기타 요소가 무엇인지 유효한 사용 사례를 고려한 다음, 나머지를 차단합니다. [Azure AD 조건부 액세스를](../../active-directory/conditional-access/overview.md)사용하면 권한이 있는 사용자가 정의한 특정 조건에 따라 앱 및 리소스에 액세스하는 방법을 제어할 수 있습니다.

### <a name="restrict-user-consent-operations"></a>사용자 동의 작업 제한

다양한 [Azure AD 응용 프로그램 동의 환경,](https://docs.microsoft.com/azure/active-directory/develop/application-consent-experience) [사용 권한 및 동의 유형](https://docs.microsoft.com/azure/active-directory/develop/v2-permissions-and-consent)및 조직의 보안 태세에 미치는 영향을 이해하는 것이 중요합니다. 기본적으로 Azure AD의 모든 사용자는 Microsoft ID 플랫폼을 활용하여 조직의 데이터에 액세스하는 응용 프로그램을 부여할 수 있습니다. 사용자가 스스로 동의할 수 있도록 허용하면 사용자가 Microsoft 365, Azure 및 기타 서비스와 통합되는 유용한 응용 프로그램을 쉽게 획득할 수 있지만 신중하게 사용하고 모니터링하지 않으면 위험을 나타낼 수 있습니다.

[향후 사용자 동의 작업을 사용하지 않도록 설정하여](https://docs.microsoft.com/azure/active-directory/manage-apps/methods-for-removing-user-access#i-want-to-disable-all-future-user-consent-operations-to-any-application) 노출 영역을 줄이고 이러한 위험을 완화하는 것이 좋습니다. 최종 사용자 동의를 사용하지 않도록 설정하면 이전 동의 권한 부여는 계속 적용되지만 향후 모든 동의 작업은 관리자가 수행해야 합니다. 통합된 [관리자 동의 요청 워크플로를](https://docs.microsoft.com/azure/active-directory/manage-apps/configure-admin-consent-workflow) 통해 또는 자체 지원 프로세스를 통해 사용자가 관리자 동의를 요청할 수 있습니다. 최종 사용자 동의를 비활성화하기 전에 [권장 사항을](https://docs.microsoft.com/azure/active-directory/manage-apps/manage-consent-requests) 사용하여 조직에서 이러한 변경 사항을 계획합니다. 모든 사용자가 액세스할 수 있도록 허용하려는 응용 프로그램의 경우 [모든 사용자를 대신하여 동의를 부여하는](https://docs.microsoft.com/azure/active-directory/develop/v2-admin-consent)것을 고려하여 아직 개별적으로 동의하지 않은 사용자가 앱에 액세스할 수 있도록 합니다. 모든 시나리오에서 모든 사용자가 이러한 응용 프로그램을 사용할 수 있도록 하지 않으려면 [응용 프로그램 할당](https://docs.microsoft.com/azure/active-directory/manage-apps/methods-for-assigning-users-and-groups) 및 [조건부 액세스를](https://docs.microsoft.com/azure/active-directory/conditional-access/overview) 사용하여 앱에 대한 사용자 액세스를 제한합니다.

사용자가 새 응용 프로그램에 대한 관리자 승인을 요청하여 사용자 마찰을 줄이고 지원 볼륨을 최소화하며 사용자가 Azure AD가 아닌 자격 증명을 사용하여 응용 프로그램에 등록하지 못하도록 할 수 있습니다. 동의 작업을 규제한 후에는 관리자는 정기적으로 앱을 감사하고 동의한 권한을 받아야 합니다.
...
권한 있는 계정 프로세스를 배포하는 과정에서 [모범 사례를 따라 두 개 이상의 긴급 계정을 만들어](../../active-directory/users-groups-roles/directory-admin-roles-secure.md) 자신을 잠그는 경우에도 Azure AD에 계속 액세스할 수 있는지 확인합니다.
...
사용자 위험은 사용자의 ID가 손상되었을 가능성을 나타내며 사용자의 ID와 연결된 [사용자 위험 검색을](../../active-directory/identity-protection/overview.md) 기반으로 계산됩니다. 사용자 위험 정책은 특정 사용자 또는 그룹에 대한 위험 수준을 평가하는 조건부 액세스 정책입니다. 낮음, 보통, 높음 위험 수준에 따라 액세스를 차단하거나 다단계 인증을 사용한 보안 암호 변경을 요구하도록 정책을 구성할 수 있습니다. Microsoft의 권장 사항은 높은 위험 수준에 있는 사용자의 경우 보안 암호 변경이 필요합니다.
...
로그인 위험은 계정 소유자가 아닌 누군가가 ID를 사용하여 로그인을 시도할 가능성을 말합니다. [로그인 위험 정책은](../../active-directory/identity-protection/overview.md) 특정 사용자 또는 그룹에 대한 위험 수준을 평가하는 조건부 액세스 정책입니다. 위험 수준(높음/보통/낮음)에 따라 정책은 액세스를 차단하거나 다단계 인증을 강제 적용하도록 구성할 수 있습니다. 보통 이상의 위험 로그인에서는 다단계 인증을 받도록 해야 합니다.
...
보안 관련 이벤트의 감사와 로깅 및 관련 경고는 효과적인 보호 전략의 중요한 구성 요소입니다. 보안 로그 및 보고서는 의심스러운 활동에 대한 전자 기록을 제공하며, 네트워크의 외부 침투와 내부 공격의 시도 또는 성공을 나타낼 수 있는 패턴을 검색할 수 있도록 도움을 줍니다. 감사를 사용하여 사용자 활동을 모니터링하고, 규정 준수를 문서화하고, 포렌식 분석을 수행하는 등의 작업을 수행할 수 있습니다. 경고는 보안 이벤트의 알림을 제공합니다.
...
[Azure AD Identity Protection](../../active-directory/identity-protection/overview.md)은 조직의 ID에 영향을 주는 잠재적 취약성을 검색하는 데 사용할 수 있는 알림, 모니터링 및 보고 도구입니다. 유출된 자격 증명, 불가능한 이동 및 감염된 장치의 로그인, 익명 IP 주소, 의심스러운 활동과 관련된 IP 주소 및 알 수 없는 위치와 같은 위험 검색을 감지합니다. 위험한 사용자의 이메일을 수신 및/또는 주 단위 요약 이메일을 수신하는 알림 경고를 사용하도록 설정합니다.
...
사용자는 자신의 프로필 정보 및 사용자 데이터에 액세스 할 수있는 손상된 웹 사이트 또는 응용 프로그램(예: 이메일)으로 이동하도록 속일 수 있습니다. 악의적인 행위자는 획득한 승인된 권한을 사용하여 사서함 콘텐츠를 암호화하고, 사서함 데이터를 되찾으려면 몸값을 지불하라고 요구할 수 있습니다. 관리자는 사용자가 제공한 권한을 검토하고 감사하거나 기본적으로 동의를 제공하는 사용자의 기능을 [비활성화해야 합니다.](https://docs.microsoft.com/office365/securitycompliance/detect-and-remediate-illicit-consent-grants)

사용자가 부여한 권한을 감사하는 것 외에도 프리미엄 환경에서 [위험하거나 원치 않는 OAuth 응용 프로그램을 찾을](https://docs.microsoft.com/cloud-app-security/investigate-risky-oauth) 수 있습니다.
...
Azure AD의 [셀프 서비스 암호 재설정(SSPR)은](../../active-directory/authentication/quickstart-sspr.md) IT 관리자가 사용자가 헬프 데스크 나 관리자의 개입 없이 암호 또는 계정을 재설정하거나 잠금을 해제할 수 있는 간단한 방법을 제공합니다. 이 시스템에는 사용자가 암호를 재설정한 시기를 추적하는 자세한 보고와 오용 또는 남용을 알리는 알림이 포함됩니다.

### <a name="implement-self-service-group-and-application-access"></a>셀프 서비스 그룹 및 애플리케이션 액세스 구현

Azure AD는 관리자가 아닌 관리자가 보안 그룹, Office 365 그룹, 응용 프로그램 역할 및 패키지 카탈로그에 액세스하여 리소스에 대한 액세스를 관리할 수 있는 기능을 제공합니다.  [셀프 서비스 그룹 관리를](../../active-directory/users-groups-roles/groups-self-service-management.md) 사용하면 그룹 소유자가 관리 역할을 할당할 필요 없이 자신의 그룹을 관리할 수 있습니다. 또한 사용자는 관리자가 요청을 처리하는 데 의존하지 않고 Office 365 그룹을 만들고 관리할 수 있으며 사용하지 않는 그룹은 자동으로 만료됩니다.  [Azure AD 권한 관리를](../../active-directory/governance/entitlement-management-overview.md) 사용하면 포괄적인 액세스 요청 워크플로 및 자동 만료를 통해 위임 및 가시성을 추가로 사용할 수 있습니다.  관리자가 아닌 관리자에게 자신의 그룹, Teams, 응용 프로그램 및 SharePoint Online 사이트에 대해 자신의 액세스 패키지를 구성할 수 있는 기능을 직원의 구성을 포함하여 액세스를 승인해야 하는 사용자 지정 정책을 사용하여 위임할 수 있습니다. 관리자 및 비즈니스 파트너 스폰서를 승인자로 승인합니다.

### <a name="implement-azure-ad-access-reviews"></a>Azure AD 액세스 검토 구현

[Azure AD 액세스 검토를](../../active-directory/governance/access-reviews-overview.md)사용하면 액세스 패키지 및 그룹 구성원, 엔터프라이즈 응용 프로그램에 대한 액세스 및 권한 있는 역할 할당을 관리하여 보안 표준을 유지할 수 있습니다.  사용자 자신, 리소스 소유자 및 기타 검토자에 대한 정기적인 감독은 사용자가 더 이상 필요하지 않을 때 장시간 액세스 권한을 유지하지 못하도록 합니다.
...
이러한 모든 단계가 완료된 것으로 확신하는 경우 Microsoft의 [ID 보안 점수를](../../active-directory/fundamentals/identity-secure-score.md)사용하여 최신 모범 [사례](identity-management-best-practices.md) 및 보안 위협에 대한 최신 정보를 확인할 수 있습니다.
