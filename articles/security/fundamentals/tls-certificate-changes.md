...
- [Azure Active Directory](/azure/active-directory) (Azure AD) 서비스는 2020 년 7 월 7 일에이 전환을 시작 했습니다.
...
- [Azure Storage](/azure/storage) 는 Baltimore CYBERTRUST Root CA에 그대로 남아 있지만 중간 ca는 변경 됩니다. [자세한 내용을 보려면 여기를 클릭하세요](https://techcommunity.microsoft.com/t5/azure-storage/azure-storage-tls-changes-are-coming-and-why-you-care/ba-p/1705518).
...
현재 CA 인증서는 2021 년 2 월 15 일까 *지 해지 되지* 않습니다. 이 날짜가 지나면 코드에서 이전 지문을 제거할 수 있습니다.

이 날짜가 변경될 경우 새로운 해지 날짜에 대한 알림이 제공됩니다.

## <a name="will-this-change-affect-me"></a>이 변경 내용이 영향을 미칩니까? 
...
- [여기](https://www.microsoft.com/pki/mscorp/cps/default.htm)에 있는 Microsoft IT TLS CA의 지문, 일반 이름 및 기타 인증서 속성을 소스코드에서 검색합니다. 일치하는 항목이 있으면 애플리케이션이 영향을 받습니다. 이 문제를 해결하려면 새 CA를 포함하도록 소스 코드를 업데이트합니다. 모범 사례에 따라 짧은 알림에 Ca를 추가 하거나 편집할 수 있는지 확인 합니다. 업계 규정을 준수 하려면 CA 인증서가 7 일 이내에 교체 되어야 하므로, 고정에 의존 하는 고객은 즉각적으로 대응 해야 합니다.
...
- Azure 서비스와 통신하는 여러 운영 체제 및 언어 런타임은 다음과 같은 새 루트를 사용하여 인증서 체인을 올바르게 빌드하는 추가 단계가 필요할 수 있습니다.
    - **Linux**: 대부분의 배포에서는/Etc/ssl/certs.에 ca를 추가 해야 합니다. 특정 지침은 배포의 설명서를 참조 하세요.
    - **Java**: 위에 나열된 CA가 Java 키 저장소에 들어 있어야 합니다.
    - **연결 되지 않은 환경에서 실행 되는 Windows**: 연결 되지 않은 환경에서 실행 되는 시스템에는 신뢰할 수 있는 루트 인증 기관 저장소에 새 루트가 추가 되 고 Intermediates이 중간 인증 기관 저장소에 추가 되어야 합니다.
