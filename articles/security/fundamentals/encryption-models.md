...
키 암호화 키가 손실 되 면 데이터가 손실 됩니다. 이러한 이유로 키를 삭제 해서는 안 됩니다. 키를 만들거나 회전할 때마다 백업 해야 합니다. 키 암호화 키를 저장 하는 자격 증명 모음에 대해 [일시 삭제](../../key-vault/general/soft-delete-overview.md) 를 사용 하도록 설정 해야 합니다. 키를 삭제 하는 대신 enabled를 false로 설정 하거나 만료 날짜를 설정 합니다.
