...
| 네트워킹                                                   | 이미지에서 사용자 지정 네트워크 구성을 제거 합니다. Resolv.conf: `rm /etc/resolv.conf`을 삭제 합니다.                                                                                                                                                                                |
...
| 배포                                                   | Azure 지원이 필요할 때 파트너에 게 직렬 콘솔 출력을 제공 하 고 클라우드 저장소에서 OS 디스크를 탑재 하는 데 적절 한 제한 시간을 제공할 수 있는지 확인 합니다. 다음 매개 변수를 이미지 커널 부팅 줄에 추가 합니다. `console=ttyS0 earlyprintk=ttyS0 rootdelay=300`. |
