...
|[가상 머신 및 클라우드 서비스](../../azure-monitor/learn/quick-collect-azurevm.md)|Windows 이벤트 로그 서비스 및 Linux Syslog|  가상 머신에서 시스템 데이터와 로깅 데이터를 캡처하고 사용자가 선택한 스토리지 계정으로 해당 데이터를 전송합니다.|   Azure Monitor의 Windows([WAD](../../azure-monitor/platform/diagnostics-extension-overview.md)[Microsoft Azure Diagnostics 스토리지] 사용) 및 Linux|
