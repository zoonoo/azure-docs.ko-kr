---
title: Azure 보안 로깅 및 감사 | 마이크로 소프트 문서
description: Azure에서 사용할 수 있는 로그와 얻을 수 있는 보안 통찰력에 대해 알아봅니다.
services: security
...
클라우드 애플리케이션은 이동하는 부분이 많아 복잡합니다. 로깅 데이터는 응용 프로그램에 대한 통찰력을 제공하고 다음과 같은 데 도움이 될 수 있습니다.

- 과거 문제 해결 또는 잠재적 문제 방지
- 애플리케이션 성능 또는 유지 관리 가능성 향상
- 수동 개입이 필요한 작업 자동화
...
* **데이터 평면 로그는** Azure 리소스 사용의 일부로 발생한 이벤트에 대한 정보를 제공합니다. 이 로그 유형의 예로 VM(가상 머신)의 Windows 이벤트 시스템, 보안 및 애플리케이션 로그와 Azure Monitor를 통해 구성된 [진단 로그](../../azure-monitor/platform/platform-logs-overview.md)가 있습니다.
...
|[Azure Active Directory 보고](../../active-directory/reports-monitoring/overview-reports.md)|로그 및 보고서 | 사용자 및 그룹 관리에 대한 사용자 로그인 활동 및 시스템 활동 정보를 보고합니다.|[Graph API](../../active-directory/develop/active-directory-graph-api-quickstart.md)|
...
|[데이터 처리/보안 경고](../../security-center/security-center-intro.md)|  Azure 보안 센터 경고, Azure 모니터 로그 경고|    보안 정보 및 경고를 제공합니다.|  REST API, JSON|

## <a name="log-integration-with-on-premises-siem-systems"></a>온-프레미스 SIEM 시스템과 로그 통합
[보안 센터 경고를 통합하면](../../security-center/security-center-export-data-to-siem.md) Azure 진단 로그에서 수집한 보안 센터 경고, Azure 진단 로그에 의해 수집된 가상 시스템 보안 이벤트 및 Azure 감사 로그를 Azure Monitor 로그 또는 SIEM 솔루션과 동기화하는 방법에 대해 설명합니다.
