---
title: Microsoft Azure 고객 Lockbox에 대 한 Azure 보안 기준
description: Microsoft Azure 고객 Lockbox에 대 한 Azure 보안 기준
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/05/2020
...
**지침**: 해당 사항 없음 가상 네트워크, 서브넷 또는 네트워크 보안 그룹을 고객 Lockbox과 연결할 수 없습니다.
...
**지침**: 해당 사항 없음 가상 네트워크, 서브넷 또는 네트워크 보안 그룹을 Azure 고객 Lockbox에 연결할 수 없습니다.
...
**지침**: 해당 사항 없음 가상 네트워크, 서브넷 또는 네트워크 보안 그룹을 고객 Lockbox과 연결할 수 없습니다. 만들거나 모니터링할 네트워크 구성이 없습니다.
...
**지침**: 해당 사항 없음 Microsoft는 고객 Lockbox 같은 리소스에 사용 되는 시간 원본을 유지 관리 합니다.
...
**지침**: 해당 사항 없음 고객 Lockbox는 맬웨어 방지 관련 로그를 처리 하거나 생성 하지 않습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox는 DNS 관련 로그를 처리 하거나 생성 하지 않습니다.
...
**지침**: Azure Active Directory에는 기본 암호 개념이 없습니다. 암호를 요구 하는 다른 Azure 리소스는 복잡성 요구 사항과 최소 암호 길이를 사용 하 여 암호를 강제로 만들도록 합니다 .이는 서비스에 따라 다릅니다. 기본 암호를 사용할 수 있는 타사 응용 프로그램 및 marketplace 서비스를 담당 합니다.
...
**지침**: 해당 사항 없음 고객 Lockbox에 대 한 액세스는 Azure Portal를 통해 담당 하 고, 테 넌 트 역할이 소유자 인 계정에 대해 예약 됩니다. Single sign-on은 지원 되지 않습니다.
...
**지침**: 환경에서 의심 스러운 활동이 나 안전 하지 않은 활동이 발생 하는 경우 로그 및 경고를 생성 하는 데 PIM (Azure Active Directory Privileged Identity Management)을 사용 합니다.
...
Azure 센티널 또는 타사 SIEM과 통합 하는 데 사용할 수 있는 Azure Active Directory 로그인 활동, 감사 및 위험 이벤트 로그 원본에 액세스할 수 있습니다.
...
**지침**: 고객 Lockbox에는이 권장 사항이 적용 되지 않습니다.
...
**지침**:이 권장 사항은 적용 되지 않습니다. 태그는 고객 Lockbox 지원 되지 않습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox 자체에는 고객 데이터가 포함 되지 않습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox 자체는 고객 데이터를 포함 하지 않습니다.
...
**지침**: 해당 사항 없음 Microsoft는 고객 Lockbox을 지 원하는 기본 시스템에서 취약성 관리를 수행 합니다.
...
**지침**: 태그는 고객 Lockbox에 대해 지원 되지 않습니다.
...
**지침**: 해당 없음 고객 Lockbox에 구성 가능한 보안 설정이 없습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox에 구성 가능한 보안 설정이 없습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox 요청에 대 한 액세스는 리소스를 보관 하는 Azure 구독의 소유자로 제한 됩니다. 테 넌 트 소유자로 로그인 하지 않고 고객 Lockbox 액세스 하는 데 필요한 암호, 비밀 또는 키가 없습니다.
...
**지침**: 해당 없음. 이 지침은 컴퓨팅 리소스를 위한 것입니다. 고객 Lockbox 솔루션을 지 원하는 기본 호스트에서 Microsoft 맬웨어 방지 프로그램을 사용할 수 있습니다.
...
**지침**: 고객 Lockbox와 같은 Azure 서비스를 지 원하는 기본 호스트에서 Microsoft 맬웨어 방지를 사용 하도록 설정 합니다.
...
**지침**: 해당 사항 없음 고객 Lockbox 자체는 고객 데이터를 저장 하지 않습니다.
...
**지침**: 해당 사항 없음 고객 Lockbox 자체는 고객 데이터를 저장 하지 않으며 액세스에 키 또는 암호를 사용 하지 않습니다.
