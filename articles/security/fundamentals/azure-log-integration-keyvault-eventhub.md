...
- [Azure 로그 통합](azure-log-integration-overview.md)
...
* Azure Log Integration이 설치되어 있어야 합니다. 설치하려면:
   ...
   b. Azure 로그 통합 설치 관리자를 시스템에 복사합니다. c. 설치 관리자를 시작하고 Microsoft 소프트웨어 사용 조건에 동의합니다.

* 원격 분석 정보를 입력하는 경우 확인란을 선택한 상태로 둡니다. 사용 정보를 Microsoft로 보내지 않으려면 확인란의 선택을 취소합니다.

   Azure 로그 통합 및 설치 방법에 대한 자세한 내용은 [Azure 로그 통합, Azure Diagnostics 로깅 및 Windows 이벤트 전달](azure-log-integration-get-started.md)을 참조하세요.
...
* 최신 PowerShell 버전입니다.
   ...
   PowerShell 5.0 이상이 있으면 [Azure PowerShell 설치](/powershell/azure/install-az-ps)의 지침에 따라 최신 버전 설치를 계속할 수 있습니다.
...
1. LoadAzLogModule.ps1 스크립트를 실행하여 AzLog cmdlet을 가져옵니다. `.\LoadAzLogModule.ps1` 명령을 입력합니다. (해당 명령에서 ".\" 확인) 다음과 같이 표시되어야 합니다.</br>

   ![로드된 모듈 목록](./media/azure-log-integration-keyvault-eventhub/loaded-modules.png)

1. `Connect-AzAccount` 명령을 입력합니다. 로그인 창에서 이 자습서에 사용할 구독에 대한 자격 증명 정보를 입력합니다.

   >[!NOTE]
   >이 컴퓨터에서 Azure에 처음 로그인하는 경우 Microsoft에서 PowerShell 사용량 현황 데이터를 수집하도록 허용하라는 메시지가 표시됩니다. 이 기능은 Azure PowerShell을 개선하는 데 사용되므로, 이 데이터 수집을 사용하도록 설정하는 것이 좋습니다.

1. 인증에 성공 하면 로그인 됩니다. 구독 ID 및 구독 이름을 메모해 두세요. 이후 단계를 완료하는 데 필요합니다.
...
1. 가능한 모든 Azure 위치를 가져오고 이후 단계에서 사용할 수 있는 변수에 이름을 추가합니다.
    ...
    이 시점에서 `$locations` 을 입력 하면 AzLocation에서 반환 하는 추가 정보가 없는 위치 이름이 표시 됩니다.
...
> [!NOTE]
> 로그 프로필을 만들려고 할 때 오류 메시지가 표시될 수도 있습니다. 그런 다음 AzLogProfile 및 AzLogProfile에 대 한 설명서를 검토할 수 있습니다. AzLogProfile를 실행 하는 경우 로그 프로필에 대 한 정보가 표시 됩니다. ```Remove-AzLogProfile -name 'Log Profile Name'``` 명령을 입력하면 기존 로그 프로필을 삭제할 수 있습니다.
>
>![Resource Manager 프로필 오류](./media/azure-log-integration-keyvault-eventhub/rm-profile-error.png)
...
1. 암호를 설정하고 읽어서 추가 로그 항목을 생성합니다.
   ...
   ![반환된 암호](./media/azure-log-integration-keyvault-eventhub/keyvaultsecret.png)
...
- [Azure 로그 통합 FAQ](azure-log-integration-faq.md)
- [Azure 로그 통합 시작](azure-log-integration-get-started.md)
- [Azure 리소스의 로그를 SIEM 시스템에 통합](azure-log-integration-overview.md)
