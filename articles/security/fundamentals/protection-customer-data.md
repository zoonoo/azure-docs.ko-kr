...
**암호화**: 고객은 스토리지 데이터와 전송 중인 데이터의 암호화를 데이터의 기밀성과 무결성을 보장하기 위한 모범 사례로 배포할 수 있습니다. 고객이 TLS를 사용하여 인터넷및 Azure 호스팅 VM 간의 통신을 보호하도록 Azure 클라우드 서비스를 구성하는 것은 간단합니다.

**데이터 중복성**: Microsoft는 데이터 센터에 사이버 공격이나 물리적 손상이 있는 경우 데이터를 보호합니다. 고객이 선택할 수 있는 항목은 다음과 감습니다.

- 규정 준수 또는 대기 시간 고려 사항을 고려하여 국가/지역 내 저장소입니다.
- 보안 또는 재해 복구를 위해 국가 외/지역 외 저장소입니다.
