---
title: 안전한 PaaS 배포를 위한 모범 사례 - Microsoft Azure
description: Azure에서 보안 클라우드 응용 프로그램을 설계, 구축 및 관리하는 모범 사례를 알아보고 PaaS의 보안 이점과 다른 클라우드 서비스 모델의 보안 이점을 이해합니다.
services: security
...
[Azure에서 보안 응용 프로그램을 개발하는](abstract-develop-secure-apps.md) 것은 클라우드용 응용 프로그램을 개발할 때 소프트웨어 개발 수명 주기의 각 단계에서 고려해야 할 보안 질문 및 제어에 대한 일반적인 지침입니다.

## <a name="cloud-security-advantages"></a>클라우드 보안 이점
귀하와 Microsoft 간의 [책임 분담을](shared-responsibility.md) 이해하는 것이 중요합니다. 온-프레미스에서는 전체 스택을 가지고 있지만 클라우드로 이동할 때 일부 책임이 Microsoft로 이전됩니다.

[클라우드에 있는 보안 이점이](shared-responsibility.md#cloud-security-advantages)있습니다. 온-프레미스 환경에서 조직은 충족되지 않은 책임과 제한된 리소스를 보안에 투자할 가능성이 높으며, 이로 인해 공격자가 모든 계층의 취약점을 악용할 수 있는 환경이 만들어집니다.
...
스택 맨 위에는 데이터 관리와 권한 관리가 있으며 키 관리를 통해 완화될 수 있는 위험이 하나 있습니다. (키 관리는 모범 사례에서 다룹니다.) 키 관리는 추가 책임이지만 더 이상 관리할 필요가 없는 PaaS 배포 영역에 는 리소스를 키 관리로 전환할 수 있습니다.
...
가능한 경우에는 Azure Virtual Network에서 가상 사설망을 사용하는 등의 다른 방법을 사용해야 합니다. 다른 방법을 사용할 수 없는 경우 복잡한 암호 구문을 사용하고 2단계 인증(예: [Azure Multi-Factor Authentication](/azure/active-directory/authentication/multi-factor-authentication))을 사용해야 합니다.
...
따라서 2단계 인증을 사용해야 합니다. 2단계 인증은 인증의 사용자 이름과 암호 유형에 내재된 보안 약점을 방지할 수 있기 때문에 인증 및 권한 부여의 현재 표준입니다. Azure 관리(포털/원격 PowerShell) 인터페이스와 고객 대상 서비스에 대한 액세스는 [Azure Multi-Factor Authentication](/azure/active-directory/authentication/multi-factor-authentication)을 사용하도록 설계되고 구성되어야 합니다.
...
Application Insight에는 수집하는 데이터와 상호 작용할 수 있는 광범위한 도구가 있습니다. Application Insights는 공용 저장소에 데이터를 저장합니다. Kusto 쿼리 언어를 사용하여 경고, 대시보드 및 심층 분석과 같은 공유 기능을 활용할 수 있습니다.

## <a name="perform-security-penetration-testing"></a>보안 침투 테스트 수행
보안 방어의 유효성을 검사하는 것은 다른 기능을 테스트하는 것만큼 중요합니다. [침투 테스트를](pen-testing.md) 빌드 및 배포 프로세스의 표준 부분으로 만듭니다. 배포된 응용 프로그램에서 정기적인 보안 테스트 및 취약성 검색을 예약하고 열린 포트, 끝점 및 공격을 모니터링합니다.

퍼즈 테스트는 이 데이터를 구문 분석하고 사용하는 프로그램 인터페이스(진입점)에 잘못된 입력 데이터를 제공하여 프로그램 오류(코드 오류)를 찾는 방법입니다. [Microsoft 보안 위험 감지는](https://www.microsoft.com/en-us/security-risk-detection/) Azure에 배포하기 전에 소프트웨어의 버그 및 기타 보안 취약점을 찾는 데 사용할 수 있는 클라우드 기반 도구입니다. 이 도구는 소프트웨어를 배포하기 전에 취약점을 포착하도록 설계되어 버그를 패치하거나 충돌을 처리하거나 소프트웨어가 출시된 후 공격에 대응할 필요가 없습니다.
...
클라우드용 응용 프로그램을 개발할 때 소프트웨어 개발 수명 주기의 각 단계에서 고려해야 할 보안 질문 및 제어에 대한 [보안 응용](abstract-develop-secure-apps.md) 프로그램 개발을 참조하세요.
