...
이 문서에서는 PaaS 웹 및 모바일 애플리케이션 보안을 위한 [Azure App Services](/azure/app-service/overview) 보안 모범 사례에 대해 설명합니다. 이러한 모범 사례는 Azure에 대한 Microsoft와 고객의 경험에서 비롯된 것입니다.
...
데이터 액세스를 위한 보안 정책을 시행하려는 조직에서는 액세스를 반드시 제한해야 합니다. RBAC(역할 기반 액세스 제어)를 사용하여 특정 범위에서 사용자, 그룹 및 애플리케이션에 권한을 할당할 수 있습니다. 예를 들어 꼭 알아야 할 필요가 있는 경우에 한하여 권한을 할당하거나, 최소 권한 보안 원칙에 따라 권한을 할당할 수 있습니다. 사용자에게 애플리케이션 액세스 권한을 부여하는 방법에 대한 자세한 내용은 [역할 기반 액세스 제어란](/azure/role-based-access-control/overview)을 참조하세요.

## <a name="protect-your-keys"></a>키 보호
보안이 아무리 훌륭하더라도 구독 키를 분실하면 안됩니다. Azure Key Vault는 클라우드 애플리케이션 및 서비스에서 사용되는 암호화 키 및 비밀을 보호하는데 도움이 됩니다. Key Vault를 사용하면 HSM(하드웨어 보안 모듈)을 통해 보호되는 키를 통해 키와 비밀(예: 인증 키, 스토리지 계정 키, 데이터 암호화 키, .PFX 파일 및 암호)를 암호화할 수 있습니다. 추가된 보증을 위해, HSM에서 키를 생성하거나 가져올 수 있습니다. 또한 Key Vault를 사용하여 자동 갱신으로 TLS 인증서를 관리할 수도 있습니다. 자세히 알아보려면 [Azure Key Vault란](../../key-vault/key-vault-overview.md)을 참조하세요.
