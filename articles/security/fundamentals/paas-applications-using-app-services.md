...
데이터 액세스를 위한 보안 정책을 시행하려는 조직에서는 액세스를 반드시 제한해야 합니다. RBAC(역할 기반 액세스 제어)를 사용하여 특정 범위에서 사용자, 그룹 및 응용 프로그램에 권한을 할당할 수 있습니다. 예를 들어 꼭 알아야 할 필요가 있는 경우에 한하여 권한을 할당하거나, 최소 권한 보안 원칙에 따라 권한을 할당할 수 있습니다. 사용자에게 애플리케이션 액세스 권한을 부여하는 방법에 대한 자세한 내용은 [역할 기반 액세스 제어란](/azure/role-based-access-control/overview)을 참조하세요.
