...
데이터 센터 네트워크는 [Clos 네트워크](https://en.wikipedia.org/wiki/Clos_network)의 수정 된 버전으로, 클라우드 규모 트래픽에 대 한 높은 bi-단면 대역폭을 제공 합니다. 네트워크는 개별 하드웨어 오류로 인 한 영향을 줄이기 위해 많은 수의 상용 장치를 사용 하 여 생성 됩니다. 이러한 장치는 환경 이벤트의 영향을 줄이기 위해 별도의 전원 및 냉각 도메인을 사용 하는 서로 다른 물리적 위치에 전략적으로 배치 됩니다.  제어 평면에서 모든 네트워크 장치는 OSI 모델 계층 3 라우팅 모드로 실행 되며,이는 트래픽 루프의 기록 문제를 제거 합니다. 여러 계층 간의 모든 경로는 ECMP (Equal-Cost 다중 경로) 라우팅을 사용 하 여 높은 중복성과 대역폭을 제공 하기 위해 활성화 됩니다.
