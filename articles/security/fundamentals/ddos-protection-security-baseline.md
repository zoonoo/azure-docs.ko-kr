...
description: Azure DDoS Protection 표준에 대 한 Azure 보안 기준
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/22/2020
...
ms.locfileid: "85255885"
---
# <a name="azure-security-baseline-for-azure-ddos-protection-standard"></a>Azure DDoS Protection 표준에 대 한 Azure 보안 기준

Azure DDoS Protection Standard의 Azure 보안 기준에는 배포의 보안 상태를 개선 하는 데 도움이 되는 권장 사항이 포함 되어 있습니다.
...
**지침**: 해당 사항 없음 Azure DDoS는 다른 서비스를 호스팅하는 가상 네트워크에서 사용할 수 있도록 설계 되었습니다. 자체는 핵심 Azure 제품 이며 네트워킹을 포함 하 여 기본 인프라는 Microsoft에서 완벽 하 게 관리 됩니다.
...
**지침**: 해당 사항 없음 Microsoft는 Azure 리소스에 사용 되는 시간 원본 (예: Azure DDoS Protection)을 유지 관리 합니다.
...
* [DDoS protection 메트릭에 대 한 경고를 구성 하는 방법](https://docs.microsoft.com/azure/virtual-network/manage-ddos-protection#configure-alerts-for-ddos-protection-metrics)
...
**지침**: 해당 사항 없음 Azure DDoS Protection는 맬웨어 방지 관련 로그를 처리 하거나 생성 하지 않습니다.
...
**지침**: 해당 사항 없음 Azure DDoS Protection는 DNS 관련 로그를 처리 하거나 생성 하지 않습니다.
...
* [Azure DDoS protection에서 사용 권한 이해](https://docs.microsoft.com/azure/virtual-network/manage-ddos-protection#permissions)
...
* [Azure DDos Protection API 정보](https://docs.microsoft.com/rest/api/virtual-network/)
...
**지침**: MFA (azure Multi-Factor Authentication)를 사용 하는 PAW (권한 있는 액세스 워크스테이션)를 사용 하 여 azure 고객 Lockbox 요청에 로그인 하 고 구성 합니다.
...
* [Azure AD 액세스 검토를 사용 하는 방법](https://docs.microsoft.com/azure/active-directory/governance/access-reviews-overview)
...
**지침**: Azure DDoS Protection에는이 권장 사항이 적용 되지 않습니다.
...
**지침**: 해당 사항 없음 사용자가 정의 하는 리소스 그룹에 DDoS Protection 요금제가 프로 비전 됩니다. 보호 하거나 격리할 공용 끝점이 없습니다.
...
**지침**: Microsoft는 Azure DDoS Protection에 대 한 기본 인프라를 관리 하 고, 고객 데이터 손실 또는 노출을 방지 하기 위해 엄격한 컨트롤을 구현 했습니다.
...
**지침**: 해당 사항 없음 Azure DDoS Protection는 미사용 고객 데이터를 포함 하지 않습니다.
...
**지침**: 해당 사항 없음 Azure DDoS protection은 고객 데이터를 보유 하지 않습니다.
...
**지침**: DDoS 보호 계획을 사용 하려면 계정을 네트워크 기여자 역할 또는 특정 작업에 할당 된 사용자 지정 역할에 할당 해야 합니다.

* [Azure DDoS Protection에서 역할 기반 액세스 제어 관리](https://docs.microsoft.com/azure/virtual-network/manage-ddos-protection)
...
**지침**: 해당 없음. 이 권장 사항은 컴퓨팅 리소스를 위한 것입니다. Microsoft는 Azure DDoS Protection에 대 한 기본 인프라를 관리 하 고, 고객 데이터의 손실 또는 노출을 방지 하기 위해 엄격한 컨트롤을 구현 했습니다.
...
**지침**: 해당 사항 없음 Azure DDoS Protection에는 고객 데이터가 저장 되지 않습니다.
...
**지침**: 해당 사항 없음 Microsoft는 Azure DDoS Protection을 지 원하는 기본 시스템에서 취약성 관리를 수행 합니다.
...
**지침**: 해당 사항 없음 DDoS Protection 계획을 사용 하려면 계정을 네트워크 기여자 역할 또는 적절 한 작업이 할당 된 사용자 지정 역할에 할당 해야 합니다. DDoS Protection에 액세스 하는 데 필요한 암호, 비밀 또는 키가 없습니다.
...
**지침**: 해당 사항 없음 Azure DDoS Protection는 관리 되는 id를 사용 하지 않습니다.
...
**지침**: 해당 없음. 이 지침은 컴퓨팅 리소스를 위한 것입니다. Microsoft 맬웨어 방지는 Azure 서비스 (예: Azure DDoS Protection)를 지 원하는 기본 호스트에서 사용 하도록 설정 되어 있지만 고객 콘텐츠에서 실행 되지 않습니다.
...
**지침**: 해당 사항 없음 Azure DDoS Protection 자체는 고객 데이터를 저장 하지 않습니다.
