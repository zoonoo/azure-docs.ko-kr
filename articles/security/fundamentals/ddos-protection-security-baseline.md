---
title: Azure DDoS Protection 표준에 대한 Azure 보안 기준
description: Azure DDoS Protection 표준 보안 기준은 Azure Security Benchmark에 지정된 보안 추천 사항을 구현하기 위한 절차 지침과 리소스를 제공합니다.
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/29/2021
...
ms.locfileid: "105966999"
---
# <a name="azure-security-baseline-for-azure-ddos-protection-standard"></a>Azure DDoS Protection 표준에 대한 Azure 보안 기준

이 보안 기준은 [Azure Security Benchmark 버전 1.0](../benchmarks/overview-v1.md)의 지침을 Azure DDoS Protection 표준에 적용합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다. 콘텐츠는 Azure Security Benchmark에서 정의한 **보안 제어** 및 Azure DDoS Protection 표준에 적용되는 관련 지침에 따라 그룹화됩니다. Azure DDoS Protection 표준에 적용되지 않거나 Microsoft의 책임인 **컨트롤** 은 제외되었습니다.

Azure DDoS Protection 표준이 Azure Security Benchmark에 완전히 매핑되는 방법을 확인하려면 [전체 Azure DDoS Protection 표준 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조하세요.
...
**지침**: Azure 활동 로그 진단 설정을 사용하도록 설정하고 보관을 위해 로그를 Log Analytics 작업 영역, Azure 이벤트 허브 또는 Azure 스토리지 계정으로 보냅니다. 활동 로그는 컨트롤 플레인 수준에서 Azure DDoS Protection 계획에 대해 수행된 작업에 대한 인사이트를 제공합니다. Azure 활동 로그를 사용하면 컨트롤 플레인 수준에서 Azure DDoS Protection 인스턴스에 대해 수행된 쓰기 작업(PUT, POST, DELETE)에 대한 "대상, 주체, 시기(what, who, when)"를 확인할 수 있습니다.
...
**지침**: Azure Monitor 경고 구성을 사용하면 공격 중에 활성 완화가 있는 경우 알리는 데 사용할 수 있는 DDoS 보호 메트릭 중 하나를 선택할 수 있습니다. 조건에 부합하면 지정된 주소로 경고 메일이 수신됩니다.

Azure 활동 로그 진단 설정을 사용하도록 설정하고 보관을 위해 로그를 Log Analytics 작업 영역, Azure 이벤트 허브 또는 Azure 스토리지 계정으로 보냅니다. 활동 로그는 컨트롤 플레인 수준에서 Azure Cache for Redis 인스턴스에 대해 수행된 작업에 대한 인사이트를 제공합니다. Azure 활동 로그를 사용하면 컨트롤 플레인 수준에서 Azure DDoS Protection 인스턴스에 대해 수행된 쓰기 작업(PUT, POST, DELETE)에 대한 "대상, 주체, 시기(what, who, when)"를 확인할 수 있습니다.
...
**지침**: Azure Monitor에서 조직의 규정 준수 규정에 따라 Azure DDoS Protection과 연결된 Log Analytics 작업 영역에 대한 로그 보존 기간을 설정합니다.
...
**지침**: Azure 활동 로그 진단 설정을 사용하도록 설정하고, 로그를 Log Analytics 작업 영역에 보냅니다. Log Analytics에서 쿼리를 수행하여 용어를 검색하고, 추세를 식별하고, 패턴을 분석하고, Recovery Services vaults에 대해 수집되었을 수 있는 Activity Log 데이터를 기반으로 하여 다양한 인사이트를 제공합니다.

- [DDoS Protection 표준 서비스에 대한 원격 분석, 로그 및 공격 분석에 액세스하는 방법에 대한 정보](/azure/ddos-protection/telemetry-monitoring-alerting#configure-alerts-for-ddos-protection-metrics)
...
**지침**: 경고 및 공격 분석 구성 Azure DDoS Protection은 사용자 개입 없이 DDoS 공격을 식별하여 완화합니다.

SOAR(보안 오케스트레이션 자동화 응답) 솔루션을 제공하므로 Log Analytics 작업 영역을 Azure Sentinel에 온보딩합니다. 이를 통해 플레이북(자동화된 솔루션)을 만들어 보안 문제를 수정하는 데 사용할 수 있습니다. 또한 Azure Monitor를 사용하여 Log Analytics 작업 영역에서 사용자 지정 로그 경고를 만들 수 있습니다.

- [DDoS 메트릭에 대한 경고를 구성하는 방법](../../ddos-protection/alerts.md)
...
**지침**: DDoS 보호 계획을 사용하려면 적절한 작업이 할당된 네트워크 참가자 역할 또는 사용자 지정 역할에 계정을 할당해야 합니다.

또한 Azure AD(Azure Active Directory)에는 명시적으로 할당되고 쿼리할 수 있어야 하는 기본 제공 역할이 있습니다. Azure AD PowerShell 모듈을 사용하여 임시 쿼리를 수행해서 관리 그룹의 구성원인 계정을 검색합니다.
...
**지침**: Azure 앱 등록(서비스 주체)을 사용하여 API 호출을 통해 DDoS Protection 계획과 상호 작용하는 데 사용할 수 있는 토큰을 검색합니다.
...
**지침**: Azure AD(Azure Active Directory) 다단계 인증을 사용하도록 설정하고, Azure Security Center ID 및 액세스 관리 권장 사항을 따릅니다.

- [Azure Active Directory Multi-Factor Authentication을 사용하는 방법](../../active-directory/authentication/howto-mfa-getstarted.md)
...
**지침**: Azure Active Directory Multi-Factor Authentication이 사용하도록 설정된 안전한 Azure 관리형 워크스테이션을 사용하여 로그인하고 Azure 고객 Lockbox 요청을 구성합니다.

- [안전한 Azure 관리형 워크스테이션 배포](/security/compass/overview)
...
- [Privileged Identity Management를 배포하는 방법](../../active-directory/privileged-identity-management/pim-deployment-plan.md)
...
**지침**: Azure AD(Azure Active Directory) 명명된 위치를 사용하여 IP 주소 범위 또는 국가/지역의 특정 논리 그룹에서만 Azure Portal에 액세스하도록 허용합니다.
...
**지침**: Azure DDoS Protection 계획을 사용하려면 특정 작업이 할당된 네트워크 참가자 역할 또는 사용자 지정 역할에 계정을 할당해야 합니다.

- [Azure DDoS Protection에서 Azure RBAC 관리](/azure/virtual-network/manage-ddos-protection)
...
**지침**: Azure Monitor에서 Azure 활동 로그를 사용하여 Azure DDoS Protection 계획 및 기타 중요하거나 관련된 리소스가 변경되는 경우에 대한 경고를 만듭니다.
...
**지침**: 메타데이터를 제공하는 Azure 리소스에 태그를 적용하여 논리적 분류 체계로 구성합니다.
...
**지침**: Azure Policy를 사용하여 구독에 만들 수 있는 리소스 형식에 관한 제한을 설정합니다.
...
**지침**: Azure Policy를 사용하여 Azure DDoS Protection에 대한 표준 보안 구성을 정의 및 구현합니다. "Microsoft.Network" 네임스페이스에서 Azure Policy 별칭을 사용하여 Recovery Services vaults의 구성을 감사 및 시행하는 사용자 지정 정책을 만듭니다.
...
**지침**: Microsoft 맬웨어 방지는 Azure 서비스(예: Azure DDoS Protection)를 지원하는 기본 호스트에서 사용하도록 설정되지만 고객 콘텐츠에서는 실행되지 않습니다.
...
DDoS 공격을 시뮬레이션하기 위해 애플리케이션에 대한 트래픽을 생성하여 서비스가 공격에 어떻게 대응할지에 대한 가정을 테스트하세요. 실제 공격이 발생할 때까지 기다리지 마세요. Microsoft는 Azure DDoS Protection 고객이 Azure 퍼블릭 엔드포인트에 대해 DDoS 테스트 트래픽을 시뮬레이션할 수 있는 셀프 서비스 트래픽 생성기(BreakingPoint Cloud)를 제공하기 위해 Keysight 회사인 Ixia와 파트너십을 맺었습니다.
...
Azure Monitor 경고 구성을 사용하면 공격 중에 활성 완화가 있는 경우 알리도록 제공되는 DDoS 보호 메트릭 중 하나를 선택합니다. 조건이 충족되면 지정된 주소에서 경고 메일을 받습니다.
...
**지침**: 침투 테스트가 Microsoft 정책을 위반하지 않도록 Microsoft 침투 테스트 시행 규칙을 따릅니다. Microsoft 관리형 클라우드 인프라, 서비스 및 애플리케이션에 대한 Microsoft의 전략과 Red Teaming 및 라이브 사이트 침투 테스트의 실행을 사용합니다.
