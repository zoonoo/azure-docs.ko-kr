...
미사용 데이터 암호화는 일반적인 보안 요구 사항입니다. Azure에서는 조직에서 구현 및 관리 비용과 사용자 지정 키 관리 솔루션에 대한 위험을 부담하지 않고도 미사용 데이터 암호화를 달성할 수 있다는 것입니다. 조직에는 Azure에서 미사용 데이터 암호화를 완전히 관리할 수 있도록 하는 옵션이 제공됩니다. 또한 암호화 또는 암호화 키를 면밀하게 관리할 수 있는 다양한 옵션도 제공됩니다.
...
- 키는 특정 ID에 대한 액세스를 제한하고 키 사용을 기록하는 액세스 제어 정책을 사용하여 안전한 위치에 저장해야 합니다. 데이터 암호화 키는 종종 액세스를 더욱 제한하기 위해 비대칭 암호화로 암호화됩니다.
...
Azure SQL Database는 현재 Microsoft 관리 서비스 쪽 및 클라이언트 쪽 암호화 시나리오에 대한 미사용 데이터 암호화를 지원합니다.
