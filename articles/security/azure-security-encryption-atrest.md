...
서비스 (PaaS) 고객의 데이터 플랫폼 일반적으로 Blob 저장소와 같은 저장소 서비스에 상주 하지만 캐시 되었거나 응용 프로그램 실행 환경 등 가상 컴퓨터에에서 저장 된. 저장 데이터 암호화 옵션을 사용할 수 있는지 확인하려면 아래 표에서 사용할 수 있는 저장소 및 애플리케이션 플랫폼을 살펴보세요.
...
PaaS와 마찬가지로 IaaS 솔루션은 암호화된 미사용 데이터를 저장하는 다른 Azure 서비스를 활용할 수 있습니다. 이 경우 사용된 각 Azure 서비스에서 제공하는 대로 미사용 데이터 암호화 지원을 사용하도록 설정할 수 있습니다. 아래 표에서는 주 저장소, 서비스 및 애플리케이션 플랫폼과 지원되는 미사용 데이터 암호화 모델을 나열합니다. 

#### <a name="encrypted-compute"></a>암호화된 계산

모든 Managed Disks, 스냅숏 및 이미지는 암호화 된 Storage 서비스 암호화를 사용 하 여 서비스 관리 키를 사용 하 여 합니다. 전체 휴지 상태의 암호화 솔루션은 데이터를 저장할 수 없는 암호화 된 형식에서 확인 합니다. 가상 머신에서 데이터를 처리 하는 동안 데이터를 Windows 페이지 파일 또는 Linux 스왑 파일, 크래시 덤프, 또는 응용 프로그램 로그에 유지할 수 있습니다. 이러한 미사용 데이터가 암호화되어 있는지 확인하려면 IaaS 애플리케이션이 Azure IaaS 가상 머신(Windows 또는 Linux) 및 가상 디스크에서 Azure Disk Encryption을 사용할 수 있습니다.
...
모든 Azure Storage 서비스 (Blob storage, Queue storage, Table storage 및 Azure Files), 미사용 서버 쪽 암호화를 지원합니다. 일부 서비스는 또한 고객 관리 키 및 클라이언트 쪽 암호화를 지원합니다. 
...
|                                  |                    | **암호화 모델 및 키 관리** |                    |
|----------------------------------|--------------------|-----------------------------------------|--------------------|
|                                  | **서비스 관리 키를 사용하는 서버 쪽**     | **고객 관리 키를 사용 하 여 서버 쪽**             | **클라이언트 관리를 사용하는 클라이언트 쪽**      |
| **AI 및 기계 학습**      |                    |                    |                    |
| Azure Search                     | 예                | -                  | -                  |
...
| MySQL 용 azure SQL Database     | 예                | -                  | -                  |
| PostgreSQL 용 azure SQL Database | 예                | -                  | -                  |
