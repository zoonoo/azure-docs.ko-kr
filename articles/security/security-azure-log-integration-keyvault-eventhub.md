...
1. `Connect-AzureRmAccount` 명령을 입력합니다. 로그인 창에서 이 자습서에 사용할 구독에 대한 자격 증명 정보를 입력합니다.
...
1. 인증에 성공하면 로그인되고 다음 스크린샷의 정보가 표시됩니다. 구독 ID 및 구독 이름을 메모해 두세요. 이후 단계를 완료하는 데 필요합니다.
