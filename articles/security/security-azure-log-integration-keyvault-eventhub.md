...
3. [Azure 로그 통합](https://www.microsoft.com/download/details.aspx?id=53324) 설치됨. 설치하려면:
   1. 원격 데스크톱을 사용하여 2단계에서 언급한 시스템에 연결합니다.
   2. Azure 로그 통합 설치 관리자를 시스템에 복사합니다. [설치 파일을 다운로드](https://www.microsoft.com/download/details.aspx?id=53324)할 수 있습니다.
   3. 설치 관리자를 시작하고 Microsoft 소프트웨어 사용 조건에 동의합니다.
   4. 원격 분석 정보를 입력하는 경우 확인란을 선택한 상태로 둡니다. 사용 정보를 Microsoft로 보내지 않으려면 확인란의 선택을 취소합니다.
...
4. 최신 PowerShell 버전입니다.
   ...
   1. PowerShell 창에 ```Install-Module Azure```를 입력하고 Enter 키를 누릅니다. 설치 단계를 완료합니다. 
   2. ```Install-Module AzureRM```을 입력하고 Enter 키를 누릅니다. 설치 단계를 완료합니다.
...
2. LoadAzLogModule.ps1 스크립트를 실행하여 AzLog cmdlet을 가져옵니다. (다음 명령에서 “.\”를 확인하세요.) `.\LoadAzLogModule.ps1`을 입력하고 Enter 키를 누릅니다.
다음과 유사한 결과가 표시됩니다.</br>

   ![로드된 모듈 목록](./media/security-azure-log-integration-keyvault-eventhub/loaded-modules.png)

3. `Login-AzureRmAccount`를 입력하고 Enter 키를 누릅니다. 로그인 창에서 이 자습서에 사용할 구독에 대한 자격 증명 정보를 입력합니다.
...
5. 나중에 사용할 값을 저장하는 변수를 만듭니다. 다음의 PowerShell 줄을 각각 입력하고 각 줄 다음에 Enter 키를 누릅니다. 환경에 맞게 값을 조정해야 할 수도 있습니다.
    - ```$subscriptionName = ‘Visual Studio Ultimate with MSDN’```(구독 이름은 다를 수 있습니다. 이는 이전 명령의 출력에서 볼 수 있습니다.)
    - ```$location = 'West US'```(이 변수는 리소스를 만들 위치를 전달하는 데 사용됩니다. 이 변수를 선택한 다른 위치로 변경할 수 있습니다.)
    - ```$random = Get-Random```
...
7. 리소스 그룹 만들기:
   ...
   이때 `$rg`를 입력하고 Enter 키를 누르면 이 스크린샷과 유사한 출력이 나타납니다.
...
11. 가능한 모든 Azure 위치를 가져오고 이후 단계에서 사용할 수 있는 변수에 이름을 추가합니다.
    ...
    이때 `$locations`를 입력하고 Enter 키를 누르면 Get-AzureRmLocation에서 반환되는 추가 정보 없이 위치 이름이 표시됩니다.
...
>[!NOTE]
>로그 프로필을 만들려고 할 때 오류 메시지가 표시될 수도 있습니다. 그러면 Get-AzureRmLogProfile 및 Remove-AzureRmLogProfile에 대한 설명서를 검토하면 됩니다. Get-AzureRmLogProfile을 실행하는 경우 로그 프로필에 대한 정보가 표시됩니다. ```Remove-AzureRmLogProfile -name 'Log Profile Name' ```을 입력하고 Enter 키를 누르면 기존 로그 프로필을 삭제할 수 있습니다.
>
