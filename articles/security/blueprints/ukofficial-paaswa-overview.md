...
이 청사진은 디렉터리 및 ID 관리 서비스를 통해 리소스에 대한 액세스를 보호합니다. 이 아키텍처는 [ID를 보안 경계로](../fundamentals/paas-deployments.md) 최대한 활용합니다. 
...
- [Azure Active Directory 조건부 액세스](../../active-directory/active-directory-conditional-access-azure-portal.md)를 활용하여 고객은 위치, 디바이스, 상태 및 로그인 위험과 같은 특정 조건에 따라 환경에서 앱 또는 사용자의 액세스에 대한 추가 보안 제어를 적용할 수 있습니다.
- [Azure DDoS Protection](../fundamentals/paas-deployments.md#security-advantages-of-a-paas-cloud-service-model)은 애플리케이션 설계 모범 사례와 결합하여 상시 모니터링 트래픽 및 일반적인 네트워크 수준 공격에 대한 실시간 완화를 통해 DDoS 공격 방어를 제공합니다. PaaS 아키텍처에서는 플랫폼 수준의 DDoS 보호가 고객에게 투명하고 플랫폼에 통합되지만 애플리케이션 보안 설계 책임은 고객에게 있다는 점에 유의해야 합니다.
...
- 스토리지 계정에 대한 [감사 로그](https://docs.microsoft.com/azure/sql-database/sql-database-auditing)
- 실패한 DB 연결에 대한 메트릭 [경고](../../azure-monitor/app/alerts.md)
...
- 고객이 연결하는 웹앱의 [관리 ID](https://docs.microsoft.com/azure/app-service/overview-managed-identity)에 읽기 액세스 권한이 부여된 스토리지 액세스 키를 보유합니다.
...
[Azure Monitor](../../azure-monitor/overview.md)를 사용하면 메트릭, 활동 로그 및 진단 로그를 수집하도록 허용하여 Azure 서비스에 대한 핵심 모니터링을 수행할 수 있습니다. Azure Monitor는 대부분의 Microsoft Azure 서비스에 대해 기본 수준의 인프라 메트릭과 로그를 제공합니다.
