...
Azure는 고객에게 다양한 보고 및 분석 서비스를 제공합니다. 이 솔루션은 Azure Machine Learning 서비스와 SQL Database를 사용하여 데이터를 빠르게 탐색하며, 더욱 지능적인 데이터 모델링을 통해 결과를 더 빨리 제공합니다. Machine Learning은 데이터 집합 간의 새로운 관계를 검색하여 쿼리 속도를 높입니다. 처음에는 몇 가지 통계 기능을 통해 데이터 학습을 수행합니다. 그 후에는 최대 7개의 추가 쿼리 풀을 같은 테이블 형식 모델과 동기화하여 쿼리 워크로드를 분산시키고 응답 시간을 줄일 수 있습니다. 이 경우 고객 서버의 총 쿼리 풀 수는 8개가 됩니다.
...
데이터가 SQL Database에 업로드되고 Machine Learning에서 학습되면 운영 사용자와 SQL/데이터 관리자가 Power BI를 사용하여 이러한 데이터를 요약합니다. Power BI는 직관적으로 데이터를 표시하고, 더 많은 통찰력을 얻기 위해 여러 데이터 집합에서 정보를 가져옵니다. Power BI는 원하는 방식으로 세밀하게 조정할 수 있으며 SQL Database와 쉽게 통합할 수 있습니다. 고객은 다양한 비즈니스 요구에 따라 필요한 폭넓은 시나리오를 처리하도록 Power BI를 구성할 수 있습니다.
...
**Azure Event Grid**: [Event Grid](https://docs.microsoft.com/azure/event-grid/overview)를 사용하면 고객이 이벤트 기반 아키텍처를 통해 응용 프로그램을 쉽게 구축할 수 있습니다. 사용자는 구독하려는 Azure 리소스를 선택한 다음 이벤트 처리기나 웹후크에 이벤트를 전송할 엔드포인트를 제공합니다. 고객은 이벤트 구독을 만들 때 웹후크 URL에 쿼리 매개 변수를 추가하여 웹후크 엔드포인트를 보호할 수 있습니다. Event Grid는 HTTPS 웹후크 엔드포인트만 지원합니다. 고객은 Event Grid를 사용해 다양한 관리 작업을 수행할 수 있도록 사용자별로 제공할 액세스 수준을 제어할 수 있습니다. 사용자는 이벤트 구독을 나열하고 새 구독을 만들고 키를 생성할 수 있습니다. Event Grid는 Azure RBAC를 사용합니다.
...
**Azure Machine Learning**: [Machine Learning](https://docs.microsoft.com/azure/machine-learning/preview/)은 컴퓨터에서 기존 데이터를 사용하여 향후의 동작, 결과 및 추세를 예측할 수 있도록 하는 데이터 과학 기술입니다.
...
**Log Analytics**: 로그는 처리, 저장 및 대시보드 보고를 위해 [Log Analytics](https://azure.microsoft.com/services/log-analytics/)에 통합됩니다. 수집된 데이터는 Operations Management Suite 작업 영역 내에서 각 데이터 형식용 개별 테이블로 구성됩니다. 이러한 방식으로 인해 원래 원본에 관계없이 모든 데이터를 함께 분석할 수 있습니다. Security Center는 Log Analytics와 통합됩니다. 고객은 Log Analytics 쿼리를 사용하여 보안 이벤트 데이터에 액세스한 다음 다른 서비스의 데이터와 결합할 수 있습니다.
...
**Application Insights**: [Application Insights](https://docs.microsoft.com/azure/application-insights/)는 여러 플랫폼의 웹 개발자를 위한 확장 가능한 응용 프로그램 성능 관리 서비스입니다. 성능 이상을 검색하는 이 솔루션에는 유용한 분석 도구가 포함되어 있습니다. 이러한 도구를 통해 문제를 진단할 수 있으며, 고객은 사용자가 앱을 사용하여 수행하는 작업을 파악할 수 있습니다. 사용자가 성능 및 가용성을 지속적으로 향상시킬 수 있도록 설계되었습니다.
