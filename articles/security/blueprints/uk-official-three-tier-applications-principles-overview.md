...
| **공급자&nbsp;(Microsoft&nbsp;Azure)** | Microsoft 규정 준수 프레임 워크에는 규정 준수 도메인을 정의하고, 특정 팀 또는 자산에 적용되는 목표를 결정하고, 도메인 제어 목표를 특정 산업 표준, 규정 또는 비즈니스 요구 사항에 적용할 때 처리하는 방식을 자세히 캡처하기 위한 표준 방법론이 포함되어 있습니다. 프레임워크는 컨트롤을 다수의 규제 표준에 매핑하기 때문에 Microsoft는 공통된 컨트롤 집합을 사용하여 서비스를 설계하고 구축할 수 있으며 이로 인해 현재의 다양한 규제와 미래의 진화하는 규제 전반에 걸쳐 규정 준수를 간소화할 수 있습니다. <br /> <br /> Microsoft 규정 준수 프로세스를 통해 고객은 여러 서비스에서 규정 준수를 보다 쉽게 실현하고 변화하는 요구 사항을 효율적으로 충족시킬 수 있습니다. 보안 강화 기술과 효과적인 규정 준수 프로세스를 통해 Microsoft는 다양한 타사 인증을 유지 관리하고 확장할 수 있습니다. 이러한 인증은 고객, 감사원 및 규제 당국에 규정 준수 준비 상태를 입증하는 데 도움이 됩니다. <br /> <br />  Azure는 ISO 27001, FedRAMP, SOC 1 및 SOC 2와 같은 광범위한 국제 표준 및 지역별 및 산업별 규정 준수 표준을 준수합니다. 이러한 표준에 포함된 엄격한 보안 컨트롤 준수는 Azure 서비스가 세계적인 수준의 산업 표준, 인증, 증명 및 권한 부여 작업을 수행하고 충족한다는 것을 입증하는 엄격한 타사 감사를 통해 검증됩니다. <br /> <br /> Azure는 고객이 비즈니스 목표는 물론 산업 표준 및 규정을 처리하는 데 도움을 주는 규정 준수 전략으로 설계되었습니다. 보안 규정 준수 프레임워크에는 인증 및 증명을 구현하기 위한 테스트 및 감사 단계, 보안 분석, 위험 관리 모범 사례 및 보안 벤치마크 분석이 포함되어 있습니다. <br /> <br /> Microsoft의 규정 준수 프레임 워크 준수에 대한 자세한 내용은 [Azure Blueprint - NCSC 클라우드 보안 원칙 - 고객 책임 매트릭스](https://aka.ms/blueprintuk-gcrm)의 전체 설명을 참조하세요.|
...
|||
|---|---|
| **고객** | Azure Resource Manager 템플릿과 Azure Blueprint를 구성하는 함께 제공되는 리소스는 배포된 아키텍처의 "코드로 구성" 기준을 나타냅니다. 솔루션은 GitHub를 통해 제공되며, 구성 제어에 사용할 수 있습니다. <br /> <br /> 배포된 리소스를 보고 제어할 수 있는 사용자를 철저하게 제어하는 역할에 사용자를 할당함으로써 역할 기반 액세스 제어를 사용하여 Azure Active Directory 계정 권한을 구현합니다. Active Directory 계정 권한은 보안 그룹에 사용자를 할당함으로써 역할 기반 액세스 제어를 사용하여 구현됩니다. 이러한 보안 그룹은 운영 체제 구성과 관련하여 사용자가 수행할 수 있는 작업을 제어합니다. 이러한 역할 기반 체계는 업무 요구 사항을 충족시키기 위해 고객이 확장할 수 있습니다. <br /> <br /> 이 원칙을 준수하려면 프로덕션에 사용하기 위해 고객이 추가 구성을 해야 합니다. 따라서 이러한 구성은 고객의 변경 관리 프로세스의 일부가 되어야 합니다. |
...
|||
|---|---|
| **고객** | 이 Azure Blueprint에서 배포한 가상 머신은 Windows 운영 체제를 실행합니다. Windows는 실시간 무결성 검사가 가능한 WRP(Windows 리소스 보호) 기능을 통해 권한 있는 Windows 시스템 업데이트 또는 Windows의 일부로 설치되는 핵심 시스템 파일의 복구, 실시간 파일 무결성 유효성 검사 및 보호 기능을 제공합니다. <br /> <br /> Windows에는 NX(No Execute), ASLR(Address Space Layout Randomization) 및 DEP(데이터 실행 방지) 등 제한된 메모리 위치에서 코드 실행을 방지하는 보호 기능이 있습니다. <br /> <br /> 이 Azure Blueprint는 배포된 모든 Windows 가상 머신에 대해 Microsoft Windows Defender를 사용하여 구현되는 호스트 기반 맬웨어 방지 보호를 배포합니다. Windows Defender는 릴리스가 제공될 때 맬웨어 방지 엔진 및 보호가 모두 자동으로 업데이트되도록 구성됩니다. <br /> <br /> 이 원칙을 준수하려면 프로덕션에 사용하기 위해 고객이 추가 구성을 해야 합니다. 따라서 이러한 구성은 고객의 보안 개발 프로세스의 일부가 되어야 합니다. |
| **공급자&nbsp;(Microsoft&nbsp;Azure)** | Microsoft SDL(Security Development Lifecycle)은 소프트웨어 및 서비스에서 위협 및 취약점을 파악하는 효과적인 위협 모델링 프로세스를 제공합니다. 위협 모델링은 작업 관리자, 프로그램/프로젝트 관리자, 개발자 및 테스터를 포괄하는 팀 작업이며 솔루션 설계를 위해 수행된 주요 보안 분석 작업을 나타냅니다. 팀 멤버는 SDL 위협 모델링 도구를 사용하여 모든 서비스와 프로젝트를 구축할 때와 새 특징과 기능으로 업데이트하는 경우 모두 모델링을 수행합니다. 위협 모델은 공격 대상에 노출된 모든 코드와 타사가 작성했거나 라이선스가 부여된 모든 코드를 포함하며 모든 트러스트 경계를 고려합니다. STRIDE 시스템(스푸핑, 변조, 거절, 정보 유출, 서비스 거부 및 권한 상승)은 설계 프로세스 초기에 보안 위협이 고객에게 영향을 미치기 전에 이를 식별하고 해결하는 데 도움이 됩니다. |
...
| **공급자&nbsp;(Microsoft&nbsp;Azure)** | MCSC(Microsoft 클라우드 공급망) 그룹은 공급망에 대한 위협으로부터 Azure를 보호하는 데 기여하는 6개의 고유한 팀으로 구성됩니다.  <br />  <br /> - 조달 <br /> - 고객 운영 <br /> - 배포 품질 <br /> - 공급자 관계 관리 <br /> - 스패어 <br />  <br /> Microsoft의 MCSC 그룹에 대한 자세한 내용은 [Azure Blueprint - NCSC 클라우드 보안 원칙 - 고객 책임 매트릭스](https://aka.ms/blueprintuk-gcrm)의 전체 설명을 참조하세요. |
