...
**세부 정보**: 키와 자격 증명 분실은 흔히 발생하는 문제입니다. 이러한 문제를 방지하기 위해 하드웨어 보안 모듈에 키와 비밀을 저장할 수 있는 중앙 집중식 솔루션을 사용할 수 있습니다. Azure에서는 [Azure Key Vault](../key-vault/key-vault-whatis.md)를 사용하여 HSM을 클라우드에 제공하고 있습니다.
...
**모범 사례**: [Azure Active Directory를 통해 인증합니다](../app-service/app-service-authentication-overview.md).   
**세부 정보**: App Service는 ID 공급자를 위한 OAuth 2.0 서비스를 제공합니다. OAuth 2.0은 클라이언트 개발자가 개발을 손쉽게 수행할 수 있도록 하는 기능을 중점적으로 제공하는 동시에, 웹 응용 프로그램/데스크톱 응용 프로그램/휴대폰용 특정 권한 부여 흐름을 제공합니다. Azure AD는 OAuth 2.0을 사용하여 모바일 및 웹 응용 프로그램에 대한 액세스 권한을 부여합니다.
...
**세부 정보**: Azure Security Center를 사용하여 App Service Environment를 모니터링합니다. Security Center는 잠재적 보안 취약성이 확인되면 필요한 컨트롤을 구성하는 과정을 안내하는 [권장 사항](../security-center/security-center-virtual-machine-recommendations.md)을 만듭니다.
