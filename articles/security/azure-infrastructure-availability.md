...
Microsoft 클라우드 인프라 및 운영 팀은 클라우드 인프라를 디자인하고, 빌드하고, 운영하고, 보호합니다. 이 팀은 Azure 인프라가 고가용성 및 안정성, 고효율성, 스마트 확장성 및 안전하고 신뢰할 수 있는 개인 클라우드를 제공하는지 확인합니다.

무정전 전원 공급 장치 및 엄청나게 많은 배터리는 단기 전원 중단이 발생할 경우 전기가 계속 유지되도록 보장합니다.

응급 생성기는 확장된 작동 중단 및 계획된 유지 관리에 대해 백업 전원을 제공합니다. 데이터 센터는 자연 재해가 발생한 경우 현장 연료 탱크를 사용하여 운영됩니다.

고속 및 강력한 광섬유 네트워크는 다른 주요 허브와 인터넷 사용자와 데이터 센터를 연결합니다. 대기 시간을 줄이기 위해 최종 사용자에게 가까운 계산 노드 호스트 워크로드는 지역 중복을 제공하고 전체 서비스 복원력을 증가시킵니다. 엔지니어 팀은 서비스가 영구적으로 고객에게 제공되도록 쉬지않고 작동합니다.
...
Azure는 백업 사이트의 위치를 선택할 수 있는 역량이 있는 고객의 경우 두 위치에서 고객 데이터를 지속 가능하도록 유지합니다. Azure는 두 위치 모두에서 지속적으로 고객 데이터의 여러(3개) 정상 복제본을 유지 관리합니다.
...
Azure에서는 확장성이 높고 영속성이 있는 저장소 서비스를 통해 저장소를 제공합니다. 여기서는 사용 중인 응용 프로그램에서 직접 액세스할 수 있도록 하는 연결 엔드포인트를 제공합니다. 저장소 서비스를 통해 트랜잭션 무결성을 사용하여 들어오는 저장소 요청을 처리합니다.
