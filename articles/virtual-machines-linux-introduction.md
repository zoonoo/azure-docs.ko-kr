...
-   [인증: 사용자 이름, 암호 및 SSH 키][인증: 사용자 이름, 암호 및 SSH 키]
-   [Linux 가상 컴퓨터 로그인용 SSH 키 생성 및 사용][Linux 가상 컴퓨터 로그인용 SSH 키 생성 및 사용]
-   [sudo를 사용하여 superuser 권한 얻기][sudo를 사용하여 superuser 권한 얻기]
-   [방화벽 구성][방화벽 구성]
-   [호스트 이름 변경][호스트 이름 변경]
-   [가상 컴퓨터 이미지 캡처][가상 컴퓨터 이미지 캡처]
-   [디스크 연결][디스크 연결]

## <span id="authentication"></span></a>인증: 사용자 이름, 암호 및 SSH 키

Azure 관리 포털에서 Linux 가상 컴퓨터를 만들 경우 사용자 이름, 암호 및 SSH 공개 키(선택적으로)를 지정해야 합니다. Azure에서 Linux 가상 컴퓨터를 배포하기 위해 사용자 이름을 선택할 경우 제약 조건이 적용됩니다. 즉, 가상 컴퓨터에 이미 존재하는 시스템 계정(UID \<100)의 이름(예: 루트)은 사용할 수 없습니다.

-   [Azure에서 Linux와 함께 SSH를 사용하는 방법][Azure에서 Linux와 함께 SSH를 사용하는 방법] 참조

### <span id="keygeneration"></span></a>SSH 키 생성
...
-   [Azure의 Linux 가상 컴퓨터에서 루트 권한 사용][Azure의 Linux 가상 컴퓨터에서 루트 권한 사용] 참조

## <span id="firewallconfiguration"></span></a>방화벽 구성
...
-   [Azure Linux 에이전트 사용자 가이드][Azure Linux 에이전트 사용자 가이드]

### Ubuntu 이미지

Ubuntu 이미지는 cloud-init를 활용하여 가상 컴퓨터를 부트스트랩하기 위한 추가 기능을 제공합니다.

-   [Microsoft Azure의 사용자 지정 데이터 및 Cloud-Init][Microsoft Azure의 사용자 지정 데이터 및 Cloud-Init](영문) 참조

## <span id="virtualmachine"></span></a>가상 컴퓨터 이미지 캡처
...
3.  관리 포털에서 *캡처*를 클릭하거나 Powershell 또는 CLI 도구를 사용하여 가상 컴퓨터를 이미지로 캡처합니다.

-   다음을 참조하세요. [Linux 가상 컴퓨터를 캡처하여 템플릿으로 사용하는 방법][Linux 가상 컴퓨터를 캡처하여 템플릿으로 사용하는 방법]

## <span id="attachingdisks"></span></a>디스크 연결
...
Linux에서 데이터 디스크 이름은 커널에 의해 `/dev/sdc`로 지정될 수 있으며 사용자는 해당 리소스를 파티셔닝, 형식 지정 및 마운트해야 합니다. 이 내용은 자습서의 다음 부분에서 단계별로 설명됩니다. [가상 컴퓨터에 데이터 디스크를 연결하는 방법][가상 컴퓨터에 데이터 디스크를 연결하는 방법]

-   참고 항목: [Linux에서 소프트웨어 RAID 구성][Linux에서 소프트웨어 RAID 구성]
  ...
  [Azure에서 Linux와 함께 SSH를 사용하는 방법]: ../linux-use-ssh-key/
  [Azure의 Linux 가상 컴퓨터에서 루트 권한 사용]: ../virtual-machines-linux-use-root-privileges/
  [가상 컴퓨터에 끝점을 설정하는 방법]: ../virtual-machines-set-up-endpoints/
  ...
  [Linux 가상 컴퓨터를 캡처하여 템플릿으로 사용하는 방법]: ../virtual-machines-linux-capture-image/
  [가상 컴퓨터에 데이터 디스크를 연결하는 방법]: ../virtual-machines-linux-how-to-attach-disk/
