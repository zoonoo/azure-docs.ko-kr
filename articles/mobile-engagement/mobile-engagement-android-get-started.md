...
1. [Azure 포털](https://manage.windowsazure.com)에 로그인한 다음 화면 아래쪽에서 **+새로 만들기**를 클릭합니다.
...
3. 표시되는 팝업에서 다음 정보를 입력합니다.
    ...
    - **응용 프로그램 이름**: 응용 프로그램의 이름을 입력할 수 있습니다. 원하는 모든 문자를 사용할 수 있습니다.
    - **플랫폼**: 앱의 대상 플랫폼을 선택합니다. 앱의 대상 플랫폼이 여러 개이면 각 플랫폼에 대해 이 자습서의 작업을 반복합니다.
    - **응용 프로그램 리소스 이름**: API 및 URL을 통해 이 응용 프로그램에 액세스하는 데 사용되는 이름입니다. 기본 URL 문자만 사용하는 것이 좋습니다. 자동으로 생성되는 이름을 기준으로 사용할 수 있습니다. 또한 이 이름은 고유해야 하므로 이름 충돌을 방지하기 위해 플랫폼 이름을 추가하는 것이 좋습니다.
    - **위치**: 이 앱과 해당 컬렉션(아래 설명 참조)을 호스트할 데이터 센터를 선택합니다.
    - **컬렉션**: 응용 프로그램을 이미 만든 경우 이전에 만든 컬렉션을 선택하고 그러지 않은 경우 **새 컬렉션**을 선택합니다.
    - **컬렉션 이름**: 응용 프로그램 그룹을 나타냅니다. 또한 그룹을 사용하는 경우에는 모든 앱이 그룹에 포함되므로 집계 계산이 가능합니다. 회사 이름이나 부서를 사용하는 것이 좋습니다.

	작업을 완료했으면 확인 표시 단추를 클릭하여 앱 만들기를 완료합니다.

4. 이제 **응용 프로그램** 탭에서 방금 만든 앱을 클릭하거나 선택합니다.
...
6. 마지막으로 응용 프로그램 코드에서 이 앱을 식별하는 데 필요한 **연결 문자열**을 작성합니다.

    ![][11]

	>[AZURE.TIP]**연결 문자열** 오른쪽에 있는 **복사** 아이콘을 사용하여 연결 문자열을 클립보드로 편리하게 복사할 수 있습니다.
...
2. 앱 이름 및 회사 도메인을 입력합니다. 나중에 필요하므로 앱 이름 및 회사 도메인을 기록해 둔 후 **Next**를 클릭합니다.

    ![][13]

3. 이제 대상 폼 팩터 및 API 수준을 선택하고 **Next**를 클릭합니다.
...
5. 이 자습서에서는 마법사의 마지막 화면에서 모든 항목을 있는 그대로 둘 수 있으며, **Finish**를 클릭합니다.
...
4. **EngagementConfiguration** 및 **EngagementAgent**는 확인되지 않음으로 표시될 가능성이 높습니다(코드에서 빨간색으로 표시됨). 확인되지 않은 각 클래스를 클릭한 다음 Alt+Enter를 누르면 자동으로 확인됩니다.
...
데이터 보내기를 시작하고 사용자가 활성 상태인지 확인하려면 Mobile Engagement 백 엔드에 화면(활동)을 하나 이상 보내야 합니다. SDK에서 제공하는 **EngagementActivity**와 함께 작업의 서브클래스를 지정하여 이 작업을 수행합니다. 이렇게 하려면 아래와 같이 **ActionBarActivity** 앞에 있는 **MainActivity**의 상위 클래스를 **EngagementActivity**로 바꿉니다.

 ![][22]

>[AZURE.NOTE]클래스가 빨간색으로 표시되는 경우 잊지 말고 해당 클래스를 클릭하고 Alt+Enter를 눌러 확인합니다.

##<a id="monitor"></a>앱이 실시간 모니터링 기능과 연결되어 있는지 확인
...
3. 모니터는 앱을 시작할 장치를 실시간으로 표시할 준비가 되어 있습니다. ![][31]

4. Android Studio로 돌아가서 녹색 삼각형을 클릭한 다음 장치를 선택하여 모니터 또는 연결된 장치에서 앱을 시작합니다. ![][32]
...
다음 코드에서는 앱 및 시스템 알림 둘 다에 표시하는 데 사용되는 아이콘을 정의합니다.

이 아이콘은 앱 내 알림에는 옵션이지만 시스템 알림에는 필수입니다. Android에서는 아이콘이 잘못된 시스템 알림을 거부합니다.

이 XML 조각을 Manifest.xml의 `<application>` 태그와 `</application>` 태그 사이에 붙여넣습니다.

**drawable** 폴더(예: ``engagement_close.png``) 중 하나에 있는 아이콘을 사용하고 있는지 확인하세요. **mipmap** 폴더는 지원되지 않습니다.

		<meta-data android:name="engagement:reach:notification:icon" android:value="engagement_close"/>

구문을 보여 주기 위한 예일 뿐입니다. 물론 [Android 디자인 지침](http://developer.android.com/design/patterns/notifications.html)에 따라 알림에 적합한 아이콘을 사용해야 합니다.

**시작 관리자** 아이콘은 사용하면 안 됩니다. 시작 관리자는 해상도가 다르며 일반적으로 지원되지 않는 mipmap 폴더에 있습니다.
...
1. gcm:sender 메타데이터를 입력하고 다음을 복사하여 Manifest.xml의 `<application>` 태그와 `</application>` 태그 사이에 붙여넣습니다. 아래에서 별표로 숨겨진 값은 Google Play 콘솔에서 가져온 `project number`입니다. \\n은 프로젝트 번호가 \\n으로 끝나도록 의도적으로 사용되었습니다.

		<meta-data android:name="engagement:gcm:sender" android:value="************\n" />

2. 아래 코드를 Manifest.xml 파일의 `<application>` 태그와 `</application>` 태그 사이에 붙여넣습니다. <Your package name>을 패키지 이름으로 바꿉니다.
...
2. 그러면 Engagement 포털의 **설정** 페이지가 표시됩니다. 이 페이지에서 **네이티브 푸시** 섹션을 클릭하여 GCM 키를 입력합니다. ![][27]

3. 아래와 같이 **GCM 설정** 섹션의 **API 키** 앞에 있는 **편집** 아이콘을 클릭합니다. ![][28]
...
> [AZURE.IMPORTANT]빌드했는지 확인하고 이 새 코드로 시작하고 앱을 종료한 후 약 1분간 기다렸다가 다음 단계를 수행합니다.
...
2. **새 공지**를 클릭하여 푸시 알림 캠페인을 만듭니다. ![][35]
...
3. 다음 단계를 수행하여 캠페인의 첫 번째 필드를 설정합니다. ![][36]
	...
	3. 앱 시작 여부와 관계없이 앱에서 알림을 받을 수 있도록 **배달 시간**을 *모든 날짜*로 선택합니다.
