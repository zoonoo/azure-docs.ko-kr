<properties 
	pageTitle="Windows Phone Silverlight Engagement SDK 통합" 
...
>
> 응용 프로그램이 닫힐 때 SDK에서 `EndActivity` 메서드를 자동으로 호출합니다. 따라서 사용자 활동이 변경될 때마다 `StartActivity` 메서드를 호출하는 것이 **상당히** 좋으며 `EndActivity` 메서드는 호출하지 **않는** 것이 좋습니다. 이 메서드를 호출하면 현재 세션이 강제로 종료되기 때문입니다.
...
자세한 내용은 [Windows Phone Silverlight 앱에서 고급 Mobile Engagement 태깅 API를 사용하는 방법](../mobile-engagement-windows-phone-use-engagement-api/)을 참조하세요
