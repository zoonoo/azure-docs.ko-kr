<properties
	pageTitle="MobileApps Engagement를 사용한 Windows 유니버설 고급 보고"
...
# Windows 유니버설 앱 Engagement SDK의 고급 보고
...
> [AZURE.IMPORTANT] 페이지가 `OnNavigatedTo` 메서드를 재정의하는 경우에는 `base.OnNavigatedTo(e)`을(를) 호출해야 합니다. 그렇지 않으면 활동이 보고되지 않습니다. `EngagementPage`는 `OnNavigatedTo` 메서드 내에서 `StartActivity`를 호출합니다.
