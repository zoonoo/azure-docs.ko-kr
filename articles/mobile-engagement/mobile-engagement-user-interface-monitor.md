...
UI의 모니터 섹션에서는 실시간 분석 정보를 제공하며, 이전부터 UI의 "분석" 섹션에 있었던 거의 대부분의 정보에 대해 임계값에 도달하면 전송할 경고를 설정할 수 있습니다. "개념"의 "용어집"에 분석 및 모니터링에서 사용되는 용어와 약어의 정의가 나와 있습니다. 이러한 용어로는 활성 사용자, 새 사용자, 재방문 사용자, 세션, 사용자 경로 그래프, 사용자 맵, 추적 URL, 추세, 활동, 이벤트, 작업, 오류, 추가 정보, 작동 중단, 앱 정보 등이 있습니다.

### 참고 항목
-  [개념 - 용어집][Link 6], [문제 해결 가이드 - 분석][Link 21]
...
## 모니터 - 경고
전자 메일 또는 인스턴트 메시지를 통해 자동으로 전송할 경고에 대한 조건을 지정할 수도 있습니다. Google의 GTalk, Apple의 iChat 등 XMPP 호환 서비스가 지원됩니다. 경고는 초/분/시간당 특정 세션, 작업, 이벤트, 오류 또는 작동 중단 수보다 크거나(>) 작은(<) 미리 정의된 검색 임계값을 기준으로 합니다. 경고는 지정된 유형의 모든 활동을 모니터링할 수도 있고 특정 작업, 이벤트 또는 오류 활동만 모니터링할 수도 있습니다. 경고 트리거 시 x분마다 알림을 두 개 이상 받지 않도록 같은 경고에 대해 알림을 두 개로 구분하여 간격을 유지할 최소 시간인 최소 검색 속도를 지정할 수도 있습니다.
