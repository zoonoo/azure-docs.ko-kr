<properties 
	pageTitle="미디어 앱에 대한 Azure Mobile Engagement 구현"
...
1. **운영 체제** : 각자 푸시 알림을 관리하는 자신만의 규칙이 있기 때문에 John은 모든 케이스를 목록으로 모아서 API가 해당 케이스를 처리하는지 확인하기로 합니다. 예를 들어, Android 푸시 시스템은 전체적인 그림이 가능하지만 iOS의 경우는 그렇지 않습니다.
