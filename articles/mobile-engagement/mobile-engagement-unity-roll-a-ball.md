<properties
	pageTitle="Unity Roll a Ball 자습서"
...
2. **프로젝트 이름** 및 **위치**를 입력하고, **3D**를 선택한 다음 **Create project**를 클릭합니다.
...
6. **Ground** 개체의 **Gizmos 메뉴**에서 **Show Grid**의 선택을 취소합니다.

	![][4]

7. **Ground** 개체의 **Scale** 구성 요소를 [X = 2,Y = 1, Z = 2]로 업데이트합니다.
...
9. **Player** 개체를 선택하고 평면 개체와 마찬가지로 **Reset Transform**을 클릭합니다.

10. Player Y의 **Transform -> Position -> Y Coordinate** 구성 요소를 0.5로 업데이트합니다.

	![][7]

11. 프로젝트에서 **Materials**라는 새 폴더를 만들고 여기서 플레이어에게 색을 입힐 재질을 만들겠습니다.

12. 이 폴더에 **Background**라고 하는 새 **재질**을 만듭니다.
...
17. 마지막으로 선명도를 높이기 위해 Directional Light 개체에서 **Transform -> Rotation -> Y**를 60으로 업데이트합니다.
...
3. **Add Component-> New Script -> C# Script**를 클릭합니다. 이름을 **PlayerController**로 지정하고 **Create and Add**를 클릭합니다. 그러면 스크립트가 생성되어 플레이어 개체에 연결될 것입니다.

	![][14]

5. 이 스크립트를 프로젝트의 **Scripts** 폴더로 이동합니다.

6. 스크립트를 편집하기 위해 선호하는 스크립트 편집기에서 스크립트를 열고, 다음 코드를 사용하여 스크립트 코드를 업데이트하고 저장합니다.
...
8. 위의 스크립트는 **Scripts** 속성을 사용합니다. Unity 편집기에서 Scripts 속성을 10으로 업데이트합니다.

	![][15]

9. Unity 편집기에서 **Play**를 누릅니다. 이제 키보드를 사용하여 공을 회전하고 주변을 이동하면서 제어할 수 있을 것입니다.
...
2. **MainCamera**에 **CameraController**라고 하는 새 스크립트를 추가하고 해당 스크립트를 Scripts 폴더로 이동합니다.

	![][17]

3. 편집을 위해 해당 스크립트를 열고 다음 코드를 추가합니다.
...
1. **Create -> Create Empty -> Game Object**를 만들고 이름을 **벽**으로 지정합니다.

	![][19]

2. 이 벽 개체에서 새 **3D Object -> Cube**를 만들고 이름을 "서쪽 벽"으로 지정합니다.

	![][20]

3. 이 서쪽 벽 개체의 **Transform -> Position** 및 **Transform -> Scale**을 업데이트합니다.
...
7. Unity 편집기에서 **Play**를 누르면 키보드의 방향 키로 Player 개체를 이동하여 이 **Roll a Ball** 게임을 플레이할 수 있습니다.

### 모바일 플레이가 가능하도록 게임 업데이트
