...
1. `Program` 클래스에 다음 두 메서드를 추가합니다. 두 메서드는 REST API를 실행하고 콘솔에 대한 응답을 표시하는 데 사용됩니다.
...
1. `Main` 메서드에 다음 코드를 추가하여 수집한 인증 매개 변수를 사용하는 인증 토큰을 생성합니다.
...
3. 마지막으로, [캠페인 푸시](https://msdn.microsoft.com/library/azure/mt683734.aspx) REST API를 사용하여 목록의 모든 장치 ID에 캠페인을 푸시하겠습니다. 이것은 **앱 내** 알림입니다. 따라서 사용자가 알림을 받으려면 앱이 장치에서 실행 중이어야 합니다.
...
4. 콘솔 앱을 빌드하고 실행합니다. 다음과 유사하게 출력될 것입니다.
