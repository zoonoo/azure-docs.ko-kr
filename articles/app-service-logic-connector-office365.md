...
## 논리 앱 만들기##
논리 앱에서 Office 365 Connector를 사용하려면 커넥터 API 앱을 이미 만든 상태거나 논리 앱에서 즉시 커넥터 API 앱을 만들 수 있습니다. 메일이 수신되면(판매 문의 메일 ID - 예: sales@contoso.com) 트리거되는 간단한 논리 앱을 만듭니다. 그런 후에 이 논리 앱은 이벤트를 만들고 보낸 사람의 세부 정보가 포함된 연락처를 추가하며 개인 계정으로 메일을 보내고 마지막으로 승인이 포함된 회신을 보냅니다.
...
4.	갤러리에서 '새로 만들기'를 확장하여 사용 가능한 모든 API 앱을 표시합니다. 갤러리에서 'Office 365'를 선택하면 'Office 365 Trigger'가 흐름에 추가됩니다.
	![][4]

5.	그러면 논리 앱과 동일한 리소스 그룹에 새 'Office 365 Connector' API 앱이 만들어집니다. API 앱을 만드는 데 30초 정도 걸립니다.

6.	앱이 만들어지고 나면 '권한 부여'를 클릭하여 Office 365 로그인 자격 증명을 입력합니다.
...
11. Office 365 'New Email'(새 메일) 트리거가 구성되고 출력 매개 변수도 표시된 것을 볼 수 있습니다.
...
25. 논리 앱 편집기 화면에서 확인을 클릭한 다음  'Create'를 클릭합니다. 만들기를 완료하는 데 30초 정도 걸립니다.
