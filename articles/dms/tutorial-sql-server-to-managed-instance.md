---
title: '자습서: SQL 서버를 SQL 관리 인스턴스로 마이그레이션'
...
---
# <a name="tutorial-migrate-sql-server-to-an-azure-sql-database-managed-instance-offline-using-dms"></a>자습서: DMS를 사용 하 여 오프라인으로 관리 되는 Azure SQL 데이터베이스 관리 인스턴스에 SQL 서버마이그레이션

Azure Database Migration Service를 사용하여 온-프레미스 SQL Server 인스턴스에서 [Azure SQL Database 관리형 인스턴스](../sql-database/sql-database-managed-instance.md)로 마이그레이션할 수 있습니다. 일부 수동 작업이 필요할 수 있는 추가 메서드는 [Azure SQL Database 관리 인스턴스로 마이그레이션하는](../sql-database/sql-database-managed-instance-migrate.md)문서를 참조하십시오.
...
- 원본 SQL Server와 대상 관리되는 인스턴스를 연결하는 데 사용되는 로그인이 sysadmin 서버 역할의 구성원인지 확인합니다.
    ...
    >[!NOTE]
    >기본적으로 Azure 데이터베이스 마이그레이션 서비스는 SQL 로그인 마이그레이션만 지원합니다. 그러나 다음을 통해 Windows 로그인을 마이그레이션하는 기능을 활성화할 수 있습니다.
    >
    >- 대상 SQL Database 관리 인스턴스에 AAD 읽기 액세스 권한이 있는지 확인하여 **회사 관리자**또는 글로벌 **관리자**" 역할을 가진 사용자가 Azure 포털을 통해 구성할 수 있습니다.
    >- Azure 데이터베이스 마이그레이션 서비스 인스턴스를 구성하여 구성 페이지에서 Azure 포털을 통해 설정된 Windows 사용자/그룹 로그인 마이그레이션을 사용하도록 설정합니다. 이 설정을 사용하도록 설정한 후 변경 사항이 적용될 수 있도록 서비스를 다시 시작합니다.
    >
    > 서비스를 다시 시작하면 Windows 사용자/그룹 로그인이 마이그레이션에 사용할 수 있는 로그인 목록에 나타납니다. 마이그레이션하는 모든 Windows 사용자/그룹 로그인의 경우 연결된 도메인 이름을 제공하라는 메시지가 표시됩니다. 서비스 사용자 계정(도메인 이름 NT AUTHORITY가 있는 계정) 및 가상 사용자 계정(도메인 이름 NT SERVICE가 있는 계정 이름)은 지원되지 않습니다.
...
1. **로그인 선택** 화면에서 마이그레이션할 로그인을 선택합니다.
    ...
    >[!NOTE]
    >기본적으로 Azure 데이터베이스 마이그레이션 서비스는 SQL 로그인 마이그레이션만 지원합니다. Windows 로그인 마이그레이션을 지원하려면 이 자습서의 **필수 구성 조건** 섹션을 참조하세요.
...
1. **마이그레이션 설정 구성** 화면에서 다음 세부 정보를 입력합니다.
    ...
    |**사용자 이름** | Windows 사용자가 위에서 입력한 네트워크 공유에 대한 전체 제어 권한을 갖고 있는지 확인합니다. Azure Database 마이그레이션 서비스는 복원 작업을 위해 백업 파일을 Azure Storage 컨테이너에 업로드하기 위해 사용자 자격 증명을 가장합니다. TDE 가능 데이터베이스를 마이그레이션하기로 선택하는 경우 인증서 파일을 업로드하고 삭제할 수 있도록 위의 Windows 사용자가 기본 제공 관리자 계정이어야 하고, Azure Database Migration Service에 [사용자 계정 컨트롤](https://docs.microsoft.com/windows/security/identity-protection/user-account-control/user-account-control-overview)을 사용하지 않도록 설정해야 합니다. |
    |**암호** | 사용자에 대한 암호입니다. |
    |**스토리지 계정 설정** | Azure 데이터베이스 마이그레이션 서비스에 서비스가 백업 파일을 업로드하고 데이터베이스를 Azure SQL Database 관리 인스턴스로 마이그레이션하는 데 사용되는 저장소 계정 컨테이너에 대한 액세스를 제공하는 SAS URI입니다. [Blob 컨테이너에 대한 SAS URI를 얻는 방법에 대해 알아봅니다.](https://docs.microsoft.com/azure/vs-azure-tools-storage-explorer-blobs#get-the-sas-for-a-blob-container)|
