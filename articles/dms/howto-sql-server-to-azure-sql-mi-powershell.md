...
이 문서에서는 Microsoft Azure PowerShell를 사용 하 여 SQL Server 2005 이상의 온-프레미스 인스턴스로 복원 된 **Adventureworks2016** 데이터베이스를 관리 되는 Azure SQL Database 인스턴스로 마이그레이션합니다. Microsoft Azure PowerShell에서 모듈을 `Az.DataMigration` 사용 하 여 온-프레미스 SQL Server 인스턴스에서 Azure SQL Database 관리 되는 인스턴스로 데이터베이스를 마이그레이션할 수 있습니다.
...
[`New-AzResourceGroup`](https://docs.microsoft.com/powershell/module/az.resources/new-azresourcegroup) 명령을 사용 하 여 리소스 그룹을 만듭니다.
...
* *Azure 리소스 그룹 이름*입니다. 명령을 사용 [`New-AzResourceGroup`](https://docs.microsoft.com/powershell/module/az.resources/new-azresourcegroup) 하 여 이전에 표시 된 대로 Azure 리소스 그룹을 만들고 해당 이름을 매개 변수로 제공할 수 있습니다.
...
* *가상 서브넷 식별자*. Cmdlet [`New-AzVirtualNetworkSubnetConfig`](https://docs.microsoft.com//powershell/module/az.network/new-azvirtualnetworksubnetconfig) 을 사용 하 여 서브넷을 만들 수 있습니다.
...
`New-AzDmsConnInfo` Cmdlet을 사용 하 여 데이터베이스 연결 정보 개체를 만들 수 있습니다. 여기에는 다음 매개 변수가 필요 합니다.
...
* *Trustservercertificate*. 이 매개 변수는 신뢰의 유효성을 검사 하기 위해 인증서 체인을 우회 하는 동안 채널이 암호화 되는지 여부를 나타내는 값을 설정 합니다. 값은 또는 `$false`일 `$true` 수 있습니다.
...
프로젝트 만들기를 위한 `AzDataMigrationDatabaseInfo` 매개 변수로 제공할 수 있는 Azure Database Migration Service 프로젝트의 일부로 데이터베이스를 지정 하는 개체 목록을 만듭니다. Cmdlet `New-AzDataMigrationDatabaseInfo` 을 사용 하 여를 만들 `AzDataMigrationDatabaseInfo`수 있습니다.

다음 예에서는 **AdventureWorks2016** 데이터베이스 `AzDataMigrationDatabaseInfo` 에 대 한 프로젝트를 만들고 프로젝트를 만들기 위한 매개 변수로 제공할 목록에 추가 합니다.
...
마지막으로,를 사용 하 여 `New-AzDataMigrationProject` *미국 동부* 에 있는 *MyDMSProject* 라는 Azure Database Migration Service 프로젝트를 만들고 이전에 만든 원본 및 대상 연결과 마이그레이션할 데이터베이스 목록을 추가할 수 있습니다.
...
이제 `New-AzDmsFileShare` cmdlet을 사용 하 여 원본 데이터베이스 백업을 수행할 수 AZURE DATABASE MIGRATION SERVICE 로컬 SMB 네트워크 공유를 나타내는 파일 공유 개체를 만듭니다.
...
다음 단계는 `New-AzDmsSelectedDB` cmdlet을 사용 하 여 원본 및 대상 데이터베이스를 선택 하는 것입니다.

다음 예는 단일 데이터베이스를 SQL Server에서 관리 되는 Azure SQL Database 인스턴스로 마이그레이션하는 것입니다.
...
전체 SQL Server 인스턴스에서 Azure SQL Database 관리 되는 인스턴스로 리프트 앤 시프트를 수행 해야 하는 경우 원본에서 모든 데이터베이스를 가져오는 루프가 아래에 제공 됩니다. 다음 예에서는 $Server, $SourceUserName 및 $SourcePassword에 대해 소스 SQL Server 세부 정보를 제공 합니다.
...
* *TargetConnection*. 대상 Azure SQL Database Managed Instance 연결을 나타내는 AzDmsConnInfo 개체입니다.
...
또한 `New-AzDataMigrationTask` cmdlet에는 수행 중인 마이그레이션 유형 (오프 라인 또는 온라인)에 고유한 매개 변수가 필요 합니다.
...
* **온라인 마이그레이션**. 온라인 마이그레이션의 경우에도 `New-AzDataMigrationTask` cmdlet에는 다음 매개 변수가 필요 합니다.
...
2. 변수를 `$CheckTask` 사용 하 여 마이그레이션 작업의 현재 상태를 가져옵니다.

    `$CheckTask` 변수를 사용 하 여 마이그레이션 작업의 현재 상태를 가져오려면 다음 예제와 같이 작업의 상태 속성을 쿼리하여 실행 중인 마이그레이션 작업을 모니터링할 수 있습니다.
...
Azure SQL Database 관리 되는 인스턴스의 데이터베이스를 최신 데이터로 업데이트 하 고 원본 데이터베이스와 동기화 하는 경우에는 조치를 수행할 수 있습니다.
