...
5.  데이터베이스 마이그레이션 상태가 **완료됨**으로 표시되면 애플리케이션을 Azure SQL Database Managed Instance의 새 대상 인스턴스에 연결합니다.
