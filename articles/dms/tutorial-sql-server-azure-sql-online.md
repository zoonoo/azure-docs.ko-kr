---
title: '자습서: SQL 서버를 SQL 단일 데이터베이스로 온라인으로 마이그레이션'
...
ms.locfileid: "80298964"
---
# <a name="tutorial-migrate-sql-server-to-a-single-database-or-pooled-database-in-azure-sql-database-online-using-dms"></a>자습서: DMS를 사용 하 여 온라인 Azure SQL 데이터베이스에서 단일 데이터베이스 또는 풀된 데이터베이스로 SQL Server 마이그레이션
...
- 대상 Azure SQL Database 인스턴스에 연결하는 데 사용되는 자격 증명에 대상 Azure SQL Database 인스턴스에 대한 CONTROL 데이터베이스 권한이 있는지 확인합니다.
...
5. 활동 **유형 선택** 섹션에서 **온라인 데이터 마이그레이션을**선택합니다.
...
- **마이그레이션 실행**을 선택합니다.
    ...
    마이그레이션 활동 창이 나타나고 활동의 **상태가** **초기화되고**있습니다.
