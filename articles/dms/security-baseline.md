---
title: Azure Database Migration Service에 대 한 Azure 보안 기준
description: Azure Database Migration Service 보안 기준은 Azure 보안 벤치 마크에 지정 된 보안 권장 사항을 구현 하기 위한 절차 지침과 리소스를 제공 합니다.
author: msmbaldwin
ms.service: dms
ms.topic: conceptual
ms.date: 12/08/2020
...
ms.locfileid: "98202490"
---
# <a name="azure-security-baseline-for-azure-database-migration-service"></a>Azure Database Migration Service에 대 한 Azure 보안 기준

이 보안 기준은 [Azure Security 벤치 마크 버전 2.0](../security/benchmarks/overview.md) 의 지침을 Azure Database Migration Service 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다. 콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤** 및 Azure Database Migration Service에 적용 되는 관련 지침에 따라 그룹화 됩니다. Azure Database Migration Service에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

Azure Database Migration Service 완전히 Azure 보안 벤치 마크에 매핑되는 방법을 보려면 [전체 Azure Database Migration Service 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
**지침**: Azure Database Migration Service 리소스를 배포할 때 기존 가상 네트워크를 만들거나 사용 해야 합니다. 모든 Azure virtual network가 비즈니스 위험에 맞는 엔터프라이즈 구분 원칙을 따르는지 확인 합니다. 조직에서 더 높은 위험을 초래할 수 있는 모든 시스템은 해당 가상 네트워크 내에서 격리 되 고 NSG (네트워크 보안 그룹) 및/또는 Azure 방화벽으로 충분히 안전 하 게 보호 되어야 합니다.

Azure Database Migration Service은 기본적으로 TLS 1.2을 사용 합니다. 마이그레이션되는 데이터 원본에 대 한 이전 버전과의 호환성을 위해 필요한 경우 Azure Database Migration Service의 서비스 구성 블레이드에서 TLS 1.0 또는 TLS 1.1에 대 한 지원을 사용 하도록 설정할 수 있습니다.
...
- [Azure Database Migration Service에 대 한 포트가 열려 있는 필수 구성 요소 설명서를 참조 하세요.](pre-reqs.md#prerequisites-common-across-migration-scenarios)
...
**지침**: 마이그레이션 사용 사례에 네트워크 트래픽이 교차 하는 경우 azure express 경로 또는 azure VPN (가상 사설망)을 사용 하 여 공동 배치 환경의 온-프레미스 인프라와 azure 데이터 센터 간에 개인 연결을 만들 수 있습니다. Express 경로 연결은 공용 인터넷을 통해 이동 하지 않으며 일반적인 인터넷 연결 보다 안정적이 고 속도가 빠르며 대기 시간이 짧습니다. 지점 및 사이트 간 VPN 및 사이트 간 VPN의 경우 이러한 VPN 옵션과 Azure Express 경로를 조합 하 여 온-프레미스 장치 또는 네트워크를 가상 네트워크에 연결할 수 있습니다. Azure에서 두 개 이상의 가상 네트워크를 연결 하려면 가상 네트워크 피어 링을 사용 합니다. 피어 링 가상 네트워크 간의 네트워크 트래픽은 비공개 이며 Azure 백본 네트워크에 유지 됩니다.
...
**지침**: 해당 하는 경우 Azure 개인 링크를 사용 하 여 원본 및 대상 서비스 (예: azure SQL Server 또는 마이그레이션 중에 필요한 기타 서비스)에 대 한 개인 액세스를 사용 하도록 설정 합니다.  Azure 개인 링크를 아직 사용할 수 없는 경우 Azure Virtual Network 서비스 끝점을 사용 합니다.  Azure 개인 링크 및 서비스 끝점은 인터넷을 거치지 않고 Azure 백본 네트워크를 통해 최적화 된 경로를 통해 서비스에 대 한 보안 액세스를 제공 합니다.  

또한 허용 해야 하는 통신 포트를 포함 하 여 가상 네트워크에 Azure Database Migration Service를 프로 비전 하기 전에 필수 구성 요소가 충족 되는지 확인 합니다. 
...
**지침**: Azure Database Migration Service는 기본 id 및 액세스 관리 서비스로 Azure Active Directory (Azure AD)를 사용 합니다. Azure Portal, Azure Storage, Azure 가상 머신 (Linux 및 Windows), Azure Key Vault, PaaS 및 SaaS 응용 프로그램과 같은 Microsoft 클라우드 리소스에서 조직의 id 및 액세스 관리를 관리 하기 위해 Azure AD를 표준화 해야 합니다.
...
**지침**: Azure Database Migration service를 사용 하려면 사용자가 ' 온라인 ' 모드로의 Azure SQL Database Managed Instance 마이그레이션을 위해 Azure Active Directory (azure AD)에서 응용 프로그램 ID (서비스 주체) 및 인증 키를 만들어야 합니다. 이 응용 프로그램 ID를 사용 하려면 구독 수준의 참가자 역할 (과도 한 액세스 권한 참여자 역할이 부여 된 것으로 인해 권장 되지 않음) 또는 Azure Database migration Service에서 요구 하는 특정 권한으로 사용자 지정 역할을 만들어야 합니다.

마이그레이션이 완료 되 면이 응용 프로그램 ID를 제거 하는 것이 좋습니다.

- [온라인 마이그레이션 모드에서 응용 프로그램 ID를 사용 하는 방법](tutorial-sql-server-managed-instance-online.md)

- [Azure SQL Managed Instance 온라인 마이그레이션에 SQL Server 하기 위한 사용자 지정 역할](resource-custom-roles-sql-db-managed-instance.md)
...
**지침**: Azure Database Migration Service에서는 다단계 인증을 통해 강력한 인증 제어를 지 원하는 Azure Active Directory를 사용 하 고 강력한 암호 없는 메서드를 사용 합니다.
...
관리자 및 권한 있는 사용자의 경우 가장 강력한 수준의 인증을 사용 하 고 그 뒤에 적절 한 강력한 인증 정책을 다른 사용자에 게 배포 해야 합니다.
...
**지침**: Azure Database Migration Service은 로그인-관리 되는 응용 프로그램 및 사용자 로그인 활동 사용에 대 한 정보를 제공 하는 로그인 보고서를 제공 하는의 Azure Active Directory와 통합 되어 있습니다.
...
**지침**: Azure Database Migration Service는 사용자 정의 조건에 따라 보다 세부적인 액세스 제어를 위해 조건부 액세스를 사용할 수 있는 인증에 Azure AD를 사용 합니다. 예를 들어 특정 IP 범위의 사용자 로그인은 로그인에 다단계 인증을 사용 해야 합니다. 다양한 사용 사례에 대해 세부적인 인증 세션 관리 정책을 사용할 수도 있습니다.
...
**지침** Azure Database Migration Service: 고객은 id/secretes를 사용 하 여 코드 또는 구성 또는 지속형 데이터를 배포할 수 있도록 하 고, 코드 또는 구성 또는 지속형 데이터 내의 자격 증명을 식별 하는 자격 증명 스캐너를 구현 하는 것이 좋습니다. 또한 자격 증명 스캐너는 검색된 자격 증명을 더 안전한 위치(예: Azure Key Vault)로 이동하도록 추천합니다.

GitHub를 사용 하는 경우 네이티브 암호 검색 기능을 사용 하 여 코드 내에서 자격 증명 또는 다른 형식의 암호를 식별할 수 있습니다.
...
**지침**: Azure Database Migration service를 사용 하려면 사용자가 ' 온라인 ' 모드로의 Azure SQL Database Managed Instance 마이그레이션을 위해 Azure Active Directory (azure AD)에서 응용 프로그램 ID (서비스 주체) 및 인증 키를 만들어야 합니다. 마이그레이션이 완료 되 면이 응용 프로그램 ID를 제거 하는 것이 좋습니다.
...
**지침**: Azure AD 조직에서 실수로 잠기는 것을 방지 하는 일반적인 방법으로 Azure Database Migration Service 외부에서 일반 관리 계정을 사용할 수 없는 경우 액세스를 위한 응급 액세스 계정을 설정 합니다. 응급 액세스 계정은 일반적으로 권한이 높으며 특정 사용자에게 할당되면 안 됩니다. 응급 액세스 계정은 일반 관리 계정을 사용할 수 없는 '비상' 시나리오의 긴급한 상황으로 제한됩니다.
...
**지침**: Azure Database Migration Service는 Azure Active Directory와 통합 되어 해당 리소스를 관리 합니다. Azure AD 자격 관리 기능을 사용 하 여 액세스 권한 부여, 검토, 만료 등의 액세스 요청 워크플로를 자동화할 수 있습니다. 이중 또는 다단계 승인도 지원 됩니다.
...
- [권한 있는 액세스 워크스테이션 이해](https://4sysops.com/archives/understand-the-microsoft-privileged-access-workstation-paw-security-model/.md) 
...
**지침**: Azure Database Migration Service은 기본적으로 TLS 1.2 이상을 사용 하 여 고객이 구성한 원본에서 데이터베이스 마이그레이션 서비스 인스턴스로 전송 중인 데이터를 암호화 합니다. 원본 서버에서 TLS 1.2 연결을 지원 하지 않는 경우에는이 작업을 수행 하지 않는 것이 좋지만이를 사용 하지 않도록 선택할 수 있습니다. 데이터베이스 마이그레이션 서비스 인스턴스에서 대상 인스턴스로 데이터를 전송 하는 것은 항상 암호화 됩니다.  

Azure Database Migration Service 외부에서 액세스 제어를 사용할 수 있으며, 전송 중인 데이터를 암호화를 사용 하 여 ' 대역 외 ' 공격 (예: 트래픽 캡처) 으로부터 보호 하 여 공격자가 데이터를 쉽게 읽거나 수정할 수 없도록 해야 합니다. Azure 리소스에 연결 하는 모든 클라이언트가 TLS v 1.2 이상을 협상할 수 있도록 HTTP 트래픽을 확인 합니다.
...
Azure Database Migration Service은 해당 리소스에 대해 응용 프로그램 또는 소프트웨어 설치를 실행 하는 것을 허용 하지 않습니다. 
...
**지침**: 보안 분석을 위한 nsg (네트워크 보안 그룹) 리소스 로그, nsg 흐름 로그, Azure 방화벽 로그 및 Waf (웹 응용 프로그램 방화벽) 로그를 사용 하도록 설정 하 고 수집 하 여 인시던트 조사, 위협 구하기 및 보안 경고 생성을 지원 합니다. 흐름 로그를 Azure Monitor Log Analytics 작업 영역으로 보낸 다음 트래픽 분석를 사용 하 여 정보를 제공할 수 있습니다.

Azure Database Migration Service는 사용 하도록 설정 해야 하는 DNS 쿼리 로그를 생성 하거나 처리 하지 않습니다.
