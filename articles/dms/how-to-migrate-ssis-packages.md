...
* SSIS는 [Azure Data Factory에서 Azure-SSIS Integration Runtime 프로 비전](../data-factory/tutorial-deploy-ssis-packages-azure.md)문서에 설명 된 대로 SQL Database에서 호스트 하는 대상 SSISDB를 사용 하 여 AZURE-SSIS INTEGRATION RUNTIME (IR)를 포함 하는 AZURE DATA FACTORY (ADF)에서 프로 비전 되어야 합니다.
...
14. 배포 프로세스가 완료된 후에 각 배포 작업의 성공 여부를 표시하는 결과 페이지를 볼 수 있습니다.
    ...
    b. 필요에 따라 **보고서 저장** 을 선택 하 여 결과를 XML 파일에 저장 합니다.
