...
대금 청구는 해당 월에 인증을 수행했는지 여부에 관계없이 Multi-Factor Authentication을 사용하도록 구성된 사용자 수를 기준으로 합니다. 

**Q:Multi-Factor Authentication은 어떤 방식으로 청구됩니까?**

Azure Multi-Factor Authentication을 독립 실행형 서비스로 구입한 경우(사용자 단위 또는 인증 단위 MFA 공급자를 만들어) 조직의 Azure 구독은 사용량에 따라 매달 청구됩니다. 이 청구 모델은 가상 컴퓨터 및 웹 사이트의 사용량에 대해 Azure에서 청구하는 방식과 유사합니다.

Azure Multi-Factor Authentication에 대한 구독을 구입하면(사용자당 연간 라이선스로나 Office 365, Azure AD Premium, Enterprise Mobility + Security 번들의 일부로) 조직은 각 사용자에 대한 연간 라이선스 비용만 지불합니다.

[Azure Multi-factor Authentication 획득 방법](multi-factor-authentication-versions-plans.md)에서 옵션에 대해 자세히 알아보세요.
...
하지만 MFA 공급자가 Azure AD 테넌트에 연결되지 않거나 새 MFA 공급자를 다른 Azure AD 테넌트에 연결한 경우 사용자 설정 및 구성 옵션이 새 MFA 공급자로 전송되지 않습니다. 또한 새 MFA 공급자를 통해 생성된 활성화 자격 증명을 사용하여 기존 Azure MFA 서버를 다시 활성화해야 합니다. MFA 서버를 새 MFA 공급자에 연결하기 위해 다시 활성화해도 전화 및 문자 메시지 인증에는 영향을 미치지 않지만 모바일 앱을 다시 활성화할 때까지 모바일 앱 알림이 모든 사용자에 대해 작동 중지됩니다.
...
디렉터리에 *인증 단위* Azure Multi-Factor Authentication 공급자가 있는 경우 MFA 공급자가 구독에 연결되기만 한다면 항상 인증마다 요금이 청구됩니다. 사용자에게 MFA 라이선스를 할당할 수 있지만 사용자의 MFA 라이선스 할당 여부에 관계없이 2단계 인증 요청마다 요금이 계속 청구됩니다. 
...
사용자에게 [최종 사용자 문제 해결 가이드](./end-user/multi-factor-authentication-end-user-troubleshoot.md)를 안내해줄 수 있습니다.


**Q: 내 사용자 중 하나가 계정에 로그인할 수 없는 경우 어떻게 해야 하나요?**

등록 프로세스를 다시 진행하도록 하여 사용자 계정을 재설정할 수 있습니다. [클라우드에서 Azure Multi-Factor Authentication을 사용하여 사용자 및 장치 설정 관리](multi-factor-authentication-manage-users-and-devices.md)에 대해 자세히 알아보세요.

**Q: 내 사용자 중 하나가 앱 암호를 사용하는 전화기를 분실한 경우 어떻게 해야 하나요?**

무단 액세스를 방지하려면 모든 사용자 앱 암호를 삭제합니다. 사용자에게 교체용 장치가 있는 경우 암호를 다시 만들 수 있습니다. [클라우드에서 Azure Multi-Factor Authentication을 사용하여 사용자 및 장치 설정 관리](multi-factor-authentication-manage-users-and-devices.md)에 대해 자세히 알아보세요.

**Q: 사용자가 비브라우저 앱에 로그인할 수 없으면 어떻게 하나요?**

조직에서 레거시 클라이언트를 계속 사용하고 [앱 암호 사용을 허용](multi-factor-authentication-whats-next.md#app-passwords)한 경우 사용자는 자신의 사용자 이름 및 암호로 이러한 레거시 클라이언트에 로그인할 수 없습니다. 대신, [앱 암호를 설정](./end-user/multi-factor-authentication-end-user-app-passwords.md)해야 합니다. 사용자는 로그인 정보를 지우고(삭제)하고 앱을 다시 시작한 후 일반 암호 대신 *앱 암호*와 사용자 이름을 사용하여 로그인해야 합니다. 
...
일부 경우에 가능합니다. Azure MFA 서버 7.0 이상에서 양방향 문자 메시지에 대한 시간 제한 설정을 구성할 수 있습니다. 

Azure MFA 서버는 기본적으로 300초(5분) 동안 일회용 암호를 저장합니다. 300초가 경과한 후 사용자가 이 코드를 입력하면 인증이 거부됩니다. 레지스트리 키를 설정하여 시간 제한을 조정할 수 있습니다. 
...
단방향 문자 메시지의 경우 MFA 서버는 300초 동안 일회용 암호를 저장하고 Azure AD에서 클라우드 기반 MFA는 180초 동안 암호를 저장합니다. 이 설정은 구성할 수 없습니다. 
...
비밀 키를 CSV 파일에 추가하고 Azure Multi-factor Authentication 서버에 가져올 수 있는 경우 OATH TOTP 토큰에 해당하는 ActiveIdentity 토큰을 사용할 수 있습니다. OATH 토큰은 ADFS(Active Directory 페더레이션 서비스), IIS(인터넷 정보 서버) 폼 기반 인증, RADIUS(원격 인증 전화 접속 사용자 서비스)(클라이언트 시스템에서 액세스 챌린지 응답을 처리할 수 있는 경우)에서 사용할 수 있습니다.
...
2단계 인증이 필요한 계정에서 작동하지 않는 로컬 컴퓨터에 설치된 비브라우저 응용 프로그램에 로그인할 때 0x800434D4L 오류가 발생합니다. 
