...
여기에서 Azure Multi-Factor Authentication에 대한 질문과 대답을 찾을 수 있습니다. 여기에는 팁 또는 문제 해결 정보가 포함될 수 있습니다. 오른쪽 탐색 메뉴를 사용하여 질문을 빠르게 찾을 수 있습니다.

## Azure Multi-Factor Authentication에 대한 도움을 얻으려면 어떻게 하나요?
...
- 커뮤니티에서 기술 질문 및 대답을 검색하고 찾아보거나 [여기](https://social.msdn.microsoft.com/Forums/azure/newthread?category=windowsazureplatform&forum=WindowsAzureAD&prof=required)를 클릭하여 직접 질문을 할 수 있습니다.

[암호 재설정](mailto:phonefactorsupport@microsoft.com)

- 암호 재설정과 관련된 질문이 있거나 도움이 필요한 경우 [phonefactorsupport@microsoft.com](mailto:phonefactorsupport@microsoft.com)으로 메일을 보내세요.
...
## 모바일 응용 프로그램 알림을 사용하여 인증할 때 "Authentication request is not for an activated account(활성화된 계정에 대한 인증 요청이 아닙니다.)”가 표시될 경우 어떻게 해야 하나요?
...
## 휴대폰에서 응답이 수신되지 않거나 휴대폰을 잃어버린 경우 어떻게 해야 하나요?
...
## 비브라우저 응용 프로그램으로 로그인을 시도할 때 0x800434D4L 오류가 표시되면 어떻게 해야 하나요?
...
이 문제를 해결하려면 관리 관련 작업용 사용자 계정과 비관리 관련 작업용 사용자 계정을 따로 두어야 합니다. 나중에 관리 계정과 비관리 계정 간의 사서함을 연결하면 비관리 계정을 사용하여 Outlook에 로그인할 수 있습니다. 이에 대한 자세한 내용은 [관리자에게 사용자 사서함의 내용을 열고 보는 기능 제공](http://help.outlook.com/141/gg709759(d=loband).aspx?sl=1))을 참조하세요.




## 관리자 역할에서 사용자를 제거했으나 다단계 인증을 사용하지 않도록 설정하는 것을 잊어서 목록에 표시되지 않습니다. 이 기능을 제거하려면 어떻게 해야 하나요?
...
## 사용자가 잠긴 계정을 사용하여 관리자에게 연락할 경우 수행할 작업
...
## 앱 암호를 사용하는 장치를 분실했거나 도난당한 경우 수행할 작업
...
## 사용자가 비브라우저 앱에 로그인할 수 없는 경우 어떻게 하나요?
...
## 문자 메시지가 수신되지 않거나 양방향 문자 메시지에 응답했으나 확인 시간이 초과되면 어떻게 해야 하나요?
...
## 사용자를 인증하는 데 사용되는 전화 통화 또는 문자 메시지에 대한 요금이 조직에 부과되나요?
...
## 조직이 Azure Multi-Factor Authentication에 대금을 청구하는 방식
...
## ‘사용자 기준' 청구 모델은 Multi-Factor Authentication이 설정된 사용자 수에 따라 요금이 청구되나요 아니면 확인을 수행하는 사용자의 수에 따라 요금이 청구되나요?
