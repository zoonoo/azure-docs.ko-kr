...
이 문서에서는 2단계 인증에서 발생할 수 있는 몇 가지 문제에 대해 설명합니다. 발생한 문제가 여기에 포함되지 않은 경우 개선할 수 있도록 의견 섹션에 자세한 피드백을 제공해 주세요.
...
다른 장치에서 보조 전화 번호 또는 Authenticator 앱을 포함하여 여러 인증 옵션을 설정한 경우 로그인하려면 다음 중 하나를 사용할 수 있습니다.
...
보조 인증 전화 번호를 구성하지 않은 경우 관리자에 게 문의해야 합니다. 다음 사용자가 로그인하도록 설정을 지운 경우 다시 [계정을 설정](multi-factor-authentication-end-user-first-time.md)하라는 메시지가 표시됩니다.
...
먼저 앱 암호를 올바르게 입력했는지 확인합니다.  계속 작동하지 않으면 로그인을 시도하고 [새 앱 암호를 생성](multi-factor-authentication-end-user-app-passwords.md)합니다.  계속 작동하지 않으면 관리자에게 [기존 앱 암호를 삭제](../multi-factor-authentication-manage-users-and-devices.md) 하도록 문의한 다음 새 암호를 만들 수 있습니다.
