...
다음 정보에서는 휴대폰 또는 사무실 전화 통화를 사용한&2;단계 확인 환경에 대해 설명합니다.
...
다음 정보에서는 휴대폰으로 문자 메시지를 발송하는&2;단계 확인 환경에 대해 설명합니다.
...
Microsoft Authenticator 앱을 사용하여 확인 코드를 가져오는 경우 앱을 열면 계정 이름 아래에 숫자가 표시됩니다. 이 숫자는 같은 숫자를 두 번 사용하는 일이 없도록 30초마다 변경됩니다. 확인 코드를 묻는 메시지가 표시되면 앱을 열고 현재 표시되는 숫자를 사용합니다. 
