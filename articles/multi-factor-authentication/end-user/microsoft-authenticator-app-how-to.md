...
Microsoft Authenticator 앱은 Azure 계정(예: bsimon@contoso.onmicrosoft.com),), 온-프레미스 회사 계정(예: bsimon@contoso.com),) 또는 Microsoft 계정(예: bsimon@outlook.com))에 추가 보안 수준을 제공합니다.
...
### <a name="add-an-account-to-the-app-by-using-the-qr-code-scanner"></a>QR 코드 스캐너를 사용하여 앱에 계정 추가
...
3. Microsoft Authenticator 앱을 엽니다. **계정** 화면에서 **+**를 선택한 다음 회사 또는 학교 계정을 추가할지 지정합니다.
    ...
    ![더하기 기호가 있는 계정 화면](./media/authenticator-app-how-to/addaccount3.png)

    ![회사 또는 학교 계정을 지정하기 위한 화면](./media/authenticator-app-how-to/scan.png)
...
5. 계정이 활성화되는 동안 기다립니다. 활성화가 완료되면 **연락처**를 선택합니다.  이렇게 하면 휴대폰으로 알림 또는 확인 코드가 전송됩니다.  **확인**을 선택합니다.
    ...
    ![로그인에 대해 확인을 선택하는 화면](./media/authenticator-app-how-to/verify.png)
...
5. 앱에서 해당 상자에 코드와 URL을 입력합니다.
    ...
    ![코드와 URL을 입력하기 위한 화면](./media/authenticator-app-how-to/addaccount2.png)
...
### <a name="add-an-account-to-the-app-by-using-touch-id"></a>Touch ID를 사용하여 앱에 계정 추가
...
## <a name="uninstall-the-old-azure-authentication-app"></a>이전 Azure 인증 앱 제거
...
![삭제 단추](./media/authenticator-app-how-to/remove.png)
