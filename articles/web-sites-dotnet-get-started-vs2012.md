...
1.  [개발 환경 설정][개발 환경 설정]
2.  [Azure에서 웹 사이트 만들기][Azure에서 웹 사이트 만들기]
3.  [ASP.NET MVC 4 응용 프로그램 만들기][ASP.NET MVC 4 응용 프로그램 만들기]
...
1.  [Azure 관리 포털][Azure 관리 포털]에서 **웹 사이트**를 클릭한 다음 **새로 만들기**를 클릭합니다.
...
3.  **지역** 드롭다운 목록에서 가장 가까운 지역을 선택합니다.
    이 설정은 웹 사이트가 실행되는 데이터 센터를 지정합니다.
...
4.  **웹 사이트 만들기** 화살표를 클릭합니다.
    ...
    ![관리 포털의 웹 사이트 페이지, 만들어진 웹 사이트][관리 포털의 웹 사이트 페이지, 만들어진 웹 사이트]

## <a name="createmvc4app"></a><span class="short-header">앱 만들기</span>ASP.NET MVC 4 응용 프로그램 만들기
...
## <a name="deploytowindowsazure"></a><span class="short-header">앱 배포</span>Azure에 응용 프로그램 배포
...
1.  이전에 Visual Studio에 Azure 구독을 추가하지 않은 경우 다음 단계를 수행하세요. 이들 단계에서 구독을 추가하여 **Azure 웹 사이트에서 가져오기** 아래의 드롭다운 목록에 웹 사이트를 포함합니다.

    -   **게시 프로필 가져오기** 대화 상자에서 **Azure 웹 사이트에서 가져오기**를 클릭한 후 **Add Azure subscription**를 클릭합니다.

    ![Azure 구독 추가][Azure 구독 추가]

    -   **Import Azure Subscriptions** 대화 상자에서 **구독 파일 다운로드**를 클릭합니다.

    ![구독 파일 다운로드][구독 파일 다운로드]

    -   브라우저 창에서 *.publishsettings* 파일을 저장합니다.

    ![.publishsettings 파일 다운로드][.publishsettings 파일 다운로드]
...
4.  연결 유효성이 검사되면 **연결 유효성 검사** 단추 옆에 녹색 확인 표시가 나타납니다. **다음**을 클릭합니다.
    ...
    ![유효성이 검사된 연결][유효성이 검사된 연결]

5.  이 응용 프로그램에 데이터베이스를 사용하지 않으므로 **설정** 탭에서 **Use this connection string at runtime** 옵션의 확인란을 선택 취소합니다. 이 페이지에서 남은 항목에 대해서는 기본 설정을 적용할 수 있습니다. 릴리스 빌드 구성을 배포 중이며 대상 서버의 파일을 삭제할 필요가 없는 경우 응용 프로그램을 미리 컴파일하거나 App\_Data 폴더의 파일을 제외합니다.
    **다음**을 클릭합니다.
...
6.  **미리 보기** 탭에서 **미리 보기 시작**을 클릭합니다.
    ...
    ![StartPreview 파일 출력][StartPreview 파일 출력]

7.  **게시**를 클릭합니다. Visual Studio에서 Azure 서버로 파일을 복사하는 프로세스를 시작합니다.

8.  **출력** 창에 수행된 배포 작업이 표시되고 성공적인 배포 완료가 보고됩니다.

    ![성공적인 배포를 보고하는 출력 창][성공적인 배포를 보고하는 출력 창]

9.  배포에 성공하면 기본 브라우저에서 배포된 웹 사이트의 URL이 자동으로 열립니다.
    만든 응용 프로그램이 이제 클라우드에서 실행되고 있습니다.

    ![Azure에서 실행되는 웹 사이트][웹 사이트 예]
...
![서버 탐색기의 Azure 웹 사이트][서버 탐색기의 Azure 웹 사이트]

![Visual Studio의 웹 사이트 구성][Visual Studio의 웹 사이트 구성]

웹 사이트를 삭제하려면 Azure 관리 포털을 사용하면 됩니다. 다음 스크린샷은 관리 포털의 **대시보드** 탭에 있는 **중지**, **다시 시작** 및 **삭제** 단추를 보여 줍니다.

![관리 포털 대시보드 탭][관리 포털 대시보드 탭]

**구성** 탭에서 사이트 설정을 변경할 수 있습니다. 자세한 내용은 [웹 사이트를 관리하는 방법][웹 사이트를 관리하는 방법]을 참조하세요.

### 웹 사이트를 크기 조정하는 방법

사이트가 공용이고 트래픽이 증가하기 시작하면 응답 시간이 느려질 수 있습니다. 이 문제를 해결하기 위해 관리 포털의 **크기 조정** 탭에서 서버 리소스를 쉽게 추가할 수 있습니다.

![관리 포털 크기 조정 탭][관리 포털 크기 조정 탭]

자세한 내용은 [웹 사이트 크기를 조정하는 방법][웹 사이트 크기를 조정하는 방법]을 참조하세요. 웹 사이트 크기를 조정하기 위해 서버 리소스를 추가하는 것은 무료가 아닙니다.

### 웹 사이트 문제를 해결하는 방법

문제 해결을 위해 추적 또는 로그 출력을 확인할 수도 있습니다. Visual Studio는 Azure 로그가 생성될 때 실시간으로 보기 쉽도록 기본 제공 도구를 제공합니다.

![Visual Studio의 로그][Visual Studio의 로그]

자세한 내용은 [Visual Studio에서 Azure 웹 사이트 문제 해결][Visual Studio에서 Azure 웹 사이트 문제 해결]을 참조하세요.

### 데이터베이스 및 권한 부여 기능을 추가하는 방법

대부분의 프로덕션 웹 사이트는 데이터베이스를 사용하고 일부 사이트 기능을 권한 있는 특정 사용자로 제한합니다. 데이터베이스 액세스, 인증 및 권한 부여를 시작하는 방법을 보여 주는 자습서는 [Azure 웹 사이트에 멤버 자격, OAuth 및 SQL 데이터베이스가 포함된 보안 ASP.NET MVC 앱 배포][Azure 웹 사이트에 멤버 자격, OAuth 및 SQL 데이터베이스가 포함된 보안 ASP.NET MVC 앱 배포]를 참조하세요.
