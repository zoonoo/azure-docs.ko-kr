...
**AssetFile** 인스턴스 및 실제 미디어 파일은 별개의 두 개체입니다. 미디어 파일에 실제 미디어 콘텐츠가 포함되어 있지만 AssetFile 인스턴스는 해당 미디어 파일에 대한 메타데이터를 포함합니다.
...
지원되는 인코더에 대한 자세한 내용은 [인코더]를 참조하세요(../media-services-encode-asset).
...
라이브 인코더 또는 트랜스코더는 카메라에서 스트리밍되는 오디오 및/또는 비디오를 다중 비트 전송률 RTMP 또는 부드러운 스트리밍 형식으로 변환합니다. 그런 다음 트랜스코더는 적응 비트 전송률 RTMP 또는 부드러운 스트림을 미디어 서비스 채널로 푸시합니다. 그러면 미디어 서비스에서 이벤트를 라이브로 브로드캐스트합니다.
...
Microsoft Azure 미디어 서비스를 사용하면 128비트 암호화 키를 사용하는 AES(Advanced Encryption Standard) 및 PlayReady DRM으로 동적으로 암호화된 콘텐츠를 배달할 수 있습니다. 
...
동적 패키징 및/또는 동적 암호화를 사용하려면 스트리밍하려는 끝점에 대한 스트리밍 단위가 하나 이상 있어야 합니다. 자세한 내용은 [스트리밍 단위 크기 조정]을 참조하세요(../media-services-manage-origins#scale_streaming_endpoints).
...
저장소에서 암호화된 자산이 있는 경우 자산을 스트리밍하기 전에 스트리밍 서버에서 저장소 암호화를 제거하고 지정된 배달 정책을 사용하여 콘텐츠를 스트리밍합니다. 예를 들어 AES(Advanced Encryption Standard) 암호화 키로 암호화된 자산을 배달하려면 정책 유형을 DynamicEnvelopeEncryption으로 설정합니다. 저장소 암호화를 제거하고 암호화되지 않은 자산을 스트리밍하려면 정책 유형을 NoDynamicEncryption으로 설정합니다. 
