...
>[WACOM.NOTE] 이 자습서에서는 사용자 지정 자격 증명을 사용해서 모바일 서비스를 인증하는 ���급 방법을 보여 줍니다. 많은 앱에서는 기본 제공되는 소셜 ID 공급자를 대신 사용하여 Facebook, Twitter, Google, Microsoft 계정 및 Azure Active Directory를 통해 로그인할 수 있도록 허용하는 것이 가장 적합할 것입니다. 모바일 서비스에서 인증 관련 작업을 처음 수행할 경우에는 [사용자 시작] 자습서를 참조하세요.
...
1. `LoginProvider`에서 파생된 `CustomLoginProvider`라는 새 클래스를 만듭니다.
...
2. `CustomLoginProviderCredentials`라는 새 클래스를 만듭니다. 이 클래스는 사용자에 대한 정보를 나타내며 `ServiceUser.getIdentitiesAsync()`를 통해 백 엔드에서 사용자에게 제공됩니다. 사용자 지정 클레임을 추가하는 경우 이 개체에 캡처되었는지 확인합니다.
