...
> [WACOM.NOTE] 이 항목은 이 기능에 대한 [이 Azure 블로그 게시물][이 Azure 블로그 게시물]에서 좀 더 자세히 확장되었으며 추가 기능이 나오면 최신 상태로 업데이트될 것입니다.

<!--Table of contents for topic, the words in brackets must match the heading wording exactly-->

항목 내용

-   [Azure 가상 컴퓨터에 사용자 지정 데이터 삽입][Azure 가상 컴퓨터에 사용자 지정 데이터 삽입]

-   [가상 컴퓨터에서 사용자 지정 데이터 사용][가상 컴퓨터에서 사용자 지정 데이터 사용]

-   [다음 단계][다음 단계]

## <span id="injectingCustomData"></span></a>Azure 가상 컴퓨터에 사용자 지정 데이터 삽입

이 기능은 현재 [Microsoft Azure 플랫폼 간 명령줄 인터페이스][Microsoft Azure 플랫폼 간 명령줄 인터페이스]에서만 지원됩니다. `azure vm create` 명령에 대한 옵션 중 어떤 것도 사용할 수 있지만 다음 접근 방법에서는 기본적인 한 가지 방법만 보여 줍니다.
...
## <span id="usingCustomData"></span></a>가상 컴퓨터에서 사용자 지정 데이터 사용
...
## <span id="nextsteps"></span></a>다음 단계: cloud-init 사용

Azure 가상 컴퓨터가 Ubuntu 이미지이면 cloud-init를 사용하여 사용자 지정 데이터를 자동으로 사용하는 스크립트를 실행할 수 있습니다(또는 사용자 지정 데이터 파일이 스크립트인 경우 해당 스크립트 실행). 자세한 내용은 [Ubuntu에 대한 cloud-init 설명서][Ubuntu에 대한 cloud-init 설명서](영문)를 참조하세요.

<!--Link references-->

[Add Role 서비스 관리 REST API 참조][Add Role 서비스 관리 REST API 참조]

[Microsoft Azure 플랫폼 간 명령줄 인터페이스(영문)][Microsoft Azure 플랫폼 간 명령줄 인터페이스]

  [이 Azure 블로그 게시물]: http://azure.microsoft.com/blog/2014/04/21/custom-data-and-cloud-init-on-windows-azure/
  [Azure 가상 컴퓨터에 사용자 지정 데이터 삽입]: #injectingCustomData
  [가상 컴퓨터에서 사용자 지정 데이터 사용]: #usingCustomData
  [다음 단계]: #nextsteps
  [Microsoft Azure 플랫폼 간 명령줄 인터페이스]: https://github.com/Azure/azure-sdk-tools-xplat
  [Ubuntu에 대한 cloud-init 설명서]: https://help.ubuntu.com/community/CloudInit
