...
Windows용 사용자 지정 스크립트 확장을 사용하면 원격 가상 컴퓨터에 로그인하지 않고도 해당 가상 컴퓨터에서 PowerShell 스크립트를 실행할 수 있습니다. 스크립트는 VM을 프로비저닝한 이후에 실행할 수도 있고 VM 수명 주기 중에 언제든지 실행할 수 있으며 VM에서 추가 포트를 열 필요가 없습니다. 사용자 지정 스크립트는 VM 프로비전 후 작업에서 추가 소프트웨어를 실행, 설치 및 구성할 때 가장 흔히 사용됩니다.
...
사용자 지정 스크립트 확장을 템플릿에 사용하는 방법을 확인하려면 [여기] 문서를 클릭하세요(virtual-machines-extensions-customscript -with template.md).
