...
3. **Product Name**, **Organization Name** 및 **Organization Identifier**를 입력합니다. 언어에서 **Swift**를 선택했는지 확인합니다.
...
7. AzME Objective-C 코드를 Swift 코드에 노출하도록 Bridging 헤더 파일을 편집하고 다음 import를 추가합니다.
...
- `ViewController.h` 파일을 열고 `EngagementViewController.h`를 가져온 다음 `ViewController` 인터페이스의 상위 클래스를 `EngagementViewController`로 바꿉니다.
...
3. 다음과 같이 캠페인의 첫 번째 필드를 설정합니다.
	...
	- 	배달 시간을 "Out of app only (앱 외부에서만)"으로 선택합니다. 이는 일부 텍스트를 제공하는 간단한 Apple 푸시 알림 유형입니다.
