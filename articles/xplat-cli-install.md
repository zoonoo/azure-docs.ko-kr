...
신속하게 Azure CLI(Azure 명령줄 인터페이스)를 설치하여 Microsoft Azure에서 리소스를 만들고 관리하기 위한 오픈 소스 셸 기반 명령 집합을 사용합니다. 최신 버전을 설치하는 몇 가지 옵션이 있습니다.

* npm 패키지(Node.js 및 npm 필요)에서 설치
* 다른 운영 체제에 대해 제공된 설치 관리자 패키지 중 하나를 사용
* Azure CLI를 Docker 호스트의 컨테이너로 설치
...
azure help
```
> [AZURE.NOTE]일부 Linux 배포에서 `/usr/bin/env: ‘node’: No such file or directory`과 비슷한 오류가 나타날 수 있습니다. 이 오류는 nodejs를 /usr/bin/nodejs에 최근 설치할 때 생겨났습니다. 해결하려면 이 명령을 실행하여 /Usr/bin/node에 바로 가기 링크를 만듭니다.
