...
잠금은 특정 작업을 수행하기 위해 사용자 권한을 할당하는 역할 기반 액세스 제어 사용과 다릅니다. 사용자 및 역할에 대한 권한을 설정하는 방법에 대한 자세한 내용은 [포털에서 역할 기반 액세스 제어](role-based-access-control-configure.md) 및 [리소스에 대한 액세스 관리 및 감사](resource-group-rbac.md)를 참조하세요. 역할 기반 액세스 제어와 달리 관리 잠금을 사용하여 모든 사용자와 역할에 걸쳐 제한을 적용하며 일반적으로 잠금을 제한된 기간 동안만 적용합니다.
