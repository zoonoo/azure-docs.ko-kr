<properties pageTitle="낙관적 동시성을 사용하여 데이터베이스 쓰기 충돌 처리(Windows 스토어) | 모바일 개발자 센터" writer="wesmc" description="Learn how to handle database write conflicts on both the server and in your Windows Store application." documentationCenter="windows" authors="wesmc7777" manager="dwrede" editor="" services=""/>
...
+ 이 자습서는 모바일 서비스 퀵 스타트를 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 시작 프로젝트의 JavaScript 언어 버전을 다운로드하여 [모바일 서비스 시작]을 완료해야 합니다. 
+ [Azure 계정]
+ Microsoft Azure 모바일 서비스 NuGet 패키지 1.1.5 이상. 최신 버전을 얻으려면 아래 단계를 따르세요.
...
이 섹션에서는 각 항목의 텍스트 업데이트를 허용하도록 사용자 인터페이스를 업데이트합니다. 바인딩 템플릿은 데이터베이스 테이블의 각 항목에 대한 확인란 및 텍스트 클래스 컨트롤을 포함합니다. TodoItem의 텍스트 필드를 업데이트할 수 있습니다. **Enter** 키를 누르면 항목이 업데이트되도록 응용 프로그램에서  `keydown` 이벤트를 처리합니다.
...
3. Visual Studio의 솔루션 탐색기에서 **js** 폴더를 확장합니다. default.js 파일을 열고  `updateTodoItem` 함수를 다음 정의로 바꿉니다. 이 정의는 업데이트된 항목을 사용자 인터페이스에서 제거하지 않습니다.
...
4. **Enter** 키를 누르면 항목이 업데이트되도록 `keydown` 이벤트에 대한 다음 이벤트 처리기를 추가합니다.
...
1. default.js 파일의 `todoTable` 변수 선언 아래에서 쓰기 충돌 검색을 지원하도록 설정하는 **__version** 시스템 속성을 포함하도록 코드를 추가합니다.

        var todoTable = client.getTable('TodoItem');
        todoTable.systemProperties |= WindowsAzure.MobileServiceTable.SystemProperties.Version;


2. 테이블의 시스템 속성에 `Version` 시스템 속성을 추가하여 마지막 쿼리 이후에 레코드가 변경된 경우 업데이트 중에 `MobileServicePreconditionFailedException` 예외로 응용 프로그램에 통지합니다. 이 예외는 JavaScript에서 오류 함수와 함께 catch됩니다. 오류에는 충돌을 해결하는 데 사용되는 서버의 최신 항목 버전이 포함되어 있습니다. default.js에서 오류를 catch하고 `resolveDatabaseConflict` 함수를 호출하도록 `updateTodoItem` 함수를 업데이트합니다.
...
3. default.js에서,  `updateTodoItem` 함수에서 참조되는  `resolveDatabaseConflict()` 함수에 대한 정의를 추가합니다. 충돌을 해결하려면 데이터베이스에서 항목을 업데이트하기 전에 로컬 항목의 버전을 서버의 업데이트된 버전으로 설정합니다. 그렇지 않으면 충돌이 계속 발생합니다.
...
7. 앱의 인스턴스 1에서 마지막 항목의 텍스트를 **Test Write 1**로 업데이트한 후 **Enter** 키를 눌러 데이터베이스를 업데이트합니다. 아래 스크린샷에 예제가 나와 있습니다.
...
8. 이때 앱 인스턴스 2의 마지막 항목은 이전 버전입니다. 앱의 해당 인스턴스에서 마지막 항목의 `text` 속성에 대해 **Test Write 2**를 입력하고 **Enter** 키를 눌러 이전 `_version` 속성으로 데이터베이스를 업데이트합니다.
...
9. 업데이트 시도에 사용된 `__version` 값이 서버 `__version` 값과 일치하지 않았으므로 모바일 서비스 SDK는 앱에서 이 충돌을 해결할 수 있도록 `MobileServicePreconditionFailedException`을  `updateTodoItem` 함수의 오류로 발생시킵니다. 충돌을 해결하려면 **예**를 클릭하여 인스턴스 2의 값을 커밋할 수 있습니다. 또는 **아니요**를 클릭하여 인스턴스 2의 값을 취소하고 커밋된 앱 인스턴스 1의 값을 유지합니다. 
...
6. 앱 인스턴스 1에서 마지막 text 속성에 대해 다른 값을 입력한 다음 **Enter** 키를 누릅니다. 앱에서 데이터베이스를 잘못된 `__version` 속성으로 업데이트하려고 시도합니다.
