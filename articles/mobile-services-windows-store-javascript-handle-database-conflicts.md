<properties urlDisplayName="Optimistic concurrency" pageTitle="낙관적 동시성을 사용하여 데이터베이스 쓰기 충돌 처리(Windows 스토어) | 모바일 개발자 센터" metaKeywords="" writer="wesmc" description="Learn how to handle database write conflicts on both the server and in your Windows Store application." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Handling database write conflicts" authors="wesmc" manager="dwrede" />
...
<a href="/ko-kr/documentation/articles/mobile-services-windows-store-javascript-handle-database-conflicts/" title="Windows Store JavaScript" class="current">Windows 스토어 JavaScript</a>
...
+ 이 자습서는 모바일 서비스 빠른 시작을 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 시작 프로젝트의 JavaScript 언어 버전을 다운로드하여 [모바일 서비스 시작]을 완료해야 합니다. 
+ [Azure 계정]
+ Microsoft Azure 모바일 서비스 NuGet 패키지 1.1.5 이상. 최신 버전을 얻으려면 다음 단계를 따르세요.
	1. Visual Studio에서 프로젝트를 열고 솔루션 탐색기에서 프로젝트를 마우스 오른쪽 단추로 클릭한 후 **NuGet 패키지 관리**를 클릭합니다. 


	2. **온라인**을 확장하고 **Microsoft 및 .NET**을 클릭합니다. 검색 텍스트 상자에 **WindowsAzure.MobileServices.WinJS**를 입력합니다. **WinJS용 Microsoft Azure 모바일 서비스** NuGet 패키지에서 **설치**를 클릭합니다.
...
이 섹션에서는 각 항목의 텍스트 업데이트를 허용하도록 사용자 인터페이스를 업데이트합니다. 바인딩 템플릿은 데이터베이스 테이블의 각 항목에 대한 확인란 및 텍스트 클래스 컨트롤을 포함합니다. TodoItem의 텍스트 필드를 업데이트할 수 있습니다. **Enter** 키를 누르면 항목이 업데이트되도록 응용 프로그램에서는 `keydown` 이벤트를 처리합니다.


1. Visual Studio에서 [모바일 서비스 시작] 자습서에서 다운로드한 TodoList 프로젝트의 JavaScript 언어 버전을 엽니다.
2. Visual Studio 솔루션 탐색기에서 default.html을 열고 `TemplateItem` div 태그 정의를 아래 표시된 div 태그로 바꾸고 변경 내용을 저장합니다. 그러면 TodoItem의 텍스트를 편집할 수 있도록 텍스트 상자 컨트롤이 추가됩니다.
...
3. Visual Studio 솔루션 탐색기에서 **js** 폴더를 확장합니다. default.js 파일을 열고 `updateTodoItem` 함수를 다음 정의로 바꿉니다. 이 정의는 업데이트된 항목을 사용자 인터페이스에서 제거하지 않습니다.
...
4. **Enter** 키를 누르면 항목이 업데이트되도록 default.js 파일에서 `keydown` 이벤트에 대한 다음 이벤트 처리기를 추가합니다.
...
이제 응용 프로그램에서는 **Enter** 키를 누르면 각 항목의 변경된 텍스트를 데이터베이스에 다시 씁니다.

<h2><a name="enableOC"></a>응용 프로그램에서 충돌 검색 사용</h2>

Azure 모바일 서비스는 각 테이블에 추가되는 `__version` 시스템 속성 열을 사용하여 각 항목의 변경을 추적하여 낙관적 동시성 제어를 지원합니다. 이 섹션에서는 응용 프로그램에서 `__version` 시스템 속성을 통해 이러한 쓰기 충돌을 검색할 수 있도록 설정합니다. todoTable에서 이 시스템 속성을 사용하도록 설정하면 마지막 쿼리 이후에 레코드가 변경된 경우 업데이트 시도 중에 `MobileServicePreconditionFailedException`을 통해 응용 프로그램에 통지됩니다. 그러면 앱에서 변경 내용을 데이터베이스에 커밋할지 데이터베이스에 대한 마지막 변경 내용을 그대로 적용할지 여부를 선택할 수 있습니다. 모바일 서비스의 시스템 속성에 대한 자세한 내용은 [시스템 속성]을 참조하세요.

1. default.js 파일에서 `todoTable`  변수 선언 아래에 쓰기 충돌 검색을 지원하도록 설정하는 **__version** 시스템 속성을 포함하도록 코드를 추가합니다.

        var todoTable = client.getTable('TodoItem');
        todoTable.systemProperties |= WindowsAzure.MobileServiceTable.SystemProperties.Version;


2. 테이블의 시스템 속성에 `Version` 시스템 속성을 추가하여 마지막 쿼리 이후에 레코드가 변경된 경우 업데이트 중에 `MobileServicePreconditionFailedException` 예외를 발생하여 응용 프로그램에 통지합니다. 이 예외는 JavaScript에서 오류 함수와 함께 catch됩니다. 오류에는 충돌을 해결하는 데 사용되는 서버의 최신 항목 버전이 포함되어 있습니다. default.js에서 오류를 catch하고 `resolveDatabaseConflict` 함수를 호출하도록 `updateTodoItem` 함수를 업데이트합니다.
...
3. default.js에 `updateTodoItem` 함수에서 참조되는 `resolveDatabaseConflict()` 함수에 대한 정의를 추가합니다. 충돌을 해결하려면 데이터베이스에서 항목을 업데이트하기 전에 로컬 항목의 버전을 서버의 업데이트된 버전으로 설정합니다. 그렇지 않으면 충돌이 계속 발생합니다.
...
7. 앱의 인스턴스 1에서 마지막 항목의 텍스트를 **테스트 쓰기 1**로 업데이트한 후 **Enter** 키를 눌러 데이터베이스를 업데이트합니다. 예제는 아래 스크린샷을 참조하세요.
...
8. 이때 앱 인스턴스 2의 마지막 항목은 이전 버전입니다. 앱의 해당 인스턴스에서 마지막 항목의 `text` 속성에 **테스트 쓰기 2**를 입력하고 **Enter** 키를 눌러 데이터베이스를 이전 `_version` 속성으로 업데이트합니다.
...
9. 업데이트 시도에 사용된 `__version` 값이 서버 `__version` 값과 일치하지 않으므로 모바일 서비스 SDK는 앱에서 `updateTodoItem` 함수의 오류를 통해 이 충돌을 해결할 수 있도록 `MobileServicePreconditionFailedException`을 발생합니다. 충돌을 해결하려면 **예**를 클릭하여 인스턴스 2의 값을 커밋할 수 있습니다. 인스턴스 2의 값을 취소하고 커밋된 앱 인스턴스 1의 값을 그대로 유지하려면 **아니요**를 클릭합니다. 
...
5. 두 컴퓨터 모두에서 **todolist** 앱을 실행합니다. 인스턴스 2에서 마지막 항목에 대한 TodoItem `text`를 변경하고 앱에서 데이터베이스를 업데이트하도록 **Enter** 키를 누릅니다.
...
6. 앱 인스턴스 1에서 마지막 text 속성에 대해 다른 값을 입력한 후 **Enter** 키를 누릅니다. 앱에서 데이터베이스를 잘못된 `__version` 속성으로 업데이트하려고 시도합니다.
...
9. 인스턴스 2에서 마지막 TodoItem의 텍스트를 업데이트하고 **Enter** 키를 누릅니다. 완료 필드를 true로 설정하여 업데이트했기 때문에 충돌이 발생합니다. 충돌에 대한 응답으로 스크립트는 항목이 이미 완료되었으므로 업데이트를 거부하여 충돌을 해결했습니다. 스크립트는 메시지를 응답으로 제공했습니다.  
