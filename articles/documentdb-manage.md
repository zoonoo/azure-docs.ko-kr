...
다음 표에서는 다양한 데이터베이스 작업 목록 및 CU당 사용 가능한 요청 단위를 제공합니다. 이 표를 사용하여 지정된 데이터베이스 작업이 CU와 관련해서 프로비전된 처리량을 사용하는 방식을 계획할 수 있습니다. 문서 크기는 1KB이고 기본 일관성 수준이 "세션"으로 설정된 고유한 속성 값 10개로 구성되며 모든 문서가 DocumentDB에서 자동으로 인덱싱된다고 가정합니다.
