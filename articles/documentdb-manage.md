<properties 
	...
	description="응용 프로그램의 용량 요구 사항을 충족하기 위해 탄력적으로 DocumentDB의 크기를 조정할 수 있는 방법에 대해 알아봅니다." 
	services="documentdb" 
...
> [AZURE.NOTE] 각 컬렉션은 저장소와 처리량 모두 지원되는 최대값[한도](http://azure.microsoft.com/documentation/articles/documentdb-limits/)까지 확장할 수 있습니다. 처리량은 컬렉션당 최대 한도까지 모든 컬렉션에 고르게 분포됩니다.
...
Azure 구독자는 DocumentDB 데이터베이스 계정을 하나 이상 프로비전할 수 있습니다. 각 데이터베이스 계정에는 데이터베이스, 사용자, 사용 권한을 비롯한 관리 리소스 할당량이 있습니다. 이러한 리소스는 [한도 및 할당량](http://azure.microsoft.com/documentation/articles/documentdb-limits/)의 영향을 받습니다. 추가적인 관리 리소스가 필요하면 지원 센터에 문의하세요.   
...
DocumentDB에서는 UDF, 저장 프로시저 및 트리거와 함께 데이터베이스 컬렉션 내부의 문서에서 작동하는 모든 관계형 및 계층형 쿼리를 포함해서 다양한 데이터베이스 작업이 허용됩니다. 각 작업과 관련된 비용은 작업을 완료하는 데 필요한 CPU, IO 및 메모리에 따라 달라집니다.  하드웨어 리소스를 고려하고 관리하는 대신 다양한 데이터베이스 작업을 수행하고 응용 프로그램 요청을 처리하는 데 필요한 리소스의 단일 측정값으로 RU(요청 단위)를 고려할 수 있습니다.   
...
다음 표에서는 CU당 사용 가능한 다양한 데이터베이스 작업 및 요청 단위 목록을 보여 줍니다. 이 표는 특정 데이터베이스 작업이 CU와 연관된 프로비전된 처리량을 소비하는 방법을 계획하는 데 도움이 됩니다.  문서 크기는 10개의 고유 속성 값으로 구성된 1KB이고, 기본 일관성 수준은 "세션"으로 설정되며, 모든 문서가 DocumentDB에 의해 자동으로 인덱싱된다고 가정합니다. 
...
Azure Preview 관리 포털에서 일관성 수준을 변경하는 방법에 대한 지침은 [DocumentDB 계정 관리 방법]에서 방법: DocumentDB 일관성 설정 관리를 참조하세요(../documentdb-manage-account/).
...
Azure Preview 포털에서 DocumentDB 관리에 대한 지침은 [DocumentDB 계정 관리 방법]에서 방법: DocumentDB 일관성 설정 관리를 참조하세요(../documentdb-manage-account/).

Azure Preview 포털에서 성능 모니터링에 대한 지침은 [DocumentDB 계정 모니터링]을 참조하세요(../documentdb-monitor-accounts/).
