...
일반 시나리오를 수행하는 방법을 보여 줍니다. 샘플은 Python
API를 사용하여 작성되었습니다. 여기서 다루는 시나리오에는 큐 메시지 **삽입**, **보기**,
**가져오기** 및 **삭제**와 **큐 만들기 및
삭제**가 포함됩니다. 큐에 대한 자세한 내용은 [다음 단계][] 섹션을 참조하세요.
...
**QueueService** 개체를 사용하면 큐로 작업할 수 있습니다. 다음 코드는 **QueueService** 개체를 만듭니다. 프로그래밍 방식으로 Azure 저장소에 액세스하려는 Python 파일의 맨 위에 다음을 추가합니다.
...
큐에 메시지를 삽입하려면 **put\_message** 메서드를 사용하여
...
큐에서 메시지를 제거하지 않고도
**peek\_messages** 메서드를 호출하여 큐의 맨 앞에서 원하는 메시지를 볼 수 있습니다. 기본적으로
...
**get\_messages**에서 반환된 메시지는
이 큐의 메시지를 읽는 다른 코드에는 표시되지 않습니다. 기본적으로 이 메시지는
30초간 표시되지 않습니다. 큐에서 메시지 제거를 완료하려면
**delete\_message**도 호출해야 합니다. 메시지를 제거하는 이 2단계 프로세스는
코드가 하드웨어 또는 소프트웨어 오류로 인해 메시지를 처리하지 못하는 경우
코드의 다른 인스턴스가 동일한 메시지를 가져와서
다시 시도할 수 있도록 보장합니다. 코드는 메시지가 처리된 직후에 **delete\_message**를
호출합니다.
...
작업의 상태를 업데이트할 수 있습니다. 아래 코드는 **update\_message**
...
**get\_messages** 메서드를 사용하여 한 번 호출에 16개의 메시지를 가져옵니다. 그런 다음
for 루프를 사용하여 각 메시지를 처리합니다. 또한 각 메시지에 대해
표시되지 않는 제한 시간을 5분으로 설정합니다.
...
**get\_queue\_metadata** 메서드는 큐 서비스에서 큐에 대한
메타데이터를 반환하고 **x-ms-approximate-messages-count**를 반환된 사전의 인덱스로 사용하여 수를 찾도록 요청합니다.
