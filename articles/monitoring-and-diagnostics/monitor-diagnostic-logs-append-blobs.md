...
> [!WARNING]
> [리소스 진단 설정을 사용하여 Azure 리소스 진단 로그 또는 메트릭을 저장소 계정](./monitoring-archive-diagnostic-logs.md)에 전송하는 경우 또는 [로그 프로필을 사용하여 활동 로그를 저장소 계정](./monitoring-archive-activity-log.md)에 전송하는 경우, 2018년 11월 1일에 저장소 계정의 데이터 형식이 JSON 줄로 변경됩니다. 아래 지침에서는 새 형식을 처리하도록 도구를 업데이트하는 방법 및 영향에 대해 설명합니다. 
>
> 
...
* 이 변경은 다음 데이터 형식에 영향을 줍니다.
  * [Azure 리소스 진단 로그](./monitoring-archive-diagnostic-logs.md)([여기서 리소스 목록 확인](./monitoring-diagnostic-logs-schema.md))
  * [진단 설정을 통해 내보내지는 Azure 리소스 메트릭](./monitoring-overview-of-diagnostic-logs.md#resource-diagnostic-settings)
  * [로그 프로필을 통해 내보내지는 Azure 활동 로그 데이터](./monitoring-archive-activity-log.md)
...
![Azure Monitor 진단 설정 블레이드](./media/monitor-diagnostic-logs-append-blobs\portal-diag-settings.png)

진단 상태가 사용으로 설정된 경우, 해당 리소스에 활성 진단 설정이 있습니다. 리소스를 클릭하여 저장소 계정에 데이터를 보내는 진단 설정이 있는지 확인합니다.

![저장소 계정 사용](./media/monitor-diagnostic-logs-append-blobs\portal-storage-enabled.png)
...
* [리소스 진단 로그를 저장소 계정에 보관](./monitoring-archive-diagnostic-logs.md)하는 방법을 알아봅니다.
* [활동 로그 데이터를 저장소 계정에 보관](./monitoring-archive-activity-log.md)하는 방법을 알아봅니다.
