---
title: "Azure 진단 1.3, 1.4, 1.5, 1.6, 1.7 구성 스키마 | Microsoft Docs"
description: "Azure 진단용 스키마 버전 1.3 이상은 Microsoft Azure SDK의 일부로 제공됩니다."
...
---
# <a name="azure-diagnostics-13-14-15-configuration-schema"></a>Azure 진단 1.3, 1.4, 1.5 구성 스키마
> [!NOTE]
...
Azure 진단 구성 파일은 진단 모니터가 시작될 때 진단 구성 설정을 초기화하는 데 사용됩니다.  

이 페이지는 버전 1.3, 1.4 및 1.5(Azure SDK 2.4 이상).에 유효합니다. 최신 구성 섹션은 추가된 버전에 표시하도록 주석 처리되었습니다.  
...
다음은 이전 XML 구성 파일에 해당하는 JSON입니다.  
```json
{
    "WadCfg": {
        "DiagnosticMonitorConfiguration": {
...
|자식 요소|설명|  
|--------------------|-----------------|  
|**DiagnosticMonitorConfiguration**|필수 요소입니다.<br /><br /> 선택적 특성은 다음과 같습니다.<br /><br /> - **overallQuotaInMB** - Azure 진단으로 수집된 진단 데이터의 다양한 형식에서 사용될 수 있는 로컬 디스크 공간의 최대 크기입니다. 기본 설정은 5120MB입니다.<br /><br /> - **useProxyServer** - IE 설정에서 설정한 대로 프록시 서버 설정을 사용하도록 Azure 진단을 구성합니다.|  
 ...
 **resourceId** 특성이 필요합니다.  이는 Azure 진단을 배포하는 가상 컴퓨터의 리소스 ID입니다. [Azure Portal](https://portal.azure.com)에서 **resourceID**를 가져옵니다. **찾아보기** -> **리소스 그룹** -> **<이름\>**을 선택합니다. **속성** 타일을 클릭하고 **ID** 필드에서 값을 복사합니다.  
...
|자식 요소|설명|  
|--------------------|-----------------|  
|**StorageAccount**|사용할 저장소 계정입니다. 다음과 같은 특성이 필요<br /><br /> - **이름** - 저장소 계정의 이름입니다.<br /><br /> - **키** - 저장소 계정의 키입니다.<br /><br /> - **끝점** - 저장소 계정에 액세스하는 끝점입니다.|  
