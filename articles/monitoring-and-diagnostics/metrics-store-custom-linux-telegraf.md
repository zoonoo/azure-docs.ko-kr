...
1. VM의 크기를 선택합니다. 예를 들어,  **컴퓨팅 유형** 또는  **디스크 유형**으로 필터링할 수 있습니다. 
    ...
    ![가상 머신 크기 Telegraph 에이전트 개요](./media/metrics-store-custom-linux-telegraf/vm-size.png)
...
1. VM 창에서 **ID** 탭으로 이동합니다. VM에서 시스템 할당 ID가 **켜짐**으로 설정되었는지 확인합니다. 
    ...
    ![Telegraf VM ID 미리 보기](./media/metrics-store-custom-linux-telegraf/connect-to-VM.png)
 
## <a name="connect-to-the-vm"></a>VM에 연결 

VM과의 SSH 연결을 만듭니다. VM의 개요 페이지에서  **연결** 단추를 선택합니다. 

![Telegraf VM 개요 페이지](./media/metrics-store-custom-linux-telegraf/connect-VM-button2.png)
...
1. 새  **모니터** 탭으로 이동합니다.  **메트릭**을 선택합니다.  
     ...
     ![모니터 - 메트릭(미리 보기)](./media/metrics-store-custom-linux-telegraf/metrics.png)

1. 리소스 선택기에서 VM을 선택합니다.

     ![메트릭 차트](./media/metrics-store-custom-linux-telegraf/metric-chart.png)

1. **Telegraf/CPU** 네임 스페이스를 선택하고, **usage_system** 메트릭을 선택합니다. 이 메트릭의 차원으로 필터링하거나 차원을 분할하도록 선택할 수 있습니다.  

     ![네임스페이스 및 메트릭 선택](./media/metrics-store-custom-linux-telegraf/VM-resource-selector.png)
