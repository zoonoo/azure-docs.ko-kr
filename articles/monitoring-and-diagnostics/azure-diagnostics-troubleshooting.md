...
- **추적 로그**: 원격 데스크톱을 VM에 연결하고 앱의 구성 파일에 TextWriterTraceListener를 추가합니다.  텍스트 수신기를 설정하려면 http://msdn.microsoft.com/library/sk36c28t.aspx 를 참조하세요.  `<trace>` 요소에 `<trace autoflush="true">`가 있는지 확인합니다.<br />
...
확장 구성을 확인하는 가장 쉬운 방법은 http://resources.azure.com 으로 이동한 다음, Azure 진단 확장(IaaSDiagnostics/PaaDiagnostics)이 문제가 되는 가상 컴퓨터 또는 클라우드 서비스로 이동하는 것입니다.
