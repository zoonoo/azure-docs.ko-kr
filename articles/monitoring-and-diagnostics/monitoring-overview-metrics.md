...
* 메트릭은 **즉시 사용 가능합니다. 따라서 옵트인하거나** 추가 진단을 설정하지 않아도 됩니다.
...
* **보관** 합니다. 리소스에 대한 진단 설정을 구성할 때 메트릭을 Azure Blob Storage로 라우팅할 수 있습니다.
* 메트릭을 이벤트 허브로 **스트리밍**한 다음 Azure Stream Analytics나 사용자 지정 앱으로 라우팅하여 거의 실시간에 가까운 분석이 가능합니다. 진단 설정을 사용할 수 있습니다.
* 모든 메트릭을 Log Analytics(OMS)로 **라우팅**하여 리소스로부터 받은 메트릭 데이터에 대한 인스턴스 분석, 검색 및 사용자 지정 경고를 잠금 해제합니다.
