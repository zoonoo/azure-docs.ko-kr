---
title: "Azure에서 자동 크기 조정 시작 | Microsoft 문서"
...
Azure Monitor 자동 크기 조정은 가상 컴퓨터 확장 집합, Cloud Services, Azure App Service 계획 및 App Service 환경에만 적용됩니다. 
...
2. 왼쪽 창에서 Azure Monitor 아이콘을 클릭합니다.
  ![Azure Monitor 열기][2]
3. **자동 크기 조정**을 클릭하여 자동 크기 조정을 적용할 수 있는 모든 리소스와 해당 리소스의 현재 자동 크기 조정 상태를 확인합니다.
...
2. 현재 인스턴스 수는 1개입니다. **자동 크기 조정 사용**을 클릭합니다.
  ![새 웹앱에 대한 크기 조정 설정][5]
3. 크기 조정 설정의 이름을 입력하고 **규칙 추가**를 클릭합니다. 오른쪽에 컨텍스트 창으로 열리는 크기 조정 규칙 옵션을 확인합니다. 이 규칙은 기본적으로 리소스의 CPU 백분율이 70%를 초과하면 인스턴스 수를 하나씩 늘리도록 옵션을 설정합니다. 규칙을 기본값으로 유지하고 **추가**를 클릭합니다.
  ![웹앱에 대한 크기 조정 설정 만들기][6]
4. 이제 첫 번째 크기 조정 규칙을 만들었습니다. UX에 권장 모범 사례와 "하나 이상의 규모 감축 규칙이 있는 것이 좋습니다."라는 메시지가 표시됩니다. 이렇게 하려면 다음을 수행합니다.
...
> [!NOTE] 
> 가상 컴퓨터 확장 집합 또는 Cloud Service 역할의 크기 조정을 시작하려는 경우에도 동일한 단계를 적용할 수 있습니다.
...
2. 크기 조정 모드 및 규칙 설정은 기본 조건과 동일합니다.
3. 일정으로 **특정 일 반복**을 선택합니다.
4. 크기 조정 조건을 적용할 요일과 시작/종료 시간을 선택합니다.
...
1. **크기 조정 조건 추가**를 클릭합니다.
2. 크기 조정 모드 및 규칙 설정은 기본 조건과 동일합니다.
3. 일정으로 **시작/종료 날짜 지정**을 선택합니다.
4. 크기 조정 조건을 적용할 시작/종료 날짜와 시작/종료 시간을 선택합니다.
