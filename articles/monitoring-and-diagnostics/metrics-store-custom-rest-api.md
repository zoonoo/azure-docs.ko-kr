...
이 문서에서는 REST API를 통해 Azure 리소스에 대한 사용자 지정 메트릭을 Azure Monitor 메트릭 저장소로 보내는 방법을 보여 줍니다.  메트릭이 Azure Monitor로 전송되고 나면, 표준 메트릭과 마찬가지로 모든 작업(예: 차트 작성, 경고, 다른 외부 도구로 라우팅 등)을 수행할 수 있습니다.  

>[!NOTE] 
>REST API는 Azure 리소스에 대한 사용자 지정 메트릭 보내기만 허용합니다. 다른 환경이나 온-프레미스 리소스에 대한 메트릭을 보내려면 [Application Insights](../application-insights/app-insights-api-custom-events-metrics.md)를 사용할 수 있습니다.    
...
- 로그온 URL에 대해 임의 URL을 사용할 수 있습니다.  
...
1. 다음 JSON을 파일에 붙여넣고 로컬 컴퓨터에 custommetric.json으로 저장합니다. JSON 파일의 시간 매개 변수를 업데이트합니다. 
...
1. 명령 프롬프트 창에서 메트릭 데이터를 게시합니다. 
    - Azure 지역 - 메트릭을 내보내는 리소스의 배포 지역과 일치해야 합니다. 
    - ResourceID – 메트릭을 추적 중인 Azure 리소스의 리소스 ID입니다.  
    - 액세스 토큰 - 이전에 가져온 토큰을 붙여넣습니다.
...
2. 사용자 지정 메트릭을 지원하지 않는 지역에 대해 메트릭을 내보내는 경우가 발생할 수 있습니다. [지원되는 사용자 지정 메트릭(미리 보기) 지역](metrics-custom-overview.md#supported-regions)을 참조하세요. 
...
1. 모니터 페이지에서 **메트릭**을 클릭합니다. 
   ...
   ![액세스 토큰](./media/metrics-store-custom-rest-api/metrics.png) 

1. 집계 기간을 **지난 30분**으로 변경합니다.  

1. *리소스* 드롭다운에서 메트릭을 내보낸 리소스를 선택합니다.  

1. *네임스페이스* 드롭다운에서 **QueueProcessing**을 선택합니다. 

1. *메트릭* 드롭다운에서 **QueueDepth**를 선택합니다.  
