...
## <a name="autoscale-process-explained"></a>자동 크기 조정 프로세스 설명
다음 설명은 이전 다이어그램의 부분에 적용됩니다.   

### <a name="resource-metrics"></a>리소스 메트릭
리소스는 메트릭을 내보내며 나중에 규칙에 의해 처리됩니다. 메트릭은 다른 메서드를 통해 제공됩니다.
Web Apps 및 Cloud Services의 원격 분석은 Azure 인프라에서 직접 제공되는 반면 VM 크기 조정 설정에서는 Azure 진단 에이전트의 원격 분석 데이터를 사용합니다. 일부 자주 사용되는 통계는 CPU 사용량, 메모리 사용량, 스레드 수, 큐 길이 및 디스크 사용량을 포함합니다. 사용할 수 있는 원격 분석 데이터 목록은 [자동 크기 조정 공통 메트릭](insights-autoscale-common-metrics.md)을 참조하세요.
