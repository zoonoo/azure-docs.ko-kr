...
1. **서브넷 추가** 를 다시 선택한 다음, **서브넷 이름** 및 **서브넷 주소 범위** 를 입력합니다(예: *apps-subnet* 및 10.1.1.0/24).  **추가** 를 클릭합니다.
...
*ap-app_{service instance name}_{service instance region}* 이라는 리소스 그룹에는 서비스 인스턴스의 Spring Boot 마이크로서비스 애플리케이션에 대한 네트워크 리소스가 포함되어 있습니다.
