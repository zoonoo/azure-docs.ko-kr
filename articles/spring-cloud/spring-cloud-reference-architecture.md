...
Azure 스프링 클라우드는 보안 환경에 배포 될 때 Azure에서 송신 하는 관리 트래픽이 필요 합니다. 이를 위해서는 [VNET에서 Azure 스프링 클라우드를 실행 하기 위해 고객 책임](/azure/spring-cloud/spring-cloud-vnet-customer-responsibilities)에 나열 된 네트워크 및 응용 프로그램 규칙을 허용 해야 합니다.
