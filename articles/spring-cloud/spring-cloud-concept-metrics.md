...
모든 메트릭 옵션에 대 한 자세한 내용은 아래 [섹션](#user-metrics-options) 에서 찾을 수 있습니다.

다음으로 각 메트릭에 대해 집계 유형을 선택 합니다.

![메트릭 집계](media/metrics/metrics-5.png)

집계 유형은 시간별로 차트에서 메트릭 점수를 집계 하는 방법을 나타냅니다. 1 분 마다 하나의 원시 메트릭 지점이 있으며, a 분 내에 사전 집계 형식은 메트릭 유형에 의해 미리 정의 됩니다.
...
>|----|----|----|------------|
>| TomcatErrorCount<br><br>Tomcat 전역 오류 (사용 되지 않음) | tomcat | 개수 | 처리 된 요청에 발생 한 오류 수 |
>| tomcat | tomcat | 개수 | 처리 된 요청에 발생 한 오류 수 |

### <a name="performance"></a>성능
>[!div class="mx-tdCol2BreakAll"]
>| 이름 | 스프링 발동기 메트릭 이름 | 단위 | 세부 정보 |
>|----|----|----|------------|
>| SystemCpuUsagePercentage<br><br>시스템 CPU 사용량 비율 (사용 되지 않음) | 시스템 cpu 사용량 | 백분율 | 전체 시스템에 대 한 최근 CPU 사용량입니다. 이 값은 [0.0, 1.0] 간격의 double입니다. 0.0 값은 최근 관찰 된 기간 동안 모든 Cpu가 유휴 상태임을 의미 하 고, 값 1.0은 모든 Cpu가 최근 관찰 되는 기간 동안 100%를 적극적으로 실행 하 고 있음을 의미 합니다.|
...
>| AppCpuUsagePercentage<br><br>앱 CPU 사용량 비율 (사용 되지 않음) | 앱 CPU 사용량 비율 | 백분율 | Java Virtual Machine 프로세스에 대 한 최근 CPU 사용량입니다. 이 값은 [0.0, 1.0] 간격의 double입니다. 0.0 값은 최근 관찰 된 기간 동안 JVM 프로세스에서 스레드를 실행 하 고 있던 Cpu가 없음을 의미 하 고, 값 1.0은 모든 Cpu가 최근 관찰 되는 시간 동안 JVM 100%에서 적극적으로 스레드를 실행 하 고 있음을 의미 합니다. JVM의 스레드는 응용 프로그램 스레드와 JVM 내부 스레드를 포함 합니다.|
>| 프로세스. cpu 사용량 | 앱 CPU 사용량 비율 | 백분율 | Java Virtual Machine 프로세스에 대 한 최근 CPU 사용량입니다. 이 값은 [0.0, 1.0] 간격의 double입니다. 0.0 값은 최근 관찰 된 기간 동안 JVM 프로세스에서 스레드를 실행 하 고 있던 Cpu가 없음을 의미 하 고, 값 1.0은 모든 Cpu가 최근 관찰 되는 시간 동안 JVM 100%에서 적극적으로 스레드를 실행 하 고 있음을 의미 합니다. JVM의 스레드는 응용 프로그램 스레드와 JVM 내부 스레드를 포함 합니다.|
>| AppMemoryCommitted<br><br>할당 된 앱 메모리 (사용 되지 않음) | jvm. 메모리 커밋 | 바이트 | JVM에서 사용할 수 있도록 보장 되는 메모리의 양을 나타냅니다. JVM은 시스템에 메모리를 릴리스할 수 있으며, 커밋됨은 init 보다 낮을 수 있습니다. 커밋됨은 항상 사용 되는 것 보다 크거나 같아야 합니다. |
...
>| AppMemoryUsed <br><br>사용 되는 앱 메모리 (사용 되지 않음) | jvm. memory. 사용 됨 | 바이트 | 현재 사용 중인 메모리의 양 (바이트)을 나타냅니다. |
>| jvm. memory. 사용 됨 | jvm. memory. 사용 됨 | 바이트 | 현재 사용 중인 메모리의 양 (바이트)을 나타냅니다. |
>| AppMemoryMax<br><br>최대 앱 메모리 (사용 되지 않음) | jvm. memory. max | 바이트 | 메모리 관리에 사용할 수 있는 최대 메모리 양을 나타냅니다. Max가 정의 된 경우 사용 된 메모리와 커밋된 메모리의 양은 항상 max 보다 작거나 같아야 합니다. 사용 된 <= max가 여전히 true 인 경우 (예: 시스템의 가상 메모리가 부족 한 경우)에도 사용 되는 사용 되 >는 메모리를 늘려야 하는 경우 메모리 할당이 실패할 수 있습니다. |
...
>| MaxOldGenMemoryPoolBytes<br><br>사용 가능한 최대 이전 세대 데이터 크기 (사용 되지 않음) | jvm. 최대 데이터 크기 | 바이트 | Java 가상 머신이 시작 된 이후의 이전 세대 메모리 풀의 최대 메모리 사용량입니다. |
>| jvm. 최대 데이터 크기 | jvm. 최대 데이터 크기 | 바이트 | Java 가상 머신이 시작 된 이후의 이전 세대 메모리 풀의 최대 메모리 사용량입니다. |
>| OldGenMemoryPoolBytes<br><br>이전 세대 데이터 크기 (사용 되지 않음) | jvm. 데이터 크기 | 바이트 | 전체 GC 후의 이전 세대 메모리 풀 크기입니다. |
>| jvm. 데이터 크기 | jvm. 데이터 크기 | 바이트 | 전체 GC 후의 이전 세대 메모리 풀 크기입니다. |
>| OldGenPromotedBytes<br><br>이전 세대 데이터 크기 (사용 되지 않음)로 승격 | jvm. | 바이트 | Gc 이전 세대 메모리 풀의 크기에서 GC 이후로의 양의 증가가 수입니다. |
>| jvm. | jvm. | 바이트 | Gc 이전 세대 메모리 풀의 크기에서 GC 이후로의 양의 증가가 수입니다. |
>| YoungGenPromotedBytes<br><br>젊은 생성 데이터 크기로 수준 올리기 (사용 되지 않음) | jvm. 메모리 할당 | 바이트 | 한 GC 후에 한 GC에서 다음 시간까지 증가 하는 경우 증가 합니다. |
>| jvm. 메모리 할당 | jvm. 메모리 할당 | 바이트 | 한 GC 후에 한 GC에서 다음 시간까지 증가 하는 경우 증가 합니다. |
>| GCPauseTotalCount<br><br>GC 일시 중지 횟수 (사용 되지 않음) | jvm. 일시 중지 (총 개수) | 개수 | 젊은 및 이전 GC를 포함 하 여이 JMV가 시작 된 이후의 총 GC 수입니다. |
>| jvm. 일시 중지. 총 개수 | jvm. 일시 중지 (총 개수) | 개수 | 젊은 및 이전 GC를 포함 하 여이 JMV가 시작 된 이후의 총 GC 수입니다. |
>| GCPauseTotalTime<br><br>GC 일시 중지 총 시간 (사용 되지 않음) | jvm. 일시 중지 (전체 시간) | 밀리초 | 젊은 및 이전 GC를 포함 하 여이 JMV가 시작 된 후 사용 된 총 GC 시간입니다. |
>| jvm. gc. total. time | jvm. 일시 중지 (전체 시간) | 밀리초 | 젊은 및 이전 GC를 포함 하 여이 JMV가 시작 된 후 사용 된 총 GC 시간입니다. |
>| tomcat. | tomcat. | 개수 | Tomcat Config 최대 스레드 수 |
>| tomcat | tomcat | 개수 | Tomcat 현재 스레드 수 |
>| tomcat-평균 시간 | tomcat-평균 시간 | 밀리초 | Tomcat 요청 평균 시간 |
...
>| 이름 | 스프링 발동기 메트릭 이름 | 단위 | 세부 정보 |
>|----|----|----|------------|
>| TomcatSentBytes<br><br>총 보낸 바이트 Tomcat (사용 되지 않음) | tomcat | 바이트 | Tomcat 웹 서버에서 보낸 데이터의 양 |
>| tomcat | tomcat | 바이트 | Tomcat 웹 서버에서 보낸 데이터의 양 |
>| TomcatReceivedBytes<br><br>Tomcat Total Bytes (사용 되지 않음) | tomcat | 바이트 | Tomcat 웹 서버에서 받은 데이터의 양 |
>| tomcat | tomcat | 바이트 | Tomcat 웹 서버에서 받은 데이터의 양 |
>| TomcatRequestTotalTime<br><br>Tomcat 요청 총 시간 (사용 되지 않음) | tomcat (총 시간) | 밀리초 | 요청을 처리 하는 Tomcat 웹 서버의 총 시간 |
>| TomcatRequestTotalCount<br><br>Tomcat Request 총 개수 (사용 되지 않음) | tomcat (총 개수) | 개수 | Tomcat 웹 서버 처리 된 요청의 총 수입니다. |
>| tomcat. 총 개수 | tomcat (총 개수) | 개수 | Tomcat 웹 서버 처리 된 요청의 총 수입니다. |
>| TomcatRequestMaxTime<br><br>Tomcat 요청 최대 시간 (사용 되지 않음) | tomcat. 최대 | 밀리초 | Tomcat 웹 서버가 요청을 처리 하는 최대 시간 |
...
>| 이름 | 스프링 발동기 메트릭 이름 | 단위 | 세부 정보 |
>|----|----|----|------------|
>| TomcatSessionActiveMaxCount<br><br>Tomcat Session 최대 활성 수 (사용 되지 않음) | tomcat. | 개수 | 동시에 활성화 된 최대 세션 수 |
>| tomcat. | tomcat. | 개수 | 동시에 활성화 된 최대 세션 수 |
>| TomcatSessionAliveMaxTime<br><br>Tomcat 세션 최대 활성 시간 (사용 되지 않음) | tomcat. | 밀리초 | 만료 된 세션이 활성 상태인 가장 긴 시간 (초) |
>| tomcat. | tomcat. | 밀리초 | 만료 된 세션이 활성 상태인 가장 긴 시간 (초) |
>| TomcatSessionCreatedCount<br><br>Tomcat 세션 생성 횟수 (사용 되지 않음) | tomcat 생성 | 개수 | 생성 된 세션 수 |
>| tomcat 생성 | tomcat 생성 | 개수 | 생성 된 세션 수 |
>| TomcatSessionExpiredCount<br><br>Tomcat 세션 만료 횟수 (사용 되지 않음) | tomcat. | 개수 | 만료 된 세션 수 |
>| tomcat. | tomcat. | 개수 | 만료 된 세션 수 |
>| TomcatSessionRejectedCount<br><br>Tomcat 세션 거부 횟수 (사용 되지 않음) | tomcat 거부 | 개수 | 최대 활성 세션 수에 도달 하 여 만들지 않은 세션 수입니다. |
