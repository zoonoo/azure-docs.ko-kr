...
해당하는 특정 토폴로지 버전을 프로비전하는 데 필요한 모든 파일이 포함된 zip 파일을 업로드합니다. 이 zip 파일에는 다음 파일이 포함되어 있어야 합니다.
...
지금까지 솔루션 템플릿을 만들고 zip 파일을 업로드했으므로 제품을 스테이징으로 푸시하기 전에 [Marketplace 마케팅 콘텐츠 가이드](marketplace-publishing-push-to-staging.md)의 지침을 따르세요. 전체 Marketplace 게시 문서를 확인하려면 [시작: Azure Marketplace에 제품을 게시하는 방법](marketplace-publishing-getting-started.md)을 참조하세요.

다음 관련 문서를 참조할 수도 있습니다.

* VM 이미지: [Azure의 Virtual Machine 이미지 정보](https://msdn.microsoft.com/library/azure/dn790290.aspx)
* VM 확장: [Azure VM 확장 및 기능](../virtual-machines/extensions/features-windows.md)
...
* 스토리지 계정 제한: [스토리지 계정 제한을 모니터링하는 방법](https://blogs.msdn.com/b/mast/archive/2014/08/02/how-to-monitor-for-storage-account-throttling.aspx) 및 [Premium Storage](../virtual-machines/windows/premium-storage.md#scalability-and-performance-targets)
