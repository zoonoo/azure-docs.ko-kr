...
>[AZURE.IMPORTANT] **새 이미지 버전에서 디스크 수를 변경하지 마세요.** 이미지에서 데이터 디스크를 다시 구성해야 하는 경우 새 SKU를 정의합니다. 디스크 수가 다른 새 이미지 버전을 게시하면 자동 확장, ARM 템플릿을 통한 솔루션의 자동 배포 및 기타 시나리오에서 새 이미지 버전을 기반으로 새로운 배포를 포함할 가능성이 있습니다.
