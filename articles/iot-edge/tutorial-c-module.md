...
4. 명령 팔레트에서 **Azure IoT Edge: 새 IoT Edge 솔루션** 명령을 입력하고 실행합니다. 명령 팔레트에서 다음 정보를 제공하여 솔루션을 만듭니다. 
   ...
   5. 이전 섹션에서 만든 Azure Container Registry를 첫 번째 모듈에 대한 이미지 리포지토리로 지정합니다. **localhost:5000**을 복사한 로그인 서버 값으로 바꿉니다. 마지막 문자열은 **\<registry name\>.azurecr.io/cmodule** 형식입니다.
 
4. VS Code 창에서 IoT Edge 솔루션 작업 영역을 로드합니다. 여기에는 **modules** 폴더, **.vscode** 폴더, 배포 매니페스트 템플릿 파일 및 **.env** 파일이 있습니다. 기본 모듈 코드는 파이프 모듈로 구현됩니다. 

5. JSON 형식의 메시지를 필터링하려면 C용 JSON 라이브러리를 가져와야 합니다. 아무 JSON 라이브러리를 선택해도 되고, C 모듈에서 JSON을 구문 분석하는 라이브러리를 직접 작성해도 됩니다. 아래 단계에서는 [Parson](https://github.com/kgabis/parson)을 예제로 사용합니다.
   1. [Parson Github 리포지토리](https://github.com/kgabis/parson)에서 **parson.c** 및 **parson.h**를 다운로드합니다. 두 파일을 복사하여 **CModule** 폴더에 붙여넣습니다.
   2. **modules** > **CModule** > **CMakeLists.txt**를 엽니다. my_parson으로 parson 라이브러리를 가져올 아래 줄을 추가합니다.
   ...
   3. **CMakeLists.txt**의 `target_link_libraries`에 `my_parson`을 추가합니다.
   4. **modules** > **CModule** > **main.c**를 엽니다. JSON을 지원하기 위한 `parson.h`가 포함되도록 포함 섹션의 맨 아래에 다음 코드를 추가합니다.

      ```c
      #include "parson.h"
      ```

6. 포함 섹션 뒤에 `temperatureThreshold` 글로벌 변수를 추가합니다. 이 변수는 데이터가 IoT Hub로 전송되기 위해 측정된 온도가 초과해야 하는 값을 설정합니다. 

    ```c
    static double temperatureThreshold = 25;
    ```

7. `CreateMessageInstance` 함수 전체를 아래 코드로 바꿉니다. 이 함수는 콜백에 대한 컨텍스트를 할당합니다. 
...
8. `InputQueue1Callback` 함수 전체를 아래 코드로 바꿉니다. 이 함수는 실제 메시지 필터를 구현합니다. 
...
10. `SetupCallbacksForModule`의 `moduleTwinCallback`에 대한 새 콜백을 추가합니다. `SetupCallbacksForModule` 함수 전체를 아래 코드로 바꿉니다.
...
이전 섹션에서는 IoT Edge 솔루션을 만들고 CModule에 코드를 추가하여 보고된 머신 온도가 허용 가능한 임계값 미만인 메시지를 필터링했습니다. 이제 솔루션을 컨테이너 이미지로 빌드하고 컨테이너 레지스트리로 푸시해야 합니다. 
...
3. **deployment.template.json** 파일에는 Docker 레지스트르 자격 증명을 저장하는 **registryCredentials** 섹션이 있습니다. 실제 사용자 이름 및 암호 쌍은 Git를 무시하는 .env 파일에 저장됩니다.
...
솔루션을 빌드하도록 Visual Studio Code에 지시하면 먼저 배포 템플릿의 정보를 가져와서 새 **config** 폴더에 `deployment.json` 파일을 생성합니다. 그런 다음, 통합 터미널에서 두 개의 명령, 즉 `docker build`과 `docker push`를 실행합니다. 이 두 명령은 코드를 빌드하고, `CModule.dll`을 컨테이너화하고, 솔루션을 초기화할 때 지정한 컨테이너 레지스트리로 푸시합니다. 

VS Code 통합 터미널에 태그와 함께 전체 컨테이너 이미지 주소를 볼 수 있습니다. 이미지 주소는 `module.json` 파일의 정보에서 **\<리포지토리\>:\<버전\>-\<플랫폼\>**  형식으로 작성됩니다. 이 자습서에서 이 주소는 **registryname.azurecr.io/cmodule:0.0.1-amd64** 형식입니다.
...
1. IoT Hub용 연결 문자열로 Azure IoT Toolkit 확장을 구성합니다. 
    ...
    2. 탐색기에서 **Azure IoT Hub 장치**를 클릭하고 **...** 를 클릭합니다. **IoT Hub 선택**을 클릭합니다. 지침에 따라 Azure 계정에 로그인하고 IoT Hub를 선택합니다. 
       **IoT Hub 연결 문자열 설정**을 클릭하여 설정할 수도 있습니다. 팝업 창에서 IoT Edge 장치를 연결할 IoT Hub의 연결 문자열을 입력합니다.

2. Azure IoT Hub 장치 탐색기에서 IoT Edge 장치를 마우스 오른쪽 단추로 클릭한 다음, **IoT Edge 장치에 대한 배포 만들기**를 클릭합니다. **config** 폴더에서 **deployment.json** 파일을 선택한 다음, **에지 배포 매니페스트 선택**을 클릭합니다.

3. 새로고침 단추를 클릭합니다. **TempSensor** 모듈과 **$edgeAgent** 및 **$edgeHub**와 함께 실행되는 새 **CModule**이 표시됩니다. 
...
> [!IMPORTANT]
> Azure 리소스 그룹을 삭제하면 되돌릴 수 없습니다. 일단 삭제되면 리소스 그룹 및 해당 그룹에 포함된 모든 리소스가 영구적으로 삭제됩니다. 잘못된 리소스 그룹 또는 리소스를 자동으로 삭제하지 않도록 해야 합니다. 보관할 리소스가 포함된 기존 리소스 그룹 내에 IoT Hub를 만든 경우 리소스 그룹을 삭제하지 말고 IoT Hub 리소스만 삭제하면 됩니다.
>
