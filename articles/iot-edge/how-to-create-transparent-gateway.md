...
[프로덕션 인증서](how-to-install-production-certificates.md)로 구성 된 Azure IoT Edge 장치
...
Azure Portal에서 장치 세부 정보를 확인 하 고, Visual Studio 또는 Visual Studio Code에서 장치 상태를 확인 하거나, 장치 자체에서 명령 `iotedge list`을 실행 하 여 장치에서 실행 되는 모듈을 확인할 수 있습니다. 
...
아래 경로는 다운스트림 장치에서 `ai_insights`라는 모듈로 메시지를 보낸 다음 `ai_insights`에서 IoT Hub로 메시지를 보내는 예제입니다.
