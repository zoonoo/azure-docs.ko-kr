...
8. OpenSSL가 올바르게 설치 되었는지 확인 하 고 기존 인증서와 이름이 충돌 하지 않는지 확인 합니다. 문제가 있는 경우 스크립트 출력은 시스템에서 문제를 해결 하는 방법을 설명 해야 합니다.
...
X.509 인증서를 사용 하거나 자체 서명 된 인증서를 사용 하거나 CA (인증 기관) 서명 된 인증서를 사용 하 여 IoT 장치를 인증 하는 방법에는 두 가지가 있습니다. X.509 자체 서명 된 인증의 경우 (지문 인증이 라고도 함) IoT 장치에 새 인증서를 만들어야 합니다.
