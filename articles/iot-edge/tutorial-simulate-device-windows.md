...
Azure IoT Edge는 클라우드의 강력한 기능을 IoT(사물 인터넷) 장치로 가져다줍니다. 이 자습서에서는 센서 데이터를 생성하는 시뮬레이트된 IoT Edge 장치를 만드는 과정을 안내합니다. 다음 방법에 대해 알아봅니다.
...
다음 단계를 사용하여 IoT Edge 런타임을 설치하고 시작합니다.

1. 이전 섹션의 IoT Edge 장치 연결 문자열로 런타임을 구성합니다.
...
1. Docker를 확인하여 IoT Edge 에이전트가 모듈로 실행되고 있는지 알아봅니다.
