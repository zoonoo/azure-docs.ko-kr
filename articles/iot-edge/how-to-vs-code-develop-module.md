...
C#, Node.js 또는 Java에서 Visual Studio 코드로 작성된 모듈을 디버깅하는 방법에는 두 가지가 있습니다. Python 또는 C로 작성된 모듈을 디버깅하려면 Linux amd64 컨테이너의 프로세스에만 연결할 수 있습니다.

Visual Studio Code의 디버깅 기능에 익숙하지 않은 경우, [디버깅](https://code.visualstudio.com/Docs/editor/debugging)에 대해 읽어 보시기 바랍니다.

이 문서에서는 여러 아키텍처에 대해 여러 언어로 모듈을 개발하고 디버깅하는 방법에 대한 지침을 제공합니다. 현재 Visual Studio 코드는 C#, C, 파이썬, Node.js 및 Java로 작성된 모듈에 대한 지원을 제공합니다. 지원되는 장치 아키텍처는 X64 및 ARM32입니다. 지원되는 운영 체제, 언어 및 아키텍처에 대한 자세한 내용은 [언어 및 아키텍처 지원을](module-development.md#language-and-architecture-support)참조하십시오.

>[!NOTE]
>리눅스 ARM64 장치에 대한 개발 및 디버깅 지원은 [공개 미리보기에](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)있습니다. 자세한 내용은 [Visual Studio Code(미리 보기)에서 ARM64 IoT Edge 모듈 개발 및 디버그](https://devblogs.microsoft.com/iotdev/develop-and-debug-arm64-iot-edge-modules-in-visual-studio-code-preview)를 참조하세요.

## <a name="prerequisites"></a>사전 요구 사항

Windows, macOS 또는 Linux를 실행하는 컴퓨터 또는 가상 머신을 개발 머신으로 사용할 수 있습니다. 윈도우 컴퓨터에서 당신은 윈도우 또는 리눅스 모듈 중 하나를 개발할 수 있습니다. Windows 모듈을 개발하려면 버전 1809/빌드 17763 또는 최신 버전을 실행하는 Windows 컴퓨터를 사용합니다. Linux 모듈을 개발하려면 [Docker 데스크톱에 대한 요구 사항을](https://docs.docker.com/docker-for-windows/install/#what-to-know-before-you-install)충족하는 Windows 컴퓨터를 사용합니다.

[Visual Studio Code](https://code.visualstudio.com/)를 먼저 설치한 후 다음 확장을 추가합니다.

- [Azure IoT Tools](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools)
- [도커 확장](https://marketplace.visualstudio.com/items?itemName=PeterJausovec.vscode-docker)
- 개발 중인 언어에 특정한 Visual Studio 확장:
  - Azure 함수를 포함한 [C#: C# 확장](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)
  - 파이썬 : [파이썬 확장](https://marketplace.visualstudio.com/items?itemName=ms-python.python)
  - 자바: [비주얼 스튜디오 코드에 대 한 자바 확장 팩](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)
  - C: [C/C++ 확장](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)

또한 모듈을 개발하려면 언어별 몇 가지 도구를 추가로 설치해야 합니다.

- Azure Functions를 포함한 C#: [.NET Core 2.1 SDK](https://www.microsoft.com/net/download)

- 파이썬 : 파이썬 패키지를 설치하기위한 [파이썬과](https://www.python.org/downloads/) [핍](https://pip.pypa.io/en/stable/installing/#installation) (일반적으로 파이썬 설치에 포함).
...
- 자바 : [자바 SE 개발 키트 10](https://aka.ms/azure-jdks) [메이븐](https://maven.apache.org/). JDK 설치를 가리키려면 [`JAVA_HOME` 환경 변수를 설정](https://docs.oracle.com/cd/E19182-01/820-7851/inst_cli_jdk_javahome_t/)해야 합니다.

모듈 이미지를 빌드하고 배포하려면 모듈 이미지를 보관할 모듈 이미지와 컨테이너 레지스트리를 빌드하려면 Docker가 필요합니다.

- 개발 컴퓨터에 [Docker 커뮤니티 에디션.](https://docs.docker.com/install/)

- [Azure 컨테이너 레지스트리](https://docs.microsoft.com/azure/container-registry/) 또는 [도커 허브](https://docs.docker.com/docker-hub/repos/#viewing-repository-tags)

    > [!TIP]
    > 클라우드 레지스트리 대신 로컬 Docker 레지스트리를 프로토타입 및 테스트 목적으로 사용할 수 있습니다.

C에서 모듈을 개발하지 않는 한, IoT Edge 솔루션을 디버그, 실행 및 테스트하는 로컬 개발 환경을 설정하기 위해 Python 기반 [Azure IoT EdgeHub 개발 도구](https://pypi.org/project/iotedgehubdev/)도 필요합니다. 아직 수행하지 않은 경우 [파이썬 (2.7 / 3.6 / 3.7)과 핍을](https://www.python.org/) 설치한 다음 터미널에서이 명령을 실행하여 **iotedgehubdev를** 설치하십시오.
...
> [!NOTE]
> 현재 iotedgehubdev는 파이썬 3.8과 호환되지 않는 도커 파이 라이브러리를 사용합니다.
>
> 사전 설치된 파이썬 2.7을 포함하여 여러 파이썬이있는 경우 (예 : 우분투 `pip` 또는 `pip3` macOS에서) 올바른 파이썬을 사용하거나 **iotedgehubdev를** 설치하십시오.
...
1. 명령 팔레트에서 **Azure IoT Edge: 새 IoT 에지 솔루션을**입력하고 실행합니다.
...
- **modules** 폴더에는 각 모듈의 하위 폴더가 있습니다.  각 모듈의 폴더 에는 모듈을 빌드하고 배포하는 방법을 제어하는 파일 **module.json이**있습니다.  모듈 배포 컨테이너 레지스트리를 localhost에서 원격 레지스트리로 변경하려면 이 파일을 수정해야 합니다. 이 시점에서 모듈은 하나만 있습니다.  하지만 **Azure IoT Edge: Add IoT Edge Module** 명령을 사용하여 명령 팔레트에서 모듈을 더 추가할 수 있습니다.
...
시뮬레이션된 온도 모듈의 작동 방식을 보려면 [시뮬레이션온도Sensor.csproj 소스 코드를](https://github.com/Azure/iotedge/tree/master/edge-modules/SimulatedTemperatureSensor)참조하십시오.

## <a name="add-additional-modules"></a>모듈 더 추가

솔루션에 추가 모듈을 추가하려면 Azure IoT Edge: 명령 팔레트에서 **IoT 에지 모듈 추가** 명령을 실행합니다. Visual Studio Code Explorer 보기의 **modules** 폴더 또는 `deployment.template.json` 파일을 마우스 오른쪽 단추로 클릭한 후 **IoT Edge 모듈 추가**를 선택할 수도 있습니다.

## <a name="develop-your-module"></a>모듈 개발

솔루션과 함께 제공되는 기본 모듈 코드는 다음 위치에 있습니다.

- Azure Function(C#): **모듈 이름을 > * &lt;&gt;모듈**&lt;이름&gt;* > .cs**
...
모듈 및 deployment.template.json 파일은 솔루션을 빌드하고, 컨테이너 레지스트리에 푸시하고, 디바이스에 배포하여 코드를 변경하지 않고 테스트를 시작하도록 설정됩니다. 이 모듈은 소스(이 경우 데이터를 시뮬레이션하는 SimulatedTemperatureSensor 모듈)의 입력을 가져와 IoT Hub에 파이프로 간단하게 파이프하도록 제작되었습니다.
...
시뮬레이터를 설정하고 시작하려면 Azure IoT Edge: Visual Studio 코드 명령 팔레트에서 **단일 모듈용 IoT 에지 허브 시뮬레이터 를 시작합니다.** 메시지가 표시되면 기본 모듈 코드의 **input1** 값(또는 사용자 코드의 해당 값)을 애플리케이션의 입력 이름으로 사용합니다. 이 명령은 **iotedgehubdev** CLI를 트리거한 후 IoT Edge 시뮬레이터 및 테스트 유틸리티 모듈 컨테이너를 시작합니다. 시뮬레이터가 단일 모듈 모드에서 성공적으로 시작된 경우 통합 터미널에서 다음과 같은 출력을 볼 수 있습니다. 또한 메시지를 전송하는 데 도움이 되는 `curl` 명령도 볼 수 있습니다. 나중에 필요합니다.
...
1. 개발 언어의 요구 사항에 따라 디버그할 환경을 준비하고, 모듈에서 중단점을 설정하고, 사용할 디버그 구성을 선택합니다.
   - **C #**
     - Visual Studio Code 통합 터미널에서 디렉터리를 *** &lt;모듈&gt; 이름*** 폴더로 변경한 다음 다음 명령을 실행하여 .NET Core 응용 프로그램을 빌드합니다.
     ...
     - **보기 > 디버그**를 선택하여 Visual Studio Code 디버그 보기로 이동합니다. 드롭다운에서 **모듈 이름 * &lt;&gt; * 로컬 디버그(.NET Core)를** 디버그 구성을 선택합니다.

        > [!NOTE]
        > 에서 .NET `TargetFramework` Core가 프로그램 경로와 `launch.json`일치하지 않는 경우 Visual Studio Code에서 `launch.json` 이 프로그램을 `TargetFramework` 성공적으로 시작할 수 있도록 .csproj 파일의 프로그램 경로를 수동으로 업데이트해야 합니다.

   - **Node.js**
     - Visual Studio Code 통합 터미널에서 디렉터리를 *** &lt;모듈&gt; 이름*** 폴더로 변경한 다음 다음 명령을 실행하여 노드 패키지를 설치합니다.
     ...
     - **보기 > 디버그**를 선택하여 Visual Studio Code 디버그 보기로 이동합니다. 드롭다운에서 ** * &lt;모듈&gt; 이름* 로컬 디버그(Node.js)의** 디버그 구성을 선택합니다.
   - **Java**
     - `App.java` 파일을 열고 중단점을 추가합니다.

     - **보기 > 디버그**를 선택하여 Visual Studio Code 디버그 보기로 이동합니다. 드롭다운에서 **모듈 이름 * &lt;&gt; * 로컬 디버그(Java)를 디버그** 구성을 선택합니다.
...
1. 디버깅 세션을 중지하려면 중지 단추를 선택하거나 **Shift + F5를**누린 다음 Azure IoT Edge: 명령 팔레트에서 **IoT 에지 시뮬레이터를 중지하여** 시뮬레이터를 중지하고 정리합니다.
...
1. Visual Studio Code 디버그 보기로 이동하고 모듈의 디버그 구성 파일을 선택합니다. 디버그 옵션 이름은 모듈 ** * &lt;이름&gt; * 원격 디버그와 유사해야 합니다.**

1. **디버깅 시작**을 선택하거나 **F5** 키를 누릅니다. 연결할 프로세스를 선택합니다.

1. Visual Studio Code 디버그 보기의 왼쪽 패널에 변수가 표시됩니다.

1. 디버깅 세션을 중지하려면 먼저 중지 단추를 선택하거나 **Shift + F5를**누린 다음 Azure IoT Edge: 명령 팔레트에서 **IoT 에지 시뮬레이터 중지를** 선택합니다.
...
1. Python에서 모듈을 개발하는 경우 계속하기 전에 다음 단계를 따르세요.
   ...
   - 디버그할 콜백에 다음 단일 코드 줄을 추가합니다.
     ...
     예를 들어 `receive_message_listener` 함수를 디버깅하려는 경우 아래와 같이 해당 코드 줄을 삽입합니다.
...
- **Node.js**
  ...
  - 개발 컴퓨터에서 Visual Studio 코드를 연 다음 `launch.json` ** * &lt;모듈&gt; 이름* 원격 디버그(Node.js)** 프로필(또는 ** * &lt;모듈&gt; 이름이* Windows 컨테이너로 실행되는 경우 모듈 이름 원격 디버그(Node.js)** 프로필이 디버깅되는 컴퓨터의 IP가 되도록 편집합니다.

- **Java**

  - `ssh -f <username>@<target-machine> -L 5005:127.0.0.1:5005 -N`을 실행하여 디버그할 머신에 대한 SSH 터널을 빌드합니다.
  
  - 개발 컴퓨터에서 Visual Studio 코드를 열고 ** * &lt;모듈 이름&gt; * 원격 디버그(Java)** 프로필을 `launch.json` 편집하여 대상 컴퓨터에 연결할 수 있습니다. Visual Studio Code를 사용하여 `launch.json`을 편집하고 Java를 디버그하는 방법을 자세히 알아보려면 [디버거 구성](https://code.visualstudio.com/docs/java/java-debugging#_configuration) 섹션을 참조하세요.
...
- **Python**
  ...
  - 개발 컴퓨터에서 Visual Studio 코드를 연 다음 `launch.json` 모듈 `host` ** * &lt;이름&gt; * 원격 디버그(Python)** 프로필의 값이 대상 컴퓨터 대신 `localhost`대상 컴퓨터의 IP 주소를 사용되도록 편집합니다.

### <a name="debug-your-module"></a>모듈 디버그

1. Visual Studio Code 디버그 보기에서 모듈의 디버그 구성 파일을 선택합니다. 디버그 옵션 이름은 모듈 ** * &lt;이름&gt; * 원격 디버그와 유사해야 합니다.**

1. 개발 언어의 모듈 파일을 열고 중단점을 추가합니다.

   - **Azure 함수(C#)**: 파일에 `<your module name>.cs`중단점을 추가합니다.
   - **C #**: 파일에 `Program.cs`중단점을 추가합니다.
   - **Node.js**: 파일에 `app.js`중단점을 추가합니다.
   - **Java**: 파일에 `App.java`중단점을 추가합니다.
   - **파이썬**: `ptvsd.break_into_debugger()` 라인을 추가 `main.py`한 콜백 메서드의 파일에 중단점을 추가합니다.
   - **C**: 파일에 `main.c`중단점을 추가합니다.
...
모듈을 빌드한 후 [Visual Studio 코드에서 Azure IoT Edge 모듈을 배포하는](how-to-deploy-modules-vscode.md)방법을 알아봅니다.
