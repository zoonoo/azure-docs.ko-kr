...
C에서 모듈을 개발하지 않는 한, IoT Edge 솔루션을 디버그, 실행 및 테스트하는 로컬 개발 환경을 설정하기 위해 Python 기반 [Azure IoT EdgeHub 개발 도구](https://pypi.org/project/iotedgehubdev/)도 필요합니다. 아직 수행 하지 않은 경우 [Python (2.7/3.6 +) 및 Pip](https://www.python.org/) 를 설치한 다음 터미널에서이 명령을 실행 하 여 **iotedgehubdev** 를 설치 합니다.
