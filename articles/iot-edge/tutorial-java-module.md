---
title: 자습서 - Azure IoT Edge를 사용하는 사용자 지정 Java 모듈 자습서
...
비즈니스 논리를 직접 Azure IoT Edge 디바이스에 구현하는 코드를 배포하려면 IoT Edge 모듈을 사용할 수 있습니다. 이 자습서에서는 센서 데이터를 필터링하는 IoT Edge 모듈을 만들고 배포하는 과정을 안내합니다. 빠른 시작 문서의 시뮬레이션된 디바이스에 Azure IoT Edge 배포에서 만든 시뮬레이션된 IoT Edge 디바이스를 사용합니다. 이 자습서에서는 다음 작업 방법을 알아봅니다.
...
이 자습서에서는 **Visual Studio Code** 를 사용하여 **Java** 에서 모듈을 개발하고 IoT Edge 디바이스에 배포하는 방법을 보여줍니다. IoT Edge는 Windows 컨테이너로 빌드된 Java 모듈을 지원하지 않습니다.
...
* [Maven](https://maven.apache.org/)
   ...
   >[!TIP]
   >Java 및 Maven 설치 프로세스는 시스템에 환경 변수를 추가합니다. 설치를 완료한 후 열려 있는 Visual Studio Code 터미널, Powershell 또는 명령 프롬프트 인스턴스를 다시 시작합니다. 이 단계를 통해 이러한 유틸리티는 앞으로 Java 및 Maven 명령을 인식합니다.
...
Java 모듈을 처음 만드는 경우 maven 패키지를 다운로드하는 데 몇 분이 걸릴 수 있습니다. 솔루션이 준비되면 VS Code 창에서 IoT Edge 솔루션 작업 영역을 로드합니다. 솔루션 작업 영역에는 다음과 같은 최상위 구성 요소 5개가 포함됩니다.

* **modules** 폴더에는 모듈 및 컨테이너 이미지로 모듈을 빌드하기 위한 Docker 파일의 Java 코드가 포함되어 있습니다.
* **\.env** 파일은 컨테이너 레지스트리 자격 증명을 저장합니다.
* **deployment.template.json** 파일에는 IoT Edge 런타임에서 디바이스에 모듈을 배포하는 데 사용하는 정보가 포함되어 있습니다.
* **deployment.debug.template.json** 파일에는 모듈의 디버그 버전이 포함되어 있습니다.
* 이 자습서에서는 **\.vscode** 폴더 또는 **\.gitignore** 파일을 편집하지 않습니다.
...
4. 배포되어 실행 중인 모듈의 목록을 보려면 디바이스 아래에서 **모듈** 을 확장합니다. 새로고침 단추를 클릭합니다. **SimulatedTemperatureSensor** 모듈과 **$edgeAgent** 및 **$edgeHub** 와 함께 실행되는 새 **JavaModule** 이 표시됩니다.  
...
2. IoT Hub에 메시지가 들어오는 것을 확인합니다. 메시지가 도착하는 데 시간이 걸릴 수 있습니다. IoT Edge 디바이스는 새 배포를 수신하고 모든 모듈을 시작해야 합니다. 그런 다음, JavaModule 코드를 변경할 경우 머신 온도가 25도에 도달할 때까지 기다렸다가 메시지를 보냅니다. 또한 온도 임계값에 도달하는 모든 메시지에 메시지 유형 **경고** 를 추가합니다.
