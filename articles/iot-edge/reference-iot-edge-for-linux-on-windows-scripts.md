...
| vnetType | **투명** 또는 **ICS** | 가상 스위치의 유형입니다. **기본값**: 투명 합니다. 투명은 외부 스위치를 나타내며, ICS는 내부 스위치를 참조 합니다. |
...
>[!NOTE]
>기본적으로 프로세스에서 이름이 인 외부 스위치를 찾을 수 없는 경우 `External` IP 주소를 가져올 기존 외부 스위치를 검색 합니다. 사용할 수 있는 외부 스위치가 없으면 내부 스위치를 검색 합니다. 사용할 수 있는 내부 스위치가 없는 경우 IP 주소를 가져올 기본 스위치를 만들려고 합니다.
