...
IoT Edge 솔루션을 디버그, 실행 및 테스트하기 위한 로컬 개발 환경을 설치하려면 [Azure IoT EdgeHub Dev Tool](https://pypi.org/project/iotedgehubdev/)이 필요하며, [Python(2.7/3.6) 및 Pip](https://www.python.org/)를 설치합니다. Pip는 Python 설치 관리자에 포함되어 있습니다. 그런 후 터미널에서 아래 명령을 실행하여 **iotedgehubdev**를 설치합니다.
...
IoT Edge Node.js 모듈은 Azure IoT Node.js 디바이스 SDK에 따라 달라집니다. 기본 모듈 코드에서 환경 설정 및 입력 이름을 사용하여 **ModuleClient**를 초기화합니다. 즉, IoT Edge Node.js 모듈에는 시작 및 실행을 위한 환경 설정이 필요하므로 입력 채널로 메시지를 전송하거나 라우트해야 합니다. 기본 Node.js 모듈은 하나의 입력 채널만 포함하며 이름은 **input1**입니다.
...
### <a name="debug-nodejs-module-in-launch-mode"></a>시작 모드에서 Node.js 모듈 디버그

1. 통합 터미널에서 디렉터리를 **NodeModule** 폴더로 변경하고 다음 명령을 실행하여 Node 패키지 설치
...
3. VS Code 디버그 보기로 이동합니다. 디버그 구성 **ModuleName 로컬 디버그(Node.js)** 를 선택합니다. 
...
기본 솔루션에는 2개의 모듈, 즉 시뮬레이트된 온도 센서 모듈과 Node.js 파이프 모듈이 포함되어 있습니다. 시뮬레이트된 온도 센서는 Node.js 파이프 모듈로 메시지를 계속 전송하고, 해당 메시지는 IoT Hub에 파이프됩니다. 만든 모듈 폴더에는 서로 다른 컨테이너 유형에 사용되는 여러 Docker 파일이 있습니다. **.debug** 확장명으로 끝나는 이러한 파일을 사용하여 테스트할 모듈을 빌드합니다. 현재, Node.js 모듈은 linux-amd64, windows-amd64 및 linux-arm32v7 컨테이너에서만 디버깅을 지원합니다.
