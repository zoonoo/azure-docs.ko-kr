...
이 모듈을 수반 **deviceToCloudUpload** 하 고 **deviceAutoDelete** 기능입니다.
> [!NOTE]
...
**deviceToCloudUpload** 는 일시적인 인터넷 연결 지원 사용 하 여 Azure에 로컬 blob storage에서 데이터를 자동으로 업로드할 수 있는 기능을 구성할 수 있습니다. 할 수 있습니다.

- / 사용 안 함 deviceToCloudUpload 기능입니다.
...
- 클라우드 저장소에 업로드 완료 된 후 blob을 즉시 삭제 하는 기능 선택
- 전체 blob 업로드를 수행 (사용 하 여 `Put Blob` 작업) 및 블록 수준 업로드 (사용 하 여 `Put Block` 고 `Put Block List` 작업).

이 모듈의 블록 blob 구성 된 경우 블록 수준 업로드를 사용 합니다. 몇 가지 일반적인 시나리오는 다음과 같습니다.
...
**deviceAutoDelete** 기능은 구성할 수 있는 모듈을 자동으로 blob의 로컬 저장소에서 지정 된 기간 (분 단위로 측정) 만료 되 면 삭제 됩니다. 할 수 있습니다.

- / 사용 안 함 deviceAutoDelete 기능입니다.
- 가 이후에 blob가 자동으로 삭제 된 시간 (분) (deleteAfterMinutes)를 지정 합니다.
- 이 업로드 하는 동안 deleteAfterMinutes 값 만료 된 경우 blob을 유지 하는 기능을 선택 합니다.
...
DeviceToCloudUploadProperties 및 deviceAutoDeleteProperties 설정 하려면 원하는 속성을 사용 합니다. 배포 중에 설정 또는 다시 배포 하지 않아도 모듈 쌍을 편집 하 여 나중에 변경할 수 있습니다. 에 대 한 "모듈 쌍" 확인을 좋습니다 `reported configuration` 및 `configurationValidation` 값 올바르게 전파 되도록 합니다.
...
- 외부 모듈 또는 응용 프로그램에서 실행 되는 보다는 IoT Edge 모듈에서 Azure Blob Storage는를 실행 하면 네트워크 설정에 따라 다른 장치에서 외부 모듈 또는 응용 프로그램의 데이터 트래픽을 장치를 연결할 수 있도록 blob 끝점 중 하나입니다 IoT Edge 모듈에서 Azure Blob Storage를 실행 합니다.
...
Azure Blob Storage 설명서는 여러 언어에서 빠른 시작 샘플 코드를 포함합니다. 이 샘플을 IoT Edge에서 Azure Blob Storage를 로컬 blob 저장소 모듈에 연결 하는 blob 끝점을 변경 하 여 테스트를 실행할 수 있습니다.

다음 빠른 시작 샘플 에서도 blob 저장소 모듈와 함께 IoT Edge 모듈로 배포할 수 있도록 IoT Edge에서 지원 되는 언어를 사용 합니다.
...
사용할 수 있습니다 [Azure Storage 탐색기](https://azure.microsoft.com/features/storage-explorer/) 로컬 저장소 계정에 연결 합니다.
...
다음은 [docker 허브에서 릴리스](https://hub.docker.com/_/microsoft-azure-blob-storage) 이 모듈에 대 한
