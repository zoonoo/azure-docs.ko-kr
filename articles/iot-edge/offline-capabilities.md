...
description: IoT Edge 장치 및 모듈을 오프라인으로 더 오래 작동하는 방법과 IoT Edge를 사용하여 일반 IoT 장치를 오프라인으로 작동하는 방법을 이해합니다.
author: kgremban
manager: philmea
ms.author: kgremban
ms.date: 09/20/2018
...
Azure IoT Edge는 IoT Edge 장치에서 확장된 오프라인 작업을 지원하고, 비 Edge 자식 장치에서도 오프라인 작업을 지원합니다. IoT Edge 장치를 IoT Hub에 연결할 수 있는 한, 해당 장치 및 자식 장치는 일시적으로 인터넷에 연결하여 또는 인터넷 연결 없이 계속 작동할 수 있습니다. 
...
IoT Edge 장치가 오프라인으로 전환되면 Edge 허브는 세 가지 역할을 맡습니다. 첫째, 업스트림으로 이동하는 모든 메시지를 저장하고 장치가 다시 연결될 때까지 보관합니다. 둘째, 모듈 및 자식 장치가 계속 작동할 수 있도록 IoT Hub를 대신하여 모듈 및 자식 장치를 인증합니다. 셋째, 일반적으로 IoT Hub를 통과하는 자식 장치 간 통신을 지원합니다. 
...
IoT Edge 장치 및 할당된 자식 장치는 초기 일회성 동기화 후 오프라인으로 무기한 작동할 수 있습니다. 그러나 메시지 저장소는 TTL(Time to Live) 및 메시지 저장에 사용 가능한 디스크 공간에 따라 달라집니다. 
...
동일한 IoT Hub에 등록된 모든 비 Edge 장치는 자식 장치가 될 수 있습니다. 새 장치를 만들 때 또는 부모 IoT Edge 장치 또는 자식 IoT 장치의 장치 세부 정보 페이지에서 부모-자식 관계를 관리할 수 있습니다. 
...
장치를 오프라인으로 오랫동안 실행한 후 생성된 모든 메시지를 수집하려면 모든 메시지를 저장할 수 있도록 Edge 허브를 구성해야 합니다. 장기 메시지 저장소를 사용하도록 Edge 허브를 변경하는 두 가지 방법이 있습니다. 먼저 TTL(Time to Live)을 늘린 다음, 메시지 저장을 위한 추가 디스크 공간을 추가합니다. 
...
기본적으로 메시지는 Edge 허브 컨테이너의 파일 시스템에 저장됩니다. 저장소가 오프라인 요구 사항을 처리하기에 충분하지 않은 경우 로컬 저장소를 IoT Edge 장치에 전용할 수 있습니다. 컨테이너의 저장소 폴더를 가리키는 Edge 허브에 대한 환경 변수를 만들어야 합니다. 그런 다음, 만들기 옵션을 사용하여 해당 저장소 폴더를 호스트 머신의 폴더에 바인딩합니다. 
