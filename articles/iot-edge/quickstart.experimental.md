...
이 빠른 시작에서는 사용자가 Windows를 실행하는 컴퓨터 또는 가상 머신을 사용하여 IoT 장치를 시뮬레이션한다고 가정합니다. 가상 머신에서 Windows를 실행하는 경우 [중첩된 가상화][lnk-nested]를 사용하도록 설정하고 최소 2GB 메모리를 할당하세요. 
