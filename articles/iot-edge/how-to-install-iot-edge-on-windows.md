...
* Windows 관리 센터에 대 한 Azure IoT Edge 확장을 설치 하 여 Windows 관리 센터에 액세스:
   ...
   1. 다운로드 한 설치 관리자를 실행 하 고 설치 마법사 프롬프트에 따라 Windows 관리 센터를 설치 합니다. 

   1. 설치 되 면 지원 되는 브라우저를 사용 하 여 Windows 관리 센터를 엽니다. 지원 되는 브라우저에는 Microsoft Edge (Windows 10, 버전 1709 이상), Google Chrome 및 Microsoft Edge Insider이 포함 됩니다.
...
1. **2.2 설정** 탭에서 배포의 구성 설정을 검토 합니다. 설정에 만족 하면 **다음** 을 선택 합니다.

   ![배포의 구성 설정 검토](./media/how-to-install-iot-edge-on-windows/default-deployment-configuration-settings.png)

   >[!NOTE]
   >Windows 가상 컴퓨터를 사용 하는 경우 외부 스위치 대신 기본 스위치를 사용 하 여 배포에서 만든 Linux 가상 컴퓨터에서 IP 주소를 가져올 수 있도록 하는 것이 좋습니다.
   >
   >기본 스위치를 사용 하면 Linux 가상 머신에 내부 IP 주소가 할당 됩니다. 이 내부 IP 주소는 Windows 가상 머신 외부에서 연결할 수 없지만, Windows 가상 머신에 로그온 한 상태에서는 로컬에 연결할 수 있습니다.
   >
   >Windows Server를 사용 하는 경우 Windows에서 Linux에 대 한 Azure IoT Edge 기본 스위치를 자동으로 지원 하지 않습니다. 로컬 Windows Server 가상 머신의 경우 Linux 가상 머신이 외부 스위치를 통해 IP 주소를 가져올 수 있는지 확인 합니다. Azure의 Windows Server 가상 컴퓨터의 경우 Windows에서 Linux 용 IoT Edge를 배포 하기 전에 내부 스위치를 설정 합니다.
...
1. Windows 배포에서 Linux에 대 한 IoT Edge를 만듭니다.
   ...
   > [!NOTE]
   > 매개 변수 없이이 명령을 실행 하거나 선택적으로 매개 변수를 사용 하 여 배포를 사용자 지정할 수 있습니다. [Windows PowerShell 스크립트 참조에서 Linux에 대 한 IoT Edge](reference-iot-edge-for-linux-on-windows-scripts.md#deploy-eflow) 를 참조 하 여 매개 변수 의미 및 기본값을 확인할 수 있습니다.
...
1. PowerShell 세션에서 다음 명령을 사용 하 여 Windows 가상 머신에서 Linux 용 IoT Edge에 로그인 합니다.
   ...
   >[!NOTE]
   >가상 컴퓨터에 대 한 SSH를 허용 하는 유일한 계정은 가상 컴퓨터를 만든 사용자입니다.
