...
`<HostStoragePath>` 및 `<ModuleStoragePath>`을 호스트 및 모듈 저장소 경로로 바꿉니다. 두 값 모두 절대 경로 여야 합니다.

예를 들어 Linux 시스템에서 `"Binds":["/etc/iotedge/storage/:/iotedge/storage/"]`는 호스트 시스템의 디렉터리 **/etc/iotedge/storage** 가 컨테이너의 **/iotedge/storage/** 디렉터리에 매핑되는 것을 의미 합니다. 또 다른 예로 Windows 시스템에서 `"Binds":["C:\\temp:C:\\contemp"]`은 호스트 시스템의 **c:\\temp** 디렉터리가 컨테이너의 **c:\\연결 emp** 에 매핑되는 것을 의미 합니다.

또한 Linux 장치에서 모듈의 사용자 프로필에 호스트 시스템 디렉터리에 대 한 읽기, 쓰기 및 실행 권한이 있어야 합니다. IoT Edge 허브가 장치의 로컬 저장소에 메시지를 저장 하도록 설정 하는 이전 예제로 돌아가면 UID 1000 사용자 프로필에 대 한 사용 권한을 부여 해야 합니다. IoT Edge 에이전트는 루트로 작동 하므로 추가 권한이 필요 하지 않습니다. `chown`를 사용 하 여 디렉터리 소유자를 변경한 `chmod` 후 권한을 변경 하는 등 Linux 시스템에서 디렉터리 권한을 관리 하는 방법에는 여러 가지가 있습니다. 예를 들면 다음과 같습니다.
