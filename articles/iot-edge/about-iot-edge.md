...
분석은 IoT 솔루션의 비즈니스 가치를 높이지만 모든 분석을 클라우드에서 할 필요는 없습니다. 응급 상황에 최대한 신속하게 대응하려면 에지에서 변칙 검색 워크로드를 실행하면 됩니다. 대역폭 비용을 줄이고 수테라바이트의 원시 데이터를 전송하는 일이 없도록 하려면 로컬로 데이터를 정리하고 집계한 다음, 인사이트만 클라우드로 보내서 분석하면 됩니다.
...
제조사와 모델이 다르거나 지리적으로 분산된 수백만 대의 IoT 디바이스의 소프트웨어 수명 주기를 관리하기란 힘든 일입니다. 특정 유형의 디바이스에 대한 워크로드를 만들어서 구성하고, 모든 디바이스에 배포하고, 모니터링하면서 오작동하는 디바이스를 찾아내야 합니다. 이러한 작업은 디바이스 단위로 수행할 수 없고 대규모로 수행해야 합니다.
...
[시뮬레이션된 디바이스에 IoT Edge를 배포](quickstart.md)하여 학습한 개념을 확인해 보세요.
