...
이 문서에서는 런타임 모듈 트윈스의 desired 속성 및 reported 속성을 제공합니다. For more information on how to deploy modules on IoT Edge devices, see [Learn how to deploy modules and establish routes in IoT Edge](module-composition.md).

A module twin includes: 

* **desired 속성**. The solution backend can set desired properties, and the module can read them. The module can also receive notifications of changes in the desired properties. Desired properties are used along with reported properties to synchronize module configuration or conditions.

* **reported 속성**. The module can set reported properties, and the solution backend can read and query them. Reported properties are used along with desired properties to synchronize module configuration or conditions. 
...
| modules.{moduleId}.imagePullPolicy | {"on-create" \| "never"} | 아닙니다. |
...
| lastDesiredStatus.code | This status code refers to the last desired properties seen by the IoT Edge agent. 허용되는 값: `200` 성공, `400` 잘못된 구성, `412` 잘못된 스키마 버전, `417` 비어 있는 desired 속성, `500` 실패 |
...
| -------- | ----------- | -------- |
| schemaVersion | "1.0"이어야 합니다. | yes |
| routes.{routeName} | IoT Edge 허브 경로를 나타내는 문자열입니다. For more information, see [Declare routes](module-composition.md#declare-routes). | `routes` 요소는 존재하지만 비어 있을 수 있습니다. |
...
| lastDesiredStatus.code | The status code referring to last desired properties seen by the IoT Edge hub. 허용되는 값: `200` 성공, `400` 잘못된 구성, `500` 실패 |
