...
이 문서에서는 런타임 모듈 트윈스의 desired 속성 및 reported 속성을 제공합니다. IoT Edge 장치에 모듈을 배포 하는 방법에 대 한 자세한 내용은 참조 하세요. [모듈을 배포 하 여 IoT Edge의 경로 설정 하는 방법을 알아봅니다](module-composition.md)합니다.

모듈 쌍에는 다음이 포함 됩니다. 

* **desired 속성**. 솔루션 백 엔드는 desired 속성을 설정할 수 있습니다 및 모듈을 읽을 수 있습니다. 모듈의 desired 속성 변경 알림을 받을 수도 있습니다. Desired 속성은 reported 속성과 함께 모듈 구성 또는 조건을 동기화에 사용 됩니다.

* **reported 속성**. 모듈에는 reported 속성을 설정할 수 있으며 솔루션 백 엔드에서 읽고 쿼리할 수 있습니다. Reported 속성은 모듈 구성 또는 조건을 동기화 하 desired 속성과 함께 사용 됩니다. 
...
이 마지막 정보, 현재 원하는 속성의 복사본 장치가 마지막 desired 속성이 적용 되는지 여전히 이전 배포 매니페스트를 실행 중인지 여부에 게 유용 합니다.
...
| lastDesiredStatus.code | 이 상태 코드 마지막 desired 속성이 IoT Edge 에이전트에서 표시를 가리킵니다. 허용되는 값은 다음과 같습니다. `200` 성공, `400` 잘못된 구성, `412` 잘못된 스키마 버전, `417` 비어 있는 desired 속성, `500` 실패 |
...
| systemModules.edgeHub.exitCode | IoT Edge 허브 컨테이너에서 보고 한 컨테이너를 종료 하는 경우 종료 코드 |
...
| modules.{moduleId}.exitCode | 컨테이너를 종료 하는 경우 모듈 컨테이너에서 보고 한 종료 코드 |
...
| -------- | ----------- | -------- |
| schemaVersion | "1.0"이어야 합니다. | 예 |
| routes.{routeName} | IoT Edge 허브 경로를 나타내는 문자열입니다. 자세한 내용은 [경로 선언](module-composition.md#declare-routes)합니다. | `routes` 요소는 존재하지만 비어 있을 수 있습니다. |
| storeAndForwardConfiguration.timeToLiveSecs | 시간 (초)는 IoT Edge 허브 유지 여부 라우팅 끝점에서 연결이 끊어진 경우 메시지 IoT Hub 또는 로컬 모듈입니다. 임의의 양의 정수 값일 수 있습니다. | 예 |
...
| lastDesiredStatus.code | IoT Edge 허브에서 표시 한 마지막 desired 속성을 참조 하는 상태 코드입니다. 허용되는 값은 다음과 같습니다. `200` 성공, `400` 잘못된 구성, `500` 실패 |
| lastDesiredStatus.description | 상태 텍스트 설명입니다. |
...
| clients.{device or moduleId}.lastConnectTime | 마지막으로 장치 또는 모듈과 연결 된 시간입니다. |
| clients.{device or moduleId}.lastDisconnectTime | 마지막으로 장치 또는 모듈과 연결이 끊어졌습니다. |
