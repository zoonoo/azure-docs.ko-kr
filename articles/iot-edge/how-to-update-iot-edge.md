---
title: 최신 버전의 Azure IoT Edge로 장치 업데이트 | Microsoft Docs
...
IoT Edge 서비스가 새 버전을 릴리스하면 최신 기능 및 개선된 보안을 사용하도록 IoT Edge 장치를 업데이트할 수 있습니다. 이 문서에서는 새 버전을 사용할 수 있을 때 IoT Edge 장치를 업데이트하는 방법에 대한 정보를 제공합니다. 

새 버전으로 전환하려면 IoT Edge 장치의 두 가지 구성 요소를 업데이트해야 합니다. 첫 번째는 장치에서 실행되고 장치가 시작될 때 런타임을 시작하는 보안 디먼입니다. 현재, 보안 디먼은 장치 자체에서만 업데이트할 수 있습니다. 두 번째 구성 요소는 런타임으로, 에지 허브 및 에지 에이전트 모듈로 구성됩니다. 배포 구성 방법에 따라 장치에서 또는 원격으로 런타임을 업데이트할 수 있습니다. 
...
IoT Edge 보안 디먼은 IoT Edge 장치에서 패키지 관리자를 사용하여 업데이트해야 하는 네이티브 구성 요소입니다. 

`iotedge version` 명령을 사용하여 장치에서 실행 중인 보안 디먼의 버전을 확인합니다. 

### <a name="linux-devices"></a>Linux 장치

Linux 장치에서는 apt-get 또는 해당 패키지 관리자를 사용하여 보안 디먼을 업데이트합니다. 
...
Windows 장치에서는 PowerShell 스크립트를 사용하여 보안 디먼을 제거했다가 다시 설치합니다. 설치 스크립트는 최신 버전의 보안 디먼을 자동으로 끌어옵니다. 설치 프로세스 중에 장치에 대한 연결 문자열을 다시 제공해야 합니다. 
...
IoT Edge 장치가 Windows 컨테이너를 사용하는지 또는 Linux 컨테이너를 사용하는지에 따라 보안 디먼을 다시 설치합니다. **\<Windows 또는 Linux\>** 구를 컨테이너 운영 체제 중 하나로 바꿉니다. 
...
* **롤링 태그** - 버전 번호의 처음 두 개 값만 사용하여 해당 숫자와 일치하는 최신 이미지를 가져옵니다. 예를 들어, 최신 1.0.x 버전을 가리키는 새 릴리스가 있을 때마다 1.0이 업데이트됩니다. IoT Edge 장치의 컨테이너 런타임이 이미지를 다시 끌어오면 런타임 모듈은 최신 버전으로 업데이트됩니다. 이 접근 방법은 개발 목적으로 제안됩니다. Azure Portal에서 배포할 때는 기본적으로 롤링 태그가 사용됩니다. 
...
배포에 롤링 태그를 사용하는 경우(예: mcr.microsoft.com/azureiotedge-hub:  **1.0**) 장치의 컨테이너 런타임이 강제로 최신 버전의 이미지를 가져오도록 해야 합니다. 

IoT Edge 장치에서 이미지의 로컬 버전을 삭제합니다. 
...
IoT Edge 서비스는 최신 버전의 런타임 이미지를 끌어오고 해당 이미지를 장치에서 자동으로 다시 시작합니다. 

### <a name="update-a-specific-tag-image"></a>특정 태그 이미지 업데이트

배포에서 특정 태그를 사용하는 경우(예: mcr.microsoft.com/azureiotedge-hub:**1.0.2**) 배포 매니페스트에서 태그를 업데이트하고 변경 내용을 장치에 적용하기만 하면 됩니다. 
