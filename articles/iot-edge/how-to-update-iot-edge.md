...
>[!NOTE]
>매개 `-OfflineInstallationPath` 변수는 제공 된 디렉터리에서 **Microsoft-Azure-IoTEdge** 이라는 파일을 찾습니다. IoT Edge 버전 1.0.9부터 사용 가능한 두 개의 .cab 파일 (AMD64 장치와 ARM32 용)이 있습니다. 장치에 맞는 파일을 다운로드 한 다음 파일 이름을 변경 하 여 아키텍처 접미사를 제거 합니다.

업데이트 옵션에 대 한 자세한 내용을 보려면 명령을 `Get-Help Update-IoTEdge -full` 사용 하거나 [모든 설치 매개 변수](how-to-install-iot-edge-windows.md#all-installation-parameters)를 참조 하십시오.
...
장치를 오프 라인으로 업데이트 하거나 최신 버전이 아닌 IoT Edge의 특정 버전으로 업데이트 하려는 경우 `-OfflineInstallationPath` 매개 변수를 사용 하 여이 작업을 수행할 수 있습니다.
...
4. 오프 라인 구성 요소로 업데이트 하려면 [도트 원본](https://docs.microsoft.com/powershell/module/microsoft.powershell.core/about/about_scripts?view=powershell-7#script-scope-and-dot-sourcing) 에 PowerShell 스크립트의 로컬 복사본을 사용 합니다. 그런 다음 매개 변수 `-OfflineInstallationPath` 를 `Update-IoTEdge` 명령의 일부로 사용 하 고 파일 디렉터리에 대 한 절대 경로를 제공 합니다. 예를 들면 다음과 같습니다.
