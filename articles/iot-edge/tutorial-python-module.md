...
4. 명령 팔레트에서 **Azure: Sign in**(Azure: 로그인) 명령을 입력 및 실행하고, 지침에 따라 Azure 계정에 로그인합니다. 이미 로그인한 경우 이 단계는 건너뛸 수 있습니다.

5. 명령 팔레트에서 **Azure IoT Edge: New IoT Edge solution**(Azure IoT Edge: 새 IoT Edge 솔루션) 명령을 입력 및 실행합니다. 명령 팔레트에서 다음 정보를 제공하여 솔루션을 만듭니다. 
...
7. PythonModule이 포함된 솔루션 폴더로 이동합니다. **config** 폴더를 열고 **deployment.json** 파일을 선택합니다. **Edge 배포 매니페스트 선택**을 클릭합니다.
