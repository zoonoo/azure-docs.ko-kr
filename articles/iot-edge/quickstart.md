...
이 빠른 시작에서 배포하는 모듈은 온도, 습도 및 압력 데이터를 생성하는 시뮬레이트된 센서입니다. 다른 Azure IoT Edge 자습서에서는 비즈니스 정보를 위해 시뮬레이트된 데이터를 분석하는 모듈을 배포하는 과정을 설명하므로 여기에서 수행하는 작업을 토대로 진행됩니다. 
...
클라우드 셸 인스턴스에 Azure IoT 확장을 추가합니다.
...
이 빠른 시작에는 무료 수준의 IoT Hub가 작동합니다. 이전에 IoT Hub를 사용하고 이미 무료 허브를 만들었다면 해당 IoT 허브를 사용할 수 있습니다. 구독마다 하나의 무료 IoT Hub만 가질 수 있습니다. 

1. Azure Cloud Shell에서 리소스 그룹을 만듭니다. 다음 코드는 **미국 서부** 지역에 **TestResources**라는 리소스 그룹을 만듭니다. 빠른 시작 및 자습서의 모든 리소스를 한 그룹에 배치하여 다 함께 관리할 수 있습니다. 

   ```azurecli-interactive
   az group create --name TestResources --location westus
   ```

1. 새 리소스 그룹에 IoT Hub를 만듭니다. 다음 코드는 리소스 그룹 **TestResources**에 무료 **F1** 허브를 만듭니다. *{hub_name}* 을 IoT 허브의 고유한 이름으로 바꿉니다.
...
1. Azure Cloud Shell의 허브에 **myEdgeDevice**라는 장치를 만드는 다음 명령을 입력합니다.

   ```azurecli-interactive
   az iot hub device-identity create --device-id myEdgeDevice --hub-name {hub_name} --edge-enabled
   ```

1. IoT Hub에 해당 ID를 가진 물리적 장치에 연결하는 장치에 대한 연결 문자열을 검색합니다. 
...
IoT Edge 런타임은 모든 IoT Edge 장치에 배포되며, 세 가지 구성 요소가 있습니다. **IoT Edge 보안 디먼**은 Edge 장치가 부팅될 때마다 시작하고 IoT Edge 에이전트를 시작하여 장치를 부트스트랩합니다. **IoT Edge 에이전트**는 IoT Edge 허브를 포함한 IoT Edge 장치에서 모듈의 배포 및 모니터링을 지원합니다. **IoT Edge 허브**는 IoT Edge 장치의 모듈 간 통신과 장치와 IoT Hub 간의 통신을 관리합니다. 

>[!NOTE]
>이 섹션의 설치 단계는 설치 스크립트를 개발하는 현재로서는 수동입니다. 

이 섹션의 지침은 Linux 컨테이너를 사용하여 IoT Edge 런타임을 구성합니다. Windows 컨테이너를 사용하려는 경우 [Windows 컨테이너를 사용하려면 Windows에 Azure IoT Edge 런타임 설치](how-to-install-iot-edge-windows-with-windows.md)를 참조합니다.
...
6. `config.yaml` 파일에서 **연결 설정** 섹션을 찾습니다. 이전 섹션에서 연 포트 및 IP 주소를 사용하여 **management_uri** 및 **workload_uri** 값을 업데이트합니다. 
...
[Visual Studio Code에 대해 Azure IoT Toolkit 확장](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit) 또는 [IoT Hub 탐색기 도구][lnk-iothub-explorer]를 사용하여 IoT 허브에서 받는 메시지를 볼 수 있습니다. 

## <a name="clean-up-resources"></a>리소스 정리

IoT Edge 자습서를 테스트하려면 이 빠른 시작에서 구성한 시뮬레이트된 장치를 사용할 수 있습니다. tempSensor 모듈이 IoT 허브에 데이터를 보내지 못하게 하려면 다음 명령을 사용하여 IoT Edge 서비스를 중지하고 장치에서 생성된 컨테이너를 삭제합니다. IoT Edge 장치로 컴퓨터를 다시 사용하려는 경우 해당 서비스를 시작해야 합니다. 
...
만든 Azure 리소스가 더 이상 필요하지 않은 경우 다음 명령을 사용하여 만든 리소스 그룹 및 이와 연결된 모든 리소스를 삭제할 수 있습니다.
...
> [Azure 함수를 사용하여 센서 데이터 필터링](tutorial-deploy-function.md)
