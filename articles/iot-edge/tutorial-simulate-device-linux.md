...
이 자습서에서는 시뮬레이트된 IoT Edge 장치를 만든 다음 센서 데이터를 생성하는 모듈을 배포하는 과정을 안내합니다. 다음 방법에 대해 알아봅니다.
...
1. IoT Edge 장치를 실행할 컴퓨터에서 IoT Edge 컨트롤 스크립트를 다운로드합니다.
...
이 빠른 시작에서는 새 IoT Edge 장치를 만들고 여기에 IoT Edge 런타임을 설치했습니다. 그런 다음 장치 자체를 변경하지 않고도 장치에서 실행할 IoT Edge 모듈을 푸시할 수 있도록 Azure Portal을 사용했습니다. 이 경우 푸시한 모듈이 자습서에 사용할 수 있는 환경 데이터를 생성합니다. 

tempSensor 모듈에서 전송되는 메시지를 확인합니다.
