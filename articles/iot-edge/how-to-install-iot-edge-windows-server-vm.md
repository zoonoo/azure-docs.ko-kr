...
1. 새 가상 머신을 만듭니다.
   ...
   * 이 명령은 암호를 묻는 메시지를 표시 하지만 `--admin-password` 옵션을 추가 하 여 스크립트에서 보다 쉽게 설정할 수 있습니다.
   * Windows Server Core 이미지에는 원격 데스크톱 에서만 명령줄이 지원 되므로 전체 데스크톱 환경을 원하는 경우 `MicrosoftWindowsServer:WindowsServer:2019-Datacenter-with-Containers:latest`를 이미지로 지정 합니다.
