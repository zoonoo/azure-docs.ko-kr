...
1. **삭제**를 선택합니다.
1. 이 작업으로 이 배포가 삭제되고 모든 장치에 대한 이전 상태로 되돌릴 것임을 알리는 메시지가 표시됩니다.  즉 우선 순위가 낮은 배포가 적용됩니다.  다른 배포가 대상으로 지정되지 않으면 모듈이 제거되지 않습니다. 장치에서 모든 모듈을 제거하려는 경우 모듈이 없는 배포 및 동일한 장치에 배포합니다. **예**를 선택하여 계속합니다. 
