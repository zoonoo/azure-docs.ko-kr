...
1. 관리형 인스턴스에 필요한 몇 가지 특수 권한을 Virtual Network 및 서브넷에 적용합니다. `az role assignment create` 명령을 사용하여 `<subscription ID>`, `<resource group name>`, `<VNet name>` 및 `<subnet name>`을 적절한 값으로 바꿉니다.
