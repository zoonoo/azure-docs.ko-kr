...
먼저, 다음과 같이 사용할 두 개의 커넥터인 **Dropbox 커넥터** 및 **Twitter 커넥터**를 만들어야 합니다. 해당 커넥터를 만들려면:

1. 홈 화면에서 **마켓플레이스**를 클릭하고 **Twitter**를 검색합니다. 

2. Twitter 커넥터를 선택하고 만들기 단추를 클릭합니다. 모든 설정에 대한 블레이드를 가져올 수 있습니다. 이름으로 **Twitter 커넥터**로 둡니다.

3. **새 앱 서비스 요금제 만들기**에 요금제 이름을 입력합니다.
	
	>[AZURE.NOTE]이 섹션의 단계에서는 새 앱 서비스 요금제를 만들고 있다고 가정합니다. 기존 앱 서비스 요금제를 사용하는 경우 **기존 선택**을 클릭하고 기존 요금제를 선택하고 다음 섹션으로 건너뜁니다.
 
4.  새 요금제의 **가격 책정 계층**을 선택합니다.
 
	>[AZURE.NOTE]기본적으로 앱 논리에 대한 권장 요금제만 표시됩니다. **모두 보기**를 클릭하여 사용 가능한 모든 요금제를 표시할 수 있습니다. 무료 계층에서 논리 앱을 실행할 경우 1시간에 한 번씩만 실행할 수 있으며, 매월 1,000개까지 작업을 사용할 수 있습니다.
...
2. **이름**에서 논리 앱에 대한 의미 있는 이름을 입력합니다.

3. 커넥터를 만들 때 사용한 **앱 서비스 요금제**를 선택합니다. 그러면 위치, 구독 및 리소스 그룹이 자동으로 선택됩니다.
...
4.  되풀이 **주기** 및 **간격**(예: 1시간마다 한 번씩)을 선택한 다음, 녹색 확인 표시를 클릭합니다.
...
1. 오른쪽 창에서 **Twitter 커넥터**를 찾아서 이를 클릭합니다. 


2. 해당 커넥터가 로드되면 **인증** 단추를 클릭하고, Twitter 계정으로 로그인하고 **앱 인증**을 클릭합니다. 
...
2. 프로비전이 완료되면 **인증** 단추를 클릭하고, Dropbox 계정으로 로그인하고 **허용**을 클릭합니다.
...
4. **콘텐츠** 필드에서 `...` 단추를 클릭하고 **트윗 텍스트** 옵션을 클릭합니다. 
	...
	그러면 텍스트 상자에 `@first(body('twitterconnector')).TweetText` 값이 입력됩니다. 생성된 값에는 다음과 같은 부분이 포함됩니다.
...
5. 이제 디자인을 완료했으므로 디자이너의 상단 왼쪽에서 **코드 보기**를 클릭하면 표시되는 코드가 디자이너에서 방금 만든 워크플로를 정의하는 JSON 코드인 것을 알 수 있습니다. [다음 항목인 ][논리 앱 기능 사용]에서 이 코드에 대해 더 자세히 살펴보겠습니다.

6. 화면 하단에서 **확인** 단추를 클릭한 다음 **만들기** 단추를 클릭합니다. 
