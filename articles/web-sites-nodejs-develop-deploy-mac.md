...
7. 상태가 **실행 중**으로 변경되면 포털이 웹 앱에 대한 블레이드를 자동으로 엽니다. 

	![][go-to-dashboard]

8. **배포**를 클릭합니다. (블레이드의 이 부분을 보려면 스크롤해야 할 수 있습니다.)

	![][deployment-part]

9. **원본 선택**, **로컬 Git 리포지토리**를 차례로 클릭합니다. **확인**을 클릭합니다.

	![][setup-git-publishing]


10. **배포 자격 증명** 부분을 클릭합니다. 사용자 이름 및 암호를 만듭니다. **저장**을 클릭합니다. (이전에 웹 앱에 대한 게시를 설정한 적이 없는 경우 이 단계가 필요하지 않습니다.)
...
Node.js는 응용 프로그램에 사용될 수 있는 풍부한 모듈 에코시스템을 제공합니다. 웹 앱에서 모듈을 사용하는 방법에 대한 자세한 내용은 [Azure 응용 프로그램에 Node.js 모듈 사용](nodejs-use-node-modules-azure-apps.md)을 참조하세요
...
응용 프로그램을 Azure에 배포한 후 문제가 발생하는 경우 [Azure 웹 사이트에서 Node.js 응용 프로그램 디버그 방법](web-sites-nodejs-debug.md)에서 문제를 진단하는 방법에 대한 정보를 참조하세요.
