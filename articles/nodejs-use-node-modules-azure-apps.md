...
* 모든 네이티브 모듈의 필수 구성 요소가 설치되어 있는 Windows 컴퓨터에서 **npm install**을 실행합니다. 그런 다음 만든 **node\_modules** 폴더를 응용 프로그램의 일부로 Azure 웹 사이트에 배포합니다.
...
> [AZURE.NOTE]
Azure 웹 사이트에 배포할 때는 <b>package.json</b> 파일이 네이티브 모듈을 참조하는 경우 Git를 사용하는 응용 프로그램을 게시하면 다음과 유사한 오류가 발생합니다.
...
> [AZURE.NOTE]
Azure 웹 사이트에 배포할 때는 <b>npm-shrinkwrap.json</b> 파일이 네이티브 모듈을 참조하는 경우 Git를 사용하는 응용 프로그램을 게시하면 다음과 유사한 오류가 발생합니다.
