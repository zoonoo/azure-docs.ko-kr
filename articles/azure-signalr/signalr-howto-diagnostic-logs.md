...
로그는 **진단 로그** 창에 구성 된 저장소 계정에 저장 됩니다. 이라는 컨테이너는 `insights-logs-alllogs` 리소스 로그를 저장 하기 위해 자동으로 만들어집니다. 컨테이너 내에서 로그는 파일에 저장 됩니다 `resourceId=/SUBSCRIPTIONS/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX/RESOURCEGROUPS/XXXX/PROVIDERS/MICROSOFT.SIGNALRSERVICE/SIGNALR/XXX/y=YYYY/m=MM/d=DD/h=HH/m=00/PT1H.json` . 기본적으로 경로는 및로 결합 `resource ID` 됩니다 `Date Time` . 로그 파일은로 분할 됩니다 `hour` . 따라서 분은 항상 `m=00` 입니다.
...
connectionId | 연결 id
transportType | 연결의 전송 유형입니다. 허용 되는 값은 다음과 같습니다. `Websockets` \| `ServerSentEvents` \|`LongPolling`
...
1. `Logs`대상 Log Analytics를 클릭 합니다.
...
연결의 연결이 끊어지면 리소스 로그에서이 연결 끊기 이벤트를 기록 하 고에 또는가 표시 됩니다 `ConnectionAborted` `ConnectionEnded` `operationName` .

와의 차이점 `ConnectionAborted` 은 `ConnectionEnded` `ConnectionEnded` 가 클라이언트 또는 서버 쪽에서 트리거되는 예상 되는 연결이 끊어지는 것입니다. 는 `ConnectionAborted` 일반적으로 예기치 않은 연결 삭제 이벤트 이며, 중단 이유는에서 제공 됩니다 `message` .
...
클라이언트 요청에 대해 인증 되지 않은 401이 반환 되는 경우 리소스 로그를 확인 합니다. 사용자가 발생 하 `Failed to validate audience. Expected Audiences: <valid audience>. Actual Audiences: <actual audience>` 는 경우 액세스 토큰의 모든 대상 그룹이 잘못 된 것입니다. 로그에 제안 된 올바른 대상 그룹을 사용 하십시오.
