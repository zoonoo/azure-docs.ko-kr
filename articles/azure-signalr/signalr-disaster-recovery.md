...
취약한 연결의 주요 차이점은 보조 인스턴스가 일반적으로 다른 지역에 있으므로 클라이언트 연결 라우팅을 수락하지 않는다는 점입니다. 다른 지역으로 클라이언트를 라우팅하는 것은 일반적으로 최적의 선택이 아닙니다(대기 시간이 증가).
...
![토폴로지](media/signalr-disaster-recovery/topology.png)
...
`Azure:SignalR:ConnectionString`이라는 구성 항목을 통해 환경 변수/앱 설정/web.cofig로 SignalR Service 연결 문자열을 설정하는 방법을 미리 알고 있어야 합니다.
...
그림 1. 장애 조치(failover) 전![장애 조치(failover) 전](media/signalr-disaster-recovery/before-failover.png)

그림 2. 장애 조치(failover) 후![장애 조치(failover) 후](media/signalr-disaster-recovery/after-failover.png)

그림 3. 기본 복구 후 잠시 동안 ![기본 복구 후 잠시 동안](media/signalr-disaster-recovery/after-recover.png)
...
이 문서에서는 SignalR Service에 대한 복원력을 달성하기 위해 애플리케이션을 구성하는 방법을 알아보았습니다. SignalR Service의 서버/클라이언트 연결 및 연결 라우팅에 대해 자세히 알아보려면 [이 문서](signalr-internals.md)에서 SignalR Service 내부 기능에 대해 읽어보세요.