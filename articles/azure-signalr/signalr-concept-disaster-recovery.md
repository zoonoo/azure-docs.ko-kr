...
서비스 인스턴스는 항상 한 리전에서 실행되며 지역 전체 가동 중단이 있을 때 다른 리전으로 장애 조치되지 않습니다.

Microsoft의 서비스 SDK는 대신 여러 SignalR Service 인스턴스를 지원하고 그 중 일부를 사용할 수 없는 경우 다른 인스턴스로 자동으로 전환하는 기능을 제공합니다.
이 기능을 사용하면 재해가 발생하면 복구할 수 있지만 직접 올바른 시스템 토폴로지를 설정해야 합니다. 이 문서에서는 그 방법에 대해 알아봅니다.
...
기본 인스턴스와 앱 서버는 일반 서버 연결을 통해 연결되지만 보조 인스턴스및 앱 서버는 약한 연결이라는 특별한 유형의 연결을 통해 연결됩니다.
약한 연결의 주요 차이점은 보조 인스턴스가 다른 지역에 있기 때문에 클라이언트 연결 라우팅을 허용하지 않는다는 것입니다. 클라이언트를 다른 지역으로 라우팅하는 것은 최적의 선택이 아닙니다(대기 시간 증가).
...
`Azure:SignalR:ConnectionString`환경 변수/앱 설정/web.cofig를 통해 SignalR 서비스 연결 문자열을 설정 하는 방법을 이미 알고 있어야 합니다.
...
연결 문자열을 다른 곳에 저장하려는 경우 코드에서 해당 문자열을 읽고 호출 `AddAzureSignalR()` 할 때 (ASP.NET Core) 또는 `MapAzureSignalR()` (ASP.NET) 매개 변수로 사용할 수도 있습니다.
...
여러 기본 또는 보조 인스턴스를 구성할 수 있습니다. 여러 기본 및/또는 보조 인스턴스가 있는 경우 협상은 다음 순서로 끝점을 반환합니다.

1. 온라인에 하나 이상의 기본 인스턴스가 있는 경우 임의의 기본 온라인 인스턴스를 반환합니다.
2. 모든 기본 인스턴스가 다운된 경우 임의의 보조 온라인 인스턴스를 반환합니다.
...
어떤 패턴을 사용하든 각 SignalR 서비스 인스턴스를 기본 으로 앱 서버에 연결해야 합니다.
...
여러 인스턴스를 함께 사용하여 많은 수의 연결을 처리하는 샤딩과 같은 크기 조정 시나리오의 경우 [여러 인스턴스를 확장하는 방법을](signalr-howto-scale-multi-instances.md)읽어보십시오.
