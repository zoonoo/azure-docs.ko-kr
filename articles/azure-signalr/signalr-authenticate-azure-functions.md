---
title: '자습서: Azure Functions를 통한 Azure SignalR Service 인증 | Microsoft Docs'
description: 이 자습서에서는 Azure SignalR Service 클라이언트를 인증 하는 방법을 배웁니다.
services: signalr
documentationcenter: ''
author: sffamily
manager: cfowler
...
ms.locfileid: "46978428"
---
# <a name="tutorial-azure-signalr-service-authentication-with-azure-functions"></a>자습서: Azure Functions를 통한 Azure SignalR Service 인증
...
1. **SignalR Service**를 검색하여 선택합니다. **만들기**를 클릭합니다.
    ...
    ![새 SignalR Service](media/signalr-authenticate-azure-functions/signalr-quickstart-new.png)
...
1. VS Code에서 Azure Functions 확장을 사용하여 주 프로젝트 폴더에서 함수 앱을 초기화합니다.
    ...
    ![함수 앱 만들기](media/signalr-authenticate-azure-functions/signalr-create-vscode-app.png)
...
1. 메뉴에서 **보기 > 통합 터미널**을 차례로 선택하여 VS Code에서 터미널을 엽니다(Ctrl-`).
...
1. 파일의 내용을 다음으로 바꿉니다.
    ...
    * `Host` 섹션은 로컬 Functions 호스트에 대한 포트 및 CORS 설정을 구성합니다(Azure에서 실행하는 경우 이 설정은 효과가 없음).
    ...
    ![SignalR Service 키 가져오기](media/signalr-authenticate-azure-functions/signalr-get-key.png)

1. 파일을 저장합니다.

    ![로컬 설정 업데이트](media/signalr-authenticate-azure-functions/signalr-update-local-settings.png)


## <a name="create-a-function-to-authenticate-users-to-signalr-service"></a>사용자를 SignalR Service에 인증하는 함수 만들기

브라우저에서 채팅 앱을 처음 열 때 Azure SignalR Service에 연결하려면 유효한 연결 자격 증명이 필요합니다. 함수 앱에서 이 연결 정보를 반환하는 *SignalRInfo*라는 HTTP 트리거 함수를 만듭니다.
...
1. **SignalRInfo/function.json**을 열어 함수에 대한 바인딩을 구성합니다. 파일의 내용을 다음과 같이 수정합니다. 이렇게 하면 클라이언트에서 `chat`이라는 Azure SignalR Service 허브에 연결할 수 있는 유효한 자격 증명을 생성하는 입력 바인딩이 추가됩니다.
...
1. **SignalRInfo/index.js**를 열어 함수 본문을 봅니다. 파일의 내용을 다음과 같이 수정합니다.
    ...
    이 함수는 입력 바인딩의 SignalR 연결 정보를 가져와서 HTTP 응답 본문의 클라이언트에 반환합니다.
...
1. **SendMessage/function.json**을 열어 함수에 대한 바인딩을 구성합니다. 파일의 내용을 다음과 같이 수정합니다.
    ...
    * `chat`이라는 SignalR Service 허브에 연결된 모든 클라이언트에 메시지를 보내는 SignalR Service 출력 바인딩을 추가합니다.
...
1. **[index.html](https://raw.githubusercontent.com/Azure-Samples/signalr-service-quickstart-serverless-chat/master/docs/demo/chat-with-auth/index.html)** 의 내용을 복사하여 붙여넣습니다.
...
1. 메시지가 표시되면 다음 정보를 제공합니다.
    ...
    | Storage 계정 이름 | 고유한 이름 입력합니다(3-24자, 영숫자만). |
    | 위치 | 가까운 위치를 선택합니다. |
...
1. 열려 있는 포털의 **플랫폼 기능** 탭 아래에서 **CORS**를 선택합니다.

    ![CORS 찾기](media/signalr-authenticate-azure-functions/signalr-find-cors.png)

1. `*` 값이 있는 항목을 추가합니다.

1. 다른 모든 기존 항목을 제거합니다.
...
1. 함수 앱의 URL을 복사합니다.
    ...
    ![URL 가져오기](media/signalr-authenticate-azure-functions/signalr-get-url.png)
...
1. 다음 정보를 입력합니다.
    ...
    | 성능 | Standard |
    | 액세스 계층 | 핫 |
    | 리소스 그룹 | 이 자습서의 다른 리소스와 동일한 리소스 그룹을 선택합니다. |

1. **만들기**를 클릭합니다.

    ![저장소 만들기](media/signalr-authenticate-azure-functions/signalr-create-storage.png)

1. 저장소 계정이 만들어지면 Azure Portal에서 엽니다.

1. 왼쪽의 탐색 영역에서 **정적 웹 사이트(미리 보기)** 를 선택합니다.

1. **사용**을 선택합니다.

1. **인덱스 문서 이름**으로 `index.html`을 입력합니다.

1. **저장**을 클릭합니다.

    ![정적 사이트 구성](media/signalr-authenticate-azure-functions/signalr-static-websites.png)

1. 페이지에서 **$web** 링크를 클릭하여 Blob 컨테이너를 엽니다.
...
1. 포털에서 함수 앱이 아직 열려 있는 상태에서 **플랫폼 기능** 탭을 찾아서 **인증/권한 부여**를 선택합니다.
...
1. 선택한 로그인 공급자에 대한 설명서에 따라 구성을 완료합니다.
    ...
    - [Microsoft 계정](https://docs.microsoft.com/azure/app-service/app-service-mobile-how-to-configure-microsoft-authentication)
...
![데모](media/signalr-authenticate-azure-functions/signalr-serverless-chat.gif)
