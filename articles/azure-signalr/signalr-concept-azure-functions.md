---
title: 실시간 앱 빌드 - Azure 함수 & Azure SignalR 서비스
description: 다음 예제를 통해 Azure SignalR 서비스를 사용하여 실시간 서버리스 웹 응용 프로그램을 개발하는 방법에 대해 알아봅니다.
author: sffamily
ms.service: signalr
ms.topic: conceptual
ms.date: 11/13/2019
...
---
# <a name="build-real-time-apps-with-azure-functions-and-azure-signalr-service"></a>Azure 기능 및 Azure SignalR 서비스로 실시간 앱 빌드

Azure SignalR 서비스 및 Azure Functions는 모두 인프라를 관리하는 대신 응용 프로그램 빌드에 집중할 수 있는 완전히 관리되고 확장성이 뛰어난 서비스이므로 두 서비스를 함께 사용하여 [서버없는](https://azure.microsoft.com/solutions/serverless/) 환경에서 실시간 통신을 제공하는 것이 일반적입니다.

> [!NOTE]
> 대화형 자습서에서 SignalR 및 Azure 함수를 함께 사용하는 방법을 알아봅니다 [Azure Functions 및 SignalR Service를 사용하여 웹 응용 프로그램에서 자동 업데이트 활성화.](https://docs.microsoft.com/learn/modules/automatic-update-of-a-webapp-using-azure-functions-and-signalr)

## <a name="integrate-real-time-communications-with-azure-services"></a>Azure 서비스를 통해 실시간 통신 통합

Azure 함수를 사용하면 클라우드에서 이벤트가 발생할 때마다 트리거되는 JavaScript, 파이썬, C#및 Java를 비롯한 [여러 언어로](../azure-functions/supported-languages.md)코드를 작성할 수 있습니다. 이러한 이벤트의 예는 다음과 같습니다.
...
Azure Functions 및 SignalR 서비스를 함께 사용하는 방법에 대한 자세한 내용은 다음 리소스를 방문하십시오.

* [SignalR 서비스를 통해 Azure 기능 개발 및 구성](signalr-concept-serverless-development-config.md)
...
* [Azure SignalR 서비스 서버없는 빠른 시작 - C #](signalr-quickstart-azure-functions-csharp.md)
