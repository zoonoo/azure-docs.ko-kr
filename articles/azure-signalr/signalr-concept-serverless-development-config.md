...
SignalR 클라이언트 Sdk는 협상 핸드셰이크를 수행 하는 데 필요한 논리를 이미 포함 하 고 있습니다. Negotiate 끝점의 URL을 제외 하 고 `negotiate` 세그먼트를 SDK의 `HubConnectionBuilder`에 전달 합니다. JavaScript의 예제는 다음과 같습니다.
...
로컬 컴퓨터에서 함수 앱을 실행 하는 경우 CORS에 섹션을 `Host` 추가 하 *여 CORS* 를 사용 하도록 설정할 수 있습니다. 섹션에서 `Host` 다음 두 가지 속성을 추가 합니다.
...
*SignalRConnectionInfo* binding 구성에서 이러한 헤더를 사용 하 여 인증 된 연결을 만들 수 있습니다. 다음은 헤더를 `x-ms-client-principal-id` 사용 하는 c # negotiate 함수 예제입니다.
