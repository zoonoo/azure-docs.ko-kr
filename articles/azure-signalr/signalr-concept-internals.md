...
description: Azure SignalR Service 내부 기능, 아키텍처, 연결 및 데이터 전송 방법에 대해 알아봅니다.
author: sffamily
ms.service: signalr
ms.topic: conceptual
...
Azure SignalR Service는 ASP.NET Core SignalR 프레임워크를 기반으로 구축되었습니다. Azure SignalR Service는 또한 ASP.NET Core 프레임워크를 기반으로 ASP.NET SignalR의 데이터 프로토콜을 다시 구현하여 ASP.NET SignalR을 지원합니다.

코드 몇 줄을 변경하여 로컬 ASP.NET Core SignalR 애플리케이션 또는 ASP.NET SignalR 애플리케이션을 SignalR Service에서 작동하도록 쉽게 마이그레이션할 수 있습니다.
...
SignalR Service는 고객 데이터를 저장하지 않으며 수신된 모든 고객 데이터는 실시간으로 대상 서버 또는 클라이언트로 전송됩니다.
