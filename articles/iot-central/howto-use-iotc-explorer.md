...
> [!NOTE]
> 일반적으로 Unix와 유사한 환경에서를 사용 `sudo` 하 여 설치 명령을 실행 해야 합니다.

## <a name="run-iotc-explorer"></a>Iotc-탐색기 실행

다음 섹션에서는를 실행할 `iotc-explorer`때 사용할 수 있는 일반적인 명령 및 옵션에 대해 설명 합니다. 명령 및 옵션의 전체 집합을 보려면 또는 해당 하위 `--help` 명령 `iotc-explorer` 에를 전달 합니다.
