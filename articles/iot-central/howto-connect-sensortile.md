...
* SensorTile.box 장치를 참조 하세요 [SensorTile.box](https://www.st.com/content/st_com/en/products/evaluation-tools/SensorTile.box) 자세한 내용은 합니다.
* ST BLE 센서 앱이 Android 장치의 수 [여기에서 다운로드]를 설치 (https://play.google.com/store/apps/details?id=com.st.bluems)합니다. 자세한 내용은 참조: [ST BLE 센서] (http://www.st.com/stblesensor)
* 만든 Azure IoT Central 응용 프로그램을 **DevKits** 응용 프로그램 템플릿. 자세한 내용은 [애플리케이션 만들기 빠른 시작](quick-deploy-iot-central.md)을 참조하세요.
* 추가 합니다 **SensorTile.box** 방문 하 여 IoT Central 응용 프로그램에 장치 템플릿을 **장치 템플릿** 페이지를 클릭 하 **+ 새로 만들기**를 선택 합니다 **SensorTile** 템플릿.

### <a name="get-your-device-connection-details"></a>장치 연결 정보 가져오기

Azure IoT Central 응용 프로그램에서 실제 장치를 추가 합니다 **SensorTile.box** 장치 템플릿 및 장치 연결 세부 정보를 기록 합니다. **ID, 장치 ID 및 기본 키 범위**:
...
이 섹션에서는 장치에서 응용 프로그램 펌웨어 푸시 에너지 BLE (Bluetooth Low) 연결을 통해 IoT central ST BLE 센서 모바일 앱을 통해 장치 데이터를 전송 하는 방법을 배웁니다.
...
1. 프로세스가 완료 되 면는 SensorTile.box 온도, 압력 및 습도 BLE 통해 스트림 됩니다.

## <a name="connect-the-sensortilebox-to-the-cloud"></a>SensorTile.box를 클라우드에 연결

이 섹션에서는 모바일 응용 프로그램에 SensorTile.box를 연결 하 여 모바일 응용 프로그램을 클라우드에 연결 하는 방법을 배웁니다.
...
1. 몇 초 후 데이터는 IoT Central 응용 프로그램 대시보드에 표시 됩니다.
...
제안 된 다음 단계에 알아보려면는 이제는 SensorTile.box Azure IoT Central 응용 프로그램에 연결 하는 방법을 배웠으므로 하는 방법 [템플릿으로 사용자 지정 장치 설정](howto-set-up-template.md) 사용자 고유의 IoT 장치에 대 한 합니다.
