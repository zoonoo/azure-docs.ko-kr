...
Azure IoT Central 응용 프로그램에서 **MXChip** 장치 템플릿의 실제 장치를 추가 하 고 장치 연결 정보를 적어 둡니다. **범위 ID, 장치 id 및 기본 키**:
...
1. Device Explorer에서 **실제 장치** 를 추가 하 고 **+ 새로 > 만들기** 를 선택 하 여 실제 장치를 추가 합니다.
    ...
    * **장치 이름을**입력 하거나 제안 된 이름을 사용 합니다.
    ...
    ![장치 추가](media/howto-connect-devkit/add-device.png)
...
Visual Studio Code를 사용 하 여 `MXCHIP/mxchip_advanced` `iot-central-firmware` 폴더에서 폴더를 엽니다.

![Visual Studio 코드](media/howto-connect-devkit/vscodeview.png)

원격 분석이 Azure IoT Central 응용 프로그램으로 전송 되는 방법을 보려면 다음 `src` 폴더에서 **원격 분석 .cpp** 파일을 엽니다.

- `TelemetryController::buildTelemetryPayload` 함수는 디바이스의 센서 데이터를 사용하여 JSON 원격 분석 페이로드를 만듭니다.

- 함수 `TelemetryController::sendTelemetryPayload` 는 **azureiotclient .cpp** 에서를 호출 `sendTelemetry` 하 여 Azure IoT Central 응용 프로그램에서 사용 하는 IoT Hub JSON 페이로드를 보냅니다.

Azure IoT Central 응용 프로그램에 속성 값을 보고 하는 방법을 보려면 다음 `src` 폴더에서 **원격 분석 .cpp** 파일을 엽니다.

- 함수 `TelemetryController::loop` 는 보고 된 **위치** 속성을 약 30 초 마다 보냅니다. 이 함수는 `sendReportedProperty` **azureiotclient .cpp** 소스 파일의 함수를 사용 합니다.

- 이 함수 `TelemetryController::loop` 는 장치가 속도계가 두 번 탭을 검색할 때 **dieNumber** 보고 속성을 보냅니다. 이 함수는 `sendReportedProperty` **azureiotclient .cpp** 소스 파일의 함수를 사용 합니다.

장치가 IoT Central 응용 프로그램에서 호출 되는 명령에 응답 하는 방법을 확인 하려면 `src` 폴더에서 registeredMethodHandlers 파일을 엽니다.

- **DmEcho** 함수는 **echo** 명령에 대 한 처리기입니다. 장치의 화면에서 페이로드의 **displayedValue** 을 표시 합니다.

- **Dmcountdown** 함수는 **카운트다운** 명령에 대 한 처리기입니다. 장치의 LED 색을 변경 하 고 보고 된 속성을 사용 하 여 카운트다운 값을 IoT Central 응용 프로그램으로 다시 보냅니다. 보고 된 속성의 이름은 명령과 동일 합니다. 함수는 `sendReportedProperty` **azureiotclient .cpp** 소스 파일의 함수를 사용 합니다.
...
| 디바이스 속성 | 장치 위치   | 위치  | 위치    |
| 텍스트            | 제조 일자     | manufacturedIn   | 해당 사항 없음       |

### <a name="commands"></a>명령

| Display name | 필드 이름 | 반환 형식 | 입력 필드 표시 이름 | 입력 필드 이름 | 입력 필드 형식 |
| ------------ | ---------- | ----------- | ------------------------ | ---------------- | ---------------- |
