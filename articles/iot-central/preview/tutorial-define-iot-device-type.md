...
작성자는 IoT 플러그 앤 플레이 인증 디바이스를 사용하여 솔루션 빌드를 빠르게 시작할 수 있습니다. [Azure IoT 디바이스 카탈로그](https://catalog.azureiotsolutions.com/alldevices)의 목록을 참조하세요. IoT Central은 디바이스 카탈로그와 통합되어 이러한 IoT 플러그 앤 플레이 인증 디바이스에서 디바이스 기능 모델을 가져올 수 있습니다. IoT Central에 있는 이러한 디바이스 중 하나에서 디바이스 템플릿을 만들려면 다음을 수행합니다.

1. IoT Central 애플리케이션의 **디바이스 템플릿** 페이지로 이동합니다.
1. **+ 새로 만들기**를 선택한 다음, 카탈로그에서 IoT 플러그 앤 플레이 인증 디바이스를 선택합니다. IoT Central에서 이 디바이스 기능 모델을 기반으로 하여 디바이스 템플릿을 만듭니다.
...
1. **인터페이스 선택** 페이지에서 다음을 수행합니다.
    ...
    - **디바이스 정보** 인터페이스와 같은 표준 인터페이스 중 하나를 선택합니다. 표준 인터페이스는 많은 디바이스에 공통적인 기능을 지정합니다. 이러한 표준 인터페이스는 Azure IoT에서 게시하며 버전을 지정하거나 편집할 수 없습니다.
...
| response | 사용하도록 설정하면 이름, 표시 이름, 스키마, 단위 및 표시 단위를 포함한 명령 응답의 정의입니다. |
...
이 자습서에서는 작성자가 게이트웨이 디바이스 템플릿을 사용하여 IoT Central 애플리케이션에서 새 유형의 IoT 디바이스를 정의하는 방법을 보여 줍니다. 
...
IoT 디바이스 템플릿을 만드는 방법은 다음과 같습니다. 

1. 왼쪽 탐색 영역에서 **디바이스 템플릿**을 선택합니다. 그런 다음, **+ 새로 만들기**를 선택하고, **IoT 디바이스** 타일 및 선점 센서 타일을 선택합니다. 완료되면 **다음: 사용자 지정**을 선택합니다.

   ![디바이스 템플릿 페이지 및 옵션의 스크린샷](./media/tutorial-define-iot-device-type/gateway-downstream-new.png)

1. **검토** 페이지에서 **만들기**를 선택합니다. 

   ![검토 페이지의 스크린샷](./media/tutorial-define-iot-device-type/gateway-downstream-review.png)

1. 새 디바이스 템플릿이 만들어집니다. 

   ![새 디바이스 템플릿의 스크린샷](./media/tutorial-define-iot-device-type/occupancy-sensor.png)

S1 센서에 대한 디바이스 템플릿을 만드는 방법은 다음과 같습니다.

1. 왼쪽 탐색 영역에서 **디바이스 템플릿**을 선택합니다. 그런 다음, **+ 새로 만들기**, **IoT 디바이스** 타일, 선점 센서 타일을 차례로 선택합니다. 완료되면 **다음: 사용자 지정**을 선택합니다.

   ![디바이스 템플릿 페이지 및 옵션의 스크린샷](./media/tutorial-define-iot-device-type/s1-sensor.png)

1. **검토** 페이지에서 **만들기**를 선택합니다. 

   ![검토 페이지의 스크린샷](./media/tutorial-define-iot-device-type/s1-review.png)

1. 새 디바이스 템플릿이 만들어집니다. 

   ![새 디바이스 템플릿의 스크린샷](./media/tutorial-define-iot-device-type/s1-template.png)

## <a name="create-an-iot-gateway-device-template"></a>IoT 게이트웨이 디바이스 템플릿 만들기

IoT 게이트웨이 디바이스 템플릿을 만들도록 선택할 수 있습니다. 게이트웨이 디바이스는 게이트웨이 디바이스를 통해 IoT Central에 연결하는 다운스트림 디바이스와 관계가 있습니다. 

### <a name="downstream-device-relationships-with-gateway-device"></a>게이트웨이 디바이스와 다운스트림 디바이스의 관계

IoT 디바이스는 IoT 게이트웨이 디바이스에 연결할 수 있습니다.

![게이트웨이 디바이스와 다운스트림 디바이스 간의 관계에 대한 다이어그램](./media/tutorial-define-iot-device-type/gatewaypattern.png)

작성자는 애플리케이션에서 IoT 게이트웨이 디바이스 템플릿을 만들고 편집할 수 있습니다. 디바이스 템플릿이 게시되면 디바이스 템플릿을 구현하는 실제 디바이스를 연결할 수 있습니다.

### <a name="select-a-device-template-type"></a>디바이스 템플릿 유형 선택 

새 디바이스 템플릿을 애플리케이션에 추가하려면 다음을 수행합니다.

1. 왼쪽 창에서 **디바이스 템플릿** 탭을 선택합니다.

   ![디바이스 템플릿 페이지의 스크린샷](./media/tutorial-define-iot-device-type/devicetemplate.png)

1. **+ 새로 만들기**를 선택하여 새 디바이스 템플릿 만들기를 시작합니다.

   ![새로 만들기가 강조 표시된 디바이스 템플릿 페이지의 스크린샷](./media/tutorial-define-iot-device-type/devicetemplatenew.png)

   ![디바이스 사용자 지정 페이지의 스크린샷](./media/tutorial-define-iot-device-type/gateway-review.png)

1. **템플릿 유형 선택** 페이지에서 **Azure IoT**를 선택하고, **다음: 사용자 지정**을 선택합니다.

   ![템플릿 유형 선택 페이지의 스크린샷](./media/tutorial-define-iot-device-type/gateway-customize.png)

1. 게이트웨이 확인란, **만들기**를 차례로 선택합니다.

   ![게이트웨이가 강조 표시된 디바이스 사용자 지정 페이지의 스크린샷](./media/tutorial-define-iot-device-type/gateway-review.png)

1. 검토 페이지에서 **만들기**를 선택합니다. 

1. 게이트웨이 템플릿 이름으로 **스마트 빌딩 게이트웨이 템플릿**을 입력합니다. **사용자 지정** 타일을 선택합니다.

1. **디바이스 정보** 표준 인터페이스를 추가합니다.

### <a name="add-relationships"></a>관계 추가

게이트웨이 디바이스에 연결할 디바이스의 디바이스 기능 모델에 다운스트림 관계를 추가할 수 있습니다.

다운스트림 디바이스 기능 모델과의 관계를 만듭니다. **저장**을 선택합니다.

![다양한 옵션이 강조 표시된 스마트 빌딩 게이트웨이 템플릿의 스크린샷](./media/tutorial-define-iot-device-type/gateway-occupancy-s1-rel.png)
...
**기본 보기 생성**을 선택하면 **개요** 및 **정보** 대시보드를 생성할 수 있습니다. 
...
## <a name="create-a-gateway-simulated-device"></a>게이트웨이가 시뮬레이션된 디바이스 만들기

디바이스 탐색기에서 시뮬레이션된 스마트 빌딩 게이트웨이를 만듭니다. 

![새 디바이스 만들기 대화 상자의 스크린샷](./media/tutorial-define-iot-device-type/smartbuildingdevice.png)

## <a name="create-downstream-simulated-devices"></a>다운스트림이 시뮬레이션된 디바이스 만들기

디바이스 탐색기에서 시뮬레이션된 선점 센서를 만듭니다. 

![새 디바이스 만들기 대화 상자의 스크린샷](./media/tutorial-define-iot-device-type/occupancydevice.png)

디바이스 탐색기에서 시뮬레이션된 S1 센서를 만듭니다. 

![새 디바이스 만들기 대화 상자의 스크린샷](./media/tutorial-define-iot-device-type/s1device.png)

## <a name="add-downstream-devices-relationships-to-a-gateway-device"></a>게이트웨이 디바이스에 다운스트림 디바이스 관계 추가

S1 센서 및 선점 센서를 선택하고, **게이트웨이에 연결**을 선택합니다. 

![게이트웨이에 연결이 강조 표시된 선점 센서의 스크린샷](./media/tutorial-define-iot-device-type/connecttogateway.png)
