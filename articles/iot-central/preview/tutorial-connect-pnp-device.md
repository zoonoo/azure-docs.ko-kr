...
1. [Azure IoT Central 애플리케이션 만들기(미리 보기 기능)](./quick-deploy-iot-central.md) 빠른 시작에서 **사용자 지정 앱 > 미리 보기 애플리케이션** 템플릿을 사용하여 만든 IoT Central 애플리케이션에 로그인합니다.
...
1. 빌드가 성공적으로 완료되면 동일한 명령 프롬프트에서 애플리케이션을 실행합니다. `scopeid`, `primarykey`를 이전에 기록한 값으로 바꿉니다.
...
이 자습서에서는 공용 모델 리포지토리의 DCM에서 생성된 IoT 플러그 앤 플레이 디바이스를 연결하는 방법을 알아보았습니다.

DCM에 대한 자세한 내용과 나만의 모델을 만드는 방법에 대한 자세한 내용을 보려면 방법 가이드를 계속 진행하세요.

> [!div class="nextstepaction"]
> [디바이스 그룹 만들기](./tutorial-use-device-groups.md)
