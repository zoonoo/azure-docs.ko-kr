...
-   이제 고객은 먼저 장치를 IoT Central에 등록할 필요 없이 장치 자격 증명을 생성하고 오프라인에서 장치를 구성할 수 있음
-   IoT Central은 업계에서 권장하는 X509 인증 기반 연결을 통해 장치 연결을 지원하면서 계속해서 SAS(공유 액세스 서명) 연결을 지원하고 개선함
-   IoT Central 고객은 자신의 고유 장치 ID를 통해 장치를 IoT Central에 등록할 수 있고, 기존 백 오피스 시스템과 간단한 통합을 사용하도록 설정함
-   IoT Central에 장치를 연결하기 위한 하나의 일관된 방법이 있음 

>[!NOTE]
>IoT Central은 모든 장치 등록 및 연결에 대해 Azure IoT DPS(Device Provisioning Service)를 사용합니다. [DPS에 대한 자세한 정보](https://docs.microsoft.com/azure/iot-dps/about-iot-dps)를 참조하세요.

사용 사례를 기반으로 IoT Central에 장치를 연결하기 위한 지침을 따릅니다.
1. [단일 장치를 신속하게 연결(공유 액세스 서명 사용)](#connect-a-single-device)
1. [SAS(공유 액세스 서명)를 사용하여 대규모로 장치 연결](#connect-devices-at-scale-using-shared-access-signatures)
1. [X509 인증을 사용하여 대규모로 장치 연결](#connect-devices-using-x509-certificates) **프로덕션 워크로드에 권장됨**
1. [먼저 장치를 등록하지 않고 연결](#connect-without-first-registering-devices) 


>[!NOTE]
>장치가 **global.azure-devices-provisioning.net**을 연결하고 프로비전할 수 있는 글로벌 엔트포인트입니다.
...
1. 장치 코드에서 이러한 연결 세부 정보**장치 ID, 장치 이름 및 장치 기본 키**를 사용하여 장치를 프로비전 및 연결하고 즉시 데이터 흐름이 끝까지 표시되기 시작합니다. MxChip 장치를 사용하는 경우 [여기의 단계별 지침](howto-connect-devkit.md#add-a-real-device)을 따라 **DevKit 장치 준비** 섹션부터 시작합니다.   

    다음은 사용하려는 다른 언어에 대한 참조입니다.

    *   **C 언어:** C를 사용하는 경우 [이 C 샘플 장치 클라이언트](https://github.com/Azure/azure-iot-sdk-c/blob/dps_symm_key/provisioning_client/devdoc/using_provisioning_client.md)를 따라 샘플 장치를 연결합니다. 샘플에서 다음 설정을 사용합니다.   
...
## <a name="connect-devices-at-scale-using-shared-access-signatures"></a>공유 액세스 서명을 사용하여 대규모로 장치 연결

SAS를 사용하여 IoT Central에서 대규모로 장치를 연결하려면 두 가지 관련 단계가 있습니다. 
1. **장치 등록**: CSV 파일을 통해 IoT Central로 장치를 가져와 장치를 등록하고, 장치를 연결하는 데 사용할 장치 연결 세부 정보를 사용하여 장치를 내보냅니다.
1. **장치 설정**: 프로비전 서비스가 켜져 있는 경우 연결 정보/IoT Central 앱 할당을 가져오기 위해 프로비전 서비스를 호출할 수 있도록 연결 세부 정보(**범위 ID, 장치 ID 및 기본 키**)를 사용하여 장치를 프로그래밍합니다.

>[!NOTE]
>먼저 IoT Central에 장치를 등록하지 않고도 장치를 연결할 수 있는 경우 고급 옵션을 사용할 수도 있습니다. [여기서 자세히 알아봅니다](https://docs.microsoft.com/azure/iot-dps/about-iot-dps).
...
장치 연결 세부 정보를 가져오기 위해 장치를 내보냅니다. [내보내기]는 장치 ID, 장치 이름 및 장치 키를 사용하여 CSV 파일을 만듭니다. 이러한 정보를 사용하여 IoT Central에 장치를 연결합니다.
...
장치 코드의 이러한 연결 세부 정보(**장치 ID(IOTC_DEVICEID), 장치 기본 키(IOTC_SASKEY_PRIMARY) 및 범위 ID**)를 사용하여 장치를 프로비전하고 연결합니다. 아직 없는 경우 IoT Central 앱(**관리 > 장치 연결 > 범위 ID**)에서 **범위 ID**를 가져옵니다.
**MxChip** 장치를 사용하여 연결하는 경우 [여기의 단계별 지침](howto-connect-devkit.md#add-a-real-device)을 따라 **DevKit 장치 준비** 섹션부터 시작합니다.   
...
1. **연결 설정 구성**: 장치 인증서를 생성하는 데 사용되는 X509 루트/중간 인증서를 추가/확인하여 IoT Central 앱에서 연결 설정을 구성합니다.  X509 인증서에 대한 연결 설정을 구성하기 위한 두 단계가 있습니다.  
    ...
    *   리프 장치 인증서를 생성하는 데 사용하는 **X509 루트 또는 중간 인증서 추가**합니다. 관리 > 장치 연결 > 인증서로 이동합니다. 
    ...
    *   이 [명령줄 도구](https://github.com/Azure/azure-iot-sdk-c/blob/master/tools/CACertificates/CACertificateOverview.md )를 사용하여
        ...
        * 도구(4단계에 따라)에 대한 매개 변수로 장치 ID를 사용하여 리프 인증서를 만듭니다. 인증서를 저장하여 장치에서 사용합니다.     

1. **장치 등록**: CSV 파일을 통해 장치를 IoT Central로 가져와 등록합니다.

1. **장치 설정**: 업로드된 루트 인증서를 사용하여 리프 인증서를 생성합니다. **장치 ID**를 리프 인증서의 CNAME으로 사용하되 **소문자**여야 합니다. **테스트 전용** 리프/장치 인증서를 생성하기 위한 [명령줄 도구](https://github.com/Azure/azure-iot-sdk-c/blob/master/tools/CACertificates/CACertificateOverview.md )가 있습니다.
...
>[!NOTE]
>장치에 대한 리프 인증서를 생성할 때 **장치 ID**를 cname으로 사용합니다.

>[!NOTE]
>**장치 ID**는 소문자여야 합니다. 
 
## <a name="connect-without-first-registering-devices"></a>먼저 장치를 등록하지 않고 연결
IoT Central이 사용하는 주요 시나리오 중 하나는 OEM이 장치를 대량으로 제작하고, 자격 증명을 생성하고, 먼저 IoT Central에 등록할 필요 없이 팩터리에서 자격 증명을 구성하는 것입니다. 장치가 켜지고 IoT Central에 연결되면 운영자는 장치가 IoT Central 앱에 연결되도록 승인합니다.

다음에서는 이 기능을 사용하여 장치를 연결하는 흐름을 보여줍니다.

![연결 설정](media\concepts-connectivity\device-connection-flow.PNG)


사용자가 선택한 장치 인증 체계(X509/SAS)에 따라 단계를 따릅니다.

1. **연결 설정** 
    * **X509 인증서:** [루트/중간 인증서를 추가 및 확인하여](#connect-devices-using-x509-certificates) 다음 단계에서 장치 인증서를 생성하는 데 사용합니다.
    * **SAS:** 기본 키(이 키는 이 IoT Central 응용 프로그램에 대한 그룹 SAS 키입니다)를 복사하여 다음 단계에서 장치 SAS 키를 생성하는 데 사용합니다. 
![연결 설정 SAS](media\concepts-connectivity\connection-settings-sas.png)

1. **장치 자격 증명 생성** 
    *   **X509 인증서:** 이 앱에 추가한 루트/중간 인증서를 사용하여 장치에 대한 리프 인증서를 생성합니다. **장치 ID**를 리프 인증서의 cname으로 사용하되 **<span style="color:Red">(소문자여야 합니다)</span>**. 테스트용 리프/장치 인증서를 생성하기 위한 [명령줄 도구](https://github.com/Azure/azure-iot-sdk-c/blob/master/tools/CACertificates/CACertificateOverview.md )가 있습니다.
    *   **SAS** 장치: 이 [명령줄 도구](https://www.npmjs.com/package/dps-keygen)를 사용하여 SAS 키를 생성할 수 있습니다. 이전 단계의 기본 SAS 키(그룹 SAS 키)를 사용합니다. 장치 ID는 **<span style="color:Red">소문자</span>** 여야 합니다.

        아래 지침을 사용하여 장치 SAS 키를 생성함           
...
1. **장치 설정** 
     ...
     **범위 ID, 장치 ID, 장치 인증서/SAS 키**를 사용하여 장치를 깜박거린 다음, 장치를 켜고 IoT Central 앱에 연결합니다.

1. **IoT Central에 장치 연결:** 장치가 켜지면 등록을 위해 DPS/IoT Central에 연결됩니다.

1. **장치를 템플릿에 연결:** 연결된 장치는 **Device Explorer**의 **연결되지 않은 장치** 아래에 표시됩니다. 장치의 프로비전 상태는 **등록**됩니다. 장치를 적절한 장치 템플릿에 **연결**하고 장치가 IoT Central 앱에 연결되도록 승인합니다. 그러면 장치는 IoT Central 앱에 대한 연결 세부 정보를 가져오고, 연결되고, 데이터를 보내기 시작합니다. 이제 장치 프로비전이 완료되었으므로 *프로비전 상태*가 **프로비전됨**으로 전환됩니다.

## <a name="device-provisioning-status"></a>장치 프로비전 상태
실제 장치를 Azure IoT Central에 연결한 경우 일련의 관련 단계가 있음 
1. **등록됨**: 장치가 먼저 **등록된**다는 의미는 장치가 IoT Central에서 만들어지고 장치에 대한 장치ID를 보유한다는 것입니다.
장치는 다음과 같은 경우에 등록됩니다.  
    *   새 실제 장치가 **Explorer**에 추가될 경우
    *   장치 집합이 **Explorer**에서 **가져오기**를 사용하여 추가될 경우
    *   등록되지 않았지만 유효한 자격 증명을 사용하여 연결되고 **연결되지 않은** 장치 아래에 표시되는 장치의 경우 

    위의 모든 경우에는 *프로비전 상태*가 **등록됨**으로 표시됩니다.

1. **프로비전됨**: 다음 단계는 장치가 유효한 자격 증명과 연결될 때 IoT Central이 IoT Hub에서 장치를 만들어 프로비전 단계를 완료하는 것입니다. 그런 다음, IoT Central은 데이터에 연결 문자열을 반환하여 데이터에 연결하고 데이터를 보내기 시작합니다. 장치의 *프로비전 상태*는 이제 **등록됨**에서 **프로비전됨**으로 전환됩니다.

1.  **차단됨**: 운영자는 장치를 차단할 수 있습니다. 장치가 차단되면 IoT Central에 데이터를 보낼 수 없고 장치를 다시 설정해야 합니다. 차단된 장치는 *프로비전 상태*가 **차단됨**으로 표시됩니다. 운영자는 또한 장치의 차단을 해제할 수 있습니다. 장치 차단이 해제되면 *프로비전 상태*가 이전의 *프로비전 상태*(등록됨 또는 프로비전됨)로 돌아갑니다. 

## <a name="getting-device-connection-string"></a>장치 연결 문자열 가져오기
다음 단계를 사용하여 IoT Hub 장치 연결 문자열을 Azure IoT Hub로 가져올 수 있음 
1. 장치 페이지에서 **범위 ID, 장치 ID 및 장치 기본 키** 같은 연결 세부 정보를 가져옵니다(장치 페이지 > 연결 클릭으로 이동). 

    ![연결 세부 정보](media\concepts-connectivity\device-connect.PNG)

1. 아래 명령줄 도구를 사용하여 장치 연결 문자열을 가져옵니다.
    다음 지침을 사용하여 장치 연결 문자열을 가져옵니다.  
