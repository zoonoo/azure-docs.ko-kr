...
1. **장치 템플릿** 페이지를 사용 하 여 규칙을 추가할 장치 템플릿으로 이동 합니다.

1. 아직 규칙을 하나도 만들지 않은 경우 다음 화면이 표시됩니다.

    ![규칙 없음](media/howto-create-event-rules/rules_landing_page1.png)

1. **규칙** 탭에서 **+ 새 규칙** 을 선택 하 여 만들 수 있는 규칙의 유형을 확인 합니다.

1. 이벤트 타일 을 선택 하 여 이벤트 모니터링 규칙을 만듭니다.

    ![규칙 유형](media/howto-create-event-rules/rule_types1.png)

1. 이 디바이스 템플릿에서 규칙을 식별하는 데 도움이 되는 이름을 입력합니다.

1. 이 템플릿에서 만든 모든 장치에 대해 규칙을 즉시 사용 하도록 설정 하려면 **이 템플릿의 모든 장치에 대해 규칙 사용**을 설정/해제 합니다.

    ![규칙 세부 정보](media/howto-create-event-rules/rule_detail1.png)
...
1. 단위 드롭다운에서 모니터링하려는 이벤트를 선택합니다. 이 예제에서는 **팬 모터 오류** 이벤트를 선택했습니다.
   ...
   ![조건](media/howto-create-event-rules/condition_filled_out1.png)
...
1. 필요에 따라 **집계**로 **Count**를 설정하고 해당 임계값을 제공할 수도 있습니다.
   ...
   - 집계 함수로 Count가 사용되는 경우 사용자는 조건을 평가할 **임계값** 및 **집계 기간**을 제공해야 합니다. 이 경우 이벤트 수가 집계 되 고 집계 된 이벤트 수가 임계값과 일치 하는 경우에만 규칙이 트리거됩니다.
     ...
     ![이벤트 조건 추가](media/howto-create-event-rules/aggregate_condition_filled_out1.png)
...
1. **작업** 옆에 있는 **+** 기호를 선택합니다. 사용 가능한 작업 목록을 표시됩니다.
    ...
    ![작업 추가](media/howto-create-event-rules/add_action1.png)
...
1. **이메일** 작업을 선택하고, **받는 사람** 필드에 유효한 이메일 주소를 입력하고, 규칙이 트리거될 때 이메일 본문에 표시할 메모를 입력합니다.
   ...
   ![작업 구성](media/howto-create-event-rules/configure_action1.png)
