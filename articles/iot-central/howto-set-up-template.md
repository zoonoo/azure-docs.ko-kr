...
장치 템플릿은 Azure IoT Central 응용 프로그램에 연결하는 장치 유형의 특징과 동작을 정의하는 청사진입니다.
...
3. 새 장치 템플릿의 이름(예: Refrigerator-1)을 입력하고 **만들기**를 선택합니다.
...
> [!NOTE]
> 실제 장치를 연결할 때 원격 분석 측정값이 응용 프로그램에 표시되려면 장치 템플릿의 필드 이름이 해당 장치 코드의 속성 이름과 일치해야 합니다. 다음 섹션에서 장치 템플릿 정의를 계속하면서 설정, 장치 속성 및 명령을 구성할 때도 동일하게 수행합니다.
...
**완료**를 선택하면 측정값 목록에 **온도** 측정값이 나타납니다. 잠시 후에 시뮬레이션된 장치에서 생성한 온도 데이터의 시각화를 확인할 수 있습니다. 장치 템플릿을 만드는 경우 물리적/실제 장치를 연결하기 전에 응용 프로그램 동작을 테스트할 수 있게 해주는 시뮬레이션된 장치가 템플릿에서 생성됩니다.
...
**완료**를 선택하면 측정값 목록에 **팬 모터 오류** 측정값이 나타납니다. 작업자는 장치가 보내는 이벤트 데이터의 시각화를 볼 수 있습니다.
...
**완료**를 선택하면 측정값 목록에 **팬 모드** 상태 측정값이 나타납니다. 작업자는 장치가 보내는 상태 데이터의 시각화를 볼 수 있습니다.
...
설정은 장치를 제어합니다. 응용 프로그램 운영자는 설정을 통해 장치에 입력을 제공할 수 있습니다. 장치 템플릿에 여러 설정을 추가할 수 있으며, 설정은 운영자가 사용할 수 있도록 **설정** 탭에 타일로 표시됩니다. 숫자, 텍스트, 날짜, 설정/해제, 선택 목록, 섹션 레이블 등 여러 유형의 설정을 추가할 수 있습니다. 
...
타일을 만든 후 화면의 오른쪽 위에서 **완료**를 선택합니다. 실제 장치가 응용 프로그램에 연결되면 설정 값이 동기화됨으로 변경됩니다.

![타일에 대한 "디자인 모드" 스위치가 있는 "설정" 탭](./media/howto-set-up-template/settingstile.png)

## <a name="properties"></a>properties

속성은 장치 위치나 일련 번호처럼 장치와 연결된 장치 메타데이터입니다. 장치 템플릿에 여러 속성을 추가할 수 있으며, 추가된 속성은 **속성** 탭에 타일로 표시됩니다. 숫자, 텍스트, 날짜, 설정/해제, 장치 속성, 레이블, 위치 등 여러 유형의 속성을 추가할 수 있습니다. 운영자는 장치를 만들 때 속성 값을 지정할 수 있으며, 언제든지 속성 값을 편집할 수 있습니다. 그러나 장치 속성은 읽기 전용이고 장치에서 응용 프로그램으로 전송되므로 운영자가 변경할 수 없습니다. 실제 장치가 연결되면 응용 프로그램에서 장치 속성 타일이 업데이트됩니다. 

속성에는 다음 두 카테고리가 있습니다.

- 장치가 IoT Central 응용 프로그램에 보고하는 **장치** 속성. 장치에서 보고하는 읽기 전용 값이며 실제 장치가 연결되면 응용 프로그램에서 업데이트됩니다. 
- 순수하게 응용 프로그램에만 저장되며 운영자가 편집할 수 있는 **응용 프로그램** 속성. 장치는 응용 프로그램 속성을 인식하지 않습니다.

예를 들어, **템플릿 편집**을 선택하고 새 속성을 입력하면 장치 위치를 새 **텍스트** 속성(응용 프로그램 속성)으로 추가할 수 있습니다.
...
3. 표시 이름과 필드 이름을 구성하고 데이터 형식으로 **위치**를 선택합니다. 
    ...
    | 표시 이름  | 필드 이름 | 데이터 형식 |
    | --------------| -----------|-----------| 
    | 장치 위치 | deviceLoc| location  |

   > [!NOTE]
   > 필드 이름은 해당 장치 코드의 속성 이름과 일치해야 합니다.

   ![위치에 대한 세부 정보가 있는 "장치 속성 구성" 양식](./media/howto-set-up-template/locationdeviceproperty2.png)

실제 장치가 연결되면 장치 속성으로 추가한 위치가 장치에서 전송한 값으로 업데이트됩니다. 응용 프로그램 속성으로 추가한 위치인 설치 주소는 편집 가능한 타일입니다. 위치 속성을 구성했으므로 [장치 대시보드에서 위치를 시각화하려면 지도를 추가](#add-an-azure-maps-location-in-the-dashboard)할 수 있습니다.
...
**저장**, **완료**를 차례로 선택하면 **에코** 명령이 타일로 표시되며, 실제 장치가 연결되면 장치를 에코하는 데 사용하도록 준비됩니다. 명령을 실행하려면 명령의 필드 이름이 해당 장치 코드의 속성 이름과 일치해야 합니다.
...
대시보드는 운영자가 장치에 대한 정보를 볼 수 있는 곳입니다. 작성자는 장치의 동작에 대한 운영자의 이해를 도와주는 타일을 이 페이지에 추가할 수 있습니다. 장치 템플릿에 대시보드 타일을 여러 개 추가할 수 있습니다. 이미지, 꺾은선형 차트, 가로 막대형 차트, KPI(핵심 성과 지표), 설정 및 속성, 레이블 등 여러 유형의 대시보드 타일을 추가할 수 있습니다.
