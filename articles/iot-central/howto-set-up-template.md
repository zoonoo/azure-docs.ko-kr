...
- **위치** 측정 시간 동안 장치의 경도 및 위도 좌표 됩니다. 예를 들어 팬 다른 위치로 이동할 수 있습니다.
...
### <a name="create-a-location-measurement"></a>위치 측정 값 만들기

새 위치 측정을 추가 하려면 선택 **+ 새 측정**, 선택 **위치** 측정으로 형식과 세부 정보를 입력 합니다 **측정 만들기** 폼입니다.

예를 들어 새 위치 원격 분석 값을 추가할 수 있습니다.

| 표시 이름        | 필드 이름    |
| --------------------| ------------- |
| 자산 위치      |  assetloc     |

![위치 측정에 대 한 세부 정보를 사용 하 여 "위치 만들기" 양식](./media/howto-set-up-template/locationmeasurementsform.png)

선택한 후 **저장할**의 **위치** 측정의 측정값 목록에 나타납니다. 잠시, 시뮬레이션된 된 장치에서 위치 데이터의 시각화를 볼 수 있습니다.

위치를 표시 하는 경우 다음 옵션 중에서 선택할 수 있습니다: 최신 위치 및 위치 기록 합니다. **위치 기록** 선택한 시간 범위 동안에 적용 됩니다.

위치 측정의 데이터 형식은 선택적 고도 경도 및 위도 포함 하는 개체입니다. 다음 코드 조각은 JavaScript 구조를 보여 줍니다.
...
설정은 디바이스를 제어합니다. 운영자는 설정을 통해 디바이스에 입력을 제공할 수 있습니다. 디바이스 템플릿에 여러 설정을 추가할 수 있으며, 설정은 운영자가 사용할 수 있도록 **설정** 탭에 타일로 표시됩니다. 다양 한 유형의 설정 추가할 수 있습니다: 숫자, 텍스트, 날짜, 토글 및 섹션 레이블.
...
속성은 고정된 장치 위치 및 일련 번호와 같은 장치와 연결 된 메타 데이터입니다. **속성** 탭에 타일로 표시되는 여러 개의 속성을 디바이스 템플릿에 추가합니다. 속성 수, 텍스트, 날짜, 설정/해제, 장치 속성, 레이블 또는 고정된 된 위치와 같은 형식이 있습니다. 운영자는 장치를 만들고 언제 든 지 이러한 값을 편집할 수 있습니다 속성의 값을 지정 합니다. 디바이스 속성은 읽기 전용이며 디바이스에서 애플리케이션으로 전송됩니다. 운영자는 장치 속성을 변경할 수 없습니다. 실제 장치에 연결 하는 경우 응용 프로그램에서 장치 속성 타일을 업데이트 합니다.

속성에는 다음 두 카테고리가 있습니다.

- 디바이스가 IoT Central 애플리케이션에 보고하는 ‘디바이스 속성’.  디바이스 속성은 디바이스에서 보고하는 읽기 전용 값이며, 실제 디바이스가 연결되면 애플리케이션에서 업데이트됩니다.
- 애플리케이션에 저장되며 운영자가 편집할 수 있는 ‘애플리케이션 속성’.  응용 프로그램 속성 응용 프로그램에만 저장 됩니다 하 고는 장치에서 본 적이 없습니다.
...
### <a name="create-a-location-property"></a>위치 속성 만들기

Azure IoT Central의 위치 데이터에 지리적 컨텍스트를 제공 하 고 모든 위도 및 경도 좌표나 주소를 매핑할 수 있습니다. Azure Maps는 IoT Central에서 이 기능을 구현합니다.

다음 두 가지 종류의 위치 속성을 추가할 수 있습니다.

- 애플리케이션에 저장되는 **애플리케이션 속성인 위치**. 응용 프로그램 속성 응용 프로그램에만 저장 됩니다 하 고는 장치에서 본 적이 없습니다.
- 장치에서 애플리케이션에 보고하는 **장치 속성인 위치**. 이 속성 형식은 정적 위치에 가장 적합 합니다.

> [!NOTE]
> 속성으로 위치 내역을 기록 하지 않습니다. 기록 원한다 면 위치 측정 값을 사용 합니다.
...
실제 장치가 연결 되 면 위치 추가한 장치 속성을 장치에서 보낸 값으로 업데이트 됩니다. 할 수 있습니다 위치 속성을 구성한 후 [장치가 대시보드에서 위치를 시각화 하도록 지도 추가](#add-a-location-in-the-dashboard)합니다.
...
**저장**을 선택하면 **Echo** 명령이 타일로 표시되며, 실제 디바이스가 연결되면 **Device Explorer**에서 사용할 수 있습니다. 명령의 필드 이름을 성공적으로 실행 하려면 명령에 대 한 순서 대로 해당 장치 코드의 속성 이름과 일치 해야 합니다.
...
대시보드는 장치에 대 한 정보를 보려는 운영자의 이동 위치. 작성기를으로 연산자 장치가 작동 하는 방법을 이해 하는 데이 페이지에 타일을 추가 합니다. 이미지, 꺾은선형 차트, 가로 막대형 차트, KPI(핵심 성과 지표), 설정 및 속성, 레이블 등 여러 유형의 대시보드 타일을 추가할 수 있습니다.
...
### <a name="add-a-location-in-the-dashboard"></a>대시보드에서 위치를 추가 합니다.

위치 측정 값을 구성한 경우 장치 대시보드에서는 지도 사용 하 여 위치를 시각화할 수 있습니다.

1. **대시보드** 탭으로 이동합니다.

1. 디바이스 대시보드의 라이브러리에서 **맵**을 선택합니다.

1. 지도에 제목을 지정합니다. 다음 예제에서는 제목이 **장치의 현재 위치**합니다. 다음에서 이전에 구성한 위치 측정을 선택 합니다 **측정** 탭 합니다. 다음 예제에서는 **자산 위치** 측정을 선택 합니다.
...
지도 타일의 크기를 조정할 수 있습니다. 연산자에서 대시보드를 볼 때 합니다 **Device Explorer**, 구성한, 위치 맵 등은 표시 하는 모든 대시보드 타일입니다.
...
> [새 장치 템플릿 버전을 만들려면](howto-version-device-template.md)
