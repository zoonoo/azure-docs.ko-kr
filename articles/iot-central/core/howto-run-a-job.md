...
7. 저장 된 작업을 클릭 하 고 실행 단추를 클릭 하 여 작업을 실행 합니다. 작업 실행 팝업이 표시 됩니다. 작업 실행 단추를 클릭 하 여 확인 합니다. 

    ![작업 실행](./media/howto-run-a-job/run-job.png)

8. 작업은 보류 중, 실행 중, 완료 됨의 여러 단계를 거칩니다. 작업 실행 세부 정보에는 결과 메트릭, 기간 정보 및 장치 목록 표가 포함 됩니다. 이 개요에서 **결과 로그** 를 선택 하 여 장치 및 해당 상태 값을 비롯 한 작업 세부 정보의 CSV 파일을 다운로드할 수도 있습니다. 이 정보는 문제 해결에 유용할 수 있습니다.
...
작업이 중지 됨 상태 이면 **계속** 을 클릭 하 여 작업 실행을 다시 시작할 수 있습니다. 작업 상태가 반영 중으로 변경 되 면 작업이 이제 다시 실행 됩니다. **요약** 섹션은 최신 진행 상태로 계속 업데이트 됩니다.

![중지 된 작업](./media/howto-run-a-job/stopped-job.png)

## <a name="copy-a-job"></a>작업 복사

기존 작업 중 하나를 복사 하려면 **작업** 페이지에서 해당 작업을 선택 하 고 **작업 세부 정보**를 선택 합니다. 작업 세부 정보 페이지가 표시 됩니다. 

![작업 세부 정보](./media/howto-run-a-job/job-details.png)

**복사** 클릭

![작업 세부 정보](./media/howto-run-a-job/job-details-copy.png)
