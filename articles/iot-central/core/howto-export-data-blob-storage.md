...
이 문서에서는 Azure IoT Central의 연속 데이터 내보내기 기능을 사용 하 여 주기적으로 **Azure Blob storage 계정** 또는 **Azure Data Lake Storage Gen2 storage 계정**으로 데이터를 내보내는 방법을 설명 합니다. **측정**, **장치**및 **장치 템플릿을** JSON 또는 Apache Avro 형식의 파일로 내보낼 수 있습니다. 내보낸 데이터는 Azure Machine Learning의 학습 모델 또는 Microsoft Power BI의 장기 추세 분석과 같은 콜드 경로 분석에 사용할 수 있습니다.
...
1. [Azure Portal에서 새 스토리지 계정](https://ms.portal.azure.com/#create/Microsoft.StorageAccount-ARM)을 만듭니다. 새 [Azure Blob storage 계정](https://aka.ms/blobdocscreatestorageaccount) 또는 [Azure Data Lake Storage v2 저장소 계정](https://docs.microsoft.com/azure/storage/blobs/data-lake-storage-quickstart-create-account)만들기에 대해 자세히 알아볼 수 있습니다.
...
4. 드롭다운 목록 상자에서 **저장소 계정 네임 스페이스**를 선택 합니다. **연결 문자열 입력**인 목록의 마지막 옵션을 선택할 수도 있습니다. 
    ...
    ![Blob에 대 한 새 내보내기 만들기](media/howto-export-data/export-create-blob2.png)

5. (선택 사항) **연결 문자열 입력**을 선택한 경우 연결 문자열을 붙여넣을 수 있는 새 상자가 나타납니다. 저장소 계정에 대 한 연결 문자열을 가져오려면 Azure Portal의 저장소 계정으로 이동 하 고, [ **설정**] 아래에서 [ **액세스 키** ]를 선택 합니다. key1 연결 문자열 또는 key2 연결 문자열 복사
...
7. 선호 하는 **데이터 형식** (JSON 또는 [Apache Avro](https://avro.apache.org/docs/current/index.html) 형식)을 선택 합니다.

8. **데이터 내보내기** 아래에서 형식을 **켜기**로 설정하여 내보낼 각 데이터 형식을 지정합니다.

9. 연속 데이터 내보내기를 설정 하려면 **데이터 내보내기** 토글이 **on**으로 설정 되어 있는지 확인 합니다. **저장**을 선택합니다.

   ![연속 데이터 내보내기 구성](media/howto-export-data/export-list-blob2.png)

10. 몇 분 후에 데이터는 저장소 계정에 표시 됩니다.
...
> [!NOTE]
> 장치가 `contentType: application/JSON` 하 고 `contentEncoding:utf-8` (또는 `utf-16``utf-32`) 메시지를 보내고 있는지 확인 합니다. 예는 [IoT Hub 설명서](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-routing-query-syntax#message-routing-query-based-on-message-body) 를 참조 하세요.
...
내보낸 파일은 레코드 당 한 줄을 포함 합니다. 다음 예에서는 디코딩되는 Avro 형식의 레코드를 보여 줍니다.
