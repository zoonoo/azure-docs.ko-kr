...
Microsoft Flow를 사용하여 비즈니스 사용자가 의존하는 많은 애플리케이션 및 서비스 전반에서 워크플로를 자동화합니다. Microsoft Flow에서 IoT Central 커넥터를 사용하면 IoT Central에서 규칙이 트리거될 때 워크플로를 트리거할 수 있습니다. IoT Central 또는 다른 응용 프로그램에 의해 트리거되는 워크플로에서 IoT Central 커넥터의 작업을 사용 하 여 다음 작업을 수행할 수 있습니다.
- 디바이스 만들기
- 장치 정보 가져오기
- 장치의 속성 및 설정 업데이트
- 장치에서 명령 실행
...
이 섹션에서는 IoT Central에서 규칙이 트리거될 때 Flow 모바일 앱에서 모바일 알림을 트리거하는 방법을 보여 줍니다. 포함 된 Microsoft Flow 디자이너를 사용 하 여 IoT Central 앱 내에서이 전체 워크플로를 빌드할 수 있습니다.

1. [IoT Central에서 규칙을 만들어](howto-create-telemetry-rules.md) 시작합니다. 규칙 조건을 저장 한 후 새 작업으로 **Microsoft Flow 작업** 을 선택 합니다. 워크플로를 구성 하는 데 사용할 수 있는 대화 상자 창이 열립니다. 로그인 한 IoT Central 사용자 계정은 Microsoft Flow에 로그인 하는 데 사용 됩니다.

    ![새 Microsoft Flow 작업 만들기](media/howto-add-microsoft-flow/createflowaction.png)

1. 사용자가 액세스할 수 있고이 IoT Central 규칙에 연결 된 워크플로 목록이 표시 됩니다. 템플릿 **탐색** 또는 **새로 만들기 > 템플릿에서 만들기** 를 클릭 하 고 사용할 수 있는 템플릿 중에서 선택할 수 있습니다. 

    ![사용 가능한 Microsoft Flow 템플릿](media/howto-add-microsoft-flow/flowtemplates1.png)

1. 선택한 템플릿의 커넥터에 로그인 하 라는 메시지가 표시 됩니다. 

    > [!NOTE]
    > Azure IoT Central 커넥터를 사용 하려면 Azure Active Directory 계정 (회사 또는 학교 계정)을 사용 하 여 로그인 해야 합니다. abc@outlook.com 또는 abc@live.com와 같은 개인 계정은 Azure IoT Central 커넥터에서 지원 되지 않습니다.

    커넥터에 로그인 한 후에는 디자이너를 통해 워크플로를 빌드합니다. 워크플로에는 애플리케이션 및 규칙이 이미 채워져 있는 IoT Central 트리거가 있습니다.

1. 작업에 전달 되는 정보를 사용자 지정 하 고 새 작업을 추가 하 여 워크플로를 사용자 지정할 수 있습니다. 이 예제에서 작업은 알림입니다. **모바일 알림을 보냅니다**. 알림에 디바이스 이름 및 타임스탬프와 같은 중요한 정보를 전달하여 IoT Central 규칙에서 *동적 콘텐츠*를 포함할 수 있습니다.

    > [!NOTE]
    > 규칙을 트리거한 측정 값과 속성 값을 가져오려면 동적 콘텐츠 창에서 **더 많은 텍스트 보기** 를 선택 합니다.

    ![동적 창을 열어 흐름 편집 작업](./media/howto-add-microsoft-flow/flowdynamicpane1.png)

1. 작업 편집이 완료 되 면 **저장**을 선택 합니다. 워크플로의 개요 페이지로 이동됩니다. 여기에서 실행 기록을 보고 다른 동료와 공유할 수 있습니다.

    > [!NOTE]
    > IoT Central 앱의 다른 사용자가 이 규칙을 편집하도록 하려는 경우 Microsoft Flow에서 공유해야 합니다. 워크플로에서 소유자로 해당 Microsoft Flow 계정을 추가합니다.

1. IoT Central 앱으로 돌아가 작업 영역에서이 규칙이 Microsoft Flow 작업을 수행 하는 것을 볼 수 있습니다.

Microsoft Flow에서 직접 IoT Central 커넥터를 사용 하 여 워크플로를 빌드할 수도 있습니다. 그러면 연결할 IoT Central 앱을 선택할 수 있습니다.
...
1. 애플리케이션을 선택하고, 드롭다운 목록에서 디바이스를 만들 디바이스 템플릿을 선택합니다. 디바이스의 모든 속성 및 설정을 표시하는 작업 확장이 표시됩니다.

1. 디바이스 이름 필드를 선택합니다. 동적 콘텐츠 창에서 **디바이스 이름**을 선택합니다. 이 값은 사용자가 모바일 앱을 통해 입력 한 입력에서 전달 되며 IoT Central의 새 장치 이름입니다. 이 예제에서 유일한 필수 필드는 빨간색 별표로 표시되는 디바이스 이름입니다. 다른 디바이스 템플릿에는 새 디바이스를 만들기 위해 채워야 하는 여러 개의 필수 필드가 있을 수 있습니다.
...
1. 드롭다운에서 애플리케이션을 선택합니다. 이제 업데이트하려는 기존 디바이스의 ID가 필요합니다. 
    ...
    > [!NOTE] 
    > 업데이트 하려는 장치의 장치 세부 정보 페이지에서 **URL에 있는 ID를 사용 해야** 합니다. 장치 탐색기의 장치 목록에 있는 장치 ID는 Microsoft Flow에서 사용 하기에 적합 하지 않습니다.

    ![URL의 IoT Central ID](./media/howto-add-microsoft-flow/iotcdeviceidurl.png)
...
**Azure IoT Central-장치 가져오기** 작업을 사용 하 여 해당 ID로 장치 정보를 가져올 수 있습니다. 
> [!NOTE] 
> 업데이트 하려는 장치의 장치 세부 정보 페이지에서 **URL에 있는 ID를 사용 해야** 합니다. 장치 탐색기의 장치 목록에 있는 장치 ID는 Microsoft Flow에서 사용 하기에 적합 하지 않습니다.

워크플로의 이후 작업에 전달할 장치 이름, 장치 템플릿 이름, 속성 값, 설정 값 등의 정보를 가져올 수 있습니다. 다음은 장치에서 Microsoft 팀으로 고객 이름 속성 값을 따라 전달 하는 예제 워크플로입니다.

   ![Flow 장치 워크플로 가져오기](./media/howto-add-microsoft-flow/flowgetdevice1.png)


## <a name="run-a-command-on-a-device-in-a-workflow"></a>워크플로의 장치에서 명령 실행
**Azure IoT Central-명령 실행 작업을** 사용 하 여 해당 ID로 지정 된 장치에서 명령을 실행할 수 있습니다. 

> [!NOTE] 
> 업데이트 하려는 장치의 장치 세부 정보 페이지에서 **URL에 있는 ID를 사용 해야** 합니다. 장치 탐색기의 장치 목록에 있는 장치 ID는 Microsoft Flow에서 사용 하기에 적합 하지 않습니다.
    
명령을 선택 하 여 실행 하 고이 작업을 통해 명령의 매개 변수를 전달할 수 있습니다. Microsoft Flow 모바일 앱의 단추에서 장치 재부팅 명령을 실행 하는 예제 워크플로는 다음과 같습니다.

   ![Flow 장치 워크플로 가져오기](./media/howto-add-microsoft-flow/flowrunacommand1.png)

## <a name="delete-a-device-in-a-workflow"></a>워크플로에서 디바이스 삭제

**Azure IoT Central-장치 삭제** 작업을 사용 하 여 해당 ID로 장치를 삭제할 수 있습니다. 
> [!NOTE] 
> 업데이트 하려는 장치의 장치 세부 정보 페이지에서 **URL에 있는 ID를 사용 해야** 합니다. 장치 탐색기의 장치 목록에 있는 장치 ID는 Microsoft Flow에서 사용 하기에 적합 하지 않습니다.
...
1. Microsoft 개인 계정(예: @hotmail.com, @live.com, @outlook.com 도메인)은 현재 지원되지 않습니다. Azure Active Directory (AD) 회사 또는 학교 계정을 사용 해야 합니다.
...
3. Azure AD 계정을 사용 하는 동안 오류가 발생 하는 경우 Windows PowerShell을 열고 다음 commandlet을 관리자 권한으로 실행 합니다.
...
이제 Microsoft Flow를 사용 하 여 워크플로를 작성 하는 방법을 배웠으므로 제안 된 다음 단계는 [장치를 관리](howto-manage-devices.md)하는 것입니다.
