...
원격 분석을 표시할 때 평균, 최소값, 최대값, 합계 및 개수의 집계 옵션 중에서 선택할 수 있습니다. 차트의 기본 집계로 **평균이** 선택 됩니다.
...
- **동기화됨**: 설정 값을 반영하도록 디바이스가 변경되었습니다.

- **보류 중**: 디바이스가 현재 설정 값으로 변경 중입니다.

- **오류**: 디바이스에서 오류를 반환했습니다.

예를 들어 **설정을** 선택 하 고 새 **숫자** 설정을 입력 하 여 새 팬 속도 설정을 추가할 수 있습니다.

| 표시 이름  | 필드 이름    |  단위  | 10진수 |Initial|
| --------------| ------------- |---------| ---------|---- |
...
속성은 장치 (예: 고정 장치 위치 및 일련 번호)와 연결 된 메타 데이터입니다. **속성** 탭에서 타일로 표시 되는 여러 속성을 장치 템플릿에 추가 합니다. 속성에는 숫자, 텍스트, 날짜, 토글, 장치 속성, 레이블 또는 고정 위치와 같은 형식이 있습니다. 연산자는 장치를 만들 때 속성의 값을 지정 하 고, 언제 든 지 이러한 값을 편집할 수 있습니다. 디바이스 속성은 읽기 전용이며 디바이스에서 애플리케이션으로 전송됩니다. 운영자는 장치 속성을 변경할 수 없습니다. 실제 장치가 연결 되 면 장치 속성 타일이 응용 프로그램에서 업데이트 됩니다.
...
- **설정**: 설정은 장치에 적용 하려는 구성입니다. 변경할 때까지 해당 구성을 디바이스에서 유지하려고 합니다. 예를 들어, 냉동기 온도를 설정하고 냉동기를 다시 시작해도 해당 설정이 유지되기를 바랍니다.

- **명령**: IoT Central에서 원격으로 디바이스에 명령을 즉시 실행하기 위해 명령을 사용합니다. 디바이스가 연결되지 않은 경우 명령은 시간 초과되고 실패합니다. 예를 들어, 디바이스를 다시 시작하려 합니다.
...
1. 지도에 제목을 지정합니다. 다음 예제에는 **현재 위치**의 제목 장치가 있습니다. 그런 다음 **속성** 탭에서 이전에 구성한 location 속성을 선택 합니다. 다음 예제에서는 **장치 위치** 측정을 선택 합니다.
