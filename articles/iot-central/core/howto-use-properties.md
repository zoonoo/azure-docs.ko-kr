...
이 방법 가이드에서는 장치 개발자가 Azure IoT Central 응용 프로그램의 장치 템플릿에 정의 된 장치 속성을 사용 하는 방법을 보여 줍니다.

속성은 특정 시간 값을 나타냅니다. 예를 들어 디바이스에서 속성을 사용하여 도달하려는 목표 온도를 보고할 수 있습니다. 기본적으로 장치 속성은 IoT Central에서 읽기 전용입니다. 쓰기 가능 속성을 사용 하면 장치와 Azure IoT Central 응용 프로그램 간의 상태를 동기화 할 수 있습니다.
...
속성은 장치의 상태를 나타내는 데이터 필드입니다. 속성을 사용 하 여 장치의 설정/해제 상태와 같은 장치의 내구성이 있는 상태를 나타냅니다. 속성은 장치의 소프트웨어 버전 같은 기본 장치 속성을 나타낼 수도 있습니다. 속성은 읽기 전용 또는 쓰기 가능으로 선언 합니다.

다음 스크린샷은 Azure IoT Central 응용 프로그램의 속성 정의를 보여 줍니다.

:::image type="content" source="media/howto-use-properties/property-definition.png" alt-text="Azure IoT Central 응용 프로그램의 속성 정의를 보여 주는 스크린샷":::
...
| Name            | 속성의 이름입니다. Azure IoT Central는 표시 이름에서이 필드에 대 한 값을 생성 하지만 필요한 경우 고유한 값을 선택할 수 있습니다. 이 필드는 영숫자여야 합니다.  장치 코드는이 **이름** 값을 사용 합니다.           |
| 기능 유형 | 속성입니다.                                                                                                                                                                                                                          |
...
이 예제에서는 두 개의 속성을 보여 줍니다. 이러한 속성은 UI의 속성 정의와 관련이 있습니다.
...
:::image type="content" source="media/howto-use-properties/object.png" alt-text="개체를 정의 하는 방법을 보여 주는 스크린샷":::
...
기본적으로 속성은 읽기 전용입니다. 읽기 전용 속성을 사용 하면 장치 보고서 속성 값이 Azure IoT Central 응용 프로그램으로 업데이트 됩니다. Azure IoT Central 응용 프로그램에서 읽기 전용 속성 값을 설정할 수 없습니다.
...
장치 모델의 다음 코드 조각은 읽기 전용 속성 유형에 대 한 정의를 보여 줍니다.
...
장치에서 JSON 페이로드로 속성 업데이트를 보냅니다. 자세한 내용은 [페이로드](./concepts-telemetry-properties-commands.md)를 참조 하세요.
...
:::image type="content" source="media/howto-use-properties/read-only.png" alt-text="읽기 전용 속성의 뷰를 보여 주는 스크린샷":::
...
장치 모델의 다음 코드 조각은 쓰기 가능한 속성 형식의 정의를 보여 줍니다.
