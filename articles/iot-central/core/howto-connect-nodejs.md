...
**장치 연결** 페이지에서 **범위 ID**, **장치 ID**및 **기본 키**에 대 한 장치 연결 정보를 적어 둡니다. 이 방법 가이드의 뒷부분에서 이러한 값을 장치 코드에 추가 합니다.
...
1. 다음 변수 선언을 파일에 추가합니다.
    ...
    자리 표시자 `{your Scope ID}`, `{your Device ID}`및 `{your Primary Key}`를 앞에서 기록한 값으로 업데이트 합니다. 이 샘플에서는 `targetTemperature`를 0으로 초기화 하 고, 장치의 현재 읽기 또는 장치 쌍의 값을 사용할 수 있습니다.
