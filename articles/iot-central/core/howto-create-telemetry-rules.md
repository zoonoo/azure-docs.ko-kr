---
title: Azure IoT Central 응용 프로그램에서 원격 분석 규칙 사용 | Microsoft Docs
...
원격 분석 규칙을 만들려면 디바이스 템플릿에 하나 이상의 원격 분석 측정이 정의되어 있어야 합니다. 이 예제에서는 온도 및 습도 원격 분석 데이터를 전송하는 냉장 자동 판매기를 사용합니다. 규칙은 장치에서 보고 하는 온도를 모니터링 하 고 70&deg; F 이상으로 이동 하면 전자 메일을 보냅니다.
...
1. 이 템플릿에 대해 만들어진 모든 장치에 대해 규칙을 즉시 사용 하도록 설정 하려면 **이 템플릿의 모든 장치에 대해 규칙 사용**을 설정/해제 합니다.
...
1. **조건** 옆에 있는 **+** 를 선택 하 여 새 조건을 추가 합니다.

1. **단위** 드롭다운에서 모니터링하려는 원격 분석을 선택합니다.

1. 그런 다음, **집계**, **연산자**를 선택하고 **임계값**을 입력합니다.
   - 집계는 선택 사항입니다. 집계를 사용하지 않으면 조건을 충족하는 각 원격 분석 데이터 요소에 대해 규칙이 트리거됩니다. 예를 들어 온도가 70&deg; F를 초과 하는 경우 트리거를 구성 하는 경우, 장치가 온도 > 70를 보고할 때 규칙은 거의 즉시 트리거됩니다.
   - Average, Min, Max, Count와 같은 집계 함수를 선택할 경우, 사용자는 조건을 평가할 **집계 기간**을 지정해야 합니다. 예를 들어 기간을 "5 분"으로 설정 하 고 규칙이 70 이상 평균 온도를 검색 하는 경우 5 분 이상 평균 온도가 70&deg; F 인 경우 규칙이 트리거됩니다. 규칙 평가 주기는 **집계 기간**과 동일합니다. 즉, 이 예에서 규칙은 5분에 한 번 평가됩니다.
...
규칙은 **디바이스 속성**에서 특정 값을 매개 변수로 얻을 수 있습니다. 매개 변수를 사용하면 디바이스에 따라 원격 분석 임계값이 달라지는 시나리오에서 유용합니다. 규칙을 만들 때 70&deg; F와 같이 절대 값을 제공 하는 대신 **최대 이상적인 임계값**과 같은 임계값을 지정 하는 장치 속성을 선택 합니다. 규칙을 실행 하면 장치 속성에 설정 된 값을 사용 하 여 장치 원격 분석과 일치 합니다.
