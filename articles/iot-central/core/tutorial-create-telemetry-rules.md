...
원격 분석 규칙을 만들려면 디바이스 템플릿에 하나 이상의 원격 분석 측정이 정의되어 있어야 합니다. 이 자습서에서는 온도 및 습도 원격 분석 데이터를 전송하는 환경 센서 디바이스를 사용합니다. [IoT Central 애플리케이션에 시뮬레이션된 디바이스 추가](./quick-create-pnp-device.md) 빠른 시작에서는 이 디바이스 템플릿을 추가하고 시뮬레이션된 디바이스를 만들었습니다. 이 규칙은 디바이스에서 보고한 온도를 모니터링하다가 온도가 70도를 초과하면 이메일을 보냅니다.
...
1. 다음으로, **연산자**로 **보다 큼**을 선택하고 **값**으로 _70_을 입력합니다.
...
1. 선택 사항으로, **시간 집계**를 설정할 수 있습니다. 시간 집계를 선택하는 경우 집계 드롭다운에서 평균 또는 합계와 같은 집계 유형도 선택해야 합니다.
    ...
    * 집계를 사용하지 않으면 조건을 충족하는 각 원격 분석 데이터 요소에 대해 규칙이 트리거됩니다. 예를 들어 온도가 70도를 초과하면 트리거되도록 규칙을 구성한 경우 디바이스에서 보고하는 온도가 70도를 초과하면 거의 즉시 규칙이 트리거됩니다.
    * 집계를 사용하면 해당 시간 범위의 원격 분석 데이터 요소 집계 값이 조건을 충족할 때 규칙이 트리거됩니다. 예를 들어 온도가 70도를 초과하면 트리거되도록 규칙을 구성하고, 시간 집계를 10분으로 설정하고, 집계 유형이 평균인 경우 디바이스에서 10분 간격으로 계산하여 보고하는 평균 온도가 70도를 초과하면 규칙이 트리거됩니다.
