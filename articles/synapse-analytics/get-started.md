...
description: Azure Synapse의 기본 개념을 단계별로 빠르게 이해할 수 있습니다.
...
## <a name="verify-the-synapse-workspace-msi-has-access-to-the-storage-account"></a>스토리지 계정에 대한 액세스 권한이 Synapse 작업 영역 MSI에 있는지 확인

이 작업은 이미 완료되었을 수 있습니다. 어떻든 간에 확인해야 합니다.

1. [Azure Portal](https://portal.azure.com)을 열고, 작업 영역에 대해 선택한 기본 스토리지 계정을 엽니다.
1. 왼쪽 탐색 영역에서 **액세스 제어(IAM)** 를 선택합니다. 그런 다음, 다음 역할을 할당하거나 이미 할당되어 있는지 확인합니다. 
    a. 작업 영역 ID를 스토리지 계정의 **Storage Blob 데이터 기여자** 역할에 할당합니다. 작업 영역 ID는 작업 영역의 이름과 동일합니다. 이 문서에서 작업 영역 이름은 `myworkspace`이므로 작업 영역 ID도 `myworkspaced`입니다.
1. **저장**을 선택합니다.
    
## <a name="launch-synapse-studio"></a>Synapse Studio 시작

Synapse 작업 영역이 만들어지면 두 가지 방법으로 Synapse Studio를 열 수 있습니다.
* [Azure Portal](https://portal.azure.com)에서 Synapse 작업 영역을 열고, **개요** 섹션 위쪽에서 **Synapse Studio 시작**을 선택합니다.
* https://web.azuresynapse.net 으로 직접 이동하여 작업 영역에 로그인합니다.
...
1. Synapse Studio의 왼쪽에서 **관리 > Apache Spark 풀**을 차례로 선택합니다.
1. **+ 새로 만들기**를 선택하고 다음 설정을 입력합니다.
...
> [!NOTE]
> 이름과 관계없이 Apache Spark 풀은 SQL 풀과 다릅니다. Spark와 상호 작용하는 방법을 Synapse 작업 영역에 알리는 데 사용하는 몇 가지 기본 메타데이터일 뿐입니다. 
...
모든 작업 영역에는 미리 빌드되고 삭제할 수 없는 **SQL 주문형**이라는 풀이 제공됩니다. SQL 주문형 풀을 사용하면 Synapse SQL 풀을 만들지 않고 SQL을 사용하거나 Synapse SQL 풀의 관리를 고려할 수 있습니다. 다른 종류의 풀과 달리 SQL 주문형에 대한 요금 청구는 쿼리를 실행하는 데 사용된 리소스 수가 아니라 쿼리를 실행하는 데 검사된 데이터의 양을 기준으로 합니다.

* SQL 주문형에는 SQL 주문형 풀과 별도로 존재하는 자체의 SQL 주문형 데이터베이스도 있습니다.
* 현재 작업 영역에는 항상 **SQL 주문형**이라는 하나의 SQL 주문형 풀이 반드시 있습니다.

## <a name="load-the-nyc-taxi-sample-data-into-the-sqldb1-database"></a>SQLDB1 데이터베이스에 NYC 택시 샘플 데이터 로드

1. Synapse Studio 위쪽의 파란색 메뉴에서 **?** 를 선택합니다. 아이콘.
1. **시작 > 허브 시작**을 차례로 선택합니다.
1. **쿼리 샘플 데이터** 레이블이 지정된 카드에서 `SQLDB1`이라는 SQL 풀을 선택합니다.
1. **데이터 쿼리**를 선택합니다. "샘플 데이터 로드 중"이라는 알림이 표시되었다가 사라집니다.
1. Synapse Studio 위쪽 근처에 데이터가 SQLDB1에 로드되고 있음을 나타내는 연한 파란색 알림 표시줄이 표시됩니다. 녹색이 될 때까지 기다렸다가 해제합니다.
...
1. 이 쿼리는 총 주행 거리 및 평균 주행 거리와 승객 수 간의 관계를 보여 줍니다.
...
1. **코드 추가**를 선택하여 Notebook 코드 셀을 추가하고 아래 텍스트를 붙여넣습니다.
...
1. 다음 코드를 실행하여 이전에 `SQLDB1` SQL 풀에서 수행한 것과 동일한 분석을 수행합니다. 또한 이 코드는 분석 결과를 `nyctaxi.passengercountstats`라는 테이블에 저장하고 결과를 시각화합니다.
...
## <a name="customize-data-visualization-data-with-spark-and-notebooks"></a>Spark 및 Notebook을 사용하여 데이터 시각화 데이터 사용자 지정

Notebook을 사용하면 차트를 렌더링하는 방법을 제어할 수 있습니다. 다음 코드에서는 인기 있는 `matplotlib` 및 `seaborn` 라이브러리를 사용하는 간단한 예제를 보여 줍니다. 이전에 SQL 쿼리를 실행할 때 표시된 것과 동일한 종류의 꺾은선형 차트를 렌더링합니다.
...
이전에는 데이터를 `SQLDB1.dbo.Trip` SQL 풀 테이블에서 `nyctaxi.trip` Spark 테이블로 복사했습니다. 그런 다음, Spark를 사용하여 데이터를 `nyctaxi.passengercountstats` Spark 테이블에 집계했습니다. 이제 데이터를 `nyctaxi.passengercountstats`에서 `SQLDB1.dbo.PassengerCountStats`라는 SQL 풀 테이블로 복사합니다. 

Notebook에서 아래 셀을 실행합니다. 집계된 Spark 테이블을 SQL 풀 테이블에 다시 복사합니다.
...
## <a name="analyze-nyc-taxi-data-in-spark-databases-using-sql-on-demand"></a>SQL 주문형을 사용하여 Spark 데이터베이스의 NYC 택시 데이터 분석 

Spark 데이터베이스의 테이블은 SQL 주문형에서 자동으로 표시되고 쿼리할 수 있습니다.
...
1. 다음 텍스트를 스크립트에 붙여넣고, 해당 스크립트를 실행합니다.
    ...
    ```
    > [!NOTE]
    > SQL 주문형을 사용하는 쿼리를 처음 실행하는 경우 SQL 주문형에서 쿼리를 실행하는 데 필요한 SQL 리소스를 수집하는 데 약 10초 정도 걸립니다. 이후 쿼리에서는 이러한 대기 시간이 필요하지 않으므로 훨씬 빨라집니다.
  
## <a name="orchestrate-activities-with-pipelines"></a>파이프라인을 사용하여 작업 오케스트레이션

Azure Synapse에서는 다양한 작업을 오케스트레이션할 수 있습니다. 이 섹션에서 얼마나 쉬운지 확인합니다.

1. Synapse Studio에서 **오케스트레이션** 허브로 이동합니다.
1. **+** , **파이프라인**을 차례로 선택합니다. 새 파이프라인이 만들어집니다.
...
1. **확인**을 선택합니다.
1. **모두 게시**를 선택합니다. 그러면 파이프라인이 1시간마다 실행됩니다.
1. 다음 시간을 기다리지 않고 지금 파이프라인을 실행하려면 **트리거 추가 > 새로 만들기/편집**을 차례로 선택합니다.

## <a name="working-with-data-in-a-storage-account"></a>스토리지 계정의 데이터 작업

지금까지 데이터가 작업 영역의 데이터베이스에 상주한 시나리오에 대해 살펴보았습니다. 이제 스토리지 계정의 파일을 사용하는 방법을 보여 주겠습니다. 이 시나리오에서는 작업 영역을 만들 때 지정한 작업 영역 및 컨테이너의 기본 스토리지 계정을 사용합니다.

* 스토리지 계정의 이름: `contosolake`
* 스토리지 계정에 있는 컨테이너의 이름: `users`

### <a name="creating-csv-and-parquet-files-in-your-storage-account"></a>스토리지 계정에 CSV 및 Parquet 파일 만들기

Notebook에서 다음 코드를 실행합니다. CSV 파일과 parquet 파일을 스토리지 계정에 만듭니다.
...
### <a name="analyzing-data-in-a-storage-account"></a>스토리지 계정의 데이터 분석
...
1. 셀을 실행합니다.
1. 마우스 오른쪽 단추로 내부의 parquet 파일을 클릭하고 **새 SQL 스크립트 > 상위 100개 행 선택**을 차례로 선택합니다. 그러면 다음과 같은 SQL 스크립트가 만들어집니다.
...
1. 스크립트에서 **연결 대상** 필드가 **SQL 주문형**으로 설정됩니다.
1. 스크립트를 실행합니다.

## <a name="visualize-data-with-power-bi"></a>Power BI를 사용하여 데이터 시각화

NYX 택시 데이터에서 집계된 데이터 세트를 두 테이블에 만들었습니다.
* `nyctaxi.passengercountstats`
* `SQLDB1.dbo.PassengerCountStats`

Power BI 작업 영역을 Synapse 작업 영역에 연결할 수 있습니다. 이렇게 하면 데이터를 Power BI 작업 영역에 쉽게 가져올 수 있으며, Synapse 작업 영역에서 Power BI 보고서를 직접 편집할 수 있습니다.
...
### <a name="link-your-synapse-workspace-to-your-new-power-bi-workspace"></a>새 Power BI 작업 영역에 Synapse 작업 영역 연결

1. Synapse Studio에서 **관리 > 연결 된 서비스**로 차례로 이동합니다.
...
1. 다운로드한 `.pbids` 파일을 엽니다. 
1. 그러면 Power BI 데스크톱이 시작되어 Synapse 작업 영역의 `SQLDB1`에 자동으로 연결됩니다.
1. **SQL 서버 데이터베이스**라는 대화 상자가 표시되는 경우 입니다. a. **Microsoft 계정**을 선택합니다. 
    b. **로그인**을 선택하여 로그인합니다.
    다. **연결**을 선택합니다.
1. **탐색기** 대화 상자가 열립니다. 그러면 **PassengerCountStats** 테이블을 확인하고 **로드**를 선택합니다.
1. **연결 설정** 대화 상자가 표시됩니다. **DirectQuery**를 선택하고, **확인**을 선택합니다.
1. 왼쪽의 **보고서** 단추를 선택합니다.
1. **꺾은선형 차트**를 보고서에 추가합니다.
    a. **PasssengerCount** 열을 **시각화 > 축**으로 끕니다. b. **SumTripDistance** 및 **AvgTripDistance** 열을 **시각화 > 값**으로 끕니다.
1. **홈** 탭에서 **게시**를 선택합니다.
1. 변경 내용을 저장할 것인지 묻는 메시지가 표시됩니다. **저장**을 선택합니다.
1. 파일 이름을 선택하라는 메시지가 표시됩니다. 그러면 `PassengerAnalysis.pbix`를 선택하고 **저장**을 선택합니다.
1. **대상 선택**을 요청하는 메시지가 표시됩니다.그러면 `NYCTaxiWorkspace1`, **선택**을 차례로 선택합니다.
1. 게시가 완료될 때까지 기다립니다.
...
1. 이 작업 영역 내에는 `Passenger Analysis`라는 데이터 세트와 `Passenger Analysis`라는 보고서가 표시됩니다.
1. 마우스로 `PassengerAnalysis` 데이터 세트 위를 가리키고, 세 개의 점이 있는 아이콘을 선택하고, **설정**을 선택합니다.
1. **데이터 원본 자격 증명**에서 **인증 방법**을 **OAuth2**로 설정하고, **로그인**을 선택합니다.

### <a name="edit-a-report-in-synapse-studio"></a>Synapse Studio에서 보고서 편집

1. Synapse Studio로 돌아가서 **닫기 및 새로 고침**을 선택합니다. 
1. **개발** 허브로 이동합니다. 
1. 마우스로 **Power BI** 위를 가리키고, **Power BI 보고서** 노드 새로 고침을 클릭합니다.
1. 이제 **Power BI** 아래에 다음과 같이 표시됩니다. a. * **NYCTaxiWorkspace1 > Power BI 데이터 세트** 아래에 **PassengerAnalysis**라는 새 데이터 세트가 있습니다.
    b. * **NYCTaxiWorkspace1 > Power BI 보고서** 아래에 **PassengerAnalysis**라는 아래에 새 보고서가 있습니다.
1. **PassengerAnalysis** 보고서를 선택합니다. 
1. 보고서가 열리고, 이제 Synapse Studio 내에서 보고서를 직접 편집할 수 있습니다.
...
1. **파이프라인 실행**, **Apache Spark 애플리케이션** 및 **SQL 요청**을 검색하면 작업 영역에서 이미 수행한 작업을 확인할 수 있습니다.

## <a name="next-steps"></a>다음 단계

[Azure Synapse Analytics(미리 보기)](overview-what-is.md)에 대해 자세히 알아봅니다.
