---
title: '자습서: AutoML을 사용하여 기계 학습 모델 학습'
description: AutoML을 사용하여 Azure Synapse에서 기계 학습 모델을 학습시키는 방법에 대한 자습서입니다.
...
ms.locfileid: "96464490"
---
# <a name="tutorial-code-free-machine-learning-model-training-in-azure-synapse-with-automl-preview"></a>자습서: AutoML을 사용하여 Azure Synapse에서 코드 프리 기계 학습 모델 학습(미리 보기)

Azure Machine Learning에서 [AutoML](https://docs.microsoft.com/azure/machine-learning/concept-automated-ml)을 사용하여 학습하는 새로운 기계 학습 모델을 통해 Spark 테이블의 데이터를 쉽게 보강하는 방법에 대해 알아봅니다.  Synapse의 사용자는 Azure Synapse 작업 영역에서 Spark 테이블을 선택하여 코드 프리 환경에서 기계 학습 모델을 빌드하기 위한 학습 데이터 세트로 사용할 수 있습니다.

이 자습서에서 학습할 방법은 다음과 같습니다.

> [!div class="checklist"]
> - Azure ML에서 자동화된 ML을 사용하는 Azure Synapse 스튜디오에서 코드 프리 환경을 사용하여 기계 학습 모델을 학습시킵니다. 학습되는 모델의 유형은 해결하려는 문제에 따라 달라집니다.
...
## <a name="launch-automl-wizard-to-train-a-model"></a>AutoML 마법사를 시작하여 모델 학습

마우스 오른쪽 단추로 이전 단계에서 만든 Spark 테이블을 클릭합니다. "Machine Learning-> 새 모델로 보강"을 차례로 선택하여 마법사를 엽니다.
![AutoML 마법사 시작](media/tutorial-automl-wizard/tutorial-automl-wizard-00d.png)

구성 패널이 표시되고, Azure Machine Learning에서 AutoML 실험을 만들기 위한 구성 세부 정보를 제공하라는 메시지가 표시됩니다. 이 실행은 여러 모델을 학습시키고, 성공적인 최상의 실행 모델이 Azure ML 모델 레지스트리에 등록됩니다.

![실행 구성 단계 1](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00a.png)

- **Azure ML 작업 영역**: AutoML 실험 실행을 만들려면 Azure ML 작업 영역이 필요합니다. 또한 [연결된 서비스](quickstart-integrate-azure-machine-learning.md)를 사용하여 Azure Synapse 작업 영역을 Azure ML 작업 영역과 연결해야 합니다. 모든 필수 구성 요소가 있으면 이 AutoML 실행에 사용하려는 Azure ML 작업 영역을 지정할 수 있습니다.

- **실험 이름**: 실험 이름을 지정합니다. AutoML 실행을 제출할 때 실험 이름을 제공합니다. 실행에 대한 정보는 Azure ML 작업 영역의 해당 실험 아래에 저장됩니다. 이 환경에서는 기본적으로 새 실험을 만들고 제안된 이름을 생성하지만 기존 실험의 이름을 제공할 수도 있습니다.

- **최상의 모델**: AutoML 실행에서 최상의 모델에 대한 이름을 지정합니다. 최상의 모델에는 이 이름이 지정되고, 이 실행 후에 Azure ML 모델 레지스트리에 자동으로 저장됩니다. AutoML 실행은 많은 기계 학습 모델을 만듭니다. 이후 단계에서 선택하는 기본 메트릭에 따라 이러한 모델을 비교하여 최상의 모델을 선택할 수 있습니다.

- **대상 열**: 이는 예측을 위해 모델에서 학습되는 항목입니다. 예측하려는 열을 선택합니다.

- **Spark 풀**: AutoML 실험 실행에 사용할 Spark 풀입니다. 계산은 지정한 풀에서 실행됩니다.
...
- **기본 메트릭**: 모델의 성능을 측정하는 데 사용되는 메트릭입니다. 이는 AutoML 실행에서 만든 여러 모델을 비교하여 가장 효율적으로 수행되는 모델을 결정하는 데 사용되는 메트릭입니다.

- **학습 작업 시간(시간)** : 실험에서 모델을 실행하고 학습시키는 데 걸리는 최대 시간(시간)입니다. 1보다 작은 값을 제공할 수도 있습니다. 예: `0.5`.

- **최대 동시 반복 횟수**: 동시에 실행되는 최대 반복 횟수를 나타냅니다.

- **ONNX 모델 호환성**: 사용하도록 설정되면 AutoML을 통해 학습된 모델이 ONNX 형식으로 변환됩니다. 이는 특히 모델을 Azure Synapse SQL 풀의 점수 매기기에 사용하려는 경우에 적합합니다.
...
필요한 모든 구성이 완료되면 AutoML 실행을 시작할 수 있습니다.

Azure Synapse에서 AutoML 실행을 시작하는 두 가지 방법이 있습니다. 코드 프리 환경에서는 **실행 만들기** 를 직접 선택할 수 있습니다. 코드를 선호하는 경우 **Notebook에서 열기** 를 선택할 수 있습니다. 그러면 실행을 만들고 Notebook을 실행하는 코드를 확인할 수 있습니다.

### <a name="create-run-directly"></a>직접 실행 만들기

"실행 시작"을 클릭하여 AutoML 실행을 직접 시작합니다. AutoML 실행이 시작되고 있다는 알림이 표시됩니다.

AutoML 실행이 성공적으로 시작되면 또 다른 성공 알림이 표시됩니다. 알림 단추를 클릭하여 실행 제출 상태를 확인할 수도 있습니다.
성공 알림의 링크를 클릭하여 Azure ML.
...
이렇게 하면 추가 설정을 AutoML 실행에 추가할 수도 있습니다.

![Notebook 열기](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00e.png)

Notebook의 실행이 성공적으로 제출되면 Notebook 출력에 Azure ML 작업 영역의 실험 실행에 대한 링크가 있습니다. 링크를 클릭하여 Azure ML에서 AutoML 실행을 모니터링할 수 있습니다.
