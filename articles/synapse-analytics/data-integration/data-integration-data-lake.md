---
title: Azure 시냅스 분석에서 Azure 데이터 레이크 스토리지 Gen2로 수집
description: Azure 시냅스 분석에서 Azure 데이터 레이크 스토리지 Gen2로 데이터를 수집하는 방법 알아보기
...
---
# <a name="ingesting-data-into-azure-data-lake-storage-gen2"></a>Azure 데이터 레이크 스토리지 Gen2로 데이터 수집 

이 문서에서는 Azure Synapse 분석을 사용하여 Azure Data Lake Gen 2(Azure Data Lake Gen 2) 저장소 계정에서 한 위치에서 다른 위치로 데이터를 수집하는 방법을 알아봅니다.

## <a name="prerequisites"></a>사전 요구 사항

* **Azure 구독**: Azure 구독이 없는 경우 시작하기 전에 [무료 Azure 계정을](https://azure.microsoft.com/free/) 만듭니다.
* **Azure 저장소 계정**: Azure Data Lake Gen 2를 *원본* 데이터 저장소로 사용합니다. 저장소 계정이 없는 경우 [Azure Storage 계정 만들기를](../../storage/blobs/data-lake-storage-quickstart-create-account.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json) 참조하여 계정을 만드는 단계를 확인합니다.
...
파이프라인에는 활동 집합의 실행에 대한 논리적 흐름이 포함되어 있습니다. 이 섹션에서는 Azure Data Lake Gen 2의 데이터를 SQL 풀로 수집하는 복사 활동이 포함된 파이프라인을 만듭니다.
...
1. 데이터 저장소를 저장할 때 Azure 데이터 레이크 저장소 Gen2를 선택하고 계속을 클릭합니다.
...
1. 서식으로 구분텍스트를 선택하고 계속을 클릭합니다.
1. 속성 설정 창에서 만든 ADLS 연결 서비스를 선택합니다. 데이터를 작성할 폴더의 경로를 지정합니다. 작업이 완료되면 [확인]을 클릭합니다.
...
Synapse Analytics의 데이터 통합에 대한 자세한 내용은 [SQL 풀에 데이터를 통합하는](data-integration-sql-pool.md) 문서를 참조하세요.
