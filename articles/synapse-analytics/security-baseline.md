...
ms.locfileid: "104586852"
---
# <a name="azure-security-baseline-for-synapse-analytics"></a>Synapse Analytics에 대 한 Azure 보안 기준

이 보안 기준은 [Azure Security 벤치 마크 버전 1.0](../security/benchmarks/overview-v1.md) 의 지침을 Synapse Analytics에 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다.
콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤과** Synapse Analytics에 적용 되는 관련 지침에 따라 그룹화 됩니다. Synapse Analytics에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

 
Synapse Analytics가 Azure Security 벤치 마크에 완전히 매핑되는 방식을 보려면 [Full Synapse Analytics 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
- [Azure Synapse SQL에 대 한 개인 링크 이해](../azure-sql/database/private-endpoint-overview.md)
...
- [Azure Synapse SQL에 대 한 ATP 이해](../azure-sql/database/threat-detection-overview.md)

- [Azure SQL Database에 대 한 고급 데이터 보안을 사용 하도록 설정 하는 방법](../azure-sql/database/azure-defender-for-sql.md)

- [광고 개요](../azure-sql/database/azure-defender-for-sql.md)
...
- [Azure SQL Database에 대 한 서비스 끝점을 사용 하 여 서비스 태그 이해](https://docs.microsoft.com/azure/azure-sql/database/vnet-service-endpoint-rule-overview#limitations)
...
- [Azure SQL Database 감사를 사용 하도록 설정 하는 방법](../azure-sql/database/auditing-overview.md)

- [서버에 대 한 감사를 사용 하도록 설정 하는 방법](https://docs.microsoft.com/azure/azure-sql/database/auditing-overview#setup-auditing)

- [서버 수준 및 데이터베이스 수준 감사 정책의 차이점](https://docs.microsoft.com/azure/azure-sql/database/auditing-overview#server-vs-database-level)
...
- [Log Analytics 작업 영역에서 로그 보존 매개 변수를 설정 하는 방법](https://docs.microsoft.com/azure/azure-monitor/logs/manage-cost-storage#change-the-data-retention-period)
...
- [Azure SQL Database에 대 한 고급 위협 방지 및 경고 이해](../azure-sql/database/threat-detection-overview.md)

- [Azure SQL Database에 대 한 고급 데이터 보안을 사용 하도록 설정 하는 방법](../azure-sql/database/azure-defender-for-sql.md)

- [Azure SQL Database에 대 한 사용자 지정 경고를 구성 하는 방법](../azure-sql/database/alerts-insights-configure-portal.md)
...
**지침**: 사용자는 Azure Active Directory (Azure AD) 또는 SQL 인증을 사용 하 여 인증 됩니다.

처음으로 Azure SQL을 배포할 때 해당 로그인에 대 한 관리자 로그인 및 연결 된 암호를 지정 합니다. 이 관리 계정을 서버 관리자 라고 합니다. Azure Portal를 열고 서버 또는 관리 되는 인스턴스의 속성 탭으로 이동 하 여 데이터베이스에 대 한 관리자 계정을 식별할 수 있습니다. 모든 관리 권한을 사용 하 여 Azure ad 관리자 계정을 구성할 수도 있습니다 .이는 Azure AD 인증을 사용 하도록 설정 하려는 경우에 필요 합니다.
...
**참고**: Azure Active Directory (Azure AD)에는 기본 암호 개념이 없습니다. 전용 SQL 풀을 프로 비전 하는 경우 인증을 Azure AD와 통합 하도록 선택 하는 것이 좋습니다. 이 인증 방법을 사용 하는 경우 사용자는 사용자 계정 이름을 전송 하 고 서비스에서 Azure AD에 저장 된 자격 증명 정보를 사용 하도록 요청 합니다.
...
**지침**: 전용 관리 계정의 사용에 대 한 정책과 절차를 만듭니다. Azure Security Center Id 및 액세스 관리를 사용 하 여 Azure Active Directory (Azure AD)를 통해 로그인 하는 관리 계정 수를 모니터링 합니다.
...
- [감사 SQL Server 이해](/sql/relational-databases/security/auditing/sql-server-audit-database-engine)
...
**지침**: 전용 SQL 풀에서 Azure SQL Database 서버에 대 한 Azure Active Directory (Azure AD) 관리자를 만듭니다.
...
**지침**: Azure Active Directory (Azure AD)는 오래 된 계정을 검색 하는 데 도움이 되는 로그를 제공 합니다. 또한 Azure AD 액세스 검토를 사용 하 여 그룹 멤버 자격, 엔터프라이즈 응용 프로그램에 대 한 액세스 및 역할 할당을 효율적으로 관리할 수 있습니다. 사용자의 액세스를 정기적으로 검토 하 여 적절 한 사용자만 계속 액세스할 수 있도록 할 수 있습니다.

SQL 인증을 사용하는 경우 데이터베이스에서 포함된 데이터베이스 사용자를 만듭니다. 사용자 그룹에 적절 한 특정 사용 권한이 있는 사용자 지정 데이터베이스 역할에 하나 이상의 데이터베이스 사용자를 두어야 합니다.

- [액세스 검토를 사용 하는 방법](../active-directory/governance/access-reviews-overview.md)
...
**지침**: azure SQL을 사용 하 여 Azure Active Directory (azure ad) 인증을 구성 하 고 azure ad 사용자 계정에 대 한 진단 설정을 사용 하도록 설정 하 여 감사 로그 및 로그인 로그를 Log Analytics 작업 영역으로 보냅니다. Log Analytics 내에서 원하는 경고를 구성합니다.

SQL 인증을 사용하는 경우 데이터베이스에서 포함된 데이터베이스 사용자를 만듭니다. 사용자 그룹에 적절 한 특정 사용 권한이 있는 사용자 지정 데이터베이스 역할에 하나 이상의 데이터베이스 사용자를 두어야 합니다.

- [액세스 검토를 사용 하는 방법](../active-directory/governance/access-reviews-overview.md)

- [Azure SQL Database를 사용 하 여 Azure AD 인증을 구성 하 고 관리 하는 방법](../azure-sql/database/authentication-aad-configure.md)
...
**지침**: 사용자 id와 관련 된 검색 된 의심 스러운 작업에 대 한 자동화 된 응답을 구성 하려면 Azure AD (Azure Active Directory) id 보호 및 위험 검색 기능을 사용 합니다. 또한 추가 조사를 위해 온-보드 및 데이터를 Azure 센티널로 수집 합니다.
...
데이터 검색 &amp; 분류는 Azure SYNAPSE SQL에 기본 제공 됩니다. 데이터베이스에서 중요한 데이터의 검색, 분류, 레이블 지정 및 보고를 위한 고급 기능을 제공합니다.
...
- [Azure SQL Database 인스턴스에서 데이터 반출을 방지 하도록 개인 링크 및 nsgs를 구성 하는 방법](../azure-sql/database/private-endpoint-overview.md)

- [Azure SQL Database에 대 한 Advanced Threat Protection 이해](../azure-sql/database/threat-detection-overview.md)
...
- [Azure Synapse SQL에 대 한 동적 데이터 마스킹 이해](../azure-sql/database/dynamic-data-masking-overview.md)
...
### <a name="46-use-azure-rbac-access-control-to-control-access-to-resources"></a>4.6: Azure RBAC access control을 사용 하 여 리소스에 대 한 액세스 제어 
...
- [인증을 위해 Azure SQL Server와 Azure Active Directory (Azure AD)를 통합 하는 방법](../azure-sql/database/authentication-aad-overview.md)

- [Azure SQL Server에서 액세스를 제어 하는 방법](../azure-sql/database/logins-create-manage.md)
...
- [서비스 관리 투명 한 데이터 암호화 이해](../azure-sql/database/transparent-data-encryption-tde-overview.md)

- [고객 관리 투명 한 데이터 암호화 이해](https://docs.microsoft.com/azure/azure-sql/database/transparent-data-encryption-tde-overview#customer-managed-transparent-data-encryption---bring-your-own-key)

- [사용자 고유의 키를 사용 하 여 TDE를 설정 하는 방법](../azure-sql/database/transparent-data-encryption-byok-configure.md)
...
- [Azure SQL Synapse에 대 한 경고를 만드는 방법](../azure-sql/database/alerts-insights-configure-portal.md)
...
- [Azure SQL 데이터베이스에서 취약성 평가를 실행 하는 방법](../azure-sql/database/sql-vulnerability-assessment.md)
...
**지침**: Azure Key Vault에서 고객 관리 키를 사용 하는 투명한 데이터 암호화 (tde)는 Tde 보호기 라는 고객이 관리 하는 비대칭 키를 사용 하 여 자동으로 생성 된 Dek (데이터베이스 암호화 키)를 암호화할 수 있습니다. 일반적으로 투명한 데이터 암호화를 위한 BYOK(Bring Your Own Key) 지원이라고도 합니다. BYOK 시나리오에서 TDE 보호기는 고객이 소유 하 고 관리 하는 Azure Key Vault에 저장 됩니다. 또한 Azure Key Vault에서 일시 삭제를 사용 하도록 설정 해야 합니다.

- [Azure Key Vault에서 고객 관리 키를 사용 하 여 TDE를 사용 하도록 설정 하는 방법](../azure-sql/database/transparent-data-encryption-byok-configure.md)
...
- [Azure Key Vault 키를 백업 하는 방법](/powershell/module/az.keyvault/backup-azkeyvaultkey)
...
- [Azure Key Vault 키를 복원 하는 방법](/powershell/module/az.keyvault/restore-azkeyvaultkey)

- [전용 SQL 풀에서 백업 및 복원](sql-data-warehouse/backup-and-restore.md)

- [기존 전용 SQL 풀을 복원 하는 방법](sql-data-warehouse/sql-data-warehouse-restore-active-paused-dw.md)
...
- [Key Vault에서 일시 삭제를 사용 하도록 설정 하는 방법](../storage/blobs/soft-delete-blob-overview.md)
...
**지침**: Security Center는 경고에 대 한 심각도를 할당 하 여 각 경고에 참석 하는 순서의 우선 순위를 지정할 수 있도록 합니다. 따라서 리소스가 손상 되 면 즉시 가져올 수 있습니다. 심각도는 경고를 실행 하는 데 사용 되는 검색 또는 메트릭에 Security Center 하는 방법에 따라 달라 지 며, 경고를 발생 시킨 활동의 악의적인 의도를가지고 있는 신뢰도 수준을 기준으로 합니다.
