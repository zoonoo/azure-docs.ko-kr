...
더 나은 성능이 필요한 경우 SAS 자격 증명을 사용 하 여 저장소에 액세스를 시도 합니다.
...
### <a name="use-proper-collation-to-utilize-predicate-pushdown-for-character-columns"></a>적절 한 데이터 정렬을 사용 하 여 문자 열에 대 한 조건자 푸시 다운 활용

Parquet 파일의 데이터는 행 그룹으로 구성 됩니다. 서버를 사용 하지 않는 SQL 풀은 WHERE 절에서 지정 된 조건자를 기준으로 행 그룹을 건너뛰고 따라서 IO를 줄여 쿼리 성능을 향상 시킵니다. 

Parquet 파일에서 문자 열에 대 한 조건자 푸시 다운은 Latin1_General_100_BIN2_UTF8 데이터 정렬에만 지원 됩니다. WITH 절을 사용 하 여 특정 열의 데이터 정렬을 지정할 수 있습니다. WITH 절을 사용 하 여이 데이터 정렬을 지정 하지 않으면 데이터베이스 데이터 정렬이 사용 됩니다.
...
CETAS를 사용 하 여 조인 된 참조 테이블과 같은 자주 사용 되는 쿼리 부분을 새 파일 집합으로 구체화할 수 있습니다. 그러면 여러 쿼리에서 공통 조인을 반복하는 대신, 이 단일 외부 테이블에 조인할 수 있습니다.
...
일반적인 문제에 대한 해결 방법은 [문제 해결](resources-self-help-sql-on-demand.md) 문서를 검토하세요. 서버를 사용 하지 않는 SQL 풀 대신 전용 SQL 풀을 사용 하는 경우 특정 지침은 [전용 sql 풀에 대 한 모범 사례](best-practices-dedicated-sql-pool.md) 를 참조 하세요.
