...
SQL 스크립트 만들기</br>|Synapse 사용자 또는 </br>작업 영역의 Azure 소유자 또는 참가자 </br>*Sql 스크립트를 실행 하려면 추가 sql 권한이 필요* 합니다.|
...
전용 SQL 풀에서 SQL 스크립트 실행|풀에 대 한 SQL 권한이 필요 합니다.|없음
...
노트북 또는 작업 정의 만들기|작업 영역에서 사용자 또는 Azure 소유자, 참가자 또는 읽기 권한자 Synapse</br> *실행, 게시 또는 저장 하려면 추가 권한이 필요 합니다.*|읽기
...
Git 작업 분기에 대 한 노트북 또는 작업 정의 변경 내용 커밋|Git 권한|없음
**파이프라인, 통합 런타임, 데이터 흐름, 데이터 집합 및 트리거**||
...
파이프라인 만들기 |Synapse 사용자 </br>[**_고려 중 + Synapse Credential User On WorkspaceSystemIdentity_* _]</br>게시 또는 저장 하려면 _Additional 권한이 필요 합니다. *|읽기, 자격 증명/작업/작업
데이터 흐름, 데이터 집합 또는 트리거 만들기 |Synapse 사용자</br>*게시 또는 저장 하려면 추가 권한이 필요 합니다.*|읽기
...
지금 트리거를 포함 하 여 트리거 만들기|WorkspaceSystemIdentity의 Synapse User + Synapse Credential User|읽기, 자격 증명/작업/작업
...
새, 업데이트 또는 삭제 된 파이프라인, 데이터 흐름 또는 트리거를 서비스에 게시 합니다.|작업 영역에서 아티팩트 게시자 Synapse|파이프라인/쓰기, 삭제</br>데이터 흐름 write, delete</br>트리거/쓰기, 삭제
업데이트 되거나 삭제 된 새 데이터 흐름, 데이터 집합 또는 트리거를 서비스에 게시|작업 영역의 아티팩트 게시자|트리거/쓰기, 삭제
파이프라인, 데이터 흐름, 데이터 집합, 트리거에 대 한 변경 내용을 Git 리포지토리에 저장 (커밋) |Git 권한|없음 
**연결된 서비스**||
연결 된 서비스 만들기 (자격 증명 할당 포함)|Synapse 사용자</br>*실행, 게시 또는 저장 하려면 추가 권한이 필요 합니다.*|읽기
게시 된 연결 된 서비스 나열 및 열기|Synapse 아티팩트 사용자|Linkedservices.json 및 datasets.json/write, delete  
자격 증명으로 보호 되는 연결 된 서비스에 대 한 연결 테스트|Synapse User 및 Synapse Credential User|자격 증명/작업/작업|
연결 된 서비스 게시|Synapse 아티팩트 게시자|Linkedservices.json 및 datasets.json/write, delete
연결 된 서비스 정의를 Git 리포지토리에 저장 (커밋)|Git 권한|없음
**액세스 관리**||
...
코드 아티팩트에 대 한 Synapse RBAC 액세스를 만들거나 제거 합니다.|작업 영역 범위에서 관리자 Synapse|roleAssignments/write, delete   
