...
통계가 누락 된 경우 쿼리 최적화 프로그램은 쿼리 조건자 또는 조인 조건의 개별 열에 대 한 통계를 작성 하 여 쿼리 계획에 대 한 카디널리티 예상치를 향상 시킵니다.
...
통계의 자동 생성은 동기적으로 수행 되므로 열에 통계가 없는 경우 쿼리 성능이 약간 저하 될 수 있습니다. 단일 열에 대 한 통계를 만드는 시간은 테이블 크기에 따라 달라 집니다.
...
Table_name은 표시할 통계가 포함 된 테이블의 이름입니다. 이 테이블은 외부 테이블 일 수 없습니다. 대상은 통계 정보를 표시할 대상 인덱스, 통계 또는 열의 이름입니다.

## <a name="update-statistics"></a>통계 업데이트

모범 사례 중 하나는 새로운 날짜가 추가되는 날마다 날짜 열에 통계를 업데이트하는 것입니다. 새 행이 SQL 풀로 로드 될 때마다 새 로드 날짜나 트랜잭션 날짜가 추가 됩니다. 이러한 추가 기능은 데이터 배포를 변경 하 고 통계를 오래 된 것으로 만듭니다.
...
이 질문은 데이터의 연령에 의해 대답할 수 없는 것입니다. 기본 데이터에 중대한 변경이 없는 경우 최신 통계 개체가 오래되었을 수 있습니다.

> [!TIP]
> 행 수가 상당히 변경되었거나 열에 대한 값의 분포에 중대한 변경이 있는 경우 *통계를 업데이트해야 하는 시간*입니다.

통계를 마지막으로 업데이트 한 이후 테이블 내의 데이터가 변경 되었는지 여부를 확인할 수 있는 동적 관리 뷰는 없습니다. 통계 기간을 알면 그림의 일부를 제공할 수 있습니다.
...
예를 들어 SQL 풀의 **날짜 열** 에는 일반적으로 자주 통계 업데이트가 필요 합니다. 새 행이 SQL 풀로 로드 될 때마다 새 로드 날짜나 트랜잭션 날짜가 추가 됩니다. 이러한 추가 기능은 데이터 배포를 변경 하 고 통계를 오래 된 것으로 만듭니다.
...
UPDATE STATISTICS 문은 사용 하기 쉽습니다. 테이블에 대한 *모든* 통계를 업데이트하므로 필요한 것보다 더 많은 작업을 수행할 수 있습니다 성능이 문제가 아니면 통계가 최신 임을 보장 하는 가장 쉽고 완전 한 방법입니다.
...
> [!NOTE]
> SQL 풀은 통계 개체의 데이터를 사용 하 여 카디널리티 예상치를 계산 합니다.
...
- 통계 데이터의 특정 하위 집합에 대한 결과를 조인할 수 없습니다. 예를 들어 STAT_HEADER DENSITY_VECTOR 조인 합니다.
