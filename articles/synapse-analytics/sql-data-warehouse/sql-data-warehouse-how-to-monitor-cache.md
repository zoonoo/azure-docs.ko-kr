...
Azure Monitor를 사용하여 Gen2 캐시 메트릭을 보고 쿼리 성능 문제를 해결할 수 있습니다. 먼저 Azure 포털로 이동하여 **모니터,** **메트릭을** 클릭하고 **범위를 선택합니다.**

![Azure Monitor](./media/sql-data-warehouse-how-to-monitor-cache/cache-0.png)

검색 및 드롭다운 막대를 사용하여 데이터 웨어하우스를 찾습니다. 그런 다음 적용을 선택합니다.

![Azure Monitor](./media/sql-data-warehouse-how-to-monitor-cache/cache-1.png)

Gen2 캐시 문제 해결에 대한 키 메트릭은 **캐시 적중 비율** 및 **캐시 사용 비율**입니다. **캐시 적중률을** 선택한 다음 **메트릭 추가** 단추를 사용하여 캐시 사용 **비율을 추가합니다.** 
...
**시나리오 1:** 현재 캐시를 최적으로 사용하고 있습니다. 쿼리 속도가 느려질 수 있는 다른 영역을 [해결합니다.](sql-data-warehouse-manage-monitor.md)

**시나리오 2:** 현재 작업 중인 데이터 집합이 물리적 읽기로 인해 낮은 캐시 적중 비율을 초래할 수 있어 캐시에 적합하지 않습니다. 성능 수준의 규모를 확장하고 워크로드를 다시 실행하여 캐시를 채우는 것을 고려하세요.

**시나리오 3:** 캐시와 관련이 없는 이유로 인해 쿼리가 느리게 실행되는 것 같습니다. 쿼리 속도가 느려질 수 있는 다른 영역을 [해결합니다.](sql-data-warehouse-manage-monitor.md) 또한 캐시 크기를 줄여 비용을 절약하도록 [인스턴스 규모를 축소](sql-data-warehouse-manage-monitor.md)하는 것을 고려해 보세요. 

**시나리오 4:** 쿼리가 느려진 원인일 수 있는 콜드 캐시가 있습니다. 작업 데이터 세트가 캐시될 때 쿼리를 다시 실행해보세요. 

> [!IMPORTANT]
> 워크로드를 다시 실행한 후 캐시 적중 비율 또는 사용된 캐시 백분율이 업데이트되지 않으면 작업 집합이 이미 메모리에 있을 수 있습니다. 클러스터된 열저장소 테이블만 캐시됩니다.
