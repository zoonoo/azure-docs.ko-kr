---
title: 테이블 분할
description: SQL 분석에서 테이블 파티션을 사용하는 권장 사항 및 예
...
ms.locfileid: "80351242"
---
# <a name="partitioning-tables-in-sql-analytics"></a>SQL 분석에서 테이블 분할
SQL 분석에서 테이블 파티션을 사용하기 위한 권장 사항 및 예제입니다.

## <a name="what-are-table-partitions"></a>테이블 파티션이란?
테이블 파티션을 사용하면 데이터를 더 작은 데이터 그룹으로 나눌 수 있습니다. 대부분의 경우 테이블 파티션은 날짜 열에 만들어집니다. 분할은 모든 SQL Analytics 테이블 유형에서 지원됩니다. 클러스터된 열 저장소, 클러스터된 인덱스 및 힙을 포함합니다. 또한 해시 또는 라운드 로빈 배포를 비롯한 모든 배포 유형에서도 분할이 지원됩니다.  
...
SQL Analytics에서 분할의 주요 이점은 파티션 삭제, 전환 및 병합을 사용하여 데이터 로드의 효율성과 성능을 향상시키는 것입니다. 대부분의 경우 데이터는 데이터가 데이터베이스에 로드되는 순서에 밀접하게 관련된 날짜 열에서 분할됩니다. 파티션을 사용하여 데이터를 유지 관리할 때의 가장 큰 장점 중 하나는 트랜잭션 로깅이 방지된다는 것입니다. 단순히 데이터를 삽입, 업데이트 또는 삭제하는 것이 생각과 노력이 거의 필요하지 않은 가장 간단한 방법일 수 있지만, 로드 프로세스 중에 분할을 사용하면 성능을 크기 향상시킬 수 있습니다.
...
**클러스터형 columnstore** 테이블에서 파티션을 만들 때 각 파티션에 얼마나 많은 행 수를 둘지 고려하는 것은 중요합니다. 클러스터형 columnstore 테이블에 대한 최적의 압축 및 성능을 고려할 때, 배포 및 파티션당 최소 1백만 개의 행이 필요합니다. 파티션을 생성하기 전에 SQL Analytics는 이미 각 테이블을 60개의 분산 데이터베이스로 나눕니다. 백그라운드에서 생성된 배포 외에, 테이블에 분할이 추가됩니다. 이 예제를 사용하여 판매 팩트 테이블에 36개의 월별 파티션이 포함되어 있고 SQL Analytics 데이터베이스에 60개의 배포가 있는 경우 판매 팩트 테이블에는 매월 6천만 개의 행 또는 모든 월이 채워질 때 21억 개의 행이 포함되어야 합니다. 테이블이 파티션당 권장되는 최소 행 수보다 적은 행을 포함하면 파티션당 행 수를 늘리기 위해 더 적은 수의 파티션을 사용할 것을 고려해야 합니다. 자세한 내용은 [인덱싱](sql-data-warehouse-tables-index.md) 문서를 참조하세요. 여기에는 클러스터 columnstore 인덱스의 품질을 평가할 수 있는 쿼리가 포함되어 있습니다.

## <a name="syntax-differences-from-sql-server"></a>SQL Server와의 구문 차이
SQL Analytics는 SQL Server보다 간단한 파티션을 정의하는 방법을 소개합니다. 분할 함수 및 구성표는 SQL Server에서와 같이 SQL Analytics에서 사용되지 않습니다. 대신 분할된 열 및 경계 지점을 식별하기만 하면 됩니다. 분할의 구문은 SQL Server와 약간 다르지만 기본 개념은 동일합니다. SQL Server 및 SQL Analytics는 테이블당 하나의 파티션 열을 지원하며, 이 열은 파티션범위로 지정할 수 있습니다. 분할에 대한 자세한 내용은 [분할된 테이블 및 인덱스](/sql/relational-databases/partitions/partitioned-tables-and-indexes)를 참조하세요.
...
SQL Server 파티션 정의를 SQL 애널리틱스로 간단히 마이그레이션하려면 다음을 수행하십시오.
...
SQL Server 인스턴스에서 분할된 테이블을 마이그레이션하는 경우 다음 SQL이 각 파티션에 있는 행의 수를 파악하는 데 도움이 될 수 있습니다. SQL Analytics에서 동일한 분할 세분성을 사용하는 경우 파티션당 행 수가 60배 줄어듭니다.  
...
SQL Analytics는 파티션 분할, 병합 및 전환을 지원합니다. 이러한 각 함수는 [ALTER TABLE](/sql/t-sql/statements/alter-table-transact-sql) 문을 사용하여 실행됩니다.
...
### <a name="load-new-data-into-partitions-that-contain-data-in-one-step"></a>한 단계로 데이터가 포함된 파티션에 새 데이터 로드
파티션 전환을 사용하여 파티션으로 데이터를 로드하는 것은 사용자가 새 데이터의 스위치를 볼 수 없는 테이블에서 새 데이터를 단계별로 만드는 편리한 방법입니다.  파티션 전환과 관련된 잠금 경합을 처리하는 데 는 바쁜 시스템에서 어려울 수 있습니다.  파티션의 기존 데이터를 지우려면 `ALTER TABLE` 데이터를 전환하는 데 필요한 데이터입니다.  그런 `ALTER TABLE` 다음 새 데이터를 전환해야 했습니다.  SQL 분석에서는 `TRUNCATE_TARGET` `ALTER TABLE` 명령에서 옵션이 지원됩니다.  명령을 `TRUNCATE_TARGET` `ALTER TABLE` 사용하면 파티션의 기존 데이터를 새 데이터로 덮어씁니다.  다음은 기존 데이터로 `CTAS` 새 테이블을 만들고 새 데이터를 삽입한 다음 모든 데이터를 대상 테이블로 다시 전환하여 기존 데이터를 덮어쓰는 데 사용하는 예제입니다.
...
이 방법을 사용하면 소스 제어의 코드가 정적으로 유지되고 분할 경계 값이 동적일 수 있습니다. 시간이 지남에 따라 데이터베이스와 함께 진화하고 있습니다.
