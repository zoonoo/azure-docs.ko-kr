...
Azure Synapse Analytics에서 계산 리소스 전용 SQL 풀 (이전의 SQL DW) 관리에 대해 알아봅니다. 전용 SQL 풀을 일시 중지 하 여 비용을 절감 하거나 성능 요구에 맞게 전용 SQL 풀의 크기를 조정 합니다.

## <a name="what-is-compute-management"></a>컴퓨팅 관리란?

전용 SQL 풀의 아키텍처 (이전의 SQL DW)는 저장소와 계산을 분리 하 여 각각을 독립적으로 확장할 수 있도록 합니다. 이에 따라 데이터 스토리지와는 관계없이 성능 요구 사항에 맞게 컴퓨팅의 크기를 조정할 수 있습니다. 또한 컴퓨팅 리소스를 일시 중지했다가 다시 시작할 수도 있습니다. 이 아키텍처의 자연스러운 결과는 컴퓨팅 및 스토리지에 대한 [청구](https://azure.microsoft.com/pricing/details/sql-data-warehouse/)가 분리되어 있다는 것입니다. 잠시 동안 전용 SQL 풀 (이전의 SQL DW)을 사용할 필요가 없는 경우 계산을 일시 중지 하 여 계산 비용을 절감할 수 있습니다.

## <a name="scaling-compute"></a>컴퓨팅 크기 조정

전용 SQL 풀 (이전의 SQL DW)에 대 한 [데이터 웨어하우스 단위](what-is-a-data-warehouse-unit-dwu-cdwu.md) 설정을 조정 하 여 계산을 확장 하거나 축소할 수 있습니다. 데이터 웨어하우스 단위를 더 추가하면 로드 및 쿼리 성능이 선형적으로 증가할 수 있습니다.
...
크기 조정 작업을 수행 하기 위해 전용 SQL 풀 (이전의 SQL DW)은 먼저 들어오는 모든 쿼리를 종료 한 다음 트랜잭션을 롤백하여 일관 된 상태를 유지 합니다. 크기 조정은 트랜잭션 롤백이 완료된 후에만 수행됩니다. 크기 조정 작업의 경우 시스템은 계산 노드에서 저장소 계층을 분리 하 고, 계산 노드를 추가한 다음, 저장소 계층을 계산 계층에 다시 연결 합니다. 각 전용 SQL 풀 (이전의 SQL DW)은 계산 노드에 균등 하 게 배포 되는 60 배포로 저장 됩니다. 계산 노드를 더 추가 하면 계산 전력이 늘어납니다. 계산 노드 수가 늘어나면 계산 노드당 배포 수가 감소 하 여 쿼리에 더 많은 계산 능력을 제공 합니다. 마찬가지로, 데이터 웨어하우스 단위를 줄이면 계산 노드의 수가 줄어들어 쿼리에 대 한 계산 리소스를 줄일 수 있습니다.
...
확장, 특히 더 큰 데이터 웨어하우스 단위의 성능상 이점을 알아보기 위해 최소한 1TB의 데이터 집합을 사용하려고 합니다. 전용 SQL 풀 (이전의 SQL DW)에 대해 가장 적합 한 데이터 웨어하우스 단위 수를 찾으려면 확장 및 축소를 시도 합니다. 데이터를 로드한 후 서로 다른 데이터 웨어하우스 단위 수를 사용하여 몇 가지 쿼리를 실행합니다. 크기 조정이 빠르기 때문에 1시간 이내에 다양한 성능 수준을 시험해 볼 수 있습니다.

최적의 데이터 웨어하우스 단위 수를 찾는 데 권장되는 사항은 다음과 같습니다.

- 개발 중인 전용 SQL 풀 (이전의 SQL DW)의 경우 더 적은 수의 데이터 웨어하우스 단위를 선택 하 여 시작 합니다.  좋은 출발점은 DW400c 또는 DW200c입니다.
...
컴퓨팅을 일시 중지하면 스토리지 계층이 컴퓨팅 노드에서 분리됩니다. 계산 리소스는 계정에서 해제 됩니다. 컴퓨팅이 일시 중지된 동안에는 컴퓨팅 비용이 청구되지 않습니다. 컴퓨팅을 다시 시작하면 스토리지를 컴퓨팅 노드에 다시 연결하여 컴퓨팅 비용에 대한 청구를 다시 시작합니다.
전용 SQL 풀을 일시 중지 하는 경우 (이전의 SQL DW):
...
전용 SQL 풀을 다시 시작 하는 경우 (이전의 SQL DW):

- 전용 SQL 풀 (이전의 SQL DW)은 데이터 웨어하우스 단위 설정에 대해 계산 및 메모리 리소스를 가져옵니다.
- 데이터 웨어하우스 단위에 대한 컴퓨팅 비용 청구가 다시 시작됩니다.
- 데이터를 사용할 수 있게 됩니다.
- 전용 SQL 풀 (이전의 SQL DW)이 온라인 상태 이면 작업 쿼리를 다시 시작 해야 합니다.

항상 전용 SQL 풀 (이전의 SQL DW)에 액세스할 수 있도록 하려면 일시 중지 하는 대신 가장 작은 크기로 확장 하는 것이 좋습니다.
...
각각의 확장, 일시 중지 및 다시 시작 작업을 완료하는 데 몇 분이 걸릴 수 있습니다. 자동으로 크기 조정, 일시 중지 또는 다시 시작하는 경우, 다른 작업을 진행하기 전에 특정 작업이 완료되었는지 확인하는 논리를 구현하는 것이 좋습니다. 다양 한 끝점을 통해 전용 SQL 풀 (이전의 SQL DW) 상태를 확인 하면 이러한 작업의 자동화를 올바르게 구현할 수 있습니다.
