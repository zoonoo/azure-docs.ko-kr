...
- **팩트 테이블에는** 트랜잭션 시스템에서 일반적으로 생성된 다음 데이터 웨어하우스에 로드되는 정량적 데이터가 포함되어 있습니다. 예를 들어 소매점에서 판매 트랜잭션을 매일 생성한 다음 분석을 위해 데이터 웨어하우스의 팩트 테이블에 데이터를 로드합니다.
...
스키마는 비슷한 방식으로 사용되는 테이블을 함께 그룹화하는 좋은 방법입니다.  온프레미 솔루션에서 SQL 데이터 웨어하우스로 여러 데이터베이스를 마이그레이션하는 경우 모든 팩트, 차원 및 통합 테이블을 SQL 데이터 웨어하우스의 하나의 스키마로 마이그레이션하는 것이 가장 좋습니다. 예를 들어 모든 테이블을 [WideWorldImportersDW](/sql/sample/world-wide-importers/database-catalog-wwi-olap) 샘플 데이터 웨어하우스에 wwi라는 하나의 스키마 내에 저장할 수 있습니다. 다음 코드는 wwi라는 [사용자 정의 스키마를](/sql/t-sql/statements/create-schema-transact-sql) 만듭니다.
...
임시 테이블은 세션 기간 동안만 유지됩니다. 임시 테이블을 사용하여 다른 사용자가 임시 결과를 볼 수 없도록 하고 정리의 필요성을 줄일 수 있습니다.  임시 테이블은 로컬 저장소를 사용하여 빠른 성능을 제공합니다.  자세한 내용은 [임시 테이블](sql-data-warehouse-tables-temporary.md)을 참조하세요.
...
SQL 데이터 웨어하우스의 기본 기능은 [배포간에](massively-parallel-processing-mpp-architecture.md#distributions)테이블을 저장하고 작동할 수 있는 방법입니다.  SQL Data Warehouse는 데이터 배포방법( 기본값), 해시 및 복제를 위한 세 가지 방법을 지원합니다.
...
기본적으로 SQL Data Warehouse는 테이블을 클러스터형 columnstore 인덱스로 저장합니다. 이러한 형태의 데이터 스토리지는 대형 테이블에서 데이터 압축률과 쿼리 성능이 높습니다.  일반적으로 클러스터형 columnstore 인덱스가 가장 좋은 옵션이지만 클러스터형 인덱스 또는 힙이 적절한 스토리지 구조인 경우도 있습니다.  힙 테이블은 최종 테이블로 변환되는 스테이징 테이블과 같은 일시적인 데이터를 로드하는 데 특히 유용할 수 있습니다.
...
쿼리 최적화 프로그램은 쿼리 실행 계획을 만들 때 열 수준 통계를 사용합니다. 쿼리 성능을 향상하려면 개별 열, 특히 쿼리 조인에 사용되는 열에 대한 통계를 사용하는 것이 중요합니다. [통계 만들기는](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-tables-statistics#automatic-creation-of-statistic) 자동으로 수행됩니다.  그러나 통계 업데이트는 자동으로 수행되지 않습니다. 많은 행을 추가하거나 변경한 후에는 통계를 업데이트합니다. 예를 들어 로드 후 통계를 업데이트합니다. 자세한 내용은 [통계 가이드](sql-data-warehouse-tables-statistics.md)를 참조하세요.

## <a name="primary-key-and-unique-key"></a>기본 키 및 고유 키
기본 키는 비CLUSTERED 및 강제 적용되지 않는 둘 다 사용되는 경우에만 지원됩니다.  고유 제약 조건은 NOT ENFORCED가 사용되지 않는 경우에만 지원됩니다.  [SQL 데이터 웨어하우스 테이블 제약 조건을 확인합니다.](sql-data-warehouse-table-constraints.md)
...
| [외부 테이블 만들기](/sql/t-sql/statements/create-external-table-transact-sql) | 외부 테이블을 만듭니다. 테이블 정의는 SQL Data Warehouse에 저장됩니다. 테이블 데이터는 Azure Blob Storage 또는 Azure Data Lake Store에 저장됩니다. |
...
- 외래 키, [테이블 제약 조건](/sql/t-sql/statements/alter-table-table-constraint-transact-sql) 확인
- [계산된 열](/sql/t-sql/statements/alter-table-computed-column-definition-transact-sql)
- [인덱싱된 보기](/sql/relational-databases/views/create-indexed-views)
