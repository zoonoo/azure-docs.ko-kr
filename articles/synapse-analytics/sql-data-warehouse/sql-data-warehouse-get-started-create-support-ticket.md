...
1. **세부 정보** 창에서 **세부 정보 제공**을 선택하여 추가 정보를 입력합니다.
...
**세부 정보 제공**을 클릭하면 정보를 추가할 수 있는 **할당량 세부 정보** 창이 표시됩니다. 다음 섹션에서는 Azure Synapse Analytics에 사용할 수 있는 다양한 할당량 요청에 대해 설명합니다.

### <a name="data-warehouse-units-dwus-per-server"></a>서버당 DWU(데이터 웨어하우스 단위)

서버당 DWU의 증가를 요청하려면 다음 단계를 사용합니다.

1. **서버당 DTU(데이터 웨어하우스 단위)** 할당량 유형을 선택합니다.

1. **리소스** 목록에서 대상으로 할 리소스를 선택합니다.

1. **할당량 요청** 필드에 요청하는 새 DWU 제한을 입력합니다.
...
구독에 특정 지역의 액세스가 필요한 경우 **다른 할당량 요청** 옵션을 사용하여 액세스를 요청합니다. 요청에서 지역에 대해 사용하도록 설정할 제품 및 SKU 세부 정보를 지정합니다. 제품 및 SKU 옵션을 탐색하려면 [Azure Synapse Analytics 가격 책정](https://azure.microsoft.com/pricing/details/synapse-analytics/)을 참조하세요.
...
마지막 단계는 SQL Database 지원 요청에 대한 나머지 세부 정보를 입력하는 것입니다. 그런 다음, **Next: 검토 + 만들기>>** 를 선택하고 요청 세부 정보를 검토한 후 **만들기**를 클릭하여 요청을 제출합니다.
...
또한 [Stack Overflow](https://stackoverflow.com/questions/tagged/azure-synapse+or+azure-sql-data-warehouse) 또는 [Azure SQL Data Warehouse에 대한 Microsoft Q&A 질문 페이지](https://docs.microsoft.com/answers/topics/azure-synapse-analytics.html)를 통해 Azure Synapse Analytics 커뮤니티에 연결할 수도 있습니다.
