...
- 6천만 개 미만의 행이 있는 작은 테이블입니다. 힙 테이블을 고려합니다.
...
클러스터형 인덱스는 단일 행을 빠르게 검색해야 하는 경우 클러스터형 columnstore 테이블보다 더 나은 성능을 제공할 수 있습니다. 단일 또는 극한 속도로 수행하려면 행 조회가 거의 필요한 쿼리의 경우 클러스터 인덱스 또는 클러스터되지 않은 보조 인덱스를 고려하십시오. 클러스터형 인덱스를 사용할 때의 단점은 클러스터형 인덱스 열에 고도의 선택 필터를 사용하는 쿼리에만 도움이 된다는 것입니다. 다른 열에 대한 필터를 향상시키기 위해 비클러스터형 인덱스를 다른 열에 추가할 수 있습니다. 그러나 테이블에 각 인덱스가 추가될 때마다 차지하는 공간이 늘어나고 로드 처리 시간도 늘어납니다.
...
테이블에 클러스터되지 않은 인덱스를 추가하려면 다음 구문을 사용합니다.
...
| [CLOSED_rowgroup_count] |닫힌 행 그룹이 있더라도 그 수가 적어야 합니다. 닫힌 행 그룹은 ALTER INDEX를 사용하여 압축 된 행 그룹으로 변환 할 수 있습니다 ... 명령을 재구성합니다. 그러나 일반적으로는 이 작업이 필요하지 않습니다. 닫힌 그룹은 백그라운드 "튜플 이동기" 프로세스를 통해 자동으로 columnstore 행 그룹으로 변환됩니다. |
...
파티션 정렬 배포당 102,400개 행의 대량 임계값을 초과하는 일괄 처리된 업데이트 및 삽입 작업은 columnstore 형식으로 직접 기록됩니다. 그러나 균일한 배포를 가정하여 한 작업에서 614만 4천 개 이상의 행을 수정해야 할 수 있습니다. 지정된 파티션 정렬 분포의 행 수가 102,400개 미만이면 행이 델타 저장소로 이동하여 행 그룹을 닫기 위해 충분한 행이 삽입되거나 수정되거나 인덱스가 다시 작성될 때까지 계속 유지됩니다.
