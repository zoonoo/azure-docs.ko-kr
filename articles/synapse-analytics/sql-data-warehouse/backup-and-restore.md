...
지리적 백업은 쌍을 이루는 데이터 [센터에](../../best-practices-availability-paired-regions.md?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json)하루에 한 번 만들어집니다. 지역 복원의 RPO는 24시간입니다. SQL 풀이 지원되는 다른 지역의 서버에 지리적 백업을 복원할 수 있습니다. 지역 백업을 사용하면 주 지역의 복원 지점에 액세스할 수 없는 경우에 데이터 웨어하우스를 복원할 수 있습니다.
...
복원된 데이터 웨어하우스와 현재 데이터 웨어하우스 중 하나를 유지하거나 삭제할 수 있습니다. 현재 데이터 웨어하우스를 복원된 데이터 웨어하우스로 바꾸려면 ALTER [데이터베이스(SQL POOL)를](/sql/t-sql/statements/alter-database-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) 사용하여 이름 수정 옵션을 사용하여 이름을 바꿀 수 있습니다.
...
구독 간에 직접 복원해야 하는 경우 [여기에서](https://feedback.azure.com/forums/307516-sql-data-warehouse/suggestions/36256231-enable-support-for-cross-subscription-restore)이 기능에 투표하십시오. 다른 논리 서버로 복원하고 구독 간에 [서버를 '이동'하여](/azure/azure-resource-manager/resource-group-move-resources?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json) 교차 구독 복원을 수행합니다.
