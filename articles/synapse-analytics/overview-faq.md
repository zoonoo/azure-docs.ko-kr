...
A: Azure Synapse는 BI, AI 및 지속적인 인텔리전스를 위한 통합 데이터 플랫폼입니다. 다음과 같은 통합 방법을 제공하는 단일 플랫폼을 통해 다양한 분석 런타임(예: SQL 및 Apache Spark)을 연결합니다.
...
- 엔진 전체에서 메타데이터를 관리합니다. 서버리스 Apache Spark 풀 테이블을 만들면 Azure Synapse 데이터베이스에서 자동으로 사용할 수 있습니다.
...
- 분석 기능은 전용 SQL 풀 또는 서버리스 SQL 풀(미리 보기)을 통해 제공됩니다.
- Scala, Python, Spark, SQL 및 C#을 완벽하게 지원하는 서버리스 Apache Spark 풀(미리 보기)
...
A: Azure Synapse Analytics는 Azure SQL Data Warehouse를 데이터 웨어하우스 솔루션으로 전용 SQL 풀을 포함하는 분석 플랫폼으로 발전시킨 것입니다. 이 플랫폼은 데이터 검색, 수집, 변환, 준비 및 서비스 분석 계층을 결합합니다.
...
### <a name="q-what-is-a-good-use-case-for-dedicated-sql-pool"></a>Q: 전용 SQL 풀에 적합한 사용 사례는 무엇인가요?

A: 전용 SQL 풀은 데이터 웨어하우스 요구 사항의 핵심입니다. [가격/성능](https://azure.microsoft.com/services/sql-data-warehouse/compare/) 측면에서 선도적인 데이터 웨어하우스 솔루션입니다. 전용 SQL 풀은 다음을 수행할 수 있기 때문에 업계 최고의 클라우드 데이터 웨어하우스 솔루션입니다.
...
### <a name="q-what-is-a-good-use-case-for-serverless-apache-spark-pool-in-azure-synapse"></a>Q: Azure Synapse에서 서버리스 Apache Spark 풀에 적합한 사용 사례는 무엇인가요?
...
또 다른 Apache Spark 사용 사례는 데이터 과학자가 다음을 수행하는 것입니다.

- 기능 추출
- 데이터 검색
- 모델 학습

### <a name="q-what-is-a-good-use-case-for-serverless-sql-pool-in-azure-synapse"></a>Q: Azure Synapse에서 서버리스 SQL 풀에 적합한 사용 사례는 무엇인가요?

A: 서버리스 SQL 풀은 데이터 레이크의 데이터에 대한 쿼리 서비스입니다. 데이터를 특수화된 저장소에 복사하거나 로드할 필요 없이 데이터를 적절히 쿼리할 수 있는 친숙한 T-SQL 구문을 제공하여 모든 데이터에 대한 액세스를 민주화할 수 있습니다.
...
### <a name="q-what-is-a-good-use-case-for-data-flow-in-azure-synapse"></a>Q: Azure Synapse의 데이터 흐름에 적합한 사용 사례는 무엇인가요?
...
A: Synapse Analytics에서 엔드투엔드 Single Sign-On 환경은 중요한 인증 프로세스입니다. 완전한 Azure AD 통합을 통해 ID를 관리하고 전달해야 합니다.
...
A: 예, Tableau 및 Power BI와 같은 타사 비즈니스 애플리케이션을 사용하여 전용 SQL 풀 및 서버리스 SQL 풀에 연결할 수 있습니다. Spark는 IntelliJ를 지원합니다.
