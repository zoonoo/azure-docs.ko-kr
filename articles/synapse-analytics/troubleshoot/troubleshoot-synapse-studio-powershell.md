...
또한이 세션의 진단 로그 파일은 문제 해결 스크립트와 동일한 폴더에 생성 됩니다. 해당 위치는 "일반 팁" 섹션 (`D:\TestAzureSynapse_2020....log`)에 표시 됩니다. 필요한 경우 기술 지원 서비스에이 파일을 보낼 수 있습니다.
...
* "통과"로 표시 된 모든 테스트 항목 (요청)은 HTTP 상태 코드에 관계 없이 연결 테스트를 통과 했음을 의미 합니다.
 실패 한 요청의 경우 이유는 또는 `NamedResolutionFailure` `ConnectFailure`와 같이 노란색으로 표시 됩니다. 이러한 이유는 네트워크 환경에 잘못 된 구성이 있는지 파악 하는 데 도움이 될 수 있습니다.
