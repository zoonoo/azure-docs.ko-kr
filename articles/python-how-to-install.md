...
-   **Azure용 PowerShell(Windows에만 해당)**. 클라우드 서비스 및 가상 컴퓨터와 같은 Azure 서비스를 배포 및 관리하기 위한 PowerShell cmdlet 집합입니다.
...
-   **Windows**: 클라우드 서비스 -- 예: Webrole을 사용하는 Django 사이트
...
이 릴리스에서는 **CPython**만이 테스트를 거쳤으며 지원됩니다. 또한 버전 2.7 이상을 사용하는 것이 좋습니다. 머지않아 **IronPython** 지원도 추가될 예정입니다.
...
-   www.python.org에서 직접
-   www.enthought.com 또는 www.ActiveState.com과 같은 신뢰할 수 있는 배포자
-   원본에서 빌드
...
Windows의 경우 기본 Python 개발자 센터에서 제공되는 [WebPI 설치 프로그램](http://go.microsoft.com/fwlink/?LinkId=254281&clcid=0x409)을 사용하여 설치를 간소화할 수 있습니다(www.python.org에서 CPython 사용).
...
WebPI 설치 프로그램은 Python Azure 앱에 필요한 모든 것뿐만 아니라 Django 앱을 위한 특정 지원도 제공합니다.
...
이 릴리스는 Django를 사용한 웹 앱에 주로 초점을 두지만, [PyPI(Python 패키지 인덱스)](http://pypi.python.org/pypi)(영문)를 찾아보고 다양한 다른 소프트웨어를 선택할 수도 있습니다. Distro를 설치하도록 선택했다면 웹 개발 및 기술 컴퓨팅을 포함한 다양한 시나리오에 맞는 거의 모든 흥미로운 기능을 이미 갖추었습니다.

**site-packages** 에서 어느 Python 패키지가 설치되었는지 확인하려면 다음을 입력하여 위치를 찾으십시오.

![how-to-install-python-win-site](./media/python-how-to-install/how-to-install-python-win-site.png)

시스템에 설치된 항목의 목록이 나타납니다.

설치한 후에는 기본 위치에서 Python, Django, 클라이언트 라이브러리를 사용할 수 있습니다.
...
Python Tools for Visual Studio는 VS를 완전한 Python IDE로 전환하는 Microsoft의 무료/OSS 플러그 인입니다.

![how-to-install-python-ptvs](./media/python-how-to-install/how-to-install-python-ptvs.png)

Python Tools for Visual Studio는 선택 사항이지만 Python 및 Django 프로젝트/솔루션 지원, 디버깅, 프로파일링, 템플릿 편집과 Intellisense, 클라우드에 배포 등을 제공하므로 권장됩니다. 이 추가 기능은 기존 VS2010 설치와 호환됩니다. VS2010이 없는 경우 WebPI는 기본적으로 **완전히 무료**로 "VS Python Express" IDE를 제공하는 무료 통합형 셸 + PTVS를 설치합니다. 자세한 내용은 [CodePlex의 Python Tools for Visual Studio](http://pytools.codeplex.com)(영문)를 참조하십시오.

참고: PTVS 플러그 인은 크기가 작지만 통합형 셸로 인해 다운로드 시간이 길어집니다. 통합형 셸 버전은 현재 "Azure 배포 프로젝트 추가" 기능을 지원하지 않습니다.

Windows 제거
------------

**Python용 Azure SDK 2012년 6월** WebPI 제품은 일반적인 의미의 응용 프로그램이 아니며 실제로 32비트 Python 2.7, 함께 묶어서 제공되는 Python 및 Django용 Azure 클라이언트 API 등과 같은 개별 제품의 컬렉션입니다. 이에 따라, 전통적인 개념의 고유 제거 프로그램이 없으므로 설치된 프로그램을 Windows 제어판에서 개별적으로 제거해야 합니다.
...
여기서 Azure 자습서 및 Django 샘플을 실행하는 데 적합한 CPython 2.7.2가 이 Azure SUSE VM에 설치되어 있는 것을 확인할 수 있습니다. 업그레이드해야 하는 경우 사용 중인 OS의 권장 패키지 업그레이드 지침을 따르십시오. 하지만 일반적으로 시스템 Python을 그대로 두고(다른 사람에게 해당 버전이 필요할 수 있음) [Virtualenv](http://pypi.python.org/pypi/virtualenv)(영문)를 통해 최신 버전을 설치하는 것이 더 좋습니다.
...
![MacOS 사이트 패키지](./media/python-how-to-install/how-to-install-python-mac-site.png)

mac/linux에서 개발할 때 이 릴리스에 대해 다음 두 가지 기본 시나리오가 지원됩니다.
...
VM 시나리오의 경우, 원하는 Linux VM(Ubuntu, CentOS, SUSE)을 시작한 후 원하는 대로 실행하고 관리하면 됩니다. 예를 들어 Windows/Mac/Linux 컴퓨터에서 [IPython](http://ipython.org) REPL/Notebook을 실행하고 Azure에서 IPython Engine을 실행하는 Linux 또는 Windows 다중 프로세싱 VM으로 브라우저를 가리키면 됩니다. IPython 설치에 대한 자세한 내용은 해당 자습서를 참조하십시오.

Linux VM을 설정하는 방법에 대한 자세한 내용은 [Linux 관리](/en-us/manage/linux/) 섹션을 참조하십시오.
