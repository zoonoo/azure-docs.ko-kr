...
Windows의 경우 기본 Python 개발자 센터에서 제공되는 [WebPI 설치 프로그램][WebPI 설치 프로그램]을 사용하여 설치를 간소화할 수 있습니다(www.python.org 에서 CPython 사용).
...
이 릴리스는 웹 앱에 주로 초점을 두지만, [PyPI(Python 패키지 인덱스)][PyPI(Python 패키지 인덱스)]를 찾아보고 다양한 다른 소프트웨어를 선택할 수도 있습니다. Distro를 설치하도록 선택했다면 웹 개발 및 기술 컴퓨팅을 포함한 다양한 시나리오에 맞는 거의 모든 흥미로운 기능을 이미 갖추었습니다.
...
또한 PTVS를 사용하면 [클라우드 서비스 및 웹 사이트 배포][클라우드 서비스 및 웹 사이트 배포] 지원과 함께 Microsoft Azure에 쉽게 배포할 수 있습니다.

PTVS는 기존 Visual Studio 2010, 2012 또는 2013 설치와 함께 작동합니다. 자세한 내용 및 다운로드는 [CodePlex의 Python Tools for Visual Studio][CodePlex의 Python Tools for Visual Studio]를 참조하세요.
...
VM 시나리오의 경우, 원하는 Linux VM(Ubuntu, CentOS, SUSE)을 시작한 후 원하는 대로 실행하고 관리하면 됩니다. 예를 들어 Windows/Mac/Linux 컴퓨터에서 [IPython][IPython] REPL/Notebook을 실행하고 Azure에서 IPython Engine을 실행하는 Linux 또는 Windows 다중 프로세싱 VM으로 브라우저를 가리키면 됩니다. IPython 설치에 대한 자세한 내용은 해당 자습서를 참조하십시오.

Linux VM을 설정하는 방법에 대한 자세한 내용은 [Linux 관리][Linux 관리] 섹션을 참조하십시오.

## 추가 소프트웨어 및 리소스

-   [Enthought Python 분산(영문)][Enthought Python 분산(영문)]
-   [ActiveState Python 분산(영문)][ActiveState Python 분산(영문)]
-   [SciPy - Scientific Python 라이브러리 제품군(영문)][SciPy - Scientific Python 라이브러리 제품군(영문)]
-   [NumPy - Python의 숫자 라이브러리(영문)][NumPy - Python의 숫자 라이브러리(영문)]
-   [Django 프로젝트 - 완성도 높은 웹 프레임워크/CMS(영문)][Django 프로젝트 - 완성도 높은 웹 프레임워크/CMS(영문)]
