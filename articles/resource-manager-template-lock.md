...
| 이름 | 형식 | 필수 | 허용된 값 | 설명 |
| ------- | ---- | ---------------- | -------- | ----------- |
| level | enum | 예 | **CannotDelete** <br /> **ReadOnly** | 범위에 적용되는 잠금 유형입니다. CanNotDelete를 사용하면 수정은 할 수 있지만 삭제는 할 수 없으며 ReadOnly는 수정 또는 삭제할 수 없습니다. |
...
관리 잠금을 만들거나 삭제하려면 **Microsoft.Authorization/*** 또는 **Microsoft.Authorization/locks/*** 작업에 대한 액세스 권한이 있어야 합니다. 기본 제공 역할의 경우 **소유자** 및 **사용자 액세스 관리자**에게만 이러한 작업의 권한이 부여됩니다. 역할 기반 액세스 제어에 대한 자세한 내용은 [리소스 액세스 관리](./azure-portal/resource-group-rbac.md)를 참조하세요.
...
다음 예제에서는 리소스 그룹에 읽기 전용 잠금을 적용합니다.
