...
1.  Visual Studio에서 Azure에 아직 로그인하지 않았으면 **서버 탐색기**에서 **Connect to Azure** 단추를 클릭합니다.
    ...
    Visual Studio에서 Azure 리소스에 연결하는 방법에 대한 자세한 내용은 [계정, 구독 및 관리 역할 관리][계정, 구독 및 관리 역할 관리]를 참조하십시오.

2.  **서버 탐색기**에서 **Azure**를 확장한 후 **웹 사이트**를 확장합니다.

3.  [Azure 및 ASP.NET 시작][Azure 및 ASP.NET 시작]에서 만든 웹 사이트의 노드를 마우스 오른쪽 단추로 클릭한 후 **설정 보기**를 클릭합니다.

    ![서버 탐색기에서 설정 보기][서버 탐색기에서 설정 보기]

    **Azure 웹 사이트** 탭이 나타나면 이 탭에서 Visual Studio에서 사용할 수 있는 사이트 관리 및 구성 작업이 무엇인지 확인할 수 있습니다.

    ![Azure 웹 사이트 창][Azure 웹 사이트 창]

    이 자습서에서는 로깅 및 추적 드롭다운을 사용합니다. 원격 디버깅도 사용하지만 이를 사용하도록 설정하는 방법은 다른 방법을 사용하게 될 것입니다.

    이 창에 있는 응용 프로그램 설정 및 연결 문자열 상자에 대한 자세한 내용은 [Azure 웹 사이트: 응용 프로그램 설정 및 연결 문자열 작동 방식][Azure 웹 사이트: 응용 프로그램 설정 및 연결 문자열 작동 방식](영문)을 참조하십시오.

    이 창에서 지원하지 않는 사이트 관리 작업을 수행하려는 경우 **Full Website Settings**를 클릭하여 브라우저 창에서 관리 포털을 열 수 있습니다. 자세한 내용은 [웹 사이트를 구성하는 방법][웹 사이트를 구성하는 방법]을 참조하십시오.

## <a name="remoteview"></a>원격 뷰
...
**'/' 응용 프로그램 서버 오류 메시지:**
![도움이 되지 않는 오류 메시지][도움이 되지 않는 오류 메시지]

**'오류가 발생했습니다' 메시지:**
![도움이 되지 않는 오류 메시지][1]

**'웹 사이트에서 페이지를 표시할 수 없습니다' 메시지**
![도움이 되지 않는 오류 메시지][2]

오류의 원인을 찾는 가장 쉬운 방법은 주로 자세한 오류 메시지를 사용하도록 설정하는 것입니다. 이전 스크린샷 중 첫 번째 스크린샷에 그 방법이 설명되어 있습니다. 자세한 오류 메시지를 사용하도록 설정하려면 배포된 Web.config 파일의 내용을 변경해야 합니다. 프로젝트의 *Web.config* 파일을 편집한 후 프로젝트를 다시 배포하거나, [Web.config 변환][Web.config 변환](영문)을 만들어 디버그 빌드를 배포할 수 있습니다. 하지만 가장 빠른 방법은 **솔루션 탐색기**에서 *원격 뷰* 기능을 사용하여 원격 사이트에 있는 파일을 바로 보고 편집하는 것입니다.

1.  **서버 탐색기**에서 **Azure**를 확장한 후 **웹 사이트**를 확장하고 프로젝트를 배포할 웹 사이트의 노드를 확장합니다.

    표시되는 노드를 통해 웹 사이트의 콘텐츠 파일 및 로그 파일에 액세스할 수 있습니다.

    ![파일 및 로그 파일][파일 및 로그 파일]

2.  **파일** 노드를 확장하고 *Web.config* 파일을 두 번 클릭합니다.

    ![Web.config 열기][Web.config 열기]
...
이 섹션에서는 [Azure 및 ASP.NET 시작][Azure 및 ASP.NET 시작]에서 만든 프로젝트를 사용하여 원격으로 디버그하는 방법을 보여 줍니다.

1.  [Azure 및 ASP.NET 시작][Azure 및 ASP.NET 시작]에서 만든 웹 프로젝트를 엽니다.
...
4.  `ViewBag.Message` 줄에 [중단점을 설정][디버그 모드에서 실행]합니다.

5.  **솔루션 탐색기**에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **게시**를 클릭합니다.

6.  [Azure 및 ASP.NET 시작][Azure 및 ASP.NET 시작]에서 사용한 프로필과 동일한 프로필을 **프로필** 드롭다운 목록에서 선택합니다.

7.  **설정** 탭을 클릭하고 **구성**을 **디버그**로 변경한 후 **게시**를 클릭합니다.

    ![디버그 모드에서 게시][디버그 모드에서 게시]
...
10. Visual Studio 2012 Update 4의 경우:<span id="vs2012"></span></a>
    ...
    ![관리 포털에서 원격 디버깅 설정][관리 포털에서 원격 디버깅 설정]
...
12. `currentTime` 변수를 마우스 포인터로 가리켜 시간 값을 봅니다.
    ...
    ![Azure에서 실행 중인 디버그 모드의 변수 보기][Azure에서 실행 중인 디버그 모드의 변수 보기]
...
14. F5 키를 눌러 계속 실행합니다.
    ...
    ![새 값이 표시된 정보 페이지][새 값이 표시된 정보 페이지]
...
-   디버그하는 동안 서버는 데이터를 Visual Studio로 보내며, 이로 인해 대역폭 사용 요금에 영향을 줄 수 있습니다. 대역폭 요금에 대한 자세한 내용은 [Azure 가격 책정][Azure 가격 책정]을 참조하십시오.
...
3.  Web.config 파일의 `<system.web>` 아래에 [trace 요소][trace 요소]를 다음 예와 같이 추가합니다.

        <trace enabled="true" writeToDiagnosticsTrace="true" mostRecent="true" pageOutput="false" />

4.  Ctrl+F5를 눌러 응용 프로그램을 실행합니다.

5.  브라우저 창의 주소 표시줄에서 *trace.axd*를 URL에 추가한 후 Enter 키를 누릅니다. URL은 <http://localhost:53370/trace.axd>와 유사해집니다.

6.  **응용 프로그램 추적** 페이지에서 **자세히 보기**를 클릭합니다.
...
## <a name="webserverlogs"></a><span class="short-header">웹 서버 로그</span>웹 서버 로그 보기
...
4.  웹 사이트가 표시되는 브라우저 창에서 **홈**을 클릭하고 **정보**를 클릭한 후 **연락처**를 클릭합니다.
    ...
    ![출력 창의 웹 서버 로그][출력 창의 웹 서버 로그]

기본적으로 Visual Studio를 사용하여 웹 서버 로그를 사용하도록 처음 설정하는 경우 Azure는 로그를 파일 시스템에 기록합니다. 또는, 관리 포털을 사용하여 웹 서버 로그가 저장소 계정의 Blob 컨테이너에 기록되도록 지정할 수 있습니다. 자세한 내용은 [웹 사이트를 구성하는 방법][웹 사이트를 구성하는 방법]의 **사이트 진단** 섹션을 참조하십시오.
...
## <a name="detailederrorlogs"></a><span class="short-header">오류 로그</span>자세한 오류 메시지 로그 보기
...
1.  **서버 탐색기**에서 열었던 **Azure Web Site Configuration** 탭에서 **자세한 오류 메시지**를 **켜기**로 변경한 후 **저장**을 클릭합니다.
    ...
    ![자세한 오류 메시지 사용][자세한 오류 메시지 사용]

2.  **출력** 창에서 **Specify which Azure logs to monitor** 단추를 클릭합니다.

3.  **Azure 로깅 옵션** 대화 상자에서 **모든 로그**를 클릭한 후 **확인**을 클릭합니다.

    ![모든 로그 모니터링][모든 로그 모니터링]
...
4.  404 오류를 유발하도록 브라우저 창의 주소 표시줄에서 추가 문자를 URL에 추가(예: `http://localhost:53370/Home/Contactx`)한 후 Enter 키를 누릅니다.
    ...
    ![출력 창의 자세한 오류 로그][출력 창의 자세한 오류 로그]

    Ctrl 키를 누른 채 링크를 클릭하면 브라우저에 서식이 지정된 로그 출력이 표시됩니다.

    ![브라우저 창의 자세한 오류 로그][브라우저 창의 자세한 오류 로그]

## <a name="downloadlogs"></a><span class="short-header">로그 다운로드</span>파일 시스템 로그 다운로드
...
2.  *.zip* 파일의 압축을 풀면 다음 폴더 구조가 표시됩니다.
    ...
    -   웹 서버 로그는 *LogFiles\\http\\RawLogs* 폴더에 *.log* 파일로 기록됩니다. [Log Parser][Log Parser] 같은 도구를 사용하여 이들 파일을 보고 조작할 수 있습니다.
...
## <a name="storagelogs"></a><span class="short-header">저장소 로그</span>저장소 로그 보기
...
테이블을 사용하면 로그 세부 정보를 온라인으로 쉽게 볼 수 있으며 스트리밍도 지원됩니다. 테이블에서 로그를 쿼리할 수 있고 새 로그가 생성될 때 새 로그를 확인할 수도 있습니다. Blob을 사용하면 로그를 파일로 쉽게 다운로드할 수 있고, Blob 저장소와의 연동 방식이 인식되는 HDInsight를 사용하여 로그를 분석할 수도 있습니다. 자세한 내용은 [데이터 저장소 옵션(Azure에서 실제 클라우드 앱 빌드)][데이터 저장소 옵션(Azure에서 실제 클라우드 앱 빌드)](영문)에서 **Hadoop 및 MapReduce**를 참조하십시오.
...
7.  **Manage Table Storage**를 클릭합니다.
    ...
    ![Manage Table Storage 클릭][Manage Table Storage 클릭]
    ...
    ![Manage table storage][Manage table storage]
...
12. **View all application logs**를 클릭합니다.
    ...
    ![서버 탐색기의 추적 테이블][서버 탐색기의 추적 테이블]

    ![테이블 보기의 저장소 로그][테이블 보기의 저장소 로그]

    이 보기에는 다른 보기에서 볼 수 없는 추가 필드가 표시됩니다. 이 보기에서는 또한 쿼리를 작성하는 특수 쿼리 작성기 UI를 사용하여 로그를 필터링할 수 있습니다. 자세한 내용은 [서버 탐색기를 사용하여 저장소 리소스 탐색][서버 탐색기를 사용하여 저장소 리소스 탐색](영문)에서 "테이블 리소스 사용 - 엔터티 필터링"을 참조하십시오.

13. 단일 행에 대한 세부 정보를 보려면 행 중 하나를 마우스 오른쪽 단추로 클릭한 후 **편집**을 클릭합니다.

    ![서버 탐색기의 추적 테이블][8]

## <a name="failedrequestlogs"></a><span class="short-header">실패한 요청 로그</span>실패한 요청 추적 로그 보기
...
4.  관리 포털에서 **대시보드**를 클릭한 후 **간략 상태** 섹션의 **Reset your deployment credentials**를 클릭합니다.
    ...
    ![대시보드에서 FTP 자격 증명 링크 다시 설정][대시보드에서 FTP 자격 증명 링크 다시 설정]

5.  새 사용자 이름 및 암호를 입력합니다.

    ![새 FTP 사용자 이름 및 암호][새 FTP 사용자 이름 및 암호]
...
9.  *LogFiles* 폴더를 엽니다.
    ...
    ![LogFiles 폴더 열기][LogFiles 폴더 열기]

10. 이름에 W3SVC와 숫자 값이 있는 폴더를 엽니다.

    ![W3SVC 폴더 열기][W3SVC 폴더 열기]
...
11. 추적 정보를 확인하려는 실패한 요청의 XML 파일을 클릭합니다.
    ...
    ![브라우저의 실패한 요청 추적][브라우저의 실패한 요청 추적]
...
-   [Azure 문제 해결][Azure 문제 해결]
     WAWS 관련 단문 섹션을 포함하여 기초 정보를 소개하는 백서입니다.
-   [웹 사이트 문제 해결][웹 사이트 문제 해결]
     WAWS를 중점적으로 소개합니다.
-   [Azure 웹 사이트에 진단 로그 사용][Azure 웹 사이트에 진단 로그 사용]
     이 자습서에서 제공하는 정보와 거의 동일한 정보를 다루지만 Visual Studio를 사용하지 않고 진단 로그를 확인하는 방법을 중점적으로 설명합니다.
-   [웹 사이트를 모니터링하는 방법][웹 사이트를 모니터링하는 방법]
     [진단 구성 및 로그 다운로드][진단 구성 및 로그 다운로드] 섹션에서는 문제 해결 문서에 수록되지 않은 유용한 정보를 제공합니다.
-   [Visual Studio 2013을 사용하여 Azure 웹 사이트에서 메모리 누수 검사][Visual Studio 2013을 사용하여 Azure 웹 사이트에서 메모리 누수 검사]. 관리되는 메모리 문제를 분석하는 데 사용되는 Visual Studio 기능에 대한 Microsoft ALM 블로그 게시물입니다.
-   [알아 두면 도움이 되는 Windows Azure 웹 사이트 온라인 도구][알아 두면 도움이 되는 Windows Azure 웹 사이트 온라인 도구]. Amit Apple의 블로그 게시물입니다.

특정 문제 해결 질문과 관련하여 도움이 필요한 경우 다음 포럼 중 하나에서 게시물을 작성하십시오.

-   [ASP.NET 사이트의 Azure 포럼][ASP.NET 사이트의 Azure 포럼](영문)
-   [MSDN의 Azure 포럼][MSDN의 Azure 포럼](영문)
-   [StackOverflow.com][StackOverflow.com](영문)

### Visual Studio의 디버깅

Visual Studio에서 디버그 모드를 사용하는 방법에 대한 자세한 내용은 MSDN 토픽 [Visual Studio의 디버깅][Visual Studio의 디버깅]과 [Visual Studio 2010을 사용한 디버깅 팁][Visual Studio 2010을 사용한 디버깅 팁](영문)을 참조하십시오.

Azure 웹 사이트의 원격 디버깅에 대한 자세한 내용은 다음 리소스를 참조하십시오.

-   [Azure 웹 사이트의 원격 디버깅 소개][Azure 웹 사이트의 원격 디버깅 소개]
-   [Azure 웹 사이트의 원격 디버깅 소개 2부 - 원격 디버깅 세부 정보][Azure 웹 사이트의 원격 디버깅 소개 2부 - 원격 디버깅 세부 정보]
-   [Azure 웹 사이트의 원격 디버깅 소개 3부 - 다중 인스턴스 환경 및 GIT][Azure 웹 사이트의 원격 디버깅 소개 3부 - 다중 인스턴스 환경 및 GIT]

웹 사이트에 Azure 웹 API 또는 모바일 서비스 백 엔드가 사용되는 경우 이들을 디버그해야 하면 [Visual Studio에서 .NET 백 엔드 디버깅][Visual Studio에서 .NET 백 엔드 디버깅](영문)을 참조하십시오.
...
-   [모니터링 및 원격 분석(Azure에서 실제 클라우드 앱 빌드)][모니터링 및 원격 분석(Azure에서 실제 클라우드 앱 빌드)](영문)
