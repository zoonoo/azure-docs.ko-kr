...
위의 표에서 "Direct"는 가상 WAN에서 Azure 방화벽을 통과 하는 트래픽이 없는 두 연결 간의 직접 연결을 나타내며 "AzFW"는 흐름이 Azure 방화벽을 통과 함을 나타냅니다. 행렬에는 서로 다른 두 가지 연결 패턴이 있으므로 다음과 같이 구성 될 두 개의 경로 테이블이 필요 합니다.
