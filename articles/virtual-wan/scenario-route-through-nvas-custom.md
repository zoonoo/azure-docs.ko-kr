...
* 가상 허브에 연결 된 가상 네트워크에 대 한 "스포크" ( **그림 1**의 vnet 1, vnet 2 및 vnet 3).
* 사용자가 NVA ( **그림 1**의 VNet 4)를 배포한 가상 네트워크에 대 한 "서비스 VNet"을 사용 하 여 인터넷이 아닌 트래픽을 검사 하 고 스포크에서 액세스 하는 일반 서비스를 사용할 수 있습니다.
...
| From          | 아래와 같이 변경합니다.|*스포크*|*서비스 VNet*|*분기*|*인터넷*|
|---|---|:---:|:---:|:---:|:---:|:---:|
| **스포크**| &#8594;| 직접 |직접 | 서비스 VNet을 통해 |DMZ VNet을 통해 |
| **서비스 VNet**| &#8594;| 직접 |해당 없음| 직접 | |
| **분기** | &#8594;| 서비스 VNet을 통해 |직접| 직접 |  |

연결 매트릭스의 각 셀은 연결이 가상 WAN을 통해 직접 이동 하는지 또는 NVA를 사용 하는 Vnet 중 하나를 통해 전달 되는지를 설명 합니다. 여러 행에 대해 자세히 살펴보겠습니다.
...
Microsoft의 연결 매트릭스는 세 개의 경로 테이블로 변환 하는 세 가지 고유한 연결 패턴을 제공 합니다. 다른 Vnet에 대 한 연결은 다음과 같습니다.
