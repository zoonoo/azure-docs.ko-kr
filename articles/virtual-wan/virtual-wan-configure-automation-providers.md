---
title: Azure Virtual WAN 파트너 자동화 가이드라인 | Microsoft Docs
description: Azure Virtual WAN에 온-프레미스 VPN, SD-WAN CPE, 또는 분기 디바이스를 연결하고 구성하도록 자동화 환경을 설정합니다.
...
* Azure Virtual WAN에 대한 연결을 자동화하는 데 사용하는 [REST API](#additional)를 봅니다.
...
* Virtual Hub 만들기를 자동화하는 [REST API](/rest/api/virtualwan/virtualhubs)
* Virtual WAN용 Azure VPN Gateway를 자동화하는 [REST API](/rest/api/virtualwan/vpngateways)
* VPNSite를 Azure VPN Hub에 연결하는 [REST API](/rest/api/virtualwan/vpnconnections)
* [기본 IPsec 정책](#default)

## <a name="customer-experience"></a><a name ="ae"></a>고객 환경
...
Azure Virtual WAN과 함께 예상되는 고객 환경을 이해합니다.
  ...
  7. CPE 관리 인터페이스에서 문제 해결 및 모니터링 기능을 제공하는 것이 좋습니다. 일반적인 시나리오에는 "고객이 CPE 문제로 인해 Azure 리소스에 액세스할 수 없습니다", "CPE 쪽에 IPsec 매개 변수 표시" 등이 있습니다.
...
분기(온-프레미스 사이트) 정보를 Azure에 업로드하도록 사용자 환경을 디자인해야 합니다. VPNSite의 [REST API](/rest/api/virtualwan/vpnsites)를 사용하여 Virtual WAN에서 사이트 정보를 작성할 수 있습니다. 모든 분기 SDWAN/VPN 디바이스를 제공하거나 적절하게 디바이스 사용자 지정을 선택할 수 있습니다.

### <a name="device-configuration-download-and-connectivity"></a><a name="device"></a>디바이스 구성 다운로드 및 연결

이 단계는 Azure 구성 다운로드 및 분기 디바이스에서 Azure Virtual WAN으로 연결 설정을 포함합니다. 이 단계에서는 공급자를 사용하지 않는 고객은 수동으로 Azure 구성을 다운로드하고 온-프레미스 SDWAN/VPN 디바이스에 적용합니다. 공급자는 이 단계를 자동화해야 합니다. 추가 정보는 다운로드 [REST API](/rest/api/virtualwan/vpnsitesconfiguration/download)를 확인하세요. 디바이스 컨트롤러는 'GetVpnConfiguration' REST API를 호출하여 Azure 구성을 다운로드할 수 있습니다.
...
## <a name="device-configuration-file"></a><a name="devicefile"></a>디바이스 구성 파일
