...
* 저장소 대상에 대해 설정 하는 가상 네임 스페이스 경로 (캐시 네임 스페이스 페이지에 나열 됨)
...
1. 3 단계의 **탑재 명령** 필드는 이전 필드에서 설정한 탑재 주소, 가상 네임 스페이스 경로 및 클라이언트 경로를 사용 하는 사용자 지정 된 탑재 명령으로 자동으로 채워집니다.
   ...
   !["클립보드로 복사" 단추에 대 한 호버 텍스트를 표시 하는 프로토타입 탑재 명령 필드의 스크린샷](media/mount-command-copy.png)
...
**탑재 지침** 페이지를 사용 하지 않고 탑재 명령을 만들려면 캐시 **개요** 페이지의 탑재 주소와 **네임 스페이스** 페이지의 가상 네임 스페이스 경로를 찾을 수 있습니다.
...
가상 네임 스페이스 경로는 캐시의 **네임 스페이스** 설정 페이지에 표시 됩니다.

![테이블의 첫 번째 열 주위의 강조 상자를 사용 하 여 네임 스페이스 페이지 > 포털 설정의 스크린샷: "Namespace path"](media/view-namespace-paths.png)
