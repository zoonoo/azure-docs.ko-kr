...
캐시를 만든 후에는 NFS 클라이언트에서 간단한 `mount` 명령을 사용 하 여 해당 캐시에 액세스할 수 있습니다. 명령은 Azure HPC 캐시의 특정 저장소 대상 경로를 클라이언트 컴퓨터의 로컬 디렉터리에 연결 합니다.
...
클라이언트에서 명령을 사용 하기 전에 클라이언트가 필수 구성 요소를 충족 하는지, 위에서 설명한 대로 `mount` [클라이언트 준비](#prepare-clients)에서 NFS 명령을 사용 하는 데 필요한 소프트웨어가 있는지 확인 합니다.
...
이 명령이 성공 하면 저장소 내보내기의 내용이 클라이언트의 ``hpccache`` 디렉터리에 표시 됩니다.
...
> [!NOTE]
> 캐시 탑재 주소는 캐시의 서브넷 내에 있는 네트워크 인터페이스에 해당 합니다. 리소스 그룹에서 이러한 Nic는로 끝나는 이름 `-cluster-nic-` 및 숫자로 나열 됩니다. 이러한 인터페이스를 변경 하거나 삭제 하지 마세요. 캐시를 사용할 수 없게 됩니다.
