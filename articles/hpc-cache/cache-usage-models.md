...
* **쓰기 저장 지연** -쓰기 캐싱이 설정 된 캐시의 경우 다시 쓰기 지연 시간은 파일을 백 엔드 저장소 시스템으로 복사 하기 전에 캐시가 추가 파일 변경 내용을 대기 하는 시간입니다.
...
* **쓰기 수 15% 이상** -이 옵션을 선택 하면 읽기 및 쓰기 성능이 향상 됩니다. 이 옵션을 사용 하는 경우 모든 클라이언트는 백 엔드 저장소를 직접 탑재 하는 대신 Azure HPC 캐시를 통해 파일에 액세스 해야 합니다. 캐시 된 파일은 아직 백 엔드에 복사 되지 않은 최근 변경 내용을 포함 합니다.
...
* **클라이언트에서 NFS 대상에 기록 하 고 캐시를 무시** 합니다. 워크플로의 클라이언트에서 먼저 캐시에 쓰지 않고 저장소 시스템에 직접 데이터를 기록 하거나 데이터 일관성을 최적화 하려는 경우이 옵션을 선택 합니다. 클라이언트에서 요청 하는 파일이 캐시 (읽기) 되지만 클라이언트에서 해당 파일에 대 한 변경 내용 (쓰기)은 캐시 되지 않습니다. 백 엔드 저장소 시스템에 직접 전달 됩니다.
  ...
  이 사용 모델에서 캐시의 파일은 업데이트의 백 엔드 버전 (30 초 마다)에 대해 자주 확인 됩니다. 이 확인을 통해 데이터 일관성을 유지 하면서 캐시 외부에서 파일을 변경할 수 있습니다.

  > [!TIP]
  > 처음 세 가지 기본 사용 모델을 사용 하 여 대부분의 Azure HPC 캐시 워크플로를 처리할 수 있습니다. 다음 옵션은 보다 일반적인 시나리오에 적합 합니다.
