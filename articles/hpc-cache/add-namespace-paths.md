...
이 캐시에 대해 정의 된 모든 클라이언트 지향 경로는 **네임 스페이스** 페이지에 나열 됩니다. 네임 스페이스 경로가 정의 되지 않은 저장소 대상은 아직 테이블에 표시 되지 않습니다.

테이블 열을 정렬 하 여 캐시의 집계 된 네임 스페이스를 보다 잘 이해할 수 있습니다. 열 머리글의 화살표를 클릭 하 여 경로를 정렬 합니다.

[![테이블에 두 개의 경로가 있는 포털 네임 스페이스 페이지의 스크린샷 열 헤더: 네임 스페이스 경로, 저장소 대상, 내보내기 경로, 내보내기 하위 디렉터리 및 클라이언트 액세스 정책입니다. 첫 번째 열에 있는 경로 이름은 클릭할 수 있는 링크입니다. 위쪽 단추: 네임 스페이스 경로 추가, 새로 고침, 삭제 ](media/namespace-page.png)](media/namespace-page.png#lightbox)
...
* **새 경로를 추가 합니다.** 위쪽에서 **+ 추가** 단추를 클릭 하 고 편집 패널에 정보를 입력 합니다.
  ...
  ![Blob storage 대상이 있는 네임 스페이스 추가 편집 필드의 스크린샷 선택 내보내기 및 하위 디렉터리 경로는/로 설정 되 고 편집할 수 없습니다.](media/namespace-add-blob.png)
  ...
  * 드롭다운 목록에서 저장소 대상을 선택 합니다. Blob 저장소 대상에 이미 네임 스페이스 경로가 있는 경우이를 선택할 수 없습니다.
...
* **클라이언트 액세스 정책** -이 경로에 사용할 액세스 정책을 선택 합니다. 클라이언트 액세스 [정책 사용](access-policies.md)에서 클라이언트 액세스를 사용자 지정 하는 방법에 대해 자세히 알아보세요.
...
![편집 페이지가 오른쪽에 열려 있는 포털 네임 스페이스 페이지의 스크린샷 편집 양식에서 nfs 저장소 대상 네임 스페이스 경로에 대 한 설정을 표시 합니다.](media/namespace-edit-nfs.png)
