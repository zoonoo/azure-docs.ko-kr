<properties 
	pageTitle="트래픽 관리자를 사용하는 Azure 웹 사이트에 대한 Moniker 도메인 이름 구성" 
	description="Moniker에 등록된 도메인 이름을 사용하도록 트래픽 관리자를 사용하는 Azure 웹 사이트를 구성하는 방법에 대해 알아봅니다." 
	services="web-sites" 
	documentationCenter="" 
	authors="blackmist" 
	manager="wpickett" 
...
5. 그런 다음 **Record Type**을 먼저 선택하여 DNS 레코드를 만듭니다. 그런 다음 **Hostname** 및 **Address**를 입력합니다. 
    ...
    * CNAME 레코드를 추가할 때 **Hostname** 필드를, 사용할 하위 도메인으로 설정해야 합니다. 예를 들면 **www**로 설정해야 합니다. **Address** 필드는 Azure 웹 사이트에 사용 중인 트래픽 관리자 프로필의 **.trafficmanager.net** 도메인 이름으로 설정해야 합니다. 예를 들면 **contoso.trafficmanager.net**와 같습니다.
