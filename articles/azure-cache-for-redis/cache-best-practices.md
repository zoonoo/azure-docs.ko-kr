...
* **파이프라인을 사용 합니다.**  네트워크를 가장 효율적으로 사용 하 여 최적의 처리량을 얻기 위해 [Redis 파이프라인](https://redis.io/topics/pipelining) 을 지 원하는 Redis 클라이언트를 선택 합니다.
 ...
 * **일부 캐시 크기** 는 4 개 이상의 코어가 있는 vm에서 호스팅됩니다. 이는 tls 암호화/암호 해독 및 TLS 연결/연결 끊기 작업을 여러 코어에 분산 하 여 캐시 Vm에서 전체 CPU 사용량을 줄일 때 유용 합니다.  [VM 크기 및 코어에 대 한 자세한 내용은 여기를 참조 하세요.](cache-planning-faq.md#azure-cache-for-redis-performance)
