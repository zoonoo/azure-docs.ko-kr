...
기본적으로 Azure Monitor의 캐시 메트릭은 [30일 동안 저장](../azure-monitor/platform/data-collection.md#metrics)된 후 삭제됩니다. 캐시 메트릭을 30일보다 더 오래 유지하려면 [저장소 계정을 지정](../azure-monitor/platform/archive-diagnostic-logs.md)하고 캐시 메트릭에 대한 **보존(일)** 정책을 지정할 수 있습니다. 
...
>[!NOTE]
>캐시 메트릭을 저장소에 보관하는 것 외에 [이벤트 허브에 파일을 스트리밍하거나 Log Analytics로 전송](../azure-monitor/platform/rest-api-walkthrough.md#retrieve-metric-values)할 수도 있습니다.
>
>
...
> [!NOTE]
> 활성 클라이언트 응용 프로그램이 연결되어 있지 않아서 캐시가 유휴 상태인 경우에도 연결된 클라이언트, 메모리 사용, 수행 중인 작업 등 일부 캐시 활동이 나타날 수 있습니다. Azure Cache for Redis 인스턴스 작업 중에는 이러한 활동이 일반적으로 나타납니다.
> 
> 
