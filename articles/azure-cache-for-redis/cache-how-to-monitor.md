...
각 메트릭은 두 가지 버전을 포함합니다. 하나의 메트릭은 전체 캐시 및 [클러스터링](cache-how-to-premium-clustering.md)을 사용하는 캐시에 대한 성능을 측정합니다. 이름에 `(Shard 0-9)`를 포함하는 메트릭의 차기 버전은 캐시에서 단일 분할에 대한 성능을 측정합니다. 예를 들어 캐시에 4 개의 분할가 `Cache Hits` 있는 경우은 전체 캐시에 대 한 총 적중 수이 `Cache Hits (Shard 3)` 고는 캐시의 해당 분할 된 항목에 대 한 적중만 발생 합니다.
...
| 연결된 클라이언트 |지정한 보고 간격 동안 캐시에 설정된 클라이언트 연결 수입니다. 이 숫자는 Redis `connected_clients` INFO 명령에서에 매핑됩니다. [연결 제한](cache-configure.md#default-redis-server-configuration) 에 도달 하면 캐시에 대 한 후속 연결 시도가 실패 합니다. 활성 클라이언트 응용 프로그램이 없는 경우에도 내부 프로세스 및 연결 때문에 연결 된 클라이언트 인스턴스가 여전히 몇 개 있을 수 있습니다. |
...
| 제거된 키 |지정한 보고 간격 동안 `maxmemory` 제한 때문에 캐시에서 제거된 항목의 수입니다. 이 숫자는 Redis `evicted_keys` INFO 명령에서에 매핑됩니다. |
...
| 전체 키  | 이전 보고 기간 동안 캐시에 있는 최대 키 수입니다. 이 숫자는 Redis `keyspace` INFO 명령에서에 매핑됩니다. 기본 메트릭 시스템의 제한으로 인해 클러스터링이 사용되도록 설정된 캐시에서 전체 키는 보고 간격 동안 최대 키 수를 가진 분할된 데이터베이스의 최대 키 수를 반환합니다.  |
| 총 작업 |지정한 보고 간격 동안 캐시 서버에서 처리한 총 명령 수입니다. 이 값은 Redis INFO 명령에서 `total_commands_processed` 에 매핑됩니다. Redis 용 Azure Cache가 pub/sub에만 사용 되는 경우 `Cache Hits`, `Cache Misses` `Gets`, 또는 `Sets`에 대 한 메트릭은 없지만 pub/sub 작업의 `Total Operations` 캐시 사용을 반영 하는 메트릭이 있습니다. |
