...
캐시가 더 이상 애플리케이션 요구 사항을 충족시키지 못한다고 판단되면 애플리케이션에 적합하도록 더 크거나 더 작은 캐시 가격 책정 계층으로 규모를 변경할 수 있습니다. 사용할 캐시 가격 책정 계층을 결정하는 방법에 대한 자세한 내용은 [올바른 계층 선택](cache-overview.md#choosing-the-right-tier)을 참조하세요.
...
* **표준** 캐시가 더 큰 크기나 계층으로 확장되거나, **프리미엄** 캐시가 더 크게 확장되는 경우에는 일반적으로 모든 데이터가 유지됩니다. 표준 또는 프리미엄 캐시를 더 작게 스케일 다운하는 경우, 새로 조정된 크기 대비 캐시에 있는 데이터의 양에 따라 데이터가 손실될 수 있습니다. 크기를 축소하는 경우 데이터가 손실되면 [allkeys-lru](https://redis.io/topics/lru-cache) 제거 정책을 사용하여 키를 제거합니다.
...
* 작업을 다른 크기로 확장하는 동안 **기본** 캐시는 오프라인 상태입니다. **기본** 에서 **표준** 으로 확장하는 동안 기본 캐시를 그대로 사용할 수 있지만 작은 연결 블립이 발생할 수도 있습니다. 연결 블립이 발생하는 경우 redis 클라이언트가 일반적으로 해당 연결을 즉시 다시 설정할 수 있습니다.
...
* **기본** 캐시에서 바로 **프리미엄** 캐시로 확장할 수 없습니다. 먼저 크기 조정 작업을 통해 **기본** 에서 **표준** 으로 확장한 다음, 이후 작업을 통해 **표준** 에서 **프리미엄** 으로 확장합니다.
...
스케일링 시간은 캐시에 있는 데이터의 양에 따라 달라지며 데이터의 양이 많을수록 완료하는 데 시간이 더 오래 걸립니다. 스케일링에는 약 20분이 걸립니다. 클러스터형 캐시의 경우 스케일링은 분할당 약 20분이 걸립니다.
