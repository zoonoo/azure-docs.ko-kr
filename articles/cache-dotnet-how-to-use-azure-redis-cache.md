<properties pageTitle="Azure Redis 캐시 사용 방법" metaKeywords="" description="Learn how to create a use a cache in Azure Redis Cache" metaCanonical="" services="cache" documentationCenter="API Management" title="How to Use Azure Redis Cache" authors="sdanie" solutions="" manager="dwrede" editor="" />
...
이 가이드에서는 
**Azure Redis 캐시** 사용을 시작하는 방법을 보여 줍니다. 샘플은 C\# 코드로 작성되고
.NET API를 사용합니다. 적용되는 시나리오에는 **캐시 만들기 및 구성**, **캐시 클라이언트 구성**, **캐시에서 개체 추가 및 제거**, **캐시에 ASP.NET 세션 상태 저장** 등이 포함됩니다. 테이블에
Azure Redis 캐시 사용에 대한 자세한 내용은[다음 단계][] 섹션을 참조하세요.
...
Microsoft Azure Redis 캐시는 많이 사용되는 오픈 소스 Redis 캐시를 기반으로 합니다. Microsoft에서 관리하는 안전한 전용 Redis 캐시에 액세스할 수 있게 합니다. Azure Redis 캐시를 사용하여 만들어진 캐시는 Microsoft Azure 내의 모든 응용 프로그램에서 액세스할 수 있습니다.

Microsoft Azure Redis 캐시는 두 개의 계층으로 사용할 수 있습니다.

-	**기본** - 단일 노드. 최대 53GB까지 여러 개의 크기
-	**표준** - 2노드 마스터/슬레이브. 최대 53 GB까지 여러 개의 크기 99.9% SLA
...
캐시를 만들려면 먼저 Azure 관리 미리 보기 포털에 로그인하고 **새로 만들기**, **Redis 캐시**를 클릭합니다.

![New cache][NewCacheMenu]

**새 Redis 캐시** 블레이드에서 원하는 캐시 구성을 지정합니다.
...
>[AZURE.NOTE] 자세한 내용은 [StackExchange.Redis][] github 페이지 및 [StackExchange.Redis 캐시 클라이언트 설명서][]를 참조하세요.
...
>[AZURE.NOTE] 경고: 소스 코드에 자격 증명을 저장해서는 안 됩니다. 이 샘플을 단순하게 유지하기 위해 소스 코드로 표시합니다. [Windows Azure 웹 사이트: 응용 프로그램 설정 및 연결 문자열 작동 방식][]에서 자격 증명 저장 방법에 대해 자세히 알아보세요.
...
>[AZURE.NOTE] 고급 연결 구성 옵션에 대한 자세한 내용은 [StackExchange.Redis 구성 모델][]을 참조하세요.
...
>[AZURE.NOTE] Azure Redis 캐시는 .NET 개체 및 기본 데이터 형식을 캐시할 수 있습니다. 하지만 .NET 개체를 캐시하려면 먼저 직렬화해야 합니다. 이 과정은 응용 프로그램 개발자가 수행하며 이때 개발자는 유연하게 직렬 변환기를 선택할 수 있습니다. 자세한 내용은 [캐시의 .NET 개체 작업][]을 참조하세요.
...
Azure Redis 캐시는 메모리 내 또는 SQL Server 데이터베이스가 아니라 캐시에 세션 상태를 저장하는 데 사용할 수 있는 세션 상태 공급자를 제공합니다. 캐싱 세션
상태 공급자를 사용하려면 먼저 캐시를 구성하고 Redis 캐시 세션 상태 NuGet 패키지를 사용하여 캐시용 ASP.NET 응용 프로그램을 구성합니다.
...
[Azure Redis 세션 상태 공급자를 구성하고 사용하는 방법에 대한 자세한 내용은 ][]를 참조하세요.

<a name="next-steps"></a>
## 다음 단계

지금까지 Azure Redis 캐시의 기본 사항을 살펴보았으므로
다음 링크를 따라 좀 더 복잡한 캐싱 작업을 수행하는 방법에 알아보세요.

-	StackExchange.Redis 클라이언트에 대해 자세히 알아보기: [StackExchange.Redis 캐시 클라이언트 설명서(영문)][]
-	[redis][] 설명서를 참조하고 [redis 데이터 형식][] 및 [Redis 데이터 형식에 대한 15분 소개][]에 대해 읽어 보세요.
-   다음 MSDN 참조를 확인하세요. [Azure Redis 캐시(영문)][]
...
[어떤 유형의 캐싱은 내게 적절한가요?]: #choosing-cache
...
[캐시의 .NET 개체 사용]: http://msdn.microsoft.com/ko-kr/library/dn690521.aspx#Objects
...
[Azure Redis 캐시의 개요]: http://go.microsoft.com/fwlink/?LinkId=320830
[Azure Redis 캐시(영문)]: http://go.microsoft.com/fwlink/?LinkId=398247

[Azure Redis 캐시로 마이그레이션]: http://go.microsoft.com/fwlink/?LinkId=317347
[Azure Redis 캐시 샘플]: http://go.microsoft.com/fwlink/?LinkId=320840
[리소스 그룹을 사용하여 Azure 리소스 사용]: http://azure.microsoft.com/ko-kr/documentation/articles/azure-preview-portal-using-resource-groups/
...
[Redis 데이터 형식에 대한 15분짜리 소개]: http://redis.io/topics/data-types-intro
