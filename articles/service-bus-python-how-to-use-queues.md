...
이 가이드에서는 서비스 버스 큐를 사용하는 방법을 보여 줍니다. 샘플은 Python으로 작성되었으며 Python Azure 모듈을 사용합니다. 여기서 다루는 시나리오에는 **큐 만들기, 메시지 보내기 및 받기**, **큐 삭제** 등이 포함됩니다. 큐에 대한 자세한 내용은 [다음 단계](#next-steps) 섹션을 참조하십시오.
...
읽은 메시지는 큐에서 삭제됩니다. 그러나 선택적 매개 변수 **peek\_lock**을 **True**로 설정하여 큐에서 삭제되지 않도록 메시지를 읽은(peek) 후 잠글 수 있습니다.
