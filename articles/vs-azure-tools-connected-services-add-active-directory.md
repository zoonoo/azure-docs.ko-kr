<properties 
   pageTitle="Visual Studio에서 연결된 서비스를 사용하여 Azure Active Directory 추가"
...
1. **마침** 버튼을 선택합니다. 대화 상자는 Azure AD 인증을 위한 프로젝트를 사용 가능하도록 필요한 구성 코드 및 참조를 추가합니다. Azure 포털에서 AD 도메인을 확인할 수 있습니다.

    ![Azure 관리 포털에서 도메인 찾기](./media/vs-azure-tools-connected-services-add-active-directory/IC765882.png)

1. 다음 단계에 대한 아이디어로 브라우저에 표시되는 시작 페이지 및 변경된 내용 페이지를 검토하여 프로젝트를 수정하는 방법을 참조합니다. 모든 항목이 제대로 작동하는지 확인하려면, 수정된 구성 파일 중 하나를 열고 변경된 내용에 업근된 설정이 있는지 확인합니다. 예를 들어, ASP.NET MVC 프로젝트의 기본 web.config에 다음 설정이 추가됩니다.
