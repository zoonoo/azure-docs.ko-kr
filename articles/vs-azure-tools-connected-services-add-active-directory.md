...
### <a name="connect-to-azure-ad-using-the-connected-services-dialog"></a>연결된 서비스 대화 상자를 사용하여 Azure AD에 연결
1. Azure 계정이 있어야 합니다. Azure 계정이 없으면 [무료 평가판](http://go.microsoft.com/fwlink/?LinkId=518146)에 등록할 수 있습니다.
2. Visual Studio의 프로젝트에서 **참조** 노드의 바로 가기 메뉴를 열고 **연결된 서비스 추가**를 선택합니다.
3. **Azure AD 인증**을 선택한 다음 **구성**을 선택합니다.
...
4. **Azure AD 인증 구성**의 첫 페이지에서 **Azure AD를 사용하여 Single Sign-on 구성**을 확인합니다.
...
5. 두 번째 페이지의 **도메인** 드롭다운 목록에서 도메인을 선택합니다. 도메인 목록에는 계정 설정 대화 상자에 나열된 계정으로 액세스할 수 있는 모든 도메인을 포함합니다. 대신, 도메인을 찾을 수 없는 경우 mydomain.onmicrosoft.com 등과 같은 도메인 이름을 입력할 수 있습니다. 새 Azure AD 앱을 만들거나 기존 Azure AD 앱의 설정을 사용하는 옵션을 선택할 수 있습니다. 
   
   ![마법사에서 Azure AD 구성](./media/vs-azure-tools-connected-services-add-active-directory/configure-azure-ad-wizard-2.png)
6. 마법사의 세 번째 페이지에서 **디렉터리 데이터 읽기** 가 선택되어야 합니다. 마법사는 **클라이언트 암호**를 채웁니다. 
   
    ![마법사에서 Azure AD 구성](./media/vs-azure-tools-connected-services-add-active-directory/configure-azure-ad-wizard-3.png)
7. **마침** 단추를 선택합니다. 대화 상자는 Azure AD 인증을 위한 프로젝트를 사용 가능하도록 필요한 구성 코드 및 참조를 추가합니다. [Azure 포털](http://go.microsoft.com/fwlink/p/?LinkID=525040)에서 AD 도메인을 볼 수 있습니다.
8. 다음 단계에 대한 아이디어를 위해 브라우저에 표시되는 시작 페이지 및 프로젝트를 수정한 방법을 보기 위해 변경된 내용 페이지를 검토합니다. 모든 항목이 제대로 작동하는지 확인하려면 수정된 구성 파일 중 하나를 열고 변경된 내용에 언급된 설정이 있는지 확인합니다. 예를 들어, ASP.NET MVC 프로젝트의 기본 web.config에 다음 설정이 추가됩니다.
