...
Azure에서는 가상 컴퓨터를 사용하여 계산 집약적인 작업을 처리할 수 있습니다. 예를 들어 가상 컴퓨터에서 작업을 처리하고 클라이언트 컴퓨터나 모바일 응용 프로그램에 결과를 전달합니다. 이 가이드를 완료하면 다른 .NET 응용 프로그램에서 모니터링할 수 있는 계산 집약적인 .NET 응용 프로그램을 실행하는 가상 컴퓨터를 만드는 방법을 이해할 수 있게 됩니다.
...
![순회 외판원 문제 해 찾기][순회 외판원 문제 해 찾기]

다음은 계산 집약적인 작업을 모니터링하는 .NET 응용 프로그램의 예제입니다.

![순회 외판원 문제 클라이언트][순회 외판원 문제 클라이언트]
...
6.  **이미지** 드롭다운 목록에서 **Windows Server 2012**와 같은 이미지를 선택합니다.
...
10. **가상 컴퓨터 만들기**를 클릭합니다. 가상 컴퓨터가 만들어지기 시작합니다. 관리 포털의 **가상 컴퓨터** 섹션에서 상태를 모니터링할 수 있습니다. 상태가 **활성**으로 표시된 경우 가상 컴퓨터에 로그인할 수 있습니다.
...
새 네임스페이스에 대해 큐 만들기 등의 관리 작업을 수행하려면 네임스페이스에
대한 관리 자격 증명을 받아야 합니다.

1.  왼쪽 탐색 창에서 **서비스 버스** 노드를 클릭하여 사용 가능한
    네임스페이스 목록을 표시합니다.
    ![사용 가능한 네임스페이스 스크린샷][사용 가능한 네임스페이스 스크린샷]
2.  표시된 목록에서 방금 만든 네임스페이스를 선택합니다.
    ![네임스페이스 목록 스크린샷][네임스페이스 목록 스크린샷]
3.  **액세스 키**를 클릭합니다.
    ![액세스 키 단추][액세스 키 단추]
4.  대화 상자에서 **기본 발급자** 및 **기본 키** 항목을 찾습니다. 아래에서 이 정보를 사용하여 네임스페이스 관련 작업을 수행할 것이므로 이 값을 기록해 둡니다.

## 계산 집약적인 작업을 수행하는 .NET 응용 프로그램을 만드는 방법

1.  개발 컴퓨터(직접 생성한 가상 컴퓨터일 필요는 없음)에서 [.NET용 Azure SDK][.NET용 Azure SDK](영문)를 다운로드합니다.
...
6.  **your\_service\_bus\_namespace**, **your\_service\_bus\_owner** 및 **your\_service\_bus\_key** 자리 표시자를 각각 서비스 버스의 **네임스페이스**, **기본 발급자** 및 **기본 키** 값을 사용하도록 수정합니다.
...
1.  개발 컴퓨터에서 프로젝트 이름으로 **TSPClient**를 사용하는 .NET 콘솔 응용 프로그램을 만듭니다. 대상 프레임워크가 **.NET Framework 4**(**.NET Framework 4 Client Profile** 아님)용으로 설정되어 있는지 확인합니다. 대상 프레임워크는 다음과 같이 프로젝트를 만든 후에 설정할 수 있습니다. Visual Studio의 메뉴에서 **프로젝트**를 클릭하고 **속성**을 클릭한 후 **응용 프로그램** 탭을 클릭한 다음 **대상 프레임워크** 값을 설정합니다.
2.  Microsoft ServiceBus 라이브러리에 추가합니다. Visual Studio 솔루션 탐색기에서 **TSPSolver**를 마우스 오른쪽 단추로 클릭하고 **참조 추가**를 클릭한 후 **찾아보기** 탭을 클릭하고 **C:\\Program Files\\Microsoft SDKs\\Windows Azure.NET SDK\\2012-06\\ref**로 이동한 후 참조로 **Microsoft.ServiceBus.dll**을 선택합니다.
...
5.  **your\_service\_bus\_namespace**, **your\_service\_bus\_owner** 및 **your\_service\_bus\_key** 자리 표시자를 각각 서비스 버스의 **네임스페이스**, **기본 발급자** 및 **기본 키** 값을 사용하도록 수정합니다.
...
계산 집약적인 응용 프로그램을 실행하여 먼저 큐를 만든 후에 순회 외판원 문제를 해결합니다. 그러면 현시점에서의 최상의 경로가 서비스 버스 큐에 추가됩니다. 계산 집약적인 응용 프로그램이 실행 중인 동안 또는 실행된 이후에 클라이언트를 실행하여 서비스 버스 큐에서 가져온 결과를 표시합니다.
...
TSPSolver를 사용하여 큐를 만들거나 제거하는 대신 [Azure 관리 포털][Azure 관리 포털]을 사용하여 큐를 만들거나 제거할 수 있습니다. 관리 포털의 서비스 버스 섹션을 방문하여 큐를 만들거나 삭제할 뿐만 아니라 연결 문자열, 발급자 및 액세스 키를 검색하는 데 사용되는 사용자 인터페이스에 액세스합니다. 서비스 버스 큐의 대시보드를 표시하여 여기서 들어오는 메시지와 보내는 메시지의 메트릭을 볼 수도 있습니다.

  [순회 외판원 문제 해 찾기]: ./media/virtual-machines-dotnet-run-compute-intensive-task/WA_dotNetTSPSolver.png
  [순회 외판원 문제 클라이언트]: ./media/virtual-machines-dotnet-run-compute-intensive-task/WA_dotNetTSPClient.png
  [Azure 관리 포털]: https://manage.windowsazure.com
  [새 서비스 버스 만들기]: ./media/virtual-machines-dotnet-run-compute-intensive-task/ServiceBusCreateNew.png
  [네임스페이스 만들기 대화 상자]: ./media/virtual-machines-dotnet-run-compute-intensive-task/CreateNameSpaceDialog.png
  [만들기 클릭 스크린샷]: ./media/virtual-machines-dotnet-run-compute-intensive-task/ClickCreate.png
  [사용 가능한 네임스페이스 스크린샷]: ./media/virtual-machines-dotnet-run-compute-intensive-task/AvailableNamespaces.png
  [네임스페이스 목록 스크린샷]: ./media/virtual-machines-dotnet-run-compute-intensive-task/NamespaceList.png
  [액세스 키 단추]: ./media/virtual-machines-dotnet-run-compute-intensive-task/AccessKey.png
  [.NET용 Azure SDK]: http://www.windowsazure.com/ko-kr/develop/net/
