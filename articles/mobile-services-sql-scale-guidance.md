...
5. 열을 선택합니다. 명령 모음에서 **인덱스 설정**을 클릭합니다.
	...
	![모바일 서비스 포털 - 인덱스 설정][SetIndexJavaScriptPortal]
...
![SQL Server Management Studio - 동적 관리 뷰][SSMSDMVs]

또는 SQL 데이터베이스 관리 포털을 사용하는 경우 먼저 데이터베이스를 선택하고 **새 쿼리**를 선택합니다.

![SQL 데이터베이스 관리 포털 - 새 쿼리][PortalSqlManagementNewQuery]

아래의 쿼리를 실행하려면 창으로 전달하고 **실행**을 선택합니다.

![SQL 데이터베이스 관리 포털 - 쿼리 성능][PortalSqlManagementRunQuery]
...
자세한 내용은 [동적 관리 뷰를 사용하여 SQL 데이터베이스 모니터링][](영문)을 참조하십시오. 쿼리 실행 외에도 **SQL 데이터베이스 관리 포털**은 데이터베이스에 대해 **요약**을 선택하고 **쿼리 성능**을 선택하여 이 데이터를 볼 수 있는 효과적인 바로 가기를 제공합니다.

![SQL 데이터베이스 관리 포털 - 쿼리 성능][PortalSqlManagementQueryPerformance]
...
![SQL Server Management Studio - 쿼리 계획][SSMSQueryPlan]

**SQL 데이터베이스 관리 포털**에서 쿼리 계획을 분석하려면 강조 표시된 도구 모음 단추를 사용합니다.

![SQL 데이터베이스 관리 포털 - 쿼리 계획][PortalSqlManagementQueryPlan]
