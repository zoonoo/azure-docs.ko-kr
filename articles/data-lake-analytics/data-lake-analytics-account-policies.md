...
계정 정책은 Azure Data Lake Analytics 계정이 사용 되는 방법을 제어 하는 데 도움이 됩니다. 이러한 정책을 통해 Azure Data Lake Analytics 사용 비용을 제어할 수 있습니다. 예를 들어 이러한 정책을 사용 하면 계정에서 동시에 사용할 수 있는 au의 수를 제한 하 여 예기치 않은 비용 급증을 방지할 수 있습니다. # # 계정 수준 정책

이러한 정책은 Data Lake Analytics 계정의 모든 작업에 적용 됩니다. # # # Data Lake Analytics 계정의 최대 au 수 Data Lake Analytics 계정에서 사용할 수 있는 총 au (분석 단위) 수를 제어 합니다. 기본적으로 이 값은 250으로 설정됩니다. 예를 들어 이 값을 250 AU로 설정한 경우 250 AU가 할당되어 실행 중인 작업 한 개 또는 각각 25 AU가 할당되어 실행 중인 작업 10개가 있을 수 있습니다. 제출된 추가 작업은 실행 중인 작업이 완료될 때까지 큐에 대기됩니다. 실행 중인 작업이 완료되면 대기 작업이 실행되도록 AU가 확보됩니다.
...
Data Lake Analytics에는 작업 수준에서 설정할 수 있는 두 개의 정책 ( **작업당 * AU limit**)이 있습니다. 사용자는이 수의 AU를 포함 하는 작업만 제출할 수 있습니다. 기본적으로 이 한도는 계정의 최대 AU 한도와 같습니다.
* **우선 순위**: 사용자는 우선 순위가 이 값보다 낮거나 같은 작업만 제출할 수 있습니다. 숫자가 높을수록 우선 순위가 낮습니다. 기본적으로 이 한도는 가능한 가장 높은 우선 순위인 1로 설정됩니다.

모든 계정에는 설정된 기본 정책이 있습니다. 기본 정책은 계정의 모든 사용자에게 적용됩니다. 특정 사용자 및 그룹에 대 한 추가 정책을 만들 수 있습니다. 
...
4.  **삭제** 및 **편집** 옵션을 보려면 테이블의 맨 오른쪽 열에서 `...` . # # 작업 정책에 대 한 추가 리소스를 클릭 합니다.
...
* [작업 수준 정책 블로그 게시물](https://blogs.msdn.microsoft.com/azuredatalake/2017/06/08/managing-your-azure-data-lake-analytics-compute-resources-job-level-policy/)# # 다음 단계
