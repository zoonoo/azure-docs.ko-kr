...
1.  MSBuild에서 U-SQL 언어 대상을 찾을 수 있도록 NuGet 복원 태스크를 추가하여 `Azure.DataLake.USQL.SDK`를 포함하는 솔루션 참조 NuGet 패키지를 가져옵니다. 
...
1.  MSBuild에서 U-SQL 언어 대상을 찾을 수 있도록 NuGet 복원 태스크를 추가하여 `Azure.DataLake.USQL.SDK`를 포함하는 솔루션 참조 NuGet 패키지를 가져옵니다. 
    ...
    ![Data Lake에서 U-SQL 데이터베이스 프로젝트에 대한 CI CD MSBuild 태스크 설정](./media/data-lake-analytics-cicd-overview/data-lake-analytics-set-vsts-msbuild-task.png) 
...
1. 빌드 또는 릴리스 파이프라인에 PowerShell 스크립트 태스크를 추가하고 아래의 PowerShell 스크립트를 실행합니다. 이 태스크는 `PackageDeploymentTool.exe`에 대한 Azure SDK 종속성을 가져오는 데 도움이 됩니다. 이러한 종속성을 일부 특정 폴더에 로드하도록 -outputfolder 매개 변수를 설정할 수 있습니다. 이 폴더 경로를 2단계의 `PackageDeploymentTool.exe`로 전달합니다. 
...
2. 빌드 또는 릴리스 파이프라인에 **명령줄 태스크**를 추가하고, `PackageDeploymentTool.exe`를 호출하는 스크립트를 채웁니다. 샘플 스크립트는 다음과 같습니다. 
