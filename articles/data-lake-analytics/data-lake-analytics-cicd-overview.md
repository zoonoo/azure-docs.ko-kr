...
* **USQLSDKPath =\<U SQL Nuget 패키지> \build\runtime** 이 매개 변수는 U-SQL 언어 서비스에 대한 NuGet 패키지의 설치 경로를 나타냅니다.
...
1.  MSBuild에서 U-SQL 언어 대상을 찾을 수 있도록 NuGet 복원 작업을 추가하여 `Azure.DataLake.USQL.SDK`를 포함하는 솔루션 참조 NuGet 패키지를 가져옵니다. 2 단계에서 직접 MSBuild `$(Build.SourcesDirectory)/packages` 인수 예제를 사용 하려면 **고급** > **대상 디렉터리** 를로 설정 합니다.
...
|Interactive (대화형)|인증에 대화형 모드를 사용할지 여부입니다.|false|false|
