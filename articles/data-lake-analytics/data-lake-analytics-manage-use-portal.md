...
6. 다음 값을 입력하거나 선택합니다.
	...
	- **리소스 그룹**. 기존 Azure 리소스 그룹을 선택하거나 리소스 그룹을 새로 만듭니다. Azure 리소스 관리자(ARM)를 사용하면 응용 프로그램에서 리소스를 그룹으로 사용할 수 있습니다. 자세한 내용은 [Azure 리소스 관리자 개요](resource-group-overview.md)를 참조하세요. 
...
|판독기|모든 것을 볼 수 있지만 변경은 할 수 없습니다.|  
...
3. **카탈로그**, **마스터**, **Tables를 차례로 확장하거나 **테이블 반환 함수** 또는 **어셈블리**를 확장합니다. 다음 스크린샷은 테이블 반환 함수를 보여줍니다.
