...
---
# <a name="manage-azure-data-lake-analytics-using-azure-portal"></a>Azure 포털을 사용하여 Azure Data Lake Analytics 관리
[!INCLUDE [manage-selector](../../includes/data-lake-analytics-selector-manage.md)]

Azure Portal을 사용하여 Azure Data Lake Analytics 계정, 계정 데이터 원본, 사용자 및 작업을 관리하는 방법에 대해 알아봅니다. 다른 도구를 사용하여 관리 항목을 보려면 페이지 위의 탭 선택기를 클릭합니다.
...
데이터 레이크 분석 작업을 실행하려면 데이터 레이크 분석 계정이 있어야 합니다. Azure HDInsight와 달리 작업을 실행할 때 Data Lake Analytics 계정 비용만 지불하면 됩니다.  작업이 실행되는 시간에 대해서만 비용을 지불합니다.  자세한 내용은 [Azure 데이터 레이크 분석 개요](data-lake-analytics-overview.md)를 참조하세요.  
...
1. [Azure 포털](https://portal.azure.com/)에 로그인합니다.
2. 왼쪽 메뉴에서 **Data Lake Analytics**를 클릭합니다.  이 옵션이 보이지 않으면 **더 많은 서비스**를 클릭하고**인텔리전스 + 분석** 아래에서 **Data Lake Analytics**를 클릭합니다.
3. 액세스하려는 데이터 레이크 분석 계정을 클릭합니다. 새 블레이드에 계정이 열립니다.

**데이터 레이크 분석 계정 삭제**

1. 삭제할 데이터 레이크 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요.
2. 블레이드 위쪽의 단추 메뉴에서 **삭제** 를 클릭합니다.
3. 계정 이름을 입력하고 **삭제**를 클릭합니다.

Data Lake Analytics 계정을 삭제해도 종속 Data Lake Store 계정은 삭제되지 않습니다. 데이터 레이크 저장소 계정 삭제에 대한 자세한 내용은 [데이터 레이크 저장소 계정 삭제](../data-lake-store/data-lake-store-get-started-portal.md#delete-azure-data-lake-store-account)를 참조하세요.
...
* 관리하려는 데이터 레이크 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요. 기본 데이터 레이크 저장소는 **Essential**에 표시됩니다.
  
    ![Azure Data Lake Analytics 데이터 원본 추가](./media/data-lake-analytics-manage-use-portal/data-lake-analytics-default-adl-storage-account.png)

**데이터 원본 더 추가하기**

1. 관리하려는 데이터 레이크 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요.
...
3. **데이터 원본 추가**를 클릭합니다.
    ...
    Azure Data Lake Store 계정을 추가하려면 계정 이름이 필요하고 쿼리를 할 수 있으려면 계정에 대한 액세스가 필요합니다.
    Azure Blob Storage를 추가하려면 저장소 계정 및 계정 키가 필요하며 이 정보는 포털에서 저장소 계정으로 이동하여 찾을 수 있습니다.

<a name="explore-data-sources"></a>**데이터 원본 탐색**    

1. 관리하려는 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요.
...
3. Data Lake Store 계정을 클릭하여 엽니다.
...
1. 포털의 왼쪽 메뉴에서 **찾아보기**를 클릭한 후 **Data Lake Store**를 클릭합니다.
...
5. 파일을 업로드하려면 위쪽 메뉴에서 **업로드** 를 클릭합니다.
...
| 참여자 |포털에 액세스하고 작업을 제출하고 모니터링합니다. 작업을 제출할 수 있으려면 참여자에게 Data Lake Store 계정에 대한 읽기 또는 쓰기 권한이 필요합니다. |
| DataLakeAnalyticsDeveloper |작업을 제출, 모니터링 및 취소합니다.  이러한 사용자는 자신의 작업만 취소할 수 있습니다. 자신의 계정을 관리할 수 없습니다. 예를 들어 사용자 추가, 권한 변경 또는 계정 삭제를 할 수 없습니다. 작업을 실행할 수 있으려면 데이터 레이크 저장소 계정에 대한 읽기 또는 쓰기 액세스가 필요합니다. |
...
**Data Lake Analytics 계정에 사용자 또는 보안 그룹을 추가하려면**
...
모든 U-SQL 작업을 실행하기 전에 Data Lake Analytics 계정이 있어야 합니다.  자세한 내용은 [데이터 레이크 분석 계정 관리](#manage-data-lake-analytics-accounts)를 참조하세요.
...
2. **새 작업**을 클릭합니다.
    ...
    |----|-----------|
    |작업 이름|작업의 이름을 입력합니다.|
    |우선 순위|번호가 낮을수록 우선 순위가 높습니다. 두 작업이 모두 큐에 대기 중이면 우선 순위가 낮은 작업이 먼저 실행됩니다.|
...
4. 작업을 다시 제출하려면 **다시 제출** 을 클릭합니다.

**작업을 다시 제출하려면**

[데이터 레이크 분석 작업 모니터링](#monitor-jobs)을 참조하세요.

## <a name="monitor-account-usage"></a>계정 사용 현황 모니터링
**계정 사용 현황을 모니터링하려면**

1. 관리하려는 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요. 계정 사용 현황 패널에는 사용 현황이 표시됩니다.
...
## <a name="view-u-sql-catalog"></a>U-SQL 카탈로그 보기
[U-SQL 카탈로그](data-lake-analytics-use-u-sql-catalog.md) 는 U-SQL 스크립트에서 공유할 수 있도록 데이터 및 코드의 구조를 구성하는 데 사용됩니다. 카탈로그를 사용하면 가능한 가장 높은 성능으로 Azure 데이터 레이크의 데이터를 사용할 수 있습니다. Azure Portal을 통해 U-SQL 카탈로그를 볼 수 있습니다.

**U-SQL 카탈로그를 탐색하려면**

1. 관리하려는 분석 계정을 엽니다. 자세한 내용은 [Data Lake Analytics 계정 액세스](#access-adla-account)를 참조하세요.
2. 위쪽 메뉴에서 **데이터 탐색기** 를 클릭합니다.
3. **카탈로그**, **master**, **Tables를 차례로 확장하거나 **테이블 반환 함수** 또는 **어셈블리**를 확장합니다. 다음 스크린샷은 테이블 반환 함수를 보여줍니다.
