...
2. 원본 데이터를 준비합니다. 데이터 레이크 분석 작업은 Azure 데이터 레이크 저장소 계정 또는 Azure Blob 저장소 계정에서 데이터를 읽어올 수 있습니다. 이 예제에서는 Azure 데이터 레이크 저장소에서 데이터를 읽어옵니다.  
3. U-SQL 스크립트를 개발합니다.
...
6. 다음 값을 입력하거나 선택합니다.
	...
	- **이름**: 분석 계정의 이름을 지정합니다.
	- **데이터 레이크 저장소**: 각 데이터 레이크 분석 계정에는 종속 데이터 레이크 저장소 계정이 있습니다. 데이터 레이크 분석 계정 및 종속 데이터 레이크 저장소 계정은 동일한 Azure 데이터 센터에 있어야 합니다. 지침을 따라 새 데이터 레이크 저장소 계정을 만들거나 기존 계정을 선택합니다.
...
실제로는, 연결된 저장소 계정에 데이터를 작성하도록 응용 프로그램을 프로그래밍 하거나 데이터를 업로드합니다. 파일 업로드에 대한 내용은 [데이터 레이크 저장소에 데이터 업로드](data-lake-analytics-manage-use-portal.md#upload-data-to-adls) 또는 [Blob 저장소에 데이터 업로드](data-lake-analytics-manage-use-portal.md#upload-data-to-wasb)를 참조하세요.
...
4. **작업 이름**을 입력하고 다음 U-SQL 스크립트를 입력합니다.
    ...
    U-SQL에 대한 자세한 내용은 [Azure 데이터 레이크 분석 U-SQL 언어 시작](data-lake-analytics-u-sql-get-started.md) 및 [U-SQL 언어 참조](http://go.microsoft.com/fwlink/?LinkId=691348)를 참조하세요.
