...
2. 이러한 컨테이너 내에서 로그는 다음 구조로 저장됩니다.
   ...
   > [!NOTE]
   > 경로의 `##` 항목은 로그가 생성된 연도, 월, 일 및 시간을 포함합니다. Data Lake Analytics는 1시간마다 하나의 파일을 만들므로 `m=`은(는) 항상 `00`의 값을 포함합니다.
   > 
   > 
...
> [!NOTE]
> **resultType** 및 **resultSignature__는 작업의 결과에 대한 정보를 제공하고 작업이 완료되었을 경우에 값을 포함합니다. 예를 들어 __operationName__이 __JobStarted** 또는 __JobEnded__의 값을 포함할 때 값을 포함합니다.
> 
> 
...
> [!NOTE]
> **SubmitTime**, **StartTime**, **EndTime** 및 __Parallelism__은 작업에 대한 정보를 제공하고 작업이 시작 또는 완료된 경우에 값을 포함합니다. 예를 들어 __operationName__이 __JobSubmitted__를 표시한 후 __SubmitTime__은 값을 포함합니다.
> 
> 
