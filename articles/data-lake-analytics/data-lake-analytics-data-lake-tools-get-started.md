---
title: Azure 데이터 레이크 분석 쿼리 - 비주얼 스튜디오
...
Azure Data Lake 및 Stream Analytics Tools는 두 가지 Azure 서비스, Azure Data Lake Analytics 및 Azure Stream Analytics와 관련된 기능을 포함합니다. Azure 스트림 분석 시나리오에 대한 자세한 내용은 Visual Studio 에 [대한 Azure 스트림 분석 도구를](../stream-analytics/stream-analytics-tools-for-visual-studio-install.md)참조하십시오.

이 문서에서는 Visual Studio를 사용하여 Azure Data Lake Analytics 계정을 만드는 방법에 대해 설명합니다. [U-SQL에서](data-lake-analytics-u-sql-get-started.md)작업을 정의하고 데이터 레이크 분석 서비스에 작업을 제출할 수 있습니다. 데이터 레이크 분석에 대한 자세한 내용은 [Azure 데이터 레이크 분석 개요](data-lake-analytics-overview.md)를 참조하세요.

> [!IMPORTANT]
> Visual Studio 버전 2.3.3000.4 이상용 Azure 데이터 레이크 도구로 업그레이드하는 것이 좋습니다. 이전 버전은 더 이상 다운로드할 수 없으며 이제 사용되지 않습니다.
>
...
>    - [비주얼 스튜디오 2017 및 2019용](https://marketplace.visualstudio.com/items?itemName=ADLTools.AzureDataLakeandStreamAnalyticsTools)
...
이 자습서를 실행하려면 Data Lake Tools for Visual Studio가 설치되어 있어야 합니다. 자세한 내용은 [Visual Studio용 데이터 레이크 도구 설치를](data-lake-analytics-data-lake-tools-install.md)참조하십시오.

## <a name="connect-to-an-azure-data-lake-analytics-account"></a>Azure Data Lake Analytics 계정에 연결

1. Visual Studio를 엽니다.

1. **서버 탐색기** **보기를** > 선택하여 서버**탐색기를**엽니다.

1. **Azure를**마우스 오른쪽 단추로 클릭한 다음 **Microsoft Azure 구독에 연결 을**선택합니다. **계정에 로그인할 때**지침을 따릅니다.

1. **서버 탐색기에서** **Azure** > **데이터 레이크 분석**을 선택합니다. Data Lake Analytics 계정 목록이 표시됩니다.
...
1. 비주얼 스튜디오에서 새**프로젝트** **파일** > 을**선택합니다.** > 

1. **U-SQL 프로젝트** 유형을 선택한 다음 **다음을**선택합니다. **새 프로젝트 구성에서** **을 선택합니다.**

   Visual Studio는 **Script.usql** 파일이 포함된 솔루션을 만듭니다.

1. [첫 번째 U-SQL 스크립트 쓰기에서](#write-your-first-u-sql-script) 스크립트를 **Script.usql** 창에 붙여넣습니다.

1. **솔루션 탐색기에서** **Script.usql을**마우스 오른쪽 단추로 클릭하고 **스크립트 제출을**선택합니다.

1. **작업 제출에서**데이터 레이크 분석 계정을 선택하고 **제출을**선택합니다.
...
* **AU 분석은** 작업에 사용된 AU의 수를 보여주고 다양한 AU 할당 전략의 시뮬레이션을 탐색합니다.
...
최신 작업 상태를 확인하고 화면을 새로 고치려면 **새로 고침을 선택합니다.**

## <a name="check-job-status"></a>작업 상태 확인

1. **서버 탐색기에서** **Azure** > **데이터 레이크 분석**을 선택합니다.
