...
1. Ctrl + Shift + P를 선택 하 여 명령 팔레트를 연 다음 adl을 입력 **합니다. 로컬 실행 패키지** 를 다운로드 하 여 패키지를 다운로드 합니다.  
...
첫 번째 사용자의 경우 adl을 **사용 합니다. ** [SQL 로컬 실행 환경을 설정](#set-up-the-u-sql-local-run-environment)하지 않은 경우 로컬 실행 패키지 를 다운로드 하 여 로컬 실행 패키지를 다운로드 합니다.

1. Ctrl + Shift + P를 선택 하 여 명령 팔레트를 연 다음 adl을 입력 **합니다. 로컬 실행 서비스**를 시작 합니다.   
...
1. Adl 사용 **: ** [SQL 로컬 실행 환경을 설정](#set-up-the-u-sql-local-run-environment)하지 않은 경우 로컬 실행 패키지 를 다운로드 하 여 로컬 실행 패키지를 다운로드 합니다.
...
1. Ctrl + Shift + P를 선택 하 여 명령 팔레트를 연 다음 adl을 입력 **합니다. 로컬 실행 서비스**를 시작 합니다. cmd 콘솔이 열립니다. **DataRoot**가 설정되어 있는지 확인합니다.
