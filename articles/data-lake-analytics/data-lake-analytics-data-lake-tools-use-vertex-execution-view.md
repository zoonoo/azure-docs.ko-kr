...
* 읽은 총 데이터: 이 꼭 짓 점에서 데이터를 읽고 씁니다.
* 상태/종료 상태: 꼭 짓 점이 종료 되는 최종 상태입니다.
* 종료 코드/오류 유형: 꼭 짓 점이 실패 한 경우의 오류입니다.
* 생성 이유: 꼭 짓 점이 만들어진 이유입니다.
* 리소스 대기 시간/프로세스 대기 시간/PN 큐 대기 시간: 꼭짓점이 리소스를 기다리는 데, 데이터를 처리하는 데, 큐에서 기다리는 데 걸리는 시간을 나타냅니다.
* 프로세스/작성자 GUID: 현재 실행 중인 꼭 짓 점 또는 해당 작성자의 GUID입니다.
