...
Azure Data Lake Visual Studio 도구를 사용하여 사용자 코드에서 버그에 의해 발생한 실패한 U-SQL 작업을 디버깅하는 방법에 알아봅니다. Data Lake Visual Studio 도구를 사용하면 클러스터에서 컴파일된 코드와 필요한 꼭짓점 데이터를 다운로드한 후 실패한 작업을 추적 및 디버깅할 수 있습니다.
...
사용자 정의 연산자가 U-SQL 코드 숨김(Script.usql.cs)에서 사용되는 경우 사용자 정의 연산자 코드로 클래스 라이브러리 C# 프로젝트를 만들고 VertexDebug 솔루션에 프로젝트를 포함해야 합니다. Data Lake 분석 데이터베이스에 .dll 어셈블리를 등록한 경우 VertexDebug 솔루션에 어셈블리의 소스 코드를 추가해야 합니다.
 

**프로젝트를 구성하려면**

1.	솔루션 탐색기에서 **LocalVertexHost**를 마우스 오른쪽 단추로 클릭한 후 **속성**을 클릭합니다.
...
1.	솔루션 탐색기에서 **LocalVertexHost** 프로젝트를 마우스 오른쪽 단추로 클릭하고 **디버그**를 가리킨 다음 **새 인스턴스 시작**을 클릭합니다. LocalVertexHost를 시작 프로젝트로 설정해야 합니다. 처음으로 다음과 같은 메시지가 표시될 수 있습니다. 이 메시지는 무시해도 됩니다. 디버그 화면으로 이동하는 데 최대 1분 정도 걸릴 수 있습니다.
