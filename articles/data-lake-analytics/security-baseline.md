...
이 보안 기준은 [Azure Security 벤치 마크 버전 1.0](../security/benchmarks/overview-v1.md) 의 지침을 Data Lake Analytics 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다.
콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤** 및 Data Lake Analytics에 적용 되는 관련 지침에 따라 그룹화 됩니다. Data Lake Analytics에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

 
Data Lake Analytics 완전히 Azure 보안 벤치 마크에 매핑되는 방법을 보려면 [전체 Data Lake Analytics 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
- [방화벽 규칙 개요](/rest/api/datalakeanalytics/firewallrules) 
...
**지침**: 비정상적인 동작에 대 한 로그를 분석 및 모니터링 하 고 Data Lake Analytics 리소스에 대 한 결과를 정기적으로 검토 합니다. Azure Monitor의 Log Analytics 작업 영역을 사용 하 여 로그를 검토 하 고 로그 데이터에 대 한 쿼리를 수행 합니다. 또는 Azure 센티널 또는 타사 시스템 정보 및 이벤트 관리 솔루션에 대 한 온보드 데이터를 사용할 수 있습니다.
...
**지침** Data Lake Analytics: 인증은 Azure Active Directory (azure AD)와 함께 제공 되 고 azure 역할 기반 액세스 제어 (azure RBAC)로 보호 되므로 기본 암호의 개념이 없습니다.
...
**지침**: Azure Active Directory (Azure AD) 다단계 인증을 사용 하도록 설정 하 고 Azure Security Center id 및 액세스 관리 권장 사항에 따라 Data Lake Analytics 리소스를 보호 합니다.
...
- [Azure Active Directory (Azure AD) 다단계 인증을 사용 하도록 설정 하는 방법](/security/compass/privileged-access-devices)
...
**지침**: Azure Active Directory (Azure AD)를 중앙 인증 및 권한 부여 시스템으로 사용 합니다. Azure RBAC (역할 기반 액세스 제어)는 클라이언트에서 Data Lake Analytics 리소스에 대 한 액세스를 세부적으로 제어할 수 있도록 합니다.
...
**지침**: Azure Active Directory (Azure AD)의 위험 및 id 보호 기능을 사용 하 여 Data Lake Analytics 리소스와 관련 된 검색 된 의심 스러운 작업에 대 한 자동화 된 응답을 구성 합니다. 조직의 보안 응답을 구현 하기 위해 Azure 센티널을 통해 자동화 된 응답을 사용 하도록 설정 해야 합니다.
...
**지침**: 별도의 구독, 환경, 데이터 민감도 등의 개별 보안 도메인에 대 한 관리 그룹을 사용 하 여 격리를 구현 합니다. 응용 프로그램 및 엔터프라이즈 환경에서 요구 하는 Data Lake Analytics 리소스에 대 한 액세스 수준을 제어 하도록 Data Lake Analytics를 제한할 수 있습니다. 방화벽 규칙이 구성 된 경우 지정 된 네트워크 집합을 통해 데이터를 요청 하는 응용 프로그램만 Data Lake Analytics 리소스에 액세스할 수 있습니다. Azure RBAC (역할 기반 액세스 제어)를 통해 Azure Data Lake Analytics에 대 한 액세스를 제어할 수 있습니다.
...
- [Azure Storage를 보호 하는 방법](../storage/blobs/security-recommendations.md)
...
- [예제 작업 목록](/rest/api/datalakeanalytics/operations/list)
...
- [Azure Data Lake Storage Gen1의 데이터 암호화](../data-lake-store/data-lake-store-encryption.md)
...
**지침**: "DataLakeAnalytics" 네임 스페이스의 Azure Policy 별칭을 사용 하 여 Azure Data Lake Analytics 서비스의 구성을 감사 하거나 적용 하는 사용자 지정 정책을 만듭니다. 다음과 같이 Azure Data Lake Analytics와 관련 된 기본 제공 정책 정의를 사용할 수도 있습니다.
...
**지침**: "DataLakeAnalytics" 네임 스페이스에 Azure Policy 별칭을 사용 하 여 시스템 구성을 경고, 감사 및 적용 하기 위한 사용자 지정 정책을 만듭니다. Azure Data Lake Analytics 리소스에 대 한 구성을 자동으로 적용 하려면 Azure Policy [감사], [거부] 및 [없는 경우 배포] 효과를 사용 합니다.
...
**지침**: Data Lake Analytics 작업 로그 및 데이터 출력은 기본 Data Lake Storage Gen1 서비스에 저장 됩니다.  ADLCopy, Azure PowerShell 또는 Azure Data Factory를 포함 하 여 데이터를 복사 하는 다양 한 방법을 사용할 수 있습니다.  Azure Automation를 사용 하 여 정기적으로 데이터를 자동으로 백업할 수도 있습니다.

- [Storage Explorer를 사용하여 Azure Data Lake Storage Gen1 리소스 관리](../data-lake-store/data-lake-store-in-storage-explorer.md)

- [Azure Storage Blob에서 Azure Data Lake Storage Gen1로 데이터 복사](../data-lake-store/data-lake-store-copy-data-azure-storage-blob.md)
...
**지침**: Data Lake Storage Gen1에 저장 된 백업이 Azure Storage 나 기본적으로 암호화를 지원 하며 해제할 수 없는 Data Lake Analytics입니다. 백업을 중요 한 데이터로 처리 하 고이 기준의 일부로 관련 액세스 및 데이터 보호 제어를 적용 해야 합니다.  

- [Azure Data Lake Storage Gen1에 저장된 데이터 보호](../data-lake-store/data-lake-store-secure-data.md)
