...
| [Azure Edge Zone](#edge-zones)| 대기 시간이 짧고 처리량이 많은 요구 사항을 해결 하기 위해 Vm, 컨테이너 및 기타 서비스를 Edge 영역에 배포 합니다.| <p>[캐리어를 사용 하는에 지 영역](edge-zones-overview.md#carrier)<p> <p>[개인에 지 영역](edge-zones-overview.md#private-edge-zones)</p>|
| [Azure 궤도](#orbital)| 우주선 또는 위성 constellations, 다운 링크 및 업링크 데이터와 통신 하 고, 클라우드에서 데이터를 처리 하 고, 고유한 시나리오에서 Azure 서비스를 사용 하 여 서비스를 연결 하 고, 고객을 위한 제품을 생성 합니다.  | <p>[글로벌 통신](azure-orbital-overview.md#global-communication)</p>  <p>[파트너 그라운드 스테이션](azure-orbital-overview.md#digital-transformation)</p>|
||||
...
|[Load Balancer](#loadbalancer)|는 가용성 영역 및 Vnet 트래픽을 라우팅 하 여 지역 부하 분산을 제공 합니다. 리소스 간에 트래픽을 라우팅하고 지역 응용 프로그램을 빌드하기 위해 내부 부하 분산을 제공 합니다.|<p> [VM에 내부 트래픽 부하 분산](../load-balancer/tutorial-load-balancer-standard-manage-portal.md)</p> <p>[가상 네트워크 내의 Vm 간에 트래픽 부하 분산](../load-balancer/tutorial-load-balancer-standard-internal-portal.md)<p>[특정 Vm의 특정 포트에 대 한 포트 전달 트래픽](../load-balancer/tutorial-load-balancer-port-forwarding-portal.md)</p><p> [부하 분산 및 아웃 바운드 규칙 구성](../load-balancer/configure-load-balancer-outbound-cli.md)</p>|
