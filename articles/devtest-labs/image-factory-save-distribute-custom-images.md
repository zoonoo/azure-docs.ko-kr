...
다음은 두 개의 랩을 나열 된 파일에 labs.js예제입니다. 이 경우 두 개의 다른 랩에 이미지를 배포 합니다.
...
배포 빌드 작업이 완료 되 면 새 빌드를 큐에 대기 하 여 모든 것이 작동 하는지 확인 합니다. 빌드가 성공적으로 완료 되 면 구성 파일의 Labs.js에 입력 된 대상 랩에 새 사용자 지정 이미지가 표시 됩니다.
