...
이 항목에서는 페이징을 사용하여 Azure 모바일 서비스에서 iOS 앱에 반환되는 데이터의 양을 관리하는 방법을 보여 줍니다. 이 자습서에서는 클라이언트에서 **fetchLimit** 및 **fetchOffset** 쿼리 속성을 사용하여 데이터의 특정 "페이지"를 요청합니다.
...
이 자습서는 이전 자습서인 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-ios)의 단계 및 샘플 앱을 기반으로 합니다. 이 자습서를 시작하기 전에 적어도 데이터 계열 작업에 대한 첫 번째 자습서인 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-ios) 자습서를 완료해야 합니다.

1.  Xcode에서 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-ios) 자습서를 완료할 때 수정한 프로젝트를 엽니다.
...
6.  다음 코드 줄을 찾아서 **refreshDataOnSuccess** 메서드를 한 번 더 업데이트합니다.
    ...
    **참고**
    ...
    이 자습서에서는 **fetchOffset** 및 **fetchLimit** 속성에 대해 하드 코드된 페이징 값을 설정하여 간소화된 시나리오를 사용합니다. 실제 앱에서는 Pager 컨트롤이나 이와 비슷한 UI에서 위와 비슷한 쿼리를 사용하여 사용자가 이전 및 다음 페이지로 이동하도록 만들 수 있습니다. **query.includeTotalCount = YES**를 설정하여 서버에서 사용 가능한 총 항목 수를 페이징 데이터와 함께 가져올 수도 있습니다.
...
-   [인증 시작하기](/en-us/develop/mobile/tutorials/get-started-with-users-ios)<br/>
