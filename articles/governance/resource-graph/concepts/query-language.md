...
|AlertsManagementResources |@No__t_1 _와 관련 된_ 리소스를 포함 합니다. |
|SecurityResources |@No__t_1 _와 관련 된_ 리소스를 포함 합니다. |
...
> [!NOTE]
> @No__t_1를 사용 하 여 `join` 결과를 제한 하는 경우 `join`에서 사용 하는 속성을 사용 하 여 두 테이블을 연결 합니다. 위 예제의 _subscriptionId_ 는 `project`에 포함 되어야 합니다.
...
|[limit](/azure/kusto/query/limitoperator) |[모든 공용 IP 주소 나열](../samples/starter.md#list-publicip) |@No__t_0의 동의어 |
...
|[주문을](/azure/kusto/query/orderoperator) |[리소스를 이름별로 나열](../samples/starter.md#list-resources) |@No__t_0의 동의어 |
...
|[sort](/azure/kusto/query/sortoperator) |[리소스를 이름별로 나열](../samples/starter.md#list-resources) |@No__t_0의 동의어 |
|[summarize](/azure/kusto/query/summarizeoperator) |[Azure 리소스 개수 계산](../samples/starter.md#count-resources) |첫 페이지 단순화만 |
|[take](/azure/kusto/query/takeoperator) |[모든 공용 IP 주소 나열](../samples/starter.md#list-publicip) |@No__t_0의 동의어 |
|[top](/azure/kusto/query/topoperator) |[이름 및 해당 OS 유형별로 처음 5개의 가상 머신 표시](../samples/starter.md#show-sorted) | |
|[union](/azure/kusto/query/unionoperator) |[두 쿼리의 결과를 단일 결과로 결합](../samples/advanced.md#unionresults) |단일 테이블 허용: _T_ `| union` \[ `kind=` `inner` \| `outer` \] \[ `withsource=`_ColumnName_ 1 _테이블_ 단일 쿼리에서 2 개의 `union` 개 다리를 제한 합니다. @No__t_0 레그 테이블의 유사 항목 확인은 허용 되지 않습니다. 단일 테이블 내에서 또는 _리소스_ 와 _ResourceContainers_ 테이블 간에 사용할 수 있습니다. |
...
@No__t_0 또는 `$`을 포함 하는 속성 이름 (예:)은 쿼리에서 래핑해야 하거나 이스케이프 해야 합니다. 그렇지 않으면 속성 이름이 올바르게 해석 되 고 예상 된 결과를 제공 하지 않습니다.
