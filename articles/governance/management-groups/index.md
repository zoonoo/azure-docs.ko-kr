...
정책을 적용할 수 있도록 계층 구조를 만듭니다(예: VM 위치를 "프로덕션" 그룹의 미국 서부 지역으로 제한). 이 정책은 해당 관리 그룹의 두 EA 구독으로 상속되어 해당 구독의 모든 VM에 적용됩니다. 거버넌스 향상을 위해 리소스 또는 구독 소유자가 이 보안 정책을 변경할 수 없습니다.
...
- 모든 Azure 고객은 루트 관리 그룹을 볼 수 있지만 일부 고객에게는 해당 루트 관리 그룹을 관리하는 액세스 권한이 없습니다.
  ...
  - 사용자에게는 루트 관리 그룹에 대한 기본 액세스 권한이 부여되지 않습니다. Azure AD 글로벌 관리자는 액세스 권한을 스스로 높일 수 있는 유일한 사용자입니다.  글로벌 관리자에게 액세스 권한이 있다면 RBAC 역할을 관리할 다른 사용자에게 할당할 수 있습니다.  
