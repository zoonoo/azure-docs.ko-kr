...
명령줄 또는 스크립트에서 Azure 리소스를 만들고 관리하는 데 Azure PowerShell 모듈이 사용됩니다. 이 가이드에서는 Az를 사용하여 정책 할당을 만드는 방법에 대해 설명합니다. 해당 정책은 Azure 환경에서 비준수 리소스를 식별합니다.
