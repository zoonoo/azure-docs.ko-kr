...
1. 목록에서 국가 클라우드를 선택 합니다.
   ...
   ![Visual Studio Code에 대 한 기본 Azure 클라우드 로그인 설정](../media/extension-for-vscode/set-default-azure-cloud-sign-in.png)
...
1. Azure Policy 확장 또는 명령 팔레트에서 Azure에 로그인 합니다.
   ...
   - Azure Policy 확장
     ...
     ![Azure Policy 확장의 Visual Studio Code에 대 한 Azure cloud 로그인](../media/extension-for-vscode/azure-cloud-sign-in-policy-extension.png)

   - 명령 팔레트

     메뉴 모음에서 **보기** > **명령 팔레트**로 이동 하 고 **Azure: 로그인**을 입력 합니다.

     ![명령 팔레트의 Visual Studio Code에 대 한 Azure cloud 로그인](../media/extension-for-vscode/azure-cloud-sign-in-command-palette.png)
...
1. 명령 팔레트 또는 창 바닥글에서 subscription 명령을 시작 합니다.
   ...
   - 명령 팔레트: 
     ...
     메뉴 모음에서 **보기** > **명령 팔레트**로 이동 하 고 **Azure Policy: 구독을 선택**합니다.

   - 창 바닥글

     화면 아래쪽의 창 바닥글에서 Azure와 일치 하는 세그먼트를 선택 **\>계정 \<** 합니다.
...
기본적으로 확장은 정책 별칭이 있는 리소스 및 기존 리소스를 기준으로 ' 리소스 공급자 '의 일부를 필터링 합니다. **설정** > **확장** > **Azure Policy** 에서이 동작을 변경 하 여 필터링 하지 않고 모든 리소스 공급자를 볼 수 있습니다.
...
검색 인터페이스를 통해 또는 treeview에서 리소스를 선택 하 여 리소스를 선택 하면 Azure Policy 확장에서 해당 리소스를 나타내는 JSON 파일과 모든 리소스 관리자 속성 값이 열립니다.

리소스가 열리면 리소스 관리자 속성 이름 또는 값을 마우스로 가리키면 Azure Policy 별칭이 표시 됩니다 (있는 경우). 이 예제에서 리소스는 `Microsoft.Compute/virtualMachines` 리소스 형식이 고 **속성. storageProfile. imageReference** 속성은 가리킴입니다. 가리키기는 일치 하는 별칭을 표시 합니다.

![Azure Policy 확장 가리키기 리소스 관리자 속성 별칭을 표시 합니다.](../media/extension-for-vscode/extension-hover-shows-property-alias.png)
...
1. Azure Policy 확장 프로그램 또는 명령 팔레트에서 검색 인터페이스를 시작 합니다.
   ...
   - 명령 팔레트:
     ...
     메뉴 모음에서 **보기** > **명령 팔레트**로 이동 하 고 **정책: 검색 정책**을 입력 합니다.
