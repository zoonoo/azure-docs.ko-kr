---
title: Azure Policy가 가상 머신 내에서 감사를 수행하는 방법 이해
...
게스트 구성을 사용하려면 먼저 리소스 공급자를 등록해야 합니다. 포털 또는 PowerShell을 통해 등록을 수행할 수 있습니다.
...
가상 머신 내에서 게스트 구성 클라이언트는 로컬 도구를 사용하여 감사를 수행합니다.
...
게스트 구성에서 수행하는 각 감사에는 두 가지 정책 정의(**DeployIfNotExists** 및 **AuditIfNotExists**)가 필요합니다. **DeployIfNotExists**는 게스트 구성 에이전트 및 [유효성 검사 도구](#validation-tools) 지원을 위한 기타 구성 요소가 포함된 가상 머신을 준비하는 데 사용됩니다.

**DeployIfNotExists** 정책 정의는 다음 항목의 유효성 검사 및 수정을 수행합니다.
...
**DeployIfNotExists**가 규정을 준수하는 것으로 확인되면 **AuditIfNotExists** 정책 정의는 로컬 유효성 검사 도구를 사용하여 할당된 구성의 규정 준수 여부를 확인합니다. 유효성 검사 도구는 결과를 게스트 구성 클라이언트에 제공하며, 클라이언트는 결과를 게스트 확장에 전달하여 게스트 구성 리소스 공급자를 통해 사용 가능하도록 제공합니다.
...
할당에 사용할 정의를 그룹화할 수 있도록, 게스트 구성을 위한 모든 기본 제공 정책은 이니셔티브에 포함됩니다. 기본 제공 이니셔티브 *[미리 보기]: Linux 및 Windows 가상 머신 내의 암호 보안 설정 감사*에는 정책 18개가 포함되어 있습니다. 그리고 Window용 **DeployIfNotExists** 및 **AuditIfNotExists** 쌍 6개와 Linux용 쌍 3개가 있습니다. 각 정의 내의 논리에 따라, [정책 규칙](definition-structure.md#policy-rule) 정의를 기준으로 하여 대상 운영 체제만 평가됩니다.
