...
1. 클러스터 서비스 사용자에 대 한 역할 할당 만들기
   ...
   - Azure CLI를 사용 하 여 ' Policy Insights 데이터 기록기 (미리 보기) ' 역할 할당을 클러스터 서비스 사용자 앱 ID (이전 단계의 값 _aadClientID_ )에 할당 합니다. `<subscriptionId>`를 구독 ID로 바꾸고 `<aks engine cluster resource group>`,을 AKS Engine 자체 관리 Kubernetes 클러스터가 있는 리소스 그룹으로 바꿉니다.
...
- 투구 차트를 사용 하 여 기존 클러스터에 설치
  ...
  1. 투구 차트를 사용 하 여 추가 기능을 설치 합니다. `<subscriptionId>`를 구독 ID로 바꾸고 `<aks engine cluster resource group>`,을 AKS Engine 자체 관리 Kubernetes 클러스터가 있는 리소스 그룹으로 바꿉니다.
...
Azure Policy 추가 기능 로그를 보려면 `kubectl`를 사용 합니다.
...
게이트 키퍼 로그를 보려면 `kubectl`를 사용 합니다.
