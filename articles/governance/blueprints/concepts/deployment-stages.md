...
Blueprint 배포는 구독에 Blueprint를 할당하거나 [기존 할당을 업데이트하여](../how-to/update-existing-assignments.md)트리거됩니다. 배포 하는 동안 Blueprints는 다음과 같은 높은 수준의 단계를 수행합니다.

> [!div class="checklist"]
> - 소유자 권한을 부여한 청사진
> - Blueprint 할당 개체가 작성됩니다.
> - 선택 사항 - 블루프린트가 **시스템 할당관리 ID를** 생성합니다.
> - 관리되는 ID는 Blueprint 아티팩트를 배포합니다.
> - Blueprint 서비스 및 **시스템 할당 관리** ID 권한이 취소됩니다.

## <a name="blueprints-granted-owner-rights"></a>소유자 권한을 부여한 청사진

Azure Blueprints 서비스 주체는 시스템 할당된 [관리되는 ID](../../../active-directory/managed-identities-azure-resources/overview.md) 관리 ID를 사용할 때 할당된 구독 또는 구독에 대한 소유자 권한을 부여합니다. 부여된 역할을 통해 Blueprint는 **시스템에서 할당된** 관리 ID를 만들고 나중에 해지할 수 있습니다. **사용자 할당된** 관리 되는 ID를 사용 하는 경우 Azure Blueprints 서비스 주체는 받지 않으며 구독에 대 한 소유자 권한이 필요 하지 않습니다.
...
Blueprints 서비스는 리소스를 직접 배포하지 않습니다.
...
## <a name="optional---blueprints-creates-system-assigned-managed-identity"></a>선택 사항 - 블루프린트가 시스템 할당관리 ID를 생성합니다.

할당 중에 [시스템 할당된 관리 ID를](../../../active-directory/managed-identities-azure-resources/overview.md) 선택하면 Blueprint는 ID를 만들고 관리되는 ID를 [소유자](../../../role-based-access-control/built-in-roles.md#owner) 역할로 부여합니다. 기존 [할당이 업그레이드된](../how-to/update-existing-assignments.md)경우 Blueprint는 이전에 만든 관리ID를 사용합니다.
...
배포에 의한 액세스 실패는 관리되는 ID에 부여된 액세스 수준의 결과인 경우가 많습니다. Blueprints 서비스는 **시스템에 할당된** 관리 ID의 보안 수명 주기를 관리합니다. 그러나 사용자는 **사용자가 할당한** 관리 ID의 권한 및 수명 주기를 관리할 책임이 있습니다.
...
배포가 완료되면 Blueprint는 구독에서 **시스템 할당된** 관리 ID의 권한을 취소합니다. 그런 다음 Blueprints 서비스는 구독에서 해당 권한을 취소합니다. 권한 제거는 블루프린트가 구독에서 영구 소유자가 되는 것을 방지합니다.
