...
절대 게시되지 않는 **초안** 모드의 청사진은 **게시된** 것과 다른 아이콘을 **청사진 정의** 페이지에 표시합니다. **최신 버전은** 게시되지 않은 이러한 Blueprint에 대한 **초안으로** 표시됩니다.
...
1. 왼쪽 페이지에서 **블루프린트 정의를** 선택하고 필터 옵션을 사용하여 버전을 삭제할 Blueprint를 찾습니다. 편집 페이지를 열려면 선택합니다.

1. 게시된 **버전 탭을** 선택하고 삭제하려는 버전을 찾습니다.

1. 버전을 마우스 오른쪽 단추로 클릭하여 삭제하고 **이 버전 삭제를**선택합니다.
...
### <a name="unassigning-assignments"></a>할당 부수 취소

Blueprint가 더 이상 필요하지 않은 경우 관리 그룹 또는 구독에서 할당취소할 수 있습니다. Blueprint 할당을 취소하는 동안 다음과 같은 일이 발생합니다.

- 청사진 [리소스 잠금](resource-locking.md) 제거
- Blueprint 할당 개체 삭제
- (조건부) 시스템 **할당된 관리 ID가** 사용된 경우 시스템 ID도 삭제됩니다.

> [!NOTE]
> Blueprint 할당에 배포된 모든 리소스는 제자리에 유지되지만 더 이상 Azure Blueprint에 의해 보호되지 않습니다.
