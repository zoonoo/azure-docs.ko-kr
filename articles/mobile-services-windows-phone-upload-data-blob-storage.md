...
이 자습서에서는 사진을 찍고 모바일 서비스에서 생성된 SAS를 사용하여 Azure에 이미지를 업로드하는 기능을 [GetStartedWithData 샘플 앱 프로젝트](mobile-services-windows-phone-get-started-data.md)에 추가합니다. 이 자습서에서는 간단한 TodoList 앱을 업데이트하여 Blob 저장소 서비스에 이미지를 업로드하는 다음과 같은 기본 단계를 안내합니다.
