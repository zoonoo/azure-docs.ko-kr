...
description: 암호화된 연결을 활성화하고 Azure Cognitive Search의 인덱서에서 Azure 가상 시스템(VM)의 SQL Server에 대한 연결을 허용하도록 방화벽을 구성합니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="configure-a-connection-from-an-azure-cognitive-search-indexer-to-sql-server-on-an-azure-vm"></a>Azure 인지 검색 인덱서에서 Azure VM에서 SQL Server로의 연결 구성

[인덱서를 사용하여 Azure SQL Database를 Azure 인지 검색에 연결하는](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md#faq)데 언급했듯이 Azure VM(또는 짧은 **경우 SQL Azure VM)에서** SQL **Server에** 대한 인덱서를 만드는 것은 Azure Cognitive Search에서 지원되지만 먼저 처리해야 할 몇 가지 보안 관련 필수 구성 조건이 있습니다. 

Azure 인지 검색에서 VM의 SQL Server로의 연결은 공용 인터넷 연결입니다. 이러한 연결에 대해 일반적으로 따를 수 있는 모든 보안 조치는 여기에도 적용됩니다.
...
Azure Cognitive Search는 공용 인터넷 연결을 통해 모든 인덱서 요청에 대해 암호화된 채널을 필요로 합니다. 이 섹션에는 작동하도록 하는 단계가 나열되어 있습니다.
...
2. 레지스트리 편집기(regedit)를 사용하여 인증서를 사용하도록 SQL Server를 구성합니다. 
   ...
   * **인증서** 키 값을 VM에 가져온 TLS/SSL 인증서의 **지문으로** 설정합니다.
...
3. 서비스 계정에 권한을 부여합니다. 
    ...
    SQL Server 서비스 계정에 TLS/SSL 인증서의 개인 키에 대한 적절한 권한이 부여되었는지 확인합니다. 이 단계를 무시하면 SQL Server가 시작되지 않습니다. 이 작업에 대해 **인증서** 스냅인 또는 **CertUtils**를 사용할 수 있습니다.
    
4. SQL Server 서비스를 다시 시작합니다.

## <a name="configure-sql-server-connectivity-in-the-vm"></a>VM에서 SQL Server 연결을 구성합니다.
Azure Cognitive Search에 필요한 암호화된 연결을 설정한 후 Azure VM의 SQL Server에 내재된 추가 구성 단계가 있습니다. 아직 수행하지 않은 경우 다음 단계로 이 문서 중 하나를 사용하여 구성을 완료합니다.
...
Azure VM에서 다른 대상에 액세스할 수 있게 하기 위해 NSG 및 해당 Azure 엔드포인트 또는 ACL(Access Control 목록)을 구성하는 것은 특별하지 않습니다. 이전에 이러한 구성을 수행하여 자체 애플리케이션 논리가 SQL Azure VM에 연결되도록 했을 것입니다. SQL Azure VM에 대한 Azure 인지 검색 연결도 마찬가지입니다. 

아래 링크는 VM 배포를 위한 NSG 구성에 대한 지침을 제공합니다. 이 지침을 사용하여 IP 주소를 기반으로 Azure 인지 검색 끝점을 ACL에 사용합니다.
...
#### <a name="restrict-access-to-the-azure-cognitive-search"></a>Azure 인지 검색에 대한 액세스 제한
모든 연결 요청에 대해 SQL Azure VM을 열지 않도록 검색 `AzureCognitiveSearch` 서비스의 IP 주소와 ACL의 IP 주소 [태그](https://docs.microsoft.com/azure/virtual-network/service-tags-overview#available-service-tags) 범위에 대한 액세스를 제한하는 것이 좋습니다.

검색 서비스의 FQDN(예: `<your-search-service-name>.search.windows.net`)을 ping하여 IP 주소를 찾을 수 있습니다.
...
후속 연결 오류를 방지하는 한 가지 방법은 Azure Cognitive Search에서 두 개 이상의 복제본과 하나의 파티션을 사용하는 것입니다. 이렇게 하면 비용이 늘어나지만 IP 주소 문제도 해결합니다. Azure Cognitive Search에서 IP 주소는 두 개 이상의 검색 단위가 있는 경우 변경되지 않습니다.
...
#### <a name="include-the-azure-cognitive-search-portal-ip-addresses"></a>Azure 인지 검색 포털 IP 주소 포함
Azure 포털을 사용하여 인덱서를 만드는 경우 Azure Cognitive Search 포털 논리는 생성 시간 동안 SQL Azure VM에 대한 액세스도 필요합니다. Azure 인지 검색 포털 IP 주소ping을 `stamp2.search.ext.azure.com`통해 찾을 수 있습니다.

## <a name="next-steps"></a>다음 단계
구성을 방해하지 않으면 Azure VM의 SQL Server를 Azure 인지 검색 인덱서의 데이터 원본으로 지정할 수 있습니다. 자세한 내용은 [인덱서를 사용하여 Azure SQL 데이터베이스에 Azure 인지 검색에 연결](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md) 을 참조하십시오.
