...
SDK의 다른 NuGet 패키지는 다음과 같습니다.
  ...
  - `Microsoft.Azure.Search.Service`: Azure Cognitive Search 인덱스, 동의어 맵, 인덱서, 데이터 원본 또는 기타 서비스 수준 리소스를 관리 하기 위해 .NET에서 자동화를 개발 하는 경우이 패키지를 사용 합니다. 인덱스에서 문서를 쿼리하거나 업데이트하기만 하면 되는 경우에는 대신 `Microsoft.Azure.Search.Data` 패키지를 사용합니다. Azure Cognitive Search의 모든 기능이 필요한 경우 `Microsoft.Azure.Search` 패키지를 대신 사용 합니다.
...
을 만든 후 `SearchServiceClient` 에는 `Main` "호텔" 인덱스가 이미 있는 경우이를 삭제 합니다. 이 삭제 작업은 다음 방법으로 수행 됩니다.
...
이 메서드는 네 부분으로 이루어져 있습니다. 첫 번째는 `Hotel` `Room` 인덱스에 업로드할 입력 데이터의 역할을 하는 개체 3 개를 포함 하는 3 개의 개체 배열을 만듭니다. 이 데이터는 간단히 하기 위해 하드 코딩합니다. 사용 중인 애플리케이션의 경우, 데이터는 SQL 데이터베이스와 같은 외부 데이터 원본에서 나올 가능성이 높습니다.

두 번째 부분은 문서를 포함하는 `IndexBatch`를 만듭니다. 배치를 만들 때 배치에 적용할 작업을 지정합니다. 이 경우에는 `IndexBatch.Upload`를 호출하여 지정합니다. 그런 다음 메서드를 통해 일괄 처리를 Azure Cognitive Search 인덱스에 업로드 합니다 `Documents.Index` .

> [!NOTE]
> 이 예에서는 문서를 업로드하는 것입니다. 변경 사항을 기존 문서에 병합하거나 문서를 삭제하려면 `IndexBatch.Merge`, `IndexBatch.MergeOrUpload` 또는 `IndexBatch.Delete`를 호출하여 배치를 만듭니다. 개체의 컬렉션을 사용 하는 개체의 컬렉션을 사용 하 여 여러 작업을 단일 일괄 처리로 혼합할 수도 있습니다 `IndexBatch.New` `IndexAction` . 각 개체는 Azure Cognitive Search 문서에 대해 특정 작업을 수행 하도록 지시 합니다. `IndexAction.Merge`, `IndexAction.Upload` 등을 비롯한 해당 메서드를 호출하여 각 `IndexAction`과 자체 작업을 만들 수 있습니다.
> 
> 
...
Azure Cognitive Search .NET SDK를 사용 하 여 인덱스와 같은 사용자 정의 클래스의 인스턴스를 업로드할 수 있는 방법에 대해 궁금할 수 있습니다 `Hotel` . 이 질문에 대답하기 위해 `Hotel` 클래스를 살펴보겠습니다.
...
먼저 클래스의 각 public 속성 이름이 `Hotel` 인덱스 정의에서 이름이 같은 필드에 매핑되는지 확인 해야 합니다. 각 필드를 소문자 ("카멜식 대/소문자")로 시작 하려는 경우 SDK에 속성 이름을 클래스의 특성과 함께 자동으로 카멜식 대/소문자에 매핑하도록 지시할 수 있습니다 `[SerializePropertyNamesAsCamelCase]` . 이 시나리오는 대상 스키마가 .NET의 "파스칼식 대/소문자" 명명 지침을 위반 하지 않고도 응용 프로그램 개발자의 제어를 벗어난 데이터 바인딩을 수행 하는 .NET 응용 프로그램에서 일반적입니다.
...
Azure Cognitive Search 인덱스에 매핑할 고유한 모델 클래스를 디자인 하는 경우 및와 같은 값 형식의 속성을 null 허용으로 선언 하는 것이 좋습니다 `bool` `int` (예: `bool?` 대신 `bool` ). Null이 허용되지 않는 속성을 사용하는 경우 인덱스의 문서가 해당 필드에 대해 Null 값을 포함하지 않도록 **보장** 해야 합니다. SDK와 Azure Cognitive Search 서비스는 모두이를 적용 하는 데 도움이 됩니다.
...
이 SDK는 문서를 직렬화 및 역직렬화하는 데 JSON.NET을 사용합니다. 필요에 따라 또는을 정의 하 여 serialization 및 deserialization을 사용자 지정할 수 있습니다 `JsonConverter` `IContractResolver` . 자세한 내용은 [JSON.NET 설명서](https://www.newtonsoft.com/json/help/html/Introduction.htm)를 참조 하세요. 이는 Azure Cognitive Search 및 기타 고급 시나리오에서 사용 하기 위해 응용 프로그램에서 기존 모델 클래스를 조정 하려는 경우에 유용 합니다. 예를 들어 사용자 지정 serialization으로 다음을 수행할 수 있습니다.
...
이 경우 검색 가능한 필드에서 "motel" 이라는 단어에 대 한 전체 인덱스를 검색 하 고 매개 변수에 지정 된 대로 호텔 이름만 검색 하려고 합니다 `Select` . 결과는 다음과 같습니다.
