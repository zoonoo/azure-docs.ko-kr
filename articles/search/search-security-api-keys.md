---
title: 관리자 및 쿼리 API 키 생성, 관리 및 보안
titleSuffix: Azure Cognitive Search
description: api 키는 서비스 끝점에 대한 액세스를 제어합니다. 관리자 키는 쓰기 액세스를 부여합니다. 쿼리 키는 읽기 전용 액세스에 대해서만 만들 수 있습니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="create-and-manage-api-keys-for-an-azure-cognitive-search-service"></a>Azure 인지 검색 서비스에 대한 API 키 생성 및 관리
...
> [!NOTE]  
>  요청 URI에서 `api-key`와 같은 중요한 데이터를 전달하는 낮은 수준의 보안 사례로 간주됩니다. 이러한 이유로 Azure Cognitive Search는 쿼리 문자열에서 `api-key` 쿼리 키로만 허용하므로 인덱스의 내용을 공개적으로 사용할 수 있어야 하지 않는 한 쿼리 키를 사용하지 않아야 합니다. 일반적으로 `api-key`를 요청 헤더로 전달하는 것이 좋습니다.  
...
3. 서비스 및 개요 페이지에서 **설정 키를** >**Keys** 클릭하여 관리자 및 쿼리 키를 봅니다.

   ![포털 페이지, 설정, 키 섹션](media/search-security-overview/settings-keys.png)

## <a name="create-query-keys"></a>쿼리 키 만들기

쿼리 키는 문서 컬렉션을 대상으로 하는 작업에 대한 인덱스 내의 문서에 대한 읽기 전용 액세스에 사용됩니다. 검색, 필터링 및 제안 쿼리는 쿼리 키를 취하는 모든 작업입니다. 시스템 데이터 또는 인덱스 정의 또는 인덱서 상태와 같은 개체 정의를 반환하는 읽기 전용 작업에는 관리자 키가 필요합니다.

클라이언트 앱에서 액세스 및 작업을 제한하는 것은 서비스에서 검색 자산을 보호하는 데 필수적입니다. 클라이언트 앱에서 시작된 쿼리에 대해 항상 관리자 키가 아닌 쿼리 키를 사용합니다.

1. [Azure 포털에](https://portal.azure.com)로그인합니다.
2. 구독에 대한 [검색 서비스](https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices)를 나열합니다.
3. 서비스를 선택하고 개요 페이지에서 **설정** >**키를 클릭합니다.**
4. **쿼리 키 관리를 클릭합니다.**
5. 서비스에 대해 이미 생성된 쿼리 키를 사용하거나 최대 50개의 새 쿼리 키를 만듭니다. 기본 쿼리 키의 이름은 지정되지 않지만 관리용을 위해 추가 쿼리 키의 이름을 지정할 수 있습니다.

   ![쿼리 키 만들기 또는 사용](media/search-security-overview/create-query-key.png) 

> [!Note]
> 쿼리 키 사용량을 보여 미치는 코드 예제는 C# 의 [Azure 인지 검색 인덱스 쿼리에서](search-query-dotnet.md)찾을 수 있습니다.

<a name="regenerate-admin-keys"></a>

## <a name="regenerate-admin-keys"></a>관리자 키 다시 생성

비즈니스 연속성을 위해 보조 키를 사용하여 기본 키를 회전할 수 있도록 각 서비스에 대해 두 개의 관리 키가 만들어집니다.
...
실수로 두 키를 동시에 다시 생성하는 경우 해당 키를 사용하는 모든 클라이언트 요청은 HTTP 403 금지됨으로 실패합니다. 그러나 콘텐츠는 삭제되지 않으며 영구적으로 잠기지 않습니다. 

포털 또는 관리[계층(REST API,](https://docs.microsoft.com/rest/api/searchmanagement/) [PowerShell](https://docs.microsoft.com/azure/search/search-manage-powershell)또는 Azure 리소스 관리자)을 통해 서비스에 계속 액세스할 수 있습니다. 관리 기능은 서비스 API 키가 아닌 구독 ID를 통해 작동하므로 API 키가 없는 경우에도 계속 사용할 수 있습니다. 

포털 또는 관리 계층을 통해 새 키를 만든 후 새 키가 있고 요청에 해당 키를 제공하면 콘텐츠(인덱스, 인덱서, 데이터 원본, 동의어 맵)에 대한 액세스가 복원됩니다.
...
+ [Azure 인지 검색의 역할 기반 액세스 제어](search-security-rbac.md)
