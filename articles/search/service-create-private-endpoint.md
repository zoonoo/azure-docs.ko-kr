...
1. VM에서 검색 서비스에 연결 하 여 인덱스를 만듭니다. 이 [빠른](search-get-started-rest.md) 시작을 따라 REST API를 사용 하 여 서비스에서 새 검색 인덱스를 만들 수 있습니다. 웹 API 테스트 도구에서 요청을 설정 하려면 검색 서비스 끝점 (https://[search service name]. 검색. w i n d)과 이전 단계에서 복사한 관리 API 키가 필요 합니다.
