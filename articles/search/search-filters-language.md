...
다국어 검색 애플리케이션의 핵심 요구 사항은 사용자의 언어로 검색하고 결과를 볼 수 있는 기능입니다. Azure Cognitive Search에서 다국어 앱의 언어 요구 사항을 충족 하는 한 가지 방법은 특정 언어로 문자열을 저장 하는 전용 필드를 만든 다음 쿼리 시 전체 텍스트 검색을 해당 필드로 제한 하는 것입니다.
...
이 기술의 성공은 필드 콘텐츠의 무결성에 달려 있습니다. Azure Cognitive Search는 문자열을 변환 하거나 언어 검색을 수행 하지 않습니다. 필드에 예상하는 문자열이 포함되어 있는지 확인하는 것은 사용자의 몫입니다.
