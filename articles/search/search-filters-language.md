---
title: 검색 인덱스의 언어별 필터링
titleSuffix: Azure Cognitive Search
description: 다국어 검색을 지원하기 위해 조건을 필터링하여 쿼리 실행을 언어별 필드로 범위를 지정합니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="how-to-filter-by-language-in-azure-cognitive-search"></a>Azure 인지 검색에서 언어별로 필터링하는 방법 

다국어 검색 애플리케이션의 핵심 요구 사항은 사용자의 언어로 검색하고 결과를 볼 수 있는 기능입니다. Azure Cognitive Search에서 다국어 앱의 언어 요구 사항을 충족하는 한 가지 방법은 특정 언어로 문자열을 저장하는 데 전념하는 일련의 필드를 만든 다음 쿼리 시 해당 필드로만 전체 텍스트 검색을 제한하는 것입니다.
...
이 기술의 성공은 필드 콘텐츠의 무결성에 달려 있습니다. Azure Cognitive Search는 문자열을 번역하거나 언어 검색을 수행하지 않습니다. 필드에 예상하는 문자열이 포함되어 있는지 확인하는 것은 사용자의 몫입니다.

## <a name="define-fields-for-content-in-different-languages"></a>콘텐츠 필드를 다양한 언어로 정의

Azure 인지 검색에서 쿼리는 단일 인덱스를 대상으로 합니다. 단일 검색 환경에서 언어별 문자열을 제공하려는 개발자는 일반적으로 값을 저장하는 전용 필드(영어 문자열 필드 하나, 프랑스 문자열 필드 하나 등)를 정의합니다. 
