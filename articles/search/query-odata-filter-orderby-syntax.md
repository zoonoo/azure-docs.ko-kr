...
식별자는 필터에서 필드 이름 또는 [컬렉션 식](search-query-odata-collection-operators.md) (`any` 또는 `all`) 컨텍스트의 **범위 변수에** 대 한 참조를 참조할 수 있습니다. 범위 변수는 컬렉션의 현재 요소를 나타내는 루프 변수와 같습니다. 복합 컬렉션의 경우 해당 변수는 개체를 나타내므로 필드 경로를 사용 하 여 변수의 하위 필드를 참조할 수 있습니다. 이는 대부분의 프로그래밍 언어에서 점 표기법과 유사 합니다.
...
| `Address/City` | 인덱스에 있는 복합 필드의 `City` 하위 필드를 참조 합니다. 이 예에서는 `Address` 형식 `Edm.ComplexType`입니다. |
| `Rooms/Type` | 인덱스에 있는 복합 컬렉션 필드의 `Type` 하위 필드를 참조 합니다. 이 예에서는 `Rooms` 형식 `Collection(Edm.ComplexType)`입니다. |
| `Stores/Address/Country` | 인덱스에 있는 복합 컬렉션 필드의 `Address` 하위 필드에 대 한 `Country` 하위 필드를 참조 합니다. 이 예에서는 `Stores` 형식이 `Collection(Edm.ComplexType)`이 고 `Address` `Edm.ComplexType` 형식입니다. |
| `room/Type` | 는 필터 식에서와 같이 `room` 범위 변수의 `Type` 하위 필드를 참조 `Rooms/any(room: room/Type eq 'deluxe')` |
| `store/Address/Country` | 필터 식에서와 같이 `store` 범위 변수의 `Address` 하위 필드에 대 한 `Country` 하위 필드를 참조 `Stores/any(store: store/Address/Country eq 'Canada')` |
...
`Address/City`필드 경로를 고려 합니다. 필터에서이는 "샌프란시스코"와 같이 현재 문서에 대 한 단일 도시를 나타냅니다. 이와 대조적으로 `Rooms/Type`는 여러 대화방 (예: "standard", 두 번째 방에 대 한 "deluxe")에 대 한 `Type` 하위 필드를 참조 합니다. `Rooms/Type`는 하위 필드 `Type`의 *단일 인스턴스* 를 참조 하지 않으므로 필터에 직접 사용할 수 없습니다. 대신, 방 유형을 필터링 하려면 다음과 같이 범위 변수와 함께 [람다 식을](search-query-odata-collection-operators.md) 사용 합니다.

    Rooms/any(room: room/Type eq 'deluxe')

이 예에서는 범위 변수 `room` `room/Type` 필드 경로에 표시 됩니다. 이런 방식으로 `room/Type`는 현재 문서에서 현재 대화방의 유형을 나타냅니다. 이는 `Type` 하위 필드의 단일 인스턴스이며 필터에 직접 사용할 수 있습니다.
...
필드 경로와 상수는 OData 식의 가장 기본적인 부분 이지만 이미 전체 식입니다. 실제로 Azure Cognitive Search의 **$select** 매개 변수는 아무것도 아니지만 쉼표로 구분 된 필드 경로 목록이 며 **$orderby** 는 **$select**보다 훨씬 복잡 하지 않습니다. 인덱스에 `Edm.Boolean` 형식의 필드가 있으면 해당 필드의 경로는 아니지만 필터를 작성할 수 있습니다. 상수 `true` 및 `false`는 마찬가지로 유효한 필터입니다.
