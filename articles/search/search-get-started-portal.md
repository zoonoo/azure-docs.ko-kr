...
이 자습서에서는 **데이터 가져오기** 마법사를 통해 [*인덱서*](search-indexer-overview.md)를 사용하여 크롤링할 수 있는 기본 제공 샘플 데이터 집합을 사용합니다. 인덱서는 지원되는 Azure 데이터 원본에서 메타데이터 및 콘텐츠를 읽을 수 있는 원본 관련 크롤러입니다. 이러한 인덱서는 **데이터 가져오기** 마법사를 통해 포털에 표시됩니다. 나중에 프로그래밍 방식으로 인덱스를 독립 리소스로 만들고 관리할 수 있습니다.
