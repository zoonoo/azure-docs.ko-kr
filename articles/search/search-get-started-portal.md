...
ms.locfileid: "39007987"
---
# <a name="tutorial-use-built-in-tools-for-azure-search-indexing-and-queries"></a>자습서: Azure Search 인덱싱 및 쿼리에 기본 제공 도구 사용

Azure portal의 Azure Search 서비스 페이지에서 개념 테스트에 대한 기본 제공 도구 및 최소한의 증가로 실습 경험을 사용할 수 있습니다. 포털 도구는 .NET 및 REST API로 완벽한 패리티를 제공하지 않지만 빠른 개념 증명 테스트를 위해 마법사 및 편집기에서 손쉬운 지원을 제공합니다. 이 코드 없는 소개문을 통해 흥미로운 쿼리를 바로 작성할 수 있도록 소규모의 게시된 데이터 집합을 시작합니다. 
...
포털 도구는 전체 Azure Search 기능을 지원하지 않습니다. 도구가 너무 제한적인 경우 [.NET에서 Azure Search 프로그래밍에 대한 코드 기반 소개](search-howto-dotnet-sdk.md) 또는 [REST API 호출에 대한 웹 테스트 도구](search-fiddler.md)를 고려합니다.

Azure 구독이 아직 없는 경우 시작하기 전에 [무료 계정](https://azure.microsoft.com/free/?WT.mc_id=A261C142F)을 만듭니다. 이 [Azure Search 개요 비디오](https://channel9.msdn.com/Events/Connect/2016/138)를 시작하고 약 3분 후에 시작하는 이 자습서의 단계에 대한 6분짜리 데모 영상을 시청하셔도 됩니다.
...
특정 검색 동작을 최적화하는 데 사용되는 검색 가능한 데이터, 메타데이터 및 구문을 포함하는 [*인덱스*](search-what-is-an-index.md)에 검색 쿼리가 반복됩니다.

이 작업을 계속 포털 기반으로 유지하기 위해 **데이터 가져오기** 마법사를 통해 [*인덱서*](search-indexer-overview.md)를 사용하여 크롤링할 수 있는 기본 제공 샘플 데이터 집합을 사용하겠습니다. 인덱서는 지원되는 Azure 데이터 원본에서 메타데이터 및 콘텐츠를 읽을 수 있는 원본 관련 크롤러입니다. 코드에서 독립적인 리소스로 인덱스를 작성 및 관리할 수 있습니다. 포털에서 인덱서는 **데이터 가져오기** 마법사를 통해 노출됩니다. 
...
**데이터 가져오기**는 인덱싱에 AI 알고리즘을 추가하는 선택 사항 인식 기술을 단계를 포함합니다. 이 기능은 이 자습서에서 다루지 않으므로 **대상 인덱스 사용자 지정**으로 건너뛰어야 합니다. Azure Search에서 새 인식 검색 미리 보기 기능에 대해 궁금한 경우 [인식 검색 빠른 시작](cognitive-search-quickstart-blob.md) 또는 [자습서](cognitive-search-tutorial-blob.md)를 시도합니다.
...
서비스 대시보드의 타일은 요약 정보 뿐만 아니라 자세한 정보에 대한 액세스를 제공합니다. 예를 들어 **인덱스** 타일에서 이전 단계에서 만든 *realestate-us-sample* 인덱스를 포함한 기존 인덱스의 목록이 표시됩니다.
...
+ **검색 탐색기**는 문서 구조가 조밀하면 장황하고 읽기 어려운 JSON으로 결과를 반환합니다. 이는 의도적인 것입니다. 전체 문서의 가시성은 특히 테스트 중에 중요한 사용 사례입니다. 더 나은 사용자 환경을 위해 중요한 요소를 끌어내도록 [검색 결과를 처리](search-pagination-page-layout.md)하는 코드를 작성해야 합니다.
...
+  **$count=true** 매개 변수는 반환된 총 문서 수를 반환합니다. 이 값은 검색 결과의 위쪽에 나타납니다. **$count=true**에서 보고하는 변경 내용을 모니터링하여 필터 쿼리를 확인할 수 있습니다. 더 작은 수는 필터가 작동하는 것을 나타냅니다.
...
이 자습서에서는 Azure Portal에서 **데이터 가져오기** 마법사 및 **검색 탐색기**를 사용하는 기본 단계를 설명합니다.

데이터 가져오기 마법사의 도입으로 [게시된 인덱스에 대한 지원되는 수정](ttps://docs.microsoft.com/rest/api/searchservice/update-index)을 포함하여 [인덱서](search-indexer-overview.md) 및 인덱스 디자인에 대한 기본 워크플로를 알아보았습니다. 

필터, 적중 항목 강조 표시, 유사 항목 검색 및 지리적 검색 등의 주요 기능을 보여주는 간단한 예제를 통해 쿼리 구문을 알아보았습니다.

마지막으로 모든 인덱스, 인덱서 또는 구독에 대해 만드는 데이터 원본에 대한 대시보드의 타일을 클릭하여 정보를 가져오는 방법을 알아보았습니다. 나중에 고유한 인덱스 또는 동료가 생성한 인덱스를 작업할 때 알 수 없는 코드를 통해 검색할 필요 없이 포털을 사용하여 손쉽게 데이터 원본 정의 또는 필드 컬렉션의 생성을 확인할 수 있습니다.
...
Azure Search의 추가 도구 기반 탐색은 Postman 또는 Fiddler와 같은 REST 테스트 도구를 사용하는 것이 좋습니다.

> [!div class="nextstepaction"]
> [Azure Search REST API 호출을 위한 웹 테스트 도구](search-fiddler.md)
