---
title: "Azure Search 시작 | Microsoft Docs"
...
---
# <a name="get-started-with-azure-search-in-the-portal"></a>포털에서 Azure 검색 시작
...
많은 고객이 무료 서비스를 시작합니다. 이 버전은&3;개의 인덱스,&3;개의 데이터 소스 및&3;개의 인덱서로 제한됩니다. 시작하기 전에 추가 항목에 대한 공간이 있는지 확인합니다. 이 연습은 각 개체를 하나씩 만듭니다.

## <a name="create-an-index-and-load-data"></a>인덱스 및 부하 데이터 만들기
...
#### <a name="step-1-define-the-data-source"></a>1단계: 데이터 원본 정의
...
3. 검색 상자를 빈 상태로 두고 **검색** 단추를 클릭하여 모든 문서를 반환하는 와일드카드 검색을 실행합니다.
4. 전체 텍스트 검색 쿼리를 몇 개 입력합니다. 와일드카드 검색에서 결과를 검토하여 쿼리할 예술가, 앨범 및 장르에 친숙해 질 수 있습니다.
5. 아이디어에 대한 [이 문서의 끝에서 제공되는 예제](https://msdn.microsoft.com/library/azure/dn798927.aspx) 를 사용하여 다른 쿼리 구문을 시도하며 이는 쿼리를 수정하여 인덱스에서 찾을 가능성이 있는 검색 문자열을 사용합니다.
...
> [!NOTE]
> 새로 발표된 기능은 Azure Blob 저장소에 대한 인덱서 지원이지만 해당 기능은 미리 보기 상태이며 아직 포털 옵션은 아닙니다. 해당 인덱서를 시도하려면 코드를 작성해야 합니다. 자세한 내용은 [Azure 검색에서 Azure Blob 저장소 인덱싱](search-howto-indexing-azure-blob-storage.md) 을 참조하세요.
> <a id="apdx-sampledata"></a>
> 
> 

## <a name="appendix-create-sample-data-in-documentdb"></a>부록: DocumentDB에서 샘플 데이터 만들기
...
### <a name="create-musicstoredb-for-this-tutorial"></a>이 자습서를 위해 musicstoredb 만들기
...
3. **데이터베이스 추가**를 클릭하여 `musicstoredb`의 ID로 새 데이터베이스를 만듭니다. 생성된 후에 페이지 아래쪽에 있는 데이터베이스 타일에 표시됩니다.
4. 데이터베이스 이름을 클릭하여 데이터베이스 블레이드를 엽니다.
5. **컬렉션 추가**를 클릭하여 `musicstorecoll`의 ID로 컬렉션을 만듭니다.
6. **문서 탐색기**를 클릭합니다.
7. **업로드**를 클릭합니다.
8. **문서 업로드**에서 이전에 다운로드한 JSON 파일이 들어 있는 로컬 폴더로 이동합니다. 100개 항목 이하의 일괄 처리를 통해 JSON 파일을 업로드합니다.
   * 386.json
   * 387.json
   * . . .
   * 486.json
9. 마지막 항목인 669.json이 업로드될 때까지 다음 파일 일괄 처리 가져오기를 반복합니다.
10. **쿼리 탐색기** 를 클릭하여 데이터 업로드가 문서 탐색기의 업로드 요구 사항을 충족하는지 확인합니다.
