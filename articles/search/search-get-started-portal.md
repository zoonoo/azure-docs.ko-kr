<properties 
	pageTitle="Azure 검색 시작 | Microsoft Azure | Azure 검색 시작 | DocumentDB | 클라우드 검색 서비스" 
...
많은 고객이 무료 서비스를 시작합니다. 이 버전은 3개의 인덱스, 3개의 데이터 소스 및 3개의 인덱서로 제한됩니다. 시작하기 전에 추가 항목에 대한 공간이 있는지 확인합니다. 이 연습은 각 개체를 만듭니다.

## 인덱스 및 부하 데이터 만들기

특정 검색 동작을 최적화하는 데 사용되는 검색 가능한 데이터, 메타데이터 및 구문을 포함하는 *인덱스*에 검색 쿼리가 반복됩니다. 첫 번째 단계로 인덱스를 정의 및 구성합니다.
...
- **조회 가능**은 결과 목록 검색에 표시된다는 의미입니다. 예를 들어 필드가 필터 식에만 사용되는 경우 이 확인란을 지워 검색 결과에 대한 제한 해제로 개별 필드를 표시할 수 있습니다. 
...
1. [여기 클릭](https://github.com/HeidiSteen/azure-search-get-started-sample-data)하여 음악 스토어 JSON 데이터 파일이 포함된 ZIP 파일을 다운로드합니다. 이 데이터 집합에 대한 246 JSON 문서를 제공합니다.
...
3. **문서 탐색기**를 클릭합니다.
4. **문서 추가**를 클릭합니다.
5. **문서 추가**에서, 100회 이하의 일괄 처리를 통해 JSON 파일을 업로드합니다.
	- 386\.json
	- 387\.json
	- . . .
6. **쿼리 탐색기**를 클릭하여 데이터 업로드가 문서 탐색기의 업로드 요구 사항을 충족하는지 확인합니다.

이 작업을 수행하는 쉬운 방법은 기본 쿼리를 수정하여 상위 300개(이 데이터 집합에 300개 이하의 항목)를 선택하거나 `select * from musicstorecoll`을 작성한 다음 **쿼리 실행**을 클릭하는 것입니다.

문서 번호 386부터 시작하여 669까지의 JSON 출력을 다시 가져와야 합니다. 데이터가 로드되면 [이 연습의 단계로 돌아가서](#defineDS) **가져오기 마법사**를 사용하여 인덱스를 작성할 수 있습니다.
