...
---
# <a name="example-multi-level-facets-in-azure-cognitive-search"></a>예: Azure 인지 검색의 다단계 측면

Azure Cognitive Search 스키마는 다단계 분류 범주를 명시적으로 지원하지 않지만 인덱싱하기 전에 콘텐츠를 조작한 다음 결과에 특수 처리를 적용하여 범주를 근사화할 수 있습니다. 

## <a name="start-with-the-data"></a>데이터 시작

이 문서의 예제는 Azure 인지 검색에서 다단계 면면을 보여 주기 위해 이전 예제인 [AdventureWorks 인벤토리 데이터베이스 모델을](search-example-adventureworks-modeling.md)기반으로 합니다.
...
이 구조를 포함하는 인덱스에서 Azure Cognitive Search 스키마에 **컬렉션(Edm.String)** 필드를 만들어 이 데이터를 저장하여 필드 특성에 검색, 필터링, 얼굴 테이블 및 검색이 포함되도록 합니다.
...
count 값은 가능한 모든 분류 값을 반환할 수 있도록 충분히 높아야 합니다. AdventureWorks 데이터는 고유한 분류 값 41개를 포함하므로 `count:50`이면 충분합니다.
  ...
  ![면 필터](./media/search-example-adventureworks/facet-filter.png "면 필터")
...
이제 **범주** 개체를 사용하여 축소 가능한 분류 트리를 정확한 카운트 값으로 렌더링할 수 있습니다.
  ...
  ![다단계 면 필터](./media/search-example-adventureworks/multi-level-facet.png "다단계 면 필터")

 
트리의 각 링크는 관련 필터를 적용해야 합니다. 예를 들어:

+ **분류/액세서리** `(x:x eq 'Accessories')` 지점의 모든 문서를 반환합니다.
+ **분류/모든** `(x:x eq 'Accessories|Bike Racks')` 액세서리 지점 아래의 자전거 랙 하위 범주가 있는 문서만 반환합니다.
...
[예: Azure 인지 검색을 위한 AdventureWorks 인벤토리 데이터베이스 모델링](search-example-adventureworks-modeling.md)