...
복합 필드는 데이터 형식에 따라 문서의 단일 개체 또는 개체의 배열을 나타냅니다. 형식의 필드는 `Edm.ComplexType` 단일 개체를 나타내고, 형식의 `Collection(Edm.ComplexType)` 필드는 개체의 배열을 나타냅니다.
...
다음 JSON 문서는 간단한 필드와 복잡 한 필드로 구성 됩니다. 및 `Address` `Rooms`와 같은 복합 필드에는 하위 필드가 있습니다. `Address`에는 문서의 단일 개체 이므로 해당 하위 필드에 대 한 단일 값 집합이 있습니다. 반면에는 `Rooms` 컬렉션의 각 개체에 대해 하나씩, 하위 필드에 대 한 값 집합이 여러 개 있습니다.
...
또는 `Edm.GeographyPoint` `Collection(Edm.GeographyPoint)`형식이 아닌 경우에는 모든 하위 필드를 패싯 가능으로 표시할 수 있습니다.
...
최상위 단순 필드와 마찬가지로, 인덱스 정의에서 **필터링** 가능한 특성이로 `true` 설정 된 경우에만 복합 필드의 단순한 하위 필드를 필터에 포함할 수 있습니다. 자세한 내용은 [인덱스 API 참조 만들기](/rest/api/searchservice/create-index)를 참조 하세요.
