...
데이터 원본은 인덱싱할 데이터, 데이터에 액세스하는 데 필요한 자격 증명 및 Azure Cognitional Search가 데이터의 변경 사항을 효율적으로 식별할 수 있도록 하는 정책을 지정합니다.
...
이 인덱서는 2시간 간격으로 실행됩니다 (일전 간격이 "PT2H"로 설정됩니다.) 30분마다 인덱서를 실행하려면 간격을 "PT30M"으로 설정합니다. 지원되는 가장 짧은 간격은 5분입니다. 일정은 선택 사항입니다. 생략하는 경우 인덱서는 만들어질 때 한 번만 실행됩니다. 그러나 언제든지 필요할 때 인덱서를 실행할 수 있습니다.   
...
## <a name="help-us-make-azure-cognitive-search-better"></a>Azure Cognitive Search 개선 지원
