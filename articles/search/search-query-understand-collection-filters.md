...
Azure Cognitive Search에서 컬렉션 필드를 [필터링](query-odata-filter-orderby-syntax.md) 하려면 [ `any` 및 `all` 연산자](search-query-odata-collection-operators.md) 를 **람다 식**과 함께 사용할 수 있습니다. 람다 식은 **범위 변수**를 참조 하는 부울 식입니다. 및 연산자는 루프 변수의 역할을 `for` 취하는 범위 변수와 루프의 본문으로 사용 되는 람다 식을 사용 하는 대부분의 프로그래밍 언어의 루프와 유사 합니다. `all` `any` 범위 변수는 루프를 반복 하는 동안 컬렉션의 "current" 값을 사용 합니다.

개념적으로 작동 하는 방법은 이상입니다. 실제로 Azure Cognitive Search는 루프의 작동 방식 `for` 에 대해 매우 다양 한 방법으로 필터를 구현 합니다. 이러한 차이는 사용자에 게 표시 되지 않지만 특정 상황에서는 표시 되지 않습니다. 최종 결과에는 람다 식을 작성할 때 따라야 하는 규칙이 있습니다.
...
1. 특정 데이터 형식에 대해서는 특정 연산자만 지원 됩니다. 예를 들어, 부울 `true` 값을 비교 하 고 `false` , `lt` `gt`등을 사용 하는 것은 의미가 없습니다.
1. Azure Cognitive Search는 형식의 **correlated search** `Collection(Edm.ComplexType)`필드에 대 한 상관 관계 검색을 지원 하지 않습니다.
...
위의 `Type` 필터와는 달리, 기본적으로 "대화방이 ' Deluxe '과 같은 공간이 있고 **동일한 공간이** `BaseRate` 100 미만입니다." 문서와 일치 하는 경우 검색 쿼리는 "Deluxe" 라는 용어를 `Rooms/Type` 포함 하 고 `Rooms/Description` "city view" 라는 구를 포함 하는 "일치 문서"를 의미 합니다. 후자의 경우에는 필드의 상관 관계를 지정할 수 있는 개별 대화방의 개념이 없습니다.
...
`Collection(Edm.Int32)`, `Collection(Edm.GeographyPoint)`등의 간단한 컬렉션 보다 복잡 한 컬렉션에 비해 람다 식에 대 한 제한이 훨씬 적음을 발견할 수 있습니다. Azure Cognitive Search는 복합 컬렉션을 하위 문서의 실제 컬렉션으로 저장 하는 반면 간단한 컬렉션은 컬렉션으로 저장 되지 않기 때문입니다.
...
필드의 값은 다음과 같이 반전 된 인덱스 라는 구조에 저장 됩니다. **inverted index** `seasons`
...
이 데이터 구조는 지정 된 용어가 표시 되는 문서에 대해 좋은 속도로 한 질문에 대답 하도록 디자인 되었습니다. 이 질문에 대 한 대답은 컬렉션에 대 한 루프와는 다른 일반적인 같음 검사 처럼 작동 합니다. 실제로 문자열 컬렉션의 경우 Azure Cognitive Search는의 `eq` `any`람다 식 내에서 비교 연산자로만 허용 됩니다.
...
따라서 및 `all` `ne` `and`와 함께를 사용할 수 있습니다.
...
예 `lt`를 들어, `gt` `le`, 및 `ge` 연산자 `Collection(Edm.Int32)` 를 지 원하는 데이터 형식의 컬렉션을 필터링 하는 경우 더 광범위 한 식이 허용 됩니다. 특히를 사용 하 여 `and` 기본 비교 식이 `or` **범위 비교** 로 결합 된 다음 `and`를 사용 하 여 `or`추가로 결합 되는 한을 사용할 `any`수 있습니다. 이러한 부울 식의 구조는 [DNF (분리 Normal Form)](https://en.wikipedia.org/wiki/Disjunctive_normal_form)라고 하며, 그렇지 않은 경우에는 "ORs of ANDs" 이라고 합니다. 반대로 이러한 데이터 형식 `all` 에 대 한 람다 식은 [My.cnf (결합 Normal Form)](https://en.wikipedia.org/wiki/Conjunctive_normal_form)에 있어야 합니다. 그렇지 않으면 "ANDs of ORs"로 알려져 있습니다. Azure Cognitive Search는 문자열에 대 한 빠른 용어 조회를 수행할 수 있는 것 처럼 반전 인덱스를 사용 하 여 효율적으로 실행할 수 있으므로 이러한 범위 비교를 허용 합니다.

요약 하자면, 람다 식에서 허용 되는 항목에 대 한 다음과 같은 규칙이 있습니다.

- 내 `any`에서 *긍정 검사* `geo.intersects`는 같음, 범위 비교, 또는 or `geo.distance` `lt` `le` 와 비교 하 여 항상 허용 됩니다 (거리를 확인 하는 경우 "거리"은 같음 같음).
