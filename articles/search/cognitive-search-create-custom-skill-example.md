...
ms.locfileid: "66428444"
---
# <a name="example-create-a-custom-skill-using-the-text-translate-api"></a>예제: Translator Text API를 사용하여 사용자 지정 기술 만들기

이 예제에서는 웹 API 사용자 지정 기술을 만드는 방법에 알아봅니다. 이 기술은 모든 언어의에서 텍스트를 받아들이고 영어로 변환 합니다. 예제에서는 [Azure Function](https://azure.microsoft.com/services/functions/)을 통해 [Translator Text API](https://azure.microsoft.com/services/cognitive-services/translator-text-api/)를 래핑하여 사용자 지정 기술 인터페이스를 구현하도록 합니다.
...
**F5** 키를 눌러 프로그램을 실행하고 함수 동작을 테스트합니다. 이 경우 영어를 스페인어로 텍스트 번역을 아래 함수를 사용 하겠습니다. Postman 또는 Fiddler를 사용하여 아래와 같은 호출을 실행합니다.
...
1. 화면에 표시되는 메시지를 따릅니다. Azure 계정, 리소스 그룹, 호스팅 계획 및 사용 하려는 저장소 계정을 지정 하려면 묻는 메시지가 나타납니다. 아직 없는 경우 새 리소스 그룹, 새 호스팅 플랜 및 저장소 계정을 만들 수 있습니다. 작업을 마쳤으면 **만들기**를 선택합니다.
...
이 예제에서는 로컬 환경에서 함수를 실행 하는 경우 이전에 본 것에 비슷한 결과 생성 해야 합니다.
