...
검색 서비스가 더 이상 필요하지 않은 경우 가장 빠른 서비스 릴리스 방법은 Azure Search 서비스를 포함하는 리소스 그룹을 삭제하는 것입니다. 리소스 그룹을 영구적으로 삭제하면 해당 그룹의 모든 항목(서비스 및 저장된 모든 콘텐츠 포함)이 삭제됩니다. 포털에서 리소스 그룹 이름은 각 서비스의 개요 페이지에 있습니다.
