...
> [AZURE.NOTE]서비스 수준 계약 및 확장성은 표준 서비스의 기능이라는 점에 주의해야 합니다. 무료 서비스는 복제본과 파티션을 여러 구독자가 공유하는 고정 리소스 수준에서 제공됩니다. 무료 서비스로 시작한 후 업그레이드하려는 경우에는 표준 수준에서 새 Azure 검색 서비스를 만들고 인덱스와 데이터를 새로운 서비스에 다시 로드해야 합니다. 서비스 프로비전에 대한 자세한 내용은 [포털에서 Azure 검색 서비스 만들기](search-create-portal.md)를 참조하세요.
