...
| 보안 계층 | 설명 |
|----------------|-------------|
| 전송 중 암호화 <br>(HTTPS/SSL/TLS) | Azure 인지 검색HTTPS 포트 443에서 수신됩니다. 플랫폼 전체에서 Azure 서비스에 대한 연결이 암호화됩니다. <br/><br/>모든 클라이언트 간 Azure 인지 검색 상호 작용은 SSL/TLS 1.2 암호화를 사용합니다. 이전 버전(1.0 또는 1.1)은 지원되지 않습니다.|
...
당신은 공공 서비스가 있고 서비스의 사용을 제한하려는 경우 관리 REST API 버전에서 IP 제한 규칙을 사용할 수 있습니다 : 2020-03-13, [IpRule](https://docs.microsoft.com/rest/api/searchmanagement/2019-10-01-preview/createorupdate-service#IpRule). IpRule을 사용하면 검색 서비스에 대한 액세스 권한을 부여하려는 IP 주소를 개별적으로 또는 범위별로 식별하여 서비스에 대한 액세스를 제한할 수 있습니다. 
