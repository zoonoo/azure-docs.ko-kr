...
*Ai 보강* 는 사용자가 제공 하는 Microsoft 또는 사용자 지정 ai의 기본 제공 ai를 통합 하는 Azure Cognitive Search 인덱싱 아키텍처의 일부입니다. Blob을 처리 해야 하는 종단 간 시나리오를 구현 하는 데 도움이 됩니다. 여기에는 blob (기존 항목과 새 항목 모두)을 처리 하 고, 이미지와 텍스트를 추출 하는 모든 파일 형식을 열고, 다양 한 AI 기능을 사용 하 여 원하는 정보를 추출 하 고, 빠른 검색, 검색 및 탐색을 위해 검색 인덱스에서 인덱스를 인덱싱합니다. 
