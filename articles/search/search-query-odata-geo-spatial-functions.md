...
> [!NOTE]
> 필터에서 `geo.distance` 를 사용 하는 경우,, 또는 `ge`를 `le` `gt`사용 하 여 `lt`함수에서 반환 된 거리와 상수를 비교 해야 합니다. 연산자 `eq` 및 `ne`는 거리를 비교할 때 사용할 수 없습니다. 예를 들어,의 `geo.distance` `$filter=geo.distance(location, geography'POINT(-122.131577 47.678581)') le 5`올바른 사용법은입니다.

### <a name="geointersects"></a>geo.intersects

함수 `geo.intersects` 는 형식의 `Edm.GeographyPoint` 변수와 상수 `Edm.GeographyPolygon` 를 사용 하 고 점이 다각형 `true` `Edm.Boolean` 의범위 --  내에 있으면를 반환 하 고 그렇지않으면를반환합니다.`false`
...
Azure Search에서 180도 경도를 포함 하는 지리적 공간 쿼리는 쿼리 셰이프가 사각형이 고 좌표가 경도 및 위도 (예: `geo.intersects(location, geography'POLYGON((179 65, 179 66, -179 66, -179 65, 179 65))'`)를 따라 모눈 레이아웃에 맞춰 정렬 된 경우 예상 대로 작동 합니다. 그렇지 않은 경우 사각형이 아닌 도형이나 정렬되지 않은 도형의 경우 분할된 다각형 접근 방법을 고려해야 합니다.  

### <a name="geo-spatial-functions-and-null"></a>지리적 공간 함수 및`null`

Azure Search의 다른 모든 비 컬렉션 필드와 마찬가지로 형식의 `Edm.GeographyPoint` 필드에는 값이 포함 `null` 될 수 있습니다. Azure Search가 인 `geo.intersects` `null`필드를 평가 하는 경우 결과 `false`는 항상입니다. 이 경우의 `geo.distance` 동작은 컨텍스트에 따라 달라 집니다.

- 필터 `geo.distance` 에서 필드의 결과 `null` `null`는입니다. 즉, null이 아닌 값이로 `null` `false`계산 되기 때문에 문서가 일치 하지 않습니다.
