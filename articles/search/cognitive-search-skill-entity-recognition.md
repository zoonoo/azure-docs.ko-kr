...
|includeTypelessEntities | 현재 범주 `true` 에 맞지 않는 잘 알려진 엔터티를 인식 하려면로 설정 합니다. 인식 된 엔터티는 `entities` 복합 출력 필드에서 반환 됩니다. 예를 들어 "Windows 10"은 잘 알려진 엔터티 (a 제품) 이지만 "제품"은 지원 되는 범주가 아니므로이 엔터티는 엔터티 출력 필드에 포함 됩니다. 기본값은 `false` |
...
> [!NOTE]
> 모든 엔터티 범주가 모든 언어로 지원되는 것은 아닙니다. `"Person"`, `"Location"`및 `"Organization"` 엔터티 범주 형식은 위의 전체 언어 목록에 대해 지원 됩니다. _De_, _en_, _es_, _fr_및 _zh-cn hans_ 만 `"Quantity"` `"Datetime"`,, `"URL"`및 `"Email"` 형식의 추출을 지원 합니다. 자세한 내용은 [텍스트 분석 API에 대 한 언어 및 지역 지원](https://docs.microsoft.com/azure/cognitive-services/text-analytics/language-support)을 참조 하세요.  
