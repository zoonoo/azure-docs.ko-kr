...
Azure 검색은 매우 강력한 쿼리를 만드는 다양한 옵션을 제공합니다. 쿼리의 다른 매개 변수에 대해 자세히 알아보려면 [이 페이지](https://msdn.microsoft.com/library/azure/dn798927.aspx)를 방문합니다. 아래에는 몇 가지 예제 쿼리가 있습니다.

#### 예제 쿼리

다음은 "호텔"이라는 인덱스에 대한 몇 가지 예제 쿼리입니다. 이러한 쿼리는 가져오기 및 게시 형식으로 표시됩니다.

용어 '예산'에 대한 전체 인덱스를 검색하고 `hotelName` 필드를 반환합니다.
...
하루에 $150가 저렴한 호텔에 대한 전체 인덱스를 검색하고 `hotelId` 및 `description`를 반환합니다.
...
1. 위의 I 단계에서 찾은 쿼리 키로 `api-key` 헤더를 설정해야 합니다. 관리 키를 `api-key` 헤더로 사용할 수 있지만 인덱스와 문서에 대한 읽기 전용 액세스 권한을 부여하기 때문에 쿼리 키를 사용하는 것이 좋습니다.
...
성공적인 쿼리 요청은 `200 OK`인 상태 코드를 발생시키고 검색 결과는 응답 본문에서 JSON 형식으로 반환됩니다. 다음은 위의 쿼리 모양에 대한 결과이며 "호텔" 인덱스가 [이 문서](search-import-data-rest-api.md)에서 샘플 데이터로 채워진다고 가정합니다(명확하게 하기 위해 JSON 형식을 지정함).
...
자세한 내용은 "Response" 섹션의 [이 페이지](https://msdn.microsoft.com/library/azure/dn798927.aspx)를 방문합니다. 오류가 발생한 경우 반환될 수 있는 기타 HTTP 상태 코드에 대한 자세한 내용은 [이 문서](https://msdn.microsoft.com/library/azure/dn798925.aspx)를 참조하세요.
