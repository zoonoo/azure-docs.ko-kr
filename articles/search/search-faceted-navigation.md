...
description: Azure Cognitive Search와 통합되는 검색 애플리케이션에서 자기 주도형 필터링에 대해 패싯 탐색을 추가합니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
패싯 탐색은 검색의 대체 진입점입니다. 복잡한 검색 식을 직접 입력할 수 있는 편리한 대안을 제공합니다. 패싯을 사용하면 원하는 항목을 쉽게 찾을 수 있으며 항상 결과를 얻을 수 있습니다. 개발자는 패싯을 통해 검색 인덱스를 탐색하는 데 가장 유용한 검색 조건을 노출할 수 있습니다. 온라인 소매 애플리케이션에서는 종종 브랜드, 부서(어린이 신발), 크기, 가격, 인기도 및 등급에 대한 패싯 탐색이 작성됩니다. 
...
- [Azure Cognitive Search 구직 포털 데모](https://aka.ms/azjobsdemo)에서 온라인으로 작업 데모를 살펴보고 테스트하세요.
...
1. Azure Cognitive Search로 전송된 쿼리는 하나 이상의 패싯 쿼리 매개 변수를 통해 패싯 탐색 구조를 지정합니다. 예를 들어 쿼리는 프레젠테이션을 구체화하는 `:values` 또는 `:sort` 옵션과 함께 `facet=Rating`이 포함할 수 있습니다.
...
패싯은 쿼리 매개 변수이지만 쿼리 입력과 혼동해서는 안 됩니다. 쿼리의 선택 조건으로 사용되지 않습니다. 패싯 쿼리 매개 변수는 응답에서 반환되는 탐색 구조의 입력이라고 생각해야 합니다. 제공한 각 패싯 쿼리 매개 변수에 대해 Azure Cognitive Search는 각 패싯 값의 부분 결과에 포함된 문서 수를 평가합니다.
...
Azure Cognitive Search를 사용하여 애플리케이션 코드에서 검색 요청을 작성하는 패싯 탐색을 구현합니다. 패싯 탐색은 이전에 정의된 스키마의 요소를 사용합니다.
...
코드의 프레젠테이션 계층은 사용자 환경을 제공합니다. 레이블, 값, 확인란, 개수 등 패싯 탐색의 구성 부분을 나열합니다. Azure Cognitive Search REST API는 플랫폼에 독립적이므로 사용자가 원하는 언어와 플랫폼을 사용합니다. 단, 각 추가 패싯이 선택될 때 업데이트된 UI 상태로 증분 새로 고침을 지원하는 UI 요소를 포함해야 합니다. 
...
Azure Cognitive Search는 사용자가 입력한 하나 이상의 용어에 따라 검색 결과를 반환하고 패싯 탐색 구조를 업데이트합니다. Azure Cognitive Search에서 패싯 탐색은 패싯 값과 각 값에 대해 발견된 결과 수로 이루어진 단일 수준 구성입니다.
...
샘플 스키마에서 볼 수 있는 것처럼, ID 값과 같이 패싯으로 사용하면 안 되는 문자열 필드에 대해 `Facetable`이 해제되어 있습니다. 필요 없는 패싯을 해제하면 인덱스 크기를 작게 유지하는 데 도움이 되며 일반적으로 성능이 향상됩니다.
...
계층 구조에서 패싯 중첩은 직접 지원되지 않습니다. 기본적으로 Azure Cognitive Search의 패싯 탐색은 하나의 필터 수준만 지원합니다. 그러나 해결 방법이 있습니다. `Collection(Edm.String)` 의 계층적 패싯 구조를 계층당 하나의 항목으로 인코딩할 수 있습니다. 이 해결책을 구현하는 방법은 이 문서의 범위를 벗어납니다. 
...
> [!NOTE]
> 두 가지 이상의 형식이 있을 때 `count`를 설명하면 혼동을 일으킬 수 있습니다. 다음 표에서는 Azure Cognitive Search API, 샘플 코드 및 설명서에서 용어가 사용되는 방식에 대한 간략한 요약을 제공합니다. 
...
레이블은 일반적으로 HTML 양식(샘플 애플리케이션의 `index.cshtml`)으로 정의됩니다. Azure Cognitive Search에는 패싯 탐색 레이블 또는 다른 메타데이터에 사용할 수 있는 API가 없습니다.
...
필터 예제는 [OData 식 구문(Azure Cognitive Search)](query-odata-filter-orderby-syntax.md)에서 확인할 수 있습니다.

<a name="tryitout"></a>

## <a name="try-the-demo"></a>데모 사용해 보기
Azure Cognitive Search 구직 포털 데모에는 이 문서에 나와 있는 예제가 포함되어 있습니다.
...
[Azure Cognitive Search 심층 정보](https://channel9.msdn.com/Events/TechEd/Europe/2014/DBI-B410)를 살펴보세요. 45분 25초 구간에 패싯을 구현하는 방법에 대한 데모가 있습니다.
