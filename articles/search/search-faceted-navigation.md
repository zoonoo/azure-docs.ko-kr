...
패싯 탐색에 사용할 수 있는 모든 필드 형식은 기본적으로 `Facetable` 입니다. 이러한 필드 형식에 `Edm.String`는 `Edm.DateTimeOffset`, 및 모든 숫자 필드 형식이 포함 되어 있습니다. 기본적으로 모든 필드 형식은 패싯 가능 `Edm.GeographyPoint`를 제외 하 고 패싯 탐색에 사용할 수 없습니다. 
...
예제 스키마에서 볼 수 있듯이 `Facetable` 는 ID 값과 같이 패싯으로 사용 하지 않아야 하는 문자열 필드에 대해 해제 됩니다. 필요 하지 않은 경우 패싯을 끄면 인덱스 크기를 작게 유지 하는 데 도움이 되며 일반적으로 성능이 향상 됩니다.
