---
title: 면이 있는 탐색 범주 계층 구조 추가
titleSuffix: Azure Cognitive Search
description: Azure Cognitive Search와 통합되는 검색 응용 프로그램에서 자체 기반 필터링을 위한 면처리된 탐색을 추가합니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "79283161"
---
# <a name="how-to-implement-faceted-navigation-in-azure-cognitive-search"></a>Azure 인지 검색에서 면이 있는 탐색을 구현 하는 방법
...
패싯 탐색은 검색 애플리케이션에서 자기 주도형 드릴다운 탐색을 제공하는 필터링 메커니즘입니다. '패싯 탐색'이라는 용어가 낯설 수도 있지만 아마도 이전에 사용해 보셨을 것입니다. 다음 예제와 같이 패싯 탐색은 결과를 필터링하는 데 사용되는 범주일 뿐입니다.
 ...
 ![Azure 인지 검색 포털 데모](media/search-faceted-navigation/azure-search-faceting-example.png "Azure 인지 검색 포털 데모")

패싯 탐색은 검색의 대체 진입점입니다. 복잡한 검색 식을 직접 입력할 수 있는 편리한 대안을 제공합니다. 패싯은 원하는 것을 찾는 데 도움이 되는 동시에 결과가 0개되지 않도록 할 수 있습니다. 개발자는 면을 사용하면 검색 인덱스를 탐색하는 데 가장 유용한 검색 조건을 노출할 수 있습니다. 온라인 소매 응용 프로그램에서 면탐색은 종종 브랜드, 부서(어린이 신발), 크기, 가격, 인기도 및 등급을 통해 구축됩니다. 

패싯 탐색의 구현은 검색 기술에 따라 다릅니다. Azure Cognitive Search에서 면이 있는 탐색은 이전에 스키마에서 어트리뷰션된 필드를 사용하여 쿼리 시간에 빌드됩니다.
...
- 참조 및 Azure 인지 [검색 작업 포털 데모에서](https://aka.ms/azjobsdemo)온라인으로 작업 데모를 테스트합니다.

- [GitHub의 Azure 샘플 리포지토리](https://github.com/Azure-Samples/search-dotnet-asp-net-mvc-jobs)에서 코드를 다운로드하세요.

## <a name="get-started"></a>시작
검색 개발을 처음 접하는 경우 패싯 탐색을 자기 주도형 검색에 대한 가능성을 보여 주는 것이라고 생각하는 것이 가장 좋습니다. 미리 정의된 필터를 기반으로 한 드릴다운 검색 환경유형으로, 포인트 앤 클릭 작업을 통해 검색 결과를 빠르게 좁히는 데 사용됩니다. 

### <a name="interaction-model"></a>상호 작용 모델

면처리된 탐색에 대한 검색 환경은 반복적이므로 사용자 작업에 대한 응답으로 전개되는 쿼리 시퀀스로 이해하여 살펴보겠습니다.

시작 지점은 일반적으로 주변에서 패싯 탐색을 제공하는 애플리케이션 페이지입니다. 패싯 탐색은 각 값에 대한 확인란 또는 클릭할 수 있는 텍스트가 포함된 트리 구조인 경우가 많습니다. 

1. Azure Cognitive Search로 전송된 쿼리는 하나 이상의 facet 쿼리 매개 변수를 통해 면이 있는 탐색 구조를 지정합니다. 예를 들어 쿼리는 프레젠테이션을 구체화하는 `:values` 또는 `:sort` 옵션과 함께 `facet=Rating`이 포함할 수 있습니다.
...
패싯은 쿼리 매개 변수이지만 쿼리 입력과 혼동해서는 안 됩니다. 쿼리의 선택 조건으로 사용되지 않습니다. 패싯 쿼리 매개 변수는 응답에서 반환되는 탐색 구조의 입력이라고 생각해야 합니다. 제공하는 각 패싯 쿼리 매개 변수에 대해 Azure Cognitive Search는 각 패싯 값의 부분 결과에 있는 문서 수를 평가합니다.
...
Azure Cognitive Search에서 요청은 하나 이상의 쿼리 매개 변수를 통해 지정됩니다(각 쿼리에 대한 [설명에](https://docs.microsoft.com/rest/api/searchservice/Search-Documents) 대한 문서 검색 참조). 필수 사항인 쿼리 매개 변수는 없지만 쿼리가 유효하려면 하나 이상의 쿼리 매개 변수가 있어야 합니다.
...
두 쿼리 모두 유효하지만 시애틀에 주차가있는 비 모텔을 찾고 있다면 두 번째 쿼리가 우수합니다.
...
검색 요청을 빌드하는 응용 프로그램 코드에서 Azure Cognitive Search를 사용하여 면이 있는 탐색을 구현합니다. 패싯 탐색은 이전에 정의된 스키마의 요소를 사용합니다.
...
코드의 프레젠테이션 계층은 사용자 환경을 제공합니다. 레이블, 값, 확인란, 개수 등 패싯 탐색의 구성 부분을 나열합니다. Azure 인지 검색 REST API는 플랫폼에 구애받지 않으므로 원하는 언어와 플랫폼을 사용합니다. 단, 각 추가 패싯이 선택될 때 업데이트된 UI 상태로 증분 새로 고침을 지원하는 UI 요소를 포함해야 합니다. 
...
Azure Cognitive Search는 패싯 탐색 구조에 대한 업데이트와 함께 입력한 하나 이상의 용어를 기반으로 검색 결과를 반환합니다. Azure Cognitive Search에서 패싯 탐색은 패싯 값을 가진 단일 수준 구성이며 각 탐색에 대해 발견된 결과 수의 수입니다.
...
패싯 탐색에 사용할 수 있는 모든 필드 형식은 기본적으로 `Facetable` 입니다. 이러한 필드 `Edm.String`유형에는 의과 `Edm.DateTimeOffset`제가 사용하는 모든 숫자 필드 유형(기본적으로 `Edm.GeographyPoint`모든 필드 유형은 면 탐색에 사용할 수 없는 면이 있는 필드 유형)을 포함합니다. 

인덱스를 작성할 때 패싯으로 사용해서는 안 되는 필드에 대한 패싯을 명시적으로 해제하는 것이 가장 좋습니다.  특히 ID 또는 제품 이름과 같은 Singleton 값의 문자열 필드는 패싯 탐색에서 실수로(그리고 비효과적으로) 사용되지 않도록 `"Facetable": false`로 설정해야 합니다. 필요하지 않은 곳에 면을 돌리면 인덱스 크기를 작게 유지하는 데 도움이 되며 일반적으로 성능이 향상됩니다.
...
샘플 스키마에서 볼 수 있듯이 `Facetable` ID 값과 같은 면으로 사용해서는 안 되는 문자열 필드에 대해 해제됩니다. 필요하지 않은 곳에 면을 돌리면 인덱스 크기를 작게 유지하는 데 도움이 되며 일반적으로 성능이 향상됩니다.
...
웹 애플리케이션의 경우 주로 AJAX가 프레젠테이션 계층에서 사용되는데, 이는 증분 변경 내용을 새로 고칠 수 있기 때문입니다. 또한 ASP.NET MVC 또는 HTTP를 통해 Azure 인지 검색 서비스에 연결할 수 있는 다른 시각화 플랫폼을 사용할 수도 있습니다. 이 문서에서 참조하는 샘플 응용 **프로그램(Azure 인지 검색 구직 포털 데모)은** ASP.NET MVC 응용 프로그램입니다.
...
패싯 쿼리 매개 변수는 필드로 설정되어 있으며, 데이터 형식에 따라 `count:<integer>`, `sort:<>`, `interval:<integer>` 및 `values:<list>`를 포함하는 쉼표로 구분된 목록으로 추가 매개 변수화할 수 있습니다. 값 목록은 범위를 설정할 때 숫자 데이터에 대해 지원됩니다. 사용 세부 정보는 [문서 검색(Azure 인지 검색 API)을](https://docs.microsoft.com/rest/api/searchservice/Search-Documents) 참조하십시오.
...
계층 구조에서 패싯 중첩은 직접 지원되지 않습니다. 기본적으로 Azure 인지 검색에서 면이 적용된 탐색은 한 수준의 필터만 지원합니다. 그러나 해결 방법이 있습니다. `Collection(Edm.String)` 의 계층적 패싯 구조를 계층당 하나의 항목으로 인코딩할 수 있습니다. 이 해결책을 구현하는 방법은 이 문서의 범위를 벗어납니다. 
...
**필터로 검색 정확도 향상**
 ...
 필터를 사용합니다. 검색 식만 사용하는 경우 형태소 분석으로 인해 해당 필드에 정확한 면값이 없는 문서가 반환될 수 있습니다.
...
* `&facet=city,count:5`는 상위 순위 결과에서 발견된 처음 5개 도시만 패싯 결과로 반환되도록 지정합니다. "공항"과 32개의 일치 항목의 검색어가 있는 샘플 쿼리를 고려합니다. 쿼리에서 `&facet=city,count:5`를 지정하면 검색 결과에 문서 수가 가장 많은 상위 5개의 고유한 도시만 패싯 결과에 포함됩니다.
...
> [!NOTE]
> 두 가지 이상의 형식이 있을 때 `count`를 설명하면 혼동을 일으킬 수 있습니다. 다음 표에서는 Azure Cognitive Search API, 샘플 코드 및 설명서에서 용어가 사용되는 방법에 대한 간략한 요약을 제공합니다. 
...
* "`@odata.count`"<br/>
   쿼리 응답에서 이 값은 검색 결과의 일치하는 항목 수를 나타냅니다. 평균적으로 검색어와 일치하지만 패싯 값이 일치하지 않는 항목이 있기 때문에 결합된 모든 패싯 결과의 합계보다 큽합니다.

**패싯 결과의 개수 가져오기**

패싯 쿼리에 필터를 추가할 때 패싯 문(예: `facet=Rating&$filter=Rating ge 4`)을 유지할 수 있습니다. 기술적으로 면=등급은 필요하지 않지만 등급 4 이상에 대한 면 값 수를 반환합니다. 예를 들어 사용자가 "4"를 클릭하고 쿼리에 "4" 이상에 대한 필터가 포함되어 있으면 4 이상인 각 등급의 개수가 반환됩니다.  

**정확한 수의 패싯을 가져오는지 확인**

특정 상황에서는 면 수가 결과 집합과 일치하지 않을 수 [있습니다(Azure 인지 검색(포럼 게시물)의 Faceted 탐색](https://social.msdn.microsoft.com/Forums/azure/06461173-ea26-4e6a-9545-fbbd7ee61c8f/faceting-on-azure-search?forum=azuresearch)참조).
...
이 동작은 언제든지 변경될 수 있지만 오늘 이 동작이 발생하면 수>\<숫자를 대량으로 인위적으로 팽창시켜 각 샤드에서 전체 보고를 적용할 수 있습니다. count: 값이 필드의 고유 값 수보다 크거나 같으면 정확한 결과가 반환됩니다. 그러나 문서 수가 많은 경우에는 성능이 저하되므로 이 옵션을 신중하게 사용해야 합니다.

### <a name="user-interface-tips"></a>사용자 인터페이스 팁
**패싯 탐색의 각 필드에 대한 레이블 추가**

레이블은 일반적으로 HTML 양식(샘플 애플리케이션의 `index.cshtml`)으로 정의됩니다. Azure Cognitive Search에서 면 탐색 레이블 또는 기타 메타데이터에 대한 API가 없습니다.

<a name="rangefacets"></a>

## <a name="filter-based-on-a-range"></a>범위를 기준으로 필터링
값 범위에 대한 패싯은 일반적인 검색 애플리케이션 요구 사항입니다. 범위는 숫자 데이터 및 DateTime 값에 대해 지원됩니다. 각 접근 방식에 대한 자세한 내용은 [검색 문서(Azure 인지 검색 API)에서](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)확인할 수 있습니다.

Azure Cognitive Search는 범위를 계산하기 위한 두 가지 방법을 제공하여 범위 구성을 단순화합니다. 두 방법 모두에 대해 Azure Cognitive Search는 제공한 입력에 따라 적절한 범위를 만듭니다. 예를 들어 10|20|30 범위 값을 지정한 경우 0-10, 10-20, 20-30 범위가 자동으로 만들어집니다. 비어 있는 간격을 애플리케이션에서 선택적으로 제거할 수도 있습니다. 
...
각 범위는 0부터 작성되며, 목록의 값을 엔드포인트로 사용하고 이전 범위를 잘라 불연속 간격을 만듭니다. Azure Cognitive Search는 면이 있는 탐색의 일부로 이러한 작업을 수행합니다. 각 간격을 구성하기 위해 코드를 작성할 필요가 없습니다.
...
현재 위치와의 근접성을 기반으로 매장, 음식점 또는 목적지를 선택하는 데 도움이 되는 필터를 보는 것이 일반적입니다. 이러한 유형의 필터는 면이 있는 탐색처럼 보일 수 있지만 필터일 뿐입니다. 특별히 이와 같은 특정 디자인 문제에 대한 구현 조언을 구하는 경우 이 점에 주의해야 합니다.

Azure 인지 검색에는 **geo.distance** 및 **geo.intersects의**두 가지 지리 공간 함수가 있습니다.
...
[OData 식 구문 (Azure 인지 검색)에서](query-odata-filter-orderby-syntax.md)필터 예제를 찾을 수 있습니다.

<a name="tryitout"></a>

## <a name="try-the-demo"></a>데모 사용해 보기
Azure 인지 검색 포털 데모에는 이 문서에서 참조하는 예제가 포함되어 있습니다.
...
3. "분석가" 등의 검색 용어를 입력하고 검색 아이콘을 클릭합니다. 쿼리가 신속하게 실행됩니다.
   ...
   ![패싯을 선택하기 전의 검색 결과](media/search-faceted-navigation/faceted-search-before-facets.png "패싯을 선택하기 전의 검색 결과")
...
[시계 Azure 인지 검색 심층 다이빙](https://channel9.msdn.com/Events/TechEd/Europe/2014/DBI-B410). 45분 25초 구간에 패싯을 구현하는 방법에 대한 데모가 있습니다.
...
* [면검색 구현 시 프런트 엔드 우려 – 1부](https://articles.uie.com/faceted_search2/)
