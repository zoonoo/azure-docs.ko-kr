...
---
# <a name="add-suggesters-to-an-index-for-typeahead-in-azure-cognitive-search"></a>Azure 인지 검색에서 입력에 대 한 인덱스에 제안자를 추가 합니다.

Azure Cognitive Search에서 "사용자유형으로 검색" 또는 typeahead 기능은 [검색 인덱스에](search-what-is-an-index.md)추가하는 **제안자** 구문입니다. 형식을 사용하도록 설정하려는 하나 이상의 필드 목록입니다.

제안자는 입력하는 용어 나 구를 완료하는 *자동 완성및*일치하는 문서의 짧은 목록을 반환하는 제안의 두 가지 유형 전형 *변형을* 지원합니다.  

다음 스크린샷은 [C#](tutorial-csharp-type-ahead-and-suggestions.md) 샘플에서 첫 번째 앱 만들기에서 형식앞서 보여 줍니다. 자동 완성은 사용자가 검색 상자에 입력할 수 있는 내용을 예상합니다. 실제 입력은 "tw"이며, 자동 완성은 "in"으로 끝나고 잠재 검색어로 "트윈"으로 해결됩니다. 제안은 드롭다운 목록에 표시됩니다. 제안의 경우 결과를 가장 잘 설명하는 문서의 모든 부분을 표시할 수 있습니다. 이 예제에서 제안 사항은 호텔 이름입니다. 
...
### <a name="analyzer-restrictions-for-sourcefields-in-a-suggester"></a>제안자의 소스 필드에 대한 분석기 제한

Azure Cognitive Search는 필드 콘텐츠를 분석하여 개별 용어에 대한 쿼리를 활성화합니다. 제안자는 전체 용어 외에 접두사를 인덱싱해야 하므로 원본 필드에 대한 추가 분석이 필요합니다. 사용자 지정 분석기 구성은 다양한 토큰화기와 필터를 결합할 수 있으며, 종종 제안에 필요한 접두사를 생성하는 것이 불가능합니다. 이러한 이유로 Azure Cognitive Search는 사용자 지정 분석기의 필드가 제안자에 포함되지 않도록 합니다.

> [!NOTE] 
>  위의 제한 사항 해결이 필요한 경우 동일한 콘텐츠에 대해 두 개의 별도 필드를 사용합니다. 이렇게 하면 필드 중 하나에 제안자가 있고 다른 필드는 사용자 지정 분석기 구성으로 설정할 수 있습니다.
