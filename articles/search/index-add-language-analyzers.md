...
+ Lucene의 English analyzer는 표준 분석기를 확장 합니다. 이 분석기는 단어에서 소유격(후행 's)을 제거하고, Porter 형태소 분석 알고리즘에 따라 형태소 분석을 적용하며, 영어의 중지 단어를 제거합니다.  

+ Microsoft 영어 분석기는 형태소 분석 대신 분류 정리를 수행합니다. 즉, 더 많은 관련 검색 결과가 발생 하는 만들거나 굴절 형 및 불규칙 한 단어 형태를 처리할 수 있습니다. 

## <a name="how-to-specify-a-language-analyzer"></a>언어 분석기를 지정 하는 방법

필드 정의 중에 Edm 형식의 "검색 가능" 필드에 언어 분석기를 설정 합니다.

필드 정의에는 여러 분석기 관련 속성이 있지만 언어 분석기에는 "analyzer" 속성만 사용할 수 있습니다. "Analyzer"의 값은 지원 분석기 목록에서 언어 분석기 중 하나 여야 합니다.
...
인덱스를 만들고 필드 속성을 설정 하는 방법에 대 한 자세한 내용은 [인덱스 만들기 (REST)](/rest/api/searchservice/create-index)를 참조 하세요. 텍스트 분석에 대 한 자세한 내용은 [Azure Cognitive Search의 분석기](search-analyzers.md)를 참조 하세요.

<a name="language-analyzer-list"></a>

## <a name="supported-language-analyzers"></a>지원 되는 언어 분석기

 다음은 Lucene 및 Microsoft analyzer 이름이 포함 된 지원 되는 언어 목록입니다.  
...
+ [LexicalAnalyzerName 클래스 (Azure SDK for .NET)](/dotnet/api/azure.search.documents.indexes.models.lexicalanalyzername)