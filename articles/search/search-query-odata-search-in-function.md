...
---
# <a name="odata-searchin-function-in-azure-cognitive-search"></a>Azure `search.in` Cognitive Search의 OData 함수
...
> [!NOTE]
> OData 표준 버전 4.01에는 최근에 Azure Cognitive Search `search.in` 함수와 비슷한 동작이 있는 [ `in` 연산자](https://docs.oasis-open.org/odata/odata/v4.01/cs01/part2-url-conventions/odata-v4.01-cs01-part2-url-conventions.html#_Toc505773230)가 도입 되었습니다. 그러나 Azure Cognitive Search는이 연산자를 지원 하지 않으므로 대신 함수를 `search.in` 사용 해야 합니다.
...
| --- | --- | --- |
| `variable` | `Edm.String` | 또는 식 내에서 `search.in` `all` 가 사용 되는 경우 문자열 필드 참조 (또는 문자열 컬렉션 필드의 범위 변수) `any` 입니다. |
