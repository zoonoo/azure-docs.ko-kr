---
title: ".NET SDK를 사용하여 Azure Search 인덱스 쿼리 | Microsoft Docs"
...
---
# <a name="query-your-azure-search-index-using-the-net-sdk"></a>.NET SDK를 사용하여 Azure 검색 인덱스 쿼리
...
## <a name="ii-create-an-instance-of-the-searchindexclient-class"></a>II. SearchIndexClient 클래스의 인스턴스 만들기
...
## <a name="iii-query-your-index"></a>III. 인덱스 쿼리
...
사용할 두 가지 기본 [쿼리 유형](search-query-overview.md#types-of-queries)은 `search` 및 `filter`입니다. `search` 쿼리는 인덱스에서 모든 *검색 가능한* 필드에 대해 하나 이상의 단어를 검색합니다. `filter` 쿼리는 인덱스의 *필터링 가능한* 모든 필드에 걸쳐 부울 식을 계산합니다.
...
## <a name="iv-handle-search-results"></a>IV. 검색 결과 처리
