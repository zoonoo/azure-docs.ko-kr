...
#### 쿼리 유형
사용할 두 가지 기본 [쿼리 유형](search-query-overview.md#types-of-queries)은 `search` 및 `filter`입니다. `search` 쿼리는 인덱스에서 모든 _검색 가능한_ 필드에 대해 하나 이상의 단어를 검색합니다. `filter` 쿼리는 인덱스의 모든 _필터링 가능한_ 필드에 걸쳐 부울 식을 계산합니다.
