...
```  
 ...
 이 쿼리는 용어 'inn'을 검색하고 현재 위치를 전달합니다. 이 쿼리에는 `scoringParameter`와 같은 다른 매개 변수가 포함 되어 있습니다. 쿼리 매개 변수는 [Azure Cognitive Search REST API &#40;&#41;문서 검색](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)에 설명 되어 있습니다.  
...
검색 점수는 쿼리와 데이터의 통계 속성에 따라 계산됩니다. Azure Cognitive Search는 쿼리 문자열에 검색 단어를 포함 하는 문서를 찾습니다 (`searchMode`에 따라 일부 또는 모두). 검색 용어의 여러 인스턴스가 포함 된 문서를 찾은 다음 합니다. 데이터가 데이터 인덱스 전체에서 드물게 발생 하지만 문서 내에서 자주 발생 하는 경우 검색 점수가 더 높아집니다. 이와 같은 관련성 계산 방식의 기준을 [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf), 즉 용어 빈도와 문서 빈도 반비례라고 합니다.  
...
## <a name="workflow"></a>워크플로  
 ...
 이름을 입력합니다. 점수 매기기 프로필은 선택 사항이지만 추가하려는 경우 이름을 입력해야 합니다. 이름은 필드의 이름 지정 규칙에 따라 문자로 시작해야 하며 특수 문자와 예약어는 사용할 수 없습니다. 전체 목록은 [명명 &#40;규칙 Azure&#41; Cognitive Search](https://docs.microsoft.com/rest/api/searchservice/naming-rules) 를 참조 하세요.  
