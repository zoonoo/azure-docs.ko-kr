...
이 단계에서는 인덱서에 사용 되는 데이터 원본 연결 정보를 제공 합니다. 데이터 원본은 Azure Cognitive Search에서 연결 정보를 유지 하는 명명 된 개체입니다. 데이터 원본 유형 `azureblob`는 인덱서에 의해 호출 되는 데이터 추출 동작을 결정 합니다. 
