...
동일한 지역에서 Azure Search 및 Azure storage 모두에 동일한 Azure 구독을 사용 하는 것이 좋습니다.
...
REST API를 사용 하 여 JSON blob를 인덱싱할 수 있습니다. 여기에는 Azure Search의 모든 인덱서에 공통 된 세 부분으로 구성 된 워크플로, 즉 데이터 원본 만들기, 인덱스 만들기, 인덱서 만들기 등이 있습니다. Blob 저장소에서 데이터 추출은 인덱서 만들기 요청을 제출할 때 발생 합니다. 이 요청을 완료 한 후에는 쿼리 가능한 인덱스를 갖게 됩니다. 
...
| Blob 당 여러 개 | `jsonLines` | 여러 JSON 엔터티 ("배열")를 여러 줄로 구분 하 여 포함 하는 blob을 구문 분석 합니다. 여기서 각 엔터티는 별도의 Azure Search 문서가 됩니다. | [REST](https://docs.microsoft.com/rest/api/searchservice/indexer-operations) API와 [.net](https://docs.microsoft.com/dotnet/api/microsoft.azure.search.models.indexer) SDK에서 일반적으로 사용할 수 있습니다. |
...
인덱스 및 데이터 원본과 마찬가지로, 인덱서는 Azure Search 서비스에서 만들고 다시 사용 하는 명명 된 개체 이기도 합니다. 인덱서를 만들도록 완전히 지정 된 요청은 다음과 같을 수 있습니다.
...
Azure Search 트리거 데이터 가져오기에서 인덱서를 만듭니다. 이 메서드는 즉시 실행 되며 이후에 일정에 따라 실행 됩니다.
...
또는 JSON 배열 옵션을 사용할 수 있습니다. 이 옵션은 blob이 *잘 구성 된 JSON 개체 배열을*포함 하 고 각 요소가 별도의 Azure Search 문서가 되도록 하려는 경우에 유용 합니다. 예를 들어 다음 JSON Blob의 경우 각각 "id" 및 "text" 필드가 있는 별도의 3개 문서로 Azure Search 인덱스를 채울 수 있습니다.  
...
Blob이 줄 바꿈으로 구분 된 여러 JSON 엔터티를 포함 하 고 각 요소가 별도의 Azure Search 문서가 되도록 하려면 JSON 줄 옵션을 선택할 수 있습니다. 예를 들어, 다음 blob (3 개의 서로 다른 JSON 엔터티)이 있을 경우 각각 "id" 및 "text" 필드가 있는 별도의 3 개 문서로 Azure Search 인덱스를 채울 수 있습니다.
...
현재 Azure Search는 기본 데이터 형식, 문자열 배열 및 GeoJSON 지점만 지원 하므로 임의의 JSON 문서를 직접 인덱싱할 수 없습니다. 그러나 **필드 매핑** 을 사용하여 JSON 문서의 부분을 선택하고 검색 문서의 최상위 필드로 이 부분을 "올릴" 수 있습니다. 필드 매핑 기본 사항에 대한 자세한 내용은 [Azure Search 인덱서의 필드 매핑](search-indexer-field-mappings.md)을 참조하세요.
