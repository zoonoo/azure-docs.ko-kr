...
## <a name="results-returned"></a>반환된 결과
...
전체 텍스트 검색 쿼리의 경우 결과는 문서의 용어 빈도 및 근접성을 기준으로 계산된 검색 점수에 따라 자동으로 순위가 매겨지며, 점수가 높을수록 검색어에서 일치하는 항목이 더 많거나 더 강한 문서가 됩니다. 검색 점수는 동일한 결과 집합의 다른 문서에 비해 일반적인 관련성 을 전달하며 한 쿼리에서 다음 쿼리로 일관성이 보장되지 않습니다.

쿼리를 사용할 때 정렬된 결과에 작은 불일치가 있을 수 있습니다. 이 문제가 발생할 수 있는 이유에 대한 몇 가지 설명이 있습니다.

| 조건 | Description |
|-----------|-------------|
| 데이터 변동성 | 문서를 추가, 수정 또는 삭제할 때 인덱스의 내용은 다양합니다. 기간 빈도는 인덱스 업데이트가 시간이 지남에 따라 처리됨에 따라 변경되어 일치하는 문서의 검색 점수에 영향을 미칩니다. |
| 쿼리 실행 위치 | 여러 복제본을 사용하는 서비스의 경우 각 복제본에 대해 쿼리가 병렬로 발급됩니다. 검색 점수를 계산하는 데 사용되는 인덱스 통계는 복제본별로 계산되며 결과는 쿼리 응답에서 병합되고 정렬됩니다. 복제본은 대부분 서로의 미러이지만 상태의 작은 차이로 인해 통계가 다를 수 있습니다. 예를 들어 한 복제본이 다른 복제본에서 병합된 해당 통계에 기여하는 문서를 삭제했을 수 있습니다. 일반적으로 복제본별 통계의 차이는 작은 인덱스에서 더 두드러지다. |
| 동일한 검색 점수 간에 동률 을 끊기 | 검색 문서에 점수가 동일한 경우에도 정렬된 결과의 불일치가 발생할 수 있습니다. 이 경우 동일한 쿼리를 다시 실행할 때 어떤 문서가 먼저 나타날지 보장할 수 없습니다. |

### <a name="consistent-ordering"></a>일관된 순서

검색 점수 매기기의 유연성을 감안할 때 결과 주문의 일관성이 응용 프로그램 요구 사항인 경우 다른 옵션을 탐색할 수 있습니다. 가장 쉬운 방법은 등급이나 날짜와 같은 필드 값별로 정렬하는 것입니다. 등급이나 날짜와 같은 특정 필드별로 정렬하려는 시나리오의 경우 **정렬 가능으로**인덱싱된 모든 필드에 적용할 수 있는 [ `$orderby` 식을](query-odata-filter-orderby-syntax.md)명시적으로 정의할 수 있습니다.
