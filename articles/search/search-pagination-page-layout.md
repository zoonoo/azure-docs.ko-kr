...
### <a name="new-behavior-starting-july-15"></a>새로운 동작(7월 15일부터)

2020년 7월 15일 이후에 생성된 서비스는 다른 하이라이트 환경을 제공합니다. 해당 날짜 이전에 생성된 서비스는 강조 표시 동작에서 변경되지 않습니다. 

새 동작:

* 전체 구 쿼리와 일치하는 구문만 반환됩니다. 쿼리 "슈퍼 그릇"은 다음과 같은 하이라이트를 반환합니다.

    ```html
    '<em>super bowl</em> is super awesome with a bowl of chips'
    ```
  칩의 용어 *그릇은* 전체 구와 일치하지 않기 때문에 강조 표시가 없습니다.
  
* 강조 표시에 대해 반환되는 조각 크기를 지정할 수 있습니다. 조각 크기는 문자 수로 지정됩니다(최대 문자는 1,000자).
