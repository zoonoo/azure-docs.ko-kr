...
다른 개수의 일치 하는 문서를 반환 하려면 및 `$top` `$skip` 매개 변수를 쿼리 요청에 추가 합니다. 다음 목록에서는 논리에 대해 설명 합니다.

+ 인덱스 `$count=true` 내에 있는 총 일치 문서 수를 가져오려면를 추가 합니다.

+ 15 개의 일치 하는 문서의 첫 번째 집합과 총 일치 항목 수를 반환 합니다.`GET /indexes/<INDEX-NAME>/docs?search=<QUERY STRING>&$top=15&$skip=0&$count=true`

+ 두 번째 집합을 반환 하 고 첫 15를 건너뛰고 다음 15: `$top=15&$skip=15`을 가져옵니다. 세 번째 15 번째 집합에 대해 동일한 작업을 수행 합니다.`$top=15&$skip=30`

기본 인덱스가 변경 되는 경우 페이지가 매겨진 쿼리 결과가 안정적이 지 않을 수 있습니다. 페이징은 각 페이지 `$skip` 에 대 한 값을 변경 하지만 각 쿼리는 독립적 이며 쿼리 시의 인덱스에 있는 데이터의 현재 보기에서 작동 합니다. 즉, 일반적인 용도의 데이터베이스에 있는 것과 같은 결과의 캐싱 또는 스냅숏이 없습니다.
 
다음은 중복을 얻는 방법에 대 한 예입니다. 4 개의 문서를 포함 하는 인덱스를 가정 합니다.
    ...
    { "id": "4", "rating": 1 }
 
이제 등급 별로 정렬 된 결과를 한 번에 두 번 반환 하려는 경우를 가정 합니다. 이 쿼리를 실행 하 여 결과의 첫 번째 페이지를 가져올 `$top=2&$skip=0&$orderby=rating desc`수 있습니다. 즉, 다음과 같은 결과를 생성 합니다.

    { "id": "1", "rating": 5 }
    { "id": "2", "rating": 3 }
 
서비스에서는 쿼리 호출 사이에 다섯 번째 문서가 인덱스에 추가 된 것으로 가정 `{ "id": "5", "rating": 4 }`합니다.  잠시 후에 두 번째 페이지 `$top=2&$skip=2&$orderby=rating desc`를 인출 하는 쿼리를 실행 하 고 이러한 결과를 가져옵니다.
...
* 강조 표시에 대해 반환 되는 조각 크기를 지정할 수 있습니다. 조각 크기는 문자 수 (최대 1000 자)로 지정 됩니다.
