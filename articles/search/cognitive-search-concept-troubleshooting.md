...
문제가 파일 크기와 관련 된 경우 다음과 같은 오류가 표시 될 수 있습니다. "blob \<파일-이름>"의 \<크기가 파일 크기> 바이트 (현재 서비스 계층의 문서 추출에 대 한 최대 크기를 초과 함)입니다. " 인덱서 제한에 대한 자세한 내용은 [서비스 제한](search-limits-quotas-capacity.md)을 참조하세요.
