...
어느 방법을 사용 하 든 목표는 외부 데이터 원본에서 Azure Search 인덱스로 *데이터를 로드* 하는 것입니다. Azure Search를 사용 하면 빈 인덱스를 만들 수 있지만 데이터를 푸시 또는 끌어올 때 까지는 쿼리를 쿼리할 수 없습니다.
...
각 방법론에 대 한 소개를 보려면 [빠른 시작: PowerShell](search-create-index-rest-api.md) 또는 [ 빠른 시작을 사용 하 여 Azure Search 인덱스를 C# 만듭니다. .NET SDK](search-get-started-dotnet.md)를 사용 하 여 Azure Search 인덱스를 만듭니다.
...
REST API에서 Azure Search 인덱스의 끝점 URL에 대 한 JSON 요청 본문을 사용 하 여 HTTP POST 요청을 실행 합니다. "Value" 배열의 각 JSON 개체에는 문서의 키가 포함 되어 있으며 인덱싱 작업이 문서 콘텐츠를 추가, 업데이트 또는 삭제할지 여부를 지정 합니다. 코드 예제는 [문서 로드](search-get-started-dotnet.md#load-documents)를 참조 하세요.

.Net SDK에서 `IndexBatch` 개체에 데이터를 패키지 합니다. 는 `IndexBatch` 개체의 `IndexAction` 컬렉션을 캡슐화 합니다. 각 개체에는 문서 및 해당 문서에서 수행할 작업 Azure Search에 알려 주는 속성이 포함 되어 있습니다. 코드 예제는 [ C# 빠른](search-get-started-dotnet.md)시작을 참조 하세요.
