...
* 검색 서비스에서 호스트되는 [호텔 샘플 인덱스](search-get-started-portal.md)
...
1. 아래로 스크롤합니다. [요청]에서 **+ 샘플에서 가져오기** 단추를 선택하여 검색 서비스에 대한 쿼리 요청을 구성합니다.
   ...
   * [헤더]에 대해 `Content-Type`을 입력합니다. 이후 단계에서 값을 `application/json`으로 설정합니다.

     **Power Apps** 는 URL의 구문을 사용하여 쿼리에서 매개 변수를 추출합니다. 마법사를 진행하면서 검색, 선택 및 api-version 매개 변수를 구성할 수 있습니다.
...
1. 마법사로 다시 전환하여 **3. 요청** 단계로 돌아갑니다. 응답 섹션까지 아래로 스크롤합니다. **"기본 응답 추가"** 를 클릭합니다. 이는 Power Apps에서 응답 스키마를 인식하는 데 도움이 되므로 매우 중요합니다. 
...
1. 응답 샘플을 붙여넣습니다. 응답 샘플은 Azure Portal에서 검색 탐색기를 통해 쉽게 캡처할 수 있습니다. 검색 탐색기에서 요청에 대해 수행한 것과 동일한 쿼리를 입력해야 하지만 **$top=2** 를 추가하여 결과를 두 개의 문서로만 제한합니다(`search=*&$select=HotelName,Description,Address/City&$top=2`). 
   ...
   Power Apps는 스키마를 검색하는 데 몇 가지 결과만 필요합니다. 이제 hotels-sample-index를 사용한다고 가정하고 마법사에 다음 응답을 복사할 수 있습니다.
...
1. **가져오기** 를 클릭하여 기본 응답을 추가합니다.

1. 오른쪽 상단에 있는 **커넥터 만들기** 를 클릭하여 정의를 저장합니다.

1. **닫기** 를 클릭하여 커넥터를 닫습니다.
...
1. 목록에서 커넥터를 찾습니다(이 자습서에서는 "AzureSearchQuery").
