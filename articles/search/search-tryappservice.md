<properties 
   pageTitle="Azure 앱 서비스에서 무료로 Azure 검색 시도 | Microsoft Azure"
   description="Azure 앱 서비스 템플릿을 사용하여 1시간까지 무료로 Azure 검색을 시도합니다."
   services="search" 
   documentationCenter="" 
   authors="HeidiSteen" 
   manager="mblythe" 
...
[Azure 앱 서비스](https://tryappservice.azure.com/)는 Azure 구독 등록 없이 최대 1시간 동안 Azure 검색과 같은 선택한 Azure 서비스를 완전히 무료로 시험 사용해볼 수 있는 새로운 방법입니다.
...
[Azure 앱 서비스](https://tryappservice.azure.com/) 사이트에서 Azure 검색은 웹앱 템플릿의 일부로, 풍부한 전체 텍스트 검색 환경과 더불어 Azure 플랫폼의 이 서비스에서만 사용 가능한 다양한 검색 중심 기능도 함께 제공합니다.

SQL 데이터베이스와 같은 기타 Azure 서비스에서도 전체 텍스트 검색을 제공하기는 하지만 Azure 검색 서비스는 튜닝 제어, 페이지 매김 및 개수, 적중 항목 강조, 자동 완성 쿼리 제안, 자연 언어 지원, 패싯 탐색, 필터링 등을 제공합니다. 다양한 [샘플](https://github.com/Azure-Samples?utf8=%E2%9C%93&query=search) 시연에서 볼 수 있듯이, Azure 검색 및 ASP.NET만 사용해서도 모든 기능을 갖춘 검색 기반 응용 프로그램을 개발할 수 있습니다.

[Azure 앱 서비스](https://tryappservice.azure.com/) 제공 사항의 일부로써 앞으로 사용할 Azure 검색 서비스가 이미 사용자를 위해 만들어져 있으며 검색 쿼리를 받을 준비가 되어 있습니다. 사용자 고유의 인덱스 또는 데이터를 업로드하거나 사용할 수 없습니다. 작업할 데이터는 [미국 지리 설문 조사(USGS)](http://geonames.usgs.gov/domestic/download_data.htm)로, 미국 전체 랜드마크, 유적지, 건물 및 기타 랜드마크 특징에 대한 약 3백만 행으로 구성됩니다.
...
Azure 검색은 완전히 관리되는 [PaaS](https://en.wikipedia.org/wiki/Platform_as_a_service)(Platform as a Service) 서비스로 이를 통해 개발자는 웹 및 모바일 응용 프로그램에서 훌륭한 검색 환경을 통합하는 것이 쉬워집니다. 검색 작업에 사용되는 데이터는 Azure에서 검색 서비스와 함께, 데이터의 작업 근접도에 따라 짧은 대기 시간과 일관된 검색 동작이 보장되는 위치에 저장됩니다. 정리하자면 다음과 같습니다.

- 검색 가능한 데이터가 Azure 검색에서 관리하는 인덱스에 저장됨
- 인덱스를 정의하는 스키마는 사용자가 정의하며 필터 식에 유용할 수 있는 검색 가능 필드, 검색 불가능 필드를 지정하고 튜닝 결과에 대한 점수 매기기 프로필 등을 생성합니다.
- 검색 인덱스에는 검색하고 조회할 수 있는 하나 이상의 문서(테이블의 행과 유사)가 있습니다.
...
[Azure 앱 서비스](https://tryappservice.azure.com/) 옵션에서 "ASP.NET + Azure 검색 사이트" 템플릿은 웹 응용 프로그램에 대한 소스 코드를 제공하며 Visual Studio Team Services(1시간 세션의 일부로 사용 가능)에서 수정 가능합니다. 코드를 보거나 변경하는 데 별도의 개발 도구가 필요하지 않습니다.
...
7. **Visual Studio Team Services로 편집**을 선택하여 솔루션을 보고 사이트를 탐색합니다.
9. Visual Studio Team Services에서 페이지 맨 위에서 세션 옵션을 확장하고 **웹 사이트 찾아보기**를 클릭합니다.
...
- 별표(`*`)를 추가하여 와일드카드 검색을 시작합니다. 그러면 2,262,578 인덱스에 몇 개의 문서가 있는지 알려줍니다.
- 다음으로, "Yellowstone"을 입력한 다음 "+center", "+building", "-ND"를 추가하여 North Dakota의 결과를 제외하고 Yellowstone 방문자 센터만 포함하도록 점진적으로 검색 결과 범위를 좁힙니다. `Yellowstone +center +building -ND`  
...
1. 응용 프로그램 예제가 포함된 브라우저 창을 열고 **Visual Studio Team Services에 연결**을 선택합니다.
...
물론, 이것은 아마도 가능성이 적은 시나리오이기는 하지만 `NOT` 연산자를 포함하는 검색 구문에서 **searchMode**의 효과를 보여 주므로 동작이 발생하는 이유와 원하는 결과가 아닌 경우 변경하는 방법을 파악하는 데 도움이 됩니다.
...
[Azure 앱 서비스 시도](https://tryappservice.azure.com/) 사이트에 제공된 읽기 전용 서비스를 사용하여 쿼리 구문과 전체 텍스트 검색 작동을 확인하고 searchMode 및 필터에 대해 학습했으며 검색 응용 프로그램에 적중 항목 강조 표시를 추가해 보았습니다. 다음 단계로 인덱스 만들기 및 업데이트를 진행합니다. 이 단계를 통해 다음을 수행할 수 있습니다.
