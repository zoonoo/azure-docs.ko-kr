...
description: 여러 데이터 원본에서 단일 Azure Cognitive Search 인덱스로 데이터를 가져오는 방법에 대해 알아봅니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
ms.locfileid: "73795651"
---
# <a name="c-tutorial-combine-data-from-multiple-data-sources-in-one-azure-cognitive-search-index"></a>C#자습서: 하나의 Azure Cognitive Search 인덱스에 있는 여러 데이터 원본의 데이터 결합

Azure Cognitive Search는 여러 데이터 원본의 데이터를 결합 된 단일 검색 인덱스로 가져오고, 분석 하 고, 인덱싱할 수 있습니다. 이렇게 하면 정형 데이터가 텍스트, HTML 또는 JSON 문서와 같은 다른 원본의 비정형 데이터, 심지어 일반 텍스트 데이터를 사용하여 집계되는 상황을 지원합니다.
...
이 자습서에서는 C#, Azure Cognitive Search 용 .net SDK 및 Azure Portal를 사용 하 여 다음 작업을 수행 합니다.
...
- 샘플 호텔 데이터를 저장하기 위한 [Azure Cosmos DB 계정을 만듭니다](https://docs.microsoft.com/azure/cosmos-db/create-cosmosdb-resources-portal).
...
1. Visual Studio를 열고 Microsoft Azure Cognitive Search NuGet 패키지를 설치 합니다 (아직 설치 하지 않은 경우). **도구** 메뉴에서 **nuget 패키지 관리자** 를 선택 하 고 **솔루션에 대 한 nuget 패키지 관리**...를 선택 합니다. **찾아보기** 탭에서 **9.0.1 (버전** 이상)를 찾아서 설치 합니다. 설치를 완료하려면 추가 대화 상자를 클릭해야 합니다.
...
Azure Cognitive Search 서비스와 상호 작용 하려면 서비스 URL 및 액세스 키가 필요 합니다. 검색 서비스는 둘 다로 생성 되므로 Azure Cognitive Search를 구독에 추가한 경우 다음 단계를 수행 하 여 필요한 정보를 가져옵니다.
...
처음 두 항목은 Azure Cognitive Search 서비스에 대 한 URL 및 관리 키를 사용 합니다. 예를 들어 엔드포인트가 `https://mydemo.search.windows.net`으로 지정되면 제공할 서비스 이름은 `mydemo`입니다.
...
Azure Cognitive Search에서 키 필드는 인덱스의 각 문서를 고유 하 게 식별 합니다. 모든 검색 인덱스는 `Edm.String`형식의 키 필드를 정확히 하나만 포함해야 합니다. 이 키 필드는 인덱스에 추가되는 데이터 원본의 각 문서에 대해 있어야 합니다. (실제로 유일한 필수 필드임)
...
Azure Cognitive Search 인덱서는 인덱싱 프로세스 중에 필드 매핑을 사용 하 여 데이터 필드의 이름을 변경 하 고 서식을 다시 지정 하 여 원본 데이터를 올바른 인덱스 필드로 전송할 수 있습니다.

예를 들어 샘플 Azure Cosmos DB 데이터에서 호텔 식별자는 **HotelId**라고 합니다. 그러나 호텔 방에 대 한 JSON blob 파일에서 호텔 식별자는 **Id**로 지정 됩니다. 이 프로그램은 blob의 **Id** 필드를 인덱스의 **HotelId** 키 필드에 매핑하여이를 처리 합니다.
...
* C# 호텔 클래스의 데이터 구조를 기반으로 새 Azure Cognitive Search 인덱스를 만듭니다 (Address 및 Room 클래스도 참조).
...
* Azure Blob Storage 인덱서를 실행하여 Rooms 데이터를 로드합니다.
 ...
 프로그램을 실행하기 전에 잠시 시간을 내어 이 샘플에 대한 코드, 인덱스 및 인덱서 정의를 살펴봅니다. 관련된 코드가 다음과 같은 두 개의 파일에 있습니다.
  ...
  + **Program.cs** 에는 Azure Cognitive Search 인덱스, 데이터 원본 및 인덱서를 만들고 인덱스에 결합 된 결과를 로드 하는 함수가 포함 되어 있습니다.

### <a name="define-the-index"></a>인덱스 정의

이 샘플 프로그램은 .NET SDK를 사용 하 여 Azure Cognitive Search 인덱스를 정의 하 고 만듭니다. [FieldBuilder](https://docs.microsoft.com/dotnet/api/microsoft.azure.search.fieldbuilder) 클래스를 활용하여 C# 데이터 모델 클래스에서 인덱스 구조를 생성합니다.
...
자습서 후 정리 하는 가장 빠른 방법은 Azure Cognitive Search 서비스를 포함 하는 리소스 그룹을 삭제 하는 것입니다. 리소스 그룹을 삭제하여 이제 리소스 그룹 내의 모든 항목을 영구 삭제할 수 있습니다. 포털에서 리소스 그룹 이름은 Azure Cognitive Search 서비스의 개요 페이지에 있습니다.
...
> [Azure Cognitive Search Blob 인덱서를 사용 하 여 JSON blob을 인덱싱하는 방법](search-howto-index-json-blobs.md)

비정형 Blob 또는 전체 텍스트 콘텐츠에서 인지적으로 강화된 데이터를 사용하여 하나의 데이터 원본에 있는 정형 인덱스 데이터를 보강할 수도 있습니다. 다음 자습서에서는 .NET SDK를 사용 하 여 Azure Cognitive Search와 함께 Cognitive Services를 사용 하는 방법을 보여 줍니다.

> [!div class="nextstepaction"]
> [Azure Cognitive Search 인덱싱 파이프라인에서 Cognitive Services API 호출](cognitive-search-tutorial-blob-dotnet.md)
