...
ms.locfileid: "77212554"
---
# <a name="adjust-capacity-in-azure-cognitive-search"></a>Azure Cognitive Search의 용량 조정

[검색 서비스를 프로 비전](search-create-service-portal.md) 하 고 특정 가격 책정 계층에서 잠금을 설정 하기 전에 몇 분 정도는 서비스에서 복제본 및 파티션의 역할을 이해 하 고, 상대적으로 더 크거나 더 빠른 파티션이 필요한 지 여부 및 예상 되는 부하에 맞게 서비스를 구성 하는 방법을 이해 하는 것입니다.

Capacity는 [선택 하는 계층](search-sku-tier.md) 의 기능 (계층은 하드웨어 특성을 결정)과 프로젝션 된 워크 로드에 필요한 복제본 및 파티션 조합입니다. 이 문서에서는 복제본 및 파티션 조합 및 상호 작용에 대해 집중적으로 설명 합니다.
...
단일 서비스에는 모든 워크로드(인덱싱 및 쿼리)를 처리할 만큼 충분한 리소스가 있어야 합니다. 작업은 백그라운드에서 실행 되지 않습니다. 쿼리 요청이 자연스럽 게 자주 발생 하지 않는 시간에 대 한 인덱싱을 예약할 수 있지만, 서비스는 다른 작업에 대해 하나의 작업에 우선 순위를 지정 하지 않습니다.
...
프로덕션 서비스에서는 SLA를 위해 3 개의 복제본을 할당 해야 합니다. 쿼리 성능이 저하 되는 경우 복제본을 추가 하 여 더 큰 쿼리 작업을 지원 하 고 여러 복제본에 대 한 요청의 부하를 분산 하기 위해 인덱스의 추가 복사본을 온라인 상태로 만들 수 있습니다.
...
[선택한 계층](search-sku-tier.md) 은 파티션 크기와 속도를 결정 하 고 각 계층은 다양 한 시나리오에 맞는 특성 집합을 중심으로 최적화 됩니다. 더 높은 수준의 계층을 선택 하는 경우 S1로 이동 하는 경우 보다 더 많은 파티션이 필요할 수 있습니다.
