...
Azure Cognitive Search와 함께 사용할 [OData 필터 식을](query-odata-filter-orderby-syntax.md) 작성할 때 컬렉션 필드에 대해 필터링 하는 것이 유용한 경우가 많습니다. `any` 및 `all` 연산자를 사용 하 여이를 달성할 수 있습니다.

## <a name="syntax"></a>구문

다음 EBNF ([Extended Backus-Backus-naur Form](https://en.wikipedia.org/wiki/Extended_Backus–Naur_form))은 `any` 또는 `all`를 사용 하는 OData 식의 문법을 정의 합니다.
...
- 처음 두 개는 컬렉션 필드를 반복 하 여 람다 식의 형식으로 제공 된 조건자를 컬렉션의 각 요소에 적용 합니다.
  - `all`를 사용 하는 식은 조건자가 컬렉션의 모든 요소에 대해 true 인 경우 `true`을 반환 합니다.
  - `any`를 사용 하는 식은 컬렉션의 요소 중 하나 이상에 대해 조건자가 true 인 경우 `true`을 반환 합니다.
- 세 번째 형태의 컬렉션 필터는 람다 식 없이 `any`를 사용 하 여 컬렉션 필드가 비어 있는지 여부를 테스트 합니다. 컬렉션에 요소가 있으면 `true`반환 됩니다. 컬렉션이 비어 있으면 `false`반환 합니다.
...
모든 대화방에 대해 `rooms/amenities` 필드에 "tv"가 포함 되 고 `rooms/baseRate`가 100 보다 작은 문서를 찾습니다.
