---
title: '자습서: Azure SQL 데이터베이스에서 C#의 데이터 인덱싱'
...
ms.locfileid: "77650085"
---
# <a name="tutorial-index-azure-sql-data-in-c-using-azure-cognitive-search-indexers"></a>자습서: Azure Cognitive Search 인덱서를 사용하여 C#에서 Azure SQL 데이터 인덱싱

C#을 사용하여 Azure SQL 데이터베이스에서 검색 가능한 데이터를 추출하고 검색 인덱스로 보내는 [인덱서](search-indexer-overview.md)를 구성합니다. 이 자습서에서는 [Azure Cognitive Search .NET 클라이언트 라이브러리](https://aka.ms/search-sdk) 및 .NET Core 콘솔 애플리케이션을 사용하여 다음 작업을 수행합니다.
...
## <a name="download-source-code"></a>원본 코드 다운로드
...
1. 솔루션 탐색기에서 **appsettings.json**을 열고, 자리 표시자 값을 검색 서비스에 대한 연결 정보로 바꿉니다. 전체 URL이 "https://my-demo-service.search.windows.net"인 경우 제공할 서비스 이름은 "my-demo-service"입니다.
...
이 단계에서는 인덱서에서 크롤링할 수 있는 외부 데이터 원본을 Azure SQL Database에 만듭니다. Azure Portal 및 샘플의 *hotels.sql* 파일을 사용하여 Azure SQL Database에서 데이터 세트를 만들 수 있습니다. Azure Cognitive Search는 뷰 또는 쿼리에서 생성된 일반 행 집합을 사용합니다. 샘플 솔루션에 있는 SQL 파일은 단일 테이블을 만들고 채웁니다.
...
1. Visual Studio에 있는 **appsettings.json** 파일에서 세 번째 항목으로 "AzureSqlConnectionString"에 연결 문자열을 붙여넣습니다.
...
1. 암호를 **appsettings.json** 파일의 연결 문자열에 입력합니다. 데이터베이스와 사용자 이름은 연결 문자열에 복사되지만 암호는 수동으로 입력해야 합니다.

## <a name="build-the-solution"></a>솔루션을 빌드합니다.

F5 키를 눌러 솔루션을 빌드합니다. 디버그 모드로 프로그램을 실행합니다. 콘솔 창에서는 각 작업의 상태를 보고합니다.
...
이제 샘플 코드에서 만드는 항목을 알아보았으므로 솔루션으로 돌아가서 코드를 검토해 보겠습니다. 관련 코드는 다음 두 파일에 있습니다.
...
Azure Cognitive Search에서 인덱서는 여러 Azure 데이터 원본에 사용할 수 있습니다. 다음 단계로 Azure Blob 스토리지용 인덱서를 살펴봅니다.
