...
Azure 검색의 **인덱서**는 외부 데이터 소스에서 검색 가능한 데이터와 메타데이터를 추출하고 인덱스와 데이터 소스 간 필드 대 필드 매핑을 기초로 인덱스를 입력하는 크롤러입니다. 데이터를 인덱스에 푸시하기 위해 코드를 작성할 필요 없이 서비스가 데이터를 끌어오므로 이 방법을 종종 '끌어오기 모델'이라고도 합니다.
...
인덱서는 연결 문자열 등의 정보가 담긴 **데이터 소스**에서 데이터를 가져옵니다. 현재 지원되는 데이터 소스는 다음과 같습니다.
...
[.NET SDK](https://msdn.microsoft.com/library/azure/microsoft.azure.search.iindexersoperations.aspx) 및 [서비스 REST API](https://msdn.microsoft.com/library/azure/dn946891.aspx) 모두 인덱서 및 데이터 소스 관리를 지원합니다. 또는 **데이터 가져오기** 마법사를 사용할 때 포털에서 인덱서를 구성할 수도 있습니다.
