...
인덱서를 데이터 수집에 대 한 유일한 수단으로 사용 하거나, 인덱스에 있는 필드 중 일부를 로드 하는 것을 포함 하는 기술의 조합을 사용 하 여 필요에 따라 콘텐츠를 변환 하거나 보강 수 있습니다. 다음 표에는 주요 시나리오가 요약 되어 있습니다.
...
SDK를 사용 하는 경우 [Searchindexerclient](/dotnet/api/azure.search.documents.indexes.searchindexerclient) 를 만들어 인덱서, 데이터 원본 및 기술력과 작업을 수행 합니다. 위의 링크는 .NET SDK에 대 한 것 이지만 모든 Sdk는 SearchIndexerClient 및 유사한 Api를 제공 합니다.

처음에는 새 데이터 원본이 미리 보기 기능으로 발표 되 고 REST 전용입니다. 일반 공급으로 졸업 한 후에는 포털에서 전체 지원이 기본 릴리스 일정에 따라 다양 한 Sdk에 기본 제공 됩니다.
...
인덱서는 *데이터 원본* 개체에서 데이터 원본 연결을 가져옵니다. 데이터 원본 정의는 연결 문자열 및 자격 증명을 제공 합니다. [Create Datasource](/rest/api/searchservice/create-data-source) REST API 또는 [SearchIndexerDataSourceConnection 클래스](/dotnet/api/azure.search.documents.indexes.models.searchindexerdatasourceconnection) 를 호출 하 여 리소스를 만듭니다.
...
> [!NOTE]
> API 실행이 성공 코드를 반환 하는 경우 인덱서 호출이 예약 되었지만 실제 처리는 비동기적으로 수행 됩니다. 

포털에서 또는 [인덱서 상태 가져오기 API](/rest/api/searchservice/get-indexer-status)를 통해 인덱서 상태를 모니터링할 수 있습니다. 
