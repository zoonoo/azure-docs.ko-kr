...
`mergeOrUpload` | 이 작업은 지정된 키를 포함하는 문서가 인덱스에 이미 있는 경우 `merge`와 동일하게 작동합니다. 문서가 없으면 새 문서가 있는 `upload`과 동일하게 작동합니다. | 키, 더하기 정의하려는 기타 필드 | - `delete` | 지정된 문서를 인덱스에서 제거합니다. | 키만 해당 | 키 필드 이외에 지정한 필드는 무시됩니다. 문서에서 개별 필드를 제거하려는 경우 대신 `merge`를 사용하고 필드를 명시적으로 Null로 설정합니다.
