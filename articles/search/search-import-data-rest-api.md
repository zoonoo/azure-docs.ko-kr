<properties
    pageTitle="REST API를 사용하여 Azure 검색에 데이터 가져오기 | Microsoft Azure | 호스트된 클라우드 검색 서비스"
...
이 문서에서는 [Azure 검색 REST API](https://msdn.microsoft.com/library/azure/dn798935.aspx)를 사용하여 Azure 검색 인덱스에 데이터를 가져오는 방법을 보여줍니다. 이 연습을 시작하기 전에 [Azure 검색 인덱스를 만들어야](search-create-index-rest-api.md) 합니다.
...
`mergeOrUpload` | 이 작업은 지정된 키를 포함하는 문서가 인덱스에 이미 있는 경우 `merge`와 동일하게 작동합니다. 문서가 없으면 새 문서가 있는 `upload`과 동일하게 작동합니다. | 키, 더하기 정의하려는 기타 필드 | - `delete` | 지정된 문서를 인덱스에서 제거합니다. | 키만 해당 | 키 필드 이외에 지정한 필드는 무시됩니다. 개별 필드를 문서에서 제거하려는 경우에는 대신 *병합*을 사용하고 필드를 명시적으로 null로 설정합니다.

## III. HTTP 요청 및 요청 본문 생성
...
이 경우 `upload`, `mergeOrUpload` 및 `delete`을 검색 작업으로 사용합니다.

이 예제 "호텔" 인덱스는 문서 수로 채워진다고 가정합니다. `merge`을 사용하는 경우 가능한 문서 필드를 모두 지정하지 않는 방법 및 `delete`을 사용하는 경우 문서 키(`hotelId`)를 지정하는 방법입니다.

또한 단일 인덱싱 요청에서 1000개의 문서(또는 16MB)까지 포함할 수 있습니다.

## IV. HTTP 응답 코드 이해
#### 200
성공적인 인덱싱 요청을 제출한 후에 `200 OK`이라는 상태 코드로 HTTP 응답을 받게됩니다. HTTP 응답의 JSON 본문은 다음과 같습니다.
...
> [AZURE.NOTE] 대개는 검색 서비스에 대한 부하가 인덱싱 요청이 `503` 응답을 반환하기 시작하는 지점에 도달한 것을 의미합니다. 이 경우 다시 시도하기 전에 클라이언트 코드를 백오프하고 대기하는 것이 좋습니다. 이렇게 하면 시스템이 복구할 시간을 제공하여 이후 요청이 성공할 가능성이 높아집니다. 신속하게 요청을 다시 시도하면 이 상황만 연장됩니다.

#### 429
인덱스당 문서 수의 할당량이 초과된 경우 상태 코드 `429`이 반환됩니다.

#### 503
요청에서 어떤 항목도 성공적으로 인덱싱되지 않는 경우 상태 코드 `503`이 반환됩니다. 이 오류는 시스템의 사용량이 많아 지금 요청을 처리할 수 없다는 것을 나타냅니다.

> [AZURE.NOTE] 이 경우 다시 시도하기 전에 클라이언트 코드를 백오프하고 대기하는 것이 좋습니다. 이렇게 하면 시스템이 복구할 시간을 제공하여 이후 요청이 성공할 가능성이 높아집니다. 신속하게 요청을 다시 시도하면 이 상황만 연장됩니다.

문서 동작 및 성공/오류 응답에 대한 자세한 내용은 [이 페이지](https://msdn.microsoft.com/library/azure/dn798925.aspx)를 참조하세요. 오류가 발생한 경우 반환될 수 있는 기타 HTTP 상태 코드에 대한 자세한 내용은 [이 문서](https://msdn.microsoft.com/library/azure/dn798925.aspx)를 참조하세요.

## 다음
Azure 검색 인덱스를 채운 후에 문서를 검색하기 위해 쿼리를 발급하기 시작할 준비가 되었습니다.
