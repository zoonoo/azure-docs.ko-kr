...
Azure Search에서 [서비스는 서비스 수명 동안 수정되는 가격 책정 계층 또는 SKU에서 프로비전](search-create-service-portal.md)됩니다. 계층에는 **무료**, **기본**, **표준**이 있으며 **표준**은 여러 구성 및 용량으로 사용 가능합니다. 대부분의 고객은 평가를 위해 **무료** 계층으로 시작한 다음, 개발 및 프로덕션 배포를 위해 **표준** 계층으로 전환합니다. 리소스 집약적 인식 검색의 경우를 비롯한 모든 빠른 시작 및 자습서를 **무료** 계층에서 완료할 수 있습니다. 
...
Azure Search에서 이해할 가장 중요한 청구 개념은 SU(*검색 단위*)입니다. Azure Search은 기능하려면 복제본과 파티션 둘 다에 따라 달라지므로 하나만으로 청구하는 것은 의미가 없습니다. 대신, 청구는 둘 다의 복합을 기준으로 합니다. 
...
비용을 절감할 수 있는 한 가지 요인은 시간별 요금이 더 낮은 계층을 선택하는 것입니다. S1 시간별 요금은 S2 또는 S3 요금보다 낮습니다. 예측한 부하의 가장 낮은 값을 목표로 서비스를 프로비전할 수 있습니다. 서비스 규모를 늘리려는 경우 더 큰 두 번째 계층 서비스를 만들고, 해당 두 번째 서비스에서 인덱스를 다시 작성한 후 첫 번째 서비스를 삭제합니다. 온-프레미스 서버의 용량 계획을 완료한 경우 예상된 증가를 처리할 수 있도록 “완전히 구입”하는 것이 일반적일 것입니다. 하지만 클라우드 서비스를 사용하는 경우 특정 구매에 잠겨 있지 않으므로 비용 절감을 보다 적극적으로 모색할 수 있습니다. 현재 서비스가 충분하지 않은 경우 항상 더 높은 계층 서비스로 전환할 수 있습니다.
