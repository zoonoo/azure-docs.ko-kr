---
title: 결과를 트리밍하기 위한 보안 필터
titleSuffix: Azure Cognitive Search
description: 보안 필터 및 사용자 ID를 사용하여 Azure Cognitive Search 콘텐츠에 대한 액세스 제어
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
...
ms.locfileid: "72794273"
---
# <a name="security-filters-for-trimming-results-in-azure-cognitive-search"></a>Azure 인지 검색에서 결과 트리밍을 위한 보안 필터

보안 필터를 적용하여 사용자 ID를 기반으로 Azure Cognitive Search에서 검색 결과를 트리밍할 수 있습니다. 이 검색 환경에서는 일반적으로 검색을 요청하는 사람의 ID를 누가 문서에 대한 권한을 갖는지에 대한 원칙을 포함하는 필드와 비교해 보는 것이 필요합니다. 일치하는 항목이 발견되면, 사용자 또는 보안 주체(예: 그룹 또는 역할)는 해당 문서에 액세스할 수 있습니다.
...
이 문서에서는 Azure [구독,](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A261C142F) [Azure 인지 검색 서비스](https://docs.microsoft.com/azure/search/search-create-service-portal)및 Azure [인지 검색 인덱스가](https://docs.microsoft.com/azure/search/search-create-index-portal)있다고 가정합니다.  
...
Azure 인지 검색을 사용하여 문서를 검색하는 방법에 대한 자세한 내용은 [문서 검색](https://docs.microsoft.com/rest/api/searchservice/search-documents)을 참조할 수 있습니다.
...
사용자 ID 및 Azure 인지 검색 `search.in()` 기능을 기반으로 결과를 필터링하는 방법입니다. 이 함수를 사용하여 요청하는 사용자가 각 대상 문서와 연결된 주 식별자와 일치하도록 요청하는 원칙적으로 식별자를 전달할 수 있습니다. 검색 요청이 처리될 때, `search.in` 함수는 사용자의 보안 주체 중 아무도 읽기 권한이 없는 검색 결과를 필터링합니다. 보안 주체 식별자는 보안 그룹, 역할 또는 심지어 사용자 본인 ID와 같은 것을 나타낼 수 있습니다.
