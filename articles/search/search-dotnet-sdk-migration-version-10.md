...
* 경우에 따라 [인덱서 실행 상태](https://docs.microsoft.com/rest/api/searchservice/get-indexer-status) 에 표시 되는 오류 및 경고에는 디버깅에 도움이 되는 추가 정보가 있을 수 있습니다. 이 동작을 반영 하도록 `IndexerExecutionResult` 업데이트 되었습니다.
* [기술](cognitive-search-defining-skillset.md) 내에 정의 된 개별 기술은 `name` 속성을 지정 하 여 선택적으로 식별할 수 있습니다.
* `ServiceLimits`은 [복합 형식](https://docs.microsoft.com/azure/search/search-howto-complex-data-types) 에 대 한 제한을 표시 하 고 `IndexerExecutionInfo` 관련 인덱서 제한/할당량을 표시 합니다.

<a name="UpgradeSteps"></a>

## <a name="steps-to-upgrade"></a>업그레이드 단계

1. NuGet 패키지 관리자 콘솔을 사용 하거나 프로젝트 참조를 마우스 오른쪽 단추로 클릭 하 고 "NuGet 패키지 관리 ..."를 선택 하 여 `Microsoft.Azure.Search`에 대 한 NuGet 참조를 업데이트 합니다. Visual Studio에서
...
지정 된 `WebApiSkill`에 대 한 모델은 사전을 _포함_ 하는 개체 속성으로 `HttpHeaders`. 이 방식으로 생성 된 `WebApiSkill`를 사용 하 여 기술를 만들면 REST API는 요청의 형식이 잘못 된 것으로 간주 되기 때문에 예외가 발생 합니다. 이 문제는 `WebApiSkill` 모델 자체에서 **최상위 사전 속성을** `HttpHeaders` 하 여 (REST API에서 유효한 요청으로 간주 됨) 수정 되었습니다.

예를 들어 이전에 다음과 같이 `WebApiSkill`를 인스턴스화하려고 시도한 경우
...
Shaper 스킬은 이제 중첩 된 컨텍스트에서 입력 통합을 허용할 수 있습니다. 이러한 변경을 사용 하려면 `Source` 속성만 지정 하거나 `SourceContext` 및 `Inputs` 속성 모두를 지정 하 여 인스턴스화할 수 있도록 `InputFieldMappingEntry` 수정 했습니다.

코드를 변경 하지 않아도 되는 경우가 많습니다. 그러나 이러한 두 조합 중 하나만 사용할 수 있습니다. 이것은 다음을 의미합니다.

- `Source` 초기화 된 `InputFieldMappingEntry`를 만드는 경우에만 유효 합니다.
- `SourceContext` 및 `Inputs` 초기화 된 `InputFieldMappingEntry`를 만듭니다.
...
이제 기술 내의 각 기술에는 새 속성 `Name`있으며,이를 통해 코드에서 초기화 하 여 기술 식별에 도움이 됩니다. 이 옵션은 선택 사항입니다. 지정 하지 않으면 (명시적 코드가 변경 되지 않은 경우 기본값) 기술의 기술에 대 한 1 기반 인덱스를 사용 하 여 기본 이름이 지정 되 고 접두사는 ' # ' 문자로 지정 됩니다. 예를 들어 다음 기술 정의에서 (간단히 하기 위해 대부분의 초기화를 건너뛰었습니다.)
...
인덱서를 실행 하는 동안 발생 하는 오류 및 경고에 대 한 세부 정보를 캡슐화 하는 `ItemError` 및 `ItemWarning` 모델은 인덱서를 디버깅 하는 데 도움이 되는 목표가 포함 된 세 개의 새로운 속성을 포함 하도록 수정 되었습니다. 이러한 속성은 다음과 같습니다.

- `Name`: 오류가 발생 한 원본의 이름입니다. 예를 들어 연결 된 기술의 특정 기술을 참조할 수 있습니다.
- `Details`: 오류 또는 경고에 대 한 자세한 정보를 추가로 표시 합니다.
