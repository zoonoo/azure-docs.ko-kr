---
title: Azure Search .NET SDK 버전 10으로 업그레이드 Azure Search
...
ms.locfileid: "70182375"
---
# <a name="upgrade-to-the-azure-search-net-sdk-version-10"></a>Azure Search .NET SDK 버전 10으로 업그레이드
...
Azure Search .net SDK 버전 10은 이러한 업데이트를 사용 하 여 일반적으로 사용 가능한 최신 버전`2019-05-06`의 Azure Search REST API ()를 대상으로 합니다.
...
* `ServiceLimits`[복합 형식의](https://docs.microsoft.com/azure/search/search-howto-complex-data-types) `IndexerExecutionInfo` 제한을 보여 주고 관련 인덱서 제한/할당량을 표시 합니다.
...
사전을 포함 하 `WebApiSkill` 는 개체 속성으로 지정 된 `HttpHeaders` 의 모델입니다. 이 방식으로 생성 된 `WebApiSkill` 를 사용 하 여 기술를 만들면 REST API는 요청이 잘못 된 형식으로 간주 되기 때문에 예외가 발생 합니다. 이 문제는 `WebApiSkill` 모델 자체에 대 한 `HttpHeaders` **최상위 사전 속성** (REST API의 유효한 요청으로 간주 됨)을 만들어 수정 되었습니다.
...
Shaper 스킬은 이제 중첩 된 컨텍스트에서 입력 통합을 허용할 수 있습니다. `InputFieldMappingEntry` 이 변경을 사용 하려면 `Source` `Inputs` 속성만 지정 하거나 및 속성을 모두 지정 하 여 인스턴스화할 수 있도록 수정 했습니다. `SourceContext`

코드를 변경 하지 않아도 되는 경우가 많습니다. 그러나 이러한 두 조합 중 하나만 사용할 수 있습니다. 즉,

- `InputFieldMappingEntry` 만`Source` 초기화 되는를 만들 수 있습니다.
- `InputFieldMappingEntry` 및가초기화되는를만드는`Inputs` 것은 유효 합니다. `SourceContext`
...
`SentimentSkill`에는 `#1`이름, `ShaperSkill` `#3` 할당 됨, 할당 됨 등이 할당 됩니다. `#2` `WebApiSkill`

사용자 지정 이름으로 기술을 식별 하도록 선택 하는 경우 먼저 클라이언트의 모든 인스턴스를 SDK 버전 10으로 업데이트 해야 합니다. 그렇지 않으면 이전 버전의 SDK `null` `Name` 를 사용 하는 클라이언트가 기술에 대 한 속성을 사용 하 여 클라이언트가 기본 이름 지정 체계를 대체 하도록 할 가능성이 있습니다.
...
- `Name`: 오류가 발생 한 소스의 이름입니다. 예를 들어 연결 된 기술의 특정 기술을 참조할 수 있습니다.
...
- Shaper 기술에 대 한 변경 내용은 새 코드나 기존 코드에 대 한 가능성이 가장 높습니다. 다음 단계로 입력 구조를 보여 주는이 예제를 다시 방문 해야 합니다. [Shaper 기술 JSON 정의 예제](cognitive-search-skill-shaper.md)
- [인식 검색 소개 가이드를 참조](cognitive-search-concept-intro.md)하세요.
