...
이 문서에서는 Azure 방화벽을 사용하여 HDInsight 클러스터에서 아웃바운드 트래픽을 보호하는 단계를 제공합니다. 아래 단계는 기존 클러스터에 대한 Azure 방화벽을 구성한다고 가정합니다. 방화벽 뒤에 새 클러스터를 배포하는 경우 먼저 HDInsight 클러스터와 서브넷을 만듭니다. 그런 다음 이 가이드의 단계를 따릅니다.

## <a name="background"></a>배경

HDInsight 클러스터는 일반적으로 가상 네트워크에 배포됩니다. 클러스터는 해당 가상 네트워크 외부의 서비스에 대한 종속성을 가집니다.
...
HDInsight 아웃바운드 트래픽 종속성은 거의 전적으로 FQDN으로 정의됩니다. 뒤에 정적 IP 주소가 없습니다. 정적 주소가 없다는 것은 NSG(네트워크 보안 그룹)가 클러스터에서 아웃바운드 트래픽을 잠글 수 없다는 것을 의미합니다. 주소는 현재 이름 확인 및 사용에 따라 규칙을 설정할 수 없을 정도로 자주 변경됩니다.

도메인 이름을 기반으로 아웃바운드 트래픽을 제어할 수 있는 방화벽을 사용하여 아웃바운드 주소를 보호합니다. Azure 방화벽은 대상 또는 [FQDN 태그의](../firewall/fqdn-tags.md)FQDN을 기반으로 아웃바운드 트래픽을 제한합니다.
...
1. 응용 **프로그램 규칙 컬렉션 추가** 화면에서 다음 정보를 제공합니다.
    ...
    **대상 FQDNs 섹션**
    ...
    | 속성 | 소스 주소 | `Protocol:Port` | 대상 FQDNS | 메모 |
    | --- | --- | --- | --- | --- |
...
1. 네트워크 **규칙 수집 추가** 화면에서 다음 정보를 제공합니다.
    ...
    **IP 주소 섹션**
    ...
    | Rule_3 | TCP | * | 데이터 레이크 스토리지 계정의 IP 주소 | * | Azure Data Lake Storage를 사용하는 경우 IP 주소 섹션에 네트워크 규칙을 추가하여 ADLS Gen1 및 Gen2의 SNI 문제를 해결할 수 있습니다. 이 옵션은 트래픽을 방화벽으로 라우팅합니다. 이로 인해 큰 데이터 로드에 대한 비용이 증가할 수 있지만 트래픽은 방화벽 로그에 기록되고 감사할 수 있습니다. 데이터 레이크 스토리지 계정의 IP 주소를 확인합니다. FQDN을 IP 주소로 해결하는 것과 같은 `[System.Net.DNS]::GetHostAddresses("STORAGEACCOUNTNAME.blob.core.windows.net")` PowerShell 명령을 사용할 수 있습니다.|
    ...
    **서비스 태그 섹션**
    ...
    | --- | --- | --- | --- | --- | --- |
    | Rule_7 | TCP | * | SQL | 1433 | SQL 트래픽을 기록하고 감사할 수 있도록 SQL에 대한 서비스 태그 섹션의 네트워크 규칙을 구성합니다. HDInsight 서브넷에서 SQL Server용 서비스 엔드포인트를 구성하지 않으면 방화벽을 우회합니다. |
...
1. 연결 **서브넷** 화면에서 클러스터가 생성된 가상 네트워크를 선택합니다. 그리고 HDInsight 클러스터에 사용한 **서브넷입니다.**
...
Azure 방화벽을 Azure 모니터 로그와 통합하면 응용 프로그램이 처음 작동할 때 유용합니다. 특히 모든 응용 프로그램 종속성을 인식하지 못하는 경우. Azure Monitor 로그에 대한 자세한 내용은 [Azure Monitor에서 로그 데이터 분석](../azure-monitor/log-query/log-query-overview.md)을 참조하세요.
...
이전 지침은 HDInsight 클러스터에서 아웃바운드 트래픽을 제한하기 위해 Azure 방화벽을 구성하는 데 도움이 됩니다. Azure 방화벽은 많은 일반적인 중요한 시나리오에 대한 트래픽을 허용하도록 자동으로 구성됩니다. 다른 네트워크 가상 어플라이언스를 사용하려면 여러 가지 추가 기능을 구성해야 합니다. 네트워크 가상 어플라이언스를 구성할 때 다음 요소를 염두에 두어야 합니다.
...
| [여기에](hdinsight-management-ip-addresses.md) 게시된 IP | 이러한 IP는 HDInsight 서비스입니다. |
