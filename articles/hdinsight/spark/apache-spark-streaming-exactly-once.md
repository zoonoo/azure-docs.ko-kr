...
또 다른 예는 Azure Storage Blob이나 Azure Data Lake Store와 같은 분할된 파일 시스템을 사용하는 것입니다. 이 경우 싱크 논리는 파일의 존재 여부를 확인할 필요가 없습니다. 이벤트를 나타내는 파일이 존재하면 단순히 동일한 데이터로 덮어씁니다. 그렇지 않은 경우 새 파일이 지정된 경로에 만들어집니다.
