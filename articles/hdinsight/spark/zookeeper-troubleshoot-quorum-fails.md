...
Apache ZooKeeper 서버가 비정상으로 나타날 수 있습니다 .이는 리소스 관리자/이름 노드가 모두 대기 모드이 고, 단순 HDFS 작업이 작동 `zkFailoverController` 하지 않거나, 중지 되 고, 시작할 수 없는 경우, Yarn/Spark/Livy 작업이 사육 아웃 오류로 인해 실패 합니다. 다음과 유사한 오류 메시지가 표시 될 수 있습니다.
...
사육 사 데이터 디렉터리 `/hadoop/zookeeper/version-2` 를 `/hadoop/hdinsight-zookeepe/version-2` 확인 하 고 스냅숏 파일 크기가 큰지 확인 합니다. 규모가 많은 스냅숏이 있는 경우 다음 단계를 수행 합니다.
