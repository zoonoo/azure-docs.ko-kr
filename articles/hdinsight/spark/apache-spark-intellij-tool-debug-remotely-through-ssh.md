...
7. 오른쪽 패널에 실행 중인 로그를 표시하지 않으려면 **연결 끊기** 단추를 클릭할 수 있습니다. 그러나 백엔드에서는 계속 실행 중이며 결과가 왼쪽 패널에 표시됩니다.
