...
1. **빌드 도구** 드롭다운 목록에서 다음 중 하나를 선택합니다.
    ...
    * **SBT** - 종속성 관리 및 Scala 프로젝트용 빌드의 경우
     ...
     :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-create-projectfor-debug-remotely.png" alt-text="Intellij 새 프로젝트 만들기 Spark" border="true":::
...
1. 다음 **새 프로젝트** 창에서 다음 정보를 제공 합니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-new-project.png" alt-text="Intellij 새 프로젝트 Spark 버전 선택" border="true":::
...
1. **클러스터에서 원격으로 실행** 탭으로 전환 합니다. **이름**, **Spark 클러스터** 및 **주 클래스 이름** 에 대 한 정보를 입력 합니다. 그런 다음 **고급 구성 (원격 디버깅)** 을 클릭 합니다. 이 도구는 **실행기** 를 사용하여 디버그를 지원합니다. **numExectors** 의 기본값은 5입니다. 3보다 큰 값을 설정하지 않는 것이 좋습니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-run-debug-configurations.png" alt-text="Intellij 디버그 구성 실행" border="true":::
...
1. 구성 설정을 완료한 후에 원격 클러스터에 대해 프로젝트를 실행하거나 원격 디버깅을 수행할 수 있습니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/perform-remote-run-button.png" alt-text="Intellij Debug 원격 Spark 작업 원격 실행 단추" border="true":::

1. **연결 해제** 단추를 클릭하면 제출 로그가 왼쪽 패널에 나타나지 않습니다. 그러나 백 엔드에서는 계속 실행되고 있습니다.

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/spark-remote-run-result.png" alt-text="Intellij Debug 원격 Spark 작업 원격 실행 결과" border="true":::

## <a name="perform-remote-debugging"></a>원격 디버깅 수행

1. 중단점을 설정한 다음 **원격 디버그** 아이콘을 클릭합니다. 원격 제출과의 차이점은 SSH 사용자 이름/암호를 구성해야 한다는 것입니다.

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debug-icon.png" alt-text="Intellij Debug 원격 Spark 작업 디버그 아이콘" border="true":::
...
1. 프로그램 실행이 중단점에 도달하면 **디버거** 창에 1개의 **드라이버** 탭과 2개의 **실행기** 탭이 표시됩니다. **프로그램 다시 시작** 아이콘을 선택하여 코드를 계속 실행합니다. 그러면 다음 중단점에 도달합니다. 디버깅할 대상 실행기를 찾으려면 올바른 **실행기** 탭으로 전환해야 합니다. 해당 **콘솔** 탭에서 실행 로그를 검토할 수 있습니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debugger-tab.png" alt-text="Intellij Debug 원격 Spark 작업 디버깅 탭" border="true":::
...
1. **프로그램 다시 시작** 아이콘을 선택하여 계속합니다. 두 번째 지점에서 코드가 중지됩니다. 예상대로 예외가 catch됩니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-throw-error.png" alt-text="Intellij Debug 원격 Spark 작업 throw 오류" border="true":::

1. **프로그램 다시 시작** 아이콘을 다시 선택합니다. **HDInsight Spark 제출** 창에 “작업 실행 실패” 오류가 표시됩니다.

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-error-submission.png" alt-text="Intellij Debug 원격 Spark 작업 오류 전송" border="true":::
...
1. **디버그** 탭에서 대상을 마우스 오른쪽 단추로 클릭한 다음 **값 설정** 을 선택합니다. 다음으로 변수의 새 값을 입력합니다. 그런 다음 **Enter** 키를 선택하여 값을 저장합니다.
   ...
   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-set-value1.png" alt-text="Intellij Debug 원격 Spark 작업 집합 값" border="true":::

1. **프로그램 다시 시작** 아이콘을 선택하여 프로그램을 계속 실행합니다. 이번에는 예외가 catch되지 않습니다. 예외 없이 프로젝트가 성공적으로 실행되는지 확인할 수 있습니다.

   :::image type="content" source="./media/apache-spark-intellij-tool-debug-remotely-through-ssh/hdinsight-debug-without-exception.png" alt-text="Intellij Debug 원격 Spark 작업 (예외 없음)" border="true":::
