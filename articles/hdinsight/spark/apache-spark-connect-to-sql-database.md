...
description: HDInsight Spark 클러스터와 Azure SQL Database 간의 연결을 설정 하 여 데이터를 읽고 데이터를 쓰고 SQL Database로 데이터를 스트리밍하는 방법에 대해 알아봅니다.
...
Azure SQL Database를 사용 하 여 Azure HDInsight에서 Apache Spark 클러스터를 연결 하 고 SQL Database에서 데이터를 읽고 쓰고 스트리밍하는 방법에 대해 알아봅니다. 이 문서의 지침은 [Jupyter Notebook](https://jupyter.org/)을 사용하여 Scala 코드 조각을 실행합니다. 그러나 Scala 또는 Python에 독립 실행형 애플리케이션을 만들고 동일한 작업을 수행할 수 있습니다.

## <a name="prerequisites"></a>필수 조건

* 클러스터를 Azure HDInsight Spark 합니다.  [HDInsight에서 Apache Spark 클러스터 만들기](apache-spark-jupyter-spark-sql.md)의 지침을 따르세요.

* Azure SQL Database [Azure SQL Database 만들기](../../sql-database/sql-database-get-started-portal.md)의 지침을 따릅니다. 샘플 **AdventureWorksLT** 스키마와 데이터를 사용해서 데이터베이스를 만들어야 합니다. 또한 클라이언트의 IP 주소로 서버의 SQL Database에 액세스하도록 허용하기 위한 서버 수준 방화벽 규칙을 만들어야 합니다. 방화벽 규칙을 추가하는 지침은 같은 문서에 제공됩니다. Azure SQL Database를 만든 후에는 다음 값을 편리 하 게 유지 해야 합니다. Spark 클러스터에서 데이터베이스에 연결할 때 필요합니다.

    * Azure SQL Database를 호스트 하는 서버 이름입니다.
    * Azure SQL Database 이름입니다.
    * 관리 사용자 이름/암호를 Azure SQL Database 합니다.
...
1. 이렇게 하면 기본 이름이 **제목 없음**인 새 노트가 열립니다. 노트 이름을 클릭하고 원하는 이름을 입력합니다.
    ...
    ![노트북 이름을 제공 합니다.](./media/apache-spark-connect-to-sql-database/hdinsight-spark-jupyter-notebook-name.png "노트북에 대한 이름 제공")
...
1. 새 Jupyter 노트북의 코드 셀에서 다음 코드 조각을 붙여넣고 자리 표시자 값을 Azure SQL Database 값으로 바꿉니다.
...
1. 아래 코드 조각을 사용 하 여 Spark 데이터 프레임 Api에 전달할 수 있는 JDBC URL을 빌드합니다. 이 코드는 매개 변수를 보유 하는 `Properties` 개체를 만듭니다. 이 코드 조각을 코드 셀에 붙여 넣고 **Shift+Enter**를 눌러 실행합니다.
...
1. 아래 코드 조각을 사용 하 여 Azure SQL Database 테이블의 데이터로 데이터 프레임을 만듭니다. 이 코드 조각에서는 **AdventureWorksLT** 데이터베이스의 일부로 사용할 수 있는 `SalesLT.Address` 테이블을 사용 합니다. 이 코드 조각을 코드 셀에 붙여 넣고 **Shift+Enter**를 눌러 실행합니다.
...
이 섹션에서는 클러스터에서 사용할 수 있는 샘플 CSV 파일을 사용 하 여 Azure SQL Database 테이블을 만들고 데이터로 채웁니다. 샘플 CSV 파일(**HVAC.csv**)은 `HdiSamples/HdiSamples/SensorSampleData/hvac/HVAC.csv`의 모든 HDInsight 클러스터에서 사용할 수 있습니다.
...
1. 다음 코드 조각은 Spark 데이터 프레임 Api에 전달할 수 있는 JDBC URL을 작성 합니다. 이 코드는 매개 변수를 보유 하는 `Properties` 개체를 만듭니다. 이 코드 조각을 코드 셀에 붙여 넣고 **Shift+Enter**를 눌러 실행합니다.
...
1. 마지막으로, hive 테이블을 사용 하 여 Azure SQL Database에 테이블을 만듭니다. 다음 코드 조각은 Azure SQL Database에서 `hvactable`를 만듭니다.

       spark.table("hvactable_hive").write.jdbc(jdbc_url, "hvactable", connectionProperties)

1. SSMS를 사용 하 여 Azure SQL Database에 연결 하 고 여기에 `dbo.hvactable`가 표시 되는지 확인 합니다.

    a. SSMS를 시작 하 고 아래 스크린샷에 표시 된 것 처럼 연결 정보를 제공 하 여 Azure SQL Database에 연결 합니다.
    ...
    b. **개체 탐색기**에서 Azure SQL Database를 확장 하 고 테이블 노드를 확장 하 여 만든 **hvactable** 를 확인 합니다.
...
이 섹션에서는 이전 섹션에서 Azure SQL Database에서 이미 만든 `hvactable`로 데이터를 스트리밍합니다.

1. 첫 번째 단계로 `hvactable`에 레코드가 없는지 확인 합니다. SSMS를 사용하여 테이블에 대해 다음 쿼리를 실행합니다.
...
1. 데이터를 **`hvactable`으로 스트리밍합니다.** HVAC csv 파일은 `/HdiSamples/HdiSamples/SensorSampleData/HVAC/`의 클러스터에서 사용할 수 있습니다. 다음 코드 조각에서는 먼저 스트리밍할 데이터의 스키마를 가져옵니다. 그런 다음 해당 스키마를 사용하여 스트리밍 데이터 프레임을 만듭니다. 이 코드 조각을 코드 셀에 붙여 넣고 **Shift+Enter**를 눌러 실행합니다.

       val userSchema = spark.read.option("header", "true").csv("wasbs:///HdiSamples/HdiSamples/SensorSampleData/hvac/HVAC.csv").schema
       val readStreamDf = spark.readStream.schema(userSchema).csv("wasbs:///HdiSamples/HdiSamples/SensorSampleData/hvac/") 
       readStreamDf.printSchema

1. 출력에는 **HVAC.csv**의 스키마가 표시됩니다. `hvactable`에도 동일한 스키마가 있습니다. 출력에는 테이블의 열이 표시됩니다.

    ![hdinsight Apache Spark 스키마 테이블](./media/apache-spark-connect-to-sql-database/hdinsight-schema-table.png "테이블의 스키마")

1. 마지막으로, 다음 코드 조각을 사용 하 여 HVAC .csv에서 데이터를 읽고 Azure SQL Database의 `hvactable`으로 스트리밍합니다. 코드 셀에 코드 조각을 붙여 넣고, 자리 표시자 값을 Azure SQL Database 값으로 바꾼 다음 **SHIFT + enter** 를 눌러 실행 합니다.
...
1. SSMS (SQL Server Management Studio)에서 다음 쿼리를 실행 하 여 데이터를 `hvactable` 스트리밍하는 지 확인 합니다. 쿼리를 실행할 때마다 테이블의 행 수가 증가합니다.
