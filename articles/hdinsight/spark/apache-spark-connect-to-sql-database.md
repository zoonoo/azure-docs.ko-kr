...
description: HDInsight 스파크 클러스터와 Azure SQL 데이터베이스 간에 연결을 설정하여 데이터를 읽고, 데이터를 작성하고, 데이터를 SQL 데이터베이스로 스트리밍하는 방법을 알아봅니다.
...
Azure HDInsight에서 아파치 스파크 클러스터를 Azure SQL 데이터베이스와 연결한 다음 데이터를 읽고 쓰고 SQL 데이터베이스로 스트리밍하는 방법을 알아봅니다. 이 문서의 지침은 [Jupyter Notebook](https://jupyter.org/)을 사용하여 Scala 코드 조각을 실행합니다. 그러나 Scala 또는 Python에 독립 실행형 애플리케이션을 만들고 동일한 작업을 수행할 수 있습니다.
...
* SQL 서버 관리 스튜디오(SSMS). [SSMS를 사용하여 데이터 연결 및 쿼리](../../sql-database/sql-database-connect-query-ssms.md)의 지침을 따릅니다.

## <a name="create-a-jupyter-notebook"></a>Jupyter Notebook 만들기

먼저 스파크 클러스터와 연결된 [Jupyter 노트북을](https://jupyter.org/) 만듭니다. 이 노트를 사용해서 이 문서에 사용된 코드 조각을 실행합니다.
