...
1. [https://github.com/Azure-Samples/hdinsight-pyspark-cntk-integration](https://github.com/Azure-Samples/hdinsight-pyspark-cntk-integration)를 다운로드 하 고 압축을 풉니다.

1. 웹 브라우저에서 `https://CLUSTERNAME.azurehdinsight.net/jupyter`로 이동 합니다. 여기서 `CLUSTERNAME`은 클러스터의 이름입니다.

1. Jupyter 노트북의 오른쪽 위 모서리에서 **업로드** 를 선택 하 고 다운로드로 이동 하 여 파일 `CNTK_model_scoring_on_Spark_walkthrough.ipynb`를 선택 합니다.
