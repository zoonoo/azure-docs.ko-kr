...
이 문서에서는 사용자 지정 PySpark (파이썬용) 및 아파치 스파크 (스칼라용) 커널과 스파크 마법으로 Jupyter 노트북을 설치하는 방법을 배웁니다. 그런 다음 노트북을 HDInsight 클러스터에 연결합니다.
...
사용자 지정 커널 및 스파크 매직에 대한 자세한 내용은 [HDInsight의 아파치 스파크 리눅스 클러스터가 있는 Jupyter 노트북에 사용할 수 있는 커널을](apache-spark-jupyter-notebook-kernels.md)참조하십시오.

## <a name="prerequisites"></a>사전 요구 사항

* HDInsight의 Apache Spark. 자세한 내용은 [Azure HDInsight에서 Apache Spark 클러스터 만들기](apache-spark-jupyter-spark-sql.md)를 참조하세요. 로컬 노트북이 HDInsight 클러스터에 연결됩니다.

* HDInsight의 Spark에서 Jupyter Notebook을 사용하는 방법 이해.

## <a name="install-jupyter-notebook-on-your-computer"></a>컴퓨터에 Jupyter 노트북 설치

Jupyter 노트북을 설치하기 전에 파이썬을 설치하십시오. [아나콘다 배포판은](https://www.anaconda.com/download/) 파이썬과 주피터 노트북을 모두 설치합니다.
...
6. 커널에서 사용할 수 있는 스파크 마법을 사용할 수 있는지 확인합니다. 다음 단계를 완료합니다.
...
컴퓨터에 Jupyter를 설치한 다음 HDInsight의 아파치 스파크 클러스터에 연결하는 이유:

* 전자 필기장을 로컬로 만들고, 실행 중인 클러스터에 대해 응용 프로그램을 테스트한 다음 전자 필기장을 클러스터에 업로드하는 옵션을 제공합니다. 전자 필기장을 클러스터에 업로드하려면 실행 중인 Jupyter 노트북이나 클러스터를 사용하여 전자 필기장을 업로드하거나 클러스터와 연결된 저장소 계정의 `/HdiNotebooks` 폴더에 저장할 수 있습니다. 클러스터에 Notebook을 저장하는 방법에 대한 자세한 내용은 [Jupyter Notebook이 저장되는 위치](apache-spark-jupyter-notebook-kernels.md#where-are-the-notebooks-stored)를 참조하세요.
...
* [BI를 이용한 아파치 스파크: HDInsight의 파워 BI를 사용하여 아파치 스파크 데이터 분석](apache-spark-use-bi-tools.md)
