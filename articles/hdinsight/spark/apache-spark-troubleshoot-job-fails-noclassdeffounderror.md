...
이 오류는 실행 중인 Kafvercluster의 버전과 다른 `spark-streaming-kafka` jar 파일의 버전을 지정 하 여 발생할 수 있습니다.
...
`–packages` 옵션과 함께 Spark-submit 명령을 사용 하 고 spark-스트리밍-kafka jar 파일의 버전이 실행 중인 kafvercluster의 버전과 동일한 지 확인 합니다.
