...
1. YARN UI를 시작합니다. 클러스터 대시보드 에서 **원사를** **선택합니다.**

    ![Azure 포털 시작 YARN UI](./media/apache-spark-job-debugging/launch-apache-yarn-ui.png)

   > [!TIP]  
   > 또는 Ambari UI에서 YARN UI를 시작할 수도 있습니다. Ambari UI를 시작하려면 클러스터 대시보드에서 **Ambari 홈을** **선택합니다.** Ambari UI에서 활성 리소스 관리자 > 리소스 관리자 **UI를**> **YARN** > **빠른 링크로** 이동합니다.

2. Jupyter Notebook을 사용하여 Spark 작업을 시작했기 때문에 애플리케이션은 **remotesparkmagics**이라는 이름을 갖게 됩니다(Notebook에서 시작된 모든 애플리케이션에 대한 이름임). 작업에 대한 자세한 정보를 얻으려면 응용 프로그램 이름에 대해 응용 프로그램 ID를 선택합니다. 그러면 애플리케이션 보기를 시작합니다.

    ![스파크 이력 서버 스파크 애플리케이션 ID 찾기](./media/apache-spark-job-debugging/find-application-id1.png)
...
3. 애플리케이션 보기에서 애플리케이션 및 로그 (stdout/stderr)와 연결된 컨테이너에 대해 알아보기 위해 더 자세히 살펴볼 수 있습니다. 아래와 같이 **추적 URL**에 해당하는 연결을 클릭하여 Spark UI를 시작할 수도 있습니다.
    ...
    ![스파크 히스토리 서버 다운로드 컨테이너 로그](./media/apache-spark-job-debugging/download-container-logs.png)
...
1. 응용 프로그램 보기에서 Spark UI를 실행하려면 위의 화면 캡처와 같이 **추적 URL에**대한 링크를 선택합니다. Jupyter Notebook에서 실행 중인 애플리케이션에 의해 시작되는 모든 Spark 작업을 확인할 수 있습니다.

    ![스파크 히스토리 서버 작업 탭](./media/apache-spark-job-debugging/view-apache-spark-jobs.png)

2. **실행기 탭을** 선택하여 각 실행기의 처리 및 저장소 정보를 확인합니다. **스레드 덤프** 링크를 선택하여 호출 스택을 검색할 수도 있습니다.

    ![스파크 기록 서버 실행기 탭](./media/apache-spark-job-debugging/view-spark-executors.png)

3. **단계** 탭을 선택하여 응용 프로그램과 연결된 단계를 확인합니다.

    ![스파크 히스토리 서버 단계 탭](./media/apache-spark-job-debugging/view-apache-spark-stages.png "Spark 단계 보기")

    각 단계에서는 아래와 같이 실행 통계를 볼 수 있는 여러 작업이 있습니다.

    ![스파크 히스토리 서버 단계 탭 세부 정보](./media/apache-spark-job-debugging/view-spark-stages-details.png "스파크 스테이지 세부 정보 보기")
...
6. Spark UI에서 다른 탭도 Spark 인스턴스에 대한 유용한 정보를 제공합니다.
   ...
   * 저장소 탭 - 응용 프로그램에서 RDD를 만드는 경우 저장소 탭에서 해당 정보에 대한 정보를 찾을 수 있습니다.
...
1. **개요** 페이지에서 스파크 기록 서버를 시작하려면 클러스터 대시보드에서 **스파크 기록 서버를** **선택합니다.**

    ![Azure 포털 시작 스파크 기록 서버](./media/apache-spark-job-debugging/launch-spark-history-server.png "스파크 히스토리 서버 1 출시")

   > [!TIP]  
   > 또는 Ambari UI에서 Spark 기록 서버를 시작할 수도 있습니다. 개요 블레이드에서 Ambari UI를 시작하려면 클러스터 대시보드에서 **Ambari 홈을** **선택합니다.** Ambari UI에서 **Spark2** > **빠른 링크** > **Spark2 히스토리 서버 UI로**이동합니다.

2. 완료된 애플리케이션이 모두 표시됩니다. 응용 프로그램 ID를 선택하여 응용 프로그램으로 드릴다운하여 자세한 정보를 확인합니다.

    ![스파크 이력 서버 완료 된 응용 프로그램](./media/apache-spark-job-debugging/view-completed-applications.png "스파크 히스토리 서버2 출시")
