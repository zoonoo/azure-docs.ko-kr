...
1. YARN UI를 시작합니다. **클러스터 대시보드**아래에서 **Yarn** 을 선택 합니다.
   ...
   > [!TIP]  
   > 또는 Ambari UI에서 YARN UI를 시작할 수도 있습니다. Ambari UI를 시작 하려면 **클러스터 대시보드**에서 **Ambari 홈** 을 선택 합니다. Ambari UI에서 **YARN** > **빠른 링크** (활성 리소스 관리자 > **리소스 관리자 UI**>로 이동 합니다.

2. Jupyter Notebook을 사용하여 Spark 작업을 시작했기 때문에 애플리케이션은 **remotesparkmagics**이라는 이름을 갖게 됩니다(Notebook에서 시작된 모든 애플리케이션에 대한 이름임). 응용 프로그램 이름에 대 한 응용 프로그램 ID를 선택 하 여 작업에 대 한 자세한 정보를 가져옵니다. 그러면 애플리케이션 보기를 시작합니다.
...
6. Spark UI에서 다른 탭도 Spark 인스턴스에 대한 유용한 정보를 제공합니다.
   ...
   * 저장소 탭-응용 프로그램이 RDD를 만드는 경우 저장소 탭에서 해당 정보에 대 한 정보를 찾을 수 있습니다.
...
1. Spark 기록 서버를 시작 하려면 **개요** 페이지의 **클러스터 대시보드**아래에서 **spark 기록 서버** 를 선택 합니다.
   ...
   > [!TIP]  
   > 또는 Ambari UI에서 Spark 기록 서버를 시작할 수도 있습니다. Ambari UI를 시작 하려면 개요 블레이드에서 **클러스터 대시보드**아래의 **Ambari 홈** 을 선택 합니다. Ambari UI에서 **Spark2** > **빠른 링크** > **Spark2 History Server UI**로 이동 합니다.
