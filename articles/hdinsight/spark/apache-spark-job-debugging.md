...
3. **단계** 탭을 클릭하여 애플리케이션과 관련된 단계를 봅니다.
    ...
    ![Spark 기록 서버 단계 탭](./media/apache-spark-job-debugging/view-apache-spark-stages.png "보기 spark 단계")

    각 단계에서는 아래와 같이 실행 통계를 볼 수 있는 여러 작업이 있습니다.

    ![Spark 기록 서버 단계 탭 정보](./media/apache-spark-job-debugging/view-spark-stages-details.png "spark 단계 세부 정보 보기")
...
6. Spark UI에서 다른 탭도 Spark 인스턴스에 대한 유용한 정보를 제공합니다.
   ...
   * 스토리지 탭 - 애플리케이션이 RDD를 만드는 경우 스토리지 탭에 대한 정보를 찾을 수 있습니다.
   * 환경 탭-이 탭은 다음과 같은 Spark 인스턴스에 대 한 유용한 정보를 많이 제공 합니다.
     * Scala 버전
...
2. 완료된 애플리케이션이 모두 표시됩니다. 자세한 내용은 애플리케이션 ID를 클릭하여 애플리케이션에 대해 더 자세히 살펴봅니다.
    ...
    ![Spark 기록 서버 완료 응용 프로그램](./media/apache-spark-job-debugging/view-completed-applications.png "spark 기록 Server2")
