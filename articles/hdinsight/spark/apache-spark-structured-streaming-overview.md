...
결과 테이블의 데이터는 쿼리가 마지막으로 처리된 이후 새로운 데이터만 포함할 수*있습니다(부속 모드),* 또는 테이블이 새 데이터가 있을 때마다 테이블을 새로 고쳐질 수 있으므로 스트리밍 쿼리가 시작된 이후 의 모든 출력 데이터를 테이블이 포함할 수 있습니다(완료*모드).*
...
온도 조절기와 같은 온도 센서에서 원격 분석을 처리하는 시나리오를 생각해 보십시오. 첫 번째 트리거에서 온도 판독값이 95도인 디바이스 1에 대해 00:01 시간에 하나의 이벤트를 처리했다고 가정합니다. 쿼리의 첫 번째 트리거에서 00:01 시간이 있는 행만 결과 테이블에 표시됩니다. 00:02 시간에 다른 이벤트가 도착하는 경우 유일한 새 행은 00:02 시간이 있는 행이므로 결과 테이블에는 하나의 행만 포함됩니다.
