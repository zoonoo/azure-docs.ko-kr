...
- HDInsight 게이트웨이를 표준 `xxx.azurehdinsight.net`아닌 사용자 지정 된 DNS 이름으로 지정 했습니다.
...
1. : `NotebookApp.allow_origin='\"https://{2}.{3}\"'` 줄을 찾아 `NotebookApp.allow_origin='\"*\"'`변경 합니다.

1. Ambari에서 Jupyter 서비스를 다시 시작 합니다.

1. 명령 프롬프트에서 `ps aux | grep jupyter`를 입력 하면 모든 URL에 연결할 수 있다는 메시지가 표시 됩니다.
