...
1. Azure [포털에서](https://portal.azure.com/)Spark 클러스터를 선택합니다.  지침에 대해서는 [클러스터 나열 및 표시](../hdinsight-administer-use-portal-linux.md#showClusters)를 참조하세요. **개요보기가** 열립니다.

2. **개요** 보기에서 **클러스터 대시보드** 상자에서 **Jupyter 노트북을**선택합니다. 메시지가 표시되면 클러스터에 대한 관리자 자격 증명을 입력합니다.

    ![아파치 스파크의 주피터 노트북](./media/apache-spark-jupyter-notebook-kernels/hdinsight-spark-open-jupyter-interactive-spark-sql-query.png "스파크의 주피터 노트북")
...
3. **새로**를 선택한 다음 **Pyspark,** **PySpark3**또는 **스파크를** 선택하여 노트북을 만듭니다. Scala 애플리케이션에 대해 Spark 커널을, Python2 애플리케이션에 대해 PySpark 커널을, Python3 애플리케이션에 대해 PySpark3 커널을 사용합니다.

    ![스파크의 주피터 노트북용 커널](./media/apache-spark-jupyter-notebook-kernels/kernel-jupyter-notebook-on-spark.png "스파크의 주피터 노트북용 커널")
...
- **컨텍스트를 미리 설정합니다**. **PySpark,** **PySpark3**또는 **스파크** 커널을 사용하면 응용 프로그램 작업을 시작하기 전에 스파크 또는 하이브 컨텍스트를 명시적으로 설정할 필요가 없습니다. 이러한 컨텍스트는 기본적으로 사용할 수 있습니다. 이러한 컨텍스트는 다음과 같습니다.
...
- **매직 셀**입니다. PySpark 커널은 미리 정의된 "마법"을 제공하며, 이 명령은 `%%` 호출할 수 `%%MAGIC` `<args>`있는 특수 명령입니다(예: ). 매직 명령은 코드 셀의 첫 번째 단어여야 하고 여러 콘텐츠 줄에 허용됩니다. 매직 단어는 셀의 첫 번째 단어여야 합니다. 매직 앞에 다른 단어(주석 포함)가 있으면 오류가 발생합니다.     매직에 대한 자세한 내용은 [여기](https://ipython.readthedocs.org/en/stable/interactive/magics.html)를 참조하세요.
   ...
   | local |`%%local`<br>`a=1` |다음 줄의 모든 코드는 로컬로 실행됩니다. 코드는 사용 하려는 커널에 관계 없이 유효한 Python2 코드 여야 합니다. 따라서 노트북을 만드는 동안 **PySpark3** 또는 **Spark** 커널을 선택한 `%%local` 경우에도 셀에서 마법을 사용하는 경우 해당 셀에는 유효한 Python2 코드만 있어야 합니다. |
...
| -Q |`-q` |이 매개 변수를 사용하여 셀에 대한 시각화를 해제할 수 있습니다. 셀의 내용을 자동으로 시각화하지 않고 셀을 데이터 프레임으로 캡처하려는 경우 을 사용합니다. `-q -o <VARIABLE>` `CREATE TABLE` 문과 같은 SQL 쿼리를 실행하려는 등의 경우 결과를 캡처하지 않고 시각화를 해제하려면 `-o` 인수를 지정하지 않고 `-q`만 사용합니다. |
...
| -n |`-n <MAXROWS>` |**MAXROWS** 는 정수 값입니다. 커널은 출력 행 수를 **MAXROWS**로 제한합니다. **MAXROWS가** **-1과**같은 음수인 경우 결과 집합의 행 수는 제한되지 않습니다. |
...
사용하는 커널에 따라 Notebook을 실행 중인 상태로 두면 클러스터 리소스가 사용됩니다.  이러한 커널을 사용하면 컨텍스트가 미리 설정되므로 노트북을 종료해도 컨텍스트가 종료되지 않으므로 클러스터 리소스가 계속 사용됩니다. 좋은 방법은 전자 필기장 사용을 마치면 전자 필기장 **의 파일** 메뉴에서 **닫기 및 중지** 옵션을 사용하여 컨텍스트를 종료한 다음 전자 필기장을 종료하는 것입니다.
...
새로운 커널도 현재 개발 중이며 곧 완성될 예정입니다. 이는 API가 이러한 커널의 성숙에 따라 변경될 수 있음을 의미할 수 있습니다. 이러한 새로운 커널을 사용하는 동안 가진 의견을 보내주시면 감사하겠습니다. 이러한 커널의 최종 릴리스 형성에 유용할 것입니다. 이 문서의 하단에 **있는 피드백** 섹션 아래에 의견/피드백을 남길 수 있습니다.
