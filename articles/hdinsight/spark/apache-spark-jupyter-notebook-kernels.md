...
1. [Azure portal](https://portal.azure.com/), Spark 클러스터를 선택 합니다.  지침에 대해서는 [클러스터 나열 및 표시](../hdinsight-administer-use-portal-linux.md#showClusters)를 참조하세요. 합니다 **개요** 보기가 열립니다.

2. **개요** 보기에 **클러스터 대시보드** 상자에서 **Jupyter notebook**합니다. 메시지가 표시되면 클러스터에 대한 관리자 자격 증명을 입력합니다.

    ![Spark의 Jupyter 노트북](./media/apache-spark-jupyter-notebook-kernels/hdinsight-spark-open-jupyter-interactive-spark-sql-query.png "Spark의 Jupyter 노트북") 
...
3. 선택 **새로 만들기**, 다음 중 하나를 선택 하 고 **Pyspark**합니다 **PySpark3**, 또는 **Spark** 여 notebook을 만듭니다. Scala 애플리케이션에 대해 Spark 커널을, Python2 애플리케이션에 대해 PySpark 커널을, Python3 애플리케이션에 대해 PySpark3 커널을 사용합니다.
...
- **매직 셀**입니다. PySpark 커널은 특수 명령인 일부 미리 정의된 “매직”을 제공하며 이러한 매직은 `%%`(예: `%%MAGIC` `<args>`)를 사용하여 호출할 수 있습니다. 매직 명령은 코드 셀의 첫 번째 단어여야 하고 여러 콘텐츠 줄에 허용됩니다. 매직 단어는 셀의 첫 번째 단어여야 합니다. 매직 앞에 다른 단어(주석 포함)가 있으면 오류가 발생합니다.     매직에 대한 자세한 내용은 [여기](https://ipython.readthedocs.org/en/stable/interactive/magics.html)를 참조하세요.
   ...
   | local |`%%local`<br>`a=1` |다음 줄의 모든 코드는 로컬로 실행됩니다. 코드는 사용 중인 커널에 관계없이 유효한 Python2 코드여야 합니다. 선택한 경우에 따라서 **PySpark3** 또는 **Spark** 커널을 사용 하는 경우 notebook을 만드는 동안는 `%%local` 셀에서 매직을 해당 셀만 유효한 Python2 코드가 있어야 합니다. |
   | logs |`%%logs` |현재 Livy 세션에 대한 로그를 출력합니다. |
...
| -q |`-q` |이 매개 변수를 사용하여 셀에 대한 시각화를 해제할 수 있습니다. 셀 내용의 autovisualize 하려는 데이터 프레임으로 캡처 하려는 없는 경우 사용 하 여 `-q -o <VARIABLE>`입니다. `CREATE TABLE` 문과 같은 SQL 쿼리를 실행하려는 등의 경우 결과를 캡처하지 않고 시각화를 해제하려면 `-o` 인수를 지정하지 않고 `-q`만 사용합니다. |
...
* -Q를 사용 하므로 autovisualization 해제 합니다.
...
새로운 커널도 현재 개발 중이며 곧 완성될 예정입니다. 이는 API가 이러한 커널의 성숙에 따라 변경될 수 있음을 의미할 수 있습니다. 이러한 새로운 커널을 사용하는 동안 가진 의견을 보내주시면 감사하겠습니다. 이러한 커널의 최종 릴리스 형성에 유용할 것입니다. 아래에 의견/피드백 두어도 합니다 **피드백** 이 문서의 하단에 있는 섹션.
