...
사용하는 커널에 따라 Notebook을 실행 중인 상태로 두면 클러스터 리소스가 사용됩니다.  이러한 커널의 경우 컨텍스트가 미리 설정 되어 있기 때문에 노트북을 종료 하면 컨텍스트가 중단 되지 않으므로 클러스터 리소스가 계속 사용 됩니다. 노트북 사용이 완료 되 면 노트북의 **파일** 메뉴에서 **닫기 및 중지** 옵션을 사용 하는 것이 좋습니다 .이 경우에는 컨텍스트를 종료 하 고 노트북을 종료 합니다.
