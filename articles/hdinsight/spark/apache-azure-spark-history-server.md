...
로 이동 하 여 Spark 기록 서버를 엽니다 `https://CLUSTERNAME.azurehdinsight.net/sparkhistory` . 여기서 **CLUSTERNAME** 은 spark 클러스터의 이름입니다.
...
핫픽스를 사용 하 여 업그레이드 하려면 다음 스크립트를 사용 합니다 .이 스크립트는에서 업그레이드 됩니다 `spark-enhancement.jar*` .
...
이 도구를 사용할 때 의견이 있거나 모든 문제를 해결 하는 경우 ()로 전자 메일을 보내 주세요 [hdivstool@microsoft.com](mailto:hdivstool@microsoft.com) .
