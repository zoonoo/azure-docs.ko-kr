...
`https://CLUSTERNAME.azurehdinsight.net/sparkhistory`로 이동 하 여 Spark 기록 서버를 엽니다. 여기서 **CLUSTERNAME** 은 spark 클러스터의 이름입니다.
...
+ 각 작업의 배경색은 열 지도에 해당 합니다.
   ...
   ![Spark 응용 프로그램 & 작업 그래프 페이지의 열 지도입니다.](./media/apache-azure-spark-history-server/sparkui-graph-heatmap.png)
    ...
    |빨강|작업이 실패했습니다.|
     ...
     건너뛴 단계는 흰색으로 표시 됩니다.
    Spark 응용 프로그램 & 작업 그래프 페이지에서 건너뛴 작업을 ![합니다.](./media/apache-azure-spark-history-server/sparkui-graph-color-skip.png)
...
핫픽스를 사용 하 여 업그레이드 하려면 `spark-enhancement.jar*`를 업그레이드 하는 다음 스크립트를 사용 합니다.
...
이 도구를 사용할 때 의견이 있거나 모든 문제를 해결 하는 경우 ([hdivstool@microsoft.com](mailto:hdivstool@microsoft.com))에 전자 메일을 보냅니다.