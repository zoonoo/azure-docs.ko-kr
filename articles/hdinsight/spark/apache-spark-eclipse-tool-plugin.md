---
title: '이클립스를 위한 Azure 도구 키트: HDInsight 스파크용 스칼라 앱 만들기'
...
* [이클립스 IDE](https://www.eclipse.org/downloads/). 이 문서에서는 Java 개발자를 위한 이클립스 IDE를 사용합니다.

## <a name="install-required-plug-ins"></a>필요한 플러그인 설치
...
Eclipse를 열면 HDInsight Tools는 Scala 플러그 인을 설치했는지 여부를 자동으로 검색합니다. **확인**을 선택하여 계속 진행한 다음 Eclipse Marketplace에서 해당 플러그 인을 설치하는 지침을 따릅니다. 설치가 완료된 후 IDE를 다시 시작합니다.

![Scala 플러그 인 자동 설치](./media/apache-spark-eclipse-tool-plugin/auto-installation-scala1.png)

### <a name="confirm-plug-ins"></a>플러그인 확인

1. **이클립스 마켓 플레이스를** **돕기** > 위해 탐색 ... .

1. **설치됨** 탭을 선택합니다.

1. 적어도 다음이 표시됩니다.
    * 이클립스 \<버전에 대한 Azure 도구 키트는>.
    * 스칼라 IDE \<버전>.
...
1. **창** >  **보기 보기로** > 이동**기타...**  >  **로그인합니다.**. .

1. **보기** 보기 대화 상자에서 **Azure** > **Azure 탐색기로**이동한 다음 **열기를**선택합니다.

   ![아파치 스파크 이클립스 쇼 보기](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer1.png)

1. **Azure 탐색기에서** **Azure** 노드를 마우스 오른쪽 단추로 클릭한 다음 **로그인을**선택합니다.

1. Azure **로그인** 대화 상자에서 인증 방법을 선택하고 **로그인**을 선택하고 로그인 프로세스를 완료합니다.

   ![아파치 스파크 이클립스 Azure 기호](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer2.png)

1. 로그인하면 **구독** 대화 상자에 자격 증명과 연결된 모든 Azure 구독이 나열됩니다. **선택을** 눌러 대화 상자를 닫습니다.

   ![구독 선택 대화 상자](./media/apache-spark-eclipse-tool-plugin/Select-Subscriptions.png)

1. **Azure 탐색기에서** **Azure** >  **HDInsight로** 이동하여 구독 에서 HDInsight 스파크 클러스터를 확인합니다.

   ![Azure 탐색기3의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer3.png)
...
1. **Azure 탐색기에서** **HDInsight를**마우스 오른쪽 단추로 클릭하고 **클러스터 링크를**선택합니다.

   ![Azure 탐색기 링크 클러스터 메뉴](./media/apache-spark-eclipse-tool-plugin/link-a-cluster-context-menu.png)

1. **클러스터 이름,** **사용자 이름**및 **암호를**입력한 다음 **확인을**선택합니다. 선택적으로 스토리지 계정, 스토리지 키를 입력한 다음, Storage Explorer의 스토리지 컨테이너를 선택하여 왼쪽 트리 뷰에서 작업합니다.

   ![링크 새로운 HDInsight 클러스터 대화 상자](./media/apache-spark-eclipse-tool-plugin/link-cluster-dialog1.png)
   ...
   >
   > 키보드 사용자만 현재 포커스가 저장소 **키에**있는 경우 **Ctrl+TAB을** 사용하여 대화 상자의 다음 필드에 집중해야 합니다.

1. **HDInsight**에서 연결된 클러스터를 볼 수 있습니다. 이제 애플리케이션을 연결된 클러스터에 제출할 수 있습니다.

   ![Azure 탐색기 hdi 연결 클러스터](./media/apache-spark-eclipse-tool-plugin/hdinsight-linked-cluster.png)
...
1. **새 HDInsight Scala 프로젝트** 대화 상자에서 다음 값을 입력한 후 **다음**을 선택합니다.
   ...
   * **Spark 라이브러리** 영역에서 **Maven을 사용하여 Spark SDK 구성** 옵션을 선택할 수 있습니다.  이 도구는 Spark SDK 및 Scala SDK에 대해 적합한 버전을 통합합니다. **스파크 SDK 추가** 옵션을 수동으로 선택하고 수동으로 스파크 SDK를 다운로드하고 추가할 수도 있습니다.

   ![새 HDInsight Scala 프로젝트 대화 상자](./media/apache-spark-eclipse-tool-plugin/create-hdi-scala-app-3.png)

1. 다음 대화 상자에서 세부 정보를 검토한 다음 **완료를**선택합니다.
...
1. HDInsight Spark 클러스터에서 애플리케이션 실행:
   ...
   b. **[Spark 제출]** 대화 상자에서 다음 값을 제공한 다음 **제출을**선택합니다.
   ...
   * 이 예제의 응용 프로그램 코드에는 명령줄 인수나 참조 JAR 또는 파일이 필요하지 않으므로 나머지 텍스트 상자를 비워 둘 수 있습니다.

     ![아파치 스파크 제출 대화 상자](./media/apache-spark-eclipse-tool-plugin/create-scala-project3.png)
...
1. **Azure 탐색기에서** **HDInsight를**확장한 다음 스파크 클러스터 이름을 선택한 다음 **작업을 선택합니다.**

   ![Azure 탐색기 이클립스 작업 보기 노드](./media/apache-spark-eclipse-tool-plugin/eclipse-job-view-node.png)
...
1. **작업** 노드를 선택합니다. Java 버전이 **1.8**보다 낮으면 HDInsight Tools가 자동으로 **E(fx)clipse** 플러그인 설치를 미리 알립니다. **확인**을 선택하여 계속 진행한 다음 마법사에 따라 Eclipse Marketplace에서  Eclipse를 설치하고 다시 시작합니다.
   ...
   ![누락 된 플러그인 E (fx) 클립 설치](./media/apache-spark-eclipse-tool-plugin/auto-install-efxclipse.png)

1. **작업** 노드에서 작업 보기를 엽니다. 오른쪽 창의 **Spark 작업 보기** 탭에는 클러스터에서 실행된 모든 애플리케이션이 표시됩니다. 자세한 내용을 보려면 원하는 애플리케이션 이름을 선택합니다.

   ![아파치 이클립스 보기 작업 로그 세부 정보](./media/apache-spark-eclipse-tool-plugin/eclipse-view-job-logs.png)

   다음 작업 중 하나를 수행할 수 있습니다.

   * 작업 그래프로 마우스를 가져갑니다. 실행 중인 작업에 대한 기본 정보가 표시됩니다. 작업 그래프를 선택하면 모든 작업이 생성하는 단계 및 정보가 표시됩니다.

     ![아파치 스파크 작업 그래프 단계 정보](./media/apache-spark-eclipse-tool-plugin/Job-graph-stage-info.png)

   * **드라이버 Stderr**, **드라이버 Stdout** 및 **디렉터리 정보**와 같은 자주 사용되는 로그를 보려면 **로그** 탭을 선택합니다.

     ![아파치 스파크 이클립스 작업 로그 정보](./media/apache-spark-eclipse-tool-plugin/eclipse-job-log-info.png)
...
Windows 컴퓨터에서 로컬 Spark Scala 응용 프로그램을 실행하는 동안 [SPARK-2356에](https://issues.apache.org/jira/browse/SPARK-2356)설명된 대로 예외가 발생할 수 있습니다. 이 예외는 Windows에 **WinUtils.exe**가 없기 때문에 발생합니다.

이 오류를 해결하려면 [Winutils.exe를](https://github.com/steveloughran/winutils) **C:\WinUtils\bin과**같은 위치에 추가한 다음 환경 변수 **HADOOP_HOME** 추가하고 변수 값을 **C\WinUtils로**설정해야 합니다.
...
1. 템플릿은 컴퓨터에서 로컬로 실행할 수 있는 샘플 코드(**LogQuery**)를 **src** 폴더 아래에 추가합니다.
   ...
   ![LogQuery 로컬 스칼라 응용 프로그램의 위치](./media/apache-spark-eclipse-tool-plugin/local-scala-application.png)

1. **LogQuery.scala를** 마우스 오른쪽 단추로 클릭하고**1 스칼라 응용 프로그램으로** **실행을** > 선택합니다. **콘솔** 탭에 다음과 같은 출력이 표시됩니다.
...
2. **Azure 탐색기**에서 **HDInsight**를 확장하여 구독에 포함된 HDInsight 클러스터를 표시합니다. **"Role:Reader"** 표시가 있는 클러스터에는 판독기 전용 역할 권한만 있습니다.
    ...
    ![Azure 탐색기 역할 판독기의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer6.png)

3. 판독기 전용 역할 권한이 있는 클러스터를 마우스 오른쪽 단추로 클릭합니다. 상황에 맞는 메뉴에서 **Link this cluster**(이 클러스터 연결)를 선택하여 클러스터를 연결합니다. Ambari 사용자 이름과 암호를 입력합니다.

    ![Azure 탐색기 링크의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer7.png)

4. 클러스터가 연결되면 HDInsight가 새로 고쳐집니다.
   클러스터의 단계가 연결됨으로 변경됩니다.
  
    ![Azure 탐색기의 HDInsight 스파크 클러스터 연결](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer8.png)
...
2. **Link this cluster**(이 클러스터 연결)를 클릭하여 클러스터를 연결합니다.
    ...
    ![Azure 탐색기9의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer9.png)

### <a name="link-cluster-from-spark-submission-window"></a>스파크 제출 창에서 클러스터 링크

1. HDInsight 프로젝트를 만듭니다.

2. 패키지를 마우스 오른쪽 단추로 클릭합니다. 그런 다음 **HDInsight에 스파크 응용 프로그램 제출을**선택합니다.

   ![Azure 탐색기 제출의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer11.png)

3. 클러스터 이름에 대한 판독기 전용 역할 권한이 있는 **클러스터를**선택합니다. 경고 메시지가 표시됩니다. **이 클러스터 링크를** 클릭하여 클러스터를 연결할 수 있습니다.

   ![Azure 탐색기의 HDInsight 스파크 클러스터는](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer15.png)
...
* 판독기 전용 역할 권한이 있는 클러스터에 대해 **스토리지 계정** 노드를 클릭하면 **Storage Access Denied**(스토리지 계정 거부됨) 창이 표시됩니다.
   ...
   ![Azure 탐색기 저장소의 HDInsight 스파크 클러스터](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer13.png)

   ![Azure 탐색기의 HDInsight 스파크 클러스터거부](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer12.png)

* 연결된 클러스터에 대해 **스토리지 계정** 노드를 클릭하면 **Storage Access Denied**(스토리지 계정 거부됨) 창이 표시됩니다.

   ![Azure 탐색기의 HDInsight 스파크 클러스터 거부2](./media/apache-spark-eclipse-tool-plugin/eclipse-view-explorer14.png)

## <a name="known-problems"></a>알려진 문제

링크 **A 클러스터를**사용할 때 저장소의 자격 증명을 제공하는 것이 좋습니다.

![저장소 자격 증명 이클립이 있는 링크 클러스터](./media/apache-spark-eclipse-tool-plugin/link-cluster-with-storage-credential-eclipse.png)
...
![일식 은 클러스터 바쁜 원사 때 오류를 얻을](./media/apache-spark-eclipse-tool-plugin/eclipse-interactive-cluster-busy-submit.png "일식 은 클러스터 바쁜 원사 때 오류를 얻을")
