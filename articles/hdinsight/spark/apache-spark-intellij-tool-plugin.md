...
이 문서에서는 IntelliJ IDE에 대한 Azure 도구 **키트** 플러그인을 사용하여 Azure HDInsight에서 아파치 스파크 응용 프로그램을 개발하는 방법을 보여 줍니다. [Azure HDInsight는](../hdinsight-overview.md) 클라우드에서 관리되는 오픈 소스 분석 서비스입니다. 이 서비스를 사용하면 하두프, 아파치 스파크, 아파치 하이브, 아파치 카프카와 같은 오픈 소스 프레임워크를 사용할 수 있습니다.

다음과 같은 몇 가지 방법으로 **Azure 도구 키트** 플러그인을 사용할 수 있습니다.

* 스칼라 스파크 애플리케이션을 개발하고 HDInsight 스파크 클러스터에 제출합니다.
...
* [오라클 자바 개발 키트](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html).  이 문서에서는 Java 버전 8.0.202를 사용합니다.

* IntelliJ IDEA. 이 문서는 [IntelliJ IDEA 커뮤니티 ver를 사용합니다.  2018.3.4](https://www.jetbrains.com/idea/download/).
...
2. 시작 화면에서**플러그인** **구성으로** > 이동하여 플러그인 창을 **엽니다.**

    ![IntelliJ IDEA는 스칼라 플러그인을 가능하게 합니다.](./media/apache-spark-intellij-tool-plugin/enable-scala-plugin1.png)

3. 새 창에 제공되는 Scala 플러그인에 대해 **설치**를 선택합니다.  

    ![IntelliJ IDEA는 스칼라 플러그인을 설치합니다.](./media/apache-spark-intellij-tool-plugin/install-scala-plugin.png)
...
6. **새 프로젝트** 창에서 다음 정보를 제공합니다.  
    ...
    |프로젝트&nbsp;위치| 위치를 입력하여 프로젝트를 저장합니다.|
    |프로젝트 SDK| 이 필드는 IDEA를 처음 사용할 때 비어 있을 수 있습니다.  **새로 만들기...** 를 만들기 JDK로 이동합니다.|
    |Spark 버전|만들기 마법사는 Spark SDK 및 Scala SDK에 대해 적합한 버전을 통합합니다. 스파크 클러스터 버전이 2.0보다 이전인 경우 **스파크 1.x를**선택합니다. 그렇지 않으면 **Spark2.x**를 선택합니다. 이 예제에서는 **Spark 2.3.0(Scala 2.11.8)** 을 사용합니다.|
...
9. 다음 단계를 수행하여 응용 프로그램 소스 코드를 추가합니다.
    ...
    a. 프로젝트에서 **myApp** > **src** > **메인** > **스칼라로**이동하십시오.  
...
사용자는 [Azure 구독에 로그인하거나](#sign-in-to-your-azure-subscription) [HDInsight 클러스터를 연결할](#link-a-cluster)수 있습니다. Ambari 사용자 이름/암호 또는 도메인 에 가입된 자격 증명을 사용하여 HDInsight 클러스터에 연결합니다.

### <a name="sign-in-to-your-azure-subscription"></a>Azure 구독에 로그인합니다.

1. 메뉴 모음에서 **보기** > **도구 Windows Azure** > **탐색기로**이동합니다.

   ![IntelliJ IDEA는 푸른 탐색기를 보여줍니다](./media/apache-spark-intellij-tool-plugin/show-azure-explorer1.png)
...
3. **Azure 로그인** 대화 상자에서 **디바이스 로그인**과 **로그인**을 차례로 선택합니다.
    ...
    !['IntelliJ IDEA Azure 로그인 장치 로그인'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer2.png)

4. **Azure 디바이스 로그인** 대화 상자에서 **복사 및 열기**를 클릭합니다.

   !['인텔리제이 IDEA Azure 장치 로그인'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer5.png)

5. 브라우저 인터페이스에서 코드를 붙여넣고 **다음**을 클릭합니다.

   !['마이크로소프트 는 HDI에 대 한 코드 대화 상자를 입력'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer6.png)

6. Azure 자격 증명을 입력한 다음, 브라우저를 닫습니다.

   !['마이크로소프트는 HDI에 대 한 전자 메일 대화 상자를 입력'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer7.png)
...
1. 프로젝트에서 **myApp** > **src** > **메인** > **스칼라** > **myApp로**이동합니다.  **myApp**을 마우스 오른쪽 단추로 클릭하고 **Spark 애플리케이션 제출**(일반적으로 목록 맨 아래에 표시됨)을 선택합니다.

      ![HDInsight에 Spark 애플리케이션 제출 명령](./media/apache-spark-intellij-tool-plugin/hdi-submit-spark-app-1.png)

2. **스파크 응용 프로그램 제출** 대화 상자 창에서 **1을 선택합니다. HDInsight에 스파크**.
...
Spark 애플리케이션을 클러스터에 제출하는 또 다른 권장되는 방법이 있습니다. **구성 실행/디버그** IDE에서 매개 변수를 설정하여 이 작업을 수행할 수 있습니다. [SSH를 통해 IntelliJ에 대한 Azure 도구 키트와 HDInsight 클러스터에서 로컬 또는 원격으로 디버그 아파치 스파크 응용 프로그램을](apache-spark-intellij-tool-debug-remotely-through-ssh.md)참조하십시오.
...
IntelliJ에 대 한 Azure 도구 키트를 사용 하 여 다양 한 작업을 수행할 수 있습니다.  대부분의 작업은 **Azure 탐색기에서**시작됩니다.  메뉴 모음에서 **보기** > **도구 Windows Azure** > **탐색기로**이동합니다.

### <a name="access-the-job-view"></a>작업 보기 액세스

1. Azure 탐색기에서 **HDInsight로** > \<이동하여 클러스터> > **작업.**
...
5. 응용 프로그램 수준에서 스파크 기록 UI 및 YARN UI를 볼 수 있습니다. 창 상단의 링크를 선택합니다.
...
3. Spark 기록 서버 대시보드에서 애플리케이션 이름을 사용하여 방금 실행을 마친 애플리케이션을 찾을 수 있습니다. 위의 코드에서 `val conf = new SparkConf().setAppName("myApp")`을 사용하여 애플리케이션 이름을 설정합니다. 스파크 응용 프로그램 이름은 **myApp입니다.**
...
WINUTILS를 만족했는지 확인하십시오. EXE 전제 조건.

1. 메뉴 모음에서 구성 **편집 실행으로** > **이동합니다.**.

2. **구성 실행/디버그** 창의 왼쪽 분할 창에서 **HDInsight의 Apache Spark** > **[HDInsight의 Spark] myApp**으로 이동합니다.

3. 기본 창에서 탭을 **`Locally Run`** 선택합니다.
...
5. 프로젝트에서 **myApp** > **src** > **메인** > **스칼라** > **myApp로**이동합니다.  

6. 메뉴 모음에서 **도구** > **스파크 콘솔** > **실행 스파크 로컬 콘솔(스칼라)으로**이동합니다.
...
8. 콘솔은 아래 그림과 같이 표시됩니다. 콘솔 창에서 `sc.appName`을 입력한 다음, Ctrl+Enter를 누릅니다.  결과가 표시됩니다. 빨간색 버튼을 클릭하여 로컬 본체를 종료할 수 있습니다.
...
3. 기본 창에서 탭을 **`Remotely Run in Cluster`** 선택합니다.
...
6. 메뉴 모음에서 **도구** > **스파크 콘솔** > **실행 스파크 리비 인터랙티브 세션 콘솔(스칼라)으로**이동합니다.

7. 콘솔은 아래 그림과 같이 표시됩니다. 콘솔 창에서 `sc.appName`을 입력한 다음, Ctrl+Enter를 누릅니다.  결과가 표시됩니다. 빨간색 버튼을 클릭하여 로컬 본체를 종료할 수 있습니다.
...
로컬 콘솔 또는 Livy 대화형 세션 콘솔(Scala)에 일부 코드를 전송하여 스크립트 결과를 예측하는 것이 편리합니다. Scala 파일에서 일부 코드를 강조 표시한 다음, 마우스 오른쪽 단추로 **Spark 콘솔로 선택 내용 보내기**를 클릭합니다. 선택한 코드가 콘솔로 전송됩니다. 결과는 콘솔의 코드 뒤에 표시됩니다. 콘솔에서 오류가 발생했는지 확인합니다.  
...
일반 단계를 수행하여 Azure 구독에 로그인하여 HIB(ID 브로커)를 사용하여 HDInsight ESP 클러스터에 연결할 수 있습니다. 로그인하면 Azure 탐색기에서 클러스터 목록이 표시됩니다. 자세한 지침은 [HDInsight 클러스터에 연결](#connect-to-your-hdinsight-cluster)을 참조하세요.

### <a name="run-a-spark-scala-application-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>ID 브로커(HIB)를 사용 하 고 HDInsight ESP 클러스터에서 스파크 스칼라 응용 프로그램을 실행 합니다.

ID 브로커(HIB)를 사용하여 HDInsight ESP 클러스터에 작업을 제출하는 일반적인 단계를 따를 수 있습니다. 자세한 지침은 [HDInsight Spark 클러스터에서 Spark Scala 애플리케이션 실행](#run-a-spark-scala-application-on-an-hdinsight-spark-cluster)을 참조하세요.

로그인 계정으로 명명된 폴더에 필요한 파일을 업로드하면 구성 파일에서 업로드 경로를 볼 수 있습니다.

   ![구성의 업로드 경로](./media/apache-spark-intellij-tool-plugin/upload-path-in-the-configuration.png)

### <a name="spark-console-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>ID 브로커(HIB)가 있는 HDInsight ESP 클러스터의 스파크 콘솔

스파크 로컬 콘솔(스칼라)을 실행하거나 ID 브로커(HIB)가 있는 HDInsight ESP 클러스터에서 스파크 리비 인터랙티브 세션 콘솔(스칼라)을 실행할 수 있습니다. 자세한 지침은 [Spark 콘솔](#spark-console)을 참조하세요.
...
2. **Azure 탐색기**에서 **HDInsight**를 확장하여 구독에 포함된 HDInsight 클러스터를 표시합니다. **"Role:Reader"** 표시가 있는 클러스터에는 판독기 전용 역할 권한만 있습니다.
    ...
    !['IntelliJ Azure 탐색기 역할:리더'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer15.png)
...
2. **Spark 클러스터(Linux 전용)** 에 대해 판독기 전용 역할 권한이 있는 클러스터를 선택합니다. 경고 메시지가 표시됩니다. **이 클러스터 링크를** 클릭하여 클러스터를 연결할 수 있습니다.
...
* 판독기 전용 역할 권한이 있는 클러스터에 대해 **스토리지 계정** 노드를 클릭하면 **Storage Access Denied**(스토리지 계정 거부됨) 창이 표시됩니다. **Azure Storage Explorer 열기**를 클릭하여 Storage Explorer를 엽니다.
   ...
   !['인텔리제이 IDEA 스토리지 액세스 거부'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer14.png)
...
* 연결된 클러스터에 대해 **스토리지 계정** 노드를 클릭하면 **Storage Access Denied**(스토리지 계정 거부됨) 창이 표시됩니다. **Azure Storage 열기**를 클릭하여 Storage Explorer를 엽니다.
   ...
   !['인텔리제이 IDEA 스토리지 액세스 거부2'](./media/apache-spark-intellij-tool-plugin/intellij-view-explorer13.png)
...
1. IntelliJ IDEA를 통해 생성된 기존 스파크 스칼라 응용 프로그램의 `.iml` 경우 연결된 파일을 엽니다.

2. 루트 수준에서 다음과 같은 **모듈** 요소입니다.

        ```
        <module org.jetbrains.idea.maven.project.MavenProjectsManager.isMavenModule="true" type="JAVA_MODULE" version="4">
        ```

   모듈 요소가 다음 `UniqueKey="HDInsightTool"` 텍스트처럼 **module** 보이도록 추가할 요소를 편집합니다.
...
![Azure 포털이 HDInsight 클러스터를 삭제합니다.](./media/apache-spark-intellij-tool-plugin/hdinsight-azure-portal-delete-cluster.png "HDInsight 클러스터 삭제")

## <a name="next-steps"></a>다음 단계

이 문서에서는 IntelliJ 플러그인용 Azure 도구 키트를 사용하여 [스칼라로](https://www.scala-lang.org/)작성된 아파치 스파크 응용 프로그램을 개발하는 방법을 배웠습니다. 그런 다음 IntelliJ 통합 개발 환경(IDE)에서 직접 HDInsight 스파크 클러스터에 제출했습니다. 다음 문서로 진행하여 Apache Spark에 등록된 데이터를 Power BI와 같은 BI 분석 도구로 가져오는 방법을 확인하세요.

> [!div class="nextstepaction"]
> [Power BI를 사용하여 아파치 스파크 데이터 분석](apache-spark-use-bi-tools.md)
