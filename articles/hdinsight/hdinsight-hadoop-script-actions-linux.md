...
description: Bash 스크립트를 사용하여 HDInsight 클러스터를 사용자 지정하는 방법을 알아봅니다. 스크립트 작업을 사용하면 클러스터 생성 중 또는 후에 스크립트를 실행하여 클러스터 구성 설정을 변경하거나 추가 소프트웨어를 설치할 수 있습니다.
...
## <a name="best-practices-for-script-development"></a><a name="bestPracticeScripting"></a>스크립트 개발을 위한 모범 사례
...
### <a name="target-the-apache-hadoop-version"></a><a name="bPS1"></a>Apache Hadoop 버전 대상
...
### <a name="target-the-operating-system-version"></a><a name="bps10"></a>운영 체제 버전 대상 지정

HDInsight는 우분투 리눅스 배포판을 기반으로합니다. HDInsight는 버전이 다르면 다른 버전의 Ubuntu에 의존하는데 이는 스크립트 동작 방식을 변경할 수 있습니다. 예를 들어 HDInsight 3.4 이전 버전은 Upstart를 사용하는 Ubuntu 버전을 기반으로 합니다. 버전 3.5 이상은 Systemd를 사용하는 Ubuntu 16.04를 기반으로 합니다. Systemd 및 Upstart는 다른 명령에 의존하기 때문에 이를 사용하여 작업하기 위해 스크립트가 작성되어야 합니다.
...
### <a name="provide-stable-links-to-script-resources"></a><a name="bPS2"></a>스크립트 리소스에 대한 안정적인 링크 제공
...
### <a name="ensure-that-the-cluster-customization-script-is-idempotent"></a><a name="bPS3"></a>클러스터 사용자 지정 스크립트가 멱등원인지 확인

스크립트는 idempotent여야 합니다. 스크립트를 여러 번 실행하는 경우 매번 동일한 상태로 클러스터를 반환해야 합니다.

예를 들어 구성 파일을 수정하는 스크립트는 여러 번 실행된 경우 중복 항목을 추가해서는 안 됩니다.

### <a name="ensure-high-availability-of-the-cluster-architecture"></a><a name="bPS5"></a>클러스터 아키텍처의 고가용성 확인

Linux 기반 HDInsight 클러스터는 클러스터 내에서 활성화 되는 두 헤드 노드를 제공하며 스크립트 작업은 두 노드에서 실행됩니다. 설치하는 구성 요소에 헤드 노드가 하나만 필요한 경우 두 헤드 노드에 구성 요소를 설치하지 마십시오.
...
### <a name="configure-the-custom-components-to-use-azure-blob-storage"></a><a name="bPS6"></a>Azure Blob Storage를 사용하도록 사용자 지정 구성 요소 구성
...
대부분의 작업의 경우 파일 시스템을 지정할 필요가 없습니다. 예를 들어 다음 복사본은 로컬 파일 시스템에서 클러스터 저장소로 hadoop-common.jar 파일을 복사합니다.
...
### <a name="write-information-to-stdout-and-stderr"></a><a name="bPS7"></a>STDOUT 및 STDERR에 정보 쓰기
...
### <a name="use-retry-logic-to-recover-from-transient-errors"></a><a name="bps9"></a>재시도 논리를 사용하여 일시적인 오류로부터 복구

파일을 다운로드하거나 apt-get을 사용하여 패키지를 설치하거나 인터넷을 통해 데이터를 전송하는 기타 작업을 수행할 때 일시적인 네트워킹 오류로 인해 작업이 실패할 수 있습니다. 예를 들어 통신하는 원격 리소스가 백업 노드로 장애 처리중일 수 있습니다.
...
스크립트 작업 도우미 메서드는 사용자 지정 스크립트를 쓰는 동안 사용할 수 있는 유틸리티입니다. 이러한 메서드는 스크립트에 [https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh](https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh) 포함 됩니다. 다음을 사용하여 다운로드하고 스크립트의 일부로 사용합니다.
...
## <a name="common-usage-patterns"></a><a name="commonusage"></a>일반적인 사용 패턴
...
* HDInsight 클러스터와 연결된 __Azure Data Lake Storage 계정__ HDInsight를 사용하여 Azure 데이터 레이크 스토리지를 사용하는 방법에 대한 자세한 내용은 [빠른 시작: HDInsight의 클러스터 설정](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md)을 참조하십시오.
...
## <a name="checklist-for-deploying-a-script-action"></a><a name="deployScript"></a>스크립트 작업 배포를 위한 검사 목록
...
## <a name="how-to-run-a-script-action"></a><a name="runScriptAction"></a>스크립트 작업을 실행하는 방법
...
Microsoft에서는 HDInsight 클러스터에 구성 요소를 설치하는 샘플 스크립트를 제공합니다. [HDInsight 클러스터의 Hue 설치 및 사용을](hdinsight-hadoop-hue-linux.md) 예제 스크립트 작업으로 사용합니다.
...
*해결 방법*: 텍스트 편집기에서 옵션인 경우 줄 끝에 대해 유닉스 형식 또는 LF를 선택합니다. 또한 Unix 시스템에서 다음 명령을 사용하여 CRLF는 LF로 변경할 수 있습니다.
