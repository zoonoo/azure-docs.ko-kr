...
| IOCache Metadata 서버 상태           | IOCache 메타 데이터 서버를 클라이언트 요청에 대 한 응답으로 확인할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                            |
...
| 기록 서버 프로세스                   | 이 호스트 수준 경고는 네트워크에서 수신 대기 및 수신 대기 상태에 대 한 기록 서버 프로세스를 설정할 수 없는 경우에 트리거됩니다.                                                                |
| 기록 서버 웹 UI                    | 이 호스트 수준 경고는 기록 서버 웹 UI에 연결할 수 없는 경우에 트리거됩니다.                                                                                                              |
| ResourceManager 웹 UI                   | 이 호스트 수준 경고는 ResourceManager 웹 UI에 연결할 수 없는 경우에 트리거됩니다.                                                                                                             |
...
| 사육 사가 장애 조치 (Failover) 컨트롤러 프로세스    | 이 호스트 수준 경고는 네트워크에서 수신 대기 및 수신 대기 상태를 확인할 수 없는 경우에 트리거됩니다.                                                   |
| Oozie 서버 웹 UI                      | Oozie 서버 웹 UI에 연결할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                                                                                |
| Oozie 서버 상태                      | Oozie 서버를 실행 하 고 클라이언트 요청에 응답 하는 것을 확인할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                                      |
| Hive Metastore 프로세스                   | 이 호스트 수준 경고는 Hive Metastore 프로세스가 네트워크에서 수신 하 고 수신 대기 하 고 있는지 확인할 수 없는 경우에 트리거됩니다.                                                                 |
| HiveServer2 프로세스                      | HiveServer가 클라이언트 요청에 응답 하는지 확인할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                                        |
| WebHCat 서버 상태                    | Templeton 서버 상태가 정상이 아닌 경우이 호스트 수준 경고가 트리거됩니다.                                                                                                            |
...
| DataNode 프로세스                         | 이 호스트 수준 경고는 개별 DataNode 프로세스를 설정 하 고 네트워크에서 수신 대기 하지 못할 경우 트리거됩니다.                                                         |
...
| Zeppelin 서버 상태                   | Zeppelin 서버를 실행 하 고 클라이언트 요청에 응답 하는 것을 확인할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                                   |
| HiveServer2 대화형 프로세스          | HiveServerInteractive가 클라이언트 요청에 응답 하는지 확인할 수 없는 경우이 호스트 수준 경고가 트리거됩니다.                                                             |
...
클러스터의 SSH 사용자 계정 이름 및 SSH 주소를 제공해야 하는 클러스터에 연결하는 경우 SSH 클라이언트를 사용하는 것과 비슷합니다. 예를 들어, `sftp username@mycluster-ssh.azurehdinsight.net`을 입력합니다. 메시지가 표시되면 계정에 대한 암호를 제공하거나 `-i` 매개 변수를 사용하여 공개 키를 제공해야 합니다.
...
* **Azure CLI**: [Az hdinsight create](https://docs.microsoft.com/cli/azure/hdinsight?view=azure-cli-latest#az-hdinsight-create) 명령을 사용 하는 경우 `--headnode-size`, `--workernode-size`및 `--zookeepernode-size` 매개 변수를 사용 하 여 헤드, 작업자 및 사육 아웃 노드의 크기를 설정할 수 있습니다.
