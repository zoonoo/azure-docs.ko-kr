---
title: '자습서: Azure HDInsight에서 대화형 쿼리를 사용하여 ETL 작업 수행'
...
HDInsight 클러스터와 연결된 스토리지로 데이터를 업로드하는 여러 가지 방법이 있습니다. 이 섹션에서는 `scp`를 사용하여 데이터를 업로드합니다. 데이터를 업로드하는 다른 방법에 대해 알아보려면 [HDInsight에 데이터 업로드](../hdinsight-upload-data.md)를 참조하세요.
