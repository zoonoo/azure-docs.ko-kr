...
D14 v2 작업자 노드의 경우 LLAP 큐에 권장되는 값은 **80**입니다.   
...
구성: ***hive.llap.daemon.num.executors***, ***hive.llap.io.threadpool.size***
...
실행기 수를 조정해야 하는 경우 *hive.tez.container.size*에 지정된 대로 실행기당 4GB의 메모리를 고려하고 모든 실행기에 필요한 총 메모리가 LLAP 디먼 컨테이너에 사용 가능한 총 메모리를 초과하지 않도록 해야 합니다.  
