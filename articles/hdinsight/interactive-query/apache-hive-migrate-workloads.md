...
1. 다음 명령을 입력 하 여 열려 있는 SSH 세션에서 [Beeline 클라이언트](../hadoop/apache-hadoop-use-hive-beeline.md) 를 사용 하 여 HiveServer2에 연결 합니다.
    ...
    이 명령은 **alltables.sql. hql**라는 파일을 생성 합니다.
...
아래 표에 있는 값을 사용 합니다. 공백으로 `SQLSERVERNAME DATABASENAME USERNAME PASSWORD` 구분 된 **복사** 된 Hive metastore의 적절 한 값으로 대체 합니다. SQL server 이름을 지정할 때 ". database.windows.net"를 포함 하지 마십시오.
...
이전 단계를 완료 하 여 Hive Metastore를 HDInsight 4.0로 마이그레이션한 후 Metastore에 기록 된 테이블과 데이터베이스는 클러스터 내에서 또는 `show tables` `show databases` 를 실행 하 여 hdinsight 4.0 클러스터 내에서 볼 수 있습니다. HDInsight 4.0 클러스터에서 쿼리 실행에 대 한 자세한 내용은 [hdinsight 버전 간 쿼리 실행](#query-execution-across-hdinsight-versions) 을 참조 하세요.
...
1. 을 실행 `ls /usr/hdp`하 여 데이터 플랫폼 스택 버전을 확인 합니다. 그러면 다음 명령에서 사용 해야 하는 버전 문자열이 표시 됩니다.
