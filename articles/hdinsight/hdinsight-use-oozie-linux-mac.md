...
# Hadoop과 함께 Oozie를 사용하여 Linux 기반 HDInsight에서 워크플로 실행(미리 보기)
...
Oozie에는 작업을 같은 디렉터리에 저장하는 데 사용되는 리소스가 필요합니다. 이 예제에서는 **wasb:///tutorials/useoozie**를 사용합니다. 다음 명령을 사용하여 이 디렉터리와 이 워크플로에서 만든 새 Hive 테이블을 유지할 data 디렉터리를 만듭니다.
...
> [AZURE.IMPORTANT]이러한 단계를 수행하기 전에 [Azure CLI를 설치하고 구성](xplat-cli.md)해야 합니다. HDInsight 클러스터 또는 로컬 워크스테이션에서 CLI를 설치하고 데이터베이스를 만드는 단계를 수행할 수 있습니다.
...
1. HDInsight 클러스터에 대한 SSH 터널을 만듭니다. 작업 방법은 다음 중 하나를 참조하세요.

	* [Linux, Unix 또는 OS X의 HDInsight에서 Linux 기반 Hadoop과 SSH를 사용합니다.](hdinsight-hadoop-linux-use-ssh-unix.md#tunnel)
...
3. 페이지의 왼쪽부터 **Oozie**, **Quick Links**, **Oozie Web UI**를 차례로 선택합니다.

	![메뉴 이미지](./media/hdinsight-use-oozie-linux-mac/ooziewebuisteps.png)

4. Oozie 웹 UI는 기본적으로 실행 중인 워크플로 작업을 표시합니다. 모든 워크플로 작업을 보려면 **All Jobs**를 선택합니다.
...
6. Job Info 탭에서 기본 작업 정보 및 작업 내의 개별 동작을 볼 수 있습니다. 맨 위에 있는 탭을 사용하여 작업 정의 및 작업 구성을 보거나, 작업 로그에 액세스하거나, 작업의 DAG(방향성 비순환 그래프)를 확인할 수 있습니다.
	...
	* **Job Log**: **GetLogs** 단추를 선택하여 작업에 대한 모든 로그를 가져오거나 **Enter Search Filter** 필드를 사용하여 로그를 필터링합니다.
...
7. **Job Info** 탭에서 동작 중 하나를 선택하면 해당 동작에 대한 정보가 표시됩니다. 예를 들어 **RunHiveScript** 동작을 선택합니다.

	![동작 정보](./media/hdinsight-use-oozie-linux-mac/action.png)

8. 작업에 대한 JobTracker 정보를 보는 데 사용할 수 있는 **Console URL**의 링크를 포함하여 동작에 대한 세부 정보를 확인할 수 있습니다.
...
1. 다음을 사용하여 **coordinator.xml**이라는 새 파일을 만듭니다.
	...
	* **${coordStart}**: 작업 시작 시간
	* **${coordEnd}**: 작업 종료 시간
	* **${coordTimezone}**: 일광 절약 시간제(UTC를 사용하여 일반적으로 표시 됨)없이 고정된 시간대에서 코디네이터 작업을 처리합니다. 이 시간대를 "Oozie 처리 시간대"라고 합니다.
...
7. Oozie 웹 UI를 방문하여 **Coordinator Jobs** 탭을 선택하면 다음과 유사한 정보가 표시됩니다.

	![코디네이터 작업 탭](./media/hdinsight-use-oozie-linux-mac/coordinatorjob.png)

	**Next Materialization** 항목을 기록해 둡니다. 이 항목은 다음에 작업을 실행할 때 사용됩니다.
...
8. 이전 워크플로 작업과 마찬가지로 웹 UI에서 작업 항목을 선택하면 작업에 대한 정보가 표시됩니다.
	...
	성공한 작업 실행만 표시되며, 예약된 워크플로 내의 개별 동작은 표시되지 않습니다. 확인하려면 **Action** 항목 중 하나를 선택합니다. 이전 워크플로 작업에 대해 검색된 것과 유사한 정보가 표시됩니다.
...
2. 특정 동작에 대한 오류 또는 실패가 있는 경우 해당 동작을 선택하여 **Error Message** 필드에 오류에 대한 자세한 정보가 제공되는지 확인합니다.
...
**증상**: 작업 상태가 **KILLED**로 변경됩니다. 작업에 대한 세부 정보에 RunSqoopExport 상태가 **ERROR**로 표시됩니다. 동작을 선택하면 다음과 같은 오류 메시지가 표시됩니다.
...
1. sqljdbc4.jar 파일을 /tutorials/useoozie 디렉터리에 복사합니다.
