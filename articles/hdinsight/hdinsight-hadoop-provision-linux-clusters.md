---
title: Azure HDInsight에서 Apache 및 R Server에 대 한 클러스터 설정
...
## <a name="basic-cluster-setup"></a>기본 클러스터 설정

이 문서에서는 기본 보기 또는 *클래식*을 사용 하 여 HDInsight 클러스터를 만들 수 있는 [Azure Portal](https://portal.azure.com)의 설치 과정을 안내 합니다.

![HDInsight 생성 옵션 사용자 지정 빨리 만들기](./media/hdinsight-hadoop-provision-linux-clusters/azure-portal-cluster-basics-blank-fs.png)

화면의 지침을 따릅니다. 다음 사항에 대한 세부 정보가 아래에 제공됩니다.
...
* HTTP 사용자: 기본 사용자 이름은 *admin*입니다. Azure Portal에서 기본 구성을 사용 합니다. 경우에 따라 "클러스터 사용자"라고도 합니다.
* SSH 사용자: SSH를 통해 클러스터에 연결 하는 데 사용 됩니다. 자세한 내용은 [HDInsight와 함께 SSH 사용](hdinsight-hadoop-linux-use-ssh-unix.md)을 참조하세요.
...
## <a name="location"></a>클러스터 및 스토리지 위치(영역)

클러스터 위치를 명시적으로 지정할 필요가 없습니다. 클러스터는 기본 스토리지와 같은 위치에 있습니다. 지원되는 지역 목록은 **HDInsight 가격** 에서 [지역](https://go.microsoft.com/fwLink/?LinkID=282635&clcid=0x409)드롭다운 목록을 클릭하세요.
...
![클러스터 스토리지 설정: HDFS 호환 스토리지 엔드포인트](./media/hdinsight-hadoop-provision-linux-clusters/azure-portal-cluster-storage.png)
...
![HDInsight 생성 옵션 선택 엔터프라이즈 보안 패키지](./media/hdinsight-hadoop-provision-linux-clusters/azure-portal-cluster-security-networking-esp.png)
...
## <a name="extend-clusters-with-a-virtual-network"></a>가상 네트워크를 사용하여 클러스터 확장
...
![클러스터 구성 응용 프로그램 Azure Portal](./media/hdinsight-hadoop-provision-linux-clusters/azure-portal-cluster-configuration-applications.png)
...
![클러스터 구성 스크립트 작업 Azure Portal](./media/hdinsight-hadoop-provision-linux-clusters/azure-portal-cluster-configuration-scriptaction.png)
