...
Hortonworks Sandbox가 실행되고 있는지 확인하고 [Hadoop 에코시스템에서 시작](hdinsight-hadoop-emulator-get-started.md#set-sandbox-passwords)의 단계에 따라 SSH `root` 계정 및 Ambari `admin` 계정에 대한 암호를 구성합니다. Visual Studio에서 샌드박스에 연결할 때 이러한 암호를 사용합니다.
...
5. 서비스의 유효성 검사가 완료되기를 기다립니다. 경우에 따라 유효성 검사가 실패하고 구성을 업데이트하라는 메시지가 표시될 수 있습니다. 이 경우 **업데이트** 단추를 선택하고 서비스의 구성 및 확인이 완료되기를 기다립니다.
...
2. 새 쿼리 창에서 다음을 입력합니다.
    ...
    **전송** 옆의 드롭다운 메뉴를 사용하여 **고급**을 선택할 수도 있습니다. 그러면 작업을 제출할 때 추가 옵션을 제공할 수 있는 대화 상자가 열립니다.
...
3. 쿼리를 제출하면 작업 상태가 표시됩니다. Hadoop에서 처리되는 대로 작업에 정보를 제공합니다. **작업 상태** 항목은 작업의 현재 상태를 제공합니다. 상태는 정기적으로 업데이트됩니다. 또는 새로 고침 아이콘을 사용하여 상태를 수동으로 새로 고칠 수 있습니다.
    ...
    **작업 상태**가 **완료됨**으로 변경되면 DAG(방향성 비순환 그래프)가 표시됩니다. Tez(로컬 클러스터의 Hive에 대한 기본 실행 엔진)에서 결정된 실행 경로를 설명합니다.
...
4. **Batch** 필드를 **대화형**으로 변경한 다음 **실행**을 선택하면 Hive 작업을 대화형으로 실행할 수도 있습니다.
    ...
    처리 중에 생성된 출력 로그를 **HiveServer2 출력** 창으로 스트리밍합니다.

    > [!NOTE]
    > 이는 작업 완료 후 **작업 로그** 링크에서 사용할 수 있는 것과 동일한 정보입니다.
...
2. 이 프로젝트로 만든 **script.pig** 파일의 콘텐츠로 다음을 입력합니다.
...
1. **서버 탐색기**에서 로컬 클러스터를 마우스 오른쪽 단추로 클릭한 다음 **작업 보기**를 선택합니다. 그러면 클러스터에 제출된 작업 목록이 표시됩니다.
...
1. **서버 탐색기**에서 **HDInsight 로컬 클러스터** 항목, **Hive 데이터베이스**를 차례로 확장합니다. 이렇게 하면 **기본** 및 **xademo** 데이터베이스가 로컬 클러스터에 표시됩니다. 데이터베이스 확장하면 데이터베이스 내의 테이블이 표시됩니다.
...
데이터베이스 또는 테이블에 대한 **속성**을 보도록 선택할 수 있습니다. 속성 창에서 선택한 항목에 대한 세부 정보를 보여 줍니다.
