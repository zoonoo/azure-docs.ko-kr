---
title: "두 Azure Virtual Network 간 DNS 구성 | Microsoft Docs"
...
---
# <a name="configure-dns-between-two-azure-virtual-networks"></a>두 Azure 가상 네트워크 간의 DNS 구성
...
* [두 Virtual Network 간의 VPN 연결 구성][hdinsight-hbase-geo-replication-vnet]
* 가상 네트워크에 대한 DNS 구성(이 자습서)
* [HBase 지역에서 복제 구성][hdinsight-hbase-geo-replication]

다음 다이어그램에서는 [두 Virtual Network 간의 VPN 연결 구성][hdinsight-hbase-geo-replication-vnet]에서 만든 두 Virtual Network를 보여 줍니다.
...
* **VPN으로 연결된 두 개의 Azure 가상 네트워크**.  자세한 내용은 [두 Azure Virtual Network 간의 VPN 연결 구성][hdinsight-hbase-geo-replication-vnet]을 참조하세요.

    > [!NOTE]
    > Azure 서비스 이름과 가상 컴퓨터 이름은 고유해야 합니다. 이 자습서에서 사용되는 이름은 Contoso-[Azure Service/VM name]-[EU/US]입니다. 예를 들어 Contoso-VNet-EU는 북유럽 데이터 센터에 있는 Azure 가상 네트워크이고, Contoso-DNS-US는 미국 동부 데이터 센터에 있는 DNS 서버 VM입니다. 사용자 고유의 이름을 사용해야 합니다.

## <a name="create-azure-virtual-machines-to-be-used-as-dns-servers"></a>DNS 서버로 사용할 Azure 가상 컴퓨터 만들기
...
## <a name="set-static-ip-addresses-for-the-two-virtual-machines"></a>두 가상 컴퓨터에 대한 고정 IP 주소 설정
...
## <a name="add-the-dns-server-role-the-two-virtual-machines"></a>두 가상 컴퓨터에 DNS 서버 역할 추가
...
5. RDP 세션 내에서 왼쪽 아래에 있는 Windows 단추를 클릭하여 시작 화면을 엽니다.
6. **서버 관리자** 타일을 클릭합니다.
7. **역할 및 기능 추가**를 클릭합니다.
8. 페이지 맨 아래에 있는 **다음**
9. **역할 기반 또는 기능 기반 설치**를 선택하고 **다음**을 클릭합니다.
10. DNS 가상 컴퓨터(이미 강조 표시되어 있음)를 선택하고 **다음**을 클릭합니다.
11. **DNS 서버**를 선택합니다.
12. **기능 추가**를 클릭하고 **계속**을 클릭합니다.
13. **다음**을 세 번 클릭한 후 **설치**를 클릭합니다.

**Contoso-DNS-US에 대한 DNS 서버 역할을 추가하려면**

* 같은 단계를 반복하여 **Contoso-DNS-US**에 DNS 역할을 추가합니다

## <a name="assign-dns-servers-to-the-virtual-networks"></a>가상 네트워크에 DNS 서버 할당
**두 DNS 서버를 등록하려면**
...
## <a name="configure-dns-conditional-forwarders"></a>DNS 조건부 전달자 구성
...
## <a name="test-the-name-resolution-across-the-virtual-networks"></a>가상 네트워크에서 이름 확인 테스트
...
* [두 Azure Virtual Network 간의 VPN 연결 구성][hdinsight-hbase-geo-replication-vnet]
* [HBase 지역에서 복제 구성][hdinsight-hbase-geo-replication]
