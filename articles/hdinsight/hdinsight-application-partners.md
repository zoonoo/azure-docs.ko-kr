<properties
	pageTitle="HDInsight 응용 프로그램 파트너 | Microsoft Azure" 
	description="이제 고객이 프로비전 포털 내에서 바로 HDInsight 클러스터와 함께 빅 데이터 응용 프로그램을 쉽게 검색하고 배포할 수 있습니다."
	services="hdinsight" 
...
RedPoint 사용:
-  모든 데이터 품질 및 데이터 통합 기능을 Hadoop 클러스터에서 수행할 수 있음 - ELT, 정리, 일치, 중복 제거, 병합/삭제, 하우스홀딩, 구문 분석, 분할, 추가, 주소 표준화, 키 생성 및 유지 관리, 자동화, 모니터링, 알림  
-  MapReduce 관련 없음, MapReduce 기술 필요 없음  
-  MapReduce 기반 솔루션과 마찬가지로 RedPoint를 사용하여 데이터 품질 및 통합 프로세스를 대부분의 경우 더욱 효율적으로 수행  
-  처리, 분석, 보고 또는 다른 작업을 위해 데이터를 Hadoop 외부로 이동할 필요 없음  
-  소프트웨어를 클러스터 자체에 설치할 필요가 없으며 RedPoint는 클러스터의 컴퓨팅 리소스에 대해 경쟁하기 보다는 YARN의 작업 우선 순위 지정을 따름  
-  별도 환경에서 데이터를 함께 가져오거나 다른 환경으로 데이터를 마이그레이션하는 경우에도 사용자는 단일 제품으로 기존 및 Hadoop 리포지토리 모두에서 데이터를 관리할 수 있음
