---
title: "Apache Storm에 대한 Java 기반 토폴로지 개발| Microsoft Docs"
...
---
# <a name="develop-java-based-topologies-for-a-basic-word-count-application-with-apache-storm-and-maven-on-hdinsight"></a>HDInsight에서 Apache Storm 및 Maven으로 기본 단어 개수 응용 프로그램에 대한 Java 기반 토폴로지를 개발합니다.

Maven을 사용하여 HDInsight에서 Apache Storm에 대한 Java 기반 토폴로지를 만드는 방법을 알아봅니다. Maven 및 Java를 사용하여 기본 단어 개수 응용 프로그램을 만들고 여기서 토폴로지는 Java로 정의됩니다. 그런 다음 Flux 프레임워크를 사용하여 토폴로지를 정의하는 방법을 알아봅니다.
...
> [!NOTE]
> 이 문서에서 만든 토폴로지의 완료된 버전은 [https://github.com/Azure-Samples/hdinsight-java-storm-wordcount](https://github.com/Azure-Samples/hdinsight-java-storm-wordcount)에서 사용할 수 있습니다.
...
> [!NOTE]
> PowerShell을 사용하는 경우 큰 따옴표로 `-D` 매개 변수를 묶어야 합니다. 다음은 PowerShell을 사용하는 경우 명령의 예제입니다.
>
> `mvn archetype:generate "-DarchetypeArtifactId=maven-archetype-quickstart" "-DgroupId=com.microsoft.example" "-DartifactId=WordCount" "-DinteractiveMode=false"`

그러면 기본 Maven 프로젝트를 포함하는 현재 위치에 `WordCount`라는 새 디렉터리가 만들어집니다.

`WordCount` 디렉터리에는 다음과 같은 항목이 포함됩니다.
...
### <a name="remove-the-example-code"></a>예제 코드를 제거합니다.
...
이는 Storm 토폴로지이므로 Storm 구성 요소에 대한 종속성을 추가해야 합니다. `<dependencies>` 섹션에서 `pom.xml` 파일을 열고 다음 코드를 추가합니다.
...
> [!NOTE]
> 이 섹션에서 `<scope>provided</scope>` 줄을 확인합니다. 이는 Maven이 만든 JAR 파일에서 **storm-core**를 제외하도록 요청합니다. 시스템을 통해 제공되기 때문입니다. 이를 통해 만든 패키지를 좀 더 작게 할 수 있으며 이는 HDInsight 클러스터에서 Storm에 포함된 **storm-core** 비트를 사용합니다.
...
Storm 토폴로지의 경우 [Exec Maven 플러그 인](http://mojo.codehaus.org/exec-maven-plugin/)을 사용하면 개발 환경에서 로컬로 토폴로지를 쉽게 실행할 수 있으므로 유용합니다. Exec Maven 플러그 인을 추가하려면 `pom.xml` 파일의 `<plugins>` 섹션에 다음을 추가합니다.
...
> [!NOTE]
> `<mainClass>` 항목은 `${storm.topology}`를 사용합니다. 이 항목은 `pom.xml`의 속성 섹션에 정의되어 있지 않습니다. 대신 이후 단계의 개발 환경에서 토폴로지를 실행하는 경우 명령줄에서 이 값을 설정합니다.

다른 유용한 플러그 인은 컴파일 옵션을 변경하는 데 사용되는 [Apache Maven 컴파일러 플러그 인](http://maven.apache.org/plugins/maven-compiler-plugin/)입니다. 이 플러그 인이 필요한 주요한 이유는 사용자의 응용 프로그램에 대한 원본 및 대상에 Maven이 사용하는 Java 버전을 변경하기 위해서입니다.

* HDInsight __3.4 이하__의 경우 원본과 대상의 Java 버전을 __1.7__로 설정합니다.

* HDInsight __3.5__의 경우 원본과 대상의 Java 버전을 __1.8__로 설정합니다.

Apache Maven Compiler 플러그 인을 포함하려면 `pom.xml` 파일의 `<plugins>` 섹션에 다음을 추가합니다. 여기서 1.8을 지정하므로 대상 HDInsight 버전은 3.5가 됩니다.
...
그러면 프로젝트의 루트에 있는 리소스 디렉터리(`${basedir}`)가 리소스를 포함하는 위치로 추가되고 `log4j2.xml`이라는 파일이 포함됩니다. 이 파일은 토폴로지에서 기록하는 정보를 구성하는 데 사용됩니다.
...
spout의 경우 `src\main\java\com\microsoft\example` 디렉터리에 `RandomSentenceSpout.java`이라는 새 파일을 만들고 다음을 그 내용으로 사용합니다.
...
`src\main\java\com\microsoft\example` 디렉터리에서 두 개의 새 파일인 `SplitSentence.java` 및 `WordCount.Java`를 만듭니다. 파일 내용으로 다음을 사용합니다.
...
토폴로지를 구현하려면 `WordCountTopology.java` 디렉터리에서 `src\main\java\com\microsoft\example`이라는 새 파일을 만듭니다. 파일 내용으로 다음을 사용합니다.
...
Storm은 Apache Log4j를 사용하여 정보를 기록합니다. 로깅을 구성하지 않으면 토폴로지는 읽기 어려울 수 있는 많은 진단 정보를 내보냅니다. 로깅되는 내용을 제어하려면 `resources` 디렉터리에서 `log4j2.xml`이라는 파일을 만듭니다. 다음을 파일 콘텐츠로 사용합니다.
...
이렇게 하면 이 예제 토폴로지의 구성 요소를 포함하는 `com.microsoft.example` 클래스에 대한 새 로거가 구성됩니다. 수준은 이 로거에 대한 추적으로 설정되며, 이 토폴로지의 구성 요소에서 내보낸 모든 로깅 정보가 캡처됩니다. 이 프로젝트에 대한 코드를 다시 살펴보면 WordCount.java 파일만 로깅을 구현하며 각 단어의 개수를 기록합니다.

`<Root level="error">` 섹션은 루트 수준의 로깅(모든 항목이 `com.microsoft.example`에 있지는 않음)을 오류 정보를 기록하도록 구성합니다.

> [!IMPORTANT]
> 이렇게 하면 개발 환경에서 토폴로지를 테스트할 때 기록되는 정보가 크게 줄어들지만, 프로덕션의 클러스터에서 실행할 경우 생성되는 모든 디버그 정보가 제거되지는 않습니다. 해당 정보를 줄이려면 클러스터에 제출된 구성에서도 디버깅을 false로 설정해야 합니다. 예제는 이 문서의 WordCountTopology.java 코드를 참조하세요. 
...
또한 단어 내보내기와 개수 사이에는 5초의 간격이 있습니다. 이는 틱 튜플이 도달할 때 정보를 내보내도록 **WordCount** 구성 요소를 구성했기 때문에 발생하며 이러한 튜플이 기본적으로 5초마다 배달되도록 요청합니다.

## <a name="convert-the-topology-to-flux"></a>토폴로지를 Flux로 변환

Flux는 Storm 0.10.0 이상에서 사용할 수 있는 새로운 프레임워크로서 구성을 구현과 분리할 수 있습니다. 구성 요소(Bolt 및 Spout)는 Java로 정의되지만 토폴로지는 YAML 파일을 사용하여 정의됩니다.
...
1. `WordCountTopology.java` 파일을 프로젝트에서 제외합니다. 이전에 토폴로지를 정의하지만 Flux에는 필요하지 않습니다.

2. `resources` 디렉터리에서 `topology.yaml`라는 새 파일을 만듭니다. 이 파일의 내용으로 다음을 사용합니다.
...
3. `pom.xml` 파일을 다음과 같이 변경합니다.
   ...
   * **exec-maven-plugin** `<configuration>` 섹션에서 `<mainClass>` 값을 `org.apache.storm.flux.Flux`로 변경합니다. 이렇게 하면 개발 중에 로컬로 실행하는 경우 Flux가 토폴로지를 실행하여 처리할 수 있습니다.
