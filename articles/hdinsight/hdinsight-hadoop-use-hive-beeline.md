...
Beeline은 HDInsight 클러스터의 헤드 노드에 포함된 명령줄 도구입니다. JDBC를 사용하여 HDInsight 클러스터에서 호스팅되는 서비스인 HiveServer2에 연결합니다. 다음 표에서는 Beeline과 함께 사용할 연결 문자열을 제공합니다.

| Beeline을 실행하는 위치 | 연결 문자열 | 다른 매개 변수 |
| --- | --- | --- |
| 헤드 노드에 대한 SSH 연결 | `jdbc:hive2://localhost:10001/;transportMode=http` | `-n admin` |
| 에지 노드 | `jdbc:hive2://headnodehost:10001/;transportMode=http` | `-n admin` |
| 클러스터 외부 | `jdbc:hive2://clustername.azurehdinsight.net:443/;ssl=true;transportMode=http;httpPath=/hive2` | `-n admin -p password` |
...
* SSH 클라이언트. SSH를 사용하는 방법에 대한 자세한 내용은 [HDInsight와 함께 SSH 사용](hdinsight-hadoop-linux-use-ssh-unix.md)을 참조하세요.

## <a id="ssh"></a>SSH를 사용하여 연결

다음 명령을 사용하여 SSH를 사용하는 클러스터에 연결합니다.

```bash
ssh sshuser@myhdinsight-ssh.azurehdinsight.net
```

`sshuser`를 클러스터의 SSH 계정으로 바꿉니다. `myhdinsight` 을 HDInsight 클러스터 이름으로 바꿉니다.

HDInsight 클러스터를 만들 때 **SSH 인증을 위한 암호를 제공한 경우** 메시지가 표시되면 암호를 제공해야 합니다.
...
1. SSH 세션에서 다음 명령을 사용하여 Beeline을 시작합니다.
...
4. 다음 문을 입력하여 HDInsight 클러스터와 함께 제공되는 샘플 데이터로 **log4jLogs**라는 새 테이블을 만듭니다.
    ...
    * **ROW FORMAT** - 데이터의 형식을 지정하는 방법입니다. 이 경우, 각 로그의 필드는 공백으로 구분됩니다.
    ...
    * **INPUT__FILE__NAME LIKE '%.log'** - 다른 파일과 함께 예제 데이터 파일을 저장합니다. 이 문은 쿼리를 .log로 끝나는 파일에 저장된 데이터로 제한합니다.
...
4. Beeline을 사용하여 파일을 실행하려면 다음을 사용합니다. **HOSTNAME**을 헤드 노드에 가져온 이전 이름으로 바꾸고 **PASSWORD**를 관리자 계정의 암호로 바꿉니다.
    ...
    > [!NOTE]
    > `-i` 매개 변수는 Beeline을 시작하고 query.hql 파일의 문을 실행합니다. 쿼리가 완료되면 `jdbc:hive2://localhost:10001/>` 프롬프트가 표시됩니다. 쿼리 완료 후 Beeline을 종료하는 `-f` 매개 변수를 사용하여 파일을 실행할 수도 있습니다.
...
클러스터에 에지 노드가 있으면 SSH로 연결할 때 항상 헤드 노드 대신 에지 노드를 사용하는 것이 좋습니다. Beeline을 SSH 연결에서 에지 노드로 시작하려면 다음 명령을 사용합니다.

```bash
beeline -u 'jdbc:hive2://headnodehost:10001/;transportMode=http' -n admin
```

## <a name="remote-clients"></a>원격 클라이언트
