...
ESP (Enterprise Security Package) 클러스터는 Azure HDInsight 클러스터에 대 한 다중 사용자 액세스를 제공 합니다. ESP를 사용 하는 HDInsight 클러스터는 도메인에 연결 됩니다. 이 연결을 통해 도메인 사용자는 도메인 자격 증명을 사용 하 여 클러스터를 인증 하 고 빅 데이터 작업을 실행할 수 있습니다.
