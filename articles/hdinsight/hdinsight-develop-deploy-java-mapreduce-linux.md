...
2. Maven과 함께 설치되는 **mvn** 명령을 사용하여 프로젝트용 스캐폴딩을 생성합니다.
    ...
    이 명령은 **artifactID** 매개 변수로 지정된 이름(이 예제에서는 **wordcountjava**)을 사용하여 현재 디렉터리에 새 디렉터리를 만듭니다. 이 디렉터리에는 다음 항목이 포함됩니다.
...
3. **src/test/java/org/apache/hadoop/examples/apptest.java** 파일은 이 예에서 사용되지 않으므로 삭제합니다.
...
1. **wordcountjava** 디렉터리로 변경합니다(아직 이동하지 않은 경우).
