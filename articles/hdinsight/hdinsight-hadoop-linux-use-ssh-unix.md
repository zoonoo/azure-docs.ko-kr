...
* __프라이빗 키에 대한 경로__ 를 지정할 수 있습니다. `ssh` 클라이언트에서 `-i` 매개 변수를 사용하여 프라이빗 키에 대한 경로를 지정합니다. 정의합니다(예: `ssh -i ~/.ssh/id_rsa sshuser@myedge.mycluster-ssh.azurehdinsight.net`).
