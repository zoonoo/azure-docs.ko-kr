...
description: Azure 포털을 사용하여 Azure HDInsight 클러스터를 만들고 관리하는 방법을 알아봅니다.
...
Azure [포털을](https://portal.azure.com)사용하여 Azure HDInsight에서 [아파치 하두롭](https://hadoop.apache.org/) 클러스터를 관리할 수 있습니다. 다른 도구를 사용하여 HDInsight에서 Hadoop 클러스터를 관리하는 방법에 대한 정보를 보려면 위쪽에 있는 탭 선택기를 사용하세요.
...
에 로그인합니다. [https://portal.azure.com](https://portal.azure.com)

## <a name="list-and-show-clusters"></a><a name="showClusters"></a>클러스터 목록 및 표시
...
HDInsight 클러스터 페이지에서 클러스터 이름을 [**선택합니다.**](#showClusters)  그러면 다음 이미지와 비슷한 **개요** 보기가 열립니다.
...
**왼쪽 메뉴:**  
  ...
  - **설정 메뉴**  
    ...
    |SSH + 클러스터 로그인|SSH(보안 셸) 연결을 사용하여 클러스터에 연결하는 지침을 보여줍니다. 자세한 내용은 [HDInsight와 SSH 사용을](hdinsight-hadoop-linux-use-ssh-unix.md)참조하십시오.|
  ...
  - **모니터링 메뉴**
    ...
    |---|---|
    |경고|경고와 작업을 관리합니다.|
    |메트릭|Azure 모니터 로그에서 클러스터 메트릭을 모니터링합니다.|
    |진단 설정|진단 메트릭을 저장하는 위치에 대한 설정입니다.|
    |Azure Monitor|Azure 모니터에서 클러스터를 모니터링합니다.|
...
|Private Endpoint|클러스터의 개인 끝점입니다.|
|보안 쉘(SSH)|SSH를 통해 클러스터에 액세스할 때 사용할 사용자 이름과 호스트 이름입니다.|
|상태|중 하나: 중단, 허용 됨, 클러스터스토리지프로비저닝, AzureVMConfiguration, HDInsightConfiguration, 운영, 실행 중, 오류, 삭제, 삭제, 시간 초과, 삭제대기, 삭제 시간 초과, 삭제 오류, 패치큐, 인증서롤오버큐, 크기 조정 또는 클러스터 사용자 지정.|
...
|가상 네트워크|배포 시 클러스터가 선택된 경우 클러스터가 배포되는 가상 네트워크의 이름입니다.|
...
클러스터를 삭제해도 기본 저장소 계정이나 연결된 저장소 계정이 삭제되지 않습니다. 동일한 스토리지 계정과 동일한 Metastore를 사용하여 클러스터를 다시 만들 수 있습니다. 클러스터를 다시 만들 때 새 기본 Blob 컨테이너를 사용하는 것이 좋습니다.
...
전체 정보는 [HDInsight 스케일 클러스터를](./hdinsight-scaling-best-practices.md) 참조하십시오.

## <a name="pauseshut-down-clusters"></a>클러스터 일시 중지/종료

대부분의 Hadoop 작업은 이따금 실행되는 일괄 처리 작업입니다. 대부분의 Hadoop 클러스터의 경우 클러스터가 처리에 사용되지 않는 오랜 시간이 있습니다. HDInsight를 사용하면 데이터가 Azure Storage에 저장되기 때문에 클러스터를 사용하지 않을 때 안전하게 삭제할 수 있습니다.
...
- Azure CLI 사용 [Azure CLI를 사용하여 Azure HDInsight 클러스터 관리를](hdinsight-administer-use-command-line.md)참조하십시오.
...
1. **클러스터 대시보드**를 선택합니다.
    ...
    ![HDInsight 아파치 하두롭 클러스터 메뉴](./media/hdinsight-administer-use-portal-linux/hdinsight-azure-portal-cluster-menu2.png)
...
HDInsight 클러스터마다 두 개의 사용자 계정이 포함될 수 있습니다. HDInsight 클러스터 사용자 계정(HTTP 사용자 계정)과 SSH 사용자 계정은 생성 프로세스 중에 만들어집니다. 포털을 사용하여 클러스터 사용자 계정 암호를 변경할 수 있으며, 스크립트 작업을 사용하여 SSH 사용자 계정을 변경할 수 있습니다.
...
3. 클러스터 [홈 페이지에서](#homePage) **설정**에서 **스크립트 작업을** 선택합니다.
4. 스크립트 **작업** 페이지에서 **새 제출을**선택합니다.
5. 스크립트 **제출 작업** 페이지에서 다음 정보를 입력합니다.
   ...
   | 노드 유형: (머리, 작업자, 님버스, 감독자 또는 사육사) |나열된 모든 노드 형식에 대한 ✓ |
   | 매개 변수 |SSH 사용자 이름 및 새 암호를 입력합니다. 사용자 이름과 암호 사이에 공백이 하나 있어야 합니다. |
   | 이 스크립트 작업을 유지... |이 필드는 선택 취소로 둡니다. |

6. **만들기**를 선택하여 스크립트를 적용합니다. 스크립트가 완료되면 새 암호로 SSH를 사용하여 클러스터에 연결할 수 있습니다.
...
[클러스터 홈 페이지의](#homePage) 클러스터 **크기** 타일에는 이 클러스터에 할당된 코어 수와 클러스터 내의 노드에 할당되는 방식이 표시됩니다.

> [!IMPORTANT]  
> HDInsight 클러스터에 의해 제공되는 서비스를 모니터링하려면 Ambari 웹 또는 Ambari REST API를 사용해야 합니다. 암바리 사용에 대한 자세한 내용은 [아파치 암바리를 사용하여 HDInsight 클러스터 관리를](hdinsight-hadoop-manage-ambari.md) 참조하십시오.
