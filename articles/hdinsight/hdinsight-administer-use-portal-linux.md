...
**왼쪽 메뉴:**  
  ...
  - **설정 메뉴**  
    ...
    |저장소 계정|스토리지 계정 및 키를 봅니다. 스토리지 계정은 클러스터를 만드는 과정에서 구성됩니다.|
    |애플리케이션|HDInsight 애플리케이션을 추가/제거합니다.  [사용자 지정 HDInsight 애플리케이션 설치](hdinsight-apps-install-custom-applications.md)를 참조하세요.|
    |스크립트 동작|클러스터에서 Bash 스크립트를 실행합니다. [스크립트 작업을 사용하여 Linux 기반 HDInsight 클러스터 사용자 지정](hdinsight-hadoop-customize-cluster-linux.md)을 참조하세요.|
  ...
  - **모니터링 메뉴**
    ...
    |Operations Management Suite|Azure OMS (Operations Management Suite) 및 Azure Monitor 로그에서 클러스터를 모니터링 합니다.|
...
이러한 서비스에는 기본적으로 액세스 권한이 부여됩니다. [Azure PowerShell](hdinsight-administer-use-powershell.md#grantrevoke-access)를 사용 하 여 액세스를 취소/부여할 수 있습니다.
