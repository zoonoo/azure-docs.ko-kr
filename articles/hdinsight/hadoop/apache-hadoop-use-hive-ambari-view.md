---
title: Azure HDInsight에서 아파치 하두롭과 아파치 암바리 하이브 뷰 사용
...
1. 보기 목록에서 __Hive 보기__를 선택합니다.
    ...
    ![아파치 암바리 선택 아파치 하이브 보기](./media/apache-hadoop-use-hive-ambari-view/select-apache-hive-view.png)
...
1. 쿼리를 시작하려면 워크시트 아래에서 **실행을** 선택합니다. 단추가 주황색으로 바뀌고 텍스트가 **중지**로 변경됩니다.
...
1. 쿼리가 완료된 후에 **결과** 탭에 작업 결과가 표시됩니다. 다음 텍스트는 쿼리 결과입니다.
    ...
    **LOG** 탭을 사용하여 작업이 만든 로깅 정보를 볼 수 있습니다.

   > [!TIP]  
   > **결과** 탭 아래의 **작업** 드롭다운 대화 상자에서 결과를 다운로드하거나 저장합니다.
...
쿼리에 대한 Tez UI를 표시하려면 워크시트 아래의 **Tez UI** 탭을 선택합니다.
...
![아파치 하이브 보기 작업 탭 기록](./media/apache-hadoop-use-hive-ambari-view/apache-hive-job-history.png)

## <a name="database-tables"></a>데이터베이스 테이블

__테이블__ 탭을 사용하여 Hive 데이터베이스 내의 테이블을 사용할 수 있습니다.

![아파치 하이브 테이블 탭의 이미지](./media/apache-hadoop-use-hive-ambari-view/hdinsight-tables-tab.png)

## <a name="saved-queries"></a>저장된 쿼리

**쿼리** 탭에서 선택적으로 쿼리를 저장할 수 있습니다. 쿼리를 저장한 후에 __저장된 쿼리__ 탭에서 다시 사용할 수 있습니다.

![아파치 하이브 보기 저장된 쿼리 탭](./media/apache-hadoop-use-hive-ambari-view/ambari-saved-queries.png)
