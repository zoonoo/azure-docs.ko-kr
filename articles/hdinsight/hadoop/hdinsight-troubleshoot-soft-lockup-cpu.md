...
description: watchdog 버그 소프트 작동 중지 CPU가 커널 syslog 표시 됩니다.
...
커널 syslog에는 오류 메시지가 `watchdog: BUG: soft lockup - CPU`포함 되어 있습니다.
...
1. 3 단계와 동일한 단계를 수행 하 여 헤드 노드에서 스크립트 작업을 실행 합니다. 그러나 이번에는 노드 유형을 사용 합니다. 사장.
