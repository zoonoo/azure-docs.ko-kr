...
1. **Hive** > **Configs** > **고급** > **고급 hive-log4j**로 이동 합니다. 다음 설정을 검토 합니다.
    ...
    * `hive.root.logger=DEBUG,RFA`입니다. 이 값은 기본값입니다. 로그 [수준을](https://logging.apache.org/log4j/2.x/log4j-api/apidocs/org/apache/logging/log4j/Level.html) 줄여 인쇄할 로그 항목 수를 `INFO` 합니다.

    * `log4jhive.log.maxfilesize=1024MB`입니다. 이 값은 기본값입니다. 원하는 대로 수정 합니다.

    * `log4jhive.log.maxbackupindex=10`입니다. 이 값은 기본값입니다. 원하는 대로 수정 합니다. 매개 변수가 생략 된 경우 생성 되는 로그 파일은 무한 합니다.
...
* `/mnt/resource`은 (는) 리소스 관리자를 다시 시작 하는 경우 처럼 분리 된 파일로 채워질 수 있습니다. 필요한 경우 `/mnt/resource/hadoop/yarn/log`를 수동으로 정리 하 고 `/mnt/resource/hadoop/yarn/local`합니다.
