...
Apache Yarn log는 파일 파서에서 지원 `IndexFile` 하지 않는 형식으로 집계 됩니다.
...
1. WASB 저장소의 경우:의 `yarn.log-aggregation.file-formats` 기본값은입니다 `IndexedFormat,TFile`. 값을로 `TFile`변경 합니다.

1. ADLS 저장소의 경우:의 `yarn.nodemanager.log-aggregation.compression-type` 기본값은입니다 `gz`. 값을로 `none`변경 합니다.
