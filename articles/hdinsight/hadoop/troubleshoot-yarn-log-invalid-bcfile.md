...
Apache Yarn log는 파일 파서에서 지원 하지 않는 `IndexFile` 형식으로 집계 됩니다.

## <a name="resolution"></a>해상도

1. 웹 브라우저에서 `https://CLUSTERNAME.azurehdinsight.net`로 이동 합니다. 여기서 `CLUSTERNAME`은 클러스터의 이름입니다.

1. Ambari UI에서 **YARN** > **Configs** > **고급** > **고급 YARN**로 이동 합니다.

1. WASB 저장소: `yarn.log-aggregation.file-formats`의 기본값은 `IndexedFormat,TFile`입니다. 값을 `TFile`변경 합니다.

1. ADLS 저장소: `yarn.nodemanager.log-aggregation.compression-type`의 기본값은 `gz`입니다. 값을 `none`변경 합니다.
