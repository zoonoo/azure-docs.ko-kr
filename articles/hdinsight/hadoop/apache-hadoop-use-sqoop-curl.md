---
title: Curl을 사용하여 Azure HDInsight의 Apache Sqoop으로 데이터 내보내기
description: 원격으로 Curl을 사용하여 Azure HDInsight에 Apache Sqoop 작업을 제출하는 방법에 대해 알아봅니다.
...
* Azure SQL Database를 쿼리할 클라이언트입니다. [SQL Server Management Studio](../../azure-sql/database/connect-query-ssms.md)를 사용하거나 [Visual Studio Code](../../azure-sql/database/connect-query-vscode.md)를 사용하는 것이 좋습니다.
...
Apache Sqoop 작업을 사용하여 Azure Storage에서 SQL Server로 데이터를 내보내는 데 Curl을 사용합니다.
...
이 섹션에서 명령을 사용하는 경우 `USERNAME`을 클러스터에 인증할 사용자로 바꾸고 `PASSWORD`를 사용자 계정의 암호로 바꿉니다. `CLUSTERNAME`을 클러스터의 이름으로 바꿉니다.
...
1. 다음을 사용하여 Sqoop 작업을 제출합니다.
   ...
   * **-d** - `-G`가 사용되지 않으므로 요청은 POST 메서드로 기본 설정됩니다. `-d` 는 요청과 함께 전송되는 데이터 값을 지정합니다.
     ...
     이 명령은 작업 상태 확인에 사용할 수 있는 작업 ID를 반환해야 합니다.

       ```json
       {"id":"job_1415651640909_0026"}
       ```

1. 작업 상태를 확인하려면 다음 명령을 사용합니다. `JOBID`를 이전 단계에서 반환된 값으로 바꿉니다. 예를 들어 반환 값이 `{"id":"job_1415651640909_0026"}`인 경우, `JOBID`는 `job_1415651640909_0026`입니다. 필요에 따라 `jq`의 위치를 수정합니다.
...
1. 작업 상태가 **SUCCEEDED** 로 변경되면 Azure Blob Storage에서 작업 결과를 검색할 수 있습니다. 쿼리와 함께 전달된 `statusdir` 매개 변수에는 출력 파일의 위치(이 경우 `wasb:///example/data/sqoop/curl`)가 포함됩니다. 이 주소는 HDInsight 클러스터에서 사용하는 기본 스토리지 컨테이너의 `example/data/sqoop/curl` 디렉터리에 작업의 출력을 저장합니다.

    Azure Portal을 사용하여 stderr 및 stdout Blob에 액세스할 수 있습니다.

1. 데이터를 내보냈는지 확인하려면 SQL 클라이언트에서 다음 쿼리를 사용하여 내보낸 데이터를 확인합니다.
