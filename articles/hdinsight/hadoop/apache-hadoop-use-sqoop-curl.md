...
이 섹션의 명령에 대해 `USERNAME`를 클러스터에 인증할 사용자로 바꾸고 `PASSWORD`를 사용자 계정의 암호로 바꿉니다. `CLUSTERNAME`을 클러스터의 이름으로 바꿉니다.
...
1. 작업 상태를 확인하려면 다음 명령을 사용합니다. `JOBID`를 이전 단계에서 반환된 값으로 바꿉니다. 예를 들어 반환 값이 `{"id":"job_1415651640909_0026"}`된 경우 `JOBID` `job_1415651640909_0026`됩니다. 필요에 따라 `jq`의 위치를 수정 합니다.
...
1. 작업 상태가 **SUCCEEDED**로 변경되면 Azure Blob Storage에서 작업 결과를 검색할 수 있습니다. 쿼리와 함께 전달된 `statusdir` 매개 변수에는 출력 파일의 위치(이 경우 `wasb:///example/data/sqoop/curl`)가 포함됩니다. 이 주소는 HDInsight 클러스터에서 사용 하는 기본 저장소 컨테이너의 `example/data/sqoop/curl` 디렉터리에 작업의 출력을 저장 합니다.
