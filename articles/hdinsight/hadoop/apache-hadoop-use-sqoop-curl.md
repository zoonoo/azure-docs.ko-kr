...
Azure storage의 Apache Sqoop 작업을 사용 하 여 데이터를 SQL Server로 내보내려면 말아 넘기기를 사용 합니다.
...
2. , `SQLDATABASESERVERNAME` ,를`SQLDATABASENAME`필수 구성 요소의 적절 한 값으로 바꿉니다. `PASSWORD` `USERNAME@SQLDATABASESERVERNAME` 다음을 사용하여 Sqoop 작업을 제출합니다.
...
3. 작업 상태를 확인하려면 다음 명령을 사용합니다. `JOBID`를 이전 단계에서 반환된 값으로 바꿉니다. 예를 들어 반환 값이 `{"id":"job_1415651640909_0026"}` `JOBID` `job_1415651640909_0026`인 경우는입니다.
...
4. 작업 상태가 **SUCCEEDED**로 변경되면 Azure Blob Storage에서 작업 결과를 검색할 수 있습니다. 쿼리와 함께 전달된 `statusdir` 매개 변수에는 출력 파일의 위치(이 경우 `wasb:///example/data/sqoop/curl`)가 포함됩니다. 이 주소는 HDInsight 클러스터에서 사용 하는 기본 `example/data/sqoop/curl` 저장소 컨테이너의 디렉터리에 작업의 출력을 저장 합니다.

    Azure Portal을 사용하여 stderr 및 stdout Blob에 액세스할 수 있습니다.

5. 데이터를 내보낼지 확인 하려면 SQL 클라이언트에서 다음 쿼리를 사용 하 여 내보낸 데이터를 확인 합니다.
