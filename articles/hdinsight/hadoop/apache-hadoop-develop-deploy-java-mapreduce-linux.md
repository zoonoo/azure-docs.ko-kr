...
1. 다음 명령을 입력 하 여 **wordcountjava**라는 Maven 프로젝트를 만듭니다.
   ...
   ```
    ...
    이 명령은 `artifactID` 매개 변수로 지정 된 이름으로 디렉터리를 만듭니다 (이 예제에서는**wordcountjava** ). 이 디렉터리에는 다음 항목이 포함 됩니다.
...
1. 생성 된 예제 코드를 제거 합니다. `AppTest.java`생성 된 테스트 및 응용 프로그램 파일을 삭제 하 고 아래 명령을 입력 하 여 `App.java` 합니다.
...
1. 다음 명령을 입력 하 여 `WordCount.java`새 파일을 만들고 엽니다. 프롬프트에서 **예** 를 선택 하 여 새 파일을 만듭니다.
...
1. 클러스터에 연결 합니다. `CLUSTERNAME`를 HDInsight 클러스터 이름으로 바꾸고 다음 명령을 입력 합니다.
