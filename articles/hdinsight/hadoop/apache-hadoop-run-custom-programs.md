...
1. MapReduce 프로그램을 개발, 컴파일 및 테스트 한 후 `scp` 명령을 사용 하 여 헤드 노드에 jar 파일을 업로드 합니다.

    ```cmd
    scp mycustomprogram.jar sshuser@CLUSTERNAME-ssh.azurehdinsight.net
    ```

    CLUSTERNAME을 클러스터 이름으로 바꿉니다. 암호를 사용 하 여 SSH 계정을 보호 한 경우 암호를 입력 하 라는 메시지가 표시 됩니다. 인증서를 사용하는 경우, `-i` 매개 변수를 사용하여 프라이빗 키를 지정해야 합니다.
