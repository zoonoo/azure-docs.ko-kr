---
title: '스토리지: 온-프레미스 아파치 하두롭을 Azure HDInsight로 마이그레이션'
...
HDInsight 클러스터는 기본 파일 시스템 또는 추가 파일 시스템으로 Azure Storage에서 Blob 컨테이너를 사용할 수 있습니다.표준 계층 스토리지 계정은 HDInsight 클러스터와 함께 사용하도록 지원됩니다. 프리미어 계층은 지원되지 않습니다. 기본 Blob 컨테이너는 작업 기록 및 로그와 같은 클러스터 특정 정보를 저장합니다.여러 클러스터의 기본 파일 시스템으로 하나의 Blob 컨테이너를 공유하는 것은 지원되지 않습니다.
...
Azure 저장소는 지리적으로 복제할 수 있습니다. 지역에서 복제는 지리적 복구와 데이터 중복 기능을 제공하지만, 지역에서 복제된 위치에 대한 장애 조치(failover)는 성능에 심각한 영향을 미치게 되며 추가 비용을 발생시킬 수 있습니다. 지역에서 복제 기능은 추가 비용을 감수할 만큼 가치 있는 데이터에 한해서만 현명하게 사용하는 것이 좋습니다.
...
[표준 저장소 계정에 대한 확장성 대상에는](../../storage/common/scalability-targets-standard-account.md) Azure Storage 계정의 현재 제한이 나열되어 있습니다. 애플리케이션의 요구가 단일 스토리지 계정의 확장성 목표를 초과하는 경우 여러 스토리지 계정을 사용하도록 애플리케이션을 빌드한 다음, 데이터 개체를 이러한 스토리지 계정에 분할할 수 있습니다.

[Azure Storage Analytics는](../../storage/storage-analytics.md) 모든 저장소 서비스에 대한 메트릭을 제공하며 Azure 포털은 차트를 통해 시각화할 수집 메트릭을 구성할 수 있습니다. 스토리지 리소스 메트릭의 임계값에 도달하면 사용자에게 알리도록 경고를 만들 수 있습니다.

Azure Storage는 [Blob 개체에 대해 소프트 삭제를](../../storage/blobs/storage-blob-soft-delete.md) 제공하여 응용 프로그램 또는 다른 저장소 계정 사용자가 실수로 수정하거나 삭제한 경우 데이터를 복구하는 데 도움을 줍니다.
...
- [Azure HDInsight 클러스터와 함께 Azure 저장소 사용](../hdinsight-hadoop-use-blob-storage.md)
...
Azure 데이터 레이크 스토리지 Gen2는 최신 스토리지 제품입니다. Azure Data Lake Storage의 1세대에서 핵심 기능을 Azure Blob Storage로 직접 통합된 Hadoop 호환 파일 시스템 엔드포인트와 통합합니다. 이 향상된 기능은 개체 스토리지의 크기 조정 및 비용 혜택을 일반적으로 온-프레미스 파일 시스템에만 연결된 안정성 및 성능과 결합합니다.
...
Data Lake Storage Gen2의 기본 기능은 개체/파일을 수행 데이터 액세스를 위한 디렉터리 계층 구조로 구성하는 Blob 저장소 서비스에 [계층적 네임스페이스를](../../storage/data-lake-storage/namespace.md) 추가하는 것입니다.계층 구조를 통해 디렉터리 이름 바꾸기 또는 삭제와 같은 작업은 디렉터리의 이름 접두사를 공유하는 모든 개체를 열거 및 처리하는 대신 디렉터리에 대한 단일 원자성 메타데이터 작업이 됩니다.

과거 클라우드 기반 분석은 성능, 관리 및 보안 영역을 양보해야 했습니다. ADLS(Azure Data Lake Storage) Gen2의 주요 기능은 아래와 같습니다.

- **Hadoop 호환 액세스**: Azure Data Lake Storage Gen2를 사용하면  [HDFS(Hadoop 분산 파일 시스템)](https://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-hdfs/HdfsDesign.html)를 사용하는 것처럼 데이터를 관리하고 액세스할 수 있습니다. 새 [ABFS 드라이버는](../../storage/data-lake-storage/abfs-driver.md)  [Azure HDInsight에](../index.yml)포함된 모든 아파치 하두프 환경에서 사용할 수 있습니다. 이 드라이버를 사용하면 Data Lake Storage Gen2에 저장된 데이터에 액세스할 수 있습니다.
...
- **비용 효과**: Data Lake Storage Gen2는 낮은 비용의 스토리지 용량 및 트랜잭션을 제공합니다. 데이터가 전체 수명 주기를 통해 전환됨에 따라  [Azure Blob Storage 수명 주기](../../storage/common/storage-lifecycle-management-concepts.md)와 같은 기본 제공 기능을 통해 비용을 최소화하도록 청구 요금을 변경합니다.
...
Hadoop 구성 파일에 추가되는 Azure 저장소 키는 온-프레미스 HDFS와 Azure Blob 저장소 간의 연결을 설정합니다. Hadoop 자격 증명 공급자 프레임워크로 암호화하여 이러한 키를 보호할 수 있습니다. 암호화되면 안전하게 저장하고 액세스할 수 있습니다.
...
## <a name="restrict-azure-storage-data-access-using-sas"></a>SAS를 사용하여 Azure 저장소 데이터 액세스 제한
...
6. **Key** 및 **Value** 필드에 다음 값을 사용합니다.
    ...
    **키** `fs.azure.sas.YOURCONTAINER.YOURACCOUNT.blob.core.windows.net` : **값**: 위의 4 단계에서 파이썬 응용 프로그램에서 반환된 SAS 키입니다.
...
Azure에서 SAS 토큰의 사용에 대해 기억해야 할 세 가지 중요한 사항이 있습니다.
...
2. SAS 토큰이 `READ + LIST + WRITE` 권한을 사용하여 생성되면(`DELETE`만 제한하도록) `hadoop fs -put`과 같은 명령은 먼저 `\_COPYING\_` 파일에 작성한 다음, 파일 이름을 변경하려고 합니다. 이 HDFS 작업은 WASB에 대한 `copy+delete`에 매핑합니다. 사용 `DELETE` 권한이 제공되지 않았으면 "put"이 실패합니다. `\_COPYING\_` 작업은 일부 동시성 제어를 제공하기 위한 Hadoop 기능입니다. 현재 "WRITE" 작업에도 영향을 주지 않으면서 "DELETE" 작업만 제한할 수 있는 방법은 없습니다.

3. 안타깝게도 hadoop 자격 증명 공급자 및 암호 해독 키 공급자(ShellDecryptionKeyProvider)는 현재 SAS 토큰으로 작동하지 않으므로 현재 가시성으로부터 보호할 수 없습니다.
...
Azure Storage에 쓴 모든 데이터는  [SSE(스토리지 서비스 암호화)](../../storage/common/storage-service-encryption.md)를 사용하여 자동으로 암호화됩니다. Azure Storage 계정의 데이터는 항상 고가용성을 위해 복제됩니다.스토리지 계정을 만들 때 다음 복제 옵션 중 하나를 선택할 수 있습니다.

- [LRS(로컬 중복 스토리지)](../../storage/common/storage-redundancy-lrs.md)
- [영역 중복 스토리지(ZRS)](../../storage/common/storage-redundancy-zrs.md)
- [GRS(지역 중복 스토리지)](../../storage/common/storage-redundancy-grs.md)
- [읽기 액세스 지리적 중복 스토리지(RA-GRS)](../../storage/common/storage-redundancy.md)

Azure Data Lake Storage는 LRS(로컬 중복 스토리지)를 제공하지만 재해 복구 계획의 요구에 맞게 정렬된 주파수를 사용하여 다른 지역의 다른 Data Lake Storage 계정에 중요한 데이터를 복사해야 합니다. [ADLCopy,](../../data-lake-store/data-lake-store-copy-data-azure-storage-blob.md) [DistCp,](https://hadoop.apache.org/docs/current/hadoop-distcp/DistCp.html) [Azure PowerShell](../../data-lake-store/data-lake-store-get-started-powershell.md)또는 Azure 데이터 [팩터리](../../data-factory/connector-azure-data-lake-store.md)를 비롯한 여러 가지 방법으로 데이터를 복사할 수 있습니다.또한 실수로 삭제되지 않도록 Data Lake Storage 계정에 대한 액세스 정책을 적용하는 것이 좋습니다.
...
## <a name="attach-additional-azure-storage-accounts-to-cluster"></a>클러스터에 추가 Azure 저장소 계정 연결

HDInsight 생성 프로세스 중에 Azure 저장소 계정 또는 Azure Data Lake 저장소 계정이 기본 파일 시스템으로 선택됩니다. 클러스터 만들기 프로세스 중이나 클러스터를 만든 후에 이 기본 스토리지 계정 외에도 동일한 Azure 구독 또는 다른 Azure 구독에서 스토리지 계정을 추가할 수 있습니다.
...
이 시리즈의 다음 문서 읽기: [Azure HDInsight Hadoop 마이그레이션에 대한 온-프레미스에 대한 데이터 마이그레이션 모범 사례.](apache-hadoop-on-premises-migration-best-practices-data-migration.md)
