...
Excel의 Microsoft 파워 쿼리 추가 기능을 사용 하 여 HDInsight 클러스터와 연결 된 데이터를 Excel에서 연결할 수 있습니다. 자세한 내용은 [파워 쿼리를 사용 하 여 HDInsight에 Excel 연결](../hdinsight-connect-excel-power-query.md)을 참조 하세요.
...
1. 다음 값을 입력하거나 선택합니다.
   ...
   |  호스트 |`HDInsightClusterName.azurehdinsight.net`를 입력합니다. `myHDICluster.azurehdinsight.net`)을 입력합니다. 참고: `HDInsightClusterName-int.azurehdinsight.net` 는 클라이언트 VM이 동일한 가상 네트워크에 피어 링 지만 지원 됩니다. |
