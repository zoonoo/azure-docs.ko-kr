...
2. 형식 `https://CLUSTERNAME.azurehdinsight.net`으로 **연결 Url** 을 입력 합니다. **클러스터 이름은** 다른 필드로 이동할 때 URL의 클러스터 이름 부분으로 자동으로 채워집니다. 그런 다음 **사용자 이름** 및 **암호**를 입력 하 고 **다음**을 선택 합니다.
...
* 데이터를 보려면 `hivesampletable` 다음을 수행 합니다.
