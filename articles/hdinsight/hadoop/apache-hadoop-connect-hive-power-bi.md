...
2. 상단 메뉴에서 **홈** > **데이터** > 가져오기**자세히**로 이동 합니다.

    ![HDInsight Excel Power BI 열 데이터](./media/apache-hadoop-connect-hive-power-bi/hdinsight-power-bi-open-odbc.png)

3. **데이터 가져오기** 대화 상자의 왼쪽에서 **기타** 를 선택 하 고 오른쪽에서 **ODBC** 를 선택한 다음 아래쪽에서 **연결** 을 선택 합니다.

4. **ODBC에서** 대화 상자의 드롭다운 목록에서 마지막 섹션에서 만든 데이터 원본 이름을 선택 하 고 **확인**을 선택 합니다.

5. **탐색기** 대화 상자에서 **ODBC > HIVE > 기본값**을 확장 하 고 **hivesampletable**를 선택한 다음 **로드**를 선택 합니다.

6. **ODBC 드라이버** 대화 상자에서 **기본 또는 사용자 지정**을 선택한 다음 **연결**을 선택 합니다.
...
2. **필드** 창에서 **country** and **devicemake**를 선택 합니다. 맵에 표시된 데이터를 볼 수 있습니다.
3. 맵을 확장합니다.
