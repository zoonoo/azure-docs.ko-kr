...
* Windows 10, 7, Windows Server 2008 R2 또는 이후 운영 체제를 실행하는 워크스테이션입니다.
* Office 2016, Office 2013 Professional Plus, Office 365 ProPlus, Excel 2013 Standalone 또는 Office 2010 Professional Plus.

## <a name="install-microsoft-power-query"></a>마이크로소프트 전원 쿼리 설치
...
1. Excel 버전에 따라 다음 단계를 수행합니다.
   ...
   * Excel 2016
     ...
     * **Azure HDInsight(HDFS)에서****데이터를** >  **Data** > **가져옵니다>** > 선택합니다.

       ![Hdi. 파워 쿼리.셀렉티소스.2016](./media/apache-hadoop-connect-excel-power-query/powerquery-selecthdisource-excel2016.png)

   * Excel 2013/2010

     * **마이크로소프트 Azure HDInsight에서** > **Azure에서** >  **전원 쿼리를**선택합니다.

       ![HDI.PowerQuery.SelectHdiSource](./media/apache-hadoop-connect-excel-power-query/powerquery-selecthdisource.png)

       **참고:** **전원 쿼리** 메뉴가 표시되지 않으면 **파일** > **옵션** > **추가**기능으로 이동하여 페이지 하단의 드롭다운 **관리** 상자에서 COM **추가 기능을** 선택합니다. **이동...** 단추를 선택하고 Excel용 파워 쿼리 추가 기능에 해당하는 상자가 선택되어 있는지 확인합니다.

       **참고:** 또한 전원 쿼리를 사용하면 **다른 소스에서**를 선택하여 HDFS에서 데이터를 가져올 수 있습니다.

1. Azure **HDInsight(HDFS)** 대화 상자에서 **계정 이름 또는 URL** 텍스트 상자에 클러스터와 연결된 Azure Blob 저장소 계정의 이름을 입력합니다. 그런 다음 **확인을**선택합니다. 이 계정은 기본 스토리지 계정 또는 연결된 스토리지 계정이 될 수 있습니다.  형식은 `https://StorageAccountName.blob.core.windows.net/`입니다.

1. **계정 키의**경우 Blob 저장소 계정의 키를 입력한 다음 **연결을**선택합니다. (이 저장소에 처음 액세스할 때만 계정 정보를 입력하면 됩니다.)

1. 쿼리 편집기 왼쪽의 **Navigator** 창에서 클러스터와 연결된 Blob 저장소 컨테이너 이름을 두 번 클릭합니다. 기본적으로 컨테이너 이름은 클러스터 이름과 같습니다.

1. **이름** 열에서 **HiveSampleData.txt를** 찾습니다(폴더 경로는 **.입니다. /hive/warehouse/hivesampletable/**) 다음 HiveSampleData.txt의 왼쪽에 있는 **바이너리를** 선택합니다. HiveSampleData.txt는 모든 클러스터와 함께 제공됩니다. 필요에 따라 사용자의 파일을 사용할 수 있습니다.

    ![HDI 엑셀 전원 쿼리 가져오기 데이터](./media/apache-hadoop-connect-excel-power-query/powerquery-importdata.png)

1. 원하는 경우 열 이름을 바꿀 수 있습니다. 준비가 되면 **로드 & 닫기**선택합니다.  통합 문서에 데이터가 로드됩니다.

    ![HDI 엑셀 전원 쿼리 가져온 테이블](./media/apache-hadoop-connect-excel-power-query/powerquery-importedtable.png)

## <a name="next-steps"></a>다음 단계

이 문서에서는 파워 쿼리를 사용하여 HDInsight에서 Excel로 데이터를 가져오는 방법을 알아보았습니다. 마찬가지로 HDInsight에서 Azure SQL Database로 데이터를 가져올 수 있습니다. HDInsight에 데이터를 업로드할 수도 있습니다. 자세히 알아보려면 다음 아티클을 참조하세요.
