...
Resource Health 센터 (RHC) 경고 (`The HDInsight cluster is unable to access the key for BYOK encryption at rest`)는 클러스터 노드가 고객 Key Vault (KV)에 대 한 액세스 권한을 상실 한 BYOK (Bring Your Own Key) 클러스터에 대해 표시 됩니다. Apache Ambari UI에도 유사한 경고가 표시 될 수 있습니다.
...
자세한 내용은 [Azure Key Vault 가용성 및 중복성](../../key-vault/key-vault-disaster-recovery-guidance.md) 및 Azure 상태 페이지를 참조 하세요 https://status.azure.com/
