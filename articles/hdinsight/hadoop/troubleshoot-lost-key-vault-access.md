...
Resource Health 센터 (RHC) 경고 `The HDInsight cluster is unable to access the key for BYOK encryption at rest`는 클러스터 노드가 KV (고객 Key Vault)에 대 Bring Your Own Key 한 액세스를 잃은 (byok) 클러스터에 대해 표시 됩니다. Apache Ambari UI에도 유사한 경고가 표시 될 수 있습니다.
