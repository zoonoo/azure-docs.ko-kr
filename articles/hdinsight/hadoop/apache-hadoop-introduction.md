...
* **[도메인에 연결된 클러스터 미리 보기](../domain-joined/apache-domain-joined-introduction.md)**: Active Directory 도메인에 연결된 클러스터를 사용하여 액세스를 제어하고 데이터에 대한 거버넌스를 제공할 수 있습니다.
...
<a  target="_blank" href="http://zookeeper.apache.org/">Apache ZooKeeper</a>는 데이터 레지스터(znode)의 공유 계층 구조 네임스페이스를 사용하여 대규모 분산 시스템의 프로세스를 조정합니다. Znode에는 상태, 위치, 구성 등 프로세스를 조정하기 위해 필요한 소량의 메타 정보가 들어 있습니다. [HBase 클러스터 및 Apache Phoenix가 있는 ZooKeeper](../hbase/apache-hbase-phoenix-squirrel-linux.md)의 예제를 참조하세요. 
...
* [HDInsight에서 Hadoop 시작](apache-hadoop-linux-tutorial-get-started.md): HDInsight Hadoop 클러스터를 프로비전하고 샘플 Hive 쿼리를 실행하는 방법에 대한 빠른 시작 자습서입니다.
* [HDInsight에서 Spark 시작](../spark/apache-spark-jupyter-spark-sql.md): Spark 클러스터를 만들고 대화형 Spark SQL 쿼리를 실행하는 방법에 대한 빠른 시작 자습서입니다.
* [HDInsight에서 R Server 사용](../r-server/r-server-get-started.md): HDInsight Premium에서 R Server를 사용하기 시작합니다.
