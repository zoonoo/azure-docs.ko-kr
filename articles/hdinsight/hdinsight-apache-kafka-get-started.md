...
* **SSH 및 SCP 사용 경험**. HDInsight에서 SSH와 SCP를 사용하는 방법에 대한 자세한 내용은 다음 문서를 참조하세요.
...
2. **새 HDInsight 클러스터** 블레이드에서 **클러스터 이름**을 입력하고 이 클러스터에서 사용할 **구독**을 선택합니다.
   
    ![구독 선택](./media/hdinsight-apache-kafka-get-started/new-hdinsight-cluster-blade.png)

3. **클러스터 유형 선택**을 사용하고 **클러스터 유형 구성** 블레이드에서 다음 값을 선택합니다.
...
4. **자격 증명**를 사용하여 클러스터 로그인 및 SSH 사용자 자격 증명을 구성합니다.  설정을 저장하려면 **선택** 단추를 사용합니다.
    ...
    > [!NOTE]
    > 클러스터 로그인은 HTTPS를 사용하여 인터넷을 통해 클러스터에 액세스 할 때 사용됩니다. SSH 사용자는 클러스터에 연결하고 대화형으로 명령을 실행하는 데 사용됩니다.
   
    ![클러스터 로그인 구성](./media/hdinsight-apache-kafka-get-started/cluster-credentials.png)
   
    HDInsight에서 SSH를 사용하는 방법에 대한 자세한 내용은 다음 문서를 참조하세요.
   
    * [Linux, Unix 및 MacOS 클라이언트에서 HDInsight와 함께 SSH 사용](hdinsight-hadoop-linux-use-ssh-unix.md)
   
    * [Windows 클라이언트에서 HDInsight와 함께 SSH 사용](hdinsight-hadoop-linux-use-ssh-windows.md)

5. **데이터 원본**을 사용하여 클러스터의 기본 데이터 저장소를 구성합니다. **데이터 원본** 블레이드에서 다음 정보를 사용하여 클러스터에 대한 데이터 저장소를 만듭니다.
   
    * **새로 만들기**를 선택한 다음 저장소 계정의 이름을 입력합니다.
    
    * **위치**를 선택한 다음 지리적으로 가까운 위치를 선택합니다. 이 위치는 저장소 계정과 HDInsight 클러스터를 만드는 데 사용됩니다.
     
   마지막으로 **선택** 단추를 사용하여 이러한 설정을 저장합니다.
     
    ![저장소 구성](./media/hdinsight-apache-kafka-get-started/configure-storage.png)

6. **가격 책정**을 사용한 다음 **작업자 노드 수**를 2로 설정합니다. 2개 작업자 노드를 사용하면 클러스터 비용을 줄일 수 있으며 이 예제에서 충분합니다. 설정을 저장하려면 **선택** 단추를 사용합니다.
   
    ![가격](./media/hdinsight-apache-kafka-get-started/pricing.png)
   
    > [!NOTE]
    > 포털에 표시된 가격은 스크린샷의 가격과 다를 수 있습니다.

7. **리소스 그룹**을 사용하여 그룹을 만들고 필드에서 이름을 입력합니다. 또한 **대시보드에 고정**을 선택합니다. 완료되면 **만들기**를 선택하여 클러스터를 만듭니다.
   
    ![리소스 그룹 필드](./media/hdinsight-apache-kafka-get-started/resource-group.png)
...
##<a name="a-idgetkafkainfoaget-the-zookeeper-and-broker-host-information"></a><a id="getkafkainfo"></a>Zookeeper 및 Broker 호스트 정보 가져오기
...
2. 다음 명령을 사용하여 Ambari에서 검색한 정보로 환경 변수를 설정합니다. __KAFKANAME__은 Kafka 클러스터의 이름으로 바꿉니다. __PASSWORD__는 클러스터를 만들 때 사용한 로그인(관리자) 암호로 바꿉니다.
    ...
    > 
    > Zookeeper와 broker 호스트 정보는 항상 올바른 정보를 얻기 위해 사용하기 직전에 검색해야 합니다.
...
1. [https://github.com/Azure-Samples/hdinsight-kafka-java-get-started](https://github.com/Azure-Samples/hdinsight-kafka-java-get-started)에서 예제를 다운로드합니다. 생산자/소비자 예제의 경우 `Producer-Consumer` 디렉터리의 프로젝트를 사용합니다. 이 예제의 작동 방식을 이해하려면 코드를 살펴봅니다. 여기에는 다음과 같은 클래스가 포함되어 있습니다.
   
    * **Run** - 명령줄 인수에 기반하여 소비자 또는 생산자를 시작합니다.
...
2. 개발 환경의 명령줄에서 예제의 `Producer-Consumer` 디렉터리 위치로 디렉터리를 변경한 후 다음 명령을 사용하여 jar 패키지를 만듭니다.
    ...
    이 명령은 `kafka-producer-consumer-1.0-SNAPSHOT.jar`라는 파일이 포함된 `target`이라는 새 디렉터리를 만듭니다.
...
4. `scp` 명령으로 파일 복사를 완료하면 SSH를 사용하여 클러스터에 연결한 후 다음을 사용하여 이전에 만든 test 토픽에 레코드를 기록합니다.
    ...
    그러면 생산자가 시작되고 레코드가 기록됩니다. 기록된 레코드 수를 확인할 수 있도록 카운터가 표시됩니다.
...
Kafka의 중요한 개념은 소비자에서 레코드를 읽을 때 그룹 ID로 정의된 소비자 그룹을 사용한다는 것입니다. 동일한 그룹 부하 분산을 사용하는 여러 소비자가 토픽에서 읽습니다. 각 소비자는 레코드 일부를 받게 됩니다. 이 동작을 확인하려면 다음 단계를 사용합니다.
...
동일한 그룹 내에서 클라이언트에 의한 소비는 토픽에 대한 파티션을 통해 처리됩니다. 앞에서 만든 `test` 토픽에는 8개 파티션이 있습니다. 8개 SSH 세션을 열고 모든 세션에서 소비자를 시작하면 각 소비자는 해당 토픽에 대한 단일 파티션에서 레코드를 읽습니다.
...
1. 아직 그렇게 하지 않았으면 [https://github.com/Azure-Samples/hdinsight-kafka-java-get-started](https://github.com/Azure-Samples/hdinsight-kafka-java-get-started)에서 예제를 다운로드합니다. 스트리밍 예제의 경우 `streaming` 디렉터리의 프로젝트를 사용합니다. 이 예제의 작동 방식을 이해하려면 코드를 살펴봅니다. 
...
2. 개발 환경의 명령줄에서 `Streaming` 디렉터리 위치로 디렉터리를 변경한 후 다음 명령을 사용하여 jar 패키지를 만듭니다.
    ...
    이 명령은 `kafka-streaming-1.0-SNAPSHOT.jar`라는 파일이 포함된 `target`이라는 새 디렉터리를 만듭니다.
...
4. `scp` 명령으로 파일 복사를 완료하면 SSH를 사용하여 클러스터에 연결한 후 다음을 사용하여 스트리밍 프로세스를 시작합니다.
   
        ./kafka-streaming.jar $KAFKABROKERS $KAFKAZKHOSTS 2>/dev/null &
   
    이 명령은 백그라운드에서 스트리밍 프로세스를 시작합니다.

5. 다음을 사용하여 `test` 토픽으로 메시지를 보냅니다. 스트리밍 예제에서 처리하는 방식은 다음과 같습니다.
   
        ./kafka-producer-consumer.jar producer $KAFKABROKERS &>/dev/null &

6. 다음을 사용하여 `wordcounts` 토픽에 기록된 출력을 봅니다.
   
        /usr/hdp/current/kafka-broker/bin/kafka-console-consumer.sh --zookeeper $KAFKAZKHOSTS --topic wordcounts --from-beginning --formatter kafka.tools.DefaultMessageFormatter --property print.key=true --property key.deserializer=org.apache.kafka.common.serialization.StringDeserializer --property value.deserializer=org.apache.kafka.common.serialization.LongDeserializer
   
    > [!NOTE]
    > 키와 값에 사용할 key(word 값 포함) 및 deserializer를 출력하도록 소비자에 지시하여 데이터를 볼 수 있도록 해야 합니다.
