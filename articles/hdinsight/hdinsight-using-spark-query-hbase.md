...
* 클러스터 기본 스토리지에 대한 URI 체계입니다. 이 체계는 Azure Data Lake Storage Gen1에 대 한 Azure Data Lake Storage Gen2 `abfs://` 또는 adl://에 대 한 Azure Blob Storage wasb://됩니다. Blob Storage에 대해 보안 전송이 사용 되는 경우 URI는 `wasbs://`입니다.  [보안 전송](../storage/common/storage-require-secure-transfer.md)도 참조하세요.
...
1. SSH를 사용하여 Spark 클러스터의 헤드 노드에 연결합니다. 을 Spark 클러스터의 이름으로 `SPARKCLUSTER` 바꾸고 아래 명령을 편집 하 고 명령을 입력 합니다.
