---
title: "스크립트 동작을 사용하여 Hadoop 클러스터에 Spark 설치 | Microsoft 문서"
...
## <a name="install-spark-using-the-azure-portal"></a>Azure 포털을 사용하여 Spark 설치
...
2. 아래와 같이 마법사의 **스크립트 동작** 페이지에서 **스크립트 동작 추가**를 클릭하여 스크립트 동작에 대한 세부 정보를 제공합니다.
    ...
    <table border='1'>
        ...
        <tr><td>스크립트 URI</td>
            <td>클러스터를 사용자 지정하기 위해 호출되는 스크립트에 URI(Uniform Resource Identifier)를 지정합니다. 예: <i>https://hdiconfigactions.blob.core.windows.net/sparkconfigactionv03/spark-installer-v03.ps1</i></td></tr>
        <tr><td>노드 유형</td>
...
## <a name="use-spark-in-hdinsight"></a>HDInsight에서 Spark 사용
...
### <a name="a-namesparkshellause-the-spark-shell-to-run-interactive-queries"></a><a name="sparkshell"></a>Spark 셸을 사용하여 대화형 쿼리 실행
...
1. Azure 포털에서 Spark를 설치하여 만든 클러스터에 대해 원격 데스크톱을 사용하도록 설정한 다음 클러스터에 원격으로 연결합니다. 지침은 [RDP를 사용하여 HDInsight 클러스터에 연결](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp)을 참조하세요.
2. RDP(원격 데스크톱 프로토콜) 세션의 바탕 화면에서(바탕 화면 바로 가기에서) Hadoop 명령줄을 열고 Spark가 설치된 위치(예: **C:\apps\dist\spark-1.2.0**)로 이동합니다.
3. 다음 명령을 실행하여 Spark 셸을 시작합니다.
    ...
    명령 실행이 완료되면 Scala 프롬프트가 표시됩니다.
         ...
         scala>
4. Scala 프롬프트에 아래에 표시된 Spark 쿼리를 입력합니다. 이 쿼리는 클러스터와 연결된 Azure Blob 저장소의 /example/data/gutenberg/ 위치에서 사용할 수 있는 davinci.txt 파일에 나오는 각 단어의 수를 계산합니다.
...
6. :q 입력하여 Scala 프롬프트를 종료합니다.

        :q

### <a name="a-namesparksqlause-the-spark-shell-to-run-spark-sql-queries"></a><a name="sparksql"></a>Spark 셸을 사용하여 Spark SQL 쿼리 실행
...
> [!NOTE]
> 아래 샘플은 HDInsight 3.2 클러스터를 만드는 동안 스크립트 작업을 실행하는 경우에 설치되는 **Spark 1.2.0**에 맞게 만들어졌습니다.
>
>

1. Azure 포털에서 Spark를 설치하여 만든 클러스터에 대해 원격 데스크톱을 사용하도록 설정한 다음 클러스터에 원격으로 연결합니다. 지침은 [RDP를 사용하여 HDInsight 클러스터에 연결](hdinsight-administer-use-management-portal.md#connect-to-clusters-using-rdp)을 참조하세요.
2. RDP 세션의 바탕 화면에서(바탕 화면 바로 가기에서) Hadoop 명령줄을 열고 Spark가 설치된 위치(예: **C:\apps\dist\spark-1.2.0**)로 이동합니다.
3. 다음 명령을 실행하여 Spark 셸을 시작합니다.
...
### <a name="a-namestandaloneause-a-standalone-scala-program"></a><a name="standalone"></a>독립 실행형 Scala 프로그램 사용
...
#### <a name="write-a-scala-program"></a>Scala 프로그램 작성
...
1. 파일을 **SimpleApp.scala**라는 이름으로 저장합니다.

#### <a name="build-the-scala-program"></a>Scala 프로그램 빌드
...
1. Spark가 설치된 클러스터에 원격으로 연결합니다. SimpleApp.scala 프로그램을 만들고 빌드한 컴퓨터에서 **SimpleScalaApp\target** 폴더를 복사하여 클러스터의 위치에 붙여넣습니다.
...
## <a name="install-spark-using-powershell"></a>PowerShell을 사용하여 Spark 설치
[스크립트 동작을 사용하여 HDInsight 클러스터 사용자 지정](hdinsight-hadoop-customize-cluster.md#call-scripts-using-azure-powershell)을 참조하세요.

## <a name="install-spark-using-net-sdk"></a>.NET SDK을 사용하여 Spark 설치
[스크립트 동작을 사용하여 HDInsight 클러스터 사용자 지정](hdinsight-hadoop-customize-cluster.md#call-scripts-using-azure-powershell)을 참조하세요.
