---
title: "Linux 기반 HDInsight에서 GNU R 설치 | Microsoft Docs"
...
---
# <a name="install-and-use-gnu-r-on-linux-based-hdinsight-hadoop-clusters"></a>Linux 기반 HDInsight Hadoop 클러스터에서 GNU R 설치 및 사용

**스크립트 동작** 클러스터 사용자 지정을 사용하여 HDInsight의 Hadoop에서 모든 유형의 클러스터에 GNU R을 설치할 수 있습니다. 데이터 과학자와 분석가는 HDInsight에 배포된 Hadoop 클러스터에서 대량의 데이터 처리에 R을 사용할 수 있습니다.

> [!WARNING]
> 이 문서에서는 사용자 지정 작업으로 HDInsight에 GNU R을 설치하는 지침을 제공합니다. __HDInsight의 Microsoft R Server__와 동일한 기능을 제공하지 __않습니다.__ 이 문서의 단계 대신 HDInsight에 R Server를 사용하는 것이 좋습니다.
>
> HDInsight에서 R Server를 사용하는 방법에 대한 자세한 내용은 [HDInsight에서 R Server 사용 시작](hdinsight-hadoop-r-server-get-started.md)을 참조하세요. 
...
## <a name="install-r-using-script-actions"></a>스크립트 동작을 사용하여 R 설치
...
이 섹션에서는 Azure Portal을 사용하여 클러스터를 만들 때 스크립트를 사용하는 방법에 대한 지침을 제공합니다. 
...
1. [Linux 기반 HDInsight 클러스터 프로비전](hdinsight-hadoop-provision-linux-clusters.md)의 단계를 사용하여 클러스터 프로비전을 시작하지만 완료하지는 않도록 합니다.

2. **선택적 구성** 블레이드에서 **스크립트 동작**을 선택하고 다음 정보를 사용하여 양식을 완성합니다.
...
4. [Linux 기반 HDInsight 클러스터 프로비전](hdinsight-hadoop-provision-linux-clusters.md)에서 설명한 대로 클러스터를 계속 프로비전합니다.
...
1. SSH를 사용하여 HDInsight 클러스터에 연결합니다.
   ...
   * [Windows 10, Linux, Unix 또는 OS X의 Bash의 HDInsight에서 Linux 기반 Hadoop과 SSH 사용](hdinsight-hadoop-linux-use-ssh-unix.md)
...
3. 다음 R 프로그램을 입력합니다. 이 코드는 1에서 100 사이의 숫자를 생성한 후 2로 곱합니다.
...
4. 이제 다음을 사용하여 MapReduce 출력에 저장된 임시 경로를 확인합니다.
    ...
    이 명령은 `/tmp/file5f615d870ad2`와 비슷한 값을 반환합니다. 실제 출력을 보려면 다음 명령을 사용합니다.
