---
title: '자습서: 판매 인사이트를 얻기 위한 엔드투엔드 ETL 파이프라인 만들기'
...
1. 프로젝트의 리소스 그룹을 설정합니다.
   1. 리소스 그룹에 고유한 이름을 선택합니다.
   1. Cloud Shell에서 다음 코드 조각을 실행하여 이후 단계에서 사용할 변수를 설정합니다.
...
1. Cloud Shell에서 다음 명령을 입력하여 [HDInsight 판매 인사이트 ETL 리포지토리](https://github.com/Azure-Samples/hdinsight-sales-insights-etl)에서 이 자습서에 사용할 데이터와 스크립트를 다운로드합니다.
...
1. `./scripts/resources.sh <RESOURCE_GROUP_NAME> <LOCATION>` 명령을 사용하여 Azure에서 다음 리소스를 배포하는 스크립트를 실행합니다.
   ...
   1. Azure Blob 스토리지 계정. 이 계정은 회사 판매 데이터를 보관합니다.
   ...
   6. NSG(네트워크 보안 그룹) 규칙에서 지원하는 Azure 가상 네트워크. 이 가상 네트워크를 사용하면 클러스터에서 통신하고 해당 통신을 보호할 수 있습니다. 

클러스터를 만드는 데 약 20분 정도 걸릴 수 있습니다.

`resources.sh` 스크립트에는 다음 명령이 포함되어 있습니다. 이 명령은 Azure Resource Manager 템플릿(`resourcestemplate.json`)을 사용하여 지정된 리소스를 원하는 구성으로 만듭니다.
...
1. 판매 파일을 저장하기 위해 만든 새 Azure 스토리지 계정을 찾습니다. 스토리지 계정의 이름은 `blob`으로 시작하고, 임의의 문자열을 포함합니다. 다음을 수행합니다.
   1. 나중에 사용할 수 있도록 스토리지 계정 이름을 적어 둡니다.
   1. Blob 스토리지 계정의 이름을 선택합니다.
   1. 포털 왼쪽의 **설정** 아래에서 **액세스 키**를 선택합니다.
   1. **Key1** 상자에서 문자열을 복사하여 나중에 사용할 수 있도록 저장합니다.
1. HDInsight 클러스터용 스토리지로 만든 Data Lake Storage Gen2 계정을 찾습니다. 이 계정은 Blob 스토리지 계정과 동일한 리소스 그룹에 있지만 `adlsgen2`로 시작합니다. 다음을 수행합니다.
   1. Data Lake Storage Gen2 계정의 이름을 적어 둡니다.
   1. Data Lake Storage Gen2 계정의 이름을 선택합니다.
   1. 포털 왼쪽의 **설정** 아래에서 **액세스 키**를 선택합니다.
   1. **Key1** 상자에서 문자열을 복사하여 나중에 사용할 수 있도록 저장합니다.

> [!Note]
> 스토리지 계정의 이름을 알고 있으면 Azure Cloud Shell 프롬프트에서 다음 명령을 사용하여 계정 키를 가져올 수 있습니다.
> ```azurecli-interactive
...
Azure Data Factory는 Azure 파이프라인을 자동화하는 데 도움이 되는 도구입니다. 이러한 작업을 수행할 수 있는 유일한 방법은 아니지만 프로세스를 자동화하는 좋은 방법입니다. Azure Data Factory에 대한 자세한 내용은 [Azure Data Factory 설명서](https://azure.microsoft.com/en-us/services/data-factory/)를 참조하세요. 
...
1. `chmod +x adf.sh`를 사용하여 파일에 대한 실행 권한을 추가합니다.
1. `./adf.sh`를 사용하여 스크립트를 실행합니다. 
...
- 다음 명령을 실행하여 PowerShell에서 Data Factory 파이프라인을 트리거합니다. 
...
2. 다음 명령을 사용하여 SSH를 통해 LLAP 클러스터에 액세스한 다음, 암호를 입력합니다. `resourcesparameters.json` 파일을 변경하지 않은 경우 암호는 `Thisisapassword1`입니다.
...
1. 클러스터에 대한 URI를 여기에 붙여넣습니다. `https://<LLAP CLUSTER NAME>.azurehdinsight.net` 형식이어야 합니다.
