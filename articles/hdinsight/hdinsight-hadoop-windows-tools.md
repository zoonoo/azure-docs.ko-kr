...
* [PowerShell을 사용하여 클러스터를 만듭니다.](hdinsight-hadoop-create-linux-clusters-azure-powershell.md)
* [PowerShell을 사용하여 아파치 하이브 쿼리를 실행합니다.](hadoop/apache-hadoop-use-hive-powershell.md)
* [PowerShell을 통해 클러스터를 관리합니다.](hdinsight-administer-use-powershell.md)
...
* **[Azure Cloud Shell은](https://docs.microsoft.com/azure/cloud-shell/overview)** 브라우저와 Azure 포털 내에서 실행되는 대화형 명령줄 셸입니다.
...
* [.NET에 대한 Azure HDInsight SDK](https://docs.microsoft.com/dotnet/api/overview/azure/hdinsight?view=azure-dotnet).
* [.NET SDK를 사용하여 아파치 하이브 쿼리를 실행합니다.](hadoop/apache-hadoop-use-hive-dotnet-sdk.md)
* [아파치 하두프에서 아파치 하이브와 아파치 돼지 스트리밍C # 사용자 정의 기능을 사용합니다.](hadoop/apache-hadoop-hive-pig-udf-dotnet-csharp.md)
...
다음 문서에 방법이 나와 있습니다.
* Intellij IDEA: [Intellij 플러그인 및 스칼라 SDK용 Azure 도구 키트를 사용하여 아파치 스파크 응용 프로그램을 만듭니다.](spark/apache-spark-intellij-tool-plugin.md)
* 이클립스 IDE 또는 이클립스용 스칼라 IDE: [아파치 스파크 응용 프로그램 만들기 및 이클립스용 Azure 도구 키트](spark/apache-spark-eclipse-tool-plugin.md) 만들기
...
Linux에서만 사용할 수 있는 도구 나 기술을 사용해야 하는 상황이 발생하면 다음 옵션을 고려하십시오.

* **윈도우에 우분투에 강타 10** 윈도우에서 리눅스 하위 시스템을 제공합니다. Bash를 사용하면 전용 Linux 설치를 유지하지 않고도 Linux 유틸리티를 직접 실행할 수 있습니다. [Windows 10을 위한 Linux용 Windows 하위 시스템 설치 가이드](https://docs.microsoft.com/windows/wsl/install-win10)에서 설치 단계를 참조하세요.  다른 [Unix 셸](https://www.gnu.org/software/bash/)도 작동합니다.
...
Azure CLI(명령줄 인터페이스)는 Azure 리소스를 관리하기 위한 Microsoft의 플랫폼 간 명령줄 환경입니다.  자세한 내용은 [CLI(Azure 명령줄 인터페이스)를](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest)참조하십시오.
