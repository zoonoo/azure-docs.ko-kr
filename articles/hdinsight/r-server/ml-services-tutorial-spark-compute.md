...
description: 자습서 - ML Services에서 R 및 Spark 시작
...
## <a name="download-sample-data"></a>샘플 데이터를 다운로드

*Airline 2012 On-Time Data Set*는 미국 내 모든 민간 항공사의 2012년 항공편 도착 및 출발 세부 정보가 들어 있는 12개의 쉼표로 구분된 파일로 구성되어 있습니다. 6백만 건이 넘는 관찰 기록이 들어 있는 빅 데이터 세트입니다.
...
1. 몇 가지 환경 변수를 초기화합니다. RStudio Server 콘솔에서 다음 코드를 입력합니다.
    ...
    화면 오른쪽의 **환경** 탭에 변수가 나타납니다.

    ![RStudio](./media/ml-services-tutorial-spark-compute/rstudio.png)

2.  로컬 디렉터리를 만들고 샘플 데이터를 다운로드합니다. RStudio에서 다음 코드를 입력합니다.
...
## <a name="copy-data-to-default-storage"></a>기본 스토리지에 데이터 복사

HDFS 위치는 `airDataDir` 변수에서 지정합니다. RStudio에서 다음 코드를 입력합니다.
...
## <a name="set-up-data-set"></a>데이터 세트 설정
...
2. 원본 CSV 파일의 변수 이름은 다루기 불편하기 때문에, 보다 편하게 관리할 수 있도록 `colInfo` 목록을 제공해 드립니다. RStudio에서 다음 코드를 입력합니다.
...
## <a name="create-compute-context-for-spark"></a>Spark에 대한 컴퓨팅 컨텍스트 만들기
...
1. 결과를 확인합니다. RStudio에서 다음 코드를 입력합니다.
    ...
    결과를 보면 모든 데이터를 처리했고, 6백만 건의 관찰 기록이 있고, 모두 지정된 디렉터리에 있는 .csv 파일을 사용합니다. 또한 `cube = TRUE`로 지정했기 때문에 각 요일의 예상 계수가 있습니다(절편은 없음).

## <a name="use-composite-xdf-files"></a>복합 XDF 파일 사용

살펴본 것처럼, Hadoop에서 R을 사용하여 직접 CSV 파일을 분석할 수 있지만, 데이터를 보다 효율적인 형식으로 저장하면 더 빠르게 분석할 수 있습니다. R .xdf 형식은 매우 효율적이지만, 개별 파일을 단일 HDFS 블록 내에 유지하기 위해 HDFS에 맞게 약간 수정됩니다. (HDFS 블록 크기는 설치 방법에 따라 다르지만, 일반적으로 64MB 또는 128MB입니다.) Hadoop에서 [rxImport](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rximport)를 사용하는 경우 `AirDS` 같은 `RxTextData` 데이터 원본을 inData로 지정하고, fileSystem이 HDFS 파일 시스템으로 설정된 `RxXdfData` 데이터 원본을 outFile 인수로 지정하여 복합 .xdf 파일 세트를 만듭니다. 그러면 후속 R 분석에서 `RxXdfData` 개체를 데이터 인수로 사용할 수 있습니다.
...
분석을 실행하는 동안 효율성을 높이기 위해 CSV를 XDF로 변환했지만, 데이터를 다시 CSV로 변환하려면 [rxDataStep](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxdatastep)을 사용합니다.

CSV 파일 폴더를 만들려면 먼저 디렉터리 이름을 파일 인수로 사용하여 `RxTextData` 개체를 만듭니다. 이 디렉터리는 CSV 파일을 만들 폴더를 나타냅니다. 이 디렉터리는 `rxDataStep`을 실행할 때 만들어집니다. 그런 다음, `rxDataStep`의 `outFile` 인수에서 `RxTextData` 개체를 가리킵니다. 생성된 각 CSV는 디렉터리 이름 뒤에 숫자를 붙여 이름이 지정됩니다.
...
`rxDataStep`은 입력 복합 XDF 파일의 모든 .xdfd 파일에 대한 하나의 CSV를 작성했습니다. 이는 컴퓨팅 컨텍스트가 `RxSpark`로 설정된 경우 복합 XDF에서 HDFS로 CSV를 작성하기 위한 기본 동작입니다.
...
`RxSpark` 컴퓨팅 컨텍스트를 사용하는 경우 `createFileSet`는 기본적으로 `TRUE`로 설정되고 `rowsPerOutFile`은 아무 영향도 주지 않습니다. 따라서 단일 CSV를 만들거나 파일당 행 수를 사용자 지정하려면 `local` 컴퓨팅 컨텍스트에서 `rxDataStep`을 수행해야 합니다(데이터는 여전히 HDFS일 수 있음).
...
이 자습서에서는 Azure HDInsight의 ML Services 클러스터에서 실행되는 Apache Spark에서 R 함수를 사용하는 방법을 알아보았습니다. 자세한 내용은 다음 문서를 참조하세요.
