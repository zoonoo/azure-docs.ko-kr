...
4. **구독** 드롭다운에서 클러스터에 사용할 Azure 구독을 선택합니다.

5. **클러스터 구성**를 선택한 후 다음을 선택합니다.
   ...
   * **클러스터 계층**: Azure HDInsight는 빅 데이터 클라우드 제품을 표준 계층 및 프리미엄 계층의 두 범주로 제공합니다. 자세한 내용은 [클러스터 계층](hdinsight-hadoop-provision-linux-clusters.md#cluster-tiers)을 참조하세요.
     ...
     ![HDInsight 프리미엄 계층 구성](./media/hdinsight-hadoop-create-linux-cluster-portal/hdinsight-cluster-type-configuration.png)

6. **응용 프로그램**을 클릭하여 HDInsight 클러스터에 작동하는 응용 프로그램을 설치합니다. Microsoft, ISV(독립 소프트웨어 공급 업체) 또는 사용자가 직접 이러한 응용 프로그램을 개발할 수 있습니다. 자세한 내용은 [HDInsight 응용 프로그램 설치](hdinsight-apps-install-applications.md#install-applications-during-cluster-creation)를 참조하세요. 
...
8. **데이터 원본** 블레이드에서 Azure Storage(WASB) 또는 Data Lake Store를 기본 저장소로 사용할지 여부를 지정합니다.  

    * **기본 저장소인 Azure Storage Blob**

        **기본 저장소 형식**으로 **Azure Storage**를 클릭합니다. 저장소 계정 및 저장소 컨테이너에 대한 세부 정보를 지정하고 위치를 지정한 다음 **클러스터 AAD ID**를 클릭합니다.

        ![HDInsight 클러스터에 서비스 주체 추가](./media/hdinsight-hadoop-create-linux-cluster-portal/hdi.adl.1.png "HDInsight 클러스터에 서비스 주체 추가")

      >[!NOTE]
      > 클러스터 AAD ID는 AAD 구성에 따라 클러스터가 Azure Data Lake 저장소에 액세스할 수 있도록 하는 데 사용됩니다. Data Lake Store 계정을 클러스터와 연결된 추가 저장소로 사용하려는 경우 이 옵션을 사용합니다.
      > 
      >

        **선택** 을 클릭하여 데이터 원본 구성을 저장합니다.

    * **기본 저장소로 Azure Data Lake Store**

        기본 저장소로 Data Lake Store를 사용하여 HDInsight 클러스터를 만드는 방법에 대한 지침은 [Azure Portal을 사용하여 Data Lake Store를 포함한 HDInsight 클러스터 만들기](../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md)를 참조하세요.
...
10. **고급 구성**을 클릭하여 **가상 네트워크**에 가입, Hive 및 Oozie의 데이터를 유지하기 위한 **외부 Metastore** 설정 등 기타 선택적 설정을 구성하고 **스크립트 작업**을 사용하여 사용자 지정 구성 요소를 설치하기 위해 클러스터를 사용자 지정하거나 클러스터에 **추가 저장소 계정**을 사용합니다.
    ...
    * **스크립트 동작** : 클러스터를 만들 때 사용자 지정 스크립트를 사용하여 클러스터를 사용자 지정하려는 경우에 사용합니다. 스크립트 동작에 대한 자세한 내용은 [스크립트 동작을 사용하여 HDInsight 클러스터 사용자 지정](hdinsight-hadoop-customize-cluster-linux.md)을 참조하세요. 스크립트 동작 블레이드에서는 화면 캡처에 표시된 것과 같은 세부 정보를 제공합니다.
        ...
        ![스크립트 작업 블레이드](./media/hdinsight-hadoop-create-linux-cluster-portal/HDI.CreateCluster.8.png "스크립트 작업 지정")

    * **추가 저장소 계정**을 클릭하여 클러스터와 연결할 추가 저장소 계정을 지정합니다. **Azure 저장소 키** 블레이드에서 **저장소 키 추가**를 클릭한 다음 기존 저장소 계정을 선택하거나 새 계정을 만듭니다.
