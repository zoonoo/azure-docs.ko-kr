...
"Noconditionaltask = true 이면 noconditionaltask를 확인 하 고 map 조인의 테이블 크기 합계가 noconditionaltask 보다 작은 경우에는이 계획에서 맵 조인을 생성 하는 것입니다 .이 경우에는 계산이 수행 되지 않습니다. 입력 크기의 합계가 작은 여백 쿼리를 통해 noconditionaltask 크기 보다 작은 경우 다른 HashTable 구현에 의해 도입 된 오버 헤드를 결과로 계산 합니다. "
