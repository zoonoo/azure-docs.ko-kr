...
* T1은 STRING 열 형식이 많은 큰 테이블 TABLE1의 별칭입니다.
* 다른 테이블은 그다지 크지 않지만 많은 열이 있습니다.
...
"hive.auto.convert.join.noconditionaltask = true noconditionaltask.size를 확인하고 맵 조인의 테이블 크기 합이 noconditionaltask.size보다 작으면 계획이 맵 조인을 생성할 때 이 문제는 계산이 수행되지 않는다는 것입니다. 입력 크기의 합이 작은 여백 쿼리에 의한 무조건 작업 크기보다 작으면 다른 HashTable 구현에서 발생하는 오버헤드를 OOM에 도달합니다."
...
맵 조인이 Java 힙 스페이스의 메모리 오류 원인일 가능성이 높습니다. 블로그 게시물 [HDInsight에서 Hadoop Yarn 메모리 설정](https://blogs.msdn.com/b/shanyu/archive/2014/07/31/hadoop-yarn-memory-settings-in-hdinsigh.aspx)에 설명된 것처럼 Tez 실행 엔진을 사용할 때 사용된 힙 엔진은 실제로 Tez 컨테이너에 속합니다. Tez 컨테이너 메모리를 설명하는 다음 이미지를 참조하세요.
...
블로그 게시물에서 알 수 있듯이 **hive.tez.container.size** 및 **hive.tez.java.opts**의 두 가지 메모리 설정이 힙의 컨테이너 메모리를 정의합니다. 우리의 경험에 따르면 메모리 부족 예외가 컨테이너 크기가 너무 작다는 것을 의미하지는 않습니다. Java 힙 크기(hive.tez.java.opts)가 너무 작은 것입니다. 메모리 부족이 표시될 때마다 **hive.tez.java.opts**를 늘려볼 수 있습니다. 필요한 경우 **hive.tez.container.size**를 늘려야 할 수도 있습니다. **java.opts** 설정은 **container.size**의 80% 정도여야 합니다.

> [!NOTE]  
> **hive.tez.java.opts** 설정은 항상 **hive.tez.container.size**보다 작아야 합니다.

D12 컴퓨터에는 28GB 메모리가 있으므로 10GB(10240MB)의 컨테이너 크기를 사용하고 java.opts에 80%를 할당하기로 결정했습니다.
