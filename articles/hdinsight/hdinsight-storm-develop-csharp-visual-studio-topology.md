...
bolt 및 spout 구성 요소에서 구성 값을 검색하는 데 ConfigurationManager를 사용하지 마세요. null 포인터 예외가 발생합니다. 대신, 프로젝트에 대한 구성이 토폴로지 컨텍스트의 키/값 쌍으로 Storm 토폴로지에 전달됩니다. 구성 값을 사용하는 각 구성 요소는 초기화 중에 컨텍스트에서 값을 검색해야 합니다.
