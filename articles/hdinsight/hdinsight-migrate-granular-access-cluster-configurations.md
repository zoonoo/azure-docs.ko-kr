...
- [API](#api): 또는 `/configurations/{configurationName}` 끝점을 `/configurations` 사용 하는 사용자입니다.
...
- [.NET용 SDK](#sdk-for-net)
    - [버전 1.x 또는](#versions-1x-and-2x)2.x: 사용자는 ConfigurationsOperationsExtensions 클래스 `GetClusterConfigurations`에서, `GetConnectivitySettings` `ConfigureHttpSettings`, `EnableHttp` 또는 `DisableHttp` 메서드를 사용 합니다.
    - [버전](#versions-3x-and-up)3.x `Get`이상: `Update` `EnableHttp` `ConfigurationsOperationsExtensions` 클래스에서,, 또는 `DisableHttp` 메서드를 사용 하는 사용자입니다.
- [Python 용 SDK](#sdk-for-python): `ConfigurationsOperations` 클래스의 또는 `get` `update` 메서드를 사용 하는 사용자입니다.
- [Java 용 SDK](#sdk-for-java): `ConfigurationsInner` 클래스의 또는 `update` `get` 메서드를 사용 하는 사용자입니다.
- [Go 용 SDK](#sdk-for-go): `ConfigurationsClient` 구조체의 또는 `Get` `Update` 메서드를 사용 하는 사용자입니다.
...
2019 `GET /configurations` 년 9 월 3 일부 터 `POST /configurations/gateway` 호출은 더 이상 정보를 반환 하지 않으며 `GET /configurations/{configurationName}` 호출은 저장소 계정 키 또는 클러스터 암호와 같은 중요 한 매개 변수를 더 이상 반환 하지 않습니다. 해당 SDK 메서드 및 PowerShell cmdlet의 경우에도 마찬가지입니다.
