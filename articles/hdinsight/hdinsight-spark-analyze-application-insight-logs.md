...
3. 페이지의 첫 번째 필드( **셀**이라고 함)에 다음을 입력합니다.
...
4. **SHIFT+ENTER** 를 사용하여 코드를 실행합니다. '\*'가 셀의 왼쪽에 대괄호 사이에 표시되면 이 셀의 코드가 실행되고 있음을 나타냅니다. 완료되면 '\*'는 번호로 변경되고 셀 아래에 다음과 유사한 출력이 표시됩니다.
...
5. 새 셀은 첫 번째 셀의 아래에 생성됩니다. 새 셀에 다음을 입력합니다. **CONTAINER** 및 **STORAGEACCOUNT**를 Application Insights 연속 내보내기를 구성할 때 사용한 Azure Storage 계정 이름 및 Blob 컨테이너 이름으로 바꿉니다.
   
        %%bash
        hdfs dfs -ls wasb://CONTAINER@STORAGEACCOUNT.blob.core.windows.net/
   
    **SHIFT+ENTER**를 사용하여 이 셀을 실행합니다. 다음과 유사한 결과가 표시됩니다.
...
6. 다음 셀에 다음을 입력합니다. **WASB\_PATH**를 이전 단계의 경로로 바꿉니다.
    ...
    이렇게 하면 연속 내보내기 프로세스에서 내보낸 JSON 파일에서 새 데이터 프레임을 만듭니다. **SHIFT+ENTER** 를 사용하여 이 셀을 실행합니다.
