...
연결 API에 대한 자세한 [https://kafka.apache.org/documentation/#connect](https://kafka.apache.org/documentation/#connect)내용은 를 참조하십시오.

## <a name="prerequisites"></a>사전 요구 사항

* HDInsight에 아파치 카프카 클러스터. 자세한 내용은 [HDInsight의 Kafka 빠른 시작](apache-kafka-get-started.md) 문서를 참조하세요.

* Kafka 클러스터의 에지 노드입니다. 자세한 내용은 [HDInsight와 함께 에지 노드 사용](../hdinsight-apps-use-edge-node.md) 문서를 참조하세요.

* SSH 클라이언트. 자세한 내용은 [SSH를 사용하여 HDInsight(Apache Hadoop)에 연결](../hdinsight-hadoop-linux-use-ssh-unix.md)을 참조하세요.

* Azure IoT 허브 및 장치입니다. 이 문서에서는 [Azure IoT Hub에 라즈베리 파이 온라인 시뮬레이터 연결 을](https://docs.microsoft.com/azure/iot-hub/iot-hub-raspberry-pi-web-simulator-get-started)사용하는 것이 좋습니다.

* [스칼라 빌드 도구](https://www.scala-sbt.org/).

## <a name="build-the-connector"></a>커넥터 빌드

1. 커넥터의 소스를 로컬 [https://github.com/Azure/toketi-kafka-connect-iothub/](https://github.com/Azure/toketi-kafka-connect-iothub/) 환경으로 다운로드합니다.
...
2. 명령 프롬프트에서 `toketi-kafka-connect-iothub-master` 디렉터리로 이동합니다. 그런 다음 다음 명령을 사용하여 프로젝트를 빌드하고 패키징합니다.
    ...
    빌드를 완료하는 데 몇 분 정도 걸릴 수 있습니다. 명령은 프로젝트의 `kafka-connect-iothub-assembly_2.11-0.7.0.jar` `toketi-kafka-connect-iothub-master\target\scala-2.11` 디렉터리에서 명명된 파일을 만듭니다.

## <a name="install-the-connector"></a>커넥터 설치

1. HDInsight 클러스터에서 카프카의 가장자리 노드에 .jar 파일을 업로드합니다. 클러스터의 실제 이름으로 대체하여 `CLUSTERNAME` 아래 명령을 편집합니다. SSH 사용자 계정의 기본값과 [에지 노드](../hdinsight-apps-use-edge-node.md#access-an-edge-node) 이름은 필요에 따라 수정하여 아래에 사용됩니다.
...
나머지 단계에 대해 SSH 연결을 활성 상태로 유지합니다.

## <a name="configure-apache-kafka"></a>Apache Kafka 구성

SSH 연결에서 에지 노드에 이르기까지 다음 단계를 사용하여 Kafka를 구성하여 독립 실행형 모드에서 커넥터를 실행하도록 구성합니다.

1. 암호 변수를 설정합니다. 암호를 클러스터 로그인 암호로 바꾼 다음 명령을 입력합니다.

    ```bash
    export password='PASSWORD'
    ```

1. [jq](https://stedolan.github.io/jq/) 유틸리티를 설치합니다. jq를 사용하면 Ambari 쿼리에서 반환된 JSON 문서를 보다 쉽게 처리할 수 있습니다. 다음 명령을 입력합니다.
...
1. Kafka broker의 주소를 가져옵니다. 클러스터에 많은 broker가 있을 수 있지만 하나 또는 두 개의 참조만 필요합니다. 두 broker 호스트의 주소를 가져오려면 다음 명령을 사용합니다.
    ...
    나중에 사용할 값을 복사합니다. 반환되는 값은 다음 텍스트와 유사합니다.

    `wn0-kafka.w5ijyohcxt5uvdhhuaz5ra4u5f.ex.internal.cloudapp.net:9092,wn1-kafka.w5ijyohcxt5uvdhhuaz5ra4u5f.ex.internal.cloudapp.net:9092`

1. Apache Zookeeper 노드의 주소를 가져옵니다. 클러스터에 여러 Zookeeper 노드가 있지만 하나 또는 두 개의 참조만 필요합니다. 다음 명령을 사용하여 변수의 `KAFKAZKHOSTS`주소를 저장합니다.
...
1. 다음 을 편집합니다.
    ...
    |현재 값 |새 값 | 주석 |
    |---|---|---|
    |`bootstrap.servers=localhost:9092`|이전 `localhost:9092` 단계에서 브로커 호스트로 값을 바꿉니다.|Kafka 브로커를 찾기 위해 에지 노드의 독립 실행형 구성을 구성합니다.|
    |`key.converter=org.apache.kafka.connect.json.JsonConverter`|`key.converter=org.apache.kafka.connect.storage.StringConverter`|이 변경을 통해 Kafka에 포함된 콘솔 생산자를 사용하여 테스트할 수 있습니다. 다른 생산자와 소비자에 대한 다른 변환기가 필요할 수 있습니다. 다른 변환기 값 사용에 대한 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)자세한 내용은 을 참조하십시오.|
    ...
    |해당 없음|`consumer.max.poll.records=10`|파일 끝에 추가합니다. 이 변경은 한 번에 10개의 레코드로 제한하여 싱크 커넥터에서 시간 제한을 방지하기 위함입니다. 자세한 내용은 을 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)참조하십시오.|
...
1. 편집기에서 다음 항목을 찾아 변경합니다.
    ...
    예를 들어 구성은 [Azure IoT Hub에 대한 Kafka 연결 소스 커넥터를](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Source.md)참조하십시오.

1. 변경을 저장하려면 __Ctrl + X__, __Y__ 및 __Enter__ 키를 사용합니다.

커넥터 소스 구성에 대한 자세한 내용은 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Source.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Source.md)을 참조하십시오.
...
1. 편집기에서 다음 항목을 찾아 변경합니다.
    ...
    예를 들어 구성은 [Azure IoT Hub에 대한 Kafka 연결 싱크 커넥터를](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)참조하십시오.

1. 변경을 저장하려면 __Ctrl + X__, __Y__ 및 __Enter__ 키를 사용합니다.

커넥터 싱크 구성에 대한 자세한 내용은 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)을 참조하십시오.
...
1. **Ctrl + C를** 두 번 사용하여 몇 분 후에 커넥터를 중지합니다. 커넥터가 중지되는 데 몇 분 정도 걸릴 수 있습니다.
...
1. 카프카 *클러스터에 두 번째* SSH 세션을 엽니다.

    ```bash
    ssh sshuser@new-edgenode.CLUSTERNAME-ssh.azurehdinsight.net
    ```

1. 새로운 ssh 세션에 대한 카프카 브로커의 주소를 가져옵니다. 암호를 클러스터 로그인 암호로 바꾼 다음 명령을 입력합니다.
...
1. `iotout` 항목에 메시지를 보내려면 다음 명령을 사용합니다.
    ...
    이 명령은 일반 Bash 프롬프트로 돌아오지 않습니다. 대신 `iotout` 항목에 키보드 입력을 보냅니다.
...
1. 디바이스에 메시지를 보내려면 `kafka-console-producer`에 대한 SSH 세션에 JSON 문서를 붙여넣습니다.
    ...
    이 JSON 문서의 스키마는 에서 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)자세히 설명되어 있습니다.

    시뮬레이션된 라즈베리 파이 장치를 사용 하 고 실행 중인 경우 다음 메시지가 장치에 의해 기록 됩니다.
...
싱크 커넥터 사용에 대한 자세한 내용은 [https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md](https://github.com/Azure/toketi-kafka-connect-iothub/blob/master/README_Sink.md)을 참조하십시오.
