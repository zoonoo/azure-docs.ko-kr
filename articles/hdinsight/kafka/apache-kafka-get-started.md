...
7. __애플리케이션(선택 사항)__ 에서 __다음__을 선택하여 기본 설정을 사용해서 계속 진행합니다.
...
2. 환경 변수를 클러스터 이름으로 설정하려면 다음 명령을 사용합니다.
    ...
    > [!Important]
    > 이름에 대문자를 사용하여 프로비저닝된 경우에도, 이 명령에 대해 클러스터 이름을 모두 소문자로 입력합니다. 대문자 이름은 Zookeeper 또는 broker 정보에 사용할 수 없습니다.
