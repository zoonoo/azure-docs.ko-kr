---
title: 클러스터 구성을 최적화하는 아파치 암바리 - Azure HDInsight
description: 아파치 암바리 웹 UI를 사용하여 Azure HDInsight 클러스터를 구성하고 최적화합니다.
...
ms.locfileid: "81532535"
---
# <a name="use-apache-ambari-to-optimize-hdinsight-cluster-configurations"></a>Apache Ambari를 사용하여 HDInsight 클러스터 구성 최적화

HDInsight는 대규모 데이터 처리 애플리케이션을 위한 [아파치 하두롭](./hadoop/apache-hadoop-introduction.md) 클러스터를 제공합니다. 이렇게 복잡한 다중 노드 클러스터를 관리, 모니터링 및 최적화하는 작업은 어려울 수 있습니다. [Apache Ambari](https://ambari.apache.org/)는 HDInsight Linux 클러스터를 관리하고 모니터링하는 웹 인터페이스입니다.  Windows 클러스터의 경우 [Ambari REST API를](hdinsight-hadoop-manage-ambari-rest-api.md)사용합니다.
...
![아파치 암바리 사용자 대시보드 표시](./media/hdinsight-changing-configs-via-ambari/apache-ambari-dashboard.png)

Ambari 웹 UI는 호스트, 서비스, 경고, 구성 및 보기를 관리하는 데 사용됩니다. Ambari는 HDInsight 클러스터를 만들거나 서비스를 업그레이드하는 데 사용할 수 없습니다. 또한 스택 및 버전을 관리하거나, 호스트를 해제 또는 다시 서비스 해제하거나, 클러스터에 서비스를 추가할 수 없습니다.

## <a name="manage-your-clusters-configuration"></a>클러스터 구성 관리

구성 설정은 특정 서비스를 조정하는 데 도움이 됩니다. 서비스의 구성 설정을 수정하려면 **서비스** 사이드바(왼쪽)에서 서비스를 선택합니다. 그런 다음 서비스 세부 정보 페이지의 구성 탭으로 **이동합니다.**

![아파치 암바리 서비스 사이드바](./media/hdinsight-changing-configs-via-ambari/ambari-services-sidebar.png)

### <a name="modify-namenode-java-heap-size"></a>NameNode Java 힙 크기 수정

NameNode Java 힙 크기는 클러스터의 로드와 같은 여러 요인에 따라 달라집니다. 또한 파일 수및 블록 수입니다. 기본 크기인 1GB는 대부분의 클러스터에서 잘 작동하지만, 일부 워크로드에는 메모리가 더 많이 또는 적게 필요할 수 있습니다.

NameNode Java 힙 크기를 수정하려면:

1. 서비스 사이드바에서 **HDFS**를 선택하고 **Configs**(구성) 탭으로 이동합니다.

    ![아파치 암바리 HDFS 구성](./media/hdinsight-changing-configs-via-ambari/ambari-apache-hdfs-config.png)

1. **NameNode Java heap size**(NameNode Java 힙 크기) 설정을 찾습니다. **필터** 텍스트 상자에 특정 설정을 입력하여 찾을 수도 있습니다. 설정 이름 옆에 있는 **펜** 아이콘을 선택합니다.

    ![아파치 암바리 네임노드 자바 힙 크기](./media/hdinsight-changing-configs-via-ambari/ambari-java-heap-size.png)

1. 텍스트 상자에 새 값을 입력한 다음 **Enter** 키를 눌러 변경 내용을 저장합니다.

    ![암바리 편집 네임노드 자바 힙 크기1](./media/hdinsight-changing-configs-via-ambari/java-heap-size-edit1.png)

1. NameNode Java 힙 크기가 2GB에서 1GB로 변경되었습니다.

    ![편집 된 네임 노드 자바 힙 크기2](./media/hdinsight-changing-configs-via-ambari/java-heap-size-edited.png)

1. 구성 화면 위쪽에서 **저장** 단추를 클릭하여 변경 내용을 저장합니다.

    !['아파치 암바리 저장 구성'](./media/hdinsight-changing-configs-via-ambari/ambari-save-changes1.png)
...
1. Hive **Configs**(구성) 탭의 필터 상자에 **실행 엔진**을 입력합니다.
    ...
    ![아파치 암바리 검색 실행 엔진](./media/hdinsight-changing-configs-via-ambari/ambari-search-execution.png)

1. **Optimization**(최적화) 속성의 기본값은 **Tez**입니다.

    ![최적화 - 아파치 테즈 엔진](./media/hdinsight-changing-configs-via-ambari/optimization-apache-tez.png)
...
성능 지침으로 이러한 매개 변수를 모두 낮게 하여 대기 시간을 개선하고 처리량을 늘리세요.
...
1. 두 매개 변수를 모두 **33,554,432** 바이트(32MB)로 설정합니다.
    ...
    ![아파치 암바리 테즈 그룹화 크기](./media/hdinsight-changing-configs-via-ambari/apache-tez-grouping-size.png)
...
1. 매개 변수를 수정하려면 Hive **Configs**(구성) 탭으로 이동하고 설정 페이지에서 **Data per Reducer**(리듀서당 데이터) 매개 변수를 찾습니다.
    ...
    ![감속기당 아파치 암바리 데이터](./media/hdinsight-changing-configs-via-ambari/ambari-data-per-reducer.png)

1. **편집**을 선택하여 값을 128MB(134,217,728바이트)로 수정한 다음 **Enter**를 눌러 저장합니다.

    ![감속기 당 암바리 데이터 - 편집](./media/hdinsight-changing-configs-via-ambari/data-per-reducer-edited.png)
  
    1,024MB의 입력 크기를 감안할 때 감속기당 128MB의 데이터가 제공되며 8개의 감속기(1024/128)가 있습니다.
...
1. 병렬로 실행할 작업 수를 제한하려면 속성을 `hive.exec.parallel.thread.number` 수정합니다. 기본값은 8입니다.

    ![아파치 하이브 임원 병렬 디스플레이](./media/hdinsight-changing-configs-via-ambari/apache-hive-exec-parallel.png)
...
1. 쿼리의 리듀스 측에 대해 벡터화된 실행을 사용하도록 설정하려면 `hive.vectorized.execution.reduce.enabled` 매개 변수를 true로 설정합니다. 기본값은 False입니다.
    ...
    ![아파치 하이브 벡터화 실행](./media/hdinsight-changing-configs-via-ambari/hive-vectorized-execution.png)

### <a name="enable-cost-based-optimization-cbo"></a>CBO(비용 기반 최적화) 사용

기본적으로 Hive는 일련의 규칙에 따라 하나의 최적 쿼리 실행 계획을 찾습니다. 비용 기반 최적화(CBO)는 쿼리를 실행하기 위한 여러 계획을 평가합니다. 또한 각 계획에 비용을 할당한 다음 쿼리를 실행하는 가장 저렴한 계획을 결정합니다.

CBO를 사용하려면 **하이브** > **구성** > **설정으로** 이동하여 **비용 기반 최적화 를 찾은**다음 토글 단추를 **On으로**전환합니다.
...
* `hive.compute.query.using.stats`
    ...
    ![통계를 사용하여 아파치 하이브 계산 쿼리](./media/hdinsight-changing-configs-via-ambari/hive-compute-query-using-stats.png)
...
* `hive.stats.fetch.column.stats`
    ...
    ![아파치 하이브 통계 세트 컬럼 통계](./media/hdinsight-changing-configs-via-ambari/hive-stats-fetch-column-stats.png)

* `hive.stats.fetch.partition.stats`

    행 수, 데이터 크기 및 파일 크기와 같은 기본 파티션 통계는 metastore에 저장됩니다. true로 설정된 경우 파티션 통계는 메타스토어에서 가져옵니다. false이면 파일 시스템에서 파일 크기를 가져옵니다. 행 스키마에서 행 수가 가져옵니다.
...
일반적으로 압축 메서드 분할 가능이 중요하며 그렇지 않으면 몇 개의 매퍼가 만들어집니다. 입력 데이터가 텍스트인 경우 `bzip2`가 최고 옵션입니다. ORC 형식의 경우 Snappy가 가장 빠른 압축 옵션입니다.

1. 중간 압축을 사용하려면 Hive **Configs**(구성) 탭으로 이동한 다음 `hive.exec.compress.intermediate` 매개 변수를 true로 설정합니다. 기본값은 False입니다.

    !['하이브 임원 압축 중간'](./media/hdinsight-changing-configs-via-ambari/hive-exec-compress-intermediate.png)
...
1. 사용자 지정 설정을 추가하려면:
    ...
    a. **하이브** > **구성** > **고급** > **사용자 정의 하이브 사이트로**이동합니다.

    b. 사용자 지정 하이브 사이트 창 의 맨 아래에 있는 **속성 추가를** 선택합니다.

    다. 속성 추가 창에서 키에 `mapred.map.output.compression.codec`을 입력하고 값에 `org.apache.hadoop.io.compress.SnappyCodec`을 입력합니다.

    d. **추가**를 선택합니다.

    !['아파치 하이브 사용자 정의 속성 추가'](./media/hdinsight-changing-configs-via-ambari/hive-custom-property.png)

    이 설정은 스냅 압축을 사용하여 중간 파일을 압축합니다. 속성이 추가되면 사용자 지정 hive-site 창에 나타납니다.
...
1. 출력 압축 코덱을 선택하려면 이전 섹션 3단계의 설명에 따라 사용자 지정 hive-site 창에 `mapred.output.compression.codec` 사용자 지정 속성을 추가합니다.
    ...
    ![아파치 하이브 사용자 정의 속성 추가2](./media/hdinsight-changing-configs-via-ambari/hive-custom-property2.png)

### <a name="enable-speculative-execution"></a>투기적 실행 사용

투기적 실행은 느린 실행 작업 추적기를 검색하고 거부하기 위해 특정 수의 중복 작업을 시작합니다. 개별 작업 결과를 최적화하여 전반적인 작업 실행을 개선합니다.
...
* 투기적 실행을 사용하려면 Hive **Configs**(구성) 탭으로 이동한 다음 `hive.mapred.reduce.tasks.speculative.execution` 매개 변수를 true로 설정합니다. 기본값은 False입니다.
    ...
    !['하이브 매핑은 작업 투기 적 실행을 감소'](./media/hdinsight-changing-configs-via-ambari/hive-mapred-reduce-tasks-speculative-execution.png)

### <a name="tune-dynamic-partitions"></a>동적 파티션은 조정

Hive를 사용하면 모든 파티션을 미리 정의하지 않고 레코드를 테이블에 삽입할 때 동적 파티션을 만들 수 있습니다. 이 능력은 강력한 기능입니다. 그것은 파티션의 큰 숫자의 생성을 초래할 수 있지만. 그리고 각 파티션에 대한 파일의 큰 숫자.

1. Hive에서 동적 파티션을 수행하려면 `hive.exec.dynamic.partition` 매개 변수 값이 true(기본값)여야 합니다.

1. 동적 파티션 모드를 *strict*로 변경합니다. strict 모드에서는 적어도 하나의 파티션이 static이어야 합니다. 이 설정은 WHERE 절의 파티션 필터가 없는 *strict* 쿼리를 방지합니다. Hive **Configs**(구성) 탭으로 이동한 다음 `hive.exec.dynamic.partition.mode`를 **strict**로 설정합니다. 기본값은 **nonstrict**입니다.
...
로컬 모드를 사용하면 Hive가 단일 컴퓨터에서 작업의 모든 작업을 수행할 수 있습니다. 또는 때로는 단일 프로세스에서. 이 설정은 입력 데이터가 작은 경우 쿼리 성능을 향상시킵니다. 또한 쿼리에 대한 작업을 시작하는 오버헤드는 전체 쿼리 실행의 상당 부분을 소비합니다.
...
![아파치 하이브 exec 모드 로컬 자동](./media/hdinsight-changing-configs-via-ambari/hive-exec-mode-local-auto.png)
...
| 맵 조인 | <ul><li>하나의 테이블이 메모리에 들어가는 경우</li></ul> | <ul><li>작은 테이블을 메모리 해시 테이블로 읽기</li><li>큰 파일의 일부를 통해 스트리밍</li><li>해시 테이블에서 각 레코드 조인</li><li>매퍼 단독으로 조인</li></ul> | `hive.auto.confvert.join=true` | 빠르지만 제한적입니다. |
| 병합 버킷 정렬 | 두 테이블이 모두: <ul><li>동일하게 정렬된 경우</li><li>동일하게 버킷 처리된 경우</li><li>정렬된/버킷 처리된 열에 조인하는 경우</li></ul> | 각 프로세스가: <ul><li>각 테이블에서 버킷 읽기</li><li>값이 가장 낮은 행 처리하기</li></ul> | `hive.auto.convert.sortmerge.join=true` | 효율적인 |
...
1. 새 값을 저장하려면 창의 오른쪽 상단에 **저장을** 선택합니다. 일부 속성은 서비스를 다시 시작해야 할 수도 있습니다.

    ![고급 아파치 돼지 속성](./media/hdinsight-changing-configs-via-ambari/advanced-pig-properties.png)
...
Pig는 UDF에 필요한 JAR 파일을 태스크 노드에서 사용할 수 있도록 분산 캐시에 복사합니다. 이 항아리는 자주 변경되지 않습니다. `pig.user.cache.enabled` 설정을 사용하도록 설정하면 jar를 캐시에 배치하여 동일한 사용자가 실행하는 작업에 다시 사용할 수 있습니다. 이 설정으로 인해 작업 성능이 약간 향상됩니다.
...
* `pig.cachedbag.memusage`: 가방에 부여된 메모리양입니다. bag은 튜플의 컬렉션입니다. 튜플은 정렬된 필드 집합이며 필드는 데이터의 일부입니다. 가방에 있는 데이터가 지정된 메모리를 초과하면 디스크에 유출됩니다. 기본값은 0.2이며, 이것은 사용 가능한 메모리의 20%를 나타냅니다. 이 메모리는 애플리케이션의 모든 bag에서 공유됩니다.
...
* `pig.tmpfilecompression.codec`: 임시 파일을 압축하는 데 사용하는 압축 코덱입니다. 권장되는 압축 코덱은 낮은 CPU 사용을 위해 [LZO](https://www.oberhumer.com/opensource/lzo/) 및 Snappy입니다.

### <a name="enable-split-combining"></a>분할 결합 사용

사용하도록 설정하면 맵 작업 수가 줄어들도록 작은 파일이 결합됩니다. 이 설정은 많은 작은 파일로 작업의 효율성을 향상시킵니다. 사용하도록 설정하려면 `pig.noSplitCombination`을 true로 설정합니다. 기본값은 False입니다.

### <a name="tune-mappers"></a>매퍼 조정

매퍼 수는 `pig.maxCombinedSplitSize` 속성을 수정하여 제어됩니다. 이 속성은 단일 맵 태스크에서 처리할 데이터의 크기를 지정합니다. 기본값은 파일 시스템의 기본 블록 크기입니다. 이 값을 늘리면 매퍼 작업 수가 감소합니다.
...
[아파치 HBase](https://hbase.apache.org/) 구성은 HBase 구성 탭에서 **수정됩니다.** 다음 섹션에서는 HBase 성능에 영향을 주는 몇 가지 중요한 구성 설정에 대해 설명합니다.
...
1. 기본값을 5,000MB로 변경합니다.
    ...
    !['아파치 암바리 HBase 메모리 힙 사이즈'](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
...
1. 이 매개 변수를 수정하려면 HBase **Configs**(구성) 탭의 **Settings**(설정) 탭으로 이동한 다음 **% of RegionServer Allocated to Read Buffers**(버퍼를 읽기 위해 할당된 RegionServer의 %)를 찾습니다.
    ...
    ![아파치 HBase 메모리 블록 캐시 크기](./media/hdinsight-changing-configs-via-ambari/hbase-block-cache-size.png)

1. 값을 변경하려면 **편집** 아이콘을 선택합니다.

#### <a name="memstore-size"></a>Memstore 크기

모든 편집은 *Memstore*라는 메모리 버퍼에 저장됩니다. 이 버퍼는 단일 작업에서 디스크에 기록할 수 있는 데이터의 총량을 증가시킵니다. 또한 최근 편집에 대한 액세스 속도를 높입니다. Memstore 크기는 다음 두 가지 매개 변수로 정의됩니다.
...
`hbase.client.scanner.caching` 설정은 스캐너에서 `next` 메서드가 호출될 때 디스크에서 읽어오는 행 수를 정의합니다.  기본값은 100입니다. 숫자가 높을수록 클라이언트에서 영역 서버로의 원격 호출이 줄어들어 스캔 속도가 빨라집니다. 그러나 이 설정은 클라이언트에 대한 메모리 압력도 증가합니다.

![아파치 HBase 인출된 행 수](./media/hdinsight-changing-configs-via-ambari/hbase-num-rows-fetched.png)
...
!['아파치 H베이스 HRegion 최대 파일 크기'](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-max-filesize.png)
...
* HBase 영역 블록 승수는 에 `hbase.hregion.memstore.block.multiplier`의해 정의됩니다. 기본값은 4입니다. 허용되는 최대값은 8입니다.
...
![아파치 HBase 지역 블록 승수](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-memstore-block-multiplier.png)
...
Memstore 로컬 할당 버퍼 사용은 `hbase.hregion.memstore.mslab.enabled` 속성으로 결정됩니다. 이 설정은 true를 사용하도록 설정하면 쓰기 작업이 많은 동안 힙 조각화를 방지합니다. 기본값은 true입니다.
