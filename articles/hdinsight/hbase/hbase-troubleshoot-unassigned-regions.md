...
`hbase hbck` 명령을 실행 하면 다음과 비슷한 오류 메시지가 표시 됩니다.
...
1. `exit` 명령을 사용 하 여 사육 사 셸을 종료 합니다.

1. Apache Ambari UI를 열고 활성 HBase Master 서비스를 다시 시작합니다.

1. 더 이상 옵션을 사용 하지 않고 `hbase hbck` 명령을 다시 실행 합니다. 출력을 확인 하 고 모든 지역이 할당 되었는지 확인 합니다.
...
1. 현재 WALs: `hadoop fs -ls -R /hbase/WALs/ > /tmp/wals.out`목록을 가져옵니다.
...
1. 새 WALs 목록을 가져오려면 `hadoop fs -ls -R /hbase/WALs/ > /tmp/wals.out`를 실행 합니다.

1. *-디렉터리를 임시 폴더로 이동 하 고, `splitWAL`하 고, *-분할 디렉터리를 삭제 합니다.
