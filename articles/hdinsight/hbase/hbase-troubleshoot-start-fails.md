...
호출 스택을 확인 하 고 문제를 일으킬 수 있는 폴더 (예를 들어, WAL 폴더 또는 .tmp 폴더)를 확인 합니다. 그런 다음 클라우드 탐색기 또는 HDFS 명령을 사용하여 문제가 있는 파일을 찾습니다. 일반적으로이 `*-renamePending.json` 파일은 파일입니다. `*-renamePending.json` 파일은 WASB 드라이버에서 원자성 이름 바꾸기 작업을 구현 하는 데 사용 되는 저널 파일입니다. 이 구현의 버그로 인해 이러한 파일은 프로세스가 중단 된 후에도 남아 있을 수 있습니다. 클라우드 탐색기에서 또는 HDFS 명령을 사용 하 여이 파일을 강제로 삭제 합니다.
...
HMaster는와 유사한 예외를 제외 하 고 `java.io.IOException: Timedout 300000ms waiting for namespace table to be assigned`시간 초과 됩니다.
...
`hbase-site` 설정 `zookeeper.session.timeout` 뿐만 아니라 사육 사 `zoo.cfg` 설정 `maxSessionTimeout` 도 변경 해야 하는 경우를 제외 하 고, 사육 아웃 세션 제한 시간을 변경 합니다.
...
Ambari에서 hbase를 설정 wasb://@.blob.core.windows.net/hbase 하 고 서비스를 다시 시작 합니다.
