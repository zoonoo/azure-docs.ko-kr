...
Apache HBase 지역 서버에서 다시 시작 작업을 완료 하지 못했습니다. 영역 서버가 시작 되지 않는 작업자 노드의 `/var/log/hbase` 디렉터리에 있는 `region-server.log`에서 다음과 유사한 오류 메시지가 표시 될 수 있습니다.
...
1. 이러한 갑작스러운 종료로 인해 지역 서버 프로세스가 종료 되어도 프로세스와 연결 된 포트가 해제 되지 않을 수 있으며 결과적으로 `AddressBindException`됩니다.
