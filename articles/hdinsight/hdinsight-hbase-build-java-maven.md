...
4. __hbaseapp__ 디렉터리에 __conf__라는 새 디렉터리를 만듭니다. __conf__ 디렉터리에 __hbase-site.xml__이라는 새 파일을 만들고 다음을 그 내용으로 사용합니다.
    ...
    > [AZURE.NOTE] 이 파일은 최소 크기의 hbase-site.xml 파일로, HDInsight 클러스터용 완전한 최소 설정을 포함합니다. Linux 기반 클러스터의 경우 HBase가 사용하는 루트 Zookeeper znode를 올바르게 설정하기 위해 `zookeeper.znode.parent`에 대한 항목의 주석을 해제해야 합니다.
...
> [AZURE.NOTE] [HDInsight에서 Hadoop 작업용 데이터 업로드](hdinsight-upload-data.md)에서 설명한 대로 HDInsight 클러스터에 파일을 업로드하는 방법은 많습니다. 다음 단계에서는 [Azure PowerShell](../powershell-install-configure.md)을 사용합니다.
