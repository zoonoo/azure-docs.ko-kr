...
HDInsight의 Apache Storm은 Azure 환경에 통합된 관리되는 클러스터로서, HDInsight에서 Storm 및 다른 Hadoop 구성 요소는 HDP(Hortonworks Data Platform)에 기반한 반면 클러스터의 운영 체제는 Ubuntu(Linux 배포)입니다. Hadoop 에코시스템에서 인기 있는 도구 및 서비스와 호환되는 플랫폼을 제공합니다.
...
* 사용자가 선택한 언어 사용: **Java**, **C#** 및 **Python**으로 작성된 Storm 구성 요소에 대한 지원을 제공합니다.
...
* Event Hub, Azure Virtual Network, SQL Database, Blob Storage 및 DocumentDB 등의 다른 Azure 서비스와 통합합니다.
...
* __보안 셸 연결__: SSH를 사용하여 인터넷을 통해 HDInsight 클러스터의 헤드 노드에 액세스할 수 있습니다. 이 옵션을 사용하면 클러스터에서 직접 명령을 실행할 수 있습니다.

  자세한 내용은 [HDInsight와 함께 SSH 사용](hdinsight-hadoop-linux-use-ssh-unix.md)을 참조하세요.

* __웹 연결__: HDInsight 클러스터는 Ambari 웹 UI를 제공합니다. 그러면 클러스터에서 서비스를 쉽게 모니터링, 구성 및 관리할 수 있습니다. 또한 HDInsight의 Storm은 브라우저에서 실행 중인 Storm 토폴로지를 모니터링하고 관리할 수 있는 Storm UI를 제공합니다.
...
* __Visual Studio 통합__: Visual Studio용 Data Lake 도구(이전의 Visual Studio용 HDInsight 도구)에는 HDInsight의 Storm에 대한 도구뿐만 아니라 C# Storm 토폴로지를 만드는 프로젝트 템플릿이 포함되어 있습니다. Visual Studio에서 C# 토폴로지를 만들고 배포하며 모니터링하고 관리할 수 있습니다.
...
* __다른 Azure 서비스와 통합__
  ...
  * __Java__ 개발의 경우 Microsoft에서는 기존 Storm 구성 요소를 활용하여 가능한 다른 Azure 서비스와 통합합니다. 일부 경우에는 서비스 특정 구성 요소 또는 솔루션이 필요할 수 있습니다.
    ...
    * __Azure Blob Storage__(HDInsight의 저장소로 사용되는 경우): Java 기반 토폴로지는 `wasb://`의 URI 구성표와 함께 Storm-HDFS bolt를 사용하여 클러스터와 연결된 Azure Blob Storage에 액세스할 수 있습니다.
...
* **Spout**: 데이터 원본의 데이터를 사용하며 **스트림**을 하나 이상 내보냅니다.
  ...
  > [!NOTE]
  > 대부분의 경우에는 Kafka 또는 Azure Event Hubs와 같은 큐에서 데이터를 읽습니다. 이처럼 큐가 사용되므로 가동 중지 시에도 데이터가 유지됩니다.
...
여러 가지 방법으로 일괄 처리를 수행할 수 있습니다. C# 또는 Java 토폴로지에서는 간단한 카운터를 사용하여 X개의 튜플을 내보내기 전에 일괄 처리하거나 틱 튜플이라는 내부 타이밍 메커니즘을 사용하여 X초마다 Batch를 내보낼 수 있습니다.
