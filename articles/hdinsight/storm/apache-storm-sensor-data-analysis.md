...
![아키텍처 다이어그램](./media/apache-storm-sensor-data-analysis/devicesarchitecture.png)
...
토폴로지는 `org.apache.storm.eventhubs.spout.EventHubSpout` 클래스를 사용하여 Event Hub에서 데이터를 읽고 `org.apache.storm.hbase.bolt.HBaseBolt` 클래스를 사용하여 HBase에 데이터를 씁니다. 웹 사이트와의 통신은 [socket.io-client.java](https://github.com/nkzawa/socket.io-client.java)를 사용하여 수행됩니다.

다음 다이어그램은 토폴로지 레이아웃을 설명합니다.

![토폴로지 다이어그램](./media/apache-storm-sensor-data-analysis/sensoranalysis.png)
...
이 예제를 사용하기 전에 먼저 개발 환경을 준비해야 합니다. 이 예제에서 사용되는 Azure Event Hub도 만들어야 합니다.

개발 환경에 설치되어 있어야 하는 항목은 다음과 같습니다.
...
이벤트 허브를 만들려면 [이벤트 허브 만들기](../../event-hubs/event-hubs-create.md) 문서의 단계를 사용합니다.

> [!IMPORTANT]
> RootManageSharedAccessKey에 대한 이벤트 허브 이름, 네임스페이스 및 키를 저장합니다. 이 정보는 Storm 토폴로지를 구성하는 데 사용됩니다.

HDInsight 클러스터는 필요하지 않습니다. 이 문서의 단계에서는 이 예제에 필요한 리소스를 만드는 Azure Resource Manager 템플릿을 제공합니다. 이 템플릿에서 만드는 리소스는 다음과 같습니다.
...
3. 웹 브라우저를 열고 주소로 `http://localhost:3000/`을 입력합니다. 다음 이미지와 유사한 페이지가 표시됩니다.
    ...
    ![웹 대시보드](./media/apache-storm-sensor-data-analysis/emptydashboard.png)
...
2. 텍스트 편집기에서 `app.js` 파일을 열고 이전에 가져온 Event Hub 정보를 추가합니다.
   ...
   > [!NOTE]
   > 이 예제에서는 `RootManageSharedAccessKey`를 사용하여 이벤트 허브에 액세스한다고 가정합니다.
...
2. 토폴로지를 로컬 모드로 시작하려면 다음 명령을 사용합니다.
   ...
   일단 시작되면 토폴로지는 Event Hub에서 항목을 읽고 로컬 컴퓨터에서 실행되는 대시보드에 전송합니다. 웹 대시보드에 다음 이미지와 유사한 줄이 표시됩니다.
    ...
    ![데이터가 표시된 대시보드](./media/apache-storm-sensor-data-analysis/datadashboard.png)
...
2. **사용자 지정 배포** 섹션에서 다음 값을 입력합니다.
    ...
    ![HDInsight 매개 변수](./media/apache-storm-sensor-data-analysis/parameters.png)
...
![vnet 및 클러스터에 대한 리소스 그룹](./media/apache-storm-sensor-data-analysis/groupblade.png)
...
HDInsight의 Storm 토폴로지에 대한 자세한 내용은 [HDInsight의 Storm에 대한 예제 토폴로지](apache-storm-example-topology.md)를 참조하세요.

Apache Storm에 대한 자세한 내용은 [Apache Storm](https://storm.incubator.apache.org/) 사이트를 참조하세요.

HDInsight의 HBase에 대한 자세한 내용은 [HDInsight HBase 개요](../hbase/apache-hbase-overview.md)를 참조하세요.

Socket.io에 대한 자세한 내용은 [socket.io](http://socket.io/) 사이트를 참조하세요.

D3.js에 대한 자세한 내용은 [D3.js - 데이터 기반 문서](http://d3js.org/)를 참조하세요.

Java에서 토폴로지를 만들기에 대한 자세한 내용은 [HDInsight의 Apache Storm용 Java 토폴로지 개발](apache-storm-develop-java-topology.md)을 참조하세요.

.NET에서 토폴로지를 만들기에 대한 자세한 내용은 [Visual Studio를 사용하여 HDInsight의 Apache Storm용 C# 토폴로지 개발](apache-storm-develop-csharp-visual-studio-topology.md)을 참조하세요.
