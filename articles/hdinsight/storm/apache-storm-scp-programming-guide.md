...
Storm에서는 응용 프로그램 토폴로지에 따라 계산 그래프가 정의됩니다. 토폴로지의 각 노드에는 처리 논리가 포함되며 노드 간의 링크는 데이터 흐름을 나타냅니다. 토폴로지에 입력 데이터를 삽입하는 노드인 _spout_는 데이터 순서를 지정하는 데 사용할 수 있습니다. 입력 데이터는 파일 로그, 트랜잭션 데이터베이스, 시스템 성능 카운터 등에 있을 수 있습니다. 입력 및 출력 데이터 흐름을 포함하는 노드는 실제 데이터 필터링과 선택 및 집계를 하는 _bolt_라고 합니다.
