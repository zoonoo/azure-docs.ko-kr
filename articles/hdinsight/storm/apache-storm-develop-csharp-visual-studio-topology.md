...
description: C#으로 Storm 토폴로지를 만드는 방법을 알아봅니다. Visual Studio용 Hadoop 도구를 사용하여 Visual Studio에서 간단한 단어 카운트 토폴로지를 생성합니다.
ROBOTS: NOINDEX
...
C# 토폴로지는 .NET 4.5를 사용하고, Mono를 사용하여 HDInsight 클러스터에서 실행해야 합니다. 잠재적 호환성 없음에 대한 자세한 내용은 [Mono 호환성](https://www.mono-project.com/docs/about-mono/compatibility/)을 참조하세요. C# 토폴로지를 사용하려면 프로젝트에 사용되는 `Microsoft.SCP.Net.SDK` NuGet 패키지를 0.10.0.6 버전 이상으로 업데이트해야 합니다. 패키지 버전은 HDInsight에 설치된 Storm의 주 버전과도 일치해야 합니다.
...
[Visual Studio](https://visualstudio.microsoft.com/downloads/)를 사용하여 SCP.NET으로 C# 토폴로지를 개발할 수 있습니다. 이 지침에서는 Visual Studio 2019을 사용하지만 이전 버전의 Visual Studio를 사용할 수도 있습니다.
...
2. `JAVA_HOME`환경 변수를 Java가 포함된 디렉터리로 설정합니다.

3. `PATH`환경 변수는 `%JAVA_HOME%\bin`디렉터리를 포함하도록 합니다.

다음 C# 콘솔 애플리케이션을 구축 및 실행하여 Java 및 JDK가 올바르게 설치되었는지 확인할 수 있습니다:
...
> [!WARNING]  
> 일부 템플릿은 Linux 기반 HDInsight에서 작동합니다. 템플릿에서 사용하는 NuGet 패키지는 Mono와 호환되지 않을 수 있습니다. 잠재적 문제를 확인하기 위해 [Mono 호환성](https://www.mono-project.com/docs/about-mono/compatibility/) 문서를 확인하고 [.NET 이식성 분석기](../hdinsight-hadoop-migrate-dotnet-to-linux.md#automated-portability-analysis)를 사용합니다.
...
Visual Studio에서 C# 토폴로지 프로젝트를 생성하기 위해 다음을 수행합니다:

1. Visual Studio를 엽니다.

1. **시작** 창에서 **새 프로젝트 만들기** 를 선택합니다.

1. **새 프로젝트 만들기** 창에서 **Storm 애플리케이션** 으로 스크롤하여 선택한 후, **다음** 을 선택합니다.

1. **새 프로젝트 구성** 창에서 **프로젝트 이름** 으로 *WordCount* 를 입력하고, 프로젝트의 **위치** 디렉터리 경로를 만든 다음, **만들기** 를 선택합니다.

    :::image type="content" source="./media/apache-storm-develop-csharp-visual-studio-topology/apache-storm-new-project.png" alt-text="Storm 애플리케이션, 새 프로젝트 구성 대화 상자, Visual Studio" border="true":::

프로젝트를 만든 후에는 다음의 파일이 있어야 합니다.

* *Program.cs*: 프로젝트에 대한 토폴로지 정의. 기본적으로 하나의 Spout 및 하나의 Bolt로 구성된 기본 토폴로지가 만들어집니다.
...
다음으로, 외부 원본에서 토폴로지의 데이터를 읽는 데 사용되는 spout에 대한 코드를 추가합니다. 이 Spout는 문장을 토폴로지에 임의로 내보냅니다.

1. *Spout.cs* 를 엽니다. spout의 주요 구성 요소:

   * `NextTuple`: Spout에서 새 튜플을 내보낼 수 있도록 허용되면 Storm이 호출합니다.

   * `Ack`(트랜잭션 토폴로지만 해당): Spout에서 보낸 튜플의 토폴로지에서 다른 구성 요소에 의해 시작된 승인을 처리합니다. 튜플을 승인하면 spout가 다운스트림 구성 요소에 의해 성공적으로 처리되었음을 알 수 있습니다.
...
이제 이 예제에서 두 개의 Storm bolts를 만듭니다.

1. 프로젝트에서 기존 *Bolt.cs* 를 삭제합니다.

2. **솔루션 탐색기** 에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **추가** > **새 항목** 을 선택합니다. 목록에서 **Storm Bolt** 를 선택하고 이름으로 *Splitter.cs* 를 입력합니다. 새 파일의 코드에서 네임 스페이스 이름을 `WordCount`로 변경합니다. 이 프로세스를 반복하여 *Counter.cs* 라는 두 번째 Bolt를 만듭니다.
...
3. *Splitter.cs* 를 엽니다. 기본적으로 메서드는 `Execute`만 있습니다. `Execute` 메서드는 bolt가 처리에 대한 튜플을 받을 때 호출됩니다. 여기에서 들어오는 튜플을 읽고 처리하며 나가는 튜플을 내보낼 수 있습니다.
...
:::image type="content" source="./media/apache-storm-develop-csharp-visual-studio-topology/word-count-topology1.png" alt-text="Spout 및 볼트 구성 요소 정렬 다이어그램, Storm 토폴로지" border="false":::

Spout는 분할자 볼트의 인스턴스로 배포되는 문장을 내보냅니다. Splitter bolt는 Counter bolt로 배포된 단어로 문장을 나눕니다.
...
*Program.cs* 를 엽니다. 중요한 메서드는 `GetTopologyBuilder`이며, 이 방법은 Storm에 제출된 토폴로지를 정의하는 데 사용됩니다. 위에서 설명한 토폴로지를 구현하려면 `GetTopologyBuilder`의 내용을 다음 코드로 바꿉니다.
...
1. **토폴로지 제출** 대화 상자의 **Storm 클러스터** 드롭다운 목록에서 HDInsight 클러스터에 있는 Storm을 선택한 후 **제출** 을 선택합니다. **출력** 창을 확인하여 제출 성공 여부를 모니터링할 수 있습니다.
    ...
    토폴로지 제출에 성공하면 클러스터에 대한 **Storm 토폴로지 보기** 창이 나타나게 됩니다. 실행 중인 토폴로지에 대한 정보를 보려면 목록에서 **WordCount** 토폴로지를 선택합니다.

    :::image type="content" source="./media/apache-storm-develop-csharp-visual-studio-topology/storm-topology-view.png" alt-text="Storm 토폴로지 보기 창, HDInsight 클러스터, Visual Studio" border="true":::

    > [!NOTE]  
    > **서버 탐색기** 에서 **Storm 토폴로지** 를 볼 수도 있습니다. **Azure** > **HDInsight** 를 확장하고 HDInsight 클러스터에서 Storm을 마우스 오른쪽 단추로 클릭한 다음, **Storm 토폴로지 보기** 를 선택합니다.

    토폴로지에서 구성 요소에 대한 정보를 보려면 다이어그램에서 구성 요소를 클릭합니다.

1. 토폴로지를 중단하려면 **토폴로지 요약** 에서 **종료** 를 선택합니다.
...
* **Ack**: 토폴로지의 각 Bolt는 `this.ctx.Ack(tuple)`를 호출하여 튜플을 성공적으로 처리했음을 승인할 수 있습니다. 모든 Bolt가 튜플을 승인하면 Spout의 `Ack` 메서드가 호출됩니다. `Ack` 메서드를 통해 Spout가 재생용으로 캐시된 데이터를 제거할 수 있습니다.

* **Fail**: 각 Bolt는 `this.ctx.Fail(tuple)`을 호출하여 튜플 처리에 실패했음을 나타냅니다. 캐시된 메타데이터를 사용하여 튜플을 재생할 수 있는 Spout의 `Fail` 메서드로 오류가 전파됩니다.

* **Sequence ID**: 튜플을 내보낼 때 고유한 시퀀스 ID를 지정할 수 있습니다. 이 값은 재생(`Ack` 및 `Fail`) 처리를 위해 튜플을 식별합니다. 예를 들어 **Storm 샘플** 프로젝트의 Spout는 데이터를 내보낼 때 다음 메서드 호출을 사용합니다.

  `this.ctx.Emit(Constants.DEFAULT_STREAM_ID, new Values(sentence), lastSeqId);`

  이 코드는 `lastSeqId`에 포함된 시퀀스 ID 값을 사용하여 기본 스트림에 대한 문장이 포함된 튜플을 내보냅니다. 이 예제에서 `lastSeqId`은 내보내진 모든 튜플마다 증가합니다.
...
* **Java spout** 및 **C# bolt**: `HybridTopology_javaSpout_csharpBolt`클래스에서 정의됩니다.

  트랜잭션 버전은 `HybridTopologyTx_javaSpout_csharpBolt` 클래스에서 정의됩니다.

* **C# spout** 및 **Java bolt**: `HybridTopology_csharpSpout_javaBolt`클래스에서 정의됩니다.

  트랜잭션 버전은 `HybridTopologyTx_csharpSpout_javaBolt` 클래스에서 정의됩니다.
...
* `JavaComponentConstructor`을 사용하여 spout 또는 볼트에 대한 Java 클래스의 인스턴스를 만듭니다.

* Java 개체에서 JSON으로 java 구성 요소에서 데이터를 직렬화하거나 제외하려면 `microsoft.scp.storm.multilang.CustomizedInteropJSONSerializer`을 사용합니다.

* 서버에 토폴로지를 제출할 때 **Java 파일 경로** 를 지정하려면 **추가 구성** 옵션을 사용해야 합니다. 지정된 경로는 Java 클래스를 포함하는 JAR 파일이 들어있는 디렉터리여야 합니다.

### <a name="azure-event-hubs"></a>Azure Event Hubs

SCP.Net 버전 0.9.4.203은 Event Hub Spout(Event Hubs로부터 읽는 Java Spout)로 작업하기 위한 새로운 클래스와 메서드를 소개합니다. 이벤트 허브 spout (예: **Storm EventHub Reader Sample** 템플릿 사용)를 사용하는 토폴로지를 만들 때는 다음 API를 사용합니다.
...
> [!NOTE]  
> `CustomizedInteropJSONSerializer`을 사용하여 spout에서 생성된 데이터를 직렬화해야 합니다.
...
2. 패키지 관리자에서 **업데이트** 를 선택합니다. SCP.NET 지원 패키지에 대한 업데이트를 사용할 수 있는 경우 나열됩니다. 패키지에 대한 **업데이트** 를 선택한 다음 **변경 미리 보기** 대화 상자에서 **확인** 을 선택하여 설치합니다.

> [!IMPORTANT]  
> NuGet을 사용하지 않는 SCP.NET의 이전 버전으로 프로젝트를 만든 경우 다음 단계를 수행하여 최신 버전으로 업데이트해야 합니다.
>
> 1. **솔루션 탐색기** 에서 프로젝트의 이름을 마우스 오른쪽 단추로 클릭하고 **NuGet 패키지 관리** 를 선택합니다.
> 2. **검색** 필드를 사용하여 검색하고 `Microsoft.SCP.Net.SDK`을 프로젝트에 추가합니다.
...
자세한 내용은 이 문서의 [ConfigurationManager 사용](#use-configurationmanager) 섹션을 참조하세요.

### <a name="systemtypeloadexception"></a>System.TypeLoadException

Linux 기반 HDInsight 클러스터와 C# 토폴로지를 사용하는 경우 다음 오류가 발생할 수 있습니다.
...
토폴로지를 클러스터로 배포하는 것은 쉽지만, 일부 경우에 토폴로지를 로컬로 테스트해야 할 수 있습니다. 이 지침서의 예제 토폴로지를 사용자의 개발 환경에서 로컬로 실행 및 테스트하려면 다음 단계를 참조하세요.

> [!WARNING]  
> 로컬 테스트는 기본 C# 전용 토폴로지에서만 작동합니다. 하이브리드 토폴로지나 여러 스트림을 사용하는 토폴로지에는 로컬 테스트를 사용할 수 없습니다.

1. **솔루션 탐색기** 에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **속성** 을 선택합니다. 프로젝트 속성에서. 그 다음 **출력 형식** 을 **콘솔 애플리케이션** 으로 변경합니다.

   :::image type="content" source="./media/apache-storm-develop-csharp-visual-studio-topology/hdi-output-type-window.png" alt-text="HDInsight Storm 애플리케이션, 프로젝트 속성, 출력 형식" border="true":::

   > [!NOTE]
   > 토폴로지를 클러스터로 배포하기 전에 **출력 유형** 을 **클래스 라이브러리** 로 다시 변경해야 합니다.

1. **솔루션 탐색기** 에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **추가** > **새 항목** 을 선택합니다. **클래스** 를 선택하고 클래스 이름으로 *LocalTest.cs* 를 입력합니다. 마지막으로 **추가** 를 선택합니다.
...
1. `LocalTest` 클래스의 내용으로 다음 코드를 사용합니다.
    ...
    코드 주석을 읽어보세요. 이 코드는 `LocalContext`을 사용하여 개발 환경에서 구성 요소를 실행합니다. 구성 요소 간의 데이터 스트림을 로컬 드라이브의 텍스트 파일로 지속합니다.

1. *Program.cs* 를 열고 `Main` 메서드에 다음 코드를 추가합니다.
...
1. 변경 내용을 저장한 다음 **F5** 를 클릭하거나 **디버그** > **디버깅 시작** 을 선택하여 프로젝트를 시작합니다. 콘솔 창이 나타나며 로그 상태가 테스트로 진행됩니다. `Tests finished`이 표시되면 아무 키나 선택하여 창을 닫습니다.

1. **Windows 탐색기** 를 사용하여 프로젝트가 포함된 디렉터리를 찾습니다. (예: *C: \\ Users \\ \<your_user_name> \\ source \\ repos \\ WordCount \\ WordCount*.) 그런 다음이 디렉터리에서 *Bin* 을 열고 *디버그* 를 선택합니다. 테스트가 실행될 때 생성된 텍스트 파일 *(sentences.txt*, *counter.txt*, *splitter.txt*)이 표시됩니다. 각각의 텍스트 파일을 열고 데이터를 검사합니다.

   > [!NOTE]  
   > 문자열 데이터는 이러한 파일에 10진수 값의 배열로 유지됩니다. 예를 들어, **splitter.txt** 파일의 `[[97,103,111]]`파일은 *ago* 라는 단어입니다.

> [!NOTE]  
> HDInsight의 Storm으로 배포하기 전에 **프로젝트 형식** 을 **클래스 라이브러리** 로 다시 설정해야 합니다.
...
2. 오류가 나열된 구성 요소의 **Spout ID** 또는 **Bolt ID** 를 선택합니다. 세부 정보 페이지는 페이지 하단의 **Errors** 섹션에 추가 오류 정보를 표시합니다.

3. 더 많은 정보를 얻으려면 페이지의 **실행기** 섹션에서 **포트** 를 선택하여 지난 몇 분 동안의 Storm 작업자 로그를 확인하십시오.

### <a name="errors-submitting-topologies"></a>토폴로지 제출 중 오류

HDInsight에 토폴로지를 제출하는 도중에 오류가 발생하면 HDInsight 클러스터에서 토폴로지 제출을 처리하는 서버 쪽 구성 요소에 대한 로그를 찾아볼 수 있습니다. 이러한 로그를 다운로드하려면 명령줄에서 다음 명령을 사용합니다.
...
* 필수적인 Java 종속성이 제출에 포함되지 않습니다.
* 종속성이 호환되지 않습니다.
* 토폴로지 이름이 중복되었습니다.

*hdinsight-scpwebapi.out* 로그 파일에 `FileNotFoundException`이 포함되어 있으면 다음 조건에 따라 예외가 발생할 수 있습니다.

* JDK가 개발 환경의 경로에 없습니다. JDK가 개발 환경에 설치되어 있고 `%JAVA_HOME%/bin`이 경로에 있는지 확인합니다.
* Java 종속성이 누락되었습니다. 제출의 일부로 필수 .jar 파일을 포함시켜야 합니다.
...
* [Azure HDInsight의 Apache Storm에 대한 SCP 프로그래밍 가이드](apache-storm-scp-programming-guide.md)
