...
1. Azure SQL Database를 만듭니다. [Azure Portal에서 Azure SQL Database 만들기](../sql-database/sql-database-single-database-get-started.md)를 참조 하세요.

1. HDInsight 클러스터가 연결 된 Azure SQL Database에 액세스할 수 있도록 하려면 Azure SQL Database 방화벽 규칙을 구성 하 여 Azure 서비스 및 리소스가 서버에 액세스할 수 있도록 해야 합니다. **서버 방화벽 설정**을 선택 하 여 Azure Portal에서이 옵션을 사용 하도록 설정 하 고 **Azure 서비스 및 리소스** **에서** Azure SQL Database 서버 또는 데이터베이스에 대 한이 서버에 액세스할 수 있도록 허용을 선택 합니다. 자세한 내용은 [IP 방화벽 규칙 만들기 및 관리](../sql-database/sql-database-firewall-configure.md#use-the-azure-portal-to-manage-server-level-ip-firewall-rules)를 참조하세요.

1. [쿼리 편집기](../sql-database/sql-database-single-database-get-started.md#query-the-database) 를 사용 하 여 다음 SQL 문을 실행 하 여 각 파이프라인 실행에서 요약 된 데이터를 저장 하는 `dailyflights` 테이블을 만듭니다.
...
1. 다음 명령을 실행 하 여 클러스터에 대 한 SSH 터널을 엽니다. 여기서 `CLUSTERNAME`은 클러스터의 이름입니다.
...
1. `http://headnodehost:8080`로 이동 하 여 Ambari에 로그인 합니다.
...
1. 로컬 워크스테이션에서 `job.properties`라는 파일을 만듭니다. 아래 텍스트를 파일의 시작 콘텐츠로 사용 합니다.
그런 다음 특정 환경에 대 한 값을 업데이트 합니다. 텍스트 아래 표에는 각 속성이 요약 되어 있으며, 사용자 환경에 대 한 값을 찾을 수 있는 위치를 나타냅니다.
    ...
    | 년 | 항공편 요약이 계산되는 날짜의 연도 구성 요소입니다. 있는 그대로 둡니다. |
    | month | 항공편 요약이 계산되는 날짜의 월 구성 요소입니다. 있는 그대로 둡니다. |
    | 일 | 항공편 요약이 계산되는 날짜의 일 구성 요소입니다. 있는 그대로 둡니다. |

1. 로컬 워크스테이션에서 `hive-load-flights-partition.hql`라는 파일을 만듭니다. 아래 코드를 파일의 내용으로 사용 합니다.
    ...
    Oozie 변수는 구문 `${variableName}`을 사용합니다. 이러한 변수는 `job.properties` 파일에 설정 됩니다. Oozie는 런타임에 실제 값을 대체합니다.

1. 로컬 워크스테이션에서 `hive-create-daily-summary-table.hql`라는 파일을 만듭니다. 아래 코드를 파일의 내용으로 사용 합니다.
...
1. 로컬 워크스테이션에서 `workflow.xml`라는 파일을 만듭니다. 아래 코드를 파일의 내용으로 사용 합니다. 위의 이러한 단계는 Oozie 워크플로 파일에서 별도의 작업으로 표현 됩니다.
...
두 Hive 쿼리는 Azure Storage의 경로에 의해 액세스 되며 나머지 변수 값은 `job.properties` 파일에서 제공 됩니다. 이 파일은 2017 년 1 월 3 일에 실행 되도록 워크플로를 구성 합니다.

## <a name="deploy-and-run-the-oozie-workflow"></a>Oozie 워크플로 배포 및 실행

Bash 세션에서 SCP를 사용 하 여 Oozie 워크플로 (`workflow.xml`), Hive 쿼리 (`hive-load-flights-partition.hql` 및 `hive-create-daily-summary-table.hql`) 및 작업 구성 (`job.properties`)을 배포 합니다.  Oozie에서는 `job.properties` 파일만 헤드 노드의 로컬 스토리지에 있을 수 있습니다. 다른 모든 파일은 HDFS(이 경우 Azure Storage)에 저장되어야 합니다. 워크플로에서 사용되는 Sqoop 작업은 SQL Database와 통신하기 위한 JDBC 드라이버에 따라 달라지며, 헤드 노드에서 HDFS로 복사되어야 합니다.
...
1. 로컬 헤드 노드에서 HDFS의 workflow 폴더로 `mssql-jdbc-7.0.0.jre8.jar`를 복사 합니다. 클러스터에 다른 jar 파일이 포함 되어 있는 경우 필요에 따라 명령을 수정 합니다. 다른 jar 파일을 반영 하기 위해 필요에 따라 `workflow.xml`을 수정 합니다. 열려 있는 ssh 세션에서 다음 명령을 실행 합니다.
...
* 핵심 사항 1: `coordinator-app` 요소 자체의 `start` 및 `end` 특성이 코디네이터가 실행되는 시간 간격을 제어합니다.
    ...
    코디네이터는 `frequency` 특성으로 지정된 간격에 따라, `start` 및 `end` 데이터 범위 내에서 작업을 예약합니다. 예약된 각 작업은 구성된 대로 워크플로를 다시 실행합니다. 위의 코디네이터 정의에서 코디네이터는 2017 년 1 월 1 일에서 2017 년 1 월 5 일로 작업을 실행 하도록 구성 되어 있습니다. 빈도는 [Oozie 식 언어](https://oozie.apache.org/docs/4.2.0/CoordinatorFunctionalSpec.html#a4.4._Frequency_and_Time-Period_Representation) frequency 식 `${coord:days(1)}`1 일로 설정 됩니다. 이로 인해 코디네이터는 작업(및 워크플로)이 하루에 1번 실행되도록 예약합니다. 이 예제에서와 같이, 이전 날짜 범위에서는 작업이 지연 없이 실행되도록 예약됩니다. 작업이 실행되도록 예약된 날짜 시작을 *명목 시간*이라고 합니다. 예를 들어 2017 년 1 월 1 일에 대 한 데이터를 처리 하기 위해 코디네이터는 명목상 시간 2017-01-01T00:00:00 GMT로 작업을 예약 합니다.
