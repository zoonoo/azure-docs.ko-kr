...
다음 워크플로는 영화 데이터를 사용하는 매우 단순한 예제입니다.
...
`moviedb.txt`(영화에 대한 정보) 및 `user-ratings.txt`의 두 가지 파일이 있습니다. user-ratings.txt 파일은 분석 중에 사용되지만 moviedb.txt는 분석 결과를 표시할 때 사용자에게 친숙한 텍스트 정보를 제공하는 데 사용됩니다.
...
3. 다음 명령을 사용하여 권장 출력의 데이터에 대한 영화 이름을 찾는 새 Python 스크립트를 만듭니다.
...
5. Python 스크립트를 실행합니다. 다음 명령에서는 모든 파일이 다운로드된 디렉터리에 있다고 가정합니다.
   ...
   * **user-ratings.txt** 파일은 사용자가 등급을 평가한 영화를 검색하는 데 사용됩니다.
