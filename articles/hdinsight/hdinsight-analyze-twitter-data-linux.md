...
Twitter를 사용하여 [각 트윗에 대한 데이터](https://dev.twitter.com/docs/platform-objects/tweets) 를 REST API를 통해 JSON(JavaScript Notation) 개체로서 검색할 수 있습니다. [OAuth](http://oauth.net) 는 API에 대한 인증을 필요로 합니다. API에 액세스하는데 사용되는 설정이 포함된 _Twitter 응용 프로그램_ 도 만들어야 합니다.
...
5. Beeline 프롬프트에서 다음을 사용하여 __twitter.hql__ 파일의 HiveQL에서 만든 __트윗__ 테이블에서 데이터를 선택할 수 있음을 확인합니다.
    ...
    메시지 텍스트에 단어 __Azure__ 를 포함하는 최대 10개의 트윗을 반환합니다.
