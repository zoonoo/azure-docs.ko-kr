...
> [AZURE.NOTE]이 문서 내의 단계는 Linux 기반 HDInsight 클러스터 사용을 기반으로 합니다. Windows 기반 클러스터에 대한 특정 단는 [HDInsight에서 Hive를 사용하여 Twitter 데이터 분석](hdinsight-analyze-twitter-data.md)을 참조하세요.
...
4. 다음 명령을 사용하여 파일에 포함된 HiveQL을 실행합니다.
	...
	Hive 셸을 로드하고 __twitter.hql__ 파일에서 HiveQL을 실행하고 마지막으로 `hive >` 프롬프트를 반환합니다.
	
5. `hive >` 프롬프트에서 다음을 사용하여 __twitter.hql__ 파일의 HiveQL에서 만든 __트윗__ 테이블에서 데이터를 선택할 수 있음을 확인합니다.
