...
`azuredeploy.parameters.json`의 매개 변수를 편집 하 여 새 클러스터에 대 한 정보와 Ambari를 보관할 데이터베이스를 지정 합니다.

Azure CLI를 사용 하 여 배포를 시작할 수 있습니다. `<RESOURCEGROUPNAME>`를 클러스터를 배포 하려는 리소스 그룹으로 바꿉니다.
