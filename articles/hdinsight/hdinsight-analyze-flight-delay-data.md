...
다이어그램의 숫자는 섹션 제목에 해당합니다. **M** 은 주 프로세스의 약어입니다. **A** 는 부록 콘텐츠의 약어입니다.
...
> [!NOTE]  
> 이 문서의 단계는 Windows 기반 HDInsight 클러스터를 대상으로 합니다. Linux 기반 클러스터를 사용하는 단계는 [HDInsight에서 Apache Hive를 사용하여 비행 지연 데이터 분석(Linux)](hdinsight-analyze-flight-delay-data-linux.md)을 참조하세요.
...
HDInsight 클러스터를 만들고 Hive 작업을 실행하는 방법에 대한 자세한 내용은 [HDInsight에서 Apache Hadoop 클러스터 만들기][hdinsight-provision] 및 [HDInsight에서 Apache Hive 사용][hdinsight-use-hive]을 참조하세요.

**Azure PowerShell로 Hive 쿼리를 실행하려면**

1. [부록 C](#appendix-c)의 지침에 따라 Azure SQL 데이터베이스 및 Sqoop 작업 출력 테이블을 만듭니다.
2. Windows PowerShell ISE를 열고 다음 스크립트를 실행합니다.
    ...
    ```
3. SQL 데이터베이스에 연결하고 AvgDelays 테이블에서 도시별 평균 비행 지연을 확인합니다.
...
3. 다음 스크립트를 복사하여 스크립트 창에 붙여 넣습니다.  
   ...
   > [!NOTE]  
   > 스크립트는 REST(representational state transfer) 서비스 https://bot.whatismyipaddress.com을 사용하여 외부 IP 주소를 검색합니다. IP 주소는 SQL 데이터베이스 서버용 방화벽 규칙을 만드는 데 사용됩니다.

    스크립트에서 사용되는 일부 변수는 다음과 같습니다.

   * **$ipAddressRestService** - 기본값은 https://bot.whatismyipaddress.com입니다. 외부 IP 주소를 가져오기 위한 공용 IP 주소 REST 서비스입니다. 원하는 경우 다른 서비스를 사용할 수 있습니다. 서비스를 통해 검색된 외부 IP 주소를 사용하여 Azure SQL 데이터베이스 서버용 방화벽 규칙을 만듭니다. 따라서 Windows PowerShell 스크립트를 사용하여 워크스테이션에서 데이터베이스에 액세스할 수 있습니다.
...
이제 파일을 Azure Blob Storage로 업로드하는 방법, Azure Blob Storage의 데이터를 사용하여 Apache Hive 테이블을 채우는 방법, Hive 쿼리를 실행하는 방법, Sqoop을 사용하여 [Apache Hadoop HDFS](https://hadoop.apache.org/docs/r1.2.1/hdfs_design.html)의 데이터를 Azure SQL 데이터베이스로 내보내는 방법을 이해했습니다. 자세한 내용은 다음 문서를 참조하세요.
