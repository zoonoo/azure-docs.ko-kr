...
description: Ambari를 사용하여 Azure HDInsight에서 Hadoop 클러스터를 모니터링하고 관리하는 방법에 대해 알아봅니다. 이 문서에서는 HDInsight 클러스터에 포함 된 Ambari REST API를 사용 하는 방법에 대해 알아봅니다.
...
## <a name="what-is-apache-ambari"></a>Apache Ambari 이란?
...
클러스터 이름의 실제 대/소문자는 기대한 것과 다를 수 있습니다.  여기에서 설명 하는 단계는 실제 대/소문자를 표시 한 다음 나중에 모든 예제에 대 한 변수에 저장 합니다.
...
클러스터 노드의 FQDN (정규화 된 도메인 이름)을 알아야 할 수 있습니다. 다음 예제를 사용하여 클러스터의 다양한 노드에 대한 FQDN을 쉽게 검색할 수 있습니다.
...
이 섹션의 예제에서 반환 된 IP 주소는 인터넷을 통해 직접 액세스할 수 없습니다. HDInsight 클러스터를 포함 하는 Azure Virtual Network 내 에서만 액세스할 수 있습니다.

HDInsight 및 가상 네트워크 작업에 대 한 자세한 내용은 [hdinsight에 대 한 가상 네트워크 계획](hdinsight-plan-virtual-network-deployment.md)을 참조 하세요.

IP 주소를 찾으려면 클러스터 노드의 내부 FQDN(정규화된 도메인 이름)을 알아야 합니다. FQDN을 알고 있으므로 호스트의 IP 주소를 얻을 수 있습니다. 다음 예에서는 먼저 Ambari 모든 호스트 노드의 FQDN에 대해 쿼리를 수행 합니다. 그런 다음 쿼리는 각 호스트의 IP 주소를 Ambari 합니다.
...
HDInsight 클러스터는 Azure Storage 계정 또는 Data Lake Storage를 기본 저장소로 사용 해야 합니다. 클러스터를 만든 후 Ambari를 사용하여 이 정보를 검색할 수 있습니다. 예를 들어 HDInsight 외부 컨테이너에 데이터를 읽고 쓰려는 경우가 여기에 해당합니다.
...
이 예제에서는 설치 된 구성 요소에 대 한 현재 구성이 포함 된 JSON 문서를 반환 합니다. *태그* 값을 참조 하세요. 다음 예제는 Spark 클러스터 형식에서 반환된 데이터에서 발췌한 것입니다.
...
관심이 있는 구성 요소에 대 한 구성을 가져옵니다. 다음 예제에서는 이전 요청에서 반환된 태그 값으로 `INITIAL`을 바꿉니다.
...
1. `newconfig.json`를 만듭니다.  
   ...
   * `livy2-conf`새 구성 요소로 대체 합니다.
   * `INITIAL` `tag` [모든 구성 가져오기](#get-all-configurations)에서 검색 된 실제 값으로 대체 합니다.
     ...
     **B. PowerShell**  
     PowerShell 스크립트는 [jq](https://stedolan.github.io/jq/)를 사용 합니다.  `C:\HD\jq\jq-win64`다음을 편집 하 여 실제 경로와 [jq](https://stedolan.github.io/jq/)의 버전을 반영 합니다.
     ...
     Jq는 HDInsight에서 검색한 데이터를 새 구성 템플릿으로 반환하는 데 사용됩니다. 특히 이러한 예제에서는 다음 작업을 수행 합니다.

   * 문자열 "version" 및 날짜를 포함하는 고유 값을 만듭니다. 이 값은 `newtag`에 저장됩니다.

   * 새 구성에 대 한 루트 문서를 만듭니다.
...
3. 제출 `newconfig.json` .  
   다음 명령을 사용하여 업데이트된 구성을 Ambari에 제출합니다.
    ...
    이러한 명령은 파일의 **newconfig.js** 내용을 새 구성으로 클러스터에 제출 합니다. 이 요청은 JSON 문서를 반환합니다. 이 문서의 **versionTag** 요소는 제출한 버전과 일치해야 하며, **configs** 개체에는 요청한 구성 변경 내용이 포함됩니다.

### <a name="restart-a-service-component"></a>서비스 구성 요소 다시 시작

이 시점에서 Ambari 웹 UI는 새 구성을 적용 하기 위해 Spark 서비스를 다시 시작 해야 함을 나타냅니다. 다음 단계를 사용하여 서비스를 다시 시작합니다.
...
4. 요청을 확인 합니다.  
    `29`이전 단계에서 반환 된에 대 한 실제 값으로 대체 하 여 아래 명령을 편집 `id` 합니다.  다음 명령은 요청의 상태를 검색합니다.
