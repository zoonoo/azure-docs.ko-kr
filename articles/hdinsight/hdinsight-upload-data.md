...
Azure HDInsight는 Azure Storage 및 Azure Data Lake Store(Gen1 및 Gen2)를 통해 모든 기능을 갖춘 HDFS(Hadoop Distributed File System)를 제공합니다. Azure Storage, Data Lake Store Gen1 및 Gen2는 고객에게 원활한 환경을 제공하기 위해 HDFS 확장으로 설계되었습니다. 이를 통해 Hadoop 에코시스템에서 구성 요소의 전체 집합이 관리하는 데이터에서 직접 작동하도록 합니다. Azure Storage, Data Lake Store Gen1 및 Gen2는 데이터 스토리지 및 해당 데이터의 계산에 최적화된 별개의 파일 시스템입니다. Azure 스토리지 사용의 이점에 대한 자세한 내용은 [HDInsight를 사용한 Azure 스토리지 사용,](hdinsight-hadoop-use-blob-storage.md) [HDInsight를 사용한 데이터 레이크 스토리지 Gen1 사용,](hdinsight-hadoop-use-data-lake-store.md) [HDInsight의 데이터 레이크 스토리지 Gen2 사용](hdinsight-hadoop-use-data-lake-storage-gen2.md)을 참조하십시오.
...
## <a name="hadoop-command-line"></a><a id="commandline"></a>하두프 커맨드 라인
...
| [Azure 저장소 탐색기](../storage/blobs/storage-quickstart-blobs-storage-explorer.md) |✔ |✔ |✔ |
...
### <a name="storage-exception-for-write-on-blob"></a><a id="storageexception"></a>Blob에서 쓰기를 위한 스토리지 예외
...
1. 브라우저에서 클러스터에 대한 Ambari 웹 UI로 이동합니다. 클러스터 `https://CLUSTERNAME.azurehdinsight.net`의 `CLUSTERNAME` 이름은 다음과 다.
