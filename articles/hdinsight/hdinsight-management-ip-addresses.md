...
description: Azure HDInsight를 사용하여 가상 네트워킹을 위해 네트워크 보안 그룹 및 사용자 정의 경로를 적절하게 구성하려면 인바운드 트래픽을 허용해야 하는 IP 주소를 알아봅니다.
...
> [!Important]
> 대부분의 경우 이제 IP 주소를 수동으로 추가하는 대신 네트워크 보안 그룹에 [서비스 태그](hdinsight-service-tags.md) 기능을 사용할 수 있습니다. 새 영역은 서비스 태그에 대해서만 추가되고 정적 IP 주소는 결국 더 이상 사용되지 않습니다.

HDInsight 클러스터에 대한 인바운드 트래픽을 제어하기 위해 NSG(네트워크 보안 그룹) 또는 사용자 정의 경로(DR)를 사용하는 경우 클러스터가 중요한 Azure 상태 및 관리 서비스와 통신할 수 있는지 확인해야 합니다.  이러한 서비스의 IP 주소 중 일부는 지역별로 다르며 일부는 모든 Azure 지역에 적용됩니다. 사용자 지정 DNS를 사용하지 않는 경우에도 Azure DNS 서비스의 트래픽을 허용해야 할 수도 있습니다.

다음 섹션에서는 허용해야 하는 특정 IP 주소에 대해 설명합니다.

## <a name="azure-dns-service"></a>Azure DNS 서비스

Azure에서 제공하는 DNS 서비스를 사용하는 경우 포트 53에서 __168.63.129.16에서__ 액세스를 허용합니다. 자세한 내용은 [VM 및 역할 인스턴스](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md) 문서에 대한 이름 확인을 참조하십시오. 사용자 지정 DNS를 사용하는 경우 이 단계를 건너뜁니다.

## <a name="health-and-management-services-all-regions"></a>보건 및 관리 서비스: 모든 지역

모든 Azure 지역에 적용되는 Azure HDInsight 상태 및 관리 서비스에 대해 다음 IP 주소의 트래픽을 허용합니다.
...
## <a name="health-and-management-services-specific-regions"></a>보건 및 관리 서비스: 특정 지역

리소스가 있는 특정 Azure 지역의 Azure HDInsight 상태 및 관리 서비스에 대해 나열된 IP 주소의 트래픽 허용:

> [!IMPORTANT]  
> 사용 중인 Azure 지역이 나열되지 않은 경우 네트워크 보안 그룹에 [서비스 태그](hdinsight-service-tags.md) 기능을 사용합니다.
...
자세한 내용은 네트워크 [트래픽 제어](hdinsight-plan-virtual-network-deployment.md#networktraffic) 섹션을 참조하십시오.

사용자 정의 경로(DRS)를 사용하는 경우 경로를 지정하고 다음 홉을 "인터넷"으로 설정한 다음 홉을 사용하여 가상 네트워크에서 위 IP로의 아웃바운드 트래픽을 허용해야 합니다.

## <a name="next-steps"></a>다음 단계

* [Azure HDInsight 클러스터를 위한 가상 네트워크 만들기](hdinsight-create-virtual-network.md)
* [Azure HDInsight에 대한 NSG(네트워크 보안 그룹) 서비스 태그](hdinsight-service-tags.md)
