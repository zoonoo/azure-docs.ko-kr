...
이 자습서는 이전 자습서인 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-xamarin-android)의 단계 및 샘플 앱을 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-xamarin-android)을 완료해야 합니다.
...
1.  Xamarin Studio에서 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-xamarin-android)(영문) 자습서를 완료했을 때 생성된 프로젝트를 엽니다.
...
1.  **TodoItem** 클래스의 전용 필드 정의에 다음 코드를 추가합니다.
    ...
    **참고**
    ...
    `DataMember's Name` 주석은 앱의 새 `CreatedAt` 속성을 TodoItem 테이블에 정의된 `createdAt` 열(이름이 다름)에 매핑하도록 클라이언트에 지시합니다. 이 주석을 사용하면 앱이 SQL 데이터베이스의 열 이름과 다른 개체 속성 이름을 가질 수 있습니다. 이 주석을 사용하지 않는 경우 대/소문자 차이 때문에 오류가 발생합니다.

2.  GetView 메서드에서 `checkBox.Text`를 `currentItem.Text`로 설정하는 현재 코드 바로 위에 다음 코드를 추가합니다.
...
5.  **TodoActivity.cs**에서 **RefreshItemsFromTableAsync**의 기존 쿼리를 다음 쿼리로 바꿉니다.
...
6.  **Run** 메뉴에서 **Run**을 클릭하여 앱을 시작합니다.
...
이 자습서를 완료했으므로 이제 데이터 시리즈의 마지막 자습서인 [페이징을 사용하여 쿼리 구체화](/en-us/develop/mobile/tutorials/add-paging-to-data-xamarin-android)를 검토해 보십시오.

서버 스크립트는 사용자를 인증할 때 및 푸시 알림을 보내기 위해서도 사용됩니다. 자세한 내용은 다음 자습서를 참조하십시오.

-   [스크립트를 통해 사용자 권한 부여](/en-us/develop/mobile/tutorials/authorize-users-in-scripts-xamarin-android)
