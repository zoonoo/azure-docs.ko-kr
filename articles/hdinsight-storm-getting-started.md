...
5. 이 클러스터에 사용할 **데이터 노드**의 수를 입력하고 이 클러스터를 만들 **지역/가상 네트워크**를 입력한 후에 화살표를 선택하여 계속합니다.
...
3. Hadoop 명령줄에서 다음 명령을 사용하여 디렉터리를 Storm 명령이 포함된 디렉터리로 변경합니다.

	cd %storm_home%\bin

4. 사용 가능한 명령 목록을 보려면 매개 변수 없이 `storm`을 입력합니다.
...
> [WACOM.NOTE] 예제가 포함된 JAR 파일의 버전은 주기적으로 변경될 수 있습니다. 그러므로 이 명령을 실행할 때는 클러스터와 함께 제공된 파일의 버전을 지정하세요.
...
5. 페이지에서 **spout** 링크를 선택하고, **내보냄** 및 **전송됨** 열에 0보다 큰 수가 있는 **Executor(모든 시간)** 목록의 항목 중 하나에 대한 **포트** 번호를 선택합니다.
