...
> [AZURE.NOTE] 이 작업의 절차는 Azure 클라우드 서비스에 적용됩니다. 웹 사이트의 경우 [Azure 웹 사이트에 대해 SSL 인증서 구성](../articles/web-sites-configure-ssl-certificate/)을 참조하세요.
...
테스트 용도로 자체 서명된 인증서를 만들어 사용할 수 있습니다. 자체 서명된 인증서는 CA를 통해 인증되지 않으며 cloudapp.net 도메인을 웹 사이트 URL로 사용할 수 있습니다. 예를 들어 아래 작업에서는 인증서에서 사용되는 CN(일반 이름)이 **sslexample.cloudapp.net**인 자체 서명된 인증서를 사용합니다. IIS 관리자를 사용하여 자체 서명된 인증서를 만드는 방법에 대한 자세한 내용은 [역할에 대한 인증서를 만드는 방법][]을 참조하세요.
...
1.  개발 환경에서 서비스 정의 파일(CSDEF)을 열어
    **WebRole**섹션 내에서 **인증서** 섹션을 추가하여
    인증서에 대한 다음 정보를
    포함합니다.
    ...
    **인증서** 섹션에서는 인증서의 이름, 위치 및 인증서가 위치한 저장소의 이름을 정의합니다. 여기서는 인증서를 CA(인증 기관) 저장소에 저장하도록 선택했지만 다른 옵션을 선택할 수도 있습니다. 자세한 내용은 [인증서를 서비스와 연결하는 방법][]을 참조하세요.

2.  서비스 정의 파일에서 HTTPS:를 활성화하도록 **끝점** 섹션의
    **끝점 입력 요소**를 추가합니다.
...
3.  서비스 정의 파일에서 **사이트** 섹션의
    **바인딩** 요소를 추가합니다. 그러면 HTTPS 바인딩이 추가되어
    ...
    서비스 정의 파일에 필요한 모든 변경 사항이 
    완료되었지만 서비스 구성 파일에 인증서 정보를
    추가해야 합니다.

4.  서비스 구성 파일(CSCFG,ServiceConfiguration.Cloud.cscfg)에서 **역할** 섹션의
    **인증서** 섹션을 추가하여 샘플 지문 값을 대체하면
    인증서와 함께 아래에 표시됩니다.
...
이제 서비스 정의 및 서비스 구성 파일이 업데이트되었으므로,
Azure에 업로드딩할 배포 패키지를 만들 수 있습니다. If
방금 입력한 인증 정보를 추가 기입한 것처럼 **cspack**을
사용하는 경우에는 **/generateConfigurationFile** 플래그를
사용하지 마십시오.

<h2><a name="step3"> </a>3단계: 배포 패키지 및 인증서 업로드</h2>

배포 패키지가 인증서를 사용하도록 업데이트되었으며
HTTPS 끝점이 추가되었습니다. 이제 패키지를 업로드하거나
Azure 관리 포털에 인증서를 제공할 수 있습니다.

1. [Azure 관리 포털][]에 로그인합니다. 
2. **새로 만들기**, **클라우드 서비스** 및 **사용자 지정 만들기**를 차례로 클릭합니다.
3. **클라우드 서비스 만들기** 대화 상자에서 URL, 지역/선호도 그룹 및 구독에 대한 값을 입력합니다. **클라우드 서비스 패키지를 배포**가 선택되어 있는지 확인하고 **다음** 단추를 클릭합니다.
...
4.  **다음** 단추를 클릭합니다.
5.  **인증서 추가** 대화 상자에서 SSL에 대한 위치를 입력합니다.
    인증서.pfx 파일을 인증서에 대한 암호 및
    **인증서를 첨부**를 클릭합니다.  

    ![Add certificate][1]

6.  **인증서가 첨부**된 인증서 섹션에 나열되는지 확인합니다.
...
2.  웹 브라우저에서 **http** 대신 **https**를 사용하도록 링크를 수정한 다음 페이지를 방문합니다.
    ...
    **참고:** 자체 서명된 인증서를 사용하는 경우,
    자체 서명된 인증서와 관련된 HTTPS 끝점을
    찾으면 브라우저에서 인증서 오류를 볼 수 있습니다. 신뢰할 수 있는
...
* [HTTPS 끝점에서 SSL 인증서를 구성하는 방법][]
  ...
  [역할에 대한 인증서를 만드는 방법]: http://msdn.microsoft.com/library/windowsazure/gg432987.aspx
