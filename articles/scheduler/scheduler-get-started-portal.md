...
## Azure 관리 포털을 사용하여 작업을 쉽고 빠르게 만들 수 있게 Azure 스케줄러 구성

이 자습서를 완료하려면 Azure 스케줄러 기능이 사용되는 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 평가판](https://msdn.microsoft.com/library/)을 참조하세요.

## 시작

Azure 관리 포털을 사용하면 Azure 스케줄러에서 쉽게 작업 및 작업 컬렉션을 만들 수 있습니다. 이 자습서에서는 작업 저장하는 데 사용할 작업 컬렉션 만들기, 작업 컬렉션에 작업 만들기, 관리 포털에서 사용 가능한 작업 모니터링 및 관리 작업 개요를 단계별로 안내합니다. 이전에 Azure를 사용한 경험이 있어야 이 자습서를 사용할 수 있습니다.

Azure 관리 포털을 처음으로 열면 **모든 항목** 탭이 자동으로 표시됩니다. **모든 항목** 탭의 열은 정렬할 수 있습니다. 스케줄러 작업 및 작업 컬렉션을 보려면**스케줄러** 탭을 클릭합니다.
...
3.  **작업 컬렉션**에서, 이미 작업 컬렉션을 만들었으며 **작업 컬렉션** 드롭다운에서 기존 작업 컬렉션 이름을 선택하여 이 작업을 해당 작업 컬렉션에 추가하려는 경우 기존 작업 컬렉션을 선택합니다. 작업을 추가할 기존 작업 컬렉션이 없으면 **새로 만들기**를 클릭하고 새 작업 컬렉션을 식별하는 이름을 입력합니다.<br /><br /> ![][3]
...
7.  다음 단계에서 일정을 정의합니다. 작업은 일회성 작업으로 정의할 수 있지만 되풀이 일정을 선택해 보겠습니다. 이 자습서의 일부 스크린샷은 단지 설명을 위해 1분 되풀이를 표시하지만 12시간 되풀이를 선택합니다.

    1.  **되풀이 간격:** ` 12 Hours`  

    2.  **시작:** ` Now`

    3.  **끝:** ` Select date 2 days after current day and any time`

   	![][5]

8.  마지막으로 확인을 클릭합니다.

9.  작업 컬렉션 및 작업을 만드는데 시간이 걸릴 수 있습니다. 상태를 확인하려면 포털 화면의 아래쪽에서 알림을 모니터링할 수 있습니다.
...
작업 컬렉션을 만든 후에는 주 스케줄러 관리 화면에 표시됩니다.
...
**기록 세부 정보 보기**를 클릭하여 각 실행으 전체 응답을 살펴볼 수 있습니다. 이 대화 상자에서는 응답을 클립보드에 복사할 수도 있습니다.
...
작업 이름을 클릭하면 작업을 활성화, 비활성화 또는 삭제하는 옵션이 제공됩니다. 삭제한 작업은 복구하지 못할 수 있습니다.
