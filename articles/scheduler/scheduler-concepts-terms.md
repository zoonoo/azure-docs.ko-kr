...
* 기존 서비스 또는 워크로드에 대한 엔드포인트가 있는 경우 TLS를 지원하는 HTTPS 작업을 포함한 HTTP 작업
...
| [**작업**](#action) | 예 | 기본 동작의 세부 정보로, **errorAction** 개체를 포함할 수 있습니다. | 
| [**오류 작업**](#error-action) | 예 | 기본 동작이 실패하는 경우 실행되는 보조 동작에 대한 세부 정보 |
| [**되풀이**](#recurrence) | 예 | 되풀이 작업에 대한 빈도 및 간격 등의 세부 정보 | 
| [**다시 시도정책**](#retry-policy) | 예 | 동작을 다시 시도하는 빈도에 대한 세부 정보 | 
| [**상태**](#state) | 예 | 작업의 현재 상태에 대한 세부 정보 |
| [**상태**](#status) | 예 | 서비스에 의해 제어되는 작업의 현재 상태에 대한 세부 정보 |
||||
...
| **간격** | 예 | 1에서 1000(포함) 사이 | **frequency**에 따른 각 발생 간의 시간 단위 수를 결정하는 양의 정수 | 
...
| **분** | 예 | 0 ~ 59 | 작업 실행 시기에 대한 분 표식이 포함된 배열 | 
| **개월** | 예 | 1 - 12 | 작업 실행 시기에 대한 월 표식이 포함된 배열 | 
| **monthDays** | 예 | 다양함 | 작업 실행 시기에 대한 날짜 표식이 포함된 배열 | 
| **평일** | 예 | "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday" | 작업 실행 시기에 대한 요일 표식이 포함된 배열 | 
| **count** | 예 | <*없음*> | 되풀이 횟수입니다. 기본값은 무한 반복입니다. **count** 및 **endTime**을 둘 다 사용할 수 없지만 먼저 완료되는 규칙이 적용됩니다. | 
| **Endtime** | 예 | <*없음*> | 되풀이를 중지할 날짜 및 시간입니다. 기본값은 무한 반복입니다. **count** 및 **endTime**을 둘 다 사용할 수 없지만 먼저 완료되는 규칙이 적용됩니다. | 
||||
...
| **다시 시도 간격** | 예 | PT30S | 재시도 사이의 간격 및 빈도를 [ISO 8601 형식](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations)으로 지정합니다. 최솟값은 15초이고 최댓값은 18개월입니다. | 
