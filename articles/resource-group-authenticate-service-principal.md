---
title: PowerShell을 사용하여 Azure 서비스 주체 만들기 | Microsoft Docs
...
4. Active Directory 응용 프로그램의 응용 프로그램 ID를 전달하여 응용 프로그램에 대한 서비스 주체를 만듭니다.
        ...
        New-AzureRmADServicePrincipal -ApplicationId $app.ApplicationId
5. 서비스 사용자에게 구독에 대한 권한을 부여합니다. 이 예제에서는 구독에서 모든 리소스를 읽을 수 있는 **읽기 권한자** 역할에 서비스 주체를 추가합니다. 다른 역할에 대해서는 [RBAC: 기본 제공 역할](active-directory/role-based-access-built-in-roles.md)을 참조하세요. **ServicePrincipalName** 매개 변수의 경우 응용 프로그램을 만들 때 사용한 **ApplicationId**를 제공합니다.
