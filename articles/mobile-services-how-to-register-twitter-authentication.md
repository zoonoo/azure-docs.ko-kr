...
1.  <a href="http://go.microsoft.com/fwlink/p/?LinkId=268300" target="_blank">Twitter 개발자</a> (영문) 웹 사이트로 이동하고 Twitter 계정 자격 증명으로 로그인하고 **Create a new application**을 클릭합니다.

   	![][1]

2. 앱에 대한 **Name**, **Description** 및 **Website** 값을 입력한 후 **Callback URL**에 모바일 서비스 URL에 _/login/twitter_ 경로를 추가한 값을 입력합니다.

	>[WACOM.NOTE]Visual Studio를 사용하여 Azure에 게시된 .NET 백 엔드 모바일 서비스의 경우 리디렉션 URL은 모바일 서비스 URL에 모바일 서비스를 .NET 서비스로 사용한 _signin-twitter_ 경로를 추가한 것입니다(예: <code>https://todolist.azure-mobile.net/signin-twitter</code>).
...
6. **Consumer key** 및 **Consumer secret**의 값을 기록해 둡니다. 
	...
	<p>소비자 암호는 중요한 보안 자격 증명입니다. 다른 사람과 이 암호를 공유하거나 앱과 함께 배포하지 마세요.</p>
    </div>
