...
Web API는 .NET 개발자가 REST API를 생성하기 위한 최상의 방법을 제공하는 오픈 소스 프레임워크입니다. Web API 솔루션은 Azure 웹 사이트 또는 .NET 백 엔드를 사용하는 Azure 모바일 서비스에서 호스트하거나 사용자 지정 프로세스로 직접 호스트할 수도 있습니다. 모바일 서비스는 모바일 앱을 위해 특별히 디자인된 호스팅 환경입니다. 모바일 서비스에서 Web API 서비스를 호스트할 때는 데이터 저장소 외에도 다음과 같은 이점을 얻을 수 있습니다.
...
이 자습서에서는 [Visual Studio 2013 업데이트 3](http://go.microsoft.com/fwlink/p/?LinkID=390465)이 사용됩니다. 
...
Visual Studio 2013 업데이트 3에서 ASP.NET 웹 응용 프로그램 프로젝트에는 이제 Microsoft Azure 모바일 서비스에 대한 템플릿이 포함됩니다. 이 템플릿을 선택하고 **확인**을 클릭합니다.
...
또한 메서드를 별도의 컨트롤러에 배치할 수도 있습니다. 어느 쪽이든 특별한 이점은 없으며 코드를 구성하려는 방법에 따라 방식이 결정됩니다.
**[Route]** 특성에 대한 자세한 내용은 [Web API의 특성 라우팅](http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2)을 참조하세요.
