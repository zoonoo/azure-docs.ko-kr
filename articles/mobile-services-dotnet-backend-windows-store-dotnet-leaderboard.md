...
이 자습서에서는 .NET 백 엔드와 함께 Azure 모바일 서비스를 사용해서 Windows 스토어 앱을 작성하는 방법을 보여 줍니다. Azure 모바일 서비스는 모바일 앱 작성을 위한 크로스 플랫폼 클라이언트 라이브러리와 함께 기본 제공되는 인증, 모니터링, 푸시 알림 및 기타 기능이 포함된 확장 가능한 보안 백 엔드를 제공합니다. 모바일 서비스를 위한 .NET 백 엔드는 [ASP.NET Web API][ASP.NET Web API]를 기반으로 하며, .NET 개발자가 REST API를 생성하기 위한 최상의 방법을 제공합니다.

+ [개요][개요]
+ [샘플 앱 정보][샘플 앱 정보]
+ [데이터 모델 추가][데이터 모델 추가]
+ [Web API 컨트롤러 추가][Web API 컨트롤러 추가]
+ [DTO를 사용하여 관련 엔터티 반환][DTO를 사용하여 관련 엔터티 반환]
+ [점수 제출을 위한 사용자 지정 API 정의][점수 제출을 위한 사용자 지정 API 정의]
+ [Windows 스토어 앱 만들기][Windows 스토어 앱 만들기]
+ [모델 클래스 추가][모델 클래스 추가]
+ [뷰 모델 만들기][뷰 모델 만들기]
+ [MobileServiceClient 인스턴스 추가][MobileServiceClient 인스턴스 추가]
+ [기본 페이지 만들기][기본 페이지 만들기]
+ [모바일 서비스 게시][모바일 서비스 게시]
+ [다음 단계][다음 단계]
...
이 자습서에서는 [Visual Studio 2013 업데이트 2][Visual Studio 2013 업데이트 2]가 사용됩니다.
...
Visual Studio 2013 업데이트 2에서 ASP.NET 웹 응용 프로그램 프로젝트에는 이제 Microsoft Azure 모바일 서비스에 대한 템플릿이 포함됩니다. 이 템플릿을 선택하고 **확인**을 클릭합니다.
...
[EF Code First][EF Code First]를 사용하여 데이터베이스 테이블을 정의합니다. DataObjects 폴더 아래에서 `Player`라는 클래스를 추가합니다.
...
두 클래스는 모두 **EntityData** 클래스에서 파생됩니다. **EntityData**에서 파생되기 때문에 Azure 모바일 서비스에 대해 크로스 플랫폼 클라이언트 라이브러리를 사용해서 앱이 데이터를 쉽게 사용할 수 있습니다. **EntityData**는 또한 앱이 [데이터베이스 쓰기 충돌 해결][데이터베이스 쓰기 충돌 해결]을 쉽게 수행할 수 있게 해줍니다.

`PlayerRank` 클래스에는 관련된 `Player` 엔터티를 가리키는 [탐색 속성][탐색 속성]이 있습니다. **[ForeignKey]** 특성은 EF에 `Player` 속성이 외래 키를 나타내도록 지정합니다.
...
컨트롤러 폴더를 마우스 오른쪽 단추로 클릭하고, 추가를 선택한 후 새 스캐폴드 항목을 선택합니다.

![][5]

**스캐폴드 추가** 대화 상자에서 왼쪽에 있는 **일반**을 확장하고 **Microsoft Azure 모바일 서비스**를 선택합니다. 그런 후 **Microsoft Azure 모바일 서비스 테이블 컨트롤러**를 선택합니다. **추가**를 클릭합니다.

**컨트롤러 추가** 대화 상자에서 다음을 수행합니다.

1. **모델 클래스** 아래에서 Player를 선택합니다.
2. **데이터 컨텍스트 클래스** 아래에서 LeaderboardContext를 선택합니다.
3. 컨트롤러 이름을 “PlayerContoller”로 지정합니다.
4. **추가**를 클릭합니다.
...
- 라우팅: **TableController**의 기본 경로는 `/tables/{table_name}/{id}`입니다. 여기서 *table\_name*은 엔터티 이름과 일치합니다. 따라서 Player 컨트롤러의 경로는 */tables/player/{id}*입니다. 이 라우팅 규칙에 따라 **TableController**가 모바일 서비스 [REST API][REST API]와 일치하게 됩니다.
...
`Player`는 객체 그래프에 포함되지 않았습니다. 플레이어를 포함하려면 DTO(*데이터 전송 개체*)를 정의하여 개체 그래프를 평면화할 수 있습니다.
...
LINQ Select 문을 사용하는 대신 AutoMapper를 사용하는 옵션도 있습니다. 이 옵션을 사용하려면 추가 설정 코드가 필요하지만 도메인 엔터티에서 DTO로 자동 매핑을 설정할 수 있습니다. 자세한 내용은 [.NET 백 엔드에서 AutoMapper를 사용하여 데이터베이스 유형과 클라이언트 유형 간 매핑][.NET 백 엔드에서 AutoMapper를 사용하여 데이터베이스 유형과 클라이언트 유형 간 매핑]을 참조하세요.
...
우선, `PlayerScore`라는 클래스를 DataObjects 폴더에 추가합니다.
...
`PlayerRankController` 클래스에서 `LeaderboardContext` 변수를 생성자에서 클래스 변수로 이동합니다.
...
그런 후 다음 코드를 `PlayerRankController`에 추가합니다.
...
`PostPlayerScore` 메서드는 `PlayerScore` 인스턴스를 입력으로 사용합니다. (클라이언트가 HTTP POST 요청으로 `PlayerScore`를 전송합니다.) 이 메서드는 다음을 수행합니다.

1. 데이터베이스에 아직 항목이 없으면 플레이어에 대해 새로운 `PlayerRank`를 추가합니다.
2. 플레이어의 점수를 업데이트합니다.
...
또한 메서드를 별도의 컨트롤러에 배치할 수도 있습니다. 어느 쪽이든 특별한 이점은 없으며, 단지 코드 구성 방법에 따라 달라집니다.
**[Route]** 특성에 대해 자세히 알아보려면 [Web API에서 특성 라우팅][Web API에서 특성 라우팅]을 참조하세요.

## Windows 스토어 앱 만들기

이 섹션에서는 모바일 서비스를 사용하는 Windows 스토어 앱에 대해 설명합니다. 하지만 XAML이나 UI에 대해서는 자세히 다루지 않습니다. 대신 응용 프로그램 논리에 대해 자세히 다룹니다. 완성된 프로젝트는 [여기][여기]에서 다운로드할 수 있습니다.
...
* [Azure 모바일 서비스에 대해 자세히 알아보기][Azure 모바일 서비스에 대해 자세히 알아보기]
* [Web API에 대해 자세히 알아보기][ASP.NET Web API]
* [데이터베이스 쓰기 충돌 처리][데이터베이스 쓰기 충돌 처리]
* [푸시 알림 추가][푸시 알림 추가](예: 다른 사람이 새 플레이어를 추가하거나 점수를 업데이트하는 경우)
-   [인증 시작][인증 시작]
...
[DTO를 사용하여 관련 엔터티 반환]: #use-a-dto-to-return-related-entities
[점수 제출을 위한 사용자 지정 API 정의]: #define-a-custom-api-to-submit-scores
[Windows 스토어 앱 만들기]: #create-the-windows-store-app
[모델 클래스 추가]: #add-model-classes
[뷰 모델 만들기]: #create-a-view-model
[MobileServiceClient 인스턴스 추가]: #add-a-mobileserviceclient-instance
[기본 페이지 만들기]: #create-the-main-page
