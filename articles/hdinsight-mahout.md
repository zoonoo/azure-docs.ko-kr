...
Mahout은 Apache Hadoop용 [기계 학습][기계 학습] 라이브러리입니다. Mahout에는 필터링, 분류 및 클러스터링과 같은 데이터 처리를 위한 알고리즘이 포함됩니다. 이 문서에서는 추천 엔진을 사용하여 친구가 본 영화를 기준으로 영화 추천을 생성합니다. 또한 의사 결정 포리스트로 분류를 수행하는 방법에 대해서도 알아봅니다. 여기서는 다음 내용을 다룹니다.
...
-   [PowerShell을 사용하여 추천 생성][PowerShell을 사용하여 추천 생성]
-   [Hadoop 명령줄을 사용하여 데이터 분류][Hadoop 명령줄을 사용하여 데이터 분류]
-   [문제 해결][문제 해결]

## <a name="recommendations"></a>PowerShell을 사용하여 추천 생성

> [WACOM.NOTE] 이 섹션에 사용된 작업이 PowerShell에서 작동하지만 Mahout과 함께 제공되는 클래스 중 다수가 PowerShell에서 현재 작동하지 않으며 Hadoop 명령줄을 사용하여 실행해야 합니다. PowerShell에서 작동하지 않는 클래스의 목록에 대해서는 [문제 해결][문제 해결] 섹션을 참조하세요.
>
> Hadoop 명령줄을 사용하여 Mahout 작업을 실행하는 예제에 대해서는 [Hadoop 명령줄을 사용하여 데이터 분류][Hadoop 명령줄을 사용하여 데이터 분류]를 참조하세요.
...
편의를 위해 GroupLens Research에서 [영화에 대한 평가 데이터][영화에 대한 평가 데이터]를 Mahout 호환 형식으로 제공합니다.

1.  1700개 영화에 대한 1,000명 사용자의 100,000개 평가를 포함하는 [MovieLens 100k][MovieLens 100k] 압축 파일을 다운로드합니다.

2.  압축 파일을 풉니다. 여기에는 **u.** 접두사가 지정된 많은 데이터 파일을 포함하는 **ml-100k** 디렉터리가 있습니다. Mahout에서 분석하는 파일은 **u.data**입니다. 이 파일의 데이터 구조는 `userID`, `movieID`, `userRating` 및 `timestamp`입니다. 다음은 데이터의 예제입니다.
...
3.  HDInsight 클러스터의 **example/data/u.data**에 **u.data** 파일을 업로드합니다. [Azure PowerShell][Azure PowerShell]이 있는 경우 [HDInsight-Tools][HDInsight-Tools] PowerShell 모듈을 사용하여 파일을 업로드할 수 있습니다. 파일을 업로드하는 다른 방법에 대해서는 [HDInsight에 Hadoop 작업용 데이터 업로드][HDInsight에 Hadoop 작업용 데이터 업로드]를 참조하세요. 다음은 `Add-HDInsightFile`을 사용하여 파일을 업로드하는 과정을 나타냅니다.
    ...
    이 코드는 클러스터용 기본 저장소의 **example/data/u.data**에 **u.data** 파일을 업로드합니다. 이제 HDInsight 작업에서 **wasb:///example/data/u.data** URI를 사용하여 이 데이터에 액세스할 수 있습니다.

### 작업 실행

다음 PowerShell 스크립트를 사용하여 앞서 업로드한 **u.data** 파일과 함께 Mahout 추천 엔진을 사용하는 작업을 실행합니다.
...
>
> 이러한 파일을 제거하려면 [HDInsight에서 Hadoop 작업용 데이터 업로드][HDInsight에 Hadoop 작업용 데이터 업로드]에 언급된 유틸리티 중 하나를 사용할 수 있습니다. 또는 [HDInsight-Tools][HDInsight-Tools] PowerShell 스크립트에서 `Remove-HDInsightFile` 함수를 사용하세요.
>
> 임시 파일 또는 출력 파일을 제거하지 않으면 작업을 다시 실행할 때 오류가 발생합니다.

Mahout 작업은 STDOUT에 출력을 반환하지 않고 **part-r-00000**로 지정된 출력 디렉터리에 저장합니다. 파일을 다운로드하여 보려면 [HDInsight-Tools][HDInsight-Tools] PowerShell 모듈에서 `Get-HDInsightFile` 함수를 사용하세요.
...
생성된 출력을 응용 프로그램에서 사용할 수 있긴 하지만 사용자가 읽을 수 있는 형식은 아닙니다. 앞서 **ml-100k** 폴더에 압축을 푼 다른 파일 중 일부를 사용하여 `movieId`를 영화 이름으로 확인할 수 있습니다. **ml-100k** 폴더(**show\_recommendations.py**)에 이 작업을 수행하는 Python 스크립트가 있지만 다음 PowerShell 스크립트를 사용할 수도 있습니다.
...
이 스크립트를 사용하려면 앞서 압축을 푼 **ml-100k** 폴더뿐 아니라 Mahout 작업으로 생성된 **part-r-00000** 출력 파일의 로컬 사본도 있어야 합니다. 다음은 스크립트를 실행하는 예제입니다.
...
> [WACOM.NOTE] 예제 Python 스크립트인 **show\_recommendations.py**가 동일한 매개 변수를 사용합니다.
...
Mahout에서 사용 가능한 분류 방법 중 하나는 [랜덤 포리스트][랜덤 포리스트]를 빌드하는 것입니다. 이 방법은 학습 데이터를 사용하여 의사 결정 트리를 생성한 후 이를 사용하여 데이터를 분류하는 여러 단계의 프로세스입니다. 이 프로세스는 Mahout에서 제공하는 **org.apache.mahout.classifier.df.tools.Describe** 클래스를 사용하며, Hadoop 명령줄을 사용하여 이 프로세스를 실행해야 합니다.
...
1.  이 작업에는 Hadoop 명령줄이 필요하므로 먼저 [Azure 관리 포털][Azure 관리 포털]을 통해 원격 데스크톱을 사용하도록 설정해야 합니다. 포털에서 HDInsight 클러스터를 선택한 다음 **구성** 페이지의 하단에서 **원격 사용**을 선택합니다.
...
3.  연결한 후 **Hadoop 명령줄** 아이콘을 사용하여 Hadoop 명령줄을 엽니다.

    ![hadoop cli][hadoop cli]

4.  다음 명령으로 Mahout을 사용하여 파일 설명자(**KDDTrain+.info**)를 생성합니다.
...
5.  다음 명령을 사용하여 의사 결정 트리의 포리스트를 빌드합니다.

        hadoop jar c:/apps/dist/mahout-0.9.0.2.1.3.0-1887/examples/target/mahout-examples-0.9.0.2.1.3.0-1887-job.jar org.apache.mahout.classifier.df.mapreduce.BuildForest -Dmapred.max.split.size=1874231 -d wasb:///example/data/KDDTrain+.arff -ds wasb:///example/data/KDDTrain+.info -sl 5 -p -t 100 -o nsl-forest

    이 작업의 출력은 **nsl-forest** 디렉터리에 저장되며, 이 디렉터리는 \_\_wasb://user/\<username\>/nsl-forest/nsl-forest.seq의 HDInsight 클러스터용 저장소에 있습니다. \<username\>은 원격 데스크톱 세션에 사용되는 사용자 이름입니다. 이 파일은 사용자가 읽을 수 없습니다.

6.  다음 명령을 사용하여 **KDDTest+.arff** 데이터 집합을 분류하여 포리스트를 테스트합니다.
...
이러한 클래스를 사용하는 작업을 실행하려면 HDInsight 클러스터에 연결하여 Hadoop 명령줄을 사용하는 작업을 실행하세요. 예제에 대해서는 [Hadoop 명령줄을 사용하여 데이터 분류][Hadoop 명령줄을 사용하여 데이터 분류]를 참조하세요.
  ...
  [원격 사용]: ./media/hdinsight-mahout/enableremote.png
  [연결]: ./media/hdinsight-mahout/connect.png
  ...
  [소스에서 Mahout을 빌드하고]: http://mahout.apache.org/developers/buildingmahout.html
