...
응용 프로그램이 단순 규칙을 포함하는 사용자 지정 서비스 ID를 사용하는 경우, 특정 Relay에 대한 액세스 제어 기능을 제공하기 위해 ACS 서비스 ID를 만들었다면 마이그레이션을 손쉽게 수행할 수 있습니다. 이 시나리오는 대개 각 Relay를 테넌트 사이트나 지점에 대한 브리지로 사용하며 해당 특정 사이트용으로 서비스 ID를 만드는 SaaS 스타일 솔루션에서 사용되는 경우가 많습니다. 이 경우에는 개별 서비스 ID를 Relay에서 직접 공유 액세스 서명 규칙으로 마이그레이션할 수 있습니다. 서비스 ID 이름은 SAS 규칙 이름으로, 서비스 ID 키는 SAS 규칙 키로 설정될 수 있습니다. 그리고 나면 SAS 규칙의 권한이 엔터티에 해당하는 각 ACS 규칙과 동일하게 구성됩니다.
