...
1. [도구 및 SDK 가져오기][도구 및 SDK 가져오기] 페이지에서 .NET용 Azure SDK를 설치합니다.
...
11. Program.cs에서 네임스페이스 정의를 다음 코드로 바꿉니다. 이 코드는 프로필 서비스 및 그에 대한 호스트를 추가합니다.
    ...
    ```
12. 솔루션 탐색기에서 **App.config** 파일을 두 번 클릭하여 Visual Studio 편집기에서 엽니다. &lt;system.ServiceModel&gt; 내에서 **&lt;system.ServiceModel&gt;** 요소의 아래에 다음 XML 코드를 추가합니다. *yourServiceNamespace*를 네임스페이스의 이름으로 바꾸고 *yourKey*를 앞에서 포털에서 검색한 SAS 키로 바꿔야 합니다.
    ...
    ```
13. 계속해서 App.config의 **&lt;appSettings&gt;** 요소에서 연결 문자열 값을 포털에서 이전에 얻은 연결 문자열로 바꿉니다. 
...
7. **새 ASP.NET 프로젝트** 대화 상자의 **Microsoft Azure** 섹션에서 **클라우드에서 호스트**가 선택되어 있는지 그리고 드롭다운 목록에서 **App Service**가 선택되어 있는지 확인합니다.
   ...
   ![][19]
8. **확인**을 클릭합니다. 
9. 이제 새 웹앱에 대한 Azure 리소스를 구성해야 합니다. [새 웹앱에 대한 Azure 리소스 구성](../app-service-web/web-sites-dotnet-get-started.md#configure-azure-resources-for-a-new-web-app) 섹션의 모든 단계를 따릅니다. 그런 다음, 이 자습서로 돌아가 다음 단계를 진행합니다.
