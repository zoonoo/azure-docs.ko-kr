...
description: 이 문서에서는 Azure 릴레이 서비스에 대해 자주 묻는 몇 가지 질문에 대한 답변을 제공합니다.
...
이전의 Service Bus Relay 서비스를 이제 [WCF 릴레이](service-bus-relay-tutorial.md)라고 지칭합니다. 평소처럼 이 서비스를 계속 사용할 수 있습니다. 하이브리드 연결 기능은 Azure BizTalk Services에 옮겨진 업데이트된 버전의 서비스입니다. WCF Relay 및 하이브리드 연결은 모두 계속 지원됩니다.
...
WCF Relay는 표준 계층 네임스페이스에서만 사용할 수 있습니다. 그렇지 않은 경우 릴레이의 가격 책정 및 [연결 할당량](../service-bus-messaging/service-bus-quotas.md)은 변경되지 않습니다. 즉, 릴레이는 메시지 수(작업 아님)와 릴레이 시간에 따라 지속적으로 청구됩니다. 자세한 내용은 가격 세부 정보 페이지에서 ["하이브리드 연결 및 WCF 릴레이"](https://azure.microsoft.com/pricing/details/service-bus/) 테이블을 참조하십시오.
...
**(WCF 릴레이에만 적용됩니다. 메시지는 하이브리드 연결에 대한 비용이 아닙니다.**)
...
공유 액세스 서명(SAS)은 SHA-256 보안 해시 또는 URI에 따른 인증 메커니즘입니다. Node.js, PHP, 파이썬, 자바, C 및 C#에서 사용자 고유의 서명을 생성하는 방법에 대한 자세한 내용은 [공유 액세스 서명이 있는 서비스 버스 인증을][Shared Access Signatures]참조하십시오.
