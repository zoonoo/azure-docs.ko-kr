...
ms.locfileid: "76514955"
---
# <a name="azure-relay-hybrid-connections-protocol"></a>Azure 릴레이 하이브리드 연결 프로토콜

Azure 릴레이는 Azure Service Bus 플랫폼의 주요 기능 요소 중 하나입니다. 릴레이의 새로운 _하이브리드 연결_ 기능은 HTTP 및 WebSockets를 기반으로 하는 안전한 개방형 프로토콜의 진화된 형태입니다. 그것은 독점 프로토콜 재단에 내장 된 이전, 동등하게 명명 _된 BizTalk 서비스_ 기능을 대체합니다. 하이브리드 연결을 Azure App Services에 통합하여 그 기능을 계속 수행합니다.
...
서비스가 WebSocket을 수락하면 등록이 완료되고 설정된 WebSocket은 모든 후속 상호 작용을 사용하도록 "컨트롤 채널"로서 활성 상태가 유지됩니다. 이 서비스를 사용하면 하나의 하이브리드 연결에 대해 최대 25개의 동시 수신기를 사용할 수 있습니다. AppHook에 대한 할당량은 결정이 필요합니다.
...
| 매개 변수        | 필수 | 설명
| ---------------- | -------- | -------------------------------------------
| `sb-hc-action`   | yes      | 수신기 역할의 경우 매개 변수는 **sb-hc-action=listen여야** 합니다.
...
* **토큰** - **수신자 오른쪽을** 부여하는 네임스페이스 또는 하이브리드 연결에 대한 유효한 URL 인코딩 서비스 버스 공유 액세스 토큰입니다.
