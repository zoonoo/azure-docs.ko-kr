...
이 항목에서는 **Azure 관리 포털**을 사용하여 콘텐츠 키 인증 정책을 구성하는 방법을 보여줍니다. 키는 나중에 동적으로 콘텐츠를 암호화하는데 사용할 수 있습니다. 현재 암호화할 수 있는 스트리밍 형식: HLS, MPEG DASH 및 부드러운 스트리밍. HDS 스트리밍 형식 또는 점진적 다운로드는 암호화할 수 없습니다.
...
[미디어 서비스.NET SDK를 사용하여 콘텐츠 키 인증 정책을 구성하기](../media-services-dotnet-configure-content-key-auth-policy/)

[미디어 서비스 REST API를 사용하여 콘텐츠 키 인증 정책을 구성하기](../media-services-rest-configure-content-key-auth-policy/)
...
- 사용자의 자산은 적응 비트 전송률 MP4 또는 적응 비트 전송률 부드러운 스트리밍 파일 집합을 포힘해야 합니다. 자세한 내용은 [자산 인코드](../media-services-encode-asset/)를 참조하세요.  
...
**토큰** 제한 정책은 **보안 토큰 서비스**(STS)에 의해 발급된 토큰이 수반되어야 합니다. 미디어 서비스 지원 토큰에는 **간단한 웹 토큰**([SWT](https://msdn.microsoft.com/ko-kr/library/gg185950.aspx#BKMK_2)) 형식 및 **JSON 웹 토큰 **(JWT) 형식의 토큰을 지원합니다. 현재는 **Azure 관리 포털**만 간단한 웹 토큰을 지원하고 REST API 또는 .NET SDK를 사용하여 또는 JWT 형식을 구성합니다.  정보는 [JWT 토큰 인증](http://www.gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/)을 참조 하십시오.

미디어 서비스는 **보안 토큰 서비스**를 제공하지 않습니다. 사용자 지정 STS를 만들거나 Microsoft Azure ACS를 활용하여 토큰을 발급할 수 있습니다. 지정된 키로 서명된 토큰을 만들고 토큰 제한 구성에서 지정한 클레임을 발급하려면 반드시 STS를 구성해야 합니다. 토큰이 유효하고 해당 토큰의 클레임이 콘텐츠 키에 대해 구성된 클레임과 일치하는 경우 미디어 서비스 키 배달 서비스는 암호화 키를 클라이언트에게 반환합니다. 자세한 내용은 [토큰을 발급 하도록 Azure ACS 사용](http://mingfeiy.com/acs-with-key-services)을 참조하세요.
...
**정책 xml 가져오기** 단추를 클릭하여 [여기](https://msdn.microsoft.com/ko-kr/library/azure/dn783459.aspx)에 정의된 XML 스키마를 준수하는 다른 XML을 제공할 수 있습니다.

##다음 단계
이제 사용자가 콘텐츠 키 인증 정책을 구성을 완료하였으므로, [다음으로 이동하십시오. Azure 관리 포털을 사용하여 암호화](../media-services-manage-content#encrypt/) 항목 활성화하기
