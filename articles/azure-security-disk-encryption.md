...
- Azure Active directory에서 Azure 디스크 암호화 사용에 Azure AD 응용 프로그램을 설정하고 구성하려면 이 문서의 *필수 조건* 섹션에서 **Azure Active Directory에서 Azure AD 응용 프로그램 설치**를 참조하세요.

- Azure AD 응용 프로그램에 대한 키 자격 증명 모음 액세스 정책을 설정하고 구성하려면 이 문서의 *필수 조건* 섹션에서 **Azure AD 응용 프로그램에 대 한 설정 키 자격 증명 모음 액세스 정책**을 참조하세요.
...
- Azure 디스크 암호화 PowerShell cmdlet을 실행하려면 먼저 [Azure PowerShell 버전 1.0.2](https://github.com/Azure/azure-powershell/releases/tag/v1.0.2-December2015)를 설치해야 합니다.

	- Azure PowerShell을 설치하고 Azure 구독에 연결하려면 [Azure PowerShell 설치 및 구성하는 방법](powershell-install-configure.md)을 참조하세요.

	- 이 문서는 독자가 모듈, cmdlet 및 세션과 같은 기본 개념을 이해하고 있다고 가정합니다. 자세한 내용은 [Windows PowerShell](https://technet.microsoft.com/library/hh857337.aspx) 시작을 참조하세요.

**참고:** Azure 디스크 암호화는 [Azure PowerShell SDK 버전 1.1.0](https://github.com/Azure/azure-powershell/releases/tag/v1.1.0-January2016)에서 지원되지 않습니다.
...
- Azure AD 응용 프로그램을 만들고, 새로운 키 자격 증명 모음을 만들거나 기존 키 자격 증명 모음을 설정하여 암호화를 활성화하기 위해 꼭 필요한 Azure 디스크 암호화 필수 PowerShell 스크립트는 [여기](https://github.com/Azure/azure-powershell/blob/dev/src/ResourceManager/Compute/Commands.Compute/Extension/AzureDiskEncryption/Scripts/AzureDiskEncryptionPreRequisiteSetup.ps1)에 있습니다.
