...
솔루션은 공용 미리 보기 릴리스에서 다음 시나리오, 기능 및 기술을 지원하지 않습니다.

- 기본 VM 및 표준 DS(프리미엄 저장소) 시리즈 IaaS VM

- 클래식 VM 만들기 메서드를 사용하여 만든 IaaS VM

- Azure 디스크 암호화를 통해 설정된 IaaS VM에서 암호화를 사용하지 않도록 설정하는 기능
...
공용 미리 보기 중 Azure 디스크 암호화로 VM 디스크를 암호화하는 것에 대한 요금은 부과되지 않습니다. 디스크 암호화가 일반적으로 제공된 후에도 계속 무료로 제공될 것으로 예상됩니다. 그러나 가격 책정은 시장 및 경쟁 전망에 달라질 수 있습니다.
...
- Azure 디스크 암호화는 다음 Windows Server SKU - Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2에서 지원됩니다. 솔루션은 Windows Server 2008 운영 체제에서 지원되지 않습니다. Windows Server Technical Preview는 공용 미리 보기 릴리스에서 지원되지 않습니다.
...
- Azure 디스크 암호화는 다음 Linux Server SKU - Ubuntu, CentOS, SUSE 및 SLES(SUSE Linux Enterprise Server)에서 지원됩니다. Red Hat Enterprise Linux는 공용 미리 보기 릴리스에서 지원되지 않습니다.

- 모든 리소스(예: 주요 자격 증명 모음, 저장소 계정, VM, VNet 등)는 같은 Azure 지역 및 구독에 속해야 합니다.

**참고** - 주요 자격 증명 모음 및 VM과 같은 리소스를 다른 Azure 지역에 만드는 것은 지원되지 않으며 Azure 디스크 암호화 기능을 사용하도록 설정하는 데 실패를 유발할 수 있습니다.

- Azure 플랫폼은 VM을 부팅하고 가상 컴퓨터 OS 볼륨을 해독할 수 있도록 하기 위해 고객 Azure 키 자격 증명 모음에서 암호화 키 또는 암호에 액세스해야 합니다. Azure 플랫폼에서 주요 자격 증명 모음에 액세스하도록 권한을 부여하려면 다음 요구 사항에 따라 주요 자격 증명 모음에서 **enabledForDiskEncryption** 속성을 설정해야 합니다.
...
- Azure 디스크 암호화 PowerShell cmdlet을 실행하려면 먼저 Azure PowerShell 버전 1.0.1을 설치해야 합니다.

	- Azure PowerShell을 설치하고 Azure 구독에 연결하려면 [Azure PowerShell 설치 및 구성하는 방법](powershell-install-configure.md)을 참조하세요.

	- Azure PowerShell 1.0.1 릴리스를 포함하도록 위의 옵션이 아직 업데이트되지 않은 경우 [여기](https://github.com/Azure/azure-powershell/releases/tag/v1.0.1-November2015)에서 Azure PowerShell 1.0.1을 직접 설치할 수도 있습니다.

**참고:** 이 문서는 독자가 모듈, cmdlet 및 세션과 같은 기본 개념을 이해하고 있다고 가정합니다. 자세한 내용은 [Windows PowerShell](https://technet.microsoft.com/library/hh857337.aspx) 시작을 참조하세요.
...
이 시나리오에서는 ARM 템플릿, [PowerShell cmdlet](https://msdn.microsoft.com/library/azure/mt603746.aspx) 또는 CLI 명령을 사용하여 암호화를 사용하도록 설정할 수 있습니다. 아래 섹션에서는 ARM 템플릿과 CLI 명령에 대해 자세히 설명합니다.
...
###Azure에서 기존/실행 중인 IaaS Windows VM에서 암호화 사용

이 시나리오에서는 ARM 템플릿, [PowerShell cmdlet](https://msdn.microsoft.com/library/azure/mt622707.aspx) 또는 CLI 명령을 사용하여 암호화를 사용하도록 설정할 수 있습니다. 아래 섹션에서는 ARM 템플릿과 CLI 명령을 사용하여 암호화를 사용하도록 설정하는 방법에 대해 자세히 설명합니다.
...
| AADClientSecret | ​주요 자격 증명 모음에 암호를 쓸 수 있는 권한이 있는 Azure AD 앱의 클라이언트 암호 |
| **​**keyVaultResourceID | ARM에서 주요 자격 증명 모음 리소스를 식별하는 ResourceID입니다. PowerShell cmdlet을 사용하여 얻을 수 있음: (Get-AzureRmKeyVault -VaultName &lt;yourKeyVaultName&gt; -ResourceGroupName &lt;yourResourceGroupName&gt;).ResourceId |
| ​keyVaultURL | ​BitLocker 키가 업로드될 주요 자격 증명 모음의 URL. cmdlet을 사용하여 얻을 수 있음: (Get-AzureRmKeyVault -VaultName &lt;yourKeyVaultName&gt; -ResourceGroupName &lt;yourResourceGroupName&gt;). VaultURI |
| ​ keyEncryptionKeyURL | ​생성된 BitLocker 키를 암호화하는 데 사용되는 주요 암호화 키의 URL. 선택 사항입니다. |
...
###Azure에서 기존/실행 중인 IaaS Linux VM에서 암호화 사용
...
[여기](https://azure.microsoft.com/documentation/articles/xplat-cli-install/)에서 설치된 CLI 명령을 사용하여 고객 암호화 VHD에서 디스크 암호화를 사용하도록 설정할 수 있습니다. CLI 명령을 사용하여 Azure에서 기존/실행 중인 IaaS Linux VM에서 암호화를 사용하도록 설정하려면 아래 단계를 따르세요.
...
Azure 관리(Preview) 포털, [PowerShell cmdlet](https://msdn.microsoft.com/library/azure/mt622700.aspx) 또는 CLI 명령을 사용하여 암호화 상태를 가져올 수 있습니다. 아래 섹션에서는 Azure 관리(Preview) 포털 및 CLI 명령을 사용하여 암호화 상태를 가져오는 방법에 대해 설명합니다.

####Azure 관리 Preview 포털을 사용하여 암호화된 IaaS VM의 암호화 상태 가져오기

Azure 관리 Preview 포털에서 IaaS VM의 암호화 상태를 가져올 수 있습니다. https://portal.azure.com/에서 Azure Preview 포털에 로그온하고 왼쪽 메뉴에서 가상 컴퓨터 링크를 클릭하여 구독에서 가상 컴퓨터의 요약 보기를 확인합니다. 구독 드롭다운에서 구독 이름을 선택하여 가상 컴퓨터 보기를 필터링할 수 있습니다. 가상 컴퓨터 페이지 메뉴의 위쪽에 있는 열을 클릭합니다. 선택 열 블레이드에서 디스크 암호화 열을 선택하고 업데이트를 클릭합니다. 아래 그림에 표시된 것처럼 각 VM에 대한 "사용" 또는 "사용 안 함" 암호화 상태를 보여주는 디스크 암호화 열이 표시됩니다.
...
OSVolumeEncrypted 및 DataVolumesEncrypted 설정 값이 "True"로 설정되어 두 볼륨이 Azure 디스크 암호화를 사용하여 암호화됨을 보여줍니다.
...
**참고:** 주요 자격 증명 모음은 VM과 동일한 지역에 있어야 합니다.
...
**참고:** $azureAdApplication.ApplicationId는 Azure AD ClientID이며 $aadClientSecret은 나중에 ADE를 설정하는 데 사용할 클라이언트 암호입니다.
...
![Azure의 Microsoft 맬웨어 방지](./media/azure-security-disk-encryption\disk-encryption-fig3.JPG)

2\. 응용 프로그램 추가를 클릭하고 아래와 같이 응용 프로그램 이름을 입력합니다.

![Azure의 Microsoft 맬웨어 방지](./media/azure-security-disk-encryption\disk-encryption-fig4.JPG)

3\. 화살표 단추를 클릭하고 아래와 같이 앱 속성을 구성합니다.

![Azure의 Microsoft 맬웨어 방지](./media/azure-security-disk-encryption\disk-encryption-fig5.JPG)

4\. 왼쪽 하단에 있는 확인 표시를 클릭하여 마칩니다. 앱의 구성 페이지가 나타납니다. 아래 그림처럼 Azure AD 클라이언트 ID는 페이지의 아래쪽에 있습니다.
 
![Azure의 Microsoft 맬웨어 방지](./media/azure-security-disk-encryption\disk-encryption-fig6.JPG)
...
![Azure의 Microsoft 맬웨어 방지](./media/azure-security-disk-encryption\disk-encryption-fig7.JPG)


**참고:** 위의 작업 과정은 Preview 포털에서는 지원되지 않습니다.
...
BitLocker 암호화를 사용하도록 설정한 후에는 로컬 암호화된 VHD를 저장소 계정에 업로드해야 합니다.
