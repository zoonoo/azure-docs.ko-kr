<properties urlDisplayName="HDInsight Emulator release notes" pageTitle="릴리스 정보: Microsoft HDInsight Emulator for Azure | Azure" metaKeywords="hdinsight, Azure hdinsight, hdinsight azure, get started hdinsight, emulator, hdinsight emulator" description="HDInsight Hadoop Emulator의 최신 릴리스에 대한 새로운 정보를 제공합니다." umbracoNaviHide="0" disqusComments="1" editor="cgronlun" manager="paulettm" services="hdinsight" title="릴리스 정보: Microsoft HDInsight Emulator for Azure" authors="jgao" />
...
-   서비스의 3.1 버전에 해당하는 [업데이트된 Hadoop 구성 요소 버전][업데이트된 Hadoop 구성 요소 버전]. 여기에는 Hive 0.13 및 Tez 지원이 포함됩니다.
...
-   명령의 버그 수정이 GettingStarted 폴더에 설치된 RunSamples.ps1 스크립트에서 Pig 샘플을 실행하는 데 사용되었습니다.

-   이 버전에는 Azure HDInsight 클러스터 버전 1.6에서 사용할 수 있는 Hortonworks Data Platform 서비스에 맞는 Hortonworks Data Platform 버전 1.1에 대한 업데이트가 포함되어 있습니다.

## v0.11.0.0, 릴리스 날짜: 2013년 9월 30일
...
## v0.10.0.0, 릴리스 날짜: 2013년 8월 9일

### 새로운 기능

-   이 버전에는 Azure HDInsight의 Hortonworks Data Platform Preview와 대등한 Hortonworks Data Platform Developer Preview에 대한 업데이트가 포함되어 있습니다.

## v0.8.0.0, 릴리스 날짜: 2013년 6월 7일

### 새로운 기능

-   이 버전에는 Azure HDInsight의 Hortonworks Data Platform Preview와 대등한 Hortonworks Data Platform Developer Preview에 대한 업데이트가 포함되어 있습니다.

## v0.6.0.0, 릴리스 날짜: 2013년 5월 13일

### 새로운 기능

-   Hive Server 2가 지금 설치 중입니다. 이 업데이트와 동시에 릴리스된 Microsoft ODBC Driver for Hive의 새 버전 0.9.2.0이 있어야 합니다.

-   모든 서비스가 자동 시작으로 설정되어 있으므로 컴퓨터를 다시 부팅한 후 모두 다시 시작하도록 할 필요가 없습니다.

## v0.4.0.0, 릴리스 날짜: 2013년 3월 25일
...
-   다음 기능이 포함된 새 Microsoft HDInsight 대시보드
...
-   로컬 HDInsight 설치 및 Azure HDInsight 서비스의 REST API 끝점은 다음과 같이 동일한 서비스의 다른 포트 번호를 통해 액세스됩니다.
    ...
    ODBC: DSN 구성 또는 연결 문자열에서 포트 10000을 사용합니다.

    HDInsight 서비스:
    Oozie: http://ServerFQDN:563/oozie/v1/admin/status
    Templeton: http://ServerFQDN:563/templeton/v1/status
    ODBC: DSN 구성 또는 연결 문자열에서 포트 563을 사용합니다.

-   로컬 클러스터에 ASV를 구성합니다.

    대시보드에 "local(hdfs)"이라는 기본 로컬 클러스터가 표시됩니다. ASV를 로컬 설치에 대한 저장소로 사용하려면 다음을 수행합니다.
    ...
    4.  현재 등록된 로컬 클러스터를 삭제하고 새 ASV 자격 증명으로 로컬 클러스터를 다시 등록합니다.

## v0.3.0.0, 릴리스 날짜: 2012년 12월 13일

-   대시보드 웹 사이트가 Windows 자격 증명을 사용하지 않고 익명 인증으로 변경되었습니다. 이렇게 하면 이전 버전 릴리스 정보에서 언급된 로그인 프롬프트 문제가 없어집니다.
...
-   Hive 콘솔
    ...
    -   제출된 Hive 명령에 새 줄이 포함되어 있으면 "구문 오류"가 표시됩니다. 새 줄을 제거하고 의도한 대로 쿼리를 실행해야 합니다.

## 일반적으로 알려진 문제

-   Hadoop 사용자 암호 만료

    컴퓨터에 적용된 AD 정책에 따라 Hadoop 사용자의 암호가 만료되었을 수 있습니다. 다음 PowerShell 스크립트는 암호가 만료되지 않도록 설정하고 관리 명령 프롬프트에서 실행할 수 있습니다.
...
-   임시 디렉터리\>
    ...
    hadoop.tmp.dir이 C:\\hadoop\\hdfs를 가리키지 않고 잘못된 위치인 c:\\hdfs를 가리킵니다. 이 버그는 HDP 비트의 다음 업데이트에서 수정됩니다.

-   OS 제한

    HDInsight 서버는 64비트 OS에 설치되어야 합니다.

-   WebPI 검색 결과에서 HDInsight를 찾을 수 없습니다.

    이 문제는 대개 OS 제한으로 인해 발생합니다. HDInsight는 Windows 7 서비스 팩 1, Windows Server 2008 R2 서비스 팩 1, Windows 8 또는 Windows Server 2012의 최소 버전을 실행하는 64비트 운영 체제가 필요합니다.
...
-   [HDInsight Emulator 시작][HDInsight Emulator 시작]
  ...
  [업데이트된 Hadoop 구성 요소 버전]: http://azure.microsoft.com/ko-kr/documentation/articles/hdinsight-component-versioning/
  [http://azure.microsoft.com/ko-kr/documentation/services/hdinsight/]: http://azure.microsoft.com/ko-kr/documentation/services/hdinsight/
