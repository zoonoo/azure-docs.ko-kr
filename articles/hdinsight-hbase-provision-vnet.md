...
> [WACOM.NOTE]현재 HBase 클러스터만 Azure 가상 네트워크에 프로비전할 수 있습니다. Hadoop 클러스터는 지원되지 않습니다.
...
3.  다음 값을 입력하거나 선택합니다.
    ...
    -   **DNS 서버**: 이 문서에서는 Azure에서 제공하는 내부 DNS 서버를 사용하므로 **없음**을 선택하면 됩니다. 사용자 지정 DNS 서버가 포함된 고급 네트워킹 구성도 지원됩니다. 자세한 지침은 [][]<http://msdn.microsoft.com/library/azure/jj156088.aspx></a>를 참조하세요.
...
> [WACOM.NOTE] HDInsight 클러스터는 Azure Blob 저장소를 사용하여 데이터를 저장합니다. 자세한 내용은 [HDInsight에서 Hadoop와 함께 Azure Blob 저장소 사용][]을 참조하세요. 저장소 계정과 Blob 저장소 컨테이너가 필요합니다. 저장소 계정 위치는 가상 네트워크 위치 및 클러스터 위치와 일치해야 합니다.
...
새 HBase 클러스터를 테스트하려면 [HDInsight에서 Hadoop와 함께 HBase를 사용하여 시작][]의 절차를 사용할 수 있습니다.

## <span id="connect"></span></a>HBase Java RPC API를 사용하여 가상 네트워크에 프로비전된 HBase 클러스터에 연결

1.  동일한 Azure 가상 네트워크와 동일한 서브넷에 IaaS 가상 컴퓨터를 프로비전합니다. 따라서 가상 컴퓨터와 HBase 클러스터가 둘 다 동일한 내부 DNS 서버를 사용하여 호스트 이름을 확인합니다. 이렇게 하려면 갤러리에서 옵션을 선택한 다음 데이터 센터 대신 가상 네트워크를 선택해야 합니다. 지침은 [Windows Server를 실행하는 가상 컴퓨터 만들기][]를 참조하세요. 작은 VM 크기의 표준 Windows Server 2012 이미지면 충분합니다.

2.  HBase 클러스터의 연결 특정 DNS 접미사를 가져옵니다. 이렇게 하려면 HBase 클러스터에 RDP를 수행하고(헤드 노드에 연결됨) 명령 프롬프트에서 **ipconfig**를 실행합니다. RDP를 사용하도록 설정하고 RDP를 통해 클러스터에 연결하는 방법에 대한 지침은 [Azure 관리 포털을 사용하여 HDInsight에서 Hadoop 클러스터 관리][]를 참조하세요.
...
3.  가상 컴퓨터의 주 DNS 접미사 구성을 변경합니다. 이렇게 하면 가상 컴퓨터가 명시적 접미사 지정 없이 HBase 클러스터의 호스트 이름을 자동으로 확인할 수 있습니다. 예를 들어 *workernode0* 호스트 이름이 HBase 클러스터의 workernode0으로 올바르게 확인됩니다.
    ...
    이제 가상 컴퓨터가 HBase 클러스터와 통신할 준비가 되었습니다. 연결을 테스트하려면 가상 컴퓨터에서 "ping headnode0"을 실행합니다.
...
-   [HDInsight에서 Hadoop와 함께 HBase를 사용하여 시작][]
...
-   [가상 네트워크 개요][]
  ...
  [HDInsight에서 Hadoop와 함께 HBase를 사용하여 시작]: ../hdinsight-hbase-get-started/
