...
자세한 내용은 [시작](./data-lake-store-get-started-portal.md)을 참조 하세요.
...
> [!IMPORTANT]
> 키 회전 작업이 완료 되 면 이전 버전의 키가 더 이상 새 데이터를 암호화 하는 데 사용 되지 않습니다. 그러나 오래 된 데이터에 액세스 하는 경우 이전 키가 필요할 수 있습니다. 이러한 이전 데이터를 읽을 수 있도록 하려면 이전 키를 삭제 하지 마십시오.