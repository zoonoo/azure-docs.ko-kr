...
데이터 레이크 저장소 Gen1 쿼리 및 많은 양의 데이터를 분석을 위해 대규모 처리 능력이 필요한 대규모 분석 시스템을 실행 하기 위한 빌드됩니다. 데이터 레이크는 개별 스토리지 서버의 수 이상으로 파일의 일부분을 배포합니다. 데이터 분석을 수행하기 위해 병렬로 파일을 읽을 때 읽기 처리량이 향상됩니다.
