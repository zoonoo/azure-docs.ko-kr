...
*  **이벤트 허브 네임스페이스**. 자세한 내용은 [Event Hubs 네임스페이스 만들기](../event-hubs/event-hubs-create.md#create-an-event-hubs-namespace)를 참조하세요. Data Lake Storage Gen1 계정 및 Event Hubs 네임스페이스가 동일한 Azure 구독에 있는지 확인합니다.
...
1. Event Hubs의 데이터를 캡처하려는 Data Lake Storage Gen1 계정을 열고 **데이터 탐색기**를 클릭합니다.
    ...
    ![데이터 레이크 스토리지 Gen1 데이터 탐색기](./media/data-lake-store-archive-eventhub-capture/data-lake-store-open-data-explorer.png "데이터 레이크 스토리지 Gen1 데이터 탐색기")

1.  **새 폴더**를 클릭하고 데이터를 캡처할 폴더의 이름을 입력합니다.

    ![데이터 레이크 스토리지 Gen1에서 새 폴더 만들기](./media/data-lake-store-archive-eventhub-capture/data-lake-store-create-new-folder.png "데이터 레이크 스토리지 Gen1에서 새 폴더 만들기")

1. Data Lake Storage Gen1의 루트에서 사용 권한을 할당합니다. 

    a. **데이터 탐색기**를 클릭하고 Data Lake Storage Gen1 계정의 루트를 선택한 다음, **액세스**를 클릭합니다.

    ![데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-permissions-to-root.png "데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당")

    b. **액세스** 아래에서 **추가**를 클릭하고 **사용자 또는 그룹 선택**을 클릭한 후 `Microsoft.EventHubs`를 검색합니다. 

    ![데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-eventhub-sp.png "데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당")
    ...
    **선택**을 클릭합니다.
    ...
    ![데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-eventhub-sp1.png "데이터 레이크 저장소 Gen1 루트에 대한 사용 권한 할당")
...
1. 데이터를 캡처하려는 Data Lake Storage Gen1 계정 아래의 폴더에 대해 사용 권한을 할당합니다.
    ...
    ![데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-permissions-to-folder.png "데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당")

    b. **액세스** 아래에서 **추가**를 클릭하고 **사용자 또는 그룹 선택**을 클릭한 후 `Microsoft.EventHubs`를 검색합니다. 

    ![데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-eventhub-sp.png "데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당")
    ...
    **선택**을 클릭합니다.
    ...
    ![데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당](./media/data-lake-store-archive-eventhub-capture/data-lake-store-assign-eventhub-sp-folder.png "데이터 레이크 스토리지 Gen1 폴더에 대한 사용 권한 할당")
...
![데이터 레이크 스토리지 Gen1의 이벤트허브 샘플 데이터](./media/data-lake-store-archive-eventhub-capture/data-lake-store-eventhub-data-sample.png "데이터 레이크 스토리지 Gen1의 이벤트허브 샘플 데이터")
