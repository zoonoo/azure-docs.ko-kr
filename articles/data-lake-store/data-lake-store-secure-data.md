<properties 
   pageTitle="Azure 데이터 레이크 저장소에 저장된 데이터 보호 | Azure" 
...
이 문서에서는 Azure 포털을 사용하여 위의 작업을 수행하는 방법에 대한 지침을 제공합니다.
...
4. **사용자** 블레이드에서 **추가**를 클릭하여 **액세스 추가** 블레이드를 엽니다. **액세스 추가** 블레이드에서 **역할 선택**을 클릭한 다음 사용자/그룹에 대한 역할을 선택합니다.
	...
	**소유자** 및 **참여자** 역할은 데이터 레이크 계정에 다양한 관리 기능에 대한 액세스를 제공합니다. 데이터 레이크의 데이터와 상호 작용하는 사용자의 경우 **리더** 역할에 추가할 수 있습니다. 이러한 역할의 범위는 Azure 데이터 레이크 저장소 계정에 관련된 관리 작업으로 제한됩니다.
...
사용자/보안 그룹을 Azure 데이터 레이크 파일 시스템에 할당하여 Azure 데이터 레이크 저장소에 저장된 데이터에 대한 액세스 제어를 설정합니다. 현재 릴리스에서 파일 시스템의 루트 노드에 ACL만 설정할 수 있습니다.
...
6. **사용자 지정 액세스 추가** 블레이드에서 **확인**을 클릭합니다. 이제 연결된 권한으로 새롭게 추가된 그룹이 **액세스** 블레이드에 나열됩니다.
	...
	> [AZURE.IMPORTANT]현재 릴리스에서 **사용자 지정 액세스** 아래에 12개의 항목만이 있습니다. 12개 이상의 사용자를 추가하려는 경우 보안 그룹을 만들고 사용자를 보안 그룹에 추가하고 해당 보안 그룹에 데이터 레이크 저장소 계정에 대한 액세스를 제공해야 합니다.
