...
| 데이터 작업 - 인증 프로토콜 |[Openid connect Connect](https://openid.net/connect/). 호출은 Azure Active Directory에서 발급 한 유효한 JWT (JSON 웹 토큰)를 포함 해야 합니다.|HMAC (해시 기반 MAC(메시지 인증 코드)). 호출은 HTTP 요청 일부를 통해 Base64 인코딩된 SHA-256 해시를 포함해야 합니다. |
...
| 서비스 상태 |일반 공급 |일반 공급 |
| 지역별 가용성 |[여기](https://azure.microsoft.com/regions/#services) 를 참조 하세요. |모든 Azure 지역에서 사용가능 |
