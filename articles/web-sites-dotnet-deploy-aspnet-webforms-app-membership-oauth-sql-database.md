<properties pageTitle="Azure 웹 사이트에 멤버 자격, OAuth 및 SQL 데이터베이스를 포함한 보안 ASP.NET Web Forms 앱 배포" metaKeywords="Azure WebForms Hello World 자습서, Azure WebForms 시작 자습서, Azure C# WebForms Hello World 자습서, SQL Azure C# WebForms 자습서" description="이 자습서에서는 SQL 데이터베이스를 통합하는 보안 ASP.NET 4.5 Web Forms 웹 앱을 빌드하고 응용 프로그램을 Azure에 배포하는 방법을 보여 줍니다." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Deploy a secure ASP.NET 4.5 Web Forms app to an Azure Website" authors="erikre"  solutions="" manager="wpickett" editor="mollybos"  />
...
>[WACOM.NOTE] 
이 자습서를 완료하려면 Azure 계정이 필요합니다. 계정이 없는 경우 <a href="/ko-kr/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">MSDN 구독자 혜택 활성화</a> 또는 <a href="/ko-kr/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">무료 평가판 등록</a>을 수행할 수 있습니다. 계정을 등록하기 전에 Azure 웹 사이트 사용을 시작하려면 <a href="https://trywebsites.azurewebsites.net/">https://trywebsites.azurewebsites.net</a>으로 이동합니다. 여기서 무료로 Azure 웹 사이트에 단기 ASP.NET 시작 사이트를 즉시 만들 수 있습니다. 신용 카드가 필요하지 않으며 약정이 없습니다.
...
Visual Studio 내의 **Web Publish Activity(웹 게시 활동)** 창에 전반적인 게시 상태가 표시됩니다.  
...
응용 프로그램을 다시 게시하되 **웹 게시** 대화 상자에서 **설정** 탭을 선택합니다. 그런 다음 구성을 **디버그**로 설정하고 **Remove additional files at destination(대상에서 추가 파일 제거)** 옵션을 선택합니다. **게시**를 선택하여 응용 프로그램을 다시 배포합니다.  
...
2. 새 클래스의 이름을 *Contacts.cs*로 지정합니다.  
...
7. 왼쪽 창에서 **Web Forms**를 선택하고 가운데 페이지에서 **Web Forms Pages using Entity Framework(Entity Framework를 사용한 Web Forms 페이지)**를 선택합니다. 그런 다음 **추가**를 클릭합니다.  
...
Web Forms 스캐폴더에서 *Default.aspx*, *Delete.aspx*, *Edit.aspx* 및 *Insert.aspx* 페이지를 포함하는 새 폴더를 추가합니다. 또한 *EntityTemplates* 폴더 및 *FieldTemplates* 폴더를 포함하는 *DynamicData* 폴더도 만듭니다. 'ApplicationDbContext'는 멤버 자격 데이터베이스 및 연락처 데이터 둘 다에 사용됩니다.
...
3. **패키지 관리자 콘솔** 창에서 다음 명령을 입력합니다.  
	...
	'add-migration Initial' 명령은 데이터베이스를 만든 *Migrations* 폴더에 <date_stamp>Initial이라는 파일을 생성합니다. 첫 번째 매개 변수(Initial)는 임의이며 파일 이름을 만드는 데 사용됩니다. **솔루션 탐색기**에서 새 클래스 파일을 볼 수 있습니다. 'Initial' 클래스의 'Up' 메서드는 'Contact' 테이블을 만들고 이전 상태로 돌아가려는 경우 사용되는 'Down' 메서드는 테이블을 삭제합니다.  
...
4. 'Seed' 메서드 시작 내에서 'AddUserAndRole' 메서드 호출을 추가합니다. 'Seed' 메서드의 시작 부분만 표시됩니다.  
	<pre class="prettyprint">
        protected override void Seed(ContactManager.Models.ApplicationDbContext context)
        {
...
5. 모든 변경 내용을 저장한 후 **패키지 관리자 콘솔**에서 다음 명령을 실행합니다.  
	...
	이 코드는 새로운 'canEdit' 역할을 만들고 canEditUser@wideworldimporters.com 메일 계정이 있는 새로운 로컬 사용자를 만듭니다. 그런 다음 이 코드는 canEditUser@wideworldimporters.com을 'canEdit' 역할에 추가합니다. 자세한 내용은 [ASP.NET ID](http://www.asp.net/identity) 리소스 페이지(영문)를 참조하세요.  
...
4. 다음 3개 파일에 대해 'Response.Redirect("Default.aspx")' 코드 참조 6개를 'Response.Redirect("~/Contacts/Default.aspx")'로 업데이트합니다.  
	- *Delete.aspx.cs*
	- *Edit.aspx.cs*
	- *Insert.aspx.cs*  
...
9. **Execute Code First Migrations(Code First 마이그레이션 실행)** 확인란을 선택합니다.  
...
3. **방화벽 규칙 추가** 대화 상자가 표시되면 **방화벽 규칙 추가**를 선택합니다.  
	...
	>[WACOM.NOTE]  
	**SQL 데이터베이스**를 확장할 수 없고 Visual Studio에서 **ContactDB**를 볼 수 없는 경우 아래 지침에 따라 방화벽 포트나 포트 범위를 열어야 합니다. 그렇게 하려면 [MVC 자습서](http://azure.microsoft.com/ko-kr/documentation/articles/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/)(영문) 끝에 있는 **Azure 방화벽 규칙 설정**의 지침을 따르세요. 또는 로컬로 응용 프로그램을 빌드 및 실행하고, 이 응용 프로그램에 데이터를 추가하여 로컬 데이터베이스의 데이터를 검토할 수 있습니다(Visual Studio에서 **Ctrl+F5**를 누름).  
