...
1.  아직 설치하지 않았다면 [Visual Studio 2013][]을 설치합니다.
2.  [Azure SDK for Visual Studio 2013][]을 설치합니다. 이 자습서에서는, Azure SDK for Visual Studio 2013를 설치하려면 먼저 Visual Studio 2013이 설치되어 있어야 합니다.
...
5.  **Visual Studio 업데이트 2**를 아직 설치하지 않았다면 **[Visual Studio 2013 업데이트 2][]** 이상을 다운로드하여 설치합니다.
...
6.  `Seed` 메서드를 다음 코드로 바꿉니다.
    ...
    이 코드는 연락처 정보를 사용하여 데이터베이스를 초기화(시드)합니다. 데이터베이스 시드에 대한 자세한 내용은 [EF(Entity Framework) DB 시드 및 디버그][](영문)를 참조하세요.
...
3.  [Google Developers Console][]로 이동합니다. Google 개발자 전자 메일 계정(gmail.com)으로 로그인해야 합니다. Google 계정이 없는 경우 **가입하기** 링크를 선택합니다.
...
ASP.NET Web Forms에 대해 자세한 내용은 ASP.NET 웹 사이트의 [ASP.NET Web Forms 알아보기][](영문) 및 [Microsoft Azure 자습서 및 가이드][]를 참조하세요.

이 자습서는 Rick Anderson(Twitter [@RickAndMSFT][])이 Tom Dykstra 및 Barry Dorrans(Twitter [@blowdart][])의 도움을 받아 작성한 MVC 자습서인 [Azure 웹 사이트에 멤버 자격, OAuth 및 SQL 데이터베이스를 포함한 보안 ASP.NET MVC 5 앱 배포][](영문) 내용을 바탕으로 합니다.
