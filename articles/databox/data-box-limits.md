...
- Data Box는 Windows 기반 장치 이며 대/소문자를 구분 하는 파일 이름을 지원 하지 않습니다. 예를 들어 Azure에는 대/소문자가 다른 두 개의 다른 파일이 있습니다. 데이터 상자를 사용 하 여 이러한 파일을 내보내는 것은 장치에서 덮어쓰여집니다.
- 입력 파일이 나 태그에 동일한 데이터를 참조 하는 태그가 중복 된 경우 내보내기 Data Box 파일을 건너뛰거나 덮어쓸 수 있습니다. Azure Portal에 표시 되는 파일 수와 데이터 크기는 장치에 있는 데이터의 실제 크기와 다를 수 있습니다. 
...
- 페이지 blob 및 컨테이너 이름은 대/소문자를 구분 하므로 대/소문자가 일치 하지 않으면 blob 및/또는 컨테이너가 검색 되지 않습니다.
