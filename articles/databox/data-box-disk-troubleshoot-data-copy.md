---
title: Azure 데이터 박스 디스크 데이터 복사 문제 해결| 마이크로 소프트 문서
description: 로그를 사용하여 Azure Data Box Disk의 데이터 복사 중에 발생한 문제를 해결하는 방법을 설명합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: disk
...
---
# <a name="troubleshoot-data-copy-issues-in-azure-data-box-disk"></a>Azure 데이터 상자 디스크의 데이터 복사 문제 해결

이 문서는 Microsoft Azure 데이터 상자 디스크에 적용되며 데이터를 디스크에 복사할 때 발생하는 문제를 해결하는 방법을 설명합니다. 이 문서에서는 분할 복사 도구를 사용할 때도 문제를 다룹니다.


## <a name="data-copy-issues-when-using-a-linux-system"></a>Linux 시스템을 사용할 때 데이터 복사 문제

이 섹션에서는 Linux 클라이언트를 사용하여 디스크에 데이터를 복사할 때 직면한 주요 문제 중 일부에 대해 자세히 설명합니다.

### <a name="issue-drive-getting-mounted-as-read-only"></a>문제: 드라이브가 읽기 전용으로 마운트되는 경우
...
리눅스 시스템에서 다음 단계를 수행합니다.

1. ntfsfix 유틸리티용 `ntfsprogs` 패키지를 설치합니다.
2. 잠금 해제 도구를 사용하여 드라이브에 제공된 마운트 지점을 마운트 해제합니다. 드라이브에 따라 마운트 지점 수가 달라집니다.

    ```
    unmount /mnt/DataBoxDisk/mountVol1
    ```

3. 해당 `ntfsfix` 경로에서 실행합니다. 강조 표시된 숫자는 2단계와 같아야 합니다.

    ```
    ntfsfix /mnt/DataBoxDisk/bitlockerVol1/dislocker-file
    ```

4. 다음 명령을 실행하여 마운트 문제를 일으킬 수 있는 최대 절전 모드 메타데이터를 제거합니다.
...
6. 깨끗한 잠금 해제및 마운트를 수행합니다.
7. 파일을 작성하여 탑재 지점을 테스트합니다.
8. 파일 지속성의 유효성을 검사하기 위해 마운트를 해제하고 다시 마운트합니다.
9. 데이터 복사본을 계속합니다.
 
### <a name="issue-error-with-data-not-persisting-after-copy"></a>문제: 복사 후 데이터가 유지되지 않는 오류
...
복사 분할 도구를 사용하여 여러 디스크로 데이터를 분할할 때 발생한 문제는 다음 표에 요약되어 있습니다.

|오류 메시지/경고 |권장 사항 |
|---------|---------|
|[정보] 볼륨에 대한 BitLocker 암호 검색 : m <br>[오류] 볼륨 m에 대 한 BitLocker 키를 검색 하는 동안 발견 하는 예외:<br> 시퀀스에 요소가 없습니다.|이 오류는 대상 Data Box Disk가 오프라인인 경우 발생합니다. <br> 온라인 디스크에 `diskmgmt.msc` 도구를 사용합니다.|
...
|[오류] 예외가 발생함: 대상 드라이이브에 DriveManifest.xml 파일이 존재합니다. <br> 이는 다른 저널 파일을 사용하여 대상 드라이브가 준비되었을 수 있음을 나타냅니다. <br>동일한 드라이브에 데이터를 더 추가하려면 이전 저널 파일을 사용합니다. 기존 데이터를 삭제하고 새 가져오기 작업에 대상 드라이브를 다시 사용하려면 드라이브에서 *DriveManifest.xml을* 삭제합니다. 이 명령을 새 저널 파일을 사용하여 다시 실행합니다.| 이 오류는 다수의 가져오기 세션에 대해 동일한 드라이브 집합을 사용하려고 할 때 수신됩니다. <br> 하나의 드라이브 집합은 하나의 분할 및 복사 세션에만 사용하십시오.|
...
- [유효성 검사 도구 문제 해결](data-box-disk-troubleshoot.md)방법에 대해 알아봅니다.
