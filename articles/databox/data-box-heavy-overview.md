...
Microsoft Azure Data Box 하이브리드 솔루션을 사용하면 수백 테라바이트 단위의 데이터를 빠르고 저렴하게 신뢰할 수 있는 방식으로 Azure로 보낼 수 있습니다. 사용자에게 화물로 저장 용량이 1PB인 전용 저장소 장치를 배송하여 안전한 데이터 전송을 더욱 빠르게 실현해 줍니다. 장치는 운송 중에 데이터를 안전하게 보호하기 위해 견고하게 포장됩니다.

Data Box Heavy는 현재 미리 보기로 제공되며 Azure Portal을 통해 등록하여 장치를 요청할 수 있습니다. 데이터 센터에서 장치를 받으면 로컬 웹 UI를 통해 장치를 설정할 수 있습니다. 서버에서 장치로 데이터를 복사하고 장치를 다시 Azure에 배송합니다. 사용자의 데이터는 Azure 데이터 센터에서 자동으로 장치에서 Azure로 업로드됩니다. 전체 프로세스는 Azure Portal의 Data Box 서비스에 의해 종단 간 추적됩니다.
...
- **보안** - Data Box Heavy에는 장치, 데이터 및 서비스에 대한 보안 보호 기능이 기본적으로 제공됩니다.
    - 나사 변조 방지 및 변조 방지 스티커로 보호되어 견고하게 포장되는 장치입니다. 
...
* **Data Box Heavy 장치** - 데이터를 안전하게 저장하는 외부가 견고한 물리적 장치입니다. 이 장치에는 800TB의 사용 가능한 저장소 용량이 있습니다. 

    
* **Data Box 서비스** – 다양한 지리적 위치에서 액세스할 수 있는 웹 인터페이스에서 Data Box Heavy 장치를 관리할 수 있는 Azure Portal의 확장입니다. Data Box Heavy 서비스를 사용하여 사용자의 Data Box 장치에 대한 일일 관리를 수행합니다. 서비스 작업에는 주문 만들기 및 관리, 경고 보기 및 관리, 공유 관리 방법이 포함됩니다.  

* **로컬 웹 사용자 인터페이스** – 로컬 네트워크에 연결할 수 있도록 장치를 구성한 다음, Data Box 서비스에 장치를 등록하는 데 사용되는 웹 기반 UI입니다. 로컬 웹 UI는 장치를 종료 및 다시 시작하고, 복사 로그를 보고, 서비스 요청을 위해 Microsoft 지원에 문의하는 데도 사용됩니다.
...
Data Box Heavy는 서비스가 배포된 지역, 장치가 배송된 국가 및 데이터를 전송하는 대상 Azure 저장소에 따라 데이터를 전송할 수 있습니다. 
