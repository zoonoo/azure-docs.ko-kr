...
---
# <a name="azure-data-box-disk-frequently-asked-questions"></a>Azure 데이터 상자 디스크: 자주 묻는 질문
...
### <a name="q-will-my-data-box-disk-cross-country-borders-during-shipping"></a>17. 배송 중에 내 Data Box Disk가 국가 경계를 넘을 수 있나요?
...
A. Data Box Disk에서 사용할 수 있는 도구 세트로는 세 가지 도구가 있습니다.
 - **데이터 상자 디스크 잠금 해제 도구**: 마이크로 소프트에서 제공 된 암호화 된 디스크의 잠금을 해제하려면이 도구를 사용합니다. 이 도구를 사용하여 디스크를 잠금 해제할 때 Azure Portal의 Data Box Disk 주문에 제공되는 암호를 입력해야 합니다. 
 - **데이터 상자 디스크 유효성 검사 도구**: 이 도구를 사용하여 Azure 명명 규칙에 따라 크기, 형식 및 Blob 이름을 확인합니다. 또한 이 도구는 복사된 데이터에 대한 체크섬을 생성하며, 생성된 체크섬은 Azure에 업로드된 데이터를 확인하는 데 사용됩니다.
 - **데이터 상자 디스크 분할 복사 도구**: 여러 디스크를 사용하고 모든 디스크에 분할 및 복사해야 하는 큰 데이터 집합이 있는 경우 이 도구를 사용합니다. 이 도구는 현재 Windows에서 사용할 수 있습니다. 이 도구는 관리 디스크에는 지원되지 않습니다. 또한 이 도구는 데이터 복사 시 유효성을 검사하므로, 이 도구를 사용할 때는 유효성 검사 단계를 건너뛸 수 있습니다.
...
A. Azure 파일은 데이터 상자 디스크에서 지원되지만 Azure 파일 동기화에서 잘 작동하지 않습니다. Azure 파일 동기화와 함께 파일 데이터를 사용하는 경우 메타데이터는 유지되지 않습니다.
