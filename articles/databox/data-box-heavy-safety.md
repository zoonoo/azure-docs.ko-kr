---
title: Azure Data Box Heavy 보안 Microsoft Docs
description: 안전 규칙, 지침 및 고려 사항에 대해 설명 하 고 Azure Data Box Heavy를 안전 하 게 설치 하 고 작동 하는 방법을 설명 합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: heavy
...
* 이 장비를 이동 하 고 떼는 경우 로컬 occupational 상태 및 안전 요구 사항을 준수 합니다.
...
* 시스템은 일반적인 office 환경에서 작동 하도록 설계 되었습니다. 다음 사이트를 선택 합니다.
    ...
    - 진동 또는 물리적인 충격의 원본에서 멀리.
    - 전기 장치로 생성 된 강력한 전자기 필드에서 격리 됩니다.
    - 제대로 접지 된 벽 콘센트가 제공 됩니다.
    - 는 제품의 기본 전원 연결 끊기 역할을 하기 때문에 전원 공급 장치 코드에 액세스할 수 있는 충분 한 공간이 제공 됩니다.
...
* 다음 전원 사양을 충족하도록 전기 과부하 방지 기능이 있는 적절한 전원을 제공하세요.
    ...
    - 전압: 100V AC~240V AC
    - 현재: 6 A ~ 10 A, 전원 코드 당 최대 4 개의 전원 코드가 제공 됩니다.
    - 주파수: 50Hz~60Hz
* 장치와 함께 제공 되는 것 외에는 AC 전원 코드를 수정 하거나 사용 하지 마세요. 전원 코드는 다음 조건을 충족 해야 합니다.
...
이 섹션에서는 Azure Data Box Heavy, 규정 모델 번호 DB020에 대 한 규제 정보를 포함 합니다.
...
- 다음 환경에서 작동하도록 설계되었습니다. 
    - 운영 온도: 41 ° ~ 95 ° F (5 °에서 35 ° C로)
    - 저장소 온도:-40 ° ~ 149 ° F (-40 ° ~ 65 ° C)
    - 상대 습도: 20% ~ 85% (비응축) 
    - 운영 이상: 테스트 된 최대 6560 피트 (최대 2000 미터)
...
이 디바이스는 FCC 규칙 파트 15 및 Industry Canada 라이선스 면제 RSS 표준을 따릅니다. 작업에는 다음 두 가지 조건이 적용 됩니다. 
...
* [Data Box Heavy 케이블 연결 및 연결](data-box-heavy-deploy-set-up.md)
