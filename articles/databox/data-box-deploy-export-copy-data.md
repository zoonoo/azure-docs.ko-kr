...
2. [액세스 공유 및 데이터 복사] 대화 상자에서 공유에 해당하는 **사용자 이름** 및 **암호** 를 복사합니다. **확인** 을 선택합니다.
    ...
    ![공유 자격 증명 가져오기, 액세스 공유 및 데이터 복사](media/data-box-deploy-export-copy-data/get-share-credentials-2.png)
...
5. Windows + R을 누르고 **실행** 창에서 `\\<device IP address>`를 지정합니다. **확인** 을 선택하여 파일 탐색기를 엽니다.
    
    ![파일 탐색기를 통해 공유에 연결하고 디바이스 IP 입력](media/data-box-deploy-export-copy-data/connect-shares-file-explorer-1.png)

    이제 공유가 폴더로 표시될 것입니다.
    
    ![파일 탐색기를 통해 공유에 연결하고 공유 보기](media/data-box-deploy-export-copy-data/connect-shares-file-explorer-2.png)
