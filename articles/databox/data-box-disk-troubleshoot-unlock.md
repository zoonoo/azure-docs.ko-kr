---
title: Azure 데이터 박스 디스크 문제 해결 디스크 잠금 해제 문제 | 마이크로 소프트 문서
...
---
# <a name="troubleshoot-disk-unlocking-issues-in-azure-data-box-disk"></a>Azure 데이터 상자 디스크에서 디스크 잠금 해제 문제 해결

이 문서는 Microsoft Azure 데이터 상자 디스크에 적용되며 잠금 해제 도구를 사용할 때 문제를 해결하는 데 사용되는 워크플로에 대해 설명합니다. 
...
| 치명적인 오류: 잘못된 매개 변수<br>매개 변수 이름: invalid_arg<br>사용량:<br>DataBoxDiskUnlock /PassKeys:<passkey_list_separated_by_semicolon><br><br>예: DataBoxDiskUnlock /PassKeys:passkey1;passkey2;passkey3<br>예: DataBoxDiskUnlock /SystemCheck<br>예: DataBoxDiskUnlock /Help<br><br>/PassKeys: Azure Data Box Disk 순서에서 이 암호를 가져옵니다. 암호는 디스크를 잠금 해제합니다.<br>/Help: 이 옵션은 cmdlet 사용량 및 예제에 대한 도움말을 제공합니다.<br>/SystemCheck: 이 옵션은 시스템이 도구를 실행하기 위한 요구 사항을 충족하는지 확인합니다.<br><br>종료하려면 아무 키나 누르세요. | 잘못된 매개 변수가 입력되었습니다. 허용되는 유일한 매개 변수는 /SystemCheck, /PassKey 및 /Help입니다.|


## <a name="unlock-issues-for-disks-when-using-a-windows-client"></a>Windows 클라이언트를 사용할 때 디스크 문제 잠금 해제

이 섹션에서는 데이터 복사본에 Windows 클라이언트를 사용할 때 데이터 상자 디스크를 배포하는 동안 직면한 몇 가지 주요 문제에 대해 자세히 설명합니다.
...
Data Box Disk의 잠금을 해제하려면 Data Box Disk 잠금 해제 도구를 사용하고 Azure Portal에서 암호를 제공해야 합니다. 자세한 내용은 [자습서: Azure 데이터 상자 디스크 압축 해제, 연결 및 잠금 해제로](data-box-disk-deploy-set-up.md#connect-to-disks-and-get-the-passkey)이동하십시오.
 
### <a name="issue-could-not-unlock-or-verify-some-volumes-contact-microsoft-support"></a>문제: 일부 볼륨의 잠금을 해제하거나 확인할 수 없습니다. Microsoft 지원에 문의하세요.
...
- [유효성 검사 문제를 해결하는](data-box-disk-troubleshoot.md)방법에 대해 알아봅니다.
