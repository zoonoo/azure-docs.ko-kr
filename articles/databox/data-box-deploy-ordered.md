...
Azure Data Box는 빠르고 쉽게 신뢰할 수 있는 방식으로 온-프레미스 데이터를 Azure로 가져올 수 있는 하이브리드 솔루션입니다. Microsoft에서 제공한 80TB(사용 가능한 용량) 저장소 장치에 데이터를 전송한 다음, 장치를 다시 제공합니다. 그런 다음, 이 데이터는 Azure에 업로드됩니다.
...
장치를 배포하기 전에 Data Box 서비스 및 장치에 대해 다음 필수 구성 요소를 완료합니다.
...
장치를 주문하려면 Azure Portal에서 다음 단계를 수행합니다.
...
장치를 사용할 수 없는 경우 알림을 받습니다. 장치를 사용할 수 있으면 Microsoft에서는 배송할 장치를 확인하고 배송을 준비합니다. 장치를 준비하는 동안 수행되는 작업은 다음과 같습니다.

- 장치와 연결된 각 저장소 계정에 대해 SMB 공유가 생성됩니다. 
- 각 공유에 대한 사용자 이름 및 암호와 같은 액세스 자격 증명이 생성됩니다.
- 장치의 잠금을 해제하는 데 도움이 되는 장치 암호도 생성됩니다. 
- Data Box는 언제든지 장치에 대한 권한이 없는 액세스를 방지하기 위해 잠겨 있습니다.

장치 준비가 완료되면 포털에서는 해당 주문을 **처리됨** 상태로 표시합니다.

![Data Box 주문 처리됨](media/data-box-overview/data-box-order-status-processed.png)

그런 다음, Microsoft에서는 장치를 준비하고 지역 배송업체를 통해 발송합니다. 장치가 배송되면 추적 번호를 받게 됩니다. 포털에서 해당 작업을 **발송됨** 상태로 표시합니다.
