...
이 자습서에서는 중간 호스트가 필요 없는 데이터 복사 서비스를 사용하여 데이터를 수집하는 방법을 설명합니다. 데이터 복사 서비스는 Azure Data Box Heavy에서 로컬로 실행되며, SMB를 통해 NAS(Network Attached Storage) 디바이스에 연결되고 데이터를 Data Box Heavy에 복사합니다.
...
- 데이터를 복사하는 동안 데이터 크기가 [Azure 스토리지 및 Data Box Heavy 제한](data-box-heavy-limits.md) 문서에 설명된 크기 제한을 준수해야 합니다.
...
1. Data Box Heavy 디바이스의 로컬 웹 UI에서 **관리** > **데이터 복사**로 이동합니다.
...
3. **작업 구성 및 시작** 대화 상자에서 다음 필드를 채웁니다.
    ...
    |**파일 최적화**              |이 기능을 사용하는 경우 1MB 미만의 파일만 수집하는 동안 압축됩니다. 이 압축으로 작은 파일에 대한 데이터 복사 속도가 빨라집니다. 또한 파일 수가 디렉터리 수보다 훨씬 많은 경우 상당한 시간이 절약됩니다.        |
...
> [!IMPORTANT]
> 동일한 지침을 반복하여 Data Box Heavy의 두 번째 노드에 데이터를 복사합니다.
...
Data Box Heavy 디바이스를 Microsoft로 다시 배송하는 방법을 알아보려면 다음 자습서로 계속 진행하세요.

> [!div class="nextstepaction"]
> [Microsoft로 Azure Data Box Heavy 디바이스 배송](./data-box-heavy-deploy-picked-up.md)