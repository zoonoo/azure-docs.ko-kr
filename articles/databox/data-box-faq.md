...
Microsoft Azure Data Box 하이브리드 솔루션을 사용하면 이전 장치를 통해 테라바이트 단위의 데이터를 빠르고 저렴하게 신뢰할 수 있는 방식으로 Azure로 보낼 수 있습니다. 이 FAQ에는 Azure Portal에서 Data Box를 사용할 때 제기될 수 있는 질문과 대답이 포함되어 있습니다. 
...
a. Azure Data Box를 사용하면 Azure에서 테라바이트 단위의 데이터를 빠르고 저렴하게 신뢰할 수 있는 방식으로 전송할 수 있습니다. Azure Portal을 통해 Data Box 장치를 주문합니다. Microsoft가 지역 사업자를 통해 80TB용량의 사용 가능한 저장 장치를 배송합니다. 

장치를 받게 되면 로컬 웹 UI를 사용하여 신속하게 설정합니다. 서버에서 장치로 데이터를 복사하고 장치를 다시 Azure에 배송합니다. 사용자의 데이터는 Azure 데이터 센터에서 자동으로 장치에서 Azure로 업로드됩니다. 전체 프로세스는 Azure Portal의 Data Box 서비스에 의해 종단 간 추적됩니다.
...
a. Data Box는 10일에 대한 명목 요금으로 사용할 수 있습니다. Azure Portal에서 주문 중에 제품 모델을 선택하면 장치 요금이 표시됩니다. 또한 배송료는 무료이지만, Azure 저장소에 대한 비용이 부과됩니다. 자세한 내용은 [Azure Data Box 가격 책정](https://azure.microsoft.com/pricing/details/storage/databox/)을 참조하세요. 
...
a. Data Box의 원시 용량은 100TB, 사용 가능한 용량은 80TB입니다. Data Box로 최대 80TB의 데이터를 전송할 수 있습니다. 더 많은 데이터를 전송하려면 장치를 더 주문해야 합니다.
...
### <a name="q-i-ordered-a-couple-of-data-box-devices-i-am-not-able-to-create-any-additional-orders-why-would-this-be"></a>Q. Data Box 장치를 여러 개 주문했습니다. 다른 주문을 더 만들 수 없습니다. 왜 그럴까요?
a. 상거래 범위(선택한 국가 및 지역 조합)당 구독별로 최대 5개의 활성 주문이 가능합니다. 장치를 더 주문하려면 Microsoft 지원에 문의하여 구독 한도를 늘리세요.
...
a. 주문이 이루어지면 해당 주문에 장치가 가능한지 여부를 확인합니다. 장치가 가능하다면 10일 안에 출고합니다. 수요가 몰리는 기간이 있을 수 있습니다. 이런 경우에는 주문이 대기 상태가 되고 Azure Portal에서 상태 변경을 추적할 수 있습니다. 주문이 90일 안에 이행되지 않으면 자동으로 취소됩니다. 
...
a.  Azure Portal에서 해당 Data Box 주문, **장치 세부 정보**로 차례로 이동합니다. 잠금 해제 암호를 복사합니다. 이 암호를 사용하여 Data Box에서 로컬 웹 UI에 로그인합니다. 자세한 내용은 [자습서: Azure Data Box 압축 풀기, 케이블, 연결](data-box-deploy-set-up.md)을 참조하세요.
...
a.  Data Box에는 80TB의 사용 가능한 저장소 용량이 있습니다. 크기가 40 ~ 80TB에 해당하는 데이터에는 단일 Data Box 장치를 사용할 수 있습니다. 이보다 큰 데이터의 경우 최대 500TB까지 여러 Data Box 장치를 주문할 수 잇습니다. 500TB가 넘는 데이터는 Data Box Heavy에 가입하세요.  
...
a. 운송 중 Data Box 보호를 위해 여러 보안 기능이 구현되어 있습니다. 여기에는 손상 증명 봉인, 하드웨어 및 소프트웨어 변조 탐지, 장치 잠금 해제 암호 등을 비롯한 여러 기능이 포함됩니다. 자세한 내용은 [Azure Data Box 보안 및 데이터 보호](data-box-security.md)를 참조하세요.

### <a name="q-how-do-i-copy-the-data-to-the-data-box"></a>Q. Data Box에 어떻게 데이터를 복사하나요? 
a.  SMB 클라이언트를 사용할 경우 Robocopy, Diskboss 또는 Windows 파일 탐색기 끌어서 놓기와 같은 SMB 복사 도구를 사용하여 데이터를 장치에 복사합니다. 
...
## <a name="ship-device"></a>장치 배송

<!--### Q. How do I schedule a pickup for my Data Box?--> 

### <a name="q-my-device-was-delivered-but-the-device-seems-to-be-damaged-what-should-i-do"></a>Q. 내 장치가 배송되었으나 손상된 것 같습니다. 어떻게 해야 하나요?
a. 장치가 손상된 상태로 도착했거나 변조의 증거가 있다면 장치를 사용하지 않습니다. [Microsoft 지원에 문의](data-box-disk-contact-microsoft-support.md)하고 가능한 빨리 장치를 반환하세요. 교체 장치를 위한 새 Data Box 주문을 만들 수도 있습니다. 이런 경우 교체 장치에 대한 요금이 청구되지 않습니다.
...
a.  운송 중에는 Data Box의 다음 기능을 통해 데이터를 보호합니다.
 ...
 - 장치가 잠겨 있으며 잠금 해제 암호가 있어야 들어가 데이터에 액세스할 수 있습니다.
자세한 내용은 [Data Box 보안 기능](data-box-security.md)을 참조하세요.  

### <a name="q-i-have-finished-prepare-to-ship-and-shut-down-the-device-can-i-still-add-more-data-to-data-box"></a>Q. 배송 준비를 마치고 장치를 종료했습니다. Data Box에 데이터를 추가할 수 있나요?
a. 예. 장치를 켜고 데이터를 더 추가할 수 있습니다. 데이터 복사를 마친 후에는 **배송 준비**를 다시 실행해야 합니다.
