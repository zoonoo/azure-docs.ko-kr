...
|서버에서 요청을 인증 하지 못했습니다. 오류가 발생 하 여 REST API 인증에 실패 합니다. 권한 부여 헤더의 값이 서명을 포함 하 여 올바른 형식 인지 확인 합니다. ErrorCode: AuthenticationFailed. |이 문제가 발생 하는 이유 중 하나는 장치 시간이 Azure의 장치와 동기화 되지 않는 경우입니다. 시간이 많이 걸리는 경우 REST API를 통해 Data Box에 데이터를 복사 하려고 하면 REST API 인증이 중단 됩니다. 이 경우 아웃 바운드 UDP 123 포트를 열어에 대 한 액세스를 허용할 수 있습니다 `time.windows.com` . 장치 시간이 Azure와 동기화 되 면 인증이 성공 합니다. |
