...
을 사용 하 여 `context.bindings.<name from function.json>`blob 데이터에 액세스 합니다.
...
특성 `@BlobInput` 은 함수를 트리거한 blob에 대 한 액세스를 제공 합니다. 특성에 바이트 배열을 사용 하는 경우를로 `dataType` `binary`설정 합니다. 자세한 내용은 [입력 예](#input---example) 를 참조 하세요.
...
특성 `@BlobOutput` 은 함수를 트리거한 blob에 대 한 액세스를 제공 합니다. 특성에 바이트 배열을 사용 하는 경우를로 `dataType` `binary`설정 합니다. 자세한 내용은 [출력 예제](#output---example) 를 참조 하십시오.
