...
- [Azure Portal에서 첫 번째 함수 만들기](./functions-get-started.md)의 지침을 따르고 **리소스 정리** 단계는 수행하지 않습니다. 해당 빠른 시작에서는 함수 앱과, 여기서 사용할 함수를 만듭니다.
...
1. Azure Portal에서 [Azure Portal에서 첫 번째 함수 만들기](./functions-get-started.md)에서 만든 함수 앱에 대한 함수 앱 페이지를 엽니다. 페이지를 열려면 **함수 앱** 을 검색한 후 선택합니다. 그런 후, 함수 앱을 선택합니다.
...
1. 테스트가 아래 이미지와 일치하는지 확인하고 **실행** 을 선택합니다. 
    ...
    **요청 본문** 에 `name` 값 *Azure* 가 들어 있습니다. 이 값은 함수가 호출될 때 만들어지는 큐 메시지에 표시됩니다.
    ...
    여기서 **실행** 을 선택하는 대신 브라우저에 URL을 입력하고 쿼리 문자열에 `name` 값을 지정하여 함수를 호출할 수 있습니다. 브라우저 메서드는 [이전 빠른 시작](./functions-get-started.md)에 있습니다.
...
1. **애플리케이션 설정** 에서 **AzureWebJobsStorage** 를 선택합니다.
    ...
    :::image type="content" source="./media/functions-integrate-storage-queue-output-binding/function-find-storage-account.png" alt-text="스크린샷에는 AzureWebJobsStorage가 선택된 구성 페이지가 표시됩니다." border="true":::
