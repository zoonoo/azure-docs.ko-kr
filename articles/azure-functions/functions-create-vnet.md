...
1. 에 **기본 사항** 탭에서 이미지 아래의 표에 지정 된 대로 VM 설정을 사용 합니다.
    ...
    | **[Region](https://azure.microsoft.com/regions/)** | (유럽) 유럽 서 부 | VM에 액세스 하는 함수를 거의 또는 가까운 지역을 선택 합니다. |
    | **크기** | B1s | 선택할 **크기를 변경** 고 1 개의 vCPU 및 1GB의 메모리가 있는 B1s 표준 이미지를 선택 합니다. |
...
1. 함수 앱 선택 **프록시** >  **+** , 다음 이미지 아래의 표에 프록시 설정을 사용 합니다.
    ...
    | 설정  | 제안 값  | 설명      |
    | -------- | ---------------- | ---------------- |
    | **Name** | PlAnT | 이름을 임의의 값일 수 있습니다. 프록시를 식별 하는 것이 됩니다. |
