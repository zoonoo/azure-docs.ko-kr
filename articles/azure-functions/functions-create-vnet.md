...
1. **기본 사항** 탭에서 이미지 아래 표에 지정 된 대로 VM 설정을 사용 합니다.
    ...
    | **[리소스 그룹](../azure-resource-manager/management/overview.md)**  | myResourceGroup | 또는 `myResourceGroup`함수 앱을 사용 하 여 만든 리소스 그룹을 선택 합니다. 함수 앱, WordPress VM 및 호스팅 계획에 동일한 리소스 그룹을 사용 하면이 자습서를 완료 하면 리소스를 더 쉽게 정리할 수 있습니다. |
...
1. **가상 네트워크 만들기**에서 이미지 아래 표의 설정을 사용 합니다.
    ...
    | **주소 범위** (서브넷) | 10.10.1.0/24   | 서브넷 크기는 서브넷에 추가할 수 있는 인터페이스 수를 정의 합니다. 이 서브넷은 WordPress 사이트에서 사용 됩니다.  서브넷 `/24` 은 254 호스트 주소를 제공 합니다. |
...
1. **네트워크 기능 상태**에서 이미지 아래 표의 설정을 사용 합니다.
    ...
    | **주소 범위** | 10.10.2.0/24   | 서브넷 크기는 프리미엄 계획 함수 앱이 확장할 수 있는 총 인스턴스 수를 제한 합니다. 이 예제에서는 254 `/24` 사용 가능한 호스트 주소를 사용 하는 서브넷을 사용 합니다. 이 서브넷은 과도 하 게 프로 비전 되었지만 계산 하기 쉽습니다. |
...
1. 함수 앱에서 **프록시** > **+** 를 선택한 다음 이미지 아래의 테이블에서 프록시 설정을 사용 합니다.
    ...
    | **백 엔드 URL** | http://<YOUR_VM_IP>/wp-content/themes/twentyseventeen/assets/images/header.jpg | 을 `<YOUR_VM_IP>` 앞에서 만든 WordPress VM의 IP 주소로 바꿉니다. 이 매핑은 사이트에서 단일 파일을 반환 합니다. |
