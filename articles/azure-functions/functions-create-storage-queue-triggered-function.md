...
1. **함수**를 선택한 다음, **+ 추가**를 선택하여 새 함수를 추가합니다.
   ...
   :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-app-quickstart-choose-template.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. **Azure Queue Storage 트리거** 템플릿을 선택합니다.

1. 이미지 아래의 표에 지정된 설정을 사용합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-create-queue-storage-trigger-portal.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::
...
1. **함수 만들기**를 선택하여 함수를 만듭니다.
    ...
    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-create-queue-storage-trigger-portal-3.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::
...
1. 함수의 **개요** 페이지에서 리소스 그룹을 선택합니다.
    ...
    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-resource-group.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. 리소스 그룹의 스토리지 계정을 찾아 선택합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-account-access.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. **큐**를 선택하고 **+ 큐**를 선택합니다. 

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-add-queue.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. **이름** 필드에 `myqueue-items`를 입력한 다음, **만들기**를 선택합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-name-queue.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::
...
1. Azure Portal로 돌아가서 함수를 찾은 후 페이지 맨 아래에 있는 **로그**를 확장하고 로그 스트리밍이 일시 중지되지 않았는지 확인합니다.
    ...
    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-queue-storage-log-expander.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. 별도의 브라우저 창에서 Azure Portal의 리소스 그룹으로 이동하고 스토리지 계정을 선택합니다.

1. **큐**을 선택하고 **myqueue-items** 컨테이너를 선택합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-queue.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. **메시지 추가**를 선택하고 "Hello World!"를 **메시지 텍스트**에 입력합니다. **확인**을 선택합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/functions-storage-queue-test.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::

1. 몇 초 동안 기다린 다음 함수 로그로 돌아가서 새 메시지를 큐에서 읽었는지 확인합니다.

    :::image type="content" source="./media/functions-create-storage-queue-triggered-function/function-app-in-portal-editor.png" alt-text="함수 앱을 성공적으로 만들었습니다." border="true":::
