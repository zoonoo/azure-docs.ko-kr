...
* 다양한 수준의 가상 네트워크 연결 및 확장 옵션을 통해 다중 테넌트 인프라에서 실행되는 계획 옵션 중에서 선택할 수 있습니다.
    * [소비 계획은](functions-scale.md#consumption-plan) 부하에 따라 동적으로 확장되며 최소한의 네트워크 격리 옵션을 제공합니다.
    * 또한 [프리미엄 플랜은](functions-scale.md#premium-plan) 동적으로 확장되고 보다 포괄적인 네트워크 격리를 제공합니다.
    * Azure [앱 서비스 계획은](functions-scale.md#app-service-plan) 고정된 규모로 작동하며 프리미엄 플랜과 유사한 네트워크 격리를 제공합니다.
...
|[가상 네트워크 통합](#virtual-network-integration)|❌예|✅예(지역)|✅예(지역 및 게이트웨이)|✅예|
|[가상 네트워크 트리거(HTTP가 아닌 경우)](#virtual-network-triggers-non-http)|❌예| ✅예 |✅예|✅예|
|[하이브리드](#hybrid-connections) 연결(Windows만)|❌예|✅예|✅예|✅예|
|[아웃바운드 IP 제한](#outbound-ip-restrictions)|❌예| ✅예|✅예|✅예|
...
가상 네트워크 통합을 사용하면 함수 앱이 가상 네트워크 내부의 리소스에 액세스할 수 있습니다.
Azure Functions는 두 가지 종류의 가상 네트워크 통합을 지원합니다.
...
## <a name="regional-virtual-network-integration"></a>지역 별 가상 네트워크 통합
...
함수 앱을 만들 때 Blob, Queue 및 테이블 저장소를 지원하는 범용 Azure Storage 계정을 만들거나 링크해야 합니다. 현재 이 계정에서가상 네트워크 제한을 사용할 수 없습니다. 함수 앱에 사용 중이던 저장소 계정에 가상 네트워크 서비스 끝점을 구성하면 해당 구성이 앱을 중단합니다.

자세한 내용은 [저장소 계정 요구 사항을](./functions-create-function-app-portal.md#storage-account-requirements)참조하십시오.

## <a name="use-key-vault-references"></a>키 볼트 참조 사용

Azure Key Vault 참조를 사용하여 코드를 변경하지 않고도 Azure Key Vault의 암호를 Azure Functions 응용 프로그램에서 사용할 수 있습니다. Azure Key Vault는 액세스 정책 및 감사 기록에 대한 전체 제어와 함께 중앙 집중식 비밀 관리를 제공하는 서비스입니다.

현재 [키 볼트 참조는](../app-service/app-service-key-vault-references.md) 서비스 끝점으로 키 자격 증명 모음을 보호하면 작동하지 않습니다. 가상 네트워크 통합을 사용하여 키 자격 증명 모음에 연결하려면 응용 프로그램 코드에서 Key Vault를 호출해야 합니다.
...
Premium 계획을 실행하면 비 HTTP 트리거 함수를 가상 네트워크 내에서 실행되는 서비스에 연결할 수 있습니다. 이렇게 하려면 함수 앱에 대한 가상 네트워크 트리거 지원을 사용하도록 설정해야 합니다. **가상 네트워크 트리거 지원** 설정은 기능 앱 설정 아래의 Azure [포털에서](https://portal.azure.com) 찾을 **수 있습니다.**

![가상 네트워크 토글](media/functions-networking-options/virtual-network-trigger-toggle.png)
...
> [!IMPORTANT]
> 가상 네트워크 트리거 지원을 사용하도록 설정하면 이전 테이블에 표시된 트리거 유형만 응용 프로그램과 동적으로 확장됩니다. 테이블에 없는 트리거는 계속 사용할 수 있지만 미리 준비된 인스턴스 수를 초과하여 확장되지는 않습니다. 트리거의 전체 목록은 트리거 [및 바인딩을](./functions-triggers-bindings.md#supported-bindings)참조하십시오.
...
예를 들어 Azure Cosmos DB가 가상 네트워크에서만 트래픽을 허용하도록 구성한다고 가정합니다. 이 경우 해당 가상 네트워크와 가상 네트워크 통합을 제공하는 앱 서비스 계획에 함수 앱을 배포해야 합니다. 통합을 사용하면 Azure Cosmos DB 리소스에 의해 함수를 트리거할 수 있습니다.
...
>[!IMPORTANT]
> 하이브리드 연결은 Windows 계획에서만 지원됩니다. Linux는 지원되지 않습니다.
