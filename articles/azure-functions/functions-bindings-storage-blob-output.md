---
title: Azure 함수에 대한 Azure Blob 저장소 출력 바인딩
...
ms.locfileid: "79277246"
---
# <a name="azure-blob-storage-output-binding-for-azure-functions"></a>Azure 함수에 대한 Azure Blob 저장소 출력 바인딩

출력 바인딩을 사용하면 Azure Function에서 Blob 저장소 데이터를 수정하고 삭제할 수 있습니다.
...
특성을 `@BlobOutput` 사용하면 함수를 트리거한 Blob에 액세스할 수 있습니다. 특성이 있는 바이트 배열을 사용하는 `dataType` 경우 `binary`을 로 설정합니다. 자세한 내용은 [출력 예제를](#example) 참조하십시오.
...
|**경로** |**BlobPath** | Blob 컨테이너에 대한 경로입니다. |
...
함수 매개 변수를 다음 유형으로 선언하여 Blob 저장소에 쓸 수 있습니다.

* 문자열은`func.Out(str)`
* 스트림으로`func.Out(func.InputStream)`

자세한 내용은 [출력 예제를](#example) 참조하십시오.
