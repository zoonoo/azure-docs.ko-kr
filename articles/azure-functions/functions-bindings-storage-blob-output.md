...
특성 `@BlobOutput` 은 함수를 트리거한 blob에 대 한 액세스를 제공 합니다. 특성에 바이트 배열을 사용 하는 경우를로 `dataType` `binary`설정 합니다. 자세한 내용은 [출력 예제](#example) 를 참조 하십시오.
...
|**방향도** | 해당 없음 | 출력 바인딩에 대해 `out`로 설정해야 합니다. 예외는 [사용](#usage) 섹션에서 표시됩니다. |
