...
Azure Functions 버전 2.x [바인딩](./functions-triggers-bindings.md) functions 런타임은에서 별도 패키지로 사용할 수 있습니다. .NET 함수 바인딩을 통해 NuGet 패키지에 액세스 하는 동안 확장 번들 구성 설정을 통해 모든 바인딩에 다른 함수 액세스를 허용 합니다.

바인딩 확장에 관련 된 다음 항목을 고려 합니다.

- 바인딩 확장 함수에서 명시적으로 등록 되지 않습니다 경우는 제외 1.x [만들기를 C# Visual Studio 2017을 사용 하 여 클래스 라이브러리](#local-csharp)합니다.

- HTTP 및 타이머 트리거 기본적으로 지원 됩니다 및 확장이 필요 하지 않습니다.
