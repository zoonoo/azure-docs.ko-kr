...
이 항목에서는 포털에서 Azure Functions 빠른 시작을 사용하여 HTTP 트리거에서 호출하는 간단한 "hello world" Node.js 함수를 만드는 방법을 보여 줍니다. 포털에서 이러한 단계는 수행하는 방법을 보여 주는 짧은 비디오를 시청할 수도 있습니다.
...
3. 다른 브라우저 창이나 탭에서 동일한 함수의 실행을 트리거하려면 **개발** 탭에서 **함수 URL** 값을 복사하여 브라우저 주소 표시줄에 붙여 넣은 다음 쿼리 문자열 값 `&name=yourname`을 추가하고 Enter 키를 누릅니다. 같은 정보가 로그에 기록되고 이전처럼 브라우저에 "hello" 응답이 표시됩니다.
...
* [Azure 앱 서비스 정의](../app-service/app-service-value-prop-what-is.md)  
   Azure Functions는 배포, 환경 변수 및 진단과 같은 핵심 기능을 위해 Azure 앱 서비스 플랫폼을 활용합니다. 
