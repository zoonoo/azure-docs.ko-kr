...
버전 2.x부터 런타임은 언어 확장성 모델을 사용 하 고 함수 앱의 모든 함수는 동일한 언어를 공유 해야 합니다. 함수 앱의 함수 언어는 앱을 만들 때 선택 되며, 함수 [\_작업자\_런타임](functions-app-settings.md#functions_worker_runtime) 설정에서 유지 관리 됩니다. 

Azure Functions 1.x 실험적 언어는 새 모델을 사용할 수 없으므로 2.x에서 지원 되지 않습니다. 다음 표는 각 런타임 버전에서 현재 지원되는 프로그래밍 언어를 나타냅니다.

[!INCLUDE [functions-supported-languages](../../includes/functions-supported-languages.md)]

자세한 내용은 [지원 되는 언어](supported-languages.md)를 참조 하세요.
...
앱 구성을 수동으로 업데이트 하 여 "내부" 업그레이드를 수행할 수 있지만, 1.x에서 상위 버전으로 이동 하는 데는 몇 가지 주요 변경 사항이 포함 되어 있습니다. 예를 들어 c #에서 디버깅 개체는에서 `TraceWriter` 로 `ILogger`변경 됩니다. 새 버전 2.x 프로젝트를 만들어 최신 버전의 3(sp3) 템플릿을 기반으로 업데이트 된 함수로 시작 합니다.
...
* 모니터링을 개선 하기 위해이 [`AzureWebJobsDashboard`](functions-app-settings.md#azurewebjobsdashboard) 설정을 사용 하는 포털의 WebJobs 대시보드는 [`APPINSIGHTS_INSTRUMENTATIONKEY`](functions-app-settings.md#appinsights_instrumentationkey) 설정을 사용 하는 Azure 애플리케이션 Insights로 바뀝니다. 자세한 내용은 [Azure Functions 모니터링](functions-monitoring.md)을 참조하세요.

* 함수 앱의 모든 함수가 동일한 언어를 공유해야 합니다. 함수 앱을 만들 때 앱의 런타임 스택을 선택해야 합니다. 런타임 스택은 응용 프로그램 설정의 [`FUNCTIONS_WORKER_RUNTIME`](functions-app-settings.md#functions_worker_runtime) 값으로 지정 됩니다. 이 요구 사항은 공간 효율성 및 시작 시간을 개선하기 위해 추가되었습니다. 로컬로 개발하는 경우 [local.settings.json 파일](functions-run-local.md#local-settings-file)에 이 설정을 포함해야 합니다.
...
* HTTP concurrency 제한는 기본적으로 소비 계획 함수에 대해 구현 되며 인스턴스당 기본값 100을 사용 합니다. 이 [`maxConcurrentRequests`](functions-host-json.md#http) 설정은 호스트 json 파일의 설정에서 변경할 수 있습니다.
...
* 또는 반환 값을 `context.done` 통해 할당 된 출력 바인딩은 이제의 `context.bindings`설정과 동일 하 게 동작 합니다.

* 타이머 트리거 개체는 camelCase입니다.

* 이벤트 허브가 이진으로 `dataType` 트리거된 함수는 대신의 `binary` 배열을 받습니다. `string`

* HTTP 요청 페이로드에는를 통해 `context.bindingData.req`더 이상 액세스할 수 없습니다.  여전히에서 `context.req` `context.bindings`입력 매개 변수, 및로 액세스할 수 있습니다.
...
Azure에서 게시 된 앱이 사용 하는 함수 런타임의 버전은 응용 프로그램 설정 [`FUNCTIONS_EXTENSION_VERSION`](functions-app-settings.md#functions_extension_version) 에 따라 결정 됩니다. 지원 되는 주요 런타임 버전 값은 다음과 같습니다.
...
Visual Studio Code 개발의 경우 설치된 도구의 버전과 일치하도록 `azureFunctions.projectRuntime`에 대한 사용자 설정을 업데이트해야 할 수도 있습니다.  이 설정은 함수 앱을 만드는 동안 사용되는 템플릿 및 언어도 업데이트합니다.  에서 `~3` 앱을 만들려면 `azureFunctions.projectRuntime` 사용자 설정을로 `~3`업데이트 합니다.
...
로컬로 실행 하는 데 필요한 [핵심 도구의 3. x 버전을 설치](functions-run-local.md#install-the-azure-functions-core-tools) 하 여 Java 앱을 버전 2.x에서 2.x로 마이그레이션할 수 있습니다.  앱이 버전 3.x에서 로컬로 실행 되 고 있는지 확인 한 후 다음 예제와 같이 앱의 `POM.xml` 파일을 업데이트 하 `FUNCTIONS_EXTENSION_VERSION` 여 설정을 `~3`로 수정 합니다.
