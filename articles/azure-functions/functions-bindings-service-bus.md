...
ServiceBus 바인딩은 *형식이* 로 `serviceBusTrigger`설정 된 *함수인 json* 에 정의 됩니다.
...
`func.ServiceBusMessage`   *_Py\__ 의\_* 코드는 함수에서 큐 메시지를 읽을 수 있도록 매개 변수를 선언 합니다.
...
ServiceBue 바인딩 정의는 *type* 이로 `serviceBus`설정 된 *함수인 json* 에 정의 되어 있습니다.
