...
`UserId`바인딩 식[을 사용하여 바인딩의 ](./functions-bindings-expressions-patterns.md) 속성을 `{headers.x-ms-client-principal-id}` 또는 `{headers.x-ms-client-principal-name}` 헤더 중 하나의 값으로 설정할 수 있습니다.
