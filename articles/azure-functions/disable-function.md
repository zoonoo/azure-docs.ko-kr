...
ms.locfileid: "83115571"
---
# <a name="how-to-disable-functions-in-azure-functions"></a>Azure Functions에서 함수를 사용하지 않도록 설정하는 방법

이 문서에서는 Azure Functions에서 함수를 사용하지 않도록 설정하는 방법을 설명합니다. 함수를 *사용 하지 않도록 설정* 하는 것은 런타임이 함수에 대해 정의 된 자동 트리거를 무시 하도록 하는 것입니다. 이렇게 하면 전체 함수 앱을 중지 하지 않고 특정 함수가 실행 되지 않도록 방지할 수 있습니다.

함수를 사용 하지 않도록 설정 하는 권장 방법은 형식의 앱 설정을 사용 하는 것입니다 `AzureWebJobs.<FUNCTION_NAME>.Disabled` . [Azure CLI](/cli/azure/) 사용 및 [Azure Portal](https://portal.azure.com)의 함수 **관리** 탭에서 비롯 하 여 다양 한 방법으로이 응용 프로그램 설정을 만들고 수정할 수 있습니다. 
...
Azure CLI에서 명령을 사용 하 여 [`az functionapp config appsettings set`](/cli/azure/functionapp/config/appsettings#az-functionapp-config-appsettings-set) 앱 설정을 만들고 수정 합니다. 다음 명령은 `QueueTrigger` 로 설정 된 응용 프로그램 설정을 만들어 라는 함수를 사용 하지 않도록 `AzureWebJobs.QueueTrigger.Disabled` 설정 `true` 합니다. 
...
함수를 다시 사용 하도록 설정 하려면 값을 사용 하 여 동일한 명령을 다시 실행 합니다 `false` .
...
함수의 **개요** 페이지에서 **사용** 및 **사용 안 함** 단추를 사용할 수도 있습니다. 이러한 단추는 앱 설정을 만들고 삭제 하는 방식으로 작동 `AzureWebJobs.<FUNCTION_NAME>.Disabled` 합니다.

![함수 상태 스위치](media/disable-function/function-state-switch.png)

> [!NOTE]  
> 포털 통합 테스트 기능은 설정을 무시 합니다 `Disabled` . 즉, 사용 하지 않도록 설정 된 함수는 포털의 **테스트** 창에서 시작할 때 계속 실행 됩니다. 
...
클래스 라이브러리 함수에서 특성을 사용 `Disable` 하 여 함수가 트리거되지 않도록 할 수도 있습니다. 다음 예제에 표시된 것처럼 생성자 매개 변수 없이도 이 특성을 사용할 수 있습니다.
...
버전 1.x에서는 `disabled` *함수인 json* 파일의 속성을 사용 하 여 런타임에서 함수를 트리거하지 않도록 지시할 수도 있습니다. 이 메서드는 c # 스크립트 및 JavaScript와 같은 스크립트 언어에 대해서만 작동 합니다. `disabled`속성은 `true` 또는 앱 설정의 이름으로 설정할 수 있습니다.
...
Azure Portal에서 파일을 편집 하거나 함수의 **개요** 페이지에서 **사용** 및 **사용 안 함** 단추를 사용할 수 있습니다. 포털 스위치는 *function.json* 파일을 변경하면 작동됩니다.
