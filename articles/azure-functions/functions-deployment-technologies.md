...
Azure 함수 앱을 포함 한 zip 파일을 푸시할 수 있습니다. 선택적으로 지정할 수도 있습니다 시작 앱 [패키지에서 실행](run-functions-from-deployment-package.md) 모드입니다.
...
함수 앱이 포함 된 Linux 컨테이너 이미지를 배포 합니다.
...
해당 리포지토리의 코드에 대 한 업데이트 배포를 트리거하는 함수 앱에 git 리포지토리를 연결할 수 있습니다. 자세한 내용은 잠시 살펴 합니다 [Kudu Wiki](https://github.com/projectkudu/kudu/wiki/VSTS-vs-Kudu-deployments)합니다.
...
있습니다 Git를 사용 하 여 Azure Functions를 로컬 컴퓨터에서 코드를 푸시합니다.
...
Azure Functions에 Dropbox 및 OneDrive에서 콘텐츠를 동기화 할 수 있습니다.
...
사용 하면 직접 Azure Functions로 전송 파일.
...
함수 앱에 파일을 직접 편집 하면 포털 기반 편집기를 사용 하 여 (기본적으로 클릭 하면 언제 든 지 배포 **저장할**).
...
다음 표에서 운영 체제 및 포털 편집 지원 되는 언어를 보여 줍니다.
...
<sup>*</sup> 포털 편집 전용 계획을 사용 하 여 Linux에서 함수에 대 한 HTTP 및 타이머 트리거만 사용 됩니다.
...
다음 문서에서 함수 앱을 배포 하는 방법에 대 한 자세한 정보: 
