...
| API 범주 | Reason | 해결 방법 |
| ------------ | ------ | ---------- |
| 날짜 및 시간  | 반환 된 값은 각 재생 마다 다르므로 현재 날짜 또는 시간을 반환 하는 Api는 비결 정적입니다. | 재생에 안전한 .NET 또는 JavaScript의 **CurrentUtcDateTime** Api에서 [**CurrentUtcDateTime**](https://azure.github.io/azure-functions-durable-extension/api/Microsoft.Azure.WebJobs.DurableOrchestrationContext.html#Microsoft_Azure_WebJobs_DurableOrchestrationContext_CurrentUtcDateTime) api를 사용 합니다. |
| Guid 및 Uuid  | 임의 GUID 또는 UUID를 반환 하는 Api는 생성 된 값이 각 재생 마다 다르기 때문에 비결 정적입니다. | .NET의 [**newguid**](https://azure.github.io/azure-functions-durable-extension/api/Microsoft.Azure.WebJobs.DurableOrchestrationContext.html#Microsoft_Azure_WebJobs_DurableOrchestrationContext_NewGuid) 나 JavaScript의 **newguid** 를 사용 하 여 임의의 guid를 안전 하 게 생성 합니다. |
...
| Api 차단 | @No__t-0Thread와 같은 Api를 차단 합니다. .NET 및 유사한 Api의 Sleep @ no__t-0은 orchestrator 함수에 대해 성능 및 크기 조정 문제를 일으킬 수 있으므로 피해 야 합니다. Azure Functions 소비 계획에서는 불필요 한 런타임 요금이 발생할 수도 있습니다. | Api를 사용할 수 있는 경우 차단 하는 대체 방법을 사용 합니다. 예를 들어 **Createtimer** 를 사용 하 여 오케스트레이션 실행에서 지연을 발생 시킵니다. 지 [속성 타이머](durable-functions-timers.md) 지연은 오 케 스트레이 터 함수의 실행 시간을 계산 하지 않습니다. |
| 비동기 Api | Orchestrator 코드는 [**DurableOrchestrationContext**](https://azure.github.io/azure-functions-durable-extension/api/Microsoft.Azure.WebJobs.DurableOrchestrationContext.html) api 또는 **context.df** 개체의 api를 사용하는 경우를 제외하고는 비동기 작업을 시작하지 않아야합니다. 예를 들어 .NET의 경우 **에는** **SendAsync** , **setTimeout** 및 **setInterval** 를 JavaScript에서 사용할 수 없습니다. 지 속성 작업 프레임 워크는 단일 스레드에서 orchestrator 코드를 실행 합니다. 다른 비동기 Api에서 호출 될 수 있는 다른 스레드와 상호 작용할 수 없습니다. | Orchestrator 함수는 영 속 비동기 호출만 수행 해야 합니다. 작업 함수는 다른 비동기 API 호출을 수행 해야 합니다. |
...
| 무한 루프 | 오케스트레이터 함수에는 무한 루프를 사용하지 않도록 방지하세요. 지 속성 작업 프레임 워크는 오케스트레이션 함수가 진행 됨에 따라 실행 기록을 저장 하므로 무한 루프를 수행 하면 orchestrator 인스턴스의 메모리가 부족 해질 수 있습니다. | 무한 루프 시나리오의 경우 .NET에서 [**ContinueAsNew**](https://azure.github.io/azure-functions-durable-extension/api/Microsoft.Azure.WebJobs.DurableOrchestrationContext.html#Microsoft_Azure_WebJobs_DurableOrchestrationContext_ContinueAsNew_) 와 같은 Api 또는 JavaScript의 **ContinueAsNew** 를 사용 하 여 함수 실행을 다시 시작 하 고 이전 실행 기록을 삭제 합니다. |
...
이러한 지 속성 작업은 .NET의 **Task의 원본** 개체 목록에 의해 내부적으로 관리 됩니다. 재생 하는 동안 이러한 작업은 orchestrator 코드 실행의 일부로 생성 됩니다. 디스패처가 해당 기록 이벤트를 열거 하는 것 처럼 완료 됩니다.
...
이 섹션의 런타임 동작에 대 한 설명은 오 케 스트레이 터 함수가 비 내구성 작업에서 **wait** 또는 **yield** 를 사용할 수 없는 이유를 이해 하는 데 도움이 됩니다. 두 가지 이유는 다음과 같습니다. 디스패처 스레드는 작업이 완료 될 때까지 기다릴 수 없으며, 해당 태스크의 콜백은 오 케 스트레이 터 함수의 추적 상태가 손상 될 수 있습니다. 이러한 위반을 검색 하는 데 도움이 되는 몇 가지 런타임 검사가 준비 되어 있습니다.
