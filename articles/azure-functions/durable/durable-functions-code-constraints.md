...
| Guid 및 Uuid  | 임의 GUID 또는 UUID를 반환 하는 Api는 생성 된 값이 각 재생 마다 다르기 때문에 비결 정적입니다. | .NET, JavaScript 및 Python에서 [Newguid](/dotnet/api/microsoft.azure.webjobs.extensions.durabletask.idurableorchestrationcontext.newguid) 를 사용 `newGuid` 하 여 `new_guid` 임의의 guid를 안전 하 게 생성 합니다. |
