...
지속형 함수의 작업 허브당 여러 개의 *제어 큐* 가 있습니다. *제어 큐* 는 간단한 작업 항목 큐보다 더 복잡합니다. 제어 큐는 상태 저장 orchestrator 및 entity 함수를 트리거하는 데 사용 됩니다. Orchestrator 및 entity 함수 인스턴스는 상태 저장 단일 항목 이므로 각 오케스트레이션 또는 엔터티는 한 번에 한 작업자만 처리 하는 것이 중요 합니다. 이를 위해 각 오케스트레이션 인스턴스나 엔터티는 단일 제어 큐에 할당 됩니다. 이러한 제어 큐는 작업자 간에 부하를 분산 하 여 각 큐가 한 번에 한 작업자 에서만 처리 되도록 합니다. 이 동작에 대한 자세한 내용은 이후 섹션을 참조하세요.
...
더 많은 Vm을 추가 하 여 활동 기능을 무한정 확장할 수 있지만, 개별 orchestrator 인스턴스 및 엔터티는 단일 파티션으로 inhabit 되 고 최대 파티션 수는의 설정에 의해 제한 됩니다 `partitionCount` `host.json` . 
...
제어 큐의 수는 **host.json** 파일에서 정의됩니다. 다음 예제 host.js에서는 코드 조각에서 `durableTask/storageProvider/partitionCount` 속성 (또는 `durableTask/partitionCount` Durable Functions 1.x)을로 설정 합니다 `3` . 파티션 수 만큼의 제어 큐가 있습니다.
...
트래픽이 낮은 시나리오에서는 응용 프로그램이 규모에 맞게 확장 되므로 적은 수의 작업자에 의해 파티션이 관리 됩니다. 예를 들어 아래 다이어그램을 살펴보세요.

![오케스트레이션 다이어그램 확장](./media/durable-functions-perf-and-scale/scale-progression-1.png)

위의 다이어그램에서 orchestrator 1 ~ 6은 파티션 간에 부하가 분산 되는 것을 확인할 수 있습니다. 마찬가지로 작업과 같은 파티션은 작업자 간에 부하가 분산 됩니다. 파티션은 시작 된 orchestrator 수에 관계 없이 작업자 간에 부하가 분산 됩니다.

Azure Functions 소비 또는 탄력적 프리미엄 계획에서 실행 하는 경우 또는 부하 기반 자동 크기 조정 기능이 구성 된 경우 트래픽이 증가 함에 따라 더 많은 작업 자가 할당 되 고 파티션이 결국 모든 작업자의 부하를 분산 합니다. 계속 규모를 확장 하는 경우 각 파티션은 결국 단일 작업자에 의해 관리 됩니다. 반면에 활동은 계속 해 서 모든 작업자에 게 부하가 분산 됩니다. 이는 아래 이미지에 나와 있습니다.

![첫 번째 확장 된 오케스트레이션 다이어그램](./media/durable-functions-perf-and-scale/scale-progression-2.png)

*지정 된 시간* 에 동시 _활성_ 오케스트레이션의 최대 수의 상한을 응용 프로그램 _시간_ 에 할당 된 작업자 수와 동일 `maxConcurrentOrchestratorFunctions` 합니다. 파티션이 작업자 간에 완전히 확장 되는 경우이 상한을 보다 정확 하 게 만들 수 있습니다. 완전히 확장 된 경우와 각 작업자는 단일 함수 호스트 인스턴스만 포함 하므로 _활성_ 동시 orchestrator 인스턴스의 최대 수 _는 해당 값의 파티션 수_ 와 동일 `maxConcurrentOrchestratorFunctions` 합니다. 아래 이미지는 추가 orchestrator 추가 되었지만 회색으로 표시 된 일부 비활성 상태인 완전히 확장 된 시나리오를 보여 줍니다.

![두 번째 확장 된 오케스트레이션 다이어그램](./media/durable-functions-perf-and-scale/scale-progression-3.png)

규모 확장 중에는 파티션이 균등 하 게 분산 되도록 제어 큐 잠금을 여러 함수 호스트 인스턴스에 재배포할 수 있습니다. 이러한 잠금은 내부적으로 blob storage 임대로 구현 되며, 개별 오케스트레이션 인스턴스나 엔터티는 한 번에 하나의 호스트 인스턴스에만 실행 되도록 합니다. 작업 허브가 3 개의 파티션 (즉, 3 개의 제어 큐)로 구성 된 경우 오케스트레이션 인스턴스 및 엔터티는 세 개의 임대 보유 호스트 인스턴스 간에 부하를 분산할 수 있습니다. 작업 함수 실행을 위한 용량을 늘리기 위해 추가 VM을 추가할 수 있습니다.
