...
`systemKey`은 Azure Functions 호스트에서 자동으로 생성 되는 인증 키입니다. 특히 지속성 작업 확장 API에 대한 액세스 권한을 부여하고 [다른 권한 부여 키](https://github.com/Azure/azure-webjobs-sdk-script/wiki/Key-management-API)와 동일한 방식으로 관리할 수 있습니다. .NET의 `taskHub` 및 `connection` APIs 또는 JavaScript의 `systemKey` 및 [와 같은 ](durable-functions-bindings.md#orchestration-client)오케스트레이션 클라이언트 바인딩`CreateCheckStatusResponse` APIs를 사용하여 올바른 `CreateHttpManagementPayload`, `createCheckStatusResponse` 및 `createHttpManagementPayload` 쿼리 문자열 값을 포함 하는 url을 생성할 수 있습니다.
...
| 필드                  | 데이터 형식 | 설명 |
|------------------------|-----------|-------------|
| **`instancesDeleted`** | 정수   | 삭제 된 인스턴스 수입니다. 단일 인스턴스를 사용할 경우이 값은 항상 `1`이어야 합니다. |
