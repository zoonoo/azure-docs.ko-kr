...
백그라운드 작업의 경우 특정 오케스트레이터의 인스턴스가 한 번에 하나만 실행되는지 확인해야 하는 경우가 많습니다. [내구성 함수에서](durable-functions-overview.md) 이러한 종류의 단일 동작을 만들 때 특정 인스턴스 ID를 오케스트레이터에 할당하여 확인할 수 있습니다.

## <a name="singleton-example"></a>단일 항목 예제

다음 예제에서는 단일 백그라운드 작업 오케스트레이션을 만드는 HTTP 트리거 함수를 보여 준다. 이 코드는 지정한 인스턴스 ID에 대해 인스턴스가 하나만 존재하는지 확인합니다.
...
기본적으로 인스턴스 ID는 임의로 GUID에서 생성됩니다. 그러나 이전 예제에서는 인스턴스 ID가 URL의 경로 데이터로 전달됩니다. 코드 호출 `GetStatusAsync`(C#) `getStatus` 또는 (자바 스크립트) 지정된 ID를 가진 인스턴스가 이미 실행 중인 지 확인 하려면. 이러한 인스턴스가 실행되지 않으면 해당 ID를 통해 새 인스턴스가 만들어집니다.
...
오케스트레이터 함수의 구현 세부 사항은 실제로 중요하지 않습니다. 시작하고 완료하는 일반 오케스트레이터 함수일 수 있거나 무기한 실행하는 오케스트레이터 함수일 수 있습니다(즉, [영구 오케스트레이션](durable-functions-eternal-orchestrations.md)). 중요한 점은 한 번에 하나의 인스턴스만 실행된다는 점입니다.

## <a name="next-steps"></a>다음 단계

> [!div class="nextstepaction"]
> [오케스트레이션의 기본 HTTP 기능에 대해 알아보기](durable-functions-http-features.md)
