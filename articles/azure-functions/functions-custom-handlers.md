...
- 공식적으로 지원 되지 않는 언어로 함수 앱을 구현 합니다.
- 기본적으로 지원 되지 않는 언어 버전 또는 런타임으로 함수 앱을 구현 합니다.
- 함수 앱 실행 환경에 대 한 보다 세부적인 제어를 제공 합니다.
...
사용자 지정 처리기로 구현 된 Azure Functions 앱은 몇 가지 규칙에 따라 파일에 대 한 *host.js* 를 구성 하 고 *function.js* 해야 합니다.
...
응용 프로그램은 파일 *의host.js* 를 통해 구성 됩니다. 이 파일은 HTTP 이벤트를 처리할 수 있는 웹 서버를 가리켜 요청을 보내는 기능을 호스트에 알립니다.

사용자 지정 처리기는 섹션을 통해 웹 서버를 실행 하는 방법에 대 한 세부 정보를 사용 하 여 파일 *에host.js* 를 구성 하 여 정의 합니다 `httpWorker` .
...
`httpWorker`섹션은에 정의 된 대상을 가리킵니다 `defaultExecutablePath` . 실행 대상은 웹 서버가 구현 되는 명령, 실행 파일 또는 파일 일 수 있습니다.

스크립팅된 응용 프로그램의 경우 `defaultExecutablePath` 은 스크립트 언어의 런타임을 가리키고 `defaultWorkerPath` 스크립트 파일 위치를 가리킵니다. 다음 예제에서는 Node.js의 JavaScript 앱이 사용자 지정 처리기로 구성 되는 방법을 보여 줍니다.
...
배열을 사용 하 여 인수를 전달할 수도 있습니다 `arguments` .
...
> [!NOTE]
> 파일 *의host.js* 는 실행 중인 웹 서버와 동일한 디렉터리 구조 수준에 있어야 합니다. 일부 언어 및 도구 체인은 기본적으로 응용 프로그램 루트에이 파일을 저장 하지 않을 수 있습니다.
...
파일 *에서 다음function.js* 에 정의 된 바인딩이 제공 됩니다.
...
| ------------- | --------- | ------------------------------------------------------------ |
| `Outputs`     | JSON      | `bindings`파일 *에function.js* 배열에 정의 된 응답 값을 포함 합니다.<br /><br />예를 들어 함수가 "blob" 이라는 blob 저장소 출력 바인딩을 사용 하 여 구성 된 경우는 `Outputs` `blob` blob의 값으로 설정 된 라는 키를 포함 합니다. |
| `Logs`        | array     | 메시지는 함수 호출 로그에 표시 됩니다.<br /><br />Azure에서 실행 하는 경우 메시지가 Application Insights 표시 됩니다. |
| `ReturnValue` | string    | function.js파일에서로 출력을 구성할 때 응답을 제공 하는 데 사용 됩니다 `$return` . *function.json* |

[샘플 페이로드는 예제](#bindings-implementation)를 참조 하세요.

## <a name="examples"></a>예

사용자 지정 처리기는 HTTP 이벤트를 지 원하는 모든 언어로 구현할 수 있습니다. [Javascript 및 Node.js을 완벽 하 게 지 원하는 ](./functions-reference-node.md)Azure Functions 있지만 다음 예제에서는 명령 목적으로 Node.js에서 javascript를 사용 하 여 사용자 지정 처리기를 구현 하는 방법을 보여 줍니다.

> [!TIP]
> 다른 언어로 사용자 지정 처리기를 구현 하는 방법에 대 한 지침을 제공 하는 동안 지원 되지 않는 버전의 Node.js에서 함수 앱을 실행 하려는 경우에는 여기에 나와 있는 Node.js 기반 예제가 유용할 수도 있습니다.

## <a name="http-only-function"></a>HTTP 전용 함수

다음 예제에서는 추가 바인딩이나 출력 없이 HTTP로 트리거되는 함수를 구성 하는 방법을 보여 줍니다. 이 예제에서 구현 된 시나리오에서는 `http` 또는를 허용 하는 라는 함수를 제공 합니다 `GET` `POST` .
...
*Http*라는 폴더에서 파일 *function.js* 는 http 트리거 함수를 구성 합니다.
...
앱의 루트에서 파일 *의host.js* 은 Node.js를 실행 하 고 파일을 가리키도록 구성 됩니다 `server.js` .
...
파일 *server.js* 파일은 웹 서버 및 HTTP 함수를 구현 합니다.
...
이 예제에서 Express는 HTTP 이벤트를 처리 하는 웹 서버를 만드는 데 사용 되며를 통해 요청을 수신 하도록 설정 됩니다 `FUNCTIONS_HTTPWORKER_PORT` .

함수는의 경로에 정의 되어 `/hello` 있습니다. `GET`요청은 간단한 JSON 개체를 반환 하 여 처리 되며 요청은를 `POST` 통해 요청 본문에 액세스할 수 있습니다 `req.body` .

여기에서 order 함수의 경로는 `/hello` `/api/hello` 함수 호스트가 사용자 지정 처리기에 요청을 프록시 하 고 있기 때문입니다.
...
함수는의 경로에 정의 되어 `/order` 있습니다.  여기에서 order 함수의 경로는 `/order` `/api/order` 함수 호스트가 사용자 지정 처리기에 요청을 프록시 하 고 있기 때문입니다.
...
`message`이 함수는 요청에서 들어오는 메시지와 예상 된 HTTP 응답으로의 메시지를 설정 하 여 `res` Queue Storage 하 고 http 응답을 반환 하는 메시지를 출력 합니다.

## <a name="debugging"></a>디버깅

함수 사용자 지정 처리기 앱을 디버깅 하려면 디버깅을 사용 하기 위해 언어 및 런타임에 적절 한 인수를 추가 해야 합니다.

예를 들어 Node.js 응용 프로그램을 디버깅 하기 위해 `--inspect` 플래그는 파일 *의host.js* 에 인수로 전달 됩니다.
...
> [!NOTE]
> 디버깅 구성은 파일 *에host.js* 의 일부입니다. 즉, 프로덕션 환경에 배포 하기 전에 일부 인수를 제거 해야 할 수 있습니다.
...
다음 예제는 응용 프로그램을 Visual Studio Code 디버거에 연결 하도록 파일 *에launch.js* 를 설정할 수 있는 방법을 보여 주는 샘플 구성입니다.

이 예제는 Node.js에 대 한 것 이므로 다른 언어나 런타임에 대 한이 예제를 변경 해야 할 수 있습니다.
