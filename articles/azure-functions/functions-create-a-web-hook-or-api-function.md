...
이 항목에서는 GitHub 웹후크에서 호출되는 Node.js 함수를 만드는 방법을 보여 줍니다. 새 함수는 Azure Functions 포털에서 미리 정의된 템플릿을 기반으로 생성됩니다. 포털에서 이러한 단계는 수행하는 방법을 보여 주는 짧은 비디오를 시청할 수도 있습니다.
...
3. 함수 앱에서 **+새 함수** > **GitHub 웹후크 - 노드** > **만들기**를 클릭합니다. 이 단계에서는 기본 이름을 가진 지정된 템플릿을 기반으로 하는 함수를 만듭니다. 
...
1. **함수 URL** 및 **GitHub 암호** 값을 복사합니다. 이러한 값은 GitHub에서 웹후크를 만드는 데 필요합니다. 
...
3. 함수의 URL 및 암호를 **페이로드 URL** 및 **암호**에 붙여 넣은 다음 **개별 이벤트 선택**을 클릭하고 **문제 주석**을 선택한 다음 **웹후크 추가**를 클릭합니다.
