...
## <a name="examples"></a>예
  ...
  다음 예제에서는가 서로 `ToString` 다른 형식에서 동작 하는 방법을 보여 줍니다.   
...
```    
 다음 예에서는와 같은 `ToString` `CONCAT`다른 문자열 함수에서를 사용 하는 방법을 보여 줍니다.   
...
```
다음 예에서는와 같은 `ToString` `REPLACE`다른 문자열 함수에서를 사용 하는 방법을 보여 줍니다.   
```sql
SELECT 
    n.id AS nutrientID,
