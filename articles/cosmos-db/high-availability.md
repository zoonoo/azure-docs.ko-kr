...
| 쓰기 지역 | 자동 장애 조치(automatic failover) | 필요한 항목 | 알아두어야 할 사항 |
| -- | -- | -- | -- |
| 단일 쓰기 지역 | 사용 안 함 | 읽기 지역에서 가동 중단이 발생하는 경우, 모든 클라이언트는 다른 지역으로 리디렉션됩니다. 읽기 또는 쓰기 가용성이 손실되지 않습니다. 데이터가 손실되지 않습니다. <p/> 쓰기 지역에서 가동 중단이 발생하는 경우, 클라이언트에게 쓰기 가용성 손실이 발생합니다. 데이터 손실은 선택한 일관성 수준에 따라 달라집니다. <p/> 가동 중단이 종료되면 Cosmos DB가 자동으로 쓰기 가용성을 복원합니다. | 가동 중단 중에는 읽기 트래픽을 지원하기 위해 남은 지역에 프로비전된 용량이 충분한지 확인합니다. <p/> 장애 조치 중에는 수동 장애 조치가 트리거되지 않으므로, 이를 트리거하면 *안 됩니다*. <p/> 가동 중단이 발생하면 프로비전된 용량을 적절하게 다시 조정합니다. |
| 단일 쓰기 지역 | 사용 | 읽기 지역에서 가동 중단이 발생하는 경우, 모든 클라이언트는 다른 지역으로 리디렉션됩니다. 읽기 또는 쓰기 가용성이 손실되지 않습니다. 데이터가 손실되지 않습니다. <p/> 쓰기 지역에서 가동 중단이 발생하는 경우, Cosmos DB가 기본 설정에 따라 새 지역을 새 쓰기 지역으로 자동으로 선택하기 전까지는 클라이언트에게 쓰기 가용성 손실이 발생합니다. 데이터 손실은 선택한 일관성 수준에 따라 달라집니다. | 가동 중단 중에는 읽기 트래픽을 지원하기 위해 남은 지역에 프로비전된 용량이 충분한지 확인합니다. <p/> 장애 조치 중에는 수동 장애 조치가 트리거되지 않으므로, 이를 트리거하면 *안 됩니다*. <p/> 가동 중단이 종료되면, 실패한 지역의 복제되지 않은 데이터를 [충돌 피드에서](how-to-manage-conflicts.md#read-from-conflict-feed) 복구하고, 쓰기 지역을 원래 지역으로 다시 이동하고, 프로비전된 용량을 적절하게 다시 조정할 수 있습니다. |
| 다중 쓰기 지역 | 해당 없음 | 읽기 또는 쓰기 가용성이 손실되지 않습니다. <p/> 선택한 일관성 수준에 따라 데이터 손실이 발생합니다. | 가동 중단 중에 추가 트래픽을 지원하기 위해 남은 지역에 충분한 용량이 프로비전되어 있는지 확인합니다. <p/> 가동 중단이 종료되면, 실패한 지역의 복제되지 않은 데이터를 [충돌 피드에서](how-to-manage-conflicts.md#read-from-conflict-feed) 복구하고, 프로비전된 용량을 적절하게 다시 조정할 수 있습니다. |
