...
| Maximum RUs per (logical) partition key | 10000 |
| Maximum storage across all items per (logical) partition key| 10GB |
| Maximum number of distinct (logical) partition keys | 제한 없음 |
| Maximum storage per container | 제한 없음 |
| Maximum storage per database | 제한 없음 |
...
| 리소스 | 기본 제한 |
| --- | --- |
| Maximum number of databases | 제한 없음 |
| Maximum number of containers per database (or account) | 제한 없음 |
...
| 리소스 | 기본 제한 |
| --- | --- |
| Maximum execution time for a single operation (like a stored procedure execution or a single query page retrieval)| 5초 |
| Maximum request size (stored procedure, CRUD)| 2MB |
| Maximum response size (for example, paginated query) | 4MB |
...
| 리소스 | 기본 제한 |
| --- | --- |
| Maximum master token expiry time | 15분  |
...
| Maximum clock skew for token authorization| 15분 |
...
| Maximum execution time for MongoDB operations| 30초 |
...
| Maximum total storage per account | 10GB |
