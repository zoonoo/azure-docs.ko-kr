...
2. 대량 실행자 라이브러리는 단일 클라이언트 VM에서 50만 RUs를 사용 하도록 확장할 수 있습니다. 사용 가능한 처리량은 500만 RUs 이므로 10 개의 Ubuntu 16.04 Vm (Standard_D32_v3)은 Azure Cosmos 데이터베이스가 있는 동일한 지역에 프로 비전 되어야 합니다. 마이그레이션 도구와 해당 설정 파일을 사용 하 여 이러한 Vm을 준비 해야 합니다.  
