...
Azure Cosmos DB에 해당 하는 모든 메트릭은 **표준 메트릭에 Cosmos DB** 네임 스페이스에 저장 됩니다. 모든 Azure Monitor 지원 메트릭 (Azure Cosmos DB 포함)의 목록은 [지원 되는 메트릭 Azure Monitor](../azure-monitor/platform/metrics-supported.md)을 참조 하세요. 이 섹션에서는 Azure Cosmos DB에 대해 수집 된 자동으로 수집 된 모든 플랫폼 메트릭을 나열 합니다.  
...
자세한 내용은 [Azure Monitor에서 지원 되는 모든 플랫폼 메트릭](../azure-monitor/platform/metrics-supported.md)목록을 참조 하세요.
...
모든 Azure Monitor 로그 범주 및 관련 스키마에 대 한 링크 목록은 [Azure Monitor Logs categories and 스키마](../azure-monitor/platform/resource-logs-schema.md)를 참조 하세요. 
