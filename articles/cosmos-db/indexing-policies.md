...
**지연**: Azure Cosmos DB 컬렉션이 정지 상태 즉, 사용자 요청을 처리할 만큼 컬렉션의 처리량 용량이 충분히 활용되지 않은 경우 인덱스가 비동기적으로 업데이트됩니다. 문서 수집을 필요로 하는 "지금 수집, 나중에 쿼리" 워크로드의 경우, "지연" 인덱싱 모드가 적합할 수 있습니다. 데이터가 천천히 수집되고 인덱싱되기 때문에 비일관적인 결과가 발생할 수 있습니다. 즉, 데이터가 인덱싱될 때까지 COUNT 쿼리 또는 특정 쿼리 결과가 정확하거나 반복이 가능하지 않을 수 있습니다. 인덱스는 일반적으로 캐치 업 모드입니다. WRT 지연 인덱싱 - TTL 변경으로 인해 인덱스가 삭제되고 다시 작성되기 때문에 이 작업으로 인해 예기치 않은 결과가 발생할 수 있습니다. 대부분의 고객은 일관된 인덱싱을 사용해야 합니다.
