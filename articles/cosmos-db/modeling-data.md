...
---
# <a name="data-modeling-in-azure-cosmos-db"></a>Azure 코스모스 DB의 데이터 모델링

Azure Cosmos DB와 같은 스키마가 없는 데이터베이스는 구조화되지 않은 반정형 데이터를 매우 쉽게 저장하고 쿼리할 수 있도록 하지만 성능과 확장성 및 최저 수준 측면에서 서비스를 최대한 활용하기 위해 데이터 모델에 대해 고민하는 데 시간을 할애해야 합니다. 비용.

데이터를 어떻게 저장할 것인가? 애플리케이션에서 데이터를 검색 및 쿼리하는 방법은 무엇인가? 응용 프로그램이 읽기가 무거우거나 쓰기가 무겁습니까?

이 문서를 읽은 다음에는 다음과 같은 질문에 답할 수 있습니다.

* 데이터 모델링은 무엇이고 어떻게 사용해야 하는가?
* Azure Cosmos DB의 모델링 데이터는 관계형 데이터베이스와 어떻게 다른가요?
* 비관계형 데이터베이스에서 데이터 관계는 어떻게 표현하는가?
* 데이터를 포함해야 하는 경우 및 데이터에 링크하는 경우는 언제인가?

## <a name="embedding-data"></a>데이터 포함

Azure Cosmos DB에서 데이터 모델링을 시작하면 엔터티를 JSON 문서로 표시된 **독립형 항목으로** 처리하려고 합니다.
...
관계형 데이터베이스로 작업할 때 는 모든 데이터를 정규화하는 것이 전략입니다. 데이터를 정규화하려면 일반적으로 사람과 같은 엔터티를 사용하여 개별 구성 요소로 분류해야 합니다. 위의 예에서 한 사람은 여러 연락처 세부 정보 레코드와 여러 주소 레코드를 가질 수 있습니다. 형식과 같은 공통 필드를 추가로 추출하여 연락처 세부 정보를 더 세분화할 수 있습니다. 주소에도 동일하게 적용되며 각 레코드는 *홈* 또는 *비즈니스*유형일 수 있습니다.

데이터를 정규화할 때의 원칙은 각 레코드에서 **중복된 데이터 저장을 피하고** 데이터를 참조하는 것입니다. 이 예제에서는 모든 연락처 세부 정보 및 주소가 있는 사람을 읽으려면 JOINS를 사용하여 런타임에 데이터를 효과적으로 다시 작성(또는 비정규화)해야 합니다.
...
이제 Azure Cosmos DB의 독립형 엔터티와 동일한 데이터를 모델링하는 방법을 살펴보겠습니다.
...
위의 방법을 사용하여 연락처 정보 및 주소와 같은 이 사람과 관련된 모든 정보를 *단일 JSON* 문서에 **포함시켜** 개인 레코드를 **비정규화했습니다.**
또한 고정된 스키마로 제한되지 않기 때문에 다른 도형의 연락처 세부 정보를 완전히 포함하는 것과 같은 유연성이 있습니다.

이제 데이터베이스에서 전체 사람 레코드를 검색하는 작업은 단일 컨테이너 및 단일 항목에 대한 **단일 읽기 작업입니다.** 연락처 세부 정보 및 주소로 사람 레코드를 업데이트하는 것도 단일 항목에 대한 **단일 쓰기 작업입니다.**
...
* **바인딩되지 않고**증가하지 않는 임베디드 데이터가 있습니다.
* **함께 자주 쿼리되는**임베디드 데이터가 있습니다.
...
Azure Cosmos DB의 경험상 모든 항목을 비정규화하고 모든 데이터를 단일 항목에 포함시키는 것이지만, 이로 인해 피해야 할 몇 가지 상황이 발생할 수 있습니다.
...
이 코드 조각은 일반적인 블로그 또는 CMS 시스템을 모델링할 때 나타날 수 있는 포함된 주석이 있는 포스트 엔터티를 나타낼 수 있습니다. 이 예제에서의 문제는 주석 배열이 **바인딩되지 않음**으로써, 단일 포스트가 가질 수 있는 주석 수에 대한 제한(실질적인)이 없다는 것입니다. 항목의 크기가 무한히 커질 수 있기 때문에 문제가 될 수 있습니다.

항목의 크기가 증가함에 따라 와이어를 통해 데이터를 전송하는 기능뿐만 아니라 항목을 읽고 업데이트하는 기능이 확장되어 영향을 받게 됩니다.

이 경우 다음 데이터 모델을 고려하는 것이 좋습니다.
...
이 모델에는 고정된 특성 집합이 있는 배열인 post 컨테이너에 포함된 세 가지 최신 주석이 있습니다. 다른 주석은 100개의 주석일괄 처리로 그룹화되어 별도의 항목으로 저장됩니다. 일괄 처리의 크기를 100으로 선택한 이유는 이 예제의 가상 애플리케이션에서 사용자가 한 번에 로드할 수 있는 주석의 수가 100개로 제한되기 때문입니다.  

포함 된 데이터가 좋은 생각이 아닌 또 다른 경우는 포함 된 데이터가 항목 간에 자주 사용되고 자주 변경되는 경우입니다.
...
포함 하는 데이터는 많은 경우에 잘 작동 하지만 데이터를 비정규화 하는 경우 가치가 보다 더 많은 문제를 일으킬 시나리오가 있다. 그러면 이제 무엇을 해야 할까요?

관계형 데이터베이스는 엔터티 간의 관계를 생성할 수 있는 유일한 대안이 아닙니다. 문서 데이터베이스에서 다른 문서의 데이터와 관련된 정보를 한 문서에 가질 수 있습니다. Azure Cosmos DB 또는 다른 문서 데이터베이스의 관계형 데이터베이스에 더 적합한 시스템을 빌드하는 것은 권장하지 않지만 간단한 관계는 괜찮으며 유용할 수 있습니다.
...
저자가 있다면, 나는 즉시 그들이 쓴 책을 알고, 반대로 내가로드 된 책 문서가있다면 나는 저자의 아이디를 알 것이다 (들). 이렇게 하면 조인 테이블에 대한 중간 쿼리를 절약해서 애플리케이션에서 수행해야 하는 서버 라운드 트립 수를 줄일 수 있습니다.
...
책 문서를 보면 저자 배열을 볼 때 몇 가지 흥미로운 필드를 발견할 수 있습니다. 작성자 `id` 문서, 정규화된 모델의 표준 사례를 다시 참조하는 데 사용하는 필드가 있지만. `name` `thumbnailUrl` 우리는 붙어 "링크"를 사용하여 각 저자 문서에서 필요한 추가 정보를 얻기 위해 응용 프로그램을 떠날 수 `id` 있지만, 우리의 응용 프로그램은 저자의 이름과 표시된 모든 책과 축소판 그림을 표시하기 때문에 우리는 저자의 **일부** 데이터를 비정규화하여 목록에서 책 당 서버에 왕복을 저장할 수 있습니다.

물론 저자의 이름이 변경되었거나 사진을 업데이트하고 싶다면 저자가 이름을 자주 변경하지 않는다는 가정하에 응용 프로그램을 위해 모든 책을 업데이트해야합니다.  

이 예제에서는 읽기 작업에서 비용이 많이 드는 처리를 저장하기 위해 **미리 계산된 집계** 값이 있습니다. 이 예제에서 저자의 문서에 포함된 일부 데이터는 런타임에 계산되는 데이터입니다. 새 책이 게시될 때마다, 책 문서가 생성됩니다. **그리고** countOfBooks 필드가 특정 저자에 대해 존재하는 책 문서 번호를 기준으로 계산된 값으로 설정됩니다. 이러한 최적화는 읽기를 최적화하기 위해 읽기 계산을 수행할 수 있는 읽기에 집중된 시스템에서 효과적일 수 있습니다.
...
## <a name="distinguishing-between-different-document-types"></a>서로 다른 문서 유형 구별

일부 시나리오에서는 동일한 컬렉션에서 서로 다른 문서 형식을 혼합할 수 있습니다. 일반적으로 여러 관련 문서를 동일한 [파티션에](partitioning-overview.md)두려는 경우입니다. 예를 들어 책과 책 리뷰를 모두 동일한 컬렉션에 넣고 `bookId`로 분할할 수 있습니다. 이러한 경우 일반적으로 해당 형식을 식별하는 필드가 있는 문서에 추가하여 구별하려고 합니다.
...
실제 예제를 사용하여 Azure Cosmos DB에서 데이터를 모델링하고 파티션하는 방법을 알아보려면 [데이터 모델링 및 분할- 실제 예제를](how-to-model-partition-example.md)참조하십시오.
