...
Azure Cosmos DB와 같은 스키마 없는 데이터베이스에서는 구조화 되지 않은 데이터와 반 구조화 된 데이터를 쉽게 저장 하 고 쿼리할 수 있지만, 데이터 모델에 대 한 몇 가지 시간을 투자 하 여 성능 및 확장성 측면에서 서비스를 최대한 활용 하는 것이 가장 좋습니다. 절감.
