...
- 인덱스 전체 자릿수는 문자열 범위와 함께 사용할 때 실질적으로 더 유용합니다. 문자열은 임의의 길이일 수 있으므로 인덱스 전체 자릿수 선택은 문자열 범위 쿼리의 성능에 영향을 줄 수 있으며 필요한 인덱스 저장소 공간의 양에도 영향을 줄 수 있습니다. 문자열 범위 인덱스는 1~100 사이 또는 -1(최댓값)의 인덱스 전체 자릿수로 구성할 수 있습니다. 문자열 속성에 대한 ORDER BY 쿼리를 수행하려는 경우, 해당 경로에 대해 -1의 전체 자릿수를 지정해야 합니다.
