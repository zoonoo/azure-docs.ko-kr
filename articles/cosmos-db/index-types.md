...
- **인덱스 종류:** 범위 (같음, 범위 또는 ORDER BY 쿼리) 또는 공간 (공간 쿼리).
...
- **범위 인덱스** 효율적인 같음 쿼리, 조인 쿼리, 범위 쿼리 지원 (사용 하 여 >, <>, =, < =,! =), 및 ORDER BY 쿼리 합니다. 또한 ORDER BY 쿼리에는 기본적으로 최대 인덱스 전체 자릿수(-1)가 필요합니다. 데이터 형식은 문자열 또는 숫자일 수 있습니다.
