...
description: C# 콘솔 애플리케이션을 사용하여 Azure Cosmos DB SQL API 리소스를 만드는 방법을 알아봅니다.
...
Azure Cosmos DB SQL API 시작 자습서를 시작합니다. 이 자습서를 따라 하면 Azure Cosmos DB 리소스를 만들고 쿼리하는 콘솔 애플리케이션이 생깁니다. 이 자습서에서는 [.NET Framework](https://dotnet.microsoft.com/download) 또는 [.NET Core](https://dotnet.microsoft.com/download)를 대상으로 하는 [버전 3.0 이상](https://www.nuget.org/packages/Microsoft.Azure.Cosmos) Azure Cosmos DB .NET SDK를 사용합니다.
...
잘하셨습니다. 설치를 완료했으므로 코드를 작성해 보겠습니다. [GitHub](https://github.com/Azure-Samples/cosmos-dotnet-getting-started)에서 이 자습서의 완성된 코드 프로젝트를 찾을 수 있습니다.
...
``CosmosClient`` 클래스의 [**CreateDatabaseIfNotExistsAsync**](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmos.cosmosclient) 또는 [**CreateDatabaseAsync**](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmos.cosmosclient) 함수를 사용하여 데이터베이스를 만들 수 있습니다. 데이터베이스는 여러 컨테이너에 분할된 항목의 논리적 컨테이너입니다.

1. **CreateDatabaseAsync** 메서드를 **GetStartedDemoAsync** 메서드 아래에 복사하여 붙여넣습니다. **CreateDatabaseAsync**는 ID가 ``FamilyDatabase``인 새 데이터베이스를 만들며, 아직 없는 경우 ``databaseId`` 필드에서 ID가 지정됩니다. 

    [!code-csharp[](~/cosmos-dotnet-getting-started/CosmosGettingStartedTutorial/Program.cs?name=CreateDatabaseAsync&highlight=7)]

1. CosmosClient를 인스턴스화한 아래 코드를 복사하여 붙여넣어 방금 추가한 **CreateDatabaseAsync** 메서드를 호출합니다.
    ...
    이 시점에서 엔드포인트 및 기본 키를 입력한 코드는 다음과 같아야 합니다.
...
**CosmosDatabase** 클래스의 [**CreateContainerIfNotExistsAsync**](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmos.cosmosdatabase) 또는 [**CreateContainerAsync**](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmos.cosmosdatabase) 함수를 사용하여 컨테이너를 만들 수 있습니다. 컨테이너는 항목(SQL API의 경우 JSON 문서) 및 JavaScript의 관련 서버 쪽 애플리케이션 논리(예: 저장 프로시저, 사용자 정의 함수 및 트리거)로 구성됩니다.

1. **CreateContainerAsync** 메서드를 **CreateDatabaseAsync** 메서드 아래에 복사하여 붙여넣습니다. **CreateContainerAsync**는 ID가 ``FamilyContainer``인 새 컨테이너를 만들며, 아직 없는 경우 ``LastName`` 속성으로 분할된 ``containerId`` 필드에서 ID가 지정됩니다.
...
항목은 **CosmosContainer** 클래스의 [**CreateItemAsync**](https://docs.microsoft.com/dotnet/api/microsoft.azure.cosmos.cosmoscontainer) 함수를 사용하여 만들 수 있습니다. SQL API를 사용하는 경우 항목은 문서로 보호되며, 사용자 정의(임의) JSON 콘텐츠입니다. 이제 Azure Cosmos DB 컨테이너에 항목을 삽입할 수 있습니다.
...
1. **가족**, **부모**, **자식**, **애완 동물** 및 **주소** 클래스를 복사하여 **Family.cs**에 붙여넣습니다.

    [!code-csharp[](~/cosmos-dotnet-getting-started/CosmosGettingStartedTutorial/Family.cs)]

1. **Program.cs**로 돌아가서 **CreateContainerAsync** 메서드 아래에 **AddItemsToContainerAsync** 메서드를 추가합니다.
...
1. **ReplaceFamilyItemAsync** 메서드를 **QueryItemsAsync** 메서드 아래에 복사하여 붙여넣습니다. 제품군의 ``IsRegistered`` 속성 및 자식 중 하나인 ``Grade``를 변경하는 것입니다.
...
1. **DeleteFamilyItemAsync** 메서드를 **ReplaceFamilyItemAsync** 메서드 아래에 복사하여 붙여넣습니다.
...
1. **DeleteDatabaseAndCleanupAsync** 메서드를 **DeleteFamilyItemAsync** 메서드 아래에 복사하여 붙여넣습니다.
...
전체 앱의 출력이 콘솔 창에 표시됩니다. 출력은 추가한 쿼리 결과를 보여 주며, 아래 예제 텍스트와 일치해야 합니다.
...
이 자습서의 단계를 완료할 시간이 없거나 코드 샘플만 다운로드하려는 경우 [GitHub](https://github.com/Azure-Samples/cosmos-dotnet-getting-started)에서 가져올 수 있습니다. 
...
Visual Studio에서 Azure Cosmos DB .NET SDK에 대한 참조를 복원하려면 솔루션 탐색기에서 **GetStarted** 솔루션을 마우스 오른쪽 단추로 클릭한 다음, **NuGet 패키지 복원**을 클릭합니다. 다음으로 App.config 파일에서 EndPointUri 및 PrimaryKey 값을 [Azure Cosmos DB 계정에 연결](#Connect)에 설명된 대로 업데이트합니다.
