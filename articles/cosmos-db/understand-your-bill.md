...
Azure Cosmos DB 무료 계층을 사용 하면 계정 수준에서 적용 되는 계정에서 처음 400 r u/초 및 5gb의 저장소를 무료로 얻을 수 있습니다. 모든 r u/s와 저장소 400 r u/s 및 5gb를 초과 하는 저장소는 가격 책정 페이지 당 일반 가격 책정 가격으로 청구 됩니다. 청구서에는 무료 계층에 포함 되는 것 외에도, 무료 400 r u/s 및 5gb에 대 한 요금 또는 품목은 표시 되지 않습니다. 400 o s/s는 모든 형식/s 프로 비전 된 처리량, autopilot (미리 보기) 및 다중 마스터에 적용 됩니다.  
...
### <a name="billing-example---container-or-database-with-autopilot-throughput-preview"></a>청구 예-autopilot 처리량을 포함 하는 컨테이너 또는 데이터베이스 (미리 보기)
- 무료 계층 계정에서 autopilot를 사용 하도록 설정 된 데이터베이스 또는 컨테이너를 만듭니다 .이는 최대 r u/초/4000 초를 사용 합니다. 이 리소스는 400 r u/s-4000 r u/초 사이에서 자동으로 크기가 조정 됩니다. 
...
- 참고: r u/s 또는 저장소에 대 한 단가가 지역에서 다른 경우 무료 계층 400 r u/초 및 5gb는 지역이 생성 된 계정 비율을 반영 합니다.
