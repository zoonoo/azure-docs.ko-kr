---
title: Cosmos DB용 Azure Functions 트리거 연결 정책
...
이 문서에서는 Cosmos DB용 Azure Functions 트리거를 사용하여 Azure Cosmos 계정에 연결할 때 연결 정책을 구성하는 방법을 설명합니다.

## <a name="why-is-the-connection-policy-important"></a>연결 정책이 중요한 이유는 무엇인가요?

직접 모드와 게이트웨이 모드의 두 가지 연결 모드가 있습니다. 이러한 연결 모드에 대한 자세한 내용은 [성능 팁](./performance-tips.md#networking) 문서를 참조하세요. 기본적으로 **게이트웨이**는 Cosmos DB용 Azure Functions 트리거에서 모든 연결을 설정하는 데 사용됩니다. 그러나 성능 중심 시나리오에는 최상의 옵션이 아닐 수 있습니다.
