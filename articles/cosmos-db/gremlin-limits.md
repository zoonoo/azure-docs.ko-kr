...
*유휴 연결 시간 제한* | **1 시간** | Gremlin 서비스가 유휴 websocket 연결을 열어 둘 때까지 걸리는 시간입니다. TCP keep-alive 패킷 또는 HTTP 연결 유지 요청은이 제한을 초과 하는 연결 수명을 연장 하지 않습니다. Cosmos DB Graph 엔진은 실행 중인 활성 Gremlin 요청이 없는 경우 websocket 연결을 유휴 상태로 간주 합니다.
