...
## <a name="how-do-i-secure-my-database"></a>내 데이터베이스를 보호 하려면 어떻게 해야 하나요
...
- 최신 상태로 유지 되는 패치 된 서버
- HTTPS(기본값)/SSL 암호화
- 강력한 암호를 사용하는 관리 계정

## <a name="how-does-azure-cosmos-db-secure-my-database"></a>Azure Cosmos DB 내 데이터베이스를 보호 하는 방법
...
|Active Directory 통합(RBAC)| 제공 하거나 Cosmos 계정, 데이터베이스, 컨테이너 및 제품 (처리량) 액세스 제어 (IAM)를 사용 하 여 Azure portal에서 액세스를 제한할 수도 있습니다. IAM은 역할 기반 액세스 제어를 제공하며 Active Directory와 통합됩니다. 개인 및 그룹에 대 한 기본 제공된 역할 또는 사용자 지정 역할을 사용할 수 있습니다. 참조 [Active Directory 통합](role-based-access-control.md) 자세한 문서.|
...
|로컬 복제|단일 데이터 센터 내에서도 Azure Cosmos DB는 고가용성을 위해 데이터를 자동으로 복제하여 [일관성 수준](consistency-levels.md)을 선택할 수 있도록 합니다. 이 복제는 99.99% 보증 [가용성 SLA](https://azure.microsoft.com/support/legal/sla/cosmos-db) 모든 단일 지역 계정 및 모든 다중 지역 계정에 일관성을 완화 하 고 99.999% 읽기 가용성을 모든 다중 지역 데이터베이스 계정에 대 한 합니다.|
|자동 온라인 백업|Azure Cosmos DB 데이터베이스에 정기적으로 백업 및 지역 중복 저장소에 저장 합니다. <br><br>[Azure Cosmos DB로 자동 온라인 백업 및 복원](online-backup-and-restore.md)에서 자세히 알아보세요.|
...
|HTTPS/SSL/TLS 암호화|모든 클라이언트-서비스 Azure Cosmos DB 상호 작용에는 SSL/TLS 1.2가 지원됩니다. 또한 모든 내부 데이터 센터와 간 데이터 센터 복제 SSL/TLS 1.2가 적용 됩니다.|
...
|보안 및 데이터 보호 인증서| 인증의 최신 목록에 대 한 전체 참조 [Azure 규정 준수 사이트](https://www.microsoft.com/en-us/trustcenter/compliance/complianceofferings) 최신 뿐만 아니라 [Azure 규정 준수 문서](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942) 모든 인증 (Cosmos 검색)를 사용 하 여 합니다. 보다 중점적 읽을 2018 년 4 월 25 일 게시물 확인 [Azure #CosmosDB: SOC 1/2에 포함 된 보안, 개인, 규격 Type 2, HITRUST, PCI DSS Level 1, ISO 27001, HIPAA, FedRAMP High 및 기타.
...
마스터 키 및 리소스 토큰에 대 한 자세한 내용은 참조 하세요. [Azure Cosmos DB 데이터에 대 한 액세스 보호](secure-access-to-data.md)합니다.

감사 로깅에 대 한 자세한 내용은 참조 하세요. [Azure Cosmos DB 진단 로깅](logging.md)합니다.

Microsoft 인증에 대 한 자세한 내용은 참조 하세요. [Azure 보안 센터](https://azure.microsoft.com/support/trust-center/)합니다.