...
1. 다음과 같이 설정 목록에서 **프라이빗 엔드포인트 연결**을 선택한 다음, **프라이빗 엔드포인트**를 선택합니다.
   ...
   ![Azure Portal에서 프라이빗 엔드포인트를 만들기 위해 선택하는 항목](./media/how-to-configure-private-endpoints/create-private-endpoint-portal.png)
...
![Resource Manager 템플릿의 배포 출력](./media/how-to-configure-private-endpoints/resource-manager-template-deployment-output.png)
...
* 공용 트래픽 또는 서비스 엔드포인트를 구성하지 않고 프라이빗 엔드포인트를 만들면 프라이빗 엔드포인트를 통해서만 Azure Cosmos 계정에 액세스할 수 있습니다. 공용 트래픽 또는 서비스 엔드포인트를 구성하지 않으면 승인된 모든 프라이빗 엔드포인트가 거부 또는 삭제된 후 계정이 전체 네트워크에 대해 열립니다.
