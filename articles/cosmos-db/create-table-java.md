...
이 빠른 시작은 GitHub에서 예제를 복제하여 앱을 빌드하기 위해 Java와 Azure Cosmos DB [테이블 API](table-introduction.md)를 사용하는 방법을 보여줍니다. 이 빠른 시작은 Azure Cosmos DB 계정을 만드는 방법과 데이터 탐색기를 사용하여 웹 기반 Azure Portal에 테이블과 엔터티를 만드는 방법을 보여줍니다.
...
1. [Azure Portal](http://portal.azure.com/)에서 **연결 문자열**을 클릭합니다. 
   ...
   ![연결 문자열 창의 필요한 연결 문자열 정보를 보고 복사](./media/create-table-java/connection-string.png)

2. config.properties 파일을 열고 필요한 연결 문자열 속성을 구성 파일에 복사합니다.

3. config.properties 파일을 저장합니다.

이제 Azure Cosmos DB와 통신하는 데 필요한 모든 정보로 앱이 업데이트되었습니다. 

## <a name="run-the-app"></a>앱 실행

1. git 터미널 창에서 storage-table-java-getting-started 폴더에 `cd`합니다.
