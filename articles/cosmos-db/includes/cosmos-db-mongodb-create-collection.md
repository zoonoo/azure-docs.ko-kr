...
2. **컨테이너 추가** 페이지에서 새 컨테이너의 설정을 입력합니다.
    ...
    |**처리량**|400|처리량을 400 RU/s(초당 요청 단위)로 유지합니다. 대기 시간을 줄이면 나중에 처리량을 늘릴 수 있습니다. 필요에 따라 동적으로 증가하고 감소하는 RU/s 범위를 제공하는 [자동 크기 조정 모드](../provision-throughput-autoscale.md)를 선택할 수도 있습니다.| 
    ...
    |**분할 키**| /_id| 이 문서에서 설명하는 샘플은 분할 키를 사용하지 않으므로 */_id* 로 설정하면 자동으로 생성된 ID 필드가 분할 키로 사용됩니다. [Azure Cosmos DB에서 분할(파티셔닝)](../partitioning-overview.md)에서 분할(파티셔닝이라고도 함)에 대해 자세히 알아봅니다.|
