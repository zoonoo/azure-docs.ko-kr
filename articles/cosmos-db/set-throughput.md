...
공유 처리량 데이터베이스의 컨테이너는 해당 데이터베이스에 할당 된 처리량 (r u/초)을 공유 합니다. 공유 처리량 데이터베이스에서:

* 데이터베이스에 최소 400 r u/초를 포함 하는 최대 4 개의 컨테이너를 사용할 수 있습니다. 처음 4 개 이후의 각 새 컨테이너에는 추가 100 o s/s 최소가 필요 합니다. 예를 들어 8 개의 컨테이너를 포함 하는 공유 처리량 데이터베이스를 사용 하는 경우 데이터베이스에 대 한 최소 r u/초는 800 r u/초입니다.

* 최대 25 개의 컨테이너를 데이터베이스에 포함할 수 있습니다. 공유 처리량 데이터베이스에 25 개 이상의 컨테이너가 이미 있는 경우 컨테이너 수가 25 미만일 때까지 추가 컨테이너를 만들 수 없습니다.
