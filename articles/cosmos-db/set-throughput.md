...
데이터베이스에 프로 비전 된 처리량을 공유 하는 여러 컨테이너에 속한 여러 논리 파티션은 단일 실제 파티션에서 호스팅될 수 있습니다. 컨테이너의 단일 논리 파티션은 항상 실제 파티션 내에서 범위가 지정 되는 반면, " *L"* 데이터베이스의 프로 비전 된 처리량을 공유 하는 " *C"* 컨테이너 간의 논리 파티션은 " *R"* 물리적으로 매핑하고 호스트할 수 있습니다. 영역. 
