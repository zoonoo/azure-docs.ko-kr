...
1kb 항목을 읽는 비용은 1 개의 요청 단위 (또는 1, 000 개)입니다. 다른 모든 데이터베이스 작업에는 또한 RUs를 사용 하 여 비용이 할당 됩니다. Azure Cosmos 컨테이너 조작에 사용하는 API에 상관없이 비용은 항상 RU로 측정됩니다. 데이터베이스 작업이 쓰기, 읽기 또는 쿼리든 간에 비용은 항상 RU로 측정됩니다.
