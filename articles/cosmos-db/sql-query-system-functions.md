...
Cosmos DB 함수와 ANSI SQL 함수 간의 주요 차이점은 Cosmos DB 함수는 스키마 없는 데이터 및 혼합 스키마 데이터에서 잘 작동 하도록 설계 되었다는 것입니다. 예를 들어 속성이 누락 되었거나 `unknown`과 같은 숫자가 아닌 값을 포함 하는 경우 오류를 반환 하는 대신 항목을 건너뜁니다.
