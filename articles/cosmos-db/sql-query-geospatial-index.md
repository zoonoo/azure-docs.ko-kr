...
컨테이너에서 `geospatialConfig`는 지리 공간적 데이터를 인덱싱하는 방법을 지정 합니다. 컨테이너 (지리 또는 기 하 도형) 당 하나의 `geospatialConfig` 지정 해야 합니다. 지정 하지 않으면 기본적으로 geography 데이터 형식으로 `geospatialConfig` 됩니다. `geospatialConfig`를 수정 하면 컨테이너의 모든 기존 지리 공간적 데이터가 인덱싱해야 됩니다.

> [!NOTE]
> 현재 Azure Cosmos DB는 3.6 이상 버전 에서만 .NET SDK의 geospatialConfig 수정 사항을 지원 합니다.
>

다음은 `geospatialConfig` 속성을 설정 하 고 **boundingBox**를 추가 하 여 지리 공간적 데이터 형식을 `geometry`로 수정 하는 예제입니다.
...
Geography 데이터 형식과 마찬가지로 **geometry** 데이터 형식을 사용 하 여 인덱싱할 관련 경로와 유형을 지정 해야 합니다. 또한 인덱싱 정책 내의 `boundingBox` 지정 하 여 특정 경로에 대해 인덱싱할 원하는 영역을 나타내야 합니다. 각 지리 공간적 경로에는 자체`boundingBox`필요 합니다.
...
다음은 `geometry`로 설정 된 **geospatialConfig** 로 **geometry** 데이터를 인덱싱하는 인덱싱 정책 예제입니다.
...
> [!NOTE]
> **BoundingBox** 을 포함 하는 인덱싱 정책을 `geography` 데이터 형식의 컨테이너에 추가 하려고 하면 실패 합니다. **BoundingBox**를 추가 하기 전에 `geometry` 되도록 컨테이너의 **geospatialConfig** 를 수정 해야 합니다. 컨테이너의 지리 공간적 데이터 형식을 선택 하기 전이나 후에 데이터를 추가 하 고 인덱싱 정책의 나머지를 수정할 수 있습니다 (예: 경로 및 형식).
