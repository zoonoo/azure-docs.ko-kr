...
> [!NOTE]
> **BoundingBox** 를 포함 하는 인덱싱 정책을 데이터 형식의 컨테이너에 추가 하려고 하면 `geography` 오류가 발생 합니다.  `geometry` **BoundingBox** 를 추가 하기 전에 컨테이너의 geospatialConfig을로 수정 해야 합니다. 컨테이너의 지리 공간적 데이터 형식을 선택 하기 전이나 후에 데이터를 추가 하 고 인덱싱 정책의 나머지를 수정할 수 있습니다 (예: 경로 및 형식).
