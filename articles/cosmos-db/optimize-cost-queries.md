---
title: Azure Cosmos DB에서 쿼리를 실행하기 위해 비용 및 RU/s를 최적화합니다.
...
또한 SDK를 사용하여 쿼리 비용을 프로그래밍 방식으로 확인할 수도 있습니다. 만들기, 업데이트 또는 삭제와 같은 작업의 오버헤드를 측정하려면 REST API를 사용하여 `x-ms-request-charge` 헤더를 조사합니다. .NET 또는 Java SDK를 사용하는 경우 `RequestCharge` 속성은 요청 요금을 받을 수 있는 동등한 속성이며 이 속성은 ResourceResponse 또는 FeedResponse 내에 있습니다.
...
쿼리의 요청 단위는 여러 가지 요인에 따라 달라집니다. 예를 들어 로드/반환된 Azure Cosmos 항목의 수, 인덱스에 대한 조회 수, 쿼리 컴파일 시간 등 세부 정보입니다. Azure Cosmos DB는 동일한 데이터에 대해 실행될 경우 동일한 쿼리가 항상 반복 실행에서 동일한 요청 단위 수를 사용하도록 보장합니다. 쿼리 실행 메트릭을 사용하는 쿼리 프로필은 요청 단위가 소비되는 적절한 방식을 제시합니다.  
...
성능과 쿼리 및 UDF(사용자 정의 함수)에서 소비하는 처리량은 주로 함수 본문에 따라 달라집니다. UDF에서 쿼리 실행에 사용되는 시간 및 소비되는 RU 수를 확인하는 가장 쉬운 방법은 쿼리 메트릭을 사용하도록 설정하는 것입니다. .NET SDK를 사용하는 경우 SDK에서 반환하는 샘플 쿼리 메트릭은 다음과 같습니다.
...
* [Azure 코스모스 DB 예약 용량에](cosmos-db-reserved-capacity.md) 대해 자세히 알아보기
