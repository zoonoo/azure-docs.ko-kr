...
성능과 쿼리 및 UDF(사용자 정의 함수)에서 소비하는 처리량은 주로 함수 본문에 따라 달라집니다. UDF에서 쿼리 실행에 사용되는 시간 및 소비되는 RU 수를 확인하는 가장 쉬운 방법은 쿼리 메트릭을 사용하도록 설정하는 것입니다. .NET SDK를 사용 하는 경우 SDK에서 반환 된 샘플 쿼리 메트릭은 다음과 같습니다.
