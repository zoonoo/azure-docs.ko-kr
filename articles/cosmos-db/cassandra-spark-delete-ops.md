...
> [!NOTE]
> Spark 3.0 이상에서는 Cosmos DB 도우미 및 연결 팩터리를 설치할 필요가 없습니다. 또한 Spark 3 커넥터(위 참조)에 `connections_per_executor_max` 대신 `remoteConnectionsPerExecutor`를 사용해야 합니다. 위의 Notebook 내에서 연결 관련 속성이 정의된 것을 볼 수 있습니다. 아래 구문을 사용하여 클러스터 수준(Spark 컨텍스트 초기화)에서 정의하지 않고도 이러한 방식으로 연결 속성을 정의할 수 있습니다. 그러나 Spark 컨텍스트가 필요한 작업(예: 아래에 표시된 `delete`의 경우 `CassandraConnector(sc)`)을 사용하는 경우 연결 속성을 클러스터 수준에서 정의해야 합니다.
