...
이 문서에서는 다음과 같은 질문에 대한 대답을 제공합니다.
...
> [!NOTE]
> Azure Cosmos DB는 통신 모델이 RESTful이며 [SQL .NET 클라이언트 API](sql-api-sdk-dotnet.md)를 통해 사용할 수 있는 효율성 높은 TCP 프로토콜을 제공합니다.
> 
> 
...
| 컬렉션 |컬렉션은 JSON 문서 및 관련 JavaScript 응용 프로그램 논리의 컨테이너입니다. 컬렉션은 하나 이상의 파티션/서버에 걸쳐 있을 수 있으며 크기가 거의 무제한인 저장소 또는 처리량을 처리하도록 확장할 수 있습니다. |
| 저장 프로시저 |JavaScript로 작성된 응용 프로그램 논리로, 컬렉션에 등록되고 데이터베이스 엔진 내에서 트랜잭션으로 실행됩니다. |
| 트리거 |삽입, 바꾸기 또는 삭제 작업 전후에 실행된 JavaScript로 작성된 응용 프로그램 논리입니다. |
| UDF |JavaScript로 작성된 응용 프로그램 논리입니다. UDF를 사용하면 사용자 지정 쿼리 연산자를 모델링하여 핵심 SQL API 쿼리 언어를 확장할 수 있습니다. |
...
각 리소스에는 id 속성을 통해 노출되는 고유한 사용자 정의 이름이 있습니다. 참고: 문서의 경우 사용자가 id를 지정하지 않으면 SDK에서 자동으로 문서에 대한 고유 id를 생성합니다. id는 특정 상위 리소스 컨텍스트 내에서 고유한 사용자 정의 문자열로, 최대 256자입니다. 
...
<table border="0" cellspacing="0" cellpadding="0">
    <tbody>
        ...
        <tr>
            ...
            <td valign="top"><p>데이터베이스 계정 아래의 모든 리소스에 대한 관리 액세스 권한을 제공하는 기본 및 보조 마스터 키이며 읽기 전용 키입니다.</p></td>
        </tr>
    </tbody>
</table>
...
한 데이터베이스에는 컬렉션 내에서 분할된 무제한의 문서 저장소를 포함할 수 있습니다.

### <a name="elastic-scale-of-an-azure-cosmos-db-database"></a>Azure Cosmos DB 데이터베이스의 탄력적 확장
...
제품에 적용되는 데이터베이스 내에 컬렉션을 개수에 제한 없이 만들 수 있습니다. 각 컬렉션 또는 컬렉션 집합(데이터베이스 내)에는 선택한 제안에 따라 SSD 지원 저장소 및 처리량이 프로비전됩니다.
...
컬렉션은 본질적으로 탄력적입니다. 문서를 추가하거나 제거하면 자동으로 확장 및 축소됩니다. 컬렉션은 하나 이상의 물리적 파티션 또는 서버에 걸쳐 있을 수 있는 논리적 리소스입니다. 컬렉션에 할당된 파티션 수는 컬렉션 또는 컬렉션 집합에 대해 프로비전된 처리량 및 저장소 크기에 따라 Cosmos DB에 의해 결정됩니다. Cosmos DB의 모든 파티션은 고정된 양의 SSD 지원 저장소에 연결되며, 고가용성을 위해 복제됩니다. 파티션 관리는 Azure Cosmos DB에 의해 완전히 관리되므로 복잡한 코드를 작성하거나 파티션을 관리할 필요가 없습니다. Cosmos DB 컬렉션은 처리량 및 저장소 면에서 **제한되지 않습니다**. 
...
저장 프로시저 등록 취소는 기존 저장 프로시저 리소스에 대해 HTTP DELETE를 실행하여 수행합니다.   
...
트리거 등록 취소는 기존 트리거 리소스에 대해 HTTP DELETE를 실행하여 수행합니다.  
...
UDF는 SQL 쿼리의 일부로 지정될 수 있으며, 핵심 [SQL 쿼리 언어](sql-api-sql-query-reference.md)를 확장하는 방법으로 사용됩니다.
...
리소스 키를 받으려면 지정된 사용자로 사용 권한 리소스를 만들어야 합니다. 사용 권한을 만들거나 검색하려면 인증 헤더에 마스터 키가 있어야 합니다. 사용 권한 리소스는 리소스, 해당 액세스 권한 및 사용자를 연결합니다. 사용 권한 리소스를 만든 후에는 사용자가 관련 리소스에 액세스하기 위해 관련 리소스 키만 제공하면 됩니다. 따라서 리소스 키를 사용 권한 리소스의 단순한 논리적 표현으로 간주할 수 있습니다.  
