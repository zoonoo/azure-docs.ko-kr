...
description: Azure Cosmos DB에서 SQL 쿼리 실행에 대해 알아봅니다.
author: timsander1
...
.Net 클라이언트는 앞의 예제와 같이 `foreach` 블록에서 쿼리 결과의 모든 페이지를 자동으로 반복 합니다. [REST API](#REST-API) 섹션에 도입 된 쿼리 옵션은 `FeedOptions` `CreateDocumentQuery` 메서드의 및 `FeedResponse` 클래스를 사용 하 여 .net SDK 에서도 사용할 수 있습니다. `MaxItemCount` 설정을 사용 하 여 페이지 수를 제어할 수 있습니다.
