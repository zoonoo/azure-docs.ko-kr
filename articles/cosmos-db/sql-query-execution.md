...
이러한 리소스를 사용 하는 기본 상호 작용 모델은 HTTP `GET`동사 `PUT` `POST`,, 및 `DELETE`를 통해 표준 해석을 사용 합니다. 를 `POST` 사용 하 여 새 리소스를 만들거나 저장 프로시저를 실행 하거나 Cosmos DB 쿼리를 실행 합니다. 쿼리는 항상 파생 작업이 없는 읽기 전용 작업입니다.

다음 예에서는 샘플 항목 `POST` 에 대 한 SQL API 쿼리를 보여 줍니다. 이 쿼리는 JSON `name` 속성에 대 한 간단한 필터를 가집니다. 및 `x-ms-documentdb-isquery` content-type: `application/query+json` 헤더는 작업이 쿼리 임을 나타냅니다. Cosmos DB `mysqlapicosmosdb.documents.azure.com:443` 계정의 URI로 대체 합니다.
...
쿼리 결과가 단일 페이지에 맞지 않는 경우 REST API는 `x-ms-continuation-token` 응답 헤더를 통해 연속 토큰을 반환 합니다. 클라이언트는 후속 결과에 헤더를 포함 하 여 결과를 매길 수 있습니다. 숫자 헤더를 `x-ms-max-item-count` 통해 페이지 당 결과 수를 제어할 수도 있습니다.
...
쿼리에 대 한 데이터 일관성 정책을 관리 하려면 모든 REST API 요청 `x-ms-consistency-level` 에서와 같이 헤더를 사용 합니다. 또한 세션 일관성을 위해 쿼리 요청 `x-ms-session-token` 에서 최신 쿠키 헤더를 에코 해야 합니다. 쿼리된 컨테이너의 인덱싱 정책이 쿼리 결과의 일관성에 영향을 줄 수도 있습니다. 컨테이너에 대 한 기본 인덱싱 정책 설정을 사용 하 여 인덱스는 항상 항목 내용으로 최신 상태 이며 쿼리 결과는 데이터에 대해 선택한 일관성과 일치 합니다. 자세한 내용은 [Azure Cosmos DB 일관성 수준] [일관성 수준]을 참조 하세요.

컨테이너의 구성 된 인덱싱 정책이 지정 된 쿼리를 지원할 수 없는 경우 Azure Cosmos DB 서버는 400 "잘못 된 요청"을 반환 합니다. 이 오류 메시지는 인덱싱에서 명시적으로 제외 된 경로를 포함 하는 쿼리에 대해 반환 됩니다. 인덱스를 사용할 수 `x-ms-documentdb-query-enable-scan` 없는 경우 쿼리에서 검색을 수행할 수 있도록 헤더를 지정할 수 있습니다.

`x-ms-documentdb-populatequerymetrics` 헤더를로 `true`설정 하 여 쿼리 실행에 대 한 자세한 메트릭을 가져올 수 있습니다. 자세한 내용은 [SQL query metrics for Azure Cosmos DB](sql-api-query-metrics.md)(Azure Cosmos DB에 대한 SQL 쿼리 메트릭)를 참조하세요.
...
다음 예제에서는 LINQ `SelectMany`를 통해 표현 되는 조인을 보여 줍니다.
...
.NET 클라이언트는 앞의 예제와 같이 `foreach` 블록에서 쿼리 결과의 모든 페이지를 자동으로 반복 합니다. [REST API](#REST-API) 섹션에 도입 된 `FeedOptions` 쿼리 옵션은 `FeedResponse` `CreateDocumentQuery` 메서드의 및 클래스를 사용 하 여 .net SDK 에서도 사용할 수 있습니다. `MaxItemCount` 설정을 사용 하 여 페이지 수를 제어할 수 있습니다.

`IQueryable` 개체를 `ResponseContinuationToken` 사용 하 여 개체를 만든 `IDocumentQueryable` 다음 값을 읽고에서 `RequestContinuationToken` `FeedOptions`와 같이 다시 전달 하 여 페이징을 명시적으로 제어할 수도 있습니다. 구성 된 인덱싱 `EnableScanInQuery` 정책에서 쿼리를 지원 하지 않는 경우 검색을 사용 하도록 설정할 수 있습니다. 분할 된 컨테이너의 경우를 사용 `PartitionKey` 하 여 단일 파티션에 대해 쿼리를 실행할 수 있지만 Azure Cosmos DB는 쿼리 텍스트에서이를 자동으로 추출할 수 있습니다. 를 사용 `EnableCrossPartitionQuery` 하 여 여러 파티션에 대해 쿼리를 실행할 수 있습니다.
