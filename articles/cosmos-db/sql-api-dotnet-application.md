...
이 자습서에서는 Azure Cosmos DB를 사용하여 Azure에서 호스트하는 ASP.NET MVC 애플리케이션의 데이터를 저장하고 액세스하는 방법을 보여줍니다. 이 자습서에서는 .NET SDK V3을 사용합니다. 다음 이미지는 이 문서의 샘플을 사용하여 빌드하게 될 웹 페이지입니다.
...
> [!TIP]
> 이 자습서에서는 이전에 ASP.NET Core MVC 및 Azure App Service를 사용한 경험이 있다고 가정합니다. ASP.NET Core 또는 [필수 조건 도구](#prerequisites)를 처음 사용하는 경우 [GitHub][GitHub]에서 전체 샘플 프로젝트를 다운로드하고 필요한 NuGet 패키지를 추가하여 실행하는 것이 좋습니다. 프로젝트를 빌드하고 나면 이 문서를 검토하여 프로젝트의 컨텍스트에서 코드에 대한 인사이트를 얻을 수 있습니다.
...
이 문서의 모든 스크린샷은 Microsoft Visual Studio Community 2019를 사용하여 작성되었습니다. 시스템이 다른 버전으로 구성된 경우 화면과 옵션이 일부 달라질 수 있지만 위의 필수 구성 요소를 충족하면 이 솔루션을 사용할 수 있습니다.
...
2. **새 프로젝트** 창에서 **템플릿 검색** 입력 상자를 사용하여 "웹"을 검색한 다음, **ASP.NET Core 웹 애플리케이션**을 선택합니다. 

   ![새 ASP.NET Core 웹 애플리케이션 프로젝트 만들기](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-new-project-dialog.png)

3. **이름** 상자에 프로젝트의 이름을 입력합니다. 이 자습서에서는 "todo"라는 이름을 사용합니다. 이 이름 이외의 다른 이름을 사용하도록 선택하면 이 자습서에서 todo 네임스페이스를 지칭할 때마다 지정한 애플리케이션 이름을 사용하도록 제공된 코드 샘플을 조정합니다. 

4. **찾아보기**를 선택하여 프로젝트를 만들려는 폴더로 이동합니다. **만들기**를 선택합니다. 

5. **새 ASP.NET Core 웹 애플리케이션 만들기** 대화 상자가 표시됩니다. 템플릿 목록에서 **웹 애플리케이션(Model-View-Controller)** 을 선택합니다.

6. **만들기**를 선택하고, Visual Studio에서 빈 ASP.NET Core MVC 템플릿 주위에서 스캐폴드하도록 합니다. 
...
2. **NuGet 패키지 관리** 대화 상자가 나타납니다. NuGet **찾아보기** 상자에 **Microsoft.Azure.Cosmos**를 입력합니다. 결과에서 **Microsoft.Azure.Cosmos** 패키지를 설치합니다. 그러면 Azure Cosmos DB 패키지 및 해당 종속성이 다운로드되어 설치됩니다. **라이선스 승인** 창에서 **동의함**을 선택하여 설치를 완료합니다.
...
3. 패키지가 설치되면 Visual Studio 프로젝트에 Microsoft.Azure.Cosmos에 대한 라이브러리 참조가 있어야 합니다.
...
1. 새 클래스의 이름을 **Item.cs**로 지정하고 **추가**를 선택합니다. 

1. 다음으로, "Item.cs" 클래스의 코드를 다음 코드로 바꿉니다.
...
1. 새 컨트롤러 이름을 **ItemController**로 지정하고, 해당 파일의 코드를 다음 코드로 바꿉니다.
   ...
   **ValidateAntiForgeryToken** 특성은 여기서 교차 사이트 요청 위조 공격으로부터 이 애플리케이션을 보호하는 데 사용됩니다. 이 특성을 추가하는 것 외에 뷰가 이 위조 방지 토큰과 작동하도록 해야 합니다. 이 주제에 대한 자세한 내용과 이를 올바르게 구현하는 방법에 대한 예제는 전체 구현이 준비된 [교차 사이트 요청 위조 방지][Preventing Cross-Site Request Forgery]. The source code provided on [GitHub][GitHub]를 참조하세요.
...
먼저 Azure Cosmos DB에 연결하고 이를 사용하는 논리가 포함된 클래스를 추가합니다. 이 자습서에서는 이 논리를 `CosmosDBService` 클래스와 `ICosmosDBService` 인터페이스로 캡슐화합니다. 이 서비스는 불완전한 항목 나열, 항목 만들기, 편집 및 삭제와 같은 CRUD 및 읽기 피드 작업을 수행합니다. 

1. **솔루션 탐색기**에서 **Services**라는 프로젝트 아래에 새 폴더를 만듭니다.

1. **Services** 폴더를 마우스 오른쪽 단추로 클릭하고 **추가**를 선택한 다음, **클래스**를 선택합니다. 새 클래스 이름을 **CosmosDBService**로 지정하고, **추가**를 선택합니다.

1. 다음 코드를 **CosmosDBService** 클래스에 추가하고, 해당 파일의 코드를 다음 코드로 바꿉니다.

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Services/CosmosDbService.cs)]

1. 이번에는 **ICosmosDBService**라는 클래스에 대해 2-3단계를 반복하고, 다음 코드를 추가합니다.

   [!code-csharp[Main](~/samples-cosmosdb-dotnet-core-web-app/src/Services/ICosmosDbService.cs)]
 
1. 이전 코드는 생성자의 일부로 `CosmosClient`를 받습니다. ASP.NET Core 파이프라인에 따라 프로젝트의 **Startup.cs**로 이동하여 구성에 기반한 클라이언트를 [종속성 주입](https://docs.microsoft.com/aspnet/core/fundamentals/dependency-injection)에서 삽입할 Singleton 인스턴스로 초기화해야 합니다. **ConfigureServices** 처리기에서 다음을 정의합니다.
...
이제 애플리케이션을 실행하면 ASP.NET Core의 파이프라인에서 **CosmosDbService**를 인스턴스화하고 단일 인스턴스를 Singleton으로 유지합니다. **ItemController**를 사용하여 클라이언트 쪽 요청을 처리할 때 이 단일 인스턴스를 받고 이를 사용하여 CRUD 작업을 수행할 수 있습니다.
...
4. 목록에서 **항목** 옆의 **편집**을 클릭합니다. **편집** 뷰로 이동되며, 여기서 **완료** 플래그를 비롯한 개체 속성을 업데이트할 수 있습니다. **Complete** 플래그를 표시하고 **저장**을 클릭하면 목록에서 **Item**이 완료된 것으로 표시됩니다.
   
   ![완료 상자가 선택된 인덱스 뷰의 스크린샷](./media/sql-api-dotnet-application/asp-net-mvc-tutorial-completed-item.png)

5. 언제든지 [Cosmos Explorer](https://cosmos.azure.com) 또는 Azure Cosmos DB Emulator의 데이터 탐색기를 사용하여 Azure Cosmos DB 서비스에서 데이터의 상태를 확인할 수 있습니다.

6. 앱을 테스트하고 나면 Ctrl+F5를 눌러 앱 디버깅을 중지합니다. 배포할 준비가 되었습니다!
...
2. **게시** 대화 상자에서 **App Service**를 선택한 다음, **새로 만들기**를 선택하여 App Service 프로필을 만들거나 **기존 항목 선택**을 선택하여 기존 프로필을 사용합니다.
...
이 자습서에서는 Azure Cosmos DB에 저장된 데이터에 액세스할 수 있는 ASP.NET Core MVC 웹 애플리케이션을 빌드하는 방법에 대해 알아보았습니다. 이제 다음 문서로 진행할 수 있습니다.

* [Azure Cosmos DB에서 데이터를 분할하는 방법 알아보기](./partitioning-overview.md)
* [Azure Cosmos DB에서 고급 쿼리를 수행하는 방법 알아보기](./how-to-sql-query.md)
* [고급 시나리오에서 데이터를 모델링하는 방법 알아보기](./how-to-model-partition-example.md)
