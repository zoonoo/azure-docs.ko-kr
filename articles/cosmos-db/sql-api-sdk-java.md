---
title: 'Azure 코스모스 DB: SQL Java API, SDK & 리소스'
...
|**API 문서**|[Java API 참조 설명서](/java/api/com.microsoft.azure.documentdb)|
...
* 연결 풀 시간 시간 시간 문제를 해결합니다.
* 내부 재시도에 인증 토큰 새로 고침을 수정합니다.

### <a name="246"></a><a name="2.4.6"/>2.4.6
* 데이터베이스계정에서 올바른 클라이언트 측 복제본 정책 태그를 업데이트하고 데이터베이스계정 구성이 캐시에서 읽습니다.

### <a name="245"></a><a name="2.4.5"/>2.4.5
* 사용자가 pkRangeId를 제공하는 경우 잘못된 파티션 키 범위 오류에 대한 재시도를 방지합니다.

### <a name="244"></a><a name="2.4.4"/>2.4.4
* 최적화된 파티션 키 범위 캐시가 새로 고칩니다.
* SDK가 서버에서 파티션 분할 힌트를 접대하지 않고 잘못된 클라이언트 측 라우팅 캐시 새로 고침이 발생하는 시나리오를 수정합니다.

### <a name="242"></a><a name="2.4.2"/>2.4.2
* 최적화된 컬렉션 캐시새로 고침합니다.

### <a name="241"></a><a name="2.4.1"/>2.4.1
* 요청 진단 문자열에서 내부 예외 메시지를 검색하는 지원을 추가했습니다.

### <a name="240"></a><a name="2.4.0"/>2.4.0
* PartitionKey정의 버전 API를 도입했습니다.

### <a name="230"></a><a name="2.3.0"/>2.3.0
* 직접 모드에 대해 별도의 시간 시간 시간 지원추가했습니다.

### <a name="223"></a><a name="2.2.3"/>2.2.3
* 서비스에서 null 오류 메시지를 사용 하 고 문서 클라이언트 예외를 생성 합니다.

### <a name="222"></a><a name="2.2.2"/>2.2.2
* 소켓 연결 개선, 추가 SoKeepAlive 기본 true.

### <a name="220"></a><a name="2.2.0"/>2.2.0
* 요청 진단 문자열 지원이 추가되었습니다.

### <a name="213"></a><a name="2.1.3"/>2.1.3
* 해시 V2에 대한 파티션키의 버그를 수정했습니다.

### <a name="212"></a><a name="2.1.2"/>2.1.2
* 복합 인덱스에 대한 지원이 추가되었습니다.
* 전역 엔드포인트 관리자에서 버그를 수정하여 새로 고침을 강제수정했습니다.
* 직접 모드에서 사전 조건이 있는 업서트에 대한 버그를 수정했습니다.

### <a name="211"></a><a name="2.1.1"/>2.1.1
* 게이트웨이 주소 캐시에서 버그를 수정했습니다.
...
* 쿼리이터에서 null을 반환하지 않도록 하는 버그를 수정했습니다.
...
* 제한 오류가 발생할 때 DirectHttps 모드의 쿼리가 응답하지 않을 수 있는 버그를 수정했습니다.
...
> [!WARNING]
> Java용 SQL SDK의 모든 버전 **1.x는** **2020년 5월 30일에**사용 중지됩니다.
> 
>
