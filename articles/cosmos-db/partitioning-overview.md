...
위의 스크린 샷에서 컨테이너에는 `/foodGroup` 파티션 키가 있습니다. 그래프의 각 세 막대는 실제 파티션을 나타냅니다. 이미지에서 **파티션 키 범위** 는 실제 파티션과 같습니다. 선택한 실제 파티션에는 최상위 3 개의 가장 중요 한 논리적 파티션 `Beef Products` (, 및)이 포함 `Vegetable and Vegetable Products` `Soups, Sauces, and Gravies` 됩니다.
