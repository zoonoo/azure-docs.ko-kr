...
문서 데이터베이스를 만들려면 Azure Cosmos DB로 SQL(DocumentDB) 데이터베이스 계정을 만들어야 합니다.
...
1. **항목** 컬렉션을 확장하고 **문서** > **새 문서**를 클릭합니다.
   ...
   ![Azure Portal의 데이터 탐색기에서 새 문서 만들기](./media/create-documentdb-java/azure-cosmosdb-data-explorer-new-document.png)
...
2. 이제 다음과 같은 구조를 사용하여 컬렉션에 문서를 추가하고 **저장**을 클릭합니다.
    ...
    ![Azure Portal의 데이터 탐색기에서 Json 데이터를 복사하고 저장을 클릭합니다.](./media/create-documentdb-java/azure-cosmosdb-data-explorer-save-document.png)
...
1. 기본적으로 쿼리가 `SELECT * FROM c`로 설정되어 있는지 확인합니다. 이 기본 쿼리는 컬렉션에서 모든 문서를 검색하고 표시합니다. 
    ...
    ![데이터 탐색기에서 기본 쿼리는 `SELECT * FROM c`입니다.](./media/create-documentdb-java/azure-cosmosdb-data-explorer-query.png)

2. **필터 편집** 단추를 클릭하고, `ORDER BY c._ts DESC`를 쿼리 조건자 상자에 추가하고, **필터 적용**을 클릭하여 쿼리를 변경합니다.

    ![ORDER BY c._ts DESC를 추가하고 필터 적용을 클릭하여 기본 쿼리를 변경합니다.](./media/create-documentdb-java/azure-cosmosdb-data-explorer-edit-query.png)

이 수정된 쿼리는 해당 타임스탬프에 따라 내림차순으로 문서를 나열합니다. 따라서 이제 두 번째 문서가 첫 번째로 나열됩니다. SQL 구문에 익숙한 경우 이 상자에 지원되는 [SQL 쿼리](documentdb-sql-query.md)를 입력할 수 있습니다. 
...
1. [Azure Portal](http://portal.azure.com/)에서 **키**를 클릭합니다. 
    ...
    ![Azure Portal에서 선택키 보기 및 복사, 키 페이지](./media/create-documentdb-java/keys.png)
...
6. 콘솔 창에서 계속 키를 눌러 코드가 문서를 생성하도록 하고 쿼리를 수행합니다.
    ...
    ![콘솔 출력](./media/create-documentdb-java/console-output.png)
