---
title: Azure 코스모스 DB를 보유한 총 소유 비용(TCO)
...
* **가격 대비 훌륭한 가치:** 시장 분석가, 고객 및 파트너는 Azure Cosmos DB가 자체적으로 또는 다른 공급업체를 통해 이러한 솔루션을 구현할 때 고객이 얻을 수 있는 것과 비교하여 훨씬 저렴한 가격으로 제공하는 모든 기능의 더 큰 가치를 확인했습니다. 이러한 전역 분산, 다중 마스터, 잘 정의되고 간편한 일관성 모델, 자동 인덱싱과 같은 데이터베이스 기능은 Azure Cosmos DB를 사용할 경우 복잡성, 오버헤드 또는 가동 중지 없이 크게 간소화됩니다.

* **NoSQL DevOps 관리가 필요하지 않습니다.** Azure Cosmos DB에서는 배포를 관리하고 유지 관리, 확장 또는 패치를 수행하기 위해 DevOps를 사용할 필요가 없습니다. OSS NoSQL 클러스터를 온-프레미스 또는 클라우드 인프라에 호스트한 상태로 수행하는 모든 워크로드를 실행할 수 있습니다.

![Azure Cosmos DB 소유 비용](./media/total-cost-ownership/tco.png)

* **탄력적으로 확장할 수 있는 기능:** Azure Cosmos DB 처리량을 확장 및 축소할 수 있으므로 사용량이 많은 시간 동안 소유 비용을 줄일 수 있습니다. 클라우드 인프라에 배포된 OSS NoSQL 클러스터는 제한된 탄력성을 제공하며, 온-프레미스 배포는 본래부터 탄력적이지 않습니다. Azure Cosmos DB에서는 더 많은 처리량을 프로비전하는 경우 처리량이 비례적으로 확장될 수 있습니다. 어떤 규모에서든지 재무 SLA를 통해 99번째 백분위수로 이러한 확장이 보장됩니다.

* **규모의 경제:** Azure Cosmos DB와 같은 관리되는 서비스는 네트워킹, 저장소 및 컴퓨팅과 기본적으로 통합된 많은 수의 노드로 작동합니다. Azure Cosmos DB가 대규모로 표준화되어 있으므로 비용을 절감할 수 있습니다.

* **클라우드에 최적화된:** Azure Cosmos DB는 세분화된 다중 테넌시 및 성능 격리를 통해 처음부터 설계되었습니다. 따라서 수천 개의 테넌트와 해당 워크로드를 클러스터 및 데이터 센터에서 최적 상태로 배치, 실행하고, 부하를 분산할 수 있습니다. 반면, 현재의 OSS NoSQL 데이터베이스 세대는 전체 가상 머신이 단일 테넌트의 워크로드를 실행하는 것으로 간주되는 온-프레미스에서 작동합니다. 이러한 데이터베이스는 한 클라우드 공급자의 인프라 및 하드웨어를 최대한 활용하도록 디자인되지 않았습니다. 예를 들어, OSS NoSQL 데이터베이스 엔진은 가상 머신 작동 중단과 일상적인 이미지 업그레이드 간의 차이나 프리미엄 디스크가 이미 3방향으로 복제된다는 사실을 인식하지 못합니다. 이러한 이점을 활용할 수 없으며, 혜택 및 비용 절감을 고객에게 전달할 수 없습니다.

* **시간당 지불:** 언제든지 확장해야 하는 대규모 워크로드의 경우 시간단위로만 요금이 부과됩니다. 애플리케이션에 대한 워크로드는 일반적으로 연중 시간 및 쿼리되는 데이터에 따라 달라집니다. Azure Cosmos DB를 사용하는 경우 필요할 때 규모를 확장 및 축소할 수 있고 필요한 크기 만큼만 요금을 부과할 수 있습니다. 온-프레미스 또는 IaaS 호스팅 시스템을 사용하면 매시간 하드웨어를 해제할 수 있는 방법이 없기 때문에 이 모델을 일치시킬 수 없습니다. 이러한 경우 Azure Cosmos DB를 사용하여 평균적으로 10~14배의 비용을 절감할 수 있습니다.

* **당신은 무료로 수많은 기능을 얻을 :** Azure Cosmos DB에서 쓰기 워크로드는 대체 데이터베이스 서비스에 비해 상당히 저렴합니다. 또한 Azure Cosmos DB는 추가 요금 없이 [자동 인덱싱](indexing-policies.md), [TTL(Time to Live)](time-to-live.md), [변경 피드](change-feed.md)와 같은 기능 및 다른 데이터베이스 서비스에서는 일반적으로 요금이 청구되는 기능을 제공합니다.

* **다양한 워크로드에 통합 통화를 사용합니다.** Azure Cosmos DB의 대체 제품과 달리 워크로드를 읽기 및 쓰기로 분할할 필요가 없습니다. 또는 워크로드 유형별로 읽기 처리량 및 쓰기 처리량에 해당하는 처리량을 프로비전합니다. Azure Cosmos DB에서 프로비전된 처리량은 요청 단위 또는 RU/초를 기준으로 통합되고 정규화된 통화를 사용하여 예약됩니다. Azure Cosmos DB는 강제로 워크로드에 우선 순위를 할당하도록 하거나, 용량 계획을 수행하거나, 용량 유형별로 요금을 부과하지 않습니다. 이러한 접근 방식을 통해 다양한 작업 및 워크로드 유형 간에 동일한 RU/초를 쉽게 교환할 수 있습니다.

* **VM을 확장하기 위해 프로비저닝할 필요가 없습니다.** 대부분의 운영 데이터베이스는 시끄러운 이웃을 피하고 규모를 원하는 경우 느슨한 리소스 거버넌스를 위해 대형 가상 컴퓨터로 이동해야 합니다. 이로 인해 고객은 부담을 얻게 되며 사전 비용 약정이 요구됩니다. Azure Cosmos DB를 사용할 경우 처음에는 소규모로 시작했다가, 가동 중지 시간 또는 데이터 가용성에 미치는 영향 없이, 대규모 워크로드 크기로 원활하게 확장할 수 있습니다.

* **프로비저닝된 처리량을 최대 한도까지 활용할 수 있습니다.** Azure Cosmos DB의 서브 코어 멀티플렉싱덕분에 IaaS 호스팅 옵션 또는 타사 제공보다 프로비저닝된 처리량을 더 크게 포화시킬 수 있습니다. 이 방법은 대체 솔루션보다 훨씬 더 많은 비용 절감 효과를 가져옵니다.
...
* **기본적으로 최소 10-20개의 장애 도메인이 있는 고가용성이 자동으로** 제공됩니다. Azure Cosmos DB는 고가용성에 중요한 기능인 오류 도메인 전반에 걸쳐 워크로드 배포를 지원합니다. 전 세계 모든 위치에서 99번째 백분위수로, 읽기 및 쓰기에 대해 99.999의 고가용성을 제공합니다. 이와 같은 기능을 직접 또는 타사 솔루션을 통해 구현하게 되면 비용이 많이 듭니다.
...
* **예약된 용량으로 최대 65%의 비용을 절감할 수 있습니다.** Azure Cosmos DB [예약 용량은](cosmos-db-reserved-capacity.md) Azure Cosmos DB 리소스에 대해 1년 또는 3년 동안 선불하여 비용을 절감하는 데 도움이 됩니다. 1년 또는 3년 간의 사전 약정을 비용을 획기적으로 절감하고 정상 가격에 비해 20~65%의 할인 혜택을 받을 수 있습니다. 중요 업무용 워크로드의 경우, 용량 프로비전 측면에서 더 나은 SLA를 얻을 수 있습니다.
