---
title: Azure Cosmos DB 앱에 적합한 일관성 수준을 선택합니다.
...
고가용성, 낮은 대기 시간 또는 둘 다에 대한 복제에 의존하는 분산 데이터베이스는 읽기 일관성과 가용성, 대기 시간 및 처리량 간의 기본적인 절충을 수행합니다. 대부분의 상용 분산 데이터베이스는 개발자에게 *강력한* 일관성과 *최종* 일관성이라는 두 가지 극단적인 일관성 모델 중에서 선택하도록 요청합니다. Azure Cosmos DB를 사용하면 개발자가 잘 정의된 5가지 일관성 *모델(강력한,* *경계가 있는 부실,* *세션,* *일관된 접두사* 및 *최종.* 이러한 각 일관성 모델은 잘 정의되었으며, 직관적이고, 특정 실제 시나리오에 사용할 수 있습니다. 5가지 일관성 모델은 각각 정확한 [가용성과 성능 절충을](consistency-levels-tradeoffs.md) 제공하며 포괄적인 SLA를 통해 뒷받침됩니다. 다음 간단한 고려 사항은 여러 가지 일반적인 시나리오에서 올바른 선택을 하는 데 도움이 됩니다.

## <a name="sql-api-and-table-api"></a>SQL API 또는 Table API

SQL API 또는 테이블 API를 사용하여 응용 프로그램을 빌드하는 경우 다음 사항을 고려하십시오.

- 많은 실제 시나리오의 경우 세션 일관성이 최적이며 권장되는 옵션입니다. 자세한 내용은 응용 [프로그램의 세션 토큰을 관리하는 방법을](how-to-manage-consistency.md#utilize-session-tokens)참조하십시오.
...
- 가장 높은 가용성과 가장 낮은 대기 시간이 필요한 경우 최종 일관성 수준을 사용합니다.
...
## <a name="cassandra-mongodb-and-gremlin-apis"></a>카산드라, 몽고DB, 그렘린 API
...
실제로, 당신은 종종 더 강한 일관성 보장을 얻을 수 있습니다. 새로 고침 및 요청하는 데이터베이스의 상태 순서에 해당하는 읽기 작업에 대한 일관성 보장입니다. 읽기 일관성은 쓰기/업데이트 작업의 순서 지정 및 전파에 연결됩니다.  
...
Azure Cosmos 계정이 강력한 일관성 이외의 일관성 수준으로 구성된 경우 *PBS(Probabilistically 바인딩된 부실)* 메트릭을 확인하여 클라이언트가 워크로드에 대해 강력하고 일관된 읽기를 얻을 수 있는 가능성을 확인할 수 있습니다. 이 메트릭은 Azure Portal에서 노출됩니다. 자세히 알아보려면 [PBS(확률적 제한된 부실) 메트릭 모니터링](how-to-manage-consistency.md#monitor-probabilistically-bounded-staleness-pbs-metric)을 참조하세요.

확률적 제한된 부실은 최종이 어떻게 최종 일관성인지 보여줍니다. 이 메트릭은 Azure Cosmos 계정에서 현재 구성한 일관성 수준보다 얼마나 자주 더 강력한 일관성을 얻을 수 있는지에 대한 통찰력을 제공합니다. 즉, 쓰기 및 읽기 영역 조합에 대한 강력한 일관성 읽기를 가져오는 확률(밀리초로 제한됨)을 확인할 수 있습니다.
