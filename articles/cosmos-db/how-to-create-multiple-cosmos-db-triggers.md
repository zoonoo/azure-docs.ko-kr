---
title: 코스모스 DB에 대해 여러 개의 독립적인 Azure 함수 트리거 만들기
...
* **함수당 하나의 임대 컨테이너**만들기 : 공유 [처리량 데이터베이스를](./set-throughput.md#set-throughput-on-a-database)사용하지 않는 한 이 방법은 추가 비용으로 변환할 수 있습니다. 컨테이너 수준의 최소 처리량은 400개 [요청 단위](./request-units.md)이며, 임대 컨테이너의 경우 진행률 검사점을 확인하고 상태를 유지하는 데만 사용됩니다.
* **하나의 임대 컨테이너를** 가지고 모든 함수에 대해 공유: 이 두 번째 옵션은 여러 Azure Functions가 동일한 프로비저닝된 처리량을 공유하고 사용할 수 있도록 하기 때문에 컨테이너에서 프로비저닝된 요청 단위를 더 잘 사용할 수 있습니다.
