...
.NET SDK를 사용 하 여 큰 파티션 키를 사용 하 여 컨테이너를 만들려면, 지정 된 `PartitionKeyDefinitionVersion.V2` 속성입니다. 다음 예제에서는 PartitionKeyDefinition 개체 내에서 버전 속성을 지정 하 고 PartitionKeyDefinitionVersion.V2로 설정 하는 방법을 보여 줍니다.
...
## <a name="create-a-large-partition-key-powershell"></a>큰 파티션 키 (PowerShell) 만들기

PowerShell을 사용 하 여 큰 파티션 키를 사용 하 여 컨테이너를 만들려고 포함 `"version" = 2` 에 대 한는 `partitionKey` 개체입니다.
