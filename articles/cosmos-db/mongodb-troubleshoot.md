...
| MongoDB 유선 버전 문제 | - | 이전 버전의 MongoDB 드라이버는 연결 문자열에서 Azure Cosmos 계정의 이름을 검색할 수 없습니다. | 추가 *appName = @**accountName**@* Cosmos DB의 API에 대 한 MongoDB 연결 문자열의 끝에 있는 ***accountName*** 은 Cosmos DB 계정 이름 . |
