...
| 50 | ExceededTimeLimit | 요청이 실행 시간 제한인 60초를 초과했습니다. |  이 오류의 원인은 여러 가지가 있을 수 있습니다. 원인 중 하나는 현재 할당 된 요청 단위 용량이 부족 하 여 요청을 완료할 수 없는 경우입니다. 이는 해당 컬렉션 또는 데이터베이스의 요청 단위를 늘려서 해결할 수 있습니다. 다른 경우에는 대규모 요청을 더 작은 요청으로 분할 하 여이 오류를 해결할 수 있습니다. 이 오류가 수신 된 쓰기 작업을 다시 시도 하면 쓰기가 중복 될 수 있습니다. <br><br>RUs에 영향을 주지 않고 많은 양의 데이터를 삭제 하려는 경우: <br>-TTL (타임 스탬프 기반) 사용 고려: [MongoDB에 대 한 Azure Cosmos DB의 API를 사용 하 여 데이터 만료](https://docs.microsoft.com/azure/cosmos-db/mongodb-time-to-live) <br>-삭제를 수행 하려면 커서/일괄 처리 크기를 사용 합니다. 한 번에 하나의 문서를 가져와서 루프를 통해 삭제할 수 있습니다. 이렇게 하면 프로덕션 응용 프로그램에 영향을 주지 않고 데이터를 천천히 삭제 하는 데 도움이 됩니다.|
