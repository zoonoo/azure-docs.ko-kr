...
공개 미리 보기 릴리스에서는 Azure Cosmos DB SQL(Core) API에서만 Synapse Link가 지원됩니다. Cosmos DB의 API for MongoDB 및 Cassandra API에 대한 지원은 현재 제어된 미리 보기 상태입니다.  제어된 미리 보기에 대한 액세스를 요청하려면 [Azure Cosmos DB 팀](mailto:cosmosdbsynapselink@microsoft.com)에 문의하세요.
