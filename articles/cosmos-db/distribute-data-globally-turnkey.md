---
title: Azure Cosmos DB에서 턴키 글로벌 배포를 지원하는 방법 | Microsoft Docs
description: 전 세계적으로 배포된 다중 모델 데이터베이스 서비스인 Azure Cosmos DB에서 글로벌 데이터베이스를 사용한 전 세계적 지역에서 복제, 다중 마스터, 장애 조치(Failover), 데이터 복구에 대해 알아봅니다.
services: cosmos-db
author: markjbrown
manager: kfile
...
ms.locfileid: "46956917"
---
# <a name="how-azure-cosmos-db-enables-turnkey-global-distribution"></a>Azure Cosmos DB에서 턴키 글로벌 배포를 지원하는 방법
...
## <a id="GlobalDistribution"></a>글로벌 배포
...
### <a id="MultiMasterSupport"></a>다중 마스터 지원
Azure Cosmos DB는 원하는 위치에 쓰기 모델에 동등하게 참여하는 여러 마스터 영역에 대한 네이티브, 서버 쪽 지원을 제공합니다. 이 지원은 [경제적으로 지원되는 SLA](https://azure.microsoft.com/support/legal/sla/cosmos-db/)를 통해 10ms 미만의 쓰기 대기 시간과 99.999%의 쓰기 가용성을 제공합니다. 다중 마스터는 [SQL](sql-api-introduction.md), [MongoDB](mongodb-introduction.md), [Cassandra](cassandra-introduction.md), [Graph](graph-introduction.md) 및 [Table](table-introduction.md)을 비롯한 모든 API와 Cosmos DB용 모든 SDK 언어에 사용할 수 있습니다. Azure Cosmos DB는 다중 마스터 기능을 지원하는 계정에 4가지 일관성 수준(제한된 부실, 세션, 일관적인 접두사 및 최종)을 지원합니다.
...
![25개 Azure 지역을 아우르는 Azure Cosmos DB 데이터베이스 계정](./media/distribute-data-globally-turnkey/spanning-regions.png)
...
다중 마스터를 사용하지 않을 경우 가동 중단 발생 시 정확한 지역 장애 조치(failover) 순서를 제어할 수 있도록 Azure Cosmos DB는 데이터베이스 계정에 연결된 다양한 지역에 *우선 순위*를 연결할 수 있습니다(아래 그림 참조). Azure Cosmos DB는 사용자가 지정한 우선 순위에 따라 자동으로 장애 조치(Failover)를 수행합니다. 지역별 장애 조치(Failover)에 대한 자세한 내용은 [비즈니스 연속성을 위한 Azure Cosmos DB의 자동 지역별 장애 조치(Failover)](regional-failover.md)를 참조하세요. 다음은 Azure Cosmos DB의 테넌트가 데이터베이스 계정에 연결된 영역의 장애 조치(Failover) 우선 순위(오른쪽 창)를 구성하는 방법을 보여주는 이미지입니다.

![Azure Cosmos DB를 사용하여 장애 조치(Failover) 우선 순위 구성](./media/distribute-data-globally-turnkey/failover-priorities.png)
...
쓰기 지역이 한 개이고 읽기 복제본이 여러 개인 단일 마스터 시나리오에서는 Azure Cosmos DB를 통해 논리적(지역 무관) 또는 물리적(지역별) 엔드포인트를 사용하여 데이터베이스와 상호 작용할 수 있습니다. 논리적 엔드포인트를 사용하면 장애 조치(failover) 시 응용 프로그램을 투명하게 멀티 호밍할 수 있습니다. 물리적 엔드포인트는 읽기와 쓰기를 특정 지역에 리디렉션할 수 있는 세분화된 응용 프로그램 제어 기능을 제공합니다. 쓰기를 지원하는 지역이 여러 개 있는 시나리오에서 사용자는 사용량 및 리디렉션의 우선순위에 따라 지역이 추가된 지역별 엔드포인트를 지정해야 합니다.
...
Azure Cosmos DB의 일관성 SLA는 100%의 읽기 요청이 사용자가 지정한 일관성 모델(데이터베이스 계정 또는 요청 수준에서)에 대한 일관성 보장을 충족하도록 보장합니다. 일관성 수준과 연결된 모든 일관성 보증이 충족되면 읽기 요청이 일관성 SLA를 충족하는 것으로 간주됩니다. 다음 표에는 일관성 모델과 일관성 보증이 나와 있습니다. 이 일관성 모델은 각각 100%의 SLA를 보장합니다. 

|일관성 모델  | 특성  |
|---------|---------|
|강력 |  선형화 가능  |
|제한된 부실  |  단조 읽기(지역 내), 일관적인 접두사, 부실 제한 &lt; K,T   |
|세션  |  사용자 고유의 쓰기 읽기, 단조 읽기, 일관적인 접두사    |
|일관적인 접두사  | 일관적인 접두사  |
...
특정 일관성 모델의 구현은 선택하는 [쿼럼 유형](http://cs.brown.edu/~mph/HerlihyW90/p463-herlihy.pdf)에 따라 결정되므로 처리량 역시 선택하는 일관성 모델에 따라 결정됩니다. 예를 들어 Azure Cosmos DB에서 강력한 일관성 읽기에 대한 RU 요금은 최종적인 일관성 읽기에 대한 RU 요금의 약 *두 배*입니다. 이 경우 동일한 처리량을 달성하려면 RU의 2배를 프로비전해야 합니다. 다음은 Azure Cosmos DB에서 특정 일관성 모델에 대한 읽기 용량의 관계를 보여주는 이미지입니다.

![일관성과 처리량 간의 관계](./media/distribute-data-globally-turnkey/consistency-and-throughput.png)

## <a id="ThroughputGuarantees"></a>처리량 보장 
Azure Cosmos DB는 요구 사항 및 수요에 따라 다양한 지역의 처리량(및 저장소)을 탄력적으로 확장할 수 있습니다. 다음은 가로로 분할된 다음(지역 내에서 세 개의 리소스 파티션에서), 세 개의 Azure 지역에 걸쳐 전역적으로 분산된 단일 Azure Cosmos DB 컨테이너를 보여주는 이미지입니다.
...
![Azure Cosmos DB가 처리량을 탄력적으로 프로비전](./media/distribute-data-globally-turnkey/elastic-throughput.png)
...
Azure Cosmos DB는 처리량, 대기 시간, 일관성 및 가용성 메트릭을 투명하게 표시합니다. 이러한 메트릭은 Azure Portal을 통해 프로그래밍 방식으로 액세스할 수 있습니다(아래 그림 참조). 또한 Azure Application Insights를 사용하여 다양한 임계값에 대한 경고를 설정할 수 있습니다. 다음은 각 테넌트에 투명하게 제공되는 일관성, 대기 시간, 처리량 및 가용성 메트릭을 보여 주는 이미지입니다.
 
![Azure Cosmos DB 고객에게 표시되는 SLA 메트릭](./media/distribute-data-globally-turnkey/customer-slas.png)

## <a id="Next Steps"></a>다음 단계

* [Azure Cosmos DB 글로벌 배포의 주요 이점](distribute-data-globally-benefits.md)
