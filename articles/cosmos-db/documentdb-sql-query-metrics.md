---
title: "Azure Cosmos DB DocumentDB API에 대한 SQL 쿼리 메트릭 | Microsoft Docs"
...
Azure Cosmos DB에는 컨테이너에 데이터를 저장하여 [저장소 크기 또는 요청 처리량](partition-data.md)에 맞게 확장할 수 있습니다. Azure Cosmos DB는 데이터 증가 또는 프로비전된 처리량 증가를 처리하도록 데이터를 실제 파티션 간에 원활하게 조정합니다. REST API 또는 지원되는 [DocumentDB SDK](documentdb-sdk-dotnet.md) 중 하나를 사용하여 모든 컨테이너에 SQL 쿼리를 실행할 수 있습니다.
...
| `MaxDegreeOfParallelism` | Azure DocumentDB 데이터베이스 서비스에서 병렬 쿼리 실행 중이 클라이언트 쪽에서 실행되는 동시 작업 수를 가져오거나 설정합니다. 양수 속성 값은 동시 작업 수를 설정 값으로 제한합니다. 0보다 작은 값으로 설정할 경우 시스템이 실행할 동시 작업 수를 자동으로 결정합니다. |
...
REST API 요청 헤더 및 옵션에 대한 자세한 내용은 [DocumentDB REST API를 사용하여 리소스 쿼리](https://docs.microsoft.com/rest/api/documentdb/querying-documentdb-resources-using-the-rest-api)를 참조하세요.
...
* 지원되는 SQL 쿼리 연산자 및 키워드에 대해 알아보려면 [SQL 쿼리](documentdb-sql-query.md)를 참조하세요. 
* 요청 단위에 대해 알아보려면 [요청 단위](request-units.md)를 참조하세요.
