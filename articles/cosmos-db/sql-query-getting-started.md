...
두 번째 항목은 `givenName` `firstName` 및 `familyName` 대신및를사용합니다.`lastName`
...
다음 쿼리는 JSON 출력을 다른 모양으로 다시 포맷 합니다. 이 쿼리는 두 개의 선택 `Family` 된 `Name` 필드인 및 `City`주소 도시가 상태와 같은 새 JSON 개체를 프로젝션 합니다. "서 면"은 이러한 경우와 일치 합니다.
