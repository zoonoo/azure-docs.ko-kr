...
* 스키마의 모든 중첩 수준에서 최대 1000 속성을 가질 수 있으며 최대 중첩 깊이는 127입니다.
  * 처음 1000 속성만 분석 저장소에 표시 됩니다.
  * 처음 127 중첩 수준만 분석 저장소에 표시 됩니다.

* JSON 문서 (및 컬렉션/컨테이너 Cosmos DB)는 고유성 관점에서 대/소문자를 구분 하지만 분석 저장소는 그렇지 않습니다.

  * **동일한 문서에서:** 동일한 수준의 속성 이름은 대/소문자 소문자 비교할 때 고유 해야 합니다. 예를 들어 다음 JSON 문서에는 같은 수준에 "Name"과 "name"이 있습니다. 유효한 JSON 문서 이지만 고유성 제약 조건을 충족 하지 않으므로 분석 저장소에 완전히 표시 되지 않습니다. 이 예제에서는 대/소문자를 구분 하지 않고 비교할 때 "Name"과 "name"은 동일 합니다. 는 `"Name": "fred"` 첫 번째 항목 이므로 분석 저장소에만 표시 됩니다. 및는 `"name": "john"` 전혀 표시 되지 않습니다.
  ...
  * **다른 문서에서:** 같은 수준에 있지만 이름이 같고 이름이 같은 속성은 첫 번째 항목의 이름 형식을 사용 하 여 같은 열 내에 표시 됩니다. 예를 들어 다음 JSON 문서는 `"Name"` 같은 수준에 있는 및를 포함 합니다 `"name"` . 첫 번째 문서 형식은 이므로 `"Name"` 분석 저장소에서 속성 이름을 나타내는 데 사용 됩니다. 즉, 분석 저장소의 열 이름은이 됩니다 `"Name"` . `"fred"` `"john"` 열에 및가 모두 표시 됩니다 `"Name"` .
...
* 컬렉션의 첫 번째 문서는 초기 분석 저장소 스키마를 정의 합니다.
  * 문서의 첫 번째 수준에서 속성이 열로 표시 됩니다.
  * 초기 스키마 보다 속성이 많은 문서는 분석 저장소에 새 열을 생성 합니다.
  * 열을 제거할 수 없습니다.
  * 컬렉션에 있는 모든 문서를 삭제 해도 분석 저장소 스키마는 다시 설정 되지 않습니다.
  * 스키마 버전 관리가 없습니다. 트랜잭션 저장소에서 유추 된 마지막 버전은 분석 저장소에 표시 되는 내용입니다.

* 현재 Azure Synapse Spark는 공백 (공백)을 포함 하는 열 이름을 읽을 수 없습니다.
...
* 누락 된 열과 관련 하 여 다음과 같은 동작이 발생 합니다.
  ...
  * Azure Synapse의 SQL server 서버 리스 풀은 이러한 열을로 표시 `NULL` 합니다.
...
* 속성은 항상 여러 항목에서 동일한 형식을 갖습니다.
  ...
  * 예를 들어, `"a"`는 문자열인 경우도 있고 숫자인 경우도 있기 때문에 `{"a":123} {"a": "str"}`에는 잘 정의된 스키마가 없습니다. 이 경우 분석 저장소는의 데이터 형식을 `"a"` `“a”` 컨테이너 수명의 첫 번째 발생 항목에 있는의 데이터 형식으로 등록 합니다. 문서는 여전히 분석 저장소에 포함 되지만의 데이터 형식이 서로 다른 항목은 포함 `"a"` 되지 않습니다.
...
* 잘 정의 된 스키마의 다양 한 형식에 대해 서로 다른 동작을 수행 합니다.
  * Azure Synapse의 Spark 풀은 이러한 값을로 나타냅니다 `undefined` .
  * Azure Synapse의 SQL server 서버 리스 풀은 이러한 값을로 나타냅니다 `NULL` .
