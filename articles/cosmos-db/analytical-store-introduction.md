...
* 현재는 공백이 포함된 열 이름을 읽는 Azure Synapse Spark를 지원하지 않습니다.

* 명시적 `null` 값과 관련하여 다른 동작이 발생합니다.
  * Azure Synapse의 Spark 풀은 이러한 값을 `0`(영)으로 읽습니다.
  * Azure Synapse의 SQL 서버리스 풀은 컬렉션의 첫 번째 문서에 동일한 속성에 대해 `non-numeric` 데이터 형식의 값이 있는 경우 이러한 값을 `NULL`로 읽습니다.
  * 컬렉션의 첫 번째 문서에 동일한 속성에 대해 `numeric` 데이터 형식의 값이 있는 경우 Azure Synapse의 SQL 서버리스 풀은 이러한 값을 `0`(영)으로 읽습니다.
...
> [!NOTE]
> SQL(Core) API 계정의 경우 분석 저장소를 사용으로 설정하면 분석 저장소의 기본 스키마 표시가 올바르게 정의됩니다. Azure Cosmos DB API for MongoDB 계정의 경우 분석 저장소의 기본 스키마 표시가 전체 충실도 스키마 표시입니다. (이러한 각 API에 기본적으로 제공되는 표시와 다른 스키마 표시가 필요한 시나리오가 있으면 [Azure Cosmos DB 팀](mailto:cosmosdbsynapselink@microsoft.com)에 문의하여 사용하도록 설정하세요.)
...
> [!NOTE]
> Azure Synapse Analytics 런타임을 사용하는 분석 저장소에서만 읽을 수 있습니다. 트랜잭션 저장소에 서비스 레이어로 데이터를 다시 작성할 수 있습니다.
...
컨테이너에서 분석 저장소를 사용하도록 설정하는 경우:

* Azure Portal에서 분석 TTL 옵션은 기본값인 -1로 설정됩니다. Data Explorer 아래의 컨테이너 설정으로 이동하여 이 값을 'n'초로 변경할 수 있습니다. 
 
* Azure SDK, PowerShell 또는 CLI에서 분석 TTL 옵션을 -1 또는 'n'으로 설정하여 사용하도록 설정할 수 있습니다. 
