...
Azure Cosmos DB는 모든 API에서 다음 유형의 컨테이너를 지원합니다.
...
### <a name="physical-partition"></a>실제 파티션

*실제* 파티션은 고정된 양의 예약된 SSD 기반 저장소에 가변적 양의 계산 리소스(CPU 및 메모리)가 결합되어 있습니다. 고가용성을 위해 실제 파티션 각각을 복제합니다. 각 컨테이너 집합은 하나 이상의 실제 파티션을 공유할 수 있습니다. 실제 파티션 관리는 Azure Cosmos DB에 의해 완전히 관리되므로 복잡한 코드를 작성하거나 파티션을 관리할 필요가 없습니다. Azure Cosmos DB 컨테이너는 처리량 및 저장소 면에서 제한되지 않습니다. 실제 파티션은 Azure Cosmos DB의 내부 개념이며 일시적입니다. Azure Cosmos DB는 워크로드에 따라 실제 파티션 수를 자동으로 조정합니다. 따라서 실제 파티션 수에 따라 데이터베이스 디자인 상관 관계를 지정하는 대신 논리 파티션을 결정하는 올바른 파티션 키를 선택해야 합니다. 

### <a name="logical-partition"></a>논리 파티션
...
컨테이너가 [분할 필수 구성 요소](#prerequisites)를 충족하는 경우 분할은 응용 프로그램에 대해 투명합니다. Azure Cosmos DB에서는 실제 및 논리 파티션 간에 데이터를 분산하고 올바른 파티션에 대한 쿼리 요청을 라우팅하는 작업을 처리합니다. 

## <a name="how-does-partitioning-work"></a>분할 작동 방법

각 문서에는 *파티션 키* 및 *행 키*가 있어서 항목을 고유하게 식별해야 합니다. 파티션 키는 데이터에 대한 논리적 분할의 역할을 하고 실제 파티션 간에 데이터를 배포하는 자연 경계를 가진 Azure Cosmos DB를 제공합니다. **단일 논리 파티션에 대한 데이터는 단일 실제 파티션 내에 있어야 하며, 실제 파티션은 Azure Cosmos DB에서 관리됩니다**. 
...
* Azure Cosmos DB는 **N**의 실제 파티션에 걸쳐 균등하게 파티션 키 해시의 주요 공간을 할당합니다. 따라서 각 실제 파티션이 호스트하는 논리 파티션 수는 [**1/N** * 파티션 키 값의 수]입니다.  
...
Azure Cosmos DB는 해시 기반 분할을 사용합니다. 항목을 작성하는 경우 Azure Cosmos DB는 파티션 키 값을 해시하고 해시된 결과를 사용하여 항목을 저장할 파티션을 결정합니다. 

> [!NOTE]
> Azure Cosmos DB는 동일한 실제 파티션에 동일한 파티션 키를 가진 모든 항목을 저장합니다. 

## <a name="best-practices-when-choosing-a-partition-key"></a>파티션 키 선택 시 모범 사례

파티션 키의 선택은 디자인 타임에서 결정해야 하는 중요한 사항입니다. 다양한 범위의 값과 균일한 액세스 패턴을 가진 속성 이름을 선택합니다. 많은 수의 고유 값(예: 수백 또는 수천 개)을 갖는 파티션 키가 적합합니다. 이렇게 하면 값 사이에 워크로드가 고르게 배분됩니다. 이상적인 파티션 키는 쿼리에서 자주 필터로 표시되며 솔루션 확장에 충분한 카디널리티가 있는 키입니다.

실제 파티션이 저장소 제한에 도달하고 동일한 파티션의 데이터가 동일한 파티션 키를 갖는다면 Azure Cosmos DB는 *"파티션 키가 최대 크기인 10GB에 도달했습니다."* 라는 메시지를 반환하고 파티션은 분할되지 않습니다. 파티션 키 선택은 매우 중요한 결정입니다. 

다음과 같은 파티션 키를 선택합니다.

* 저장소가 모든 키에 균등하게 분산됩니다.  
* 데이터를 파티션에 균등하게 분산하는 파티션 키를 선택합니다.
...
* 가능한 경우 파티션 경계 내의 데이터를 가져오도록 쿼리를 최적화합니다. 최적의 분할 전략은 쿼리 패턴에 맞추는 것입니다. 단일 파티션의 데이터를 가져오는 쿼리는 가능한 최상의 성능을 제공합니다. 높은 동시성을 사용하여 호출되는 쿼리는 필터 조건자에 파티션 키를 포함하여 효율적으로 라우팅할 수 있습니다.  

* 카디널리티가 높은 파티션 키를 선택하는 것이 일반적입니다. 일반적으로 분산 및 확장성이 우수하기 때문입니다. 예를 들어 여러 속성의 값을 연결하여 카디널리티를 높이는 방법으로 가상 키를 만들 수 있습니다.  

위의 고려 사항을 참고하여 파티션 키를 선택할 때 파티션 수 또는 실제 파티션마다 할당되는 처리량을 신경 쓸 필요가 없습니다. Azure Cosmos DB가 실제 파티션 수를 자동으로 조정하고, 필요에 따라 개별 파티션도 확장할 수 있기 때문입니다.

## <a name="prerequisites"></a>분할을 위한 필수 구성 요소

Azure Cosmos DB 컨테이너를 고정 또는 무제한으로 만들 수 있습니다. 고정 크기 컨테이너는 최대 제한 10GB 및 10,000RU/s 처리량을 설정할 수 있습니다. 무제한으로 컨테이너를 만들려면 파티션 키와 최소 1,000RU/s 처리량을 지정해야 합니다. 또한 처리량을 공유하도록 Azure Cosmos DB 컨테이너를 만들 수도 있습니다. 이러한 경우, 각 컨테이너는 파티션 키를 지정해야 하며 무제한으로 증가할 수 있습니다. 

다음은 파티션 및 크기 조정에서 고려해야 하는 필수 구성 요소입니다.

* Azure Portal에서 컨테이너(예: 컬렉션, 그래프 또는 테이블)를 만들 때 무제한 확장을 활용할 수 있게 **무제한** 저장소 용량 옵션을 선택합니다. [분할 작동 방법](#how-does-partitioning-work) 문서에 설명된 대로 실제 파티션을 **p1** 및 **p2**로 자동 분할하려면 1,000RU/s 이상의 처리량을 포함(또는 컨테이너 집합 간에 처리량을 공유)하는 컨테이너를 만들고 파티션 키를 제공해야 합니다. 

* 초기 처리량이 1,000RU/s 이상인 컨테이너를 작성하고 파티션 키를 제공하는 경우, 컨테이너를 변경하지 않고도 무제한 크기 조정 기능을 이용할 수 있습니다. 즉, **고정** 컨테이너를 작성하는 경우, 처리량이 1,000RU/s 이상인 컨테이너를 작성한 경우 및 파티션 키가 지정된 경우 컨테이너가 무제한 컨테이너로 작동합니다.

* 컨테이너 집합의 일부로 처리량을 공유하도록 구성된 모든 컨테이너는 **무제한** 컨테이너로 취급됩니다.

파티션 키 없이 또는 1,000RU/s 미만의 처리량으로 **고정** 컨테이너를 만든 경우 이 컨테이너는 자동 확장되지 않습니다. 고정 컨테이너에서 무제한 컨테이너로 데이터를 마이그레이션하려면 [데이터 마이그레이션 도구](import-data.md) 또는 [변경 피드 라이브러리](change-feed.md)를 사용해야 합니다. 

## <a name="designing-for-partitioning"></a>파티션 키 만들기 
...
## <a name="form-partition-key-by-concatenating-multiple-fields"></a>여러 필드를 연결하여 파티션 키 구성

여러 속성 값을 항목의 인위적인 “partitionKey” 속성 하나에 연결하고 패딩하여 파티션 키를 구성할 수도 있습니다. 이러한 키는 가상 키로 참조됩니다.
...
한 가지 옵션은 /deviceId 또는 /date에 partitionKey를 설정하는 것입니다. 장치 ID와 날짜에 파티션 키를 구성하려는 경우 이러한 두 값을 인위적인 “partitionKey” 속성에 연결하고 파티션 키를 /partitionKey로 설정합니다.
...
실시간 시나리오에서는 수천 가지 문서가 있을 수 있으므로 값을 가상 키로 값을 연결하도록 클라이언트 쪽 논리를 정의하고, 가상 키를 문서에 삽입한 다음, 해당 가상 키를 사용하여 파티션 키를 지정해야 합니다.

## <a name="designing-for-scale"></a>크기 조정을 위한 설계
