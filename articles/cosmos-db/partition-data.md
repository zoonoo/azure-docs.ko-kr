...
Azure Cosmos DB는 해시 기반 분할을 사용합니다. 항목을 작성하는 경우 Azure Cosmos DB는 파티션 키 값을 해시하고 해시된 결과를 사용하여 항목을 저장할 파티션을 결정합니다. Azure Cosmos DB는 동일한 실제 파티션에 동일한 파티션 키를 가진 모든 항목을 저장합니다. 
...
실제 파티션이 저장소 제한에 도달하고 동일한 파티션의 데이터가 동일한 파티션 키를 갖는다면 Azure Cosmos DB는 *"파티션 키가 최대 크기인 10GB에 도달했습니다."* 라는 메시지를 반환하고 파티션은 분할되지 않습니다. 파티션 키 선택은 매우 중요한 결정입니다. 실제 파티션은 Azure Cosmos DB의 내부 개념이며 일시적입니다. Azure Cosmos DB는 워크로드에 따라 실제 파티션 수를 자동으로 조정합니다. 따라서 실제 파티션 수에 따라 데이터베이스 디자인 상관 관계를 지정하는 대신 올바른 파티션 키(논리 파티션)을 선택해야 합니다. 
...
* 카디널리티가 높은 파티션 키를 선택하는 것이 일반적입니다. 일반적으로 분산 및 확장성이 우수하기 때문입니다. 예를 들어 여러 속성의 값을 연결하여 카디널리티를 높이는 방법으로 복합 키를 만들 수 있습니다. 
...
Azure Portal에서 Azure Cosmos DB 컨테이너를 *고정* 또는 *무제한*으로 만들 수 있습니다. 고정 크기 컨테이너는 최대 제한 10GB 및 10,000RU/s 처리량을 설정할 수 있습니다. 무제한으로 컨테이너를 만들려면 파티션 키와 최소 1,000RU/s 처리량을 지정해야 합니다. Azure DB Cosmos 컨테이너를 컨테이너 집합 간에 처리량을 공유하도록 구성할 수도 있습니다. 이 경우 각 컨테이너는 파티션 키를 지정해야 하며 무제한으로 커질 수 있습니다.
