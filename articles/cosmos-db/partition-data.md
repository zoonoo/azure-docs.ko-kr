...
* 높은 동시성을 사용하여 호출되는 쿼리는 필터 조건자에 파티션 키를 포함하여 효율적으로 라우팅할 수 있습니다.  
...
Azure Portal에서 Azure Cosmos DB 컨테이너를 고정 또는 무제한으로 만들 수 있습니다. 고정 크기 컨테이너는 최대 제한 10GB 및 10,000RU/s 처리량을 설정할 수 있습니다. 무제한으로 컨테이너를 만들려면 파티션 키와 최소 1,000RU/s 처리량을 지정해야 합니다. Azure DB Cosmos 컨테이너를 컨테이너 집합 간에 처리량을 공유하도록 구성할 수도 있습니다. 이 경우 각 컨테이너는 파티션 키를 지정해야 하며 무제한으로 커질 수 있습니다. 다음은 파티션 및 크기 조정에서 고려해야 하는 필수 구성 요소입니다.

* Azure Portal에서 컨테이너(예: 컬렉션, 그래프 또는 테이블)를 만들 때 무제한 확장을 활용할 수 있게 **무제한** 저장소 용량 옵션을 선택합니다. [분할 작동 방법](#how-does-partitioning-work)에 설명된 대로 실제 파티션을 **p1** 및 **p2**로 자동 분할하는 경우 1,000RU/s 이상의 처리량을 포함(또는 컨테이너 집합 간에 처리량을 공유)하는 컨테이너를 만들고 파티션 키를 제공해야 합니다. 
...
## <a name="create-partition-key"></a>파티션 키 만들기 
