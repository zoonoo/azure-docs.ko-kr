...
Azure Cosmos DB는 해시 기반 분할을 사용합니다. 항목을 작성하는 경우 Azure Cosmos DB는 파티션 키 값을 해시하고 해시된 결과를 사용하여 항목을 저장할 파티션을 결정합니다. Azure Cosmos DB는 동일한 실제 파티션에 동일한 파티션 키를 가진 모든 항목을 저장합니다. 파티션 키의 선택은 디자인 타임에서 결정해야 하는 중요한 사항입니다. 다양한 범위의 값과 균일한 액세스 패턴을 가진 속성 이름을 선택해야 합니다. 실제 파티션이 저장소 제한에 도달하고 동일한 파티션의 데이터가 동일한 파티션 키를 갖는다면 Azure Cosmos DB는 *"파티션 키가 최대 크기인 10GB에 도달했습니다."* 라는 메시지를 반환하고 파티션은 분할되지 않습니다.  파티션 키 선택은 매우 중요한 결정입니다.
...
파티션에 데이터가 분산되는 방법을 확인하는 것이 좋습니다. 포털에서 이를 확인하려면 Azure Cosmos DB 계정으로 이동하고 **모니터링** 섹션에서 **메트릭**을 클릭한 다음, **저장소** 탭을 클릭하여 데이터가 다른 실제 파티션으로 분할되는지 확인합니다.
...
[분할 작동 방법](#how-does-partitioning-work)에 설명된 대로 실제 파티션을 **p1** 및 **p2**로 자동 분할하는 경우 1,000RU/s 이상의 처리량을 포함하는 컨테이너를 만들고 파티션 키를 제공해야 합니다. Azure Portal에서 컨테이너(예: 컬렉션, 그래프 또는 테이블)를 만들 때 무제한 확장을 활용할 수 있게 **무제한** 저장소 용량 옵션을 선택합니다. 
...
파티션 키 없이 또는 1,000RU/s 미만의 처리량으로 **고정** 컨테이너를 만든 경우 이 컨테이너는 이 문서에서 설명한 것처럼 자동 확장되지 않습니다. 이 같은 컨테이너에서 무제한 컨테이너(최소 1,000RU/s 처리량 및 파티션 키가 있는 하나의 컨테이너)로 데이터를 마이그레이션하려면 [데이터 마이그레이션 도구](import-data.md) 또는 [변경 피드 라이브러리](change-feed.md)를 사용해야 합니다. 

## <a name="partitioning-and-provisioned-throughput"></a>분할 및 프로비전된 처리량
Azure Cosmos DB는 예측 가능한 성능을 위해 설계되었습니다. 컨테이너를 만들 때 *초당 [RU(Request Units)](request-units.md) 단위*로 처리량을 예약합니다. 각 요청에서는 작업에 소비된 CPU, 메모리 및 IO 같은 시스템 리소스의 양에 비례하는 RU 요금이 발생합니다. 세션 일관성이 적용된 1KB 문서 읽기는 1RU가 소비됩니다. 저장된 항목 수 또는 동시에 실행되는 요청 수에 상관없이 읽기당 1RU입니다. 항목의 크기가 클수록 크기에 따라 더 높은 RU가 필요합니다. 응용 프로그램에 지원해야 하는 엔터티 크기 및 읽기 수를 알고 있는 경우 응용 프로그램의 요구에 맞게 정확히 필요한 처리량을 프로비전할 수 있습니다. 

> [!NOTE]
> 컨테이너를 통해 프로비전된 처리량을 완벽하게 활용하려면 모든 고유한 파티션 키 값 간에 요청을 균등하게 배분할 수 있는 파티션 키를 선택해야 합니다.
> 
> 
...
* 다른 방법은 여러 컨테이너가 있는(각 일/월에 대해 하나) 하이브리드 방식으로, 파티션 키는 *호스트 이름*처럼 더 세부적인 속성입니다. 이 방식은 시간 범위 및 규모와 성능 요구에 따라 각 컨테이너에 대해 서로 다른 처리량을 설정할 수 있는 장점이 있습니다. 예를 들어 이번 달의 컨테이너는 읽기와 쓰기를 서비스하므로 더 높은 처리량을 프로비전할 수 있습니다. 지난 달의 경우 읽기만 서비스했으므로 더 낮은 처리량을 프로비전했습니다.
