...
이 인덱싱 정책은, 및를 기본값으로 수동으로 설정 하는 아래 ```kind``` ```dataType``` 와 동일 ```precision``` 합니다. 이러한 속성은 더 이상 명시적으로 설정할 필요가 없으며, 위의 예제와 같이 인덱싱 정책에서 완전히 생략 해야 합니다.
...
[TTL (time-to-live) 기능이](time-to-live.md) 활성화 되어 있지만 추가 인덱스가 필요 하지 않은 상황에서이 정책을 사용할 수 있습니다 (Azure Cosmos DB를 순수 키-값 저장소로 사용).
...
> [!NOTE]
> 인덱싱 정책을 업데이트할 때 Azure Cosmos DB에 대 한 쓰기는 중단 되지 않습니다. [인덱싱 변환](index-policy.md#modifying-the-indexing-policy) 에 대 한 자세한 정보
