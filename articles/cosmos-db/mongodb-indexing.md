...
MongoDB server 버전 3.6에 대 한 Azure Cosmos DB API는 `_id` 필드를 자동으로 인덱싱하고 삭제할 수 없습니다. 분할 된 키 당 `_id` 필드의 고유성을 자동으로 적용 합니다.

추가 필드를 인덱싱 하려면 MongoDB 인덱스 관리 명령을 적용 합니다. MongoDB에서와 마찬가지로, MongoDB에 대 한 Azure Cosmos DB API는 `_id` 자동으로 필드를 인덱싱합니다. 이 기본 인덱싱 정책은 모든 필드를 기본적으로 인덱싱하는 Azure Cosmos DB SQL API와 다릅니다.
...
단일 필드에 인덱스를 만들 수 있습니다. 단일 필드 인덱스의 정렬 순서는 중요 하지 않습니다. 다음 명령은 필드 `name`에 인덱스를 만듭니다.
...
다음 명령은 및 `name` `age`필드에 복합 인덱스를 만듭니다.
...
앞의 복합 인덱스를 사용 하 여 모든 필드에서 정렬 순서가 반대인 쿼리를 효율적으로 정렬할 수도 있습니다. 아래 예를 살펴보세요.
...
지리 공간적 연산자는 대부분 지리 공간적 인덱스를 활용 합니다. 현재 MongoDB에 대 한 Azure Cosmos DB API는 `2dsphere` 인덱스를 지원 합니다. API는 인덱스를 아직 지원 `2d` 하지 않습니다.
...
다음 명령은 필드 `student_id`에 고유 인덱스를 만듭니다.
...
다음 ```coll``` 명령은 및 ```university``` `student_id` `university`필드에 고유 인덱스가 있는 분할 된 컬렉션 (분할 키)을 만듭니다.
...
앞의 예제에서 ```"university":1``` 절을 생략 하면 다음 메시지와 함께 오류가 반환 됩니다.

```"cannot create unique index over {student_id : 1.0} with shard key pattern { university : 1.0 }"```

### <a name="ttl-indexes"></a>TTL 인덱스

특정 컬렉션에서 문서 만료를 사용 하도록 설정 하려면 [TTL (time-to-live) 인덱스](../cosmos-db/time-to-live.md)를 만들어야 합니다. TTL 인덱스는 `_ts` `expireAfterSeconds` 값이 있는 필드의 인덱스입니다.
...
이전 명령은 지난 10 초 동안 수정 되지 ```db.coll``` 않은 컬렉션의 모든 문서를 삭제 합니다.
...
MongoDB에 대 한 Azure Cosmos DB의 API 버전 3.6은 `currentOp()` 데이터베이스 인스턴스의 인덱스 진행률을 추적 하는 명령을 지원 합니다. 이 명령은 데이터베이스 인스턴스에 대 한 진행 중인 작업에 대 한 정보가 포함 된 문서를 반환 합니다. `currentOp` 명령을 사용 하 여 네이티브 MongoDB의 모든 진행 중인 작업을 추적 합니다. MongoDB에 대 한 Azure Cosmos DB API에서이 명령은 인덱스 작업 추적만 지원 합니다.

다음은 `currentOp` 명령을 사용 하 여 인덱스 진행률을 추적 하는 방법을 보여 주는 몇 가지 예입니다.
...
- "Foo" 컬렉션 및 "bar" 데이터베이스의 인덱스 작업 (60% 완료)은 다음과 같은 출력 문서를 포함 합니다. 필드 `Inprog[0].progress.total` 는 100를 대상 완료 백분율로 표시 합니다.
...
현재 컬렉션에 문서가 포함 되지 않은 경우에만 고유 인덱스를 만들 수 있습니다. 인기 있는 MongoDB 마이그레이션 도구는 데이터를 가져온 후 고유 인덱스를 만들려고 합니다. 이 문제를 피하려면 마이그레이션 도구를 사용해 볼 수 있도록 하는 대신 해당 컬렉션 및 고유 인덱스를 수동으로 만들 수 있습니다. 명령줄에서 플래그를 ```mongorestore``` `--noIndexRestore` 사용 하 여에 대해이 동작을 수행할 수 있습니다.
