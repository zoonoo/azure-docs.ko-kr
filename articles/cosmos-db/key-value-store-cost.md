...
이 문서는 키/값 저장소로 사용될 경우 간단한 쓰기 및 읽기 작업의 Azure Cosmos DB 비용에 대해 설명합니다. 쓰기 작업에는 문서의 삽입, 바꾸기, 삭제 및 upsert가 포함됩니다. 모든 단일 지역 계정 및 모든 다중 지역 계정에 대해 99.99% 가용성 SLA를 보장 하는 것 외에도 모든 다중 지역 데이터베이스 계정에 대해 99.999%의 가용성을 보장 하는 것 외에도 Azure Cosmos DB은 < 10 밀리초의 대기 시간 는 각각 99 번째 백분위 수에서 (인덱싱된) 쓰기에 대 한 및를 읽습니다. 
...
전역적으로 분산된 데이터베이스 시스템인 Cosmos DB는 고가용성 외에 대기 시간, 처리량 및 일관성에 대한 SLA를 제공하는 유일한 Azure 서비스입니다. 프로 비전 하는 처리량은 Cosmos 데이터베이스 계정과 연결 된 각 지역에 적용 됩니다. 읽기의 경우 Cosmos DB는 선택 가능한 여러 개의 잘 정의된 [일관성 수준](consistency-levels.md)을 제공합니다. 
