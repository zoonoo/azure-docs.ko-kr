---
title: 알림과 Azure Managed Applications
...
Azure 관리 되는 응용 프로그램 알림을 통해 게시자는 관리 되는 응용 프로그램 인스턴스의 수명 주기 이벤트에 따라 작업을 자동화할 수 있습니다. 게시자는 사용자 지정 알림 웹 후크 끝점을 지정 하 여 신규 및 기존 관리 되는 응용 프로그램 인스턴스에 대 한 이벤트 알림을 받을 수 있습니다. 이를 통해 게시자는 응용 프로그램 프로 비전, 업데이트 및 삭제 시 사용자 지정 워크플로를 설정할 수 있습니다.

## <a name="getting-started"></a>시작
관리 되는 응용 프로그램 수신을 시작 하려면 공용 HTTPS 끝점을 회전 하 고 서비스 카탈로그 응용 프로그램 정의 또는 Marketplace 제품을 게시할 때 지정 합니다.

다음은 빠르게 시작 하 고 실행 하기 위해 권장 되는 일련의 단계입니다.
1. 들어오는 POST 요청을 기록 하 고 `200 OK`을 반환 하는 공용 HTTPS 끝점을 실행 합니다.
2. 아래 설명 된 대로 서비스 카탈로그 응용 프로그램 정의 또는 marketplace 제품에 끝점을 추가 합니다.
3. 응용 프로그램 정의 또는 marketplace 제품을 참조 하는 관리 되는 응용 프로그램 인스턴스를 만듭니다.
4. 알림이 성공적으로 수신 되는지 확인 합니다.
5. 아래의 **끝점 인증** 섹션에 설명 된 대로 권한 부여를 사용 하도록 설정 합니다.
...
> [!NOTE]
> 현재 응용 프로그램 정의 속성에서 하나의 끝점만 **Notificationendpoints** 의 일부로 지원 됩니다.
...
```
## <a name="adding-marketplace-managed-application-notifications"></a>Marketplace 관리 응용 프로그램 알림 추가
자세한 내용은 [Azure 응용 프로그램 제품 만들기](../marketplace/cloud-partner-portal/azure-applications/cpp-create-offer.md)를 참조 하세요.

![포털의 서비스 카탈로그 응용 프로그램 정의 알림](./media/publish-notifications/marketplace-notifications.png)
## <a name="event-triggers"></a>이벤트 트리거
다음 표에서는 EventType + ProvisioningState 및 해당 트리거의 가능한 모든 조합을 설명 합니다.
...
#### <a name="marketplace-application-notification-schema"></a>Marketplace 응용 프로그램 알림 스키마
...
관리 되는 응용 프로그램 알림 서비스에는 웹 후크 끝점에서 알림에 대 한 `200 OK` 응답이 필요 합니다. 웹 후크 끝점이 HTTP 오류 코드 > = 500, 429을 반환 하거나 끝점에 일시적으로 연결할 수 없는 경우 알림 서비스가 다시 시도 됩니다. 웹 후크 끝점을 10 시간 이내에 사용할 수 없게 되 면 알림 메시지가 삭제 되 고 다시 시도가 중지 됩니다.
