...
이 빠른 시작에서는 간소화 된 Azure DevOps 시작 환경을 사용 하 여 Azure Pipelines에서 Java 앱에 대 한 CI (지속적인 통합) 및 CD (지속적인 업데이트) 파이프라인을 설정 합니다. Azure DevOps 스타터를 사용 하 여 앱을 개발, 배포 및 모니터링 하는 데 필요한 모든 항목을 설정할 수 있습니다. 
...
## <a name="examine-the-cicd-pipeline"></a>CD 파이프라인 검토
 ...
 이전 단계에서 DevOps 스타터는 전체 CI/CD 파이프라인을 자동으로 구성 합니다. 필요에 따라 파이프라인을 탐색하고 사용자 지정합니다. 다음 단계를 수행하여 빌드 및 릴리스 파이프라인을 숙지하세요.
