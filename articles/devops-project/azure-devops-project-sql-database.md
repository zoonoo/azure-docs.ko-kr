---
title: Azure DevOps 프로젝트를 사용하여 ASP.NET 앱 및 Azure SQL Database 배포 | VSTS 자습서
...
Azure DevOps 프로젝트는 기존 코드와 Git 리포지토리를 가져오거나 샘플 응용 프로그램 중 하나를 선택하여 Azure에 CI(연속 통합) 및 CD(지속적인 업데이트) 파이프라인을 만드는 간소화된 환경을 제공합니다.  DevOps 프로젝트는 자동으로 Azure SQL Database 같은 Azure 리소스를 만들고, CI의 빌드 정의를 포함한 VSTS의 릴리스 파이프라인을 만들고 구성하고, CD에 대한 릴리스 정의를 설정한 다음, 모니터링을 위해 Azure Application Insights 리소스를 만듭니다.
...
Azure DevOps 프로젝트는 VSTS에서 CI/CD 파이프라인을 만듭니다.  **새 VSTS** 계정을 만들거나 **기존 계정**을 사용할 수 있습니다.  또한 Azure DevOps 프로젝트는 선택한 **Azure 구독**에서 Azure SQL Database 같은 **Azure 리소스**를 만듭니다.
...
Azure DevOps 프로젝트는 VSTS 계정에서 Azure 구독에 배포하는 데 필요한 단계를 자동으로 만들고 구성합니다.  이러한 단계에는 Azure 구독에 VSTS를 인증하기 위한 Azure 서비스 연결 구성이 포함됩니다.  또한 자동화는 VSTS 릴리스 정의를 만들고 릴리스는 Azure에 CD를 제공합니다.  VSTS 릴리스 정의에 대해 자세히 검사하려면 다음 단계를 수행합니다.
...
1. Azure DevOps 프로젝트는 임의의 SQL 암호를 설정하고 이 암호를 릴리스 정의를 위해 사용했습니다.  브라우저의 왼쪽에서 **변수**를 선택합니다. 
...
1. 작업은 배포 프로세스가 수행되는 활동이며 **단계**에서 그룹화됩니다.  이 릴리스 정의에 대한 단일 단계가 있습니다.  이 단계에는 **Azure App Service 배포** 및 **Azure SQL Database 배포** 작업이 포함됩니다.
...
이제 웹 사이트에 최신 작업을 자동으로 배포하는 CI/CD 프로세스를 사용하여 앱에서 팀과 협업할 준비가 되었습니다.  VSTS git 리포지토리에 대한 각 변경은 VSTS에서 빌드를 시작하고, VSTS Release Management 정의는 Azure에 배포를 실행합니다.  리포지토리에 변경 내용을 커밋하려면 아래 단계를 수행하거나 다른 기술을 사용합니다.  코드 변경은 CI/CD 프로세스를 시작하고 Azure에 새 변경 내용을 자동으로 배포합니다.
...
1. 필요에 따라 이제 **SQL Server Management Studio** 또는 **Visual Studio**와 같은 클라이언트 도구를 사용하여 Azure SQL Server 및 Database에 연결할 수 있습니다.  연결할 **서버 이름** 속성을 사용합니다.
 ...
 > [!NOTE]
 > SQL 로그인 암호를 변경한 경우 **VSTS CD Release Management 정의 검사** 섹션에 설명된 대로 VSTS 릴리스 정의 변수에서 암호를 변경해야 합니다.
...
테스트하는 경우 청구 요금 발생을 방지하기 위해 리소스를 정리할 수 있습니다.  더 이상 필요 없는 경우 Azure DevOps 프로젝트 대시보드에서 **삭제** 기능을 사용하여 이 자습서에서 만든 Azure SQL Database 및 관련 리소스를 삭제할 수 있습니다.  **주의** 삭제 기능은 Azure 및 VSTS에서 Azure DevOps 프로젝트에 의해 만들어진 데이터를 제거하므로 사라지면 데이터를 검색할 수 없습니다.
