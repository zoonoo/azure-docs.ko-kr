...
description: Azure DevOps Projects를 사용하면 Azure를 쉽게 시작할 수 있습니다. DevOps Projects를 사용하면 몇 개의 빠른 단계를 통해 Windows 웹앱에 Azure Cosmos DB에서 제공되는 Node.js 앱을 배포할 수 있습니다.
ms.author: mlearned
ms.manager: douge
ms.prod: devops
...
* Azure Cosmos DB, Application Insights, App Service 등과 같은 Azure 리소스를 자동으로 만들기
...
DevOps Projects는 Azure Pipelines에 CI/CD 파이프라인을 만듭니다. 새 Azure DevOps 조직을 만들거나 기존 조직을 사용할 수 있습니다. 또한 DevOps Projects는 사용자가 선택한 Azure 구독에서 Azure Cosmos DB, Application Insights, App Service 및 App Service 계획과 같은 Azure 리소스를 만들 수 있습니다.

1. [Azure 포털](https://portal.azure.com)

1. 왼쪽 창에서 **리소스 만들기** 섹션을 선택합니다.
...
1. **Cosmos DB**에 대해 **데이터베이스 추가** 섹션을 사용하도록 설정하고 **다음**을 클릭합니다.

    ![데이터베이스 추가](_img/azure-devops-project-cosmos-db/add-database.png)

    Cosmos DB는 **Express.js**, **Sample Node.js 앱** 및 **Sail.js**와 같은 다양한 애플리케이션 프레임워크를 지원합니다. 이 자습서에서는 **Express.js**를 사용하겠습니다.

1. 애플리케이션을 배포할 Azure 서비스를 선택합니다. Windows 웹앱, Kubernetes Service 및 Web App for Containers와 같은 다른 서비스가 있습니다. 이 자습서에서는 **Windows 웹앱**을 사용합니다. **다음**을 클릭합니다.
...
1. 몇 분 후 프로세스가 완료됩니다. 샘플 Node.js 앱이 Azure DevOps 조직의 Git 리포지토리에서 설정되고, Azure Cosmos DB, App Service, App Service 계획 및 Application Insights가 만들어지고, CI/CD 파이프라인이 실행되고, 앱이 Azure에 배포됩니다.
   ...
   이 대시보드에서는 Azure DevOps 코드 리포지토리, CI/CD 파이프라인 및 Azure Cosmos DB에 가시성을 제공합니다. Azure DevOps 파이프라인에서 추가 CI/CD 옵션을 구성할 수 있습니다. 오른쪽에서 확인할 **Azure Cosmos DB**를 선택합니다.

## <a name="examine-the-azure-cosmos-db"></a>Azure Cosmos DB 검토

DevOps Projects는 Cosmos DB를 자동으로 구성합니다. 그러면 사용자는 이 Cosmos DB를 살펴보고 사용자 지정할 수 있습니다. Cosmos DB에 익숙해지려면 다음을 수행합니다.

1. DevOps Projects 대시보드로 이동합니다.

    ![DevOps Projects 대시보드](_img/azure-devops-project-cosmos-db/devops-project-dashboard.png)

1. 오른쪽에서 Cosmos DB를 선택합니다. Cosmos DB에 대한 창이 열립니다. 이 보기에서 작업 모니터링, 로그 검색 등의 다양한 작업을 수행할 수 있습니다.

    ![함수 앱](_img/azure-devops-project-cosmos-db/cosmos-db.png)
...
1. **빌드** 아래의 하이퍼링크를 클릭합니다. 브라우저 탭에 새 프로젝트에 대한 빌드 파이프라인이 표시됩니다.
    ...
    ![빌드](_img/azure-devops-project-cosmos-db/build.png)
...
이제 Azure App Service에 최신 작업을 자동으로 배포하는 CI/CD 프로세스를 사용하여 앱에서 팀과 협업할 준비가 되었습니다. Git 리포지토리에 대한 각 변경은 Azure DevOps에서 빌드를 시작하고, CD 파이프라인은 Azure에 배포를 실행합니다. 이 섹션의 절차를 수행하거나 다른 기술을 사용하여 리포지토리에 변경 내용을 커밋합니다. 예를 들어 즐겨찾는 도구 또는 IDE에서 Git 리포지토리를 복제한 다음, 이 리포지토리에 변경 내용을 푸시할 수 있습니다.

1. Azure DevOps 메뉴에서 **리포지토리 | 파일**을 선택한 후 리포지토리로 이동합니다.

1. 리포지토리에는 사용자가 생성 프로세스에서 선택한 애플리케이션 언어를 기반으로 하는 코드가 이미 포함되어 있습니다. **Application/views/index.pug** 파일을 엽니다.

1. **편집**을 선택한 다음, **줄 번호 15**를 변경합니다. 예를 들어 **Azure Cosmos DB에서 제공되는 Azure App Service에 첫 번째 배포**로 업데이트할 수 있습니다.
