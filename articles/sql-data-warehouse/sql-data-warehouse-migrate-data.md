---
title: SQL 데이터 웨어하우스로 데이터 마이그레이션| Microsoft Docs
...
데이터가 플랫 파일에서 생성되는 경우 SQL Data Warehouse로 데이터 로드를 시작하기 전에 먼저 Azure Storage Blob에 데이터를 전송해야 합니다. 데이터가 Azure blob 저장소로 전송되면 [ADF 복사본][ADF 복사본]을 다시 사용하도록 선택하여 SQL 데이터 웨어하우스로 데이터를 푸시합니다.
...
## 다음 단계
마이그레이션에 대한 자세한 내용은 [SQL 데이터 웨어하우스로 솔루션 마이그레이션][SQL 데이터 웨어하우스로 솔루션 마이그레이션]을 참조하세요. 더 많은 개발 팁은 [개발 개요][개발 개요]를 참조하세요.
