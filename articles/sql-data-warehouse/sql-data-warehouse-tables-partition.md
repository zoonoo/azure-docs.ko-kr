...
분할은 클러스터형 columnstore, 클러스터형 인덱스 및 힙을 포함하는 모든 SQL Data Warehouse 테이블 형식에서 지원됩니다.  또한 해시 또는 라운드 로빈 배포를 비롯한 모든 배포 유형에서도 분할이 지원됩니다.  분할을 하면 데이터가 좀 더 작은 데이터 그룹으로 나뉘며 대부분의 경우 날짜 열에서 분할이 수행됩니다.
