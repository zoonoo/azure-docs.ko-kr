...
| 최대 동시성 슬롯 수 | 4 | 8 | 12 | 16 | 20 | 24 | 32 | 32 | 32 | 32 | 
...
사용자에게 전체 권한을 부여해야 합니다. 아래 예제는 SQL 데이터 웨어하우스 데이터베이스에 대한 `CONTROL`을(를) 부여합니다. 데이터베이스 수준에서 `CONTROL`은(는) SQL Server에서 db\_owner에 해당합니다.
