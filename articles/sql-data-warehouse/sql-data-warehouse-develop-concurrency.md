...
동시성 한도는 **동시 쿼리 수** 및 **동시성 슬롯 수**라는 두 개념에 의해 제어됩니다. 쿼리가 실행하려면 쿼리 동시성 한도 및 동시성 슬롯 할당 하에서 실행해야 합니다.
...
- **동시성 슬롯**은 더 동적인 개념입니다. 각 쿼리는 동시성 슬롯을 한 개 이상 사용할 수 있습니다. 쿼리가 사용하는 정확한 슬롯 수는 SQL 데이터 웨어하우스의 크기와 쿼리의 [리소스 클래스](#resource-classes)에 따라 달라집니다.
...
> [AZURE.NOTE]  DMV(동적 관리 뷰)에서 단독으로 실행하는 SELECT 쿼리 또는 카탈로그 뷰는 어떠한 동시성 한도에 의해서도 제어되지 **않**습니다. 이렇게 하면 사용자는 시스템에서 실행하는 쿼리 수에 관계 없이 시스템을 모니터링할 수 있습니다.

## 리소스 클래스

리소스 클래스는 더 많은 메모리 또는 CPU 주기가 지정된 사용자에 의해 실행되는 쿼리에 할당되도록 할 수 있기 때문에 SQL 데이터 웨어하우스 워크로드 관리의 핵심적인 부분입니다. **데이터베이스 역할**의 형태로 사용자에게 할당될 수 있는 네 가지 리소스 클래스가 있습니다. 네 가지 리소스 클래스에는 **smallrc, mediumrc, largerc 및 xlargerc**가 있습니다. Smallrc의 사용자에게 더 작은 메모리 양이 주어져 더 높은 동시성을 허용합니다. 반면, xlargerc에 할당된 사용자는 많은 양의 메모리가 주어져 더 적은 쿼리가 동시에 실행될 수 있습니다.
...
더 큰 메모리 할당에서 유용하지 않아 시스템이 리소스 클래스 할당을 무시하고 대신 작은 리소스 클래스에서 언제나 이러한 쿼리를 실행하는 몇가지 쿼리 유형이 있습니다. 이러한 쿼리가 항상 작은 리소스 클래스에서 실행되도록 강제한다면 동시성 슬롯이 부족할 때에도 실행할 수 있으며 필요 이상으로 많은 슬롯을 사용하는 것을 방지할 수 있습니다. 이러한 [리소스 클래스 예외](#resource-class-exceptions)는 이 문서의 뒷부분에 나와 있습니다.
...
사용자의 리소스 클래스를 늘리는 데 따른 장단점이 있습니다. 사용자에 대한 리소스 클래스를 늘리면 쿼리가 더 많은 메모리에 액세스하여 더 빠른, 더 높은 리소스 클래스를 실행할 수도 있는 한편 실행할 수 있는 동시 쿼리 수는 줄입니다. 이는 단일 쿼리에 많은 양의 메모리를 할당하는 것과 또한 메모리 할당을 필요로 하는 다른 쿼리에게 동시에 실행할 수 있도록 하는 것 사이의 절충입니다. 한 명의 사용자가 쿼리에 대한 메모리 할당을 높게 받는 경우 다른 사용자들은 쿼리를 실행할 목적으로 동일한 메모리에 액세스할 수 없습니다.

다음 표는 DWU 및 리소스 클래스에 의해 각 배포에 할당된 메모리를 매핑합니다. SQL 데이터 웨어하우스에는 데이터베이스당 배포가 60개 있습니다. 예를 들어, xlarge 리소스 클래스의 DW2000에서 실행하는 쿼리는 배포된 60개의 각 데이터베이스 내에서 6,400 MB에 액세스하게 될 것입니다.
...
위와 동일한 예제를 사용하여 xlarge 리소스 클래스의 DW2000에서 실행하는 시스템 전반 쿼리에 총 375 GB(6,400 MB * 60 배포 / 1,024 GB로 변환) 메모리가 할당됩니다.
...
위에서 설명했듯이 리소스 클래스가 높을수록 메모리가 더 많이 부여됩니다. 메모리는 고정된 리소스이므로 쿼리 당 메모리를 더 많이 할당할수록 지원할 수 있는 동시성은 줄어듭니다. 다음 표는 위의 모든 개념을 DWU가 사용할 수 있는 동시성 슬롯 수는 물론 각 리소스 클래스가 사용하는 슬롯 수를 한 눈에 볼 수 있게 나타냅니다.
...
이 테이블에서 DW1000으로 실행되는 SQL 데이터 웨어하우스는 동시 쿼리 최대 32개까지의 동시성 슬롯을 총 40개까지 제공하는 것을 볼 수 있습니다. 모든 사용자가 작은 리소스 클래스로 실행하는 경우 각 쿼리가 동시성 슬롯 1개를 소비한다면 32개의 동시성 쿼리가 혀용됩니다. 모든 사용자가 중간 리소스 클래스에서 실행하는 경우, 각 사용자에게 총 메모리 할당량 47 GB에 대해 배포 당 800MB가 할당되고 이러한 모든 중간 리소스 클래스 사용자에 대한 동시성은 8명의 사용자로 제한됩니다.
