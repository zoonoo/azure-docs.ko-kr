...
사용자는 예측 가능한 성능을 대규모로 제공하기 위해 Microsoft Azure SQL 데이터 웨어하우스를 사용하여 메모리 및 CPU 우선 순위 지정과 같은 리소스 할당뿐 아니라 동시성 수준을 제어할 수 있습니다. 이 문서에서는 동시성 및 워크로드 관리 개념을 소개하며, 두 기능 모두 구현된 방법 및 데이터 웨어하우스에서 이들을 제어할 수 있는 방법을 설명합니다. SQL 데이터 웨어하우스 워크로드 관리는 다중 사용자 환경을 지원합니다. 다중 테넌트 워크로드용은 아닙니다.
...
- 동시 쿼리 수란 동시에 실행하는 쿼리 수입니다. SQL 데이터 웨어하우스는 동시 쿼리를 32개까지 지원합니다.
- 동시성 슬롯은 DWU를 기준으로 할당됩니다. 100개의 DWU마다 4개의 동시성 슬롯을 제공합니다. 예를 들어 DW100은 4개의 동시성 슬롯을 할당하고, DW1000은 40개를 할당합니다. 각 쿼리는 쿼리의 [리소스 클래스](#resource-classes)에 따라 하나 이상의 동시성 슬롯을 사용합니다. smallrc 리소스 클래스에서 실행되는 쿼리는 하나의 동시성 슬롯을 사용합니다. 더 높은 리소스 클래스에서 실행되는 쿼리는 더 많은 동시성 슬롯을 사용합니다.
...
이러한 임계값 중 하나가 충족되면, 새로운 쿼리는 큐에 대기합니다. SQL 데이터 웨어하우스는 다른 쿼리가 완료되고 쿼리 수와 슬롯이 한도 밑으로 떨어질 때 대기 중인 쿼리를 선입 선출 방식으로 실행합니다.

> [AZURE.NOTE]  DMV(동적 관리 뷰)에서 단독으로 실행하는 *Select* 쿼리 또는 카탈로그 뷰는 어떠한 동시성 한도에 의해서도 제어되지 않습니다. 사용자는 시스템에서 실행하는 쿼리 수에 관계없이 시스템을 모니터링할 수 있습니다.
...
사용자의 리소스 클래스를 변경하기보다는 사용자를 리소스 클래스에 영구적으로 할당하는 것이 좋습니다. 예를 들어 클러스터형 Columnstore 테이블에 로드는 더 많은 메모리를 할당하면 더 높은 품질의 인덱스를 만듭니다. 로드가 더 높은 메모리에 액세스할 수 있도록 하려면 특별히 데이타 로드에 대한 사용자를 만들고 이 사용자를 더 높은 리소스 클래스에 영구적으로 할당합니다.
...
사용자의 리소스 클래스를 늘리는 데 따른 장단점이 있습니다. 사용자에 대한 리소스 클래스를 늘리면 쿼리가 더 많은 메모리에 액세스하여 더 빠른, 더 높은 리소스 클래스를 실행할 수도 있지만 실행할 수 있는 동시 쿼리 수는 줄어듭니다. 즉, 단일 쿼리에 많은 양의 메모리를 할당하는 방식과 동시 실행을 위해 메모리 할당을 해야 하는 여러 다른 쿼리를 실행하는 방식이 절충될 수 있습니다. 한 명의 사용자가 쿼리에 대한 메모리 할당을 높게 받는 경우 다른 사용자들은 쿼리를 실행할 목적으로 동일한 메모리에 액세스할 수 없습니다.

다음 표는 DWU 및 리소스 클래스에 의해 각 배포에 할당된 메모리를 매핑합니다. SQL 데이터 웨어하우스에는 배포가 60개 있습니다. 예를 들어, xlargerc 리소스 클래스의 DW2000에서 실행하는 쿼리는 배포된 60개의 각 데이터베이스 내에서 6,400MB의 메모리에 액세스하게 될 것입니다.
...
앞의 예제에서 xlargerc 리소스 클래스의 DW2000에서 실행하는 쿼리에는 전체 SQL 데이터 웨어하우스에 대해 총 375GB(6,400MB * 60개 배포/1,024, 이 결과를 GB로 변환) 메모리가 할당됩니다.
...
SQL 데이터 웨어하우스는 더 많은 리소스 클래스에서 실행되는 쿼리에 더 많은 메모리를 부여합니다. 메모리는 고정된 리소스이므로 쿼리 당 메모리를 더 많이 할당할수록 지원할 수 있는 동시성은 줄어듭니다. 다음 테이블에는 위의 모든 개념을 DWU가 사용할 수 있는 동시성 슬롯 수는 물론 각 리소스 클래스가 사용하는 슬롯 수를 한눈에 볼 수 있게 나타납니다.
...
- **UserConcurrencyResourceType**: 동시성 슬롯 프레임워크 내에 존재하는 쿼리. 최종 사용자 테이블에 대한 쿼리는 이 리소스 형식을 사용할 수 있는 예를 나타냅니다.
