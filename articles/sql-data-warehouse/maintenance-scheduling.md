...
ms.locfileid: "71958887"
---
# <a name="use-maintenance-schedules-to-manage-service-updates-and-maintenance"></a>유지 관리 일정을 사용하여 서비스 유지 관리 및 업데이트 관리

유지 관리 일정은 모든 Azure SQL Data Warehouse 지역에서 사용할 수 있습니다. 유지 관리 일정 기능은 Service Health 계획 된 유지 관리 알림, Resource Health 확인 모니터 및 Azure SQL Data Warehouse 유지 관리 예약 서비스를 통합 합니다.
...
예를 들어 토요일 22:00의 주 창을 일요일 01:00로 예약한 다음 수요일 19:00의 보조 창을 22:00로 예약할 수 있습니다. SQL Data Warehouse가 기본 유지 관리 기간 중에 유지 관리를 수행할 수 없는 경우 보조 유지 관리 기간 중에 유지 관리를 다시 시도합니다. 서비스 유지 관리는 기본 및 보조 창 모두에서 발생할 수 있습니다. 신속하게 모든 유지 관리 작업이 완료되도록 DW400(c) 및 낮은 데이터 웨어하우스 계층은 지정된 유지 관리 기간 외의 유지 관리를 완료할 수 있습니다.
...
각 유지 관리 기간은 3 시간에서 8 시간 사이로 설정할 수 있습니다. 유지 관리는 기간 내에 언제든 발생할 수 있습니다. 유지 관리가 시작 되 면 모든 활성 세션이 취소 되 고 커밋되지 않은 트랜잭션이 롤백됩니다. 서비스에서 새 코드를 데이터 웨어하우스에 배포할 때 연결에 대 한 간략 한 손실이 여러 번 발생할 수 있습니다. 데이터 웨어하우스 유지 관리가 완료 되 면 즉시 알림이 표시 됩니다.
 ...
 모든 유지 관리 작업은 예약된 유지 관리 기간 내에 완료되어야 합니다. 사전 알림 없이 지정된 유지 관리 기간 외에는 유지 관리가 수행되지 않습니다. 데이터 웨어하우스는 예약된 유지 관리 중에 일시 중지되는 경우 다시 시작 작업 중에 업데이트됩니다. 
...
기본적으로 새로 만드는 모든 Azure SQL Data Warehouse 인스턴스에는 배포 중에 8시간의 기본 및 보조 유지 관리 기간이 적용됩니다. 위에서 설명한 대로 배포가 완료 된 후에 windows를 변경할 수 있습니다. 사전 알림 없이 지정된 유지 관리 기간 외에는 유지 관리가 수행되지 않습니다.
