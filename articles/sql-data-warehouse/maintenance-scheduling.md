...
24시간 사전 알림은 DW400c 및 낮은 계층의 현재 예외를 사용하여 모든 유지 관리 이벤트에 우선합니다. 인스턴스 가동 중지 시간을 최소화하려면 선택한 유지 관리 기간 전에 데이터 웨어하우스에 장기 실행 트랜잭션이 없어야 합니다. 유지 관리가 시작되면 모든 활성 세션은 취소됩니다. 커밋되지 않은 트랜잭션은 롤백됩니다. 그리고 데이터 웨어하우스에 대한 연결이 잠시 손실됩니다. 데이터 웨어하우스에서 유지 관리가 완료된 직후에는 알림이 전송됩니다.

> [!NOTE]
> 우리는 시간이 중요 한 업데이트를 배포 하는 데 필요한 이벤트에서 고급 알림 횟수는 크게 줄어들 수 있습니다.
