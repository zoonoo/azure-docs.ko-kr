...
## 복구를 시작해야 하는 시기
복구 작업은 복구 시 SQL 연결 문자열을 변경해야 하며 데이터가 영구적으로 손상될 수 있습니다. 따라서 중단이 응용 프로그램의 RTO보다 오래 지속될 가능성이 있을 때만 복구를 수행해야 합니다. 다음 데이터 요소를 사용하여 복구가 보장되는지 확인합니다.
...
## 복구 후 데이터베이스 구성
복구된 데이터베이스 프로덕션을 준비하는 데 도움이 되는 검사 목록입니다.
