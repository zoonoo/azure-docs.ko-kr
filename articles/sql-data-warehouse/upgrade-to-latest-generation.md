...
3. 업그레이드 전에 워크로드가 실행되고 정지되었는지 확인합니다. 데이터 웨어하우스가 계산에 최적화된 Gen2 계층 데이터 웨어하우스로 다시 온라인 상태가 되기 전에 몇 분 동안 가동 중지 시간이 발생합니다. **업그레이드를 클릭합니다**. 계산에 최적화된 Gen2 계층 성능 계층은 현재 미리 보기 기간 중 절반 가격입니다.
