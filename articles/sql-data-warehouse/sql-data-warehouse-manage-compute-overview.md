...
* [DWU(데이터 웨어하우스 단위)][data warehouse units (DWUs)]를 조정하여 계산 능력 조정
* 계산 리소스 일시 중지 또는 다시 시작

<a name="scale-performance-bk"></a>

## <a name="scale-performance"></a>성능 조정
SQL 데이터 웨어하우스에서 CPU의 계산 리소스, 메모리, I/O 대역폭을 늘리거나 줄여 성능을 빠르게 확장 또는 축소할 수 있습니다. 성능을 조정할 경우 SQL Data Warehouse에서 데이터베이스에 할당하는 [DWU(데이터 웨어하우스 단위)][data warehouse units (DWUs)] 수를 조정하기만 하면 됩니다. SQL 데이터 웨어하우스가 빠르게 변경하고 하드웨어 또는 소프트웨어의 모든 기본 변경 사항을 처리합니다.
...
계산 성능을 탄력적으로 증가 또는 감소하려면 데이터베이스의[ DWU(데이터 웨어하우스 단위)][data warehouse units (DWUs)]를 변경하기만 하면 됩니다. 더 많은 DWU를 추가하면 성능이 연속해서 증가합니다.  높은 DWU 수준에서 성능이 크게 향상되었음을 감지하려면 DWU를 100개 이상 추가해야 합니다. 의미 있는 DWU 증가분을 선택할 때에는 최고의 결과를 제공하는 DWU 수준을 참조할 수 있습니다.
...
데이터베이스 크기를 조정하려면 [ALTER DATABASE][ALTER DATABASE]에 설명된 권한이 필요합니다.  일시 중지 및 다시 시작을 위해서는 [SQL DB 참여자][SQL DB Contributor] 권한, 특히 Microsoft.Sql/servers/databases/action이 필요합니다.
...
* [워크로드 및 동시성 관리][Workload and concurrency managment]
