...
description: SQL Data Warehouse 데이터베이스 관리 개요. 관리 도구, DWU 및 성능 확장, 쿼리 성능 문제 해결, 보안 정책 설정, 데이터 손상 또는 지역적 중단으로부터 데이터베이스 복원 등이 포함되어 있습니다.
...
이 개요에서는 SQL Data Warehouse 관리에 대한 이러한 측면에 대해 다룹니다.
...
SQL Data Warehouse는 주로 DMV를 사용하여 관리합니다. 포털에 표시되는 거의 모든 정보가 DMV에 의존합니다. SQL Data Warehouse DMV 목록을 보려면 [SQL Data Warehouse 시스템 뷰][SQL Data Warehouse system views]를 참조하세요.
...
SQL Data Warehouse에서 CPU의 계산 리소스, 메모리, I/O 대역폭을 늘리거나 줄여 성능을 빠르게 확장 또는 축소할 수 있습니다. 성능을 조정할 경우 SQL Data Warehouse에서 데이터베이스에 할당하는 DWU(데이터 웨어하우스 단위) 수를 조정하기만 하면 됩니다. SQL Data Warehouse가 빠르게 변경하고 하드웨어 또는 소프트웨어의 모든 기본 변경 사항을 처리합니다.
...
쿼리가 너무 오래 실행될 때 무엇이 문제인지 확실히 파악할 수 없는 경우가 있습니다. SQL Data Warehouse에는 어느 쿼리가 너무 오래 실행되는지 확인하는 데 사용하는 동적 관리 뷰가 있습니다.
...
좋은 데이터베이스 디자인 원칙을 사용할 경우 SQL Data Warehouse를 더 손쉽게 관리할 수 있습니다. 자세한 내용은 [개발 개요][Development overview]를 참조하세요.
