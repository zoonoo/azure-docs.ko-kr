...
Azure Portal을 시작하려면 [SQL Data Warehouse 만들기(Azure Portal)][SQL Data Warehouse 만들기(Azure Portal)]를 참조하세요.

### <a name="sql-server-data-tools-in-visual-studio"></a>Visual Studio에서 SQL Server 데이터 도구
SSDT([SSDT(SSDT(SQL Server Data Tools))][SSDT(SSDT(SQL Server Data Tools))])를 사용하여 데이터베이스를 연결, 관리, 개발할 수 있습니다. Visual Studio 또는 다른 IDE(통합 개발 환경)에 익숙한 응용 프로그램 개발자라면 Visual Studio에서 SSDT를 사용해 보세요.
...
SQL 데이터 웨어하우스는 주로 DMV를 사용하여 관리합니다. 포털에 표시되는 거의 모든 정보가 DMV에 의존합니다. SQL Data Warehouse DMV 목록을 보려면 [SQL 데이터 웨어하우스 시스템 뷰][SQL 데이터 웨어하우스 시스템 뷰]를 참조하세요.
...
오래 실행되는 쿼리를 찾으려면 [DMV를 사용하여 작업 모니터링][DMV를 사용하여 작업 모니터링]을 참조하세요.
...
<!--Article references-->
[SQL Data Warehouse 만들기(Azure Portal)]: sql-data-warehouse-get-started-provision.md
[데이터베이스 만들기(PowerShell)]: sql-data-warehouse-get-started-provision-powershell.md
...
[DMV를 사용하여 작업 모니터링]: sql-data-warehouse-manage-monitor.md
...
[계산 다시 시작]: sql-data-warehouse-manage-compute-overview.md#resume-compute-bk
...
<!--MSDN references-->
[SSDT(SSDT(SQL Server Data Tools))]: https://msdn.microsoft.com/library/mt204009.aspx
