<properties
   pageTitle="SQL 데이터 웨어하우스로 Contoso Retail 데이터 웨어하우스 로드 | Microsoft Azure"
   description="최상의 성능을 위해 PolyBase를 사용하여 SQL 데이터 웨어하우스로 Contoso 소매 데이터 웨어하우스를 로드하는 방법 알아보기"
...
# Contoso Retail 데이터 웨어하우스 로드

이 문서에서는 데이터를 Contoso Retail 데이터 웨어하우스 스키마로 신속하게 로드하는 방법을 배웁니다.

이 데이터 집합은 Azure SQL 데이터 웨어하우스에 관한 모든 것을 배우기 위해 사용할 수 있는 대체 데이터 웨어하우스를 제공합니다.

Contoso 데이터 집합은 약 GB 크기입니다. 원시 데이터는 SQL 데이터 웨어하우스에 로드하기 위해 직접 액세스하는 Azure blob 저장소에 보관됩니다.

이 자습서에서는 다음 작업을 수행합니다.

1. 공용 blob 저장소 컨테이너에서 로드하도록 PolyBase 구성
2. [CTAS][]를 사용하여 데이터 가져오기
3. Columnstore 인덱스가 최적화되도록 테이블 [다시 빌드][]
4. 키 열에 대한 통계 만들기

## PolyBase 구성

데이터를 로드할 때 첫 번째 단계는 PolyBase 필수 스크립트를 구성하는 것입니다.

### 외부 데이터 원본 만들기

데이터가 공용 컨테이너에 저장될 때 첫 번째 태스크는 외부 데이터 원본을 만드는 것입니다.
...
Blob 저장소 컨테이너는 "공용"으로 구성되어 누구나 이 컨테이너를 읽고 열거하기 때문에 이 경우 마스터 키 또는 데이터베이스 범위 자격 증명을 만들 필요가 없습니다.

> [AZURE.IMPORTANT] Azure blob 저장소 컨테이너를 공용으로 만들도록 선택하는 경우 귀하가 책임지고 있는 데이터 센터에서 데이터가 떠날 때 데이터 송신 요금이 있다는 점을 염두에 두십시오.

### 외부 파일 형식 개체 만들기

파일 형식은 blob 저장소에 보관된 텍스트 파일을 구문 분석하는 데 필요한 정보를 제공합니다.
...
우리의 경우 데이터는 압축되어 있지 않으며 파이프로 구분됩니다.

## 외부 테이블 만들기

이제 Azure blob 저장소에 대한 연결이 설정되었으므로 외부 테이블을 만들 수 있습니다.

정돈된 작업을 위해 먼저 스키마를 만들겠습니다.
...
이제 외부 테이블을 만들겠습니다. 여기서 수행하는 모든 작업은 데이터 형식의 매핑을 제공하며 파일을 찾기 위해 이 정의를 메타데이터로 가져온다는 것을 염두에 두십시오.
...
데이터를 로드하려면 방금 만든 강력한 형식의 외부 테이블을 이용하는 것이 좋습니다. 그러므로 간단한 [CTAS][] 문이면 충분합니다.

그러나 먼저 테이블을 로드할 새 스키마를 만들겠습니다.
...
데이터를 로드하기 위해 [CTAS][] 문을 사용합니다. 참고로 테이블당 [CTAS][] 한 개가 있습니다.
...
`[sys].[dm_pdw_exec_requests]` 동적 관리 뷰(DMV)를 사용하여 로드 진행률을 추적할 수 있습니다. 위의 CTAS 문은 `label`를 가지고 있으므로 `label`를 사용하여 진행률을 추적할 수 있습니다. 자세한 내용은 [레이블][] 문서를 참조하세요.

## 인덱스 다시 빌드

마지막으로, 테이블이 압축 된 columnstore 형식이 되도록 테이블에 대해 다시 빌드 작업을 실행할 수 있습니다. 이 작업은 열린 또는 닫힌 행 그룹을 압축된 columnstore 형식으로 변환합니다.
...
클러스터된 columnstore의 상태를 확인하는 방법에 관한 자세한 내용은 [columnstore 인덱스 관리][] 문서를 참조하세요.

## 일부 통계 개체 만들기

모든 열에 대한 통계를 만들기 위해 [통계][] 문서에서 설명한 저장 프로시저 코드 `prc_sqldw_create_stats`를 이용할 수 있습니다.

다음은 Contoso Retail 데이터 웨어하우스의 전체 및 모든 조인 열의 차원을 다루는 출력의 하위 집합입니다. 이것이 좋은 출발점입니다.

나중에 직접 다른 팩트 열에 더 많은 통계를 추가할 수 있습니다.
...
이제 모든 데이터를 Azure SQL 데이터 웨어하우스에 로드했습니다. 잘 하셨습니다!

이제 아래와 같은 쿼리를 사용하여 테이블 쿼리를 시작할 수 있습니다.
...
## 다음 단계
로드 개요는 [SQL 데이터 웨어하우스로 데이터 로드][]를 참조하세요. Columnstore 인덱스를 확인하는 방법을 자세히 배우려면 [columnstore 인덱스 관리][]를 참조하세요.
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/ko-KR/library/mt203887.aspx
[다시 빌드]: https://msdn.microsoft.com/ko-KR/library/ms188388.aspx
