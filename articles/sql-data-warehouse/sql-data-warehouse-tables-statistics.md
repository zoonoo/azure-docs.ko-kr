<properties
   pageTitle="SQL 데이터 웨어하우스의 테이블에 대한 통계 관리 | Microsoft Azure"
...
> [AZURE.NOTE] 테이블의 모든 통계를 업데이트하면 SQL 데이터 웨어하우스는 각 통계에 대한 테이블을 스캔하여 샘플링합니다. 테이블이 크고 열이 많고 통계가 많은 경우, 필요에 따라 개별 통계를 업데이트하는 것이 더욱 효율적일 수 있습니다.
...
자세한 내용은 MSDN에서 [DBCC SHOW\_STATISTICS][]를 참조하세요. 자세히 알아보려면 [테이블 개요][Overview], [테이블 데이터 형식][Data Types], [테이블 배포][Distribute], [테이블 인덱싱][Index], [테이블 분할][Partition] 및 [임시 테이블][Temporary]에 대한 문서를 참조하세요. 모범 사례에 대한 자세한 내용은 [SQL 데이터 웨어하우스 모범 사례][]를 참조하세요.
