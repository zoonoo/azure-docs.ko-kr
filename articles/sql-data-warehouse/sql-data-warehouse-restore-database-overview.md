---
title: Azure SQL 데이터 웨어하우스 복원(개요) | Microsoft Docs
...
## 데이터 중복
백업 외에도 SQL 데이터 웨어하우스는 [로컬 중복(LRS)][로컬 중복(LRS)] Azure 프리미엄 저장소를 사용하여 데이터를 보호합니다. 데이터의 여러 동기 복사본은 지역화된 오류 발생 시 투명 한 데이터 보호를 보장하기 위해 로컬 데이터 센터에 유지됩니다. 데이터 중복을 사용하면 데이터는 디스크 오류 등의 인프라 문제를 감당할 수 있습니다. 데이터 중복은 내결함성이 있는 비즈니스 연속성 및 고가용성 인프라를 보장합니다.
