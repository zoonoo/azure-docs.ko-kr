...
| 메트릭 이름             | 설명                                                  | 집계 형식 |
| ----------------------- | ------------------------------------------------------------ | ---------------- |
| CPU 비율          | 데이터 웨어하우스에 대한 모든 노드에서의 CPU 사용률      | 최대값          |
| 데이터 IO 백분율      | 데이터 웨어하우스에 대한 모든 노드에서의 IO 사용률       | 최대값          |
| 메모리 백분율       | 데이터 웨어하우스의 모든 노드에 걸친 메모리 사용률 (SQL Server) | 최대값          |
| 성공적인 연결  | 데이터에 대한 연결 성공 횟수                 | Total            |
| 연결 실패      | 데이터 웨어하우스에 대한 연결 실패 횟수           | Total            |
| 방화벽에 의해 차단     | 차단된 데이터 웨어하우스에 대한 로그인 횟수     | Total            |
| DWU 제한               | 데이터 웨어하우스의 서비스 수준 목표                | 최대값          |
| DWU 백분율          | CPU 비율과 데이터 IO 비율 사이의 최댓값        | 최대값          |
| DWU 사용됨                | DWU 한도 * DWU 비율                                   | 최대값          |
| 캐시 적중 비율    | (캐시 적중/캐시 누락) * 100, 여기서 캐시 적중은 로컬 SSD 캐시에서 적중된 모든 columnstore 세그먼트에 대한 합계이며, 캐시 누락은 모든 노드에 걸쳐 로컬 SSD 캐시에서 누락된 columnstore 세그먼트에 대한 합계입니다. | 최대값          |
| 캐시 사용 비율   | (사용된 캐시/캐시 용량) * 100, 여기서 사용된 캐시는 모든 노드에 걸친 로컬 SSD 캐시의 모든 바이트에 대한 합계이며, 캐시 용량은 모든 노드에 걸친 로컬 SSD 캐시의 스토리지 용량에 대한 합계입니다 | 최대값          |
| 로컬 tempdb 백분율 | 모든 컴퓨팅 노드의 로컬 tempdb 사용률 - 5분마다 값을 내보냅니다. | 최대값          |
