...
| 데이터 IO 비율      | 데이터 웨어하우스에 대한 모든 노드에서의 IO 사용률       | 최대값          |
| 메모리 비율       | 데이터 웨어하우스의 모든 노드에 걸친 메모리 사용률 (SQL Server) | 최대값          |
| 연결 성공  | 데이터에 대한 연결 성공 횟수                 | Total            |
| 연결 실패      | 데이터 웨어하우스에 대한 연결 실패 횟수           | Total            |
| 방화벽에서 차단됨     | 차단된 데이터 웨어하우스에 대한 로그인 횟수     | Total            |
