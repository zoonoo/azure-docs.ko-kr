...
SQL 데이터 웨어하우스는 방대한 병렬 처리(MPP) 분산 데이터베이스 시스템입니다. 따라서 **배포판**이라고 하는 여러 다른 위치에 데이터를 저장합니다. 각 **배포**는 버킷과 같은 것으로 데이터 웨어하우스에서 데이터의 고유 하위 집합을 저장합니다. 여러 노드에 걸쳐 데이터와 처리 기능을 구분하여 SQL 데이터 웨어하우스는 모든 단일 시스템을 넘어서는 매우 큰 확장성을 제공할 수 있습니다.
...
쿼리는 지원하지 않는 모든 사용자 정의 데이터 형식을 포함합니다. 다음은 지원하지 않는 데이터 형식 대신 사용할 수 있는 일부 대안입니다.
...
- **xml**, 더 나은 성능을 위해 varchar(max) 이하 사용
...
> [AZURE.NOTE] 기억하세요! 해시가 데이터의 값만을 따르는 것은 아닙니다. 해시는 값과 데이터 형식의 조합입니다.
...
정의에 파티션 함수 또는 스키마가 없습니다. SQL 데이터 웨어하우스는 SQL Server와는 약간 다른 단순화된 파티션의 정의를 사용합니다. 분할된 열에 대한 경계 지점을 식별하기만 하면 됩니다.
...
## 다음 단계
더 많은 개발 팁은 [개발 개요][]를 참조하세요. 모범 사례에 대한 추가 팁은 [SQL 데이터 웨어하우스 모범 사례][]를 참조하세요.

<!--Image references-->

<!--Article references-->
[개발 개요]: sql-data-warehouse-overview-develop.md
[서로게이트 키 배정]: https://blogs.msdn.microsoft.com/sqlcat/2016/02/18/assigning-surrogate-key-to-dimension-tables-in-sql-dw-and-aps/
