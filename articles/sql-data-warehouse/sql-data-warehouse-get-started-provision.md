...
1. **새 서버** 창에서 요청된 정보를 입력합니다.
    ...
    서버 이름, 관리자 이름 및 암호를 어딘가에 저장해야 합니다. 서버에 로그온하려면 이 정보가 필요합니다. - **서버 이름**. 논리 서버에 대한 이름을 입력합니다. - **서버 관리자 이름**. 서버 관리자 계정에 대한 사용자 이름을 입력합니다. - **암호**. 서버 관리자 암호를 입력합니다. - **위치**. 사용자 또는 다른 Azure 리소스 가까이에 있는 지리적 위치를 선택합니다. 이렇게 하면 논리 서버에 속하는 모든 데이터베이스와 리소스가 실제로 동일한 영역에 위치하게 됩니다.
