...
Azure SQL Data Warehouse를 이제 시작하는 사용자라면 이 문서가 어려울 수 있습니다.  항목은 주로 중요한 순서로 배치되어 있습니다.  먼저 처음에 나오는 개념부터 살펴보는 것이 좋습니다.  SQL Data Warehouse를 사용 하 여에 익숙해지면 익숙해지면 돌아와서 더 많은 개념을 살펴봅니다.  금방 모든 항목을 이해할 수 있습니다.
...
참고 항목 [테이블 통계 관리][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS]하십시오 [UPDATE STATISTICS][UPDATE STATISTICS]
...
참고 항목 [테이블 개요][Table overview], [Table distribution][Table distribution]하십시오 [테이블 배포 선택][테이블 배포 선택], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
참고 항목 [트랜잭션 이해][Understanding transactions], [Optimizing transactions][Optimizing transactions], [테이블 분할][테이블 분할]하십시오 [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE], [처럼 select (CTAS) Create table][select (CTAS)으로 새 테이블 만들기]
...
참고 항목 [테이블 개요][Table overview], [Table data types][Table data types]하십시오 [CREATE TABLE][CREATE TABLE]
...
참고 항목 [임시 테이블][Temporary tables], [CREATE TABLE][CREATE TABLE]하십시오 [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="optimize-clustered-columnstore-tables"></a>클러스터형 Columnstore 테이블 최적화
columnstore 클러스터형 인덱스는 Azure SQL Data Warehouse에 데이터를 저장할 수 있는 가장 효율적인 방법 중 하나입니다.  기본적으로 SQL Data Warehouse의 테이블은 클러스터형 ColumnStore로 만들어집니다.  columnstore 테이블에서 쿼리에 대해 최상의 성능을 얻으려면 좋은 세그먼트 품질을 갖는 것이 중요합니다.  메모리 부족 상황에서 행이 columnstore 테이블에 기록되는 경우 columnstore 세그먼트 품질이 저하될 수 있습니다.  세그먼트 품질은 압축된 행 그룹에 있는 행의 수로 측정할 수 있습니다.  참조를 [인덱스 품질 저하 columnstore 원인][Causes of poor columnstore index quality] in the [Table indexes][Table indexes] 문서에 대 한 단계별 지침은 감지 하 고 클러스터형된 columnstore 테이블에 대해 세그먼트 품질을 개선 합니다.  고품질 columnstore 세그먼트가 중요하기 때문에 데이터를 로드하기 위해서는 중간 규모 또는 대규모 리소스 클래스에 속하는 사용자 ID를 사용하는 것이 좋습니다. 낮은 [데이터 웨어하우스 단위](what-is-a-data-warehouse-unit-dwu-cdwu.md)를 사용하면 로드하는 사용자에게 더 큰 리소스 클래스를 할당하려는 것입니다.
...
참고 항목 [테이블 인덱스][Table indexes], [Columnstore indexes guide][Columnstore indexes guide]합니다 [columnstore 인덱스 다시 빌드][columnstore 인덱스 다시 빌드]
...
참고 항목 [워크 로드 관리를 위한 리소스 클래스](resource-classes-for-workload-management.md), [sys.dm_pdw_waits][sys.dm_pdw_waits]
...
참고 항목 [Dmv를 사용 하 여 워크 로드 모니터링][Monitor your workload using DMVs], [LABEL][LABEL]를 [옵션][옵션]를 [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps]하십시오 [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], [sys.dm_pdw_waits][sys.dm_pdw_waits]

## <a name="other-resources"></a>다른 리소스
또한 일반적인 문제 및 해결 방법에 대해서는 [문제 해결][Troubleshooting] 문서를 참조하세요.

이 문서에서 원하는 내용을 찾지 못하면 이 페이지의 왼쪽의 "문서 검색"을 사용하여 모든 Azure SQL Data Warehouse 문서를 검색해보세요.  합니다 [Azure SQL Data Warehouse 포럼][Azure SQL Data Warehouse MSDN Forum] is a place for you to ask questions to other users and to the SQL Data Warehouse Product Group.  We actively monitor this forum to ensure that your questions are answered either by another user or one of us.  If you prefer to ask your questions on Stack Overflow, we also have an [Azure SQL Data Warehouse Stack Overflow Forum][Azure SQL Data Warehouse Stack Overflow Forum]합니다.
