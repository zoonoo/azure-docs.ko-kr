<properties
   pageTitle="Azure SQL 데이터 웨어하우스에서 계산 규모 확장 작업 | Microsoft Azure"
   description="Azure SQL 데이터 웨어하우스 데이터베이스에 대해 계산 리소스를 일시 중지(일시 중단) 또는 시작(다시 시작)하는 방법과 서비스 수준 목표(SLO)에 대해 DWU 설정을 확장 또는 축소하는 방법을 참조하세요. 이 작업에서는 Azure PowerShell cmdlet을 사용합니다."
...
# Azure SQL 데이터 웨어하우스에서 계산 리소스의 규모 확장 또는 축소

> [AZURE.SELECTOR]
- [Azure 포털](sql-data-warehouse-manage-scale-out-tasks.md)
