---
title: 데이터 웨어하우스 만들기 및 쿼리 (Azure Portal)
description: Azure Portal를 사용 하 여 Azure Synapse Analytics SQL 풀 만들기 및 쿼리
...
ms.locfileid: "78381083"
---
# <a name="quickstart-create-and-query-an-azure-synapse-analytics-sql-pool-using-the-azure-portal"></a>빠른 시작: Azure Portal을 사용 하 여 Azure Synapse Analytics SQL 풀 만들기 및 쿼리

Azure Portal를 사용 하 여 Azure Synapse Analytics (이전의 SQL DW)에서 SQL 풀을 프로 비전 하 여 데이터 웨어하우스를 신속 하 게 만들고 쿼리 합니다.
...
**AdventureWorksDW** 샘플 데이터를 포함 하는 데이터 웨어하우스를 만들려면 다음 단계를 수행 합니다.
...
3. **기본 사항**에서 구독, 리소스 그룹, SQL 풀 이름 및 서버 이름을 제공 합니다.
   ...
   | **데이터 웨어하우스 이름** | 전역적으로 고유한 이름 (예: *mySampleDataWarehouse*) | 유효한 데이터베이스 이름은 [데이터베이스 식별자](/sql/relational-databases/databases/database-identifiers)를 참조하세요. 데이터 웨어하우스는 데이터베이스의 한 종류입니다. |
...
5. 이제 Azure Synapse Analytics 양식의 기본 사항 탭을 완료 했으므로 **검토 + 만들기** 및 **만들기** 를 차례로 선택 하 여 SQL 풀에 데이터 웨어하우스를 만듭니다. 프로비전하는 데 몇 분이 걸립니다.
...
2. 왼쪽 메뉴에서 **Azure Synapse analytics** 를 선택 하 고 **azure Synapse analytics** 페이지에서 데이터 웨어하우스를 선택 합니다.
...
- 데이터를 스토리지에 보관하려는 경우 데이터 웨어하우스를 사용하지 않을 때 컴퓨팅을 일시 중지할 수 있습니다. 컴퓨팅을 일시 중지하면 데이터 스토리지 비용만 부과됩니다. 데이터를 작업할 준비가 완료되면 언제든지 컴퓨팅을 다시 시작할 수 있습니다.
...
2. 컴퓨팅을 일시 중지하려면 **일시 중지** 단추를 선택합니다. 데이터 웨어하우스가 일시 중지되면 **다시 시작** 단추가 표시됩니다. 계산을 다시 시작 하려면 **다시 시작**을 선택 합니다.
