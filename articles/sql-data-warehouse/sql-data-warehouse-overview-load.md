...
샘플 데이터베이스를 로드하는 자습서의 경우 [샘플 데이터베이스 로드][샘플 데이터베이스 로드]를 참조하세요.
...
1. PolyBase는 현재 UTF-16을 지원하지 않으므로 데이터를 UTF-8 형식으로 지정합니다.
...
자습서는 [Azure Blob 저장소에서 SQL 데이터 웨어하우스로 데이터 로드(PolyBase)][Azure Blob 저장소에서 SQL 데이터 웨어하우스로 데이터 로드(PolyBase)]를 참조하세요.
...
자습서는 [Azure Blob 저장소에서 SQL 데이터 웨어하우스로 데이터 로드(Azure Data Factory)][Azure Blob 저장소에서 SQL 데이터 웨어하우스로 데이터 로드(Azure Data Factory)]를 참조하세요.
...
SQL Server에서 SQL Data Warehouse로 전체 데이터 마이그레이션을 계획하려면 [마이그레이션 개요][마이그레이션 개요]를 참조하세요. 
...
자습서는 [SQL Server에서 Azure SQL 데이터 웨어하우스로 데이터 로드(SSIS)][SQL Server에서 Azure SQL 데이터 웨어하우스로 데이터 로드(SSIS)]를 참조하세요.
...
자습서는 [SQL Server에서 Azure SQL 데이터 웨어하우스로 데이터 로드(bcp)][SQL Server에서 Azure SQL 데이터 웨어하우스로 데이터 로드(bcp)]를 참조하세요.

### <a name="use-importexport-recommended-for-10-tb-data"></a>가져오기/내보내기 사용(10TB를 초과하는 데이터에 권장)
10TB를 초과하는 크기의 데이터를 Azure로 이동하려는 경우 디스크 전달 서비스인 [가져오기/내보내기][가져오기/내보내기]를 사용하는 것이 좋습니다. 
...
상당수의 파트너에게는 로드 솔루션이 있습니다. 자세한 내용을 알아보려면 [솔루션 파트너][솔루션 파트너] 목록을 참조하세요. 

비관계형 원본에서 가져온 데이터를 SQL 데이터 웨어하우스로 로드하려는 경우 로드하기 전에 데이터를 행과 열로 변환해야 합니다. 변환된 데이터는 데이터베이스에 저장할 필요 없이 텍스트 파일에 저장할 수 있습니다.

새로 로드한 데이터에 대한 통계를 만듭니다. Azure SQL 데이터 웨어하우스는 자동 만들기 또는 통계 자동 업데이트를 아직 지원하지 않습니다.  쿼리에서 최상의 성능을 얻으려면, 데이터를 처음 로드하거나 데이터에 상당한 변화가 발생한 후에 모든 테이블의 모든 열에서 통계가 만들어지는 것이 중요합니다.  자세한 내용은 [통계][통계]를 참조하세요.
...
[SQL Server에서 Azure SQL Data Warehouse로 데이터 로드(AZCopy)]: ./sql-data-warehouse-load-from-sql-server-with-azcopy.md
