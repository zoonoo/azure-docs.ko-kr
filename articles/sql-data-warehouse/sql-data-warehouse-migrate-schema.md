---
title: SQL 데이터 웨어하우스로 스키마 마이그레이션| Microsoft Docs
...
SQL Data Warehouse는 일반적인 비즈니스 데이터 형식을 지원합니다. 지원되는 데이터 형식 및 [지원되지 않는 데이터 형식][지원되지 않는 데이터 형식]의 목록은 [데이터 형식][데이터 형식] 문서를 참조하세요. [데이터 형식][데이터 형식] 문서에는 [지원되지 않는 데이터 형식][지원되지 않는 데이터 형식]을 확인할 수 있는 쿼리도 포함되어 있습니다. 데이터 형식을 변환할 때는 반드시 [데이터 형식 모범 사례][데이터 형식 모범 사례]를 확인하세요.
