...
Azure SQL Data Warehouse의 테이블에 IDENTITY 속성을 사용하여 대리 키를 만들기 위한 권장 사항 및 예제입니다.

## <a name="what-is-a-surrogate-key"></a>대리 키란

테이블의 대리 키는 각 행에 대해 고유 식별자가 있는 열입니다. 이 키는 테이블 데이터에서 생성되지 않습니다. 데이터 웨어하우스 모델을 설계하는 경우 데이터 모델러는 해당 테이블에 대리 키를 만들려고 합니다. 로드 성능에 영향을 주지 않고 간단하고 효과적으로 이 목표를 달성하기 위해 IDENTITY 속성을 사용할 수 있습니다.  
...
IDENTITY 속성은 대리 값을 할당하는 순서를 보장하지 않습니다. 해당 순서는 SQL Server 및 Azure SQL Database의 동작을 반영합니다. 그러나 Azure SQL Data Warehouse에서 보장되지 않는다는 점이 더욱 분명합니다.
