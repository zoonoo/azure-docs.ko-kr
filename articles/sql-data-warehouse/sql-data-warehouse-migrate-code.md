---
title: SQL 데이터 웨어하우스에 SQL 코드 마이그레이션| Microsoft Docs
...
## 재귀 CTE
재귀 CTE는 SQL 데이터 웨어하우스에서 지원되지 않습니다. 재귀 CTE의 마이그레이션은 완료될 수는 있으나 가장 좋은 프로세스는 여러 단계로 분할하는 것입니다. 재귀 중간 쿼리를 반복할 때 일반적으로 루프를 사용하여 임시 테이블을 채울 수 있습니다. 임시 테이블을 채우고 나면 데이터를 단일 결과 집합으로 반환할 수 있습니다. [롤업/큐브/그룹화 집합 옵션을 사용하여 절에 따라 그룹화][롤업/큐브/그룹화 집합 옵션을 사용하여 절에 따라 그룹화] 문서에서 `GROUP BY WITH CUBE` 해결에 사용한 것과 비슷한 방법입니다.
