<properties
   pageTitle="Azure blob 저장소에서 SQL 데이터 웨어하우스로 데이터 로드(PolyBase) | Microsoft Azure"
...
간단히 말하자면, 이 자습서는 공용 Azure 저장소 Blob에서 두 테이블을 Contoso 소매 데이터 웨어하우스 스키마로 로드 합니다. 전체 데이터 집합을 로드하려면 예제 Microsoft SQL Server 샘플 리포지토리에서 [전체 Contoso 소매 데이터 웨어하우스 로드하기][]를 실행합니다.
...
## 시작하기 전에
이 자습서를 실행하려면 SQL 데이터 웨어하우스 데이터베이스를 이미 가지고 있는 Azure 계정이 필요합니다. 계정이 아직 없다면 [SQL 데이터 웨어하우스 만들기][]를 참조하세요.
...
Contoso 공용 데이터를 로드하는 경우에는 **이 단계를 건너뜁니다**. 공용 데이터는 누구나 액세스할 수 있으므로 보안 액세스가 필요하지 않습니다.

자신만의 데이터를 로드하기 위한 템플릿으로 이 자습서를 사용하는 경우에는 **이 단계를 건너뛰지 마십시오**. 자격 증명을 통해 액세스하려면 다음 스크립트를 사용해 데이터베이스 범위 자격 증명을 생성하고 데이터 원본의 위치를 정의할 때 이를 사용합니다.
...
이 [CREATE EXTERNAL DATA SOURCE][] 명령을 사용하여 데이터의 위치와 데이터의 형식을 저장합니다.
...
> [AZURE.IMPORTANT] Azure blob 저장소 컨테이너를 공용으로 만들도록 선택하는 경우 데이터가 데이터 센터에서 떠날 때 데이터 소유자로서 귀하에게 데이터 송신 요금이 부과될 것임을 염두에 두어야 합니다.

## 2\. 데이터 형식 구성

데이터는 Azure blob 저장소에 텍스트 파일로 저장되고 각 필드는 구분 기호로 구분됩니다. 이 [CREATE EXTERNAL FILE FORMAT][] 명령을 실행하여 텍스트 파일로 된 데이터의 형식을 지정합니다. Contoso 데이터는 압축되어 있지 않으며 파이프로 구분됩니다.
...
**위치** 매개 변수는 Azure 저장소 Blob의 루트 폴더 아래에 있는 폴더입니다. 각 테이블은 서로 다른 폴더에 있습니다.
...
## 4\. 데이터 로드
외부 데이터에 액세스하는 방법은 여러가지가 있습니다. 외부 테이블에서 직접 쿼리할 수 있고 새 데이터베이스로 데이터를 로드하거나 기존 데이터베이스 테이블에 외부 데이터를 추가할 수 있습니다.
...
`[sys].[dm_pdw_exec_requests]` 동적 관리 뷰(DMV)를 사용하여 로드 진행률을 추적할 수 있습니다.
...
## 5\. Columnstore 압축을 최적화합니다.

기본적으로 SQL 데이터 웨어하우스는 클러스터형 columnstore 인덱스로 테이블을 저장합니다. 로드를 완료한 후 데이터 행 일부는 columnstore로 압축되지 않을 수 있습니다. 여기에는 다양한 이유가 있습니다. 자세한 내용은 [columnstore 인덱스 관리][]를 참조하세요.
...
Columnstore 인덱스 유지 관리에 대한 자세한 내용은 [columnstore 인덱스 관리][] 문서를 참조하세요.
...
단일 열 통계를 모든 테이블의 모든 열에 대해 만들기로 결정한 경우 [통계][] 문서에 저장된 프로시저 코드 샘플 `prc_sqldw_create_stats`를 사용할 수 있습니다.
...
SQL 데이터 웨어하우스를 탐색해 보십시오.

## 다음 단계
전체 Contoso 소매 데이터 웨어하우스 데이터를 로드하려면 더 많은 개발 팁(For more development tips)의 스크립트를 사용하고 [SQL 데이터 웨어하우스 개발 개요][]를 참조하세요.
