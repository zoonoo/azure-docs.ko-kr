---
title: "Azure Blob 저장소에서 SQL Data Warehouse로 데이터 로드(PolyBase) | Microsoft Docs"
...
간단히 말하자면, 이 자습서는 공용 Azure 저장소 Blob에서 두 테이블을 Contoso 소매 데이터 웨어하우스 스키마로 로드 합니다. 전체 데이터 집합을 로드하려면 예제 Microsoft SQL Server 샘플 리포지토리에서 [전체 Contoso 소매 데이터 웨어하우스 로드하기][전체 Contoso 소매 데이터 웨어하우스 로드하기]를 실행합니다.
...
이 자습서를 실행하려면 SQL 데이터 웨어하우스 데이터베이스를 이미 가지고 있는 Azure 계정이 필요합니다. 계정이 아직 없다면 [SQL 데이터 웨어하우스 만들기][SQL 데이터 웨어하우스 만들기]를 참조하세요.
...
Azure Blob 저장소에서 데이터를 로드하여 데이터베이스 내부의 테이블에 저장하려면 [선택한 테이블 만들기(Transact-SQL)][선택한 테이블 만들기(Transact-SQL)] 문을 사용합니다. CTAS와 함께 로드하면 방금 생성한 강력한 형식의 외부 테이블을 사용할 수 있습니다. 새 테이블에 데이터를 로드하려면 테이블당 하나의 [CTAS][CTAS] 문을 사용하세요. 
...
기본적으로 SQL 데이터 웨어하우스는 클러스터형 columnstore 인덱스로 테이블을 저장합니다. 로드를 완료한 후 데이터 행 일부는 columnstore로 압축되지 않을 수 있습니다.  여기에는 다양한 이유가 있습니다. 자세한 내용은 [columnstore 인덱스 관리][columnstore 인덱스 관리]를 참조하세요.
...
Columnstore 인덱스 유지 관리에 대한 자세한 내용은 [columnstore 인덱스 관리][columnstore 인덱스 관리] 문서를 참조하세요.
...
단일 열 통계를 모든 테이블의 모든 열에 대해 만들기로 결정한 경우 [통계][통계] 문서에 저장된 프로시저 코드 샘플 `prc_sqldw_create_stats`를 사용할 수 있습니다.
...
전체 Contoso 소매 데이터 웨어하우스 데이터를 로드하려면 더 많은 개발 팁(For more development tips)의 스크립트를 사용하고 [SQL 데이터 웨어하우스 개발 개요][SQL 데이터 웨어하우스 개발 개요]를 참조하세요.
...
[다시 빌드]: https://msdn.microsoft.com/library/ms188388.aspx
