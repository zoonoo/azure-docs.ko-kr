...
- SQL 데이터 웨어하우스에서 Azure Blob 저장소로 데이터를 내보냅니다.
...
2. [CREATE MASTER KEY (Transact-SQL)][]를 사용하여 데이터베이스에 대한 마스터 키를 만듭니다. 데이터베이스에 이미 마스터 키가 있으면 다른 키를 만들 필요가 없습니다. 이 키는 다음 단계에서 자격 증명 “암호”를 암호화하는 데 사용됩니다.
...
[CREATE EXTERNAL FILE FORMAT (Transact-SQL)][]을 사용하여 외부 파일 형식을 만듭니다. 이 예제에서는 파일의 데이터는 압축되지 않은 텍스트이며 해당 필드는 파이프 문자('|')로 구분된다고 지정합니다.
...
Azure SQL 데이터 웨어하우스는 자동 만들기 또는 통계 자동 업데이트를 아직 지원하지 않습니다. 쿼리에서 최상의 성능을 얻으려면, 데이터를 처음 로드하거나 데이터에 상당한 변화가 발생한 후에 모든 테이블의 모든 열에서 통계가 만들어지는 것이 중요합니다. 통계에 대한 자세한 설명은 개발 그룹 항목에서 [통계][] 항목을 참조하세요. 다음은 이 예제에 로드한 테이블에 대한 통계를 만드는 방법을 간략히 보여주는 예입니다.
