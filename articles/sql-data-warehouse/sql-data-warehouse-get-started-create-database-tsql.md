...
위의 TSQL 문을 실행할 때 `MAXSIZE` 및 `SERVICE_OBJECTIVE` 매개 변수가 초기 저장소 크기를 결정하고 데이터 웨어하우스 인스턴스에 할당되도록 계산합니다. `MAXSIZE`는 다음 크기를 허용하며 증가할 공간을 생각하면 큰 크기를 선택하는 것이 좋습니다. 2
