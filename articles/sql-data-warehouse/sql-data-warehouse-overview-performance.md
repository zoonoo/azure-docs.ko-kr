...
SQL 데이터 웨어하우스의 목표:
-	페타바이트 크기의 데이터에 대한 예측 가능한 성능 및 선형 확장성
...
미리 보기 동안 SQL 데이터 웨어하우스는 새 기능을 추가하고 중요한 수정 사항을 설치하기 위해 주기적으로 업그레이드됩니다. 이러한 업그레이드로 인해 작업이 중단될 수 있으며 현재 업그레이드는 예측 가능한 일정에 따라 수행되지 않습니다. 이 프로세스가 너무 많이 작업을 중단시키면 이 프로세스 해결에 도움을 드릴 수 있도록 [지원 티켓 만들기][]를 권장합니다.
...
- **로드 속도**: 데이터 웨어하우스로 로드될 수 있는 초당 레코드 수입니다. 특히 PolyBase를 통해 Azure Blob 저장소에서 클러스터형 열-저장소 인덱스가 있는 테이블로 가져올 수 있는 레코드 수를 측정합니다.
- **검색 속도**: 데이터 웨어하우스에서 순차적으로 검색될 수 있는 초당 레코드 수입니다. 이 메트릭은 특히 클러스터형 열-저장소 인덱스가 있는 테이블에서 쿼리에 의해 반환된 레코드의 수를 측정합니다.
