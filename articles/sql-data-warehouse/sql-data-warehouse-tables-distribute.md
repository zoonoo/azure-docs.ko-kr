...
올바른 배포 열을 선택하여 데이터 이동을 최소화하는 일은 SQL 데이터 웨어하우스의 성능을 최적화하기 위한 가장 중요한 전략 중 하나입니다. 데이터 이동은 테이블이 조인되거나 집계가 수행될 때 흔히 발생합니다. 일반적으로 조인된 열에 해시 방식으로 대형 팩트 테이블을 분산하는 것이 데이터 이동을 최소화하기 위한 가장 효과적인 방법 중 하나입니다. 데이터 이동을 피하기 위해서는 조인 열을 선택하는 것 외에도, 몇 가지 조건을 충족해야 합니다. 데이터 이동을 방지하려면

1. 조인에 관련된 테이블에 조인 열 중 하나에 해시 분산해야 합니다.
2. 조인 열의 데이터 형식은 일치해야 합니다.
3. 같음 연산자로 열을 조인해야 합니다.
4. 조인 유형은 `CROSS JOIN`이 아닐 수 있습니다.

`JOIN`, `GROUP BY`, `DISTINCT`, `OVER` 및 `HAVING` 절에 사용한 열이 모두 해시 분산 후보로 적합합니다. 반면에, `WHERE` 절의 열은 쿼리에 참여하는 배포를 제한하므로 해시 열로 적합하지 **않습니다**. 일반적으로 두 개의 대형 팩트 테이블이 조인에 자주 포함될 경우 조인 열 중 하나에 좀 더 자주 배포하게 됩니다. 다른 대형 팩트 테이블에 절대 조인되지 않는 테이블이 있는 경우 `GROUP BY` 절에서 자주 나오는 열을 찾아보세요. 데이터를 균일하게 배포하는 적합한 후보 열을 찾을 수 없는 경우가 아니면, 기본 라운드 로빈 배포를 사용하는 것보다, 배포 열을 선택하는 것이 쿼리에 좀 더 도움이 됩니다.
