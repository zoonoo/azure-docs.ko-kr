<properties
   pageTitle="SQL 데이터 웨어하우스에 대한 지원 티켓 만들기 | Microsoft Azure"
...
7. **지원 계획**을 선택합니다.
    ...
    - 청구 및 구독 관리 관련 문제에 대한 지원이 모든 지원 수준에서 제공됩니다.
    - 고장 수리 문제에 대한 지원은 개발자, 표준, 전문가 직접 또는 프리미어 지원을 통해 제공됩니다. 고장 수리 문제는 고객이 Azure를 사용하는 동안 고객에게 발생한 문제 중 Microsoft로 인해 문제가 발생했다는 합리적인 이유가 존재하는 문제를 의미합니다.
    - 개발자 멘토링 및 자문 서비스는 전문가 지원 및 프리미어 지원 수준에서 제공됩니다.
    - Azure 지원 범위, 응답 시간, 가격 책정 등을 포함한 자세한 정보는 [Azure 지원 계획][]에서 확인하세요.
...
또한 [S스택 오버플로][] 또는 [Azure SQL 데이터 웨어하우스 MSDN 포럼][]에서 azure-sqldw 태그를 사용하여 SQL 데이터 웨어하우스 커뮤니티에 연결할 수 있습니다.
