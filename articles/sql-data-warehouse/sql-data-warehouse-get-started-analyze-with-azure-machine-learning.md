...
2. 화면 왼쪽 아래의 **+ New**를 클릭하고 **Blank Experiment**를 선택합니다.
...
실험 캔버스 아래에서 **Run**을 클릭하여 실험을 실행합니다.
![실험 실행][1]

실험이 성공적으로 실행되고 나면 판독기 모듈 아래쪽에서 출력 포트를 클릭하고 **Visualize**를 선택하여 가져온 데이터를 확인합니다.
![가져온 데이터 확인][3]
...
여기서는 데이터를 80:20 비율로 분할합니다. 80%는 기계 학습 모델을 학습시키는 데 사용되며, 20%는 모델을 테스트하는 데 사용됩니다. 이진 분류 문제에 대해 "Two-Class" 알고리즘을 활용합니다.

1. **Split Data** 모듈을 캔버스로 끌어서 놓습니다.
...
제공된 메트릭은 ROC 곡선, 정밀도-리콜 다이어그램 및 리프트 곡선입니다. 이러한 메트릭을 살펴보면 첫 번째 모델이 두 번째보다 더 잘 실행하는 것을 볼 수 있습니다. 첫 번째 모델이 예측하는 것을 보려면 Score Model의 출력 포트를 클릭하고 Visualize를 클릭합니다.
![점수 결과 시각화][12]
