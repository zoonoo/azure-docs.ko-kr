...
1. **서버 평가** > **평가 유형** 에서 **AVS(Azure VMware Solution)(미리 보기)** 를 선택합니다.

1. **검색 원본** 에서 다음을 수행합니다.
...
1. **평가 속성** > **대상 속성** 에서 다음을 수행합니다.
    ...
    - **대상 지역** 에서 마이그레이션할 Azure 지역을 지정합니다.
       ...
       - 현재 4개 지역(오스트레일리아 동부, 미국 동부, 서유럽, 미국 서부)에 대해 평가할 수 있습니다.
...
1. 어플라이언스를 선택하고, 그룹에 추가하려는 VM을 선택합니다. 그런 다음 **다음** 을 클릭합니다.
...
- AVS 노드에서의 사용률 모든 노드에서 예상되는 CPU, 메모리 및 스토리지 사용률입니다.
    ...
    - 메모리, 중복 제거 및 압축은 현재 메모리, 1.5 중복 제거 및 압축에 대해 100% 사용률로 설정되어 있으며 이는 향후 릴리스에서 사용자가 필요한 크기 조정을 미세 조정할 수 있도록 사용자 정의 입력이 됩니다.
