...
. **컴퓨터 검색**에서 다음을 수행합니다.
    ...
    - **컬렉션 범위**에서 VM 검색에 대한 범위를 선택합니다. 수집기는 지정된 범위 내의 VM만 검색할 수 있습니다. 범위를 특정 폴더, 데이터 센터 또는 클러스터로 설정할 수 있습니다. 여기에 1000개 이상의 VM을 포함하지 않아야 합니다. 
    - **그룹화에 대한 태그 범주**에서 **None**을 선택합니다.
1. **프로젝트 선택**에서 Azure Migrate 프로젝트 ID를 지정하고 포털에서 복사한 키를 지정합니다. 해당 항목을 복사하지 않은 경우 수집기 VM에서 Azure Portal을 엽니다. 프로젝트 **개요** 페이지에서 **컴퓨터 검색**을 클릭하고 값을 복사합니다.  
2. **전체 검색**에서 검색을 모니터링하고 VM에서 수집한 메타데이터가 범위 내에 있는지 확인합니다. 수집기는 대략적인 검색 시간을 제공합니다.
...
- 많은 수의 온-프레미스 컴퓨터에 대한 평가를 설정하는 방법을 [알아봅니다](how-to-scale-assessment.md).
- [컴퓨터 종속성 매핑](how-to-create-group-machine-dependencies.md)을 사용하여 보다 자세한 평가 그룹을 만드는 방법을 알아봅니다.
- 평가가 계산되는 방법에 대해 [자세히 알아봅니다](concepts-assessment-calculation.md).
