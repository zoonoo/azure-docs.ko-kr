...
- Azure Portal에서 Azure Migrate 설치 관리자 스크립트가 포함 된 압축 파일을 다운로드 합니다.
- 압축 된 파일에서 콘텐츠를 추출 합니다. 관리자 권한으로 PowerShell 콘솔을 시작 합니다.
- PowerShell 스크립트를 실행 하 여 어플라이언스 웹 응용 프로그램을 시작 합니다.
- 어플라이언스를 처음으로 구성하고, Azure Migrate 프로젝트에 등록합니다.

## <a name="download-the-installer-script"></a>설치 관리자 스크립트 다운로드

어플라이언스에 대 한 zip 파일을 다운로드 합니다.
...
3.  어플라이언스 버전 1.19.05.10의 경우 생성 된 해시가 이러한 설정과 일치 해야 합니다.
...
## <a name="run-the-azure-migrate-installer-script"></a>Azure Migrate 설치 관리자 스크립트를 실행 합니다.
= 설치 관리자 스크립트는 다음을 수행 합니다.

- 실제 서버 검색 및 평가를 위해 에이전트와 웹 응용 프로그램을 설치 합니다.
...
- Azure Migrate에 대 한 영구 설정 정보를 사용 하 여 레지스트리 키 (HKLM)를 업데이트 합니다.
- 경로 아래에 다음 파일을 만듭니다.
...
1. 어플라이언스를 호스트 하는 서버의 폴더에 압축 파일 압축을 풉니다.
2. 관리자 권한 (관리자)이 있는 위의 서버에서 PowerShell을 시작 합니다.
3. 다운로드 한 압축 파일에서 콘텐츠를 추출한 폴더로 PowerShell 디렉터리를 변경 합니다.
4. 다음 명령을 실행 하 여 스크립트를 실행 합니다.
    ```
    PS C:\Users\Administrators\Desktop> AzureMigrateInstaller-physical.ps1
    ```
스크립트가 성공적으로 완료 되 면 어플라이언스 웹 응용 프로그램이 시작 됩니다.
...
1. VM에 연결할 수 있는 모든 컴퓨터에서 브라우저를 열고 어플라이언스 웹 앱의 URL ( **https://*어플라이언스 이름 또는 IP 주소*: 44368**)을 엽니다.
   ...
   또는 앱 바로 가기를 클릭 하 여 데스크톱에서 앱을 열 수 있습니다.
...
1. **자격 증명 추가** 를 클릭 하 여 어플라이언스에서 서버를 검색 하는 데 사용할 계정 자격 증명을 지정 합니다.  
2. **운영 체제**, 자격 증명의 이름, **사용자 이름** 및 **암호** 를 지정 하 고 **추가**를 클릭 합니다.
Windows 및 Linux 서버에 각각 하나의 자격 증명 집합을 추가할 수 있습니다.
4. 서버 **추가**를 클릭 하 고 서버 정보-FQDN/IP 주소 및 자격 증명 이름 (행당 한 항목)을 지정 하 여 서버에 연결 합니다.
3. **유효성 검사**를 클릭합니다. 유효성 검사 후 검색 가능한 서버 목록이 표시 됩니다.
    - 서버에 대 한 유효성 검사가 실패 하는 경우 **상태** 열에서 아이콘을 가리켜 오류를 검토 합니다. 문제를 해결하고, 유효성을 다시 검사합니다.
    - 서버를 제거 하려면 > **삭제**를 선택 합니다.
