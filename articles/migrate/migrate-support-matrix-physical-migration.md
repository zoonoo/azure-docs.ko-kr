...
ms.locfileid: "98703889"
---
# <a name="support-matrix-for-physical-server-migration"></a>물리적 서버 마이그레이션을 위한 지원 매트릭스

이 문서에서는 [Azure Migrate: 서버 마이그레이션을](migrate-services-overview.md#azure-migrate-server-migration-tool) 사용 하 여 물리적 서버를 Azure로 마이그레이션하기 위한 지원 설정 및 제한 사항을 요약 합니다. Azure로의 마이그레이션을 위해 물리적 서버를 평가 하는 방법에 대 한 정보를 찾고 있는 경우 [평가 지원 매트릭스](migrate-support-matrix-physical.md)를 검토 하세요.
...
- 사설 클라우드에서 실행 되는 Vm
- Amazon Web Services (AWS) 또는 Google Cloud Platform (GCP)를 포함 하 여 공용 클라우드에서 실행 되는 Vm.
...
**UEFI 부팅** | 지원됨. UEFI 기반 컴퓨터는 Azure 2 세대 Vm으로 마이그레이션됩니다.  <br/><br/> OS 디스크에는 최대 4 개의 파티션이 있어야 하 고 볼륨은 NTFS로 포맷 되어야 합니다.
...
데이터 디스크 크기 | 최대 32 TB | 지원되지 않는 경우 확인이 실패합니다.
