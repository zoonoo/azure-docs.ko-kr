...
이러한 선택한 비교를 사용 하 여 사용할 방법을 결정 하는 데 도움을 줍니다. [에이전트 없는](migrate-support-matrix-vmware-migration.md#agentless-vmware-servers) 에이전트와 [에이전트 기반](migrate-support-matrix-vmware-migration.md#agent-based-vmware-servers) 마이그레이션에 대한 전체 지원 요구 사항도 검토할 수 있습니다.
...
**통과 디스크** | 지원하지 않음 | 지원됨
**UEFI 부팅** | 지원하지 않음 | Azure에서 마이그레이션된 VM은 자동으로 BIOS 부팅 VM으로 변환 됩니다.<br/><br/> OS 디스크에는 최대 4 개의 파티션이 있어야 하 고 볼륨은 NTFS로 포맷 되어야 합니다.
...
**Azure Migrate 어플라이언스 배포** | VM 검색 및 평가를 위해 VMware VM에서 경량 어플라이언스를 설정 합니다. | 필수 | 필요하지 않습니다.
**Vm에 모바일 서비스 설치** | 복제 하려는 각 VM에 모바일 서비스를 설치 합니다. | 필요 없음 | 필수
**Azure Migrate Server Migration replication 어플라이언스 배포** | VMware VM에서 어플라이언스를 설정 하 여 vm을 검색 하 고, Vm에서 실행 되는 모바일 서비스와 Azure Migrate 서버 마이그레이션을 연결 합니다. | 필요 없음 | 필수
