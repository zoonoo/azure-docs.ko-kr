...
**Vm에 모바일 서비스 설치** | 모바일 서비스는 복제 하려는 각 VM에서 실행 됩니다. | 필요하지 않음 | 필요한 공간
**복제 어플라이언스 배포** | [복제 어플라이언스](migrate-replication-appliance.md) 는 에이전트 기반 마이그레이션에 사용 됩니다. Vm에서 실행 되는 모바일 서비스와 서버 마이그레이션을 연결 합니다. | 필요하지 않음 | 필요한 공간
