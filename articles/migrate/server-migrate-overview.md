...
**동시 복제** | VCenter Server에서 최대 100 Vm을 동시에 복제할 수 있습니다.<br/> 마이그레이션을 위해 Vm이 50 개 이상인 경우 Vm의 여러 일괄 처리를 만듭니다.<br/> 한 번에 더 많은 복제는 성능에 영향을 줍니다. | n/a
...
**대상 디스크** | Managed Disks | Managed Disks
**디스크 제한** | OS 디스크: 2tb<br/><br/> 데이터 디스크: 4tb<br/><br/> 최대 디스크: 60 | OS 디스크: 2tb<br/><br/> 데이터 디스크: 8TB<br/><br/> 최대 디스크: 63
**통과 디스크** | Nem támogatott | Támogatott
**UEFI 부팅** | Nem támogatott | Azure에서 마이그레이션된 VM은 자동으로 BIOS 부팅 VM으로 변환 됩니다.<br/><br/> OS 디스크에는 최대 4 개의 파티션이 있어야 하 고 볼륨은 NTFS로 포맷 되어야 합니다.
...
**Tevékenység** | **Részletek** |**에이전트** | **에이전트 기반**
--- | --- | --- | ---
**Értékelés** | 마이그레이션 전에 서버를 평가 합니다.  평가는 선택 사항입니다. 마이그레이션하기 전에 컴퓨터를 평가 하는 것이 좋습니다. <br/><br/> 평가를 위해 Azure Migrate는 Vm을 검색 하 고 평가 하는 경량 어플라이언스를 설정 합니다. | 평가 후 에이전트 없는 마이그레이션을 실행 하는 경우 평가에 대해 설정 된 동일한 Azure Migrate 어플라이언스는 에이전트 없는 마이그레이션에 사용 됩니다.  |  평가 후에 에이전트 기반 마이그레이션을 실행 하는 경우 평가를 위해 설정 된 어플라이언스는 에이전트 없는 마이그레이션 중에 사용 되지 않습니다. 어플라이언스를 그대로 두거나 추가 검색 및 평가를 수행 하지 않으려는 경우 제거할 수 있습니다.
**마이그레이션을 위해 VMware 서버 및 Vm 준비** | VMware 서버 및 Vm에 대 한 다양 한 설정을 구성 합니다. | Szükséges | Szükséges
**서버 마이그레이션 도구 추가** | Azure Migrate 프로젝트에 Azure Migrate 서버 마이그레이션 도구를 추가 합니다. | Szükséges | Szükséges
**Azure Migrate 어플라이언스 배포** | VM 검색 및 평가를 위해 VMware VM에서 경량 어플라이언스를 설정 합니다. | Szükséges | 필수 아님.
**Vm에 모바일 서비스 설치** | 복제 하려는 각 VM에 모바일 서비스를 설치 합니다. | Nem szükséges | Szükséges
**Azure Migrate Server Migration replication 어플라이언스 배포** | VMware VM에서 어플라이언스를 설정 하 여 vm을 검색 하 고, Vm에서 실행 되는 모바일 서비스와 Azure Migrate 서버 마이그레이션을 연결 합니다. | Nem szükséges | Szükséges
**Vm을 복제**합니다. VM 복제를 사용 하도록 설정 합니다. | 복제 설정을 구성 하 고 복제할 Vm을 선택 합니다. | Szükséges | Szükséges
**테스트 마이그레이션 실행** | 테스트 마이그레이션을 실행하여 모든 것이 예상대로 작동하는지 확인합니다. | Szükséges | Szükséges
**전체 마이그레이션 실행** | Vm을 마이그레이션합니다. | Szükséges | Szükséges
