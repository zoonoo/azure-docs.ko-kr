---
title: Azure Migrate를 사용하여 Azure로 마이그레이션할 물리적 서버 평가
description: Azure Migrate를 사용하여 Azure로 마이그레이션할 온-프레미스 물리적 서버를 평가하는 방법에 대해 설명합니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
이 문서에서는 Azure Migrate: 검색 및 평가 도구를 사용하여 Azure로 마이그레이션할 온-프레미스 물리적 서버를 평가하는 방법을 보여줍니다.
...
- 이 자습서에 따라 Azure VM으로 마이그레이션할 서버를 평가하기 전에 평가할 서버를 검색했는지 확인합니다.
    - Azure Migrate 어플라이언스를 사용하여 서버를 검색하려면 [이 자습서를 수행](tutorial-discover-physical.md)합니다. 
    - 가져온 CSV 파일을 사용하여 서버를 검색하려면 [이 자습서를 수행](tutorial-discover-import.md)합니다.
- 평가하려는 물리적 서버가 Windows Server 2003 또는 SUSE Linux를 실행하고 있지 않은지 확인합니다. 이러한 서버에는 평가가 지원되지 않습니다.
