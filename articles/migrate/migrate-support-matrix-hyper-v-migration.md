...
| **지원**                | **세부 정보**               
| :-------------------       | :------------------- |
| **배포**       | Hyper-V 호스트는 독립 실행형이거나 클러스터에 배포될 수 있습니다. <br/>Azure 마이그레이션 복제 소프트웨어(Hyper-V 복제 공급자)는 Hyper-V 호스트에 설치해야 합니다.|
| **권한을**           | Hyper-V 호스트에 대한 관리자 권한이 필요합니다. |
| **호스트 운영 체제** | 윈도우 서버 2019, 윈도우 서버 2016, 또는 윈도우 서버 2012 R2. |
| **URL 액세스** | Hyper-V 호스트의 복제 공급자 소프트웨어는 다음 URL에 액세스해야 합니다.<br/><br/> - login.microsoftonline.com : 활성 디렉토리를 사용하여 액세스 제어 및 ID 관리.<br/><br/> - *.backup.windowsazure.com: 복제 데이터 전송 및 조정. 서비스 URL을 마이그레이션합니다.<br/><br/> - *.blob.core.windows.net: 스토리지 계정에 데이터를 업로드합니다.<br/><br/> - dc.services.visualstudio.com : 내부 모니터링에 사용되는 응용 프로그램 로그를 업로드합니다.<br/><br/> - time.windows.com : 시스템과 글로벌 시간 사이의 시간 동기화를 확인합니다.
...
| **Azure에 필요한 변경 사항** | 일부 VM은 Azure에서 실행될 수 있도록 변경해야 할 수 있습니다. 마이그레이션하기 전에 수동으로 조정해야 합니다. 관련 문서에는 이 작업을 수행하는 방법에 대한 지침이 포함되어 있습니다. |
...
| **IPv6** | 지원되지 않습니다.
| **NIC 팀 구성** | 지원되지 않습니다.
| **Azure 사이트 복구** | Azure 사이트 복구를 사용하여 복제할 때 VM이 활성화된 경우 Azure 마이그레이션 서버 마이그레이션을 사용하여 복제할 수 없습니다.
| **Ports** | VM 복제 데이터를 전송하기 위해 HTTPS 포트 443의 아웃바운드 연결입니다.
