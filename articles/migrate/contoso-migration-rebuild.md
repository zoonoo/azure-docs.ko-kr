...
- **비즈니스 성장 대응**: Contoso는 계속 성장하고 있습니다. 웹 사이트를 방문하는 고객에게 차별화된 경험을 제공하려고 합니다.
- **민첩성**: Contoso는 글로벌 경제에서 성공할 수 있으려면 시장의 변화에 더 빠르게 대응할 수 있어야 합니다. 
...
- 앱의 프런트 엔드는 주 지역에 Azure App Services 웹앱으로 배포됩니다.
- Azure 함수는 애완 동물 사진 업로드를 제공하고, 사이트는 이 기능과 상호 작용합니다.
...
- 컨테이너는 VSTS를 사용하여 빌드되어 ACR(Azure Container Registry)에 푸시됩니다.
- 이제 Contoso는 Visual Studio를 사용하여 웹앱 및 함수 코드를 수동으로 배포합니다.
...
**고려 사항** | **세부 정보**
--- | ---
**장점** | 종단 간 배포에 PaaS 및 무서버 솔루션을 사용하면 Contoso가 제공해야 하는 관리 시간이 크게 줄어듭니다.<br/><br/> 마이크로 서비스 아키텍처로 이동하면 Contoso는 시간이 지남에 따라 솔루션을 쉽게 확장할 수 있습니다.<br/><br/> 기존 솔루션 코드 베이스를 중단하지 않고 새 기능을 온라인으로 전환할 수 있습니다.<br/><br/> 웹앱은 단일 실패 지점이 없도록 여러 인스턴스를 사용하여 구성됩니다.<br/><br/> 앱이 달라지는 트래픽 볼륨을 처리할 수 있도록 자동 크기 조정이 사용됩니다.<br/><br/> PaaS 서비스로 이동한 Contoso는 Windows Server 2008 R2 운영 체제에서 실행되는 오래된 솔루션을 사용 중지할 수 있습니다.<br/><br/> CosmosDB는 Contoso에서 구성할 필요가 없는 기본 내결함성을 갖추고 있습니다. 따라서 데이터 계층은 더 이상 단일 장애 조치(failover) 지점이 아닙니다.
**단점** | 컨테이너는 다른 마이그레이션 옵션보다 복잡합니다. 학습 곡선이 Contoso에는 문제가 될 수 있습니다.  학습 곡선에도 불구하고 여러 가치를 제공하는 새로운 복잡성이 추가됩니다.<br/><br/> Contoso의 운영 팀은 Azure, 컨테이너 및 앱용 마이크로 서비스를 이해하고 지원하기 위해 노력해야 합니다.<br/><br/> Contoso는 전체 솔루션에 대한 DevOps를 완전히 구현하지 않았습니다. 서비스를 AKS, 함수 및 App Services에 배포하는 것에 대해 생각해 보아야 합니다.
...
3. 인프라가 준비되면 마이크로 서비스 컨테이너 이미지를 ACR에 푸시하는 VSTS를 사용하여 마이크로 서비스 컨테이너 이미지를 빌드해야 합니다.
...
[Azure 기능](https://azure.microsoft.com/services/functions/) | 서버 없는 이벤트 구동 계산 환경으로 개발을 가속화합니다. 필요할 때 확장할 수 있습니다.  | 사용한 리소스 비용만 지불하면 됩니다. 초당 리소스 사용량과 실행 횟수에 따라 요금이 청구됩니다. [자세히 알아보기](https://azure.microsoft.com/pricing/details/functions/).
...
**요구 사항** | **세부 정보**
--- | ---
**Azure 구독** | 이 시리즈의 첫 번째 문서에서 평가를 수행했을 때 이미 구독이 만들어졌어야 합니다. Azure 구독이 아직 없는 경우 [체험 계정](https://azure.microsoft.com/pricing/free-trial/)을 만듭니다.<br/><br/> 체험 계정을 만들면 구독 관리자로서 모든 작업을 수행할 수 있습니다.<br/><br/> 기존 구독을 사용하고 관리자가 아닌 경우 관리자와 협력하여 소유자 또는 기여자 권한을 할당받아야 합니다.
...
> * **2단계: Docker 컨테이너 빌드**: VSTS를 사용하여 Docker 컨테이너용 CI를 설정하고, ACR에 푸시합니다.
...
## <a name="step-1-provision-an-aks-cluster-and-acr"></a>1단계: AKS 클러스터 및 ACR 프로비전

Contoso는 AKS 및 Azure Container Registry를 사용하여 관리되는 Kubernetes 클러스터를 만드는 배포 스크립트를 실행합니다.
...
Contoso는 다음과 같이 프로비전합니다.

1. 시작하기 전에 Contoso는 사용 중인 개발 머신에 필수 소프트웨어가 모두 설치되어 있는지 확인합니다. 
2. Git을 사용하여 개발 머신에 로컬로 리포지토리를 복제합니다.

    **git clone https://github.com/Microsoft/SmartHotel360-Azure-backend.git**

3.  Contoso는 Visual Studio Code를 사용하여 폴더를 열고, **gen-aks-env.ps1** 스크립트가 포함된 **/deploy/k8s** 디렉터리로 이동합니다.
4. AKS 및 Container Registry를 사용하여 관리되는 Kubernetes 클러스터를 만드는 스크립트를 실행합니다.

    ![AKS](./media/contoso-migration-rebuild/aks1.png)
 
5.  파일이 열린 상태에서 $location 매개 변수를 **eastus2**로 업데이트하고 파일을 저장합니다.

    ![AKS](./media/contoso-migration-rebuild/aks2.png)

6. **보기** > **통합 터미널**을 클릭하여 Code에서 통합 터미널을 엽니다.

    ![AKS](./media/contoso-migration-rebuild/aks3.png)

7. PowerShell 통합 터미널에서 Connect-AzureRmAccount 명령을 사용하여 Azure에 로그인합니다. PowerShell을 시작하는 방법에 대해 [자세히 알아보세요](https://docs.microsoft.com/powershell/azure/get-started-azureps).

    ![AKS](./media/contoso-migration-rebuild/aks4.png)

8. **az login** 명령을 실행한 후 웹 브라우저에서 인증 지침을 수행하여 Azure CLI를 인증합니다. Azure CLI를 사용하여 로그인하는 방법에 대해 [자세히 알아보세요](https://docs.microsoft.com/cli/azure/authenticate-azure-cli?view=azure-cli-latest).

    ![AKS](./media/contoso-migration-rebuild/aks5.png)

9. 다음 명령을 실행하여 ContosoRG의 리소스 그룹 이름, AKS 클러스터 smarthotel-aks-eus2의 이름 및 새 레지스트리 이름을 전달합니다.
...
10. Azure는 AKS 클러스터에 사용할 리소스가 포함된 또 다른 리소스 그룹을 만듭니다.

    ![AKS](./media/contoso-migration-rebuild/aks7.png)

11. 배포가 완료되면 Contoso는 **kubectl** 명령줄 도구를 설치합니다. 이 도구는 Azure CloudShell에 이미 설치되어 있습니다.

    **az aks install-cli**

12. **kubectl get nodes** 명령을 실행하여 클러스터에 대한 연결을 확인합니다. 노드 이름은 자동으로 만들어진 리소스 그룹의 VM과 동일합니다.

    ![AKS](./media/contoso-migration-rebuild/aks8.png)

13. 다음 명령을 실행하여 Kubernetes 대시보드를 시작합니다. 

    **az aks browse --resource-group ContosoRG --name smarthotelakseus2**

14. 브라우저 탭이 대시보드로 열립니다. Azure CLI를 사용하는 터널링된 연결입니다. 
...
## <a name="step-2-build-a-docker-container"></a>2단계: Docker 컨테이너 빌드

### <a name="create-a-vsts-and-build"></a>VSTS 및 빌드 만들기

Contoso는 VSTS 프로젝트를 만들고, 컨테이너를 만들어서 ACR에 푸시하도록 CI 빌드를 구성합니다. 이 섹션의 명령은 [SmartHotel360-Azure-Backend](https://github.com/Microsoft/SmartHotel360-Azure-backend) 리포지토리를 사용합니다.

1. Visualstudio.com에서 새 계정(**contosodevops360.visualstudio.com**)을 만들고, Git을 사용하도록 구성합니다.

2. 버전 제어용 Git과 워크플로용 Agile을 사용하여 새 프로젝트(**smarthotelrefactor**)를 만듭니다.

    ![VSTS](./media/contoso-migration-rebuild/vsts1.png) 


3. GitHub 리포지토리를 가져옵니다.
...
7. 빌드 정의로 **호스팅되는 Linux 미리 보기**를 선택합니다.
...
9. 리포지토리의 **src** 폴더에서 **docket-compose.yaml** 파일 경로를 지정합니다. 서비스 이미지를 빌드하기로 선택하고 최신 태그를 포함합니다. 작업이 **서비스 이미지 빌드**로 변경되면 VSTS 작업의 이름이 **자동으로 서비스 빌드**로 변경됩니다.

    ![VSTS](./media/contoso-migration-rebuild/vsts9.png)

10. 이제 Contoso는 푸시할 두 번째 Docker 작업을 구성합니다. 구독을 선택하고 **smarthotelacreus2** ACR을 선택합니다. 

    ![VSTS](./media/contoso-migration-rebuild/vsts10.png)

11. 다시 한 번 docker-compose.yaml 파일에 파일을 입력하고 **서비스 이미지 푸시**를 선택한 후 최신 태그를 포함합니다. 작업이 **서비스 이미지 푸시**로 변경되면 VSTS 작업의 이름이 **자동으로 서비스 푸시**로 변경됩니다.

    ![VSTS](./media/contoso-migration-rebuild/vsts11.png)

12. VSTS 작업이 구성되면 Contoso는 빌드 정의를 저장하고 빌드 프로세스를 시작합니다.
...
## <a name="step-3-deploy-back-end-microservices"></a>3단계: 백 엔드 마이크로 서비스 배포

AKS 클러스터가 만들어지고 Docker 이미지가 빌드되어, 이제 Contoso는 백 엔드 마이크로 서비스에서 활용할 나머지 인프라를 배포합니다.
...
1. Contoso는 다음 명령을 입력하고 deploy.cmd 파일을 사용하여 ContosoRG 리소스 그룹 및 EUS2 지역에 Azure 리소스를 배포합니다.

    **.\deploy azuredeploy ContosoRG -c eastus2**

    ![백 엔드 배포](./media/contoso-migration-rebuild/backend1.png)

2. 나중에 사용할 수 있도록 각 데이터베이스의 연결 문자열을 캡처합니다.

    ![백 엔드 배포](./media/contoso-migration-rebuild/backend2.png)

## <a name="step-4-deploy-front-end-infrastructure"></a>4단계: 프런트 엔드 인프라 배포

Contoso는 프런트 엔드 앱에서 사용할 인프라를 배포해야 합니다. 애완 동물 이미지를 저장할 BLOB 컨테이너, 애완 동물 정보가 포함된 문서를 저장할 Cosmos 데이터베이스, 웹 사이트에 대한 Vision API를 만듭니다. 

이 섹션의 명령은 [SmartHotel-public-web](https://github.com/Microsoft/SmartHotel360-public-web) 리포지토리를 사용합니다.

### <a name="create-storage-containers"></a>저장소 컨테이너 만들기

1.  Azure Portal에서, Contoso는 앞에서 만든 저장소 계정을 열고 **Blob**을 클릭합니다.
...
Contoso는 애완 동물 정보에 사용할 Cosmos 데이터베이스를 프로비전합니다.
...
Contoso는 Computer Vision API를 프로비전합니다. 이 API는 함수에 의해 호출되며, 사용자가 업로드한 사진을 평가하는 용도로 사용됩니다.
...
## <a name="step-5-deploy-the-back-end-services-in-azure"></a>5단계: Azure에서 백 엔드 서비스 배포

이제 Contoso는 서비스에 대한 인바운드 트래픽을 허용하는 NGINX 수신 컨트롤러를 배포한 후 AKS 클러스터에 마이크로 서비스를 배포해야 합니다.

이 섹션의 명령은 [SmartHotel360-Azure-Backend](https://github.com/Microsoft/SmartHotel360-Azure-backend) 리포지토리를 사용합니다.



1. Visual Studio Code를 사용하여 **/deploy/k8s/config_loal.yml** 파일을 업데이트합니다. 앞에서 기록해 둔 정보의 다양한 데이터베이스 연결을 업데이트합니다.

     ![마이크로 서비스 배포](./media/contoso-migration-rebuild/deploy-micro.png)

    > [!NOTE]
    > 구성 설정 중 일부는(예: Active Directory B2C) 이 문서에서 다루지 않습니다. 이러한 설정에 대한 추가 정보는 리포지토리에 있습니다.

2. deploy.ps1 파일은 수신 및 수신 컨트롤러를 제외한 모든 클러스터 콘텐츠를 삭제하고 마이크로 서비스를 배포합니다. 다음과 같이 실행합니다.
...
3. 다음 명령을 실행하여 서비스 상태를 확인합니다.

    ```
    kubectl get services
    ```
4. Kubernetes 대시보드를 열고 배포를 검토합니다.
...
### <a name="set-up-web-app-to-use-contoso-resources"></a>Contoso 리소스를 사용하도록 웹앱 설정

1. Contoso는 Git을 사용하여 개발 머신에 로컬로 리포지토리를 복제합니다.

    **git clone https://github.com/Microsoft/SmartHotel360-public-web.git**

2. Visual Studio에서 폴더를 열어 리포지토리의 모든 파일을 표시합니다.

    ![프런트 엔드 게시](./media/contoso-migration-rebuild/front-publish1.png)

3. 필요한 구성을 기본 설정으로 변경합니다.

    - 웹앱은 시작되면 **SettingsUrl** 앱 설정을 찾습니다.
    - 이 변수는 구성 파일에 대한 URL을 포함해야 합니다.
    - 기본적으로 사용되는 설정은 공용 엔드포인트입니다.

4. **/config-sample.json/sample.json** 파일을 업데이트합니다. 이 파일은 공용 엔드포인트를 사용할 때 웹의 구성 파일입니다.
    ...
    ![프런트 엔드 게시](./media/contoso-migration-rebuild/front-publish2.png)

5. 파일이 업데이트된 후 Contoso는 이름을 **smarthotelsettingsurl**로 변경하고, 이전에 만든 저장소 BLOB에 업로드합니다.

     ![프런트 엔드 게시](./media/contoso-migration-rebuild/front-publish3.png)

6. Contoso는 파일을 클릭하여 URL을 가져옵니다. 이 URL은 앱이 구성 파일을 가져올 때 사용됩니다.

    ![프런트 엔드 게시](./media/contoso-migration-rebuild/front-publish4.png)

7. **appsettings.Production.json** 파일의 **SettingsUrl**을 새 파일의 URL로 업데이트합니다.

    ![프런트 엔드 게시](./media/contoso-migration-rebuild/front-publish5.png)


### <a name="deploy-the-website-to-the-azure-app-service"></a>Azure App Service에 웹 사이트 배포

이제 Contoso는 웹 사이트를 게시할 수 있습니다.


1. Visual Studio 2017에서 Application Insights 모니터링을 사용하도록 설정합니다. 이렇게 하려면 솔루션 탐색기에서 **PublicWeb** 프로젝트를 선택하고, **Application Insights 추가**를 검색합니다. 인프라를 배포할 때 생성된 Application Insight 인스턴스에 앱을 등록합니다.

    ![웹 사이트 게시](./media/contoso-migration-rebuild/deploy-website1.png)

2. Visual Studio에서 PublicWeb 프로젝트를 App Insights에 연결하고, 어떻게 구성되었는지 표시하도록 업데이트합니다.
 
    ![웹 사이트 게시](./media/contoso-migration-rebuild/deploy-website2.png)

3. Visual Studio에서 웹앱을 만들고 게시합니다.

    ![웹 사이트 게시](./media/contoso-migration-rebuild/deploy-website3.png)

5. 앱 이름을 지정하고, 기본 지역인 미국 동부 2의 프로덕션 리소스 그룹 **ContosoRG**에 배치합니다.

    ![웹 사이트 게시](./media/contoso-migration-rebuild/deploy-website4.png)

### <a name="deploy-the-function-to-azure"></a>Azure에 함수 배포

1. Visual Studio를 사용하여 함수를 만들고 게시합니다. 이렇게 하려면 **PetCheckerFunction** > **게시**를 마우스 오른쪽 단추로 클릭합니다. 그리고 새 App Service 함수를 만듭니다.

     ![함수 배포](./media/contoso-migration-rebuild/function1.png)

2. 이름을 **smarthotelpetchecker**로 지정하고, 리소스 그룹 ContosoRG 및 새 App Service 계획에 배치합니다.

     ![함수 배포](./media/contoso-migration-rebuild/function2.png)

3. 저장소 계정을 선택하고 함수를 만듭니다.

    ![함수 배포](./media/contoso-migration-rebuild/function3.png)

4. 배포가 시작되고 함수 앱이 Azure에 프로비전됩니다. **게시**에서, Contoso는 저장소, Cosmos 데이터베이스 및 Computer Vision API에 대한 앱 설정을 추가합니다.

    ![함수 배포](./media/contoso-migration-rebuild/function4.png)

5. 함수를 먼저 로컬로 실행하기 위해 **PetCheckerFunction/local.settings.json**에서 설정을 업데이트합니다.

    ![함수 배포](./media/contoso-migration-rebuild/function5.png)

6. 배포된 함수는 Azure Portal에 **실행 중** 상태로 표시됩니다.
...
- 데이터베이스에 지역별 장애 조치(failover)를 제공하는 SQL 장애 조치(failover) 그룹을 구현하는 방안을 고려해야 합니다. [자세히 알아보기](https://docs.microsoft.com/azure/sql-database/sql-database-geo-replication-overview).
