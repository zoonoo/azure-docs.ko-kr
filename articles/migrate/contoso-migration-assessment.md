...
이 문서에서는 Contoso에서 앱을 Azure로 마이그레이션하기 위해 준비하는 온-프레미스 SmartHotel 앱을 평가합니다.

이 문서는 가상의 회사인 Contoso에서 온-프레미스 리소스를 Microsoft Azure 클라우드로 마이그레이션하는 방법을 설명하는 문서 시리즈의 3부입니다. 이 시리즈에는 배경 정보와 마이그레이션 인프라를 설정하고, 마이그레이션에 대한 온-프레미스 리소스의 적합성을 평가하고, 다양한 유형의 마이그레이션을 실행하는 방법을 설명하는 일련의 배포 시나리오가 포함되어 있습니다. 시나리오가 복잡해지고 있으며, 시간이 지남에 따라 관련 문서가 시리즈에 추가됩니다.
...
[문서 11: Visual Studio Team Services에서 Team Foundation Server 리팩터링](contoso-migration-tfs-vsts.md) | Contoso가 Azure에서 Visual Studio Team Services로 마이그레이션하여 온-프레미스 Team Foundation Server 배포를 마이그레이션합니다. | 사용 가능
