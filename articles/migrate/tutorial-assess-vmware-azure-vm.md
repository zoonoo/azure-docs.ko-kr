---
title: Azure Migrate에서 서버 평가를 사용하여 Azure VM으로 마이그레이션할 VMware VM 평가
description: 서버 평가를 사용하여 Azure VM으로 마이그레이션할 VMware VM을 평가하는 방법을 알아봅니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
ms.locfileid: "96753147"
---
# <a name="tutorial-assess-vmware-vms-for-migration-to-azure-vms"></a>자습서: Azure VM으로 마이그레이션할 VMware VM 평가

Azure로 마이그레이션하는 과정의 일환으로 온-프레미스 워크로드를 평가하여 클라우드 준비 상태를 측정하고, 위험을 식별하며, 비용과 복잡성을 예측합니다.

이 문서에서는 Azure VM으로 마이그레이션하기 위한 준비 작업으로 Azure Migrate: Server Assessment 도구를 사용하여 검색된 VMware VM(가상 머신)을 평가하는 방법을 보여줍니다.
...
이 자습서에 따라 Azure VM으로 마이그레이션할 머신을 평가하기 전에 평가할 머신을 검색했는지 확인합니다.

- Azure Migrate 어플라이언스를 사용하여 머신을 검색하려면 [이 자습서를 수행](tutorial-discover-vmware.md)합니다. 
...
1. **평가 속성** > **대상 속성** 에서 다음을 수행합니다.
 ...
 7. **VM 크기** 에서 다음을 수행합니다.
    ...
    - **쾌적 인자** 에서 평가 중에 사용할 버퍼를 표시합니다. 이는 계절별 사용량, 짧은 성능 기록 및 향후 사용량 증가와 같은 문제를 고려합니다. 예를 들어 두 개의 쾌적 인자를 사용하는 경우: **구성 요소** | **효과적인 활용률** | **쾌적 인자(2.0) 추가** 코어 | 2 | 4 메모리 | 8GB | 16GB     
   
8. **가격 책정** 에서 다음을 수행합니다.
...
9. 변경하는 경우 **저장** 을 클릭합니다.

    ![평가 속성](./media/tutorial-assess-vmware-azure-vm/assessment-properties.png)

10. **서버 평가** 에서 **다음** 을 클릭합니다.
11. **평가할 머신** 에서 **새로 만들기** 를 선택하고 그룹 이름을 지정합니다. 
12. 어플라이언스를 선택하고 그룹에 추가할 VM을 선택합니다. 그런 다음 **다음** 을 클릭합니다.

     ![그룹에 VM 추가](./media/tutorial-assess-vmware-azure-vm/assess-group.png)

13. **검토 + 평가 만들기에서 평가 세부 정보를 검토하고 **평가 만들기** 를 클릭하여 그룹을 만들고 평가를 실행합니다.


    > [!NOTE]
    > 성능 기반 평가의 경우 평가를 만들기 전에 검색을 시작한 후 최소 하루를 기다리는 것이 좋습니다. 이렇게 하면 성능 데이터를 더 높은 신뢰도로 수집할 수 있는 시간이 제공됩니다. 검색을 시작한 후 높은 신뢰도 등급을 위해 지정한 성능 기간(일/주/월)을 기다리는 것이 좋습니다.
...
2. **평가** 에서 평가를 선택하여 엽니다. 예를 들면(예제에만 해당되는 추정 및 비용) 다음과 같습니다. 
    ...
    ![평가 요약](./media/tutorial-assess-vmware-azure-vm/assessment-summary.png)
...
![신뢰 등급](./media/tutorial-assess-vmware-azure-vm/confidence-rating.png)

신뢰 등급은 평가에서 크기 권장 사항의 안정성을 추정하는 데 도움이 됩니다. 이 등급은 평가를 컴퓨팅하는 데 필요한 데이터 요소의 가용성을 기반으로 합니다.
