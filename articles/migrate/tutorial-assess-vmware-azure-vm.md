---
title: Azure Migrate에서 Azure VM으로 마이그레이션할 VMware 서버 평가
description: Azure Migrate를 사용하여 Azure VM으로 마이그레이션할 VMware 서버를 평가하는 방법을 알아봅니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
이 문서에서는 Azure Migrate: 검색 및 평가 도구를 사용하여 Azure VM으로 마이그레이션하기 위한 준비 작업으로 VMware 환경에서 검색된 서버를 평가하는 방법을 보여줍니다.
...
이 자습서에 따라 Azure VM으로 마이그레이션할 서버를 평가하기 전에 평가할 서버를 검색했는지 확인합니다.
...
2. **Azure Migrate: 검색 및 평가** 에서 **평가** 를 클릭하고 **Azure VM** 을 선택합니다.
...
1. **Windows, Linux 및 SQL Server** > **Azure Migrate: 검색 및 평가** 에서 **Azure VM 평가** 옆에 있는 숫자를 클릭합니다.
