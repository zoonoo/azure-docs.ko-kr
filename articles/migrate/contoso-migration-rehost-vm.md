...
description: Azure Site Recovery 서비스를 사용하여 온-프레미스 머신을 마이그레이션하기 위해 리프트 앤 시프트 마이그레이션을 사용하여 Azure에 온-프레미스 앱을 다시 호스트하는 방법에 대해 알아보세요.
...
- **위험 제한**: SmartHotel 앱은 Contoso 비즈니스에 매우 중요합니다. Contoso는 이 앱을 안전하게 Azure로 이동하려고 합니다.
- **확장**: Contoso는 앱을 수정하지 않고 안정적으로 사용하기를 원합니다.
...
- 첫 번째 단계로 Site Recovery를 위한 Azure 구성 요소를 준비하고 설정하며 온-프레미스 VMware 인프라를 준비합니다.
- [Azure 인프라](contoso-migration-infrastructure.md)를 이미 갖추고 있으므로 Site Recovery를 위한 몇 개의 Azure 구성 요소만 추가하면 됩니다.
...
> * **3단계: VM 복제**: 복제를 설정하고 VM을 Azure 저장소로 복제하기 시작합니다.
...
1. Contoso는 [Azure 인프라를 배포](contoso-migration-infrastructure.md)할 때 Site Recovery에 사용할 수 있는 네트워크를 이미 설정했습니다.

    - SmartHotel 앱은 프로덕션 앱이며 VM은 주 지역인 미국 동부 2의 Azure 프로덕션 네트워크(VNET-PROD-EUS2)로 마이그레이션됩니다.
...
3. 네트워크 및 저장소 계정이 준비되면 Contoso는 이제 Recovery Services 자격 증명 모음(ContosoMigrationVault)을 만들어 주 지역인 미국 동부 2의 ContosoFailoverRG 리소스 그룹에 배치합니다.
...
모든 준비가 완료되면 Contoso는 VM에 대해 복제를 사용하도록 설정할 수 있습니다. WebVM부터 시작합니다.
...
4. Contoso는 복제를 위한 **WebVM**을 선택하고, 복제 정책을 확인하고, 복제를 사용하도록 설정합니다.

    - 이 단계에서는 VNet 및 서브넷이 선택되어 있어야 하므로 Contoso가 WEBVM만 선택하며 Contoso는 앱 VM을 다른 서브넷에 배치합니다.
...
이제 Contoso는 위와 동일한 프로세스를 사용하여 SQLVM 머신 복제를 시작할 수 있습니다.
...
Contoso는 빠른 테스트 장애 조치(failover)를 실행한 다음, 전체 장애 조치(failover)를 실행하여 VM을 마이그레이션합니다.
...
이제 Contoso가 전체 장애 조치(failover)를 실행하여 마이그레이션을 완료할 수 있습니다.
...
마이그레이션이 완료되면 SmartHotel 앱 계층이 Azure VM에서 실행됩니다.
...
Contoso는 Azure Backup 서비스를 사용하여 VM의 데이터를 백업하려고 합니다. [자세히 알아보기](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).

### <a name="licensing-and-cost-optimization"></a>라이선스 및 비용 최적화

1. Contoso는 해당 VM에 대한 기존 라이선스가 있으나 Azure 하이브리드 혜택을 활용하려고 합니다.  이 가격 책정을 활용하도록 기존 Azure VM을 변환할 것입니다.
...
이 문서에서 Contoso는 Site Recovery 서비스를 사용하여 앱 VM을 Azure VM으로 마이그레이션함으로써 Azure의 SmartHotel 앱을 다시 호스트했습니다. 
