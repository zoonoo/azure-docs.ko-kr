---
title: Azure Migrate 서버 평가를 사용하여 AWS 인스턴스 검색
description: Azure Migrate 서버 평가를 사용하여 AWS 인스턴스를 검색하는 방법을 알아봅니다.
ms.topic: tutorial
ms.date: 09/14/2020
ms.custom: mvc
...
이 자습서에서는 Azure Migrate를 사용하여 AWS(Amazon Web Services) 인스턴스를 검색하는 방법을 보여줍니다. 간단한 Azure Migrate 어플라이언스를 사용하는 서버 평가 도구입니다. 어플라이언스를 물리적 서버로 배포하여 머신 및 성능 메타데이터를 지속적으로 검색합니다.
...
8. **사용자 설정**에서 Azure AD 사용자가 애플리케이션을 등록할 수 있는지 확인합니다(기본적으로 **예**로 설정됨).
    ...
    ![사용자 설정에서 사용자가 Active Directory 앱을 등록할 수 있는지 확인합니다.](./media/tutorial-discover-aws/register-apps.png)
