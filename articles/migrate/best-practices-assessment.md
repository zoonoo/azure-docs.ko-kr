...
Azure Migrate 어플라이언스는 온-프레미스 환경을 지속적으로 프로 파일링 하 고 메타 데이터 및 성능 데이터를 Azure로 전송 합니다. 평가를 만드는 다음 모범 사례를 따르세요.

- 있는 **그대로 만들기 평가**: 컴퓨터가 Azure Migrate 포털에 표시 되 면 즉시 평가를 만들 수 있습니다.
- **성능 기반 평가를 만듭니다**. 검색을 설정한 후에는 성능 기반 평가를 실행 하기 전에 적어도 하루 이상 기다려야 하는 것이 좋습니다.
...
- **평가 다시 계산**: 평가는 특정 시점 스냅숏 이므로 최신 데이터로 자동으로 업데이트 되지 않습니다. 최신 데이터로 평가를 업데이트 하려면 다시 계산 해야 합니다.
