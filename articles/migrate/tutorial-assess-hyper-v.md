---
title: Azure Migrate에서 서버 평가를 사용하여 Azure VM으로 마이그레이션할 Hyper-V VM 평가
description: 서버 평가를 사용하여 Azure VM으로 마이그레이션할 Hyper-V VM을 평가하는 방법을 알아봅니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
ms.locfileid: "98567085"
---
# <a name="tutorial-assess-hyper-v-vms-for-migration-to-azure"></a>자습서: Azure로 마이그레이션할 Hyper-V VM 평가

Azure로 마이그레이션하는 과정의 일환으로 온-프레미스 워크로드를 평가하여 클라우드 준비 상태를 측정하고, 위험을 식별하고, 비용과 복잡성을 예측합니다.

이 문서에서는 Azure Migrate를 사용하여 Azure로 마이그레이션하기 위해 검색된 Hyper-V VM(가상 머신)을 평가하는 방법을 보여줍니다. 서버 평가 도구를 사용하여 온-프레미스 VMware VM을 평가하는 방법을 보여 줍니다.
...
- 이 자습서에 따라 Azure VM으로 마이그레이션할 머신을 평가하기 전에 평가할 머신을 검색했는지 확인합니다.
    - Azure Migrate 어플라이언스를 사용하여 머신을 검색하려면 [이 자습서를 수행](tutorial-discover-hyper-v.md)합니다. 
...
1. **그룹 선택 또는 만들기** 에서 **새로 만들기** 를 선택하고 그룹 이름을 지정합니다. 
    ...
    :::image type="content" source="./media/tutorial-assess-hyper-v/assess-machines.png" alt-text="새 그룹 생성 및 머신 추가":::
