---
title: Azure Migrate를 사용하여 Azure VM으로 마이그레이션할 Hyper-V VM 평가
description: Azure Migrate를 사용하여 Azure VM으로 마이그레이션할 Hyper-V VM을 평가하는 방법을 알아봅니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
이 문서에서는 Azure Migrate: 검색 및 평가 도구를 사용하여 Azure로 마이그레이션할 Hyper-V 환경에서 검색된 서버를 평가하는 방법을 보여줍니다.
...
- 이 자습서에 따라 Azure VM으로 마이그레이션할 서버를 평가하기 전에 평가할 서버를 검색했는지 확인합니다.
    - Azure Migrate 어플라이언스를 사용하여 서버를 검색하려면 [이 자습서를 수행](tutorial-discover-hyper-v.md)합니다. 
