...
이 문서는 가상 회사 Contoso가 온-프레미스 리소스를 Microsoft Azure 클라우드로 마이그레이션하는 방법을 설명하는 문서 시리즈의 일곱 번째 문서입니다. 이 문서 시리즈에는 배경 정보와 마이그레이션 인프라를 설정하고 다양한 유형의 마이그레이션을 실행하는 방법을 설명하는 일련의 시나리오가 담겨 있습니다. 시나리오가 점점 더 복잡해지므로 점차 문서를 추가할 예정입니다.
...
문서 7: Linux 앱을 Azure VM에 다시 호스트(본 문서) | Contoso가 Azure Site Recovery를 사용하여 osService Linux 앱을 마이그레이션하는 방법을 보여 줍니다.
...
Contoso는 Azure로 장애 조치(failover) 이후 Azure에 있는 복제된 VM에 연결하고 싶어 합니다. 이 기능을 사용하려면 다음과 같은 몇 가지 작업을 수행해야 합니다.
...
3. 장애 조치(failover)가 완료되면 Azure Portal에 Azure VM 복제본이 표시됩니다. VM의 크기가 적당하고, VM이 올바른 네트워크에 연결되어 있으며 실행 중인지 확인합니다. 
...
3. 장애 조치(failover) 후 Contoso는 Azure Portal에 Azure VM이 예상대로 표시되는지 확인합니다.
