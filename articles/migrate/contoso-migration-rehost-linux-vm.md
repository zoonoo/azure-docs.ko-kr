...
이 문서는 가상의 회사인 Contoso에서 온-프레미스 리소스를 Microsoft Azure 클라우드로 마이그레이션하는 방법을 설명하는 문서 시리즈 중 하나입니다. 이 문서 시리즈에는 배경 정보와 마이그레이션 인프라를 설정하고 다양한 유형의 마이그레이션을 실행하는 방법을 설명하는 일련의 시나리오가 담겨 있습니다. 시나리오가 점점 더 복잡해지고 시간이 지남에 따라 문서도 계속 추가될 것입니다.
...
[문서 5: Azure VM에 다시 호스팅](contoso-migration-rehost-vm.md) | Contoso에서 SmartHotel 앱 VM을 Azure VM으로 마이그레이션하는 방법을 보여줍니다. | 사용 가능
...
문서 7: Linux 앱을 Azure VM에 다시 호스트 | Contoso가 Azure Site Recovery를 사용하여 osService Linux 앱을 마이그레이션하는 방법을 보여 줍니다. | 이 문서의 내용:
[문서 8: Azure VM 및 Azure MySQL 서버에 Linux 앱 다시 호스팅](contoso-migration-rehost-linux-vm-mysql.md) | Contoso가 Site Recovery를 통해 VM을 마이그레이션하고 MySQL Workbench를 통해 Azure MySQL 서버 인스턴스로 마이그레이션하여 osService Linux 앱을 마이그레이션하는 방법을 보여 줍니다. | 사용 가능
...
- Contoso는 이 앱의 운영 모델을 변경하지 않고 지금과 마찬가지로 클라우드에서 상호 작용하기를 원합니다.
...
2. Azure와 온-프레미스 구성 요소를 준비한 후 VM에 대해 복제를 설정하고 사용하도록 설정합니다.
3. 복제가 작동하면 Azure로 장애 조치(failover)하여 VM을 마이그레이션합니다.
...
사용자(및 Contoso)가 이 시나리오를 실행하는 데 필요한 사항은 다음과 같습니다.
...
**온-프레미스 서버** | 온-프레미스 vCenter Server에서 5.5, 6.0 또는 6.5 버전을 실행해야 합니다.<br/><br/> 5.5, 6.0 또는 6.5 버전을 실행하는 ESXi 호스트<br/><br/> ESXi 호스트에서 실행되는 하나 이상의 VMware VM
...
> * **2단계: Site Recovery를 위한 온-프레미스 VMware 준비**: VM 검색 및 에이전트 설치에 사용할 계정을 준비하고, 장애 조치(failover) 후 Azure VM에 연결하도록 준비합니다.
> * **3단계: VM 복제**: 원본 및 대상 마이그레이션 환경을 설정하고, 복제 정책을 만들고, Azure Storage로 VM 복제를 시작합니다.
> * **4단계: Site Recovery를 사용하여 VM 마이그레이션**: 테스트 장애 조치(failover)를 실행하여 모든 요소가 작동하는지 확인하고 전체 장애 조치(failover)를 실행하여 VM을 Azure로 마이그레이션합니다.
...
1. Contoso는 미국 동부 2 지역에 Azure Storage 계정(contosovmsacc20180528)을 만듭니다.
    ...
    - 표준 저장소와 LRS 복제가 포함된 범용 계정을 사용합니다.

    ![Site Recovery 저장소](./media/contoso-migration-rehost-linux-vm/asr-storage.png)

2. 네트워크 및 저장소 계정이 준비되면 Contoso는 자격 증명 모음(ContosoMigrationVault)을 만들어 주 지역인 미국 동부 2 지역의 **ContosoFailoverRG** 리소스 그룹에 배치합니다.
...
Contoso가 마이그레이션하는 Linux VM에 모바일 서비스가 설치되어 있어야 합니다.
...
Azure로 장애 조치되면 Contoso는 Azure에서 복제된 VM에 연결하려고 합니다. 이 기능을 사용하려면 다음과 같은 몇 가지 작업을 수행해야 합니다.
...
- 마이그레이션(장애 조치(failover))을 실행한 후, **부트 진단**을 검사하여 VM 스크린샷을 확인해야 합니다.
...
Contoso는 원본 환경을 구성해야 합니다. 이 작업을 위해 OVF 템플릿을 다운로드하여 Site Recovery 구성 서버를 고가용성 온-프레미스 VMware VM으로 배포하는 데 사용합니다. 구성 서버가 작동하여 실행되면 자격 증명 모음에 등록합니다.
...
3. Contoso는 복제를 위해 **OSTICKETWEB**을 선택합니다. 
    ...
    - VNet 및 서브넷을 선택해야 하고 VM이 동일한 서브넷에 있지 않기 때문에 이 단계에서 Contoso는 **OSTICKETWEB**만 선택합니다.
...
이제 Contoso가 **OSTICKETMYSQL**의 복제를 시작할 수 있습니다.

1. **응용 프로그램 복제** > **원본** > **+복제**에서 원본 및 대상 설정을 선택합니다.
2. Contoso는 복제를 위해 **OSTICKETMYSQL**을 선택하고 모바일 서비스 설치에 사용할 계정을 선택합니다.

    ![복제 사용](./media/contoso-migration-rehost-linux-vm/mysql-enable.png)

3. Contoso는 OSTICKETWEB에 사용된 것과 동일한 복제 정책을 적용하고 복제를 사용하도록 설정합니다.  
...
Contoso는 빠른 테스트 장애 조치(failover)를 실행한 다음, VM을 마이그레이션합니다.
...
### <a name="create-and-customize-a-recovery-plan"></a>복구 계획 만들기 및 사용자 지정
 ...
 Contoso는 테스트 장애 조치(failover)가 예상대로 작동했는지 확인한 후 마이그레이션을 위한 복구 계획을 만듭니다. 
...
3.  **그룹 1: 시작**에서 **OSTICKETWEB**을 제거합니다.  이렇게 하면 첫 번째 시작 작업이 **OSTICKETMYSQL**에만 영향을 줍니다.

    ![복구 그룹](./media/contoso-migration-rehost-linux-vm/recovery-group1.png)

4.  **+그룹** > **보호된 항목 추가**에서 **그룹 2: 시작**에 **OSTICKETWEB**을 추가합니다.  Contoso는 두 개의 서로 다른 그룹에서 이러한 작업이 필요합니다.
...
Contoso에서 복구 계획의 장애 조치(failover)를 실행하고 VM을 마이그레이션할 준비가 되었습니다.
...
3. 장애 조치되면 Contoso는 Azure Portal에 Azure VM이 예상대로 표시되는지 확인합니다.
...
마이그레이션 프로세스의 최종 단계로, Contoso는 **OSTICKETMYSQL** VM에서 실행 중인 앱 데이터베이스를 가리키도록 응용 프로그램의 연결 문자열을 업데이트합니다. 
...
이제 Contoso는 다음과 같은 몇 가지 정리 작업을 수행해야 합니다.  

- vCenter 인벤토리에서 온-프레미스 VM을 제거합니다.
- 로컬 백업 작업에서 온-프레미스 VM을 제거합니다.
- 내부 문서를 업데이트하여 새 위치와 OSTICKETWEB 및 OSTICKETMYSQL의 IP 주소를 표시합니다.
- VM과 상호 작용하는 모든 리소스를 검토하고 모든 관련 설정이나 문서를 업데이트하여 새 구성을 반영합니다.
- Contoso는 Azure Migrate 서비스를 종속성 매핑과 함께 사용하여 마이그레이션할 VM을 평가했습니다. 이 용도로 설치한 Microsoft Monitoring Agent 및 Dependency Agent를 VM에서 제거해야 합니다.
...
- 또한 디스크 암호화 및 Azure KeyVault를 사용하여 VM 디스크의 데이터 보안을 유지하는 것도 고려하고 있습니다.
