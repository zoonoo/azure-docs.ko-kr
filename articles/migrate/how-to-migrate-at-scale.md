...
ms.locfileid: "67808016"
---
# <a name="scale-migration-of-vms-using-azure-site-recovery"></a>Azure Site Recovery를 사용하여 VM 마이그레이션의 크기 조정

이 문서에서는 많은 수의 Azure Site Recovery를 사용 하 여 Vm 마이그레이션 스크립트를 사용 하는 방법을 이해 하도록 도와줍니다. 이러한 스크립트는 GitHub의 [Azure PowerShell 샘플](https://github.com/Azure/azure-docs-powershell-samples/tree/master/azure-migrate/migrate-at-scale-with-site-recovery) 리포지토리에서 다운로드할 수 있습니다. VMware, AWS, GCP Vm 및 물리적 서버에서 Azure managed disks로 마이그레이션 스크립트를 사용할 수 있습니다. VM을 물리적 서버로 마이그레이션하는 경우 이 스크립트를 사용하여 Hyper-V VM을 마이그레이션할 수도 있습니다. Azure Site Recovery PowerShell을 활용 하는 스크립트가 나와 [여기](https://docs.microsoft.com/azure/site-recovery/vmware-azure-disaster-recovery-powershell)합니다.
...
9 | asr_postmigration.ps1 | 장애 조치(failover) 후 NIC에 네트워크 보안 그룹을 할당하려는 경우 이 스크립트를 사용하면 됩니다. 대상 VM에서에서 NIC에 NSG를 할당합니다.
