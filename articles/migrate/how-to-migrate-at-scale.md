...
모든 필수 사항을 완료했으면 마이그레이션하려는 각 원본 컴퓨터에 대한 데이터가 있는 CSV 파일을 만들어야 합니다. 입력 CSV에는 입력 세부 정보가 있는 헤더 행과 마이그레이션해야 하는 각 시스템의 세부 정보가 있는 행이 있어야 합니다. 모든 스크립트에서 동일한 CSV 파일을 사용하도록 설계되었습니다. 참조용 샘플 CSV 템플릿이 스크립트 폴더에서 제공됩니다. 
