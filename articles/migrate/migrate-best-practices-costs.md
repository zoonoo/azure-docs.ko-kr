...
온-프레미스 스토리지(SAN 또는 NAS) 및 이를 지원하는 네트워크를 튜닝하고 유지 관리하는 데 많은 비용과 시간이 소요될 수 있습니다. 파일(스토리지) 데이터는 일반적으로 운영 및 관리 문제를 완화하기 위해 클라우드로 마이그레이션됩니다. Microsoft는 데이터를 Azure로 이동하기 위한 몇 가지 옵션을 제공하며, 사용자는 이러한 옵션을 결정해야 합니다. 데이터에 적합한 스토리지 유형을 선택하면 조직에서 매월 수천 달러의 비용을 절약할 수 있습니다. 몇 가지 고려할 사항은 다음과 같습니다. 
