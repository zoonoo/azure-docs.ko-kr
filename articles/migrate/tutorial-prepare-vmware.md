...
- **평가 계정 설정**. 평가를 위해 VM을 검색하려면 Azure Migrate에서 vCenter Server에 액세스해야 합니다. Azure Migrate 액세스를 위한 읽기 전용 계정이 필요합니다.
...
평가 및 에이전트 없는 마이그레이션을 위해 VM을 검색하려면 Azure Migrate에서 vCenter Server에 액세스해야 합니다. 평가만 하는 경우에는 vCenter Server에 대한 읽기 전용 계정을 설정합니다.
