---
title: Azure Migrate 서버 평가를 사용하여 Azure로 마이그레이션할 GCP VM 인스턴스 평가
description: Azure Migrate 서버 평가를 사용하여 Azure로 마이그레이션할 GCP VM 인스턴스를 평가하는 방법을 설명합니다.
ms.topic: tutorial
ms.date: 09/14/2020
ms.custom: MVC
...
ms.locfileid: "91716442"
---
# <a name="tutorial-assess-google-cloud-platform-gcp-vm-instances-for-migration-to-azure"></a>자습서: Azure로 마이그레이션할 GCP(Google Cloud Platform) VM 인스턴스 평가

Azure로 마이그레이션하는 과정의 일환으로 온-프레미스 워크로드를 평가하여 클라우드 준비 상태를 확인하고, 위험을 식별하고, 비용과 복잡성을 예측합니다.

이 문서에서는 Azure Migrate: 서버 평가 도구를 사용하여 Azure로 마이그레이션할 GCP(Google Cloud Platform) VM 인스턴스를 평가하는 방법을 보여줍니다.
...
- 이 자습서의 단계를 수행하기 전에 이 시리즈의 첫 번째 자습서를 완료하여 [온-프레미스 인벤토리](tutorial-discover-gcp.md)를 검색합니다. 


## <a name="decide-which-assessment-to-run"></a>실행할 평가 결정

온-프레미스에서 있는 그대로 수집된 머신 구성 데이터/메타데이터를 기반으로 하는 크기 조정 기준을 사용하여 평가를 실행할지 또는 성능 데이터를 기반으로 평가를 실행할지를 결정합니다.
...
**성능 기반** | 수집된 성능 데이터를 기반으로 평가합니다. | 권장 Azure VM 크기는 CPU 및 메모리 사용률 데이터를 기반으로 합니다.<br/><br/> 권장 디스크 유형은 온-프레미스 디스크의 IOPS 및 처리량을 기반으로 합니다.
...
1. **서버** 페이지 > **Windows 및 Linux 서버**에서 **서버 평가 및 마이그레이션**을 클릭합니다.
   ...
   ![서버 평가 및 마이그레이션 단추의 위치](./media/tutorial-assess-gcp/assess.png)

2. **Azure Migrate: 서버 평가에서 **평가**를 클릭합니다.

    ![평가 단추의 위치](./media/tutorial-assess-gcp/assess-servers.png)
...
6. 평가 속성을 검토하려면 **모두 보기**를 클릭합니다.
    ...
    ![평가 속성을 검토하는 모두 보기 단추의 위치](./media/tutorial-assess-gcp/assessment-name.png)
...
10. 변경하는 경우 **저장**을 클릭합니다.
    ...
    ![평가 속성](./media/tutorial-assess-gcp/assessment-properties.png)
...
2. **평가**에서 평가를 선택하여 엽니다. 예를 들면(예제에만 해당되는 추정 및 비용) 다음과 같습니다. 
    ...
    ![평가 요약](./media/tutorial-assess-gcp/assessment-summary.png)
...
![신뢰 등급](./media/tutorial-assess-gcp/confidence-rating.png)
