---
title: Azure Migrate를 사용하여 Azure로 마이그레이션할 GCP VM 인스턴스 평가
description: Azure Migrate를 사용하여 Azure로 마이그레이션할 GCP VM 인스턴스를 평가하는 방법을 설명합니다.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
이 문서에서는 Azure Migrate: 검색 및 평가 도구를 사용하여 Azure로 마이그레이션할 GCP(Google Cloud Platform) VM 인스턴스를 평가하는 방법을 보여줍니다.
...
온-프레미스에서 있는 그대로 수집된 서버 구성 데이터/메타데이터를 기반으로 하는 크기 조정 기준을 사용하여 평가를 실행할지 또는 성능 데이터를 기반으로 평가를 실행할지를 결정합니다.
