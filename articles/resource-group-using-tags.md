...
태그는 리소스 및 리소스 그룹에 직접 존재합니다. 기존 태그를 확인하려면 **Get-AzureRmResource** 또는 **Get-AzureRmResourceGroup**을 사용하여 리소스 또는 리소스 그룹을 가져오면 됩니다. 리소스 그룹부터 살펴보겠습니다.
...
태그는 리소스 및 리소스 그룹에 직접 존재합니다. 기존 태그를 확인하려면 **azure group show**를 사용하여 리소스 그룹과 해당 리소스를 가져오면 됩니다.
...
다음 예제에서는 태그 이름 및 값이 있는 모든 리소스를 검색하는 방법을 보여 줍니다.
...
태그는 전체적으로 업데이트됩니다. 기존 태그가 있는 리소스에 태그 하나를 추가하려면 유지하려는 모든 기존 태그를 검색합니다. 리소스 그룹에 대한 태그 값을 설정하려면 **azure group set**를 사용하여 리소스 그룹에 모든 태그를 제공합니다.
...
**azure tag list**를 사용하여 구독에 있는 기존 태그를 나열하고 **azure tag create**를 사용하여 태그를 추가할 수 있습니다. 구독에 대한 분류에서 태그를 제거하려면 먼저 모든 리소스에서 태그를 제거합니다. 그런 다음 **azure tag delete**를 사용하여 태그를 제거합니다.
