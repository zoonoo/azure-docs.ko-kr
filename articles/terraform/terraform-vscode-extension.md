---
title: Azure Terraform VS Code 확장 | Microsoft Docs
description: 이 문서에서는 Visual Studio Code에서 Terraform 확장을 설치하고 사용하는 방법을 알아봅니다.
keywords: terraform, devops, 가상 머신, azure
author: v-mavick
ms.author: v-mavick
ms.date: 08/14/2018
ms.topic: article
...
ms.locfileid: "40190996"
---
# <a name="azure-terraform-vs-code-extension"></a>Azure Terraform VS Code 확장

Microsoft Azure Terraform VS Code(Visual Studio Code) 확장은 Azure에서 Terraform을 작성, 테스트 및 사용하는 동안 개발자의 생산성을 향상시키도록 디자인되었습니다. 이 확장은 VS Code 내에서 Terraform 명령 지원, 리소스 그래프 시각화 및 Cloud Shell 통합을 제공합니다.

## <a name="what-you-do"></a>수행할 작업

- 컴퓨터에 오픈 소스 HashiCorp Terraform 실행 파일을 설치합니다.
- 로컬 VS Code 설치에 Azure용 Terraform VS Code 확장을 설치합니다.

## <a name="what-you-learn"></a>학습 내용

이 자습서에서는 다음에 대해 알아봅니다.

- Terraform이 Azure 서비스 프로비전을 자동화하고 간소화하는 방법
- Azure 서비스에 대해 Microsoft Terraform VS Code 확장을 설치하고 사용하는 방법.
- VS 코드를 사용하여 Terraform 계획을 작성, 계획 및 실행하는 방법
...
- 활성 Azure 구독. [30일 평가판 Microsoft Azure 계정](https://azure.microsoft.com/free/) 활성화
- 로컬 컴퓨터의 [Terraform](https://www.terraform.io/) 오픈 소스 도구 설치
...
### <a name="install-the-azure-terraform-vs-code-extension"></a>Azure Terraform VS Code 확장 설치
...
3. **Search Extensions in Marketplace**(Marketplace에서 확장 검색) 텍스트 상자를 사용하여 Azure Terraform 확장을 검색합니다.

    ![Marketplace에서 VS Code 확장 검색](media/terraform-vscode-extension/tf-search-extensions.png)

4. **Install**을 클릭합니다.

    >[!NOTE]
    >Azure Terraform 확장에 대해 **설치**를 클릭하면 VS Code에서 해당 Azure 계정 확장을 자동으로 설치합니다. Azure 계정은 Azure 구독 인증 및 Azure 관련 코드 확장을 수행하기 위해 Azure Terraform 확장이 사용하는 종속성 파일입니다.

#### <a name="verify-the-terraform-extension-is-installed-in-visual-studio-code"></a>Visual Studio Code에서 Terraform 확장이 설치되었는지 확인

1. 확장 아이콘을 클릭합니다.
2. 검색 텍스트 상자에 `@installed`를 입력합니다.
...
이제 VS Code 내에서 Cloud Shell 환경의 지원되는 모든 Terraform 명령을 실행할 수 있습니다.
...
1. VS Code의 메뉴 모음에서 **파일 > 새 파일**을 선택합니다.
2. [azurerm_resource_group](https://www.terraform.io/docs/providers/azurerm/r/resource_group.html#)으로 이동한 후 **Example Usage** 코드 블록의 코드를 복사합니다.
3. 복사한 코드를 VS Code에서 만든 새 파일에 붙여 넣습니다.
...
4. 메뉴 모음에서 **파일 > 다른 이름으로 저장**을 선택합니다.
5. **다른 이름으로 저장** 대화 상자에서 원하는 위치로 이동한 후 **새 폴더**를 클릭합니다. (새 폴더의 이름을 *새 폴더*보다 좀 더 자세한 설명을 포함하는 이름으로 변경합니다.)

    >[!NOTE]
    >이 예제에서는 폴더 이름이 TERRAFORM-TEST-PLAN으로 지정되었습니다.

6. 새 폴더가 강조 표시(선택)되어 있는지 확인한 후 **열기**를 클릭합니다.
7. **다른 이름으로 저장** 대화 상자에서 파일의 기본 이름을 *main.tf*로 변경합니다.
...
2. VS 코드 메뉴 모음에서 **파일 > 폴더 열기...** 를 선택하고 *main.tf* 파일을 찾아 선택합니다.

    ![main.tf 파일](media/terraform-vscode-extension/tf-main-tf.png)

3. 메뉴 모음에서 **보기 > 명령 팔레트... > Azure Terraform: Init**를 선택합니다.
4. 잠시 후에 *Do you want to open Cloud Shell?*(Cloud Shell을 열까요?)라는 메시지가 표시됩니다. **확인**을 클릭합니다.

    ![Do you want to open Cloud Shell?(Cloud Shell을 열까요?)](media/terraform-vscode-extension/tf-do-you-want-to-open-cloud-shell.png)

5. 새 폴더에서 Cloud Shell을 처음 시작하면 웹 응용 프로그램을 설정하라는 메시지가 표시됩니다. **열기**를 클릭합니다.

    ![Cloud Shell 처음 시작](media/terraform-vscode-extension/tf-first-launch-of-cloud-shell.png)

6. Azure Cloud Shell 시작 페이지가 열립니다. Bash 또는 PowerShell을 선택합니다.
...
7. 아직 Azure Storage 계정을 설정하지 않은 경우 다음 화면이 나타납니다. **저장소 만들기**를 클릭합니다.
...
9. 이제 Cloud Shell를 종료할 수 있습니다.
10. 메뉴 모음에서 **보기** > **명령 팔레트** > **Azure Terraform: init**를 선택합니다.
...
이 자습서의 앞부분에서 GraphViz를 설치했습니다. Terraform은 GraphViz를 사용하여 구성 또는 실행 계획의 시각적 표현을 생성할 수 있습니다. Azure Terraform VS Code 확장은 *visualize* 명령을 통해 이 기능을 구현합니다.

- **메뉴 모음**에서 **보기 > 명령 팔레트 > Azure Terraform: Visualize**를 선택합니다.
...
- 메뉴 모음에서 **보기** > **명령 팔레트** > **Azure Terraform: plan**를 선택합니다.
...
1. Azure Portal *리소스 그룹* 페이지에서 **새로 고침**을 클릭합니다.
2. 리소스 그룹이 더 이상 나열되지 않습니다.
...
이 연습에서는 Terraform *compute* 모듈을 VS Code 환경에 로드하는 방법을 알아봅니다.
...
### <a name="open-the-folder-in-vs-code"></a>VS Code에서 폴더 열기

1. Visual Studio Code를 시작합니다.
2. **메뉴 모음**에서 **파일 > 폴더 열기**를 선택하고 이전 단계에서 만든 폴더를 선택합니다.
...
VS Code 내에서 Terraform 명령을 사용하려면 먼저 두 Azure 공급자인 random 및 azurerm에 대한 플러그 인을 다운로드합니다.

1. VS 코드 IDE의 터미널 창에서 `terraform init`를 입력합니다.

    ![terraform init 명령](media/terraform-vscode-extension/tf-terraform-init-command.png)

2. `az login`을 입력하고 화면 지침을 따릅니다.

### <a name="module-test-lint"></a>모듈 테스트: *lint*

1. **메뉴 모음**에서 **보기 > 명령 팔레트 > Azure Terraform: Execute Test**를 선택합니다.
2. 테스트 형식 옵션 목록에서 **lint**를 선택합니다.

    ![테스트 유형 선택](media/terraform-vscode-extension/tf-select-type-of-test-lint.png)

3. *Do you want to open CloudShell?*(CloudShell를 열까요?)라는 메시지가 표시되면 **확인**을 클릭하고 화면 지침을 따릅니다.
...
1. **메뉴 모음**에서 **보기 > 명령 팔레트 > Azure Terraform: Execute Test**를 선택합니다.
2. 테스트 형식 옵션 목록에서 **end to end**를 선택합니다.
...
Terraform이 Visual Studio Code 내에서 Azure 서비스 프로비전을 단순화할 수 있는 몇 가지 방법을 살펴보았습니다. 이제 다음 이러한 리소스 중 일부를 검토할 수 있습니다.
- [Terraform Module Registry](https://registry.terraform.io/)는 Azure 및 기타 지원되는 공급자에 대해 사용 가능한 모든 Terraform 모듈을 표시합니다.

이러한 각 모듈에 대해 다음 정보가 제공됩니다.

- 모듈의 일반적인 기능 및 특징에 대한 설명
- 사용법 예제
- 로컬 개발자 컴퓨터에서 각 모듈을 빌드, 실행 및 테스트하는 방법을 보여주는 테스트 구성
- 모듈 개발 환경을 로컬로 빌드 및 실행할 수 있도록 하는 Dockerfile
