---
title: Azure 부서의 범위에서 SAP R3 브리지 제품군에 대 한 메타 데이터 추출 ABAP 함수 모듈을 배포 합니다.
...
2.  \"목록에서 옵션 패키지를 선택 \" 하 고 새 패키지의 이름 (예: Z \_ MITI)을 입력 한 다음 ' Display ' 단추를 누릅니다.

3.  ' 패키지 만들기 ' 창에서 ' 예 '를 누릅니다. 따라서 창 \" 패키지 작성기: 패키지 만들기 \" 가 열립니다. ' 간단한 설명 ' 필드에 값을 입력 하 고 \" 계속 \" 아이콘을 누릅니다.

4.  ' 로컬 워크 벤치 요청 확인 ' 창에서 ' 자체 요청 '을 누릅니다. ' 개발 ' 요청을 선택 합니다.

### <a name="create-a-function-group"></a>함수 그룹 만들기 

개체 탐색기의 \" 목록에서 함수 그룹을 선택 \" 하 고 아래 입력 필드에 해당 이름을 입력 합니다 (예: Z \_ MITI \_ fgroup). 보기 아이콘을 누릅니다.

1.  \"개체 만들기 \" 창에서 예를 눌러 새 함수 그룹을 만듭니다.

2.  짧은 텍스트 필드에 적절 한 설명을 지정 \" \" 하 고 단추 저장을 누릅니다 \" \" .

3.  이전 단계에서 패키지 만들기에서 준비한 패키지를 선택 \" \" 하 고 아이콘 저장을 클릭 \" \" 합니다.

4.  아이콘 계속을 눌러 요청을 \" 확인 \" 합니다.

5.  이 함수 그룹을 활성화 합니다.

### <a name="create-the-abap-function-module"></a>ABAP 함수 모듈 만들기 

함수 그룹을 만들고 선택한 후에는 리포지토리 브라우저에서 해당 이름을 마우스 오른쪽 단추로 클릭 하 고 \" 만들기 = \> 함수 모듈을 선택 \" 합니다.

\" \_ \_ \" 함수 모듈 필드에 Z MITI DOWNLOAD를 입력 \" \" 하 고 \" 적절 한 설명으로 짧은 텍스트 입력을 채웁니다 \" .

모듈을 만든 후에는 다음 정보를 지정 합니다.

1.  \"특성 탭으로 이동 \" 합니다.

2.  처리 유형 = Remote-Enabled 함수 모듈을 선택 합니다.

    :::image type="content" source="media/abap-functions-deployment-guide/processing-type.png" alt-text="원본 등록 옵션" border="true":::
...
4.  가져오기 탭으로 이동 하 \" \" 고 다음 매개 변수를 만듭니다.
    ...
    :::image type="content" source="media/abap-functions-deployment-guide/import.png" alt-text="원본 등록 옵션" border="true":::

5.  "테이블" 탭으로 이동 하 여 다음을 정의 합니다.

    *\_TAB512와 같은 내보내기 테이블*

    :::image type="content" source="media/abap-functions-deployment-guide/export-table.png" alt-text="원본 등록 옵션" border="true":::

6.  예외 탭으로 이동 하 여 \" \" 다음 예외를 정의 합니다.

    *E \_ EXP \_ GUI를 \_ 다운로드 하지 못했습니다.*

    :::image type="content" source="media/abap-functions-deployment-guide/exceptions.png" alt-text="원본 등록 옵션" border="true":::

7.  함수를 저장 합니다. ctrl + S를 누르거나 \> 기본 메뉴에서 함수 모듈 = 저장을 선택 합니다.

8.  \" \" 도구 모음에서 활성화 아이콘 (Ctrl + F3)을 클릭 하 \" 고 \" 대화 상자 창에서 계속 단추를 누릅니다. 메시지가 표시 되 면 생성 된 포함을 선택 하 여 주 함수 모듈과 함께 활성화 해야 합니다.
...
2.  \" \> 주 메뉴에서 함수 모듈 = test = Test \> 함수 모듈 \" 을 선택 하거나 F8 키를 누릅니다.
