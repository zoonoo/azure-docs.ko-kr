...
비교 내용을 자세히 확인하려면 이 문서의 끝 부분에 있는 [Azure 진단 1.0과 1.2 비교][] 를 참조하세요.
...
<tbody>
	...
	<tr>
		<td>크래시 덤프</td>
		<td>응용 프로그램 작동 중단가 발생할 경우의 프로세스 상태에 대한 정보입니다.</td>            
	</tr>
	<tr>
		<td>사용자 지정 오류 로그</td>
	...
	<tr>
		...
		<td>.NET EventSource 클래스를 사용하여 코드에서 생성하는 <a href="http://msdn.microsoft.com/ko-kr/library/system.diagnostics.tracing.eventsource(v=vs.110).aspx">이벤트입니다.</a></td>            
	</tr>
	<tr>
		<td>매니페스트 기반 ETW</td>
...
<h2><a name="worker-role"></a>작업자 역할에서 진단을 사용하는 방법</h2>
...
<h3>1단계: 작업자 역할 만들기</h3>
1.	**Visual Studio 2013**을 실행합니다.
2.	.NET Framework 4.5를 대상으로 하는 **클라우드** 템플릿에서 새 **Microsoft Azure 클라우드 서비스** 프로젝트를 만듭니다.  프로젝트 이름을 "WadExample"이라고 지정합니다.
3.	**작업자 역할**을 선택합니다.
...
<h3>2단계: 코드 계측</h3>
WorkerRole.cs 내용을 다음 코드로 바꿉니다. [EventSource 클래스][]에서 상속되는 클래스인 SampleEventSourceWriter는 네 가지 로깅 메서드인 **SendEnums**, **MessageMethod**, **SetOther** 및 **HighFreq**를 구현합니다. **WriteEvent** 메서드에 대한 첫 번째 매개 변수는 각 이벤트의 ID를 저장합니다. Run 메서드는 **SampleEventSourceWriter** 클래스에 구현된 각각의 로깅 메서드를 10초마다 호출하는 무한 루프를 구현합니다.
...
3.	WadConfig.xsd를 구성 파일과 연결합니다. WadExample.xml 편집기 창이 활성 창인지 확인합니다. **F4** 키를 눌러 **속성** 창을 엽니다. **속성** 창에서 **스키마** 속성을 클릭합니다. **스키마** 속성에서 **...**를 클릭합니다. **추가...** 단추를 클릭하고 XSD 파일을 저장한 위치로 이동한 후 WadConfig.xsd 파일을 선택하고 **확인**을 클릭합니다.
...
2.	작업자 역할에 진단을 설치하기 위한 스크립트를 실행합니다(StorageAccountKey를 wadexample 저장소 계정의 저장소 계정 키로 바꿈).
...
<h3>6단계: 원격 분석 데이터 확인</h3>
Visual Studio **서버 탐색기**에서 wadexample 저장소 계정으로 이동합니다. 클라우드 서비스가 5분 정도 실행되면 **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** 및 **WADSetOtherTable** 테이블이 표시됩니다. 테이블 중 하나를 두 번 클릭하여 수집된 원격 분석 데이터를 확인합니다.
	![CloudServices_diag_tables](./media/cloud-services-dotnet-diagnostics/WadExampleTables.png)

<h2><a name="virtual-machine"></a>가상 컴퓨터에서 진단을 사용하는 방법</h2>
...
<h3>1단계: 가상 컴퓨터 만들기</h3>
1.	개발 컴퓨터에서 Visual Studio 2013을 실행합니다.
2.	Visual Studio **서버 탐색기**에서 **Microsoft Azure**를 마우스 오른쪽 단추로 클릭하고 **가상 컴퓨터** -> **가상 컴퓨터 만들기**를 선택합니다.
...
<h3>5단계: Azure 가상 컴퓨터에 원격으로 진단 설치 </h3>
...
1.	개발자 컴퓨터에서 Microsoft Azure PowerShell을 엽니다.
...
<h3>6단계: 원격 분석 데이터 확인</h3>
Visual Studio **서버 탐색기**에서 wadexample 저장소 계정으로 이동합니다. VM이 5분 정도 실행되면 **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** 및 **WADSetOtherTable** 테이블이 표시됩니다. 테이블 중 하나를 두 번 클릭하여 수집된 원격 분석 데이터를 확인합니다.
	![CloudServices_diag_wadexamplevm_tables](./media/cloud-services-dotnet-diagnostics/WadExampleVMTables.png)

<h2><a name="configuration-file-schema"></a>구성 파일 스키마</h2>

진단 구성 파일에서는 진단 모니터가 시작될 때 진단 구성 설정을 초기화하는 데 사용되는 값을 정의합니다. 샘플 구성 파일 및 해당 스키마에 대한 자세한 문서는 [Azure 진단 1.2 구성 스키마][]에서 확인할 수 있습니다.

<h2><a name="troubleshooting"></a>문제 해결</h2>

<h3>Azure 진단이 시작되지 않음</h3>
...
<p>해결 방법: 이는 구성 파일이 스키마 유효성 검사를 통과하지 못한 것이 원인입니다. 따라서 해결 방법은 스키마를 준수하는 구성 파일을 제공하는 것입니다. 진단 확장으로 전달되는 XML은 VM의 <i>%SystemDrive%\WindowsAzure\Config</i> 폴더에서 확인할 수 있습니다. 해당 XML 파일을 열고 <strong>Microsoft.Azure.Diagnostics</strong>를 검색하고 나서 <strong>xmlCfg</strong> 필드를 검색합니다. 데이터는 base64로 인코드되므로 <a href="http://www.bing.com/search?q=base64+decoder">진단을 통해 로드된 XML을 확인하기 위해</a> 디코드할 필요가 없습니다.</p></td>		            
	</tr>
    <tr>
		<td>-107</td>
...
<h3>진단 데이터가 저장소에 기록되지 않음</h3>
이벤트 데이터 누락의 가장 일반적인 원인은 잘못 정의된 저장소 계정 정보입니다. 

해결 방법: 진단 구성 파일을 수정하고 진단을 다시 설치합니다.
이벤트 데이터는 저장소 계정에 업로드되기 전에 폴더에 저장됩니다. 위에서 **LocalResourceDirectory**에 대한 세부 정보를 참조하세요.
...
- 코드에서 추적 리스너를 사용하는 경우 .NET EventSource를 사용하도록 코드를 수정해야 합니다. 진단 1.1 이상에서는 추적 리스너를 지원하지 않습니다.
- 배포 프로세스를 수정하여 진단 1.1 확장을 설치합니다.

**Q.** 역할 또는 VM에 이미 진단 1.1 확장을 설치한 경우 진단 1.2로 업그레이드하려면 어떻게 해야 합니까?

**A.** 진단 1.1을 설치할 때 "-Version "1.*""를 지정했다면 다음에 역할이 다시 시작되거나 VM이 다시 부팅될 때 진단은 정규식 "1.*"와 일치하는 최신 버전으로 자동으로 업데이트됩니다. 진단 1.1을 설치할 때 "-Version "1.1""을 지정한 경우에는 Set- cmdlet을 다시 실행하고 설치할 버전을 지정하여 최신 버전으로 업데이트할 수 있습니다.
...
<h2><a name="comparing"></a>Azure 진단 1.0과 1.2 비교</h2>
...
[Set-AzureServiceDiagnosticsExtension]: http://msdn.microsoft.com/ko-kr/library/dn495270.aspx
