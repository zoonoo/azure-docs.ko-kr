...
* **인증 및 권한 부여**에는 Azure Key Vault에서 RBAC(역할 기반 액세스 제어) 및 Azure AD(Active Directory) 통합, Pod ID 및 비밀을 사용하는 기능이 포함됩니다. 이러한 기능에 대 한 자세한 내용은 [AKS의 인증 및 권한 부여에 대 한 모범 사례][aks-best-practices-identity]를 참조 하세요.
...
논리적 격리를 사용하면 여러 워크로드, 팀 또는 환경에 단일 AKS 클러스터를 사용할 수 있습니다. Kubernetes [네임 스페이스][k8s-namespaces] 는 워크 로드 및 리소스에 대 한 논리적 격리 경계를 형성 합니다.
