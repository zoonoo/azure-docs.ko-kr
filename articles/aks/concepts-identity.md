...
클러스터가 생성될 때 생성되고 AKS 클러스터와 연결된 AKS 클러스터 ID로 다음 권한이 사용됩니다. 각 사용 권한은 아래와 같은 이유로 사용됩니다:
...
특정 특성이 있는 클러스터를 생성할 때 클러스터 ID에는 다음과 같은 추가 권한이 필요합니다. 이러한 사용 권한은 자동으로 할당되지 않으므로 이러한 사용 권한을 만든 후에 클러스터 ID에 추가해야 합니다.
...
## <a name="kubernetes-role-based-access-control-kubernetes-rbac"></a>Kubernetes 역할 기반 액세스 제어(Kubernetes RBAC)를 사용합니다.
