...
description: AKS (Azure Kubernetes Service) 클러스터에서 Azure Disk 및 Azure Files에 대 한 CSI (Container Storage Interface) 드라이버를 사용 하도록 설정 하는 방법에 대해 알아봅니다.
...
AKS의 CSI storage driver support를 사용 하면 기본적으로 다음을 활용할 수 있습니다.
- [*Azure 디스크*](azure-disk-csi.md) -Kubernetes *datadisk* 리소스를 만드는 데 사용할 수 있습니다. 디스크는 고성능 Ssd에서 지원 되는 Azure Premium storage 또는 일반 Hdd 또는 표준 Ssd에서 지 원하는 Azure Standard storage를 사용할 수 있습니다. 대부분의 프로덕션 및 개발 워크로드의 경우 Premium 스토리지를 사용합니다. Azure 디스크는 *Readwriteonce 번*탑재 되므로 단일 pod 에서만 사용할 수 있습니다. 여러 pod에서 동시에 액세스할 수 있는 저장소 볼륨의 경우 Azure Files를 사용 합니다.
- [*Azure Files*](azure-files-csi.md) 를 사용 하 여 Azure Storage 계정으로 지원 되는 SMB 3.0 공유를 pod에 탑재할 수 있습니다. Files를 사용하면 여러 노드 및 Pod 간에 데이터를 공유할 수 있습니다. 파일은 고성능 Ssd에서 지원 되는 regular Hdd 또는 Azure Premium storage에서 지 원하는 Azure Standard storage를 사용할 수 있습니다.

> [!IMPORTANT]
> Kubernetes 버전 1.21부터 Kubernetes는 기본적으로 CSI 드라이버만 사용 합니다. Kubernetes의 저장소 지원의 미래입니다.
>
...
Azure 디스크 및 Azure Files에 대 한 CSI 드라이버를 활용할 수 있는 AKS 클러스터를 만들려면 `EnableAzureDiskFileCSIDriver` 구독에서 기능 플래그를 사용 하도록 설정 해야 합니다.
...
CSI storage 드라이버를 사용할 수 있는 AKS 클러스터 또는 노드 풀을 만들려면 최신 *AKS 미리 보기* CLI 확장이 필요 합니다. [Az extension add][az-extension-add] 명령을 사용 하 여 *aks-preview* Azure CLI 확장을 설치 하거나 [az extension update][az-extension-update] 명령을 사용 하 여 사용 가능한 업데이트를 설치 합니다.
...
다음 CLI 명령을 사용 하 여 Azure 디스크에 대 한 CSI 저장소 드라이버를 활용 하 고 Azure Files 수 있는 새 클러스터를 만듭니다. 플래그를 사용 `--aks-custom-headers` 하 여 기능을 설정 `EnableAzureDiskFileCSIDriver` 합니다.
...
저장소 드라이버를 CSI 않고 클러스터 내 저장소 드라이버를 만들려는 경우 사용자 지정 매개 변수를 생략 하 여 수행할 수 있습니다 `--aks-custom-headers` .
...
- Azure 디스크에 CSI 드라이브 사용을 참조 하세요. [CSI 드라이버와 함께 azure Disk 사용](azure-disk-csi.md)을 참조 하세요.
- Azure 파일용 CSI 드라이브 사용에 대 한는 [CSI 드라이버에서 azure Files 사용](azure-files-csi.md)을 참조 하세요.
