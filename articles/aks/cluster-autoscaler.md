...
클러스터 자동 크기 조정기는 크기 조정 이벤트와 리소스 임계값 사이의 시간 간격 같은 항목에 시작 매개 변수를 사용합니다. 클러스터 자동 크기 조정기가 사용하는 매개 변수에 대한 자세한 내용은 [자동 크기 조정기 프로필 사용](#using-the-autoscaler-profile)을 참조하세요.
...
## <a name="update-an-existing-aks-cluster-to-enable-the-cluster-autoscaler"></a>기존 AKS 클러스터를 업데이트하여 클러스터 자동 크기 조정기를 사용하도록 설정

[az aks update][az-aks-update] 명령을 사용하여 기존 클러스터에 대한 노드 풀에서 클러스터 자동 크기 조정기를 사용하도록 설정하고 구성할 수 있습니다. `--enable-cluster-autoscaler` 매개 변수를 사용하여 `--min-count` 및 `--max-count` 노드를 지정합니다.

> [!IMPORTANT]
> 클러스터 자동 크기 조정기는 Kubernetes 구성 요소입니다. AKS 클러스터는 가상 머신 확장 집합을 노드에 사용하지만, Azure Portal에서 또는 Azure CLI를 사용하여 확장 집합 자동 크기 조정에 대한 설정을 직접 설정하거나 편집하지 마세요. Kubernetes 클러스터 자동 크기 조정기가 필수 크기 조정 설정을 자동으로 관리하게 두세요. 자세한 내용은 [노드 리소스 그룹의 AKS 리소스를 수정할 수 있나요?][aks-faq-node-resource-group]를 참조하세요.

다음 예에서는 기존 AKS 클러스터를 업데이트하여 클러스터에 대한 노드 풀에서 클러스터 자동 크기 조정기를 사용하도록 설정하고 최소 *1개* 에서 최대 *3개* 의 노드를 설정합니다.
...
> [!NOTE]
> 클러스터 자동 크기 조정기는 각 노드 풀에 설정된 최소 및 최대 개수에 따라 크기 조정을 결정하지만 최소 및 최대 개수를 업데이트한 후에는 이를 강제로 적용하지는 않습니다. 예를 들어 현재 노드 수가 3개인 경우 최소 개수를 5개로 설정하면 풀의 크기가 5개로 즉시 조정되지 않습니다. 노드 풀의 최소 개수를 현재 노드 수보다 큰 값일 경우 새로운 최소 또는 최대 설정이 적용되려면 새로운 추가 노드 2개가 필요하고 자동 크기 조정기 이벤트가 트리거되므로 예약이 불가한 Pod가 충분히 있어야 합니다. 크기 조정 이벤트 후에는 새로운 개수 제한이 적용됩니다.
...
| 확장기                         | 스케일 업에 사용할 노드 풀 [확장기](https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/FAQ.md#what-are-expanders)의 유형입니다. 가능한 값: `most-pods`, `random`, `least-waste`, `priority` | random | 
...
>
> 클러스터 자동 크기 조정기 프로필에는 Azure CLI *2.11.1* 이상 버전이 필요합니다. 설치 또는 업그레이드해야 하는 경우 [Azure CLI 설치][azure-cli-install]를 참조하세요.
...
클러스터 자동 크기 조정기를 더 이상 사용하지 않으려면 [az aks update][az-aks-update-preview] 명령을 사용하고 `--disable-cluster-autoscaler` 매개 변수를 지정하여 사용하지 않도록 설정하면 됩니다. 클러스터 자동 크기 조정기가 사용하지 않도록 설정되면 노드가 제거되지 않습니다.
...
기존 클러스터에서 클러스터 자동 확장 조정기를 다시 사용하도록 설정하려면 `--enable-cluster-autoscaler`, `--min-count` 및 `--max-count` 매개 변수가 지정된 [az aks update][az-aks-update-preview] 명령을 사용합니다.
...
AKS는 사용자를 대신하여 클러스터 자동 크기 조정기를 관리하고 관리형 컨트롤 플레인에서 실행합니다. 컨트롤 플레인 노드에서 CA의 로그 및 작업을 볼 수 있도록 설정할 수 있습니다.
...
1. Azure Portal을 통해 클러스터의 "로그" 섹션을 선택합니다.
...
또한 클러스터 자동 크기 조정기는 `cluster-autoscaler-status`라는 이름의 `configmap`에 상태를 기록합니다. 이러한 로그를 검색하려면 다음 `kubectl` 명령을 실행합니다. 클러스터 자동 크기 조정기에 구성된 각 노드 풀의 상태가 보고됩니다.
...
> [!NOTE]
> 여러 영역에 걸쳐 있고 볼륨 토폴로지 예약 등의 영역과 관련된 예약 기능을 활용하는 노드 풀과 함께 클러스터 자동 확장 조정기를 사용하려는 경우, 영역당 하나의 노드 풀을 사용하고 자동 확장 조정기 프로필을 통해 `--balance-similar-node-groups`를 사용하도록 설정하는 것이 좋습니다. 이렇게 하면 자동 확장 조정기가 성공적으로 스케일 업하고 노드 풀의 크기를 균형 있게 유지할 수 있습니다.
