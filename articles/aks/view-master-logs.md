...
Azure 모니터 로그는 Azure 포털에서 활성화 되고 관리됩니다. AKS 클러스터의 Kubernetes 마스터 구성 요소에 대한 로그 수집을 사용하도록 설정하려면 웹 브라우저에서 Azure Portal을 열고 다음 단계를 완료하세요.
...
1. *myAKSCluster와*같은 AKS 클러스터를 선택한 다음 **진단 설정 추가를**선택합니다.
1. *myAKSClusterLogs*와 같은 이름을 입력한 후 **Log Analytics에 보내기** 옵션을 선택합니다.
1. 기존 작업 영역을 선택하거나 새 작업 영역을 만듭니다. 작업 영역을 만드는 경우 작업 영역 이름, 리소스 그룹 및 위치를 제공합니다.
1. 사용 가능한 로그 목록에서 사용하도록 설정하려는 로그를 선택합니다. 일반적인 로그에는 *kube-apiserver,* *kube-controller-manager*및 *kube-scheduler가*포함됩니다. *kube-audit* 및 *cluster-autoscaler*와 같은 추가 로그를 사용하도록 설정할 수 있습니다. Log Analytics 작업 영역이 사용하도록 설정되면 수집된 로그를 반환하고 변경할 수 있습니다.
1. 준비되면 **저장**을 선택하여 선택된 로그의 수집을 사용하도록 설정합니다.

다음 예제 포털 스크린샷은 *진단 설정* 창을 보여 주며 다음과 같은
...
진단 로그를 사용하도록 설정하고 Log Analytics 작업 영역에 표시되기까지 몇 분 정도 걸릴 수 있습니다. Azure 포털에서 *myResourceGroup과*같은 로그 분석 작업 영역에 대한 리소스 그룹을 선택한 다음 *myAKSLog와*같은 로그 분석 리소스를 선택합니다.
...
로그 데이터를 쿼리하고 필터링하는 방법에 대한 자세한 내용은 [로그 분석 로그 검색으로 수집된 데이터 보기 또는 분석][analyze-log-analytics]을 참조하십시오.
...
| *시간*                   | 로그가 업로드된 시간의 타임스탬프 |
| *범주*               | 로그를 생성하는 컨테이너/로그의 이름 |
| *작업이름*          | Always *Microsoft.ContainerService/managedClusters/diagnosticLogs/Read* |
| *properties.log*         | 구성 요소 로그의 전체 텍스트 |
| *properties.stream*      | *stderr* 또는 *stdout* |
| *properties.pod*         | 로그를 가져온 Pod 이름 |
| *properties.containerID* | 이 로그가 에서 온 도커 컨테이너의 ID |

## <a name="log-roles"></a>로그 역할

| 역할                     | 설명 |
|--------------------------|-------------|
| *aksService*             | 제어 평면 작업에 대한 감사 로그에 있는 표시 이름(hcpService에서) |
| *마스터 클라이언트*           | MasterClientCertificate에 대한 감사 로그에 있는 표시 이름, az aks get-credentials에서 얻은 인증서 |
| *노드 클라이언트*             | 에이전트 노드에서 사용되는 ClientCertificate의 표시 이름 |
