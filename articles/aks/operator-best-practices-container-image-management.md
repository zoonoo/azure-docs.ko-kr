...
AKS(Azure Kubernetes Service)에서 애플리케이션을 개발 및 실행할 경우 컨테이너 보안 유지가 핵심 고려 사항입니다. 오래된 기본 이미지 또는 애플리케이션 런타임이 포함된 컨테이너는 보안 위험 및 가능한 공격 벡터를 초래합니다. 이러한 위험을 최소화하려면 문제를 검색하고 해결하는 도구를 컨테이너에 통합해야 합니다.
