...
Pod 제한은 Pod가 리소스 소비량에 대한 제어권을 상실한 경우를 정의합니다. 한도를 초과하면 Pod가 종료되로록 표시됩니다. 이 동작은 노드 상태를 유지하고 노드를 공유하는 Pod에 대한 영향을 최소화합니다. Pod 제한을 설정하지 않으면 지정된 노드에서 사용할 수 있는 가장 높은 값으로 기본 설정됩니다.
