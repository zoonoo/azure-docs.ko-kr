...
>
> Istio `1.11` `1.12`릴리스는 Kubernetes 버전,, `1.13`에 대 한 istio 팀에서 테스트 했습니다. `1.1.x` [GitHub-Istio 릴리스와][istio-github-releases] [istio][istio-release-notes]릴리스 정보에서 각 릴리스에 대 한 정보를 추가로 확인할 수 있습니다.
...
이러한 지침을 따르고 Istio를 설치 하려면 [투구][helm] 가 필요 합니다. 클러스터에 버전 `2.12.2` 이상을 올바르게 설치 하 고 구성 하는 것이 좋습니다. 투구 설치와 관련 하 여 도움이 필요한 경우 [AKS 투구 설치 지침][helm-install]을 참조 하세요. 모든 Istio pod Linux 노드에서 실행 되도록 예약 되어야 합니다.
...
PowerShell에서를 사용 `Invoke-WebRequest` 하 여 최신 istio 릴리스를 다운로드 한 후 다음과 `Expand-Archive` 같이를 사용 하 여 추출 합니다.
...
클라이언트 `istioctl` 이진 파일은 클라이언트 컴퓨터에서 실행 되며 istio 서비스 메시와 상호 작용할 수 있습니다. 설치 단계는 클라이언트 운영 체제 마다 약간 다릅니다. 기본 설정 된 환경과 일치 하는 다음 설치 단계 중 하나를 선택 합니다.
...
다음 명령을 사용 하 여 linux의 bash 기반 `istioctl` 셸 또는 [linux 용 Windows 하위 시스템][install-wsl]에 istio 클라이언트 이진을 설치 합니다. 이 명령을 사용하여 `istioctl` 클라이언트 이진 파일을 `PATH`의 표준 사용자 프로그램 위치에 복사합니다.
...
Windows의 **Powershell**기반 셸에서 `istioctl` istio 클라이언트 이진을 설치 하려면 다음 명령을 사용 합니다. 이러한 명령은 `istioctl` 클라이언트 이진 파일을 istio 폴더에 복사한 다음 즉시 (현재 셸에서) 및를 `PATH`통해 영구적으로 (셸 다시 시작에서) 사용할 수 있도록 설정 합니다. 이러한 명령을 실행 하는 데에는 상승 된 (관리자) 권한이 필요 하지 않으며 셸을 다시 시작 하지 않아도 됩니다.
...
[작업][kubernetes-jobs] 은 crds를 설치 하기 `istio-init` 위한 투구 차트의 일부로 배포 됩니다. 이러한 작업은 클러스터 환경에 따라 완료 하는 데 1 ~ 2 분 정도 걸립니다. 다음과 같이 작업이 성공적으로 완료 되었는지 확인할 수 있습니다.
...
이제 작업이 성공적으로 완료 되었음을 확인 했으므로 올바른 Istio CRDs 수가 설치 되어 있는지 확인 합니다. 사용자 환경에 적합 한 명령을 실행 하 여 모든 53 Istio CRDs가 설치 되었는지 확인할 수 있습니다. 이 명령은 번호 `53`를 반환 해야 합니다.
...
토큰을 `REPLACE_WITH_YOUR_SECURE_PASSWORD` 암호로 바꾸고 다음 명령을 실행 합니다.
...
> - `global.controlPlaneSecurityEnabled=true`-제어 평면에 대해 상호 TLS를 사용 하도록 설정
> - `mixer.adapters.useAdapterCRDs=false`-믹서 어댑터 CRDs에서 감시를 제거 합니다 .이는 사용 되지 않으므로 성능을 향상 시킵니다.
...
투구 `istio` 차트는 많은 수의 개체를 배포 합니다. 위의 `helm install` 명령 출력에서 목록을 볼 수 있습니다. Istio 구성 요소의 배포는 클러스터 환경에 따라 완료 하는 데 4 ~ 5 분 정도 걸릴 수 있습니다.
...
먼저 예상 서비스가 만들어졌는지 확인합니다. [Kubectl get svc][kubectl-get] 명령을 사용 하 여 실행 중인 서비스를 볼 수 있습니다. Istio 및 추가 기능 구성 요소가 `istio` 투구 차트에서 설치 된 네임스페이스를쿼리합니다.`istio-system`
...
상태가 인 두 개의 `istio-init-crd-*` pod이 있어야 합니다. `Completed` 이러한 pod는 이전 단계에서 CRDs를 만든 작업을 실행 해야 했습니다. 다른 모든 pod의 `Running`상태를 표시 해야 합니다. Pod에서 상태가 표시되지 않는 경우 상태가 표시될 때까지 1~2분 정도 걸릴 수 있습니다. Pod에서 문제를 보고 하는 경우 [kubectl 설명 pod][kubectl-describe] 명령을 사용 하 여 출력 및 상태를 검토 합니다.
...
Istio에 대 한 분석 및 모니터링 대시보드는 [Grafana][grafana]에서 제공 합니다. AKS 클러스터에서 Grafana `3000` 를 실행 하는 pod의 `3000` 포트에 클라이언트 컴퓨터의 로컬 포트를 전달 합니다.
...
Istio에 대 한 메트릭은 [프로메테우스][prometheus]에서 제공 합니다. AKS 클러스터에서 프로메테우스 `9090` 를 실행 하는 pod의 `9090` 포트에 클라이언트 컴퓨터의 로컬 포트를 전달 합니다.
...
Istio 내의 추적은 [Jaeger][jaeger]에서 제공 합니다. AKS 클러스터에서 Jaeger `16686` 를 실행 하는 pod의 `16686` 포트에 클라이언트 컴퓨터의 로컬 포트를 전달 합니다.
...
서비스 메시 관찰성 대시보드는 [Kiali][kiali]에서 제공 합니다. AKS 클러스터에서 Kiali `20001` 를 실행 하는 pod의 `20001` 포트에 클라이언트 컴퓨터의 로컬 포트를 전달 합니다.
...
AKS 클러스터에서 Istio를 제거 하려면 다음 명령을 사용 합니다. `istio` 이 `helm delete` 명령은 및`istio-init` 차트를 제거 하 고,이 명령은네임스페이스를제거합니다.`istio-system` `kubectl delete ns`
