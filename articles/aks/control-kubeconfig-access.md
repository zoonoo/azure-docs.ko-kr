...
`kubectl` 도구를 사용하여 AKS 클러스터와 상호 작용할 때는 클러스터 연결 정보를 정의하는 구성 파일이 사용됩니다. 이러한 구성 파일은 일반적으로 *~/.kube/config*에 저장됩니다. 이 *kubeconfig* 파일에 여러 클러스터를 정의할 수 있습니다. 사용 하 여 클러스터를 전환 합니다 [kubectl 구성 사용 하 여 상황에 맞는][kubectl-config-use-context] 명령입니다.

합니다 [az aks 자격 증명 가져오기][az-aks-get-credentials] 명령을 통해 AKS 클러스터에 대 한 액세스 자격 증명을 가져올 수 있습니다 하 고에 병합 합니다 *kubeconfig* 파일. Azure 역할 기반 액세스 제어(RBAC)를 사용하여 이러한 자격 증명에 대한 액세스를 제어할 수 있습니다. 이러한 Azure RBAC 역할을 사용하여 *kubeconfig* 파일을 검색할 수 있는 사람과 그 사람이 클러스터 내에서 가지는 권한을 정의할 수 있습니다.

두 가지 기본 제공 역할은 다음과 같습니다.

* **Azure Kubernetes Service 클러스터 관리자 역할**  
    * *Microsoft.ContainerService/managedClusters/listClusterAdminCredential/action* API 호출에 대한 액세스를 허용합니다. 이 API 호출 [클러스터 관리자 자격 증명을 나열][api-cluster-admin]합니다.
    * *clusterAdmin* 역할의 *kubeconfig*를 다운로드합니다.
* **Azure Kubernetes Service 클러스터 사용자 역할**
    * *Microsoft.ContainerService/managedClusters/listClusterUserCredential/action* API 호출에 대한 액세스를 허용합니다. 이 API 호출 [클러스터 사용자 자격 증명을 나열][api-cluster-user]합니다.
...
* 사용 하 여 클러스터 리소스 ID를 [az aks show][az-aks-show] 라는 클러스터에 대 한 명령을 *myAKSCluster* 에 *myResourceGroup* 리소스 그룹. 필요에 따라 고유한 클러스터 및 리소스 그룹 이름을 지정합니다.
* 사용 하 여 [az 계정 표시][az-account-show] and [az ad user show][az-ad-user-show] 명령에 사용자 ID를 가져옵니다.
* 마지막으로 사용 하 여 역할을 할당 합니다 [az 역할 할당 만들기][az-role-assignment-create] 명령입니다.
...
RBAC 역할 할당을 사용 합니다 [az aks 자격 증명 가져오기][az-aks-get-credentials] 명령으로 가져올 합니다 *kubeconfig* AKS 클러스터에 대 한 정의. 다음 예에서는 사용자에게 *클러스터 관리자 역할*이 부여된 경우에만 올바르게 작동하는 *--admin* 자격 증명을 가져옵니다.

```azurecli-interactive
az aks get-credentials --resource-group myResourceGroup --name myAKSCluster --admin
```

사용할 수 있습니다 합니다 [kubectl 구성 보기][kubectl-config-view] 명령을 사용 하는 확인 합니다 *상황에 맞는* 클러스터 관리자 구성 정보를 적용 된 것을 보여 줍니다.에 대 한:
...
역할 할당을 제거 하려면 사용 합니다 [az 역할 할당 삭제][az-role-assignment-delete] 명령입니다. 이전 명령에서 가져온 대로 계정 ID 및 클러스터 리소스 ID를 지정 합니다. 사용자 대신 그룹을 역할에 할당 한 경우 적절 한 그룹 개체에 대 한 계정 개체 ID가 아니라 ID를 지정 합니다 `--assignee` 매개 변수:
...
AKS 클러스터에 대 한 액세스의 보안 강화를 위해 [Azure Active Directory 인증 통합][aad-integration]합니다.
