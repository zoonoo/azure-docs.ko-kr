...
> [!IMPORTANT]
> 기본적으로 클러스터는 아웃 바운드 게이트웨이를 구성 하는 데 사용할 수 있는 [표준 SKU 부하 분산 장치][standard-sku-lb] 를 사용 합니다. 클러스터를 만드는 동안 API server 권한 있는 IP 범위를 사용 하도록 설정 하면 지정 된 범위 외에도 기본적으로 클러스터에 대 한 공용 IP가 허용 됩니다. *--------* ---------------------------------
...
기존 클러스터에서 API server 권한 있는 IP 범위를 업데이트 하려면 [az aks update][az-aks-update] 명령을 사용 하 고-- *API*----------- *--* ----------- *--.* 또는 *--부하 분산 장치-아웃 바운드-접두사* 매개 변수
