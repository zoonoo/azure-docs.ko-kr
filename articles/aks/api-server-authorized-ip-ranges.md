...
description: 해결 하는 방법을에 보안 클러스터 IP를 사용 하 여 Azure Kubernetes Service (AKS)에서 API 서버에 대 한 액세스 범위에 알아봅니다.
...
API 서버 권한이 부여 된 IP 범위를 구성 하려면 CLI 명령을 사용할 수는 *aks 미리 보기* CLI 확장 합니다. 설치를 *aks 미리 보기* 사용 하 여 Azure CLI 확장을 [az 확장 추가] [ az-extension-add] 다음 예와에서 같이 명령:
...
먼저 사용 하 여 클러스터를 만듭니다는 [az aks 만듭니다] [ az-aks-create] 명령입니다. 다음 예제에서는 라는 단일 노드 클러스터를 만듭니다 *myAKSCluster* 이라는 리소스 그룹에서 *myResourceGroup*합니다.
...
> [!WARNING]
> Azure 방화벽의 사용은 월간 청구 주기 동안 상당한 비용이 수 있습니다. Azure 방화벽을 사용 하는 요구 사항을이 초기 미리 보기 기간에만 해야 해야 합니다. 자세한 내용 및 비용 계획을 참조 하세요 [Azure 방화벽 가격 책정][azure-firewall-costs]합니다.
...
만들려면 Azure 방화벽을 설치 합니다는 *azure 방화벽* 사용 하 여 CLI 확장을 [az 확장 추가] [ az-extension-add] 명령입니다. 그런 다음 사용 하 여 방화벽을 만듭니다는 [az 네트워크 방화벽 만들기] [ az-network-firewall-create] 명령입니다. 다음 예제에서는 라는 Azure 방화벽을 만듭니다 *myAzureFirewall*:
...
Azure는 방화벽에는 송신 트래픽이 통과 하는 공용 IP 주소를 할당 됩니다. 사용 하 여 공용 주소를 [az network public ip 만들기] [ az-network-public-ip-create] 를 선택한 다음 사용 하 여 방화벽에서 IP 구성을 만듭니다는 [az 네트워크 방화벽 ip 구성 만들기] [ az-network-firewall-ip-config-create] 적용 되는 공용 IP:
...
기존 AKS 네트워크 경로 테이블 사용 하 여 경로 마지막으로 작성 합니다 [az 네트워크 경로 테이블 경로 만들기] [ az-network-route-table-route-create] API 서버에 대 한 Azure 방화벽 어플라이언스를 사용 하는 트래픽을 허용 하는 명령 통신 합니다.
...
다시 사용을 업데이트 하거나 권한 있는 IP 범위를 사용 하지 않도록 설정 하려면 [az aks 업데이트할] [ az-aks-update] 명령입니다. 허용 하거나 다음 예와에서 같이 빈 범위 API 서버를 사용 하지 않도록 설정 권한이 IP 범위를 지정 하려면 업데이트 된 CIDR 범위를 지정 합니다.
