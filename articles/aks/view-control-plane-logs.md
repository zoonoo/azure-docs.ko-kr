...
description: AKS (Azure Kubernetes Service)에서 Kubernetes 제어 평면에 대 한 로그를 사용 하도록 설정 하 고 보는 방법에 대해 알아봅니다.
...
AKS (Azure Kubernetes Service)를 사용 하면 *kube-apiserver* 및 *kube* 와 같은 제어 평면 구성 요소가 관리 되는 서비스로 제공 됩니다. *kubelet* 및 컨테이너 런타임을 실행하는 노드를 생성 및 관리하고, 관리되는 Kubernetes API 서버를 통해 애플리케이션을 배포합니다. 응용 프로그램 및 서비스의 문제를 해결 하려면 이러한 제어 평면 구성 요소에 의해 생성 된 로그를 확인 해야 할 수 있습니다. 이 문서에서는 Azure Monitor 로그를 사용 하 여 Kubernetes 제어 평면 구성 요소에서 로그를 사용 하도록 설정 하 고 쿼리 하는 방법을 보여 줍니다.
...
Azure Monitor 로그는 Azure Portal에서 사용 하도록 설정 되 고 관리 됩니다. AKS 클러스터에서 Kubernetes 제어 평면 구성 요소에 대 한 로그 수집을 사용 하도록 설정 하려면 웹 브라우저에서 Azure Portal을 열고 다음 단계를 완료 합니다.
...
3. AKS 클러스터를 선택 하 고 (예: *myAKSCluster*) **진단 설정을 추가** 하도록 선택 합니다.
  :::image type="content" source="media\view-control-plane-logs\select-add-diagnostic-setting.PNG" alt-text="' 진단 설정 추가 '가 선택 되었음을 나타내는 진단 설정을 표시 하는 브라우저 창에서 Azure Portal의 스크린샷":::

4. *myAKSClusterLogs* 등의 이름을 입력하고 **Log Analytics 작업 영역에 보내기** 옵션을 선택합니다.

5. 기존 작업 영역을 선택 하거나 새 작업 영역을 만듭니다. 작업 영역을 만드는 경우 작업 영역 이름, 리소스 그룹 및 위치를 제공 합니다.

6. 사용 가능한 로그 목록에서 사용하도록 설정하려는 로그를 선택합니다. 이 예에서는 *kube* 및 *kube* 로그를 사용 하도록 설정 합니다. 일반적인 로그에는 *kube 서버*, *kube* 및 *kube* 가 포함 됩니다. Log Analytics 작업 영역이 사용하도록 설정되면 수집된 로그를 반환하고 변경할 수 있습니다.

7. 준비되면 **저장** 을 선택하여 선택된 로그의 수집을 사용하도록 설정합니다.
  :::image type="content" source="media\view-control-plane-logs\settings-selected.PNG" alt-text="Azure Portal ' 진단 설정 추가 ' 화면의 스크린샷 ' Log Analytics 작업 영역으로 보내기 ' 및 ' kube ' 및 ' kube ' 로그의 대상이 선택 됩니다.":::
...
이 문서에서는 AKS 클러스터에서 Kubernetes 제어 평면 구성 요소에 대 한 로그를 사용 하도록 설정 하 고 검토 하는 방법을 배웠습니다. 추가로 모니터링하고 문제를 해결하려면 [Kubelet 로그를 확인][kubelet-logs]하고 [SSH 노드 액세스를 사용하도록 설정][aks-ssh]하면 됩니다.
