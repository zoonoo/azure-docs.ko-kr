...
Kubernetes 대시보드에 대 한 자세한 내용은 참조 하세요. [Kubernetes 웹 UI 대시보드][kubernetes-dashboard]합니다.

## <a name="before-you-begin"></a>시작하기 전 주의 사항

이 문서에 설명된 단계에서는 AKS 클러스터를 만들고 클러스터와 `kubectl` 연결을 설정했다고 가정합니다. AKS 클러스터를 만드는 경우 참조를 [AKS 빠른 시작][aks-quickstart]합니다.

또한 Azure CLI 버전 2.0.46 이상이 설치되고 구성되어 있어야 합니다.  `az --version` 을 실행하여 버전을 찾습니다. 설치 또는 업그레이드를 참조 해야 하는 경우 [Azure CLI 설치][install-azure-cli]합니다.

## <a name="start-the-kubernetes-dashboard"></a>Kubernetes 대시보드 시작

Kubernetes 대시보드를 시작 하려면 사용 합니다 [az aks 찾아보기][az-aks-browse] 명령입니다. 다음 예제에서는 *myResourceGroup* 리소스 그룹에 *myAKSCluster*라는 클러스터의 대시보드를 엽니다.
...
바인딩을 만들려면 사용 합니다 [kubectl 만들 clusterrolebinding][kubectl-create-clusterrolebinding] 다음 예와에서 같이 명령입니다. 
...
>
> 다른 인증 방법에 대 한 자세한 내용은 Kubernetes 대시보드 wiki를 참조 하세요 [액세스 제어][dashboard-authentication]입니다.

```console
kubectl create clusterrolebinding kubernetes-dashboard --clusterrole=cluster-admin --serviceaccount=kube-system:kubernetes-dashboard
```

이제 RBAC 지원 클러스터의 Kubernetes 대시보드에 액세스할 수 있습니다. Kubernetes 대시보드를 시작 하려면 사용 합니다 [az aks 찾아보기][az-aks-browse] 이전 단계에 설명 된 대로 명령입니다.
...
Kubernetes 대시보드에 대 한 자세한 내용은 참조는 [Kubernetes 웹 UI 대시보드][kubernetes-dashboard]합니다.
