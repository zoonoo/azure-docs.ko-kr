...
업그레이드 프로세스 진행중에는 AKS 노드가 개별적으로 클러스터에서 중단(cordon)되므로 새 pod는 여기에 스캐줄링 되지 않습니다. 그리고, 노드는 다음과 같이 이동(drain)되고 업그레이드됩니다.

- 새 노드가 노드 풀에 배포됩니다. 이 노드에서는 최신 운영 체제 이미지 및 패치를 실행합니다.
- 업그레이드를 위해 기존 노드 중 하나를 식별 합니다. Pod는 이 노드에서 정상적으로 종료되며 노드 풀의 다른 노드로 스캐줄링됩니다.
- 기존 노드는 AKS 클러스터에서 삭제됩니다.
- 클러스터에 있는 다음 노드는 업그레이드 과정의 일부분으로 모든 노드가 성공적으로 대체 될 때까지 같은 과정으로 중단(cordon)과 이동(drain)을 합니다.
