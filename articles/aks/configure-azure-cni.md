...
* 서비스 주체 대신 관리되는 ID를 할당된 시스템에서 사용 권한에 사용할 수 있습니다. 자세한 내용은 [관리되는 ID 사용을](use-managed-identity.md)참조하십시오.
* AKS 노드 풀에 할당된 서브넷은 [위임된 서브넷일](../virtual-network/subnet-delegation-overview.md)수 없습니다.
...
>   - 이 고려 사항은 Windows Server 노드 풀(현재 AKS에서 미리 보기)을 사용할 때 특히 중요합니다. AKS의 Windows 서버 노드는 Windows 업데이트를 자동으로 적용하지 않고 노드 풀에서 업그레이드를 수행합니다. 이 업그레이드는 최신 Window Server 2019 기본 노드 이미지 및 보안 패치를 통해 새 노드를 배포합니다. Windows 서버 풀 업그레이드에 대한 자세한 내용은 [AKS의 노드 풀 업그레이드를][nodepool-upgrade]참조하십시오.
>
...
| Kubernetes 서비스 주소 범위 | 이 범위는 이 가상 네트워크 또는 이 가상 네트워크에 연결된 모든 네트워크 요소에서 사용하지 말아야 합니다. 서비스 주소 CIDR은 /12보다 작아야 합니다. 다른 AKS 클러스터에서 이 범위를 재사용할 수 있습니다. |
...
| Docker 브리지 주소 | Docker 브리지 네트워크 주소는 모든 Docker 설치에 있는 기본 *docker0* 브리지 네트워크 주소를 나타냅니다. *docker0* 브리지는 AKS 클러스터 또는 포드 자체에서 사용되지 않지만 AKS 클러스터 내에서 docker *빌드와* 같은 시나리오를 계속 지원하려면 이 주소를 설정해야 합니다. Docker는 다른 CIDR과 충돌할 수 있는 서브넷을 자동으로 선택하므로 Docker 브리지 네트워크 주소에 대한 CIDR을 선택해야 합니다. 클러스터의 서비스 CIDR 및 포드 CIDR을 포함하여 네트워크의 나머지 CIDR과 충돌하지 않는 주소 공간을 선택해야 합니다. 172.17.0.1/16의 기본값 다른 AKS 클러스터에서 이 범위를 재사용할 수 있습니다. |

## <a name="maximum-pods-per-node"></a>노드당 최대 포드

AKS 클러스터의 노드당 최대 포드 수는 250개입니다. 노드당 *기본* 최대 pod 수는 *Kubenet* 및 *Azure CNI* 네트워킹과 클러스터 배포 방법에 따라 다릅니다.

| 배포 방법 | Kubenet 기본 | Azure CNI 기본 | 배포 시 구성 가능 |
| -- | :--: | :--: | -- |
| Azure CLI | 110 | 30 | 예(최대 250개) |
| Resource Manager 템플릿 | 110 | 30 | 예(최대 250개) |
| 포털 | 110 | 30 | 예 |

### <a name="configure-maximum---new-clusters"></a>최댓값 구성 - 새 클러스터

*클러스터 배포 시간에만* 노드당 최대 Pod 수를 구성할 수 있습니다. Azure CLI 또는 리소스 관리자 템플릿을 사용 하 고 배포 하는 경우 노드 당 최대 포드를 최대 250으로 설정할 수 있습니다.

클러스터 상태에 중요한 시스템 포드의 공간을 보장하기 위해 노드당 최대 포드에 대한 최소 값이 적용됩니다. 각 노드 풀의 구성에 최소 30개의 포드를 위한 공간이 있는 경우에만 노드당 최대 포드에 대해 설정할 수 있는 최소 값은 10입니다. 예를 들어 노드당 최대 포드를 최소 10개로 설정하려면 각 개별 노드 풀에 최소 3개의 노드가 있어야 합니다. 이 요구 사항은 생성된 각 새 노드 풀에도 적용되므로 10개가 노드당 최대 포드로 정의된 경우 추가된 각 후속 노드 풀에는 최소 3개의 노드가 있어야 합니다.
...
> [!NOTE]
> 위의 표의 최소 값은 AKS 서비스에 의해 엄격하게 적용됩니다. maxPods 값을 표시된 최소값보다 낮게 설정할 수 없으므로 클러스터가 시작되지 않을 수 있습니다.
...
**Kubernetes 서비스 주소 범위**: Kubernetes가 클러스터의 내부 [서비스에][services] 할당하는 가상 IP 집합입니다. 다음 요구 사항을 충족하는 모든 프라이빗 주소 범위를 사용할 수 있습니다.

* 클러스터의 가상 네트워크 IP 주소 범위에 속하지 않아야 합니다.
* 클러스터 가상 네트워크가 피어링된 다른 가상 네트워크와 겹치지 않아야 합니다.
* 온-프레미스 IP와 겹치지 않아야 합니다.
* 범위 내에 있지 않아야 `169.254.0.0/16` `172.30.0.0/16`합니다. `172.31.0.0/16``192.0.2.0/24`
...
**Docker 브리지 주소**: Docker 브리지 네트워크 주소는 모든 Docker 설치에 있는 기본 *docker0* 브리지 네트워크 주소를 나타냅니다. *docker0* 브리지는 AKS 클러스터 또는 포드 자체에서 사용되지 않지만 AKS 클러스터 내에서 docker *빌드와* 같은 시나리오를 계속 지원하려면 이 주소를 설정해야 합니다. Docker는 다른 CIDR과 충돌할 수 있는 서브넷을 자동으로 선택하므로 Docker 브리지 네트워크 주소에 대한 CIDR을 선택해야 합니다. 클러스터의 서비스 CIDR 및 포드 CIDR을 포함하여 네트워크의 나머지 CIDR과 충돌하지 않는 주소 공간을 선택해야 합니다.
...
* *포드별 네트워크 정책을 구성할 수 있습니까?*

  예, Kubernetes 네트워크 정책은 AKS에서 사용할 수 있습니다. 시작하려면 [AKS의 네트워크 정책을 사용하여 포드 간의 보안 트래픽을][network-policy]참조하십시오.
...
* *AKS 클러스터 생성 중에 만든 서브넷에 대한 추가 속성을 구성하려면 어떻게 해야 합니까? 예를 들어 서비스 끝점입니다.*

  AKS 클러스터를 만드는 동안 만든 가상 네트워크와 서브넷에 대한 속성의 전체 목록은 Azure Portal의 표준 가상 네트워크 구성 페이지에서 구성할 수 있습니다.

* **Kubernetes 서비스 주소 범위에**클러스터 *가상 네트워크 내에서 다른 서브넷을 사용할 수 있습니까?*
