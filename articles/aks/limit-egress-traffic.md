...
AKS 클러스터는 기본적으로 아웃 바운드 (송신) 인터넷을 무제한 권한이 있습니다. 이 수준의 네트워크 액세스에는 노드 및 필요에 따라 외부 리소스에 액세스 하기 위해 실행 하는 서비스 수 있습니다. 송신 트래픽을 제한 하려는 경우에 제한 된 수의 포트 및 주소 정상 클러스터 유지 관리 작업을 유지 하기 위해 액세스할 수 있어야 합니다. 그런 다음 클러스터를만 Microsoft 컨테이너 레지스트리 (MCR) 또는 하지 외부 공용 리포지토리에 Azure 컨테이너 레지스트리 (ACR)에서 기본 시스템 컨테이너 이미지를 사용 하도록 구성 됩니다.
...
사용할 수 있습니다 [Azure 방화벽] [ azure-firewall] 또는 타사 방화벽 어플라이언스를 송신 트래픽이 보호 및 정의 하는 데 필요한 이러한 포트 및 주소입니다.
...
* TCP 포트 *443*
* TCP 포트 *9000* API 서버의 터널 종료 통신할 터널 프런트 pod에 대 한 합니다.
...
다음 아웃 바운드 포트 / 네트워크 규칙이 올바르게 작동 하려면 AKS 클러스터에 필요 하지 않지만 것이 좋습니다.
