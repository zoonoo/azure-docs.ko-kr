...
- **LoadBalancer** - Azure 부하 분산 장치 리소스를 만들고, 외부 IP 주소를 구성하고, 요청된 Pod를 부하 분산 장치 백 엔드 풀에 연결합니다. 고객의 트래픽이 응용 프로그램에 도달할 수 있도록 원하는 포트에 로드 분산 규칙이 만들어집니다. 
...
*내부* 및 *외부* 부하 분산 장치를 모두 만들 수 있습니다. 내부 로드 밸로드솔더는 개인 IP 주소만 할당되므로 인터넷에서 액세스할 수 없습니다.
...
노드는 [kubenet][kubenet] Kubernetes 플러그인을 사용합니다. Azure 플랫폼에서 가상 네트워크를 만들고 구성하거나 기존 가상 네트워크 서브넷에 AKS 클러스터를 배포하도록 선택할 수 있습니다. 다시 말하지만 노드만 라우팅 가능한 IP 주소를 수신하고 포드는 NAT를 사용하여 AKS 클러스터 외부의 다른 리소스와 통신합니다. 이 방법을 사용하면 네트워크 공간에서 Pod가 사용하도록 예약해야 하는 IP 주소의 수가 크게 줄어듭니다.
...
Azure CNI를 사용하면 모든 Pod가 서브넷에서 IP 주소를 가져오고 직접 액세스가 가능합니다. 이러한 IP 주소는 네트워크 공간에서 고유해야 하며 미리 계획되어야 합니다. 각 노드에는 지원하는 최대 Pod 수에 대한 구성 매개 변수가 있습니다. 그러면 노드당 동일한 IP 주소 수가 해당 노드에 대해 미리 예약됩니다. 이 접근 방식은 IP 주소 소모또는 응용 프로그램 요구가 증가함에 따라 더 큰 서브넷에서 클러스터를 다시 빌드해야 할 필요가 있으므로 더 많은 계획이 필요합니다.
...
kubenet과 Azure CNI는 모두 AKS 클러스터에 대한 네트워크 연결을 제공합니다. 그러나 각각의 장점과 단점이 있습니다. 높은 수준에서 다음 고려 사항이 적용됩니다.

* **쿠베넷 (주)**
    * IP 주소 공간을 절약합니다.
    * Kubernetes 내부 또는 외부 로드 밸런서를 사용하여 클러스터 외부에서 포드에 도달합니다.
    * 사용자 정의 경로(DR)를 수동으로 관리하고 유지 관리해야 합니다.
    * 클러스터당 최대 400개의 노드.
* **Azure CNI**
    * 포드는 완전한 가상 네트워크 연결을 얻고 연결된 네트워크에서 개인 IP 주소를 통해 직접 도달 할 수 있습니다.
...
| 기능                                                                                   | 쿠베넷 (주)   | Azure CNI |
|----------------------------------------------------------------------------------------------|-----------|-----------|
| 기존 또는 새 가상 네트워크에 클러스터 배포                                            | 지원 - DR이 수동으로 적용 | 지원됨 |
| 포드 포드 연결                                                                         | 지원됨 | 지원됨 |
| 포드 VM 연결; 동일한 가상 네트워크의 VM                                          | 창에 의해 시작될 때 작동 | 두 가지 방식으로 작동 |
| 포드 VM 연결; 피어블로된 가상 네트워크의 VM                                            | 창에 의해 시작될 때 작동 | 두 가지 방식으로 작동 |
| VPN 또는 익스프레스 경로를 사용하여 온-프레미스 액세스                                                | 창에 의해 시작될 때 작동 | 두 가지 방식으로 작동 |
| 서비스 끝점에 의해 보호되는 리소스에 대한 액세스                                             | 지원됨 | 지원됨 |
| 로드 밸러버 서비스, 앱 게이트웨이 또는 침투 컨트롤러를 사용하여 Kubernetes 서비스 노출 | 지원됨 | 지원됨 |
| 기본 Azure DNS 및 개인 영역                                                          | 지원됨 | 지원됨 |

DNS에 관해서는, kubenet 및 Azure CNI 플러그인 DNS는 CoreDNS, AKS에서 실행되는 데몬 세트에 의해 제공됩니다. Kubernetes의 CoreDNS에 대한 자세한 내용은 [DNS 서비스 사용자 지정을](https://kubernetes.io/docs/tasks/administer-cluster/dns-custom-nameservers/)참조하십시오. CoreDNS는 알 수 없는 도메인을 노드 DNS 서버로 전달하도록 기본값에 따라 구성되며, 즉 AKS 클러스터가 배포된 Azure 가상 네트워크의 DNS 기능으로 전달됩니다. 따라서 Azure DNS 및 개인 영역은 AKS에서 실행되는 포드에 대해 작동합니다.

### <a name="support-scope-between-network-models"></a>네트워크 모델 간의 지원 범위

사용하는 네트워크 모델에 관계없이 kubenet 및 Azure CNI는 다음 방법 중 하나로 배포할 수 있습니다.

* Azure 플랫폼은 AKS 클러스터를 만들 때 가상 네트워크 리소스를 자동으로 만들고 구성할 수 있습니다.
* AKS 클러스터를 만들 때 가상 네트워크 리소스를 수동으로 만들고 구성하고 해당 리소스에 연결할 수 있습니다.

서비스 끝점 또는 DNI와 같은 기능은 kubenet 및 Azure CNI모두에서 지원되지만 [AKS에 대한 지원 정책은][support-policies] 수행할 수 있는 변경 사항을 정의합니다. 예를 들어:

* AKS 클러스터에 대한 가상 네트워크 리소스를 수동으로 만드는 경우 자체 DR 또는 서비스 끝점을 구성할 때 지원됩니다.
* Azure 플랫폼에서 AKS 클러스터에 대한 가상 네트워크 리소스를 자동으로 만드는 경우 AKS 관리 리소스를 수동으로 변경하여 자체 DR 또는 서비스 끝점을 구성하는 것은 지원되지 않습니다.
...
또한 AKS 클러스터의 컨테이너에 대한 요청에 대한 클라이언트 소스 IP를 유지하도록 받는 컨트롤러를 구성할 수도 있습니다. 클라이언트의 요청이 받는 컨트롤러를 통해 AKS 클러스터의 컨테이너로 라우팅되면 해당 요청의 원래 원본 IP를 대상 컨테이너에서 사용할 수 없습니다. *클라이언트 소스 IP 보존을*사용하도록 설정하면 클라이언트의 소스 IP를 *X-Forwarded-For*아래의 요청 헤더에서 사용할 수 있습니다. 침입 컨트롤러에서 클라이언트 소스 IP 보존을 사용하는 경우 SSL 통과를 사용할 수 없습니다. 클라이언트 소스 IP 보존 및 SSL 통과는 *로드밸서* 유형과 같은 다른 서비스와 함께 사용할 수 있습니다.

## <a name="network-security-groups"></a>네트워크 보안 그룹

네트워크 보안 그룹은 AKS 노드와 같은 VM의 트래픽을 필터링합니다. LoadBalancer와 같은 Service를 만들 때 Azure 플랫폼은 필요한 모든 네트워크 보안 그룹 규칙을 자동으로 구성합니다. AKS 클러스터의 Pod에 대한 트래픽을 필터링하는 네트워크 보안 그룹 규칙은 수동으로 구성하지 않습니다. Kubernetes Service 매니페스트의 일부로 필요한 포트 및 전달을 정의하고, Azure 플랫폼에서 적절한 규칙을 만들거나 업데이트하도록 합니다. 다음 섹션에서 설명한 대로 네트워크 정책을 사용하여 트래픽 필터 규칙을 창에 자동으로 적용할 수도 있습니다.
...
네트워크 정책은 AKS에서 사용할 수 있는 Kubernetes 기능으로, 포드 간의 트래픽 흐름을 제어할 수 있습니다. 할당된 레이블, 네임스페이스 또는 트래픽 포트와 같은 설정에 따라 트래픽을 허용하거나 거부하도록 선택할 수 있습니다. 네트워크 보안 그룹은 pod가 아닌 AKS 노드에 더 적합합니다. 네트워크 정책을 통해 트래픽 흐름을 제어하기 위한 더 적합한 클라우드 네이티브 방법을 사용할 수 있습니다. Pod는 AKS 클러스터에서 동적으로 생성되므로 필요한 네트워크 정책을 자동으로 적용할 수 있습니다.
...
관련 모범 사례는 [AKS의 네트워크 연결 및 보안에 대한 모범 사례를][operator-best-practices-network]참조하십시오.
