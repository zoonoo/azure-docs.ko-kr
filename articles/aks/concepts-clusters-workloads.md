...
노드 리소스는 AKS에서 클러스터의 일부로 노드 기능을 수행 하는 데 활용 됩니다. 이 사용은 AKS에서 사용 될 때 노드의 총 리소스와 할당 된 리소스 간의 불일치를 만들 수 있습니다. 이 정보는 사용자가 배포 된 pod에 대 한 요청 및 제한을 설정할 때 주의 해야 합니다.
...
예약 된 두 가지 유형의 리소스는 다음과 같습니다.

- **Cpu** 예약 cpu는 노드 유형 및 클러스터 구성에 따라 다르며,이로 인해 추가 기능 실행으로 인해 할당 가능한 cpu가 줄어들 수 있습니다.
...
- **메모리** -AKS에서 사용 하는 메모리는 두 값의 합계를 포함 합니다.
   ...
   1. 컨테이너 만들기 및 종료를 관리 하기 위해 kubelet 데몬이 모든 Kubernetes agent 노드에 설치 됩니다. 기본적으로 AKS에서이 디먼은 다음 제거 규칙을 포함 합니다 *. 메모리. 750Mi<사용할 수 있습니다*. 즉, 항상 노드에 750 mi를 항상 할당 해야 함을 의미 합니다.  호스트가 사용 가능한 메모리의 임계값 보다 낮은 경우 kubelet는 실행 중인 pod 중 하나를 종료 하 여 호스트 컴퓨터에서 메모리를 확보 하 고 보호 합니다. 사용 가능한 메모리가 750Mi 임계값을 초과 하면이 작업이 트리거됩니다.
...
위의 메모리 및 CPU 할당 규칙은 클러스터 상태에 중요 한 일부 호스팅 시스템 pod 포함 하 여 에이전트 노드를 정상 상태로 유지 하는 데 사용 됩니다. 이러한 할당 규칙으로 인해 노드가 Kubernetes 클러스터의 일부가 아닌 경우에 비해 할당 된 메모리와 CPU의 수가 줄어듭니다. 위의 리소스 예약은 변경할 수 없습니다.
...
Pod는 논리적인 리소스이지만, 컨테이너는 애플리케이션 워크로드가 실행되는 위치입니다. Pod는 일반적으로 사용 후 삭제 가능한 리소스이며, 개별적으로 예약된 Pod에는 Kubernetes에서 제공하는 고가용성 및 중복성 기능 중 일부가 누락됩니다. 대신 배포 컨트롤러와 같은 Kubernetes *컨트롤러* 에서 pod을 배포 하 고 관리 합니다.
...
애플리케이션에서 관리 결정을 위해 인스턴스의 쿼럼을 항상 사용할 수 있어야 하는 경우 업데이트 프로세스에서 해당 기능을 중단하지 않도록 합니다. *Pod 중단 예산* 은 업데이트 또는 노드 업그레이드 중에 중단할 수 있는 배포의 복제본 수를 정의하는 데 사용할 수 있습니다. 예를 들어 배포에 *5* 개의 복제본이 있는 경우 pod 중단을 *4* 로 정의 하 여 한 번에 하나의 복제본만 삭제/예약 되지 않도록 할 수 있습니다. Pod 리소스 제한과 마찬가지로 항상 최소 개수의 복제본이 존재하도록 요구하는 애플리케이션에 Pod 중단 예산을 정의하는 것이 가장 좋습니다.

배포는 일반적으로 `kubectl create` 또는 `kubectl apply`를 사용하여 만들어지고 관리됩니다. 배포를 만들려면 매니페스트 파일을 YAML(YAML Ain't Markup Language) 형식으로 정의합니다. 다음 예제에서는 NGINX 웹 서버의 기본 배포를 만듭니다. 배포에서는 만들 3 *개의* 복제본을 지정 하 고 컨테이너에서 포트 *80* 을 열어야 합니다. CPU 및 메모리에 대한 리소스 요청 및 제한도 정의됩니다.
