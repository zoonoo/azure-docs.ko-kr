...
다른 호스트 OS, 컨테이너 런타임을 사용하거나 사용자 지정 패키지를 포함해야 하는 경우 [aks-engine][aks-engine]을 사용하여 사용자 고유의 Kubernetes 클러스터를 배포할 수 있습니다. 업스트림 `aks-engine`은 AKS 클러스터에서 공식적으로 지원하기 전에 기능을 릴리스하여 구성 옵션이 제공됩니다. 예를 들어 모비(Moby) 이외의 컨테이너 런타임을 사용하려면 `aks-engine`을 구성하여 Kubernetes 클러스터를 배포할 수 있습니다.
...
AKS 클러스터의 여러 노드 풀을 포함 하는 경우 지정된 된 리소스에 대 한 사용 하는 노드 풀 Kubernetes 스케줄러를 지시 해야 합니다. 예를 들어, 수신 컨트롤러 (현재 AKS에서 미리 보기)는에서 Windows Server 노드에서 실행 하지 않아야 합니다. 노드 선택 기가 pod를 예약 해야 하는 컨트롤에 OS, 노드 등 다양 한 매개 변수를 정의할 수 있습니다.
...
AKS의 상태 비저장 애플리케이션 대부분은 개별 Pod를 예약하는 것이 아니라 배포 모델을 사용해야 합니다. Kubernetes는 배포의 상태를 모니터링하여 필요한 수의 복제본이 클러스터 내에서 실행되는지 확인할 수 있습니다. 만 개별 pod를 예약할 때 pod에 문제가 발생 하며 현재 노드에 해당 문제가 발생 하는 경우에서 정상 노드로 다시 예약 되지 경우 다시 시작 되지 않습니다.
