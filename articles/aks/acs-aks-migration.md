...
* AKS 노드 [는 관리 디스크](../virtual-machines/windows/managed-disks-overview.md)를 사용 합니다.
    ...
    * Azure `StorageClass` 디스크에 대 한 사용자 지정 개체를 `unmanaged` 에서 `managed`로 변경 해야 합니다.
    * Any `PersistentVolumes` 는를 `kind: Managed`사용 해야 합니다.
* AKS는 [여러 노드 풀](https://docs.microsoft.com/azure/aks/use-multiple-node-pools) (현재 미리 보기 상태)을 지원 합니다.
...
기존 CI/CD 파이프라인을 사용 하 여 AKS에 알려진 양호한 구성을 배포 하는 것이 좋습니다. 기존 배포 작업을 복제 하 고이 `kubeconfig` 새 AKS 클러스터를 가리키는지 확인 합니다.
...
* [Velero](https://github.com/heptio/ark) 이 도구를 사용 하려면 Kubernetes 1.7가 필요 합니다.
...
2. YAML 정의를 필요한 대로 변경 합니다. 예를 들어를 `apps/v1beta1` `apps/v1`로바꿉니다. `Deployments`

3. [볼륨 마이그레이션](#migrating-persistent-volumes) (선택 사항) ACS 클러스터에서 AKS 클러스터로
