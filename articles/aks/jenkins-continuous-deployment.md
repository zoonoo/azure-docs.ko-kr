...
빌드 프로세스 중에는 GitHub 리포지토리가 Jenkins 빌드 서버에 복제됩니다. 새 컨테이너 이미지가 빌드되어 ACR 레지스트리로 푸시됩니다. 마지막으로, AKS 클러스터에서 실행 중인 Azure 투표 애플리케이션이 새 이미지를 사용하도록 업데이트됩니다. 응용 프로그램 코드에는 변경된 내용이 없으므로 웹 브라우저에서 앱 예제를 보는 경우 응용 프로그램은 변경되지 않습니다.
