...
* 미리 보기 중 `outboundType` 에는 클러스터를 만들 때만 정의할 수 있으며 나중에 업데이트할 수 없습니다.
* 미리 보기 중 `outboundType` 에 AKS 클러스터는 AZURE cni를 사용 해야 합니다. Kubenet 구성 가능, 사용에는 AKS 서브넷에 대 한 경로 테이블의 수동 연결이 필요 합니다.
* 을 `outboundType` 설정 `vm-set-type` 하려면의 `VirtualMachineScaleSets` 및 `load-balancer-sku` 의가 포함 된 `Standard`AKS 클러스터가 필요 합니다.
* 값 `outboundType` 을로 설정 하려면 `UDR` 클러스터에 대 한 유효한 아웃 바운드 연결을 사용 하는 사용자 정의 경로가 필요 합니다.
* 값 `outboundType` 으로 설정 하면 부하 `UDR` 분산 장치로 라우팅되는 수신 원본 IP가 클러스터의 나가는 송신 대상 주소 **와 일치 하지** 않을 수 있음을 의미 합니다.

## <a name="overview-of-outbound-types-in-aks"></a>AKS의 아웃 바운드 형식 개요

AKS 클러스터는 고유한 `outboundType` 유형의 부하 분산 장치 또는 사용자 정의 라우팅을 사용 하 여 사용자 지정할 수 있습니다.
...
가 `loadBalancer` 설정 된 경우 AKS는 다음 설치를 자동으로 완료 합니다. 부하 분산 장치는 AKS 할당 된 공용 IP를 통해 송신 하는 데 사용 됩니다. 의 `loadBalancer` 아웃 바운드 형식은 AKS 리소스 공급자에서 만든 `loadBalancer`부하 분산 장치에서 송신 하는 것으로 간주 되는 형식의 Kubernetes services를 지원 합니다.
...
다음은 기본적으로 `outboundType` 의 `loadBalancer`를 사용 하는 AKS 클러스터에 배포 되는 네트워크 토폴로지입니다.
...
이제 이전 명령 `APPID` 출력 `PASSWORD` 에 의해 자동으로 생성 되는 서비스 사용자 appid 및 서비스 주체 암호를 사용 하 여 아래를 바꿉니다. AKS에서 리소스를 배포할 수 있도록 서비스 주체에 대 한 사용 권한을 부여 하는 VNET 리소스 ID를 참조 합니다.
...
마지막으로, AKS 클러스터를 클러스터 전용의 기존 서브넷에 배포할 수 있습니다. 에 배포할 대상 서브넷은 환경 변수를 `$SUBNETID`사용 하 여 정의 됩니다. 이전 단계에서 변수 `$SUBNETID` 를 정의 하지 않았습니다. 서브넷 ID의 값을 설정 하려면 다음 명령을 사용할 수 있습니다.
...
```
 ...
 [Az aks][az-aks-get-credentials] 명령을 사용 하 여 새로 만든 Kubernetes 클러스터 `kubectl` 에 연결 하도록를 구성 합니다. 
...
아래 yaml을 이라는 `example.yaml`파일로 복사 하 여 Azure 투표 앱 응용 프로그램을 배포 합니다.
