...
애플리케이션 성능 요구 및 액세스 패턴을 이해하여 적절한 스토리지 계층을 선택합니다. 관리 되는 디스크 크기와 성능 계층에 대 한 자세한 내용은 참조 하세요. [Azure Managed Disks 개요][managed-disks]

### <a name="create-and-use-storage-classes-to-define-application-needs"></a>스토리지 클래스를 만들고 사용하여 애플리케이션 요구 정의

사용하는 스토리지 유형은 Kubernetes *스토리지 클래스*를 사용하여 정의합니다. 그런 후에는 스토리지 클래스가 pod 또는 배포 사양에서 참조됩니다. 적절한 스토리지를 만들고 pod에 연결하는 데 이러한 정의가 함께 사용됩니다. 자세한 내용은 [AKS에 대 한 저장소 클래스][aks-concepts-storage-classes]합니다.
...
사용 가능한 VM 크기에 대 한 자세한 내용은 참조 하세요. [Azure에서 Linux 가상 머신 크기][vm-sizes]합니다.
...
동적으로 만들고 볼륨을 사용 하는 방법에 대 한 개념을 참조 하세요 [영구적 볼륨 클레임][aks-concepts-storage-pvcs]합니다.

작업에서 이러한 볼륨을 확인 하려면 동적으로 만들고 사용 하 여 영구 볼륨을 사용 하는 방법을 참조 하세요 [Azure Disks][dynamic-disks] or [Azure Files][dynamic-files]합니다.
...
저장소 클래스 옵션에 대 한 자세한 내용은 참조 하세요. [저장소 확보 정책을][reclaim-policy]합니다.
...
애플리케이션이 디스크 또는 파일에 유지되는 데이터를 저장 및 사용할 경우에는 해당 데이터의 백업 또는 스냅샷을 정기적으로 생성해야 합니다. Azure 디스크는 기본 제공 스냅샷 기술을 사용할 수 있습니다. 스냅샷 작업을 수행하기 전에 애플리케이션이 디스크에 쓰기를 플러시하기 위한 후크가 필요할 수 있습니다. [Velero][velero] can back up persistent volumes along with additional cluster resources and configurations. If you can't [remove state from your applications][remove-state], 영구적 볼륨에서 데이터를 백업 하 고 필요한 프로세스 및 데이터 무결성을 확인 하 고 복원 작업을 정기적으로 테스트 합니다.

다양한 데이터 백업 방법의 제한 사항을 이해하고 스냅샷 전에 데이터를 중지해야 하는지를 확인합니다. 데이터 백업을 수행해도 클러스터 배포의 애플리케이션 환경을 복원하지 못할 수 있습니다. 이러한 시나리오에 대 한 자세한 내용은 참조 하세요. [AKS에 비즈니스 연속성 및 재해 복구에 대 한 유용한][best-practices-multi-region]합니다.

## <a name="next-steps"></a>다음 단계

이 문서에서는 AKS의 스토리지 모범 사례를 중점적으로 설명했습니다. Kubernetes에서 저장소 기본 사항에 대 한 자세한 내용은 참조 하세요. [AKS에서 응용 프로그램에 대 한 저장소 개념][aks-concepts-storage]합니다.
