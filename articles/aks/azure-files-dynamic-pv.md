...
영구적 볼륨은 Kubernetes Pod와 함께 사용하기 위해 프로비전된 저장소 부분을 나타냅니다. 하나 이상의 Pod에서 영구적 볼륨을 사용할 수 있으며 동적 또는 정적으로 프로비전할 수 있습니다. 여러 pod를 동일한 저장소 볼륨에 대 한 동시 액세스에 필요한 경우 사용 하 여 연결 하려면 Azure Files를 사용할 수 있습니다 합니다 [서버 메시지 블록 (SMB) 프로토콜][smb-overview]합니다. 이 문서에서는 AKS(Azure Kubernetes Service) 클러스터에서 여러 Pod에 사용할 Azure Files공유를 동적으로 만드는 방법을 설명합니다.
...
저장소 클래스는 Azure 파일 공유를 만드는 방법을 정의하는 데 사용됩니다. 저장소 계정에 자동으로 만들어집니다 합니다 [노드 리소스 그룹][node-resource-group] for use with the storage class to hold the Azure file shares. Choose of the following [Azure storage redundancy][storage-skus] 에 대 한 *skuName*:
...
Azure Files 용 Kubernetes 저장소 클래스에 자세한 내용은 참조 [Kubernetes 저장소 클래스][kubernetes-storage-classes]합니다.

`azure-file-sc.yaml` 파일을 만들고 다음 예제 매니페스트를 복사합니다. 에 대 한 자세한 *mountOptions*를 참조 합니다 [탑재 옵션][mount-options] 섹션입니다.
...
사용 하 여 저장소 클래스를 만듭니다는 [kubectl 적용][kubectl-apply] 명령:
...
AKS 클러스터는 Kubernetes RBAC(역할 기반 액세스 제어)를 사용하여 수행할 수 있는 작업을 제한합니다. *역할*은 부여할 사용 권한을 정의하고 *바인딩*은 원하는 사용자에게 역할을 적용합니다. 이러한 할당은 주어진 네임스페이스 또는 전체 클러스터에 적용될 수 있습니다. 자세한 내용은 [RBAC를 사용 하 여 권한 부여][kubernetes-rbac]합니다.
...
사용 하 여 사용 권한을 할당 합니다 [kubectl 적용][kubectl-apply] 명령:
...
PVC(영구적 볼륨 클레임)는 저장소 클래스 개체를 사용하여 Azure 파일 공유를 동적으로 프로비전합니다. 다음 YAML을 사용하여 크기가 *5GB*이고 *ReadWriteMany* 액세스 권한을 가진 영구적 볼륨 클레임을 만들 수 있습니다. 액세스 모드에 대 한 자세한 내용은 참조는 [Kubernetes 영구적 볼륨][access-modes] 설명서.
...
사용 하 여 영구적 볼륨 클레임 만들기 합니다 [kubectl 적용][kubectl-apply] 명령:

```console
kubectl apply -f azure-file-pvc.yaml
```

완료되면 파일 공유가 생성됩니다. 연결 정보 및 자격 증명을 포함하고 있는 Kubernetes 비밀도 생성됩니다. 사용할 수는 [kubectl get][kubectl-get] PVC의 상태를 확인 하는 명령:
...
사용 하 여 pod를 만들 합니다 [kubectl 적용][kubectl-apply] 명령입니다.
