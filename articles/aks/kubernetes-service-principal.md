...
ms.locfileid: "67615120"
---
# <a name="service-principals-with-azure-kubernetes-service-aks"></a>AKS(Azure Kubernetes Service)를 사용하는 서비스 주체

Azure Api와 상호 작용을 AKS 클러스터 필요는 [Azure Active Directory (AD) 서비스 주체][aad-service-principal]합니다. 서비스 주체는 Azure 부하 분산 장치 또는 컨테이너 레지스트리(ACR)와 같은 다른 Azure 리소스를 동적으로 만들고 관리하는 데 필요합니다.
...
만들 때 AKS 클러스터를 azure에서 포털 또는 사용 하 여는 [az aks 만들기][az-aks-create] 명령, Azure 서비스 주체를 자동으로 생성할 수 있습니다.
...
Azure CLI를 사용 하 여 서비스 주체를 만들려면 수동으로 사용 합니다 [az ad sp 만들기 rbac 용으로][az-ad-sp-create] 명령입니다. 다음 예제에서 `--skip-assignment` 매개 변수는 추가 기본 할당이 할당되는 것을 방지합니다.
...
사용 하 여 AKS 클러스터를 만들 때 기존 서비스 주체를 사용 하는 [az aks 만들기][az-aks-create] 명령에 사용 하 여는 `--service-principal` 및 `--client-secret` 매개 변수를 지정 합니다 `appId` 및 `password` 의 출력에서는 [az ad sp 만들기 rbac 용][az-ad-sp-create] 명령:
...
ACR(Azure Container Registry)을 컨테이너 이미지 저장소로 사용하는 경우 이미지를 읽고 끌어오기 위해서는 AKS 클러스터에 대한 사용 권한을 부여해야 합니다. AKS 클러스터의 서비스 주체에는 레지스트리에서 *Reader* 역할이 위임되어야 합니다. 자세한 단계를 참조 하세요 [ACR에 대 한 권한 부여 AKS 액세스][aks-to-acr]합니다.

### <a name="networking"></a>네트워킹

가상 네트워크와 서브넷 또는 공용 IP 주소가 다른 리소스 그룹에 있는 고급 네트워킹을 사용할 수 있습니다. 다음 역할 권한 집합 중 하나를 할당합니다.

- 만들기는 [사용자 지정 역할][rbac-custom-role] 다음 역할 권한을 정의 합니다.
...
- 또는 할당 된 [Storage 계정 참여자][rbac-storage-contributor] 리소스 그룹에 기본 제공 역할
...
- 사용 하는 경우는 [az aks 만듭니다][az-aks-create] 서비스 주체를 자동으로 생성 하는 명령을 서비스 주체 자격 증명 파일에 기록 됩니다 `~/.azure/aksServicePrincipal.json` 명령을 실행 하는 데 사용 하는 컴퓨터입니다.
- 가 만든 AKS 클러스터를 삭제 하는 경우 [az aks 만들기][az-aks-create]를 자동으로 생성 된 서비스 주체는 삭제 되지 않습니다.
    - 서비스 주체를 삭제 하려면 클러스터에 대 한 쿼리 *servicePrincipalProfile.clientId* 하 고 사용 하 여 delete [az ad app delete][az-ad-app-delete]합니다. 다음 리소스 그룹과 클러스터 이름을 고유한 값으로 바꿉니다.
...
Azure CLI에서 서비스 주체 자격 증명을 AKS 클러스터에 대 한 캐시 됩니다. 이러한 자격 증명이 만료 되는 경우 AKS 클러스터를 배포 하는 오류가 발생 합니다. 실행 하는 경우 다음 오류 메시지가 [az aks 만들기][az-aks-create] 캐시 된 서비스 주체 자격 증명을 사용 하 여 문제를 나타낼 수 있습니다.
