<properties
	...
	description="아티팩트를 사용하여 DevTest Lab에 VM을 추가하는 방법 알아보기"
...
기본 이미지([사용자 지정 이미지](./devtest-lab-create-template.md) 또는 마켓플레이스 이미지)를 사용하여 DevTest Lab에 VM을 만듭니다.
...
1. 기본 이미지를 선택하고 **확인**을 누르면 **사용자 이름**, **인증 유형**(선택한 기본 이미지의 OS 유형이 Linux인 경우), **암호**(*암호*의 인증 유형으로 가정)를 비롯한 사용자 계정 정보를 지정하는 UI 요소가 포함되도록 **랩 VM** 블레이드가 확장됩니다.
...
1. **가상 네트워크**를 누르고 원하는 VNET을 선택합니다.

1. **서브넷**을 누르고 서브넷을 선택합니다.

1. 선택한 서브넷에 공용 IP 주소를 허용하도록 랩 정책이 설정되어 있으면 **예** 또는 **아니요**를 선택하여 IP 주소를 공용으로 할 것인지 여부를 지정합니다. 선택한 서브넷에 공용 IP 주소를 허용하도록 랩 정책이 설정되어 있지 않으면 이 옵션이 비활성화되고 **아니요**가 선택됩니다.
