<properties
	pageTitle="랩에 대한 가상 네트워크 구성 | Microsoft Azure"
	description="기존 가상 네트워크 및 서브넷을 구성하고 DevTest Lab을 통해 VM에서 사용하는 방법에 대해 알아봅니다."
...
[아티팩트를 사용하여 랩에 VM 추가](devtest-lab-add-vm-with-artifacts.md) 문서에 설명된 대로 랩에 VM을 만들 때 해당 VM에 대해 구성된 가상 네트워크(및 서브넷)을 지정할 수 있습니다. 이 작업을 수행하는 시나리오 중 하나는 Express 경로 또는 사이트 간 VPN으로 설정된 가상 네트워크를 사용하여 VM에서 회사 네트워크 리소스에 액세스할 수 있게 되기를 원하는 경우입니다. 다음 섹션에서는 VM을 만들 때 선택할 수 있도록 랩의 가상 네트워크 설정으로 기존 가상 네트워크를 추가하는 방법을 보여 줍니다.
