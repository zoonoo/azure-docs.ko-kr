...
랩에서 VM의 [사용자 지정 아티팩트를 만들거나](devtest-lab-artifact-author.md) [Azure 리소스 관리자 템플릿을 사용하여 사용자 지정 테스트 환경을 만들려는](devtest-lab-create-environment-from-arm.md) 경우 팀이 만드는 아티팩트 또는 Azure 리소스 관리자 템플릿을 포함할 개인 Git 리포지토리도 추가해야 합니다. 리포지토리는 [GitHub](https://github.com) 또는 [VSTS(Visual Studio Team Services)](https://visualstudio.com)에서 호스팅될 수 있습니다.
...
VM을 만들 경우 Azure 리소스 관리자 템플릿을 저장하고 원할 경우 사용자 지정한 다음 나중에 추가 VM을 손쉽게 만들 수 있습니다. 사용자 지정 Azure 리소스 관리자 템플릿을 저장할 개인 리포지토리를 만들어야 합니다.  
...
1. 아티팩트 또는 Azure 리소스 관리자 템플릿 정의를 포함하는 GitHub 리포지토리의 홈 페이지로 이동합니다.
...
7. 두 번째 **리포지토리** 페이지에서 다음 정보를 지정합니다.
   ...
   * **폴더 경로** - 아티팩트 또는 Azure 리소스 관리자 템플릿 정의가 포함된 복제 URL에 상대적인 폴더 경로를 하나 이상 입력합니다. 하위 디렉터리를 지정하는 경우 폴더 경로에 슬래시를 포함해야 합니다.
...
* [Azure 리소스 관리자 템플릿으로 다중 VM 환경 및 PaaS 리소스를 만든](devtest-lab-create-environment-from-arm.md) 다음 개인 리포지토리에 템플릿을 저장합니다.
