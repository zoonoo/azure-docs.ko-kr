...
이 자습서에서는 각 구독에 대해 다른 계정을 사용합니다. 두 구독 모두에 대해 권한이 있는 계정을 사용할 경우 모든 단계에 동일한 계정을 사용하고, 포털 로그아웃 절차와 가상 네트워크에 다른 사용자 권한을 할당하는 절차를 생략할 수 있습니다. 다음 단계 중 하나를 완료하기 전에 미리 보기에 등록해야 합니다. 등록하려면 이 문서의 [미리 보기에 등록](#register) 섹션의 절차를 완료합니다. 두 구독 모두 미리 보기에 등록되기 전에는 남은 단계를 진행하지 않습니다.
...
9. **Save**를 클릭합니다.
10. UserA를 포털에서 로그아웃한 다음 UserB로 로그인합니다.
...
15. 나타나는 **myVnetB** 블레이드의 왼쪽에 있는 세로 옵션 목록에서 **속성**을 클릭합니다. 이후 단계에서 사용하는 **RESOURCE ID**를 복사합니다. 리소스 ID는 /subscriptions/<Susbscription ID>/resourceGroups/myResoureGroupB/providers/Microsoft.ClassicNetwork/virtualNetworks/myVnetB와 유사합니다.
16. MyVnetB에 대해 5-9단계를 완료하고 8단계에서 **UserA**를 입력합니다.
17. UserB를 포털에서 로그아웃한 다음 UserA로 로그인합니다.
...
다음 단계 중 하나를 완료하기 전에 미리 보기에 등록해야 합니다. 등록하려면 이 문서의 [미리 보기에 등록](#register) 섹션의 절차를 완료합니다. 두 구독 모두 미리 보기에 등록되기 전에는 남은 단계를 진행하지 않습니다.

1. 가상 네트워크(클래식)를 만들려면 Azure 명령줄 인터페이스 1.0을 [설치](../cli-install-nodejs.md?toc=%2fazure%2fvirtual-network%2ftoc.json)합니다.
...
4. 다음 명령을 입력하여 가상 네트워크(클래식)를 만듭니다.
    ...
    ```
5. 나머지 단계는 Azure 명령줄 인터페이스 2.0.4 이상이 [설치된](/cli/azure/install-azure-cli?toc=%2fazure%2fvirtual-network%2ftoc.json) Bash 셸이나 Azure Cloud Shell을 사용하여 완료해야 합니다. Azure Cloud Shell은 Azure Portal에서 직접 실행할 수 있는 평가판 Bash 셸입니다. Azure 명령줄 인터페이스가 사전 설치되어 계정에서 사용하도록 구성되어 있습니다. 다음 스크립트에서 **사용해보기** 단추를 클릭하면 Azure 계정에 로그인하는 Cloud Shell이 열립니다. Windows 클라이언트에서 Bash CLI 스크립트 실행과 관련된 옵션은 [Windows에서 Azure 명령줄 인터페이스 실행](../virtual-machines/windows/cli-options.md?toc=%2fazure%2fvirtual-network%2ftoc.json)을 참조하세요. 
...
7. Azure에서 UserB를 로그아웃하고 CLI 2.0에 UserA로 로그인합니다. 
...
7. UserB로 Azure에서 로그아웃하고 `login-azurermaccount` 명령을 입력하여 UserA의 구독에 UserA로 로그인합니다.  로그인하는 데 사용하는 계정에 가상 네트워크 피어링을 만드는 데 필요한 권한이 있어야 합니다. 자세한 내용은 이 문서의 [권한](#permissions) 섹션을 참조하세요.
...
2. 가상 네트워크(클래식)를 삭제하기 위해 다음 명령을 통해 Azure 명령줄 인터페이스 1.0을 사용하여 Azure에 로그인합니다.
