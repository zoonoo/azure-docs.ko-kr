<properties 
   pageTitle="Preview 포털을 사용하여 클래식 모드에서 정적 개인 IP를 설정하는 방법 | Microsoft Azure"
   description="정적 개인 IP(DIP) 및 포털을 사용하여 클래식 모드에서 관리 방법 이해"
...
# Preview 포털에서 정적 개인 IP 주소(클래식) 설정 방법
...
2. **새로 만들기** > **계산** > **Windows Server 2012 R2 Datacenter**를 클릭하고 **배포 모델 선택** 목록에 **클래식**이 이미 표시되는지 확인한 후 **만들기**를 클릭합니다.
	...
	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure01.png)

3. **VM 만들기** 블레이드에서 만들 VM의 이름(이 시나리오에서는 *DNS01*), 로컬 관리자 계정 및 암호를 입력합니다.

	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure02.png)
...
4. **옵션 구성** > **네트워크** > **가상 네트워크**를 클릭하고 **TestVNet**을 클릭합니다. **TestVNet**을 사용할 수 없는 경우 *미국 중부* 위치를 사용 중이고 이 문서의 시작 부분에서 설명한 테스트 환경을 만들었는지 확인합니다.
	...
	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure03.png)

5. **네트워크** 블레이드에서 현재 선택된 서브넷이 *FrontEnd*인지 확인하고 **IP 주소 할당** 아래에서 **IP 주소**를 클릭하고 **정적**을 클릭한 후 아래와 같이 **IP 주소**에 대해 *192.168.1.101*을 입력합니다.

	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure04.png)
...
7. **VM 만들기** 블레이드에서 **만들기**를 클릭합니다. 대시보드에 아래 타일이 표시되는지 확인합니다.
	...
	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure05.png)

## VM의 정적 개인 IP 주소 정보를 검색하는 방법

위의 단계를 사용하여 만든 VM에 대한 정적 개인 IP 주소를 보려면 아래 단계를 실행합니다.

1. Azure Preview 포털에서 **모두 찾아보기** > **가상 컴퓨터(클래식)** > **DNS01** > **모든 설정** > **IP 주소**를 클릭하고 아래와 같이 IP 주소 할당 및 IP 주소를 확인합니다.

	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure06.png)
...
1. 위에 표시된 **IP 주소** 블레이드에서 **IP 주소 할당** 오른쪽에서 **동적**을 클릭한 후 **저장**, **예**를 차례로 클릭합니다.
	...
	![Preview 포털에서 VM 만들기](./media/virtual-networks-static-ip-classic-pportal/figure07.png)
