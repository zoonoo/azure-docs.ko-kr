...
2017 년 11 월 15 일 이후에 생성 된 종 량 제 구독의 경우 이러한 구독 내의 Vm에서 직접 전송 되는 전자 메일을 차단 하는 기술 제한이 있습니다. 인증 된 SMTP 릴레이를 사용 하지 않고 Azure Vm에서 외부 전자 메일 공급자로 직접 전자 메일을 보내고 지불 기록과 관련 된 계정이 있는 경우 Azure Portal에서 Azure Virtual Network 리소스에 대 한 **진단 및 해결** 블레이드의 **연결** 섹션에서 제한을 제거 하도록 요청할 수 있습니다. 자격이 있는 경우 구독을 사용 하도록 설정 하거나 다음 단계에 대 한 지침을 받게 됩니다. 
...
구독이 제외 되 고 Vm이 중지 되 고 Azure Portal에서 시작 된 후에는 해당 구독의 모든 Vm이 제외 됩니다. 예외는 요청 된 구독에만 적용 되 고 인터넷으로 직접 라우팅되는 VM 트래픽에만 적용 됩니다.
...
여전히 도움이 필요한 경우 **기술** Virtual Network 연결에서 전자 메일을 보낼 수 없음 (SMTP/포트 25)을 사용 하 여 문제를 신속 하 게 해결 하려면 [지원 서비스에 문의 하세요](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade) .  >  **Virtual Network**  >  **Connectivity**  >  **Cannot send email (SMTP/Port 25)**
