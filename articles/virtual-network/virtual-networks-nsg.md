...
위의 그림에서는 두 서브넷이 있는 가상 컴퓨터와 4대의 VM(각 서브넷에 2대)를 보여줍니다. *백엔드* 서브넷의 VM에는 이와 직접 관련된 공용 IP(PIP가 있으며 *프런트엔드* 서브넷의 VM은 Azure 부하 분산 장치 뒤에 있습니다. VIP 또는 PIP에서 시작되는 여부와 무관하게 각 서브넷에 연결된 NSG를 사용하여 트래픽이 서브넷에 흐르는 방법을 제어할 수 있습니다.
...
NSG 규칙에는 다음 속성이 포함됩니다.

|속성|설명|샘플 값|
|---|---|---|
|**설명**|규칙에 대한 설명|서브넷 X에서 모든 VM에 인바운드 트래픽 허용|
|**프로토콜**|규칙과 일치하는 프로토콜|TCP, UDP 또는 *| |**원본 포트 범위**|규칙과 일치하는 원본 포트 범위|80, 100-200, *| |**대상 포트 범위**|규칙과 일치하는 대상 포트 범위|80, 100-200, *| |**원본 주소 접두사**|규칙과 일치하는 원본 주소 접두사|10\.10.10.1, 10.10.10.0/24, VIRTUAL\_NETWORK|
|**대상 주소 접두사**|규칙과 일치하는 대상 주소 접두사|10\.10.10.1, 10.10.10.0/24, VIRTUAL\_NETWORK|
|**방향**|규칙과 일치하는 트래픽의 방향|인바운드 또는 아웃바운드|
