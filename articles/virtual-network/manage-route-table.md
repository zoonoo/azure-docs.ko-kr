...
* 포털을 사용 하는 경우 열 https://portal.azure.com, 및 Azure 계정으로 로그인 합니다.<br>
...
로그인 또는 연결을 사용 하 여 Azure 계정에 할당 되어야 합니다는 [네트워크 참가자](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor) 역할 또는 [사용자 지정 역할](../role-based-access-control/custom-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json) 에 나열 된 적절 한 조치를 할당 된 [권한 ](#permissions).
...
1. 경로 테이블의 **이름**을 입력하고 **구독**을 선택하고 새 **리소스 그룹**을 만들거나 기존 리소스 그룹을 선택하고 **위치**를 선택한 후 **만들기**를 선택합니다. VPN gateway 통해 온-프레미스 네트워크에 연결 된 가상 네트워크의 서브넷에 경로 테이블 연결을 계획 하 고 사용 하지 않도록 설정한 경우 **가상 네트워크 게이트웨이 경로 전파**, 온-프레미스 경로 되지 서브넷에 네트워크 인터페이스에 전파 합니다.

### <a name="create-route-table---commands"></a>경로 테이블-명령 만들기
...
### <a name="view-details-of-route-table---commands"></a>명령 경로 테이블의 세부 정보 보기
...
### <a name="change-a-route-table---commands"></a>경로 테이블-명령을 변경합니다
...
서브넷에는 0개 또는 1개의 경로 테이블이 연결될 수 있습니다. 경로 테이블은 0개 또는 여러 개의 서브넷에 연결할 수 있습니다. 경로 테이블은 가상 네트워크에 연결되지 않기 때문에 경로 테이블을 연결하려는 각 서브넷에 경로 테이블을 연결해야 합니다. 서브넷에서 나가는 모든 트래픽은 경로 테이블 내에서 만든 경로에 따라 라우팅됩니다 [기본 경로](virtual-networks-udr-overview.md#default), 경로 Azure 가상 네트워크 게이트웨이 (가상 네트워크를 연결할 경우 온-프레미스 네트워크에서 전파 ExpressRoute 또는 VPN)입니다. 경로 테이블과 동일한 Azure 위치 및 구독에 있는 가상 네트워크의 서브넷에만 경로 테이블을 연결할 수 있습니다.
...
### <a name="associate-a-route-table---commands"></a>경로 테이블을 연결-명령
...
### <a name="dissociate-a-route-table---commands"></a>경로 테이블을 분리할-명령
...
### <a name="delete-a-route-table---commands"></a>경로 테이블-명령 삭제
...
### <a name="view-details-of-a-route---commands"></a>경로-명령 세부 정보 보기
...
### <a name="change-a-route---commands"></a>경로-명령을 변경합니다
...
### <a name="delete-a-route---commands"></a>경로-명령 삭제
...
### <a name="validate-routing-between-two-endpoints---commands"></a>명령 두 끝점 간의 라우팅 유효성 검사
