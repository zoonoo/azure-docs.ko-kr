---
title: Azure 저장소로 데이터 유출 제한 - Azure PowerShell
description: 이 문서에서는 Azure PowerShell을 사용하는 가상 네트워크 서비스 끝점 정책을 사용하여 가상 네트워크 데이터 유출을 Azure Storage 리소스로 제한하고 제한하는 방법을 알아봅니다.
...
---
# <a name="manage-data-exfiltration-to-azure-storage-accounts-with-virtual-network-service-endpoint-policies-using-azure-powershell"></a>Azure PowerShell을 사용하여 가상 네트워크 서비스 엔드포인트 정책을 사용하여 Azure 저장소 계정에 대한 데이터 유출 관리
...
* 가상 네트워크를 만듭니다.
* 서브넷을 추가하고 Azure 저장소에 대한 서비스 끝점을 활성화합니다.
...
가상 네트워크에서 서브넷을 만듭니다. 이 예제에서는 *Private*라는 서브넷이 *Microsoft.Storage*에 대한 서비스 엔드포인트로 생성됩니다. 
...
[New-AzNetworkSecurityRuleConfig를](/powershell/module/az.network/new-aznetworksecurityruleconfig)사용하여 네트워크 보안 그룹 보안 규칙을 만듭니다. 다음 규칙을 사용하면 Azure Storage 서비스에 지정된 공용 IP 주소에 대한 아웃바운드 액세스가 허용됩니다. 
...
네트워크 보안 그룹을 [Set-AzVirtualNetworkSubnetConfig를](/powershell/module/az.network/set-azvirtualnetworksubnetconfig) 사용하여 *개인* 서브넷에 연결한 다음 서브넷 구성을 가상 네트워크에 씁니다. 다음 예제에서는 *myNsgPrivate* 네트워크 보안 그룹을 *프라이빗* 서브넷에 연결합니다.
...
[New-AzStorageAccount를](/powershell/module/az.storage/new-azstorageaccount)사용하여 Azure 저장소 계정을 만듭니다.
...
저장소 계정을 만든 후 저장소 계정의 키를 [Get-AzStorageAccountKey를](/powershell/module/az.storage/get-azstorageaccountkey)통해 변수로 검색합니다.
...
이제 위의 단계를 반복하여 두 번째 저장소 계정을 만듭니다.
...
또한 나중에 파일 공유를 만드는 데 사용할 수 있도록 이 계정에서 저장소 계정 키를 검색합니다.

```azurepowershell-interactive
$storageAcctKey2 = (Get-AzStorageAccountKey -ResourceGroupName myResourceGroup -AccountName $storageAcctName2).Value[0]
```

### <a name="create-a-file-share-in-each-of-the-storage-account"></a>각 저장소 계정에서 파일 공유 만들기

[New-AzStorageContext를](/powershell/module/az.storage/new-AzStoragecontext)사용하여 저장소 계정 및 키에 대한 컨텍스트를 만듭니다. 이 컨텍스트는 스토리지 계정 이름 및 계정 키를 캡슐화합니다.
...
[New-AzStorageShare를](/powershell/module/az.storage/new-azstorageshare)사용하여 파일 공유 만들기:
...
### <a name="deny-all-network-access-to-a-storage-accounts"></a>저장소 계정에 대한 모든 네트워크 액세스 거부

기본적으로 스토리지 계정은 네트워크에 있는 클라이언트의 네트워크 연결을 허용합니다. 선택한 네트워크에 대한 액세스를 제한하려면 기본 작업을 [Update-AzStorageAccountNetworkRuleSet을](/powershell/module/az.storage/update-azstorageaccountnetworkruleset)사용하여 *거부로* 변경합니다. 네트워크 액세스가 거부되면 네트워크에서 스토리지 계정에 액세스할 수 없습니다.
...
### <a name="enable-network-access-only-from-the-vnet-subnet"></a>VNet 서브넷에서만 네트워크 액세스 활성화

[Get-AzVirtualNetwork를](/powershell/module/az.network/get-azvirtualnetwork) 통해 생성된 가상 네트워크를 검색한 다음 [Get-AzVirtualNetworkSubnetConfig를](/powershell/module/az.network/get-azvirtualnetworksubnetconfig)통해 개인 서브넷 개체를 변수로 검색합니다.
...
[Add-AzStorageAccountNetworkRule을](/powershell/module/az.network/add-aznetworksecurityruleconfig)사용하여 개인 서브넷의 *저장소* 계정에 대한 네트워크 액세스를 허용합니다.
...
가상 네트워크의 사용자가 안전하고 허용되는 Azure Storage 계정에만 액세스할 수 있도록 하려면 정의에서 허용된 저장소 계정 목록이 있는 서비스 끝점 정책을 만들 수 있습니다. 그런 다음 이 정책은 서비스 끝점을 통해 저장소에 연결된 가상 네트워크 서브넷에 적용됩니다.

### <a name="create-a-service-endpoint-policy"></a>서비스 엔드포인트 정책 만들기

이 섹션에서는 서비스 끝점에 대한 액세스에 대해 허용된 리소스 목록으로 정책 정의를 만듭니다.

첫 번째(허용된) 저장소 계정에 대한 리소스 ID 검색 

```azurepowershell-interactive
$resourceId = (Get-AzStorageAccount -ResourceGroupName myresourcegroup -Name $storageAcctName1).id
```

위의 리소스를 허용하도록 정책 정의를 만듭니다.
...
위에서 만든 정책 정의를 사용하여 서비스 끝점 정책 만들기
...
### <a name="associate-the-service-endpoint-policy-to-the-virtual-network-subnet"></a>서비스 끝점 정책을 가상 네트워크 서브넷에 연결

서비스 끝점 정책을 만든 후 대상 서브넷과 Azure Storage의 서비스 끝점 구성과 연결합니다.
...
[New-AzVM을](/powershell/module/az.compute/new-azvm)사용하여 *개인* 서브넷에 가상 컴퓨터를 만듭니다. 다음 명령을 실행하면 자격 증명을 묻는 메시지가 표시됩니다. 입력하는 값은 VM에 대한 사용자 이름과 암호로 구성됩니다. `-AsJob` 옵션은 백그라운드에서 VM을 만들므로 다음 단계를 계속 진행할 수 있습니다.
...
### <a name="confirm-access-to-the-allowed-storage-account"></a>*허용된* 저장소 계정에 대한 액세스 확인
...
*myVmPrivate* VM에서 허용된 저장소 계정에서 Azure 파일 공유를 매핑하여 PowerShell을 사용하여 Z를 구동합니다. 
...
### <a name="confirm-access-is-denied-to-non-allowed-storage-account"></a>*허용되지 않는* 저장소 계정에 대한 액세스가 거부됨 확인

동일한 *myVmPrivate* VM에서 Azure 파일 공유를 매핑하여 X를 구동하려고 시도합니다. 
...
공유에 대한 액세스가 거부되고 `New-PSDrive : Access is denied` 오류가 수신됩니다. 저장소 계정이 *허용되지 않음계정 서비스가* 종료지점 정책의 허용된 리소스 목록에 없기 때문에 액세스가 거부됩니다. 
