...
2. 포털에서 **더 많은 서비스**를 클릭하고 필터 상자에 *가상 컴퓨터*를 입력하고 **가상 컴퓨터**를 클릭합니다.
3. **가상 컴퓨터** 블레이드에서 IP 주소를 추가하려는 VM을 클릭합니다. 나타나는 가상 컴퓨터 블레이드에서 **네트워크 인터페이스**를 클릭한 다음 IP 주소를 추가할 네트워크 인터페이스를 선택합니다. 다음 그림의 예제에서는 *myVM*이라는 VM에서 *myNIC*라는 NIC가 선택됩니다.
