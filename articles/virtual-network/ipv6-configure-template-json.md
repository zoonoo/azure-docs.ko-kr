---
title: Azure virtual network에서 기본 Load Balancer를 사용 하 여 IPv6 이중 스택 응용 프로그램 배포-리소스 관리자 템플릿
...
이 문서에서는에 적용 되는 Azure Resource Manager VM 템플릿의 일부가 포함 된 IPv6 구성 작업 목록을 제공 합니다. 이 문서에서 설명 하는 템플릿을 사용 하 여 이중 스택 (ipv4 + ipv6) 응용 프로그램을 IPv4 및 IPv6 서브넷을 포함 하는 이중 스택 가상 네트워크, 이중 (IPv4 + IPv6) 프런트 엔드 구성으로 기본 Load Balancer, 이중 IP 구성, 네트워크 보안 그룹 및 공용 Ip를 포함 하는 Nic가 있는 Vm을 포함 하는 기본 Load Balancer를 배포 합니다.
