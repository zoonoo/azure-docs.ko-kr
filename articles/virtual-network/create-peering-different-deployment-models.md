...
이 자습서에서는 서로 다른 배포 모델을 통해 만들어진 가상 네트워크 간의 가상 네트워크 피어링을 만드는 방법을 알아봅니다. 두 가상 네트워크는 동일한 구독에 있어야 합니다. 두 가상 네트워크를 피어링하면 서로 다른 가상 네트워크에 있는 리소스가 같은 가상 네트워크에 있는 리소스인 것처럼 같은 대역폭 및 대기 시간으로 서로 통신할 수 있습니다. [가상 네트워크 피어링에](virtual-network-peering-overview.md)대해 자세히 알아보십시오.
...
Azure 포털, CLI(Azure [명령줄 인터페이스),](#cli) Azure [PowerShell](#powershell)또는 Azure 리소스 관리자 템플릿을 사용하여 가상 네트워크 피어링을 만들 수 있습니다. 앞의 도구 링크 중 원하는 도구 링크를 클릭하여 원하는 도구를 사용하여 가상 네트워크 피어링을 만드는 단계로 바로 이동하세요.

## <a name="create-peering---azure-portal"></a>피어링 만들기 - Azure Portal

1. [Azure 포털에](https://portal.azure.com)로그인합니다. 로그인하는 데 사용하는 계정에 가상 네트워크 피어링을 만드는 데 필요한 권한이 있어야 합니다. 사용 권한 목록은 [가상 네트워크 피어링 사용 권한](virtual-network-manage-peering.md#requirements-and-constraints)을 참조하세요.
...
3. **가상 네트워크 만들기** 블레이드에서 다음 설정에 대한 값을 입력하거나 선택한 다음 **만들기**를 클릭합니다.
    ...
    - **위치**: *미국 동부*
4. **+ 새**. **Marketplace 검색** 상자에 *가상 네트워크*를 입력합니다. **가상 네트워크**가 검색 결과에 표시되면 클릭합니다.
...
6. **가상 네트워크 만들기** 블레이드에서 다음 설정에 대한 값을 입력하거나 선택한 다음 **만들기**를 클릭합니다.
    ...
    - **구독**: 사용자의 구독을 선택합니다.
    - **리소스 그룹**: **기존 사용을** 선택하고 *myResourceGroup을* 선택합니다.
    - **위치**: *미국 동부*
...
1. 포털 검색 상자에 **myResourceGroup**을 입력합니다. 검색 결과에서 **myResourceGroup을 클릭합니다.**
2. myResourceGroup 블레이드에서 **삭제** 아이콘을 **클릭합니다.**
3. 삭제를 확인하려면 **리소스 그룹 이름 유형** 상자에서 **myResourceGroup**을 입력한 다음 **삭제를**클릭합니다.
