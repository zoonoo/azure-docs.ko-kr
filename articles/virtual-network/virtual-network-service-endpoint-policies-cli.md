...
허용 되는 저장소 계정에 대 한 리소스 URI를 변수에 저장 합니다. 아래 명령을 실행 하기 전에 *-subscription id >\<* 을 구독 id의 실제 값으로 바꿉니다.
...
만든 디렉터리에 Azure 파일 공유를 탑재합니다. 아래 명령을 실행 하기 전에 *\<저장소 계정 키 >* 를 **$saConnectionString 1**의 값 *AccountKey* 으로 바꿉니다.
...
아래 명령을 실행 하기 전에 *\<저장소 계정 키 >* 를 **$saConnectionString 2**의 값 *AccountKey* 으로 바꿉니다.

```bash
sudo mount --types cifs //notallowedstorageacc.file.core.windows.net/my-file-share /mnt/MyAzureFileShare2 --options vers=3.0,username=notallowedstorageacc,password=<storage-account-key>,dir_mode=0777,file_mode=0777,serverino
```

이 저장소 계정은 서브넷에 적용 한 서비스 끝점 정책의 허용 목록에 없으므로 액세스가 거부 되 고 `mount error(13): Permission denied` 오류가 발생 합니다. 
