...
* 데이터 센터를 안전 하 게 확장 합니다. VNet을 사용하여 기존의 사이트 간(S2S) VPN을 빌드하여 데이터 센터 용량을 안전하게 확장할 수 있습니다. S2S VPN은 IPSEC를 사용하여 회사 VPN 게이트웨이와 Azure 간의 보안 연결을 제공합니다.
...
아닙니다. VNet과 함께 IPv6를 사용할 수 없습니다. 단, Azure 부하 분산 장치에 IPv6 주소를 할당하여 가상 머신의 부하를 분산할 수 있습니다. 자세한 내용은 [Azure Load Balancer의 IPv6에 대한 개요](../load-balancer/load-balancer-ipv6-overview.md?toc=%2fazure%2fvirtual-network%2ftoc.json)를 참조하세요.
...
예. VNet에 대한 DNS 서버 목록을 언제든지 변경할 수 있습니다. DNS 서버 목록을 변경 하는 경우 새 DNS 설정을 적용 하려면 VNet에서 영향을 받는 모든 Vm에 대해 DHCP 임대 갱신을 수행 해야 합니다. Windows OS를 실행 하는 Vm의 경우 VM에서 직접 `ipconfig /renew`를 입력 하 여이 작업을 수행할 수 있습니다. 다른 OS 유형에 대 한 자세한 내용은 특정 OS 유형에 대 한 DHCP 임대 갱신 설명서를 참조 하세요. 
...
두 가상 네트워크가 서로 다른 지역 (글로벌 VNet 피어 링)에 있는 경우 기본 Load Balancer를 사용 하는 리소스에 연결할 수 없습니다. 표준 Load Balancer를 사용 하는 리소스에 연결할 수 있습니다.
다음 리소스는 기본 부하 분산 장치를 사용 하며,이는 글로벌 VNet 피어 링을 통해 통신할 수 없음을 의미 합니다.
...
첫 번째 단계는 네트워크 쪽 작업이고, 두 번째 단계는 서비스 리소스 쪽 작업입니다. 두 단계 모두 관리자 역할에 부여된 RBAC 권한을 기준으로 동일한 관리자 또는 서로 다른 관리자가 수행할 수 있습니다. Azure 서비스 쪽에서 VNet ACL을 설정하기 전에 먼저 가상 네트워크에 대한 서비스 엔드포인트를 켜는 것이 좋습니다. 따라서이 단계는 VNet 서비스 끝점을 설정 하기 위해 위에 나열 된 순서 대로 수행 해야 합니다.
...
Azure 서비스를 가상 네트워크 내의 여러 서브넷 또는 여러 가상 네트워크에서 보호 하려면 각 서브넷에서 네트워크 쪽의 서비스 끝점을 개별적으로 사용 하도록 설정 하 고 Azure 서비스 리소스를 모든 서브넷에 대해 보호 합니다. Azure 서비스 측에서 적절 한 VNet Acl.
