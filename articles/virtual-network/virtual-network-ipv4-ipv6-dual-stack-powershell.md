...
---
# <a name="deploy-an-ipv6-dual-stack-application-using-basic-load-balancer---powershell-preview"></a>기본 로드 밸런서 - PowerShell(미리 보기)을 사용하여 IPv6 듀얼 스택 응용 프로그램 배포

이 문서에서는 듀얼 스택 가상 네트워크 및 서브넷, 듀얼 (IPv4 + IPv6) 프런트 엔드 구성이있는 기본 로드 밸러저, NIC가있는 VM을 포함하는 Azure PowerShell을 사용하여 기본 로드 밸러서를 사용하여 이중 스택 (IPv4 + IPv6) 응용 프로그램을 배포하는 방법을 보여 주며, 이중 IP 구성, 네트워크 보안 그룹 및 공용 IP를 가지고 있습니다.
