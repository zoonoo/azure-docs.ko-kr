---
title: 클래식 배포 모델에서 PowerShell을 사용하여 라우팅 제어 및 가상 어플라이언스 사용 | Microsoft Docs
...
1. **`New-AzureRouteTable`** cmdlet을 실행하여 프런트 엔드 서브넷에 대한 경로 테이블을 만듭니다.
...
2. **`Set-AzureRoute`** cmdlet을 실행하여 위에서 만든 경로 테이블에 경로를 만들고 백 엔드 서브넷(192.168.2.0/24)으로 보내진 모든 트래픽을 **FW1** VM(192.168.0.4)으로 보냅니다.
...
3. **`Set-AzureSubnetRouteTable`** cmdlet을 실행하여 위에서 만든 경로 테이블을 **FrontEnd** 서브넷에 연결합니다.
...
1. **`New-AzureRouteTable`** cmdlet을 실행하여 백 엔드 서브넷에 대한 경로 테이블을 만듭니다.
   
        New-AzureRouteTable -Name UDR-BackEnd `
            -Location uswest `
            -Label "Route table for back end subnet"
2. **`Set-AzureRoute`** cmdlet을 실행하여 위에서 만든 경로 테이블에 경로를 만들고 프런트 엔드 서브넷(192.168.1.0/24)으로 보내진 모든 트래픽을 **FW1** VM(192.168.0.4)으로 보냅니다.
   
        Get-AzureRouteTable UDR-BackEnd `
            |Set-AzureRoute -RouteName RouteToFrontEnd -AddressPrefix 192.168.1.0/24 `
            -NextHopType VirtualAppliance `
            -NextHopIpAddress 192.168.0.4
3. **`Set-AzureSubnetRouteTable`** cmdlet을 실행하여 위에서 만든 경로 테이블을 **BackEnd** 서브넷에 연결합니다.
