...
* 다음 세 개의 서브넷이 있는 Virtual Network “CorpNetwork”: “SecNet”, “FrontEnd” 및 “BackEnd”
...
1. 방화벽 규칙을 설정합니다. 아래 섹션에서 설명합니다. 방화벽 규칙 설명
2. 또는 참조 섹션의 두 스크립트를 사용하여 간단한 웹 애플리케이션을 사용하는 웹 서버와 앱 서버를 설정하여 이 DMZ 구성을 이용한 테스트를 허용합니다.

스크립트가 성공적으로 실행되면 방화벽 규칙을 완료해야 합니다. 이 내용은 다음 섹션에서 설명합니다. 방화벽 규칙
...
> [!NOTE]
> ExpressRoute 및 VPN Gateway와 함께 UDR(사용자 정의 라우팅)를 사용하여 아웃바운드 및 인바운드 크로스-프레미스 트래픽을 NVA(네트워크 가상 어플라이언스)로 강제로 라우팅할 수 있습니다.
> 
> 
...
2. 다음 홉이 방화벽으로 정의된 Virtual Network 트래픽, 이 경우 로컬 VNet 트래픽이 직접 라우팅되도록 하는 기본 규칙을 재정의합니다.
...
* 외부 규칙(인바운드 트래픽용):
  ...
  3. 애플리케이션 트래픽 규칙: 프런트 엔드 웹 트래픽용 규칙과 백 엔드 트래픽(예: 웹 서버에서 데이터 계층으로)용 규칙의 2가지 애플리케이션 트래픽 규칙이 있습니다. 이러한 규칙은 서버를 배치하는 네트워크 아키텍처 및 트래픽 흐름(트래픽이 흐르는 방향, 사용하는 포트)에 따라 다르게 구성합니다.
     * 첫 번째 규칙은 실제 애플리케이션 트래픽이 애플리케이션 서버에 도달하도록 허용합니다. 다른 규칙이 보안, 관리 등을 허용하지만 애플리케이션 규칙은 외부 사용자 또는 서비스가 애플리케이션에 액세스하도록 허용하는 규칙입니다. 이 예제에서는 포트 80에 단일 웹 서버가 있으므로 단일 방화벽 애플리케이션 규칙이 인바운드 트래픽을 외부 IP로 리디렉션한 다음, 웹 서버의 내부 IP 주소로 리디렉션합니다. 리디렉션된 트래픽 섹션은 내부 서버로 NAT 작업이 수행됩니다.
     * 두 번째 애플리케이션 트래픽 규칙은 웹 서버가 임의 포트를 통해 AppVM01 서버(AppVM02 제외)와 통신하도록 하는 백 엔드 규칙입니다.
* 내부 규칙(인트라-VNet 트래픽용)
...
> [!TIP]
> 두 번째 애플리케이션 트래픽 규칙의 경우 이 예제에서는 쉬운 설명을 위해 모든 포트를 허용하지만, 실제 시나리오에서는 이 규칙의 공격 표면을 줄이기 위해 가장 구체적인 포트와 주소 범위를 사용해야 합니다.
> 
> 
...
방화벽을 실행하는 Virtual Machine은 공용 엔드포인트여야 한다는 한 가지 필수 조건을 충족해야 합니다. 방화벽이 트래픽을 처리하려면 적절한 공용 엔드포인트를 열어야 합니다. 이 예제에는 1) 방화벽 및 방화벽 규칙을 관리하는 관리 트래픽, 2) Windows Server를 관리하는 RDP 트래픽, 3) 애플리케이션 트래픽의 세 가지 유형의 트래픽이 있습니다. 이러한 트래픽 유형은 위 방화벽 규칙의 논리적 보기에서 상반부에 표시된 세 가지 주요 트래픽 유형입니다.
...
관리 클라이언트를 PC에 설치하여 방화벽을 관리하고 필요에 따라 구성을 만들어야 합니다. 디바이스를 관리하는 방법은 방화벽(또는 다른 NVA) 공급업체의 설명서를 참조하세요. 이 섹션 및 다음 섹션인 방화벽 규칙 만들기의 나머지 부분에서는 공급업체 관리 클라이언트를 통한 방화벽 자체(즉, Azure 포털 또는 PowerShell 아님) 구성에 대해 설명합니다.
...
애플리케이션 리디렉션 규칙: ![애플리케이션 리디렉션 아이콘][7]
...
* **애플리케이션 트래픽 규칙**: 프런트 엔드 웹 트래픽용 규칙과 백 엔드 트래픽(예: 웹 서버에서 데이터 계층으로)용 규칙의 2가지 애플리케이션 트래픽 규칙이 있습니다. 이러한 규칙은 서버를 배치하는 네트워크 아키텍처 및 트래픽 흐름(트래픽이 흐르는 방향, 사용하는 포트)에 따라 다릅니다.
  
    가장 먼저 웹 트래픽의 프런트 엔드 규칙에 대해 설명합니다.
  
    ![방화벽 웹 규칙][12]
  
    이 대상 NAT 규칙은 실제 애플리케이션 트래픽이 애플리케이션 서버에 도달하도록 허용합니다. 다른 규칙이 보안, 관리 등을 허용하지만 애플리케이션 규칙은 외부 사용자 또는 서비스가 애플리케이션에 액세스하도록 허용하는 규칙입니다. 이 예제에서는 포트 80에 단일 웹 서버가 있으므로 단일 방화벽 애플리케이션 규칙이 인바운드 트래픽을 외부 IP로 리디렉션한 다음, 웹 서버의 내부 IP 주소로 리디렉션합니다.
    ...
    **참고**: 대상 목록 필드에 할당된 포트가 없으므로 웹 서버의 리디렉션에 인바운드 포트 80(또는 선택한 서비스에 대해 443)을 사용합니다. 웹 서버가 포트 8080과 같은 다른 포트에서 수신 대기 중인 경우 포트 리디렉션도 허용할 수 있도록 대상 목록 필드를 10.0.1.4:8080으로 업데이트할 수 있습니다.
    ...
    다음 애플리케이션 트래픽 규칙은 웹 서버가 임의 서비스를 통해 AppVM01 서버(AppVM02 제외)와 통신하도록 하는 백 엔드 규칙입니다.
...
> [!TIP]
> 이 규칙은 샘플 애플리케이션을 쉽게 설정 및 사용할 수 있도록 "Any" 서비스를 사용합니다. 또한 단일 규칙에 ICMPv4(ping)를 허용합니다. 하지만 권장되는 방식은 아닙니다. 이 경계에서 공격 표면을 줄일 수 있도록 애플리케이션이 가능한 한 포트와 프로토콜("서비스")을 최소한으로 좁혀야 합니다.
> 
> 
...
2. 클라우드 서비스가 포트 80에서 열린 엔드포인트를 통해 10.0.0.4:80의 방화벽 인터페이스로 트래픽을 전달합니다.
...
2. 클라우드 서비스가 포트 8025에서 열린 엔드포인트를 통해 10.0.0.4:8025의 방화벽 인터페이스로 트래픽을 전달합니다.
3. 보안 서브넷에 할당된 NSG가 없으므로 시스템 NSG 규칙에서 방화벽에 대한 트래픽을 허용합니다.
...
2. HTTP 트래픽을 위해 열린 엔드포인트가 없으므로 클라우드 서비스를 통과하지 않고 서버에 도달하지 않습니다.
3. 어떤 이유로 엔드포인트가 열린 경우 프런트 엔드 서브넷의 NSG(인터넷 차단)가 이 트래픽을 차단합니다.
...
3. 어떤 이유로 엔드포인트가 열린 경우 NSG(인터넷 차단)가 이 트래픽을 차단합니다.
...
2. DNS 트래픽을 위해 열린 엔드포인트가 없으므로 클라우드 서비스를 통과하지 않고 서버에 도달하지 않습니다.
3. 어떤 이유로 엔드포인트가 열린 경우 프런트 엔드 서브넷의 NSG 규칙(인터넷 차단)에서 이 트래픽을 차단합니다.
...
3. 어떤 이유로 SQL 엔드포인트가 열린 경우 방화벽에서 규칙 처리를 시작합니다.
   1. FW 규칙 1(FW 관리)이 적용되지 않고 다음 규칙으로 이동합니다.
   2. FW 규칙 2-5(RDP 규칙)이 적용되지 않고 다음 규칙으로 이동합니다.
   3. FW 규칙 6-7(애플리케이션 규칙)이 적용되지 않고 다음 규칙으로 이동합니다.
   4. FW 규칙 8(앱 규칙)이 적용되지 않고 다음 규칙으로 이동합니다.
   5. FW 규칙 9(DNS)이 적용되지 않고 다음 규칙으로 이동합니다.
