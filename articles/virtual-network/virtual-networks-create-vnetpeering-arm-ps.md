...
5. 일단 이 VNET 피어링 링크를 만듭니다. 다음과 같은 연결 상태를 확인할 수 있습니다.
...
그리고 변경 후에 Get-AzureRmVirtualNetworkPeering을 실행하여 속성 값을 다시 확인할 수 있습니다. 위의 cmdlet을 실행한 후에 AllowForwardedTraffic을 True로 변경한 것을 확인할 수 있습니다.
...
PowerShell에서 구독에 VNet 피어링을 만들려면 다음 단계를 따르세요.
...
3. 그런 다음 사용자A의 로그인 세션에서 다음 cmdlet을 실행합니다.
...
4. 그런 다음 사용자B의 로그인 세션에서 다음 cmdlet을 실행합니다.
...
1. 이 시나리오에서는 다음 PowerShell cmdlet을 실행하여 VNet 피어링을 설정할 수 있고 특히 AllowForwardedTraffic 속성을 True로 설정하여 VNET1을 HubVnet에 연결해야 합니다. 이렇게 하면 피어링 VNet 주소 공간의 외부에서 인바운드 트래픽을 허용합니다.
...
1.	VNet 피어링을 제거하기 위해 cmdlet을 실행하여

        Remove-AzureRmVirtualNetworkPeering  
    
    다음과 같이 링크를 모두 제거해야 합니다.

        Remove-AzureRmVirtualNetworkPeering -ResourceGroupName vnet101 -VirtualNetworkName vnet1 -Name linktovnet2 
        Remove-AzureRmVirtualNetworkPeering -ResourceGroupName vnet101 -VirtualNetworkName vnet1 -Name linktovnet2 

3. VNET 피어링에서 하나의 링크만을 제거하면 피어링 연결 상태는 연결이 끊깁니다.
  
4. 이 상태에서는 피어링 연결 상태가 시작됨으로 변경될 때까지 링크를 다시 만들 수 없습니다. VNET 피어링을 다시 만들기 전에 링크를 모두 제거하는 것이 좋습니다.
