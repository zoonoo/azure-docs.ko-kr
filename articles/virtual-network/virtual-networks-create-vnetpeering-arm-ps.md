---
title: "Powershell cmdlet을 사용하여 VNet 피어링 만들기 | Microsoft Docs"
...
---
# <a name="create-vnet-peering-using-powershell-cmdlets"></a>Powershell cmdlet을 사용하여 VNet 피어링 만들기
...
2. VNet 피어링을 설정하려면 각 방향에 하나씩 두 개의 링크를 만들어야 합니다. 다음 단계에서는 먼저 VNet1-VNet2에 VNet 피어링 링크를 만듭니다.
...
3. 이 단계에서는 VNet2-VNet1에 VNet 피어링 링크를 만듭니다.
    ...
    출력은 다음과 같습니다.
        ...
        RemoteGateways        : null
        RemoteVirtualNetworkAddressSpace : null
4. VNet 피어링 링크가 만들어지면 다음에서 링크 상태를 확인할 수 있습니다.
...
1. 구독-A에 대한 권한이 있는 사용자-A 계정으로 Azure에 로그인하고 다음 cmdlet을 실행합니다.
    ...
    이는 요구 사항이 아니며, 요청이 일치하면 사용자가 해당 VNet에 대한 피어링 요청을 개별적으로 요구하는 경우에도 피어링을 설정할 수 있습니다. 다른 VNet의 권한있는 사용자를 로컬 VNet의 사용자로 추가하면 설치가 수월해집니다.
...
1. Azure Resource Manager 가상 네트워크인 **VNET1**의 가상 네트워크 개체를 다음과 같이 해석합니다.
   
        $vnet1 = Get-AzureRmVirtualNetwork -ResourceGroupName vnet101 -Name vnet1
2. 이 시나리오에서 VNet 피어링을 설정하려면 하나의 링크, 특히 **VNET1**에서 **VNET2**로의 링크만 필요합니다. 이 단계에서는 클래식 VNet의 리소스 ID를 알아야 합니다. 리소스 그룹 ID 형식은 다음과 같습니다.
...
2. VNET 피어링에서 한 링크만 제거하면 피어링 링크 상태의 연결이 끊깁니다. 이 상태에서는 피어링 링크 상태가 시작됨으로 변경될 때까지 링크를 다시 만들 수 없습니다. VNet 피어링을 다시 만들기 전에 링크를 모두 제거하는 것이 좋습니다.
