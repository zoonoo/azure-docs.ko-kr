...
description: '빠른 시작: Azure Portal에서 가상 네트워크를 만듭니다. 이러한 네트워크를 사용하면 VM과 같은 Azure 리소스가 서로 인터넷을 통해 안전하게 통신할 수 있습니다.'
author: KumudD
...
1. **서브넷 추가** 를 선택한 다음, **서브넷 이름** 에 대해 *myVirtualSubnet* 을 입력하고 **서브넷 주소 범위** 에 대해 *10.1.0.0/24* 을 입력합니다.
...
1. **가상 머신 만들기 - 기본 사항** 에서 다음 정보를 입력하거나 선택합니다.
    ...
    | 지역 | **미국 동부** 를 선택합니다. |
    | 가용성 옵션 | **인프라 중복이 필요하지 않습니다** 로 기본값이 지정되어 있습니다. |
    | 이미지 | **Windows Server 2019 Datacenter** 로 기본값이 지정되어 있습니다. |
    | 크기 | **표준 DS1 v2** 로 기본값이 지정되어 있습니다. |
    | **관리자 계정** |  |
    ...
    | 인바운드 포트 선택 | *HTTP(80)* 및 *RDP(3389)* 를 입력합니다. |
    | **비용 절감** |  |
    | 이미 Windows 라이선스가 있나요? | **아니오** 로 기본값이 지정되어 있습니다. |
...
1. **가상 머신 만들기 - 네트워킹** 에서 다음 정보를 선택합니다.
    ...
    | 설정 | 값 |
    | ------- | ----- |
    | 가상 네트워크 | **myVirtualNetwork** 로 기본값이 지정되어 있습니다. |
    | 서브넷 | **myVirtualSubnet(10.1.0.0/24)** 으로 기본값이 지정되어 있습니다. |
    | 공용 IP | **(신규) myVm-ip** 로 기본값이 지정되어 있습니다. |
    | NIC 네트워크 보안 그룹 추가 | **기본** 으로 기본값이 지정되어 있습니다. |
    | 공용 인바운드 포트 | **선택한 포트 허용** 으로 기본값이 지정되어 있습니다. |
    | 인바운드 포트 선택 | **HTTP** 및 **RDP** 로 기본값이 지정되어 있습니다.

1. 완료되면 **다음: 관리** 를 선택합니다.
...
1. **확인** 을 선택한 다음, **검토 + 만들기** 를 선택합니다. **검토 + 만들기** 페이지로 이동됩니다. 여기서 구성이 유효한지 검사됩니다.

1. **유효성 검사 통과** 메시지가 표시되면 **만들기** 를 선택합니다.

### <a name="create-the-second-vm"></a>두 번째 VM 만들기

이전 섹션의 절차를 반복하여 다른 가상 머신을 만듭니다.

> [!IMPORTANT]
> **가상 머신 이름** 에 대해 *myVm2* 를 입력합니다.
>
> **진단 스토리지 계정** 에 대해 만드는 대신 **myvmstorageaccount** 를 선택해야 합니다.

## <a name="connect-to-a-vm-from-the-internet"></a>인터넷에서 VM에 연결

*myVm1* 을 만든 후 인터넷에 연결합니다.

1. Azure Portal에서 *myVm1* 을 검색하고 선택합니다.

1. **연결** 을 선택한 다음, **RDP** 를 선택합니다.
...
1. 로그인할 때 인증서 경고가 표시될 수도 있습니다. 인증서 경고가 표시되면 **예** 또는 **계속** 을 선택합니다.
...
> [!NOTE] 
> Azure 서비스는 비용이 듭니다. Azure Cost Management를 사용하면 예산을 설정하고 제어 상태를 유지하도록 경고를 구성할 수 있습니다. Cost Management를 사용하여 Azure 비용을 분석, 관리 및 최적화합니다. 자세한 내용은 [비용 분석에 대한 빠른 시작](../cost-management-billing/costs/quick-acm-cost-analysis.md?WT.mc_id=costmanagementcontent_docsacmhorizontal_-inproduct-learn)을 참조하세요.