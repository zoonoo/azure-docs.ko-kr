...
2. Azure Marketplace에서 **네트워킹** > **가상 네트워크**를 선택합니다.

3. **가상 네트워크 만들기**에서 다음 정보를 입력하거나 선택합니다.
...
4. 나머지 항목은 기본값으로 유지하고 **만들기**를 선택합니다.
...
2. Azure Marketplace에서 **컴퓨팅** > **Windows Server 2019 데이터 센터**를 선택합니다.
...
9. **스토리지 계정 만들기**에서 다음 정보를 입력하거나 선택합니다.
...
11. **검토 + 만들기**를 선택합니다. **검토 + 만들기** 페이지로 이동됩니다. 여기서 구성이 유효한지 검사됩니다.
...
2. **검토 + 만들기**를 선택합니다. **검토 + 만들기** 페이지로 이동되고, Azure에서 구성의 유효성을 검사합니다.

3. **유효성 검사 통과** 메시지가 표시되면 **만들기**를 선택합니다.
...
3. 이후 단계에서 *myVm2*가 *myVm1*을 ping할 수 있도록 하려면 다음 명령을 입력합니다.
...
4. *myVm1*에 대한 원격 데스크톱 연결을 닫습니다.

5. [인터넷에서 VM에 연결](#connect-to-a-vm-from-the-internet)의 단계를 다시 수행하지만 *myVm2*에 연결합니다.

6. 명령 프롬프트에서 `ping myvm1`을 입력합니다.
