---
title: Azure CLI 스크립트 샘플-IPv6 가상 네트워크 끝점 구성-표준 Load Balancer (미리 보기)
...
이 문서에서는 이중 스택 서브넷이 있는 이중 스택 (ipv4 + IPv6) 응용 프로그램, 이중 (IPv4 + IPv6) 프런트 엔드 구성이 있는 표준 Load Balancer, 이중 IP가 있는 Nic가 있는 Vm을 포함 하는 Azure에서 이중 스택 (IPv4 + IPv6) 응용 프로그램을 배포 하는 방법을 보여 줍니다. 구성, 이중 네트워크 보안 그룹 규칙 및 이중 공용 Ip가 있습니다.
