...
3. 다음 설정에 대한 값을 입력하거나 선택한 다음, **만들기**를 선택합니다.
    ...
    |리소스 그룹|yes|기존 [리소스 그룹](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#resource-group)을 선택하거나 리소스 그룹을 만듭니다. 네트워크 인터페이스는 연결하는 가상 머신이나 연결된 가상 네트워크와 같은 리소스 그룹에 있을 수도 있고 다른 리소스 그룹에 있을 수도 있습니다.|
...
포털은 네트워크 인터페이스를 만들 때 응용 프로그램 보안 그룹에 네트워크 인터페이스를 할당하는 옵션을 제공하지 않지만, Azure CLI 및 PowerShell은 제공합니다. 네트워크 인터페이스가 가상 머신에 연결된 경우, 포털을 사용하여 응용 프로그램 보안 그룹에 기존 네트워크 인터페이스를 할당할 수 있습니다. 응용 프로그램 보안 그룹에 네트워크 인터페이스를 할당하는 방법을 알아보려면 [응용 프로그램 보안 그룹에 추가 또는 제거](#add-to-or-remove-from-application-security-groups)를 참조하세요.
...
포털을 사용하면 네트워크 인터페이스가 가상 머신에 연결된 경우에만 응용 프로그램 보안 그룹에 네트워크 인터페이스를 추가 또는 제거할 수 있습니다. PowerShell 또는 Azure CLI를 사용하면 네트워크 인터페이스가 가상 머신에 연결되었는지 여부에 관계없이 응용 프로그램 보안 그룹에 네트워크 인터페이스를 추가 또는 제거할 수 있습니다. [응용 프로그램 보안 그룹](security-overview.md#application-security-groups) 및 [응용 프로그램 보안 그룹을 만드는](manage-network-security-group.md#create-an-application-security-group) 방법을 자세히 알아봅니다.

1. 포털의 맨 위에 있는 ‘리소스, 서비스 및 문서 검색’ 상자에서 응용 프로그램 보안 그룹에 추가 또는 제거하려는 네트워크 인터페이스가 포함된 가상 머신의 이름을 입력하기 시작합니다. VM 이름이 검색 결과에 나타나면 이 이름을 선택합니다.
2. **설정**에서 **네트워킹**을 선택합니다.  **응용 프로그램 보안 그룹 구성**을 선택하고, 네트워크 인터페이스를 추가할 응용 프로그램 보안 그룹을 선택하거나 네트워크 인터페이스를 제거할 응용 프로그램 보안 그룹을 선택 취소한 후, **저장**을 선택합니다. 동일한 가상 네트워크에 있는 네트워크 인터페이스만 동일한 응용 프로그램 보안 그룹에 추가할 수 있습니다. 응용 프로그램 보안 그룹은 네트워크 인터페이스와 동일한 위치에 있어야 합니다.
