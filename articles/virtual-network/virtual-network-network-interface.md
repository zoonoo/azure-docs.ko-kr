...
3. 다음 설정에 대한 값을 입력하거나 선택한 다음, **만들기** 를 선택합니다.
    ...
    |구독|Yes|Azure [구독](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#subscription) 중 하나를 선택합니다. 네트워크 인터페이스를 연결하는 가상 머신과 네트워크 인터페이스에 연결되는 가상 네트워크가 같은 구독에 있어야 합니다.|
...
2. **설정** 에서 **네트워킹** 을 선택 합니다.  **응용 프로그램 보안 그룹** 을 선택 하 고 **응용 프로그램 보안 그룹을 구성** 네트워크 인터페이스를 추가 하려는 응용 프로그램 보안 그룹을 선택 하거나, 네트워크 인터페이스를 제거 하려는 응용 프로그램 보안 그룹을 선택 취소 하 고 **저장** 을 선택 합니다. 동일한 가상 네트워크에 있는 네트워크 인터페이스만 동일한 애플리케이션 보안 그룹에 추가할 수 있습니다. 애플리케이션 보안 그룹은 네트워크 인터페이스와 동일한 위치에 있어야 합니다.
