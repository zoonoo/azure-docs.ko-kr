...
VNet은 지역 범위 내에서 만들어집니다. 두 개의 서로 다른 지역(예: 미국 동부 및 미국 서부)에서 주소 공간이 동일한 VNet을 *만들* 수 있지만 주소 공간이 같기 때문에 함께 연결할 수 없습니다. 
...
**Q: 전체 지역에 대해 중단이 발생하면 어떻게 해야 합니까? 예를 들어 자연 재해로 인해 지역이 완전히 차단된 경우? 리전에서 호스팅되는 가상 네트워크는 어떻게 됩니까?**
