...
description: 네트워크 보안 그룹에 대해 자세히 알아보세요. 네트워크 보안 그룹을 사용하면 Azure 리소스 간에 네트워크 트래픽을 필터링할 수 있습니다.
...
**원본** 및 **대상** 열에서 *VirtualNetwork*, *AzureLoadBalancer* 및 *인터넷*은 IP 주소가 아닌 [서비스 태그](service-tags-overview.md)입니다. 프로토콜 열에서 **Any는** TCP, UDP 및 ICMP를 포함합니다. 규칙을 만들 때 TCP, UDP, ICMP 또는 Any를 지정할 수 있습니다. **소스** 및 **대상** 열에서 *0.0.0.0/0*은 모든 주소를 나타냅니다. Azure 포털, Azure CLI 또는 PowerShell과 같은 클라이언트는 *또는 이 식에 사용할 수 있습니다.
...
서비스 태그는 지정된 Azure 서비스의 IP 주소 접두사 그룹을 나타냅니다. 네트워크 보안 규칙에 대한 자주 업데이트의 복잡성을 최소화하는 데 도움이 됩니다.

자세한 내용은 [Azure 서비스 태그를](service-tags-overview.md)참조하십시오. 저장소 서비스 태그를 사용하여 네트워크 액세스를 제한하는 방법에 대한 예는 [PaaS 리소스에 대한 네트워크 액세스 제한을](tutorial-restrict-network-access-to-resources.md)참조하십시오.

#### <a name="application-security-groups"></a>애플리케이션 보안 그룹

애플리케이션 보안 그룹을 사용하면 네트워크 보안을 애플리케이션 구조의 자연 확장으로 구성하여 가상 머신을 그룹화하고 해당 그룹에 따라 네트워크 보안 정책을 정의할 수 있습니다. 명시적 IP 주소를 수동으로 유지 관리하지 않고 대규모 보안 정책을 재사용할 수 있습니다. 자세한 내용은 [응용 프로그램 보안 그룹을](application-security-groups.md)참조하십시오.
...
### <a name="intra-subnet-traffic"></a>인트라 서브넷 트래픽

서브넷에 연결된 NSG의 보안 규칙은 그 안에 있는 VM 간의 연결에 영향을 줄 수 있습니다. 예를 들어 모든 인바운드 및 아웃바운드 트래픽을 거부하는 규칙이 *NSG1에* 추가되면 *VM1* 및 *VM2는* 더 이상 서로 통신할 수 없습니다. 이를 허용하려면 다른 규칙을 추가해야 합니다. 
...
> [!NOTE]
> 네트워크 보안 그룹은 서브넷 또는 클래식 배포 모델에 배포된 가상 컴퓨터 및 클라우드 서비스 및 Resource Manager 배포 모델의 서브넷 또는 네트워크 인터페이스에 연결됩니다. Azure 배포 모델에 대해 자세히 알아보려면 [Azure 배포 모델 이해](../azure-resource-manager/management/deployment-models.md?toc=%2fazure%2fvirtual-network%2ftoc.json)를 참조하세요.
...
- **호스트 노드의 가상 IP**: DHCP, DNS, IMDS 및 상태 모니터링과 같은 기본 인프라 서비스는 가상화된 호스트 IP 주소 168.63.129.16 및 169.254.169.254를 통해 제공됩니다. 이러한 IP 주소는 Microsoft에 속하며, 이 용도로 모든 지역에서 유일하게 사용되는 가상화된 IP 주소입니다. 효과적인 보안 규칙 및 효과적인 경로에는 이러한 플랫폼 규칙이 포함되지 않습니다. 이 기본 인프라 통신을 재정의하려면 네트워크 보안 그룹 규칙에서 다음과 같은 [서비스 태그를](service-tags-overview.md) 사용하여 트래픽을 거부하는 보안 규칙을 만들 수 있습니다. [네트워크 트래픽 필터링을 진단하고](diagnose-network-traffic-filter-problem.md) [네트워크 라우팅을 진단하는](diagnose-network-routing-problem.md)방법을 알아봅니다.
