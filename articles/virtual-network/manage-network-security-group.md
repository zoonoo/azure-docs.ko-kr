...
description: 보안 규칙에 대 한 정보를 찾을 수 있는 위치와 네트워크 보안 그룹을 만들거나 변경 하거나 삭제 하는 방법에 대해 알아봅니다.
...
네트워크 보안 그룹의 보안 규칙을 사용하면 가상 네트워크 서브넷 및 네트워크 인터페이스 내외부로 이동할 수 있는 네트워크 트래픽 유형을 필터링할 수 있습니다. 네트워크 보안 그룹에 대한 자세한 내용은 [네트워크 보안 그룹 개요](./network-security-groups-overview.md)를 참조하세요. 네트워크 보안 그룹에 대 한 몇 가지 경험을 얻기 위해 [네트워크 트래픽 필터링](tutorial-filter-network-traffic.md) 자습서를 완료 합니다.
...
3. 네트워크 보안 그룹의 메뉴 모음에서 **인바운드 보안 규칙** 또는 **아웃 바운드 보안 규칙** 을 선택 합니다.
    ...
    추가 하지 않을 수 있는 몇 가지 기존 규칙이 나열 됩니다. 네트워크 보안 그룹을 만들 때 몇 가지 기본 보안 규칙이 만들어집니다. 자세한 내용은 [기본 보안 규칙](./network-security-groups-overview.md#default-security-rules)을 참조하세요.  기본 보안 규칙을 삭제할 수 없으나 더 높은 우선 순위의 규칙으로 재정의할 수 있습니다.
...
4. <a name="security-rule-settings"></a>**추가** 를 선택 합니다. 다음 설정에 대 한 값을 선택 하거나 추가 하 고 **확인** 을 선택 합니다.
    ...
    | **원본 서비스 태그** | 드롭다운 목록의 서비스 태그 | 인바운드 보안 규칙에 대 한 **원본** **서비스 태그** 를 설정 하는 경우이 선택적 설정이 표시 됩니다. 서비스 태그는 IP 주소 범주에 대한 사전 정의된 식별자입니다. 사용 가능한 서비스 태그와 각 태그가 나타내는 항목에 대 한 자세한 내용은 [서비스 태그](./network-security-groups-overview.md#service-tags)를 참조 하세요. |
...
네트워크 보안 그룹에는 0 개 이상의 규칙이 포함 되어 있습니다. 경로를 볼 때 나열되는 정보에 대한 자세한 내용은 [네트워크 보안 그룹 개요](./network-security-groups-overview.md)를 참조합니다.
...
이 목록에는 사용자가 만든 모든 규칙과 네트워크 보안 그룹의 [기본 보안 규칙이](./network-security-groups-overview.md#default-security-rules)포함 되어 있습니다.
