...
- 기본 네트워크 인터페이스에는 Azure DHCP 서버에서 기본 게이트웨이를 할당하지만, 보조 네트워크 인터페이스에는 그렇지 않습니다. 보조 네트워크 인터페이스에는 기본 게이트웨이가 할당되지 않으므로 이 네트워크 인터페이스는 기본적으로 서브넷 외부의 리소스와 통신할 수 없습니다. 보조 네트워크 인터페이스가 서브넷 외부의 리소스와 통신할 수 있도록 하려면 [여러 네트워크 인터페이스를 사용하여 가상 컴퓨터 운영 체제 내에서 라우팅](#routing-within-a-virtual-machine-operating-system-with-multiple-network-interfaces)을 참조하세요.
...
- 네트워크 인터페이스에 할당된 개인 IPv6 주소가 있는 경우 VM을 만들 때 해당 VM에 연결할 수 있습니다. VM을 만든 후에 VM에 할당된 IPv6 주소에 네트워크 인터페이스를 연결할 수 없습니다. 가상 컴퓨터를 만들 때 할당된 개인 IPv6 주소에 네트워크 인터페이스를 연결하는 경우 VM 크기에서 지원하는 네트워크 인터페이스 개수에 관계없이 해당 네트워크 인터페이스를 가상 컴퓨터에 연결할 수 있습니다. 네트워크 인터페이스에 IP 주소를 할당하는 방법에 대한 자세한 내용을 보려면 [네트워크 인터페이스 IP 주소](virtual-network-network-interface-addresses.md)를 참조하세요.
...
### <a name="routing-within-a-virtual-machine-operating-system-with-multiple-network-interfaces"></a>여러 네트워크 인터페이스를 사용하여 가상 컴퓨터 운영 체제 내에서 라우팅

Azure에서는 가상 컴퓨터에 연결된 첫 번째(기본) 네트워크 인터페이스에 기본 게이트웨이를 할당합니다. 가상 컴퓨터에 연결된 추가(보조) 네트워크 인터페이스에는 기본 게이트웨이를 할당하지 않습니다. 따라서 보조 네트워크 인터페이스가 있는 서브넷 외부의 리소스와는 기본적으로 통신할 수 없습니다. 하지만 보조 네트워크 인터페이스는 서브넷 외부의 리소스와 통신할 수 있습니다. 단, 통신을 사용하도록 설정하는 단계는 운영 체제에 따라 다릅니다.

### <a name="windows"></a>Windows

Windows 명령 프롬프트에서 다음 단계를 완료합니다.

1. `route print` 명령을 실행합니다. 연결된 네트워크 인터페이스가 두 개인 가상 컴퓨터에 대해 다음과 유사한 출력이 반환됩니다.
...
기본 동작에서 취약한 호스트 라우팅을 사용하므로 리소스 간의 보조 네트워크 인터페이스 트래픽을 동일한 서브넷으로 제한하는 것이 좋습니다. 보조 네트워크 인터페이스의 서브넷 외부와 통신해야 하는 경우 가상 컴퓨터가 특정 네트워크 인터페이스를 통해 트래픽을 주고받도록 허용하는 라우팅 규칙을 만들어야 합니다. 그러지 않으면, 예를 들어 eth1에 속한 트래픽을 정의된 기본 경로에서 올바르게 처리할 수 없습니다. 라우팅 규칙을 구성하는 방법을 알아보려면 [여러 NIC에 대해 Linux 구성](../virtual-machines/linux/multiple-nics.md?toc=%2fazure%2fvirtual-network%2ftoc.json#configure-guest-os-for-multiple-nics)을 참조하세요.
...
네트워크 인터페이스를 추가하는 VM 크기에서 지원하는 만큼 많은 네트워크 인터페이스를 VM에 추가할 수 있습니다. 각 VM 크기에서 지원하는 네트워크 인터페이스의 수에 대한 자세한 내용은 [Linux](../virtual-machines/linux/sizes.md?toc=%2fazure%2fvirtual-network%2ftoc.json) 또는 [Windows](../virtual-machines/virtual-machines-windows-sizes.md?toc=%2fazure%2fvirtual-network%2ftoc.json) VM 크기 문서를 참조하세요. 네트워크 인터페이스를 추가하려는 VM은 추가하려는 네트워크 인터페이스 수를 지원하고 중지(할당 취소)된 상태여야 합니다. 추가하려는 네트워크 인터페이스는 현재 다른 VM에 연결할 수 없습니다. Azure Portal에서는 기존 VM에 네트워크 인터페이스를 추가할 수 없습니다. 기존 VM에 네트워크 인터페이스를 추가하려면 CLI 또는 PowerShell을 사용해야 합니다. 

Azure에서는 가상 컴퓨터에 연결된 첫 번째(기본) 네트워크 인터페이스에 기본 게이트웨이를 할당합니다. 가상 컴퓨터에 연결된 추가(보조) 네트워크 인터페이스에는 기본 게이트웨이를 할당하지 않습니다. 따라서 보조 네트워크 인터페이스가 있는 서브넷 외부의 리소스와는 기본적으로 통신할 수 없습니다. 그러나 보조 네트워크 인터페이스는 서브넷 외부의 리소스와 통신할 수 있습니다. 보조 네트워크 인터페이스가 서브넷 외부의 리소스와 통신해야 하는 경우 [여러 네트워크 인터페이스를 사용하여 가상 컴퓨터 운영 체제 내에서 라우팅](#routing-within-a virtual-machine-operating-system-with-multiple-network-interfaces)을 참조하세요.

> [!WARNING]
> 네트워크 인터페이스에 개인 IPv6 주소가 할당된 경우 기존 가상 컴퓨터에 추가할 수 없습니다. 가상 컴퓨터를 만들 때에만 할당된 개인 IPv6 주소가 있는 네트워크 인터페이스를 가상 컴퓨터에 추가할 수 있습니다. 네트워크 인터페이스에 IP 주소를 할당하는 방법에 대한 자세한 내용을 보려면 [네트워크 인터페이스 IP 주소](virtual-network-network-interface-addresses.md)를 참조하세요.
...
4. 선택한 VM에 대해 표시되는 가상 컴퓨터 블레이드의 **설정** 섹션에서 **네트워킹**을 클릭합니다. 네트워크 인터페이스 설정 및 변경 방법에 대한 자세한 내용은 [네트워크 인터페이스 관리](virtual-network-network-interface.md) 문서를 참조하세요. 네트워크 인터페이스에 할당된 IP 주소를 추가, 변경 또는 제거하는 방법을 알아보려면 [IP 주소 관리](virtual-network-network-interface-addresses.md)를 참조하세요.
...
4. 선택한 VM에 대해 표시되는 가상 컴퓨터 블레이드의 **설정** 섹션에서 **네트워킹**을 클릭합니다. 네트워크 인터페이스 설정 및 변경 방법에 대한 자세한 내용은 [네트워크 인터페이스 관리](virtual-network-network-interface.md) 문서를 참조하세요. 네트워크 인터페이스에 할당된 IP 주소를 추가, 변경 또는 제거하는 방법을 알아보려면 [IP 주소 관리](virtual-network-network-interface-addresses.md)를 참조하세요.
5. **X 네트워크 인터페이스 분리**를 클릭합니다.
6. 드롭다운 목록에서 분리할 네트워크 인터페이스를 선택한 다음 **확인**을 클릭합니다.
