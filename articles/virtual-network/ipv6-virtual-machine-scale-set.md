...
ms.locfileid: "73164992"
---
# <a name="deploy-virtual-machine-scale-sets-with-ipv6-in-azure-preview"></a>Azure에서 i p v 6를 사용 하 여 가상 머신 확장 집합 배포 (미리 보기)
...
개별 Vm과 다른 유일한 단계는 가상 머신 확장 집합 리소스를 사용 하는 NIC (네트워크 인터페이스) 구성 (networkProfile/networkInterfaceConfigurations)을 만드는 것입니다. JSON 구조는 개별 Vm에 사용 되는 Microsoft 네트워크/networkInterfaces 개체의 경우와 유사 합니다 .이 개체는 다음에 표시 된 것 처럼 **"primary": true** 특성을 사용 하 여 기본 인터페이스로 NIC와 IPv4 IpConfiguration을 설정 하는 것을 추가 합니다. 예를 들면 다음과 같습니다.
