...
[New-Az가상 네트워크SubnetConfig를](/powershell/module/az.network/new-azvirtualnetworksubnetconfig)사용하여 서브넷 구성을 만듭니다. 다음 예제에서는 *Public*이라는 서브넷에 대한 서브넷 구성을 만듭니다.
...
[Set-AzVirtualNetwork를](/powershell/module/az.network/Set-azVirtualNetwork)사용하여 서브넷 구성을 가상 네트워크에 작성하여 가상 네트워크에서 서브넷만들기:
...
[New-AzStorageAccount를](/powershell/module/az.storage/new-azstorageaccount)사용하여 Azure 저장소 계정을 만듭니다. `<replace-with-your-unique-storage-account-name>`을 모든 Azure 위치에서 고유한 이름으로 바꿉니다. 이름은 3~24자 사이여야 하고 숫자와 소문자만 사용할 수 있습니다.
...
$share = 새 AzStorage내 파일 공유 -컨텍스트 $storageContext 공유
...
[New-AzVM을](/powershell/module/az.compute/new-azvm)사용하여 *공용* 서브넷에 가상 컴퓨터를 만듭니다. 다음 명령을 실행하면 자격 증명을 묻는 메시지가 표시됩니다. 입력하는 값은 VM에 대한 사용자 이름과 암호로 구성됩니다. `-AsJob` 옵션은 백그라운드에서 VM을 만들므로 다음 단계를 계속 진행할 수 있습니다.
...
액세스가 거부되고 *Get-AzStorageFile : 원격 서버가 오류를 반환했습니다: (403) 금지됨. HTTP 상태 코드: 403 - HTTP 오류 메시지: 컴퓨터가* *MyVirtualNetwork* 가상 네트워크의 *개인* 서브넷에 없기 때문에 이 요청은 이 작업 오류를 수행할 권한이 없습니다.
