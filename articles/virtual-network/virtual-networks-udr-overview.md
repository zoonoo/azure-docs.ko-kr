...
|가상 네트워크 피어링         |VNet 피어링                                    |해당 사항 없음|
|가상 네트워크 서비스 엔드포인트|VirtualNetworkServiceEndpoint                   |해당 사항 없음|
...
* **ExpressRoute**: BGP를 사용 하 여 온-프레미스 경로를 Microsoft Edge 라우터에 보급 해야 합니다. ExpressRoute 유형으로 배포된 가상 네트워크 게이트웨이를 배포하는 경우 ExpressRoute 가상 네트워크 게이트웨이로 트래픽을 강제하기 위해 사용자 정의 경로를 만들 수 없습니다. 예를 들어 사용자 정의 경로를 사용하여 Express Route에서 네트워크 가상 어플라이언스로 트래픽을 강제로 라우팅할 수 있습니다.<br>
...
|ID  |Source |State  |주소 접두사    |다음 홉 유형          |다음 홉 IP 주소|사용자 정의 경로 이름| 
|----|-------|-------|------              |-------                |--------           |--------      |
...
|10  |기본값|활성 |[X.X.X.X]           |VirtualNetworkServiceEndpoint    |         |              |
...
|Source  |State  |주소 접두사    |다음 홉 유형             |다음 홉 IP 주소|
|------- |-------|------              |-------                   |--------           
|기본값 |활성 |10.0.0.0/16         |가상 네트워크           |                   |
|기본값 |활성 |10.1.0.0/16         |VNet 피어링              |                   |
|기본값 |활성 |10.2.0.0/16         |VNet 피어링              |                   |
|기본값 |활성 |10.10.0.0/16        |가상 네트워크 게이트웨이   |[X.X.X.X]          |
|기본값 |활성 |0.0.0.0/0           |인터넷                  |                   |
|기본값 |활성 |10.0.0.0/8          |없음                      |                   |
|기본값 |활성 |100.64.0.0/10       |없음                      |                   |
|기본값 |활성 |192.168.0.0/16      |없음                      |                   |
