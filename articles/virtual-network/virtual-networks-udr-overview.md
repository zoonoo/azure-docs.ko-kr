...
아래 그림은 프런트엔드 서브넷에서 인터넷으로 이동하는 패킷이 가상 어플라이언스를 통과하고 프런트엔드 서브넷에서 백엔드 서브넷으로 이동하는 모든 패킷이 서로 다른 어플라이언스를 통과하도록 강제하기 위한 사용자 정의 경로 및 IP 전달의 예시입니다. 백엔드 서브넷에서 프런트엔드 서브넷로 이동하는 트래픽은 여전히 시스템 경로를 통해 이동하며 어플라이언스를 우회하게 된다는 점에 유념하십시오.
...
Azure의 VM에서 IP 전달을 사용하도록 설정하는 방법은 [Azure에서 경로를 만들고 IP 전달을 사용하도록 설정하는 방법](../virtual-networks-udr-how-to#How-to-Manage-IP-Forwarding)을 참조하십시오
