...
4. 다음 그림에 표시 된 것 처럼 공용IP 주소 페이지에서 개요 **를 선택 하**고 분리를 선택 합니다.
...
1. Bash에서 로컬로 CLI를 사용 하는 경우 `az login`를 사용 하 여 Azure에 로그인 합니다.
...
1. PowerShell을 로컬로 사용 하는 경우 `Connect-AzAccount`를 사용 하 여 Azure에 로그인 합니다.
