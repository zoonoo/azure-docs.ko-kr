---
title: Azure CLI를 사용하여 리소스 관리자에서 라우팅 제어 및 가상 어플라이언스 사용 | Microsoft Docs
...
이 문서에서는 리소스 관리자 배포 모델에 대해 설명합니다. [클래식 배포 모델에서 UDR을 만들](virtual-network-create-udr-classic-cli.md) 수도 있습니다.
...
1. **`azure network route-table create`** 명령을 실행하여 프런트 엔드 서브넷에 대한 경로 테이블을 만듭니다.
...
2. **`azure network route-table route create`** 명령을 실행하여 위에서 만든 경로 테이블에 경로를 만들고 백 엔드 서브넷(192.168.2.0/24)으로 보내진 모든 트래픽을 **FW1** VM(192.168.0.4)으로 보냅니다.
...
3. **`azure network vnet subnet set`** 명령을 실행하여 위에서 만든 경로 테이블을 **FrontEnd** 서브넷에 연결합니다.
...
2. **`azure network route-table route create`** 명령을 실행하여 위에서 만든 경로 테이블에 경로를 만들고 프런트 엔드 서브넷(192.168.1.0/24)으로 보내진 모든 트래픽을 **FW1** VM(192.168.0.4)으로 보냅니다.
   
        azure network route-table route create -g TestRG -r UDR-BackEnd -n RouteToFrontEnd -a 192.168.1.0/24 -y VirtualAppliance -p 192.168.0.4
3. **`azure network vnet subnet set`** 명령을 실행하여 위에서 만든 경로 테이블을 **BackEnd** 서브넷에 연결합니다.
