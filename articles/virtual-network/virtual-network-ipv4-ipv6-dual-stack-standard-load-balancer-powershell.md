---
title: IPv6 듀얼 스택 애플리케이션 배포 - 표준 로드 밸러저 - PowerShell
titlesuffix: Azure Virtual Network
description: 이 문서에서는 Azure Powershell을 사용하여 Azure 가상 네트워크에서 표준 로드 밸런서를 사용하여 IPv6 이중 스택 응용 프로그램을 배포하는 방법을 보여 주며 이 문서에서는
...
[New-AzLoadBalancer를](/powershell/module/az.network/new-azloadbalancer)사용하여 표준 로드 밸러블러를 만듭니다. 다음 예제에서는 이전 단계에서 만든 IPv4 및 IPv6 프런트 엔드 IP 구성, 백 엔드 풀 및 로드 밸런싱 규칙을 사용하여 *myLoadBalancer라는* 공용 표준 로드 밸런서를 만듭니다.
...
[New-AzVirtualNetwork](/powershell/module/az.network/new-azvirtualnetwork)를 사용하여 가상 네트워크를 만듭니다. 다음 예제는 *mySubnet을*가진 *dsVnet라는* 가상 네트워크를 만듭니다.
