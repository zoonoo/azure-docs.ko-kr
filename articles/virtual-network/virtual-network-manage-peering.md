...
5. <a name="add-peering"></a>다음 설정에 대한 값을 입력하거나 선택합니다.
    ...
    - **가상 네트워크 액세스 허용:** 기본 `VirtualNetwork` 흐름을 통해 두 가상 네트워크 간 통신을 사용하도록 설정하려면 **사용**(기본값)을 선택합니다. 가상 네트워크 간 통신을 사용하도록 설정하면 어느 쪽 가상 네트워크에든 연결된 리소스가 같은 가상 네트워크에 연결된 것처럼 같은 대역폭 및 대기 시간으로 서로 통신할 수 있습니다. 두 가상 네트워크의 리소스 간 모든 통신은 Azure 프라이빗 네트워크를 통해 이루어집니다. 네트워크 보안 그룹에 대한 **VirtualNetwork** 서비스 태그는 설정이 **허용** 일 때 가상 네트워크와 피어링된 가상 네트워크를 포함합니다. (네트워크 보안 그룹 서비스 태그에 대한 자세한 내용은 [네트워크 보안 그룹 개요](./network-security-groups-overview.md#service-tags)를 참조하세요.) 기본적으로 트래픽이 피어링된 가상 네트워크로 흐르지 않도록 하려면 **비활성화** 를 선택합니다. 가상 네트워크를 다른 가상 네트워크와 피어링 했지만, 종종 두 가상 네트워크 간 트래픽 흐름을 비활성화하려는 경우에도 **사용 안 함** 을 선택할 수 있습니다. 피어링을 삭제하고 다시 만드는 것보다 사용/사용 안 함을 설정하는 것이 더 편리함을 알 수 있습니다. 이 설정을 사용하지 않도록 설정하면 기본적으로 피어링된 가상 네트워크 간에 트래픽이 흐르지 않습니다. 그러나 적절한 IP 주소 또는 응용 프로그램 보안 그룹을 포함하는 [네트워크 보안 그룹](./network-security-groups-overview.md) 규칙을 통해 명시적으로 허용된 경우 트래픽이 계속 흐를 수 있습니다.
      > [!WARNING]
      > 가상 네트워크 **액세스 허용** 설정을 사용하지 않도록 설정하면 **VirtualNetwork** 서비스 태그의 정의만 변경됩니다. 이 설정 설명에 설명된 대로 피어 연결을 통해 트래픽 흐름을 완전히 *차단하지는 않습니다*.    
...
5. 해당 설정을 변경합니다. 피어링 만들기 섹션의 [5단계](#add-peering) 에서 각 설정의 옵션에 대해 읽어보세요.
6. **저장** 을 선택합니다.
...
- 글로벌 피어링을 만들 때 피어링된 가상 네트워크는 모든 Azure 공용 클라우드 지역 또는 중국 클라우드 지역 또는 정부 클라우드 지역에 있을 수 있습니다. 클라우드 간에 피어를 만들 수 없습니다. 예를 들어 Azure 공용 클라우드의 VNet은 Azure 중국 클라우드의 VNet에 피어링 될 수 없습니다.
- 하나의 가상 네트워크의 리소스는 전역적으로 피어링된 가상 네트워크에 있는 기본 내부 부하 분산 장치의 프런트 엔드 IP 주소와 통신할 수 없습니다. 기본 Load Balancer에 대한 지원은 동일한 지역 내에서만 가능합니다. 표준 Load Balancer에 대한 지원은 VNet Peering 및 글로벌 VNet Peering에서 모두 가능합니다. 글로벌 VNet 피어링에서 작동하지 않는 기본 부하 분산 장치를 사용하는 서비스는 [여기에 문서화되어 있습니다.](virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers)
