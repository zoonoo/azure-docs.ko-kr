...
5. 다음을 지정한 다음, **확인** 을 선택합니다.
   ...
   |형식|Yes|기존 네트워크 인터페이스에 IP 구성을 추가할 것이며, 각 NIC에는 [기본](#primary) IP 구성이 있어야 하므로 **보조** 옵션만 선택 가능합니다.|
   ...
   |공용 IP 주소|No|**사용 안 함:** 현재 공용 IP 주소 리소스는 IP 구성과 연결되지 않습니다. **사용:** 기존 IPv4 공용 IP 주소를 선택하거나 새 공용 IP 주소를 만듭니다. 공용 IP 주소를 만드는 방법에 대한 자세한 내용은 [공용 IP 주소](virtual-network-public-ip-address.md#create-a-public-ip-address) 문서를 참조하세요.|
...
- 또한 공용 IPv4 또는 IPv6 주소를 할당할 수 있습니다. 여러 IPv4 주소를 네트워크 인터페이스에 할당 하는 것은 다음과 같은 시나리오에서 유용 합니다.
  - 단일 서버에서 여러 IP 주소 및 TLS/SSL 인증서를 사용 하 여 여러 웹 사이트 또는 서비스를 호스트 합니다.
