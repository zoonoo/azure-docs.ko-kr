...
## <a name="creating-and-migrating-to-regional-vnets"></a>지역 VNet 만들기 및 마이그레이션
앞으로는 새 VNet을 만들 경우 *지역*을 사용합니다. 지역은 관리 포털에서 옵션으로 표시됩니다. 네트워크 구성 파일에서는 *위치*로 표시됩니다.

> [!IMPORTANT]
> 선호도 그룹과 연결된 가상 네트워크를 만드는 것이 여전히 기술적으로 가능하지만 그렇게 할 만큼 설득력 있는 이유가 없습니다. 네트워크 보안 그룹과 같은 새로운 많은 기능이 지역 VNet을 사용하는 경우에만 사용할 수 있으며, 선호도 그룹과 연결된 가상 네트워크에서는 사용할 수 없습니다.
> 
> 

### <a name="about-vnets-currently-associated-with-affinity-groups"></a>현재 선호도 그룹과 연결된 VNet에 대한 정보
...
2. 이전 값을 새 값으로 바꿔 네트워크 구성 파일을 편집합니다. 
   
   > [!NOTE]
   > **위치** 는 VNet과 연관된 선호도 그룹에 대해 지정한 영역입니다. 예를 들어 VNet이 미국 서부에 위치한 선호도 그룹과 연결된 경우 마이그레이션할 때 위치가 미국 서부를 가리켜야 합니다. 
   > 
   > 
...
3. 변경 내용을 저장하고 Azure에 네트워크 구성을 [가져옵니다](virtual-networks-using-network-configuration-file.md) .
...
## <a name="affinity-groups-and-vms"></a>선호도 그룹 및 VM
...
### <a name="what-to-do-if-you-have-a-vm-in-an-affinity-group"></a>선호도 그룹에 VM이 있는 경우 수행할 수 있는 작업
