...
6. 네트워크 가상 어플라이언스 가상 컴퓨터를 만듭니다.
    ...
    - 표시되는 **만들기** 블레이드에서 **만들기**를 클릭합니다. 가상 컴퓨터 배포에는 몇 분 정도 걸립니다.

    > [!NOTE]
    > Azure Portal은 가상 컴퓨터를 만들 뿐만 아니라 기본적으로 공용 IP 주소를 만들어 이를 가상 컴퓨터에 할당합니다. 프로덕션 환경에서 가상 컴퓨터를 배포하는 경우, 가상 컴퓨터에 공용 IP 주소를 할당하지 않을 수 있습니다. 대신 가상 네트워크 내의 다른 가상 컴퓨터에서 네트워크 가상 어플라이언스에 액세스할 수도 있습니다. 공용 IP 주소에 대한 자세한 내용은 [공용 IP 주소 관리](virtual-network-public-ip-address.md)를 참조하세요.
...
7. 정적 개인 IP 주소를 할당하고 이전 단계에서 만들어진 포털에서 네트워크 인터페이스에 대한 IP 전달을 사용 가능하게 합니다. 
    ...
    - **myVm-Nva - 네트워크 인터페이스**에서 네트워크 인터페이스 이름을 클릭합니다. 이름은 **myvm-nva***X*입니다. 여기서 *X* 는 포털에서 할당한 번호입니다.
    ...
    - 앞 그림과 같이 **개인 IP 주소 설정**의 **IP 주소** 아래에 *10.0.2.4*를 입력합니다. 네트워크 인터페이스에 정적 IP 주소를 할당하면 네트워크 인터페이스가 연결된 가상 컴퓨터의 수명 동안 주소가 변경되지 않습니다. 입력한 주소는 현재 네트워크 인터페이스가 있는 DMZ 서브넷의 다른 리소스에 할당되지 않습니다. 
...
11. **선택 사항:** 공개 및 비공개 서브넷에서 가상 컴퓨터를 만들고, [라우팅 확인](#validate-routing)의 단계를 완료하여 가상 컴퓨터 간의 통신이 네트워크 가상 어플라이언스를 통해 라우팅되는지 확인합니다. 
