...
* **지리적:** '지리적'을 선택하여 사용자가 해당 DNS 쿼리가 시작된 지리적 위치에 기반한 특정 끝점(Azure, 외부 또는 중첩됨)에 지정됩니다. 이렇게 하면 Traffic Manager 고객이 사용자의 지리적 위치를 파악하고 그에 따라 라우팅되는 중요한 시나리오를 사용할 수 있습니다. 데이터 독립성 지시 사항, 콘텐츠 및 사용자 환경의 지역화를 준수하고 다른 지역의 트래픽을 측정하는 작업을 예로 들 수 있습니다.
...
## <a name="geographic-traffic-routing-method"></a>지리적 트래픽 라우팅 메서드
...
지역 또는 지역 집합이 끝점에 할당되는 경우 해당 지역의 요청은 해당 끝점에만 라우팅됩니다. Traffic Manager는 DNS 쿼리의 원본 IP 주소를 사용하여 사용자를 쿼리할 지역을 결정합니다. 대부분의 경우에 사용자를 대신하여 쿼리를 수행하는 로컬 DNS 해결 프로그램의 IP 주소입니다.  

![Azure Traffic Manager '지리적' 트래픽 라우팅 메서드](./media/traffic-manager-routing-methods/geographic.png)

Traffic Manager는 DNS 쿼리의 원본 IP 주소를 읽고 해당 사항이 발생하는 지리적 지역을 결정합니다. 그런 다음 끝점에 여기에 매핑된 해당 지리적 지역이 있는지 확인합니다. 이 조회는 세분성 수준이 가장 낮은 곳(지원되는 주/지방 및 국가/지역 수준)에서 시작되고 세계라는 가장 높은 수준까지 올라갑니다. 이 통과를 사용하는 첫 번째 일치 결과는 쿼리 응답에서 반환되는 끝점으로 지정됩니다. 중첩 유형 끝점과 일치하는 경우 해당 라우팅 메서드를 기반으로 하는 해당 하위 프로필 내의 끝점이 반환됩니다. 다음 사항을 이러한 동작에 사용할 수 있습니다.

1. 지리적 지역은 라우팅 형식이 지리적 라우팅일 경우 Traffic Manager 프로필에서 끝점에만 매핑될 수 있습니다. 그렇게 하면 사용자의 라우팅은 결정적이며 고객은 모호하지 않은 지리적 경계를 필요로 하는 시나리오를 사용할 수 있습니다.
2. 사용자의 지역이 두 개의 끝점에서 지리적 매핑에서 제공되는 경우 Traffic Manager는 세분성이 가장 낮은 끝점을 선택하고 다른 끝점에 해당 지역의 요청을 라우팅하도록 고려하지 않습니다. 예제: 두 개의 끝점인 끝점1과 끝점2를 사용하는 지리적 라우팅 유형 프로필을 고려합니다. 끝점1은 아일랜드에서 트래픽을 수신하도록 구성되고 끝점2는 유럽에서 트래픽을 수신하도록 구성되어 있습니다. 요청이 아일랜드에서 시작되는 경우 항상 끝점1에 라우팅됩니다.
3. 지역이 한 끝점에만 매핑될 수 있으므로 Traffic Manager는 끝점이 정상인지 여부에 관계없이 반환합니다. 따라서 고객이 지리적 라우팅 메서드를 사용하여 내부에 두 개 이상의 끝점을 포함하는 하위 프로필을 가진 중첩 유형 끝점과 연결하는 것이 좋습니다.
4. 끝점이 일치하고 해당 끝점이 **중지된** 상태인 경우 Traffic Manager는 NODATA 응답을 반환합니다. 이 경우에 지리적 지역 계층 구조에서 더 이상 조회가 수행되지 않습니다. 이 동작은 하위 프로필이 **중지됨** 또는 **비활성화** 상태인 경우 중첩 끝점 유형에 적용됩니다.
5. 끝점이 **비활성화** 상태인 경우 지역 일치 과정에 포함되지 않습니다. 이 동작은 끝점이 **비활성화** 상태인 경우 중첩 끝점 유형에도 적용됩니다.
6. 쿼리가 해당 프로필에서 매핑되지 않는 지리적 지역에서 비롯된 경우 Traffic Manager는 NODATA 응답을 반환합니다. 따라서 고객이 하나의 끝점(이상적으로 중첩 유형인 두 개 이 상의 끝점)을 하위 프로필 내에 포함하며 지역이 **전 세계**로 할당된 지리적 라우팅을 사용하는 것이 좋습니다. 이렇게 하면 지역에 매핑되지 않는 모든 IP 주소가 처리됩니다.
