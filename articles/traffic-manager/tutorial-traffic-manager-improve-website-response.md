---
title: '자습서: 웹 사이트 응답 향상-Azure Traffic Manager'
...
- 서로 다른 Azure 지역에서 실행 되는 기본 웹 사이트의 두 인스턴스- **미국 동부** 및 **유럽 서부**
- **미국 동부** 에서 TRAFFIC MANAGER 한 vm과 **유럽 서부**의 두 번째 vm을 테스트 하기 위한 두 개의 테스트 vm입니다. 테스트 VM은 Traffic Manager가 사용자 트래픽을 대기 시간이 가장 짧은 동일한 지역에서 실행되는 웹 사이트로 라우팅하는 방식을 설명하는 데 사용됩니다.
...
2. **가상 머신 만들기**의 **기본** 탭에서 다음 값을 입력하거나 선택합니다.
   ...
   - **구독** > **리소스 그룹**: **새로 만들기** 를 선택한 다음 **myResourceGroupTM1**를 입력 합니다.
   - **인스턴스 세부 정보** > **가상 컴퓨터 이름**: *myIISVMEastUS*을 입력 합니다.
   - **인스턴스 세부 정보** > **지역**: **미국 동부**를 선택 합니다.
   - **관리자 계정** > **Username**: 선택한 사용자 이름을 입력 합니다.
   - **관리자 계정** > **암호**: 선택한 암호를 입력 합니다. 암호는 12자 이상이어야 하며 [정의된 복잡성 요구 사항](../virtual-machines/windows/faq.md?toc=%2fazure%2fvirtual-network%2ftoc.json#what-are-the-password-requirements-when-creating-a-vm)을 충족해야 합니다.
   - **인바운드 포트 규칙** > **공용 인바운드 포트**: **선택한 포트 허용**을 선택 합니다.
   - 인바운드 포트 **규칙** > 인바운드 포트 **선택**: 풀 다운 상자에서 **RDP** 및 **HTTP** 를 선택 합니다.

3. **관리** 탭을 선택 하거나 **다음: 디스크**, 다음: **네트워킹**, 다음 **: 관리**를 차례로 선택 합니다. **모니터링**에서 **부트 진단**을 **끄기**로 설정합니다.
4. **검토 + 만들기**를 선택합니다.
...
이 섹션에서는 두 개의 Vm *myIISVMEastUS* 및 *myIISVMWestEurope*에 IIS 서버를 설치한 다음 기본 웹 사이트 페이지를 업데이트 합니다. 사용자 지정된 웹 사이트 페이지에는 웹 브라우저에서 웹 사이트를 방문할 때 연결하는 VM의 이름이 표시됩니다.
...
9. IIS를 설치 하 고 기본 웹 페이지를 사용자 지정 하려면 *myResourceGroupTM2* 리소스 그룹 내의 VM *myIISVMWestEurope* 을 사용 하 여 RDP 연결을 만들어 1-8 단계를 반복 합니다.
...
2. **가상 머신 만들기**의 **기본** 탭에서 다음 값을 입력하거나 선택합니다.
   ...
   - **구독** > **리소스 그룹**: **myResourceGroupTM1**을 선택 합니다.
   - **인스턴스 세부 정보** > **가상 컴퓨터 이름**: *myVMEastUS*을 입력 합니다.
   - **인스턴스 세부 정보** > **지역**: **미국 동부**를 선택 합니다.
   - **관리자 계정** > **Username**: 선택한 사용자 이름을 입력 합니다.
   - **관리자 계정** > **암호**: 선택한 암호를 입력 합니다. 암호는 12자 이상이어야 하며 [정의된 복잡성 요구 사항](../virtual-machines/windows/faq.md?toc=%2fazure%2fvirtual-network%2ftoc.json#what-are-the-password-requirements-when-creating-a-vm)을 충족해야 합니다.
   - **인바운드 포트 규칙** > **공용 인바운드 포트**: **선택한 포트 허용**을 선택 합니다.
   - 인바운드 포트 **규칙** > 인바운드 포트 **선택**: 풀 다운 상자에서 **RDP** 를 선택 합니다.
...
2. **Traffic Manager 프로필 만들기**에서 다음 정보를 입력하거나 선택하고, 나머지 설정은 기본값을 그대로 적용한 다음, **만들기**를 선택합니다.
    ...
    | 구독            | 사용 중인 구독을 선택합니다.                          |
    | 리소스 그룹          | *MyResourceGroupTM1*리소스 그룹을 선택 합니다. |
...
IIS 서버 *myIISVMEastUS* & *myIISVMWestEurope* 를 실행 하는 두 vm을 추가 하 여 사용자에 게 가장 가까운 끝점으로 사용자 트래픽을 라우팅합니다.
...
4. 2 단계와 3 단계를 반복 하 여 *myIISVMWestEurope*이라는 IIS 서버 VM에 연결 된 공용 IP 주소 *myIISVMWestEurope* 에 *myWestEuropeEndpoint* 이라는 다른 끝점을 추가 합니다.
...
2. 다음으로, 1~5단계를 사용하여 *유럽 서부*에 있는 VM **myVMWestEurope**에 연결하고 이 VM에서 Traffic Manager 프로필 도메인 이름을 찾아서 이동합니다. **유럽 서부**에 있는 VM은 이제 **유럽 서부**에 있는 가장 가까운 IIS 서버 *myIISVMWestEurope* 에서 호스팅되는 웹 사이트로 라우팅됩니다.
