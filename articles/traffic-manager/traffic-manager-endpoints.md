...
>[AZURE.NOTE]*장애 조치(Failover)* 부하 분산 방법을 사용하는 경우 끝점을 추가하거나 제거한 후 구성에 사용할 장애 조치(Failover) 순서를 반영하도록 구성 페이지의 장애 조치(Failover) 우선 순위 목록을 조정해야 합니다. 자세한 내용은 [장애 조치(Failover) 부하 분산 구성](traffic-manager-configure-failover-load-balancing.md)을 참조하세요.
