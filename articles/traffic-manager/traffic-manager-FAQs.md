...
[트래픽 관리자의 작동 방식에](../traffic-manager/traffic-manager-how-it-works.md)설명된 대로 트래픽 관리자는 DNS 수준에서 작동합니다. 클라이언트를 적절한 서비스 엔드포인트로 보내기 위해 DNS 응답을 전송합니다. 그러면 클라이언트는 Traffic Manager를 통해서가 아니라 직접 서비스 엔드포인트에 연결합니다.
...
### <a name="does-traffic-manager-support-sticky-sessions"></a>트래픽 관리자가 "고정" 세션을 지원합니까?

[트래픽 관리자의 작동 방식에](../traffic-manager/traffic-manager-how-it-works.md)설명된 대로 트래픽 관리자는 DNS 수준에서 작동합니다. 클라이언트를 적절한 서비스 엔드포인트로 보내기 위해 DNS 응답을 사용합니다. 클라이언트는 Traffic Manager를 통해서가 아니라 직접 서비스 엔드포인트에 연결합니다. 따라서 Traffic Manager는 클라이언트와 서비스 간에 HTTP 트래픽을 표시하지 않습니다.
...
[트래픽 관리자의 작동 방식에](../traffic-manager/traffic-manager-how-it-works.md)설명된 대로 트래픽 관리자는 DNS 수준에서 작동합니다. 클라이언트를 적절한 서비스 엔드포인트로 보내기 위해 DNS 응답을 사용합니다. 그러면 클라이언트는 Traffic Manager를 통해서가 아니라 직접 서비스 엔드포인트에 연결합니다. Traffic Manager는 클라이언트와 서버 간에 HTTP 트래픽을 표시하지 않습니다. 따라서 표시된 모든 HTTP 오류는 애플리케이션에서 가져온 것이어야 합니다. 클라이언트를 애플리케이션에 연결하는 경우 모든 DNS 확인 단계를 완료합니다. Traffic Manager가 애플리케이션 트래픽 흐름에 있는 모든 상호 작용을 포함합니다.
...
[트래픽 관리자의 작동 방식에](../traffic-manager/traffic-manager-how-it-works.md)설명된 대로 트래픽 관리자는 DNS 수준에서 작동합니다. 클라이언트가 서비스 엔드포인트에 직접 연결되므로 연결이 설정된 후 Traffic Manager를 사용하면 성능 영향이 발생하지 않습니다.

Traffic Manager는 DNS 수준에서 애플리케이션과 통합하므로 추가 DNS 조회를 DNS 확인 체인에 삽입해야 합니다. DNS 확인 시간에 대한 Traffic Manager의 영향은 최소입니다. 트래픽 관리자는 이름 서버의 글로벌 네트워크를 [사용하고, 모든 캐스트](https://en.wikipedia.org/wiki/Anycast) 네트워킹을 사용하여 DNS 쿼리가 항상 가장 가까운 이름 서버로 라우팅되도록 합니다. 또한 DNS 응답의 캐싱은 Traffic Manager를 사용하여 발생한 추가 DNS 대기 시간이 세션 중 일부에만 적용되는 것을 의미합니다.
...
[트래픽 관리자의 작동 방식에](../traffic-manager/traffic-manager-how-it-works.md)설명된 대로 트래픽 관리자는 DNS 수준에서 작동합니다. DNS 조회가 완료되면 클라이언트는 Traffic Manager를 통해서가 아닌 애플리케이션 엔드포인트에 직접 연결됩니다. 따라서 이 연결에서는 모든 애플리케이션 프로토콜을 사용할 수 있습니다. TCP를 모니터링 프로토콜로 선택하면 애플리케이션 프로토콜을 사용하지 않고 Traffic Manager의 엔드포인트 상태 모니터링을 수행할 수 있습니다. 애플리케이션 프로토콜을 사용하여 상태가 확인되도록 선택한 경우 엔드포인트가 HTTP 또는 HTTPS GET 요청에 응답할 수 있어야 합니다.

### <a name="can-i-use-traffic-manager-with-a-naked-domain-name"></a>'벌거벗은' 도메인 이름으로 트래픽 관리자를 사용할 수 있나요?

예. Azure Traffic Manager 프로필을 참조하기 위해 도메인 이름 정점에 대한 별칭 레코드를 만드는 방법을 알아보려면 트래픽 관리자를 사용하여 [정점 도메인 이름을 지원하도록 별칭 레코드 구성을](../dns/tutorial-alias-tm.md)참조하십시오.
...
아니요, 트래픽 관리자는 DNS 쿼리의 소스 IP 주소에서 유추하는 지리적 지역이 다음과 같은 이유로 항상 사용자의 위치와 일치한다고 보장할 수 없습니다.

- 첫째, 이전 FAQ에 설명된 것처럼 보고 있는 원본 IP 주소는 DNS 확인자가 사용자를 대신해서 조회를 수행하는 주소입니다. DNS 확인자의 지리적 위치는 사용자의 지리적 위치에 대한 적절한 프록시이지만, 해당 DNS 확인자 서비스의 사용 공간 및 고객이 사용하도록 선택한 특정 DNS 확인자 서비스에 따라 다를 수 있습니다. 예를 들어 말레이시아에 있는 고객은 장치의 설정에서 DNS 확인 서비스를 사용하여 지정할 수 있으며 싱가포르의 DNS 서버가 해당 사용자/장치에 대한 쿼리 해결을 처리하기 위해 선택될 수 있습니다. 이 경우 트래픽 관리자는 싱가포르 위치에 해당하는 해결 사람의 IP 주소만 볼 수 있습니다. 이 페이지에서 클라이언트 서브넷 주소 지원과 관련된 이전 FAQ도 참조하세요.
...
지리적 라우팅 방법을 사용하는 경우 프로필 내 하나의 엔드포인트에만 지역을 할당할 수 있습니다. 해당 끝점이 하위 프로필이 연결된 중첩 형식이 아닌 경우 해당 끝점이 비정상이 되면 트래픽 관리자는 트래픽을 보내지 않는 대안이 더 좋지 않기 때문에 트래픽을 계속 전송합니다. 트래픽 관리자는 할당된 지역이 비정상으로 이동한 끝점에 할당된 영역의 "부모"인 경우에도 다른 끝점으로 장애 조치되지 않습니다(예: 스페인 지역이 비정상인 끝점이 비정상인 경우, 다른 끝점으로 장애 조치 하지 않음) 유럽이 할당된 지역)을 가지고 있습니다) 이는 고객이 그들의 프로필에서 설정한 지리적 경계를 Traffic Manager가 존중하도록 하기 위함입니다. 엔드포인트가 비정상 상태가 될 때 다른 엔드포인트로 장애 조치(failover)하는 방법을 활용하려면 지리적 지역을 개별 엔드포인트 대신 프로필 내 여러 엔드포인트를 사용하는 중첩 프로필에 할당하는 것이 좋습니다. 이 방식에서 중첩 자식 프로필의 엔드포인트가 실패하면 트래픽은 동일한 중첩 자식 프로필 내부의 다른 엔드포인트로 장애 조치할 수 있습니다.
...
-    CIDR 표기법의 경우 '/' 이전의 IP 주소가 해당 범위의 시작 주소여야 합니다(예: 1.2.3.0/24는 유효하지만 1.2.3.4.4/24는 유효하지 않음).
...
서브넷 라우팅을 사용한 프로필에서 엔드포인트가 비활성화된 경우 Traffic Manager는 해당 엔드포인트 및 서브넷 매핑이 존재하지 않는 것처럼 작동합니다. IP 주소 매핑과 일치하는 쿼리가 수신되고 끝점이 비활성화된 경우 트래픽 관리자는 대체 끝점(매핑이 없는 끝점)을 반환하거나 이러한 끝점이 없는 경우 NXDOMAIN 응답을 반환합니다.
...
MultiValue 라우팅 방법의 또 다른 용도는 끝점이 IPv4 및 IPv6 주소 모두에 "이중 홈"이고 호출자에게 끝점에 대한 연결을 시작할 때 선택할 수 있는 두 가지 옵션을 모두 제공하려는 경우입니다.
...
아니요, 트래픽 관리자를 사용할 필요가 없습니다. 트래픽 관리자의 라우팅 측면은 실제 사용자 측정 부분과 별도로 작동하며 동일한 웹 속성에 둘 다 두 를 두는 것이 좋지만 필요하지는 않습니다.
...
아니요, 실제 사용자 측정이 작동하도록 Azure에서 서버 측 구성 요소를 호스팅할 필요가 없습니다. 단일 픽셀 이미지를 JavaScript 측정값에서 다운로드하고 다른 Azure 지역에서 실행되는 서비스를 Azure에서 호스트하고 관리합니다. 
...
집계 수준에서 프로필과 해당 엔드포인트에서 받은 트래픽을 이해하는 데 Azure Monitor를 사용할 수 있습니다. 또한 이를 통해 상태 검사 결과를 노출하여 엔드포인트의 상태를 추적할 수 있습니다. 이러한 수준을 넘어 지역 수준에서 Azure에 연결하는 최종 사용자의 환경을 이해해야 하는 경우 트래픽 보기를 사용하여 이를 달성할 수 있습니다.
...
Traffic Manager 프로필의 Azure 지역 외부에서 호스팅되는 외부 엔드포인트를 사용하는 경우 해당 대기 시간 특성에 대한 프록시인 Azure 지역에 매핑되도록 선택할 수 있습니다(실제로 성능 라우팅 메서드를 사용하는 경우 필요함). 이 Azure 지역 매핑이 있는 경우 트래픽 보기 출력을 만들 때 해당 Azure 지역의 대기 시간 메트릭이 사용됩니다. Azure 지역을 지정하지 않은 경우 해당 외부 엔드포인트의 데이터에서 대기 시간 정보가 비게 됩니다.
...
### <a name="how-do-i-move-my-traffic-manager-profiles-azure-endpoints-to-a-different-resource-group-or-subscription"></a>트래픽 관리자 프로필의 Azure 끝점을 다른 리소스 그룹 또는 구독으로 이동하려면 어떻게 해야 합니까?

Traffic Manager 프로필과 연결된 Azure 엔드포인트는 리소스 ID를 사용하여 추적됩니다. 엔드포인트로 사용 중인 Azure 리소스(예: 공용 IP, 클래식 클라우드 서비스, WebApp 또는 중첩된 방식으로 사용되는 다른 트래픽 관리자 프로필)가 다른 리소스 그룹 또는 구독으로 이동되면 해당 리소스 ID가 변경됩니다. 이 시나리오의 경우 현재로서는 프로필에 먼저 엔드포인트를 삭제한 다음 다시 추가하여 Traffic Manager 프로필을 업데이트해야 합니다.
...
### <a name="what-happens-when-an-incoming-querys-record-type-is-different-from-the-record-type-associated-with-the-addressing-type-of-the-endpoints"></a>들어오는 쿼리의 레코드 유형이 끝점의 주소 지정 유형과 연결된 레코드 유형과 다른 경우 어떻게 됩니까?
...
### <a name="i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this"></a>트래픽 관리자 프로필에서 웹 응용 프로그램 끝점을 중지했지만 다시 시작한 후에도 트래픽이 수신되지 않습니다. 이 문제를 어떻게 해결할 수 있나요?

Azure 웹 응용 프로그램 끝점이 중지되면 트래픽 관리자는 해당 상태 확인을 중지하고 끝점이 다시 시작된 것을 감지한 후에만 상태 검사를 다시 시작합니다. 이러한 지연을 방지하려면 엔드포인트를 다시 시작한 후에 Traffic Manager 프로필에서 해당 엔드포인트를 사용하지 않도록 설정한 후 다시 사용하도록 설정합니다.
...
TCP 모니터링을 사용하는 경우 Traffic Manager는 엔드포인트의 지정된 포트에 SYN 요청을 전송하여 3방향 TCP 핸드셰이크를 시작합니다. 그런 다음 시간(시간 지정 설정에 지정된) 시간 동안 끝점에서 SYN-ACK 응답을 기다립니다.

- 모니터링 설정에 지정된 시간 설정 기간 내에 SYN-ACK 응답이 수신되면 해당 끝점이 정상으로 간주됩니다. FIN 또는 FIN-ACK는 정기적으로 소켓을 종료할 때 트래픽 관리자의 예상 응답입니다.
- 지정된 시간 시간 이후에 SYN-ACK 응답이 수신되면 트래픽 관리자는 RST로 응답하여 연결을 재설정합니다.
...
이에 대한 일반적인 사용 사례는 Traffic Manager 요청이 다중 테넌트 환경에서 호스트된 엔드포인트로 올바르게 라우팅되도록 호스트 헤더를 지정하는 것입니다. 이 것의 또 다른 사용 사례는 엔드 포인트의 HTTP(S) 요청 로그에서 트래픽 관리자 요청을 식별하는 것입니다.
