<properties 
   pageTitle="장애 조치(Failover) 부하 분산 구성"
   description="이 문서는 트래픽 관리자에서 장애 조치(Failover) 부하 분산을 구성하는 데 도움이 됩니다."
...
Azure 웹 사이트는 웹 사이트 모드에 관계없이 데이터 센터(지역이라고도 함) 내의 웹 사이트에 대해 이미 장애 조치(Failover) 부하 분산 기능을 제공합니다. 트래픽 관리자를 통해 다른 데이터 센터의 웹 사이트에 대해 장애 조치(Failover) 부하 분산을 지정할 수 있습니다.

## 장애 조치(Failover) 부하 분산을 구성하려면
