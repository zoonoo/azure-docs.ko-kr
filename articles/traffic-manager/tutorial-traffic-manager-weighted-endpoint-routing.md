---
title: 자습서 - Azure Traffic Manager를 사용하여 가중 엔드포인트로 트래픽 라우팅
...
ms.locfileid: "49649437"
---
# <a name="tutorial-control-traffic-routing-with-weighted-endpoints-using-traffic-manager"></a>자습서: Traffic Manager를 사용하여 가중 트래픽 라우팅 제어 

이 자습서는 Traffic Manager를 사용하여 가중 라우팅 방법으로 엔드포인트 간에 사용자 트래픽 라우팅을 제어하는 방법을 설명합니다. 이 라우팅 방법에서는 Traffic Manager 프로필 구성에서 각 엔드포인트에 가중치를 할당하며, 사용자 트래픽은 각 엔드포인트에 할당된 가중치에 기반하여 라우팅됩니다. 가중치는 1에서 1000 사이의 정수입니다. 엔드포인트에 할당된 가중치 값이 높을수록 우선 순위가 높습니다.
...
- Traffic Manager를 테스트하기 위한 두 개의 테스트 VM - **미국 동부**에 VM 하나 및 **유럽 서부**에 두 번째 VM. 테스트 VM을 사용하여 Traffic Manager가 엔드포인트에 더 높은 가중치가 할당된 웹 사이트로 사용자 트래픽을 라우팅하는 방법을 자세히 보여줍니다.
...
이 섹션에서는 VM, *mVMEastUS*를 만듭니다. 이 VM을 사용하여 Traffic Manager가 어떻게 트래픽을 가중치 값이 더 높은 웹 사이트 엔드포인트로 라우팅하는지 테스트하려고 합니다.
...
6. **요약**의 **만들기** 아래에서 **만들기**를 선택하여 VM 배포를 시작합니다.
8. VM을 만드는 데 몇 분이 걸립니다. 두 VM이 모두 만들어질 때까지 나머지 단계를 수행하지 마세요.
...
2. **Traffic Manager 프로필 만들기**에서 다음 정보를 입력하거나 선택하고, 나머지 설정은 기본값을 그대로 적용한 다음, **만들기**를 선택합니다.
    ...
    | 구독            | 구독을 선택합니다.                          |
    | 리소스 그룹          | **기존 항목 사용**을 선택하고 *myResourceGroupTM1*을 선택합니다. |
    |        |   |
...
IIS 서버를 실행하는 두 개의 VM, *myIISVMEastUS*  & *myIISVMWEurope*을 추가하여 사용자 트래픽을 이들 엔드포인트로 라우팅합니다.
...
4. 2단계와 3단계를 반복하여 *myIISVMWEurope*라는 이름의 IIS 서버 VM과 연결된 공용 IP 주소 *myIISVMWEurope-ip*에 대해 *myWestEuropeEndpoint*라는 이름의 또 다른 엔드포인트를 추가하며 **가중치**에는 **25**를 입력합니다. 
...
4. **확인**을 선택합니다.
5. 로그인 프로세스 중에 인증서 경고가 나타날 수 있습니다. 경고 메시지가 표시되면 **예** 또는 **계속**을 선택하여 연결을 계속합니다. 
6. VM *myVMEastUS*의 웹 브라우저에서 Traffic Manager 프로필의 DNS 이름을 입력하여 웹 사이트를 봅니다. IIS 서버 *myIISVMWEurope*(**25**의 낮은 엔드포인트 가중치 값 할당)과 비교하여 **100**의 더 높은 가중치 값이 할당되었으므로 IIS 서버 *myIISVMEastUS*에서 호스팅된 웹 사이트로 라우팅됩니다.
...
더 이상 필요하지 않은 경우 이 자습서에서 리소스 그룹을 삭제합니다. 이렇게 하려면 리소스 그룹(**ResourceGroupTM1** 또는 **ResourceGroupTM2**)을 선택한 다음, **삭제**를 선택합니다.
