...
DNS 이름이 사용자 프로필에서 끝점의 IP 주소로 확인되는지 검사 중입니다. 이름은 Traffic Manager 프로필에서 정의된 트래픽 라우팅 방법과 일치하는 방식으로 해결해야 합니다. DNS 이름을 해결하기 위해 **nslookup**와 같은 도구를 사용하거나 또는 **dig**할 수 있습니다.
...
3. 확인된 IP 주소가 기본 끝점과 일치하는지 확인합니다.
4. Traffic Manager가 응용 프로그램이 종료되었다고 인식되도록 기본 끝점을 종료하거나 모니터링 파일을 제거합니다.
...
7. 확인된 IP 주소가 보조 끝점과 일치하는지 확인합니다.
8. 프로세스를 반복하여 각 끝점을 차례로 종료합니다. DNS가 목록에서 다음 끝점의 IP 주소를 반환하는지 확인합니다. 모든 끝점이 종료되면 다시 기본 끝점의 IP 주소를 얻게 됩니다.

## <a name="how-to-test-the-weighted-traffic-routing-method"></a>가중치 적용 트래픽 라우팅 메서드를 테스트하는 방법

1. 모든 끝점을 실행 상태로 둡니다.
2. 단일 클라이언트를 사용할 경우, Nslookup이나 유사한 유틸리티를 사용하여 회사의 도메인 이름에 대한 DNS 확인을 요청합니다.
3. 확인된 IP 주소가 끝점 중 하나와 일치하는지 확인합니다.
