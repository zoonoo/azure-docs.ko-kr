...
4. 가져와서 확인된 IP 주소가 기본 끝점에 대한 주소인지 확인합니다.
5. 트래픽 관리자가 종료되었다고 인식되도록 기본 끝점을 종료하거나 모니터링 파일을 제거합니다.
6. 트래픽 관리자 프로필의 DNS TTL(Time-to-Live)에 2분을 추가하여 기다립니다. 예를 들어, DNS TTL이 300초(5분)인 경우 7분을 대기해야 합니다.
7. DNS 클라이언트 캐시 및 요청 DNS 확인을 플러시하십시오. Windows에서는 명령이나 Windows PowerShell 프롬프트에서 ipconfig /flushdns 명령을 실행하여 DNS 캐시를 플러시할 수 있습니다.
8. 보조 끝점에 대한 IP 주소를 가져왔는지 확인합니다.
