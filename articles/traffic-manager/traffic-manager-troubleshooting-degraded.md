...
이 문서에서는 성능 저하 상태를 보여 주는 Azure Traffic Manager 프로필 문제를 해결하는 방법을 설명합니다. Azure Traffic Manager 저하 된 상태 문제를 해결 하는 첫 번째 단계는 로깅을 사용 하도록 설정 하는 것입니다.  자세한 내용은 [리소스 로그 사용](./traffic-manager-diagnostic-logs.md) 을 참조 하세요. 이 시나리오의 경우 사용자의 일부 .cloudapp.net 호스티드 서비스를 가리키는 Traffic Manager 프로필을 구성했다는 점을 고려합니다. Traffic Manager의 상태가 **성능 저하됨** 상태를 표시하는 경우 하나 이상의 엔드포인트 상태가 **성능 저하됨** 일 수 있습니다.
...
* Traffic Manager는 프로브가 프로브 경로에서 다시 HTTP 200 응답을 수신하는 경우에만 엔드포인트를 온라인 상태로 변경할 것을 고려합니다. 응용 프로그램에서 다른 HTTP 응답 코드를 반환 하는 경우 해당 응답 코드를 Traffic Manager 프로필의 [예상 상태 코드 범위](./traffic-manager-monitoring.md#configure-endpoint-monitoring) 에 추가 해야 합니다.
* Traffic Manager 프로필의 [예상 상태 코드 범위](./traffic-manager-monitoring.md#configure-endpoint-monitoring) 에서 유효한 응답 코드로 지정 하지 않는 한 30 배 리디렉션 응답은 실패로 처리 됩니다. Traffic Manager는 리디렉션 대상을 검색 하지 않습니다.
