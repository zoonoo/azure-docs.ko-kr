...
ms.locfileid: "76938376"
---
# <a name="troubleshooting-degraded-state-on-azure-traffic-manager"></a>Azure Traffic Manager의 성능 저하 상태 문제 해결

이 문서에서는 성능 저하 상태를 보여 주는 Azure Traffic Manager 프로필 문제를 해결하는 방법을 설명합니다. Azure 트래픽 관리자 성능저하 상태의 문제를 해결하는 첫 번째 단계는 진단 로깅을 사용하도록 설정하는 것입니다.  자세한 내용은 [진단 로그 활성화를](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-diagnostic-logs) 참조하십시오. 이 시나리오의 경우 사용자의 일부 .cloudapp.net 호스티드 서비스를 가리키는 Traffic Manager 프로필을 구성했다는 점을 고려합니다. Traffic Manager의 상태가 **성능 저하됨** 상태를 표시하는 경우 하나 이상의 엔드포인트 상태가 **성능 저하됨**일 수 있습니다.
...
* Traffic Manager는 프로브가 프로브 경로에서 다시 HTTP 200 응답을 수신하는 경우에만 엔드포인트를 온라인 상태로 변경할 것을 고려합니다. 응용 프로그램에서 다른 HTTP 응답 코드를 반환하는 경우 해당 응답 코드를 트래픽 관리자 프로필의 [예상 상태 코드 범위에](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) 추가해야 합니다.
* 트래픽 관리자 프로필의 예상 상태 코드 범위에서 유효한 응답 코드로 지정하지 않은 경우 30x 리디렉션 응답은 실패로 [처리됩니다.](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) 트래픽 관리자는 리디렉션 대상을 검색하지 않습니다.
...
* [컬](https://curl.haxx.se/)
* [wget](http://gnuwin32.sourceforge.net/packages/wget.htm)

또한 Internet Explorer에서 F12 디버깅 도구의 네트워크 탭을 사용하여 HTTP 응답을 확인할 수 있습니다.

이 예제에서는 프로브 URL의 응답을 보려고 합니다: http:\//watestsdp2008r2.cloudapp.net:80/Probe. 다음 PowerShell 예는 이러한 문제를 보여 줍니다.
