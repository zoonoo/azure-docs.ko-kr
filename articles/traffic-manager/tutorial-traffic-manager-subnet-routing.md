...
4. **확인**을 선택합니다.
5. 로그인 프로세스 중에 인증서 경고가 나타날 수 있습니다. 경고 메시지가 표시되면 **예** 또는 **계속**을 선택하여 연결을 계속합니다. 
1. VM *UserVMUS*의 웹 브라우저에서 Traffic Manager 프로필의 DNS 이름을 입력하여 웹 사이트를 봅니다. VM *UserVMUS* IP 주소가 엔드포인트 *myInternalWebsiteEndpoint*와 연결되어 있으므로 웹 브라우저는 테스트 웹 사이트 서버 - *InternalWebsite*를 시작합니다.

2. 다음으로, 1~5단계를 사용하여 **유럽 서부**에 있는 VM *UserVMEurope*에 연결하고 이 VM에서 Traffic Manager 프로필 도메인 이름을 찾아서 이동합니다. VM *UserVMEurope* IP 주소가 엔드포인트 *myProductionWebsiteEndpoint*와 연결되어 있으므로 웹 브라우저는 테스트 웹 사이트 서버 - *ProdWebsite*를 시작합니다. 
