...
이 섹션에서는 **미국 동부**와 **유럽 서부** Azure 지역에 *InternalWebsite* 및 *ProdWebsite*라는 2개의 VM을 만듭니다.
...
7. VM -*InternalWebsite*에서 Windows PowerShell을 실행하고, 다음 명령을 사용하여 IIS 서버를 설치하고 기본 htm 파일을 업데이트합니다.
    ...
    ```
8. *InternalWebsite* VM과의 RDP 연결을 종료합니다.
9. *myResourceGroupTM2* 리소스 그룹 내에서 *ProdWebsite* VM을 사용하여 RDP 연결을 만들어서 IIS를 설치하고 기본 웹 페이지를 사용자 지정하여 1-6 단계를 반복합니다.
10. *ProdWebsite* VM에서 Windows PowerShell을 실행하고, 다음 명령을 사용하여 IIS 서버를 설치하고 기본 htm 파일을 업데이트합니다.
    ```powershell-interactive
    # Install IIS
    Install-WindowsFeature -name Web-Server -IncludeManagementTools
...
Traffic Manager는 서비스 엔드포인트의 DNS 이름을 기반으로 사용자 트래픽을 라우팅합니다. 이 섹션에서는 *InternalWebsite* 및 *ProdWebsite*라는 IIS 서버의 DNS 이름을 구성합니다.

1. 왼쪽 메뉴에서 **모든 리소스**를 클릭한 다음, 리소스 목록에서 *myResourceGroupTM1* 리소스 그룹에 있는 *InternalWebsite*를 선택합니다.
2. **개요** 페이지의 **DNS 이름**에서 **구성**을 선택합니다.
3. **구성** 페이지의 DNS 이름 레이블 아래에서 고유 이름을 추가한 다음, **저장**을 선택합니다.
4. *myResourceGroupTM2* 리소스 그룹에 있는 이름이 *ProdWebsite*인 VM에 대해 1~3단계를 반복합니다.

### <a name="create-test-vms"></a>테스트 VM 만들기

이 섹션에서는 각 Azure 지역(**미국 동부** 및 **유럽 서부**)에 VM(*UserVMUS* 및 *UserVMEurope*)을 만듭니다. 이 VM을 사용하여 사용자가 웹 사이트를 탐색할 때 Traffic Manager가 가장 가까운 IIS 서버로 트래픽을 라우팅하는 방식을 테스트합니다.

1. Azure Portal의 왼쪽 위 모서리에서 **리소스 만들기** > **Compute** > **Windows Server 2016 Datacenter**를 선택합니다.
2. **기본 사항**에 다음 정보를 입력하거나 선택하고, 나머지 설정에 대한 기본값을 그대로 적용한 다음, **만들기**를 선택합니다.
...
IIS 서버를 실행하는 두 개의 VM - *InternalWebsite* & *ProdWebsite*을 추가하여 사용자 쿼리의 서브넷을 기반으로 사용자 트래픽을 라우팅합니다.
...
3. 다음 정보를 입력하거나 선택하고, 나머지 설정에 대한 기본값을 그대로 적용한 다음, **확인**을 선택합니다.
    ...
    | ---                     | ---                                                |
    | type                    | Azure 엔드포인트                                   |
    | 이름           | myInternalWebSiteEndpoint                                        |
    | 대상 리소스 종류           | 공용 IP 주소                          |
    | 대상 리소스          | **공용 IP 주소를 선택**하여 동일한 구독에 속하는 공용 IP 주소가 있는 리소스 목록을 표시합니다. **리소스**에서 *InternalWebsite-ip*라는 이름의 공용 IP 주소를 선택합니다. 이것은 미국 동부에 있는 IIS 서버 VM의 공용 IP 주소입니다.|
    |  서브넷 라우팅 설정    |   *UserVMUS* 테스트 VM의 IP 주소를 추가합니다. 이 VM에서 시작된 사용자 쿼리는 *InternalWebSiteEndpoint*로 전달됩니다.    |

4. 2단계와 3단계를 반복하여 *ProdWebsite*라는 이름의 IIS 서버 VM과 연결된 공용 IP 주소 *ProdWebsite-ip*에 대해 *ProdWebsiteEndpoint*라는 이름의 또 다른 엔드포인트를 추가합니다. **서브넷 라우팅 설정**에 대해 테스트 VM - *UserVMEurope*의 IP 주소를 추가합니다. 이 테스트 VM의 모든 사용자 쿼리는 엔드포인트 - *myProdWebsiteEndpoint*로 라우팅됩니다.
...
2. 실행 중인 Traffic Manager를 보는 방법은 다음과 같습니다.
    - **미국 동부** 지역에 있는 테스트 VM(*UserVMUS*)의 웹 브라우저에서 Traffic Manager 프로필의 DNS 이름을 찾아서 이동합니다.
    - **유럽 서부** 지역에 있는 테스트 VM(*UserVMEurope*)의 웹 브라우저에서 Traffic Manager 프로필의 DNS 이름을 찾아서 이동합니다.
...
1. 왼쪽 메뉴에서 **모든 리소스**를 선택한 다음, 리소스 목록에서 *myResourceGroupTM1* 리소스 그룹에 있는 *myUserVMUS*를 클릭합니다.
2. **개요** 페이지에서 **연결**을 클릭한 다음, **가상 머신에 연결**에서 **RDP 파일 다운로드**를 선택합니다.
3. 다운로드한 rdp 파일을 엽니다. 메시지가 표시되면 **연결**을 선택합니다. VM을 만들 때 지정한 사용자 이름과 암호를 입력합니다. **추가 선택 사항**, **다른 계정 사용**을 차례로 선택하여 VM을 만들 때 입력한 자격 증명을 지정해야 할 수도 있습니다.
4. **확인**을 선택합니다.
5. 로그인 프로세스 중에 인증서 경고가 나타날 수 있습니다. 경고 메시지가 표시되면 **예** 또는 **계속**을 선택하여 연결을 계속합니다.
1. VM *UserVMUS*의 웹 브라우저에서 Traffic Manager 프로필의 DNS 이름을 입력하여 웹 사이트를 봅니다. VM *UserVMUS* IP 주소가 엔드포인트 *myInternalWebsiteEndpoint*와 연결되어 있으므로 웹 브라우저는 테스트 웹 사이트 서버인 *InternalWebsite*를 시작합니다.

2. 다음으로, 1~5단계를 사용하여 **유럽 서부**에 있는 VM *UserVMEurope*에 연결하고 이 VM에서 Traffic Manager 프로필 도메인 이름을 찾아서 이동합니다. VM *UserVMEurope* IP 주소가 엔드포인트 *myProductionWebsiteEndpoint*와 연결되어 있으므로 웹 브라우저는 테스트 웹 사이트 서버인 *ProdWebsite*를 시작합니다.
