...
2. **Traffic Manager 프로필** 블레이드에서 **설정** 섹션의 **끝점**을 클릭합니다.
3. 표시되는 **끝점** 블레이드에서 **추가**를 클릭합니다.
4. **끝점 추가** 블레이드에서 다음과 같이 완료합니다.
    1. **형식**의 경우 **Azure 끝점**을 클릭합니다.
    2. 이 끝점을 인식하는 기준으로 사용할 **이름**을 제공합니다.
    3. **대상 리소스 형식**의 경우 **App Service**를 클릭합니다.
    4. **대상 리소스**의 경우 **앱 서비스 선택**을 클릭하여 동일한 구독에서 웹앱의 목록을 표시합니다. 표시되는 **리소스** 블레이드에서 첫 번째 끝점으로 추가할 App Service를 선택합니다.
...
더 이상 필요하지 않으면 사용자가 만든 리소스 그룹 및 Traffic Manager 프로필을 삭제합니다. 이렇게 하려면 **Traffic Manager 프로필** 블레이드에서 리소스 그룹을 선택하고 **삭제**를 클릭합니다.
