---
title: 트래픽 관리자 라운드 로빈 트래픽 라우팅 방법 구성 | Microsoft Docs
...
---
# 라운드 로빈 라우팅 방법 구성
일반적인 트래픽 라우팅 방법 패턴은 클라우드 서비스와 웹 사이트를 포함하는 일련의 동일한 끝점을 제공하고 각 끝점에 라운드 로빈 방식으로 트래픽을 전송하는 것입니다. 아래 단계에서는 이 유형의 트래픽 라우팅 방법을 수행하기 위해 트래픽 관리자를 구성하는 방법을 설명합니다. 다른 트래픽 라우팅 방법에 대한 자세한 내용은 [트래픽 관리자 트래픽 라우팅 방법 정보](traffic-manager-routing-methods.md)를 참조하세요.

> [!NOTE]
> Azure 웹 사이트는 데이터 센터(지역이라고도 함) 내의 웹 사이트에 대해 이미 라운드 로빈 부하 분산 기능을 제공합니다. 트래픽 관리자를 통해 다른 데이터 센터의 웹 사이트에 대해 라운드 로빈 트래픽 라우팅 방법을 지정할 수 있습니다.
> 
> 
