...
- Windows 장치 관리자에서 장치가 열거 되는지 먼저 확인 합니다.
...
- 먼저 장치 관리자에서 마이크 배열이 열거 되었는지 확인 합니다.
- 장치가 열거 되 고 Windows에서 제대로 작동 하지 않는 경우 펌웨어 업데이트 창에서 깊이 카메라에 다른 컨테이너 ID를 할당 한 후 문제가 발생할 수 있습니다.
- 장치 관리자로 이동 하 고 "Azure Kinect 마이크 배열"을 마우스 오른쪽 단추로 클릭 한 다음 "장치 제거"를 선택 하 여 다시 설정 해 볼 수 있습니다. 완료 되 면 센서를 분리 하 고 다시 연결 합니다.
...
**Windows:** 열려 있는 호스트 컨트롤러를 확인 하려면 장치 관리자
