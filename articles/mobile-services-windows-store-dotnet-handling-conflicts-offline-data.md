...
<a href="/ko-kr/documentation/articles/mobile-services-windows-store-dotnet-handling-conflicts-offline-data" title="Windows 스토어 C#" class="current">Windows 스토어 C#</a>
...
이 자습서에서는 오프라인 데이터와 온라인 데이터를 모두 지원하는 앱을 다운로드하고, 모바일 서비스를 앱에 통합한 다음 Azure 관리 포털에 로그인하여 앱을 실행하면서 데이터베이스를 보고 업데이트합니다.
...
1.  [충돌 처리 코드 샘플][충돌 처리 코드 샘플]의 C# 버전을 다운로드합니다.

2.  [SQLite for Windows 8.1][SQLite for Windows 8.1]을 아직 설치하지 않은 경우 설치합니다.
...
앱을 다른 컴퓨터에 배포하여 앱의 두 인스턴스를 실행함으로써 충돌을 생성하려면 [데이터베이스 충돌 처리][데이터베이스 충돌 처리] 자습서의 배포 지침을 따를 수 있습니다.
