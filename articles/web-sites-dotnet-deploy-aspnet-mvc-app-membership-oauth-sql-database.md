...
1. **새 ASP.NET 프로젝트** 대화 상자에서 **MVC** 템플릿을 선택합니다. **인증**이 **개별 사용자 계정**으로 설정되고 **클라우드에 호스트**가 선택되고 **웹 앱**이 선택되었는지 확인합니다.
	...
	![새 ASP.NET 프로젝트 대화 상자](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/newproject.png)

1. 구성 마법사는 *ContactManager*를 기반으로 고유 이름을 제안합니다. 새 앱 서비스 계획 및 리소스 그룹을 만들지 여부를 결정해야 합니다. 새 계획 또는 리소스 그룹을 만들지 여부 결정에 대한 지침은 [Azure 앱 서비스에 대한 자세한 개요 계획](azure-web-sites-web-hosting-plans-in-depth-overview.md)을 참조하세요. 이 자습서에서 하나 이상의 새 리소스 그룹을 만들 수 있습니다. 근처에 있는 지역을 선택합니다. [azurespeed.com](http://www.azurespeed.com/ "AzureSpeed.com")을 사용하여 대기 시간이 가장 짧은 데이터 센터를 찾을 수 있습니다. 다음 단계에서 데이터베이스를 설정하므로 **확인**을 클릭하지 마세요.

   ![새 계획 및 리소스 그룹](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/newplanandgroup.png)
 
2. 기존에 만든 데이터베이스 서버가 없으면 **새 서버 만들기**를 선택하고 데이터베이스 이름, 사용자 이름 및 암호를 입력합니다.

   ![새 데이터베이스 사용](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/newdb.png)
...
3. 데이터베이스 서버가 있는 경우 해당 서버를 사용하여 새 데이터베이스를 만듭니다. 데이터베이스 서버는 중요한 리소스이며, 일반적으로 데이터베이스별로 데이터베이스 서버를 만들기보다는 테스트 및 개발을 위해 같은 서버에서 여러 데이터베이스 만들려고 합니다. 웹 앱 및 데이터베이스가 동일한 지역에 있어야 합니다.
    ...
    ![기존 데이터베이스 사용](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/useexistingdb.png)
...
1. Ctrl+F5를 눌러 앱을 실행합니다.
	...
	![로컬로 실행되는 웹 앱](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/rr2.png)
...
4. 웹 앱을 마우스 오른쪽 단추로 클릭하고 **중지**를 선택합니다. 
	...
	![웹 앱 중지](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/s1.png)

	또는 [Azure 포털](http://go.microsoft.com/fwlink/?LinkId=529715)에서 웹 앱을 선택한 다음 페이지 맨 아래에서 **중지** 아이콘을 클릭합니다.

	![웹 앱 포털 중지](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/stopweb.png)
...
5. Visual Studio 또는 포털에서 웹 앱을 시작합니다. **웹 앱이 중지된 동안에는 게시할 수 없습니다.**
	...
	![웹 앱 시작](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/ss15.png)
...
3. 이전에 이 데이터베이스에 연결하지 않은 경우, 사용자의 현재 IP 주소에 대한 액세스가 가능하도록 방화벽 규칙을 추가 하라는 메시지가 표시될 수 있습니다. IP 주소가 미리 채워집니다. **방화벽 규칙 추가**를 클릭만 하면 액세스할 수 있도록 합니다.
  ...
  ![방화벽 규칙 추가](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/addfirewallrule.png)
...
1. **canEdit** 역할 및 *user1@contoso.com*의 ID가 되도록 등록한 Google 계정의 ID를 확인합니다. **canEdit** 역할에 포함된 유일한 사용자여야 합니다. 다음 단계에서 확인하겠습니다.
	...
	![CM 페이지](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/s2.png)
