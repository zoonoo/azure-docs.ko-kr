...
아니요. 하지만 피어 링을 통해 Azure Red Hat OpenShift 클러스터를 기존 가상 네트워크에 연결할 수 있습니다. 자세한 내용은 [기존 가상 네트워크에 클러스터의 가상 네트워크 연결](tutorial-create-cluster.md#create-the-cluster) 을 참조 하세요.
...
고객은 명령을 `oc adm top nodes` 사용 하거나 `kubectl top nodes` 고객 관리 clusterrole를 사용 하 여 노드 수준에서 CPU/메모리 메트릭에 액세스할 수 있습니다.  또한 고객은 명령을 `pods` `oc adm top pods` 사용 하 여의 CPU/메모리 메트릭에 액세스할 수 있습니다.`kubectl top pods`
