...
> [!IMPORTANT]
> Azure Red Hat OpenShift 4.3은 현재 미국 동부 및 미국 동부 2에서 비공개 미리 보기 에서만 사용할 수 있습니다. 비공개 미리 보기 수락은 초대 전용입니다. 이 기능을 사용 하도록 설정 하기 전에 구독을 등록 해야 합니다. [Azure Red Hat OpenShift 비공개 미리 보기 등록](https://aka.ms/aro-preview-register)
...
- 에서 자동으로 AAD 응용 프로그램 및 서비스 주체를 만들 `az aro create` 수 있도록 하는 클러스터 AAD 응용 프로그램 (클라이언트 ID 및 비밀) 및 서비스 주체 또는 충분 한 aad 권한이 필요 합니다.

- RP 서비스 사용자 및 클러스터 서비스 주체는 클러스터 가상 네트워크에 대 한 참가자 역할을 보유 해야 합니다.  가상 네트워크에서 "사용자 액세스 관리자" 역할을 사용 하는 경우에서 `az aro create` 자동으로 역할 할당을 설정 합니다.
...
> [!Note] 
> (이므로 `az aro` 확장은 미리 보기로 제공 됩니다. 이후 릴리스에서 변경 되거나 제거 될 수 있습니다.
...
2. 다음 명령을 실행 하 여 확장을 `az aro` 설치 합니다.
...
Red Hat pull 비밀을 사용 하면 클러스터가 Red Hat 컨테이너 레지스트리 및 추가 콘텐츠에 액세스할 수 있습니다. 끌어오기 암호를 사용 하는 것은 선택 사항 이지만 권장 됩니다.

끌어오기 비밀을 가져오려면 다음을 수행 합니다.

1. https://cloud.redhat.com/openshift/install/azure/aro-provisioned 으로 이동합니다.
1. Red Hat 계정에 로그인 하거나 비즈니스 전자 메일을 사용 하 여 새 Red Hat 계정을 만듭니다. 사용 약관에 동의 합니다.
1. **끌어오기 비밀 다운로드**를 선택 합니다.

*Pull-secret* 파일을 안전한 위치에 저장 합니다. 클러스터를 만들 때마다이 파일을 사용 합니다.
...
Azure Red Hat OpenShift 4.3 클러스터 리소스 아래에서 형식의 `https://console-openshift-console.apps.<random>.<location>.aroapp.io/`클러스터 콘솔 URL을 찾을 수 있습니다. 다음 명령을 실행 하 여 리소스를 봅니다.
