...
`az` CLI 명령을 사용 하 여 Azure Red Hat OpenShift 클러스터를 만들 수 없는 경우 만들기를 다시 시도 하면 계속 실패 합니다.
...
현재 Azure CLI (`az openshift create` 명령)에서 자동으로 생성 되는 `Microsoft.ContainerService/openShiftManagedClusters` 리소스는 Azure Portal에서 숨겨집니다. **리소스 그룹** 보기에서 **숨겨진 유형 표시** 를 선택 하 여 리소스 그룹을 봅니다.
