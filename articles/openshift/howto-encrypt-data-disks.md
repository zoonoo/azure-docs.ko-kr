...
* **Oc** openshift 명령줄 도구인 base64 (coreutils의 일부)와 **az** Azure CLI가 설치 되어 있습니다.
...
Azure Key Vault 인스턴스를 사용 하 여 키를 저장 해야 합니다. 제거 보호를 사용 하는 새 Key Vault를 만듭니다. 그런 다음, 자격 증명 모음 내에 새 키를 만들어 사용자 지정 키를 저장 합니다.
...
## <a name="create-a-k8s-storage-class-for-encrypted-premium--ultra-disks"></a>암호화 된 프리미엄 & 울트라 디스크에 대 한 k8s 저장소 클래스 만들기
...
## <a name="test-encryption-with-customer-managed-keys-optional"></a>고객 관리 키를 사용 하 여 암호화 테스트 (선택 사항)
클러스터에서 PVC 암호화를 위해 고객이 관리 하는 키를 사용 하 고 있는지 확인 하려면 새 저장소 클래스를 사용 하 여 영구 볼륨 클레임을 만듭니다. 아래 코드 조각은 pod를 만들고 프리미엄 디스크를 사용 하 여 영구 볼륨 클레임을 탑재 합니다.
```
...
EOF
```
### <a name="apply-the-test-pod-configuration-file-optional"></a>테스트 pod 구성 파일을 적용 합니다 (선택 사항).
...
```
> [!NOTE]
> 경우에 따라 Azure Active Directory 내에서 역할 할당을 적용할 때 약간의 지연이 있을 수 있습니다. 이러한 지침이 실행 되는 속도에 따라 "전체 Azure 디스크 이름 확인" 명령이 성공 하지 못할 수 있습니다. 이 문제가 발생 하는 경우 **oc 설명 pvc mypod-cmk-encryption-pvc** 를 검토 하 여 디스크가 성공적으로 프로 비전 되었는지 확인 합니다. 역할 할당 전파가 완료 되지 않은 경우 Pod &를 *삭제* 하 고이를 다시 *적용* 해야 할 수 있습니다.

### <a name="verify-pvc-disk-is-configured-with-encryptionatrestwithcustomerkey-optional"></a>PVC 디스크가 "EncryptionAtRestWithCustomerKey"로 구성 되어 있는지 확인 합니다 (선택 사항).
