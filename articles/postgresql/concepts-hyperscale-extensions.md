...
PostgreSQL 확장을 사용하려면 먼저 데이터베이스에 설치해야 합니다. 특정 확장을 설치 하려면 psql 도구에서 [확장 만들기](https://www.postgresql.org/docs/9.6/static/sql-createextension.html) 명령을 실행 하 여 패키지 된 개체를 데이터베이스에 로드 합니다.
...
Pg_stat_statements에서 제공 하는 쿼리 실행 정보와 각 SQL 문을 로깅할 때 서버 성능에 미치는 영향을 서로 절충 합니다. Pg_stat_statements 확장을 사용 하지 않는 경우 `none``pg_stat_statements.track` 설정 하는 것이 좋습니다. 일부 타사 모니터링 서비스는 pg_stat_statements를 사용 하 여 쿼리 성능 정보를 제공할 수 있으므로 이것이 사용자의 사례 인지 여부를 확인 합니다.
