...
이러한 단계는 범용 또는 메모리 최적화 계층에서 마스터 서버를 준비하는 데 사용합니다. 마스터 서버는 azure.replication_support 매개 변수를 설정하여 복제를 위해 준비됩니다. 복제 매개 변수가 변경되는 경우, 변경 내용을 적용하기 위해서는 서버 재시작이 필요합니다. Azure portal에서 이러한 두 단계는 **복제 지원 사용**이라는 하나의 단추로 가능합니다.
...
5. 복제본에 대한 위치를 선택합니다. 기본 위치는 마스터 서버와 동일합니다.
