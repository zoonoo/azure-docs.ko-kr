---
title: 읽기 복제본 관리 - Azure 포털 - PostgreSQL용 Azure 데이터베이스 - 단일 서버
description: Azure 포털에서 PostgreSQL - 단일 서버에 대한 읽기 복제본 Azure 데이터베이스를 관리하는 방법을 알아봅니다.
author: rachel-msft
ms.author: raagyema
...
---
# <a name="create-and-manage-read-replicas-in-azure-database-for-postgresql---single-server-from-the-azure-portal"></a>PostgreSQL - Azure 포털에서 단일 서버에 대한 Azure 데이터베이스에서 읽기 복제본 만들기 및 관리
...
이러한 단계는 범용 또는 메모리 최적화 계층에서 마스터 서버를 준비하는 데 사용합니다. 마스터 서버는 azure.replication_support 매개 변수를 설정하여 복제할 준비가 되어 있습니다. 복제 매개 변수가 변경되면 변경이 적용되려면 서버를 다시 시작해야 합니다. Azure 포털에서 이러한 두 단계는 **복제 지원 활성화라는**단일 단추로 캡슐화됩니다.

1. Azure Portal에서 마스터로 사용할 기존 Azure Database for PostgreSQL 서버를 선택합니다.

2. 서버 사이드바에서 **설정**에서 **복제를**선택합니다.

> [!NOTE] 
> **복제 지원 사용 중지가** 회색으로 표시되면 복제 설정이 기본적으로 서버에 이미 설정되어 있습니다. 다음 단계를 건너뛰고 읽기 복제본을 만들 수 있습니다. 

3. **복제 지원 활성화를 선택합니다.** 

   ![복제 지원 사용](./media/howto-read-replicas-portal/enable-replication-support.png)

4. 복제 지원을 사용하도록 설정하려는지 확인합니다. 이 작업은 마스터 서버를 다시 시작합니다. 

   ![복제 지원 활성화 확인](./media/howto-read-replicas-portal/confirm-enable-replication.png)
   
5. 작업이 완료되면 두 개의 Azure 포털 알림을 받게 됩니다. 서버 매개 변수를 업데이트하는 알림이 하나 있습니다. 서버 다시 시작에 대한 또 다른 알림이 즉시 표시됩니다.

   ![성공 알림 - 사용](./media/howto-read-replicas-portal/success-notifications-enable.png)

6. Azure 포털 페이지를 새로 고쳐 복제 도구 모음을 업데이트합니다. 이제 이 서버에 대한 읽기 복제본을 만들 수 있습니다.

   ![업데이트된 도구 모음](./media/howto-read-replicas-portal/updated-toolbar.png)
   
복제 지원 은 마스터 서버당 일회성 작업입니다. 사용자의 편의를 위해 **복제 지원 사용 안 함** 버튼이 제공됩니다. 이 마스터 서버에서 복제본을 만들지 않을 것이라고 확신하지 않는 한 복제 지원을 사용하지 않는 것은 권장되지 않습니다. 마스터 서버에 기존 복제본이 있는 동안에는 복제 지원을 비활성화할 수 없습니다.
...
5. 복제본의 위치를 선택합니다. 기본 위치는 마스터 서버의 위치와 동일합니다.
...
- Azure Portal에서 읽기 복제본에 대한 **개요** 페이지를 엽니다. **삭제를 선택합니다.**
...
2.  **메트릭을 선택합니다.** **메트릭** 창에서 **복제본 간 최대 지연 시간**을 선택합니다.
