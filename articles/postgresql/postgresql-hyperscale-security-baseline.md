---
title: Azure 보안 벤치 마크에 대 한 Azure Database for PostgreSQL-Hyperscale 보안 기준
...
ms.locfileid: "86531560"
---
# <a name="azure-database-for-postgresql---hyperscale-security-baseline-for-azure-security-benchmark"></a>Azure 보안 벤치 마크에 대 한 Azure Database for PostgreSQL-Hyperscale 보안 기준
...
**지침**: 해당 사항 없음 Citus (Azure Database for PostgreSQL-Hyperscale)는 가상 네트워크, 서브넷 또는 네트워크 보안 그룹에 연결할 수 없습니다.
...
**지침**: 해당 사항 없음 Citus (Azure Database for PostgreSQL-Hyperscale)에서 사용 하는 끝점은 모두 Microsoft에서 관리 합니다. 온-프레미스 시스템에 배포 하려는 추가 컨트롤을 담당 합니다.
...
**지침**: Microsoft는 로그의 타임 스탬프에 대 한 Citus (Azure Database for PostgreSQL-hyperscale)와 같은 Azure 리소스에 사용 되는 시간 원본을 유지 관리 합니다.
...
**지침**: 해당 사항 없음 Citus (Azure Database for PostgreSQL-Hyperscale)는 맬웨어 방지 관련 로그를 처리 하거나 생성 하지 않습니다.
...
**지침**: Citus (Azure Database for PostgreSQL-hyperscale)에 로그인 하는 것은 데이터베이스에서 직접 구성 된 사용자 이름/암호를 사용 하 고, ad (Azure Active Directory) id를 사용 하 고, Azure ad 토큰을 사용 하 여 연결 하는 데 사용할 수 있습니다. Azure AD 토큰을 사용하는 경우 Azure AD 사용자, Azure AD 그룹 또는 데이터베이스에 연결하는 Azure AD 애플리케이션과 같은 다양한 방법이 지원됩니다.
...
**지침**: 로컬 데이터베이스에 대 한 액세스 권한이 있는 사용자와 Azure Active Directory를 통해 액세스 권한을 검토 하 고 조정 합니다.

AD (Azure Active Directory) 로그를 검토 하 여 Citus (Hyperscale) 관리 역할이 있는 계정을 포함할 수 있는 부실 계정을 검색할 수 있습니다. 또한 Azure Id 액세스 검토를 사용 하 여 그룹 멤버 자격을 효율적으로 관리 하 고, Citus (Hyperscale)에 액세스 하는 데 사용할 수 있는 엔터프라이즈 응용 프로그램에 액세스 하 고, 역할 할당을 사용 합니다. 적절한 사용자만 계속 액세스할 수 있도록 사용자 액세스를 90일마다 정기적으로 검토해야 합니다.
...
**지침**: Microsoft Citus ()에 대 Azure Database for PostgreSQL 한 기본 인프라를 관리 하 고 고객 데이터 손실 또는 노출을 방지 하기 위해 엄격한 컨트롤을 구현 했습니다.
...
**지침**: 데이터 식별, 분류 및 손실 방지 기능은 아직 Citus (hyperscale)에서 사용할 수 없습니다.
...
Microsoft에서는 Citus (Hyperscale)에 대 한 기본 인프라를 관리 하 고 고객 데이터 손실 또는 노출을 방지 하기 위해 엄격한 컨트롤을 구현 했습니다.
...
**지침**: Azure Database for PostgreSQL 하루에 한 번 Citus ()는 데이터 파일과 데이터베이스 트랜잭션 로그의 스냅숏 백업을 수행 합니다. 백업을 사용 하면 보존 기간 내의 특정 시점으로 서버를 복원할 수 있습니다. (보존 기간은 현재 모든 클러스터에 대해 35 일입니다.) 모든 백업은 AES 256 비트 암호화를 사용 하 여 암호화 됩니다. Citus (PostgreSQL Hyperscale) 제품은 암호화에 Microsoft 관리 키를 사용 합니다.
...
**지침**: microsoft에서 관리 하는 기본 플랫폼의 경우 microsoft는 모든 고객 콘텐츠를 중요 한 것으로 간주 하 고 고객 데이터 손실 및 노출을 방지 하기 위해 좋은 길이로 이동 합니다. Azure 내에서 고객 데이터를 안전하게 유지하기 위해 Microsoft는 강력한 데이터 보호 제어 및 기능 모음을 구현하고 유지 관리합니다.
...
**지침**: Microsoft는 Citus (Azure Database for PostgreSQL-hyperscale)를 지 원하는 기본 시스템에서 취약성 관리를 수행 합니다. 데이터베이스 클러스터 및 관련 리소스의 구성과 관련 된 취약성에 대해서는 일반적인 위험 점수 매기기 프로그램 (예: 일반적인 취약성 점수 매기기 시스템) 또는 타사 검사 도구에서 제공 하는 기본 위험 등급을 사용 합니다.
...
**지침**: 해당 사항 없음 Azure Key Vault를 사용 하 여 데이터베이스 관리자 암호를 안전 하 게 관리할 수 있습니다. 데이터베이스 암호에 대 한 액세스를 제한 하는 Key Vault 비밀에 대해 적절 한 범위에 있는 사용 권한을 할당 합니다. 연결 문자열 또는 데이터베이스 암호를 코드 리포지토리에 저장 하지 마십시오.

* [PostgreSQL 데이터베이스에 연결 하는 방법](https://docs.microsoft.com/azure/postgresql/quickstart-create-hyperscale-portal#connect-to-the-database-using-psql)
...
Microsoft 맬웨어 방지는 Azure 서비스 (예: Hyperscale (Citus))를 지 원하는 기본 호스트에서 사용 하도록 설정 되지만 고객 콘텐츠에서 실행 되지 않습니다.
