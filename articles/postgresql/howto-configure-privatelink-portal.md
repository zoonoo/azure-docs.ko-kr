---
title: 개인 링크 - Azure 포털 - PostgreSQL용 Azure 데이터베이스 - 단일 서버
description: Azure 포털에서 PostgreSQL- 단일 서버에 대한 Azure 데이터베이스에 대한 개인 링크를 구성하는 방법에 대해 알아봅니다.
author: kummanish
ms.author: manishku
...
ms.locfileid: "79370919"
---
# <a name="create-and-manage-private-link-for-azure-database-for-postgresql---single-server-using-portal"></a>PostgreSQL - 포털을 사용하는 단일 서버에 대한 Azure 데이터베이스에 대한 개인 링크 만들기 및 관리

프라이빗 엔드포인트는 Azure에서 프라이빗 링크를 만드는 데 사용되는 기본 구성 요소입니다. 프라이빗 엔드포인트는 VM(Virtual Machines) 같은 Azure 리소스가 프라이빗 링크 리소스와 비공개로 통신할 수 있게 해줍니다.  이 문서에서는 Azure 포털을 사용하여 Azure 가상 네트워크에서 VM을 만들고 Azure 개인 끝점이 있는 PostgreSQL 단일 서버에 대한 Azure 데이터베이스를 만드는 방법을 배웁니다.
...
이 섹션에서는 개인 링크 리소스(Azure의 PostgreSQL 서버)에 액세스하는 데 사용되는 VM을 호스트하는 가상 네트워크및 서브넷을 만듭니다.
...
이 섹션에서는 Azure에서 PostgreSQL 서버에 대한 Azure 데이터베이스를 만듭니다. 

1. Azure 포털화면 왼쪽 상단에서**PostgreSQL에 대한**리소스 > **데이터베이스** > Azure 데이터베이스 **만들기를**선택합니다.

1. **PostgreSQL 배포에 대한 Azure 데이터베이스에서** **단일 서버를** 선택하고 다음 정보를 제공합니다.
...
이 섹션에서는 PostgreSQL 서버를 만들고 개인 끝점을 추가합니다. 
...
6. **프라이빗 엔드포인트 만들기 - 리소스**에서 다음 정보를 입력하거나 선택합니다.
    ...
    | Subscription| 구독을 선택합니다. |
    | 리소스 유형 | **마이크로소프트.DBforPostgreSQL/서버를 선택합니다.** |
    | 리소스 |*myServer*를 선택합니다.|
    |대상 하위 리소스 |*포스트그만sql서버* 선택|
    |||
7. **다음 을 선택합니다: 구성.**
8. **개인 끝점 만들기 - 구성에서**다음 정보를 입력하거나 선택합니다.
    ...
    |프라이빗 DNS 영역 |*(새)개인 링크.postgres.database.azure.com을 선택합니다.* |
    |||

1. **검토 + 만들기를**선택합니다. **검토 + 만들기** 페이지로 이동됩니다. 여기서 구성이 유효한지 검사됩니다. 
2. 유효성 검사 전달 메시지가 표시되면 **에서 만들기를** **선택합니다.** 

    ![만든 비공개 링크](media/concepts-data-access-and-security-private-link/show-postgres-private-link.png)
...
개인 끝점, PostgreSQL 서버 및 VM을 사용하여 완료되면 리소스 그룹과 포함된 모든 리소스를 삭제합니다.
...
이 방법에서는 가상 네트워크에서 VM, PostgreSQL용 Azure 데이터베이스 - 단일 서버 및 개인 액세스를 위한 개인 끝점을 만들었습니다. 인터넷에서 하나의 VM에 연결 하 고 개인 링크를 사용 하 여 PostgreSQL 서버에 보안 통신. 개인 끝점에 대한 자세한 내용은 [Azure 개인 끝점입니다.](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)
