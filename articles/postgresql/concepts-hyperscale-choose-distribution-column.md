...
다음 다이어그램에서는 다중 테넌트 데이터 모델에서의 공동 배치를 보여줍니다. Accounts 및 Campaigns 두 개의 테이블을 포함하며, 각각 `account_id`로 분산됩니다. 회색 상자는 분할을 포함하는 작업자 노드를 나타내는 해당 색의 분할을 나타냅니다. 녹색 분할은 하나의 작업자 노드에 함께 저장되고 파란색은 다른 노드에 저장됩니다. 동일한 account\_id로 두 테이블을 제한하는 경우 Accounts 및 Campaigns 간의 join 쿼리가 한 노드에서 필요한 모든 데이터를 함께 포함하는 방식에 주목하세요.
...
-   **공통 tenant\_id 열로 분산된 테이블을 파티션합니다.** 예를 들어, 테넌트가 회사들인 SaaS 응용 프로그램에서 tenant\_id는 company\_id가 될 것입니다.
