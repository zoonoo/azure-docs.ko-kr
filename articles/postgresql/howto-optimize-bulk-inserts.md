...
임시 데이터를 포함하거나 큰 데이터 세트를 대량으로 삽입하는 워크로드 작업이 있는 경우 unlogged 테이블을 사용하는 방안을 고려해 보세요.

Unlogged 테이블은 대량 삽입을 최적화하는 데 효과적으로 사용할 수 있는 PostgreSQL 기능입니다. PostgreSQL는 WAL(미리 쓰기 로그)을 사용합니다. 기본적으로 원자성 및 내구성을 제공합니다. 원자성, 일관성, 격리 및 내구성이 모여서 ACID 속성을 구성합니다. 

Unlogged 테이블로 삽입한다는 것은 PostgreSQL이 트랜잭션 로그에 쓰지 않고 삽입을 수행한다는 의미이며, 그 자체가 I/O 작업입니다. 따라서 unlogged 테이블은 일반 테이블보다 훨씬 빠릅니다.

Unlogged 테이블을 만들려면 다음 옵션을 사용합니다.
- `CREATE UNLOGGED TABLE <tableName>` 구문을 사용하여 unlogged 테이블을 새로 만듭니다.
- `ALTER TABLE <tableName> SET UNLOGGED` 구문을 사용하여 기존의 logged 테이블을 unlogged 테이블로 변환합니다.  
