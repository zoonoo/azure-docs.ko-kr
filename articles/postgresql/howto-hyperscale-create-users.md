...
* 모든 pg\_stat\_\* 뷰를 읽고, 일반적으로 superusers에만 표시 되는 다양 한 통계 관련 확장 (뷰 또는 확장)을 사용 합니다.
* 테이블에 대 한 액세스 공유 잠금을 사용할 수 있는 모니터링 함수를 실행 합니다.
* [PostgreSQL 확장을 만듭니다](concepts-hyperscale-extensions.md) (역할은 `azure_pg_admin`의 멤버 이기 때문에).
...
앞서 설명한 것 처럼 `citus` 관리자 계정에는 추가 사용자를 만들 수 있는 권한이 없습니다. 사용자를 추가 하려면 Azure Portal 인터페이스를 사용 합니다.
...
예를 들어 `db_user` `mytable`를 읽도록 허용 하려면 사용 권한을 부여 합니다.
