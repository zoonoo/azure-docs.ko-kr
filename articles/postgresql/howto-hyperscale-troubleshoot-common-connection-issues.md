...
일반적으로 Hyperscale (Citus)에 대 한 연결 문제는 다음과 같이 분류할 수 있습니다.
...
Citus (Hyperscale) 서버 그룹 노드에 대해 고가용성을 사용 하도록 설정 하면 이러한 유형의 문제를 자동으로 완화할 수 있습니다. 그러나 응용 프로그램은 계속 해 서 연결을 손실할 준비가 되어 있습니다. 또한 큰 트랜잭션이 장기 실행 복구를 야기 하는 경우와 같이 다른 이벤트를 완화 하는 데 더 많은 시간이 걸릴 수 있습니다.
...
1. 클라이언트 IP 주소를 허용 하도록 [방화벽 규칙](howto-hyperscale-manage-firewall-using-portal.md) 을 설정 합니다. 임시 테스트 용도로만 목적으로만 0.0.0.0을 시작 IP 주소로 사용하고 255.255.255.255를 끝 IP 주소로 사용하여 방화벽 규칙을 설정합니다. 해당 규칙은 모든 IP 주소에 대한 서버를 엽니다. 규칙에서 연결 문제를 해결 하는 경우 해당 규칙을 제거 하 고 적절 하 게 제한 된 IP 주소 또는 주소 범위에 대 한 방화벽 규칙을 만듭니다.
