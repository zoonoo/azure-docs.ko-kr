...
description: 고객 관리 키를 사용 하 여 단일 서버 데이터 암호화를 Azure Database for PostgreSQL 하 여 미사용 데이터 보호를 위해 Bring Your Own Key (BYOK) 할 수 있으며, 조직에서 키 및 데이터 관리의 의무 분리를 구현할 수 있습니다.
author: kummanish
ms.author: manishku
...
Key Vault에 저장 된 고객의 관리 키를 사용 하 여 Azure Database for PostgreSQL 단일 서버를 암호화 하 고 나면 새로 만든 서버 복사본 (로컬 또는 지역 복원 작업 또는 읽기 복제본을 통해)도 동일한를 사용 하 여 암호화 됩니다. 고객의 관리 되는 키입니다. 그러나 암호화를 위해 새로운 고객의 관리 키를 반영 하도록 변경할 수 있습니다. 고객 관리 키가 변경 되 면 서버의 이전 백업에서 최신 키를 사용 하기 시작 합니다.
...
[Azure Portal를 사용 하 여 Azure database For PostgreSQL 단일 서버에 대해 고객 관리 키를 사용 하 여 데이터 암호화를 설정](howto-data-encryption-portal.md)하는 방법에 대해 알아봅니다.
