...
2. **데이터베이스**  >  **Azure Database for PostgreSQL**를 선택 합니다. 검색 상자에 PostgreSQL를 입력 하 여 서비스를 찾을 수도 있습니다. **단일 서버** 배포 옵션을 사용 하도록 설정 했습니다.
...
3. 서버에 대 한 기본 정보를 제공 합니다. **추가 설정을** 선택 하 고 **인프라 이중 암호화** 확인란을 설정 하 여 매개 변수를 설정 합니다.
    ...
    ![Azure Database for PostgreSQL 선택](./media/howto-infrastructure-double-encryption/infrastructure-encryption-selected.png)

4. **검토 + 만들기** 를 선택 하 여 서버를 프로 비전 합니다.

    ![Azure Database for PostgreSQL 요약](./media/howto-infrastructure-double-encryption/infrastructure-encryption-summary.png)

5. 서버를 만든 후 **데이터 암호화** 서버 블레이드의 상태를 확인 하 여 인프라의 이중 암호화의 유효성을 검사할 수 있습니다.

    ![Azure Database for MySQL 유효성 검사](./media/howto-infrastructure-double-encryption/infrastructure-encryption-validation.png)
