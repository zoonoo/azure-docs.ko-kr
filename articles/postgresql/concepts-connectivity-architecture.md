...
* 애플리케이션의 연결 문자열에 게이트웨이 IP 주소를 하드 코드하는 경우. 이는 **권장되지 않습니다**. 애플리케이션의 연결 문자열에서 서버의 FQDN(정규화된 도메인 이름)을 <servername>.postgres.database.azure.com 형식으로 사용해야 합니다. 
