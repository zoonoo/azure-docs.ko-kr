...
description: Azure PowerShell을 사용하여 Azure Database for PostgreSQL에서 서버를 백업 및 복원하는 방법을 알아봅니다.
author: sr-msft
ms.author: srranga
...
`New-AzPostgreSqlServer` 명령을 통해 서버를 만드는 동안 **GeoRedundantBackup** 매개 변수는 백업 중복성 옵션을 결정합니다. 이 매개 변수가 **사용** 으로 설정되어 있으면 지역 중복 백업이 수행됩니다. 또는 **사용 안 함** 으로 설정되어 있으면 로컬 중복 백업이 수행됩니다.

백업 보존 기간은 **BackupRetentionDay** 매개 변수로 설정됩니다.

서버를 만드는 동안 이러한 값을 설정하는 방법에 대한 자세한 내용은 [PowerShell을 사용하여 Azure Database for PostgreSQL 서버 만들기](quickstart-create-postgresql-server-database-using-azure-powershell.md)를 참조하세요.
...
`Restore-AzPostgreSqlServer` cmdlet의 **PointInTimeRestore** 매개 변수 집합에는 다음 매개 변수가 필요합니다.
...
지역 중복 백업을 사용하여 서버를 만들려면 **UseGeoRestore** 매개 변수와 함께 `Restore-AzPostgreSqlServer` 명령을 사용합니다.
...
`Restore-AzPostgreSqlServer` cmdlet의 **GeoRestore** 매개 변수 집합에는 다음 매개 변수가 필요합니다.
...
> [PowerShell을 사용하여 Azure Database for PostgreSQL 연결 문자열을 생성하는 방법](howto-connection-string-powershell.md)