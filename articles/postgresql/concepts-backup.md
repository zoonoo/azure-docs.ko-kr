...
예상 복구 시간은 데이터베이스 크기, 트랜잭션 로그 크기, 네트워크 대역폭 및 동일한 지역에서 동시에 복구되는 데이터베이스의 총 수를 포함한 여러 요소에 따라 달라집니다. 복구 시간은 마지막 데이터 백업 및 수행 해야 하는 복구의 양에 따라 달라 집니다. 일반적으로 12 시간 미만입니다.
