---
title: Azure Database for PostgreSQL - 단일 서버의 Azure 보안 기준
description: Azure Database for PostgreSQL - 단일 서버 보안 기준은 Azure Security Benchmark에 지정된 보안 권장 사항을 구현하기 위한 절차 지침과 리소스를 제공합니다.
author: msmbaldwin
...
ms.locfileid: "107284332"
---
# <a name="azure-security-baseline-for-azure-database-for-postgresql---single-server"></a>Azure Database for PostgreSQL - 단일 서버의 Azure 보안 기준

이 보안 기준에서는 [Azure Security Benchmark 버전 1.0](../security/benchmarks/overview-v1.md)의 지침을 Azure Database for PostgreSQL - Single Server에 적용합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다. 콘텐츠는 Azure Security Benchmark에서 정의한 **보안 컨트롤** 과 Azure Database for PostgreSQL - Single Server에 적용되는 관련 지침에 따라 그룹화됩니다. 

> [!NOTE]
> Azure Database for PostgreSQL - Single Server에 적용되지 않거나 Microsoft의 책임인 **컨트롤** 은 제외되었습니다. Azure Database for PostgreSQL - Single Server가 Azure 보안 벤치마크에 완전히 매핑되는 방식을 확인하려면 **[전체 Azure Database for PostgreSQL - Single Server 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/azure-database-for-postgresql-single-server-security-baseline-v1.1.xlsx)** 을 참조하세요.
...
- [Azure Database for PostgreSQL에 대한 Private Link를 구성하는 방법](howto-configure-privatelink-portal.md)

- [Azure Database for PostgreSQL에서 VNet 서비스 엔드포인트 및 VNet 규칙을 만들고 관리하는 방법](howto-manage-vnet-using-portal.md)

- [Azure Database for PostgreSQL 방화벽 규칙을 구성하는 방법](howto-manage-firewall-using-portal.md)
...
**Azure Policy 기본 제공 정의 - Microsoft.DBforPostgreSQL**:
...
- [Azure Database for PostgreSQL용 Advanced Threat Protection을 구성하는 방법](howto-database-threat-protection-portal.md)
...
- [Azure Database for PostgreSQL의 서비스 태그 사용 이해](https://docs.microsoft.com/azure/postgresql/concepts-data-access-and-security-vnet#terminology-and-description)
...
- [네트워킹을 위한 Azure Policy 샘플](/azure/governance/policy/samples)
...
- [Azure Database for PostgreSQL에 대한 서버 로그를 구성하고 액세스하는 방법](howto-configure-server-logs-in-portal.md)

- [Azure Database for PostgreSQL에 대한 감사 로그를 구성하고 액세스하는 방법](concepts-audit.md)
...
- [Log Analytics 작업 영역에 대한 로그 보존 매개 변수를 설정하는 방법](https://docs.microsoft.com/azure/azure-monitor/logs/manage-cost-storage#change-the-data-retention-period)

- [Azure Storage 계정에 리소스 로그 저장](https://docs.microsoft.com/azure/azure-monitor/essentials/resource-logs#send-to-azure-storage)
...
- [Azure Database for PostgreSQL용 Advanced Threat Protection을 사용하도록 설정하는 방법](howto-database-threat-protection-portal.md)
...
- [Azure Database for PostgreSQL에 대한 액세스 관리 이해](https://docs.microsoft.com/azure/postgresql/concepts-security#access-management)
...
**지침**: Azure AD(Azure Active Directory) 및 Azure Database for PostgreSQL에는 기본 암호에 대한 개념이 없습니다.
...
- [Azure Database for PostgreSQL에 대한 추가 계정을 만드는 방법](howto-create-users.md)
...
**지침**: 데이터베이스에 직접 구성된 사용자 이름/암호를 사용하거나 Azure AD(Azure Active Directory) ID를 사용하고 연결할 Azure AD 토큰을 활용하여 Azure Database for PostgreSQL에 로그인할 수 있습니다. Azure AD 토큰을 사용하는 경우 Azure AD 사용자, Azure AD 그룹 또는 데이터베이스에 연결하는 Azure AD 애플리케이션과 같은 다양한 방법이 지원됩니다.

이와는 달리 PostgreSQL에 대한 컨트롤 플레인 액세스는 REST API를 통해 사용할 수 있으며 SSO를 지원합니다. 인증을 위해 요청에 대한 권한 부여 헤더를 Azure AD에서 가져오는 JSON Web Token으로 설정합니다.

- [Azure AD를 사용하여 Azure Database for PostgreSQL 인증](howto-configure-sign-in-aad-authentication.md)
...
**지침**: Azure AD(Azure Active Directory) 다단계 인증을 사용하도록 설정하고, Azure Security Center ID 및 액세스 관리 추천 사항을 따릅니다. Azure AD 토큰을 활용하여 데이터베이스에 로그인하는 경우 데이터베이스 로그인에 대한 다단계 인증을 요구할 수 있습니다.
...
**지침**: Azure 리소스에 로그인하고 구성하도록 구성된 다단계 인증을 통해 PAW(Privileged Access Workstation)를 사용합니다. 
...
또한 환경에서 의심스럽거나 안전하지 않은 활동이 발생하는 경우 로그와 경고를 생성하려면 Azure AD(Azure Active Directory) PIM(Privileged Identity Management)을 사용할 수 있습니다.

Azure AD 위험 검색을 사용하여 위험한 사용자 동작에 대한 경고 및 보고서를 봅니다.

- [Azure Database for PostgreSQL용 Advanced Threat Protection을 설정하는 방법](howto-database-threat-protection-portal.md)
...
**지침**: Azure AD(Azure Active Directory) 로그를 검토하여 Azure Database for PostgreSQL 관리 역할이 있는 계정을 포함하는 부실 계정을 검색할 수 있습니다. 또한 Azure ID 액세스 검토를 사용하여 그룹 멤버 자격, Azure Database for PostgreSQL에 액세스하는 데 사용할 수 있는 엔터프라이즈 애플리케이션에 대한 액세스 및 역할 할당을 효율적으로 관리할 수 있습니다. 적절한 사용자만 계속 액세스할 수 있도록 사용자 액세스를 90일마다 정기적으로 검토해야 합니다.
...
**지침**: Azure Database for PostgreSQL 및 Azure AD(Azure Active Directory)에 대한 진단 설정을 사용하도록 설정하여 모든 로그를 Log Analytics 작업 영역으로 보냅니다. Log Analytics 내에서 원하는 경고(예: 인증 시도 실패)를 구성합니다.
...
- [Azure Database for PostgreSQL 방화벽 규칙을 구성하는 방법](concepts-firewall-rules.md)
...
- [Azure Database for PostgreSQL 데이터의 반출을 완화하는 방법](concepts-data-access-and-security-private-link.md)
...
- [Azure Database for PostgreSQL에 대한 전송 중 암호화를 구성하는 방법](concepts-ssl-connection-security.md)
...
**지침**: Azure RBAC(Azure 역할 기반 액세스 제어)를 사용하여 Azure Database for PostgreSQL 컨트롤 플레인(예: Azure Portal)에 대한 액세스를 제어합니다. 데이터 평면 액세스(데이터베이스 자체 내)의 경우 SQL 쿼리를 사용하여 사용자를 만들고 사용자 권한을 구성합니다. Azure RBAC는 데이터베이스 내의 사용자 권한에 영향을 주지 않습니다.

- [Azure RBAC를 구성하는 방법](../role-based-access-control/role-assignments-portal.md)

- [SQL을 사용하여 Azure Database for PostgreSQL에 대한 사용자 액세스를 구성하는 방법](howto-create-users.md)
...
**지침**: Azure Database for PostgreSQL 및 관련 리소스 보안에 관한 Azure Security Center의 권장 사항에 따릅니다.

Microsoft는 Azure Database for PostgreSQL을 지원하는 기본 시스템에서 취약성 관리를 수행합니다.
...
**지침**: Azure Resource Graph를 사용하여 구독 내의 모든 리소스(Azure Database for PostgreSQL 인스턴스 포함)를 쿼리하고 검색합니다. 테넌트에서 적절한 권한(읽기)이 있는지 확인하고, 모든 Azure 구독 및 구독 내의 리소스를 열거할 수 있습니다.
...
- [Azure Policy를 사용하여 특정 리소스 종류를 거부하는 방법](/azure/governance/policy/samples/built-in-policies#general)
...
**지침**: Azure Database for PostgreSQL 서버는 데이터베이스에 액세스하기 위한 Azure AD(Active Directory) 인증을 지원합니다. Azure Database for PostgreSQL 서버를 만드는 동안 관리자에 대한 자격 증명을 제공합니다. 이 관리자는 추가 데이터베이스 사용자를 만드는 데 사용할 수 있습니다.
...
- [Azure Database for PostgreSQL에서 서버를 백업하는 방법](howto-restore-server-portal.md)
...
- [Azure Database for PostgreSQL의 백업 및 복원 이해](concepts-backup.md)
...
**지침**: 연속 내보내기 기능을 사용하여 Azure Security Center 경고 및 추천 사항을 내보냅니다. 연속 내보내기를 사용하면 경고 및 추천 사항을 수동으로 또는 지속적으로 내보낼 수 있습니다. Azure Security Center 데이터 커넥터를 사용하여 Azure Sentinel로 경고를 스트림할 수 있습니다.
