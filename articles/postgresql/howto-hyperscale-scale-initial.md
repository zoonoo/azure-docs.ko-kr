...
작업자 RAM: 최상의 사례는 대부분의 작업 집합이 메모리에 맞는 충분한 메모리를 제공하는 것입니다. 애플리케이션에서 사용하는 쿼리 유형은 메모리 요구 사항에 영향을 줍니다. 쿼리에 대해 설명 하는 분석을 실행 하 여 필요한 메모리 양을 확인할 수 있습니다. VCores 및 RAM은 [Citus (Hyperscale) 구성 옵션](concepts-hyperscale-configuration-options.md) 문서에 설명 된 대로 함께 확장 됩니다.
