...
3. 액세스할 수 없는 상태를 수정 하려면 복원 된 서버에서 키의 유효성을 다시 검사 해야 합니다. **데이터 암호화** 블레이드를 클릭 한 다음 **키 유효성** 다시 검사 단추를 클릭 합니다.
   ...
   > [!NOTE]
   > 새 서버의 서비스 주체에 게 키 자격 증명 모음에 대 한 액세스 권한을 부여 해야 하기 때문에 첫 번째 유효성 재검사 시도는 실패 합니다. 서비스 사용자를 생성 하려면 유효성 재검사 **키**를 클릭 합니다. 그러면 오류가 발생 하지만 서비스 주체는 생성 됩니다. 그 후에는 위의 [섹션 2에 있는](https://docs.microsoft.com/azure/postgresql/howto-data-encryption-portal#setting-the-right-permissions-for-key-operations) 단계를 참조 하세요.
