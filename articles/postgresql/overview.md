...
Azure Database for PostgreSQL 서비스는 미사용 데이터에 대해 스토리지 암호화를 사용하고 FIPS 140-2를 준수합니다. 백업을 포함하여 데이터는 디스크에서 암호화됩니다. 이 서비스는 Azure 스토리지 암호화에 포함된 AES 256비트 암호화를 사용하며, 키는 시스템에서 관리됩니다. 스토리지 암호화는 항상 켜져 있고 해제할 수 없습니다. 기본적으로 Azure Database for PostgreSQL 서비스에는 네트워크 간 및 데이터베이스와 클라이언트 애플리케이션 간에 이동하는 데이터에 대한 보안 연결이 필요합니다.
