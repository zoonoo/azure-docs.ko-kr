...
이 문서는 Linux 또는 Windows VM에서 부트스트랩 워크로드용 사용자 지정 스크립트 확장을 사용하여 Azure 리소스 관리자 템플릿을 작성하는 개요를 제공합니다.

사용자 지정 스크립트 확장의 개요는 <a href="https://azure.microsoft.com/ko-kr/documentation/articles/virtual-machines-extensions-customscript/" target="_blank">여기</a>에서 문서를 참조하십시오.

시작 이래 사용자 지정 스크립트 확장은 Windows 및 Linux VM에 워크로드를 구성하는 데 광범위하게 사용되었습니다. Azure 리소스 관리자 템플릿을 도입하면서 이제 사용자가 VM을 프로비전할 뿐만 아니라 작업 부하를 구성하는 단일 템플릿을 만들 수 있습니다.
...
1. <a href="http://azure.microsoft.com/downloads" target="_blank">여기</a>에서 최신 Azure PowerShell Cmdlet 또는 Azure CLI을 설치합니다.
...
## 템플릿으로 사용자 지정 스크립트 확장을 사용하는 개요입니다.
...
템플릿을 배포하는 경우 Azure 서비스 관리 API에 사용할 수 있는 동일한 버전의 사용자 지정 스크립트 확장을 사용합니다. 확장은 Azure 저장소 계정 또는 Github 위치에 파일을 업로드하는 동일한 매개 변수 및 시나리오를 지원합니다. 템플릿을 사용하는 동안 중요한 차이점은 majorversion.* 형식에서 버전을 지정하는 대신 확장의 정확한 버전을 지정해야 한다는 점입니다.
 ...
 ## Linux VM에서 사용자 지정 스크립트 확장에 대한 템플릿 코드 조각
...
## Windows VM에서 사용자 지정 스크립트 확장에 대한 템플릿 코드 조각
...
사용자 지정 스크립트 확장을 사용하여 VM에서 응용 프로그램 구성의 전체 샘플에 대해 아래 예를 참조하십시오.
