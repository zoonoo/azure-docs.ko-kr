...
이 자습서에서는 Azure Active Directory에서 [클라이언트 지향 로그인 작업](http://msdn.microsoft.com/ko-kr/library/azure/jj710106.aspx)을 지원하기 위해 Active Directory 인증 라이브러리를 사용하여 빠른 시작 프로젝트에 인증을 추가합니다. Azure Active Directory에서 [서비스 지향 로그인 작업](http://msdn.microsoft.com/ko-kr/library/azure/dn283952.aspx)을 지원하려면 [모바일 서비스 앱에 인증 추가](/ko-kr/documentation/articles/mobile-services-dotnet-backend-windows-store-dotnet-get-started-users/) 자습서를 시작하세요.
...
>[AZURE.NOTE] 이 자습서는 모바일 서비스에서 [클라이언트 지향 로그인 작업](http://msdn.microsoft.com/ko-kr/library/azure/jj710106.aspx)을 사용하여 Windows 스토어 앱에 대해 Single Sign-On Azure Active Directory 인증을 수행할 수 있도록 하는 방법의 이해를 돕기 위한 것입니다. 모바일 서비스를 처음 사용하는 경우 [모바일 서비스 시작] 자습서를 완료하는 것이 좋습니다.
...
Azure Active Directory에 앱을 등록하려면 Windows 스토어에 앱을 연결하고 앱용 패키지 SID(보안 식별자)를 받아야 합니다. 패키지 SID가 Azure Active Directory의 네이티브 응용 프로그램 설정에 등록됩니다.
...
5. **리디렉션 URI** 상자에서 앞서 복사한 앱 패키지 SID를 붙여넣은 후 클릭하여 네이티브 앱 등록을 완료합니다.
...
6. 위의 `AuthenticateAsync` 메서드 코드에서 **INSERT-AUTHORITY-HERE**를 응용 프로그램을 프로비전한 테넌트의 이름으로 바꿉니다. 형식은 https://login.windows.net/tenant-name.onmicrosoft.com이어야 합니다. 이 값은 [Azure 관리 포털]에서 Azure Active Directory의 도메인 탭에서 복사할 수 있습니다.

7. 위의 `AuthenticateAsync` 메서드 코드에서 **INSERT-RESOURCE-URI-HERE**를 모바일 서비스의 **앱 ID URI**로 바꿉니다. [Azure Active Directory에 등록하는 방법] 항목을 따르면 앱 ID URI가 https://todolist.azure-mobile.net/login/aad와 유사합니다.

8. 위의 `AuthenticateAsync` 메서드 코드에서 **INSERT-CLIENT-ID-HERE**를 네이티브 클라이언트 응용 프로그램에서 복사한 클라이언트 ID로 바꿉니다.

9. Visual Studio 솔루션 탐색기 창의 클라이언트 프로젝트에서 Package.appxmanifest 파일을 엽니다. **기능** 탭을 클릭하고 **엔터프라이즈 응용 프로그램** 및 **개인 네트워크(클라이언트 및 서버)**를 사용하도록 설정합니다. 파일을 저장합니다.
