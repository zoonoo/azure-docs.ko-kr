<properties 
	pageTitle="Application Insights : 자동 관리 이상 진단 | Microsoft Azure" 
...
[Visual Studio Application Insights](app-insights-overview.md)는 앱 원격 분석의 심층 분석을 수행하고 잠재적 성능 문제에 대해 경고할 수 있습니다. 여러분은 아마도 이메일을 통해 자동 경고 중 하나를 수신했기 때문에 이 글을 읽고 있을 것입니다.

이 기능은 설정이 필요하지 않고 앱이 충분한 원격 분석을 생성하는 경우 자동으로 활성화됩니다.


## 자동 관리 이상 검색이 무엇인가요?

자동 관리 이상 검색은 Application Insights에 보내는 원격 분석 데이터를 분석하여 앱에서 성능의 비정상적인 패턴을 검색합니다.
...
여러분의 피드백을 기다리고 있습니다. 자동 관리 검색 기능이 어떻게 도움이 되었고 개선 방안은 무엇이 있는지 또한 추가했으면 하는 기능이 있다면 알려주시기 바랍니다. 포털에서 웃는 얼굴 보내기/찡그린 얼굴 보내기를 통해 피드백을 보내거나 AppInsightsML@microsoft.com으로 메일을 보내시면 됩니다.
 ...
 * 현재는 암호화되지 않습니다. 현재는 요청 응답 시간, 종속성 응답 시간 및 페이지 로드 시간을 분석합니다. 추가적인 메트릭 분석이 곧 제공될 예정입니다.
...
![Azure에서 전자 메일 경고의 이상 보고서를 열려면 링크를 클릭합니다.](./media/app-insights-proactive-anomaly-diagnostics/03.png)
 ...
 * 경고 또는 이메일의 구독 취소 링크를 클릭합니다.
    ...
    또한 자동 관리 감지 블레이드에서 받는 사람 목록 설정을 편집할 수도 있습니다.
* *이 메시지를 너무 많이 받고 싶지 않습니다.*
 * 아직 보고하지 않은 문제 중에서 가장 높은 관련성을 가진 문제로, 하루 한 번으로 제한됩니다. 모든 메시지는 중복으로 보내지 않습니다.
* *아무일도 하지 않는 경우 미리 알림을 받습니까?*
 * 아니요, 각 문제에 대해 한 번만 메시지가 나타납니다.
* *이메일이 삭제되었습니다. 포털의 어디에서 알림을 찾을 수 있습니까?*
 * 앱의 Application Insights 개요에서 **자동 관리 감지** 타일을 클릭합니다. 여기에서 7일 내의 모든 알림을 찾을 수 있습니다.
