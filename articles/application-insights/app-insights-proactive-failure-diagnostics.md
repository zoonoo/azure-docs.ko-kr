...
이 기능은 자체 서버 또는 클라우드에서 호스팅되는 Java 및 ASP.NET 웹 앱에 대해 작동 합니다. 요청 원격 분석 또는 종속성 원격 분석을 생성하는 모든 앱에 대해 작동합니다(예를 들어 [TrackRequest()](app-insights-api-custom-events-metrics.md#track-request) 또는 [TrackDependency()](app-insights-api-custom-events-metrics.md#track-dependency)를 호출하는 작업자 역할이 있는 경우). 
