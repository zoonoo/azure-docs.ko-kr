...
자동 관리 감지는 기계 학습 및 데이터 마이닝 알고리즘을 사용하여 응용 프로그램 성능에 영향을 미치는 비정상적인 패턴을 감지합니다. 자동 관리 감지는 Application Insights를 통해 수집된 성능 원격 분석 데이터를 자동으로 분석합니다. 자동 관리 감지는 응용 프로그램의 비정상적인 성능에 대해 전자 메일을 보내줍니다. 사용자가 임계값 규칙을 설정할 필요가 없습니다. 자동 관리 감지 알림은 Application Insights 분석 기능과 통합되어 문제에 대한 신속한 심사 및 분석이 가능합니다.
