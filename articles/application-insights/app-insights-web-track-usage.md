...
웹 응용 프로그램의 사용량 데이터는 클라이언트 브라우저에서 제공됩니다.

#### Application Insights 리소스 설치 
...
[Azure 포털][portal]의 사용자 리소스에서 빠른 시작을 열고 웹 페이지를 모니터링하는 코드 조각을 가져옵니다.

![](./media/app-insights-web-track-usage/02-monitor-web-page.png)

코드를 (.NET에서) \_Layout.cshtml 같은 마스터 페이지 또는 포함 파일에 넣어서 사용자의 모든 페이지에 포함되도록 합니다.

코드 조각에 리소스를 식별하는 계측 키(iKey)가 들어 있습니다. 예를 들어 테스트하는 동안 데이터를 다른 리소스로 보내려면 iKey만 교체하면 됩니다.

사용자의 웹 페이지를 게시하거나 디버그 모드에서 해당 페이지를 사용하여 일부 원격 분석 데이터를 생성합니다.
