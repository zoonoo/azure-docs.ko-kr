...
# Application Insights 포털에서 대시보드 및 탐색
...
[Azure 포털](https://portal.azure.com)에 로그인하면 먼저 대시보드가 나타납니다. 이를 사용자 지정하거나 전체 화면 모드로 전환할 수 있습니다.

대시보드는 호스트된 위치에 관계없이 여러 응용 프로그램을 구성하는 솔루션을 모니터링하는 데 특히 유용합니다. 스트림 분석 등의 Azure 구성 요소가 시스템에 설치된 경우 대시보드에 모니터링 차트를 추가할 수도 있습니다.
...
4. 대시보드 도구 모음을 사용하여 대시보드를 편집하고 만듭니다. 각 응용 프로그램마다 [다른 대시보드를 만들](#dashboards) 수 있습니다.
...
시스템이 웹앱, 백 엔드 서버 및 일부 Azure 서비스 등 여러 구성 요소로 구성된 경우 모든 구성 요소의 핵심 성과 지표를 한 화면에 표시할 수 있습니다.

응용 프로그램이 둘 이상인 경우 여러 대시보드를 만들어 서로 전환할 수 있습니다.

![편집을 클릭합니다. 타일 및 차트를 끕니다. 갤러리에서 타일을 추가합니다. 완료를 클릭합니다.](./media/app-insights-dashboards/30.png)
...
![개요 블레이드에서 메트릭 클릭](./media/app-insights-dashboards/16-metrics.png)

### 차트 및 표 편집

블레이드에 새 차트를 추가하려면:

![메트릭 탐색기에서 추가 차트 선택](./media/app-insights-dashboards/04-add.png)

기존 또는 새 차트를 선택하여 보이는 내용 편집:

![하나 이상의 메트릭 선택](./media/app-insights-dashboards/08-select.png)
...
![그룹화를 선택한 다음 그룹별에서 속성 선택](./media/app-insights-dashboards/15-segment.png)
...
![슬라이더 조정](./media/app-insights-dashboards/18-height.png)

### 데이터 필터링

속성 값의 선택한 집합에 대한 메트릭 보기:

![필터를 클릭하고 속성을 확장하여 값 선택](./media/app-insights-dashboards/19-filter.png)
...
![즐겨찾기 선택](./media/app-insights-dashboards/21-favorite-save.png)

블레이드를 다시 보려면 **개요 블레이드로 이동**하여 즐겨찾기를 엽니다.

![개요 블레이드에서 즐겨찾기 선택](./media/app-insights-dashboards/22-favorite-get.png)
...
![메트릭 탐색기 위쪽에 있는 단추](./media/app-insights-dashboards/17-reset.png)
...
![Azure 포털에서 응용 프로그램의 개요 블레이드 열기](./media/app-insights-dashboards/03-range.png)
...
![차트의 일부를 끕니다.](./media/app-insights-dashboards/12-drag.png)
...
![차트 위로 마우스 이동](./media/app-insights-dashboards/02-focus.png)

특정 지점에서 메트릭 값은 이전 샘플링 간격에 걸쳐 집계됩니다.

샘플링 간격 또는 "세분성"는 블레이드 위쪽에 표시됩니다.

![블레이드의 헤더입니다.](./media/app-insights-dashboards/11-grain.png)

시간 범위 블레이드에서 세분성을 조정할 수 있습니다.

![블레이드의 헤더입니다.](./media/app-insights-dashboards/grain.png)
...
![진단 검색 열기](./media/app-insights-dashboards/01-open-Diagnostic.png)

필터 블레이드를 열고 확인하려는 이벤트 유형을 선택합니다. (나중에 열었던 블레이드로 필터를 복원하려는 경우 재설정을 클릭합니다.)

![필터 선택 및 원격 분석 유형 선택](./media/app-insights-dashboards/02-filter-req.png)
...
![속성 확장 및 값 선택](./media/app-insights-dashboards/03-response500.png)
...
![속성 확장 및 값 선택](./media/app-insights-dashboards/04-failingReq.png)
...
![즐겨찾기 클릭, 이름 설정 및 저장 클릭](./media/app-insights-dashboards/08-favorite-save.png)


검색을 다시 보려면 **개요 블레이드로 이동**하여 즐겨찾기를 엽니다.

![즐겨찾기 타일](./media/app-insights-dashboards/22-favorite-get.png)

상대 시간 범위로 저장한 경우 다시 열린 블레이드는 최신 데이터입니다. 절대 시간 범위로 저장한 경우 매번 같은 데이터가 보입니다.

## 분석

[분석](app-insights-analytics.md)은 앱에 대한 성능 질문을 진단 및 이해할 수 있는 고급 검색 기능입니다.

![분석 예제](./media/app-insights-dashboards/025.png)
