...
![웹 사이트의 최소 URL 채우기](./media/app-insights-monitor-web-app-availability/001-create-test.png)
...
* **경고 위치 임계값**: 최소한 3/5 위치를 권장합니다. 경고 위치 임계값과 테스트 위치 수 사이의 최적 관계는 **경고 위치 임계값** = **테스트 위치 수** - 2이고, 최소 테스트 위치 수는 5입니다.
...
[여기에서](app-insights-transaction-diagnostics.md) 종단 간 트랜잭션 진단 환경에 대해 자세히 알아봅니다.

가상 가용성 테스트를 실패하게 만든 서버 쪽 예외 세부 정보를 확인하려면 예외 행을 클릭합니다. 다양한 코드 수준 진단에 대한 [디버그 스냅숏](app-insights-snapshot-debugger.md)을 가져올 수도 있습니다.

![서버 쪽 진단](./media/app-insights-monitor-web-app-availability/open-instance-4.png)
...
![환경 만들기](./media/app-insights-monitor-web-app-availability/appinsights-71webtestUpload.png)

**중요**: [새로 통합된 경고](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-unified-alerts)를 사용하여 경고 규칙 심각도 및 [작업 그룹](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-action-groups)이 포함된 알림 기본 설정은 경고 환경에서 구성**되어야 합니다**. 다음 단계 없이도 포털 내 알림을 받게 됩니다. 

1. 가용성 테스트를 저장한 후 새 테스트 이름을 클릭하여 해당 세부 정보로 이동합니다. "경고 편집"인 ![저장 후 편집](./media/app-insights-monitor-web-app-availability/editaftersave.png) 클릭

2. 원하는 심각도 수준, 규칙 설명 및 가장 중요한 것으로 이 경고 규칙에 대해 사용하려는 알림 기본 설정을 보유한 작업 그룹을 설정합니다.
![저장 후 편집](./media/app-insights-monitor-web-app-availability/setactiongroup.png)
...
1. 메트릭 환경에서 Application Insights 리소스를 선택하고, 가용성 메트릭인 ![가용성 메트릭 선택](./media/app-insights-monitor-web-app-availability/selectmetric.png)을 선택합니다.

2. 메뉴에서 경고 구성 옵션을 사용하면 경고 규칙을 설정할 특정 테스트 또는 위치를 선택할 수 있는 새 환경으로 이동합니다. 또한 여기에서 이 경고 규칙에 대한 작업 그룹을 구성할 수도 있습니다.
    ![가용성 경고 구성](./media/app-insights-monitor-web-app-availability/availabilitymetricalert.png)
...
개요 블레이드에서 **설정**, **성능 테스트**를 엽니다. 테스트를 만들 때 Azure DevOps 계정에 연결하거나 만들어야 합니다.
...
* *webhook 알림을 받지 못했습니다.*
    ...
    webhook 알림을 수신하는 응용 프로그램이 사용 가능하며 webhook 요청을 성공적으로 처리하는지 확인합니다. 자세한 내용은 [이것](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitor-alerts-unified-log-webhook)을 참조하세요.
...
* *테스트 실패를 진단할만한 관련 서버 쪽 원격 분석 데이터가 표시되지 않습니다.*
    ...
    서버 쪽 응용 프로그램에 대해 Application Insights를 설정한 경우, [샘플링](app-insights-sampling.md)이 작동 중이기 때문일 수 있습니다. 다른 가용성 결과를 선택합니다.
