...
배포 시 처음 몇 분간은 앱이 제대로 작동하는지를 알려줍니다. 특히, 이전 버전을 대체하는 경우 성능이 향상되었는지 여부를 알고 싶어 합니다. 문제가 있는 경우에는 이전 버전으로 되돌리는 것이 좋습니다.

라이브 스트림은 주요 성능 메트릭에 대한 즉시 보기를 제공합니다. 다시 배포하거나 구성하는 동안 볼 수 있도록 설계되었습니다.

![개요 블레이드에서 라이브 스트림 클릭](./media/app-insights-asp-net/45.png)

다른 메트릭 차트와 달리, 라이브 스트림은 지난 몇 분간의 데이터만 보여주고 어떠한 데이터도 보존하지 않습니다. 집계 파이프라인은 최소한이며 디스플레이는 매초마다 새로 고쳐집니다.

라이브 스트림에는 SDK 2.1.0-beta1 이상의 버전이 필요합니다.

*라이브 스트림이 구성 페이지에 멈춰있나요? – 브라우저를 새로 고치세요(F5).*
