<properties 
	pageTitle="Application Insights에서 사전 진단 | Microsoft Azure" 
...
#  Application Insights에서 사전 진단

 사전 진단은 웹 응용 프로그램에서 잠재적인 성능 문제를 자동으로 경고해 줍니다. 앱에서 [Visual Studio Application Insights](app-insights-overview.md)로 보내는 원격 분석의 스마트 분석을 수행합니다. 실패율이나 클라이언트 또는 서버 성능의 비정상적인 패턴이 갑자기 증가하는 경우 경고가 발생합니다. 이 기능에는 구성이 필요하지 않습니다. 응용 프로그램에서 충분한 원격 분석을 보내는 경우 작동합니다.
...
* [거의 실시간 실패 경고](app-insights-nrt-proactive-diagnostics.md). 기계 학습을 사용하여 앱에 대해 실패한 요청의 예상 비율을 설정하고 로드 및 다른 요소와 상관 관계를 지정합니다. 실패율이 예상된 범위를 벗어나는 경우 경고를 보냅니다.
* [비정상적인 동작](app-insights-proactive-detection.md). 응답 시간 및 실패율에서 비정상적인 패턴을 매일 검색합니다. 이러한 문제를 위치, 브라우저, 클라이언트 OS, 서버 인스턴스 및 하루 중 시간과 같은 속성과 상관 관계를 지정합니다.
...
사전 진단은 완전히 자동으로 수행됩니다. 하지만 보다 많은 경고를 설정하고 싶을 수 있습니다.
