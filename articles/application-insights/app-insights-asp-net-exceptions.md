...
* [TrackEvent()](app-insights-api-custom-events-metrics.md#track-event) 는 일반적으로 사용 패턴 모니터링을 위해 사용되지만 진단 검색의 사용자 지정 이벤트에서도 전송하는 데이터를 표시합니다. 이벤트의 이름을 지정하고, [진단 검색을 필터링](app-insights-diagnostic-search.md)할 수 있는 문자열 속성 및 숫자 메트릭 수를 수행할 수있습니다. 
* [TrackTrace()](app-insights-api-custom-events-metrics.md#track-trace) 를 사용하여 POST 정보와 같은 긴데이터를 보낼 수 있습니다.
...
* 응용 프로그램에 코드를 삽입하여 [Microsoft.ApplicationInsights.TrackTrace()](app-insights-api-custom-events-metrics.md#track-trace)를 호출합니다. 메시지 매개 변수의 POST 데이터를 보냅니다. 허용되는 크기에 제한이 있으므로 꼭 필요한 데이터만 보내야 합니다.
