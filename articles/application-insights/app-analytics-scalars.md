...
스칼라 식은 결과가 테이블인 [쿼리](app-analytics-queries.md)와 구별됩니다.
...
* *ifTrue:* *조건자*가 `true`으로 계산되는 경우 계산이 이루어지는 식 및 함수에서 반환되는 식의 값.
* *ifFalse:* *조건자*가 `false`으로 계산되는 경우 계산이 이루어지는 식 및 함수에서 반환되는 식의 값.
...
값을 지정된 bin 크기의 아래쪽 정수 배로 반올림합니다. [`summarize by`](app-analytics-queries.md#summarize-operator) 쿼리에 많이 사용합니다. 분산된 값 집합이 있는 경우 특정 값의 더 작은 집합으로 그룹화됩니다.
...
* `sqrt(x) * sqrt(x) == x`이 다음과 같이 되도록 하는 양수
...
현재 UTC 시계 시간에서 지정된 시간 범위를 뺍니다. `now()`과 마찬가지로, 이 함수를 문에 여러 번 사용할 수 있으며 참조하는 UTC 시계 시간은 모든 인스턴스에 대해 동일합니다.
...
텍스트 문자열에서 [정규식](app-analytics-reference.md#regular-expressions)에 대한 일치 항목을 가져옵니다. 선택적으로 그런 다음 추출된 부분 문자열을 지시된 유형으로 변환합니다.
...
* *regex:* [정규식](app-analytics-reference.md#regular-expressions)입니다.
...
[let 절](app-analytics-queries.md#let-clause)은 동적 값을 문자열로 저장하므로 이 두 절은 동일하며 둘 다 사용하기 전에 `parsejson`(또는 `todynamic`)가 필요함;
...
다음 예제에서 `context_custom_metrics`이 `string`인 경우 다음과 유사 합니다.
