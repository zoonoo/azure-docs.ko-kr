...
* 응용 프로그램이 다양한 원격 분석을 보내는 경우 샘플링은 자동으로 발생합니다. 자동 샘플링은 높은 볼륨으로 ASP.NET 웹 서버 앱에서만 시작됩니다.
* 샘플링을 가격 책정 페이지의 포털 또는 .config 파일의 ASP.NET SDK에서 수동으로 설정하여 네트워크 트래픽을 줄일 수도 있습니다(유지된 원격 분석의 볼륨을 줄이고 월간 할당량 내에 유지함).
* 현재 샘플링 속도는 각 레코드의 속성입니다. 검색 창에서 요청과 같은 이벤트를 엽니다. 전체 속성 줄임표 "..."를 확장하여 예를 들어, 원격 분석의 종류에 따라 "요청 수" 또는 "이벤트 수"라는 이름의 "* count" 속성을 찾습니다. > 1이면 샘플링이 발생합니다. 3개라는 수는 샘플링이 33%임을 의미합니다. 유지된 레코드는 각각 원래 생성된 3개의 레코드를 나타냅니다.
...
* **적응 샘플링**은 ASP.NET 앱의 SDK에서 전송되는 원격 분석의 양을 자동으로 조정합니다. 기본 버전은 SDK v 2.0.0-beta3입니다.
* **고정 비율 샘플링**은 ASP.NET 서버와 사용자 브라우저에서 전송되는 원격 분석의 양을 줄입니다. 비율은 사용자가 설정합니다.
* **수집 샘플링**은 사용자가 설정한 비율로 Application Insights 서비스에서 보존되는 원격 분석의 양을 줄입니다. 원격 분석 트래픽을 줄이지는 않지만 월간 할당량 내로 유지하는 데 도움이 됩니다.
...
클라이언트 측(JavaScript) SDK는 서버 측 SDK와 고정 비율 샘플링에 참여합니다. 계측된 페이지는 서버 측이 "샘플"로 결정한 동일한 사용자로부터 클라이언트 측 원격 분석만을 보냅니다. 이 논리는 전체 클라이언트 및 서버 측에 대한 사용자 세션의 무결성을 유지하도록 설계되었습니다. 결과적으로 Application Insights의 특정 원격 분석 항목에서 해당 사용자 또는 세션에 대한 다른 모든 원격 분석 항목을 찾을 수 있습니다.
