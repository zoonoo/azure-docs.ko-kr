...
* **적응 샘플링**은 특정 요청 볼륨을 달성하기 위해 샘플링 비율을 자동으로 조정합니다. 적응 샘플링은 ASP.NET, 버전 2.0.0-beta3 이상인 경우 Application Insights SDK에서 기본적으로 사용하도록 설정되어 있습니다. 
* SDK 버전 2 이상에서 **고정 비율 샘플링**을 사용할 수도 있습니다. 사용자가 샘플링 비율을 지정합니다. 이 샘플링은 JavaScript 클라이언트와 ASP.NET 서버 모두에서 작동합니다. 클라이언트와 서버는 샘플링을 동기화하므로 검색에서 관련된 페이지 보기 및 요청 사이를 이동할 수 있습니다.
* **수집 샘플링**입니다. 이 샘플링은 Application Insights 서비스에서 발생합니다. SDK는 브라우저와 서버에서 모든 원격 분석을 보내지만 비율만 보존합니다. 앱이 원격 분석의 최소 볼륨보다 더 많이 보내는 경우 및 SDK가 다른 종류의 샘플링 중 하나를 수행하지 않는 경우 작동하는 적응 샘플링입니다. 또한 사용하는 가격 책정 계층에 의해 영향을 받습니다. 트래픽을 줄이지는 않지만 월간 할당량 내로 유지하는 데 도움이 됩니다.
...
ASP.NET SDK 버전 2.0.0-beta3 이상을 사용하는 경우 적응 샘플링이 자동으로 사용되도록 설정됩니다. 사용하는 SDK 버전에 관계없이 SDK에 샘플링이 없는 경우 수집 샘플링(서버에서)이 작동할 수 있습니다.
...
모든 샘플링 형식은 트래픽의 특정 임계값을 넘을 때만 작동합니다. 예를 들어 대부분의 디버깅 세션이나 사용이 적은 사이트에는 영향을 주지 않습니다.
...
그렇지 않은 경우 적응 샘플링을 사용하는 것이 좋습니다. 이 샘플링은 ASP.NET 서버 SDK에서 기본적으로 설정됩니다.

SDK에서 샘플링을 사용하지 않는 경우(예: Java를 사용하는 경우 또는 ApplicationInsights.config에서 샘플링을 사용하지 않도록 설정한 경우) Application Insights 서비스에 데이터가 도착하면 수집 샘플링이 작동할 수 있습니다. 이는 일종의 적응 샘플링입니다.
 ...
 * 예, 적응 샘플링 구성의 코드 메서드를 사용하면 샘플링 비율을 가져오는 콜백을 제공할 수 있습니다. 연속 내보내기를 사용하는 경우 내보낸 데이터 요소에 나열된 샘플링 비율을 볼 수 있습니다.
