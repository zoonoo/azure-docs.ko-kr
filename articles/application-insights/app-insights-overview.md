---
title: "Azure Application Insights를 사용한 응용 프로그램 성능 관리 | Microsoft Docs"
description: "라이브 웹 응용 프로그램의 성능 및 사용 현황을 추적합니다.  문제를 감지, 심사 및 진단합니다."
...
---
# <a name="application-performance-management-with-application-insights"></a>Application Insights를 사용한 응용 프로그램 성능 관리
...
## <a name="how-do-i-use-it"></a>어떻게 사용합니까?
### <a name="monitor"></a>모니터
웹앱에서 Application Insights를 설치하고 가용성 웹 테스트를 설정합니다.

* 종속성, 페이지 로드 및 AJAX 호출의 부하, 응답성 및 성능을 모니터링하기 위한 팀 공간용 대시보드를 설정합니다.
* 가장 느리고 대부분 실패한 요청을 검색합니다.
* 새 릴리스를 배포할 때 Live Stream을 보고 성능 저하를 즉시 인식합니다.

### <a name="diagnose"></a>진단
...
* 사용 현황을 기록하기 위해 코드에 사용자 지정 원격 분석을 작성합니다.
* 각 개발 주기는 원격 분석에서 얻은 구체적인 증거를 기반으로 합니다.
...
> [!NOTE]
> 이 시점에서는 작업을 진행하면서 테스트만 해보려고 할 것입니다. 하지만 Application Insights가 수행할 수 있는 작업을 보려면 계속 읽어보세요.
>
>

## <a name="explore-metrics"></a>메트릭 탐색
...
#### <a name="get-more-detail"></a>자세한 정보 얻기
...
#### <a name="customize-a-chart"></a>차트 사용자 지정
...
## <a name="search-instance-data"></a>인스턴스 데이터 검색
...
자습서 창을 열어 쿼리 예제를 보고 데이터에 대해 실행해보거나 더 자세한 [자습서 연습](app-insights-analytics-tour.md) 내용을 읽어볼 수도 있습니다. Intellisense는 사용할 수 있는 쿼리를 표시합니다. 또한 [전체 언어 참조](app-insights-analytics-reference.md)를 제공합니다.

쿼리는 일반적으로 요청, 예외, 종속성 등의 원격 분석 스트림 이름으로 시작합니다. 왼쪽의 스키마를 열면 사용 가능한 원격 분석 스트림 목록이 표시됩니다. 쿼리는 부울 필터인 `where` 또는 새 속성을 계산하는 `project`와 같은 [쿼리 작업](app-insights-analytics-reference.md#queries-and-operators)의 파이프라인입니다. `summarize`는 인스턴스를 집계하고, 사용자가 정의한 함수별로 그룹화한 다음 그룹화된 데이터에 대해 집계 함수를 적용합니다.
...
## <a name="development-in-visual-studio"></a>Visual Studio에서 개발
...
![프로젝트를 마우스 오른쪽 단추로 클릭하고 Application Insights 및 검색을 선택합니다.](./media/app-insights-overview/34.png)
...
![진단 도구 창을 열고 Application Insights 이벤트를 검사합니다.](./media/app-insights-overview/31.png)
...
![예외 스택 추적](./media/app-insights-overview/17.png)

또한 각 메서드의 코드 렌즈 줄에 지난 24시간 동안 Application Insights에 의해 기록된 예외 수를 표시합니다.

![예외 스택 추적](./media/app-insights-overview/21.png)
...
![추세](./media/app-insights-overview/51.png)

## <a name="releasing-a-new-build"></a>새 빌드 릴리스
...
![개요 블레이드에서 라이브 스트림 클릭](./media/app-insights-overview/live-stream.png)
...
### <a name="proactive-diagnostics"></a>사전 진단
[사전 진단](app-insights-proactive-failure-diagnostics.md)은 자동으로 설정되므로 구성할 필요가 없습니다. 사이트에 충분한 트래픽의 있는 경우, 하루 중에 특정 시간 동안 실패한 요청 수가 이상하게 증가하거나 요청 속도가 증가하는 경우 전자 메일을 받게 됩니다. 이러한 경고에는 진단 정보가 포함됩니다.

샘플 경고는 다음과 같습니다.

![실패에 대한 클러스터 분석을 보여주는 샘플 인텔리전트 경고](./media/app-insights-overview/proactive-alert.png)
...
![메트릭 탐색기에서 경고 규칙, 경고 추가 선택](./media/app-insights-overview/appinsights-413setMetricAlert.png)
...
![웹 테스트의 예](./media/app-insights-overview/appinsights-10webtestresult.png)

## <a name="export-and-api"></a>내보내기 및 API
여러 가지 방법으로 Application Insights 포털에서 원격 분석 데이터를 가져올 수 있습니다.

* [데이터 액세스 REST API](https://dev.applicationinsights.io/)는 Analytics 쿼리 실행을 포함하여 데이터를 검색하고 추출하는 데 사용할 수 있습니다. 
* [Analytics 쿼리를 Power BI](app-insights-export-power-bi.md) 대시보드로 내보내서 자동으로 새로고쳐지는 Power BI 시각화로 결과를 볼 수 있습니다.
* [연속 내보내기](app-insights-export-telemetry.md) 는 표준 보존 기간보다 오랫동안 많은 원격 분석 데이터를 유지하려는 경우에 적합합니다.
* [메트릭](app-insights-metrics-explorer.md#export-to-excel) 테이블, 검색 결과 및 [Analytics](app-insights-analytics.md) 결과를 모두 Excel 스프레드시트로 내보낼 수 있습니다.
