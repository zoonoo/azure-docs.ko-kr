...
더 이상 사용되지 않는 `evaluate basket` 구문을 대체하세요.

**반환**

이벤트의 지정된 비율(기본값 0.05)보다 많이 나타나는 모든 패턴입니다. 각 패턴에 대해 패턴에 설정되지 않은 열(즉, 특정 값에 대한 제한 없음)에는 기본적으로 null 값인 와일드카드 값이 포함됩니다(아래의 인수 섹션에서 수동으로 변경하는 방법 참조).
...
* 임계값: 0.015 < *double* < 1(기본값: 0.05) 
    ...
    예제: `T | evaluate basket(0.02)`
* 가중치 열 *itemCount*
  
    샘플링 및 메트릭 사전 집계를 고려할 때 사용합니다. 각 행은 이 열에 지정된 가중치의 영향을 받습니다. 기본적으로 각 행의 가중치는 '1'입니다. 이 경우 각 행에 이미 포함된 데이터의 버킷팅 또는 집계가 고려됩니다.
  
    예제: `T | evaluate basket('~', itemCount)`
* 최대 크기: 1 < *int*(기본값: 5)
  
    쿼리 런타임의 줄이기 위해 기본적으로 제한되는 바구니당 상관 관계가 없는 차원의 최대 수를 설정합니다.

    예제: `T | evaluate basket('~', '~', 3)`
* 사용자 지정 와일드카드 형식: *형식별 임의 값*
    ...
    예제: `T | evaluate basket_v2('~', '~', '~', '*', int(-1), double(-1), long(0), datetime(1900-1-1))`
...
이 함수는 필터 계수의 동적 배열 및 정적 동적 배열을 포함하는 열을 입력으로 사용하고 열에 해당 필터를 적용합니다. 필터링된 출력을 포함하는 새 동적 배열 열을 출력합니다. 
...
* *rsquare:* r 제곱은 적합 품질의 표준 측정입니다. [0-1] 범위의 숫자입니다. 여기서 1은 가장 적합할 가능성이 높은 경우이고 0은 데이터 순서가 완전히 잘못되었으며 어떤 선에도 맞지 않음을 의미합니다.
* *split_idx:* 점을 2개 세그먼트로 분리하는 인덱스(0부터 시작)
* *variance:* 입력 데이터의 분산입니다.
...
rs(r 제곱), si(분할 인덱스) 및 v(분산)를 반환합니다.
...
이 함수는 필터 계수 a 및 b의 동적 배열 및 정적 동적 배열 2개를 포함하는 열을 입력으로 사용하고 열에 해당 필터를 적용합니다. 필터링된 출력을 포함하는 새 동적 배열 열을 출력합니다. 
...
* 위에서 설명한 대로 결과 기간은 bin의 단위입니다. 예를 들어 원래 계열이 일별 기간을 가지며 시간별 bin으로 집계될 경우 출력의 일별 기간은 24가 됩니다. 데이터가 분 단위로 집계되면 출력은 60*24=1440가 됩니다.
* min\_period는 시계열에서 찾으려는 기간보다 약간 더 낮게 설정하고 max\_period는 약간 더 높게 설정해야 합니다. 예를 들어 시간별로 집계되는 신호가 있으며 일별 및 주별 데이터를 모두 원할 경우(각각 24 및 168일 수 있음) 일별 및 주별 마침표를 찾으려는 경우 min\_period=0.824, *max\_period=1.2*168을 설정할 수 있습니다.
...
이 계열에서 series_periods()를 실행하면 다음과 같은 주별 기간(14개 지점)이 사용됩니다.
...
> ![참고] 큰따옴표(`"`)는 JSON에서 레이블 및 문자열 값을 묶는 데 사용해야 합니다. 그러므로 일반적으로 작은따옴표(`'`)를 사용하여 JSON 인코딩 문자열 리터럴을 인용하는 것이 더 쉽습니다.
> 
...
> ![참고] 큰따옴표 문자는 JSON에서 레이블 및 문자열 값을 묶는 데 사용해야 합니다. 
>
