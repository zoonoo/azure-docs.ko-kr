...
* "메일 소유자..." 확인란을 선택하면 이 리소스에 액세스하는 모든 사용자에게 메일을 통해 경고가 보내집니다.
* "추가 메일"을 지정하면 "메일 소유자" 확인란의 선택 여부와 상관없이 이러한 개인 또는 그룹에게 경고가 보내집니다. 
* 경고에 응답하는 웹앱을 설정한 경우 [webhook 주소](../azure-portal/insights-webhooks-alerts.md)를 설정합니다. 경고가 활성화(즉, 트리거)될 때 및 해결될 때 모두 호출됩니다.
