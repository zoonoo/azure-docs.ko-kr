...
Application Insights가 강력한 Log Analytics 데이터 플랫폼에서 지원되므로 Log Analytics의 모든 기능을 사용하여 데이터를 수집할 수 있습니다. 그런 다음, Log Analytics에서 사용할 수 있는 데이터와 이 사용자 지정 데이터의 상관 관계를 만드는 "조인" 연산자를 사용하여 쿼리를 작성합니다. 

## <a name="ingesting-data"></a>데이터 수집

이 섹션에서는 Log Analytics로 데이터를 가져오는 방법을 검토합니다.

아직 없는 경우 [이러한 지침]( https://docs.microsoft.com/azure/log-analytics/log-analytics-quick-collect-azurevm)에 따라 "작업 영역 만들기" 단계를 포함하여 새 Log Analytics 작업 영역을 프로비전합니다.

Log Analytics로 보낸 데이터를 시작하려면 다음과 같은 여러 옵션이 있습니다.
...
- 동기화 메커니즘의 경우 [데이터 수집기 API](https://docs.microsoft.com/azure/log-analytics/log-analytics-data-collector-api)를 직접 호출하거나 논리 앱 커넥터를 사용할 수 있습니다. 단순히 "Azure Log Analytics"를 찾아서 "데이터 보내기" 옵션을 선택하면 됩니다.
 ...
 ![스크린샷 선택 및 작업](./media/app-insights-custom-data-correlation/01-logic-app-connector.png)  
