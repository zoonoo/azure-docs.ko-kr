...
* 할당량은 *데이터 요소*로 측정됩니다. 단일 데이터 요소는 코드에서 또는 표준 원격 분석 모듈 중 하나가 명시적으로 호출하든지, 추적 방법 중 하나를 호출합니다. 
...
* 데이터 속도가 [적응 샘플링](app-insights-sampling)에서 기본적으로 감소합니다. 이는 응용 프로그램 사용이 늘어나도 원격 분석 비율이 예상만큼 증가하지 않는다는 것입니다.
...
[샘플링](app-insights-sampling) 후 차트에 Application Insights 서비스에 도착하는 데이터 양이 표시됩니다.
...
* 원시 데이터 요소(즉, 진단 검색에서 검사할 수 있는 인스턴스): 7일입니다.
* 집계 데이터(즉, 메트릭 탐색기에 표시되는 개수, 평균 및 기타 통계 데이터)는 30일 동안 1분 단위로 보존되고, 90일 동안 형식에 따라 1시간 또는 1일 단위로 보존됩니다.


## 샘플링

[샘플링](app-insights-sampling.md)은 진단 검색 중에 관련 이벤트를 찾는 기능은 계속 유지하고 올바른 이벤트 개수를 유지하면서 앱이 보존하고 있는 원격 분석의 양을 줄이는 방법입니다. 샘플링은 월간 할당량 내로 유지하는 데 도움이 됩니다.
...
![할당량 및 가격 책정 블레이드에서 샘플 타일을 클릭하고 샘플링 비율을 선택합니다.](./media/app-insights-sampling/04.png)
