...
파이프 문자 `|`이 접두사로 붙는 각 필터는 몇몇 매개 변수를 가진 *연산자*의 인스턴스입니다. 연산자에 대한 입력은 앞에 붙는 파이프라인의 결과인 테이블입니다. 대부분의 경우 모든 매개 변수는 입력 열에 대한 [스칼라 식](app-insights-analytics-scalars.md)입니다. 매개 변수가 입력 열의 이름인 경우도 있고 매개 변수가 두 번째 테이블인 경우도 있습니다. 테이블에 열 및 행이 한 개만 있더라도 쿼리의 결과는 언제나 테이블입니다.
...
* *plain\_query:* let 절이 접두사로 붙지 않는 쿼리
...
대화형으로 작업하는 경우 `Take`를 사용하면 결과의 샘플을 간편하면서도 효율적으로 확인할 수 있습니다. 특정 행을 생성하거나 행을 특정 순서로 생성한다는 보장이 없다는 데 유의하십시오.

`take`를 사용하지 않더라도 클라이언트에 반환되는 행 수는 암시적으로 제한됩니다. 이 제한을 제거하려면 `notruncation` 클라이언트 요청 옵션을 사용합니다.
...
반대 함수를 수행하는 [`summarize makelist`](#summarize-operator)도 참조하세요.
...
* *RowLimit:* 각 원래 행에서 생성되는 최대 행 수입니다. 기본값은 128입니다.
...
* `bagexpansion=array`: 속성 모음이 두 요소로 이루어진 `[`*key*`,`*value*`]` 배열로 확장되므로 키와 값에 동일한 방식으로 액세스할 수 있습니다. 또한 예를 들어 속성 이름에 대해 고유 수 집계 등을 실행할 수도 있습니다. 
...
`with` 절의 요소가 원본 문자열과 일치하는지를 차례로 확인합니다. 각 요소는 소스 텍스트의 청크를 분할합니다. 일반 문자열인 경우 일치한다면 일치하는 커서가 따라서 움직입니다. 형식 이름을 가진 열인 경우 커서는 지정된 형식을 구문 분석하기에 충분할 만큼 따라서 움직입니다. (문자열 일치는 다음 요소에 대한 일치가 발견될 때까지 따라서 움직입니다.) regex인 경우 정규 식을 일치시킵니다(그리고 결과 열이 언제나 문자열 형식을 가짐).
...
* *SimpleMatch:* 텍스트의 다음 부분과 일치 여부를 확인할 문자열입니다.
* *Column:* 일치 항목을 할당할 새 열을 지정합니다.
* *Type:* 원본 문자열의 다음 부분을 구문 분석할 방법을 지정합니다.
* *Regex:* 문자열의 다음 부분과 일치 여부를 확인할 정규식입니다. 
...
`parse` 연산자는 같은 `string` 식에 대해 여러 개의 `extract` 응용 프로그램을 사용하여 테이블에 대해 `extend`를 실행하는 간소화된 방법을 제공합니다. developer trace("`printf`"/"`Console.WriteLine`") 문에 의해 생성된 열과 같이 개별 열로 분할할 여러 값이 포함된 `string` 열이 테이블에 있는 경우 이 방법을 사용하면 가장 유용합니다.

아래 예제에서는 테이블 `StormEvents`의 `EventNarrative` 열에 `{0} at {1} crested at {2} feet around {3} on {4} {5}` 형식의 문자열이 포함되어 있다고 가정합니다. 아래 연산은 `SwathSize` 및 `FellLocation`의 두 열을 사용하여 테이블을 확장합니다.
...
포함, 이름 바꾸기 또는 삭제할 열을 선택하고 새 계산된 열을 삽입합니다. 결과의 열 순서는 인수 순서에 의해 지정됩니다. 인수에 지정된 열만이 결과에 포함되며: 입력의 다른 열은 삭제됩니다. `extend`도 참조하세요.
...
* *T:* 입력 테이블입니다.
* *ColumnName:* 출력에 표시할 열 이름입니다. *Expression*이 없는 경우 해당 이름의 열이 입력에 표시되어야 합니다. 
...
다음 예제에서는 `project` 연산자를 사용하여 수행할 수 있는 여러 종류의 조작을 보여줍니다. 입력 테이블 `T`에는 `int` 형식의3개 열(`A`, `B`, `C`)이 있습니다.
...
* *Stop:* 출력에서 생성되는 가장 큰 값이거나, *step*을 사용하여 이 값을 프로시저 단위로 실행하는 경우에는 가장 큰 값의 경계입니다.
* *Step:* 두 연속 값의 차입니다. 

인수는 숫자, 날짜 또는 시간 간격 값이어야 합니다. 아무 테이블의 열이나 참조할 수는 없습니다. 입력 테이블을 기반으로 범위를 계산하려면 [range *함수*](app-insights-analytics-scalars.md#range)를 사용합니다. 이때 [mvexpand 연산자](#mvexpand-operator)를 함께 사용할 수 있습니다.

**반환**

*ColumnName*이라는 단일 열이 포함된 테이블이 반환됩니다. 이 열의 값은 *Start*, *Start* + *Step*~*Stop*까지입니다.
...
`Steps`라는 단일 열이 포함된 테이블이 반환됩니다. 해당 열의 형식은 `long`이고 값은 `1`, `4` 및 `7`입니다.
...
`range` 연산자를 사용하여 작은 임시 차원 테이블을 만드는 방법을 보여줍니다. 이 테이블은 원본 데이터에 값이 없는 경우 0을 추가하는 데 사용됩니다.

## reduce 연산자

    exceptions | reduce by outerMessage

유사한 레코드에 대해 그룹화를 시도합니다. 각 그룹에 대해 연산자는 그룹을 가장 잘 설명한다고 간주하는 `Pattern` 및 해당 그룹의 레코드 `Count`를 출력합니다.
...
`Pattern` 및 `Count`의 두 열이 반환됩니다. 대부분의 경우 패턴은 열의 전체 값입니다. 경우에 따라 일반적인 용어를 식별하고 변수 부분을 '*'로 바꿉니다.
...
* *T:* 정렬할 테이블 입력입니다.
* *Column:* 정렬 기준으로 사용할 *T*의 열입니다. 값의 형식은 숫자, 날짜, 시간 또는 문자열이어야 합니다.
* `asc`: 오름차순으로, 즉 낮은 값에서 높은 값의 순서로 정렬합니다. 기본값은 `desc`, 즉 내림차순(높은 값에서 낮은 값의 순서로 정렬)입니다.
...
[더 많은 예제](app-insights-analytics-aggregations.md)를 확인해 보세요.
...
* *Aggregation:* 열 이름을 인수로 사용하는 `count()` 또는 `avg()` 등의 [집계 함수](app-insights-analytics-aggregations.md)에 대한 호출입니다. [집계 함수의 목록](app-insights-analytics-aggregations.md)을 참조하세요.
...
입력 행은 `by` 식의 값이 같은 그룹으로 정렬됩니다. 그런 다음 지정된 집계 함수를 각 그룹에 대해 계산하여 각 그룹에 대해 한 행을 생성합니다. 결과에는 `by` 열이 포함되며 계산된 각 집계에 대한 열도 하나 이상 포함됩니다. (일부 집계 함수는 여러 열을 반환합니다.)

결과에는 `by` 값의 고유 조합 수에 해당하는 행이 있습니다. 숫자 값 범위를 요약하려면 `bin()`를 사용하여 범위를 불연속 값으로 줄입니다.

**참고**

집계와 그룹화 식에 대해 모두 임의 식을 제공할 수는 있지만 단순 열 이름을 사용하거나 `bin()`을 숫자 열에 적용하는 것이 더 효율적입니다.



## take 연산자

[limit](#limit-operator)의 별칭입니다.
...
* *Sort\_expression:* 행 정렬 기준으로 사용되는 식입니다. 일반적으로 단지 열 이름일 뿐입니다. sort\_expression을 두 개 이상 지정할 수 있습니다.
...
* *Table1*, *Table2*...
 *  `events` 등의 테이블 이름 또는
 *  `(events | where id==42)` 등의 쿼리 식
 *  와일드 카드를 사용하여 지정한 테이블 집합입니다. 예를 들어 `E*`를 사용하는 경우 이름이 `E`로 시작하는 데이터베이스의 모든 테이블의 합집합이 생성됩니다.
* `kind`: 
 * `inner` - 모든 입력 테이블에 공통인 열의 하위 집합이 결과에 포함됩니다.
 * `outer` - 입력에 나타나는 모든 열이 결과에 포함됩니다. 입력 행에 의해 정의되지 않은 셀은 `null`로 설정됩니다.
* `withsource=`*ColumnName:* 지정하는 경우 출력에는 *ColumnName*이라는 열이 포함됩니다. 이 열의 값은 각 행에 값을 제공한 원본 테이블을 나타냅니다.
...
* *T*: 레코드를 필터링할 테이블 형식입력입니다.
* *Predicate:* *T*의 열에 대한 `boolean` [식](app-insights-analytics-scalars.md#boolean)으로, *T*의 각 행에 대해 계산됩니다.
...
* 열 이름과 상수 사이에 **단순 비교를 사용**합니다. '상수'는 테이블에 대한 상수를 의미합니다. 따라서 `now()` 및 `ago()`를 사용할 수 있으며, [`let` 절](#let-clause)을 사용하여 할당된 스칼라 값도 사용 가능합니다.

    예를 들어 `where floor(Timestamp, 1d) == ago(1d)`보다는 `where Timestamp >= ago(1d)`를 사용하는 것이 좋습니다.

* **가장 단순한 용어를 먼저 사용**합니다. `and`를 사용하여 여러 절을 결합하는 경우에는 열이 하나만 포함된 절을 앞쪽에 배치합니다. 따라서 `Timestamp > ago(1d) and OpId == EventId`와 같이 배치하는 것이 더 효율적입니다.
