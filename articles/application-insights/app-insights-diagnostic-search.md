...
---
# <a name="using-diagnostic-search-in-application-insights"></a>Application Insights에서 진단 검색 사용
진단 검색은 페이지 보기, 예외 사항 또는 웹 요청과 같은 개별 원격 분석 항목을 찾고 검색하는 데 사용하는 [Application Insights][시작]의 기능입니다. 또한 코딩한 로그 추적 및 이벤트를 볼 수 있습니다.

## <a name="where-do-you-see-diagnostic-search"></a>진단 검색은 어디에서 보나요?
...
* **사용자 지정 이벤트** - [사용량을 모니터링][추적]하기 위해 TrackEvent()에 호출을 삽입한 경우 여기에서 검색할 수 있습니다.
...
## <a name="inspect-individual-occurrences"></a>개별 항목 검사
...
## <a name="search-by-metric-value"></a>메트릭 값을 기준으로 검색
...
속성 값 중 하나에서 용어를 검색할 수 있습니다. 속성 값을 포함하는 [사용자 지정 이벤트][추적]를 작성한 경우에 특히 유용합니다. 
...
문자열의 일부가 아닌 용어를 검색합니다. 용어는 '.' 및 '_'과 같은 문장 부호를 포함하는 영숫자 문자열입니다. 예:

| 용어 | 해당 용어가 검색되지 *않는* 문자열 | 해당 용어가 검색되는 문자열 |
| --- | --- | --- |
| HomeController.About |about<br/>home |h\*about<br/>home\* |
| IsLocal |local<br/>해당 용어가 검색되지 <br/>\*local |isl\*<br/>islocal<br/>i\*l\* |
...
| database?? |database01, databaseAB 등과 일치합니다.<br/>? 기호는 검색 용어의 시작 부분에 사용할 수 없습니다. |
...
* [코드를 작성][추적]하여 사용자 지정 이벤트, 페이지 보기 및 예외 사항을 보냅니다. 
...
게시 데이터를 자동으로 기록하지는 않지만 [Track추적 또는 로그 호출][trace]. 메시지 매개 변수에 게시 데이터를 넣습니다. 속성을 지정할 수 있는 방법으로 메시지에서 필터링할 수는 없지만 크기 제안은 더 깁니다.
