...
* `asc`은 오름차순으로, 즉 낮은 값에서 높은 값의 순서로 정렬합니다. 기본값은 `desc` 내림차순, 즉 높은 값에서 낮은 값으로 정렬하는 것입니다.
...
입력 행은 `by` 식의 같은 값을 가진 그룹으로 배열됩니다. 그런 다음 지정된 집계 함수를 각 그룹에 대해 계산하여 각 그룹에 대해 한 행을 생성합니다. 결과는 `by` 열 및 각 계산된 집계에 대해 하나 이상의 열을 포함하고 있습니다. (일부 집계 함수는 여러 열을 반환합니다.)

결과에는 `by` 값의 고유 집합이 있는 만큼 행이 있습니다. 숫자 값의 범위에 대해 요약하려는 경우 `bin()`를 사용하여 범위를 불연속 값으로 줄입니다.
...
* 선택이 실제로 범위의 “맨 아래"인지 또는 “맨 위"인지를 제어하기 위해 `asc` 또는 `desc`(기본값)가 나타날 수 있습니다.


**팁**

`top 5 by name`은 겉보기에 `sort by name | take 5`와 같습니다. 그러나 이는 더 빠르게 실행되며 언제나 정렬된 결과를 반환하는 반면에, `take`은 그러한 보장이 없습니다.
...
* `kind`: 
 * `inner` - 결과에는 모든 입력된 테이블에 공통인 열의 부분집합이 있습니다.
...
* *T*: 레코드를 필터링할 테이블 형식 데이터입니다.
* *Predicate:* *T*의 열에 대한 `boolean` [식](app-analytics-scalars.md#boolean)입니다. *T*의 각 행에 대해 계산합니다.

**반환**

*Predicate*이 is `true`인 *T*의 행입니다.

**팁**

가장 빠른 성능을 얻으려면:

* 열 이름과 상수 사이에 **단순 비교 사용**.('상수'는 테이블에 대한 상수를 의미함 - 따라서 `now()` 및 `ago()`은 좋으므로 [`let` 문](app-analytics-syntax.md#let-statements)을 사용하여 할당한 스칼라 값입니다.)

    예를 들어 `where floor(Timestamp, 1d) == ago(1d)`보다 `where Timestamp >= ago(1d)`을 선호합니다.

* **가장 단순한 항 먼저**: 여러 개의 절을 `and`와 공동 조인한 경우 열을 한 개만 포함하고 있는 절을 먼저 놓습니다. 따라서 `Timestamp > ago(1d) and OpId == EventId`이 다른 방법보다 낫습니다.
