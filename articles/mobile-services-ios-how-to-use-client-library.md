...
다음 예에서는 [Live Connect SDK]를 사용하여 iOS 앱용 Single Sign-On을 사용하도록 설정합니다.
...
_(선택 사항) 이 섹션에서는 클라이언트 라이브러리를 사용할 때 단위 테스트를 작성하는 방법을 보여 줍니다(Yavor의 정보)._

<h2><a name="#customizing"></a>방법: 클라이언트 사용자 지정</h2>

_(선택 사항) 이 섹션에서는 클라이언트 사용자 지정 동작을 보내는 방법을 보여 줍니다._

###<a name="custom-headers"></a>방법: 요청 헤더 사용자 지정

_(선택 사항) 이 섹션에서는 사용자 지정 요청 헤더를 보내는 방법을 보여 줍니다._

자세한 내용은 서버 쪽에서 헤더 처리에 대한 새 항목을 참조하세요.

###<a name="custom-serialization"></a>방법: serialization 사용자 지정

_(선택 사항) 이 섹션에서는 특성을 사용하여 데이터 형식이 serialize되는 방식을 사용자 지정하는 방법을 보여 줍니다._
