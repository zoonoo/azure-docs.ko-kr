...
이 가이드는 Operational Insights 대시보드가 저장된 모든 검색을 시각화하여 환경을 보는 단일 렌즈를 제공하는 방법을 이해하는 데 도움이 됩니다.
...
대시보드에서 타일은 저장된 검색을 기반으로 합니다. Operational Insights에는 여러 저장된 검색이 미리 만들어져 있으므로 바로 시작할 수 있습니다. 시작하는 방법을 간략하게 설명하는 다음 그림이 표시됩니다.

![그림](./media/operational-insights-use-dashboards/pictorial.png)

내 대시보드 뷰에서 페이지 맨 아래에 있는 '사용자 지정' 기어를 클릭하여 사용자 지정 모드로 전환합니다. 페이지 오른쪽에 열리는 패널에는 작업 영역의 저장된 검색이 모두 표시됩니다.

![타일 추가 1](./media/operational-insights-use-dashboards/add-tile1.png)

저장된 검색을 타일로 시각화하려면 왼쪽의 빈 공간으로 끌어다 놓습니다. 저장된 검색을 끌면 타일로 바뀝니다.
...
검색 결과가 필드에 따라 집계되는지 여부에 따라 저장된 검색 결과의 타임라인 또는 필드별 결과 목록이 표시됩니다.

**메트릭** <p> ![메트릭](./media/operational-insights-use-dashboards/metric.png)

총 검색 결과 적중 횟수가 타일에 숫자로 표시됩니다. 메트릭 타일을 사용하면 임계값에 도달할 때 타일이 강조 표시되도록 임계값을 설정할 수 있습니다.
