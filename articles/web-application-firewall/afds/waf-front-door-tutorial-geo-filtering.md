...
`nonUSBlockRule`New-AzFrontDoorWafCustomRuleObject[를 사용하여 일치 조건, 작업, 우선 순위에 따라 CustomRule 개체(](/powershell/module/az.frontdoor/new-azfrontdoorwafcustomruleobject))를 만듭니다.  CustomRule은 여러 MatchCondition을 가질 수 있습니다.  이 예제에서는 작업이 가장 우선 순위가 높은 차단 및 우선 순위 1로 설정됩니다.
...
`Get-AzResourceGroup`을 사용하여 Front Door 프로필이 포함된 리소스 그룹의 이름을 찾습니다. 그런 다음, Front Door 프로필을 포함하는 지정된 리소스 그룹에 `geoPolicy`New-AzFrontDoorWafPolicy`nonUSBlockRule`를 사용하여 [이 포함된 ](/powershell/module/az.frontdoor/new-azfrontdoorwafpolicy) 정책 개체를 만듭니다. 지역 정책에 고유한 이름을 제공해야 합니다. 
...
다음으로, `geoPolicy`Set-AzFrontDoor[를 사용하여 프런트 엔드 WebApplicationFirewallPolicyLink 속성을 ](/powershell/module/az.frontdoor/set-azfrontdoor)의 resourceId로 설정합니다.
