...
Azure PowerShell를 사용 하 여 관리 되는 규칙을 사용 하지 않으려면 개체 설명서를 참조 하세요 [`PSAzureManagedRuleOverride`](/powershell/module/az.frontdoor/new-azfrontdoorwafmanagedruleoverrideobject?preserve-view=true&view=azps-4.7.0) . Azure CLI를 사용 하려면 설명서를 참조 [`az network front-door waf-policy managed-rules override`](/cli/azure/ext/front-door/network/front-door/waf-policy/managed-rules/override?preserve-view=true&view=azure-cli-latest) 하세요.
