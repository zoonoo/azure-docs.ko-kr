...
1. **Azure Front 도어 서비스 또는 Application Gateway에 대해 웹 응용 프로그램 방화벽을 사용 하도록 설정 해야 합니다**. 리소스를 만들 때 waf가 있는 경우 Azure Front 도어 서비스 및 응용 프로그램 게이트웨이는에 대해 평가 됩니다. 정책에는 감사, 거부 및 사용 안 함 이라는 세 가지 효과가 있습니다. 감사 트랙은 Azure Front 도어 서비스 또는 Application Gateway에 WAF가 없고 사용자에 게 현재 준수 하지 않는 Azure Front 도어 서비스 또는 Application Gateway를 볼 수 있도록 합니다. Deny는 WAF가 연결 되지 않은 경우 Azure Front 도어 서비스 또는 Application Gateway 생성 되지 않도록 합니다. 사용 안 함이 정책을 해제 합니다.

2. **웹 응용 프로그램 방화벽은 Application Gateway 및 Azure Front 도어 서비스에 대해 설정 된 모드 여야 합니다**. 웹 응용 프로그램 방화벽은 방지 또는 검색 중 어떤 모드에서 든 평가 됩니다. 이 정책은 웹 응용 프로그램 방화벽에서 모드 일관성을 보장 합니다. 정책에는 감사, 거부 및 사용 안 함 이라는 세 가지 효과가 있습니다. WAF가 지정 된 모드에 맞지 않는 경우 감사 추적입니다. Deny는 올바른 모드가 아닌 경우 WAF를 만들지 않도록 합니다. 사용 안 함이 정책을 해제 합니다.
...
![Azure 웹 애플리케이션 방화벽](../media/waf-azure-policy/policy-listings.png)
