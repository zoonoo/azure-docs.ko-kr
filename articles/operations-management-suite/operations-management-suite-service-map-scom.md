...
서비스 맵과 System Center Operations Manager(SCOM) 간 통합의 이 미리 보기를 사용하면 서비스 맵의 동적 종속성 맵을 기반으로 SCOM에서 분산 응용 프로그램 다이어그램을 자동으로 만들 수 있습니다.
...
4.    OMS 작업 영역과 연결된 Azure 구독에 액세스할 수 있는 서비스 주체.  서비스 주체에 대한 자세한 내용은 부록을 참조하세요.
...
3. 이 마법사의 첫 번째 단계는 서비스 주체 이름을 구성하는 연결 구성입니다. 서비스 주체의 테넌트 ID, 응용 프로그램 ID(또는 사용자 이름 또는 클라이언트 ID) 및 암호를 입력하세요.  서비스 주체를 만드는 방법에 대한 부록 섹션을 참조하세요.
...
**_Microsoft.SystemCenter.ServiceMap.Import.Rule** 규칙은 서비스 맵에서 주기적으로 정보를 가져오기 위해 생성됩니다.  사용자는 이 규칙의 재정의를 구성하여 동기화 시간을 변경할 수 있습니다.
