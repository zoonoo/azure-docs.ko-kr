...
서비스 맵에 문제가 발생하면 다음 정보를 사용하여 여러 구성 요소에서 문제 해결 정보를 수집할 수 있습니다.

### <a name="windows-agents"></a>Windows 에이전트

#### <a name="microsoft-dependency-agent"></a>Microsoft 종속성 에이전트
종속성 에이전트에서 문제 해결 데이터를 생성하려면 관리자 권한으로 명령 프롬프트를 열고 다음 명령을 사용하여 CollectDependencyAgentData.vbs 스크립트를 실행합니다.  --help 플래그를 추가하여 추가 옵션을 표시할 수 있습니다.
...
종속성 에이전트 관리 팩은 Microsoft 관리 에이전트 내에서 실행됩니다.  종속성 에이전트에서 데이터를 수신하고 서비스 맵 클라우드 서비스에 전달합니다.
...
종속성 에이전트 Fluentd 플러그 인은 OMS Linux 에이전트 내에서 실행됩니다.  종속성 에이전트에서 데이터를 수신하고 서비스 맵 클라우드 서비스에 전달합니다.  
...
다음 섹션에서는 종속성 에이전트에 대해 지원되는 운영 체제를 나열합니다.   운영 체제에 대해&32;비트 아키텍처는 지원되지 않습니다.
