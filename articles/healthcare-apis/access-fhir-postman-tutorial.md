...
- FHIR 서비스에 액세스하는 데 사용할 [클라이언트 애플리케이션](register-confidential-azure-ad-client-app.md)입니다.
- FHIR 서비스에 액세스할 수 있는 권한(예: "FHIR 데이터 기여자")을 클라이언트 애플리케이션에 부여했습니다. [Azure RBAC for FHIR 구성](./configure-azure-rbac.md)에 대한 자세한 정보
...
- 구성된 `audience`. 이는 일반적으로 FHIR 서버의 URL입니다(예: `https://<FHIR-SERVER-NAME>.azurehealthcareapis.com` 또는 `https://azurehealthcareapis.com`).
