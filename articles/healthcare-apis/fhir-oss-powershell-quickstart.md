...
Azure용 Microsoft FHIR 서버 [GitHub 리포지토리](https://github.com/Microsoft/fhir-server)에는 필요한 모든 리소스를 배포하는 템플릿이 포함되어 있습니다. 배포 프로세스는 필요한 Azure 리소스를 만들고 구성하는 데 몇 분이 걸립니다. 다음을 사용하여 배포합니다.
...
> [!NOTE]
> 아직 로그인하지 않은 경우 먼저 다음 명령을 실행합니다.
...
기존 리소스 그룹을 사용하려면 코드에 표시된 대로 $rg 변수 정의 줄 및 Azure ARM 템플릿 배포 명령줄에서 $rg 값을 변경합니다.
