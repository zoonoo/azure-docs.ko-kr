...
내보내기의 다음 단계는 저장소 계정에 쓰도록 Azure API 용 Azure API에 대 한 권한을 할당 하는 것입니다.

저장소 계정을 만든 후 저장소 계정의 **Access Control (IAM)** 로 이동 하 여 **역할 할당 추가** 를 선택 합니다.

![역할 할당 내보내기](media/export-data/fhir-export-role-assignment.png)

여기서는 서비스 이름에 역할 **저장소 Blob 데이터 참가자** 를 추가 하 고 **저장** 을 선택 합니다.
