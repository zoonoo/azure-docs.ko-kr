...
2. 3 단계에 대 한 스크립트에서 다음 변수를 업데이트 합니다.
      1. `$contentID`는 인코딩되지 않은 라이선스 파일 (.xml)의 ContentID 값입니다. 선택한 텍스트 편집기에서 라이선스 파일을 열 수 있습니다.
      2. `$licenseBlob`인코딩된 라이선스 파일 (. bin)의 라이선스 blob에 대 한 전체 문자열입니다. 선택한 텍스트 편집기에서 인코딩된 라이선스 파일을 열 수 있습니다. 
