---
title: IOS에서 Windows 가상 데스크톱 (클래식)에 연결-Azure
description: IOS 클라이언트를 사용 하 여 Windows 가상 데스크톱 (클래식)에 연결 하는 방법
...
ms.locfileid: "87270739"
---
# <a name="connect-to-windows-virtual-desktop-classic-with-the-ios-client"></a>IOS 클라이언트를 사용 하 여 Windows 가상 데스크톱 (클래식)에 연결

> 적용 대상: iOS 13.0 이상. IPhone, iPad 및 iPod touch와 호환 됩니다.

>[!IMPORTANT]
>이 콘텐츠는 windows 가상 데스크톱 개체 Azure Resource Manager를 지원 하지 않는 Windows 가상 데스크톱 (클래식)에 적용 됩니다. Azure Resource Manager Windows 가상 데스크톱 개체를 관리 하려는 경우 [이 문서](../connect-ios.md)를 참조 하세요.
