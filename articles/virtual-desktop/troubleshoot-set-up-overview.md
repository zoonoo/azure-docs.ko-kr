...
| 시작 시 원격 데스크톱 클라이언트 오작동                                                 | [원격 데스크톱 클라이언트 연결](troubleshoot-client-connection.md) 을 참조 하 고, 문제가 해결 되지 않으면 **Windows 가상 데스크톱 (Compute)에 대 한 지원 요청을 엽니다**.  <br> <br> 네트워크 문제인 경우 사용자는 네트워크 관리자에 게 문의 해야 합니다. |
| 연결 되었지만 피드 없음                                                                 | [원격 데스크톱 클라이언트 연결](troubleshoot-client-connection.md)의 "사용자 연결을 제외한 아무 것도 표시 되지 않습니다 (피드 없음)" 섹션을 사용 하 여 문제를 해결 합니다. <br> <br> 사용자가 앱 그룹에 할당 된 경우 **Windows 가상 데스크톱 (Compute)에 대 한 지원 요청을 엽니다**. |
