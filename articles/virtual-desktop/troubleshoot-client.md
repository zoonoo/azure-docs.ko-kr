...
1.2.790 버전부터 정보 페이지 또는 명령을 사용 하 여 사용자 데이터를 다시 설정할 수 있습니다.

다음 명령을 사용 하 여 사용자 데이터를 제거 하 고, 기본 설정을 복원 하 고, 모든 작업 영역에서 구독을 취소 합니다.

```cmd
msrdcw.exe /reset [/f]
```

이전 버전의 원격 데스크톱 클라이언트를 사용 하는 경우 클라이언트를 제거한 후 다시 설치 하는 것이 좋습니다.
