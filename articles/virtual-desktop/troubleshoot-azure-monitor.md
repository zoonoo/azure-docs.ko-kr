...
## <a name="the-configuration-workbook-isnt-working-properly"></a>구성 통합 문서가 제대로 작동 하지 않습니다.

Azure Monitor 구성 통합 문서가 작동 하지 않는 경우 이러한 리소스를 사용 하 여 파트를 수동으로 설정할 수 있습니다.
...
또는 리소스가 부족 하거나 필요한 권한이 없는 경우에도 문제가 발생할 수 있습니다.

구독에 Windows 가상 데스크톱 리소스가 없으면 *subscription* 매개 변수에 표시 되지 않습니다.

올바른 구독에 대 한 읽기 권한이 없는 경우 *구독* 매개 변수에 표시 되지 않으며 대시보드에 데이터가 표시 되지 않습니다. 이 문제를 해결 하려면 구독 소유자에 게 문의 하 고 읽기 액세스를 요청 하세요.

## <a name="my-data-isnt-displaying-properly"></a>데이터가 제대로 표시 되지 않습니다.

데이터가 올바르게 표시 되지 않는 경우 Azure Monitor 구성 프로세스 중에 발생 했을 수 있습니다. 먼저 [Windows 가상 데스크톱에 대 한 Azure Monitor를 사용 하 여 배포를 모니터링 하는 방법](azure-monitor.md)에 설명 된 대로 구성 통합 문서의 모든 필드를 입력 했는지 확인 합니다. 언제 든 지 새 환경과 기존 환경 모두에 대 한 설정을 변경할 수 있습니다. 카운터 또는 이벤트가 없는 경우 연결 된 데이터는 Azure Portal에 표시 되지 않습니다.

정보가 누락 되지 않았지만 데이터가 여전히 제대로 표시 되지 않는 경우 쿼리 또는 데이터 원본에 문제가 있을 수 있습니다. 

설치 오류가 표시 되지 않고 계속 해 서 원하는 데이터가 표시 되지 않는 경우 15 분 동안 기다렸다가 피드를 새로 고칠 수 있습니다. Azure Monitor는 로그 데이터를 채우는 데 15 분의 대기 시간이 있습니다. 자세히 알아보려면 [Azure Monitor의 로그 데이터 수집 시간](../azure-monitor/platform/data-ingestion-time.md)을 참조 하세요.

마지막으로 정보를 누락 하지는 않지만 데이터가 여전히 표시 되지 않으면 쿼리 또는 데이터 원본에 문제가 있을 수 있습니다. 해당 하는 경우 지원에 문의 하 여 문제를 해결 해야 할 수 있습니다.
...
제품 그룹이 원본 템플릿을 업데이트할 때 사용자 지정 된 템플릿은 업데이트 되지 않습니다. 이는 통합 문서 도구에서 설계 된 것입니다. 업데이트 된 템플릿의 복사본을 저장 하 고 업데이트를 적용 하도록 사용자 지정을 다시 빌드해야 합니다. 자세한 내용은 [통합 문서 기반 정보](../azure-monitor/insights/troubleshoot-workbooks.md) 및 통합 문서 [개요](../azure-monitor/platform/workbooks-overview.md)문제 해결을 참조 하세요.
...
다음은 현재 인식 하 고 해결 하는 데 사용 하는 문제입니다.

- 현재는 한 번에 하나씩만 모니터링할 구독, 리소스 그룹 및 호스트 풀을 선택할 수 있습니다. 이로 인해 사용자 보고서 페이지를 사용 하 여 사용자의 경험을 이해 하는 경우 사용자가 사용 하 고 있는 올바른 호스트 풀이 있는지 확인 해야 합니다. 그렇지 않으면 해당 데이터는 시각적 개체를 채우지 않습니다.

- 현재 통합 문서의 사용자 지정 템플릿을 저장 하지 않는 한 Azure Monitor에서 즐겨 사용 하는 설정을 저장할 수 없습니다. 즉, IT 관리자가 Windows 가상 데스크톱에 대 한 Azure Monitor를 열 때마다 구독 이름, 리소스 그룹 이름 및 호스트 풀 기본 설정을 입력 해야 합니다.

- 현재 Windows 가상 데스크톱의 Azure Monitor에서 Excel로 데이터를 내보낼 수 있는 방법이 없습니다.

- 선택한 구독 내의 모든 제품에 대 한 모든 심각도 1 Azure Monitor 경고가 개요 페이지에 표시 됩니다. 이는 구독에 있는 다른 제품의 경고가 Windows 가상 데스크톱에 영향을 줄 수 있으므로 의도적으로 설계 되었습니다. 현재 쿼리는 개요 페이지에서 우선 순위가 높은 심각도 0 경고를 제외 하 고 심각도 1 경고로 제한 됩니다.
