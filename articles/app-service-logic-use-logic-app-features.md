...
[이전 항목][새 논리 앱 만들기]에서는 첫 번째 논리 앱을 만들었습니다. 이제 앱 서비스 논리 앱을 사용하여 보다 완전한 프로세스를 빌드하는 방법을 설명하겠습니다. 이 항목에서는 다음과 같은 새 논리 앱 개념을 소개합니다.
...
이 항목을 완료하기 전에 [새 논리 앱 만들기]의 단계를 완료해야 합니다. [Azure 포털]에서 논리 앱을 찾은 다음 요약에서 **트리거 및 동작**을 클릭하여 논리 앱이란 무엇인가요?를 편집합니다.
...
2. **Dropbox 커넥터** 작업을 편집하려면 연필 아이콘을 클릭합니다.
...
2. `twitterconnector` 작업으로 스크롤하고 쿼리 값을 찾은 다음 `@concat('#', parameters('topic'))`으로 바꿉니다.
	**concat** 함수는 두 개 이상의 문자열을 결합합니다. 
 
3. 끝으로, `dropboxconnector` 작업으로 이동하여 다음과 같이 topic 매개 변수를 추가합니다.
...
이 콜백을 사용하여 사용자 지정 응용 프로그램 내에서 논리 앱을 호출할 수 있습니다. **기본** 인증을 사용해야 합니다. 사용자 이름 `default`가 자동으로 만들어지며, 암호는 **속성** 블레이드의 **기본 액세스 키**입니다. 예를 들면 다음과 같습니다. 
