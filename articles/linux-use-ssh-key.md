...
현재 버전의 Azure 관리 포털은 X509 인증서에 캡슐화된 SSH 공개 키만 허용합니다. SSH 키를 생성하고 Azure에 사용하려면 아래 단계를 따르십시오.
...
1.  다음 위치에서 msysgit를 다운로드하여 설치합니다. [http://msysgit.github.com/][1]
2.  설치된 디렉터리(예: c:\\msysgit\\msys.exe)에서 `msys`를 실행합니다.
3.  `cd bin`을 입력하여 `bin` 디렉터리로 변경합니다.

### Windows용 GitHub 사용

1.  다음 위치에서 Windows용 GitHub를 다운로드하여 설치합니다. [http://windows.github.com/][2]
2.  시작 메뉴 > 모든 프로그램 > GitHub, Inc에서 Git 셸을 실행합니다.

### cygwin 사용

1.  다음 위치에서 Cygwin을 다운로드하여 설치합니다. [http://cygwin.com/][3]
...
4.  표시되는 질문에 대답합니다.
5.  두 개의 파일, 즉 `myPrivateKey.key`와 `myCert.pem`이 만들어집니다.
...
1.  다음 위치에서 puttygen을 다운로드하여 설치합니다. [http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html][4]
2.  `puttygen.exe`를 실행합니다.
...
1.  다음 위치에서 putty를 다운로드하여 설치합니다. [http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html][4]
2.  putty.exe를 실행합니다.
3.  관리 포털의 IP를 사용하여 호스트 이름을 채웁니다.
    
    ![linuxputtyconfig](./media/linux-use-ssh-key/linuxputtyconfig.png)

4.  **열기**를 선택하기 전에 연결 > SSH > 인증 탭을 클릭하여 키를 선택합니다. 채울 필드는 아래 스크린샷을 참조하십시오.
