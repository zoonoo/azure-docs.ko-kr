...
-   **게시/구독 라우팅.** 각 장치에서는 해당 핸들을 알림 허브로 보낼 때 하나 이상의 *태그*를 지정할 수 있습니다. 태그에 대한 자세한 내용은 다음 섹션을 참조하십시오. 태그는 사전 프로비전하거나 삭제할 필요가 없습니다. 태그는 사용자 또는 관심 그룹으로 알림을 보낼 수 있는 간단한 방법을 제공합니다. 태그는 모든 앱별 식별자(예: 사용자 또는 그룹 ID)를 포함할 수 있으므로 태그를 사용하면 앱 백 엔드가 장치 핸들을 저장하고 관리해야 하는 부담에서 자유로워집니다.
-   **확장.** 알림 허브는 다시 설계하거나 분할할 필요 없이 수백 개의 장치로 확장됩니다.
