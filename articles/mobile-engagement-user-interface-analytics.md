<properties 
   pageTitle="Azure Mobile Engagement 사용자 인터페이스 - 분석" 
...
UI의 분석 섹션에서는 24시간마다 업데이트되는 기록 데이터에 따라 응용 프로그램에 대한 집계 정보를 제공합니다. 정보는 꺾은선형/막대형/원형 차트, 표, 맵으로 구성된 여러 대시보드에 표시됩니다. 데이터를 .csv 파일로 다운로드할 수도 있습니다. 이와 동일한 대부분의 정보는 UI의 모니터 섹션에서도 실시간으로 제공되며 분석 API에서 액세스할 수도 있습니다. "개념"의 "용어집"에 분석 및 모니터링에서 사용되는 용어와 약어의 정의가 나와 있습니다. 이러한 용어로는 활성 사용자, 새 사용자, 재방문 사용자, 세션, 사용자 경로 그래프, 사용자 맵, 추적 URL, 추세, 활동, 이벤트, 작업, 오류, 추가 정보, 작동 중단, 앱 정보 등이 있습니다.
...
## 표준 분석 및 사용자 지정 분석
Azure Mobile Engagement에서는 앱을 SDK와 통합하는 즉시 그래프로 작성할 수 있는 응용 프로그램에 대한 기본적인 표준 분석 정보 집합을 제공합니다. 또한 최종 사용자 동작에 대해 원하는 추가 사용자 지정 분석 정보를 수집하는 기능도 제공합니다. 이러한 정보를 수집하려면 Azure Mobile Engagement에서 해당 추가 데이터를 자동으로 수집할 수 있도록 사용자 지정 "앱 정보 태그"를 사용하는 태그 계획을 "설정"에서 작성하면 됩니다.

**참고 항목:**
-  [UI 설명서 - 설정][Link 20]
 
## 분석 헤더
- 항목 이름: 계산 중인 항목의 레이블을 지정합니다.
- 도움말 표시: 이 섹션에 대한 상황에 맞는 정보를 표시합니다.
- 버전: 응용 프로그램의 각 버전에 대해 각기 다른 분석 정보를 표시하거나 모든 버전에 대한 정보를 표시할 수 있습니다. 참고: UI에서 분석 데이터를 필터링하면 앱 버전에 관계없이 해당 형식의 모든 예제가 표시됩니다. 즉, "작동 중지"를 이름으로 필터링하면 앱 버전 1과 2의 예제가 모두 표시됩니다.
- 기간: 지난 7일, 지난 30일, 모든 시간, 사용자 지정
- 속도: 단위: 시간, 일, 주, 월
- 보기: 꺾은선형 차트, 표, 대시보드로 보내기, CSV 파일 다운로드
...
- 원본별 추적에는 지정된 프로모션 캠페인을 통해 응용 프로그램을 다운로드한 새 사용자의 수가 표시됩니다. 장치 식별자를 통해 사용자를 식별합니다. 이 식별자는 각 장치에서 고유합니다. 각각의 새 사용자는 새 장치 하나에 해당합니다. 사용자는 지정된 시간 간격 동안 첫 번째 세션을 수행하는 경우 새 사용자로 간주됩니다.
- 스토어별 추적에는 지정된 스토어에서 응용 프로그램을 다운로드한 새 사용자의 수가 표시됩니다. 장치 식별자를 통해 사용자를 식별합니다. 이 식별자는 각 장치에서 고유합니다. 각각의 새 사용자는 새 장치 하나에 해당합니다. 사용자는 지정된 시간 간격 동안 첫 번째 세션을 수행하는 경우 새 사용자로 간주됩니다.

> 참고: 기간은 사용자 장치 설정의 날짜를 기준으로 하므로 휴대폰에 날짜가 잘못 설정된 사용자의 경우 잘못된 기간의 데이터가 표시됩니다.
...
각 카드는 3개의 주요 부분으로 구성됩니다.

1. 1: 고려 대상 집단 및 기간
2. 2-4: 현재 기간의 재방문 주기
3. 5: 기록의 스파크라인

**아래에서 각 요소에 대해 자세히 설명합니다.**
