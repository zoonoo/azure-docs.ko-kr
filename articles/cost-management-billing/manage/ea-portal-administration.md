...
엔터프라이즈 관리자는 Azure EA 등록을 관리할 때 가장 많은 권한을 가집니다. EA 계약이 설정될 때 초기 Azure EA 관리자가 생성되었습니다. 하지만 언제든지 관리자를 새로 추가하거나 제거할 수 있습니다. 기존 관리자만이 새 관리자를 추가할 수 있습니다. Enterprise admins를 추가 하는 방법에 대 한 자세한 내용은 [다른 엔터프라이즈 관리자 만들기](ea-portal-get-started.md#create-another-enterprise-admin)를 참조 하세요. 청구 프로필 역할 및 작업에 대 한 자세한 내용은 [청구 프로필 역할 및 작업](understand-mca-roles.md#billing-profile-roles-and-tasks)을 참조 하세요.
...
부서 관리자 추가에 대한 자세한 내용은 [Azure EA 부서 관리자 만들기](ea-portal-get-started.md#add-a-department-admin)를 참조하세요.
...
부서 지출 할당량은 이제 부서 탭의 부서 목록 보기에 표시 됩니다. 현재 약정의 끝에 Azure EA 포털은 다음 약정 기간에 대 한 지출 할당량을 유지 합니다.
...
계정 전송은 한 등록에서 다른 등록으로 계정 소유자를 이동 합니다. 계정 소유자의 모든 관련 구독이 대상 등록으로 이동 됩니다. 이는 여러 활성 등록 있고 선택한 계정 소유자만 이동 하려는 경우에 수행 됩니다.
...
- 전송이 완료 되 면 전송 된 계정이 원본 등록에서 비활성 상태로 표시 되 고 대상 등록 아래에 활성 상태로 표시 됩니다.
...
등록 전송은 다음과 같은 경우에 고려 됩니다.

- 현재 등록 약정 기간이 끝에 도달 했습니다.
- 등록이 만료 됨/확장 됨 상태 이며 새로운 계약이 협상 되었습니다.
- 여러 등록 있고 모든 계정 및 청구를 단일 등록으로 통합 하려는 경우.

엔터프라이즈 등록 전체를 다른 등록에 양도해 달라고 요청하면 다음 작업이 수행됩니다.

- 모든 EA 부서 관리자를 포함 하 여 모든 Azure 서비스, 구독, 계정, 부서 및 전체 등록 구조가 새 대상 등록으로 전송 됩니다.
...
- 대상 및 원본 등록 EA 관리자의 승인이 필요 합니다.
- 등록을 양도해도 요구 사항이 충족되지 않는 경우 계정 양도를 고려해 보세요.
- 원본 등록 상태는 전송 됨으로 업데이트 되 고 기록 사용 현황 보고를 위해서만 사용할 수 있습니다.
...
### <a name="no-services-affected-for-account-and-enrollment-transfers"></a>계정 및 등록 전송에 영향을 주는 서비스가 없음

계정 또는 등록을 전송 하는 동안 가동 중지 시간이 발생 하지 않습니다. 모든 필수 정보를 입력하면 요청 당일에 양도를 완료할 수 있습니다.
