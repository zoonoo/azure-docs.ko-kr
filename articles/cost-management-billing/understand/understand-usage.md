...
AdditionalInfo | 전체 | 서비스 특정 메타데이터입니다. 예를 들어 가상 머신용 이미지 형식입니다.
BillingAccountId<sup>1</sup> | 전체 | 루트 청구 계정의 고유 식별자입니다.
BillingAccountName | 전체 | 청구 계정의 이름입니다.
BillingCurrency | 전체 | 청구 계정과 연결된 통화입니다.
BillingPeriod | EA, PAYG | 요금 청구 기간입니다.
BillingPeriodEndDate | 전체 | 청구 기간의 종료 날짜입니다.
BillingPeriodStartDate | 전체 | 청구 기간의 시작 날짜입니다.
BillingProfileId<sup>1</sup> | 전체 | EA 등록계약, PAYG 구독, MCA 청구 프로필 또는 AWS 통합 계정의 고유 식별자입니다.
BillingProfileName | 전체 | EA 등록계약, PAYG 구독, MCA 청구 프로필 또는 AWS 통합 계정의 이름입니다.
ChargeType | 전체 | 요금이 사용량(**Usage**), 구매(**Purchase**) 또는 환불(**Refund**) 중 무엇을 나타내는지 알려줍니다.
ConsumedService | 전체 | 요금과 연결된 서비스의 이름입니다.
...
Date<sup>1</sup> | 전체 | 요금의 사용량 또는 구매 날짜입니다.
EffectivePrice | 전체 | 기간의 혼합 단가입니다. 혼합 가격은 단가의 변동 평균을 계산한 것입니다(예: 시간이 지남에 따라 수량이 증가하면 가격을 할인해주는 점진적 차등 적용).
ExchangeRateDate | MCA | 환율이 설정된 날짜입니다.
ExchangeRatePricingToBilling | MCA | 가격 책정 통화의 비용을 청구 통화로 변환하는 데 사용되는 환율입니다.
빈도 | 전체 | 요금이 반복될 것으로 예상되는지 여부를 나타냅니다. 요금은 일회성(**OneTime**)이거나, 매월 또는 매년 반복되거나(**Recurring**), 사용량을 기준으로 할 수 있습니다(**UsageBased**).
...
IsAzureCreditEligible | 전체 | Azure 크레딧을 사용 하기 위해 요금이 청구 될 수 있는지 여부를 나타냅니다 (값: True, False).
위치 | MCA | 리소스가 실행되고 있는 데이터 센터 위치입니다.
MeterCategory | 전체 | 측정기에 대한 분류 범주의 이름입니다. 예를 들어 *클라우드 서비스* 및 *네트워킹*입니다.
MeterId<sup>1</sup> | 전체 | 미터의 고유 식별자입니다.
MeterName | 전체 | 미터의 이름입니다.
MeterRegion | 전체 | 위치에 따라 가격이 책정되는 서비스의 데이터 센터 위치 이름입니다. Location을 참조하세요.
MeterSubCategory | 전체 | 미터의 하위 분류 범주 이름입니다.
OfferId<sup>1</sup> | 전체 | 구매한 제품의 이름입니다.
...
ProductOrderId | 전체 | 제품 주문의 고유 식별자입니다.
ProductOrderName | 전체 | 제품 주문의 이름입니다.
PublisherName | 전체 | Marketplace 서비스의 게시자입니다.
PublisherType | 전체 | 게시자 유형 (값: **Azure**, **AWS**, **Marketplace**).
...
SubscriptionId<sup>1</sup> | 전체 | Azure 구독의 고유 식별자입니다.
SubscriptionName | 전체 | Azure 구독의 이름입니다.
Tags<sup>1</sup> | 전체 | 리소스에 할당된 태그입니다. 리소스 그룹 태그를 포함하지 않습니다. 내부 지불 거절 비용을 그룹화하거나 균등 맞춤하는 데 사용할 수 있습니다. 자세한 내용은 [태그를 사용하여 Azure 리소스 구성](https://azure.microsoft.com/updates/organize-your-azure-resources-with-tags/)을 참조하세요.
조건 | 전체 | 제품의 유효 기간을 표시합니다. 예를 들어 예약 된 인스턴스의 경우 12 개월을 용어로 표시 합니다. 일회성 구매 또는 되풀이 구매의 기간은 1개월(SaaS, Marketplace 지원)입니다. Azure 소비에는 적용되지 않습니다.
UnitOfMeasure | 전체 | 서비스 청구를 위한 측정 단위. 예를 들어 컴퓨팅 서비스는 시간당 청구됩니다.
