...
Azure 계약 또는 계정을 등록할 때 관리자 권한이 있는 계정이 Cloudyn에 만들어졌으므로 이 자습서의 모든 단계를 수행할 수 있습니다. 이 자습서에서는 사용자 관리 및 엔터티 관리를 비롯한 Cloudyn 데이터 액세스 권한에 대해 설명합니다. 다음 방법을 알아봅니다.
...
Cloudyn에서 개인 데이터를 삭제하거나 내보내려면 지원 티켓을 만들어야 합니다. 생성된 지원 티켓은 공식 요청(데이터 주체 요청)의 역할을 합니다. 그러면 Microsoft는 프롬프트 작업을 수행하여 계정을 제거하고 고객 또는 개인 데이터 삭제합니다.
