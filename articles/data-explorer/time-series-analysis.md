...
ADX(Azure 데이터 탐색기)는 클라우드 서비스 또는 IoT 디바이스에서 원격 분석 데이터의 수집을 지속적으로 수행합니다. 서비스 상태 모니터링, 실제 프로덕션 프로세스 및 사용량 추세와 같은 다양한 인사이트를 위해 이 데이터를 분석할 수 있습니다. 일반적인 기준 패턴과 비교해 패턴의 편차를 발견하려면 선택한 메트릭의 시계열에서 분석을 수행합니다.
