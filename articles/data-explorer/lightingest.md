...
1. 수집을 관리하는 Azure Data Explorer 클러스터에 연결 문자열다음에 입력합니다. `LightIngest`
...
* 권장되는 방법은 `LightIngest` 에서 `https://ingest-{yourClusterNameAndRegion}.kusto.windows.net`섭취 끝점을 사용하는 것입니다. 이렇게 하면 Azure Data Explorer 서비스가 수집 로드를 관리할 수 있으며 일시적인 오류로부터 쉽게 복구할 수 있습니다. 그러나 엔진 끝점()에서 `LightIngest` `https://{yourClusterNameAndRegion}.kusto.windows.net`직접 작동하도록 구성할 수도 있습니다.
* 최적의 수집 성능을 위해 LightIngest는 원시 데이터 크기를 알고 로컬 `LightIngest` 파일의 압축되지 않은 크기를 추정하는 것이 중요합니다. 그러나 `LightIngest` 먼저 다운로드하지 않고는 압축된 Blob의 원시 크기를 올바르게 추정하지 못할 수 있습니다. 따라서 압축된 Blob을 수집할 때 `rawSizeBytes` Blob 메타데이터의 속성을 압축되지 않은 데이터 크기로 바이트로 설정합니다.
...
|-형식               |-f           |문자열  |Optional  |원본 데이터 형식입니다. [지원되는 형식](https://docs.microsoft.com/azure/kusto/management/data-ingestion/#supported-data-formats) 중 하나여야 합니다. |
...
* 타임스탬프 바로 다음에 있는 상수 텍스트 예를 들어 Blob 이름이 'historicalvalues19840101.parquet'로 끝나는 경우(타임스탬프는 연도의 네 자리, 월의 두 자리, `-creationTimePattern` 해당 월의 두 자리)로 끝나는 경우 인수의 해당 값은 '히스토리값'yyymMdd'입니다.
...
Azure Blob과 `LightIngest` 함께 사용하면 특정 Blob 메타데이터 속성을 사용하여 수집 프로세스를 보강합니다.
