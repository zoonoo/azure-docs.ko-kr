...
1. **새 데이터 팩터리** 페이지에서 다음 필드에 대 한 값을 입력 한 다음 **만들기**를 선택 합니다.
    ...
    **설정**  | **필드 설명**
    ...
    | **버전(Version)** | **V2**를 선택합니다. |
    | **Location**: | 데이터 팩터리의 위치를 선택합니다. 지원되는 위치만 드롭다운 목록에 표시됩니다. Data factory에서 사용 하는 데이터 저장소는 다른 위치 또는 지역에 있을 수 있습니다. |
    | | |
...
1. **새 연결 된 서비스 (Amazon S3)** 페이지에서 다음 단계를 수행 합니다.
    ...
    * **마침**을 선택합니다.
    ...
    > [!NOTE]
    > Amazon S3의 탐색 모음에서 Amazon 사용자 이름을 선택한 다음 **내 보안 자격 증명** 을 선택 하 여 **액세스 키**를 찾습니다. 
