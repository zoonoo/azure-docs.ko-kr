<properties urlDisplayName="Command-Line Tools" pageTitle="Mac 및 Linux용 Azure 명령줄 도구" metaKeywords="Azure command-line, Azure tools Mac, Azure tools Linux" description="Learn about using the command-line tool for Mac and Linux in Azure." metaCanonical="" services="web-sites,virtual-machines,mobile-services,cloud-services" documentationCenter="" title="" authors="carolz" solutions="" manager="need to identify contact" editor="" />
...
Mac에서 이 도구를 설치하려면 [Azure SDK 설치 관리자](http://go.microsoft.com/fwlink/?LinkId=252249)를 다운로드하여 실행하세요.
...
**create-new**는 Blob 저장소에서 드라이브(즉, 다이어그램의 e:\)를 만들고, **attach**는 이미 만들어졌지만 연결되지 않은 디스크를 가상 컴퓨터에 연결합니다.

**vm create [옵션] <dns-이름> <이미지> <사용자> [암호]**
...
**-w** 가상 네트워크 이름 <br/>
**-l, --location** 위치를 지정합니다(예: "North Central US"). <br />
**-a, --affinity-group**은 선호도 그룹을 지정합니다.<br />
**-w, --virtual-network-name** 새 가상 컴퓨터를 추가할 가상 네트워크를 지정합니다. 가상 네트워크는 Azure 포털에서 설정하고 관리할 수 있습니다.<br />
**-b, --subnet-names** 가상 컴퓨터를 할당할 서브넷 이름을 지정합니다.

이 예에서 MSFT__Win2K8R2SP1-120514-1520-141205-01-ko-kr-30GB는 플랫폼에서 제공되는 이미지입니다. 운영 체제 이미지에 대한 자세한 내용은 vm 이미지 목록을 참조하세요.
...
가상 컴퓨터의 상태가 **중지됨**이 아닌 경우 가상 컴퓨터 이미지를 캡처할 수 없습니다.
...
**vm endpoint create-multiple [옵션] <vm 이름> <lb 포트>[:<vm 포트>[:<프로토콜>[:<lb 세트 이름>[:<prob 프로토콜>:<lb prob 포트>[:<prob 경로>]]]]] ]{1-*}**
...
이 명령은 가상 컴퓨터 이미지 목록을 가져옵니다. 세 가지 유형의 이미지가 있습니다. 즉, "MSFT" 접두사가 붙은 Microsoft에서 만든 이미지, 일반적으로 공급업체의 이름이 접두사로 붙는 타사에서 만든 이미지, 사용자가 만든 이미지가 있습니다. 이미지를 만들려면 기존 가상 컴퓨터를 캡처하거나 Blob 저장소에 업로드된 사용자 지정 .vhd에서 이미지를 만들 수 있습니다. 사용자 지정 .vhd를 사용하는 방법에 대한 자세한 내용은 vm image create를 참조하세요.
...
데이터 디스크는 가상 컴퓨터에서 사용할 수 있는 Blob 저장소에 있는 .vhd 파일입니다. 데이터 디스크를 Blob 저장소에 배포하는 방법에 대한 자세한 내용은 앞에 나오는 Azure 기술 다이어그램을 참조하세요. 
...
이 명령은 인증서를 업로드합니다. 암호로 보호되지 않는 인증서의 경우 암호 프롬프트를 비워 두세요.
...
**site deployment user set [옵션] [사용자 이름] [암호]**
...
+ **--job-type** <작업 유형>: 선택 사항입니다. 웹 작업 유형입니다. 유효한 값은 "triggered" 또는 "continuous"입니다. 기본적으로
모든 유형의 웹 작업을 반환합니다.
+ **--slot** <슬롯>: 다시 시작할 슬롯의 이름입니다.

**site job show [옵션] <작업 이름> <작업 유형> [이름]**

이 명령은 특정 웹 작업의 세부 정보를 보여 줍니다.

이 명령은 다음과 같은 추가 옵션을 지원합니다.

+ **--job-name** <작업 이름>: 필수입니다. 웹 작업의 이름입니다.
+ **--job-type** <작업 유형>: 필수입니다. 웹 작업 유형입니다. 유효한 값은 "triggered" 또는 "continuous"입니다.
+ **--slot** <슬롯>: 다시 시작할 슬롯의 이름입니다.

**site job delete [옵션] <작업 이름> <작업 유형> [이름]**
...
**site job upload [옵션] <작업 이름> <작업 유형> <작업 파일> [이름]**
...
+ **--job-name** <작업 이름>: 필수입니다. 웹 작업의 이름입니다.
+ **--job-type** <작업 유형>: 필수입니다. 웹 작업 유형입니다. 유효한 값은 "triggered" 또는 "continuous"입니다.
+ **--job-file** <작업 파일>: 필수입니다. 작업 파일입니다.
+ **--slot** <슬롯>: 다시 시작할 슬롯의 이름입니다.

**site job start [옵션] <작업 이름> <작업 유형> [이름]**
...
**site job stop [옵션] <작업 이름> <작업 유형> [이름]**
...
**site job history list [옵션] [작업 이름] [이름]**
...
**site job history show [옵션] [작업 이름] [실행 Id] [이름]**
...
+ **--job-name** <작업 이름>: 필수입니다. 웹 작업의 이름입니다.
+ **--run-id** <실행 id>: 선택 사항입니다. 실행 기록의 id입니다. 지정하지 않으면 가장 최근 실행을 표시합니다.
+ **--slot** <슬롯>: 다시 시작할 슬롯의 이름입니다.
...
**mobile create [옵션] [서비스 이름] [sql 관리 사용자 이름] [sql 관리 암호]**
...
+ **-l `<위치>`** 또는 **--location `<위치>`**: `<위치>`로 지정된 특정 위치에 서비스를 만듭니다. azure mobile locations를 실행하여 사용 가능한 위치를 가져옵니다.	
+ **--sqlLocation `<위치>`**: 특정 `<위치>`에 SQL Server를 만듭니다. 기본 위치는 모바일 서비스 위치입니다.
...
+ **-r `<쿼리>`** 또는 **--query `<쿼리>`**: 지정된 로그 쿼리를 실행합니다.
+ **-t `<유형>`** 또는 **--type `<유형>`**:  반환된 로그를 항목 `<유형>`, `information`, `warning` 또는 `error`별로 필터링합니다.
+ **-k `<건너뛰기>`** 또는 **--skip `<건너뛰기>`**: `<건너뛰기>`에 지정된 행 수를 건너뜁니다.
+ **-p `<맨 위>`** 또는 **--top `<맨 위>`**: `<맨 위>`로 지정된 특정 수의 행을 반환합니다.

<div class="dev-callout"><b>참고</b>
   <p><b>--query</b> 매개 변수가 <b>--type</b>, <b>--skip</b> 및 <b>--top</b>보다 우선합니다.</p>
</div>

**mobile recover [옵션] [비정상 서비스 이름] [정상 서비스 이름]**
...
+ **-p `<사용 권한>`** 또는 **--permissions `<사용 권한>`**: 쉼표로 분리된 `<작업>`=`<사용 권한>` 쌍 목록입니다. 여기서 `<작업>`은 `insert`, `read`, `update` 또는 `delete`이고 `<<사용 권한>`은 `public`, `application`(기본값), `user` 또는 `admin`입니다.
...
+ **-k `<건너뛰기>`** 또는 **--skip `<건너뛰기>`**: `<건너뛰기>`에 지정된 행 수를 건너뜁니다.
+ **-t `<맨 위>`** 또는 **--top `<맨 위>`**: `<맨 위>`로 지정된 특정 수의 행을 반환합니다.
...
+ **-p `<사용 권한>`** 또는 **--permissions `<사용 권한>`**: 쉼표로 분리된 `<작업>`=`<사용 권한>` 쌍 목록입니다. 여기서 `<작업>`은 `insert`, `read`, `update` 또는 `delete`이고 `<<사용 권한>`은 `public`, `application`(기본값), `user` 또는 `admin`입니다.
+ **--deleteColumn `<열>`**: 삭제할 열의 쉼표로 구분된 목록이며 `<열>`로 지정됩니다.
+ **-q** 또는 **--quiet**: 확인 메시지를 표시하지 않고 열을 삭제합니다.
+ **--addIndex `<열>`**: 인덱스에 포함할 열의 쉼표로 구분된 목록입니다.
+ **--deleteIndex `<열>`**: 인덱스에서 제외할 열의 쉼표로 구분된 목록입니다.
...
+ **-p `<경로>`** 또는 **--path `<경로>`**: 스크립트를 저장할 파일 내 위치입니다. 기본값은 현재 작업 디렉터리입니다.
+ **-f `<파일>`** 또는 **--file `<파일>`**: 스크립트를 저장할 파일의 이름입니다.
...
파일의 이름은 테이블 및 작업 이름으로부터 구성해야 하고, 명령이 실행되는 위치를 기준으로 테이블 하위 폴더에 위치해야 합니다. 또한 **-f `<파일>`** 또는 **--file `<파일>`** 매개 변수를 사용하여 등록할 스크립트를 포함하는 파일에 대한 다른 파일 이름 및 경로를 지정할 수 있습니다.
...
+ **-i `<번호>`** 또는 **--interval `<번호>`**: 작업 간격을 정수로 나타냅니다. 기본값은 `15`입니다.
+ **-u `<단위>`** 또는 **--intervalUnit `<단위>`**: _interval_의 단위이며 다음 값 중 하나입니다. 
	+ **minute**(기본값)
	+ **시간**
	+ **일**
	+ **월** 
	+ **none**(주문형 작업)
+ **-t `<시간>`** **--startTime `<시간>`** 스크립트에 대한 첫 번째 실행의 시작 시간을 ISO 형식으로 나타냅니다. 기본값은 `now`입니다.
...
+ **-u `<단위>`** 또는 **--intervalUnit `<단위>`**: _interval_의 단위이며 다음 값 중 하나입니다. 
	...
	+ **none**(주문형 작업)
+ **-t `<시간>`** **--startTime `<시간>`** 스크립트에 대한 첫 번째 실행의 시작 시간을 ISO 형식으로 나타냅니다. 기본값은 `now`입니다.
+ **-a `<상태>`** 또는 **--status `<상태>`**: 작업 상태를 나타내며 `enabled` 또는 `disabled`입니다.
...
+ **-c `<모드>`** or **--computeMode `<모드>`**: 계산 모드는 `Free` 또는 `Reserved`여야 합니다.
+ **-i `<개수>`** 또는 **--numberOfInstances `<개수>`**: reserved 모드에서 실행 중일 때 사용되는 인스턴스 수를 나타냅니다.
...
**mobile preview enable [옵션] [서비스 이름] [기능 이름]**

이 명령은 모바일 서비스에 대해 지정된 미리 보기 기능을 사용하도록 설정합니다. 모바일 서비스에 대해 한 번 사용하도록 설정되면 사용하지 않도록 설정할 수 없습니다.

###모바일 서비스 API를 관리하는 명령

**mobile api list [옵션] [서비스 이름]**
...
**mobile api create [옵션] [서비스 이름] [api 이름]**
...
**-p** 또는 **--permissions** <사용 권한>:  쉼표로 구분된 <메서드>=<사용 권한> 쌍 목록입니다.

**mobile api update [옵션] [서비스 이름] [api 이름]**
...
**mobile api delete [옵션] [서비스 이름] [api 이름]**
...
**mobile appsetting list [옵션] [서비스 이름]**
...
**mobile appsetting add [옵션] [서비스 이름] [이름] [값]**
...
**mobile appsetting delete [옵션] [서비스 이름] [이름]**
...
**mobile appsetting show [옵션] [서비스 이름] [이름]**
...
##<a name="Commands_to_manage_your_Storage_objects"></a>저장소 개체를 관리하는 명령

###저장소 계정을 관리하는 명령

**storage account list [옵션]**
...
**storage account show [옵션] <이름>**

이 명령은 URI 및 계정 속성을 포함하여 지정된 저장소 계정에 대한 정보를 표시합니다.

**storage account create [옵션] <이름>**
...
+ **-e** 또는 **--label** <레이블>: 저장소 계정에 대한 레이블입니다.
+ **-d** 또는 **--description** <설명>:  설명 저장소 계정입니다.
+ **-l** 또는 **--location** <이름>: 저장소 계정을 만들 지리적 영역입니다.
+ **-a** 또는 **--affinity-group** <이름>: 저장소 계정을 연결할 선호도 그룹입니다.
+ **--geoReplication**:  지역에서 복제가 사용하도록 설정되었는지를 나타냅니다.
+ **--disable-geoReplication**: 지역에서 복제가 사용하지 않도록 설정되었는지를 나타냅니다.

**storage account set [옵션] <이름>**
...
**storage account keys list [옵션] <이름>**

이 명령은 지정된 저장소 계정에 대한 기본 및 보조 키를 나열합니다.

**storage account keys renew [옵션] <이름>**

###저장소 컨테이너를 관리하는 명령

**storage container list [옵션] [접두사]**

이 명령은 지정된 저장소 계정에 대한 저장소 컨테이너 목록을 표시합니다. 저장소 계정은 연결 문자열이나 저장소 계정 이름 및 계정 키로 지정됩니다.

이 명령은 다음과 같은 추가 옵션을 지원합니다.

+ **-p** 또는 **-prefix** <접두사>: 저장소 컨테이너 이름 접두사입니다.
+ **-a** 또는 **--account-name** <계정 이름>: 저장소 계정 이름입니다.
+ **-k** 또는 **--account-key** <계정 키>: 저장소 계정 키입니다.
+ **-c** 또는 **--connection-string** <연결 문자열>: 저장소 연결 문자열입니다.
+ **--debug**: 디버그 모드에서 저장소 명령을 실행합니다.

**storage container show [옵션] [컨테이너]**
**storage container create [옵션] [컨테이너]**

이 명령은 지정된 저장소 계정에 대한 저장소 컨테이너를 만듭니다. 저장소 계정은 연결 문자열이나 저장소 계정 이름 및 계정 키로 지정됩니다.

이 명령은 다음과 같은 추가 옵션을 지원합니다.

+ **--container** <컨테이너>: 만들려는 저장소 컨테이너의 이름입니다.
+ **-p** 또는 **-prefix** <접두사>: 저장소 컨테이너 이름 접두사입니다.
+ **-a** 또는 **--account-name** <계정 이름>: 저장소 계정 이름입니다.
+ **-k** 또는 **--account-key** <계정 키>: 저장소 계정 키입니다.
+ **-c** 또는 **--connection-string** <연결 문자열>: 저장소 연결 문자열입니다.
...
**storage container delete [옵션] [컨테이너]**
...
**storage blob show [옵션] [컨테이너] [blob]**
...
**storage blob delete [옵션] [컨테이너] [blob]**

이 명령은 다음과 같은 추가 옵션을 지원합니다.

+ **--container** <컨테이너>: 만들려는 저장소 컨테이너의 이름입니다.
+ **-b** 또는 **--blob** <blob 이름>: 삭제할 Blob 저장소의 이름입니다.
...
**storage blob upload [옵션] [파일] [컨테이너] [blob]**
...
+ **--container** <컨테이너>: 만들려는 저장소 컨테이너의 이름입니다.
+ **-b** 또는 **--blob** <blob 이름>: 업로드할 Blob 저장소의 이름입니다.
+ **-t** 또는 **--blobtype** <blob 유형>: 저장소 Blob 유형: Page 또는 Block.
+ **-p** 또는 **--properties** <속성>: 업로드된 파일에 대한 저장소 Blob 속성입니다. 속성은 키=값 쌍이며 세미콜론 (;)으로 구분합니다. 사용 가능한 속성은 contentType, contentEncoding, contentLanguage, cacheControl입니다.
+ **-m** 또는 **--metadata** <메타데이터>: 업로드된 파일에 대한 저장소 Blob 메타데이터입니다. 메타데이터는 키=값 쌍이며 세미콜론 (;)으로 구분합니다.
+ **--concurrenttaskcount** <동시 작업 수>: 최대 동시 업로드 요청 수입니다.
...
**storage blob download [옵션] [컨테이너] [blob] [대상]**
...
+ **--container** <컨테이너>: 만들려는 저장소 컨테이너의 이름입니다.
+ **-b** 또는 **--blob** <blob 이름>: 저장소 Blob 이름입니다.
+ **-d** 또는 **--destination** [대상n]: 다운로드 대상 파일 또는 디렉터리 경로입니다.
...
**sql firewallrule create [옵션] <서버 이름> <규칙 이름> <시작 IP 주소> <끝 IP 주소>**
