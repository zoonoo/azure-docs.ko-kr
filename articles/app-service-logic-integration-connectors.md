...
BizTalk 플랫 파일 인코더 | **작업** API 앱입니다. 플랫 파일 데이터(예: Excel, csv)와 XML 데이터가 함께 작동하게 합니다(상호 운용성). 플랫 파일 인스턴스를 XML로 변환하며 그 반대로 변환할 수도 있습니다.
BizTalk JSON 인코더 | **작업** API 앱입니다. JSON과 XML 데이터가 함께 작동하게 합니다(상호 운용성). JSON 인스턴스를 XML로 변환하며 그 반대로 변환할 수도 있습니다.
BizTalk 규칙 | **작업** API 앱입니다. 비즈니스 논리 또는 "규칙"을 구현 및 적용하여 비즈니스 작업을 제어할 수 있습니다. 규칙은 동적이며 언제든지 변경될 수 있습니다. BizTalk 규칙은 사용자가 코드를 작성하지 않고 비즈니스 논리를 사용하도록 설정할 수 있게 하며, 응용 프로그램에 영향을 주지 않고 쉽게 업데이트할 수 있습니다.
BizTalk 변환 | **작업** API 앱입니다. 데이터를 한 형식에서 다른 형식으로 변환합니다. 다양한 기본 제공 함수를 사용하여 문자열 조작, 산술 식 완성, 날짜 및 시간 형식 지정 등을 수행할 수 있습니다. 
BizTalk XML 유효성 검사기 | **작업** API 앱입니다. 미리 정의된 XML 스키마와 비교하여 XML 데이터의 유효성을 검사합니다. 플랫 파일 인스턴스,   JSON 인스턴스 또는 기존 커넥터를 기준으로 스키마를 사용할 수 있습니다. 
BizTalk XPath 추출기 | **작업** API 앱입니다. 특정 XPath를 기준으로 XML 콘텐츠에서 데이터를 조회 및 추출합니다.
Wait |  입력한 기간 동안 또는 특정 시간까지 실행을 지연합니다. 논리 앱에 추가할 경우 전체 앱의 실행을 지연하는 데 사용할 수 있습니다.

> [AZURE.NOTE] 입력 xml에 특성(예: "<authorid= "1">abc</author>")을 가진 단순한 노드가 있는 경우 라이브러리의 JSON 출력은 { "author": "id : "1", "#text": "abc"}}입니다. "Id" 특성을 처리하기 위해 노드의 텍스트 콘텐츠에 대해 새로운 "#text" 키가 추가됩니다. 이러한 종류의 노드를 처리하려면 상수 키를 추가합니다. 이는 Newtonsoft.Json 라이브러리의 정상적인 동작입니다. 이 키를 SQL에 삽입하는 경우 "JSONOutput.Author.#text"를 사용합니다. "JsonOutput.Author"를 사용하지 마세요.
...
3. **갤러리**에서 BizTalk API 앱을 선택합니다. 그러면 자동으로 앱에 추가됩니다. 
