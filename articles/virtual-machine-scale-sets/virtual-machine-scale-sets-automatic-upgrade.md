...
- Azure Disk Encryption은 현재 가상 머신 확장 집합 자동 OS 업그레이드를 지원하지 **않습니다**.


## <a name="register-to-use-automatic-os-upgrade"></a>자동 OS 업그레이드를 사용하도록 등록
자동 OS 업그레이드 기능을 사용하려면 Azure Powershell 또는 Azure CLI 2.0을 사용하여 미리 보기 공급자를 등록합니다.

### <a name="powershell"></a>PowerShell

1. [Register-AzureRmProviderFeature](/powershell/module/azurerm.resources/register-azurermproviderfeature)를 사용하여 등록합니다.

     ```powershell
     Register-AzureRmProviderFeature -ProviderNamespace Microsoft.Compute -FeatureName AutoOSUpgradePreview
     ```

2. 등록 상태가 *등록됨*으로 보고될 때까지 약 10분 정도 소요됩니다. [Get-AzureRmProviderFeature](/powershell/module/AzureRM.Resources/Get-AzureRmProviderFeature) 명령을 사용하여 현재 등록 상태를 확인할 수 있습니다. 

3. 일단 등록되면 *Microsoft.Compute* 공급자가 등록되었는지 확인합니다. 다음 예제에서는 Azure Powershell에서 [Register-AzureRmResourceProvider](/powershell/module/AzureRM.Resources/Register-AzureRmResourceProvider)를 사용합니다.
...
1. [az feature register](/cli/azure/feature#az-feature-register)를 사용하여 등록합니다.

     ```azurecli
     az feature register --name AutoOSUpgradePreview --namespace Microsoft.Compute
     ```

2. 등록 상태가 *등록됨*으로 보고될 때까지 약 10분 정도 소요됩니다. [az feature show](/cli/azure/feature#az-feature-show) 명령을 사용하여 현재 등록 상태를 확인할 수 있습니다. 
 
3. 일단 등록되면 *Microsoft.Compute* 공급자가 등록되었는지 확인합니다. 다음 예제에서는 Azure CLI(2.0.20 이상)에서 [az provider register](/cli/azure/provider#az-provider-register)를 사용합니다.
...
> [!NOTE]
> Service Fabric 클러스터에는 응용 프로그램 상태에 대한 고유의 개념이 있지만 Service Fabric이 없는 확장 집합은 부하 분산 장치 상태 프로브를 사용하여 응용 프로그램 상태를 모니터링합니다. 
>
> ### <a name="azure-powershell"></a>Azure Powershell
>
> 1. [Register-AzureRmProviderFeature](/powershell/module/azurerm.resources/register-azurermproviderfeature)를 사용하여 상태 프로브에 대한 공급자 기능을 등록합니다.
>
>      ```powershell
>      Register-AzureRmProviderFeature -ProviderNamespace Microsoft.Network -FeatureName AllowVmssHealthProbe
>      ```
>
> 2. 마찬가지로, 등록 상태가 *등록됨*으로 보고될 때까지 약 10분 정도 소요됩니다. [Get-AzureRmProviderFeature](/powershell/module/AzureRM.Resources/Get-AzureRmProviderFeature)를 사용하여 현재 등록 상태를 확인할 수 있습니다.
>
> 3. 등록되었으면 다음과 같이 [Register-AzureRmResourceProvider](/powershell/module/AzureRM.Resources/Register-AzureRmResourceProvider)를 사용하여 *Microsoft.Network* 공급자를 등록합니다.
>
>      ```powershell
...
> ### <a name="cli-20"></a>CLI 2.0
>
> 1. [az feature register](/cli/azure/feature#az-feature-register)를 사용하여 상태 프로브에 대한 공급자 기능을 등록합니다.
>
>      ```azurecli
>      az feature register --name AllowVmssHealthProbe --namespace Microsoft.Network
>      ```
>
> 2. 마찬가지로, 등록 상태가 *등록됨*으로 보고될 때까지 약 10분 정도 소요됩니다. [az feature show](/cli/azure/feature#az-feature-show) 명령을 사용하여 현재 등록 상태를 확인할 수 있습니다. 
>
> 3. 등록되었으면 다음과 같이 [az provider register](/cli/azure/provider#az-provider-register)를 사용하여 *Microsoft.Network* 공급자를 등록합니다.
>
>      ```azurecli
...
| MicrosoftWindowsServer  | WindowsServer | 2016-Datacenter-with-Containers | 최신   |
...
### <a name="important-keep-credentials-up-to-date"></a>중요: 자격 증명을 최신 상태로 유지
확장 집합에서는 자격 증명을 사용하여 외부 리소스에 액세스하는 경우 자격 증명이 최신 상태인지 확인해야 합니다. 예를 들어, 저장소 계정에 대해 SAS 토큰을 사용하도록 VM 확장을 구성할 수 있습니다. 인증서 및 토큰을 포함한 자격 증명이 만료되는 경우 업그레이드에 실패하고 VM의 첫 번째 일괄 처리가 실패한 상태가 됩니다.

리소스 인증 오류 시 VM을 복구하고 자동 OS 업그레이드를 다시 설정하기 위한 권장 단계는 다음과 같습니다.

* 확장에 전달된 토큰(또는 다른 자격 증명)를 다시 생성합니다.
* 외부 엔터티와 소통하기 위해 VM 내에서 사용되는 자격 증명이 최신 상태인지 확인합니다.
...
확장 집합 상태에 대한 부하 분산 장치 프로브를 명시적으로 *만들어야* 합니다. 기존 HTTP 프로브 또는 TCP 프로브에 대한 동일한 끝점을 사용할 수 있지만, 상태 프로브에 기존의 부하 분산 장치 프로브와 다른 동작이 필요할 수 있습니다. 예를 들어, 인스턴스에 대한 부하가 너무 높으면 기존의 부하 분산 장치 프로브가 비정상 상태를 반환할 수 있습니다. 반대로, 이러한 프로브가 자동 OS 업그레이드 동안 인스턴스 상태를 확인하는 데는 적합하지 않을 수 있습니다. 검색 속도가 2분을 넘지 않도록 프로브를 구성하세요.
...
### <a name="cli-20"></a>CLI 2.0
기본 제공 Azure Resource Manager 정책을 사용하여 구독에 정책을 할당합니다.
...
확장 집합 OS 업그레이드 엔진은 모든 일괄 처리를 업그레이드하기 전에 전체 VM 인스턴스 상태를 검사합니다. 일괄 처리를 업그레이드하는 동안 Azure 데이터 센터에서 VM의 가용성에 영향을 줄 수 있는 다른 동시 계획된 유지 관리 또는 계획되지 않은 유지 관리가 발생할 수 있습니다. 따라서 20%가 넘는 인스턴스는 일시적으로 중지될 수 있습니다. 이 경우 현재 일괄 처리가 끝나면 확장 집합 업그레이드가 중지됩니다.
