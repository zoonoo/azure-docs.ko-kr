---
title: 가용성 영역을 사용하는 Azure 규모 집합 만들기
...
최대 확산을 사용하려면 *플랫폼FaultDomainCount를* *1로*설정합니다. 정적 5 장애 도메인 분산을 사용하려면 *platformFaultDomainCount*를 *5*로 설정합니다. API 버전 *2017-12-01에서* *platformFaultDomainCount기본값은* 단일 영역 및 교차 영역 배율 집합에 대해 *1로* 설정합니다. 현재 는 지역(비구역) 규모 집합에 대해 정적 5개의 오류 도메인 분산만 지원됩니다.

### <a name="placement-groups"></a>배치 그룹

확장 집합을 배포할 때 가용성 영역당 하나의 [배치 그룹](./virtual-machine-scale-sets-placement-groups.md)으로 배포하거나 영역마다 여러 개의 배치 그룹으로 배포하는 옵션도 있습니다. 지역(비구역) 축척 집합의 경우 선택한 방법은 지역에 단일 배치 그룹을 갖거나 지역에 여러 배치 그룹을 두는 것입니다. 대부분 워크로드의 경우 더 큰 규모가 가능하도록 여러 배치 그룹을 사용하는 것이 좋습니다. API 버전 *2017-12-01에서*배율은 기본적으로 단일 영역 및 교차 영역 축척 집합에 대해 여러 배치 그룹으로 설정하지만 기본적으로 지역(비구역) 축척 집합에 대한 단일 배치 그룹으로 설정됩니다.
...
최상의 영역 균형을 사용하면 확장 집합은 균형을 유지하면서 규모 확장 및 감축을 시도합니다. 그러나 어떤 이유로 이것이 불가능한 경우(예: 한 영역이 다운되면 배율 집합이 해당 영역에서 새 VM을 만들 수 없음) 배율 집합을 사용하면 임시 불균형이 성공적으로 확장 또는 축소될 수 있습니다. 이후 의 확장 을 시도할 때 배율 집합은 배율 집합을 균형을 위해 더 많은 VM이 필요한 영역에 VM을 추가합니다. 마찬가지로 후속 규모 축소 시도에서 확장 집합은 확장 집합이 균형을 이루기 위해 더 적은 VM이 필요한 영역에서 VM을 제거합니다. "엄격한 영역 균형"을 사용하면 확장 집합은 규모 확장 또는 축소 시도에 실패하고 그렇게 하는 경우 불균형이 발생합니다.

최상의 영역 균형을 사용하려면 *zoneBalance*를 *false*로 설정합니다. 이 설정은 API 버전 *2017-12-01*에서 기본값입니다. 엄격한 영역 균형을 사용하려면 *영역균형을* *true로*설정합니다.
...
- Azure CLI
- [Azure 파워쉘](#use-azure-powershell)
