...
Azure Analysis Services는 클라우드에서 엔터프라이즈급 데이터 모델링을 제공합니다. Azure 데이터 플랫폼 서비스와 긴밀하게 통합된 서비스로서 완벽하게 관리되는 플랫폼(PaaS)입니다. Analysis Services는 고도로 최적화된 OLAP 데이터 분석 엔진을 사용하여 크고, 복잡하며, 종종 서로 다른 데이터 원본 간에 풍부한 의미 체계 모델 계층을 제공합니다. 데이터를 통합하여 의미 체계 데이터 모델을 정의하고, 최신 클라이언트 도구에서 풍부한 대화형 분석 환경을 강력하게 하여 고수준 사용자 지정 기능과 뛰어난 성능을 갖춘 분석을 만들 수 있습니다.
...
Azure Analysis Services는 이미 알고 있는 것과 동일한 SQL Server Analysis Services Enterprise Edition과 호환되며, 1200 및 1400 호환성 수준의 테이블 형식 모델을 지원합니다. 파티션, 행 수준 보안, 양방향 관계 및 변환을 모두 지원합니다. 메모리 내 및 DirectQuery 모드는 복잡한 대용량 데이터 집합에 대해 매우 빠른 쿼리를 제공합니다.
...
1400 테이블 형식 모델의 새 기능에서는 세부 정보 행, 개체 수준 보안, 비정형 계층, 데이터 연결을 위한 SSDT의 최신 데이터 가져오기 환경 및 기타 향상된 여러 가지 기능을 지원합니다. 그리고 기본 모델 메타데이터가 동일하기 때문에 기존의 온-프레미스 테이블 형식 모델 솔루션을 클라우드로 쉽게 마이그레이션할 수 있습니다.


## <a name="better-with-azure"></a>Azure를 통해 더욱 향상된 환경
Azure Analysis Services는 다양한 Azure 데이터 서비스와 통합되어 정교한 분석 솔루션을 빌드할 수 있습니다.

Azure Analysis Services는 Azure SQL Database, Azure SQL Data Warehouse 및 Azure Blob 저장소의 데이터를 사용할 수 있습니다. Azure에서 허브 및 스포크(hub-and-spoke) 모델을 사용하여 센터에는 SQL 데이터 웨어하우스, 주변에는 다양한 비즈니스 그룹 또는 주제 영역을 대상으로 하는 여러 BI 모델이 포함된 엔터프라이즈 데이터 웨어하우스 솔루션을 구축할 수 있습니다.

Azure Data Factory를 사용하면 엔터프라이즈 BI/분석 솔루션의 핵심 기능인 데이터의 이동과 변환을 조정할 수 있습니다. Azure Analysis Services는 모델에 데이터를 로드하는 활동을 포함하여 Azure Data Factory 파이프라인에 통합될 수 있습니다. Azure Automation 및 Azure Functions도 사용자 지정 코드를 사용하여 모델의 간단한 오케스트레이션을 수행하는 데 사용할 수 있습니다.

Azure Analysis Services는 Azure Active Directory와 긴밀하게 통합되어 중요한 데이터에 대한 안전한 역할 기반 액세스를 제공합니다.
...
![계층 업그레이드](./media/analysis-services-overview/aas-overview-tier-up.png)

계층을 즉시 변경하려면 Azure Portal 또는 Set-AzureRmAnalysisServicesServer PowerShell cmdlet을 사용합니다. 다양한 계획 및 계층에 대해 자세히 알아보고 가격 계산기를 사용하여 올바른 계획을 결정하려면 [Azure Analysis Services 가격](https://azure.microsoft.com/pricing/details/analysis-services/)을 참조하세요.

## <a name="scale-resources"></a>리소스 크기 조정
서버를 강화, 축소 또는 일시 중지합니다. Azure Portal을 사용하거나 즉시로 완전히 제어하려면 PowerShell을 사용합니다. 사용한 양만큼만 요금을 지급합니다.

새 서버를 만드는 경우 [New-AzureRmAnalysisServicesServer](https://docs.microsoft.com/en-us/powershell/module/azurerm.analysisservices/new-azurermanalysisservicesserver) cmdlet을 사용하여 계획을 설정합니다. 기존 서버의 경우 [Set-AzureRmAnalysisServicesServer](https://docs.microsoft.com/en-us/powershell/module/azurerm.analysisservices/set-azurermanalysisservicesserver) cmdlet을 사용하여 계획을 변경합니다. 서비스를 항상 사용하지 않습니까? 그러면 포털을 사용하거나 [Suspend-AzureRmAnalysisServicesServer](https://docs.microsoft.com/en-us/powershell/module/azurerm.analysisservices/suspend-azurermanalysisservicesserver) cmdlet을 사용하여 일시 중단할 수 있습니다. [Resume-AzureRmAnalysisServicesServer](https://docs.microsoft.com/en-us/powershell/module/azurerm.analysisservices/resume-azurermanalysisservicesserver) cmdlet을 사용하여 다시 시작합니다. 서버가 활성화된 경우에만 요금을 지불합니다.
...
| 아메리카 | 유럽 | 아시아 태평양 |
|----------|--------|--------------|
|  브라질 남부<br> 캐나다 중부<br> 미국 동부 2<br> 미국 중북부<br> 미국 중남부<br> 미국 중서부<br> 미국 서부 | 북유럽<br> 영국 남부<br> 서유럽 |   오스트레일리아 남서부<br> 일본 동부<br> 동남아시아<br> 인도 서부  |
...
Azure Portal을 사용하면 수분 내에 [서버를 만들 수 있습니다](analysis-services-create-server.md). 그리고 Azure Resource Manager 템플릿과 PowerShell을 사용하면 선언적 템플릿을 통해 서버를 프로비전할 수 있습니다. 단일 템플릿을 사용하면 저장소 계정과 같은 다른 Azure 구성 요소와 함께 여러 서비스를 배포할 수 있습니다.  자세한 내용은 [Resource Manager 템플릿과 Azure PowerShell로 리소스 배포](../azure-resource-manager/resource-group-template-deploy.md)를 참조하세요.

서버를 만들었으면 Azure Portal에서 테이블 형식 모델을 바로 만들 수 있습니다. 새로운 웹 디자이너(미리 보기) 기능을 사용하면 Azure SQL Database, Azure SQL Data Warehouse 데이터 원본에 연결하거나 .pbix Power BI Desktop 파일을 가져올 수 있습니다. 테이블 간의 관계를 자동으로 만들므로 브라우저에서 직접 측정값을 만들거나 json 형식의 model.bim 파일을 편집할 수 있습니다.

## <a name="migrate-existing-tabular-models"></a>기존 테이블 형식 모델 마이그레이션
...
Azure Analysis Services는 조직의 온-프레미스 데이터 원본 및 클라우드의 데이터 원본에 대한 연결을 지원합니다. 하이브리드 솔루션을 위해 온-프레미스 및 클라우드 데이터 원본의 데이터를 모두 결합합니다. 
...
Azure Analysis Services는 Azure SQL Database, Azure SQL Data Warehouse, SQL Server, SQL Server Data Warehouse, Oracle 및 Teradata 관계형 데이터베이스에 직접 연결하는 데 [DirectQuery](https://docs.microsoft.com/sql/analysis-services/tabular-models/directquery-mode-ssas-tabular)를 사용하도록 지원합니다.

자세한 내용은 [Azure Analysis Services에서 지원되는 데이터 원본](analysis-services-datasource.md)을 참조하세요.
...
새로운 SSDT 및 1400 테이블 형식 모델을 사용하면 더 이상 Analysis Services의 로컬 인스턴스를 설치하여 작업 영역 데이터베이스를 호스팅할 필요가 없습니다. SSDT에는 이제 자체 통합된 Analysis Services 엔진과 데이터베이스가 포함되어 있습니다. 준비가 되면 SSDT에서 Azure의 서버로 바로 배포합니다. 그리고 SSDT는 매월 업데이트되므로 최신 기능을 빨리 사용할 수 있습니다.
...
Power BI, Excel 및 기타 타사 도구와 같은 최신 데이터 탐색 및 시각화 도구는 모델 데이터에 대해 시각적으로 풍부한 대화형 정보를 최종 사용자에게 제공합니다.
