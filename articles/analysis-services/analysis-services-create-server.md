...
2. **Analysis Services**에서 필수 필드를 입력한 다음, **만들기**를 누릅니다.
   ...
   * **백업 스토리지 설정**: (선택 사항) [스토리지 계정](../storage/common/storage-introduction.md)이 이미 있는 경우 모델 데이터베이스 백업에 대한 기본값으로 지정할 수 있습니다. [백업 및 복원](analysis-services-backup.md) 설정을 나중에 지정할 수도 있습니다.
   * **스토리지 키 만료**: (선택 사항) 스토리지 키 만료 시간을 지정합니다.
