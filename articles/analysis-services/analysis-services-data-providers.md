...
1.  `C:\Program Files\Microsoft Analysis Services\AS OLEDB\` 으로 이동합니다. 둘 이상의 폴더가 있는 경우 더 높은 숫자를 선택합니다.
  
2.  **Msolap** > **Properties**속성 > **세부 정보**를 마우스 오른쪽 단추로 클릭 합니다. 파일 이름이 msolap140.dll이면 최신 버전보다 오래된 것이므로 업그레이드해야 합니다.
...
1. `C:\Windows\Microsoft.NET\assembly\GAC_MSIL\Microsoft.AnalysisServices\` 으로 이동합니다. 둘 이상의 폴더가 있는 경우 더 높은 숫자를 선택합니다.
2. **Microsoft.analysisservices.sharepoint.integration.dll** > **Properties**속성 > **세부 정보**를 마우스 오른쪽 단추로 클릭 합니다.  

### <a name="adomd"></a>ADOMD

1. `C:\Windows\Microsoft.NET\assembly\GAC_MSIL\Microsoft.AnalysisServices.AdomdClient\` 으로 이동합니다. 둘 이상의 폴더가 있는 경우 더 높은 숫자를 선택합니다.
2. **Microsoft.analysisservices.sharepoint.integration.dll. microsoft.analysisservices.adomdclient.cellset<** > **속성** > **세부 정보**를 마우스 오른쪽 단추로 클릭 합니다.  
