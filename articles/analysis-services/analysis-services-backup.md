...
백업은 .abf 확장명으로 저장됩니다. 메모리 내 테이블 형식 모델의 경우 모델 데이터와 메타데이터가 모두 저장됩니다. 직접 쿼리 테이블 형식 모델의 경우 모델 메타데이터만 저장됩니다. 백업은 선택한 옵션에 따라 압축하고 암호화할 수 있습니다. 
...
5. 백업 설정을 저장합니다. 저장소 설정을 변경하거나 백업을 사용하거나 사용하지 않도록 설정할 때마다 변경 내용을 저장해야 합니다.
...
3. **다른 이름으로 파일 저장** 대화 상자에서 폴더 경로 확인한 다음 백업 파일의 이름을 입력합니다. 기본적으로 파일 이름에는 .abf 확장명이 지정됩니다. 
...
> [!NOTE]
> 온-프레미스 SQL Server Analysis Services 서버에서 테이블 형식의 모델 데이터베이스를 복원하는 경우 먼저 모델 역할에서 모든 도메인 사용자를 제거하고 Azure Active Directory 사용자로 역할에 다시 추가해야 합니다. 역할은 동일합니다.
> 
> 
