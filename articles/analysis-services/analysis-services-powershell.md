...
description: 서버 만들기, 작업 일시 중단 또는 서비스 수준 변경과 같은 일반적인 관리 작업에 대한 Azure 분석 서비스 PowerShell cmdlet에 대해 설명합니다.
author: minewiskan
...
서버 생성 또는 삭제, 서버 작업 일시 중단 또는 다시 시작, 서비스 수준 변경(계층) 변경과 같은 서버 리소스 관리 작업은 Azure Analysis Services cmdlet을 사용합니다. 역할 멤버 추가 또는 제거, 처리 또는 분할과 같은 기타 데이터베이스 관리 작업에서는 SQL Server Analysis Services와 동일한 SqlServer 모듈에 포함된 cmdlet을 사용합니다.
...
Azure PowerShell cmdlet을 사용하는 서버 작업의 경우 계정 또는 실행 스케줄러도 [RBAC(Azure 역할 기반 액세스 제어)의](../role-based-access-control/overview.md)리소스에 대한 소유자 역할에 속해야 합니다. 
...
문서 - [Az.AnalysisServices 참조](/powershell/module/az.analysisservices)
