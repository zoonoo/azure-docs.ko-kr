...
Azure SQL Database Managed Instance 데이터 원본은 개인 IP 주소를 사용하여 Azure VNet 내에서 실행됩니다. 인스턴스에서 퍼블릭 엔드포인트를 사용하도록 설정한 경우에는 게이트웨이가 필요하지 않습니다. 퍼블릭 엔드포인트가 활성화되지 않은 경우 온-프레미스 데이터 게이트웨이가 필요하고 AlwaysUseGateway 속성을 true로 설정해야 합니다.
