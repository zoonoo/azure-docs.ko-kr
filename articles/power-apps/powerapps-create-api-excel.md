<properties
	...
	description="조직의 앱 서비스 환경에서 새 Excel API 만들기 또는 구성"
	services=""
    suite="powerapps"
	documentationCenter="" 
...
# PowerApps 엔터프라이즈에 새 Excel API 만들기

조직의 (테넌트) 앱 서비스 환경에 Excel API를 추가합니다.
...
7. **원본**에서 **사용 가능한 API**를 선택하여 이미 만들어진 API를 선택한 다음 **Excel**을 선택합니다. ![Excel API를 선택합니다.][3]

8. **확인**을 선택하여 이 단계를 완료합니다.

완료하면 새 Excel API가 앱 서비스 환경에 추가됩니다.

## 요약 및 다음 단계
이 항목에서는 PowersApps 엔터프라이즈에 Excel API를 추가했습니다. 그 다음에는 이 API를 사용자의 앱에 추가할 수 있도록 사용자에게 이 API에 대한 액세스 권한을 부여합니다.
