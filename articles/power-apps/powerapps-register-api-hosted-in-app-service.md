...
앱 서비스 환경에서 API를 호스트하면 [앱 서비스 환경](../app-service-web/app-service-app-service-environment-intro.md)에 있는 기존의 모든 기능을 활용할 뿐만 아니라 더 나은 통합 환경을 사용할 수 있습니다.

앱에서 이러한 API를 사용하려면 관리되는 API 또는 앱 서비스 환경의 기존 API를 사용하거나, Swagger로 API를 만들어 Azure 포털에 API를 "등록"해야 합니다.
...
이 항목에서는 두 번째 옵션인 **앱 서비스 환경 내에서 호스트되는 웹앱, API 앱 및 모바일 앱 등록**에 대해 중점적으로 설명합니다.
...
Visual Studio에서 배포 및 소스 제어 시스템을 사용하여 지속적으로 배포하는 옵션뿐만 아니라 앱 서비스 환경으로 웹 API를 배포하는 옵션도 있습니다. [Azure 앱 서비스에서 웹앱 배포](../app-service-web/web-sites-deploy.md)는 좋은 리소스입니다.
