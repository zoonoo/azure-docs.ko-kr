...
# 조직의 앱 서비스 환경에서 새 Google 드라이브 API 만들기
...
10. 클라이언트 ID 만들기 페이지에서:
	...
	a) **애플리케이션 유형**에서 **웹 애플리케이션**을 선택합니다. b) 클라이언트의 이름을 입력합니다. c) 리디렉션 URL을 이 항목의 Azure 포털에서 새 Google 드라이브 API를 추가했을 때 받은 리디렉션 URL로 설정합니다. d) **생성**을 선택합니다.
