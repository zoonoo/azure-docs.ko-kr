---
title: 논리 흐름 및 PowerApps에서 사용자 지정 API를 사용하는 방법 | Microsoft Docs
...
> [!IMPORTANT]
> 이 항목은 [사용자 지정 API란](https://powerapps.microsoft.com/tutorials/register-custom-api/)의 powerapps.microsoft.com으로 이동되었습니다. 최신 버전에 대한 PowerApps로 이동하세요. 이 Azure 링크가 보관됩니다.
> 
> 
...
> [!TIP]
> .json 파일에서 의견을 추가할 수 없습니다. 모든 텍스트는 데이터의 일부로 간주됩니다.
> 
> 
...
* 게시되어 공개적으로 사용할 수 있는 모든 API. 예로는 [Spotify][2], [Uber][3], [Slack][4], [Rackspace][5] 등이 있습니다.
...
1. PowerApps [웹 포털][9]로 이동하여 회사 계정으로 로그인합니다.
   ...
   > [!NOTE]
   > 현재 사용자 지정 API는 PowerApps 웹 포털에서만 사용할 수 있습니다. PowerApps 클라이언트에서는 사용할 수 없습니다.
   > 
   > 
...
> [!TIP]
> Swagger 파일이 유효성 검사에 실패한 경우 추가 문자가 있을 수 있습니다. 예를 들어 웹 사이트를 포함하여 거의 모든 데이터는 따옴표로 묶어야 합니다. 따라서 `https://mywebapi.mywebsite.com`이 따옴표 밖에 있는 경우 파일은 유효성 검사에 실패합니다.
> 
> 
...
> [!NOTE]
> 사용자의 조직에 있는 다른 사용자와만 사용자 지정 API를 공유할 수 있습니다.
> 
> 
