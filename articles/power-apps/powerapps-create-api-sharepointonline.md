<properties
	pageTitle="PowerApps 엔터프라이즈에 SharePoint Online API 추가 | Microsoft Azure"
	description="조직의 앱 서비스 환경에서 새 SharePoint Online API 만들기 또는 구성"
	services=""
    suite="powerapps"
	documentationCenter="" 
...
# PowerApps 엔터프라이즈에서 새 SharePoint Online API 만들기

조직의 (테넌트) 앱 서비스 환경에 SharePoint Online API를 추가합니다.
...
7. **원본**에서 **사용 가능한 API**를 선택하여 이미 만들어진 API를 선택한 다음 **SharePoint Online**을 선택합니다. ![SharePoint Online API 선택][3]

8. **설정 - 필수 설정 구성**을 선택합니다. ![SharePoint Online API 설정 구성][4]

9. SharePoint Online AAD(Azure Active Directory) 응용 프로그램의 *클라이언트 ID*와 *앱 암호*를 입력합니다. 없는 경우 이 항목의 "PowerApps에서 사용할 AAD 앱 등록" 섹션을 참조하여 필요한 ID와 암호 값을 만듭니다.

	> [AZURE.IMPORTANT] **리디렉션 URL**을 저장합니다. 이 항목 뒷부분에서 이 값이 필요할 수 있습니다.

10. **확인**을 선택하여 이 단계를 완료합니다.

완료하면 새 SharePoint Online API가 앱 서비스 환경에 추가됩니다.


## PowerApps SharePoint Online API에서 사용할 AAD 앱 등록
...
8. _OAuth 2_ 섹션의 **회신 URL**을 이 항목의 Azure 포털에서 새 SharePont Online API를 추가했을 때 받은 리디렉션 URL로 설정합니다. **응용 프로그램 추가**를 선택합니다. ![Contoso AAD 앱 구성][11]
...
11. _Office 365 Exchange Online_에 대해 **위임된 권한**을 선택하고 다음과 같은 권한을 선택합니다.
	...
	- 모든 사이트 모음의 항목 읽기 및 쓰기
	- 모든 사이트 모음의 항목과 목록 읽기 및 쓰기

	![Contoso 앱 대리자 권한][13]

새 Azure Active Directory 앱이 만들어졌습니다. 이 앱을 Azure 포털의 SharePoint Online API 구성에서 사용할 수 있습니다.

## 요약 및 다음 단계
이 항목에서는 PowersApps 엔터프라이즈에 SharePoint Online API를 추가했습니다. 그 다음에는 이 API를 사용자의 앱에 추가할 수 있도록 사용자에게 이 API에 대한 액세스 권한을 부여합니다.
