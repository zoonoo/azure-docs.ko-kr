...
- **백 엔드 연결 시간**
  ...
  백 엔드 응용 프로그램과의 연결을 설정 하는 데 걸린 시간입니다. 여기에는 네트워크 대기 시간 뿐만 아니라 백 엔드 서버의 TCP 스택에서 새 연결을 설정 하는 데 걸리는 시간이 포함 됩니다. SSL의 경우 핸드셰이크에 소요 된 시간도 포함 됩니다. 

- **백 엔드 첫 번째 바이트 응답 시간**

  백 엔드 서버에 대 한 연결을 설정 하 고 응답 헤더의 첫 번째 바이트를 수신 하는 시작 사이의 시간 간격입니다. 이는 백 엔드 응용 프로그램의 *백 엔드 연결 시간* 및 응답 시간 합계 (서버가 콘텐츠를 생성 하 고 잠재적으로 데이터베이스 쿼리를 인출 하 고 응답을 다시 Application Gateway으로 전송 하기 시작 하는 데 걸린 시간)의 합계와 비슷합니다.

- **백 엔드 마지막 바이트 응답 시간**

  백 엔드 서버에 대 한 연결을 설정 하 고 응답 본문의 마지막 바이트를 받는 시작 사이의 시간 간격입니다. 이는 *백 엔드 첫 번째 바이트 응답 시간* 및 데이터 전송 시간 합계를 대략적으로 계산 합니다 .이 수는 요청 된 개체의 크기와 서버 네트워크의 대기 시간에 따라 크게 다를 수 있습니다.

- **Application gateway 총 시간**

  요청을 처리 하는 데 걸리는 평균 시간 및 응답을 보내는 데 걸리는 시간입니다. 이 값은 응답 전송 작업이 완료 된 시간에 대 한 HTTP 요청의 첫 번째 바이트를 Application Gateway 수신 하는 시간에 대 한 평균으로 계산 되어 Application Gateway 처리 시간 및 *백 엔드 마지막 바이트 응답 시간* 합계를 대략적으로 계산 합니다.

- **클라이언트 RTT**

  클라이언트와 Application Gateway 사이의 평균 왕복 시간입니다. 이 메트릭은 연결을 설정 하 고 승인을 반환 하는 데 걸리는 시간을 나타냅니다. 

이러한 메트릭은 관찰 된 속도가 Application Gateway, 네트워크 및 백 엔드 서버 TCP 스택 포화, 백 엔드 응용 프로그램 성능 또는 큰 파일 크기 때문 인지 여부를 확인 하는 데 사용할 수 있습니다.
예를 들어 백 엔드의 첫 번째 바이트 응답 시간이 급증 하지만 백 엔드 연결 시간이 일정 한 경우에는 응용 프로그램 게이트웨이가 백 엔드 대기 시간으로, 연결을 설정 하는 데 걸리는 시간이 안정적이 고, 다음과 같은 이유로 인해 스파이크가 발생 한다는 것을 유추할 수 있습니다. n 백 엔드 응용 프로그램의 응답 시간이 증가 합니다. 마찬가지로 백 엔드의 스파이크가 응답 시간이 백 엔드 연결 시간에 해당 스파이크와 연결 된 경우 네트워크 또는 서버 TCP 스택의 포화 상태를 추론할 수 있습니다. 백 엔드의 마지막 바이트 응답 시간이 급증 하지만 백 엔드 첫 바이트 응답 시간이 일정 한 경우에는 더 큰 파일이 요청 되기 때문에 스파이크가 발생할 수 있습니다. 마찬가지로, 응용 프로그램 게이트웨이 총 시간이 백 엔드 마지막 바이트 응답 시간 보다 훨씬 많은 경우 Application Gateway에서 성능 병목 현상이 발생할 수 있습니다.
