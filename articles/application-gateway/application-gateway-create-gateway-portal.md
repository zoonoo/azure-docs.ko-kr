...
Azure 응용 프로그램 게이트웨이에는 자체 서브넷이 필요합니다. 가상 네트워크를 만들 때 여러 서브넷을 둘 수 있는 충분한 주소 공간이 있는지 확인합니다. 응용 프로그램 게이트웨이를 서브넷에만 배포한 경우 추가 응용 프로그램 게이트웨이를 서브넷에 추가할 수 있습니다.
...
- **SKU 크기** - 응용 프로그램 게이트웨이의 크기입니다. 사용 가능한 옵션은 소형, 중형 및 대형입니다.
- **인스턴스 수** - 인스턴스 수입니다. 이 값은 2에서 10 사이의 숫자여야 합니다.
- **리소스 그룹** - 응용 프로그램 게이트웨이를 유지할 리소스 그룹입니다. 기존 리소스 그룹 또는 새 리소스 그룹일 수 있습니다.
- **위치** - 응용 프로그램 게이트웨이의 지역입니다. 리소스 그룹에서의 위치와 같은 위치입니다. *이는 가상 네트워크와 공용 IP가 게이트웨이와 동일한 위치에 있어야 하므로 중요합니다*.
...
기본 설정을 정의한 후에는 사용할 가상 네트워크를 정의합니다. 가상 네트워크는 응용 프로그램 게이트웨이에서 부하를 분산하는 응용 프로그램을 수용합니다.
...
응용 프로그램 게이트웨이를 만들 때 구성할 마지막 설정은 수신기 구성입니다. **http**가 사용되는 경우 구성할 항목이 더 이상 없으므로 **확인**을 클릭하면 됩니다. **https**를 활용하려면 추가 구성이 필요합니다.
...
여기서는 수신기, 백 엔드 풀, 백 엔드 http 설정 및 규칙에 대한 기본 설정으로 기본 응용 프로그램 게이트웨이를 만듭니다. 프로비전에 성공하면 배포에 맞게 이러한 설정을 수정할 수 있습니다.
