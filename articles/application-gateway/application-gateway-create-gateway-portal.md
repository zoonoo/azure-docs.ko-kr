...
>[AZURE.IMPORTANT] 초기 배포 중이 아닌 경우 응용 프로그램 게이트웨이를 구성하면 사용자 지정 상태 프로브, 백 엔드 풀 주소, 추가 규칙 등 응용 프로그램 게이트웨이에 대한 추가 구성이 구성됩니다.
...
이러한 단계에서는 수신기, 백 엔드 풀, 백 엔드 http 설정 및 규칙에 대한 기본 설정으로 기본 Application Gateway를 만듭니다. 프로비전에 성공하면 배포에 맞게 이러한 설정을 수정할 수 있습니다. 앞에서 설명한 대로 이 프로세스는 기본 응용 프로그램 게이트웨이 만듭니다.  다음 단계에서는 풀 멤버를 추가하고, 설정을 수정하고, 제대로 작동하도록 게이트웨이의 규칙을 조정하여 응용 프로그램 게이트웨이를 구성합니다.
