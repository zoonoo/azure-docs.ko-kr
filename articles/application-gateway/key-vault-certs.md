...
1. **주요 자격 증명 모음 구성**
   ...
   > [!IMPORTANT]
   > 현재 Application Gateway에는 통합을 활용 하기 위해 모든 네트워크에서 액세스를 허용 하는 Key Vault가 필요 합니다. Key Vault가 전용 엔드포인트만 허용 하 고 네트워크 액세스를 선택 하는 경우 Key Vault 통합을 지원 하지 않습니다. 개인 및 선택 네트워크에 대 한 지원은 Application Gateway와 Key Vault의 전체 통합에 적합 합니다. 
