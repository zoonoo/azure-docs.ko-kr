---
title: PowerShell을 사용 하 여 SSL 종료
...
Azure PowerShell을 사용하여 백엔드 서버에 [가상 머신 확장 집합](overview.md)을 사용하는 [SSL 종료](ssl-overview.md)용 인증서가 있는 [애플리케이션 게이트웨이](../virtual-machine-scale-sets/virtual-machine-scale-sets-overview.md)를 만들 수 있습니다. 이 예제에서 확장 집합은 애플리케이션 게이트웨이의 기본 백 엔드 풀에 추가되는 두 개의 가상 머신 인스턴스를 포함합니다. 
...
리소스 그룹은 Azure 리소스가 배포 및 관리되는 논리적 컨테이너입니다. *New-AzResourceGroup*으로 [myResourceGroupAG](/powershell/module/az.resources/new-azresourcegroup)라는 Azure 리소스 그룹을 만듭니다. 
