---
title: Azure 응용 프로그램 게이트웨이를 사용하여 종단 간 TLS 구성
description: 이 문서에서는 PowerShell을 사용하여 Azure 응용 프로그램 게이트웨이를 사용하여 종단 간 TLS를 구성하는 방법에 대해 설명합니다.
...
ms.locfileid: "81312505"
---
# <a name="configure-end-to-end-tls-by-using-application-gateway-with-powershell"></a>PowerShell을 사용하여 응용 프로그램 게이트웨이를 사용하여 종단 간 TLS 구성

## <a name="overview"></a>개요

Azure Application Gateway는 트래픽의 엔드투엔드 암호화를 지원합니다. 응용 프로그램 게이트웨이는 응용 프로그램 게이트웨이에서 TLS/SSL 연결을 종료합니다. 그러면 게이트웨이에서 트래픽에 라우팅 규칙을 적용하고, 패킷을 다시 암호화하고, 정의된 라우팅 규칙에 따라 적절한 백 엔드 서버에 패킷을 전달합니다. 웹 서버의 모든 응답은 동일한 프로세스를 거쳐 최종 사용자에게 돌아갑니다.

애플리케이션 게이트웨이는 사용자 지정 TLS 옵션 정의를 지원합니다. 또한 **TLSv1.0**, **TLSv1.1** 및 **TLSv1.2**와 같은 프로토콜 버전을 사용하지 않고 사용할 암호 그룹 및 기본 설정의 순서를 정의하도록 지원합니다. 구성 가능한 TLS 옵션에 대한 자세한 내용은 [TLS 정책 개요를](application-gateway-SSL-policy-overview.md)참조하십시오.
...
이 시나리오에서는 PowerShell을 사용하여 종단 간 TLS를 사용하여 응용 프로그램 게이트웨이를 만드는 방법을 배웁니다.
...
* TLS 프로토콜 버전 및 암호 제품군을 제한하는 종단 간 TLS 암호화를 지원하는 작은 응용 프로그램 게이트웨이를 만듭니다.

## <a name="before-you-begin"></a>시작하기 전에

[!INCLUDE [updated-for-az](../../includes/updated-for-az.md)]

응용 프로그램 게이트웨이로 종단 간 TLS를 구성하려면 게이트웨이에 대한 인증서가 필요하고 백 엔드 서버에 대한 인증서가 필요합니다. 게이트웨이 인증서는 TLS 프로토콜 사양에 따라 대칭 키를 파생하는 데 사용됩니다. 이렇게 파생된 대칭 키는 게이트웨이로 전송되는 트래픽을 암호화하고 암호를 해독하는 데 사용됩니다. 게이트웨이 인증서는 개인 정보 교환(PFX) 형식이어야 합니다. 이 파일 형식을 사용하면 애플리케이션 게이트웨이에서 트래픽의 암호화 및 암호 해독을 수행하는 데 필요한 프라이빗 키를 내보낼 수 있습니다.

종단 간 TLS 암호화의 경우 백 엔드는 응용 프로그램 게이트웨이에서 명시적으로 허용되어야 합니다. 백 엔드 서버의 공용 인증서를 Application Gateway에 업로드합니다. 인증서를 추가하면 Application Gateway가 알려진 백 엔드 인스턴스하고만 통신하게 됩니다. 그러면 엔드투엔드 통신의 보안이 유지됩니다.
...
5. Application Gateway에 대한 인증서를 구성합니다. 이 인증서는 Application Gateway의 트래픽을 다시 암호화하고 암호 해독하는 데 사용됩니다.
   ...
   > [!NOTE]
   > 이 샘플은 TLS 연결에 사용되는 인증서를 구성합니다. 인증서는 .pfx 형식으로 4~12자의 암호여야 합니다.

6. Application Gateway에 대한 HTTP 수신기를 만듭니다. 사용할 프런트 엔드 IP 구성, 포트 및 TLS/SSL 인증서를 할당합니다.

   ```powershell
   $listener = New-AzApplicationGatewayHttpListener -Name listener01 -Protocol Https -FrontendIPConfiguration $fipconfig -FrontendPort $fp -SSLCertificate $cert
   ```

7. TLS 지원 백 엔드 풀 리소스에 사용할 인증서를 업로드합니다.

   > [!NOTE]
   > 기본 프로브는 백 엔드의 IP 주소에 바인딩되는 기본 TLS 바인딩에서 *공개* 키를 얻고 수신하는 공개 키 값을 여기에서 제공하는 공개 키 값과 비교합니다. 
   > 
   > 백 엔드에서 호스트 헤더와 SNI(서버 이름 표시)를 사용하는 경우 검색된 공개 키는 트래픽이 흐르는 의도된 사이트가 아닐 수 있습니다. 의심스러운 경우 백 엔드 https://127.0.0.1/ 서버를 방문하여 *기본* TLS 바인딩에 사용되는 인증서를 확인합니다. 이 섹션에서는 해당 요청에서 공개 키를 사용합니다. HTTPS 바인딩에서 호스트 헤더 와 SNI를 사용하고 있고 수동 브라우저 요청에서 https://127.0.0.1/ 백 엔드 서버에 대한 응답 및 인증서를 받지 못하는 경우 이에 대한 기본 TLS 바인딩을 설정해야 합니다. 그렇게 하지 않으면 프로브가 실패하고 백 엔드가 허용 목록에 추가되지 않습니다.
   ...
   > [!NOTE]
   > 이전 단계에서 제공된 인증서는 백 엔드에 있는 .pfx 인증서의 공개 키여야 합니다. 백 엔드 서버에 설치된 인증서(루트 인증서 제외)를 CER(Claim, Evidence, Reasoning) 형식으로 내보내고 이 단계에서 사용합니다. 이 단계에서는 Application Gateway를 통해 백 엔드를 허용 목록에 추가합니다.

   Application Gateway v2 SKU를 사용하는 경우 인증 인증서 대신 신뢰할 수 있는 루트 인증서를 만듭니다. 자세한 내용은 [응용 프로그램 게이트웨이를 사용하여 종단 간 TLS 개요를](ssl-overview.md#end-to-end-tls-with-the-v2-sku)참조하십시오.
...
11. 응용 프로그램 게이트웨이에서 사용할 TLS 정책을 구성합니다. 응용 프로그램 게이트웨이는 TLS 프로토콜 버전에 대한 최소 버전을 설정하는 기능을 지원합니다.
...
V1 SKU의 경우 아래 명령을 사용합니다.
```powershell
$appgw = New-AzApplicationGateway -Name appgateway -SSLCertificates $cert -ResourceGroupName "appgw-rg" -Location "West US" -BackendAddressPools $pool -BackendHttpSettingsCollection $poolSetting -FrontendIpConfigurations $fipconfig -GatewayIpConfigurations $gipconfig -FrontendPorts $fp -HttpListeners $listener -RequestRoutingRules $rule -Sku $sku -SSLPolicy $SSLPolicy -AuthenticationCertificates $authcert -Verbose
```

V2 SKU의 경우 아래 명령을 사용합니다.
```powershell
$appgw = New-AzApplicationGateway -Name appgateway -SSLCertificates $cert -ResourceGroupName "appgw-rg" -Location "West US" -BackendAddressPools $pool -BackendHttpSettingsCollection $poolSetting01 -FrontendIpConfigurations $fipconfig -GatewayIpConfigurations $gipconfig -FrontendPorts $fp -HttpListeners $listener -RequestRoutingRules $rule -Sku $sku -SSLPolicy $SSLPolicy -TrustedRootCertificate $trustedRootCert01 -Verbose
```

## <a name="apply-a-new-certificate-if-the-back-end-certificate-is-expired"></a>백 엔드 인증서가 만료된 경우 새 인증서 적용

백 엔드 인증서가 만료된 경우 이 절차를 사용하여 새 인증서를 적용합니다.
...
2. .cer 파일에서 새 인증서 리소스를 추가합니다.

   ```powershell
   Add-AzApplicationGatewayAuthenticationCertificate -ApplicationGateway $gw -Name 'NewCert' -CertificateFile "appgw_NewCert.cer" 
   ```
    
3. 새 인증 인증서 개체를 변수로 가져옵니다(TypeName: Microsoft.Azure.Commands.Network.Model.Models.PSApplication게이트웨이인증인증서).

   ```powershell
   $AuthCert = Get-AzApplicationGatewayAuthenticationCertificate -ApplicationGateway $gw -Name NewCert
   ```
 
 4. 백 **엔드Http** 설정에 새 인증서를 할당 하 고 $AuthCert 변수와 함께 참조 합니다. (변경할 HTTP 설정 이름을 지정합니다.)
 
   ```powershell
   $out= Set-AzApplicationGatewayBackendHttpSetting -ApplicationGateway $gw -Name "HTTP1" -Port 443 -Protocol "Https" -CookieBasedAffinity Disabled -AuthenticationCertificates $Authcert
   ```
    
 5. 변경 내용이 응용 프로그램 게이트웨이에 커밋되고 포함된 새 구성을 $out 변수에 전달합니다.
 
   ```powershell
   Set-AzApplicationGateway -ApplicationGateway $gw  
   ```

## <a name="remove-an-unused-expired-certificate-from-http-settings"></a>HTTP 설정에서 사용되지 않는 만료된 인증서 제거

이 절차를 사용하여 HTTP 설정에서 사용되지 않는 만료된 인증서를 제거합니다.
...
2. 제거할 인증 인증서의 이름을 나열합니다.

   ```powershell
   Get-AzApplicationGatewayAuthenticationCertificate -ApplicationGateway $gw | select name
   ```
    
3. 응용 프로그램 게이트웨이에서 인증 인증서를 제거합니다.
...
## <a name="limit-tls-protocol-versions-on-an-existing-application-gateway"></a>기존 애플리케이션 게이트웨이에서 TLS 프로토콜 버전 제한

앞의 단계를 통해 종단 간 TLS를 사용하여 응용 프로그램을 만들고 특정 TLS 프로토콜 버전을 사용하지 않도록 설정했습니다. 다음 예제는 기존 응용 프로그램 게이트웨이에서 특정 TLS 정책을 사용하지 않도록 설정합니다.
...
3. 마지막으로 게이트웨이를 업데이트합니다. 이 마지막 단계는 오래 실행되는 작업입니다. 작업이 완료되면 종단 간 TLS가 응용 프로그램 게이트웨이에서 구성됩니다.
