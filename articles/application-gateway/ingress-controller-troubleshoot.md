...
위의 스크립트에서 [Azure Cloud Shell](https://shell.azure.com/)한 번에 모든 줄을 복사 하 여 붙여 넣습니다. 전체 명령이 복사 되었는지 확인 하 `cat` 고, 마지막 `EOF`를 포함 하 여 시작 합니다.
...
조절기 `kubectl get ingress`의 목록을 가져옵니다. ' Agic-수신 ' 이라는 수신 리소스가 생성 된 것으로 간주 됩니다. 리소스의 호스트 이름은 ' test.agic.contoso.com '입니다.

![pod](./media/application-gateway-ingress-controller-troubleshooting/tsg--get-ingress.png)

Pod 중 하나는 AGIC가 됩니다. `kubectl get pods`pod 목록이 표시 되며, 그 중 하나는 ' 수신-azure '로 시작 됩니다. 이 pod `kubectl logs <name-of-ingress-controller-pod>` 의 모든 로그를 가져와서 성공적으로 배포 되었는지 확인 합니다. 성공적으로 배포 되 면 로그에 다음 줄이 추가 됩니다.
```
...
또는 [Cloud Shell](https://shell.azure.com/) 를 사용 하 여 성공적으로 `kubectl logs <ingress-azure-....> | grep 'Applied App Gateway config in'`Application Gateway 구성을 나타내는 줄만 검색할 수 있습니다. `<ingress-azure....>` 여기서은 AGIC pod의 정확한 이름입니다.
...
- 백 엔드 풀:
  - 백 엔드 주소 풀에는 하나의 ip 주소가 있으며 앞 `kubectl get pods -o wide` 
 ![에서 관찰 한 Pod의 ip 주소와 일치 하 게 됩니다 backend_pool](./media/application-gateway-ingress-controller-troubleshooting/tsg--backendpools.png)


마지막으로 Cloud Shell 내에서 `cURL` 명령을 사용 하 [Cloud Shell](https://shell.azure.com/) 여 새로 배포 된 앱에 대 한 HTTP 연결을 설정할 수 있습니다.

1. 을 `kubectl get ingress` 사용 하 여 Application Gateway의 공용 IP 주소를 가져옵니다.
...
의 `HTTP/1.1 200 OK` 결과 APPLICATION GATEWAY + AKS + AGIC 시스템이 예상 대로 작동 하 고 있음을 나타냅니다.
...
AGIC가 예상 대로 작동 하려면 다음 사항을 준비 해야 합니다.
  1. AKS에는 정상 **pod**가 하나 이상 있어야 합니다.
     를 사용 `apsnetapp`하 여 Pod `kubectl get pods -o wide --show-labels` 가 있는 경우 [Cloud Shell](https://shell.azure.com/) 에서이를 확인 합니다. 출력은 다음과 같습니다.
     ```bash
  ...
  2. 일치 하 `selector` 는 레이블을 통해 위의 pod를 참조 하는 하나 이상의 **서비스**.
  ...
  3. **Ingress**위의 서비스를 참조 `kubernetes.io/ingress.class: azure/application-gateway`하 여에 주석이 추가 된 수신, [Cloud Shell](https://shell.azure.com/) 에서이를 확인 합니다.`kubectl get ingress -o wide --show-labels`
     ```bash
  ...
  4. 위의 수신에 대 한 주석 보기 `kubectl get ingress aspnetapp -o yaml` : ( `aspnetapp` 수신 이름으로 대체)
     ...
     수신 리소스는로 `kubernetes.io/ingress.class: azure/application-gateway`주석을 추가 해야 합니다.
...
* AGIC pod는 `default` 네임 스페이스 (열 `NAMESPACE`참조)에 있어야 합니다. 정상 pod는 `Running` `STATUS` 열에 있습니다. AGIC pod가 하나 이상 있어야 합니다.
...
* AGIC pod가 정상이 아닌 경우 (`STATUS` 위 명령의 열이이 아닌 `Running`경우):
...
* 다음 `kubernetes.io/ingress.class: azure/application-gateway`을 사용 하 여 [수신](https://kubernetes.io/docs/concepts/services-networking/ingress/) 주석을 추가 했습니까? AGIC는이 주석이 있는 Kubernetes 수신 리소스만 감시 합니다.
...
세부 정보 표시 수준은 `verbosityLevel` [투구-.config](#sample-helm-config-file) 파일의 변수를 통해 조정할 수 있습니다. 세부 정보 표시 수준을 `5` 로 늘려 [ARM](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview)에 디스패치 된 JSON 구성을 가져옵니다.
  - 투구 `verbosityLevel: 5` 에서 한 줄에 단독으로 추가 하 고 다시 설치 [합니다.](#sample-helm-config-file)
