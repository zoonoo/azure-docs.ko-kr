...
인터넷 연결 VIP 또는 ILB (내부 부하 분산 장치) 끝점이 라고도 하는 인터넷에 노출 되지 않은 내부 끝점 (프런트 엔드 IP 주소에 대 한 개인 IP 사용)을 사용 하 여 Azure 애플리케이션 게이트웨이를 구성할 수 있습니다. 프런트 엔드 개인 IP 주소를 사용 하 여 게이트웨이를 구성 하는 것은 인터넷에 노출 되지 않는 내부 lob (기간 업무) 응용 프로그램에 유용 합니다. 또한 인터넷에 노출되지 않은 보안 경계에 앉아 있는 다중 계층 애플리케이션 내에 포함된 서비스 및 계층에 유용하지만 여전히 라운드 로빈 부하 분산, 세션 인력 또는 SSL(Secure Sockets Layer) 종료가 필요합니다.

이 문서에서는 Azure Portal에서 프런트 엔드 개인 IP 주소를 사용 하 여 응용 프로그램 게이트웨이를 구성 하는 단계를 안내 합니다.

이 아티클에서는 다음 방법을 설명합니다.

- Application Gateway에 대 한 개인 프런트 엔드 IP 구성 만들기
...
7. 프런트 엔드 IP 구성을 개인으로 선택 하 고 기본적으로 동적 IP 주소 할당입니다. 선택한 서브넷의 사용 가능한 첫 번째 주소가 프런트 엔드 IP 주소로 할당 됩니다.
8. 서브넷 주소 범위 (정적 할당)에서 개인 IP를 선택 하려면 **특정 개인 ip 주소 선택** 상자를 클릭 하 고 IP 주소를 지정 합니다.
   > [!NOTE]
...
4. **확인**을 클릭합니다.
5. 가상 컴퓨터 크기에 대 한 **DS1_V2** 를 선택 하 고 **선택**을 클릭 합니다.
