...
프런트 엔드 개인 IP 주소를 사용 하 여 게이트웨이를 구성 하는 것은 인터넷에 노출 되지 않는 내부 lob (기간 업무) 응용 프로그램에 유용 합니다. 인터넷에 노출 되지 않은 보안 경계에 있는 다중 계층 응용 프로그램 내의 서비스 및 계층에도 유용 합니다.

- 라운드 로빈 부하 분산 필요
- 세션에 대 한 세션
- 또는 TLS (Transport Layer Security) 종료 (이전에는 SSL(Secure Sockets Layer) (SSL))로 알려져 있습니다.
...
Azure가 사용자가 만든 리소스 간에 통신하려면 가상 네트워크가 필요합니다. 새 가상 네트워크를 만들거나 기존 가상 네트워크를 사용 합니다. 

이 예제에서는 새 가상 네트워크를 만듭니다. 애플리케이션 게이트웨이를 만드는 동시에 가상 네트워크를 만들 수 있습니다. 별도의 서브넷으로 Application Gateway 인스턴스가 만들어집니다. 이 예에는 두 개의 서브넷이 있습니다. 하나는 응용 프로그램 게이트웨이이 고 다른 하나는 백 엔드 서버용입니다.
...
6. **확인** 을 선택 하 여 가상 네트워크 및 서브넷을 만듭니다.
...
13. **추가** 를 선택합니다.
14. **다음: 구성** 을 선택 합니다.
15. **라우팅 규칙** 에서 **라우팅 규칙 추가** 를 선택 합니다.
...
4. 가상 머신에 대해 다음 값을 입력합니다.
   ...
   - **가상 컴퓨터 이름** 에 *myvm* 을 입력 합니다.
   - **이미지** 에 대 한 **Windows Server 2019 Datacenter** 를 선택 합니다.
   - 올바른 **사용자 이름을** 입력 하십시오.
   - 올바른 **암호** 를 입력 하십시오.
1. 나머지 기본값을 적용 하 고 **다음: 디스크** 를 선택 합니다.
1. 기본값을 그대로 적용 하 고 **다음: 네트워킹** 을 선택 합니다.
1. 가상 네트워크에 대해 **Myvnet** 이 선택 되어 있고 서브넷이 **myBackendSubnet** 인지 확인 합니다.
1. 나머지 기본값을 그대로 적용 하 고 **다음: 관리** 를 선택 합니다.
1. 부팅 진단을 사용 하지 않도록 설정 하려면 **사용 안 함** 을 선택 합니다.
1. **검토 + 만들기** 를 선택합니다.
...
3. 두 번째 가상 머신을 만들고, 방금 완료한 단계를 사용하여 IIS를 설치합니다. 에서 가상 컴퓨터 이름 및에 대해 myVM2를 `VMName` 사용 `Set-AzVMExtension` 합니다.

### <a name="add-backend-servers-to-backend-pool"></a>백 엔드 풀에 백 엔드 서버 추가

1. **모든 리소스** 를 선택한 다음, **myAppGateway** 를 선택합니다.
2. **백 엔드 풀** 을 선택한 다음 **appGatewayBackendPool** 를 선택 합니다.
3. **대상 유형** 아래에서 **가상 머신** 를 선택 하 고 **대상** 에서 myvm과 연결 된 vNIC을 선택 합니다.
4. 반복 하 여 MyVM2를 추가 합니다.
   ![대상 유형 및 대상이 강조 표시 된 백 엔드 풀 창을 편집 합니다.](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-4.png)
5. **저장** 을 선택합니다.

## <a name="create-a-client-virtual-machine"></a>클라이언트 가상 컴퓨터 만들기

클라이언트 가상 머신은 application gateway 백 엔드 풀에 연결 하는 데 사용 됩니다.

- 이전 단계를 사용 하 여 세 번째 가상 컴퓨터를 만듭니다. 가상 컴퓨터 이름에 myVM3를 사용 합니다.

## <a name="test-the-application-gateway"></a>애플리케이션 게이트웨이 테스트

1. MyAppGateway 페이지에서 **프런트 엔드 Ip 구성** 을 선택 하 여 프런트 엔드 개인 IP 주소를 확인 합니다.
    ![비공개 유형을 강조 표시 한 프런트 엔드 IP 구성 창](./media/configure-application-gateway-with-private-frontend-ip/private-frontendip-5.png)
2. 개인 IP 주소를 복사한 다음 myVM3의 브라우저 주소 표시줄에 붙여넣어 application gateway 백 엔드 풀에 액세스 합니다.

## <a name="next-steps"></a>다음 단계

백 엔드 풀의 상태를 모니터링 하려는 경우 [Application Gateway에 대 한 백 엔드 상태 및 진단 로그](application-gateway-diagnostics.md)를 참조 하세요.
