<properties 
   pageTitle="가상 네트워크의 내부 부하 분산 장치(ILB)와 응용 프로그램 게이트웨이 생성 및 구성 | Microsoft Azure"
...
4. [게이트웨이를 확인합니다.](#verify-the-gateway-status)
...
`Get-AzureApplicationGateway` cmdlet을 사용하여 게이트웨이의 상태를 확인합니다. *Start-AzureApplicationGateway*가 이전 단계에서 성공한 경우, 상태가 *실행 중*이어야 하고, Vip와 DNS 이름은 유효한 항목이어야 합니다. 이 출력 다음 샘플에서는 첫 줄에 cmdlet을 보여줍니다. 이 샘플에서 게이트웨이가 실행되며 트래픽을 받을 준비가 되었습니다.

> [AZURE.NOTE] 응용 프로그램 게이트웨이는 이 예에서 구성된 ILB 끝점인 10.0.0.10에서 트래픽을 허용하도록 구성됩니다.
