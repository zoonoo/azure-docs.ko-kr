...
**원인:** TCP 연결이 설정되고 SSL 핸드셰이크가 수행된 후(SSL이 활성화된 경우) 응용 프로그램 게이트웨이는 프로브를 HTTP GET 요청으로 백 엔드 서버로 보냅니다. 앞에서 설명한 바와 같이, \<기본\>프로브는 프로토콜 ://127.0.0.1:\<포트/이\>될 것이며, 분노 200에서 399까지의 응답 상태 코드를 정상으로 고려한다. 서버가 다른 상태 코드를 반환하면 이 메시지와 함께 비정상으로 표시됩니다.
...
SSL 인증서를 신뢰할 수 있도록 하려면 응용 프로그램 게이트웨이의 신뢰할 수 있는 저장소에 포함된 CA에서 백 엔드 서버의 인증서를 발급해야 합니다. 신뢰할 수 있는 CA에서 인증서를 발급하지 않은 경우(예: 자체 서명된 인증서가 사용된 경우) 사용자는 발급자의 인증서를 응용 프로그램 게이트웨이에 업로드해야 합니다.
...
**원인:** 응용 프로그램 게이트웨이 v2를 사용 하 여 종단 간 SSL 서버 정상으로 간주 하기 위해 백 엔드 서버의 인증서를 확인 해야 합니다.
SSL 인증서를 신뢰할 수 있도록 하려면 응용 프로그램 게이트웨이의 신뢰할 수 있는 저장소에 포함 된 CA에서 백 엔드 서버 인증서를 발급 해야 합니다. 신뢰할 수 있는 CA에서 인증서를 발급하지 않은 경우(예: 자체 서명된 인증서가 사용됨) 사용자는 발급자의 인증서를 응용 프로그램 게이트웨이에 업로드해야 합니다.
...
**원인:** 응용 프로그램 게이트웨이는 백 엔드 HTTP 설정에 지정된 호스트 이름이 백 엔드 서버의 SSL 인증서에서 제공하는 CN의 이름과 일치하는지 여부를 확인합니다. 이것은 Standard_v2 및 WAF_v2 SKU 동작입니다. 표준 및 WAF SKU의 서버 이름 표시(SNI)는 백 엔드 풀 주소의 FQDN으로 설정됩니다.
...
**원인:** 모든 인증서에는 유효성 범위가 제공되며 서버의 SSL 인증서가 유효하지 않으면 HTTPS 연결이 안전하지 않습니다. 현재 데이터는 **유효한 데이터** 내에 있어야 하며 **범위까지 유효해야** 합니다. 그렇지 않은 경우 인증서는 유효하지 않은 것으로 간주되며 응용 프로그램 게이트웨이가 백 엔드 서버를 비정상으로 표시하는 보안 문제가 발생합니다.

**솔루션:** SSL 인증서가 만료된 경우 공급업체에서 인증서를 갱신하고 새 인증서로 서버 설정을 업데이트합니다. 자체 서명된 인증서인 경우 유효한 인증서를 생성하고 루트 인증서를 응용 프로그램 게이트웨이 HTTP 설정에 업로드해야 합니다. 이렇게 하려면 다음 단계를 수행하세요.
...
**메시지:** 백 엔드 인증서의 유효성을 확인할 수 없습니다. 이유를 알아보려면 오류 코드 {errorCode}와 연결된 메시지에 대한 SSL 진단 열기를 선택합니다.
