...
description: Azure 응용 프로그램 게이트웨이에서 한 수신기에서 받은 트래픽을 다른 수신기 또는 외부 사이트로 리디렉션하는 리디렉션 기능에 대해 알아봅니다.
...
ms.locfileid: "74129874"
---
# <a name="application-gateway-redirect-overview"></a>Application Gateway 리디렉션 개요

응용 프로그램 게이트웨이를 사용하여 트래픽을 리디렉션할 수 있습니다.  Application Gateway는 한 수신기에서 수신한 트래픽을 다른 수신기 또는 외부 사이트로 리디렉션하는 일반적인 리디렉션 메커니즘을 갖고 있습니다. 이를 통해 애플리케이션 구성이 간소화되고, 리소스 사용이 최적화되고, 전역 및 경로 기반 리디렉션을 비롯한 새로운 리디렉션 시나리오가 지원됩니다.

많은 웹 응용 프로그램에 대 한 일반적인 리디렉션 시나리오는 암호화 된 경로 통해 응용 프로그램과 사용자 간의 모든 통신을 발생 하도록 HTTPS 리디렉션에 자동 HTTP를 지원 하는 것입니다. 이전에는 HTTP에서 수신하는 요청을 HTTPS로 리디렉션하는 것이 유일한 목적인 전용 백 엔드 풀 만들기와 같은 기술을 고객이 사용했습니다. 응용 프로그램 게이트웨이에서 리디렉션 지원을 사용하면 라우팅 규칙에 새 리디렉션 구성을 추가하고 HTTPS 프로토콜을 대상으로 다른 수신기를 지정하여 이 작업을 수행할 수 있습니다.

다음과 같은 리디렉션 유형이 지원됩니다.
