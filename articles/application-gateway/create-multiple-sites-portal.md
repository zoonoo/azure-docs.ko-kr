---
title: 여러 웹 사이트를 호스트하는 응용 프로그램 게이트웨이 만들기 - Azure Portal | Microsoft Docs
description: Azure Portal을 사용하여 여러 웹 사이트를 호스트하는 응용 프로그램 게이트웨이를 만드는 방법을 알아봅니다.
...
ms.locfileid: "39045022"
---
# <a name="create-and-configure-an-application-gateway-to-host-multiple-web-sites-using-the-azure-portal"></a>Azure Portal을 사용하여 여러 웹 사이트를 호스트하는 응용 프로그램 게이트웨이 생성 및 구성

Azure Portal을 사용하여 [응용 프로그램 게이트웨이](multiple-site-overview.md)를 만들 때 [여러 웹 사이트의 호스팅](overview.md)을 구성할 수 있습니다. 이 자습서에서는 가상 머신을 사용하여 백 엔드 주소 풀을 정의합니다. 그런 다음, 웹 트래픽이 풀에서 적절한 서버에 도착하도록 소유한 도메인을 기준으로 수신기와 규칙을 구성합니다. 이 자습서에서는 여러 도메인을 소유하고 있으며 *www.contoso.com* 및 *www.fabrikam.com*의 예를 사용한다고 가정합니다.
...
3. 응용 프로그램 게이트웨이에 대해 다음 값을 입력합니다.
    ...
    ![새 응용 프로그램 게이트웨이 만들기](./media/create-multiple-sites-portal/application-gateway-create.png)
...
2. **서브넷**을 클릭한 다음, **서브넷**을 클릭합니다.
    ...
    ![서브넷 만들기](./media/create-multiple-sites-portal/application-gateway-subnet.png)
...
3. *contosoPool*의 이름을 입력하고 **대상 추가**를 사용하여 *contosoVM*을 추가합니다.
    ...
    ![백 엔드 서버 추가](./media/create-multiple-sites-portal/application-gateway-multisite-backendpool.png)

4. **확인**을 클릭합니다.
5. **백엔드 풀**을 클릭한 다음, **추가**를 클릭합니다.
6. 방금 완료한 단계를 사용하여 *fabrikamVM*으로 *fabrikamPool*을 만듭니다.

## <a name="create-backend-listeners"></a>백 엔드 수신기 만들기
...
4. 백 엔드 풀로 *contosoPool*을 선택합니다.
    ...
    ![경로 기반 규칙 만들기](./media/create-multiple-sites-portal/application-gateway-multisite-rule.png)
