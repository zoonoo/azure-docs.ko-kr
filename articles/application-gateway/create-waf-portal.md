...
Azure Portal을 사용하여 WAF([웹 응용 프로그램 방화벽](waf-overview.md))이 있는 [응용 프로그램 게이트웨이](overview.md)를 만들 수 있습니다. WAF는 [OWASP](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) 규칙을 사용하여 응용 프로그램을 보호합니다. 이러한 규칙에는 SQL 삽입, 사이트 간 스크립팅 공격 및 세션 하이재킹과 같은 공격으로부터의 보호가 포함됩니다.
...
![웹 응용 프로그램 방화벽 예제](./media/create-waf-portal/scenario-waf.png)
...
3. 응용 프로그램 게이트웨이에 대해 다음 값을 입력합니다.
    ...
    ![새 응용 프로그램 게이트웨이 만들기](./media/create-waf-portal/application-gateway-create.png)
...
5. **가상 네트워크 선택**을 클릭하고 **새로 만들기**를 클릭한 다음, 가상 네트워크에 대해 다음 값을 입력합니다.
    ...
    ![가상 네트워크 만들기](./media/create-waf-portal/application-gateway-vnet.png)
...
2. **서브넷**을 클릭한 다음, **서브넷**을 클릭합니다.
    ...
    ![서브넷 만들기](./media/create-waf-portal/application-gateway-subnet.png)
...
1. 대화형 셸을 열고 **PowerShell**로 설정되어 있는지 확인합니다.
    ...
    ![사용자 지정 확장 설치](./media/create-waf-portal/application-gateway-extension.png)
...
3. **대상 추가**를 클릭하여 생성된 각각의 가상 머신을 백 엔드 풀에 추가합니다.
    ...
    ![백 엔드 서버 추가](./media/create-waf-portal/application-gateway-backend.png)
...
7. **저장**을 클릭합니다.
    ...
    ![진단 구성](./media/create-waf-portal/application-gateway-diagnostics.png)
...
1. [개요] 화면에서 응용 프로그램 게이트웨이에 대한 공용 IP 주소를 찾습니다. **모든 리소스**를 클릭한 다음, **myAGPublicIPAddress**를 클릭합니다.
    ...
    ![응용 프로그램 게이트웨이에 대한 공용 IP 주소 기록](./media/create-waf-portal/application-gateway-record-ag-address.png)

2. 공용 IP 주소를 복사한 다음, 브라우저의 주소 표시줄에 붙여넣습니다.

    ![응용 프로그램 게이트웨이 테스트](./media/create-waf-portal/application-gateway-iistest.png)
