...
- Azure 응용 프로그램 게이트웨이의 백 엔드 풀은 구성되거나 비어 있지 않습니다.
...
###원인
응용 프로그램 게이트웨이에 백 엔드 주소 풀에 구성된 VM 또는 VM 크기 조정 설정이 없는 경우 고객의 요청을 라우팅할 수 없고 잘못된 게이트웨이 오류를 throw할 수 없습니다.
...
위의 cmdlet에서 출력은 비어 있지 않은 백 엔드 주소 풀을 포함해야 합니다. 다음은 백 엔드 VM에 대한 FQDN 또는 IP 주소로 구성된 두 개의 풀을 반환하는 예제입니다. BackendAddressPool의 상태를 프로비전하는 작업은 '성공'해야 합니다.
...
###원인
BackendAddressPool의 모든 인스턴스가 정상이 아닌 경우 응용 프로그램 게이트웨이에는 사용자 요청을 라우팅할 백 엔드가 없습니다. 백 엔드 인스턴스가 정상이지만 필요한 응용 프로그램이 배포되지 않은 경우일 수도 있습니다.
...
###해결 방법
아래 테이블을 기준으로 사용자 지정 상태 프로브를 올바르게 구성했는지 유효성을 검사합니다. 위의 문제 해결 단계 외에도 다음 사항을 확인합니다.
...
##요청 시간 초과
###원인
사용자 요청이 수신되면 응용 프로그램 게이트웨이가 요청에 구성된 규칙을 적용하고 백 엔드 풀 인스턴스를 라우팅합니다. 백 엔드 인스턴스의 응답을 위해 구성 가능한 시간 간격을 기다립니다. 기본적으로 이 간격은 **30초**입니다. 응용 프로그램 게이트웨이가 이 간격에서 백 엔드 응용 프로그램의 응답을 수신하지 않으면 사용자 요청에 502 오류가 표시됩니다.

###해결 방법
응용 프로그램 게이트웨이를 사용하면 사용자가 다른 풀에 적용할 수 있는 BackendHttpSetting을 통해 이 설정을 구성할 수 있습니다. 다른 백 엔드 풀에는 다른 BackendHttpSetting 및 구성된 다른 요청 시간 초과가 있습니다.

	New-AzureRmApplicationGatewayBackendHttpSettings -Name 'Setting01' -Port 80 -Protocol Http -CookieBasedAffinity Enabled -RequestTimeout 60

##다음 단계

위의 단계에서 문제가 해결되지 않으면 [지원 티켓](https://azure.microsoft.com/support/options/)을 엽니다.
