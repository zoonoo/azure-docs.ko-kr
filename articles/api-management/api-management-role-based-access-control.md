...
API Management는 현재 3가지 기본 제공 역할을 제공하며 곧 2가지 역할을 추가할 예정입니다. 이러한 역할은 구독, 리소스 그룹 및 개별 API Management 인스턴스를 포함하여 서로 다른 범위에서 할당될 수 있습니다. 예를 들어 리소스 그룹 수준에서 사용자에게 "API 관리 서비스 판독기" 역할을 할당하면 사용자는 리소스 그룹 내의 모든 API 관리 인스턴스에 대한 읽기 액세스 권한을 갖습니다. 
...
| API 관리 서비스 리더 | ✓ | | || API Management 서비스 및 엔터티에 대한 읽기 전용 액세스 권한을 보유합니다. |
| API 관리 서비스 운영자 | ✓ | | ✓ | | 엔터티가 아닌 API Management 서비스를 관리할 수 있습니다.|
| API 관리 서비스 편집기<sup>*</sup> | ✓ | ✓ | |  | API Management 엔터티는 관리할 수 있지만 서비스는 관리할 수 없습니다.|
| API 관리 콘텐츠 관리자<sup>*</sup> | ✓ | | | ✓ | 개발자 포털을 관리할 수 있습니다. 서비스 및 엔터티에 대한 읽기 전용 액세스.|
...
<sup>\*기존 게시자 포털에서 Azure Portal로 모든 관리자 UI를 마이그레이션한 후 서비스 편집기 역할을 사용할 수 있습니다. 콘텐츠 관리자 역할은 게시자 포털이 개발자 포털 관리와 관련된 기능만 포함하도록 리팩터링된 후에 사용할 수 있습니다.</sup>  
...
> [!NOTE]
> Azure 포털에서 API 관리 인스턴스를 볼 수 있도록 사용자 ```Microsoft.ApiManagement/service/read``` 지정 역할에 작업을 포함해야 합니다.

사용자 지정 역할을 만들 때는 기본 제공 역할 중 하나로 시작하는 것이 쉽습니다. 속성을 편집하여 **Actions**, **NotActions** 또는 **AssignableScopes**를 추가한 다음 변경 사항을 새 역할로 저장합니다. 다음 예제에서는 "API 관리 서비스 판독기" 역할로 시작 하 고 라는 사용자 지정 역할을 만듭니다 "계산기 API 편집기." 사용자 지정 역할을 특정 API에 할당할 수 있습니다. 따라서 이 역할만 해당 API에 대한 액세스 권한을 보유합니다. 
...
Azure의 역할 기반 액세스 제어에 대한 자세한 정보는 다음 문서를 참조하세요.
  ...
  * [Azure 리소스 관리자 리소스 공급자 작업](../role-based-access-control/resource-provider-operations.md#microsoftapimanagement)
