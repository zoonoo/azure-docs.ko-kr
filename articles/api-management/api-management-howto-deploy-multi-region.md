...
API 관리 서비스가 처음 만들어질 때 한 [단위][]만 포함하며 단일 Azure 지역에 있어 기본 지역으로 지정됩니다. Azure 포털을 통해 추가 지역을 쉽게 추가할 수 있습니다. API 관리 프록시 서버는 각 지역에 배포되고 호출 트래픽이 가장 가까운 프록시에 라우팅됩니다. 지역이 오프라인 상태가 되면 다음으로 가장 가까운 프록시에 트래픽이 자동으로 리디렉션됩니다.
