...
개발자 로그인 및 등록을 기존 웹 사이트에 위임하려면 API Management 개발자 포털에서 시작된 이러한 요청에 대한 진입점 역할을 하는 특수한 위임 끝점을 사이트에 만들어야 합니다.
...
* **위임 인증 키** 필드에서 요청이 Azure API Management에서 들어오는지 확인하기 위해 사용자에게 제공된 서명을 계산하는 데 사용되는 암호를 입력합니다. **생성** 단추를 클릭하여 API 관리가 임의로 사용자를 위해 키를 생성하도록 할 수 있습니다.
