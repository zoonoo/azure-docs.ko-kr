...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| choose    | 루트 요소입니다.                                                                                                                                                                                                                                                             | yes      |
| when      | `if` 정책의 `ifelse` 또는 `choose` 부분에 사용할 조건입니다. `choose` 정책에 여러 `when` 섹션이 있는 경우 순차적으로 평가됩니다. when 요소의 `condition`이 `true`로 평가되면 `when` 조건이 더 이상 평가되지 않습니다. | yes      |
| otherwise | `when`로 평가되는 `true` 조건이 없으면 사용할 정책 조각을 포함합니다.                                                                                                                                                                               | 예       |
...
Event Hubs에 기록할 값으로 모든 문자열을 사용할 수 있습니다. 이 예제에서는 모든 인바운드 호출에 대 한 날짜 및 시간, 배포 서비스 이름, 요청 ID, IP 주소 및 작업 이름이 `contoso-logger` ID로 등록 된 이벤트 허브로 거에 기록 됩니다.
...
| attribute        | Description                                                                                                                                           | 필수 | 기본값 |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| condition(조건)        | 재시도를 중지([) 또는 진행(](api-management-policy-expressions.md))해야 하는지 여부를 지정하는 부울 리터럴 또는 `false`식`true`입니다.      | yes      | 해당 없음     |
...
| first-fast-retry | `true`로 설정 하면 첫 번째 다시 시도가 즉시 수행 됩니다.                                                                                  | 예       | `false` |
...
### <a name="set-variableExample"></a> 예제
...
| --------- | ------------------------------------------------------------------------ | -------- |
| name      | 변수의 이름입니다.                                                | yes      |
| 값     | 변수의 값입니다. 식 또는 리터럴 값일 수 있습니다. | yes      |
...
| metadata | Application Insights [추적](https://docs.microsoft.com/azure/azure-monitor/app/data-model-trace-telemetry) 원격 분석에 사용자 지정 속성을 추가 합니다. | 예       |
...
| attribute | Description                                                                                                                                                                                                                                                                                                                                                                                                            | 필수 | 기본값 |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- |
| for       | `wait` 정책에서 모든 직계 자식 정책 또는 한 정책이 완료될 때까지 대기할지 여부를 결정합니다. 허용된 값은<br /><br /> - `all` - 모든 직계 자식 정책이 완료될 때까지 대기합니다.<br />-any-모든 직계 자식 정책이 완료 될 때까지 대기 합니다. 첫 번째 직계 자식 정책이 완료되면 `wait` 정책이 완료되고 다른 직계 자식 정책의 실행이 종료됩니다. | 예       | all     |
