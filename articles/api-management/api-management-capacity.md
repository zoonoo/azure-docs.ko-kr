...
ms.locfileid: "67836959"
---
# <a name="capacity-of-an-azure-api-management-instance"></a>Azure API Management 인스턴스의 용량

**용량** 가장 중요 [Azure Monitor 메트릭](api-management-howto-use-azure-monitor.md#view-metrics-of-your-apis) 더 많은 부하를 수용 하기 위해 API Management 인스턴스를 크기 조정 여부 합리적된 결정을 내리는 대 한 합니다. 이러한 구성은 복잡하며 특정 동작을 적용합니다.
...
**용량** 부하 API Management 인스턴스를 나타냅니다. 여기에는 리소스 사용량(CPU, 메모리)과 네트워크 큐 길이가 반영됩니다. CPU와 메모리 사용량은 다음을 통해 리소스 사용량을 나타냅니다.

+ 요청을 전달 하거나 정책 실행을 포함할 수 있는 관리 작업 또는 요청 처리와 같은 API Management 서비스
+ 선택된 운영 체제 프로세스(새로운 연결에서 SSL 핸드셰이크 비용과 관련된 프로세스 포함)

총 **용량**은 API Management 인스턴스의 모든 단위에서 가져온 자체 값의 평균입니다.

하지만 합니다 **용량 메트릭은** 는 API Management 인스턴스를 사용 하 여 문제를 노출 하도록 설계 된 경우가 문제에서 변경 사항을 반영 되지 않습니다는 **용량 메트릭은**.
...
낮은 **용량 메트릭은** 서 반드시 API Management 인스턴스에 문제가 되지 않습니다.
