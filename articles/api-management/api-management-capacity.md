...
**용량은** 더 많은 부하를 수용하도록 API 관리 인스턴스를 확장할지 여부를 정보에 입각한 결정을 내리는 데 가장 중요한 [Azure Monitor 메트릭입니다.](api-management-howto-use-azure-monitor.md#view-metrics-of-your-apis) 이러한 구성은 복잡하며 특정 동작을 적용합니다.
...
> [!IMPORTANT]
> 이 문서에서는 용량 메트릭을 기반으로 Azure API Management 인스턴스를 모니터링하고 확장하는 방법에 대해 설명합니다. 그러나 개별 API 관리 인스턴스가 실제로 용량에 *도달했을* 때 어떤 일이 발생하는지 이해하는 것도 똑같이 중요합니다. Azure API Management는 인스턴스의 물리적 오버로드를 방지하기 위해 서비스 수준 제한을 적용하지 않습니다. 인스턴스가 물리적 용량에 도달하면 들어오는 요청을 처리할 수 없는 오버로드된 웹 서버와 유사하게 작동합니다: 대기 시간이 증가하고 연결이 끊어지고 시간 초과 오류가 발생합니다. 즉, API 클라이언트는 다른 외부 서비스(예: 재시도 정책을 적용)와 유사한 가능성을 처리할 준비가 되어 있어야 합니다.
...
**용량은** API 관리 인스턴스의 부하를 나타내는 지표입니다. 여기에는 리소스 사용량(CPU, 메모리)과 네트워크 큐 길이가 반영됩니다. CPU와 메모리 사용량은 다음을 통해 리소스 사용량을 나타냅니다.

+ API 관리 데이터 평면 서비스(예: 요청 처리)로, 여기에는 전달 요청 또는 정책 실행이 포함될 수 있습니다.
+ Azure Portal 또는 ARM을 통해 적용된 관리 작업 또는 [개발자 포털에서](api-management-howto-developer-portal.md)들어오는 로드와 같은 API 관리 평면 서비스 입니다.
+ 새 연결에서 TLS 핸드셰이크 비용이 포함된 프로세스를 포함하여 선택한 운영 체제 프로세스입니다.

총 **용량**은 API Management 인스턴스의 모든 단위에서 가져온 자체 값의 평균입니다.

용량 **메트릭은** API 관리 인스턴스의 문제를 표시하도록 설계되었지만 **용량 메트릭의**변경 사항에 문제가 반영되지 않는 경우가 있습니다.
...
**용량이 낮다고** 해서 반드시 API 관리 인스턴스에 문제가 발생하지 않는다는 의미는 아닙니다.
...
5. 섹션 위쪽의 막대에서 원하는 시간 범위를 선택합니다.
    ...
    예상치 못한 상황이 발생할 때 알려주도록 메트릭 경고를 설정할 수 있습니다. 예를 들어 APIM 인스턴스가 예상 최대 용량을 20분 이상 초과하면 알림을 받습니다.
