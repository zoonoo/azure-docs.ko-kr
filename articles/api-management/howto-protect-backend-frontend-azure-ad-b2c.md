...
1. 요청이 인증 되지 않은 경우 수행할 작업 드롭다운을 "Azure Active Directory 사용 하 여 로그인"으로 설정 하 고 저장을 클릭 합니다.
   ...
   > API Management 소비 계층을 사용 하는 경우 VIP로이 잠금을 수행할 수 없습니다. 해당 계층에 대 한 전용 고정 IP가 없기 때문에 공유 비밀 기능 키를 통해 API 호출의 잠금을 해제 하는 방법을 사용 해야 하므로 11-13 단계를 수행할 수 없습니다.
