...
> [!IMPORTANT]
> 명명된 값으로 정의되지 않은 모든 암호는 리포지토리에 저장되며 Git 액세스를 비활성화하고 다시 활성화할 때까지 기록에 남아 있습니다. 명명된 값은 모든 API 구성 및 정책에서 비밀을 포함하여 일정한 문자열 값을 관리할 수 있는 안전한 위치를 제공하므로 정책 설명에 직접 저장할 필요가 없습니다. 자세한 내용은 [Azure API Management 정책에 명명된 값을 사용하는 방법](api-management-howto-properties.md)을 참조하세요.
>
>
