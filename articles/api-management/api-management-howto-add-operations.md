...
> URL 템플릿은 와일드 카드 패턴을 포함할 수 있습니다. 예를 들어 `/*`를 지정하면 해당 HTTP 메서드에 대한 모든 요청을 백 엔드 서비스로 전달합니다.
...
> 캐싱 및 캐싱 정책에 대한 자세한 내용은 [Azure API 관리에서 작업 결과를 캐시하는 방법][How to cache operation results in Azure API Management]을 참조하세요.
> 
> 
