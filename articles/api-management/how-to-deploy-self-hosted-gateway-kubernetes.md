...
7. **배포** 텍스트 상자의 오른쪽 아래 모서리에 있는 `kubectl` **복사** 아이콘을 선택 하 여 명령을 클립보드에 저장 합니다.
...
> [!TIP]
> <code>kubectl logs deployment/<gateway-name></code> 명령을 실행 하 여 임의로 선택한 pod에서 로그를 확인 합니다.
> 특정 <code>kubectl logs -h</code> pod 또는 컨테이너에 대 한 로그를 보는 방법 등 명령 옵션의 전체 집합에 대해를 실행 합니다.
