...
| * / 6381 - 6383              | 인바운드 및 아웃바운드 | TCP                | VIRTUAL_NETWORK / VIRTUAL_NETWORK     | 머신 간 [속도 제한](api-management-access-restriction-policies.md#LimitCallRateByKey) 정책을 위해 Redis Service에 액세스         | 외부 및 내부  |
...
* **증분 업데이트**: 네트워크를 변경할 때 [NetworkStatus API](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/networkstatus)를 참조하여 API Management 서비스에서 사용하는 중요한 리소스에 대한 액세스를 손실하지 않았는지 확인합니다. 연결 상태는 15분마다 업데이트되어야 합니다.
...
IP 주소는 **Azure 환경**으로 구분됩니다. 허용될 경우 **전역**으로 표시된 인바운드 요청 IP 주소는 **지역**별 IP 주소와 함께 허용 목록에 추가되어야 합니다.
