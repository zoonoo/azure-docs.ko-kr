...
> [!IMPORTANT]
> VNET에 사용자 지정 DNS 서버를 사용하고 있는 경우에는 API Management 서비스를 배포하기 **전에** 설정하는 것이 좋습니다. 그렇지 않으면 [네트워크 구성 작업 적용](https://docs.microsoft.com/rest/api/apimanagement/ApiManagementService/ApplyNetworkConfigurationUpdates)을 실행하여 DNS 서버를 변경할 때마다 API Management 서비스를 업데이트해야 합니다.
