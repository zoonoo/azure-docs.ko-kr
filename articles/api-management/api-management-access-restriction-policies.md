...
| 다시 시도-헤더 이름    | 지정 된 호출 비율이 초과 된 후의 값이 권장 되는 다시 시도 간격 (초) 인 응답 헤더의 이름입니다. |  아니요 | 해당 없음  |
| 다시 시도-변수-이름    | 지정 된 호출을 초과한 후 권장 된 다시 시도 간격 (초)을 저장 하는 정책 식 변수의 이름입니다. |  아니요 | 해당 없음  |
| 나머지-호출 헤더-이름    | 각 정책 실행 후의 값이에 지정 된 시간 간격에 대해 허용 되는 나머지 호출 수 인 응답 헤더의 이름입니다 `renewal-period` . |  아니요 | 해당 없음  |
| 나머지-호출 변수-이름    | 각 정책 실행 후에 지정 된 시간 간격에 대해 허용 되는 남은 호출 수를 저장 한 정책 식 변수의 이름입니다 `renewal-period` . |  아니요 | 해당 없음  |
| total-calls-header-name    | 값이에 지정 된 값을 갖는 응답 헤더의 이름입니다 `calls` . |  아니요 | 해당 없음  |
...
| increment-condition | 요청을 rate ()로 계산할지 여부를 지정 하는 부울 식 `true` 입니다.        | 아니요       | 해당 없음     |
| renewal-period      | 허용 되는 요청 수가에 지정 된 값을 초과 하지 않아야 하는 슬라이딩 윈도우의 길이 (초)입니다 `calls` .                                           | 예      | 해당 없음     |
| 다시 시도-헤더 이름    | 지정 된 호출 비율이 초과 된 후의 값이 권장 되는 다시 시도 간격 (초) 인 응답 헤더의 이름입니다. |  아니요 | 해당 없음  |
| 다시 시도-변수-이름    | 지정 된 호출을 초과한 후 권장 된 다시 시도 간격 (초)을 저장 하는 정책 식 변수의 이름입니다. |  아니요 | 해당 없음  |
| 나머지-호출 헤더-이름    | 각 정책 실행 후의 값이에 지정 된 시간 간격에 대해 허용 되는 나머지 호출 수 인 응답 헤더의 이름입니다 `renewal-period` . |  아니요 | 해당 없음  |
...
| 출력-토큰 변수-이름      | 문자열입니다. 토큰 유효성 검사가 성공 했을 때 형식의 개체로 토큰 값을 받는 컨텍스트 변수의 이름입니다. [`Jwt`](api-management-policy-expressions.md)                                                                                                                                                                                                                                                                                     | 아니요                                                                               | 해당 없음                                                                               |
