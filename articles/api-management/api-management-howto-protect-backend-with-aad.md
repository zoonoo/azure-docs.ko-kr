...
1. **리디렉션 URI** 섹션에서 `Web`을 선택 하 고 URL을 입력 `https://contoso5.portal.azure-api.net/signin`
...
1. `client-app`를 선택 하 고, 앱에 대 한 페이지 목록에서 **API 권한**으로 이동 합니다.

1. **권한 추가를**선택 합니다.

1. **API 선택**에서 `backend-app`를 찾고 선택 합니다.

1. **위임 된 권한**에서 `backend-app`에 대 한 적절 한 사용 권한을 선택한 다음 **권한 추가**를 클릭 합니다.
