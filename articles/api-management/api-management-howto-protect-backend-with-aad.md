...
1. 로 이동 합니다 [Azure portal-앱 등록](https://go.microsoft.com/fwlink/?linkid=2083908) 페이지입니다. 

2. **새 등록**을 선택합니다. 

1. **애플리케이션 등록** 페이지가 표시되면 애플리케이션의 등록 정보를 입력합니다. 
    - **이름** 섹션에서 앱의 사용자에게 표시되는 의미 있는 애플리케이션 이름(예: `backend-app`)을 입력합니다. 
    - 에 **지원 되는 계정 유형** 섹션에서 **조직 디렉터리의 계정**합니다. 

1. 유지 된 **리디렉션 URI** 지금은 빈 섹션입니다.
...
1. 에 **리디렉션 URI** 섹션에서 `Web` URL 입력 `https://contoso5.portal.azure-api.net/signin`
...
1. 클라이언트 앱에 대 한 페이지의 목록에서 선택 **인증서 및 비밀**, 선택한 **새 클라이언트 암호**합니다.

2. 아래 **클라이언트 암호 추가**를 제공는 **설명**합니다. 키 만료, 되 고 선택 되는 경우 선택할 **추가**합니다.
...
2. 선택 `client-app`를 앱에 대 한 페이지의 목록으로 이동 하 고 **API 사용 권한**합니다.

3. 선택 **권한 추가**합니다.

4. 아래 **API 선택**찾기 및 선택, `backend-app`합니다.

5. 아래 **위임 된 권한**, 적절 한 사용 권한을 선택 `backend-app`합니다.
