...
1. [Azure Portal-앱 등록](https://go.microsoft.com/fwlink/?linkid=2083908) 페이지로 이동 합니다. 

1. **새 등록**을 선택합니다. 

1. **애플리케이션 등록** 페이지가 표시되면 애플리케이션의 등록 정보를 입력합니다. 
    - **이름** 섹션에서 앱의 사용자에게 표시되는 의미 있는 애플리케이션 이름(예: `backend-app`)을 입력합니다. 
...
1. **API** 표시를 선택 하 고 **저장 및 계속** 을 클릭 하 여 응용 프로그램 ID URI를 만듭니다.

1. **범위 추가** 페이지에서 API가 지 원하는 새 범위를 만듭니다. (예: 읽기) 범위를 만들려면 *범위 추가* 를 클릭 합니다. API에서 지원 되는 모든 범위를 추가 하려면이 단계를 반복 합니다.
...
1. **애플리케이션 등록** 페이지가 표시되면 애플리케이션의 등록 정보를 입력합니다. 
    ...
    - **지원 되는 계정 유형** 섹션에서 **조직 디렉터리의 계정**을 선택 합니다. 

1. **URI 리디렉션** 섹션에서 URL을 선택 `Web` 하 고 입력 합니다.`https://contoso5.portal.azure-api.net/signin`
...
비밀을 만든 후에는 다음 단계에서 사용할 키 값을 기록해 둡니다. 
...
1. 을 `client-app`선택 하 고 앱의 페이지 목록에서 **API 권한**으로 이동 합니다.

1. **권한 추가를**선택 합니다.

1. **API 선택**에서을 찾아 선택 `backend-app`합니다.

1. **위임 된 권한**에서 적절 한 사용 권한을 `backend-app` 선택한 다음 **권한 추가**를 클릭 합니다.

1. 필요에 따라 **API 사용 권한** 페이지에서 페이지 맨 아래에 있는 **-테 넌 트-이름 > <에 대 한 관리자 동의 부여** 를 클릭 하 여이 디렉터리의 모든 사용자를 대신 하 여 동의를 부여 합니다. 
...
1. client-app의 **설정** 페이지로 돌아갑니다.

1. **회신 URL**을 선택하고 **redirect_url**을 첫 번째 행에 붙여 넣습니다. 이 예제에서는 `https://localhost`를 첫 번째 행의 URL로 바꾸었습니다.  
...
2. 보호하려는 API를 선택합니다. 예를 들어,를 사용할 `Echo API`수 있습니다.
