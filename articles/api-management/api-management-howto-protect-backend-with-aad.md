...
2. **클라이언트 암호 추가**에서 **설명을**입력 합니다. 키가 만료 되는 시점을 선택 하 고 **추가**를 선택 합니다.
...
2. 을 `client-app`선택 하 고 앱의 페이지 목록에서 **API 권한**으로 이동 합니다.

3. **권한 추가를**선택 합니다.

4. **API 선택**에서을 찾아 선택 `backend-app`합니다.

5. **위임 된 권한**에서에 대 `backend-app`한 적절 한 권한을 선택 합니다.
