...
1. 필요에 따라 **API 사용 권한** 페이지에서이 디렉터리의 모든 사용자를 대신 하 여 동의를 부여 하는 **>에 대 한 \<관리자 동의 부여** 를 선택 합니다. 
...
1. **V2** 끝점을 사용 하는 경우 **기본 범위** 필드에서 백 엔드 앱에 대해 만든 범위를 사용 합니다. 또한 [응용 프로그램 매니페스트에서](/azure/active-directory/develop/reference-app-manifest) [`accessTokenAcceptedVersion`](/azure/active-directory/develop/reference-app-manifest#accesstokenacceptedversion-attribute) 속성의 값을로 `2` 설정 해야 합니다.
...
2. 보호하려는 API를 선택합니다. 예를 들어,를 사용할 수 `Echo API`있습니다.
...
```
> [!NOTE]
> 이 `openid-config` URL은 v1 끝점에 해당 합니다. V2 `openid-config`끝점의 경우를 사용 `https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration`합니다.
