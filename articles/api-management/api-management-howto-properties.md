...
* 키 자격 증명 모음에서 업데이트 된 비밀은 API Management에서 자동으로 회전 됩니다. 키 자격 증명 모음에서 업데이트 후에는 API Management의 명명 된 값이 4 시간 이내에 업데이트 됩니다. 
...
키 자격 증명 모음에서 방화벽을 사용 하는 [Key Vault](../key-vault/general/network-security.md) 경우 주요 자격 증명 모음 비밀을 사용 하기 위한 추가 요구 사항은 다음과 같습니다.
...
* API Management 서브넷에 Azure Key Vault [서비스 끝점](../key-vault/general/overview-vnet-service-endpoints.md) 을 사용 하도록 설정 합니다.
* AzureKeyVault 및 AzureActiveDirectory [service 태그로](../virtual-network/service-tags-overview.md)의 아웃 바운드 트래픽을 허용 하도록 nsg (네트워크 보안 그룹) 규칙을 구성 합니다. 

자세한 내용은 [가상 네트워크에 연결](api-management-using-with-vnet.md#-common-network-configuration-issues)의 네트워크 구성 정보를 참조 하세요.
