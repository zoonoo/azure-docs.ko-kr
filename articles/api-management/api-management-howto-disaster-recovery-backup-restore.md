---
title: API 관리에서 백업 및 복원을 사용하여 재해 복구 구현
...
API Management 서비스를 호스트하는 지역에 영향을 주는 가용성 문제로부터 서비스를 복구하려면 언제든지 다른 지역에서 서비스를 다시 구축할 준비를 하세요. 복구 시간 목표에 따라 하나 이상의 리전에서 대기 서비스를 유지할 수 있습니다. 복구 지점 목표에 따라 해당 구성 및 콘텐츠를 활성 서비스와 동기화된 상태로 유지 관리할 수도 있습니다. 서비스 백업 및 복원 기능은 재해 복구 전략을 구현하는 데 필요한 구성 요소를 제공합니다.

백업 및 복원 작업은 개발 및 스테이징과 같은 운영 환경 간에 API 관리 서비스 구성을 복제하는 데도 사용할 수 있습니다. 사용자 및 구독과 같은 런타임 데이터도 복사되며 항상 바람직하지는 않을 수 있습니다.

이 가이드에서는 백업 및 복원 작업을 자동화하는 방법과 Azure Resource Manager에서 백업 및 복원 요청을 성공적으로 인증하는 방법을 보여 줍니다.

> [!IMPORTANT]
> 복원 작업은 대상 서비스의 사용자 지정 호스트 이름 구성을 변경하지 않습니다. 활성 및 대기 서비스 모두에 동일한 사용자 지정 호스트 이름 및 TLS 인증서를 사용 하는 것이 좋습니다., 복원 작업이 완료 된 후 간단한 DNS CNAME 변경 하 여 대기 인스턴스에 트래픽을 다시 지시할 수 있도록.
>
> 백업 작업은 Azure 포털의 애널리틱스 블레이드에 표시된 보고서에 사용된 사전 집계된 로그 데이터를 캡처하지 않습니다.
...
> [!IMPORTANT]
> 백업 및 복원을 위한 REST API는 Azure Resource Manager를 사용하며 API Management 엔터티를 관리하기 위한 REST API와는 다른 인증 메커니즘입니다. 이 섹션의 단계에는 Azure 리소스 관리자 요청을 인증하는 방법을 설명합니다. 자세한 내용은 [Azure 리소스 관리자 요청 인증](/rest/api/index)을 참조하십시오.
...
1. 응용 프로그램이 만들어지면 **API 권한을 클릭합니다.**
2. **+ 권한 추가**를 클릭합니다.
4. **마이크로소프트 API 선택을**누릅니다.
5. **Azure 서비스 관리를 선택합니다.**
6. **선택**을 누릅니다.
...
백업을 생성하고 복원하는 API를 호출하기 전에 토큰을 가져와야 합니다. 다음 예제에서는 [Microsoft.IdentityModel.Client.ActiveDirectory](https://www.nuget.org/packages/Microsoft.IdentityModel.Clients.ActiveDirectory) NuGet 패키지를 사용하여 토큰을 검색합니다.
...
1. `{tenant id}`를 사용자가 만든 Azure Active Directory 애플리케이션의 테넌트 ID로 바꿉니다. 앱 등록**끝점을**클릭하여 ID에 액세스할 수 **있습니다.** -> 
...
-   `api-version`- 로 교체`2018-06-01-preview`
...
백업 또는 복원 요청을 수행할 때 다음 제약 조건을 참고하십시오.

-   요청 본문에 지정된 **Container**가 **있어야 합니다**.
-   백업이 진행되는 동안 SKU 업그레이드 또는 다운그레이드, 도메인 이름 변경 **등과** 같은 서비스의 관리 변경을 방지합니다.
...
-   또한 다음 항목은 백업 데이터의 일부가 아닙니다: 사용자 지정 도메인 TLS/SSL 인증서 및 고객, 개발자 포털 콘텐츠 및 가상 네트워크 통합 설정에서 업로드한 중간 또는 루트 인증서입니다.
...
-   [방화벽이][azure-storage-ip-firewall] 활성화되어 있는 경우 제어 평면에서 Azure 저장소 계정에 대한 액세스를 **허용합니다.** 고객은 백업 또는 복원을 위해 저장소 계정에서 [Azure API 관리 제어 평면 IP 주소][control-plane-ip-address] 집합을 열어야 합니다. 

> [!NOTE]
> 동일한 Azure 리전에서 [방화벽이][azure-storage-ip-firewall] 활성화된 저장소 계정을 사용하여 API 관리 서비스에서 백업/복원을 수행하려고 하면 작동하지 않습니다. 이는 Azure 저장소에 대한 요청이 계산 >(Azure Api 관리 제어 평면)의 공용 IP에 SNATed되지 않기 때문입니다. 지역 간 저장소 요청이 SNATed됩니다.
...
> [!NOTE]
> 백업 및 복원 작업은 각각 PowerShell [_백업-AzApiManagement_](/powershell/module/az.apimanagement/backup-azapimanagement) 및 [_복원-AzApiManagement_](/powershell/module/az.apimanagement/restore-azapimanagement) 명령을 사용 하 여 수행할 수 있습니다.
...
-   [Azure API 관리: 백업 및 구성](https://blogs.msdn.com/b/stuartleeks/archive/2015/04/29/azure-api-management-backing-up-and-restoring-configuration.aspx)
    복원_Stuart에 의해 자세히 설명된 접근 방식은 공식 지침과 일치하지 않지만 흥미롭습니다._
