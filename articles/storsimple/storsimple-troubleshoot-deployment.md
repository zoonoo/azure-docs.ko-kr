...
4. 장치 등록: Microsoft Azure에서 실행되는 StorSimple 관리자 서비스를 사용하여 장치를 등록하는 최종 단계입니다. 등록하려면 Azure 관리 포털에서 [서비스 등록 키를 받고](storsimple-manage-service.md#get-the-service-registration-key) 설치 마법사에 키를 입력해야 합니다. 장치가 성공적으로 등록되면 서비스 데이터 암호화 키가 제공됩니다. 해당 서비스로 모든 후속 장치 서비스를 등록할 때 필요하기 때문에 이 암호화 키를 안전한 위치에 보관해야 합니다.
