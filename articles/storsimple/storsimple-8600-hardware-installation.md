...
- 클라우드 액세스에 대한 하나의 네트워크 인터페이스 및 iSCSI에 대한 하나의 네트워크 인터페이스로 각 컨트롤러에 연결된 적어도 두 개의 네트워크 인터페이스. DATA 0 포트는 장치의 직렬 콘솔을 통해 자동으로 활성화 및 구성됩니다. DATA 0 이외에도 관리 포털을 통해 다른 데이터 포트를 구성해야 합니다. 이 경우 DATA 0 포트를 기본 LAN(인터넷에 연결된 네트워크)에 연결합니다. 의도한 역할에 따라 다른 데이터 포트를 네트워크의 SAN/iSCSI LAN(VLAN) 세그먼트에 연결할 수 있습니다.
