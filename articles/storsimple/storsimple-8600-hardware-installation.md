---
title: Microsoft Azure StorSimple 8600 디바이스 설치 | Microsoft Docs
...
다음 단계는 8600 StorSimple 저장 디바이스를 개봉하는 방법을 명확하고 자세하게 설명합니다. 이 디바이스는 2개의 상자에 제공되는데 하나에는 기본 인클로저, 다른 하나에는 EBOD 인클로저가 들어 있습니다. 이러한 두 상자는 단일 상자에 배치됩니다.
...
2. 외부 상자를 연 다음 기본 인클로저와 EBOD 인클로저에 해당하는 두 상자를 꺼냅니다. 이제 기본 인클로저와 EBOD 인클로저를 개봉할 수 있습니다. 다음 단계는 인클로저 중 하나의 개봉된 상태를 보여 줍니다.
    ...
    ![저장 디바이스 개봉하기](./media/storsimple-8600-hardware-installation/HCSUnpackyour4Udevice.png)
...
다음 단계를 따라 전면/후면 포스트가 있는 표준 19인치 랙에 StorSimple 8600 저장 디바이스를 설치하세요. 이 디바이스에는 기본 인클로저 및 EBOD 인클로저, 두 개의 인클로저가 있습니다. 양쪽 모두 랙에 탑재해야 합니다.
...
> [!NOTE]
> **정전 시 관리:** 장치가 원격 데이터 센터에서 또는 액세스가 제한 된 컴퓨터실에 설치 되 면 두 컨트롤러에 대 한 직렬 연결이 항상 직렬 콘솔 스위치 또는 유사한 장비에 연결 되어 있는지 확인 합니다. 이렇게 하면 네트워크 중단 또는 예기치 않은 장애 발생 시 대역 외 원격 제어 및 지원 작업이 허용됩니다.
> 
> 
