...
**범위** 창의 **볼륨 그룹** 노드를 사용하여 볼륨 그룹에 볼륨을 할당하고, 볼륨 그룹에 대한 정보를 보고, 백업을 예약하고, 볼륨 그룹을 편집할 수 있습니다.
...
**그림 1: StorSimple 스냅샷 관리자 볼륨 그룹 노드** 
...
2. **범위** 창에서 **볼륨 그룹** 을 마우스 오른쪽 단추로 클릭하고 **볼륨 그룹 만들기** 를 클릭합니다.
...
3. 다음 정보를 입력합니다.
   ...
   3. **볼륨** 상자에서 볼륨 그룹에 추가할 StorSimple 볼륨을 선택합니다. 
      ...
      * 단일 또는 다중 파티션이 있는 볼륨을 포함할 수 있습니다. (다중 파티션 볼륨은 다중 파티션이 있는 기본 디스크 또는 동적 디스크일 수 있습니다.) 여러 파티션을 포함하는 볼륨은 단일 단위로 처리됩니다. 따라서 파티션 중 하나만 볼륨 그룹에 추가해도 다른 모든 파티션이 해당 볼륨 그룹에 동시에 자동으로 추가됩니다. 볼륨 그룹에 다중 파티션 볼륨을 추가한 후에도 해당 다중 파티션 볼륨은 계속 단일 단위로 취급됩니다.
...
3. **백업 수행** 대화 상자에서 **로컬 스냅숏** 또는 **클라우드 스냅숏** 을 선택하고 **만들기** 를 클릭합니다.
   
    ![백업 수행 대화 상자](./media/storsimple-snapshot-manager-manage-volume-groups/HCS_SSM_TakeBackup_dialog.png)
4. 백업이 실행 중인 것을 확인하려면 **작업** 노드를 확장하고 **실행 중** 을 클릭합니다. 백업이 나열됩니다.
...
* [StorSimple 스냅샷 관리자를 사용하여 백업 정책을 만들고 관리](storsimple-snapshot-manager-manage-backup-policies.md)하는 방법을 알아봅니다.
