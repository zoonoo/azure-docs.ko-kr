...
다음 표에서는 이전 릴리스에서 언급된 문제 릴리스를 포함하여 StorSimple 가상 배열에 대해 알려진 문제를 간략하게 제공합니다. **이 릴리스에서 언급한 문제 릴리스에는 별표가 표시됩니다. 이 목록의 거의 모든 문제는 StorSimple 가상 배열의 GA 릴리스에서 이월되었습니다.**
...
StorSimple 가상 어레이에 [업데이트를 설치합니다.](storsimple-ova-install-update-01.md)
