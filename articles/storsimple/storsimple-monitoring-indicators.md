...
| 녹색 |ON |ON |N/A |ops 패널 전원 켜기(5초) 상태 테스트 |
| 녹색 |끄기 |끄기 |N/A |전원이 켜짐, 모든 기능이 정상 상태임 |
| 녹색 |ON |N/A |PCM 결함 LED, 팬 결함 LED |모든 PCM 결함, 팬 결함, 온도 초과 또는 미만 |
| 녹색 |ON |N/A |I/O 모듈 LED |모든 컨트롤러 모듈 결함 |
| 녹색 |ON |N/A |N/A |인클로저 논리적 결함 |
...
| AC 전원 없음(이 PCM만 해당) |끄기 |ON |끄기 |ON |
| AC 있음 PCM 켜짐-정상 |ON |OFF |OFF |끄기 |
| PCM 오류(팬 오류) |끄기 |끄기 |ON |N/A |
| PCM 결함(과도 진폭, 과전압, 과전류) |끄기 |ON |ON |켜기 |
| PCM(팬이 허용 오차를 벗어남) |ON |OFF |끄기 |ON |
| 대기 모드 |깜박임 |끄기 |OFF |끄기 |
...
| SES(SCSI 인클로저 서비스) 디바이스 ID 집합 |ON |깜박임 1초 켜기/1초 끄기 |없음 |
| SES 디바이스 결함 비트 집합 |ON |ON |논리적 결함(빨간색) |
| 전원 제어 회로 오류 |끄기 |ON |모듈 결함(빨간색) |
