...
- **정책 이름** – 정책과 연결된 이름입니다. 다음과 같은 다양한 유형의 정책이 포함됩니다.
   ...
   - 볼륨 생성 시 이 볼륨 옵션에 대한 기본 백업이 활성화되면 만들어지는 자동 정책입니다. 이 정책은 볼륨 이름이 관리 포털에서 사용자가 구성한 StorSimple 볼륨의 이름으로 참조되는 VolumeName_Default로 이름이 지정됩니다. 자동 정책을 사용하면 22시 30분 장치 시간에서 시작하는 일일 클라우드 스냅숏이 발생됩니다.
