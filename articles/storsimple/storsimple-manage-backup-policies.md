...
- **정책 이름** – 정책과 연결된 이름입니다. 다음과 같은 다양한 유형의 정책이 포함됩니다.
   ...
   - 볼륨 생성 시 이 볼륨 옵션에 대한 기본 백업이 활성화되면 만들어지는 자동 정책입니다. 이 정책은 볼륨 이름이 관리 포털에서 사용자가 구성한 StorSimple 볼륨의 이름으로 참조되는 VolumeName\_Default로 이름이 지정됩니다. 자동 정책을 사용하면 22시 30분 장치 시간에서 시작하는 일일 클라우드 스냅숏이 발생됩니다.
...
StorSimple 장치의 기존 백업 정책에 첨부된 일정을 추가하거나 수정할 수 있습니다. 일정을 추가하거나 수정하려면 관리 포털에서 다음 단계를 수행합니다.

[AZURE.INCLUDE [storsimple-add-modify-backup-schedule](../../includes/storsimple-add-modify-backup-schedule.md)]

## 백업 정책 삭제

StorSimple 장치에 대한 백업 정책을 삭제하려면 관리 포털에서 다음 단계를 수행합니다.
...
단일 볼륨에 대한 주문형(수동) 백업을 만들려면 관리 포털에서 다음 단계를 수행합니다.

[AZURE.INCLUDE [storsimple-create-manual-backup](../../includes/storsimple-create-manual-backup.md)]

## 여러 볼륨과 일정의 사용자 지정 백업 정책 만들기

여러 볼륨 및 일정이 있는 사용자 지정 백업 정책을 만들려면 관리 포털에서 다음 단계를 수행합니다.
