...
> [AZURE.NOTE]서비스 등록 키는 100자 이상의 문자를 포함 하는 긴 키입니다. 키를 복사해서 안전한 위치에 텍스트 파일로 저장해두면 필요에 따라 추가 장치를 인증하는 데 사용할 수 있습니다.
> 
...
자세한 내용을 보고 CHAP를 구성하려면 [StorSimple 장치에 대한 CHAP 구성](storsimple-configure-chap.md)으로 이동하세요.
...
> [AZURE.IMPORTANT]<ul><li>이 암호는 14-15자여야 하며 3자 이상의 대문자, 소문자, 숫자 및 특수 문자의 조합이 포함되어야 합니다.</li><li>StorSimple 스냅숏 관리자 암호를 설정한 후 변경할 수 있지만 검색할 수는 없습니다. 암호를 변경하는 경우 모든 원격 사용자에게 알려야 합니다.</li></ul>
...
> * 서비스 데이터 암호화 키 및 데이터 암호화 인증서가 만료되지 않았습니다. 그러나 키 손상을 방지하려면 매년 서비스 데이터 암호화 키를 변경하는 것이 좋습니다. </li></ul>


## 휴지 상태의 데이터 보호

StorSimple 장치는 사용 빈도에 따라 데이터를 로컬과 클라우드 계층에 저장하여 관리합니다. 장치에 연결된 모든 호스트 컴퓨터는 데이터를 장치로 보낸 다음 적절한 경우 클라우드로 이동합니다. 데이터는 인터넷을 통해 장치에서 클라우드로 안전하게 전송됩니다. 각 장치에는 해당 장치의 모든 공유 볼륨을 표시하는 하나의 iSCSI 대상이 있습니다. 클라우드 저장소에 전송되기 전에 모든 데이터는 암호화 됩니다. 클라우드로 이동 하는 데이터의 무결성을 보장 하기 위해 Azure StorSimple는 다음과 같이 클라우드 저장소 암호화 키를 정의할 수 있습니다.
...
- 분기별로 클라우드 저장소 암호화 키를 순환하는 것이 좋습니다. 이러한 키의 순환은 강제 적용되지 않습니다.
