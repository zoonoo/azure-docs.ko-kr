...
Microsoft Azure StorSimple 가상 배열은 하이퍼바이저 및 Microsoft Azure 클라우드 스토리지에서 실행되는 온-프레미스 가상 배열 간의 스토리지 작업을 관리하는 통합 스토리지 솔루션입니다. 가상 배열은 다양한 문제와 엔터프라이즈 스토리지 및 데이터 보호와 관련된 비용을 해소하는 효율적이고, 비용 효과적이며, 관리가 용이한 파일 서버 또는 iSCSI 서버 솔루션입니다. 가상 배열은 자주 액세스하지 않는 보관 데이터를 스토리지하는 데 특히 적합합니다.
...
| 가용성 |단일 노드 |
| 총 용량(클라우드 포함) |가상 배열 당 최대 64 TB의 사용 가능한 용량 |
| 로컬 용량 |가상 배열 당 390 g b에서 6.4 TB의 용량을 사용할 수 있습니다 (500 GB ~ 8TB의 디스크 공간을 프로 비전 해야 합니다). |
...
StorSimple은 애플리케이션을 수정하지 않고 사용자와 서버를 몇 분 안에 Azure Storage에 연결합니다.
...
| 스토리지 비용 감소 |StorSimple을 통해 가장 많이 사용되는 핫 데이터에 대한 현재 요구를 충족하기에 충분한 로컬 스토리지를 프로비전합니다. 스토리지에 대한 수요가 증가하면, StorSimple은 콜드 데이터를 비용 효과적인 클라우드 스토리지로 계층화합니다. 스토리지 요구 사항 및 비용을 좀 더 줄이기 위해 데이터를 클라우드로 보내기 전에 중복 제거 및 압축이 진행됩니다. |
| 단순화된 스토리지 관리 |StorSimple은 StorSimple Device Manager를 사용하여 여러 디바이스를 관리하는 중앙 집중식 관리를 클라우드에 제공합니다. |
| 향상된 재해 복구 및 규정 준수 |StorSimple은 메타데이터를 즉시 복원하고 필요에 따라 데이터를 복원하여 보다 빠른 재해 복구를 지원합니다. 정상적인 작업은 중단을 최소화 하면서 계속할 수 있습니다. |
...
StorSimple 가상 배열은 자주 액세스되지 않는 데이터에 가장 적합합니다. 가상 배열에는 성능 향상을 위한 로컬 캐시가 있지만 사용자는 디바이스가 스토리지의 가장 낮은 계층(클라우드)에서 파일을 서비스한다고 가정해야 합니다. 각 가상 배열은 약 100Mbps로 Azure Storage에 쓰고 읽을 수 있습니다. 해당 링크는 디바이스에 들어오는 모든 요청 간에 공유되며 아래 다이어그램에 나와 있는 것처럼 병목 상태가 될 수 있습니다.

![클라우드 보관](./media/storsimple-ova-overview/cloud-archiving.png)

여러 동시 사용자가 가상 배열에 액세스 하는 경우 모든 사용자가 Azure에 대 한 연결을 공유 하 여 성능 저하를 초래 합니다. 사용자 당 성능이 보장 되지 않으며 장치는 도착 하는 개별 요청을 처리 합니다.

StorSimple 가상 배열은 고가용성이 필요한 워크로드에는 적합하지 않습니다. 가상 배열은 소프트웨어 업데이트를 설치할 때 가동 중지 시간이 발생 하는 단일 노드 장치입니다. 관리자는 1년에 3-4회의, 30분 유지 관리 기간을 계획해야 합니다.

## <a name="workflows"></a>워크플로

StorSimple 가상 배열은 다음과 같은 워크플로에 특히 적합합니다.

* [클라우드 기반 스토리지 관리](#cloud-based-storage-management)
* [위치와 무관한 백업](#location-independent-backup)
* [데이터 보호 및 재해 복구](#data-protection-and-disaster-recovery)

### <a name="cloud-based-storage-management"></a>클라우드 기반 스토리지 관리
Azure Portal에서 실행되는 StorSimple Device Manager 서비스를 사용하여 여러 디바이스와 여러 위치에 저장된 데이터를 관리할 수 있습니다. 이는 분산 된 분기 시나리오에서 특히 유용 합니다. 가상 배열과 물리적 StorSimple 장치를 관리 하려면 StorSimple 장치 관리자 서비스의 개별 인스턴스를 만들어야 합니다. 가상 배열은 이제 Azure 클래식 포털 대신 새로운 Azure Portal를 사용 합니다.<!--Is the "now" element still in date? Could it go at this point? Just checking.-->

![클라우드 기반 스토리지 관리](./media/storsimple-ova-overview/cloud-based-storage-management.png)
...
* **항목 수준 복구** – 공유를 통해 최근 백업에 간편하게 액세스할 수 있습니다. 클라우드에서 사용 가능한 특수 한 *백업* 폴더에서 개별 파일을 쉽게 복구할 수 있습니다. 이러한 복원 기능은 사용자를 기반으로 하며 관리자의 개입이 필요하지 않습니다.
...
* [가상 배열](#virtual-array) – 가상화 된 환경 또는 하이퍼바이저에 프로 비전 된 가상 머신을 기반으로 하는 하이브리드 클라우드 저장 장치입니다.
* [StorSimple 디바이스 관리자 서비스](#storsimple-device-manager-service) – 다양한 지리적 위치에서 액세스할 수 있는 단일 웹 인터페이스에서 하나 이상의 StorSimple 디바이스를 관리할 수 있는 Azure Portal의 확장입니다. StorSimple Device Manager 서비스를 사용하여 서비스를 만들고 관리하고, 디바이스와 경고를 보고 관리하고, 볼륨, 공유, 기존 스냅샷을 관리할 수 있습니다.
* [로컬 웹 사용자 인터페이스](#local-web-user-interface) – 로컬 네트워크에 연결할 수 있도록 장치를 구성 하 고 StorSimple 장치 관리자 서비스에 장치를 등록 하는 데 사용 되는 웹 기반 UI입니다. 
* [명령줄 인터페이스](#command-line-interface) – 가상 배열의 지원 세션을 시작 하는 데 사용할 수 있는 Windows PowerShell 인터페이스입니다.
  다음 섹션에서는 각 구성 요소에 대해 자세히 설명 하 고 솔루션이 데이터를 정렬 하 고 저장소를 할당 하 고 저장소 관리 및 데이터 보호를 용이 하 게 하는 방법을 설명 합니다.

### <a name="virtual-array"></a>가상 배열

가상 배열은 기본 저장소를 제공 하 고, 클라우드 저장소와의 통신을 관리 하 고, 장치에 저장 된 모든 데이터의 보안과 기밀성을 보장 하는 데 도움이 되는 단일 노드 저장소 솔루션입니다.

가상 배열은 다운로드가 가능한 하나의 모델로 제공됩니다. 가상 배열에 대해 디바이스의 최대 용량은 6.4TB(기본 스토리지 요구 사항 8TB)이고 클라우드 스토리지를 포함하여 64TB입니다.

가상 배열에는 다음과 같은 기능이 있습니다.

* 비용 효율성이 높습니다. 기존 가상화 인프라를 활용하며 기존 Hyper-V 또는 VMware 하이퍼바이저에 배포될 수 있습니다.
...
Microsoft Azure StorSimple는 storsimple 저장소를 중앙에서 관리할 수 있는 웹 기반 사용자 인터페이스인 StorSimple 장치 관리자 서비스를 제공 합니다. 다음 작업을 수행하는 데 StorSimple Device Manager 서비스를 사용할 수 있습니다.

* 단일 서비스에서 다수의 StorSimple 가상 배열을 관리합니다.
* StorSimple 가상 배열에 대한 보안 설정을 구성하고 관리합니다. (클라우드의 암호화는 Microsoft Azure API에 따라 달라집니다.)
* 스토리지 계정 자격 증명 및 속성을 구성합니다.
* 볼륨 또는 공유를 구성하고 관리합니다.
* 볼륨 또는 공유의 데이터를 백업하고 복원합니다.
* 성능을 모니터링합니다.
* 시스템 설정을 검토하고 발생 가능한 문제를 식별합니다.

StorSimple 장치 관리자 서비스를 사용 하 여 가상 배열의 일상적인 관리를 수행 합니다.
...
포함 된 Windows PowerShell 인터페이스를 사용 하면 사용자가 가상 배열에서 발생할 수 있는 문제를 해결 하는 데 도움이 될 수 있도록 Microsoft 지원 지원 세션을 시작할 수 있습니다.

## <a name="storage-management-technologies"></a>스토리지 관리 기술

가상 배열 및 기타 구성 요소 외에도 StorSimple 솔루션은 다음과 같은 소프트웨어 기술을 사용 하 여 중요 한 데이터에 빠르게 액세스 하 고, 저장소 사용량을 줄이고, 가상 배열에 저장 된 데이터를 보호 합니다.
...
계층화 된 특정 공유 또는 볼륨에 대 한 데이터는 자체 로컬 계층 공간이 보장 됩니다 (해당 공유 또는 볼륨에 대해 프로 비전 된 총 공간의 약 10%). 이렇게 하면 해당 공유 또는 볼륨에 대 한 가상 배열에서 사용 가능한 저장소를 줄일 수 있지만, 한 공유 또는 볼륨에 대 한 계층화가 다른 공유 또는 볼륨의 계층화 요구에 영향을 받지 않습니다. 따라서 한 공유 또는 볼륨에서 매우 바쁜 작업은 다른 모든 워크 로드를 클라우드에 적용할 수 없습니다.

iSCSI용으로 만든 계층화된 볼륨은 볼륨의 크기에 관계 없이 200GB의 최대 로컬 예약 공간을 포함합니다.

![자동 스토리지 계층화](./media/storsimple-ova-overview/automatic-storage-tiering.png)
...
StorSimple은 중복 제거 및 데이터 압축을 사용하여 클라우드의 스토리지 요구 사항 많이 감소시킵니다. 중복 제거는 저장된 데이터 집합에서 중복을 제거하여 저장된 데이터의 전체 크기를 줄일 수 있습니다. 정보가 변경되면 StorSimple은 변경되지 않은 데이터를 무시하고 변경 내용만 캡처합니다. 또한 StorSimple은 중복 정보를 식별하고 제거하여 저장된 데이터의 양을 줄입니다.
...
StorSimple 데이터 보호 기능을 통해 주문형 백업을 만들 수 있습니다. 또한, 기본 백업 일정은 매일 데이터가 백업되도록 보장합니다. Backup은 증분 스냅샷의 형태로 생성되고 클라우드에 저장됩니다. 스냅샷은 마지막 백업 이후의 변경 사항만을 기록하기 때문에 신속한 생성과 복원이 가능합니다. 이러한 스냅샷은 보조 스토리지 시스템(예: 테이프 백업)을 대체하기 때문에 재해 복구 시나리오에서 매우 중요할 수 있으며, 필요한 경우 데이터 센터 또는 대체 사이트에 데이터를 복원할 수 있게 해줍니다.
...
가상 계열에 대한 StorSimple 디바이스 관리자는 두 가지 키 인스턴스에서 개인 정보를 수집합니다.
 ...
 - 공유의 데이터에 액세스할 수 있는 사용자입니다. 공유 데이터에 액세스할 수 있는 사용자의 목록을 표시하고 내보낼 수 있습니다. 이 목록은 공유가 삭제 될 때 삭제 됩니다.

자세한 내용은 [보안 센터에서 Microsoft 개인 정보 취급 방침](https://www.microsoft.com/trustcenter)을 검토 하세요.
