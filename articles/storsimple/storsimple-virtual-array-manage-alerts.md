...
가상 배열에 메일 알림을 사용하도록 설정한 후에 알림 목록의 멤버는 중요한 경고가 발생할 때마다 메일 메시지를 받게 됩니다. 메시지를 보내는 *storsimple-경고-noreply\@mail.windowsazure.com* 및 경고 조건에 설명 합니다. 받는 사람은 **구독 취소**를 클릭하여 메일 알림 목록에서 제거할 수 있습니다.
...
3. **일반 설정** 블레이드에서 **경고 설정** 섹션으로 이동하고 다음 항목을 설정합니다.
   ...
   3. **추가 메일 받는 사람** 필드에 경고 알림을 받을 다른 모든 받는 사람의 메일 주소를 입력합니다. 형식으로 이름을 입력 *누군가\@somewhere.com*합니다. 세미콜론을 사용하여 메일 주소를 구분합니다. 가상 디바이스당 최대 20개의 메일 주소를 구성할 수 있습니다.
...
| 경고 텍스트 | 행사 | 자세한 내용 / 권장 작업 |
|:--- |:--- |:--- |
| <*디바이스 이름*\>의 백업을 완료할 수 없습니다. |Backup 작업이 실패했습니다. |백업을 만들 수 없습니다. 다음 중 하나를 고려하십시오.<ul><li>연결 문제가 성공적인 백업 작업을 방해할 수 있습니다. 연결 문제가 없는지 확인합니다. 연결 문제 해결에 대한 자세한 내용은 가상 디바이스의 [로컬 웹 UI](storsimple-ova-web-ui-admin.md)로 이동합니다.</li><li>사용 가능한 스토리지 공간의 한도에 도달했습니다. 공간을 확보하려면 더 이상 필요 없는 모든 백업을 삭제하는 것이 좋습니다.</li></ul> 문제를 해결하고 경고를 지우고 작업을 다시 시도합니다. |
