---
title: StorSimple 가상 어레이에 대한 경고 보기 및 관리
...
가상 배열에 메일 알림을 사용하도록 설정한 후에 알림 목록의 멤버는 중요한 경고가 발생할 때마다 메일 메시지를 받게 됩니다. 메시지는 *\@storsimple-alerts-noreply mail.windowsazure.com* 전송되며 경고 조건을 설명합니다. 받는 사람은 **구독 취소**를 클릭하여 메일 알림 목록에서 자신을 제거할 수 있습니다.
...
3. **일반 설정** 블레이드에서 **경고 설정** 섹션으로 이동하고 다음 항목을 설정합니다.
   ...
   3. **추가 메일 받는 사람** 필드에 경고 알림을 받을 다른 모든 받는 사람의 메일 주소를 입력합니다. *somewhere.com\@* 있는 형식에 이름을 입력합니다. 세미콜론을 사용하여 메일 주소를 구분합니다. 가상 디바이스당 최대 20개의 메일 주소를 구성할 수 있습니다.
...
| *장치 이름에*\>프로비저닝된 디스크 공간이 부족하면 <. |디스크 공간 경고입니다. |프로비전된 디스크 공간이 부족합니다. 공간을 확보하려면 다른 볼륨으로 작업을 이동하거나 데이터 공유 또는 삭제하는 것이 좋습니다. |

### <a name="job-failure-alerts"></a>작업 실패 경고

| 경고 텍스트 | 행사 | 자세한 내용 / 권장 작업 |
|:--- |:--- |:--- |
| *<장치 이름* \> 백업을 완료할 수 없습니다. |Backup 작업이 실패했습니다. |백업을 만들 수 없습니다. 다음 중 하나를 고려하십시오.<ul><li>연결 문제는 백업 작업이 성공적으로 완료되지 않도록 막을 수 있습니다. 연결 문제가 없는지 확인합니다. 연결 문제 해결에 대한 자세한 내용은 가상 디바이스의 [로컬 웹 UI](storsimple-ova-web-ui-admin.md)로 이동합니다.</li><li>사용 가능한 스토리지 공간의 한도에 도달했습니다. 공간을 확보하려면 더 이상 필요 없는 모든 백업을 삭제하는 것이 좋습니다.</li></ul>  문제를 해결하고 경고를 지우고 작업을 다시 시도합니다. |
| <장치 *이름* \> 복제를 완료할 수 없습니다. |복제 작업이 실패했습니다. |복제본을 만들 수 없습니다. 다음 중 하나를 고려하십시오.<ul><li>백업 목록이 잘못되었을 수 있습니다. 목록을 새로고쳐서 여전히 유효한지 확인하십시오.</li><li>연결 문제는 성공적인 복제 작업을 방해할 수 있습니다. 연결 문제가 없는지 확인합니다.</li><li>사용 가능한 스토리지 공간의 한도에 도달했습니다. 공간을 확보하려면 더 이상 필요 없는 모든 백업을 삭제하는 것이 좋습니다.</li></ul> 문제를 해결하고 경고를 지우고 작업을 다시 시도합니다. |
...
| *장치 이름에서*\>로컬 예약 디스크 공간이 부족<. |느린 응답 시간. |<*장치 이름에* \> 대한 전체 프로비저닝 된 크기의 10 %는 로컬 장치에서 예약되어 있으며 이제 예약 된 공간에서 부족합니다. <장치 *이름의* \> 워크로드로 인해 이탈률이 높아지거나 최근에 많은 양의 데이터를 마이그레이션했을 수 있습니다. 이로 인해 성능이 저하될 수 있습니다. 다음 작업 중 하나를 고려하여 이것을 해결하십시오.<ul><li>이 디바이스에 대한 클라우드 대역폭을 늘립니다.</li><li>워크로드를 줄이거나 다른 볼륨 또는 공유에 이동시킵니다.</li></ul> |

### <a name="security-alerts"></a>보안 경고

| 경고 텍스트 | 행사 | 자세한 내용 / 권장 작업 |
|:--- |:--- |:--- |
| <*장치 이름에* \> 대한 암호는 *<번호* \> 일 에서 만료됩니다. |암호 경고. |암호는 *<번호* \> 일 이내에 만료됩니다. 암호를 변경하는 것이 좋습니다. 자세한 내용은 [StorSimple Virtual Array 디바이스 관리자 암호를 변경](storsimple-virtual-array-change-device-admin-password.md)으로 이동합니다. |
