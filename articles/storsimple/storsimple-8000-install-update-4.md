...
* [핫픽스를 다운로드하려면](#to-download-hotfixes)에 나열된 단계를 사용하여 유지 관리 모드 업데이트를 다운로드한 후 KB4011837을 검색한 후 다운로드합니다. 이 KB는 디스크 펌웨어 업데이트를 설치합니다(다른 업데이트가 이미 설치되어 있어야 함). 설치에 나열된 단계를 따르고 [유지 관리 모드 핫픽스를 확인하여](#to-install-and-verify-maintenance-mode-hotfixes) 유지 관리 모드 업데이트를 설치합니다.
