...
## <a name="connect-to-the-storsimple-manager-service"></a>StorSimple 관리자 서비스에 연결
...
## <a name="navigate-the-storsimple-manager-service-ui"></a>StorSimple 관리자 서비스 UI로 이동
...
## <a name="use-the-storsimple-manager-service-to-perform-management-tasks"></a>StorSimple 관리자 서비스를 사용하여 관리 작업 수행
...
| --- | --- | --- |
| 서비스 만들기</br>서비스 삭제</br>서비스 등록 키 가져오기</br>서비스 등록 키 다시 생성 |StorSimple 관리자 서비스 |[StorSimple 관리자 서비스 배포](storsimple-ova-manage-service.md) |
...
| 재해 복구 및 장치 장애 조치(Failover)</br>장애 조치 필수 구성 요소</br>가상 장치에 대한 장애 조치</br>비즈니스 연속성 재해 복구(BCDR)</br>재해 복구 중 오류 |StorSimple Manager 서비스 → 장치 |[StorSimple 가상 배열에 대한 재해 복구 및 장치 장애 조치(failover)](storsimple-ova-failover-dr.md) |
| 공유 및 볼륨 백업</br>수동 백업 수행</br>백업 일정 변경</br>기존 백업 확인 |StorSimple 관리자 서비스 → 백업 카탈로그 |[StorSimple 가상 배열 백업](storsimple-ova-backup.md) |
| 백업 세트에서 공유 복원</br>백업 세트에서 볼륨 복원</br>항목 수준 복구(파일 서버에만 해당) |StorSimple 관리자 서비스 → 백업 카탈로그 |[StorSimple 가상 배열의 백업에서 복원](storsimple-ova-restore.md) |
| Storage 계정 정보</br>저장소 계정 추가</br>저장소 계정 편집</br>저장소 계정 삭제 |StorSimple Manager 서비스 → 구성 |[StorSimple 가상 배열에 대한 저장소 계정 관리](storsimple-ova-manage-storage-accounts.md) |
...
| 소프트웨어 업데이트 설치 |StorSimple Manager 서비스 → 장치 → 유지 관리 |[가상 배열 업데이트](storsimple-ova-install-update-01.md) |
