...
| 15 |Azure PowerShell cmdlet 및 로컬에 고정된 볼륨 |Azure PowerShell cmdlet을 통해 로컬에 고정된 볼륨을 만들 수 없습니다. (Azure PowerShell을 통해 만드는 모든 볼륨은 계층화됩니다.) 또한 계층화된 볼륨 형식을 수정하는 원하지 않는 영향을 미치기 때문에 Azure PowerShell cmdlet를 사용하지 않아서 로컬로 고정된 볼륨의 속성을 수정하는 않아야 합니다. |로컬에 고정된 볼륨을 구성 및 수정하려면 항상 StorSimple 관리자 서비스를 사용하세요.  |예 |아니요 |
...
| 22 |업데이트 |업데이트 3을 적용하는 경우 Azure 클래식 포털의 **유지 관리** 페이지에 업데이트 2와 관련하여 다음과 같은 메시지가 표시됩니다. – “StorSimple 8000 시리즈 업데이트 2에는 잠재적인 문제가 검색되는 경우 Microsoft에서 사용자의 장치에서 로그 정보를 적극적으로 수집할 수 있는 기능이 포함되어 있습니다.” 장치가 업데이트 2로 업데이트 중이라는 것을 나타내기 때문에 이것을 잘못된 메시지입니다. 장치가 업데이트 3으로 업데이트된 후에 이 메시지가 사라집니다. |이 동작은 이후 릴리스에서 수정될 예정입니다. |예 |아니요 |
