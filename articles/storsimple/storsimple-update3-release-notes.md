...
| 22 |Updates |업데이트 3을 적용하는 경우 Azure 클래식 포털의 **유지 관리** 페이지에 업데이트 2와 관련하여 다음과 같은 메시지가 표시됩니다. – "StorSimple 8000 시리즈 업데이트 2에는 잠재적인 문제가 검색되는 경우 Microsoft에서 사용자의 디바이스에서 로그 정보를 적극적으로 수집할 수 있는 기능이 포함되어 있습니다." 디바이스가 업데이트 2로 업데이트 중이라는 것을 나타내기 때문에 이것을 잘못된 메시지입니다. 디바이스가 업데이트 3으로 업데이트된 후에 이 메시지가 사라집니다. |이 동작은 이후 릴리스에서 수정될 예정입니다. |예 |아니요 |
