...
* **볼륨을 오프라인해야 합니다.** – 복원 작업을 시작하기 전에 호스트와 디바이스 모두에서 볼륨을 오프라인해야 합니다. 복원 작업은 디바이스에서 볼륨을 온라인으로 자동으로 전환하지만 호스트에서 디바이스를 수동으로 온라인 상태로 만들어야 합니다. 볼륨이 디바이스에서 온라인이 되는 즉시 호스트에서 볼륨을 온라인으로 전환할 수 있습니다. 복원 작업이 완료될 때까지 기다릴 필요가 없습니다. 프로시저의 경우 [볼륨을 오프라인으로 가져가십시오.](storsimple-8000-manage-volumes-u2.md#take-a-volume-offline)
...
리하이드레이션 작업을 취소하려면 PowerShell cmdlet을 사용하면 됩니다. 향후 모든 복원에 대해 리하이드레이션 작업을 영구적으로 비활성화하려면 [Microsoft 지원에 문의하십시오.](storsimple-8000-contact-microsoft-support.md)
...
7. **복원을 클릭합니다.** 그러면 **작업** 페이지에 액세스하여 볼 수 있는 복원 작업이 시작됩니다.
