...
* **볼륨을 오프라인해야 합니다.** – 복원 작업을 시작하기 전에 호스트와 장치 모두에서 볼륨을 오프라인해야 합니다. 복원 작업은 디바이스에서 볼륨을 온라인으로 자동으로 전환하지만 호스트에서 디바이스를 수동으로 온라인 상태로 만들어야 합니다. 볼륨이 디바이스에서 온라인이 되는 즉시 호스트에서 볼륨을 온라인으로 전환할 수 있습니다. (복원 작업이 완료될 때까지 기다릴 필요가 없습니다.) 절차를 보려면 [볼륨을 오프라인으로 전환](storsimple-8000-manage-volumes-u2.md#take-a-volume-offline)으로 이동합니다.
