...
Microsoft Azure StorSimple 가상 배열은 하이퍼바이저 및 Microsoft Azure 클라우드 저장소에서 실행되는 온-프레미스 가상 장치 간의 저장소 작업을 관리하는 통합 저장소 솔루션입니다. StorSimple 가상 배열은 8000 시리즈 물리적 배열을 대신하는 효율적이고 경제적인 대체 수단입니다. 가상 배열은 기존 하이퍼바이저 인프라에서 실행할 수 있고, iSCSI와 SMB 프로토콜을 모두 지원하며, 특히 원격 사무실/지점 시나리오에 적합합니다. StorSimple 솔루션에 대한 자세한 내용은 [Microsoft Azure StorSimple 개요](https://www.microsoft.com/ko-KR/server-cloud/products/storsimple/overview.aspx)를 참조하세요.

이 문서는 StorSimple 가상 배열의 초기 설치, 배포 및 관리에서 구현된 모범 사례를 다룹니다. 이러한 모범 사례는 가상 배열의 설치 및 관리에 대한 검증된 지침을 제공합니다. 이 문서는 데이터 센터에 가상 배열을 배포하고 관리할 책임이 있는 IT 관리자를 대상으로 합니다.
...
이 모범 사례에서는 가상 배열의 초기 설치 및 배포 시 따라야 하는 지침을 다룹니다. 여기에는 가상 컴퓨터 프로비전, 그룹 정책 설정, 크기 조정, 네트워킹 설정, 저장소 계정 구성, 가상 배열에 대한 공유 및 볼륨 만들기와 관련된 모범 사례가 포함됩니다.
...
StorSimple 가상 배열 크기를 조정할 때 다음 요소를 고려해야 합니다.

- 볼륨 또는 공유에 사용할 로컬 예약. 프로비전된 각 계층화 볼륨 또는 공유마다 로컬 계층의 공간 약 12%가 예약됩니다. 또한 파일 시스템에 로컬로 고정된 볼륨에 대해 약 10%의 공간이 예약됩니다.
...
위의 요소를 기반으로 크기 조정 요구 사항을 다음과 같은 수식으로 나타낼 수 있습니다.
...
위의 볼륨 또는 공유에 필요한 로컬 계층의 공간을 계산해 보겠습니다.
...
셋째, 지금까지 계산한 총 로컬 공간의 15%는 로컬 스냅숏을 저장하는 데 필요하므로 총 공간의 85%만 사용할 수 있습니다. 이 예에서는 약 990GB = 0.85*프로비전된 데이터 디스크(TB)입니다. 따라서 프로비전된 데이터 디스크는 (990*(1/0.85))= 1320GB = 1.16TB ~ 1.25TB(가장 가까운 사분위 수로 반올림)입니다.

예기치 않은 증가 및 새로운 복원 작업을 대비하여 약 1.25-1.5TB의 로컬 디스크를 프로비전해야 합니다.

> [AZURE.NOTE] 또한 로컬 디스크는 씬 프로비전이 좋습니다. 복원 공간은 5일을 초과한 데이터를 복원하려는 경우에만 필요하기 때문입니다. 항목 수준 복구를 사용하면 복원에 필요한 추가 공간 없이 최근 5일의 데이터를 복원할 수 있습니다.
...
총 디스크의 15%는 스냅숏을 저장하는 데 사용되므로 0.85만 사용할 수 있습니다. 따라서 디스크 크기는 (900*(1/0.85)) = 1.06TB ~ 1.25TB(가장 가까운 사분위 수로 반올림)입니다.
...
-   GPO(그룹 정책 개체)가 가상 배열에 적용되지 않도록 합니다. 가상 배열(자식 노드)이 부모로부터 GPO를 자동으로 상속하지 않도록 상속을 차단할 수 있습니다. 자세한 내용은 [상속 차단](https://technet.microsoft.com/library/cc731076.aspx)을 참조하세요.
...
-   장치를 iSCSI 서버로 배포하려는 경우:
	...
	-   고정 IP 주소를 구성합니다. 기본 및 보조 DNS 서버도 구성해야 합니다.

	-   가상 배열에 여러 네트워크 인터페이스를 정의하는 경우 첫 번째 네트워크 인터페이스만(기본적으로 **이더넷**) 클라우드에 연결할 수 있습니다. 트래픽 유형을 제어하려면 가상 배열(iSCSI 서버로 구성된)에 여러 가상 네트워크 인터페이스를 만들어서 여러 서브넷에 연결하면 됩니다.
...
-   새 저장소 계정을 만드는 경우
	...
	-   데이터 센터 간에 중복성을 구현하는 저장소 계정을 사용합니다. GRS(지역 중복 저장소), ZRS(영역 중복 저장소) 및 LRS(로컬 중복 저장소)는 모두 가상 배열에 사용할 수 있습니다. 다양한 저장소 계정에 대한 자세한 내용은 [Azure 저장소 복제](../storage/storage-redundancy.md)를 참조하세요.


### 공유 및 볼륨

StorSimple 가상 배열이 파일 서버로 구성되면 공유를 프로비전할 수 있고, iSCSI 서버로 구성되면 볼륨을 프로비전할 수 있습니다. 공유 및 볼륨 만들기에 대한 모범 사례는 구성된 유형뿐 아니라 크기와도 관련이 있습니다.

#### 볼륨/공유 크기

가상 배열이 파일 서버로 구성되면 공유를 프로비전할 수 있고, iSCSI 서버로 구성되면 볼륨을 프로비전할 수 있습니다. 공유 및 볼륨 만들기에 대한 모범 사례는 구성된 유형뿐 아니라 크기와도 관련이 있습니다.
...
-   가상 배열에 최대 16개 볼륨/공유를 만들 수 있습니다. 로컬로 고정된 경우 볼륨/공유의 크기가 50GB에서 2TB 사이일 수 있습니다. 계층화된 경우 볼륨/공유가 500GB에서 20TB 사이여야 합니다.
...
-   StorSimple 볼륨을 포맷할 때에는 할당 단위 크기(AUS) 64KB(기본값은 4KB)를 사용합니다. 64KB AUS는 다른 워크로드뿐 아니라 일반적인 StorSimple 워크로드에 대해 사내에서 수행된 테스트를 기반으로 합니다.

-   iSCSI 서버로 구성된 StorSimple 가상 배열을 사용할 경우 스팬된 볼륨 또는 동적 디스크를 사용하지 마세요. StorSimple에서 지원되지 않습니다.
...
StorSimple 가상 배열에 iSCSI 볼륨을 구성할 경우 필요한 모든 곳의 액세스를 제어하는 것이 중요 합니다. ACR(액세스 제어 레코드)를 만들고 StorSimple 볼륨에 연결하여 볼륨에 액세스할 수 있는 호스트 서버를 결정합니다.

StorSimple 볼륨에 대한 ACR을 구성할 때에는 다음 모범 사례를 사용합니다.

-   볼륨 하나에 반드시 하나 이상의 ACR을 연결합니다.

-   다중 ACR은 클러스터된 환경에서만 정의해야 합니다.
...
운영 모범 사례는 일상적인 관리나 가상 배열 운영에서 따라야 하는 지침입니다. 이 모범 사례에서는 백업 수행, 백업 세트에서 복원, 장애 조치(Failover) 수행, 배열 비활성화 및 삭제, 시스템 사용량 및 상태 모니터링, 가상 배열의 바이러스 스캔 실행 등과 같은 특정 관리 작업에 대해 다룹니다.
...
-   장애 조치(Failover)가 실패하더라도 **데이터가 항상 클라우드에서 안전하게 보관**됩니다. 장애 조치(Failover)가 실패하는 다음 세 가지 시나리오를 고려해 보세요.
    ...
    -   장애 조치(Failover)가 완료된 후 원본 장치가 삭제되었지만 대상 장치에 문제가 있어서 사용자가 데이터에 액세스할 수 없습니다. 데이터는 여전히 클라우드에서 안전하게 보관되며 다른 가상 배열을 만들어서 DR에 대한 대상 장치로 사용하면 간편하게 데이터를 검색할 수 있습니다.
...
## 참고 항목
StorSimple Manager 서비스를 통해 [StorSimple 가상 배열을 관리하는 방법](storsimple-ova-manager-service-administration.md)에 대해 알아보세요.
