...
* 계층화 공유의 프로비전된 크기와 관련된 파일 크기가 계층화 성능에 영향을 줄 수 있습니다. 큰 파일을 사용하면 계층 시간이 길어질 수 있습니다. 큰 파일을 사용하는 경우 가장 큰 파일이 공유 크기의 3% 미만인 것이 좋습니다.
...
* 볼륨/공유를 만든 후 StorSimple 볼륨/공유의 크기를 축소할 수 없습니다.
...
* StorSimple Manager 서비스를 통해 스토리지 계정을 구성할 때 StorSimple 디바이스와 클라우드 간 네트워크 통신을 위한 보안 채널이 생성되도록 TLS 모드를 사용해야 합니다.
