...
셋째, 지금까지 계산한 총 로컬 공간의 15%는 로컬 스냅숏을 저장하는 데 필요하므로 총 공간의 85%만 사용할 수 있습니다. 이 예에서는 약 990GB = 0.85*프로비전된 데이터 디스크(TB)입니다. 따라서 프로비전된 데이터 디스크는 (990*(1/0.85))= 1164GB = 1.16TB ~ 1.25TB(가장 가까운 사분위 수로 반올림)입니다.
