...
| 메뉴 항목 | Description |
|:--- |:--- |
| 신규 |**새로 만들기** 를 클릭하면 StorSimple 스냅샷 관리자를 기반으로 새 콘솔을 만듭니다. |
| 개방성 |**열기** 를 클릭하면 기존 콘솔을 엽니다. |
...
| 복제 |**Backup 카탈로그**, **클라우드 스냅샷**을 확장하고 날짜가 지정된 백업을 클릭한 다음, **결과** 창에서 볼륨을 선택합니다. |**복제** 를 클릭하여 클라우드 스냅샷의 복사본을 만들고 사용자가 지정한 위치에 저장합니다. |
...
| 여기에서 창 새로 만들기 |노드 또는 결과(모든 **작업** 메뉴 및 **작업** 창에 표시되는 항목)를 클릭합니다. |**여기에서 창 새로 만들기** 를 클릭하여 새 StorSimple 스냅샷 관리자 창을 엽니다. |
| 새로 고침 |노드 또는 결과(모든 **작업** 메뉴 및 **작업** 창에 표시되는 항목)를 클릭합니다. |**새로 고침** 을 클릭하여 현재 표시된 StorSimple 스냅샷 관리자 창을 업데이트합니다. |
| 디바이스 새로 고침 |**디바이스** 노드를 클릭하고 **결과** 창에서 디바이스를 마우스 오른쪽 단추로 클릭합니다. |**디바이스 새로 고침**을 클릭하여 특정 연결 디바이스를 StorSimple Snapshot Manager와 동기화합니다. |
| 디바이스 새로 고침 |**디바이스** 노드를 마우스 오른쪽 단추로 클릭합니다. |**디바이스 새로 고침**을 클릭하여 연결 디바이스 목록을 StorSimple Snapshot Manager와 동기화합니다. |
| 볼륨 다시 검사 |**볼륨** 노드를 마우스 오른쪽 단추로 클릭합니다. |**볼륨 다시 검사**를 클릭하여 **결과** 창에 표시되는 볼륨 목록을 업데이트합니다. |
| 복원 |**Backup 카탈로그**, 볼륨 그룹, **로컬 스냅샷** 또는 **클라우드 스냅샷**을 차례로 확장한 다음, 백업을 마우스 오른쪽 단추로 클릭합니다. |**복원** 을 클릭하여 현재 볼륨 그룹 데이터를 선택한 백업의 데이터로 대체합니다. |
...
메뉴 모음에서 **도움말** 메뉴에 액세스할 수 있습니다. **작업** 창에서 StorSimple 스냅샷 관리자 도움말 항목에 액세스할 수도 있습니다.
...
**로컬 스냅샷** 노드는 특정 볼륨 그룹의 로컬 스냅샷을 나열합니다. 이 노드는 **범위** 창의 **Backup 카탈로그** 노드 아래에 있습니다. 로컬 스냅샷은 Azure StorSimple 디바이스에 저장된 볼륨 데이터의 지정 시간 사본입니다. 일반적으로 이러한 유형의 백업은 신속하게 만들고 복원할 수 있습니다. 로컬 스냅샷은 로컬 백업 사본처럼 사용할 수 있습니다.
...
**클라우드 스냅샷** 노드는 특정 볼륨 그룹의 클라우드 스냅샷을 나열합니다. 이 노드는 **범위** 창의 **Backup 카탈로그** 노드 아래에 있습니다. 클라우드 스냅샷은 클라우드에 저장된 볼륨 데이터의 지정 시간 사본입니다. 클라우드 스냅샷은 다른 오프 사이트 스토리지 시스템에 복제된 스냅샷과 같습니다. 클라우드 스냅샷은 특히 재해 복구 상황에서 유용합니다.
...
**결과** 창은 StorSimple 스냅샷 관리자 UI의 가운데 창입니다. 여기에는 **범위** 창에서 선택한 노드에 대한 목록과 상세 상태 정보가 들어 있습니다.
...
**작업** 창은 StorSimple 스냅샷 관리자 UI의 오른쪽 창입니다. 여기에는 **범위** 창 또는 **결과** 창에서 선택한 노드, 보기 또는 데이터에 대해 수행 가능한 작업 메뉴가 들어 있습니다. **작업** 창에는 **범위** 창 및 **결과** 창의 항목에 사용 가능한 것과 동일한 **작업** 메뉴가 들어 있습니다. 각 동작에 대한 설명은 **동작** 메뉴의 섹션을 참조하세요.
...
|  |조 |모든 열려 있는 콘솔 창을 그리드 모양으로 표시합니다. | |
|  |I |화면 아래쪽에서 가로 행으로 아이콘을 정렬합니다. | |
| ALT + H |**도움말** 메뉴를 엽니다. |H |온라인 도움말을 엽니다. |
|  |조 |Microsoft TechNet Tech Center 웹 페이지가 열립니다. | |
