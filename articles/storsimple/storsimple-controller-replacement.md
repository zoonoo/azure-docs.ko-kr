<properties 
   pageTitle="StorSimple 장치의 컨트롤러 모듈 교체 | Microsoft Azure"
...
1. StorSimple Manager 서비스의 관리 포털에서 **장치** 탭을 클릭한 다음 모니터링하려는 장치의 이름을 클릭합니다.
...
8. 컨트롤러가 다시 시작된 후 관리 포털에서 **컨트롤러 상태** 및 **클러스터 상태**를 검사하여 컨트롤러가 다시 정상 상태로 돌아갔으며 대기 모드에 있는지 확인합니다.
...
5. 교체에 성공했는지 확인하려면 관리 포털에서 **장치** > **유지 관리** > **하드웨어 상태**로 이동한 다음 컨트롤러 0과 컨트롤러 1이 모두 정상(녹색 상태)인지 확인합니다.
...
- [관리 포털을 사용하여 활성 컨트롤러 식별](#use-the-management-portal-to-identify-the-active-controller)
