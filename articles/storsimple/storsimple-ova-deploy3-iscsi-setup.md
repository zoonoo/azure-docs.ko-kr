...
* 기존 StorSimple Manager 서비스에 가상 장치를 두 번째 또는 후속으로 등록하는 경우에는 서비스 데이터 암호화 키가 있어야 합니다. 이 키는 이 서비스에 첫 번째 장치가 등록될 때 생성되었습니다. 이 키를 잃어버린 경우에는 **웹 UI를 사용하여 StorSimple 가상 배열 관리** 의 [서비스 데이터 암호화 키 가져오기](storsimple-ova-web-ui-admin.md#get-the-service-data-encryption-key)를 참조하세요.
...
6. **장치 설정** 페이지에서:
   ...
   2. 만드는 장치의 **유형**에 대해 **iSCSI 서버** 아이콘 ![iSCSI 서버 아이콘](./media/storsimple-ova-deploy3-iscsi-setup/image7.png)을 클릭합니다. iSCSI 서버에서 블록 저장소를 프로비전할 수 있습니다.
   3. 이 장치를 도메인에 가입할지 지정합니다. 장치가 iSCSI 서버인 경우, 도메인 가입은 선택 사항입니다. iSCSI 서버를 도메인에 가입하지 않으려면, **적용**을 클릭하고 설정이 적용될 때까지 기다린 후에 다음 단계로 건너뜁니다.
      ...
      > 
      > 
   4. 대화 상자가 표시됩니다. 지정된 형식으로 도메인 자격 증명을 입력합니다. 확인 아이콘 ![확인 아이콘](./media/storsimple-ova-deploy3-iscsi-setup/image15.png)을 클릭합니다. 도메인 자격 증명이 확인됩니다. 자격 증명이 올바르지 않으면 오류 메시지가 표시됩니다.
...
9. 장치에 대한 클라우드 설정을 구성합니다. 이 단계에서는 로컬 장치 구성을 완료한 후 StorSimple Manager 서비스에 장치를 등록합니다.
   ...
   2. 이 서비스에 장치를 처음으로 등록하는 경우가 아니라면 **서비스 데이터 암호화 키**를 제공해야 합니다. 이 키는 StorSimple Manager 서비스에 추가 장치를 등록하기 위한 서비스 등록 키에 필요합니다. 자세한 내용은 로컬 웹 UI의 [서비스 데이터 암호화 키 받기](storsimple-ova-web-ui-admin.md#get-the-service-data-encryption-key)를 참조하세요.
...
3. 장치 구성 마법사의 **기본 설정** 페이지에서 다음을 수행합니다.
   ...
   3. 확인 아이콘 ![확인 아이콘](./media/storsimple-ova-deploy3-iscsi-setup/image15.png)을 클릭합니다.
      
      ![기본 설정](./media/storsimple-ova-deploy3-iscsi-setup/image16.png)
      
      이제 설정이 업데이트됩니다. 설정이 업데이트된 후에는 장치 설치 완료 단추를 사용할 수 없게 됩니다. 장치 **빠른 시작** 페이지로 돌아갑니다.                                                        
...
1. 장치 **빠른 시작** 페이지에서 **볼륨 추가**를 클릭합니다. 볼륨 추가 마법사가 시작됩니다.
2. 볼륨 추가 마법사의 **기본 설정**에서 다음을 수행합니다.
   
   1. 볼륨에 고유한 이름을 지정합니다. 이름은 3~127자를 포함하는 문자열이어야 합니다.
...
3. **추가 설정** 페이지에서 새 ACR(액세스 제어 레코드)을 추가합니다.
   ...
   4. 확인 아이콘 ![확인 아이콘](./media/storsimple-ova-deploy3-iscsi-setup/image15.png)을 클릭합니다. 볼륨 만들기 작업이 시작됩니다. 다음과 유사한 진행 메시지가 표시됩니다.
