...
| 번호 | 기능 | 문제 |
| --- | --- | --- |
| 1 |백업 복원력| 이전 릴리스에서는 특정 예외에 대해서만 백업이 다시 시도되었습니다. 이 릴리스에는 모든 백업 예외를 다시 시도하여 백업 복원력을 높이는 수정 내용이 포함되어 있습니다.|
| 2 |모니터링| 2017년 6월 30일부터 StorSimple 가상 장치 시리즈에 대한 저장소 사용 모니터링이 사용되지 않습니다. 이 작업은 StorSimple 가상 배열(1200 모델)에서 실행되는 StorSimple 장치 관리자 서비스의 모니터링 차트에 영향을 줍니다. 이 릴리스에는 사용자가 2017년 6월 30일 이후에도 가상 배열에서 저장소 사용 모니터링을 사용할 수 있게 해주는 업데이트가 포함되어 있습니다.|
| 3 |파일 서버| 이전 릴리스에서는 사용자가 암호화된 파일을 가상 배열에 실수로 복사할 수 있었습니다. 이 릴리스에는 가상 배열에 암호화된 파일의 복사를 허용하지 않는 수정 내용이 포함되어 있습니다. 업데이트 전에 장치에 암호화된 기존 파일이 있는 경우 암호화된 파일을 시스템에서 모두 삭제할 때까지 백업이 계속 실패합니다. |
