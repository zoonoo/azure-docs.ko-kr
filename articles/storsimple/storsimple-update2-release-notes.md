<properties 
   pageTitle="StorSimple 8000 시리즈 업데이트 2 릴리스 정보 | Microsoft Azure"
...
업데이트 2는 릴리스(GA) 또는 업데이트 0.1에서 업데이트 1.2를 실행하는 모든 StorSimple 장치에 적용할 수 있습니다.
...
- **StorSimple 가상 장치 개선** – 이전에 StorSimple 8000 시리즈는 가상 장치를 재해 복구 또는 개발/테스트 솔루션으로 배치했습니다. 가상 장치 모델은 한 가지(모델 1100)뿐이었습니다. 업데이트 2는 두 가지 가상 장치 모델을 포함합니다.
     ...
     - 8020 – 용량이 64TB이고 향상된 성능을 위해 Azure 프리미엄 저장소를 사용합니다.

    이 두 가지 가상 장치 모델(8010/8020)에 대한 단일 VHD가 있습니다. 가상 장치를 처음 시작하면, 플랫폼 매개 변수를 감지하여 올바른 모델 버전을 적용합니다.
...
- **업데이트 개선** – 업데이트 1.2 이전에, StorSimple 8000 시리즈는 클러스터링, iSCSI 등에 대한 Windows 업데이트 및 바이너리 및 펌웨어용 Microsoft Update라는 두 가지 채널을 통해 업데이트되었습니다. 업데이트 2는 모든 업데이트 패키지에 Microsoft 업데이트를 사용합니다. 이를 통해 패치 작업이나 장애 조치(failover)에 소요되는 시간이 감소됩니다.

- **펌웨어 업데이트** – 다음과 같은 펌웨어 업데이트가 포함됩니다.
    - LSI: lsi\_sas2.sys Product Version 2.00.72.10
    - SSD 전용(HDD 업데이트 제외): XMGG, XGEG, KZ50, F6C2, 및 VR08

- **주도적인 지원** – 업데이트 2를 통해 Microsoft는 장치로부터 추가적인 진단 정보를 확보할 수 있습니다. 운영 팀에서 문제가 있는 장치를 확인하는 경우, 장치로부터 정보를 수집하고 문제를 진단할 준비가 보다 잘 갖추어져 있습니다. **업데이트 2를 수락하면, 이러한 주도적 지원을 제공해도 된다고 허락하는 것입니다**.
...
| 2 | 업데이트 | 업데이트 1로 업그레이드 후에, 다수의 장치에서 Azure 클래식 포털의 가청 알람 경고가 발생했습니다. 이 문제가 해결되었습니다. | 예 | 아니요 |
| 3 | Openstack 인증 | Openstack을 클라우드 서비스 공급자로 사용하는 경우 클라우드 인증 문자열이 너무 길다는 오류를 수신할 수 있습니다. 이 문제가 해결되었습니다. | 예 | 아니요 |
...
| 17 | 로컬로 고정된 볼륨 | 복원 작업에서는 백업 카탈로그의 임시 스냅숏 백업을 복원 작업 기간에만 노출합니다. | 복원 작업에 로컬로 고정된 볼륨만 있거나 로컬로 고정된 볼륨과 계층화된 볼륨이 함께 있는 경우에 이 동작이 발생할 수 있습니다. 복원 작업에 계층화된 볼륨만 있으면 이 동작이 발생 하지 않습니다. 사용자 개입이 필요 없습니다. | 예 | 아니요 |
...
| 20 |StorSimple 관리자 서비스 생성 시 미리 보기 메시지 | StorSimple 관리자 서비스의 생성 시 표시되는 미리 보기 메시지는 Microsoft Azure StorSimple Virtual Array 시리즈에만 적용됩니다. Virtual Array 시리즈는 최근에 출시되었으며, 현재는 미리 보기로 제공되기 때문에 [미리 보기 보충 약관](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)이 적용됩니다. StorSimple 관리자 서비스 및 StorSimple 물리적 장치 시리즈는 일반 공급 솔루션이며 미리 보기 추가 사용 약관이 적용되지 않습니다. | | |
| 21 |로컬로 고정된 볼륨 | 복원 작업에서는 접두사 **tmpCollection**이 붙은 가상 디스크 그룹을 **백업 정책** 페이지에 복원 작업 기간에만 노출합니다.|복원 작업에 로컬로 고정된 볼륨만 있거나 로컬로 고정된 볼륨과 계층화된 볼륨이 함께 있는 경우에 이 동작이 발생할 수 있습니다. 복원 작업에 계층화된 볼륨만 있으면 이 동작이 발생 하지 않습니다. 사용자 개입이 필요 없습니다.|
| 22 |로컬로 고정된 볼륨 | 계층화된 볼륨(업데이트 1.2 이전을 통해 생성 또는 복제됨)을 로컬로 고정된 볼륨으로 변환하려고 하고 장치에 공간이 부족하거나 클라우드가 중단된 경우 복제는 손상될 수 있습니다.| 이 문제는 사전 업데이트 2 소프트웨어로 생성되고 복제된 볼륨을 사용한 경우에만 발생합니다. 자주 발생하지 않는 시나리오여야 합니다.|
| 23 | 볼륨 변환 | 볼륨 변환이 진행 중인 동안 볼륨에 연결된 ACR을 업데이트하지 마세요.(로컬로 고정된 볼륨에 계층화되거나 그 반대임) ACR를 업데이트하는 작업은 데이터가 손상될 수 있습니다. | 필요한 경우 볼륨 변환 전에 ACR을 업데이트하고 변환이 진행 중인 동안 ACR을 더 이상 업데이트하지 않습니다. |

## 업데이트 2의 컨트롤러 및 펌웨어 업데이트
