...
| 8 | 긴 클라우드 대기 시간 및 많은 I/O 워크로드 | StorSimple 장치에서 클라우드 대기 시간(초 순서)이 매우 길고 I/O 작업이 많으면 장치 볼륨의 성능이 저하되며 "장치가 준비 되지 않았습니다"라는 오류와 함께 I/O가 실패할 수 있습니다. | 이 상황에서 복구하려면 수동으로 장치 컨트롤러를 다시 부팅하거나 장치 장애 조치를 수행해야 합니다. | 예 | 아니요 |
...
| 20 |StorSimple 관리자 서비스 생성 시 표시되는 미리 보기 메시지 | StorSimple 관리자 서비스 생성 시 표시되는 미리 보기 메시지는 Microsoft Azure StorSimple Virtual Array 시리즈에만 적용됩니다. Virtual Array 시리즈는 최근에 출시되었으며, 현재는 미리 보기로 제공되기 때문에 [미리 보기 보충 약관](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)이 적용됩니다. StorSimple 관리자 서비스와 StorSimple 물리적 장치 시리즈는 일반 공급 솔루션이기 때문에 미리 보기 약관이 적용되지 않습니다. | | |
