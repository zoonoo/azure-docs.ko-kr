...
다음 단계는 8100 StorSimple 스토리지 디바이스를 개봉하는 방법을 명확하고 자세하게 설명합니다. 이 디바이스는 하나의 상자에 제공됩니다.
...
2. 상자를 개봉합니다. 다음 이미지는 StorSimple 디바이스의 개봉된 모습을 보여 줍니다.
     ...
     ![스토리지 디바이스 개봉하기](./media/storsimple-8100-hardware-installation/HCSUnpackyour2Udevice.png)
...
다음 단계를 따라 전면/후면 포스트가 있는 표준 19인치 랙에 StorSimple 8100 스토리지 디바이스를 설치하세요. StorSimple 8100 디바이스에는 단일 기본 인클로저가 있습니다.
...
5. 다음과 같이 직렬 연결을 설정합니다. 115200 보드, 8 데이터 비트, 1 정지 비트, 패리티 없음, 흐름 제어를 없음으로 설정 합니다.
6. 콘솔에서 Enter 키를 눌러 연결이 작동하는지 확인합니다. 그러면 직렬 콘솔 메뉴가 나타납니다.

> [!NOTE]
> **조명 관리**: 장치가 원격 데이터 센터나 액세스가 제한 된 컴퓨터 방에 설치 된 경우 두 컨트롤러에 대 한 직렬 연결이 항상 직렬 콘솔 스위치 또는 유사한 장비에 연결 되어 있는지 확인 합니다. 이렇게 하면 네트워크 중단 또는 예기치 않은 장애 발생 시 대역 외 원격 제어 및 지원 작업이 허용됩니다.
> 
> 
