...
## Azure 클래식 포털에서 지원 패키지 만들기 및 업로드

Azure 클래식 포털에서 서비스의 **유지 관리** 페이지를 통해 Microsoft 지원 사이트에 지원 패키지를 만들고 업로드할 수 있습니다.

> [AZURE.NOTE] 업로드하려면 지원 암호가 필요합니다. 지원 엔지니어는 암호를 전자 메일로 제공해야 합니다.
...
### Export-HcsSupportPackage cmdlet 매개 변수
...
> [AZURE.IMPORTANT] StorSimple용 Windows PowerShell을 통해 생성된 지원 패키지를 편집할 수 있습니다. StorSimple Manager 서비스로 Azure 클래식 포털에서 생성한 패키지를 편집할 수 없습니다.
