...
다음 절차에 따라 StorSimple 볼륨을 탑재, 초기화 및 포맷할 수 있습니다. 이 절차는 하드 디스크 및 해당 볼륨 또는 파티션을 관리하기 위해 디스크 관리, 시스템 유틸리티를 사용합니다. 디스크 관리에 대한 자세한 내용은 Microsoft TechNet 웹 사이트에서 [디스크 관리](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770943(v=ws.11))로 이동하세요.
...
4. 볼륨을 온라인 상태로 만듭니다.
   ...
   2. **디스크 다시 활성화** 를 클릭합니다. 다시 활성화되면 디스크는 **온라인** 으로 표시됩니다.
5. 볼륨 초기화:
   
   1. 검색된 볼륨을 마우스 오른쪽 단추로 클릭합니다.
   2. 메뉴에서 **디스크 초기화** 를 선택합니다.
   3. **디스크 초기화** 대화 상자에서 초기화할 디스크를 선택한 다음 **확인** 을 클릭합니다.
6. 단순 볼륨을 포맷합니다.
   ...
   3. 새 단순 볼륨 마법사를 사용하여 볼륨을 포맷합니다.
      ...
      * 빠른 포맷을 수행합니다.
7. 다중 파티션 볼륨을 포맷합니다. 자세한 지침은 [디스크 관리 구현](/previous-versions/tn-archive/dd163556(v=technet.10))에서 "파티션 및 볼륨" 섹션을 참조하세요.
...
2. **범위** 창에서 **볼륨** 노드를 클릭합니다. 모든 Azure StorSimple 볼륨을 포함하여 로컬 볼륨 및 탑재된 볼륨 목록이 **결과** 창에 표시됩니다. **결과** 창의 열은 구성할 수 있습니다. ( **볼륨** 노드를 마우스 오른쪽 단추로 클릭 하 고 **보기** 를 선택한 다음 **열 추가/제거** 를 선택 합니다.)
...
> [!NOTE]
> 볼륨 그룹에 속해 있는 볼륨은 삭제할 수 없습니다. 볼륨 그룹의 구성원 인 볼륨에 대해서는 삭제 옵션을 사용할 수 없습니다. 볼륨을 삭제 하려면 전체 볼륨 그룹을 삭제 해야 합니다.
...
1. 바탕 화면 아이콘을 클릭하여 StorSimple 스냅샷 관리자를 시작합니다.
2. **범위** 창에서 **볼륨** 을 마우스 오른쪽 단추로 클릭하고 **볼륨 다시 검사** 를 선택합니다.
...
* StorSimple Snapshot Manager 설치 및 구성. 자세한 내용은 [StorSimple Snapshot Manager 배포](storsimple-snapshot-manager-deployment.md)로 이동 하세요.
...
4. **범위** 창에서 **볼륨** 노드를 마우스 오른쪽 단추로 클릭하고 **볼륨 다시 검사** 를 선택합니다. 검색이 끝나면 **결과** 창에 볼륨 목록이 나타나야 합니다. 
5. **결과** 창에서 볼륨을 마우스 오른쪽 단추로 클릭한 다음 **볼륨 그룹 만들기** 를 클릭합니다. 
   
    ![볼륨 그룹 만들기](./media/storsimple-snapshot-manager-manage-volumes/HCS_SSM_Create_volume_group.png) 
6. **볼륨 그룹 만들기** 대화 상자에서 볼륨 그룹의 이름을 입력하고 볼륨 그룹에 볼륨을 할당한 다음 **확인** 을 클릭합니다.
7. **범위** 창에서 **볼륨 그룹** 노드를 확장합니다. 새 볼륨 그룹이 **볼륨 그룹** 노드 아래에 나타나야 합니다. 
8. 볼륨 그룹 이름을 마우스 오른쪽 단추로 클릭합니다.
   
   * 대화형(주문형) 백업 작업을 시작하려면 **백업 수행** 을 클릭합니다. 
   * 자동 백업을 예약하려면 **백업 정책 만들기** 를 클릭합니다. **일반** 페이지의 목록에서 볼륨 그룹을 선택합니다. **일정** 페이지에서 일정 세부 정보를 입력합니다. 작업을 마쳤으면 **확인** 을 클릭합니다. 
9. 백업 작업이 시작되었는지 확인하려면 **범위** 창에서 **작업** 노드를 확장하고 **실행 중** 노드를 클릭합니다. 현재 실행 중인 작업의 목록이 **결과** 창에 나타납니다. 
...
* StorSimple 디바이스 및 호스트 컴퓨터가 올바르게 구성되었는지 확인합니다. 자세한 내용은 [온-프레미스 StorSimple 디바이스 배포](storsimple-8000-deployment-walkthrough-u2.md)를 참조하세요.
* StorSimple Snapshot Manager 설치 및 구성. 자세한 내용은 [StorSimple Snapshot Manager 배포](storsimple-snapshot-manager-deployment.md)로 이동 하세요.
* StorSimple 디바이스에 두 볼륨을 구성합니다. 이 예에서 사용할 수 있는 볼륨은 **디스크 1** 과 **디스크 2** 입니다. 

### <a name="step-1-use-disk-management-to-create-a-dynamic-mirrored-volume"></a>1단계: 디스크 관리를 사용하여 동적 미러 볼륨 만들기
디스크 관리는 하드 디스크 및 볼륨 또는 하드 디스크와 볼륨이 포함된 파티션을 관리하기 위한 시스템 유틸리티입니다. 디스크 관리에 대한 자세한 내용은 Microsoft TechNet 웹 사이트에서 [디스크 관리](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770943(v=ws.11))로 이동하세요.
...
1. 다음 옵션 중 하나를 사용하여 디스크 관리를 시작합니다. 
   ...
   * 서버 관리자를 시작하고 **저장소** 노드를 확장한 다음 **디스크 관리** 를 선택합니다. 
   * **관리 도구** 를 시작하고 **컴퓨터 관리** 노드를 확장한 다음 **디스크 관리** 를 선택합니다. 
2. StorSimple 디바이스에서 사용할 수 있는 두 볼륨이 있는지 확인합니다. 이 예에서 사용할 수 있는 볼륨은 **디스크 1** 과 **디스크 2** 입니다. 
3. 디스크 관리 창의 아래쪽 창 오른쪽 열에서 **디스크 1** 을 마우스 오른쪽 단추로 클릭하고 **새 미러 볼륨** 을 선택합니다. 
   
    ![새 미러 볼륨](./media/storsimple-snapshot-manager-manage-volumes/HCS_SSM_New_mirrored_volume.png) 
4. **새 미러 볼륨** 마법사 페이지에서 **다음** 을 클릭합니다.
5. **디스크 선택** 창의 **선택됨** 창에서 **디스크 2** 를 선택하고 **추가** 를 클릭한 후 **다음** 을 클릭합니다. 
...
### <a name="step-2-use-storsimple-snapshot-manager-to-configure-backup"></a>2단계: StorSimple Snapshot Manager를 사용하여 백업 구성
...
2. **범위** 창에서 **볼륨** 노드를 마우스 오른쪽 단추로 클릭하고 **볼륨 다시 검사** 를 선택합니다. 검색이 끝나면 **결과** 창에 볼륨 목록이 나타나야 합니다. 동적 미러 볼륨은 단일 볼륨으로 나열됩니다. 
...
6. 볼륨 그룹 이름을 마우스 오른쪽 단추로 클릭합니다. 
   ...
   * 대화형(주문형) 백업 작업을 시작하려면 **백업 수행** 을 클릭합니다. 
   * 자동 백업을 예약하려면 **백업 정책 만들기** 를 클릭합니다. **일반** 페이지의 목록에서 볼륨 그룹을 선택합니다. **일정** 페이지에서 일정 세부 정보를 입력합니다. 작업을 마쳤으면 **확인** 을 클릭합니다. 
7. 실행되는 백업 작업을 모니터링할 수 있습니다. **범위** 창에서 **작업** 노드를 확장하고 **실행** 을 클릭하면 **결과** 창에 작업 세부 정보가 나타납니다. 백업 작업이 완료되면 세부 정보가 **최근 24** 시간 작업 목록으로 전송됩니다. 

## <a name="next-steps"></a>다음 단계
* [StorSimple 스냅샷 관리자를 사용하여 StorSimple 솔루션을 관리](storsimple-snapshot-manager-admin.md)하는 방법을 알아봅니다.
* [StorSimple 스냅샷 관리자를 사용하여 볼륨 그룹을 만들고 관리](storsimple-snapshot-manager-manage-volume-groups.md)하는 방법을 알아봅니다.
