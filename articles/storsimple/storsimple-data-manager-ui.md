---
title: Microsoft Azure StorSimple 데이터 관리자 UI
description: StorSimple 데이터 관리자 UI를 사용하여 StorSimple 8000 시리즈 디바이스에 있는 데이터를 변환하는 방법을 알아봅니다.
author: alkohli
...
ms.locfileid: "94957931"
---
# <a name="manage-the-storsimple-data-manager-service-in-azure-portal"></a>Azure Portal에서 StorSimple 데이터 관리자 서비스 관리

이 문서에서는 StorSimple 데이터 관리자 UI를 사용하여 StorSimple 8000 시리즈 디바이스에 있는 데이터를 변환하는 방법을 설명합니다. 그러면 Azure Media Services, Azure HDInsight, Azure Machine Learning 및 Azure Cognitive Search와 같은 다른 Azure 서비스에서 변환된 데이터를 사용할 수 있습니다.
...
3. 새 서비스의 경우 다음을 지정합니다.
   ...
   3. 기존 리소스 그룹을 선택하거나 새 리소스 그룹을 만듭니다. 자세한 내용은 [Azure 리소스 그룹](../azure-resource-manager/management/manage-resource-groups-portal.md)을 참조하세요.
...
7. **데이터 리포지토리** 에 대한 드롭다운 목록에서 만든 데이터 리포지토리를 선택합니다. 
   ...
   3. **필터** 하위 섹션에서 관심 있는 데이터를 포함하는 루트 디렉터리를 _\MyRootDirectory\Data_ 형식으로 입력합니다. _\C:\Data_ 와 같은 드라이브 문자는 지원되지 않습니다. 여기에서 파일 필터를 추가할 수 있습니다.

   4. 데이터 변환 서비스는 Azure까지 푸시되는 데이터의 최신 스냅샷에서만 작동합니다.
...
작업을 완료한 후 작업의 상태를 볼 수 있습니다. 작업 상태는 **성공**, **부분적으로 성공** 및 **실패** 일 수 있습니다. 성공적으로 복사된 파일과 복사되지 못한 파일의 목록을 볼 수 있습니다. 이러한 목록은 대상 스토리지 계정 내의 **"storsimple-data-manager-joblogs"** 라는 컨테이너에서 사용할 수 있습니다. 이 컨테이너 내에서 작업 정의와 이름이 같은 폴더를 찾을 수 있습니다. 이 안에서 목록을 포함하는 모든 작업 실행에 대해 폴더가 생성됩니다. 이 폴더의 이름은 작업 세부 정보 페이지에서 가져올 수 있는 작업의 GUID가 됩니다. 또는 대부분의 경우 작업 페이지 내에 복사 로그에 대한 링크가 표시됩니다.
이 폴더에는 두 개의 csv 파일 집합이 표시됩니다. **copiedfilelist...** 로 시작하는 모든 파일에는 성공적으로 복사된 파일 목록이 포함됩니다. **failedfilelist...** 로 시작하는 모든 파일에는 오류 메시지와 함께 복사할 수 없는 파일이 포함됩니다.
