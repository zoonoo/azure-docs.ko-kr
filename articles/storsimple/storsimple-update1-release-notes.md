...
StorSimple 솔루션에 업데이트를 배포하기 전에 릴리스 정보에 포함된 정보를 검토하세요. 자세한 내용은 [StorSimple 장치에 업데이트 1.2를 설치하는 방법](storsimple-install-update-1.md)을 참조하세요.
...
다음 표에서는 업데이트 1.2, 1.1 및 1에서 해결된 문제를 간략하게 설명합니다.
...
| 4 | 장치 장애 조치 | 동일한 원본 장치에서 다른 대상 장치로의 볼륨 컨테이너의 다중 장애 조치는 지원되지 않습니다. 단일 데드 장치에서 여러 장치로의 장애 조치로 첫 번째 장애 조치된 장치의 볼륨 컨테이너에서 데이터 소유권이 손실됩니다. 이러한 장애 조치 후 Azure 포털에서 볼 때 이 볼륨 컨테이너가 나타나거나 다르게 동작합니다. | | 예 | 아니요 |
