...
| **개가.** |업데이트 |로컬 웹 UI를 사용하여 가상 배열에 업데이트 1을 설치하는 경우 업데이트 0.6을 실행 중인지 확인해야 합니다. 업데이트 0.6보다 낮은 버전을 실행 중인 경우 업데이트 0.6을 먼저 설치하고 업데이트 1을 적용해야 합니다. 사전 업데이트 0.6 버전에서 업데이트 1.0을 직접 설치하는 경우 일부 업데이트가 누락되며 모니터링 차트가 작동하지 않습니다.   | |
