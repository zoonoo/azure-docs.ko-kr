---
title: StorSimple 가상 배열 업데이트 1.0 릴리스 정보
...
**업데이트 1.0에는 StorSimple 디바이스 관리자 서비스의 인증과 관련된 변경 사항이 포함되어 있으므로 가장 빨리 배포해야 합니다.** 이 업데이트에는 다음과 같은 향상된 기능 및 버그 수정이 포함되어 있습니다.
 ...
 - **가비지 수집 개선** - 이 릴리스에는 디바이스 및 스토리지 계정이 두 개의 먼 지역에 있을 때 가비지 수집 주기의 성능을 향상시키기 위해 버그 수정을 포함합니다.
...
| 아니요. | 기능 | 문제 | 해결 방법/설명 |
| --- | --- | --- | --- |
| **1.** |Updates |미리 보기 릴리스에서 만든 가상 배열은 지원되는 일반 사용 가능 버전으로 업데이트될 수 없습니다. |이러한 가상 배열은 재해 복구(DR) 워크플로를 사용하여 일반 사용 가능 릴리스로 장애 조치되어야 합니다. |
...
| **5-10.** |Azure PowerShell |이번 릴리스에서는 Azure PowerShell을 통해 StorSimple 가상 배열을 관리할 수 없습니다. |가상 디바이스에 대한 모든 관리는 Azure Portal 및 로컬 웹 UI를 통해 수행됩니다. |
...
| **개가.** |Updates |로컬 웹 UI를 사용하여 가상 배열에 업데이트 1을 설치하는 경우 업데이트 0.6을 실행 중인지 확인해야 합니다. 업데이트 0.6보다 낮은 버전을 실행 중인 경우 업데이트 0.6을 먼저 설치하고 업데이트 1을 적용해야 합니다. 사전 업데이트 0.6 버전에서 업데이트 1.0을 직접 설치하는 경우 일부 업데이트가 누락되며 모니터링 차트가 작동하지 않습니다.   | |
