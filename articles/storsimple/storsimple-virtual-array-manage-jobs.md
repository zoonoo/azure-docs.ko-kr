...
**작업** 블레이드에서는 StorSimple 디바이스 관리자 서비스에 연결된 가상 배열에서 시작한 작업을 보고 관리하기 위한 단일 중앙 포털을 제공합니다. 여러 가상 디바이스에 대한 실행, 완료 및 실패한 작업을 볼 수 있습니다. 결과는 표 형식으로 나타납니다.
...
다음 오류가 표시됩니다. "장치 시간이 15 분 이상에 Microsoft Azure를 사용 하 여 동기화 되어 있습니다. 하이퍼바이저와 디바이스 시간이 NTP 서버와 동기화되어 있는지 확인합니다. 연결 문제가 없는지 확인합니다. 연결 문제를 해결하려면 가상 디바이스의 로컬 웹 UI에서 진단 테스트를 실행하세요."
