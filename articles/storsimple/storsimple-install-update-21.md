<properties
   pageTitle="StorSimple 장치에 업데이트 2.1 설치 | Microsoft Azure"
   description="StorSimple 8000 시리즈 장치에서 StorSimple 8000 시리즈 업데이트 2.1을 설치하는 방법에 대해 설명합니다."
   services="storsimple"
...
# StorSimple 장치에 업데이트 2.1 설치

## 개요

이 자습서에서는 Azure 클래식 포털을 통해서와 핫픽스 방법을 사용하여 이전 소프트웨어 버전을 실행하는 StorSimple 장치에 업데이트 2.1을 설치하는 방법에 대해 설명합니다. 핫픽스 방법은 StorSimple 장치의 DATA 0 이외의 네트워크 인터페이스에 게이트웨이가 구성되어 있고 업데이트 1 이전 소프트웨어 버전에서 업데이트하려는 경우에 사용됩니다.

업데이트 2.1은 장치 소프트웨어, WMI 및 iSCSI 업데이트를 포함합니다. 업데이트 2 이전 버전에서 업데이트하는 경우에도 LSI 드라이버, Spaceport, Storport 및 디스크 펌웨어 업데이트를 적용해야 합니다. 장치 소프트웨어, WMI, iSCSI, LSI 드라이버, Spaceport 및 Storport 픽스는 무중단 업데이트이며 Azure 클래식 포털을 통해 적용할 수 있습니다. 디스크 펌웨어 업데이트는 작업 중단 업데이트이며 장치의 Windows PowerShell 인터페이스를 통해서만 적용할 수 있습니다.

> [AZURE.IMPORTANT]

> -  업데이트의 단계적 롤아웃이 수행되므로 업데이트 2.1이 즉시 표시되지는 않을 수도 있습니다. 해당 업데이트를 곧 사용할 수 있게 되므로 몇 일 후에 업데이트를 스캔합니다.
...
> - 선택적 StorSimple Snapshot Manager를 실행하는 경우 장치를 업데이트하기 전에 Snapshot Manager 버전을 업데이트 2.1로 업그레이드했는지 확인합니다.

[AZURE.INCLUDE [storsimple-preparing-for-update](../../includes/storsimple-preparing-for-updates.md)]

## Azure 클래식 포털을 통해 업데이트 2.1 설치

다음 단계를 수행하여 장치를 [업데이트 2.1](storsimple-update21-release-notes.md)로 업데이트합니다.
...
12. 장치가 **StorSimple 8000 시리즈 업데이트 2.1(6.3.9600.17704)**를 실행하고 있는지 확인합니다. **마지막 업데이트 날짜**도 수정해야 합니다. 
...
장치에서 업데이트 2를 실행 중인 경우 다음 핫픽스를 사전 설정된 순서대로 다운로드하고 설치해야 합니다.

| 순서 | KB | 설명 | 업데이트 유형 | 설치 시간 |
|--------|-----------|-------------------------|------------- |-------------|
| 1\. | KB3162954 | 소프트웨어 업데이트 | 일반 | ~ 45분 |
| 2\. | KB3146621 | iSCSI 패키지 | 일반 | ~ 20분 |
...
| 순서 | KB | 설명 | 업데이트 유형 | 설치 시간 |
|--------|-----------|-------------------------|------------- |-------------|
| 4\. | KB3121900 | LSI 드라이버 및 펌웨어 | 일반 | ~ 20분 |
...
| 순서 | KB | 설명 | 업데이트 유형 | 설치 시간 |
|--------|-----------|-------------------------|------------- |-------------|
| 5\. | KB3090322 | Spaceport 픽스 </br> Windows Server 2012 R2 | 일반 | ~ 20분 |
| 6\. | KB3080728 | Storport 픽스 </br> Windows Server 2012 R2 | 일반 | ~ 20분 |
...
| 순서 | KB | 설명 | 업데이트 유형 | 설치 시간 |
|--------|-----------|-------------------------|------------- |-------------|
| 7\. | KB3121899 | 디스크 펌웨어 | 유지 관리 | ~ 30분 |
 
<br></br>

> [AZURE.IMPORTANT]
>
> - 이 절차는 한 번만 수행하여 업데이트 2.1을 적용해야 합니다. Azure 클래식 포털을 사용하여 후속 업데이트를 적용할 수 있습니다.
