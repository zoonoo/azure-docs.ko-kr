...
11. 이 자습서에서는 **Common virtual path for uploaded artifacts**에 **${BUILD_ID}/${BUILD_NUMBER}**를 입력합니다.
...
- **URL 형식**: 다음 URL 형식을 사용하여 Blob에 주소를 지정할 수 있습니다.
    ...
    위 형식에서  `storageaccount`는 저장소 계정 이름을 나타내고,  `container_name`은 컨테이너 이름을 나타내며,  `blob_name`은 Blob 이름을 각각 나타냅니다. 컨테이너 이름에는 슬래시(**/**)로 구분하여 여러 경로를 사용할 수 있습니다. 이 자습서에서는 예제 컨테이너 이름으로 **MyJob**이 사용되고 **${BUILD_ID}/${BUILD_NUMBER}**는 일반 가상 경로에 사용되었습니다. 따라서 Blob의 URL 형식은 다음과 같습니다.
