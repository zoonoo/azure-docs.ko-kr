...
-   [Twilio 정의][Twilio 정의]
-   [Twilio 등록(Microsoft 할인)][Twilio 등록(Microsoft 할인)]
-   [node.js Azure 웹 사이트 만들기 및 배포][node.js Azure 웹 사이트 만들기 및 배포]
-   [Twilio 모듈 구성][Twilio 모듈 구성]
-   [아웃바운드 전화 걸기][아웃바운드 전화 걸기]
...
Twilio를 통해 개발자는 전화 및 문자 메시지를 보내고 받는 데 모두 사용할 수 있는 [프로그램 가능 전화 번호를 구입][프로그램 가능 전화 번호를 구입]할 수 있습니다. Twilio 번호가 인바운드 전화 또는 문자를 받으면 Twilio는 웹 응용 프로그램에 HTTP POST 또는 GET 요청을 보내 전화 또는 문자를 처리하는 방법에 대한 지침을 요청합니다. 서버는 전화 또는 문자를 처리하는 방법에 대한 지침이 포함된 간단한 XML 태그 집합인 [TwiML][TwiML]을 통해 Twilio의 HTTP 요청에 응답합니다. 잠시 후에 TwiML 예제를 살펴보겠습니다.
...
node.js 웹 사이트를 선택하고 "구성" 링크를 클릭합니다. 아래로 조금 스크롤하면 응용 프로그램의 구성 속성을 설정할 수 있는 영역이 표시됩니다. 표시된 것처럼 Twilio 계정 자격 증명([Twilio 대시보드에서 찾을 수 있음][Twilio 대시보드에서 찾을 수 있음](영문))을 입력하고 이름을 각각 "TWILIO\_ACCOUNT\_SID" 및 "TWILIO\_AUTH\_TOKEN"으로 지정합니다.

![Azure 관리 콘솔][Azure 관리 콘솔]

이러한 변수를 구성하고 나서 Azure 콘솔에서 응용 프로그램을 다시 시작합니다.

### package.json에서 Twilio 모듈 선언

이제 [npm][npm]을 통해 노드 모듈 종속성을 관리하기 위한 package.json을 만들어야 합니다. Azure/node.js 자습서에서 만든 "server.js" 파일과 같은 수준에서 "package.json"이라는 파일을 만듭니다. 이 파일 내에 다음을 배치합니다.
...
그러면 널리 사용되는 [express 웹 프레임워크][express 웹 프레임워크](영문) 및 EJS 템플릿 엔진뿐만 아니라 종속성으로 twilio 모듈이 선언됩니다. 이제 모두 준비되었으니 코드를 작성하겠습니다!
...
이제 node.js 및 Twilio를 사용하여 통신하는 앱을 빌드하는 방법에 대한 기본 사항을 배웠습니다. 그러나 이러한 예제는 Twilio 및 node.js를 사용하여 수행할 수 있는 작업의 극히 일부에 불과합니다. Twilio와 node.js 사용에 대한 자세한 내용은 다음 리소스를 참조하세요.

-   [공식 모듈 문서][공식 모듈 문서]
-   [node.js 응용 프로그램에 VoIP 사용 자습서][Twilio 개발자 블로그]
-   [Votr - node.js 및 CouchDB를 사용한 실시간 SMS 투표 응용 프로그램(3개 부분으로 구성되어 있음)][Votr - node.js 및 CouchDB를 사용한 실시간 SMS 투표 응용 프로그램(3개 부분으로 구성되어 있음)]
-   [node.js를 사용한 브라우저에서의 쌍 프로그래밍][node.js를 사용한 브라우저에서의 쌍 프로그래밍]
...
Azure에서 node.js와 Twilio 해킹을 즐기시기를 바랍니다!
  ...
  [다음 단계]: #nextsteps
  [프로그램 가능 전화 번호를 구입]: https://www.twilio.com/user/account/phone-numbers/available/local
  ...
  [npm]: http://npmjs.org
  [express 웹 프레임워크]: http://expressjs.com
  [공식 모듈 문서]: http://twilio.github.io/twilio-node/
  [Votr - node.js 및 CouchDB를 사용한 실시간 SMS 투표 응용 프로그램(3개 부분으로 구성되어 있음)]: http://www.twilio.com/blog/2012/09/building-a-real-time-sms-voting-app-part-1-node-js-couchdb.html
  [node.js를 사용한 브라우저에서의 쌍 프로그래밍]: http://www.twilio.com/blog/2013/06/pair-programming-in-the-browser-with-twilio.html
