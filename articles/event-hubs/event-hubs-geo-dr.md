...
1. 기본 네임 스페이스를 만듭니다.
1. 구독에 보조 네임 스페이스를 만들고 기본 네임 스페이스를 포함 하는 리소스 그룹을 만듭니다. 이 단계는 선택 사항입니다. 다음 단계에서 페어링을 만드는 동안 보조 네임 스페이스를 만들 수 있습니다. 
...
1. **페어링 시작** 페이지에서 기존 보조 네임 스페이스를 선택 하거나 구독에서 기본 네임 스페이스가 있는 리소스 그룹을 만듭니다. 그런 다음 **만들기** 를 선택합니다. 다음 예에서는 기존 보조 네임 스페이스를 선택 합니다. 

    :::image type="content" source="./media/event-hubs-geo-dr/initiate-pairing-page.png" alt-text="보조 네임 스페이스 선택":::        
1. 이제 기본 네임 스페이스에 대해 **지역에서 복구** 를 선택 하면 다음 이미지 처럼 **지리적 DR 별칭** 페이지가 표시 됩니다.
...
1. **지리적 DR 별칭** 페이지에서 **공유 액세스 정책** 을 선택 하 여 별칭의 기본 연결 문자열에 액세스 합니다. 기본/보조 네임 스페이스에 대 한 연결 문자열을 직접 사용 하는 대신이 연결 문자열을 사용 합니다. 
