...
이제 생산자는 토픽 `test` 에서 이벤트 허브로 이벤트를 보내고 stdout에 이벤트를 인쇄 하기 시작 합니다.
...
이벤트 허브에 이벤트가 있는 경우 (예: 생산자가 실행 중인 경우) 소비자는 이제 토픽 `test`에서 이벤트를 수신 하기 시작 합니다.
