...
> [!NOTE]
> 이 문서는 Azure 이벤트 허브 SDK의 이전 버전에 적용됩니다. 코드를 최신 버전의 SDK로 마이그레이션하는 방법에 대해 알아보려면 다음 마이그레이션 가이드를 참조하세요. 
> - [.NET](https://github.com/Azure/azure-sdk-for-net/blob/master/sdk/eventhub/Azure.Messaging.EventHubs/MigrationGuide.md)
> - [Java](https://github.com/Azure/azure-sdk-for-java/blob/master/sdk/eventhubs/azure-messaging-eventhubs/migration-guide.md)
> - [Python](https://github.com/Azure/azure-sdk-for-python/blob/master/sdk/eventhub/azure-eventhub/migration_guide.md)
> - [자바 스크립트](https://github.com/Azure/azure-sdk-for-js/blob/master/sdk/eventhub/event-hubs/migrationguide.md)
>
> 또한 [응용 프로그램의 여러 인스턴스에서 파티션 로드 균형을](event-processor-balance-partition-load.md)참조하십시오.
...
먼저 사용하는 애플리케이션은 [OpenAsync, CloseAsync, ProcessErrorAsync 및 ProcessEventsAsync](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor?view=azure-dotnet#methods)라는 네 가지 메서드가 있는 [IEventProcessor](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor) 인터페이스를 구현합니다. 이 인터페이스에는 Event Hubs가 전송하는 이벤트를 사용하는 실제 코드가 포함됩니다. 다음 코드에서는 구현 예제를 보여줍니다.
...
- **hostName:** 각 소비자 인스턴스의 이름입니다. **EventProcessorHost의** 각 인스턴스는 소비자 그룹 내에서 이 변수에 대한 고유 값을 가져야 하므로 이 값을 하드 코딩하지 마십시오.
...
- **스토리지연결스트링:** 내부 리소스 관리에 사용되는 저장소 계정입니다.
...
비교적 빠르게 작업을 수행하는 것이 좋습니다. 즉, 가능한 작은 처리로 수행합니다. 대신 소비자 그룹을 사용합니다. 저장소에 쓰고 일부 라우팅을 수행해야 하는 경우 두 개의 소비자 그룹을 사용하고 별도로 실행되는 두 개의 [IEventProcessor](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor) 구현을 사용하는 것이 좋습니다.
...
마지막으로, [EventProcessorHost.UnregisterEventProcessorAsync](/dotnet/api/microsoft.azure.eventhubs.processor.eventprocessorhost.unregistereventprocessorasync)는 모든 파티션 판독기를 완전히 종료할 수 있고 [EventProcessorHost](/dotnet/api/microsoft.azure.eventhubs.processor.eventprocessorhost)의 인스턴스를 종료할 때 항상 호출되어야 합니다. 이렇게 하는 데 실패하면 임대 만료 및 Epoch 충돌로 인해 **EventProcessorHost**의 다른 인스턴스를 시작할 때 지연이 발생할 수 있습니다. 에포크 관리는 문서의 [Epoch](#epoch) 섹션에서 자세히 다룹니다. 
...
- [InvokeProcessorAfterReceiveTimeout](/dotnet/api/microsoft.azure.eventhubs.processor.eventprocessoroptions.invokeprocessorafterreceivetimeout): 이 매개 변수가 **true이면**파티션에서 이벤트를 수신하는 기본 호출이 시간 아웃될 때 [ProcessEventsAsync가](/dotnet/api/microsoft.azure.eventhubs.processor.ieventprocessor.processeventsasync) 호출됩니다. 이 메서드는 파티션에서 비활성 기간 동안 시간 기반 작업을 수행 하는 데 유용 합니다.
...
수신 시대의 작동 방식은 다음과 같습니다.

### <a name="with-epoch"></a>시대와 함께
Epoch는 파티션/임대 소유권을 적용하기 위해 서비스가 사용하는 고유 식별자(epoch 값)입니다. [CreateEpochReceiver](https://docs.microsoft.com/dotnet/api/microsoft.azure.eventhubs.eventhubclient.createepochreceiver?view=azure-dotnet) 메서드를 사용하여 Epoch 기반 수신기를 만듭니다. 이 메서드는 Epoch 기반 수신기를 만듭니다. 수신기는 지정된 소비자 그룹의 특정 이벤트 허브 파티션에 대해 만들어집니다.

이 획기적인 기능은 사용자에게 다음 규칙을 통해 언제든지 소비자 그룹에 수신기가 하나만 있는지 확인할 수 있는 기능을 제공합니다.

- 소비자 그룹에 기존 수신기가 없는 경우 사용자는 모든 epoch 값을 가진 수신기를 만들 수 있습니다.
- epoch 값 e1이있는 수신기가 있고 e1 <= e2가있는 epoch 값 e2로 새 수신기가 생성되면 e1이있는 수신기가 자동으로 분리되고 e2가있는 수신기가 성공적으로 생성됩니다.
- epoch 값 e1을 가진 수신기가 있고 새로운 수신기가 e1이 e2를 > epoch 값 e2로 생성되면 오류와 함께 실패하는 e2를 생성합니다: epoch e1이 있는 수신기는 이미 존재합니다.

### <a name="no-epoch"></a>시대 없음
[CreateReceiver](https://docs.microsoft.com/dotnet/api/microsoft.azure.eventhubs.eventhubclient.createreceiver?view=azure-dotnet) 메서드를 사용하여 비에포크 기반 수신기를 만듭니다. 

스트림 처리에는 사용자가 단일 소비자 그룹에 여러 수신기를 만들려고 하는 몇 가지 시나리오가 있습니다. 이러한 시나리오를 지원하기 위해, 우리는 시대없이 수신기를 만들 수있는 능력을 가지고 있으며,이 경우 우리는 소비자 그룹에 최대 5 개의 동시 수신기를 허용합니다.

### <a name="mixed-mode"></a>Mixed Mode
epoch가 있는 수신기를 만든 다음 동일한 소비자 그룹에서 노 에보로 전환하거나 그 반대로 전환하는 응용 프로그램 사용은 권장하지 않습니다. 그러나 이 문제가 발생하면 서비스는 다음 규칙을 사용하여 처리합니다.

- 이미 epoch e1로 생성된 수신기가 있고 이벤트를 적극적으로 수신하고 새 수신기가 획기적인 경우 새 수신기 생성이 실패합니다. Epoch 수신기는 항상 시스템에서 우선합니다.
- 이미 epoch e1로 만든 수신기가 있고 연결이 끊어졌고 새 수신기가 새 MessagingFactory에 획기적인 것으로 생성되면 새 수신기의 생성이 성공할 것입니다. 여기서는 ~10분 후에 "수신기 연결 끊김"을 감지할 수 있다는 주의 사항이 있습니다.
- epoch 없이 생성된 하나 이상의 수신기가 있고 epoch e1로 새 수신기가 생성되면 모든 이전 수신기의 연결이 끊어집니다.


> [!NOTE]
> 시대를 사용하는 응용 프로그램과 오류를 피하기 위해 시대를 사용하지 않는 응용 프로그램에 는 다른 소비자 그룹을 사용하는 것이 좋습니다. 
