...
기존 짝수 허브에 파티션을 추가 하는 경우 이벤트 허브 클라이언트는 `MessagingException` 엔터티 메타 데이터 (엔터티는 event hub이 고 메타 데이터는 파티션 정보)가 변경 된 클라이언트를 알려 주는 서비스 로부터를 수신 합니다. 클라이언트는 AMQP 링크를 자동으로 다시 연 다음, 변경된 메타데이터 정보를 선택합니다. 그러면 클라이언트가 정상적으로 작동합니다.
