...
---
# <a name="migrate-to-azure-event-hubs-for-apache-kafka-ecosystems"></a>Apache Kafka 에코 시스템에 대 한 Azure Event Hubs로 마이그레이션
Azure Event Hubs는 Kafka 프로토콜을 사용 하 여 Event Hubs에 연결 하는 데 사용할 수 있는 Apache Kafka 엔드포인트를 노출 합니다. 기존 Kafka 응용 프로그램을 최소한으로 변경 하 여 Azure Event Hubs에 연결 하 고 Azure 에코 시스템의 이점을 누릴 수 있습니다. Kafka 지원 [Apache Kafka 버전 1.0](https://kafka.apache.org/10/documentation.html) 이상에 대 한 Event Hubs입니다.
...
Kafka 사용 이벤트 허브에 연결 하려면 Kafka 클라이언트 구성을 업데이트 해야 합니다. 사용자를 찾는 데 문제가 있는 경우 응용 프로그램에서가 설정 `bootstrap.servers` 된 위치를 검색 해 보세요.

응용 프로그램에서 적합 한 위치에 다음 configs을 삽입 합니다. 클라이언트를 올바른 인증을 `bootstrap.servers` 사용 `sasl.jaas.config` 하 여 Event Hubs kafka 끝점으로 보내도록 및 값을 업데이트 해야 합니다. 
...
에서 `sasl.jaas.config` 프레임 워크의 구성이 지원 되지 않는 경우 SASL 사용자 이름 및 암호를 설정 하는 데 사용 되는 구성을 찾아 대신 사용 합니다. 사용자 이름을로 `$ConnectionString` 설정 하 고 암호를 Event Hubs 연결 문자열로 설정 합니다.
