...
이 문서에서는 Event Hubs를 포함하여 Azure Service Bus 메시지 API에서 생성하는 몇 가지 예외를 설명합니다. 이 참조는 변경될 수 있으므로 나중에 다시 업데이트를 확인하세요.
...
2. Event Hubs 네임스페이스에 충분한 처리량 단위가 없습니다([Azure Portal](https://portal.azure.com)의 Event Hubs 네임스페이스 블레이드에서 **메트릭** 블레이드를 확인할 수 있음). 포털에는 집계된(1분) 정보가 표시되지만 우리는 실시간으로 처리량을 측정하게 되므로 예상값에 불과합니다.

    해결 방법: 네임스페이스의 처리량 단위를 늘리면 도움이 될 수 있습니다. 포털의 Event Hubs 네임스페이스 블레이드에 있는 **규모**에서 이 작업을 수행할 수 있습니다.
