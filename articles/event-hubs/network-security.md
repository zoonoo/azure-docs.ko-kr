...
이 기능은 Azure Event Hubs가 잘 알려진 특정 사이트에서만 액세스할 수 있어야 하는 시나리오에서 유용합니다. 방화벽 규칙을 사용하면 특정 IPv4 주소에서 발생한 트래픽을 허용하도록 규칙을 구성할 수 있습니다. 예를 들어 [Azure Express Route][익스프레스 경로]와 함께 이벤트 허브를 사용하는 경우 온-프레미스 인프라 IP 주소에서만 트래픽을 허용하는 **방화벽 규칙을** 만들 수 있습니다. 
...
이벤트 허브를 [VNet(가상 네트워크) 서비스 엔드포인트][vnet-sep]와 통합하면 가상 네트워크에 바인딩된 가상 컴퓨터와 같은 워크로드에서 메시징 기능에 안전하게 액세스할 수 있으며 네트워크 트래픽 경로가 보호됩니다. 양쪽.
