<properties 
   pageTitle="이벤트 허브 가용성 및 지원 FAQ"
   description="이벤트 허브 가격 정보 및 FAQ"
   services="event-hubs"
...
이 섹션에서는 가용성 정보를 제공하고 일부 자주 묻는 질문에 답변합니다.

## 가격 정보

이벤트 허브 가격에 대한 자세한 내용은 [이벤트 허브 가격 정보](http://azure.microsoft.com/pricing/details/event-hubs/)를 참조하세요.
...
이벤트 허브 처리량 단위는 Azure 포털 또는 이벤트 허브 관리 API를 통해 사용자가 명시적으로 선택합니다. 처리량 단위는 서비스 버스 네임스페이스의 모든 이벤트 허브에 적용하고 각 처리량 단위로 네임 스페이스가 다음 기능을 수행합니다.
...
전체 수신 처리량 또는 네임 스페이스의 모든 이벤트 허브에서의 전체 수신 이벤트 비율이 집계 처리량 단위 허용 한도를 초과하면, 보낸 사람이 제한되고 수신 할당량을 초과했음을 나타내는 오류를 수신합니다.

전체 수신 처리량 또는 네임 스페이스의 모든 이벤트 허브에서의 전체 수신 이벤트 비율이 집계 처리량 단위 허용 한도를 초과하면, 받는 사람이 제한되고 수신 할당량을 초과했음을 나타내는 오류를 수신합니다. 송/수신 할당량을 개별적으로 적용하므로, 보낸 사람이 이벤트 소비 속도를 저하시키거나 수신기가 이벤트에서 이벤트 허브로 전송되지 못하게 방해할 수 없습니다.

처리량 단위 선택은 이벤트 허브 파티션 수와 무관합니다. 각 파티션에 초당 최대 1MB의 처리량(초당 최대 1000개의 이벤트) 및 초당 2MB 송신을 제공하면, 파티션 자체에 대한 고정된 비용은 없습니다. 서비스 버스 네임스페이스의 모든 이벤트 허브에서 집계된 처리량 단위 기준으로 산정됩니다. 이 패턴을 사용하면 시스템의 이벤트 로드가 실제로 더 높은 처리량을 필요로 할 때까지 처리량 단위 요금이 발생하지 않으며 시스템의 부하가 늘어날 때 사용자 시스템의 구조아 아키텍처를 변경하지 않아도 시스템에 필요한 최대 예상 부하를 지원하도록 충분한 파티션을 만들 수 있습니다.
...
이벤트 허브 표준 계층은 최대 30일 동안 24시간 이상 메시지 보존을 허용합니다. 저장된 이벤트의 총 크기가 선택한 처리량 단위(처리량 단위당 84GB)에 대한 저장소 허용 한도를 초과하면, 해당 허용 한도를 초과하는 크기는 게시된 Azure Blob 저장소 요금으로 청구됩니다. 처리량 단위가 최대 수신 허용 한도까지 사용되었더라도 각 처리량 단위의 저장소 허용 한도는 24 시간(기본값) 동안의 보존 기간에 대한 모든 저장소 비용이 포함됩니다.
...
예, 모든 이벤트 허브, 큐 및 항목은 동일한 서비스 버스 네임스페이스에 있습니다. 이와 같이 비용 효율적이고 확장성이 뛰어난 방식으로 1초 미만의 대기 시간으로 양방향 조정된 연결을 여러 장치에 구현할 수 있습니다.
