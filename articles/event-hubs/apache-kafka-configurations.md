...
증상 | 문제 | 해결 방법
----|---|-----
균형 재조정으로 인 한 오프셋 커밋 실패 | 소비자가 폴링 ()에 대 한 호출 사이에 너무 오래 대기 하 고 있으며, 서비스는 소비자를 그룹 외부로 시작 합니다. | 여러 옵션이 있습니다. <ul><li>폴링 처리 제한 시간 늘리기 ( `max.poll.interval.ms` )</li><li>처리 속도를 높이기 위해 메시지 일괄 처리 크기 줄이기</li><li>소비자 폴링합니다 () 차단을 방지 하기 위해 처리 병렬화 개선</li></ul> 이 세 가지 조합을 적용 하는 것이 wisest 가능성이 높습니다.
