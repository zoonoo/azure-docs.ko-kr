...
Azure Event Hubs를 사용하면 Azure [Blob 저장소](https://azure.microsoft.com/services/storage/blobs/) 또는 Azure Data Lake [Storage Gen 1 또는 Gen 2](https://azure.microsoft.com/services/data-lake-store/) 계정에서 이벤트 허브에서 스트리밍 데이터를 자동으로 캡처할 수 있으며 시간 또는 크기 간격을 지정하는 유연성이 추가됩니다. 캡처는 빠르게 설정할 수 있으며 실행을 위한 관리 비용이 없고 Event Hubs [처리량 단위](event-hubs-scalability.md#throughput-units)에 따라 크기가 자동으로 조정됩니다. Event Hubs 캡처는 스트리밍 데이터를 Azure에 로드하는 가장 쉬운 방법이며 데이터 캡처보다 데이터 처리에 집중할 수 있게 해줍니다.
...
이벤트 허브 캡처를 사용하면 캡처된 데이터를 저장하는 데 사용되는 자체 Azure Blob 저장소 계정 및 컨테이너 또는 Azure Data Lake Storage 계정을 지정할 수 있습니다. 이러한 계정은 이벤트 허브와 동일한 지역에 있을 수도 있고 다른 지역에 있을 수도 있으므로 Event Hubs 캡처 기능의 유연성이 향상됩니다.
...
Azure 저장소 Blob을 일시적으로 사용할 수 없는 경우 이벤트 허브 캡처이벤트 허브에 구성된 데이터 보존 기간에 대한 데이터를 유지하고 저장소 계정을 다시 사용할 수 있게 되면 데이터를 다시 채웁니다.
...
[아파치 드릴: Azure Blob 스토리지 플러그인][Apache Drill: Azure Blob Storage Plugin]
...
[Apache Spark][Apache Spark]는 “대규모 데이터 처리를 위한 통합형 분석 엔진”입니다. SQL을 비롯한 다른 언어를 지원하며, Azure Blob Storage에 쉽게 액세스할 수 있습니다. Azure에서 아파치 스파크를 실행하는 몇 가지 옵션이 있으며, 각각 Azure Blob 저장소에 쉽게 액세스할 수 있습니다.

- [HDInsight: Azure 저장소의 주소 파일][HDInsight: Address files in Azure storage]
- [Azure 데이터 브릭: Azure Blob 저장소][Azure Databricks: Azure Blob Storage]
- [Azure Kubernetes 서비스](https://docs.microsoft.com/azure/aks/spark-job) 
...
Apache Avro에는 [Java][Java] 및 [Python][Python]에 대한 전체 시작 가이드가 있습니다. 이벤트 허브 캡처 시작하기 문서를 읽을 수도 [있습니다.](event-hubs-capture-python.md)
...
Capture는 별도로 청구되기 때문에 송신 할당량을 사용하지 않습니다. 
...
Azure 포털 및 Azure 리소스 관리자 템플릿을 사용하여 이 기능을 활성화하는 방법에 대해 알아봅니다.

- [Azure Portal을 사용하여 Event Hubs 캡처를 사용하도록 설정](event-hubs-capture-enable-through-portal.md)
- [Azure 리소스 관리자 템플릿을 사용하여 이벤트 허브 캡처 사용](event-hubs-resource-manager-namespace-event-hub-enable-capture.md)
...
[대규모 스트리밍: 이벤트 허브 캡처]:https://github.com/yorek/streaming-at-scale/tree/master/event-hubs-capture
