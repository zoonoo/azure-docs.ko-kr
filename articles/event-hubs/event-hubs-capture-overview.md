...
캡처된 데이터는 기록 [Apache Avro][Apache Avro] 형식: 인라인 스키마를 사용 하 여 다양 한 데이터 구조를 제공 하는 간단한, 빠른 이진 형식입니다. 이 형식은 Hadoop 에코시스템, Stream Analytics 및 Azure Data Factory에서 널리 사용됩니다. Avro 작업에 대한 자세한 내용은 이 문서의 뒷부분을 참조하세요.
...
Event Hubs 트래픽은 [처리량 단위](event-hubs-scalability.md#throughput-units)로 제어됩니다. 단일 처리량 단위는 초당 1MB 또는 초당 1000개의 이벤트 수신을 허용하고 송신량은 그 두 배입니다. Standard Event Hubs는 1-20 처리량 단위를 사용 하 여 구성할 수 있습니다 및 더 구입할 수 할당량을 늘릴 [지원 요청][support request]합니다. 구입한 처리량 단위 범위를 벗어나는 사용량은 제한됩니다. Event Hubs 캡처는 내부 Event Hubs 저장소에서 데이터를 직접 복사하여 처리량 단위 송신 할당량을 우회하고 Stream Analytics, Spark 등의 다른 처리 판독기를 위해 송신 내용을 저장합니다.
...
Avro 파일을 탐색 하는 쉬운 방법을 사용 하는 것을 [Avro Tools][Avro Tools] jar from Apache. You can also use [Apache Drill][Apache Drill] 간단한 SQL 기반 환경에 대 한 또는 [Apache Spark][Apache Spark] 수행 하려면 복잡 한 분산 수집 된 데이터에서 처리 합니다. 

### <a name="use-apache-drill"></a>Apache Drill 사용

[Apache Drill][Apache Drill] 는 "오픈 소스 SQL 쿼리 엔진 빅 데이터 탐색에 대 한"입니다 때마다 구조화 및 반 구조화 된 데이터를 쿼리할 수 있습니다. 엔진은 독립 실행형 노드 또는 뛰어난 성능을 위한 대규모 클러스터로 실행할 수 있습니다.

다음 설명서에 나온 것처럼 Avro 파일에서 데이터를 손쉽게 쿼리할 수 있는 Azure Blob Storage에 대한 기본 지원이 제공됩니다.

[Apache Drill: Azure Blob Storage 플러그인][Apache Drill: Azure Blob Storage Plugin]
...
[Apache Spark][Apache Spark] "통합된 분석 엔진이 대규모 데이터 처리에 대 한 합니다." SQL을 비롯한 다른 언어를 지원하며, Azure Blob Storage에 쉽게 액세스할 수 있습니다. Azure에서 Apache Spark를 실행하는 방법은 두 가지가 있으며, 모두 Azure Blob Storage에 손쉽게 액세스할 수 있습니다.
...
[Avro 도구][Avro Tools] jar 패키지를 사용할 수 있습니다. jar 파일을 다운로드한 후 다음 명령을 실행하여 특정 Avro 파일의 스키마를 확인할 수 있습니다.
...
Apache Avro에 대 한 전체 시작 가이드가 [Java][Java] and [Python][Python]합니다. [Event Hubs 캡처 시작](event-hubs-capture-python.md) 문서를 참조할 수도 있습니다.
