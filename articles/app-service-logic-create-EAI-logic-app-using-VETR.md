...
이 항목에서는 일반적인 통합 패턴을 살펴봅니다. "단방향 메시지 중재" 또는 VETR(유효성 검사, 보강, 변환, 경로 지정) VETR 패턴은 소스 엔터티와 대상 엔터티 간에 데이터를 중재합니다. 일반적으로 소스 및 대상은 데이터 소스입니다. 
...
*유효성 검사* - 들어오는 데이터의 정확성을 유효성 검사합니다.
...
Azure 관리 포털 화면의 왼쪽 하단에서 **+새로 만들기** 단추를 클릭하고 논리 앱을 클릭합니다. 이름, 위치, 구독, 리소스 그룹 및 작동 위치를 선택합니다. 리소스 그룹은 앱 컨테이너 역할을 하며 앱의 모든 리소스가 동일한 리소스 그룹으로 이동됩니다.
...
2. **자동으로 응답을 보내나요?** 설정을 false로 그대로 둡니다. _HTTP Method_를 _POST_로 설정하고 _Relative URL_을 _ /OneWayPipeline_으로 설정하여 트리거 동작을 구성합니다.
...
1. 갤러리에서 **BizTalk XML Validator**를 추가하고 _(Validate1)_라는 이름을 지정하여 인스턴스를 만듭니다.
2. 들어오는 XML 메시지의 유효성을 검사하도록 XSD 스키마를 구성합니다. _Validate_ 동작을 선택하고 _triggers('httplistener').outputs.Content_를 _InputXml_ 매개 변수의 값으로 선택합니다.
...
3. 마지막으로, 변환은 유효성 검사에 성공한 경우에만 실행됩니다. 이 조건을 구성하려면 _"Add a condition to be met"_의 상단 오른쪽에서 기어 아이콘을 선택합니다. 조건을 ```equals(actions('xmlvalidator').status,'Succeeded')```로 설정합니다.
...
1. 갤러리에서 **서비스 버스 커넥터**를 추가합니다. _Name_을 _Servicebus1_로 설정하고, _Connection String_을 서비스 버스 인스턴스에 대한 연결 문자열로 설정하고, _Entity Name_을 _Queue_로 설정하고 _Subscription name_을 건너뜁니다. 
2. **메시지 보내기** 동작을 선택하고 동작의 _Message_ 필드를 _actions('transformservice').outputs.OutputXml_로 설정합니다.
...
2. _Response Content_를 "파이프라인 처리 완료됨"으로 설정하고 _Response Status Code_를 "200"으로 설정하여 HTTP 200 OK를 나타내고 조건을 ```@equals(actions('servicebusconnector').status,'Succeeded')``` 식으로 설정합니다.
	
위의 단계를 반복하여 실패한 경우도 HTTP 응답을 보냅니다. 조건을 ```@not(equals(actions('servicebusconnector').status,'Succeeded')).```로 변경합니다.


## 완료
누군가가 HTTP 끝점에 메시지를 보낼 때마다 앱이 트리거되고 방금 만든 작업이 실행됩니다. 앞서 만든 논리 앱을 관리하려면 Azure 관리 포털에서 **찾아보기**를 클릭하고 **논리 앱**을 클릭합니다. 자세한 내용을 보려면 앱을 클릭합니다.
