...
고가용성을 위해 Azure 스택에 Azure 앱 서비스를 준비 하려면 필요한 파일 서버 및 고가용성 구성에서 SQL Server 인스턴스를 배포 합니다. Azure 스택 여러 오류 도메인을 지 원하는에서는 항상 사용 가능한 구성에서 Azure 스택에 Azure 앱 서비스를 사용 하도록 설정 하는 방법에 지침을 제공 합니다.
...
프로덕션 환경에서 리소스 공급자를 작동 하는 경우, 다음과 같은 4 개의 인증서를 제공 해야 합니다.
...
- 가상 컴퓨터 크기 작업자 계층에서 통합 설정
- Azure 함수 포털 및 고급 개발자 도구에 대 한 SSO
