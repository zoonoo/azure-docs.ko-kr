...
7. 파일 공유에 대 한 정보를 입력 한 다음 클릭 **다음**합니다. 파일 공유의 주소는 정규화 된 도메인 이름, 파일 서버 또는 IP 주소를 사용 해야 합니다. 예를 들어 \\\appservicefileserver.local.cloudapp.azurestack.external\websites, 또는 \\\10.0.0.1\websites 합니다.
...
9. 세 개의 각에 대 한 파일 상자 인증서, **찾아보기** 적절 한 인증서 파일을 이동 합니다. 각 인증서에 대 한 암호를 제공 합니다. 이 인증서는에서 만든 것과 [만들기 필요한 인증서 단계](azure-stack-app-service-deploy.md#create-the-required-certificates)합니다. 클릭 **다음** 모든 정보를 입력 한 후 합니다.
