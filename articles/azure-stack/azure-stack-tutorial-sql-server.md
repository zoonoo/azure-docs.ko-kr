...
ms.locfileid: "35234753"
---
# <a name="tutorial-make-sql-databases-available-to-your-azure-stack-users"></a>자습서: 사용자에 게 SQL 데이터베이스 사용 가능한 Azure 스택
...
배포 프로세스에 자세히 설명 되어는 [Azure 스택 아티클에서 사용 하 여 SQL 데이터베이스](azure-stack-sql-resource-provider-deploy.md), 다음과 같은 기본 단계로 구성 됩니다.
...
3. 호스팅 SQL server에 연결 하 여 용량을 제공 합니다. 자세한 내용은 참조 [호스팅 서버 추가](azure-stack-sql-resource-provider-hosting-servers.md)
...
2.  [계획 만들기](azure-stack-create-plan.md)합니다. 이름을 *TestSQLServerPlan*을 선택는 **Microsoft.SQLAdapter** 서비스 및 **SQLServerQuota** 할당량입니다.
    ...
    > [!NOTE]
    > 사용자가 다른 응용 프로그램을 만들 수 있도록, 다른 서비스 계획에 필요할 수 있습니다. 예를 들어 Azure 함수 필요는 **Microsoft.Storage** Wordpress 반면 계획에서 서비스 **Microsoft.MySQLAdapter**합니다.
...
2. 선택 **구독** 다음을 입력 하 고 **TestSQLServerSubscription** 아래 **표시 이름**합니다.
3. 선택 **제안을 선택** > **TestSQLServerOffer** > **만들기**합니다.
4. 선택 **더 많은 서비스** > **구독** > **TestSQLServerSubscription** > **리소스 공급자**합니다.
5. 선택 **등록** 옆에 **Microsoft.SQLAdapter** 공급자입니다.

### <a name="create-a-sql-database"></a>SQL 데이터베이스 만들기

1. 선택 **+**  >  **데이터 + 저장소** > **SQL 데이터베이스**합니다.
2. 기본값을 유지 하거나 다음 필드에 대 한 이러한 예제를 사용 합니다.
    - **데이터베이스 이름**: SQLdb
    - **최대 크기 (MB)**: 100
    - **구독**: TestSQLOffer
    - **리소스 그룹**: SQL RG
3. 선택 **로그인 설정**를 데이터베이스에 대 한 자격 증명을 입력 한 다음 선택 **확인**합니다.
4. 선택 **SKU** > SQL 호스팅 서버에 대해 만든 SQL SKU 선택 >를 클릭 한 **확인**합니다.
5. **만들기**를 선택합니다.
