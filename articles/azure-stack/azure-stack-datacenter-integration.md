...
사용 하 여 서버를 사용 하 여 Azure Stack을 동기화 하는 특정 시간을 선택 해야 합니다.  시간 동기화는 서로 사용 하 여 내부 서비스를 인증 하는 데 사용 되는 Kerberos 티켓을 생성에 사용 되는 Azure Stack 및 해당 인프라 역할을 하는 데 중요 합니다.

시간 동기화 서버에 대 한 IP 인프라의 구성 요소는 대부분 URL을 확인할 수 있지만 일부 지원할 수 있습니다만 IP 주소를 지정 해야 합니다. 연결이 끊긴된 배포 옵션을 사용 하는 경우 Azure Stack에 인프라 네트워크에서 액세스 수 있는지 확인 시간 서버에 있는 회사 네트워크에 지정 해야 합니다.
