...
서식 파일의 유효성을 검사 하려면 다음 PowerShell 모듈에서는 사용 **TemplateValidator** 및 **CloudCapabilities** 폴더: 
...
이 문서에서는 클라우드 기능 파일을 작성 및 유효성 검사기 도구를 실행 합니다.

## <a name="build-cloud-capabilities-file"></a>클라우드 기능 파일 빌드
서식 파일 유효성 검사기를 사용 하려면 먼저 JSON 파일을 작성 AzureRM.CloudCapabilities PowerShell 모듈을 실행 합니다. 통합된 시스템 사용자를 업데이트 하거나 새 서비스 또는 VM 확장을 추가할 경우 또한 해당 모듈을 다시 실행 해야 합니다.
...
3.  서비스 버전을 검색 하 고 클라우드 기능 JSON 파일을 만들도록 하려면 Get CloudCapabilities cmdlet을 사용 합니다. -OutputPath 지정 하지 않으면 파일 AzureCloudCapabilities.Json 만들어집니다 현재 디렉터리에 있습니다. 사용자의 실제 위치를 사용 합니다.
...
서식 파일 유효성 검사 경고 또는 오류 PowerShell 콘솔 및 원본 디렉터리에 HTML 파일에 기록 됩니다. 유효성 검사 보고서의 예는 다음과 같습니다.
