...
템플릿 유효성 검사 도구를 사용 하 여 검사할 수 있는지 여부를 Azure 리소스 관리자 [템플릿](azure-stack-arm-templates.md) Azure Stack에 배포 하기 위한 준비가 되었습니다. 템플릿 유효성 검사 도구는 Azure Stack 도구의 일부로 사용할 수 있습니다. 에 설명 된 단계를 사용 하 여 Azure Stack 도구를 다운로드 합니다 [GitHub에서 도구를 다운로드](azure-stack-powershell-download.md) 문서.

## <a name="overview"></a>개요

서식 파일의 유효성을 검사 하려면 먼저 클라우드 기능 파일을 작성 하며 유효성 검사 도구를 실행 합니다. Azure Stack 도구에서 다음 PowerShell 모듈을 사용 하는 경우:

- 에 **CloudCapabilities** 폴더: `AzureRM.CloudCapabilities.psm1` 서비스 및 Azure Stack 클라우드에 버전을 나타내는 클라우드 기능 JSON 파일을 만듭니다.
- 에 **TemplateValidator** 폴더: `AzureRM.TemplateValidator.psm1` 클라우드 기능 JSON 파일을 사용 하 여 Azure Stack의 템플릿 배포에 대 한 테스트를 합니다.
...
3. 사용 된 `Get-CloudCapabilities` 서비스 버전을 검색 하 고 클라우드 기능 JSON 파일을 만들 cmdlet. 지정 하지 않으면 **-OutputPath**, 파일 AzureCloudCapabilities.Json 현재 디렉터리에 만들어집니다. 실제 Azure 위치를 사용 합니다.
...
템플릿 유효성 검사 경고 또는 오류 PowerShell 콘솔에 표시 되며 원본 디렉터리에 HTML 파일에 기록 됩니다. 다음 스크린샷은 다음과 같습니다. 유효성 검사 보고서의 예
...
| ----- | -----| ----- |
| TemplatePath | 재귀적으로 경로 Azure Resource Manager 템플릿의 찾을 지정 합니다. | 예 |
...
| IncludeStorageCapabilities | SKU 형식 등의 저장소 리소스의 평가 포함합니다. | 아닙니다. |
| 보고서 | 생성된 된 HTML 보고서의 이름을 지정 합니다. | 아닙니다. |
...
- [Azure Stack 용 템플릿 개발](azure-stack-develop-templates.md)
