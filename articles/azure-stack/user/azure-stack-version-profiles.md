...
- API 프로필은 생성 하 여 여러 Azure 클라우드에서 템플릿을 만들 수 있습니다. 프로필에 호환 되 고 안정적인 인터페이스를 제공합니다.
- 프로필에는 1 년에 4 번이 릴리스됩니다.
- 세 개의 프로필 명명 규칙이 사용 됩니다.
    - **latest**  
        전역 Azure에서 릴리스된 최신 API 버전을 포함 합니다.
    - **yyyy-mm-dd-hybrid**  
    번 출시이 이번에 중점을 둡니다 일관성과 안정성 여러 클라우드에서 합니다. 이 프로필에는 최적의 Azure Stack 호환성 대상 으로합니다.
    - **yyyy mm-dd 프로필** 최적의 안정성 및 최신 기능을 분산 합니다.
...
**yyyy-mm-dd-profile**  
이 프로필은 글로벌 Azure 용 년 6 월 및 12 월에 해제 됩니다. Azure Stack;과 함께 작동 하지 않습니다. 일반적으로 많은 주요 변경 됩니다. 최적의 안정성 및 최신 기능 간의 차이 감안 하 하지만 **최신** 는이 프로필이 더 **최신** API 된 시기에 관계 없이 최신 API 버전을 항상으로 구성 해제 합니다. 예를 들어 내일 새 API 버전을 계산 API에 대 한 만들어지면 해당 API 버전에 포함 된 합니다 **최신**, 되지 않습니다는 **yyyy mm-dd 프로필** 이 프로필에 이미 있기 때문에 프로 파일링 합니다. **yyyy mm-dd 프로필** 년 6 월 또는 년 12 월 하기 전에 릴리스된 최신 버전을 설명 합니다.

## <a name="azure-resource-manager-api-profiles"></a>Azure Resource Manager API 프로필

Azure Stack에서 최신 버전의 글로벌 Azure에 API 버전을 사용 하지 않습니다. 솔루션을 만들 때 Azure Stack을 사용 하 여 호환 되는 각 Azure 리소스 공급자에 대 한 API 버전을 찾아야 합니다.

대신 모든 리소스 공급자 및 Azure Stack에서 지 원하는 특정 버전을 연구, 보다 API 프로필을 사용할 수 있습니다. 프로필에는 리소스 공급자 및 API 버전 집합을 지정합니다. SDK 또는 SDK를 사용 하 여 빌드한 도구를 대상으로 되돌아갑니다 `api-version` 프로필에 지정 합니다. API 프로필을 사용 하 여 전체 템플릿의 런타임 시 적용 되는 프로필 버전을 지정할 수 있습니다, Azure Resource Manager 리소스의 올바른 버전을 선택 합니다.
...
예를 들어, 사용 하 여 저장소 계정을 만드는 PowerShell을 사용 하는 경우는 **Microsoft.Storage** 지 원하는 리소스 공급자 **api 버전** 2016-03-30 사용 하 여 VM 및는  **Microsoft.Compute** 인 리소스 공급자 **api-version** 2015-12-01, 저장소에 대 한 PowerShell 모듈 지 원하는 2016-03-30 조회 해야 합니다 및는 모듈은 계산, 2015-02-01을 지원 하 고 다음 설치 합니다. 대신 프로필을 사용할 수 있습니다. Cmdlet을 사용 하 여 `Install-Profile <profilename>`, PowerShell 모듈의 올바른 버전을 로드 합니다.

마찬가지로, Python SDK를 사용 하 여 Python 기반 응용 프로그램 빌드를 프로필을 지정할 수 있습니다. SDK에서 스크립트에 지정 된 리소스 공급자에 대 한 올바른 모듈을 로드 합니다.

개발자 솔루션 작성에 집중할 수 있습니다. API 버전, 리소스 공급자 및 클라우드를 연구 하는 대신 함께, 프로필을 사용 하 고 코드는 해당 프로필을 지 원하는 모든 클라우드에 걸쳐 작동 하는지 알 수 있습니다.
...
- **Go**  
Go SDK에서 프로필을 다른 서비스에서 다른 버전을 사용 하 여 다양 한 리소스 유형 조합입니다. 프로필은 해당 버전을 사용 하 여 경로 프로필에서 사용할 수는 **YYYY-월-일** 형식입니다. 정보를 참조 하세요 [Go 사용 하 여 API 버전 프로필](azure-stack-version-profiles-go.md)합니다.
- **Ruby**  
...
- **Python**  
Python SDK는 Azure Stack 및 전역 Azure와 같은 다른 클라우드 플랫폼을 대상으로 하는 API 버전 프로필을 지원 합니다. 하이브리드 클라우드를 위한 솔루션을 만드는 데 API 프로필을 사용할 수 있습니다. 내용은 [Python과 함께 사용 하 여 API 버전 프로필](azure-stack-version-profiles-python.md)
