...
이 문서에서는 Microsoft Azure ExpressRoute 직접 연결을 사용 하 여 Azure Stack virtual network를 Azure virtual network에 연결 하는 방법을 보여 줍니다.
...
다음 아키텍처 다이어그램은 여러 테 넌 트에 연결할 ExpressRoute 라우터를 통해 Azure Stack 인프라에서 Azure Microsoft edge에서 보여 줍니다.

*그림 2. 다중 테 넌 트 연결*

![ExpressRoute 사용 하 여 다중 테 넌 트 연결](media/azure-stack-connect-expressroute/Architecture.png)

이 문서의 예제에 표시 된 동일한 다중 테 넌 트 아키텍처를 사용 *그림 2* Azure Stack ExpressRoute 개인 피어 링을 사용 하 여 Azure에 연결 합니다. 가상 네트워크 게이트웨이에서 사이트 간 VPN 연결을 ExpressRoute 라우터는 Azure Stack에서 사용 하 여 종료 되었습니다.

이 문서의 단계는 Azure에서 해당 Vnet에 Azure Stack에서 두 명의 다른 테 넌 트에서 두 개의 Vnet 간에 종단 간 연결을 만드는 방법을 보여 줍니다. 두 테 넌 트를 설정 하는 것은 선택 사항, 단일 테 넌 트에 대 한이 단계를 사용할 수도 있습니다.
...
1. 아래 **추천**를 선택 **Virtual network**합니다.

1. 아래 **가상 네트워크 만들기**, 해당 필드에 다음 표에 나와 있는 값을 입력 합니다.
...
1. 앞서 만든 구독이 **구독** 필드에 채워지는 것을 볼 수 있습니다. 나머지 필드:

    * 아래 **리소스 그룹**를 선택 **새로 만들기** 새 리소스 그룹을 만들거나 이미 있는 경우 하나를 선택 **기존 항목 사용**합니다.
    * 기본 확인 **위치**합니다.
    * **만들기**를 선택합니다.
    * (선택 사항) 선택 **대시보드에 고정**합니다.

#### <a name="create-the-gateway-subnet"></a>게이트웨이 서브넷 만들기

1. 아래 **가상 네트워크**, Tenant1VNet1를 선택 합니다.
1. **설정**에서 **서브넷**을 선택합니다.
...
1. 선택 **공용 IP 주소**>**공용 IP 주소 선택**를 선택한 후 **새로 만들기**합니다.
1. 에 **이름을** 필드를 입력 합니다 **GW1-PiP** 선택한 **확인**합니다.
1. **VPN 종류**에 **Route-based**(경로 기반)이 기본으로 선택되어 있습니다. 이 설정을 유지합니다.
1. **구독** 및 **위치**가 올바른지 확인합니다. **만들기**를 선택합니다.
...
1. 에 대 한 합니다 **IP 주소** 필드를 참조 하십시오 *그림 2*합니다. 테 넌 트 1에 대 한 ExpressRoute 라우터의 LAN 하위 인터페이스의 IP 주소가 10.60.3.255 없습니다. 사용자 고유의 환경에 대 한 라우터의 해당 인터페이스의 IP 주소를 입력 합니다.
1. 에 **주소 공간** 필드를 Azure에 연결 하려는 Vnet의 주소 공간을 입력 합니다. 테 넌 트 1에 대 한 서브넷 *그림 2* 됩니다.

   * 192.168.2.0/24는 Azure에서 VNet 허브입니다.
   * 10.100.0.0/16 스포크에서 Azure VNet입니다.

   > [!IMPORTANT]
   > 이 예제에서는 Azure Stack 게이트웨이 및 ExpressRoute 라우터 간의 사이트 간 VPN 연결에 대 한 정적 경로 사용 하는 것을 가정 합니다.

1. 확인 프로그램 **구독**를 **리소스 그룹**, 및 **위치** 올바른지 합니다. **만들기**를 선택합니다.
...
1. 선택 된 **구독**를 **리소스 그룹**, 및 **위치**합니다. **확인**을 선택합니다.
...
1. **가상 머신 만들기**>**기본 사항**를 입력 **VM01** 으로 **이름**합니다.
...
1. 아래 **설정을**, 있는지 확인 합니다.
   ...
   * 로 설정 된 서브넷 **10.1.1.0/24**합니다.
   ...
   기본 설정을 사용 하 여 선택한 **확인**합니다.

1. 아래 **요약**VM 구성을 검토 하 고 선택한 **확인**합니다.
...
>* 가상 머신 만들기
>
>예를 들어 테 넌 트 2를 사용 하려는 경우 겹침 방지 하려면 IP 주소를 변경 해야 합니다.

### <a name="configure-the-nat-virtual-machine-for-gateway-traversal"></a>게이트웨이 통과 위해 NAT 가상 머신 구성

> [!IMPORTANT]
> 이 섹션은 Azure Stack 개발 키트 배포에만 해당 합니다. NAT는 다중 노드 배포에 필요 하지 않습니다.

Azure Stack 개발 키트 독립적 이며 실제 호스트를 배포할 네트워크에서 격리 합니다. VIP 네트워크에 게이트웨이가 연결 된 외부, 네트워크 주소 변환 (NAT)를 수행 하는 라우터 뒤에 숨겨져 합니다.
...
Azure Stack 구성를 마친 후에 Azure 리소스를 배포할 수 있습니다. 다음 다이어그램은 Azure에서 테 넌 트 가상 네트워크의 예를 보여 줍니다. Azure에서 VNet에 대 한 다른 이름 및 주소 지정 체계를 사용할 수 있습니다. 그러나 Azure 및 Azure Stack에서 Vnet의 주소 범위는 고유 해야 하며 겹치지 않습니다.
...
1. 단계를 따릅니다 [만들기 및 ExpressRoute 회로 수정](../expressroute/expressroute-howto-circuit-portal-resource-manager.md) Azure 구독을 사용 하 여 ExpressRoute 회로 만드는 합니다.
   ...
   >[!NOTE]
   >가 ExpressRoute 회로 설정할 수 있도록 서비스에 회로 서비스 키를 제공 합니다.
...
기본적으로 Windows Server 2016에는 방화벽을 통해 들어오는 ICMP 패킷을 허용 하지 않습니다. Ping 테스트에 대 한 사용 중인 모든 가상 머신에 대 한 들어오는 ICMP 패킷을 허용 해야 합니다. ICMP에 대 한 방화벽 규칙을 만들려면 관리자 권한 PowerShell 창에서 다음 cmdlet을 실행 합니다.
...
1. Azure VNet의 가상 컴퓨터에서 IPv4 주소를 ping 합니다.
   ...
   예제 환경에서는 IPv4 주소 10.1.1.x/24 서브넷 에서입니다. 사용자 환경에서 주소가 다를 수 있습니다. 하지만 테 넌 트 VNet 서브넷에 대해 만든 서브넷에 있어야 합니다.
