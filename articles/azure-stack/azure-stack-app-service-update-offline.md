...
이 문서의 지침을 따르면 업그레이드할 수 있습니다는 [앱 서비스 리소스 공급자](azure-stack-app-service-overview.md) 있는 Azure 스택 환경에 배포 합니다.

* 인터넷에 연결 되어 있지
* Active Directory Federation Services (AD FS)에 의해 보안.

> [!IMPORTANT]
> 업그레이드를 실행 하기 전에 이미 완료 되었는지 확인는 [Azure 스택 리소스 공급자에 있는 Azure 앱 서비스의 배포](azure-stack-app-service-deploy-offline.md)
>
>
...
연결이 끊어진된 환경에 응용 프로그램 서비스를 업그레이드 하려면 먼저 만들어야 합니다 오프 라인 업그레이드 패키지에서 인터넷에 연결 하는 컴퓨터입니다.

1. Appservice.exe 관리자 권한으로 실행

    ![앱 서비스 설치 관리자][1]

2. 클릭 **고급** > **오프 라인 패키지 만들기**

    ![고급 응용 프로그램 서비스 설치 관리자][2]

3. 앱 서비스 설치 관리자는 오프 라인 업그레이드 패키지를 만들고에 경로 표시 합니다.  클릭할 수 있는 **폴더 열기** 파일 탐색기에서 폴더를 열려고 합니다.

4. Azure 스택 호스트 컴퓨터에 설치 관리자 (AppService.exe) 및 오프 라인 업그레이드 패키지를 복사 합니다.

## <a name="complete-the-upgrade-of-app-service-on-azure-stack"></a>Azure 스택 앱 서비스의 업그레이드를 완료

> [!IMPORTANT]
> 앱 서비스 설치 관리자는 Azure 관리자 스택에서 Azure 리소스 관리자 끝점에 도달할 수 있는 컴퓨터에서 실행 되어야 합니다.
>
>
...
2. 클릭 **고급** > **오프 라인 설치 또는 업그레이드를 완료**합니다.
