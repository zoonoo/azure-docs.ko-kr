...
2. 베이스 보드 관리 컨트롤러 (BMC)에서 올바른 IP 주소를 구성 하 고 OEM 제공 설명서 당 BIOS 설정을 모두 적용 합니다.
3. 현재 펌웨어 기준선을는 HLH에서 실행 되는 하드웨어 제조업체에서 제공 하는 도구를 사용 하 여 모든 구성 요소에 적용 됩니다.
4. Azure Stack 관리 포털에서 노드 추가 작업을 실행 합니다.
5. 노드 추가 작업에 성공 했는지 확인 합니다. 이렇게 하려면 다음을 확인 합니다 [ **상태** 배율 단위의](#monitor-add-node-operations)합니다. 

## <a name="add-the-node"></a>노드 추가 
새 노드를 추가 하는 관리 콘솔 또는 PowerShell을 사용할 수 있습니다. 노드 추가 작업 먼저 사용 가능한 계산 용량으로 새 규모 단위 노드를 추가 하 고 저장소 용량을 자동으로 확장 합니다. Azure Stack은 하이퍼 수렴 형 시스템 이므로 용량을 자동으로 확장 위치 *계산* 하 고 *저장소* 함께 확장 합니다.

### <a name="use-the-admin-console"></a>관리 콘솔을 사용 하 여
1. Azure Stack 운영자로 서 Azure Stack 관리 포털에 로그인 합니다.
2. 이동할 **지역 관리** > **단위**, 한 다음에 대 한 용량을 확장 하려는 배율 단위를 선택 합니다.  
   ![배율 단위를 선택 합니다.](media/azure-stack-add-scale-node/select-node1.png)

   ![배율 단위 세부 정보 보기](media/azure-stack-add-scale-node/select-node2.png)
 
3. 다음 매개 변수를이 필요 합니다.  
   - **BMC IP 주소** 추가한 각 눈금 단위 노드에 대 한 합니다. (하나의 IP 주소가 한 줄입니다.) ![노드 추가](media/azure-stack-add-scale-node/add-node.png)

### <a name="use-powershell"></a>PowerShell 사용

사용 합니다 **새로 만들기-AzsScaleUnitNodeObject** 하나 이상의 노드를 추가 하는 cmdlet입니다.  
...
## <a name="monitor-add-node-operations"></a>노드 추가 작업을 모니터링 
