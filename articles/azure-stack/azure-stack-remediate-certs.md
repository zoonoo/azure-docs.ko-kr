...
**재구성** -내보내기 PFX 파일을 **TripleDES-SHA1** 암호화 합니다. 이 모든 Windows 10 클라이언트에 대 한 기본 인증서 스냅인 또는 사용 하 여에서 내보낼 때 `Export-PFXCertificate`합니다.

## <a name="read-pfx"></a>PFX 읽기

**경고** -암호는 인증서의 개인 정보 보호 합니다.  

**재구성** -내보내기 PFX 파일에 대 한 선택적 설정이 **인증서 개인 정보 사용**합니다.  
...
**재구성** -Azure Stack 인증서 서명 요청 (CSR (인증서)의 SHA256 서명 알고리즘을 사용 하 여 다시 생성 요청 생성 서명의 단계를 사용 합니다. 인증서 다시 발급 인증 기관에 CSR을 다시 제출 하십시오.

## <a name="private-key"></a>개인 키

**오류** -개인 키 누락 되었거나 로컬 컴퓨터 특성이 포함 되지 않습니다.  

**재구성** -CSR을 생성 하는 컴퓨터에서 다시의 단계를 사용 하 여 인증서를 내보냅니다 [배포에 대 한 준비 Azure Stack PKI 인증서](azure-stack-prepare-pki-certs.md#prepare-certificates-for-deployment)합니다. 이러한 단계는 로컬 컴퓨터 인증서 저장소에서 내보내기 포함 됩니다.

## <a name="certificate-chain"></a>인증서 체인

**오류** -인증서 체인이 완료 되지 않습니다.  

**재구성** -인증서는 완전 한 인증서 체인을 포함 해야 합니다. 다시의 단계를 사용 하 여 인증서를 내보냅니다 [배포에 대 한 준비 Azure Stack PKI 인증서](azure-stack-prepare-pki-certs.md#prepare-certificates-for-deployment) 옵션을 선택 하 고 **가능 하면 인증 경로 있는 모든 인증서를 포함 합니다.**
...
**재구성** -Azure Stack 인증서 서명 요청 생성 올바른 DNS 이름 사용 하 여 CSR을 다시 생성 하는 단계를 사용 하 여 Azure Stack 끝점을 지원 합니다. 인증 기관에 CSR을 다시 제출 하 고 다음의 단계에 따라 [배포에 대 한 준비 Azure Stack PKI 인증서](azure-stack-prepare-pki-certs.md#prepare-certificates-for-deployment) CSR을 생성 하는 컴퓨터에서 인증서를 내보냅니다.  

## <a name="key-usage"></a>키 사용

**오류** -디지털 서명 또는 키 암호화 키 사용은 누락 되었거나 확장 된 키 사용 서버 인증과 클라이언트 인증입니다.  

**재구성** -의 단계를 따르세요 [Azure Stack 인증서 서명 요청 생성](azure-stack-get-pki-certs.md) 올바른 키 사용 특성을 사용 하 여 CSR을 다시 생성 합니다. 인증 기관에 CSR을 다시 제출 하 고 인증서 템플릿을 요청에서 키 용도 덮어쓰지 않고는 확인 합니다.
...
**재구성** -다시의 단계를 사용 하 여 인증서를 내보냅니다 [배포에 대 한 준비 Azure Stack PKI 인증서](azure-stack-prepare-pki-certs.md#prepare-certificates-for-deployment) 옵션을 선택 하 고 **가능 하면 인증 경로 있는 모든 인증서를 포함 합니다.** 내보내기에 대 한 리프 인증서만 선택 되어 있는지 확인 합니다.
...
합니다 **AzsReadinessChecker** 호출 하는 도우미 cmdlet 포함 `Repair-AzsPfxCertificate`을 가져올 수 있는 고 그런 다음 내보내기 PFX 파일을 비롯 한 일반적인 패키징 문제를 해결 하려면:

- *PFX 암호화* TripleDES-SHA1 아닙니다.
- *개인 키* 로컬 컴퓨터 특성이 없습니다.
- *인증서 체인* 불완전 하거나 잘못 되었습니다. PFX 패키지, 없는 경우 로컬 컴퓨터 인증서 체인을 포함 해야 합니다.
- *다른 인증서*

`Repair-AzsPfxCertificate` 새 CSR을 생성 하 고 인증서를 다시 실행 하는 경우 도움이 되지 않습니다.

### <a name="prerequisites"></a>필수 조건

다음 필수 구성 요소 도구를 실행 하는 컴퓨터의 준비 되어야 합니다.
...
2. PowerShell 프롬프트에서 PFX 암호를 설정 하려면 다음 cmdlet을 실행 합니다. 대체 `PFXpassword` 실제 암호를 사용 하 여:
...
- [Azure Stack 보안에 자세히 알아보려면 여기](azure-stack-rotate-secrets.md)합니다.
