...
다음 다이어그램에 표시 된 것과 같이 개인 ip 보급 TOR 스위치에 공간이 접두사 목록을 사용 하 여 제한 합니다. 접두사 목록을 개인 IP 서브넷 및 경로 맵 TOR 및 테두리 사이의 연결에 적용 하는 것을 거부 합니다.
...
테두리 장치 TOR 장치 P2P 인프라 네트워크에 외부 네트워크 또는 공용 Vip를 트래픽은를 가리키는 정적 경로 사용 하 여 구성 되어야 합니다. 배포에 대 한 정적 경로 BMC 네트워크에 필요합니다. 고객은 BMC 네트워크에 상주 하는 리소스에 액세스 하는 테두리에서 고정 경로 그대로 선택할 수 있습니다.  고정 경로를 추가 *스위치 인프라* 하 고 *전환 관리* 네트워크는 선택 사항입니다.
