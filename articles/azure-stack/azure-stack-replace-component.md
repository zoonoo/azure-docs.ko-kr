...
이 문서에서는 비 핫 스왑 가능한 되는 하드웨어 구성 요소를 대체 하는 일반적인 프로세스를 설명 합니다. 실제 대체 단계는 달라질 수 (oem) 하드웨어 공급 업체에 따라. Azure 스택 통합 시스템에만 적용 되는 자세한 단계에 대 한 공급 업체의 필드 (FRU) 교체 장치 설명서를 참조 하십시오.
...
Azure 스택 상태 및 모니터링 시스템에는 저장소 공간 다이렉트에 의해 제어 되는 데이터 드라이브 및 네트워크 어댑터의 상태를 모니터링 합니다. 다른 하드웨어 구성 요소를 모니터링 하지 않습니다. 다른 하드웨어 구성에 대 한 경고는 모니터링 하드웨어 수명 주기 호스트에서 실행 되는 솔루션 공급 업체 관련 하드웨어에서 발생 합니다.

## <a name="component-replacement-process"></a>구성 요소 교체 프로세스

다음 단계는 구성 요소 교체 프로세스에 개략적으로 제공 됩니다. OEM 제공 FRU 설명서를 참조 하지 않고 다음이 단계를 수행 하지 마십시오.
