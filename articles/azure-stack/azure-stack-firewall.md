...
ms.locfileid: "56207132"
---
# <a name="azure-stack-firewall-integration"></a>Azure Stack 방화벽 통합
Azure Stack 보안 하는 데 방화벽 장치를 사용 하는 것이 좋습니다. 방화벽 분산된의 서비스 거부 (DDOS) 공격, 침입 감지 및 콘텐츠 검사 등을 방어 하는 데 도움이 됩니다. 그러나 blob, 테이블 및 큐와 같은 Azure storage 서비스에 대 한 처리량 병목 지점이 될 수도 있습니다.

 연결이 끊긴된 배포 모드를 사용 하는 경우 AD FS 끝점에 게시 해야 합니다. 자세한 내용은 참조는 [데이터 센터 통합 identity 문서](azure-stack-integrate-identity.md)합니다.

Azure 리소스 관리자 (관리자), 관리자 포털 및 Key Vault (관리자) 끝점 반드시 필요 하지 않습니다 외부 게시 합니다. 예를 들어, 서비스 공급자, 네트워크, 내부 및 인터넷에서에서 Azure Stack만 관리 하 여 공격 노출 영역을 제한할 수 있습니다.

엔터프라이즈 조직에 기존 회사 네트워크 외부 네트워크 일 수 있습니다. 이 시나리오에서는 Azure Stack을 회사 네트워크에서 작동 하도록 끝점을 게시 해야 합니다.

### <a name="network-address-translation"></a>NAT(Network Address Translation)
네트워크 주소 변환 (NAT)는 (dvm이) 외부 리소스에 액세스 하 고 배포 하는 동안 인터넷 뿐만 아니라 응급 복구 콘솔 (ERCS) Vm 배포 가상 머신 또는 권한 있는 끝점 (PEP) 하는 동안 허용 하는 것이 좋습니다. 등록 및 문제 해결입니다.

NAT는 공용 IP 주소에서 외부 네트워크 또는 공용 Vip 대신 수도 있습니다. 그러나 테 넌 트 사용자 경험을 제한 하 고 복잡성 증가 하므로 이렇게 하지 좋습니다. 한 가지 옵션 한 일 NAT 풀에서 IP 사용자 당 하나의 공용 IP를 계속 해야 하는 것입니다. 또 다른 방법은 사용자가 사용할 수 있습니다 하는 모든 포트에 대 한 VIP 사용자 당 NAT 규칙을 필요한 NAT 다 합니다.
...
라우팅할 수 있는 공용 IP 주소는 외부 네트워크에서 공용 VIP 풀에 대 한 배포 시 지정 됩니다. Edge 시나리오에서 좋지 않습니다 보안상의 이유로 다른 네트워크에 라우팅할 수 있는 공용 Ip를 사용 하도록 합니다. 이 시나리오에서는 Azure와 같은 공용 클라우드에 같이 자체 제어 되는 전체 클라우드 환경을 경험 하기 사용자.  
