...
Azure Stack 준비 상태 검사기 도구를 사용 (**AzsReadinessChecker**) Azure Active Directory (Azure AD)이 Azure Stack과 함께 사용할 준비가 유효성을 검사 합니다. Azure Stack 배포를 시작 하기 전에 Azure id 솔루션의 유효성을 검사 합니다.  

준비 상태 검사기의 유효성을 검사 합니다.

- Azure Active Directory (Azure AD)를 id 공급자로 Azure Stack에 대 한 합니다.
- 사용 하려는 Azure AD 계정을 Azure Active Directory의 전역 관리자로 로그인 수 있습니다.
...
- Azure stack 및 Azure Active Directory 전역 관리자 인지 확인 하려면 Azure AD 계정을 식별 합니다.
- Azure AD 테 넌 트 이름을 식별 합니다. 테 넌 트 이름에 Azure Active Directory;에 대 한 기본 도메인 이름 이어야 합니다. 예를 들어 **contoso.onmicrosoft.com**합니다.
- 사용 하 여 Azure 환경을 식별 합니다. 환경 이름 매개 변수에 대해 지원 되는 값은 **AzureCloud**를 **AzureChinaCloud**, 또는 **AzureUSGovernment**사용할 Azure 구독에 따라 합니다.

## <a name="steps-to-validate-azure-identity"></a>Azure id의 유효성을 검사 하는 단계

1. 설치 하려면 다음 명령을 실행 하 고 필수 구성 요소를 충족 하는 컴퓨터에서 관리자 권한 PowerShell 명령 프롬프트를 엽니다 **AzsReadinessChecker**:  

   ```powershell
   Install-Module Microsoft.AzureStack.ReadinessChecker -Force
   ```

2. PowerShell 프롬프트에서 설정 하려면 다음 명령을 실행 **$serviceAdminCredential** Azure AD 테 넌 트에 대 한 서비스 관리자.  바꿉니다 **serviceadmin\@contoso.onmicrosoft.com** 계정 및 테 넌 트 이름:

   ```powershell
   $serviceAdminCredential = Get-Credential serviceadmin@contoso.onmicrosoft.com -Message "Enter credentials for service administrator of Azure Active Directory tenant"
   ```

3. PowerShell 프롬프트에서 Azure AD의 유효성 검사를 시작 하려면 다음 명령을 실행 합니다.

   - 환경 이름 값을 지정 **AzureEnvironment**합니다. 환경 이름 매개 변수에 대해 지원 되는 값은 **AzureCloud**를 **AzureChinaCloud**, 또는 **AzureUSGovernment**사용할 Azure 구독에 따라 합니다.
   - 바꿉니다 **contoso.onmicrosoft.com** Azure Active Directory 테 넌 트 이름입니다.
...
**원인** -계정 암호가 만료 되거나 임시이 때문에 로그인 할 수 없습니다.

**해상도** -PowerShell에서 다음 명령을 실행 하 고 다음 지시에 따라 암호를 다시 설정 합니다.
...
**원인** -지정된 된 Azure Active Directory에 로그인 한 계정 수 없습니다 (**AADDirectoryTenantName**). 이 예에서 **AzureChinaCloud** 로 지정 합니다 **AzureEnvironment**합니다.
...
**원인** -계정이 Azure Active Directory의 관리자가 아닌 계정 로그인 할 수 있지만 (**AADDirectoryTenantName**).  

**해상도** -에 로그인 합니다 [Azure portal](https://portal.azure.com) 로 계정 소유자 **Azure Active Directory**, 다음 **사용자**, 다음 **선택는 사용자**, 한 다음 **디렉터리 역할**, 사용자가 확인을 **전역 관리자**합니다. 계정이 있는 경우는 **사용자**로 이동 하세요 **Azure Active Directory** > **사용자 지정 도메인 이름**에 대 한 제공 이름 확인 및  **AADDirectoryTenantName** 이 디렉터리에 대 한 주 도메인 이름으로 표시 됩니다. 이 예에서 **contoso.onmicrosoft.com**합니다.
