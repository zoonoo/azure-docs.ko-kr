...
[AzCopy][azure-azcopy], [Azure PowerShell][azure-powershell], [Azure Storage Client Library for .NET][azure-storage-client-library] 또는 탐색 도구를 통해 Azure Blob 저장소에 액세스할 수 있습니다. 사용 가능한 도구는 다음과 같습니다.
...
1. [Azure PowerShell 설치 및 구성][powershell-install-configure]의 지침에 따라 Azure PowerShell 콘솔 창을 엽니다.
...
12. 다음 샘플은 HDInsight 헤드 노드의 로컬 파일 시스템에서 /example/data 디렉터리로 davinci.txt 파일을 복사하는 방법을 보여 줍니다.
...
기본적으로 Azure SQL 데이터베이스는 Azure HDInsight 같은 Azure 서비스로부터의 연결을 허용합니다. 이 방화벽 설정을 사용하지 않도록 설정한 경우 Azure 관리 포털에서 사용하도록 설정해야 합니다. SQL 데이터베이스 만들기 및 방화벽 규칙 구성에 대한 지침은 [SQL 데이터베이스 만들기 및 구성][sqldatabase-create-configue]을 참조하세요. 
...
3. Azure PowerShell 콘솔 창에 스크립트를 붙여넣고 실행합니다. Azure Blob 저장소에서 데이터 파일을 검색하기 위한 PowerShell 샘플을 보려면 [HDInsight 시작][hdinsight-get-started]을 참조하세요.

Sqoop 사용에 대한 자세한 내용은 [HDInsight와 함께 Sqoop 사용][hdinsight-use-sqoop]을 참조하세요.
