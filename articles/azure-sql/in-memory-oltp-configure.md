...
네이티브 모듈은 트랜잭션 관리에 하나의 큰 [ATOMIC 블록](/sql/relational-databases/in-memory-oltp/atomic-blocks-in-native-procedures) 을 사용해야 합니다. 명시적 BEGIN TRANSACTION 또는 ROLLBACK TRANSACTION에 대한 역할이 없습니다. 코드가 비즈니스 규칙 위반을 감지한 경우 [THROW](/sql/t-sql/language-elements/throw-transact-sql) 문으로 ATOMIC 블록을 종료할 수 있습니다.
...
3. 저장 프로시저 T-SQL 코드가 고유하게 컴파일된 저장 프로시저에 지원하지 않는 기능을 사용하는지를 확인합니다. 필요한 경우 해결 방법을 구현합니다.
   ...
   자세한 내용은 [고유하게 컴파일된 저장 프로시저에 대한 마이그레이션 문제](/sql/relational-databases/in-memory-oltp/a-guide-to-query-processing-for-memory-optimized-tables)를 참조하세요.
...
* [고유하게 컴파일된 저장 프로시저의 소개](/sql/relational-databases/in-memory-oltp/a-guide-to-query-processing-for-memory-optimized-tables)
* [메모리 최적화 관리자](/sql/relational-databases/in-memory-oltp/memory-optimization-advisor)
