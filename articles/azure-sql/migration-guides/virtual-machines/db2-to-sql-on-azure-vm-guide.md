...
description: 이 가이드에서는 db2 용 SQL Server Migration Assistant를 사용 하 여 Azure Vm에서 Db2 데이터베이스를 SQL Server으로 마이그레이션하는 방법을 설명 합니다.
ms.custom: ''
...
Db2 데이터베이스를 SQL Server로 마이그레이션하려면 다음이 필요 합니다.

- [원본 환경이 지원](/sql/ssma/db2/installing-ssma-for-Db2-client-Db2tosql#prerequisites)되는지 확인 합니다.
- [Db2 용 SSMA (SQL Server Migration Assistant)](https://www.microsoft.com/download/details.aspx?id=54254)
- Azure에서 원본 환경과 SQL Server VM 간의 [연결](../../virtual-machines/windows/ways-to-connect-to-sql.md) . 
- [AZURE VM의 대상 SQL Server](../../virtual-machines/windows/create-sql-vm-portal.md)입니다. 
...
1. 스키마 게시: **SQL Server 메타 데이터 탐색기** **의 데이터베이스 노드에서 데이터베이스** 를 마우스 오른쪽 단추로 클릭 하 고 **데이터베이스와 동기화** 를 선택 합니다.
...
1. Db2 및 SQL Server 인스턴스 모두에 대 한 연결 정보를 제공 합니다. 
