...
> [!NOTE]
> 구성 가능한 백업 저장소 중복성은 현재 동남 아시아 Azure 지역 에서만 사용 가능 합니다. Azure SQL Database 미리 보기에서 원본 데이터베이스가 로컬 중복 또는 영역 중복 백업 저장소 중복성으로 생성 된 경우 다른 Azure 지역의 서버에 대 한 데이터베이스 복사는 지원 되지 않습니다. 
