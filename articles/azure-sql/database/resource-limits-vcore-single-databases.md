...
\* 최대 vCore 구성이 더 작은 서비스 목표는 열 저장소 인덱스를 만들고 사용하기 위한 메모리가 부족할 수 있습니다.  열 저장소에 성능 문제가 발생하는 경우 최대 vCore 구성을 높여 사용 가능한 최대 메모리를 늘리세요.  
\*\* 최대 IO 크기 값은 8KB~64KB입니다. 실제 IOPS는 워크로드에 따라 달라집니다. 자세한 내용은 [데이터 IO 거버넌스](resource-limits-logical-server.md#resource-governance)를 참조하세요.
...
\* 로컬 SSD IO 외에도 워크로드는 원격 [페이지 서버](service-tier-hyperscale.md#page-server) IO를 사용합니다. 유효 IOPS는 워크로드에 따라 달라집니다. 자세한 내용은 [데이터 IO 거버넌스](resource-limits-logical-server.md#resource-governance) 및 [리소스 사용률 통계의 데이터 IO](hyperscale-performance-diagnostics.md#data-io-in-resource-utilization-statistics)를 참조하세요.
...
|스토리지 유형| [참고 1](#notes) |[참고 1](#notes)|[참고 1](#notes)|[참고 1](#notes) |[참고 1](#notes) |[참고 1](#notes) |[참고 1](#notes) |
|최대 로컬 SSD IOPS *|64000 |72000 |80,000 |96000 |128000 |160000 |204800 |
|최대 로그 전송률(MBps)|100 |100 |100 |100 |100 |100 |100 |
|IO 대기 시간(근사치)|[참고 2](#notes)|[참고 2](#notes)|[참고 2](#notes)|[참고 2](#notes)|[참고 2](#notes)|[참고 2](#notes)|[참고 2](#notes)|
...
|스토리지 유형|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|로컬 SSD|
|IO 대기 시간(근사치)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|1-2ms(쓰기)<br>1-2ms(읽기)|
|최대 데이터 IOPS *|12,499|15,624|18,748|21,873|24,998|28,123|
...
|최대 데이터 IOPS *|31,248|37,497|49,996|99,993|160,000|
