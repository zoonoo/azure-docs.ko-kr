...
> 지역 보조 쓰기 가능으로 만들어야 하는 경우 아래 단계를 사용하여 지역 복제 링크를 중단하면 됩니다.
> 1. [Remove-AzSqlDatabaseSecondary](/powershell/module/az.sql/remove-azsqldatabasesecondary) cmdlet을 사용하여 보조 데이터베이스를 읽기-쓰기 독립 실행형 데이터베이스로 만듭니다. 주 데이터베이스에서 커밋되었지만 보조 데이터베이스로 복제되지 않은 데이터는 모두 손실됩니다. 이전의 주 데이터베이스가 사용 가능할 때 또는 경우에 따라 이전의 주 데이터베이스를 사용 가능한 최신 시점으로 복원함으로써 이러한 변경 내용을 복구할 수 있습니다.
...
> [!NOTE]
> [Azure Active Directory](https://techcommunity.microsoft.com/t5/azure-sql/support-for-azure-ad-user-creation-on-behalf-of-azure-ad/ba-p/2346849) 인증이 주 또는 보조 논리 서버에서 활성 상태인(사용하도록 설정된) 경우에는 다른 Azure 테넌트의 논리 서버에 지역 복제본을 만드는 것이 지원되지 않습니다.
