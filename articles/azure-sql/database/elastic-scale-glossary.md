...
**데이터 종속 라우팅**: 특정 분할 키가 있는 경우 애플리케이션에서 분할된 데이터베이스에 연결할 수 있는 기능입니다. [데이터 종속 라우팅](elastic-scale-data-dependent-routing.md)을 참조하세요. **[Multi-Shard Query](elastic-scale-multishard-querying.md)** 를 포함합니다.
...
![단일 및 다중 테넌트 데이터베이스](./media/elastic-scale-glossary/shards-single-multi.png)
...
**수직적 크기 조정**: 개별 분할된 데이터베이스의 컴퓨팅 크기를 확장 또는 축소하는 작업입니다. 예를 들어, 표준에서 프리미엄(더 많은 컴퓨팅 리소스 결과)으로 분할된 데이터베이스를 변경합니다. 
