...
1. `SELECT` 문을 사용하여 테이블을 쿼리합니다.
    ...
    **결과**
    ...
    ![RTestData 테이블의 콘텐츠](./media/r-script-create-quickstart/select-rtestdata.png)
...
1. 다음 R 스크립트를 실행합니다. `SELECT` 문을 사용하여 테이블에서 데이터를 검색하고 R 런타임을 통해 이를 전달하고 데이터를 데이터 프레임으로 반환합니다. `WITH RESULT SETS` 절에서는 SQL Database에 대해 반환된 데이터 테이블의 스키마를 정의하고, *NewColName* 열 이름을 추가합니다.
    ...
    **결과**
    ...
    ![테이블에서 데이터를 반환하는 R 스크립트의 출력](./media/r-script-create-quickstart/r-output-rtestdata.png)
...
1. 입력 데이터 없이 R 스크립트만 사용하여 값을 생성할 수도 있습니다(`@input_data_1`이 공백으로 설정됨).
    ...
    **결과**
    ...
    ![@script를 입력으로 사용하는 쿼리 결과](./media/r-script-create-quickstart/r-data-generated-output.png)
...
![R의 설치된 패키지](./media/r-script-create-quickstart/r-installed-packages.png)
...
> [Azure SQL Database Machine Learning Services(미리 보기)를 사용하여 R에서 예측 모델 만들기 및 학습](r-train-score-model-create-quickstart.md) 빠른 시작을 수행합니다.
