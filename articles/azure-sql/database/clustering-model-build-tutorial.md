...
세 부분으로 구성된 이 자습서 시리즈의 2부에서는 R에서 K-평균 모델을 빌드하여 클러스터링을 수행합니다. 이 시리즈의 다음 부분에서는 Azure SQL Database Machine Learning Services(미리 보기)를 사용하여 이 모델을 Azure SQL Database의 데이터베이스에 배포합니다.
...
* 이 자습서 시리즈의 2부에서는 [**1부**](clustering-model-prepare-data-tutorial.md) 및 해당 전제 작업을 완료했다고 가정합니다.
...
![Elbow 그래프](./media/clustering-model-build-tutorial/elbow-graph.png)
...
[1부](clustering-model-prepare-data-tutorial.md#separate-customers)에 정의된 변수를 사용하여 4개의 클러스터 평균이 제공되었습니다.
...
> [자습서: Azure SQL Database Machine Learning Services(미리 보기)를 사용하여 R에서 클러스터링 모델 배포](clustering-model-deploy-tutorial.md)
