...
Azure SQL Database의 [secure enclaves Always Encrypted](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-enclaves) 는 intel [SGX (Software Guard Extensions)](https://itpeernetwork.intel.com/microsoft-azure-confidential-computing/) enclaves를 사용 하 고 [Microsoft Azure 증명이](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-enclaves#secure-enclave-attestation)필요 합니다.
...
Azure SQL Database에서 Always Encrypted에 대 한 Intel SGX enclaves 및 증명을 지원 하도록 환경을 구성 하는 작업에는 Microsoft Azure 증명, Azure SQL Database, 그리고 enclave 증명을 트리거하는 응용 프로그램 등 다양 한 유형의 구성 요소를 설정 해야 합니다. 각 유형의 구성 요소 구성은 다음과 같은 고유한 역할 중 하나를 가정 하는 사용자에 의해 수행 됩니다.
...
프로덕션 환경 (실제 중요 한 데이터 처리)에서는 증명을 구성할 때 조직에서 역할 분리를 준수 하는 것이 중요 합니다. 여기서 각 개별 역할은 서로 다른 사용자가 가정 합니다. 특히 조직에서 Always Encrypted를 배포 하는 것이 목표는 Azure SQL Database 관리자가 중요 한 데이터에 액세스할 수 없도록 하 여 공격 노출 영역을 줄이는 것입니다 Azure SQL Database 관리자는 증명 정책을 제어 하지 않아야 합니다.
