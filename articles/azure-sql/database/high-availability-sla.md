...
범용 서비스 계층에 대 한 영역 중복 구성은 [Azure 가용성 영역](../../availability-zones/az-overview.md)   를 활용 하 여 Azure 지역 내의 여러 물리적 위치에서 데이터베이스를 복제 합니다.영역 중복성을 선택 하 여 응용 프로그램 논리를 변경 하지 않고 새로운 기존 범용 단일 데이터베이스 및 탄력적 풀을 심각한 데이터 센터 중단을 포함 하 여 훨씬 더 큰 오류 집합으로 복원 력을 만들 수 있습니다.
