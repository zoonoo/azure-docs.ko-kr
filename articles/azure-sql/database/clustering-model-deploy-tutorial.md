...
세 부분으로 이루어진 이 자습서 시리즈의 3부에서는 Azure SQL Database Machine Learning Services(미리 보기)를 사용하여 R에서 개발한 클러스터링 모델을 Azure SQL Database의 데이터베이스에 배포합니다.
...
[1부](clustering-model-prepare-data-tutorial.md)에서는 클러스터링을 수행하기 위해 데이터베이스의 데이터를 준비하는 방법을 배웠습니다.

[2부](clustering-model-build-tutorial.md)에서는 R에서 K-평균 클러스터링 모델을 만들고 학습하는 방법을 알아보았습니다.

## <a name="prerequisites"></a>필수 구성 요소

* 이 자습서 시리즈의 3부에서는 [**1부**](clustering-model-prepare-data-tutorial.md) 및 [**2부**](clustering-model-build-tutorial.md)를 완료했다고 가정합니다.
...
환불 동작이 적극적으로 이루어지는 그룹인 클러스터 3에 속한 고객에게 홍보 이메일을 보내려 한다고 가정해 봅시다(네 가지 클러스터에 대한 설명은 [2부](clustering-model-build-tutorial.md#analyze-the-results)에서 확인 가능). 다음 코드는 클러스터 3에 속한 고객의 이메일 주소를 선택합니다.
...
이 자습서를 완료했으면 서버에서 tpcxbb_1gb 데이터베이스를 삭제할 수 있습니다.
...
* [자습서: Azure SQL Database Machine Learning Services(미리 보기)를 사용하여 R에서 예측 모델을 학습하기 위한 데이터 준비](predictive-model-prepare-data-tutorial.md)
...
* [R 패키지를 Azure SQL Database Machine Learning Services(미리 보기)에 추가](machine-learning-services-add-r-packages.md)
