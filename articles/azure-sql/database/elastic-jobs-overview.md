...
description: Elastic Database 작업 (미리 보기)을 구성 하 여에서 하나 이상의 데이터베이스 집합에 대해 T-sql (Transact-sql) 스크립트를 실행 Azure SQL Database
...
1. [PowerShell](elastic-jobs-powershell-create.md) 또는 [t-sql](elastic-jobs-tsql-create-manage.md#create-a-credential-for-job-execution)을 사용 하 여 *작업 데이터베이스* 에서 작업 실행을 위한 자격 증명을 만듭니다.
2. [PowerShell](elastic-jobs-powershell-create.md) 또는 [t-sql](elastic-jobs-tsql-create-manage.md#create-a-target-group-servers)을 사용 하 여 대상 그룹 (작업을 실행 하려는 데이터베이스)을 정의 합니다.
...
4. [PowerShell](elastic-jobs-powershell-create.md) 또는 [t-sql](elastic-jobs-tsql-create-manage.md#deploy-new-schema-to-many-databases)을 사용 하 여 작업을 만듭니다.
...
6. [PowerShell](elastic-jobs-powershell-create.md#run-the-job) 또는 [t-sql](elastic-jobs-tsql-create-manage.md#begin-unplanned-execution-of-a-job)을 사용 하 여 작업을 실행 합니다.
7. Portal, [PowerShell](elastic-jobs-powershell-create.md#monitor-status-of-job-executions) 또는 [t-sql](elastic-jobs-tsql-create-manage.md#monitor-job-execution-status)을 사용 하 여 작업 실행 상태를 모니터링 합니다.
...
`sp_add_jobstep` `@max_parallelism` T-sql 또는 PowerShell에서 저장 프로시저의 매개 변수를 설정 하 여 작업이 실행 되는 동시 데이터베이스의 수를 설정 합니다 `Add-AzSqlElasticJobStep -MaxParallelism` .
