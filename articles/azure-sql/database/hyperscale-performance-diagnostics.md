...
로컬 RBPEX 캐시가 로컬 SSD 저장소의 계산 복제본에 있습니다. 따라서이 캐시에 대 한 IO는 원격 페이지 서버에 대 한 IO 보다 빠릅니다. 현재 하이퍼 규모 데이터베이스의 [sys.dm_io_virtual_file_stats ()](/sql/relational-databases/system-dynamic-management-views/sys-dm-io-virtual-file-stats-transact-sql/) 는 계산 복제본의 로컬 RBPEX 캐시에 대해 io를 보고 하는 특수 행을 포함 합니다. 이 행은 및 열 모두에 대해 0 값을 갖습니다 `database_id` `file_id` . 예를 들어 다음 쿼리는 데이터베이스 시작 이후 RBPEX 사용 통계를 반환 합니다.
...
- 기본 계산에서는 sys.dm_io_virtual_file_stats의 file_id 2에서 로그 쓰기가 고려 됩니다. 기본 계산에 대 한 로그 쓰기는 로그 방문 영역에 대 한 쓰기입니다.
