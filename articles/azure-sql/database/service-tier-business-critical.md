...
-   **데이터를 수정 하는 장기 실행 트랜잭션입니다** . 오랜 시간 동안 열리는 트랜잭션은 로그 파일 잘림을 방지 하 여 로그 크기와 [가상 로그 파일 (VLF)](/sql/relational-databases/sql-server-transaction-log-architecture-and-management-guide#physical_arch)수를 늘릴 수 있습니다. Vlf의 수가 높으면 장애 조치 (failover) 후 데이터베이스의 복구 속도가 느려질 수 있습니다.
