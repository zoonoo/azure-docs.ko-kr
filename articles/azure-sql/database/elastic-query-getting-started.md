...
[탄력적 쿼리](elastic-query-overview.md)를 사용 하 여 단일 연결 지점에서 여러 데이터베이스의 보고서를 만들 수 있습니다. 데이터베이스를 가로로 분할해야 합니다("분할됨"이라고도 함).
...
2. 분할 된 데이터베이스 설정과 동일한 서버에 Azure SQL Database에서 새 데이터베이스를 만듭니다. 데이터베이스 이름을"ElasticDBQuery."로 합니다.
...
2. ElasticDBQuery 데이터베이스에 연결한 다음 T-SQL 명령을 실행 합니다.
    ...
    "username" 및 "password"는 **Elastic Database 도구 시작** 문서에서 [샘플 앱 다운로드 및 실행](elastic-scale-get-started.md#download-and-run-the-sample-app) 문서에서 사용 되는 로그인 정보와 동일 해야 합니다.
