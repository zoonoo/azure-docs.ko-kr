...
[서버 트러스트 그룹](https://aka.ms/mi-server-trust-group-arm) 은 Azure Portal에서 **SQL 트러스트 그룹** 으로 레이블이 지정 된 Azure Resource Manager 개체입니다.
 ...
 * 분산 트랜잭션의 추가 제한 사항은 시나리오와 관련이 있을 수 있습니다. 가장 주목할 만한 것은 VNET 또는 VNET 피어 링을 통해 개인 끝점에 대 한 관리 되는 인스턴스 간에 연결 해야 한다는 것입니다. [Managed Instance에 대 한 현재 분산 트랜잭션 제한 사항을](https://docs.microsoft.com/azure/azure-sql/database/elastic-transactions-overview#limitations)알고 있는지 확인 합니다.
