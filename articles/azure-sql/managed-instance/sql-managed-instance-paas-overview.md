...
Azure SQL Managed Instance은 새로운 관리 되는 인스턴스를 자동으로 배포 하 고, 인스턴스 속성을 업데이트 하 고, 더 이상 필요 하지 않은 경우 인스턴스를 삭제 하는 데 사용할 수 있는 관리 작업을 이 섹션에서는 관리 작업 및 일반적인 기간에 대 한 정보를 제공 합니다.

[Azure 가상 네트워크 내에서 배포](../../virtual-network/virtual-network-for-azure-services.md) 를 지원 하 고 고객에 대 한 격리 및 보안을 제공 하기 위해 SQL Managed Instance는 고객의 가상 네트워크 서브넷 내에 배포 된 격리 된 가상 머신의 전용 집합을 나타내는 [가상 클러스터](connectivity-architecture-overview.md#high-level-connectivity-architecture)를 사용 합니다. 기본적으로 빈 서브넷에 있는 모든 관리 되는 인스턴스 배포는 새 가상 클러스터가 buildout을 생성 합니다.
...
- **가상 클러스터 만들기**: 인스턴스 관리 작업의 동기 단계입니다. **작업의 90%가 4 시간 내에 완료**되었습니다.
...
- **Azure Storage에서 데이터베이스 파일 연결**: 계산 (vcore) 또는 범용 서비스 계층의 저장소 확장/축소와 같은 동기 단계입니다. **이러한 작업의 90%는 5 분 안에 완료**됩니다.
- **Always On 가용성 그룹 시드**: 계산 (vcore), 중요 비즈니스용 서비스 계층의 저장소 크기 조정 뿐만 아니라 서비스 계층을 일반적인 용도에서 중요 비즈니스용 (또는 그 반대로)로 변경 하는 등의 동기 단계입니다. 이 작업의 기간은 총 데이터베이스 크기 및 현재 데이터베이스 작업 (활성 트랜잭션 수)에 비례 합니다. 데이터베이스 작업 인스턴스를 업데이트할 때 전체 기간에 상당한 차이를 일으킬 수 있습니다. **이러한 작업의 90%는 220 g b/시간 이상에서 실행**됩니다.
...
|배포 |비어 있거나 비어 있지 않은 서브넷에 있는 4 개 vCores의 첫 번째 인스턴스 생성|가상 클러스터 만들기 * *|작업의 90%가 4 시간 내에 완료 되었습니다.|
...
|업데이트 |인스턴스를 4 개까지 확장 (범용)|-가상 클러스터 크기 조정 (처음으로 수행 하는 경우 가상 클러스터를 만드는 데 필요 합니다. * *)<br>-데이터베이스 파일 연결|90%의 작업은 4 h 5 분 내에 완료 됩니다. * *|
|업데이트 |인스턴스를 4 개 (중요 비즈니스용)까지 확장|-가상 클러스터 크기 조정 (처음으로 수행 하는 경우 가상 클러스터를 만드는 데 필요 합니다. * *)<br>-Always On 가용성 그룹 시드|90%의 작업이 4 시간 이상 완료 되 고 모든 데이터베이스의 초기값 (220 g b/시간)입니다.|
...
\*\*4-vCores 옵션은 6 월 2019에 출시 되었으며 새 가상 클러스터 버전이 필요 합니다. 6 월 12 일 이전에 생성 된 대상 서브넷에 인스턴스가 있는 경우 새 가상 클러스터가 4 개 vCore 인스턴스를 호스트 하기 위해 자동으로 배포 됩니다.
...
관리 되는 인스턴스의 관리 작업은 동일한 가상 클러스터 내에 배치 된 인스턴스의 다른 관리 작업에 영향을 줄 수 있습니다. 여기에는 다음과 같은 사항이 포함됩니다.
...
[![작업 취소](./media/sql-managed-instance-paas-overview/canceling-operation.png)](./media/sql-managed-instance-paas-overview/canceling-operation.png#lightbox)

취소 요청을 제출 하 고 처리 한 후에는 제출 취소에 성공 하거나 실패 한 경우 알림을 받게 됩니다.

취소 성공의 경우 관리 작업은 몇 분 안에 취소 되 고 오류가 발생 합니다.

![작업 결과 취소](./media/sql-managed-instance-paas-overview/canceling-operation-result.png)

취소 요청이 실패 하거나 취소 단추가 활성화 되지 않은 경우 관리 작업에서 취소할 수 없음 상태가 되 고 몇 분 안에 완료 됩니다. 관리 작업은 완료 될 때까지 실행을 계속 합니다.

> [!IMPORTANT]
> 작업 취소는 현재 포털 에서만 지원 됩니다.
