...
description: 이 문서에서는 SQL Server on Linux 가상 컴퓨터 실행에 대 한 질문과 대답을 제공 합니다.
...
이 문서에서는 [SQL Server on Linux 가상 컴퓨터](sql-server-on-linux-vm-what-is-iaas-overview.md)실행에 대해 가장 일반적인 질문 중 일부에 대 한 답변을 제공 합니다.
...
1. **사용 가능한 SQL Server 가상 머신 갤러리 이미지는 무엇인가요?**
   ...
   Azure는 Linux 및 Windows의 모든 버전에서 지원 되는 모든 버전의 SQL Server에 대 한 VM (가상 머신) 이미지를 유지 관리 합니다. 자세한 내용은 [Linux VM 이미지](sql-server-on-linux-vm-what-is-iaas-overview.md#create) 및 [Windows VM 이미지](../windows/sql-server-on-azure-vm-iaas-what-is-overview.md#payasyougo)의 전체 목록은 참조하세요.

1. **기존 SQL Server 가상 머신 갤러리 이미지가 업데이트되나요?**

   2개월마다 가상 머신 갤러리의 SQL Server 이미지는 최신 Linux 및 Windows 업데이트로 업데이트됩니다. Linux 이미지의 경우 최신 시스템 업데이트가 포함됩니다. Windows 이미지의 경우 중요 한 SQL Server 보안 업데이트 및 서비스 팩을 포함 하 여 Windows 업데이트에서 중요 한 것으로 표시 된 모든 업데이트가 포함 됩니다. SQL Server 누적 업데이트는 Linux 및 Windows에서 다르게 처리됩니다. Linux의 경우 SQL Server 누적 업데이트는 새로 고침에도 포함됩니다. 하지만 이번에 Windows VM은 SQL Server 또는 Windows Server 누적 업데이트로 업데이트되지 않습니다.

1. **함께 설치되는 관련 SQL Server 패키지는 무엇인가요?**

   SQL Server on Linux Vm에서 기본적으로 설치 되는 SQL Server 패키지를 보려면 [설치 된 패키지](sql-server-on-linux-vm-what-is-iaas-overview.md#packages)를 참조 하세요.
...
1. **SQL Server를 사용 하 여 Linux 가상 머신을 만들 어떻게 할까요? 있나요?**

   가장 쉬운 솔루션은 SQL Server를 포함 하는 Linux 가상 머신을 만드는 것입니다. Azure에 등록 하 고 포털에서 SQL Server VM를 만드는 방법에 대 한 자습서는 [Azure Portal에서 SQL Server를 실행 하는 Linux 가상 컴퓨터 프로 비전](sql-vm-create-portal-quickstart.md)을 참조 하세요. 무료 라이선스 버전(Developer 또는 Express)을 사용하거나 온-프레미스 라이선스를 재사용하여 VM에 SQL Server를 수동으로 설치할 수도 있습니다. 사용자 라이선스가 필요하면 [Azure에서 Software Assurance를 통한 라이선스 이동](https://azure.microsoft.com/pricing/license-mobility)이 가능해야 합니다.
...
1. **SQL Server VM에서 SQL Server를 완전히 제거할 수 있나요?**
   ...
   예, 그러나 SQL Server Azure VM에 대한 [가격 책정 지침](../windows/pricing-guidance.md?toc=%2fazure%2fvirtual-machines%2flinux%2fsql%2ftoc.json)에 설명된 대로 SQL Server VM에 대한 비용 청구가 계속됩니다. SQL Server가 더 이상 필요하지 않는 경우 새 가상 컴퓨터를 배포하고 데이터와 애플리케이션을 새 가상 컴퓨터에 마이그레이션할 수 있습니다. 그런 다음 SQL Server 가상 머신을 제거할 수 있습니다.
...
1. **Azure Vm에서 지원 되는 고가용성 솔루션 SQL Server 있나요?**

   지금은 없습니다. Always On 가용성 그룹 및 장애 조치 (Failover) 클러스터링에는 모두 Pacemaker와 같은 Linux의 클러스터링 솔루션이 필요 합니다. SQL Server에 대해 지원 되는 Linux 배포판은 클라우드에서 고가용성 추가 기능을 지원 하지 않습니다.
