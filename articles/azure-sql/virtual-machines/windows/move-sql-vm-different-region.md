...
1. [테스트](#test-move-process): SQL Server VM을 마이그레이션하려면 원본 지역에서 복제된 대상 지역으로 VM을 장애 조치(failover)해야 합니다. 이동 프로세스가 성공 하도록 하려면 먼저 SQL Server VM 대상 지역으로 장애 조치 (failover) 할 수 있는지 테스트 해야 합니다. 이 테스트를 통해 문제가 노출되므로 실제 이동을 수행할 때 이를 방지할 수 있습니다. 
...
- Windows VM의 경우 VM에 최신 Windows 업데이트를 모두 설치하여 신뢰할 수 있는 모든 루트 인증서가 컴퓨터에 있도록 합니다. 연결 되지 않은 환경에서는 조직의 표준 Windows 업데이트 및 인증서 업데이트 프로세스를 따릅니다. 
