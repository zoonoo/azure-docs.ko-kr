---
title: 프리미엄 파일 공유를 사용하는 SQL Server FCI - Azure 가상 머신
description: 이 문서에서는 Azure 가상 머신에서 프리미엄 파일 공유를 사용하여 SQL Server 장애 조치(failover) 클러스터 인스턴스를 만드는 방법을 설명합니다.
...
ms.locfileid: "84032424"
---
# <a name="configure-a-sql-server-failover-cluster-instance-with-premium-file-share-on-azure-virtual-machines"></a>Azure 가상 머신에서 프리미엄 파일 공유를 사용하여 SQL Server 장애 조치(failover) 클러스터 인스턴스 구성
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

이 문서에서는[프리미엄 파일 공유](../../../storage/files/storage-how-to-create-premium-fileshare.md)를 사용하여 Azure 가상 머신에서 SQL Server FCI(장애 조치(failover) 클러스터 인스턴스)를 만드는 방법을 설명합니다.

프리미엄 파일 공유는 Windows Server 2012 이상의 SQL Server 2012 이상에서 장애 조치(failover) 클러스터 인스턴스와 함께 사용할 수 있도록 완벽하게 지원되며, 대기 시간이 짧은 SSD 기반 파일 공유입니다. 프리미엄 파일 공유는 뛰어난 유연성을 제공하여 가동 중지 시간 없이 파일 공유 크기를 조정하고 크기를 조정할 수 있습니다.
...
한 가지 알아야 할 점은 Azure IaaS VM 장애 조치(failover) 클러스터에서는 서버(클러스터 노드)당 단일 NIC 및 단일 서브넷이 권장된다는 것입니다. Azure 네트워킹에는 Azure IaaS VM 게스트 클러스터에서 추가 NIC 및 서브넷을 불필요하게 만드는 물리적 중복성이 있습니다. 클러스터 유효성 검사 보고서는 노드가 단일 네트워크에서만 연결할 수 있다는 경고를 표시합니다. Azure IaaS VM 장애 조치(failover) 클러스터에서는 이 경고를 무시할 수 있습니다.
...
프리미엄 파일 공유는 많은 워크로드의 요구 사항을 충족하는 IOPS 및 처리량 용량을 제공합니다. IO를 많이 사용하는 워크로드의 경우 관리되는 프리미엄 디스크 또는 Ultra Disk에 따라 [스토리지 공간 다이렉트를 사용하는 SQL Server 장애 조치(failover) 클러스터 인스턴스](failover-cluster-instance-storage-spaces-direct-manually-configure.md)를 고려합니다.  

프리미엄 파일 공유에서 배포 또는 마이그레이션을 시작하기 전에 사용자 환경의 IOPS 활동을 확인하고 필요한 IOPS를 제공하는지 확인합니다. Windows 성능 모니터 디스크 카운터를 사용하여 SQL Server Data, Log 및 Temp DB 파일에 필요한 총 IOPS(초당 디스크 전송 수) 및 처리량(초당 디스크 바이트 수)를 모니터링합니다.

대부분의 워크로드에는 IO 버스트가 있으므로 사용량이 많은 기간 중에 확인하고 최대 IOPS 및 평균 IOPS를 모두 기록해 두는 것이 좋습니다. 프리미엄 파일 공유는 공유 크기에 따라 IOPS를 제공합니다. 프리미엄 파일 공유는 또한 최대 1시간 동안 IO를 기준 용량의 3배까지 버스트하는 데 사용할 수 있는 무료 버스트를 제공합니다.

프리미엄 파일 공유 성능에 대한 자세한 내용은 [파일 공유 성능 계층](https://docs.microsoft.com/azure/storage/files/storage-files-planning#storage-tiers)을 참조하세요.
...
Filestream은 프리미엄 파일 공유를 사용하는 장애 조치(failover) 클러스터에 대해 지원되지 않습니다. Filestream을 사용하려면 [스토리지 공간 다이렉트](failover-cluster-instance-storage-spaces-direct-manually-configure.md)를 사용하여 클러스터를 배포하세요.
...
- Azure 가상 머신과 Active Directory 모두에서 개체를 만들 수 있는 권한이 있는 도메인 사용자 계정
- SQL Server 서비스를 실행하고, 파일 공유 탑재 시 가상 머신에 로그인하는 데 사용할 수 있는 도메인 사용자 계정  
...
- 클라우드 감시는 Windows 2016 이상에서 지원되므로 Windows Server 2012 R2 이하 버전을 사용하는 경우 파일 공유 감시로 사용할 다른 파일 공유가 필요합니다. 다른 Azure 파일 공유를 사용하거나 별도의 가상 머신에서 파일 공유를 사용할 수 있습니다. 다른 Azure 파일 공유를 사용하려는 경우 클러스터형 드라이브에 사용되는 프리미엄 파일 공유와 동일한 프로세스를 사용하여 탑재할 수 있습니다. 
...
1. 가용성 집합에 가상 머신을 만듭니다.
   ...
   >[!IMPORTANT]
   > 가상 머신을 만든 후에 사전 설치된 독립 실행형 SQL Server 인스턴스를 제거합니다. 장애 조치(failover) 클러스터 및 프리미엄 파일 공유를 스토리지로 설정한 다음에는 미리 설치된 SQL Server 미디어를 사용하여 SQL Server FCI를 만듭니다.

   또는 운영 체제만 포함된 Azure Marketplace 이미지를 사용할 수 있습니다. 장애 조치(failover) 클러스터 및 프리미엄 파일 공유를 스토리지로 설정한 후 **Windows Server 2016 Datacenter** 이미지를 선택하고 SQL Server FCI를 설치합니다. 이 이미지에는 SQL Server 설치 미디어가 포함되어 있지 않습니다. SQL Server 설치 미디어를 각 서버에 대해 실행할 수 있는 위치에 배치합니다.
...
1. SQL Server 기반 가상 머신 이미지 중 하나를 사용하고 있는 경우 SQL Server 인스턴스를 제거합니다.
   ...
   1. **데이터베이스 엔진 서비스**에서 모든 기능을 제거합니다. **공유 기능**을 제거하지 마세요. 다음 스크린샷과 같은 출력이 표시됩니다.
        ...
        ![기능 선택](./media/failover-cluster-instance-premium-file-share-manually-configure/03-remove-features.png)
...
1. <span id="ports"> </span> 방화벽 포트를 엽니다.  
   ...
   | 파일 공유 | 445 | 파일 공유 서비스에 사용되는 포트입니다.
...
## <a name="step-2-mount-the-premium-file-share"></a>2단계: 프리미엄 파일 공유 탑재

1. [Azure Portal](https://portal.azure.com)에 로그인하고 스토리지 계정으로 이동합니다.
1. **파일 공유**로 이동하여 **파일 서비스**에서 SQL 스토리지에 사용할 프리미엄 파일 공유를 선택합니다.
1. **연결**을 선택하여 파일 공유에 대한 연결 문자열을 가져옵니다.
1. 드롭다운 목록에서 사용하려는 드라이브 문자를 선택하고 두 코드 블록을 모두 메모장에 복사합니다.


   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/premium-file-storage-commands.png" alt-text="파일 공유 연결 포털에서 두 PowerShell 명령 복사":::

1. RDP를 사용하여 SQL Server FCI가 서비스 계정에 사용할 계정으로 SQL Server VM에 연결합니다.
1. 관리 PowerShell 명령 콘솔을 엽니다.
1. 앞서 포털에서 작업하면서 저장한 명령을 실행합니다.
1. 파일 탐색기 또는 **실행** 대화 상자(Windows 로고 키 + r)를 사용하여 공유로 이동합니다. 네트워크 경로 `\\storageaccountname.file.core.windows.net\filesharename`을 사용합니다. 예를 들어 `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`

1. 새로 연결된 파일 공유에서 SQL 데이터 파일을 저장할 폴더를 하나 이상 만듭니다.
1. 클러스터에 참여하는 각 SQL Server VM에 대해 이러한 단계를 반복합니다.

  > [!IMPORTANT]
  > - 백업 파일에 별도의 파일 공유를 사용하여 이 데이터 및 로그 파일용 공유에서 IOPS 및 공간 용량을 절약하는 것이 좋습니다. 백업 파일에 프리미엄 또는 표준 파일 공유 중 하나를 사용할 수 있습니다.
  > - Windows 2012 R2 이전 버전을 사용하는 경우 다음 단계를 수행하여 파일 공유 감시로 사용할 파일 공유를 탑재합니다. 

## <a name="step-3-configure-the-failover-cluster"></a>3단계: 장애 조치(failover) 클러스터 구성

다음 단계는 장애 조치(failover) 클러스터를 구성하는 것입니다. 이 단계에서는 다음 하위 단계를 수행합니다.

1. Windows Server 장애 조치(failover) 클러스터링 기능을 추가합니다.
1. 클러스터 유효성을 검사합니다.
1. 장애 조치(failover) 클러스터를 만듭니다.
1. 클라우드 감시(Windows Server 2016 이상의 경우) 또는 파일 공유 감시(Windows Server 2012 R2 이하의 경우)를 만듭니다.
...
1. **테스트 선택**에서 다음과 같이 **스토리지** 및 **스토리지 공간 다이렉트**를 선택합니다.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/cluster-validation.png" alt-text="클러스터 유효성 검사 테스트 선택":::
...
Windows Server 2016 이상에서는 클라우드 감시를 만들어야 합니다. 클라우드 감시는 Azure Storage Blob에 저장된 새로운 유형의 클러스터 쿼럼 감시입니다. 이를 사용하면 감시 공유를 호스트하는 별도의 VM 또는 별도의 파일 공유를 사용하지 않아도 됩니다.
...
Windows Server 2016 이상에서는 방금 만든 클라우드 감시를 사용하도록 클러스터를 구성합니다. [사용자 인터페이스에서 쿼럼 감시를 구성](https://technet.microsoft.com/windows-server-docs/failover-clustering/deploy-cloud-witness#to-configure-cloud-witness-as-a-quorum-witness)하는 모든 단계를 수행합니다.

Windows Server 2012 R2 및 이전 버전의 경우 [사용자 인터페이스에서 쿼럼 감시를 구성](https://technet.microsoft.com/windows-server-docs/failover-clustering/deploy-cloud-witness#to-configure-cloud-witness-as-a-quorum-witness)하는 동일한 단계를 수행하되, **쿼럼 감시 선택** 페이지에서 **파일 공유 감시 구성** 옵션을 선택합니다. 별도의 가상 머신에서 구성하거나 Azure로부터 탑재하여 할당한 파일 공유를 파일 공유 감시로 지정합니다. 


## <a name="step-4-test-cluster-failover"></a>4단계: 클러스터 장애 조치(failover) 테스트

클러스터의 장애 조치(failover)를 테스트합니다. **장애 조치(Failover) 클러스터 관리자**에서 클러스터를 마우스 오른쪽 단추로 클릭하고 **추가 작업** > **코어 클러스터 리소스 이동** > **노드 선택**을 선택한 다음 클러스터의 다른 노드를 선택합니다. 코어 클러스터 리소스를 클러스터의 모든 노드로 이동한 다음 다시 기본 노드로 이동합니다. 클러스터를 각 노드로 성공적으로 이동할 수 있는 경우 SQL Server를 설치할 준비가 된 것입니다.  

:::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/test-cluster-failover.png" alt-text="코어 리소스를 다른 노드로 이동하여 클러스터 장애 조치(failover) 테스트":::

## <a name="step-5-create-the-sql-server-fci"></a>5단계: SQL Server FCI 만들기
...
1. **SQL Server 장애 조치(failover) 클러스터 새로 설치**를 선택합니다. 마법사의 지침에 따라 SQL Server FCI를 설치합니다.
   ...
   FCI 데이터 디렉터리는 프리미엄 파일 공유에 있어야 합니다. 공유의 전체 경로를 `\\storageaccountname.file.core.windows.net\filesharename\foldername` 형식으로 입력합니다. 파일 서버를 데이터 디렉터리로 지정했음을 알려주는 경고가 표시됩니다. 이 경고는 예상된 것입니다. 파일 공유를 보관할 때 VM에 RDP하는 데 사용한 사용자 계정이 SQL Server 서비스가 가능한 오류를 방지하는 데 사용하는 계정과 동일한 계정인지 확인합니다.

   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/use-file-share-as-data-directories.png" alt-text="파일 공유를 SQL 데이터 디렉터리로 사용":::
...
## <a name="step-6-create-the-azure-load-balancer"></a>6단계: Azure 부하 분산 장치 만들기
...
1. **상태 프로브 추가** 블레이드에서 <span id="probe"> </span> 상태 프로브 매개 변수를 설정합니다.

   - **Name**: 상태 프로브의 이름입니다.
   - **프로토콜**: TCP입니다.
   - **포트**: [이 단계](#ports)에서 상태 프로브용으로 방화벽에서 만든 포트입니다. 이 문서에서는 TCP 포트 `59999`를 사용합니다.
   - **간격**: 5초입니다.
   - **비정상 임계값**: 2번 연속 실패입니다.
