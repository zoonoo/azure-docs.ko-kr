---
title: 장애 조치 (failover) 클러스터 인스턴스에 대 한 DNN 구성
...
Azure Virtual Machines에서 분산 네트워크 이름 (DNN)은 트래픽을 적절 한 클러스터 된 리소스로 라우팅합니다. Azure Load Balancer 필요 없이 VNN (가상 네트워크 이름) 보다 SQL Server FCI (장애 조치 (failover) 클러스터 인스턴스)에 연결 하는 더 쉬운 방법을 제공 합니다. 

이 문서에서는 고가용성 및 HADR (재해 복구)을 위해 Azure Vm의 SQL Server를 사용 하 여 장애 조치 (failover) 클러스터 인스턴스로 트래픽을 라우팅하는 DNN 리소스를 구성 하는 방법을 설명 합니다. 

DNN 기능은 현재 Windows Server 2016 이상에서 SQL Server 2019 CU2 이상 에서만 사용할 수 있습니다. 

대체 연결 옵션의 경우 [가상 네트워크 이름을 고려 하 고 대신 Azure Load Balancer](failover-cluster-instance-vnn-azure-load-balancer-configure.md) 합니다. 

## <a name="overview"></a>개요

DNN (분산 네트워크 이름)는 [SQL Server vm에서 Always On 장애 조치 (failover) 클러스터 인스턴스와](failover-cluster-instance-overview.md)함께 사용 될 때 vnn (가상 네트워크 이름)을 연결 지점으로 대체 합니다. 이렇게 하면 VNN으로 트래픽을 라우팅하는 Azure Load Balancer, 배포 및 유지 관리를 간소화 하 고 장애 조치 (failover)를 향상 시킬 필요가 없습니다. 

FCI 배포를 사용 하면 VNN이 여전히 존재 하지만 클라이언트는 VNN 이름 대신 DNN DNS 이름에 연결 합니다. 

## <a name="prerequisites"></a>필수 구성 요소 

이 문서의 단계를 완료하기 전에 다음이 준비되어 있어야 합니다.

- CU2 이상, Windows Server 2016 이상에서 SQL Server 2019
...
- 현재 FCI가 있는 DNN는 Windows Server 2016 이상에서 SQL Server 2019 CU2 이상 에서만 지원 됩니다. 
