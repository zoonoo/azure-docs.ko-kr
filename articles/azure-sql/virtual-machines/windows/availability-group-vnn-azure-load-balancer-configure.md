---
title: AG VNN 수신기에 대해 부하 분산 장치 구성
description: 고가용성 및 HADR (재해 복구)을 위해 Azure Vm의 SQL Server를 사용 하 여 가용성 그룹의 VNN (가상 네트워크 이름) 수신기로 트래픽을 라우팅하는 Azure Load Balancer를 구성 하는 방법을 알아봅니다.
...
ms.locfileid: "92168898"
---
# <a name="configure-load-balancer-for-ag-vnn-listener"></a>AG VNN 수신기에 대해 부하 분산 장치 구성
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

Azure Virtual Machines에서 클러스터는 부하 분산 장치를 사용 하 여 한 번에 하나의 클러스터 노드에 있어야 하는 IP 주소를 저장 합니다. 이 솔루션에서 부하 분산 장치는 AG (Always On 가용성 그룹)에 대 한 VNN (가상 네트워크 이름) 수신기에 대 한 IP 주소를 보유 합니다. 

이 문서에서는 Azure Load Balancer 서비스를 사용 하 여 부하 분산 장치를 구성 하는 방법을 설명 합니다. 부하 분산 장치는 고가용성 및 HADR (재해 복구)을 위해 Azure Vm의 SQL Server를 사용 하 여 [AG (가용성 그룹) 수신기](availability-group-overview.md) 로 트래픽을 라우팅합니다. 

SQL Server 2019 CU8 이상에 있는 고객에 대 한 대체 연결 옵션은 간소화 된 구성 및 향상 된 장애 조치 (failover)를 위한 대신 [DNN 수신기](availability-group-vnn-azure-load-balancer-configure.md) 를 고려 하는 것이 좋습니다.  
...
- [가용성 그룹 수신기](availability-group-overview.md)를 구성 했습니다.
...
1. 부하 분산 규칙 매개 변수를 설정 합니다.
   ...
   - **이름**: 부하 분산 규칙의 이름입니다.
   - **프런트 엔드 ip 주소**: SQL Server fcis 또는 AG 수신기의 클러스터 된 네트워크 리소스에 대 한 IP 주소입니다.
...
|`AG listener IP Address Resource Name`|SQL Server FCI의 또는 AG 수신기의 IP 주소에 대 한 리소스 이름입니다. **장애 조치(failover) 클러스터 관리자** > **역할**의 SQL Server FCI 역할 아래에 있는 **서버 이름**에서 IP 주소 리소스를 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다. 올바른 값은 **일반** 탭의 **이름** 아래에 있습니다.|
|`ILBIP`|ILB (내부 부하 분산 장치)의 IP 주소입니다. 이 주소는 Azure Portal에서 ILB의 프런트 엔드 주소로 구성 됩니다. 이는 SQL Server FCI의 IP 주소 이기도 합니다. 이 주소는 `<AG listener IP Address Resource Name>`이 있는 동일한 속성 페이지의 **장애 조치(Failover) 클러스터 관리자**에서 확인할 수 있습니다.|
...
연결을 테스트하려면 동일한 가상 네트워크의 다른 가상 머신에 로그인합니다. **SQL Server Management Studio** 를 열고 가용성 그룹 수신기에 연결 합니다.
