---
title: 장애 조치 (failover) 클러스터 인스턴스 VNN Azure Load Balancer 구성
description: 고가용성 및 재해 복구 (HADR)를 위해 Azure Vm의 SQL Server를 사용 하 여 FCI (장애 조치 (failover) 클러스터 인스턴스)에 대 한 VNN (가상 네트워크 이름)으로 트래픽을 라우팅하는 Azure Load Balancer를 구성 하는 방법을 알아봅니다.
...
이 문서에서는 Azure Load Balancer 서비스를 사용 하 여 부하 분산 장치를 구성 하는 방법을 설명 합니다. 부하 분산 장치는 고가용성 및 HADR (재해 복구)을 위해 Azure Vm의 SQL Server를 사용 하 여 [FCI (장애 조치 (failover) 클러스터 인스턴스](failover-cluster-instance-overview.md) )로 트래픽을 라우팅합니다. 

SQL Server 2019 CU2 이상에 대 한 대체 연결 옵션은 간소화 된 구성과 장애 조치 (failover)를 위한 대신 [분산 네트워크 이름을](failover-cluster-instance-distributed-network-name-dnn-configure.md) 고려 하세요. 
