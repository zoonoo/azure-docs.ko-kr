...
SQL Server 인증을 사용하도록 설정하는 경우 **로그인 이름** 및 **암호** 를 지정합니다. 이 로그인 이름은 SQL Server 인증 로그인 및 **sysadmin** 고정 서버 역할의 멤버로 구성 됩니다. 인증 모드에 대한 자세한 내용은 [인증 모드 선택](/sql/relational-databases/security/choose-an-authentication-mode) 을 참조하세요.
