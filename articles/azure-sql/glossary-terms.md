...
|컴퓨팅 세대|Gen5, M 시리즈, Fsv2 시리즈, DC 시리즈|[하드웨어 생성](database/service-tiers-vcore.md#hardware-generations)
|서버 엔터티| 서버 |[논리 SQL 서버](database/logical-servers.md)|
|리소스 종류|vCore|단일 데이터베이스 탄력적 풀에 대 한 계산 리소스에 제공 된 CPU 코어입니다. |
...
|리소스 종류|vCore|SQL Managed Instance에 대 한 계산 리소스에 제공 된 CPU 코어입니다.|
