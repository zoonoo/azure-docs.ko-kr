...
파일이 폴더에 저장되듯이 스토리지 Blob은 컨테이너에 저장됩니다. Blob을 만든 후 해당 Blob에 하나 이상의 컨테이너를 만듭니다. 예를 들어 “Scrapbook”이라는 Blob에 사진을 저장할 “images”라는 컨테이너를 만들고 오디오 파일을 저장할 “audio”라는 컨테이너를 만들 수 있습니다. 컨테이너를 만든 후 컨테이너에 개별 파일을 업로드할 수 있습니다. [빠른 시작: Blob를 프로그래밍 방식으로 조작 하는 방법](../storage/blobs/storage-quickstart-blobs-dotnet.md) 에 대 한 자세한 내용은 .net을 사용 하 여 blob 업로드, 다운로드 및 나열
...
1. `CloudBlobClient` 개체를 사용하여 스토리지 계정의 기존 컨테이너를 참조할 `CloudBlobContainer` 참조를 가져옵니다.
...
`CloudBlobClient`를 사용하면 `CreateIfNotExistsAsync`를 호출하여 스토리지 계정에 컨테이너를 만들 수도 있습니다.
...
[빠른 시작: Blob 컨테이너의 콘텐츠를 나열 하는 다른](../storage/blobs/storage-quickstart-blobs-dotnet.md#list-the-blobs-in-a-container) 방법으로 .net을 사용 하 여 blob을 업로드, 다운로드 및 나열 합니다.
...
[빠른 시작: Blob을 파일로 저장 하는 다른 방법은 .net](../storage/blobs/storage-quickstart-blobs-dotnet.md#download-blobs) 을 사용 하 여 blob을 업로드, 다운로드 및 나열 합니다.
