...
1.  Appcelerator Titanium Studio에서 [모바일 서비스 시작][모바일 서비스 시작] 자습서를 완료했을 때 만든 프로젝트를 엽니다.

2.  Run 단추를 눌러 프로젝트를 빌드하고 iPhone 에뮬레이터에서 앱을 시작합니다. 앱이 시작된 후 상태 코드 401(인증되지 않음)의 처리되지 않은 예외가 발생하는지 확인합니다.
...
1.  프로젝트 파일 index.js를 열고 테이블 이벤트 Lister 메서드에서 `case 2:`를 찾습니다.
    ...
    앱에서 사용자에게 사용할 수 있는 ID 공급자가 포함된 메시지를 표시하거나 ID 공급자의 하나를 자동으로 제공할 수 있습니다.
...
1.  Run 단추를 눌러 프로젝트를 빌드하고 iPhone 시뮬레이터나 Android 에뮬레이터에서 앱을 시작하고 With Login 옵션을 클릭하여 ID 공급자로 로그온합니다.
