...
식별 작업은 사람(또는 여러 사람)의 이미지를 가져와서 이미지에서 각 얼굴의 ID를 찾습니다(얼굴 인식 검색). 감지된 각 얼굴을 얼굴 특징이 알려진 다른 **Person** 개체의 데이터베이스인 **PersonGroup**과 비교합니다. 식별 작업을 수행하려면 먼저 **PersonGroup**을 만들고 학습해야 합니다.
...
`dotnet run` 명령을 사용하여 애플리케이션 디렉터리에서 얼굴 인식 앱을 실행니다.
