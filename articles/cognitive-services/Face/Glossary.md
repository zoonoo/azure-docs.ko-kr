...
PersistedFace는 Face API의 데이터 구조입니다. PersistedFace 함께 제공 되는 [PersistedFace ID](#persisted-face-id)이름과 사용자 데이터 등의 다른 특성 외에 합니다.
...
#### <a name="persisted-face-id"></a>지속형된 Face ID

Face ID를 유지 때 생성 되는 [PersistedFace](#persistedface) 성공적으로 생성 됩니다. [Face API](#face-api)에서 이 Face를 나타내는 문자열이 만들어집니다.
...
자세한 내용은 [얼굴 마이그레이션 지침](./face-api-how-to-topics/how-to-migrate-face-data.md)뿐만 아니라 [스냅숏 - 촬영](/rest/api/cognitiveservices/face/snapshot/take) 및 [스냅숏 - 적용](/rest/api/cognitiveservices/face/snapshot/apply) 참조 설명서(REST)를 참조하세요.
