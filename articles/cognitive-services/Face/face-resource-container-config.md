...
환경 변수 값은 명령줄 인수 값을 재정의합니다. 그러면 컨테이너 이미지에 대한 기본값이 재정의됩니다. 즉, `Logging:Disk:LogLevel`과 같은 동일한 구성 설정에 대한 환경 변수와 명령줄 인수에 다른 값을 지정한 다음, 컨테이너를 인스턴스화하면 환경 변수의 값이 인스턴스화된 컨테이너에서 사용됩니다.
...
컨테이너가 인스턴스화되면 컨테이너에서 사용자 환경 변수를 읽습니다. 환경 변수가 있으면 환경 변수의 값이 지정된 구성 설정에 대한 기본값을 재정의합니다. 환경 변수를 사용하면 컨테이너를 인스턴스화하기 전에 여러 구성 설정을 지정할 수 있고, 여러 컨테이너에서 동일한 구성 설정 세트를 자동으로 사용할 수 있습니다.
...
다운로드한 컨테이너 이미지에서 컨테이너를 인스턴스화하는 데 사용되는 [docker run](https://docs.docker.com/engine/reference/commandline/run/) 명령의 선택적 `ARGS` 매개 변수에서 구성 설정을 지정할 수 있습니다. 명령줄 인수를 사용하면 각 컨테이너에서 다른 사용자 지정 구성 설정 세트를 사용할 수 있다는 이점이 있습니다.
...
`ApplicationInsights` 섹션의 구성 설정을 사용하면 [Azure Application Insights](https://docs.microsoft.com/azure/application-insights) 원격 분석 지원을 컨테이너에 추가할 수 있습니다. Application Insights는 컨테이너를 코드 수준까지 자세히 모니터링합니다. 컨테이너에 대한 가용성, 성능 및 사용량을 쉽게 모니터링할 수 있습니다. 또한 사용자가 보고할 때까지 기다리지 않고 컨테이너의 오류를 빠르게 식별하고 진단할 수 있습니다.

다음 표에서는 `ApplicationInsights` 섹션에서 지원되는 구성 설정에 대해 설명합니다.
...
## <a name="authentication-configuration-settings"></a>Authentication 구성 설정
...
## <a name="billing-configuration-setting"></a>Billing 구성 설정

`Billing` 구성 설정은 컨테이너에 대한 청구 정보를 추적하는 데 사용되는 Azure의 Face 리소스에 대한 엔드포인트 URI를 지정합니다. 이 구성 설정에 대한 값을 지정해야 하며, 이 값은 Azure의 Face 리소스에 대한 유효한 엔드포인트 URI여야 합니다.
...
`Eula` 구성 설정은 컨테이너에 대한 라이선스에 동의했음을 나타냅니다. 이 구성 설정에 대한 값을 지정해야 하며, 이 값은 `accept`로 설정해야 합니다.
...
다음 표에서는 `Fluentd` 섹션에서 지원되는 구성 설정에 대해 설명합니다.

| 이름 | 데이터 형식 | 설명 |
|------|-----------|-------------|
| `Host` | 문자열 | Fluentd 서버의 IP 주소 또는 DNS 호스트 이름입니다. |
| `Port` | 정수  | Fluentd 서버의 포트입니다.<br/> 기본값은 24224입니다. |
| `HeartbeatMs` | 정수  | 하트비트 간격(밀리초)입니다. 이 간격이 만료되기 전에 이벤트 트래픽이 전송되지 않으면 하트비트가 Fluentd 서버로 전송됩니다. 기본값은 60,000밀리초(1분)입니다. |
| `SendBufferSize` | 정수  | 전송 작업에 할당된 네트워크 버퍼 공간(바이트)입니다. 기본값은 32,768바이트(32킬로바이트)입니다. |
| `TlsConnectionEstablishmentTimeoutMs` | 정수  | Fluentd 서버와의 SSL/TLS 연결 설정의 시간 제한(밀리초)입니다. 기본값은 10,000밀리초(10초)입니다.<br/> `UseTLS`가 false로 설정되면 이 값이 무시됩니다. |
| `UseTLS` | BOOLEAN | 컨테이너에서 Fluentd 서버와 통신하는 데 SSL/TLS를 사용해야 하는지 여부를 나타냅니다. 기본값은 False입니다. |

## <a name="logging-configuration-settings"></a>Logging 구성 설정
...
* 디스크  
  JSON 로깅 공급자입니다. 이 로깅 공급자는 로그 데이터를 출력 탑재에 씁니다.  
  `Disk` 로깅 공급자에서 지원되는 구성 설정은 다음과 같습니다.  

  | 이름 | 데이터 형식 | 설명 |
  |------|-----------|-------------|
  | `Format` | 문자열 | 로그 파일에 대한 출력 형식입니다.<br/> **참고:** 로깅 공급자를 사용하도록 설정하려면 이 값을 `json`으로 설정해야 합니다. 또한 컨테이너를 인스턴스화하는 동안 출력 탑재를 지정하지 않고 이 값을 지정하면 오류가 발생합니다. |
  | `MaxFileSize` | 정수  | 로그 파일의 최대 크기(MB)입니다. 현재 로그 파일의 크기가 이 값과 같거나 초과하면 로깅 공급자에서 새 로그 파일을 시작합니다. -1이 지정되면 로그 파일의 크기가 출력 탑재에 대한 최대 파일 크기(있는 경우)로만 제한됩니다. 기본값은 1입니다. |

ASP.NET Core 로깅 지원을 구성하는 방법에 대한 자세한 내용은 [설정 파일 구성](https://docs.microsoft.com/aspnet/core/fundamentals/logging/?view=aspnetcore-2.1#settings-file-configuration)을 참조하세요.

## <a name="mounts-configuration-settings"></a>Mounts 구성 설정

Face에서 제공하는 Docker 컨테이너는 상태를 저장하지 않으며 변경할 수 없도록 설계되었습니다. 즉, 컨테이너 내부에서 만들어진 파일은 쓰기 가능한 컨테이너 계층에 저장되어 컨테이너가 실행되는 동안만 유지되며 쉽게 액세스할 수 없습니다. 해당 컨테이너가 중지되거나 제거되면 컨테이너 내부에서 만들어진 파일이 삭제됩니다.

그러나 Docker 컨테이너이므로 볼륨 및 바인딩 탑재와 같은 Docker 스토리지 옵션(해당 컨테이너에서 지원하는 경우)을 사용하여 컨테이너 외부에서 영구 데이터를 읽고 쓸 수 있습니다. Docker 스토리지 옵션을 지정하고 관리하는 방법에 대한 자세한 내용은 [Docker에서 데이터 관리](https://docs.docker.com/storage/)를 참조하세요.

> [!NOTE]
> 일반적으로 이러한 구성 설정에 대한 값은 변경할 필요가 없습니다. 대신, 컨테이너에 대한 입력 및 출력 탑재를 지정할 때 이러한 구성 설정에 지정된 값을 대상으로 사용합니다. 입력 및 출력 탑재를 지정하는 방법에 대한 자세한 내용은 [입력 및 출력 탑재](#input-and-output-mounts)를 참조하세요.

다음 표에서는 `Mounts` 섹션에서 지원되는 구성 설정에 대해 설명합니다.
...
다운로드한 컨테이너 이미지에서 컨테이너를 인스턴스화하는 데 사용되는 [docker run](https://docs.docker.com/engine/reference/commandline/run/) 명령에서 `--mount` 옵션을 지정하여 입력 탑재 또는 출력 탑재를 지정할 수 있습니다. 기본적으로 입력 탑재는 `/input`을 대상으로 사용하고, 출력 탑재는 `/output`을 대상으로 사용합니다. Docker 컨테이너 호스트에서 사용할 수 있는 모든 Docker 스토리지 옵션은 `--mount` 옵션에서 지정할 수 있습니다.
