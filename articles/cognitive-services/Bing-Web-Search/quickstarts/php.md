...
계속하기 전에 몇 가지 변수를 설정해야 합니다. `$endpoint`은 아래의 글로벌 엔드포인트이거나 리소스의 Azure Portal에 표시되는 [사용자 지정 하위 도메인](../../../cognitive-services/cognitive-services-custom-subdomains.md) 엔드포인트일 수 있습니다. `$endpoint`가 올바른지 확인하고 `$accesskey` 값을 Azure 계정의 유효한 구독 키로 바꿉니다. `$term` 값을 바꿔 검색 쿼리를 사용자 지정할 수 있습니다.
