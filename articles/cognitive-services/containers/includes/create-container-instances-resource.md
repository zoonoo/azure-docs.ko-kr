...
2. **기본 사항** 탭에서 다음 세부 정보를 입력 합니다.
    ...
    |위치|배포할 지역을 선택 합니다.|
    |이미지 형식|컨테이너 이미지가 자격 증명이 필요 하지 않은 컨테이너 레지스트리에 저장 되어 있는 경우를 선택 `Public` 합니다. 컨테이너 이미지에 액세스 하려면 자격 증명이 필요 `Private` 합니다 .를 선택 합니다. 컨테이너 이미지가 [](../../cognitive-services-container-support.md#container-repositories-and-images) `Public` 또는 `Private` ("공개 미리 보기") 인지 여부에 대 한 자세한 내용은 컨테이너 리포지토리 및 이미지를 참조 하세요. |
