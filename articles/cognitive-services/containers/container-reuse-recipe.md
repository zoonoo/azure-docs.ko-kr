...
* 모델 입력 디렉터리의 상대 부모에서 docker 파일을 실행 합니다. 다음 예에서는의 `docker build` `docker run` `/input`상대 부모에서 및 명령을 실행 합니다. `COPY` 명령의 첫 번째 `/input` 는 호스트 컴퓨터의 디렉터리입니다. 두 번째 `/input` 는 컨테이너의 디렉터리입니다.
...
1. 다음 명령을 사용 하 여 명령줄 또는 터미널에서 이미지에 파일을 빌드합니다. 꺾쇠 괄호 `<>`의 값을 사용자 고유의 컨테이너 이름 및 태그로 바꿉니다.  
    ...
    태그 옵션인 `-t`는 컨테이너에 대해 변경한 내용에 대 한 정보를 추가 하는 방법입니다. 예를 들어의 `modified-LUIS` 컨테이너 이름은 원래 컨테이너가 계층화 되었음을 나타냅니다. 태그 이름은 Language Understanding ( `with-billing-and-model` LUIS) 컨테이너가 수정 된 방법을 나타냅니다.
