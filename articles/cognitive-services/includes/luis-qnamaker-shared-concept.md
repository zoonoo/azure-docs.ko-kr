...
예제 텍스트 `How do I get to the Human Resources building on the Seattle North campus?`를 계속 사용 하면 QnA Maker 낮은 신뢰 점수를 반환 한다고 가정 합니다. LUIS에서 반환 된 의도를 사용 `FindLocation` 하 고, `Human Resources building` 및 `Seattle North campus`와 같은 추출 된 엔터티를 사용 하 여이 정보를 매핑 또는 검색 서비스에 다른 답변으로 보냅니다.
