...
LUIS를 쿼리하기 위한 엔드포인트에 대한 액세스는 **설정** 페이지에서 앱의 **공용** 설정을 통해 제어합니다. 개인 앱의 엔드포인트 쿼리는 나머지 할당량 적중을 사용하여 권한이 있는 키를 확인합니다. 공용 앱의 엔드포인트 쿼리는 나머지 할당량 적중도 확인하는 엔드포인트 키를 제공해야 합니다(누구든지 쿼리를 만드는 사용자의 엔드포인트 키). 

엔드포인트 키는 GET 요청의 쿼리 문자열 또는 POST 요청의 헤더로 전달됩니다.
...
|**[게시](luis-how-to-publish-app.md)** 페이지에서 추가된 엔드포인트 키|소유자 및 협력자가 엔드포인트 키를 추가할 수 있음|

기타 작성 또는 엔드포인트 키에는 액세스 권한이 **없습니다**.

### <a name="public-app-endpoint-access"></a>공용 앱 엔드포인트 액세스
앱의 **설정** 페이지에서 앱을 **공용**으로 구성합니다. 앱이 공용으로 구성되면 키에 전체 엔드포인트 할당량이 사용되지 않는 한 ‘모든’ 유효한 LUIS 작성 키 또는 LUIS 엔드포인트 키가 앱을 쿼리할 수 있습니다.
