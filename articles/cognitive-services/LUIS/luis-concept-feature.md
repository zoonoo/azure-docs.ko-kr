...
구문 목록은 서로 교환 가능하거나 서로 교환 불가능할 수 있습니다. *서로 교환 가능한* 구문 목록은 동의어인 값에 해당하고 *서로 교환 불가능한* 구문 목록은 앱의 특정 어휘 목록에 사용됩니다. 앱 어휘 구 목록을 만들면서 어떤 용어들은 많은 형태(동의어)를 가지고 있음을 알게 될 것입니다. 이러한 용어를 서로 교환 가능한 다른 구 목록으로 나눕니다. 
...
단어 또는 구의 목록이 클래스 또는 그룹을 만들 때 서로 교환 가능한 구 목록을 사용합니다. 예를 들면 "January", "February", "March"와 같은 월 목록이나 "John", "Mary", "Frank"와 같은 이름 목록이 있습니다.  이러한 목록은 구 목록의 다른 단어가 사용될 경우 발언이 같은 의도나 엔터티로 레이블이 지정된다는 측면에서 서로 교환 가능합니다. 예를 들어, "show the calendar for January"가 "show the calendar for February"와 같은 의도를 갖는 경우 단어가 서로 교환 가능한 목록에 포함됩니다. 
