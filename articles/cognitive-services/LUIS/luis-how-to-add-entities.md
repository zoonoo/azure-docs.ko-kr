...
|텍스트 일치 엔터티|목적|
|--|--|
|[목록 엔터티](#add-list-entities-for-exact-matches)|다른 형태의 동의어와 함께 정식 이름 목록|
|정규식 엔터티|정규식 엔터티를 사용 하 여 텍스트 일치|
|[미리 빌드된 엔터티](tutorial-machine-learned-entity.md#add-prebuilt-number-to-help-extract-data)|숫자, 전자 메일, 날짜 등의 공통 데이터 형식 일치|
...
1. **목록 엔터티 만들기** 대화 상자의 새 하위 목록 **추가 ...** 에서 목록 항목 이름 (예: `Green`)을 입력 한 다음 동의어를 추가 합니다.
...
다음 utterance에는 두 위치가 있으며 각 위치는 `to` 및 `from`와 같은 단어를 통해 의미 체계를 지정 합니다. 

`Pick up the package from Seattle and deliver to New York City.`

이 절차에서는 미리 작성 `origin` 된 `destination` geographyV2 엔터티에 및 역할을 추가 합니다.
...
1. **유형 역할 ...** 텍스트 상자에 역할 `Origin` 의 이름을 입력 한 다음 enter 키를 누릅니다. 의 `Destination` 두 번째 역할 이름을 추가 하 고 enter 키를 누릅니다. 
