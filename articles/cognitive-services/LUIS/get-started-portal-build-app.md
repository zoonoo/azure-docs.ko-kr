...
|이름  |설명  |
|---------|---------|
|이름     | 앱의 이름입니다. 예: "홈 자동화". 필수 사항입니다.        |
|culture     | 앱이 이해하고 말하는 언어입니다. 필수 사항입니다.   |
|설명 | 앱에 대한 설명입니다. (선택 사항)
|예측 리소스 | 쿼리를 수신할 예측 리소스입니다. (선택 사항) |
...
1. 앱을 만든 후 **빌드** 섹션의 **의도** 페이지에 있는지 확인합니다. **만들기** 를 선택합니다.
...
의도를 만든 후에 예제 발언을 추가합니다. 예제 발언은 챗봇이나 기타 클라이언트 애플리케이션에 사용자가 입력하는 텍스트입니다. 사용자 텍스트의 의도를 LUIS 의도에 매핑합니다. 이 예제 애플리케이션의 `FindForm` 의도는, 예제 발언에 양식 번호가 포함됩니다. 사용자의 요청을 이행하려면 클라이언트 애플리케이션에 양식 번호가 필요하기 때문에 발언에 양식 번호를 포함하는 것이 중요합니다.

`FindForm`에 대한 **의도** 페이지에서 **예제 발화** 아래에 다음 예제 발화를 추가합니다. 
