...
LUIS 앱 요청 비율이 허용된 [할당량 비율을](https://azure.microsoft.com/pricing/details/cognitive-services/language-understanding-intelligent-services/)초과하는 경우 다음을 수행할 수 있습니다.

* 동일한 앱 정의를 사용하여 더 많은 LUIS [앱에](#use-multiple-apps-with-same-app-definition)로드를 분산시입니다. 여기에는 선택적으로 [컨테이너에서](luis-container-howto.md)LUIS를 실행하는 것이 포함됩니다. 
* 앱에 [여러 키를](#assign-multiple-luis-keys-to-same-app) 만들고 할당합니다. 
...
이러한 형제 앱을 학습할 [때는 모든 데이터로 학습해야](luis-how-to-train.md#train-with-all-data)합니다.
...
활성 학습을 위한 [끝점 발언의](luis-how-to-review-endpoint-utterances.md) 2주마다와 같은 정기적인 검토를 예약한 다음 다시 학습하고 다시 게시합니다. 
...
500개 이상의 의도를 가지고 있는 사무실 도우미를 개발한다고 가정합니다. 200개의 의도는 모임 예약에 관련되고, 200개는 미리 알림에 관련되고, 200개는 동료 정보 가져오기에 관련되고, 200개는 메일 보내기에 관련된 경우, 각 그룹이 단일 앱에 있도록 의도를 그룹화한 다음, 각 의도가 포함된 최상위 앱을 만듭니다. [디스패치 모델을](#dispatch-tool-and-model) 사용하여 최상위 앱을 빌드합니다. 그런 다음 [디스패치 모델의 자습서에](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)표시된 대로 계단식 호출을 사용하도록 봇을 변경합니다. 

## <a name="when-you-need-to-combine-several-luis-and-qna-maker-apps"></a>여러 LUIS 및 QnA Maker 앱을 결합해야 하는 경우
봇에 응답해야 하는 여러 LUIS 및 QnA 메이커 앱이 있는 경우 [디스패치 모델을](#dispatch-tool-and-model) 사용하여 최상위 앱을 빌드합니다.  그런 다음 [디스패치 모델의 자습서에](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)표시된 대로 계단식 호출을 사용하도록 봇을 변경합니다. 
...
채팅 봇은 발언을 받은 다음 예측을 위해 부모 LUIS 앱으로 보냅니다. 상위 앱에서 예측된 상위 의도에 따라 다음에 호출되는 LUIS 자식 앱이 결정됩니다. 채팅 봇은 보다 구체적인 예측을 위해 발언을 자식 앱으로 보냅니다.
...
* [봇 프레임워크 SDK](https://github.com/Microsoft/botframework)
* [디스패치 모델 자습서](https://docs.microsoft.com/azure/bot-service/bot-builder-tutorial-dispatch?view=azure-bot-service-4.0&branch=master&tabs=cs)
