...
|엔터티 형식|description|
|--|--|
|Calendar|달력 개인 회 및 약속 하는 방법에 대 한 부분이 _되지_ public 이벤트 (world cup 일정, 시애틀 이벤트 달력 등 일반 달력 (어떤 day는 현재와 같은 새로운 않습니다 fall 시작 노동 일 경우).|
...
|ToDo_IPA|ToDo 도메인에 따라 ToDo_IPA는 할 일의 엔터티를 확장 하려면 사용자 지정 된 버전입니다. IPA 버전 인식 todo 목록 형식을 지원 하기 위해 의도 및 엔터티를 제공 합니다. 모델은 세 가지 할 일 목록 정의: 식료품 목록, 쇼핑 목록과 다른 경우를 포함 하는 할 일 목록입니다.|
