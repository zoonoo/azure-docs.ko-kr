...
ms.locfileid: "79221318"
---
# <a name="install-and-run-luis-docker-containers"></a>LUIS docker 컨테이너 설치 및 실행
 
언어 이해(LUIS) 컨테이너는 학습된 또는 게시된 언어 이해 모델을 로드합니다. LUIS [앱으로](https://www.luis.ai)docker 컨테이너는 컨테이너의 API 끝점에서 쿼리 예측에 대한 액세스를 제공합니다. 컨테이너에서 쿼리 로그를 수집하고 언어 이해 앱에 다시 업로드하여 앱의 예측 정확도를 향상시킬 수 있습니다.

다음 비디오는 이 컨테이너를 사용하는 방법을 보여줍니다.

[![코그너티브 서비스를 위한 컨테이너 데모](./media/luis-container-how-to/luis-containers-demo-video-still.png)](https://aka.ms/luis-container-demo)

Azure 구독이 아직 없는 경우 시작하기 전에 [체험 계정](https://azure.microsoft.com/free/?WT.mc_id=A261C142F)을 만듭니다.

## <a name="prerequisites"></a>사전 요구 사항

LUIS 컨테이너를 실행하려면 다음 필수 구성 조건을 기록합니다.
...
|Azure `Cognitive Services` 리소스 및 LUIS [패키지앱](luis-how-to-start-new-app.md) 파일 |컨테이너를 사용하려면 다음이 있어야 합니다.<br><br>* _코그너티브 서비스_ Azure 리소스 및 관련 청구 키가 청구 끝점 URI입니다. 두 값 모두 리소스의 개요 및 키 페이지에서 사용할 수 있으며 컨테이너를 시작해야 합니다. <br>* 연결된 앱 ID와 함께 컨테이너에 대한 탑재된 입력으로 패키징된 학습된 또는 게시된 앱 LUIS 포털 또는 작성 API에서 패키지된 파일을 얻을 수 있습니다. [작성 API에서](#authoring-apis-for-package-file)LUIS 패키지 앱을 가져오는 경우 _작성 키도_필요합니다.<br><br>이러한 요구 사항은 아래 변수에 대한 명령줄 인수를 전달하는데 사용됩니다.<br><br>**{AUTHORING_KEY}**: 이 키는 클라우드의 LUIS 서비스에서 패키지된 앱을 다운로드하고 쿼리 로그를 클라우드로 다시 업로드하는 데 사용됩니다. 형식은 `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`입니다.<br><br>**{APP_ID}**: 이 ID는 앱을 선택하는 데 사용됩니다. 형식은 `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`입니다.<br><br>**{API_KEY}**: 이 키는 컨테이너를 시작하는 데 사용됩니다. 두 위치에서 엔드포인트 키를 찾을 수 있습니다. 첫 번째는 _인지 서비스_ 리소스의 키 목록 내의 Azure 포털입니다. 엔드포인트 키는 키와 엔드포인트 설정 페이지에서 LUIS 포털을 사용할 수도 있습니다. 시작 키를 사용하지 마세요.<br><br>**{ENDPOINT_URI}**: 개요 페이지에 제공된 끝점입니다.<br><br>[작성 키 및 엔드포인트 키](luis-boundaries.md#key-limits)는 용도가 서로 다릅니다. 서로 교환하여 사용하지 마세요. |

[!INCLUDE [Gathering required container parameters](../containers/includes/container-gathering-required-parameters.md)]

### <a name="authoring-apis-for-package-file"></a>패키지 파일에 대한 API 작성
...
* [게시되지 않은 학습된 전용 패키지 API](https://westus.dev.cognitive.microsoft.com/docs/services/5890b47c39e2bb17b84a55ff/operations/apps-packagetrainedapplicationasgzip)
...
[`docker pull`](https://docs.docker.com/engine/reference/commandline/pull/) 명령을 사용하여 리포지토리에서 `mcr.microsoft.com/azure-cognitive-services/luis` 컨테이너 이미지를 다운로드합니다.
...
1. [호스트 컴퓨터](#the-host-computer)에서 패키지 파일을 필수 **입력** 디렉터리로 이동합니다. LUIS 패키지 파일의 이름을 바꾸거나 변경하거나 덮어쓰거나 압축을 풀지 마십시오.
...
입력 마운트 디렉터리에는 앱의 **프로덕션,** **스테이징**및 **버전이 적용된** 모델이 동시에 포함될 수 있습니다. 모든 패키지는 탑재되어 있습니다.

|패키지 형식|쿼리 엔드포인트 API|쿼리 가용성|패키지 파일 이름 형식|
|--|--|--|--|
|버전이|받기, 포스트|컨테이너만|`{APP_ID}_v{APP_VERSION}.gz`|
|스테이징|받기, 포스트|Azure 및 컨테이너|`{APP_ID}_STAGING.gz`|
|프로덕션|받기, 포스트|Azure 및 컨테이너|`{APP_ID}_PRODUCTION.gz`|

> [!IMPORTANT]
> LUIS 패키지 파일의 이름을 바꾸거나 변경하거나 덮어쓰거나 압축을 풀지 마십시오.
...
|패키징 요구 사항|세부 정보|
|--|--|
|Azure _코그너티브 서비스_ 리소스 인스턴스|지원되는 지역은 다음을 포함합니다.<br><br>미국 서부(`westus`)<br>서유럽(`westeurope`)<br>오스트레일리아 동부(`australiaeast`)|
...
### <a name="export-versioned-apps-package-from-luis-portal"></a>LUIS 포털에서 버전이 적용된 앱 패키지 내보내기

버전이 적용된 앱의 패키지는 버전 목록 페이지에서 사용할 수 **있습니다.**
...
게시된 패키지를 다운로드하려면 [여기의 API 설명서를][download-published-package]참조하십시오. 성공적으로 다운로드하면 응답은 LUIS 패키지 파일입니다. 컨테이너의 입력 탑재에 대한 지정된 스토리지 위치에 파일을 저장합니다. 

### <a name="export-versioned-apps-package-from-api"></a>API에서 버전이 적용된 앱 패키지 내보내기
...
| 자리 표시자 | 값 |
|-------------|-------|
| **{APP_ID}** | 학습된 LUIS 앱의 응용 프로그램 ID입니다. |
| **{APP_VERSION}** | 학습된 LUIS 앱의 응용 프로그램 버전입니다. |
| **{AUTHORING_KEY}** | 게시된 LUIS 앱에 대한 LUIS 계정의 작성 키입니다.<br/>LUIS 포털의 **사용자 설정** 페이지에서 작성 키를 가져올 수 있습니다. |
| **{AZURE_REGION}** | 해당 Azure 지역:<br/><br/>`westus` - 미국 서부<br/>`westeurope` - 서유럽<br/>`australiaeast` - 오스트레일리아 동부 |

버전이 있는 패키지를 다운로드하려면 [여기의 API 설명서를][download-versioned-package]참조하십시오. 성공적으로 다운로드하면 응답은 LUIS 패키지 파일입니다. 컨테이너의 입력 탑재에 대한 지정된 스토리지 위치에 파일을 저장합니다. 
...
* 이 예제에서는 Windows에서 `C:` 권한 충돌을 피하기 위해 드라이브 에서 디렉터리를 사용합니다. 입력 디렉터리로 특정 디렉터리를 사용해야 할 경우 Docker 서비스 권한을 받아야 할 수도 있습니다. 
* docker 컨테이너에 익숙하지 않은 경우 인수의 순서를 변경하지 마십시오.
...
* 컨테이너 호스트에 있는 *C:\input에서*입력 마운트에서 LUIS 앱을 로드합니다.
* CPU 코어 두 개 및 4GB 메모리를 할당합니다.
* 5000 TCP 포트 표시 및 컨테이너에 의사-TTY 할당
* 컨테이너 및 LUIS 로그를 컨테이너 호스트에 있는 *C:\output에서*출력 마운트에 저장합니다.
...
> ApiKey 값은 LUIS 포털의 **Azure 리소스** 페이지의 `Cognitive Services` **키이며** Azure 리소스 키 페이지에서도 사용할 수 있습니다.  

[!INCLUDE [Running multiple containers on the same host](../../../includes/cognitive-services-containers-run-multiple-same-host.md)]

## <a name="endpoint-apis-supported-by-the-container"></a>컨테이너에서 지원하는 끝점 API
...
컨테이너는 REST 기반 쿼리 예측 엔드포인트 API를 제공합니다. 게시된(스테이징 또는 프로덕션) 앱의 끝점은 버전이 변경된 앱의 끝점과 _다른_ 경로를 가있습니다.
...
|--|--|--|--|
|게시 날짜|받기, 포스트|`/luis/v3.0/apps/{appId}/slots/{slotName}/predict?`|`query={query}`<br>[`&verbose`]<br>[`&log`]<br>[`&show-all-intents`]|
|버전이|받기, 포스트|`/luis/v3.0/apps/{appId}/versions/{versionId}/predict?`|`query={query}`<br>[`&verbose`]<br>[`&log`]<br>[`&show-all-intents`]|
...
|`show-all-intents`|boolean|모든 의도 또는 상위 점수 매기기 의도만 반환할지 여부를 나타내는 부울 값입니다. 기본값은 false입니다.|

# <a name="v2-prediction-endpoint"></a>[V2 예측 엔드포인트](#tab/v2)

|패키지 유형|HTTP 동사|라우팅|쿼리 매개 변수|
|--|--|--|--|
|게시 날짜|[받기](https://westus.dev.cognitive.microsoft.com/docs/services/5819c76f40a6350ce09de1ac/operations/5819c77140a63516d81aee78), [포스트](https://westus.dev.cognitive.microsoft.com/docs/services/5819c76f40a6350ce09de1ac/operations/5819c77140a63516d81aee79)|`/luis/v2.0/apps/{appId}?`|`q={q}`<br>`&staging`<br>[`&timezoneOffset`]<br>[`&verbose`]<br>[`&log`]<br>|
|버전이|받기, 포스트|`/luis/v2.0/apps/{appId}/versions/{versionId}?`|`q={q}`<br>[`&timezoneOffset`]<br>[`&verbose`]<br>[`&log`]|
...
**스테이징** 환경에 대한 쿼리를 `production` 만들려면 경로에서 다음을 다음으로 `staging`바꿉습니다.

`http://localhost:5000/luis/v3.0/apps/{APP_ID}/slots/staging/predict`

버전이 있는 모델을 쿼리하려면 다음 API를 사용합니다.
...
LUIS 컨테이너에 대해 출력 마운트가 지정되면 앱 쿼리 로그 파일이 `{INSTANCE_ID}` 컨테이너 ID인 출력 디렉토리에 저장됩니다. 앱 쿼리 로그는 LUIS 컨테이너에 제출된 각 예측 쿼리에 대한 쿼리, 응답 및 타임스탬프를 포함합니다. 
...
LUIS 컨테이너는 Azure 계정의 _인지 서비스_ 리소스를 사용하여 Azure에 청구 정보를 보냅니다. 
...
* 구성 설정에 대한 [컨테이너 구성을](luis-container-configuration.md) 검토합니다.
* 알려진 기능 [제한에](luis-container-limitations.md) 대한 LUIS 컨테이너 제한을 참조하십시오.
