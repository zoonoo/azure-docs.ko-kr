---
title: Java를 사용하여 LUIS(Language Understanding) 앱을 호출하는 방법을 알아보는 자습서 | Microsoft Docs
...
ms.locfileid: "36263723"
---
# <a name="tutorial-call-a-luis-endpoint-using-java"></a>자습서: Java를 사용하여 LUIS 엔드포인트 호출
...
4. [LUIS](luis-reference-regions.md) 웹 사이트로 돌아가서 Azure 계정을 사용하여 로그인합니다. 
    ...
    [![](media/luis-get-started-java-get-intent/app-list.png "앱 목록 스크린샷")](media/luis-get-started-java-get-intent/app-list.png)
...
2. 브라우저 창에 URL을 붙여넣고 Enter 키를 누릅니다. LUIS에서 `HomeAutomation.TurnOn` 의도 및 `bedroom` 값이 있는 `HomeAutomation.Room` 엔터티를 검색한다고 나타내는 JSON 결과가 브라우저에 표시됩니다.
    ...
    ![JSON 결과에서 TurnOn 의도 검색](./media/luis-get-started-java-get-intent/turn-on-bedroom.png)
3. URL의 `q=` 매개 변수 값을 `turn off the living room light`로 변경하고 Enter 키를 누릅니다. 이제 결과에는 LUIS에서 `HomeAutomation.TurnOff` 의도 및 `living room` 값이 있는 `HomeAutomation.Room` 엔터티를 검색했다고 표시됩니다. 
    ...
    ![JSON 결과에서 TurnOff 의도 검색](./media/luis-get-started-java-get-intent/turn-off-living-room.png)


## <a name="consume-a-luis-result-using-the-endpoint-api-with-java"></a>Java에서 엔드포인트 API를 사용하여 LUIS 결과 사용 

Java를 사용하여 이전 단계의 브라우저 창에서 본 것과 동일한 결과에 액세스할 수 있습니다. 
1. 다음 코드를 복사하여 IDE에서 클래스를 만듭니다.
...
2. `SubscriptionKey` 변수 값을 LUIS 구독 키로 바꿉니다.

3. IDE에서 `httpclient` 및 `httpcore` 라이브러리에 대한 참조를 추가합니다.
