...
미리 빌드된 모델은 도메인, 의도, 발언 및 엔터티를 제공합니다. 미리 빌드된 모델로 앱을 시작하거나 나중에 앱에 관련 모델을 추가할 수 있습니다. 
...
언어 이해(LUIS)는 미리 *빌드된 도메인을*제공하며, 미리 학습된 [의도](luis-how-to-add-intents.md) 및 [엔터티의](luis-concept-entity-types.md) 모델은 도메인 또는 클라이언트 응용 프로그램의 공통 범주에 대해 함께 작동합니다. 

미리 빌드된 도메인은 LUIS 앱에 추가되도록 교육 및 준비됩니다. 미리 빌드된 도메인의 의도와 엔터티는 앱에 추가한 후 완전히 사용자 지정할 수 있습니다. 
...
미리 빌드된 도메인에 포함되는 의도가 LUIS 앱에 포함하려는 의도와 유사함을 알 수 있지만 해당 의도가 다르게 동작하도록 합니다. 예를 들어 **Places** 미리 빌드된 장소 `MakeReservation` 도메인은 음식점 예약을 위한 의도를 제공하지만 앱에서 해당 의도를 사용하여 호텔을 예약하도록 합니다. 이 경우 호텔 예약을 한 다음 앱을 다시 학습하려는 의도에 예제 발언을 추가하여 해당 의도의 동작을 수정할 수 있습니다. 

[미리 빌드된 도메인 참조](./luis-reference-prebuilt-domains.md)에서 미리 빌드된 도메인의 전체 목록을 찾을 수 있습니다.

## <a name="prebuilt-intents"></a>미리 빌드된 의도

LUIS는 미리 빌드된 각 도메인에 대해 미리 빌드된 의도와 발언을 제공합니다. 전체 도메인을 추가하지 않고 의도를 추가할 수 있습니다. 의도를 추가하는 것은 앱에 의도 및 발언을 추가하는 프로세스입니다. 의도 이름과 발언 목록 둘 다 수정할 수 있습니다.  
