...
단어나 구가 둘 이상의 엔터티와 일치하는 경우, 끝점 쿼리는 각 엔터티를 반환합니다. 미리 빌드된 number 엔터티와 미리 빌드된 datetimeV2를 둘 다 추가하고 `create meeting on 2018/03/12 for lunch with wayne` 발화가 있는 경우 LUIS는 모든 엔터티를 인식하고 엔터티의 배열을 JSON 끝점 응답의 일부로 반환합니다. 
...
단어나 구가 둘 이상의 목록 엔터티와 일치하는 경우, 끝점 쿼리는 각 목록 엔터티를 반환합니다.

`when is the best time to go to red rock?` 쿼리에서 앱이 둘 이상의 목록에 `red`라는 단어를 갖는 경우 LUIS는 모든 엔터티를 인식하고 엔터티의 배열을 JSON 끝점 응답의 일부로 반환합니다. 
...
정기적으로 끝점 발화를 검토하여 엔터티를 정규식으로 식별하거나 정확한 텍스트 일치를 찾을 수 있는 일반적인 사용 패턴을 찾습니다.  
