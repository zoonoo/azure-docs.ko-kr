---
title: PHP를 사용하여 LUIS 앱에 발언을 추가하는 방법을 알아보는 자습서 | Microsoft Docs
description: 이 자습서에서는 PHP를 사용하여 LUIS 앱을 호출하는 방법을 알아봅니다.
...
ms.locfileid: "36263783"
---
# <a name="tutorial-add-utterances-to-app-using-php"></a>자습서: PHP를 사용하여 앱에 발언 추가 
이 자습서에서는 PHP로 API 작성을 사용하여 의도에 발언을 추가하는 프로그램을 작성합니다.
...
* 기존 LUIS [**응용 프로그램 ID**](./luis-get-started-create-app.md) 응용 프로그램 ID는 응용 프로그램 대시보드에 표시됩니다. `utterances.json` 파일에서 사용된 의도 및 엔터티를 포함한 LUIS 응용 프로그램은 `add-utterances.php`에서 코드를 실행하기 전에 설치되어야 합니다. 이 아티클의 코드는 의도 및 엔터티를 만들지 않습니다. 기존 의도 및 엔터티에 대한 발언만을 추가합니다. 
* 발언을 수신하는 응용 프로그램 내의 **버전 ID**입니다. 기본 ID는 "0.1"입니다.
* VSCode에서 `add-utterances.php` 프로젝트라는 새 파일을 만듭니다.

> [!NOTE] 
> 전체 `add-utterances.cs` 파일 및 예제 `utterances.json` 파일은 [**LUIS 샘플** Github 리포지토리](https://github.com/Microsoft/LUIS-Samples/blob/master/documentation-samples/authoring-api-samples/php/)에서 지원됩니다.


## <a name="write-the-php-code"></a>PHP 코드 작성
...
> [!NOTE]
> `utterances.json` 파일에서 사용된 의도 및 엔터티를 포함한 LUIS 응용 프로그램은 `add-utterances.php`에서 코드를 실행하기 전에 설치되어야 합니다. 이 아티클의 코드는 의도 및 엔터티를 만들지 않습니다. 기존 의도 및 엔터티에 대한 발언만을 추가합니다.
...
utterance.json을 인수로 사용하여 `add-utterances.php`를 호출하면 새 발언에서 LUIS를 추가하지만 학습하지는 않습니다.
````
> php add-utterances.php ./utterances.json
````

다음 JSON은 발언 추가 API 호출에서 반환됩니다. `response` 필드는 추가된 발언에서 이 형식으로 표시됩니다. `hasError`는 false이며 발언이 추가되었음을 나타냅니다.  
...
`-train` 인수를 사용하여 `add-utterance.php`를 호출하면 학습할 요청을 보냅니다. 
