...
Utterance를 삭제 해야 하는지 확실 하지 않은 경우이를 None으로 이동 하거나와 `miscellaneous` 같은 새 의도를 만들어 utterance을 해당 의도로 이동 합니다.

## <a name="disable-active-learning"></a>활성 학습 사용 안 함

활성 학습을 사용 하지 않도록 설정 하려면 사용자 쿼리를 기록 하지 않습니다. 이렇게 하려면 기본값은 false 이므로 querystring 매개 변수 및 `log=false` 값을 사용 하 여 [끝점 쿼리](luis-get-started-create-app.md#query-the-v2-api-prediction-endpoint) 를 설정 하거나 querystring 값을 사용 하지 않고이를 수행 합니다.
