...
일괄 처리 테스트는 활성 학습된 버전의 유효성을 검사하여 예측 정확도를 측정합니다. 일괄 처리 테스트를 사용하면 활성 버전에서 각 의도 및 엔터티의 정확도를 볼 수 있으므로 차트로 결과를 표시할 수 있습니다. 일괄 처리 테스트 결과를 검토하여 앱이 발화 내의 올바른 의도 또는 레이블 엔터티를 자주 식별하지 못하는 경우 의도에 예제 발언을 추가하는 등 정확도를 높이기 위해 적절한 조치를 취합니다.

## <a name="group-data-for-batch-test"></a>일괄 처리 테스트를 위해 데이터 그룹화

일괄 처리 테스트에 사용되는 발언은 LUIS에 새로 추가된 발언이어야 합니다. 발언데이터 세트가 있는 경우 발언을 의도에 추가된 예시 발언, 게시된 끝점에서 받은 발언 및 학습 후 LUIS를 일괄 테스트하는 데 사용되는 발언의 세 가지 집합으로 나눕니다. 

## <a name="a-data-set-of-utterances"></a>발언의 데이터 집합

일괄 처리를 위해 *데이터 집합이라고*하는 발언의 일괄 처리 파일을 제출합니다. 데이터 집합은 최대 1,000개의 레이블이 지정된 **중복되지 않은** 발언을 포함하는 JSON 형식의 파일입니다. 앱에서 최대 10개의 데이터 세트를 테스트할 수 있습니다. 더 많은 테스트를 수행해야 하는 경우 데이터 집합을 삭제한 다음 새 데이터를 추가합니다.
...
**거짓 긍정** 및 **거짓 부정** 섹션의 데이터 요소는 오류를 나타내며, 조사해야 합니다. 모든 데이터 요소가 **참 긍정** 및 **참 부정** 섹션에 있는 경우 앱의 정확도는 이 데이터 집합에서 완벽합니다.
