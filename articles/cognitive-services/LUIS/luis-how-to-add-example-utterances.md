...
다음 절차에서는 **의도** 페이지의 다음 utterance 내에서 사용자 지정 엔터티를 만들고 태그를 지정 합니다.
...
1. 발언에서 `SQL server`을 선택하고 단순 엔터티로 레이블을 지정합니다. 나타나는 엔터티 드롭다운 상자에서 기존 엔터티를 선택하거나 새 엔터티를 추가합니다. 새 엔터티를 추가 하려면 텍스트 상자에 이름을 `Job` 입력 한 다음 **새 엔터티 만들기**를 선택 합니다.
    ...
    > * 두 개 이상의 단어 집합의 경우 첫 번째 단어를 선택한 다음 마지막 단어를 선택 합니다.
...
목록 엔터티는 시스템에서 관련 단어와 정확히 일치 하는 텍스트의 집합을 나타냅니다. 
...
1. **의도** 페이지의 예제 utterance에서 새 목록에 표시할 단어나 구를 선택 합니다. 엔터티 드롭다운이 표시 되 면 맨 위 텍스트 상자에 새 목록 엔터티의 이름을 입력 한 다음 **새 엔터티 만들기**를 선택 합니다.   
...
## <a name="add-a-composite-entity"></a>복합 엔터티 추가
...
1. 드롭다운 목록에서 **복합 엔터티에서 줄 바꿈** 을 선택 합니다. 

1. 복합 엔터티의 마지막(맨 오른쪽) 단어를 선택합니다. 녹색 선은 복합 엔터티를 따릅니다. 이는 복합 엔터티에 대 한 시각적 표시기 이며, 가장 왼쪽 자식 엔터티에서 가장 오른쪽 자식 엔터티로 복합 엔터티의 모든 단어 아래에 있어야 합니다.
...
## <a name="add-entitys-role-to-utterance"></a>Utterance에 엔터티 역할 추가

역할은 utterance 컨텍스트에 따라 결정 되는 엔터티의 명명 된 하위 형식입니다. Utterance 내에서 엔터티를 엔터티로 표시 하거나 해당 엔터티 내에서 역할을 선택할 수 있습니다. 컴퓨터에서 학습 한 사용자 지정 엔터티 (단순 엔터티 및 복합 엔터티)를 비롯 한 모든 엔터티에는 컴퓨터에서 학습 된 엔터티 (미리 작성 된 엔터티, 정규식 엔터티, 엔터티 나열)를 비롯 한 역할이 있을 수 있습니다. 

실습 자습서에서 [엔터티 역할로 utterance을 표시 하는 방법](tutorial-entity-roles.md) 에 대해 알아봅니다. 
...
![엔터티 상태 선택의 스크린샷](./media/luis-how-to-add-example-utterances/entity-prediction-error-correction.png)
...
의도 페이지의 utterance에서 컴퓨터에서 학습 한 엔터티 레이블을 제거할 수 있습니다. 엔터티가 기계 학습되지 않은 경우 발언에서 제거할 수 없습니다. 발언에서 기계 학습되지 않은 엔터티를 제거해야 할 경우에는 전체 앱에서 해당 엔터티를 삭제해야 합니다. 
