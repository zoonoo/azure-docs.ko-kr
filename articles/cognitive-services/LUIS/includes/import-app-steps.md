...
1. **관리** 섹션의 **버전** 탭에서 `sentiment` 버전을 선택한 다음, **복제**를 선택하여 버전을 복제하고 `ml-entity`의 새 이름을 지정한 다음, **완료**를 선택하여 복제 프로세스를 마칩니다. 버전 이름이 URL 경로의 일부로 사용되므로 이름에는 URL에 유효하지 않은 문자가 포함될 수 없습니다.
