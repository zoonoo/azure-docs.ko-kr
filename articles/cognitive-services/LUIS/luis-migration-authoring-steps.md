...
1. 선택적으로, 앱에 협력자가 있는 경우 마이그레이션에 대 한 정보를 알려 주는 **전자 메일을 보낼지** 묻는 메시지가 표시 됩니다. 이 단계는 선택 사항입니다.
...
1. 기존 제작 리소스를 사용 하거나 새 제작 리소스를 만들도록 선택 하 여 LUIS authoring 리소스를 만들도록 선택 합니다.

    > [!div class="mx-imgBorder"]
    > 제작 리소스](./media/migrate-authoring-key/choose-existing-authoring-resource.png) 만들기 ![
...
마이그레이션의 앱 소유자와 동일한 단계를 수행 합니다. 이 프로세스는 `LUIS.Authoring`종류의 새 제작 리소스를 만듭니다.
