...
1. 구 목록은 **전역** 설정을 사용하여 전체 앱 또는 특정 모델(의도 또는 엔터티)에 적용할 수 있습니다. 의도 또는 엔터티의 _설명자로_ 구 목록을 만들면 토글이 전역이 아닌 것으로 설정됩니다. 이 경우 토글의 의미는 설명자가 해당 모델에만 있는 기능이므로 응용 프로그램에 _대한 전역이 아니라는_ 것입니다.
