---
title: 데이터 추출 - LUIS
description: 의도 및 엔터티를 사용하여 발언 텍스트에서 데이터를 추출합니다. 언어 이해(LUIS)에서 어떤 종류의 데이터를 추출할 수 있는지 알아봅니다.
author: diberry
ms.topic: conceptual
ms.date: 01/23/2020
...
ms.locfileid: "79221528"
---
# <a name="extract-data-from-utterance-text-with-intents-and-entities"></a>의도 및 엔터티를 사용하여 발언 텍스트에서 데이터 추출
...
챗봇 또는 LUIS 호출 앱이 두 개 이상의 의도 점수를 기반으로 결정을 내리는 경우 모든 의도 점수를 반환합니다.


#### <a name="v2-prediction-endpoint-response"></a>[V2 예측 엔드포인트 응답](#tab/V2)

쿼리 문자열 매개 `verbose=true`변수를 설정합니다. 엔드포인트 응답은 다음과 같습니다.
...
쿼리 문자열 매개 `show-all-intents=true`변수를 설정합니다. 엔드포인트 응답은 다음과 같습니다.
...
LUIS에서 [토큰 지원을](luis-language-support.md#tokenization) 검토합니다.
...
[복합 엔터티는](reference-entity-composite.md) 미리 빌드된 엔터티, 단순 엔터티, 정규식 및 목록 엔터티와 같은 다른 엔터티로 구성됩니다. 개별 엔터티가 전체 엔터티를 형성합니다.

## <a name="list-entity-data"></a>목록 엔터티 데이터

[목록 엔터티는](reference-entity-list.md) 동의어와 함께 고정된 닫힌 관련 단어 집합을 나타냅니다. LUIS는 목록 엔터티에 대한 추가 값을 검색하지 않습니다. **권장** 기능을 사용하여 현재 목록을 기준으로 권장되는 새 단어를 확인합니다. 동일한 값을 갖는 목록 엔터티가 둘 이상 있는 경우, 각 엔터티가 엔드포인트 쿼리에서 반환됩니다.
...
쿼리 문자열 매개 `verbose=true`변수가 없으면 다음
...
쿼리 문자열 매개 `verbose=true`변수를 사용하면 다음을 수행합니다.
...
[정규식 엔터티는](reference-entity-regular-expression.md) 제공한 정규식 패턴을 기반으로 엔터티를 추출합니다.
...
사용자의 이름에는 언어 및 문화권에 따라 일부 약한 형식이 적용될 수 있습니다. 미리 빌드된 **[personName](luis-reference-prebuilt-person.md)** 엔터티 또는 이름과 성의 [역할을](luis-concept-roles.md) 가진 **[간단한 엔터티를](luis-concept-entity-types.md#simple-entity)** 사용합니다.

단순 엔터티를 사용하는 경우 발언의 다른 부분, 서로 다른 길이의 발언 및 None 의도를 포함한 모든 의도에서 이름과 성을 사용하는 예제를 제공해야 합니다. 정기적으로 엔드포인트 발화를 [검토](luis-how-to-review-endoint-utt.md)하여 올바르게 예측되지 않은 이름에 레이블을 지정합니다.

### <a name="names-of-places"></a>위치 이름

위치 이름은 도시, 카운티, 주, 주, 국가/지역과 같이 설정되고 알려져 있습니다. 미리 빌드된 엔터티 **[지리V2를](luis-reference-prebuilt-geographyv2.md)** 사용하여 위치 정보를 추출합니다.

### <a name="new-and-emerging-names"></a>새롭게 떠오르는 이름

일부 앱은 제품이나 회사와 같이 새롭게 떠오르는 이름을 찾을 수 있어야 합니다. 이러한 유형의 이름은 가장 어려운 유형의 데이터 추출입니다. **[간단한 엔터티로](luis-concept-entity-types.md#simple-entity)** 시작하여 [구 목록을 추가합니다.](luis-concept-feature.md) 정기적으로 엔드포인트 발화를 [검토](luis-how-to-review-endoint-utt.md)하여 올바르게 예측되지 않은 이름에 레이블을 지정합니다.
...
엔터티 이름은 `Location`두 가지 `Origin` 역할및 `Destination`.
...
[pattern.any는](reference-entity-pattern-any.md) 엔터티가 시작되고 끝나는 위치를 표시하기 위해 패턴의 템플릿 발언에만 사용되는 가변 길이 자리 표시자입니다.
...
LUIS 끝점은 다른 엔터티에서 동일한 데이터를 검색할 수 있습니다.
...
쿼리 `verbose=true` 문자열 매개 변수가 없습니다.
...
쿼리 `verbose=true` 문자열 매개 변수로 사용 합니다.
...
쿼리 `verbose=true` 문자열이 없으면 다음을 수행합니다.
...
쿼리 `verbose=true` 문자열에서 다음을 수행합니다.
