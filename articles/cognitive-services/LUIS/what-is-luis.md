...
LUIS 앱은 도메인별 자연어 모델을 포함합니다. 미리 빌드된 모델로 LUIS 앱을 시작할 수도 있고, 직접 작성할 수도 있고, 미리 빌드된 도메인 조각을 사용자 고유의 정보와 혼합할 수도 있습니다.

* **미리 빌드된 모델** LUIS에는 의도, 발언 및 미리 빌드된 엔터티를 비롯하여 미리 빌드된 여러 도메인 모델이 포함되어 있습니다. 미리 빌드된 모델의 의도 및 발언을 사용하지 않고도 미리 빌드된 엔터티를 사용할 수 있습니다. [미리 빌드된 도메인 모델](./howto-add-prebuilt-models.md "미리 빌드된 도메인 모델")에는 모든 디자인이 포함되며 LUIS를 빠르게 시작하는 유용한 방법입니다.
