...
Local Business Search API는 Bing에 검색 쿼리를 전송하여 식당, 호텔 또는 기타 로컬 비즈니스를 포함하는 결과를 가져옵니다. 장소의 경우 쿼리에서 로컬 비즈니스의 이름 또는 범주를 지정할 수 있습니다(예: 근처의 식당). 엔터티 결과에는 사람, 장소 또는 사물이 포함됩니다. 이 컨텍스트의 장소는 회사 엔터티, 주, 국가 등입니다.  
...
|<a name="mkt" />mkt|결과가 나오는 지역/국가입니다. <br /><br />가능한 지역/국가 값 목록은 [지역/국가 코드](#market-codes)를 참조하세요.<br /><br /> **참고:** Local Business Search API는 현재 en-us 지역/국가와 언어만 지원합니다.<br /><br />|문자열|yes|
