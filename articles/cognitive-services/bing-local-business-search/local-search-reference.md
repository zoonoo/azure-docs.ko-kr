...
|이름|값|Type|필수|  
|----------|-----------|----------|--------------|
|<a name="count" />count|지정 된 인덱스부터 반환할 결과 수는 `offset` 매개 변수입니다.|문자열|아닙니다.|   
|<a name="localCategories" />localCategories|비즈니스 범주별로 검색을 정의하는 옵션 목록입니다.  [로컬 비즈니스 범주 검색](local-categories.md) 참조|문자열|아닙니다.|  
...
|<a name="offset"/>offset|`count` 매개 변수에서 지정된 결과를 시작하는 인덱스입니다.|정수 |아닙니다.|  
