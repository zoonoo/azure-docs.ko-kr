...
이 문서에서는 [텍스트 분석 API](//go.microsoft.com/fwlink/?LinkID=759711)를 사용하도록 서비스나 응용 프로그램을 등록하는 방법에 대해 설명합니다. 이러한 API를 사용하여 텍스트에서 감성, 핵심 문구, 항목 및 언어를 검색할 수 있습니다. [대화형 데모 환경을 보려면 여기를 클릭합니다.](//go.microsoft.com/fwlink/?LinkID=759712)
...
- **항목** - 여러 문서에 걸쳐 논의되는 사항

- **언어** - 텍스트가 작성된 언어

이 API는 제출된 텍스트 레코드당 하나의 트랜잭션을 사용합니다.
...
- **중지 단어.** 이러한 단어와 해당 유사 형식(예: 복수형)은 전체 항목 검색 파이프라인에서 제외됩니다. 일반적인 단어의 경우 이 옵션을 사용합니다.예를 들어 “issue”, “error” 및 “user”는 소프트웨어에 대한 고객 불만 사항에 대한 적절한 선택일 수 있습니다. 각 문자열에는 단어 한 개가 있어야 합니다.
- **중지 문구** - 이러한 문구는 반환된 항목 목록에서 제외됩니다. 이 옵션을 사용하여 결과에 표시하지 않으려는 일반적 항목을 제외합니다. 예를 들어, "Microsoft" 및 "Azure"는 제외할 항목에 대한 적합한 선택입니다. 문자열에는 여러 개의 단어가 포함될 수 있습니다.

텍스트에서 항목을 검색하려면 다음 단계를 수행합니다.
...
1. 작업 2에서 정의한 대로 동일한 헤더를 사용하여 항목 끝점에 대한 **POST** 호출을 수행합니다.

        POST https://westus.api.cognitive.microsoft.com/text/analytics/v2.0/topics

1. 이렇게 하면 응답에서 헤더로 `operation-location`을 반환하며, 여기서 값은 결과 항목에 대해 쿼리하는 URL입니다.
