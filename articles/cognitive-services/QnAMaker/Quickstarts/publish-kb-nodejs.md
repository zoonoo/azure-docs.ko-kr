...
리소스에서 키를 가져온 후 [ 및 ](https://docs.microsoft.com/azure/cognitive-services/cognitive-services-apis-create-account#configure-an-environment-variable-for-authentication)라는 리소스에 대해 `QNAMAKER_RESOURCE_KEY`환경 변수를 만듭니다`QNAMAKER_AUTHORING_ENDPOINT`. Azure Portal에서 리소스의 **빠른 시작** 페이지에 있는 키와 엔드포인트 값을 사용합니다.
