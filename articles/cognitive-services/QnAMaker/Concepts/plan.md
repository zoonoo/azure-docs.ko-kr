...
description: QnA Maker 앱을 계획 하려면 QnA Maker 작동 하 고 다른 Azure 서비스와 상호 작용 하는 방법 및 몇 가지 기술 자료 개념을 이해 해야 합니다.
ms.topic: conceptual
ms.date: 07/2/2020
...
ms.locfileid: "85875714"
---
# <a name="plan-your-qna-maker-app"></a>QnA Maker 앱 계획

QnA Maker 앱을 계획 하려면 QnA Maker 작동 하 고 다른 Azure 서비스와 상호 작용 하는 방법 및 몇 가지 기술 자료 개념을 이해 해야 합니다.

## <a name="azure-resources"></a>Azure 리소스

QnA Maker를 사용 하 여 만든 각 [Azure 리소스](azure-resources.md#resource-purposes) 는 특정 목적이 있습니다. 각 리소스에는 고유한 용도, 제한 및 [가격 책정 계층이](azure-resources.md#pricing-tier-considerations)있으므로 이러한 리소스가 계획 프로세스의 일부로 수행 하는 작업을 이해 하는 것이 중요 합니다.
...
작성 및 쿼리 예측을 학습 하는 동안에는 각 리소스의 무료 계층을 사용 하 여이 작업을 수행 하 `F0` 고 작성 및 쿼리 예측 환경을 모두 제공 합니다. 프로덕션, 라이브 시나리오로 이동 하는 경우 리소스 선택을 다시 계산 합니다.
...
실제 앱을 빌드할 계획인 경우 기술 자료의 크기와 필요한 쿼리 예측 요청에 대 한 리소스를 계획 합니다.
...
이러한 리소스 중 일부를 이미 사용 중인 경우 리소스 공유를 고려할 수 있습니다. 일부 리소스는 [공유할 수](azure-resources.md#share-services-with-qna-maker)있지만이는 고급 시나리오입니다.

동일한 QnA Maker 리소스에서 만든 모든 기술 자료가 동일한 **테스트** 쿼리 예측 끝점을 공유 합니다.

### <a name="understanding-impact-of-resource-selection"></a>리소스 선택의 영향 이해

적절 한 리소스 선택은 기술 자료가 쿼리 예측에 성공적으로 응답 하는 것을 의미 합니다.

기술 자료가 제대로 작동 하지 않는 경우 일반적으로 문제는 부적절 한 리소스 관리입니다.
...
기술 자료는 QnA Maker 리소스에 직접 연결 되며 쿼리 예측 요청에 대답 하는 데 사용 되는 QnA (질문과 대답) 쌍을 보유 합니다.
...
쿼리 예측 끝점에 쿼리를 보내기 전에 언어를 사용 하 여 QnA Maker 리소스를 구조화 하거나 [번역기](../../translator/translator-info-overview.md) 를 사용 하 여 다른 언어에서 기술 자료의 언어로 쿼리를 변경 합니다.

### <a name="ingesting-data-sources"></a>수집 데이터 원본

기술 자료를 만드는 데 사용 되는 수집 [데이터 원본은](knowledge-base.md)다음 중 하나일 수 있습니다.
...
QnA Maker는 이미지를 비롯 한 콘텐츠 인증을 제공 하지 않기 때문에 클라이언트 응용 프로그램 뿐만 아니라 QnA Maker 포털의 테스트 창에 표시 하기 위해 공용 URL에서 연결 된 이미지를 사용할 수 있어야 합니다.
...
봇이 기술 자료에 연결 하는 경우 bot를 권장 합니다. 기술 자료 중 하나를 사용 하 여 여러 서비스에 연결 하는 경우 기술 자료에 대 한 chit 채팅을 계속 사용 하도록 선택할 수 있지만, 사용자가 사용 하기에 적합 한 아키텍처 디자인 인지 확인 하기 위해 봇 서비스의 상호 작용 방식을 검토 해야 합니다.

### <a name="conversation-flow-with-a-knowledge-base"></a>기술 자료가 있는 대화 흐름

일반적으로 대화 흐름은 또는와 같은 사용자의 인사말으로 시작 `Hi` `Hello` 합니다. 기술 자료는와 같은 일반적인 답변을 사용 하 여 답변할 수 있으며 `Hi, how can I help you` , 대화를 계속 하기 위해 후속 작업 프롬프트를 제공할 수도 있습니다.
...
[클라이언트 응용 프로그램과](integration-with-other-applications.md) 의 통합은 예측 런타임 끝점으로 쿼리를 보내는 것을 의미 합니다. 쿼리는 QnA Maker의 웹 앱 끝점에 대 한 SDK 또는 REST 기반 요청을 사용 하 여 특정 기술 자료에 전송 됩니다.

클라이언트 요청을 올바르게 인증 하려면 클라이언트 응용 프로그램에서 올바른 자격 증명과 기술 자료 ID를 전송 해야 합니다. Azure Bot Service를 사용 하는 경우 Azure Portal에서 봇 구성의 일부로 설정을 구성 합니다.
...
클라이언트 응용 프로그램에서 추가 정보를 처리 하는 대체 방법을 제공 하는 대화 흐름을 지원 하는 경우 (추가 정보를 제공 하 여 추가 정보를 제공 하거나),이를 조기에 설계 하 고 클라이언트 응용 프로그램에서를 사용 하는 쿼리를 다른 서비스에서 사용 하거나 기술 자료에 전송 하 여 올바르게 처리 해야 합니다.

### <a name="dispatching-between-qna-maker-and-language-understanding-luis"></a>QnA Maker 및 Language Understanding 간 디스패치 (LUIS)

클라이언트 응용 프로그램은 여러 기능을 제공할 수 있으며,이 중 하나는 기술 자료에서 답변 합니다. 다른 기능은 여전히 대화형 텍스트를 이해 하 고 그에 대 한 의미를 추출 해야 합니다.

일반적인 클라이언트 응용 프로그램 아키텍처는 QnA Maker와 [Language Understanding (LUIS)](../../LUIS/what-is-luis.md) 를 함께 사용 하는 것입니다. LUIS는 다른 서비스를 비롯 하 여 모든 쿼리에 대 한 텍스트 분류 및 추출을 제공 하며 QnA Maker는 기술 자료에서 답변을 제공 합니다.

[공유 아키텍처](../choose-natural-language-processing-service.md)에서 두 서비스 간의 디스패치는 Bot Framework의 [디스패치](https://github.com/Microsoft/botbuilder-tools/tree/master/packages/Dispatch) 도구를 사용 하 여 수행 됩니다.

### <a name="active-learning-from-a-client-application"></a>클라이언트 응용 프로그램에서의 활성 학습

QnA Maker는 답변에 대 한 대체 질문을 제안 하 여 기술 자료를 개선 하는 데 _활성 학습_ 을 사용 합니다. 클라이언트 응용 프로그램은이 [활성 학습](active-learning-suggestions.md)의 일부를 담당 합니다. 대화형 프롬프트를 통해 클라이언트 응용 프로그램은 기술 자료에서 반환 된 대답이 사용자가 찾으려는 답변이 아닌 것을 확인할 수 있으며, 더 나은 답을 결정 합니다. 클라이언트 응용 프로그램은 예측 품질을 향상 시키기 위해 [해당 정보를 기술 자료로 다시 보내야](active-learning-suggestions.md#how-you-give-explicit-feedback-with-the-train-api) 합니다.

### <a name="providing-a-default-answer"></a>기본 대답 제공

기술 자료가 대답을 찾지 못하면 _기본 답_을 반환 합니다. 이 응답은 QnA Maker 포털, **설정** 페이지 또는 [api](https://docs.microsoft.com/rest/api/cognitiveservices/qnamaker/knowledgebase/update#request-body)에서 구성할 수 있습니다.

이 기본 대답은 Azure bot 기본 대답과 다릅니다. Azure 봇 기본 대답은 구성 설정의 일부로 봇에 대해 Azure Portal에서 구성 되며 점수 임계값이 충족 되지 않을 경우 반환 됩니다.

## <a name="prediction"></a>예측

예측은 기술 자료의 응답 이며 대답 보다 많은 정보를 포함 합니다. 쿼리 예측 응답을 가져오려면 [Geneateanswer API](query-knowledge-base.md)를 사용 합니다.
...
* Cognitive Search 우선 순위-Cognitive Search에서 응답이 반환 되는 경우 허용 되는 _대답_ 수는 Cognitive Search에서 최상의 답을 반환 하 여 QnA Maker ranker에 전달할 수 있도록 충분히 높아야 합니다.
* QnA Maker-두 번째 순위-기능화 및 기계 학습을 적용 하 여 최상의 답을 결정 합니다.

### <a name="service-updates"></a>서비스 업데이트

서비스 업데이트는 [최신 런타임 업데이트](../how-to/set-up-qnamaker-service-azure.md#get-the-latest-runtime-updates)를 적용 하 여 자동으로 관리 됩니다.
...
* 메타 데이터-쿼리할 때 필터링 가능 이렇게 하면 데이터의 원본, 콘텐츠, 형식 및 용도에 대 한 추가 정보로 QnA 쌍에 태그를 지정할 수 있습니다.
* 후속 프롬프트-사용자가 올바른 답변을 받을 때 기술 자료를 통해 경로를 결정 합니다.
* 대체 질문-검색을 다양 한 형식의 질문에 대 한 답변에 일치 시킬 수 있도록 대체 질문이 중요 합니다. [활성 학습 제안은](active-learning-suggestions.md) 대체 질문으로 전환 됩니다.
...
및 기술 자료에 대해 _동일한 점수_ 를 갖도록 `test` 하려면 `production` QnA Maker 리소스를 단일 기술 자료로 격리 합니다. 이 아키텍처에서 리소스는 격리 된 일괄 처리 테스트의 길이에만 있어야 합니다.
