...
대량 Kb 경우 [Generateanswer API](../how-to/metadata-generateanswer-usage.md#get-answer-predictions-with-the-generateanswer-api) 및 body 속성으로 자동화 된 테스트를 `isTest` 사용 합니다 .이 속성은 `test` 게시 된 기술 자료 대신 기술 자료를 쿼리 합니다.
...
*테스트 기술 자료* 는 응답의 정확성 및 완전성을 위해 현재 편집, 저장 및 테스트 된 버전입니다. 테스트 기술 자료에 대 한 변경 내용은 응용 프로그램이 나 채팅 봇의 최종 사용자에 게 영향을 주지 않습니다. 테스트 기술 자료는 HTTP 요청에서 `test` 로 알려져 있습니다. 이 `test` 정보는 QnA Maker의 포털 대화형 **테스트** 창에서 사용할 수 있습니다.
