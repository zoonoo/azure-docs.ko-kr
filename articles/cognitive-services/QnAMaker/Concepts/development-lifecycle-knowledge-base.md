...
QnA Maker는 모델 변경, 발언 예제, 게시 및 엔드포인트 쿼리에서 데이터 수집이란 반복 주기 안에서 가장 잘 학습합니다. 
...
편집 또는 자동 추출을 통해 기술 자료에 콘텐츠가 채워지면 기술 자료를 테스트 할 수 있습니다. **테스트** 패널을 통해 일반적인 사용자 쿼리를 입력하고 반환된 응답이 예상대로 충분한 신뢰성 점수를 받았는지 확인할 수 있습니다. 대체 질문을 추가하여 낮은 신뢰성 점수를 바로 잡을 수 있습니다. 또한 쿼리가 "KB에 일치하는 항목이 없습니다" 기본 답변을 반환하면 새 답변을 추가할 수 있습니다. 이  테스트-업데이트 반복 과정은 사용자가 결과에 만족할 때까지 계속됩니다. [기술 자료 테스트](../How-To/test-knowledge-base.md) 방법을 알아보세요.
...
각각의 기술 자료를 테스트 대상으로 지정할 수 있습니다. API를 사용하여 generateAnswer 호출시에 `isTest=true` 플래그를 지정하면, 테스트 버전의 기술 자료를 지정할 수 있습니다.
...
서비스 사용에 대한 다양한 분석 자료를 얻을 수 있습니다. Application Insights를 사용하여 [QnA Maker 서비스에 대한 분석](../How-To/get-analytics-knowledge-base.md) 자료를 얻는 방법을 자세히 알아보세요.
