...
각각의 기술 자료를 테스트 대상으로 지정할 수 있습니다. API를 사용하여 generateAnswer 호출 시에 `isTest=true` 플래그를 지정하면 테스트 버전의 기술 자료를 지정할 수 있습니다.
