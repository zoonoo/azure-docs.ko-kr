...
> ![개념 모델의 3 단계 다중 전환 질문](../media/qnamaker-concepts-knowledgebase/nested-multi-turn.png)

다중 턴 `.tsv`열에 대 한 열에는 다중 전환에 대 한 **메시지가 표시**됩니다. Excel에 표시 되는 `.tsv`예제는 다중 전환 자식을 정의 하기 위해 포함할 정보를 표시 합니다.
...
> [!div class="mx-imgBorder"]
> Excel에 표시 된 것 처럼 다중 전환 질문 예제를 ![](../media/qnamaker-concepts-knowledgebase/multi-turn-tsv-columns-excel-example.png)

## <a name="export-as-example"></a>예제로 내보내기

`.tsv` 파일에서 QnA 집합을 표시 하는 방법을 잘 모를 경우 QnA Maker 포털에서 집합을 만들고 저장 한 다음 해당 집합을 나타내는 방법에 대 한 예의 기술 자료를 내보냅니다.
