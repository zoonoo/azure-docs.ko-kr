...
1. 프로젝트에 대한 이름과 설명을 입력합니다. 그런 다음 리소스 그룹을 선택합니다. 로그인한 계정이 Azure 계정과 연결되어 있으면 리소스 그룹 드롭다운에는 Custom Vision Service 리소스를 포함하는 모든 Azure 리소스 그룹이 표시됩니다. 
   ...
   > [!NOTE]
   > 사용 가능한 리소스 그룹이 없는 경우, [Azure portal](https://portal.azure.com/)에 로그인 할 때 사용한 동일한 계정으로 [customvision.ai](https://customvision.ai) 로그인 했는지 확인 하십시오. 또한 Custom Vision 리소스가 있는 Azure Portal의 디렉터리와 동일한 “디렉터리”를 Custom Vision 포털에서 선택했는지 확인합니다. 두 사이트 모두, 화면 오른쪽 위에 있는 드롭다운 계정 메뉴에서 디렉터리를 선택할 수 있습니다. 
