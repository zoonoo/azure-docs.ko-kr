...
Notebook은 데이터 세트에서 임의 사용자, 시간 및 날씨 유형을 선택합니다. 사용자 정보는 다음과 같이 요약됩니다.

|고객 - 컨텍스트 기능|하루 중 시간|날씨 유형|
|--|--|--|
|Alice<br>Bob<br>Cathy<br>Dave|아침<br>낮<br>저녁|맑음<br>비<br>눈| 

시간이 지남에 따라 Personalizer에서 각 개인의 올바른 커피 선택 항목을 학습할 수 있도록 _시스템_에서도 커피에 대한 세부 정보를 인식하고 있습니다.
...
실행 시스템의 초기에는 Personalizer의 제안이 20%에서 30% 사이(보상 점수가 1로 표시됨)에서만 성공합니다. 일부 요청이 있으면 시스템이 향상됩니다.

초기 10,000개의 요청 후에는 오프라인 평가를 실행합니다. 이렇게 하면 Personalizer에서 데이터를 검토하여 더 나은 학습 정책을 제안할 수 있습니다. 새 학습 정책을 적용하고 2,000개의 요청으로 Notebook을 다시 실행합니다. 루프가 더 효율적으로 수행됩니다.
...
* 순위/요청 이벤트의 고유 ID
* 컨텍스트 - 사용자, 날씨 및 하루 중 시간을 임의로 선택하여 사용자를 웹 사이트 또는 모바일 디바이스에서 시뮬레이션합니다.
* 기능 - Personalizer에서 _모든_ 커피 데이터를 제안합니다.

시스템은 커피 선택 항목의 순위를 받은 다음, 동일한 하루 중 시간 및 날씨에 대해 해당 예측과 사용자의 알려진 선택 항목을 비교합니다. 알려진 선택 항목이 예측된 선택 항목과 동일하면 1의 **보상**을 Personalizer에 다시 보냅니다. 그렇지 않으면 보상은 0입니다. 
