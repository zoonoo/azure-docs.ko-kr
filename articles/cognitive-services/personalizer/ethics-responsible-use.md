...
description: 이러한 지침은 회사와 서비스에서 신뢰를 구축하는 데 도움이 되는 방식으로 맞춤 설정을 구현하기 위한 것입니다. 개인화가 사람들의 삶에 미치는 영향에 대해 연구하고, 배우고, 심의하기 위해 잠시 멈춰야 합니다. 의심스러우면 지침을 확인해 보세요.
...
개인과 사회에서 AI의 잠재력을 최대한 실현하려면 AI를 애플리케이션에 추가하는 실체와 AI를 통해 빌드된 애플리케이션 사용자의 신뢰를 받을 수 있는 방식으로 구현을 설계해야 합니다. 이러한 지침은 회사와 서비스에서 신뢰를 구축하는 데 도움이 되는 방식으로 Personalizer를 구현하기 위한 것입니다. 개인화가 사람들의 삶에 미치는 영향에 대해 연구하고, 배우고, 심의하기 위해 잠시 멈춰야 합니다. 의심스러우면 지침을 확인해 보세요.
...
다음 내용은 시작을 위한 검사 목록으로 사용하여 자신의 시나리오에 맞게 사용자 지정하고 구체화할 수 있습니다. 이 문서에는 두 가지 주요 섹션이 있습니다: 첫 번째 섹션은 개인화에 대한 시나리오, 기능 및 보상을 선택할 때 책임 있는 사용 고려 사항을 강조하는 데 전념합니다. 두 번째 섹션에서는 Microsoft에서 AI 시스템을 구축할 때 고려해야 한다고 확신하는 일단의 가치를 소개하고, Personalizer를 사용함으로써 AI 시스템에 미치는 영향에 조치 가능한 제안과 위험에 대해 설명합니다.
...
부정적 부작용 또는 투명성 부족에 대한 잠재력이 높은 Personalizer 사용의 예로, 지나치게 간소화된 즉각적인 보상으로 인해 개인에게 불리한 결과를 초래할 수 있는 많은 장기적 복합 요인에 따라 "보상"이 달라지는 시나리오가 있습니다. 이들은 "결과적인" 선택, 또는 해의 리스크를 관련시키는 선택으로 여겨지는 경향이 있습니다. 다음은 그 예입니다.


* **금융**: 개인이 모르는 데이터를 기반으로 하는 대출, 금융 및 보험 상품에 대한 맞춤형 제공, 얻을 수 없거나 이의를 제기할 수 없는 경우.
* **교육**: 권장 사항이 편견을 전파하고 다른 옵션에 대한 사용자의 인식을 줄일 수있는 학교 과정 및 교육 기관의 순위를 개인화합니다.
* **민주주의와 시민 참여**: 의견에 영향을 미치는 목적으로 사용자를 위한 콘텐츠를 개인화하는 것은 결과적이고 조작적입니다.
* **제3자 보상 평가**: 이용자의 행동으로 인해 발생하는 보상이 아닌, 후자의 3자 평가를 기반으로 하는 개인화 아이템.
* **탐사에 대한 편협성**: 개인화자의 탐사 행동이 해를 끼칠 수 있는 모든 상황.
...
* 사용 사례가 [GDPR](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32016R0679) 조 22 또는 기타 법률에 따라 규제되는 데이터 주체에 큰 영향을 미치는 자동 처리를 구성하는지 여부를 고려하십시오.
* 자기 충족 예언 루프를 고려합니다. 이는 맞춤 설정 보상에서 모델을 학습시키고 이후에 인구 통계 그룹이 관련 콘텐츠에 액세스하지 못하도록 추가로 배제할 수 있는 경우에 발생할 수 있습니다. 예를 들어, 저소득 지역의 대부분의 사람들은 프리미엄 보험 을 받지 못하며, 탐사가 충분하지 않으면 동네에서 천천히 아무도 제안을 전혀 보는 경향이 없습니다.
...
* **사용자 인구 통계**: 성별, 연령, 인종, 종교에 관한 특징 : 이러한 기능은 규제 상의 이유로 특정 응용 프로그램에서 허용되지 않을 수 있으며 개인화가 일반화 및 편견을 전파하기 때문에 주변을 개인화하는 것이 윤리적이지 않을 수 있습니다. 이러한 편견 전파의 예로 노인 또는 성별에 기반한 대상 그룹에 표시되지 않는 엔지니어링에 대한 채용 공고가 있습니다.
* **로케일 정보**: 전 세계 여러 곳에서 위치 정보(우편 번호, 우편 번호 또는 이웃 이름)는 소득, 인종 및 종교와 밀접한 관련이 있을 수 있습니다.
* **공정성에 대한 사용자 인식**: 응용 프로그램이 올바른 결정을 내리는 경우에도 사용자가 응용 프로그램에 표시되는 콘텐츠가 차별적인 기능과 상관 관계가 있는 것처럼 보이는 방식으로 변경되는 것을 인식하는 효과를 고려합니다.
* **피처의 의도하지 않은 바이어스**: 채우기의 하위 집합에만 영향을 주는 기능을 사용하여 도입될 수 있는 편향 유형이 있습니다. 이미지 분석을 사용하여 그림의 항목을 추출하거나 텍스트 분석을 사용하여 텍스트의 엔터티를 검색하는 경우와 같이 기능이 알고리즘 방식으로 생성되는 경우 특별히 주의해야 합니다. 이러한 기능을 만드는 데 사용하는 서비스의 특성을 알고 있어야 합니다.
...
* 자신에게 물어보십시오: 이 정보를 사용하여 콘텐츠를 개인화하면 사용자가 관심을 가지고 행복할까요? 특정 항목을 강조 표시하거나 숨기도록 결정한 방법을 보여 주는 것이 편할까요?
...
사용자 환경, 보안 또는 DevOps와 관련된 작업과 같이 응용 프로그램 수명 주기의 다른 교차 절단 작업과 마찬가지로 책임 있는 사용과 관련된 작업을 처리하는 것이 좋습니다. 이러한 작업과 해당 요구 사항은 나중에 생각할 수 없습니다. 사용 책임은 애플리케이션 수명 주기 전체에 걸쳐 논의하고 확인해야 합니다.
...
* 2018년 1월 저서 ['미래 컴퓨팅'에](https://news.microsoft.com/futurecomputed/) 게재된 AI의 책임있는 개발에 대한 Microsoft의 6가지 원칙을 참조하십시오.
...
[특징: 작업 및 컨텍스트](concepts-features.md).
