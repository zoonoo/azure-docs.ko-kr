...
개인과 사회에서 AI의 잠재력을 최대한 실현하려면 AI를 애플리케이션에 추가하는 실체와 AI를 통해 빌드된 애플리케이션 사용자의 신뢰를 받을 수 있는 방식으로 구현을 설계해야 합니다. 이러한 지침은 회사와 서비스에서 신뢰를 구축하는 데 도움이 되는 방식으로 Personalizer를 구현하기 위한 것입니다. 사용자의 생활에 대 한 개인 설정의 영향을 연구 하 고 배우고 학습 하려면 일시 중지 해야 합니다. 의심스러우면 지침을 확인해 보세요.
...
다음 내용은 시작을 위한 검사 목록으로 사용하여 자신의 시나리오에 맞게 사용자 지정하고 구체화할 수 있습니다. 이 문서에는 두 가지 주요 섹션이 있습니다. 첫 번째 섹션은 시나리오, 기능 및 Personalizer에 대 한 보상을 선택할 때의 책임이 있는 고려 사항을 강조 하는 데만 사용 두 번째 섹션에서는 Microsoft에서 AI 시스템을 구축할 때 고려해야 한다고 확신하는 일단의 가치를 소개하고, Personalizer를 사용함으로써 AI 시스템에 미치는 영향에 조치 가능한 제안과 위험에 대해 설명합니다.
...
* **교육**: 권장 사항이 편향을 전파 하 고 다른 옵션에 대 한 사용자의 인식을 줄일 수 있는 학교 강좌 및 교육 기관의 순위를 개인 설정 합니다.
* **Democracy 및 도시 참가**: 의견에 영향을 주는 목표를 가진 사용자를 위한 개인 설정은 파생적 및 manipulative입니다.
* **타사 보상 평가**: 사용자의 고유한 동작에 따라 보상을 생성 하는 대신, 사용자의 타사 평가를 기반으로 하는 항목을 개인 설정 합니다.
* **Intolerance**탐색: Personalizer의 탐색 동작이 손상 될 수 있는 상황입니다.
...
* 사용 사례가 [Gdpr](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32016R0679) 문서 22 또는 기타 법률에서 규제 된 데이터 주제에 상당한 영향을 주는 자동화 된 처리를 구성 하는지 여부를 고려 합니다.
* 자기 충족 예언 루프를 고려합니다. 이는 맞춤 설정 보상에서 모델을 학습시키고 이후에 인구 통계 그룹이 관련 콘텐츠에 액세스하지 못하도록 추가로 배제할 수 있는 경우에 발생할 수 있습니다. 예를 들어, 낮은 수입 환경에서 대부분의 사람들은 프리미엄 보험 제품을 받지 않으며, 환경에서 천천히의 천천히는 충분 한 탐색이 없는 경우 제품을 볼 수 있습니다.
...
* **사용자 인구 통계**: 성별, 성별, age, 레이스, religion 관련 기능: 이러한 기능은 특정 응용 프로그램에서 특정 응용 프로그램에 허용 되지 않을 수 있으며, 개인 설정에서 일반화 및 바이어스를 전파 하므로 이러한 기능은 특정 응용 프로그램에서 허용 되지 않을 수 있습니다. 이러한 편견 전파의 예로 노인 또는 성별에 기반한 대상 그룹에 표시되지 않는 엔지니어링에 대한 채용 공고가 있습니다.
* **로캘 정보**: 전 세계의 여러 위치에서 위치 정보 (예: 우편 번호, 우편 번호 또는 환경 이름)는 소득, 경합 및 religion와 매우 밀접 하 게 연관 될 수 있습니다.
* **사용자가 공평**하 게 결정: 응용 프로그램에서 소리를 결정 하는 경우에도 응용 프로그램에 표시 되는 콘텐츠가 차별적인 된 기능과 상관 관계가 지정 된 방식으로 변경 되는 것을 perceiving 사용자의 영향을 고려해 야 합니다.
* **기능의 의도**하지 않은 바이어스: 모집단의 하위 집합에만 영향을 주는 기능을 사용 하 여 도입할 수 있는 편향 유형이 있습니다. 이미지 분석을 사용하여 그림의 항목을 추출하거나 텍스트 분석을 사용하여 텍스트의 엔터티를 검색하는 경우와 같이 기능이 알고리즘 방식으로 생성되는 경우 특별히 주의해야 합니다. 이러한 기능을 만드는 데 사용하는 서비스의 특성을 알고 있어야 합니다.
...
* 사용자에 게 문의 하세요 .이 정보를 사용 하 여 콘텐츠를 개인 설정 하는 경우 사용자에 게 관심이 있나요? 특정 항목을 강조 표시하거나 숨기도록 결정한 방법을 보여 주는 것이 편할까요?
...
### <a name="reliability-and-safety"></a>신뢰성 및 안전성
*AI 시스템은 안정적이 고 안전 하 게 수행 해야*합니다. Personalizer를 사용하는 경우 다음과 같습니다.
...
*AI 시스템은 안전 하 고 개인 정보 취급 방침을 준수 해야*합니다. Personalizer를 사용하는 경우 다음과 같습니다.
...
* [향후 계산 된](https://news.microsoft.com/futurecomputed/) 1 월 2018 책에 게시 된 AI를 담당 하는 개발자를 위한 Microsoft의 6 가지 원칙을 참조 하세요.
