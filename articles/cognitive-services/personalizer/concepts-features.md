...
* _와_ 같은 `Sports_Shopper`사용자 사용자 사용자 개별 사용자 ID가 아니어야 합니다. 
* _콘텐츠_(예: 동영상이 `Documentary`, `Movie` 또는 `TV Series` 중 어느 유형인지, 또는 소매 항목이 점포에서 구입할 수 있는지 여부)
* _현재_ 기간(예: 요일)

Personalizer는 작업 및 컨텍스트에 대해 보낼 수 있는 기능을 규정, 제한 또는 수정하지 않습니다.
...
* 범주형 피처의 경우 가능한 값을 정의할 필요가 없으며 숫자 값에 대한 범위를 미리 정의할 필요가 없습니다.

## <a name="supported-feature-types"></a>지원되는 기능 형식

Personalizer는 문자열, 숫자 및 부울 형식의 기능을 지원합니다.

### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>기능 유형의 선택이 개인화에서 기계 학습에 미치는 영향

* **문자열**: 문자열 형식의 경우 키와 값의 모든 조합은 Personalizer 기계 학습 모델에서 새로운 가중치를 만듭니다. 
* **숫자**: 숫자가 개인 화 결과에 비례하여 영향을 줄 때 숫자 값을 사용해야 합니다. 이것은 매우 시나리오에 따라 다릅니다. 예를 들어 소매 경험을 개인화할 때 NumberOfPetsOwned는 2~3마리의 애완동물을 가진 사람들이 1마리의 애완동물을 갖는 것만큼 개인화 결과에 두 번 또는 세 번 영향을 미치기를 원할 수 있으므로 숫자인 기능이 될 수 있습니다. 숫자 단위를 기반으로 하지만 의미가 선형이 아닌 경우(예: 나이, 온도 또는 사람 높이)는 문자열로 인코딩하는 것이 가장 적합하며 일반적으로 범위를 사용하여 기능 품질을 향상시킬 수 있습니다. 예를 들어 나이는 "나이":"0-5", "나이":"6-10" 등으로 인코딩할 수 있습니다.
* "false"의 값으로 전송된 **부울** 값은 전혀 전송되지 않은 것처럼 작동합니다.
...
규칙이 유효한 JSON 키인 경우 자기만의 규칙에 따라 기능 네임스페이스의 이름을 지정할 수 있습니다. 네임스페이스는 피처를 별개의 집합으로 구성하고 비슷한 이름의 피처를 모호하게 하는 데 사용됩니다. 네임스페이스는 피처 이름에 추가되는 '접두사'로 생각할 수 있습니다. 네임스페이스는 중첩할 수 없습니다.


다음 JSON에서 `user`, `state` 및 `device`는 기능 네임스페이스입니다. 

> [!Note]
> 현재 UTF-8을 기반으로 하고 다른 문자로 시작하는 피쳐 네임스페이스에 이름을 사용하는 것이 좋습니다. 예를 들어 `user` `state`, `device` 및 `u`에서 `s`를 `d`시작하고 을 시작합니다. 현재 첫 번째 문자가 같은 네임스페이스를 사용하면 기계 학습에 사용되는 인덱스가 충돌할 수 있습니다.
...
### <a name="restrictions-in-character-sets-for-namespaces"></a>네임스페이스에 대한 문자 집합 제한

네임스페이스 이름을 지정하는 데 사용하는 문자열은 몇 가지 제한 사항을 따라야 합니다. 
* 유니코드일 수 없습니다.
* 네임스페이스 이름에 는 코드 < 256과 함께 인쇄 가능한 기호 중 일부를 사용할 수 있습니다. 
* 코드 < 32(인쇄 불가), 32(공간), 58(콜론), 124(파이프) 및 126-140과 < 기호는 사용할 수 없습니다.
* 밑줄 "_"로 시작하거나 기능이 무시됩니다.
...
위치 정보는 일반적으로 더 광범위한 분류를 만들면 이점을 얻을 수 있습니다. 예를 들어 위도-경도 좌표:47.67402° N, Long: 122.12154° W가 너무 정확하고 모델이 위도와 경도를 고유한 치수로 학습하도록 강제합니다. 위치 정보를 기반으로 개인 설정하려고 할 때 더 큰 섹터의 위치 정보를 그룹화하는 데 도움이 됩니다. 이를 쉽게 수행할 수 있는 방법은 Lat-Long 숫자에 적합한 반올림 정밀도를 선택하고 위도와 경도를 하나의 문자열로 만들어 "영역"으로 결합하는 것입니다. 예를 들어, 47.67402° N, Long: 약 몇 킬로미터 너비의 지역에서 122.12154° W를 나타내는 좋은 방법은 "location":"34.3, 12.1"입니다.


#### <a name="expand-feature-sets-with-extrapolated-information"></a>추정한 정보를 사용하여 기능 세트 확장

이미 가지고 있는 정보에서 추정할 수 있는 암묵적 특성을 생각하여 더 많은 기능을 얻을 수도 있습니다. 예를 들어 가상의 영화 목록 개인 정보화에서 주말 대 평일 은 사용자와 다른 행동을 유도할 수 있습니까? 시간은 "주말" 또는 "평일" 특성을 갖도록 확장할 수 있습니다. 국경일에는 특정 동영상 유형에 주의를 기울이게 될까요? 예를 들어 "핼러윈" 특성은 핼러윈 행사와 관련된 장소에서 유용합니다. 비가 오는 날씨는 많은 사람의 동영상 선택에 크게 영향을 미칠 수 있습니까? 기상 서비스에서 시간 및 장소와 함께 해당 정보를 제공하면 사용자는 해당 서비스를 특별한 기능으로 추가할 수 있습니다. 
...
* [컴퓨터 비전](../computer-vision/home.md)

## <a name="actions-represent-a-list-of-options"></a>작업은 옵션 목록을 나타냅니다.

각 작업:

* _이벤트_ ID가 있습니다. 이벤트 ID가 이미 있는 경우 이를 제출해야 합니다. 이벤트 ID가 없는 경우 보내지 말고, Personalizer는 사용자를 위해 이벤트를 만들고 Rank 요청에 대한 응답으로 반환합니다. ID는 사용자가 아닌 Rank 이벤트와 연결됩니다. ID를 만들 면 GUID가 가장 잘 작동합니다. 
* 기능 목록을 갖습니다.
