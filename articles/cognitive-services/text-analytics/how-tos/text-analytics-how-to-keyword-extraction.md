---
title: 텍스트 분석 REST API를 사용한 핵심 구 추출
titleSuffix: Azure Cognitive Services
description: Azure 코그너티브 서비스에서 텍스트 분석 REST API를 사용하여 주요 구를 추출하는 방법.
...
---
# <a name="example-how-to-extract-key-phrases-using-text-analytics"></a>예: 텍스트 분석을 사용하여 핵심 구를 추출하는 방법
...
자세한 내용은 [지원되는 언어를](../text-analytics-supported-languages.md)참조하십시오.
...
문서 크기는 문서당 5,120자 이하여야 하며 컬렉션당 최대 1,000개의 항목(아이디)을 가질 수 있습니다. 컬렉션은 요청 본문에 제출됩니다. 다음 예제는 핵심 구 추출에 제출할 수 있는 콘텐츠에 대한 설명입니다.
...
요청 정의에 대한 자세한 내용은 [텍스트 분석 API를 호출하는 방법을](text-analytics-how-to-call-api.md)참조하십시오. 편의상 다음 사항을 다시 설명합니다.

+ **POST** 요청을 만듭니다. 이 요청에 대한 API 설명서를 검토합니다: [키 구 API](https://westcentralus.dev.cognitive.microsoft.com/docs/services/TextAnalytics-v2-1/operations/56f30ceeeda5650db055a3c6).

+ Azure의 텍스트 분석 리소스 또는 인스턴스화된 [텍스트 분석 컨테이너를](text-analytics-how-to-install-containers.md)사용하여 핵심 구 추출에 대한 HTTP 끝점을 설정합니다. URL에 `/text/analytics/v2.1/keyPhrases`를 포함해야 합니다. 예: `https://<your-custom-subdomain>.api.cognitiveservices.azure.com/text/analytics/v2.1/keyPhrases`
...
요청을 받으면 분석이 수행됩니다. 분당 또는 초당 보낼 수 있는 요청의 크기와 수에 대한 자세한 내용은 개요의 [데이터 제한](../overview.md#data-limits) 섹션을 참조하십시오.

상태 비저장 서비스임에 유의하세요. 계정에는 아무 데이터도 저장되지 않습니다. 결과는 응답으로 즉시 반환됩니다.

## <a name="step-3-view-results"></a>3단계: 결과 보기

모든 POST 요청에서는 ID 및 감지된 속성이 있는 JSON 형식의 응답을 반환합니다. 반환된 키 구의 순서는 모델에 의해 내부적으로 결정됩니다.
...
앞서 언급했듯이 분석기는 필수적이지 않은 단어를 찾아 버리고 문장의 주체 또는 개체로 보이는 단일 용어 나 구를 유지합니다.

## <a name="summary"></a>요약

이 문서에서는 코그너티브 서비스에서 텍스트 분석을 사용하여 핵심 구 추출에 대한 개념및 워크플로를 배웠습니다. 요약하면 다음과 같습니다.
