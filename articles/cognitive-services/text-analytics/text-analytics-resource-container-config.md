...
`ApiKey` 설정은 컨테이너에 대한 청구 정보를 추적하는 데 사용되는 Azure 리소스 키를 지정합니다. ApiKey에 대한 값을 지정해야 하며 이 값은[ 구성 설정에 대해 지정된 `Billing`Text Analytics](#billing-configuration-setting) 리소스에 대한 유효한 키여야 합니다.
