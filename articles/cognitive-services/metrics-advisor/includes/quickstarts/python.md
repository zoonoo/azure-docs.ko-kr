...
| [메트릭](https://azuresdkdocs.blob.core.windows.net/$web/python/azure-ai-metricsadvisor/latest/azure.ai.metricsadvisor.models.html?highlight=metric#azure.ai.metricsadvisor.models.Metric) | `Metric`은 특정 비즈니스 프로세스의 상태를 모니터링하고 평가하는 데 사용되는 정량 측정값입니다. 차원으로 나눈 여러 개의 시계열 값의 조합일 수 있습니다. 예를 들어 웹 상태 메트릭은 사용자 수 및 en-us 시장의 차원을 포함할 수 있습니다. |
...
새 메서드에서 아래 예제와 같이 import 문을 만듭니다. `sql_server_connection_string`을 사용자 고유의 SQL 서버 연결 문자열로 바꾸고 `query`를 단일 타임스탬프에서 데이터를 반환하는 쿼리로 바꿉니다. 사용자 지정 데이터를 기반으로 `metric` 및 `dimension` 값도 조정해야 합니다.
...
새 메서드에서 아래 예제와 같이 import 문을 만듭니다. `data_feed_id`를 사용자가 만든 데이터 피드의 ID로 바꿉니다. 키와 엔드포인트를 사용하여 클라이언트를 만들고 `client.get_data_feed_ingestion_progress()`를 사용하여 수집 진행률을 가져옵니다. 마지막 활성 및 성공한 타임스탬프와 같은 세부 정보를 출력합니다.
...
새 메서드에서 아래 예제와 같이 import 문을 만듭니다. `metric_id`를 구성하려는 메트릭의 ID로 바꿉니다. 키 및 엔드포인트를 사용하여 클라이언트를 만들고 `client.create_metric_anomaly_detection_configuration`을 사용하여 새 검색 구성을 만듭니다. 임계값 조건은 변칙 검색에 대한 매개 변수를 지정합니다.
...
새 메서드에서 아래 예제와 같이 import 문을 만듭니다. `anomaly_detection_configuration_id`를 변칙 검색 구성의 ID로 바꾸고 `hook_id`를 이전에 만든 후크로 바꿉니다. 키와 엔드포인트를 사용하여 클라이언트를 만들고 `client.create_anomaly_alert_configuration()`을 사용하여 경고 구성을 만듭니다. `metric_alert_configurations`는 각 구성의 조건과 범위를 지정하는 `MetricAlertConfiguration` 개체의 목록입니다.
...
새 메서드에서 아래 예제와 같이 import 문을 만듭니다. `alert_id`를 경고 ID로 바꾸고 `alert_config_id`를 경고 구성 ID로 바꿉니다. 키와 엔드포인트를 사용하여 클라이언트를 만들고 `client.list_anomalies_for_alert()`를 사용하여 경고 구성을 나열합니다. 
