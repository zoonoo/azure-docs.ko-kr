---
title: 변칙 탐지기 API를 사용하기 위한 컨테이너를 설치하고 실행하는 방법
...
ms.locfileid: "79221394"
---
# <a name="install-and-run-anomaly-detector-containers-preview"></a>변칙 검출기 컨테이너 설치 및 실행(미리 보기)

변칙 검출기에는 다음과 같은 컨테이너 기능 기능이 있습니다.

| 함수 | 기능 |
|--|--|
| Anomaly Detector | <li> 실시간으로 발생하는 이상 징후를 감지합니다. <li> 데이터 집합 전체에서 일괄 처리로 이상 징후를 검색합니다. <li> 데이터의 예상 정상 범위를 유추합니다. <li> 데이터에 더 잘 맞도록 변칙 검색 감도 조정을 지원합니다. |

API에 대한 자세한 내용은 다음을 참조하십시오.
...
변칙 검출기 컨테이너를 사용하기 전에 다음 필수 구성 조건을 충족해야 합니다.
...
|변칙 탐지기 리소스 |이러한 컨테이너를 사용하려면 다음이 있어야 합니다.<br><br>연결된 API 키 및 끝점 URI를 얻기 위한 Azure _변칙 검색기_ 리소스입니다. 두 값 모두 Azure 포털의 **변칙 검색기** 개요 및 키 페이지에서 사용할 수 있으며 컨테이너를 시작하는 데 필요합니다.<br><br>**{API_KEY}**: **키** 페이지에서 사용 가능한 두 리소스 키 중 하나<br><br>**{ENDPOINT_URI}**: **개요** 페이지에 제공된 끝점|
...
다음 표는 변칙 검출기 컨테이너에 할당할 최소 및 권장 CPU 코어 및 메모리에 대해 설명합니다.

| QPS(초당 쿼리) | 최소 | 권장 |
|-----------|---------|-------------|
| 10 QPS | 4코어, 1GB 메모리 | 8 코어 2GB 메모리 |
| 20 QPS | 8코어, 2GB 메모리 | 16 코어 4GB 메모리 |
...
### <a name="docker-pull-for-the-anomaly-detector-container"></a>변칙 검출기 컨테이너에 대한 도커 풀
...
`<container-registry>`와 `<container-name>`을 사용하는 컨테이너의 값으로 대체합니다. 동일한 컨테이너일 필요는 없습니다. 이상 징후 검출기 컨테이너와 LUIS 컨테이너가 호스트에서 함께 실행중이거나 여러 변칙 검출기 컨테이너가 실행중일 수 있습니다. 
...
변칙 탐지기 컨테이너는 Azure 계정의 _변칙 탐지기_ 리소스를 사용하여 Azure에 청구 정보를 보냅니다. 
...
이 문서에서는 변칙 탐지기 컨테이너를 다운로드, 설치 및 실행하기 위한 개념 및 워크플로를 배웠습니다. 요약하면 다음과 같습니다.

* 변칙 검출기는 Docker용 리눅스 컨테이너 하나를 제공하여 일괄 처리 대 스트리밍, 예상 범위 추론 및 감도 조정으로 변칙 검색을 캡슐화합니다.
* 컨테이너 이미지는 컨테이너 미리 보기 전용 개인 Azure 컨테이너 레지스트리에서 다운로드됩니다.
* 컨테이너 이미지는 Docker에서 실행됩니다.
* REST API 또는 SDK를 사용하여 컨테이너의 호스트 URI를 지정하여 변칙 탐지기 컨테이너에서 작업을 호출할 수 있습니다.
* 컨테이너를 인스턴스화할 때 청구 정보를 지정해야 합니다.

> [!IMPORTANT]
> Cognitive Services 컨테이너는 측광을 위해 Azure에 연결되지 않은 상태에서 실행할 수 있는 권한이 없습니다. 고객은 컨테이너에서 항상 계량 서비스와 청구 정보를 통신할 수 있도록 설정해야 합니다. 코그너티브 서비스 컨테이너는 고객 데이터(예: 분석 중인 타임시리즈 데이터)를 Microsoft로 전송하지 않습니다.
