...
1. 코드를 `.go` 확장명의 파일로 저장합니다. 예들 들어 `analyze-image.go`입니다.
1. 명령 프롬프트 창을 엽니다.
1. 프롬프트에서 `go build` 명령을 실행하여 파일에서 패키지를 컴파일합니다. 예들 들어 `go build analyze-image.go`입니다.
1. 프롬프트에서 컴파일된 패키지를 실행합니다. 예들 들어 `analyze-image`입니다.
