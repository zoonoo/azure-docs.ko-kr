...
읽기 작업은 출력에 인식된 단어의 원래 줄 그룹 유지합니다. 각 줄에는 경계 상자 좌표가 있고 줄 안의 각 단어에도 자체 좌표가 있습니다. 낮은 신뢰도로 단어가 인식된 경우 해당 정보도 전달됩니다. 자세히 알아보려면 [api 참조 문서 읽기](https://westus.dev.cognitive.microsoft.com/docs/services/5adf991815e1060e6355ad44/operations/2afb498089f74080d7ef85eb) 및 [api 미리 보기 참조 읽기 문서](https://go.microsoft.com/fwlink/?linkid=2118322) 를 참조 하세요.
...
Computer Vision의 OCR(광학 문자 인식) API는 읽기 API와 유사하지만 동기적으로 실행되고 큰 문서에 대해 최적화되지 않습니다. 이전 인식 모델을 사용 하지만 더 많은 언어에서 작동 합니다. 지원 되는 언어의 전체 목록은 [언어 지원](language-support.md#text-recognition) 을 참조 하세요.
...
텍스트가 많은 사진에서는 부분적으로 인식된 단어에서 가양성이 발생할 수 있습니다. 일부 사진, 특히 텍스트가 없는 사진에서는 이미지의 유형에 따라 정밀도가 달라질 수 있습니다.
...
텍스트 인식 API는 OCR과 비슷하지만 비동기적으로 실행되고 업데이트된 인식 모델을 사용합니다. 자세히 알아보려면 [텍스트 인식 API 참조 문서](https://westus.dev.cognitive.microsoft.com/docs/services/5adf991815e1060e6355ad44/operations/587f2c6a154055056008f200) 를 참조 하세요.
