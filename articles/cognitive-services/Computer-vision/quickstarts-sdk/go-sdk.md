...
Go용 Computer Vision 클라이언트 라이브러리를 시작합니다. 이러한 단계에 따라 패키지를 설치하고 기본 작업을 위한 예제 코드를 사용해 봅니다. Computer Vision은 이미지를 처리하고 정보를 반환하는 고급 알고리즘에 대한 액세스를 제공합니다.

Go용 Computer Vision 클라이언트 라이브러리를 사용하여 다음을 수행합니다.
