...
합니다 `ApiKey` 설정은 Azure `Cognitive Services` 컨테이너에 대 한 청구 정보를 추적 하는 데 사용 되는 리소스 키입니다. ApiKey에 대 한 값을 지정 해야 하며 값에 대 한 유효한 키 여야 합니다.는 _Cognitive Services_ 에 대 한 지정 된 리소스는 [ `Billing` ](#billing-configuration-setting) 구성 설정입니다.
...
추가 해야 합니다 `vision/v1.0` 다음 표에 나와 있는 것 처럼 끝점 URI로 라우팅할 합니다. 
