...
이 자습서에서는 Azure Logic Apps에서 Azure Cognitive Services 제품군에 속한 서비스인 Form Recognizer를 사용하여 청구서로부터 데이터를 추출하는 워크플로를 만듭니다. Form Recognizer를 사용하여 먼저 샘플 데이터 세트를 통해 모델을 학습시킨 다음, 다른 데이터 세트를 통해 모델을 테스트합니다. 이 자습서에서 사용되는 샘플 데이터는 Azure Storage Blob 컨테이너에 저장됩니다.
...
모델을 학습시키고 테스트하는 데 사용하는 샘플 데이터 세트는 [GitHub](https://go.microsoft.com/fwlink/?linkid=2090451)에서 .zip 파일로 제공됩니다. .zip 파일을 다운로드하여 압축을 풀고, **/Train** 폴더 아래에 있는 청구서 PDF 파일을 엽니다. 청구서 번호, 청구서 날짜 등이 포함된 테이블의 모양을 확인해 보세요. 

> [!div class="mx-imgBorder"]
> ![샘플 청구서](media/tutorial-form-recognizer-with-logic-apps/sample-receipt.png)

이 자습서에서는 Azure Logic Apps와 Form Recognizer를 사용하여 만든 워크플로에서 이러한 테이블의 정보를 JSON 형식으로 추출하는 방법에 대해 알아봅니다.
...
컨테이너의 URL을 복사합니다. 이는 자습서의 뒷부분에서 필요합니다. 이 자습서에 나열된 것과 동일한 이름을 사용하여 스토리지 계정과 컨테이너를 만든 경우 URL은 *https:\//formrecostorage.blob.core.windows.net/formrecocontainer/* 입니다.
...
이제 시작해 보겠습니다! 다음 단계에 따라 워크플로를 설정합니다.
...
이 자습서에서는 첨부된 청구서가 포함된 이메일을 받으면 워크플로를 트리거합니다. 이 자습서의 경우 Office 365를 이메일 서비스로 선택하지만 사용하려는 다른 이메일 공급자를 사용할 수도 있습니다.
...
1. **양식 분석** 대화 상자에서 다음을 수행합니다.
    ...
    2. **문서** 텍스트 상자를 클릭하고, 열리는 대화 상자의 **동적 콘텐츠** 탭 아래에서 **첨부 파일 콘텐츠**를 선택합니다. 이렇게 하면 워크플로를 트리거하기 위해 보내진 이메일에 첨부된 샘플 청구서 파일을 사용하도록 흐름을 구성합니다.
...
논리 앱을 테스트하려면 [GitHub](https://go.microsoft.com/fwlink/?linkid=2090451)에서 다운로드한 샘플 데이터 세트의 **/Test** 폴더에 있는 샘플 청구서를 사용합니다. 다음 단계를 수행합니다.
