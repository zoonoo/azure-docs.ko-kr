...
샘플 레이블 지정 도구에서 모델을 작성하려면 왼쪽에 있는 모델 작성(병합 화살표) 아이콘을 클릭합니다. 왼쪽에서 함께 작성할 모델을 선택합니다. 화살표 아이콘이 있는 모델은 이미 작성된 모델입니다.
"작성" 단추를 클릭합니다. 팝업에서 새로 작성된 모델의 이름을 지정하고 "작성"을 클릭합니다. 작업이 완료되면 새로 작성된 모델이 목록에 표시됩니다.
