...
- **향상 된 테이블 추출** -이제 강력한 OCR (광학 문자 인식) 기능과 심층 학습 테이블 추출 모델을 결합 하는 향상 된 테이블 추출을 제공 합니다. 양식 인식기는 병합 된 열, 행, 테두리 없음 등을 포함 하는 복잡 한 테이블을 포함 하 여 테이블에서 데이터를 추출할 수 있습니다. 
  ...
  :::image type="content" source="./media/tables-example.jpg" alt-text="테이블 예" lightbox="./media/tables-example.jpg":::

 
  > [레이아웃 추출에 대 한 자세한 정보](concept-layout.md)

- **클라이언트 라이브러리 업데이트** -.Net, Python, Java 및 JavaScript 용 [클라이언트 라이브러리](quickstarts/client-library.md) 는 일반 공급으로 전환 되었습니다.
