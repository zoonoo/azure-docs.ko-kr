...
1. 필수 청구 설정을 사용하여 [컨테이너를 실행](#run-the-container-by-using-the-docker-run-command)합니다. [ 명령의 자세한 ](form-recognizer-container-configuration.md#example-docker-run-commands)예`docker run`를 사용할 수 있습니다.
...
[Docker 실행](https://docs.docker.com/engine/reference/commandline/run/) 명령을 사용하여 컨테이너를 실행합니다. [, ](#gathering-required-parameters), `{COMPUTER_VISION_ENDPOINT_URI}` 및 `{COMPUTER_VISION_API_KEY}` 값을 가져오는 방법에 대한 자세한 내용은 `{FORM_RECOGNIZER_ENDPOINT_URI}`필수 매개 변수 수집`{FORM_RECOGNIZER_API_KEY}`을 참조하세요.

[명령의 ](form-recognizer-container-configuration.md#example-docker-run-commands)예`docker run`를 사용할 수 있습니다.
