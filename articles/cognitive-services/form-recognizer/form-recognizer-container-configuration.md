...
명령 인수를 사용 하 여 폼 인식기 컨테이너 런타임 환경을 구성 합니다 `docker run` . 이 컨테이너에는 몇 가지 필수 설정과 몇 가지 선택적 설정이 있습니다. 몇 가지 예제는 ["예제 docker run 명령"](#example-docker-run-commands) 섹션을 참조 하세요. 청구 설정은 컨테이너별로 다릅니다.
...
> [!IMPORTANT]
> [`ApiKey`](#apikey-configuration-setting), [`Billing`](#billing-configuration-setting) 및 [`Eula`](#eula-setting) 설정이 함께 사용 됩니다. 세 가지 설정 모두에 대해 유효한 값을 제공 해야 합니다. 그렇지 않으면 컨테이너가 시작 되지 않습니다. 이러한 구성 설정을 사용하여 컨테이너를 인스턴스화하는 방법에 대한 자세한 내용은 [청구](form-recognizer-container-howto.md#billing)를 참조하세요.
...
바인딩 탑재를 사용하여 컨테이너에서 또는 컨테이너로 읽고 씁니다. `--mount` [ `docker run` 명령](https://docs.docker.com/engine/reference/commandline/run/)에서 옵션을 지정 하 여 입력 탑재 또는 출력 탑재를 지정할 수 있습니다.
...
| **{COMPUTER_VISION_ENDPOINT_URI}** | 청구 엔드포인트입니다. 클라우드 기반 Computer Vision 리소스를 사용 하는 경우 URI 값은 Azure Portal Computer Vision API 개요 페이지에서 사용할 수 있습니다. *인식 서비스-인식-텍스트* 컨테이너를 사용 하는 경우 명령에서 컨테이너에 전달 되는 청구 끝점 URL을 사용 `docker run` 합니다. |
...
> [!IMPORTANT]
> 컨테이너를 실행 하려면, 및 옵션을 지정 합니다. `Eula` `Billing` `ApiKey` 그렇지 않으면 컨테이너가 시작 되지 않습니다. 자세한 내용은 [Billing](#billing-configuration-setting)를 참조하세요.
