...
요청 본문에 다음 콘텐츠를 추가 하면 API가 하위 폴더에 있는 문서를 학습 합니다. `"prefix"` 필드는 선택 사항이 며, 지정 된 문자열로 시작 하는 경로가 포함 된 파일로 학습 데이터 집합을 제한 합니다. 따라서 예를 들어 `"Test"`값을 사용 하면 API가 "Test" 라는 단어로 시작 되는 파일이 나 폴더만 볼 수 있습니다.
...
* [REST API 및 Python을 사용 하 여 레이블로 학습](./quickstarts/python-labeled-data.md)