---
title: 데모 Conversation Learner 응용 프로그램, 가상 현실 앱 시작 관리자 - Microsoft Cognitive Services | Microsoft Docs
titleSuffix: Azure
...
이 데모에서는 “start Skype and put in on the wall” 등의 명령을 지원하는 가상 현실 응용 프로그램 시작 관리자를 보여 줍니다. 앱을 시작하려면 사용자가 앱 이름과 위치를 말해야 합니다. 앱 시작은 API 호출을 통해 처리됩니다. 사용자로부터 앱 이름이 인식되면 entityDetectionCallback은 요청된 앱이 설치된 앱 목록에 있는 하나 이상의 앱과 일치하는지 확인합니다. 요청된 앱이 설치되어 있지 않고 앱 이름이 모호한(둘 이상의 설치된 앱과 일치함) 경우를 처리합니다.
...
Web UI의 앱 목록에서 VRAppLauncher를 클릭합니다. 
...
7. 시스템: ‘starting outlook on the wall’.
    - 이로 인해 API 호출이 트리거되었습니다. 이 호출의 코드는 C:\<\installedpath>\src\demos\demoVRAppLauncher.ts에 있습니다. 그러나 이 데모를 위해 Outlook을 시작하는 논리가 실제로는 없습니다.
...
1. ‘start OneNote’를 입력합니다. 
    - 코드에 정의된 EntityDetectionCallback이 사용자가 입력한 이름을 사용하고 코드에 정의된 앱 목록과 일치하여 앱 이름으로 확인하므로 앱 이름으로 인식됩니다. 그런 다음, 일치하는 모든 앱 집합이 반환됩니다. 
    - 일치 항목 목록이 0이면 앱이 설치되지 않은 것입니다. unknownAppName에 배치됩니다.
    - 앱을 두 개 이상 찾은 경우 DisambigAppNames에 복사하고 AppName 엔터티를 지웁니다.
2. Score Action(작업에 점수 지정)을 클릭합니다.
3. ‘Sorry, I don't know the app $UknownAppName’을 클릭하여 선택합니다.
4. ‘start amazon’을 입력합니다. 다른 경로를 사용해 보겠습니다.
5. Score Actions(작업에 점수 지정)를 클릭합니다.
    - 이제 Amazon 비디오 및 Amazon 음악이 DisambigAppNames 메모리에 있습니다. 또한 OneNote가 지워졌습니다.
3. ‘There are few apps that sound like that...’을 클릭하여 선택합니다.
    - 이 시점까지 보유한 학습 대화가 몇 개밖에 되지 않으므로 점수는 별로 높지 않습니다. 모델을 보다 명확하게 만들기 위해 여기에 몇 개가 더 필요한 것 같습니다.
2. Score Actions(작업에 점수 지정)를 입력합니다.
4. 학습 완료를 클릭합니다.

지금까지 엔터티 확인을 수행하는 방법을 살펴보았습니다. 데모에서는 API 콜백을 설명하고 정보 수집, 현재 상태 및 모호성 확인, 그에 따른 올바른 작업 수행을 위한 템플릿을 보여 주었습니다.
