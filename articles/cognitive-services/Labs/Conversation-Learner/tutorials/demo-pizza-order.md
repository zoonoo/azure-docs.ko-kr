---
title: 데모 Conversation Learner 응용 프로그램, 피자 주문 - Microsoft Cognitive Services | Microsoft Docs
titleSuffix: Azure
...
- Toppings: 사용자가 요청한 토핑을 누적합니다. 재고가 있는 유효한 토핑을 포함합니다. 토핑에 재고가 있는지를 확인합니다.
- OutofStock: 사용자에게 선택한 토핑에 재고가 없다는 것을 알리는 데 사용됩니다.
...
5. 작업에 점수 지정을 클릭합니다.
    - 버섯과 치즈는 토핑의 메모리에 포함되지 않았습니다.
3. '피자에 $Toppings이 있습니다'를 선택하도록 클릭합니다.
    - 이것은 비 대기 작업이므로 봇이 다음 작업을 요청합니다.
6. '추가로 주문하시겠습니까?'를 선택합니다.
7. '버섯 빼고 피망 추가'를 입력합니다.
    - **버섯**에는 옆에 '-' 기호가 있어서 제거할 수 있습니다. 피망에는 토핑을 추가하는 '+'가 있습니다.
2. 작업에 점수 지정을 클릭합니다.
    - **피망**은 새로운 항목이라 굵게 표시되었습니다. 또한 **버섯**은 엑스 표시되었습니다.
8. '피자에 $Toppings이 있습니다'를 선택하도록 클릭합니다.
6. '추가로 주문하시겠습니까?'를 선택합니다.
7. '콩 추가'를 입력합니다.
    - 콩은 제고가 없는 토핑의 예제입니다. 토핑으로 여전히 레이블이 지정됩니다.
2. 작업에 점수 지정을 클릭합니다.
    - 콩은 OutOfStock으로 표시됩니다.
    - 이러한 발생을 확인하려면 C:\<\installedpath>\src\demos\demoPizzaOrder.ts에 있는 코드를 열어보겠습니다. 또한 EntityDetectionCallback 메서드를 기록해 둡니다. 토핑에 재고가 있는지 확인하기 위해 각 토핑 이후에 이 메서드가 호출됩니다. 그렇지 않으면 토핑 집합에서 지우고 OutOfStock 엔터티에 추가합니다. 재고 토핑 목록이 있는 해당 메서드 위에 inStock 변수가 정의됩니다.
...
9. 작업에 점수 지정을 클릭합니다.
    - 치즈와 피망은 최근 주문의 토핑으로 메모리에 포함됩니다.
