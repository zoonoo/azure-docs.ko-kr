...
Translator 글로벌 엔드포인트가 `endpoint`로 설정되어 있습니다. `path`는 `transliterate` 루트를 설정하며 API의 버전 3을 실행하기 원한다는 것을 식별합니다.

`params`는 입력 언어와 입력 및 출력 스크립트를 설정하는 데 사용됩니다. 이 샘플에서는 일본어를 라틴어 알파벳으로 음역합니다.

>[!NOTE]
> 엔드포인트, 경로 및 요청 매개 변수에 대한 자세한 내용은 [Translator 3.0: Transliterate](https://docs.microsoft.com/azure/cognitive-services/translator/reference/v3-0-transliterate)를 참조하세요.
