...
Microsoft Translator에 대한 요청은 대부분 요청이 시작된 위치와 가장 가까운 데이터 센터에서 처리됩니다. 데이터 센터 오류가 발생한 경우 요청은 Azure 지역 외부로 라우팅될 수 있습니다.

요청을 특정 Azure 지역에서 처리되도록 하려면 API 요청의 글로벌 엔드포인트를 원하는 지역 엔드포인트로 변경합니다.

|Description|Azure 지역|기준 URL(지리적 엔드포인트)|
|:--|:--|:--|
...
Translator를 호출하는 데 필요한 헤더는 2개가 있습니다.
...
다중 서비스 비밀 키를 사용하는 경우 요청에 두 개의 인증 헤더를 포함시켜야 합니다. Translator를 호출하는 데 필요한 헤더는 2개가 있습니다.
...
이제 Azure 퍼블릭 클라우드의 모든 지역에서 Virtual Network(VNET) 기능을 사용하여 Translator 서비스를 사용할 수 있습니다. Virtual Network를 사용하도록 설정하려면 [Azure Cognitive Services 가상 네트워크 구성](../../cognitive-services-virtual-networks.md?tabs=portal)을 참조하세요. 
...
| 408001| 요청된 번역 시스템이 준비되고 있습니다. 잠시 후에 다시 시도하세요.|
