...
description: Translator v 2.0에 대 한 참조 설명서입니다.
...
> [!IMPORTANT]
> 이 버전의 변환기는 더 이상 사용 되지 않습니다. [변환기 버전 3에 대 한 설명서를 봅니다](v3-0-reference.md).
...
- 액세스 토큰입니다. 인증 서비스에 대 한 POST 요청을 수행 하 여 액세스 토큰을 만들려면 구독 키를 사용 합니다. 자세한 내용은 토큰 서비스 설명서를 참조하세요. `Authorization`헤더 또는 쿼리 매개 변수를 사용 하 여 액세스 토큰을 번역기 서비스로 전달 합니다 `access_token` . 액세스 토큰은 10분 동안 유효합니다. 10 분 마다 새 액세스 토큰을 획득 하 고 10 분 동안 반복 된 요청에 대해 동일한 액세스 토큰을 계속 사용 합니다.
- 직접 사용 되는 구독 키입니다. 구독 키를 요청에 포함 된 헤더의 값으로 `Ocp-Apim-Subscription-Key` 변환기에 전달 합니다. 구독 키를 직접 사용 하는 경우에는 토큰 인증 서비스를 호출 하 여 액세스 토큰을 만들 필요가 없습니다.
...
원본 텍스트에 있는 경우에도 변환의 사용 방지를 방지 하려는 경우이를 지 원하는 메서드에 대 한 사용 방지 필터링 옵션을 사용할 수 있습니다. 옵션을 사용 하면 사용 되지 않는 사용 금지를 표시할지 아니면 적절 한 태그로 표시할지 선택할 수 있으며, 대상에서 사용 금지를 허용할지 여부를 선택할 수 있습니다. 허용 되는 값 `ProfanityAction` 은 `NoAction` (기본값), `Marked` 및 `Deleted` 입니다.
...
HTML ()과 같은 태그를 사용 하 여 콘텐츠를 변환 하 `contentType=text/html` 는 경우 번역에서 특정 콘텐츠를 제외 하는 것이 유용할 수 있습니다. 특성 `class=notranslate`를 사용하여 원래 언어로 유지해야 하는 콘텐츠를 지정할 수 있습니다. 다음 예제에서는 첫 번째 `div` 요소의 내용이 변환 되지 않지만 두 번째 `div` 요소의 내용이 변환 됩니다.
...
이전에를 사용 하 여 `AddTranslation` `AddTranslationArray` 동일한 원본 문장에 대해 5 이상의 등급이 있는 번역을 입력 하는 경우는 `Translate` 시스템에 사용할 수 있는 최상위 선택만 반환 합니다. "동일 원본 문장"은 문장의 끝에 대문자, 공백, 태그 값 및 문장 부호를 제외 하 고 정확히 동일한 (100% 일치)를 의미 합니다. 등급이 5 이상인 등급이 저장 되지 않은 경우 반환 되는 결과는 Microsoft Translator에서 자동으로 변환 됩니다.
...
|401    |자격 증명이 잘못 되었습니다.|
|500    |서버 오류 오류가 지속되면 알려주세요. 요청에 대 한 대략적인 날짜 & 시간을 제공 하 고, 응답 헤더에 요청 ID를 포함 해 주세요 `X-MS-Trans-Info` .|
...
* `AppId`: 필수 사항입니다. `Authorization`또는 헤더를 `Ocp-Apim-Subscription-Key` 사용 하는 경우 필드를 `AppId` 비워 둡니다. 그렇지 않으면를 포함 하는 문자열을 포함 `"Bearer" + " " + "access_token"` 합니다.
...
선택적 요소는 생략할 수 있습니다. 의 직계 자식인 요소는 `TranslateArrayRequest` 사전순으로 나열 되어야 합니다.

`TranslateArray`메서드는 `application/xml` 또는을 허용 `text/xml` `Content-Type` 합니다.
...
### <a name="response-class-status-200"></a>Response 클래스 (상태 200)
성공적인 응답은 `TranslateArrayResponse` 앞에서 설명한 형식으로 배열 배열을 포함 합니다.
...
### <a name="implementation-notes"></a>구현 참고 사항
전달 된 언어로 지역화 된 매개 변수로 전달 된 언어의 이름을 검색 `languageCodes` `locale` 합니다.
...
번역 메모리에 번역 배열을 추가 합니다. 이 메서드는의 배열 버전입니다 `AddTranslation` .
...
* `Translations`: 필수 사항입니다. 번역 메모리에 추가할 번역의 배열입니다. 각 번역 `OriginalText` 에는, 및가 포함 되어야 합니다 `TranslatedText` `Rating` . 각 및의 최대 크기 `OriginalText` 는 `TranslatedText` 1000 자입니다. 모든 `OriginalText` 및 요소의 합계는 `TranslatedText` 1만 자를 초과할 수 없습니다. 배열 요소의 최대 개수는 100입니다.
...
### <a name="implementation-notes"></a>구현 참고 사항
저장소 및 MT 엔진에서 지정된 언어 쌍에 대한 번역 배열을 검색합니다. `GetTranslations``Translate`는 사용 가능한 모든 번역을 반환 한다는 점에서와 다릅니다.

요청 URI는 `https://api.microsofttranslator.com/V2/Http.svc/GetTranslations`입니다.

요청의 본문에는 `TranslationOptions` 다음과 같은 형식의 선택적 개체가 포함 됩니다.
...
* `Translations`: 개체에 저장 된 일치 하는 항목의 배열 `TranslationMatch` 입니다 (다음 섹션에 설명 되어 있음). 번역에는 원래 텍스트의 약간 변형 (유사 항목 일치)이 포함 될 수 있습니다. 번역이 정렬 됩니다. 100% 일치 항목: 다음에 유사 항목 일치 항목이 있습니다.
* `From`: 메서드가 언어를 지정 하지 않는 경우 `From` 이 값은 자동 언어 검색에서 제공 됩니다. 그렇지 않으면 지정 된 `From` 언어가 됩니다.
* `State`: 요청 및 응답의 상관 관계를 관리할 수 있는 사용자 상태입니다. 매개 변수에 제공 된 값을 포함 합니다 `TranslateOptions` .
...
### <a name="response-class-status-200"></a>Response 클래스 (상태 200)
`GetTranslationsResponse`앞에서 설명한 형식의 개체입니다.
...
선택적 요소는 생략할 수 있습니다. 의 직계 자식인 요소는 `GetTranslationsArrayRequest` 사전순으로 나열 되어야 합니다.
...
`TranslationMatch`개체에는 다음 값이 포함 됩니다.
* `Error`: 특정 입력 문자열에 대 한 오류가 발생 하는 경우 오류 코드입니다. 그렇지 않으면이 필드는 비어 있습니다.
* `MatchDegree`: 입력 텍스트가 저장소에서 찾은 원래 텍스트와 일치 하는 정도를 나타냅니다. 시스템은 입력 문장과 일치하는 문장을 저장소에서 찾습니다. 정확하지 않은 일치도 검색됩니다. 반환 되는 값의 범위는 0에서 100입니다. 여기서 0은 유사성은 없고 100은 대/소문자를 구분 하는 정확히 일치 합니다.
* `MatchedOriginalText`: 이 결과에서 일치된 원본 텍스트입니다. 이 값은 일치 하는 원본 텍스트가 입력 텍스트와 다른 경우에만 반환 됩니다. 유사 항목 일치의 원본 텍스트를 반환 하는 데 사용 됩니다. Microsoft Translator 결과에 대해서는이 값이 반환 되지 않습니다.
* `Rating`: 품질을 결정하는 사용자의 권한을 나타냅니다. 기계 번역 결과의 등급은 5입니다. 익명으로 제공 된 번역은 일반적으로 1에서 4 사이의 등급을 갖습니다. 정식으로 제공 되는 번역의 등급은 일반적으로 6부터 10 까지입니다.
