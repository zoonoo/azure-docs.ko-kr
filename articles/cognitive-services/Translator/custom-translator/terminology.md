...
description: 사용자 지정 변환기 문서에 사용 된 용어 목록입니다.
author: laujan
manager: nitinme
...
ms.locfileid: "104657824"
---
# <a name="custom-translator-terminology"></a>사용자 지정 번역기 용어

다음 표에서는 [사용자 지정 번역기](https://portal.customtranslator.azure.ai)로 작업할 때 찾을 수 있는 용어 목록을 제공 합니다.

| 단어 또는 구|정의|
|------------------|-----------|
| 원본 언어 | 소스 언어는 다른 언어 ("대상")로 변환 하려는 시작 언어입니다.|
| 대상 언어| 대상 언어는 원본 언어를 수신한 후 기계 번역에서 제공해야 하는 언어입니다. |
| 단일어 파일 | Monolingual 파일에는 다른 언어의 다른 파일과 쌍으로 연결 되지 않은 단일 언어가 있습니다. |
...
| 학습 파일 | 학습 파일은 기계 번역 시스템에게 한 언어(원본)에서 대상 언어(대상)로 매핑하는 방법을 학습시키는 데 사용됩니다. 제공 하는 데이터가 많을 수록 시스템이 더 효율적으로 수행 됩니다. |
| 튜닝 파일 | 이러한 파일은 종종 학습 집합에서 무작위로 파생 됩니다 (튜닝 집합을 선택 하지 않은 경우). 문장은 자동으로 선택 되 고 시스템을 튜닝 하 고 올바르게 작동 하는지 확인 하는 데 사용 됩니다. 범용 변환 모델을 만들고 사용자 고유의 튜닝 파일을 만들려면 도메인 전체에서 임의 문장 집합 인지 확인 합니다. |
| 테스팅 파일| 이러한 파일은 종종 학습 세트에서 임의로 선택된 파일에서 파생됩니다(테스트 세트를 선택하지 않은 경우). 이러한 문장의 목적은 번역 모델의 정확도를 평가하는 것입니다. 이러한 문장이 시스템에서 정확 하 게 변환 하 고 있는지 확인 하려는 경우에는 테스트 집합을 만들어 번역기에 업로드할 수 있습니다. 이렇게 하면 이러한 문장이 시스템의 평가 (BLEU 점수 생성)에 사용 됩니다.   |
| 콤보 파일   | 원본 및 번역된 문장이 같은 파일에 포함된 파일 형식입니다. 지원 되는 파일 형식 (.tmx, .tdf, .ici, .xlsx). |
