...
description: 이 자습서를 사용하여 Bing Spell Check API로 쿼리를 전송하고 결과를 표시하는 웹 페이지를 빌드합니다.
...
텍스트 필드를 사용하여 HTML 양식을 만듭니다. **onsubmit** 이벤트를 처리하고 bingSpellCheck 함수를 호출하면서 텍스트 필드와 Bing Spell Check API 키의 내용을 전달합니다.
