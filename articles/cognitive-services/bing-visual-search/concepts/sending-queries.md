...
- [Bing Image Search API](/rest/api/cognitiveservices/bing-images-api-v7-reference) 끝점 중 하나에 대 한 이전 호출의 이미지에서 가져온 정보 토큰을 사용 합니다.
- 이미지의 URL 보내기
- 이미지 업로드 (이진 형식)
...
`imageInfo` 개체에는 `url` 또는 `imageInsightsToken` 필드 중 하나만 포함되어야 합니다. 필드를 `url` 인터넷에서 액세스할 수 있는 이미지의 URL로 설정 합니다. 지원되는 최대 이미지 크기는 1MB입니다.
...
`filters` 개체에는 유사한 이미지 및 유사한 제품 결과를 특정 도메인으로 제한하는 데 사용할 수 있는 사이트 필터(`site` 필드 참조)가 포함되어 있습니다. 예를 들어 이미지가 Surface Book 인 경우를로 설정할 수 있습니다 `site` `www.microsoft.com` .
...
`enableEntityData` `true` 웹 및 특성 정보에 대 한 링크를 포함 하 여 업로드 하는 이미지의 주 엔터티에 대 한 자세한 정보를 보려면 필요에 따라 헤더의 특성을로 설정할 수 있습니다. 이 필드는 `false` 기본적으로입니다.
...
로컬 이미지를 업로드 하는 경우 다음 코드 조각은 게시물의 본문에 포함 해야 하는 폼 데이터를 보여 줍니다. 양식 데이터는 헤더를 포함 해야 합니다 `Content-Disposition` . 해당 `name` 매개 변수를 "image"로 설정해야 하고 `filename` 매개 변수를 임의의 문자열 매개 변수로 설정할 수 있습니다. `Content-Type`헤더는 일반적으로 사용 되는 이미지 mime 형식으로 설정할 수 있습니다. 폼의 내용은 이미지의 이진 데이터입니다. 업로드할 수는 최대 이미지 크기는 1MB입니다. 가장 큰 너비 또는 높이는 1,500픽셀 미만이어야 합니다.
...
이미지에 서비스가 인식하는 텍스트가 포함된 경우 태그 중 하나에는 TextResults 인사이트(작업)가 포함됩니다. 정보는 `displayName` 인식 된 텍스트를 포함 합니다.
...
이미지에 문화권 잘 알려진/인기 있는 사람, 장소 또는 사물과 같은 인식 된 엔터티가 포함 된 경우 태그 중 하나에 엔터티 정보를 포함할 수 있습니다. `mainEntity`및 `data` 필드는 `enableEntityData` 헤더의 특성이 `Content-Type` 로 설정 된 경우에만 사용할 수 있습니다 `true` .
