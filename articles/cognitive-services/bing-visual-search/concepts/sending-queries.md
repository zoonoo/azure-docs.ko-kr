...
`filters` 개체에는 유사한 이미지 및 유사한 제품 결과를 특정 도메인으로 제한하는 데 사용할 수 있는 사이트 필터(`site` 필드 참조)가 포함되어 있습니다. 예를 들어 이미지가 Surface Book 인 경우를로 `site` `www.microsoft.com`설정할 수 있습니다.
...
다음은 요청에서 지정해야 하는 헤더입니다. `Content-Type` `Ocp-Apim-Subscription-Key` 및 헤더 `User-Agent`는 유일 하 게 필요한 헤더 이지만, `X-MSEdge-ClientID` `X-MSEdge-ClientIP`, 및 `X-Search-Location`도 포함 해야 합니다.
...
각 요청에는 `Content-Type` 헤더가 포함 되어야 합니다. 헤더는로 설정 `multipart/form-data; boundary=\<boundary string\>`해야 합니다. 여기서 \<경계 문자열\> 은 폼 데이터의 경계를 식별 하는 고유 하 고 불투명 문자열입니다. `boundary=boundary_1234-abcd`)을 입력합니다.

이미지 토큰 또는 URL Visual Search를 전송 하는 경우 다음 코드 조각은 게시물의 본문에 포함 해야 하는 폼 데이터를 보여 줍니다. 양식 데이터는 헤더를 `Content-Disposition` 포함 해야 하며 해당 `name` 매개 변수를 "knowledgeRequest"로 설정 해야 합니다. `imageInfo` 개체에 대 한 자세한 내용은 요청을 참조 하십시오.
...
웹 및 특성 정보에 `enableEntityData` 대 한 링크를 포함 `true` 하 여 업로드 하는 이미지의 주 엔터티에 대 한 자세한 정보를 보려면 필요에 따라 헤더의 특성을로 설정할 수 있습니다. 이 필드는 `false` 기본적으로입니다.
...
로컬 이미지를 업로드 하는 경우 다음 코드 조각은 게시물의 본문에 포함 해야 하는 폼 데이터를 보여 줍니다. 양식 데이터는 헤더를 `Content-Disposition` 포함 해야 합니다. 해당 `name` 매개 변수를 "image"로 설정해야 하고 `filename` 매개 변수를 임의의 문자열 매개 변수로 설정할 수 있습니다. 헤더 `Content-Type` 는 일반적으로 사용 되는 이미지 mime 형식으로 설정할 수 있습니다. 폼의 내용은 이미지의 이진 데이터입니다. 업로드할 수는 최대 이미지 크기는 1MB입니다. 가장 큰 너비 또는 높이는 1,500픽셀 미만이어야 합니다.
...
이미지에 사용할 수 있는 인사이트가 있는 경우 응답에는 인사이트를 포함하는 `tags`가 하나 이상 포함됩니다. 필드 `image` 는 입력 이미지에 대 한 정보 토큰을 포함 합니다.
...
이미지에 서비스가 인식하는 텍스트가 포함된 경우 태그 중 하나에는 TextResults 인사이트(작업)가 포함됩니다. 정보는 인식 `displayName` 된 텍스트를 포함 합니다.
...
이미지에 문화권 잘 알려진/인기 있는 사람, 장소 또는 사물과 같은 인식 된 엔터티가 포함 된 경우 태그 중 하나에 엔터티 정보를 포함할 수 있습니다. 및 `mainEntity` `data` 필드는 `enableEntityData` `Content-Type` 헤더의 특성이로 설정 된 경우에 `true`만 사용할 수 있습니다.
