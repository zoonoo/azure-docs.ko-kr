...
각 요청에는 `Content-Type` 헤더가 포함 되어야 합니다. 헤더는 `multipart/form-data; boundary=\<boundary string\>`로 설정 되어야 합니다. 여기서 \<경계 문자열\>은 폼 데이터의 경계를 식별 하는 고유 하 고 불투명 한 문자열입니다. 예: `boundary=boundary_1234-abcd`.
