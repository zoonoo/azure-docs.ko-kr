...
이미지에 대한 인사이트를 가져올 수 있는 세 가지 방법은 다음과 같습니다. 

- [Bing Images API](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference) 엔드포인트 중 하나에 대한 이전 호출의 이미지에서 가져온 인사이트 토큰 사용
- 이미지의 URL 보내기
- 이미지 업로드(이진 형식)
...
* [자습서: 단일 페이지 웹앱 빌드 - Bing Visual Search](../tutorial-bing-visual-search-single-page-app.md)
