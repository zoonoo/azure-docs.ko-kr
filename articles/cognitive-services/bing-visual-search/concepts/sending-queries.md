...
description: Bing Visual Search API에 사용되는 REST API 매개 변수에 대해 알아봅니다.
...
- 이미지 중 하나에 대 한 이전 호출에서 얻은 insights 토큰을 사용 하 여 [Bing Image Search API](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference) 끝점입니다.
- 이미지의 URL 보내기
- 이진 형식의 이미지를 업로드 합니다.

## <a name="bing-visual-search-requests"></a>Bing Visual Search 요청

에 이미지 토큰 또는 URL Visual Search를 보내는 경우 다음 코드 조각은 POST의 본문에 포함 해야 하는 JSON 개체를 보여 줍니다.
...
`imageInfo` 개체에는 `url` 또는 `imageInsightsToken` 필드 중 하나만 포함되어야 합니다. 설정 된 `url` 필드를 인터넷에 액세스할 수 있는 이미지의 URL입니다. 지원되는 최대 이미지 크기는 1MB입니다.
...
`cropArea` 필드는 선택 사항입니다. 자르기 영역 왼쪽 위 모퉁이 오른쪽 아래 모퉁이의 관심 영역을 지정합니다. 0\.0~1.0 범위의 값을 지정합니다. 값은 전체 너비 또는 높이의 백분율입니다. 예를 들어 위 예제에서는 이미지의 오른쪽 절반을 관심 영역으로 표시합니다. 인사이트 요청을 관심 영역으로 제한하려면 포함합니다.
...
로컬 이미지를 업로드 하는 경우 다음 코드 예제 양식 데이터는 POST의 본문에 포함 해야 합니다. 양식 데이터를 포함 해야 합니다는 `Content-Disposition` 헤더입니다. 해당 `name` 매개 변수를 "image"로 설정해야 하고 `filename` 매개 변수를 임의의 문자열 매개 변수로 설정할 수 있습니다. `Content-Type` 자주 사용 되는 이미지 mime 형식으로 헤더를 설정할 수 있습니다. 폼의 콘텐츠는 이미지의 이진 데이터입니다. 업로드할 수는 최대 이미지 크기는 1MB입니다. 가장 큰 너비 또는 높이는 1,500픽셀 미만이어야 합니다.
...
다음 코드 조각에는 업로드 된 이미지의 관심 영역을 지정 하는 방법을 보여 줍니다.
...
다음 코드 조각은 이미지 토큰과 관심 영역을 전달 하는 이미지 완료 insights 요청을 보여 줍니다. /Images/search 이전 호출에서 insights 토큰을 가져옵니다.
...
이미지에 사용할 수 있는 인사이트가 있는 경우 응답에는 인사이트를 포함하는 `tags`가 하나 이상 포함됩니다. `image` 필드 입력된 이미지에 대 한 insights 토큰을 포함 합니다.
...
`tags` 필드에는 표시 이름 및 작업(인사이트) 목록이 포함됩니다. 태그 중 하나에는 빈 문자열로 설정된 `displayName` 필드가 있습니다. 이 태그에는 이미지를 포함하는 웹 페이지, 시각적으로 유사한 이미지, 이미지에서 찾은 항목에 대한 쇼핑 소스 등의 기본 인사이트가 포함됩니다. 전체 이미지 관심 이기 때문에 기본 insights 태그 경계 상자의 지역에 대 한 관심 없는:
...
목록은 기본 정보를 참조 하세요 [기본 insights 태그](../default-insights-tag.md)합니다.

나머지 태그에는 사용자가 관심을 가질 수 있는 기타 인사이트가 포함됩니다. 예를 들어 이미지에 텍스트가 포함된 경우 태그 중 하나에는 인식된 텍스트를 포함하는 TextResults 인사이트가 포함될 수 있습니다. 또는 Bing 이미지에서 (즉, 사람, 장소 또는 것) 엔터티를 인식 하는 경우 엔터티를 식별할 수 태그 중 하나입니다. Visual Search는 입력 이미지에서 파생된 다양한 용어(태그) 집합도 반환합니다. 이러한 태그를 사용 하면 이미지에 있는 개념을 탐색할 수 있도록 합니다. 예를 들어 입력 이미지가 유명 운동 선수인 경우 태그 중 하나는 스포츠 이미지에 대한 링크가 포함된 스포츠일 수 있습니다.
...
이미지에 서비스가 인식하는 텍스트가 포함된 경우 태그 중 하나에는 TextResults 인사이트(작업)가 포함됩니다. Insight의 `displayName` 인식된 된 텍스트를 포함 합니다.
