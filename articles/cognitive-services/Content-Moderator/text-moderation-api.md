...
Content Moderator의 기계 지원 텍스트 조정 및 [인간 참여형](Review-Tool-User-Guide/human-in-the-loop.md) 기능을 사용하여 텍스트 콘텐츠를 조정할 수 있습니다.

비즈니스는 텍스트 조정 서비스를 사용하여 정책 및 임계 값에 따라 콘텐츠를 차단, 승인 또는 검토합니다. 텍스트 조정 서비스를 사용하여 파트너, 직원 및 소비자가 텍스트 콘텐츠를 생성해야 하는 환경에 대한 사용자 조정을 보강할 수 있습니다. 여기에는 채팅방, 토론 게시판, 챗봇, 전자 상거래 카탈로그, 문서 등이 포함됩니다. 

API는 들어오는 텍스트(최대 1024자)에서 욕설을 검사하고, 원치 않는 텍스트를 분류하고(미리 보기), 텍스트를 자동 수정하고, PII(개인 식별 정보)를 감지합니다. 또한 사용자 지정 용어 목록과 일치하는 용어를 찾습니다. 자동 고침 기능은 의도적으로 철자가 틀린 단어를 catch하는 데 도움이 됩니다. 콘텐츠가 처리되면 서비스에서 자세한 응답을 반환합니다. 응답을 사용하여 검토 도구에서 사용자 검토를 만들거나 기록합니다.

서비스 응답에는 다음 정보가 포함됩니다.

- 욕설: 여러 언어의 기본 제공 욕설 목록을 사용하는 용어 기반 일치
...
Content Moderator의 기계 지원 **텍스트 분류 기능**은 **영어만** 지원하며, 잠재적으로 원치 않는 콘텐츠를 감지하는 데 도움이 됩니다. 플래그가 지정된 콘텐츠는 컨텍스트에 따라 부적절한 콘텐츠로 간주될 수 있습니다. 각 범주의 가능성을 전달할 뿐 아니라 콘텐츠에 대한 사용자 검토를 권장할 수 있습니다. 이 기능은 학습된 모델을 사용하여 상스럽거나 경멸적이거나 차별적인 언어를 식별합니다. 여기에는 은어, 약어, 불쾌한 단어, 의도적으로 철자가 틀린 단어의 검토가 포함됩니다. 

JSON 추출의 다음 추출은 예제 출력을 보여 줍니다.

> [!NOTE]
> 기계 지원 ‘분류’ 기능은 현재 미리 보기로 제공되고 있습니다.
...
대부분의 경우 기본 전역 용어 목록을 사용해도 되지만, 비즈니스 요구와 관련된 용어를 기준으로 심사하는 것이 좋습니다. 예를 들어 사용자 게시물에서 경쟁 브랜드 이름을 필터링할 수 있습니다. 허용되는 텍스트 콘텐츠의 임계값은 기본 목록과 다를 수 있습니다.
