---
title: '빠른 시작: C#의 Content Moderator에서 이미지 콘텐츠 확인'
titlesuffix: Azure Cognitive Services
description: C#용 Content Moderator SDK를 사용하여 이미지 콘텐츠를 확인하는 방법
...
ms.locfileid: "49309300"
---
# <a name="quickstart-check-image-content-in-c"></a>빠른 시작: C#에서 이미지 콘텐츠 확인 

이 문서에서는 [.NET용 Content Moderator SDK](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.ContentModerator/)를 사용하여 다음에 대한 이미지를 동시에 확인할 수 있도록 지원하는 정보 및 코드 샘플을 제공합니다. 
...
REST API 또는 SDK를 통해 Content Moderator 서비스를 사용하려면 먼저 API 키 및 API 계정의 지역이 필요합니다. 둘 다 구하려면 Azure Portal에서 [Content Moderator 서비스를 구독](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesContentModerator)합니다.
...
입력 파일 _ImageFiles.txt_를 만들고 분석하려는 이미지 URL을 추가합니다. 이 빠른 시작에서는 다음 두 URL을 사용하여 샘플 출력을 생성합니다.
- https://moderatorsampleimages.blob.core.windows.net/samples/sample2.jpg
- https://moderatorsampleimages.blob.core.windows.net/samples/sample5.png

## <a name="create-a-class-to-handle-results"></a>결과를 처리하는 클래스 만들기

**Program** 클래스에 다음 클래스를 추가합니다. 이 클래스의 인스턴스를 사용하여 검토한 각각의 이미지에 대한 조정 결과를 기록합니다.
...
## <a name="create-the-image-evaluation-method"></a>이미지 평가 메서드 만들기
...
## <a name="evaluate-the-images-in-your-code"></a>코드에서 이미지 평가
...
_ModerationOutput.json_ 파일을 열고 출력 콘텐츠를 확인합니다. 다음 콘텐츠와 유사해야 합니다. 각 이미지에는 `ImageModeration`, `FaceDetection` 및 `TextDetection`에 대해 다른 섹션이 있으며 **EvaluateImage** 메서드의 3가지 API 호출에 해당합니다.
