---
title: '빠른 시작: .NET용 Content Moderator 클라이언트 라이브러리 | Microsoft Docs'
titleSuffix: Azure Cognitive Services
description: .NET용 Content Moderator 클라이언트 라이브러리를 시작합니다.
...
ms.locfileid: "74539026"
---
# <a name="quickstart-content-moderator-client-library-for-net"></a>빠른 시작: .NET용 Content Moderator 클라이언트 라이브러리

.NET용 Content Moderator 클라이언트 라이브러리를 시작합니다. 이러한 단계에 따라 패키지를 설치하고 기본 작업을 위한 예제 코드를 사용해 봅니다. Content Moderator는 자료에서 모욕적이거나, 위험하거나, 원치 않는 텍스트, 이미지 및 비디오 콘텐츠를 확인하는 인지 서비스입니다. 이러한 자료가 발견되면 서비스가 콘텐츠에 적절한 레이블(플래그)을 적용합니다. 그러면 앱이 플래그가 지정된 콘텐츠를 처리하여 규정을 준수하거나 원하는 사용자 환경을 유지할 수 있습니다.
...
다음은 Content Moderator .NET SDK의 주요 기능 중 일부를 처리하는 클래스입니다.
...
#### <a name="iterate-through-image-urls"></a>이미지 URL 반복
...
조정 담당 직원이 검토할 수 있도록 Content Moderator .NET SDK를 사용하여 콘텐츠를 [검토 도구](https://contentmoderator.cognitive.microsoft.com)에 제공할 수 있습니다. 검토 도구에 대한 자세한 내용은 [검토 도구 개념 가이드](./review-tool-user-guide/human-in-the-loop.md)를 참조하세요.
...
* 이 샘플의 소스 코드는 [GitHub](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/blob/master/documentation-samples/quickstarts/ContentModerator/Program.cs)에서 확인할 수 있습니다.
