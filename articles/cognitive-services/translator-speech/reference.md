...
|X-ClientVersion|(empty)    |클라이언트 애플리케이션의 버전을 식별합니다. 예제: "2.1.0.123".<br/>헤더를 사용하는 대신, 이 값을 쿼리 매개 변수 `X-ClientVersion`에 전달할 수 있습니다. 헤더와 쿼리 매개 변수를 둘 다 설정한 경우 쿼리 매개 변수는 무시됩니다.|머리글|string|
|X-OsPlatform|(empty)   |클라이언트 애플리케이션에서 실행되는 운영 체제의 이름 및 버전을 식별합니다. 예제: "Android 5.0", "iOs 8.1.3", "Windows 8.1".<br/>헤더를 사용하는 대신, 이 값을 쿼리 매개 변수 `X-OsPlatform`에 전달할 수 있습니다. 헤더와 쿼리 매개 변수를 둘 다 설정한 경우 쿼리 매개 변수는 무시됩니다.|머리글|string|
