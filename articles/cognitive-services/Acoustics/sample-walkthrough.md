---
title: 프로젝트 Acoustics 샘플 연습 - Cognitive Services
...
샘플에는 데모 장면 **Assets/AcousticsDemo/ProjectAcousticsDemo.unity**가 포함됩니다. 이 장면에는 부동 큐브(**계층 구조**의 **AudioHolder**)에서 재생되는 단일 공간화 오디오 원본이 있습니다. 일반 탐색 스크립트를 쉽게 만들도록 하기 위해 Main Camera는 CameraHolder 개체의 자식으로 지정됩니다. 

![계층 구조 뷰](media/SampleHierarchyView.png)

이 장면은 미리 준비되었으며, **계층 구조**에서 **MicrosoftAcoustics** prefab에 연결된 ACE 파일이 있습니다. 

Unity 편집기에서 재생 단추를 클릭하여 장면에 어떤 소리가 나는지 들어봅니다. W, A, S, D 및 마우스를 사용하여 이동합니다. 음향 효과를 사용한 장면과 사용하지 않은 장면에서 어떤 소리가 나는지 비교하려면 왼쪽 마우스 단추 또는 기본 컨트롤러 단추를 클릭합니다. 다양한 소리 원본을 순환하려면 마우스 오른쪽 단추나 컨트롤러의 뒤로 단추를 클릭합니다.
