...
메시가 음향 준비에 참여하는 장면을 보여 주는 데 사용되는 복셀 크기가 디자인 타임 및 런타임 뷰에서 다르다는 것을 알 수 있습니다. 이러한 차이는 선택한 시뮬레이션 주파수의 품질/세분성에는 영향을 주지 않으며, 입체 화상으로 만든 장면을 런타임으로 사용할 때 부수적으로 발생할 뿐입니다. 런타임 시 시뮬레이션 복셀은 원본 위치 사이의 보간을 지원하도록 "미세 조정"됩니다. 또한 음향 처리된 메시를 포함하는 복셀 내 원본은 소리를 생성하지 않으므로, 디자인 타임 소리 원본을 시뮬레이션 복셀 크기가 허용하는 것보다 장면 메시에 더 가깝게 배치할 수 있습니다.
...
* **Enable Acoustics**(음향 효과 사용) - 이 원본에 음향 효과가 적용되는지 여부를 제어합니다. 선택을 취소하면 원본이 HRTF 또는 이동을 통해 입체화되지만 음향 효과는 사용되지 않습니다. 즉, 레벨 및 감소 시간과 같은 장애, 폐색 및 동적 반향 매개 변수가 사용되지 않습니다. 반향은 고정된 수평 및 감쇠 시간을 사용하여 계속 적용됩니다.
...
* **Outdoorness**(야외성) - 원본에 대한 반향이 발생하는 위치의 “야외성”에 대한 음향 시스템 예측값을 추가로 조정합니다. 이 값을 1로 설정하면 소리가 항상 완전히 야외에서 나오며, -1로 설정하면 소리가 실내에서 나옵니다.
