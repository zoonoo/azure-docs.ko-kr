...
음성 학습 데이터 집합은 오디오 파일 집합과, 오디오 파일의 스크립트가 포함된 텍스트 파일로 구성되어 있습니다.
...
2. 주 페이지에서 **사용자 지정 음성** 아래의 **데이터**를 선택합니다.   
    ...
    **내 음성** 테이블이 표시됩니다. 음성 데이터 집합을 아직 업로드하지 않았으면 이 테이블이 빈 상태로 표시됩니다.
...
8. **가져오기**를 선택하여 데이터를 업로드합니다. 데이터 집합이 더 큰 경우 가져오는 데 몇 분 정도 걸릴 수 있습니다.

> [!NOTE]
> 무료 구독 사용자는 한 번에 두 개의 데이터 집합을 업로드할 수 있습니다. 표준 구독 사용자는 5개의 데이터 집합을 동시에 업로드할 수 있습니다. 업로드 개수가 초과되면 적어도 한 개의 데이터 집합에 대한 가져오기가 완료될 때까지 기다립니다. 그런 다음, 다시 시도하세요.

업로드가 완료되면 **내 음성 데이터** 테이블이 다시 나타납니다. 방금 업로드한 데이터 집합에 해당하는 항목을 볼 수 있습니다. 

데이터 집합은 업로드 후 자동으로 유효성이 검사됩니다. 데이터 유효성 검사에는 오디오 파일의 파일 형식, 크기 및 샘플링 레이트를 확인하는 일련의 검사가 포함됩니다. 스크립트 파일에 대한 검사를 통해 파일 형식의 유효성이 확인되고 일부 텍스트 정규화가 진행됩니다. 발화가 음성 인식을 통해 스크립트로 작성됩니다. 그런 다음, 결과 텍스트는 사용자가 제공한 기록와 비교됩니다.
...
유효성 검사가 완료되면 **발화** 열의 각 데이터 집합에 대해 일치하는 발화의 총 개수를 확인할 수 있습니다.
...
발음 점수가 낮거나 신호 대 잡음 비율이 떨어지는 모든 발화는 다시 녹음하는 것이 좋습니다. 다시 녹음할 수 없다면 해당 발화를 데이터 집합에서 제외할 수 있습니다.

## <a name="build-your-voice-font"></a>음성 글꼴 작성

유효성이 검사된 데이터 집합은 사용자 지정 음성 글꼴을 작성하는 데 사용할 수 있습니다. 
