...
1. 데이터 세트를 업로드했을 때처럼 이 모델을 식별하는 데 도움이 되는 이름과 설명을 입력합니다.
    ...
    신중하게 이름을 선택합니다. 여기에 입력한 이름은 음성 합성 요청에서 음성을 지정할 때 SSML 입력의 일부로 사용됩니다. 문자, 숫자 및 몇 가지 문장 부호 문자(예: `-`, `_` 및 `(', ')`)만 허용됩니다.
...
> [!NOTE]
> 프랑스어, 독일어 및 이탈리아 음성 학습은 2,000개 이상의 발화가 있는 데이터 세트로 시작합니다. 중국어-영어 이중 언어 모델도 2,000개 이상의 발화로 이루어진 데이터 세트로 지원됩니다.
