...
경고 임계값을 설정 하려면 `SpeechConfig` 개체에 지정 해야 합니다. 새 인식기를 만들 때이 개체를 확인 합니다. 다음 예에서는 `config`이라는 `SpeechConfig` 인스턴스를 만들었다고 가정 합니다.
...
오류 임계값을 설정 하려면 `SpeechConfig` 개체에 지정 해야 합니다. 새 인식기를 만들 때이 개체를 확인 합니다. 다음 예에서는 `config`이라는 `SpeechConfig` 인스턴스를 만들었다고 가정 합니다.
...
> [!TIP]
> 이 속성의 기본값은 `size_t` 데이터 형식에 대 한 플랫폼별 최대값입니다. 일반적인 인식은 7 개에서 10 개 사이의 내부 개체를 사용 합니다.
