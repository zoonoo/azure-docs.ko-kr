...
1. 사용자가 장치를 켤 지 여부를 나타내는 새 매개 변수를 만듭니다.
   ...
   | 전역          | unchecked           | 이 매개 변수의 값이 프로젝트의 모든 명령에 전역적으로 적용 되는지 여부를 나타내는 확인란 |
   | 필수           | checked             | 명령을 완료 하기 전에이 매개 변수의 값이 필요한 지 여부를 나타내는 확인란          |
   | 응답 템플릿  | "-On 또는 off?"      | 이 매개 변수 값을 알 수 없는 경우 요청 하는 프롬프트입니다.                                       |
   | Type               | 문자열              | 숫자, 문자열 또는 날짜 시간과 같은 매개 변수의 형식입니다.                                               |
   | Configuration      | 문자열 목록         | 문자열의 경우 문자열 목록에서 가능한 값 집합에 대 한 입력을 제한 합니다.                                      |
   ...
   | 전역          | unchecked             | 이 매개 변수의 값이 프로젝트의 모든 명령에 전역적으로 적용 되는지 여부를 나타내는 확인란 |
   | 필수           | checked               | 명령을 완료 하기 전에이 매개 변수의 값이 필요한 지 여부를 나타내는 확인란          |
   | 응답 템플릿  | "-" 장치     | 이 매개 변수 값을 알 수 없는 경우 요청 하는 프롬프트입니다.                                       |
   | Type               | 문자열                | 숫자, 문자열 또는 날짜 시간과 같은 매개 변수의 형식입니다.                                               |
   | Configuration      | 문자열 목록           | 문자열의 경우 문자열 목록에서 가능한 값 집합에 대 한 입력을 제한 합니다.                                      |
...
> [!TIP]
> 샘플 문장 편집기에서 중괄호를 사용 하 여 매개 변수를 참조 합니다. - `turn {OnOff} the {SubjectDevice}`탭 완성 기능을 사용 하 여 이전에 만든 매개 변수를 참조 합니다.
...
1. 새 조건을 추가 하 고 필수 매개 변수를 선택 합니다. 및 둘 `OnOff` 다 선택`SubjectDevice`
1. 음성 응답 작업을 편집 하 여 `OnOff` 및 `SubjectDevice`를 사용 합니다.
