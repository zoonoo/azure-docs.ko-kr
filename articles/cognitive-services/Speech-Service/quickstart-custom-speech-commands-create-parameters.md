---
title: '빠른 시작: 매개 변수를 사용 하 여 사용자 지정 명령 만들기 (미리 보기)'
...
1. 사용자가 장치를 켤 지 여부를 나타내는 새 매개 변수를 만듭니다.
   ...
   | 전역          | 선택 취소       | 이 매개 변수의 값이 프로젝트의 모든 명령에 전역적으로 적용 되는지 여부를 나타내는 확인란 |
   | 필수           | 검사할         | 명령을 완료 하기 전에이 매개 변수의 값이 필요한 지 여부를 나타내는 확인란          |
   | 응답 템플릿  | 설정 또는 해제 여부      | 이 매개 변수 값을 알 수 없는 경우 요청 하는 프롬프트입니다.                                       |
   | 형식               | 문자열          | 숫자, 문자열 또는 날짜 시간과 같은 매개 변수의 형식입니다.                                               |
...
매개 변수를 사용 하 여 가능한 모든 조합을 다루는 샘플 문장을 추가 하는 것이 좋습니다. 예:
