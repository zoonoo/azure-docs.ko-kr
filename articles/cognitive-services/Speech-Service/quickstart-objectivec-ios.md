---
title: '빠른 시작: Speech Service SDK를 사용하여 iOS의 Objective-C에서 음성 인식'
...
1. 프로젝트 옵션 대화 상자
    ...
    1. Apple 개발자 계정이 이미 있는 경우 적절한 조직 이름 및 조직 식별자를 입력합니다. 테스트를 위해 `testorg`와 같은 이름을 선택할 수 있습니다. 앱에 서명하기 위해 적절한 프로비전 프로이 필요합니다. 자세한 내용은 [Apple 개발자 사이트](https://developer.apple.com/)를 참조하세요.
...
예제 앱에서는 UI가 아주 단순합니다. 파일 또는 마이크에서 음성 인식을 시작할 수 있는 단추 두 개와 결과를 표시하는 텍스트 레이블로 구성되어 있습니다.
...
1. **제품** -> **대상** 메뉴의 목록에서 앱에 대한 대상으로 개발 머신에 연결된 iOS 시뮬레이터 또는 iOS 장치를 선택합니다.
