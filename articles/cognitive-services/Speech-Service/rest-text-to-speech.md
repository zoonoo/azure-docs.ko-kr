...
description: 텍스트 음성 변환 REST API를 사용하는 방법을 알아봅니다. 이 문서에서는 권한 부여 옵션, 쿼리 옵션, 요청을 구성하고 응답을 받는 방법을 알아봅니다.
...
음성 서비스를 사용하면 [텍스트를 합성된 음성으로 변환하고](#convert-text-to-speech) REST API 집합을 사용하여 지역에 대해 [지원되는 음성 목록을 얻을 수](#get-a-list-of-voices) 있습니다. 사용 가능한 각 끝점은 영역과 연결됩니다. 사용할 엔드포인트/리전에 대한 구독 키가 필요합니다.
...
이 API를 사용하기 전에 다음을 이해하십시오.

* Text-to-Speech REST API에는 인증 헤더가 필요합니다. 즉, 서비스에 액세스하기 위해 토큰 교환을 완료해야 합니다. 자세한 내용은 [인증](#authentication)을 참조하세요.

[!INCLUDE [](../../../includes/cognitive-services-speech-service-rest-auth.md)]

## <a name="get-a-list-of-voices"></a>음성 목록 받기

`voices/list` 끝점을 사용하면 특정 지역/끝점에 대한 전체 음성 목록을 얻을 수 있습니다.
...
`v1` 끝점을 사용하면 [SSML(음성 합성 태그 언어)을](speech-synthesis-markup.md)사용하여 텍스트 음성변환을 수행할 수 있습니다.
...
| `User-Agent` | 애플리케이션 이름. 제공된 값은 255자 미만이어야 합니다. | 필수 |

### <a name="audio-outputs"></a>오디오 출력

각 요청에서 `X-Microsoft-OutputFormat` 헤더로 전송되는 지원되는 오디오 형식 목록입니다. 각 항목에 전송률 및 인코딩 형식이 포함됩니다. 음성 서비스는 24kHz, 16kHz 및 8kHz 오디오 출력을 지원합니다.
...
> [!NOTE]
> 선택한 음성 및 출력 형식의 비트 전송률이 다른 경우 필요에 오디오가 다시 샘플링됩니다. 그러나 24kHz 음성은 `audio-16khz-16kbps-mono-siren` 형식을 `riff-16khz-16kbps-mono-siren` 지원하지 않습니다.
...
이 HTTP 요청은 SSML을 사용하여 음성 및 언어를 지정합니다. 몸길이가 길고 결과 오디오가 10분을 초과하는 경우- 10분으로 잘립니다. 즉, 오디오 길이는 10분을 초과할 수 없습니다.
...
언어별 예제는 빠른 시작을 참조하십시오.

* [.NET 코어, C #](~/articles/cognitive-services/Speech-Service/quickstarts/text-to-speech.md?pivots=programming-language-csharp&tabs=dotnetcore)
...
| 415 | 지원되지 않는 미디어 유형 | 잘못된 `Content-Type` 것이 제공되었을 수 있습니다. `Content-Type``application/ssml+xml`로 설정해야 합니다. |
| 429 | 너무 많은 요청 | 구독에 허용되는 요청의 할당량 또는 속도가 초과되었습니다. |
| 502 | 잘못된 게이트웨이    | 네트워크 또는 서버 쪽 문제입니다. 잘못된 헤더를 나타낼 수도 있습니다. |
...
- [장편 오디오를 위한 비동기 합성](quickstarts/text-to-speech/async-synthesis-long-form-audio.md)
