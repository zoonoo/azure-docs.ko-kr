...
- **Java**: 낮은 수준의 디버깅이 향상 되었습니다. [`Connection`](https://docs.microsoft.com/java/api/com.microsoft.cognitiveservices.speech.connection?view=azure-java-stable)이제 클래스에는 `MessageReceived` 다른 프로그래밍 언어와 유사한 이벤트가 있습니다 (c + +, c #). 이 이벤트는 서비스에서 들어오는 데이터에 대 한 하위 수준 액세스를 제공 하며 진단 및 디버깅에 유용할 수 있습니다.
- **JavaScript**: [`BotFrameworkConfig`](https://docs.microsoft.com/javascript/api/microsoft-cognitiveservices-speech-sdk/botframeworkconfig) 이제 `fromHost()` `fromEndpoint()` 사용자 지정 서비스 위치를 편리 하 게 설정 하 고 속성을 설정 하는 방법 및 팩터리 메서드를 사용 하는 및 팩터리 메서드를 사용 하 여 음성 도우미와 봇을 쉽게 설정할 또한 `botId` 구성 팩터리에 대해 기본이 아닌 봇을 사용 하기 위해의 선택적 사양을 표준화 합니다.
