...
사용자 지정 음성은 **기본 모델과** **사용자 지정 모델** 을 모두 사용 합니다. 각 언어에는 하나 이상의 **기본 모델이** 있습니다. 일반적으로 새 음성 모델을 일반 음성 서비스로 출시할 때 새 **기본 모델로** Custom Speech 서비스에도 가져옵니다. 이러한 업데이트는 3-6 개월 마다 업데이트 되며, 이전 모델은 일반적으로 최신 모델의 정확도가 상당히 높기 때문에 일반적으로 시간이 지남에 따라 더 유용 하지 않게 됩니다.

반면, 특정 고객 시나리오에 대해 선택한 기본 모델을 적용 하 여 **사용자 지정 모델** 을 만듭니다. 사용자의 요구를 충족 하는 일정에 도달 하면 오랜 시간 동안 특정 사용자 지정 모델을 계속 사용 하거나 추가 데이터를 사용 하 여 시간에 따라 다시 학습을 유지할 수 있습니다. 
...
새 기능을 추가 하 고 성능을 향상 시키기 위해 기본 모델을 변경 하면 이전 모델에 대해 이전 버전과의 호환성 문제가 발생할 수 있으며 특정 테스트 데이터 집합의 지정 된 모델에서 관찰 된 정확도도 변경 될 수 있습니다. 모델 및 끝점 유지 관리 작업을 관리 하려면 모델 및 끝점 만료 타임 라인에 대 한 다음 시간 줄을 참조 하세요.
