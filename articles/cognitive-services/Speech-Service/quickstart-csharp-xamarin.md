...
ms.locfileid: "72675615"
---
# <a name="quickstart-recognize-speech-by-using-a-cross-platform-xamarin-app-that-uses-the-speech-sdk"></a>빠른 시작: Speech SDK를 사용하는 플랫폼 간 Xamarin 앱을 통해 음성 인식
...
이 문서에서는 Azure Cognitive Services [Speech SDK](speech-sdk.md)를 사용하는 UWP(유니버설 Windows 플랫폼), Android 및 iOS용 Xamarin을 사용하여 플랫폼 간 C# 애플리케이션을 개발합니다. 프로그램은 디바이스 마이크의 음성을 실시간으로 텍스트로 전사합니다. 이 애플리케이션은 [Speech SDK NuGet 패키지](https://aka.ms/csspeech/nuget) 및 모든 버전의 Microsoft Visual Studio 2019을 사용하여 빌드됩니다.
...
* iOS를 대상으로 하는 경우: 
    * iOS 디바이스(ARM64) 또는 iOS 시뮬레이터(x64) - [개발 용도로 사용](https://docs.microsoft.com/xamarin/ios/get-started/installation/device-provisioning/)할 수 있고 작동하는 마이크가 장착되어 있어야 합니다.
...
## <a name="add-sample-code-for-the-helloworldandroid-project"></a>helloworld.Android 프로젝트의 샘플 코드 추가
...
## <a name="add-sample-code-for-the-helloworldios-project"></a>helloworld.iOS 프로젝트의 샘플 코드 추가
...
1. 다음으로, Visual Studio의 helloworld.UWP 프로젝트 아래에서 `Package.appxmanifest` 파일을 두 번 클릭합니다. **기능** 아래에서 **마이크**가 선택되어 있는지 확인하고, 파일을 저장합니다.
   ...
   > [!NOTE] 
   > "helloworld.UWP_TemporaryKey.pfx 인증서 파일이 없습니다." 경고가 표시되면 [음성 텍스트 변환](quickstart-csharp-uwp.md) 샘플을 확인하여 자세한 내용을 확인하세요.
