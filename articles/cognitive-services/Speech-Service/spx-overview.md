...
description: 음성 CLI는 코드를 작성 하지 않고도 음성 서비스를 사용 하는 명령줄 도구입니다. 음성 CLI에는 최소의 설정이 필요 하며, 음성 서비스의 주요 기능 실험을 바로 시작 하 여 사용 사례가 충족 될 수 있는지 확인 하는 것이 쉽습니다.
...
음성 SDK의 대부분 기능은 음성 CLI에서 사용할 수 있으며, 일부 고급 기능 및 사용자 지정은 음성 CLI에서 간소화 되었습니다. 음성 CLI 또는 Speech SDK를 사용할 시기를 결정 하려면 다음 지침을 고려 하세요.
...
* 특정 언어 또는 플랫폼 (예: c #, Python, c + +) 내에서 음성 서비스 기능을 통합 하려고 합니다.
...
음성 CLI를 시작 하려면 [빠른](spx-basics.md)시작을 참조 하세요. 이 문서에서는 몇 가지 기본 명령을 실행 하는 방법을 보여 주고 음성 텍스트 및 텍스트 음성 변환에 대해 일괄 처리 작업을 실행 하기 위한 약간 더 고급 명령도 보여 줍니다. 기본 문서를 읽은 후에는 몇 가지 사용자 지정 명령을 작성 하거나 간단한 음성 서비스 작업을 자동화 하는 구문을 충분히 이해 해야 합니다.

## <a name="next-steps"></a>다음 단계

- [음성 CLI 빠른](spx-basics.md) 시작 시작
- [데이터 저장소 구성](./spx-data-store-configuration.md)
- [음성 CLI를 사용 하 여 batch 작업을 실행](./spx-batch-operations.md) 하는 방법을 알아봅니다.
