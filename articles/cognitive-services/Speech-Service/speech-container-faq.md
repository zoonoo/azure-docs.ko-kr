...
잠시 후 `ja-JP` 컨테이너 및 최신 모델을 고려 합니다. 어쿠스틱 모델은 CPU를 가장 많이 필요로 하는 부분으로, 언어 모델은 대부분의 메모리를 요구 합니다. 하도록 벤치마킹를 사용 하는 경우 오디오를 실시간으로 이동 하는 경우 (예: 마이크에서) 한 번의 음성-텍스트 요청을 처리 하기 위해 약 0.6 CPU 코어가 소요 됩니다. 파일에서와 같이 실시간으로 오디오를 공급 하는 경우 해당 사용은 이중 (1.2 x 코어)이 될 수 있습니다. 한편, 아래에 나열 된 메모리는 음성을 디코딩하는 데 사용할 수 있는 운영 체제입니다. 파일 캐시에 상주 하는 언어 모델의 실제 전체 크기를 고려 *하지* 않습니다. 추가 `ja-JP` 2gb 인 경우 의 `en-US`경우 더 많은 (6-7 g b) 일 수 있습니다.
...
마지막으로, 변수를 사용 하 여 `DECODER MAX_COUNT` *단일* 컨테이너 내에서 원하는 디코더 수를 설정할 수 있습니다. 따라서 기본적으로 SKU (CPU/memory)로 시작 하 고이를 최대한 활용 하는 방법을 제안할 수 있습니다. 좋은 출발점은 권장 되는 호스트 컴퓨터 리소스 사양을 참조 하는 것입니다.
...
<details>
<summary>
<b>온-프레미스 음성 컨테이너의 용량 계획 및 비용 예측을 도와줄 수 있나요?</b>
</summary>
...
그러나 각 컨테이너 인스턴스가 더 많은 디코더를 사용 하 여 실행 될 수도 있습니다. 예를 들어 8 개 코어 컴퓨터 (각각 2 배 이상)에서 컨테이너 인스턴스당 7 개의 디코더를 설정 하 여 15x 처리량을 얻을 수 있습니다. 알고 있어야 하는 `DECODER_MAX_COUNT` 매개 변수가 있습니다. 극단적인 경우 안정성 및 대기 시간 문제가 발생 하 고 처리량이 크게 증가 합니다. 마이크의 경우,이는 1x 실시간입니다. 전체 사용량은 단일 인식에 대해 핵심입니다.
...
하드웨어의 경우 표준 Azure VM `DS13_v2` 을 참조로 사용 합니다 (AVX2 명령 집합을 사용 하도록 설정한 경우 각 코어는 2.6 GHz 이상 이어야 함).
...
**답변 2:** 대/소문자를 구분 하는 요청에 올바른 음성 이름을 제공 해야 합니다. 전체 서비스 이름 매핑을 참조 하세요. 텍스트 음성 변환의 `en-US-JessaRUS`컨테이너 버전 `en-US-JessaNeural` 에서 현재 사용할 수 없으므로를 사용 해야 합니다.
...
- 3 개의<sup>ts</sup> 파트 끝점이 받아쓰기 끝점에 대해 반환 하는 `speech.hypothesis` 메시지 대신, 한 개의<sup>st</sup> 파티 끝점은 메시지 대신 음성 조각화 메시지를 반환 합니다.
- 참조 퀵 스타트 모두 사용 `RecognizeOnce` (대화형 모드)
- 상호 작용을 필요로 하는 `speech.fragment` 메시지에 대 한 어설션이 대화형 모드로 반환 되지 않습니다.
...
**답변:** 사용해 `-p <outside_unique_port>:5000`보세요. `-p 5001:5000`)을 입력합니다.
...
<details>
<summary>
<b>15 초 길이의 오디오 &lt;를 처리 하는 비 batch api를 어떻게 가져올 수 있나요?</b>
</summary>

**대답:** `RecognizeOnce()` 대화형 모드에서는 길이 발언가 짧을 것으로 예상 되는 음성 명령에만 사용할 수 있는 최대 15 초의 오디오를 처리 합니다. 받아쓰기 또는 대화 `StartContinuousRecognition()` 에를 사용 하는 경우 15 초 제한이 없습니다.
...
**답변:** 실시간으로는 최근 `en-US`8 개를 사용 하 여 6 개 이상의 동시 요청 보다 많은 docker 컨테이너를 사용 하는 것이 좋습니다. 16 개 코어를 초과 하는 crazier을 가져오고, NUMA (non-uniform memory access) 노드가 중요 한 것으로 인식 됩니다. 다음 표에서는 각 음성 컨테이너에 대 한 최소 및 권장 리소스 할당을 설명 합니다.
...
- 디코더는 약 2 ~ 3 배 실시간 작업을 수행할 수 있습니다. 이를 위해 전체 CPU 사용량은 단일 인식을 위해 두 코어에 가깝습니다. 따라서 컨테이너 인스턴스당 세 개 이상의 활성 연결을 유지 하지 않는 것이 좋습니다. 극단적인 측면은와 같은 `DS13_V2`8 개의 코어 컴퓨터에서 10 개의 디코더를 2 배 이상 실시간으로 배치 하는 것입니다. 컨테이너 버전 1.3 이상에서는 설정을 `DECODER_MAX_COUNT=20`시도할 수 있는 매개 변수가 있습니다.
...
이전 컨테이너에는 `FromHost` API와 함께 작동 하는 데 필요한 끝점이 없습니다. 버전 1.3에 컨테이너를 사용 하는 경우 다음 코드를 사용 해야 합니다.
...
다음은 API를 `FromEndpoint` 사용 하는 예입니다.
...
```
 ...
 업데이트 `SetSpeechSynthesisVoiceName` 된 텍스트 음성 변환 엔진을 사용 하는 컨테이너에 음성 이름이 필요 하기 때문에 함수가 호출 됩니다.
...
- 사용자 지정 끝점을 사용 하는 단일 인식 (대화형 모드)의 경우입니다. `SpeechConfig` 끝점 매개 변수 사용)을 참조 `speech_recognize_once_from_file_with_custom_endpoint_parameters()`하십시오.
- 연속 인식 (대화 모드)의 경우 위와 같이 사용자 지정 끝점을 사용 하도록를 수정 합니다. `speech_recognize_continuous_from_file()`를 참조 하세요.
- 위와 같은 샘플에서 받아쓰기를 사용 하도록 설정 하려면 (필요한 경우에만) 코드를 만든 `speech_config`후 바로 코드 `speech_config.enable_dictation()`를 추가 합니다.

받아쓰기를 사용 하도록 설정 하려면 c # `SpeechConfig.EnableDictation()` 에서 함수를 호출 합니다.
...
**답변:** 새 `FromHost` API가 있습니다. 기존 Api를 바꾸거나 수정 하지는 않습니다. 사용자 지정 호스트를 사용 하 여 음성 구성을 만드는 다른 방법을 추가 하기만 하면 됩니다.
...
호스트 `protocol://hostname:port` `:port` 의 형식은 선택적입니다 (아래 참조).
...
위의 Python 샘플 이지만 대신 매개 변수 `host` 를 사용 합니다 `endpoint`.
