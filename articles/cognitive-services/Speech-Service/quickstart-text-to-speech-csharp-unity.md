...
여기 장면에는 합성을 위한 텍스트를 입력하는 입력 필드, 음성 합성을 트리거하는 단추, 결과를 표시하는 텍스트 필드로 구성된 최소한의 UI만 추가됩니다.

* [계층 구조 창](https://docs.unity3d.com/Manual/Hierarchy.html)(기본적으로 왼쪽에 있음)에는 Unity에서 새 프로젝트로 만든 장면 샘플이 표시됩니다.
* 계층 구조 창의 위쪽에 있는 **만들기** 단추를 클릭하고, **UI** > **입력 필드**를 차례로 선택합니다.
* 이렇게 하면 계층 구조 창에서 볼 수 있는 3개의 게임 개체, 즉 **Canvas** 개체 내에 중첩된 **입력 필드** 개체와 **EventSystem** 개체가 만들어집니다.
* [장면 보기](https://docs.unity3d.com/Manual/UsingTheSceneView.html)에서 캔버스와 입력 필드를 잘 볼 수 있도록 [장면 보기](https://docs.unity3d.com/Manual/SceneViewNavigation.html)를 살펴봅니다.
* 계층 구조 창에서 **입력 필드** 개체를 클릭하면 [검사기 창](https://docs.unity3d.com/Manual/UsingTheInspector.html)(기본적으로 오른쪽에 있음)에 설정이 표시됩니다.
* 입력 필드가 캔버스 중앙에 있도록 **Pos X** 및 **Pos Y** 속성을 **0**으로 설정합니다.
* 계층 구조 창의 위쪽에 있는 **만들기** 단추를 다시 클릭하고, **UI** > **단추**를 선택하여 단추를 만듭니다.
* 계층 구조 창에서 **Button** 개체를 클릭하여 [검사기 창](https://docs.unity3d.com/Manual/UsingTheInspector.html)(기본적으로 오른쪽에 있음)에 설정을 표시합니다.
* 단추와 입력 필드가 겹치지 않도록 **Pos X** 및 **Pos Y** 속성을 각각 **0** 및 **-48**로 설정하고, **너비** 및 **높이** 속성을 각각 **160** 및 **30**으로 설정합니다.
...
* 텍스트 필드와 입력 필드가 겹치지 않도록 **Pos X** 및 **Pos Y** 속성을 각각 **0** 및 **80**으로 설정하고, **너비** 및 **높이** 속성을 각각 **320** 및 **80**으로 설정합니다.
* 계층 구조 창의 위쪽에 있는 **만들기** 단추를 다시 클릭하고, **오디오** > **오디오 원본**을 선택하여 오디오 원본을 만듭니다.
...
1. Unity 편집기로 돌아가서 스크립트를 구성 요소로 게임 개체 중 하나에 추가해야 합니다.
   ...
   * Hello World 구성 요소에는 `HelloWorld` 클래스의 공용 속성과 일치하는 **출력 텍스트**, **입력 필드**, **말하기 단추** 및 **오디오 원본**이라는 초기화되지 않은 두 가지 속성이 있습니다.
     두 속성을 연결하려면 개체 선택기(속성 오른쪽에 있는 작은 원형 아이콘)를 클릭하고, 앞에서 만든 텍스트 및 단추 개체를 선택합니다.

     > [!NOTE]
     > 입력 필드와 단추에는 중첩 텍스트 개체도 있습니다. 실수로 텍스트 출력을 선택하지 않도록 합니다(또는 혼란을 방지하기 위해 검사기 창의 이름 필드를 사용하여 텍스트 개체의 이름을 변경합니다).

## <a name="run-the-application-in-the-unity-editor"></a>Unity 편집기에서 애플리케이션 실행

* Unity 편집기 도구 모음(메뉴 모음 아래에 있음)에서 **재생** 단추를 누릅니다.

* 앱이 시작되면 입력 필드에 텍스트를 입력하고 단추를 클릭합니다. 해당 텍스트는 Speech Services로 전송되고 음성으로 합성되어 스피커로 재생됩니다.

  [![Unity 게임 창에서 실행되는 빠른 시작의 스크린샷](media/sdk/qs-tts-csharp-unity-output-inline.png)](media/sdk/qs-tts-csharp-unity-output-expanded.png#lightbox)

* [콘솔 창](https://docs.unity3d.com/Manual/Console.html)에서 디버그 메시지를 확인합니다.

* 음성 합성이 완료되면 Unity 편집기 도구 모음에서 **재생** 단추를 클릭하여 앱을 중지합니다.
