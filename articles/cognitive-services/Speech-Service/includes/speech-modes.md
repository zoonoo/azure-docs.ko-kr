...
- 최대 인식 제한은 .20s입니다.
- 호출에 대 한 일반적인 요청 `RecognizeOnceAsync`은입니다.
...
- 를 사용 하 여 `StartContinuousRecognition`호출 됩니다.

**받아쓰기**
- 사용자가 문장 부호를 지정 하 여 지정할 수 있습니다.
- 인식을 시작 하는 API `EnableDictation` 호출에 `SpeechConfig` 관계 없이 개체에를 지정 하 여 참조에서 호출 됩니다.
- 1 개의<sup>st</sup> 파티 클러스터는 `speech.fragment` 중간 결과의 메시지를 반환 하 고, 3 `speech.hypothesis` 개의<sup>rd</sup> 파티 반환 메시지를 반환 합니다.