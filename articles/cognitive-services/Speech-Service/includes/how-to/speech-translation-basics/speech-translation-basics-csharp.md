...
이 문서의 예를 실행 하려면 *Program.cs* 파일의 맨 위에 `using` 다음 문을 포함 합니다.
...
음성 번역의 일반적인 작업 중 하나는 입력 (또는 원본) 언어를 지정 하는 것입니다. 입력 언어를 이탈리아어로 변경하는 방법을 살펴보겠습니다. 코드에서 [`SpeechTranslationConfig`][config] 인스턴스와 상호 작용 하 여 `SpeechRecognitionLanguage` 속성에 할당 합니다.
...
개체 `TranslationRecognizer` 는 이벤트를 `Synthesizing` 노출 합니다. 이벤트는 여러 번 발생 하며, 번역 인식 결과에서 합성 된 오디오를 검색 하는 메커니즘을 제공 합니다. 여러 언어로 번역 하는 경우 [수동 합성](#manual-synthesis)을 참조 하세요. 을 [`VoiceName`][voicename] 할당 하 여 합성 음성을 지정 하 고 `Synthesizing` 이벤트에 대 한 이벤트 처리기를 제공 하 여 오디오를 가져옵니다. 다음 예제에서는 변환 된 오디오를 *.wav* 파일로 저장 합니다.

> [!IMPORTANT]
> 이벤트 기반 합성은 단일 번역 에서만 작동 하며 여러 대상 번역 언어를 추가 **하지** 않습니다. 또한는 [`VoiceName`][voicename] 대상 번역 언어와 동일한 언어 여야 합니다. 예를 들면와 같습니다. `"de"` 에 `"de-DE-Hedda"`매핑할 수 있습니다.
...
[`Translations`][translations] 사전을 사용 하 여 번역 텍스트에서 오디오를 합성할 수 있습니다. 각 번역을 반복 하 고 번역을 합성 합니다. `SpeechSynthesizer` 인스턴스를 만들 때 개체의 `SpeechConfig` [`SpeechSynthesisVoiceName`][speechsynthesisvoicename] 속성을 원하는 음성으로 설정 해야 합니다. 다음 예제에서는 5 개의 언어로 변환 하 고 각 번역은 해당 신경망에서 오디오 파일로 합성 됩니다.
