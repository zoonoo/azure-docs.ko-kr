...
Speech SDK를 [.zip 패키지](https://aka.ms/csspeech/jsbrowserpackage)로 다운로드하여 새로 만든 폴더에 압축 해체합니다. 이로 인해 `microsoft.cognitiveservices.speech.sdk.bundle.js` 및 `microsoft.cognitiveservices.speech.sdk.bundle.js.map`을 포함하여 다섯 개의 파일이 압축 해제됩니다.
...
앱을 시작하려면 index.html 파일을 두 번 클릭하거나 선호하는 웹 브라우저를 사용하여 index.html을 엽니다. 구독 키 및 [지역](../../../../regions.md)을 입력하고 입력 음성의 텍스트 전사를 트리거할 수 있는 간단한 GUI가 제공됩니다.

## <a name="build-and-run-the-sample-via-a-web-server"></a>웹 서버를 통해 샘플 빌드 및 실행

앱을 시작하려면 즐겨 찾는 웹 브라우저를 열고 폴더를 호스트하는 공용 URL을 가리킨 다음, [지역](../../../../regions.md)을 입력하고 입력 음성의 텍스트 전사를 트리거합니다. 구성된 경우 토큰 소스에서 토큰을 획득합니다.
