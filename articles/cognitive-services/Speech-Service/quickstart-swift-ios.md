---
title: '빠른 시작: 음성 인식, Swift - Speech Service'
...
1. **Project Options** 대화 상자에서:
    ...
    1. 스토리보드를 사용하고 문서 기반 애플리케이션을 만들려면 확인란을 사용하지 않도록 설정합니다. 샘플 앱에 대한 간단한 UI는 프로그래밍 방식으로 만들어집니다.
...
1. **Objective-C 브리징 헤더** 필드에서 helloworld 대상의 Swift 프로젝트 설정에 브리징 헤더에 대한 상대 경로 `helloworld/MicrosoftCognitiveServicesSpeech-Bridging-Header.h`를 추가합니다.
   ...
   ![헤더 속성](media/sdk/qs-swift-ios-bridging-header.png)

1. 자동 생성된 `AppDelegate.swift` 파일의 내용을 다음 코드로 바꿉니다.
...
1. 자동 생성된 `ViewController.swift` 파일의 내용을 다음 코드로 바꿉니다.
...
1. 터미널에서 helloworld 디렉터리로 이동하여 `pod install` 명령을 실행합니다. 이 명령은 종속성으로 샘플 앱 및 Speech SDK가 모두 포함되는 `helloworld.xcworkspace` Xcode 작업 영역을 생성합니다. 이 작업 영역은 다음 단계에서 사용됩니다.

## <a name="build-and-run-the-sample"></a>샘플 빌드 및 실행

1. Xcode에서 `helloworld.xcworkspace` 작업 영역을 엽니다.
1. **뷰** > **디버그 영역** > **콘솔 활성화**를 선택하여 디버그 출력을 표시합니다.
1. **제품** > **대상** 메뉴의 목록에서 앱에 대한 대상으로 개발 머신에 연결된 iOS 시뮬레이터 또는 iOS 디바이스를 선택합니다.
1. 메뉴에서 **제품** > **실행**을 선택하여 iOS 시뮬레이터에서 예제 코드를 빌드하고 실행합니다. **재생** 단추를 선택할 수도 있습니다.
