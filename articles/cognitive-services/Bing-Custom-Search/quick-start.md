...
2.  **로그인** 단추를 클릭하고 MSA(Microsoft 계정)를 사용하여 포털에 로그인합니다. 
...
4.  **검색 환경**의 **활성** 탭에서 검색에 포함할 웹 사이트 하나 이상의 URL을 입력합니다.
5.  인스턴스가 결과를 반환하는지 확인하려면 오른쪽 미리 보기 창에 쿼리를 입력합니다. 결과가 없으면 새 웹 사이트를 지정합니다. Bing은 인덱싱된 공용 웹 사이트에 대해서만 결과를 반환합니다.
