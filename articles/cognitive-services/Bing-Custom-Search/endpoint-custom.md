---
title: Custom Search 끝점 | Microsoft Docs
description: Custom Search API 끝점에 대한 요약입니다.
...
## <a name="custom-search-endpoint"></a>Custom Search 끝점
Bing Custom Search API를 사용하여 결과를 가져오려면 `GET` 요청을 다음 끝점에 전송합니다. 헤더 및 URL 매개 변수를 사용하여 추가 사양을 정의합니다.
...
## <a name="custom-autosuggest-endpoint"></a>Custom Autosuggest 끝점
...
## <a name="custom-image-search-endpoint"></a>Custom Image Search 끝점
...
**Bing** API는 해당 유형에 따라 결과를 반환하는 검색 작업을 지원합니다. 모든 검색 끝점은 결과를 JSON 응답 개체로 반환합니다.  모든 끝점은 경도, 위도 및 검색 반경으로 특정 언어 및/또는 위치를 반환하는 쿼리를 지원합니다.
