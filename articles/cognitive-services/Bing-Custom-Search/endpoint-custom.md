...
description: 관심 있는 항목에 대한 맞춤형 검색 환경을 만듭니다. 사용자는 관심 있는 콘텐츠에 맞게 조정된 검색 결과를 볼 수 있습니다.
...
Custom Search 원본을 설정하는 방법을 설명하는 예제는 [자습서](./tutorials/custom-search-web-page.md)를 참조하세요. 헤더, 매개 변수, 지역/국가 코드, 응답 개체, 오류 등에 대한 자세한 내용은 [Bing Custom Search API v7](/rest/api/cognitiveservices-bingsearch/bing-custom-search-api-v7-reference) 참조를 참조하세요.

## <a name="custom-search-response-json"></a>Custom Search 응답 JSON
사용자 지정 검색 요청은 결과를 JSON 개체로 반환합니다. [응답 개체](/rest/api/cognitiveservices-bingsearch/bing-custom-search-api-v7-reference#response-objects)를 참조하세요. 
...
Custom Search 인스턴스 구성에 대한 자세한 내용은 [사용자 지정 검색 환경 구성](./define-your-custom-view.md)을 참조하세요.
...
Custom Search API를 사용하는 기본 요청의 예는 [Custom Search 빠른 시작](/azure/cognitive-services/bing-custom-search/)을 참조하세요.