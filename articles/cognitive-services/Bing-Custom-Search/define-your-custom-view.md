...
또한 포털을 사용하면 사용자가 특정 검색어를 입력할 경우 검색 결과의 맨 위에 특정 웹 페이지를 고정할 수 있습니다. **고정됨** 탭에는 특정 쿼리의 맨 위 결과로 표시되는 웹 페이지를 지정하는 쿼리 용어 및 웹 페이지 쌍 목록이 포함됩니다. 결과 고정에 대한 자세한 내용은 [순위 조정](#adjustrank)을 참조하세요.
