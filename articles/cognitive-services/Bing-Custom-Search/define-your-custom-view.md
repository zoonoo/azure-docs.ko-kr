...
1.  고 **정됨**  탭에서 결과의 맨 위에 고정할 웹 페이지의 URL 및 고정을 트리거할 쿼리를 입력합니다.  
  
2.  **미리 보기** 창에서 쿼리 용어를 입력하고 검색을 클릭합니다. 그런 다음, 사용자가 동일한 쿼리를 입력할 경우 결과의 맨 위에 고정할 웹 페이지를 결과에서 식별합니다. 위 **쪽에 고정**을 클릭합니다. 웹 페이지와 쿼리가 **고정됨** 목록에 추가됩니다. 

고 **정됨**  탭에서 핀 고정을 추적할 수 있습니다. 고정은 ‘\<쿼리\>, \<웹 페이지\>’ 쌍으로 표시됩니다. 
