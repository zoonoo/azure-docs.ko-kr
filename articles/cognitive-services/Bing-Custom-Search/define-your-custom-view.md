...
description: 포털을 사용하면 웹 조각을 지정하는 검색 인스턴스를 만들 수 있습니다(도메인, 서브 페이지, 웹 페이지).
...
| 조각 이름 | Description                                                                                                                                                                                                                                                                                                |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 도메인     | 도메인 조각에는 인터넷 도메인 내에서 찾은 모든 콘텐츠가 포함됩니다. 예들 들어 `www.microsoft.com`입니다. `www.`를 생략하면 Bing에서 도메인의 하위 도메인도 검색합니다. 예를 들어 `microsoft.com`을 지정하면 Bing에서 `support.microsoft.com` 또는 `technet.microsoft.com`의 결과도 반환합니다. |
...
포함할 조각을 잘 모르는 경우 **미리 보기** 창에서 Bing에 검색 쿼리를 보내고 원하는 조각을 선택할 수 있습니다. 가상 하드 디스크 파일에 대한 중요 정보를 제공하려면 
...
웹 조각을 **활성** 목록에 추가하면 Bing Custom Search 포털에서 탭 아래쪽에 웹 사이트 및 하위 페이지 제안을 생성합니다. 이는 Bing Custom Search가 판단하기에 사용자가 포함하기를 원할 수도 있는 조각입니다. **새로 고침** 을 클릭하여 사용자 지정 검색 인스턴스의 설정을 업데이트한 후 업데이트된 제안을 가져옵니다. 이 섹션은 제안을 사용할 수 있는 경우에만 표시됩니다.
...
| 조정 | Description |
|------------|-------------|
| 차단      | 도메인, 하위 페이지 또는 웹 페이지를 차단됨 목록으로 이동합니다. Bing에서는 선택한 사이트의 콘텐츠가 검색 결과에 표시되지 않습니다.                    |
...
또한 포털에서는 **고정** 탭을 사용하여 특정 검색어에 대한 검색 결과의 맨 위에 URL을 고정할 수 있습니다. URL 및 쿼리를 입력하여 상위 결과로 표시될 웹 페이지를 지정합니다. 검색 쿼리당 최대 하나의 웹 페이지를 고정할 수 있으며 인덱싱된 웹 페이지만 검색에 표시됩니다. 이미지 또는 비디오 검색에서는 결과 고정을 사용할 수 없습니다.
