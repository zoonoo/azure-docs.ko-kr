...
description: 포털을 사용하면 웹 조각을 지정하는 검색 인스턴스를 만들 수 있습니다. 도메인, 하위 페이지 및 웹 페이지입니다.
...
| 조각 이름 | 설명                                                                                                                                                                                                                                                                                                |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 도메인     | 도메인 조각에는 인터넷 도메인 내에서 찾은 모든 콘텐츠가 포함됩니다. `www.microsoft.com`)을 입력합니다. `www.`를 생략하면 Bing에서 도메인의 하위 도메인도 검색합니다. 예를 들어 , `microsoft.com`을 지정하면 `support.microsoft.com` Bing은 `technet.microsoft.com`또는 에서 결과를 반환합니다. |
...
**활성** 목록에 웹 조각을 추가한 후 Bing 사용자 지정 검색 포털은 탭 하단에 웹 사이트 및 하위 페이지 제안을 생성합니다. Bing 사용자 지정 검색에 포함할 수 있는 조각입니다. **새로 고침**을 클릭하여 사용자 지정 검색 인스턴스의 설정을 업데이트한 후 업데이트된 제안을 가져옵니다. 이 섹션은 제안을 사용할 수 있는 경우에만 표시됩니다.
...
**활성** 목록에서 **순위 조정** 컨트롤을 사용하거나 미리 보기 창에서 부스트 및 강등 컨트롤을 사용하여 항목을 크게 향상, 부스트 또는 강등할 수 있습니다. 활성 목록에 조각이 추가되고 그에 따라 순위가 조정됩니다.
...
또한 포털에서는 **고정 탭을** 사용하여 URL을 특정 검색어의 검색결과 상단에 고정할 수 있습니다. 검색 쿼리당 최대 하나의 웹 페이지를 고정할 수 있으며 인덱싱된 웹 페이지만 검색에 표시됩니다. 이미지 또는 비디오 검색에서는 결과 고정을 사용할 수 없습니다.

두 가지 방법으로 웹 페이지를 맨 위에 고정할 수 있습니다.

* 고정 **탭에서** 웹 페이지의 URL을 입력하여 상단에 고정하고 해당 쿼리를 입력합니다.

* **미리 보기** 창에서 검색 쿼리를 입력하고 [검색]을 클릭합니다. 쿼리에 고정할 웹 페이지를 찾고 상단에 **핀을 클릭합니다.** 웹 페이지와 쿼리가 **고정됨** 목록에 추가됩니다.
