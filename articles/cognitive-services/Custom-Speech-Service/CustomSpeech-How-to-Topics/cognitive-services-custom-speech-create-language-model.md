...
이 자습서에서는 텍스트 쿼리에 대한 사용자 지정 언어 모델 또는 사용자가 애플리케이션에 입력 하거나 언급할 것으로 예상하는 발언을 만듭니다. 그런 다음, 애플리케이션에 음성 상호 작용을 추가하려면 Microsoft에서 기존의 최신 음성 모델과 함께 이 사용자 지정 언어 모델을 사용할 수 있습니다.
...
애플리케이션에 대한 사용자 지정 언어 모델을 만들려면 예를 들어 시스템에 발화 예제 목록을 제공해야 합니다.

*   "그는 지난 주 내내 두드러기가 났습니다."
*   "그 환자는 탈장 봉합 상처가 잘 아물었습니다."

문장은 완벽한 문장이거나 문법적으로 정확할 필요는 없으며 시스템이 배포 시 발생하리라 예상하는 음성 입력을 정확하게 반영해야 합니다. 이 예제에서는 사용자가 애플리케이션으로 수행할 작업의 내용과 스타일 모두를 반영해야 합니다.
