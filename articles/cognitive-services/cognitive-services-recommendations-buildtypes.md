---
title: "빌드 형식 및 모델 품질: 추천 API | Microsoft Docs"
...
---
# <a name="build-types-and-model-quality"></a>빌드 형식 및 모델 품질
<a name="TypeofBuilds"></a>

## <a name="supported-build-types"></a>지원되는 빌드 형식
추천 API는 현재 두 가지 빌드 형식, 즉 *추천* 및 *FBT*를 지원합니다. 각각은 다양한 알고리즘을 사용하여 빌드되며 서로 다른 강점이 있습니다. 이 문서에서는 각 빌드 및 생성된 모델의 품질을 비교하는 기법에 대해 설명합니다.

[빠른 시작 가이드](cognitive-services-recommendations-quick-start.md)를 아직 완료하지 않은 경우, 먼저 완료하는 것이 좋습니다.

<a name="RecommendationBuild"></a>

### <a name="recommendation-build-type"></a>추천 빌드 형식
...
#### <a name="recommendations-build-parameters"></a>추천 빌드 매개 변수
...
### <a name="fbt-build-type"></a>FBT 빌드 형식
FBT(자주 함께 구매하는 항목) 빌드는 둘 또는 세 개의 서로 다른 제품에 대한 구매가 함께 이루어지는 횟수를 계산하는 분석을 수행합니다. 그런 다음 유사성 함수(**co-occurrences**, **Jaccard**, **lift**)에 따라 해당 집합을 정렬합니다.

**Jaccard** 및 **lift**는 공동 발생을 정규화하는 방법으로서 간주합니다.  즉, 해당 항목은 시드 항목과 함께 구매한 경우에만 반환됩니다.
...
#### <a name="fbt-build-parameters"></a>FBT 빌드 매개 변수
| 이름 | 설명 | 형식,   <br> 유효한 값 <br> (기본값) |
| --- | --- | --- |
| *FbtSupportThreshold* |모델의 보수적인 정도입니다. 모델링 시 고려할 항목의 공동 발생 횟수입니다. |정수,  <br> 3-50 <br> (6) |
| *FbtMaxItemSetSize* |FBT 집합의 항목 수를 제한합니다. |Integer    <br> 2-3 <br> (2) |
| *FbtMinimalScore* |FBT 집합이 반환된 결과에 포함되어야 할 최소 점수입니다. 높을수록 좋습니다. |Double <br> 0 이상 <br> (0) |
...
## <a name="build-evaluation-and-selection"></a>빌드 평가 및 선택
...
오프라인 평가의 목적은 추천 정밀도(추천 항목 중 하나를 구입할 사용자 수) 및 다양성(추천 항목 수)을 예측하기 위한 것입니다.
정밀도 및 다양성 메트릭 평가의 일환으로 시스템은 사용자의 샘플을 찾고 해당 사용자의 트랜잭션을 두 그룹, 즉 학습 데이터 집합과 테스트 데이터 집합으로 분할합니다.

> [!NOTE]
> 오프라인 메트릭을 사용하려면 사용 현황 데이터에 타임스탬프가 있어야 합니다.
> 학습 및 테스트 데이터 집합 간에 사용 현황을 정확하게 분할하기 위해서는 시간 데이터가 필요합니다.
> 
...
### <a name="precision-at-k"></a>정밀도-K
다음 표는 정밀도-K 오프라인 평가 결과를 나타냅니다.

| K | 1 | 2 | 3 | 4 | 5 |
| --- | --- | --- | --- | --- | --- |
| 백분율 |13.75 |18.04 |21 |24.31 |26.61 |
| 테스트에 참가한 사용자 |10,000 |10000 |10000 |10000 |10,000 |
| 고려 대상 사용자 |10,000 |10000 |10000 |10000 |10,000 |
...
#### <a name="percentile-buckets"></a>백분위수 버킷
...
#### <a name="unique-items-recommended"></a>고유한 추천 항목
고유한 추천 항목 메트릭은 평가에 대해 반환된 고유 항목 수를 나타냅니다.

#### <a name="total-items-recommended"></a>총 추천 항목
총 추천 항목 메트릭은 추천된 항목 수를 나타냅니다. 일부 항목이 중복될 수 있습니다.

<a name="ImplementingEvaluation"></a>

### <a name="offline-evaluation-metrics"></a>오프라인 평가 메트릭
...
* 필요에 따라 *splitterStrategy*(*RandomSplitter* 또는 *LastEventSplitter* 중 하나)를 선택합니다.
  *RandomSplitter*는 지정된 *randomSplitterParameters* 테스트 백분율 및 임의 초기값을 기준으로 학습 및 테스트 집합의 사용 현황 데이터를 분할합니다.
  *LastEventSplitter* 는 각 사용자에 대한 마지막 트랜잭션을 기반으로 학습 및 테스트 집합의 사용 현황 데이터를 분할합니다.
