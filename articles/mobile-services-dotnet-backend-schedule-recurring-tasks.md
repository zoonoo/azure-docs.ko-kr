...
[.NET 백 엔드](/ko-kr/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/ ".NET 백 엔드") | [JavaScript 백 엔드](/ko-kr/documentation/articles/mobile-services-schedule-recurring-tasks/ "JavaScript 백 엔드")
...
> [WACOM.NOTE]이 자습서에서는 타사 LINQ to Twitter 라이브러리를 사용하여 OAuth 2.0에서의 Twitter v1.1. API 액세스를 간소화합니다. 이 자습서를 완료하려면 LINQ to Twitter NuGet 패키지를 다운로드하여 설치해야 합니다. 자세한 내용은 [LINQ to Twitter CodePlex 프로젝트](http://linqtotwitter.codeplex.com/)(영문)를 참조하십시오.

Twitter v1.1 API 액세스 및 저장 자격 증명 등록
----------------------------------------------
...
2.  동일한 섹션에서 다음과 같은 새 앱 설정을 추가하고 자리 표시자를 포털에서 앱 설정으로 지정한 Twitter 액세스 토큰 및 액세스 토큰 암호 값으로 바꿉니다.
...
2.  SampleJob.cs 내용을 다음 코드로 바꿉니다.
    ...
    위의 코드에서 *todolistService* 및 *todolistContext* 문자열을 다운로드한 프로젝트의 네임스페이스 및 DbContext인 *mobile\_service\_name*Service 및 *mobile\_service\_name*Context로 바꾸어야 합니다.
...
2.  열린 브라우저 창에서 모바일 서비스 URL을 복사하고, URL에 `/tables/samplejob` 경로를 추가한 다음 HTTP 유틸리티(예: Fiddler)를 사용하여 이 URL에 대한 새 POST 요청을 실행합니다.

    *\*ExecuteAsync* \*메서드가 로컬 컴퓨터에서 시작됩니다.
...
2.  [Azure 관리 포털](https://manage.windowsazure.com/)에서 모바일 서비스를 클릭한 후 앱을 클릭합니다.
...
4.  스케줄러 대화 상자에서 **작업 이름**으로 *SampleJob*을 입력하고 일정 간격 및 단위를 설정한 후 확인 단추를 클릭합니다.

	![][4]

	**SampleJob**라는 새 작업이 만들어집니다. 
...
5.  방금 만든 새 작업을 클릭한 후 **한 번 실행**을 클릭하여 스크립트를 테스트합니다.
    ...
    >[WACOM.NOTE]POST 요청을 사용하여 예약된 작업을 시작할 수도 있습니다. 하지만 권한 부여 기본값은 사용자로 설정되므로 요청의 헤더에 응용 프로그램 키를 포함해야 합니다.

1.  (옵션) [Azure 관리 포털](https://manage.windowsazure.com/)에서 모바일 서비스에 연결된 데이터베이스에 대한 관리를 클릭합니다.

    ![](./media/mobile-services-dotnet-backend-schedule-recurring-tasks/manage-sql-azure-database.png)

2.  관리 포털에서 앱에서 변경한 내용을 표시하는 쿼리를 실행합니다. 쿼리는 다음 쿼리와 비슷하지만, 모바일 서비스 이름을 `todolist` 대신 스키마 이름으로 사용합니다.
...
[Register your apps for Twitter login with Mobile Services]: /ko-kr/documentation/articles/mobile-services-how-to-register-twitter-authentication
[Twitter Developers]: http://go.microsoft.com/fwlink/p/?LinkId=268300
[App settings]: http://msdn.microsoft.com/ko-kr/library/windowsazure/b6bb7d2d-35ae-47eb-a03f-6ee393e170f7
