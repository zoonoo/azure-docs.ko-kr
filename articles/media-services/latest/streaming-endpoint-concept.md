...
Media Services 계정을 만들 경우 **기본** 스트리밍 엔드포인트가 중지됨 상태로 만들어집니다. **기본** 스트리밍 끝점을 삭제할 수 없습니다. 더 많은 스트리밍 엔드포인트는 계정에서 만들 수 [있습니다(할당량 및 제한 사항](limits-quotas-constraints.md)참조).
