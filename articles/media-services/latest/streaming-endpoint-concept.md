...
제목: 스트리밍 끝점 (원본): Azure Media Services 설명: 콘텐츠를 클라이언트 플레이어 앱 또는 CDN (Content Delivery Network)로 직접 배달 하는 동적 패키징 및 스트리밍 서비스인 스트리밍 끝점 (원본)에 대해 알아봅니다. services: media-services documentationcenter: '' author: IngridAtMicrosoft 관리자: femila editor: ''
...
Media Services 계정을 만들 경우 **기본** 스트리밍 엔드포인트가 중지됨 상태로 만들어집니다. 계정으로 더 많은 스트리밍 끝점을 만들 수 있습니다 ( [할당량 및 제한](limits-quotas-constraints.md)참조).
...
최대 스트리밍 단위 제한은 일반적으로 10입니다. 계정에 대 한 제한을 높이려면 [여기](https://azure.microsoft.com/support/create-ticket/) 로 문의 하세요.
