...
|용어|설명|
|---|---|
|라이브 이벤트|**라이브 이벤트** 는 비디오, 오디오 및 실시간 메타 데이터의 라이브 스트림 수집, 코드 변환 (선택 사항) 및 패키징을 위한 파이프라인을 나타냅니다.<br/><br/>Media Services v2 Api에서 마이그레이션하는 고객의 경우 **라이브 이벤트** 는 v 2의 **채널** 엔터티를 대체 합니다. 자세한 내용은 [v2에서 v3로 마이그레이션](migrate-v-2-v-3-migration-introduction.md)을 참조 하세요.|
