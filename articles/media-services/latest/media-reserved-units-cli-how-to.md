...
ms.locfileid: "70102941"
---
# <a name="scaling-media-processing"></a>미디어 처리 크기 조정

Azure Media Services를 사용하면 MRU(미디어 예약 단위)를 관리하여 계정에서 미디어 처리의 크기를 조정할 수 있습니다. Mru는 미디어 처리 작업이 처리 되는 속도를 결정 합니다. 예약 단위 유형 **S1**, **S2** 또는 **S3** 중에서 선택할 수 있습니다. 예를 들어 **S2** 예약 단위 유형을 사용하는 경우 **S1** 유형에 비해 동일한 인코딩 작업이 더 빠르게 실행됩니다. 
...
문서의 나머지 부분에서는 [Media Services V3 CLI](https://aka.ms/ams-v3-cli-ref) 를 사용 하 여 mru 크기를 조정 하는 방법을 보여 줍니다.
...
>
> 현재는 Azure Portal을 사용 하 여 다른 v3 리소스를 관리할 수 없습니다. [REST API](https://aka.ms/ams-v3-rest-ref), [CLI](https://aka.ms/ams-v3-cli-ref) 또는 지원되는 [SDK](media-services-apis-overview.md#sdks) 중 하나를 사용하세요.
