...
* 그런 다음, [작업](./transform-jobs-concept.md)을 다른 지역에 다시 제출해야 하는 경우 `JobInputHttp`를 사용하거나 `Copy-Blob`을 사용하여 원본 자산 컨테이너에서 대체 지역의 자산 컨테이너로 데이터를 복사할 수 있습니다.
...
* Azure Event Grid를 통해 각 계정에서 `JobStateChange` 메시지를 구독합니다.
    ...
    * 어떤 이유로 Azure Event Grid 메시지가 지연되는 경우 대체 추가를 포함하여 Azure Event Grid로 작업을 모니터링하는 방법을 보여주는 [Audio Analytics 샘플](./transform-jobs-concept.md)을 참조하세요.
    * [Media Services 이벤트에 대한 Azure Event Grid 스키마](./media-services-event-schemas.md)를 참조하세요.

* [작업](./transform-jobs-concept.md)을 만들 때는 다음을 수행합니다.
