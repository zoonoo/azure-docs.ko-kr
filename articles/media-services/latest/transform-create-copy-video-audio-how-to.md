---
title: CopyVideo CopyAudio 변환 만들기
description: Media Services API를 사용하여 CopyVideo CopyAudio 변환을 만듭니다.
services: media-services
documentationcenter: ''
author: IngridAtMicrosoft
manager: femila
editor: ''
ms.assetid: ''
ms.service: media-services
ms.devlang: multiple
ms.topic: how-to
ms.tgt_pltfrm: multiple
ms.workload: na
ms.date: 11/19/2020
ms.author: inhenkel
ms.openlocfilehash: 9a3d9ff9947a97182964f805d0b8da7e3abcad69
ms.sourcegitcommit: 02bc06155692213ef031f049f5dcf4c418e9f509
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/03/2021
ms.locfileid: "106281536"
---
# <a name="create-a-copyvideo-copyaudio-transform"></a>CopyVideo CopyAudio 변환 만들기

[!INCLUDE [media services api v3 logo](./includes/v3-hr.md)]

이 문서에서는 `CopyVideo/CopyAudio` 변환을 만드는 방법을 보여줍니다.

## <a name="prerequisites"></a>필수 구성 요소

[Media Services 계정 만들기](./account-create-how-to.md)의 단계에 따라 자산을 만드는 데 필요한 Media Services 계정 및 리소스 그룹을 만듭니다.

## <a name="methods"></a>메서드

### <a name="using-the-rest-api"></a>REST API 사용

[!INCLUDE [task-create-copy-video-audio-rest.md](./includes/task-create-copy-video-audio-rest.md)]

## <a name="next-steps"></a>다음 단계

[!INCLUDE [transforms next steps](./includes/transforms-next-steps.md)]
