...
출력 자산은 작업의 결과를 저장합니다. 프로젝트는 출력 자산의 결과를 "output" 폴더로 저장하는 **DownloadResults** 함수를 정의합니다. 따라서 무엇을 다운로드했는지 볼 수 있습니다.
...
위에서 언급했듯이 **Transform** 개체는 레시피이며 **Job**은 주어진 입력 비디오 또는 오디오 콘텐츠에 **Transform**을 적용하라는 Media Services에 대한 실제 요청입니다. **Job**은 입력 비디오의 위치 및 출력 위치와 같은 정보를 지정합니다. 비디오의 위치는 HTTPS URL, SAS URL 또는 Media Service 계정에 있는 자산을 사용하여 지정할 수 있습니다. 

이 예제에서 입력 비디오는 지정된 HTTPS URL에서 업로드됩니다.  
...
작업을 완료하는 데 시간이 다소 걸리기 때문에 완료되면 알림을 받는 것이 좋습니다. 작업 완료에 대한 알림을 받는 옵션은 여러 가지입니다. 가장 간단한 옵션(여기에 표시됨)은 폴링을 사용하는 것입니다. 
...
### <a name="download-the-result-of-the-job"></a>작업 결과 다운로드
 ...
 다음 함수는 출력 자산의 결과를 "output" 폴더로 다운로드하기 때문에 직업의 결과를 검사할 수 있습니다. 
