...
1. 포털 내의 Azure Media Services 계정으로 이동한 후 Media Services 목록에서 **라이브 스트리밍**을 선택합니다.<br/>
![라이브 스트리밍 링크 선택](media/live-events-wirecast-quickstart/select-live-streaming.png)<br/>
1. **라이브 이벤트 추가**를 클릭하여 새 라이브 스트리밍 이벤트를 만듭니다.<br/>
![라이브 이벤트 아이콘 추가](media/live-events-wirecast-quickstart/add-live-event.png)<br/>
1. 라이브 이벤트 **이름** 필드에서 새 이벤트의 이름(예: *TestLiveEvent*)을 입력합니다.<br/>
![라이브 이벤트 이름 텍스트 필드](media/live-events-wirecast-quickstart/live-event-name.png)<br/>
1. **설명** 필드에서 이벤트에 대한 설명(선택 사항)을 입력합니다.
1. **통과 – 클라우드 인코딩 없음** 라디오 단추를 선택합니다.<br/>
![클라우드 인코딩 라디오 단추](media/live-events-wirecast-quickstart/cloud-encoding.png)
1. **RTMP** 라디오 단추를 선택합니다.
1. 라이브 이벤트를 준비하기 전에 이 이벤트에 대한 요금이 청구되지 않도록 하려면 시작 라이브 이벤트에 대해 **아니요** 라디오 단추를 선택해야 합니다.  (라이브 이벤트가 시작되면 청구가 시작됩니다.) ![라이브 이벤트 시작 라디오 단추](media/live-events-wirecast-quickstart/start-live-event-no.png)<br/>
...
1. 마우스로 Wirecast 계층 영역의 첫 번째 계층 위를 가리킵니다.  표시되는 **추가** 아이콘을 클릭하고, 스트림하려는 비디오 입력을 선택합니다.  마스터 계층 1 대화 상자가 열립니다.<br/>
![Wirecast 추가 아이콘](media/live-events-wirecast-quickstart/add-icon.png)
1. 메뉴에서 **비디오 캡처**를 선택한 다음, 사용하려는 카메라를 선택합니다. 카메라를 선택하면 카메라의 보기가 미리 보기 영역에 표시됩니다.
![Wirecast 비디오 샷 선택 화면](media/live-events-wirecast-quickstart/video-shot-selection.png)
...
![Wirecast 오디오 샷 선택 화면](media/live-events-wirecast-quickstart/audio-shot-select.png)
1. 주 메뉴에서 **출력 설정**을 선택합니다.  출력 대화 상자가 표시됩니다.
1. 출력 드롭다운에서 **Azure Media Services**를 선택합니다.  Azure Media Services에 대한 출력 설정은 *대부분*의 출력 설정을 자동으로 채웁니다.<br/>
...
1. 포털의 Azure Media Services 페이지로 돌아가서 **시작**을 클릭하여 라이브 스트림 이벤트를 시작합니다. (이제 청구가 시작됩니다.)<br/>
...
4. Wirecast 애플리케이션으로 전환하고, **입력 URL**을 출력 설정의 **주소** 필드에 붙여넣습니다.<br/>
...
![출력 이름 필드](media/live-events-wirecast-quickstart/output-name.png)
...
1. Media Services 목록에서 **스트리밍 엔드포인트**를 선택하여 스트리밍 엔드포인트가 실행되고 있는지 확인합니다. 스트리밍 엔드포인트 페이지로 이동합니다.<br/>
...
1. **시작**을 클릭합니다.  그러면 스트리밍 엔드포인트가 시작됩니다.<br/>
![스트리밍 엔드포인트 메뉴 항목](media/live-events-wirecast-quickstart/start.png)
