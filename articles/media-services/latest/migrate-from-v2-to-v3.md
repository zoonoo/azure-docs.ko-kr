...
ms.locfileid: "79472070"
---
# <a name="migration-guidance-for-moving-from-media-services-v2-to-v3"></a>Media Services v2에서 v3로 이동하기 위한 마이그레이션 지침

>이 URL을 복사하여 붙여넣기하여 이 페이지를 다시 방문할 시기에 `https://docs.microsoft.com/api/search/rss?search=%22Migrate+from+Azure+Media+Services+v2+to+v3%22&locale=en-us` 대한 알림을 RSS 피드 리더에 붙여넣습니다.

이 문서에서는 미디어 서비스 v2에서 v3로의 마이그레이션 지침을 제공합니다.
...
* [미디어 서비스 검토 v2 vs. v3](media-services-v2-vs-v3.md)
...
* [콘텐츠 보호는](content-key-policy-concept.md) 다중 주요 기능을 지원합니다.
...
* 작업, 특히 비디오 또는 오디오 분석과 관련된 작업의 동시성 및 성능을 제어하려면 계정에서 MRU(미디어 예약 단위)를 프로비전해야 합니다. 자세한 내용은 [미디어 처리 크기 조정](../previous/media-services-scale-media-processing-overview.md)을 참조하세요. [미디어 서비스 v3에 CLI 2.0을](media-reserved-units-cli-how-to.md)사용하거나 [Azure 포털을](../previous/media-services-portal-scale-media-processing.md)사용하거나 [v2 API를](../previous/media-services-dotnet-encoding-units.md)사용하여 Mrus를 관리할 수 있습니다. Media Services v2를 사용하든 아니면 v3 API를 사용하든, MRU를 프로비전해야 합니다.
* v3 API로 만든 Media Services 엔터티는 v2 API를 사용하여 관리할 수 없습니다.  
* V2 API의 모든 엔터티가 V3 API에 자동으로 표시되지는 않습니다.  다음은 호환되지 않는 두 버전의 엔터티의 예입니다.  
