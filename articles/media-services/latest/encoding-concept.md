...
현재 다음과 같은 기본 제공 기본 설정도 지원 됩니다.

- **EncoderNamedPreset AACGoodQualityAudio**: 192 kbps로 인코딩된 스테레오 오디오만 포함 하는 단일 MP4 파일을 생성 합니다.
- **EncoderNamedPreset. AdaptiveStreaming** (권장):이는 h.264 적응 비트 전송률 인코딩을 지원 합니다. 자세한 내용은 [비트 전송률 사다리 자동 생성](autogen-bitrate-ladder.md)을 참조하세요.
- **EncoderNamerPreset. H265AdaptiveStreaming** : AdaptiveStreaming 사전 설정와 비슷하지만 HEVC (265) 코덱을 사용 합니다. 265 video 및 스테레오 AAC audio를 사용 하 여 GOP 정렬 MP4 파일 집합을 생성 합니다. 입력 해상도, 비트 전송률 및 프레임 속도에 따라 비트 전송률 사다리를 자동으로 생성 합니다. 자동 생성 된 기본 설정은 입력 해상도를 초과 하지 않습니다. 예를 들어 입력이 720p 인 경우 출력은 가장 적합 한 720p로 유지 됩니다.
- **ContentAwareEncoding: EncoderNamedPreset** 콘텐츠 인식 인코딩에 대 한 미리 설정을 노출 합니다. 입력 콘텐츠가 지정 된 경우 서비스는 최적의 계층 수를 자동으로 결정 하 고 적응 스트리밍을 통해 배달할 적절 한 비트 전송률 및 해상도 설정을 자동으로 결정 합니다. 기본 알고리즘은 시간이 지남에 따라 지속적으로 발전 합니다. 출력에는 비디오 및 오디오가 인터리브된 MP4 파일이 포함됩니다. 자세한 내용은 [콘텐츠 인식 인코딩](content-aware-encoding.md)을 참조 하세요.
- **EncoderNamedPreset H265ContentAwareEncoding**: HEVC (265) 콘텐츠 인식 인코딩에 대 한 사전 설정을 노출 합니다. 내용 인식 인코딩을 사용 하 여 GOP 정렬 Mp4 집합을 생성 합니다. 입력 콘텐츠를 제공 하는 경우 서비스는 입력 콘텐츠에 대 한 초기 경량 분석을 수행 하 고 결과를 사용 하 여 최적의 계층 수, 적절 한 비트 전송률 및 적응 스트리밍을 통해 배달할 해상도 설정을 결정 합니다. 이 사전 설정은 저렴 하 고 보통 복잡성의 비디오에 특히 유용 합니다 .이 경우 출력 파일은 더 낮은 비트 전송률 이지만 여전히 뷰어에 좋은 경험을 제공 하는 품질에서 제공 됩니다. 출력에는 비디오 및 오디오가 인터리브된 MP4 파일이 포함됩니다.
  > [!NOTE]
  > 이제 사용 되지 않는 **ContentAwareEncoding** not ContentAwareEncodingExperimental을 사용 해야 합니다.
...
- **EncoderNamedPreset**: HEVC (265) 코덱이 1800 kbps이 고 그림 높이가 720 픽셀인 MP4 파일을 생성 하 고 스테레오 오디오가 AAC-LC 코덱으로 128 kbps로 인코딩됩니다.
- **EncoderNamedPreset**: HEVC (265) 코덱이 3500 kbps이 고 그림 높이가 1080 픽셀인 MP4 파일을 생성 하 고 스테레오 오디오가 AAC-LC 코덱으로 128 kbps로 인코딩됩니다.
- **EncoderNamedPreset**: HEVC (265) 코덱이 9500 kbps이 고 그림 높이가 2160 픽셀인 MP4 파일을 생성 하 고 스테레오 오디오가 AAC-LC 코덱으로 128 kbps로 인코딩됩니다.
...
**2020-05-01** 버전의 API 또는 Azure Portal를 통해에서 만든 계정의 경우 크기 조정 및 미디어 예약 단위는 더 이상 필요 하지 않습니다. 크기 조정은 내부적으로 서비스에 의해 자동으로 처리 됩니다.
