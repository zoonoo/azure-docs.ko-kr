...
[라이브 이벤트](https://docs.microsoft.com/rest/api/media/liveevents) 의 **LiveEventEncodingType** 표준 또는 Media Services Premium1080p로 설정 된 경우 입력 피드가 손실 된 후 12 시간 동안 계속 **실행** 되는 라이브 이벤트를 자동으로 종료 하 고, **라이브 출력이** 없는 경우 s를 실행 합니다. 그러나 라이브 이벤트가 **실행 중** 상태였던 시간에 대한 요금은 청구됩니다.
...
|상태|설명|
|---|---|
|**중지**| 자동 시작이 true로 설정 되지 않은 경우 생성 후 라이브 이벤트의 초기 상태입니다. 이 상태에서는 청구가 발생 하지 않습니다. 이 상태에서 라이브 이벤트 속성을 업데이트할 수 있지만 스트리밍은 허용되지 않습니다.|
