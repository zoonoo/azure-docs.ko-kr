...
스트림에 대한 암호화 옵션을 지정하려면 [스트리밍 정책](streaming-policy-concept.md)을 만들어 [스트리밍 로케이터](streaming-locators-concept.md)에 연결해야 합니다. [콘텐츠 키 정책](/rest/api/media/contentkeypolicies)을 만들어 [자산](assets-concept.md)에 대한 보안 액세스를 제공하는 콘텐츠 키가 최종 클라이언트에 배달되는 방식을 구성합니다. 지정된 구성의 키를 사용하여 클라이언트에게 배달하기 위해 충족되어야 하는 콘텐츠 키 정책 요구 사항(제한)을 설정해야 합니다. 콘텐츠 키 정책은 투명한 스트리밍 또는 다운로드에 필요하지 않습니다. 

보통 콘텐츠 키 정책을 [스트리밍 로케이터](streaming-locators-concept.md)와 연결합니다. 아니면 고급 시나리오에 대한 사용자 지정 스트리밍 정책을 만들 때 [스트리밍 정책](streaming-policy-concept.md) 내에서 콘텐츠 키 정책을 지정할 수 있습니다. 

## <a name="best-practices-and-considerations"></a>모범 사례 및 고려 사항

> [!IMPORTANT]
> 다음 권장 사항을 검토할 수 있습니다.

* Media Services 계정에 대한 제한된 정책 세트를 설계하고 동일한 옵션이 필요할 때마다 스트리밍 로케이터에 해당 세트를 다시 사용하는 것이 좋습니다. 자세한 내용은 [할당량 및 한도](limits-quotas-constraints.md)를 참조하세요.
* 콘텐츠 키 정책은 업데이트할 수 있습니다. 키 배달 캐시가 업데이트되고 업데이트된 정책을 적용하는 데 최대 15분 걸릴 수 있습니다. 

   정책을 업데이트하여 캐시된 콘텐츠를 사용하는 고객에게 재생 문제를 일으킬 수 있는 기존 CDN 캐시를 덮어씁니다.  
* 각 자산을 위한 새 콘텐츠 키 정책은 만들지 않는 것이 좋습니다. 동일한 정책 옵션이 필요한 자산 간에 동일한 콘텐츠 키 정책을 공유할 경우의 주요 이점은 다음과 같습니다.
   
   * 적은 수의 정책을 더 쉽게 관리할 수 있습니다.
...
키를 가져오려면 [기존 정책에서 서명 키 가져오기](get-content-key-policy-dotnet-howto.md#get-contentkeypolicy-with-secrets) 예제에 표시된 대로 `GetPolicyPropertiesWithSecretsAsync`를 사용합니다.
