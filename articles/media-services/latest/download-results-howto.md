...
Azure Media Services에서 비디오를 처리하는 경우(예: 인코딩 또는 분석) [작업](transforms-jobs-concept.md) 결과를 저장하는 출력 [자산](assets-concept.md)을 만들어야 합니다. 그런 다음, Media Service 및 Storage API를 사용하여 로컬 폴더에 이러한 결과를 다운로드할 수 있습니다. 
...
전체 코드 샘플([EncodingWithMESPredefinedPreset](https://github.com/Azure-Samples/media-services-v3-dotnet/blob/master/VideoEncoding/EncodingWithMESPredefinedPreset/Program.cs))을 참조하세요.
