---
title: 다중 DRM 콘텐츠 보호 시스템
description: 이 문서에서는 Azure Media Services를 사용하여 다중 DRM 콘텐츠 보호 시스템을 디자인하는 방법에 관한 자세한 설명을 제공합니다.
...
| **macOS** | FairPlay | FairPlay용 Safari(macOS X 10.11 이상 El Capitan의 Safari 9 이상부터)|
| **tvOS** | FairPlay | |
...
![CENC를 사용하는 DRM 하위 시스템](./media/architecture-design-multi-drm-system/media-services-generic-drm-subsystem-with-cenc.png)
...
![Media Services의 CENC](./media/architecture-design-multi-drm-system/media-services-cenc-subsystem-on-AMS-platform.png)
...
3. Media Services API를 사용하여 테스트 자산에 대한 다중 DRM 라이선스 배달 서비스를 구성합니다. 회사 또는 회사의 공급업체에서 Media Services의 라이선스 서비스 대신 사용자 지정 라이선스 서버를 사용하는 경우 이 단계를 건너뜁니다. 라이선스 배달 구성 단계에 라이선스 획득 URL을 지정할 수 있습니다. Media Services API는 권한 부여 정책 제한, 다양한 DRM 라이선스 서비스에 대한 라이선스 응답 템플릿 등과 같은 일부 세부적인 구성을 지정하는 데 필요합니다. 현재는 Azure Portal에서 이 구성에 필요한 UI를 제공하지 않습니다. API 수준 정보 및 샘플 코드에 대해서는 [PlayReady 및/또는 Widevine 동적 일반 암호화 사용](drm-protect-with-drm-tutorial.md)을 참조하세요.

4. Media Services API를 사용하여 테스트 자산에 대한 자산 배달 정책을 구성합니다. API 수준 정보 및 샘플 코드에 대해서는 [PlayReady 및/또는 Widevine 동적 일반 암호화 사용](drm-protect-with-drm-tutorial.md)을 참조하세요.
...
* 애플리케이션의 **구성** 탭에서 Azure AD의 애플리케이션에 권한을 추가합니다. 각 애플리케이션(로컬 및 배포된 버전)에 사용 권한이 필요합니다.
    ...
    ![권한](./media/architecture-design-multi-drm-system/media-services-perms-to-other-apps.png)
...
![사용자 지정 Azure AD 테넌트 도메인 계정 1](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain1.png)

**스마트 카드를 사용한 Microsoft 도메인 계정**: Microsoft 회사 IT에서 2단계 인증으로 사용자 지정한 로그인 페이지

![사용자 지정 Azure AD 테넌트 도메인 계정 2](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain2.png)

**Microsoft 계정**: 소비자를 위한 Microsoft 계정의 로그인 페이지

![사용자 지정 Azure AD 테넌트 도메인 계정 3](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain3.png)
...
![PlayReady에 EME 사용](./media/architecture-design-multi-drm-system/media-services-eme-for-playready1.png)
...
![PlayReady용 플레이어 플러그 인](./media/architecture-design-multi-drm-system/media-services-eme-for-playready2.png)
...
![Widevine에 EME 사용](./media/architecture-design-multi-drm-system/media-services-eme-for-widevine1.png)

Widevine은 보호된 비디오의 화면 캡처 만들기를 차단하지 않습니다.

![Widevine용 플레이어 플러그 인](./media/architecture-design-multi-drm-system/media-services-eme-for-widevine2.png)
...
![자격이 없는 사용자](./media/architecture-design-multi-drm-system/media-services-unentitledusers.png)
...
![대칭 키가 있는 사용자 지정 STS](./media/architecture-design-multi-drm-system/media-services-running-sts1.png)

다음 스크린샷은 X509 인증서를 통해 비대칭 키를 사용하는 시나리오를 보여 줍니다(Microsoft 최신 브라우저 사용).

![비대칭 키가 있는 사용자 지정 STS](./media/architecture-design-multi-drm-system/media-services-running-sts2.png)
