...
다음 Azure REST 명령을 사용 하면 추가 인코딩 없이 비디오와 오디오를 한 곳에서 다른 위치로 복사할 수 있습니다. `subscriptionID`, 및 값을 `resourceGroup` `accountName` 현재 작업 중인 값으로 바꿉니다. 을 설정 하 여 변환 이름을 지정 `transformName` 합니다.
