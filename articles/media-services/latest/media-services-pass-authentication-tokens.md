...
description: 클라이언트에서 Media Services v3 키 배달 서비스로 인증 토큰을 보내는 방법에 대해 알아봅니다.
...
ms.locfileid: "105572546"
---
# <a name="pass-tokens-to-the-azure-media-services-v3-key-delivery-service"></a>Azure Media Services v3 키 배달 서비스에 토큰 전달
...
## <a name="pass-a-token-through-the-http-authorization-header"></a>HTTP 권한 부여 헤더를 통해 토큰을 전달 합니다.

> [!NOTE]
> OAuth 2.0 사양마다 “Bearer” 접두사가 필요합니다. 동영상 원본을 설정하려면 **AES(JWT 토큰)** 또는 **AES(SWT 토큰)** 를 선택합니다. 토큰은 인증 헤더를 통해 전달됩니다.

## <a name="pass-a-token-via-the-addition-of-a-url-query-parameter-with-tokentokenvalue"></a>URL 쿼리 매개 변수 추가를 통해 "token = tokenvalue"로 토큰을 전달 합니다.
