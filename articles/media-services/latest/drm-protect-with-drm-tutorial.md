...
Azure Media Services를 사용하여 Microsoft PlayReady, Google Widevine 또는 Apple FairPlay 라이선스로 암호화된 스트림을 배달할 수 있습니다. 자세한 설명은 [동적 암호화를 사용하여 콘텐츠 보호](drm-content-protection-concept.md)를 참조하세요.
...
이 아티클은 [DRM으로 암호화](https://github.com/Azure-Samples/media-services-v3-dotnet-tutorials/blob/main/AMSV3Tutorials/EncryptWithDRM) 샘플에 기반합니다.

이 아티클에서 설명된 샘플에서는 다음과 같은 결과를 생성합니다.

![Azure Media Player에서 DRM으로 보호된 비디오가 포함 된 AMS](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
위에서 언급했듯이 **Transform** 개체는 레시피이며 [Job](transform-jobs-concept.md)은 주어진 입력 비디오 또는 오디오 콘텐츠에 **Transform** 을 적용하라는 Media Services에 대한 실제 요청입니다. **Job** 은 입력 비디오의 위치 및 출력 위치와 같은 정보를 지정합니다.
...
콘텐츠 키는 자산에 대한 보안 액세스를 제공합니다. DRM을 사용하여 콘텐츠를 암호화할 때는 [콘텐츠 키 정책](drm-content-key-policy-concept.md)을 만들어야 합니다. 이 정책은 콘텐츠 키를 최종 클라이언트에 배달하는 방법을 구성합니다. 콘텐츠 키는 스트리밍 로케이터와 연결됩니다. Media Services는 권한 있는 사용자에게 암호화 키 및 라이선스를 배달하는 키 배달 서비스도 제공합니다.

지정된 구성을 사용하여 키를 배달하기 위해 충족되어야 하는 **콘텐츠 키 정책** 에 대한 요구 사항(제한)을 설정해야 합니다. 이 예제에서는 다음 구성 및 요구 사항을 설정합니다.

* 구성

    [PlayReady](drm-playready-license-template-concept.md) 및 [Widevine](drm-widevine-license-template-concept.md) 라이선스를 Media Services 라이선스 배달 서비스에서 배달할 수 있도록 구성합니다. 이 샘플 앱이 [FairPlay](drm-fairplay-license-overview.md) 라이선스를 구성하지 않았지만 FairPlay를 구성하는 데 사용할 수 있는 메서드가 포함됩니다. 또 다른 옵션으로 FairPlay 구성을 추가할 수 있습니다.
...
> [!IMPORTANT]
> 사용자 지정 [스트리밍 정책](stream-streaming-policy-concept.md)을 사용하는 경우 Media Service 계정에 대해 이러한 정책을 제한적으로 설계하고 동일한 암호화 옵션 및 프로토콜이 필요할 때마다 StreamingLocator에 다시 사용해야 합니다. Media Service 계정에는 StreamingPolicy 항목의 수에 대한 할당량이 있습니다. 각 StreamingLocator에 대해 새 StreamingPolicy를 만들지 말아야 합니다.
...
![DRM으로 보호](./media/drm-protect-with-drm-tutorial/playready_encrypted_url.png)
...
> [AES-128로 보호](drm-playready-license-template-concept.md)
