...
description: Azure 미디어 서비스에서 AudioAnalyzerPreset 및 비디오 분석기사전설정을 사용하여 오디오 및 비디오 콘텐츠를 분석하는 방법을 알아봅니다.
...
---
# <a name="analyze-video-and-audio-files-with-azure-media-services"></a>Azure 미디어 서비스를 통해 비디오 및 오디오 파일 분석

Azure 미디어 서비스 v3를 사용하면 비디오 인덱서를 통해 비디오 및 오디오 파일에서 통찰력을 추출할 수 있습니다. 이 문서에서는 이러한 통찰력을 추출하는 데 사용되는 미디어 서비스 v3 분석기 사전 설정에 대해 설명합니다. 더 자세한 인사이트가 필요하면 Video Indexer를 직접 사용해 보세요. 비디오 인덱서와 미디어 서비스 분석기 사전 설정을 사용하는 시기를 이해하려면 [비교 문서를](../video-indexer/compare-video-indexer-with-media-services-presets.md)확인하십시오.

Media Services v3 사전 설정을 사용하여 콘텐츠를 분석하려면 **AudioAnalyzerPreset** **변환을** 만들고 이러한 사전 설정 [VideoAnalyzerPreset](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#videoanalyzerpreset) 중 하나를 사용하는 **작업을** 제출합니다. **VideoAnalyzerPreset을**사용하는 방법을 보여 주는 자습서는 [Azure Media Services의 비디오 분석을](analyze-videos-tutorial-with-api.md)참조하십시오.
...
|**미리 설정 이름**|**시나리오**|**세부 정보**|
|---|---|---|
|[AudioAnalyzerPreset](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#audioanalyzerpreset)|오디오 분석|사전 설정은 음성 기록을 포함하여 미리 정의된 AI 기반 분석 작업 세트를 적용합니다. 현재 사전 설정은 단일 언어 음성을 포함하는 단일 오디오 트랙을 사용하여 콘텐츠를 처리하도록 지원합니다. BCP-47 형식의 ‘language tag-region’을 사용하여 입력에서 오디오 페이로드의 언어를 지정할 수 있습니다. 지원되는 언어는 영어('en-US'와 'en-GB'), 스페인어('es-ES', 'es-MX'), 프랑스어('fr-FR'), 이탈리아어('it-IT'), 일본어('ja-JP'), 포르투갈어('pt-BR'), 중국어('zh-CN'), 독일어('ar-EG'), 아랍어('ar-EG'), 독일어('ar-EG') 및 'ar-EG'를 말합니다. 힌디어 ('하이인'), 한국어 ('ko-KR').<br/><br/> 언어가 지정되지 않았거나 null로 설정되지 않은 경우 자동 언어 검색은 검색된 첫 번째 언어를 선택하고 파일 기간 동안 선택한 언어로 계속됩니다. 자동 언어 감지 기능은 현재 영어, 중국어, 프랑스어, 독일어, 이탈리아어, 일본어, 스페인어, 러시아어 및 포르투갈어를 지원합니다. 첫 번째 언어가 검색된 후 언어 간 동적으로 전환하는 것은 지원되지 않습니다. 자동 언어 검색 기능은 분명히 구별할 수 있는 음성이 포함된 오디오 녹음에 가장 적합합니다. 자동 언어 검색이 언어를 찾지 못하면 기록은 영어로 돌아갑니다.|
...
|[얼굴 감지기사전](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#facedetectorpreset)|비디오에 존재하는 얼굴 감지|비디오를 분석하여 존재하는 모든 얼굴을 감지할 때 사용할 설정을 설명합니다.|
...
* **오디오 전사**: 타임 스탬프가있는 음성 단어의 성적 증명서. 여러 언어가 지원됩니다.
* **스피커 인덱싱**: 스피커와 해당 음성 단어의 매핑입니다.
* **음성 감정 분석**: 오디오 전사에서 수행된 감정 분석의 출력입니다.
* **키워드**: 오디오 기록에서 추출된 키워드입니다.
...
* **얼굴 추적**: 동영상에 얼굴이 있는 시간입니다. 각 얼굴에는 얼굴 ID와 해당 축소판 그림 컬렉션이 있습니다.
* **시각적 텍스트**: 광학 문자 인식을 통해 감지되는 텍스트입니다. 텍스트는 타임스탬프가 찍혀 있으며 오디오 자막 외에 키워드를 추출하는 데도 사용됩니다.
* **키프레임**: 비디오에서 추출된 키프레임 모음입니다.
* **시각적 콘텐츠 조정**: 성인용 또는 외설적인 것으로 표시된 동영상의 일부입니다.
* **추가 :** 미리 정의된 개체 모델을 기반으로 비디오에 추가한 결과

## <a name="insightsjson-elements"></a>insights.json 요소

출력에는 비디오 또는 오디오에서 발견되는 모든 인사이트가 포함된 JSON 파일(insights.json)이 포함됩니다. JSON에는 다음 요소가 포함될 수 있습니다.
...
|---|---|
|id|얼굴 ID입니다.|
|name|얼굴 이름입니다. '알 수 없는 #0', 식별된 유명 인사 또는 고객 교육을 받은 사람이 될 수 있습니다.|
|신뢰도|얼굴 인식 신뢰도입니다.|
|description|유명인에 대한 설명입니다. |
|thumbnailId|얼굴 썸네일의 ID입니다.|
|knownPersonId|내부 ID(알려진 사람인 경우).|
|referenceId|빙 ID (빙 유명 인사인 경우).|
|referenceType|현재는 그냥 Bing입니다.|
|title|제목(예: "Microsoft의 CEO"와 같은 유명인사인 경우).|
|imageUrl|이미지 URL(유명인인 경우)입니다.|
|인스턴스|지정된 시간 범위에 얼굴이 나타난 인스턴스입니다. 인스턴스마다 thumbnailsId가 있습니다. |
...
|SpeakerLongestMonolog|화자의 가장 긴 단독 발언입니다. 스피커에 모노로그 내부에 무음이 있으면 포함됩니다. 단독 발언의 시작과 끝 부분에 있는 침묵은 제거됩니다.|
...
visualContentModeration 블록에는 Video Indexer에서 잠재적 성인 콘텐츠가 있는 것으로 확인된 시간 범위가 포함됩니다. visual콘텐츠 모드레이션이 비어 있으면 식별된 성인 콘텐츠가 없습니다.

성인 또는 외설 콘텐츠가 포함된 것으로 확인된 비디오는 프라이빗 보기만 가능합니다. 사용자는 콘텐츠에 대한 사용자 검토 요청을 제출할 수 `IsAdult` 있으며, 이 경우 특성에는 인간 검토 결과가 포함됩니다.
