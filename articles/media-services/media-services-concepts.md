...
##<a id="encoding"></a>인코딩 \\ 패키징

###인코딩

인터넷을 통해 디지털 비디오를 배달하려면 미디어를 압축해야 합니다. 디지털 비디오 파일은 크기가 상당히 크기 때문에 인터넷을 통해 전달하거나 고객의 장치에서 제대로 표시하지 못할 수 있습니다. 사람들은 셋톱 박스가 있는 TV, 데스크톱 PC에서 태블릿 및 스마트폰에 이르는 다양한 장치에서 비디오를 시청합니다. 이러한 각 장치는 대역폭 및 압축 요구 사항이 다릅니다. 인코딩은 압축 프로그램/압축 풀기 프로그램이나 코덱을 사용하여 비디오 및 오디오를 압축하는 프로세스입니다.

코드 변환은 인코딩된 비디오를 촬영하는 프로세스이며 이 비디오를 다른 인코딩 형식으로 다시 인코딩합니다. 대부분의 카메라는 비디오를 어느 정도 인코딩하므로 Azure 미디어 서비스에서 수행된 대부분의 인코딩 작업은 기술적으로 코드 변환됩니다.
...
온-프레미스 라이브 인코더 또는 트랜스코더는 카메라에서 스트리밍되는 오디오 및/또는 비디오를 다중 비트 전송률 RTMP 또는 부드러운 스트리밍 형식으로 변환합니다. 그런 다음 트랜스코더는 적응 비트 전송률 RTMP 또는 부드러운 스트림을 미디어 서비스 채널로 푸시합니다. 그러면 미디어 서비스에서 이벤트를 라이브로 브로드캐스트합니다.
...
기본적으로 미디어 서비스 계정에 5개의 채널을 추가할 수 있습니다. 더 높은 한도를 요청하려면 [할당량 및 제한 사항](media-services-quotas-and-limitations.md)을 참조하세요
...
미디어 서비스는 PlayReady 라이선스 및 AES 암호화되지 않은 키를 인증된 클라이언트에 배달하는 서비스를 제공합니다. Azure 클래식 포털, REST API 또는 Media Services SDK for .NET을 사용하여 라이선스 및 키에 대한 권한 부여 및 인증 정책을 구성할 수 있습니다.
