---
title: HEVC에 대한 Azure MS-SSTR(Media Services - 부드러운 스트리밍 프로토콜) 수정 사항 | Microsoft Docs
description: 이 사양은 Azure Media Services에서 HEVC를 사용한 조각화된 MP4 기반 라이브 스트리밍에 대한 프로토콜 및 형식을 설명합니다. HEVC 수집 및 스트리밍 지원을 포함하도록 MS-SSTR(부드러운 스트리밍 프로토콜) 설명서를 수정하였습니다. 텍스트가 단순히 설명용으로 복사되었음을 나타내는 "(변경 없음)"을 제외하고, HEVC를 제공하는 데 필요한 변경 내용만 이 문서에 지정되었습니다.
...
---
# <a name="smooth-streaming-protocol-ms-sstr-ammendment-for-hevc"></a>HEVC에 대한 MS-SSTR(부드러운 스트리밍 프로토콜) 수정 사항
