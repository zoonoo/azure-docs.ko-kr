<properties 
	pageTitle="필터 및 동적 매니페스트 | Microsoft Azure" 
...
이 토픽에서는 필터를 사용하는 것이 고객에게 매우 유용한 일반적인 시나리오를 설명하고, 프로그래밍 방식으로 필터를 만드는 방법(현재 REST API로만 필터를 만들 수 있음)을 보여 주는 토픽에 대한 링크를 제공합니다.
...
다음 토픽에서는 필터와 연결된 Media Services 엔터티에 대해 설명합니다. 또한 이 토픽에서는 프로그래밍 방식으로 필터를 만드는 방법을 보여 줍니다.
