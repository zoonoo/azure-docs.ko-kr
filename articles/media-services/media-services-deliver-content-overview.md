...
1. 입력 파일(중 2층 파일이라고 함)을 자산에 업로드합니다. 예를 들어, H.264, MP4 또는 WMV입니다.
1. 적응 비트 전송률 MP4 집합으로 자산을 인코딩하십시오.
1. 자산을 게시합니다. 
2. [동적 패키징](http://msdn.microsoft.com/library/azure/jj889436.aspx)을 사용하여 다음 형식으로 클라이언트에 콘텐츠를 제공할 수 있습니다. MPEG DASH, Apple HLS 또는 부드러운 스트리밍입니다. 

이 항목에서는 [작업](../media-services-deliver-content.md#tasks). 관련 콘텐츠 배달 수행 방법을 보여주는 항목에 대한 기본 [개념](../media-services-deliver-content.md#concepts) 및 링크의 개요를 제공합니다.

## <a id="concepts"></a>개념

다음 목록에서는 미디어 배달 시 유용한 용어 및 개념을 설명합니다.

### 로케이터

콘텐츠를 스트림하거나 다운로드하는 데 사용할 수 있는 URL을 사용자이게 제공하려면 먼저 로케이터를 만들어 자산을 "게시"해야 합니다.  로케이터는 자산에 포함된 파일에 액세스할 수 있는 진입점을 제공합니다. 미디어 서비스는 두 가지 유형의 로케이터를 지원합니다. 

- **OnDemandOrigin** 로케이터로, 미디어 (예: MPEG DASH, HLS 또는 부드러운 스트리밍) 또는 점진적으로 파일을 다운로드하는 데 사용합니다.
-  **SAS**(액세스 서명) URL 로케이터로, 미디어 파일을 로컬 컴퓨터로 다운로드하는 데 사용합니다.

**액세스 정책**은 사용 권한(읽기, 쓰기 및 목록과 같은) 및 클라이언트가 지정된 자산에 대한 액세스 권한을 갖는 기간을 정의하는데 사용됩니다. OrDemandOrigin 로케이터를 만들 때는 목록 권한(AccessPermissions.List)을 사용할 수 없습니다.
...
콘텐츠를 점진적으로 다운로드하려면 로케이터의 OnDemandOrigin 유형을 사용합니다. 다음 예제에서는 locator.r의 OnDemandOrigin 유형을 기반으로 하는 URL을 보여줍니다.
