...
이 항목에서는 콘텐츠 배달 [작업](media-services-deliver-content-overview.md#tasks)을 수행하는 방법을 보여 주는 항목에 대한 [콘텐츠 배달 개념](media-services-deliver-content-overview.md#concepts)의 개요와 링크를 제공합니다.
...
자세한 내용은 [필터리 및 동적 매니페스트](media-services-dynamic-manifest-overview.md)를 참조하세요.
...
**액세스 정책**은 사용 권한(읽기, 쓰기 및 목록과 같은) 및 클라이언트가 지정된 자산에 대한 액세스 권한을 갖는 기간을 정의하는 데 사용됩니다. OrDemandOrigin 로케이터를 만들 때는 목록 권한(AccessPermissions.List)을 사용할 수 없습니다.
