...
파일이 업로드되면 자산과 연관된 Blob 저장소에 업로드한 미디어 파일에 대한 메타데이터를 자산에 만들어야 합니다.
