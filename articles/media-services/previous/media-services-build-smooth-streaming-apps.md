...
1. **FILE** 메뉴에서 **새로**를 클릭한 다음 **프로젝트를**클릭합니다.
...
2. ** &lt;그리드&gt; ** 및 **&gt; /Grid 태그를 XAML 파일에 대 고 두 태그 간에 다음 코드를 &lt;** 붙여 넣습니다.
...
5. **MainPage** 클래스의 끝에 다음 코드를 추가합니다.
   ...
   > [!NOTE]
   > CoreDispatcher는 비UI 스레드에서 UI 스레드를 변경하는 데 사용됩니다. 디스패처 스레드에서 병목 현상이 발생할 경우 개발자는 업데이트하려는 UI 요소에서 제공하는 디스패처를 사용하도록 선택할 수 있습니다.  예를 들어:
...
## <a name="lesson-4-select-smooth-streaming-tracks"></a>제4과: 부드러운 스트리밍 트랙 선택
