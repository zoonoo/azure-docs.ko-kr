...
1. [여기](https://aka.ms/amseforv2)에서 AMS v2용 AMSE 도구를 다운로드합니다.
...
![계정 이름과 키를 입력할 수 있는 Microsoft Azure Media Services를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough001.png)
...
![Azure Media Redactor가 있는 프로세스 자산이 포함된 메뉴를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough002.png)

![First Pass: Analyze 모드가 선택된 Azure Media Redactor를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough003.png)

출력에는 얼굴 위치 데이터가 있는 주석 json 파일과 감지된 각 얼굴의 jpg가 포함됩니다. 

![분석 결과를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough004.png)

### <a name="second-pass--redact-mode"></a>2 패스 – 교정 모드

1. 1 패스의 출력으로 원본 비디오 자산을 업로드하고 기본 자산으로 설정합니다. 

    ![기본으로 업로드 및 설정 단추를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough005.png)

2. (선택 사항) 교정하려는 ID의 줄 바꿈으로 구분된 목록이 포함된 'Dance_idlist.txt' 파일을 업로드합니다. 

    ![텍스트 파일을 업로드하는 옵션을 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough006.png)

3. (선택 사항) 경계 상자 경계를 늘리는 등, annotations.json 파일을 편집합니다. 
4. 1 패스의 출력 자산을 마우스 오른쪽 단추로 클릭하고 Redactor를 선택한 후 **교정** 모드에서 실행합니다. 

    ![Second Pass: Redact 모드가 선택된 Azure Media Redactor를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough007.png)

5. 교정된 최종 출력 자산을 다운로드하거나 공유합니다. 

    ![다운로드 단추를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough008.png)
...
1.  전체 솔루션을 다운로드하고 빌드합니다. 
    ...
    ![메뉴에서 빌드 솔루션이 선택된 모습을 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough009.png)
...
3.  ffmpeg.exe 및 ffprobe.exe를 AzureMediaRedactor.exe와 동일한 출력 폴더에 복사합니다. 
    ...
    ![ffmpeg 및 ffprobe를 비롯한 폴더의 내용을 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough010.png)
...
3. Visualizer 애플리케이션을 실행하고 위의 파일을 선택합니다. 
    ...
    ![파일을 업로드하는 Azure Media Redactor를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough011.png)

4. 파일을 미리 봅니다. 오른쪽의 사이드바에서 흐리게 표시하려는 얼굴을 선택합니다. 
    
    ![은 흐리게 처리할 얼굴을 미리 보고 선택할 수 있는 Azure Media Redactor를 보여 주는 스크린샷.](./media/media-services-redactor-walkthrough/media-services-redactor-walkthrough012.png)
