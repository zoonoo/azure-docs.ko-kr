...
description: 이 문서에서는 Microsoft Azure Media Services 개념에 대한 간략한 개요와 자세한 내용을 볼 수 있는 다른 문서에 대한 링크를 제공합니다.
...
이 지원되는 인코더에 대한 자세한 내용은 [인코더](media-services-encode-asset.md)를 참조하세요.
...
- [AES-128로 보호](media-services-playready-license-template-overview.md)
...
StreamingEndpoint는 콘텐츠를 클라이언트 플레이어 애플리케이션 또는 CDN(Content Delivery Network)에 직접 전달하여 추가 배포할 수 있는 스트리밍 서비스를 나타냅니다(Azure Media Services는 이제 Azure CDN 통합을 제공함). 스트리밍 엔드포인트 서비스의 아웃바운드 스트림은 라이브 스트림이거나 Media Services 계정의 주문형 비디오 자산이 될 수 있습니다. Media Services 고객은 필요에 따라 **표준** 스트리밍 엔드포인트나 하나 이상의 **프리미엄** 스트리밍 엔드포인트를 선택합니다. 표준 스트리밍 엔드포인트는 대부분의 스트리밍 워크로드에 적합합니다. 
...
Media Services 콘텐츠 전송 워크플로의 단계 중 하나는 스트리밍할 [자산에 대한 전달 정책](/rest/api/media/operations/assetdeliverypolicy)을 구성하는 것입니다. 자산 배달 정책은 어떤 스트리밍 프로토콜(예: MPEG DASH, HLS, 부드러운 스트리밍 또는 모두)로 사용자의 자산을 동적으로 패키지할 지와 같은 사용자가 원하는 자산 배달 방법과 사용자의 자산을 동적으로 암호화할 지 여부 및 방법(봉투 또는 일반 암호화)를 Media Services에 알려줍니다.
