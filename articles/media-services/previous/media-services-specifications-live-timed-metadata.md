...
광고 또는 사용자 지정 이벤트를 클라이언트 플레이어에 쉽게 삽입할 수 있도록 하기 위해 방송사에서 비디오 내에 포함되는 타이밍 메타데이터를 사용하는 경우가 많습니다. 이러한 시나리오를 사용할 수 있도록 Media Services는 라이브 스트리밍 채널의 수집 지점에서 클라이언트 응용 프로그램까지 미디어와 함께 타이밍 메타데이터를 전송하도록 지원합니다.
...
| 실시간       | Number     | 필수 | 프레젠테이션 시간에 있는 스플라이스의 시간이어야 합니다. 단위는 소수 자릿수 초입니다.                                     |
...
| 실시간       | Number     | 필수 | 이벤트 또는 광고 스플라이스의 프레젠테이션 시간입니다.  프레젠테이션 시간과 재생 시간은 [ISO-14496-12] 부록 I에서 정의한 대로 유형 1 또는 유형 2의 SAP(스트림 액세스 포인트)와 일치해야 합니다. HLS 송신의 경우, 시간과 재생 시간은 세그먼트 경계와 일치해야 합니다. 동일한 이벤트 스트림 내의 다른 이벤트 메시지에 대한 프레젠테이션 시간과 재생 시간은 겹치지 않아야 합니다. 단위는 소수 자릿수 초입니다.

---------------------------

#### <a name="211-cancelation-and-updates"></a>2.1.1 취소 및 업데이트
...
### <a name="224-cancelation-and-updates"></a>2.2.4 취소 및 업데이트
...
HLS 플레이어 응용 프로그램 계층에서는 TYPE을 사용하여 메시지 형식을 식별하고, 메시지를 디코딩하며, 필요한 시간 변환을 적용하고, 이벤트를 처리합니다.  이벤트는 이벤트 타임스탬프에 따라 부모 트랙의 세그먼트 재생 목록에서 시간 동기화됩니다.  이러한 이벤트는 가장 가까운 세그먼트(#EXTINF 태그) 앞에 삽입됩니다.
...
| 이벤트 요소 값 | string                  | 필수      | [IETF RFC 4648](http://tools.ietf.org/html/rfc4648)에서 설명한 대로 base64 문자열로 된 이벤트 메시지입니다.                                                                                                                   |
