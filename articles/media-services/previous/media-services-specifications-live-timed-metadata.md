...
| 필드 이름 | 필드 형식 | 필수입니까? | 설명                                                                                                             |
|------------|------------|----------|--------------------------------------------------------------------------------------------------------------------------|
| cue        | 문자열     | 필수 | 이벤트 메시지입니다.  단순 모드 스플라이스를 지정하려면 "SpliceOut"이어야 합니다.                                              |
| ID         | 문자열     | 필수 | 스플라이스 또는 세그먼트를 설명하는 고유 식별자입니다. 메시지의 이 인스턴스를 식별합니다.                            |
| 기간   | 번호     | 필수 | 스플라이스의 재생 시간입니다. 단위는 소수 자릿수 초입니다.                                                                |
| elapsed    | 번호     | 옵션 | 선국(tune in)을 지원하기 위해 신호가 반복되는 경우. 이 필드는 스플라이스가 시작된 이후 경과한 프레젠테이션 시간의 양이어야 합니다. 단위는 소수 자릿수 초입니다. 단순 모드를 사용하는 경우, 이 값은 스플라이스의 원래 재생 시간을 초과하지 않아야 합니다.                                                  |
| 시간       | 번호     | 필수 | 프레젠테이션 시간에 있는 스플라이스의 시간이어야 합니다. 단위는 소수 자릿수 초입니다.                                     |
...
| 기간   | 번호     | 필수 | 알려진 경우 이벤트 또는 광고 스플라이스 세그먼트의 재생 시간입니다. 알 수 없는 경우 값은 0이어야 합니다.                                                                 |
| elapsed    | 번호     | 옵션 | 선국(tune in)하기 위해 [SCTE-35] 광고 신호가 반복되는 경우. 이 필드는 스플라이스가 시작된 이후 경과한 프레젠테이션 시간의 양이어야 합니다. 단위는 소수 자릿수 초입니다. [SCTE-35] 모드에서 이 값은 스플라이스 또는 세그먼트의 지정된 원래 재생 시간을 초과할 수 있습니다.                                                  |
| 시간       | 번호     | 필수 | 이벤트 또는 광고 스플라이스의 프레젠테이션 시간입니다.  프레젠테이션 시간과 재생 시간은 [ISO-14496-12] 부록 I에서 정의한 대로 유형 1 또는 유형 2의 SAP(스트림 액세스 포인트)와 일치해야 합니다. HLS 송신의 경우, 시간과 재생 시간은 세그먼트 경계와 일치해야 합니다. 동일한 이벤트 스트림 내의 다른 이벤트 메시지에 대한 프레젠테이션 시간과 재생 시간은 겹치지 않아야 합니다. 단위는 소수 자릿수 초입니다.
...
|--------------------|----------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| systemBitrate      | 번호         | 필수      | "0"이어야 하며, 알 수 없는 가변 비트 전송률이 있는 트랙을 나타냅니다.                                                                                                                                                                                                 |
| parentTrackName    | 문자열         | 필수      | 스파스 트랙 타임 코드가 날짜 표시줄에 맞춰진 부모 트랙의 이름이어야 합니다. 부모 트랙은 스파스 트랙이 될 수 없습니다.                                                                                                                    |
| manifestOutput     | 부울        | 필수      | 스파스 트랙이 Smooth 클라이언트 매니페스트에 포함된다는 것을 나타내려면 "true"여야 합니다.                                                                                                                                                               |
| Subtype            | 문자열         | 필수      | 네 문자 코드인 "DATA"여야 합니다.                                                                                                                                                                                                                         |
| Scheme             | 문자열         | 필수      | 메시지 구성표를 식별하는 URN 또는 URL이어야 합니다. [SCTE-35] 메시지의 경우, [SCTE-67]에 따라 메시지를 HLS, Smooth, Dash 클라이언트로 보내려면 이 메시지는 "urn:scte:scte35:2013a:bin"이어야 합니다. |
...
| timescale          | 번호         | 옵션      | 부모 트랙의 날짜 표시줄이어야 합니다.                                                                                                                                                                                                                      |
...
|----------------|-------------------------|---------------|----------------------------------------------------------------------------------------------------------------|
| 기간       | 부호 없는 64비트 정수 | 필수      | 트랙 상자에 샘플이 없고 트랙 상자에 있는 샘플의 총 재생 시간이 0이므로 0이어야 합니다. |
...
| 유형               | 따옴표가 붙은 문자열                 | 필수                                  | 메시지 구성표를 식별하는 URN 또는 URL입니다. [SCTE-35] 메시지의 경우 이 특성은 특수 값인 "scte35"를 사용합니다.                                                                                                                                |
...
| 시간               | 10진수 부동 소수점 숫자 | 필수                                  | 이벤트의 프레젠테이션 시간입니다. 단위는 소수 자릿수 초입니다.                                                                                                                                                                                                                    |
...
| 기간            | 부호 없는 32비트 정수 | 옵션      | 이벤트의 재생 시간입니다. 재생 시간을 알 수 없는 경우 이 특성은 생략해야 합니다.                                                                                                                                                 |
