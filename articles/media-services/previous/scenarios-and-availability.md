...
description: 이 문서에서는 Microsoft Azure Media Services 시나리오에 대해 간략히 설명합니다.
...
이 문서에서는 라이브 또는 주문형 콘텐츠를 제공하는 일반적인 시나리오를 보여 줍니다.
...
1. 주문형 또는 SAS 로케이터를 만들어 자산을 게시합니다. SAS 로케이터를 사용하는 경우 콘텐츠는 Azure Blob Storage에서 다운로드됩니다. 스트리밍 엔드포인트가 시작된 상태에 있을 필요가 없습니다.
...
1. 라이브 스트림을 미리 봅니다.
1. 다음을 통해 콘텐츠를 제공합니다.
    1. 일반적인 스트리밍 프로토콜(예: MPEG DASH, Smooth, HLS)을 고객에게 직접 제공
    1. 추가 배포 시 CDN(Content Delivery Network)에 제공 또는
    1. 나중에 스트리밍하기 위해 수집된 콘텐츠를 기록 및 저장(주문형 비디오)
...
![“통과” 워크플로에 사용되는 A M S 플랫폼의 주요 부분을 보여 주는 다이어그램입.](./media/scenarios-and-availability/media-services-live-streaming-current.png)
