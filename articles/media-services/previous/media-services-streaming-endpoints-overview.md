...
Azure Media Services 스트리밍 끝점 엔터티에 **Cdnprovider**, **cdnprovider**, **streamingendpointversion**속성이 추가 되었습니다. 이러한 속성의 자세한 개요는 [여기](https://docs.microsoft.com/rest/api/media/operations/streamingendpoint)를 참조하세요. 
...
Media Services 1 월 2017 릴리스부터 **표준** (미리 보기) 및 **프리미엄**의 두 가지 스트리밍 유형이 있습니다. 이러한 유형은 스트리밍 엔드포인트 버전 "2.0"의 일부입니다.


|Type|설명|
|--------|--------|  
|**Standard**|기본 스트리밍 끝점은 **표준** 형식이 며, 스트리밍 단위를 조정 하 여 프리미엄 형식으로 변경할 수 있습니다.|
|**Premium** |이 옵션은 큰 규모나 세밀한 컨트롤을 필요로 하는 전문 시나리오에 적합합니다. 스트리밍 단위를 조정 하 여 **프리미엄** 유형으로 이동 합니다.<br/>전용 스트리밍 끝점은 격리 된 환경에서 라이브 되며 리소스를 경합 하지 않습니다.|
...
|Type|StreamingEndpointVersion|ScaleUnits|CDN|청구|
|--------------|----------|-----------------|-----------------|-----------------|
|클래식|1.0|0|해당 없음|체험판|
...
동적 암호화|yes|yes
동적 패키징|yes|yes
확장성|대상 처리량까지 자동으로 확장합니다.|추가 스트리밍 단위입니다.
