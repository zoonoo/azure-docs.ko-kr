...
Media Services의 2017년 1월 릴리스부터 다음 두 가지 스트리밍 유형을 사용할 수 있습니다. **표준** (미리 보기) 및 **Premium**합니다. 이러한 유형은 스트리밍 엔드포인트 버전 "2.0"의 일부입니다.


|Type|설명|
|--------|--------|  
|**Standard**|기본 스트리밍 끝점은는 **표준** 입력, 스트리밍 단위를 조정 하 여 프리미엄 유형을 변경할 수 있습니다.|
|**Premium** |이 옵션은 큰 규모나 세밀한 컨트롤을 필요로 하는 전문 시나리오에 적합합니다. 이동 하는 **Premium** 스트리밍 단위를 조정 하 여 형식입니다.<br/>전용된 스트리밍 끝점 격리 된 환경 및 리소스에 대 한 경쟁 하지 않습니다.|
...
동적 암호화|예|예
동적 패키징|예|예
확장|대상 처리량까지 자동으로 확장합니다.|스트리밍 단위를 추가 합니다.
