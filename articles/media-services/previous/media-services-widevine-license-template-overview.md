...
| provider |string |콘텐츠 키 및 정책을 조회하는 데 사용합니다. Widevine 라이선스 배달에 Microsoft 키 배달을 사용하는 경우 이 매개 변수는 무시됩니다. |
| policy_name |string |이전에 등록된 정책의 이름입니다. 선택 사항입니다. |
...
| policy_overrides. renewal_server_url |string |이 라이선스에 대한 모든 하트비트(갱신) 요청이 지정된 URL로 리디렉션됩니다. 이 필드는 can_renew가 true인 경우에만 사용됩니다. |
