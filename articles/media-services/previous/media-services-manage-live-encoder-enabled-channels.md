...
description: 이 항목에서는 온-프레미스 인코더에서 단일 비트 전송률 라이브 스트림을 받은 다음 Media Services를 사용하여 적응 비트 전송률 스트림으로 라이브 인코딩을 수행하는 채널을 설정하는 방법에 대해 설명합니다. 적응 스트리밍 프로토콜인 HLS, 부드러운 스트림, MPEG DASH 중 하나를 사용하여 스트림을 하나 이상의 스트리밍 엔드포인트를 통해 클라이언트 재생 애플리케이션에 배달할 수 있습니다.
...
* **시작 중**. 채널이 시작되고 있습니다. 이 상태에서는 요금이 청구되지 않습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다. 오류가 발생하는 경우 채널이 중지됨 상태로 돌아갑니다.
* **실행 중**. 채널이 라이브 스트리밍을 처리할 수 있습니다. 이제 사용 요금이 청구됩니다. 추가 요금 청구를 방지하기 위해 채널을 중지해야 합니다. 
* **중지 중**. 채널이 중지되고 있습니다. 이 일시적인 상태에서는 요금이 청구되지 않습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다.
* **삭제 중**. 채널이 삭제되고 있습니다. 이 일시적인 상태에서는 요금이 청구되지 않습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다.

다음 표에서는 채널 상태가 청구 모드에 매핑되는 방식을 보여 줍니다. 

| 채널 상태 | 포널 UI 표시기 | 요금이 청구됩니까? |
| --- | --- | --- |
| 시작 중 |시작 중 |아니요(임시 상태) |
...
* **중지됨**. 생성된 채널의 초기 상태입니다. 이 상태에서, 채널 속성을 업데이트할 수 있지만 스트리밍은 허용되지 않습니다.
* **시작 중**. 채널이 시작되고 있습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다. 오류가 발생하는 경우 채널이 중지됨 상태로 돌아갑니다.
* **실행 중**. 채널이 라이브 스트리밍을 처리할 수 있습니다.
* **중지 중**. 채널이 중지되고 있습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다.
* **삭제 중**. 채널이 삭제되고 있습니다. 이 상태 중에는 업데이트나 스트리밍이 허용되지 않습니다.

다음 표에서는 채널 상태가 청구 모드에 매핑되는 방식을 보여 줍니다. 

| 채널 상태 | 포널 UI 표시기 | 청구되었는지 여부 |
| --- | --- | --- |
| 시작 중 |시작 중 |아니요(임시 상태) |
| 실행 중 |준비(실행 중인 프로그램 없음)<br/>or<br/>스트리밍(실행 중인 프로그램이 하나 이상 있음) |yes |
| 중지 중 |중지 중 |아니요(임시 상태) |
| 중지됨 |중지됨 |아닙니다. |
