...
Media Services 2.10 릴리스부터, 채널을 만들 때 채널이 입력 스트림을 받는 방법과 채널이 스트림의 라이브 인코딩을 수행할지 여부를 지정할 수 있습니다. 두 가지가 있습니다.
...
| --- | --- | --- |
| 시작하는 중 |시작하는 중 |없음(일시적인 상태) |
| 실행 중 |준비(실행 중인 프로그램이 없음)<br/>로 구분하거나 여러<br/>스트리밍(실행 중인 프로그램이 하나 이상임) |예 |
...
> [!NOTE]
> 현재 라이브 이벤트의 최대 권장 기간은 8시간입니다. 더 시간 동안 채널을 실행해야 하는 경우 amshelp@microsoft.com에 문의하세요. 라이브 인코딩의 청구 영향이 있고 라이브 인코딩 채널을 "실행 중" 상태로 두면 시간당 요금이 청구됨을 기억해야 합니다.  시간당 추가 요금 청구를 방지하려면 라이브 스트리밍 이벤트가 완료된 직후 실행 중인 채널을 중지하는 것이 좋습니다. 
...
사용자 지정 사전 설정이 필요한 경우에 문의 amshelp@microsoft.com해야 합니다.
...
| --- | --- | --- |
| 시작 중 |시작하는 중 |없음(일시적인 상태) |
| 실행 중 |준비(실행 중인 프로그램이 없음)<br/>로 구분하거나 여러<br/>스트리밍(실행 중인 프로그램이 하나 이상 있음) |예 |
| 중지 중 |중지 중 |없음(일시적인 상태) |
| 중지됨 |중지됨 |아니요 |
