...
description: 이 자습서에서는 Azure에서 **Aspera Server On Demand** 서비스를 사용하여 Media Services 계정과 연결된 스토리지 계정에 파일을 업로드하는 단계를 안내합니다.
...
**Aspera** 는 고속 파일 전송 소프트웨어입니다. Azure용 **Aspera Server On Demand**는 Azure BLOB 개체 스토리지에 직접 대용량 파일을 고속으로 업로드 및 다운로드할 수 있게 해줍니다. **Aspera On Demand**에 대한 자세한 내용은 [Aspera Cloud](http://cloud.asperasoft.com/) 사이트를 참조하세요. 
...
이 자습서에서는 Azure에서 **Aspera Server On-Demand** 서비스를 사용하여 Media Services 계정과 연결된 스토리지 계정에 파일을 업로드하는 단계를 안내합니다. 
...
2. 첫 번째 전송을 수행하세요. Aspera 전송 서비스로 전송할 때 Aspera 클라이언트를 사용하려면 다음을 완료해야 합니다. 
   ...
   3. [Azure Portal](https://portal.azure.com/)을 사용하여 Azure 미디어 계정과 연결된 스토리지 계정의 일부 정보를 검색합니다. 구체적으로, 이름과 키, 그리고 콘텐츠를 배치할 스토리지 BLOB 컨테이너 이름 등이 있습니다. 

       * 포털에서 스토리지 정보를 가져오려면 스토리지 계정을 찾고, 선택키를 클릭하고, 계정의 이름 및 키를 복사합니다.
       * 컨테이너 이름을 가져오려면 스토리지 계정을 찾고 **BLOB**을 선택하고 콘텐츠를 업로드하려는 컨테이너의 이름을 선택합니다. 

      다음은 Aspera 클라이언트 **연결 관리자**의 스크린샷입니다. 여기서 'Azure' 스토리지 유형 및 자격 증명과 BLOB 컨테이너를 지정해야 합니다.
