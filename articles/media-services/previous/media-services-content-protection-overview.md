...
저장소 암호화를 사용하면 AES 256비트 암호화를 사용하여 암호화되지 않은 콘텐츠를 로컬로 암호화할 수 있습니다. 그런 다음, Azure Storage 저장소에 업로드할 수 있습니다. 이 저장소에서는 암호화된 상태로 저장됩니다. 저장소 암호화로 보호된 자산은 자동으로 암호 해제되어 인코딩되기 전에 암호화된 파일 시스템에 배치됩니다. 그리고 필요에 따라 새 출력 자산으로 다시 업로드되기 전에 다시 암호화됩니다. 저장소 암호화를 사용하는 기본적인 사례는 디스크에 저장된 상태일 때 강력한 암호화로 고품질의 입력 미디어 파일을 보호하려는 경우입니다.
...
* [Azure Media Services에서 AES 암호화된 스트림에 대한 디버그](http://mingfeiy.com/debug-aes-encrypted-stream-azure-media-services)
