...
description: 이 항목에서는 온-프레미스 인코더에서 다중 비트 전송률 라이브 스트림을 받는 채널을 설정하는 방법에 대해 설명합니다. 적응 스트리밍 프로토콜인 HLS, 부드러운 스트리밍, DASH 중 하나를 사용하여 스트림을 하나 이상의 스트리밍 엔드포인트를 통해 클라이언트 재생 애플리케이션에 배달할 수 있습니다.
