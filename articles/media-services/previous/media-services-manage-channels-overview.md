...
Media Services를 사용 하 여 활용을 걸릴 수 있습니다 [동적 패키징](media-services-dynamic-packaging-overview.md), MPEG DASH, HLS에서 라이브 스트림에서 브로드캐스트할 수 있습니다 및 부드러운 스트리밍 서비스에 전송 되는지 기여 피드에서 형식을 지정 합니다. 뷰어는 HLS, DASH 또는 부드러운 스트리밍이 호환되는 플레이어를 사용해 라이브 스트림을 재생할 수 있습니다. 이러한 프로토콜 중 하나에서 스트림을 전달할 웹 또는 모바일 응용 프로그램에서 Azure Media Player를 사용할 수 있습니다.
