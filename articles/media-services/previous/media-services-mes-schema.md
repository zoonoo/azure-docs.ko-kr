...
## <a name="encoding"></a><a name="Encoding"></a>인코딩
다음 요소의 시퀀스를 포함합니다.  
...
| **AAC오디오** |[AAC오디오](media-services-mes-schema.md#AACAudio) |AAC 오디오 인코딩 설정입니다. |
...
## <a name="h264video"></a><a name="H264Video"></a>H264비디오
### <a name="elements"></a>요소
...
| 이름 | Type | Description |
| --- | --- | --- |
| **조건** |**xs:string** | 입력에 비디오가 없는 경우 인코더가 흑백 비디오 트랙을 삽입하도록 강제할 수 있습니다. 이렇게 하려면 조건="InsertBlackIfNoVideoBottomLayerOnly"(가장 낮은 비트 레이트에서만 비디오를 삽입하려면) 또는 조건="InsertBlackIfNoVideo"(모든 출력 비트 레이트에서 비디오를 삽입하려면)를 사용합니다. 자세한 내용은 [이](media-services-advanced-encoding-with-mes.md#no_video) 문서를 참조하세요.|

## <a name="h264layers"></a><a name="H264Layers"></a>H264레이어
...
| 이름 | Type | Description |
| --- | --- | --- |
| **H264레이어**<br/><br/> minOccurs="0" maxOccurs="unbounded" |[H264레이어](media-services-mes-schema.md#H264Layer) |H264 레이어의 컬렉션입니다. |
...
| **너비**<br/><br/> minOccurs="0" |**xs: int** |출력 비디오 프레임의 너비(픽셀)입니다.<br/><br/> 현재는 너비와 높이를 모두 지정해야 합니다. 너비와 높이는 짝수여야 합니다. |
| **높이**<br/><br/> minOccurs="0" |**xs:int** |출력 비디오 프레임의 높이(픽셀)입니다.<br/><br/> 현재는 너비와 높이를 모두 지정해야 합니다. 너비와 높이는 짝수여야 합니다.|
...
| 이름 | Type | Description |
| --- | --- | --- |
| **파일** |**xs:string** |출력 파일 이름입니다.<br/><br/> 다음 표에 설명된 매크로를 사용하여 출력 파일 이름을 작성할 수 있습니다. 예를 들어:<br/><br/> **"Outputs": [      {       "FileName": "{Basename}*{Resolution}*{Bitrate}.mp4",       "Format": {         "Type": "MP4Format"       }     }   ]** |
...
| **{코덱}** |비디오의 경우 “H264”, 오디오의 경우 “AAC”에 매핑됩니다. |
| **{비트레이트}** |비디오와 오디오가 출력 파일에 포함되는 경우 대상 비디오 비트 전송률이고, 오디오만 출력 파일에 포함되는 경우 대상 오디오 비트 전송률입니다. 사용되는 값은 Kbps 단위의 비트 전송률입니다. |
| **{채널}** |오디오가 파일에 포함되는 경우 오디오 채널 수입니다. |
| **{너비}** |비디오가 출력 파일에 포함되는 경우 이 파일의 비디오 너비(픽셀)입니다. |
| **{높이}** |비디오가 출력 파일에 포함되는 경우 이 파일의 비디오 높이(픽셀)입니다. |
| **{확장}** |출력 파일의 “Type” 속성에서 상속됩니다. 출력 파일 이름의 확장명은 “mp4”, “ts”, “jpg”, “png” 또는 “bmp” 중 하나입니다. |
| **{인덱스}** |미리 보기의 경우 필수 항목입니다. 한 번만 존재해야 합니다. |
...
### <a name="preserveresolutionafterrotation"></a><a name="PreserveResolutionAfterRotation"></a>보존해상도후회전
...
또는 **보존 해결ResolutionAfterRotation** 플래그를 사용하여 "true"로 설정할 수 있습니다(기본값은 "false"). 따라서 기본 설정에 Width = “100%”, Height = “100%”가 있고 PreserveResolutionAfterRotation이 “true”로 설정된 경우, 너비 1280픽셀, 높이 720픽셀, 회전 90도의 입력 비디오는 너비 720픽셀, 높이 1280픽셀, 회전 0도의 출력을 생성합니다. 다음 그림을 참조하세요.  
...
## <a name="pnglayer"></a><a name="PngLayer"></a>Png레이어
### <a name="element"></a>요소
