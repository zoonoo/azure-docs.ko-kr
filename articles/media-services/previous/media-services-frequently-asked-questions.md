---
title: Azure 미디어 서비스에서 자주 묻는 질문
description: 이 문서에서는 Azure Media 서비스에 대한 자주 묻는 질문에 대한 답변을 제공합니다.
...
A: 가장 일반적인 이유 중 하나는 **실행 중인** 상태에서 재생하려는 스트리밍 끝점이 없기 때문에 입니다.  
...
Q: 한 미디어 서비스 계정에서 다른 미디어 서비스 계정으로 자산을 복사하려면 어떻게 해야 합니까?
...
A: 미디어 서비스는 스트리밍 콘텐츠에 대한 URL을 작성할 때 IAssetFile.Name 속성값을 사용합니다(예: http://{AMSAccount}.mediaservices.windows.net/{GUID}/{IAssetFile.Name}/스트리밍매개변수). 이러한 이유로 백분율 인코딩은 허용되지 않습니다. **Name** 속성의 값은 다음과 같은 [퍼센트 인코딩 예약 문자](https://en.wikipedia.org/wiki/Percent-encoding#Percent-encoding_reserved_characters)중 어느 값도 가질 수 없습니다: !*();;)&=+$,??#[". 또한 ‘.’ 하나만 사용할 수 있습니다. 또한 파일 이름 확장명에는 ‘.’ 하나만 사용할 수 있습니다.
...
Q: 인코딩 프로세스 중에 비디오를 회전하는 방법은 무엇입니까?
