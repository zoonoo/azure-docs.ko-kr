...
Media Services는 키를 요청 하는 사용자를 인증 하는 여러 방법을 지원합니다. 콘텐츠 키 인증 정책에는 개방형 제한 또는 토큰 제한을 사용하여 하나 이상의 권한 부여 제한이 있을 수 있습니다. 토큰 제한 정책에는 STS(보안 토큰 서비스)에서 발급한 토큰이 수반되어야 합니다. 미디어 서비스는 간단한 웹[토큰(SWT)](https://msdn.microsoft.com/library/gg185950.aspx#BKMK_2)및 JSON 웹 토큰(JWT) 형식의 토큰을 지원합니다.
...
> 
> Media Services에서 엔터티에 액세스하는 경우 HTTP 요청에 특정 헤더 필드와 값을 설정해야 합니다. 자세한 내용은 [미디어 서비스 REST API 개발 설정을](media-services-rest-how-to-use.md)참조하십시오.
> 
> 
> 
