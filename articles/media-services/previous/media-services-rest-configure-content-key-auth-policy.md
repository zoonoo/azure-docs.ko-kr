...
Media Services는 키를 요청 하는 사용자를 인증 하는 여러 방법을 지원합니다. 콘텐츠 키 인증 정책에는 개방형 제한 또는 토큰 제한을 사용하여 하나 이상의 권한 부여 제한이 있을 수 있습니다. 토큰 제한 정책에는 STS(보안 토큰 서비스)에서 발급한 토큰이 수반되어야 합니다. Media Services는 [SWT(단순 웹 토큰)](/previous-versions/azure/azure-services/gg185950(v=azure.100)#BKMK_2) 형식 및 JWT(JSON Web Token) 형식의 토큰을 지원합니다.
