...
###<a name="problem:-unable-to-playback-the-published-stream."></a>문제: 게시된 스트림을 재생할 수 없습니다.
 
- **잠재적인 문제**: 실행 중인 스트리밍 끝점이 없거나 할당된 스트리밍 단위(배율 단위)가 없습니다. 

    **문제 해결 단계**: AMSE 도구에서 "스트리밍 끝점" 탭으로 이동하고 하나의 스트리밍 단위를 사용하여 실행 중인 스트리밍 끝점이 있는지 확인합니다. 
    


>[AZURE.NOTE] 문제 해결 단계를 수행한 후에도 여전히 성공적으로 스트리밍되지 않으면 Azure Portal을 사용하여 지원 티켓을 제출하세요.
