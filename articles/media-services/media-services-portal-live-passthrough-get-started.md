...
보관된 콘텐츠를 삭제하려면 이벤트를 중단 및 삭제한 다음 연결된 자산을 삭제합니다. 이벤트가 자산을 사용하는 경우 자산을 삭제할 수 없습니다. 이벤트를 먼저 삭제해야 합니다.

이벤트를 중단 및 삭제한 다음에도 자산을 삭제하지 않는 한 사용자는 주문형 비디오로 보관된 콘텐츠를 스트림할 수 있어야 합니다.
...
4. 새 채널 이름을 지정하고 **만들기**를 클릭합니다. 
	...
	이 채널은 또한 기본 라이브 이벤트/프로그램을 추가, 시작 및 게시합니다. 이 이벤트는 8시간의 보관 기간을 포함하도록 구성됩니다.

	더 많은 이벤트를 추가하려면 **라이브 이벤트** 단추를 누릅니다.
...
라이브 이벤트는 주문형 콘텐츠를 중지될 때 자동으로 변환합니다.

##정리

통과 채널에 대한 자세한 내용은 [다중 비트 전송률 스트림을 만드는 온-프레미스 인코더를 사용한 라이브 스트리밍](media-services-live-streaming-with-onprem-encoders.md)을 참조하세요.

- 채널에 있는 모든 이벤트/프로그램이 중지되었을 때만 채널을 중지할 수 있습니다. 채널이 중지되면 채널에 대한 요금이 발생하지 않습니다. 채널을 다시 시작해야 하는 경우 채널의 수집 URL은 동일하므로 인코더를 다시 구성하지 않아도 됩니다.
- 채널에 있는 모든 이벤트가 삭제되었을 때만 채널을 삭제할 수 있습니다.

##보관된 콘텐츠 보기

이벤트를 중단 및 삭제한 다음에도 자산을 삭제하지 않는 한 사용자는 주문형 비디오로 보관된 콘텐츠를 스트림할 수 있어야 합니다. 이벤트가 자산을 사용하는 경우 자산을 삭제할 수 없습니다. 이벤트를 먼저 삭제해야 합니다.
