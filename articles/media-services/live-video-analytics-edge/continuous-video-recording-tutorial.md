---
title: 비디오를 클라우드에 연속 녹화하고 클라우드에서 재생 자습서 - Azure
description: 이 자습서에서는 Live Video Analytics on IoT Edge를 사용하여 클라우드에 비디오를 연속 녹화하고, Azure Media Services를 사용하여 비디오에서 원하는 부분을 스트리밍하는 방법을 알아봅니다.
ms.topic: tutorial
ms.date: 05/27/2020
...
ms.locfileid: "84259990"
---
# <a name="tutorial-continuous-video-recording-to-cloud-and-playback-from-cloud"></a>자습서: 비디오를 클라우드에 연속 녹화하고 클라우드에서 재생  

이 자습서에서는 Live Video Analytics on IoT Edge를 사용하여 클라우드에 [비디오를 연속 녹화(CVR)](continuous-video-recording-concept.md)하고, Azure Media Services를 사용하여 비디오에서 원하는 부분을 스트리밍하는 방법을 알아봅니다. 이 자습서는 카메라에 기록된 내용을 수일(또는 수주일) 동안 유지해야 하는 안전, 규정 준수 등의 시나리오에 유용합니다.
...
[여기](media-graph-concept.md)에 설명된 것처럼, 미디어 그래프에서 미디어를 캡처할 위치, 처리하는 방법 및 결과를 전달해야 하는 위치를 정의할 수 있습니다. CVR을 수행하려면 RTSP 가능 카메라에서 비디오를 캡처하고, [Azure Media Services 자산](terminology.md#asset)에 연속 녹화해야 합니다. 아래 다이어그램은 해당 미디어 그래프의 그래픽 표현을 보여줍니다.

![미디어 그래프](./media/continuous-video-recording-tutorial/continuous-video-recording-overview.png)

이 자습서에서는 [Live555 Media Server](https://github.com/Azure/live-video-analytics/tree/master/utilities/rtspsim-live555)를 사용하여 빌드된 Edge 모듈 하나를 사용하여 RTSP 카메라를 시뮬레이션합니다. 미디어 그래프 내에서 [RTSP 원본](media-graph-concept.md#rtsp-source) 노드를 사용하여 라이브 피드를 가져온 다음, 비디오를 자산에 기록할 [자산 싱크 노드](media-graph-concept.md#asset-sink)로 해당 비디오를 보냅니다.
...
Visual Studio Code에서 “src/edge/deployment.template.json”을 엽니다. 이 템플릿은 에지 디바이스(Azure Linux VM)에 배포할 에지 모듈을 정의합니다. "modules" 섹션 아래에는 다음 이름을 사용하는 두 개 항목이 있습니다.

* lvaEdge – Live Video Analytics on IoT Edge 모듈
* rtspsim – RTSP 시뮬레이터

그런 다음, "src/cloud-to-device-console-app" 폴더로 이동합니다. 여기에는 앞에서 만든 appsettings.json 파일과 몇 가지 다른 파일이 함께 표시됩니다.
...
1. 다음으로, "src/edge/deployment.template.json" 파일을 마우스 오른쪽 단추로 클릭하고 "IoT Edge 배포 매니페스트 생성"을 클릭합니다. Visual Studio Code는 .env 파일의 값을 사용하여 배포 템플릿 파일에서 찾은 값을 대체합니다. 이렇게 하면 src/edge/config 폴더에 "deployment.amd64.json"이라는 매니페스트 파일이 생성됩니다.

   ![IoT Edge 배포 매니페스트 생성](./media/quickstarts/generate-iot-edge-deployment-manifest.png)
1. "src/edge/config/deployment.amd64.json"을 마우스 오른쪽 단추로 클릭하고 "단일 디바이스용 배포 만들기"를 클릭합니다.
...
1. 약 30초 후에 왼쪽 하단 섹션에서 Azure IoT Hub를 새로 고치면 다음 모듈이 배포된 에지 디바이스가 표시됩니다.
    * Live Video Analytics on IoT Edge(모듈 이름 "lvaEdge")
    * RTSP 시뮬레이터(모듈 이름 "rtspsim")
 
    ![IoT 허브](./media/continuous-video-recording-tutorial/iot-hub.png)
...
1. 그래프 인스턴스가 계속 실행되고 비디오를 녹화합니다. RTSP 시뮬레이터는 원본 비디오를 계속 반복합니다. 녹화를 중지하려면 터미널 창으로 돌아가서 "Enter" 키를 누릅니다. 리소스 정리를 위해 다음과 같은 일련의 호출을 수행합니다.
...
미디어 그래프를 실행하면 Live Video Analytics on IoT Edge 모듈은 특정 진단 및 작업 이벤트를 IoT Edge Hub로 보냅니다. 이러한 이벤트는 Visual Studio Code의 출력 창에 표시되는 메시지이며, "body" 섹션과 "applicationProperties" 섹션을 포함하고 있습니다. 이러한 섹션이 나타내는 내용을 이해하려면 [이 문서](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-messages-construct)를 참조하세요.
...
본문에는 출력 위치에 대한 정보가 포함됩니다. 여기서는 비디오가 기록되는 Azure Media Service 자산의 이름입니다. 이 값을 기록해 두어야 합니다.
...
이 이벤트는 플레이어/클라이언트가 비디오 재생을 시작하기에 충분한 데이터가 자산에 기록되었음을 나타냅니다.

applicationProperties의 "subject"는 이 메시지를 생성한 그래프의 AssetSink 노드를 참조합니다.
...
그래프 인스턴스를 비활성화하면 자산 싱크 노드는 자산에 비디오를 녹화하는 작업을 중지하고 Microsoft.Media.Graph.Operational.RecordingStopped 유형의 이벤트를 내보냅니다.
...
1. sampleAsset-CVRToAMSAsset-Sample-Graph-1이라는 이름의 자산이 나열되는데, 이것은 그래프 토폴로지 파일에서 선택한 명명 패턴입니다.
1. [자산]을 클릭합니다.
...
> [!NOTE]
> 비디오의 원본은 카메라 피드를 시뮬레이션하는 컨테이너이므로, 비디오의 타임스탬프는 그래프 인스턴스를 활성화한 시간 및 비활성화한 시간과 관련이 있습니다. [이](playback-multi-day-recordings-tutorial.md) 자습서를 통해 여러 날의 기록을 찾아서 해당 보관 파일의 일부를 보는 방법을 알아볼 수 있습니다. 또한 이 자습서에서는 화면에 표시되는 비디오에 타임스탬프가 표시됩니다.
