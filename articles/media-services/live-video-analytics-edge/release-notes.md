...
* [싱크 노드에 대 한 디스크 공간 관리](upgrading-lva-module.md#disk-space-management-with-sink-nodes)에 대 한 지원이 추가 되었습니다.
...
* [IoT Edge에서 라이브 비디오 분석](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.live-video-analytics-edge) 릴리스-lva 미디어 그래프를 관리 하는 데 도움이 되는 Visual Studio Code 확장입니다. 이 확장은 **Lva 2.0 모듈** 에서 작동 하며 슬림 하 고 사용 하기 쉬운 그래픽 인터페이스를 사용 하 여 미디어 그래프를 편집 및 관리 하는 기능을 제공 합니다.
...
* [Custom Vision 서비스](https://azure.microsoft.com/services/cognitive-services/custom-vision-service/) 를 사용 하 고 실시간 비디오를 실시간으로 [분석](custom-vision-tutorial.md) 하는 데 사용 하는 시나리오 관련 컴퓨터의 비전 모델 개발에 대 한 새로운 자습서입니다.
...
* 이제 gRPC 프레임 워크를 사용 하 여 IoT Edge에서 라이브 비디오 분석과 사용자 지정 확장 간에 높은 데이터 콘텐츠 전송 성능을 얻을 수 있습니다. 시작 하려면 [빠른](analyze-live-video-use-your-grpc-model-quickstart.md) 시작을 참조 하세요.
* 라이브 비디오 분석의 광범위 한 지역 배포와 클라우드 서비스만 업데이트 되었습니다.  
* Live Video Analytics는 이제 전 세계 25 개 이상의 지역에서 사용할 수 있습니다. 다음은 사용 가능한 모든 지역 [목록](https://azure.microsoft.com/global-infrastructure/services/?products=media-services) 입니다.  
...
* 설치 스크립트에서 사용 되지 않는 Azure 확장의 사용을 제거 합니다.
...
* 이제 자산 싱크 노드 및 신호 게이트 프로세서 노드 다운스트림의 파일 싱크 노드를 포함 하는 그래프 토폴로지를 만들 수 있습니다. 예는 [토폴로지를](https://github.com/Azure/live-video-analytics/tree/master/MediaGraph/topologies/evr-motion-assets-files) 참조 하십시오.
