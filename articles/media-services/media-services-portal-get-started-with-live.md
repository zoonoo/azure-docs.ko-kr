<properties
	pageTitle="Azure 클래식 포털을 사용하여 온-프레미스 인코더에서 다중 비트 전송률 라이브 스트림을 받는 채널 만들기 | Microsoft Azure"
...
# Azure 클래식 포털을 사용하여 온-프레미스 인코더에서 다중 비트 전송률 라이브 스트림을 받는 채널 만들기
...
Azure 클래식 포털에서 **채널** 페이지를 선택합니다. 그런 다음 **새로 만들기**를 클릭합니다. **새 라이브 채널 만들기** 페이지에서 채널의 이름을 입력합니다.
...
Wirecast를 구성하고 스트림 수집을 시작하는 방법에 대한 자세한 내용은 [Wirecast 구성](https://azure.microsoft.com/blog/2014/09/18/azure-media-services-rtmp-support-and-live-encoders/)을 참조하세요.

>[AZURE.NOTE] 인코더를 중지한 후 다시 시작해야 하는 경우 먼저 Azure 클래식 포털에서 **다시 설정** 명령을 클릭하여 채널을 다시 설정해야 합니다.
...
**지금 이 프로그램 게시**를 선택하면 게시 URL이 만들어집니다.
...
콘텐츠를 스트리밍하는 데 사용할 수 있는 URL을 사용자에게 제공하려면 먼저 로케이터를 만들어(Azure 클래식 포털을 사용하여 자산을 게시하는 경우에는 로케이터가 자동으로 만들어짐) 자산을 *게시*해야 합니다(이전 섹션에 설명되어 있음). 로케이터는 자산에 포함된 파일에 대한 액세스를 제공합니다.
...
## 추가 리소스
- [Azure 클래식 포털을 사용하여 라이브 스트리밍 시작](https://azure.microsoft.com/blog/getting-started-with-live-streaming-using-the-azure-management-portal/)
