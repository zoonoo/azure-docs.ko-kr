...
**비디오 인덱스 가져오기** API를 호출하고 응답 상태가 정상이면 자세한 JSON 출력을 응답 콘텐츠로 가져옵니다. JSON 콘텐츠에는 지정된 비디오 인사이트의 세부 정보가 포함됩니다. 이 정보에는 성적 증명서, OCRs, 얼굴, 토픽, 블록 등이 포함 됩니다. 각 정보 형식에는 비디오에 정보를 표시 하는 시간 범위 인스턴스가 포함 됩니다. 
...
|id|브랜드 ID입니다.|
|name|브랜드 이름입니다.|
|referenceId | 브랜드의 Wikipedia URL에 대한 접미사입니다. 예를 들어 "Target_Corporation"은의 [https://en.wikipedia.org/wiki/Target_Corporation](https://en.wikipedia.org/wiki/Target_Corporation)접미사입니다.
