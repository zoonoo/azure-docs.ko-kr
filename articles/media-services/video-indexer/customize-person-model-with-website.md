...
2. 페이지의 오른쪽 위 모퉁이에 있는 콘텐츠 모델 사용자 지정 단추를 클릭 합니다.
...
3. 사용자 탭을 선택 합니다.
    ...
    다른 사람 모델을 만든 경우에는이 페이지에도 나열 됩니다. 
...
1. **+ 모델 추가** 단추를 클릭 합니다.
    ...
    ![새 사용자 추가](./media/customize-face-model/add-new-person.png)
2. 모델 이름을 입력 하 고 이름 옆에 있는 확인 단추를 클릭 합니다.

    ![새 사용자 추가](./media/customize-face-model/add-new-person2.png)

    새 사람 모델을 만들었습니다. 이제 새 사람 모델에 얼굴을 추가할 수 있습니다.
3. 목록 메뉴 단추를 클릭 하 고 **+ 사용자 추가**를 선택 합니다.

    ![새 사용자 추가](./media/customize-face-model/add-new-person3.png)
    
## <a name="add-a-new-person-to-a-person-model"></a>Person 모델에 새 사용자 추가

> [!NOTE]
> Video Indexer를 사용 하 여 사용자 모델에서 이름이 같은 여러 사용자를 추가할 수 있습니다. 그러나 유용성 및 명확성을 위해 모델의 각 사용자에 게 unque 이름을 제공 하는 것이 좋습니다.

1. Person 모델에 새 얼굴을 추가 하려면 얼굴을 추가 하려는 Person 모델 옆의 목록 메뉴 단추를 클릭 합니다.
1. 메뉴에서 **+ 사용자 추가** 를 클릭 합니다.

    ![사용자에 게 새 얼굴 추가](./media/customize-face-model/add-new-face.png)
 
    팝업에 사용자의 세부 정보를 입력 하 라는 메시지가 표시 됩니다. 사용자의 이름을 입력 하 고 확인 단추를 클릭 합니다.
...
이 새 얼굴을 추가한 사람 모델을 사용 하 여 인덱싱하는 이후 비디오와 이미 인덱싱되는 현재 비디오에서이 사용자의 발생을 검색할 수 Video Indexer 합니다. 일괄 처리 프로세스 이므로 현재 비디오의 사용자를 인식 하는 데 다소 시간이 걸릴 수 있습니다.
...
1. 이름을 바꿀 Person 모델 옆의 목록 메뉴 단추를 클릭 합니다.
2. 메뉴에서 **이름 바꾸기** 를 클릭 합니다.

    ![개인 이름 바꾸기](./media/customize-face-model/rename-person.png)
3. 모델의 현재 이름을 클릭 하 고 새 이름을 입력 합니다.

    ![개인 이름 바꾸기](./media/customize-face-model/rename-person2.png)
4. 이름을 바꿀 모델의 확인 단추를 클릭 합니다.
...
1. 메뉴에서 **삭제** 를 클릭 합니다.
    ...
    ![사람 삭제](./media/customize-face-model/delete-person.png)
    
    팝업이 표시 되 고이 작업에서 개인 모델과 여기에 포함 된 모든 사용자 및 파일을 삭제 한다는 알림이 표시 됩니다. 이 작업은 취소할 수 없습니다. 

    ![사람 삭제](./media/customize-face-model/delete-person2.png)
1. 잘 모르겠으면 삭제를 다시 클릭 합니다.
...
사용자 모델의 콘텐츠를 보려면 Person 모델 이름 옆의 화살표를 클릭 합니다.
드롭다운은 해당 특정 사용자 모델의 모든 사용자를 표시 합니다. 각 사람들 옆의 목록 메뉴 단추를 클릭 하면 관리, 이름 바꾸기 및 삭제 옵션이 표시 됩니다.  

![사용자에 게 새 얼굴 추가](./media/customize-face-model/manage-people.png)

### <a name="rename-a-person"></a>개인 이름 바꾸기

1. 사용자 모델에서 개인의 이름을 바꾸려면 목록 메뉴 단추를 클릭 하 고 목록 메뉴에서 **이름 바꾸기** 를 선택 합니다.
1. 사용자의 현재 이름을 클릭 하 고 새 이름을 입력 합니다.
1. 확인 단추를 클릭 하면 해당 사용자의 이름이 바뀝니다.

### <a name="delete-a-person"></a>사람 삭제

1. 사용자 모델에서 사용자를 삭제 하려면 목록 메뉴 단추를 클릭 하 고 목록 메뉴에서 **삭제** 를 선택 합니다.
1. 팝업은이 작업을 통해 사용자가 삭제 되 고이 작업은 실행 취소할 수 없음을 나타냅니다.
1. 삭제를 다시 클릭 하면 Person 모델에서 해당 사용자가 제거 됩니다.

### <a name="manage-a-person"></a>사용자 관리

**관리**를 클릭 하면이 사용자 모델을 학습 하는 모든 얼굴이 표시 됩니다. 이러한 얼굴은이 사람 모델을 사용 하는 비디오 또는 수동으로 업로드 한 이미지에서 해당 사용자가 발생 하는 경우에 발생 합니다. 

이미지 추가를 클릭 하 여 사용자에 게 얼굴을 더 추가할 수 있습니다.
...
새 비디오에서 사용자 모델을 사용 하려면 다음을 수행 합니다.

1. 페이지 맨 위에서 **업로드** 단추를 클릭 합니다.
...
1. 드롭다운을 클릭 하 고 만든 Person 모델을 선택 합니다.

    ![업로드](./media/customize-face-model/upload3.png)
1. 페이지 맨 아래에 있는 업로드 옵션을 클릭 하면 사용자 모델을 사용 하 여 새 비디오가 인덱싱됩니다.
...
얼굴을 편집 하 고 삭제 하 여 인덱싱하는 비디오에서 인식 된 얼굴 및 검색 된 사용자를 관리할 수 있습니다.

얼굴을 삭제 하면 비디오의 정보에서 특정 얼굴이 제거 됩니다.

얼굴을 편집 하 여 비디오에서 검색 되 고 인식 될 수 있는 얼굴의 이름을 바꿉니다. 비디오에서 얼굴을 편집 하는 경우 해당 이름은 업로드 및 인덱싱을 수행 하는 동안 비디오에 할당 된 person 모델에 사람 항목으로 저장 됩니다.
...
> [!NOTE]
> Person 모델에 같은 이름의 사람이 두 명 이상 있는 경우 해당 사용자 모델을 사용 하는 비디오 내에서 해당 이름의 태그를 지정할 수 없습니다. Video Indexer 콘텐츠 모델 사용자 지정 페이지의 사용자 탭에서 해당 이름을 공유 하는 사용자만 변경할 수 있습니다. 따라서 사용자 모델에서 각 사용자에 게 고유한 이름을 지정 하는 것이 좋습니다.

1. Video Indexer 웹 사이트로 이동 하 여 로그인 합니다.
1. 계정에서 보고 편집하려는 비디오를 검색합니다.
1. 비디오에서 얼굴을 편집 하려면 Insights 탭으로 이동 하 고 창의 오른쪽 위 모퉁이에 있는 연필 아이콘을 클릭 합니다.

    ![비디오에서 얼굴 편집](./media/customize-face-model/edit-face.png)
1. 감지된 얼굴을 클릭하고 “알 수 없는 #X”(또는 이전에 얼굴에 할당된 이름)에서 이름을 변경합니다. 
1. 새 이름을 입력한 후 새 이름 옆에 있는 확인 아이콘을 클릭합니다. 그러면 새 이름이 저장 되 고 다른 현재 비디오 및 업로드 하는 이후 비디오에서이 얼굴의 모든 항목이 인식 되 고 이름이 표시 됩니다. 일괄 처리 프로세스 이기 때문에 다른 현재 비디오에서 얼굴을 인식 하는 데 다소 시간이 걸릴 수 있습니다.

비디오에서 사용 하 고 있는 개인 모델에서 기존 사람의 이름을 사용 하 여 얼굴 이름을 변경한 경우 해당 사용자의 비디오에서 검색 된 얼굴 이미지가 모델에 이미 존재 하는 항목으로 병합 됩니다. 새 이름으로 얼굴 이름을 변경한 경우 비디오에서 사용 하는 사람 모델에 새 사용자 항목이 생성 됩니다. 

    ![Edit a face in your video](./media/customize-face-model/edit-face2.png)

### <a name="delete-a-face"></a>얼굴 삭제

비디오에서 검색 된 얼굴을 삭제 하려면 Insights 창으로 이동 하 고 창의 오른쪽 위 모퉁이에 있는 연필 아이콘을 클릭 합니다. 얼굴 이름 아래의 삭제 옵션을 클릭 합니다. 이렇게 하면 삭제한 얼굴이 비디오에서 제거됩니다. 사용자의 얼굴은 표시 되는 다른 비디오에서 여전히 검색 되지만, 해당 비디오를 인덱싱한 후에도 해당 비디오에서 얼굴을 삭제할 수 있습니다. 이름이 지정 된 사용자는 사용자 모델에서 사용자를 특별히 삭제 하지 않는 한 얼굴을 삭제 한 비디오를 인덱싱하는 데 사용 된 사람 모델에도 계속 존재 합니다.
