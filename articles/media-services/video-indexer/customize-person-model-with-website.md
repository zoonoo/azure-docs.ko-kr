---
title: Video Indexer 웹 사이트를 사용하여 인물 모델 사용자 지정
titleSuffix: Azure Media Services
description: Video Indexer 웹 사이트를 사용하여 인물 모델을 사용자 지정하는 방법을 알아봅니다.
...
Video Indexer는 비디오 콘텐츠에서 유명인 인식 기능을 지원합니다. 유명인 인식 기능은 IMDB, Wikipedia, 상위 LinkedIn 영향력 행사자 등 일반적으로 요청되는 데이터 원본을 기준으로 하는 약 백만 개의 얼굴을 처리합니다. 자세한 개요는 [Video Indexer에서 인물 모델 사용자 지정](customize-person-model-overview.md)을 참조하세요.

Video Indexer 웹 사이트를 사용하여 이 항목에 설명된 대로 비디오에서 감지된 얼굴을 편집할 수 있습니다. [API를 사용하여 인물 모델 사용자 지정](customize-person-model-with-api.md)에 설명된 대로 API를 사용할 수도 있습니다.

## <a name="central-management-of-person-models-in-your-account"></a>계정에서 인물 모델의 중앙 관리

1. 계정에서 인물 모델을 보고 편집하고 삭제하려면 Video Indexer 웹 사이트로 이동하여 로그인합니다.
...
## <a name="add-a-new-person-to-a-person-model"></a>인물 모델에 새 인물 추가

> [!NOTE]
> Video Indexer를 사용하여 인물 모델에서 이름이 같은 여러 인물을 추가할 수 있습니다. 그러나 유용성과 명확성을 위해 모델의 각 인물에게 고유한 이름을 제공하는 것이 좋습니다.
...
1. 메뉴에서 **+ 인물 추가** 를 선택합니다.
    ...
    그런 다음, 파일 탐색기에서 선택하거나 얼굴 이미지를 끌어서 놓을 수 있습니다. Video Indexer는 모든 표준 이미지 파일 형식(예: JPG, PNG 등)을 사용합니다.

    Video Indexer는 새 얼굴을 추가한 인물 모델을 사용하여 인덱싱한 이후의 비디오와 이미 인덱싱한 현재 비디오에서 이 인물을 감지할 수 있습니다. 일괄 처리 프로세스이므로 현재 비디오의 인물을 인식하는 데 다소 시간이 걸릴 수 있습니다.
...
1. 페이지 하단에서 **업로드** 옵션을 선택하면 새 비디오가 인물 모델을 사용하여 인덱싱됩니다.

업로드하는 동안 인물 모델을 지정하지 않으면 Video Indexer는 계정의 기본 인물 모델을 사용하여 비디오를 인덱싱합니다.

## <a name="use-a-person-model-to-reindex-a-video"></a>인물 모델을 사용하여 비디오를 다시 인덱싱

인물 모델을 사용하여 컬렉션의 비디오를 다시 인덱싱하려면 Video Indexer 홈페이지의 계정 비디오로 이동하여 다시 인덱싱하려는 비디오의 이름을 마우스로 가리킵니다.

비디오를 편집, 삭제, 다시 인덱싱하는 옵션이 표시됩니다.
...
> [!NOTE]
> 인물 모델에 이름이 같은 인물이 두 명 이상 있는 경우 해당 인물 모델을 사용하는 비디오 내에서 이름을 태그할 수 없습니다. Video Indexer 내 콘텐츠 모델 사용자 지정 페이지의 사람 탭에서 해당 이름을 공유하는 인물만 변경할 수 있습니다. 따라서 인물 모델의 각 인물에게 고유한 이름을 제공하는 것이 좋습니다.
