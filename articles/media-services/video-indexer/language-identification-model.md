...
API를 사용 하 여 [비디오를 인덱싱하거나 인덱싱할 때](https://api-portal.videoindexer.ai/docs/services/operations/operations/Re-Index-Video?) `auto detect` 매개 변수에서 옵션을 선택 합니다 `sourceLanguage` .
...
해당 언어에 대 한 신뢰도가 speech 면 가장 가능성이 높은 언어에 따라 비디오를 합니다 `> 0.6` . Video Indexer 언어가 자신을 식별할 수 없는 경우 음성 언어는 영어 라고 가정 합니다. 

모델 기준 언어는 insights JSON에서 특성으로 사용할 수 있습니다 `sourceLanguage` (root/비디오/정보에서). 특성에서 해당 신뢰 점수를 사용할 수도 있습니다 `sourceLanguageConfidence` .
...
* Video Indexer 높은 신뢰도 ()로 언어를 식별할 수 없는 경우 `>0.6` 대체 언어는 영어입니다.
