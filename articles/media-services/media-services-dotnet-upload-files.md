...
아래 예제는 .NET SDK Extensions를 사용하여 단일 파일을 업로드하는 방법을 보여 줍니다. 이 경우 **CreateFromFile** 메서드가 사용되지만 비동기 버전(**CreateFromFileAsync**)도 사용됩니다. **CreateFromFile** 메서드를 사용하면 파일 이름, 암호화 옵션 및 파일의 업로드 과정을 보고하기 위한 콜백을 지정할 수 있습니다.
