---
title: HEVC에 대한 부드러운 스트리밍 프로토콜(MS-SSTR) 개정 - Azure
...
이 문서는 부드러운 스트리밍 매니페스트에서 HEVC 비디오 코덱('hev1' 또는 'hvc1' 형식 트랙 사용)의 시그널링에 대한 기술 구현 요구 사항을 제공하며, 표준 참조는 현재 MPEG 표준을 참조하도록 업데이트됩니다. HEVC, HEVC의 일반 암호화 및 ISO 기본 미디어 파일 형식의 상자 이름이 최신 사양과 일치하도록 업데이트되었습니다. 
...
>  **구성 시간:** [[ISO/IEC-14496-12]에](https://go.microsoft.com/fwlink/?LinkId=183695)정의된 대로 클라이언트에서 샘플이 표시되는 시간입니다.
> 
>   **CENC**: Common Encryption으로, [ISO/IEC 23001-7] 제2판에 정의되어 있습니다.
> 
>   **디코딩 시간:** [[ISO/IEC 14496-12:2008]에](https://go.microsoft.com/fwlink/?LinkId=183695)정의된 대로 클라이언트에서 샘플을 디코딩하는 데 필요한 시간입니다.
...
> 
>   **프레젠테이션:** 단일 동영상을 재생하는 데 필요한 모든 **스트림** 및 관련 메타데이터 집합. **요청:** [[RFC2616]](https://go.microsoft.com/fwlink/?LinkId=90372) **응답에** 정의된 대로 클라이언트에서 서버로 전송된 HTTP 메시지: [[RFC2616]에](https://go.microsoft.com/fwlink/?LinkId=90372) 정의된 대로 서버에서 클라이언트로 전송된 HTTP 메시지
> 
>   **샘플:****미디어**가 저장되고 처리되는 가장 작은 기본 단위(예: 프레임).
> 
>   **5월, 해야 할, 안, 해서는 안됩니다:** 이러한 용어(모든 대문자)는 [[RFC2119]](https://go.microsoft.com/fwlink/?LinkId=90317) 선택적 동작사용의 모든 문에 설명된 대로 5월, 해야 또는 안 됩니다.
...
> 
>   [RFC2119] 브래드너, S., "요구 사항 수준을 나타내는 RFC에서 사용하기위한 핵심 단어", BCP 14, RFC 2119, 1997 년 3 월,[https://www.rfc-editor.org/rfc/rfc2119.txt](https://go.microsoft.com/fwlink/?LinkId=90317)
...
> 
>   [RFC3548] 요제프슨, S., Ed., "Base16, Base32 및 Base64 데이터 인코딩", RFC 3548, 2003년 7월,[https://www.ietf.org/rfc/rfc3548.txt](https://go.microsoft.com/fwlink/?LinkId=90432)
> 
>   [RFC5234] 크로커, D., Ed., 그리고 Overell, P., "구문 사양에 대한 증강 된 BNF : ABNF", STD 68, RFC 5234, 2008 년 1 월,[https://www.rfc-editor.org/rfc/rfc5234.txt](https://go.microsoft.com/fwlink/?LinkId=123096)
...
>   구현자는 [[ISO/IEC-14496-12]에](https://go.microsoft.com/fwlink/?LinkId=183695) 지정된 대로 MPEG4-RA에 확장 코드를 등록하여 확장이 충돌하지 않도록 할 수 있습니다.
...
> 1. (변경 없음)
> 
>    권장값은 분수 프레임 속도 비디오(예: 30/1.001 Hz)를 포함하는 비디오 프레임 및 조각의 정확한 지속 시간을 나타내는 90000입니다.
...
>
> * "hvc1": 이 트랙의 비디오 샘플은 [ISO/IEC-14496-15]에 지정된 'hvc1' 샘플 설명 형식을 사용하여 HEVC 비디오를 사용합니다.
> 
...
> 
>   * TrackElement가 HEVC 비디오를 설명할 때 **FourCC** 필드는 **"hev1" 또는 "hvc1"과** 같아야 합니다. **"hvc1"**
> 
>   **CodecPrivateData** 필드에는 ABNF [[RFC5234]에](https://go.microsoft.com/fwlink/?LinkId=123096) 지정된 다음 바이트 시퀀스의 육선 코딩된 문자열 표현이 포함되어야 합니다.
> 
...
>   **참고**: **마이너 버전** 2 및 'hev1'또는 'hvc1'에 대해 요청 된 기본 미디어 형식은 [ISO / IEC 14496-12] ISO 베이스 미디어 파일 형식 네 번째 에디션및 [ISO / IEC 23001-7] 공통 암호화 두 번째 버전에 지정된 'iSO8'브랜드 ISO 기본 미디어 파일 형식입니다.
