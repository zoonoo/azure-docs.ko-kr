...
3. **미디어 서비스 계정 만들기**에 필요한 값을 입력합니다.
   ...
   3. **리소스 그룹**에서 새 또는 기존 리소스를 선택합니다. 리소스 그룹은 수명 주기, 권한 및 정책을 공유하는 리소스의 컬렉션입니다. [여기](../resource-group-overview.md#resource-groups)(영문)를 참조하십시오.
   4. **위치**에서 미디어 서비스 계정에 대한 메타데이터 레코드 및 미디어를 저장하는 데 사용할 지리적 지역을 선택합니다. 이 지역은 미디어를 처리하고 스트림하는 데 사용됩니다. 사용 가능한 미디어 서비스 지역만 드롭다운 목록 상자에 표시됩니다.
