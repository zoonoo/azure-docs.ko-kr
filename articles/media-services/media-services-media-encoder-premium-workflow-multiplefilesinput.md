...
**미디어 인코더 프리미엄 워크플로** 미디어 프로세서로 태스크를 제출할 때 구성 요소 속성을 사용자 지정하고 클립 목록 XML 콘텐츠를 지정하거나 여러 입력 파일을 전송해야 하는 시나리오가 있습니다. 일부 사례:

- 비디오에 텍스트 오버레이 및 런타임 시 각 입력 비디오에 대한 텍스트 값 설정(예: 현재 날자)
- 클립 목록 XML 사용자 지정(잘라내기 등을 사용/사용하지 않고 하나 이상의 소스 파일 지정)
- 인코딩하는 동안 입력 비디오에 로고 이미지 오버레이

태스크를 만들거나 여러 입력 파일을 전송할 때 **미디어 인코더 프리미엄 워크플로**에 사용자가 워크플로의 일부 속성을 변경하고 있음을 알리려면 **setRuntimeProperties** 및/또는 **transcodeSource**를 포함하는 구성 문자열을 사용해야 합니다. 이 항목에서는 이를 사용하는 방법을 설명합니다.
...
파일에서 XML 구성을 읽고 이를 작업의 태스크에 전달하는 코드는 다음과 같습니다.
...
###간단한 값이 있는 속성
일부 경우 구성 요소 속성을 미디어 인코더 Premium 워크플로에서 실행할 워크플로 파일과 함께 사용자 지정하는 것이 좋습니다. 비디오에 텍스트를 오버레이하는 워크플로를 지정했고 텍스트(예: 현재 날짜)가 런타임에 설정된다고 가정합니다. 인코딩 작업에서 오버레이 구성 요소의 텍스트 속성에 대한 새 값으로 텍스트를 전송하여 이 작업을 수행할 수 있습니다. 이 메커니즘을 사용하여 워크플로에서 구성 요소의 기타 속성을 변경할 수 있습니다(예: 오버레이의 위치 또는 색 변경, AVC 인코더의 비트 전송률 변경). 워크플로의 구성 요소에서 속성을 재정의하는 데 **setRuntimeProperties**가 사용됩니다.
...
XML 값이 예상되는 속성을 설정하려면 <![CDATA[ 및 ]]>를 사용하여 캡슐화하세요.
...
경로 및 속성 이름을 확인하려면 각 속성 바로 옆의 작업 단추를 사용합니다. 이 작업 단추를 클릭하고 '편집'을 선택합니다. 그러면 속성의 실제 이름이 네임스페이스 바로 위에 표시됩니다.
...
**미디어 인코더 Premium 워크플로**에 제출하는 각 태스크에는 두 개의 자산이 필요합니다.
...
- **setRuntimeProperties** 및/또는 **transcodeSource** 요소를 포함하는 구성 데이터를 프로세서에 전달하는 데 필요합니다.
  - 워크플로의 구성 요소에서 파일 이름 또는 다른 속성을 재정의하는 데 **setRuntimeProperties**가 사용됩니다.
...
*파일 이름을 설정하는 데 setRuntimeProperties를 사용하는 경우 기본 파일에서 미디어 파일 입력 구성 요소로 연결이 없음*
...
*클립 목록 XML을 미디어 원본에 연결하고 transcodeSource를 사용할 수 있음*
...
'Expressions'를 사용하여 출력 파일의 이름을 지정하는 데 이 속성을 사용하도록 /primarySourceFile을 지정하는 경우 클립 목록이 /primarySourceFile 설정으로 재정의되지 않도록 클립 목록 XML을 /primarySourceFile 속성 *뒤에* 속성으로 전달하는 것이 좋습니다.
...
인코딩하는 동안 로고 이미지를 입력 비디오에 오버레이하는 예를 살펴보겠습니다. 이 예에서는 입력 비디오의 이름을 "MyInputVideo.mp4"로 지정하고 로고 이름을 "MyLogo.png"로 지정합니다. 다음 단계를 수행해야 합니다.

- 워크플로 파일로 워크플로 자산 만들기(아래 예)
- 두 파일(기본 파일로 MyInputVideo.mp4 및 MyLogo.png)을 포함하는 미디어 자산 만들기
- 위의 입력 자산과 함께 미디어 인코더 Premium 워크플로 미디어 프로세서에 태스크 보내기 및 다음 구성 문자열 지정
...
위의 예제에서 비디오 파일의 이름은 미디어 파일 입력 구성 요소 및 primarySourceFile 속성으로 전송되며 로고 파일의 이름은 그래픽 오버레이 구성 요소에 연결된 다른 미디어 파일 입력으로 전송됩니다.

>[AZURE.NOTE]비디오 파일 이름은 primarySourceFile 속성에 전송됩니다. 그 이유는 예를 들어 Expressions를 사용하여 올바른 출력 파일 이름을 작성하기 위해 워크플로에서 이 속성을 사용하기 위해서입니다.
...
비디오에서 로고의 위치를 수정하려면(예를 들어 비디오의 왼쪽 상단에서 10% 떨어진 위치로 지정할 수 있음) "수동 입력"의 선택을 취소합니다. 오버레이 구성 요소에 로고 파일을 제공하는 데 미디어 파일 입력을 사용하고 있으므로 이 옵션을 선택 취소할 수 있습니다.
...
출력 파일의 이름을 설정해야 합니다. "파일 출력" 구성 요소를 클릭하고 파일에 대한 식을 편집합니다.
...
이 작업에 성공하면 Azure 미디어 서비스에서 실행할 수 있습니다.

먼저 Azure 미디어 서비스에서 안에 두 파일(비디오 파일 및 로고)이 있는 자산을 준비합니다. .NET 또는 REST API를 사용하여 수행할 수 있습니다. 또한 Azure 포털이나 [Azure 미디어 서비스 탐색기](https://github.com/Azure/Azure-Media-Services-Explorer)(AMSE)를 사용하여 수행할 수도 있습니다.

이 자습서에는 AMSE를 사용하여 자산을 관리하는 방법을 보여 줍니다. 두 가지 방법으로 자산에 파일을 추가합니다.

1. 로컬 폴더를 만들고, 안에 두 개의 파일을 복사하고 해당 폴더를 자산 탭으로 끌어서 놓습니다.
1. 비디오 파일을 자산으로 업로드한 다음 자산 정보를 표시하고 파일 탭으로 이동하여 추가 파일(로고)을 업로드합니다.

>[AZURE.NOTE]자산에서 기본 파일을 설정해야 합니다(기본 비디오 파일).
...
프로세서에 데이터를 전달하는 단추를 클릭하고 다음 XML을 추가하여 런타임 속성을 설정합니다.
...
.NET SDK를 사용하여 태스크를 만들고 실행한 경우 이 XML 데이터를 구성 문자열로 전달해야 합니다.
...
[여기](https://github.com/Azure/azure-media-services-samples/tree/master/Encoding%20Presets/VoD/MediaEncoderPremiumWorkfows/)에서 샘플 워크플로를 다운로드할 수 있습니다.
