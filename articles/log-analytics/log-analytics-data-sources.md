<properties 
   pageTitle="Log Analytics의 데이터 원본 | Microsoft Azure"
...
2. OMS 콘솔에서 **설정** 타일을 선택합니다.
3. **데이터**를 선택합니다.
4. 구성할 데이터 원본을 클릭합니다.
5. 해당 구성에 대한 세부 정보를 보려면 위 표에서 각 데이터 원본에 대한 설명서 링크를 따릅니다.
...
연결된 관리 그룹 내 SCOM(System Center Operations Manager) 에이전트의 경우 데이터 원본 구성은 관리 팩으로 변환되어 기본적으로 5분마다 관리 그룹에 전달됩니다. 에이전트는 관리 팩을 다운로드하고 지정된 데이터를 수집한 후 OMS로 데이터를 전달하는 관리 서버에 보냅니다. 어떤 데이터 원본에도 에이전트와 OMS 간의 직접 통신은 필요 없습니다. SCOM과 OMS의 연결 및 구성 전달 주기 수정에 대한 자세한 내용은 [System Center Operations Manager와의 통합 구성](log-analytics-om-agents.md)에서 확인할 수 있습니다.
...
- 데이터 원본 및 솔루션에서 수집된 중요한 데이터를 사전에 알리도록 경고를 구성합니다.
