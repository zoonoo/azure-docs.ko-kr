...
Microsoft에는 Microsoft의 모든 서비스를 감독 및 평가하는 보안 및 규정 준수 팀이 있습니다. 이 팀은 정보 보안 책임자로 구성되어 있으며 Log Analytics를 개발하는 엔지니어링 부서와 독립적입니다. 보안 책임자는 자체 관리 체임이 있으며 제품과 서비스에 대한 독립적 평가를 실시하여 보안과 규정 준수를 보장합니다.
...
수집되어 데이터베이스에 저장된 데이터의 보존 기간은 작업 영역 생성 시 선택한 계획에 따라 다릅니다.  유료 계층의 경우 수집된 데이터는 기본적으로 31일 동안 사용할 수 있지만 365일까지 사용할 수 있도록 연장 가능합니다.  이러한 데이터는 미사용 시 아직 암호화되지 않지만 2018년 중반기에 암호화될 예정입니다. 
...
* [Azure VM 빠른 시작](log-analytics-quick-collect-azurevm.md)에 따라 Azure VM용 Log Analytics를 사용하여 데이터를 수집하는 방법에 대해 알아봅니다.  

*  사용자 환경에서 물리적 또는 가상 Windows 또는 Linux 컴퓨터로부터 데이터를 수집하려면 [Linux 컴퓨터용 빠른 시작](log-analytics-quick-collect-linux-computer.md) 또는 [Windows 컴퓨터용 빠른 시작](log-analytics-quick-collect-windows-computer.md)을 참조하세요.
