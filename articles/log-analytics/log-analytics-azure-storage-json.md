<properties
    pageTitle="Log Analytics를 사용하여 Azure 진단 로그 분석 | Microsoft Azure"
...
Log Analytics는 JSON 형식의 Blob Storage에 [Azure 진단 로그](../azure-portal/monitoring-overview-of-diagnostic-logs.md)를 기록하는 다음 Azure 서비스에 대해 로그를 읽을 수 있습니다.
...
일부 리소스의 경우 [Azure 진단 로그 개요](../azure-portal/monitoring-overview-of-diagnostic-logs.md#how-to-enable-collection-of-diagnostic-logs)에서 설명한 절차를 통해 Azure Portal에서 앞의 구성을 수행할 수 있습니다.
...
>[AZURE.NOTE] 리소스와 작업 영역이 다른 Azure 구독에 있는 경우 필요에 따라 `Select-AzureRmSubscription -SubscriptionId <Subscription the resource is in>`을 사용하여 구독 사이를 전환합니다.
...
>[AZURE.NOTE] 이 구성은 Azure Portal에서 표시되지 않습니다. `Get-AzureRmOperationalInsightsStorageInsight` cmdlet을 사용하여 구성을 확인할 수 있습니다.  
