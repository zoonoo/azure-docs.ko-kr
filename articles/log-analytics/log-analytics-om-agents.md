...
IT 보안 정책이 네트워크의 컴퓨터가 인터넷에 연결하도록 허용하지 않을 경우 OMS 게이트웨이에 연결하여 구성 정보를 받고 사용하도록 설정한 솔루션에 따라 수집된 데이터를 보내도록 관리 서버를 구성할 수 있습니다.  Operations Manager 관리 그룹이 OMS 게이트웨이를 통해 OMS 서비스와 통신할 수 있도록 구성하는 방법에 대한 추가 정보 및 단계에 대해서는 [OMS 게이트웨이 사용하여 OMS에 컴퓨터 연결](log-analytics-oms-gateway.md)을 참조하세요.  
...
OMS 서비스와의 통합을 지원하는 데 필요한 관리 팩 뿐만 아니라 Operations Manager와 통합되며 사용자가 사용하도록 설정한 솔루션용 관리 팩도 관리 그룹에서 쉽게 삭제할 수 없습니다.  일부 OMS 관리 팩이 관련된 다른 관리 팩에 대해 종속성을 갖기 때문입니다.  다른 관리 팩에 대해 종속성이 있는 관리 팩을 삭제하려면 TechNet 스크립트 센터에서 [종속성이 있는 관리 팩 제거](https://gallery.technet.microsoft.com/scriptcenter/Script-to-remove-a-84f6873e) 스크립트를 다운로드합니다.  
