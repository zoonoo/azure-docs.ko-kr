...
## 일정
저장된 검색은 하나 이상의 일정을 가질 수 있습니다. 일정은 검색이 실행되는 빈도 및 조건이 식별되는 기간을 정의합니다. 일정은 다음 표의 속성을 가집니다.
...
Get 메서드를 사용하여 저장된 검색에 대한 특정 일정을 검색합니다.
...
### 일정 만들기 또는 편집
저장된 검색에 고유한 일정 ID와 함께 Put 메서드를 사용하여 새 일정을 만듭니다. 기존 일정 ID를 사용하는 경우 해당 일정이 수정됩니다. OMS 콘솔에서 일정을 만드는 경우 일정 ID에 대해 GUID가 생성됩니다.
...
### 작업 만들기 또는 편집
일정에 고유한 작업 ID와 함께 Put 메서드를 사용하여 새 작업을 만듭니다. 기존 작업 ID를 사용하는 경우 해당 작업이 수정됩니다. OMS 콘솔에서 작업을 만드는 경우 작업 ID에 대해 GUID가 생성됩니다.
...
작업 ID와 함께 Put 메서드를 사용하여 일정에 대한 새 임계값 작업을 만듭니다. 작업에 대해 새 ID를 사용하면 새 작업이 생성됩니다. 일정에 대해 이미 존재하는 ID를 사용하는 경우 해당 작업이 수정됩니다.
...
작업 ID와 함께 Put 메서드를 사용하여 일정에 대한 새 전자 메일 작업을 만듭니다. 작업에 대해 새 ID를 사용하면 새 작업이 생성됩니다. 일정에 대해 이미 존재하는 ID를 사용하는 경우 해당 작업이 수정됩니다.

다음 예제에서는 저장된 검색의 결과가 임계값을 초과하는 경우 전자 메일을 보내도록 임계값을 가진 전자 메일 알림을 만듭니다.
...
작업 ID와 함께 Put 메서드를 사용하여 일정에 대한 새 수정 작업을 만듭니다. 작업에 대해 새 ID를 사용하면 새 작업이 생성됩니다. 일정에 대해 이미 존재하는 ID를 사용하는 경우 해당 작업이 수정됩니다.

다음 예제에서는 저장된 검색의 결과가 임계값을 초과하는 경우 Runbook이 시작되도록 임계값을 가진 수정을 만듭니다.
...
#### 웹후크 작업 만들기 또는 편집
작업 ID와 함께 Put 메서드를 사용하여 일정에 대한 새 웹후크 작업을 만듭니다. 작업에 대해 새 ID를 사용하면 새 작업이 생성됩니다. 일정에 대해 이미 존재하는 ID를 사용하는 경우 해당 작업이 수정됩니다. 다음 예제에서는 저장된 검색의 결과가 임계값을 초과하는 경우 웹후크가 트리거되도록 웹후크 작업 및 임계값을 가진 웹후크 작업을 만듭니다.
