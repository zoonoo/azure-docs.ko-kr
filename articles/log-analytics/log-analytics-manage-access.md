...
- 글로벌 회사이며 데이터 주권 / 준수 이유 때문에 특정 지역에 저장된 데이터가 필요합니다.
- Azure를 사용하고 있으며 Log Analytics 작업 영역을 자신이 관리하는 Azure 리소스와 같은 영역에 두어서 아웃바운드 데이터 전송 요금을 피하려고 합니다.
- 용도를 바탕으로 서로 다른 부서/비즈니스 그룹에 요금을 할당하려고 합니다. 각 부서/비즈니스 그룹에 대한 작업 영역을 만들면 Azure 청구 및 사용량 명세서에 각 작업 영역의 요금이 따로 표시됩니다.
...
사람들에게 OMS 작업 영역에 대한 액세스를 부여하는 것은 2곳에서 제어될 수 있습니다.

- Azure 역할 기반 액세스 제어를 사용하면 Azure 구독 및 연결된 Azure 리소스에 대한 액세스를 제공할 수 있습니다. 이 방법은 PowerShell 및 REST API 액세스에도 사용됩니다.
- OMS 포털만 액세스하고 연결된 Azure 구독에 액세스하지 않는 것을 OMS 포털 내에서 관리할 수 있습니다.
...
Azure 사용 권한을 사용하여 사람들에게 Log Analytics 작업 영역에 대한 액세스를 제공하는 경우(예: Azure 포털에서) 같은 사용자가 Log Analytics 포털에도 액세스할 수 있습니다 . 사용자가 Azure 포털 내에 있는 경우 사용자는 Log Analytics 작업 영역 리소스를 볼 때 "OMS 포털" 작업을 클릭하여 OMS 포털로 이동할 수 있습니다.

알아야 할 네 가지 사항:

1. 이는 역할 기반 액세스 제어가 아닙니다. Azure 포털에서 Log Analytics 작업 영역에 대한 "독자" 액세스 권한을 가지고 있는 경우 OMS 포털을 통해 변경할 수 있는 권한을 갖게 됩니다. OMS 포털에는 관리자, 참가자 및 읽기 전용 사용자의 개념이 있습니다. 로그인한 계정이 작업 영역에 연결된 Azure Active Directory 내에 있는 경우 OMS 포털의 관리자가 되며 그렇지 않으면 참가자가 됩니다.
2. mms.microsoft.com을 통해 OMS 포털에 로그인한 경우 기본적으로 “작업 영역 선택” 목록을 보면 OMS 포털에 추가한 작업 영역만 포함하게 됩니다. Azure 구독을 통해 액세스 권한을 가진 작업 영역을 보려면 URL의 일부로 테넌트를 지정합니다. 예: mms.microsoft.com/?tenant=contoso.com(테넌트 ID는 흔히 로그인한 전자 메일 주소의 마지막 부분입니다.)
3. 로그인한 계정이 테넌트 AAD의 계정인 경우(일반적으로 SCP로 로그인하지 않은 한 이 경우에 해당함) OMS 포털의 관리자가 됩니다. 계정이 테넌트 AAD에 있지 않은 경우 OMS 포털의 사용자가 됩니다.
4. Azure 사용 권한을 통해 액세스 권한을 가진 포털로 직접 이동하려면 URL의 일부로 리소스를 지정해야 합니다. PowerShell 통해 이 URL을 가져올 수 있습니다(예: Get-AzureRmOperationalInsightsWorkspace). PortalUrl URL은 다음과 같이 표시됩니다. https://eus.mms.microsoft.com/?tenant=contoso.com&resource=%2fsubscriptions%2faaa5159e-dcf6-890a-a702-2d2fee51c102%2fresourcegroups%2fdb-resgroup%2fproviders%2fmicrosoft.operationalinsights%2fworkspaces%2fmydemo12
...
3. **사용자 관리** 섹션에서 추가할 계정 유형: **조직 계정** ,**Microsoft 계정**, **Microsoft 지원**을 선택합니다.
    ...
    >[AZURE.NOTE] 최상의 성능 결과를 얻으려면 단일 OMS 계정과 연결된 Active Directory 그룹 수를 두 개(관리자용 그룹 한 개, 참가자용 한 개 및 읽기 전용 사용자용 한 개)로 제한합니다. 더 많은 그룹을 사용하면 Log Analytics 성능에 영향을 줄 수 있습니다.

7. 추가할 사용자 또는 그룹의 유형: **관리자**, **참가자** 또는 **읽기 전용 사용자**를 선택합니다..
8. **추가**를 클릭합니다.
...
OMS 계정과 연결된 사용자의 계정 역할을 변경할 수 있습니다. 다음과 같은 역할 옵션이 있습니다.
 ...
 - *읽기 전용사용자*: 읽기 전용으로 표시된 사용자는 다음을 할 수 없습니다.
   1. 솔루션 추가/제거(솔루션 갤러리 숨김)
   2. '내 대시보드'에서 타일 추가/수정/제거
   3. 설정 페이지 보기(페이지 숨김)
   4. 검색 뷰에서: PowerBI 구성, 저장된 검색 및 경고 작업 숨김
