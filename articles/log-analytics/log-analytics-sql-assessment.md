...
>[AZURE.NOTE] SCOM 에이전트가 아닌 OMS 직접 에이전트를 사용하는 경우 관리 팩은 항상 로컬 시스템 계정의 보안 컨텍스트에서 실행됩니다. 아래의 1-5단계를 건너뛰고 T-SQL 또는 Powershell 샘플을 실행하고 NT AUTHORITY\SYSTEM을 사용자 이름으로 지정합니다.
...
### <a name="to-create-and-use-an-ignorerecommendations.txt-text-file"></a>IgnoreRecommendations.txt 텍스트 파일을 만들고 사용하려면
