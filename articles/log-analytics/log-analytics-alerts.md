<properties 
   pageTitle="Log Analytics의 경고 | Microsoft Azure"
...
Runbook 작업은 [웹후크](../automation/automation-webhooks.md)를 사용하여 Runbook을 시작합니다. 경고 규칙을 만들 때 이름 **OMS 경고 수정**에 이어 GUID를 가진 Runbook에 대해 새 웹후크가 자동으로 생성됩니다. 경고 규칙은 페이로드를 보내지 않으며 Runbook의 어떤 매개 변수도 채울 수 없으므로 매개 변수가 필요 없는 Runbook을 사용해야 합니다.

또한 이전 섹션에서 설명한 대로 웹후크 작업을 포함한 경고에서 Runbook을 시작할 수도 있습니다. 웹후크 작업은 데이터를 Runbook에 보내는 페이로드를 제공한다는 장점이 있습니다. Runbook 작업의 장점은 웹후크를 스스로 만들거나 사용법을 이해할 필요가 없다는 것입니다.
...
4.	**저장**을 클릭하여 경고 규칙을 완료합니다. 실행을 즉시 시작합니다.
...
| Webhook | 경고가 트리거될 때 웹후크를 호출하려면 **예**를 지정합니다. |
...
| 실행 | Azure 클라우드에서 Runbook을 실행하려면 **Azure**를 선택합니다. 로컬 환경의 [Hybrid Runbook Worker](..\automation\automation-hybrid-runbook-worker.md)에서 Runbook을 실행할 **Hybrid Worker**를 선택합니다. |
...
예를 들어 프로세서가 30분 동안 90% 넘게 실행하는 경우 경고하게 하려면 *Type=Perf ObjectName=Processor CounterName="% 프로세서 시간" CounterValue > 90*과 같은 쿼리를 사용하고 경고 규칙에 대한 임계값을 *0보다 큰* 값으로 설정할 수 있습니다.
