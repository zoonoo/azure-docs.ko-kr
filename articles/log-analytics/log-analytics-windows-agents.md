...
>[AZURE.NOTE] Azure에서 실행되는 가상 컴퓨터의 경우 [가상 컴퓨터 확장](log-analytics-azure-vm-extension.md)을 사용하여 설치를 간소화할 수 있습니다.
...
2. **직접 컴퓨터 연결**에서, 사용하는 컴퓨터 프로세서 유형에 해당하는 **Windows 에이전트 다운로드**를 클릭하여 설치 파일을 다운로드합니다.
3. **작업 영역 ID** 오른쪽에서 복사 아이콘을 클릭하고 ID를 메모장에 붙여 넣습니다.
4. **기본 키** 오른쪽에서 복사 아이콘을 클릭하고 키를 메모장에 붙여 넣습니다.     
    ![작업 영역 ID 및 기본 키 복사](./media/log-analytics-windows-agents/oms-direct-agent-primary-key.png)
...
>[AZURE.NOTE] 이전에 명령줄 또는 스크립트를 사용하여 에이전트를 설치 또는 구성한 경우 `EnableAzureOperationalInsights`가 `AddCloudWorkspace`로 바뀌었습니다.
...
## <a name="optionally,-configure-agents-to-use-the-oms-log-analytics-forwarder"></a>또는 에이전트에서 OMS Log Analytics Forwarder를 사용하도록 구성합니다.
