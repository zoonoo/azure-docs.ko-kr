---
title: "Log Analytics에 Configuration Manager 연결 | Microsoft Docs"
description: "이 문서는 Configuration Manager를 Log Analytics에 연결하고 데이터 분석을 시작하는 단계를 보여줍니다."
...
---
# <a name="connect-configuration-manager-to-log-analytics"></a>Log Analytics에 Configuration Manager 연결
System Center Configuration Manager를 OMS의 Log Analytics에 연결하여 장치 수집 데이터를 동기화할 수 있습니다. 이 경우 OMS에서 Configuration Manager 계층 구조의 데이터를 사용할 수 있습니다.
...
2. Azure Portal에서 [Configuration Manager(등록된 웹앱)에 OMS에 대한 액세스 권한을 제공](#provide-configuration-manager-with-permissions-to-oms)합니다.
3. Configuration Manager에서 [DMS 연결 추가 마법사를 사용하여 연결을 추가](#add-an-oms-connection-to-configuration-manager)합니다.
4. Configuration Manager에서 암호 또는 클라이언트 비밀 키가 만료되거나 분실된 경우 [연결 속성을 업데이트](#update-oms-connection-properties)합니다.
5. OMS 포털의 정보를 사용하여 Configuration Manager 서비스 연결 지점 사이트 시스템 역할을 실행하는 컴퓨터에서 [Microsoft Monitoring Agent를 다운로드 및 설치](#download-and-install-the-agent)합니다. 에이전트가 Configuration Manager 데이터를 OMS로 전송합니다.
6. Log Analytics에서 컴퓨터 그룹으로 [Configuration Manager의 컬렉션을 가져옵니다](#import-collections).
7. Log Analytics에서 [컴퓨터 그룹](log-analytics-computer-groups.md)으로 Configuration Manager에서 데이터를 봅니다.

[Microsoft Operations Management Suite에 Configuration Manager의 데이터 동기화](https://technet.microsoft.com/library/mt757374.aspx)에서 Configuration Manager를 OMS에 연결하는 방법에 대해 자세히 읽을 수 있습니다.

## <a name="provide-configuration-manager-with-permissions-to-oms"></a>Configuration Manager에 OMS에 대한 사용 권한 제공
다음 절차에서는 Azure Portal에 OMS에 대한 액세스 권한을 제공합니다. 특히, Azure Portal에서 Configuration Manager를 OMS에 연결할 수 있도록 하기 위해 리소스 그룹의 사용자에게 *참가자 역할*을 부여해야 합니다.

> [!NOTE]
> Configuration Manager의 OMS에 사용 권한을 지정해야 합니다. 그렇지 않고 Configuration Manager에서 구성 마법사를 사용하면 오류 메시지가 표시됩니다.
>
>
...
3. **OMS 작업 영역**에서 다음 정보를 제공한 다음 **확인**을 클릭합니다.
   ...
   * **가격 책정 계층**  
     ...
     > [!NOTE]
     > 위의 예제는 새 리소스 그룹을 만듭니다. 리소스 그룹은 Configuration Manager에 이 예제의 OMS 작업 영역에 대한 권한을 제공하는 데에만 사용됩니다.
     >
     >
...
9. **사용자 추가**를 클릭하고 Configuration Manager 사용자를 선택한 다음 **선택**을 클릭하고 **확인**을 클릭합니다.  
...
OMS 연결을 추가하려면 Configuration Manager 환경에 온라인 모드를 위해 구성된 [서비스 연결 지점](https://technet.microsoft.com/library/mt627781.aspx)이 있어야 합니다.

1. Configuration Manager의 **관리** 작업 영역에서 **OMS 커넥터**를 선택합니다. 그러면 **OMS 연결 추가 마법사**가 열립니다. **다음**을 선택합니다.
...
OMS에 Configuration Manager를 연결한 후에는 컬렉션을 추가 또는 제거하고 OMS 연결의 속성을 확인합니다.
...
2. 다음 중 한 가지 방법을 사용하여 Configuration Manager 서비스 연결 지점 사이트 시스템 역할을 실행하는 컴퓨터에 에이전트를 설치 및 구성합니다.
...
Configuration Manager에 OMS 연결을 추가하고 Configuration Manager 서비스 연결 지점 사이트 시스템 역할을 실행하는 컴퓨터에 에이전트를 설치한 후 다음 단계는 컴퓨터 그룹으로 OMS의 Configuration Manager 컬렉션을 가져오는 것입니다.
...
3. **Configuration Manager 컬렉션 멤버 자격 가져오기**를 선택한 다음 **저장**을 클릭합니다.  
   ![컴퓨터 그룹 - SCCM 탭](./media/log-analytics-sccm/sccm-computer-groups01.png)

## <a name="view-data-from-configuration-manager"></a>Configuration Manager의 데이터 보기
Configuration Manager에 OMS 연결을 추가하고 Configuration Manager 서비스 연결 지점 사이트 시스템 역할을 실행하는 컴퓨터에 에이전트를 설치하면 에이전트의 데이터가 OMS로 전송됩니다. OMS에서 Configuration Manager 컬렉션이 [컴퓨터 그룹](log-analytics-computer-groups.md)으로 나타납니다. **설정**의 **컴퓨터 그룹**에서 **Configuration Manager** 페이지의 그룹을 볼 수 있습니다.
...
각 항목을 클릭하면 가져온 그룹 전체 또는 각 그룹에 속하는 전체 컴퓨터가 표시된 검색이 열립니다. [로그 검색](log-analytics-log-searches.md)을 사용하여 Configuration Manager 데이터의 자세한 분석을 시작할 수 있습니다.

## <a name="next-steps"></a>다음 단계
* [로그 검색](log-analytics-log-searches.md)을 사용하여 Configuration Manager 데이터에 대한 자세한 정보를 볼 수 있습니다.
