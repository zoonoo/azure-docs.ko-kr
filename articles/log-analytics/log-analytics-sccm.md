<properties
    pageTitle="Log Analytics에 구성 관리자 연결 | Microsoft Azure"
...
>[AZURE.NOTE] 구성 관리자의 OMS에 사용 권한을 지정해야 합니다. 그렇지 않고 구성 관리자에세 구성 마법사를 사용하면 오류 메시지가 표시됩니다.
...
3. **OMS 작업 영역** 블레이드에서 다음 정보를 제공한 다음 **확인**을 클릭합니다.
  ...
  - **가격 책정 계층**  
    ...
    >[AZURE.NOTE] 위의 예제는 새 리소스 그룹을 만듭니다. 리소스 그룹은 구성 관리자에 이 예제의 OMS 작업 영역에 대한 권한을 제공하는 데에만 사용됩니다.
...
>[AZURE.NOTE] OMS를 계층의 최고 계층 사이트에 연결해야 합니다. OMS를 독립형 기본 사이트에 연결한 다음 환경에 중앙 관리 사이트를 추가할 경우 새 계층 안에서 OMS 연결을 삭제하고 다시 만들어야 합니다.
