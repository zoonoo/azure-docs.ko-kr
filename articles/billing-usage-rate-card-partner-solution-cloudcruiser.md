...
워크북으로 데이터를 가져온 후에는 API의 정보를 처리할 수 있도록 *UsageAPI* 시트에서 변환 단계를 만들 것입니다. 첫 번째 단계는 "JSON 분할" 프로세서를 사용하여 단일 필드에서 태그를 추출하고 각 태그(부서, 프로젝트, 소유자 및 환경)에 대한 필드를 만드는 것입니다.
