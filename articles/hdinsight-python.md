...
-   [HDInsight의 Python][HDInsight의 Python]
-   [Hive 및 Python][Hive 및 Python]
-   [Pig 및 Python][Pig 및 Python]
-   [예제 실행][예제 실행]
-   [문제 해결][문제 해결]
-   [다음 단계][다음 단계]
...
2.  **SELECT TRANSFORM ... USING 'D:\\Python27\\python.exe streaming.py'** 문은 **hivesampletable**의 데이터를 선택하고 clientid, devicemake 및 devicemodel을 **streaming.py** 스크립트로 전달합니다.
    ...
    > [WACOM.NOTE] **USING** 절은 경로에 없는 python.exe에 대한 전체 경로를 지정합니다.
...
HDInsight 클러스터에서 이 예제를 실행하는 방법에 대해서는 [예제 실행][예제 실행]을 참조하세요.
...
이 단계에서는 Microsoft Azure PowerShell을 사용합니다. 이 도구를 아직 개발 컴퓨터에 설치하여 구성하지 않은 경우 다음 단계를 사용하기 전에 [Azure PowerShell을 설치 및 구성하는 방법][Azure PowerShell을 설치 및 구성하는 방법]을 참조하세요.

1.  Python 예제 [streaming.py][streaming.py] 및 [jython.py][jython.py]를 사용하여, 개발 컴퓨터에서 파일의 로컬 사본을 만듭니다.
...
2.  다음 PowerShell 스크립트를 사용하여 **streaming.py** 및 **jython.py** 파일을 서버에 업로드합니다. 스크립트의 첫 3개 줄에서 Azure HDInsight 클러스터의 이름과 **streaming.py** 및 **jython.py** 파일의 경로를 바꿉니다.
    ...
    > [WACOM.NOTE] 그 외에 스크립트를 업로드하는 방법은 [HDInsight에서 Hadoop 작업용 데이터 업로드][HDInsight에서 Hadoop 작업용 데이터 업로드] 문서에서 찾아볼 수 있습니다.
...
2.  **Hive 편집기**를 사용하여 `select * from hivesampletable` 줄을 다음 HiveQL로 바꿉니다.
...
기본적으로 제공되지 않는 Python 모듈을 로드해야 하는 경우 수행 방법의 예제에 대해서는 [Azure HDInsight에 모듈을 배포하는 방법][Azure HDInsight에 모듈을 배포하는 방법](영문)을 참조하세요.

PowerShell을 사용하지 않고 HDInsight에서 작업을 원격으로 실행하려는 경우 Python을 사용하여 WebHCat REST API를 통해 작업을 실행하는 예제에 대해서는 [Linux에서 Azure HDInsight를 사용하는 방법][Linux에서 Azure HDInsight를 사용하는 방법]을 참조하세요.
