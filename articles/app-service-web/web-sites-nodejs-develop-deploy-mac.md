<properties
	pageTitle="Azure 앱 서비스에서 Node.js 웹앱 만들기 | Microsoft Azure"
...
이 자습서에서는 간단한 [Node.js](http://nodejs.org) 응용 프로그램을 만들고 [Git](http://git-scm.com)를 사용하여 [Azure 앱 서비스](../app-service/app-service-value-prop-what-is.md)의 [웹앱](app-service-web-overview.md)에 배포하는 방법을 보여줍니다. 이 자습서의 지침은 Node.js를 실행할 수 있는 모든 운영 체제에 적용될 수 있습니다.

다음 내용을 배웁니다.

* Azure Preview 포털을 사용하여 Azure 앱 서비스에서 웹앱을 만드는 방법
...
2. 포털의 왼쪽 맨 위에서 **+ 새로 만들기** 아이콘을 클릭합니다.
...
8. **웹앱** 블레이드에서 **배포** 부분을 클릭합니다.

	![][deployment-part]

9. **지속적인 배포** 블레이드에서 **원본 선택**을 클릭합니다.

14. **로컬 Git 리포지토리**를 클릭한 다음 **확인**을 클릭합니다.

	![][setup-git-publishing]

11. 아직 수행하지 않은 경우 배포 자격 증명을 설치합니다.
...
11. 웹앱 블레이드에서 **설정**을 클릭한 후 **속성**을 클릭합니다.
 
	게시하려면 원격 Git 리포지토리에 푸시합니다. 리포지토리에 대한 URL는 **GIT URL**에 나열됩니다. 이 URL은 자습서의 뒷부분에서 필요합니다.
...
2. 다음 코드를 **server.js** 파일에 복사한 다음 파일을 저장합니다.
...
3. 명령줄을 열고 다음 명령을 사용하여 웹앱을 로컬에서 시작합니다.

        node server.js

4. 웹 브라우저를 열고 http://localhost:1337으로 이동합니다.
...
1. 아직 수행하지 않았다면 Git을 설치합니다.
	...
	플랫폼별 설치 지침은 [Git 다운로드 페이지](http://git-scm.com/download)를 참조하세요.
...
3. 다음 명령을 사용하여 이전에 만든 웹앱으로 업데이트를 푸시하는 Git 원격을 추가합니다.
...
5. 앱을 보려면 Azure 포털의 **웹앱** 부분에서 **찾아보기** 단추를 클릭합니다.
...
3. 웹앱의 URL을 탐색하는 브라우저 창을 새로 고칩니다.

	!['Hello Azure'를 표시하는 웹 페이지][helloworld-completed]

## 배포 롤백

**웹앱** 블레이드에서 **설정 > 지속적인 배포**를 클릭하여 **배포** 블레이드에서 배포 기록을 볼 수 있습니다. 이전 배포를 롤백해야 하는 경우 **배포 세부 정보** 블레이드에서 **재배포**를 선택하고 클릭할 수 있습니다.

## 다음 단계

Azure 앱 서비스에서 웹앱에 Node.js 응용 프로그램을 배포했습니다. 앱 서비스 웹앱에서 Node.js 응용 프로그램을 실행하는 방법에 대해 자세히 알아보려면 [Azure 앱 서비스 웹앱: Node.js](http://blogs.msdn.com/b/silverlining/archive/2012/06/14/windows-azure-websites-node-js.aspx) 및 [Azure 응용 프로그램에서 Node.js 버전 지정](../nodejs-specify-node-version-azure-apps.md)을 참조하세요.
...
이 문서에서는 Azure 포털을 사용하여 웹앱을 만듭니다. 또한 [Azure 명령줄 인터페이스](../xplat-cli-install.md) 또는 [Azure PowerShell](../install-configure-powershell.md)을 사용하여 동일한 작업을 수행할 수 있습니다.
