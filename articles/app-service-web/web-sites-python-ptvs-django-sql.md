...
7. <kbd>F5</kbd> 키를 눌러 응용프로그램이 작동하는지 확인합니다.
...
1. [Azure 포털]에 로그인합니다.
2. 탐색 창 맨 아래에 있는 **새로 만들기**를 클릭한 다음 **데이터 + 저장소** > **SQL 데이터베이스**를 클릭합니다.
3. 새 리소스 그룹을 만들어 새 SQL 데이터베이스를 구성하고 적절한 위치를 선택합니다.
4. SQL 데이터베이스를 만든 후 데이터베이스 블레이드에서 **Visual Studio에서 열기**를 클릭합니다.
5. **방화벽 구성**을 클릭합니다.
6. **방화벽 설정** 블레이드에서 **시작 IP** 및 **종료 IP**를 개발 컴퓨터의 공용 IP 주소로 설정하여 방화벽 규칙을 추가합니다. **Save**를 클릭합니다.
   
   개발 컴퓨터에서 데이터베이스 서버에 연결할 수 있게 됩니다.
7. 데이터베이스 블레이드로 돌아가서 **속성**을 클릭하고 **데이터베이스 연결 문자열 표시**를 클릭합니다.
8. 복사 단추를 사용하여 **ADO.NET** 값을 클립보드에 저장합니다.
...
5. `F5` 키를 눌러 응용 프로그램을 실행합니다. **Create Sample Polls**를 사용하여 만든 설문 조사와 투표를 통해 제출된 데이터는 SQL 데이터베이스에서 serialize됩니다.
...
6. 웹 브라우저가 자동으로 게시된 웹앱으로 열립니다. Azure에 호스트된 **SQL** 데이터베이스를 사용하여 예상한 대로 웹앱이 작동하는지 확인합니다.
