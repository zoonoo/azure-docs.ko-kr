...
## <a name="route-traffic-segment-to-a-deployment-slot"></a>배포 슬롯에 라우팅 트래픽 세그먼트
모든 TiP 시나리오의 기본 수준에서 비프로덕션 배포 슬롯에 미리 정의된 비율의 라이브 트래픽을 라우팅합니다. 이렇게 하려면 다음 단계를 수행하세요.

> [!NOTE]
> 여기에서 단계는 [비-프로덕션 배포 슬롯](web-sites-staged-publishing.md) 및 웹앱 콘텐츠가 이미 [배포](web-sites-deploy.md)되었기를 원합니다.
> 
> 
...
## <a name="force-client-requests-to-a-specific-slot"></a>특정 슬롯에 클라이언트 요청을 강제로
...
### <a name="opt-users-in-to-beta-app"></a>베타 앱에서 사용자 참여
