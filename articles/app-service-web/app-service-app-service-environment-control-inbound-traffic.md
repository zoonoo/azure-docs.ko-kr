...
앱 서비스 환경이 제대로 작동하려면, 동일한 Azure 지역의 SQL 데이터베이스뿐 아니라 전 세계 Azure 저장소로의 아웃바운드 액세스 또한 필요합니다. 가상 네트워크에서 아웃바운드 인터넷 액세스가 차단 될 경우, 앱 서비스 환경은 이러한 Azure 끝점에 액세스 할 수 없습니다.
...
**참고**: 네트워크 보안 그룹은 아래 설명된 Powershell cmdlet을 사용해서만 구성할 수 있습니다. [Azure 포털](portal.azure.com)은 "v2" 가상 네트워크와 연결된 NSG의 그래픽 방식 구성만 허용하기 때문에 네트워크 보안 그룹은 Azure 포털을 사용하여 그래픽 방식으로 구성할 수 없습니다. 하지만 현재 작동하는 앱 서비스 환경은 클래식 "v1" 가상 네트워크에서만 지원됩니다. 따라서 “v1” 가상 네트워크에 연결된 네트워크 보안 그룹은 Powershell cmdlet을 사용해서만 구성할 수 있습니다.
