...
VNET가 ASE에서 별도 리소스 그룹을 가지려면 먼저 VNET를 별도로 만든 다음 ASE를 만드는 동안 VNET를 선택해야 합니다. 또한 ASE를 만드는 동안 기존 VNET에서 서브넷을 만들려면 ASE는 VNET와 동일한 리소스 그룹에 있어야 합니다.
...
지정한 ASE 이름은 ASE에서 만드는 앱에 사용됩니다. ASE 이름이 appsvcenvdemo인 경우 도메인 이름은 .*appsvcenvdemo.p.azurewebsites.net*입니다. 따라서 *mytestapp*이라는 앱을 만든 경우 해당 주소는 *mytestapp.appsvcenvdemo.p.azurewebsites.net*으로 지정될 수 있습니다. ASE의 이름에 공백을 사용할 수 없습니다. 이름에 대문자를 사용한 경우 전체 도메인 이름은 해당 이름의 소문자 버전이 됩니다.

기본값이 있으면 특정 개수의 상황에 매우 유용하지만 자주 무언가를 조정해야 합니다. 다음 섹션은 ASE와 관련된 구성 섹션 각각을 안내합니다.

### 가상 네트워크 ###
새 VNET을 자동으로 만드는 빨리 만들기 기능이 있지만 이 기능은 기존 VNET의 선택 및 수동으로 VNET 만들기도 지원합니다. 앱 서비스 환경 배포를 지원할 정도의 규모인 경우 기존 VNET(지금은 클래식 "v1" 가상 네트워크만 지원됨)을 선택할 수 있습니다. VNET에는 적어도 8개 이상의 주소가 있어야 합니다. 현재 RFC1918 주소 공간(즉, 개인 주소)을 사용하는 가상 네트워크만 지원됩니다.
...
기존 VNET에서 서브넷을 만들려면 ASE는 VNET와 동일한 리소스 그룹에 있다는 점을 알립니다. VNET에서 별도 리소스 그룹에 ASE을 두려면 단순히 VNET 및 서브넷을 모두 ASE를 만들기 전에 별도로 만듭니다.
