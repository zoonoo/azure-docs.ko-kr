...
6. **리소스 그룹** 상자 옆에서 **새로 만들기**를 클릭한 다음, "MyExample" 또는 원하는 다른 이름을 입력합니다.
	...
	리소스 그룹은 API 앱, 데이터베이스, VM 등 Azure 리소스의 컬렉션입니다. 자습서에서는 일반적으로 새 리소스 그룹을 만드는 것이 가장 좋습니다. 자습서에서 만든 모든 Azure 리소스를 한 번에 쉽게 삭제할 수 있기 때문입니다. 자세한 내용은 [Azure Resource Manager 개요](../resource-group-overview.md)를 참조하세요.
...
7. **앱 서비스 만들기** 대화 상자에서 **만들기**를 클릭합니다.
	...
	**Azure 앱 서비스 작업** 창에 웹앱이 만들어졌다고 표시됩니다.
	...
	이제 **클라우드 탐색기**에서 웹 앱을 확인할 수 있습니다.
...
1. **솔루션 탐색기**에서 프로젝트를 마우스 오른쪽 단추로 클릭하고 **게시**를 선택합니다.
	...
	게시 프로필은 배포하기 위한 사용자 이름 및 암호를 포함합니다. 이러한 자격 증명이 생성되었지만 사용자가 선택한 값으로 변경할 수 있습니다. Azure 포털을 소개하는 자습서 섹션에서 작업을 수행하는 방법을 확인합니다. 암호는 `Properties\PublishProfiles` 폴더의 숨겨진 사용자 특정 파일에 암호화됩니다.
...
3. **설정** 블레이드는 더 많은 웹 앱 구성 옵션을 표시합니다.
	...
	**게시** 섹션에서 **배포 자격 증명**이 연결됩니다. 여기에 배포를 위해 새 사용자 이름 및 암호를 만듭니다. 선택한 사용자 이름 및 암호를 입력한 후에 동일한 값을 웹 프로젝트의 **웹 게시** 마법사에 있는 **연결** 탭에 입력할 수 있습니다.
...
Azure 포털에서 **웹 앱** 블레이드의 위에 있는 **삭제** 단추를 클릭하면 웹 앱을 쉽게 삭제할 수 있습니다. 더 나은 방법은 웹 앱을 포함하도록 만든 리소스 그룹을 삭제하는 것입니다. 이 자습서의 경우 리소스 그룹에는 웹 앱만이 포함되어 있지만 일반적인 리소스 그룹은 관련된 리소스의 컬렉션을 모두 포함합니다. 예를 들어, 웹 앱은 웹 앱이 더 이상 필요없는 경우 필요하지 않은 데이터베이스 또는 Azure 저장소 계정을 사용할 수 있습니다. 리소스 그룹을 삭제하면 포함된 모든 항목이 삭제됩니다. Azure 포털을 사용하여 리소스 그룹을 삭제하려면 다음 단계를 수행합니다.
...
* 채팅과 같은 실시간 기능을 추가하는 방법
	...
	채팅 서비스, 게임 또는 주식 기호와 같은 실시간 기능의 경우 [WebSocket](/blog/2013/11/14/introduction-to-websockets-on-windows-azure-web-sites/) 전송 방식으로 [ASP.NET SignalR](http://www.asp.net/signalr)을 사용하여 최상의 성과를 얻을 수 있습니다. 자세한 내용은 [Azure 웹앱에서 SignalR 사용](http://www.asp.net/signalr/overview/signalr-20/getting-started-with-signalr-20/using-signalr-with-windows-azure-web-sites)을 참조하세요.
