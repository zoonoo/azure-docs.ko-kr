...
## <a name="deploy-a-java-app-to-your-web-app"></a>웹앱에 Java 앱 배포
이제 FTPS를 사용하여 Azure에 Java 앱을 배포해보겠습니다.

1. 웹앱 블레이드에서 **응용 프로그램 설정** 까지 아래로 스크롤하거나 해당 옵션을 검색한 다음 클릭합니다. 
...
5. 웹앱 블레이드로 돌아가 **개요**를 클릭합니다. **FTP/Deployment username** 및 **FTPS 호스트 이름** 옆에 있는 **복사** 단추를 클릭하여 이들 값을 복사합니다.
   
    ![](./media/app-service-web-get-started-languages/get-ftp-url.png)
   
    이제 FTPS를 사용하여 Java 앱을 배포할 준비가 되었습니다.
6. FTP/FTPS 클라이언트에서 마지막 단계에서 복사한 값을 사용하여 Azure 웹앱의 FTP 서버에 로그인합니다. 앞에서 만든 배포 암호를 사용합니다.
...
7. [이 링크](https://github.com/Azure-Samples/app-service-web-java-get-started/raw/master/webapps/ROOT.war) 를 클릭하여 WAR 파일을 로컬 컴퓨터에 다운로드합니다.
8. FTP/FTPS 클라이언트에서, 원격 사이트에 있는 **/site/wwwroot/webapps** 로 이동하여 로컬 컴퓨터에 다운로드한 WAR 파일을 해당 원격 디렉터리로 끕니다.
   ...
   > [!NOTE]
   > Tomcat의 기본 동작에 따라 /site/wwwroot/webapps의 파일 이름 **ROOT.war**는 루트 웹앱(http://*&lt;appname>*.azurewebsites.net)을, 파일 이름***&lt;anyname>*.war**는 명명된 웹앱(http://*&lt;appname>*.azurewebsites.net/*&lt;anyname>*)을 제공합니다.
   > 
   > 
