...
Microsoft Azure에서 웹 앱에 대한 향상된 성능 및 처리량은 [Azure 포털](http://portal.azure.com)을 사용하여 [앱 서비스](http://go.microsoft.com/fwlink/?LinkId=529714) 계획을 **무료** 모드에서 **공유**, **기본**, **표준** 또는 **프리미엄** 모드로 크기를 조정할 수 있습니다.
...
크기 조정 설정을 적용하는 데 몇 초밖에 걸리지 않으며 앱 서비스 계획의 모든 웹앱에 영향을 줍니다. 또한 변경을 적용하기 위해 코드를 변경하거나 응용 프로그램을 다시 배포할 필요도 없습니다.

앱 서비스 계획에 대한 자세한 내용은 [앱 서비스 계획 정의](../app-service/app-service-how-works-readme.md) 및 [Azure 앱 서비스 호스팅 계획의 포괄 개요](../app-service/azure-web-sites-web-hosting-plans-in-depth-overview.md)를 참조하세요. 개별 앱 서비스 계획의 가격 및 기능에 대한 자세한 내용은 [앱 서비스 가격 정보](/pricing/details/web-sites/)를 참조하세요.
...
2. 웹앱 블레이드에서 **모든 설정**을 클릭한 다음 **강화**을 클릭합니다.
...
1. **표준** 또는 **프리미엄** 모드로 크기를 조정하려면 **공유** 또는 **기본**으로 크기를 조정할 때와 동일한 초기 단계를 따라 **가격 책정 계층 선택**에서 **표준** 또는 **프리미엄** 모드를 선택한 다음 **선택**을 클릭합니다. 
	...
	작업이 완료되면 **알림** 탭에 **성공**이 녹색으로 깜박이고 **자동 크기 조정**이 사용으로 설정됩니다.
	
	![표준 또는 프리미엄 모드로 크기 조정][ScaleStandard]
	
	위에 표시된 **기본** 모드와 마찬가지로 **인스턴스** 막대를 밀어 수동으로 추가 인스턴스로 크기를 조정할 수도 있습니다. 그러나 여기서는 앱의 크기를 자동으로 조정하는 방법을 알아봅니다.
	
2. 앱의 크기를 자동으로 조정하려면 **크기 조정 기준**에서 **일정 및 성능 규칙**을 선택합니다.
	
	![자동 크기 조정 모드를 성능으로 설정][Autoscale]
	
3. **설정**에서 **기본값, 크기 조정 1-1**을 클릭하고 두 개의 슬라이더를 이동하여 앱 서비스 계획에 대해 자동으로 크기를 조정할 최소 인스턴스 수와 최대 인스턴스 수를 정의합니다. 이 자습서에서는 최대 슬라이더를 **6**개 인스턴스로 이동합니다.
	
4. **확인**을 클릭합니다.
	
4. **설정**에서 **CPU 비율 > 80(개수 1씩 증가)**을 클릭하여 기본 메트릭에 대한 자동 크기 조정 규칙을 구성합니다.
	...
	- 최근 10분간 CPU가 80%를 초과할 경우 크기를 1개 인스턴스 확장합니다.
	- 최근 5분간 CPU가 90%를 초과할 경우 크기를 3개 인스턴스 확장합니다.
	- 최근 30분간 CPU가 50% 미만일 경우 크기를 1개 인스턴스 축소합니다. 
	
	
4. **메트릭 이름** 드롭다운을 **CPU 백분율**로 유지합니다.
	
5. **확대 규칙**에서 **연산자**는 **보다 큼**, **임계값**은 **70**(%), **기간**은 **10**(분), **시간 집계**는 평균, **작업**은 **다음을 기준으로 개수 늘이기** **1**(인스턴스), **정지 시간**은 **10**(분)으로 설정하여 첫 번째 규칙을 구성합니다.
	
	![첫 번째 자동 크기 조정 규칙 설정][SetFirstRule]
	
	>[AZURE.NOTE] **냉각** 설정은 이전 크기 조정 작업 이후에 이 규칙이 다시 크기를 조정하기 위해 대기해야 하는 기간을 지정합니다.
	
6. **규칙 추가**를 클릭한 다음 **연산자**는 **보다 큼**, **임계값**은 **90**(%), **기간**은 **1**(분), **시간 집계**은 평균, **작업**은 **다음을 기준으로 개수 늘이기**, **값**은 **3**(인스턴스), **정지 시간**은 **1**(분)로 설정하여 두 번째 규칙을 구성합니다.

7. **확인**을 클릭합니다.
	
	![두 번째 자동 크기 조정 규칙 설정][SetSecondRule]
	
5. **설정**에서 **규칙 추가**를 클릭하여 **연산자**는 **보다 작음**, **임계값**은 **50**(%), **기간**은 **30**(분), **시간 집계**은 **평균**, **작업**은 **다음을 기준으로 개수 줄이기**, **값**은 **1**(인스턴스), **정지 시간**은 **60**(분)으로 설정하여 세 번째 규칙을 구성합니다.
	
	![세 번째 자동 크기 조정 규칙 설정][SetThirdRule]
	
7. **확인**을 클릭합니다. 이제 자동 크기 조정 규칙이 **크기 조정 설정** 블레이드에 반영됩니다.
...
2. 연결된 SQL 데이터베이스 블레이드에서 **설정** > **가격 책정 계층** 부분을 클릭하고 성능 요구 사항에 따라 계층 중 하나를 선택한 다음 **선택**을 클릭합니다.
