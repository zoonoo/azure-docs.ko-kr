...
Node.js http 서버에서 기본 제공을 시작하려면 터미널 창을 열고 샘플의 `npm start` 스크립트를 사용하여 응용 프로그램을 로컬로 실행합니다.
...
이번에는 Azure App Service 웹앱으로 실행되는 Node.js 코드를 사용하여 Hello World 메시지를 표시하는 페이지가 실행되고 있습니다.

## <a name="updating-and-deploying-the-code"></a>코드 업데이트 및 배포

로컬 텍스트 편집기를 사용하여 Node.js 앱 내에서 `index.js` 파일을 열고 `response.end`에 대한 호출 내의 텍스트를 약간 변경합니다.
