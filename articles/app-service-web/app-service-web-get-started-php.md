...
3. “표준” [App Service 계획](../app-service/azure-web-sites-web-hosting-plans-in-depth-overview.md)을 만듭니다. 표준 계층은 Linux 컨테이너를 실행해야 합니다.

    ```azurecli
    az appservice plan create --name my-free-appservice-plan --resource-group myResourceGroup --sku S1 --is-linux 
    ```

4. `<app_name>`에 고유한 이름이 있는 웹앱을 만듭니다.

    ```azurecli
    az appservice web create --name <app_name> --resource-group myResourceGroup --plan my-free-appservice-plan
    ```

4. 기본 PHP 7.0.6 이미지를 사용하도록 Linux 컨테이너를 구성합니다.

    ```azurecli
    az appservice web config update --php-version 7.0.6 --name <app_name> --resource-group myResourceGroup
    ```

4. GitHub에서 샘플 PHP 앱을 배포합니다.
