...
**IP 주소별 액세스 제한**|클라이언트 엔드포인트 액세스의 제한 여부를 결정하는 설정.|예/아니요| 아니오
...
**정렬자 노드 수** |트랜잭션을 블록으로 정렬(구성)하는 노드 수.--> 이 명령문은 길고 장황합니다. 정렬 서비스에 대한 자세한 내용은 Hyperledger [설명서](http://hyperledger-fabric.readthedocs.io/en/latest/orderingservice.html)를 참조하세요.<br /><br />이 값은 현재 노드 1개로 제한되어 있습니다. |1 |1
