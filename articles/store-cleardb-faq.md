...
## <a name="do-i-need-a-credit-card-for-the-web-app-mysql-template-in-the-azure-marketplace"></a>Azure 마켓플레이스의 웹앱 + MySQL 템플릿에 신용 카드가 필요한가요?
...
* [EA(기업계약)](/pricing/enterprise-agreement/): EA 고객은 Azure Marketplace(타사)에서 구매한 모든 제품에 대해 EA 분기별로 별도의 통합 청구서로 청구됩니다. 모든 마켓플레이스에서 구매한 제품은 현금 약정 금액과 별도로 청구됩니다. 현재 아제르바이잔, 크로아티아, 노르웨이 및 푸에르토리코에서 등록한 고객은 Azure 저장소를 사용할 수 없습니다. 
...
## <a name="why-was-i-charged-350-for-a-web-app-mysql-from-the-azure-marketplace"></a>왜 Azure 마켓플레이스에서 웹앱+ MySQL에 대해 3.50달러가 청구되었나요?
...
웹앱의 경우 기본이나 더 높은 가격 책정 계층을 사용합니다. ClearDB의 경우 Saturn 또는 Jupiter 플랜을 사용하는 것이 좋습니다. [Web Apps](/pricing/details/app-service/) 및 [ClearDB MySQL 데이터베이스](/marketplace/partners/cleardb/databases/)에 대한 각 가격 책정 계층의 기능 및 제한 사항을 검토하여 요구에 맞는 계층을 선택합니다.
...
Azure Resource Manager를 사용하여 ClearDB 데이터베이스 또는 [새 Azure Portal](https://portal.azure.com)을 만드는 경우 [이전 Azure Portal](https://manage.windowsazure.com)에는 표시되지 않습니다. 이 문제를 해결하려면 데이터베이스를 웹앱에 수동으로 연결합니다. 마찬가지로 [이전 포털](https://manage.windowsazure.com)에서 ClearDB 데이터베이스를 만드는 경우 [새 Azure Portal](https://portal.azure.com)에서 데이터베이스를 볼 수 없습니다. 후자의 시나리오의 경우 해결 방법은 없습니다.
