...
- 사용 가능한 클라이언트 SDK 중 하나: [Azure Media Services SDK for .NET](https://github.com/Azure/azure-sdk-for-media-services), [Java용 Azure SDK](https://github.com/Azure/azure-sdk-for-java), [Azure Media Services for Node.js](https://github.com/fritzy/node-azure-media), [Azure PHP SDK](https://github.com/Azure/azure-sdk-for-php)
...
다음 포스터는 미디어 만들기부터 소비까지의 Azure 미디어 서비스 워크플로를 보여 줍니다. 포스터는 [http://www.microsoft.com/download/details.aspx?id=38195)Azure 미디어 서비스 포스터]((영문)에서 다운로드할 수 있습니다.

![Overview][overview]

##개념

자세한 내용은 [개념]을 참조하세요(../media-services-concepts).
...
##일반적인 개발 워크플로: 주문형 비디오 및 라이브 스트리밍
...
1. (선택 사항) 콘텐츠 보호 및 키 권한 부여 정책(동적 암호화에서 사용)을 구성합니다.
1. 자산을 게시합니다.
1. 게시된 콘텐츠를 스트리밍합니다. 
...
[미디어 서비스 주문형 비디오]에는(../media-services-video-on-demand-workflow) 위에서 설명한 태스크 수행 방법을 보여 주는 항목에 대한 링크가 포함됩니다.

###라이브 스트리밍

1. 채널을 만들고 시작합니다.
1. 채널 수집 URL을 검색합니다.
1. 원하는 라이브 트랜스코더를 시작하고 구성합니다.
1. 채널의 미리 보기 끝점을 검색하고 채널이 라이브 스트림을 제대로 받고 있는지 확인합니다.
1. 프로그램 및 자산을 만듭니다. 
2. 자산에 대한 배달 정책(종적 패키징에서 사용)을 구성합니다.
3. (선택 사항) 콘텐츠 보호 및 키 권한 부여 정책(동적 암호화에서 사용)을 구성합니다.
1. 사용자가 스트림을 사용할 수 있게 하려면 프로그램과 연결된 자산에 액세스하는 데 필요한 스트리밍 로케이터를 만듭니다.
1. 스트리밍 및 보관을 시작할 준비가 되었으면 프로그램을 시작합니다.
1. 이벤트 스트리밍 및 보관을 중지할 때마다 프로그램을 중지 합니다.
1. 프로그램을 삭제하고 필요에 따라 자산을 삭제합니다.

[미디어 서비스 라이브 스트리밍](../media-services-live-streaming-wrokflow) 위에서 설명한 태스크 수행 방법을 보여 주는 항목에 대한 링크가 포함됩니다.
...
- [OSMF용 Microsoft 부드러운 스트리밍 플러그 인](https://www.microsoft.com/download/details.aspx?id=36057)
...
자세한 내용은 [비디오 플레이어 응용 프로그램 개발]을 참조하세요.(../media-services-develop-video-players)
