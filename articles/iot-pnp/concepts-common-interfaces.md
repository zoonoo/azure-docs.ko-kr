...
| -------- | -------- | -------- | -------- | -------- | -------- |
| 모델 정보 | 항아리:azureiot:모델 디스커버리:모델 정보:1 | 장치가 기능 모델 ID 및 인터페이스를 선언합니다. 모든 IoT 플러그 앤 플레이 장치에 필요합니다. | yes | 예 |
| 디지털 트윈 클라이언트 SDK 정보 | 항아리:azureiot:클라이언트:SDK정보:1 | 장치를 Azure와 연결하기 위한 클라이언트 SDK입니다. [인증에](tutorial-build-device-certification.md) 필요 | yes | 예 |
| 장치 정보 | 항아리:azureiot:장치 관리:장치 정보:1 | 장치에 대한 하드웨어 및 운영 체제 정보입니다. [인증에](tutorial-build-device-certification.md) 필요 | 예 | yes |
| 모델 정의 | 항아리:azureiot:모델 디스커버리:모델 정의:1 | 장치가 해당 기능 모델 및 인터페이스에 대한 전체 정의를 선언합니다. 모델 정의가 모델 리포지토리에서 호스팅되지 않는 경우 구현해야 합니다. | 예 | yes |
