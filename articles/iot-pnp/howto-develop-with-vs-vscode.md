...
- **프로젝트 형식**입니다. 코드 생성기는 또한 프로젝트 파일을 생성 하 여 코드를 사용자 고유의 프로젝트 또는 장치 SDK 프로젝트에 통합할 수 있습니다. 현재 지원 되는 프로젝트 형식은 다음과 같습니다.

    - **Cmake 프로젝트**: [cmake](https://cmake.org/) 를 빌드 시스템으로 사용 하는 장치 프로젝트의 경우 이 옵션은 C `CMakeLists.txt` 코드와 동일한 폴더에 파일을 생성 합니다.
...
- **Azure IoT에 연결 하는 방법**입니다. 생성 된 파일에는 Azure IoT Hub에 연결 하도록 장치를 구성 하는 코드도 포함 되어 있습니다. [Azure IoT Hub](https://docs.microsoft.com/azure/iot-hub) 에 직접 연결 하거나 [장치 프로 비전 서비스](https://docs.microsoft.com/azure/iot-dps)를 사용 하도록 선택할 수 있습니다.
    ...
    - **Dps 대칭 키를 통해**: IoT Hub에 연결 하거나 DPS를 사용 하 여 IoT Central 하는 데 필요한 장치 응용 프로그램의 **범위 ID**, **등록 ID**및 **SaS 키** 를 지정 합니다.
...
1. VS Code에서 **Ctrl + Shift + P** 를 사용 하 여 명령 팔레트를 열고를 입력 하 **고 CC++/:를 선택 합니다. 구성 (json)** 을 편집 하 여 **c_cpp_properties** 파일을 엽니다.
...
Ubuntu 또는 Debian와 같은 Linux 환경에서 CMake를 사용 하 여 장치 C SDK와 함께 장치 코드를 빌드하려면 다음을 수행 합니다.

1. 터미널 응용 프로그램을 엽니다.

1. 다음 `apt-get` 명령을 사용 하 여 `cmake` **GCC**, **Git**, 및 모든 종속성을 설치 합니다.
    ...
    버전 `cmake` 을 **2.8.12** 이상으로 확인 하 고 **GCC** 버전을 **4.4.7**이상으로 확인 합니다.
...
1. [Azure IoT C SDK](https://github.com/Azure/azure-iot-sdk-c) 리포지토리를 복제 합니다.
...
1. 생성 된 코드가 포함 된 폴더를 장치 SDK 루트 폴더에 복사 합니다.

1. VS Code에서 장치 SDK 루트 `CMakeLists.txt` 폴더에 있는 파일을 엽니다.

1. SDK를 컴파일할 때 장치 코드 스텁 폴더를 `CMakeLists.txt` 포함 하려면 파일의 끝에 다음 줄을 추가 합니다.

    ```txt
    add_subdirectory({generated_code_folder_name})
    ```

1. 장치 SDK 루트 폴더 `cmake` 에 이라는 폴더를 만들고 해당 폴더로 이동 합니다.
...
명령줄에서 CMake 및 Visual Studio C/C++ 컴파일러를 사용 하 여 Windows의 장치 C SDK와 함께 장치 코드를 빌드하려면 [IoT 플러그 앤 플레이 빠른](./quickstart-create-pnp-device.md)시작을 참조 하세요. 다음 단계에서는 Visual Studio에서 CMake 프로젝트로 장치 C SDK와 함께 장치 코드를 빌드하는 방법을 보여 줍니다.
...
1. Visual Studio를 열고 **시작** 페이지에서 **복제 또는 체크 아웃 코드**를 선택 합니다.

1. **리포지토리 위치**에서 [AZURE IoT C SDK](https://github.com/Azure/azure-iot-sdk-c) 리포지토리를 복제 합니다.

    ```txt
    https://github.com/Azure/azure-iot-sdk-c
    ```

    이 작업을 완료 하는 데 몇 분이 소요 될 것으로 예측 하면 **팀 탐색기** 창에서 진행률을 볼 수 있습니다.

1. **팀 탐색기**에서 **azure-iot-c** 리포지토리를 열고 **분기**를 선택 하 고, **공개-미리 보기** 분기를 검색 하 여 체크 아웃 합니다.

    ![공개 미리 보기](media/howto-develop-with-vs-vscode/vs-public-preview.png)

1. 생성 된 코드가 포함 된 폴더를 장치 SDK 루트 폴더에 복사 합니다.

1. VS에서 `azure-iot-sdk-c` 폴더를 엽니다.

1. 장치 SDK `CMakeLists.txt` 루트 폴더에서 파일을 엽니다.
...
1. **Cmake 설정**에서 새 구성을 추가 하 고 **x64 릴리스** 를 대상으로 선택 합니다.
...
1. **솔루션 탐색기**에서 루트 폴더 `CMakeLists.txt` 의을 마우스 오른쪽 단추로 클릭 하 고 상황에 맞는 메뉴에서 **빌드** 를 선택 하 여 장치 SDK 및 생성 된 코드 스텁을 빌드합니다.
...
다음 단계는 CMake를 사용 하 여 macOS에서 장치 C SDK와 함께 장치 코드를 빌드하는 방법을 보여 줍니다.
