...
1. 도구가 디바이스의 인터페이스 모델 정의를 읽을 수 있도록 **설정**을 선택합니다. [설정] 메뉴의 [플러그 앤 플레이 구성]에 **연결된 디바이스**가 이미 표시되어 있을 수도 있습니다. 표시되지 있지 않으면 **+ 모듈 정의 원본 새로 만들기**를 선택한 다음, **연결된 디바이스**를 선택하여 추가합니다.
