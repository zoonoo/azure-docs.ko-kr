...
- 큐 레이트 모델 Microsoft는 열려 있는 GitHub PR 유효성 검사 워크플로를 사용 하 여 사용 가능한 모든 인터페이스를 검토 하 고 승인 합니다.
- 불변성.  게시 된 후에는 인터페이스를 업데이트할 수 없습니다.
- 하이퍼-규모. Microsoft는 안전 하 고 확장성이 뛰어난 끝점을 만드는 데 필요한 모든 인프라를 제공 합니다.
...
모델 리포지토리에 저장 된 공용 디지털 쌍 모델은 모든 사용자가 응용 프로그램에서 사용 하 고 통합 하는 데 사용할 수 있습니다. 공용 모델을 사용 하면 장치 빌더 및 솔루션 개발자가 IoT 플러그 앤 플레이 장치 모델을 공유 하 고 다시 사용할 수 있는 열린 에코 시스템을 사용할 수 있습니다.
...
1. [변경 내용의 유효성을 검사 하기 위해 스크립트](#validate-files) 를 사용 하 여 로컬에서 모델의 유효성을 검사 합니다.
1. 로컬에서 변경 내용을 커밋하고 포크로 푸시합니다.
1. 포크에서 분기를 대상으로 하는 PR을 만듭니다 `main` . [문제 만들기 또는 끌어오기 요청](https://docs.github.com/free-pro-team@latest/desktop/contributing-and-collaborating-using-github-desktop/creating-an-issue-or-pull-request) 문서를 참조 하세요.
1. [PR 요구 사항을](https://github.com/Azure/iot-plugandplay-models/blob/main/pr-reqs.md)검토 합니다.

PR은 제출 된 새 인터페이스의 유효성을 검사 하는 일련의 GitHub 작업을 트리거하고 PR이 모든 검사를 충족 하는지 확인 합니다.

Microsoft는 영업일 3 일 이내에 모든 검사를 통해 PR에 응답 합니다.
...
PR을 제출 하기 전에 로컬에서 동일한 유효성 검사를 실행 하 여 문제를 미리 진단할 수 있습니다.
...
[Dtdl 유효성 검사 샘플](https://github.com/Azure-Samples/DTDL-Validator) 을 실행 하 여 로컬에서 모델의 유효성을 검사할 수 있습니다.
