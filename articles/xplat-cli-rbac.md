<properties
	pageTitle="Mac, Linux 및 Windows용 Azure CLI를 사용하여 역할 기반 액세스 제어 관리"
	description="Azure CLI를 사용하여 역할 기반 액세스 제어 관리."
	services=""
	documentationCenter=""
	authors="squillace"
	manager="timlt"
...
명령줄 프롬프트에 Azure 계정 암호를 입력합니다. 회사 또는 학교 ID만 지원됩니다(**조직 ID**라고도 부름). Azure CLI는 이 계정에 포함된 모든 구독을 가져오며 첫 번째 구독을 기본값으로 사용하도록 자체적으로 구성합니다. RBAC를 사용하는 경우에는 자신이 공동 관리자이거나 역할을 할당받아 특정 권한을 가진 구독만 가져올 수 있습니다.
...
자세한 내용은 [Azure CLI 명령](azure-cli-arm-commands.md)을 참조하세요.
