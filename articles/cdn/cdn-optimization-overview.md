...
Akamai의 Azure Content Delivery Network는 다음을 지원합니다.
...
Verizon의 Azure Content Delivery Network는 일반 웹 배달만 지원하며, 주문형 비디오 및 대용량 파일 다운로드에 사용할 수 있습니다. 최적화 형식을 선택할 필요가 없습니다.

다양한 공급자 간의 성능 차이를 테스트하여 배달에 가장 적합한 공급자를 선택하는 것이 좋습니다.

## <a name="select-and-configure-optimization-types"></a>최적화 형식 선택 및 구성

새 끝점을 만들려면 끝점에서 제공할 시나리오 및 콘텐츠 형식과 가장 잘 맞는 최적화 형식을 선택합니다. **일반 웹 배달**이 기본 선택 항목입니다. 언제든지 기존 Akamai 끝점에 대한 최적화 옵션을 업데이트할 수 있습니다. 이러한 변경으로 인해 CDN에서 배달이 중단되지 않습니다. 

1. 표준 Akamai 프로필 내에서 끝점을 선택합니다.

    ![끝점 선택 ](./media/cdn-optimization-overview/01_Akamai.png)

2. **설정** 아래에서 **최적화**를 선택합니다. 그런 다음 **Optimized for**(최적화 대상) 드롭다운 목록에서 형식을 선택합니다.
...
> [!NOTE]
> Akamai의 Azure Content Delivery Network를 사용하는 경우 평균 파일 크기가 10MB보다 작으면 이 최적화를 사용할 수 있습니다. 평균 파일 크기가 10MB보다 큰 경우 **Optimized for**(최적화 대상) 드롭다운 목록에서 **대용량 파일 다운로드**를 선택합니다.
...
Akamai의 Azure Content Delivery Network를 사용하는 경우 대용량 파일 다운로드를 사용하여 1.8GB보다 큰 파일을 배달해야 합니다. Verizon의 Azure Content Delivery Network에서는 일반 웹 배달 최적화의 파일 다운로드 크기에 대한 제한이 없습니다.
...
### <a name="dynamic-site-acceleration"></a>동적 사이트 가속
 ...
 동적 사이트 가속은 Akamai 및 Verizon Content Delivery Network 프로필에서 모두 사용할 수 있습니다. 이 최적화는 추가 사용 요금에 포함됩니다. 자세한 내용은 가격 책정 페이지를 참조하세요.
