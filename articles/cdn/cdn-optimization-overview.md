...
ms.locfileid: "79252117"
---
# <a name="optimize-azure-cdn-for-the-type-of-content-delivery"></a>콘텐츠 전송 형식에 맞게 Azure CDN 최적화

전 세계의 수많은 대상 고객에게 콘텐츠를 배달하는 경우 콘텐츠의 최적화된 배달을 보장해야 합니다. [AZURE 콘텐츠 배달 네트워크(CDN)는](cdn-overview.md) 사용자 콘텐츠 유형에 따라 배달 환경을 최적화할 수 있습니다. 콘텐츠는 다운로드를 위한 웹 사이트, 라이브 스트림, 비디오 또는 대용량 파일일 수 있습니다. CDN 엔드포인트를 만들 때 **최적화됨** 옵션에서 시나리오를 지정합니다. 선택 항목에 따라 CDN 엔드포인트에서 배달되는 콘텐츠에 적용될 최적화가 결정됩니다.
...
> [!NOTE]
> Microsoft의 동적 사이트 가속은 [Azure 정문 서비스를](https://docs.microsoft.com/azure/frontdoor/front-door-overview)통해 제공됩니다.
...
2. 설정에서 **최적화를 선택합니다.** 그런 다음, **다음에 대해 최적화됨** 드롭다운 목록에서 형식을 선택합니다.
...
> [!NOTE]
> CDN 엔드포인트에서 주로 주문형 비디오 콘텐츠를 제공하는 경우 이 최적화 형식을 사용합니다. 이 최적화 유형과 일반 미디어 스트리밍 최적화 유형 간의 가장 큰 차이점은 연결 재시도 시간 입니다. 라이브 스트리밍 시나리오에서는 시간 시간이 훨씬 짧습니다.
>
...
> [!NOTE]
> Microsoft의 동적 사이트 가속은 앱 워크로드를 제공하기 위해 Microsoft의 사설 글로벌 네트워크를 활용하는 글로벌 [anycast](https://en.wikipedia.org/wiki/Anycast) 서비스인 [Azure Front Door Service를](https://docs.microsoft.com/azure/frontdoor/front-door-overview) 통해 제공됩니다.
