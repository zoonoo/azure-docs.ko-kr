...
2. **HTTP Large** 탭을 선택한 다음, **규칙 엔진**을 선택합니다.
   ...
   > [!IMPORTANT]
   > 여러 규칙이 나열된 순서는 규칙이 처리되는 방식에 영향을 줍니다. 후속 규칙은 이전 규칙에서 지정된 동작을 재정의할 수 있습니다. 예를 들어 요청 속성 및 모든 요청에 대 한 액세스를 거부 하는 규칙을 기반으로 리소스에 대 한 액세스를 허용 하는 규칙이 있는 경우 두 번째 규칙은 첫 번째 규칙을 재정의 합니다. 규칙은 동일한 속성으로 상호 작용 하는 경우에만 이전 규칙을 재정의 합니다.
   >
