...
* 원본 서버에서 압축을 사용합니다. 이 경우 CDN은 압축된 파일을 통과시켜 요청하는 클라이언트에 전달합니다.
* CDN 에지 서버에서 직접 압축을 사용합니다. 이 경우 원본 서버에서 압축되지 않더라도 CDN이 파일을 압축하여 최종 사용자에게 제공합니다.

> [!IMPORTANT]
> CDN 구성 변경이 네트워크 통해 전파되려면 다소 시간이 걸릴 수 있습니다.  <b>Akamai의 Azure CDN</b> 프로필의 경우, 일반적으로 1분 이내에 전파가 완료됩니다.  <b>Verizon의 Azure CDN</b> 프로필의 경우, 변경 내용은 일반적으로 90분 내에 적용됩니다.  CDN 끝점에 처음으로 압축을 설정하는 경우 압축 설정이 POP까지 전파되도록 1-2시간 기다렸다가 문제를 해결합니다.
> 
> 
...
2. **구성** 단추를 클릭합니다.
    ...
    ![CDN 프로필 관리 단추](./media/cdn-file-compression/cdn-config-btn.png)
...
1. CDN 프로필 페이지에서 **관리** 단추를 클릭합니다.
    ...
    ![CDN 프로필 관리 단추](./media/cdn-file-compression/cdn-manage-btn.png)
...
2. **HTTP Large** 탭을 가리킨 다음 **캐시 설정** 플라이아웃을 가리킵니다.  **압축**을 클릭합니다.
    ...
    ![파일 압축 옵션](./media/cdn-file-compression/cdn-compress-files.png)
...
> 
>이들 프로필은 **gzip** 인코딩만 지원합니다. 프로필 끝점이 **gzip** 인코딩 파일을 요청하는 경우 클라이언트 요청에 관계없이 원본으로부터 항상 요청됩니다. 
...
| 압축 |캐시되지 않음 |압축 |원본이 미압축 상태로 반환되면 CDN가 압축을 수행합니다.  **Verizon에서 Azure CDN** 은 첫 번째 요청에 압축되지 않은 파일을 전달한 다음 후속 요청에 대한 파일을 압축하고 캐시합니다.  `Cache-Control: no-cache` 헤더가 있는 파일은 압축되지 않습니다. |
...
Azure Portal에서 언급된 형식에 대해 압축을 사용하거나 사용하지 않도록 설정할 수 없습니다.  
