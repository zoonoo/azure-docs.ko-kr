...
-	**Azure Managed Library 참조**에서 제공된 관리되는 API 사용
-	타사 저장소 관리 도구 사용
-	Azure 저장소 서비스 REST API 사용  

다음 코드 예제는 Azure Managed Library를 사용하여 컨테이너를 만들고, 공용 액세스를 위해 사용 권한을 설정하고, 컨테이너 내에 Blob을 만드는 콘솔 응용 프로그램입니다. 또한 Blob의 Cache-Control 헤더를 설정하여 원하는 새로 고침 간격을 명시적으로 지정합니다.
