...
| 대륙(요청자) | %{geo_continent} | 해당 약어를 통해 요청자의 대륙을 나타냅니다. <br />유효한 값은 <br />AF 아프리카<br />는 아시아<br />EU Europe<br />수치 North America<br />OC 오세아니아<br />SA 남아메리카<br /><br />**사용되지 않음:** %{virt_dst_continent}. <br />이 변수는% {geo_continent}을 (를) 위해 더 이상 사용 되지 않습니다. <br />이 사용되지 않는 변수를 사용하는 규칙을 계속 사용할 수 있더라도, 새 변수를 사용하도록 업데이트해야 합니다.| 해당 사항 없음 |
...
| 쿼리 문자열 있음 | %{is_args} | 이 변수에 대한 값은 요청에 쿼리 문자열이 포함되어 있는지 여부에 따라 달라집니다.<br /><br />- 쿼리 문자열 있음: ?<br />-쿼리 문자열이 없습니다. NULL | ? |
...
| 요청 URI(쿼리 문자열을 사용하지 않는 상대값) | %{uri} | 요청된 콘텐츠에 대한 상대 경로를 나타냅니다. <br /><br/>주요 정보:<br />- 이 상대 경로는 쿼리 문자열을 제외합니다.<br />- 이 상대 경로는 URL 다시 작성을 반영합니다. URL은 다음과 같은 조건에서 다시 작성됩니다.<br />  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-URL 재작성 기능: 이 기능은 요청 URI에 정의 된 상대 경로를 다시 작성 합니다.<br />    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-에 지 CNAME URL: 이 유형의 요청은 해당 CDN URL로 다시 작성 됩니다. |/800001/corigin/rewrittendir/foo.js |
...
- 변수와 연결된 값을 조작합니다.
     ...
     예제: 정규식을 사용 하 여 HTTP 변수와 연결 된 값을 변경할 수 있습니다.
...
| 잘못된 문자 또는 구문 | 잘못된 문자 또는 구문을 포함하는 변수는 리터럴 값으로 처리됩니다. <br /><br />#1 예: 지정 된 값에 잘못 된 문자가 포함 된 경우 (예:-) <br /><br />#2 예: 지정 된 값에 이중 중괄호 집합이 포함 되어 있습니다. <br /><br />#3 예: 지정 된 값에 닫는 중괄호가 없습니다.<br /> | 예제 #1: %{resp_user-agent} <br /><br />예제 #2: %{{host}} <br /><br />예제 #3: %{host |
...
| 조건 | 구문 | 예제 | 설명 |
| --------- | ------ | --------| ----------- |
| 다음 조건 중 하나라도 충족할 경우 헤더를 기본값으로 설정합니다. <br /><br />- 헤더 누락 <br /><br />- 헤더 값이 NULL로 설정됨.| %{Variable:=Value} | %{http_referrer:=unspecified} | 참조 페이지 헤더는 없거나 NULL로 설정 된 경우에만 *지정 되지 않음으로* 설정 됩니다. 설정된 경우 아무 작업도 수행되지 않습니다. |
| 누락된 경우 헤더를 기본값으로 설정합니다. | %{Variable=Value} | %{http_referrer=unspecified} | 참조 페이지 헤더는 없는 경우에만 *지정 되지 않음으로* 설정 됩니다. 설정된 경우 아무 작업도 수행되지 않습니다. |
| 다음 조건 중 하나라도 충족하지 않을 경우 헤더를 기본값으로 설정합니다. <br /><br />- 누락됨<br /><br /> - NULL로 설정됨. | %{Variable:+Value} | %{http_referrer:+unspecified} | 참조 페이지 헤더는 값이 할당 된 경우에만 *지정 되지 않음으로* 설정 됩니다. 누락되거나 NULL로 설정되지 않은 경우 아무 작업도 수행되지 않습니다. |
...
- 오프셋 용어에 할당된 값은 부분 문자열의 시작 문자를 결정합니다.
     ...
     - 반환 부분 문자열의 시작 문자는 문자열의 첫 번째 문자입니다.
     - 음수: 부분 문자열의 시작 문자는 문자열의 마지막 문자에서 계산 됩니다.

- 부분 문자열의 길이는 *길이* 용어에 의해 결정됩니다.

     - 하지 길이 단어를 생략 하면 부분 문자열이 시작 문자와 문자열 끝 사이의 모든 문자를 포함할 수 있습니다.
     - 양의 시작 문자에서 오른쪽으로 부분 문자열의 길이를 결정 합니다.
     - 음수: 왼쪽의 시작 문자에서 부분 문자열의 길이를 확인 합니다.
...
- 연산자는 패턴 앞에 지정되어야 합니다. 지정된 연산자는 패턴 캡처 동작을 결정합니다.
     ...
     - `=`: 지정 된 패턴의 모든 항목을 캡처하고 다시 써야 함을 나타냅니다.
     - `^`: 지정 된 패턴으로 시작 하는 텍스트만 캡처되는 것을 나타냅니다.
     - `$`: 지정 된 패턴으로 끝나는 텍스트만 캡처 됨을 나타냅니다.
