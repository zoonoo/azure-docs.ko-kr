...
4. 가장자리 노드에서 제거하려는 자산을 선택합니다. 모든 자산을 지우려면 **모두 제거** 확인란을 클릭합니다. 그렇지 않은 경우 제거하려는 각 자산의 전체 경로(예: `/pictures/kitten.png`)를 **경로** 텍스트 상자에 입력합니다.
	...
	>
	> 경로는 다음 [정규식](https://msdn.microsoft.com/library/az24scfc.aspx)에 맞는 상대 URL이어야 합니다. `^\/(?:[a-zA-Z0-9-_.\u0020]+\/)**$";`. 별표(*)를 와일드카드(예: `/music/*`)로 사용할 수 있습니다.

5. **제거** 단추를 클릭합니다.

	![제거 단추](./media/cdn-purge-endpoint/cdn-purge-button.png)

> [AZURE.NOTE] 요청을 삭제하는 데 대략 2-3분이 걸립니다. 삭제 요청은 CDN 프로필별로 분당 10개로 제한됩니다.
