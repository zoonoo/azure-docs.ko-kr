...
4. 가장자리 노드에서 제거하려는 자산을 선택합니다.  모든 자산을 지우려면 **모두 제거** 확인란을 클릭합니다.  그렇지 않은 경우 제거하려는 각 자산의 경로를 **경로** 텍스트 상자에 입력합니다. 경로에 지원되는 형식은 아래와 같습니다.
    1. **단일 URL 제거**: 예를 들어, 파일 확장명 없이 전체 URL을 지정 하 여 개별 자산 제거`/pictures/strasbourg.png`; `/pictures/strasbourg`
    2. **와일드카드 제거**: 별표(\*)를 와일드 카드로 사용할 수 있습니다. 경로에 `/*`가 포함된 엔드포인트의 모든 폴더, 하위 폴더 및 파일을 제거하거나 폴더를 지정하고 맨 뒤에 `/*`를 붙여(예: `/pictures/*`) 특정 폴더의 모든 하위 폴더 및 파일을 제거합니다.  와일드 카드 제거는 현재 Akamai의 Azure CDN에서 지원되지 않습니다. 
...
> [!IMPORTANT]
> 제거 요청으로 처리 하는 데 약 2 분이 소요 **Microsoft의 Azure CDN** 하 고 **verizon에서의 Azure CDN** (표준 및 프리미엄) 및 사용 하 여 약 10 초 **Azure CDN akamai에서**합니다.  Azure CDN은 프로필 수준에서 동시 제거 요청이 항상 50개로 제한됩니다. 
> 
> 
