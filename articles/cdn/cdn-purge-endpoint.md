...
3. 제거 블레이드에서 URL 드롭다운 목록에서 제거하려는 서비스 주소를 선택합니다.
   ...
   > 
   > 
4. 가장자리 노드에서 제거하려는 자산을 선택합니다.  모든 에셋을 지우려면 **모든 제거** 확인란을 클릭합니다.  그렇지 않은 경우 제거하려는 각 자산의 경로를 **경로** 텍스트 상자에 입력합니다. 경로에 지원되는 형식은 아래와 같습니다.
    1. **단일 URL 제거**: 파일 확장자의 유무에 관계없이 전체 URL을 지정하여 개별`/pictures/strasbourg.png`자산을 제거합니다.`/pictures/strasbourg`
...
> [!IMPORTANT]
> 퍼지 요청은 **Microsoft의 Azure CDN으로**처리하는 데 약 10분, **버라이존(표준 및 프리미엄)의 Azure CDN으로** 약 2분, **Akamai의 Azure CDN에서**약 10초 가 소요됩니다.  Azure CDN은 프로필 수준에서 동시 제거 요청이 항상 50개로 제한됩니다. 
> 
> 
