...
>
> CDN 가격 책정은 CDN 프로필 수준에서 적용됩니다. 따라서 다양한 Azure CDN 가격 책정 계층을 사용하려면 여러 CDN 프로필을 만들어야 합니다.
> 
> 
...
2. **엔드포인트**를 선택합니다.
    ...
    **끝점 추가** 창이 나타납니다.
    ...
    ![끝점 추가 창][cdn-add-endpoint]

3. **이름**에 새 CDN 엔드포인트의 고유한 이름을 입력합니다. 이 이름은 `<endpointname>.azureedge.net` 도메인의 캐시된 리소스에 액세스하기 위해 사용됩니다.

4. **원본 형식**에서 원본 형식을 선택합니다. Azure Storage 계정에 대해 **저장소**, Azure 클라우드 서비스에 대해 **클라우드 서비스**, Azure 웹앱에 대해 **웹앱**을 선택하고 기타 공개적으로 액세스할 수 있는 웹 서버 원본(Azure 또는 다른 곳에서 호스팅)에 대해 **사용자 지정 원본**을 선택합니다.
   
    ![CDN 원본 형식](./media/cdn-create-new-endpoint/cdn-origin-type.png)

5. **원본 호스트 이름**에서 원본 도메인을 선택하거나 입력합니다. 4단계에서 지정한 사용할 수 있는 모든 원본 형식이 드롭다운에 나열됩니다. 원본 형식으로 **사용자 지정 원본**을 선택한 경우 사용자 지정 원본의 도메인을 입력합니다.
    
6. **원본 경로**에 캐시하려는 리소스의 경로를 입력하거나, 5단계에서 지정한 도메인의 모든 리소스를 캐시하도록 허용하려면 비워둡니다.
...
8. **프로토콜** 및 **원본 포트**에 원본의 리소스에 액세스하는 데 사용되는 프로토콜과 포트를 지정합니다. 프로토콜을 적어도 하나는(HTTP 또는 HTTPS) 선택해야 합니다. CDN 제공 도메인(`<endpointname>.azureedge.net`)을 사용하여 HTTPS 콘텐츠에 액세스합니다. 
   ...
   > [!NOTE]
   > **원본 포트** 값은 엔드포인트가 원본에서 정보를 검색하는 데 사용하는 포트에만 영향을 줍니다. 엔드포인트 자체는 **원본 포트** 값에 관계없이 기본 HTTP 및 HTTPS 포트(80 및 443)의 최종 클라이언트에만 사용할 수 있습니다.  
   > 
   ...
   > 
   > HTTPS를 사용하여 CDN 콘텐츠에 액세스할 경우 다음과 같은 제약 조건이 있습니다.
   > 
...
9. 새 엔드포인트를 만들려면 **추가**를 선택합니다.
   ...
   > [!IMPORTANT]
   > 등록이 전파되는 등록에 시간이 걸리기 때문에, 엔드포인트를 즉시 사용할 수는 없습니다. **Akamai의 Azure CDN** 프로필의 경우, 일반적으로 1분 이내에 전파가 완료됩니다. **Verizon의 Azure CDN** 프로필의 경우 일반적으로 90분 이내에 전파가 완료되지만 더 오래 소요될 수도 있습니다.
    > 
    > 엔드포인트 구성이 POP에 전파되기 전에 CDN 도메인 이름을 사용하려는 경우 HTTP 404 응답 상태를 받을 수 있습니다. 엔드포인트를 만든 후 몇 시간 후에도 404 응답 상태가 수신되는 경우 [404 상태를 반환하는 CDN 엔드포인트 문제 해결](cdn-troubleshoot-endpoint.md)을 참조하세요.
    > 
    > 
