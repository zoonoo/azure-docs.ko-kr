...
CDN은 [Azure 포털](https://portal.azure.com)에서 활성화할 수 있습니다. 웹앱, Blob 저장소, 클라우드 서비스를 비롯한 몇 가지 유형의 통합 Azure 원본이 지원됩니다. 또한 Azure 미디어 서비스 스트리밍 끝점에 대 해 CDN을 사용할 수 있습니다. 원본이 이러한 Azure 서비스 중 하나가 아니거나 Azure 외부에서 호스팅되는 경우에는 사용자 지정 원본을 만들 수 있습니다. 원본에 대해 CDN 끝점을 사용하도록 설정하면 공개적으로 사용 가능한 모든 개체가 CDN 에지 캐싱에 적합합니다.

>[AZURE.NOTE] CDN 작동 방법 및 기능 목록에 대한 소개는 [CDN 개요](./cdn-overview.md)를 참조하세요.
...
8. **프로토콜** 및 **원본 포트**에 원본의 리소스에 액세스하는 데 사용되는 프로토콜과 포트를 지정합니다. 클라이언트가 CDN에서 리소스에 액세스하는 경우 동일한 프로토콜 및 포트를 계속 사용하게 됩니다. 프로토콜을 적어도 하나는(HTTP 또는 HTTPS) 선택해야 합니다.
	...
	> - CDN 제공 도메인(`<identifier>.azureedge.net`)을 사용하여 HTTPS 콘텐츠에 액세스해야 합니다. 현재 CDN이 사용자 지정 인증서를 지원하지 않으므로 CNAME(사용자 지정 도메인 이름)에는 HTTPS 지원을 사용할 수 없습니다.
