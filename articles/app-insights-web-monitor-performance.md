...
응용 프로그램이 정상적으로 작동하는지 확인하고 오류가 발생하는지 신속하게 파악합니다. [Application Insights][Application Insights]는 성능 문제 및 예외에 대한 정보와, 이러한 현상에 대한 근본 원인을 확인하고 진단하는 기능을 제공합니다.

Application Insights는 온-프레미스 또는 가상 컴퓨터와 Microsoft Azure 웹 사이트에서 호스트되는 ASP.NET 웹 응용 프로그램을 모니터링할 수 있습니다.

-   [성능 모니터링 설정][성능 모니터링 설정]
-   [데이터 확인][데이터 확인]
-   [성능 메트릭의 의미][성능 메트릭의 의미]
-   [문제 진단][문제 진단]
-   [다음 단계][다음 단계]
...
-   [Visual Studio의 앱 프로젝트에 Application Insights 추가][Application Insights] - 권장 방법입니다. 이 방식을 사용하면 수동 성능 모니터링을 수행할 수 있을 뿐 아니라 진단 로깅을 삽입하고 사용 현황을 추적할 수도 있습니다.
-   [라이브 웹 사이트의 성능 모니터링][라이브 웹 사이트의 성능 모니터링] - 이 방식을 사용하는 경우 응용 프로그램 프로젝트를 업데이트하거나 웹 사이트를 다시 배포하지 않아도 됩니다.
...
![클릭하여 추가 데이터를 표시한 다음 시간 범위 선택][클릭하여 추가 데이터를 표시한 다음 시간 범위 선택]

그래프를 클릭하여 표시할 메트릭을 선택합니다.

![그래프를 클릭하여 메트릭 선택][그래프를 클릭하여 메트릭 선택]
...
![모든 메트릭의 선택을 취소하여 전체 집합 표시][모든 메트릭의 선택을 취소하여 전체 집합 표시]
...
-   웹 사이트의 작동이 중단되거나 응답이 잘못되거나 속도가 느려지는 경우 경고를 받도록 [웹 테스트][웹 테스트]를 설정합니다.
-   요청 수를 다른 메트릭과 비교하여 오류 또는 느린 응답이 부하와 관련되어 있는지 확인합니다.
-   코드에서 [검사 추적 문을 삽입 및 검색][검사 추적 문을 삽입 및 검색]하여 문제를 파악합니다.

## <a name="next"></a>다음 단계

[웹 테스트][웹 테스트] - 전 세계에서 웹 요청이 일정한 간격으로 응용 프로그램에 전송되도록 합니다.

[진단 추적 캡처 및 검색][검사 추적 문을 삽입 및 검색] - 추적 호출을 삽입하고 결과를 확인하여 문제를 파악합니다.

[사용 현황 추적][사용 현황 추적] - 사용자의 응용 프로그램 사용 방식을 파악합니다.

[문제 해결][문제 해결] - 문제 해결 정보 및 질문과 대답을 확인합니다.

## 자세한 정보

-   [Application Insights - 시작][Application Insights]
-   [라이브 웹 서버 모니터링][라이브 웹 사이트의 성능 모니터링]
-   [웹 응용 프로그램의 성능 모니터링][웹 응용 프로그램의 성능 모니터링]
-   [진단 로그 검색][검사 추적 문을 삽입 및 검색]
-   [웹 테스트를 사용한 가용성 추적][웹 테스트]
-   [사용 현황 추적][사용 현황 추적]
...
<!--Link references-->
  ...
  [문제 진단]: #diagnosis
  [다음 단계]: #next
  [라이브 웹 사이트의 성능 모니터링]: ../app-insights-monitor-performance-live-website-now/
  ...
  [검사 추적 문을 삽입 및 검색]: ../app-insights-search-diagnostic-logs/
