...
웹 애플리케이션 방화벽을 통해 SQL 삽입, 사이트 간 스크립팅, 맬웨어 업로드 및 애플리케이션 DDoS와 기타 공격을 차단하기 위해 인바운드 웹 트래픽을 검사하여 웹 애플리케이션을 보호할 수 있습니다. 데이터 손실 방지 DLP (Data Loss Prevention)에 대한 백엔드 웹 서버로부터의 응답도 검사합니다. Azure marketplace에서 WAF 장치를 얻거나 [Azure 애플리케이션 게이트웨이][appgw]를 사용할 수 있습니다.
...
ILB App Service Environment을 만드는 방법에 대 한 자세한 내용은 [ilb App Service Environment 만들기 및 사용][ilbase]을 참조 하세요.
...
웹 [앱에 대 한 사용자 지정 도메인 이름 설정][custom-domain]문서에서 웹 앱에 대 한 사용자 지정 도메인 이름을 설정 하는 방법에 대 한 정보가 있습니다. 그렇지만 ILB App Service Environment의 앱에서는 도메인 이름에 대한 유효성 검사가 없습니다. 앱 엔드포인트를 관리하는 DNS를 소유하고 있으므로 원하는 항목을 여기에 배치할 수 있기 때문입니다. 이 경우에 추가하는 사용자 지정 도메인 이름은 DNS에 포함될 필요는 없지만 앱으로 구성해야 합니다. 
