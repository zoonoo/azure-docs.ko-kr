...
예제 Azure Resource Manager 템플릿 및 관련 매개 변수 파일 [은 GitHub에서 사용할 수 있습니다.][quickstartilbasecreate]
...
"기본" SSL 인증서가 앱에 대한 SSL 연결을 설정하는 데 사용하므로 ILB ASE를 만든 후에는 SSL 인증서를 ASE에 연결해야 합니다.  가상의 Contoso Corporation 예제를 계속 살펴보면, ASE의 기본 DNS 접미사가 *internal-contoso.com*인 경우 *[https://some-random-app.internal-contoso.com](https://some-random-app.internal-contoso.com )* 에 연결하기 위해 * *.internal-contoso.com*에 유효한 SSL 인증서가 필요합니다. 
...
아래의 PowerShell 코드 조각은 자체 서명된 인증서를 생성하고, 인증서를 .pfx 파일로 내보내고, .pfx 파일을 Base64 인코딩 문자열로 변환한 다음 Base64 인코딩 문자열을 개별 파일로 저장하는 예를 보여 줍니다.  B a s e 64 인코딩에 대 한 Powershell 코드는 [Powershell 스크립트 블로그][examplebase64encoding]에서 적용 되었습니다.
...
SSL 인증서가 성공적으로 생성 되 고 base64 인코딩 문자열로 변환 되 면 [기본 ssl 인증서를 구성][configuringDefaultSSLCertificate] 하기 위한 GitHub의 예제 Azure Resource Manager 템플릿을 사용할 수 있습니다.
...
템플릿이 완료되면 ILB ASE의 앱에 HTTPS를 통해 액세스할 수 있으며 기본 SSL 인증서를 사용하여 연결이 보호됩니다.  기본 SSL 인증서는 ILB ASE의 앱 주소는 애플리케이션 이름과 기본 호스트 이름 조합을 사용하여 지정됩니다.  예를 들어 *[https://mycustomapp.internal-contoso.com](https://mycustomapp.internal-contoso.com )* 은 * *.internal-contoso.com*에 기본 SSL 인증서를 사용합니다.
