...
3. 업로드된 SSL 인증서는 해당 "기본" SSL 인증서로 ILB ASE에 명시적으로 할당됩니다.  이 SSL 인증서는 ASE에 할당된 공통 루트 도메인을 사용하여 앱을 해결할 때 ILB ASE의 앱에 대한 `https://someapp.mycustomrootcomain.com`SSL 트래픽에 사용됩니다(예: ).
...
템플릿이 완료되면 ILB ASE의 앱에 HTTPS를 통해 액세스할 수 있으며 기본 SSL 인증서를 사용하여 연결이 보호됩니다.  기본 SSL 인증서는 ILB ASE의 앱 주소는 애플리케이션 이름과 기본 호스트 이름 조합을 사용하여 지정됩니다.  예를 *https://mycustomapp.internal-contoso.com* 들어 **.internal-contoso.com*에 대한 기본 SSL 인증서를 사용합니다.
