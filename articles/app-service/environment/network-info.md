...
작동 하려면 ASE에 대 한 ASE에는 다음 포트를를 열어 필요 합니다.
...
7654 및 1221 포트 검색에서 열린 상태로 표시할 수 있는 다른 포트 2 가지 있습니다. 이러한 IP 주소 등 nothing을 사용 하 여 회신합니다. 원하는 경우 차단할 수 있습니다. 

인바운드 관리 트래픽은 시스템 모니터링 외에도 ASE의 명령 및 제어를 제공합니다. 이 트래픽의 원본 주소 목록은 [ASE 관리 주소][ASEManagement] 문서에 있습니다. 네트워크 보안 구성이 포트 454 및 455에서 ASE 관리 주소에서 액세스를 허용 해야 합니다. 해당 주소의 액세스를 차단하면 ASE가 비정상화된 다음, 일시 중단됩니다. 포트 454 및 455에서 들어오는 TCP 트래픽은 동일한 VIP에서 되돌아 가야 합니다. 그렇지 않으면 비대칭 라우팅 문제가 발생합니다. 

ASE 서브넷 내에서 여러 포트를 내부 구성 요소 통신에 사용 되며 변경할 수 있습니다. 이를 위해서는 ASE 서브넷에 있는 모든 포트를 ASE 서브넷에서 액세스할 수 있어야 합니다. 

Azure Load Balancer 및 ASE 서브넷 간의 통신을 위해서는 최소 포트 454, 455 및 16001이 열려 있어야 합니다. 16001 포트는 부하 분산 장치 및 ASE 사이에 활성 트래픽을 유지하는 데 사용됩니다. ILB ASE를 사용 하는 경우 454, 455, 16001 트래픽을 잠글 수 있습니다 포트입니다.  외부 ASE를 사용 하는 다음 경우 정상적인 앱 액세스 포트를 고려해 야 합니다.  

사용 하 여 직접 신경 써야 하는 다른 포트는 응용 프로그램 포트:
...
응용 프로그램 포트를 차단 하면 ASE 여전히 작동할 수 있지만 앱 되지 않을 수 있습니다.  ASE 서브넷에 ASE 포털에 표시 된 포트에서 앱에 할당 된 Ip에서의 트래픽을 허용 해야 앱 할당 된 IP 주소 외부 ase를 사용 하는 경우 > IP 주소 페이지입니다.
...
| DNS | 53 |
| NTP | 123 |
| Azure 서비스, Linux 종속성, Windows 업데이트, 8CRL | 80/443 |
| Azure SQL | 1433 | 
| 모니터링 | 12000 |

아웃 바운드 종속성을 설명 하는 문서에 나와 [App Service Environment의 아웃 바운드 트래픽을 잠그는](./firewall-integration.md)합니다. ASE가 해당 종속성에 액세스하지 못하는 경우 작동이 중지됩니다. 이러한 작동 중지 상태가 오랜 기간 지속되면 ASE가 일시 중단됩니다. 
