...
두 명령은 모두와 같은 URL을 `https://<deployment-username>@<app-name>.scm.azurewebsites.net/<app-name>.git`반환 합니다. 다음 단계에서이 URL을 사용 하 여 앱을 배포 합니다.
...
1. 배포가 완료 되 면에서 빌드 진행률 `https://<azure_devops_account>.visualstudio.com/<project_name>/_build`을 확인 하 고에서 배포 진행률을 확인 `https://<azure_devops_account>.visualstudio.com/<project_name>/_release`합니다.
...
|`No refs in common and none specified; doing nothing. Perhaps you should specify a branch such as 'master'.`|중 `git push`에 분기를 지정 하지 않았거나에서 `push.default` `.gitconfig`값을 설정 하지 않았습니다.|마스터 `git push` 분기를 지정 하 여를 다시 `git push azure master`실행 합니다.|
|`src refspec [branchname] does not match any.`|' Azure ' 원격의 마스터가 아닌 다른 분기에 푸시 하려고 했습니다.|마스터 `git push` 분기를 지정 하 여를 다시 `git push azure master`실행 합니다.|
