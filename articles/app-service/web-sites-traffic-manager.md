...
Azure Traffic Manager를 사용하면 웹 클라이언트의 요청을 Azure App Service의 앱에 분산하는 방법을 제어할 수 있습니다. App Service 끝점을 Azure Traffic Manager 프로필에 추가하는 경우 Azure Traffic Manager가 App Service의 상태(실행 중, 중지됨 또는 삭제됨)를 계속 추적하므로 트래픽을 수신할 끝점을 결정할 수 있습니다.
...
앱 트래픽을 제어하도록 구성하려면 앞서 설명한 세 가지 부하 분산 방법 중 하나를 사용하는 프로필을 Azure Traffic Manager에서 만든 후 트래픽을 제어하려는 끝점(이 경우에는 App Service)을 프로필에 추가합니다. 앱 상태(실행 중, 중지됨 또는 삭제됨)가 정기적으로 프로필에 전달되므로 Azure Traffic Manager가 상태에 따라 트래픽을 보낼 수 있습니다.
...
* 동일한 지역에서 다른 Azure 클라우드 서비스와 함께 App Service를 사용하는 배포의 경우 끝점의 두 유형을 결합하여 하이브리드 시나리오를 사용할 수 있습니다.
* 프로필에서 지역당 하나의 App Service 끝점만 지정할 수 있습니다. 한 지역의 끝점으로 하나의 앱을 선택한 경우 해당 지역의 나머지 앱은 해당 프로필에 대해 선택할 수 없게 됩니다.
* Azure Traffic Manager 프로필에서 지정한 App Service 끝점은 프로필에서 앱에 대한 구성 페이지의 **도메인 이름** 섹션에 표시되지만 이 섹션에서 구성할 수 없습니다.
...
* 사용자 지정 도메인 이름이 예상대로 작동하지만 앱에 해당 이름을 추가하는 작업 외에도 DNS 맵이 Traffic Manager URL을 가리키도록 구성해야 합니다. App Service 앱에 대해 사용자 지정 도메인을 설정하는 방법에 대한 정보는 [Azure Web Apps에 기존 사용자 지정 DNS 이름 매핑](app-service-web-tutorial-custom-domain.md)을 참조하세요.
