...
코드에서 SSL 인증서를 사용하는 이 방법을 사용하면 App Service에서 SSL 기능을 사용하게 됩니다. 이 경우에 앱은 **기본** 계층 이상이어야 합니다. 대신 애플리케이션 디렉터리에 인증서 파일을 포함하고 직접 로드할 수 있습니다([대안: 인증서 파일 로드](#file) 참조). 그러나 이 대안을 사용하면 애플리케이션 코드 또는 개발자로부터 인증서의 프라이빗 키를 숨길 수 없습니다. 또한 애플리케이션 코드가 오픈 소스 리포지토리에 있는 경우 리포지토리에서 프라이빗 키로 인증서를 사용할 수 없습니다.
