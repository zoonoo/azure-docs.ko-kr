...
1. **앱 이름** 및 새 앱에 대 한 **응용 프로그램 설명을** 입력 합니다. 응용 프로그램의 **url** 을 **웹 사이트 url** 필드에 붙여넣습니다. **콜백 url** 섹션에서 App Service 앱의 HTTPS url을 입력 하 고 경로를 추가 `/.auth/login/twitter/callback` 합니다. 예: `https://contoso.azurewebsites.net/.auth/login/twitter/callback`
