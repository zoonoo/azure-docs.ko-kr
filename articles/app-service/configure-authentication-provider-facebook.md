...
1. **대시보드** > **Facebook 로그인** > **Set up**설정 > **웹**을 선택 합니다.
1. **Facebook 로그인**아래의 왼쪽 탐색 영역에서 **설정**을 선택 합니다.
1. **유효한 OAuth 리디렉션 uri** 필드에를 입력 `https://<app-name>.azurewebsites.net/.auth/login/facebook/callback`합니다. 을 Azure App Service 앱 `<app-name>` 의 이름으로 바꾸어야 합니다.
1. **변경 내용 저장**을 선택합니다.
...
1. 응용 프로그램을 등록 하는 데 사용한 Facebook 계정은 앱의 관리자입니다. 이 시점에서 관리자만이 응용 프로그램에 로그인 할 수 있습니다.
   ...
   다른 Facebook 계정을 인증 하려면 **앱 검토** 를 선택 하 고, 일반 ** \<사용자** 가 Facebook 인증을 사용 하 여 앱에 액세스할 수 있도록 앱 이름> 공개로 설정을 사용 하도록 설정 합니다.
