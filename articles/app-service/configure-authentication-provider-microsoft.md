...
1. **리디렉션 uri**에서 **웹**을 선택한 다음 `https://<app-domain-name>/.auth/login/microsoftaccount/callback`를 입력 합니다. *\<앱 도메인 이름 >* 를 앱의 도메인 이름으로 바꿉니다.  `https://contoso.azurewebsites.net/.auth/login/microsoftaccount/callback`)을 입력합니다. URL에서 HTTPS 체계를 사용 해야 합니다.
