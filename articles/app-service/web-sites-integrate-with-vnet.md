...
인스턴스에 할당 된 개인 IP는 **WEBSITE_PRIVATE_IP** 환경 변수를 통해 노출 됩니다. Kudu 콘솔 UI는 웹 앱에서 사용할 수 있는 환경 변수 목록도 보여 줍니다. 이 IP는 통합 서브넷의 주소 범위에서 할당 됩니다. 지역 VNet 통합의 경우 WEBSITE_PRIVATE_IP의 값은 위임 된 서브넷의 주소 범위에 있는 IP이 고, 게이트웨이 필수 VNet 통합의 경우 값은 Virtual Network 게이트웨이에서 구성 된 지점 및 사이트 간 주소 풀의 주소 범위에서 IP입니다. 웹 앱에서 Virtual Network를 통해 리소스에 연결 하는 데 사용 되는 IP입니다. 

> [!NOTE]
> WEBSITE_PRIVATE_IP 값이 변경 되도록 바인딩되어 있습니다. 그러나 통합 서브넷 또는 지점 및 사이트 간 주소 범위의 주소 범위 내에 있는 IP 이므로 전체 주소 범위에 대 한 액세스를 허용 해야 합니다.
>
