...
이 기능은 미리 보기로 제공 하지만 다음과 같은 제한 사항이 있는 Windows 앱 프로덕션 워크 로드에 지원 됩니다.
...
* 앱이 한 번에 하나의 VNet에 연결할 수 있도록
* 최대 5 개의 Vnet App Service 계획에 통합할 수 있도록 합니다. 
...
* 지점과 사이트 간 VPN으로 구성 된 가상 네트워크 게이트웨이가 필요 합니다.
* Linux 앱을 사용 하 여 사용 하 여 지원 되지 않습니다.
* 게이트웨이의 SLA 때문 99.9 %SLA 지원합니다.
...
**라우팅** VNet에 정의된 경로는 트래픽을 앱에서 VNet으로 전달하는 데 사용됩니다. 아웃바운드 트래픽을 VNet에 추가로 보내야 하는 경우 해당 주소 블록을 여기에 추가할 수 있습니다. VNet 통합 해야 하는 게이트웨이 사용 하 여이 기능 에서만 작동 합니다.
