---
title: 앱 모니터링
description: Azure Portal을 사용하여 Azure App Service에서 앱을 모니터링하는 방법을 알아보세요. 보고된 할당량 및 메트릭을 이해합니다.
author: btardif
...
ms.locfileid: "79500427"
---
# <a name="monitor-apps-in-azure-app-service"></a>Azure App Service에서 앱 모니터링
[Azure App Service는](https://go.microsoft.com/fwlink/?LinkId=529714) [Azure 포털에서](https://portal.azure.com)웹 앱, 모바일 및 API 앱에 대한 기본 제공 모니터링 기능을 제공합니다.

Azure 포털에서 앱 및 앱 서비스 계획에 대한 *할당량* 및 *메트릭을* 검토하고 *경고* 및 *규칙* 기반 메트릭을 설정할 수 있습니다.
...
*기본,* *표준*및 *프리미엄에서* 호스팅되는 앱에 적용할 수 있는 유일한 할당량은 파일 시스템입니다.
...
| 메트릭 | 설명 |
| --- | --- |
| **응답 시간** | 앱이 요청을 제공하는 데 걸린 시간(초)입니다. |
| **평균 응답 시간(더 이상 사용되지 않습니다)** | 앱이 요청을 제공하는 데 걸린 평균 시간(초)입니다. |
...
| **CPU 시간** | 앱에서 사용한 CPU의 양(초)입니다. 이 메트릭에 대한 자세한 내용은 [CPU 시간 대 CPU 백분율을](#cpu-time-vs-cpu-percentage)참조하십시오. |
| **현재 어셈블리 수** | 이 애플리케이션의 모든 AppDomains에 로드된 어셈블리의 현재 개수입니다. |
| **데이터 에서** | 앱에서 사용한 들어오는 대역폭 양(MiB)입니다. |
| **데이터 출력** | 앱에서 사용한 나가는 대역폭 양(MiB)입니다. |
| **파일 시스템 사용** | 앱에서 사용하는 파일 시스템 할당량의 백분율입니다. |
...
| **초당 IO 기타 바이트 수** | 앱 프로세스가 제어 작업과 같은 데이터를 포함하지 않는 I/O 작업에 바이트를 발행하는 속도입니다.|
| **초당 IO 기타 작업 수** | 읽기 또는 쓰기 작업이 아닌 앱 프로세스가 I/O 작업을 발행하는 속도입니다.|
...
| **프라이빗 바이트** | 개인 바이트는 앱 프로세스가 다른 프로세스와 공유할 수 없는 할당된 메모리의 현재 크기(바이트)입니다.|
...
| **데이터 에서** | 계획의 모든 인스턴스 간에 사용된 평균 들어오는 대역폭입니다. |
| **데이터 출력** | 계획의 모든 인스턴스 간에 사용된 평균 나가는 대역폭입니다. |
| **디스크 큐 길이** | 스토리지에 대기 중인 평균 읽기 및 쓰기 요청 수입니다. 높은 디스크 큐 길이는 과도한 디스크 I/O로 인해 속도가 느려질 수 있는 앱을 나타냅니다. |
...
**CPU 시간**: 할당량 중 하나가 앱에서 사용하는 CPU 분에 정의되어 있기 때문에 무료 또는 공유 계획에서 호스팅되는 앱에 유용합니다.

**CPU 백분율**: 기본, 표준 및 프리미엄 요금제에서 호스팅되는 앱에 유용합니다. CPU 백분율은 모든 인스턴스에서 전체 사용량을 나타내는 좋은 지표입니다.
...
* **분별체** 메트릭은 30시간 동안 유지됩니다.
* **시간** 분별체 메트릭은 30일 동안 유지됩니다.
* **일** 세분성 메트릭은 30일 동안 유지됩니다.
...
![Azure 포털의][metrics] 메트릭 차트 리소스 **개요** 페이지에서 직접 메트릭에 액세스할 수 있습니다. 여기에 일부 앱 메트릭을 나타내는 차트가 표시됩니다.

이러한 차트를 클릭하면 사용자 지정 차트를 만들고 다른 메트릭을 쿼리할 수 있는 메트릭 보기로 이동합니다. 
...
앱 또는 App Service 계획의 메트릭은 경고에 연결할 수 있습니다. 자세한 내용은 [알림 수신 을](../monitoring-and-diagnostics/insights-alerts-portal.md)참조하십시오.

기본 또는 더 높은 앱 서비스 요금제에서 호스팅되는 앱 서비스 앱은 자동 크기 조정을 지원합니다. 자동 크기 조정을 사용하여 App Service 계획 메트릭을 모니터링하는 규칙을 구성할 수 있습니다. 규칙은 필요에 따라 추가 리소스를 제공할 수 있도록 인스턴스 수를 눌리거나 줄일 수 있습니다. 규칙을 사용하면 앱이 과도하게 프로비전될 때 비용을 절감할 수도 있습니다.
