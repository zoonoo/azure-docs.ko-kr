...
### <a name="quotas"></a>할당량
App Service에서 호스팅되는 애플리케이션에는 사용 가능한 리소스에 대한 특정 *한도* 가 적용됩니다. 이러한 한도는 해당 앱과 연결된 **App Service 계획**으로 정의됩니다.

[!INCLUDE [app-service-dev-test-note](../../includes/app-service-dev-test-note.md)]

애플리케이션이 **무료** 또는 **공유** 계획에 호스팅되는 경우 앱에서 사용할 수 있는 리소스에 대한 한도는 **할당량**으로 정의됩니다.

애플리케이션이 **기본**, **표준** 또는 **프리미엄** 계획에 호스팅되는 경우 사용할 수 있는 리소스에 대한 한도는 **크기**(작음, 중간, 큼) 및 **App Service 계획**의 **인스턴스 수**(1, 2, 3, ...)로 설정됩니다.

**무료** 또는 **공유** 앱에 대한 **할당량**은 다음과 같습니다.

* **CPU(Short)**
  * 5분 간격으로 이 애플리케이션에 대해 허용되는 CPU의 양입니다. 이 할당량은 5분마다 재설정됩니다.
* **CPU(Day)**
  * 하루에 이 애플리케이션에 대해 허용되는 총 CPU 양입니다. 이 할당량은 자정 UTC에 24시간마다 재설정됩니다.
* **메모리**
  * 이 애플리케이션에 대해 허용되는 총 메모리 양입니다.
* **대역폭**
  * 하루에 이 애플리케이션에 대해 허용되는 나가는 총 대역폭 양입니다.
...
* **CPU 시간**
  * 앱에서 사용한 CPU의 양(초)입니다. 이 메트릭에 대한 자세한 내용은 다음을 참조하세요. [CPU 시간 및 CPU 비율](#cpu-time-vs-cpu-percentage)
* **데이터 입력**
...
* **디스크 큐 길이**
  * 저장소에 대기 중인 평균 읽기 및 쓰기 요청 수입니다. 디스크 큐 길이 값이 높으면 과도한 디스크 I/O로 인해 애플리케이션의 속도가 느려질 수 있음을 나타냅니다.
* **Http 큐 길이**
...
애플리케이션 및 App Service 계획에 대한 메트릭은 다음 세분성 및 보존 정책에 따라 서비스에 의해 기록 및 집계됩니다.
...
리소스 페이지에서 ![][metrics]
**메트릭**에 직접 액세스할 수 있습니다. 다음과 같이 차트를 사용자 지정할 수도 있습니다. (1) 차트를 **클릭**하고 (2) **차트 편집**을 선택합니다.
여기에서 표시할 (3) **시간 범위**, (4) **차트 종류**, (5) **메트릭**을 변경할 수 있습니다.  

다음에서 메트릭을 자세히 알아볼 수 있습니다. [서비스 메트릭 모니터링](../monitoring-and-diagnostics/insights-how-to-customize-monitoring.md).
