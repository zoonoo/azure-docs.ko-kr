---
title: 앱 모니터링
description: Azure Portal을 사용하여 Azure App Service에서 앱을 모니터링하는 방법을 알아보세요. 보고된 할당량과 메트릭을 이해합니다.
author: btardif
...
[Azure App Service](./overview.md)는 [Azure Portal](https://portal.azure.com)의 웹앱, 모바일 앱 및 API 앱에 대해 기본 제공 모니터링 기능을 제공합니다.

Azure Portal에서 앱 및 App Service 플랜에 대한 *할당량* 및 *메트릭* 을 검토하고, *경고* 및 *자동 스케일링* 규칙 기반 메트릭을 설정할 수 있습니다.
...
*기본*, *표준* 및 *프리미엄* 플랜에 호스트되는 앱에 적용 가능한 유일한 할당량은 파일 시스템입니다.
...
> [!NOTE]
> **파일 시스템 사용량** 은 전체적으로 롤아웃되는 신규 메트릭으로서 앱이 App Service Environment에서 호스트되는 경우를 제외하면 데이터가 필요하지 않습니다.
> 

> [!IMPORTANT]
> 메트릭 집계와의 혼동을 피하기 위해 **평균 응답 시간** 은 사용되지 않습니다. **응답 시간** 으로 대체하여 사용합니다.
...
| **평균 응답 시간(사용되지 않음)** | 앱에서 요청을 처리하는 데 걸린 평균 소요 시간(초)입니다. |
...
**CPU 비율**: 확장이 가능하므로 기본, 표준 및 프리미엄 플랜에서 호스트된 앱에 유용합니다. CPU 비율은 모든 인스턴스에 걸친 전체 CPU 사용량을 나타냅니다.

## <a name="metrics-granularity-and-retention-policy"></a>메트릭 세분성 및 보존 정책
앱 및 앱 서비스 플랜에 대한 메트릭은 서비스에 따라 로그 및 집계되고 [다음 규칙에 따라 보존됩니다](../azure-monitor/essentials/data-platform-metrics.md#retention-of-metrics).
...
![Azure Portal에서 보는 메트릭 차트][metrics] 리소스 **개요** 페이지에서 직접 메트릭에 액세스할 수 있습니다. 여기에서는 일부 앱 메트릭을 나타내는 차트를 보여 줍니다.
...
앱 또는 App Service 계획의 메트릭은 경고에 연결할 수 있습니다. 자세한 내용은 [경고 알림 받기](../azure-monitor/alerts/alerts-classic-portal.md)를 참조하세요.
