---
title: 템플릿을 사용하여 애플리케이션 배포
description: App Service 앱을 프로비전 및 배포하기 위한 Azure Resource Manager 템플릿 생성에 대한 지침을 확인하세요.
author: tfitzmac
ms.topic: article
ms.date: 01/03/2019
ms.author: tomfitz
...
* 웹앱을 대상으로 하는 Azure Application Insights 인스턴스는 웹앱에 따라 달라집니다.
...
3. appManagerStatus.xml 및 appManagerLog.xml 파일을 찾습니다. 첫 번째 파일은 상태를 기록합니다. 두 번째 파일은 오류에 대한 정보를 기록합니다. 오류가 명확하지 않은 경우 [포럼](/answers/topics/azure-webapps.html)에서 도움말을 요청할 때 해당 내용을 포함할 수 있습니다.
...
템플릿에 TLS/SSL 바인딩에 대한 [Microsoft.Web/certificates](/azure/templates/microsoft.web/certificates) 리소스가 포함되어 있고 인증서가 Key Vault에 저장되어 있는 경우 App Service ID가 인증서에 액세스할 수 있는지 확인해야 합니다.
...
예제 템플릿을 보려면 [Deploy a Web App certificate from Key Vault secret and use it for creating SSL binding](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.web/web-app-certificate-from-key-vault)(Key Vault 비밀에서 웹앱 인증서 배포 및 SSL 바인딩 생성에 사용)을 참조하세요.
