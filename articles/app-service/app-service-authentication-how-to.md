...
사용자는 앱의 `GET` 엔드포인트에 `/.auth/logout` 요청을 전송하여 로그아웃을 시작할 수 있습니다. `GET` 요청은 다음을 수행합니다.
...
[의 값을 ](https://wikipedia.org/wiki/Percent-encoding)인코딩`post_logout_redirect_uri`하는 것이 좋습니다.
