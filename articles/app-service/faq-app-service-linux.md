...
**내 웹앱은 Docker Hub에서 이미지를 업데이트한 후에도 여전히 이전 Docker 컨테이너 이미지를 사용합니다. 사용자 지정 컨테이너의 연속 통합 및 배포를 지원하나요?**
...
**Linux 웹앱을 사용하는 경우 내 애플리케이션의 Git 배포가 실패합니다. 이 문제를 해결하려면 어떻게 해야 하나요?**
...
**내 사용자 지정 컨테이너를 사용하고 있습니다. 플랫폼에서 SMB 공유를 `/home/` 디렉터리에 탑재하려고 합니다.**

`WEBSITES_ENABLE_APP_SERVICE_STORAGE` 설정이 **지정되지 않았** 거나 *false* 로 설정된 경우 `/home/` 디렉터리는 확장 인스턴스 간에 **공유되지 않으며** 작성된 파일은 다시 시작해도 **지속되지 않습니다**. 명시적으로 `WEBSITES_ENABLE_APP_SERVICE_STORAGE`를 *true* 로 설정하면 마운트가 활성화됩니다.
...
프라이빗 레지스트리 URL(예: myacr.azurecr.io/dotnet:latest)을 포함하여 전체 이미지 이름을 추가합니다. 사용자 지정 포트를 사용하는 이미지 이름은 [포털을 통해 입력할 수 없습니다](https://feedback.azure.com/forums/169385-web-apps/suggestions/31304650). `docker-custom-image-name`을 설정하려면 [`az` 명령줄 도구](/cli/azure/webapp/config/container#az-webapp-config-container-set)를 사용합니다.
...
**내 사용자 지정 컨테이너는 포트 80 이외의 포트를 수신 대기합니다. 요청을 해당 포트로 라우팅하도록 앱을 구성하려면 어떻게 해야 하나요?**
...
## <a name="multi-container-with-docker-compose"></a>Docker Compose를 사용하는 다중 컨테이너
...
**"요청된 기능을 리소스 그룹에서 사용할 수 없습니다."는 무엇을 의미하나요?**

ARM(Azure Resource Manager)을 사용하여 웹앱을 만들 때 이 메시지가 표시될 수 있습니다. 현재 제한 사항에 따라 동일한 리소스 그룹에 대해 동일한 지역에서 Windows 및 Linux 앱을 혼합할 수 없습니다.
