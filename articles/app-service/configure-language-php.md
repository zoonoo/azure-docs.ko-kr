---
title: Windows PHP 앱 구성
description: App Service의 네이티브 Windows 인스턴스에서 PHP 앱을 구성 하는 방법에 대해 알아봅니다. 이 문서에서는 가장 일반적인 구성 작업을 보여줍니다.
ms.devlang: php
ms.topic: article
ms.date: 06/02/2020
...
ms.locfileid: "84907988"
---
# <a name="configure-a-windows-php-app-for-azure-app-service"></a>Azure App Service에 대 한 Windows PHP 앱 구성

이 가이드에서는 Azure App Service에서 PHP 웹 앱, 모바일 백 엔드 및 API 앱을 구성 하는 방법을 보여 줍니다. 이 가이드는 Windows 플랫폼에서 호스팅되는 앱에 적용 됩니다. Linux 앱에 대 한 자세한 내용은 [Azure App Service에 대 한 LINUX PHP 앱 구성](containers/configure-language-php.md)을 참조 하세요.

이 가이드에서는 App Service에 앱을 배포 하는 PHP 개발자를 위한 주요 개념 및 지침을 제공 합니다. Azure App Service를 처음 사용하는 경우 먼저 [PHP 빠른 시작](app-service-web-get-started-php.md) 및 [MySQL을 사용하는 PHP 자습서](app-service-web-tutorial-php-mysql.md)를 따라하세요.
...
- 프로덕션 모드에서 로컬 상태로 앱을 테스트합니다. App Service는 프로덕션 모드에서 응용 프로그램을 실행 하므로 프로젝트가 프로덕션 모드에서 로컬에서 예상 대로 작동 하는지 확인 해야 합니다. 예를 들어:
...
> [자습서: MySQL을 사용하는 PHP 앱](app-service-web-tutorial-php-mysql.md)
