...
[Azure Traffic Manager](/azure/traffic-manager/) 을 사용 하 여 트래픽 부하를 [Azure App Service](overview.md)으로 분산 하는 경우 **\<Traffic Manager (trafficmanager >)** 를 사용 하 여 App Service 앱에 액세스할 수 있습니다. 사용자에 게 보다 알기 쉬운 도메인 이름을 제공 하기 위해 www\.contoso.com와 같은 사용자 지정 도메인 이름을 App Service 앱에 할당할 수 있습니다.
...
각 도메인 공급자의 세부 사항이 변경 되는 동안 사용자 지정 도메인 이름 (예: **contoso.com**) *에서* 앱과 통합 된**contoso.trafficmanager.net**(Traffic Manager 도메인 이름) *로* 매핑합니다.

> [!NOTE]
> 레코드를 이미 사용 중이고 앱을 우선적으로 바인딩해야 하는 경우 추가 CNAME 레코드를 만들 수 있습니다. 예를 들어 미리 **www\.contoso.com** 를 앱에 바인딩하려면 **awverify** 에서 **contoso.trafficmanager.net**로 CNAME 레코드를 만듭니다. 그런 다음 "www" CNAME 레코드를 변경할 필요 없이 "www\.contoso.com"를 앱에 추가할 수 있습니다. 자세한 내용은 [Azure App Service에 활성 DNS 이름 마이그레이션](manage-custom-dns-migrate-domain.md)을 참조 하세요.
...
2. 왼쪽 탐색 영역에서 **사용자 지정 도메인** 을 선택 하 > **호스트 이름 추가**를 선택 합니다.
