---
title: Linux에서 Azure App Service를 사용하여 Java APM 및 모니터링 도구 구성
...
1. NewRelic에서 Java 에이전트를 다운로드합니다. 파일 이름은 `newrelic-java-x.x.x.zip`과 유사합니다.
1. 라이선스 키를 복사합니다. 이 키는 나중에 에이전트를 구성하는 데 필요합니다.
...
1. 압축이 풀린 NewRelic Java 에이전트 파일을 `/home/site/wwwroot/apm` 아래 디렉터리에 업로드합니다. 에이전트 파일은 `/home/site/wwwroot/apm/newrelic`에 있어야 합니다.
1. `/home/site/wwwroot/apm/newrelic/newrelic.yml`에서 YAML 파일을 수정하고 자리 표시자 라이선스 값을 사용자 라이선스 키로 바꿉니다.
