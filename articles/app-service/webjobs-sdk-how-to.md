...
> * Azure Functions C# 클래스 라이브러리에 대 한 샘플 코드는 WebJobs sdk 프로젝트에 `FunctionName` 특성이 필요 하지 않은 경우를 제외 하 고는 WebJobs sdk 코드와 비슷합니다.
...
버전 3. *x* 는 표준 ASP.NET Core api를 사용 합니다. [인스턴스에서`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder) 메서드 [`UseEnvironment`](/dotnet/api/microsoft.extensions.hosting.hostinghostbuilderextensions.useenvironment) 를 호출 합니다. 이 예제와 같이 `development`라는 문자열을 전달 합니다.
...
`JobHostConfiguration` 클래스에는 개발 모드를 사용하도록 설정하는 `UseDevelopmentSettings` 메서드가 있습니다.  다음 예제에서는 개발 설정을 사용하는 방법을 보여줍니다. 로컬에서 실행 `true` 될 때 `Development`반환 하려면 값을 사용 하 여 라는 `AzureWebJobsEnv` 로컬 환경 변수를 설정 합니다. `config.IsDevelopment`
...
Flow를 사용 하 `HttpClient` 여 함수에서 수행 하는 모든 나가는 HTTP 요청입니다. `ServicePointManager` 에 `DefaultConnectionLimit`설정 된 값에 도달 하면에서 `ServicePointManager` 큐를 보내기 전에 요청을 시작 합니다. `DefaultConnectionLimit`가 2로 설정되었고 코드에서 HTTP 요청 1,000개를 만든다고 가정해 봅시다. 처음에는 OS까지 전달되는 요청이 2개밖에 없습니다. 나머지 998개는 공간이 생길 때까지 큐에서 대기합니다. 즉, 요청을 했지만 OS에서 대상 서버로 요청이 전송 되지 않았기 때문에 시간이 초과 될수있습니다.`HttpClient` 이와 같은 이유로 로컬 `HttpClient`가 요청을 완료하는 데 10초가 걸리지만 서비스가 200ms 후에 모든 요청을 반환하는 이상한 동작이 관찰될 수 있습니다. 
...
특성 `QueueTrigger` 은 큐 메시지가 `myqueue-items` 큐에 나타날 때마다 함수를 호출 하도록 런타임에 지시 합니다. 특성 `Blob` 은 런타임에 큐 메시지를 사용 하 여 *샘플* 작업 항목 컨테이너의 blob을 읽도록 지시 합니다. `myQueueItem` 매개 변수의 함수에 전달 되는 큐 메시지의 내용은 blob의 이름입니다.
...
버전 3 *x*, 저장소 바인딩이 `Microsoft.Azure.WebJobs.Extensions.Storage` 패키지에 포함 됩니다. 다음과 같이 `ConfigureWebJobs` 메서드에서 확장메서드를호출합니다.`AddAzureStorage`
...
다른 트리거 및 바인딩 형식을 사용하려면 해당 항목을 포함하는 NuGet 패키지를 설치하고 확장에서 구현된 `Add<binding>` 확장 메서드를 호출합니다. 예를 들어 Azure Cosmos DB 바인딩을 사용 하려는 경우를 설치 `Microsoft.Azure.WebJobs.Extensions.CosmosDB` 하 고 다음과 같이를 호출 `AddCosmosDB`합니다.
...
다음과 같이 `ConfigureWebJobs` 메서드에서 확장메서드를호출합니다.`AddExecutionContextBinding`
...
* **버전 3. *x*:** `Add<Binding>` 에서`ConfigureWebJobs`메서드를 호출할 때 구성을 설정 합니다.
* **버전 2. *x*:** 에 전달 `JobHost`하는 구성 개체의 속성을 설정 하 여 구성을 설정 합니다.
...
클래스는 다음과 같이에서 `appSettings`큐 이름을 가져옵니다. `NameResolver`
...
다음과 같이 `JobHost` 클래스를 개체에 전달 합니다. `NameResolver`
...
, `Queue` `IBinder` 또는 와`Table`같은 바인딩 특성을 사용 하기 전에 함수에서 일부 작업을 수행 해야 하는 경우 인터페이스를 사용할 수 있습니다. `Blob`
...
* [예](../azure-functions/functions-bindings-storage-queue.md#trigger---example). 코드 샘플. 클래스 C# 라이브러리 예제는 WebJobs SDK에 적용 됩니다. 특성을 `FunctionName` 생략 하면 됩니다.
...
다음 예제에서 앱 설정 `Disable_TestJob` 의 `1` 값이 또는 `True` (대/소문자 구분 안 함) 이면 함수는 실행 되지 않습니다. 이 경우 런타임에서 *Function 'Functions.TestJob' is disabled* 로그 메시지를 작성합니다.
...
큐 및 blob 트리거는 함수가 큐 메시지 또는 blob를 두 번 이상 처리 하지 않도록 자동으로 차단 합니다. 함수는 idempotent 필요가 없습니다.
...
다음 예에서는 기본적으로 `Warning` 수준의 모든 로그를 필터링 하는 필터를 생성 합니다. 버전 `Function` 2 `results` 의 에`Host.Results` 해당 하는 및 범주입니다. *x*)는 `Error` 수준에서 필터링 됩니다. 이 필터는 현재 범주를 `LogCategories` 인스턴스의 모든 등록된 수준과 비교하여 가장 긴 일치 항목을 선택합니다. 즉,에 대해 `Host.Triggers` 등록 된 `Host.Triggers.Queue` `Host.Triggers.Blob`수준이 또는와 일치 합니다. `Debug` 따라서 각각을 추가하지 않고도 더 광범위한 범주를 제어할 수 있습니다.
...
버전 2에서 *x* SDK의 경우 `LogCategoryFilter` 클래스를 사용 하 여 필터링을 제어할 수 있습니다. 에 `LogCategoryFilter` 는 초기 `Default` 값 `Information`이 인 속성이 있습니다 `Debug` . `Information` `Warning` `Critical` 즉,,, 또는 수준의 모든 메시지가 기록 되지만 `Error` 또는 `Trace` 수준이 필터링 됩니다.
...
다음 예제는 기본적으로 `Warning` 수준에서 모든 로그를 필터링하는 필터를 작성합니다. `Function` 및`Host.Results`범주는 수준`Error` 에서 필터링 됩니다. `LogCategoryFilter`는 현재 범주를 모든 등록된 `CategoryLevels`와 비교하여 가장 긴 일치 항목을 선택합니다. 따라서에 `Debug` 대해 `Host.Triggers` 등록 된 수준은 또는 `Host.Triggers.Queue` `Host.Triggers.Blob`와 일치 합니다. 따라서 각각을 추가하지 않고도 더 광범위한 범주를 제어할 수 있습니다.
...
버전 3 *x*에서는 호스트가 중지 [`TelemetryClient`] 될 때 더 이상를 플러시할 필요가 없습니다. .NET Core 종속성 주입 시스템은 [`TelemetryClient`]를 플러시하는 등록된 `ApplicationInsightsLoggerProvider`를 자동으로 제거합니다.
...
Application Insights 파이프라인의 특정 부분을 수정하려는 경우 사용자 고유의 `ITelemetryClientFactory`를 제공하면 호스트가 해당 클래스를 사용하여 [`TelemetryClient`]를 생성합니다. 예를 들어 다음 코드는 `DefaultTelemetryClientFactory` 를 재정의 하 여의 `ServerTelemetryChannel`속성을 수정 합니다.
