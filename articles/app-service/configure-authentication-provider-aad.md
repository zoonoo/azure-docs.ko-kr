...
1. **관리 모드** 에서 **고급** 을 선택하고 다음 표에 따라 App Service 인증을 구성합니다.
    ...
    |클라이언트 암호(선택 사항)| 앱 등록에서 생성한 클라이언트 암호를 사용합니다. 클라이언트 암호를 사용 하면 하이브리드 흐름이 사용 되며 App Service는 액세스 및 새로 고침 토큰을 반환 합니다. 클라이언트 암호를 설정 하지 않으면 암시적 흐름이 사용 되며 id 토큰만 반환 됩니다. 이러한 토큰은 공급자가 보내고 EasyAuth 토큰 저장소에 저장 됩니다.|
