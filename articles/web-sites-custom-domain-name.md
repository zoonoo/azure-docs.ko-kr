...
1. 도메인을 Azure 웹 사이트에 매핑하는 DNS 레코드를 만듭니다. 
1. Azure 관리 포털 안에 도메인 이름을 추가합니다. 
...
CNAME 레코드를 만들 경우에는 이 단계를 건너뛰세요. A 레코드를 만들려면 웹 사이트의 가상 IP 주소가 필요합니다. IP 주소를 가져오려면
...
<h2 id="awverify">"awverify" 레코드 만들기(A 레코드 전용)</h2>
