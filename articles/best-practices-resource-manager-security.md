...
이 내용을 작성할 당시에는 Azure 리소스 관리자의 [네트워크 리소스 공급자](virtual-network/resource-groups-networking.md)에서 [Express 경로](expressroute/expressroute-introduction.md)가 아직 지원되지 않았습니다. 온-프레미스 네트워크와 Azure 간에 ExpressRoute 연결이 있는 경우 BGP를 사용하도록 설정하여 ExpressRoute가 NRP에서 지원되고 나서 온-프레미스 네트워크에서 Azure로 경로를 전파할 수 있습니다. 이러한 BGP 경로는 각 Azure 서브넷의 사용자 정의 경로 및 기본 경로와 동일한 방식으로 사용됩니다. 자세한 내용은 [ExpressRoute 소개](expressroute/expressroute-introduction.md)를 참조하세요.
