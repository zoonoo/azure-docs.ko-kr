<properties 
	pageTitle="웹 사이트를 관리하는 방법 - Microsoft Azure 서비스 관리" 
	description="Microsoft Azure의 포털 웹 사이트 관리 페이지에 대한 참조입니다. 각 웹 사이트 관리 페이지에 대한 자세한 정보가 제공됩니다." 
	services="web-sites" 
	documentationCenter="" 
	authors="MikeWasson" 
	writer="mwasson" 
...
- **도구 얻기** - [WebMatrix 설치][mswebmatrix] 및 [Microsoft Azure SDK][azuresdk]에 대한 링크를 제공합니다.
...
**Note:** 페이지 아래에 있는 **메트릭 추가**를 선택하여 **모니터** 관리 페이지에서 성능 메트릭을 더 추가할 수 있습니다. 자세한 내용은 [웹 사이트를 모니터링하는 방법][Monitor]을 참조하세요.

**웹 끝점 상태** - 모니터링을 위해 구성된 웹 끝점의 목록입니다. 끝점이 구성되어 있지 않은 경우 **웹 끝점 모니터링 구성**을 클릭하고 **구성** 관리 페이지의 **모니터링** 섹션으로 이동하세요. 자세한 내용은 [웹 사이트를 모니터링하는 방법][Monitor]을 참조하세요.
...
**빠른 보기** 섹션은 다음 요약 정보 및 링크를 포함합니다(설정에 따라, 아래 나열된 옵션 중 일부는 나타나지 않을 수 있음).
 ...
 - **관리 서비스** - Microsoft Azure 관리 서비스 포털에서 웹 사이트에 대한 **작업 로그**를 보려면 작업 로그 링크를 클릭합니다.
 ...
 - **컴퓨팅 모드** - 웹 사이트가 무료, 공유, 기본 또는 표준 모드에서 실행 중인지 여부를 지정합니다. 웹 크기 조정 그룹 모드에 대한 자세한 내용은 [웹 사이트 크기를 조정하는 방법][Scale]을 참조하세요.
...
자세한 내용은 [웹 사이트를 구성하는 방법][Configure]을 참조하세요
...
웹 사이트의 크기 조정 옵션 구성에 대한 자세한 내용은 [웹 사이트 크기를 조정하는 방법][Scale]을 참조하세요.
...
사용할 수 있는 FTP 자격 증명 집합에는  *deployment* 자격 증명과  *publishing profile* 자격 증명의 두 가지가 있습니다.  주요 차이점은 다음과 같습니다.
...
두 가지 자격 증명 집합 중 하나를 선택할 수 있습니다. FTP 및 FTPS 호스트 이름은 대시보드의 **빠른 보기** 아래에 나열됩니다.
...
참고: 2단계에서 배포 자격 증명이 이미 있는 경우 포털에서 **배포 자격 증명 재설정** 메시지를 표시합니다. 이를 클릭하여 새 암호를 설정하거나 사용자 이름을 변경합니다.
...
전체 FTP 사용자 이름은 "website\username"입니다.  이는 포털의 **빠른 보기** 아래에 **배포/FTP 사용자**로 나열됩니다.
