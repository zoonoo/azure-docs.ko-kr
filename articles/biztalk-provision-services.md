<properties urlDisplayName="Provision BizTalk Services in management portal" pageTitle="관리 포털에서 BizTalk 서비스 만들기 | Azure" metaKeywords="Get started Azure biztalk services, provision, Azure unstructured data" description="Azure 관리 포털에서 BizTalk 서비스를 프로비전하고 선택적 SQL 데이터베이스 서버와 저장소 계정을 만드는 방법에 대해 알아봅니다." metaCanonical="http://www.windowsazure.com/ko-kr/manage/services/biztalk-services/provisioning-biztalk-service" services="biztalk-services" documentationCenter="" title="BizTalk 서비스: Azure 관리 포털을 사용하여 프로비전" authors="mandia" solutions="" manager="dwrede" editor="cgronlun" />
...
**팁**
Azure 관리 포털에 로그인하려면 Azure 계정과 Azure 구독이 있어야 합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. [Azure 무료 평가판][Azure 무료 평가판]을 참조하세요.

</div>

## <a name="BizTalk"></a>BizTalk 서비스 만들기
...
4.  BizTalk 서비스 설정을 입력합니다.
    ...
    |-------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | **BizTalk 서비스 이름** | 아무 이름이나 입력할 수 있지만 구체적이어야 합니다. 일부 사례:                                                                                                                                                    
                                *mycompany*.biztalk.windows.net                                                                                                                                                                                   
                                *mycompanymyapplication*.biztalk.windows.net                                                                                                                                                                      
                                *myapplication*.biztalk.windows.net                                                                                                                                                                               
                                입력한 이름에 ".biztalk.windows.net"이 자동으로 추가됩니다. 이 항목은 BizTalk 서비스에 액세스하는 데 사용되는 URL(예: **https://*myapplication*.biztalk.windows.net**)을 만듭니다.                                |
    | **버전**                | 테스트/개발 단계에 있으면 **Developer**를 선택합니다. 프로덕션 단계에 있는 경우 [BizTalk 서비스: Edition 차트][BizTalk 서비스: Edition 차트]를 사용하여 **Premium**, **Standard**, **Basic** 중 비즈니스 시나리오에 적합한 버전을 결정합니다. |
    | **지역**                | BizTalk 서비스를 호스트할 지리적 지역을 선택합니다.                                                                                                                                                               |
    | **도메인 URL**          | **옵션**. 기본적으로 도메인 URL은 *YourBizTalkServiceName*.biztalk.windows.net입니다. 사용자 지정 도메인을 입력할 수도 있습니다. 예를 들어 도메인이 *contoso*이면 다음과 같이 입력할 수 있습니다.                 
...
5.  저장소 및 데이터베이스 설정을 입력합니다.
    ...
    <tbody>
    <tr class="odd">
    <td align="left"><strong>저장소 계정 모니터링/보관</strong></td>
    <td align="left">기존 저장소 계정을 선택하거나 새 저장소 계정 만들기를 선택합니다.<br /><br /> 새 저장소 계정을 만들 경우 <strong>저장소 계정 이름</strong>을 입력합니다.</td>
    </tr>
    <tr class="even">
    <td align="left"><strong>추적 데이터베이스</strong></td>
    <td align="left">기존 Azure SQL 데이터베이스를 사용하는 경우 다른 BizTalk 서비스에서는 이 데이터베이스를 사용할 수 없습니다. Azure SQL 데이터베이스 서버를 만들 때 입력한 로그인 이름과 암호가 있어야 합니다.<br /><br />
    <div class="dev-callout">
    <strong>팁</strong>
    <p>BizTalk 서비스와 동일한 지역에 추적 데이터베이스 및 모니터링/보관 저장소 계정을 만듭니다.</p>
    </div></td>
    </tr>
    </tbody>
    </table>
...
6.  데이터베이스 설정을 입력합니다.
    ...
    |----------------------|-----------------------------------------------------------------------------------------------------------------------------------|
    | **이름**             | 이전 화면에서 **새 SQL 데이터베이스 인스턴스 만들기**를 선택한 경우에 사용할 수 있습니다.                                         
                             BizTalk 서비스에서 사용할 SQL 데이터베이스 이름을 입력합니다.                                                                     |
    | **서버**             | 이전 화면에서 **새 SQL 데이터베이스 인스턴스 만들기**를 선택한 경우에 사용할 수 있습니다.                                         
                             기존 SQL 데이터베이스 서버를 선택하거나 새 SQL 데이터베이스 서버를 만듭니다.                                                      |
    | **서버 로그인 이름** | 로그인 사용자 이름을 입력합니다.                                                                                                  |
    | **서버 로그인 암호** | 로그인 암호를 입력합니다.                                                                                                         |
    | **지역**             | **새 SQL 데이터베이스 인스턴스 만들기**를 선택한 경우에 사용할 수 있습니다. SQL 데이터베이스를 호스트할 지리적 지역을 선택합니다. |
...
BizTalk 서비스를 프로비전할 때 자동으로 만들어진 자체 서명된 인증서는 개발 환경에서만 사용하기 위한 용도로 제공됩니다. 프로덕션 시나리오에서는 이 인증서를 프로덕션이 준비된 인증서로 대체해야 합니다.

1.  **대시보드** 탭에서 **SSL 인증서 업데이트**를 클릭합니다.
2.  BizTalk 서비스 이름을 포함하는 개인 SSL 인증서(*CertificateName*.pfx)를 찾아 암호를 입력하고 확인 표시를 선택합니다.
...
다음 요구 사항은 Free Edition에는 적용되지 않습니다.
...
<td>
구독은 Azure 관리 포털에 로그인할 수 있는 사람을 결정합니다. 계정 소유자는 [Azure 구독][Azure 구독]에서 구독을 만듭니다.
Azure 계정은 여러 구독을 소유할 수 있으며, 허용된 모든 사람이 관리할 수 있습니다. 예를 들어 Azure 계정 소유자는 *BizTalkServiceSubscription*이라는 구독을 만들고, 회사의 BizTalk 관리자(예: ContosoBTSAdmins@live.com)에게 이 구독에 대한 액세스를 제공합니다. 이 시나리오에서 BizTalk 관리자는 Azure 관리 포털에 로그인하고 구독의 모든 호스팅 서비스(Azure BizTalk 서비스 포함)에 대해 전체 관리자 권한을 가집니다. BizTalk 관리자는 Azure 계정 소유자가 아니므로 대금 청구 정보에 대한 액세스 권한이 없습니다.
[Azure 관리 포털에서 구독 및 저장소 계정 관리][Azure 관리 포털에서 구독 및 저장소 계정 관리]에 자세한 내용이 나와 있습니다.
...
<td>
추적 데이터를 포함하여 BizTalk 서비스에서 사용되는 테이블, 보기 및 저장 프로시저를 저장합니다.
BizTalk 서비스를 만들 때 기존 Azure SQL Server 또는 Azure SQL 데이터베이스를 사용하거나 새 서버 또는 데이터베이스를 자동으로 만들 수 있습니다.
SQL 데이터베이스 크기는 자동으로 구성됩니다. 일반적으로 기본 크기는 BizTalk 서비스에 사용하기에 충분합니다. 크기를 변경하면 가격이 영향을 받습니다. [Azure SQL 데이터베이스의 계정 및 대금 청구][Azure SQL 데이터베이스의 계정 및 대금 청구]를 참조하세요.
**참고 사항**

-   새 Azure SQL Server 및 데이터베이스를 만들면 Azure 서비스가 자동으로 사용하도록 설정됩니다. BizTalk 서비스의 경우에는 Azure 서비스를 사용하도록 설정해야 합니다.
-   기존 Azure SQL Server에서 새 Azure SQL 데이터베이스를 만들면 서버의 방화벽 규칙이 변경되지 않습니다. 따라서 다른 Azure 서비스가 서버의 데이터베이스에 액세스하지 못할 수 있습니다.
...
<td>
Azure BizTalk 서비스로 인증합니다. Visual Studio에서 BizTalk 서비스를 배포할 때 이 액세스 제어 네임스페이스를 입력합니다. BizTalk 서비스를 만들면 액세스 제어 네임스페이스가 자동으로 만들어집니다.
...
<td>
BizTalk 서비스에서 다음을 저장하는 데 사용되는 테이블, Blob 및 큐에 대한 액세스를 제공합니다.

-   BizTalk 서비스를 모니터링하는 로그 파일. 또한 모니터링한 결과도 Azure 관리 포털의 모니터링 탭에 표시됩니다.
-   파트너 사이에 X12 또는 AS2 계약을 생성할 때 보관 기능을 사용하도록 설정하여 메시지 속성을 저장할 수 있습니다. 이 데이터는 저장소 계정에 저장됩니다.
...
-   인증서 만료 날짜는 5년 미만이어야 합니다.
-   모든 개인 인증서에는 암호가 필요합니다. 이 암호를 확인하고 관리자와 공유하는 것이 좋습니다.
-   자체 서명된 인증서는 테스트/개발 환경에서 사용됩니다. 자체 서명된 인증서를 사용하는 경우 인증서를 개인 인증서 저장소 및 신뢰할 수 있는 루트 인증 기관 인증서 저장소로 가져옵니다.

프로덕션 인증서 요청을 인증 기관에 보내려면 다음 인증서 속성을 제공합니다.

-   **확장된 키 사용**: Azure BizTalk 서비스는 최소한 서버 인증이 필요합니다.
-   **일반 이름**: Azure BizTalk 서비스 URL의 FQDN(정규화된 도메인 이름)을 입력합니다. 이 항목의 [BizTalk 서비스 만들기][BizTalk 서비스 만들기]를 참조하세요.
...
BizTalk 서비스를 만들었으므로 이제 [BizTalk 서비스: 대시보드, 모니터 및 크기 조정 탭][대시보드, 모니터 및 크기 조정 탭]에서 다른 탭도 익숙해질 수 있습니다. 응용 프로그램에 BizTalk 서비스를 사용할 준비가 되었습니다. 응용 프로그램을 만들려면 [Azure BizTalk 서비스][Azure BizTalk 서비스]로 이동하세요.

## 참고 항목

-   [BizTalk 서비스: Editions 차트][BizTalk 서비스: Edition 차트]
...
-   [하이브리드 연결][하이브리드 연결]
  ...
  [완료되면 진행률 아이콘이 표시됨]: ./media/biztalk-provision-services/WABS_ProgressComplete.png
  [대시보드, 모니터 및 크기 조정 탭]: http://go.microsoft.com/fwlink/p/?LinkID=302281
  [BizTalk 서비스 상태 차트]: http://go.microsoft.com/fwlink/p/?LinkID=329870
