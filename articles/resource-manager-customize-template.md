<properties 
	...
	description="내보낸 Azure Resource Manager 템플리셍 매개 변수를 추가하고 Azure PowerShell 또는 Azure CLI를 통해 다시 배포합니다." 
...
# 내보낸 Resource Manager 템플릿 사용자 지정

이 항목에서는 내보낸 템플릿을 수정하여 매개 변수로 추가 값의 전달을 허용하는 방법에 대해 설명합니다. 이 항목은 [Resource Manager 템플릿 내보내기](resource-manager-export-template.md) 항목에서 수행한 단계를 기준으로 하지만 반드시 해당 항목을 먼저 완료해야 하는 것은 아닙니다. 이 항목에서 필요한 템플릿과 스크립트를 찾을 수 있습니다.
...
이전 연습을 완료하지 않은 경우 내보낸 템플릿은 다음과 같이 표시됩니다. **template.json**이라는 파일을 만들고 파일에 아래 내용을 추가합니다.
...
위에 표시된 템플릿은 모든 배포에 대해 동일한 주소 접두사와 동일한 서브넷 접두사를 사용하는 가상 네트워크가 구축된 동일 지역에 동일한 유형의 저장소 계정을 만들려는 경우에만 올바르게 작동합니다. 하지만 Resource Manager를 사용하면 훨씬 더 유연하게 템플릿을 배포할 수 있습니다. 예를 들어 배포 중에 만들 저장소 계정 유형 또는 가상 네트워크 주소 접두사 및 서브넷 접두사에 사용할 값을 지정할 수 있습니다.
...
3. 리소스 정의의 매개 변수와 변수를 사용하려면 **리소스** 섹션을 다음 정의로 바꿉니다. 리소스 속성에 할당된 값을 제외하고 리소스 정의에서 실제로 변경된 사항은 거의 없습니다. 속성은 내보낸 템플릿의 속성과 정확하게 동일합니다. 하드 코드된 값 대신 매개 변수 값에 속성을 할당하기만 하면 됩니다. 리소스 위치는 **resourceGroup().location** 식을 통한 리소스 그룹과 동일한 위치를 사용하도록 설정됩니다. 저장소 계정 이름에 대해 만든 변수는 **variables** 식을 통해 참조됩니다.
...
다운로드한 매개 변수 파일은 더 이상 템플릿의 매개 변수와 일치하지 않습니다. 매개 변수 파일을 사용하지 않아도 되지만 환경을 다시 간단히 배포할 수 있습니다. 템플릿에서 다양한 매개 변수에 대해 정의한 기본값을 사용하므로 매개 변수 파일은 두 개 값만 필요합니다.
...
위에 표시된 매개 변수 파일은 기본값이 없는 매개 변수의 값만 제공합니다. 기본값과 다른 값을 원할 경우에만 다른 매개 변수의 값을 제공할 수 있습니다.

## 템플릿 배포

Azure PowerShell 또는 Azure CLI를 사용하여 사용자 지정된 템플릿과 매개 변수 파일을 배포할 수 있습니다. 필요할 경우 [Azure PowerShell](powershell-install-configure.md) 또는 [Azure CLI](xplat-cli-install.md)를 설치합니다. 원본 템플릿을 내보낼 때 템플릿과 함께 다운로드한 스크립트를 사용하거나 고유 스크립트를 작성하여 템플릿을 배포할 수 있습니다. 두 옵션은 이 토픽에 나와 있습니다.

2. 사용자 고유 스크립트를 배포하려면 다음 중 하나를 사용합니다.
...
- [Resource Manager 템플릿 연습](resource-manager-template-walkthrough.md)은 더 복잡한 솔루션의 템플릿을 만들어 이 항목에서 학습한 내용을 바탕으로 자세한 내용을 배웁니다. 이 연습을 통해 사용 가능한 리소스와 제공할 값을 결정하는 방법을 이해할 수 있습니다.
