<properties 
	pageTitle="프로그래밍 방식으로 HDInsight 응용 프로그램 로그 액세스| Azure" 
...
# 프로그래밍 방식으로 HDInsight 응용 프로그램 로그 액세스

이 항목에서는 HDInsight의 Hadoop 클러스터에서 완료된 YARN 응용 프로그램을 프로그래밍 방식으로 열거하는 방법과 클러스터로 RDP하지 않고 프로그래밍 방식으로 이 응용 프로그램 로그에 액세스하는 방법을 설명합니다. 특히 다음과 같은 새 구성 요소와 새 API가 추가되었습니다.

  1. HDInsigh 클러스터에서 제네릭 응용 프로그램 기록 서버를 사용하도록 설정되었습니다. 이 서버는 YARN Timeline Server 내의 구성 요소로, 완료된 응용 프로그램에 대한 제네릭 정보의 저장 및 검색을 처리합니다.
  2. 클러스터에서 실행한 응용 프로그램을 프로그래밍 방식으로 열거하고 발생하는 응용 프로그램 문제를 디버깅하기 위해 관련 응용 프로그램 또는 컨테이너별 로그(일반 텍스트)를 다운로드할 수 있도록 Azure HDInsight .Net SDK의 새 API가 추가되었습니다.


## 필수 조건

.NET 응용 프로그램에서 이 항목에서 제공하는 코드를 사용하려면 Azure HDInsight SDKrk 필요합니다. 최근 게시된 SDK 빌드는 [NuGet](http://nuget.codeplex.com/wikipage?title=Getting%20Started)(영문)에서 다운로드할 수 있습니다. 
...
- [YARN 응용 프로그램 및 로그](#YARNAppsAndLogs)
- [프로그래밍 방식으로 응용 프로그램 열거 및 로그 다운로드](#enumerate-and-download)
...
* 3.1.1.374 이상 버전에서는 HDInsigh 클러스터에서 제네릭 응용 프로그램 정보의 저장 및 검색을 사용하도록 설정되었습니다. 
* Timeline Server의 프레임워크별 응용 프로그램 정보 구성 요소는 HDInsight 클러스터에서 현재 사용할 수 없습니다.


응용 프로그램에 대한 제네릭 정보에는 다음과 같은 데이터가 포함됩니다. 

* **ApplicationId**(응용 프로그램의 고유 식별자) 
* 응용 프로그램을 시작한 **사용자** 
* 응용 프로그램을 완료하려고 **시도한 횟수** 
* 지정된 응용 프로그램 시도에 사용된 **컨테이너** 

HDInsight 클러스터에서 이 정보는 리소스 관리자가 기본 저장소 계정의 기본 컨테이너에 있는 기록 저장소에 저장합니다. 완료된 응용 프로그램에 대한 이 제네릭 데이터는 REST API를 통해 검색할 수 있습니다.

    GET on https://<cluster-dns-name>.azurehdinsight.net/ws/v1/applicationhistory/apps

Microsoft는 이 데이터를 프로그래밍 방식으로 쉽게 검색할 수 있게 해주는 새 API를 HDInsight .Net SDK에 추가했습니다. 또한 RDP를 사용하는 클러스터에 연결한 후 클러스터 노드에서 직접 YARN CLI 명령을 실행하여 제네릭 데이터를 검색할 수도 있습니다.

## <a name="YARNAppsAndLogs"></a>YARN 응용 프로그램 및 로그

YARN(Yet Another Resource Negotiator)은 여러 프로그래밍 모델(예: MapReduce)을 지원하여 리소스 관리를 응용 프로그램 예약/모니터링과 분리합니다. 이 작업은 전역  *Resource Manager*(RM)을 통해, 작업 노드 *Node Managers*(NM)별 및 응용 프로그램 *Application Masters*(AM)별로 수행됩니다. 응용 프로그램별 AM은 응용 프로그램을 실행하기 위한 리소스(CPU, 메모리, 디스크, 네트워크)를 RM과 협상합니다. RM은 NM과 협력하여 이러한 리소스를 부여하며, 이 리소스는  *Containers*로 부여됩니다. AM은 RM에 의해 부여받은 컨테이너의 진행률 추적합니다. 응용 프로그램의 특성에 따라 응용 프로그램에 여러 컨테이너가 필요할 수 있습니다. 

또한 각 응용 프로그램은 작동 중단이 발생하거나 AM과 RM 간의 통신 손실로 인해 응용 프로그램을 완료하기 위해 여러  *Application Attempts*로 구성될 수도 있습니다. 따라서 컨테이너는 응용 프로그램의 특정 시도에 부여됩니다. 컨테이너는 YARN 응용 프로그램에서 수행하는 기본 작업 단위에 대한 컨텍스트를 제공하고 컨테이너의 컨텍스트 내에서 수행되는 모든 작업은 컨테이너가 할당된 단일 작업자 노드에서 수행된다고 볼 수 있습니다. 자세한 내용은 [YARN 개념][YARN-concepts](영문)을 참조하세요.
...
여기서 "*user*"는 응용 프로그램을 시작한 사용자의 이름이고, "*applicationId*"는 YARN 리소스 관리자가 할당한 응용 프로그램의 고유 식별자입니다.
...
다음 섹션에서는 HDInsight 클러스터로 RDP하지 않고 프로그래밍 방식으로 응용 프로그램 또는 컨테이너별 로그에 액세스하는 방법에 대해 설명합니다.
...
또는 아래 코드 조각은 "ApplicationId"(고유 식별자)를 아는 응용 프로그램의 로그를 다운로드합니다. ApplicationId는 리소스 관리자에서 할당한 응용 프로그램의 전역 고유 식별자입니다. 이 식별자는 리소스 관리자의 시작 시간과 리소스 관리자에 제출된 응용 프로그램 수를 나타내는 단조롭게 증가하는 카운터를 함께 사용하여 생성됩니다. ApplicationId의 형식은 "application\_&lt;RM-start-time&gt;\_&lt;Counter&gt;"입니다. ApplicationId와 JobId는 다릅니다. JobId는 MapReduce 프레임워크 관련 개념이지만 ApplicationId는 프레임워크와 상관없는 YARN 개념입니다. YARN에서 JobId는 리소스 관리자에 제출된 MapReduce 응용 프로그램의 응용 프로그램 마스터에서 처리하는 특정 MapReduce 작업을 식별합니다.
