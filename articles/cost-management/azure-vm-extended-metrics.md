...
예를 들어 Azure VM의 CPU %와 메모리 %를 모니터링하고자 할 수 있습니다. Azure VM 메트릭은 _사용 중인 CPU 및 메모리\% 커밋된 바이트_의 _백분율_ 에 해당 합니다.

> [!NOTE]
> 확장 메트릭 데이터 수집은 Azure 게스트 수준 모니터링을 통해서만 지원됩니다. Cloudyn는 Azure Monitor logs VM 확장과 호환 되지 않습니다.
...
표준 메트릭은 호스트 컴퓨터 메트릭입니다. _CPU의 백분율_ 메트릭은 한 가지 예입니다. 게스트 VM의 기본 메트릭도 있는데 역시 확장 메트릭이라고 합니다. 확장 메트릭의 예로는 _사용 중인 메모리\% 커밋된 바이트_ 와 _\memory\available bytes_가 있습니다.
