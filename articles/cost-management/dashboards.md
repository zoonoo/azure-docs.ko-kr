---
title: Azure Cost Management 대시보드에서 주요 메트릭 보기 | Microsoft Docs
description: 이 문서에서는 Azure Cost Management의 대시보드를 사용하여 주요 메트릭을 보는 방법을 설명합니다.
...
ms.locfileid: "35303158"
---
# <a name="view-key-cost-metrics-with-dashboards"></a>대시보드를 사용하여 주요 비용 메트릭 보기

Cloudyn의 대시보드에서는 개요 수준의 보고서를 확인할 수 있습니다. 대시보드를 사용하면 핵심 비용 메트릭을 단일 뷰에서 볼 수 있습니다. 또한 비즈니스 추세 하이라이트도 제공되므로, 중요한 비즈니스 의사 결정을 내리는 데 활용할 수 있습니다.

대시보드는 조직에서 다음과 같은 업무의 담당자에게 필요한 뷰를 작성하는 데에도 사용됩니다.
...
대시보드 버전은 MSP(Management), Enterprise 및 Premium Cloudyn 사용자에 따라 다릅니다. 차이는 엔터티 액세스 레벨에 의해 결정됩니다. 액세스 레벨에 대한 자세한 내용은 [Entity access levels](tutorial-user-access.md#entity-access-levels)(엔터티 액세스 레벨)를 참조하세요.

대시보드 가용성은 대시보드를 볼 때 사용되는 클라우드 서비스 공급자 계정의 유형에 따라 다릅니다. 사용 가능한 정보 및 Cloudyn에서 수집하는 정보의 유형에 따라 대시보드의 보고서 내용이 달라집니다. 예를 들어, AWS 계정이 없는 경우 S3 Tracker 대시보드가 표시되지 않습니다. 마찬가지로, Azure Resource Manager에서 Cloudyn에 액세스할 수 있도록 설정하지 않는 경우 최적화 프로그램 대시보드 위젯에 Azure 특정 정보가 표시되지 않습니다.

미리 만들어진 대시보드를 사용하거나 사용자 지정 보고서를 갖춘 고유한 대시보드를 만들 수 있습니다. Cloudyn 보고서에 대해 잘 모르는 경우에는 [Use Cost Management reports](use-reports.md)(Cost Management 보고서 사용)를 참조하세요.

## <a name="create-a-custom-dashboard"></a>사용자 지정 대시보드 만들기

사용자 지정 대시보드를 빠르게 시작하려면 기존 대시보드를 복제하여 속성을 활용하면 됩니다. 그런 다음, 필요에 맞게 수정할 수 있습니다. 복사할 대시보드에서 **다른 이름으로 저장**을 클릭합니다. 사용자 지정 대시보드만 복제할 수 있습니다. Cloudyn에 포함된 대시보드는 복제할 수 없습니다.
...
## <a name="modify-a-custom-dashboard"></a>사용자 지정 대시보드 수정
...
## <a name="share-a-custom-dashboard"></a>사용자 지정 대시보드 공유
...
3. **예**를 클릭합니다. 이제 대시보드를 다른 사람이 사용할 수 있습니다.

## <a name="delete-a-custom-dashboard-report"></a>사용자 지정 대시보드 보고서 삭제
...
사용자 지정 대시보드를 조직의 모든 사용자 또는 현재 엔터티의 사용자와 공유할 수 있습니다. 대시보드를 공유하면 다른 사용자들이 개요 수준의 KPI를 신속하게 확인할 수 있습니다. 대시보드를 공유하면 대시보드가 자동으로 모든 Cloudyn 엔터티/고객에 복제됩니다. 공유 대시보드를 변경하는 경우, 변경 사항이 자동으로 업데이트됩니다.
...
## <a name="duplicate-a-custom-dashboard"></a>사용자 지정 대시보드 복제

새 대시보드를 만들 때 기존 대시보드에서 유사한 속성을 사용할 수 있습니다. 대시보드를 복제하여 새 대시보드를 만들 수 있습니다.

사용자 지정 대시보드만 복제할 수 있으며 표준 대시보드는 복제할 수 없습니다.
...
임의의 대시보드를 기본으로 설정할 수 있습니다. 기본으로 설정한 대시보드는 대시보드 탭 목록에서 맨 왼쪽 탭으로 표시됩니다. 기본 대시보드는 Cloudyn 포털을 열 때 표시됩니다.
...
관리 대시보드(MSP 사용자용 MSP 대시보드)에서는 기본 보고서 유형을 확인할 수 있습니다.  
...
이 위젯은 비용 추세를 발견하는 데 유용합니다. 지난 30일의 추세를 기반으로 마지막 날의 비용을 보여 줍니다.
...
### <a name="compute-instances---daily-trend"></a>계산 인스턴스 - 일별 추세
...
### <a name="compute-instances---daily-trend"></a>계산 인스턴스 - 일별 추세
이 위젯은 지난 30일 동안의 일별 계산 인스턴스(즉석, 예약, 주문형)를 보여 줍니다.
- 그래프 위로 마우스를 가져가면 유형별 계산 인스턴스 수가 일별로 표시됩니다.
- 위젯을 클릭하면 시간에 따른 인스턴스 보고서가 열립니다.

### <a name="all-buckets-s3"></a>모든 버킷(S3)
이 위젯은 총 S3 저장 공간 및 저장된 개체 수를 보여 줍니다.
- 위젯을 클릭하면 S3 트래커 대시보드가 열립니다. 이 대시보드를 사용하면 현재 저장소 사용량 및 추세를 찾고 분석하고 표시할 수 있습니다.
...
## <a name="optimizer-dashboard"></a>최적화 프로그램 대시보드
이 대시보드에서는 다운사이징 권장 사항, 사용되지 않는 리소스, 절약 가능 금액을 보여 줍니다.  
![최적화 프로그램 대시보드](./media/dashboards/optimizer-dashboard.png)
...
- 위젯을 클릭하면 RDS 크기 조정 권장 사항 보고서가 열리고 자세한 Amazon RDS 크기 권장 사항이 표시됩니다.

최적화 권장 사항은 지난달에 모니터링한 사용량 및 성능 데이터를 기반으로 합니다.

## <a name="s3-tracker-dashboard"></a>S3 트래커 대시보드
S3 트래커 대시보드를 사용하면 현재 저장소 사용량 및 추세를 찾고 분석하고 표시할 수 있습니다.  
![S3 트래커 대시보드](./media/dashboards/s3-tracker-dashboard.png)
...
클라우드 비교 대시보드를 사용하면 가격, CPU 종류 및 RAM 크기를 기준으로 여러 클라우드 제공자의 비용을 비교할 수 있습니다.  
...
이 위젯은 현재 Amazon EC2 비용을 제시하고 이를 Azure와 비교합니다. 비교는 지난 30일 동안 주문형 요금 사용 추세를 기반으로 합니다.
- 위젯을 클릭하면 배포 포팅 - 비용 분석 보고서가 열립니다.

### <a name="ec2azure-instance-type-mapping"></a>EC2/Azure 인스턴스 유형 매핑
이 위젯은 Amazon EC2와 Azure 간에 가장 좋은 탄력적 계산 단위 매핑을 보여 줍니다.
