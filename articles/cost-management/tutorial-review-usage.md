...
시간 경과별 보고서를 통해 사용량 및 비용에 소요된 실제 금액을 추적하여 추세를 확인합니다. 추세 확인을 보려면 시간 경과별 실제 비용 보고서를 사용합니다. 포털 위쪽의 보고서 메뉴에서 **비용** > **비용 분석** > **시간 경과별 실제 비용**을 차례로 클릭합니다. 보고서를 처음 열면 그룹이나 필터가 적용되어 있지 않습니다.
...
포털 위쪽의 보고서 메뉴에서 **최적화 프로그램** > **가격 책정 최적화** > **비용 효율적 크기 조정 권장 사항**을 차례로 클릭합니다. 공급자를 Azure로 필터링하여 Azure VM만 살펴봅니다. 다음은 예제 이미지입니다.
...
비용 보고서를 사용하여 지출에 대한 경고를 만듭니다. 이 예에서는 시간 경과별 실제 비용 보고서를 사용하여 Azure VM 지출이 총 예산에 도달할 때 이를 알려줍니다. 포털 위쪽의 보고서 메뉴에서 **비용** > **비용 분석** > **시간 경과별 실제 비용**을 차례로 클릭합니다. **그룹**을 **서비스**로 설정하고, **서비스 기준 필터**를 **Azure/VM**으로 설정합니다. 보고서의 오른쪽 위에서 **작업**을 클릭한 다음 **보고서 예약**을 선택합니다.

원하는 빈도에 따라 자신에게 보고서의 전자 메일을 보내도록 하려면 **일정 예약** 탭을 사용합니다. 사용한 모든 태그, 그룹 및 필터링이 전자 메일로 보내는 보고서에 포함됩니다. **임계값** 탭을 클릭하고 **실제 비용 대 임계값**을 선택합니다. 총 예산이 50만 달러이고 비용이 절반 정도에 도달할 때 알림을 원하는 경우 25만 달러에는 **빨간색 경고**, 24만 달러에는 **노란색 경고**를 만듭니다. 그런 다음 연속되는 경고의 횟수를 선택합니다. 지정한 총 경고 횟수에 도달하면 추가 경고를 보내지 않습니다. 예약된 보고서를 저장합니다.
