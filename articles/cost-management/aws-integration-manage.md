...
| 치수 | Amazon CUR header | 범위 | 의견 |
| --- | --- | --- | --- |
| 가용성 영역 | lineitem/AvailabilityZone | 전체 |   |
| 위치 | product/Region | 전체 |   |
| 미터 |   | 전체 |   |
| Meter category | lineItem/ProductCode | 전체 |   |
| Meter subcategory | lineitem/UsageType | 전체 |   |
| 작업(Operation) | lineItem/Operation | 전체 |   |
| 리소스 | lineItem/ResourceId | 전체 |   |
| 리소스 종류 | product/instanceType | 전체 | If product/instanceType is null, lineItem/UsageType is used. |
| ResourceGuid | N/A | 전체 | Azure meter GUID. |
| 서비스 이름 | product/ProductName | 전체 | If product/ProductName is null, lineItem/ProductCode is used. |
| 서비스 계층 |   |   |   |
| 구독 ID | lineItem/UsageAccountId | Consolidated account and management group |   |
| 구독 이름 | N/A | Consolidated account and management group | Account names are collected using the AWS Organization API. |
| 태그 | resourceTags/\* | 전체 | The _user:_ prefix is removed from user-defined tags to allow cross-cloud tags. The _aws:_ prefix is left intact. |
| Billing account ID | bill/PayerAccountId | 관리 그룹 |   |
| Billing account name | N/A | 관리 그룹 | Account names are collected using the AWS Organization API. |
| 공급자 | N/A | 관리 그룹 | Either AWS or Azure. |
