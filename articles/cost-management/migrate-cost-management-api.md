...
| 이전 속성 | 새 속성 | 메모 |
| --- | --- | --- |
| AccountId | N/A | 구독 작성자 추적 되지 않습니다. InvoiceSectionId (departmentId 동일)을 사용 합니다. |
...
| ConsumedService | consumedService | 정확한 문자열 값이 달라질 수 있습니다. |
| ConsumedServiceId | 없음 | &nbsp; |
| CostCenter | costCenter | &nbsp; |
| 날짜 및 usageStartDate | date | &nbsp;  |
| 일 | 없음 | 날짜에서 하루를 구문 분석합니다. |
| DepartmentId | invoiceSectionId | 정확한 값이 다릅니다. |
...
| 되풀이 요금은 | 없음 | &nbsp;  |
| Location | location | &nbsp;  |
| MeterCategory | meterCategory | 정확한 문자열 값이 달라질 수 있습니다. |
| MeterId | meterId | 정확한 문자열 값이 다릅니다. |
| MeterName | meterName | 정확한 문자열 값이 달라질 수 있습니다. |
| MeterRegion | meterRegion | 정확한 문자열 값이 달라질 수 있습니다. |
| MeterSubCategory | meterSubCategory | 정확한 문자열 값이 달라질 수 있습니다. |
...
| UnitOfMeasure | unitOfMeasure | 정확한 문자열 값이 다릅니다. |
| usageEndDate | date | &nbsp;  |
| Year | 없음 | 날짜의 연도 구문 분석합니다. |
| (신규) | billingCurrency | 요금은 사용 되는 통화입니다. |
| (신규) | billingProfileId | 청구 프로필 (등록 동일)에 대 한 고유 ID입니다. |
| (신규) | billingProfileName | (등록 동일) 청구 프로필의 이름입니다. |
| (신규) | chargeType | Azure 서비스 사용량이, Marketplace 사용량 및 구매를 구분 하기 위해 사용 합니다. |
| (신규) | invoiceId | 청구서에 대 한 고유 ID입니다. 현재 달에 비어 있습니다. |
| (신규) | publisherType | 구매에 대 한 게시자의 유형입니다. 사용량에 대 한 비어 있습니다. |
| (신규) | serviceFamily | 구매의 형식입니다. 사용량에 대 한 비어 있습니다. |
| (신규) | servicePeriodEndDate | 구입한 서비스에 대 한 종료 날짜입니다. |
| (신규) | servicePeriodStartDate | 구입한 서비스에 대 한 날짜를 시작 합니다. |
...
| 미터 이름 | meterName | 측정기의 이름입니다. 미터는 Azure 서비스 배포 가능한 리소스를 나타냅니다. |
...
|  제품 Id  | productOrderName | OfferId, 대신 productOrderName를 사용 합니다. 그러나 동일 하지는 않습니다 OfferId, productOrderName 및 미터 확인 Microsoft 고객 계약의 가격 책정 합니다. 레거시 등록에서 meterId 및 Offerid 관련이 있습니다. |
