<properties pageTitle="서버 스크립트를 사용하여 데이터 유효성 검사 및 수정(Android) | 모바일 개발자 센터" description="Android 앱에서 서버 스크립트를 사용하여 전송된 데이터의 유효성을 검사하고 데이터를 수정하는 방법에 대해 알아봅니다." services="mobile-services" documentationCenter="android" authors="RickSaling" manager="dwrede" editor=""/>
...
1. Eclipse에서 [데이터 시작] 자습서를 완료했을 때 생성된 프로젝트를 엽니다.
...
1. 패키지 탐색기에서 ToDoItem.java 파일을 열고 다음 **import**문을 추가합니다.
...
2. **TodoItem** 클래스의 전용 필드 정의에 다음 코드를 추가합니다.
    ...
    > [AZURE.NOTE]  `SerializedName` 주석은 앱의 새  `mCreatedAt` 속성을 TodoItem 테이블에 정의된  `createdAt` 열(이름이 다름)에 매핑하도록 클라이언트에 지시합니다. 이 주석을 사용하면 앱이 SQL 데이터베이스의 열 이름과 다른 개체 속성 이름을 가질 수 있습니다. 이 주석을 사용하지 않는 경우 대/소문자 차이 때문에 오류가 발생합니다.
...
5. 패키지 탐색기에서 ToDoItemAdapter.java 파일을 열고 다음 **import** 문을 추가합니다.
...
7. `checkBox.setText(currentItem.getText());` 코드를 찾아 이 코드 줄을 다음으로 바꿉니다.
...
7. 기존 **RefreshItemsFromTable** 메서드를 다음 코드로 바꿉니다.
