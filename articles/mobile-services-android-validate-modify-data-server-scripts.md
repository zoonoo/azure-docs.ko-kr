<properties urlDisplayName="Validate Data - Android" pageTitle="서버 스크립트를 사용하여 데이터 유효성 검사 및 수정(Android) | 모바일 개발자 센터" metaKeywords="" description="Learn how to validate and modify data sent using server scripts from your Android app." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Validate and modify data in Mobile Services by using server scripts" authors="ricksal" solutions="" manager="dwrede" editor="" />
...
1. Eclipse에서 [데이터 작업 시작] 자습서를 완료했을 때 생성된 프로젝트를 엽니다.
...
2. **TodoItem** 클래스의 전용 필드 정의에 다음 코드를 추가합니다.
    ...
    <div class="dev-callout"><b>참고</b>
	<p><code>SerializedName</code> 주석은 앱의 새 <code>mCreatedAt</code> 속성을 TodoItem 테이블에 정의된 <code>createdAt</code> 열(이름이 다름)에 매핑하도록 클라이언트에 지시합니다. 이 주석을 사용하면 앱이 SQL 데이터베이스의 열 이름과 다른 개체 속성 이름을 가질 수 있습니다. 이 주석을 사용하지 않는 경우 대/소문자 차이 때문에 오류가 발생합니다.</p>
    </div>

2. ToDoItem 클래스에 다음 메서드를 추가하여 새 mCreatedAt 속성을 가져와서 설정합니다.
...
5. Package Explorer에서 ToDoItem.java 파일을 열고 다음 **import** 문을 추가합니다.

		import java.text.DateFormat;

6. GetView 메서드에 다음 코드를 추가합니다.
...
7. `checkBox.setText(currentItem.getText());` 코드를 찾아서 이 코드 줄을 다음과 같이 바꿉니다.

		checkBox.setText(currentItem.getText() + " " + createdAtText);

	항목에 표시할 타임스탬프 날짜가 추가됩니다.
	
6. **실행** 메뉴에서 **실행**을 클릭하여 앱을 시작합니다. 

   	타임스탬프는 삽입 스크립트를 업데이트한 후 삽입된 항목에만 표시됩니다.

7. 기존 **onRegistered** 메서드를 다음 코드로 바꿉니다.
