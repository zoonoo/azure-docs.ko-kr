...
이 자습서는 이전 자습서인 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-android)의 단계 및 샘플 앱을 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [데이터 시작](/en-us/develop/mobile/tutorials/get-started-with-data-android)을 완료해야 합니다.
...
2.  **TodoItem** 클래스의 전용 필드 정의에 다음 코드를 추가합니다.
    ...
    **참고**
    ...
    `SerializedName` 주석은 앱의 새 `mCreatedAt` 속성을 TodoItem 테이블에 정의된 `createdAt` 열(이름이 다름)에 매핑하도록 클라이언트에 지시합니다. 이 주석을 사용하면 앱이 SQL 데이터베이스의 열 이름과 다른 개체 속성 이름을 가질 수 있습니다. 이 주석을 사용하지 않는 경우 대/소문자 차이 때문에 오류가 발생합니다.
...
6.  `checkBox.setText(currentItem.getText());` 코드를 찾아서 이 코드 줄을 다음과 같이 바꿉니다.
...
이 자습서를 완료했으므로 이제 데이터 시리즈의 마지막 자습서인 [페이징을 사용하여 쿼리 구체화](/en-us/develop/mobile/tutorials/add-paging-to-data-android)를 검토해 보십시오.

서버 스크립트는 사용자를 인증할 때 및 푸시 알림을 보내기 위해서도 사용됩니다. 자세한 내용은 다음 자습서를 참조하십시오.

-   [스크립트를 통해 사용자 권한 부여](/en-us/develop/mobile/tutorials/authorize-users-in-scripts-android)
