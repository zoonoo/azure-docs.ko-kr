...
<div class="dev-center-tutorial-selector sublanding"> 
    <a href="/ko-kr/develop/mobile/tutorials/push-notifications-to-users-wp8" title="Windows Phone">Windows Phone</a><a href="/ko-kr/develop/mobile/tutorials/push-notifications-to-users-ios" title="iOS" class="current">iOS</a><a href="/ko-kr/develop/mobile/tutorials/push-notifications-to-users-android" title="Android">Android</a>
</div>

이 항목에서는 APNS(Apple Push Notification Service) 토큰을 저장하는 새 테이블을 추가하여 [이전의 푸시 알림 자습서][이전의 푸시 알림 자습서]를 확장했습니다. 이 토큰을 사용하여 iPhone 또는 iPad 앱 사용자에게 푸시 알림을 보낼 수 있습니다.

이 자습서에서는 앱에서 푸시 알림을 업데이트하는 다음 단계를 단계별로 안내합니다.

1.  [Device 테이블 만들기][Device 테이블 만들기]
2.  [앱 업데이트][앱 업데이트]
...
## <span class="short-header">테이블 만들기</span>새 Devices 테이블 만들기
...
3.  삽입 함수를 다음의 코드로 바꾼 후 **저장**을 클릭합니다.
    ...
    이 스크립트에서 같은 토큰을 사용하여 기존 장치에 대한 **Devices** 테이블을 확인합니다. 일치하는 장치가 없는 경우 삽입만 진행됩니다. 이를 통해 중복 장치 레코드를 예방할 수 있습니다.
...
2.  알림이 수신되는지 확인하고, **확인**을 클릭하여 알림을 해제합니다.

    ![][7]

3.  2단계를 반복하여 앱을 즉시 닫은 후 다음 알림이 표시되는지 확인합니다.
...
-   [모바일 서비스 서버 스크립트 참조][모바일 서비스 서버 스크립트 참조]
  ...
  [데이터 시작하기]: /ko-kr/develop/mobile/tutorials/get-started-with-data-ios
