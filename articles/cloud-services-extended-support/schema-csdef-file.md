---
title: Azure Cloud Services(추가 지원) 정의 스키마(.csdef 파일) | Microsoft Docs
description: Cloud Services(추가 지원)용 정의 스키마와 관련된 정보(csdef)
ms.topic: article
...
---
# <a name="azure-cloud-services-extended-support-definition-schema-csdef-file"></a>Azure Cloud Services(추가 지원) 정의 스키마(.csdef 파일)

서비스 정의 파일은 애플리케이션에 대한 서비스 모델을 정의합니다. 파일은 클라우드 서비스에 사용할 수 있는 역할에 대한 정의가 포함되며, 서비스 엔드포인트를 지정하고, 서비스에 대한 구성 설정을 설정합니다. 구성 설정 값은 [Cloud Service(추가 지원) 구성 스키마](schema-cscfg-file.md)에 설명된 대로 서비스 구성 파일에서 설정됩니다.
...
| upgradeDomainCount      | 선택 사항입니다. 이 서비스의 역할이 할당되는 업그레이드 도메인의 수를 지정합니다. 서비스를 배포할 때 역할 인스턴스가 업그레이드 도메인에 할당됩니다. 자세한 내용은 [클라우드 서비스 역할 또는 배포 업데이트](sample-update-cloud-service.md) 및 [가상 머신의 가용성 관리](../virtual-machines/availability.md)를 참조하세요. 최대 20개의 업그레이드 도메인을 지정할 수 있습니다. 지정하지 않은 경우 업그레이드 도메인의 기본값은 5입니다.|
