<properties 
	pageTitle="SignalR 및 모바일 서비스를 사용하여 실시간 푸시 시작 | 모바일 개발자 센터" 
	description="SignalR 및 모바일 서비스를 사용하여 실시간 푸시를 시작하는 방법에 대해 알아봅니다." 
	documentationCenter="windows" 
	authors="mattchenderson" 
	manager="dwrede" 
	editor="" 
	services=""/>

<tags 
	ms.service="mobile-services" 
	ms.workload="mobile" 
	ms.tgt_pltfrm="mobile-multiple" 
	ms.devlang="multiple" 
	ms.topic="article" 
	ms.date="08/19/2014" 
	ms.author="mahender"/>

# SignalR 및 모바일 서비스를 사용하여 실시간 푸시 시작


>[AZURE.NOTE] 이 콘텐츠는 아직 준비되지 않았지만 알려드리고 싶었습니다. 이 항목에 특히 관심이 있을 경우 [@AzureMobile] 로 트윗하거나 [포럼] 또는 [UserVoice] 에 게시하여 알려주세요.
> **당분간은 동일한 일부 콘텐츠에 대해 설명하는 이 [블로그 게시물] 을 확인해볼 수 있습니다.**



<!-- URLs. -->
[@AzureMobile]: https://twitter.com/AzureMobile
[포럼]: http://social.msdn.microsoft.com/Forums/windowsazure/ko-kr/home?forum=azuremobile
[UserVoice]: http://feedback.azure.com/forums/216254-mobile-services
[블로그 게시물]: http://blogs.msdn.com/b/azuremobile/archive/2014/05/30/realtime-with-signalr-and-azure-mobile-net-backend.aspx

<!--HONumber=42-->
