...
이 단계에서는 Graph API를 사용하여 Azure Active Directory 테넌트에 연결하고 데이터를 읽는 방법을 보여 줍니다. Graph API를 시작하려면 ASP.NET 응용 프로그램인 [Graph API용 쓰기 지원이 포함된 샘플 응용 프로그램](http://code.msdn.microsoft.com/Write-Sample-App-for-79e55502)(영문)을 다운로드하십시오. 이 응용 프로그램에는 Graph API에 대해 쉽게 인증하고 요청하는 도우미 메서드가 포함되어 있습니다.
...
4.  **\$appPrincipal.ObjectId** 변수는 서비스 사용자의 objectId이며, 다음 명령을 입력하고 Enter 키를 눌러 이 값을 가져올 수 있습니다.
    ...
    **참고**
    ...
    2단계에서 서비스 사용자를 만들 때 위의 **AppPrincipalId 값이 반환되었습니다.
