<properties urlDisplayName="Get Started with Data" pageTitle="데이터 시작(Xamarin.Android) - Azure 모바일 서비스" metaKeywords="Azure Xamarin.Android 데이터, Azure 모바일 서비스 데이터" description="Azure 모바일 서비스 Xamarin.Android 앱에서 데이터를 저장하고 액세스하는 방법에 대해 알아봅니다." metaCanonical="" disqusComments="1" umbracoNaviHide="1" title="Get started with data in Mobile Services" documentationCenter="Mobile" authors="donnam" manager="dwrede" services="mobile-services" />
...
<p>이 자습서는 Xamarin.Android 앱에서 모바일 서비스를 통해 Azure를 사용하여 데이터를 저장하고 검색할 수 있는 방법을 더욱 잘 이해할 수 있도록 돕기 위한 것입니다. 이 항목에서는 모바일 서비스 빠른 시작에서 완료한 다수의 단계를 순서대로 안내합니다. 모바일 서비스를 처음 사용하는 경우 먼저 <a href="/ko-kr/develop/mobile/tutorials/get-started-xamarin-android">모바일 서비스 시작</a> 자습서를 완료하는 것이 좋습니다.</p>
</div>
...
3. **TodoActivity** 클래스를 찾아서 엽니다.
   	...
   	모바일 서비스를 사용하여 이 앱을 만들기 위해 진행해야 하는 단계를 지정하는 `// TODO::` 주석이 있습니다.
...
6. 앱에서 _자습서 완료_ 등의 의미 있는 텍스트를 입력하고 **추가**를 클릭합니다.
...
6. 이제 모바일 서비스를 추가합니다. 다음 코드 줄의 주석을 제거합니다.
...
9. **Constants** 클래스에서 다음 멤버 변수의 주석을 제거합니다.
...
11. **OnCreate** 메서드에서 **MobileServiceClient** 변수를 정의하는 다음 코드 줄의 주석을 제거합니다.
...
12. 파일 아래쪽에서 ProgressFilter 클래스를 찾아 주석을 제거합니다. 이 클래스는 MobileServiceClient에서 네트워크 작업을 실행하는 동안 '로드' 표시기를 표시합니다.

13. **checkItem** 메서드에서 다음 줄에 달린 주석을 제거합니다.
...
14. **AddItem** 메서드에서 다음 줄에 달린 주석을 제거합니다.
...
15. **RefreshItemsFromTableAsync** 메서드에서 다음 줄에 달린 주석을 제거합니다.
