...
이 자습서에서는 업데이트 1 이전 소프트웨어 버전을 실행하는 StorSimple 장치에 업데이트 1을 설치하는 방법에 대해 설명합니다. 장치는 일반적으로 사용 가능한(GA) 릴리스, 업데이트 0.1, 업데이트 0.2 또는 업데이트 0.3 소프트웨어를 실행 중일 수 있습니다. 또한 자습서에서는 StorSimple 장치에서 데이터 0 이외의 다른 네트워크 인터페이스에 게이트웨이를 구성하는 경우 수행할 작업을 설명합니다.

이 설치 중 장치가 업데이트 1.0 이전 버전을 실행 중인 경우 장치에서 검사가 수행됩니다. 이러한 검사는 하드웨어 상태 및 네트워크 연결 측면에서 장치 상태를 확인합니다.
...
## 비데이터 0 네트워크 인터페이스에는 게이트웨이 장치에 업데이트 1을 설치합니다. 

이 절차는 업데이트 1.0 이전의 소프트웨어 버전을 실행 중이고 데이터 0 이외의 다른 네트워크 인터페이스에 게이트웨이가 설정된 StorSimple 장치에 적용됩니다.
 
장치에 비데이터 0 네트워크 인터페이스에 대한 게이트웨이가 없는 경우, 관리 포털에서 직접 장치를 업데이트할 수 있습니다. [관리 포털을 사용하여 업데이트 1 설치](#use-the-management-portal-to-install-update-1)를 참조하세요.
 
> [AZURE.NOTE]이 절차는 업데이트 1.0에만 적용하도록 한 번만 수행해야 됩니다. Azure 관리 포털을 사용하여 후속 업데이트를 적용할 수 있습니다.
 
장치가 업데이트 1.0 이전 소프트웨어를 실행하고 데이터 0 이외의 다른 네트워크 인터페이스에 대해 게이트웨이가 설정된 경우, 다음 두가지 방법으로 업데이트 1.0을 적용할 수 있습니다.

- **옵션 1**: 업데이트를 다운로드하고 장치의 Windows PowerShell 인터페이스에서 [Start-HcsHotfix](https://technet.microsoft.com/library/dn688134.aspx) cmdlet를 사용하여 적용합니다. 이것이 권장된 방법입니다.

- **옵션 2**: 관리 포털에서 직접 업데이트를 설치합니다.
 
각각에 대한 자세한 지침은 다음 섹션에 제공됩니다.

### 옵션 1: StorSimple용 Windows PowerShell을 사용하여 업데이트 1 적용

이 절차를 사용하기 전에 업데이트를 적용하려면 다음을 확인하세요.

- 두 장치 컨트롤러가 온라인 상태가 됩니다.

- 데이터 2 및 데이터 3이 비활성화됩니다. 장치에서 GA 릴리스를 실행 중인 경우에만 이 작업을 수행해야 합니다. 업데이트 0.2 및 0.3을 실행하는 장치에서는 사용하지 않도록 설정하지 않아도 됩니다. 업데이트가 완료되면 이 네트워크 인터페이스를 다시 설정할 수 있습니다.
 
다음 단계를 수행하여 업데이트 1.0을 적용합니다. 업데이트를 완료하는데 몇 시간이 걸릴 수 있습니다.

[AZURE.INCLUDE [storsimple-install-update-option1](../includes/storsimple-install-update-option1.md)]

### 옵션 2: Azure 관리 포털을 사용하여 업데이트 1 적용
...
게이트웨이 설정의 선택을 취소한 다음 업데이트를 적용하려면 다음 단계를 수행합니다.
