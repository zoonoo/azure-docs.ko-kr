...
[Blitline 정의][Blitline 정의]
[Blitline이 수행할 수 없는 작업][Blitline이 수행할 수 없는 작업]
[Blitline 계정 만들기][Blitline 계정 만들기]
[Blitline 작업을 만드는 방법][Blitline 작업을 만드는 방법]
[Azure 저장소에 이미지를 저장하는 방법][Azure 저장소에 이미지를 저장하는 방법]
[다음 단계][다음 단계]

## <span id="whatis"></span></a>Blitline 정의
...
자세한 내용은 [][]<http://www.blitline.com></a>(영문)을 참조하십시오.

## <span id="whatisnot"></span></a>Blitline이 수행할 수 없는 작업
...
## <span id="createaccount"></span></a>Blitline 계정 만들기

[WACOM.INCLUDE [blitline-signup](../includes/blitline-signup.md)]

## <span id="createjob"></span></a>Blitline 작업을 만드는 방법
...
JSON을 가졌으면 `http://api.blitline.com/jobs`에 **게시**하기만 하면 됩니다.
...
## <span id="saveazure"></span></a>Azure 저장소 계정에 이미지를 저장하는 방법
...
CAPITALIZED 값을 자체 값으로 채우면 이 JSON을 <http://api.blitline.com/job>에 제출할 수 있으며 "src" 이미지가 흐리게 하는 필터로 처리된 다음 Azure 대상에 푸시됩니다.
...
Blitline의 Azure 저장소 문서의 최신 버전을 [여기][여기]에서 읽을 수도 있습니다.
