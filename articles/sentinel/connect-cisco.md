---
title: Azure Sentinel 미리 보기에서 Cisco 데이터 수집 | Microsoft Docs
description: Azure Sentinel에서 Cisco 데이터를 수집 하는 방법에 알아봅니다.
services: sentinel
...
전용된 컴퓨터에 에이전트를 배포 해야 Azure Sentinel Cisco ASA 어플라이언스에서 연결 하려면 (VM 또는 온-프레미스) 어플라이언스 및 Azure Sentinel 간의 통신을 지원 하도록 합니다. 자동 또는 수동으로 에이전트를 배포할 수 있습니다. 자동 배포만 전용된 컴퓨터에 Azure에서 만드는 새 VM 인 경우 제공 됩니다. 
...
Syslog 에이전트를 통해 Azure 작업 영역에 대 한 Syslog 메시지를 전달 하는 Cisco ASA를 구성 합니다.
...
1. 오른쪽 포트 Syslog 에이전트에 로그를 가져오고 있는지 확인 합니다. 이 명령은 Syslog 에이전트 컴퓨터를 실행 합니다. `tcpdump -A -ni any  port 514 -vv` 이 명령은 Syslog 컴퓨터에 장치에서 스트림 로그를 보여 줍니다. 이 명령은 Syslog 컴퓨터에 장치에서 스트리밍하는 로그를 보여 줍니다. 로그 원본 어플라이언스 오른쪽 시설에 올바른 포트에서에서 수신 되는 있는지 확인 합니다.
...
이 문서에서는 Azure Sentinel Cisco ASA 어플라이언스에 연결 하는 방법을 알아보았습니다. Azure에서 감시 하는 방법에 대 한 자세한 내용은 다음 문서를 참조 합니다.
