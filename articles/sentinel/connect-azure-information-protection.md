---
title: Azure 센티널 Preview에 Azure Information Protection 데이터 연결 | Microsoft Docs
...
- 다음은 테 넌 트에 대 한 Azure AD 관리자 역할 중 하나입니다. 관리자, 보안 관리자 또는 전역 관리자를 Azure Information Protection 합니다.
    
    > [!NOTE]
    > 테 넌 트가 [통합 레이블 지정 플랫폼](https://docs.microsoft.com/azure/information-protection/faqs#how-can-i-determine-if-my-tenant-is-on-the-unified-labeling-platform)에 있는 경우 Azure Information Protection 관리자 역할을 사용할 수 없습니다.

- 센티널 및 Azure Information Protection에 사용 중인 Log Analytics 작업 영역에 대 한 읽기 및 쓰기 권한입니다.
...
1. Azure 센티널에서 **데이터 커넥터**를 선택한 다음 **Azure Information Protection**합니다.

2. **Azure Information Protection** 블레이드에서 **커넥터 페이지 열기**를 선택 합니다.

3. 다음 블레이드의 **구성** 섹션에서 **Azure Information Protection** 을 선택 하 여 **Azure Information Protection analytics**로 이동 합니다.

4. 사용 가능한 작업 영역 목록에서 현재 Azure 센티널에 사용 중인 작업 영역을 선택 합니다. 다른 작업 영역을 선택 하는 경우 Azure Information Protection의 보고 데이터를 Azure 센티널에서 사용할 수 없습니다.

5. 작업 영역을 선택한 경우 **확인** 을 선택 하면 커넥터 **상태가** **연결 됨**으로 변경 됩니다.

6. Azure Information Protection의 보고 데이터는 선택한 작업 영역 내의 **InformationProtectionLogs_CL** 테이블에 저장 됩니다. 
