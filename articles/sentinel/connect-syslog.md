...
5. **고급 설정** 블레이드에서 **데이터** > **Syslog**를 선택 합니다. 그런 다음 수집할 커넥터에 대 한 기능을 추가 합니다.
    ...
    Syslog 어플라이언스에 포함 된 기능을 로그 헤더에 추가 합니다. Syslog 어플라이언스에서 `/etc/rsyslog.d/security-config-omsagent.conf` 폴더의 **syslog-d** 및의 **r-Syslog** `/etc/syslog-ng/security-config-omsagent.conf`에서이 구성을 볼 수 있습니다.
...
1. 이전 절차의 5 단계에는 **auth** 및 **authpriv** 가 모두 모니터링할 기능으로 선택 되어 있는지 확인 합니다. 심각도 옵션의 기본 설정은 모두 선택 된 상태로 유지 합니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
