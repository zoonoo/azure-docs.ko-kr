---
title: Azure 센티널 Preview의 구하기 기능 | Microsoft Docs
...
ms.locfileid: "68780455"
---
# <a name="hunt-for-threats-with-in-azure-sentinel-preview"></a>Azure 센티널 Preview에서 위협에 대 한 헌트
...
예를 들어 기본 제공 쿼리 중 하나는 인프라에서 실행 되는 가장 일반적인 프로세스에 대 한 데이터를 제공 하는 것입니다. 실행 될 때마다 경고를 표시 하지 않는 것이 좋습니다. 그렇지 않은 경우에는 쿼리를 살펴보세요. ere은 거의 없습니다. 



Azure 센티널 구하기를 사용 하면 다음과 같은 기능을 활용할 수 있습니다.

- 기본 제공 쿼리: 시작 하기 위해 시작 페이지는 미리 로드 된 쿼리 예제를 제공 하 여 시작 하 고 테이블 및 쿼리 언어를 익히는 데 도움이 됩니다. 이러한 기본 제공 검색 쿼리는 지속적으로 Microsoft 보안 연구원에 의해 개발 되 고, 새 쿼리를 추가 하 고, 기존 쿼리를 미세 조정 하 여 새 검색을 찾고, 검색을 시작할 위치를 파악 하는 진입점을 제공 합니다. 새 공격의 처음입니다. 

- IntelliSense를 사용 하는 강력한 쿼리 언어: 쿼리 언어를 기반으로 하 여 다음 수준으로 이동 해야 하는 유연성을 제공 합니다.

- 사용자 고유의 책갈피를 만듭니다. 검색 프로세스 중에는 비정상적인 또는 의심 스러운 결과, 대시보드 또는 활동에 걸쳐 있을 수 있습니다. 이러한 항목을 표시 하기 위해 나중에 다시 돌아올 수 있도록 책갈피 기능을 사용 합니다. 책갈피를 사용 하 여 나중에 조사를 위해 인시던트를 만드는 데 사용할 항목을 저장할 수 있습니다. 책갈피에 대 한 자세한 내용은 사용 [사냥에서 책갈피]를 참조 하세요.

- 전자 필기장을 사용 하 여 조사 자동화: 노트북은 조사 및 사냥 단계를 안내 하기 위해 빌드할 수 있는 단계별 플레이 북과 비슷합니다.  노트북은 조직의 다른 사용자와 공유할 수 있는 재사용 가능한 플레이 북의 모든 구하기 단계를 캡슐화 합니다. 
- 저장 된 데이터를 쿼리 합니다. 쿼리를 위해 테이블에서 데이터에 액세스할 수 있습니다. 예를 들어 프로세스 만들기, DNS 이벤트 및 기타 많은 이벤트 유형을 쿼리할 수 있습니다.
...
Azure 센티널의 구하기는 Azure Log Analytics 쿼리 언어를 기반으로 합니다. 쿼리 언어 및 지원 되는 연산자에 대 한 자세한 내용은 [쿼리 언어 참조](https://docs.loganalytics.io/docs/Language-Reference/)를 참조 하세요.
...
일반적인 쿼리는 테이블 이름 다음에로 \|구분 된 일련의 연산자를 사용 하 여 시작 합니다.
