...
지원 되는 시나리오에 대 한 경고에 대 한 자세한 내용은이 페이지의 [다단계 공격 감지에 대해 지원 되는 시나리오](#scenarios-supported-for-advanced-multistage-attack-detection) 섹션을 참조 하세요.
...
5. **규칙 만들기 마법사** 블레이드에서 상태 변경이 자동으로 선택 되므로 다음을 선택 **합니다. 을**검토 하 고 **저장**합니다. 

규칙 템플릿은 advanced 다단계 공격 감지에는 적용 되지 않습니다.

## <a name="scenarios-supported-for-advanced-multistage-attack-detection"></a>Advanced 다단계 공격 검색에 대해 지원 되는 시나리오
...
- **의심 스러운 클라우드 앱 관리 활동으로 이어지는 불규칙 한 위치로 이동 불가능**
    ...
    이 경고 \<는 > *계정*이름별로 로그인 이벤트를 표시 \< *> 하는*것입니다.
...
- **Office 365 가장으로 이어지는 익숙하지 않은 위치의 로그인 이벤트**
    ...
    그런 \<다음 계정 *계정 이름은*단일 세션에서 > \< *여러 계정에 대해*가장 >.
...
이제 고급 다단계 공격 감지에 대해 자세히 알아보았습니다. 다음 빠른 시작을 통해 데이터와 잠재적 위협을 파악 하는 방법을 배울 수 있습니다. [Azure 센티널을 시작](quickstart-get-visibility.md)하세요.

사용자를 위해 생성 된 인시던트를 조사할 준비가 된 경우 다음 자습서를 참조 하세요. [Azure 센티널을 사용 하 여 인시던트를 조사](tutorial-investigate-cases.md)합니다.
