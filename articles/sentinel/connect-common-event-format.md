...
Azure 센티널과 CEF 어플라이언스 간의 연결은 세 단계로 이루어집니다.

1. 어플라이언스에서이 값을 설정 해야 합니다. 그러면 어플라이언스에서 Microsoft Monitoring Agent에 따라 필요한 형식으로 필요한 로그를 Azure 센티널 Syslog 에이전트로 전송 합니다. 이러한 매개 변수는 Azure 센티널 에이전트의 Syslog 디먼 에서도 수정할 수 있는 경우 어플라이언스에서 수정할 수 있습니다.
    - 프로토콜 = UDP
    - Port = 514
    - 기능 = Local4
    - 형식 = CEF
2. Syslog 에이전트는 데이터를 수집 하 고 Log Analytics에 안전 하 게 전송 하 여 구문 분석 되 고 보강 됩니다.
3. 에이전트는 Log Analytics 작업 영역에 데이터를 저장 하므로 분석, 상관 관계 규칙 및 대시보드를 사용 하 여 필요에 따라 쿼리할 수 있습니다.

> [!NOTE]
> 에이전트는 여러 원본에서 로그를 수집할 수 있지만 전용 컴퓨터에 설치 해야 합니다.
...
## <a name="step-1-configure-your-syslog-vm"></a>1단계: Syslog VM 구성

어플라이언스와 Azure 센티널 간의 통신을 지원 하려면 전용 Linux 컴퓨터 (VM 또는 온-프레미스)에 에이전트를 배포 해야 합니다. 
...
1. CEF 커넥터 화면의 **Syslog 구성 및 전달**에서 syslog 데몬이 **rsyslog. d** 또는 **syslog**를 사용할지 여부를 설정 합니다. 
1. 다음 명령을 복사 하 여 어플라이언스에서 실행 합니다.
    - Rsyslog. d를 선택한 경우:
       ...
       1. Syslog 데몬을 다시 시작 합니다.`sudo service rsyslog restart`<br> 자세한 내용은 [rsyslog 설명서](https://www.rsyslog.com/doc/v8-stable/tutorials/tls_cert_summary.html) 를 참조 하세요.
    ...
    - Syslog를 선택한 경우:
       ...
       2. 25226 포트에서 수신 하도록 Syslog 에이전트를 구성 하는 [security_events config 파일](https://aka.ms/asi-syslog-config-file-linux) 을 다운로드 하 여 설치 합니다. `sudo wget -O /etc/opt/microsoft/omsagent/{0}/conf/omsagent.d/security_events.conf "https://aka.ms/syslog-config-file-linux"`여기서 {0} 는 작업 영역 GUID로 바꾸어야 합니다.
        ...
        3. Syslog 데몬을 다시 시작 합니다.`sudo service syslog-ng restart` <br>자세한 내용은 [syslog 기능 설명서](https://www.syslog-ng.com/technical-documents/doc/syslog-ng-open-source-edition/3.16/mutual-authentication-using-tls/2) 를 참조 하세요.
...
CEF 이벤트의 Log Analytics에서 관련 스키마를 사용 하려면를 검색 `CommonSecurityLog`합니다.

## <a name="step-2-forward-common-event-format-cef-logs-to-syslog-agent"></a>2단계: CEF (Common Event Format) 로그를 Syslog 에이전트로 전달

CEF 형식의 Syslog 메시지를 Syslog 에이전트로 보내도록 보안 솔루션을 설정 합니다. 에이전트 구성에 표시 되는 것과 동일한 매개 변수를 사용 해야 합니다. 일반적으로 다음과 같습니다.
