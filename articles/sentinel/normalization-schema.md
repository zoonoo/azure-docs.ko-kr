...
| **사용자** | 문자열 | 다음 3 개의 사용자 필드를 사용할 수 있습니다.<ul><li>사용자 이름</li><li>사용자 UPN</li><li>사용자 도메인</li></ul> |
| **사용자 ID** | 문자열 | 현재 지원 되는 사용자 Id의 두 가지는 다음과 같습니다.<ul><li>사용자 SID</li><li>Azure Active directory ID</li></ul> |
| **디바이스** | 문자열 | 다음 3 개의 장치/호스트 열이 지원 됩니다.<ul><li>ID</li><li>속성</li><li>FQDN (정규화 된 도메인 이름)</li></ul> |
| **국가** | 문자열 | 이 우선 순위에 따라 ISO 3166-1를 사용 하는 문자열:<ul><li>알파 2 코드 (예: 미국)</li><li>알파-3 코드 (예: 미국)</li><li>짧은 이름</li></ul> |
| **지역** | 문자열 | ISO 3166-2를 사용 하는 국가 하위 부서 이름 |
| **도시** | 문자열 | |
| **경도** | Double | ISO 6709 좌표 표현 (부호 있는 10 진수) |
| **위도** | Double | ISO 6709 좌표 표현 (부호 있는 10 진수) |
| **해시 알고리즘** | 문자열 | 다음 4 개의 해시 열이 지원 됩니다.<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **파일 형식** | 문자열 | 파일 형식의 형식입니다.<ul><li>내선 번호</li><li>클래스</li><li>NamedType</li></ul> |
| 
...
|-|-|-|-|-|
| EventType | 문자열 | 트래픽 | 수집 되는 이벤트의 유형입니다. | Event |
| EventSubType 형식 | 문자열 | 인증 | 해당 하는 경우 형식의 추가 설명 | Event |
| EventCount | 정수  | 10 | 집계 된 이벤트의 수입니다 (해당 하는 경우). | Event |
| EventEndTime | 날짜/시간 | "데이터 형식"을 참조 하세요. | 이벤트가 종료 된 시간입니다. | Event |
| EventMessage | 문자열 |  액세스 거부 | 에 포함 되었거나 레코드에서 생성 된 일반 메시지 또는 설명입니다. | Event |
| DvcIpAddr | IP 주소 |  23.21.23.34 | 레코드를 생성 하는 장치의 IP 주소입니다. | 디바이스<br>IP |
| DvcMacAddr | 문자열 | 06:10:9f: eb: 8f: 14 | 이벤트를 보낸 보고 장치의 네트워크 인터페이스에 대 한 MAC 주소입니다. | 디바이스<br>Mac |
| DvcHostname | 장치 이름 (문자열) | syslogserver1.contoso.com | 메시지를 생성 하는 장치의 장치 이름입니다. | 디바이스 |
| EventProduct | 문자열 | OfficeSharepoint | 이벤트를 생성 하는 제품입니다. | Event |
| EventProductVersion | 문자열 | 9.0 |  이벤트를 생성 하는 제품의 버전입니다. | Event |
| EventResourceId | 장치 ID (문자열) | /subscriptions/3c1bb38c-82e3-4f8d-a115-a7110ba70d05/resourcegroups/contoso77/providers/microsoft.compute/virtualmachines/syslogserver1 | 메시지를 생성 하는 장치의 리소스 ID입니다. | Event |
| EventReportUrl | 문자열 | https://192.168.1.1/repoerts/ae3-56.htm | 보고 장치에서 만든 전체 보고서에 대 한 링크 | Event |
| EventVendor | 문자열 | Microsoft | 이벤트를 생성 하는 제품의 공급 업체입니다. | Event |
| EventResult | 다중값: Success, Partial, Failure, [Empty] (String) | 성공 | 작업에 대해 보고 된 결과입니다. 해당 하지 않는 경우 빈 값입니다. | Event |
| EventResultDetails | 문자열 | 잘못 된 암호 | EventResult에 보고 된 결과의 이유 또는 세부 정보 | Event |
| EventSchemaVersion | Real | 0.1 | Azure 센티널 스키마 버전입니다. 현재 0.1입니다. | Event |
| EventSeverity | 문자열 | 낮음 | 보고 된 작업이 보안에 영향을 주는 경우는 영향의 심각도를 나타냅니다. | Event |
| EventOriginalUid | 문자열 | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | 보고 장치의 레코드 ID입니다. | Event |
| EventStartTime | 날짜/시간 | "데이터 형식"을 참조 하세요. | 이벤트가 명시 된 시간 | Event |
| TimeGenerated | 날짜/시간 | "데이터 형식"을 참조 하세요. | 보고 원본에서 보고 한 이벤트가 발생 한 시간입니다. | 사용자 지정 필드 |
| EventTimeIngested | 날짜/시간 | "데이터 형식"을 참조 하세요. | 이벤트가 Azure 센티널로 수집 된 시간입니다. Azure 센티널에서 추가 됩니다. | Event |
| EventUid | Guid (문자열) | 516a64e3-8360-4f1e-a67c-d96b3d52df54 | 센티널에서 행을 표시 하는 데 사용 하는 고유 식별자입니다. | Event |
| NetworkApplicationProtocol | 문자열 | HTTPS | 연결 또는 세션에서 사용 하는 응용 프로그램 계층 프로토콜입니다. | 네트워크 |
...
| Dstgeoc | 문자열 | 벌링턴 | 대상 IP 주소와 연결 된 구/군/시입니다. | 대상이<br>지역 |
...
| FileHashMd5 | 문자열 | 51BC68715FC7C109DCEA406B42D9D78F | 프로토콜에 대 한 네트워크 연결을 통해 전송 되는 파일의 MD5 해시 값입니다. | 파일 |
