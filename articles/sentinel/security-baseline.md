...
이 보안 기준은 [Azure Security 벤치 마크 버전 1.0](../security/benchmarks/overview-v1.md) 의 지침을 azure 센티널에 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다.
콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤과** azure 센티널에 적용 되는 관련 지침에 따라 그룹화 됩니다. Azure 센티널에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

 
Azure 센티널이 Azure Security 벤치 마크에 완전히 매핑되는 방식을 보려면 [전체 Azure 센티널 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
- [로그 보존 매개 변수를 설정 하는 방법](https://docs.microsoft.com/azure/azure-monitor/logs/manage-cost-storage#change-the-data-retention-period)
...
구독에 대 한 Azure Portal에서 Id 및 액세스 제어 창을 사용 하 여 azure RBAC (역할 기반 액세스 제어)를 구성할 수 있습니다. 역할은 Azure Active Directory (Azure AD)의 사용자, 그룹, 서비스 사용자 및 관리 되는 id에 적용 됩니다. 또한 azure 센티널은 azure RBAC를 사용 하 여 azure의 사용자, 그룹 및 서비스에 할당할 수 있는 Azure 센티널 참가자와 같은 기본 제공 관리 역할을 제공 합니다. 
...
**지침**: Azure 센티널 작업 영역 (예: Azure Portal)에 대 한 제어 평면 액세스는 REST API를 통해 제공 되며 SSO를 지원 합니다. 인증 하려면 요청에 대 한 인증 헤더를 Azure Active Directory (Azure AD)에서 가져오는 JSON Web Token 설정 합니다.
...
**지침**: azure AD 다단계 인증을 사용 하는 PAW (권한 있는 액세스 워크스테이션)를 사용 하 여 azure 센티널 관련 리소스에 로그인 하 고 구성 합니다. 
...
**지침**: azure 센티널 작업 영역에 대 한 중앙 인증 및 권한 부여 시스템으로 Azure Active Directory (azure AD)를 사용 합니다. Azure AD는 강력한 암호화를 저장 데이터 및 전송 중 데이터에 사용하여 데이터를 보호합니다. 또한 Azure AD는 사용자 자격 증명을 솔트하고, 해시하고, 안전하게 저장합니다.

Azure AD 로그인 활동, 감사 및 위험 이벤트 로그 원본에 액세스할 수 있으며,이를 통해 Azure 센티널 또는 타사 시스템 정보 및 이벤트 관리 솔루션과 통합할 수 있습니다.
...
**지침**: 필요에 따라 개발, 테스트 및 프로덕션 센티널 작업 영역에 대 한 관리 그룹을 포함할 별도의 구독을 구현 합니다.
...
- [Azure Monitor 고객 관리 키 개요](https://docs.microsoft.com/azure/azure-monitor/logs/customer-managed-keys#customer-managed-key-overview) 
...
**지침**: 기본 제공 Azure Policy 정의 및 "OperationalInsights" 네임 스페이스의 별칭 Azure Policy 사용 하 여 시스템 구성을 경고, 감사 및 적용 하기 위한 사용자 지정 정책을 만듭니다. Azure 리소스에 대 한 구성을 자동으로 적용 하려면 Azure 정책 [감사], [거부] 및 [없는 경우 배포] 효과를 사용 합니다.
