...
이 항목에서는 서버 스크립트를 사용하여 인증된 사용자가 iOS 앱에서 Azure 모바일 서비스의 데이터에 액세스할 수 있게 권한을 부여하는 방법을 보여 줍니다.  이 자습서에서는 모바일 서비스에 스크립트를 등록하여 인증된 사용자의 사용자 ID에 따라 쿼리를 필터링함으로써 각 사용자가 자신의 데이터만 볼 수 있게 합니다.
