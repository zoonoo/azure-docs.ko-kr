...
| 장치 업데이트 | OTA 업데이트를 수행 하면 업데이트 그룹이 손실 됩니다. | 다음 [지침](https://docs.microsoft.com/azure/azure-percept/how-to-update-over-the-air#create-a-device-update-group)에 따라 장치의 태그를 업데이트 합니다. |
| 개발자 도구 팩 설치 관리자 | Docker가 시스템에서 제대로 실행 되지 않는 경우 선택적 Caffe 설치가 실패할 수 있습니다. | Docker가 설치 되어 실행 중인지 확인 한 다음 Caffe 설치를 다시 시도 합니다. |
| 개발자 도구 팩 설치 관리자 | 호환 되지 않는 시스템에서 선택적으로 설치 되지 않습니다. | 설치 관리자를 실행 하기 전에이 시스템 호환성을 확인 합니다. |
