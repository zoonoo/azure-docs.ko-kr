...
이 자습서에서는 IoT Hub에 대한 디바이스 업데이트를 통해 엔드투엔드 패키지 기반 업데이트를 완료하는 단계를 안내합니다. 이 자습서에서는 Azure IoT Edge 및 디바이스 업데이트 패키지 에이전트를 실행하는 Ubuntu Server 18.04 x64를 사용합니다. 자습서에서 샘플 패키지를 업데이트하는 방법을 보여 주지만, 비슷한 단계를 사용하여 Azure IoT Edge 또는 사용하는 컨테이너 엔진과 같은 다른 패키지를 업데이트할 수 있습니다.
...
* IoT Hub에 연결된 디바이스 업데이트 인스턴스 및 계정
  * 이전에 수행하지 않은 경우 지침에 따라 [디바이스 업데이트 계정을 만들고 연결](create-device-update-account.md)합니다.
...
편의를 위해 이 자습서에서는 [cloud-init](../virtual-machines/linux/using-cloud-init.md) 기반 [Azure Resource Manager 템플릿](../azure-resource-manager/templates/overview.md)을 사용하여 Ubuntu 18.04 LTS 가상 머신을 빠르게 설정할 수 있습니다. Azure IoT Edge 런타임과 디바이스 업데이트 패키지 에이전트를 모두 설치한 다음, 사용자가 제공하는 IoT Edge 디바이스(필수 구성 요소)에 대한 디바이스 연결 문자열을 사용하여 프로비저닝 정보를 사용하여 디바이스를 자동으로 구성합니다. 이렇게 하면 설정을 완료하기 위해 SSH 세션을 시작할 필요가 없습니다.

1. 시작하려면 아래 단추를 클릭합니다.

   [![iotedge-vm-deploy를 위한 Azure에 배포하기 단추](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2Fazure%2Fiotedge-vm-deploy%2F1.2.0-rc4%2FedgeDeploy.json)
...
수동으로 디바이스를 설치하고 구성하는 다음 단계는 이 [cloud-init 스크립트](https://github.com/Azure/iotedge-vm-deploy/blob/1.2.0-rc4/cloud-init.txt)에서 자동화된 단계와 동일합니다. 이러한 단계는 물리적 디바이스를 준비하는 데 사용할 수 있습니다.
...
2. 왼쪽 탐색 창의 'IoT Edge'에서 IoT Edge 디바이스를 찾고 [디바이스 쌍]으로 이동합니다.
