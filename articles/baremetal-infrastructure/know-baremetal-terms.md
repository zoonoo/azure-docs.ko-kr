...
ms.locfileid: "97829154"
---
# <a name="know-the-terms-for-baremetal-infrastructure"></a>BareMetal 인프라에 대 한 용어 파악

이 문서에서는 몇 가지 중요 한 BareMetal 용어를 다룹니다.

- **수정 버전**: BareMetal 인스턴스 스탬프에 대 한 두 가지 스탬프 수정 버전이 있습니다. 각 버전은 Azure 가상 컴퓨터 호스트와 아키텍처 및 근접 하 게 다릅니다.
    - **수정 버전 3** (Rev 3): 원래 디자인입니다.
    - **수정 버전 4** (Rev 4):은 azure vm (가상 머신) 호스트에 더 근접 하 게 근접 하 고 azure Vm과 BareMetal 인스턴스 단위 간의 대기 시간을 낮추는 새로운 디자인입니다. 
    - **수정 버전 4.2** (rev 4.2):은 기존 수정 버전 4 아키텍처를 사용 하는 최신 리브랜딩 BareMetal 인프라입니다. Azure Portal를 통해 BareMetal 인스턴스에 액세스 하 고 관리할 수 있습니다.  
...
[Azure Portal](workloads/sap/baremetal-infrastructure-portal.md)를 통해 BareMetal 인스턴스 단위를 식별 하 고 상호 작용 하는 방법을 알아봅니다.
