<properties
   pageTitle="Visual Studio에서 Azure 클라우드 서비스에 대한 역할 구성 방법 | Microsoft Azure"
...
클라우드 서비스의 성능을 개선하기 위해 실행 중인 역할의 인스턴스 수를 사용자 수 또는 특정 역할에 대해 예상되는 부하에 따라 변경할 수 있습니다. Azure에서 클라우드 서비스를 실행하는 경우 별도의 가상 컴퓨터가 역할의 각 인스턴스에 대해 생성됩니다. 이 클라우드 서비스의 배포에 대한 대금 청구에 영향을 줍니다. 대금 청구에 대한 자세한 내용은 [Azure 청구서 이해](billing-understand-your-bill.md)를 참조하세요.
...
1. 사용자 지정 자격 증명을 사용하려면 **수동으로 입력한 자격 증명** 옵션 단추를 선택합니다. 저장소 계정 이름과 기본 또는 보조 키를 입력합니다. 저장소 계정을 만드는 방법 및 **저장소 연결 문자열 만들기** 대화 상자에 저장소 계정에 대한 세부 정보를 입력하는 방법에 대한 자세한 내용은 [Visual Studio에서 클라우드 서비스를 게시하는 데 필요한 서비스 설정](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md)을 참조하세요.
...
1. 로컬 연결 문자열에 대한 저장소 계정을 사용하려면 **수동으로 입력한 자격 증명** 옵션을 선택합니다.
    ...
    저장소 계정을 만드는 방법 및 저장소 연결 문자열 만들기 대화 상자에 저장소 계정에 대한 세부 정보를 입력하는 방법에 대한 자세한 내용은 [Visual Studio에서 클라우드 서비스를 게시하는 데 필요한 서비스 설정](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md)을 참조하세요.
...
각 역할에 대한 가상 컴퓨터 크기를 설정할 수 있습니다. 모든 서비스 구성에 대한 이 크기만 설정할 수 있습니다. 작은 컴퓨터 크기를 선택하는 경우 적은 CPU 코어, 메모리 및 로컬 디스크 저장소가 할당됩니다. 할당된 대역폭도 작아집니다. 이러한 크기 및 할당된 리소스에 대한 자세한 내용은 [클라우드 서비스에 적합한 크기 구성](https://msdn.microsoft.com/library/azure/ee814754)을 참조하세요.

Azure에서 각 가상 컴퓨터에 필요한 리소스는 Azure에서 클라우드 서비스를 실행하는 비용에 영향을 줍니다. Azure 청구서에 대한 자세한 내용은 [Azure 청구서 이해](billing-understand-your-bill.md)를 참조하세요.
