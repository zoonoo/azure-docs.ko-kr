...
이 가이드에서는 Azure 모바일 서비스용 Android 클라이언트를 사용하여 일반적인 시나리오를 수행하는 방법을 보여 줍니다. 여기서 다루는 시나리오에는 데이터 쿼리, 삽입, 업데이트, 삭제 및 사용자 인증, 오류 처리 및 클라이언트 사용자 지정이 포함됩니다. 모바일 서비스를 처음 접하는 경우 먼저 [모바일 서비스 빠른 시작][모바일 서비스 빠른 시작]을 완료하는 것이 좋습니다. 빠른 시작 자습서를 참조하여 계정을 구성하고 첫 모바일 서비스를 만들 수 있습니다.

샘플은 Java로 작성되었으며 [모바일 서비스 SDK][모바일 서비스 SDK]가 필요합니다. 이 자습서에는 Eclipse IDE(통합 개발 환경) 및 ADT(Android 개발자 도구) 플러그 인이 포함되어 있는 [Android SDK][Android SDK](영문)도 필요합니다. 모바일 서비스 SDK는 Android 버전 2.2 이상을 지원하지만, Android 버전 4.2 이상에 대해 빌드하는 것이 좋습니다.

## 목차

-   [모바일 서비스 정의][모바일 서비스 정의]
-   [개념][개념]
-   [설정 및 필수 조건][설정 및 필수 조건]
...
-   [방법: 모바일 서비스에서 데이터 쿼리][방법: 모바일 서비스에서 데이터 쿼리]
    ...
    -   [특정 열 선택][특정 열 선택]
    -   [방법: 쿼리 메서드 연결][방법: 쿼리 메서드 연결]
-   [방법: 모바일 서비스에 데이터 삽입][방법: 모바일 서비스에 데이터 삽입]
-   [방법: 모바일 서비스의 데이터 업데이트][방법: 모바일 서비스의 데이터 업데이트]
-   [방법: 모바일 서비스의 데이터 삭제][방법: 모바일 서비스의 데이터 삭제]
-   [방법: 특정 항목 조회][방법: 특정 항목 조회]
-   [방법: 형식화되지 않은 데이터 작업][방법: 형식화되지 않은 데이터 작업]
-   [방법: 사용자 인터페이스에 데이터 바인딩][방법: 사용자 인터페이스에 데이터 바인딩]

    -   [방법: 레이아웃 정의][방법: 레이아웃 정의]
    -   [방법: 어댑터 정의][방법: 어댑터 정의]
    -   [방법: 어댑터 사용][방법: 어댑터 사용]
-   [방법: 사용자 인증][방법: 사용자 인증]
    ...
    -   [인증 토큰 캐시][인증 토큰 캐시]
-   [방법: 오류 처리][방법: 오류 처리]
-   [방법: 클라이언트 사용자 지정][방법: 클라이언트 사용자 지정]

    -   [요청 헤더 사용자 지정][요청 헤더 사용자 지정]
...
이미 모바일 서비스 및 테이블을 만들었다고 가정합니다. 자세한 내용은 [테이블 만들기][테이블 만들기]를 참조하십시오. 이 항목에서 사용되는 코드에서 테이블 이름이 *ToDoItem*이고 테이블에 다음과 같은 열이 있다고 가정합니다.
...
다음 코드는 모바일 서비스에 액세스하는 데 사용되는 [MobileServiceClient][MobileServiceClient] 개체를 만듭니다.
...
모바일 서비스에서 데이터를 쿼리하거나 수정하는 가장 쉬운 방법은 *형식화된 프로그래밍 모델*을 사용하는 것입니다. Java는 강력한 형식의 언어이기 때문입니다(*형식화되지 않은* 모델에 대해서는 나중에 다룸). 이 모델에서는 클라이언트와 모바일 서비스 간에 데이터를 전송할 때 [gson][gson] 라이브러리를 사용하여 JSON에서 원활한 serialization 및 역직렬화가 가능합니다. 개발자는 아무 것도 할 필요가 없으며, 프레임워크에서 모든 것이 처리됩니다.

데이터를 쿼리하거나 수정하기 위해 처음으로 하는 일은 [**MobileServiceClient**][MobileServiceClient]에 대해 **getTable** 메서드를 호출하여 [MobileServiceTable][MobileServiceTable] 개체를 만드는 것입니다. 이 메서드의 오버로드 두 가지를 살펴보겠습니다.
...
[첫 번째 오버로드][첫 번째 오버로드]는 클래스 이름과 테이블 이름이 같을 때 사용합니다.

        MobileServiceTable<ToDoItem> mToDoTable = mClient.getTable(ToDoItem.class);

[두 번째 오버로드][두 번째 오버로드]는 테이블 이름이 형식 이름과 다를 때 사용합니다.
...
이와 같은 쿼리는 [**TableQueryCallback\<E\>**][**TableQueryCallback\<E\>**] 콜백 개체를 사용합니다.
...
테이블 참조에 대한 [**where**][**where**] 메서드 호출에서 필터를 시작합니다. 그 뒤에 [**field**][**field**] 메서드 호출, 논리 조건자를 지정하는 메서드 호출을 차례로 사용합니다. 가능한 조건자 메서드에는 [**eq**][**eq**], [**ne**][**ne**], [**gt**][**gt**], [**ge**][**ge**], [**lt**][**lt**], [**le**][**le**] 등이 포함됩니다.

이는 숫자 및 문자열 필드를 특정 값과 비교하기에 충분합니다. 하지만 훨씬 더 많은 기능을 사용할 수 있습니다.

예를 들어 날짜를 기준으로 필터링할 수 있습니다. 전체 날짜 필드를 비교할 수 있으며 [**year**][**year**], [**month**][**month**], [**day**][**day**], [**hour**][**hour**], [**minute**][**minute**], [**second**][**second**] 등의 메서드를 사용하여 날짜의 부분을 비교할 수도 있습니다. 다음 부분 코드는 *기한*이 2013과 같은 항목에 해당하는 필터를 추가합니다.

        mToDoTable.where().year("due").eq(2013)

[**startsWith**][**startsWith**], [**endsWith**][**endsWith**], [**concat**][**concat**], [**subString**][**subString**], [**indexOf**][**indexOf**], [**replace**][**replace**], [**toLower**][**toLower**], [**toUpper**][**toUpper**], [**trim**][**trim**], [**length**][**length**] 등의 메서드를 사용하여 문자열 필드에 다양한 복합 필터를 적용할 수 있습니다. 다음 부분 코드는 *text* 열이 "PRI0"으로 시작되는 테이블 행을 필터링합니다.

        mToDoTable.where().startsWith("text", "PRI0")

숫자 필드에도 [**add**][**add**], [**sub**][**sub**], [**mul**][**mul**], [**div**][**div**], [**mod**][**mod**], [**floor**][**floor**], [**ceiling**][**ceiling**], [**round**][**round**] 등의 메서드를 사용하여 다양한 복합 필터를 적용할 수 있습니다. 다음 부분 코드는 *duration*이 짝수인 테이블 행을 필터링합니다.

        mToDoTable.where().field("duration").mod(2).eq(0)

조건자를 [**and**][**and**], [**or**][**or**], [**not**][**not**] 등의 메서드와 결합할 수 있습니다. 다음 부분 코드는 위의 예 중 두 가지를 결합합니다.
...
필터링에 대한 자세한 내용과 예를 보려면 [모바일 서비스 Android 클라이언트 쿼리 모델의 다양한 기능 알아보기][모바일 서비스 Android 클라이언트 쿼리 모델의 다양한 기능 알아보기](영문)를 참조하십시오.
...
[**orderBy**][**orderBy**] 메서드의 첫 번째 매개 변수는 정렬 기준이 되는 필드의 이름과 동일한 문자열입니다.

두 번째 매개 변수는 [**QueryOrder**][**QueryOrder**] 열거형을 사용하여 오름차순이나 내림차순으로 정렬할지를 지정합니다.
...
[**select**][**select**] 메서드는 [**where**][1], [**orderBy**][2] 등의 메서드(있는 경우) 뒤에 나와야 합니다. 그 뒤에 [**top**][**top**] 등의 메서드가 나올 수 있습니다.
...
사용하는 쿼리 메서드가 [**MobileServiceQuery\<T\>**][**MobileServiceQuery\<T\>**] 개체를 반환하며 이 개체에 대해 호출되는 추가 메서드가 있을 수 있기 때문에 이런 작업이 가능합니다. 일련의 메서드를 종료하고 실제로 쿼리를 실행하려면 [**execute**][**execute**] 메서드를 호출합니다.
...
그런 다음, [**insert**][**insert**] 메서드를 호출합니다.
...
**insert** 작업의 경우 콜백 개체는 [**TableOperationCallback\<ToDoItem\>**][**TableOperationCallback\<ToDoItem\>**]입니다.
...
-   제어 문자: [0x0000-0x001F] 및 [0x007F-0x009F]. 자세한 내용은 [ASCII 제어 코드 C0 및 C1][ASCII 제어 코드 C0 및 C1](영문)을 참조하세요.
-   인쇄 가능한 문자: **"**(0x0022), **+** (0x002B), **/** (0x002F), **?** (0x003F), **\\** (0x005C), **\`** (0x0060)
-   ID "." 및 ".."

또는 테이블에 정수 ID를 사용할 수 있습니다. 정수 ID를 사용하려면 `mobile table create` 명령으로 `--integerId` 옵션을 사용하여 테이블을 만들어야 합니다. 이 명령은 Azure용 CLI(명령줄 인터페이스)와 함께 사용됩니다. CLI 사용에 대한 자세한 내용은 [모바일 서비스 테이블 관리 CLI][모바일 서비스 테이블 관리 CLI](영문)를 참조하세요.

## <a name="updating"></a><span class="short-header">데이터 업데이트</span>방법: 모바일 서비스의 데이터 업데이트
...
*delete*의 경우, 콜백 개체는 [**TableDeleteCallback**][**TableDeleteCallback**]이며 테이블 행이 반환되지 않는다는 점에서 **onCompleted** 메서드가 약간 다릅니다.
...
## <a name="lookup"></a><span class="short-header">데이터 조회</span>방법: 특정 항목 조회
...
형식화된 모델과 유사하게 테이블 참조를 가져와서 시작합니다. 하지만 이 경우에는 이 참조가 [MobileServicesJsonTable][MobileServicesJsonTable] 개체입니다. 모바일 서비스 클라이언트 인스턴스에 대해 [getTable()][getTable()] 메서드를 호출하여 참조를 가져옵니다.
...
다음 코드는 삽입하는 방법을 보여 줍니다. 첫 번째 단계는 [gson][gson] 라이브러리의 일부인 [**JsonObject**][**JsonObject**]를 만드는 것입니다.
...
다음 단계는 개체를 삽입하는 것입니다. [**insert**][3] 메서드에 전달되는 콜백 함수는 [**TableJsonOperationCallback**][**TableJsonOperationCallback**] 클래스의 인스턴스입니다. *onCompleted* 메서드의 첫 번째 매개 변수는 JsonObject입니다.
...
다음 코드는 전체 테이블을 검색하는 방법을 보여 줍니다. 형식화되지 않은 프로그래밍 모델은 다른 콜백 개체인 [**TableJsonQueryCallback**][**TableJsonQueryCallback**]을 사용합니다.
...
## <a name="binding"></a><span class="short-header">데이터 바인딩</span>방법: 사용자 인터페이스에 데이터 바인딩
...
모바일 서비스는 Facebook, Google, Microsoft 계정, Twitter, Azure Active Directory 등 다양한 외부 ID 공급자를 사용하는 앱 사용자에 대한 인증 및 권한 부여를 지원합니다. 테이블에 대해 사용 권한을 설정하여 특정 작업을 위한 액세스를 인증된 사용자로만 제한할 수 있습니다. 인증된 사용자의 ID를 사용하여 서버 스크립트에 인증 규칙을 구현할 수도 있습니다. 자세한 내용은 [인증 시작][인증 시작](영문)을 참조하십시오.
...
처음 두 가지 작업은 [Azure 관리 포털][Azure 관리 포털]을 사용하여 수행합니다. 자세한 내용은 [인증 시작][인증 시작](영문)을 참조하십시오.
...
유효성 검사 및 오류 처리의 예를 [여기][여기]에서 볼 수 있습니다. 오류에 대한 예외를 반환하는 서버 스크립트를 통해 유효성 검사를 구현하며, 예외를 처리하는 클라이언트 코드도 제공됩니다.
...
## <a name="customizing"></a><span class="short-header">클라이언트 사용자 지정</span>방법: 클라이언트 사용자 지정
...
클라이언트 이름을 서버에 있는 *ToDoItem* 테이블의 열 이름과 일치하는 JSON 이름으로 직렬화해야 합니다. 다음 코드는 [gson][gson] 라이브러리를 이용하여 이를 수행합니다.
...
클라이언트 테이블 이름을 다른 모바일 서비스 테이블 이름에 매핑하는 작업은 쉽습니다. 다음 코드에 나온 것처럼[getTable()][두 번째 오버로드] 함수의 재정의 중 하나를 사용하면 됩니다.

        mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="conversions"></a>방법: 열 이름 매핑 자동화

열이 몇 개뿐인 작은 테이블의 열 이름을 매핑하는 일은 이전 섹션에서 보았듯이 어렵지 않습니다. 하지만 테이블에 예를 들어 20개나 30개 정도의 많은 열이 있는 경우, [gson][gson] API를 호출하고 모든 열에 적용되는 변환 전략을 지정하여 모든 열 이름에 주석을 달 필요를 없앨 수 있습니다.

이렇게 하려면 Android 클라이언트 라이브러리리에서 자동으로 Java 개체를 JSON 데이터에 직렬화하여 Azure 모바일 서비스로 전송하는 데 사용하는 [gson][gson] 라이브러리를 사용합니다.
...
이를 수행하는 방법의 예를 보려면 블로그 게시물인 [모바일 서비스 Android 클라이언트에서][모바일 서비스 Android 클라이언트에서][gson][gson] 라이브러리를 사용하여 serialization 사용자 지정</a>(영문)을 확인하십시오.
...
Android 클라이언트 API에 대한 Javadoc 참조는 [][]<http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/package-summary.html></a>(영문)에서 제공됩니다.
  ...
  [첫 번째 오버로드]: http://go.microsoft.com/fwlink/p/?LinkId=296839
  [두 번째 오버로드]: http://go.microsoft.com/fwlink/p/?LinkId=296840
  ...
  [모바일 서비스 Android 클라이언트 쿼리 모델의 다양한 기능 알아보기]: http://hashtagfail.com/post/46493261719/mobile-services-android-querying
  ...
  [인증 시작]: http://go.microsoft.com/fwlink/p/?LinkId=296316
  [Azure 관리 포털]: https://manage.windowsazure.com/
  [여기]: https://www.windowsazure.com/ko-kr/develop/mobile/tutorials/validate-modify-and-augment-data-dotnet/
  [모바일 서비스 Android 클라이언트에서]: http://hashtagfail.com/post/44606137082/mobile-services-android-serialization-gson
  []: http://go.microsoft.com/fwlink/p/?LinkId=298735 "여기"
