...
Azure IoT Suite는 다음과 같이 *미리 구성된 솔루션*의 집합입니다.
...
미리 구성된 솔루션은 구독을 사용하여 Azure에 배포할 수 있는 일반적인 IoT 솔루션 패턴의 오픈 소스 구현입니다. 미리 구성된 각 솔루션은 사용자 지정 코드와 Azure 서비스를 결합하여 특정 IoT 시나리오 또는 일반 시나리오를 구현합니다. 특정 요구 사항에 맞게 시나리오를 사용자 지정할 수 있습니다. 이 시나리오에는 다음이 포함됩니다.
...
미리 구성된 각 솔루션은 시뮬레이션된 장치 또는 물리적 장치를 사용하여 원격 분석을 생성하는 완전한 종단 간 구현입니다. 미리 구성된 솔루션을 솔루션 가속기로 사용하여 다음을 수행할 수 있습니다.

* 사용자 고유의 IoT 솔루션에 대한 시작점을 제공합니다.
* IoT 솔루션 설계 및 개발의 일반적인 패턴에 대해 알아봅니다.

현재 미리 구성된 세 가지 솔루션을 사용할 수 있습니다.

* [원격 모니터링](iot-suite-remote-monitoring-explore.md)
* [예측 유지 관리](iot-suite-predictive-overview.md)
* [연결된 팩터리](iot-suite-connected-factory-overview.md)
...
> [!NOTE]
> 미리 구성된 솔루션을 배포하고 이를 사용자 지정하는 방법에 대해 자세히 알아보려면 [Microsoft Azure IoT Suite](https://www.azureiotsuite.com/)를 방문하세요.

## <a name="azure-services"></a>Azure 서비스

미리 구성된 솔루션을 배포하면 프로비전 프로세스에서 다양한 Azure 서비스를 구성합니다. 다음 표에서는 미리 구성된 솔루션에서 사용되는 서비스를 보여 줍니다.
...
## <a name="whats-new-in-preconfigured-solutions"></a>미리 구성된 솔루션의 새로운 기능
...
다음 섹션에서는 미리 구성된 솔루션의 새로운 마이크로 서비스 기반 기능에 대해 설명합니다.

### <a name="microservices"></a>마이크로 서비스

미리 구성된 원격 모니터링 솔루션의 새 버전에서는 마이크로 서비스 아키텍처를 사용합니다. 미리 구성된 이 솔루션은 *IoT Hub 관리자* 및 *Storage 관리자*와 같은 여러 마이크로 서비스로 구성됩니다. Java 및 .NET 버전 모두의 각 마이크로 서비스는 관련된 개발자 설명서와 함께 다운로드할 수 있습니다. 마이크로 서비스에 대한 자세한 내용은 [원격 모니터링 아키텍처](iot-suite-remote-monitoring-sample-walkthrough.md)를 참조하세요.
...
* **Enterprise:** 프로덕션 배포를 개발할 수 있도록 확장된 인프라 배포입니다. Azure Container Service는 마이크로 서비스를 여러 개의 Azure 가상 컴퓨터에 배포합니다. Kubernetes는 개별 마이크로 서비스를 호스팅하는 Docker 컨테이너를 오케스트레이션합니다.
...
* [원격 모니터링 .NET GitHub 리포지토리](https://github.com/Azure/azure-iot-pcs-remote-monitoring-dotnet)
* [원격 모니터링 Java GitHub 리포지토리](https://github.com/Azure/azure-iot-pcs-remote-monitoring-java)
...
이제 미리 구성된 IoT Suite 솔루션의 개요를 살펴보았으므로 미리 구성된 각 솔루션에 제안되는 단계는 다음과 같습니다.

* [Azure IoT Suite 원격 모니터링 솔루션 탐색(Resource Manager 배포 모델)](iot-suite-remote-monitoring-explore.md)
* [미리 구성된 예측 유지 관리 솔루션 개요](iot-suite-predictive-overview.md)
* [미리 구성된 연결된 팩터리 솔루션 시작](iot-suite-connected-factory-overview.md)
