...
* *명령 및 컨트롤*: 장치가 작업을 수행하도록 클라우드에서 장치에 메시지를 보냅니다.
...
장치를 미리 구성된 솔루션에 있는 IoT Hub에 처음으로 연결하는 경우, 장치가 응답할 수 있는 메서드를 열거하는 허브에 장치 정보 메시지를 보냅니다. 미리 구성된 원격 모니터링 솔루션에서 시뮬레이션된 장치는 이러한 메서드를 지원합니다.
...
장치를 미리 구성된 솔루션에 있는 IoT Hub에 처음으로 연결하는 경우, 장치가 응답할 수 있는 명령을 열거하는 허브에 장치 정보 메시지를 보냅니다. 미리 구성된 원격 모니터링 솔루션에서 시뮬레이션된 장치는 이러한 명령을 지원합니다.
...
* *DeviceInfo 작업* - 솔루션 장치 레지스트리(Azure Cosmos DB 데이터베이스)에 장치 등록 특정 메시지를 라우팅하는 이벤트 허브에 데이터를 출력합니다. 장치를 처음으로 연결하거나 **장치 상태 변경** 명령에 대한 응답으로 이 메시지가 전송됩니다.
