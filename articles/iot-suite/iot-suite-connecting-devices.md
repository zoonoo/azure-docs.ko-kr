...
3.  realdevice1과 같은 이름을 입력하여 고유한 장치 ID를 선택하고 ID 확인을 클릭하여 이름이 아직 사용되지 않았는지 확인합니다.

    ![][3]

5. 제공된 자격 증명(장치 ID, IoT Hub 호스트 이름 및 장치 키)을 복사하세요. 클라이언트 응용 프로그램에서 장치를 솔루션에 연결하려면 나중에 필요합니다.
...
7. 원격 모니터링 솔루션 대시보드로 돌아가기 장치의 상태를 장치 목록에서 실행 중으로 변경해야 합니다.
