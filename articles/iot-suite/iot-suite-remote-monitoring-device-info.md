...
[IoT Hub 장치 관리 미리 보기][lnk-dm-preview]에서는 이 문서에 설명된 미리 구성된 원격 모니터링 솔루션의 장치 정보 관리 기능과 유사한 기능을 IoT Hub에 추가합니다. 그러나 이 시점에 원격 모니터링 솔루션은 IoT Hub에서 일반적으로 사용 가능한(GA) 기능만을 활용합니다.
...
- **HubEnabledState**: IoT Hub에서 장치의 상태입니다. 장치가 처음 연결될 때까지 우선 **null**로 설정합니다. 솔루션 포털에서 이것은 "등록되었지만 표시되지 않는" 장치를 나타납니다.
...
솔루션 포털에서 장치를 처음 만드는 경우 솔루션은 위와 같이 장치 레지스트리에서 항목을 만듭니다. 대부분의 정보는 처음부터 지워져 있고 **HubEnabledState**는 **null**로 설정됩니다. 또한 이 시점에서 솔루션은 장치가 IoT Hub로 인증하는 데 사용할 키를 생성하는 IoT Hub 장치 ID 레지스트리에서 장치에 대한 항목을 만듭니다.
...
솔루션 포털의 **장치 세부 정보** 창에서 **편집**을 클릭하면 이러한 속성을 모두 편집할 수 있습니다. 이러한 속성을 편집하면 DocumentDB 데이터베이스에서 장치에 대한 레코드를 업데이트합니다. 그러나 장치가 업데이트된 장치 정보 메시지를 전송하면 솔루션 포털에서 변경 내용을 덮어씁니다. 이러한 속성에 대한 권한이 장치에만 있기 때문에 솔루션 포털에서 **DeviceId**, **Hostname**, **HubEnabledState**, **CreatedTime**, **DeviceState** 및 **UpdatedTime** 속성을 편집할 수 없습니다.

![장치 편집][img-device-edit]

솔루션 포털을 사용하여 솔루션에서 장치를 제거할 수 있습니다. 장치를 제거하면 솔루션은 솔루션 장치 레지스트리에서 장치 정보 메타데이터를 제거하고 IoT Hub 장치 ID 레지스트리에서 장치 항목을 제거합니다. 장치를 제거하려면 사용 하지 않도록 설정해야 합니다.

![장치 제거][img-device-remove]

## 장치 정보 메시지 처리

장치에서 전송되는 장치 정보 메시지는 장치 속성, 장치가 응답할 수 있는 명령 및 명령 기록과 같은 정보를 포함한다는 점에서 원격 분석 메시지와 구별됩니다. IoT Hub 자체에는 장치 정보 메시지에 포함된 메타데이터의 정보가 없으며 장치-클라우드 메시지를 처리하는 것과 동일한 방식으로 메시지를 처리합니다. 원격 모니터링 솔루션에서 [Azure 스트림 분석][lnk-stream-analytics](ASA) 작업은 IoT Hub의 메시지를 읽습니다. **DeviceInfo** 스트림 분석 작업은 **"ObjectType": "DeviceInfo"**를 포함하는 메시지를 필터링하고 웹 작업에서 실행되는 **EventProcessorHost** 호스트 인스턴스에 전달합니다. **EventProcessorHost** 인스턴스의 논리는 장치 ID를 사용하여 특정 장치에 대한 DocumentDB 레코드를 찾고 레코드를 업데이트합니다. 이제 장치 레지스트리 레코드는 장치 속성, 명령 및 명령 기록 등의 정보를 포함합니다.
