...
HDInsight용 Microsoft .NET API를 사용하여 응용 프로그램에서 MapReduce 프로그램을 프로그래밍 방식으로 실행할 수도 있습니다. 작업 제출에 HDInsight API를 사용하는 방법에 대한 자세한 내용은 [HDInsight에서 Hadoop 작업 제출][hdinsight-submit-hadoop-jobs-programmatically]을 참조하세요.
...
- Microsoft Azure 계정이 있어야 합니다. 계정 등록 옵션은 [Azure: 1개월 무료 평가판](http://azure.microsoft.com/pricing/free-trial/)을 참조하세요.

- HDInsight 클러스터를 미리 프로비전해야 합니다. 이 클러스터를 만드는 여러 방법에 대한 지침은 [사용자 지정 옵션을 사용하여 HDInsight의 Hadoop 클러스터 프로비전](hdinsight-provision-clusters.md)을 참조하세요.

- Azure PowerShell을 설치하고 계정과 함께 사용하도록 구성해야 합니다. 작업 방법에 대한 지침은 [Azure PowerShell 설치 및 구성 방법][powershell-install-configure]을 참조하세요.
