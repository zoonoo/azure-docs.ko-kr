...
Microsoft Azure 미디어 서비스를 사용하면 128 비트 암호화 키를 사용하는 AES(Advanced Encryption Standard) 및 PlayReady DRM로 동적 암호화된 콘텐츠를 제공할 수 있습니다. 또한 미디어 서비스는 인증된 클라이언트에게 키 및 PlayReady 라이선스를 배달하는 서비스를 제공합니다. 

미디어 서비스로 자산을 암호화하려는 경우 암호화 키(**CommonEncryption** 또는 **EnvelopeEncryption**)와 자산을 연결([여기](../media-services-rest-create-contentkey/)에 설명)하고 키에 대한 인증 정책을 구성해야 합니다(이 기사에서 설명). 

현재 암호화할 수 있는 스트리밍 형식: HLS, MPEG DASH 및 부드러운 스트리밍. HDS 스트리밍 형식 또는 점진적 다운로드는 암호화할 수 없습니다.
...
미디어 서비스는 키를 요청 하는 사용자를 인증 하는 여러 방법을 지원합니다. 콘텐츠 키 인증 정책은 다음과 같은 하나 이상의 권한 부여 제한이 있을 수 있습니다. **개방**, **토큰** 제한 또는 **IP** 제한. 토큰 제한 정책은 보안 토큰 서비스(STS)에 의해 발급된 토큰이 수반되어야 합니다. 미디어 서비스는 **간단한 웹 토큰**([SWT](https://msdn.microsoft.com/ko-kr/library/gg185950.aspx#BKMK_2)) 형식 및 **JSON 웹 토큰**(JWT) 형식의 토큰을 지원합니다.  
...
[JWT 토큰 인증](http://www.gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/)을 참조하세요.
...
- 동적 패키징 및 동적 암호화를 사용할 수 있으려면 하나 이상의 배율 단위(스트리밍 단위)가 있어야 합니다. 자세한 내용은 [미디어 서비스 크기를 조정하는 방법](../media-services-manage-origins#scale_streaming_endpoints)을 참조하십시오. 
- 자산에는 다중 비트 전송률 mp4나 다중 비트 전송률 부드러운 스트리밍 파일 집합이 포함 되어야 합니다. 자세한 내용은 [자산 인코드](../media-services-encode-asset/)를 참조하세요.  
- **AssetCreationOptions.StorageEncrypted** 옵션을 사용하여 사용자의 자산을 업로드 및 인코딩합니다.
...
- 사용자의 자산 배달 정책을 추가하거나 업데이트 하는 경우 기존 로케이터(있는 경우)를 삭제하고 새 로케이터를 만들어야 합니다.


##AES 128 동적 암호화. 

>[AZURE.NOTE] 미디어 서비스 REST API로 작업할 때 고려할 사항:
>
>미디어 서비스에서 엔터티를 액세스할 때 HTTP 요청에 구체적인 헤더 필드 및 값 설정 해야 합니다. 자세한 내용은 [미디어 서비스 REST API 개발을 위한 설치](../media-services-rest-how-to-use)를 참조하십시오.

>Https://media.windows.net에 연결하면 다른 미디어 서비스 URI를 지정하는 301 리디렉션을 받게 됩니다. [미디어 서비스 REST API를 사용하여 연결](../media-services-rest-connect_programmatically/)에서 설명한 대로 새 URI에 대한 후속 호출을 생성해야 합니다. 
...
다음 예제에서는 개방 인증 정책을 만들고 콘텐츠 키에 추가합니다.
...
이 섹션에서는 콘텐츠 키 인증 정책을 만들고 콘텐츠 키와 연결하는 방법을 설명합니다. 인증 정책은 사용자가 키를 받도록 인증받는지 여부를 결정하기 위해 어떤 인증 요구 사항이 충족되야 하는지 설명합니다(예: "확인 키"목록은 토큰 서명에 사용된 키를 포함).

토큰 제한 옵션을 구성하려면 XML을 사용하여 토큰의 인증 요구 사항을 설명해야 합니다. 토큰 제한 구성 XML은 다음 XML 스키마를 준수 해야 합니다.
...
**토큰** 제한 정책을 구성 하는 경우 기본 **확인 키**, **발급자** 및 **청중** 매개 변수를 지정해야 합니다. **기본 확인 키**는 토큰이 서명된 키를 포함하며 **발급자**는 토큰을 발행하는 보안 토큰 서비스입니다. **청중**(**범위**) 라고도 함)은 토큰의 의도 또는 토큰이 접근을 인증하는 대상 리소스를 설명합니다. 미디어 서비스 키 배달 서비스는 이러한 토큰의 값이 템플릿 파일에 있는 값과 일치하는지 확인합니다. 
...
[여기]에 표새된 대로 "Token Restriction Policy" [생성](#ContentKeyAuthorizationPolicies).
...
미디어 서비스를 사용하면 사용자가 보호된 콘텐츠를 재생하려고 할 때 PlayReady DRM 런타임으로 적용 하려는 권한 및 제한 사항을 구성할 수 있습니다. 

PlayReady로 콘텐츠를 보호하는 경우, 인증 정책에서 지정해야하는 것 중 하나는 [PlayReady 라이선스 템플릿](https://msdn.microsoft.com/ko-kr/library/azure/dn783459.aspx)을 정의하는 XML 문자열입니다. 
...
토큰 제한 옵션을 구성하려면 XML을 사용하여 토큰의 인증 요구 사항을 설명해야 합니다. 토큰 제한 구성 XML은 [여기]에 표시된 XML 스키마를 준수 해야 합니다 [섹션](#schema).
	
####ContentKeyAuthorizationPolicies 만들기
	
[여기]에 표시된 대로 ContentKeyAuthorizationPolicies [민들기](#ContentKeyAuthorizationPolicies2).
...
##다음 단계
이제 사용자가 콘텐츠 키 인증 정책 구성을 완료하였으므로, [자산 배달 정책을 구성 하는 방법](../media-services-rest-configure-asset-delivery-policy/) 항목으로 이동하십시오.
