...
응용 프로그램 프레임 워크 내에서 구성 데이터를 사용 하는 경우 키 값에 대 한 특정 명명 스키마를 지시할 수 있습니다. 예를 들어 Java의 스프링 클라우드 프레임 워크는 스프링 응용 프로그램에 설정을 제공 하는 `Environment` 리소스를 정의 합니다.  이러한 매개 변수는 *응용 프로그램 이름* 및 *프로필*을 포함 하는 변수에 의해 매개 변수화 됩니다. Spring Cloud 관련 구성 데이터에 대한 키는 일반적으로 구분 기호로 구분된 이러한 두 가지 요소로 시작합니다.
...
`*`, `,`및 `\`를 제외 하 고 키 이름에 임의의 유니코드 문자를 사용할 수 있습니다.  이러한 예약 문자 중 하나를 포함 해야 하는 경우 `\{Reserved Character}`를 사용 하 여 이스케이프 합니다. 
