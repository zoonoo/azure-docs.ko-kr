...
이 문서에서는 기본 앱 구성 저장소에서 보조 저장소로 키 값의 자동 백업을 설정 하는 방법에 대해 설명 합니다. 앱 구성과 Azure Event Grid의 통합을 활용 합니다. 설정 되 면 앱 구성이 구성 저장소에서 키 값에 대 한 변경 내용에 대 한 Event Grid에 이벤트를 게시 합니다. Event Grid은 사용자가 키-값이 생성, 업데이트 또는 삭제 될 때마다 발생 하는 이벤트를 구독할 수 있는 다양 한 Azure 서비스를 지원 합니다.

## <a name="overview"></a>개요

이 자습서에서는 Azure Storage 큐를 사용 하 여 Event Grid에서 이벤트를 수신 하 고 Azure Functions의 타이머 트리거를 사용 하 여 저장소 큐의 이벤트를 일괄 처리로 처리 합니다. 이벤트가 트리거될 때이 함수는 기본 앱 구성 저장소에서 변경 된 키의 최신 값을 가져오고 그에 따라 보조 저장소를 업데이트 합니다. 이 설정은 하나의 백업 작업에서 짧은 기간에 발생 하는 여러 변경 내용을 결합 하 고 앱 구성 저장소에 대 한 과도 한 요청을 방지 하는 데 도움이 됩니다.

![앱 구성 저장소 백업 아키텍처](./media/config-store-backup-architecture.png)

## <a name="resource-provisioning"></a>리소스 프로비전
...
 `<primary_appconfig_name>`및를 `<secondary_appconfig_name>` 구성 저장소의 고유한 이름으로 바꿉니다. 저장소 이름은 DNS 이름으로 사용 되므로 고유 해야 합니다.
...
## <a name="create-azure-storage-queue"></a>Azure Storage 큐 만들기

Event Grid에서 게시 한 이벤트를 수신 하기 위한 저장소 계정 및 저장소 큐를 만듭니다.
...
다음 명령은 저장소 큐에 전송 된 두 이벤트에 대 한 Event Grid 구독을 만듭니다. 여기서 끝점 형식은로 설정 되 `storagequeue` 고 끝점은 큐 ID로 설정 됩니다. 을 `<event_subscription_name>` 이벤트 구독에 대해 선택한 이름으로 바꿉니다.
...
## <a name="create-azure-functions-for-handling-events-from-storage-queue"></a>저장소 큐에서 이벤트를 처리 하는 Azure Functions 만들기

### <a name="setup-with-ready-to-use-azure-functions"></a>즉시 사용 가능한 Azure Functions 설정

이 자습서에서는 다음 속성을 사용 하 여 c # Azure Functions를 사용 합니다.
- 런타임 스택 .NET Core 3.1
- Azure Functions 런타임 버전 3(sp3)
- 10 분 마다 타이머에 의해 트리거되는 함수

데이터의 백업을 쉽게 시작할 수 있도록 코드를 변경 하지 않고 사용할 수 있는 [Azure Functions](https://github.com/Azure/AppConfiguration/tree/master/examples/ConfigurationStoreBackup) 테스트 하 고 게시 했습니다. 프로젝트 파일을 다운로드 하 고 [Visual Studio에서 사용자 고유의 Azure 함수 앱에 게시 합니다.](/azure/azure-functions/functions-develop-vs#publish-to-azure)

> [!IMPORTANT]
> 다운로드 한 코드에서 환경 변수를 변경 하지 마십시오. 다음 섹션에서 필요한 앱 설정을 만듭니다.
>

### <a name="build-your-own-azure-functions"></a>사용자 고유의 Azure Functions 빌드

위에서 제공한 샘플 코드가 요구 사항을 충족 하지 않는 경우 사용자 고유의 Azure Functions 만들 수도 있습니다. 함수는 백업을 완료 하기 위해 다음 작업을 수행할 수 있어야 합니다.
- 저장소 큐의 내용을 주기적으로 읽어 Event Grid의 알림이 포함 되어 있는지 확인 합니다. 구현에 대 한 자세한 내용은 [저장소 큐 SDK](/azure/storage/queues/storage-quickstart-queues-dotnet) 를 참조 하세요.
- 저장소 큐에 [Event Grid의 이벤트 알림이](/azure/azure-app-configuration/concept-app-configuration-event?branch=pr-en-us-112982#event-schema)포함 된 경우 이벤트 메시지에서 고유한 <키, 레이블>를 모두 추출 합니다. 키와 레이블 조합은 기본 저장소의 키-값 변경 내용에 대 한 고유 식별자입니다.
- 기본 저장소에서 모든 설정을 읽습니다. 저장소 큐에 해당 이벤트가 있는 보조 저장소의 설정만 업데이트 합니다. 저장소 큐에 있지만 주 저장소에는 없는 모든 설정을 보조 저장소에서 삭제 합니다. [앱 구성 SDK](https://github.com/Azure/AppConfiguration#sdks) 를 활용 하 여 프로그래밍 방식으로 구성 저장소에 액세스할 수 있습니다.
- 처리 하는 동안 예외가 발생 하지 않은 경우 저장소 큐에서 메시지를 삭제 합니다.
- 필요에 따라 오류 처리를 구현 해야 합니다. 위의 코드 샘플을 참조 하 여 처리할 수 있는 몇 가지 일반적인 예외를 확인할 수 있습니다.

Azure Functions를 만드는 방법에 대 한 자세한 내용은 [Azure에서 타이머에 의해 트리거되는 함수 만들기](/azure/azure-functions/functions-create-scheduled-function) 및 [Visual Studio를 사용 하 여 Azure Functions 개발](/azure/azure-functions/functions-develop-vs)을 참조 하세요.
...
## <a name="grant-access-to-the-managed-identity-of-azure-function-app"></a>Azure 함수 앱의 관리 되는 id에 대 한 액세스 권한 부여

다음 명령 또는 [Azure Portal](/azure/app-service/overview-managed-identity#add-a-system-assigned-identity) 를 사용 하 여 Azure 함수 앱에 대 한 시스템 할당 관리 id를 추가 합니다.
...
앱 구성 저장소에 대 한 Azure 함수 앱 액세스의 관리 id를 부여 하려면 다음 명령 또는 [Azure Portal](/azure/azure-app-configuration/howto-integrate-azure-managed-service-identity#grant-access-to-app-configuration) 을 사용 합니다.
...
모든 것이 작동 하는지 테스트 하려면 기본 저장소에서 키 값을 만들거나 업데이트 하거나 삭제할 수 있습니다. 타이머가 Azure Functions 트리거된 후 몇 초 후에 보조 저장소에서이 변경 내용을 자동으로 확인 해야 합니다.

```azurecli-interactive
az appconfig kv set --name $primaryAppConfigName --key Foo --value Bar --yes
```

이벤트를 트리거 했 고 몇 분 후에 Azure Storage 큐에 이벤트 알림을 보냅니다 Event Grid. ***다음에 예약 된 Azure Functions 실행 후***보조 저장소에서 구성 설정을 확인 하 여 기본 저장소의 업데이트 된 키 값이 포함 되어 있는지 확인 합니다.

> [!NOTE]
> 예약 된 타이머 트리거를 기다리지 않고 테스트 및 문제 해결 중에 [Azure Functions를 수동으로 트리거할](/azure/azure-functions/functions-manually-run-non-http) 수 있습니다.
...
- Event Grid에서 저장소 큐에 이벤트 알림을 시간 안에 전송 하지 못할 수 있습니다. 저장소 큐에 여전히 주 저장소의 이벤트 알림이 포함 되어 있는지 확인 하 고, 그렇지 않으면 백업 기능을 다시 트리거합니다.
- 오류 또는 경고에 대 한 [Azure Functions 로그](/azure/azure-functions/functions-create-scheduled-function#test-the-function) 를 확인 합니다.
- [Azure Portal](/azure/azure-functions/functions-how-to-use-azure-function-app-settings#get-started-in-the-azure-portal) 를 사용 하 여 Azure 함수 앱에서 Azure Functions 읽으려는 응용 프로그램 설정에 올바른 값을 포함 하는지 확인 합니다.
...
이제 키 값의 자동 백업을 설정 하는 방법을 배웠으므로 응용 프로그램의 지역 복원 력을 높이는 방법에 대해 자세히 알아보세요.
