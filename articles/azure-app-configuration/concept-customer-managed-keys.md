...
>[!IMPORTANT]
> 앱 구성 인스턴스에 할당 된 id에 더 이상 인스턴스의 암호화 키를 래핑 해제할 수 있는 권한이 없거나 관리 되는 키가 영구적으로 삭제 된 경우 앱에 저장 된 중요 한 정보를 더 이상 해독할 수 없게 됩니다. 구성 인스턴스입니다. Azure Key Vault의 [일시 삭제](../key-vault/key-vault-ovw-soft-delete.md) 기능을 사용 하면 암호화 키를 실수로 삭제할 가능성이 줄어듭니다.
...
2. 대상 Key Vault의 액세스 정책에서 id `GET`, `WRAP`및 `UNWRAP` 권한을 부여 합니다.
...
- [Azure 앱 구성을 사용 하 여 .NET Framework 앱 만들기](quickstart-dotnet-app.md)
- [Azure 앱 구성을 사용 하 여 Java 스프링 앱 만들기](quickstart-java-spring-app.md)
...
1. Azure CLI를 사용 하 여 Azure Key Vault를 만듭니다.  `vault-name`와 `resource-group-name`는 모두 사용자가 제공 하며 고유 해야 합니다.  이 예에서는 `contoso-vault` 및 `contoso-resource-group`를 사용 합니다.
...
1. Key Vault 키를 만듭니다. 이 키에 대해 고유한 `key-name`를 제공 하 고 1 단계에서 만든 Key Vault (`contoso-vault`) 이름을 대체 합니다. `RSA` 또는 `RSA-HSM` 암호화를 사용할지 여부를 지정 합니다.
...
1. Azure CLI를 사용 하 여 시스템 할당 관리 id를 만들고, 이전 단계에서 사용한 앱 구성 인스턴스 및 리소스 그룹의 이름을 대체 합니다. 관리 되는 id는 관리 되는 키에 액세스 하는 데 사용 됩니다. `contoso-app-config`를 사용 하 여 앱 구성 인스턴스의 이름을 설명 합니다.
...
1. 키 유효성 검사, 암호화 및 암호 해독을 수행 하려면 Azure 앱 구성 인스턴스의 관리 되는 id에 키에 대 한 액세스 권한이 있어야 합니다. 액세스 해야 하는 특정 작업 집합에는 키에 대 한 `GET`, `WRAP`및 `UNWRAP`가 포함 됩니다.  액세스 권한을 부여 하려면 앱 구성 인스턴스의 관리 id의 보안 주체 ID가 필요 합니다. 이 값은 이전 단계에서 가져온 것입니다. 이는 `contoso-principalId`로 표시 됩니다. 명령줄을 사용 하 여 관리 되는 키에 대 한 사용 권한을 부여 합니다.

    ```azurecli
    az keyvault set-policy -n contoso-vault --object-id contoso-principalId --key-permissions get wrapKey unwrapKey
    ```

1. Azure 앱 구성 인스턴스가 관리 키에 액세스할 수 있으면 Azure CLI를 사용 하 여 서비스에서 고객이 관리 하는 키 기능을 사용 하도록 설정할 수 있습니다. 키 생성 단계 중에 기록 된 다음 속성을 회수 합니다. `key name` `key vault URI`.
