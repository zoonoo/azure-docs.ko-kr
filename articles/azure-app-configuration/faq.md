...
ms.locfileid: "65411898"
---
# <a name="azure-app-configuration-faq"></a>Azure 앱 구성 FAQ

이 문서에서는 Azure 앱 구성에 대 한 질문을 자주 합니다.

## <a name="how-is-app-configuration-different-from-key-vault"></a>앱 구성 Key Vault와 어떻게 다른가요?

앱 구성 사용 사례의 고유 집합 용인지: 개발자가 응용 프로그램 설정을 관리 하 고 기능 가용성을 제어할 수 있도록 합니다. 다양 한 복잡 한 구성 데이터를 사용 하는 작업을 단순화 하려고 합니다. 계층 구조 네임 스페이스, 레이블 지정, 광범위 한 쿼리, 일괄 처리, 검색 및 특수 한 관리 작업 및 사용자 인터페이스를 지원합니다. 앱 구성에는 Key Vault에 보완 되며 대부분의 응용 프로그램 배포의 두를 함께 사용 해야 합니다.

## <a name="should-i-store-secrets-in-app-configuration"></a>앱 구성에서 비밀 정보를 저장 해야 합니까?

앱 구성 강화 된 보안에서 제공 하는 동안 Key Vault는 여전히 응용 프로그램 비밀을 저장 하는 데 좋습니다. 하드웨어 수준 암호화, 세분화 된 액세스 정책 및 인증서 회전 등의 관리 작업을 제공합니다.

## <a name="does-app-configuration-encrypt-my-data"></a>앱 구성 않습니다 내 데이터를 암호화?

예. 앱 구성 보유 하 고 네트워크 통신이 모든 키 값을 암호화 합니다. 키 이름에는 암호화 되지 않은 및 구성 데이터를 검색 하는 것에 대 한 인덱스로 사용 됩니다.

## <a name="does-app-configuration-support-azure-virtual-network-vnet"></a>앱 구성에서 Azure Virtual Network (VNET)를 지원 하나요?

아직 없습니다. VNET 지원 출시 예정입니다.
...
현재 저장소 수준에서 앱 구성에 대 한 액세스 권한이 있는 사용자를 제어할 수 있습니다. 다른 권한이 필요한 각 환경에 대 한 별도 저장소를 사용 해야 합니다. 이 방법은 최상의 보안 격리를 제공합니다.
