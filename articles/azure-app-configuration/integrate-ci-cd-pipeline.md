...
ms.locfileid: "60000020"
---
# <a name="integrate-with-a-cicd-pipeline"></a>CI/CD 파이프라인과 통합

Azure App Configuration에 연결할 수 없는 희박한 가능성에 대비하여 애플리케이션의 복원력을 향상할 수 있습니다. 이렇게 하려면 시작할 때 애플리케이션과 함께 배포되어 로컬로 로드된 파일에 현재 구성 데이터를 패키징해야 합니다. 이 방법을 사용하면 애플리케이션이 적어도 기본 설정 값을 갖게 됩니다. 앱 구성 저장소에 새로운 변경 내용이 있으면 해당 변경 내용이 이러한 값을 덮어씁니다.
...
2. Program.cs를 열고, `config.AddJsonFile()` 메서드를 호출하여 내보낸 JSON 파일을 사용하도록 `CreateWebHostBuilder` 메서드를 업데이트합니다.
