...
애플리케이션이 Azure App Configuration에 의존하는데 연결할 수 없는 경우 실행되지 않을 수 있습니다. 이러한 이벤트를 처리하기 위해 애플리케이션 복원력을 향상시킬 수 있지만 이러한 이벤트는 발생하지 않을 가능성이 높습니다. 이렇게 하려면 시작할 때 애플리케이션과 함께 배포되어 로컬로 로드된 파일에 현재 구성 데이터를 패키징해야 합니다. 이 방법을 사용하면 애플리케이션이 적어도 기본 설정 값을 갖게 됩니다. App Configuration 저장소에 새로운 변경 내용이 있으면 해당 변경 내용이 이러한 값을 덮어씁니다.
...
1. *.csproj* 파일을 열고 다음 스크립트를 추가합니다.
    ...
    App Configuration 저장소와 연결된 *ConnectionString*을 환경 변수로 추가합니다.
