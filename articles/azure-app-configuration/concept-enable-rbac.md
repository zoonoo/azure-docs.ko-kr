...
1. 보안 주체의 id가 인증 되 고 OAuth 2.0 토큰이 반환 됩니다.  토큰 `https://login.microsoftonline.com/{tenantID}` 을 요청 하는 리소스 이름은에서 서비스 `{tenantID}` 주체가 속한 AZURE ACTIVE DIRECTORY 테 넌 트 ID와 일치 합니다.
