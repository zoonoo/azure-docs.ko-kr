...
2. 구성 저장소에 대한 App Configuration 포털에서 액세스 키로 이동합니다. 읽기 전용 키 탭을 선택합니다. 이 탭에서 연결 문자열 중 하나의 값을 복사하고, 변수 이름이 `APP_CONFIGURATION_CONNECTION_STRING`인 새 환경 변수로 추가합니다.

3. 기본 애플리케이션 Java 파일을 열고 `@EnableConfigurationProperties`를 추가하여 이 기능을 활성화합니다.
...
5. 앱의 템플릿 디렉터리에 *welcome.html*이라는 새 HTML 파일을 만듭니다. 다음 줄을 추가합니다.
...
6. static 아래에 CSS라는 새 폴더를 만들고 그 안에 *main.css*라는 새 CSS 파일을 만듭니다. 다음 줄을 추가합니다.
