---
title: Azure Firewall 샘플
description: Azure Firewall PowerShell 샘플
services: firewall
author: vhorne
manager: jpconnock
editor: tysonn
tags: azure-resource-manager
ms.service: firewall
ms.topic: article
ms.date: 7/11/2018
ms.author: victorh
ms.custom: mvc
ms.openlocfilehash: 3dd751c5dd4edbc2c132859f142ef927e63298bc
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/23/2019
ms.locfileid: "60192968"
---
# <a name="azure-firewall-powershell-samples"></a>Azure Firewall PowerShell 샘플

다음 표에는 방화벽을 만드는 Azure PowerShell 스크립트 샘플의 링크가 제공됩니다.


| | |
|----|----|
|[Azure Firewall 및 테스트 인프라 만들기](scripts/sample-create-firewall-test.md)|Azure Firewall 및 테스트 네트워크 인프라를 만듭니다.|





