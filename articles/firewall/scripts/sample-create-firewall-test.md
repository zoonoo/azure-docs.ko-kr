...
description: Azure PowerShell 스크립트 샘플 - Azure Firewall 테스트 환경 만들기.
...
PowerShell을 로컬로 실행하는 경우 이 스크립트에는 최신 AzureRM PowerShell 모듈 버전(6.9.0 이상)이 필요합니다. 설치되어 있는 버전을 확인하려면 `Get-Module -ListAvailable AzureRM`을 실행합니다. 
