...
2. Azure SQL Database, SQL Data Warehouse 또는 SQL Managed Instance 트래픽을 필터링 하는 경우 SQL 연결 모드가 **프록시**로 설정 되었는지 확인 합니다. SQL 연결 모드를 전환 하는 방법에 대 한 자세한 내용은 [AZURE Sql 연결 설정](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-settingse#change-azure-sql-database-connection-policy)을 참조 하세요.
