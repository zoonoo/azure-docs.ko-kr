...
* **애플리케이션 규칙 로그**
   ...
   응용 프로그램 규칙 로그는 저장소 계정에 저장 되 고, 이벤트 허브로 스트리밍 되며, 각 Azure 방화벽에 대해 사용 하도록 설정한 경우에만 Azure Monitor 로그에 전송 됩니다. 허용/거부된 연결에 대한 로그에서 구성된 애플리케이션 규칙 결과 중 하나와 일치하는 각 새 연결입니다. 데이터는 다음 예제와 같이 JSON 형식으로 기록 됩니다.
