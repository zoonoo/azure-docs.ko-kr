...
.NET용 Batch 파일 규칙 라이브러리는 [NuGet][nuget_package]에서 사용할 수 있습니다. 라이브러리는 새 메서드가 포함된 [CloudJob][net_cloudjob] 및 [CloudTask][net_cloudtask] 클래스를 확장합니다. 또한 파일 규칙 라이브러리에 대한 [참조 설명서](/dotnet/api/microsoft.azure.batch.conventions.files)를 참조하세요.
