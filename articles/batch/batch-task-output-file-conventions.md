...
.NET용 파일 규칙 라이브러리는 파일 규칙 표준에 따라 스토리지 컨테이너와 태스크 출력 파일의 이름을 자동으로 지정합니다. 또한 파일 규칙 라이브러리는 작업 ID, 태스크 ID 또는 용도에 따라 Azure Storage의 출력 파일을 쿼리하는 메서드를 제공합니다.
...
> [!WARNING]
> 파일 규칙 라이브러리를 사용하여 유지되는 모든 작업 및 태스크 출력은 동일한 컨테이너에 저장됩니다. 많은 수의 태스크에서 동시에 파일을 유지하려고 시도하면 [스토리지 제한](../storage/common/storage-performance-checklist.md#blobs)이 적용될 수 있습니다.

### <a name="create-storage-container"></a>스토리지 컨테이너 만들기

Azure Storage에 작업 출력을 유지 하려면 먼저 [Cloudjob][net_cloudjob]을 호출 하 여 컨테이너를 만듭니다. [PrepareOutputStorageAsync][net_prepareoutputasync]. 이 확장 메서드는 [Cloudstorageaccount][net_cloudstorageaccount] 개체를 매개 변수로 사용 합니다. Azure Portal과 이 문서의 뒷부분에서 설명하는 검색 메서드에서 해당 콘텐츠를 검색할 수 있도록 파일 규칙 표준에 따라 명명된 컨테이너를 만듭니다.
...
이제 Azure Storage 컨테이너를 준비 했으므로 작업에서 파일 규칙 라이브러리에 있는 [Taskoutputstorage][net_taskoutputstorage] 클래스를 사용 하 여 컨테이너에 출력을 저장할 수 있습니다.

작업 코드에서 먼저 [taskoutputstorage][net_taskoutputstorage] 개체를 만든 다음 태스크가 해당 작업을 완료 하면 [taskoutputstorage][net_taskoutputstorage]를 호출 합니다. Azure Storage에 출력을 저장 하는 [SaveAsync][net_saveasync] 메서드
...
[TaskOutputStorage](/dotnet/api/microsoft.azure.batch.conventions.files.taskoutputstorage).[SaveAsync](/dotnet/api/microsoft.azure.batch.conventions.files.taskoutputstorage.saveasync#overloads) 메서드의 `kind` 매개 변수는 유지된 파일을 분류합니다. 4 가지 미리 정의 된 [taskoutputkind][net_taskoutputkind] 형식이 `TaskOutput` `TaskPreview` `TaskLog`있습니다.,, `TaskIntermediate.` 및 출력의 사용자 지정 범주를 정의할 수도 있습니다.
...
[Joboutputstorage][net_joboutputstorage]를 호출 하 여 작업 출력을 저장 합니다. [SaveAsync][net_joboutputstorage_saveasync] 메서드를 지정 하 고 [joboutputkind][net_joboutputkind] 와 파일 이름을 지정 합니다.
...
다음 코드 조각에서는 [SaveTrackedAsync][net_savetrackedasync] 를 사용 하 여 작업을 `stdout.txt` 실행 하는 동안 15 초 마다 Azure Storage를 업데이트 합니다.
...
주석 처리된 `Code to process data and produce output file(s)` 섹션은 태스크에서 일반적으로 수행하는 코드에 대한 자리 표시자입니다. 예를 들어 Azure Storage에서 데이터를 다운로드하고 변환 또는 계산을 수행하는 코드가 있을 수 있습니다. 이 코드 조각의 중요 한 부분은 [SaveTrackedAsync][net_savetrackedasync]를 사용 하 여 파일을 주기적으로 업데이트 `using` 하기 위해 블록에서 이러한 코드를 래핑하는 방법을 보여 주는 것입니다.
...
다음 코드 조각에서는 작업의 태스크를 반복하고, 태스크의 출력 파일에 대한 정보 일부를 출력한 다음, 스토리지에서 해당 파일을 다운로드합니다.
...
1. 출력을 보관할 때 스토리지 컨테이너 및 파일에 대한 미리 정의된 명명 규칙을 따릅니다. 이러한 규칙의 정의는 파일 규칙 라이브러리 [추가 정보][github_file_conventions_readme]에서 찾을 수 있습니다. [Azure Batch 파일 규칙][nuget_package] 라이브러리를 사용 하 여 출력을 유지 하는 경우 파일 규칙 표준에 따라 파일이 유지 됩니다.
...
.NET 용 Batch 파일 규칙 라이브러리는 [NuGet][nuget_package]에서 사용할 수 있습니다. 라이브러리는 [Cloudjob][net_cloudjob] 및 [cloudjob][net_cloudtask] 클래스를 새로운 메서드로 확장 합니다. 또한 파일 규칙 라이브러리에 대한 [참조 설명서](https://docs.microsoft.com/dotnet/api/microsoft.azure.batch.conventions.files)를 참조하세요.

파일 규칙 라이브러리의 [소스 코드][github_file_conventions] 는 .net 용 Microsoft Azure SDK 리포지토리의 GitHub에서 사용할 수 있습니다. 
