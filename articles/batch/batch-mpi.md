...
5. 주 및 모든 하위 작업에서 조정 명령이 성공적으로 완료된 *후* 주 작업은 마스터 노드에서 **애플리케이션 명령**을 실행합니다. 애플리케이션 명령은 다중 인스턴스 작업 자체의 명령줄이며 주 작업에서만 실행됩니다. [MS mpi][msmpi_msdn]기반 솔루션에서는를 사용 하 여 `mpiexec.exe`mpi 지원 응용 프로그램을 실행 합니다.
