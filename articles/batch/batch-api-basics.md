...
- **시작** - 이 위치는 시작 태스크에서 작업 디렉터리로 사용됩니다. 시작 태스크를 시작하기 위해 배치 서비스에서 다운로드한 모든 파일도 이 디렉터리 아래에 저장됩니다. 노드에서 시작 디렉터리는 `%AZ_BATCH_NODE_STARTUP_DIR%`을 통해 사용할 수 있습니다. 시작 태스크는 이 디렉터리 아래에서 파일을 만들고, 읽고, 업데이트하고, 삭제할 수 있으며 이 디렉터리는 시작 태스크가 운영 체제를 구성하는 데 사용할 수 있습니다.
