...
태스크가 생성한 로그 파일의 복사본을 유지하거나 실패한 애플리케이션에서 생성될 수 있는 덤프 파일의 작동을 중단할 수 있습니다. 이 경우 **작업 릴리스 태스크** 를 사용 하 여이 데이터를 압축 하 고 [Azure Storage][azure_storage] 계정에 업로드 합니다.
...
위에서 설명했듯이 작업이 종료되거나 삭제될 때 해제 태스크가 실행됩니다. [JobOperations. joboperations.terminatejobasync][net_job_terminate]. Delete a job with [JobOperations.DeleteJobAsync][net_job_delete]를 사용 하 여 작업을 종료 합니다. 일반적으로 작업의 태스크들이 완료되거나 정의한 시간 제한에 도달했을 때 작업을 종료하거나 삭제합니다.
