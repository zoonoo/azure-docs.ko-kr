...
위의 설명대로 배치 관리 API의 주요 기능은 Azure 지역 내에서 배치 계정을 만들고 삭제하는 것입니다. 이를 위해 [BatchManagementClient.Accounts.CreateAsync][net_create], [DeleteAsync][net_delete] 또는 해당 동기 항목을 사용합니다.
...
> [AZURE.NOTE]배치 관리 .NET 라이브러리 및 해당 BatchManagementClient를 사용하는 응용 프로그램에는 관리할 배치 계정을 소유하는 구독에 대한 **서비스 관리자** 또는 **공동 관리자** 액세스 권한이 필요합니다. 자세한 내용은 아래 [Azure Active Directory](#aad) 섹션과 [AccountManagement][acct_mgmt_sample] 코드 샘플을 참조하세요.

## 계정 키 검색 및 다시 생성

[ListKeysAsync][net_list_keys]를 사용하여 구독 내 배치 계정에서 기본 및 보조 계정 키를 가져오고 [RegenerateKeyAsync][net_regenerate_keys]로 해당 키를 다시 생성합니다.
...
> [AZURE.TIP]관리 응용 프로그램에 대한 간소화된 연결 워크플로를 만들 수 있습니다. 먼저 [ListKeysAsync][net_list_keys]로 관리할 배치 계정에 대한 계정 키를 가져온 후 [BatchClient][net_batch_client] 초기화에 사용된 배치 .NET 라이브러리의 [BatchSharedKeyCredentials][net_sharedkeycred]를 초기화할 때 이 키를 사용합니다.

## Azure 구독 및 배치 계정 할당량 확인

Azure 구독 및 배치와 같은 개별 Azure 서비스는 모두 포함되는 특정 엔터티 수를 제한하는 기본 할당량이 있습니다. Azure 구독에 대한 기본 할당량은 [Azure 구독 및 서비스 제한, 할당량 및 제약 조건](./../azure-subscription-service-limits.md)에서 확인할 수 있으며 배치 서비스에 대한 할당량은 [Azure 배치 서비스에 대한 할당량 및 제한](batch-quota-limit.md)에서 확인할 수 있습니다. 배치 관리 .NET 라이브러리는 응용 프로그램 내에서 이러한 할당량을 확인하는 기능을 제공하여 풀 및 계산 노드와 같은 계산 리소스나 계정을 추가하기 전에 할당을 결정할 수 있습니다.

### Azure 구독에서 배치 계정 할당량 확인

지역에 배치 계정을 만들기 전에 Azure 구독에서 해당 지역에 계정을 추가할 수 있는지 여부를 확인할 수 있습니다.

아래 코드 조각에서 먼저 [BatchManagementClient.Accounts.ListAsync][net_mgmt_listaccounts]를 사용하여 구독 내에서 모든 배치 계정의 컬렉션을 가져옵니다. 이 컬렉션을 가져온 후에는 대상 지역에 계정이 얼마나 있는지 확인한 후 [BatchManagementClient.Subscriptions][net_mgmt_subscriptions]를 사용하여 배치 계정 할당량을 가져오고 해당 지역에 계정(있는 경우)을 얼마나 만들 수 있는지 확인합니다.
...
배치 솔루션 내에서 계산 리소스를 늘리기 전에 할당하려는 리소스가 현재 가능한 계정 할당량을 초과하지 않는지 확인할 수 있습니다. 아래 코드 조각에서는 `mybatchaccount`라는 배치 계정에 대한 할당량 정보를 인쇄하지만 사용자 고유의 응용 프로그램에서 이러한 정보를 사용하여 해당 계정에서 만들 추가 리소스를 처리할 수 있는지 여부를 확인할 수 있습니다.
...
일반적으로 배치 관리 .NET 라이브러리를 사용할 때는 [Azure Active Directory][aad_about](AAD) 및 [Azure 리소스 관리자][resman_overview]의 두 기능을 모두 활용합니다. 아래 설명된 샘플 프로젝트에서는 배치 관리 .NET API를 설명하는 동안 Azure Active Directory와 리소스 관리자를 모두 사용합니다.

### <a name="aad"></a>Azure Active Directory

Azure에서는 해당 고객, 서비스 관리자 및 조직 사용자의 인증을 위해 Azure Active Directory(AAD)를 자체적으로 사용합니다. 배치 관리 .NET 컨텍스트에서 이를 사용하여 구독 관리자 또는 공동 관리자를 인증한 후 관리 라이브러리에서 배치 서비스를 쿼리하고 이 문서에 설명된 작업을 수행하도록 허용합니다.

아래 설명된 샘플 프로젝트에서 Azure [Active Directory Authentication Library][aad_adal](ADAL)는 사용자에게 Microsoft ID 자격 증명을 묻는 데 사용됩니다. 서비스 관리자 또는 공동 관리자 자격 증명을 제공하는 경우 응용 프로그램이 Azure에 구독 목록을 쿼리하고 리소스 그룹 및 배치 계정을 만들고 삭제할 수 있습니다.
...
실제 배치 관리 .NET 라이브러리를 보려면 GitHub에서 [AccountManagment][acct_mgmt_sample] 샘플 프로젝트를 참조하세요. 이 콘솔 응용 프로그램은 [BatchManagementClient][net_mgmt_client] 및 [ResourceManagementClient][resman_client]의 생성 및 사용을 보여주고 두 클라이언트 모두에 필요한 Azure [Active Directory Authentication Library][aad_adal](ADAL)의 사용을 보여줍니다.

> [AZURE.IMPORTANT]응용 프로그램 예제를 성공적으로 실행하려면 먼저 Azure 관리 포털을 사용하여 Azure Active Directory에 등록해야 합니다. [Azure Active Directory와 응용 프로그램 통합][aad_integrate]에서 **응용 프로그램을 추가**를 확인한 후 이 문서의 단계에 따라 사용자 고유 계정 내에 응용 프로그램 예제를 등록합니다.

이 응용 프로그램 예제는 다음 작업을 보여줍니다.

1. [ADAL][aad_adal]을 사용하여 Azure Active Directory(AAD)에서 보안 토큰을 획득합니다. 사용자가 아직 로그인하지 않은 경우 Azure 자격 증명을 묻는 메시지가 표시됩니다.
2. AAD에서 획득한 보안 토큰을 사용하여 [SubscriptionClient][resman_subclient]를 만들고 Azure에 해당 계정과 연결된 구독 목록을 쿼리하며 발견된 구독이 여러 개인 경우 사용자가 하나를 선택할 수 있도록 합니다.
3. 선택한 구독에 연결된 새 자격 증명 개체 만들기
4. 새 자격 증명을 사용하여 [ResourceManagementClient][resman_client] 만들기
5. [ResourceManagementClient][resman_client]를 사용하여 새 리소스 그룹 만들기
6. [BatchManagementClient][net_mgmt_client]를 사용하여 여러 배치 계정 작업 수행
  - 새로 만든 리소스 그룹 내에서 새 배치 계정 만들기
  - 배치 서비스에서 새로 만든 계정 가져오기
  - 새 계정에 대한 계정 키 인쇄
  - 계정에 대한 새 기본 키 다시 생성
  - 계정에 대한 할당량 정보 인쇄
  - 구독에 대한 할당량 정보 인쇄
