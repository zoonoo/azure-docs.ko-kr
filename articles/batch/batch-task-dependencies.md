...
하나 이상의 상위 태스크를 완료하는 데 따라 달라지는 태스크를 만들려면 태스크가 다른 태스크"에 종속"되도록 지정할 수 있습니다. Batch .NET에서 [Cloudtask][net_cloudtask]를 구성 합니다. [Taskdependencies][net_taskdependencies] 클래스의 인스턴스를 사용 하는 [DependsOn][net_dependson] 속성:
...
> [!NOTE]
> 기본적으로 태스크가 **완료** 상태이고 해당 **종료 코드**가 `0`인 경우 성공적으로 완료되었다고 간주됩니다. Batch .NET에서 이는 [Cloudtask][net_cloudtask]를 의미 합니다. `Completed` 및 cloudtask[의 taskexecutioninformation][net_taskexecutioninformation]의 State [속성][net_taskstate] 값입니다.[ ExitCode][net_exitcode] 속성 값은 `0`입니다. 변경 방법에 대해서는 [종속성 작업](#dependency-actions) 섹션을 참조하세요.
> 
> 
...
일대다 관계의 경우 태스크는 여러 상위 태스크를 완료했는지에 따라 달라집니다. 종속성을 만들려면 태스크 Id의 컬렉션을 [Taskdependencies][net_taskdependencies]에 제공 합니다. [Cloudtask][net_cloudtask]의 [DependsOn][net_dependson] 속성을 채우는 경우 [onids][net_onids] 정적 메서드
...
종속성을 만들려면 범위의 첫 번째 및 마지막 태스크 Id를 [Taskdependencies][net_taskdependencies]에 제공 합니다. [Cloudtask][net_cloudtask]의 [DependsOn][net_dependson] 속성을 채우는 경우 [onidrange][net_onidrange] 정적 메서드입니다.
