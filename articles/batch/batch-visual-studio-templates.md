...
Batch에서 태스크를 실행하는 데 태스크 프로세서가 반드시 필요하지는 않지만 태스크 프로세서를 사용할 경우 주요 장점은 한 위치에서 모든 태스크 실행 작업을 구현하는 래퍼를 제공한다는 것입니다. 예를 들어 각 태스크의 컨텍스트에서 여러 응용 프로그램을 실행해야 하는 경우 또는 각 태스크를 완료한 후 데이터를 영구 저장소에 복사해야 하는 경우가 있습니다.
