...
---
# <a name="query-the-azure-batch-service-efficiently"></a>효율적인 Azure 배치 서비스 쿼리
여기서는 [배치.NET][api_net] 라이브러리를 사용하여 작업, 태스크 및 계산 노드를 쿼리할 때 서비스에서 반환하는 데이터 양을 줄여 Azure 배치 응용 프로그램의 성능을 향상시키는 방법을 알아봅니다.
...
이 [Batch .NET][api_net] API 코드 조각은 각 태스크의 속성 *모두*와 함께 작업에 연결된 *모든* 태스크를 나열합니다.
...
그러나 쿼리에 "세부 수준"을 적용하여 훨씬 더 효율적인 목록 쿼리를 수행할 수 있습니다. 이렇게 하려면 [JobOperations.ListTasks][net_list_tasks] 메서드에 [ODATADetailLevel][odata] 개체를 제공합니다. 이 코드 조각은 완료된 태스크의 ID, 명령줄 및 계산 노드 정보 속성만 반환합니다.
...
[배치.NET][api_net] 및 [배치 REST][api_rest] API는 목록에 반환되는 항목 수와 각 항목에 반환되는 정보 양을 줄일 수 있는 기능을 제공합니다. 이렇게 하려면 목록 쿼리를 수행할 때 **filter**, **select** 및 **expand 문자열**을 지정합니다.
...
* [배치.NET][api_net] 또는 다른 배치 SDK 중 하나를 사용하는 경우에도 filter, select, expand 문자열의 속성 이름은 [Batch REST][api_rest] API에서와 동일하게 나타나야 합니다.
...
[배치.NET][api_net] API 안에서 [ODATADetailLevel][odata] 클래스는 filter, select 및 expand 문자열을 제공하여 작업을 나열하는 데 사용됩니다. ODataDetailLevel 클래스에는 생성자 내에 지정되어 있거나 개체에 직접 설정된 세 개의 공용 문자열 속성이 있습니다. 그런 다음 매개 변수로 ODataDetailLevel 개체를 다양한 목록 작업(예: [ListPools][net_list_pools], [ListJobs][net_list_jobs] 및 [ListTasks][net_list_tasks])에 전달합니다.

* [ODATADetailLevel][odata].[FilterClause][odata_filter]: 반환되는 항목 수를 제한합니다.
* [ODATADetailLevel][odata].[SelectClause][odata_select]: 각 항목과 함께 반환되는 속성 값을 지정합니다.
...
> [!TIP]
> Select 및 Expand 절로 구성된 [ODATADetailLevel][odata]의 인스턴스는 적절한 Get 메서드(예: [PoolOperations.GetPool](https://msdn.microsoft.com/library/azure/microsoft.azure.batch.pooloperations.getpool.aspx))에 전달되어 반환되는 데이터 양을 제한할 수도 있습니다.
> 
> 
...
* **.NET 메서드 목록**: 이 열의 각 .NET API 메서드는 매개 변수로 [ODATADetailLevel][odata] 개체를 받아들입니다.
...
| [CloudTask.ListNodeFiles][net_list_task_files] |[태스크에 연결된 파일 나열][rest_list_task_files] |
| [JobOperations.ListJobPreparationAndReleaseTaskStatus][net_list_jobprep_status] |[작업 준비 및 작업 해제 태스크의 상태 나열][rest_list_jobprep_status] |
...
| [JobOperations.ListTasks][net_list_tasks] |[작업에 연결된 태스크 나열][rest_list_tasks] |
| [JobScheduleOperations.ListJobSchedules][net_list_job_schedules] |[계정에 작업 일정 나열][rest_list_job_schedules] |
| [JobScheduleOperations.ListJobs][net_list_schedule_jobs] |[작업 일정에 연결된 작업 나열][rest_list_schedule_jobs] |
...
[ODATADetailLevel.FilterClause][odata_filter]에 대한 filter 문자열을 구성할 때 위의 "filter 문자열 매핑" 표를 참조하면 수행하려는 목록 작업에 해당하는 REST API 설명서 페이지를 찾을 수 있습니다. 해당 페이지의 첫 번째 다중 행 표에 필터링 가능한 속성과 지원되는 연산자가 있습니다. 예를 들어 종료 코드가 0이 아닌 모든 태스크를 검색하려는 경우 다음의 [작업에 연결된 태스크 나열][rest_list_tasks] 행은 적용 가능한 속성 문자열과 허용 가능한 연산자를 지정하고 있습니다.
...
[ODATADetailLevel.SelectClause][odata_select]를 구성하려면 위의 "select 문자열 매핑" 표를 참조하여 나열하려는 엔터티 형식에 해당하는 REST API 페이지로 이동합니다. 해당 페이지의 첫 번째 다중 행 표에 선택 가능한 속성과 지원되는 연산자가 있습니다. 목록의 각 태스크에 대한 ID와 명령줄만 검색하려면 [태스크 정보 가져오기][rest_get_task]의 해당 표에서 다음 행을 찾을 수 있습니다.
...
GitHub의 [EfficientListQueries][efficient_query_sample] 샘플 프로젝트를 확인하면 효율적인 목록 쿼리가 응용 프로그램의 성능에 미치는 영향력을 확인할 수 있습니다. 이 C# 콘솔 응용 프로그램은 많은 작업을 만들고 또 작업에 추가합니다. 그런 다음 [JobOperations.ListTasks][net_list_tasks] 메서드에 대해 여러 호출을 만들고 다른 속성 값으로 구성된 [ODATADetailLevel][odata] 개체를 전달하여 반환될 데이터 양을 다양하게 변경합니다. 다음과 유사한 출력을 생성합니다.
...
[BatchMetrics][batch_metrics] 샘플에는 자체 프로젝트에 통합할 수 있는 .NET 클래스 라이브러리 프로젝트 및 라이브러리 사용을 연습하고 설명하는 간단한 명령줄 프로그램이 포함되어 있습니다.
