...
Azure 배치를 사용하는 거의 모든 응용 프로그램은 정기적으로 배치 서비스를 쿼리하는 특정 형식의 모니터링 또는 다른 작업을 수행합니다. 예를 들어 용량 및 풀의 상태를 확인하려면 풀 내에서 모든 노드를 쿼리해야 합니다. 작업의 태스크가 큐에 대기 중인지를 확인하려면 작업 내에서 모든 태스크를 쿼리해야 합니다. 이 문서에서는 가장 효율적인 방법으로 이러한 형식의 쿼리를 실행하는 방법을 설명합니다.
...
[배치 .NET][api_net] 및 [Batch REST][api_rest] API는 목록에 반환되는 항목 수와 각각에 대해 반환되는 정보의 크기를 줄일 수 있습니다. 목록 쿼리를 수행할 때 **filter**, **select** 및 **expand** 문자열을 지정하여 수행할 수 있습니다.

### 필터
필터 문자열은 반환되는 항목 수를 줄이는 식입니다. 예를 들어, 한 작업에 대해 실행 중인 태스크만 나열하거나 태스크를 실행할 준비가 된 계산 노드만 나열할 수 있습니다.
...
- 이 예제 선택 문자열은 각 태스크에 대해 반환되어야 하는 세 가지 속성 값을 지정합니다. `id, state, stateTransitionTime`

### 확장
확장 문자열은 특정 정보를 얻는 데 필요한 API 호출 수를 줄입니다. 확장 문자열을 사용하면 단일 API 호출로 각 항목에 대한 자세한 정보를 얻을 수 있습니다. 엔터티의 목록을 첫 번째로 가져오기 보다 목속에서 각 항목에 대한 정보를 요청하여 단일 API 호출에서 동일한 정보를 얻기 위해 확장 문자열을 사용할 수 있습니다. API 호출이 적어지면 성능이 향상되었음을 의미합니다.
...
[ODATADetailLevel.SelectClause][odata_select]를 구성하려면 "선택 문자열에 대한 매핑"에서 위의 표를 참조하고 나열하는 엔터티 형식에 해당하는 REST API 페이지로 이동합니다. 해당 페이지의 첫 번째 다중 행 표에 선택 가능한 속성과 지원되는 연산자가 있습니다. 목록의 각 태스크에 대해 ID와 명령줄만 검색하려면 [태스크 관련 정보 가져오기][rest_get_task]의 해당하는 표에서 이 행을 찾을 수 있습니다.
