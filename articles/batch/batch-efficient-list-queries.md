<properties
	pageTitle="Azure 배치의 효율적인 목록 쿼리 | Microsoft Azure"
	description="풀, 작업, 태스크 및 계산 노드와 같은 배치 엔터티에 대한 정보를 요청할 때 쿼리를 필터링하여 성능을 향상합니다."
	services="batch"
...
거의 모든 배치 응용 프로그램은 정기적으로 배치 서비스를 쿼리하는 특정 형식의 모니터링 또는 다른 작업을 수행해야 합니다. 예를 들어 작업에 남아 있는 대기 중인 태스크가 있는지 여부를 확인하려면 작업 내에서 모든 태스크에 데이터를 가져와야 합니다. 풀에 있는 노드의 상태를 확인하려면 풀의 모든 노드에 데이터를 가져와야 합니다. 이 문서에서는 가장 효율적인 방법으로 이러한 형식의 쿼리를 실행하는 방법을 설명합니다.

## DetailLevel 충족

프로덕션 배치 응용 프로그램에서 작업, 태스크 및 계산 노드 같은 엔터티가 수천 개 있을 수 있습니다. 이러한 항목에 대한 정보를 얻으면 각 쿼리의 응용 프로그램에 대한 서비스에서 “통신을 교차"해야 하는 많은 양의 데이터가 생성될 수 있습니다. 항목의 수와 쿼리에 의해 반환되는 정보의 형식을 제한하여 쿼리의 속도 및 응용 프로그램의 성능을 높일 수 있습니다.
...
## Filter, select 및 expand

[배치 .NET][api_net] 및 [Batch REST][api_rest] API는 목록에 반환되는 항목 수와 각각에 대해 반환되는 정보의 크기를 줄일 수 있습니다. 목록 쿼리를 수행할 때 **filter**, **select** 및 **expand 문자열**을 지정하여 수행할 수 있습니다.
...
- 이 예제 filter 문자열은 실행 중인 "렌더링" 태스크만 나열합니다. `(state eq 'running') and startswith(id, 'renderTask')`
...
- 이 예제 select 문자열은 각 태스크에 대해 반환되어야 하는 세 가지 속성 값을 지정합니다. `id, state, stateTransitionTime`
...
예를 들어 다음 메서드는 BatchMetrics 라이브러리에 나타납니다. `id` 및 `state` 속성만 쿼리되는 엔터티에 대해서 가져와야 한다고 지정하는 ODATADetailLevel을 반환합니다. 또한 지정된 `DateTime` 매개 변수로 인해 상태가 변경된 엔터티만을 반환해야 한다고 지정합니다.
