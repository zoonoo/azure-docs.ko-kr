...
Azure Batch는 정의한 매개 변수에 따라 풀을 자동으로 크기 조정하여 시간과 비용을 절약해줍니다. 자동 크기 조정을 사용하면 Batch는 작업 요구가 증가함에 따라 동적으로 풀에 컴퓨팅 노드를 추가하고, 작업 요구가 감소하면 노드를 제거합니다.

컴퓨팅 노드 풀에서 자동 크기 조정을 사용하려면 풀을 사용자가 정의한 '자동 크기 조정 수식'에 연결합니다. Batch 서비스는 자동 크기 조정 수식을 사용하여 워크로드를 실행하는 데 필요한 노드의 수를 결정합니다. 이러한 노드는 전용 노드 또는 [우선 순위가 낮은 노드](batch-low-pri-vms.md)일 수 있습니다. 그런 다음 Batch는 정기적으로 서비스 메트릭 데이터를 검토하고 사용하여 사용자가 지정한 수식 및 간격에 따라 풀의 노드 수를 조정합니다.

자동 크기 조정은 풀을 만들 때 사용하도록 설정하거나 기존 풀에 적용할 수 있습니다. Batch를 사용하면 풀에 할당하기 전에 수식을 평가하고, 자동 크기 조정 실행 상태를 모니터링할 수 있습니다. 자동 크기 조정을 사용하도록 풀을 구성하면 나중에 수식을 변경할 수 있습니다.

> [!IMPORTANT]
> Batch 계정을 만들 때 [풀 할당 모델](accounts.md)을 지정할 수 있습니다. 이 모델은 풀을 Batch 서비스 구독(기본값)에 할당할지 사용자 구독에 할당할지 결정합니다. 기본 Batch 서비스 구성으로 Batch 계정을 만든 경우 계정이 처리에 사용할 수 있는 코어의 최대 개수가 제한됩니다. Batch 서비스는 컴퓨팅 노드를 해당 코어 제한까지만 확장합니다. 이러한 이유로 Batch 서비스는 자동 크기 조정 수식에 지정된 컴퓨팅 노드의 목표 수에 도달하지 못할 수 있습니다. 계정 할당량을 보고 늘리는 방법에 대한 내용은 [Azure Batch 서비스에 대한 할당량 및 제한](batch-quota-limit.md)을 참조하세요.
>
>사용자 구독 모드로 계정을 만든 경우 계정은 구독의 코어 할당량을 공유합니다. 자세한 내용은 [Azure 구독 및 서비스 제한, 할당량 및 제약 조건](../azure-resource-manager/management/azure-subscription-service-limits.md)에서 [Virtual Machines 제한](../azure-resource-manager/management/azure-subscription-service-limits.md#virtual-machines-limits)을 참조하세요.

## <a name="autoscale-formulas"></a>자동 크기 조정 수식

자동 크기 조정 수식은 사용자가 정의하는 하나 이상의 문을 포함하는 문자열 값입니다. 자동 크기 조정 수식은 풀의 [autoScaleFormula](/rest/api/batchservice/enable-automatic-scaling-on-a-pool) 요소(Batch REST) 또는 [CloudPool.AutoScaleFormula](/dotnet/api/microsoft.azure.batch.cloudpool.autoscaleformula) 속성(Batch .NET)에 할당됩니다. Batch 서비스는 처리할 다음 간격을 위해 풀의 대상 컴퓨팅 노드 수를 결정하는 수식을 사용합니다. 수식 문자열은 8KB를 초과할 수 없고, 세미콜론으로 구분된 문을 100개까지 포함할 수 있으며, 줄 바꿈과 주석을 포함할 수 있습니다.

자동 크기 조정 수식은 Batch 자동 크기 조정 "언어"로 고려할 수 있습니다. 수식 문은 자유 형식이고 서비스 정의 변수(Batch 서비스에 의해 정의됨)와 사용자 정의 변수를 포함할 수 있습니다. 수식은 기본 제공 형식, 연산자 및 함수를 사용하여 이러한 값에 다양한 연산을 수행할 수 있습니다. 예를 들어 문은 다음과 같은 형태일 수 있습니다.
...
자동 크기 조정 수식에 이러한 문을 포함하여 컴퓨팅 노드의 목표 수에 도달합니다. 전용 노드와 우선 순위가 낮은 노드에는 각각 고유한 대상 설정이 있습니다. 자동 크기 조정 수식에는 전용 노드의 목표 값, 우선 순위가 낮은 노드의 목표 값 또는 둘 다가 포함될 수 있습니다.

노드의 목표 수는 풀에 있는 해당 노드 형식의 현재 수보다 높거나, 낮거나, 같을 수 있습니다. Batch는 풀의 자동 크기 조정 수식을 특정 [자동 크기 조정 간격](#automatic-scaling-interval)으로 평가합니다. Batch는 풀에 있는 각 노드 형식의 목표 수를 크기 조정 수식에서 평가할 때 지정하는 수로 조정합니다.

### <a name="sample-autoscale-formulas"></a>샘플 자동 크기 조정 수식

다음은 대부분의 시나리오에 맞게 조정할 수 있는 두 가지 자동 크기 조정 수식의 예제입니다. 예제 수식에서 변수 `startingNumberOfVMs` 및 `maxNumberofVMs`는 필요에 따라 조정할 수 있습니다.

#### <a name="pending-tasks"></a>작업 보류 중

이 자동 크기 조정 수식을 사용하면 풀은 단일 VM으로 처음에 생성됩니다. `$PendingTasks` 메트릭은 실행 중이거나 큐에 대기 중인 작업의 수를 정의합니다. 수식은 지난 180초 동안 보류 중인 작업의 평균 수를 찾고 이에 따라 `$TargetDedicatedNodes` 변수를 설정합니다. 수식은 전용 노드의 목표 수가 25개 VM을 절대로 초과하지 않도록 합니다. 새 작업이 제출되면 풀은 자동으로 증가합니다. 작업이 완료되면 VM이 비워지고 자동 크기 조정 수식이 풀을 축소합니다.
...
이 예제에서는 우선 순위가 낮은 25개 노드로 시작하는 풀을 만듭니다. 낮은 우선 순위의 노드가 선점될 때마다 전용 노드로 대체됩니다. 첫 번째 예제와 마찬가지로 `maxNumberofVMs` 변수는 풀이 25개의 VM을 초과하지 않도록 합니다. 이 예제는 우선 순위가 낮은 VM을 활용하는 동시에 풀의 수명 동안 고정된 수의 선점만 발생하도록 보장하는 데 유용합니다.
...
[자동 크기 조정 수식을 만드는 방법](#write-an-autoscale-formula)에 대한 자세한 내용은 이 항목의 뒷부분에 나오는 추가 [자동 크기 조정 수식 예제](#example-autoscale-formulas)를 참조하세요.

## <a name="variables"></a>variables

자동 크기 조정 수식에는 **서비스 정의** 및 **사용자 정의** 변수를 모두 사용할 수 있습니다.

서비스 정의 변수는 Batch 서비스에 기본 제공되어 있습니다. 서비스 정의 변수 일부는 읽기-쓰기이고, 일부는 읽기 전용입니다.

사용자 정의 변수는 사용자가 정의한 변수입니다. 위에 표시된 수식 예제에서 `$TargetDedicatedNodes` 및 `$PendingTasks`는 서비스 정의 변수이고 `startingNumberOfVMs` 및 `maxNumberofVMs`은 사용자 정의 변수입니다.

> [!NOTE]
> 서비스 정의 변수에는 항상 달러($) 기호가 앞에 붙습니다. 사용자 정의 변수의 경우 달러 기호는 선택 사항입니다.

아래 표에서는 Batch 서비스에서 정의하는 읽기-쓰기 및 읽기 전용 변수를 보여 줍니다.

### <a name="read-write-service-defined-variables"></a>읽기-쓰기 서비스 정의 변수

다음과 같은 서비스 정의 변수의 값을 가져오고 설정하여 풀의 컴퓨팅 노드 수를 관리할 수 있습니다.

| 변수 | Description |
| --- | --- |
| $TargetDedicatedNodes |풀에 대한 전용 컴퓨팅 노드의 대상 수입니다. 풀이 항상 원하는 수의 노드에 도달할 수 없으므로 이 수는 목표로 지정됩니다. 예를 들어 풀에서 최초 목표에 도달하기 전에 자동 크기 조정 평가에 따라 전용 노드의 목표 수가 수정되는 경우 풀에서 목표에 도달하지 못할 수 있습니다. <br /><br /> 목표가 Batch 계정 노드 또는 코어 할당량을 초과하는 경우 Batch 서비스 모드에서 만든 계정의 풀이 해당 목표에 도달하지 못할 수 있습니다. 목표가 구독의 공유 코어 할당량을 초과하는 경우 사용자 구독 모드에서 만든 계정의 풀이 해당 목표에 도달하지 못할 수 있습니다.|
| $TargetLowPriorityNodes |풀에 대한 우선 순위가 낮은 컴퓨팅 노드의 목표 수입니다. 풀이 항상 원하는 수의 노드에 도달하지 못할 수 있으므로 이 수는 목표로 지정됩니다. 예를 들어 풀에서 최초 목표에 도달하기 전에 자동 크기 조정 평가에 따라 우선 순위가 낮은 노드의 목표 수가 수정되는 경우 풀에서 목표에 도달하지 못할 수 있습니다. 목표가 Batch 계정 노드 또는 코어 할당량을 초과하는 경우 풀에서 해당 목표에 도달하지 못할 수도 있습니다. <br /><br /> 우선 순위가 낮은 컴퓨팅 노드에 대한 자세한 내용은 [Batch에서 낮은 우선 순위 VM 사용](batch-low-pri-vms.md)을 참조하세요. |
| $NodeDeallocationOption |풀에서 컴퓨팅 노드가 제거되는 경우 발생하는 작업입니다. 가능한 값은 다음과 같습니다.<ul><li>**requeue**: 기본값입니다. 태스크를 즉시 종료하고 일정을 재조정하도록 작업 큐에 다시 배치합니다. 이 작업을 수행하면 목표 노드 수에 최대한 빠르게 도달할 수 있습니다. 그러나 실행 중인 태스크가 중단되고 완전히 다시 시작되어야 하므로 효율성이 떨어질 수 있습니다. <li>**terminate**: 태스크를 즉시 종료하고 작업 큐에서 제거합니다.<li>**taskcompletion**: 현재 실행 중인 태스크가 완료되기를 기다린 다음 풀에서 해당 노드를 제거합니다. 태스크가 중단되고 큐에 대기하여 태스크가 이미 완료한 작업을 낭비하는 것을 방지하려면 이 옵션을 사용합니다.<li>**retaineddata**: 노드의 모든 로컬 태스크 보유 데이터가 정리되기를 기다린 다음 풀에서 해당 노드를 제거합니다.</ul> |

> [!NOTE]
> `$TargetDedicated` 별칭을 사용하여 `$TargetDedicatedNodes` 변수를 지정할 수도 있습니다. 마찬가지로 `$TargetLowPriorityNodes` 변수는 `$TargetLowPriority` 별칭을 사용하여 지정할 수 있습니다. 완전히 명명된 변수와 해당 별칭이 수식에 의해 설정된 경우에는 완전히 명명된 변수에 할당된 값이 우선 적용됩니다.

### <a name="read-only-service-defined-variables"></a>읽기 전용 서비스 정의 변수

이러한 서비스 정의 변수의 값을 가져와서 Batch 서비스에서 메트릭을 기반으로 조정할 수 있습니다.

> [!IMPORTANT]
> 작업 릴리스 태스크는 현재 $ActiveTasks 및 $PendingTasks와 같은 태스크 개수를 제공하는 변수에 포함되지 않습니다. 자동 크기 조정 수식에 따라 작업 릴리스 태스크를 실행하는 데 사용할 수 있는 노드가 없어 노드가 제거될 수 있습니다.
...
| $ActiveTasks |실행할 준비가 되었지만 아직 실행되지 않은 작업 수입니다. 여기에는 활성 상태에 있고 종속성이 충족된 태스크가 모두 포함됩니다. 활성 상태이지만 종속성이 충족되지 않은 작업은 모두 $ActiveTasks 수에서 제외됩니다. 다중 인스턴스 태스크의 경우 $ActiveTasks는 태스크에 설정된 인스턴스 수를 포함합니다.|
...
| $TaskSlotsPerNode |풀의 단일 컴퓨팅 노드에서 동시 태스크를 실행하는 데 사용할 수 있는 태스크 슬롯의 수입니다. |
...
> [!TIP]
> 이러한 읽기 전용 서비스 정의 변수는 각각에 연결된 데이터에 액세스하는 다양한 메서드를 제공하는 '개체'입니다. 자세한 내용은 이 문서의 뒷부분에 나오는 [샘플 데이터 가져오기](#obtain-sample-data)를 참조하세요.
...
- doubleVecList
- 문자열
- timestamp - 다음 멤버를 포함하는 복합 구조입니다.
  - year
  - month (1-12)
  - day (1-31)
...
자동 크기 조정 수식을 정의할 때 이러한 미리 정의된 **함수** 를 사용할 수 있습니다.
...
## <a name="obtain-sample-data"></a>샘플 데이터 가져오기

자동 크기 조정 수식의 핵심 작업은 작업 및 리소스 메트릭 데이터(샘플)를 가져오고 데이터를 기반으로 풀 크기를 조정하는 것입니다. 따라서 자동 크기 조정 수식이 샘플과 상호 작용하는 방식을 명확히 이해해야 합니다.

### <a name="methods"></a>메서드

자동 크기 조정 수식은 Batch 서비스에서 제공하는 메트릭 데이터의 샘플에 대해 작동합니다. 수식은 가져온 값에 따라 풀 크기를 늘리거나 줄입니다. 서비스 정의 변수는 해당 개체에 연결된 데이터에 액세스하는 메서드를 제공하는 개체입니다. 예를 들어, 다음 식은 최근 5분 동안의 CPU 사용률을 얻기 위한 요청을 보여 줍니다.
...
Batch 서비스는 정기적으로 작업 및 리소스 메트릭의 샘플을 가져와 자동 크기 조정 수식에 사용할 수 있도록 합니다. 이러한 샘플은 Batch 서비스에서 30초 마다 기록됩니다. 하지만 일반적으로 이러한 샘플을 기록하는 시간과 자동 크기 조정 수식에서 사용할 수 있을 시간(읽을 수 있는 시간) 사이에 지연이 있습니다. 또한 네트워크 또는 다른 인프라 문제와 같은 요인으로 인해 샘플이 특정 기간에 기록되지 않을 수 있습니다.

### <a name="sample-percentage"></a>샘플 비율
...
예를 들어 10분 TimeSpan을 살펴보겠습니다. 샘플이 이 10분 동안에서 30초마다 기록되므로 Batch에 의해 기록된 샘플의 최대 총 수는 20개 샘플(분당 2개)입니다. 그러나 보고 메커니즘의 고유한 대기 시간 또는 Azure 내의 다른 문제로 인해 자동 크기 조정 수식에서 읽는 데 사용할 수 있는 샘플은 15개뿐일 수 있습니다. 즉 10분 동안 기록하는 샘플의 총 개수 중 75%만 수식에서 사용할 수 있습니다.

### <a name="getsample-and-sample-ranges"></a>GetSample() 및 샘플 범위

자동 크기 조정 수식은 노드를 추가 또는 제거하여 풀을 확장하고 축소합니다. 노드에는 비용이 들기 때문에 수식은 충분한 데이터를 기반으로 지능형 분석 메서드를 사용해야 합니다. 수식에 추세 형식 분석을 사용하는 것이 좋습니다. 이 형식은 수집된 샘플의 범위에 따라 풀을 확장하고 축소합니다.
...
또한 샘플 가용성에 지연 시간이 있을 수 있으므로 시간 범위를 1분 이상인 돌아보기 시작 시간으로 지정해야 합니다. 샘플이 시스템을 통해 전파되는 데 약 1분 정도가 걸리므로 `(0 * TimeInterval_Second, 60 * TimeInterval_Second)` 범위에 있는 샘플은 사용하지 못할 수도 있습니다. 다시 `GetSample()`라는 백분율 매개 변수를 사용하여 특정 샘플 비율 요구 사항을 강제할 수 있습니다.
...
위의 구성 요소를 사용하는 구문을 구성하여 자동 크기 조정 수식을 작성한 다음 해당 구문을 완전한 수식으로 결합합니다. 이 섹션에서는 실제 크기 조정 결정 및 조정을 수행할 수 있는 자동 크기 조정 수식 예제를 만듭니다.

먼저 새 자동 크기 조정 수식에 대한 요구 사항을 정의합니다. 수식은 다음과 같아야 합니다.

- CPU 사용량이 많은 경우 풀에 있는 전용 컴퓨팅 노드의 목표 수를 늘립니다.
- CPU 사용량이 적은 경우 풀에 있는 전용 컴퓨팅 노드의 목표 수를 줄입니다.
- 전용 노드의 최대 수는 항상 400으로 제한합니다.
- 노드 수를 줄이는 경우 태스크를 실행 중인 노드를 제거하지 마세요. 필요한 경우 태스크가 완료될 때까지 기다렸다 노드를 제거합니다.

이 수식의 첫 번째 문은 CPU 사용량이 많은 동안 노드 수를 늘립니다. 지난 10분 동안 CPU 최소 평균 사용량이 70%를 초과한 경우에만 사용자 정의 변수(`$totalDedicatedNodes`)를 전용 노드의 현재 목표 수의 110%인 값으로 채우는 문을 정의합니다. 그렇지 않으면 전용 노드의 현재 수에 대한 값을 사용합니다.
...
CPU 사용량이 적은 동안 노드 수를 줄이려면, 지난 60분 동안 CPU 평균 사용량이 20% 미만인 경우 수식의 다음 문에서 동일한 `$totalDedicatedNodes` 변수를 전용 노드의 현재 목표 수의 90%로 설정합니다. 그렇지 않으면 위 문에 채워진 `$totalDedicatedNodes`의 현재 값을 사용합니다.
...
이제 전용 컴퓨팅 노드의 목표 수를 최대 400으로 제한합니다.

```
$TargetDedicatedNodes = min(400, $totalDedicatedNodes)
```

마지막으로 태스크가 완료될 때까지 노드가 제거되지 않도록 해야 합니다.
...
> [!NOTE]
> 원하는 경우 수식 문자열에 주석과 줄 바꿈을 모두 포함할 수 있습니다. 또한 세미콜론을 생략하면 평가 오류가 발생할 수 있습니다.
...
다음 예제에서는 .NET에서 자동 크기 조정 가능한 풀을 만듭니다. 풀의 자동 크기 조정 수식에서 전용 노드의 목표 수를 월요일에는 5로, 나머지 요일에는 1로 설정합니다. [자동 크기 조정 간격](#automatic-scaling-interval)은 30분으로 설정됩니다. 이 코드 조각과 이 문서의 다른 C# 코드 조각에서 `myBatchClient`는 [BatchClient](/dotnet/api/microsoft.azure.batch.batchclient) 클래스의 올바르게 초기화된 인스턴스입니다.
...
> [!IMPORTANT]
> 자동 크기 조정 가능한 풀을 만들 때는 **CreatePool** 호출에서 _targetDedicatedNodes_ 매개 변수 또는 _targetLowPriorityNodes_ 매개 변수를 지정하지 마세요. 대신 풀에 **AutoScaleEnabled** 및 **AutoScaleFormula** 속성을 지정합니다. 이러한 속성의 값은 각 노드 형식의 목표 수를 결정합니다.
>
> 자동 크기 조정 가능한 풀의 크기를 수동으로 조정하려는 경우(예: [BatchClient.PoolOperations.ResizePoolAsync](/dotnet/api/microsoft.azure.batch.pooloperations.resizepoolasync) 사용) 먼저 풀에서 자동 크기 조정을 사용하지 않도록 설정한 다음 풀의 크기를 조정해야 합니다.

### <a name="python"></a>Python

Python SDK를 사용하여 자동 크기 조정 가능한 풀을 만들려면

1. 풀을 만들고 해당 구성을 지정합니다.
1. 서비스 클라이언트에 풀을 추가합니다.
1. 작성하는 수식을 사용하여 풀에서 자동 크기 조정을 사용하도록 설정합니다.

다음 예제에서는 이러한 단계를 보여줍니다.
...
각 Batch SDK는 자동 크기 조정을 사용하도록 설정하는 방법을 제공합니다. 다음은 그 예입니다.
...
- [풀에서 자동 크기 조정 사용](/rest/api/batchservice/enable-automatic-scaling-on-a-pool)(REST API)

기존 풀에서 자동 크기 조정을 사용하도록 설정하는 경우 다음에 유의하세요.

- 현재 풀에서 자동 크기 조정이 사용하지 않도록 설정된 경우 요청을 발급할 때 유효한 자동 크기 조정 수식을 지정해야 합니다. 선택적으로 자동 크기 조정 간격을 지정할 수 있습니다. 간격을 지정하지 않으면 기본값 15분이 사용됩니다.
- 현재 풀에서 자동 크기 조정이 사용하도록 설정된 경우 새 수식, 새 간격 또는 둘 다를 지정할 수 있습니다. 이러한 속성 중 하나 이상을 지정해야 합니다.
  - 새 자동 크기 조정 간격을 지정하는 경우 기존 일정이 중지되고 새 일정이 시작됩니다. 새 일정의 시작 시간은 자동 크기 조정을 사용하기 위한 요청이 발급된 시간입니다.
  - 자동 크기 조정 수식 또는 간격을 생략하면 Batch 서비스가 해당 설정의 현재 값을 계속 사용합니다.
...
이 C# 예제에서는 [Batch .NET](/dotnet/api/microsoft.azure.batch) 라이브러리를 사용하여 기존 풀에서 자동 크기 조정을 사용하도록 설정합니다.
...
수식은 풀에 적용하기 전에 평가할 수 있습니다. 이렇게 하면 프로덕션 환경에 배치하기 전에 수식의 결과를 테스트할 수 있습니다.

자동 크기 조정 수식을 평가하려면 먼저 한 줄 수식 `$TargetDedicatedNodes = 0`과 같은 유효한 수식으로 풀에서 자동 크기 조정을 사용하도록 설정해야 합니다. 그런 후에 다음 중 하나를 사용하여 테스트할 수식을 평가합니다.
...
이 [Batch .NET](/dotnet/api/microsoft.azure.batch) 예제에서는 자동 크기 조정 수식을 평가합니다. 풀에서 자동 크기 조정을 아직 사용하지 않는 경우 먼저 사용하도록 설정합니다.
...
다음 C# 예제에서는 Batch .NET 라이브러리를 사용하여 _myPool_ 풀에서 마지막으로 실행된 자동 크기 조정에 대한 정보를 출력합니다.
...
## <a name="get-autoscale-run-history-using-pool-autoscale-events"></a>풀 자동 크기 조정 이벤트를 사용하여 자동 크기 조정 실행 기록 가져오기
[PoolAutoScaleEvent](batch-pool-autoscale-event.md)를 쿼리하여 자동 크기 조정 기록을 확인할 수도 있습니다. 이 이벤트는 Batch 서비스가 각 자동 크기 조정 수식 평가 및 실행을 기록하기 위해 내보냅니다. 이는 잠재적 문제를 해결하는 데 도움이 될 수 있습니다.

PoolAutoScaleEvent 샘플 이벤트:
```json
{
    "id": "poolId",
...
`$curTime`은 `TimeZoneInterval_Hour`의 제품에 `time()` 및 UTC 오프셋을 추가하여 현지 표준 시간대를 반영하도록 조정할 수 있습니다. 예를 들어 MDT(산지 일광 절약 시간제)에 `$curTime = time() + (-6 * TimeInterval_Hour);`를 사용합니다. 일광 절약 시간(해당 하는 경우)의 시작과 끝에 오프셋을 조정해야 한다는 점에 유의하세요.

### <a name="example-2-task-based-adjustment"></a>예제 2: 작업 기반 조정

이 C# 예제에서 풀 크기는 큐에 있는 작업의 수에 따라 조정됩니다. 수식 문자열에 주석과 줄 바꿈을 모두 포함했습니다.
...
이 C# 예제에서는 태스크 수에 따라 풀 크기를 조정합니다. 이 수식은 또한 풀에 설정된 [TaskSlotsPerNode](/dotnet/api/microsoft.azure.batch.cloudpool.taskslotspernode) 값을 고려합니다. 이 방법은 [병렬 작업 실행](batch-parallel-node-tasks.md)이 풀에서 사용된 경우에 특히 유용합니다.
...
### <a name="example-4-setting-an-initial-pool-size"></a>예 4: 초기 풀 크기 설정

이 예제에서는 초기 기간 동안 풀 크기를 지정된 노드 수로 설정하는 자동 크기 조정 수식이 있는 C# 예제를 보여 줍니다. 그런 다음 실행 중인 태스크 수와 활성 태스크 수에 따라 풀 크기를 조정합니다.

특히 이 수식은 다음 작업을 수행합니다.
...
- [풀의 컴퓨팅 노드에서 여러 태스크를 동시에 실행](batch-parallel-node-tasks.md)하는 방법을 알아봅니다. 자동 크기 조정과 함께 이 기능은 일부 워크로드에 대한 작업 기간을 줄여서 비용을 절약하는 데 도움이 될 수 있습니다.
- 추가 효율성을 위해 [Azure Batch 서비스를 효율적으로 쿼리](batch-efficient-list-queries.md)하는 방법을 알아봅니다.
