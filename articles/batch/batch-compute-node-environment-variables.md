---
title: 작업 런타임 환경 변수 - Azure 일괄 처리 | 마이크로 소프트 문서
description: Azure 일괄 처리 분석에 대한 작업 런타임 환경 변수 지침 및 참조입니다.
services: batch
author: LauraBrenner
manager: evansma
ms.assetid: ''
...
ms.locfileid: "80053943"
---
# <a name="azure-batch-runtime-environment-variables"></a>Azure 일괄 처리 런타임 환경 변수
...
일괄 처리를 사용하여 환경 변수를 사용하는 자세한 내용은 [작업에 대한 환경 설정을](https://docs.microsoft.com/azure/batch/batch-api-basics#environment-settings-for-tasks)참조하십시오.
...
환경 변수의 현재 값을 얻으려면 `cmd.exe` Windows 계산 노드 또는 `/bin/sh` Linux 노드에서 시작하십시오.
...
| AZ_BATCH_APP_PACKAGE            | 모든 앱 패키지 환경 변수의 접두사입니다. 예를 들어 응용 프로그램 "FOO" 버전 "1"이 풀에 설치된 경우 환경 변수는 AZ_BATCH_APP_PACKAGE_FOO_1(Linux) 또는 AZ_BATCH_APP_PACKAGE_FOO#1(Windows)입니다. AZ_BATCH_APP_PACKAGE_FOO_1 패키지가 다운로드된 위치(폴더)를 가리킵니다. 앱 패키지의 기본 버전을 사용하는 경우 버전 번호 없이 AZ_BATCH_APP_PACKAGE 환경 변수를 사용합니다. Linux에서 응용 프로그램 패키지 이름이 "에이전트-linux-x64"이고 버전이 "1.1.46.0"인 경우 환경 이름은 실제로 밑줄과 소문자를 사용하여 AZ_BATCH_APP_PACKAGE_agent_linux_x64_1_1_46_0. 자세한 내용은 [여기](https://docs.microsoft.com/azure/batch/batch-application-packages#execute-the-installed-applications)를 참조하세요. | 관련 앱 패키지가 있는 모든 작업입니다. 또한 노드 자체에 애플리케이션 패키지가 있는 경우 모든 작업에 사용할 수 있습니다. | AZ_BATCH_APP_PACKAGE_FOO_1 (리눅스) 또는 AZ_BATCH_APP_PACKAGE_FOO #1 (윈도우) |
...
| AZ_BATCH_CERTIFICATES_DIR       | Linux 컴퓨팅 노드에 대한 인증서가 저장된 [태스크 작업 디렉터리][files_dirs] 내 디렉터리입니다. 이 환경 변수는 Windows 계산 노드에 적용되지 않습니다.                                                  | 모든 태스크입니다.   |  /mnt/batch/tasks/workitems/batchjob001/job-1/task001/certs |
...
| AZ_BATCH_JOB_PREP_WORKING_DIR   | 노드에서 [디렉터리로 작업하는][files_dirs] 작업 준비 작업의 전체 경로입니다. | 시작 태스크 및 작업 준비 태스크를 제외한 모든 태스크입니다. 작업에 작업 준비 태스크가 구성된 경우에만 사용할 수 있습니다. | C:\user\tasks\workitems\jobprepreleasesamplejob\job-1\jobpreparation\wd |
| AZ_BATCH_MASTER_NODE            | [다중 인스턴스 태스크][multi_instance]의 기본 태스크가 실행되는 컴퓨팅 노드의 IP 주소와 포트입니다. | 다중 인스턴스 기본 및 하위 태스크입니다. | `10.0.0.4:6000` |
...
| AZ_BATCH_NODE_MOUNTS_DIR        | 모든 마운트 디렉터리가 있는 노드 수준 파일 시스템의 전체 [경로는](virtual-file-mount.md) 위치를 탑재합니다. Windows 파일 공유는 드라이브 문자를 사용하므로 Windows의 경우 마운트 드라이브는 장치 및 드라이브의 일부입니다.  |  시작 작업을 포함한 모든 작업은 사용자가 마운트 디렉터리에 대한 마운트 권한을 알고 있는 경우 사용자에게 액세스할 수 있습니다. | 예를 들어 우분투에서 위치는 다음과 같은 위치에 있습니다.`/mnt/batch/tasks/fsmounts` |
| AZ_BATCH_NODE_ROOT_DIR          | 노드의 모든 [Batch 디렉터리][files_dirs] 루트의 전체 경로입니다. | 모든 태스크입니다. | C:\user\tasks |
| AZ_BATCH_NODE_SHARED_DIR        | 노드에서 공유 [디렉터리전체][files_dirs] 경로입니다. 노드에서 실행되는 모든 태스크는 이 디렉터리에 대한 읽기/쓰기 권한이 있습니다. 다른 노드에서 실행되는 태스크는 이 디렉터리에 대한 원격 액세스 권한이 없습니다("공유" 네트워크 디렉터리가 아님). | 모든 태스크입니다. | C:\user\tasks\shared |
...
| AZ_BATCH_TASK_DIR               | 노드에서 작업 [디렉터리전체][files_dirs] 경로입니다. 이 디렉터리에는 태스크에 대한 `stdout.txt` 및 `stderr.txt`, 그리고 AZ_BATCH_TASK_WORKING_DIR이 포함됩니다. | 모든 태스크입니다. | C:\user\tasks\workitems\batchjob001\job-1\task001 |
| AZ_BATCH_TASK_ID                | 현재 태스크의 ID | 시작 태스크를 제외한 모든 태스크입니다. | task001 |
| AZ_BATCH_TASK_SHARED_DIR | [다중 인스턴스 태스크][multi_instance]의 기본 태스크 및 모든 하위 태스크일 경우 동일한 디렉터리 경로입니다. 경로는 다중 인스턴스 태스크가 실행되며 해당 노드에서 실행 중인 태스크 명령에 읽기/쓰기 액세스 권한을 가진 모든 노드에 존재합니다([조정 명령][coord_cmd]과 [애플리케이션 명령][app_cmd] 둘 다). 하위 작업 또는 다른 노드에서 실행되는 기본 작업은 이 디렉터리에 대한 원격 액세스 권한이 없습니다("공유" 네트워크 디렉터리아님). | 다중 인스턴스 기본 및 하위 태스크입니다. | C:\user\tasks\workitems\multiinstancesamplejob\job-1\multiinstancesampletask |
