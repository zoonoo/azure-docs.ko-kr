...
일괄 처리 템플릿은 풀, 작업, 태스크 및 기타 항목을 만들 때 속성 값을 지정 하기 위해 JSON 파일에 대 한 [Azure CLI](batch-cli-get-started.md#json-files-for-resource-creation) 의 기존 일괄 처리 지원을 기반으로 합니다. Batch 템플릿은 다음과 같은 기능을 추가합니다.
...
-   **매개 변수**
    ...
    -   템플릿이 사용될 때 제공해야 하는 매개 변수 값으로 속성 값을 본문 섹션에서 지정할 수 있습니다. 예를 들어 풀에 대 한 전체 정의를 본문에 배치 하 고에 대해 `poolId`정의 된 매개 변수를 하나만 지정할 수 있습니다. 따라서 풀을 만들려면 풀 ID 문자열만 제공 해야 합니다.
...
CLI는 `poolId` 및 `nodeCount` 매개 변수에 대한 값을 제공하라는 메시지를 표시합니다. JSON 파일의 매개 변수를 제공할 수도 있습니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
...
1. Batch Explorer에서 **갤러리** > **로컬 템플릿**을 선택 합니다.
