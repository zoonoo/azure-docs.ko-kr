...
**지침**: 사용할 수 없습니다. Azure 일괄 처리에 대해 아직 지원되지 않는 고객 잠금 상자입니다. 고객 잠금 상자 지원 서비스 목록:https://docs.microsoft.com/azure/security/fundamentals/customer-lockbox-overview#supported-services-and-scenarios-in-general-availability
...
Azure 리소스 그래프 탐색기를 통해 클래식 Azure 리소스를 검색할 수 있지만 앞으로 AZURE 리소스 관리자(Arm) 리소스를 만들고 사용하는 것이 좋습니다.


Azure 리소스 그래프 탐색기를 사용하여 쿼리를 만드는 방법:
...
Azure 리소스 그래프 탐색기를 사용하여 쿼리를 만드는 방법:https://docs.microsoft.com/azure/governance/resource-graph/first-query-portal
...
- 서브넷은 네트워크 보안 그룹 -저장소 계정 가상 네트워크 서비스 엔드포인트를 사용해야 합니다.
