...
[Azure portal][portal] 또는 [Batch 관리 .NET](batch-management-dotnet.md)라이브러리를 사용하여 Batch 계정에서 응용 프로그램 패키지를 관리합니다. 다음 섹션에서는 먼저 Storage 계정을 연결하는 방법을 보여 준 다음 응용 프로그램 및 패키지를 추가하고 포털에서 관리하는 방법을 설명합니다.

### <a name="link-a-storage-account"></a>Storage 계정 연결
응용 프로그램 패키지를 사용하려면 먼저 Azure Storage 계정을 Batch 계정에 연결해야 합니다. Storage 계정을 아직 구성하지 않은 경우 **Batch 계정** 블레이드에서 **응용 프로그램** 타일을 처음으로 클릭하면 Azure Portal에서 경고를 표시합니다.
...
Batch 서비스는 연결된 Storage 계정을 사용하여 응용 프로그램 패키지를 저장합니다. 계정 두 개를 연결한 후에 Batch는 연결된 Storage 계정에 저장된 패키지를 계산 노드에 자동으로 배포할 수 있습니다. Storage 계정을 Batch 계정에 연결하려면 **경고** 블레이드에서 **Storage 계정 설정**을 클릭한 다음 **Storage 계정** 블레이드에서 **Storage 계정**을 클릭합니다.

![Azure portal에서 저장소 계정 블레이드 선택][10]

*특별히* Batch 계정과 함께 사용할 Storage 계정을 만들었으면 여기에서 선택하는 것이 좋습니다. Storage 계정을 만드는 방법에 대한 자세한 내용은 [Azure storage 계정 정보](../storage/common/storage-create-storage-account.md)의 "Storage 계정 만들기"를 참조하세요. Storage 계정을 만든 후에 **Storage 계정** 블레이드를 사용하여 Batch 계정에 연결할 수 있습니다.
