...
>
> 응용 프로그램 패키지를 만들고 관리 하는 api는 [Batch 관리 .net][api_net_mgmt] library. The APIs for installing application packages on a compute node are part of the [Batch .NET][api_net] 라이브러리의 일부입니다. 다른 언어의 경우 상응하는 기능을 Batch API에서 제공합니다. 
>
...
Batch .net에서 새 풀을 만들 때 또는 기존 풀에 대해 하나 이상의 [cloudpool][net_cloudpool] .[ApplicationPackageReferences][net_cloudpool_pkgref] 을 지정 합니다. [ApplicationPackageReference][net_pkgref] 클래스는 풀의 계산 노드에 설치할 애플리케이션 ID 및 버전을 지정합니다.
...
작업 응용 프로그램 패키지를 설치 하려면 태스크의 [cloudtask][net_cloudtask] .[ApplicationPackageReferences][net_cloudtask_pkgref] 속성을 구성 합니다.
...
이 예에서 기존 풀에는 [Cloudpool][net_cloudpool].[ApplicationPackageReferences][net_cloudpool_pkgref]중 하나로 구성 된 *blender* 응용 프로그램의 버전 2.7이 있습니다. 풀의 노드를 2.76b 버전으로 업데이트하려면 새 버전을 사용하여 [ApplicationPackageReference][net_pkgref]를 지정하고 변경 내용을 적용합니다.
...
[Applicationoperations][net_appops] .[ListApplicationSummaries][net_appops_listappsummaries] 메서드를 사용 하 여 Batch 계정에 응용 프로그램 및 해당 패키지를 나열할 수 있습니다.
...
* REST API를 사용 하 여 설치할 패키지를 지정 하는 방법에 대 한 자세한 내용은 [계정에 풀 추가][rest_add_pool] 의 [Batch REST API][api_rest] also provides support to work with application packages. For example, see the [applicationPackageReferences][rest_add_pool_with_packages] 요소를 사용 합니다. 일괄 처리 REST API를 사용 하 여 응용 프로그램 정보를 얻는 방법에 대 한 자세한 내용은 [응용 프로그램][rest_applications] 을 참조 하십시오.
