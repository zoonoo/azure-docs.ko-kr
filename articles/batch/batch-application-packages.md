...
Azure 포털을 사용하여 응용 프로그램 패키지를 추가, 업데이트 및 삭제하고, 각 응용 프로그램의 기본 버전을 구성할 수 있습니다. 현재 이러한 작업은 Azure 포털에서만 지원됩니다.
...
배치 서비스는 연결된 저장소 계정을 사용하여 응용 프로그램 패키지를 저장 및 검색합니다. 계정 두 개를 연결하면 배치가 연결된 저장소 계정에 저장된 패키지를 계산 노드에 자동으로 배포할 수 있습니다. *경고* 블레이드에서 **저장소 계정 설정**을 클릭한 다음 *저장소 계정* 블레이드에서 **저장소 계정**을 클릭하여 기존 저장소 계정을 배치 계정에 연결합니다.

![Azure 포털에서 저장소 계정 블레이드 선택][10]

구독 내에서 기존 저장소 계정을 사용하거나 응용 프로그램 패키지 저장소에 대한 새 계정을 만들 수 있습니다. 저장소 계정을 만드는 방법에 대한 자세한 내용은 [Azure 저장소 계정 정보](../storage/storage-create-storage-account.md)의 "저장소 계정 만들기"를 참조하세요. 저장소 계정을 만든 후에는 *저장소 계정* 블레이드를 사용하여 배치 계정에 연결합니다.
...
**응용 프로그램 ID** 및 **버전** 텍스트 상자에 값을 직접 입력하여 응용 프로그램 메타데이터를 수동으로 제공할 수도 있고, 또는 이 메타데이터가 포함된 JSON 파일을 업로드할 수도 있습니다. 응용 프로그램 ID 및 버전을 수동으로 지정하려면 **메타데이터** 드롭다운 선택기를 **메타데이터 입력**(기본값)으로 그대로 두고 **응용 프로그램 ID** 및 **버전** 텍스트 상자에 수동으로 값을 입력합니다.

패키지의 ID 및 버전이 포함된 JSON 형식의 메타데이터 파일을 지정하려면 **메타데이터** 드롭다운에서 **메타데이터 파일 업로드**를 선택합니다.

![메타데이터 파일 업로드 드롭다운 선택기][6]

그런 다음, 나타나는 **메타데이터 파일** 텍스트 상자 옆에 있는 폴더 아이콘을 클릭하고 JSON 데이터가 포함된 로컬 파일을 찾아봅니다. 이 예에서는 `litware_1.1001.2b.json` 파일을 업로드하기로 선택했으며, **응용 프로그램 ID** 및 **버전** 텍스트 상자가 해당 파일의 정보로 자동으로 채워집니다.

![메타데이터 파일 선택 세부 사항][13]

다음 JSON 형식을 사용하여 파일에서 응용 프로그램 패키지 메타데이터를 지정합니다.
...
> [AZURE.NOTE] ID 및 버전에 대한 JSON 메타데이터 파일을 업로드하는 경우 "응용 프로그램 ID" 또는 "버전" 텍스트 상자는 JSON 파일의 데이터로 자동으로 채워지므로 편집할 필요가 *없습니다*.
...
보시다시피, 비활성화된 "응용 프로그램 ID" 텍스트 상자를 제외하고, 필드가 *새 응용 프로그램* 블레이드의 필드와 일치합니다. 위와 마찬가지로, 새 패키지의 **버전**을 지정하고, **응용 프로그램 패키지** ZIP 파일을 찾은 다음 **확인**을 클릭하여 패키지를 업로드합니다.
...
**삭제**를 클릭하면 패키지 버전을 정말로 삭제할 것인지 묻는 메시지가 나타나고, 배치가 Azure 저장소에서 패키지를 삭제합니다. 응용 프로그램의 기본 버전을 삭제하면 해당 응용 프로그램의 기본 버전 설정이 제거됩니다.
...
풀의 계산 노드에 응용 프로그램 패키지를 설치하려면 풀에 대해 응용 프로그램 패키지 *참조*를 하나 이상 지정해야 합니다. 배치 .NET에서, 풀을 만들 때 또는 기존 풀에 하나 이상의 [CloudPool][net_cloudpool].[ApplicationPackageReferences][net_cloudpool_pkgref]를 추가하면 됩니다.
...
[6]: ./media/batch-application-packages/app_pkg_06.png "메타데이터 파일 업로드 드롭다운 선택기"
