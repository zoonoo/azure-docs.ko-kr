...
* Batch 서비스는 작업 예약을 위해 풀 컴퓨팅 노드와 통신해야 합니다. 이 통신을 사용하려면 Batch 계정이 존재하는 지역의 Batch 서비스에서 사용하는 각 IP 주소에 대해 사용자 정의 경로를 추가해야 합니다. Batch 서비스의 IP 주소 목록을 가져오는 방법에 대해 알아보려면 [온-프레미스 서비스 태그](../virtual-network/service-tags-overview.md)를 참조 하세요. Batch 서비스 IP 주소는 `BatchNodeManagement` 서비스 태그 (또는 batch 계정 지역과 일치 하는 지역 변형)와 연결 됩니다.
...
> [!WARNING]
> Batch 서비스 IP 주소는 시간이 지남에 따라 변경 될 수 있습니다. IP 주소 변경으로 인 한 가동 중단을 방지 하려면 Batch 서비스 IP 주소를 자동으로 새로 고치는 정기적인 프로세스를 설정 하 고 경로 테이블에서 최신 상태로 유지 하는 것이 좋습니다.
