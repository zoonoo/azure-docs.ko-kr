...
> [!IMPORTANT]
> Batch 서비스 버전 2017-01-01.4.0에서는 해당 버전 이상을 호출하기 위해 코드를 업데이트해야 하는 변경 사항이 추가되었습니다. Batch 코드를 구 버전에서 업데이트하기 위한 빠른 가이드라인을 보려면 [최신 Batch 클라이언트 라이브러리로 코드 업데이트](#update-your-code-to-the-latest-batch-client-library) 섹션을 참조하세요.
...
기본적으로 태스크는 자동 사용자 계정에서 Batch로, 관리자 액세스 권한이 없는 표준 사용자로, 풀 범위에서 실행됩니다. 풀 범위는 태스크가 풀의 모든 작업에 사용할 수 있는 자동 사용자 계정에서 실행됨을 의미합니다. 풀 범위에 대한 자세한 내용은 [풀 범위에서 자동 사용자로 태스크 실행](#run-a-task-as-an-auto-user-with-pool-scope)을 참조하세요.

풀 범위의 대안으로 태스크 범위가 있습니다. 태스크 범위에 대해 자동 사용자 지정이 구성되면 Batch 서비스는 해당 태스크에 대해서만 자동 사용자 계정을 만듭니다.

자동 사용자 지정에는 각각 고유한 자동 사용자 계정에 해당하는 4개의 구성이 있을 수 있습니다.

- 태스크 범위를 사용하는 비관리자 액세스
...
> [!NOTE]
> 필요한 경우에만 관리자 계정을 사용하십시오. 모범 사례는 원하는 결과를 달성하는 데 필요한 최소 권한을 부여하는 것입니다. 예를 들어 시작 태스크가 모든 사용자가 아닌 현재 사용자에 대해 소프트웨어를 설치하는 경우 태스크에 관리자 권한 액세스를 부여하지 않도록 할 수 있습니다. 시작 태스크를 포함하여 동일한 계정으로 실행해야 하는 모든 태스트에 대해 풀 범위 및 비관리자 액세스 권한에 대한 자동 사용자 지정을 구성할 수 있습니다.
...
> [!NOTE]
> 2개의 풀 전체 자동 사용자 계정은 별도 계정입니다. 풀 전체 관리 계정에서 실행되는 태스크는 표준 계정에서 실행되는 태스크와 데이터를 공유할 수 없으며, 그 반대의 경우도 마찬가지입니다.
...
서비스 버전 2017-01-01.4.0인 Batch에는 중요한 변경 내용이 추가되었으며, 이전 버전에서 사용할 수 있었던 **runElevated** 속성이 **userIdentity** 속성으로 바뀌었습니다. 다음 표에서는 이전 버전의 클라이언트 라이브러리에서 코드를 업데이트하는 데 사용할 수 있는 간단한 매핑을 제공합니다.
