<properties
	...
	description="Azure 일괄 처리 및 간단한 시나리오를 사용 하 여 일괄 처리 서비스에 대 한 개발 하는 방법에 대 한 기본 개념에 알아봅니다."
	services="batch"
...
-	[.NET 용 Azure 일괄 처리 라이브러리 자습서 1:](#tutorial1)
-	[.NET 용 자습서 2: 일괄 처리 응용 프로그램을 Azure 라이브러리](#tutorial2)  


배경 정보 및 Azure 일괄 처리에 대 한 시나리오의 경우 참조 [Azure 일괄 처리 기술 개요](batch-technical-overview.md).

##<a name="tutorial1"></a>.NET 용 Azure 일괄 처리 라이브러리 자습서 1:
...
>
>NuGet을 가져오는데 사용할 필요는 **Microsoft.Azure.Batch.dll** 어셈블리입니다. Visual Studio에서 프로젝트를 만든 후에 프로젝트를 마우스 오른쪽 단추로 **솔루션 탐색기** 선택 하 고 **NuGet 패키지 관리**. 온라인에서 검색할 **Azure.Batch** Azure 일괄 처리 패키지와 종속성을 설치 하 여 설치를 클릭 하 고 있습니다.
>
...
>
>또한를 참조할 수 있습니다는 [Azure 일괄 처리 Hello World 예제](https://code.msdn.microsoft.com/Azure-Batch-Sample-Hello-6573967c) msdn 샘플은 여기에서 설명 하는 코드와 비슷합니다.
...
-	**계정** -는 서비스 내에서 엔터티를 고유 하 게 식별 합니다. 모든 처리는 배치 계정을 통해 수행됩니다.
-	**풀** -작업 실행 작업 응용 프로그램에 있는 가상 컴퓨터의 컬렉션입니다.
-	**가상 컴퓨터 작업** -풀에 할당 된 풀에서 실행 되는 작업에 할당 된 작업에서 사용 되 고 있는 컴퓨터입니다.
-	**작업 가상 컴퓨터 사용자** – 작업 가상 컴퓨터에 대 한 사용자 계정이 있습니다.
-	**Workitem** -계산 된 풀의 작업 가상 컴퓨터에서 수행 되는 방법을 지정 합니다.
-	**작업** 는-작업 항목의 인스턴스를 실행 하 고 작업의 컬렉션으로 구성 됩니다.
-	**작업** -작업 가상 컴퓨터에서 실행 하는 작업과 연결 된 응용 프로그램입니다.
-	**파일** – 작업에서 처리 되는 정보를 포함 합니다.  

가장 기본적인 사용 방식부터 살펴보겠습니다.

###Azure 배치 계정 만들기
관리 포털을 사용하여 배치 계정을 만들 수 있습니다. 키는 계정이 만들어진 후에 제공됩니다. 자세한 내용은 참조 [Azure 일괄 처리 기술 개요](batch-technical-overview.md).

###방법: 풀 계정에 추가
태스크용 가상 컴퓨터 풀은 태스크를 실행하려고 할 때 만들어야 하는 첫 번째 리소스 집합입니다.

1.	Microsoft Visual Studio 2013에서 엽니다는 **파일** 메뉴를 클릭 하 여 **새로**, 클릭 하 고 **프로젝트**.

2.	 **Windows**, 아래에서 **Visual C#**, 를 클릭 하 여 **콘솔 응용 프로그램**, 프로젝트 이름을 **GettingStarted**, 솔루션의 이름을 **AzureBatch**, 클릭 하 고 **확인**.
...
4.	Program 클래스에 다음 변수를 추가합니다.
	...
	다음 값을 바꿉니다:- **[풀의 이름을]** -풀. 사용 하려는 이름- **[이름의 일괄 처리 계정]** -일괄 처리 계정. 이름- **[키의 일괄 처리 계정]** -일괄 처리 계정에 대 한 제공 된 키입니다.
...
7.	풀이 없는 경우 풀을 만드는 다음 코드를 Main에 추가합니다.
		...
		Console.ReadLine();
8.	프로그램을 저장한 후 실행합니다. 상태는 **활성** 성공적으로 추가 하는 풀에 있습니다.  
...
2.	작업 항목이 만들어지면 작업도 만들어집니다. 작업 항목에 이름을 할당할 수 있지만 작업의 이름을 항상 할당은 **작업 0000000001**. 작업 항목을 추가하는 다음 코드를 Main(작업 항목 나열 코드 앞)에 추가합니다.
		...
		Console.ReadLine();
3.	프로그램을 저장한 후 실행합니다. 상태는 **활성** 성공적으로 추가 된 작업 항목에 대 한 합니다. 다음 출력이 표시되어야 합니다.
...
1.	솔루션 탐색기에서 라는 새 콘솔 응용 프로그램 프로젝트를 만듭니다 **ProcessTaskData** 에 **AzureBatch** 솔루션입니다.
...
####저장소 계정 가져오기
이 자습서의 나머지 부분을 계속 완료하려면 저장소 계정이 필요합니다. 이 작업을 수행 하는 방법을 모르면 참조 [Azure 저장소 계정 만들기](#tutorial1_storage).
...
>[AZURE.NOTE]Azure 저장소 팀에는 [블로그 게시물](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) 파일을 업로드 하는 데 도움이 되는 Azure 저장소 탐색기를 나열 합니다.
...
1.	Program 클래스에 다음 변수를 추가합니다.
	...
	다음 값을 바꿉니다:- **[저장소 경로]** -저장소에서 blob에 대 한 경로입니다. 예: http://yiding.blob.core.windows.net/gettingstarted/
...
2.	탐색 창의 아래쪽 클릭 **새로**. ![][1]
3.	클릭 하 여 **데이터 서비스**, 다음 **저장소**, 를 클릭 하 고 **빠른 생성**. ![][2]

4.	 **URL**, 저장소 계정에 대 한 URI에 사용할 하위 도메인 이름을 입력 합니다. 이 입력에는 3-24자의 소문자와 숫자를 사용할 수 있습니다. 이 값은 구독에 대한 Blob, 큐 또는 테이블 리소스의 주소를 지정하는 데 사용되는 URI 내의 호스트 이름이 됩니다.
5.	선택 된 **위치/선호도 그룹** 저장소를 찾을 수 있는 합니다.
6.	선택적으로, 지역에서 복제를 사용하도록 설정할 수 있습니다.
7.	**CREATE STORAGE ACCOUNT**를 클릭합니다.  

Azure 저장소에 대 한 자세한 내용은 참조 [.NET에서 Azure Blob 저장소 서비스를 사용 하는 방법을](http://azure.microsoft.com/develop/net/how-to-guides/blob-storage/).


##<a name="tutorial2"></a>.NET 용 자습서 2: Azure 일괄 처리 응용 프로그램 라이브러리
...
>[AZURE.NOTE]이 자습서를 완료하려면 Azure 계정이 필요합니다. 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 체험](http://azure.microsoft.com/pricing/free-trial/)을 참조하십시오. NuGet을 사용 하 여를 모두 가져올 수는 <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps.Cloud/">일괄 처리 앱 클라우드</a> 어셈블리 및 <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps/">일괄 처리 응용 프로그램 클라이언트</a> 어셈블리. Visual Studio에서 프로젝트를 만든 후에 프로젝트를 마우스 오른쪽 단추로 **솔루션 탐색기** 선택 하 고 **NuGet 패키지 관리**. 프로젝트 템플릿을 사용 클라우드 응용 프로그램 및 응용 프로그램을 배포 하는 기능을 포함 하는 일괄 처리 응용 프로그램에 대 한 Visual Studio 확장을 다운로드할 수도 있습니다 <a href="https://visualstudiogallery.msdn.microsoft.com/8b294850-a0a5-43b0-acde-57a07f17826a">여기</a> 또는 검색을 통해 **일괄 처리 응용 프로그램** 확장 및 업데이트 메뉴 항목을 통해 Visual Studio에서. 찾을 수도 있습니다 <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">msdn-종단간 예제입니다.</a>
>
...
3.	Azure에서 사용하도록 설정된 응용 프로그램에 작업을 제출하는 방법을 제공합니다. 이 프로세서는 응용 프로그램 UI 또는 웹 포털의 플러그 인이거나 실행 파이프라인의 일부로 포함된 무인 서비스일 수도 있습니다. 참조는 <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">샘플</a> msdn에 대 한 예입니다.
...
####작업
A **작업** 은 사용자가 제출한 작업 합니다. 작업이 제출될 때 사용자는 작업 유형, 해당 작업에 대한 설정 및 작업에 필요한 데이터를 지정합니다. 활성화된 구현이 사용자 대신 이러한 세부 정보를 확인할 수도 있고, 경우에 따라 사용자가 클라이언트를 통해 명시적으로 이 정보를 제공할 수도 있습니다. 작업에는 반환되는 결과가 있습니다. 각 작업에는 기본 출력이 있으며 필요에 따라 미리 보기 출력이 있습니다. 원하는 경우 작업이 추가 출력을 반환할 수도 있습니다.

####작업
A **작업** 작업 작업의 일부로 수행 됩니다. 사용자가 작업을 제출하는 경우 더 작은 태스크로 세분화됩니다. 그러면 서비스는 이러한 개별 태스크를 처리한 다음 태스크 결과를 전반적인 작업 출력으로 조합합니다. 태스크의 특성은 작업 종류에 따라 달라집니다. 작업 분할자는 응용 프로그램이 처리하도록 디자인된 작업 청크에 대한 지식을 바탕으로 작업을 태스크로 세분화하는 방법을 정의합니다. 태스크 출력은 개별적으로 다운로드 가능하며 사용자가 애니메이션 작업에서 개별 태스크를 다운로드하려는 경우와 같은 상황에서 유용할 수 있습니다.

####병합 태스크
A **병합 작업** 최종 작업 결과에 개별 작업의 결과 조합 하는 작업의 특별 한 종류입니다. 영화 렌더링 작업의 경우 병합 태스크는 렌더링된 프레임을 한 편의 영화로 조합하거나 렌더링된 모든 프레임을 단일 파일로 zip할 수 있습니다. 모든 작업에는 실제 '병합'이 필요하지 않더라도 병합 태스크가 있습니다.

####파일
A **파일** 는 작업에 대 한 입력으로 사용 되는 데이터입니다. 작업에는 연결된 입력 파일이 없거나 하나 또는 여러 개 있을 수 있습니다. 동일한 파일을 작업을 렌더링 하는 영화에 대 한 예:도 여러 작업에 사용 될 수 있습니다, 그리고 질감, 모델 등의 파일 수 있습니다. 데이터 분석 작업을 파일 집합 관찰 또는 측정 될 수 있습니다.
