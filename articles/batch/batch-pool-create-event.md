...
description: 풀을 만든 후에 내보내는 Batch 풀 만들기 이벤트에 대 한 참조입니다. 로그의 내용은 풀에 대한 일반 정보를 노출합니다.
ms.topic: article
ms.date: 04/20/2017
ms.author: labrenne
...
# <a name="pool-create-event"></a>풀 만들기 이벤트
 ...
 다음 예에서는 속성을 `CloudServiceConfiguration` 사용 하 여 만든 풀에 대 한 풀 만들기 이벤트의 본문을 보여 줍니다.
...
|`imageType`|문자열|이미지에 대 한 배포 방법입니다. 지원 되는 `virtualMachineConfiguration` 값은 또는입니다.`cloudServiceConfiguration`|
...
|`targetOSVersion`|문자열|풀의 가상 머신에 설치할 Azure 게스트 OS 버전입니다.<br /><br /> 기본값은 지정 된 **\*** 제품군의 최신 운영 체제 버전을 지정 하는입니다.<br /><br /> 기타 허용된 값은 [Azure 게스트 OS 릴리스](https://azure.microsoft.com/documentation/articles/cloud-services-guestos-update-matrix/#releases)를 참조하세요.|
...
|`nodeAgentId`|문자열|컴퓨팅 노드에서 프로비전되는 Batch 노드 에이전트의 SKU입니다.|
