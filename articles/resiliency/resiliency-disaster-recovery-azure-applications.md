---
title: Azure 응용 프로그램에 대한 재해 복구 | Microsoft Docs
...
다음 다이어그램에서 사용자는 Traffic Manager에 지정된 URL(**http://myATMURL.trafficmanager.net**)에 연결하고 실제 사이트 URL(**http://app1URL.cloudapp.net** 및 **http://app2URL.cloudapp.net**)을 추상화합니다. 사용자를 라우팅할 시기에 대한 조건을 구성하는 방법에 따라 정책이 요구하는 경우 사용자를 올바른 실제 사이트로 전송합니다. 정책 옵션에는 라운드 로빈, 성능 또는 장애 조치가 있습니다. 이 문서에서는 장애 조치 옵션에 대해서만 알아보겠습니다.
...
> [!NOTE]
> 이 프로세스는 사용자가 제어할 수 없으며 지역 전체 서비스 중단에 대해서만 발생합니다. 이 때문에 가장 높은 수준의 가용성을 달성하기 위해 다른 응용 프로그램에 특정된 백업 전략에 의존해야 합니다. 자세한 내용은 [재해 복구를 위한 데이터 전략](#data-strategies-for-disaster-recovery)의 섹션을 참조하세요.
> 
> 
