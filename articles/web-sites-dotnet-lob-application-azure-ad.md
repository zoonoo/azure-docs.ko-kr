<properties 
	pageTitle="Azure AD를 사용하는 Azure 웹 사이트의 LOB 응용 프로그램" 
	description="Azure 웹 사이트에서 Azure Active Directory로 인증하는 ASP.NET LOB 응용 프로그램을 만드는 방법에 대해 알아봅니다." 
...
# Azure 웹 사이트에서 Azure Active Directory로 인증하는 ASP.NET MVC LOB(기간 업무) 응용 프로그램 만들기 #

이 문서에서는 [Azure 웹 사이트](http://azure.microsoft.com/services/websites/)에서 [Azure Active Directory](http://azure.microsoft.com/services/active-directory/)(AAD)를 ID 공급자로 사용하는 ASP.NET MVC LOB(기간 업무) 응용 프로그램을 쉽게 만드는 방법에 대해 알아봅니다. 또한 [Azure AD Graph 클라이언트 라이브러리](http://blogs.msdn.com/b/aadgraphteam/archive/2014/06/02/azure-active-directory-graph-client-library-1-0-publish.aspx)를 사용하여 응용 프로그램의 디렉터리 데이터를 쿼리하는 방법에 대해 알아봅니다.
...
Azure 웹 사이트에서 다음 기능을 통해 작업 항목을 추적하는 간단한 LOB CRUD(만들기-읽기-업데이트-삭제) 응용 프로그램을 빌드합니다.

- AAD에 대해 사용자 인증
- 로그인 및 로그아웃 기능
- `[Authorize]`를 사용하여 사용자에게 여러 CRUD 작업에 대한 권한 부여
- [Azure AD Graph API](http://msdn.microsoft.com/library/azure/hh974476.aspx)를 사용하여 AAD 데이터 쿼리
- WIF(Windows Identity Foundation) 대신 [Microsoft.Owin](http://www.asp.net/aspnet/overview/owin-and-katana/an-overview-of-project-katana)(ASP.NET의 미래이며, WIF보다 인증 및 권한 부여 설정이 훨씬 간편함) 사용
...
- 여러 그룹의 사용자가 포함된 AAD 테넌트
- AAD 테넌트에서 응용 프로그램을 만들 수 있는 권한
- Visual Studio 2013 

<a name="bkmk_sample"></a>
## LOB 템플릿에 예제 응용 프로그램 사용 ##

이 자습서의 예제 응용 프로그램 [WebApp-GroupClaims-DotNet](https://github.com/AzureADSamples/WebApp-GroupClaims-DotNet)은 AAD 팀이 만들었으며, 새 LOB 응용 프로그램을 간편하게 만들 수 있는 템플릿으로 사용될 수 있습니다. 기본 제공 기능은 다음과 같습니다.

- [OpenID Connect](http://openid.net/connect/)를 사용하여 AAD로 인증
- AAD 검색 필터가 포함되어 있으며 AAD 사용자 또는 그룹을 응용 프로그램 역할에 쉽게 매핑할 수 있도록 해주는 `Roles` 컨트롤러
...
2.	[README.md](https://github.com/AzureADSamples/WebApp-GroupClaims-DotNet/blob/GroupClaims/README.md)의 지침에 따라 AAD 응용 프로그램 및 프로젝트를 설정합니다.

	> [AZURE.NOTE] AAD 응용 프로그램에 구성된 권한에는 **전역 관리자**가 아니라 <strong>사용자</strong> 역할만 필요합니다.
	
3.	응용 프로그램 구성을 마친 후에는 `F5` 키를 눌러 응용 프로그램을 실행합니다.
4.	응용 프로그램이 로드되면 **로그인**을 클릭합니다. 
5.	AAD 응용 프로그램을 구성하고 Web.config에서 해당 설정을 지정한 경우 로그인으로 리디렉션됩니다. 이 경우 AAA 응용 프로그램의 기본 소유자이므로 Azure 포털에서 AAD 응용 프로그램을 만드는 데 사용한 계정으로 로그인하기만 하면 됩니다. 
...
5.	**사용자/그룹 검색**에서 원하는 사용자 이름 또는 그룹 이름을 입력하기 시작하면 드롭다운 목록에 AAD 테넌트의 사용자 및/또는 그룹에 대한 필터링된 목록이 표시됩니다.

	![](./media/web-sites-dotnet-lob-application-azure-ad/select-user-group.png) 

	> [AZURE.NOTE] Views\Roles\Index.cshtml에서는 이 보기에서 <code>AadPicker</code>(Scripts\AadPickerLibrary.js에 정의됨)라는 JavaScript 개체를 사용하여 <code>Roles</code> 컨트롤러의 <code>Search</code> 작업에 액세스한다는 것을 확인할 수 있습니다.
    ...
    var picker = new <mark>AadPicker(searchUrl, maxResultsPerPage, input, token, tenant)</mark>;</pre>
		Controllers\RolesController.cs에서는 AAD Graph API로 실제 요청을 보내고 페이지로 응답을 다시 반환하는 <code>Search</code> 작업을 확인할 수 있습니다.
		나중에 동일한 메서드를 사용하여 응용 프로그램에서 간단한 기능을 만들 수 있습니다.

6.	드롭다운에서 사용자 또는 그룹을 선택하고 역할을 선택한 후 **역할 할당**을 클릭합니다.

<a name="bkmk_deploy"></a>
## Azure 웹 사이트에 예제 응용 프로그램 배포

여기에서는 응용 프로그램을 Azure 웹 사이트에 게시합니다. [README.md](https://github.com/AzureADSamples/WebApp-GroupClaims-DotNet/blob/GroupClaims/README.md)에 이미 Azure 웹 사이트 게시에 대한 지침이 있지만 이러한 단계는 로컬 디버그 환경에 대한 구성을 취소하기도 합니다. 디버그 구성을 유지하면서 배포하는 방법에 대해 알아보겠습니다.

1. 프로젝트를 마우스 오른쪽 단추로 클릭하고 **게시**를 선택합니다.

	![](./media/web-sites-dotnet-lob-application-azure-ad/publish-app.png)

2. **Microsoft Azure 웹 사이트**를 선택합니다.
3. Azure에 로그인하지 않은 경우 **로그인**을 클릭하고 Azure 구독에 대한 Microsoft 계정을 사용하여 로그인합니다.
4. 로그인되면 **새로 만들기**를 클릭하여 새 웹 사이트를 만듭니다.
...
6. **만들기**를 클릭합니다. 웹 사이트가 만들어지면 웹 게시 대화 상자가 열립니다.
...
2. [Azure 관리 포털](https://manage.windowsazure.com)에서 AAD 테넌트로 이동하여 **응용 프로그램** 탭을 클릭합니다.
2. 페이지 맨 아래에 있는 **추가**를 클릭합니다.
3. **내 조직에서 개발 중인 응용 프로그램 추가**를 선택합니다.
4. 응용 프로그램의 이름을 지정하고 **다음**을 클릭합니다.
5. 앱 속성에서 **로그온 URL**을 이전에 저장한 웹 사이트 URL(예: `https://<site-name>.azurewebsites.net`)로 설정하고 **앱 ID URI**를 `https://<aad-tenanet-name>/<app-name>`으로 설정합니다. 그런 후 **완료**를 클릭합니다.
...
8. **다른 응용 프로그램에 대한 권한** 아래의 **위임된 권한** 드롭다운에서 **Windows Azure Active Directory** 항목에 대해 **조직의 디렉터리에 액세스**를 선택합니다.
...
11. Visual Studio,에서 프로젝트의 **Web.Release.config**를 엽니다. 다음 XML을 `<configuration>` 태그에 삽입하고 각 키 값을 새 AAD 응용 프로그램에 대해 저장한 정보로 바꿉니다.  
...
2. **게시**를 클릭하여 Azure 웹 사이트에 게시합니다.

작업을 마치면 Azure 관리 포털에 AAD 응용 프로그램 두 개가 구성됩니다. 하나는 Visual Studio의 디버그 환경용 AAD 응용 프로그램이고, 나머지 하나는 Azure에 게시된 웹 사이트용 AAD 응용 프로그램입니다. 디버그하는 동안 Web.config의 응용 프로그램 설정을 통해 **디버그** 구성이 AAD와 작동하도록 설정되며, 게시된 경우(기본적으로 **릴리스** 구성이 게시됨) 변환된 Web.config가 업로드되어 응용 프로그램 설정 변경 사항이 Web.Release.config에 포함됩니다.

게시된 웹 사이트를 디버거에 연결하려는 경우(즉, 게시된 웹 사이트에서 코드의 디버그 기호를 업로드해야 함) Web.Release.config의 AAD 설정을 사용하는 고유한 사용자 지정 Web.config 변환(예: Web.AzureDebug.config)을 통해 Azure 디버깅을 위한 디버그 구성의 복제본을 만들 수 있습니다. 이렇게 하면 여러 환경에서 정적 구성을 유지 관리할 수 있습니다.

<a name="bkmk_crud"></a>
## 예제 응용 프로그램에 LOB 기능 추가

자습서의 이 부분에서는 예제 응용 프로그램을 기반으로 원하는 LOB 기능을 빌드하는 방법에 대해 알아봅니다. TaskTracker 컨트롤러와 유사하지만 표준 CRUD 스캐폴딩 및 디자인 패턴을 사용하는 간단한 CRUD 작업 항목 추적기를 만듭니다. 또한 포함된 Scripts\AadPickerLibrary.js를 사용하여 AAD Graph API의 데이터로 응용 프로그램을 보강합니다.  
...
8.	새 스캐폴드된 항목 `WorkItemsController`를 Controllers 폴더에 추가합니다. 이렇게 하려면 **컨트롤러**를 마우스 오른쪽 단추로 클릭하고 **추가**를 가리킨 후 **새 스캐폴드된 항목**을 선택합니다. 
...
11. 강조 표시된 [Authorize] 장식을 아래의 각 작업에 추가합니다.
	...
	> + AAD가 위조 방지 토큰에 기본적으로 필요한 http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/identityprovider를 보내지 않습니다.
	> + AAD가 AD FS와 디렉터리 동기화된 경우 http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/identityprovider 클레임을 보내도록 AD FS를 수동으로 구성할 수 있지만 AD FS 트러스트에서 기본적으로 이 클레임을 보내지 않습니다.
...
12.  App_Start\Startup.Auth.cs에서 `ConfigureAuth` 메서드에 다음 코드 줄을 추가합니다.
	...
	`ClaimTypes.NameIdentifies`는 AAD에서 제공하는 클레임 `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier`를 지정합니다. 이제 인증 부분을 해결했으므로(오래 걸리지는 않았음) 작업의 실제 기능에 집중할 수 있습니다. 
...
14.	Views\WorkItems\Create.cshtml(자동으로 스캐폴드된 항목)에서 `Html.BeginForm` 도우미 메서드를 찾아 다음과 같이 수정합니다.  
	...
	스크립트에서 AAD Picker 개체는 `~/Roles/Search` 작업에서 입력과 일치하는 AAD 사용자 및 그룹을 검색합니다. 그런 다음 제출 단추가 클릭되면 사용자 ID를 숨겨진 `AssignedToID` 필드에 저장합니다.  

15. 이제 응용 프로그램을 Visual Studio 디버거에서 실행하거나 Azure 웹 사이트에 게시합니다. 응용 프로그램 소유자로 로그인하여 `~/WorkItems/Create`로 이동합니다. 이 예제의 게시된 LOB 응용 프로그램의 경우 `https://mylobapp.azurewebsites.net/WorkItems/Create`로 이동합니다. 이제 동일한 AAD Picker 검색 필터를 사용하여 AAD 사용자를 선택할 수 있습니다.
...
이제 WorkItems 컨트롤러에서 여러 작업에 대한 권한 부여 및 LOB 기능을 구성했으므로 여러 응용 프로그램 역할의 사용자로 로그인할 수 있습니다.
...
- [Azure의 새로운 하이브리드 연결](http://azure.microsoft.com/documentation/videos/new-hybrid-connections-not-your-fathers-hybridcloud/)
- [Active Directory와 Azure AD 간의 유사점](http://technet.microsoft.com/ library/dn518177.aspx)
- [Single Sign-On으로 디렉터리 동기화 시나리오](http://technet.microsoft.com/ library/dn441213.aspx)
- [Azure AD 지원 토큰 및 클레임 유형](http://msdn.microsoft.com/library/azure/dn195587.aspx)
