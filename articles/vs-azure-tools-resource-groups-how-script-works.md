...
> [!NOTE]
> 이것은 Deploy-AzureResourceGroup.ps1 스크립트 1.0 버전에 대한 설명입니다.
> 
> 
...
5. JSON 매개 변수 컬렉션을 반복합니다. *\_artifactsLocation* 또는 *\_artifactsLocationSasToken*에 매개 변수 값이 할당된 경우에는 해당 값에 *$OptionalParameters* 매개 변수를 설정합니다. 이렇게 하면 사용자가 제공한 매개 변수 값을 스크립트가 의도치 않게 덮어쓰는 것을 방지합니다.
   ...
   ```
6. 배포용 아티팩트를 보관하는 데 사용되는 저장소 계정 리소스에 대한 컨텍스트와 저장소 계정 키를 가져옵니다.
   ...
   ```
7. PowerShell DSC를 사용하여 가상 컴퓨터를 구성하는 경우, 단일 zip 파일 내에 포함된 아티팩트가 DSC 확장에 필요합니다. 따라서 DSC 구성을 위해 .zip 보관 파일을 만듭니다. 이를 위해, $DSCSourceFolder가 존재하는지 확인합니다. DSC 구성이 있으면, 제거한 후 dsc.zip이라는 새 압축 파일을 만듭니다.
   ...
   ```
8. 매개 변수 파일에 Azure 아티팩트에 대한 경로가 제공되지 않으면, 아티팩트를 업로드할 때 사용할 PowerShell 스크립트 경로를 설정합니다. 이를 위해, 저장소 계정의 끝점 경로와 저장소 컨테이너 이름의 조합을 사용하여 경로를 만듭니다. 그 후 새 경로로 매개 변수 파일을 업데이트합니다.
...
9. **AzCopy** 유틸리티(Azure 리소스 그룹 배포 프로젝트의 **Tools** 폴더에 포함된)를 사용하여 로컬 저장소 계정의 모든 파일을 온라인 Azure 저장소 계정으로 복사합니다. 이 단계에 실패하면 필요한 아티팩트 없이 배포에 성공할 가능성이 낮으므로 스크립트를 종료합니다.
   ...
   ```
10. 매개 변수 파일에 아티팩트 위치에 대한 SAS 토큰이 제공되지 않으면, 해당 토큰을 만들어서 온라인 저장소 컨테이너에 임시 읽기 전용 액세스를 제공합니다. 그 후 SAS 토큰을 명령줄에 “optionalParameter”로 전달합니다. 명령줄로 전달되는 모든 매개 변수는 매개 변수 파일에 제공되는 값보다 우선적으로 적용됩니다.
    ...
    ```
11. 리소스 그룹이 없으면 만들고 템플릿과 매개 변수 파일에 배포를 성공하는 데 방해가 될만한 유효성 검사 오류가 있는지 확인합니다.
    ...
    ```
12. 마지막으로 템플릿을 배포합니다. 이 코드는 타임스탬프를 사용하여 배포용 고유 이름을 만듭니다.
...
3. 메시지가 표시되면 **리소스 그룹 만들기** 대화 상자에 리소스 그룹 이름 및 위치를 입력한 후 **만들기** 단추를 선택합니다.
    ...
    ![][2]
4. **매개 변수 편집** 단추를 선택하여 **매개 변수 편집** 대화 상자를 열고 누락된 매개 변수 값이 있으면 입력합니다.
   
    ![][3]
   
   > [!NOTE]
   > 필수 매개 변수에 값이 필요하면, 배포 시 이 대화 상자에 자동으로 표시됩니다.
   > 
   > 
   
    ![][4]
5. 매개 변수 값 입력을 완료하면 **저장** 단추를 선택한 후 **배포** 단추를 선택합니다.
