...
description: 비즈니스 시나리오에 대해 오류가 발생할 가능성이 있는 지점을 예측하는 Azure IoT 예측 유지 관리 솔루션 가속기의 개요입니다.
author: dominicbetts
manager: timlt
...
**작업: 원격 분석은** 다음 두 문을 사용하여 들어오는 장치 원격 분석 스트림에서 작동합니다.
...
Machine Learning 구성 요소는 실제 항공기 엔진에서 수집된 데이터에서 파생된 모델을 사용합니다. [azureiotsolutions.com][lnk-azureiotsolutions] 페이지의 솔루션 타일에서 기계 학습 작업 영역으로 이동할 수 있습니다. 솔루션이 **준비** 상태일 때 타일이 제공됩니다.
...
*\[1\] A. 색세나와 케이 괴벨 (2008). "터보 팬 엔진 저하 시뮬레이션 데이터 세트", NASA 에임스 예후 데이터https://c3.nasa.gov/dashlink/resources/139/)저장소 (, NASA 에임스 연구 센터, 모펫 필드, 캘리포니아*
