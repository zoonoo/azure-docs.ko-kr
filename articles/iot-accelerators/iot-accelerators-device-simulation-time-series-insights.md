---
title: Time Series Insights로 시뮬레이션 원격 분석 시각화 - Azure | Microsoft Docs
...
이 방법 가이드의 단계에서는 Azure 구독에 디바이스 시뮬레이션 솔루션 가속기를 배포했다고 가정합니다. 아직 디바이스 시뮬레이션을 배포하지 않은 경우 GitHub의 [디바이스 시뮬레이션 배포](https://github.com/Azure/azure-iot-pcs-device-simulation/blob/master/README.md)를 참조하세요.
...
1. 왼쪽의 용어 패널에서 **측정값** 으로 **온도** 및 **분할 기준** 값으로 **iothub-connection-device-id** 를 선택합니다.
    ...
    ![‘측정값’ 및 ‘분할 기준’ 값이 강조 표시된 Time Series Insights ‘용어’ 패널을 보여 주는 스크린샷.](./media/iot-accelerators-device-simulation-time-series-insights/time-series-insights-query1.png)
