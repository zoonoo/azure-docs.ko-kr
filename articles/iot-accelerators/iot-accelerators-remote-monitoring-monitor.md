---
title: 원격 모니터링 솔루션에서 디바이스 모니터링 - Azure | Microsoft Docs
...
ms.locfileid: "73890877"
---
# <a name="tutorial-monitor-your-iot-devices"></a>자습서: IoT 디바이스 모니터링

이 자습서에서는 원격 모니터링 솔루션 가속기를 사용하여 연결된 IoT 디바이스를 모니터링합니다. 솔루션 대시보드를 사용하여 원격 분석 데이터, 디바이스 정보, 경고 및 KPI를 볼 수 있습니다.

이 자습서에서는 위치, 속도 및 화물 온도 원격 분석을 전송하는 두 개의 시뮬레이션된 트럭 디바이스를 사용합니다. 트럭은 Contoso라는 조직에서 관리하고 원격 모니터링 솔루션 가속기에 연결됩니다. Contoso 운영자로서 현장에 있는 트럭 중 하나(truck-02)의 위치 및 동작을 모니터링해야 합니다.
...
필터를 만들고 편집하고 삭제하려면 **디바이스 그룹 관리**를 클릭합니다.

## <a name="view-real-time-telemetry"></a>실시간 원격 분석 보기

솔루션 가속기는 **대시보드** 페이지의 차트에 실시간 원격 분석 데이터를 표시합니다. 원격 분석 차트의 맨 위에는 truck-02를 포함하여 현재 필터로 선택된 디바이스에 사용할 수 있는 원격 분석 유형이 표시됩니다. 기본적으로 이 차트는 트럭의 위도를 표시하며 truck-02는 움직이지 않는 것으로 나타납니다.
...
맵은 현재 필터에 의해 선택된 시뮬레이트된 트럭에 대한 정보를 표시합니다. 맵을 확대/축소 및 이동하여 위치를 더 자세하게 혹은 덜 자세하게 표시할 수 있습니다. 맵의 디바이스 아이콘 색은 디바이스에 대해 활성 상태인 **경고**(진한 파랑) 또는 **알림**(빨간색)이 있는지 나타냅니다. **경고** 및 **알림** 수의 요약은 맵의 왼쪽에 표시됩니다.

truck-02의 세부 정보를 보려면 맵을 이동하고 확대/축소하여 truck-02를 찾은 다음, 맵에서 트럭을 선택합니다. 그런 다음, 디바이스 레이블을 클릭하여 **디바이스 세부 정보** 패널을 엽니다. 디바이스 세부 정보에는 다음이 포함됩니다.
...
대시보드에 현재 디바이스 및 시간 간격 필터로 선택된 경고에 대한 세 가지 KPI가 표시됩니다.
...
경고의 시간 범위를 설정하고 표시할 디바이스를 제어하는 필터와 동일한 필터가 KPI가 집계되는 방식을 결정합니다. 기본적으로 패널은 지난 1시간에 걸쳐 집계된 KPI를 표시합니다.
...
> [모니터링 솔루션에 연결된 디바이스를 사용하여 문제 감지](iot-accelerators-remote-monitoring-automate.md)