...
스트림이 연결되고 나면 개인 설정 대시보드를 만들 준비가 됩니다. 아래 대시보드는 시뮬레이션된 장치에서 내보낸 원격 분석 데이터를 가져와서 다음과 같이 다양한 피벗을 보여주는 예입니다. 

* 지도에 장치 위치 표시(오른쪽)
* 장치와 함께 상태 및 심각도 표시 (왼쪽 위)
* 규칙이 적용되는 장치 및 해당 장치에 대한 경보 발생 여부(왼쪽 맨 아래)
