...
> [AZURE.NOTE] 이 자습서는 Android 앱에서 모바일 서비스를 통해 Azure를 사용하여 데이터를 저장하고 검색할 수 있는 방법을 더욱 잘 이해할 수 있도록 돕기 위한 것입니다. 이 항목에서는 모바일 서비스 퀵 스타트에서 완료한 여러 단계를 순서대로 안내합니다. 모바일 서비스를 처음 사용하는 경우 먼저 [모바일 서비스 시작](/develop/mobile/tutorials/get-started-android-EC) 자습서를 완료하는 것이 좋습니다.
> 
> 완성된 앱의 소스 코드를 참조하려는 경우 [여기](https://github.com/RickSaling/mobile-services-samples/tree/futures/GettingStartedWithData/Android/GetStartedWithData)로 이동하세요.


이 자습서에서는 다음 기본 단계를 단계별로 안내합니다.

1. [Android 앱 프로젝트 다운로드] 
2. [모바일 서비스 만들기]
3. [저장소로 사용할 데이터 테이블 추가]
4. [모바일 서비스를 사용하도록 앱 업데이트]
5. [모바일 서비스에 대해 앱 테스트]

> [AZURE.IMPORTANT] 이 자습서를 완료하려면 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 평가판](http://www.windowsazure.com/pricing/free-trial/?WT.mc_id=AED8DE357"%20target="_blank)을 참조하세요. 

이 자습서에는 [모바일 서비스 Android SDK], Eclipse IDE(통합 개발 환경) 및 ADT(Android 개발자 도구) 플러그 인이 포함된 <a href="https://go.microsoft.com/fwLink/p/?LinkID=280125" target="_blank">Android SDK</a>, 그리고 Android 4.2 이상 버전이 필요합니다. 
...
> [AZURE.NOTE] 이 자습서에서는 최신 버전의 모바일 서비스 SDK를 사용합니다. 이전 버전과의 호환성을 위해 <a href="http://go.microsoft.com/fwlink/p/?LinkID=280126">여기</a>에서 이전 버전을 찾을 수 있지만 이러한 자습서에 포함된 코드는 해당 버전에서 작동하지 않습니다.
...
* [스크립트를 사용하여 데이터 유효성 검사 및 수정]
  <br/>모바일 서비스에서 서버 스크립트를 사용하여 앱에서 전송된 데이터를 변경하고 해당 유효성을 검사하는 방법을 자세히 알아봅니다.
