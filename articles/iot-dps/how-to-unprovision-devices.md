...
ms.locfileid: "39522675"
---
# <a name="how-to-deprovision-devices-that-were-previously-auto-provisioned"></a>이전에 자동으로 프로비전된 장치의 프로비전을 해제하는 방법 

이전에 Device Provisioning Service를 통해 프로비전된 장치를 프로비전 해제해야 하는 경우가 있을 수 있습니다. 예를 들어 디바이스가 다른 IoT 허브로 판매, 이동하거나 분실, 도난당하거나 손상될 수 있습니다. 
...
- 등록 그룹에서 단일 장치의 프로비전을 해제하려면
  1. 리프(장치) 인증서에 대해 사용하지 않도록 설정된 개별 등록을 만듭니다. 이렇게 하면 인증서 체인에 등록 그룹의 서명 인증서가 있는 다른 디바이스에 대한 액세스를 허용하면서도 해당 디바이스에 대한 프로비전 서비스에 대한 액세스가 취소됩니다. 디바이스에 대해 사용하지 않도록 설정된 개별 등록은 삭제하지 않습니다. 이렇게 하면 해당 디바이스가 등록 그룹을 통해 다시 등록될 수 있습니다. 
