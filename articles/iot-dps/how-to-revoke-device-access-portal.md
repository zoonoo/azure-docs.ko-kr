...
X.509 인증서는 일반적으로 신뢰할 수 있는 인증서 체인에 정렬됩니다. 체인의 어느 단계에서 인증서가 손상되면 트러스트가 끊어집니다. 이 인증서를 포함 하는 체인에서 장치 프로 비전 서비스의 다운스트림 장치 프로 비전을 방지 하기 위해 인증서를 허용 하지 않아야 합니다. X.509 인증서 및 프로비전 서비스를 통해 사용되는 방법에 대한 자세한 내용은 [X.509 인증서](./concepts-security.md#x509-certificates)를 참조하세요. 
