---
title: Azure IoT Hub Device Provisioning Service에서 디바이스 등록 해제
description: Azure IoT Hub DPS(Device Provisioning Service)를 통한 프로비저닝을 방지하기 위해 디바이스 등록을 해제하는 방법
author: wesmc7777
ms.author: wesmc
ms.date: 04/05/2018
ms.topic: conceptual
...
Device Provisioning Service에 등록하면 디바이스가 [프로비저닝](about-iot-dps.md#provisioning-process)될 수 있습니다. 프로비전된 디바이스는 IoT Hub에 등록된 디바이스로, 초기 [디바이스 쌍](~/articles/iot-hub/iot-hub-devguide-device-twins.md) 상태를 수신하고 원격 분석 데이터 보고를 시작할 수 있습니다. 이 문서에서는 프로비전 서비스 인스턴스에서 디바이스 등록을 해제하여 앞으로 다시 프로비전되지 못하게 하는 방법을 설명합니다.
...
## <a name="disallow-devices-by-using-an-individual-enrollment-entry"></a>개별 등록 항목을 사용하여 디바이스 허용 안 함

개별 등록은 단일 디바이스에 적용되며, X.509 인증서, TPM 인증 키(실제 또는 가상 TPM에서) 또는 SAS 토큰을 증명 메커니즘으로 사용할 수 있습니다. 개별 등록이 있는 디바이스를 허용하지 않으려면 해당 등록 항목을 사용하지 않도록 설정하거나 삭제할 수 있습니다. 

해당 등록 항목을 사용하지 않도록 설정하여 디바이스를 일시적으로 허용하지 않으려면 다음을 수행합니다. 

1. Azure Portal에 로그인하고 왼쪽 메뉴에서 **모든 리소스** 를 클릭합니다.
2. 리소스 목록에서 디바이스를 허용하지 않으려는 프로비저닝 서비스를 선택합니다.
3. 프로비전 서비스에서 **등록 관리** 를 클릭한 후 **개별 등록** 탭을 선택합니다.
4. 허용하지 않으려는 디바이스의 등록 항목을 선택합니다. 
...
해당 등록 항목을 삭제하여 디바이스를 영구적으로 허용하지 않으려면 다음을 수행합니다.

1. Azure Portal에 로그인하고 왼쪽 메뉴에서 **모든 리소스** 를 클릭합니다.
2. 리소스 목록에서 디바이스를 허용하지 않으려는 프로비저닝 서비스를 선택합니다.
3. 프로비전 서비스에서 **등록 관리** 를 클릭한 후 **개별 등록** 탭을 선택합니다.
4. 허용하지 않으려는 디바이스의 등록 항목 옆에 있는 확인란을 선택합니다. 
...
## <a name="disallow-an-x509-intermediate-or-root-ca-certificate-by-using-an-enrollment-group"></a>등록 그룹을 사용하여 X.509 중간 또는 루트 CA 인증서 허용 안 함

X.509 인증서는 일반적으로 신뢰할 수 있는 인증서 체인에 정렬됩니다. 체인의 어느 단계에서 인증서가 손상되면 트러스트가 끊어집니다. 디바이스 프로비저닝 서비스가 해당 인증서를 포함하는 모든 체인에서 디바이스 다운스트림을 프로비저닝하지 않도록 해당 인증서를 허용하지 않아야 합니다. X.509 인증서 및 프로비전 서비스를 통해 사용되는 방법에 대한 자세한 내용은 [X.509 인증서](./concepts-x509-attestation.md#x509-certificates)를 참조하세요. 
...
1. Azure Portal에 로그인하고 왼쪽 메뉴에서 **모든 리소스** 를 클릭합니다.
2. 리소스 목록에서 서명 인증서를 허용하지 않으려는 프로비저닝 서비스를 선택합니다.
3. 프로비전 서비스에서 **등록 관리** 를 선택한 후 **등록 그룹** 탭을 선택합니다.
4. 허용하지 않으려는 인증서를 사용하는 등록 그룹을 선택합니다.
...
2. 리소스 목록에서 디바이스를 허용하지 않으려는 프로비저닝 서비스를 선택합니다.
3. 프로비전 서비스에서 **등록 관리** 를 선택한 후 **등록 그룹** 탭을 선택합니다.
4. 허용하지 않으려는 인증서의 등록 그룹 옆에 있는 확인란을 선택합니다. 
...
## <a name="disallow-specific-devices-in-an-enrollment-group"></a>등록 그룹에서 특정 디바이스 허용 안 함
...
1. Azure Portal에 로그인하고 왼쪽 메뉴에서 **모든 리소스** 를 클릭합니다.
2. 리소스 목록에서 허용하지 않으려는 디바이스에 대한 등록 그룹이 포함된 프로비저닝 서비스를 선택합니다.
3. 프로비전 서비스에서 **등록 관리** 를 클릭한 후 **개별 등록** 탭을 선택합니다.
4. 위쪽에서 **개별 등록 추가** 단추를 선택합니다. 
5. **등록 추가** 페이지에서 해당 디바이스에 대한 증명 **메커니즘** 으로 **X.509** 를 선택합니다.

    디바이스 인증서를 업로드하고 허용하지 않으려는 디바이스의 디바이스 ID를 입력합니다. 인증서의 경우에는 디바이스에 설치된 서명된 최종 엔터티 인증서를 사용합니다. 디바이스는 서명된 최종 엔터티 인증서를 인증에 사용합니다.

    ![허용하지 않은 디바이스의 디바이스 속성 설정](./media/how-to-revoke-device-access-portal/disable-individual-enrollment-in-enrollment-group-1.png)
