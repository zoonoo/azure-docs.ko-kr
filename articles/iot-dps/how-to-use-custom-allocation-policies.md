...
이 섹션에서는 Azure Cloud Shell를 사용 하 여 **Contoso Toers** 와 **contoso 열 펌프 나누기**를 나타내는 두 개의 새 IoT hub를 만듭니다.
...
1. Azure Cloud Shell을 사용하여 [az group create](/cli/azure/group#az-group-create) 명령으로 리소스 그룹을 만듭니다. Azure 리소스 그룹은 Azure 리소스가 배포 및 관리되는 논리적 컨테이너입니다.
    ...
    다음 예제에서는 *eastus* 지역에 *contoso-us-resource-group*이라는 리소스 그룹을 만듭니다. 이 문서에 만든 모든 리소스에 이 그룹을 사용하는 것이 좋습니다. 이 접근 방식은 완료 된 후 더 쉽게 정리 됩니다.

    ```azurecli-interactive 
    az group create --name contoso-us-resource-group --location eastus
    ```

2. Azure Cloud Shell을 사용하여 **az iot hub create** 명령으로 [Contoso Toasters Division](/cli/azure/iot/hub#az-iot-hub-create) IoT Hub를 만듭니다. IoT Hub는 *contoso-us-resource-group*에 추가됩니다.

    다음 예제에서는 *eastus* 위치에 *contoso-toasters-hub-1098*이라는 IoT Hub를 만듭니다. 고유한 허브 이름을 사용 해야 합니다. 허브 이름에서 **1098** 대신 고유한 접미사를 구성합니다. 사용자 지정 할당 정책에 대한 예제 코드는 허브 이름에 `-toasters-`가 필요합니다.
...
3. Azure Cloud Shell을 사용하여 **az iot hub create** 명령으로 [Contoso Heat Pumps Division](/cli/azure/iot/hub#az-iot-hub-create) IoT Hub를 만듭니다. 이 IoT Hub도 *contoso-us-resource-group*에 추가됩니다.

    다음 예제에서는 *eastus* 위치에 *contoso-heatpumps-hub-1098*이라는 IoT Hub를 만듭니다. 고유한 허브 이름을 사용 해야 합니다. 허브 이름에서 **1098** 대신 고유한 접미사를 구성합니다. 사용자 지정 할당 정책에 대한 예제 코드는 허브 이름에 `-heatpumps-`가 필요합니다.
...
7. **함수 앱**에서 열리는 페이지를 만들고 새 함수에 대한 다음 설정을 입력한 후 **만들기**를 클릭합니다.
    ...
    **Application Insights**:이 연습에서는이 기능을 해제할 수 있습니다.

    ![함수 앱 만들기](./media/how-to-use-custom-allocation-policies/function-app-create.png)

8. **등록 그룹 추가** 페이지로 돌아가 새 함수 앱이 선택되었는지 확인합니다. 함수 앱 목록을 새로 고치려면 구독을 다시 선택 해야 할 수 있습니다.
...
11. **등록 그룹 추가** 페이지로 돌아가서 새 함수가 선택되었는지 확인합니다. 함수 목록을 새로 고치려면 함수 앱을 다시 선택 해야 할 수 있습니다.
...
1. 토스터 디바이스를 시뮬레이션하려면 `prov_dev_set_symmetric_key_info()`prov**dev\_client\_sample.c\_에서 주석으로 처리된**  호출을 찾습니다.
...
1. 열 펌프 디바이스를 시뮬레이션하려면 다시 `prov_dev_set_symmetric_key_info()`prov**dev\_client\_sample.c\_의**  호출을 이전에 생성한 열 펌프 등록 ID 및 파생된 디바이스 키로 업데이트합니다. 아래 표시된 키 값 **6uejA9PfkQgmYylj8Zerp3kcbeVrGZ172YLa7VSnJzg**도 예제로만 제공됩니다.
...
2. Visual Studio 메뉴에서 **디버그** > **디버깅하지 않고 시작**을 선택하여 솔루션을 실행합니다. 프로젝트를 다시 빌드하는 프롬프트에서 **예** 를 클릭 하 여 실행 하기 전에 프로젝트를 다시 빌드합니다.
...
4. 리소스 그룹의 삭제를 확인 하 라는 메시지가 표시 됩니다. 리소스 그룹의 이름을 다시 입력하여 확인한 다음, **삭제**를 클릭합니다. 잠시 후 리소스 그룹 및 해당 그룹에 포함된 모든 리소스가 삭제됩니다.
