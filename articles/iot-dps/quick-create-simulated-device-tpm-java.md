...
1. Azure Portal에 로그인하여 왼쪽 메뉴에서 **모든 리소스** 단추를 선택하고 Device Provisioning 서비스를 엽니다. _ID 범위_ 및 _Provisioning Service 글로벌 엔드포인트_ 를 확인합니다.

    ![Device Provisioning Service 정보](./media/java-quick-create-simulated-device/extract-dps-endpoints.png)

1. 앞에서 설명한 대로 _ID 범위_ 및 _Provisioning Service 글로벌 엔드포인트_ 를 포함하도록 `src/main/java/samples/com/microsoft/azure/sdk/iot/ProvisioningTpmSample.java`를 편집합니다.  
...
1. 프로그램이 실행되기 시작합니다. 다음 섹션에서 사용할 _인증 키_ 와 _등록 ID_ 를 적어 두고, 프로그램을 실행 상태로 둡니다.
