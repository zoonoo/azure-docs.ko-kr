...
| 역할 | 작업(Operation) | 설명 | 
|------| --------- | ------------| 
| 제조업체 | ID 및 등록 URL 인코딩 | 사용되는 증명 메커니즘에 따라, 제조업체는 디바이스 ID 정보 및 Device Provisioning Service 등록 URL을 인코딩해야 합니다.<br><br>**빠른 시작**: 디바이스가 시뮬레이트되므로 제조업체 역할은 없습니다. 샘플 등록 응용 프로그램을 코딩하는 데 사용되는 이러한 정보를 얻는 방법에 대한 자세한 내용은 개발자 역할을 참조하세요. |  
