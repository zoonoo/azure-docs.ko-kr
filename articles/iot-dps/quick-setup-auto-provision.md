...
2. **Device Provisioning Service**에 대한 *Marketplace를 검색*합니다. **IoT Device Provisioning Service(미리 보기)**를 선택하고 **만들기** 단추를 클릭합니다. 
...
3. **IoT Hub 또는 풀에 링크 추가** 포털 블레이드에서 현재 구독을 선택하거나 다른 구독에 대한 이름과 연결 문자열을 입력합니다. **리소스 종류**로 *IoT Hub*를 선택하고 드롭다운 목록에서 허브 이름을 선택합니다. 완료되면 **저장**을 클릭합니다. 
