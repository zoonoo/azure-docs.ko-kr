...
1. 명령 프롬프트를 엽니다. 장치 시뮬레이션 샘플 코드에 대한 GitHub 리포지토리를 복제합니다.
    
    ```cmd/sh
    git clone https://github.com/Azure/azure-iot-sdk-java.git --recursive
    ```
1. 필요한 모든 패키지를 다운로드하려면 azure-iot-sdk-java 루트 디렉터리로 이동하고 프로젝트를 빌드합니다.
   
   ```cmd/sh
   cd azure-iot-sdk-java
   mvn install -DskipTests=true
   ```
1. 인증서 생성기 프로젝트로 이동하여 프로젝트를 빌드합니다. 
...
1. 대상 폴더로 이동하고 만든 jar 파일을 실행합니다.
...
1. 설정에 따라 다음 방법 중 하나를 사용하여 등록 정보를 입력합니다.
    ...
    - **개별 등록**: 
        ...
        1. **등록 목록 항목 추가** 아래에 다음 정보를 입력합니다.
            ...
            - *.pem 또는 .cer 파일 인증* 아래에서 *파일 탐색기* 위젯을 사용하여 이전 단계에서 만든 **_X509individual.pem_** 인증서 파일을 선택합니다.
            - 필요에 따라 다음 정보를 입력합니다.
        ...
        ![포털 블레이드에 X.509 장치 등록 정보 입력](./media/java-quick-create-simulated-device-x509/enter-device-enrollment.png)  
    ...
    - **등록 그룹**: 
        ...
        1. **인증서 추가** 아래에서 다음 정보를 입력합니다.
            - 고유한 인증서 이름을 입력합니다.
            - 이전에 만든 **_X509group.pem_** 파일을 선택합니다.
            - 완료되면 **저장** 단추를 클릭합니다.

        ![인증서 추가](./media/java-quick-create-simulated-device-x509/add-certificate.png)

        1. 새로 만든 인증서를 선택합니다.
            - **확인 코드 생성**을 클릭합니다. 생성된 코드를 복사합니다.
            - _확인 코드_를 입력하거나 마우스 오른쪽 단추로 클릭하여 실행 중인 _provisioning-x509-cert-generator_ 창에 붙여넣습니다.  **Enter** 키를 누릅니다.
            - *-----BEGIN CERTIFICATE-----*에서 시작하여 *-----END CERTIFICATE-----*에서 끝나는 `Verification Cert`의 출력을 클립보드에 복사합니다.
            
                ![유효성 검사 생성기](./media/java-quick-create-simulated-device-x509/validation-generator.png)

            - Windows 컴퓨터에 **_X509validation.pem_**이라는 파일을 만들어 원하는 편집기에서 연 다음, 클립보드의 내용을 이 파일에 복사합니다. 파일을 저장합니다.
            - Azure Portal에서 **_X509validation.pem_** 파일을 선택합니다. **확인**을 클릭합니다.

            ![인증서 유효성 검사](./media/java-quick-create-simulated-device-x509/validate-certificate.png)
        ...
        ![포털 블레이드에서 X.509 그룹 등록 정보 입력](./media/java-quick-create-simulated-device-x509/enter-group-enrollment.png)

        성공적으로 등록되면 *등록 그룹* 탭의 *그룹 이름* 열 아래에 X.509 장치 그룹이 표시됩니다.
...
1. 설정에 따라 다음 방법 중 하나를 사용하여 등록 정보를 입력합니다.
    ...
    - **등록 그룹**: 
        ...
        1. 위의 **개별 등록**에 대한 지침을 따릅니다.

        1. `main` 함수의 시작 부분에 다음 코드 줄을 추가합니다.
