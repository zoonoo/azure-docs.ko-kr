...
description: 이 자습서에서는 IoT Hub DPS(Device Provisioning Service)를 사용하여 [Azure Portal](https://portal.azure.com)에서 디바이스 프로비저닝을 위한 클라우드 리소스를 설정하는 방법을 보여줍니다.
author: wesmc7777
ms.author: wesmc
ms.date: 11/12/2019
ms.topic: tutorial
...
이 자습서에서는 IoT Hub Device Provisioning Service를 사용하여 자동 디바이스 프로비전을 위해 클라우드를 설정하는 방법을 보여줍니다. 이 자습서에서는 다음과 같은 작업을 수행하는 방법을 살펴봅니다.
...
2. **균등 가중치 배포**(기본값): 연결된 IoT Hub들은 동일하게 해당 허브에 프로비전된 디바이스를 갖게 됩니다. 이 설정은 기본값입니다. 디바이스를 단 하나의 IoT Hub에 프로비전하려는 경우 이 설정을 유지할 수 있습니다. 
