...
description: Azure Portal에서 부하가 분산된 IoT Hub에 Device Provisioning Service 자동 장치 프로비저닝
author: sethmanheim
ms.author: sethm
ms.date: 09/05/2017
ms.topic: tutorial
...
이 자습서에서는 Device Provisioning Service를 사용하여 부하가 분산된 여러 IoT Hub에 대해 장치를 프로비전하는 방법을 보여줍니다. 이 자습서에서는 다음 방법에 대해 알아봅니다.
...
등록 목록은 장치와 함께 사용 중인 증명 메서드(장치 ID 확인을 위한 메서드)가 무엇인지를 Device Provisioning Service에 알려줍니다. 다음 단계는 두 번째 장치에 대한 등록 목록 항목을 추가하는 것입니다. 
...
할당 정책은 IoT Hub에 장치를 할당하는 방법을 결정하는 Device Provisioning Service 설정입니다. 세 가지의 지원되는 할당 정책이 있습니다. 
...
Device Provisioning Service가 해당 허브에 장치를 등록할 수 있도록 Device Provisioning Service 및 IoT Hub를 연결합니다.
