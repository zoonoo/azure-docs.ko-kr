...
* 하이브리드 연결 관리자를 설치한 후에 다음이 진행됩니다. 
  ...
  * Azure에 호스트된 하이브리드 연결은 기본 애플리케이션 연결 문자열을 사용하도록 자동으로 구성됩니다. 
  * 온-프레미스 리소스는 기본 온-프레미스 연결 문자열을 사용하도록 자동으로 구성됩니다.
* 하이브리드 연결 관리자는 권한 부여를 위해 유효한 온-프레미스 연결 문자열을 사용해야 합니다. Azure Web Apps 또는 Mobile Apps는 권한 부여를 위해 유효한 애플리케이션 연결 문자열을 사용해야 합니다.
