...
> [!TIP]
> Azure Portal에 로그인하려면 Azure 계정과 Azure 구독이 있어야 합니다. 계정이 없는 경우 몇 분 만에 평가판 계정을 만들 수 있습니다. [Azure 평가판](http://go.microsoft.com/fwlink/p/?LinkID=239738)을 참조하세요.


## <a name="CreateService"></a>BizTalk 서비스 만들기
선택한 버전에 따라 일부 BizTalk 서비스 설정을 사용하지 못할 수도 있습니다.

1. [Azure Portal](http://go.microsoft.com/fwlink/p/?LinkID=213885)에 로그인합니다.
...
5. 저장소 및 데이터베이스 설정을 입력합니다.  <table border="1">
    ...
    <td>기존 저장소 계정을 선택하거나 새 저장소 계정을 만듭니다. <br/><br/>새 Storage 계정을 만들 경우 <strong>Storage 계정 이름</strong>을 입력합니다.</td>
    </tr>
    <tr>
    <td><strong>데이터베이스 추적</strong></td>
    <td>기존 Azure SQL Database를 사용하는 경우 다른 BizTalk 서비스에서는 이 데이터베이스를 사용할 수 없습니다. Azure SQL Database 서버를 만들 때 입력한 로그인 이름과 암호가 있어야 합니다.<br/><br/><strong>팁</strong> BizTalk 서비스와 동일한 지역에 추적 데이터베이스 및 모니터링/보관 저장소 계정을 만듭니다.</td>
    </tr>
    </table>
다음 화살표를 선택합니다.
...
기존 SQL Database 서버를 선택하거나 새 SQL Database 서버를 만듭니다.</td>
    ...
    <tr>
    <td><strong>지역</strong></td>
    <td><strong>새 SQL Database 인스턴스 만들기</strong>를 선택한 경우에 사용할 수 있습니다. SQL Database를 호스트할 지리적 지역을 선택합니다.</td>
    </tr>
    </table>
...
완료되면 Azure BizTalk 서비스가 만들어져 응용 프로그램에서 사용할 준비가 됩니다. 기본 설정이 충분합니다. 기본 설정을 변경하려면 왼쪽 탐색 창에서 **BizTalk Services**를 선택하고 해당 BizTalk 서비스를 선택합니다. 위쪽의 [대시보드, 모니터 및 크기 조정 탭](biztalk-dashboard-monitor-scale-tabs.md)에 추가 설정이 표시됩니다.
...
1. [Azure Portal](http://go.microsoft.com/fwlink/p/?LinkID=213885)에 로그인합니다.
2. 왼쪽 탐색 창에서 **BizTalk Services**를 선택한 다음 해당 BizTalk 서비스 구독을 선택합니다.
3. **대시보드** 탭을 선택합니다.
...
BizTalk Services를 만들 때 자동으로 만들어진 자체 서명된 인증서는 개발 환경에서만 사용하기 위한 용도로 제공됩니다. 프로덕션 시나리오의 경우 이 인증서를 프로덕션이 준비된 인증서로 대체합니다.
...
1. [Azure Portal](http://go.microsoft.com/fwlink/p/?LinkID=213885)에 로그인합니다.
2. 왼쪽 탐색 창에서 **BizTalk Services**를 선택한 다음 해당 BizTalk 서비스를 선택합니다.
3. 작업 표시줄에서 **연결 정보**를 선택합니다.  
   ![연결 정보 선택][ACSConnectInfo]
4. Access Control 값을 복사합니다.
...
<tr>
<td>Azure 구독</td>
<td>구독은 Azure Portal에 로그인할 수 있는 사용자를 결정합니다. 계정 소유자는 <a HREF="https://account.windowsazure.com/Subscriptions">Azure 구독</a>에서 구독을 만듭니다.
<br/><br/>
Azure 계정에는 여러 구독이 있을 수 있으며, 허용된 모든 사용자가 관리할 수 있습니다. 예를 들어 Azure 계정 소유자는 <em>BizTalkServiceSubscription</em>이라는 구독을 만들고, 회사 내의 BizTalk 관리자(예: ContosoBTSAdmins@live.com)에게 이 구독에 대한 액세스를 제공합니다. 이 시나리오에서 BizTalk 관리자는 Azure Portal에 로그인하고 구독의 모든 호스팅 서비스(Azure BizTalk Services 포함)에 대해 전체 관리자 권한을 가집니다. BizTalk 관리자는 Azure 계정 소유자가 아니므로 대금 청구 정보에 대한 액세스 권한이 없습니다.
<br/><br/>
<a HREF="http://go.microsoft.com/fwlink/p/?LinkID=267577">Azure Portal에서 구독 및 Storage 계정 관리</a>에 자세한 내용이 나와 있습니다.
</td>
</tr>
<tr>
...
하이브리드 연결은 Azure 웹 사이트 또는 Azure 모바일 서비스를 정적 TCP 포트를 사용하는 온-프레미스 리소스(예: SQL Server, MySQL, HTTP 웹 API, Mobile Services 및 대부분의 사용자 지정 웹 서비스)에 연결하는 데 사용됩니다.  하이브리드 연결은 BizTalk 어댑터 서비스와 다릅니다. BizTalk 어댑터 서비스는 Azure BizTalk Services를 온-프레미스 LOB(기간 업무) 시스템에 연결하는 데 사용됩니다.
