...
예를 들어, 빌드 시스템에서 및 `pull` `pull` 액세스를 모두 `push` 제공 하는 서비스 주체를 사용 하는 동안에는 이미지 액세스만 제공 하는 서비스 주체를 사용 하도록 웹 응용 프로그램을 구성 합니다. 애플리케이션 개발 팀 인력에 변화가 있으면, 빌드 시스템에 영향을 주지 않으면서 서비스 주체 자격 증명을 순환할 수 있습니다.
...
Azure 컨테이너 레지스트리로 인증할 수 있는 모든 Azure 서비스의 서비스 주체 자격 증명을 사용할 수 있습니다. 예를 들어 다음과 같습니다.
...
서비스 주체를 사용 `docker login` 하 여를 실행할 수도 있습니다. 다음 예제에서는 서비스 주체 응용 프로그램 ID가 환경 변수 `$SP_APP_ID`및 변수에 `$SP_PASSWD`암호를 전달 합니다. Docker 자격 증명을 관리 하는 모범 사례는 [docker login](https://docs.docker.com/engine/reference/commandline/login/) 명령 참조를 참조 하세요.
