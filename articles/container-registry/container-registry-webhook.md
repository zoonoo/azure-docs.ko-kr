...
* Docker CLI-로컬 컴퓨터를 Docker 호스트로 설정 하 고 Docker CLI 명령에 액세스 하려면 [Docker 엔진](https://docs.docker.com/engine/installation/)을 설치 합니다.
...
1. 다음 정보를 사용 하 여 *Webhook 만들기* 양식을 완료 합니다.

| 값 | 설명 |
|---|---|
| Webhook 이름 | 웹후크에 지정하려는 이름입니다. 문자 및 숫자만 포함할 수 있으며 길이는 5-50 자 여야 합니다. |
| 위치 | 지리적으로 [복제](container-registry-geo-replication.md) 된 레지스트리에 대해 레지스트리 복제본의 Azure 지역을 지정 합니다. 
