...
Docker 허브와 같은 공용 레지스트리에서 유지 관리 되는 기본 콘텐츠에 따라 달라 지는 레지스트리 콘텐츠의 경우 Azure container registry 또는 다른 개인 레지스트리에 콘텐츠를 복사 하는 것이 좋습니다. 그런 다음 개인 기본 이미지를 참조 하 여 응용 프로그램 이미지를 빌드해야 합니다. Azure Container Registry는 공개 레지스트리 또는 기타 Azure Container registry에서 콘텐츠를 쉽게 복사할 수 있는 [이미지 가져오기](container-registry-import-images.md) 기능을 제공 합니다. 다음 섹션에서는 응용 프로그램 업데이트를 빌드할 때 ACR 작업을 사용 하 여 기본 이미지 업데이트를 추적 하는 방법을 설명 합니다. 사용자 고유의 Azure container registry에서 기본 이미지 업데이트를 추적 하 고 선택적으로 업스트림 공용 레지스트리를 추적할 수 있습니다.

## <a name="track-base-image-updates"></a>기본 이미지 업데이트 추적

ACR 작업에는 컨테이너의 기본 이미지가 업데이트될 때 이미지를 자동으로 빌드하는 기능이 포함되어 있습니다. 이 기능을 사용 하 여 Azure container registry에서 공용 기본 이미지의 복사본을 유지 관리 하 고 업데이트 한 다음 기본 이미지에 종속 된 응용 프로그램 이미지를 다시 빌드할 수 있습니다.
