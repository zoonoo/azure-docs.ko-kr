...
예. Here is [a template](https://github.com/Azure/azure-quickstart-templates/tree/master/101-container-registry) that you can use to create a registry.
...
예. See the documentation from [Twistlock](https://www.twistlock.com/2016/11/07/twistlock-supports-azure-container-registry/) and [Aqua](https://blog.aquasec.com/image-vulnerability-scanning-in-azure-container-registry).
...
> [!IMPORTANT]
> The admin user account is designed for a single user to access the registry, mainly for testing purposes. 관리자 계정 자격 증명을 여러 사용자와 공유하지 않는 것이 좋습니다. 헤드리스 시나리오의 경우 사용자 및 서비스 주체는 개별 ID를 사용하는 것이 좋습니다. See [Authentication overview](container-registry-authentication.md).
...
ACR supports Docker Registry HTTP API V2. The APIs can be accessed at `https://<your registry login server>/v2/`. 예: `https://mycontainerregistry.azurecr.io/v2/`
...
예. Enable TLS by using any recent docker client (version 18.03.0 and above). 
...
오류를 해결하려면:
1. Add the option `--signature-verification=false` to the Docker daemon configuration file `/etc/sysconfig/docker`. 다음은 그 예입니다.
...
| GitLab | https://gitlab.com/user/myapp-repo.git#mybranch:myfolder | yes | 아닙니다. |
| BitBucket | https://user@bitbucket.org/user/mayapp-repo.git#mybranch:myfolder | yes | 아닙니다. |
