...
- [익명 끌어오기 액세스를 사용 하도록 설정 어떻게 할까요??](#how-do-i-enable-anonymous-pull-access)
...
* 모든 역할 (위임 역할 제외)의 공개 키 및 인증서는에 저장 됩니다 `root.json`.
...
* Azure Portal: 레지스트리-> Access Control (IAM)-> 추가 (역할에 `AcrPull` 대해 `AcrPush` 또는 선택).
...
### <a name="how-do-i-enable-anonymous-pull-access"></a>익명 끌어오기 액세스를 사용 하도록 설정 어떻게 할까요??

익명 (공용) 끌어오기 액세스에 대해 Azure container registry를 설정 하는 것은 현재 미리 보기 기능입니다. 공용 액세스를 사용 하도록 설정 하려면에서 https://aka.ms/acr/support/create-ticket지원 티켓을 여세요. 자세한 내용은 [Azure 피드백 포럼](https://feedback.azure.com/forums/903958-azure-container-registry/suggestions/32517127-enable-anonymous-access-to-registries)을 참조 하세요.
...
와 같이 `myRegistry`레지스트리 리소스 이름이 대문자 또는 혼합 된 경우에도 모두 `docker push myregistry.azurecr.io/myimage:latest`소문자 서버 URL을 사용 해야 합니다.

### <a name="enable-and-get-the-debug-logs-of-the-docker-daemon"></a>Docker 디먼의 디버그 로그를 사용 하도록 설정 하 고 가져옵니다.    

`debug` 옵션으로 시작 `dockerd` 합니다. 먼저 Docker 디먼 구성 파일 (`/etc/docker/daemon.json`)이 없는 경우 만들고 `debug` 옵션을 추가 합니다.
...
특히 `--location` (리디렉션을 수행 `InvalidAuthenticationInfo` 하려면) 옵션과 `curl` `-L`함께 도구를 사용 하 여 오류가 발생할 수 있습니다.
...
이 문제를 해결 하려면 헤더 없이 리디렉션을 수동으로 수행 해야 합니다. 다음 `-D -` 옵션 `curl` 을 사용 하 여 응답 헤더를 인쇄 한 다음 헤더 `Location` 를 추출 합니다.
...
네트워크 관리자에 게 문의 하거나 네트워크 구성 및 연결을 확인 하십시오. 사용자 환경 `az acr check-health -n yourRegistry` 에서 Container Registry에 연결할 수 있는지 확인 하려면 Azure CLI을 사용 하 여 실행 해 보세요. 또한 브라우저에서 incognito 또는 private 세션을 시도 하 여 오래 된 브라우저 캐시 또는 쿠키를 방지할 수도 있습니다.
...
`az acr build` 명령에 로컬 원본 폴더를 전달 하면 기본적으로 업로드 된 `.git` 패키지에서 폴더가 제외 됩니다. 다음 설정을 사용 하 `.dockerignore` 여 파일을 만들 수 있습니다. 업로드 된 패키지의 아래 `.git` 에 있는 모든 파일을 복원 하는 명령을 지시 합니다. 
...
| 오류 메시지 | 문제 해결 가이드 |
|---|---|
|VM에 대 한 액세스가 구성 되지 않았으므로 구독을 찾을 수 없습니다.|ACR 작업에서를 사용 하 `az login --identity` 는 경우이 문제가 발생할 수 있습니다. 이 오류는 일시적인 오류 이며 관리 되는 Id의 역할 할당이 전파 되지 않은 경우에 발생 합니다. 몇 초 후에 작업을 다시 시도 합니다.|
