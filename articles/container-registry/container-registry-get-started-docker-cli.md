...
프라이빗 컨테이너 레지스트리에 대해 [몇 가지 방법으로 인증](container-registry-authentication.md)할 수 있습니다. 명령줄에서 작업할 때 권장되는 방법은 Azure CLI 명령 [az acr login](/cli/azure/acr#az-acr-login)입니다. 예를 들어 *myregistry* 라는 레지스트리에 로그인하려면 Azure CLI에 로그인한 다음 레지스트리에 인증합니다.
...
> [!NOTE]
>* 더 빠르고 편리한 로그인을 위해 Docker 확장이 포함된 Visual Studio Code를 사용하는 것이 좋습니다.
