...
* [익명 끌어오기 액세스](container-registry-faq.md#how-do-i-enable-anonymous-pull-access)를 사용 하도록 설정 된 레지스트리에서 범위 맵을 만들 수 없습니다.
...
* 생성 된 암호와 함께 **토큰** 을 사용 하면 사용자가 레지스트리를 사용 하 여 인증할 수 있습니다. 토큰 암호에 대 한 만료 날짜를 설정 하거나 언제 든 지 토큰을 사용 하지 않도록 설정할 수 있습니다.  
  ...
  |`content/write`     |  리포지토리에 데이터 쓰기     | 와 함께 `content/read` 사용 하 여 아티팩트 푸시 |
...
다음 예제는 리포지토리에 대 `samples/hello-world` 한 다음 사용 권한을 사용 하 여 레지스트리 *myregistry* 에서 토큰을 만듭니다 `content/write` . `content/read`및. 기본적으로이 명령은 기본 토큰 상태를로 `enabled`설정 하지만 언제 든 지 상태를로 `disabled` 업데이트할 수 있습니다.
...
출력에는 생성 된 명령의 범위 맵에 대 한 세부 정보가 포함 됩니다. 이라는 `MyToken-scope-map`범위 맵을 사용 하 여 다른 토큰에 동일한 리포지토리 작업을 적용할 수 있습니다. 또는 나중에 범위 맵을 업데이트 하 여 연결 된 토큰의 사용 권한을 변경 합니다.
...
범위 맵을 만들려면 [az acr scope-map create][az-acr-scope-map-create] 명령을 사용 합니다. 다음 명령은 이전에 사용 된 `samples/hello-world` 리포지토리에 대해 동일한 권한으로 범위 맵을 만듭니다. 
...
[Az acr token create][az-acr-token-create] 를 실행 하 여 토큰을 만들고 *MyScopeMap* 범위 맵을 지정 합니다. 이전 예제와 같이 명령은 기본 토큰 상태를로 `enabled`설정 합니다.
...
Azure Portal를 사용 하 여 토큰 및 범위 맵을 만들 수 있습니다. `az acr token create` CLI 명령과 마찬가지로 기존 범위 맵을 적용 하거나 하나 이상의 리포지토리 및 관련 작업을 지정 하 여 토큰을 만들 때 범위 맵을 만들 수 있습니다. 리포지토리가 아직 레지스트리에 있을 필요는 없습니다. 

다음 예제에서는 토큰을 만들고 `samples/hello-world` 리포지토리에 대 한 다음 권한으로 범위 맵을 만듭니다. `content/write` 및. `content/read`
...
1. 범위 맵을 구성 합니다.
    ...
    1. **리포지토리**에서을 입력 `samples/hello-world`하 고 **사용 권한**아래에서 `content/read` 및 `content/write`를 선택 합니다. 그런 다음 **+ 추가**를 선택 합니다.  
...
다음 예제에서는이 문서의 앞부분에서 만든 토큰을 사용 하 여 리포지토리에 대 한 일반적인 작업을 수행 합니다. 이미지 밀어넣기 및 끌어오기, 이미지 삭제 및 리포지토리 태그 나열. 토큰은 처음에`content/write` `content/read` `samples/hello-world` 저장소의 푸시 권한 (및 작업)으로 설정 되었습니다.

### <a name="pull-and-tag-test-images"></a>테스트 이미지 끌어오기 및 태그

다음 예제에서는 Docker 허브에서 `hello-world` 및 `alpine` 이미지를 가져와서 레지스트리 및 리포지토리에 대 한 태그를 합니다.
...
을 `docker login` 실행 하 여 레지스트리를 인증 하 고, 토큰 이름을 사용자 이름으로 제공 하 고, 암호 중 하나를 제공 합니다. 토큰은 상태를 `Enabled` 포함 해야 합니다.
...
성공적으로 로그인 한 후 태그가 지정 된 이미지를 레지스트리에 푸시합니다. 토큰에는 `samples/hello-world` 리포지토리에 이미지를 푸시할 수 있는 권한이 있기 때문에 다음 푸시가 성공 합니다.

```bash
docker push myregistry.azurecr.io/samples/hello-world:v1
```

토큰에는 리포지토리에 대 `samples/alpine` 한 권한이 없으므로 다음과 같은 오류가 발생 하 여 다음과 같은 푸시 시도가 실패 합니다 `requested access to the resource is denied`.
...
예 `MyToken-scope-map` 를 들어 `content/write` `content/read` `samples/alpine` 리포지토리에서 및 작업을 업데이트 하 고 `content/write` `samples/hello-world` 리포지토리에서 작업을 제거 합니다.  
...
1. **리포지토리**에서을 입력 `samples/alpine`하 고 **사용 권한**아래에서 `content/read` 및 `content/write`를 선택 합니다. 그런 다음 **+ 추가**를 선택 합니다.
1. **리포지토리**에서 **사용 권한**아래에서을 선택 `content/write` `samples/hello-world` 하 고 선택을 취소 합니다. 그런 다음 **저장**을 선택합니다.
...
범위 맵에는 `content/read` `samples/hello-world` 리포지토리에 대 한 권한만 있으므로 이제 리포지토리에 대 한 푸시 시도가 `samples/hello-world` 실패 합니다.
 
```bash
docker push myregistry.azurecr.io/samples/hello-world:v1
```

범위 맵은 두 리포지토리에 대 한 권한을 제공 `content/read` 하므로 두 리포지토리에서 이미지를 모두 성공적으로 끌어옵니다.
...
다음 [az acr repository delete][az-acr-repository-delete] 명령을 사용 하 여 `samples/alpine` 리포지토리를 삭제 합니다. 이미지 또는 리포지토리를 삭제 하기 위해 토큰은를 통해 `docker login`인증 하지 않습니다. 대신 토큰의 이름과 암호를 명령에 전달 합니다. 다음 예제에서는이 문서의 앞부분에서 만든 환경 변수를 사용 합니다.
...
`samples/hello-world` 리포지토리의 메타 데이터를 읽으려면 [az acr repository show-manifest][az-acr-repository-show-manifests] 또는 [az acr repository show-tags][az-acr-repository-show-tags] 명령을 실행 합니다. 

메타 데이터를 읽으려면 토큰은를 통해 `docker login`인증 하지 않습니다. 대신 토큰의 이름과 암호를 두 명령에 전달 합니다. 다음 예제에서는이 문서의 앞부분에서 만든 환경 변수를 사용 합니다.
...
다음 예에서는 만료 기간이 30 일인 *Mytoken* 토큰에 대해 password1의 새 값을 생성 합니다. 환경 변수에 `TOKEN_PWD`암호를 저장 합니다. 이 예제는 bash 셸에 대해 형식이 지정 됩니다.
...
Azure CLI를 사용 하 여 [az acr token update][az-acr-token-update] 명령을 실행 하 여를 `status` 로 `disabled`설정 합니다.
