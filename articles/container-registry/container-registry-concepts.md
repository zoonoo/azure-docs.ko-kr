...
컨테이너 이미지 구성 된 하나 이상의 *레이어*, 각 해당 이미지를 정의 하는 Dockerfile의 줄으로 합니다. 레지스트리의 이미지는 공용 계층을 공유하므로 스토리지 효율성이 높아집니다. 예를 들어, 다른 리포지토리에 있는 여러 이미지가 동일한 Alpine Linux 기본 계층을 공유할 수 있지만 해당 계층의 복사본 하나만 레지스트리에 저장됩니다.
