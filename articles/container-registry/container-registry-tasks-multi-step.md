...
* [`build`](container-registry-tasks-reference-yaml.md#build): 익숙한 `docker build` 구문이 나 병렬를 사용 하 여 하나 이상의 컨테이너 이미지를 빌드합니다.
...
* [작업](container-registry-tasks-samples.md) 예-간단 `task.yaml` 하 고 복잡 한 여러 시나리오에 대 한 예제 및 Docker 파일.
