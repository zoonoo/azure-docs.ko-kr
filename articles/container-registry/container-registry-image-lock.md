...
매니페스트 다이제스트 (`sha256:...`로 표시 되는 SHA-256 hash)로 식별 되는 *myrepo/myrepo* 이미지를 잠그려면 다음 명령을 실행 합니다. 하나 이상의 이미지 태그와 연결 된 매니페스트 다이제스트를 찾으려면 [az acr repository show-manifest][az-acr-repository-show-manifests] 명령을 실행 합니다.
