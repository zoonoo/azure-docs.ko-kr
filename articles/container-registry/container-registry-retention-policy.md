...
1. @No__t-0 이미지를 태그 지정 해제. 예를 들어 [az acr repository 태그 지정 해제][az-acr-repository-untag] 명령을 사용 합니다. 태그가 없는 매니페스트는 레지스트리에 남아 있습니다.
    ```azurecli
...
레지스트리에서 보존 정책을 사용 하지 않도록 설정 하려면 [az acr config 보존 업데이트][az-acr-config-retention-update] 명령을 실행 하 고 `--status disabled`을 설정 합니다.
