...
[ACR 작업](container-registry-tasks-overview.md)은 기본 이미지 중 하나에서 OS 또는 애플리케이션 프레임워크를 패치할 때와 같이 컨테이너의 [기본 이미지가 업데이트](container-registry-tasks-base-images.md)될 때 자동화되는 이미지 빌드를 지원합니다. 
...
이 작업은 [이전 자습서](container-registry-tutorial-build-task.md)에서 만든 작업과 비슷합니다. 커밋이 `--context`에 지정된 리포지토리에 푸시될 때 ACR 작업에서 이미지 빌드를 트리거하도록 지시합니다. 이전 자습서에서 이미지를 빌드하는 데 사용한 Dockerfile에서 퍼블릭 기본 이미지(`FROM node:15-alpine`)를 지정하지만, 이 작업의 Dockerfile([dockerfile-app][dockerfile-app])에서는 기본 이미지 레지스트리에서 기본 이미지를 지정합니다.
...
새로 빌드한 컨테이너를 실행하여 업데이트된 버전 번호를 확인하는 다음의 선택적 단계를 수행하려면 Image Update가 트리거된 빌드에 대한 **RUN ID** 값을 적어 둡니다(앞의 출력에서 "ca13"임).
