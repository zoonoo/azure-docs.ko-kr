...
이미지 및 기타 아티팩트를 Azure 컨테이너 레지스트리에 저장하면 Azure에서 [서비스 관리형 키](../security/fundamentals/encryption-models.md)를 사용하여 저장된 레지스트리 콘텐츠를 자동으로 암호화합니다. Azure Key Vault (고객이 관리 하는 키)에서 만들고 관리 하는 키를 사용 하 여 추가 암호화 계층으로 기본 암호화를 보완할 수 있습니다. 이 문서에서는 Azure CLI, Azure Portal 또는 리소스 관리자 템플릿을 사용 하는 단계를 안내 합니다.
...
> [!IMPORTANT]
> 공용 액세스를 거부 하 고 개인 끝점 또는 선택한 가상 네트워크만 허용 하는 기존 Azure key vault에 레지스트리 암호화 키를 저장 하려는 경우 추가 구성 단계가 필요 합니다. 이 문서의 [고급 시나리오: Key Vault 방화벽](#advanced-scenario-key-vault-firewall) 을 참조 하세요.
...
[Key Vault 방화벽](../key-vault/general/network-security.md)으로 구성 된 기존 Azure key vault를 사용 하 여 암호화 키를 저장 하는 것이 좋습니다 .이는 공용 액세스를 거부 하 고 개인 끝점 또는 선택한 가상 네트워크만 허용 합니다. 

이 시나리오에서는 먼저 [Azure CLI](#enable-customer-managed-key---cli), [포털](#enable-customer-managed-key---portal)또는 [템플릿을](#enable-customer-managed-key---template)사용 하 여 고객이 관리 하는 키로 암호화 된 사용자 할당 id, 주요 자격 증명 모음 및 컨테이너 레지스트리를 새로 만듭니다. 자세한 단계는이 문서의 이전 섹션에 나와 있습니다.
   > [!NOTE]
   > 새 키 자격 증명 모음은 방화벽 외부에 배포 됩니다. 고객이 관리 하는 키를 저장 하는 데 일시적 으로만 사용 됩니다.

레지스트리를 만든 후 다음 단계를 계속 합니다. 세부 정보는 다음 섹션에 나와 있습니다.

1. 레지스트리의 시스템 할당 id를 사용 하도록 설정 합니다.
1. Key Vault 방화벽으로 제한 된 key vault의 키에 대 한 액세스 권한을 시스템 할당 id에 부여 합니다.
1. Key Vault 방화벽에서 신뢰할 수 있는 서비스의 바이패스를 허용 하는지 확인 합니다. 현재, Azure container registry는 시스템 관리 id를 사용 하는 경우에만 방화벽을 무시할 수 있습니다. 
1. Key Vault 방화벽으로 제한 된 주요 자격 증명 모음에서 하나를 선택 하 여 고객 관리 키를 회전 합니다.
1. 더 이상 필요 하지 않은 경우 방화벽 외부에서 만든 키 자격 증명 모음을 삭제할 수 있습니다.


### <a name="step-1---enable-registrys-system-assigned-identity"></a>1 단계-레지스트리의 시스템 할당 id 사용
...
### <a name="step-2---grant-system-assigned-identity-access-to-your-key-vault"></a>2 단계-시스템 할당 id에 키 자격 증명 모음에 대 한 액세스 권한 부여
...
### <a name="step-3---enable-key-vault-bypass"></a>3 단계-키 자격 증명 모음 바이패스 사용
...
### <a name="step-4---rotate-the-customer-managed-key"></a>4 단계-고객이 관리 하는 키 회전

이전 단계를 완료 한 후에는 방화벽 뒤에 있는 key vault에 저장 된 키로 회전 합니다.

1. 포털에서 레지스트리로 이동합니다.
1. **설정** 에서 **암호화**  >  **키 변경** 을 선택 합니다.
1. **Id** 에서 **시스템 할당 됨** 을 선택 합니다.
1. **Key Vault에서 선택** 을 선택 하 고 방화벽 뒤에 있는 Key Vault의 이름을 선택 합니다.
1. 기존 키를 선택 하거나 **새로 만듭니다**. 선택 하는 키는 버전이 지정 되지 않으며 자동 키 회전을 사용 합니다.
1. 키 선택을 완료하고, **저장** 을 선택합니다.
