...
규정 준수 정책에 따라 레지스트리 암호화에 사용 되는 고객 관리 키를 회전 합니다. 새 키를 만들거나 키 버전을 업데이트한 다음, 키를 사용하여 데이터를 암호화하도록 레지스트리를 업데이트합니다. 이러한 단계는 Azure CLI 또는 포털을 사용하여 수행할 수 있습니다.
