...
고객 관리 키를 사용 하는 서버 쪽 암호화는 [Azure Key Vault](../key-vault/key-vault-overview.md)와의 통합을 통해 지원 됩니다. 사용자 고유의 암호화 키를 만들고 Azure Key Vault에 저장할 수도 있고 Azure Key Vault의 API를 사용하여 암호화 키를 생성할 수도 있습니다. Azure Key Vault를 사용 하 여 키 사용을 감사할 수도 있습니다.
...
명령 출력에서 다음 값을 기록해 둡니다. `id` 및 `principalId`. 이후 단계에서 키 자격 증명 모음에 대 한 레지스트리 액세스를 구성 하는 데 이러한 값이 필요 합니다.
...
명령 출력에서 키의 ID를 기록해 둡니다 `kid`합니다. 다음 단계에서이 ID를 사용 합니다.
...
주요 자격 증명 모음을 만드는 단계는 [빠른 시작: Azure Portal를 사용 하 여 Azure Key Vault에서 비밀 설정 및 검색](../key-vault/quick-create-portal.md)을 참조 하세요.
...
다음 템플릿은 새 컨테이너 레지스트리 및 사용자 할당 관리 id를 만듭니다. 다음 내용을 새 파일에 복사 하 고 `CMKtemplate.json`같은 파일 이름을 사용 하 여 저장 합니다.
...
* 액세스 정책 및 [키 자격 증명 모음에](../key-vault/key-vault-secure-your-key-vault.md)대 한 액세스를 보호 하는 방법에 대해 자세히 알아보세요.
