...
> [!TIP]
> *태그에 따라* 삭제하는 방식을 태그 삭제(태그 해제)와 혼동하면 안 됩니다. Azure CLI 명령 [az acr repository untag][az-acr-repository-untag]를 사용하여 태그를 삭제할 수 있습니다. 해당 [매니페스트와](container-registry-concepts.md#manifest) 계층 데이터가 레지스트리에 남아 있으므로 이미지를 태그 지정 해제 때 공간이 해제 되지 않습니다. 태그 참조 자체만 삭제됩니다.
