...
InSpec을 사용하여 가상 컴퓨터, 네트워크 구성, Azure Active Directory 설정 등을 비롯한 구독의 리소스 및 리소스 그룹의 상태를 확인할 수 있습니다.
...
스프레드시트 또는 Word 문서로 작성된 규정 준수 문서는 요구 사항을 해석할 수 있는 열린 여지를 남깁니다. InSpec으로 요구 사항을 버전이 지정되고 실행 가능하며 사람이 읽을 수 있는 코드로 변환합니다. 코드는 명확한 의도로 실제 테스트를 우호적으로 평가해야 하는 것에 대한 대화를 대신합니다.
...
## <a name="audit-azure-virtual-machines-with-policy-guest-configuration"></a>정책 게스트 구성을 통해 Azure 가상 시스템 감사

Azure는 [Azure 정책 게스트 구성을](/azure/governance/policy/concepts/guest-configuration)통해 Azure 가상 컴퓨터를 감사하기 위해 Chef InSpec 정의 사용을 직접 지원합니다. 게스트 구성은 Linux 가상 컴퓨터를 제공된 Chef InSpec 정의로 평가하고 Azure 정책을 통해 규정 준수를 다시 보고합니다. 이러한 감사 결과는 Azure Monitor 로그를 통해서도 보고됩니다. 경고 및 기타 자동화 시나리오를 사용하도록 설정합니다.
...
> [Azure 클라우드 셸에서 InSpec 사용](https://shell.azure.com)