...
- **Azure 구독**: Azure 구독이 없는 경우 시작하기 전에 [무료 계정을](https://azure.microsoft.com/free/?ref=microsoft.com&utm_source=microsoft.com&utm_medium=docs&utm_campaign=visualstudio) 만듭니다.
...
1. **기본** 탭에서 다음 값을 지정한 다음, **확인**을 선택합니다.
   ...
   - **VM 디스크 유형** - 스토리지 디스크 유형에 대해 **SSD** 또는 **HDD**를 지정합니다. Azure의 가상 머신 디스크 유형에 대한 자세한 내용은 [디스크 유형 선택](../virtual-machines/windows/disks-types.md) 문서를 참조하세요.
...
1. **확장 설치** 탭에서 다음 값을 지정한 다음, **확인**을 선택합니다.
    ...
    - **실행 목록** - 컴퓨터에 추가된 Chef 실행 목록을 입력합니다. 이 값은 비워 두면 됩니다.
    - **유효성 검사 클라이언트 이름** - Chef 유효성 검사 클라이언트 이름을 입력합니다. 예를 들어, `tarcher-validator`을 참조하십시오.
    - **유효성 검사 키** - 컴퓨터를 부트스트래핑할 때 사용되는 유효성 검사 키를 포함하는 파일을 선택합니다.
    - **클라이언트 구성 파일** - chef-client의 구성 파일을 선택합니다. 이 값은 비워 두면 됩니다.
    - **Chef 클라이언트 버전** - 설치할 chef 클라이언트의 버전을 입력합니다. 이 값은 비워 두면 최신 버전이 설치됩니다.
    - **SSL 확인 모드** - **없음** 또는 **피어**를 선택합니다. 데모에 ‘없음’이 선택되었습니다.**
    - **Chef 환경** - 이 노드가 구성원이어야 하는 Chef 환경을 입력합니다. 이 값은 비워 두면 됩니다.
    - **암호화된 데이터 백 비밀** - 이 기기에서 액세스하는 데 필요한 암호화된 데이터 백의 비밀이 포함된 파일을 선택합니다. 이 값은 비워 두면 됩니다.
    - **Chef 서버 SSL 인증서** - Chef 서버에 할당된 SSL 인증서를 선택합니다. 이 값은 비워 두면 됩니다.

      ![Linux 가상 머신에 Chef 서버 설치](./media/chef-extension-portal/install-extension.png)

1. 확장 탭이 표시되면 **확인을** **선택합니다.**

1. **설정** 탭이 표시되면 **확인을**선택합니다.

1. **만들기** 탭에 선택한 옵션의 요약이 표시됩니다. 정보 및 **이용 약관을**확인하고 **만들기를**선택합니다.
