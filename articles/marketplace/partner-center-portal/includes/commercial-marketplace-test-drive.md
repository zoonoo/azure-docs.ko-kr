...
- **인스턴스 웹 api url** (필수): Microsoft 365 계정에 로그인 하 고 **설정** \&gt;로 이동 하 여 Dynamics 365 인스턴스의 Web API url을 검색 합니다. **사용자 지정** \&gt; **개발자 리소스** \&gt; **인스턴스 웹 API (서비스 루트 URL)** 에서 여기에 있는 url을 복사 합니다 ( `https://testdrive.crm.dynamics.com/api/data/v9.0`예:).
