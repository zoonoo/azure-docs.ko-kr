...
| x-ms-correlationid | 아닙니다.           | 클라이언트의 작업에 대한 고유한 문자열 값입니다. 이 필드는 클라이언트 작업에서 서버 쪽의 이벤트와 모든 이벤트의 상관 관계를 설정 합니다. 이 값을 제공하지 않으면 값이 하나 생성된 후 응답 헤더에 제공됩니다. |
...
| content-type           |   예        |    `application/json`                                                                   |
|  권한 부여         |   예        |    JWT(JSON Web Token) 전달자 토큰입니다.                                               |
| x-ms-marketplace-session-mode| 아닙니다. | SaaS 제안을 구독하는 동안 시험 실행 모드를 사용하도록 설정하기 위한 플래그입니다. 설정되면 구독 요금이 청구되지 않습니다. ISV 테스트 시나리오에 유용합니다. **' Dryrun '** 로 설정 하세요.|
|  |  |  |
