...
1. **항목의 글머리 기호 목록을**추가 하려면 아래 `<li>` 태그 안에 텍스트를 입력 합니다. @No__t-2 및 `</ul>` 태그 내에서 더 많은 글머리 기호 항목 (`<li>`과 `</li>` 태그 사이에 있는 항목)을 복사 하 여 붙여 넣을 수 있습니다. @No__t-0을 추가 했는지 확인 합니다. 
...
1. 콘텐츠를 **굵게** 표시 하려면 굵게 표시할 텍스트의 시작 부분에 `<b>`을 추가 하 고 굵게 표시할 텍스트의 끝에 `</b>`를 추가 합니다. 
...
1. 콘텐츠 사이에 **줄 바꿈을** 추가 하려면 새 줄에서 시작 하려는 콘텐츠 앞에 `<br>`을 추가 합니다. 공백을 두고 콘텐츠가 새 줄에서 시작 되도록 하려면 콘텐츠 앞에 `<br><br>`을 추가 합니다. 
...
1. **텍스트의 크기를 늘리려면**먼저 텍스트를 표시할 크기를 선택 합니다. 아래 예제를 사용 합니다. 텍스트의 크기를 선택한 후에는 텍스트의 시작 부분 및 끝 부분에 해당 하는 `<H*></H*>` 태그를 추가 합니다. 
    ...
    **예제**:
    ...
    `<h1>`은 제목 1 @ no__t-1입니다. <br>
    `<h2>`은 제목 2 @ no__t-1입니다. <br>
    `<h3>` 제목 3 no__t-1 <br>
    `<h4>`은 제목 4 @ no__t-1입니다. <br>
    `<h5>`은 제목 5 @ no__t-1입니다. <br>
    `<h6>`은 제목 6 @ no__t-1입니다. 
...
- **이미지** (선택 사항): 제품 로고 이미지가 marketplace 전체에 나타날 수 있으며, 크기는 작음: 48 x 48 픽셀 _(필수),_ 보통: 90 x 90 픽셀, 큼: 216 x 216 픽셀 _( 필수),_ 와이드: 255 x 115 픽셀 및 주인공: 815 x 290 픽셀 모든 이미지는에 있어야 합니다. PNG 형식입니다.
...
*@no__t 가격 데이터의 내보내기를 사용 하려면 먼저 가격 책정 변경 내용을 저장 해야 합니다.*
