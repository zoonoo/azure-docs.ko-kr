---
title: Azure 응용 프로그램 제품 만들기-Microsoft 상업적 marketplace
description: 파트너 센터의 상용 marketplace 포털에서 새 Azure 응용 프로그램 제품을 만들기 위한 단계 및 고려 사항에 대해 알아봅니다. Azure Marketplace 또는 CSP (클라우드 솔루션 공급자) 프로그램을 통해 Azure 응용 프로그램 제품을 나열 하거나 판매할 수 있습니다.
author: qianw211
ms.author: dsindona
...
ms.locfileid: "82790690"
---
# <a name="create-an-azure-application-offer"></a>Azure 애플리케이션 제품 만들기

이 문서에서는 상업적 marketplace에서 새 Azure 응용 프로그램 제품을 만들기 위한 단계 및 고려 사항을 설명 합니다. 새 Azure 응용 프로그램 제품을 만들기 전에 이러한 개념을 잘 알고 있어야 합니다. 

새 Azure 응용 프로그램 제품을 게시 하기 전에 [파트너 센터에서 상업적 marketplace 계정을 만들고](https://docs.microsoft.com/azure/marketplace/partner-center-portal/create-account) 계정이 상업적 marketplace 프로그램에 등록 되어 있는지 확인 합니다.

## <a name="before-you-begin"></a>시작하기 전에

Azure 응용 프로그램을 디자인, 빌드 및 테스트 하려면 제품을 빌드하는 데 사용 되는 Azure 플랫폼 및 기술에 대 한 기술 지식이 필요 합니다. 엔지니어링 팀에는 다음 Microsoft 기술에 대한 지식이 있어야 합니다.
...
### <a name="technical-documentation-and-resources"></a>기술 설명서 및 리소스

상업적 marketplace에 대 한 Azure 응용 프로그램 제품을 준비 하는 경우 다음 리소스를 검토 합니다.
...
[Azure Marketplace의 솔루션 템플릿 및 관리 되는 응용](https://channel9.msdn.com/Events/Build/2018/BRK3603) 프로그램은 Azure 응용 프로그램 제품 유형에 대 한 포괄적인 소개를 제공 합니다.
...
Azure 응용 프로그램 계획에는 솔루션 템플릿 및 관리 되는 응용 프로그램의 두 가지 종류가 있습니다.

* **솔루션 템플릿은** Marketplace에 솔루션을 게시 하는 주요 방법 중 하나입니다. 솔루션에 단일 VM (가상 머신) 이외에 추가 배포 및 구성 자동화가 필요한 경우이 계획 유형을 사용 합니다. 솔루션 템플릿을 사용 하면 Vm, 네트워킹 및 저장소 리소스를 비롯 한 둘 이상의 리소스를 자동으로 제공 하 여 복잡 한 IaaS 솔루션을 제공할 수 있습니다.  솔루션 템플릿을 빌드하는 방법에 대 한 자세한 내용은 [Azure Resource Manager](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview)를 참조 하세요.
...
모든 Azure 응용 프로그램에는 `.zip` 보관 파일의 루트 폴더에 적어도 두 개 이상의 파일이 포함 되어 있습니다.

* [Maintemplate.json](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview)라는 리소스 관리자 템플릿 파일입니다.  이 템플릿은 고객의 Azure 구독에 배포할 리소스를 정의 합니다.  리소스 관리자 템플릿의 예는 [Azure 빠른 시작 템플릿 갤러리](https://azure.microsoft.com/resources/templates/) 또는 해당 [GitHub: Azure Resource Manager 빠른 시작 템플릿](https://github.com/azure/azure-quickstart-templates) 리포지토리를 참조 하세요.
...
각 응용 프로그램 계획의 게시 요구 사항에 대 한 자세한 내용은 [솔루션 템플릿 제공 게시 요구](../marketplace-solution-templates.md) 사항 및 [관리 되는 응용 프로그램 제공 게시 요구](../marketplace-managed-apps.md)사항을 참조 하세요.

## <a name="create-a-new-offer"></a>새 제안 만들기

>[!NOTE]
>제품이 게시 된 후에는 제품을 다시 게시할 때까지 파트너 센터에서 사용자가 만든 내용이 상점에 표시 되지 않습니다. 변경을 수행한 후에는 항상 제품을 다시 게시 해야 합니다.

1. [파트너 센터](https://partner.microsoft.com/dashboard/home)에 로그인 합니다.

1. 왼쪽 메뉴에서 **상업용 마켓플레이스** > **개요**를 선택 합니다.

1. 개요 페이지에서 **+ 새 제품** > **Azure 애플리케이션**을 선택 합니다.

    ![왼쪽 탐색 메뉴를 보여 줍니다.](./media/new-offer-azure-app.png)

1. **새 제품** 페이지에서 **제품 ID**를 입력 합니다. 계정의 각 제품에 대 한 고유 식별자입니다.
...
1. **제품 별칭**을 입력 합니다. 파트너 센터의 제안에 사용 되는 이름입니다.

     * 이 이름은 marketplace에서 사용 되지 않으며 고객에 게 표시 되는 제품 이름 및 기타 값과 다릅니다.
     * **만들기**를 선택한 후에는 제품 별칭을 변경할 수 없습니다.

1. **만들기** 를 선택 하 여 제품을 생성 하 고 계속 합니다.

## <a name="offer-setup"></a>제품 설정

**제품 설치** 페이지에서 제품에 대 한 테스트 드라이브 및 리드 관리를 구성할 수 있습니다. 
...
자세한 내용은 [상용 marketplace에서 제품 시험](https://docs.microsoft.com/azure/marketplace/partner-center-portal/test-drive)등록을 참조 하세요. [시험 사용 모범 사례](https://github.com/Azure/AzureTestDrive/wiki/Test-Drive-Best-Practices) 를 읽고 [테스트 드라이브 개요 PDF](https://assetsprod.microsoft.com/mpn/azure-marketplace-appsource-test-drives.pdf) 를 다운로드할 수도 있습니다 (팝업 차단이 해제 되어 있는지 확인).

>[!Note]
>모든 Azure 응용 프로그램은 Azure Resource Manager 템플릿을 사용 하 여 구현 되므로 Azure 애플리케이션에 사용할 수 있는 유일한 테스트 드라이브 유형은 [Azure Resource Manager 기반 테스트 드라이브](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal/test-drive/azure-resource-manager-test-drive)입니다.
...
**속성** 페이지에서는 marketplace에서 제품을 그룹화 하는 데 사용 되는 범주와 산업, 앱 버전 및 제품을 지 원하는 법적 계약을 정의 합니다.

제품을 적절 한 마켓플레이스 검색 영역에 추가 하려면 하나 이상의 범주를 선택 합니다. 제품이 제품 설명에서 이러한 범주를 어떻게 지원 하는지 설명 해야 합니다.
...
이 페이지에서는 상용 marketplace 제품에 대 한 복사 및 이미지를 관리할 수 있습니다. 
...
검색 결과에 사용할 수 있는 제품에 대 한 간략 한 설명을 제공 합니다 (최대 100 자).

#### <a name="long-summary"></a>긴 요약

제품에 대 한 더 긴 설명 (최대 256 자)을 제공 합니다. 이 설명은 검색 결과에 사용할 수 있습니다.

#### <a name="description"></a>설명

제품에 대 한 더 긴 설명 (최대 3000 자)을 제공 합니다. 이 설명은 목록 개요에서 고객에 게 표시 됩니다. 제품의 가치 제안, 주요 이점, 범주 및/또는 업계 연결, 앱 내 구매 기회 및 필요한 공개를 포함 합니다.

SHere는 설명을 작성 하기 위한 몇 가지 팁이 있습니다.
...
* HTML 태그를 사용 하 여 설명 서식을 지정 하는 것이 좋습니다.

#### <a name="search-keywords"></a>검색 키워드

필요에 따라 고객이 marketplace에서 제품을 찾을 수 있도록 최대 3 개의 검색 키워드를 입력할 수 있습니다. 최상의 결과를 위해 설명에 이러한 키워드를 사용 합니다.
...
**+ 링크 추가**를 선택 하 여 솔루션에 대 한 선택적 보조 온라인 문서에 대 한 링크를 추가 합니다.

### <a name="contact-information"></a>연락처 정보

**지원 담당자**, **엔지니어링 연락처**및 **CSP 프로그램 연락처**에 대 한 이름, 전자 메일 및 전화 번호를 제공 합니다. 이 정보는 고객에 게 표시 되지 않지만 Microsoft에서 제공 되며 CSP 파트너에 게 제공 될 수 있습니다. 일부 연락처에는 추가 정보가 필요할 수 있습니다.
...
다음 세 픽셀 크기의 제품 로고에 대 한 PNG 파일을 제공 합니다.
...
3 개의 로고가 모두 필요 하며 목록의 다른 위치에서 사용 됩니다.
...
제품에 Marketplace 계량 서비스 API를 사용 하 여 계량 이벤트를 내보내는 관리 되는 응용 프로그램이 포함 되어 있는 경우에만이 섹션을 완료 합니다. 계량 이벤트를 내보낼 때 서비스에서 사용할 **Azure Active Directory 테 넌 트 id** 및 **AZURE ACTIVE DIRECTORY 응용 프로그램 id** 를 입력 합니다.
...
>[!Note]
>제품 수준 기술 세부 정보는 선택 사항입니다.  요금제를 사용 하 여 관리 되는 응용 프로그램을 게시 하 고 Azure AD 보안 토큰을 사용 하 여 인증 하는 서비스가 있는 경우에만 이러한 세부 정보를 구성 해야 합니다. 자세한 내용은 여러 인증 옵션에 대 한 [인증 전략](./marketplace-metering-service-authentication.md) 을 참조 하세요.

기술 구성에서는 서비스를 식별 하는 데 사용 되는 세부 정보 (테 넌 트 ID 및 앱 ID)를 정의 하며,이를 통해 [Marketplace 계량 서비스 api](./marketplace-metering-service-apis.md)를 사용 하 여 관리 되는 응용 프로그램의 계량 이벤트를 내보냅니다  계량 이벤트를 내보낼 때 서비스에서 사용할 id를 입력 합니다.
...
>[!Note]
>Azure 응용 프로그램 ID는 게시자 ID와 연결 되며,이 게시자 계정 내 에서만 다시 사용할 수 있습니다.

>[!Note]
>[Batch 사용 이벤트](https://docs.microsoft.com/azure/marketplace/partner-center-portal/marketplace-metering-service-apis#batch-usage-event)를 사용 하려는 경우이 구성이 필요 합니다.  [사용 이벤트](https://docs.microsoft.com/azure/marketplace/partner-center-portal/marketplace-metering-service-apis#usage-event)를 제출 하려는 경우 [인스턴스 메타 데이터 서비스](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview) 를 사용 하 여 [JWT (JSON web token) 전달자 토큰](https://docs.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-registration#get-a-token-based-on-the-azure-ad-app)을 가져올 수도 있습니다.

## <a name="plan-overview"></a>계획 개요

이 탭에서는 동일한 제품 내에서 다른 계획 옵션을 제공할 수 있습니다. 이러한 계획 (Cloud 파트너 포털의 Sku 라고도 함)은 계획 유형 (솔루션 템플릿 및 관리 되는 응용 프로그램), 수익 화 또는 대상의 측면에서 다를 수 있습니다.  Marketplace에서 제품을 나열 하기 위해 하나 이상의 계획을 구성 합니다.
...
이 탭에서는 계획의 유형에 대 한 고급 구성을 설정 하 고, 다른 계획의 패키지를 다시 사용 하는지 여부와 계획을 사용할 수 있는 클라우드를 설정할 수 있습니다. 이 탭에 대 한 답변은 동일한 계획에 대 한 다른 탭에 표시 되는 필드에 영향을 줍니다.

#### <a name="plan-type"></a>계획 유형
제품에 대 한 계획 유형을 선택 합니다. **솔루션 템플릿** 계획은 고객이 완전히 관리 합니다. **관리 되는 응용 프로그램** 계획을 통해 게시자는 고객을 대신 하 여 응용 프로그램을 관리할 수 있습니다. 자세한 내용은 [Azure 응용 프로그램 계획의 유형](#types-of-azure-application-plans)을 참조 하세요.
...
>[!Note]
>다른 계획에서 패키지를 다시 사용 하는 경우이 계획에서 전체 기술 구성 탭이 사라집니다. 미래의 모든 업데이트를 포함 하 여 다른 계획의 기술 구성 세부 정보는이 계획에도 사용 됩니다.<br><br>이 계획을 게시 한 후에는이 설정을 변경할 수 없습니다.
...
### <a name="availability-solution-template-plans-only"></a>가용성 (솔루션 템플릿 요금제에만 해당)

계획을 모든 사용자에 게 표시 하 고, 특정 고객 (개인 대상 그룹)에만 표시 하 고, 다른 솔루션 템플릿 또는 관리 되는 응용 프로그램 에서만 사용할 수 있도록 계획을 숨길지 여부를 지정할 수 있습니다.

#### <a name="plan-visibility"></a>계획 표시 유형

모든 사용자 또는 선택한 특정 대상 그룹에 표시 되도록 각 계획을 구성할 수 있습니다. Azure 구독 Id를 사용 하 여이 제한 된 대상 그룹의 멤버 자격을 할당할 수 있습니다.

계획을 비공개로 설정 하 고 선택 하는 제한 된 대상 그룹에만 표시 하기 위한 **비공개 요금제** 를 선택 합니다. 비공개 계획으로 게시 한 후에는 대상 그룹을 업데이트 하거나 계획을 모든 사용자가 사용할 수 있도록 선택할 수 있습니다. 계획이 모든 사용자에 게 표시 되도록 게시 된 후에는 모든 사용자에 게 계속 표시 되어야 합니다. 개인 계획으로 다시 구성할 수 없습니다.

공장을 비공개로 설정 하는 경우 **Azure 구독 id** 와 해당 설명을 입력 합니다. 각은이 개인 계획에 대 한 액세스 권한을 보유 하는 대상입니다. 할당 된 각 Azure 구독 ID에 대 한 설명을 포함 하는 옵션과 함께 Azure 구독 Id를 사용 하 여 액세스 권한을 할당 합니다. CSV 파일을 가져와서 최대 10 명의 고객 구독 Id를 개별적으로 또는 2만으로 추가 합니다. Azure 구독 Id는 Guid로 표시 되며 문자는 소문자 여야 합니다.
...
### <a name="pricing-and-availability-managed-application-plans-only"></a>가격 책정 및 가용성 (관리 되는 응용 프로그램 요금제에만 해당)

이 요금제를 사용 하 여이 요금제를 사용할 수 있는 **시장** , 솔루션 관리의 월별 **가격** 및 특정 고객만 볼 수 있는 경우 **계획 표시** (개인 대상 사용자)를 구성 합니다.
...
월별 가격 외에도 [요금제](./azure-app-metered-billing.md)를 사용 하 여 비표준 단위의 소비량에 대 한 가격을 설정할 수 있습니다.  월 단위 가격은 0으로 설정 하 고, 원하는 경우 요금제를 사용 하 여 독점적으로 요금이 청구 될 수 있습니다. 

USD (USD = 미국 달러)로 설정 된 가격은 저장할 때 현재 환율을 사용 하 여 선택한 모든 시장의 현지 통화로 변환 됩니다. 가격 책정 스프레드시트를 내보내고 각 시장에서 가격을 검토 하 여 게시 하기 전에 이러한 가격의 유효성을 검사 합니다. 개별 시장에서 사용자 지정 가격을 설정 하려면 가격 책정 스프레드시트를 수정 하 고 가져오세요. 
...
이 탭에서 고객이 요금제를 배포할 수 있도록 하는 배포 패키지를 업로드할 수 있습니다.
...
이 탭에서 기술 구성의 초안 버전을 편집할 수 있습니다.

**버전** – 기술 구성의 현재 버전을 할당 합니다.  이 페이지에 변경 내용을 게시할 때마다이 버전을 증가 시킵니다. 버전은 형식 `{integer}.{integer}.{integer}`이어야 합니다.

**패키지 파일** (.zip) –이 패키지에는이 계획에 필요한 모든 템플릿 파일 뿐만 아니라 `.zip` 파일로 패키지 된 추가 리소스도 포함 됩니다.

모든 Azure 응용 프로그램 계획 패키지는 `.zip` 보관 파일의 루트 폴더에 다음 두 개의 파일을 포함 해야 합니다.
...
지원 되는 최대 파일 크기는 다음과 같습니다.

* 총 압축 `.zip` 된 보관 파일 크기 (최대 1gb)
* `.zip` 보관 내에서 압축 되지 않은 개별 파일의 경우 최대 1gb  
...
관리 되는 응용 프로그램 계획에는이 탭에 대 한 추가 정보가 필요 합니다.

#### <a name="previously-published-packages"></a>이전에 게시 된 패키지

**이전에 게시 된 패키지** 하위 탭에서는 게시 된 모든 버전의 기술 구성을 볼 수 있습니다.
...
>[!Note]
>이 기능을 지원 하기 `createUiDefinition.json` 위해 파일을 업데이트 해야 합니다.  

#### <a name="deployment-mode"></a>배포 모드

이 계획을 배포할 때 **전체** 또는 **증분 배포 모드** 를 구성할 지 여부를 선택 합니다. 

* **전체 모드**에서 고객이 응용 프로그램을 다시 배포 하면에서 리소스가 정의 되지 않은 경우 관리 되는 리소스 그룹의 리소스가 제거 됩니다 `mainTemplate.json`. 
...
고객이 관리 되는 리소스에 대해 수행할 수 있는 작업을 지정 하 고, 기본적으로 사용할`*/read`수 있는 "" 작업을 지정 하려면이 옵션을 선택 합니다. 
...
공동 판매 탭에 대 한 정보를 제공 하는 것은 제품을 게시 하는 데 모두 선택 사항입니다. 공동 판매 준비 및 IP 공동 판매 준비 상태를 구현 해야 합니다. 사용자가 제공 하는 정보는 Microsoft 영업 팀에서 고객 요구 사항에 대 한 적합을 평가할 때 솔루션에 대 한 자세한 정보를 제공 하는 데 사용 됩니다. 고객에 게 직접 제공 되지 않습니다.

이 탭에 대 한 자세한 내용은 [파트너 센터의 공동 판매 옵션](https://docs.microsoft.com/azure/marketplace/partner-center-portal/commercial-marketplace-co-sell)을 참조 하세요.

## <a name="resell-through-csps"></a>Csp를 통한 재판매

CSP ( [클라우드 솔루션 공급자](https://azure.microsoft.com/offers/ms-azr-0145p/) ) 프로그램에서 파트너에 게 제공 되는 제품의 도달 범위를 확장 합니다. 이를 통해 대리점은 고객에 게 제품을 판매 하 고 함께 제공 되는 솔루션을 만들 수 있습니다.
...
사용자를 대신해 테스트 드라이브를 배포할 수 있도록 하려면 별도의 고유한 Azure 구독을 만들고 제공 합니다 (Power BI 테스트 드라이브에는 필요 하지 않음).
...
* **AZURE ad 앱 id** (필수) – ad (Azure Active Directory) [응용 프로그램 id](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#get-values-for-signing-in)를 입력 합니다. 이 ID를 찾으려면 [Azure Portal](https://portal.azure.com/)에 로그인 하 고 왼쪽 탐색 메뉴에서 Active Directory 탭을 선택한 다음 **앱 등록**를 선택 하 고 나열 된 **응용 프로그램 ID** 번호 (예: 50c464d3-4930-494c-963c-1e951d15360e)를 찾습니다.
* **Azure Active Directory 응용 프로그램 클라이언트 암호** (필수) – Azure AD 응용 프로그램 [클라이언트 암호](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal#certificates-and-secrets)를 입력 합니다. 이 값을 찾으려면 [Azure Portal](https://portal.azure.com/)에 로그인 합니다. 왼쪽 탐색 메뉴에서 **Azure Active Directory** 탭을 선택 하 고 **앱 등록**을 선택한 다음 테스트 드라이브 앱을 선택 합니다. 그런 다음 **인증서 및 비밀**을 선택 하 고, **새 클라이언트 암호**를 선택 하 고, 설명을 입력 하 고, **만료**됨 **을 선택 하** 고, **추가**를 선택 합니다. 이 페이지를 벗어나기 전에 값을 복사 해야 합니다.

계속 하기 전에 **초안 저장** 을 선택 합니다.

### <a name="marketplace-listing-optional"></a>Marketplace 목록 (선택 사항)

테스트 드라이브 환경에 대해 설명 합니다.
...
* **비디오: 비디오 추가** (선택 사항) – 비디오를 YouTube 또는 Vimeo에 업로드 하 고 링크 및 미리 보기 이미지 (533 x 324 픽셀)를 사용 하 여 여기에서 참조할 수 있으므로 고객은 제품의 기능을 성공적으로 사용 하 고 혜택을 강조 하는 시나리오를 이해 하는 방법을 비롯 하 여 테스트 드라이브를 보다 잘 이해할 수 있도록 정보를 단계별로 볼 수 있습니다.
  * **이름** (필수)
  * **Address** (YouTube 또는 Vimeo only; 필수)
  * **축소판 그림** (이미지 파일은 PNG 형식 이어야 하며 533 x 324 px).

계속 하기 전에 **초안 저장** 을 선택 합니다.

## <a name="publish"></a>게시하기

제품의 필수 섹션을 모두 완료 한 경우 포털의 오른쪽 위 모서리에서 **검토 및 게시** 를 선택 합니다.

제품의 각 섹션에 대 한 완료 상태를 검토 합니다.
...
이 제품을 처음으로 게시 하는 경우 응용 프로그램을 이해 하는 데 도움이 되는 보조 노트 외에도 인증 팀에 테스트 지침을 제공 하 여 앱이 제대로 테스트 되었는지 확인할 수 있습니다.

**제출** 을 선택 하 여 게시를 위해 제품을 제출 합니다. 제품의 미리 보기 버전을 검토 하 고 승인할 수 있는 시기를 알려 주는 전자 메일을 보내 드리겠습니다.

파트너 센터로 돌아가서 제품을 공개 (또는 개인 제품의 경우 비공개 사용자에 게 공개)에 게시 하기 위해 **live live** 를 선택 합니다.
...
하나 이상의 게시 단계에서 오류가 발생 한 경우 제품을 다시 게시 하기 전에 수정 합니다.
