...
ms.locfileid: "69036369"
---
# <a name="saas-offer-creation-checklist"></a>SaaS 제안 생성 검사 목록
...
| **필드 이름**    | **참고**   |  
| :---------------- | :-----------| 
| 제공 ID  | 필수 이며, 만든 후에는 변경할 수 없습니다. 최대 50 문자 및은 소문자, 영숫자, 대시 또는 밑줄로만 구성 되어야 합니다. |
| 제품 별칭  | 필수 요소. |
...
| 잠재적 고객이 제품 목록과 상호 작용 하려면 어떻게 해야 하나요? (동작 호출)  | Microsoft를 통해 판매 하지 않는 경우 필수 사항입니다. 기본값: 무료 평가판, 옵션: "지금 받기", "무료 평가판", "연락처"로 차례로 이동 하세요. |
| 체험 URL  | 고객이 제품 목록과 상호 작용 하는 방식으로 "무료 평가판"을 선택 하는 경우에 필요 합니다. |
| 제안 URL  | 고객이 제품 목록과 상호 작용 하는 방식으로 "지금 가져오기"를 선택 하는 경우에 필요 합니다. |
| 채널  | 선택 사항입니다. 기본값: CSP (대리점) 채널에 옵트인 (opt in) 하지 않습니다.  |
| 시험 사용 | 선택 사항입니다. 기본값: 테스트 드라이브를 사용할 수 없습니다.  |
| 시험 사용 유형 | 테스트 드라이브를 사용 하도록 설정한 경우 필수입니다. 기본값: 선택된 항목이 없습니다. 옵션: Azure Resource Manager 365, dynamics 365 for Business Central, dynamics for Customer Engagement, Dynamics 365 for Operations, 논리 앱, Power BI.  |
...
| **필드 이름**    | **참고**   | 
| :---------------- | :-----------|  
| 범주 및 하위 범주 | 1 및 max 3이 필요 합니다. 기본값: 선택된 항목이 없습니다. |
| 산업 및 하위 산업 | 선택 사항입니다. 각 L1 산업에서 최대 2 개의 L1 산업 및 최대 2 개의 하위 산업, 기본값: 선택된 항목 없음 |
| 앱 버전  | 선택 사항입니다. 기본값: 없음 |
| 표준 계약 사용  | 선택 사항입니다. 기본값: 선택 하지 않습니다.  | |
| 서비스 계약  | 표준 계약을 선택 하지 않은 경우에 필요 합니다.  |
...
| 매장 로고 (소형, 중형, 크게, 전체, 영웅)  | 작음 (48 X 48) 및 큼 (216 X 216) 필요; 다른 크기는 선택 사항 이지만 권장 됩니다. 중간 (90 x 90), 와이드 (255 x 115), 주인공 (815 x 290). PNG 형식 이어야 합니다. |
...
| 계획 이름  | Microsoft를 통해 판매 하는 경우 필요 합니다. 제품의 모든 요금제에서 고유 해야 합니다. 최대 50 문자. |
...
| 가격 플랜  | 필수 요소. 기본값: Flat rate. 옵션: 사용자 당 정액 요금입니다. |
| 최소 및 최대 좌석  | 선택 사항으로, 사용자 기반 가격 책정 모델을 선택한 경우에만 사용할 수 있습니다. |
| 청구 기간  | 필수 요소. 기본값: 월별. 옵션: 매월, 연간 |
| 가격  | 월간 청구 기간을 선택한 경우 매월 필수 USD 연간 청구 기간을 선택한 경우에는 연간 USD입니다. |
| 대상 계획  | 선택 사항입니다. 기본값: 공개 계획. 옵션: 공용, 테 넌 트 ID 별 개인 |
...
| :---------------- | :-----------| 
| 설명  | 필수 요소. |
| 사용자 수동 이름 + 파일  | 필수, 최대 1 개 문서 PDF 형식 이어야 합니다. |
