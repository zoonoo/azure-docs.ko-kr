---
title: Azure 리소스 관리자 테스트 드라이브 | Azure 마켓플레이스
...
Azure 리소스 관리자(리소스 관리자) 템플릿은 솔루션을 가장 잘 나타내도록 디자인하는 Azure 리소스의 코딩된 컨테이너입니다. 리소스 관리자 템플릿이 무엇인지 잘 모르는 경우 [리소스 관리자 템플릿 이해](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview) 및 리소스 관리자 템플릿 [작성을](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-authoring-templates) 읽고 자체 템플릿을 빌드하고 테스트하는 방법을 알고 있는지 확인합니다.
...
Azure 리소스 관리자 테스트 드라이브를 빌드하는 프로세스는 다음과 같습니다.

1. 흐름 다이어그램에서 고객이 수행할 작업을 디자인합니다.
1. 고객이 구축할 환경을 정의합니다.
1. 위의 정의에 따라 고객이 이러한 경험을 수행하는 데 필요한 부분과 리소스(예: D365 인스턴스 또는 데이터베이스가 있는 웹 사이트)를 결정합니다.
1. 로컬로 디자인을 빌드하고 환경을 테스트합니다.
1. ARM 템플릿 배포및 거기에서 환경을 패키징합니다.
    1. 리소스의 일부를 입력 매개 변수로 정의합니다.
    1. 변수는 무엇입니까;
    1. 고객 경험에 제공되는 출력
1. 게시, 테스트 및 라이브 로 이동합니다.
...
제품을 과시하는 가장 좋은 방법은 무엇인지 평가하는 데 충분한 시간을 할애해야 합니다. 특히 리소스 관리자 템플릿을 충분히 쉽게 패키징할 수 있으므로 필요한 모든 리소스를 중심으로 합니다.
...
원하는 리소스 패키지를 디자인한 후에는 이제 테스트 드라이브 리소스 관리자 템플릿을 작성하고 빌드합니다.
...
또한 **모든 매개 변수는 선택 사항이므로**사용하지 않으려면\'필요하지\'않습니다.
...
| **사용자**    | 문자열          | 새 임의 사용자 이름입니다.| admin68876      |
| **다시 설정**    | 보안 문자열    | 새 임의 암호 | Lp!ACS\^2kh     |
...
| templates/solution.json           | \//\<https:\... \>.blob.core.windows.net/\<\... \>/템플릿/솔루션.json |
...
첫 번째 필드이자 가장 중요한 필드는 사용자 제품에 시험 사용을 설정할지 여부를 토글하는 것입니다. **예를** 선택하면 필요한 모든 필드가 있는 양식의 나머지 부분을 입력할 수 있습니다. **아니요를** 선택하면 양식이 비활성화되고 테스트 드라이브를 사용하지 않도록 설정한 후 다시 게시하면 테스트 드라이브가 프로덕션에서 제거됩니다.
...
**테스트 드라이브 기간(시간) -** 테스트 드라이브가 활성 상태로 \# 유지되는 기간에 *소요되는* 시간(시간)입니다. 이 기간이 끝나면 시험 사용이 자동으로 종료됩니다.
...
1. 역할 **할당 탭을 클릭합니다.**  ![새 액세스 제어 주체 추가](./media/azure-resource-manager-test-drive/SetupSub7_1.jpg)
1. **역할 할당 추가를 클릭합니다.**
1. 역할을 **기여자로**설정합니다.
