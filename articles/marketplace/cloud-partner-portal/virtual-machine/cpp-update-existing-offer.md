---
title: Azure Marketplace에서 기존 VM 제안 업데이트 | Microsoft Docs
...
5.  새 VM 이미지 **디스크 버전**을 제공합니다. 디스크 버전은 [의미 체계 버전](http://semver.org/) 형식을 따라야 합니다. 버전은 X.Y.Z 형식이며, X, Y 및 Z는 정수여야 합니다. 제공하는 새 버전이 모든 이전 버전보다 높은지 확인합니다. 그렇지 않으면 새 버전이 다시 게시된 후에 포털 또는 Azure Marketplace에 표시되지 않습니다.
