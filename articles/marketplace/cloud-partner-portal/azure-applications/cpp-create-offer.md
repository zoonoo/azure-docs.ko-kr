---
title: Azure 애플리케이션 제품 만들기 | Microsoft Docs
description: Azure Marketplace에 새 Azure 애플리케이션 제품을 게시하는 방법입니다.
services: Azure, Marketplace, Cloud Partner Portal,
documentationcenter: ''
author: dan-wesley
manager: Patrick.Butler
editor: ''
ms.assetid: ''
ms.service: marketplace
ms.workload: ''
ms.tgt_pltfrm: ''
ms.devlang: ''
ms.topic: conceptual
ms.date: 12/14/2018
ms.author: pbutlerm
ms.openlocfilehash: cae657736d78958259c04dee48034f97ac9d3d1c
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/23/2019
ms.locfileid: "60745244"
---
# <a name="create-an-azure-application-offer"></a>Azure 애플리케이션 제품 만들기

이 문서에서는 Azure Marketplace에 Azure 애플리케이션 제안 항목을 만들고 게시하는 방법에 대해 설명합니다. 모든 제품은 Azure Marketplace에서 고유 엔터티로 나타나며, 제품 정보, SKU, 마케팅 아티팩트 및 지원 정보를 포함하는 자산 및 지원 서비스 그룹으로 구성됩니다.

## <a name="new-offer-form"></a>새 제안 양식

[Cloud 파트너 포털](https://cloudpartner.azure.com/)에 로그인한 다음, 왼쪽 메뉴 모음에서 **+ 새 제품**을 선택합니다. 새 제품 메뉴에서 **Azure 애플리케이션**을 선택하여 **새 제품** 양식을 표시하고 새 제품에 대한 자산을 정의하는 과정을 시작합니다.

![새 제품 양식](./media/azure-new-managedapp-offer.png)

## <a name="next-steps"></a>다음 단계

새 제품 페이지에는 새 제품을 만드는 데 사용할 일단의 탭과 양식 필드가 제공됩니다. 다음 문서에서는 새 제품에 대한 자산 그룹 및 지원 서비스를 정의하는 방법에 대해 설명합니다.

* [제품 설정 탭](./cpp-offer-settings-tab.md)
* [SKU 탭](./cpp-skus-tab.md)
* [시험 사용 탭](./cpp-testdrive-tab.md)
* [Marketplace 탭](./cpp-marketplace-tab.md)
* [지원 탭](./cpp-support-tab.md)
