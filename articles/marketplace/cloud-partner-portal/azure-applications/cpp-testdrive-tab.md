---
title: Azure 응용 프로그램 제공 테스트 드라이브 | Azure 마켓플레이스
...
테스트 드라이브를 사용하도록 설정한 후 다음 양식을 작성하여 테스트 드라이브를 설정합니다.
...
다음 표에서는 관리되는 애플리케이션에 대해 시험 사용을 설정하는 데 필요한 필드를 설명합니다.  별표가 추가된 필드는 필수입니다.
...
| **시험 사용 데모 비디오** |  테스트 드라이브의 선택적 비디오 연습입니다. 고객은 시험 사용을 진행하기 전에 이 비디오를 시청할 수 있습니다. YouTube 또는 Vimeo에 비디오 URL을 제공합니다. **+ 비디오 추가를**선택하면 다음과 같은 정보를 제공하라는 메시지가 표시됩니다.<ul><li>이름</li><li>URL</li><li>썸네일(PNG 형식, 533x324픽셀)</li></ul>  |
...
| **Azure 구독 ID\***       | Azure 서비스 및 Azure Portal 액세스 권한을 부여합니다. 해당 구독에서 리소스 사용량이 보고되며 서비스 대금이 청구됩니다. 테스트 드라이브에 대한 별도의 Azure 구독만 아직 없는 경우 구독을 만듭니다.  |
