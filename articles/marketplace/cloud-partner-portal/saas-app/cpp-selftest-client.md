---
title: Azure SaaS 애플리케이션의 사전 유효성 검사를 위한 자체 테스트 클라이언트 | Microsoft Docs
description: Azure SaaS 애플리케이션의 사전 유효성 검사를 위한 자체 테스트 클라이언트를 만드는 방법입니다.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Azure SaaS 애플리케이션의 사전 유효성 검사를 위한 자체 테스트 클라이언트 만들기
...
|          DNSName          |         테스트하려는 VM의 DNS 이름          |
|          사용자          |          가상 머신에 연결하고 로그인하기 위한 사용자 이름         |
|           암호         |         VM에 연결하고 로그인하기 위한 암호          |
|          OS          |         VM의 운영 체제("Linux" 또는 "Windows")          |
|         PortNo           |         VM에 연결하기 위해 열리는 포트 번호입니다. 포트 번호는 일반적으로 Linux의 경우 "22"이고, Windows의 경우 "5986"입니다.          |
...
다음 화면 캡처에는 온라인 JSON 뷰어에 표시된 JSON 테스트 결과의 예가 나와
...
3. 왼쪽의 탐색 모음에서 **모든 서비스**, **Azure Active Directory**를 차례로 선택합니다.
   ...
   다음 단계에서는 테넌트 이름(또는 디렉터리 이름) 또는 테넌트 ID(또는 디렉터리 ID)가 필요할 수 있습니다.
...
Azure SaaS 애플리케이션이 성공적으로 테스트되면 [제안을 게시](./cpp-publish-offer.md)할 수 있습니다.