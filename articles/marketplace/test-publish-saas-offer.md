---
title: SaaS 제품을 테스트 하 고 Microsoft 상업적 marketplace에 게시 하는 방법
description: 파트너 센터를 사용 하 여 미리 보기, 제품 미리 보기, 테스트를 위한 SaaS 제품을 제출 하 고 Microsoft 상업적 marketplace에 게시 합니다.
...
2. 각 페이지의 **상태** 열에 **완료** 가 표시 되는지 확인 합니다. 가능한 세 가지 상태는 다음과 같습니다.
...
사용자가 로그 오프할 준비가 되 면 제품 미리 보기를 검토 하 고 승인 하도록 요청 하는 전자 메일을 보냅니다. 브라우저에서 **제품 개요** 페이지를 새로 고쳐 제품이 게시자 로그 오프 단계에 도달 했는지 확인할 수도 있습니다. 있는 경우 **라이브 이동** 단추와 미리 보기 링크를 사용할 수 있습니다. 제품을 만들 때 선택한 옵션에 따라 Microsoft AppSource 미리 보기, Azure Marketplace 미리 보기 중 하나 또는 둘 다에 대 한 링크가 있습니다. Microsoft를 통해 제품을 판매 하도록 선택한 경우 미리 보기 대상에 추가 된 모든 사용자가 제품의 획득 및 배포를 테스트 하 여이 단계에서 요구 사항을 충족 하는지 확인할 수 있습니다.

다음 스크린샷은 SaaS 제품에 대 한 **제품 개요** 페이지를 보여 주며 **라이브 이동** 단추 아래에 두 개의 미리 보기 링크가 있습니다. 이 페이지에 표시 되는 유효성 검사 단계는 제품을 만들 때 선택한 내용에 따라 달라 집니다.
...
1. 제품을 라이브로 게시 하기 전에 [Microsoft 상업적 marketplace의 SaaS 제품 api 버전 2](./partner-center-portal/pc-saas-fulfillment-api-v2.md#development-and-testing) 에서 테스트 지침을 검토 하 고 수행 하 여 제품이 api와 성공적으로 통합 되었는지 확인 합니다.

1. 제공 유효성 검사 단계에서 경고가 발생 한 경우에는 **제품 개요** 페이지에 **유효성 검사 보고서 보기** 링크가 표시 됩니다. **라이브 이동** 단추를 선택 하기 전에 보고서를 검토 하 고 문제를 해결 해야 합니다. 그렇지 않으면 인증이 실패 하 고 제품이 실시간으로 이동 하는 것이 지연 될 가능성이 높습니다.
