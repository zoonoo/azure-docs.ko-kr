---
title: Azure 마켓플레이스 및 앱소스 로드맵
description: Microsoft Azure Marketplace에서 일어나고 있는 내용에 대한 로드맵
author: qianw211
ms.author: dsindona
ms.service: marketplace
ms.subservice: partnercenter-marketplace-publisher
ms.topic: conceptual
ms.date: 11/02/2019
ms.openlocfilehash: 24e622f76e604d7c0c48e2f3c354e3b4aa64e549
ms.sourcegitcommit: 09a124d851fbbab7bc0b14efd6ef4e0275c7ee88
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 04/23/2020
ms.locfileid: "82084909"
---
# <a name="microsoft-commercial-marketplace-roadmap"></a>마이크로소프트 상용 마켓플레이스 로드맵

이 문서에서는 게시자, 판매자 및 고객을 위한 Microsoft의 상용 마켓플레이스의 다음 에 대한 보기를 제공합니다. 그것은 우리가 저지른 중요한 기능 중 일부를 캡처, 당신이 그들을 볼 것으로 예상 할 수있는 경우 대략적인 시간 프레임과 매월 새로 고쳐질 것입니다. 모든 새로운 기능의 포괄적인 목록은 아니지만 주요 투자에 대한 가시성을 제공하기 위한 것입니다. 이러한 기능 세트 및 제공 시간 프레임은 현재 상태 기준이며 변경될 수 있습니다.

제안 사항을 보내 주세요! 새 기능, 프로그램 및 이벤트에 대한 최신 업데이트를 받으려면 [Microsoft 파트너 커뮤니티](https://www.microsoftpartnercommunity.com/)의 대화에 참여하세요.

## <a name="roadmap"></a>로드맵
![2019년 11월마켓 로드맵](./media/marketplace-publishers-guide/roadmap-nov19.png)

## <a name="recent-releases"></a>최신 릴리스

기능이 작동되면 로드맵에서 제거됩니다. 마켓플레이스에 추가된 기능은 다음 설명서에서 찾을 수 있습니다.

* [마켓플레이스 + CSP 업데이트 블로그](https://aka.ms/marketplacemarchupdateblog)
* [CSP 채널(ISV)에 오퍼링 게시](https://aka.ms/publishtocsp)
* [마켓플레이스 오퍼링 재판매(CSP)](https://aka.ms/marketplaceincsp)
* [표준 계약](https://aka.ms/standardcontract)
* [SaaS 주문 처리 API](https://aka.ms/saasapiv2)
* [마켓플레이스 지오 및 통화](https://aka.ms/marketplacegeos)
* [Seller Insights](./partner-center-portal/analytics.md)
* [마이크로소프트 빌드 마켓플레이스 블로그](https://aka.ms/marketplacebuildblog)
* [마켓플레이스 5월 업데이트 블로그](https://azure.microsoft.com/blog/microsoft-commercial-marketplace-updates-may-2019/)
* 파트너 센터:[CPP에서](https://docs.microsoft.com/azure/marketplace/partner-center-portal/account-migration-from-cpp-to-pc) [계정](https://docs.microsoft.com/azure/marketplace/partner-center-portal/create-account) / [관리 계정](https://docs.microsoft.com/azure/marketplace/partner-center-portal/manage-account) / 마이그레이션 만들기
* 파트너 센터: [SaaS 오퍼](https://docs.microsoft.com/azure/marketplace/partner-center-portal/create-new-saas-offer) / [게시 상태](https://docs.microsoft.com/azure/marketplace/partner-center-portal/publishing-status
) / [업데이트 기존 오퍼](https://docs.microsoft.com/azure/marketplace/partner-center-portal/update-existing-offer) / [오퍼 분석](https://docs.microsoft.com/azure/marketplace/partner-center-portal/analytics) 만들기
* [파트너 센터에서 마켓플레이스 지원 받기](https://docs.microsoft.com/azure/marketplace/partner-center-portal/support)

## <a name="next-steps"></a>다음 단계

[Azure Marketplace 및 AppSource 게시자 가이드](https://docs.microsoft.com/azure/marketplace/marketplace-publishers-guide) 페이지를 방문합니다.
