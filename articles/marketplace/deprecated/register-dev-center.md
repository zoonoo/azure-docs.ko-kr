...
ms.locfileid: "73825922"
---
# <a name="register-in-dev-center"></a>개발자 센터에 등록  
개인 브라우저 계정에 로그인되지 않았는지 확인하려면 새 Internet Explorer InPrivate 또는 Chrome Incognito{b> <b}브라우징 세션을 엽니다.  
1.  Dev Center에서 판매자로 등록합니다.  
    *   판매자로 로그인하고 등록하려면 [dev.windows.com/registration?accountprogram=azure](https://dev.windows.com/registration?accountprogram=azure)에서 Azure Dev Center 페이지를 방문합니다.  
...
5.  *게시자 표시 이름*을 입력합니다. 이는 일반적으로 여러분의 회사 이름입니다.  
...
9.  `Registration - Review` 섹션에서 계정 정보를 검토하고 모든 정보가 올바른지 확인합니다.  
    ...
    Microsoft Azure Marketplace [게시자 계약](https://cloudpartner.azure.com/Content/Unversioned/PublisherAgreement2.pdf)의 약관을 읽습니다.  
...
11. 사용자 제품에 대한 청구 모델을 사용하여 다음 단계를 선택합니다.  
    ...
    | 상업용(거래) | 계정 정보를 업데이트합니다.<ul> <li>계정 정보를 추가하는 방법에 대한 자세한 내용은 [은행 및 세금 정보를 추가하는 방법](#add-bank-information) 섹션을 방문하세요.</li> </ul> |  
...
1.  로그인 페이지에서 Microsoft ID를 사용하여 로그인합니다. Microsoft ID가 없는 경우 하나를 만듭니다.  
    *   Microsoft ID 만들기에 대한 자세한 내용은 Azure Marketplace 계정을 관리하기 위한 Microsoft ID 만들기 지침 섹션을 참조하세요.  
2.  문제의 세부 정보를 입력합니다.  
...
* **개발자 센터 계정 등록이 필요한 게시자는 무엇인가요?** 
  > 거래 나열 형식(VM, Azure 앱: 솔루션 템플릿 및 관리되는 앱)으로 게시하는 게시자입니다.  

* **개발자 센터 계정이 필요한 이유는 무엇인가요?**
  > 개발자 센터 계정은 Microsoft에서 고객에 게 Transact-sql을 대신 하 여 고객에 게 요금을 청구할 수 있도록 하는 데 필요한입니다. 개발자 센터 계정을 등록하면 Microsoft가 회사의 법률, 세금 및 은행 정보의 유효성을 검사할 수 있습니다. 자세히 알아보려면 [여기](https://docs.microsoft.com/azure/marketplace/become-publisher)를 클릭하세요.

* **개발자 센터 등록을 시작하려면 어떻게 할까요?**
  > 중복을 방지하려면 회사에 등록된 개발자 센터 계정이 없는지 확인합니다. 개발자 계정과 연결하려는 Microsoft 계정으로 로그인해야 합니다. 아직 Microsoft 계정 없는 경우 여기에서 새 계정을 만들 수 있습니다 [.](https://signup.live.com/signup?contextid=AEB123C8F2B1DA7A&bk=1529968434&ru=https%3a%2f%2flogin.live.com%2flogin.srf%3fcontextid%3dAEB123C8F2B1DA7A%26mkt%3dEN-US%26lc%3d1033%26bk%3d1529968434&uiflavor=web&uaid=9987e727fedb459780d95c1b9e46c82c&mkt=EN-US&lc=1033&lic=1) (예: contoso_marketplace@live.com) 로그인하고 등록하려면 https://dev.windows.com/registration?accountProgram=Azure로 이동합니다.
...
* **도시를 정확 하 게 입력 했지만 ' 도시 '가 필요 합니다. "라는 오류 메시지가 표시 되는 이유는 무엇 인가요?**
  > 우편 번호도 올바르게 입력되어 있는지 확인합니다. 도시, 우편 번호 유효성 검사가 있습니다.

* **프로모션 코드를 어떻게 얻을 수 있나요?**
  > [Marketplace 등록](https://partner.microsoft.com/dashboard/account/v3/enrollment/introduction/azureisv) 양식을 완료하여 개발자 센터 프로모션 코드를 받습니다. 자세한 내용은 [파트너 센터 계정을 만드는](https://docs.microsoft.com/azure/marketplace/partner-center-portal/create-account) 방법을 참조 하세요.
 
* **등록을 완료한 후에는 어떻게 되나요?**
  > 제목 줄 "작업 필요: Microsoft 전자 메일 계정 확인"을 사용 하 여 verify@microsoft.com에서 전자 메일을 찾습니다. 등록을 완료하려면 시간이 중요한 확인 링크를 클릭합니다. *24시간 내에 이메일을 받지 못한 경우 스팸 폴더를 확인하세요.*

* **개발자 센터를 확인한 후에 다음 단계는 무엇인가요?**
  > "사용자의 프로그램" (https://developer.microsoft.com/dashboard/Account/Programs)로 이동 하 여 개발자 센터 계정에 로그인 합니다. Azure가 등록 된 프로그램으로 표시 되지 않는 경우 "권장 프로그램을 선택 하십시오."를 확인 한 다음 Azure에서 "시작"을 선택 합니다.
...
* **내 세금 및 지급액 정보를 추가하려면 어떻게 할까요?**
  > 개발 센터 계정에서 세금 및 은행 정보를 완료하지 않고 거래 제안을 게시할 수 없습니다.  계정 정보를 추가하는 방법에 대한 자세한 내용은 [은행 및 세금 정보를 추가하는 방법](https://docs.microsoft.com/azure/marketplace/register-dev-center)을 방문하세요.
