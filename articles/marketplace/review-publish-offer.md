...
- **제공 유효성 검사** (30 분 <)
    ...
    SaaS 제품에 대해서만 콘텐츠 및 기술 구성에 대 한 빠른 유효성 검사를 수행 합니다. 이 단계에서 검색 된 유효성 검사 문제에 대 한 미리 보기 만들기는 차단 되지 않습니다. 인증에 실패할 수 있는 문제가 발견 되 면 **제품 개요** 페이지에 **유효성 검사 보고서 보기** 링크가 표시 됩니다. 이 보고서에는 정책 위반에 대 한 설명이 포함 됩니다.

    > [!IMPORTANT]
    > **라이브 이동** 단추를 선택 하기 전에 제품 유효성 검사 단계에서 모든 경고를 검토 하 고 해결 해야 합니다. 그렇지 않으면 제품이 실시간으로 이동 하는 것을 지연 시키는 인증에 실패할 위험이 있습니다.
...
다음 스크린샷은 파트너 센터의 상업 marketplace 프로그램에서 제공 하는 **개요** 페이지를 보여 줍니다. 이 페이지에 표시 되는 유효성 검사 단계는 제품을 만들 때 선택한 내용 및 제안 유형에 따라 달라 집니다. 인증 단계는 제품 유형에 따라 미리 보거나 라이브에 제출 하는 동안 발생할 수 있습니다.
...
> [!IMPORTANT]
> 종단 간 구매 및 설정 흐름의 유효성을 검사 하려면 미리 보기로 제공 되는 동안 제품을 구매 하세요. 먼저 Microsoft에 [지원 티켓](https://aka.ms/marketplacesupport) 을 알리고 요금을 처리 하지 않도록 합니다. 제품이 _연락처_ 목록 인 경우 미리 보기 중에 연락처 세부 정보를 제공 하 여 잠재 고객이 예상 대로 생성 되는지 테스트 합니다.
...
> [!IMPORTANT]
> **라이브 이동** 단추를 선택 하기 전에 _제품 유효성 검사_ 단계에서 모든 경고를 검토 하 고 해결 해야 합니다. 그렇지 않으면 제품이 실시간으로 이동 하는 것을 지연 시키는 인증에 실패할 위험이 있습니다.
