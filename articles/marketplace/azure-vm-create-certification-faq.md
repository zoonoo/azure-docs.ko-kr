---
title: Azure Marketplace에 대 한 VM 인증 문제 해결
description: 이 문서에서는 Azure Marketplace에 대 한 VM 이미지를 테스트 하 고 인증 하는 일반적인 문제 해결 항목을 다룹니다.
...
게시자는 VM 인증 중에 수행 되는 몇 가지 테스트에 대해 예외를 요청할 수 있습니다. 예외는 게시자가 요청을 지원 하기 위해 증명 정보를 제공 하는 경우 매우 드문 경우에 제공 됩니다. 인증 팀은 언제 든 지 예외를 거부 하거나 승인할 수 있는 권리를 보유 합니다.

아래 섹션에서는 예외를 요청 하는 기본 시나리오와 요청 하는 방법에 대해 설명 합니다.

### <a name="scenarios-for-exception"></a>예외 시나리오

일반적으로 게시자는 예외를 요청 하는 세 가지 시나리오/사례가 있습니다.

- **하나 이상의 테스트 사례에 대 한 예외** – 게시자는 파트너 센터 [지원](https://aka.ms/marketplacepublishersupport) 에 문의 하 여 테스트 사례에 대 한 예외를 요청 합니다.

- **잠긴 vm/루트 액세스 없음** – vm에 설치 된 방화벽과 같은 소프트웨어가 있는 경우 vm을 잠가야 하는 시나리오가 소수의 게시자에 있습니다. 이 경우 게시자는 [인증 된 테스트 도구](https://aka.ms/AzureCertificationTestTool) 를 다운로드 하 고 파트너 센터 [지원](https://aka.ms/marketplacepublishersupport)에서 보고서를 제출할 수 있습니다.

- **사용자 지정 템플릿** – 일부 게시자는 vm을 배포 하기 위해 사용자 지정 ARM 템플릿이 필요한 vm 이미지를 게시 합니다. 이 경우 게시자는 파트너 센터 [지원](https://aka.ms/marketplacepublishersupport) 에서 사용자 지정 템플릿을 제출 해야 하므로 인증 팀에서 유효성 검사를 위해 동일한를 사용할 수 있습니다.

### <a name="information-to-provide-for-exception-scenarios"></a>예외 시나리오에 제공할 정보

게시자는 다음 추가 정보를 사용 하 여 위의 시나리오에 대 한 예외를 요청 하기 위해 파트너 센터 [지원](https://aka.ms/marketplacepublishersupport) 에 문의 해야 합니다.
...
## <a name="address-a-vulnerability-or-exploit-in-a-vm-offer"></a>VM 제품의 취약점 또는 악용 해결

이 섹션에서는 VM 이미지 중 하나를 사용 하 여 취약성 또는 익스플로잇을 검색할 때 새 VM 이미지를 제공 하는 방법을 설명 합니다. 이는 Azure Marketplace에 게시 된 Azure Virtual Machine 제안에만 적용 됩니다.
...
- 취약 한 VM 이미지를 대체할 새 VM 이미지가 있는 경우 아래의 [고정 vm 이미지 제공](#provide-a-fixed-vm-image) 을 참조 하세요.
...
취약성 또는 익스플로잇이 있는 VM 이미지를 대체할 고정 VM 이미지를 제공 하려면 다음을 수행 합니다.

1. 새 VM 이미지를 제공 하 여 보안 취약성 또는 악용을 해결 합니다.
2. 보안 취약성 또는 익스플로잇으로 VM 이미지를 제거 합니다.
3. 제품을 다시 게시 합니다.
...
> [!NOTE]
> 한 번에 하나의 계획에 VM 이미지를 하나만 추가할 수 있습니다. 여러 VM 이미지를 추가 하려면 첫 번째 이미지를 게시 하 고 게시자 로그 오프 단계에 도달할 때까지 기다린 후 다음 VM 이미지를 추가 합니다.
