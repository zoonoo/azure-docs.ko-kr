...
파일의 보안을 유지하려면 모든 파트너가 Azure Blob 저장소 계정에 AppSource 패키지 파일을 저장하고 SAS 키를 사용하여 해당 파일을 공유해야 합니다. Microsoft는 Azure 저장소 위치에서 패키지 파일을 검색하여 인증을 하고 AppSource 평가판에 사용합니다.

Blob 저장소에 패키지를 업로드하려면 다음 단계를 수행합니다.

1.  <http://azure.microsoft.com>으로 이동하여 무료 평가판 또는 유료 계정을 만듭니다.
