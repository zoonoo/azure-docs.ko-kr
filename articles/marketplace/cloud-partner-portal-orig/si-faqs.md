...
지급 모듈에서 트랜잭션을 다운로드 한 후 **트랜잭션 상태**라는 열을 찾아 "유료" 값을 필터링 합니다.   나열 된 각 트랜잭션에 대해 **지급 amount (PC)** 열은 지불 된 금액을 나타냅니다.  트랜잭션과 관련 된 사용 기간을 예측 하려면 트랜잭션 적용 기간에 대 한 마지막 사용 날짜의 근사치 인 열 **요금**을 사용 합니다.
...
이전 달 말일까지 지급 준비가 된 모든 금액이 매월 15일에 지급됩니다. Microsoft는 해당 월의 세 번째 날에 이전 달의 지급 금액을 계산 하 고, 다운로드에서 적용 가능한 모든 청구 트랜잭션을 **지급 Status** 열의 "예정 된 지급"로 업데이트 합니다. 이러한 트랜잭션은 지불 요청이 은행 계정으로 전송 될 때까지 해당 상태로 유지 되며, 이때 **지급 상태가** "지불 됨"으로 업데이트 되 고 "지급 date"는 지불 요청을 제출한 날짜를 표시 하도록 업데이트 됩니다. bank.
