...
지급 모듈에서 트랜잭션을 다운로드하여 레이블이 **지급 상태**인 열을 찾은 다음 “지급됨” 값만 표시되도록 필터를 적용합니다. 고객 세부 정보를 포함 하는 다음과 같은 열이 표시 됩니다. **회사 이름**, **고객 전자 메일**, **고객 국가**, **고객 상태**및 **고객 우편 번호**입니다.
...
지급 모듈에서 트랜잭션을 다운로드 한 후 **트랜잭션 상태**라는 열을 찾아 "유료" 값을 필터링 합니다.   그러면 나열되는 각 트랜잭션에서 레이블이 **지급 금액(PC)** 인 열이 지급된 금액을 나타냅니다.  트랜잭션과 관련 된 사용 기간을 예측 하려면 트랜잭션 적용 기간에 대 한 마지막 사용 날짜의 근사치 인 열 **요금**을 사용 합니다.
