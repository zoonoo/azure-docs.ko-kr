...
위 예제와 같이 이미지, 로고 등의 아티팩트를 웹의 액세스 가능 위치에 업로드한 후 각각을 PUT 요청에 URI로 포함하여 공유해야 합니다. 시스템에서는 이러한 파일이 Azure Marketplace 저장소에 없음을 감지하고 저장소에 다운로드합니다.  결과적으로, 후속 GET 요청은 이러한 파일의 Azure Marketplace 서비스 URL을 반환한다는 것을 알 수 있습니다.
