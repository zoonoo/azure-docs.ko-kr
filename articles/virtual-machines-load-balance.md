<properties urlDisplayName="Load Balancing for Azure Infrastructure Services" pageTitle="Azure 인프라 서비스를 위한 부하 분산" metaKeywords="load balancer, Traffic manager" description="Azure에서 지원하는 두 가지 부하 분산 유형인 클라우드 서비스용 부하 분산 장치와 클라우드 트래픽용 Azure 트래픽 관리자에 대해 설명합니다." metaCanonical="http://www.windowsazure.com/ko-kr/manage/windows/common-tasks/how-to-load-balance-virtual-machines/" services="virtual-machines" documentationCenter="" title="" authors="josephd" solutions="" manager="timlt" editor="" />

<tags ms.service="virtual-machines" ms.workload="infrastructure-services" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="01/01/1900" ms.author="timlt" />

[WACOM.INCLUDE [load-balancing-vms](../includes/load-balancing-vms.md)]

