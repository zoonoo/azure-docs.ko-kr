...
1. 필수 매개 변수를 다음과 같이 입력합니다.
    ...
    [![입력할 매개 변수](media/v2-update-provision/device-five-params.png)](media/v2-update-provision/device-five-params.png#lightbox)
    ...
    | **디바이스 모델** | **이름**: `Chiller`을 입력합니다. </br>**수량**: `3`을 입력합니다. |
...
1. 페이지의 필드를 다음과 같이 입력합니다.
   ...
   [![Time Series Insights 환경을 만들기 위한 페이지](media/v2-update-provision/payg-two-create.png)](media/v2-update-provision/payg-two-create.png#lightbox)

1. 이벤트 원본에 대한 페이지에서 필드를 다음과 같이 입력합니다.

   | | |
   | --- | --- |
   | **이벤트 원본을 만드시겠습니까?** | `Yes`을 입력합니다.|
   | **Name** | 이벤트 원본 이름을 지정하는 데 사용되는 고유한 값을 입력합니다.|
   | **원본 유형** | **IoT Hub**를 선택합니다. |
   | **허브를 선택하시겠습니까?** | **기존 항목 선택**을 누릅니다. |
...
1. 표시된 시계열에서 첫 번째 인스턴스를 선택합니다. 그런 다음, **평균 압력 표시**를 선택합니다.
   ...
   [![평균 압력을 표시하기 위한 메뉴 명령이 포함된 선택된 인스턴스](media/v2-update-provision/analyze-three-show-pressure.png)](media/v2-update-provision/analyze-three-show-pressure.png#lightbox)

   시계열 차트는 오른쪽에 표시됩니다. **간격**을 `15s`로 조정합니다.

   [![시계열 차트](media/v2-update-provision/analyze-four-chart.png)](media/v2-update-provision/analyze-four-chart.png#lightbox)

1. 다른 두 개의 시계열을 사용하여 **3단계**를 반복합니다. 그런 다음, 이 차트에 표시된 대로 모든 시계열을 볼 수 있습니다.
...
1. 형식에 대한 세 개의 변수인 압력, 온도 및 습도를 정의합니다. 다음 정보를 입력합니다.
   ...
   | | |
   | --- | ---|
   | **Name** | `Chiller`을 입력합니다. |
   | **설명** | `This is a type definition of Chiller`을 입력합니다. |

   * 세 개의 변수를 사용하여 압력을 정의합니다.

      | | |
      | --- | ---|
      | **Name** | `Avg Pressure`을 입력합니다. |
...
1. 계층 구조를 정의합니다. 다음과 같이 필드를 입력합니다.
   ...
   | | |
   | --- | ---|
   | **Name** | `Location Hierarchy`을 입력합니다. |
   | **수준 1** | `Country`을 입력합니다. |
   | **수준 2** | `City`을 입력합니다. |
...
1. 오른쪽에서 텍스트 편집기가 표시됩니다. 다음 정보를 추가합니다.
   ...
   [![저장 단추가 포함된 인스턴스 필드](media/v2-update-provision/define-eleven-chiller.png)](media/v2-update-provision/define-eleven-chiller.png#lightbox)
...
1. 다른 센서에서 이전 단계를 반복합니다. 다음 필드를 사용합니다.
   ...
   * 냉각기 01.2의 경우:
     ...
     | --- | --- |
     | **형식** | **냉각기**를 선택합니다. |
     | **설명** | `Instance for Chiller-01.2`을 입력합니다. |
     | **계층 구조** | **위치 계층 구조**를 선택합니다. |
     | **국가** | `USA`을 입력합니다. |
     | **도시** | `Seattle`을 입력합니다. |
     | **빌딩** | `Pacific Science Center`을 입력합니다. |
   ...
   * 냉각기 01.3의 경우:
     ...
     | --- | --- |
     | **형식** | **냉각기**를 선택합니다. |
     | **설명** | `Instance for Chiller-01.3`을 입력합니다. |
     | **계층 구조** | **위치 계층 구조**를 선택합니다. |
     | **국가** | `USA`을 입력합니다. |
     | **도시** | `New York`을 입력합니다. |
...
1. **태평양 과학 센터**에서 시계열을 선택하고 **최대 습도 표시**를 선택합니다.
    ...
    [![최대 습도 표시 메뉴 선택 항목을 사용하여 선택한 시계열](media/v2-update-provision/define-fourteen-show-max.png)](media/v2-update-provision/define-fourteen-show-max.png#lightbox)

1. **1분 간격**으로 **최대 습도**에 대한 시계열이 열립니다. 범위를 필터링하려면 지역을 선택합니다. 그런 다음, 마우스 오른쪽 단추로 클릭하고 **확대/축소**를 선택하여 시간 프레임에서 이벤트를 분석합니다.
