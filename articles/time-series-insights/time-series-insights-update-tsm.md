...
| 이름 | **Name** 속성은 선택 사항이 며 대/소문자를 구분 합니다. **이름을** 사용할 수 없는 경우 기본적으로 **Timeseriesid**로 설정 됩니다. 이름이 제공 되는 경우에도 **Timeseriesid** 는 여전히 사용할 수 [있습니다.](time-series-insights-update-explorer.md#4-time-series-well) |
...
* `Location`부모 `states` 및 자식 `cities`계층을 정의 합니다. 각 `location` 에는 여러 `states`가 있을 수 있으며,이 경우 `cities`에는 여러가 있을 수 있습니다.
* `ManufactureDate`부모 `year` 및 자식 `month`계층을 정의 합니다. 각 `ManufactureDate` 에는 여러 `years`가 있을 수 있으며,이 경우 `months`에는 여러가 있을 수 있습니다.
...
**InstanceFieldNames** 정의의 일부로 계층 **H1** 에 `building`, `floor`및 `room` 가 있는 경우를 예로 들어 보겠습니다.
