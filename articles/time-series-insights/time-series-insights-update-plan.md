...
> [!NOTE]
> 참조 [Azure Time Series Insights GA 환경 계획](time-series-insights-environment-planning.md), 일반 공급 TSI 인스턴스를 계획 하기 위한 모범 사례에 대 한 합니다.
...
* A [시계열 모델](./time-series-insights-update-tsm.md)합니다.
* [이벤트 원본을 Time Series Insights를 연결할](./time-series-insights-how-to-add-an-event-source-iothub.md)합니다.
* [이벤트 소스로 들어오는 이벤트](./time-series-insights-send-events.md) 모두 모델에 매핑되는 및 올바른 JSON 형식입니다.

## <a name="configure-time-series-ids-and-timestamp-properties"></a>시간 시계열 Id 및 타임 스탬프 속성 구성

새로운 Time Series Insights 환경을 만들려면 선택 된 **시간 시리즈 ID**합니다. 이렇게 하면 데이터의 논리 파티션으로 작동합니다. 설명한 대로, 시계열 ID가 준비되었는지 확인합니다.
...
* 시계열 모델에 메타데이터 저장
* 시간 시리즈 모드, 인스턴스 필드 및 이벤트 필요한 정보만 같은 포함 된 **시간 시리즈 ID** 또는 **타임 스탬프**.
