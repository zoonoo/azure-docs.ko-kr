...
1. 앱을 게시하기 전에 *index.html*에서 구성 설정을 변경해야 합니다.
   ...
   1. Azure AD 앱 등록 ID를 사용하도록 앱을 구성하려면 `clientID` 값을 변경하여 [Azure AD를 사용하도록 애플리케이션을 등록](#register-the-application-with-azure-ad)했을 때 **3단계**에서 복사한 **애플리케이션 ID**를 사용합니다. Azure AD에서 **로그아웃 URL**을 만든 경우 해당 값을 `postLogoutRedirectUri` 값으로 설정합니다.
