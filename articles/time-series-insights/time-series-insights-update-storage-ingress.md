...
Azure Time Series Insights 미리 보기는 인스턴스당 최대 2 개의 이벤트 원본을 지원 합니다. 이벤트 원본을 연결 하면 TSI 환경에서 가장 오래 된 이벤트부터 시작 하 여 현재 Iot 또는 Event Hub에 저장 된 모든 이벤트를 읽습니다.
...
| 데이터 형식 | Description |
|---|---|
| **bool** | 두 상태 중 하나를 포함 하는 데이터 `true` 형식 `false`또는입니다. |
| **날짜** | 일반적으로 날짜와 시간으로 표시된 시간을 나타냅니다. [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html) 형식으로 표현 됩니다. |
...
기본적으로 Time Series Insights 미리 보기는 **Time Series Insights 환경 당 최대 1mb (초당 메가바이트)** 의 속도로 들어오는 데이터를 수집할 수 있습니다. [허브 파티션 당](https://docs.microsoft.com/azure/time-series-insights/time-series-insights-update-storage-ingress#hub-partitions-and-per-partition-limits)추가 제한 사항이 있습니다.
...
* **예 1:**
    ...
    Contoso 배송에는 1 분 마다 세 번 이벤트를 내보내는 10만 장치가 있습니다. 이벤트 크기는 200 바이트입니다. Time Series Insights 이벤트 원본으로 4 개의 파티션이 있는 Iot Hub를 사용 하 고 있습니다.
...
* **예 2:**
    ...
    Contoso 제 분석에는 1 초 마다 이벤트를 내보내는 6만 장치가 있습니다. 파티션 수가 4 인 이벤트 허브를 Time Series Insights 이벤트 원본으로 사용 하 고 있습니다. 이벤트 크기는 200 바이트입니다.

    * 환경 수집 율은 **6만 장치 * 200 바이트/이벤트 * 1 이벤트/초 = 12 MBps**입니다.
    * 파티션당 빈도는 3mbps입니다.
    * Contoso 제 분석의 수집 율은 환경 및 파티션 제한을 초과 합니다. Azure Portal를 통해 Time Series Insights 요청을 제출 하 여 환경에 대 한 수집 율을 높이고 미리 보기 제한 내에 더 많은 파티션이 있는 이벤트 허브를 만들 수 있습니다.
...
Azure Storage 계정에서 직접 데이터에 액세스 하려면 Time Series Insights 미리 보기 데이터를 저장 하는 데 사용 되는 계정에 대 한 읽기 권한이 있어야 합니다. 그런 다음 `PT=Time` [Parquet 파일 형식](#parquet-file-format-and-folder-structure) 섹션에서 아래 설명 된 폴더에 있는 Parquet 파일의 생성 시간을 기준으로 선택한 데이터를 읽을 수 있습니다.  저장소 계정에 대 한 읽기 액세스를 사용 하도록 설정 하는 방법에 대 한 자세한 내용은 [저장소 계정 리소스에 대 한 액세스 관리](../storage/blobs/storage-manage-access-to-resources.md)를 참조 하세요.
...
* 첫 번째 초기 복사본은 수집 시간을 기준으로 분할 되 고 거의 도착 하기 위해 데이터를 저장 합니다. 이 데이터는 `PT=Time` 다음 폴더에 있습니다.

  `V=1/PT=Time/Y=<YYYY>/M=<MM>/<YYYYMMDDHHMMSSfff>_<TSI_INTERNAL_SUFFIX>.parquet`

* 두 번째 다시 분할 copy는 시계열 Id로 그룹화 되 고 `PT=TsId` 폴더에 상주 합니다.

  `V=1/PT=TsId/Y=<YYYY>/M=<MM>/<YYYYMMDDHHMMSSfff>_<TSI_INTERNAL_SUFFIX>.parquet`

두 경우 모두 Parquet 파일의 time 속성은 blob 작성 시간에 해당 합니다. 파일에 기록 `PT=Time` 된 후에는 폴더의 데이터가 변경 되지 않고 유지 됩니다. `PT=TsId` 폴더의 데이터는 시간에 따른 쿼리에 최적화 되며 정적이 지 않습니다.

> [!NOTE]
>
> * `<YYYY>`네 자리 연도 표현에 매핑됩니다.
> * `<MM>`두 자리 월 표현에 매핑됩니다.
> * `<YYYYMMDDHHMMSSfff>`네 자리`YYYY`연도 (), 두 자리 월`MM`(), 두 자리 일 (`DD`), 두 자리 시간`HH``MM`(), 두 자리 숫자 (), 두 자리 초`SS`() 및 3 자리 밀리초 (`fff`)를 사용 하 여 타임 스탬프 표현에 매핑합니다.
...
* 원격 분석 데이터로 전송 되는 다른 모든 속성은 속성 형식에 따라 ( `_string` string), `_bool` (부울), `_datetime` (datetime) 또는 `_double` (double)로 끝나는 열 이름에 매핑됩니다.
