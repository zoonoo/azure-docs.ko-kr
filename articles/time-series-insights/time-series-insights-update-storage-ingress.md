...
폴더에 있는 blob 이름의 타임 스탬프는 `PT=Time` 이벤트의 타임 스탬프가 아니라 TSI에 대 한 데이터 도착 시간에 해당 합니다.

`PT=TsId` 폴더의 데이터는 시간이 지남에 따라 쿼리에 최적화되며 정적이지 않습니다. 다시 분할 하는 동안 동일한 이벤트가 여러 blob에 있을 수 있습니다. 또한 blob의 이름은 나중에 변경 될 수 있습니다.
