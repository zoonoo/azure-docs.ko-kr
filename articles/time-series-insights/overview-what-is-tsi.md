---
title: '개요: Azure Time Series Insights Gen2란? - Azure Time Series Insights Gen2 | Microsoft Docs'
description: Azure Time Series Insights Gen2의 변경 내용, 개선 사항 및 기능에 대해 알아봅니다.
...
ms.locfileid: "91667064"
---
# <a name="what-is-azure-time-series-insights-gen2"></a>Azure Time Series Insights Gen2란?

Azure Time Series Insights Gen2는 강력한 기능을 기존 워크플로 또는 애플리케이션에 통합하기 위한 최고의 사용자 환경과 다양한 API를 갖춘 확장 가능한 오픈형 엔드투엔드 IoT 분석 서비스입니다.

이 제품을 사용하여 IoT(사물 인터넷) 규모에서 시계열에 맞게 고도로 컨텍스트화되고 최적화된 데이터를 수집, 처리, 저장, 쿼리 및 시각화할 수 있습니다.

Azure Time Series Insights Gen2는 임시 데이터 탐색 및 운영 분석을 위해 설계되어 숨겨진 추세, 탐지 변칙을 파악하고 근본 원인 분석을 수행할 수 있습니다. 산업용 IoT 배포의 광범위한 필요를 충족하는 유연한 오픈형 제품입니다.

## <a name="video"></a>비디오

Azure Time Series Insights Gen2에 대해 자세히 알아보세요.
...
## <a name="azure-time-series-insights-gen2-for-industrial-iot"></a>산업 IoT용 Azure Time Series Insights Gen2
...
Azure Time Series Insights Gen2는 시계열 데이터의 컨텍스트화를 위한 풍부한 의미 체계 모델링, 자산 기반 인사이트, 검색, 추세, 변칙 검색, 운영 인텔리전스를 위한 최고의 사용자 환경을 갖춘 턴키 및 엔드투엔드 IoT 분석 솔루션을 제공하여 이 시장의 요구 사항을 해결합니다.

대화형 데이터 탐색 기능과 결합된 풍부한 운영 분석 플랫폼에서 Azure Time Series Insights Gen2를 사용하여 IoT 자산에서 수집한 데이터를 최대한 활용할 수 있습니다. Gen2 제품은 다음을 지원합니다.
...
* 고객 소유 Azure Storage 계정에 기록 시계열 데이터를 저장하여 고객이 IoT 데이터 소유권을 유지할 수 있도록 하는 유연한 분석 플랫폼. 데이터는 예측 분석, 기계 학습 및 친숙한 기술(예: Spark 및 Databricks)을 사용하여 수행되는 기타 사용자 지정 컴퓨팅을 비롯한 다양한 데이터 시나리오에서 연결 및 상호 운용성을 지원하는 오픈 소스 Apache Parquet 형식으로 저장됩니다.
...
* 엔드투엔드 분석에 대한 확장성 및 통합 지원. Azure Time Series Insights Gen2는 다양한 데이터 시나리오에 대한 확장 가능한 분석 플랫폼을 제공합니다. Power BI 커넥터를 사용하여 고객은 Azure Time Series Insights Gen2에서 수행하는 쿼리를 Power BI로 직접 가져와 단일 창구에서 통합된 BI 및 시계열 분석을 볼 수 있습니다.
...
Azure Time Series Insights Gen2는 데이터 처리, 스토리지(데이터 및 메타데이터) 및 쿼리에 대해 확장 가능한 종량제 가격 책정 모델을 제공하므로 고객운 비즈니스 요구에 맞게 사용량을 조정할 수 있습니다.

이러한 주요 산업 IoT 기능이 도입되면서, Azure Time Series Insights Gen2는 다음과 같은 주요 혜택도 제공합니다.  
...
| 다른 데이터 솔루션과의 지속적인 통합 | Azure Time Series Insights Gen2 콜드 저장소의 데이터는 오픈 소스 Apache Parquet 파일에 [저장](./concepts-storage.md)됩니다. 이를 통해 비즈니스 인텔리전스, 고급 기계 학습 및 예측 분석을 포함하는 시나리오에서 다른 데이터 솔루션(자사 또는 타사)과 데이터를 통합할 수 있습니다. |
| 거의 실시간 데이터 검색 | [Azure Time Series Insights Gen2 탐색기](./time-series-insights-update-explorer.md) 사용자 환경은 수집 파이프라인을 통한 모든 데이터 스트리밍에 대해 시각화를 제공합니다. 이벤트 원본을 연결하면 이벤트 데이터를 보고 탐색하고 쿼리할 수 있습니다. 이러한 방식으로, 디바이스에서 예상대로 데이터를 내보내는지 여부를 확인할 수 있습니다. IoT 자산의 상태, 생산성 및 전반적인 효율성을 모니터링할 수도 있습니다. |
...
Azure Time Series Insights Gen2 시작:
...
> [!div class="nextstepaction"]
> [Azure Time Series Insights Gen2 자습서](./time-series-insights-update-use-cases.md)
