...
이 환경의 일별 수신 요금이 하루 0.166 GB를 초과할 때마다 일부 데이터가 제거 되므로 180 일 동안 데이터를 저장할 수 없습니다. 이와 동일한 환경을 바쁜 시간대에 사용한다고 생각해 보세요. 환경의 수신 비율이 하루 평균 0.189 GB로 증가할 수 있다고 가정 합니다. 이러한 바쁜 시간대에는 데이터가 약 158일 동안 보존됩니다(30GB/0.189 = 158.73일). 원하는 데이터 보존 기간보다 줄어든 기간입니다.
