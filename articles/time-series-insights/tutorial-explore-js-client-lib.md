...
4. 이제 `<div class="chartsWrapper">` 요소 바로 아래의 `<script type="text/javascript">` 요소를 확장합니다. 인증, TSI 서비스 API 호출, 차트 컨트롤 렌더링 등과 같은 사항에 대한 모든 페이지 로직을 처리하는 데 사용하는 페이지 수준 JavaScript 섹션의 시작 부분이 나타납니다.
