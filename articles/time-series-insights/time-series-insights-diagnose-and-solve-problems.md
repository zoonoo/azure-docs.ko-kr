---
title: 문제 진단, 문제 해결 및 해결 - Azure 타임시리즈 인사이트 | 마이크로 소프트 문서
description: 이 문서에서는 Azure Time Series Insights 환경에서 일반적인 문제를 진단, 해결 및 해결하는 방법에 대해 설명합니다.
...
### <a name="learn-about-common-time-series-insights-customer-challenges-and-mitigationsbr"></a>일반적인 타임시리즈 인사이트 고객 과제 및 완화 방법에 대해 알아보십시오.</br>

> [!VIDEO https://www.youtube.com/embed/7U0SwxAVSKw]

## <a name="problem-no-data-is-shown"></a>문제: 데이터가 표시되지 않습니다.
...
Azure Time Series Insights는 JSON 데이터만 지원합니다. JSON 샘플의 경우 [지원되는 JSON 셰이프를](./how-to-shape-query-json.md)참조하십시오.

### <a name="cause-b-the-event-source-key-is-missing-a-required-permission"></a>원인 B: 이벤트 소스 키에 필요한 권한이 없습니다.

* Azure IoT Hub에 있는 IoT Hub의 경우 **서비스 연결** 사용 권한이 있는 키를 제공해야 합니다. 둘 다 **서비스 연결** 권한이 있기 때문에 **iothubowner** 또는 **서비스** 정책을 선택합니다.
...
### <a name="cause-c-the-consumer-group-provided-isnt-exclusive-to-time-series-insights"></a>원인 C: 제공된 소비자 그룹은 타임시리즈 인사이트를 단독으로 제공하지 않습니다.
...
### <a name="cause-d-the-environment-has-just-been-provisioned"></a>원인 D: 환경이 방금 프로비전되었습니다.

데이터가 환경과 해당 데이터를 처음 만든 후 몇 분 이내에 타임시리즈 인사이트 탐색기에 표시됩니다.

## <a name="problem-some-data-is-shown-but-data-is-missing"></a>문제: 일부 데이터가 표시되지만 데이터가 누락되었습니다.

데이터가 부분적으로만 표시되고 지연되는 것처럼 보이면 여러 가지 가능성을 고려해야 합니다.

### <a name="cause-a-your-environment-is-being-throttled"></a>원인 A: 환경이 제한되고 있습니다.

[제한은](time-series-insights-environment-mitigate-latency.md) 데이터가 있는 이벤트 원본을 만든 후 환경을 프로비전할 때 일반적인 문제입니다. Azure IoT Hub 및 Azure Event Hubs는 최대 7일 동안의 데이터를 저장합니다. Time Series Insights는 항상 이벤트 원본에서 가장 오래된 이벤트부터 시작합니다(선입선출 또는 *FIFO*).
...
제한적 한도는 환경의 SKU 유형 및 용량을 기준으로 적용됩니다. 환경의 모든 이벤트 원본은 이 용량을 공유합니다. IoT 허브 또는 이벤트 허브의 이벤트 소스가 적용된 제한을 초과하는 데이터를 푸시하는 경우 제한 및 지연이 발생합니다.
...
예를 들어 환경이 이벤트 허브에서 메시지를 인더스트한다고 가정합니다. 일일 수신 속도는 67,000개 이하 메시지입니다. 이 속도를 1분마다 약 46개 메시지로 변환합니다. 
...
병합 논리의 작동 방식에 대한 높은 수준의 이해를 위해 [지원되는 JSON 셰이프를](./how-to-shape-query-json.md)참조하십시오.

#### <a name="recommended-resolutions-for-excessive-throttling"></a>과도한 제한에 대한 권장 해결 방법

지연을 해결하려면 사용자 환경의 SKU 용량을 늘립니다. 자세한 내용은 [열렬 인사이트 환경 규모를](time-series-insights-how-to-scale-your-environment.md)참조하십시오.

### <a name="cause-b-initial-ingestion-of-historical-data-slows-ingress"></a>원인 B: 기록 데이터의 초기 수집으로 인해 수레가 느려집니다.
...
## <a name="problem-data-was-showing-previously-but-is-no-longer-showing"></a>문제: 이전에 데이터가 표시되었지만 더 이상 표시되지 않습니다.

TSI는 더 이상 데이터를 수집하지 않지만 이벤트는 여전히 Iot Hub 또는 이벤트 허브로 스트리밍됩니다.

### <a name="cause-a-your-hub-access-key-was-regenerated-and-your-environment-needs-updating"></a>원인 A: 허브 액세스 키가 다시 생성되었으며 환경을 업데이트해야 합니다.
...
새 키로 타임시리즈 인사이트 환경을 업데이트하려면 Azure 포털에서 허브 리소스를 열고 새 키를 복사합니다. TSI 리소스로 이동하여 이벤트 소스를 클릭합니다. 

   [![키를 업데이트합니다.](media/diagnose-and-solve-problems/update-hub-key-step-1.png)](media/diagnose-and-solve-problems/update-hub-key-step-1.png#lightbox)

섭취가 중지된 이벤트 소스를 선택하고 새 키에 붙여넣은 다음 저장을 클릭합니다.

   [![키를 업데이트합니다.](media/diagnose-and-solve-problems/update-hub-key-step-2.png)](media/diagnose-and-solve-problems/update-hub-key-step-2.png#lightbox)

## <a name="problem-my-event-sources-timestamp-property-name-setting-doesnt-work"></a>문제: 내 이벤트 소스의 타임스탬프 속성 이름 설정이 작동하지 않습니다.
...
- *(abc)*: 타임시리즈 인사이트가 데이터 값을 문자열로 읽고 있음을 나타냅니다.
- *캘린더 아이콘*: 타임 시리즈 인사이트가 데이터 값을 *날짜 시간으로*읽고 있음을 나타냅니다.
- *#*: 타임시리즈 인사이트가 데이터 값을 정수로 읽고 있음을 나타냅니다.

## <a name="next-steps"></a>다음 단계

- Azure [Time series Insights에서 대기 시간을 완화하는 방법에](time-series-insights-environment-mitigate-latency.md)대해 읽어보십시오.
