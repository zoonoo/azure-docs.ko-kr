...
이 섹션에서는 응용 프로그램을 대신해 Time Series Insights API에 액세스하도록 응용 프로그램을 구성하는 방법을 설명합니다. 응용 프로그램은 사용자 자격 증명이 아니라 응용 프로그램 자격 증명으로 Time Series Insights 환경에서 데이터를 쿼리하거나 참조 데이터를 게시할 수 있습니다.
...
2. 이 응용 프로그램에 Time Series Insights 환경에 액세스할 수 있는 권한을 부여합니다.
3. 응용 프로그램 ID 및 키를 사용하여 `"https://api.timeseries.azure.com/"` 대상 그룹 또는 리소스에 대한 토큰을 가져옵니다. 그런 다음 토큰을 사용하여 Time Series Insights API를 호출할 수 있습니다.

세부 단계는 다음과 같습니다.

1. Azure Portal에서 **Azure Active Directory** > **앱 등록** > **새 응용 프로그램 등록**을 선택합니다.
...
2. 응용 프로그램의 이름을 지정하고, **웹앱/API** 형식을 선택하고, **로그온 URL**에 대한 유효한 URI를 선택하고, **만들기**를 클릭합니다.

   ![Azure Active Directory에서 응용 프로그램 만들기](media/authentication-and-authorization/active-directory-create-web-api-application.png)

3. 새로 만든 응용 프로그램을 선택하고 해당 응용 프로그램 ID를 원하는 텍스트 편집기에 복사합니다.

   ![응용 프로그램 ID 복사](media/authentication-and-authorization/active-directory-copy-application-id.png)
...
5. 원하는 텍스트 편집기에 키를 복사합니다.
   ...
   ![응용 프로그램 키 복사](media/authentication-and-authorization/active-directory-copy-application-key.png)
...
7. **사용자 선택** 대화 상자에서 응용 프로그램 이름(2단계) 또는 응용 프로그램 ID(3단계)를 붙여 넣습니다.

   ![사용자 선택 대화 상자에서 응용 프로그램 찾기](media/authentication-and-authorization/time-series-insights-data-access-policies-select-user.png)
...
10. 응용 프로그램 ID(3단계)와 응용 프로그램 키(5단계)를 사용하여 응용 프로그램 대신 토큰을 가져옵니다. 응용 프로그램이 Time Series Insights API를 호출할 때 `Authorization` 헤더에서 이 토큰을 전달할 수 있습니다.
