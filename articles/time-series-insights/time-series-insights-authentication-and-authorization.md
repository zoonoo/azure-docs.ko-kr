...
1. **확인** 을 선택하여 정책을 저장합니다.
   ...
   > [!TIP]
   > 고급 데이터 액세스 옵션에 대한 내용은 [데이터 액세스 권한 부여](./concepts-access-policies.md)를 읽어보세요.
...
* MSAL을 사용 하 여 인증 하려면:
   ...
   1. C#에서 다음 코드는 애플리케이션 대신 토큰을 가져올 수 있습니다. Gen1 환경에서 데이터를 쿼리 하는 방법에 대 한 전체 예제는 [c #을 사용 하 여 쿼리 데이터](time-series-insights-query-data-csharp.md)를 참조 하세요.
        ...
        C # 코드에 액세스 하려면 [Azure Time Series Insights](https://github.com/Azure-Samples/Azure-Time-Series-Insights/blob/master/gen1-sample/csharp-tsi-gen1-sample/Program.cs)] 리포지토리를 참조 하세요.
...
[AZURE TIME SERIES INSIGHTS Rest api](/rest/api/time-series-insights/)에 대해 인증 된 쿼리를 수행 하려면 선택한 rest 클라이언트 (Postman, JavaScript, c #)를 사용 하 여 유효한 OAuth 2.0 전달자 토큰을 [인증 헤더](/rest/api/apimanagement/2019-12-01/authorizationserver/createorupdate) 에 전달 해야 합니다.
...
* API 참조 정보를 보려면 [쿼리 API 참조](/rest/api/time-series-insights/gen1-query-api) 설명서를 참조하세요.
