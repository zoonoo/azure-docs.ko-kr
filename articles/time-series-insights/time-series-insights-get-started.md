...
description: Azure Portal를 사용 하 여 새 Azure Time Series Insights 환경을 만드는 방법에 대해 알아봅니다.
...
이 문서에서는 Azure Portal를 사용 하 여 새 Azure Time Series Insights 환경을 만드는 방법을 설명 합니다.

Azure Time Series Insights를 사용 하 여 Azure IoT Event Hubs Hub로 이동 하는 데이터를 몇 분 내에 시각화 하 고 쿼리할 수 있으므로 대량의 시계열 데이터를 몇 초 안에 쿼리할 수 있습니다.  이 기능은 IoT(사물 인터넷) 규모에 맞게 설계되었으며 테라바이트 단위의 데이터를 처리할 수 있습니다.
...
1. **(선택 사항)** 오른쪽 위 모서리에 있는 **고정 아이콘** 을 선택 하 여 나중에 Azure Time Series Insights 환경에 쉽게 액세스할 수 있습니다.
...
* [Azure Time Series Insights 탐색기](https://insights.timeseries.azure.com)에서 환경을 봅니다.
