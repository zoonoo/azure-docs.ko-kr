<properties
   pageTitle="VETR을 사용하여 EAI 논리 앱 만들기 | Microsoft Azure"
...
Azure 관리 포털 화면의 왼쪽 아래에서 **+새로 만들기** 단추를 클릭하고 **논리 앱**을 클릭합니다. 이름, 위치, 구독, 리소스 그룹 및 작동 위치를 선택합니다. 리소스 그룹은 앱 컨테이너 역할을 하며 앱의 모든 리소스가 동일한 리소스 그룹으로 이동됩니다.

이제 트리거 및 동작을 추가하겠습니다.


## HTTP 트리거 추가
1. **처음부터 새로 만들기**를 선택합니다.
1. 갤러리에서 **HTTP 수신기**를 선택하여 새 수신기를 만듭니다. 이를 **HTTP1**이라고 합니다.
2. **자동으로 응답을 보내나요?** 설정을 false로 그대로 둡니다. _HTTP Method_를 _게시_로 설정하고 _상대 URL_을 _ /OneWayPipeline_으로 설정하여 트리거 동작을 구성합니다.  
...
2. 들어오는 XML 메시지를 변환하도록 변환을 구성하려면 **변환** 작업을 이 API가 호출될 때 실행할 작업으로 선택하고 ```triggers(‘httplistener’).outputs.Content```를 _inputXml_의 값으로 선택합니다. 들어오는 데이터가 구성된 모든 변환과 일치하므로 *Map*은 선택적 매개 변수이며, 스키마와 일치하는 항목만 적용됩니다.
3. 마지막으로, 변환은 유효성 검사에 성공한 경우에만 실행됩니다. 이 조건을 구성하려면 오른쪽 맨 위에 있는 기어 아이콘을 클릭하고 _충족할 조건 추가_를 선택합니다. 조건을 ```equals(actions('xmlvalidator').status,'Succeeded')```로 설정합니다.  
...
3. **콘텐츠 형식** 필드를 application/xml로 설정합니다.
...
2. **응답 콘텐츠**를 *파이프라인 처리 완료*로 설정합니다.
3. **응답 상태 코드**를 *200*으로 설정하여 HTTP 200 OK를 나타냅니다.
4. 오른쪽 위에서 드롭다운 메뉴를 클릭하고 **충족할 조건 추가**를 선택합니다. 조건을 ```@equals(actions('azureservicebusconnector').status,'Succeeded')``` <br/> 식으로 설정합니다.
5. 위의 단계를 반복하여 실패에 대해서도 HTTP 응답을 보냅니다. **조건**을 ```@not(equals(actions('azureservicebusconnector').status,'Succeeded'))``` <br/> 식으로 변경합니다.
6. **확인**과 **만들기**를 차례로 클릭합니다.



## 완료
누군가가 HTTP 끝점에 메시지를 보낼 때마다 앱이 트리거되고 방금 만든 작업이 실행됩니다. 만든 논리 앱을 관리하려면 Azure 포털에서 **찾아보기**를 클릭한 후 **논리 앱**을 클릭합니다. 자세한 내용을 보려면 앱을 선택합니다.
