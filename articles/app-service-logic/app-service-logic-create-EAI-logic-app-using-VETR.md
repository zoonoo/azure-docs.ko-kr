...
2. **자동으로 응답을 보내나요?** 설정을 false로 설정합니다. _HTTP Method_를 _게시_로 설정하고 _상대 URL_을 _ /OneWayPipeline_으로 설정하여 트리거 동작을 구성합니다. ![HTTP 트리거][2]
3. 녹색 확인 표시를 선택하여 트리거를 완료합니다.
...
4. 오른쪽 위에서 드롭다운 메뉴를 선택하고 **충족할 조건 추가**를 선택합니다. 조건을 ```@equals(actions('azureservicebusconnector').status,'Succeeded')``` <br/> 식으로 설정합니다.
5. 위의 단계를 반복하여 실패에 대해서도 HTTP 응답을 보냅니다. **조건**을 ```@not(equals(actions('azureservicebusconnector').status,'Succeeded'))``` <br/> 식으로 변경합니다.
6. **확인**과 **만들기**를 차례로 선택합니다.
