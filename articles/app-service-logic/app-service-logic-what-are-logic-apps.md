...
무엇보다도, 논리 앱을 [관리 API][managedapis]와 결합하여 까다로운 통합 시나리오도 쉽게 해결할 수 있습니다.
...
비즈니스 프로세스를 자동화하려는 경우(예: 부정적인 트윗을 찾아 내부 여유 시간 채널에 게시하거나 도착 시 SQL의 새 고객 레코드를 CRM 시스템으로 복제) 논리 앱은 클라우드의 개별 데이터 원본을 온-프레미스로 쉽게 통합할 수 있게 해줍니다. [관리 API][managedapis]에서 자세한 내용을 검토하고 지금 [시작][create]하여 어떤 작업을 수행할 수 있는지 확인하세요.

추가적으로 [BizTalk 관리 API][biztalk]를 사용하여 [규칙 엔진][rules], [거래 업체 관리][tpm] 등의 권한으로 통합 시나리오를 확장할 수 있습니다.
...
- **내재된 확장성** - 필요한 API가 보이지 않나요? 논리 앱은 API 앱과 함께 작동하도록 설계되었기 때문에 사용자 지정 API로 사용할 고유한 API 앱을 쉽게 만들 수 있습니다. 전용 앱을 새로 빌드하거나 마켓플레이스에서 공유하여 수익을 창출합니다.

- **진정한 통합 능력** - 쉽게 시작되고 필요에 따라 확장됩니다. 논리 앱은 통합 전문가가 필요한 솔루션을 빌드할 수 있도록 하기 위해 Microsoft의 업계 선도적인 통합 솔루션인 BizTalk 기능을 쉽게 활용할 수 있습니다. 자세한 내용은 [논리 앱과 함께 제공되는 BizTalk 기능][biztalk]을 참조하세요.
...
- **관리 API** -논리 앱은 데이터 및 서비스에 액세스해야 합니다. 관리 API는 데이터에 연결하고 작업할 때 도움이 되도록 특별히 생성됩니다. [관리 API][managedapis]에서 현재 사용 가능한 API의 목록을 보세요.
- **트리거** - 일부 관리 API는 트리거로도 작동할 수 있습니다. 트리거는 메일 도착이나 Azure 저장소 계정 변경과 같은 특정 이벤트에 따라 워크플로의 새 인스턴스를 시작합니다.
