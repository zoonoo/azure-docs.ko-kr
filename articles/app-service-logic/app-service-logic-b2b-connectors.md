---
title: 논리 앱에서 B2B 커넥터 및 API 앱 | Microsoft Docs
...
> [!NOTE]
> BizTalk TPM(거래 업체 관리) 인스턴스는 AS2 커넥터 및 X12 또는 EDIFACT API 앱을 만드는 데 필요합니다. TPM API 앱을 만들 때 TPM 인스턴스를 만듭니다.
> 
> 
...
> [!IMPORTANT]
> 이전에 만든 커넥터 및 API 앱을 추가하려면 동일한 리소스 그룹에서 논리 앱, 모바일 앱 또는 웹앱을 만듭니다.
> 
> 
...
3. **갤러리**에서 B2B API 앱을 선택하면 자동으로 앱에 추가됩니다. 새 B2B API 앱을 만들 수도 있습니다.
   ...
   > [!IMPORTANT]
   > AS2 커넥터 및 X12, EDIFACT API 앱에는 TPM 인스턴스가 필요합니다. 따라서 새 B2B API 앱을 만드는 경우 먼저 TPM API 앱을 만든 후 AS2 커넥터, X12 API 앱 또는 EDIFACT API 앱을 만듭니다.
   > 
   > 
4. **확인**을 선택하여 변경 내용을 저장합니다.

> [!NOTE]
> Azure 계정에 등록하기 전에 Azure Logic Apps를 시작하려면 [논리 앱을 사용해 보세요](https://tryappservice.azure.com/?appservice=logic). 단기 시작 논리 앱을 즉시 만들 수 있습니다. 신용 카드는 필요하지 않으며 약정도 필요하지 않습니다.
> 
> 
