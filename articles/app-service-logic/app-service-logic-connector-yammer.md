...
4. "Yammer 커넥터"를 클릭하여 Yammer 커넥터 API 앱을 편집기에 놓을 수 있습니다. 권한 부여 단추를 클릭합니다. Yammer 자격 증명을 제공합니다. "허용"을 클릭합니다.
	...
	![][6]
	![][7]
5.	이제 흐름에서 Yammer 커넥터를 사용할 수 있습니다. 흐름의 다른 동작에서 Yammer 트리거("새 메시지")로부터 검색된 새 메시지를 사용할 수 있습니다. 다음과 같이 Yammer 트리거에 대한 입력 속성을 구성합니다.

	- **그룹 ID** - 새 메시지를 검색할 그룹의 ID입니다. 그룹 ID를 제공하지 않는 경우 다음 피드에서 메시지가 검색됩니다. Yammer의 그룹 URL에서 그룹 ID를 검색할 수 있습니다.
		예제: 아래 URL에서 그룹 ID는 "5453203"입니다.
...
6.	유사한 방식으로 흐름에서 "메시지 게시" 동작을 선택하여 메시지를 게시하도록 Yammer 동작을 사용할 수 있습니다. 다음과 같이 "메시지 게시" 동작에 대한 입력 속성을 구성합니다.
	- **메시지 텍스트** - 게시할 메시지의 텍스트내용입니다.
	- **그룹 ID** - 새 메시지가 게시될 그룹의 ID를 지정합니다. 그룹 ID를 제공하지 않는 경우 회사 전체 피드로 게시됩니다. Yammer의 그룹 URL에서 그룹 ID를 검색할 수 있습니다. 
	예제: 아래 URL에서 그룹 ID는 "5453203"입니다.
	https://www.yammer.com/microsoft.com/#/threads/inGroup?type=in_group&feedId=5453203
	- 	**태그 사용자** - 메시지에 태그되어야 하는 사용자 네트워크의 배열입니다. 
	- 	**첨부 파일** - 게시물과 함께 게시될 첨부 파일입니다. 다음 필드를 포함합니다.
		- 파일 이름
		- 콘텐츠
		- 콘텐츠 전송 인코딩
		- 콘텐츠 형식
