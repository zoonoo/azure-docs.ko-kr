...
OneDrive 커넥터를 사용하려면 먼저 OneDrive 커넥터 API 앱의 인스턴스를 만들어야 합니다. 이것은 논리 앱 디자이너가 직접 또는 외부에서 실행될 수 있습니다. 디자이너 외부에서는 다음과 같이 하여 인스턴스를 만들 수 있습니다.

1.	Azure 포털 홈페이지에서 Azure Marketplace를 엽니다.
2.	“모든 항목”에서 “OneDrive 커넥터”를 검색합니다.
3.	다음과 같이 OneDrive 커넥터를 구성합니다.

	![][1] - **이름** - OneDrive 커넥터의 이름 제공 - **앱 서비스 계획** - 앱 서비스 계획 선택 또는 만들기 - **가격 계층** - 커넥터에 대한 가격 책정 계층 선택 - **리소스 그룹** - 커넥터가 있어야 하는 리소스 그룹 선택 또는 만들기 - **구독** - 이 커넥터를 만들 구독 선택 - **위치** - 커넥터를 배포할 지리적 위치 선택
