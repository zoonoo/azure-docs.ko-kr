...
1.	Azure 포털의 왼쪽 위에 있는 + 새로 만들기 옵션을 사용하여 Azure 마켓플레이스를 엽니다.
2.	"웹 + 모바일"으로 이동하여 "SFTP 커넥터"를 검색합니다.
3.	다음과 같이 SFTP 커넥터를 구성합니다.
	...
	- **이름** - SFTP 커넥터에 이름을 부여합니다
	- **패키지 설정**
		- **서버 주소** - SFTP 서버 이름 또는 IP 주소를 지정합니다.
		- **모든 SSH 서버 호스트 키 허용** - 서버의 SSH 공개 호스트 키 지문을 허용할지 결정합니다. false로 설정할 경우 호스트 키가 “SSH 서버 호스트 키 지문” 속성에 지정된 키와 일치하게 지정됩니다.
		- **SSH 서버 호스트 키** - SSH 서버에 대한 공개 호스트 키의 지문을 지정합니다 - *선택 사항*.
		- **루트 폴더** - 루트 폴더 경로를 지정합니다. 빈 경우 루트로 초기화됩니다.
