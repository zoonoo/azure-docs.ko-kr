---
title: 논리 앱에서 BizTalk 규칙 API 앱 알아보기 및 만들기 | Microsoft Docs
...
4. *만들기* 단추를 선택합니다.
   ![대체 텍스트][3]
5. 열리는 새 블레이드에서 다음 정보를 입력합니다.
   1. 이름 – 규칙 API 앱의 이름을 지정합니다.
   2. 앱 서비스 계획 - 새 앱 서비스 계획을 선택하거나 만듭니다.
   3. 가격 책정 계층 – 이 앱이 상주할 가격 책정 계층을 선택합니다.
   4. 리소스 그룹 – 앱이 상주할 리소스 그룹을 선택하거나 만듭니다.
   5. 구독 - 사용하려는 구독을 선택합니다.
   6. 위치 – 앱을 배포할 지리적 위치를 선택합니다.
6. *만들기*를 선택합니다. 몇 분 내에 BizTalk 규칙 API 앱이 만들어집니다.
...
2\. "어휘 정의"를 선택합니다. 그러면 어휘 제작 화면이 표시됩니다. 

1. "추가"를 선택하면 새 어휘 정의를 추가할 수 있습니다.  
   현재 지원되는 두 가지 유형의 어휘 정의는 리터럴과 XML입니다.
...
1. "추가"를 클릭하면 새 "정의 추가" 블레이드가 열립니다. 다음 값을 입력합니다.
   ...
   4. 데이터 형식 – 사용자가 정의 데이터 형식을 선택할 수 있습니다. 현재 다음 4가지 데이터 형식이 지원됩니다.
...
"일괄 추가"를 선택하면 새 블레이드로 이동됩니다. 첫 번째 단계는 여러 정의를 만들 스키마를 선택하는 것입니다. 이 입력을 선택하면 이미 업로드된 스키마 목록에서 선택하거나 새 스키마를 업로드할 수 있는 새 블레이드가 열립니다. 
이제 XPATH 속성이 잠금 해제됩니다. 이 입력을 선택하면 여러 노드를 선택할 수 있는 스키마 뷰어가 열립니다. 
...
개발자가 필요한 어휘를 만들고 나면 비즈니스 분석가가 Azure 포털을 통해 비즈니스 정책을 만듭니다.  
    ...
    2.    이 페이지에는 이 특정 규칙 앱에 있는 정책 목록이 표시됩니다. 분석자는 정책 이름을 입력하고 탭을 두 번 누르기만 하면 새 정책을 추가할 수 있습니다. 여러 정책이 단일 규칙 API 앱에 상주할 수 있습니다.  
    3.    만든 정책을 선택하면 정책에 포함된 규칙을 볼 수 있는 정책 정보 페이지로 이동합니다.  
    ![대체 텍스트][8]
    4.    “추가”를 선택하여 새 규칙을 추가합니다. 그러면 새 블레이드로 이동합니다.

## 규칙 만들기
규칙은 조건 및 동작문의 컬렉션입니다. 조건이 true로 평가되면 동작이 실행됩니다. 규칙 만들기 블레이드에서 해당 정책에 고유한 규칙 이름 및 설명(선택 사항)을 입력합니다. 
조건(IF) 상자를 사용하여 복잡한 조건문을 생성할 수 있습니다. 지원되는 키워드는 다음과 같습니다.  
