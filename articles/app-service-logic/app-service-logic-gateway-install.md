...
1. 서비스가 쿼리를 분석하고 Azure 서비스 버스에 요청을 푸시합니다.
1. 온-프레미스 데이터 게이트웨이가 보류 중인 요청을 위해 Azure 서비스 버스를 폴링합니다.
1. 게이트웨이가 쿼리를 가져오고, 자격 증명의 암호를 해독하고, 해당 자격 증명으로 데이터 원본에 연결합니다.
1. 게이트웨이가 실행을 위해 데이터 원본에 쿼리를 보냅니다.
1. 결과가 데이터 원본에서 게이트웨이로 다시 전송된 후 클라우드 서비스로 전송됩니다. 서비스에서 해당 결과를 사용합니다.
