...
SMTP 커텍터를 사용하면 SMTP 서버에 연결하고 첨부 파일이 있는 이메일을 보내는 동작을 수행할 수 있습니다. SMTP 커넥터 "이메일 보내기" 동작을 사용하면 지정된 이메일 주소로 이메일을 보낼 수 있습니다.
...
7.	이제 흐름에서 SMTP 커넥터를 사용할 수 있습니다. "이메일 보내기" 동작을 선택하고 다음과 같이 입력 속성을 구성합니다.
	- **수신** - 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예제: recipient1@domain.com;recipient2@domain.com.
	- **참조** - 참조 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예제: recipient1@domain.com;recipient2@domain.com.
	- **제목** - 이메일 제목입니다.
	- **본문** - 이메일 본문입니다.
	- **Is HTML** - 이 속성이 true로 설정되면 본문의 내용이 HTML로 보내집니다.
	- **숨은 참조** - 숨은 참조를 위한 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예제: recipient1@domain.com;recipient2@domain.com.
	- **중요도** - 이메일의 중요도입니다. 옵션은 Normal(보통), Low(낮음), High(높음)입니다.
	- **첨부** - 이메일과 함께 보낼 첨부 파일입니다. 다음 필드를 포함합니다.
		- 콘텐츠(문자열)
