...
SMTP 커텍터를 사용하면 SMTP 서버에 연결하고 첨부 파일이 있는 이메일을 보내는 동작을 수행할 수 있습니다.

- SMTP 커넥터 "이메일 보내기" 동작을 사용하면 지정된 이메일 주소로 이메일을 보낼 수 있습니다. 
...
2.	"웹 및 모바일 > API 앱"으로 이동하여 "SMTP 커넥터"를 검색합니다.
3.	다음과 같이 SMTP 커넥터를 구성합니다.
	...
	- **이름** - SMTP 커넥터의 이름을 지정합니다.
	- **패키지 설정**
		- **사용자 이름** SMTP 서버에 연결할 때 사용할 사용자 이름을 지정합니다.
		- **암호** SMTP 서버에 연결할 때 사용할 암호를 지정합니다.
		- **서버 주소** SMTP 서버 이름 또는 IP 주소를 지정합니다.
		- **서버 포트** SMTP 서버 포트 번호를 지정합니다.
		- **SSL 사용** - SSL/TLS 보안 채널을 통해 SMTP를 사용하려면 true를 지정합니다.
...
7.	이제 흐름에서 SMTP 커넥터를 사용할 수 있습니다. "이메일 보내기" 동작을 선택하고 다음과 같이 입력 속성을 구성합니다.
	- **To** - 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예를 들면 다음과 같습니다. recipient1@domain.com;recipient2@domain.com.
	- **Cc** - 참조 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예를 들면 다음과 같습니다. recipient1@domain.com;recipient2@domain.com.
	- **Subject** - 이메일 제목입니다.
	- **Body** - 이메일 본문입니다.
	- **Is HTML** - 이 속성이 true로 설정되면 본문의 내용이 HTML로 보내집니다.
	- **Bcc** - 숨은 참조를 위한 받는 사람의 이메일 주소입니다. 세미콜론(;)을 사용하여 여러 이메일 주소를 구분합니다. 예를 들면 다음과 같습니다. recipient1@domain.com;recipient2@domain.com.
	- **Importance** - 이메일의 중요도입니다. 옵션은 Normal(보통), Low(낮음), High(높음)입니다.
	- **Attachments** - 이메일과 함께 보낼 첨부 파일입니다. 다음 필드를 포함합니다.
		- 콘텐츠(문자열)
		- 콘텐츠 전송 인코딩(Enum)("없음"|"base64")
		- 콘텐츠 형식(문자열)
		- 콘텐츠 ID(문자열)
		- 파일 이름(문자열)
