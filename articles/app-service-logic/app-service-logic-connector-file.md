---
title: "논리 앱에서 파일 커넥터 사용 | Microsoft Docs"
description: "파일 커넥터 또는 API 앱을 만들어서 구성하고 Azure App Service의 논리 앱에서 사용하는 방법"
...
---
# <a name="get-started-with-the-file-connector-and-add-it-to-your-logic-app"></a>파일 커넥터 시작 및 논리 앱에 추가
> [!NOTE]
...
## <a name="creating-a-file-connector-for-your-logic-app"></a>논리 앱용 파일 커넥터 만들기
...
4. **만들기** 단추를 선택합니다.
5. 다음과 같이 파일 커넥터를 구성합니다.  
   ![][1]

   * **이름** - 파일 커넥터의 이름을 지정합니다.
   * **패키지 설정**
     * **루트 폴더** - 호스트 컴퓨터에서 루트 폴더 경로를 지정합니다. 예: D:\FileConnectorTest
     * **Service Bus 연결 문자열** - Service Bus 연결 문자열을 제공합니다. Service Bus 릴레이를 사용할 수 있도록 하려면 Service Bus 네임스페이스는 기본이 아닌 표준 유형이어야 합니다.  Service Bus 릴레이는 하이브리드 연결 관리자에 연결하는 데 사용됩니다.
   * **App Service 계획** - App Service 계획을 선택하거나 만듭니다.
   * **가격 책정 계층** - 커넥터에 대한 가격 책정 계층을 선택합니다.
   * **리소스 그룹** - 커넥터가 상주할 리소스 그룹을 선택하거나 만듭니다.
   * **구독** - 이 커넥터를 만들 구독을 선택합니다.
   * **위치** - 커넥터를 배포할 지리적 위치를 선택합니다.
6. 만들기를 클릭합니다. 새 파일 커넥터가 만들어집니다.

## <a name="configure-hybrid-connection-manager"></a>하이브리드 연결 관리자 구성
API 앱 인스턴스가 만들어지면 해당 대시보드를 찾습니다.  찾아보기 > API 앱 > 파일 커넥터 API 앱 선택을 클릭하여 완료할 수 있습니다.  여기서 하이브리드 연결 관리자를 구성해야 합니다.
하이브리드 연결 관리자의 구성 및 문제 해결에 대한 자세한 내용은 [하이브리드 연결 관리자 사용]을 참조하십시오.

## <a name="using-the-file-connector-in-your-logic-app"></a>논리 앱에서 파일 커넥터 사용
...
4. “파일 커넥터”를 클릭하여 파일 커넥터 API 앱을 편집기에 놓을 수 있습니다. 파일 커넥터는 트리거 1개 및 동작 4개를 노출합니다.  
   ![][5]
5. 각각은 특정 속성을 노출합니다. 아래 이미지는 트리거 및 파일 가져오기 작업에 대한 속성을 나열합니다.  
   ![][6]
6. 이러한 속성이 구성되면 흐름에서 트리거 및 동작을 사용할 수 있습니다. 마찬가지로 다른 동작도 구성할 수 있습니다.

> [!NOTE]
> 파일 트리거는 파일을 폴더에서 성공적으로 읽은 후 해당 파일을 삭제합니다.
>
>

## <a name="file-connector-rest-apis"></a>파일 커넥터 REST API
...
이제 커넥터를 만들었으므로 논리 앱을 사용하여 비즈니스 워크플로에 추가할 수 있습니다. [논리 앱 정의](../logic-apps/logic-apps-what-are-logic-apps.md)를 참조하세요.
...
커넥터의 성능 통계를 검토하고 보안을 제어할 수 있습니다. [논리 앱 모니터링](../logic-apps/logic-apps-monitor-your-logic-apps.md)을 참조하세요.
...
[하이브리드 연결 관리자 사용]: ../app-service-web/web-sites-hybrid-connection-get-started.md
