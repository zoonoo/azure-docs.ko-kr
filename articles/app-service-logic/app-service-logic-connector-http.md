---
title: 논리 앱에서 HTTP Listener 및 커넥터 사용 | Microsoft Docs
...
> [!NOTE]
> 이제 새 논리 앱을 만들 때 이 커넥터의 기능이 기본적으로 **수동 트리거**로 포함되므로 이 커넥터에 대한 지원은 곧 종료될 예정입니다. 이 커넥터를 사용하는 모든 논리 앱을 업그레이드하는 것이 좋습니다. 이 문서 버전은 논리 앱 2014-12-01-미리 보기 스키마 버전에 적용됩니다.
> 
> 
...
1. 새 논리 앱 만들기
2. "트리거 및 동작"을 열어 논리 앱 디자이너를 열고 흐름을 구성합니다. 갤러리에 HTTP Listener가 나열됩니다. 이를 선택합니다.
3. 이제 수신기가 흐름을 트리거하도록 할 HTTP 메서드 및 상대 URL을 설정할 수 있습니다. ![][4] ![][5]
4. 전체 URI를 얻으려면 HTTP Listener를 두 번 클릭하여 해당 구성 설정을 보고 API 앱의 “Host”에 대한 URL을 복사합니다. ![][6]
5. 이제 다음과 같이 흐름에서 다른 작업에서 HTTP 요청에서 받은 데이터를 사용할 수 있습니다. ![][7] ![][8]
6. 마지막으로, 응답을 보내려면 다른 HTTP Listener를 추가하고 Send HTTP Response(HTTP 응답 보내기) 동작을 선택합니다. HTTP Listener에서 얻은 RequestID에 Request ID를 설정하고 다시 반환하려는 응답 본문 및 HTTP 상태를 채웁니다. ![][9]
