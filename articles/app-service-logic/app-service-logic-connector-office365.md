---
title: 논리 앱에서 Office 365 커넥터 사용 | Microsoft Docs
...
> [!NOTE]
> 이 문서 버전은 논리 앱 2014-12-01-미리 보기 스키마 버전에 적용됩니다. 2015-08-01-preview 스키마 버전을 보려면 [Office 365 Outlook API](../connectors/connectors-create-api-office365-outlook.md) 또는 [Office 365 사용자 API](../connectors/connectors-create-api-office365-users.md)를 클릭합니다.
> 
> 
...
5. Office 365에 연결하려면 계정에 액세스할 수 있는 권한을 논리 앱에 부여해야 합니다. '권한 부여'를 클릭하여 Office 365 로그인 자격 증명을 제공합니다. ![][5]
6. Office 365 로그인 페이지로 리디렉션되며 Office 365 계정 자격 증명으로 인증할 수 있습니다. ![][6] ![][7]
7. 권한 부여가 완료되면 Office 365 트리거가 표시됩니다. ![][8]
8. 'New Email(새 메일)' 트리거를 선택하면 입력 매개 변수가 표시됩니다.
9. '분'에 대한 트리거 빈도를 변경하고 ✓를 클릭합니다. ![][9]
10. Office 365 'New Email(새 메일)' 트리거가 구성되고 출력 매개 변수도 표시되는 것을 확인할 수 있습니다. ![][10]
11. 갤러리의 'Recently Used(최근에 사용한 항목)' 섹션에서 'Office 365 Connector'를 선택하면 새 'Office 365' 동작이 추가됩니다.
12. 동작 목록에서 'Send Event(이벤트 보내기)'를 선택하면 'Send Event(이벤트 보내기)' 동작의 입력 매개 변수가 표시됩니다. ![][11]
13. 이벤트 세부 정보를 지정하고 ✓를 클릭합니다. ![][12]
14. 갤러리의 'Recently Used(최근에 사용한 항목)' 섹션에서 'Office 365 Connector'를 선택하면 새 'Office 365' 동작이 추가됩니다.
15. 동작 목록에서 'Add Contact(연락처 추가)'를 선택하면 'Add Contact(연락처 추가)' 동작의 입력 매개 변수가 표시됩니다. ![][13]
16. 'Email Address(메일 주소)' 필드 옆의 '+'를 클릭하고 트리거에서 'From(보낸 사람)' 출력 필드 값을 선택합니다. ![][14]
17. ✓를 클릭하면 동작 구성이 완료됩니다. ![][15]
18. 갤러리의 'Recently Used(최근에 사용한 항목)' 섹션에서 'Office 365 Connector'를 선택하면 새 'Office 365' 동작이 추가됩니다.
19. 동작 목록에서 'Send Email(메일 보내기)'을 선택하면 'Send Email(메일 보내기)' 동작의 입력 매개 변수가 표시됩니다. ![][19]
20. 메일을 보내는 데 필요한 세부 정보를 입력합니다. 아래와 같은 내용을 입력하여 메시지를 작성할 수 있습니다. ‘메일 보내기' 동작이 구성되면 ✓를 클릭합니다.
    
     Body - @concat('You got a new sales enquiry from',triggers().output.body.From)
    
    ![][20]
21. 갤러리의 'Recently Used(최근에 사용한 항목)' 섹션에서 'Office 365 Connector'를 선택하면 새 'Office 365' 동작이 추가됩니다.
22. 동작 목록에서 'Reply To(회신)'를 선택하면 'Reply To(회신)' 동작의 입력 매개 변수가 표시됩니다. ![][16]
23. 'From(보낸 사람)' 필드 옆의 '+'를 클릭하고 트리거의 출력 메시지 ID에서 값을 선택한 후 ✓를 클릭합니다. ![][17]
24. 논리 앱 편집기 화면에서 확인을 클릭한 다음 '만들기'를 클릭합니다. 만들기를 완료하는 데 30초 정도 걸립니다.
25. 트리거를 사용하여 구성한 계정에 메일을 보내면 Office 365 메일 계정에 개인 메일 계정의 메일, 일정 이벤트 및 연락처가 표시되어야 합니다. 또한 판매 문의에 곧 응답하겠다고 알리는 회신이 수신되어야 합니다.
