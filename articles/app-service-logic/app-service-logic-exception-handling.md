...
1. **배열 필터링**의 조건은 상태가 `Failed`와(과) 같은 모든 `@result()` 항목입니다.  `My_Scope` 에서 모든 작업 결과 배열을 실패한 작업 결과 배열만으로 필터링합니다.
1. **필터링된 배열** 출력에서 **각** 작업을 수행합니다.  위에서 필터링한 실패한 작업 결과에 *for each* 작업을 수행합니다.
