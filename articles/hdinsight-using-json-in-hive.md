...
- 둘째로, get_json_object()는 배열의 문자열 표현을 반환합니다. 따라서 이를 하이브 배열로 변환하려면 정규식을 사용하여 대괄호 ‘[‘ 및 ‘]’를 바꾼 다음 분할 호출하여 배열을 가져와야 합니다.
...
c) 지정된 학생의 성적이 80점을 넘는 과목 찾기 
    ...
    select jt.StudentClassCollection.ClassId from json_table jt lateral view explode(jt.StudentClassCollection.Score) collection as score where score > 80;
