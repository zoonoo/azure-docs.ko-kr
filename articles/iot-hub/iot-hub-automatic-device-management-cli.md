...
* [Azure CLI에 대 한 IoT 확장](https://github.com/Azure/azure-cli)입니다.
...
자동 모듈 구성은 매우 유사 하 게 작동 하지만 대신 `moduleContent` 를 대상 `deviceContent`으로 합니다.
...
모듈에 대 한 메트릭 쿼리는 장치에 대 한 쿼리와도 유사 하지만 `moduleId` 에서 `devices.modules`를 선택 합니다. 다음은 그 예입니다. 
