...
1. **addtagsandqueryapp**라는 빈 폴더를 새로 만듭니다. **addtagsandqueryapp** 폴더의 명령 프롬프트에 다음 명령을 사용하여 package.json 파일을 만듭니다. 매개 `--yes` 변수는 모든 기본값을 허용합니다.
...
4. **AddTagsAndQuery.js** 파일에 다음 코드를 추가합니다. IoT 허브 연결 문자열 받기에서 복사한 `{iot hub connection string}` [IoT Hub 연결 문자열로](#get-the-iot-hub-connection-string)바꿉입니다.
...
5. 다음 코드를 **queryTwins** 함수를 구현할 **AddTagsAndQuery.js** 끝 부분에 추가합니다.
   ...
   ```
    ...
    코드에서 **쿼리** 개체를 만들면 두 번째 매개 변수에서 반환되는 문서의 최대 수를 지정합니다. **query** 개체에는 모든 결과를 검색하기 위해 여러번 **nextAsTwin** 메서드를 호출하는 데 사용할 수 있는 **hasMoreResults** 부울 속성이 들어 있습니다. **다음이라는** 메서드는 장치 쌍이 아닌 결과(예: 집계 쿼리의 결과)에 사용할 수 있습니다.
...
6. 다음으로 애플리케이션을 실행합니다.
   ...
   **Redmond43**에 위치한 모든 디바이스를 요청하는 쿼리에 대한 결과로는 하나의 디바이스를 보고 셀룰러 네트워크를 사용하는 디바이스에 대해서는 결과를 제한하는 쿼리에 대한 결과로는 아무 디바이스도 볼 수 없어야 합니다.
   ...
   ![쿼리 결과에서 하나의 장치 보기](media/iot-hub-node-node-twin-getstarted/service1.png)
...
1. **reportconnectivity**라는 빈 폴더를 새로 만듭니다. **reportconnectivity** 폴더의 명령 프롬프트에서 다음 명령을 사용하여 package.json 파일을 만듭니다. 매개 `--yes` 변수는 모든 기본값을 허용합니다.

    ```cmd/sh
    npm init --yes
    ```

2. **보고서 연결** 폴더의 명령 프롬프트에서 다음 명령을 실행하여 **azure-iot 장치**및 **azure-iot 장치-mqtt** 패키지를 설치합니다.
...
4. **보고서 연결.js** 파일에 다음 코드를 추가합니다. `{device connection string}` [IoT 허브에서 새 장치 등록에서](#register-a-new-device-in-the-iot-hub) **myDeviceId** 장치 ID를 만들 때 복사한 장치 연결 문자열로 바꿉꿉입니다.
...
6. 디바이스가 연결 정보를 보고했으므로 두 쿼리 모두에 나타나야 합니다. **addtagsandqueryapp** 폴더로 돌아가 쿼리를 다시 실행합니다.
    ...
    ![두 쿼리 결과모두에서 myDeviceId 표시](media/iot-hub-node-node-twin-getstarted/service2.png)
...
* [IoT Hub 자습서를 시작으로](quickstart-send-telemetry-node.md) 장치에서 원격 분석을 전송하고,

* [desired 속성을 사용하여 디바이스 구성](tutorial-device-twins.md) 자습서를 참조하여 디바이스 쌍의 desired 속성을 사용하여 디바이스 구성,

* 직접 [메서드 사용](quickstart-control-device-node.md) 자습서를 사용하여 대화형으로 제어할 수 있습니다(예: 사용자 제어 앱에서 팬 켜기).
