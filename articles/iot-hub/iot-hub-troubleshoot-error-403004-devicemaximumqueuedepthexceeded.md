...
이 한도를 실행 하는 가장 큰 이유는 HTTPS를 사용 하 여 메시지를 수신 하는 것입니다 .이 경우 `ReceiveAsync`를 사용 하 여 연속 폴링이 발생 하므로 요청을 IoT Hub 제한 하 게 됩니다.
...
마지막으로, 한도에 도달 하기 전에 [제거 큐 API](https://docs.microsoft.com/rest/api/iothub/service/purgecommandqueue) 를 사용 하 여 보류 중인 메시지를 정기적으로 정리 하는 것이 좋습니다.