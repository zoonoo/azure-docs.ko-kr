...
이 시리즈의 첫 번째 문서에서는 [C용 Azure IoT 디바이스 SDK를 소개](iot-hub-device-sdk-c-intro.md)했습니다. 다음 문서에서는 [C용 Azure IoT 디바이스 SDK -- IoTHubClient](iot-hub-device-sdk-c-iothubclient.md)에 대해 자세히 설명합니다. 이 문서에서는 나머지 구성 요소인 **serializer** 라이브러리에 대한 보다 자세한 설명을 제공하여 SDK의 범위를 보완합니다.
...
이 샘플에서는 SDK에서 지원되는 추가 데이터 형식을 다루지 않습니다. 다음에 설명합니다.
...
마지막 데이터 형식부터 살펴봅시다. **DECLARE\_STRUCT** 로 기타 기본 형식의 그룹인 복합 데이터 형식을 정의할 수 있습니다. 이러한 그룹을 통해 다음과 같은 모델을 정의할 수 있습니다.
...
이 코드는 이전 섹션에 설명된 **SendAsync** 도우미의 하위 집합이므로 여기서는 다시 다루지 않겠습니다.
...
이제 동일한 데이터를 포함하지만 다른 구조체를 사용하도록 모델을 수정해 봅시다.
...
예제에서는 **DECLARE\_STRUCT** 매크로를 제거했으며 모델링 언어의 단순 형식을 사용하여 시나리오의 데이터 항목을 정의합니다.

일단 **시간** 이벤트는 무시하겠습니다. 이와 별개로 다음은 **Temperature** 를 수신하는 코드입니다.
...
즉, 이 코드는 **온도** 및 **습도** 를 별도로 전송하는 것과 같다고 예상할 수 있습니다. 두 이벤트를 **SERIALIZE** 에 동일한 호출로 전달하는 것은 단지 편의를 위해서입니다. 그러나 이 경우에는 해당하지 않습니다. 대신, 위의 코드는 IoT Hub에 단일 데이터 이벤트를 전송합니다.
...
더 중요한 것은 **Temperature** 및 **Humidity** 가 동일한 구조체에 있는 이벤트를 모델링하지 않았다는 점입니다.
...
이 동작은 **serializer** 라이브러리의 가정 사항을 알고 있는 경우 보다 쉽게 이해할 수 있습니다. 이해를 위해 모델을 다시 살펴봅시다.
...
이 모델을 개체 지향 용어로 생각해봅니다. 예제에서는 물리적 디바이스(자동 온도 조절기)를 모델링하고 해당 디바이스가 **온도** 및 **습도** 와 같은 특성을 포함합니다.
...
그러면 **Temperature** 및 **Time** 멤버를 포함하는 **TemperatureEvent** 를 정의한 것처럼 정확히 같은 직렬화된 이벤트를 생성합니다. 이것은 모델 1에서 진행했습니다. 예제에서는 **SERIALIZE** 를 다른 방식으로 호출했으므로 다른 모델(모델 2)을 사용하여 정확히 동일하게 직렬화된 이벤트를 생성할 수 있었습니다.

중요한 사항은 여러 데이터 이벤트를 **SERIALIZE** 로 전달하면 각 이벤트가 단일 JSON 개체의 속성인 것으로 가정한다는 것입니다.

최적의 접근 방식은 사용자와 사용자가 모델에 대해 어떻게 생각하는지에 따라 결정됩니다. “이벤트”를 클라우드로 전송하고 각 이벤트에 정의된 속성 집합이 있다면 첫 번째 방법이 훨씬 효율적입니다. 이 경우 **DECLARE\_STRUCT** 를 사용하여 각 이벤트의 구조체를 정의한 후 **WITH\_DATA** 매크로를 사용하여 이를 모델에 포함합니다. 그런 다음 위의 첫 번째 예에서 했던 것처럼 각 이벤트를 전송합니다. 이 방법에서는 단일 데이터 이벤트만 **SERIALIZER** 에 전달합니다.

모델을 개체 지향 방식으로 생각한다면 두 번째 접근 방식이 적합할 수 있습니다. 이 경우에 **WITH\_DATA** 를 사용하여 정의된 요소는 개체의 "속성"입니다. 클라우드로 전송하려는 “개체” 상태의 양에 따라 이벤트의 하위 집합을 원하는 대로 **SERIALIZE** 에 전달합니다.
...
지금까지 이 문서에서는 이벤트를 IoT Hub에 전송하는 것에 대해서만 다루었으며 수신 메시지에 대해서는 다루지 않았습니다. 메시지 수신에 대해 알아야 하는 사항은 [C용 Azure IoT 디바이스 SDK](iot-hub-device-sdk-c-intro.md) 문서에서 폭넓게 다뤘기 때문입니다. 메시지 콜백 함수를 등록하여 메시지를 처리했던 문서를 떠올려 보세요.
...
GitHub에서 Azure-iot-sdk-c 리포지토리를 복제하고 `git submodule update --init` 명령을 실행한 경우 아래에서 이 공유 유틸리티 라이브러리를 찾을 수 있습니다.
...
이 솔루션의 프로그램은 **macro\_utils.h** 파일을 생성합니다. SDK에는 기본 macro\_utils.h 파일이 포함되어 있습니다. 이 솔루션을 사용하면 일부 매개 변수를 수정하고 이러한 매개 변수를 기반으로 헤더 파일을 다시 작성할 수 있습니다.

여기서 고려할 두 가지 핵심 매개 변수는 **nArithmetic** 및 **nMacroParameters** 로, macro\_utils.tt에 있는 다음 두 줄로 정의됩니다.
...
마찬가지로, 라이브러리 작업을 완료한 후 **serializer\_deinit** 을 마지막으로 호출합니다.
...
이 문서에서는 **C용 Azure IoT 디바이스 SDK** 에 포함된 **serializer** 라이브러리의 고유한 측면을 자세히 설명합니다. 제공된 정보로 모델을 사용하여 이벤트를 전송하고 IoT Hub에서 메시지를 수신하는 방법을 잘 이해할 수 있습니다.

또한 **C용 Azure IoT 디바이스 SDK** 로 애플리케이션을 개발하는 방법에 대한 3부로 구성된 시리즈를 완료합니다. 이 시리즈는 시작하고 API의 작동 방식을 완전히 이해하는 데 충분한 정보를 제공합니다. 자세한 정보를 위해 여기에서 다루지 않은 몇 가지 샘플이 SDK에 제공됩니다. [Azure IoT SDK 설명서](https://github.com/Azure/azure-iot-sdk-c) 리소스에서도 유용한 추가 정보가 제공됩니다.
