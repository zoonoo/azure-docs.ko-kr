...
IoT Hub의 주요 사용 사례 중 하나는 장치에서 원격 분석을 수집하는 것입니다. 이러한 이유로 IoT Hub를 [Azure Event Hubs][Azure Event Hubs]와 자주 비교합니다. IoT Hub와 마찬가지로 이벤트 허브는 짧은 대기 시간 및 높은 안정성으로 엄청난 규모의 클라우드에 이벤트 및 원격 분석을 가능하게 하는 이벤트 처리 서비스입니다.
