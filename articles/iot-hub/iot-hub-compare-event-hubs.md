---
title: Azure IoT Hub를 Azure 이벤트 허브에 비교 | Microsoft Docs
...
---
# <a name="comparison-of-iot-hub-and-event-hubs"></a>IoT Hub 및 이벤트 허브의 비교
Azure IoT Hub의 주요 사용 사례 중 하나는 장치에서 원격 분석을 수집하는 것입니다. 이런 이유로 IoT Hub를 [Azure 이벤트 허브][Azure 이벤트 허브]와 자주 비교합니다. IoT Hub와 마찬가지로 이벤트 허브는 짧은 대기 시간 및 높은 안정성으로 엄청난 규모의 클라우드에 이벤트 및 원격 분석을 가능하게 하는 이벤트 처리 서비스입니다.
...
| 장치 프로토콜 지원 |MQTT, AMQP, Websockets를 통한 AMQP 및 HTTP/1을 지원합니다. 또한 IoT Hub는 사용자 지정이 가능한 프로토콜 게이트웨이 구현인 [Azure IoT 프로토콜 게이트웨이][lnk-azure-protocol-gateway]에서 작동하여 사용자 지정 프로토콜을 지원합니다. |AMQP, Websockets를 통한 AMQP 및 HTTP/1을 지원합니다. |
| 보안 |장치 단위 ID 및 취소 가능한 액세스 제어를 제공합니다. [IoT Hub 개발자 가이드의 보안 섹션]을 참조하세요. |[게시자의 정책][Event Hubs 게시자 정책]을 통해 제한된 해지 지원을 포함하는 Event Hubs 전반의 [공유 액세스 정책][Event Hubs - 보안]을 제공합니다. IoT 솔루션은 종종 장치당 자격 증명 및 스푸핑 방지 측정을 지원하기 위해 사용자 지정 솔루션을 구현해야 합니다. |
...
| 장치 SDK |다양한 플랫폼 및 언어에 대한 [장치 SDK][Azure IoT Hub SDK]를 제공합니다. |.NET 및 C에서 지원되고 AMQP 및 HTTP 송신 인터페이스를 제공합니다. |
...
[Event Hub - 보안]: ../event-hubs/event-hubs-authentication-and-security-model-overview.md
[Event Hub 게시자 정책]: ../event-hubs/event-hubs-overview.md#common-publisher-tasks
[Azure Service Bus 할당량]: ../service-bus-messaging/service-bus-quotas.md
[Azure IoT Hub SDK]: https://github.com/Azure/azure-iot-sdks/blob/master/readme.md
