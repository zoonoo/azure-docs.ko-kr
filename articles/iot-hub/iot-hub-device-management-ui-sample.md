...
샘플 장치 관리 UI와 상호 작용하면 Azure IoT Hub 장치 관리 [개요][lnk-dm-overview]와 [시작하기][lnk-get-started] 문서에서 다룬 개념과 기능을 확고히 다지는 데 도움이 될 것입니다. 이 문서는 샘플 장치 관리 웹 UI에 나타난 바와 같이 각각의 세 가지 주요 장치 관리 개념(*장치 쌍*, *장치 쿼리*, *장치 작업*)을 둘러봅니다.
...
자습을 시작하기 전에 [Azure IoT Hub 장치 관리 시작하기][lnk-get-started] 문서에 있는 단계를 완료해야 합니다. 아직 완료하지 않았다면 진행하기 전에 그 문서로 돌아가서 모든 단계를 완료하세요.

“시작하기" 자습서를 완료하면 테스트 시스템에 다음이 실행되고 있을 것입니다.
...
[장치 관리 개요][lnk-dm-overview]에서는 Azure IoT Hub의 물리적(또는 시뮬레이션된) 장치를 나타내는 *장치 쌍* 개념을 소개했습니다. 장치 그리드의 장치 목록에서 등록된 장치를 선택하면 해당 장치의 장치 쌍을 확인 및 편집할 수 있습니다.

해당 장치 행을 선택하고 **편집** 단추를 클릭하여 첫 번째 시뮬레이션 장치 **Device11-7ce4a850**에 관한 상세 뷰를 입력합니다(행을 마우스로 두 번 클릭하거나 검색 상자에 장치 ID를 입력할 수도 있습니다).

이제 장치 쌍 구성 요소를 완전히 볼 수 있습니다. 여기서 쓰기 가능한 속성을 업데이트하고 아래와 같이 상세하게 다른 장치 작업을 실행할 수 있습니다.

![][2]

1.  **장치 쌍 편집**: 해당 장치의 **활성화/비활성화** 전환도 포함됩니다.

2.  **서비스 속성**: 장치 **태그**가 포함됩니다.
...
장치 관리 개요에서 배운 바와 같이 장치 작업을 사용하여 하나 이상의 물리적 장치에서 간단하거나 복잡한 작업을 오케스트레이션할 수 있습니다. 이 섹션에서는 샘플 UI에서 장치 작업을 만들고, “bacon” 태그가 달린 모든 시뮬레이션 장치에서 재부팅 작업을 수행할 것입니다.

1.  **Only Bacon** 장치 쿼리 목록에서 각 장치 행을 클릭하고 재부팅 작업으로 표시합니다.
...
잠시 후 화면이 새로 고침되면서 상위 작업과 3개의 하위 작업 상태가 **완료됨**으로 변경됩니다. 이는 시뮬레이션 장치에서 재부팅 작업이 성공하였고 확인이 완료되었음을 나타냅니다. **장치 ID** 열을 사용하여 어느 작업과 어느 장치가 연결되어 있는지 확인합니다.


> [AZURE.NOTE] 하위 작업이 “실패함" 상태를 반환하면 시뮬레이션 장치가 테스트 시스템에서 실행 중이 아닌지 확인합니다. 그렇지 않다면 simulate.bat 또는 simulate.sh 스크립트를 다시 실행하고 위의 장치 재부팅 작업 단계를 반복합니다.

## 다음 단계

이제 여러분은 안내에 따라 샘플 장치 관리 UI 경험을 사용하여 장치 관리 개념의 탐색을 완료하였습니다. 장치 관리 API와 코드 예제 실험에 대해 더욱 심도 있게 이해하고 싶다면 다음 개발자 자습서를 참조하세요.
