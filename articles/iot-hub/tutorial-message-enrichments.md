...
* 방화벽에서 포트 8883가 열려 있는지 확인 합니다. 이 자습서의 장치 샘플에서는 포트 8883을 통해 통신 하는 MQTT 프로토콜을 사용 합니다. 이 포트는 일부 회사 및 교육용 네트워크 환경에서 차단 될 수 있습니다. 이 문제를 해결 하는 방법 및 방법에 대 한 자세한 내용은 [IoT Hub에 연결 (MQTT)](iot-hub-mqtt-support.md#connecting-to-iot-hub)을 참조 하세요.
...
다음 스크립트를 사용 하거나 다운로드 한 리포지토리의/resources 폴더에 있는 스크립트를 열 수 있습니다. 스크립트는 다음 단계를 수행 합니다.
...
솔루션 파일 **IoT_SimulatedDevice .sln** 을 두 번 클릭 하 여 Visual Studio에서 코드를 열고 **Program.cs**을 엽니다. 마커 `{your hub name}`의 IoT hub 이름을 대체 합니다. IoT hub 호스트 이름의 형식은 **{your hub name}. azure-devices.net**입니다. 이 자습서에서는 허브 호스트 이름이 ContosoTestHubMsgEn.azure-devices.net입니다. 그런 다음, 스크립트를 실행 하 여 `{your device key}`표식을 위한 리소스를 만들 때 이전에 저장 한 장치 키로 대체 합니다.
...
앱은 1초마다 새로운 디바이스-클라우드 메시지를 IoT Hub에 보냅니다. 메시지에는 디바이스 ID, 온도, 습도 및 메시지 수준(기본값이 `normal`)과 함께 JSON 직렬화된 개체가 포함됩니다. `critical` 또는 `storage`의 수준을 임의로 할당 하므로 메시지는 저장소 계정 또는 기본 끝점으로 라우팅됩니다. 저장소 계정의 **보강** 컨테이너로 전송 되는 메시지는 보강 됩니다.
...
리소스 그룹을 제거하려면 [az group delete](https://docs.microsoft.com/cli/azure/group?view=azure-cli-latest#az-group-delete) 명령을 사용합니다. 이 자습서의 시작 부분에서 `$resourceGroup`가 **ContosoResourcesMsgEn** 로 설정 되어 있음을 기억 하세요.
