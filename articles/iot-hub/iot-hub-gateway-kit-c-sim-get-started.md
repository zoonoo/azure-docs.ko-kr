...
이 단원에서는 스크립트를 사용하여 게이트웨이에서 시뮬레이트된 장치 앱의 구성 및 실행을 자동화합니다. 시뮬레이트된 장치 앱은 샘플 온도 데이터를 생성하여 IoT Hub 모듈에 보냅니다. IoT Hub 모듈은 받은 데이터를 패키지하고 Azure IoT Gateway SDK에서 제공된 게이트웨이 프레임워크를 통해 IoT Hub로 보냅니다.
