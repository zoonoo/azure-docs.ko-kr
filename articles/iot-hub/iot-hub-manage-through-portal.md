...
**무료**, **표준 1** 및 **표준 2**의 3개 계층 중에서 선택할 수 있습니다. 무료 계층에서는 IoT Hub에 500개 장치만 연결할 수 있으며 하루에 8,000개 메시지까지 허용합니다.

**S1(낮은 빈도)**: IoT Hubs S1(낮은 빈도) edition은 장치당 상대적으로 적은 양의 데이터를 생성하는 장치 수가 많은 IoT 솔루션을 위해 설계되었습니다. S1(낮은 빈도) 버전의 각 단위를 통해 연결된 모든 장치에서 하루에 최대 400,000개의 메시지를 전송할 수 있습니다.

**S2(높은 빈도)**: IoT Hub S2(높은 빈도) edition은 장치가 대량의 데이터를 생성하는 IoT 솔루션을 위해 설계되었습니다. S2(높은 빈도) 버전의 각 단위를 통해 연결된 모든 장치 간에 하루에 최대 600만 개의 메시지를 전송할 수 있습니다.
...
상위 계층(S2)에서 하위 계층(S1)으로 이동하는 것은 해당 날짜에 전송된 메시지 수가 충돌하지 않는 경우에만 허용됩니다. 예를 들어 하루당 메시지 수가 400,000개를 초과하면 IoT Hub에 대한 계층을 S2에서 S1로 변경할 수 없습니다.
