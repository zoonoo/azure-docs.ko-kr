...
2. 텍스트 편집기에서 작업 디렉토리에 **dmpatterns_getstarted_device.py라는** 파일을 만듭니다.
...
4. **CONNECTION_STRING** 변수를 추가합니다. `{deviceConnectionString}` 자리 표시자 값을 장치 연결 문자열로 바꿉습니다. IoT [허브에서 새 장치 등록에서](#register-a-new-device-in-the-iot-hub)이전에 이 연결 문자열을 복사했습니다.  
...
2. 텍스트 편집기에서 작업 디렉토리에 **dmpatterns_getstarted_service.py라는** 파일을 만듭니다.
...
4. 다음 변수 선언을 추가합니다. `{IoTHubConnectionString}` 자리 표시자 값을 IoT 허브 연결 문자열 에서 이전에 복사한 [IoT 허브 연결 문자열로](#get-the-iot-hub-connection-string)바꿉꿉입니다. 자리 `{deviceId}` 표시자 값을 [IoT 허브에서 새 장치 등록에 등록한](#register-a-new-device-in-the-iot-hub)장치 ID로 바꿉습니다.
...
3. 콘솔에서 직접 메서드에 대한 디바이스 응답을 확인합니다.
   ...
   다음은 재부팅 직접 메서드에 대한 장치 응답을 보여 주며 있습니다.

   ![시뮬레이션된 장치 앱 출력](./media/iot-hub-python-python-device-management-get-started/device.png)

   다음은 재부팅 직접 메서드를 호출하고 상태 대장치 쌍폴링 서비스를 보여 주며 다음과 같은 것입니다.

   ![트리거 재부팅 서비스 출력](./media/iot-hub-python-python-device-management-get-started/service.png)
