...
1. **simDevice.py** 파일의 시작 부분에 다음 `import` 문 및 변수를 추가합니다. `deviceConnectionString`을 위에서 만든 디바이스의 연결 문자열로 바꿉니다.
...
1. 디바이스 쌍 업데이트를 처리하는 다른 함수 콜백을 추가합니다.
...
1. 디바이스를 조회하는 데 사용되는 다음 함수를 추가합니다.
...
1. 직접 메서드와 디바이스 쌍을 호출하는 작업을 실행하는 다음 메서드를 추가합니다.
...
1. 콘솔에서 직접 메서드와 디바이스 쌍 업데이트에 대한 디바이스 응답을 확인합니다.
