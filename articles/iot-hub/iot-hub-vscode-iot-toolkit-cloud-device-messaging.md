...
장치-클라우드 메시지를 모니터링하고 클라우드-장치 메시지를 보내려면 Visual Studio Code용 Azure IoT Toolkit 확장을 사용하는 방법을 알아봅니다. 디바이스-클라우드 메시지는 디바이스에서 수집한 다음 IoT Hub로 보내는 센서 데이터일 수 있습니다. 클라우드-디바이스 메시지는 IoT Hub에서 디바이스로 보내 사용자 디바이스에 연결된 LED를 깜박이는 명령일 수 있습니다.
