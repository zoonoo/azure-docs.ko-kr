...
먼저 Edison에서 BlueZ 소프트웨어의 버전을 업데이트해야 합니다. 버전 5.37이 이미 설치되어 있으면 다음 단계를 완료하여 설치가 완료되었는지 확인해야 합니다.

1. 현재 실행 중인 Bluetooth 디먼을 중지합니다.
    
    ```
    systemctl stop bluetooth
    ```

2. BlueZ 버전 5.37용 [소스 코드](http://www.kernel.org/pub/linux/bluetooth/bluez-5.37.tar.xz)를 다운로드하고 압축을 풉니다.
...
4. **/lib/systemd/system/bluetooth.service** 파일을 편집하여 새 Bluetooth 디먼을 가리키도록 Bluetooth에 대한 *systemd* 서비스 구성을 변경합니다. **ExecStart** 특성 값을 다음과 같이 바꿉니다.
...
다음에는 Edison에서 SensorTag 장치에 연결할 수 있는지 확인해야 합니다.
...
2. **bluetoothctl** 명령을 실행합니다. 다음과 유사한 결과가 표시됩니다.
    
    ```
    [NEW] Controller 98:4F:EE:04:1F:DF edison [default]
    ```

3. 현재 대화형 Bluetooth 셸에서 작업하고 있습니다. 명령 **scan on**을 입력하여 Bluetooth 장치를 검색합니다. 다음과 유사한 결과가 표시됩니다.
...
4. 작은 단추(녹색 표시등이 깜박거림)를 눌러 SensorTag 장치를 검색 가능하게 합니다. Edison은 SensorTag 장치를 검색해야 합니다.
...
7. 이제 **disconnect** 명령을 사용하여 장치에서 연결을 끊은 다음 **quit** 명령을 사용하여 Bluetooth 셸을 끝낼 수 있습니다.
