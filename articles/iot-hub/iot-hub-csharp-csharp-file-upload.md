...
5. **Program** 클래스에 다음 필드를 추가합니다. 자리 표시자 값을 [IoT Hub로 시작]에서 만든 IoT Hub 연결 문자열로 대체합니다.
