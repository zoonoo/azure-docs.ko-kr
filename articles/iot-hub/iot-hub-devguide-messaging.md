...
IoT Hub는 다음 속성을 노출하여 **messages/events** 기본 제공 메시징 끝점을 제어할 수 있게 합니다.

* **분할 개수**. 이 속성은 생성 시 설정하여 장치-클라우드 이벤트 수집에 대한 파티션 수를 정의합니다.
* **보존 시간**. 이 속성은 장치-클라우드 메시지에 대한 보존 시간을 지정합니다. 기본값은&1;일이지만&7;일로 늘릴 수 있습니다.
...
IoT Hub는 허브에서 수신한 장치-클라우드 메시지를 읽도록 백 엔드 서비스의 기본 제공 끝점인 **messages/events**를 공개합니다. 이 끝점은 Event Hubs와 호환되므로 Event Hubs 서비스에서 메시지 읽기를 지원하는 모든 메커니즘을 사용할 수 있습니다.
