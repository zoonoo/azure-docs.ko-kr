...
일반적으로 메시지 손실이 응용 프로그램 논리에 영향을 주지 않으면 디바이스가 클라우드-디바이스 메시지를 완료합니다. 예를 들어 디바이스가 메시지 콘텐츠를 로컬에 유지하거나 작업을 성공적으로 실행한 경우입니다. 또한 메시지가 임시 정보를 전달하고 있으므로 손실되더라도 응용 프로그램의 기능에 영향을 주지 않을 수도 있습니다. 경우에 따라 장기 실행 작업에 다음을 수행할 수 있습니다.
