...
**Event Grid와 IoT Hub 통합**: Azure Event Grid는 게시-구독 모델을 사용하는 완전 관리형 이벤트 라우팅 서비스입니다. IoT Hub 및 Event Grid는 거의 실시간으로 [IoT Hub 이벤트를 Azure 및 외부 Azure 서비스에 통합](iot-hub-event-grid.md)하기 위해 함께 작동합니다. IoT Hub은 일반적으로 사용할 수 있는 [장치 이벤트](iot-hub-event-grid.md#event-types)를 게시 하 고, 현재 공개 미리 보기로 제공 되는 원격 분석 이벤트도 게시 합니다.
...
| ------- | --------------- | ---------- |
| **장치 메시지 및 이벤트** | 예, 메시지 라우팅은 원격 분석 데이터, 보고서 장치 쌍 변경, 장치 수명 주기 이벤트 및 디지털 쌍 변경 이벤트 ( [IoT 플러그 앤 플레이 공개 미리 보기](../iot-pnp/overview-iot-plug-and-play.md)의 일부)에 사용할 수 있습니다. | 예, Event Grid 원격 분석 데이터에 사용할 수 있지만 장치를 만들고, 삭제 하 고, 연결 하 고, 연결을 해제 하는 경우에도 보고할 수 있습니다 IoT Hub |
...
| **신뢰성** | 높음: 각 경로에 대해 한 번 이상 끝점에 각 메시지를 배달 합니다. 한 시간 내에 배달되지 않는 모든 메시지에 만료됩니다. | 높음: 각 구독에 대해 한 번 이상 webhook에 각 메시지를 배달 합니다. 24시간 이내에 배달되지 않는 모든 이벤트 만료됩니다. | 
| **확장성** | 높음: 수백만 개의 메시지를 전송 하는 동시에 연결 된 수백만 대의 장치를 지원 하도록 최적화 되었습니다. | 높음: 지역별로 1000만 이벤트를 초당 라우팅할 수 있습니다. |
| **대기 시간** | 낮음: 거의 실시간입니다. | 낮음: 거의 실시간입니다. |
