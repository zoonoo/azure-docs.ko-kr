...
장치가 큐에서 클라우드-장치 메시지를 받을 때 (예: [`ReceiveAsync()`](https://docs.microsoft.com/dotnet/api/microsoft.azure.devices.client.deviceclient.receiveasync?view=azure-dotnet)사용) 잠금 제한 시간 (1 분) 동안 IoT Hub에 의해 메시지가 잠깁니다. 잠금 시간 제한이 만료 된 후 장치에서 메시지를 완료 하려고 하면 IoT Hub이 예외를 throw 합니다.
