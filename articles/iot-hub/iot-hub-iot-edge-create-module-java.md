...
모듈은 `com.microsoft.azure.gateway.messaging.Message` 클래스를 사용하여 서로 통신합니다. `Message`의 **콘텐츠**는 사용자가 선호하는 모든 종류의 데이터를 표시할 수 있는 바이트 배열입니다. **속성**은 `Message`에서 사용할 수 있으며 단순히 문자열-문자열 매핑입니다. **속성**을 HTTPS 요청의 헤더 또는 파일의 메타데이터로 생각할 수도 있습니다.
