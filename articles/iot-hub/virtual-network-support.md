...
> [!NOTE]
> 이 문서에서 설명 하는 IoT Hub 기능은 현재 미국 동부, 미국 서 부 및 미국 서 부 2 지역에서 [관리 서비스 id를 사용 하 여 만든](#create-an-iot-hub-with-managed-service-identity) IoT hub에서 사용할 수 있습니다.
...
![IoT Hub 공용 끝점](./media/virtual-network-support/public-endpoint.png)
...
* IoT hub [는 관리 서비스 id](#create-an-iot-hub-with-managed-service-identity)로 프로 비전 되어야 합니다.
...
2. IoT Hub 포털에서 **개인 끝점 연결** 탭으로 이동 합니다 (이 탭은 [지원 되는 지역](#regional-availability-private-endpoints)에서 IoT hub에만 사용할 수 있음). 그리고 **+** 기호를 클릭 하 여 새 개인 끝점을 추가 합니다.
...
* IoT Hub에는 허브 프로 비전 시간에 관리 서비스 id를 할당 해야 합니다. [관리 서비스 id를 사용 하 여 허브를 만드는](#create-an-iot-hub-with-managed-service-identity)방법에 대 한 지침을 따르세요.
...
### <a name="create-an-iot-hub-with-managed-service-identity"></a>관리 서비스 id를 사용 하 여 IoT hub 만들기

관리 서비스 id는 리소스 프로 비전 시간에 허브에 할당할 수 있습니다 .이 기능은 현재 기존 허브에 대해 지원 되지 않습니다 .이 경우 IoT hub는 최소 버전으로 TLS 1.2을 사용 해야 합니다. 이러한 목적을 위해 아래 ARM 리소스 템플릿을 사용 해야 합니다.
...
리소스 `name`, `location` `SKU.name` 및 `SKU.tier`에 대 한 값을 대체 한 후 Azure CLI를 사용 하 여 다음을 사용 하 여 기존 리소스 그룹에 리소스를 배포할 수 있습니다.
...
고객 소유의 저장소 계정으로 메시지를 라우팅하도록 IoT Hub를 구성할 수 있습니다. 방화벽 제한이 적용 되는 동안 라우팅 기능에서 저장소 계정에 액세스 하도록 허용 하려면 IoT Hub 관리 서비스 id가 있어야 합니다 ( [관리 서비스 id로 허브를 만드는](#create-an-iot-hub-with-managed-service-identity)방법 참조). 관리 서비스 id가 프로 비전 되 면 아래 단계에 따라 저장소 계정에 액세스 하기 위해 허브의 리소스 id에 RBAC 권한을 부여 합니다.
...
고객 소유 event hubs 네임 스페이스로 메시지를 라우팅하도록 IoT Hub를 구성할 수 있습니다. 방화벽 제한이 적용 되는 동안 라우팅 기능에서 event hubs 리소스에 액세스할 수 있도록 하려면 IoT Hub 관리 서비스 id가 있어야 합니다 ( [관리 서비스 id로 허브를 만드는](#create-an-iot-hub-with-managed-service-identity)방법 참조). 관리 서비스 id가 프로 비전 되 면 아래 단계에 따라 허브의 리소스 id에 대 한 RBAC 권한을 제공 하 여 event hubs에 액세스 합니다.
...
고객 소유의 service bus 네임 스페이스로 메시지를 라우팅하도록 IoT Hub를 구성할 수 있습니다. 방화벽 제한이 적용 되는 동안 라우팅 기능에서 service bus 리소스에 액세스할 수 있도록 하려면 IoT Hub 관리 서비스 id가 있어야 합니다 ( [관리 서비스 id로 허브를 만드는](#create-an-iot-hub-with-managed-service-identity)방법 참조). 관리 서비스 id가 프로 비전 되 면 아래 단계에 따라 service bus에 액세스 하기 위해 허브의 리소스 id에 RBAC 권한을 부여 합니다.
...
이 기능을 사용 하려면 IoT Hub에서 저장소 계정으로 연결 해야 합니다. 방화벽 제한이 적용 되는 동안 service bus 리소스에 액세스 하려면 IoT Hub 관리 서비스 id가 있어야 합니다 ( [관리 서비스 id를 사용 하 여 허브를 만드는](#create-an-iot-hub-with-managed-service-identity)방법 참조). 관리 서비스 id가 프로 비전 되 면 아래 단계에 따라 service bus에 액세스 하기 위해 허브의 리소스 id에 RBAC 권한을 부여 합니다.
...
이제 대량 가져오기/내보내기 기능을 사용 하는 방법에 대 한 정보에 대 한 [가져오기 내보내기 작업을 만들기](https://docs.microsoft.com/rest/api/iothub/service/jobclient/getimportexportjobs) 위한 Azure IoT REST API를 사용할 수 있습니다. 요청 본문 `storageAuthenticationType="identityBased"` 에를 제공 하 고 및 `inputBlobContainerUri="https://..."` `outputBlobContainerUri="https://..."` 을 각각 저장소 계정의 입력 및 출력 URL로 사용 해야 합니다.


Azure IoT Hub SDK는 또한 서비스 클라이언트의 레지스트리 관리자에서이 기능을 지원 합니다. 다음 코드 조각에서는 c # SDK를 사용 하 여에서 가져오기 작업 또는 내보내기 작업을 시작 하는 방법을 보여 줍니다.
...
이 지역에서 제한 된 버전의 Azure IoT Sdk를 사용 하려면 c #, Java 및 node.js에 대해 가상 네트워크를 지원 합니다.

1. 이라는 `EnableStorageIdentity` 환경 변수를 만들고 해당 값을로 `1`설정 합니다.

2. SDK 다운로드: [Java](https://aka.ms/vnetjavasdk) | [c #](https://aka.ms/vnetcsharpsdk) | [node.js](https://aka.ms/vnetnodesdk)
...
2. 다음 파일을 다운로드 합니다 .이 파일은 릴리스 페이지의 아래쪽에 있는 **자산**이라는 헤더 아래에 있습니다.
    > *azure_iot_hub-2.2.0_limited-py2. py3-none-any.*

3. 터미널을 열고 다운로드 한 파일이 있는 폴더로 이동 합니다.

4. 다음 명령을 실행 하 여 가상 네트워크에 대 한 지원을 포함 하는 Python Service SDK를 설치 합니다.
    > pip install./azure_iot_hub-2.2.0_limited-py2. py3-none-any.
