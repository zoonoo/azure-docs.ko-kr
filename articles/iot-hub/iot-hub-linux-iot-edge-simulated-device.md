...
**build.sh** 스크립트는 **iot-edge** 리포지토리의 로컬 복사본에 있는 **build** 폴더에 해당 출력을 생성합니다. 이 출력에는 이 샘플에서 사용된 네 개의 IoT Edge 모듈이 포함됩니다.

빌드 스크립트는 다음을 배치합니다.

* **build/modules/logger** 폴더에 **liblogger.so**.
* **build/modules/iothub** 폴더에 **libiothub.so**.
* **build/modules/identitymap** 폴더에 **lib\_identity\_map.so**.
* **build/modules/simulated\_device** 폴더에 **libsimulated\_device.so**.

다음 JSON 설정 파일에 표시된 대로 **module path** 값에 이러한 경로를 사용합니다.

simulated\_device\_cloud\_upload\_sample 프로세스는 JSON 구성 파일에 대한 경로를 명령줄 인수 형태로 사용합니다. 다음 예제 JSON 파일은 **samples\\simulated\_device\_cloud\_upload\_sample\\src\\simulated\_device\_cloud\_upload\_sample\_lin.json**의 SDK 리포지토리에서 제공됩니다. 이 구성 파일은 빌드 스크립트를 수정하여 IoT Edge 모듈이나 샘플 실행 파일을 기본 위치가 아닌 위치에 배치한 경우 외에는 그대로 작동합니다.

> [!NOTE]
> 모듈 경로는 실행 파일이 있는 디렉터리가 아니라, simulated\_device\_cloud\_upload\_sample 실행 파일이 실행되는 위치의 디렉터리에 대한 상대 경로입니다. 샘플 JSON 구성 파일은 현재 작업 디렉터리에서 작성 중인 ‘deviceCloudUploadGatewaylog.log’를 기본값으로 사용합니다.

텍스트 편집기에서 **iot-edge** 리포지토리의 로컬 복사본에 있는 **samples/simulated\_device\_cloud\_upload\_sample/src/simulated\_device\_cloud\_upload\_lin.json** 파일을 엽니다. 이 파일은 샘플 게이트웨이에서 IoT Edge 모듈을 구성합니다.
...
* **BLE1** 및 **BLE2** 모듈은 시뮬레이션된 장치입니다. 해당 MAC 주소가 **mapping** 모듈의 주소와 어떻게 일치하는지 확인합니다.
* **Logger** 모듈은 게이트웨이 활동을 파일에 로깅합니다.
* 예제에 나온 **module path** 값은 **iot-edge** 리포지토리의 로컬 복사본에 있는 **build** 폴더에서 샘플을 실행하는 것으로 가정합니다.
...
Azure IoT Edge와 코드 예제 실험에 대해 더욱 심도 있게 이해하려면 다음 개발자 자습서 및 리소스를 참고하세요.
