...
메시지 라우팅을 사용하면 메시지 속성 및 메시지 본문 뿐만 아니라 장치 쌍 태그 및 장치 쌍 속성에 대해서도 쿼리할 수 있습니다. 메시지 본문이 JSON이 아닌 경우 메시지 라우팅에서 메시지를 계속 라우팅할 수 있지만 메시지 본문에 쿼리를 적용할 수는 없습니다.  쿼리는 부울 식으로 기술되는데, 부울 true는 쿼리가 모든 들어오는 데이터의 라우팅에 성공한 것이고, 부울 false는 쿼리에 실패하여 라우팅된 데이터가 없는 것입니다. 식이 null 또는 정의되지 않은 것으로 평가되면 false로 처리되고 실패한 경우 진단 로그에 오류가 생성됩니다. 저장하고 평가할 경로에 대해 올바른 쿼리 구문을 사용해야 합니다.  
