...
* 여러 장치에서 **lockDoor** 직접 메서드를 호출하는 작업을 만드는 백 엔드 앱을 만듭니다. 다른 작업이 여러 디바이스로 원하는 속성 업데이트를 보냅니다.
...
1. 개발 컴퓨터에서 **iot-java-schedule-jobs라는**빈 폴더를 만듭니다.

2. **iot-java-schedule-jobs** 폴더에서 명령 프롬프트에서 다음 명령을 사용하여 **일정 작업이라는** Maven 프로젝트를 만듭니다. 긴 단일 명령입니다.

   ```cmd/sh
   mvn archetype:generate -DgroupId=com.mycompany.app -DartifactId=schedule-jobs -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false
   ```

3. 명령 프롬프트에서 **일정 작업** 폴더로 이동합니다.

4. 텍스트 편집기를 사용하여 **일정 작업** 폴더에서 **pom.xml** 파일을 열고 **종속성** 노드에 다음 종속성을 추가합니다. 이러한 종속성을 사용하면 앱에서 **iot 서비스-클라이언트** 패키지를 사용하여 IoT 허브와 통신할 수 있습니다.
...
7. 텍스트 편집기에서 **일정-작업\src\main\java\com\mycompany\App.java** 파일을 엽니다.
...
9. **App** 클래스에 다음 클래스 수준 변수를 추가합니다. IoT 허브 연결 문자열 Get에서 이전에 복사한 `{youriothubconnectionstring}` [IoT 허브 연결 문자열로](#get-the-iot-hub-connection-string)바꾸기:
...
15. 두 작업을 순차적으로 실행하고 모니터링하려면 **기본** 메서드의 코드를 다음 코드로 바꿉니다.
...
16. **예약 작업\src\main\java\com\mycompany\App\App.java** 파일을 저장하고 닫습니다.

17. **schedule-jobs** 앱을 빌드하고 오류를 수정합니다. 명령 프롬프트에서 **일정 작업** 폴더로 이동하여 다음 명령을 실행합니다.
...
1. **iot-java-schedule-jobs** 폴더에서 명령 프롬프트에서 다음 명령을 사용하여 **시뮬레이션 장치라는** Maven 프로젝트를 만듭니다. 긴 단일 명령입니다.
...
9. **App** 클래스에 다음 클래스 수준 변수를 추가합니다. IoT 허브 섹션의 새 장치 [등록섹션에서](#register-a-new-device-in-the-iot-hub) 이전에 복사한 장치 연결 문자열로 바꾸기: `{yourdeviceconnectionstring}`
...
17. 시뮬레이션 된 **장치 \src\main\java\com\mycompany\App\App.java** 파일을 저장하고 닫습니다.

18. **simulated-device** 앱을 빌드하고 오류를 수정합니다. 명령 프롬프트에서 **시뮬레이션된 장치** 폴더로 이동하여 다음 명령을 실행합니다.
...
1. **시뮬레이션 된 장치** 폴더의 명령 프롬프트에서 다음 명령을 실행하여 원하는 속성 변경 및 직접 메서드 호출을 수신 대기하는 장치 앱을 시작합니다.
