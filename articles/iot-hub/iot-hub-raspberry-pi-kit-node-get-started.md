---
title: 클라우드에서 Azure IoT 허브에 라즈베리 파이 연결 (Node.js)
description: 이 자습서에서는 라즈베리 파이를 Azure IoT 허브에 설정하고 연결하여 이 자습서에서 Azure 클라우드 플랫폼으로 데이터를 보내는 방법을 알아봅니다.
author: wesmc7777
manager: eliotgra
...
1. Raspbian을 다운로드합니다.
   ...
   a. 바탕 화면 (.zip [파일)와 Raspbian 버스터.](https://www.raspberrypi.org/downloads/raspbian/)

   b. 컴퓨터의 폴더에 Raspbian 이미지의 압축을 풉니다.

2. microSD 카드에 Raspbian을 설치합니다.

   a. [다운로드 및 에칭 SD 카드 버너 유틸리티를 설치합니다.](https://etcher.io/)
...
2. Pi를 시작한 다음 사용자 이름과 `pi` 암호로 사용하여 `raspberry` Raspbian에 로그인합니다.

3. 환경 설정**라즈베리 파이 구성을**> 라즈베리 **아이콘을** > 클릭 합니다.
...
[라즈베리 파이](/windows/iot-core/learn-about-hardware/pinmappings/pinmappingsrpi) 를 보려면 클릭 2 & 3 참조에 대 한 핀 매핑.
...
2. Node.js 및 NPM을 Pi에 설치합니다.
   ...
   버전이 10.x보다 낮거나 Pi에 Node.js가 없는 경우 최신 버전을 설치합니다.
...
1. 다음 명령을 실행하여 config 파일을 엽니다.
   ...
   *참고: 이 자습서에서 사용되는 i2c 주소는 기본적으로 0x77입니다. 구성에 따라 0x76일 수도 있습니다: i2c 오류가 발생하면 값을 118로 변경하고 더 잘 작동하는지 확인하십시오. 센서에서 사용하는 주소를 보려면 라즈베리 파이의 껍질에서 실행하십시오. `sudo i2cdetect -y 1`*
...
센서 데이터를 수집하고 IoT 허브로 전송하기 위해 샘플 응용 프로그램을 실행했습니다.
