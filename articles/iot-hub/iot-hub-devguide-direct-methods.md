...
요청에서로 `responseTimeoutInSeconds` 제공 되는 값은 IoT Hub 서비스가 장치에서 직접 메서드 실행이 완료 될 때까지 기다려야 하는 시간입니다. 이 시간 제한을 장치에서 직접 메서드를 실행 하는 데 예상 되는 시간 이상으로 설정 합니다. Timeout을 지정 하지 않으면 기본값인 30 초가 사용 됩니다. 의 `responseTimeoutInSeconds` 최소값 및 최대값은 각각 5 및 300 초입니다.
...
* *HTTP 상태 코드*:
  ...
  * 504는 장치가 내에서 `responseTimeoutInSeconds`직접 메서드 호출에 응답 하지 않아 발생 한 게이트웨이 시간 초과를 나타냅니다.
