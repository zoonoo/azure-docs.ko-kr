...
각 장치 메서드는 단일 장치를 대상으로 합니다. [여러 장치에서 작업 예약](iot-hub-devguide-jobs.md)에서는 여러 장치에서 직접 메서드를 호출하고 연결되지 않은 장치에 대한 메서드 호출을 예약하는 방법을 보여줍니다.
...
장치의 직접 메서드 호출은 다음 항목으로 구성된 HTTPS 호출입니다.
...
IoT 장치에서 직접 메서드를 처리하는 방법을 살펴보겠습니다.
...
장치는 MQTT 토픽으로 직접 메서드 요청을 수신합니다. `$iothub/methods/POST/{method name}/?$rid={request id}` 장치당 구독 수가 5로 제한됩니다. 따라서 각 직접 메서드를 개별적으로 구독하지 않는 것이 좋습니다. 대신 `$iothub/methods/POST/#`을 구독한 다음, 원하는 메서드 이름을 기반으로 배달된 메시지를 필터링하는 것이 좋습니다.
...
장치는 `amqps://{hostname}:5671/devices/{deviceId}/methods/deviceBound` 주소에서 수신 링크를 만들어 직접 메서드 요청을 수신합니다.
...
장치는 `amqps://{hostname}:5671/devices/{deviceId}/methods/deviceBound` 주소에서 메서드 응답을 반환하기 위한 전송 링크를 만듭니다.
