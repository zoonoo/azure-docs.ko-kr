...
* **읽기-파일-업로드 알림-** IoT hub에서 파일 업로드 알림을 수신 합니다.
...
7. 다음 클래스 수준 변수를 **App** 클래스에 추가 합니다. 자리 표시자 `{Your IoT Hub connection string}` 값을 이전에 [iot Hub 연결 문자열 가져오기](#get-the-iot-hub-connection-string)에서 복사한 iot hub 연결 문자열로 바꿉니다.
