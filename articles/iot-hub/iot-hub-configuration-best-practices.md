...
* **단계별 배포를 사용하여 구성 변경 내용 수행:** 단계별 배포는 운영자가 대규모 IoT 디바이스 집합에 변경 내용을 배포하는 전체적인 프로세스입니다. 목표는 점진적으로 변경하여 광범위한 새로운 변경에 따른 위험을 줄이는 것입니다.  운영자는 솔루션의 인터페이스를 사용 하 여 [자동 장치 구성을](iot-hub-auto-device-config.md) 만들어야 하며 대상 조건은 초기 장치 집합 (예: 카나리아 그룹)을 대상으로 해야 합니다. 그런 다음, 운영자는 디바이스의 초기 집합에서 구성 변경 내용의 유효성을 검사해야 합니다.
