---
title: IoT 허브 연결 관리 & 장치 SDK를 통한 신뢰할 수 있는 메시징
...
* [파이썬 SDK](https://github.com/Azure/azure-iot-sdk-python) (신뢰성은 아직 구현되지 않음)

## <a name="designing-for-resiliency"></a>복원력을 위한 디자인

IoT 디바이스는 주로 불연속 네트워크 연결 또는 불안정한 네트워크 연결(예: GSM 또는 위성)을 사용합니다. 일시적인 서비스 가용성 및 인프라 수준 또는 일시적 오류 때문에 디바이스가 클라우드 기반 서비스와 상호 작용할 때 오류가 발생할 수 있습니다. 장치에서 실행되는 응용 프로그램은 연결, 재연결 및 메시지 송수신을 위한 재시도 논리에 대한 메커니즘을 관리해야 합니다. 또한 다시 시도 전략 요구 사항은 디바이스의 IoT 시나리오, 컨텍스트, 기능에 따라 크게 달라집니다.
...
이 섹션에서는 연결을 관리할 때 사용할 수 있는 재연결 및 재시도 패턴에 대한 개요를 제공합니다. 또한 디바이스 애플리케이션에서 서로 다른 재시도 정책을 사용하기 위한 구현 지침에 대해 자세히 설명하고 디바이스 SDK에서 관련 API를 나열합니다.
...
* **사용자 지정 재시도**: 일부 SDK 언어의 경우 시나리오에 더 적합한 사용자 지정 재시도 정책을 설계한 다음, RetryPolicy에 삽입할 수 있습니다. 사용자 지정 재시도는 C SDK에서 사용할 수 없으며 현재 Python SDK에서 지원되지 않습니다. 파이썬 SDK는 필요에 따라 다시 연결됩니다.
