...
* **레지스트리 읽기** 및 **레지스트리 쓰기** 정책은 ID 레지스트리에 대한 읽기 및 쓰기 액세스 권한을 부여합니다. 이러한 권한은 백 엔드 클라우드 서비스에서 장치 ID를 관리하는 데 사용됩니다. 쓰기 옵션을 선택하면 읽기 옵션도 자동으로 선택됩니다.

* **서비스 연결** 정책은 서비스 끝점에 액세스할 수 있는 권한을 부여합니다. 이 권한은 백 엔드 클라우드 서비스에서 장치에서 메시지를 보내고 받을 뿐만 아니라 장치 쌍 및 모듈 쌍 데이터를 업데이트하고 읽는 데 사용됩니다.

* **디바이스 연결** 정책은 IoT Hub 디바이스 쪽 엔드포인트를 사용하여 메시지를 주고받기 위한 권한을 부여합니다. 이 권한은 장치에서 IoT 허브에서 메시지를 보내고 받고, 장치 쌍 및 모듈 쌍 데이터를 업데이트 및 읽고, 파일 업로드를 수행하는 데 사용됩니다.

**만들기** 를 클릭하여 새로 만들어진 이 정책을 기존 목록에 추가합니다.

특정 사용 권한에 의해 부여된 액세스에 대한 자세한 내용은 [IoT Hub 사용 권한을](./iot-hub-devguide-security.md#iot-hub-permissions)참조하십시오.
...
사용자 지정 끝점 탭을 **클릭합니다.** 이미 만든 사용자 지정 끝점이 표시됩니다. 여기서는 새 엔드포인트를 추가하거나 기존 엔드포인트를 삭제할 수 있습니다. 
