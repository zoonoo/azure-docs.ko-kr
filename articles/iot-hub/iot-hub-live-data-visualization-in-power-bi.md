---
title: 데이터 frm Azure IoT Hub의 실시간 데이터 시각화 – Power BI
...
Azure IoT Hub에서 Power BI를 통해 받는 실시간 센서 데이터를 시각화하는 방법에 대해 알아봅니다. 웹 앱을 사용하여 IoT 허브의 데이터를 시각화하려면 웹 앱 [사용을 참조하여 Azure IoT Hub에서 실시간 센서 데이터를 시각화합니다.](iot-hub-live-data-visualization-in-web-apps.md)
...
* [라즈베리 파이 온라인 시뮬레이터](iot-hub-raspberry-pi-web-simulator-get-started.md) 자습서 또는 장치 자습서 중 하나를 완료; 예를 들어, [node.js와 라즈베리 파이](iot-hub-raspberry-pi-kit-node-get-started.md). 이 문서에서는 다음 요구 사항을 다룹니다.
  
  * 활성화된 Azure 구독.
  * 구독 중인 Azure IoT Hub
  * 메시지를 Azure IoT Hub로 보내는 클라이언트 애플리케이션

* Power BI 계정 (무료로[파워 BI를 사용해보십시오)](https://powerbi.microsoft.com/)
...
3. **입력** 창에서 **스트림 입력 추가를**선택한 다음 드롭다운 목록에서 **IoT Hub를** 선택합니다. 새 입력 창에서 다음 정보를 입력합니다.

   **입력 별칭**: 입력에 대한 고유한 별칭을 입력합니다.

   **구독에서 IoT Hub 제공**: 이 라디오 버튼을 선택합니다.

   **구독**: 이 자습서에 사용 하는 Azure 구독을 선택 합니다.

   **IoT Hub**: 이 자습서에 사용 하는 IoT 허브를 선택 합니다.

   **엔드포인트**: **메시징**을 선택합니다.

   **공유 액세스 정책 이름**: Stream Analytics 작업이 IoT 허브에 사용할 공유 액세스 정책의 이름을 선택합니다. 이 자습서에서는 *서비스를*선택할 수 있습니다. *서비스* 정책은 기본적으로 새 IoT 허브에서 만들어지며 IoT 허브에서 노출된 클라우드 측 엔드포인트에서 보내고 받을 수 있는 권한을 부여합니다. 자세한 내용은 [액세스 제어 및 사용 권한을](iot-hub-devguide-security.md#access-control-and-permissions)참조하십시오.

   **공유 액세스 정책 키**: 이 필드는 공유 액세스 정책 이름에 대한 선택에 따라 자동으로 채워져 있습니다.
...
2. 출력 창에서 **추가** 및 **전원 BI를** **선택합니다.**

3. Power **BI - 새 출력** 창에서 **권한 부여를** 선택하고 프롬프트를 따라 Power BI 계정에 로그인합니다.

4. Power BI에 로그인한 후 다음 정보를 입력합니다.

   **출력 별칭**: 출력에 대한 고유한 별칭입니다.
...
분석 스트림 작업에서 **개요를**선택한 **Start** > 다음 지금**시작을****선택합니다.** >  작업이 성공적으로 시작되면 작업 상태가 **중지됨**에서 **실행 중**으로 변경됩니다.
...
3. 사용한 작업 영역인 **내 작업 영역을**선택합니다.

4. **데이터 세트**를 선택합니다.

   Stream Analytics 작업에 대한 출력을 만들 때 지정한 데이터 세트가 표시됩니다.

5. 만든 데이터 집합의 경우 **보고서 추가(데이터** 집합 이름 오른쪽에 있는 첫 번째 아이콘)를 선택합니다.
...
6. 시간이 지남에 따라 실시간 온도를 표시하는 꺾은선형 차트를 만듭니다.
   ...
   1. 보고서 만들기 페이지의 **시각화** 창에서 선부표 아이콘을 선택하여 선형 차트를 추가합니다.
...
9. 왼쪽 창에서 **보고서를** 선택한 다음 방금 만든 보고서를 선택합니다.

10. **웹에 파일** > **게시를**선택합니다.

11. **포함 코드 만들기를**선택한 다음 **게시를**선택합니다.

보고서 액세스를 위해 누구와도 공유할 수 있는 보고서 링크와 보고서를 블로그 또는 웹 사이트에 통합하는 데 사용할 수 있는 코드 조각이 제공됩니다.
...
Azure IoT Hub에서 데이터를 시각화하는 또 다른 방법은 [웹 앱 사용을 참조하여 Azure IoT Hub에서 실시간 센서 데이터를 시각화합니다.](iot-hub-live-data-visualization-in-web-apps.md)
