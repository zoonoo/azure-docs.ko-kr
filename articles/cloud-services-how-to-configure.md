...
5.  **operating system settings**에서 운영 체제 제품군 또는 역할 인스턴스(가상 컴퓨터) 버전을 변경하거나 **자동**을 선택하여 현재 운영 체제 버전의 자동 업데이트를 다시 시작할 수 있습니다. 운영 체제 설정은 웹 역할 및 작업자 역할에 적용되지만 이전 Azure 관리 포털에서 호스티드 서비스에 추가된 VM 역할에는 영향을 주지 않습니다.

    새 클라우드 서비스를 배포할 때 Windows Server 2008 R2, Windows Server 2008 SP2(서비스 팩 2), Windows Server 2012 운영 체제 중에서 선택할 수 있습니다. 배포하는 동안 모든 역할 인스턴스에 최신 운영 체제 버전이 설치되며 운영 체제는 기본적으로 자동 업데이트됩니다.
...
1.  구성 업데이트를 마치면 확인(확인 표시)을 클릭합니다.
...
2.  역할 인스턴스에 연결하려면
    ...
    c. **연결**을 클릭하고 지침에 따라 가상 컴퓨터의 데스크톱을 엽니다.
...
3.  **역할**에서 업데이트할 서비스 역할을 선택하거나 모든 역할을 원하면 **모두**를 선택합니다.
