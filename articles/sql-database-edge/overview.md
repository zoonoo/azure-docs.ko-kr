---
title: Azure SQL Database Edge 란? | Microsoft 문서
...
Azure SQL Database Edge는 업계 최고의 성능, 보안 및 쿼리 처리 기능을 제공 하는 최신 버전의 [Microsoft SQL Server 데이터베이스 엔진](/sql/sql-server/sql-server-technical-documentation?toc=/azure/sql-database-edge/toc.json)를 기반으로 합니다. Azure SQL Database Edge는 SQL Server 및 Azure SQL Database와 동일한 엔진을 기반으로 하기 때문에 응용 프로그램 또는 솔루션 개발을 더 쉽고 빠르게 하는 동일한 T-sql 프로그래밍 노출 영역을 제공 하 고 동시에 응용 프로그램을 만들 수 있습니다. IoT Edge 장치, 데이터 센터 및 클라우드 간의 이식성.
...
SQL Database Edge는 세 가지 버전 또는 소프트웨어 계획과 함께 사용할 수 있습니다. 이러한 버전은 동일한 기능 집합을 가지 며 사용 권한 및 지 원하는 cpu/메모리의 양에 따라 다릅니다.
   ...
   |**계획**  |**설명**  |
   |---------|---------|
   |Developer  |  아래에서 언급 한 표준 sku에 의해 적용 되는 것과 같은 제한 사항이 있는 개발 전용 sku |
   |Standard   |  표준 요금제는 SQL Database Edge 컨테이너에 대해 최대 4 개의 CPU와 최대 32 GB의 메모리를 지원 합니다. |
   |Premium    |  Premium sku는 SQL Database Edge 컨테이너에 최대 8 개의 코어와 최대 64 GB의 메모리를 지원 합니다. |
