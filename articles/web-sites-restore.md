...
이 문서에서는 Azure 웹 사이트 백업 기능을 사용하여 이전에 백업한 웹 사이트를 복원하는 방법을 보여 줍니다. 자세한 내용은 [Microsoft Azure 웹 사이트 백업][Microsoft Azure 웹 사이트 백업]을 참조하십시오.
...
-   [이전에 만든 백업에서 Azure 웹 사이트를 복원하려면][이전에 만든 백업에서 Azure 웹 사이트를 복원하려면]
-   [저장소 계정에서 바로 Azure 웹 사이트를 복원하려면][저장소 계정에서 바로 Azure 웹 사이트를 복원하려면]
-   [웹 사이트 복원 설정 선택 및 복원 작업 시작][웹 사이트 복원 설정 선택 및 복원 작업 시작]
-   [작업 로그 보기][작업 로그 보기]
...
4.  이 문서의 뒷부분에 나오는 [웹 사이트 복원 설정 선택][웹 사이트 복원 설정 선택 및 복원 작업 시작] 섹션의 단계를 따릅니다.
...
6.  저장소 계정 파일이 선택된 채 저장소 계정 상자에 표시됩니다. 오른쪽 화살표를 클릭하여 계속합니다.
    ...
    ![선택된 저장소 계정 파일][선택된 저장소 계정 파일]

7.  다음에 나오는 섹션인 [웹 사이트 복원 설정 선택 및 복원 작업 시작][웹 사이트 복원 설정 선택 및 복원 작업 시작]을 계속합니다.

<a name="RestoreSettings"></a>

## 웹 사이트 복원 설정 선택 및 복원 작업 시작

1.  **웹 사이트 복원 설정 선택**, **복원 대상**에서 **현재 웹 사이트** 또는 **새 웹 사이트 인스턴스**를 선택합니다.

    ![웹 사이트 복원 설정 선택][웹 사이트 복원 설정 선택]
...
2.  웹 사이트와 함께 데이터베이스를 복원하려면 **포함된 데이터베이스**에서 **복원 대상** 아래에 있는 드롭다운을 사용하여 데이터베이스를 복원할 데이터베이스 서버의 이름을 선택합니다. 또한 복원할 데이터베이스 서버를 새로 만들도록 선택하거나 **복원 안 함**을 선택하여 기본값인 데이터베이스를 복원하지 않을 수도 있습니다.
    ...
    ![데이터베이스 서버 호스트 선택][데이터베이스 서버 호스트 선택]
...
3.  기존 데이터베이스를 복원하도록 선택한 경우에는 사용자 이름과 암호를 제공해야 합니다. 새 데이터베이스로 복원하도록 선택한 경우에는 새 데이터베이스 이름을 제공해야 합니다.
    ...
    ![새 SQL 데이터베이스로 복원][새 SQL 데이터베이스로 복원]
...
4.  새 데이터베이스를 만들도록 선택한 경우 다음 대화 상자에서 데이터베이스에 대한 자격 증명 및 다른 초기 구성 정보를 제공해야 합니다. 아래 예제에서는 새 SQL 데이터베이스를 보여 줍니다. (새 MySQL 데이터베이스의 옵션은 약간 다릅니다.)
    ...
    ![새 SQL 데이터베이스 설정][새 SQL 데이터베이스 설정]

5.  확인 표시를 클릭하여 복원 작업을 시작합니다. 복원 작업이 완료되면 포털의 웹 사이트 목록에 새 웹 사이트 인스턴스(선택한 복원 옵션인 경우)가 표시됩니다.

    ![복원된 Contoso 웹 사이트][복원된 Contoso 웹 사이트]

<a name="OperationLogs"></a>

## 작업 로그 보기

1.  웹 사이트 복원 작업의 성공 또는 실패에 대한 세부 정보를 보려면 웹 사이트의 대시보드 탭으로 이동하세요. **간략 상태** 섹션의 **관리 서비스**에서 **작업 로그**를 클릭합니다.

    ![대시보드 - 작업 로그 링크][대시보드 - 작업 로그 링크]

2.  관리 서비스 포털 **작업 로그** 페이지로 이동되며, 이 페이지의 작업 로그 목록에서 복원 작업에 대한 로그를 볼 수 있습니다.

    ![관리 서비스 작업 로그 페이지][관리 서비스 작업 로그 페이지]
...
<!-- IMAGES -->
  ...
  [웹 사이트 복원 설정 선택]: ./media/web-sites-restore/05ChooseRestoreSettings.png
