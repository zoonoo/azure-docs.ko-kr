...
예제에서 매퍼 및 리듀서는 모두 [stdin](http://msdn.microsoft.com/ko-kr/library/3x292kth(v=vs.110).aspx)에서 입력을 줄 단위로 읽고 출력을 [stdout](http://msdn.microsoft.com/ko-kr/library/3x292kth(v=vs.110).aspx)으로 내보내는 실행 파일입니다. 프로그램은 텍스트의 모든 단어 수를 계산합니다.

**매퍼**에 대해 실행 파일이 지정된 경우 각 매퍼 작업은 매퍼가 초기화될 때 개별 프로세스로 실행 파일을 시작합니다. 실행 시 매퍼 작업은 입력을 줄로 변환하고 프로세스의 [stdin](http://msdn.microsoft.com/ko-kr/library/3x292kth(v=vs.110).aspx)에 줄을 공급합니다. 그 동안 매퍼는 프로세스의 stdout에서 줄 지향 출력을 수집하고 각 줄을 키/값 쌍으로 변환합니다. 키/값 쌍은 매퍼의 출력으로 수집됩니다. 기본적으로 첫 번째 탭 문자까지 줄의 접두사는 키이고 줄의 나머지(탭 문자 제외)는 값입니다. 줄에 탭 문자가 없는 경우에는 전체 줄이 키로 간주되고 값은 null입니다.

**리듀서**에 대해 실행 파일이 지정된 경우 각 리듀서 작업은 리듀서가 초기화될 때 개별 프로세스로 실행 파일을 시작합니다. 실행 시 리듀서 작업은 입력 키/값 쌍을 줄로 변환하고 프로세스의 [stdin](http://msdn.microsoft.com/ko-kr/library/3x292kth(v=vs.110).aspx)에 줄을 공급합니다. 그 동안 리듀서는 프로세스의 [stdout](http://msdn.microsoft.com/ko-kr/library/3x292kth(v=vs.110).aspx)에서 줄 지향 출력을 수집하고 각 줄을 키/값 쌍으로 변환합니다. 키/값 쌍은 리듀서의 출력으로 수집됩니다. 기본적으로 첫 번째 탭 문자까지 줄의 접두사는 키이고 줄의 나머지(탭 문자 제외)는 값입니다.
...
wc.cs 파일의 리듀서 코드는 [StreamReader](http://msdn.microsoft.com/ko-kr/library/system.io.streamreader.aspx) 개체를 사용하여 cat.exe 매퍼에 의해 출력된 표준 입력 스트림에서 문자를 읽습니다. [Console.Writeline](http://msdn.microsoft.com/ko-kr/library/system.console.writeline) 메서드를 사용하여 문자를 읽는 경우 각 단어의 끝에 있는 공백과 줄의 끝 문자를 포함하여 단어 수를 계산한 다음 [Console.Writeline](http://msdn.microsoft.com/ko-kr/library/system.console.writeline) 메서드를 사용하여 합계를 표준 출력 스트림에 씁니다.
