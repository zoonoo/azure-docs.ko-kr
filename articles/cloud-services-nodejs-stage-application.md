...
2.  [Azure 관리 포털](http://manage.windowsazure.com)에 로그온하고 **클라우드 서비스**를 선택합니다. 클라우드 서비스가 만들어지고 **스테이징** 열 상태가 **실행 중**으로 업데이트된 후 서비스 이름을 클릭합니다.
...
업그레이드 시나리오의 경우 미리 구성된 응용 프로그램이 프로덕션에 이미 배포한 업그레이드된 버전의 응용 프로그램이므로 [VIP를 교환하여 프로덕션 환경에서 응용 프로그램을 업그레이드](#step2)할 수 있습니다.
...
1.  [Azure 관리 포털](http://manage.windowsazure.com)에 로그인하고 **클라우드 서비스**를 클릭한 후 서비스 이름을 선택합니다.
