<properties urlDisplayName="Web App with Express" pageTitle="Express를 사용하여 웹 앱 빌드(Node.js) - Azure 자습서" metaKeywords="Azure Node.js hello world tutorial, Azure Node.js hello world, Azure Node.js Getting Started tutorial, Azure Node.js tutorial, Azure Node.js Express tutorial" description="클라우드 서비스 자습서를 기반으로 웹 앱을 빌드하고 Express 모듈 사용 방법을 보여 주는 자습서입니다." metaCanonical="" services="cloud-services" documentationCenter="nodejs" title="Azure 클라우드 서비스에서 Express를 사용하여 Node.js 웹 응용 프로그램 빌드" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
개발자는 Node.js 응용 프로그램을 개발할 때 추가 기능을 제공하기 위해
종종 타사 모듈을 사용합니다. 이 자습서에서는
Node.js 웹 응용 프로그램을 만들기 위한 MVC 프레임워크를 제공하는 [Express][Express] 모듈을 사용하여 새 응용 프로그램을 만들어 봅니다.

아래에는 완성된 응용 프로그램의 스크린샷이 표시되어 있습니다.

![Welcome to Express in Azure를 표시하는 웹 브라우저][Welcome to Express in Azure를 표시하는 웹 브라우저]
...
2.  디렉터리를 **c:\\node** 디렉터리로 변경한 후 다음 명령을 입력하여 **expressapp**라는 이름의 새 솔루션과 **WebRole1**이라는 웹 역할을 만듭니다.
    ...
    > [WACOM.NOTE] 기본적으로 **Add-AzureNodeWebRole**에서는 이전 버전의 Node.js를 사용합니다. 위의 **Set-AzureServiceProjectRole** 문은 v0.10.21 노드를 사용하도록 Azure에 지시합니다.
...
1.  다음 명령을 실행하여 Express 생성기를 설치합니다.
    ...
    ![express 설치 npm 명령의 출력을 표시하는 Windows PowerShell][express 설치 npm 명령의 출력을 표시하는 Windows PowerShell]
...
2.  디렉터리를 **WebRole1** 디렉터리로 변경하고 express 명령을 사용하여 새 응용 프로그램을 만듭니다.
    ...
    ![express 명령의 출력][express 명령의 출력]

3.  package.json 파일에 정의된 추가 종속성을 설치하려면
    다음 명령을 입력합니다.
...
6.  다음 명령을 사용하여 Azure 에뮬레이터에서 응용 프로그램을
    실행합니다.

        PS C:\node\expressapp\WebRole1> Start-AzureEmulator -launch

    ![welcome to express가 들어 있는 웹 페이지][welcome to express가 들어 있는 웹 페이지]

## 뷰 수정

이제 "Welcome to Express in Azure" 메시지를 표시하도록
뷰를 수정합니다.
...
1.  다음 명령을 입력하여 index.jade 파일을 엽니다.
    ...
    ![index.jade 파일의 내용][index.jade 파일의 내용]

    Jade는 Express 응용 프로그램에서 사용하는 기본 뷰 엔진입니다. Jade 뷰 엔진에 대한
    자세한 내용은 <http://jade-lang.com>을 참조하세요.

2.  **in Azure**를 추가하여 텍스트의 마지막 줄을 수정합니다.

    ![index.jade 파일, 마지막 줄: p Welcome to \#{title} in Azure][index.jade 파일, 마지막 줄: p Welcome to \#{title} in Azure]

3.  파일을 저장하고 메모장을 종료합니다.

4.  브라우저를 새로 고치면 변경 내용이 표시됩니다.

    ![브라우저 창, Welcome to Express in Azure가 들어 있는 페이지][브라우저 창, Welcome to Express in Azure가 들어 있는 페이지]
...
![Express 페이지를 표시하는 웹 브라우저입니다. URL은 현재 Azure에서 호스트되어 있음을 나타냅니다.][Welcome to Express in Azure를 표시하는 웹 브라우저]
