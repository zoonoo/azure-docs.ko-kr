...
SendGrid는 사용자 지정 통합을 쉽게 만드는 유연한 API와 함께 신뢰할 만한 [트랜잭션 전자 메일 배달](http://sendgrid.com/transactional-email), 확장성 및 실시간 분석을 제공하는 [클라우드 기반 전자 메일 서비스](http://sendgrid.com/solutions)입니다. 자세한 내용은 <http://sendgrid.com>(영문)을 참조하십시오.

이 자습서에서는 전자 메일 기능을 사용하도록 설정하는 다음 기본 단계를 단계별로 안내합니다.

1.  [SendGrid 계정 만들기](#sign-up)
2.  [전자 메일을 보내기 위한 스크립트 추가](#add-script)
3.  [전자 메일을 받기 위한 데이터 삽입](#insert-data)

이 자습서는 모바일 서비스 quickstart를 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [모바일 서비스 시작하기](/en-us/develop/mobile/tutorials/get-started)를 완료해야 합니다.

새 계정 만들기새 SendGrid 계정 만들기
-------------------------------------

[WACOM.INCLUDE [sendgrid-sign-up](../includes/sendgrid-sign-up.md)]

스크립트 등록전자 메일을 보내는 새 스크립트 등록
------------------------------------------------
...
5.  위 스크립트에서 자리 표시자를 올바른 값, 즉
    ...
    -   ***username* 및 *password***( [SendGrid 계정 만들기](#sign-up)에서 식별한 SendGrid 자격 증명)로 바꿉니다.
...
테스트 데이터 삽입전자 메일을 받기 위해 테스트 데이터 삽입
----------------------------------------------------------
