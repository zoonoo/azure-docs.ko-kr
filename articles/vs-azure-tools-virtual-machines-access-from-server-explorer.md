...
1. 서버 탐색기에서 가상 컴퓨터를 선택한 다음 F4 키를 선택하여 속성 창을 표시합니다.
    ...
    다음의 테이블은 사용 가능하지만 모두 읽기 전용인 속성을 보여줍니다. 변경하려면 관리 포털을 사용합니다.
  	...
  	|상태|값은 시작, 시작됨, 중지, 중지됨, 및 상태 검색을 포함합니다. 상태 검색이 나타나면 현재 상태는 알 수 없습니다. 이 속성의 값은 관리 포털에서 사용된 값과 다릅니다.|
  	|구독 ID|Azure 계정에 대한 구독 ID 구독 속성을 확인하여 관리 포털에 대한 정보를 표시할 수 있습니다.|

1. 끝점 노드를 선택한 다음 **속성** 창을 확인합니다.

1. 다음 테이블은 사용 가능하지만 읽기 전용인 끝점 속성을 설명합니다. 가상 컴퓨터에 대해 끝점을 추가 또는 편집하려면 관리 포털을 사용합니다.
