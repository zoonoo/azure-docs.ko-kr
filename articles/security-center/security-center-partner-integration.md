...
## <a name="deploy-partner-solutions-from-security-center"></a>Security Center의 파트너 솔루션 배포

Security Center에서 파트너 통합을 활용하는 네 가지 주요 이유는 다음과 같습니다.
...
Security Center와 Azure Marketplace에서 사용할 수 있는 파트너의 솔루션의 네이티브 통합은 현재 다음과 같습니다.

- **Endpoint protection** [Trend Micro](https://help.deepsecurity.trendmicro.com/azure-marketplace-getting-started-with-deep-security.html), Symantec 및 [Azure Cloud Services와 Virtual Machines용 Microsoft 맬웨어 방지 프로그램](https://docs.microsoft.com/azure/security/azure-security-antimalware)
...
배포 후에 솔루션의 상태에 대한 정보를 보고 기본 관리 작업을 수행하려면 **Security Center** 대시보드에서 **파트너 솔루션**을 선택합니다.
...
- **검색된 솔루션** Security Center에 연결되지 않은 솔루션을 표시합니다. 이러한 솔루션을 연결하면 **연결된 솔루션** 아래에 표시됩니다. Security Center가 연결되지 않은 솔루션을 감지하지 않는 경우 이 섹션은 숨겨집니다.
- **데이터 원본 추가** Security Center에 추가할 수 있는 Azure 및 비 Azure 데이터 원본을 표시합니다.
...
표시되는 정보는 솔루션에 따라 달라질 수 있습니다. 각 타일의 일부 정보에는 다음과 같습니다.

- **파트너의 회사 아이콘** Security Center에 회사 아이콘이 없는 경우 파트너 이름의 첫 번째 문자가 표시됩니다.
- **솔루션 형식** 솔루션의 유형이 표시됩니다.
- **컴퓨터 이름** 컴퓨터 이름이 표시됩니다.
...
> [!NOTE]
> Security Center는 마지막으로 받은 이벤트의 날짜와 시간을 표시하여 어플라이언스가 보고하는지 여부를 보여줍니다. 상태 표시기를 보내지 않는 솔루션은 지난 14일 이내에 경고 또는 이벤트를 보낸 경우 연결된 것으로 표시됩니다.
>  

이들 솔루션 중 일부는 Azure에서 완벽하게 통합되고, 나머지 솔루션은 온-프레미스에 통합되기도 합니다. Security Center가 [CEF](https://docs.microsoft.com/azure/operations-management-suite/oms-security-connect-products#what-is-cef)를 지원하기 때문에 CEF를 지원하는 방화벽 등 CEF를 사용하는 솔루션과 연결할 수 있습니다. 이 솔루션이 Security Center에 추가된 후에 방화벽은 CEF 형식의 로그를 Security Center로 보내고 Security Center는 이를 [Azure Log Analytics](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)에 적용합니다. 방화벽은 이벤트를 보내지만 상태 표시기는 보내지 않는 비 Azure 리소스입니다. Security Center에 있는 유일한 상태 정보는 이 어플라이언스가 이벤트를 보낸 최종 시간입니다. 모든 비 Azure 리소스의 경우, 타일의 상태 영역에서 Security Center는 마지막 이벤트를 받은 날짜 및 시간을 표시합니다. 이 정보는 비 Azure 리소스가 여전히 보고하고 있음을 나타냅니다.
...
Security Center는 [Azure AD(Azure Active Directory) ID 보호](https://docs.microsoft.com/azure/active-directory/active-directory-identityprotection)와 같은 기본 제공 Azure 솔루션을 통합할 수 있습니다. Azure AD ID 보호에 대한 라이선스가 있지만 Security Center에 연결되지 않은 경우, Azure AD ID 보호는 **검색된 솔루션** 아래에 나열됩니다. Security Center와 이 솔루션을 통합하기 위해 **Azure AD ID 보호** 타일에서 **연결**을 선택하면 다음과 같은 페이지가 표시됩니다.

![Azure AD ID 보호](./media/security-center-partner-integration/security-center-partner-integration-fig6.png)

Azure AD ID 보호의 연결을 종료하려면 데이터가 저장된 작업 영역을 선택합니다. Azure AD ID 보호의 모든 데이터는 이 단계에서 선택한 작업 영역으로부터 흘러 나옵니다. 작업 영역 선택기를 사용하여 작업 영역을 선택하면 데이터가 해당 작업 영역에서 흐르기 시작합니다.

Security Center에 연결하려면 사용자가 전역 관리자 또는 보안 관리자이어야 합니다. 권한이 없는 경우는 **연결** 단추가 비활성화됩니다. 단추가 비활성화되는 이유를 설명한 메시지가 나타납니다.

Azure AD ID 보호 경고는 Security Center 감지 파이프를 거칩니다. 이러한 방식으로 Security Center와 Azure AD ID 보호의 알림을 가져옵니다. Security Center는 [보안 인시던트](https://docs.microsoft.com/azure/security-center/security-center-incident)를 만드는 것과 관련된 것처럼 보이는 모든 경고를 병합합니다. 보안 인시던트 설명은 의심스러운 활동에 대한 자세한 정보를 제공합니다.
