---
title: 표준 계층의 향상된 보안 - Azure 보안 센터
...
Security Center 표준 계층으로 업그레이드하면 하이브리드 클라우드 작업을 위해 강화된 보안 관리 및 위협 방지 기능을 활용할 수 있습니다. 표준 평가판을 사용해 볼 수 있습니다. 자세한 내용은 보안 센터 [가격 페이지를](https://azure.microsoft.com/pricing/details/security-center/) 참조하십시오.

보안 센터 표준 계층에는 다음이 포함됩니다.
...
전체 Azure 구독을 구독 내에서 지원되는 모든 리소스에 의해 상속되는 표준 계층으로 업그레이드할 수 있습니다. 작업 영역에 표준 계층을 적용하는 것은 작업 영역에 보고되는 모든 리소스에 적용됩니다.
...
구독 또는 작업 영역을 표준으로 업그레이드하려면 다음을 수행하십시오.
...
3.  **평가판 시작**을 클릭하여 선택한 구독에서 평가판을 시작할 수 있습니다.
  ![구독 선택](./media/security-center-onboarding/select-subscription.png)
   ...
   > [!NOTE]
   > 보안 센터의 무료 기능은 Azure VM 및 VMSS에만 적용됩니다. 즉, 비 Azure 컴퓨터에는 무료 기능이 적용되지 않습니다. 표준을 선택하면 표준 기능이 모든 Azure VM, VM 배율 집합 및 작업 영역에 보고하는 비Azure 컴퓨터에 적용됩니다. 그러므로 Azure 및 비 Azure 리소스에 고급 보안을 제공하려는 경우 표준을 적용하는 것이 좋습니다.
   >
...
2. **보안 및 감사에서** **OMS 작업 영역을** 선택하여 새 작업 영역을 만듭니다.
   > [!NOTE]
...
- [데이터 수집 사용](security-center-enable-data-collection.md)
