---
title: Azure 보안 센터 FAQ - 기존 로그 분석 에이전트에 대한 질문
description: 이 FAQ는 이미 Log Analytics 에이전트를 사용하고 있으며 위협을 방지, 탐지 및 대응하는 데 도움이 되는 제품인 Azure Security Center를 고려하는 고객에 대한 질문에 답합니다.
...
VM에 이미 로그 분석 에이전트가 Azure 확장으로 설치되어 있는 경우 보안 센터는 기존 작업 영역 연결을 재정의하지 않습니다. 대신 Security Center에서 기존 작업 영역을 사용합니다. 보고하는 작업 영역에 "보안" 또는 "SecurityCenterFree" 솔루션이 설치된 경우 VM이 보호됩니다. 
...
> [!TIP]
> 로그 분석 에이전트가 Azure 확장이 아닌 VM에 직접 설치되는 경우 보안 센터는 Log Analytics 에이전트를 설치하지 않으며 보안 모니터링이 제한됩니다.
...
VM에 이미 Azure 확장으로 설치된 Log Analytics 에이전트가 있는 경우 보안 센터는 기존 연결된 작업 영역을 사용합니다. Security Center 솔루션이 작업 영역에 없는 경우 설치되고 솔루션은 [솔루션 대상 지정](../operations-management-suite/operations-management-suite-solution-targeting.md)을 통해 관련 VM에만 적용됩니다.

보안 센터에서 VM에 로그 분석 에이전트를 설치하면 보안 센터에서 만든 기본 작업 영역을 사용합니다.
