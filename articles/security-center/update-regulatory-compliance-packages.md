...
ms.locfileid: "82598347"
---
# <a name="update-to-dynamic-compliance-packages-in-your-regulatory-compliance-dashboard"></a>규정 준수 대시보드의 동적 준수 패키지 업데이트
...
**동적 준수 패키지** 기능을 사용 Security Center 하면 *시간에 따른 업계 표준의 적용 범위가 자동으로 향상*됩니다. 

규정 준수 상태를 추적할 수 있는 표준 하나는 [AZURE cis 1.1.0](https://www.cisecurity.org/benchmark/azure/) (보다 공식적이 고 "Ci Microsoft Azure 기초 벤치 마크 버전 1.1.0")입니다. 규정 준수 대시보드에 처음 표시 되는 Azure CI 표현은 Security Center에 포함 된 정적 규칙 집합을 사용 합니다.

호환성 패키지는 기본적으로 Azure Policy에 정의 된 이니셔티브입니다. 선택한 범위 (구독, 관리 그룹 등)에 할당할 수 있습니다. 대시보드에서 평가로 매핑된 준수 데이터를 확인 하려면 보안 정책 내에서 관리 그룹 또는 구독에 규정 준수 패키지를 추가 합니다. 규정 준수 패키지를 추가 하면 선택한 범위에 규정 준수 이니셔티브를 효과적으로 할당 합니다. 이러한 방식으로 새로 게시 된 규정 이니셔티브를 대시보드의 규정 준수 표준으로 추적할 수 있습니다. Microsoft에서 이니셔티브 (표준의 더 많은 컨트롤에 매핑되는 새 정책)에 대해 새 콘텐츠를 릴리스할 때 추가 콘텐츠가 대시보드에 자동으로 표시 됩니다.
...
1. 페이지 맨 위에서 **준수 정책 관리**를 선택 합니다. 그러면 정책 관리 페이지가 열립니다.
...
1. 업계 & 규정 표준 섹션에서 새 콘텐츠에 대 한 Azure CIS 1.1.0를 업데이트할 수 있는 것을 볼 수 있습니다. **지금 업데이트**를 클릭 합니다. 

1. 필요에 따라 **표준 추가** 를 클릭 하 여 **규제 준수 표준 추가** 페이지를 엽니다. 여기에서 다음과 같은 다른 규정 준수 표준에 대해 **AZURE CIS 1.1.0 (신규)** 및 동적 패키지를 수동으로 검색할 수 있습니다.

    - **Azure 보안 벤치 마크** ([세부 정보](https://docs.microsoft.com/azure/security/benchmarks/introduction))
    - **NIST SP 800-53 R4**
...
1. Security Center의 사이드바에서 **규제** 준수를 다시 선택 하 여 규정 준수 대시보드로 돌아갑니다.
    ...
    * Azure CIS 1.1.0 준수의 원래 *정적* 보기도 함께 유지 됩니다. 나중에 자동으로 제거 될 수 있습니다.
    ...
    [![이전 및 새 Azure CI를 표시 하는 규정 준수 대시보드](media/update-regulatory-compliance-packages/security-center-dynamic-regulatory-compliance-cis-old-and-new.png)](media/update-regulatory-compliance-packages/security-center-dynamic-regulatory-compliance-cis-old-and-new.png#lightbox)
