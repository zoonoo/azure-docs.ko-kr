---
title: 모든 Azure Security Center 경고에 대한 참조 테이블
description: 이 문서에는 Azure Security Center의 위협 방지 모듈에 표시되는 보안 경고가 나와 있습니다.
...
ms.locfileid: "83745074"
---
# <a name="security-alerts---a-reference-guide"></a>보안 경고 - 참조 가이드

이 문서에는 Azure Security Center의 위협 방지 모듈에서 볼 수 있는 보안 경고가 나와 있습니다. 사용자 환경에 표시되는 경고는 보호하는 리소스 및 서비스와 사용자 지정된 구성에 따라 다릅니다.

이러한 경고에 대응하는 방법을 알아보려면 [Azure Security Center에서 보안 경고 관리 및 대응](security-center-managing-and-responding-alerts.md)을 참조하세요.

경고 (및 권장 사항)를 내보내는 방법을 알아보려면 [보안 경고 및 권장 사항 내보내기(미리 보기)](continuous-export.md)를 참조하세요.
...
[추가 세부 정보 및 참고 사항](threat-protection.md#windows-machines)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**악성 IP에서 로그온이 감지됨**|'tristan.schleining' 계정 및 'Advapi' 프로세스에 대해 원격 인증이 완료되었지만 로그온 IP 주소(x.x.x.x)가 이전에 악성이거나 매우 이례적인 것으로 보고되었습니다. 공격이 발생했을 수 있습니다.|-|높음|
|**악성 IP에서 로그온이 감지됨[여러 번 발생함]**|'IUSR_10001' 계정 및 'Advapi' 프로세스에 대해 원격 인증이 완료되었지만 로그온 IP 주소(x.x.x.x)가 이전에 악성이거나 매우 이례적인 것으로 보고되었습니다. 공격이 발생했을 수 있습니다. 확장명이 .scr인 파일은 화면 보호기 파일이며 일반적으로 Windows 시스템 디렉터리에 있으며 이곳에서 실행됩니다.|-|높음|
...
|**파일리스 공격 도구 키트가 감지됨**|지정된 프로세스의 메모리에 파일리스 공격 도구 키트([도구 키트 이름]])가 포함되어 있습니다. 파일리스 공격 도구 키트는 디스크에서 맬웨어의 추적을 최소화하거나 제거하는 기술을 사용하며 디스크 기반 맬웨어 검사 솔루션을 통한 감지 가능성을 대폭 줄입니다. 구체적인 동작은 다음과 같습니다.<br>1) Shellcode. 일반적으로 소프트웨어 취약성을 악용하는 페이로드로 사용되는 작은 코드 조각입니다.<br>2) 코드 삽입 공격과 같이 프로세스에 삽입되는 실행 파일 이미지입니다.<br>3) 보안에 중요한 운영 체제 인터페이스에 대한 함수 호출입니다. 참조된 OS 기능에 대해서는 아래 기능을 참조하세요.<br>4) 동적으로 할당된 코드 세그먼트에서 시작된 스레드가 포함됩니다. 이는 프로세스 삽입 공격의 일반적인 패턴입니다.|DefenseEvasion|높음|
...
|**성공적인 무차별 암호 대입 공격**|동일한 원본에서 발생한 여러 가지 실패한 인증 시도가 Azure 구독의 여러 호스트에서 감지되었습니다. 이는 암호 스프레이 공격과 유사하며 공격자가 여러 호스트에서 수많은 인증 시도를 수행합니다. 인증 시도 중 일부가 이 구독의 호스트에 성공적으로 로그인되었습니다.|-|높음|
...
|**의심스러운 시스템 파일 실행**|호스트 데이터를 분석한 결과, 비정상적인 위치에서 실행 중인 %{Compromised Host}에서 실행 파일이 감지되었습니다. 이 실행 파일은 합법적인 활동일 수도 있고 손상된 호스트를 암시할 수도 있습니다.|-|높음|
...
[추가 세부 정보 및 참고 사항](threat-protection.md#linux-machines)
...
[추가 세부 정보 및 참고 사항](threat-protection.md#app-services)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**Windows App Service에서 Linux 명령을 실행하려고 함**|App Service 프로세스를 분석한 결과, Windows App Service에서 Linux 명령을 실행하려는 시도가 감지되었습니다. 이 작업은 웹 응용 프로그램에 의해 실행되었습니다. 이 동작은 일반적인 웹 응용 프로그램의 취약성을 악용하는 캠페인 중에 나타나는 경우가 많습니다.|-|중간|
|**Azure App Service FTP 인터페이스에 연결된 IP가 위협 인텔리전스에 있음**|App Service FTP 로그를 분석한 결과, 위협 인텔리전스 피드에 있는 소스 주소에서 연결이 감지되었습니다. 이 연결 도중 나열된 페이지에 사용자가 액세스했습니다.|-|중간|
|**비정상적인 요청 패턴이 감지됨**|Azure App Service 활동 로그에 %{Source IP}에서 App Service에 대한 비정상적인 HTTP 활동이 나타납니다. 이 활동은 퍼지 테스트 및 무차별 암호 대입 활동의 패턴과 유사합니다.|-||
|**높은 권한 명령을 실행하려는 시도가 감지됨**|App Service 프로세스를 분석한 결과, 높은 권한이 필요한 명령을 실행하려는 시도가 감지되었습니다. 웹 응용 프로그램 컨텍스트에서 실행된 명령입니다. 이 동작은 합법적일 수 있지만 웹 응용 프로그램에서 이 동작은 악의적인 활동을 나타낼 수도 있습니다.|-|중간|
|**비정상적인 IP 주소에서 웹 페이지에 대한 연결이 감지됨**|Azure App Service 활동 로그에 이전에 한 번도 연결되지 않은 소스 IP 주소(%{Source IP Address})에서 중요한 웹 페이지로의 연결이 나타납니다. 이는 누군가 웹 앱 관리 페이지에 대한 무차별 암호 대입 공격을 시도하고 있음을 나타낼 수도 있습니다. 합법적인 사용자가 새 IP 주소를 사용했기 때문일 수도 있습니다.|-||
|**피싱 콘텐츠가 Azure Webapps에 호스트됨**|피싱 공격에 사용된 URL이 Azure AppServices 웹 사이트에 있습니다. 이 URL은 O365 고객에게 전송된 피싱 공격의 일부입니다. 해당 콘텐츠는 일반적으로 방문자가 회사 자격 증명이나 재무 정보를 합법적으로 보이는 웹 사이트에 입력하도록 유도합니다.|컬렉션|높음|
|**업로드 파일의 PHP 파일**|Azure App Service 활동 로그에 업로드 폴더에 있는 의심스러운 PHP 페이지에 대한 액세스가 나타납니다. 이 유형의 폴더에는 일반적으로 PHP 파일이 포함되지 않습니다. 이 형식의 파일이 존재하면 임의의 파일 업로드 취약성을 이용하는 악용을 암시할 수도 있습니다.|-||
|**원시 데이터 다운로드가 감지됨**|App Service 프로세스를 분석한 결과, Pastebin과 같은 원시 데이터 웹 사이트에서 코드를 다운로드하려는 시도가 감지되었습니다. 이 작업은 PHP 프로세스를 통해 실행되었습니다. 이 동작은 웹 셸 또는 기타 악성 구성 요소를 App Service에 다운로드하려는 시도와 관련이 있습니다.|-|중간|
|**디스크에 넘기기 출력 저장이 감지됨**|App Service 프로세스를 분석한 결과, 디스크에 출력이 저장된 넘기기 명령 실행이 감지되었습니다. 이 동작은 합법적일 수 있지만 웹 응용 프로그램에서는 웹 셸로 웹 사이트를 감염하려는 시도와 같은 악의적인 활동에서도 이 동작이 관찰됩니다.|-|낮음|
|**스팸 폴더 참조 페이지가 감지됨**|Azure App Service 활동 로그에 SPAM 활동과 관련된 웹 사이트에서 발생한 것으로 식별된 웹 활동이 나타납니다. 이는 웹 사이트가 손상되어 스팸 활동에 사용되는 경우에 발생할 수 있습니다.|-||
|**취약할 수 있는 웹 페이지에 대한 의심스러운 액세스가 감지됨**|App Service 활동 로그에 중요한 것으로 보이는 웹 페이지에 액세스된 것으로 나타납니다.<br>이 의심스러운 활동은 액세스 패턴이 웹 스캐너의 액세스 패턴과 유사한 소스 주소에서 시작되었습니다. 이러한 종류의 활동은 공격자가 민감하거나 취약한 웹 페이지에 액세스하기 위해 네트워크를 검사하려는 시도와 관련이 있는 경우가 많습니다. |-|중간|
|**의심스러운 PHP 실행이 감지됨**|컴퓨터 로그에 의심스러운 PHP 프로세스가 실행 중인 것으로 나타납니다. 이 작업에는 PHP 프로세스를 사용하여 명령줄에서 운영 체제 명령 또는 PHP 코드를 실행하려는 시도가 있었습니다. 이 동작은 합법적일 수 있지만 웹 응용 프로그램에서는 이 동작이 웹 셸로 웹 사이트를 감염하려는 시도와 같은 악의적인 활동으로 암시될 수도 있습니다.|실행|중간|
|**의심스러운 사용자 에이전트가 감지됨**|Azure App Service 활동 로그에 의심스러운 사용자 에이전트가 있는 요청이 나타납니다. 이 동작은 App Service 응용 프로그램의 취약성을 악용하려는 시도를 나타낼 수 있습니다.|-||
|**의심스러운 WordPress 테마 호출이 감지됨**|App Service 활동 로그에 App Service 리소스에서의 코드 삽입 활동이 발생한 것으로 나타납니다.<br>이 의심스러운 활동은 코드의 서버 쪽 실행을 지원하기 위해 WordPress 테마를 조작한 후 조작된 테마 파일을 호출하도록 직접 웹 요청을 수행하는 활동과 유사합니다. 이러한 유형의 작업은 WordPress에 대한 공격 캠페인의 일부일 수 있습니다.|-|높음|
|**취약성 검사기가 감지됨**<br>(Joomla/WordPress/CMS)|Azure App Service 활동 로그에 App Service 리소스에서 취약성 스캐너가 사용되었던 것으로 나타냅니다. 의심스러운 활동이 Joomla 응용 프로그램/WordPress 응용 프로그램/CMS(콘텐츠 관리 시스템)를 대상으로 하는 도구와 유사한 것으로 감지되었습니다.|-|중간|
|**웹 지문이 감지됨**<br>(NMAP/Blind Elephant)|App Service 활동 로그에 App Service 리소스에서 웹 지문 활동이 나타납니다.<br>이 의심스러운 활동은 Blind Elephant라는 도구와 관련되어 있습니다. 도구는 웹 서버의 지문을 채취하고 설치된 응용 프로그램 및 해당 버전을 감지하려고 합니다. 공격자는 이 도구를 사용하여 웹 응용 프로그램을 검색해 취약성을 찾는 경우가 많습니다. |-|중간|
|||||
...
[추가 세부 정보 및 참고 사항](threat-protection.md#azure-containers)
...
## <a name="alerts-for-sql-database-and-sql-data-warehouse"></a><a name="alerts-sql-db-and-warehouse"></a>SQL Database 및 SQL Data Warehouse에 대한 경고

[추가 세부 정보 및 참고 사항](threat-protection.md#data-sql)
...
|**잠재적으로 위험한 응용 프로그램에서 로그온을 시도함**|잠재적으로 위험한 응용 프로그램이 데이터베이스에 액세스하는 데 사용되었습니다. 일부 경우에서 경고는 작업의 침투 테스트를 검색합니다. 다른 경우에 경고는 일반 도구를 사용하여 공격을 감지합니다.|검색|높음|
|**알 수 없는 보안 주체가 액세스**|SQL Server에 대한 액세스 패턴이 변경되었습니다. 누군가가 비정상적인 보안 주체(사용자)를 사용하여 서버에 로그인했습니다. 일부 경우에서 경고는 합법적인 작업(새 애플리케이션 또는 개발자 유지 관리)을 검색합니다. 다른 경우에 경고는 악의적인 작업(퇴사 직원 또는 외부 공격자)을 감지합니다.|악용|중간|
...
|**잠재적으로 안전하지 않은 작업**|악의적인 세션에서 흔히 사용되는 높은 권한의 SQL 명령이 SQL Server에서 실행되었습니다. 이러한 명령은 기본적으로 사용하지 않도록 설정하는 것이 좋습니다. 어떤 경우에 경고는 합법적인 작업(관리자 스크립트 실행)을 감지합니다. 또 다른 경우 경고는 악의적인 작업(SQL 트러스트를 사용하여 공격자가 Windows 계층을 위반함)을 감지합니다.|실행|높음|
|**비정상적인 내보내기 위치**|SQL 가져오기 및 내보내기 작업을 위한 내보내기 저장소 대상이 변경되었습니다. 어떤 경우에 경고는 합법적인 변경(새 백업 대상)을 감지합니다. 또 다른 경우에 경고는 악의적인 작업(공격자가 데이터를 파일로 쉽게 반출함)을 감지합니다.|반출|높음|
|||||


## <a name="alerts-for-azure-storage"></a><a name="alerts-azurestorage"></a>Azure Storage에 대한 경고

[추가 세부 정보 및 참고 사항](threat-protection.md#azure-storage)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**Tor 종료 노드에서 저장소 계정으로 액세스**|Tor(익명 프록시)의 활성 종료 노드로 알려진 IP 주소에서 이 계정에 성공적으로 액세스한 것으로 나타납니다. 이 경고의 심각도는 사용된 인증 유형(있는 경우)과 이 액세스가 첫 번째 사례인지를 고려합니다. 잠재적 원인은 Tor를 사용하여 저장소 계정에 액세스한 공격자 또는 Tor를 사용하여 저장소 계정에 액세스한 합법적인 사용자일 수 있습니다.|검색/악용|높음|
|**비정상적인 위치에서 저장소 계정으로 액세스**|Azure Storage 계정에 대한 액세스 패턴이 변경된 것으로 나타납니다. 최근 활동과 비교할 때 익숙하지 않은 것으로 간주되는 IP 주소를 통해 누군가가 이 계정에 액세스했습니다. 공격자가 계정에 대한 액세스 권한을 얻었거나, 합법적인 사용자가 새롭거나 비정상적인 지리적 위치에서 연결했습니다. 후자의 경우 새 응용 프로그램 또는 개발자의 원격 유지 관리입니다.|악용|낮음|
|**저장소 계정에 대한 익명 액세스**|저장소 계정에 대한 액세스 패턴이 변경된 것으로 나타납니다. 예를 들어 계정이 (인증 없이) 익명으로 액세스된 경우는 이 계정의 최근 액세스 패턴과 비교할 때 예기치 않은 상황입니다. 잠재적 원인은 공격자가 Blob Storage를 보유한 컨테이너에 대한 공용 읽기 액세스 권한을 악용한 것일 수 있습니다.|악용|높음|
|**저장소 계정에 업로드된 잠재적 맬웨어**|잠재적 맬웨어가 포함된 Blob이 저장소 계정에 업로드된 것으로 나타납니다. 이 경고는 바이러스, 트로이 목마, 스파이웨어 및 랜섬웨어에 대한 해시를 포함한 Microsoft 위협 인텔리전스의 기능을 활용하는 해시 평판 분석을 기반으로 합니다. 잠재적 원인에는 공격자의 의도적인 맬웨어 업로드 또는 합법적인 사용자가 잠재적으로 악의적인 Blob을 의도치 않게 업로드하는 경우가 있을 수 있습니다. Microsoft의 위협 인텔리전스 기능에 대한 자세한 내용은 다음을 참조하세요. https://go.microsoft.com/fwlink/?linkid=2128684 |측면 이동|높음|
|**저장소 계정에서 비정상적인 액세스 검사**|이 계정의 최근 활동과 비교할 때 저장소 계정의 액세스 권한이 비정상적인 방식으로 검사된 것으로 나타납니다. 잠재적 원인은 공격자가 향후 공격을 위해 정찰을 수행한 것일 수 있습니다.|컬렉션|중간|
|**저장소 계정에서 추출된 비정상적인 데이터 크기**|이 저장소 컨테이너의 최근 활동과 비교할 때 비정상적으로 많은 양의 데이터가 추출된 것으로 나타납니다. 잠재적 원인은 공격자가 Blob Storage를 보유한 컨테이너에서 대량의 데이터를 추출한 것일 수 있습니다.|반출|중간|
|**비정상적인 응용 프로그램이 저장소 계정에 액세스함**|비정상적인 응용 프로그램이 이 저장소 계정에 액세스한 것으로 나타납니다. 잠재적 원인은 공격자가 새 응용 프로그램을 사용 하여 저장소 계정에 액세스한 것일 수 있습니다.|악용|중간|
|**저장소 계정에서 비정상적인 액세스 권한 변경**|이 저장소 컨테이너의 액세스 권한이 비정상적으로 변경된 것으로 나타납니다. 잠재적 원인은 공격자가 보안 태세를 약화하거나 지속성을 얻기 위해 컨테이너 권한을 변경한 것일 수 있습니다.|지속성|중간|
|**저장소 계정에서의 비정상적인 데이터 악용**|이 계정의 최근 활동과 비교할 때 저장소 계정의 Blob 또는 컨테이너가 비정상적인 방식으로 열거된 것으로 나타납니다. 잠재적 원인은 공격자가 향후 공격을 위해 정찰을 수행한 것일 수 있습니다.|컬렉션|중간|
|**저장소 계정에서의 비정상적인 삭제**|이 계정의 최근 활동과 비교할 때 저장소 계정에서 하나 이상의 예기치 않은 삭제 작업이 발생한 것으로 나타납니다. 잠재적 원인은 공격자가 저장소 계정에서 데이터를 삭제한 것일 수 있습니다.|반출|중간|
|**저장소 계정에 비정상적인 .cspkg 업로드**|이 계정의 최근 활동과 비교할 때 Azure Cloud Services 패키지(.cspkg 파일)가 비정상적인 방식으로 저장소 계정에 업로드된 것으로 나타납니다. 잠재적 원인은 공격자가 저장소 계정에서 Azure 클라우드 서비스로 악성 코드를 배포할 준비를 한 것일 수 있습니다.|측면 이동/실행|중간|
|**저장소 계정에 비정상적인 .exe 업로드**|이 계정의 최근 활동과 비교할 때 .exe 파일이 비정상적인 방식으로 저장소 계정에 업로드된 것으로 나타납니다. 잠재적 원인은 공격자가 악성 실행 파일을 저장소 계정에 업로드했거나 합법적인 사용자가 실행 파일을 업로드했기 때문일 수 있습니다.|측면 이동/실행|중간|
|||||


## <a name="alerts-for-azure-cosmos-db-preview"></a><a name="alerts-azurecosmos"></a>Azure Cosmos DB에 대한 경고(미리 보기)

[추가 세부 정보 및 참고 사항](threat-protection.md#cosmos-db)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**비정상적인 위치에서 Cosmos DB 계정으로 액세스**|Azure Cosmos DB 계정에 대한 액세스 패턴이 변경된 것으로 나타납니다. 최근 활동과 비교할 때 익숙하지 않은 IP 주소를 통해 누군가가 이 계정에 액세스했습니다. 공격자가 계정에 대한 액세스 권한을 얻었거나, 합법적인 사용자가 새롭거나 비정상적인 지리적 위치에서 액세스했습니다. 후자의 경우 새 응용 프로그램 또는 개발자의 원격 유지 관리입니다.|악용|중간|
|**계정에서 추출된 비정상적인 데이터 크기**|Azure Cosmos DB 계정에서 데이터 추출 패턴이 변경된 것으로 나타납니다. 최근 활동과 비교할 때 누군가가 비정상적인 양의 데이터를 추출했습니다. 공격자가 Azure Cosmos DB 데이터베이스에서 대량의 데이터를 추출했을 수 있습니다(예: 데이터 반출이나 누출 또는 무단 데이터 전송). 또는 합법적인 사용자 또는 응용 프로그램이 컨테이너에서 비정상적인 양의 데이터를 추출했을 수도 있습니다(예: 유지 관리 백업 활동).|반출|중간|
|||||


## <a name="alerts-for-azure-network-layer"></a><a name="alerts-azurenetlayer"></a>Azure 네트워크 계층에 대한 경고

[추가 세부 정보 및 참고 사항](threat-protection.md#network-layer)
...
[추가 세부 정보 및 참고 사항](threat-protection.md#management-layer)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**익명 IP 주소에서 발생한 활동**|익명 프록시 IP 주소로 식별된 IP 주소에서 사용자 활동이 감지되었습니다.<br>이 프록시는 해당 장치의 IP 주소를 숨기려는 사용자가 사용하며 악의적인 의도로 사용될 수 있습니다. 이 감지는 조직의 다른 사용자가 널리 사용하는 태그가 잘못 지정된 IP 주소와 같은 가양성을 줄이는 기계 학습 알고리즘을 이용합니다.|-|중간|
|**자주 사용되지 않는 국가에서 발생한 활동**|조직의 사용자가 최근에 방문하지 않았거나 방문한 적이 없는 위치에서 활동이 발생했습니다.<br>이 검색은 이전 활동 위치를 고려하여 새 위치 및 드문 위치를 확인합니다. 이상 문제 검색 엔진은 조직의 사용자가 사용하는 이전 위치에 대한 정보를 저장합니다.|-|중간|
|**불가능한 이동 활동**|지리적으로 멀리 떨어진 위치에서 시작하여 두 번의 사용자 활동(단일 또는 여러 세션)이 발생했습니다. 이 문제는 사용자가 첫 번째 위치에서 두 번째 위치로 이동하는 데 걸리는 시간보다 짧은 시간 내에 발생합니다. 이는 다른 사용자가 동일한 자격 증명을 사용하고 있음을 나타냅니다.<br>이 감지는 조직의 다른 사용자가 정기적으로 사용하는 VPN 및 위치와 같은 불가능한 이동 조건에 영향을 주는 확실한 가양성을 무시하는 기계 학습 알고리즘을 이용합니다. 감지에는 7일의 초기 학습 기간이 있으며 이 기간 동안 새 사용자의 활동 패턴을 학습합니다. |-|중간|
...
|**고급 Azure 지속성 기술 사용**|구독 활동 로그를 분석한 결과, 의심스러운 동작이 감지되었습니다. 사용자 지정된 역할에 합법적인 ID 엔터티가 부여되었습니다. 이로 인해 공격자가 Azure 고객 환경에서 지속성을 확보할 수 있습니다.|-||
|||||


## <a name="alerts-for-azure-key-vault-preview"></a><a name="alerts-azurekv"></a>Azure Key Vault에 대한 경고(미리 보기)

[추가 세부 정보 및 참고 사항](threat-protection.md#azure-keyvault)

|경고|Description|의도([자세한 정보](#intentions))|심각도|
|----|----|:----:|--|
|**Tor 종료 노드에서 Key Vault로 액세스**|누군가가 TOR IP 익명화 시스템을 사용해 Key Vault에 액세스하여 자신의 위치를 숨겼습니다. 악의적인 행위자는 인터넷에 연결된 리소스에 무단으로 액세스하려고 할 때 자신의 위치를 숨기려는 경우가 많습니다.|-|중간|
|**Key Vault에서의 대용량 작업**|이전 데이터와 비교하여 대용량 Key Vault 작업이 수행되었습니다. 일반적으로 Key Vault 활동은 시간이 지나도 동일합니다. 이는 활동의 합법적인 변경일 수도 있습니다. 또는 인프라가 손상되어 추가 조사가 필요할 수도 있습니다.|-|중간|
|**Key Vault에서의 의심스러운 정책 변경 및 비밀 쿼리**|Key Vault 정책이 변경된 후 비밀을 나열하거나 가져오는 작업이 발생했습니다. 또한 이 작업 패턴은 일반적으로 이 자격 증명 모음의 사용자가 수행하지 않습니다. 이는 Key Vault가 손상되었으며 악의적인 행위자가 비밀을 유출했을 가능성이 높음을 암시합니다.|-|중간|
|**Key Vault에서의 의심스러운 비밀 목록 및 쿼리**|비밀 목록 작업에 많은 비밀 가져오기 작업이 이어졌습니다. 또한 이 작업 패턴은 일반적으로 이 자격 증명 모음의 사용자가 수행하지 않습니다. 이는 잠재적으로 악의적인 목적을 가지고 누군가가 Key Vault에 저장된 비밀을 덤프했을 수 있음을 나타냅니다.|-|중간|
|**비정상적인 응용 프로그램이 Key Vault에 액세스함**|일반적으로 액세스하지 않는 응용 프로그램에서 Key Vault에 액세스했습니다. 이는 합법적인 액세스 시도일 수도 있습니다(예: 인프라 또는 코드 업데이트 후). 인프라가 손상되었고 악의적인 행위자가 Key Vault에 액세스하려고 시도하는 것을 암시할 수도 있습니다.|-|중간|
|**Key Vault에서의 비정상적인 작업 패턴**|이전 데이터와 비교하여 비정상적인 Key Vault 작업이 수행되었습니다. 일반적으로 Key Vault 활동은 시간이 지나도 동일합니다. 이는 활동의 합법적인 변경일 수도 있습니다. 또는 인프라가 손상되어 추가 조사가 필요할 수도 있습니다.|-|중간|
|**비정상적인 사용자가 Key Vault에 액세스함**|일반적으로 액세스하지 않는 사용자가 Key Vault에 액세스했습니다. 이는 합법적인 액세스 시도일 수도 있습니다(예: 액세스가 필요한 새 사용자가 조직에 가입한 경우). 인프라가 손상되었고 악의적인 행위자가 Key Vault에 액세스하려고 시도하는 것을 암시할 수도 있습니다.|-|중간|
|**비정상적인 사용자-응용 프로그램 쌍이 Key Vault에 액세스함**|일반적으로 액세스하지 않는 사용자-응용 프로그램 쌍이 Key Vault에 액세스했습니다. 이는 합법적인 액세스 시도일 수도 있습니다(예: 인프라 또는 코드 업데이트 후). 인프라가 손상되었고 악의적인 행위자가 Key Vault에 액세스하려고 시도하는 것을 암시할 수도 있습니다.|-|중간|
|**사용자가 대용량의 Key Vault에 액세스함**|이전 데이터와 비교하여 사용자 또는 응용 프로그램이 액세스하는 자격 증명 모음의 수가 변경되었습니다. 일반적으로 Key Vault 활동은 시간이 지나도 동일합니다. 이는 활동의 합법적인 변경일 수도 있습니다. 또는 인프라가 손상되어 추가 조사가 필요할 수도 있습니다.|-|중간|
|||||


## <a name="alerts-for-azure-ddos-protection"></a><a name="alerts-azureddos"></a>Azure DDoS Protection에 대한 경고

[추가 세부 정보 및 참고 사항](threat-protection.md#azure-ddos)
