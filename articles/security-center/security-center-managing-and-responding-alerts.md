...
-   **정보**: 보안 인시던트로 드릴다운하거나 특정 경고 ID가 있는 REST API를 사용하는 경우 정보 경고만 표시됩니다. 일반적으로 한 인시던트는 여러 경고로 구성되며, 그 중 일부는 오직 정보 제공만을 위해 단독으로 표시될 수 있지만 다른 경고의 컨텍스트에서는 자세히 살펴볼 필요가 있습니다.  

> [!NOTE]
> **2015-06-01-preview** API 버전을 사용 하는 경우 위에 나열 된 것부터 어떤 시나리오에도 경보 심각도 유형이 적용 되는 차이점이 있습니다.  
...
**설명** 필드에이 이벤트에 대 한 자세한 정보를 찾을 수 있습니다. 이러한 추가 세부 사항은 수정 방법에 대한 원본 IP 주소 및 권장사항을 적용할 수 있는 경우 보안 경고인 대상 리소스를 트리거한 항목에 대한 정보를 제공합니다.  일부 경우에 일부 Windows 보안 이벤트 로그가 IP 주소를 포함하지 않기 때문에 원본 IP 주소가 비어 있습니다(사용할 수 없음).

Security Center에서 제안하는 수정은 보안 경고에 따라 다릅니다. 경우에 따라 권장된 수정을 구현하기 위해 기타 Azure 기능을 사용해야 할 수 있습니다. 예를 들어이 공격의 해결 방법은 [네트워크 ACL](../virtual-network/virtual-networks-acl.md) 또는 [네트워크 보안 그룹](../virtual-network/security-overview.md#security-rules) 규칙을 사용 하 여이 공격을 생성 하는 IP 주소를 허용 하지 않는 것입니다. 여러 유형의 경고에 대 한 자세한 내용은 [보안 경고 유형](security-center-alerts-overview.md#security-alert-types)을 참조 하세요.
