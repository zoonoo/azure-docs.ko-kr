...
- **네트워크 데이터에 대한 무차별 암호 대입(brute force) 검색**: 응용 프로그램에 대한 일반적인 네트워크 트래픽 패턴을 이해하는 기계 학습 모델을 사용하며 이는 합법적인 사용자가 아닌 위험 인물이 실행하는 액세스 시도를 보다 효과적으로 검색할 수 있습니다.
...
이 경우 트리거된 경고는 의심스러운 RDP(원격 데스크톱 프로토콜) 활동을 참조합니다. 첫 번째 열은 공격 받은 리소스, 두 번째 열은 공격을 검색한 시간, 세 번째 열은 경고 상태, 네 번째 열은 공격의 심각도를 표시합니다. 이 정보를 검토한 후 공격을 받은 리소스를 클릭하면 새 블레이드가 열립니다.
...
**잠재적인 SQL 삽입** SQL 삽입은 구문 분석 및 실행을 위해 나중에 SQL Server의 인스턴스로 전달된 문자열에 악성 코드를 삽입한 공격입니다. SQL Server가 수신하는 모든 구문상 유효한 쿼리를 실행하기 때문에 SQL 문을 생성하는 모든 프로시저에 삽입 취약성이 있는지 검토해야 합니다.
...
![악의적인 컴퓨터와 네트워크 통신 경고](./media/security-center-managing-and-responding-alerts/security-center-managing-and-responding-alerts-fig-11.png)
