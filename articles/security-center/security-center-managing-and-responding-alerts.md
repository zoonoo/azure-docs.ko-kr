...
-   **정보 제공 용 이므로**: 보안 인시던트로 드릴다운하거나 특정 경고 ID가 있는 REST API를 사용하는 경우 정보 경고만 표시됩니다. 일반적으로 한 인시던트는 여러 경고로 구성되며, 그 중 일부는 오직 정보 제공만을 위해 단독으로 표시될 수 있지만 다른 경고의 컨텍스트에서는 자세히 살펴볼 필요가 있습니다.  

> [!NOTE]
> 사용 중인 경우는 **2015-06-01-미리 보기** API 버전 차이가 경보 심각도 형식에 적용 되는 시나리오에서 위에 나열 된 다음입니다.  
