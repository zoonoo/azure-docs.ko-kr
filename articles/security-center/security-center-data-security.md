...
Azure에서 Security Center를 사용하는 경우 각 Azure 구독에 대해 데이터 수집이 활성화됩니다. 또한 Azure Security Center의 보안 정책 섹션에서 구독에 대한 데이터 수집을 설정할 수 있습니다. 데이터 수집이 설정 되 면 지원 되는 모든 기존 Azure 가상 컴퓨터 및 새로 만든 가상 컴퓨터에서 Log Analytics 에이전트를 프로 비전 할 Azure Security Center.
Log Analytics 에이전트는 다양 한 보안 관련 구성 및 이벤트를 [ETW(Windows용 이벤트 추적)](https://msdn.microsoft.com/library/windows/desktop/bb968803.aspx) (ETW) 추적으로 검색 합니다. 또한 운영 체제는 컴퓨터를 실행하는 동안 이벤트 로그 이벤트를 발생시킵니다. 이러한 데이터의 예: 운영 체제 유형 및 버전, 운영 체제 로그(Windows 이벤트 로그), 프로세스 실행, 컴퓨터 이름, IP 주소, 로그인된 사용자 및 테넌트 ID입니다. Log Analytics 에이전트는 이벤트 로그 항목과 ETW 추적을 읽고 분석을 위해 작업 영역에 복사 합니다. 또한 Log Analytics 에이전트는 크래시 덤프 파일을 작업 영역에 복사 하 고, 프로세스 생성 이벤트를 사용 하도록 설정 하 고, 명령줄 감사를 사용 하도록 설정 합니다.
