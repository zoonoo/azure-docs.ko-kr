...
보안 센터는 자동으로 연속 검사를 실행하여 Azure 리소스의 보안 상태를 분석합니다. Security Center는 잠재적 보안 취약성이 확인되면 필요한 보안 컨트롤을 구성하는 과정을 안내하는 권장 사항을 만듭니다. 보안 센터는 24시간 이내에 권장 사항을 업데이트하지만 다음과 같은 예외는 없습니다.

- 운영 체제 보안 구성 권장 사항이 48시간 이내에 업데이트됩니다.
- 엔드포인트 보호 문제 권장 사항이 8시간 이내에 업데이트됩니다.
...
David는 모든 구독에서 Azure Security 표준 계층을 실행하여 사용 가능한 권장 사항 및 보안 기능의 전체 제품군을 가져옵니다. 또한 Jeff는 아직 클라우드로 마이그레이션되지 않은 모든 기존 온-프레미스 서버를 온보더업하여 [Windows](quick-onboard-windows-computer.md) 및 [Linux](quick-onboard-linux-computer.md) 서버에서 보안 센터의 하이브리드 지원을 활용할 수 있도록 합니다.
...
- 보안 권장 사항을 평가하고 권장 사항을 적용하거나 해제할지 여부를 결정합니다.
- 보안 권장 사항 적용

### <a name="remediate-threats-using-recommendations"></a>권장 사항을 사용하여 위협 해결
일상적인 모니터링 활동의 일환으로 Jeff는 Azure에 서명하고 보안 센터를 엽니다. 

1. Jeff는 워크로드의 구독을 선택합니다.

2. Jeff는 **보안 점수를** 확인하여 구독의 보안 에 대한 전반적인 그림을 얻고 점수가 548임을 확인합니다.

3. Jeff는 먼저 처리할 권장 사항을 결정해야 합니다. 따라서 Jeff는 보안 점수를 클릭하고 [보안 점수 영향을](security-center-secure-score.md)얼마나 향상시키는지에 따라 권장 사항을 처리하기 시작합니다.

4. Jeff에 많은 VM 및 서버에 연결되어 있으므로 **Compute 및 앱**에 집중하기로 결정했습니다.

5. Jeff가 **계산 및 앱을**클릭하면 권장 사항 목록이 표시되고 보안 점수 영향에 따라 처리됩니다.

6. Jeff는 VM을 향한 수많은 인터넷을 가지고 있으며 포트가 노출되어 있기 때문에 공격자가 서버를 제어할 수 있다고 걱정합니다. 따라서 Jeff는 [**적시 VM 액세스를**](security-center-just-in-time.md)사용하도록 선택합니다.

Jeff는 계속해서 높은 우선 순위 및 중간 우선 순위 권장 사항을 살펴보면서 구현할지 결정했습니다. 각 권장 사항에 대해 Jeff는 보안 센터에서 제공하는 자세한 정보를 살펴보고 어떤 리소스가 영향을 받는지, 보안 점수에 미치는 영향, 각 권장 사항이 의미하는 것 및 각 문제를 완화하는 방법에 대한 수정 단계를 파악합니다.

## <a name="conclusion"></a>결론
Security Center에서 권장 사항을 모니터링하면 공격이 발생하기 전에 보안 취약점을 제거할 수 있습니다. 권장 사항을 수정하면 보안 점수와 워크로드의 보안 태세가 개선됩니다. Security Center는 사용자가 배포한 새 리소스를 자동으로 검색하고, 보안 정책에 따라 평가하고, 보안을 유지하기 위한 새 권장 사항을 제공합니다.
...
보안 경고 관리 및 응답을 통해 위협에 대응하는 방법을 [알아봅니다.](security-center-managing-and-responding-alerts.md)
