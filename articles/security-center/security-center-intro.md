---
title: Azure 보안 센터 소개 | Microsoft Docs
...
| 회사의 보안 요구 사항 및 사용할 응용 프로그램 형식 및 데이터의 민감도에 따라 Azure 구독 및 리소스 그룹에 대한 정책 정의 | |
...
| Microsoft 제품과 서비스, Microsoft DCU(Digital Crimes Unit), MSRC(Microsoft 보안 대응 센터) 및 외부 피드에서 글로벌 위협 인텔리전스 이용 | |
...
* 보안 모니터링 및 권장 사항을 위해 지원되는 가상 컴퓨터를 매일 검색합니다.
* 분석 및 위협 감지를 위해 보안 이벤트를 수집합니다.

**지역별 저장소 계정 선택**(위 참조)을 사용하면 가상 컴퓨터를 실행 중인 각 영역에 대해 가상 컴퓨터에서 수집한 데이터를 저장하는 저장소 계정을 선택할 수 있습니다. 각 지역에 대한 저장소 계정을 선택하지 않으면 사용자를 위해 계정이 생성됩니다. 수집된 데이터는 보안상의 이유로 다른 고객의 데이터와 논리적으로 격리됩니다.
...
* 고유로 정의하면 리소스 그룹에 사용자 지정 보안 정책이 설정됩니다. **권장 사항 표시**에서 변경해야 합니다.
