<properties
   pageTitle="Azure 보안 센터에서 VM 에이전트 사용 | Microsoft Azure"
...
1. **권장 사항 블레이드**에서 **VM 에이전트 활성화**를 선택합니다. ![VM 에이전트 사용][1]

2. 이렇게 하면 **VM 에이전트가 없거나 응답하지 않습니다.** 블레이드가 열립니다. 이 블레이드에 VM 에이전트가 필요한 VM을 나열합니다. VM 에이전트를 설치하려면 블레이드의 지침을 따릅니다. ![VM 에이전트가 없습니다][2]
