...
* [Get mpcomputerstatus](https://docs.microsoft.com/powershell/module/defender/get-mpcomputerstatus?view=win10-ps) 를 실행 하 고 결과 **를 amserviceenabled로 설정 하면 **"virtual machine에 endpoint protection 솔루션 설치"** 권장 사항이 생성 됩니다. 허위**

* **"컴퓨터에서 endpoint protection 상태 문제 해결"** 권장 사항은 [Get mpcomputerstatus](https://docs.microsoft.com/powershell/module/defender/get-mpcomputerstatus?view=win10-ps) 를 실행 하 고 다음 중 하나 또는 둘 모두를 수행 하는 경우에 생성 됩니다.
  ...
  * 다음 속성 중 하나 또는 둘 다가 7 보다 크거나 같은 경우
...
* **SCEPMpModule ("$env:P Rogramfiles\microsoft Security Client\MpProvider\MpProvider.psd1")** 를 가져오고 실행 **하는 경우 "가상 컴퓨터에 endpoint protection 솔루션 설치" 권장 사항이 생성 됩니다.**  **Amserviceenabled = False** 인 MProtComputerStatus 결과

* **"사용자 컴퓨터에서 endpoint protection 상태 문제 해결"** 권장 사항은 **MprotComputerStatus** 를 실행 하 고 다음 중 하나 또는 둘 모두를 수행 하는 경우에 생성 됩니다.
...
* 다음 검사 중 하나 이상이 충족 되지 않으면 **"가상 머신에 endpoint protection 솔루션 설치"** 권장 사항이 생성 됩니다.
...
다음 검사 중 하나라도 충족 되지 않으면 **"가상 머신에 endpoint protection 솔루션 설치"** 권장 사항이 생성 됩니다.
...
다음 검사 중 하나라도 충족 되지 않으면 **"컴퓨터에서 endpoint protection 상태 문제 해결"** 권장 사항이 생성 됩니다.  
...
다음 검사가 충족 되지 않으면 **"가상 머신에 endpoint protection 솔루션 설치"** 권장 사항이 생성 됩니다.

* **HKLM:\SOFTWARE\McAfee\Endpoint\AV\ProductVersion** exists

* **HKLM:\SOFTWARE\McAfee\AVSolution\MCSHIELDGLOBAL\GLOBAL\enableoas = 1**

다음 검사가 충족 되지 않으면 **"컴퓨터에서 endpoint protection 상태 문제 해결"** 권장 사항이 생성 됩니다.
...
다음 검사 중 하나 또는 둘 다가 충족 되지 않는 경우 **virtual machine에 대 한 endpoint protection 솔루션 설치** 권장 사항이 생성 됩니다.  
...
다음 검사 중 하나 이상이 충족 되지 않으면 **컴퓨터에서 endpoint protection 상태 문제 해결** 권장 사항이 생성 됩니다.
