...
| Azure VM 에이전트가 누락되었거나 잘못됨 | Microsoft Monitoring Agent가 아직 설치되지 않았습니다.  Security Center에서 확장을 설치하려면 유효한 Azure VM 에이전트가 필요합니다. | Azure VM 에이전트를 VM에 설치, 다시 설치 또는 업그레이드합니다. |
