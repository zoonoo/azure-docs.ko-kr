...
- Endpoint Protection 상태:
   ...
   > [!NOTE]
   > Security Center와 통합된 엔드포인트 보호 보안 솔루션 목록은 [보안 솔루션 통합](security-center-partner-integration.md#integrated-azure-security-solutions)을 참조하세요.
   >
   >
...
1. Security Center 주 메뉴 또는 **개요** 아래에서 **계산**을 선택합니다.
...
2. **계산** 아래에서 **Endpoint Protection 문제**를 선택합니다. **Endpoint Protection 문제** 대시보드가 열립니다.
   ...
   - 공격받은 컴퓨터 - Security Center에서 맬웨어에 의한 공격을 보고하는 VM 및 컴퓨터의 수가 표시됩니다.
...
2. **비 Azure 컴퓨터에 Endpoint Protection이 설치되지 않음**에서 작업 영역을 선택합니다. 해당 작업 영역을 표시하도록 필터링된 Log Analytics 검색 쿼리가 열리고 맬웨어 방지 솔루션이 누락된 컴퓨터가 나열됩니다. 목록에서 컴퓨터를 선택하면 자세한 내용을 확인할 수 있습니다.
