...
Id 활동을 모니터링 하 여 인시던트가 발생 하기 전이나 사후 조치를 통해 공격 시도를 중단할 수 있습니다. 예를 들어 Security Center은 더 이상 사용 되지 않는 계정 (더 이상 필요 하지 않으며 Azure Active Directory에서 로그인이 차단 됨)을 제거 하기 위해 플래그를 지정 합니다. 

Azure Security Center의 **id 및 액세스** 리소스 보안 섹션에서 볼 수 있는 권장 사항의 예는 다음과 같습니다.
...
이러한 권장 사항 및 여기에 표시 될 수 있는 권장 사항의 전체 목록은 이러한 권장 사항에 대 한 자세한 내용은 [id 및 액세스 권장 사항](recommendations-reference.md#recs-identity)을 참조 하세요.
