...
|----|----|----|----|
|구독|50|구독 소유자 권한이 있는 계정에서 MFA는 사용할 수 있어야|계정 또는 리소스 위반을 방지하려면 관리자 권한이 있는 모든 구독 계정에 대해 MFA(Multi-factor Authentication)를 사용합니다.|
|구독|40|쓰기 권한이 있는 사용자 구독 계정에서 MFA는 사용할 수 있어야|계정 또는 리소스 위반을 방지하려면 쓰기 권한이 있는 모든 구독 계정에 대해 MFA(Multi-factor Authentication)를 사용합니다.|
|구독|30|소유자 권한이 있는 외부 계정은 구독에서 제거 해야|모니터링되지 않는 액세스를 방지하려면 소유자 권한이 있는 외부 계정을 구독에서 제거합니다.|
|구독|30|읽기 권한이 있는 사용자 구독 계정에서 MFA는 사용할 수 있어야|계정 또는 리소스 위반을 방지하려면 읽기 권한이 있는 모든 구독 계정에 대해 MFA(Multi-factor Authentication)를 사용합니다.|
|구독|25|구독에서 제거 해야 하는 사용 권한을 쓰기를 사용 하 여 외부 계정|모니터링되지 않는 액세스를 방지하려면 쓰기 권한이 있는 외부 계정을 구독에서 제거합니다. |
|구독|20|소유자 권한이 있는 사용 되지 않는 계정은 구독에서 제거 해야|구독에서 소유자 권한이 있는 사용되지 않는 계정을 제거합니다.|
|구독|5|사용 되지 않는 계정은 구독에서 제거 해야|현재 사용자에 대해서만 액세스할 수 있게 구독에서 더 이상 사용되지 않는 계정을 제거합니다. |
|구독|5|구독에 할당 하는 둘 이상의 소유자 해야|관리자 액세스 중복성을 유지하려면 둘 이상의 구독 소유자를 지정합니다.|
|구독|5|최대 3 소유자는 구독에 대해 지정 되어야 합니다.|보안이 침해된 소유자의 위반 가능성을 줄이려면 최소 3명의 구독 소유자를 지정합니다.|
|주요 자격 증명 모음|5|Key Vault에 진단 로그를 사용 해야|로그를 사용하도록 설정하고 최대 1년 간 보존합니다. 이렇게 하면 보안 인시던트가 발생하거나 네트워크가 손상된 경우 조사 목적으로 활동 내역을 다시 만들 수 있습니다. |
|구독|15|읽기 권한이 있는 외부 계정은 구독에서 제거 해야|모니터링되지 않는 액세스를 방지하려면 읽기 권한이 있는 외부 계정을 구독에서 제거합니다.| 

> [!NOTE]
> MFA 필요 하지만 설정 제외 하는 조건부 액세스 정책을 만든 경우 보안 센터 MFA 권장 사항 평가 고려 정책 비준수 일부 사용자가 MFA 하지 않고 Azure에 로그인 할 수 있기 때문에 합니다.
