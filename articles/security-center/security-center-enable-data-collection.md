...
자동 프로비저닝을 On으로 설정 하면 지원 되는 모든 Azure Vm 및 새로 생성 된 Vm에 대 한 Log Analytics 에이전트가 프로 비전 Security Center. 자동 프로비저닝을 사용하는 것이 좋지만 수동 에이전트 설치도 사용할 수 있습니다. [Log Analytics 에이전트 확장을 설치 하는 방법을 알아봅니다](#manualagent).
...
> - 수동 프로 비전에 대 한 지침은 [수동으로 Log Analytics 에이전트 확장 설치](#manualagent)를 참조 하세요.
...
- **없음** – 보안 이벤트 스토리지를 사용하지 않도록 설정합니다. 이것이 기본 설정입니다.
