...
예를 들어 개발 또는 테스트에 사용되는 리소스는 프로덕션 응용 프로그램에 사용되는 해당 리소스와 보안 요구 사항이 다릅니다. 마찬가지로 PII(Personally Identifiable Information) 같은 규제된 데이터를 가진 응용 프로그램에 대해서는 더 높은 수준의 보안이 필요할 수 있습니다. Azure 보안 센터에서 활성화한 보안 정책에 따라 잠재적 취약점을 파악하고 위협을 완화하는 데 도움이 되는 보안 권장 사항과 모니터링이 결정됩니다. 더 적절한 옵션을 결정하는 방법에 대한 자세한 내용은 [Azure 보안 센터 계획 및 운영 가이드](security-center-planning-and-operations-guide.md)를 참고하세요.

## 구독에 대한 보안 정책 설정

보안 정책은 각 구독 또는 리소스 그룹에 대해 구성됩니다. 보안 정책을 수정하려면 해당 구독의 소유자 또는 참여자여야 합니다. Azure Portal에 액세스하고 다음 단계에 따라 보안 센터에서 보안 정책을 구성합니다.
...
3. 해당 구독에 대한 **보안 정책** 블레이드가 열리고 다음 화면과 비슷한 일련의 옵션이 표시됩니다.
...
6. 이 경고가 표시되면 이 옵션을 클릭하고 다음 화면에서 보여준 대로 지역을 선택합니다.
...
9. **보안 정책** 블레이드에서 **켜기**를 클릭하고 이 구독에 사용할 보안 권장 사항을 활성화합니다. **방지 정책** 옵션을 클릭합니다. **보안 정책** 블레이드가 다음 화면에서 보여준 대로 열립니다.

	![보안 정책 선택](./media/security-center-policies/security-center-policies-fig4-ga.png)

다음 테이블을 참조로 사용하여 각 옵션의 역할에 대해 이해합니다.

| 정책 | 상태가 켜진 경우 |
|----- |-----|
| 시스템 업데이트 | 해당 가상 컴퓨터에 대해 구성된 서비스에 따라 매일 Windows 업데이트 또는 WSUS에서 사용 가능한 보안 및 중요 업데이트를 검색하여 누락된 업데이트를 적용하도록 권장합니다. 패키지 업데이트를 사용할 수 있는지를 확인하려면 패키지 관리 시스템을 제공하는 배포판을 사용하여 Linux 시스템에서 최신 업데이트를 확인합니다. 또한 [클라우드 서비스](./cloud-services/cloud-services-how-to-configure.md) 가상 컴퓨터에서 보안 및 중요 업데이트를 확인합니다. |
...
옵션을 모두 구성하여 완료되면 권장 사항이 있는 **보안 정책** 블레이드에서 **확인**을 클릭하고 초기 설정이 있는 **보안 정책** 블레이드에서 **저장**을 클릭합니다.
