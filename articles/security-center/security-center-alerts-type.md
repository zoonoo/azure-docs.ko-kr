...
### <a name="code-injection-discovered"></a>코드 삽입 발견
코드 삽입은 실행 중인 프로세스 또는 스레드에 실행 파일 모듈을 삽입하는 작업입니다.  이 방법은 데이터에 액세스하거나 해당 데이터의 삭제를 숨기거나 제거되지 않도록 방지하는 맬웨어에서 사용합니다. 이 경고는 삽입된 모듈이 크래시 덤프에 있는지를 알려줍니다. 합법적인 소프트웨어 개발자는 상황에 따라 기존 응용 프로그램이나 운영 체제 구성 요소를 수정하거나 확장하는 등 악의적이지 않은 이유로 코드 삽입을 수행합니다.  악의적인 모듈과 악의적이지 않은 삽입 모듈을 구분하기 위해 Security Center는 삽입 모듈이 의심스러운 동작의 프로필을 준수하는지 여부를 확인합니다. 이 검사의 결과는 경고의 "서명" 필드에 의해 나타나고 경고, 경고 설명 및 경고 문제 해결 단계의 심각도에 반영됩니다. 

이 경고는 다음과 같은 추가 필드를 제공합니다.

- ADDRESS: 삽입 모듈의 메모리에 있는 위치입니다.
- IMAGENAME: 삽입 모듈의 이름입니다. 이미지 이름이 이미지 내에 제공되지 않은 경우 비워 둘 수 있습니다.
- SIGNATURE: 삽입 모듈이 의심스러운 동작의 프로필을 준수하는지 나타냅니다. 

다음 표에서는 결과 및 해당 설명의 예제를 보여줍니다.

| 서명 값                      | 설명                                                                                                       |
|--------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| 의심스러운 반사 로더 악용 | 이 의심스러운 동작은 종종 운영 체제 로더와 독립적으로 삽입 코드를 로드하는 것과 상관 관계가 있습니다. |
| 의심스러운 삽입 공격          | 메모리에 코드를 삽입하는 것과 종종 상관 관계가 있는 악의적 동작을 의미합니다.                                       |
| 의심스러운 삽입 공격         | 메모리에서 삽입 코드의 사용과 종종 상관 관계가 있는 악의적 동작을 의미합니다.                                   |
| 의심스러운 삽입 디버거 공격 | 종종 디버거의 탐지나 회피와 상관 관계가 있는 악의적 동작을 의미합니다.                         |
| 의심스러운 원격 삽입 공격   | 종종 C2(명령 및 제어) 시나리오와 상관 관계가 있는 악의적 동작을 의미합니다.                                 |

이러한 유형의 경고 예제는 다음과 같습니다.

![코드 삽입 경고](./media/security-center-alerts-type/security-center-alerts-type-fig21.png)

### <a name="suspicious-code-segment"></a>의심스러운 코드 세그먼트
의심스러운 코드 세그먼트는 코드 세그먼트가 반사 삽입 및 프로세스 공백에서 사용되는 등 비표준 메서드를 사용하여 할당되는 것을 나타냅니다.  또한 이 경고는 코드 세그먼트의 추가 특징을 처리하여 보고된 코드 세그먼트의 기능 및 동작에 대한 컨텍스트를 제공합니다.

이 경고는 다음과 같은 추가 필드를 제공합니다.

- ADDRESS: 삽입 모듈의 메모리에 있는 위치입니다.
- SIZE: 의심스러운 코드 세그먼트의 크기
- STRINGSIGNATURES: 이 필드는 함수 이름이 코드 세그먼트 내에 포함된 API의 기능을 나열합니다. 예제 기능은 다음을 포함합니다.
    - 이미지 섹션 설명자, x64용 동적 코드 실행, 메모리 할당 및 로더 기능, 원격 코드 주입 용량, 하이재킹 컨트롤 기능, 환경 변수 읽기, 임의 프로세스 메모리 읽기, 토큰 권한 쿼리 또는 수정, HTTP/HTTPS 네트워크 통신 및 네트워크 소켓 통신
- IMAGEDETECTED: 이 필드는 PE 이미지가 의심스러운 코드 세그먼트를 감지한 프로세스 및 삽입 모듈이 시작된 주소에 삽입되었는지를 나타냅니다.
- SHELLCODE:이 필드는 중요한 추가 보안 운영 체제 기능에 대한 액세스 권한을 얻기 위해 악의적인 페이로드에서 일반적으로 사용하는 동작의 존재를 나타냅니다. 

이러한 유형의 경고 예제는 다음과 같습니다.

![의심스러운 코드 세그먼트 경고](./media/security-center-alerts-type/security-center-alerts-type-fig22.png)
