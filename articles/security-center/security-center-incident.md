<properties
   pageTitle="Azure 보안 센터에서 보안 인시던트 처리 | Microsoft Azure"
...
2.  이 타일을 클릭하여 확장하고 보안 인시던트가 감지되면 이 이미지에 표시된 대로 보안 경고 그래프에 표시됩니다.
...
4. 	**보안 인시던트가 발견된** 블레이드에서 이 보안 인시던트에 대한 세부 사항을 확인합니다. 여기에 인시던트의 전체 설명, 심각도(이 경우에 높음), 인시던트의 현재 상태(이 경우에 *활성화*), 공격을 받은 리소스(이 경우에 *VM1*), 재구성 단계를 포함하며 아래 쪽에는 이 인시던트에 포함된 경고가 있습니다. 각 경고에 대한 자세한 정보를 가져오는 경우 클릭하면 아래와 같이 다른 블레이드가 열립니다.
