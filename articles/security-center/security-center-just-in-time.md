...
보안 센터의 표준 가격 [책정 계층(가격 책정](/azure/security-center/security-center-pricing)참조)에 있는 경우 JIT(적시) 가상 시스템(VM) 액세스를 통해 Azure VM에 대한 인바운드 트래픽을 잠글 수 있습니다. 따라서 공격에 대한 노출이 줄어들고 필요할 때 VM에 쉽게 연결할 수 있습니다.
...
- [Azure 보안 센터에서 JIT 액세스 구성](#jit-asc)
- [Azure VM 페이지에서 JIT 액세스 구성](#jit-vm)
- [VM에서 프로그래밍 방식으로 JIT 정책 구성](#jit-program)

## <a name="configure-jit-in-azure-security-center"></a>Azure 보안 센터에서 JIT 구성

보안 센터에서 JIT 정책을 구성하고 JIT 정책을 사용하여 VM에 대한 액세스를 요청할 수 있습니다.

### <a name="configure-jit-access-on-a-vm-in-security-center"></a>보안 센터의 VM에서 JIT 액세스 구성<a name="jit-asc"></a>
...
1. 왼쪽 창에서 **Just-In-Time VM 액세스**를 선택합니다.
    ...
    **적시 VM 액세스** 창이 열리고 VM 상태에 대한 정보가 표시됩니다.

    - **구성됨** - Just-In-Time VM 액세스를 지원하도록 구성된 VM입니다. 표시된 데이터는 지난 주에 대한 데이터이며 각 VM에 대해 승인된 요청 수, 마지막 액세스 날짜 및 시간, 마지막 사용자를 포함합니다.
    - **권장** - 적시에 VM 액세스를 지원할 수 있지만 구성되지 않은 VM입니다. 이러한 VM에는 Just-In-Time VM 액세스 제어를 사용하도록 설정하는 것이 좋습니다.
    - **권장 사항 없음** - VM이 권장되지 않을 수 있는 이유입니다.
      ...
      - 기타 - 적시 솔루션이 구독 또는 리소스 그룹의 보안 정책에서 꺼져 있거나 VM에 공용 IP가 없고 NSG가 없는 경우 VM이 이 범주에 속합니다.

1. **권장** 탭을 선택합니다.

1. **가상 머신에서**활성화할 VM을 클릭합니다. 그러면 VM 옆에 있는 확인 표시가 선택됩니다.

      ![Just-In-Time VM 액세스 사용](./media/security-center-just-in-time/enable-just-in-time.png)

1. **VM에서 JIT 활성화를**클릭합니다. Azure 보안 센터에서 권장하는 기본 포트를 표시하는 창이 열립니다.
    - 22 - SSH
    - 3389 - RDP
    - 5985 - WinRM 
    - 5986 - WinRM
1. 선택적으로 목록에 사용자 지정 포트를 추가할 수 있습니다.

      1. **추가**를 클릭합니다. **포트 구성 추가** 창이 열립니다.
      1. 구성하도록 선택한 각 포트에 대해 기본 설정과 사용자 지정 을 모두 사용하여 다음 설정을 사용자 지정할 수 있습니다.
...
> [!NOTE]
>VM에 대해 JIT VM Access를 사용하도록 설정하면 Azure Security Center는 연결된 네트워크 보안 그룹 및 Azure 방화벽에서 선택한 포트에 대해 "모든 인바운드 트래픽 거부" 규칙을 만듭니다. 선택한 포트에 대해 다른 규칙이 생성된 경우 기존 규칙이 새 "모든 인바운드 트래픽 거부" 규칙보다 우선합니다. 선택한 포트에 기존 규칙이 없는 경우 새 "모든 인바운드 트래픽 거부" 규칙이 네트워크 보안 그룹 및 Azure 방화벽에서 최우선 순위입니다.


## <a name="request-jit-access-via-security-center"></a>보안 센터를 통해 JIT 액세스 요청

보안 센터를 통해 VM에 대한 액세스를 요청하려면 다음 을 수행하십시오.

1. **Just-In-Time VM 액세스**에서 **구성됨** 탭을 선택합니다.

1. **가상 컴퓨터에서**액세스를 요청하려는 VM을 클릭합니다. 이렇게 하면 VM 옆에 체크 표시가 있습니다.

    - 연결 세부 **정보** 열의 아이콘은 JIT가 NSG 또는 FW에서 활성화되어 있는지 여부를 나타냅니다. 둘 다에서 활성화된 경우 방화벽 아이콘만 나타납니다.

    - **연결 세부 정보** 열은 VM 및 해당 열린 포트를 연결하는 데 필요한 정보를 제공합니다.

      ![Just-In-Time 액세스 요청](./media/security-center-just-in-time/request-just-in-time-access.png)

1. 액세스 요청 을 **클릭합니다.** **액세스 요청** 창이 열립니다.

      ![JIT 세부 정보](./media/security-center-just-in-time/just-in-time-details.png)

1. **액세스 요청** 아래에서 각 VM에 대해 열려는 포트, 포트가 열려 있는 원본 IP 주소 및 포트를 열어 둘 기간을 구성합니다. 적시 정책에 구성된 포트에 대한 액세스만 요청할 수 있습니다. 각 포트에는 Just-In-Time 정책에서 파생된 최대 허용된 시간이 있습니다.
...
## <a name="edit-a-jit-access-policy-via-security-center"></a>보안 센터를 통해 JIT 액세스 정책 편집
...
## <a name="audit-jit-access-activity-in-security-center"></a>보안 센터에서 JIT 액세스 활동 감사

로그 검색을 사용하여 VM 활동에 대한 정보를 얻을 수 있습니다. 로그를 보려면

1. **Just-In-Time VM 액세스**에서 **구성됨** 탭을 선택합니다.
2. **VM에서**해당 VM의 행 내의 세 점을 클릭하여 정보를 보고 메뉴에서 **활동 로그를** 선택하여 VM을 선택합니다. **활동 로그가** 열립니다.
...
필터를 수정하고 **적용을** 클릭하여 검색 및 로그를 만듭니다.



## <a name="configure-jit-access-from-an-azure-vms-page"></a>Azure VM 페이지에서 JIT 액세스 구성<a name="jit-vm"></a>

편의를 위해 보안 센터의 VM 페이지에서 JIT를 사용하여 VM에 직접 연결할 수 있습니다.

### <a name="configure-jit-access-on-a-vm-via-the-azure-vm-page"></a>Azure VM 페이지를 통해 VM에서 JIT 액세스 구성

VM에서 Just-In-Time 액세스를 쉽게 롤아웃할 수 있도록 하려면 VM 내에서 직접 Just-In-Time 액세스만 허용하도록 VM을 설정하면 됩니다.

1. Azure [포털에서](https://ms.portal.azure.com) **가상 컴퓨터를**검색하고 선택합니다. 
2. 적시 액세스로 제한할 가상 컴퓨터를 선택합니다.
3. 메뉴에서 **구성**을 선택합니다.
4. **적시 액세스에서** **적시 사용 (를)을 선택합니다.** 
...
### <a name="request-jit-access-to-a-vm-via-an-azure-vms-page"></a>Azure VM 페이지를 통해 VM에 대한 JIT 액세스 요청

Azure Portal에서 VM에 연결하려고 할 때 Azure는 해당 VM에 구성된 Just-In-Time 액세스 정책이 있는지 확인합니다.

- VM에 구성된 JIT 정책이 있는 경우 액세스 **요청을** 클릭하여 VM에 대해 설정된 JIT 정책에 따라 액세스 권한을 부여할 수 있습니다. 

  >![jit 요청](./media/security-center-just-in-time/jit-request.png)

  다음과 같은 기본 매개 변수로 액세스가 요청됩니다.

  - **출처 :**'Any' (*) (변경할 수 없음)
  - **시간 범위**: 3시간(변경할 수 없음) <!--Isn't this set in the policy-->
  - **포트 번호** 리눅스용 Windows/포트 22용 RDP 포트 3389(변경 시 있음)

    > [!NOTE]
    > Azure 방화벽으로 보호되는 VM에 대한 요청이 승인된 후 보안 센터는 사용자에게 VM에 연결하는 데 사용할 적절한 연결 세부 정보(DNAT 테이블의 포트 매핑)를 제공합니다.

- VM에 JIT가 구성되지 않은 경우 JIT 정책을 구성하라는 메시지가 표시됩니다.

  ![jit 프롬프트](./media/security-center-just-in-time/jit-prompt.png)

## <a name="configure-a-jit-policy-on-a-vm-programmatically"></a>VM에서 프로그래밍 방식으로 JIT 정책 구성<a name="jit-program"></a>
...
### <a name="jit-vm-access-via-powershell"></a>파워쉘을 통한 JIT VM 액세스
...
## <a name="automatic-cleanup-of-redundant-jit-rules"></a>중복 JIT 규칙의 자동 정리 

JIT 정책을 업데이트할 때마다 정리 도구가 자동으로 실행되어 전체 규칙 집합의 유효성을 확인합니다. 이 도구는 정책의 규칙과 NSG의 규칙 간의 불일치를 찾습니다. 정리 도구에서 불일치가 발견되면 원인을 파악하고 안전한 경우 더 이상 필요하지 않은 기본 제공 규칙을 제거합니다. 클리너는 생성한 규칙을 삭제하지 않습니다.

클리너가 기본 제공 규칙을 제거할 수 있는 경우의 예:

- 동일한 정의를 가진 두 규칙이 있고 다른 규칙보다 우선 순위가 높은 규칙이 있는 경우(즉, 우선 순위가 낮은 규칙은 사용되지 않습니다).
- 규칙 설명에 규칙의 대상 IP와 일치하지 않는 VM의 이름이 포함된 경우 
