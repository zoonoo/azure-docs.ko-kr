...
description: 이 문서에서는 2019 년 7 월 31 일에 사용이 중지 되는 Security Center 기능에 대해 설명 합니다.
...
지난 6 개월 동안 Azure Security Center에 대해 몇 가지 [향상 된 기능](https://azure.microsoft.com/updates/?product=security-center) 을 제공 했습니다.
이러한 향상 된 기능을 통해 2019 년 7 월 31 일에 Security Center에서 일부 중복 기능 및 관련 Api를 제거 하 고 있습니다.  

이러한 사용 중지 기능 중 대부분은 Azure Security Center 또는 Azure Log Analytics의 새로운 기능으로 바꿀 수 있습니다. 다른 기능은 [Azure 센티널 (미리 보기)](https://azure.microsoft.com/services/azure-sentinel/)을 사용 하 여 구현할 수 있습니다.

사용 중지 될 Security Center 기능은 다음과 같습니다.
...
작업 영역을 선택할 때 표시 되는 이벤트 대시보드를 사용 중지 하 고 있습니다.
...
Azure Security Center 현재 Azure Monitor 로그 검색을 사용 하 여 보안 데이터를 검색 하 고 분석 합니다. 이 화면은 검색 페이지 Log Analytics 창 역할을 하며 사용자가 선택한 작업 영역에서 검색 쿼리를 실행할 수 있도록 합니다. 자세한 내용은 [Azure Security Center 검색](https://docs.microsoft.com/azure/security-center/security-center-search)을 참조 하세요. 이 검색 윈도를 사용 중지 하 고 있습니다.
...
이전 단계에 표시 된 세 가지 화면을 모두 사용 중지 하 고 있습니다. 데이터는 Log Analytics 보안 솔루션에서 계속 제공 되며 수정 되거나 제거 되지 않습니다.
...
**보안 이벤트로 이동 맵** 단추와 작업 영역 당 대시보드를 제거 하 고 있습니다.

![보안 경고 맵-단추][10]

**보안 이벤트로 이동 맵** 단추를 선택 하면 위협 인텔리전스 대시보드가 열립니다. 위협 인텔리전스 대시보드를 사용 중지 하 고 있습니다.  

![위협 인텔리전스 대시보드][11]

작업 영역을 선택 하 여 위협 인텔리전스 대시보드를 볼 때 Log Analytics의 보안 경고 맵 (미리 보기) 화면을 엽니다. 이 화면을 사용 중지 하 고 있습니다.

![Log Analytics의 보안 경고 매핑][12]

기존 데이터는 Log Analytics 보안 솔루션에서 계속 사용할 수 있으며 수정 되거나 제거 되지 않습니다.
...
기본 인프라가 사용 중지 되기 때문에 2019 년 6 월 30 일에 [사용자 지정 경고 환경을 사용 중지](https://docs.microsoft.com/azure/security-center/security-center-custom-alert) 하 고 있습니다. 그때 까지는 기존 사용자 지정 경고 규칙을 편집할 수 있지만 새 경고 규칙을 추가할 수는 없습니다. 사용 중지 날짜 후에 정의 된 모든 사용자 지정 경고는 적용 되지 않으며 이러한 규칙을 기반으로 하는 보안 경고가 생성 되지 않습니다.
...
기존 경고를 유지 하 고 Azure 센티널로 만들려면 다음을 수행 합니다.
...
Security Center의 [조사 기능은](https://docs.microsoft.com/azure/security-center/security-center-investigation) 잠재적인 보안 인시던트를 심사 하는 데 도움이 됩니다. 기능을 사용 하면 인시던트의 범위를 이해 하 고 근본 원인을 추적할 수 있습니다. [Azure 센티널](https://azure.microsoft.com/services/azure-sentinel/)에서 개선 된 환경으로 대체 되었기 때문에 Security Center에서이 기능을 제거 하는 중입니다.

![보안 인시던트][14]

**보안 인시던트** 화면에서 **조사** 단추를 선택 하는 경우 Log Analytics에서 조사 대시보드 (미리 보기)를 엽니다. 조사 대시보드를 사용 중지 하 고 있습니다.  
...
Security Center는 [Azure에서 통합 보안 솔루션](https://docs.microsoft.com/azure/security-center/security-center-partner-integration)을 사용 하도록 설정할 수 있습니다. Security Center에서 다음과 같은 파트너 솔루션을 사용 중지 하 고 있습니다. 이러한 솔루션은 여러 추가 데이터 원본과 함께 [Azure 센티널](https://azure.microsoft.com/services/azure-sentinel/) 에서 사용 하도록 설정 됩니다.
...
Azure Security Center은 [150 이상의 권장 규칙](https://gallery.technet.microsoft.com/Azure-Security-Center-a789e335)집합을 적용 하 여 보안 구성을 모니터링 합니다. OS를 강화 하기 위한입니다. 이러한 규칙은 방화벽, 감사, 암호 정책 등에 관련 됩니다. 컴퓨터에 취약한 구성이 있는 것으로 확인되면 Security Center에서 보안 권장 사항이 생성됩니다. [보안 구성 편집 화면](https://docs.microsoft.com/azure/security-center/security-center-customize-os-security-config) 에서 고객이 Security Center의 기본 OS 보안 구성을 사용자 지정할 수 있습니다.

이 미리 보기 기능을 사용 중지 하 고 있습니다. 사용 중지 날짜 후에 보안 구성을 기본값으로 다시 설정 하려는 경우 [다음 지침](https://aka.ms/ascresetsecurityconfigurations) 을 사용 하 여 API 또는 Powershell을 통해 수행할 수 있습니다.
...
보안 및 감사 대시보드는 원래 OMS 포털에서 사용 되었습니다. Log Analytics에서 대시보드는 작업 영역에 저장 된 보안 이벤트에 대 한 중요 한 보안 이벤트와 위협, 위협 인텔리전스 맵 및 id 및 액세스 평가의 작업 영역 당 개요를 제공 합니다. 대시보드를 제거 하는 중입니다. 대시보드 UI에 이미 권장 된 것 처럼 Azure Security Center로 전환 하는 것이 좋습니다.
