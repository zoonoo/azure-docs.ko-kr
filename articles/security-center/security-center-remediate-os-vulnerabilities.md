...
모니터링되는 특정 구성에 대한 자세한 내용은 [권장 구성 규칙 목록](https://gallery.technet.microsoft.com/Azure-Security-Center-a789e335) 을 참조하세요. Security Center에서 보안 구성 평가를 사용자 지정하는 방법을 알아보려면 [OS 보안 구성 사용자 지정](security-center-customize-os-security-config.md)을 참조하세요.

## <a name="implement-the-recommendation"></a>권장 사항 구현
보안 구성 불일치 수정은 Security Center에서 권장 사항으로 표시됩니다. 이 권장 사항은 **권장 사항** 및 **계산** 아래에 표시됩니다.

이 예제에서는 **계산** 아래의 **보안 구성 수정** 권장 사항을 살펴보겠습니다.
1. Security Center 주 메뉴 아래에서 **계산**을 선택합니다.

   ![보안 구성 수정][1]

2. **계산** 아래에서 **보안 구성 수정**을 선택합니다. **보안 구성**이 열립니다.
...
4. 위쪽 리본에서 **검색** 아이콘을 선택합니다. 검색이 열리고 선택한 보안 구성 불일치가 있는 VM과 컴퓨터가 포함된 작업 영역이 나열됩니다. 선택한 규칙이 서로 다른 작업 영역에 연결된 여러 VM에 적용되는 경우에만 이 작업 영역 선택 블레이드가 표시됩니다.

  ![나열된 작업 영역][4]

5. 작업 영역을 선택합니다. 보안 구성 불일치가 있는 작업 영역이 표시되도록 필터링된 Log Analytics 검색 쿼리가 열립니다.
...
이 문서에서는 Security Center 권장 사항 “보안 구성 수정”을 구현하는 방법을 살펴보았습니다. Security Center에서 보안 구성 평가를 사용자 지정하는 방법을 알아보려면 [OS 보안 구성 사용자 지정](security-center-customize-os-security-config.md)을 참조하세요.

[여기](https://gallery.technet.microsoft.com/Azure-Security-Center-a789e335)서 구성 규칙 집합을 검토할 수 있습니다. 보안 센터는 일반 구성 열거형(CCE)을 사용하여 구성 규칙에 대한 고유 ID를 할당합니다. 자세한 내용은 [CCE](https://nvd.nist.gov/cce/index.cfm) 사이트를 방문하세요.
