---
title: Azure Security Center의 향상 된 보안 점수 (미리 보기)
description: Azure Security Center의 향상 된 보안 점수 (미리 보기) 및 보안 컨트롤에 대 한 설명
...
ms.locfileid: "79086538"
---
# <a name="the-enhanced-secure-score-preview"></a>향상 된 보안 점수 (미리 보기) 

이 문서에서는 향상 된 보안 점수 (현재 미리 보기 상태), 함께 제공 되는 보안 컨트롤 및 가져오는 이점을 소개 합니다. 점수가 계산 되는 방법에 대해서도 설명 합니다.
...
*향상* 된 보안 점수 (현재 미리 보기 상태)는 **공격 노출 영역에 초점** 을 맞추고 세 가지 이점을 제공 합니다.

- **보안 제어** -이제 보안 권장 사항이 취약 한 공격 화면을 더 잘 반영 하는 논리적 집합으로 그룹화 됩니다. 자세한 내용은 아래의 [보안 점수를 계산 하는 방법](secure-score-security-controls.md#how-the-secure-score-is-calculated) 을 참조 하세요.

- **전반적인 점수는 전체 상태를 반영 하는** 것이 권장 수준에서 제시 되었다는 것을 반영 합니다. 이러한 향상을 통해 점수는 컨트롤 내의 단일 리소스에 대 한 *모든* 권장 사항을 재구성 하는 경우에만 개선 됩니다. 즉, 리소스 보안이 향상 되는 경우에만 점수가 향상 됩니다. 

- **개별 공격 표면의 보안 상태가 더 표시 됩니다** . 보안 제어 당 점수를 표시 하 여 보안 점수 페이지는 조직이 각 개별 공격 노출 영역을 얼마나 잘 보호 하 고 있는지 자세히 볼 수 있는 위치가 됩니다.

다음 스크린샷에 표시 된 것 처럼 향상 된 보안 점수가 백분율로 표시 됩니다.

[이제 향상 된 보안 점수 (미리 보기)에 백분율이 포함 ![](media/secure-score-security-controls/secure-score-with-percentage.png)](media/secure-score-security-controls/secure-score-with-percentage.png#lightbox)
...
이 미리 보기 전에는 각 권장 사항을 개별적으로 고려 하 고 심각도를 기준으로 값을 할당 Security Center. 보안 상태를 개선 하기 위해 작업 하는 보안 팀은 전체 검색 목록에 따라 Security Center 권장 사항에 대 한 응답의 우선 순위를 지정 해야 했습니다. 단일 리소스에 대 한 권장 사항을 재구성 할 때마다 보안 점수가 향상 됩니다.

보안 점수 향상의 일환으로 이제 권장 사항이 **보안 제어**로 그룹화 됩니다. 컨트롤은 보안 권장 사항 집합과 이러한 권장 사항을 구현 하는 데 도움이 되는 지침을 제공 합니다. 컨트롤은 관련 권장 구성의 논리적 그룹화입니다. 점수는 더 이상 권장 사항 수준에서 부여 되지 않습니다. 대신 컨트롤 내의 단일 리소스에 대 한 *모든* 권장 사항을 재구성 하는 경우에만 점수가 개선 됩니다.

전반적인 보안 점수에 대 한 각 보안 제어의 기여는 권장 사항 페이지에서 명확 하 게 표시 됩니다.

[향상 된 보안 점수 (미리 보기)에 보안 제어를 도입 ![](media/secure-score-security-controls/security-controls.png)](media/secure-score-security-controls/security-controls.png#lightbox)
...
["시스템 업데이트 적용" 보안 제어 ![](media/secure-score-security-controls/apply-system-updates-control.png)](media/secure-score-security-controls/apply-system-updates-control.png#lightbox)
...
|**취약점 해결**|6|-SQL server에서 고급 데이터 보안을 사용 하도록 설정 해야 합니다.<br>-Azure Container Registry 이미지의 취약성을 재구성 해야 함 (미리 보기)<br>-SQL 데이터베이스에 대 한 취약성을 재구성 해야 합니다.<br>-취약점 평가 솔루션에서 취약점을 수정 해야 합니다.<br>-SQL 관리 되는 인스턴스에서 취약점 평가를 사용 하도록 설정 해야 합니다.<br>-SQL server에서 취약성 평가를 사용 하도록 설정 해야 합니다.<br>-취약성 평가 솔루션을 가상 머신에 설치 해야 합니다.|
...
|**액세스 및 사용 권한 관리**|4|-구독에 대해 최대 3 개의 소유자를 지정 해야 합니다.<br>-사용 되지 않는 계정은 구독에서 제거 해야 합니다.<br>-소유자 권한이 있는 사용 되지 않는 계정은 구독에서 제거 해야 합니다.<br>-소유자 권한이 있는 외부 계정을 구독에서 제거 해야 합니다.<br>-읽기 권한이 있는 외부 계정을 구독에서 제거 해야 합니다.<br>-쓰기 권한이 있는 외부 계정을 구독에서 제거 해야 합니다.<br>-구독에 둘 이상의 소유자가 할당 되어 있어야 합니다.<br>-Kubernetes Services (미리 보기)에서 RBAC (역할 기반 Access Control)를 사용 해야 합니다.<br>-Service Fabric 클러스터는 클라이언트 인증을 위해 Azure Active Directory만 사용 해야 합니다.|
|**보안 구성 수정**|4|-Pod 보안 정책은 Kubernetes Services (미리 보기)에서 정의 해야 합니다.<br>-컨테이너 보안 구성의 취약성을 재구성 해야 함<br>-컴퓨터에서 보안 구성의 취약성을 재구성 해야 함<br>-가상 머신 확장 집합에 대 한 보안 구성의 취약성을 재구성 해야 함<br>-가상 컴퓨터에 모니터링 에이전트를 설치 해야 합니다.<br>-모니터링 에이전트가 컴퓨터에 설치 되어 있어야 합니다.<br>-가상 머신 확장 집합에 모니터링 에이전트를 설치 해야 합니다.<br>-컴퓨터에서 모니터링 에이전트 상태 문제를 해결 해야 합니다.|
...
이러한 향상 된 보안 점수에 도입 된 변경 내용에 따르면 점수를 받도록 리소스에 대 한 모든 권장 사항을 해결 해야 합니다. 점수가 0-10로 변경 됩니다.
...
전혀 단일 리소스에 대 한 모든 권장 사항을 수정할 때까지 변경 되지 않습니다. 컨트롤의 최대 점수를 얻으려면 모든 리소스에 대해 모든 권장 사항을 수정 해야 합니다.

### <a name="will-this-enhanced-secure-score-replace-the-existing-secure-score"></a>이 향상 된 보안 점수가 기존 보안 점수를 대체 하나요? 
예. 하지만의 경우 쉽게 전환할 수 있도록 side-by-side로 실행 됩니다.
...
이 문서에서는 향상 된 보안 점수와이에 도입 된 새로운 보안 컨트롤에 대해 설명 했습니다. 관련 자료는 다음 문서를 참조 하세요.
