...
1. **다음: 규칙** 을 선택합니다.
1. **규칙 컬렉션 추가** 를 선택합니다.
1. **이름** 에 대해 **RCNet01** 을 입력합니다.
1. **규칙 컬렉션 형식** 에 대해 **네트워크** 를 선택합니다.
1. **우선 순위** 에 대해 **100** 을 입력합니다.
1. **동작** 에 대해 **허용** 을 선택합니다.
1. **규칙** 아래에서 **이름** 에 대해 **AllowWeb** 을 입력합니다.
1. **원본** 에 대해 **192.168.1.0/24** 를 선택합니다.
1. **프로토콜** 의 경우 **TCP** 를 선택합니다.
1. **대상 포트** 에 대해 **80** 을 입력합니다.
1. **대상 유형** 에 대해 **IP 주소** 를 선택합니다.
1. **대상** 에 대해 **10.6.0.0/16** 을 입력합니다.
1. 다음과 같이 규칙 행에서 해당 정보를 입력합니다.
 
    이름: **AllowRDP** 입력<br>
    원본: 유형 **192.168.1.0/24**.<br>
    프로토콜: **TCP** 선택<br>
    대상 포트: **3389** 입력<br>
    대상 유형: **IP 주소** 선택<br>
    대상: **10.6.0.0/16** 입력
...
2. **가상 네트워크** 를 검색한 다음, **가상 네트워크** 를 선택합니다.
1. **만들기** 를 선택합니다.
...
1. **IPv4 주소 공간** 에 **10.5.0.0/16** 을 입력합니다.
1. **서브넷 이름** 아래에서 **기본값** 을 선택합니다.
1.  **서브넷 이름** 을 **AzureFirewallSubnet** 으로 변경합니다. 방화벽은 이 서브넷에 있고 서브넷 이름은 AzureFirewallSubnet이 **되어야** 합니다.
1. **서브넷 주소 범위** 에 **10.5.0.0/26** 을 입력합니다. 
1. 다른 기본 설정을 유지한 다음, **저장** 을 선택합니다.
1. **검토 + 만들기** 를 선택합니다.
1. **만들기** 를 선택합니다.
...
1. **IPv4 주소 공간** 에 **10.6.0.0/16** 을 입력합니다.
1. **서브넷 이름** 아래에서 **기본값** 을 선택합니다.
1. **서브넷 이름** 을 **SN-Workload** 로 변경합니다.
1. **서브넷 주소 범위** 에 **10.6.0.0/24** 를 입력합니다. 
1. 다른 기본 설정을 유지한 다음, **저장** 을 선택합니다.
1. **검토 + 만들기** 를 선택합니다.
1. **만들기** 를 선택합니다.
...
1. **IPv4 주소 공간** 에 **192.168.0.0/16** 을 입력합니다.
1. **서브넷 이름** 아래에서 **기본값** 을 선택합니다.
1. **서브넷 이름** 을 **SN-Corp** 로 변경합니다.
1. **서브넷 주소 범위** 에 **192.168.1.0/24** 를 입력합니다. 
1. 다른 기본 설정을 유지한 다음, **저장** 을 선택합니다.
2. **서브넷 추가** 를 선택합니다.
3. **서브넷 이름** 에 **GatewaySubnet** 을 입력합니다.
...
보안 정책이 허브와 연결된 경우 이를 *허브 가상 네트워크* 라고 합니다.
...
1. **Virtual Network** 에서 **VNet-hub** 의 확인란을 선택합니다.
1. **보안 관리** 를 선택한 다음, **방화벽 정책을 사용하여 방화벽 배포** 를 선택합니다.
1. **가상 네트워크 변환** 페이지의 **방화벽 정책** 에서 **Pol-Net01** 의 확인란을 선택합니다.
1. **다음 : 검토 + 확인** 을 선택합니다.
1. 세부 정보를 검토한 다음, **확인** 을 선택합니다.
...
이 단계에서는 허브 가상 네트워크에서 온-프레미스 가상 네트워크로의 연결을 만듭니다. 예제에서 참조된 공유 키를 볼 수 있습니다. 공유 키에 대해 고유한 값을 사용할 수 있습니다. 중요한 점은 두 연결에서 모두 공유 키가 일치해야 한다는 것입니다. 연결을 만드는 데 약간의 시간이 걸립니다.

1. **FW-Hybrid-Test** 리소스 그룹을 열고, **GW-hub** 게이트웨이를 선택합니다.
2. 왼쪽 열에서 **연결** 을 선택합니다.
3. **추가** 를 선택합니다.
4. 연결 이름에 **Hub-to-Onprem** 을 입력합니다.
...
4. **이 가상 네트워크** 에서 다음을 확인할 수 있습니다.
   ...
   |가상 네트워크 게이트웨이 또는 경로 서버    |  이 가상 네트워크의 게이트웨이 사용       |
...
5. **추가** 를 선택합니다.
   ...
   :::image type="content" source="media/secure-hybrid-network/firewall-peering.png" alt-text="Vnet 피어링":::
...
3. **경로 테이블** 을 선택합니다.
4. **만들기** 를 선택합니다.
1. 리소스 그룹에 대해 **FW-Hybrid-Test** 를 선택합니다.
1. **지역** 에 대해 **미국 동부** 를 선택합니다.
1. 이름에 대해 **UDR-Hub-Spoke** 를 입력합니다.
1. **검토 + 만들기** 를 선택합니다.
1. **만들기** 를 선택합니다.
1. 경로 테이블이 만들어지면 해당 테이블을 선택하여 경로 테이블 페이지를 엽니다.
1. 왼쪽 열에서 **경로** 를 선택합니다.
1. **추가** 를 선택합니다.
1. 경로 이름에 대해 **ToSpoke** 를 입력합니다.
1. 주소 접두사에 대해 **10.6.0.0/16** 을 입력합니다.
1. 다음 홉 형식에 대해 **가상 어플라이언스** 를 선택합니다.
1. 다음 홉 주소에 대해 앞에서 적어둔 방화벽의 개인 IP 주소를 입력합니다.
1. **확인** 을 선택합니다.
...
1. 왼쪽 열에서 **경로** 를 선택합니다.
1. **추가** 를 선택합니다.
1. 경로 이름에 대해 **ToHub** 를 입력합니다.
1. 주소 접두사에 대해 **0.0.0.0/0** 을 입력합니다.
1. 다음 홉 형식에 대해 **가상 어플라이언스** 를 선택합니다.
1. 다음 홉 주소에 대해 앞에서 적어둔 방화벽의 개인 IP 주소를 입력합니다.
1. **확인** 을 선택합니다.
...
1. **검토 + 만들기** 를 선택하고 요약 페이지에서 설정을 검토한 다음, **만들기** 를 선택합니다.
...
2. **설정** 아래에서 **규칙 컬렉션** 을 선택합니다.
1. **RCNet01** 규칙 컬렉션을 선택합니다.
1. **규칙 컬렉션 작업** 에 대해 **거부** 를 선택합니다.
1. **저장** 을 선택합니다.

변경된 규칙을 테스트하려면 먼저 **VM-Onprem** 에서 기존 원격 데스크톱 및 브라우저를 닫습니다. 규칙 컬렉션 업데이트가 완료되면 테스트를 다시 실행합니다. 이번에는 모두 연결에 실패해야 합니다.
