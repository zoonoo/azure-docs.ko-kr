---
title: Azure 방화벽 관리자에 대 한 azure 보안 기준
description: Azure 방화벽 관리자 보안 기준은 Azure 보안 벤치 마크에 지정 된 보안 권장 사항을 구현 하기 위한 절차 지침과 리소스를 제공 합니다.
author: msmbaldwin
...
ms.locfileid: "96171044"
---
# <a name="azure-security-baseline-for-azure-firewall-manager"></a>Azure 방화벽 관리자에 대 한 azure 보안 기준

이 보안 기준은 [Azure Security 벤치 마크 버전 2.0](../security/benchmarks/overview.md) 의 지침을 Azure 방화벽 관리자에 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다. 콘텐츠는 azure 보안 벤치 마크에 정의 된 **보안 컨트롤** 및 Azure 방화벽 관리자에 적용 되는 관련 지침에 따라 그룹화 됩니다. Azure 방화벽 관리자에 적용할 수 없는 **컨트롤이** 제외 되었습니다.

Azure 방화벽 관리자가 Azure 보안 벤치 마크에 완전히 매핑되는 방식을 보려면 [전체 Azure 방화벽 관리자 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
**지침**: Azure 방화벽 관리자는 기본 id 및 액세스 관리 서비스로 Azure Active Directory (azure AD)를 사용 합니다. 에서 조직의 id 및 액세스 관리를 관리 하려면 Azure AD를 표준화 해야 합니다.
...
**지침**: Azure 방화벽 관리자는 MFA (multi-factor authentication)를 통해 강력한 인증 제어를 지 원하는 Azure Active Directory 사용 하 고 강력한 암호 없는 메서드를 사용 합니다.
...
**지침**: Azure 방화벽 관리자는 다음과 같은 데이터 원본을 제공 하는 Azure Active Directory와 통합 되어 있습니다.
...
현재 방화벽 정책 규칙 컬렉션 그룹을 둘러싼 작업은 활동 로그에서 지원 되지 않습니다 .이 문제는 알려진 문제 이며 향후 업데이트에서 해결 되 고 있습니다.
...
- [Azure 활동 로그를 Azure Monitor에 통합하는 방법](/active-directory/reports-monitoring/howto-integrate-activity-logs-with-log-analytics)

- [Azure 방화벽 관리자의 알려진 문제](overview.md#known-issues)
...
**지침**: Azure 방화벽 관리자는 로그인에 MFA를 사용 해야 하는 특정 IP 범위의 사용자 로그인과 같은 사용자 정의 조건에 따라 보다 세분화 된 액세스 제어를 위해 azure AD (Azure Active Directory) 조건부 액세스를 지원 합니다. 여러 사용 사례에 대해 세부적인 인증 세션 관리 정책을 사용할 수도 있습니다.
...
**지침**: Azure 방화벽 관리자는 id 및 액세스에 Azure Active Directory (azure AD)를 사용 합니다. 가장 중요 한 기본 제공 역할은 전역 관리자이 고, 권한 있는 역할 관리자는 이러한 두 역할에 할당 된 사용자가 관리자 역할을 위임할 수 있습니다.
...
권한 있는 특정 권한이 할당 된 사용자 지정 역할을 사용 하는 경우 관리 해야 하는 다른 중요 한 역할이 있을 수 있습니다. 또한 중요 한 비즈니스 자산의 관리자 계정에 비슷한 컨트롤을 적용할 수 있습니다.
...
**지침**: 사용자 지정 azure RBAC 역할을 사용 하 여 azure 방화벽 정책에 대 한 규칙 컬렉션 그룹에 대 한 액세스를 격리 합니다. Azure 사용자 지정 역할 정의를 사용 하 여 실수로 인 한 기본 정책 제거를 방지 하 고 구독 또는 리소스 그룹 내에서 규칙 컬렉션 그룹에 대 한 선택적 액세스를 제공 합니다.

비즈니스에 중요 한 시스템에 대 한 관리 권한이 있는 관리, id 및 보안 시스템에 대 한 액세스도 제한 해야 합니다. 이러한 관리 및 보안 시스템을 손상 시키는 공격자는 비즈니스에 중요 한 자산을 손상 시키기 위해 즉시 weaponize 수 있습니다.

모든 종류의 액세스 제어는 일관 된 액세스 제어를 보장 하기 위해 엔터프라이즈 조각화 전략에 맞춰야 합니다.

- [Azure 방화벽 정책을 사용 하 여 규칙 hierarch 정의](rule-hierarchy.md)
...
**지침**: Azure 방화벽 관리자는 Azure Active Directory (azure AD) 계정을 사용 하 여 해당 리소스를 관리 합니다. 사용자 계정 및 액세스 할당을 정기적으로 검토 하 여 계정 및 액세스 권한이 올바른지 확인 합니다. Azure AD 액세스 검토를 사용 하 여 그룹 멤버 자격, 엔터프라이즈 응용 프로그램에 대 한 액세스 및 역할 할당을 검토할 수 있습니다. Azure AD reporting은 오래 된 계정을 검색 하는 데 도움이 되는 로그를 제공할 수 있습니다. 또한 Azure AD Privileged Identity Management를 사용 하 여 검토 프로세스를 용이 하 게 하는 액세스 검토 보고서 워크플로를 만들 수 있습니다.
...
**지침**: Azure 방화벽 관리자는 Azure Active Directory를 사용 하 여 서비스를 관리 하는 사용자를 인증 합니다. 실수로 Azure AD 조직에서 잠기는 것을 방지 하려면 일반 관리 계정을 사용할 수 없는 경우 액세스를 위한 응급 액세스 계정을 설정 합니다. 응급 액세스 계정은 일반적으로 매우 특권 이며 특정 사용자에 게 할당 되 면 안 됩니다. 응급 액세스 계정은 일반 관리 계정을 사용할 수 없는 '비상' 시나리오의 긴급한 상황으로 제한됩니다.
...
**지침**: Azure 방화벽 관리자는 리소스를 관리 하기 위해 Azure Active Directory와 통합 됩니다. Azure AD 자격 관리 기능을 사용 하 여 액세스 권한 부여, 검토, 만료 등의 액세스 요청 워크플로를 자동화할 수 있습니다. 이중 또는 다단계 승인도 지원 됩니다.
...
**지침**: 보안이 유지 되 고 격리 된 워크스테이션은 관리자, 개발자 및 중요 서비스 운영자와 같은 중요 한 역할의 보안에 매우 중요 합니다. 프로덕션 환경에서 Azure 방화벽 관리자 리소스를 사용 하 여 관리 관리 작업을 수행 하는 데 매우 안전한 사용자 워크스테이션을 사용 합니다. Azure Active Directory, Microsoft Defender ATP (Advanced Threat Protection) 및/또는 Microsoft Intune를 사용 하 여 관리 작업을 위한 안전 하 고 관리 되는 사용자 워크스테이션을 배포할 수 있습니다. 보안 된 워크스테이션을 중앙에서 관리 하 여 강력한 인증, 소프트웨어 및 하드웨어 기준, 제한 된 논리 및 네트워크 액세스를 비롯 한 보안 구성을 적용할 수 있습니다.
...
**지침**: azure RBAC (Azure 역할 기반 Access Control), 네트워크 기반 액세스 제어 및 azure 서비스의 특정 제어를 사용 하 여 액세스를 제한 하 여 Azure 방화벽 정책에 대 한 구성 데이터와 같은 중요 한 데이터를 보호 합니다.
...
**지침**: Azure 방화벽 정책 리소스는 인증 된 사용자만 액세스할 수 있습니다. 고객은 권한 있는 사용자만 데이터에 액세스할 수 있는지 확인 해야 합니다.

- [규칙 컬렉션 그룹에 액세스 하기 위한 사용자 지정 역할 만들기](rule-hierarchy.md#create-custom-roles-to-access-the-rule-collection-groups)
...
Azure 방화벽 관리자는 TLS v 1.2 이상으로 전송 중인 데이터 암호화를 지원 합니다.
...
**지침**: 보안 팀이 Azure에서 Azure 방화벽 관리자 자산의 지속적으로 업데이트 된 인벤토리에 액세스할 수 있는지 확인 합니다. Azure 리소스 그래프를 사용 하 여 Azure 서비스, 응용 프로그램 및 네트워크 리소스를 비롯 하 여 구독에 있는 모든 Azure 방화벽 리소스를 쿼리하고 검색할 수 있습니다.
...
**지침**: Azure Policy을 사용 하 여 사용자 환경에서 프로 비전 할 수 있는 서비스를 감사 하 고 제한할 수 있습니다. 여기에는 Azure 방화벽 리소스의 배포를 허용 하거나 거부할 수 있는 기능이 포함 됩니다. Azure 리소스 그래프를 사용 하 여 구독 내에서 리소스를 쿼리하고 검색 합니다. 또한 Azure Monitor를 사용 하 여 승인 되지 않은 서비스가 검색 될 때 경고를 트리거하는 규칙을 만들 수 있습니다.
...
- [Azure 방화벽 정책 CLI](https://docs.microsoft.com/cli/azure/ext/azure-firewall/network/firewall/policy?view=azure-cli-latest&amp;preserve-view=true)
...
**지침**: Azure 방화벽 관리자는 id 및 인증을 위해 azure AD (Azure Active Directory)와 통합 됩니다. Azure 조건부 액세스를 사용 하 여 "Microsoft Azure 관리" 앱에 대 한 "액세스 차단"을 구성 하 여 Azure 리소스 관리자와 상호 작용 하는 사용자의 기능을 제한할 수 있습니다.
...
**지침**:에 의해 생성 되거나 사용자 지정 위협 검색을 설정 하는 데 사용할 수 있는 siem에 대 한 방화벽 정책과 관련 된 활동 로그를 전달 합니다. 잠재적 위협 및 비정상에 대해 다양 한 유형의 Azure 자산을 모니터링 하 고 있는지 확인 합니다. 분석에 대 한 가양성을 줄이기 위해 고품질 경고를 얻는 데 집중 하세요. 경고는 로그 데이터, 에이전트 또는 기타 데이터에서 원본으로 사용할 수 있습니다.
...
- [Azure Firewall 로그 및 메트릭](../firewall/firewall-diagnostics.md)
...
**참고**: 자동으로 사용할 수 있는 활동 로그에는 읽기 작업 (GET)을 제외한 방화벽 정책 리소스에 대 한 모든 쓰기 작업 (PUT, POST, DELETE)이 포함 됩니다. 활동 로그를 사용 하 여 문제를 해결할 때 오류를 찾거나 조직의 사용자가 리소스를 수정한 방법을 모니터링할 수 있습니다.
...
또한 Azure 센티널 또는 타사 SIEM에 로그 데이터를 사용 하도록 설정 하 고 등록 합니다.
...
**지침**: 방화벽 정책 로그를 저장 하는 데 사용 되는 저장소 계정 또는 Log Analytics 작업 영역에 조직의 규정 준수 규정에 따라 설정 된 로그 보존 기간이 있는지 확인 합니다.

Azure Monitor에서 조직의 규정 준수 규정에 따라 Log Analytics 작업 영역 보존 기간을 설정할 수 있습니다. 장기 및 보관 저장소에 Azure Storage, Data Lake 또는 Log Analytics 작업 영역 계정을 사용 합니다.
...
**지침**: Azure Resource Manager 템플릿, azure RBAC 컨트롤 및 Azure Policy와 같은 메커니즘을 사용 하 여 환경에서 Azure 방화벽 관리자 리소스의 배포 및 구성을 자동화 하 고 보호 합니다. 배포 시 Azure 방화벽 관리자 리소스에 대 한 보안 구성을 정의 하 고, ' Azure. Network ' 네임 스페이스의 별칭을 사용 하 여 사용자 지정 Azure Policy 정의를 정의 하 여 이러한 구성을 감사 하 고 적용 합니다.
...
**지침**: Azure 방화벽 관리자는 Azure Resource Manager 기반 템플릿 및 Azure Policy를 통해 구성 설정 적용을 지원 합니다. ' Azure. Network ' 네임 스페이스의 별칭을 사용 하 여 Azure 방화벽 관리자 리소스 구성을 감사 하 고 적용 하는 사용자 지정 Azure Policy 정의를 정의 합니다.
...
**지침**: 해당 사항 없음 Azure 방화벽 관리자는 방화벽 제어 평면 관리 서비스 이며, 고객이 구성할 수 있도록 기본 서비스의 계산 인프라를 노출 하지 않습니다.
...
**지침**: Azure 방화벽 관리자는 고객 지향 시스템 백업 개념을 포함 하지 않습니다. 기본 인프라는 Microsoft에서 처리 합니다.

리소스 구성 백업의 경우 Azure Resource Manager를 사용 하 여 구성 백업으로 사용할 수 있는 JavaScript Object Notation (JSON) 템플릿에서 방화벽 정책 및 관련 리소스를 내보냅니다. Azure Portal에서 Azure 방화벽의 템플릿 내보내기 기능을 사용 하 여 방화벽 정책 구성을 내보낼 수도 있습니다. 사용자 지정 백업 스크립트를 실행 하 여 Azure 방화벽 관리자 리소스의 리소스 구성을 자동으로 캡처하려면 Azure Automation를 사용 합니다.

- [템플릿을 사용 하 여 보안 가상 허브 배포](quick-secure-virtual-hub.md)

- [Microsoft 방화벽 정책 템플릿 참조](/azure/templates/microsoft.network/firewallpolicies)
...
**지침**: Azure 방화벽 관리자에는 고객 지향 시스템 백업의 개념이 없습니다. 내보낸 Azure 방화벽 관리자 리소스 템플릿에서는 이러한 Azure Resource Manager 템플릿 파일을 사용 하 여 정기적으로 복원을 수행 합니다.

- [Azure Resource Manager 템플릿을 사용 하 여 보안 가상 허브 배포](quick-secure-virtual-hub.md)
