<properties 
   pageTitle="Azure Mobile Engagement 방법 가이드" 
   description="Azure Mobile Engagement의 방법 가이드" 
...
# Azure Mobile Engagement - 방법 가이드

## 소개

다음 Azure Mobile Engagement 사용자 인터페이스의 방법 가이드는 Azure Mobile Engagement의 기본 [개념]을 잘 알고 있으며,[Link 6] Azure Mobile Engagement [SDK]를[Link 5] 응용 프로그램으로 통합한 후 작업을 진행한다고 가정합니다. 이러한 연습에서는 표준 사용자가 [사용자 인터페이스]를 어떻게 사용할 수 있는지에 집중하며,[Link 1] 개발자 사용자가 HTTP REST 기반 [API]를 어떻게 사용할 수 있는지는[Link 4]거의 다루지 않습니다. 이러한 [연습]을 진행하는 데 어려움이 있는 경우[Link 3] Azure Mobile Engagement [문제 해결 가이드]를[Link 2]참조하세요.

## <a name="#First">첫 번째 푸시 알림 캠페인 수행</a>
...
## <a name="#Personalize">개인 설정 푸시 보내기</a>
...
> Warning: 앱 정보 태그가 사용자에 대해 정의되지 않은 경우 사용자는 푸시를 받지 못합니다.
...
## <a name="#Differentiate">푸시 알림 차별화(앱 내부 또는 외부에서)</a>
...
## <a name="#Schedule">푸시 캠페인 예약</a>
 
### 캠페인 시작 계획
 
![Shedule1][18]

3월 21일인데, 3월 22일 자정에 발표를 계획한 알림이 있습니다. 
이 경우 푸시를 수행하기 위해 인터페이스 앞에 대기하고 있지 않아도 됩니다! 정확한 시간에 알림이 전송되도록 미리 계획할 수 있습니다.
...
> 참고: 종료 날짜 없이 생성한 캠페인은 푸시를 장치에 로컬로 저장하고 캠페인이 수동으로 종료된 경우에도 다음에 앱이 열릴 때 표시됩니다.

## <a name="#TextView">텍스트 뷰로 푸시 알림 개선</a>
...
## <a name="#WebView">웹 뷰로 푸시 알림 개선</a>
...
- 알림만: 간단한 표준 알림입니다. 즉, 사용자가 클릭하는 경우 표시되는 추가 뷰는 없지만 연결된 작업이 실행됩니다.
- 텍스트 알림: 사용자가 텍스트 뷰를 살펴볼 수 있는 알림입니다.
- 웹 알림: 사용자가 웹 뷰를 살펴볼 수 있는 알림입니다.
"웹 알림" 콘텐츠를 선택합니다.
