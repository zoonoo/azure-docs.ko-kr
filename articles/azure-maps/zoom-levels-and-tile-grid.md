---
title: 확대/축소 수준 및 타일 그리드 | 마이크로소프트 Azure 지도
description: 이 문서에서는 Microsoft Azure 지도에서 확대/축소 수준 및 타일 그리드에 대해 알아봅니다.
...
Azure Maps에서는 구면 메르카토르 도법 구면좌표계를 사용합니다(EPSG: 3857). 투영은 구형 글로브를 플랫 맵으로 변환하는 데 사용되는 수학적 모델입니다. 구형 메르카토르 투영은 기둥에서 맵을 확장하여 정사각형 맵을 만듭니다. 이 투영은 맵의 배율과 면적을 크게 왜곡하지만 이 왜곡보다 큰 두 가지 중요한 특성을 가시합니다.

- 이는 비교적 작은 물체의 모양을 보존한다는 것을 의미하는 형상 적응형 투영입니다. 작은 물체의 모양을 보존하는 것은 항공 이미지를 표시할 때 특히 중요합니다. 예를 들어, 건물의 모양을 왜곡하지 않으려고 합니다. 정사각형 건물은 직사각형이 아닌 정사각형으로 표시되어야 합니다.
- 원통형 투영입니다. 북쪽과 남쪽은 항상 위아래로, 서쪽과 동쪽은 항상 좌우입니다. 

맵 검색 및 표시 성능을 최적화하기 위해 맵은 정사각형 타일로 나뉩니다. Azure Maps SDK의 사용 타일은 도로 맵의 경우 512 x 512 픽셀, 위성 이미지의 경우 256 x 256 픽셀로 더 작습니다. Azure Maps는 0부터 22까지의 23개의 확대/축소 수준에 대해 래스터 및 벡터 타일을 제공합니다. 확대/축소 수준 0에서 전 세계는 단일 타일로 맞춰집니다.

<center>

![월드 맵 타일](./media/zoom-levels-and-tile-grid/world0.png)</center>

확대/축소 수준 1은 4개의 타일을 사용하여 전 세계를 2x2 사각형으로 렌더링합니다.

<center>

![2x2 맵 타일 레이아웃](media/zoom-levels-and-tile-grid/map-2x2-tile-layout.png)</center>

각 추가 확대/축소 수준은 이전 타일의 타일을 쿼드로 나누어 2<sup>줌</sup> x 2<sup>줌의</sup>격자를 만듭니다. 확대/축소 수준 22는 2<sup>22</sup> x 2<sup>22</sup> 그리드 또는 4,194,304 x 4,194,304 타일(총 17,592,186,044,416개)입니다.

웹 및 Android 지원 25 줌 수준에 대한 Azure Maps 대화형 맵 컨트롤은 0부터 24까지의 숫자로 표시됩니다. 도로 데이터는 타일을 사용할 수 있을 때 확대/축소 수준에서만 사용할 수 있지만.

다음 표에서는 타일 크기가 512픽셀 정사각형인 확대/축소 수준에 대한 전체 값 목록을 제공합니다.
...
각 확대/축소 수준에서 사용할 투영 및 배율을 선택한 후 지리적 좌표를 픽셀 좌표로 변환할 수 있습니다. 특정 확대/축소 수준에 대한 세계의 맵 이미지의 전체 픽셀 너비와 높이는 다음과 같이 계산됩니다.
...
각 확대/축소 수준에서 맵 너비와 높이가 다르기 때문에 픽셀 좌표도 마찬가지입니다. 맵의 왼쪽 위 모서리에 있는 픽셀에는 항상 픽셀 좌표(0, 0)가 있습니다. 맵의 오른쪽 아래 모서리에 있는 픽셀에는 픽셀 *좌표(너비-1, 높이-1)가*있거나 이전 섹션의 *방정식을 \* 참조합니다(tileSize \* 2<sup>확대/축소</sup>-1, tileSize 2<sup>확대/축소</sup>-1).* 예를 들어 수준 2에서 512제곱 타일을 사용하는 경우 픽셀 좌표의 범위는 다음과 같습니다(0, 0)에서 (2047, 2047)까지의 픽셀 좌표는 다음과 같습니다.

<center>

![픽셀 크기를 보여주는 맵](media/zoom-levels-and-tile-grid/map-width-height.png)

</center>

위도 및 경도의 각도와 세부 수준이 주어지면 픽셀 XY 좌표는 다음과 같이 계산됩니다.
...
위도 및 경도 값은 WGS 84 데이텀에 있는 것으로 가정합니다. Azure Maps는 구형 투영을 사용하지만 모든 지리적 좌표를 공통 데이텀으로 변환하는 것이 중요합니다. WGS 84는 선택된 데이텀이다. 경도 값은 -180도에서 +180도사이로 가정하고 위도 값은 -85.05112878에서 85.05112878까지의 범위로 잘려야 합니다. 이러한 값을 준수하면 극에서 특이점을 방지하고 투영된 맵이 제곱 된 모양인지 확인합니다.

## <a name="tile-coordinates"></a>타일 좌표

맵 검색 및 표시 성능을 최적화하기 위해 렌더링된 맵이 타일로 잘려나갑니다. 픽셀 수와 타일 수는 각 확대/축소 수준에서 다릅니다.
...
각 타일에는 왼쪽 상단의 (0, 0)에서 오른쪽 *하단의 XY 좌표(2<sup>줌</sup>-1, 2<sup>줌</sup>-1)에* 이르는 XY 좌표가 주어집니다. 예를 들어 확대/축소 수준 2에서 타일 좌표는 다음과 같이 (0, 0) 에서 (7, 7) 범위입니다.

<center>

![타일 좌표의 지도](media/zoom-levels-and-tile-grid/map-tiles-x-y-coordinates-7x7.png)</center>

픽셀 XY 좌표 쌍이 주어지면 해당 픽셀을 포함하는 타일의 타일 XY 좌표를 쉽게 확인할 수 있습니다.
...
타일은 확대/축소 수준에서 호출됩니다. x 및 y 좌표는 해당 확대/축소 레벨의 격자에 있는 타일의 위치에 해당합니다.

사용할 확대/축소 수준을 결정할 때각 위치가 타일의 고정 된 위치에 있다는 것을 기억하십시오. 결과적으로 지정된 영역을 표시하는 데 필요한 타일 수는 세계 지도에서 확대/축소 그리드의 특정 배치에 따라 달라집니다. 예를 들어 900미터 떨어진 두 개의 지점이 있는 경우 확대/축소 수준 17에서 서로 간에 경로를 표시하기 위해 세 개의 타일만을 사용할 *수* 있습니다. 그러나 서쪽 지점이 해당 타일의 오른쪽에 있고 동쪽 지점이 해당 타일의 왼쪽에 있는 경우 4개의 타일이 필요할 수 있습니다.

<center>

![확대/축소 데모 크기 조정](media/zoom-levels-and-tile-grid/zoomdemo_scaled.png)</center>

확대/축소 수준이 결정되면 x 및 y 값을 계산할 수 있습니다. 각 확대/축소 그리드의 왼쪽 상단 타일은 x=0, y=0입니다. 오른쪽 아래 타일은 x=2<sup>줌-1,</sup>y=2<sup>확대/1입니다.</sup>
...
## <a name="quadkey-indices"></a>쿼드키 지수

일부 매핑 플랫폼은 `quadkey` 타일 ZY 좌표를 키라고 하는 1차원 문자열로 `quadtree` `quadkeys` 결합하는 인덱싱 명명 규칙을 사용합니다. 각 `quadkey` 타일은 특정 세부 수준에서 단일 타일을 고유하게 식별하며 공통 데이터베이스 B-트리 인덱스의 키로 사용할 수 있습니다. Azure Maps SDK는 타일 레이어 추가 문서에 `quadkey` 설명된 대로 다른 명명 규칙 외에 명명 규칙을 사용하는 [타일 레이어의 오버레이를](map-add-tile-layer.md) 지원합니다.

> [!NOTE]
> `quadkeys` 명명 규칙은 하나 이상 확대/축소 수준에 대해서만 작동합니다. Azure 지도 SDK의 지원 확대/축소 수준 0은 전 세계에 대한 단일 맵 타일입니다. 

타일 좌표를 a로 `quadkey`변환하려면 Y 및 X 좌표의 비트가 인터리브되고 결과는 기본 4 숫자(선행 영점 유지)로 해석되고 문자열로 변환됩니다. 예를 들어 레벨 3에서 타일 XY 좌표(3, `quadkey` 5)가 주어지면 다음과 같이 결정됩니다.
...
`Qquadkeys`몇 가지 흥미로운 속성이 있습니다. 먼저 숫자(숫자 `quadkey` 수)의 길이는 해당 타일의 확대/축소 수준과 같습니다. 둘째, `quadkey` 모든 타일의 부모 `quadkey` 타일(이전 레벨의 포함 타일)으로 시작합니다. 아래 예에서 와 같이 타일 2는 타일 20부터 23까지의 상위 입니다.

<center>

![쿼드키 타일 피라미드](media/zoom-levels-and-tile-grid/quadkey-tile-pyramid.png)</center>

마지막으로 `quadkeys` XY 공간에서 타일의 근접성을 유지하는 1차원 인덱스 키를 제공합니다. 즉, 근처의 XY 좌표가 있는 두 `quadkeys` 타일에는 일반적으로 비교적 가까운 타일입니다. 이는 인접 타일이 종종 그룹으로 요청되기 때문에 데이터베이스 성능을 최적화하는 데 중요하며 디스크 읽기 횟수를 최소화하기 위해 해당 타일을 동일한 디스크 블록에 유지하는 것이 바람직합니다.

## <a name="tile-math-source-code"></a>타일 수학 소스 코드

다음 샘플 코드는 이 문서에서 설명하는 함수를 구현하는 방법을 보여 줍니다. 이러한 함수는 필요에 따라 다른 프로그래밍 언어로 쉽게 번역할 수 있습니다.
...
> [!NOTE]
> Azure Maps SDK의 대화형 맵 컨트롤에는 지리 공간 위치와 뷰포트 픽셀 간에 변환하기 위한 도우미 기능이 있습니다. 
> - [웹 SDK: 맵 픽셀 및 위치 계산](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map#pixelstopositions-pixel---)

## <a name="next-steps"></a>다음 단계

Azure Maps REST 서비스에서 맵 타일에 직접 액세스합니다.

> [!div class="nextstepaction"]
> [맵 타일 받기](https://docs.microsoft.com/rest/api/maps/render/getmaptile)

> [!div class="nextstepaction"]
> [트래픽 흐름 타일 받기](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficflowtile)

> [!div class="nextstepaction"]
> [교통 사고 타일 받기](https://docs.microsoft.com/rest/api/maps/traffic/gettrafficincidenttile)

지리 공간 개념에 대해 자세히 알아보기:
