...
[다각형 입체 면 계층](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.layer.polygonextrusionlayer?view=azure-maps-typescript-latest) 을 데이터 원본에 연결 합니다. 그런 다음 맵에 로드 합니다. 다각형 입체 면 계층은 `Polygon` 및 `MultiPolygon` 기능의 영역을 입체 도형으로 렌더링 합니다. 다각형 `height` 입체 `base` 면 계층의 및 속성은 입체 도형의 너비와 높이의 기본 거리를 **미터**단위로 정의 합니다. 다음 코드에서는 다각형을 만들고 데이터 소스에 추가 하 고 다각형 입체 면 계층 클래스를 사용 하 여 렌더링 하는 방법을 보여 줍니다.

> [!Note]
> 다각형 `base` 입체 면 계층에 정의 된 값은의 값 보다 작거나 같아야 합니다 `height`.
...
<a href='https://codepen.io'>CodePen</a>의 Azure Maps (<a href='https://codepen.io/azuremaps'>@azuremaps</a>)로 펜 <a href='https://codepen.io/azuremaps/pen/wvvBpvE'>돌출 다각형</a> 을 참조 하세요.</iframe>


## <a name="add-data-driven-polygons"></a>데이터 중심 다각형 추가

다각형 입체 면 레이어를 사용 하 여 choropleth 지도를 렌더링할 수 있습니다. 밀어내기 계층 `height` 의 `fillColor` 및 속성을 `Polygon` 및 `MultiPolygon` 기능 기 하 도형에서 통계 변수의 측정으로 설정 합니다. 다음 코드 샘플은 상태별 모집단 밀도의 측정을 기반으로 하는 choropleth의 돌출 된 지도를 보여 줍니다.
...
<a href='https://codepen.io'>CodePen</a>에서 Azure Maps (<a href='https://codepen.io/azuremaps'>@azuremaps</a>)로 Pen <a href='https://codepen.io/azuremaps/pen/eYYYNox'>돌출 choropleth map</a> 을 참조 하십시오.
</iframe>

## <a name="add-a-circle-to-the-map"></a>맵에 원 추가

Azure Maps에는 [여기](https://docs.microsoft.com/azure/azure-maps/extend-geojson#circle)에 나와 있는 것 처럼 원에 대 한 정의를 제공 하는 GeoJSON 스키마의 확장 된 버전이 사용 됩니다. 의 `Circle` 속성을 `point` `subType` 사용 하 여 기능을 만들고 **미터를 미터**단위로 나타내는 번호가 매겨진 `Radius` 속성을 사용 하 여 입체 원을 렌더링할 수 있습니다. 다음은 그 예입니다.
...
Azure Maps 웹 SDK는 이러한 `Point` 기능을 내부적 `Polygon` 으로 기능으로 변환 합니다. 다음 `Point` 코드 샘플에 표시 된 것 처럼 다각형 입체 면 계층을 사용 하 여 지도에서 이러한 기능을 렌더링할 수 있습니다.
...
<a href='https://codepen.io'>CodePen</a>의 Azure Maps (<a href='https://codepen.io/azuremaps'>@azuremaps</a>)을 통해 펜 <a href='https://codepen.io/azuremaps/pen/zYYYrxo'>drone에 어 스페이스 다각형</a> 을 참조 하세요.
</iframe>
...
<iframe height='700' scrolling='no' title='PoogBRJ' src='//codepen.io/azuremaps/embed/PoogBRJ/?height=700&theme-id=0&default-tab=result' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'><a href='https://codepen.io'>CodePen</a>에서 Azure Maps ()로 Pen<a href='https://codepen.io/azuremaps'>@azuremaps</a> <a href='https://codepen.io/azuremaps/pen/PoogBRJ/'>PoogBRJ</a> ()를 참조 하세요.
</iframe>
