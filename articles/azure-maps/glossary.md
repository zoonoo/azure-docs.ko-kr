...
<a name="advanced-routing"></a>**고급 라우팅**: 연결 가능한 범위 (등시성), 거리 행렬 및 배치 경로 요청 계산과 같이도로 라우팅 데이터를 사용 하 여 고급 작업을 수행 하는 서비스 모음입니다.

<a name="aerial-imagery"></a>**항공 이미지**: [위성 이미지](#satellite-imagery)를 참조 하세요. 

<a name="along-a-route-search"></a>**경로 검색**: 지정 된 우회 시간 내에 있는 데이터 또는 경로 경로에서의 거리를 찾는 공간 쿼리
...
<a name="antimeridian"></a>**Antimeridian**: 180<sup>번째</sup> 자오선는-180도와 경도의 180도를 만족 하는 지점입니다. 지구의 본초자오선과 반대입니다.
...
<a name="bearing"></a>**베어링**: 다른 점을 기준으로 하는 점의 가로 방향입니다. 시계 방향의 0도-360도 사이에서 북쪽을 기준으로 하는 각도로 표현됩니다. 
...
<a name="choropleth-map"></a>**Choropleth map**: 지도에 표시 되는 통계 변수의 측정에 비례하여 영역을 음영 처리 하는 테마 맵입니다. 예를 들어 다른 모든 주에 대한 상대 인구를 기준으로 각 미국 주의 경계 색을 지정합니다.
...
<a name="elevation"></a>**상승**: 참조 서피스나 데이텀 위 또는 아래에 있는 점 또는 개체의 세로 거리 (일반적으로는 해상 수준)입니다. 일반적으로 상승은 토지의 세로 높이를 가리킵니다.

<a name="envelope"></a>**봉투 (Envelope**): [경계 상자](#bounding-box)를 참조 하세요.

<a name="extended-postal-code"></a>**확장 된 우편 번호**: 추가 정보를 포함할 수 있는 우편 번호입니다. 예를 들어 미국에서 우편 번호는 5자리 숫자이지만, 확장된 우편 번호(zip+4라고도 함)는 네 자리 숫자를 추가로 포함합니다. 이러한 추가 자릿수는 도시 블록, 아파트의 그룹 또는 사서함 등 5자리 숫자 배달 영역 내에서 지리적 세그먼트를 식별하는 데 사용됩니다. 그러면 메일을 효율적으로 분류하고 배달하는 데 도움이 됩니다.
...
<a name="fleet-management"></a>**차량 관리**: 자동차, 트럭, 선박 및 평면과 같은 상업 차량 관리 선단 관리에는 차량 비용, 유지 관리, 컴퓨터 통신(추적 및 진단)뿐만 아니라 드라이버, 속도, 연료, 상태 및 안전 관리와 같은 다양한 함수가 포함될 수 있습니다. 선단 관리는 정부 법률에서 규정을 준수하면서 위험을 최소화하고 전반적인 교통 및 직원 비용을 줄이기 위해 해당 비즈니스에서 운송을 사용하는 회사에서 사용하는 프로세스입니다.
...
<a name="landsat"></a>**Landsat**: Multispectral, 농업, 임업 및 지도 제작와 같은 여러 산업에서 사용 되는 육지 이미지를 수집 하는 NASA에서 개발한 위성 orbiting 위성입니다.
...
<a name="multipolygon"></a>**Multipolygon**: polygon 개체의 컬렉션을 나타내는 기 하 도형입니다. 예를 들어 하와이의 경계를 표시하려면 각 섬은 다각형을 사용하여 테두리를 그리므로 하와이의 경계는 MultiPolygon입니다.
...
<a name="postal-code"></a>**우편 번호**: 메일 배달을 간소화 하기 위해 국가/지역의 우편 서비스에서 지리적 영역을 영역으로 나누는 데 사용 하는 일련의 문자, 숫자 또는 둘 다를 특정 형식으로 사용 합니다.
...
<a name="waypoint-optimization"></a>**중간 지점 최적화**: 제공 된 모든 waypoints를 통과 하는 데 필요한 이동 시간이 나 거리를 최소화 하기 위해 waypoints 집합을 다시 정렬 하는 프로세스입니다. 최적화의 복잡성에 따라 [세일즈 외판원 문제](#traveling-salesmen-problem-tsp) 또는 [차량 라우팅 문제](#vehicle-routing-problem-vrp)라고도 합니다.
...
<a name="web-mercator"></a>**웹 Mercator**(구형 Mercator 프로젝션이 라고도 함)는 주로 웹 기반 매핑 프로그램에서 사용 되는 Mercator 프로젝션의 약간 변형입니다. 소규모 지도에 사용되는 표준 메르카토르 도법과 동일한 수식을 사용합니다. 그러나 대규모 메르카토르 지도가 일반적으로 타원의 투영 형식을 사용하는 반면 웹 메르카토르 도법은 모든 규모에서 구형 수식을 사용합니다. 글로벌 규모에서는 다른 점을 감지할 수 없지만 로컬 영역 지도가 같은 규모의 타원 메르카토르 지도에서 약간 벗어나게 됩니다.
...
<a name="Zoom level"></a>**확대/축소 수준**: 세부 수준 및 표시 되는 지도의 크기를 지정 합니다. 수준 0을 모두 확대 하는 경우 전체 세계 맵은 종종 보기에 있지만 국가/지역 이름, 테두리 및 해양 이름과 같은 제한 된 세부 정보를 표시 합니다. 수준 17에 가깝게 확대하면 지도는 자세한 정보를 포함한 몇 가지 도시 블록 영역을 표시합니다. 자세한 내용은 [확대/축소 수준 및 타일 그리드 설명서](zoom-levels-and-tile-grid.md)를 참조하세요.
