...
<a name="boundary"></a> **경계**: 선 또는 다각형 속성, 국가/지역 및 구역 등의 인접 한 정치적 엔터티를 분리 합니다. 경계는 강, 산 또는 벽 등 물리적 물체를 따르거나 따르지 않는 선입니다.
...
<a name="country-code"></a> **국가 번호**: ISO 표준을 기반으로 하는 국가/지역에 대 한 고유 식별자입니다. ISO2는 국가의 두자리 코드(예: US)이고, ISO3은 3자리 코드(예: USA)를 나타냅니다.

<a name="country-subdivision"></a> **국가 구획**: 국가/지역, 시 /도 라고의 첫 번째 수준의 하위 분류 합니다.

<a name="country-secondary-subdivision"></a> **국가 보조 구획**: 두 번째 수준의 하위 구역입니다 국가/지역은 군의 경계 라고 합니다.

<a name="country-tertiary-subdivision"></a> **국가 3차 구획**: 국가/지역, 일반적으로 명명 된 영역을 ward 등의 세 번째 수준의 하위 분류 합니다.
...
<a name="postal-code"></a> **우편 번호**: 일련의 문자, 숫자 또는 메일의 배달 간소화 하기 위해 지역은 영역으로 나누는 데 국가/지역의 우편 서비스에서 사용 하는 특정 서식에서 둘 다.
...
<a name="thematic-maps"></a> **주제도**: 주제도는 지리적 영역에 대한 테마를 반영하도록 만든 간단한 지도입니다. Map의이 형식에 대 한 일반적인 시나리오는 데이터의 일부 메트릭을 기반으로 국가/지역 등 관리 지역 색 방법은입니다.
...
<a name="time-zone"></a> **표준 시간대**: 법률, 상업 및 소셜용으로 균일한 표준 시간대가 관찰되는 전 세계의 지역입니다. 국가/지역 및 해당 하위 구분선의 경계에 따라 표준 시간대는 경향이 있습니다.
...
<a name="Zoom level"></a> **확대/축소 수준**: 세부 정보 및 지도 표시 정도를 지정합니다. 수준 0으로 축소, 전체 세계 지도 표시 되는 경우가 자주 되지만 ocean 이름과 방법과으로 국가/지역 이름 등 테두리 제한 세부 정보를 표시 됩니다. 수준 17에 가깝게 확대하면 지도는 자세한 정보를 포함한 몇 가지 도시 블록 영역을 표시합니다. 자세한 내용은 [확대/축소 수준 및 타일 그리드 설명서](zoom-levels-and-tile-grid.md)를 참조하세요.
