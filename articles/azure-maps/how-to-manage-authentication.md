...
Azure Maps 계정을 만든 후에는 기본 키와 보조 키가 생성 됩니다. 기본 키를 구독 키로 사용 하 고, 경우에 따라 이러한 이름이 교대로 사용 됩니다. 키 롤링 변경 등의 시나리오에서 보조 키를 사용할 수 있습니다. 어느 쪽이 든 Azure Maps를 호출 하는 데 키가 필요 합니다. 이 프로세스를 [공유 키 인증](https://docs.microsoft.com/azure/azure-maps/azure-maps-authentication#shared-key-authentication)이라고 합니다. 공유 키 및 Azure AD 인증에 대해 자세히 알아보려면 [Azure Maps 인증](https://aka.ms/amauth) 을 참조 하세요.
...
1. 포털 메뉴에서 **Azure Active Directory** 를 선택 합니다. 등록에 대 한 이름을 제공 합니다. **앱 등록** 클릭 한 다음 **새 등록**을 클릭 합니다. **URI 리디렉션** 상자에서 웹 앱의 홈 페이지를 제공 합니다. https://localhost/ )을 입력합니다. 등록 된 앱이 이미 있는 경우 2 단계로 이동 합니다.

    ![앱 등록](./media/how-to-manage-authentication/app-registration.png)

    ![앱 등록 세부 정보](./media/how-to-manage-authentication/app-create.png)

2. Azure Maps에 위임 된 API 권한을 할당 하려면 **앱 등록**에서 응용 프로그램으로 이동한 다음 **API 권한**을 선택 합니다. **권한 추가**를 선택 합니다. **API 선택**에서 **Azure Maps** 을 검색 하 고 선택 합니다.

    ![앱 API 권한](./media/how-to-manage-authentication/app-permissions.png)

3. **권한 선택**에서 **사용자 가장**에 대 한 확인란을 선택 하 고 맨 아래에 있는 **선택** 단추를 클릭 합니다.

    ![앱 API 권한 선택](./media/how-to-manage-authentication/select-app-permissions.png)

4. 인증 방법에 따라 a 또는 b 단계를 완료 합니다.

    1. 응용 프로그램에서 Azure Maps 웹 SDK를 사용 하 여 사용자 토큰 인증을 사용 하는 경우 응용 프로그램 등록의 매니페스트 섹션에서 true로 설정 하 여 `oauth2AllowImplicitFlow`를 사용 하도록 설정 합니다.
    
       ![앱 매니페스트](./media/how-to-manage-authentication/app-manifest.png)

    2. 응용 프로그램에서 서버/응용 프로그램 인증을 사용 하는 경우 앱 등록에서 **인증서 & 비밀** 블레이드로 이동 하 여 암호를 만들거나 앱 등록에 공개 키 인증서를 업로드 합니다. 암호를 만드는 경우 나중에 사용 하기 위해 안전 하 게 저장 합니다. 이 암호를 사용 하 여 Azure AD에서 토큰을 가져옵니다.
