---
title: 지도에서 경로 경로 표시 | 마이크로소프트 Azure 지도
description: 이 문서에서는 Microsoft Azure Maps Web SDK를 사용하여 맵에서 두 위치 간의 방향을 표시하는 방법을 알아봅니다.
...
ms.locfileid: "80371419"
---
# <a name="show-directions-from-a-to-b"></a>A에서 B로의 방향 표시

이 문서에서는 경로 요청을 만들고 맵에 경로를 표시하는 방법을 설명합니다.

그렇게 하려면 두 가지 방법이 있습니다. 첫 번째 방법은 서비스 모듈을 통해 [Azure Maps 경로 API](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)를 쿼리하는 것입니다. 두 번째 방법은 [가져오기 API를](https://fetch.spec.whatwg.org/) 사용하여 [Azure Maps 경로 API에](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)대한 검색 요청을 하는 것입니다. 두 방법 모두 아래에 설명되어 있습니다.
...
세 번째 코드 블록은 [DataSource](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.source.datasource?view=azure-iot-typescript-latest) 개체를 생성하고 맵에 추가합니다.

네 번째 코드 블록은 시작 및 끝 [점](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data.point?view=azure-iot-typescript-latest) 개체를 만들고 dataSource 개체에 추가합니다.

선은 LineString의 [피쳐입니다.](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data.feature?view=azure-iot-typescript-latest) [LineLayer](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.layer.linelayer?view=azure-iot-typescript-latest)는 [DataSource](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.source.datasource?view=azure-iot-typescript-latest)에 래핑된 선 개체를 지도에 선으로 렌더링합니다. 네 번째 코드 블록은 선 계층을 만들고 지도에 추가합니다. [LinestringLayerOptions](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.linelayeroptions?view=azure-iot-typescript-latest)에서 선 계층의 속성을 참조하세요.

[심볼 레이어는](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.layer.symbollayer?view=azure-iot-typescript-latest) 텍스트 또는 아이콘을 사용하여 [DataSource](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.source.datasource?view=azure-iot-typescript-latest)에 래핑된 포인트 기반 데이터를 렌더링합니다. 텍스트 또는 아이콘은 맵에서 기호로 렌더링됩니다. 다섯 번째 코드 블록은 맵에 기호 레이어를 만들고 추가합니다.

코드의 여섯 번째 블록은 [서비스 모듈의](how-to-use-services-module.md)일부인 Azure Maps 라우팅 서비스를 쿼리합니다. [RouteURL의 routeDirections](https://docs.microsoft.com/javascript/api/azure-maps-rest/atlas.service.routeurl?view=azure-iot-typescript-latest#methods) 메서드 계산은 시작점과 끝점 사이의 경로를 얻는 데 사용됩니다. 응답에서 GeoJSON 기능 컬렉션 메서드를 `geojson.getFeatures()` 사용 하 여 추출 하 고 데이터 원본에 추가 됩니다. 그런 다음, 응답을 지도에 경로로 렌더링합니다. 지도에 선을 추가하는 방법에 대한 자세한 내용은 [지도에 선 추가](map-add-line-layer.md)를 참조하세요.

코드의 마지막 블록은 맵의 [setCamera](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#setcamera-cameraoptions---cameraboundsoptions---animationoptions-) 속성을 사용하여 맵의 경계를 설정합니다.

경로 쿼리, 데이터 원본, 기호, 선 레이어 및 카메라 경계는 [이벤트 수신기](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#events)내부에 만들어집니다. 이 코드 구조는 맵이 완전히 로드된 후에만 결과가 표시됩니다.

## <a name="query-the-route-via-fetch-api"></a>가져오기 API를 통해 경로 쿼리
...
세 번째 코드 블록은 경로의 시작 및 대상 지점을 만듭니다. 그런 다음 데이터 원본에 추가합니다. [addPins](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest) 사용 방법에 대한 지침은 [지도에 핀 추가](map-add-pin.md)를 참조하세요.
...
코드의 마지막 블록은 [가져오기 API를](https://fetch.spec.whatwg.org/) 사용하여 [Azure Maps 경로 API에](https://docs.microsoft.com/rest/api/maps/route/getroutedirections)대한 검색 요청을 합니다. 그런 다음 응답이 구문 분석됩니다. 응답이 성공하면 위도 및 경도 정보를 사용하여 해당 점을 연결하여 배열을 한 줄로 만듭니다. 그런 다음 선 데이터가 데이터 원본에 추가되어 맵에서 경로를 렌더링합니다. 지침은 [맵에 선 추가](map-add-line-layer.md)를 참조하세요.

경로 쿼리, 데이터 원본, 기호, 선 레이어 및 카메라 경계는 [이벤트 수신기](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#events)내부에 만들어집니다. 다시 말하지만 맵이 완전히 로드된 후에 결과가 표시되도록 하려고 합니다.

## <a name="next-steps"></a>다음 단계

> [!div class="nextstepaction"]
> [라우팅 서비스 사용에 대한 모범 사례](how-to-use-best-practices-for-search.md)
...
> [!div class="nextstepaction"]
> [맵과의 상호 작용 - 마우스 이벤트](./map-events.md)
