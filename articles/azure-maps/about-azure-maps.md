...
Azure Maps는 키 기반 인증 체계를 사용합니다. 계정에는 미리 생성된 두 개의 키가 함께 제공됩니다. 두 키 중 하나를 Azure Maps 요청에 사용하여 이러한 위치 기능을 응용 프로그램에 직접 통합하세요.
...
현재 IP 주소를 확인하고 IP 주소의 위치가 지원되지 않는 아래 국가 중 하나에 있지 않은지 확인합니다.
