...
> [!TIP]
> 기본적으로 열 지도 레이어는 데이터 원본에 있는 모든 도형의 좌표를 렌더링합니다. 계층이 지점 기하 도형만 렌더링하도록 제한하려면 계층의 `filter` 속성을 `['==', '$type', 'Point']`로 설정합니다.
...
* `radius`: 각 데이터 요소를 렌더링할 픽셀 반경을 정의합니다. 반경은 정수 또는 식으로 설정할 수 있습니다. 식을 사용하여 지도에서 일관적인 공간 영역을 나타내는 확대/축소 수준에 따라 반경을 확대/축소할 수 있습니다(예: 5마일 반경).
* `color`: 열 지도에 색이 지정되는 방식을 지정합니다. 그라데이션 색상표는 열 지도에 자주 사용되지만, 열 지도를 더 외곽선 데이터처럼 보이게 하고 싶다면 계단식 색상표도 유용합니다. 이러한 색상표는 색을 최소에서 최대 밀도 값까지 정의합니다. 열 지도에 대한 색 값은 `heatmap-density` 값에서 식으로 지정됩니다. 그라데이션 식에서 인덱스 0의 색 또는 단계 색의 기본 색은 데이터가 없는 영역의 색을 정의하며 배경색을 정의하는 데 사용할 수 있습니다. 대부분 이 값을 투명 또는 반투명 검은색으로 설정하는 것을 선호합니다. 색 식의 예가 있습니다.
...
> [!TIP]
> 데이터 원본에서 클러스터링을 사용하도록 설정하면 서로 인접한 요소는 클러스터된 요소로 그룹화됩니다. 각 클러스터의 요소 수를 열 지도에 대한 가중치 식으로 사용하면 렌더링해야 할 요소 수를 크게 줄일 수 있습니다. 한 클러스터의 요소 수는 요소 기능의 `point_count property` 속성에 저장됩니다(아래 그림 참조). 
> ```JavaScript
