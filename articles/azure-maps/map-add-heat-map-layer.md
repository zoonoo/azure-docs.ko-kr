...
ms.locfileid: "75408533"
---
# <a name="add-a-heat-map-layer"></a>열 지도 계층 추가

점 밀도 맵이 라고도 하는 열 맵은 색 범위를 사용 하 여 데이터 밀도를 나타내는 데 사용 되는 데이터 시각화 유형입니다. 이러한 데이터는 종종 맵에 데이터 "핫 스폿"을 표시 하는 데 사용 되며 큰 점 데이터 집합을 렌더링 하는 좋은 방법입니다. 

예를 들어 지도 보기 내에서 수십 개의 요소를 기호로 렌더링 하면 대부분의 맵 영역이 포함 됩니다. 이로 인해 많은 기호가 서로 겹칠 수 있으므로 데이터에 대 한 정보를 크게 파악 하기가 어려워집니다. 그러나이 데이터 집합을 열 맵으로 시각화 하면 점 데이터가 densest 위치 및 다른 영역에 대 한 상대 밀도를 쉽게 확인할 수 있습니다.

다음과 같은 다양 한 시나리오에서 열 지도를 사용할 수 있습니다.

- **온도 데이터**: 두 데이터 요소 사이의 온도에 대 한 근사치을 제공 합니다.
- **노이즈 센서에 대 한 데이터**: 센서가 있는 노이즈의 강도 뿐만 아니라 거리에 대 한 자세한 정보를 제공할 수도 있습니다. 한 사이트의 노이즈 수준이 높지 않을 수 있습니다. 그러나 여러 센서의 노이즈 범위 영역이 겹치면이 겹치는 영역에서 더 높은 의미의 소음을 겪을 수 있으므로 열 지도에 표시 될 수 있습니다.
- **GPS 추적**: 속도를 가중치 높이 맵으로 포함 합니다. 여기서 각 데이터 요소의 강도는 속도를 기반으로 합니다. 예를 들어이는 차량의 속도를 높여 주는 위치를 확인 하는 방법을 제공 합니다.
...
다음 코드에서 각 열 점은 모든 확대/축소 수준에서 10 픽셀의 반지름을 갖습니다. 지도에 열 지도 계층을 추가 하는 경우이 샘플은 더 나은 사용자 환경을 만들기 위해 레이블 계층 아래에 삽입 합니다. 레이블은 열 지도 위에 명확 하 게 표시 됩니다. 이 샘플의 데이터는 [USGS 지진 위험 프로그램](https://earthquake.usgs.gov/)을 기반으로 하며 지난 30 일 동안 발생 한 중요 한 지진를 나타냅니다.
...
* `color`: 열 지도의 색이 지정 되는 방법을 지정 합니다. 색 그라데이션은 열 지도의 일반적인 기능이 며 `interpolate` 식으로 효과를 달성할 수 있습니다. 열 지도를 색으로 표시 하는 데 `step` 식을 사용할 수도 있습니다. 밀도를 윤곽선 또는 방사형 스타일 맵과 비슷한 범위로 시각적으로 나눕니다. 이러한 색상표는 색을 최소에서 최대 밀도 값까지 정의합니다. 

  열 맵의 색 값을 `heatmap-density` 값에 대 한 식으로 지정 합니다. 보간 식의 인덱스 0에 있는 색 또는 단계 식의 기본 색은 데이터가 없는 영역의 색을 정의 합니다. 배경색을 정의 하는 데 사용할 수 있습니다. 일반적으로이 값은 투명 또는 반투명 검정으로 설정 됩니다. 
...
- `intensity`: 각 데이터 요소의 가중치에 승수를 적용 하 여 열 지도의 전체 강도를 늘립니다. 이렇게 하면 데이터 요소 가중치의 작은 차이를 쉽게 시각화할 수 있습니다.
- `weight`: 기본적으로 모든 데이터 요소의 가중치는 1 이며 동일 하 게 가중치가 적용 됩니다. Weight 옵션은 승수 역할을 하며 숫자 또는 식으로 설정할 수 있습니다. 숫자 (예: 2)가 가중치로 설정 된 경우에는 맵의 각 데이터 요소를 두 번 배치 하 여 밀도가 배가 되는 것과 동일 합니다. 숫자가 가중치 옵션을 설정하면 밀도 옵션을 사용하는 것과 비슷한 방식으로 열 지도를 렌더링합니다. 

  그러나 식을 사용 하는 경우 각 데이터 요소의 가중치는 각 데이터 요소의 속성을 기반으로 할 수 있습니다. 예를 들어 각 데이터 요소가 지진을 나타낸다고 가정 합니다. 각 지진 데이터 요소에 있는 중요 한 메트릭은 크기 값입니다. 지진은 항상 발생 하지만 대부분은 낮은 크기를 가지 며, 심지어는 그렇지 않습니다. 식의 크기 값을 사용 하 여 각 데이터 요소에 가중치를 할당 하면 열 지도 내에서 지진의 의미를 보다 잘 표현할 수 있습니다.
...
각 데이터 요소가 지도의 동일한 실제 영역을 포함 하도록 각 확대/축소 수준에 대 한 반지름의 크기를 조정 하려면 `zoom` 식을 사용 합니다. 이렇게 하면 열 지도 계층이 더 정적이 고 일관 되 게 보입니다. 지도의 각 확대/축소 수준에는 이전 확대/축소 수준과 가로 및 세로로 두 배의 픽셀이 있습니다. 

각 확대/축소 수준에서 두 배가 되도록 반지름의 크기를 조정 하면 모든 확대/축소 수준에서 일치 하는 열 지도를 만듭니다. 이렇게 하려면 다음 샘플과 같이 기본 2 `exponential interpolation` 식과 함께 `zoom`를 사용 합니다. 지도를 확대 하 여 확대/축소 수준으로 열 지도의 크기를 조정 하는 방법을 확인 합니다.
...
> [!TIP]
> 데이터 원본에서 클러스터링을 사용 하도록 설정 하면 서로 가까운 점이 클러스터형 점으로 그룹화 됩니다. 각 클러스터의 점 개수를 열 지도에 대 한 가중치 식으로 사용 하 고 렌더링 해야 하는 점 수를 크게 줄일 수 있습니다. 클러스터의 점 개수는 point 기능의 `point_count` 속성에 저장 됩니다. 
> ```JavaScript
> var layer = new atlas.layer.HeatMapLayer(datasource, null, {
>    weight: ['get', 'point_count']
> });
> ```
> 클러스터링 반지름이 몇 픽셀만 있으면 렌더링에 시각적 차이가 거의 없습니다. 더 큰 radius 그룹은 각 클러스터에 대 한 더 많은 요소를 추가 하 고 열 지도의 성능을 향상 시킵니다.
