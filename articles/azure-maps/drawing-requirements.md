...
| 기능 | 더 많은 메타 데이터 정보와 함께 기 하 도형을 결합 하는 개체입니다. |
...
* 그리기 패키지의 DWG 파일을 설명 하는 파일 _에 대 한manifest.js_ 입니다.

그리기 패키지는 .zip 확장명을 사용 하 여 단일 보관 파일에 압축 되어야 합니다. DWG 파일은 패키지 내에서 어떤 방식으로든 구성 될 수 있지만 매니페스트 파일은 압축 된 패키지의 루트 디렉터리에 있어야 합니다. 다음 섹션에서는 DWG 파일, 매니페스트 파일 및 이러한 파일의 내용에 대한 요구 사항을 자세히 설명합니다.
...
* 는 그리기 패키지의 다른 DWG 파일과 동일한 측정 시스템 및 측정 단위를 참조 해야 합니다.
...
* 자체 교차 다각형이 자동으로 복구 되 고 [Azure Maps 변환 서비스](/rest/api/maps/conversion) 에서 경고가 발생 합니다. 예상한 결과와 일치 하지 않을 수 있으므로 복구 된 결과를 수동으로 검사 하는 것이 좋습니다.

모든 레이어 엔터티는 선, 다중선, 다각형, 원호, 원, 타원 (닫힌) 또는 텍스트 (한 줄) 형식 중 하나 여야 합니다. 다른 모든 엔터티 형식은 무시 됩니다.

아래 표에서는 각 계층에 대해 지원 되는 엔터티 형식 및 변환 된 맵 기능을 간략하게 설명 합니다. 계층에 지원 되지 않는 엔터티 형식이 포함 된 경우 [Azure Maps 변환 서비스](/rest/api/maps/conversion) 는 이러한 엔터티를 무시 합니다.  

| 계층 | 엔터티 형식 | 변환 된 기능 |
| :----- | :-------------------| :-------
| [실외](#exterior-layer) | 다각형, 다중선 (닫힘), 원, 타원 (닫힘) | Levels
| [단위](#unit-layer) |  다각형, 다중선 (닫힘), 원, 타원 (닫힘) | 세로 penetrations, 단위
| [벽](#wall-layer)  | 다각형, 다중선 (닫힘), 원, 타원 (닫힘) | 해당 사항 없음 자세한 내용은 [벽 레이어](#wall-layer)를 참조하세요.
| [문](#door-layer) | 다각형, 폴리라인(닫힌), 선, 원호(CircularArc), 원 | 통로
| [영역](#zone-layer) | 다각형, 다중선 (닫힘), 원, 타원 (닫힘) | 영역
...
각 수준에 대 한 DWG 파일은 단위를 포함 하는 계층을 정의 합니다. 단위는 사무실, 복도, 계단 및 엘리베이터와 같은 건물 내에서 탐색 가능한 공간입니다. `VerticalPenetrationCategory`속성이 정의 된 경우 엘리베이터 및 계단과 같은 여러 수준에 걸친 탐색 가능한 단위는 수직 침투 기능으로 변환 됩니다. 서로 겹치는 수직 침투 기능은 하나 할당 됩니다 `setid` .

단위 레이어는 다음 요구 사항을 따라야 합니다.

* 단위는 다각형, 다중선 (닫힌), 원 또는 타원 (닫힌)으로 그려야 합니다.
...
* 벽은 다각형, 다중선 (닫힌), 원 또는 타원 (닫힌)으로 그려야 합니다.
...
각 수준의 DWG 파일에는 영역의 실제 익스텐트를 정의 하는 영역 계층이 포함 될 수 있습니다. 영역은 이름이 지정 되 고 렌더링 될 수 있는 탐색 불가능 공간입니다. 영역은 여러 수준으로 확장 될 수 있으며 zoneSetId 속성을 사용 하 여 함께 그룹화 됩니다.

* 영역은 다각형, 폴리라인 (닫힘) 또는 타원 (닫힌)으로 그려야 합니다.
* 영역이 겹칠 수 있습니다.
...
각 수준의 DWG 파일에는 사업부 레이블 계층이 포함 될 수 있습니다. Unit 레이블 계층은 Unit 계층에서 추출 된 단위에 이름 속성을 추가 합니다. 이름 속성이 있는 단위는 매니페스트 파일에 더 많은 세부 정보를 지정할 수 있습니다.
...
각 수준의 DWG 파일에는 ZoneLabel 계층이 포함 될 수 있습니다. 이 레이어는 이름 속성을 영역 레이어에서 추출된 영역에 추가합니다. 이름 속성이 있는 영역에는 매니페스트 파일에 지정 된 세부 정보가 포함 될 수 있습니다.
...
| `unitProperties` | false | 장치 기능에 대 한 메타 데이터를 더 삽입 하는 데 사용할 수 있습니다. |
| `zoneProperties` | false | 영역 기능에 대 한 메타 데이터를 더 삽입 하는 데 사용할 수 있습니다. |
...
| `isOpenArea` | bool | false | 단위에 연결 된 열을 요구 하지 않고 탐색 에이전트가 단위를 입력할 수 있습니다. 기본적으로이 값은 여가 `true` 없는 단위에 대해로 설정 되 고, 여는 단위에 대해로 설정 됩니다 `false` . 시작 하 `isOpenArea` 는 `false` 에이전트는 결과 단위에 연결할 수 없기 때문에 시작이 없는 단위에서 수동으로를로 설정 하면 경고가 발생 합니다.|
...
다음은 샘플 드로잉 패키지에 대 한 매니페스트 파일입니다. 전체 패키지를 다운로드 하려면 [샘플 그리기 패키지](https://github.com/Azure-Samples/am-creator-indoor-data-examples)를 참조 하세요.
