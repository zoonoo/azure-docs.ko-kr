...
This article shows you how to use [map class events](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?#events) property to highlight events on the map and on different layers of the map. 또한 map 클래스 events 속성을 사용하여 HTML 표식과 상호 작용할 때 발생하는 이벤트를 강조 표시하는 방법을 보여 줍니다.

## <a name="interact-with-the-map"></a>맵과 상호 작용

Play with the map below, and see the corresponding mouse events highlighted on the right. **JS 탭**을 클릭하면 JavaScript 코드를 보고 편집할 수 있습니다. **CodePen에서 편집** 단추를 클릭하고 CodePen에서 코드를 편집할 수도 있습니다.
...
The following code adds Javascript map events to an HTML marker. 또한 HTML 표식과 상호 작용할 때 발생한 이벤트의 이름을 강조 표시합니다.
