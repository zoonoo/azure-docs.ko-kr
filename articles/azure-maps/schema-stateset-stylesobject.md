...
는 `StyleObject` [`BooleanTypeStyleRule`](#booleantypestylerule) 또는입니다 [`NumericTypeStyleRule`](#numerictypestylerule) .
...
| 속성 | 유형 | Description | 필수 |
|-----------|----------|-------------|-------------|
| `minimum` | double | X가 ≥ 모든 숫자 x입니다 `minimum` .| 예 |
| `maximum` | double | X가 ≤ 인 모든 숫자 x입니다 `maximum` . | 예 |
| `exclusiveMinimum` | double | X > 된 모든 숫자 x입니다 `exclusiveMinimum` .| 예 |
| `exclusiveMaximum` | double | X < 된 모든 숫자 x입니다 `exclusiveMaximum` .| 예 |
...
| 속성 | 유형 | Description | 필수 |
|-----------|----------|-------------|-------------|
| `keyName` | 문자열 |  *상태* 또는 동적 속성 이름입니다.  는 `keyName` 스타일 배열 내에서 고유 해야 합니다.| 예 |
