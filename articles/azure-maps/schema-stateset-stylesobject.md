...
| `rules` | [`BooleanRuleObject`](#booleanruleobject)비슷합니다| 및 상태 값에 대 한 색이 있는 부울 쌍 `true` `false`  입니다.| 예 |
