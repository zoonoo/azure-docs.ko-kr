...
ms.locfileid: "75910751"
---
# <a name="migrate-web-service-from-google-maps"></a>Google Maps에서 웹 서비스 마이그레이션

Azure 및 Google Maps 둘 다 REST 웹 서비스를 통해 공간 API에 액세스할 수 있습니다. 이러한 플랫폼의 API 인터페이스는 비슷한 기능을 수행하지만 서로 다른 명명 규칙과 응답 개체를 사용합니다.

다음 표에는 나열된 Google Maps 서비스 API와 유사한 기능을 제공하는 Azure Maps 서비스 API가 정리되어 있습니다.
...
- 상승
- 지리적 위치
- 위치 세부 정보 및 사진. Azure Maps 검색 API에서 사용할 수 있는 전화 번호 및 웹 사이트 URL입니다.
- 맵 URL
- 도로 – 속도 제한 데이터는 Azure Maps의 경로 및 역방향 지오코딩 API를 통해 사용할 수 있습니다.
- 정적 주소 보기
...
지오코딩은 주소(예: "1 Microsoft way, Redmond, WA")를 좌표(예: 경도: -122.1298, 위도: 47.64005)로 변환하는 프로세스입니다. 그 후 주로 좌표를 사용하여 맵에서 표식 위치를 지정하거나 지도를 가운데에 맞춥니다.

Azure Maps는 다음과 같은 주소를 지오코딩하는 여러 가지 방법을 제공합니다.

- [**자유 형식 주소 지오코딩**](https://docs.microsoft.com/rest/api/maps/search/getsearchaddress): 단일 주소 문자열(예: "1 Microsoft way, Redmond, WA")을 지정하고 요청을 즉시 처리합니다. 개별 주소를 신속하게 지오코딩해야 하는 경우에 추천하는 방법입니다.
- [**정형 주소 지오코딩**](https://docs.microsoft.com/rest/api/maps/search/getsearchaddressstructured): 도로명, 구/군/시, 국가, 우편 번호 등의 단일 주소 부분을 지정하고 요청을 즉시 처리합니다. 개별 주소를 신속하게 지오코딩하고 데이터를 개별 주소 부분으로 구문 분석해야 하는 경우에 추천하는 방법입니다.
...
> [!TIP]
> 요청 URL에 `&amp;typeahead=true`를 추가하여 자동 완성 모드에서 자유 형식 주소 지오코딩 및 유사 항목 검색 API를 사용할 수 있습니다. 이렇게 하면 입력 텍스트가 일부일 가능성이 높다는 사실을 서버에 알리고 자동 완성 모드로 전환됩니다.
...
Azure Maps 역방향 지오코딩 API에는 Google Maps에 없는 다음과 같은 추가 기능이 있으며, 이러한 기능은 앱을 마이그레이션할 때 통합에 유용하게 사용할 수 있습니다.
...
Google Maps에서는 장소 검색 API를 사용하여 관심 지점 데이터를 검색할 수 있습니다. 이 API는 관심 지점을 검색하는 다음 세 가지 방법을 제공합니다.

- **텍스트에서 장소 찾기:** 이름, 주소 또는 전화 번호를 기반으로 관심 지점을 검색합니다.
- **주변 검색**: 한 위치에서 특정 거리 내에 있는 관심 지점을 검색합니다.
- **텍스트 검색:** 관심 지점 및 위치 정보를 포함하는 자유 형식 텍스트를 사용하여 장소를 검색합니다. 예: "뉴욕의 피자 가게" 또는 "메인 스트리트 근처의 식당".
...
현재 Azure Maps에는 Google Maps 텍스트 검색 API와 비슷한 API가 없습니다.

> [!TIP]
> 요청 URL에 `&amp;typeahead=true`를 추가하여 자동 완성 모드에서 POI 검색, POI 범주 검색 및 유사 항목 검색 API를 사용할 수 있습니다. 이렇게 하면 입력 텍스트가 일부일 가능성이 높다는 사실을 서버에 알리고 자동 완성 모드로 전환됩니다.

[검색 모범 사례](how-to-use-best-practices-for-search.md) 설명서를 검토하세요.

### <a name="find-place-from-text"></a>텍스트에서 장소 찾기

이름 또는 주소로 관심 지점을 찾기 위해 Azure Maps [POI 검색](https://docs.microsoft.com/rest/api/maps/search/getsearchpoi) 및 [유사 항목 검색](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy) API를 사용할 수 있습니다.
...
Azure Maps에서 [주변 검색](https://docs.microsoft.com/rest/api/maps/search/getsearchnearby) API를 사용하여 가까운 관심 지점을 검색할 수 있습니다.
...
Azure Maps를 사용하여 경로 및 방향을 계산할 수 있습니다. Azure Maps에는 다음과 같이 Google Maps 라우팅 서비스와 동일한 여러 기능이 있습니다.

- 도착 및 출발 시간
- 실시간 및 예측 기반 트래픽 경로
- 다양한 교통 모드(자동차, 도보, 자전거)
...
Azure Maps 경로 API에는 Google Maps에 없는 다음과 같은 여러 추가 기능이 있으며, 이러한 기능은 앱을 마이그레이션할 때 통합에 유용하게 사용할 수 있습니다.
...
- 상용차 경로 매개 변수(차량 크기, 무게, 악셀 수, 화물 유형)를 지원합니다.
- 최대 차량 속도를 지정합니다.

그 외에도 Azure Maps의 경로 서비스는 등시성이라고도 하는 [라우팅 가능한 범위를 계산](https://docs.microsoft.com/rest/api/maps/route/getrouterange)할 수 있으며, 지정된 시간 또는 연료/충전량 내에서 원점으로부터 어느 방향으로나 이동할 수 있는 영역을 포함하는 다각형을 생성합니다.

## <a name="retrieve-a-map-image"></a>맵 이미지 검색

Azure Maps는 데이터가 오버레이된 정적 맵 이미지를 렌더링하기 위한 API를 제공합니다. Azure Maps [맵 이미지 렌더링](https://docs.microsoft.com/rest/api/maps/render/getmapimagerytile) API는 Google Maps의 정적 맵 API와 비슷합니다.

> [!NOTE]
> Azure Maps는 중심, 모든 표식 및 경로 위치가 "경도,위도" 형식의 좌표여야 하는 반면, Google Maps는 "위도,경도" 형식을 사용합니다. 가장 먼저 주소를 지오코딩해야 합니다.
...
> [!NOTE]
> Azure Maps는 타일 크기가 Google Maps에 사용되는 지도 타일의 두 배인 타일 시스템을 사용합니다. 따라서 Azure Maps의 확대/축소 수준 값은 Google Maps와 비교할 때 Azure Maps에서 한 수준 더 크게 표시됩니다. 이를 보정하려면 마이그레이션하는 요청에서 확대/축소 수준을 한 단계 낮춥니다.
...
> [!TIP]
> 많은 Google Maps 애플리케이션이 수년 전에 비용 절감을 위해 대화형 맵 환경에서 정적 맵 이미지로 전환되었습니다. Azure Maps는 지도 타일 부하에 따라 요금이 청구되므로 웹 SDK에서 대화형 지도 컨트롤을 사용하는 것이 훨씬 더 비용 효율적인 경우가 많습니다. Azure Maps의 지도 타일은 크기가 크며 몇 개만 있으면 정적 맵과 동일한 맵 보기를 다시 만들 수 있는 경우가 자주 있습니다. 브라우저에서 지도 타일을 자동으로 캐시하기 때문입니다. 따라서 대화형 지도 컨트롤은 종종 정적 맵 보기를 다시 만들 때 트랜잭션의 일부만 생성합니다. 지동 및 확대/축소는 더 많은 타일을 로드하지만, 원한다면 이 동작을 해제할 수 있는 옵션이 지도 컨트롤에 있습니다. 또한 대화형 지도 컨트롤은 정적 맵 서비스보다 훨씬 많은 시각화 옵션을 제공합니다.

### <a name="marker-url-parameter-format-comparison"></a>표식 URL 매개 변수 형식 비교

**이전: Google Maps**

Google Maps에서는 URL에 `markers` 매개 변수를 사용하여 정적 맵 이미지에 표식을 추가할 수 있습니다. `markers` 매개 변수는 아래와 같이 맵에 렌더링할 스타일 및 위치 목록을 가져옵니다.

```
&markers=markerStyles|markerLocation1|markerLocation2|...
```

다른 스타일 및 위치 세트를 사용하여 URL에 `markers` 매개 변수를 추가하는 방법으로 추가 스타일을 사용할 수 있습니다.

표식 위치는 "위도,경도" 형식으로 지정됩니다.

Google Maps의 표식 스타일은 `optionName:value` 형식으로 추가되며, "optionName1:value1\|optionName2:value2"처럼 여러 스타일이 파이프(\|) 문자로 구분됩니다. 옵션 이름 및 값은 콜론(:)으로 구분됩니다. 다음 스타일 옵션 이름을 사용하여 Google Maps에서 표식 스타일을 지정할 수 있습니다.
...
다음 스타일 옵션 이름을 사용하여 Google Maps에서 사용자 지정 아이콘을 사용할 수도 있습니다.
...
예를 들어 Google Maps에서 다음 URL 매개 변수를 사용하여 빨간색의 중간 크기 표식을 맵의 좌표(경도: -110, 위도: 45)에 추가할 수 있습니다.
...
Google Maps에서는 URL에 `pins` 매개 변수를 지정하여 정적 맵 이미지에 표식을 추가할 수 있습니다. Google Maps와 마찬가지로, 이 매개 변수에서는 스타일 및 위치 목록을 지정할 수 있으며, `pins` 매개 변수를 여러 번 지정하여 여러 스타일의 표식을 지원할 수 있습니다.
...
핀 위치의 경우 Azure Maps는 좌표가 "경도 위도" 형식이어야 하는 반면, Google Maps는 "위도,경도" 형식을 사용합니다. 또한 Azure Maps의 쉼표로 분리된 경도 및 위도가 아니라 공백이 있습니다.
...
Azure Maps의 핀 스타일은 `optionNameValue` 형식으로 추가되며, `iconType|optionName1Value1|optionName2Value2`처럼 여러 스타일이 파이프(\|) 문자로 구분됩니다. 옵션 이름과 값은 구분되지 않습니다. 다음 스타일 옵션 이름을 사용하여 Azure Maps에서 표식 스타일을 지정할 수 있습니다.

- `al` – 표식의 불투명도(알파)를 지정합니다. 0~1 사이의 숫자입니다.
...
위치 목록의 모든 표식에 적용되는 단일 레이블 값을 갖는 것이 아니라 각 핀의 레이블 값이 지정됩니다. 레이블 값은 여러 문자로 된 문자열일 수 있으며, 스타일 또는 위치 값으로 잘못 읽지 않도록 작은따옴표로 래핑됩니다.
...
Google Maps에서는 URL에 `path` 매개 변수를 사용하여 정적 맵 이미지에 선 및 다각형을 추가할 수 있습니다. `path` 매개 변수는 아래와 같이 맵에 렌더링할 스타일 및 위치 목록을 가져옵니다.

```
&path=pathStyles|pathLocation1|pathLocation2|...
```

다른 스타일 및 위치 세트를 사용하여 URL에 `path` 매개 변수를 추가하는 방법으로 추가 스타일을 사용할 수 있습니다.

Google Maps의 경로 위치는 `latitude1,longitude1|latitude2,longitude2|…`형식으로 지정됩니다. 경로는 인코딩할 수도 있고, 지점의 주소를 포함할 수도 있습니다.

Google Maps의 경로 스타일은 `optionName:value` 형식으로 추가되며, `optionName1:value1|optionName2:value2`처럼 여러 스타일이 파이프(\|) 문자로 구분됩니다. 옵션 이름 및 값은 콜론(:)으로 구분됩니다. 다음 스타일 옵션 이름을 사용하여 Google Maps에서 스타일 경로를 지정할 수 있습니다.
...
예를 들어 Google Maps에서 불투명도가 50%이고 두께가 4픽셀인 빨간색 선은 다음 URL 매개 변수를 사용하여 좌표(경도: -110, 위도: 45 및 경도: -100, 위도: 50) 사이의 맵에 추가할 수 있습니다.
...
Azure Maps에서는 URL에 `path` 매개 변수를 지정하여 정적 맵 이미지에 선과 다각형을 추가할 수 있습니다. Google Maps와 마찬가지로, 이 매개 변수에서는 스타일 및 위치 목록을 지정할 수 있으며, `path` 매개 변수를 여러 번 지정하여 여러 가지 스타일로 여러 원, 선 및 다각형을 렌더링할 수 있습니다.

```
&path=pathStyles||pathLocation1|pathLocation2|...
```

경로 위치의 경우 Azure Maps는 좌표가 "경도 위도" 형식이어야 하는 반면, Google Maps는 "위도,경도" 형식을 사용합니다. 또한 Azure Maps의 쉼표로 분리된 경도 및 위도가 아니라 공백이 있습니다. Azure Maps는 요소에 대해 인코딩된 경로 또는 주소를 지원하지 않습니다. [여기](how-to-render-custom-data.md#get-data-from-azure-maps-data-storage)에 설명된 것처럼 큰 데이터 세트를 Azure Maps 데이터 스토리지 API에 GeoJSON 채우기로 업로드할 수 있습니다.
...
예를 들어 Azure Maps에서 불투명도가 50%이고 두께가 4픽셀인 빨간색 선은 다음 URL 매개 변수를 사용하여 좌표(경도: -110, 위도: 45 및 경도: -100, 위도: 50) 사이의 맵에 추가할 수 있습니다.
...
Azure Maps는 위치 세트 간의 이동 시간 및 거리를 거리 행렬로 계산할 수 있는 API를 제공합니다. Azure Maps 거래 행렬 API는 Google Maps의 거리 행렬 API와 유사합니다.

- [**경로 행렬**](https://docs.microsoft.com/rest/api/maps/route/postroutematrixpreview): 출발지 및 목적지 세트 간의 이동 시간 및 거리를 비동기적으로 계산합니다. 요청당 최대 700개 셀을 지원합니다(출발지 수 * 목적지 수). 이 제약 조건을 고려할 때 구현 가능한 행렬 크기는 700x1, 50x10, 10x10, 28x25, 10x70입니다.
...
Azure Maps는 좌표가 속한 표준 시간대를 검색할 수 있는 API를 제공합니다. Azure Maps 표준 시간대 API는 Google Maps의 표준 시간대 API와 비슷합니다.
...
다음은 Azure Maps REST 서비스에 대한 추가 설명서 및 리소스입니다.
