<properties title="How to Configure PHP in Azure Websites" pageTitle="Azure 웹 사이트에서 PHP를 구성하는 방법" metaKeywords ="Azure, Azure 웹 사이트, 구성, PHP" description="Learn how to configure the default PHP installation or add a custom PHP installation in Azure Websites." services="Web Sites" documentationCenter="PHP" authors="cephalin" manager="wpickett" />
...
Azure 웹 사이트를 사용하면 Azure에서 확장성이 뛰어난 웹 사이트를 구축할 수 있습니다. 소규모로 시작했다가 트래픽이 증가함에 따라 확장할 수 있는 고확장성 클라우드 환경에 빠르고 쉽게 사이트를 배포할 수 있습니다. Azure 웹 사이트는 사용자가 선택한 언어 및 오픈 소스 앱을 사용하고 Git, FTP 및 TFS를 통해 배포할 수 있도록 지원합니다. MySQL, SQL 데이터베이스, 캐싱, CDN, 저장소 등 기타 서비스를 쉽게 통합할 수 있습니다.
...
1. `php.ini` 파일에 사용한 것과 동일한 구문을 사용하여 구성 설정을 '.user.ini' 파일에 추가합니다. 예를 들어 'display_errors' 설정을 on으로 전환하고 'upload_max_filesize' 설정을 10M로 설정하려면 '.user.ini' 파일에 다음 텍스트를 포함합니다.
...
`.user.ini` 파일을 사용하는 대신 스크립트에 [ini_set()] 함수를 사용하여 시스템 수준 지시문이 아닌 구성 옵션을 설정할 수 있습니다.

<h2><a name="EnableExtDefaultPHP"></a>방법: 기본 PHP 런타임에서 확장 사용</h2>
이전 섹션에서 설명한 대로 기본 PHP 버전, 관련 기본 구성 및 사용하도록 설정된 확장을 확인하는 최상의 방법은 [phpinfo()]를 호출하는 스크립트를 배포하는 것입니다. 추가 확장을 사용하도록 설정하려면 아래 단계를 따르세요.

1. `bin` 디렉터리를 루트 디렉터리에 추가합니다.
1. `.dll` 확장 파일을 `bin` 디렉터리에 저장합니다(예: `php_mongo.dll`). 확장이 기본 버전의 PHP(본 문서 게시 시점에는 PHP 5.4)와 호환되며 nts(스레드로부터 안전하지 않은) VC9과 호환 가능해야 합니다.
1. 응용 프로그램을 배포합니다.
...
1. 스레드로부터 안전하지 않은 VC9과 호환되는 버전의 Windows용 PHP를 받아야 합니다. 최신 Windows용 PHP 릴리스는 [http://windows.php.net/download/]에서 찾을 수 있습니다. 기존 릴리스는 [http://windows.php.net/downloads/releases/archives/](영문)에서 보관 파일로 찾을 수 있습니다.
...
- [Azure에서 웹 사이트 구성, 모니터링 및 크기 조정(영문)]
- [PHP용 Azure SDK 다운로드]


[무료 평가판]: https://www.windowsazure.com/ko-kr/pricing/free-trial/
[PHP 개발자 센터 자습서]: https://www.windowsazure.com/ko-kr/develop/php/tutorials/
