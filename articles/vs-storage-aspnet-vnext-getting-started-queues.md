...
ASP.NET vNext 프로젝트에서 프로그래밍 방식으로 큐에 액세스하려면 다음 작업을 수행해야 합니다.
...
큐로 작업을 수행하려면 먼저 큐를 저장할 저장소 계정의 연결 문자열을 가져와야 합니다. **CloudStorageAccount** 유형을 사용하여 저장소 계정 정보를 나타낼 수 있습니다. ASP.NET vNext 프로젝트를 사용하는 경우 다음 코드에 나온 대로 구성 개체의 get 메서드를 호출하여 Azure 서비스 구성에서 저장소 연결 문자열과 저장소 계정 정보를 가져올 수 있습니다.
...
자세한 내용은 [ASP.NET vNext][ASP.NET vNext](영문)를 참조하세요.
  ...
  [시작하기]: /documentation/articles/vs-storage-aspnet-vnext-getting-started-queues/
