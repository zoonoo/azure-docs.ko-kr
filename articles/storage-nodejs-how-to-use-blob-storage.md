<properties urlDisplayName="Blob Service" pageTitle="Blob 저장소 사용 방법(Node.js) | Microsoft Azure" metaKeywords="Get started Azure blob, Azure unstructured data, Azure unstructured storage, Azure blob, Azure blob storage, Azure blob Node.js" description="Learn how to use the Azure blob service to upload, download, list, and delete blob content. Samples written in Node.js." metaCanonical="" services="storage" documentationCenter="nodejs" title="How to Use the Blob Service from Node.js" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
이 가이드에서는 Azure Blob 서비스를 사용하여 일반 시나리오를 수행하는 방법을 보여 줍니다. 샘플은 Node.js API를 사용하여 작성되었습니다. 여기서 다루는 시나리오에는 Blob **업로드**, **나열**, **다운로드** 및 **삭제**가 포함됩니다. Blob에 대한 자세한 내용은
[다음 단계][] 섹션을 참조하세요.
...
코드에서는 **BlobService** 개체를 만듭니다. 
**server.js**의 위쪽에 다음을 추가합니다.

    var blobSvc = azure.createBlobService();

> [WACOM.NOTE] **createBlobServiceAnonymous**를 사용하고 호스트 주소를 제공하여 익명으로 Blob에 액세스할 수 있습니다. 예를 들면 `var blobSvc = azure.createBlobService('https://myblob.blob.core.windows.net/');`과 같습니다.
...
컨테이너가 만들어지면 `result`는 true가 됩니다. 컨테이너가 이미 있으면 `result`는 false가 됩니다. `response`에는 컨테이너에 대한 [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) 정보를 포함하여 작업에 대한 정보가 포함됩니다.
...
Azure SDK for Node.js에는 재시도 논리를 구현하는 **ExponentialRetryPolicyFilter** 및 **LinearRetryPolicyFilter** 필터 두 개가 포함되어 있습니다. 다음에서는 **ExponentialRetryPolicyFilter**를 사용하는 **BlobService** 개체를 만듭니다.
...
Blob은 블록 기반 또는 페이지 기반일 수 있습니다. 블록 Blob을 사용하면 큰 데이터를 더 효율적으로 업로드할 수 있고, 페이지 Blob은 읽기/쓰기 작업용으로 최적화되어 있습니다. 자세한 내용은 [블록 Blob 및 페이지 Blob 이해](http://msdn.microsoft.com/ko-kr/library/azure/ee691964.aspx)를 참조하세요.
...
이러한 메서드에서 반환한 `result`에는 Blob의 **ETag**와 같은 작업에 대한 정보가 포함됩니다.
...
> [WACOM.NOTE] 페이지 Bobl은 512바이트의 '페이지'로 구성됩니다. 크기가 512의 배수가 아닌 경우에는 데이터를 업로드할 때 오류가 표시될 수 있습니다.
...
`result`에는 각 Blob을 설명하는 개체의 배열인 `entries` 컬렉션이 포함됩니다. 모든 Blob을 반환할 수 없는 경우 `result`는 추가 항목을 검색하는 두 번째 매개 변수로 사용할 수 있는 `continuationToken`도 제공합니다.
...
`result`에는 **ETag** 정보를 포함하여 Blob에 대한 정보가 포함됩니다.

## <a name="delete-blob"> </a>방법: Blob 삭제

마지막으로 Blob을 삭제하려면 **deleteBlob**을 호출합니다. 다음 예제에서는 **myblob**이라는 Blob을 삭제합니다.
...
ETag 조건은 선택적인 `options.accessConditions` 매개 변수를 사용하여 설정할 수 있습니다. 다음 예제에서는 Blob이 이미 있고 `etagToMatch`에 포함된 ETag 값이 있는 경우에만 **test.txt** 파일을 업로드합니다.
...
**myblob**에 대한 후속 작업에서는 `options.leaseId` 매개 변수를 제공해야 합니다. 임대 ID는 **acquireLease**에서 `result.id`로 반환됩니다.

> [WACOM.NOTE] 기본적으로 임대 기간은 무한입니다. 15초에서 60초까지의 유한한 기간을 지정하려면 `options.leaseDuration` 매개 변수를 제공할 수 있습니다.
