...
Azure Automation는 감시자 태스크를 사용 하 여 PowerShell runbook으로 이벤트를 찾고 작업을 트리거합니다. 감시자 태스크에는 감시자 및 동작의 두 부분이 포함 됩니다. 감시자 runbook은 감시자 태스크에 정의 된 간격으로 실행 되 고 작업 runbook에 데이터를 출력 합니다. 

이 자습서에서는 새 파일이 디렉터리에 추가되는 시기를 모니터링하기 위한 감시자 태스크를 만드는 과정을 안내합니다. 다음과 같은 작업을 수행하는 방법을 살펴봅니다.
...
* PowerShell runbook. PowerShell 워크플로 runbook은 감시자 작업에서 지원 되지 않습니다.
...
2. **갤러리 찾아보기**를 클릭 합니다.
3. **감시자 runbook**을 검색 하 고, **디렉터리에서 새 파일을 찾는 감시자 runbook**을 선택 하 고, **가져오기**를 클릭 합니다.
  ![UI에서 Automation Runbook 가져오기](media/automation-watchers-tutorial/importsourcewatcher.png)
4. Runbook에 이름 및 설명을 지정 하 고 **확인** 을 클릭 하 여 Runbook을 Automation 계정으로 가져옵니다.
5. **편집**을 선택하고 **게시**를 클릭합니다. 메시지가 표시 되 면 **예** 를 클릭 하 여 runbook을 게시 합니다.
...
1. **공유 리소스** 아래에서 **변수** 를 선택 하 고 **+ 변수 추가**를 클릭 합니다.
1. 이름에 대해 Watch-NewFileTimestamp를 입력 합니다.
1. 형식에 대해 DateTime을 선택 합니다.
1. **만들기** 를 클릭 하 여 Automation 변수를 만듭니다.

## <a name="create-an-action-runbook"></a>작업 Runbook 만들기

작업 Runbook은 감시자 Runbook에서 전달된 데이터에 대한 작업을 수행하기 위해 감시자 태스크에서 사용됩니다. [PowerShell 갤러리](https://www.powershellgallery.com)에서 **프로세스-NewFile** 이라는 미리 정의 된 작업 runbook을 가져와야 합니다. 

작업 runbook을 만들려면 다음을 수행 합니다.

1. [Process-NewFile](https://gallery.technet.microsoft.com/scriptcenter/Watcher-action-that-b4ff7cdf)에 대 한 갤러리 페이지로 이동 합니다.
2. **Azure Automation** 탭에서 **Azure Automation 배포를**클릭 합니다.

Azure Portal에서이 runbook을 Automation 계정으로 가져올 수도 있습니다.

1. Automation 계정으로 이동 하 여 **프로세스 자동화**아래에서 **runbook** 을 선택 합니다.
1. **갤러리 찾아보기**를 클릭 합니다.
1. 감시자 **작업**을 검색 하 고 **감시자 runbook에 의해 트리거되는 이벤트를 처리 하는 감시자 작업**을 선택 하 고 **가져오기**를 클릭 합니다.
  ![UI에서 작업 Runbook 가져오기](media/automation-watchers-tutorial/importsourceaction.png)
1. Runbook에 이름 및 설명을 지정 하 고 **확인** 을 클릭 하 여 Runbook을 Automation 계정으로 가져옵니다.
1. **편집**을 선택하고 **게시**를 클릭합니다. 메시지가 표시 되 면 **예** 를 클릭 하 여 runbook을 게시 합니다.

## <a name="create-a-watcher-task"></a>감시자 태스크 만들기

이 단계에서는 이전 섹션에서 정의한 감시자 및 작업 runbook을 참조 하는 감시자 태스크를 구성 합니다.

1. Automation 계정으로 이동 하 여 **프로세스 자동화**아래에서 **감시자 작업** 을 선택 합니다.
1. 감시자 작업 페이지를 선택 하 고 **+ 감시자 작업 추가**를 클릭 합니다.
1. 이름으로 **WatchMyFolder** 을 입력 합니다.

1. **감시자 구성** 을 선택 하 고, **보기-NewFile** runbook을 선택 합니다.

1. 매개 변수에 대해 다음 값을 입력합니다.

   * **FOLDERPATH** -새 파일이 생성 된 Hybrid Runbook Worker의 폴더입니다 (예: **d:\examplefiles**).
   * **확장** -구성의 확장입니다. 모든 파일 확장명을 처리하려면 비워 둡니다.
   * **재귀-재귀** 작업 이 값은 기본값으로 둡니다.
   * **실행 설정** -runbook을 실행 하기 위한 설정입니다. 하이브리드 작업자를 선택합니다.

1. **확인**을 클릭 한 다음를 **선택** 하 여 감시자 페이지로 돌아갑니다.
1. **작업 구성** 을 선택 하 고 **프로세스-NewFile** runbook을 선택 합니다.
1. 매개 변수에 대해 다음 값을 입력합니다.

   * **EVENTDATA** -이벤트 데이터입니다. 비워 둡니다. 데이터는 감시자 Runbook에서 제공됩니다.
   * **실행 설정** -runbook을 실행 하기 위한 설정입니다. 이 runbook은 Azure Automation에서 실행 되므로 Azure로 그대로 둡니다.

1. **확인**을 클릭 한 다음를 **선택** 하 여 감시자 페이지로 돌아갑니다.
1. **확인** 을 클릭 하 여 감시자 태스크를 만듭니다.

![UI에서 감시자 작업 구성](media/automation-watchers-tutorial/watchertaskcreation.png)

## <a name="trigger-a-watcher"></a>감시자 트리거

감시자 작업이 예상 대로 작동 하는지 확인 하려면 아래에 설명 된 대로 테스트를 실행 해야 합니다. 

1. Hybrid Runbook Worker에 원격으로 로그인 합니다. 
...
1. Automation 계정으로 이동 하 여 **프로세스 자동화**아래에서 **감시자 작업** 을 선택 합니다.
1. 감시자 태스크 **WatchMyFolder**를 선택 합니다.
1. **흐름** 에서 **감시자 스트림 보기** 를 클릭 하 여 감시자가 새 파일을 발견 하 고 작업 runbook을 시작 했는지 확인 합니다.
1. 작업 runbook 작업을 보려면 **감시자 작업 작업 보기**를 클릭 합니다. 각 작업을 선택 하 여 작업에 대 한 세부 정보를 볼 수 있습니다.
