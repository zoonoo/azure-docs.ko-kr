...
Azure Automation은 감시자 태스크를 사용하여 이벤트를 감시하고 작업을 트리거합니다. 이 자습서에서는 새 파일이 디렉터리에 추가되는 시기를 모니터링하기 위한 감시자 태스크를 만드는 과정을 안내합니다.
...
1. "감시자 Runbook"을 검색하고 **디렉터리에서 새 파일을 찾는 감시자 Runbook**을 선택한 후에 **가져오기**를 선택합니다.
...
작업 Runbook은 감시자 Runbook에서 전달된 데이터에 대한 작업을 수행하기 위해 감시자 태스크에서 사용됩니다. 이 단계에서는 "Process-NewFile"이라는 미리 정의된 작업 Runbook을 업데이트하고 가져옵니다.
