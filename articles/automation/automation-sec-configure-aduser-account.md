<properties
   pageTitle="Azure AD 사용자 계정 구성 | Microsoft Azure"
...
# Azure 서비스 관리 및 Resource Manager로 Runbook 인증
...
## 새 Azure Active Directory 사용자 만들기
...
7. **사용자 프로필** 페이지에 이름과 성, 별명 및 **역할** 목록의 사용자를 입력합니다. **Multi-Factor Authentication을 활성화**하지 마세요.
...
## Azure 클래식 포털에서 자동화 계정 만들기
이 섹션에서는 Azure 포털에서 다음 단계를 수행하여 Runbook과 함께 ASM 및 ARM 모드에서 리소스를 관리하는 데 사용할 새 Azure 자동화 계정을 만들겠습니다.

>[AZURE.NOTE] Azure 클래식 포털을 사용하여 만든 자동화 계정은 Azure 클래식 포털 및 Azure 포털 그리고 각 cmdlet 집합에서 관리할 수 있습니다. 계정이 만들어지면 계정 내에서 리소스를 만들고 관리하는 방법에는 차이가 없습니다. Azure 클래식 포털을 계속 사용하려는 경우, Azure 포털 대신 사용하여 모든 자동화 계정을 만들어야 합니다.


1. 관리하려는 Azure 구독의 서비스 관리자 권한으로 Azure 클래식 포털에 로그인합니다.
2. **자동화**를 선택합니다.
3. **자동화** 페이지에서 **자동화 계정 만들기**를 선택합니다.
4. **자동화 계정 만들기** 상자에서 새 자동화 계정의 이름을 입력하고 드롭다운 목록에서 **지역**을 선택합니다.  
5. **확인**을 클릭하여 설정을 수락하고 계정을 만듭니다.
6. 계정이 만들어지면 **자동화** 페이지에 표시될 것입니다.
...
12. 이후에 나오는 **자격 증명 정의** 페이지에서는 이전에 만든 AD 사용자 계정의 사용자 이름을 **사용자 이름** 필드에 입력하고 암호를 **암호** 및 **암호 확인** 필드에 입력합니다. **확인**을 클릭하여 변경 내용을 저장합니다.

## Azure 포털에서 자동화 계정 만들기
...
2. **자동화 계정 추가** 블레이드의 **이름** 상자에 새 자동화 계정에 대한 이름을 입력합니다.
5. 구독이 둘 이상인 경우 새 계정에 대해 하나를 지정하고 새로운 또는 기존 **리소스 그룹** 및 Azure 데이터 센터 **위치**를 지정합니다.
3. **Azure 실행 계정 만들기** 옵션에 **아니요** 값을 선택하고 **만들기** 단추를 클릭합니다.  
...
4. Azure에서 자동화 계정을 만드는 동안 메뉴에서 **알림**의 진행률을 추적할 수 있습니다.

자격 증명이 생성되면 자격 증명 자산을 만들어서 자동화 계정을 앞에서 만든 AD 사용자 계정과 연결해야 합니다. 자동화 계정을 만들기만 했을 뿐, 아직 인증 ID와 연결하지는 않았다는 점을 잊지 마세요. [Azure 자동화의 자격 증명 자산](../automation/automation-credentials.md#creating-a-new-credential) 문서에 설명된 단계를 수행하고 **사용자 이름**의 값을 **도메인\\사용자** 형식으로 입력합니다.
...
## 다음 단계
* 다음 [Azure 자동화 Runbook 형식](../automation/automation-runbook-types.md) 문서에서 다양한 Runbook 유형 및 사용자 고유의 Runbook을 만드는 단계를 검토합니다.
