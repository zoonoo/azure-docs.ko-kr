...
자격 증명이 생성되면 자격 증명 자산을 만들어서 자동화 계정을 앞에서 만든 AD 사용자 계정과 연결해야 합니다. 자동화 계정을 만들기만 했을 뿐, 아직 인증 ID와 연결하지는 않았다는 점을 잊지 마세요. [Azure 자동화의 자격 증명 자산](automation-credentials.md#creating-a-new-credential) 문서에 설명된 단계를 수행하고 **사용자 이름** 값을 **도메인\\사용자** 형식으로 입력합니다.
...
## 다음 단계
* 다음 [Azure 자동화 Runbook 형식](automation-runbook-types.md) 문서에서 다양한 Runbook 형식 및 사용자 고유의 Runbook을 만드는 단계를 검토합니다.
