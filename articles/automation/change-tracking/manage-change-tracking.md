...
7. 변경 내용 추적에 대 한 Windows 파일 추가 창에서 추적할 파일이 나 폴더에 대 한 정보를 입력 하 고 **저장** 을 클릭 합니다. 다음 표에는 정보에 사용할 수 있는 속성이 정의되어 있습니다.
    ...
    와일드 카드를 사용 하 여 파일 및 폴더 모니터링을 구성 하려면 다음 사항을 고려 하세요.

    - 와일드 카드는 여러 파일을 추적하는 데 필요합니다.
    - 와일드 카드는 *C:\folder\file* 또는 */etc/**와 같이 경로의 마지막 세그먼트에만 사용할 수 있습니다.
    - 환경 변수에 유효하지 않은 경로가 포함된 경우 인벤토리를 실행하면 유효성 검사는 성공하지만 경로는 실패합니다.
    - 경로를 설정 하는 경우 *c:*. * *와 같은 일반적인 경로를 사용 하지 않아야 합니다 .이로 인해 너무 많은 폴더가 트래버스 됩니다.
...
* 기본적으로 스토리지 계정은 네트워크에 있는 모든 클라이언트의 연결을 허용합니다. 특정 트래픽만 허용 하도록 저장소 계정을 보호 한 경우 Automation 계정에 연결할 수 있도록 구성 규칙을 수정 해야 합니다. [Azure Storage 방화벽 및 가상 네트워크 구성을](../../storage/common/storage-network-security.md)참조 하세요.
