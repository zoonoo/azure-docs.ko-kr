...
7. Runbook의 최신 초안 버전을 게시하려면 **게시**를 클릭합니다.

### Runbook에 활동을 삽입하려면
1. 텍스트 편집기의 캔버스에서 활동을 배치할 위치에 커서를 놓습니다.
2. 화면 아래쪽에서 **삽입**과 **활동**을 차례로 클릭합니다.
3. **통합 모듈** 열에서 활동이 포함 된 모듈을 선택합니다.
4. **활동** 창에서 활동을 선택합니다.
5. **설명** 열에서 활동의 설명을 확인합니다. 선택적으로 상세 도움말 보기를 클릭하여 브라우저에서 활동에 대한 도움말을 실행할 수 있습니다.
6. 오른쪽 화살표를 클릭합니다. 활동에 매개 변수가 있으면 정보를 위해 목록에 표시됩니다.
7. 선택 단추를 클릭합니다. 활동을 실행하는 코드가 Runbook에 삽입됩니다.
8. 활동에 매개 변수가 필요한 경우 괄호 <>로 둘러 쌓인 데이터 형식 대신에 적합한 값을 입력합니다.
...
6. Runbook에 매개 변수가 필요한 경우 괄호 <>로 둘러 쌓인 데이터 형식 대신에 적합한 값을 입력합니다.

### Runbook에 자산을 삽입하려면
1. 텍스트 편집기의 캔버스에서 자산을 가져오는 활동을 배치할 위치에 커서를 놓습니다.
2. 화면 아래쪽에서 **삽입**과 **설정**을 차례로 클릭합니다.
3. **설정 작업** 열에서 원하는 작업을 선택합니다.
...
다음 샘플 명령에서는 스크립트 파일의 콘텐츠로 기존 Runbook 콘텐츠를 대체하는 방법을 보여줍니다. 이 내용은 [Windows PowerShell을 사용하여 스크립트 파일에서 Runbook을 가져오려면](../automation-creating-or-importing-a-runbook.md#ImportRunbookScriptPS)에서 설명한 절차와 동일한 예제입니다.
