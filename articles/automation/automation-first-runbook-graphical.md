...
5.	이 경우 **Runbook 형식**으로 **그래픽**을 선택하여 [그래픽 Runbook](automation-graphical-authoring-intro.md)을 만듭니다.<br> ![새 runbook](media/automation-first-runbook-graphical/new-runbook.png)<br>
...
1.	**테스트 창**을 클릭하여 테스트 블레이드를 엽니다.<br> ![테스트 창](media/automation-first-runbook-graphical/runbook-toolbar-test.png)
...
이제 구독 ID를 보관할 변수가 있으므로 [필수 조건](#prerequisites)에 나온 자격 증명과 ID를 사용하도록 runbook을 구성할 수 있습니다. 이 작업을 하려면 **Add-AzureRMAccount** cmdlet을 추가합니다. 하지만 자격 증명에 대한 runbook 입력 매개 변수는 **Add-AzureRMAccount** **자격 증명** 매개 변수에서 직접 호출할 수 없기 때문에 해당 매개 변수를 미리 만들어야 합니다.
...
4.	**이름**에 대해 *AzureCredentialAssetName*을 지정합니다. **형식**에 대해 *문자열*을 유지하고 **사용자 지정 기본값**에 Azure 자동화 자격 증명을 입력합니다. **확인**을 클릭합니다.
5.	라이브러리 컨트롤에서 **자산**, **변수**를 확장하고 **"변수 가져오기"를 캔버스에 추가**를 선택하여 **AzureSubscriptionId**를 캔버스에 추가합니다.
6.	캔버스에서 **Get-AutomationVariable**을 선택하고 구성 제어판의 **레이블** 텍스트 상자에 **구독 ID 가져오기**를 입력합니다.  
...
9.	캔버스에서 **Get-AutomationPSCredential**을 선택하고 구성 제어판의 **레이블** 텍스트 상자에 **Azure 자격 증명 가져오기**를 입력합니다.
10.	**매개 변수**를 클릭하면 활동 매개 변수 구성 블레이드가 표시됩니다. 
11. 활동 매개 변수 구성 블레이드에서 **이름**을 클릭하면 매개 변수 값 블레이드가 표시됩니다.  
12.	 매개 변수 값 블레이드에서 **데이터 원본**에 대해 **Runbook 입력**을 선택하고 목록에서 **AzureCredentialAssetName**을 선택한 다음 **확인**을 두 번 클릭합니다.   
13.	도형 아래에 원이 나타날 때까지 **구독 ID 가져오기**를 마우스로 가리킵니다. 원을 클릭하고 화살표를 **Azure 자격 증명 가져오기**로 끕니다. 방금 만든 화살표는 *링크*입니다. runbook이 **구독 ID 가져오기**로 시작되면 **Azure 자격 증명 가져오기**를 실행합니다.<br> ![활동 간 링크 만들기](media/automation-first-runbook-graphical/runbook-link-getsubscription-getcredential.png)
...
16.	도형 아래에 원이 나타날 때까지 **Azure 자격 증명 가져오기**를 마우스로 가리킵니다. 원을 클릭하고 화살표를 **Add-AzureRmAccount**로 끌어 놓습니다.
17.	구성 제어판에서 **Add-AzureRmAccount**를 선택하고 **매개 변수**를 클릭합니다.
18.	**Add-AzureRmAccount**에는 여러 매개 변수 집합이 있으므로 매개 변수 값을 입력하기 전에 하나를 선택해야 합니다. **매개 변수 집합**을 클릭한 후 **SubscriptionId** 매개 변수 집합을 선택합니다.
...
20.	매개 변수 값 블레이드에서 **데이터 원본**에 대해 **활동 출력**을 선택하고 목록에서 **Azure 자격 증명 가져오기**를 선택한 다음 **확인**을 클릭합니다.   
21.	**SubscriptionId**를 클릭하고 매개 변수 값 블레이드의 목록에서 **구독 ID 가져오기**를 선택한 다음 **확인**을 두 번 클릭하고 캔버스로 돌아옵니다.
...
3. 아래에 원 모양이 나타날 때까지 **Azure에 연결**을 마우스로 가리킵니다. 원을 클릭하고 화살표를 **Start-AzureRmVM**으로 끌어 놓습니다. 
...
2. **Start-AzureRmVM**을 클릭하고 *Delete* 키를 눌러 링**Azure에 연결** 간의 링크를 제거합니다.
...
9. **Azure에 연결**에서 **Get-AzureRmVM**에 대한 링크를 만듭니다.
...
16. **상태 가져오기**에서 **Start-AzureRmVM**에 대한 링크를 만듭니다.<br> ![코드 모듈과 Runbook](media/automation-first-runbook-graphical/runbook-startazurermvm-get-status.png)
...
24. **InputObject**에서 **데이터 원본**을 **PowerShell 식**으로 변경하고 *"$VMName could not start"* 식을 입력합니다.
25. **Start-AzureRmVM**에서 **VM 중지 알림** 및 **VM 시작 실패 알림**에 대한 링크를 만듭니다.
26. **VM 시작 알림** 링크를 선택하고 **조건 적용**을 **True**로 변경합니다.
...
28. **VM 시작 실패 알림** 링크를 선택하고 **조건 적용**을 **True**로 변경합니다.
