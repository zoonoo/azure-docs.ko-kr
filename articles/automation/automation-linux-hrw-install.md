...
1. *-w*, *-k*, *-g* 및 *-e* 매개 변수의 값을 변경하여 다음 명령을 실행합니다. *-g* 매개 변수의 경우 새 Linux 하이브리드 Runbook 워커가 참여해야 하는 하이브리드 Runbook 작업자 그룹의 이름으로 값을 바꿉니다. 해당 이름이 Automation 계정에 없는 경우 해당 이름의 Hybrid Runbook Worker 그룹이 새로 만들어집니다.
...
* 하이브리드 Runbook 작업자를 제거하는 방법에 대한 지침은 [Azure 자동화 하이브리드 Runbook 작업자 제거를](automation-hybrid-runbook-worker.md#remove-a-hybrid-runbook-worker)참조하십시오.
