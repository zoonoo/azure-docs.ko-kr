...
Azure PowerShell Az 모듈의 경우 다음 표의 cmdlet은 Windows PowerShell을 사용하여 자동화 자격 증명 자산을 만들고 관리하는 데 사용됩니다. 자동화 런북 및 DSC 구성에서 사용할 수 있는 [AzureAz.Automation 모듈의](/powershell/azure/new-azureps-module-az?view=azps-1.1.0)일부로 제공됩니다.

| Cmdlet | 설명 |
|:--- |:--- |
| [Get-Az자동화 자격 증명](/powershell/module/az.automation/get-azautomationcredential?view=azps-3.3.0) |자격 증명 자산에 대한 정보를 검색합니다. PSCredential 개체는 반환 되지 않습니다.  |
...
1. **자격 증명 추가**를 선택합니다.
1. 양식을 작성하고 **만들기를** 선택하여 새 자격 증명을 저장합니다.
...
다음 명령 예제에서는 새 자동화 자격 증명을 만드는 방법을 보여 줍니다. 먼저 이름 및 암호를 사용하여 PSCredential 개체를 만든 다음 이를 사용하여 자격 증명 자산을 만듭니다. **Get-Credential** cmdlet을 사용하여 메시지가 표시되면 이름과 암호를 입력할 수도 있습니다.
...
**Get-AutomationPSCredential** 활동을 사용하여 runbook 또는 DSC 구성의 자격 증명 자산을 검색합니다. 그러면 PSCredential 매개 변수가 필요한 작업이나 cmdlet에 사용할 수 있는 [PSCredential 개체](/dotnet/api/system.management.automation.pscredential)가 반환됩니다. 자격 증명 개체의 속성을 검색하여 개별적으로 사용할 수도 있습니다. 개체에는 사용자 이름과 보안 암호에 해당하는 속성이 있습니다. **GetNetworkCredential** 메서드를 사용하여 비보안 버전 암호를 제공하는 [NetworkCredential](/dotnet/api/system.net.networkcredential) 개체를 반환할 수도 있습니다.

> [!NOTE]
> **Get-AzAutomationCredential** 인증에 사용할 수 있는 **PS자격 증명을** 반환 하지 않습니다. 자격 증명에 대한 정보만 제공합니다. Runbook에서 자격 증명을 사용해야 하는 경우 **Get-AutomationPSCredential를** 사용하여 **PSCredential** 개체를 검색해야 합니다.
...
자격 증명을 사용하여 [Connect-AzAccount](/powershell/module/az.accounts/connect-azaccount?view=azps-3.3.0)를 사용하여 Azure에 인증할 수도 있습니다. 대부분의 경우 Run As [계정을](../manage-runas-account.md) 사용하고 [Get-AzAutomationConnection](../automation-connections.md)을 사용하여 검색해야 합니다.
...
다음 그림에서는 그래픽 Runbook에서 자격 증명을 사용하는 예제를 보여 줍니다. 이 경우 [Azure AD 사용자 계정으로 Runbook 인증에](../automation-create-aduser-account.md)설명된 대로 Runbook에 대한 인증을 Azure 리소스에 제공하는 데 사용됩니다. 첫 번째 활동에서는 Azure 구독에 액세스할 수 있는 자격 증명을 검색합니다. 그런 다음 **Connect-AzureRmAccount** 활동은 이 자격 증명을 사용하여 이후에 발생하는 모든 활동에 대한 인증을 제공합니다. [Get-AutomationPSCredential](../automation-graphical-authoring-intro.md#links-and-workflow) 에는 단일 개체가 필요하기 때문에 여기에서는 **파이프라인 링크** 를 사용합니다.  
...
Azure 자동화의 DSC 구성은 **Get-AutomationPSCredential를**사용하여 자격 증명 자산을 참조할 수 있지만 원하는 경우 자격 증명 자산을 매개 변수를 통해 전달할 수도 있습니다. 자세한 내용은 [Azure Automation DSC에서 구성을 컴파일](../automation-dsc-compile.md#credential-assets)을 참조하세요.
