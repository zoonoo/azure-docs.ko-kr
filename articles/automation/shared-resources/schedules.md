...
| Cmdlet | 설명 |
|:--- |:--- |
| [Get-AzureRm 자동화 일정](/powershell/module/azurerm.automation/get-azurermautomationschedule) |일정을 검색합니다. |
| [New-AzureRmAutomationSchedule](/powershell/module/azurerm.automation/new-azurermautomationschedule) |새 일정을 만듭니다. |
| [제거-AzureRm 자동화 일정](/powershell/module/azurerm.automation/remove-azurermautomationschedule) |일정을 제거합니다. |
| [설정-AzureRm 자동화 일정](/powershell/module/azurerm.automation/set-azurermautomationschedule) |기존 일정에 대한 속성을 설정합니다. |
...
4. **한 번** 또는 **되풀이**를 선택하여 일정을 한 번 실행할지 또는 되풀이 일정에 따라 실행할지를 선택합니다. **한 번**을 선택하는 경우 **시작 시간**을 지정한 다음, **만들기**를 클릭합니다. **되풀이**를 선택하는 경우 **시작 시간**을 지정하고 **되풀이 간격**에서는 Runbook 반복 빈도를 **시간**, **일**, **주** 또는 **월** 단위로 지정합니다.
    1. **주를**선택하면 선택할 요일 목록이 제공됩니다. 요일을 원하는 수만큼 선택합니다. 첫 번째 일정은 시작 시간 이후에 선택한 첫 번째 요일에 실행됩니다. 예를 들어 주말 일정을 선택하려면 **토요일과** **일요일을**선택합니다.
...
### <a name="to-create-a-new-schedule-with-powershell"></a>PowerShell을 사용하여 새 일정을 만들려면

[New-AzureRmAutomationSchedule](/powershell/module/azurerm.automation/new-azurermautomationschedule) cmdlet을 사용하여 일정을 만듭니다. 일정의 시작 시간 및 실행 빈도를 지정합니다. 다음 예제는 다양한 일정 시나리오를 만드는 방법을 보여 주며,
...
다음 샘플 명령은 1년 동안 매일 1:00PM에 실행되는 되풀이 일정을 만드는 방법을 보여 준다.
...
다음 샘플 명령은 평일만 실행되는 주간 일정을 만드는 방법을 보여 준다.
...
다음 샘플 명령은 한 달의 1일, 15일 및 마지막 날에 실행되는 되풀이 일정을 만드는 방법을 보여 준다.
...
4. Runbook에 매개 변수가 있는 경우 **실행 설정 수정(기본값:Azure)** 옵션을 선택하고 정보를 입력할 수 있는 **매개 변수** 창이 표시됩니다.

### <a name="to-link-a-schedule-to-a-runbook-with-powershell"></a>PowerShell을 사용하여 일정을 Runbook에 연결하려면
...
### <a name="to-disable-a-schedule-with-powershell"></a>PowerShell으로 일정을 비활성화하려면
