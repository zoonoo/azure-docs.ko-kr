...
1. **저장** 을 클릭하여 Runbook의 초안 복사본을 저장합니다.
    ...
    :::image type="content" source="./media/automation-quickstart-create-runbook/automation-edit-runbook.png" alt-text="오른쪽 창에 코드 예제가 있는 PowerShell Runbook 편집 페이지의 스크린샷.":::
...
1. **이름** 에 대한 값을 입력하고 **시작** 을 클릭합니다. 테스트 작업이 시작되고 작업 상태와 출력이 표시됩니다.
    ...
    :::image type="content" source="./media/automation-quickstart-create-runbook/automation-test-runbook.png" alt-text="이름 필드에 예제 값이 있는 테스트 창의 스크린샷.":::
...
1. PowerShell Runbook 편집 페이지에서 **게시** 를 클릭하여 이 Runbook을 계정의 공식적인 Runbook 버전으로 게시합니다.
   ...
   :::image type="content" source="./media/automation-quickstart-create-runbook/automation-hello-world-runbook-job.png" alt-text="선택된 게시 단추를 보여주는 PowerShell Runbook 편집 페이지의 스크린샷.":::
...
1. Runbook 개요 페이지에서 **시작** 을 클릭하여 이 Runbook에 대한 Runbook 시작 구성 페이지를 엽니다.
   ...
   :::image type="content" source="./media/automation-quickstart-create-runbook/automation-hello-world-runbook-start.png" alt-text="[Runbook 시작] 구성 페이지의 스크린샷.":::

1. 기본값이 사용되도록 **이름** 은 비워 두고, **확인** 을 클릭합니다. Runbook 작업이 제출되고 작업 페이지가 표시됩니다.

   :::image type="content" source="./media/automation-quickstart-create-runbook/automation-job-page.png" alt-text="선택한 출력 단추를 보여주는 작업 페이지의 스크린샷.":::

1. 작업 상태가 `Running` 또는 `Completed`이면 **출력** 을 클릭하여 출력 창을 열고 Runbook 출력을 봅니다.

   :::image type="content" source="./media/automation-quickstart-create-runbook/automation-hello-world-runbook-job-output.png" alt-text="Runbook 출력을 보여주는 출력 창의 스크린샷.":::
