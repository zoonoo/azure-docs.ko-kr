...
ms.locfileid: "86186064"
---
# <a name="enable-update-management-from-an-automation-account"></a>Automation 계정에서 업데이트 관리 사용

이 문서에서는 Automation 계정을 사용하여 사용자 환경의 VM에 [업데이트 관리](automation-update-management.md) 기능을 사용하도록 설정하는 방법을 설명합니다. 대규모로 Azure VM을 사용하도록 설정하려면 업데이트 관리를 사용하여 기존 VM을 사용하도록 설정해야 합니다. 
...
2. **비Azure 컴퓨터 추가**를 클릭합니다. 이 작업은 컴퓨터가 업데이트 관리 보고 작업을 시작할 수 있게 [Windows에 대한 Log Analytics 에이전트를 설치 및 구성하기 위한 지침](../azure-monitor/platform/log-analytics-agent.md)을 표시하는 새 브라우저 창을 엽니다. Operations Manager에서 현재 관리하고 있는 컴퓨터를 사용하는 경우 새 에이전트가 필요하지 않으며 작업 영역 정보가 기존 에이전트에 입력됩니다.
