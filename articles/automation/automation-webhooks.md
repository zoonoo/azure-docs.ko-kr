...
Webhook를 만들 때 `WebhookData` 값을 지정 하는 경우 웹 후크는 클라이언트 POST 요청의 데이터로 runbook을 시작할 때 재정의 됩니다. 이는 응용 프로그램에 요청 본문의 데이터가 포함 되지 않은 경우에도 마찬가지입니다. 

Webhook 이외의 메커니즘을 사용 하 여 `WebhookData`을 정의 하는 runbook을 시작 하는 경우 runbook에서 인식 하는 `WebhookData` 값을 제공할 수 있습니다. 이 값은 웹 후크를 통해 전달 된 실제 `WebhookData` 개체와 함께 작동 하는 것 처럼 runbook에서 사용할 수 있도록 `WebhookData` 매개 변수와 동일한 [속성](#webhook-properties) 을 사용 하는 개체 여야 합니다.
...
다음 runbook 예제에서는 `WebhookData`에 대 한 다음 속성을 정의 하겠습니다.
...
다음 예에서는 `WebhookData`의 `RequestBody` 속성에서 runbook에 사용할 수 있는 요청의 본문을 보여 줍니다. 이 값은 요청의 본문에 포함 된 형식과 호환 되도록 JSON 형식으로 지정 됩니다.
