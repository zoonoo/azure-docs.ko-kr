...
- 솔루션에 온보온된 Linux VM용 업데이트 리포지토리에 대한 액세스.
...
- **평가되지 않음**: 예상 기간 내에 컴퓨터의 업데이트 평가 데이터를 받지 못했습니다. Linux 컴퓨터의 경우 예상 기간은 마지막 1시간입니다. Windows 컴퓨터의 경우, 예상 시간 범위는 지난 12시간입니다.
...
- **업데이트할 컴퓨터**: 저장된 검색, 가져온 그룹 또는 머신을 선택하여 업데이트할 머신을 선택합니다.
   ...
   **컴퓨터를**선택하면 컴퓨터의 준비 준비가 업데이트 **에이전트 준비** 열에 표시됩니다. 업데이트 배포를 예약하기 전에 컴퓨터의 상태를 확인할 수 있습니다. Azure Monitor 로그에서 컴퓨터 그룹을 만드는 다른 방법에 대해 알아보려면 [Azure Monitor 로그의 컴퓨터 그룹](../azure-monitor/platform/computer-groups.md)을 참조하세요.
...
- **시도하지 않음**: 정의된 유지 관리 기간에 따라 사용할 수 있는 시간이 부족하여 업데이트가 설치되지 않았습니다.
- **성공**: 업데이트가 성공했습니다.
- **실패**: 업데이트가 실패했습니다.
