...
**업데이트 관리** 대화 상자의 맨 위에서 **업데이트 배포 예약**을 선택하여 하나 이상의 가상 머신에 대한 새 업데이트 배포를 예약합니다. **새 배포 업데이트** 창에서 다음을 지정합니다.

* **이름**: 업데이트 배포를 식별하는 고유 이름을 제공합니다.
* **OS 형식**: Windows 또는 Linux를 선택합니다.
* **업데이트할 컴퓨터**: 업데이트하려는 가상 머신을 선택합니다.

  !["새 업데이트 배포" 창](./media/manage-update-multi/update-select-computers.png)

* **업데이트 분류**: 업데이트 배포가 포함되는 소프트웨어의 종류를 선택합니다. 분류 형식은 다음과 같습니다.
  * 중요 업데이트
  * 보안 업데이트
  * 업데이트 롤업
  * 기능 팩
