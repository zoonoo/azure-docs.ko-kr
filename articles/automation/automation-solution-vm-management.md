...
1. **솔루션 추가** 페이지에서 **작업 영역**을 선택합니다. Automation 계정이 속하는 동일한 Azure 구독에 연결된 Log Analytics 작업 영역을 선택합니다. 작업 영역이 없으면 **새 작업 영역 만들기**를 선택합니다. **OMS 작업 영역** 페이지에서 다음을 수행합니다.
   ...
   * **가격 책정 계층**을 선택합니다. 솔루션은 **무료** 및 **노드당(OMS)**이라는 두 가지 계층을 제공합니다. 무료 계층은 하루에 수집되는 데이터의 양, 보존 기간 및 Runbook 작업 런타임 시간(분)이 제한됩니다. 노드당 계층은 하루에 수집할 수 있는 데이터의 양이 제한되지 않습니다.
        ...
        > [!NOTE]
        > 독립 실행형 유료(GB 기준) 계층이 옵션으로 표시되긴 하지만 적용할 수는 없습니다. 이를 선택하고 구독에서 이 솔루션의 만들기를 계속하는 경우 실패합니다. 이 솔루션이 공식적으로 릴리스되면 해결될 것입니다. 이 솔루션은 Automation 작업 분 및 로그 수집만 사용합니다. 솔루션은 사용자 환경에 추가 노드를 추가하지 않습니다.
