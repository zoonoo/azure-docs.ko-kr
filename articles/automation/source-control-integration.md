...
* 모듈을 포함 하 여 Automation 계정의 [최신 Azure 모듈](automation-update-azure-modules.md) (에 `AzureRM.Profile`해당 하는 Az module) `Az.Accounts`
...
1. Automation 계정에서 **원본 제어** 를 선택 하 고 **추가**를 클릭 합니다.
...
> [!NOTE]
> 소스 제어 리포지토리의 로그인은 Azure Portal 로그인과 다를 수 있습니다. 원본 제어를 구성할 때 소스 제어 리포지토리의 올바른 계정으로 로그인 했는지 확인 합니다. 확실 하지 않은 경우 브라우저에서 새 탭을 열고 **dev.azure.com**, **visualstudio.com**또는 **github.com**에서 로그 아웃 한 다음 소스 제어에 다시 연결 합니다.
...
> [!NOTE]
> Azure Repos (Git)는 이전 형식에서 사용 되는 **visualstudio.com**대신 **dev.azure.com** 에 액세스 하는 URL을 사용 합니다. 이전 URL 형식은 `https://<accountname>.visualstudio.com/<projectname>/_git/<repositoryname>` 더 이상 사용 되지 않지만 여전히 지원 됩니다. 새 형식을 설정 하는 것이 좋습니다.
...
> [!NOTE]
> TFVC (Azure Repos)는 이전 형식에서 사용 되는 **visualstudio.com**대신 **dev.azure.com** 에 액세스 하는 URL을 사용 합니다. 이전 URL 형식은 `https://<accountname>.visualstudio.com/<projectname>/_versionControl` 더 이상 사용 되지 않지만 여전히 지원 됩니다. 새 형식을 설정 하는 것이 좋습니다.
...
<sup>1</sup> autosync `Service connections` 를 사용 하도록 설정한 경우에만 사용 권한이 필요 합니다.
...
현재는 Azure Portal을 사용 하 여 소스 제어에서 PAT를 업데이트할 수 없습니다. PAT가 만료 되거나 해지 되 면 다음 방법 중 하나로 새 액세스 토큰을 사용 하 여 소스 제어를 업데이트할 수 있습니다.
