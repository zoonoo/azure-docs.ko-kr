...
Azure 자동화는 지역에서 복제를 지원합니다. 지역에서 복제를 사용하여 Azure 자동화는 데이터가 두 지역에 유지되게 합니다. Azure 포털에서 자동화 계정을 만드는 동안 주 지역으로 만들어져야 하는 지역을 선택합니다. 데이터가 지역에서 복제된 지역을 보조 지역이라고 합니다. 주 지역 및 보조 지역은 자동화 계정에 수행된 업데이트를 지역에서 복제할 수 있도록 서로 통신합니다. 주 지역에서 보조 지역으로의 자동화 계정 장애 조치(failover)가 있는 경우 보조 지역이 정보의 복사본을 저장하므로 모든 자동화 계정 정보는 여전히 보조 지역에 있게 됩니다.

지역에서 복제는 자동화 계정에 기본 제공되며 추가 비용 없이 제공됩니다. 보조 지역을 선택할 권한은 없으며 주 지역으로 선택한 위치에 따라 자동으로 결정됩니다.

 
###지역에서 복제본의 위치

현재 자동화 계정은 다섯 지역 미만으로 만들 수 있으며 더 많은 지역에 대한 지원은 향후 추가될 예정입니다. 다음 표에서는 기본 및 보조 지역 쌍을 보여 줍니다.
...
주요 재해가 주 지역에 영향을 미치는 경우 먼저 자동화 팀이 주 지역을 복원하려고 시도합니다. 일부 예에서 주 지역을 복원할 수 없는 경우 지역 장애 조치(failover)가 수행되고 영향을 받는 고객은 해당 구독을 통해 이에 대한 알림을 받게 됩니다.
