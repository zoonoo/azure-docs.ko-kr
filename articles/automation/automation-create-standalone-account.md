...
ms.locfileid: "82995791"
---
# <a name="create-a-standalone-azure-automation-account"></a>독립 실행형 Azure Automation 계정 만들기

이 문서에서는 Azure Portal에서 Azure Automation 계정을 만드는 방법을 보여줍니다. 포털 Automation 계정을 사용 하 여 추가 관리 기능을 사용 하거나 Azure Monitor 로그와 통합 하지 않고도 Automation을 평가 하 고 알아볼 수 있습니다. 이후 언제 든 지 runbook 작업의 고급 모니터링을 위해 관리 기능을 추가 하거나 Azure Monitor 로그와 통합할 수 있습니다.
...
Azure Portal에서 Automation 계정을 만드는 경우 **실행** 계정이 자동으로 만들어집니다. 이 계정에서 수행하는 작업은 다음과 같습니다.
...
이 계정을 만든 후에는 자동화 요구 사항을 지원 하기 위해 runbook을 빠르게 빌드하고 배포할 수 있습니다.
...
* Automation 계정을 만들려면 리소스에 대 한 `Microsoft.Automation` 소유자 역할에 해당 하는 권한이 있는 역할에 Azure AD 사용자 계정을 추가 해야 합니다. 자세한 내용은 [Azure Automation의 역할 기반 액세스 제어](automation-role-based-access-control.md)를 참조하세요.
* Azure Portal의 > **사용자 설정**관리 **Azure Active Directory** > **MANAGE**에서 **앱 등록** **예**로 설정 된 경우 Azure AD 테 넌 트의 관리자가 아닌 사용자가 [Active Directory 응용 프로그램을 등록할](../active-directory/develop/howto-create-service-principal-portal.md#check-azure-subscription-permissions)수 있습니다. **앱 등록 설정**이 **아니요**로 설정된 경우, 이 작업을 수행하는 사용자는 Azure AD의 전역 관리자여야 합니다.

구독의 전역 관리자/공동 관리자 역할에 추가 되기 전에 구독 Active Directory 인스턴스의 멤버가 아닌 경우 Active Directory에 게스트로 추가 됩니다. 이 시나리오에서는 Automation 계정 추가 창에서 다음 메시지가 표시 됩니다.`You do not have permissions to create.`

사용자가 전역 관리자/공동 관리자 역할에 먼저 추가 되는 경우 해당 사용자를 구독의 Active Directory 인스턴스에서 제거할 수 있습니다. Active Directory에서 사용자 역할에 사용자를 다시 추가할 수 있습니다.
...
1. Automation 계정 추가 창에서 **이름** 필드에 새 Automation 계정의 이름을 입력 합니다. 선택한 후에는이 이름을 변경할 수 없습니다. 

    > [!NOTE]
    > Automation 계정 이름은 하위 지역 및 리소스 그룹별로 고유합니다. 삭제 된 Automation 계정의 이름을 즉시 사용할 수 없을 수도 있습니다.

1. 둘 이상의 구독이 있는 경우 **구독** 필드를 사용 하 여 새 계정에 사용할 구독을 지정할 수 있습니다.
1. **리소스 그룹**에서 기존 또는 새 리소스 그룹을 입력하거나 선택합니다.
1. **위치**에서 Azure 데이터 센터 위치를 선택합니다.
1. **Azure 실행 계정 만들기** 옵션에 대해 **예** 를 선택 했는지 확인 한 다음 **만들기**를 클릭 합니다.
...
1. Automation 계정 만들기의 진행률을 추적 하려면 메뉴에서 **알림** 을 선택 합니다.
...
클래식 실행 계정은 Azure Automation 계정을 만들 때 더 이상 기본적으로 생성 되지 않습니다. 여전히 클래식 실행 계정이 필요한 경우:

1. Automation 계정에서 **계정 설정**아래에 있는 **실행 계정** 을 선택 합니다.
...
* PowerShell runbook을 시작 하려면 [내 첫 번째 powershell runbook](automation-first-runbook-textual-powershell.md)을 참조 하세요.
* PowerShell 워크플로 runbook을 시작 하려면 [내 첫 번째 powershell 워크플로 runbook](automation-first-runbook-textual.md)을 참조 하세요.
* Python 2 runbook을 시작 하려면 [내 첫 번째 Python2 runbook](automation-first-runbook-textual-python2.md)을 참조 하세요.
