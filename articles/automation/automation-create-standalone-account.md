...
Azure 포털에서 자동화 계정을 만들면 **실행 됨** 계정이 자동으로 만들어집니다. 이 계정에서 수행하는 작업은 다음과 같습니다.
...
이 계정을 만들면 자동화 요구 사항을 지원하기 위해 Runbook을 신속하게 빌드하고 배포할 수 있습니다.
...
* 자동화 계정을 만들려면 Azure AD 사용자 계정이 리소스에 대한 `Microsoft.Automation` 소유자 역할과 동일한 사용 권한을 가진 역할에 추가되어야 합니다. 자세한 내용은 [Azure Automation의 역할 기반 액세스 제어](automation-role-based-access-control.md)를 참조하세요.
...
구독의 글로벌 관리자/공동 관리자 역할에 추가하기 전에 구독의 Active Directory 인스턴스의 구성원이 아닌 경우 게스트로 Active Directory에 추가됩니다. 이 시나리오에서는 자동화 계정 추가 창에 이 메시지가 표시 됩니다.`You do not have permissions to create.`

사용자가 글로벌 관리자/공동 관리자 역할에 먼저 추가된 경우 구독의 Active Directory 인스턴스에서 사용자를 제거할 수 있습니다. Active Directory에서 사용자 역할을 읽을 수 있습니다.
...
1. 자동화 계정 추가 창에서 **이름** 필드에 새 자동화 계정의 이름을 입력합니다. 선택한 후에는 이 이름을 변경할 수 없습니다. 

    > [!NOTE]
    > 자동화 계정 이름은 지역 및 리소스 그룹당 고유합니다. 삭제된 자동화 계정의 이름을 즉시 사용할 수 없을 수 있습니다.

1. 구독이 두 개 이상인 경우 **구독** 필드를 사용하여 새 계정에 사용할 구독을 지정합니다.
1. **리소스 그룹**에서 기존 또는 새 리소스 그룹을 입력하거나 선택합니다.
1. **위치**에서 Azure 데이터 센터 위치를 선택합니다.
1. Azure **실행 계정으로 만들기** 옵션의 경우 **예가** 선택되어 있는지 확인하고 에서 **만들기를**클릭합니다.
...
1. 자동화 계정 생성 진행률을 추적하려면 메뉴에서 **알림을** 선택합니다.
