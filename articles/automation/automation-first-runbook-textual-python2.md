...
1. 이 창에서 [runbook 작업](automation-runbook-execution.md) 이 생성되고 상태를 보여줍니다.
   작업 상태는 대기 중으로 시작되며 클라우드에서 Runbook 작업자가 사용 가능해지기를 기다리고 있음을 나타냅니다. 작업자가 작업을 요구한 경우 시작 중으로 바뀐 다음 Runbook이 실제로 실행되기 시작하면 실행 중으로 바뀝니다.
1. Runbook 작업이 완료되면 해당 출력이 표시됩니다. 이 경우 을 볼 `Hello World`수 있습니다.
...
Runbook을 게시할 때 초안 버전으로 기존 게시된 버전을 덮어씁니다.
이 예에서는 Runbook을 방금 만들었으므로 아직 게시된 버전이 없습니다.

1. **게시**를 클릭하여 Runbook을 게시한 다음 확인 메시지가 표시되면 **예**를 클릭합니다.
1. 왼쪽을 스크롤하여 Runbook 페이지에서 **Runbook을** 보려면 **게시된**의 **작성 상태가** 표시됩니다.
1. 오른쪽으로 다시 스크롤하면 **MyFirstRunbook-Python**창이 표시됩니다.
   상단의 옵션을 사용하면 Runbook을 시작하거나 Runbook을 보거나 나중에 시작하도록 예약할 수 있습니다.
2. **실행을** 클릭한 다음 Runbook 시작 블레이드가 열리면 **확인을** 클릭합니다.
3. 만든 Runbook 작업에 대해 작업 창이 열립니다. 이 창을 닫을 수 있지만 작업의 진행 상황을 볼 수 있도록 열어 두겠습니다.
1. Runbook을 테스트할 때와 동일한 작업 상태가 **작업 요약**에 표시됩니다.
2. Runbook 상태가 완료됨으로 표시되면 **출력**을 클릭합니다. 출력 창이 열리면 을 볼 `Hello World`수 있습니다.
3. 출력 창을 닫습니다.
4. **모든 로그** 를 클릭하여 Runbook 작업에 대한 스트림 창을 엽니다. 출력 스트림에만 표시됩니다. `Hello World` 그러나 이 창은 Runbook이 해당 스트림에 쓰는 경우 자세한 내용 및 오류와 같은 Runbook 작업에 대한 다른 스트림을 표시할 수 있습니다.
5. MyFirstRunbook-Python 창으로 돌아가려면 스트림 창 및 작업 창을 닫습니다.
6. 이 Runbook의 작업 페이지를 열려면 **작업을** 클릭합니다. 이 페이지에는 이 Runbook에서 만든 모든 작업이 나열됩니다. 작업을 한 번만 실행했으므로 하나의 작업만 표시됩니다.
7. Runbook을 시작했을 때 표시된 동일한 작업창을 열려면 이 작업을 클릭하면 됩니다. 이 창을 사용하면 시간을 거슬러 올라가 특정 Runbook에 대해 만들어진 작업의 세부 정보를 볼 수 있습니다.
...
이렇게 하려면 스크립트는 자동화 계정의 자격 증명을 사용하여 인증해야 합니다. [Azure Automation 유틸리티 패키지](https://github.com/azureautomation/azure_automation_utility)를 사용하면 더욱 간편하게 Azure 리소스를 인증하고 조작할 수 있습니다.

> [!NOTE]
> 인증서로 실행하려면 서비스 주 기능으로 자동화 계정이 만들어졌어야 합니다.
> 자동화 계정이 서비스 주체로 생성되지 않은 경우 [Python에 대한 Azure 관리 라이브러리를](/azure/python/python-sdk-azure-authenticate)통해 인증에 설명된 대로 인증할 수 있습니다.
...
VM을 포함 하는 리소스 그룹의 이름은 `MyResourceGroup` 어디 `TestVM` 이며 시작 하려는 VM의 이름입니다.

Runbook을 다시 테스트 및 실행하여 VM이 시작되는지 확인합니다.

## <a name="use-input-parameters"></a>입력 매개 변수 사용

Runbook은 현재 리소스 그룹 및 VM의 이름에 대해 하드 코딩된 값을 사용합니다.
...
파이썬 Runbook을 시작할 때(테스트 창또는 게시된 Runbook에서) 매개 변수 아래의 Runbook 시작 페이지에서 매개 변수에 대한 값을 입력할 수 **있습니다.**

첫 번째 상자에 값을 입력하기 시작하면 두 번째 값이 나타나므로 필요한 만큼 매개 변수 값을 입력할 수 있습니다.

방금 추가한 코드와 마찬가지로 `sys.argv` 배열의 스크립트에서 값을 사용할 수 있습니다.
...
> [!NOTE]
> Azure 자동화는 `sys.stderr`을 지원하지 않습니다.

## <a name="next-steps"></a>다음 단계

- PowerShell 실행책을 시작하려면 [내 첫 번째 PowerShell 실행책을](automation-first-runbook-textual-powershell.md)참조하십시오.
- 그래픽 런북을 시작하려면 [내 첫 번째 그래픽 실행책을](automation-first-runbook-graphical.md)참조하십시오.
- PowerShell 워크플로 런북을 시작하려면 [내 첫 번째 PowerShell 워크플로 런북을](automation-first-runbook-textual.md)참조하십시오.
- Runbook 유형, 장점 및 제한 사항에 대해 자세히 알아보려면 [Azure Automation Runbook 유형을](automation-runbook-types.md)참조하십시오.
