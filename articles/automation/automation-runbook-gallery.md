...
4. **TechNet 스크립트 센터** 의 항목을 확인하려면 [소스 프로젝트 보기](https://gallery.technet.microsoft.com/)를 클릭합니다.
...
* 업로드는 단일 `.ps1` 또는 `.graphrunbook` 파일이어야 합니다.  Runbook에 필요한 모듈, 자식 Runbook 또는 자산이 있는 경우 제출 설명과 Runbook의 주석 섹션에 해당 항목을 나열해야 합니다.  여러 Runbook이 필요한 시나리오인 경우 각 Runbook을 개별적으로 업로드하고 설명마다 관련 Runbook의 이름을 나열합니다. 동일한 태그를 사용하여 동일한 범주에 표시되는지 확인합니다. 시나리오가 작동하기 위해 다른 Runbook이 필요하다는 설명을 사용자가 읽고 알 수 있어야 합니다.
* **그래픽 Runbook**(그래픽 워크플로 아님)을 게시하는 경우 “GraphicalPS” 태그를 추가합니다.
