...
description: Microsoft 및 커뮤니티의 Runbook과 모듈을 Azure Automation 환경에 설치하여 사용할 수 있습니다.  이 문서에서는 이러한 리소스에 액세스하고 Runbook을 갤러리에 제공하는 방법을 설명합니다.
services: automation
...
Azure Automation에서 사용자 고유의 Runbook 및 모듈을 만드는 대신 Microsoft 및 커뮤니티에서 이미 빌드한 시나리오에 액세스할 수 있습니다. 스크립트 센터 갤러리에서 PowerShell 갤러리 및 [파이썬 런북에서](#use-python-runbooks) PowerShell 런북 및 [모듈을](#modules-in-powershell-gallery) 얻을 수 있습니다. 또한 개발한 시나리오를 공유하여 [커뮤니티에](#add-a-powershell-runbook-to-the-gallery)기여할 수도 있습니다. 
...
## <a name="common-solutions-available-in-powershell-gallery"></a>PowerShell 갤러리에서 사용할 수 있는 공통 솔루션
...
아래 목록에는 일반적인 시나리오에 대한 솔루션을 제공하는 몇 가지 Runbook이 포함되어 있습니다. Azure 자동화 팀에서 만든 Runbook의 전체 목록은 [AzureAutomationTeam 프로필을](https://www.powershellgallery.com/profiles/AzureAutomationTeam)참조하십시오.
   ...
   * [업데이트 모듈InAutomationToLatestVersion](https://www.powershellgallery.com/packages/Update-ModulesInAutomationToLatestVersion/) - PowerShell 갤러리에서 자동화 계정에 있는 모든 모듈의 최신 버전을 가져오고 있습니다.
   * [Azure진단 -](https://www.powershellgallery.com/packages/Enable-AzureDiagnostics/) 작업 상태 및 작업 스트림을 포함하는 Azure 자동화 로그를 수신하도록 Azure 진단 및 로그 분석을 구성합니다.
   * [복사 항목FromAzureVM](https://www.powershellgallery.com/packages/Copy-ItemFromAzureVM/) - Windows Azure 가상 컴퓨터에서 원격 파일을 복사합니다.
   * [복사 항목FromAzureVM](https://www.powershellgallery.com/packages/Copy-ItemToAzureVM/) - 로컬 파일을 Azure 가상 시스템에 복사합니다.

## <a name="import-a-powershell-runbook-from-the-runbook-gallery-with-the-azure-portal"></a>Azure 포털을 사용 하 고 Runbook 갤러리에서 PowerShell 실행 북 가져오기

1. Azure Portal에서 Automation 계정을 엽니다.
2. **프로세스 자동화에서** **Runbook 갤러리를** 선택합니다.
3. **소스 선택: 파워쉘 갤러리.**
...
6. 항목을 가져오려면 항목을 클릭하여 세부 정보를 보고 **가져오기**를 클릭합니다.

   ![가져오기 단추](media/automation-runbook-gallery/gallery-item-detail.png)

7. 선택적으로 Runbook의 이름을 변경한 다음 **확인** 을 클릭하여 해당 Runbook을 가져옵니다.
8. Runbook은 자동화 계정의 **Runbook** 탭에 나타납니다.

## <a name="add-a-powershell-runbook-to-the-gallery"></a>갤러리에 PowerShell 런북 추가
...
## <a name="import-a-module-from-the-module-gallery-with-the-azure-portal"></a>Azure 포털을 사용 하 고 모듈 갤러리에서 모듈 가져오기
...
6. 모듈을 Azure 자동화에 직접 설치하려면 **가져오기**를 클릭합니다.
7. 가져오기 창에서 가져올 모듈의 이름을 볼 수 있습니다. 모든 종속성이 설치되면 **확인** 단추가 활성화됩니다. 종속성이 없는 경우 해당 종속성을 가져와야만 이 모듈을 가져올 수 있습니다.
8. 가져오기 창에서 **확인을** 클릭하여 모듈을 가져옵니다. Azure Automation에서 모듈을 계정에 가져오는 동안 모듈 및 cmdlet에 대한 메타데이터를 추출합니다. 이 작업은 각 활동을 추출해야 하므로 몇 분 정도 걸릴 수 있습니다.
...
## <a name="use-python-runbooks"></a>파이썬 런북 사용

Python Runbooks는 [스크립트 센터 갤러리](https://gallery.technet.microsoft.com/scriptcenter/site/search?f%5B0%5D.Type=RootCategory&f%5B0%5D.Value=WindowsAzure&f%5B1%5D.Type=ProgrammingLanguage&f%5B1%5D.Value=Python&f%5B1%5D.Text=Python&sortBy=Date&username=)에서 사용할 수 있습니다. **기여도 업로드를**클릭하여 Python Runbook을 스크립트 센터 갤러리에 기여할 수 있습니다. 이렇게 하면 기여를 업로드할 때 `Python` 태그를 추가해야 합니다.
...
* Runbook을 시작하려면 [Azure 자동화에서 Runbook 관리를](manage-runbooks.md)참조하십시오.
* Runbook을 사용하여 PowerShell 및 PowerShell 워크플로의 차이점을 이해하려면 [PowerShell 학습 워크플로를](automation-powershell-workflow.md)참조하십시오.
