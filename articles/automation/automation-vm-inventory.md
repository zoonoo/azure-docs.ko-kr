...
1. 인벤토리 페이지에서 페이지 위쪽의 **설정 편집** 을 클릭 합니다.
2. 새 컬렉션 설정을 추가 하려면 **Windows 레지스트리**, **Windows 파일**또는 **Linux 파일** 탭을 선택 하 여 추가할 설정 범주로 이동 합니다.
3. 적절 한 범주를 선택 하 고 페이지 위쪽에서 **추가** 를 클릭 합니다.
...
|경로 입력     | 파일을 확인할 경로 (예: **c:\temp\myfile.txt**)입니다.
...
|경로 유형     | 추적할 항목의 형식입니다. 값은 파일 및 디렉터리입니다.        |
|재귀     | 추적할 항목을 찾을 때 재귀가 사용 되 면 True이 고, 그렇지 않으면 False입니다.        |
|sudo 사용     | 항목을 확인할 때 sudo를 사용 하면 True이 고, 그렇지 않으면 False입니다.         |
|링크     | 디렉터리를 트래버스할 때 기호화 된 링크를 처리 하는 방법을 나타내는 값입니다. 가능한 값은 다음과 같습니다. <br> Ignore - 심볼 링크를 무시하고 참조된 파일/디렉터리를 포함하지 않음<br>Follow - 재귀 중에 심볼 링크를 따르고 참조된 파일/디렉터리도 포함<br>Manage - 심볼 링크를 따르고 반환된 콘텐츠의 처리를 변경하도록 허용      |
...
**+ 클론** 을 클릭 하 여 컴퓨터 그룹을 복제 합니다. 그룹에 그룹의 새 이름과 별칭을 지정 해야 합니다. 이번에는 정의를 변경할 수 있습니다. 쿼리를 변경한 후 **쿼리 유효성 검사** 를 클릭 하 여 선택할 컴퓨터를 미리 봅니다. 그룹에 만족 하는 경우 **만들기** 를 클릭 하 여 컴퓨터 그룹을 만듭니다.

새 컴퓨터 그룹을 만들려면 **+ 컴퓨터 그룹 만들기**를 클릭 합니다. 이 단추를 클릭 하면 새 그룹을 정의할 수 있는 **컴퓨터 그룹 만들기** 페이지가 열립니다. **만들기**를 클릭하여 그룹을 만듭니다.
...
3. **작업 영역 데이터 원본**에서 **Virtual Machines** 를 선택 합니다.
4. 목록에서 분리할 가상 머신을 선택합니다. 가상 머신은 **OMS 연결** 열의 **이 작업 영역** 옆에 녹색 확인 표시가 있습니다.

   >[!NOTE]
   >이제 OMS (Operations Management Suite)를 Azure Monitor 로그 라고 합니다.
   
5. 다음 페이지의 맨 위에서 **연결 끊기**를 클릭 합니다.
6. 확인 창에서 **예** 를 클릭 하 여 관리에서 컴퓨터의 연결을 끊습니다.
