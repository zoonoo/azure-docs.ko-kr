...
이 시나리오에서는 Azure RM 모듈 및 Git 도구가 설치된 Hybrid Runbook Worker로 구성된 Windows 컴퓨터가 데이터 센터에서 사용됩니다.  Hybrid Worker 컴퓨터에는 로컬 Git 리포지토리 복제본이 생성됩니다.  Runbook이 Hybrid Worker에서 실행될 때 Git 디렉터리가 동기화되고 runbook 파일 내용이 Automation 계정으로 전달됩니다.

이 문서에서는 Azure Automation 환경에서 이러한 구성을 설정하는 방법을 설명합니다. Runbook을 실행하고 GitHub Enterprise 리포지토리에 액세스하여 Runbook을 Automation 계정과 동기화하려면 먼저 데이터 센터에서 보안 자격 증명, 이 시나리오를 지원하는 데 필요한 runbook 및 Hybrid Runbook Worker 배포를 통해 Automation을 구성합니다.  
...
Runbook | 설명| 
--------|------------|
Export-RunAsCertificateToHybridWorker | Runbook은 Automation 계정에서 Hybrid Worker로 실행 인증서를 내보내 Worker의 runbook이 Automation 계정으로 runbook을 가져오기 위해 Azure에서 인증을 받을 수 있도록 합니다.| 
Sync-LocalGitFolderToAutomationAccount | Runbook은 하이브리드 컴퓨터의 로컬 Git 폴더를 동기화하고 runbook 파일(*.ps1)을 Automation 계정으로 가져옵니다.|

### <a name="credentials"></a>자격 증명

자격 증명 | 설명|
-----------|------------|
GitHRWCredential | Hybrid Worker에 대한 권한이 있는 사용자의 사용자 이름 및 암호를 포함하는 자격 증명 자산을 만들게 됩니다.|
...
2. Azure Automation 솔루션이 사용되도록 설정되고 구성된 Microsoft OMS(Operations Management Suite) 작업 공간도 필요합니다.  이 시나리오를 설치하고 구성하는 데 사용되는 Automation 계정과 연결된 작업 공간이 없으면 Hybrid Runbook Worker에서 **New-OnPremiseHybridWorker.ps1** 스크립트를 실행할 때 만들어지고 구성됩니다.        

    > [!NOTE]
    > 현재 OMS와의 Automation 통합이 지원되는 유일한 하위 지역은 **오스트레일리아 남동부**, **미국 동부 2**, **동남 아시아** 및 **서유럽**뿐입니다. 
...
1. 로컬 관리자 권한이 있는 계정으로 Hybrid Runbook Worker 역할을 호스트하는 컴퓨터에 로그온하고 Git runbook 파일을 저장할 디렉터리를 만듭니다.  디렉터리에 내부 Git 리포지토리를 복제합니다.
...
5. Runbook **Export-RunAsCertificateToHybridWorker**를 시작하고 **Runbook 시작** 블레이드의 **실행 설정** 옵션에서 **Hybrid Worker** 옵션을 선택하고 드롭다운 목록에서 이 시나리오에 대해 이전에 만든 Hybrid Worker 그룹을 선택합니다.  
    ...
    이렇게 하면 Worker에 대한 runbook이 Azure 리소스를 관리하기 위해(이 시나리오의 경우 특히 Automation 계정으로 runbook 가져오기) 실행 연결을 사용하여 Azure에서 인증을 받을 수 있도록 인증서가 Hybrid Worker로 내보내집니다.

4. Automation 계정에서 이전에 만든 Hybrid Worker 그룹을 선택하고 Hybrid Worker 그룹에 대해 [실행 계정을 지정](automation-hybrid-runbook-worker.md#runas-account)하고 방금 또는 이전에 만든 자격 증명 자산을 선택합니다.  이렇게 하면 동기화 Runbook이 Git 명령을 실행할 수 있습니다. 
