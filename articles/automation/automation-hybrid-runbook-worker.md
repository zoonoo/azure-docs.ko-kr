<properties
   pageTitle="Azure 자동화 Hybrid Runbook Worker | Microsoft Azure"
...
Hybrid Runbook 작업을 실행하려면 온-프레미스 컴퓨터를 적어도 하나 이상 지정해야 합니다.  이 컴퓨터에는 다음 항목이 있어야 합니다.
...
>[AZURE.NOTE] 현재 Hybrid Runbook Worker 버전 7.2.11136.0은 PowerShell 스크립트를 사용하여 프록시 서버를 통해서만 통신을 지원합니다.  PowerShell 워크플로 스크립트에 대한 지원은 향후 릴리스에서 사용할 수 있습니다.  
...
그룹에서 하나 이상의 Hybrid Runbook Worker를 제거하거나 요구 사항에 따라 그룹을 제거할 수 있습니다.  온-프레미스 컴퓨터에서 Hybrid Runbook Worker를 제거하려면 관리자 모드에서 PowerShell 세션을 열고 다음 명령 - **Remove-HybridRunbookWorker** cmdlet을 입력합니다.  제거 과정에 대한 자세한 로그를 보려면 **-Verbose** 스위치를 사용합니다. 

컴퓨터의 Microsoft Monitoring Agent가 아닌 Hybrid Runbook Worker 역할의 기능 및 구성만을 제거합니다.  

그룹을 제거하려면 먼저 앞서 보았던 명령을 사용하여 그룹의 구성원인 모든 컴퓨터에서 Hybrid Runbook Worker를 제거한 후 다음 단계를 수행하여 그룹을 제거해야 합니다.  
...
- Runbook을 시작하는 데 사용할 수 있는 여러 가지 방법에 대해 자세히 알아보려면 [Azure 자동화에서 Runbook 시작](automation-starting-a-runbook.md)
