---
title: Azure Automation 상태 구성에서 DSC 구성 컴파일
...
ms.locfileid: "82996069"
---
# <a name="compile-dsc-configurations-in-azure-automation-state-configuration"></a>Azure Automation 상태 구성에서 DSC 구성 컴파일
...
Azure DSC (필요한 상태 구성) 확장을 사용 하 여 Azure Resource Manager 템플릿을 사용 하 여 Azure Vm에 구성을 푸시할 수도 있습니다. Azure DSC 확장은 Azure VM 에이전트 프레임워크를 사용하여 Azure VM에서 실행되는 DSC 구성을 제공하고 적용하며 보고합니다. Azure Resource Manager 템플릿 사용에 대 한 자세한 내용은 [Azure Resource Manager 템플릿을 사용 하 여 필요한 상태 구성 확장](https://docs.microsoft.com/azure/virtual-machines/extensions/dsc-template#details)을 참조 하세요. 

## <a name="compiling-a-dsc-configuration-in-azure-state-configuration"></a>Azure 상태 구성에서 DSC 구성 컴파일

### <a name="portal"></a>포털

1. Automation 계정에서 **상태 구성 (DSC)** 을 클릭 합니다.
...
다음 예제에서는 및 `FeatureName` `IsPresent` 매개 변수를 사용 하 여 컴파일 중에 생성 된 **ParametersExample** 노드 구성의 속성 값을 확인 합니다.
...
개체를 매개 변수로 `PSCredential` 전달 하는 방법에 대 한 자세한 내용은 [자격 증명 자산](#credential-assets)을 참조 하세요.
...
`ConfigurationData`는 PowerShell DSC를 사용 하는 동안 환경 관련 구성에서 구조적 구성을 분리할 수 있도록 하는 기본 제공 DSC 매개 변수입니다. 자세한 내용은 [POWERSHELL DSC의 "What"에서 "Where](https://devblogs.microsoft.com/powershell/separating-what-from-where-in-powershell-dsc/)"를 구분 하는 방법을 참조 하세요.

> [!NOTE]
> Azure Automation 상태 구성에서 컴파일하는 경우에는를 `ConfigurationData` 사용할 수 있지만 Azure Portal는 Azure PowerShell 사용할 수 없습니다.

다음 예제 DSC 구성은 및 키워드 `ConfigurationData` 를 `$ConfigurationData` 통해를 `$AllNodes` 사용 합니다. 또한이 예제에는 [Xwebadministration 모듈이](https://www.powershellgallery.com/packages/xWebAdministration/) 필요 합니다.
...
Azure Automation에서 DSC 구성은 `Get-AutomationPSCredential` cmdlet를 사용하여 Automation 자격 증명 자산을 참조할 수 있습니다. 구성에 개체를 `PSCredential` 지정 하는 매개 변수가 있는 경우 Azure Automation `Get-AutomationPSCredential` 자격 증명 자산의 문자열 이름을 cmdlet에 전달 하 여를 사용 하 여 자격 증명을 검색 합니다. 그런 다음 `PSCredential` 개체를 필요로 하는 매개 변수에 대해 해당 개체를 사용 합니다. 내부적으로 해당 이름을 가진 Azure Automation 자격 증명 자산은 검색되고 구성에 전달됩니다. 아래 예제에서는이 시나리오를 수행 하는 방법을 보여 줍니다.
...
구성 데이터를 사용 하 여 생성 된 노드 구성 Mof에서 일반 텍스트로 자격 증명이 출력 되도록 PowerShell DSC에 지시할 수 있습니다. DSC 구성에 `PSDscAllowPlainTextPassword = $true` 표시 `ConfigurationData` 되 고 자격 증명을 사용 하는 각 노드 블록 이름에 대해를 통해를 전달 해야 합니다.
...
PowerShell로 이전 DSC 구성을 컴파일할 수 있습니다. 다음 PowerShell 코드는 Azure Automation 상태 구성 끌어오기 서버: **Credentialsample. MyVM1** 및 **MyVM2**에 두 개의 노드 구성을 추가 합니다.
...
> [!NOTE]
> 컴파일이 완료 되 면이 메시지를 무시 해도 되는 `The 'Microsoft.PowerShell.Management' module was not imported because the 'Microsoft.PowerShell.Management' snap-in was already imported.` 오류 메시지가 표시 될 수 있습니다.

## <a name="compiling-your-dsc-configuration-in-windows-powershell"></a>Windows PowerShell에서 DSC 구성 컴파일

Azure 외부에서 컴파일된 노드 구성 (MOF 파일)을 가져올 수도 있습니다. 가져오기에는 개발자 워크스테이션 또는 [Azure DevOps](https://dev.azure.com)와 같은 서비스에서 컴파일이 포함 됩니다. 이 방법은 성능 및 안정성을 비롯 한 여러 가지 이점을 제공 합니다.
...
> [!NOTE]
> 노드 구성 파일은 Azure Automation 가져올 수 있도록 1mb 보다 크지 않아야 합니다.
...
1. Automation 계정의 **구성 관리**아래에서 **상태 구성 (DSC)** 을 클릭 합니다.
1. 상태 구성 (DSC) 페이지에서 **구성** 탭을 클릭 한 다음 **추가**를 클릭 합니다.
1. 가져오기 페이지에서 **노드 구성 파일** 필드 옆에 있는 폴더 아이콘을 클릭 하 여 로컬 컴퓨터에서 노드 구성 MOF 파일을 찾습니다.
