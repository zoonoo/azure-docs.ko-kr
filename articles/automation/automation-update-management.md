...
업데이트 배포에 컴퓨터 그룹이 포함되면 일정을 만들 때 그룹 멤버 자격이 한 번만 평가됩니다. 이후의 그룹 변경 내용은 반영되지 않습니다. 이러한 [동적 그룹](#using-dynamic-groups)을 사용하여 이를 해결하기 위해 배포 시에 확인되고 azure vm에 대한 쿼리 또는 비 azure vm에 대한 저장된 검색에 의해 정의됩니다.
...
유지 관리 기간 업데이트를 설치 하는 데 허용 되는 시간을 제어 합니다. 유지 관리 기간을 지정할 때는 다음 사항을 고려 하십시오.

* 유지 관리 기간 설치 하려는 업데이트의 수를 제어 합니다.
* 유지 관리 기간이 끝나면 새 업데이트를 설치 하는 것을 중지 하지 않습니다 업데이트 관리.
* 유지 관리 기간을 초과 하는 경우 업데이트 관리은 진행 중인 업데이트를 종료 하지 않습니다.
* Windows에서 유지 관리 기간을 초과 하는 경우 Service Pack 업데이트를 설치 하는 데 오랜 시간이 소요 되는 경우가 많습니다.
