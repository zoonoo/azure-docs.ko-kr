<properties 
   ...
   description="Azure 자동화의 Runbook은 Windows PowerShell 워크플로를 기반으로 합니다. 이 문서는 PowerShell과 PowerShell 워크플로 간의 중대한 차이를 이해하는 PowerShell에 익숙한 작성자의 간단한 설명으로 제공됩니다."
   services="automation"
   documentationCenter=""
   authors="bwren"
   manager="stevenka"
...
Azure 자동화에서는 *텍스트*와 *그래픽* 두 종류의 runbook이 있습니다. runbook을 만들 때, runbook의 유형을 정할 수 있고, 한번 만들어진 runbook은 유형을 변경할 수 없습니다.

텍스트 runbook은 PowerShell 워크플로를 사용하여 직접 작업하거나 PowerShell ISE같은 오프라인 편집기나 Azure 자동화의 텍스트 편집기 사용을 선호하는 사용자들에게 적합합니다. 텍스트 runbook을 만드는 경우 이 문서의 정보를 이해해야 합니다.
