...
RBAC(역할 기반 액세스 제어)를 통해 Azure 리소스에 대한 액세스 관리가 가능합니다. [RBAC](../role-based-access-control/overview.md)를 사용하여 팀 내에서 업무를 분리하고 사용자, 그룹 및 애플리케이션에 해당 작업을 수행하는 데에만 필요한 권한을 부여할 수 있습니다. Azure Portal, Azure 명령줄 도구 또는 Azure 관리 Api를 사용 하 여 사용자에 게 역할 기반 액세스 권한을 부여할 수 있습니다.
...
| Automation 운영자 |Automation 연산자 역할을 사용하면 Runbook 이름 및 속성을 보고 Automation 계정의 모든 Runbook에 대한 작업을 만들고 관리할 수 있습니다. 이 역할은 자격 증명 자산과 runbook을 보거나 수정 하지는 않지만 조직의 구성원이 이러한 runbook을 실행할 수 있도록 허용 하는 등 Automation 계정 리소스를 보호 하려는 경우에 유용 합니다. |
...
|Microsoft.Automation/automationAccounts/linkedWorkspace/read|Automation 계정에 연결 된 작업 영역을 가져옵니다.|
...
|Microsoft.Automation/automationAccounts/*|Automation 계정 관리|
...
|Microsoft.OperationalInsights/workspaces/savedSearches/*|저장 된 검색 Azure Monitor 로그를 관리 합니다.|
...
|Microsoft.OperationalInsights/workspaces/storageinsightconfigs/*|Azure Monitor logs 저장소 정보 구성을 관리 합니다.|
|Microsoft.Support/*|지원 티켓을 만들고 관리합니다.|
...
## <a name="onboarding-permissions"></a>온 보 딩 권한

다음 섹션에서는 변경 내용 추적 또는 업데이트 관리 솔루션에 대해 가상 컴퓨터를 등록 하는 데 필요한 최소 사용 권한에 대해 설명 합니다.

### <a name="permissions-for-onboarding-from-a-vm"></a>VM에서 온 보 딩에 대 한 사용 권한
...
| Log Analytics 공급자 등록 |Microsoft 인 사이트/등록/동작 | Subscription|

<sup>1</sup> 이 권한은 VM 포털 환경을 통해 등록 하는 데 필요 합니다.

### <a name="permissions-for-onboarding-from-automation-account"></a>Automation 계정에서 온 보 딩에 대 한 사용 권한
...
다음 섹션에서는 [Azure Portal](#configure-rbac-using-the-azure-portal) 및 [PowerShell](#configure-rbac-using-powershell)을 통해 Automation 계정에서 RBAC를 구성 하는 방법을 보여 줍니다.
...
2. 액세스 **제어 (iam)** 를 클릭 하 여 액세스 제어 (iam) 페이지를 엽니다. 이 페이지를 사용 하 여 새 사용자, 그룹 및 응용 프로그램을 추가 하 여 Automation 계정을 관리 하 고 Automation 계정에 구성 가능한 기존 역할을 볼 수 있습니다.
3. **역할 할당** 탭을 클릭합니다.
...
다음 [Azure PowerShell cmdlet](../role-based-access-control/role-assignments-powershell.md)을 사용 하 여 Automation 계정에 대 한 역할 기반 액세스를 구성할 수도 있습니다.

[AzRoleDefinition](https://docs.microsoft.com/powershell/module/Az.Resources/Get-AzRoleDefinition?view=azps-3.7.0) 에는 Azure Active Directory에서 사용할 수 있는 모든 RBAC 역할이 나열 됩니다. 이 cmdlet을 `Name` 매개 변수와 함께 사용 하 여 특정 역할이 수행할 수 있는 모든 작업을 나열할 수 있습니다.
...
[AzRoleAssignment](https://docs.microsoft.com/powershell/module/az.resources/get-azroleassignment?view=azps-3.7.0) 는 지정 된 범위에서 AZURE AD RBAC 역할 할당을 나열 합니다. 매개 변수를 사용 하지 않으면이 cmdlet은 구독에서 만든 모든 역할 할당을 반환 합니다. `ExpandPrincipalGroups` 매개 변수를 사용 하 여 지정 된 사용자에 대 한 액세스 할당 및 사용자가 속한 그룹을 나열 합니다.

**예:** Automation 계정 내에서 모든 사용자와 해당 역할을 나열 하려면 다음 cmdlet을 사용 합니다.
...
[AzRoleAssignment](https://docs.microsoft.com/powershell/module/Az.Resources/New-AzRoleAssignment?view=azps-3.7.0) 를 사용 하 여 특정 범위에 대 한 사용자, 그룹 및 응용 프로그램에 대 한 액세스 권한을 할당 합니다.
...
특정 범위에서 지정 된 사용자, 그룹 또는 응용 프로그램에 대 한 액세스 권한을 제거 하려면 [AzRoleAssignment](https://docs.microsoft.com/powershell/module/Az.Resources/Remove-AzRoleAssignment?view=azps-3.7.0) 를 사용 합니다.

**예:** 다음 명령을 사용 하 여 Automation 계정 범위의 Automation Operator 역할에서 사용자를 제거 합니다.

```azurepowershell-interactive
Remove-AzRoleAssignment -SignInName <sign-in Id of a user you wish to remove> -RoleDefinitionName 'Automation Operator' -Scope '/subscriptions/<SubscriptionID>/resourcegroups/<Resource Group Name>/Providers/Microsoft.Automation/automationAccounts/<Automation account name>'
```

`sign-in ID of a user you wish to remove`앞의 예제에서, `SubscriptionID` `Resource Group Name`, 및 `Automation account name` 를 사용자의 계정 세부 정보로 바꿉니다. 사용자 역할 할당 제거를 계속 하기 전에 확인 메시지가 표시 되 면 **예** 를 선택 합니다.

### <a name="user-experience-for-automation-operator-role---automation-account"></a>Automation 운영자 역할에 대 한 사용자 환경-Automation 계정

Automation 계정 범위의 automation Operator 역할에 할당 된 사용자가 자신에 게 할당 된 Automation 계정을 볼 때 사용자는 Automation 계정에서 만든 runbook, runbook 작업 및 일정의 목록만 볼 수 있습니다. 이 사용자는 이러한 항목의 정의를 볼 수 없습니다. 사용자는 runbook 작업을 시작, 중지, 일시 중단, 다시 시작 또는 예약할 수 있습니다. 그러나 사용자는 구성, hybrid worker 그룹 또는 DSC 노드와 같은 다른 Automation 리소스에 대 한 액세스 권한이 없습니다.

![리소스에 대한 액세스 권한 없음](media/automation-role-based-access-control/automation-10-no-access-to-resources.png)

## <a name="configure-rbac-for-runbooks"></a>Runbook에 대 한 RBAC 구성

Azure Automation를 사용 하면 RBAC를 특정 runbook에 할당할 수 있습니다. 이렇게 하려면 다음 스크립트를 실행 하 여 사용자를 특정 runbook에 추가 합니다. Automation 계정 관리자 또는 테넌트 관리자에서이 스크립트를 실행할 수 있습니다.
...
스크립트가 실행 되 면 사용자가 Azure Portal에 로그인 하 고 **모든 리소스**를 선택 합니다. 사용자는 목록에서 Automation Runbook 연산자로 추가 된 runbook을 볼 수 있습니다.
...
Runbook 범위에서 Automation Operator 역할에 할당 된 사용자가 할당 된 runbook을 볼 때 사용자는 runbook을 시작 하 고 runbook 작업만 볼 수 있습니다.

![시작에만 액세스](media/automation-role-based-access-control/automation-only-start.png)

## <a name="next-steps"></a>다음 단계

* Azure Automation에 대해 RBAC를 구성 하는 방법에 대 한 자세한 내용은 [Azure PowerShell를 사용 하 여 rbac 관리](../role-based-access-control/role-assignments-powershell.md)를 참조 하세요.
* Runbook을 시작 하는 방법에 대 한 자세한 내용은 [Runbook 시작](automation-starting-a-runbook.md)을 참조 하세요.
* Runbook 형식에 대 한 자세한 내용은 [Azure Automation runbook 형식](automation-runbook-types.md)을 참조 하세요.