...
Azure 자동화 상태 구성은 30일 동안 노드 상태 데이터를 유지합니다. 이 데이터를 더 긴 기간 동안 유지하려는 경우 Log Analytics 작업 영역으로 노드 상태 데이터를 보낼 수 있습니다. Azure Portal에서 또는 PowerShell을 사용하여 노드 및 노드 구성의 개별 DSC 리소스에 대한 준수 상태를 볼 수 있습니다. 

Azure Monitor 로그는 자동화 상태 구성 데이터에 대한 운영 가시성을 높이고 인시던트를 보다 신속하게 해결하는 데 도움이 될 수 있습니다. Azure 모니터 로그를 사용하면 다음을 수행할 수 있습니다.

- 관리되는 노드 및 개별 리소스에 대한 규정 준수 정보를 가져옵니다.
- 규정 준수 상태에 따라 이메일 또는 경고를 트리거합니다.
- 관리되는 노드 에서 고급 쿼리를 작성합니다.
- 자동화 계정 간에 규정 준수 상태를 상호 연결합니다.
...
Azure 자동화 상태 구성에서 Azure 모니터 로그로 데이터 가져오기를 시작하려면 다음 단계를 완료하십시오.
...
1. 다음 PowerShell cmdlet을 실행하고 `<AutomationResourceId>` `<WorkspaceResourceId>` 이전 `ResourceId` 각 단계의 값을 대체합니다.
...
로그 검색 창은 자동화 계정 리소스로 범위가 조정된 쿼리 영역으로 열립니다. Azure Monitor 로그를 검색하여 DSC 작업에 대한 상태 구성 로그를 검색할 수 있습니다. DSC 작업에 대한 레코드가 `AzureDiagnostics` 테이블에 저장됩니다. 예를 들어 호환되지 않는 노드를 찾으려면 다음 쿼리를 입력합니다.
...
* 해당 `DscResourceStatusData` 리소스에 적용된 노드 구성에서 호출된 각 DSC 리소스에 대한 작업을 반환하도록 필터링합니다. 
* `DscResourceStatusData` 필터링하여 실패한 DSC 리소스에 대한 오류 정보를 반환합니다.
...
Azure Monitor 로그를 사용하는 한 가지 장점은 노드 간에 실패한 검사를 검색할 수 있다는 것입니다. 실패한 DSC 리소스의 모든 인스턴스를 찾으려면 다음을 수행하십시오.
...
- 다른 검색 쿼리를 구성하고 Azure Monitor 로그를 사용하여 자동화 상태 구성 로그를 검토하는 방법에 대한 자세한 내용은 [Azure Monitor 로그에서 로그 검색을](../log-analytics/log-analytics-log-searches.md)참조하세요.
