...
description: Cloud Foundry 엔터프라이즈 환경을 개선 하기 위해 Azure 서비스를 사용할 수 하는 방법을 설명 합니다.
...
ms.locfileid: "57550176"
---
# <a name="integrate-cloud-foundry-with-azure"></a>Azure와 Cloud Foundry 통합

[Cloud Foundry](https://docs.cloudfoundry.org/)는 클라우드 공급자의 IaaS 플랫폼에서 실행되는 PaaS 플랫폼입니다. 클라우드 공급자 간에 일관된 애플리케이션 배포 환경을 제공합니다. 또한 확장성, 엔터프라이즈급 HA 사용 하 여 다양 한 Azure 서비스와 통합 및 비용 절감 합니다.
온라인으로 유연하게 확장될 수 있는 [Cloud Foundry의 하위 시스템](https://docs.cloudfoundry.org/concepts/architecture/)은 모두 6가지로, 라우팅, 인증, 응용 프로그램 수명 주기 관리, 서비스 관리, 메시징 및 모니터링 합니다. 각 하위 시스템에 대 한 해당 Azure 서비스를 사용 하는 Cloud Foundry를 구성할 수 있습니다. 
...
기본적으로 Azure 기본 부하 분산 장치는 들어오는 CF API/앱 요청에 사용되어 Gorouters에 전달합니다. Diego Brain, MySQL, ERT와 같은 CF는 부하 분산 장치를 사용하여 HA를 위해 트래픽을 분산할 수도 있습니다. Azure는 또한 완전히 관리 되는 부하 분산 솔루션 집합을 제공합니다. TLS 종료 ("SSL 오프 로드") 또는 HTTP/HTTPS 요청을 응용 프로그램 계층 처리를 원하는 경우 Application Gateway는 것이 좋습니다. 계층 4에 대한 고가용성 및 확장성 부하 분산은 표준 부하 분산 장치를 사용하세요.
...
Azure Load Balancer는 계층 4 부하 분산 장치입니다. 부하 분산 집합의 서비스 인스턴스 간에 트래픽을 분산 하기 위해 사용 됩니다. 표준 버전에서는 기본 버전을 기반으로 [고급 기능](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview)을 제공합니다. 예: 1 백 엔드 풀 최대 한도는 100대에서 1000대 VM으로 증가되었습니다.  2. 이제 엔드포인트는 단일 가용성 집합 대신 여러 가용성 집합을 지원합니다.  3. 추가 기능에는 HA 포트, 다양 한 모니터링 데이터 등과 같은 합니다. Azure 가용성 영역을 이동 하는 경우 표준 부하 분산 장치는 필요 합니다. 새 배포에서 Azure 표준 Load Balancer를 시작하는 것이 좋습니다. 
...
Azure Log Analytics Nozzle은로 메트릭을 전달 하는 Cloud Foundry 구성 요소를 [Cloud Foundry loggregator firehose](https://docs.cloudfoundry.org/loggregator/architecture.html) 하 [Azure Monitor 로그](https://azure.microsoft.com/services/log-analytics/)합니다. Nozzle을 사용하면 여러 배포에서 CF 시스템 상태와 성능 메트릭을 수집, 확인 및 분석할 수 있습니다.
클릭 [여기](https://docs.microsoft.com/azure/cloudfoundry/cloudfoundry-oms-nozzle) 모니터의 콘솔 로그를 오픈 소스 및 Pivotal Cloud Foundry 환경에 Azure Log Analytics Nozzle 배포 후 Azure에서 데이터에 액세스 하는 방법을 알아봅니다. 
> [!NOTE]
> PCF 2.0에서 Vm에 대 한 상태 메트릭을 BOSH Loggregator Firehose 기본적으로 전달 됩니다 하 고 Azure Monitor 로그 콘솔에 통합 됩니다.
...
#### <a name="use-azure-first-party-services"></a>첫 번째 Azure 타사 서비스를 사용 합니다. 
...
Azure 통합 기능을 사용할 수 있는 처음 [오픈 소스 Cloud Foundry](https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/tree/master/docs/advanced/)가 Pivotal Cloud Foundry에 사용할 수 있게 합니다. *으로 표시된 기능은 PCF를 통해 지원되지 않습니다. Azure Stack을 사용 하 여 cloud Foundry 통합 하거나이 문서에 적용 되지 않습니다.
