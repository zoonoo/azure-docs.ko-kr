...
이 문서에서는 Azure CLI를 사용 하 여 Azure Arc 사용 서버에서 지 원하는 VM 확장을 Linux 또는 Windows 하이브리드 컴퓨터에 배포 하 고 제거 하는 방법을 보여 줍니다.
...
Arc 사용 서버에서 VM 확장 목록을 가져오려면 및 매개 변수와 함께 [az connectedmachine extension list](/cli/azure/ext/connectedmachine/connectedmachine/extension#ext_connectedmachine_az_connectedmachine_extension_list) 를 사용 `--machine-name` `--resource-group` 합니다.
...
다음 예제에서는 명령의 부분 JSON 출력을 보여 줍니다 `az connectedmachine extension -list` .
...
Arc 사용 서버에서 설치 된 VM 확장을 제거 하려면, 및 매개 변수와 함께 [az connectedmachine extension delete](/cli/azure/ext/connectedmachine/connectedmachine/extension#ext_connectedmachine_az_connectedmachine_extension_delete) 를 사용 `--extension-name` `--machine-name` `--resource-group` 합니다.
...
- 명령에 대 한 자세한 내용은 Azure CLI VM 확장 [개요](/cli/azure/ext/connectedmachine/connectedmachine/extension) 문서를 참조 하세요.
