...
현재 Azure Portal에서 Azure Arc 데이터 컨트롤러를 만들 수 있습니다. Azure Arc 지원 데이터 서비스를 위한 다른 도구는 직접 연결 모드에서 데이터 컨트롤러 만들기를 지원하지 않습니다. 자세한 내용은 [알려진 문제 - Azure Arc 지원 데이터 서비스(미리 보기)](known-issues.md)를 참조하세요.
