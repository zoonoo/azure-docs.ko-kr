---
title: Azure Arc 지원 클러스터 구성에 GitOps 사용(미리 보기)
services: azure-arc
...
이 아키텍처는 GitOps 워크플로를 사용하여 클러스터를 구성하고 애플리케이션을 배포합니다. 구성은 .yaml 파일에서 선언적으로 설명되고 Git에 저장됩니다. 에이전트가 Git 리포지토리를 감시하여 변경 내용을 적용합니다.  또한 동일한 에이전트는 클러스터 상태가 Git 리포지토리에 선언된 상태와 일치하도록 주기적으로 보장하고 관리되지 않는 변경 내용이 발생한 경우 클러스터를 원하는 상태로 되돌립니다.

클러스터와 하나 이상의 Git 리포지토리 간의 연결은 Azure Resource Manager에서 `sourceControlConfiguration` 확장 리소스로 추적됩니다. `sourceControlConfiguration` 리소스 속성은 Kubernetes 리소스가 Git에서 클러스터로 이동하는 위치와 방법을 나타냅니다. 데이터 기밀성을 보장하기 위해 `sourceControlConfiguration` 데이터는 미사용 암호화를 사용하여 CosmosDb 데이터베이스에 저장됩니다.

클러스터에서 실행되는 Azure Arc 지원 Kubernetes `config-agent`는 새로운 또는 업데이트된 `sourceControlConfiguration` 리소스를 감시하고 Git 리포지토리 링크 추가, 업데이트 또는 제거를 자동으로 오케스트레이션합니다.

동일한 패턴을 사용하여 이종 환경 간에 배포될 수 있는 더 큰 클러스터 컬렉션을 관리할 수 있습니다. 예를 들어 조직의 기준 구성을 정의하고 수십 개의 Kubernetes 클러스터에 한 번에 적용하는 하나의 리포지토리가 있을 수 있습니다.
...
`config-agent`는 30초마다 Azure에서 새로운 또는 업데이트된 `sourceControlConfiguration`을 폴링합니다.  이는 `config-agent`가 새로운 구성 또는 업데이트된 구성을 검색하는 데 걸리는 최대 시간입니다.
프라이빗 리포지토리를 연결하는 경우 [프라이빗 git 리포지토리에서 구성 적용](#apply-configuration-from-a-private-git-repository)의 단계도 완료해야 합니다.

### <a name="using-azure-cli"></a>Azure CLI 사용

`k8sconfiguration`에 대한 Azure CLI 확장을 사용하여 연결된 클러스터를 [예제 Git 리포지토리](https://github.com/Azure/arc-k8s-demo)에 연결해 보겠습니다. 이 구성에 이름 `cluster-config`를 지정하고, `cluster-config` 네임스페이스에 연산자를 배포하도록 에이전트에 지시하고, 연산자에게 `cluster-admin` 권한을 부여합니다.
...
`--enable-helm-operator` : Helm 차트 배포에 대한 지원을 사용하도록 설정하는 선택적 스위치입니다. 이 옵션은 기본적으로 사용하지 않도록 설정됩니다.
...
`--operator-params` : 연산자에 대한 선택적 매개 변수입니다. 작은따옴표 안에 지정해야 합니다. 예를 들어 ```--operator-params='--git-readonly --git-path=releases/prod' ```
...
* enableHelmOperator가 true인 경우 operatorInstanceName + operatorNamespace 문자열이 총 47자를 초과할 수 없습니다.  이 제한을 준수하지 않으면 다음 오류가 발생합니다.
...
1. `config-agent`가 구성 속성을 읽고 `flux`의 관리되는 인스턴스를 배포할 준비를 합니다.
    ...
    * `flux`가 SSH 키를 생성하고 공개 키를 로깅합니다.
...
프라이빗 git 리포지토리를 사용하는 경우 하나 이상의 작업을 수행하여 루프를 닫아야 합니다. `flux`에 의해 생성된 공개 키를 리포지토리에 **배포 키**로 추가해야 합니다.
...
배포 키를 관리하는 방법에 대한 자세한 내용은 GitHub 문서를 참조하세요.
...
Azure CLI 또는 Azure Portal을 사용하여 `sourceControlConfiguration`을 삭제할 수 있습니다.  delete 명령을 시작한 후 Azure에서는 `sourceControlConfiguration` 리소스가 즉시 삭제되지만 클러스터에서 연결된 개체는 전체 삭제되려면 최대 1시간이 걸릴 수 있습니다(이를 단축하기 위한 백로그 항목이 있음). 네임스페이스 범위를 사용하여 `sourceControlConfiguration`을 만든 경우 해당 네임스페이스는 클러스터에서 삭제되지 않습니다(해당 네임스페이스에서 만들어졌을 수 있는 다른 리소스의 중단을 방지하기 위해).

`sourceControlConfiguration`이 삭제될 때 추적된 git 리포지토리로부터 배포의 결과로 생성된 클러스터에 대한 변경 내용은 삭제되지 않습니다.
...
- [클러스터 구성에 GitOps를 Helm과 함께 사용](./use-gitops-with-helm.md)
