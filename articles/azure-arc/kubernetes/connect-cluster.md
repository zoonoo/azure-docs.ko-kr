...
ms.locfileid: "100652516"
---
# <a name="connect-an-azure-arc-enabled-kubernetes-cluster-preview"></a>Azure Arc 사용 Kubernetes 클러스터 연결 (미리 보기)

이 문서에서는 기존 Kubernetes 클러스터를 Azure Arc에 연결 하는 과정을 안내 합니다. 클러스터 연결에 대 한 개념적인 사용에 대 한 자세한 내용은 [Azure Arc Enabled Kubernetes Agent Architecture 문서](./conceptual-agent-architecture.md)를 참조 하세요.
...
* 및에서 사용 되는 사용자 또는 서비스 사용자에 대 한 ' 읽기 ' 및 ' 쓰기 ' 권한은 `az login` `az connectedk8s connect` 리소스 종류에 대 한 및 명령 `Microsoft.Kubernetes/connectedclusters` 입니다. "Kubernetes Cluster-Azure Arc 온 보 딩" 역할에는 이러한 권한이 있으며 사용자 또는 서비스 주체의 역할 할당에 사용할 수 있습니다.
...
* Azure Arc enabled Kubernetes CLI 확장:
  
  * `connectedk8s`Kubernetes 클러스터를 Azure에 연결 하는 데 도움이 되도록 확장을 설치 합니다.
...
| `https://eus.his.arc.azure.com`, `https://weu.his.arc.azure.com`                                                                            |  MSI (시스템 할당 관리 서비스 ID) 인증서를 가져오는 데 필요 합니다.                                                                  |
...
[Azure Portal](https://portal.azure.com/)에서이 리소스를 볼 수도 있습니다.
1. 브라우저에서 포털을 엽니다.
1. 명령의 앞부분에서 사용 된 리소스 이름 및 리소스 그룹 이름 입력을 기반으로 리소스 그룹 및 Azure Arc enabled Kubernetes 리소스로 이동 `az connectedk8s connect` 합니다.  
> [!NOTE]
...
> * 위의 프록시 사양은 현재 Azure Arc 에이전트에만 적용 되며에서 사용 되는 flux pod는 적용 되지 않습니다 `sourceControlConfiguration` . Azure Arc enabled Kubernetes 팀이이 기능에 대해 적극적으로 작업 중입니다.
