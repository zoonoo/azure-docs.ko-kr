...
로컬 또는 클라우드 스냅숏을 사용하여 동일한 장치, 다른 장치 또는 가상 컴퓨터에 복제본을 만들 수 있습니다.
...
7. 복제 작업 완료 후:
  ...
  2. 복제한 원본 볼륨에 연관된 볼륨 컨테이너를 선택합니다. 볼륨 목록에 방금 만든 복제본이 보입니다.

>[AZURE.NOTE]모니터링 및 기본 백업은 복제된 볼륨에서 자동으로 비활성화됩니다.

이러한 방식으로 만들어진 복제본은 임시 복제본입니다. 복제 유형에 대한 자세한 내용은 [임시 및 영구 복제본 비교](#transient-vs.-permanent-clones)를 참조하세요.

이 복제본은 이제 정규 볼륨이며 볼륨에 사용할 수 있는 모든 작업은 복제본에 사용할 수 있습니다. 백업에 대해 이 볼륨을 구성해야 합니다.

## 임시 및 영구 복제본 비교

백업 세트에서 특정 볼륨을 복제할 수 있습니다. 이러한 방식으로 만들어진 복제본은 *임시* 복제본입니다. 임시 복제본에는 원본 볼륨에 대한 참조가 있으며 로컬로 쓰는 동안 읽기위해 해당 볼륨을 사용합니다. 특히 복제된 볼륨이 큰 경우에는 성능이 저하될 수 있습니다.

임시 복제본의 클라우드 스냅숏을 수행한 후 결과 복제본은 *영구* 복제본이 됩니다. 영구 복제본은 독립적이며 복제된 원본 볼륨에 대한 참조가 없습니다. 성능 향상을 위해 영구 복제본을 만드는 것이 좋습니다.

## 임시 및 영구 복제본에 대한 시나리오

다음 섹션에서는 임시 및 영구 복제본을 사용할 수 있는 예제 상황에 대해 설명합니다.

### 임시 복제본을 사용하여 항목 수준 복구

1년 된 Microsoft PowerPoint 프레젠테이션 파일을 복구해야 합니다. IT 관리자는 해당 시간 프레임에서 특정 백업을 식별하고 볼륨을 필터링합니다. 그런 다음 관리자는 해당 볼륨을 복제하고, 필요한 파일을 찾아 사용자에게 제공합니다. 이 시나리오에서는 임시 복제본이 사용됩니다.
 
### 영구 복제본을 사용하여 프로덕션 환경에서 테스트

프로덕션 환경에서 테스트 버그를 확인해야 합니다. 프로덕션 환경에서 볼륨의 복제본을 만듭니다. 향상된 성능을 위해 이 복제본의 클라우드 스냅숏을 수행해야 합니다. 복제된 볼륨은 이제 독립적이며 성능이 향상됩니다. 이 시나리오에서는 영구 복제본이 사용됩니다.
