...
![][1]
4.	생성 프로세스 후 Puttygen.exe에서 생성된 키를 표시합니다. 예를 들면 다음과 같습니다.  
![][2]
...
[Azure 미리 보기 포털](https://portal.azure.com/)의 작업 표시줄에서 **새로 만들기**를 클릭한 후 요구에 따라 Linux 이미지를 선택하여 이미지를 만듭니다. 다음 예제에서는 Ubuntu 14.04 이미지를 사용합니다. 
![][3]
...
1.	Azure 미리 보기 포털에서 **찾아보기** -> **가상 컴퓨터**를 클릭한 다음 직접 만든 가상 컴퓨터를 클릭합니다.  
![][5]
2.	가상 컴퓨터에 끝점을 추가하려면 **끝점** 확인란을 클릭합니다.
![][6] 
3.	**추가**를 클릭합니다.  
	1.	**끝점**에서 끝점에 끝점 이름을 입력하고 **공용 포트**에 80을 입력합니다.  
...
oracle-jdk를 설치한 경우 다음과 같은 메시지가 표시되어야 합니다.
![][15] 
...
tomcat7이 성공적으로 설치되었는지 확인하려면 tomcat 서버의 DNS 이름(http://tomcatexample.cloudapp.net/ is the example URL in this article)으로 이동합니다. 다음과 같은 페이지가 표시되면 tomcat7이 올바르게 설치된 것입니다.
![][16] 
...
-	**가능한 근본 원인**   
	1.	Tomcat 수신 대기 포트가 Tomcat 트래픽에 대한 가상 컴퓨터 끝점의 개인 포트와 다릅니다.  
		...
		공용 포트 및 개인 포트 끝점 설정을 검사하고 개인 포트가 Tomcat 수신 대기 포트와 같은지 확인합니다. 가상 컴퓨터의 끝점 구성에 대한 자세한 내용은 단계 1: 이미지 만들기를 참조하세요.  
