...
이 자습서에서는 모바일 서비스에서 생성한 SAS를 사용하여 사진을 찍고 이미지를 Azure에 업로드하는 기능을 [GetStartedWithData 샘플 앱 프로젝트](/ko-kr/documentation/articles/mobile-services-dotnet-backend-windows-phone-get-started-data/)에 추가합니다. 이를 위해 간단한 TodoList 앱을 업데이트하여 Blob 저장소 서비스에 이미지를 업로드하는 다음과 같은 기본 단계를 안내합니다.
