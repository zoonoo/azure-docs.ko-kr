<properties title="Azure 포털에서 Docker VM 확장을 사용하는 방법" pageTitle="Azure에서 Linux용 Docker VM 확장 사용" description="Docker 및 Azure 가상 컴퓨터 확장에 대해 설명하고, azure-cli 명령 인터페이스를 사용하여 명령줄에서 Docker 호스트인 가상 컴퓨터를 Azure에 프로그래밍 방식으로 만드는 방법을 안내합니다." metaKeywords="linux, virtual machines, vm, azure, docker, linux containers,  lxc, virtualization" services="virtual-machines" solutions="dev-test" documentationCenter="virtual-machines" authors="rasquill" videoId="" scriptId="" manager="timlt" />
...
-   [이미지 갤러리에서 새 VM 만들기][이미지 갤러리에서 새 VM 만들기]
-   [Docker 인증서 만들기][Docker 인증서 만들기]
-   [Docker VM 확장 추가][Docker VM 확장 추가]
-   [Docker 클라이언트 및 Azure Docker 호스트 테스트][Docker 클라이언트 및 Azure Docker 호스트 테스트]
-   [다음 단계][다음 단계]

> [WACOM.NOTE] 이 항목에서는 Azure 포털에서 Docker VM을 만드는 방법을 설명합니다. 명령줄에서 Docker VM을 만드는 방법을 확인하려면 [Azure 플랫폼 간 인터페이스(xplat-cli)에서 Docker VM 확장을 사용하는 방법][Azure 플랫폼 간 인터페이스(xplat-cli)에서 Docker VM 확장을 사용하는 방법]을 참조하세요. 컨테이너와 해당 이점에 대한 간략한 설명을 확인하려면 [Docker 요약 화이트보드][Docker 요약 화이트보드]를 참조하세요.

## <span id="createvm"></span>이미지 갤러리에서 새 VM 만들기</a>
...
> [WACOM.NOTE] 현재는 2014년 7월 이후의 Ubuntu 14.04 LTS 이미지만이 Docker VM 확장을 지원합니다.

![새 Ubuntu 이미지 만들기][새 Ubuntu 이미지 만들기]

## <a id'dockercerts'>Docker 인증서 만들기</a>

VM을 만든 후에는 클라이언트 컴퓨터에 Docker가 설치되어 있는지 확인합니다. 자세한 내용은 [Docker 설치 지침][Docker 설치 지침]을 참조하세요.

[https를 사용하여 Docker 실행][https를 사용하여 Docker 실행]의 지침에 따라 Docker 통신용 인증서 및 키 파일을 만든 다음 클라이언트 컴퓨터의 **`~/.docker`** 디렉터리에 저장합니다.

> [WACOM.NOTE] 현재는 포털의 Docker VM 확장을 사용하려면 base64로 인코딩된 자격 증명이 필요합니다.
...
> [WACOM.NOTE] 위의 이미지에 나와 있는 것처럼 포트에는 기본적으로 4243이 입력되어 있습니다. 여기에는 원하는 어떤 끝점이든 입력할 수 있지만 다음 단계에서 일치하는 끝점을 열어야 합니다. 기본값을 변경하는 경우에는 다음 단계에서 일치하는 끝점을 여세요.
...
## <span id="testclientandserver"></span>Docker 클라이언트 및 Azure Docker 호스트 테스트</a>
