...
12. **프로세스** 탭에서 배포 환경이 사용 중인 클라우드 서비스 또는 웹앱의 이름으로 설정된 것을 확인할 수 있습니다.
     ...
     ![][27]
13. 기본값 이외의 다른 값을 원하는 경우 속성 값을 지정합니다. Azure 게시의 속성은 **배포** 섹션에 있으며 MSBuild 매개 변수를 설정해야 할 수 있습니다. 예를 들어 클라우드 서비스 프로젝트에서 "클라우드" 외의 서비스 구성을 지정하려면 MSbuild 매개 변수를 ServiceConfiguration.*YourProfile*.cscfg와 같은 이름의 서비스 구성 파일과 일치하는 `/p:TargetProfile=[YourProfile]`*[YourProfile]*에 설정합니다.
