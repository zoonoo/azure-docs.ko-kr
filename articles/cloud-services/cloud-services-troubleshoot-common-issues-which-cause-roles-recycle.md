...
배포 문제의 일반적인 원인 및 문제 해결 팁 일부를 이러한 문제를 해결하기 위해 제공됩니다. 응용 프로그램에 문제가 있다는 것을 역할 인스턴스가 시작에 실패하거나 **초기화**, **사용 중**, 및 **중지** 상태를 반복할 경우에 알 수 있습니다.
...
[OnStart], [OnStop] 및 [실행]을 포함하는 [RoleEntryPoint] 클래스의 메서드에서 throw되는 예외는 처리되지 않은 예외입니다. 이러한 메서드에서 처리되지 않은 예외가 발생하면 역할이 재활용됩니다. 역할이 반복적으로 재활용되면 시작하려 할 때마다 처리되지 않은 예외가 throw될 수 있습니다.
