...
예를 들어 .NET Framework 4.6의 릴리스와 함께 제공되지 않는 게스트 OS 패밀리 4에 .NET Framework 4.6.2를 설치할 수 있습니다. (게스트 OS 패밀리 5에는 .NET Framework 4.6이 함께 제공됩니다.) Azure 게스트 OS 릴리스에 대한 최신 정보는 [Azure 게스트 OS 릴리스 뉴스를](cloud-services-guestos-update-matrix.md)참조하십시오. 

>[!IMPORTANT]
>Azure SDK 2.9에는 게스트 OS 패밀리 4 이상에서 .NET Framework 4.6 배포에 대한 제한이 포함되어 있습니다. 제한에 대한 수정 사항은 [Microsoft Docs](https://github.com/MicrosoftDocs/azure-cloud-services-files/tree/master/Azure%20Targets%20SDK%202.9) 사이트에서 사용할 수 있습니다.
...
* [.NET 프레임 워크 4.8 웹 설치 프로그램](https://dotnet.microsoft.com/download/thank-you/net48)
* [.NET 프레임워크 4.7.2 웹 설치 프로그램](https://go.microsoft.com/fwlink/?LinkId=863262)
* [.NET 프레임워크 4.6.2 웹 설치 프로그램](https://www.microsoft.com/download/details.aspx?id=53345)

*웹* 역할에 대한 설치 관리자를 추가하려면
  1. **솔루션 탐색기** 내 클라우드 서비스 프로젝트의 **역할** 아래에서 *웹* 역할을 마우스 오른쪽 단추로 클릭하고 **추가** > **새 폴더**를 선택합니다. **bin이라는**폴더를 만듭니다.
  2. 빈 폴더를 마우스 오른쪽 단추로 클릭하고**기존 항목** **추가를** > 선택합니다. .NET 설치 관리자를 선택하고 bin 폴더에 추가합니다.
...
> [!NOTE]
> 응용 프로그램이 .NET Framework 4.6을 대상으로 하는 경우에도 클라우드 서비스 역할에 .NET Framework 4.6.2를 설치해야 합니다. 게스트 OS에는 기술 자료 [업데이트 3098779](https://support.microsoft.com/kb/3098779) 및 [업데이트 3097997](https://support.microsoft.com/kb/3097997)이 포함됩니다. .NET Framework 4.6이 기술 자료 업데이트 위에 설치된 경우 .NET 응용 프로그램을 실행할 때 문제가 발생할 수 있습니다. 이러한 문제를 방지하려면 버전 4.6이 아닌 .NET Framework 4.6.2를 설치합니다. 자세한 내용은 [기술 자료 문서 3118750](https://support.microsoft.com/kb/3118750) 및 [4340191](https://support.microsoft.com/kb/4340191)을 참조하세요.
> 
> 
...
시작 작업을 사용하여 역할이 시작되기 전에 작업을 수행할 수 있습니다. .NET Framework를 시작 작업의 일부로 설치하면 애플리케이션 코드가 실행되기 전에 프레임워크가 설치됩니다. 시작 작업에 대한 자세한 내용은 [Azure에서 시작 작업 실행](cloud-services-startup-tasks.md)을 참조하십시오. 
...
2. **install.cmd**라는 파일을 만들어 다음 설치 스크립트를 파일에 추가합니다.
   ...
   스크립트는 레지스트리를 쿼리하여 지정된 .NET Framework 버전이 컴퓨터에 이미 설치되어 있는지 확인합니다. .NET Framework 버전이 설치되지 않은 경우 .NET Framework 웹 설치 프로그램이 열립니다. 문제 해결을 돕기 위해 이 스크립트는 **InstallLogs** 로컬 스토리지에 저장된 startuptasklog-(현재 날짜 및 시간).txt 파일에 모든 작업을 기록합니다.
...
3. 이 항목의 앞에서 설명한 대로 **솔루션 탐색기에서** **기존 항목** **추가를** > 사용하여 각 역할에 install.cmd 파일을 추가합니다. 
...
* [.NET 프레임워크 설치 문제 해결][Troubleshooting .NET Framework Installations]
