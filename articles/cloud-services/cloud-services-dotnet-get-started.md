---
title: Azure 클라우드 서비스 및 ASP.NET 시작 | Microsoft Docs
...
3. *Web.Release.config* 변환 파일에서 `{connectionstring}`을(를) 삭제 후 그 자리에 Azure 클래식 포털에서 가져온 ADO.NET 연결 문자열을 붙여 넣습니다.
