...
“사용 안 함”으로 설정하고 시간이 지나면 게스트 OS 버전이 “만료”되고 만료된 버전을 실행하는 모든 설치는 보안 및 취약성 문제에 노출됩니다. 일반적으로 만료는 일괄 처리로 수행되므로 사용할 수 없는 상태에서 만료까지의 기간은 다를 수 있습니다.

게스트 OS를 수동으로 업데이트하도록 서비스를 구성한 고객은 해당 서비스가 지원되는 게스트 OS에서 실행되고 있는지 확인해야 합니다. 게스트 OS를 자동으로 업데이트하도록 서비스를 구성한 경우 기본 플랫폼은 준수를 보장하고 최신 게스트 OS로 업그레이드합니다.
