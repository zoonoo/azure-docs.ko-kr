...
수명이 60일 동안 버전은 *"비활성화"입니다.* "사용 안함"은 포털에서 해당 버전이 제거되었음을 의미합니다. 이 버전을 CSCFG 구성 파일에서 더 이상 설정할 수도 없습니다. 기존 배포는 계속 실행됩니다. 그렇지만 새로운 배포 및 기존 배포에 대한 코드 및 구성 업데이트는 허용되지 않습니다.

"사용 안 함"이 되면 게스트 OS 버전이 "만료"되고 만료된 버전이 여전히 실행 중인 설치는 보안 및 취약점 문제에 노출됩니다. 일반적으로 만료는 일괄 처리로 수행되므로 비활성화에서 만료까지의 기간은 다를 수 있습니다.

게스트 OS를 수동으로 업데이트하도록 서비스를 구성하는 고객은 서비스가 지원되는 게스트 OS에서 실행되고 있는지 확인해야 합니다. 서비스가 게스트 OS를 자동으로 업데이트하도록 구성된 경우 기본 플랫폼은 규정 준수를 보장하고 최신 게스트 OS로 업그레이드합니다.
