...
* **HPC Pack 2012 R2** - [다운로드](http://go.microsoft.com/fwlink/p/?linkid=328024) 하고 헤드 노드 컴퓨터나 네트워크 위치에 파일을 복사합니다. Windows Server 설치와 동일한 언어의 설치 파일을 선택합니다.
...
* Azure 저장소 계정 만들기
  ...
  > [!NOTE]
  > 또한 나중에 필요한 Azure 구독 ID를 기록해 둡니다. **설정** > **구독**을 클릭하여 클래식 포털에서 이 ID를 찾습니다.
  > 
  > 
...
3. 명령 모음에서 **업로드**를 클릭합니다.
    ...
    ![인증서 설정][upload_cert1]
4. 헤드 노드에서 C:\Program Files\Microsoft HPC Pack 2012\Bin\hpccert.cer 파일을 찾은 다음 그런 후 **선택** 단추를 클릭합니다.
...
8. **Specify Node Naming Series** 대화 상자에서 기본 이름 지정 시리즈를 적용하고 **OK**를 클릭합니다.
   ...
   > 
   > 
9.  **Deployment To-do List**에서 **Create a node template**을 참조하십시오. 노드 템플릿을 사용하여 클러스터에 Azure 노드를 추가합니다.
10. Create Node Template Wizard에서 다음을 수행합니다.
    ...
    c. **구독 정보 제공** 페이지에서 Azure 구독 ID(Azure 계정 정보에서 사용 가능)를 입력합니다. 그런 후 **관리 인증서**에서 **찾아보기**를 클릭하고 **기본 HPC Azure 관리**를 선택합니다. 그런 후 **Next**를 클릭합니다.
...
4. 역할 인스턴스가 실행되고 있는지 확인하려면 [클래식 포털](https://manage.windowsazure.com)에서 **클라우드 서비스** > *your_cloud_service_name* > **인스턴스**를 클릭합니다.
...
4. 역할 인스턴스가 Azure에서 더 이상 실행되지 않는지 확인하려면 [클래식 포털](https://manage.windowsazure.com)에서 **클라우드 서비스** > *your_cloud_service_name* > **인스턴스**를 클릭합니다. 프로덕션 환경에는 인스턴스가 배포되지 않습니다.
...
[개요]: ./media/cloud-services-setup-hybrid-hpcpack-cluster/hybrid_cluster_overview.png
