...
## Azure 구독 준비
[Azure 관리 포털](https://manage.windowsazure.com)을 사용하여 Azure 구독으로 다음 단계를 수행합니다. 이러한 단계는 나중에 온-프레미스 헤드 노드에서 Azure 노드를 배포하는 데 필요합니다.
...
1. 헤드 노드 컴퓨터에서 [Azure 포털](https://manage.windowsazure.com)에 로그인합니다.
...
10. Create Node Template Wizard에서 다음을 수행합니다.
	...
	c. **구독 정보 제공** 페이지에서 Azure <a href="[https://account.windowsazure.com/Subscriptions">계정 정보</a>에 제공된 Azure 구독 ID를 입력합니다. **Management certificate**에서 **Browse**를 클릭하고 **Default HPC Azure Management**를 선택합니다. 그런 후 **Next**를 클릭합니다.
...
3. **Specify New Nodes** 페이지에서 이전에 만든 Azure 노드 템플릿(기본적으로 **Default AzureNode Template**)을 선택합니다. 크기가 **Small**인 노드 **2**개를 지정하고 **Next**를 클릭합니다.
	...
	사용 가능한 가상 컴퓨터 크기에 대한 자세한 내용은 [Azure를 위한 가상 컴퓨터 및 클라우드 서비스 크기](https://msdn.microsoft.com/library/azure/dn197896.aspx)를 참조하십시오.

4. **Completing the Add Node Wizard** 페이지에서 **완료**를 클릭합니다.
...
4. 역할 인스턴스가 실행되고 있는지 확인하려면 [포털](https://manage.windowsazure.com)에서 **클라우드 서비스**를 클릭하고 클라우드 서비스의 이름을 클릭한 후 **인스턴스**를 클릭합니다.
...
4. 역할 인스턴스가 Azure에서 더 이상 실행되지 않는지 확인하려면 [클래식 포털](https://manage.windowsazure.com)에서 **클라우드 서비스**를 클릭하고 클라우드 서비스의 이름을 클릭한 후 **인스턴스**를 클릭합니다. 프로덕션 환경에는 인스턴스가 배포되지 않습니다.
