---
title: Azure 클라우드 서비스에서 시작 작업 실행 | Microsoft Docs
...
시작 작업은 [시작] 요소 내의 [작업] 요소를 사용하여 역할이 시작되고 [작업] 파일에서 정의되기 전에 수행되는 동작입니다. 시작 작업은 흔히 배치 파일이지만 PowerShell 스크립트를 시작하는 콘솔 응용 프로그램 또는 배치 파일일 수도 있습니다.
...
2. 모든 시작 작업은 해당 **taskType** 특성에 따라 실행됩니다.
   ...
   * **백그라운드** 및 **포그라운드** 작업은 비동기적으로 시작 작업에 병렬로 시작됩니다.  
     ...
     > [!WARNING]
     > IIS는 시작 프로세스에서 시작 작업 단계 중 완전하게 구성되지 않을 수도 있으므로 역할 지정 데이터를 사용할 수도 없습니다. 역할 지정 데이터가 필요한 시작 작업은 [Microsoft.WindowsAzure.ServiceRuntime.RoleEntryPoint.OnStart](https://msdn.microsoft.com/library/azure/microsoft.windowsazure.serviceruntime.roleentrypoint.onstart.aspx)를 사용해야 합니다.
     > 
     > 
...
시작 작업에 대한 환경 변수에는 [RoleEnvironment] 클래스의 멤버에 따라 동적 환경 변수 및 환경 변수 두 가지가 있습니다. 두 변수는 모두 [작업] 파일의 [환경] 섹션에 있으며 모두 [값] 요소 및 **이름** 특성을 사용합니다.
...
예를 들어 인스턴스가 계산 에뮬레이터에서 실행되는 경우 "**true**"이고 클라우드에서 실행되는 경우 "**false**"인 환경 변수를 만들려면 다음 [값] 및 [RoleInstanceValue] 요소를 사용합니다.
...
[작업]: cloud-services-model-and-package.md#csdef
