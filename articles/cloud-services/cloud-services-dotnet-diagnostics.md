...
2. 구독을 선택합니다.
3. Microsoft **Azure 게시 설정** 대화 상자에서 **새 만들기를 선택합니다.**
...
1. 다음 PowerShell 명령을 실행하여 공용 구성 파일 스키마 정의를 다운로드합니다.
    ...
    ```
2. **WorkerRole1** 프로젝트에 XML 파일을 추가합니다. **WorkerRole1** 프로젝트를 마우스 오른쪽 단추로 클릭하고 **추가** -> **새 항목…** -> **시각적 C # 항목** -> **데이터** -> **XML 파일**. 파일 이름을 “WadExample.xml”로 지정합니다.

   ![CloudServices_diag_add_xml](./media/cloud-services-dotnet-diagnostics/AddXmlFile.png)
3. WadConfig.xsd를 구성 파일과 연결합니다. WadExample.xml 편집기 창이 활성 창인지 확인합니다. **F4**를 눌러 **속성** 창을 엽니다. **속성** 창에서 **Schemas** 속성을 클릭합니다. 를 **클릭합니다.** **...** 를 클릭합니다. **추가를 클릭합니다...** 단추를 클릭하고 XSD 파일을 저장한 위치로 이동한 후 WadConfig.xsd 파일을 선택하고 **확인**을 클릭합니다.
...
Visual Studio **서버 탐색기**에서 wadexample 스토리지 계정으로 이동합니다. 클라우드 서비스가 5분 정도 실행된 후에는 **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** 및 **WADSetOtherTable** 테이블이 표시됩니다. 수집된 원격 분석을 보려는 테이블 중 하나를 두 번 클릭합니다.
