---
title: "클라우드 서비스에서 Azure 진단(.NET)을 사용하는 방법 | Microsoft Docs"
...
---
# <a name="enabling-azure-diagnostics-in-azure-cloud-services"></a>Azure 클라우드 서비스에서 Azure 진단 사용
...
이 문서에서는 Azure 구독이 있으며 Visual Studio 2013과 Azure SDK를 함께 사용 중인 것으로 가정합니다. Azure 구독이 없는 경우 [무료 평가판][무료 평가판]에 등록할 수 있습니다. [Azure PowerShell 버전 0.8.7 이상을 설치 및 구성][Azure PowerShell 버전 0.8.7 이상을 설치 및 구성]해야 합니다.
...
WorkerRole.cs 내용을 다음 코드로 바꿉니다. [EventSource 클래스][EventSource 클래스]로부터 상속되는 SampleEventSourceWriter 클래스는 4개의 로깅 메서드 **SendEnums**, **MessageMethod**, **SetOther** 및 **HighFreq**를 구현합니다. **WriteEvent** 메서드에 대한 첫 번째 매개 변수는 각 이벤트의 ID를 저장합니다. Run 메서드는 **SampleEventSourceWriter** 클래스에 구현된 각각의 로깅 메서드를 10초마다 호출하는 무한 루프를 구현합니다.
...
Visual Studio **서버 탐색기** 에서 wadexample 저장소 계정으로 이동합니다. 클라우드 서비스가 5분 정도 실행된 후에는 **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** 및 **WADSetOtherTable** 테이블이 표시됩니다. 수집된 원격 분석을 보려는 테이블 중 하나를 더블 클릭합니다.
