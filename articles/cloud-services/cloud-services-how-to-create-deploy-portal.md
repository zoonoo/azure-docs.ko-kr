...
7. **패키지**를 클릭합니다. 그러면 **패키지 업로드** 블레이드가 열립니다. 필수 필드를 입력합니다. 역할에 단일 인스턴스가 포함된 경우 **단일 인스턴스가 포함된 역할이 하나 이상 있는 경우에도 배포합니다.** 가 선택되어 있어야 합니다.
    ...
    > [!IMPORTANT]
    > 클라우드 서비스는 [클래식 저장소 계정](../resource-manager-deployment-model.md)에만 연결될 수 있습니다. 구독 및 위치에 대한 저장소 계정을 찾을 수 없다는 메시지가 표시되면 해당 위치에 클라우드 서비스에 대한 클래식 저장소 계정을 만들었는지 확인합니다.
