...
그러나 **contoso.com**와 같은 자신의 도메인 이름에 응용 프로그램을 노출할 수도 있습니다. 이 문서에서는 클라우드 서비스 웹 역할에 대해 사용자 지정 도메인 이름을 예약 또는 구성하는 방법에 대해 설명합니다.
...
CNAME 레코드는 **contoso.com** 또는 **\.www contoso.com**와 같은 *특정* 도메인을 표준 도메인 이름에 매핑합니다. 이 경우 정식 도메인 이름은 Azure 호스티드 애플리케이션의 **[myapp].cloudapp.net** 도메인 이름입니다. CNAME을 만들면 **[myapp].cloudapp.net**에 대한 별칭이 만들어집니다. CNAME 항목은 자동으로 **[myapp].cloudapp.net** 서비스의 IP 주소로 확인되기 때문에 클라우드 서비스의 IP 주소가 변경될 경우 아무 조치도 수행할 필요가 없습니다.

> [!NOTE]
> 일부 도메인 등록 기관은 www\.contoso.com 같은 CNAME 레코드를 사용할 때만 하위 도메인을 매핑할 수 있으며 contoso.com 같은 루트 이름은 매핑하지 않습니다. CNAME 레코드에 대한 자세한 내용은 등록 기관에서 제공하는 설명서인 [CNAME 레코드에 대한 Wikipedia 항목](https://en.wikipedia.org/wiki/CNAME_record) 또는 [IETF 도메인 이름 - 구현 및 사양](https://tools.ietf.org/html/rfc1035) 문서를 참조하세요.

### <a name="a-record"></a>A 레코드
*A* 레코드는 **contoso.com** 또는 **www\.contoso.com**같은 도메인 또는 ** \*.contoso.com**와 같은 *와일드카드 도메인을* IP 주소에 매핑합니다. Azure 클라우드 서비스의 경우 서비스의 가상 IP입니다. 따라서 CNAME 레코드를 통해 A 레코드의 주요 이점은 mail.contoso.com, **login.contoso.com**또는 **www\.contso.com**같은 여러 하위 도메인에 대한 요청을 **mail.contoso.com**처리하는 \* **.contoso.com**와 같은 와일드카드를 사용하는 하나의 항목을 가질 수 있다는 것입니다.

> [!NOTE]
> A 레코드는 고정 IP 주소에 매핑되므로 변경 내용을 클라우드 서비스의 IP 주소로 자동으로 확인할 수 없습니다. 클라우드 서비스에서 사용하는 IP 주소는 빈 슬롯(프로덕션 또는 스테이징)에 처음 배포할 때 할당됩니다. 슬롯에 대한 배포를 삭제하면 Azure에서 IP 주소가 해제되고 슬롯에 대한 향후 배포에 새 IP 주소가 제공될 수 있습니다.
> 
...
1. 이러한 방법 중 하나를 사용하여 클라우드 서비스에 할당된 **.cloudapp.net** 도메인 이름을 찾습니다.
   ...
   * [Azure 포털에]로그인하고 클라우드 서비스를 선택하고 **개요** 섹션을 보고 사이트 **URL** 항목을 찾습니다.
...
4. 또한 **\.www**customdomain.com 대한 별칭을 만들려는 경우 **www와** 같이 CNAME에 대한 도메인 또는 하위 도메인 별칭을 제공해야 합니다. 루트 도메인에 대한 별칭을 만들려면 등록기관의 DNS**\@** 도구에 '기호'로 나열될 수 있습니다.
5. 정식 호스트 이름을 제공해야 합니다. 이 경우에는 애플리케이션의 **cloudapp.net** 도메인입니다.

예를 들어 다음 CNAME 레코드는 **www\.contoso.com 모든** 트래픽을 배포된 응용 프로그램의 사용자 지정 도메인 **이름인 contoso.cloudapp.net**전달합니다.
...
> [!NOTE]
> **www\.contoso.com** 방문자는 실제 호스트(contoso.cloudapp.net)를 볼 수 없으므로 최종 사용자에게 전달 프로세스가 표시되지 않습니다.
> 
...
1. 다음 방법 중 하나를 사용하여 클라우드 서비스의 IP 주소를 가져옵니다.
   ...
   * [Azure 포털에]로그인하고 클라우드 서비스를 선택하고 **개요** 섹션을 보고 공용 IP 주소 항목을 **찾습니다.**
...
4. 이 A 레코드를 사용할 도메인 또는 하위 도메인을 선택하거나 입력합니다. 예를 들어 **www customdomain.com** 대한 별칭을 만들려면 **www를\.** 선택합니다. 모든 하위 도메인에 대한 와일드카드 항목을 만들려면 '*****'를 입력합니다. 여기에는 **mail.customdomain.com,** **login.customdomain.com**및 **www\.customdomain.com**같은 모든 하위 도메인이 포함됩니다.

    루트 도메인에 대한 A 레코드를 만들려면 등록기관의 DNS**\@** 도구에 '기호'로 나열될 수 있습니다.
...
* [클라우드 서비스 관리 방법](cloud-services-how-to-manage-portal.md)
