...
CNAME 레코드를 매핑하는 *특정* 도메인와 같은 **contoso.com** 또는 **www\.contoso.com**, 정식 도메인 이름에. 이 경우 정식 도메인 이름은 Azure 호스티드 애플리케이션의 **[myapp].cloudapp.net** 도메인 이름입니다. CNAME을 만들면 **[myapp].cloudapp.net**에 대한 별칭이 만들어집니다. CNAME 항목은 자동으로 **[myapp].cloudapp.net** 서비스의 IP 주소로 확인되기 때문에 클라우드 서비스의 IP 주소가 변경될 경우 아무 조치도 수행할 필요가 없습니다.

> [!NOTE]
> 일부 도메인 등록 기관 으로만 www 같은 CNAME 레코드를 사용 하는 경우 하위 도메인을 매핑할 수 있도록\.contoso.com 및 루트 이름이 아니라, 예: contoso.com입니다. CNAME 레코드에 대한 자세한 내용은 등록 기관에서 제공하는 설명서인 [CNAME 레코드에 대한 Wikipedia 항목](https://en.wikipedia.org/wiki/CNAME_record) 또는 [IETF 도메인 이름 - 구현 및 사양](https://tools.ietf.org/html/rfc1035) 문서를 참조하세요.

### <a name="a-record"></a>A 레코드
*A* 레코드와 같은 도메인을 매핑합니다 **contoso.com** 또는 **www\.contoso.com**를 *또는 와일드 카드 도메인* 등 **\*. contoso.com**, IP 주소입니다. Azure 클라우드 서비스의 경우 서비스의 가상 IP입니다. 와 같은 와일드 카드를 사용 하는 항목이 하나는 수 이므로 CNAME 레코드는 레코드의 주요 장점은 \* **. contoso.com**와 같은 여러 하위 도메인에 대 한 요청을 처리  **mail.contoso.com**하십시오 **login.contoso.com**, 또는 **www\.contso.com**합니다.
...
1. 이러한 방법 중 하나를 사용하여 클라우드 서비스에 할당된 **.cloudapp.net** 도메인 이름을 찾습니다.
   ...
   * [Azure Portal]에 로그인하여 클라우드 서비스를 선택하고 **Essentials** 섹션에서 **사이트 URL** 항목을 찾습니다.
...
> [!NOTE]
> 방문자 **www\.contoso.com** 전달 프로세스를 최종 사용자에 게 표시 되지 않도록 호스트 (contoso.cloudapp.net)가 표시 되지 것입니다.
> 
...
1. 다음 방법 중 하나를 사용하여 클라우드 서비스의 IP 주소를 가져옵니다.
   ...
   * [Azure Portal]에 로그인하여 클라우드 서비스를 선택하고 **Essentials** 섹션에서 **공용 IP 주소** 항목을 찾습니다.
