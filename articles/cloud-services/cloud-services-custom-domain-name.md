...
> [AZURE.NOTE]위 예제는 <strong>www</strong> 하위 도메인의 트래픽에만 적용됩니다. CNAME 레코드와 함께 와일드카드를 사용할 수 없으므로 각 도메인/하위 도메인에 대해 하나의 CNAME을 만들어야 합니다. *.contoso.com 등의 하위 도메인에서 cloudapp.net 주소로 트래픽을 보내려는 경우 DNS 설정에서 <strong>URL 리디렉션</strong> 또는 <strong>URL 전달</strong> 항목을 구성하거나 A 레코드를 만들 수 있습니다.
