...
예측 분석 실험은 핵심적으로 *모델 만들기*, *모델 학습* 및 *모델 점수 매기기 및 테스트*를 위한 구성 요소로 구성됩니다. 이 요소를 결합하여 데이터를 가져오고 이 데이터에 대해 모델을 학습시키고 새 데이터에 모델을 적용하는 실험을 만들 수 있습니다. 데이터 전처리 모듈을 추가하고, 특성을 선택하고, 훈련 및 테스트 집합으로 데이터를 분리하고, 모델 품질을 평가하거나 교차 검사할 수도 있습니다.
...
-   모델 만들기
    ...
    -   [1단계: 데이터 가져오기][1단계: 데이터 가져오기]
    -   [2단계: 데이터 전처리][2단계: 데이터 전처리]
    -   [3단계: 특성 정의][3단계: 특성 정의]
-   모델 훈련

    -   [4단계: 학습 알고리즘 선택 및 적용][4단계: 학습 알고리즘 선택 및 적용]
-   모델 점수 매기기 및 테스트

    -   [5단계: 새 데이터에 따른 예측][5단계: 새 데이터에 따른 예측]
...
1.  기계 학습 스튜디오 창의 아래에 있는 **+새로 만들기**를 클릭하고 **실험**을 선택하여 새 실험을 시작합니다. 실험 이름을 "제목 없음"에서 "자동차 가격 예측"과 같은 의미 있는 이름으로 바꿉니다.
...
데이터 집합에서 특성 하위 집합을 사용하는 모델을 빌드하겠습니다. 다시 돌아와서 다양한 특성을 선택하고 실험을 다시 실행하여 더 나은 결과가 나오는지 확인할 수 있습니다. 첫 번째 추측으로서, **프로젝트 열** 모듈을 사용하여 다음 특성(열)을 선택합니다. 모델 훈련을 위해서는 예측하려는 *가격* 값을 포함해야 합니다.
...
데이터가 준비되었으며, 예측 모델 생성은 훈련과 테스트로 구성됩니다. *분류*와 *회귀*가 관리 기계 학습 기술의 두 형식입니다. 분류는 색(빨강, 파랑 또는 녹색)과 같이 정의된 값 집합에서 예측하는 데 사용됩니다. 회귀는 사람의 나이와 같이 연속된 값 집합에서 예측하는 데 사용됩니다.

임의의 값일 수 있는 자동차 가격을 예측하려고 하므로 회귀 모델을 사용하겠습니다. 이 예제에서는 간단한 *선형 회귀* 모델을 훈련하고 다음 단계에서 이를 테스트합니다.

1.  훈련 및 테스트 집합으로 데이터를 분리합니다. **분할** 모듈을 선택하여 실험 캔버스로 끌어 놓고 마지막 **프로젝트 열** 모듈의 출력과 연결합니다. **첫 번째 출력 데이터 집합의 행 분수**를 0.75로 설정합니다. 그러면 데이터의 75%를 모델 훈련에 사용하고 25%는 테스트용으로 보유합니다.
...
4.  **모델 학습** 모듈을 찾아 실험으로 끌어 놓습니다. **열 선택기 시작**을 클릭하고 *price* 열을 선택합니다. 모델이 예측할 값입니다.

    !["price" 열 선택]["price" 열 선택]

5.  왼쪽 입력 포트를 **선형 회귀** 모듈의 출력에 연결하고 오른쪽 출력 포트를 **분할** 모듈의 훈련 데이터 출력(왼쪽 포트)에 연결합니다.

6.  실험을 실행합니다.

결과는 새 샘플의 점수를 매기고 예측하는 데 사용할 수 있는 훈련된 회귀 모델입니다.

![학습 알고리즘 적용][학습 알고리즘 적용]

### 5단계: 새 데이터에 따른 예측

모델을 훈련했으며, 이제 모델을 사용하여 나머지 25% 데이터의 점수를 매기고 모델 기능이 얼마나 좋은지 확인할 수 있습니다.

1.  **모델 점수 매기기** 모듈을 찾아 실험 캔버스로 끌어 놓고 왼쪽 입력 포트를 **모델 훈련** 모듈의 출력에 연결하고 오른쪽 입력 포트를 **분할** 모듈의 테스트 데이터 출력(오른쪽 포트)에 연결합니다.

    ![모델 점수 매기기 모듈][모델 점수 매기기 모듈]
...
예측 모델 만들기, 훈련, 점수 매기기 및 게시에 대한 더 자세한 포괄적인 연습을 보려면 [연습: Azure Machine Learning을 사용한 예측 솔루션 개발][연습: Azure Machine Learning을 사용한 예측 솔루션 개발](영문)을 참조하세요.

<!-- Images -->

  [1단계: 데이터 가져오기]: #step-1-get-data
  [2단계: 데이터 전처리]: #step-2-pre-process-data
  [3단계: 특성 정의]: #step-3-define-features
  [4단계: 학습 알고리즘 선택 및 적용]: #step-4-choose-and-apply-a-learning-algorithm
  [5단계: 새 데이터에 따른 예측]: #step-5-predict-over-new-data
  [팔레트 검색]: ./media/machine-learning-create-experiment/screen1a.png
  [데이터 집합]: ./media/machine-learning-create-experiment/screen1.png
  [데이터 집합 시각화]: ./media/machine-learning-create-experiment/screen1b.png
  [열 선택]: ./media/machine-learning-create-experiment/screen3.png
  [프로젝트 열 속성]: ./media/machine-learning-create-experiment/screen4.png
  [누락 값 스크러버 속성]: ./media/machine-learning-create-experiment/screen4a.png
  [첫 번째 실험 실행]: ./media/machine-learning-create-experiment/screen5.png
  [1]: ./media/machine-learning-create-experiment/screen6.png
  ["price" 열 선택]: ./media/machine-learning-create-experiment/screen7.png
  [학습 알고리즘 적용]: ./media/machine-learning-create-experiment/screen8.png
