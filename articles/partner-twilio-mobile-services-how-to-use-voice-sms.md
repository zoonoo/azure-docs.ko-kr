<properties pageTitle="음성 및 SMS 기능을 위해 Twilio 사용 | 모바일 개발자 센터" metaKeywords="" description="Learn how to perform common tasks using the Twilio API with Azure Mobile Services." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="How to use Twilio for voice and SMS capabilities from Mobile Services" authors="MicrosoftHelp@twilio.com" solutions="" manager="twilio" editor="" />
...
3. **dependencies** 개체에 Twilio 패키지 참조를 추가하면 package.json 파일은 다음과 같이 표시됩니다.
...
4. 이 파일 업데이트를 커밋하고 업데이트를 다시 모바일 서비스로 푸시합니다.
	...
	이와 같이 package.json 파일을 업데이트하면 모바일 서비스가 다시 시작됩니다.
	
이제 모바일 서비스에서 Twilio 패키지를 설치하고 로드하므로 사용자 지정 API 및 테이블 스크립트에서 Twilio 라이브러리를 참조하고 사용할 수 있습니다.

<h2><a id="howto_make_call"></a>방법: 발신 전화 걸기</h2>
다음 스크립트는 **makeCall** 함수를 사용하여 모바일 서비스에서 발신 전화를 시작하는 방법을 보여 줍니다. 또한 이 코드는 Twilio 제공 사이트를 사용하여 TwiML(Twilio Markup Language) 응답을 반환합니다. **From** 및 **To** phone 전화 번호의 값을 바꾸고, 코드를 실행하기 전에 Twilio 계정의 **From** 전화 번호를 확인하세요.
