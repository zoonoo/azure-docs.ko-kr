...
>[AZURE.NOTE]OneDrive(소비자/개인 버전용)는 Azure RemoteApp에서 지원되지 않습니다. 비즈니스용 OneDrive의 버전이 모두 Windows Server에서 작동하도록 인증된 것은 아니기 때문에 일부만 지원됩니다. [https://support.microsoft.com/en-us/kb/2965687](https://support.microsoft.com/kb/2965687)에 설명한 대로 새 클라이언트(차세대 동기화 클라이언트) 및 오래된 Groove 버전이 Azure RemoteApp에서 잘 작동하는 것처럼 보일 수 있지만 오래된 동기화 엔진은 Citrix/터미널 서버(Windows Server)에서 전체 기능을 발휘하지 못합니다. Azure RemoteApp(및 다른 Windows Server 배포)에서 새 동기화 클라이언트를 사용합니다.

## 비즈니스용 OneDrive에 대한 다른 설정 옵션은 무엇입니까?

- **비즈니스 동기화 엔진에 대한 OneDrive의 기존 설치:** 비즈니스용 OneDrive 동기화 클라이언트는 Windows 클라이언트 SKU에서처럼 서버 SKU(RemoteApp 세션/터미널 서버 세션 뿐만 아니라 원격 데스크톱) 및 RemoteApp 세션에서 동기화를 위해 선택한 폴더에 설치될 수 있습니다. 비즈니스용 OneDrive 동기화 파일이 있는 기본 위치는 Azure RemoteApp에서 사용자 데이터 및 설정을 저장하는 데 사용된 사용자 프로필 디스크가 있는 동일한 위치인 C:\\users<username>입니다. 이 디스크는 로그인할 모든 VM에 따르고 따라서 ODB 파일은 사용자를 따릅니다. 비즈니스용 OneDrive 앱은 모든 관리자에 의해 사용자에 대해 게시되어야 하고 사용자는 모든 새 세션에서 시작하여(또는 시작이 로그인 스크립트와 함께 자동화될 수 있습니다) 동기화 엔진이 활성화되도록 해야 합니다. 비즈니스용 OneDrive는 세션이 실행되고 있는 VM에 전체 파일을 다운로드합니다. 사용자의 콘텐츠를 동기화할 경우 부하가 높은 워크로드는 각 컴퓨터에 로그인하는 사용자 수가 많은 터미널 기계에 대해 단순히 최적화되지 않은 모든 항목(가져온 CPU/데이터 전송/저장소)을 동기화합니다. 선택적 동기화 작업으로 워크로드는 줄일 수는 있지만 문제는 여전히 남아 있습니다.
