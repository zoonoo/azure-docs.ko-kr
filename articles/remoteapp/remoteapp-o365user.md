...
* 2단계: 이 단계가 매우 중요합니다. Microsoft 계정(예: @outlook.com)을 구독에서 서비스 관리자로 사용해야 합니다. 구독에 연결된 기존 Azure AD에서는 사용자 계정을 사용할 수 없기 때문입니다. 이 계정을 사용할 경우 다른 Azure AD로 이동할 수 없습니다.
* 4단계: 기존 디렉터리를 추가할 때 해당 디렉터리에 대한 관리자 계정으로 로그인하라는 메시지가 표시됩니다. 단계 1의 관리자 계정을 사용해야 합니다.
* 5단계: 구독의 부모 디렉터리를 Office 365 디렉터리로 변경합니다. 그러면 설정 -> 구독 아래에 Office 365 디렉터리가 나열됩니다. ![구독의 부모 디렉터리 변경](./media/remoteapp-o365user/settings.png)
