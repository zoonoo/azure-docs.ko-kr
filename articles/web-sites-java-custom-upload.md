<properties 
	pageTitle="Azure에 사용자 지정 Java 웹 사이트 업로드" 
	description="이 자습서에서는 Azure에 사용자 지정 Java 웹 사이트를 업로드하는 방법을 보여 줍니다." 
	services="web-sites" 
...
- 쓰기 권한이 있는 디렉터리가 필요한 응용 프로그램은 Azure 웹 사이트의 콘텐츠 디렉터리(**D:\home**)에 있어야 합니다.  환경 변수  `HOME`은 D:\home을 참조합니다.  
...
**stdoutLogEnabled**(기본값 = "true"). true인 경우 **processPath** 설정에 지정된 프로세스의 **stdout** 및 **stderr**이 **stdoutLogFile**(**stdoutLogFile** 섹션 참조)에 지정된 파일로 리디렉션됩니다.
                                    
**stdoutLogFile**(기본값 = "d:\home\LogFiles\httpPlatformStdout.log"). **processPath**에 지정된 프로세스의 **stdout** 및 **stderr**이 로깅될 절대 파일 경로입니다.
...
Liferay가 Azure 웹 사이트에서 지원됩니다. Liferay에 상당한 메모리가 필요할 수 있으므로 충분한 메모리를 제공할 수 있는 중대형 전용 작업자에서 사이트를 실행해야 합니다. 또한 Liferay는 시작하는 데 몇 분 정도 걸릴 수 있습니다. 이런 이유로 사이트를 **무중단**으로 설정하는 것이 좋습니다.  
