---
title: 오픈 허브를 통해 SAP 비즈니스 웨어하우스에서 데이터 복사
...
Open Hub 커넥터를 통한 이 SAP 비즈니스 웨어하우스는 다음 활동에 대해 지원됩니다.
...
- SAP 비즈니스 웨어하우스 **버전 7.01 이상(2015년 이후에 릴리스된 최근 SAP 지원 패키지 스택에서)**
...
SAP BW OHD(Open Hub 대상)에 따라 SAP 데이터가 릴레이되는 대상이 정의됩니다. SAP 데이터 전송 프로세스(DTP)에서 지원하는 모든 개체는 개방형 허브 데이터 원본(예: DSO, InfoCube, DataSource 등)으로 사용할 수 있습니다. 릴레이된 데이터가 저장되는 Open Hub 대상 유형은 데이터베이스 테이블(로컬 또는 원격) 및 플랫 파일일 수 있습니다. 이 SAP BW Open Hub 커넥터를 사용하면 BW에서 OHD 로컬 테이블의 데이터를 복사할 수 있습니다. 다른 형식을 사용하는 경우에는 다른 커넥터를 통해 데이터베이스 또는 파일 시스템에 직접 연결할 수 있습니다.

![SAP BW Open Hub](./media/connector-sap-business-warehouse-open-hub/sap-bw-open-hub.png)

## <a name="delta-extraction-flow"></a>델타 추출 흐름

ADF SAP BW 오픈 허브 커넥터는 `excludeLastRequest` `baseRequestId` 두 가지 선택적 속성을 제공하며 오픈 허브에서 델타 부하를 처리하는 데 사용할 수 있습니다. 

- **excludeLastRequestId**: 마지막 요청의 레코드를 제외할지 여부입니다. 기본값은 true입니다. 
- **baseRequestId**: 델타 로딩요청 ID입니다. 설정하는 경우 requestId가 이 속성의 값보다 큰 데이터만 검색됩니다. 

전반적으로 SAP 정보 공급자에서 ADF(Azure 데이터 팩터리)로의 추출은 2단계로 구성됩니다. 

1. **SAP BW 데이터 전송 프로세스(DTP)** 이 단계는 SAP BW 정보 공급자에서 SAP BW 열기 허브 테이블로 데이터를 복사합니다. 
...
첫 번째 단계에서는 DTP가 실행됩니다. 각 실행은 새 SAP 요청 ID를 만듭니다. 요청 ID는 Open Hub 테이블에 저장되고 ADF 커넥터에서 델타를 식별하는 데 사용됩니다. 두 단계는 비동기적으로 실행됩니다: DTP는 SAP에 의해 트리거되고 ADF 데이터 복사본은 ADF를 통해 트리거됩니다. 

기본적으로 ADF는 Open Hub 테이블에서 최신 델타를 읽지 않습니다("마지막 요청 제외" 옵션은 true). 따라서 ADF의 데이터는 Open Hub 테이블의 데이터와 함께 100% 최신이 아닙니다(마지막 델타가 누락되었습니다). 이 절차는 비동기 추출로 인해 발생하는 행이 손실되지 않도록 합니다. DTP가 여전히 동일한 테이블에 쓰는 동안 ADF가 Open Hub 테이블을 읽는 경우에도 잘 작동합니다. 

일반적으로 ADF가 마지막으로 실행한 최대 복사 요청 ID를 스테이징 데이터 저장소(예: 위 다이어그램의 Azure Blob)에 저장합니다. 따라서 동일한 요청은 후속 실행에서 ADF에 의해 두 번째로 읽혀지지 않습니다. 한편, 데이터는 오픈 허브 테이블에서 자동으로 삭제되지 않습니다.

적절한 델타 처리를 위해 동일한 Open Hub 테이블에서 다른 DTP의 요청 IP를 가질 수 없습니다. 따라서 각 OHD(열린 허브 대상)에 대해 둘 이상의 DTP를 생성해서는 안 됩니다. 동일한 InfoProvider에서 전체 및 델타 추출이 필요한 경우 동일한 InfoProvider에 대해 두 개의 OHD를 만들어야 합니다. 

## <a name="prerequisites"></a>사전 요구 사항

이 SAP Business Warehouse 커넥터를 사용하려면 다음 작업을 수행해야 합니다.

- 버전 3.13 이상의 자체 호스팅 통합 런타임을 설정합니다. 자세한 내용은 [자체 호스팅 통합 런타임](create-self-hosted-integration-runtime.md) 문서를 참조하십시오.
...
> [!TIP]
>
> SAP BW 오픈 허브 커넥터 사용 연습에서는 [Azure 데이터 팩터리(Azure Data Factory)를 사용하여 BW(SAP 비즈니스 웨어하우스)의 로드 데이터를](load-sap-bw-data.md)참조하십시오.
...
데이터 세트 정의에 사용할 수 있는 섹션 및 속성의 전체 목록은 [데이터 세트](concepts-datasets-linked-services.md) 문서를 참조하세요. 이 섹션에서는 SAP BW Open Hub 데이터 집합에서 지원하는 속성 목록을 제공합니다.
...
**증상:** HANA에서 SAP BW를 실행하고 ADF 복사 활동(1백만 행)을 사용하여 데이터의 하위 집합만 복사되는 것을 관찰하는 경우 가능한 원인은 DTP에서 "SAP HANA 실행" 옵션을 사용하도록 설정하는 것이며, 이 경우 ADF는 첫 번째 데이터 배치만 검색할 수 있습니다.

**해결 방법:** DTP에서 "SAP HANA 실행" 옵션을 사용하지 않도록 설정한 다음 데이터를 다시 처리한 다음 복사 작업을 다시 실행해 봅니다.
