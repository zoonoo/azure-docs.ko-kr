...
| [Exists](data-flow-exists.md) | 다중 입력/출력 | 데이터가 다른 소스 또는 스트림에 있는지 확인합니다. | 
| [필터](data-flow-filter.md) | 행 수정자 | 조건에 따라 행을 필터링합니다. |
| [평면화(flatten)](data-flow-flatten.md) | 스키마 수정자 |  JSON과 같은 계층 구조 내에서 배열 값을 가져 와서 개별 행으로 풀어줍니다. |
...
| [sink](data-flow-sink.md) | - | 데이터의 최종 목적지 |
