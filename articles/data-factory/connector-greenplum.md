...
이 Greenplum 커넥터는 다음 활동에 대해 지원됩니다.
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | type 속성은 **Greenplum**으로 설정해야 합니다. | yes |
| connectionString | Greenplum에 연결할 ODBC 연결 문자열입니다. <br/>Azure Key Vault에 암호를 넣고, 연결 문자열에서 `pwd` 구성을 끌어올 수도 있습니다. 자세한 내용은 다음 샘플 및 [Azure Key Vault에 자격 증명 저장](store-credentials-in-key-vault.md) 문서를 참조하세요. | yes |
...
데이터 집합을 정의하는 데 사용할 수 있는 섹션 및 속성의 전체 목록은 [데이터 집합](concepts-datasets-linked-services.md) 문서를 참조하세요. 이 섹션에서는 Greenplum 데이터 세트에서 지원하는 속성의 목록을 제공합니다.
...
Greenplum에서 데이터를 복사하려면 복사 작업의 원본 형식을 **GreenplumSource**로 설정합니다. 다음 속성은 복사 활동 **소스** 섹션에서 지원됩니다.

| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 원본의 type 속성은 **GreenplumSource**로 설정해야 합니다. | yes |
