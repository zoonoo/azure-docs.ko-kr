...
데이터 집합은 테이블, 파일, 폴더, 문서를 비롯한 다양한 데이터 저장소 내의 데이터를 식별합니다. 데이터 집합을 만든 후 파이프라인의 작업에 사용할 수 있습니다. 예를 들어 데이터 집합은 복사 작업 또는 HDInsightHive 작업의 입력/출력 데이터 집합일 수 있습니다. Azure 포털에서는 모든 파이프라인 및 데이터 입력 및 출력의 시각적 레이아웃을 제공합니다. 데이터가 송신되고 전송되는 위치를 항상 알 수 있도록 모든 원본에서 데이터 파이프라인의 모든 관계 및 종속성을 한 눈에 확인할 수 있습니다.
