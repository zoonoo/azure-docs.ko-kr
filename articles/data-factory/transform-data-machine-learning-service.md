...
속성 | 설명 | 허용되는 값 | 필수
-------- | ----------- | -------------- | --------
name | 파이프라인의 작업 이름입니다. | String | yes
type | 활동 유형은 'AzureML실행 파이프라인'입니다. | String | yes
linkedServiceName | Azure 기계 학습에 연결된 서비스 | 연결된 서비스 참조 | yes
ml파이프라인Id | 게시된 Azure 기계 학습 파이프라인의 ID | 문자열(또는 resultType 문자열이 있는 식) | yes
