...
| password |사용자 계정의 password입니다. 이 필드를 SecureString으로 표시합니다. |예 |
...
데이터 집합 정의에 사용할 수 있는 섹션 및 속성의 전체 목록은 데이터 집합 문서를 참조하세요. 이 섹션에서는 Amazon Redshift 데이터 집합에서 지원하는 속성의 목록을 제공합니다.

Amazon Redshift에서 데이터를 복사하려면 데이터 집합의 type 속성을 **RelationalTable**로 설정합니다. 다음과 같은 속성이 지원됩니다.
...
| s3LinkedServiceName | "AmazonS3" 형식의 ADF 연결된 서비스 이름을 지정하여 중간 저장소로 사용하려는 Amazon S3을 참조합니다. | 예(UNLOAD를 사용하는 경우) |
...
| Amazon Redshift 데이터 형식 | 데이터 팩터리 중간 데이터 형식 |
|:--- |:--- |
| BIGINT |Int64 |
| BOOLEAN |문자열 |
| CHAR |문자열 |
