...
- **LogLevel**:  미리 정의 된 **null**@no__t에서 선택한 로깅 범위를 입력 합니다.-1**기본**/**자세한**/**성능** 옵션 Azure-SSIS IR에서 패키지를 실행 합니다.
...
- **PipelineNameHashStrLen**: **Azuredtexec**를 호출할 때 제공 하는 옵션 값에서 생성할 해시 문자열의 길이를 입력 합니다.  문자열은 Azure-SSIS IR에서 패키지를 실행 하는 ADF 파이프라인의 고유 이름을 구성 하는 데 사용 됩니다.  일반적으로 32 문자 길이가 충분 합니다.
...
명령줄 프롬프트에서 **Azuredtexec** 를 호출 하 고 사용 사례 시나리오에서 특정 옵션에 대 한 관련 값을 제공할 수 있습니다.
...
- **/Conn [방법]** : 패키지의 기존 연결 관리자에 대 한 연결 문자열을 지정 합니다.  이 옵션을 사용 하면 디자인 타임에 지정 된 연결 관리자와 다른 패키지의 기존 연결 관리자에 대 한 런타임 연결 문자열을 설정할 수 있습니다.  이 옵션의 값으로 다음과 같이 지정할 수 있습니다. `connection_manager_name_or_id;connection_string [[;connection_manager_name_or_id;connection_string]...]`.

- **/Set**: 패키지에서 매개 변수, 변수, 속성, 컨테이너, 로그 공급자, Foreach 열거자 또는 연결의 구성을 재정의 합니다.  이 옵션은 여러 번 지정할 수 있습니다.  이 옵션에 대 한 값으로 다음과 같이 지정할 수 있습니다. 예를 들어 `\package.variables[counter].Value;1` @no__t `counter` 변수의 값을 1로 재정의 합니다.  패키지 구성 마법사를 사용 하 여 값을 찾고 복사 하 여 값을 붙여 넣을 수 있습니다 .이 값 @no__t은 패키지에서 재정의 하려는 항목에 대 한 자세한 내용은 [패키지 구성 마법사](https://docs.microsoft.com/sql/integration-services/package-configuration-wizard-ui-reference?view=sql-server-2014) 설명서를 참조 하세요.

- **/De [** 다음으로]: **EncryptAllWithPassword**/**EncryptSensitiveWithPassword** protection 수준으로 구성 된 패키지의 암호 해독 암호를 설정 합니다.

> [!NOTE]
> 해당 옵션에 대 한 새 값을 사용 하 여 **Azuredtexec** 를 호출 하면 옵션 **/de [c)]** 를 제외 하 고 새 파이프라인이 생성 됩니다.

## <a name="next-steps"></a>다음 단계

SSIS 패키지 실행 작업을 사용 하는 고유 파이프라인은 **Azuredtexec**를 호출한 후 생성 되 고 실행 됩니다. 이러한 파이프라인은 ADF 포털에서 모니터링할 수 있습니다. 자세한 정보는 [ADF 작업으로 SSIS 패키지 실행](https://docs.microsoft.com/azure/data-factory/how-to-invoke-ssis-package-ssis-activity) 문서를 참조 하세요.

> [!WARNING]
> 생성 된 파이프라인은 **Azuredtexec**에서만 사용 해야 합니다. 해당 속성/매개 변수는 나중에 변경 될 수 있으므로 다른 용도로 수정/재사용 해서는 안 되며 **Azuredtexec**를 중단할 수 있습니다. 이 경우 항상 파이프라인을 삭제할 수 있으며 **Azuredtexec** 는 다음에 호출 될 때 새 파이프라인을 생성 합니다.
