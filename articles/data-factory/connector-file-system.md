...
데이터 세트 정의에 사용할 수 있는 섹션 및 속성의 전체 목록은 데이터 세트 문서를 참조하세요. 이 섹션에서는 파일 시스템 데이터 세트에서 지원하는 속성의 목록을 제공합니다.

파일 시스템 간 데이터를 복사하려면 데이터 세트의 형식 속성을 **FileShare**로 설정합니다. 다음과 같은 속성이 지원됩니다.

| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 데이터 세트의 형식 속성을 **FileShare**로 설정해야 합니다. |yes |
| folderPath | 파일의 경로입니다. 와일드카드 필터는 지원되지 않습니다. 예제를 살펴보려면 [연결된 서비스 및 데이터 세트 정의 샘플](#sample-linked-service-and-dataset-definitions) 을 참조하세요. |아니요 |
