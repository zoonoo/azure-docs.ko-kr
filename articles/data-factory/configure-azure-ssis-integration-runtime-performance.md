---
title: Azure-SSIS 통합 런타임에 대한 성능 구성
...
-   D 시리즈의 성능/가격 비율은 A 시리즈보다 높으며 v3 시리즈의 성능/가격 비율은 v2 시리즈보다 높습니다.
-   D 시리즈의 처리량은 동일한 가격으로 A 시리즈보다 높으며 v3 시리즈의 처리량은 동일한 가격의 v2 시리즈보다 높습니다.
-   Azure-SSIS IR의 v2 계열 노드는 사용자 지정 설정에 적합하지 않으므로 대신 v3 계열 노드를 사용하십시오. 이미 v2 시리즈 노드를 사용하는 경우 가능한 한 빨리 v3 시리즈 노드를 사용하도록 전환하십시오.
-   E 시리즈는 메모리에 최적화된 VM 크기로 다른 컴퓨터보다 메모리 대 CPU 비율이 높습니다. 패키지에 메모리가 많은 경우 E 시리즈 VM을 선택하는 것이 좋습니다.

### <a name="configure-for-execution-speed"></a>실행 속도 구성
실행할 패키지가 많지 않고 패키지를 신속하게 실행하려면 다음 차트의 정보를 사용하여 시나리오에 적합한 가상 머신 유형을 선택합니다.

이 데이터는 단일 작업자 노드에서 단일 패키지 실행을 나타냅니다. 이 패키지는 Azure Blob Storage에서 이름과 성 열이 있는 3백만 개의 레코드를 로드하고, 전체 이름 열을 생성하고, 전체 이름이 20자 보다 긴 레코드를 Azure Blob Storage에 씁니다.
...
이미 강력한 작업자 노드를 사용하여 패키지를 실행 중인 경우 **AzureSSISMaxParallelExecutionsPerNode**를 늘리면 통합 런타임의 전체 처리량이 증가할 수 있습니다. Standard_D1_v2 노드의 경우 노드당 1-4개의 병렬 실행이 지원되며, 다른 모든 유형의 노드의 경우 노드당 1-max(코어 수 2개, 코어 수 8개)가 지원됩니다. **AzureSIsMax병렬실행퍼노드가** 지원되는 최대 값을 초과하여 원하는 경우 지원 티켓을 열 수 있으며, 그 이후에는 Azure Powershell을 사용하여 **AzureSSIsMaxParallelExecutionsPerNode를**업데이트해야 합니다.
...
-   로깅 수준이 세부정보로 설정된 경우 s3와 같은 보다 강력한 데이터베이스를 선택합니다. 비공식 사내 테스트에 따르면 s3 가격 책정 계층은 2개의 노드, 128개의 병렬 개수 및 자세한 로깅 수준으로 SSIS 패키지 실행을 지원할 수 있습니다.
