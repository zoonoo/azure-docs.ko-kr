...
| Azure SQL Database 또는 Azure Cosmos DB로 데이터 복사 |4와 싱크 Azure SQL Database 또는 Cosmos DB의 계층 (Dtu/Ru 수)에 따라 16 사이의 |
| 다른 모든 복사 시나리오 | 4 |
...
> [!TIP]
> 파일 기반 저장소 간에 데이터를 복사할 때 기본 동작을 일반적으로 사용 하면 최상의 처리량을 합니다. 기본 동작은 자동 결정에 원본 파일 패턴을 기반으로 합니다.
...
* 파일 기반 데이터 분할을 사용 하는 원본 Oracle 데이터베이스) (제외 되지 않는 저장소에서 데이터를 복사 하는 파일 기반 저장소로 데이터 이동 서비스는 무시 합니다 **parallelCopies** 속성입니다. 병렬 처리를 지정하더라도 이 경우에는 적용되지 않습니다.
...
현재 사용 하 여 아니고 준비 된 복사 하지 않고 다른 자체 호스팅 Ir을 통해 연결 된 두 데이터 저장소 간에 데이터를 복사할 수 없습니다. 이러한 시나리오에 대 한 원본에서 준비 후 싱크 스테이징에서 복사할 두 명시적으로 연결 된 복사 작업을 구성할 수 있습니다.
...
입력 또는 출력 데이터 집합이 파일인 경우에 대상에 데이터를 쓰기 때문에 압축 하거나 압축을 푸는 수행 하려면 복사 작업을 설정할 수 있습니다. 압축을 선택할 때 입력/출력(I/O) 및 CPU 간에 균형을 유지합니다. 컴퓨팅 리소스에서 데이터를 압축하는 데 추가 비용이 발생합니다. 대신에, 네트워크 I/O 및 저장소는 감소합니다. 데이터에 따라 전체 복사 처리량에서 향상을 볼 수 있습니다.
