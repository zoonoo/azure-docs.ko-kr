...
빅 데이터 분석을 위해 data lake 또는 EDW (엔터프라이즈 데이터 웨어하우스)에서 대규모 데이터 마이그레이션을 수행할지 또는 여러 원본에서 Azure로 대규모로 데이터를 수집 하려는 경우에는 최적의 성능 및 확장성을 구현 하는 것이 중요 합니다.  Azure Data Factory은 데이터를 대규모로 수집할 수 있는 성능, 복원 력 및 비용 효율적인 메커니즘을 제공 하 여 데이터 엔지니어가 뛰어난 성능 및 확장성이 뛰어난 데이터 수집 파이프라인을 구축 하는 데 적합 합니다.
