...
| quoteChar        | 열 구분 기호를 포함 하는 경우 열 값을 따옴표로 묶을 단일 문자입니다. <br>기본값은 `"`**큰따옴표** 입니다. <br>매핑 데이터 흐름의 경우 `quoteChar`은 빈 문자열일 수 없습니다. <br>복사 활동의 경우 `quoteChar`이 빈 문자열로 정의 되 면 따옴표 문자 및 열 값이 따옴표로 묶여 있지 않으며 `escapeChar`을 사용 하 여 열 구분 기호 및 자체를 이스케이프 합니다. | 아닙니다.       |
...
| 나이 설정  | 데이터 저장소에 데이터를 쓰는 방법에 대 한 속성 그룹입니다. 각 파일 기반 커넥터에는 `storeSettings`에서 자체 지원 되는 쓰기 설정이 있습니다.  | 아닙니다.       |
...
| fileExtension | 출력 파일의 이름을 사용 하는 데 사용 되는 파일 확장명 (예: `.csv` `.txt`)입니다. `fileName`를 출력 DelimitedText 데이터 집합에 지정 하지 않은 경우에는 지정 해야 합니다. | 출력 데이터 집합에 파일 이름이 지정 되지 않은 경우 예 |
