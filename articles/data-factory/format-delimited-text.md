...
| 속성         | 설명                                                  | 필수 |
| ---------------- | ------------------------------------------------------------ | -------- |
| type             | 데이터 집합의 형식 속성은 **DelimitedText**로 설정해야 합니다. | yes      |
| 위치         | 파일의 위치 설정입니다. 각 파일 기반 커넥터에는 고유한 위치 유형과 `location`지원되는 속성이 있습니다.  | yes      |
| columnDelimiter  | 파일에서 열을 구분하는 데 사용되는 문자입니다. 현재 다중 문자 구분 기호는 데이터 흐름을 매핑하는 데만 지원되지만 복사 활동은 지원되지 않습니다. <br>기본값은 **쉼표입니다 `,` **. | 예       |
| rowDelimiter     | 파일의 행을 구분하는 데 사용되는 단일 문자 또는 "\r\n"입니다.<br>기본값은 읽기시 다음과 같은 값 중 **하나입니다: ["\r\n", "\n", "\n"]** 및 **"\n" 또는 "\n" 또는 "\r\n"** 각각 매핑 데이터 흐름 및 복사 활동을 통해 쓰기에. <br>구분 `rowDelimiter` 기호(빈 문자열)로 설정되면 `columnDelimiter` 전체 콘텐츠를 단일 값으로 처리하는 구분 기호(빈 문자열)로 설정해야 합니다. | 예       |
...
| 속성       | 설명                                                  | 필수 |
| -------------- | ------------------------------------------------------------ | -------- |
| type           | 복사 활동 소스의 형식 속성은 **DelimitedTextSource**로 설정해야 합니다. | yes      |
...
| 속성      | 설명                                                  | 필수 |
| ------------- | ------------------------------------------------------------ | -------- |
| type          | 형식설정의 형식은 **구분된텍스트읽기설정으로**설정해야 합니다. | yes      |
...
| 속성       | 설명                                                  | 필수 |
| -------------- | ------------------------------------------------------------ | -------- |
| type           | 복사 활동 소스의 형식 속성은 **DelimitedTextSink로**설정해야 합니다. | yes      |
...
| 속성      | 설명                                                  | 필수                                              |
| ------------- | ------------------------------------------------------------ | ----------------------------------------------------- |
| type          | 형식 설정의 형식은 **구분텍스트쓰기 설정으로**설정해야 합니다. | yes                                                   |
