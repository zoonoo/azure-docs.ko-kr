...
2. **Azure 키 자격 증명 모음에 관리되는 ID 액세스를 부여합니다.** 키 자격 증명 모음 -> 액세스 정책 -> 새 -> 추가하려면 이 관리되는 ID를 검색하여 비밀 사용 권한 드롭다운에서 **권한 얻기를** 부여합니다. 그러면 이 지정된 팩터리가 키 자격 증명 모음에 있는 비밀에 액세스할 수 있습니다.
...
**연결** -> **연결 서비스** -> **+새** -> "Azure 키 자격 증명 모음"에 대 한 검색을 클릭 합니다.
...
| secretName | Azure 키 볼트의 비밀 이름입니다. | yes |
