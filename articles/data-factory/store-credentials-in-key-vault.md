...
1. 팩터리와 함께 생성 된 "관리 Id 응용 프로그램 ID"의 값을 복사 하 여 **데이터 팩터리 관리 id를 검색** 합니다. ADF 제작 UI를 사용 하는 경우 관리 id 응용 프로그램 ID가 Azure Key Vault 연결 된 서비스 만들기 창에 표시 됩니다. Azure Portal에서 검색할 수도 있습니다. [data factory 관리 Id 검색](data-factory-service-identity.md#retrieve-managed-identity)을 참조 하세요.
2. **관리 id에 Azure Key Vault에 대 한 액세스 권한을 부여 합니다.** 키 자격 증명 모음에서 액세스 정책 >-> 추가->이 관리 되는 id 응용 프로그램 ID를 검색 하 여 비밀 권한 드롭다운에서 **Get** 권한을 부여 합니다. 그러면 이 지정된 팩터리가 키 자격 증명 모음에 있는 비밀에 액세스할 수 있습니다.
...
| secretVersion | Azure Key Vault에 있는 암호의 버전입니다.<br/>지정하지 않으면 항상 최신 버전의 비밀을 사용합니다.<br/>지정하는 경우 지정된 버전을 사용합니다.| 아닙니다. |
