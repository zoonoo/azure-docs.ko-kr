...
가상 네트워크를 조인하여 SSISDB를 호스트하는 가상 네트워크 서비스 엔드포인트/관리되는 인스턴스(미리 보기)에 Azure SQL Database를 사용하는 경우 Azure-SSIS 통합 런타임을 동일한 가상 네트워크에 조인해야 합니다. Azure Data Factory 버전 2(미리 보기)를 사용하면 Azure-SSIS 통합 런타임을 가상 네트워크에 조인할 수 있습니다. 자세한 내용은 [Azure-SSIS 통합 런타임을 가상 네트워크에 조인](join-azure-ssis-integration-runtime-virtual-network.md)을 참조하세요.
