...
| connectionString | Couchbase에 연결할 ODBC 연결 문자열입니다. <br/>자격 증명 문자열을 Azure Key Vault에 넣고, 연결 문자열에서 `credString` 구성을 끌어올 수도 있습니다. 자세한 내용은 다음 샘플 및 [Azure Key Vault에 자격 증명 저장](store-credentials-in-key-vault.md) 문서를 참조하세요. | yes |
