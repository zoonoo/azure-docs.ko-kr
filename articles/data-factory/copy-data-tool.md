...
- Azure Data Factory 데이터 복사 도구를 사용할 때는 연결된 서비스, 데이터 집합, 파이프라인, 작업 및 트리거에 대한 Data Factory 정의를 이해할 필요가 없습니다. 
- 데이터 복사 도구의 흐름을 보면 Data Lake에 데이터를 로드하는 과정을 이해하기 쉽습니다. 이 도구는 선택한 원본 데이터 저장소에서 선택한 대상/싱크 데이터 저장소로 데이터를 복사하는 데 필요한 모든 Data Factory 리소스를 자동으로 만듭니다. 
- 데이터 복사 도구는 작성 시에 수집되는 데이터의 유효성을 검사하여 시작 시 잠재적인 오류를 피하도록 도와줍니다.
- Data Lake에 데이터를 로드하는 복잡한 비즈니스 논리를 구현해야 하는 경우, Data Factory UI의 작업별 작성 기능을 사용하여 데이터 복사 도구로 만든 Data Factory 리소스를 계속 편집할 수 있습니다. 

다음 표에서는 데이터 복사 도구 및 Data Factory UI의 작업별 작성 기능을 사용해야 하는 경우에 대한 지침을 제공합니다. 

| 데이터 복사 도구 | 작업별(복사 작업) 작성 |
| -------------- | -------------------------------------- |
| Azure Data Factory 엔터티(연결된 서비스, 데이터 집합, 파이프라인 등)에 대해 학습하지 않고도 데이터 로드 태스크를 쉽게 빌드하고 싶을 것입니다. | 데이터를 Lake에 로드하기 위한 복잡하고 유연한 논리를 구현하고 싶을 것입니다. |
| Data Lake에 많은 수의 데이터 아티팩트를 신속하게 로드하고 싶을 것입니다. | 데이터를 정리 또는 처리하기 위한 후속 작업에 복사 작업을 연결하고 싶을 것입니다. |
...
1. **원본**에 대한 설정을 구성합니다.
2. **대상**에 대한 설정을 구성합니다. 
3. 열 매핑 및 성능 설정 및 내결함성 설정과 같은 복사 작업에 대한 **고급 설정**을 구성합니다. 
4. 데이터 로드 작업에 대한 **일정**을 지정합니다. 
5. 만들려는 Data Factory 엔터티의 **요약**을 검토합니다. 
6. 파이프라인을 **편집**하여 필요에 따라 복사 작업에 대한 설정을 업데이트합니다. 
...
데이터 복사 도구는 원본 및 대상 저장소 간에 열을 매핑할 때 사용자의 동작을 모니터링하고 학습합니다. 원본 데이터 저장소에서 한 개 이상의 열을 선택하고 대상 스키마에 매핑하면, 데이터 복사 도구는 양쪽에서 선택한 열 쌍에 대한 패턴을 분석하기 시작합니다. 그런 다음, 나머지 열에 같은 패턴을 적용합니다. 따라서 여러 번 클릭한 후에만 모든 열이 원하는 방식대로 대상에 매핑될 수 있습니다.  데이터 복사 도구가 제공하는 열 매핑 선택 항목이 만족스럽지 않은 경우, 이를 무시하고 열을 수동으로 매핑할 수 있습니다. 한편, 데이터 복사 도구는 패턴을 지속적으로 업데이트하고, 궁극적으로는 도달하려는 열 매핑의 올바른 패턴에 도달합니다. 
...
### <a name="filter-data-in-a-database"></a>데이터베이스에서 데이터 필터링
다음 스크린샷에서는 데이터를 필터링하는 SQL 쿼리를 보여 줍니다.

![데이터베이스에서 데이터 필터링](./media/copy-data-tool/filter-data-in-database.png)
...
데이터 복사 도구를 사용하여 이러한 자습서를 시도해 보세요.
