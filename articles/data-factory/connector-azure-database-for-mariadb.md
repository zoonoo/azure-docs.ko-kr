---
title: Azure Data Factory를 사용하여 Azure Database for MariaDB에서 데이터 복사 | Microsoft Docs
...
| connectionString | Azure Database for MariaDB에 연결하는 연결 문자열입니다. Azure Portal -> Azure Database for MariaDB -> 연결 문자열 -> ADO.NET에서 찾을 수 있습니다. 이 필드를 SecureString으로 표시하여 Data Factory에 안전하게 저장하거나 [Azure Key Vault에 저장되는 비밀을 참조](store-credentials-in-key-vault.md)합니다. | 예 |
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 데이터 세트의 type 속성을 다음으로 설정해야 합니다. **MariaDBTable** | 예 |
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 복사 작업 원본의 type 속성을 다음으로 설정해야 합니다. **MariaDBSource** | 예 |
