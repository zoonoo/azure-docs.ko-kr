...
## 모니터링 및 관리 앱 시작 
모니터 및 관리 앱을 시작하려면 데이터 팩터리에 대한 **데이터 팩터리** 블레이드에서 **모니터링 앱** 타일을 클릭합니다.
...
### 리소스 탐색기
왼쪽 창에서 리소스 탐색기 **트리 뷰**, 위쪽에서 **다이어그램 보기** 및 가운데 창의 맨 아래에 **작업 창** 목록이 표시되며 오른쪽 창에 **속성**/**작업 창 탐색기**탭이 표시됩니다.

트리 뷰에서 데이터 팩터리의 모든 리소스(예: 파이프라인, 데이터 집합, 연결된 서비스)를 볼 수 있습니다. 리소스 탐색기에서 개체를 선택하는 경우 다음을 알려줍니다.
...
파이프라인을 사용하도록 설정한 경우(즉, 일시 중지된 상태가 아님) 아래와 같이 녹색 선으로 표시됩니다.

![파이프라인 실행](./media/data-factory-monitor-manage-app/PipelineRunning.png)

다이어그램 뷰에서 파이프라인에 대한 세 개의 명령 단추가 있다고 알려줍니다. 두 번째 단추를 사용하여 파이프라인을 일시 중지할 수 있습니다. 현재 실행 중인 활동을 종료하지 않고 완료될 때까지 계속할 수 있습니다. 세 번째 단추는 파이프라인을 일시 중지하고 실행 중인 기존의 활동을 종료합니다. 첫 번째 단추는 파이프라인을 다시 시작, 즉 일시 중지를 해제합니다. 파이프라인을 일시 중지하면 다음과 같이 파이프라인 타일에 대한 색상 변경을 확인할 수 있습니다.
...
닫힌/열린 파이프라인 뷰에서 출력 데이터 집합을 클릭할 경우, 출력 데이터 집합 위로 마우스를 이동하는 경우, 해당 데이터 집합에 작업 창 팝업이 표시됩니다.
...
다이어그램 뷰의 아래쪽 단추에서 확대, 축소, 크기에 맞게, 100% 확대, 레이아웃 잠금 단추가 표시됩니다(다이어그램 뷰에서 테이블 및 파이프라인을 실수로 이동하지 않도록 방지). 레이아웃 잠금 단추는 기본적으로 켜집니다. 기능을 해제하고 다이어그램에서 엔터티를 이동할 수 있습니다. 해제를 설정한 경우 마지막 단추를 사용하여 테이블 및 파이프라인을 자동으로 배치할 수 있습니다. 또한 마우스 휠을 사용하여 확대/축소할 수 있습니다.
...
목록에서 작업 창을 클릭할 때 오른쪽에 있는 작업 창 탐색기 또는 속성 창에서 작업 창에 대한 자세한 내용이 표시됩니다.
...
위쪽의 일정 보기에서 다른 작업 창을 클릭하여 전환할 수 있습니다. 또한 위쪽에서 **왼쪽 화살표**/**오른쪽 화살표** 단추를 사용하여 이전/다음 주에서 작업 창을 볼 수 있습니다.
...
이번에 지원되는 세 가지 시스템 뷰가 있습니다. (가운데 창의 맨 위에 있는)작업 창 목록에서 옵션을 선택하여 최근 작업 창 (또는) 실패한 작업 창 (또는) 진행 중인 작업 창을 봅니다.

**최근 작업 창** 옵션을 선택하면 **마지막 시도 시간**의 내림차순으로 최근 작업 창을 모두 표시합니다.

**실패한 작업 창** 뷰를 사용하여 목록에서 모든 실패한 작업 창을 볼 수 있습니다. 목록에서 실패한 작업 창을 선택하여 **속성** 창 (또는) **작업 창 탐색기**에서 이에 대한 자세한 내용을 봅니다. 또한 실패한 작업 창에 대한 로그를 다운로드할 수 있습니다.
...
> [AZURE.NOTE] 이번에는 모니터링 및 관리 앱에서 UTC 형식에 모든 시간이 있습니다.
...
### 선택한 작업 창 다시 실행
작업 창을 선택하고 첫 번째 명령 모음 단추에 아래쪽 화살표를 클릭한 다음 **다시 실행**/**파이프라인에서 업스트림으로 다시 실행**을 선택합니다. **파이프라인에서 업스트림으로 다시 실행** 옵션을 선택하면 모든 업스트림 작업 창을 다시 실행합니다. ![작업 창 다시 실행](./media/data-factory-monitor-manage-app/ReRunSlice.png)

또한 목록에서 여러 개의 작업 창을 선택하고 동시에 다시 실행할 수 있습니다. 상태를 기반으로 작업 창을 필터링(예: **실패**)한 다음 작업 창에 실패를 일으키는 문제를 해결한 후에 실패한 작업 창을 다시 실행할 수 있습니다. 목록에서 작업 창을 필터링하는 자세한 내용은 다음 섹션을 참조합니다.

### 여러 파이프라인 일시 중지/다시 시작
...
## 경고 생성 
경고 페이지에서 새 경고를 만들고 기존 경고를 보기/편집/삭제할 수 있습니다. 또한 경고를 사용하지 않거나/사용할 수 있습니다. 경고 탭을 클릭하여 페이지를 봅니다.
