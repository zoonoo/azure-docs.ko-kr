...
**메모장** 을 시작하고 다음의 HQL 스크립트를 붙여넣습니다. 이 Hive 스크립트는 **WebLogsRaw** 및 **WebLogsPartitioned**라는 두 개의 테이블을 만듭니다. 메뉴에서 **파일**을 클릭하고 **다른 이름으로 저장**을 선택합니다. 하드 드라이브의 **C:\adfgetstarted** 폴더로 전환합니다. **파일 형식** 필드에서 **모든 파일 (*.*)**을 선택합니다. **파일 이름**에서 **partitionweblogs.hql**을 입력합니다. 대화 상자의 아래쪽에서 **인코딩** 필드가 **ANSI**로 설정되어 있는지 확인합니다. 그렇지 않은 경우 **ANSI**로 설정합니다.  
