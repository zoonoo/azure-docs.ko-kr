---
title: 문제 해결 Azure Data Factory
...
- **권장 사항**: 유효한 JSON 페이로드를 반환 하도록 Azure 함수를 업데이트 합니다. 예를 들어 함수 C# 는 \"Id\"`(ActionResult)new<OkObjectResult("{`를 반환할 수 있습니다.\"123\"`}");`.
...
- **메시지**: AzureMLExecutePipeline 작업에 JSON의 LinkedService 정의가 없습니다.
...
- **원인**: HDInsight의 자격 증명이 잘못 되었거나 만료 되었습니다.
...
- **원인**:이 오류는 HDInsight에서 발생 합니다.

- **권장 사항**:이 오류는 HDInsight 클러스터에서 발생 합니다. 자세한 내용은 [AMBARI UI 502 오류](https://hdinsight.github.io/ambari/ambari-ui-502-error.html), [502 Errors to spark Thrift server](https://hdinsight.github.io/spark/spark-thriftserver-errors.html), [Thrift server에 연결 하는 502 오류](https://hdinsight.github.io/spark/spark-thriftserver-errors.html), [Application Gateway에서 잘못 된 게이트웨이 오류 문제 해결](https://docs.microsoft.com/azure/application-gateway/application-gateway-troubleshooting-502)을 참조 하세요.
...
- **권장 사항**: HDInsight에 제출 되는 동시 작업 수를 제한 하는 것이 좋습니다. 동일한 작업을 통해 작업을 전송 하는 경우 Data Factory 작업 동시성을 참조 하세요. 동시 파이프라인 실행이 시간이 지남에 따라 분산 되도록 트리거를 변경 합니다. 오류가 제안 된 대로 `templeton.parallellism.job.submit`를 조정 하려면 HDInsight 설명서를 참조 하세요.
...
- **원인**: 웹 작업이 작업 페이로드에 지정 된 HTTP 메서드를 지원 하지 않습니다.
...
- **권장 사항**: URL에 대 한 기능을 확인 하려면 Fiddler 또는 postman을 사용 합니다.
...
- **권장 사항**: 페이로드 형식과 일치 하는 콘텐츠 형식을 지정 합니다. Fiddler 또는 Postman을 사용 하 여 요청의 유효성을 검사 합니다.
...
- **원인**: 잘못 된 웹 작업 메서드가 요청에 지정 되었습니다.
...
- **원인**:이 오류에는 네트워크 연결, DNS 오류, 서버 인증서 유효성 검사 또는 시간 초과와 같은 여러 가지 이유가 있을 수 있습니다.
