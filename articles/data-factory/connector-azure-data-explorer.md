...
>[!TIP]
>Azure 데이터 탐색기 커넥터에 대 한 연습은 [Azure Data Factory를 사용 하 여 azure 데이터 탐색기 간에 데이터 복사](../data-explorer/data-factory-load-data.md)를 참조 하세요.
...
| 테넌트(tenant) | 애플리케이션이 있는 테넌트 정보(도메인 이름 또는 테넌트 ID)를 지정합니다. 이를 [Kusto 연결 문자열](https://docs.microsoft.com/azure/kusto/api/connection-strings/kusto#application-authentication-properties)에 "Authority ID" 라고 합니다. Azure Portal의 오른쪽 위 모서리에 마우스 포인터를 올려 검색 합니다. | 예 |
