...
- **Azure SQL Database 서버/관리되는 인스턴스(미리 보기)**. 데이터베이스 서버가 아직 없는 경우 시작하기 전에 Azure Portal에서 이 서버를 만듭니다. 이 서버는 SSISDB(SSIS 카탈로그 데이터베이스)를 호스팅합니다. Integration Runtime과 동일한 Azure 지역에 데이터베이스 서버를 만드는 것이 좋습니다. 이 구성을 사용하면 Integration Runtime에서 Azure 지역을 벗어나지 않고 SSISDB에 실행 로그를 쓸 수 있습니다. 선택한 데이터베이스 서버에 따라, 가상 네트워크를 조인하여 SSISDB를 자동으로 독립 실행형 데이터베이스로, 탄력적 풀의 일부로 또는 관리되는 인스턴스(미리 보기)에 만들고 공용 네트워크에서 액세스할 수 있습니다. Azure SQL Database에 지원되는 가격 책정 계층의 목록은 [SQL Database 리소스 제한](../sql-database/sql-database-resource-limits.md)을 참조하세요.

    Azure SQL Database 서버/관리되는 인스턴스(미리 보기)에 SSIS 카탈로그(SSIDB 데이터베이스)가 없는지 확인합니다. Azure-SSIS IR 프로비전은 기존 SSIS 카탈로그 사용을 지원하지 않습니다.
- **클래식/Azure Resource Manager 가상 네트워크(선택 사항)**. 다음 조건 중 하나 이상에 해당하는 경우 Azure 가상 네트워크가 있어야 합니다.
...
3. **SQL 설정** 페이지에서 다음 단계를 완료합니다. 
    ...
    다. **카탈로그 데이터베이스 서버 엔드포인트**로는 SSISDB를 호스트하는 데이터베이스 서버의 엔드포인트를 선택합니다. 선택한 데이터베이스 서버에 따라, 가상 네트워크를 조인하여 SSISDB를 자동으로 독립 실행형 데이터베이스로, 탄력적 풀의 일부로 또는 관리되는 인스턴스(미리 보기)에 만들고 공용 네트워크에서 액세스할 수 있습니다.
...
5. **가상 네트워크 선택...** 확인란에서 통합 런타임을 가상 네트워크에 조인할 것인지 선택합니다. Azure SQL Database를 가상 네트워크 서비스 엔드포인트/관리되는 인스턴스(미리 보기)와 함께 사용하여 SSISDB를 호스트하거나 온-프레미스 데이터에 액세스하도록 요구하는 경우 선택합니다. 즉, SSIS 패키지에 온-프레미스 데이터 원본/대상이 있습니다. [가상 네트워크에서 Azure-SSIS IR 조인](https://docs.microsoft.com/en-us/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network)을 참조하세요. 선택한 경우 다음 단계를 완료합니다.
    ...
    나. **형식**에 대해서는 가상 네트워크의 형식: 클래식 또는 Azure Resource Manager를 선택합니다. 클래식 가상 네트워크는 곧 사용이 중단될 예정이므로 Azure Resource Manager 가상 네트워크를 선택하는 것이 좋습니다.
    
    다. **VNet 이름**에서 가상 네트워크의 이름을 선택합니다. 이 가상 네트워크는 가상 네트워크 서비스 엔드포인트/관리되는 인스턴스(미리 보기)를 통한 Azure SQL Database에서 SSISDB를 호스팅하는 데 사용되는 동일한 가상 네트워크 및 또는 온-프레미스 네트워크에 연결된 가상 네트워크여야 합니다.

    d. **서브넷 이름**에서 가상 네트워크의 서브넷 이름을 선택합니다. 이 가상 네트워크는 관리되는 인스턴스(미리 보기)에서 SSISDB를 호스팅하는 데 사용되는 것과 다른 서브넷이어야 합니다.
...
가상 네트워크를 조인하는 Azure SQL 관리되는 인스턴스(미리 보기)와 함께 VnetId 및 Subnet 매개 변수에 대한 값을 전달해야 합니다. Azure SQL 관리되는 인스턴스(미리 보기)에는 CatalogPricingTier 매개 변수가 적용되지 않습니다. 
