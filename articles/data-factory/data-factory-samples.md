...
1. **파일**을 클릭하고 **새로 만들기**를 가리킨 다음 **프로젝트**를 클릭합니다. 
...
6. **데이터 팩터리 템플릿** 대화 상자에서 **사용 사례 템플릿** 섹션에 있는 샘플 템플릿을 선택하고 **다음**을 클릭합니다. 다음 단계는 **고객 프로파일링** 템플릿 사용 방법에 대해 안내합니다. 단계는 다른 샘플과 유사합니다. 
...
8. **데이터 팩터리 구성** 페이지에서 다음을 수행합니다. 
	...
	5. **지역**에서 **미국 서부** 또는 **북유럽**을 선택합니다.
	6. **다음**을 클릭합니다. 
9. **데이터 저장소 구성** 페이지에서 기존 **Azure SQL 데이터베이스** 및 **Azure 저장소 계정**을 지정 (또는) 새 레코드를 만들고 다음을 클릭합니다. 
...
Visual Studio를 사용하여 데이터 팩터리 엔터티를 작성하고 Azure에 게시하는 방법에 대한 자세한 내용은 [첫 번째 데이터 팩터리(Visual Studio) 빌드](data-factory-build-your-first-pipeline-using-vs.md)를 참조하세요.
