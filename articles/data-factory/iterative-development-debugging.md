...
데이터 흐름 디자이너의 데이터 미리 보기 및 데이터 흐름의 파이프라인 디버깅은 작은 데이터 샘플에서 가장 잘 작동 하기 위한 것입니다. 그러나 많은 양의 데이터에 대 한 파이프라인 또는 데이터 흐름에서 논리를 테스트 해야 하는 경우 더 많은 코어와 최소한의 범용 계산을 사용 하 여 디버그 세션에서 사용 되는 Azure Integration Runtime의 크기를 늘립니다.
...
> [!NOTE]
> 병렬 또는 대량 데이터 집합을 사용 하 여 테스트 해야 하는 데이터 흐름에서 데이터 흐름을 실행 하는 파이프라인이 있는 경우 "작업 런타임 사용"을 선택 하 여 Data Factory 데이터 흐름 작업에서 선택한 Integration Runtime를 사용할 수 있도록 합니다. 이렇게 하면 데이터 흐름이 여러 클러스터에서 실행 될 수 있으며 병렬 데이터 흐름 실행을 수용할 수 있습니다.
