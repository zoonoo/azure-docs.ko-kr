...
| type              | Hadoop 스트리밍 작업의 경우 작업 유형은 HDInsightStreaming입니다. | yes      |
...
| mapper            | mapper 실행 파일의 이름을 지정합니다. | yes      |
| reducer           | reducer 실행 파일의 이름을 지정합니다. | yes      |
...
| filePath          | fileLinkedService에서 참조하는 Azure Storage에 저장된 Mapper, Combiner 및 Reducer 프로그램의 경로 배열을 제공합니다. 경로는 대/소문자를 구분합니다. | yes      |
| input             | Mapper에 대한 입력 파일의 WASB 경로를 지정합니다. | yes      |
| output            | Reducer에 대한 출력 파일의 WASB 경로를 지정합니다. | yes      |
