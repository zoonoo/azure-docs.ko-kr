...
| `rfcTableFields`                   | SAP 테이블에서 복사할 필드 (열)입니다. 예를 들어, `column0, column1`을 입력합니다. | 아니요       |
| `rfcTableOptions`                  | SAP 테이블의 행을 필터링 하는 옵션입니다. 예를 들어, `COLUMN0 EQ 'SOMEVALUE'`을 입력합니다. 이 문서의 뒷부분에 나오는 SAP 쿼리 연산자 표를 참조 하세요. | 아니요       |
