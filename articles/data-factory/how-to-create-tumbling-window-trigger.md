---
title: Azure 데이터 팩터리에서 텀블링 창 트리거 만들기
...
1. 데이터 팩터리 UI에서 텀블링 창 트리거를 만들려면 **트리거 탭을** 선택한 다음 **새**을 선택합니다. 
1. 트리거 구성 창이 열리면 **[뒤로 뜨기]** 창을 선택한 다음 텀블링 윈도우 트리거 속성을 정의합니다. 
1. 완료되면 **저장**을 선택합니다.
...
|:--- |:--- |:--- |:--- |:--- |
| **종류** | 트리거의 유형입니다. 형식은 고정값 "텀블링윈도우트리거"입니다. | String | "TumblingWindowTrigger" | yes |
| **runtimeState** | 트리거 런타임의 현재 상태입니다.<br/>**참고**: 이 요소는 \<readOnly>입니다. | String | "시작된," "중지된," "사용 안 함" | yes |
| **frequency** | 트리거를 되풀이하는 빈도 단위(시간 또는 분)를 나타내는 문자열입니다. **startTime** 날짜 값이 **frequency** 값보다 더 세부적인 경우 **startTime** 날짜는 시간 경계를 계산할 때 고려됩니다. 예를 들어, **frequency** 값이 시간이고 **startTime** 값이 2017-09-01T10:10:10Z이면 첫 번째 기간은 (2017-09-01T10:10:10Z, 2017-09-01T11:10:10Z)입니다. | String | "minute," "hour"  | yes |
| **간격** | 트리거가 실행되는 빈도를 결정하는 **frequency** 값에 대한 간격을 나타내는 양의 정수입니다. 예를 들어 **interval**이 3이고 **frequency**가 "시간"인 경우 트리거가 세 시간마다 되풀이됩니다. <br/>**참고**: 최소 창 간격은 15 분입니다. | 정수 | 양의 정수입니다. | yes |
...
| **종속 성: 입력** | 텀블링윈도우트리거참조의 유형입니다. 종속성을 설정한 경우 필요합니다. | String |  "텀블링윈도우트리거리퍼런시레퍼런서", "자체의존성 텀블링윈도우트리거참조" | 예 |
| **에 따라: 크기** | 종속성 텀블링 창의 크기입니다. | Timespan<br/>(hh:mm:ss)  | 기본값이 자식 트리거의 창 크기인 양수 시간 범위 값  | 예 |
| **종속 성: 오프셋** | 종속성 트리거의 오프셋입니다. | Timespan<br/>(hh:mm:ss) |  자체 종속성에서 음수여야 하는 시간 범위 값입니다. 값을 지정하지 않으면 창이 트리거 자체와 동일합니다. | 자체 종속성: 예<br/>기타: 아니오  |
...
### <a name="tumbling-window-trigger-dependency"></a>텀블링 윈도우 트리거 종속성

데이터 팩터리에서 다른 텀블링 윈도우 트리거를 성공적으로 실행한 후에만 텀블링 윈도우 트리거가 실행되도록 하려면 [텀블링 윈도우 트리거 종속성을 만듭니다.](tumbling-window-trigger-dependency.md) 
