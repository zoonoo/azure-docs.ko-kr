...
UTC의 타임스탬프로 변환합니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 표준 시간대를 선택적으로 전달할 수 있습니다. 기본적으로 현재 표준 시간대가 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
타임스탬프의 시간 값을 가져옵니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 선택적 표준 시간대를 제공할 수 있습니다. 현지 표준 시간대가 기본적으로 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
패턴은 말 그대로 일치하는 문자열을 나타냅니다. 예외는 다음과 같은 특수 기호입니다. _는 입력에서 하나의 문자와 일치합니다( ```posix```정규식에서)%는 입력에서 0 개 이상의 문자 (정규식의. *와 유사)와 일치 합니다 ```posix``` .
...
날짜의 밀리초 값을 구합니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 선택적 표준 시간대를 제공할 수 있습니다. 현지 표준 시간대가 기본적으로 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
타임스탬프의 분 값을 가져옵니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 선택적 표준 시간대를 제공할 수 있습니다. 현지 표준 시간대가 기본적으로 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
두 날짜 사이의 개월 수를 구합니다. 계산을 반올림할 수 있습니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 선택적 표준 시간대를 제공할 수 있습니다. 현지 표준 시간대가 기본적으로 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
문자열 값을 정규화 하 여 강조 된 유니코드 문자를 구분 합니다.  
...
NULL 값을 반환합니다. ' N u l `syntax(null())` l ' 이라는 열이 있는 경우 함수를 사용 합니다. 에서 사용 하는 모든 작업은 NULL을 반환 합니다.  
...
후행 문자 문자열을 오른쪽으로 자릅니다. 두 번째 매개 변수를 지정하지 않으면 공백을 삭제합니다. 그렇지 않으면 두 번째 매개 변수에 지정 된 모든 문자를 자릅니다.  
* ``rtrim('  dumbo  ') -> '  dumbo'``  
* ``rtrim('!--!du!mbo!', '-!') -> '!--!du!mbo'``  
___
### <code>second</code>
<code><b>second(<i>&lt;value1&gt;</i> : timestamp, [<i>&lt;value2&gt;</i> : string]) => integer</b></code><br/><br/>
날짜의 초 값을 가져옵니다. 'GMT', 'PST', 'UTC', 'America/Cayman' 형태로 선택적 표준 시간대를 제공할 수 있습니다. 현지 표준 시간대가 기본적으로 사용됩니다. `SimpleDateFormat`사용 가능한 형식에 대 한 자세한 내용은 Java의 클래스를 참조 하세요. https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html.  
...
날짜 또는 타임 스탬프에서 일을 뺍니다. 날짜에 대 한-연산자와 동일 합니다.  
...
항상 true 값을 반환합니다. `syntax(true())`' T r u e ' 라는 열이 있는 경우 함수를 사용 합니다.  
* ``(10 + 20 == 30) -> true``  
* ``(10 + 20 == 30) -> true()``  
___
### <code>typeMatch</code>
...
다음 함수는 집계, 피벗, 피벗 해제 및 창 변환 에서만 사용할 수 있습니다.
___
### <code>avg</code>
...
배열 함수는 배열 데이터 구조에 대 한 변환을 수행 합니다. 여기에는 배열 요소 및 인덱스를 처리 하기 위한 특수 키워드가 포함 됩니다.

* ```#acc``` 배열을 줄일 때 단일 출력에 포함 하려는 값을 나타냅니다.
* ```#index``` 배열 인덱스 번호와 함께 현재 배열 인덱스를 나타냅니다. ```#index2, #index3 ...```
* ```#item``` 배열의 현재 요소 값을 나타냅니다.
...
___
### <code>mapIf</code>
<code><b>mapIf (<value1> : array, <value2> : binaryfunction, <value3>: binaryFunction) => any</b></코드><br/><br/>는 배열을 동일 하거나 더 작은 다른 배열에 조건부로 매핑합니다. StructTypes를 포함 한 모든 데이터 형식의 값을 사용할 수 있습니다. 배열에 있는 항목의 주소를 #item로, 현재 인덱스를 #index으로 지정할 수 있는 매핑 함수를 사용 합니다. 깊게 중첩 된 맵의 경우 괄호를 참조할 수 있습니다 t maps using the ``#item_[n](#item_1 , #index_1...)`` notation.
...
___
### <code>mapLoop</code>
<code><b>mapLoop(<value1> : integer, <value2> : unaryfunction) => any</b></코드><br/><br/>는 1에서 길이로 반복 되어 해당 길이의 배열을 만듭니다. 배열에 있는 인덱스의 주소를 #index으로 지정할 수 있는 매핑 함수를 사용 합니다. 깊게 중첩 된 맵의 경우 #index_n (#index_1, #index를 사용 하 여 부모 맵을 참조할 수 있습니다._2...) notation.
