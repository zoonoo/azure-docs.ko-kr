...
스트림의 이름으로 열 값을 선택합니다. 여러 일치 항목이 있으면 첫 번째 일치 항목이 반환 됩니다. 일치 항목이 없는 경우 NULL 값을 반환 합니다. 반환 되는 값 형식 (TO_DATE, TO_STRING...)는 형식 변환 함수 중 하나로 변환 해야 합니다.  디자인 타임에 알려진 열 이름은 이름별으로만 해결 해야 합니다. 계산 된 입력은 지원 되지 않지만 매개 변수 대체를 사용할 수 있습니다.
...
스트림의 상대 위치 (1부터)를 기준으로 열 값을 선택 합니다. 위치 범위를 벗어나는 경우에 NULL 값을 반환 합니다. 형식 변환 함수 (TO_DATE, TO_STRING...) 중 하나로 변환 하는 형식으로 반환 된 값이 계산 된 입력은 지원 되지 않지만 매개 변수 대체를 사용할 수 있습니다.
...
타임 스탬프 UTC에서 변환합니다. 필요에 따라 'GMT', '태평양 표준시', 'UTC', ' America/케이맨 ' 형식의 표준 시간대를 전달할 수 있습니다. 현재 표준 시간대에 기본값으로
...
숫자 또는 문자열을 정수 값으로 변환합니다. 선택적 Java 10 진수 형식 변환 하는 데 사용할 수 있습니다. 모든 long, float을, double 자릅니다.
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
...
악센트 부호가 있는 유니코드 문자를 구분 하는 문자열 값을 정규화 합니다.
* ``normalize('boys') -> 'boys'``
*********************************
<code>not</code>
==============================
