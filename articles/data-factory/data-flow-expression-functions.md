...
Consine 역 값을 계산 합니다.* ``acos(1) -> 0.0``
---
### <code>add</code>
...
특정 길이가 될 때까지 제공된 패딩으로 문자열의 왼쪽 여백을 채웁니다. 문자열이 길이 보다 크거나 같은 경우 * ``lpad('great', 10, '-') -> '-----great'`` 에는 no op 
* ``lpad('great', 4, '-') -> 'great'`` 
* ' ' lpad (' < > ')-> ' < ><great'``
---
...
특정 길이가 될 때까지 제공된 패딩으로 문자열의 오른쪽 여백을 채웁니다. 문자열이 길이 보다 크거나 같은 * ``rpad('great', 10, '-') -> 'great-----'``경우에는 op 
* ``rpad('great', 4, '-') -> 'great'`` 
* ``rpad('great', 8, '<>') -> 'great<><'`` 
---
