...
이 가이드에서는 표준 SQL Server AdventureWorks 예제 데이터베이스의 ```SalesOrderHeader``` 및 ```SalesOrderDetail``` 테이블을 사용 하는 "주문" 이라는 즉석에서 새 컨테이너를 작성 합니다. 이러한 테이블은에 의해 ```SalesOrderID```조인 된 판매 트랜잭션을 나타냅니다. 각 고유 정보 레코드에는의 ```SalesOrderDetailID```고유한 기본 키가 있습니다. 헤더와 세부 정보 간의 관계는 ```1:M```입니다. ADF ```SalesOrderID``` 에서 가입한 다음 각 관련 정보 레코드를 "detail" 이라는 배열로 롤포워드합니다.
...
![수집](media/data-flow/cosmosb3.png)
...
5. "SourceOrderDetails"의 원본을 정의 합니다. 데이터 집합의 경우 ```SalesOrderDetail``` 테이블을 가리키는 새 Azure SQL Database 데이터 집합을 만듭니다.

6. "SourceOrderHeader"의 원본을 정의 합니다. 데이터 집합의 경우 ```SalesOrderHeader``` 테이블을 가리키는 새 Azure SQL Database 데이터 집합을 만듭니다.

7. 위쪽 원본에서 "SourceOrderDetails" 뒤에 파생 열 변환을 추가 합니다. 새 변환 "형식 캐스팅"를 호출 합니다. ```UnitPrice``` 열을 반올림 하 고 CosmosDB에 대 한 double 데이터 형식으로 캐스팅 해야 합니다. 수식을로 설정 ```toDouble(round(UnitPrice,2))```합니다.

8. 다른 파생 열을 추가 하 고 "MakeStruct"로 호출 합니다. 여기서는 세부 정보 테이블의 값을 포함 하는 계층 구조를 만들게 됩니다. 세부 정보는 헤더에 ```M:1``` 대 한 관계를 염두에 두어야 합니다. 새 구조의 ```orderdetailsstruct``` 이름을로 설정 하 고 이러한 방식으로 계층 구조를 만들어 각 하위 열을 들어오는 열 이름으로 설정 합니다.

![Create Structure](media/data-flow/cosmosb9.png)

9. 이제 sales 헤더 원본으로 이동 하겠습니다. 조인 변환을 추가 합니다. 오른쪽에서 "MakeStruct"를 선택 합니다. 내부 조인으로 설정 된 채로 두고 조인 ```SalesOrderID``` 조건의 양쪽에 대해 선택 합니다.
...
14. 다음은 공통 키 ```SalesOrderID```를 기준으로 그룹화 하 여 행을 비 정규화는 위치입니다. 집계 변환을 추가 하 고 group by를로 ```SalesOrderID```설정 합니다.

15. 집계 수식에서 "details" 라는 새 열을 추가 하 고이 수식을 사용 하 여 앞 ```orderdetailsstruct```에서 만든 구조에서 값을 수집 ```collect(orderdetailsstruct)```합니다.
...
17. 동일한 열 이름을 유지 하 고 함수를 ```first()``` 집계로 사용 하도록 다른 속성의 "this" 구문을 사용 합니다.
...
19. 싱크 설정에서 키를로 ```\SalesOrderID``` 분할 하 고 컬렉션 작업을 "다시 만들기"로 설정 합니다. 매핑 탭이 다음과 같이 표시 되는지 확인 합니다.
