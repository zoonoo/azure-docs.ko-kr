...
자세한 내용은 [Azure 데이터 팩터리를 사용하여 온-프레미스 및 클라우드 간 데이터 이동](data-factory-move-data-between-onprem-and-cloud.md) 및 [데이터 관리 게이트웨이](data-factory-data-management-gateway.md) 문서를 참조하세요.
...
- 게이트웨이가 예상대로 작동 하지 않으면 지난 7일 간의 게이트웨이 로그를 Microsoft에 전송하여 문제 해결을 용이하게 할 수 있습니다. 게이트웨이가 클라우드 서비스에 연결되어 있지 않으면 게이트웨이 로그를 저장하고 보관할 수 있습니다.
- 게이트웨이 구성 관리자에 대한 사용자 인터페이스 개선 사항:
	- 게이트웨이 상태가 홈 탭에 잘 보이도록 합니다.
	- 재구성되고 간소화된 컨트롤입니다.
- [코드가 없는 복사 미리 보기 도구](data-factory-copy-data-wizard-tutorial.md)를 사용하여 Polybase 및 Blob 준비를 통해 이외의 저장소에서 Azure Blob Azure SQL 데이터 웨어하우스에 데이터를 복사할 수 있습니다. 일반적인 이 기능에 대한 자세한 내용은 [준비된 복사](data-factory-copy-activity-performance.md#staged-copy)를 참조하세요.
- 데이터 관리 게이트웨이를 활용하여 온-프레미스 SQL Server 데이터베이스에서 Azure 기계 학습으로 직접 데이터를 수신할 수 있습니다.
- 성능 개선
