...
---
# <a name="incrementally-load-data-from-multiple-tables-in-sql-server-to-azure-sql-database"></a>SQL Server의 여러 테이블에서 Azure SQL Database로 데이터 증분 로드
이 자습서에서는 델타 데이터를 온-프레미스 SQL 서버의 여러 테이블에서 Azure SQL 데이터베이스로 로드하는 파이프라인이 있는 Azure 데이터 팩터리를 만듭니다.    
...
3. **다음 작업을 사용하여 파이프라인을 만듭니다.** 
    ...
    1. 파이프라인에 매개 변수로 전달되는 원본 테이블 이름 목록을 반복하는 **ForEach** 작업을 만듭니다. 각 원본 테이블에 해당 테이블에 대한 델타 로드를 수행하는 다음 작업을 호출합니다. 
    2. 두 개의 **조회** 작업을 만듭니다. 첫 번째 조회 작업을 사용하여 마지막 워터마크 값을 검색합니다. 두 번째 조회 작업을 사용하여 새 워터마크 값을 검색합니다. 이러한 워터마크 값은 복사 작업에 전달됩니다. 
    3. 이전 워터마크 값보다 크고, 새 워터마크 값보다 작은 워터마크 열 값으로 원본 데이터 저장소의 행을 복사하는 **복사 작업**을 만듭니다. 그런 다음 원본 데이터 저장소의 델타 데이터를 새 파일로 Blob 저장소에 복사합니다. 
    4. 다음에 실행되는 파이프라인에 대한 워터마크 값을 업데이트하는 **저장 프로시저 작업**을 만듭니다. 

        다음은 솔루션에 대한 대략적인 솔루션 다이어그램입니다. 

        ![데이터 증분 로드](media\tutorial-incremental-copy-multiple-tables-powershell\high-level-solution-diagram.png)
...
* **Azure SQL Database**. Azure SQL 데이터베이스를 **싱크** 데이터 저장소로 사용합니다. 아직 없는 경우 Azure SQL Database를 만드는 단계는 [Azure SQL Database 만들기](../sql-database/sql-database-get-started-portal.md) 문서를 참조하세요. 

### <a name="create-source-tables-in-your-sql-server-database"></a>SQL Server 데이터베이스에 원본 테이블 만들기

1. **SQL Server Management Studio**를 시작하고 온-프레미스 SQL Server에 연결합니다. 
...
3. Azure SQL 데이터베이스에 대해 다음 SQL 명령을 실행하여 `customer_table` 및 `project_table`이라는 테이블을 만듭니다.  
...
### <a name="create-another-table-in-azure-sql-database-to-store-the-high-watermark-value"></a>Azure SQL 데이터베이스에 상위 워터마크 값을 저장하기 위한 다른 테이블 만들기
1. Azure SQL 데이터베이스에 대해 다음 SQL 명령을 실행하여 워터마크 값을 저장하는 `watermarktable` 테이블을 만듭니다.  
    ...
    ```
3. 두 원본 테이블의 초기 워터마크 값을 워터마크 테이블에 삽입합니다.
...
다음 명령을 실행하여 Azure SQL 데이터베이스에 저장 프로시저를 만듭니다. 이 저장 프로시저는 파이프라인을 실행이 끝날 때마다 워터마크 값을 업데이트합니다. 
...
다음 쿼리를 실행하여 Azure SQL 데이터베이스에 2개의 저장 프로시저와 2개의 데이터 유형을 만듭니다. 이들은 원본 테이블의 데이터를 대상 테이블로 병합하는 데 사용됩니다.
...
3. 데이터 팩터리 이름에 대한 변수를 정의합니다. 
    ...
    > [!IMPORTANT]
    >  데이터 팩터리 이름을 전역적으로 고유한 이름으로 업데이트합니다. 예: ADFIncMultiCopyTutorialFactorySP1127. 
...
데이터 팩터리에서 연결된 서비스를 만들어 데이터 저장소를 연결하고 계산 서비스를 데이터 팩터리에 연결합니다. 이 섹션에서는 온-프레미스 SQL Server 데이터베이스와 Azure SQL 데이터베이스에 대한 연결된 서비스를 만듭니다. 

### <a name="create-sql-server-linked-service"></a>SQL Server 연결 서비스 만들기
이 단계에서 온-프레미스 SQL Server를 데이터 팩터리에 연결합니다.

1. 다음 내용을 사용하여 **C:\ADFTutorials\IncCopyMultiTableTutorial** 폴더에 **SqlServerLinkedService.json**이라는 JSON 파일을 만듭니다. Microsoft SQL Server에 연결하는 데 사용하는 **인증**에 기초하여 올바른 선택을 합니다. 로컬 폴더가 아직 없으면 만듭니다. 
...
2. **Azure PowerShell**에서 **C:\ADFTutorials\IncCopyMultiTableTutorial** 폴더로 전환합니다.
...
1. **C:\ADFTutorials\IncCopyMultiTableTutorial** 폴더에 다음 내용이 포함된 **AzureSQLDatabaseLinkedService.json**이라는 JSON 파일을 만듭니다. 아직 없는 경우 ADF 폴더를 만듭니다 파일을 저장하기 전에 **&lt;server&gt; &lt;database name&gt;, &lt;user id&gt; 및 &lt;password&gt;**를 Azure SQL 서버 이름, 데이터베이스 이름, 사용자 ID 및 암호로 바꿉니다. 
...
이 단계에서는 데이터 원본, 데이터 대상을 나타내는 데이터 집합을 만듭니다. 그리고 워터마크를 저장할 곳을 만듭니다.
...
1. 동일한 폴더에 다음 내용이 포함된 **SinkDataset.json**이라는 JSON 파일을 만듭니다. tableName은 런타임 시 파이프라인에 의해 동적으로 설정됩니다. 파이프라인의 ForEach 작업은 테이블 이름 목록을 반복하고 각 반복에서 테이블 이름을 이 데이터 집합에 전달합니다. 
...
1. **조회 작업**을 사용하여 이전 워터마크 값을 검색합니다(초기 값 또는 마지막 반복에서 사용된 값).
2. **조회 작업**을 사용하여 새로운 워터마크 값을 검색합니다(원본 테이블의 워터마크 열의 최대값).
3. **복사 작업**을 사용하여 원본 데이터베이스에서 대상 데이터베이스로 2개 워터마크 값 사이에 데이터를 복사합니다. 
4. **저장 프로시저 작업**을 사용하여 다음 반복의 첫 번째 단계에 사용할 이전 워터마크 값을 업데이트합니다. 
...
1. 동일한 폴더에 다음 내용이 포함된 **Parameters.json**이라는 매개 변수 파일을 만듭니다.
    ...
    ```
2. **Invoke-AzureRmDataFactoryV2Pipeline** cmdlet을 사용하여 **IncrementalCopyPipeline** 파이프라인을 실행합니다. 자리 표시자를 사용자 고유의 리소스 그룹 및 데이터 팩터리 이름으로 바꿉니다.
...
6. **작업** 열에 있는 링크를 클릭하면 파이프라인의 모든 **작업 실행**을 보여주는 다음 페이지가 표시됩니다. 
    ...
    ![작업 실행](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-activity-runs-5.png)
7. **파이프라인 실행** 보기로 다시 전환하려면 다음 이미지와 같이 **파이프라인**을 클릭합니다. 

## <a name="review-the-results"></a>결과 검토
SQL Server Management Studio에서 대상 Azure SQL 데이터베이스에 대해 다음 쿼리를 실행하여 데이터가 원본 테이블에서 대상 테이블로 복사되었는지 확인합니다. 
...
원본 SQL Server 데이터베이스에 대해 다음 쿼리를 실행하여 customer_table의 기존 행을 업데이트하고 project_table에 새 행을 삽입합니다. 
...
2. [파이프라인 모니터링](#monitor-the-pipeline) 섹션의 지침에 따라 파이프라인 실행을 모니터링합니다. 파이프라인 상태가 **진행 중**이면 **작업** 아래 파이프라인 실행을 취소하는 다른 작업 링크가 표시됩니다. 
    ...
    ![파이프라인 실행](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-6.png)    
3. **새로 고침**을 클릭하여 파이프라인 실행이 성공할 때까지 목록을 새로 고칩니다. 

    ![파이프라인 실행](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-succeded-7.png)
4. (선택 사항) 작업 아래 **View Activity Runs**(작업 실행 보기) 링크(아이콘)을 클릭하여 이 파이프라인 실행과 연결된 모든 작업 실행을 표시합니다. 

## <a name="review-final-results"></a>최종 결과를 검토합니다.
...
PersonID: 3에 대한 Name과 LastModifytime의 새 값을 확인합니다. 
...
>[변경 내용 추적 기술을 사용하여 Azure SQL Database에서 Azure Blob Storage로 데이터 증분 로드](tutorial-incremental-copy-multiple-tables-powershell.md)
