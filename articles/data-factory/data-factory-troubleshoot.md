...
3. Select-azuresubscription <subscription name> - 올바른 구독을 선택합니다. Azure Preview 포털에서 데이터 팩터리를 만드는 데 사용한 것과 동일한 구독을 사용합니다.

## 문제: Azure 포털에서 데이터 게이트웨이 빠른 설치를 시작하지 못함
...
## 문제: Azure 포털에서 자격 증명 관리자를 시작하지 못함
Azure 포털을 통해 SQL Server 연결된 서비스를 설치 또는 업데이트하는 경우 보안을 보장하기 위해 자격 증명 관리자 응용 프로그램이 시작됩니다. Internet Explorer 또는 Microsoft ClickOnce 호환 웹 브라우저가 필요합니다. 다른 브라우저에서 실패하는 경우 Internet Explorer로 전환할 수 있습니다.
...
2. SQL Server 인스턴스에 연결을 시도합니다. 이때 Azure 포털에서 SSMS(SQL Server Management Studio)를 사용하여 지정한 자격 증명을 사용합니다.
...
1. 게이트웨이가 설치된 컴퓨터에서 데이터 관리 게이트웨이 구성 관리자를 시작합니다. **게이트웨이 이름**이 **Azure 포털**의 논리 게이트웨이 이름으로 설정되어 있고 **게이트웨이 키 상태**가 **등록됨**이고 **서비스 상태**가 **시작됨**인지 확인합니다. 
...
사용자 지정 작업이 실행될 때 Azure 데이터 팩터리는 HDInsight 클러스터에서 해당 출력을 캡처하고 Azure Blob 저장소 계정의 *adfjobs* 저장소 컨테이너에 저장합니다. 오류 발생 시 **stderr** 출력 텍스트 파일에서 텍스트를 읽을 수 있습니다. 웹 브라우저를 통해 Azure 포털 자체에서 파일을 액세스하고 읽을 수 있습니다. 또는 저장소 탐색기 도구를 사용하여 Azure Blob 저장소의 저장소 컨테이너에 저장된 파일에 액세스합니다.

특정 사용자 지정 작업에 대한 로그를 열거하고 읽으려면 이 페이지의 뒷부분에 설명된 연습 중 하나를 따를 수 있습니다. 요약하면 다음과 같습니다:

1.  Azure 포털에서 **찾아보기**를 통해 데이터 팩터리를 찾습니다.
