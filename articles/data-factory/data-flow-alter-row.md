...
Alter Row 변환을 만들고 조건이 `true()`인 행 정책을 지정 합니다. 이전에 정의 된 식 중 하나를 충족 하지 않는 각 행은 지정 된 행 정책에 대해 표시 됩니다. 기본적으로 조건 식을 충족 하지 않는 각 행은에 대해 `Insert`표시 됩니다.
