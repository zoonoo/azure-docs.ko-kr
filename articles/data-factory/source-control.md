...
-   **부분 저장**. 공장에서 많은 변경을 수행 하는 경우 일반 라이브 모드에서 변경 내용을 초안으로 저장할 수 없습니다. 준비 되지 않았거나 컴퓨터의 작동이 중단 되는 경우 변경 내용을 잃지 않으려는 것입니다. Git 통합을 사용하면 증분 변경 내용을 계속 저장하고, 준비가 되었을 때만 팩터리에 게시할 수 있습니다. Git은 사용자가 변경 내용이 만족스러운지 테스트할 때까지 작업의 준비 위치로 사용됩니다.
...
-   **차이 표시**. Git 모드에서는 팩터리에 게시 될 페이로드의 적절 한 차이를 확인할 수 있습니다. 이러한 차이는 팩터리에 마지막으로 게시된 이후 수정/추가/삭제된 모든 리소스/엔터티를 보여 줍니다. 이 차이에 따라 향후 게시를 계속하거나 변경 내용을 확인한 후 다시 돌아올 수 있습니다.
...
-   **성능 향상**. 평균 팩터리는 git을 통해 리소스를 다운로드 하기 때문에 Git 모드에서 일반 라이브 모드 보다 10 배 더 빠르게 로드 됩니다.
