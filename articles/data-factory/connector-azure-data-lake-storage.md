...
>[!NOTE]
>작성을 위해 Data Factory UI를 사용 하 고 서비스 주체가 IAM의 "저장소 Blob 데이터 판독기/참가자" 역할로 설정 되지 않은 경우 연결 테스트 또는 폴더 탐색을 수행 하는 경우 "파일 경로에 대 한 연결 테스트" 또는 "지정 된 경로에서 찾아보기"를 선택 하 고 실행 권한이 있는 파일 시스템 또는 경로를 지정 하 여 계속 합니다.
...
>[!NOTE]
>Data Factory UI를 사용 하 여 작성 하 고 관리 되는 id를 IAM의 "저장소 Blob 데이터 판독기/참가자" 역할로 설정 하지 않은 경우 연결 테스트 또는 폴더 탐색을 수행 하는 경우 "파일 경로에 대 한 연결 테스트" 또는 "지정 된 경로에서 찾아보기"를 선택 하 고 실행 권한이 있는 파일 시스템 또는 경로를 지정 하 여 계속 합니다.
...
| fileSystem | Data Lake Storage Gen2 파일 시스템 이름입니다.                              | 아닙니다.       |
...
이진 복사 옵션이 나 이진 형식을 사용 하 여 Data Lake Storage Gen2를 이진 복사 옵션 또는 이진 형식으로 Data Lake Storage Gen1로 구성 하는 경우 **데이터 복사 도구 설정** 페이지 또는 활동 제작을 위한 **복사 작업** > **설정** 탭에서 **보존** 옵션을 찾을 수 있습니다.
