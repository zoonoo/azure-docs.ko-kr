...
linkedServiceName | 활동에서 사용하는 연결된 서비스의 이름입니다. <p>활동은 필요한 계산 환경에 연결되는 연결되는 연결된 서비스를 지정할 필요가 있습니다.</p>| HDInsight 활동 및 Azure 기계 학습 일괄 처리 상태 평가 활동에 예<p>다른 모든 사용자에 아니요</p>
typeProperties | typeProperties 구역의 속성은 활동의 형식에 따라 달라집니다. 이에 대해 자세히 알아보려면 각 개별 활동에 대한 문서를 참조하십시오 | 없음
정책이 | 활동의 런타임 동작에 영향을 주는 정책입니다. 지정하지 않으면 기본 정책이 사용됩니다. 자세한 내용은 아래로 스크롤합니다 | 없음
시작 | 파이프라인의 시작 날짜-시간입니다. [ISO 형식](http://en.wikipedia.org/wiki/ISO_8601)에 있어야 합니다. 예: 2014-10-14T16:32:41Z. 시작 및 끝 속성은 함께 파이프라인의 활성 기간을 지정합니다. 활성 기간에서 출력 조각을 생성합니다. | 아니요<p>AzureDataFactoryPipelineActivePeriod cmdlet을 사용하여 파이프라인의 활성 기간을 설정할 수도 있습니다</p>
끝 | 파이프라인의 종료 날짜 및 시간입니다. 지정된 경우 ISO 형식에 있어야 합니다. 예: 2014-10-14T17:32:41Z<p>지정하지 않으면 "시작 + 48시간"으로 계산됩니다. 파이프라인을 무기한으로 실행하려면 9999-09-09을 끝 속성의 값으로 지정합니다.</p>| 아님
isPaused가 | true로 설정하는 경우 파이프라인이 실행되지 않습니다. 기본값 = false입니다. 속성을 활성화 또는 비활성화하여 사용할 수 있습니다. | 아님
