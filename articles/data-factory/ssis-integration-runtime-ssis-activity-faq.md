...
This error occurs when package execution can't find a file in the local disk in the SSIS integration runtime. 다음과 같이 작업해 보세요.
...
If many packages are running in parallel in the SSIS integration runtime, this error might occur because SSISDB has hit its request limit. Consider increasing the DTC of SSISDB to resolve this issue. 자세한 내용은 [Azure SQL Database 서버의 SQL Database 리소스 한도](https://docs.microsoft.com/azure/sql-database/sql-database-resource-limits-logical-server)를 참조하세요.
