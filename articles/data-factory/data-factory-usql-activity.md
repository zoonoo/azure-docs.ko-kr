...
> [AZURE.NOTE] 
Data Lake Analytics U-SQL 작업이 포함된 파이프라인을 만들기 전에 Azure Data Lake Analytics 계정을 만들어야 합니다. Azure Data Lake Analytics에 대해 알아보려면 [Azure Data Lake Analytics 시작](../data-lake-analytics/data-lake-analytics-get-started-portal.md)을 참조하세요.
>  
...
## Azure Data Lake Analytics 연결된 서비스
파이프라인에서 Data Lake Analytics U-SQL 작업을 사용하기 전에 **Azure Data Lake Analytics** 연결된 서비스를 만들어 Azure Data Lake Analytics 계산 서비스를 Azure Data Factory에 연결합니다.

다음 예제에서는 Azure Data Lake Analytics 연결된 서비스에 JSON 정의를 제공합니다.
...
다음은 위의 입/출력 데이터 집합에서 사용되는 Azure Data Lake Store 연결된 서비스 샘플의 정의입니다.
...
위의 Azure Data Lake Store 연결된 서비스 및 데이터 집합 JSON 조각의 JSON 속성에 대한 설명을 보려면 [Azure Data Lake Store에서 데이터 이동](data-factory-azure-datalake-connector.md)을 참조하세요.
...
Azure Data Lake Analytics 서비스에서 실행되는 작업에 대한 파이프라인 정의뿐 아니라 다른 속성 viz. degreeOfParallelism, 우선 순위 등을 지정할 수 있습니다.

## 동적 매개 변수
위의 샘플 파이프라인 정의에서 in 및 out 매개 변수는 하드 코드된 값으로 할당됩니다.
...
이 경우 입력 파일은 여전히 /datalake/input 폴더에서 가져오며 출력 파일은 /datalake/output 폴더에 생성되지만, 파일 이름은 조각 시작 시간에 따라 동적입니다.
