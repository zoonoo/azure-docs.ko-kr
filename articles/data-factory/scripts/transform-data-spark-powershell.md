---
title: 'PowerShell 스크립트-Data Factory를 사용 하 여 클라우드의 데이터 변환 '
...
2. Blob 컨테이너의 `inputfiles` 폴더에 `spark`이라는 하위 폴더를 만듭니다. 
3. `minecraftstory.txt` 하위 폴더에 `inputfiles`를 업로드합니다. 
...
| [AzDataFactoryV2Pipeline](/powershell/module/az.datafactory/set-Azdatafactoryv2pipeline) | 데이터 팩터리에서 파이프라인을 만듭니다. 파이프라인에는 특정 작업을 수행하는 하나 이상의 작업이 있습니다. 이 파이프라인에서 Spark 작업은 Azure HDInsight Spark 클러스터에서 프로그램을 실행하여 데이터를 변환합니다. |
