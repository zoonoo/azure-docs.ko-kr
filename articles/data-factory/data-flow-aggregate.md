...
* @No__t-0 또는 `first()`과 같은 집계 함수를 사용 하 여 추가 열을 포함 합니다.
...
아래 예제에서는-0 @no__t 들어오는 스트림을 사용 하 고-1 @no__t 열을 기준으로 행을 그룹화 합니다. 이 변환은 열 `Rating`의 평균으로 계산 되는-0 @no__t 집계 열을 만듭니다. 이 집계 변환의 이름은 `AvgComedyRatingsByYear`입니다. Data Factory UX에서이 변환은 아래 이미지와 같습니다.
