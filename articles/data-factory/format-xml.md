...
| preserveZipFileNameAsFolder<br>(*아래 `compressionProperties` *) | **ZipDeflate** 압축을 사용 하 여 입력 데이터 집합을 구성할 때 적용 됩니다. 원본 zip 파일 이름을 복사 중에 폴더 구조로 유지할지 여부를 나타냅니다. True (기본값)로 설정 하면 Data Factory 압축을 푼 파일을 씁니다. `<path specified in dataset>/<folder named as source zip file>/` false로 설정 하면 압축을 푼 파일을에 직접 쓸 Data Factory `<path specified in dataset>` .  | 예 |
...
| 마지막으로 수정한 사람 필터링 | 마지막으로 변경 된 시간에 따라 파일을 필터링 하도록 선택 | 예 | Timestamp | modifiedAfter <br>modifiedBefore |
...
- XML 특성:
    ...
    - 요소의 특성은 계층 구조에 있는 요소의 하위 필드로 구문 분석 됩니다.
