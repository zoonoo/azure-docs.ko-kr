...
3. 이제 실패 한 행을 기록해 야 합니다. 로깅에 대 한 ```BadRows``` 스트림에 싱크 변환을 추가 합니다. 여기에서는 전체 트랜잭션 레코드의 로깅을 사용할 수 있도록 모든 필드를 "자동 매핑" 합니다. Blob Storage의 단일 파일에 대 한 텍스트로 구분 된 CSV 파일 출력입니다. 로그 파일 "badrows. csv"를 호출 합니다.
