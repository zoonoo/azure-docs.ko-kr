...
**Azure Machine Learning Studio (클래식)** 연결 된 서비스를 만들어 Azure Machine Learning Studio (클래식) 웹 서비스를 Azure data factory에 연결 합니다. 연결 된 서비스는 Azure Machine Learning Studio (클래식) 일괄 처리 실행 작업 및 [리소스 업데이트 작업](update-machine-learning-models.md)에 사용 됩니다.
...
다음 JSON 코드 조각은 Azure Machine Learning Studio (클래식) 일괄 처리 실행 작업을 정의 합니다. 활동 정의에는 이전에 만든 Azure Machine Learning Studio (클래식) 연결 된 서비스에 대 한 참조가 있습니다.
...
이 시나리오에서 Azure Machine Learning Studio (클래식) 웹 서비스는 Azure blob 저장소에 있는 파일의 데이터를 사용 하 여 예측을 수행 하 고 예측 결과를 blob 저장소에 저장 합니다. 다음 JSON은 AzureMLBatchExecution 작업이 포함된 Data Factory 파이프라인을 정의합니다. Azure 블로그 스토리지의 입력 및 출력 데이터는 LinkedName 및 FilePath 쌍을 사용하여 참조됩니다. 입력 및 출력의 연결 된 서비스 샘플에서 Data Factory의 각 입력/출력에 대해 서로 다른 연결 된 서비스를 사용 하 여 올바른 파일을 선택 하 고 Azure Machine Learning Studio (클래식) 웹 서비스로 보낼 수 있습니다.
