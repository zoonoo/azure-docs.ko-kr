...
**와일드 카드 패턴** 옵션을 사용 하면 ADF가 일치 하는 각 폴더와 파일을 단일 원본 변환에서 반복 하도록 지시할 수 있습니다. 이 방법은 단일 흐름 내에서 여러 파일을 처리 하는 매우 효과적인 방법입니다. 현재 처리 중인 파일 이름을 추적 하려면 원본 옵션에서 "저장소 파일 이름 열" 필드의 필드 이름을 설정 합니다.

> [!NOTE]
> 와일드 카드 규칙을 추가 하려면 기존 와일드 카드 패턴 옆에 있는 + 기호를 사용 하 여 와일드 카드 일치 패턴을 여러 개 설정 합니다.
...
* **파티션 루트 경로**: 파일 원본 ```key=value``` 에 분할 된 폴더 (예: year = 2019)가 있는 경우 ADF에 해당 파티션 폴더 트리의 최상위 수준을 데이터 흐름 데이터 스트림의 열 이름에 할당 하도록 요청할 수 있습니다.

먼저 분할 된 폴더와 읽으려는 리프 파일의 모든 경로를 포함 하도록 와일드 카드를 설정 합니다.

![파티션 원본 파일 설정](media/data-flow/partfile2.png "파티션 파일 설정")

이제 파티션 루트 경로 설정을 사용 하 여 ADF에 폴더 구조의 최상위 수준을 알려 줍니다. 이제 데이터의 콘텐츠를 볼 때 ADF가 각 폴더 수준에서 발견 된 확인 된 파티션을 추가 하는 것을 확인할 수 있습니다.
...
이 경우/cv/ps 아래에서 소스인 모든 파일은/backup/priorSales.로 이동 됩니다.
...
설정 패널에서 필드 내부를 클릭 하면 "동적 콘텐츠 추가"에 대 한 하이퍼링크가 표시 됩니다. 식 작성기를 시작 하도록 선택 하는 경우 식, 정적 리터럴 값 또는 매개 변수를 사용 하 여 동적으로 값을 설정 합니다.
