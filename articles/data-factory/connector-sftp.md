...
이 문서에서는 SFTP 서버에서 데이터를 복사 하는 방법을 설명 합니다. Azure Data Factory에 대해 자세히 알아보려면 [소개 문서](introduction.md)를 참조하세요.

## <a name="supported-capabilities"></a>지원되는 기능

이 SFTP 커넥터는 다음과 같은 작업에 대해 지원 됩니다.
...
- **Basic** 또는 **SshPublicKey** 인증을 사용 하 여 SFTP에서 파일을 복사 합니다.
...
| skipHostKeyValidation | 호스트 키 유효성 검사를 건너뛸지 여부를 지정합니다.<br/>허용되는 값은 **true**, **false**(기본값)입니다.  | 예 |
| hostKeyFingerprint | 호스트 키의 지문을 지정합니다. | “skipHostKeyValidation”이 false로 설정된 경우 예  |
| authenticationType | 인증 유형을 지정합니다.<br/>허용되는 값은 **Basic** 및 **SshPublicKey**입니다. 더 많은 속성 및 각 속성의 JSON 샘플은 [기본 인증 사용](#using-basic-authentication) 및 [SSH 공개 키 인증 사용](#using-ssh-public-key-authentication) 섹션을 참조하세요. |yes |
...
| password | 사용자(userName) 암호입니다. 이 필드를 SecureString으로 표시하여 Data Factory에 안전하게 저장하거나 [Azure Key Vault에 저장되는 비밀을 참조](store-credentials-in-key-vault.md)합니다. | yes |
...
서식 기반 데이터 집합의 `location` 설정에서 SFTP에 대해 지원 되는 속성은 다음과 같습니다.

| 속성   | Description                                                  | 필수 |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | 데이터 집합의 `location`에 있는 type 속성은 **SftpLocation**로 설정 해야 합니다. | yes      |
| folderPath | 폴더에 대 한 경로입니다. 와일드 카드를 사용 하 여 폴더를 필터링 하려면이 설정을 건너뛰고 작업 원본 설정에서를 지정 합니다. | 예       |
| fileName   | 지정 된 folderPath의 파일 이름입니다. 와일드 카드를 사용 하 여 파일을 필터링 하려는 경우이 설정을 건너뛰고 작업 원본 설정에서를 지정 합니다. | 예       |
...
다음 속성은 형식 기반 복사 원본의 `storeSettings` 설정에서 SFTP에 대해 지원 됩니다.

| 속성                 | Description                                                  | 필수                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | `storeSettings` 아래의 type 속성은 **Sftpreadsettings**로 설정 되어야 합니다. | yes                                           |
| recursive                | 하위 폴더 또는 지정된 폴더에서만 데이터를 재귀적으로 읽을지 여부를 나타냅니다. recursive를 true로 설정하고 싱크가 파일 기반 저장소인 경우 빈 폴더 또는 하위 폴더가 싱크에 복사되거나 만들어지지 않습니다. 허용되는 값은 **true**(기본값) 및 **false**입니다. | 예                                            |
| wildcardFolderPath       | 원본 폴더를 필터링 할 와일드 카드 문자가 포함 된 폴더 경로입니다. <br>허용되는 와일드카드는 `*`(0개 이상의 문자 일치) 및 `?`(0-1개의 문자 일치)입니다. 실제 폴더 이름에 와일드카드 또는 이 이스케이프 문자가 있는 경우 `^`을 사용하여 이스케이프합니다. <br>더 많은 예는 [폴더 및 파일 필터 예제](#folder-and-file-filter-examples)를 참조하세요. | 예                                            |
...
### <a name="sftp-as-sink"></a>SFTP를 싱크로

[!INCLUDE [data-factory-v2-file-formats](../../includes/data-factory-v2-file-formats.md)] 

형식 기반 복사 싱크의 `storeSettings` 설정에서 SFTP에 대해 지원 되는 속성은 다음과 같습니다.

| 속성                 | Description                                                  | 필수 |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | `storeSettings` 아래의 type 속성을 **Sftpwritesettings**로 설정 해야 합니다. | yes      |
...
| useTempFileRename | 임시 파일에 업로드 하 고 이름을 바꾸거나 대상 폴더/파일 위치에 직접 쓸지 여부를 나타냅니다. 기본적으로 ADF는 먼저 임시 파일에 쓴 다음 업로드 완료 시 파일 이름 바꾸기를 수행 하 여 (1), 같은 파일에 쓰기 작업을 수행 하는 경우 충돌 쓰기를 방지 하 고 파일의 원래 버전이 존재 하는지 확인 합니다. 전체 전송. SFTP 서버에서 이름 바꾸기 작업을 지원 하지 않는 경우이 옵션을 사용 하지 않도록 설정 하 고 대상 파일에 동시 쓰기가 수행 되지 않도록 합니다. 이 표 아래의 문제 해결 팁을 참조 하세요. | 아니요. 기본값은 true입니다. |
| operationTimeout | SFTP 서버에 대 한 각 쓰기 요청이 시간 초과 되기 전의 대기 시간입니다. 기본값은 60 분 (01:00:00)입니다.|예 |

>[!TIP]
>SFTP에 데이터를 쓸 때 "UserErrorSftpPathNotFound", "UserErrorSftpPermissionDenied" 또는 "SftpOperationFail" 오류가 발생 하 고 사용 하는 SFTP 사용자에 게 적절 한 권한이 있는 경우 SFTP 서버에서 파일 이름 바꾸기 작업을 지원 하는지 확인 합니다. 그렇지 않은 경우 "임시 파일을 사용 하 여 업로드" (`useTempFileRename`) 옵션을 사용 하지 않도록 설정 하 고 다시 시도 하세요. 위의 표에서이 속성에 대해 자세히 알아보세요. 자체 호스팅 Integration Runtime를 복사에 사용 하는 경우 버전 4.6 이상을 사용 해야 합니다.
