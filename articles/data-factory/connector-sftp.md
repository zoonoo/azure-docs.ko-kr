...
description: Azure Data Factory를 사용하여 SFTP 서버 간에 데이터를 복사하는 방법에 대해 알아봅니다.
...
ms.locfileid: "83653747"
---
# <a name="copy-data-from-and-to-sftp-server-using-azure-data-factory"></a>Azure Data Factory를 사용하여 SFTP 서버에서의 데이터 복사
...
이 문서에서는 SFTP 서버 간에 데이터를 복사하는 방법을 설명합니다. Azure Data Factory에 대해 자세히 알아보려면 [소개 문서](introduction.md)를 참조하세요.

## <a name="supported-capabilities"></a>지원되는 기능

이 SFTP 커넥터는 다음과 같은 작업에 지원됩니다.
...
- **Basic** 또는 **SshPublicKey** 인증을 사용한 SFTP에서/SFTP로 파일 복사
...
| authenticationType | 인증 유형을 지정합니다.<br/>허용된 값은 **Basic**, **SshPublicKey**. 더 많은 속성 및 각 속성의 JSON 샘플은 [기본 인증 사용](#using-basic-authentication) 및 [SSH 공개 키 인증 사용](#using-ssh-public-key-authentication) 섹션을 참조하세요. |예 |
...
| 속성   | Description                                                  | 필수 |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | 데이터 세트의 `location` 아래 type 속성은 **SftpLocation**으로 설정되어야 합니다. | 예      |
...
| 속성                 | Description                                                  | 필수                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | `storeSettings` 아래의 type 속성은 **SftpReadSettings**로 설정되어야 합니다. | 예                                           |
| ***복사할 파일 찾기:*** |  |  |
...
### <a name="sftp-as-sink"></a>싱크로서의 SFTP
...
| 속성                 | Description                                                  | 필수 |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | `storeSettings` 아래의 type 속성은 **SftpWriteSettings**로 설정되어야 합니다. | 예      |
...
| useTempFileRename | 임시 파일에 업로드하고 이름을 바꾸거나 대상 폴더/파일 위치에 직접 쓸지 여부를 나타냅니다. 기본적으로 ADF는 먼저 임시 파일에 쓴 다음 업로드 완료 시 파일 이름 바꾸기를 수행합니다. 이는 1) 동일한 파일에 다른 프로세스가 쓰기 작업을 수행하는 경우 충돌 쓰기를 방지하고 2) 파일의 원래 버전이 전체 전송 중에 존재하는지 확인하기 위함입니다. SFTP 서버에서 이름 바꾸기 작업을 지원하지 않는 경우 이 옵션을 사용하지 않도록 설정하고 대상 파일에 동시 쓰기가 수행되지 않도록 합니다. 이 표 아래의 문제 해결 팁을 참조하세요. | 아니요. 기본값은 true입니다. |
| operationTimeout | SFTP 서버에 대한 각각의 쓰기 요청이 시간 초과되기까지의 대기 시간입니다. 기본값은60분(01:00:00)입니다.|예 |

>[!TIP]
>SFTP에 데이터를 쓸 때 "UserErrorSftpPathNotFound", "UserErrorSftpPermissionDenied" 또는 "SftpOperationFail" 오류가 발생하고 사용하는 SFTP 사용자에게 적절한 권한이 있는 경우, SFTP 서버에서 파일 이름 바꾸기 작업을 지원하는지 확인합니다. 그렇지 않은 경우 "임시 파일을 사용하여 업로드"(`useTempFileRename`) 옵션을 사용하지 않도록 설정하고 다시 시도합니다. 위의 표에서 이 속성에 대해 자세히 알아보세요. 자체 호스팅 통합 런타임을 복사에 사용하는 경우 버전 4.6 이상을 사용해야 합니다.
