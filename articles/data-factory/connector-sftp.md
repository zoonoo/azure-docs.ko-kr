...
- **기본**, **SSH 공개 키** 또는 **multi-factor** authentication을 사용 하 여 SFTP 서버에서 파일을 복사 합니다.
...
| authenticationType | 인증 유형을 지정합니다.<br/>허용 되는 값은 *Basic*, *SshPublicKey* 및 *다단계* 입니다. 자세한 속성은 [기본 인증 사용](#use-basic-authentication) 섹션을 참조 하세요. JSON 예제는 [SSH 공개 키 인증 사용](#use-ssh-public-key-authentication) 섹션을 참조 하세요. |예 |
...
기본 및 SSH 공개 키 인증의 조합 인 multi-factor authentication을 사용 하려면 위의 섹션에서 설명 하는 사용자 이름, 암호 및 개인 키 정보를 지정 합니다.

**예: multi-factor authentication**
...
| 옵션 2: 와일드카드<br>- wildcardFileName | 소스 파일을 필터링 하기 위해 지정 된 folderPath/wildcardFolderPath 아래의 와일드 카드 문자가 포함 된 파일 이름입니다. <br>허용 되는 와일드 카드는 `*` (0 개 이상의 문자와 일치) 및 (0 개 이상의 문자 `?` 일치)입니다. `^` 실제 파일 이름에 와일드 카드 또는 내부 이스케이프 문자가 있으면를 사용 하 여를 이스케이프 합니다.  더 많은 예제는 [폴더 및 파일 필터 예](#folder-and-file-filter-examples)를 참조 하세요. | 예 |
