...
이 커넥터와 [웹 테이블 커넥터](connector-web-table.md) 간 차이점은 웹 테이블 커넥터가 HTML 페이지에서 테이블 콘텐츠를 추출한다는 것입니다.
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | **type** 속성은 **HttpServer**로 설정해야 합니다. | yes |
...
| 암호 | 사용자(**userName** 값)의 암호입니다. 이 필드를 **SecureString** 형식으로 표시하여 Data Factory에서 안전하게 저장합니다. [Azure Key Vault에 저장된 비밀을 참조](store-credentials-in-key-vault.md)할 수도 있습니다. | yes |
...
| 암호 | 인증서와 연결된 암호입니다. 이 필드를 **SecureString** 형식으로 표시하여 Data Factory에서 안전하게 저장합니다. [Azure Key Vault에 저장된 비밀을 참조](store-credentials-in-key-vault.md)할 수도 있습니다. | 아니요 |
...
이 섹션에서는 HTTP 데이터 집합에서 지원하는 속성의 목록을 제공합니다. 

데이터 집합 정의에 사용할 수 있는 섹션 및 속성의 전체 목록은 [데이터 집합 및 연결된 서비스](concepts-datasets-linked-services.md)를 참조하세요. 

HTTP에서 데이터를 복사하려면 데이터 집합의 **type** 속성을 **HttpFile**로 설정합니다. 다음과 같은 속성이 지원됩니다.

| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 데이터 집합의 **type** 속성을 **HttpFile**로 설정해야 합니다. | yes |
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 복사 작업 원본의 **type** 속성을 **HttpSource**로 설정해야 합니다. | yes |
