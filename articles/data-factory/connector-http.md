...
ms.locfileid: "43091720"
---
# <a name="copy-data-from-http-endpoint-using-azure-data-factory"></a>Azure Data Factory를 사용하여 HTTP 엔드포인트에서 데이터 복사
> [!div class="op_single_selector" title1="Select the version of Data Factory service you are using:"]
> * [버전 1](v1/data-factory-http-connector.md)
> * [현재 버전](connector-http.md)

이 문서에서는 Azure Data Factory의 복사 작업을 사용하여 HTTP 엔드포인트에서 데이터를 복사하는 방법을 간략히 설명합니다. 이 문서는 복사 작업에 대한 일반적인 개요를 제공하는 [복사 작업 개요](copy-activity-overview.md) 문서를 기반으로 합니다.
...
- HTTP **GET** 또는 **POST** 메서드를 사용하여 HTTP 엔드포인트에서 데이터를 검색
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 형식 속성은 **HttpServer**로 설정해야 합니다. | yes |
| URL | 웹 서버에 대한 기본 URL입니다. | yes |
| enableServerCertificateValidation | HTTP 엔드포인트로 연결할 때 서버 SSL 인증서 유효성 검사를 사용할지 지정합니다. HTTPS 서버에서 자체 서명된 인증서를 사용하는 경우 false로 설정합니다. | 아니요. 기본값은 True입니다. |
| authenticationType | 인증 유형을 지정합니다. 허용되는 값: **Anonymous**, **Basic**, **Digest**, **Windows**, **ClientCertificate**. <br><br> 각 인증 형식에 대한 더 많은 속성 및 JSON 샘플은 표 아래 섹션을 참조하세요. | yes |
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| userName | HTTP 엔드포인트에 액세스하는 사용자 이름입니다. | yes |
| 암호 | 사용자(userName) 암호입니다. 이 필드를 SecureString으로 표시하여 Data Factory에 안전하게 저장하거나 [Azure Key Vault에 저장되는 비밀을 참조](store-credentials-in-key-vault.md)합니다. | yes |
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 데이터 집합의 형식 속성을 **HttpFile**로 설정해야 합니다. | yes |
...
>[!NOTE]
>지원되는 HTTP 요청 페이로드 크기는 약 500KB입니다. 웹 엔드포인트로 전달하려는 페이로드 크기가 이것보다 큰 경우 더 작은 청크로 일괄 처리하는 것이 좋습니다.
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| 형식 | 복사 작업 원본의 형식 속성을 **HttpSource**로 설정해야 합니다. | yes |
