...
* **Azure SQL 데이터베이스**. 데이터베이스를 **싱크** 데이터 저장소로 사용합니다. 아직 없는 경우 Azure SQL Database를 만드는 단계는 [Azure SQL Database 만들기](../sql-database/sql-database-get-started-portal.md) 문서를 참조하세요.
...
2. **파일**을 클릭하고 **새로 만들기**를 가리킨 다음 **프로젝트**를 클릭합니다. .NET 버전 4.5.2 이상이 필요합니다.
...
전자 메일 보내기를 트리거하려면 [Logic Apps](../logic-apps/logic-apps-what-are-logic-apps.md)를 사용하여 워크플로를 정의합니다. 논리 앱 워크플로를 만드는 방법에 대한 자세한 내용은 [논리 앱을 만드는 방법](../logic-apps/logic-apps-create-a-logic-app.md)을 참조하세요. 
...
- receiver - 이메일 주소로 전자 메일을 받는 두 웹 작업에서 사용하는 파이프라인의 매개 변수입니다.
