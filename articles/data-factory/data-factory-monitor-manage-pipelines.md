...
3. 데이터 팩터리 블레이드에서 원하는 데이터 팩터리를 선택하면 데이터 팩터리의 홈 페이지(**데이터 팩터리** 블레이드) 가 표시됩니다.
...
1. 이제 **Get-AzureRmDataFactoryRun** cmdlet을 실행하여 조각의 작업 실행에 대한 세부 정보를 가져옵니다.
...
1. 출력에 오류에 대한 세부 정보가 다음과 같이 표시됩니다.
    ...
    Type                    :
2. 출력에 표시된 ID 값을 사용하여 **Save-AzureRmDataFactoryLog** cmdlet을 실행하고 cmdlet에 **-DownloadLogsoption** 옵션을 사용하여 로그 파일을 다운로드할 수 있습니다.
