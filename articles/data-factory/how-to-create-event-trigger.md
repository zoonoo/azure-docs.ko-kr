...
EDA(이벤트 기반 아키텍처)는 프로덕션, 검색, 소비 및 이벤트에 대한 응답이 포함된 일반적인 데이터 통합 패턴입니다. 데이터 통합 시나리오에서는 Data Factory 고객이 이벤트를 기반으로 파이프라인을 트리거해야 하는 경우가 종종 있습니다.
...
-   **Blob path begins with**(Blob 경로 시작 문자)('/containername/foldername') – containername 컨테이너 및 foldername 폴더에 있는 모든 Blob에 대한 이벤트를 수신합니다.
-   **Blob path begins with**(Blob 경로 시작 문자)('/containername/foldername/file.txt') – containername 컨테이너 아래 foldername 폴더에 있는 file.txt라는 이름의 Blob에 대한 이벤트를 수신합니다.
-   **Blob path ends with**(Blob 경로 마지막 문자)('file.txt') – 모든 경로에서 file.txt라는 이름의 Blob에 대한 이벤트를 수신합니다.
-   **Blob path ends with**(Blob 경로 마지막 문자)('/containername/file.txt') – containername 컨테이너에 있는 file.txt라는 이름의 Blob에 대한 이벤트를 수신합니다.
