...
> * [버전 1 - GA](v1/data-factory-onprem-cassandra-connector.md)
...
> [!NOTE]
> 이 문서는 현재 미리 보기 상태인 Data Factory 버전 2에 적용됩니다. GA(일반 공급) 상태인 Data Factory 버전 1 서비스를 사용 중인 경우 [V1의 Cassandra 커넥터](v1/data-factory-onprem-cassandra-connector.md)를 참조하세요.
...
| password |사용자 계정으로 password를 지정합니다. 이 필드를 SecureString으로 표시합니다. |예. authenticationType은 Basic으로 설정됩니다. |
...
Cassandra에서 데이터를 복사하려면 데이터 집합의 type 속성을 **RelationalTable**로 설정합니다. 다음과 같은 속성이 지원됩니다.

| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 데이터 집합의 type 속성을 **CassandraTable**로 설정해야 합니다. | 예 |
