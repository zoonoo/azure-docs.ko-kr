---
title: Azure Data Factory를 사용 하 여 REST 끝점에서 데이터 복사
description: Azure Data Factory 파이프라인에서 복사 작업을 사용 하 여 클라우드 또는 온-프레미스 REST 원본에서 지원 되는 싱크 데이터 저장소로 또는 지원 되는 원본 데이터 저장소에서 REST 싱크로 데이터를 복사 하는 방법에 대해 알아봅니다.
...
이 문서에서는 Azure Data Factory의 복사 작업을 사용 하 여 REST 끝점에서 데이터를 복사 하는 방법을 설명 합니다. 이 문서는 복사 작업에 대한 일반적인 개요를 제공하는 [Azure Data Factory의 복사 작업](copy-activity-overview.md)을 기반으로 합니다.
...
REST 원본에서 지원되는 모든 싱크 데이터 저장소로 데이터를 복사할 수 있습니다. 지원 되는 모든 원본 데이터 저장소의 데이터를 REST 싱크로 복사할 수도 있습니다. 복사 작업에서 원본 및 싱크로 지원되는 데이터 저장소의 목록은 [지원되는 데이터 저장소 및 형식](copy-activity-overview.md#supported-data-stores-and-formats)을 참조하세요.

특히 이 일반 REST 커넥터는 다음을 지원합니다.

- **Post**, **PUT** 또는 **PATCH** 메서드를 사용 하 여 **GET** 또는 **post** 메서드를 사용 하 고 rest 끝점으로 데이터를 복사 하 여 rest 끝점에서 데이터를 복사 합니다.
- 다음 인증 중 하나를 사용 하 여 데이터를 복사 합니다. **익명**, **기본**, **AAD 서비스 사용자** 및 **Azure 리소스에 대 한 관리 되는 id** 입니다.
- REST API의 **[페이지 매김](#pagination-support)**
- 원본으로 REST의 경우 REST JSON 응답을 있는 [그대로](#export-json-response-as-is) 복사 하거나 [스키마 매핑을](copy-activity-schema-and-type-mapping.md#schema-mapping)사용 하 여 구문 분석 합니다. **JSON** 의 응답 페이로드만 지원됩니다.
...
이 섹션에서는 REST 원본 및 싱크에서 지 원하는 속성의 목록을 제공 합니다.
...
### <a name="rest-as-sink"></a>싱크로 REST
...
- _ *웹** 활동은 전달자 토큰을 검색 한 다음 권한 부여로 후속 복사 활동에 전달 합니다.
...
4. 다음 예제와 같이 생성 된 파이프라인이 표시 됩니다.  ![ 스크린샷은 템플릿에서 만든 파이프라인을 보여 줍니다.](media/solution-template-copy-from-rest-or-http-using-oauth/pipeline.png)
