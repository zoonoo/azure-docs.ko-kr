...
이제 **데이터 집합**은 입력 및 출력으로 **활동**에서 사용하려는 데이터를 단순히 가리키거나 참조하는 데이터의 명명된 뷰입니다. 데이터 세트는 테이블, 파일, 폴더, 문서를 비롯한 다양한 데이터 저장소 내의 데이터를 식별합니다. 예를 들어 Azure Blob 데이터 집합은 활동에서 데이터를 읽어들여야 하는 Blob 저장소의 Blob 컨테이너와 폴더를 지정합니다.
...
structure | 데이터 집합의 스키마입니다. 자세한 내용은 [데이터 집합 구조](#dataset-structure)를 참조하세요. | 아니오 |
