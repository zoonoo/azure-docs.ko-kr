...
| isEspEnabled      | HDInsight 클러스터에서 *Enterprise Security Package*가 사용하도록 설정되어 있으면 '[true](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-architecture)'를 지정합니다. 기본값은 ‘*false*’입니다. | 아니요       |
...
## <a name="azure-batch-linked-service"></a>Azure 배치 연결된 서비스
...
| 테넌트                 | 애플리케이션이 있는 테넌트 정보(도메인 이름 또는 테넌트 ID)를 지정합니다. Azure Portal의 오른쪽 위 모서리에 마우스를 이동하여 검색할 수 있습니다. | UpdateResourceEndpoint가 지정된 경우에 필요합니다. |
| connectVia             | 이 연결된 서비스에 작업을 디스패치하는 데 사용할 통합 런타임입니다. Azure 통합 런타임 또는 자체 호스팅 통합 런타임을 사용할 수 있습니다. 지정하지 않으면 기본 Azure Integration Runtime을 사용합니다. | 아니요                                       |
...
| 속성               | 설명                              | 필수                                 |
| ---------------------- | ---------------------------------------- | ---------------------------------------- |
| 형식                   | Type 속성은 **AzureMLService**로 설정 해야 합니다. | 예                                      |
...
| 테넌트                 | 애플리케이션이 있는 테넌트 정보(도메인 이름 또는 테넌트 ID)를 지정합니다. Azure Portal의 오른쪽 위 모서리에 마우스를 이동하여 검색할 수 있습니다. | UpdateResourceEndpoint가 지정된 경우에 필요합니다. | 아니요 |
| connectVia             | 이 연결된 서비스에 작업을 디스패치하는 데 사용할 통합 런타임입니다. Azure 통합 런타임 또는 자체 호스팅 통합 런타임을 사용할 수 있습니다. 지정하지 않으면 기본 Azure Integration Runtime을 사용합니다. | 아니요 |    
