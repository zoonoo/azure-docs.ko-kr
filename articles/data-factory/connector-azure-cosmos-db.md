---
title: Data Factory를 사용 하 여 Azure Cosmos DB (SQL API) 간 데이터 복사
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 데이터 세트의 **type** 속성을 **DocumentDbCollection**으로 설정해야 합니다. |예 |
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 원본의 **type** 속성을 **DocumentDbCollectionSource**로 설정해야 합니다. |예 |
| 쿼리 |데이터를 읽는 Azure Cosmos DB 쿼리를 지정합니다.<br/><br/>예:<br /> `SELECT c.BusinessEntityID, c.Name.First AS FirstName, c.Name.Middle AS MiddleName, c.Name.Last AS LastName, c.Suffix, c.EmailPromotion FROM c WHERE c.ModifiedDate > \"2009-01-01T00:00:00\"` |아니요 <br/><br/>지정하지 않는 경우 실행되는 SQL 문: `select <columns defined in structure> from mycollection` |
| nestingSeparator |문서가 중첩되고 결과 집합을 평면화하는 방법을 나타내는 특수 문자입니다.<br/><br/>예를 들어, Azure Cosmos DB 쿼리가 중첩된 결과 `"Name": {"First": "John"}`을 반환하는 경우 복사 작업은 `Name.First`nestedSeparator**값이**. **(점)인 경우 “John”이라는 값으로** 로 열 이름을 식별합니다. |아니요<br />(기본값: **.** (점)) |
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 싱크의 **type** 속성을 **DocumentDbCollectionSink**로 설정해야 합니다. |예 |
...
* 작업 작성을 사용하는 경우 Azure Cosmos DB 데이터 세트에서 **structure**(schema라고도 함) 섹션을 지정하지 마세요. 또한 Azure Cosmos DB 원본 또는 복사 작업의 싱크에서 **nestingSeparator** 속성을 지정하지 마세요. JSON 파일에서 가져오거나 파일로 내보낼 때 **JSON 형식** 섹션의 설명대로 해당 파일 저장소 데이터 세트에서 **format** 형식을 **JsonFormat**으로 지정하고 [filePattern](supported-file-formats-and-compression-codecs.md#json-format)을 구성합니다. 그런 다음 **structure** 섹션을 지정하지 않고 나머지 형식 설정을 건너뜁니다.
