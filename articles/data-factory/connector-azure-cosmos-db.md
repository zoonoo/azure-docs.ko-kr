...
>[!NOTE]
>이 커넥터는 SQL API Cosmos DB 지원 합니다. MongoDB API의 경우 [Azure Cosmos DB의 API for MongoDB용 커넥터](connector-azure-cosmos-db-mongodb-api.md)를 참조하세요. 기타 API 형식은 이제 지원되지 않습니다.
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | **type** 속성은 **CosmosDb**로 설정해야 합니다. | yes |
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 원본의 **type** 속성은 **CosmosDbSqlApiSource**로 설정 해야 합니다. |yes |
...
| preferredRegions | Cosmos DB에서 데이터를 검색 때 연결할 지역의 기본 설정 목록입니다. | 아닙니다. |
| pageSize | 쿼리 결과의 페이지당 문서 수입니다. 기본값은 "-1" 이며,이는 서비스 쪽 동적 페이지 크기를 최대 1000까지 사용 함을 의미 합니다. | 아닙니다. |
...
| 자산 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 싱크의 **type** 속성은 **CosmosDbSqlApiSink**로 설정 해야 합니다. |yes |
