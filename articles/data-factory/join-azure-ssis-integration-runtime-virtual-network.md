...
SSIS 패키지가 온-프레미스 데이터 저장소에 액세스하는 경우 Azure-SSIS 통합 런타임을 온-프레미스 네트워크에 연결된 VNet에 조인하십시오. 다음은 몇 가지 유의할 사항입니다. 
...
[이 문서](create-azure-ssis-integration-runtime.md)에 설명된 PowerShell 스크립트를 사용하여 VNet에 Azure-SSIS 통합 런타임을 프로비전할 수 있습니다. 이 스크립트는 Azure-SSIS 통합 런타임을 VNet에 조인할 수 있도록 VNet 권한 및 설정을 자동으로 구성합니다.  
...
7. **MicrosoftAzureBatch**를 VNet의 **클래식 가상 컴퓨터 참여자** 역할에 조인합니다. 
    ...
    2. **권한 추가** 페이지에서 **역할**에 **클래식 가상 컴퓨터 기여자**를 선택합니다. **선택** 텍스트 상자에 **MicrosoftAzureBatch**를 입력한 다음 검색 결과 목록에서 **MicrosoftAzureBatch**를 선택합니다. 
