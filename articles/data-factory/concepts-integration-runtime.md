...
통합 런타임은 [관리 허브](author-management-hub.md) 및 이를 참조하는 모든 활동, 데이터 세트 또는 데이터 흐름을 사용하여 Azure Data Factory UX에서 만들 수 있습니다.
...
- 공용 네트워크에서 Databricks Notebook/Jar/Python 작업, HDInsight Hive 작업, HDInsight Pig 작업, HDInsight MapReduce 작업, HDInsight Spark 작업, HDInsight Streaming 작업, Azure Machine Learning Studio(클래식) Batch Execution 작업, Azure Machine Learning Studio(클래식) 업데이트 리소스 작업, 저장 프로시저 작업, Data Lake Analytics U-SQL 작업, .NET 사용자 지정 작업, 웹 작업, 조회 작업, 메타데이터 가져오기 작업과 같은 변환 작업을 디스패치합니다.
...
- 온-프레미스 또는 Azure Virtual Network의 컴퓨팅 리소스에 대해 다음과 같은 변환 작업 디스패치: HDInsight Hive 작업(BYOC), HDInsight Pig 작업(BYOC), HDInsight MapReduce 작업(BYOC), HDInsight Spark 작업(BYOC), HDInsight Streaming 작업(BYOC), Azure Machine Learning Studio(클래식) Batch Execution 작업, Azure Machine Learning Studio(클래식) 업데이트 리소스 작업, 저장 프로시저 작업, Data Lake Analytics U-SQL 작업, 사용자 지정 작업(Azure Batch에서 실행), 조회 작업 및 메타데이터 가져오기 작업.
...
고객이 데이터 팩터리 인스턴스를 만들 때 데이터 팩터리의 위치를 지정해야 합니다. Data Factory 위치는 Data Factory의 메타데이터가 저장되는 곳이며 파이프라인 트리거가 시작되는 곳입니다. 팩터리에 대한 메타데이터는 고객이 선택한 지역에만 저장되고 다른 지역에는 저장되지 않습니다.
...
- 복사 작업의 경우 ADF는 자동으로 싱크 데이터 저장소의 위치를 검색한 다음 사용 가능하거나 가장 가까운 영역에 있는 경우 동일한 지역에 있는 IR을 사용하려 합니다. 또는 싱크 데이터 저장소의 지역을 검색할 수 없는 경우 데이터 팩터리 지역의 IR이 대신 사용됩니다.

  예를 들어 미국 동부에서 팩터리를 만든 경우 
  
  - 미국 서부에 있는 Azure Blob에 데이터를 복사할 때 ADF가 Blob이 미국 서부에 있음을 감지하면 복사 작업은 미국 서부의 IR에서 실행됩니다. 지역 검색에 실패한 경우 복사 작업은 미국 동부의 IR에서 실행됩니다.
...
- 조회, 메타데이터 가져오기, 삭제 작업 실행(파이프라인 작업이라고도 함), 변환 작업 디스패치(외부 작업이라고도 함) 및 작성 작업(테스트 연결, 폴더 목록 및 테이블 목록 찾아보기, 데이터 미리 보기)의 경우 ADF는 데이터 팩터리 지역에 있는 IR을 사용합니다.

- 데이터 흐름의 경우 ADF는 데이터 팩터리 지역에 있는 IR을 사용합니다. 

  > [!TIP] 
  > 데이터 흐름이 해당 데이터 저장소와 동일한 지역에서 실행되도록 하는 것이 좋습니다(가능한 경우). Azure IR 자동 확인(데이터 저장소 위치가 Data Factory 위치와 동일한 경우)을 통하거나 데이터 저장소와 동일한 지역에 새 Azure IR 인스턴스를 만든 다음 데이터 흐름에서 실행하여 이를 수행할 수 있습니다. 

Azure IR 자동 확인에 대해 관리형 Virtual Network를 사용 설정한 경우 ADF는 데이터 팩터리 지역에 있는 IR을 사용합니다. 
...
- Azure SSIS IR의 위치가 데이터 팩터리의 위치와 같을 필요는 없지만 사용자 자신의 Azure SQL Database 또는 SQL Managed Instance의 위치(SSISDB)와 같아야 합니다. 이러한 방식으로 Azure SSIS 통합 런타임은 서로 다른 위치 간에 과도한 트래픽을 초래하지 않고 SSISDB에 쉽게 액세스할 수 있습니다.
