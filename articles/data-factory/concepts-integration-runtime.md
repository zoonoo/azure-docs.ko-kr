...
Data Factory는 세 가지 유형의 IR(통합 런타임)을 제공하며, 원하는 데이터 통합 기능 및 네트워크 환경 요구 사항을 가장 잘 충족하는 유형을 선택해야 합니다.  세 유형은 다음과 같습니다.
...
Azure 통합 런타임은 데이터 저장소에 대한 연결지원을 지원하며 공용 액세스 가능한 끝점을 사용하여 서비스를 계산합니다. Azure Virtual Network 환경에는 자체 호스팅 통합 런타임을 사용합니다.
...
Azure IR을 만들고 구성하는 방법에 대한 자세한 내용은 가이드 에서 Azure IR을 만들고 구성하는 방법을 참조하십시오. 
...
- 온-프레미스 또는 Azure 가상 네트워크의 컴퓨팅 리소스에 대한 다음 변환 활동 디스패치: HDInsight Hive 활동(BYOC-Bring Your Own Cluster), HDInsight 돼지 활동(BYOC), HDInsight MapReduce 활동(BYOC), HDInsight 스파크 활동(BYOC), HDInsight 스트리밍 활동(BYOC), HDInsight 스트리밍 활동(BYOC), 기계 학습 배치 실행 활동, 기계 학습 업데이트 리소스 활동, 저장 프로시저 활동, 데이터 분석 U-SQL, Azure 에서 실행 에서는 조회 활동 및 메타데이터 받기 활동.
...
퍼블릭 클라우드 환경과 직접적인 가시가 없는 개인 네트워크 환경에서 데이터 통합을 안전하게 수행하려는 경우 회사 방화벽 뒤또는 가상 사설망 내부에 자체 호스팅 IR을 설치할 수 있습니다.  자체 호스팅 통합 런타임은 아웃바운드 HTTP 기반 연결만이 인터넷을 열게 만듭니다.

### <a name="self-hosted-ir-compute-resource-and-scaling"></a>자체 호스팅 IR 컴퓨팅 리소스 및 크기 조정

온-프레미스 컴퓨터 또는 개인 네트워크 내부의 가상 시스템에 자체 호스팅 IR을 설치합니다. 현재 Windows 운영 체제에서만 자체 호스팅 IR의 실행을 지원합니다.  
...
- 복사 활동의 경우 ADF는 싱크 및 원본 데이터 저장소를 자동으로 감지하여 동일한 지역(사용 가능한 경우) 또는 동일한 지역에서 가장 가까운 위치를 선택하거나 데이터 팩터리 영역을 대체로 사용할 수 없는 경우 최적의 위치를 선택하도록 최선을 다합니다.
...
- SSISDB가 호스팅되는 기존 Azure SQL Database 또는 관리되는 인스턴스 서버의 위치가 온-프레미스 네트워크에 연결된 가상 네트워크의 위치와 같지 않은 경우 먼저 기존 Azure SQL Database 또는 Managed Instance 서버를 사용하여 Azure-SSIS IR을 만들고 동일한 위치에 있는 다른 가상 네트워크에 가입한 다음 다른 위치 간의 가상 네트워크 연결에 가상 네트워크를 구성합니다.
...
- **두 클라우드 데이터 원본 간의 복사**: 원본 및 싱크 링크 된 서비스가 모두 Azure IR을 사용하는 경우 ADF는 지정한 경우 지역 Azure IR을 사용하거나 통합 [런타임 위치](#integration-runtime-location) 섹션에 설명된 대로 자동 해결 IR (기본값)을 선택하면 Azure IR의 위치를 자동으로 결정합니다.
