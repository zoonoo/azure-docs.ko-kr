...
| **fileName** |Azure Data Lake Store에 있는 파일의 이름입니다. **fileName** 속성은 선택 사항이며 대/소문자를 구분합니다. <br/><br/>**fileName**을 지정하는 경우 활동(복사 포함)은 특정 파일에서 작동합니다.<br/><br/>**fileName**을 지정하지 않으면 복사는 입력 데이터 세트에 대한 **folderPath**에 모든 파일을 포함합니다.<br/><br/>때 **fileName** 출력 데이터 집합을 지정 하지 않으면 및 **preserveHierarchy** 형식으로 생성된 된 파일 이름이 활동 싱크에 지정 하지 않으면 `Data._Guid_.txt`합니다. 예:  Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt. |아닙니다. |
