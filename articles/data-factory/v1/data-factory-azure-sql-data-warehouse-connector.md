...
> [!NOTE]
> 이 아티클은 Data Factory 버전 1에 적용됩니다. 최신 버전의 Data Factory 서비스를 사용 하는 경우 v 2 [의 Azure Synapse Analytics 커넥터](../connector-azure-sql-data-warehouse.md)를 참조 하세요.

이 문서에서는 Azure Data Factory의 복사 작업을 사용 하 여 Azure Synapse Analytics 간에 데이터를 이동 하는 방법을 설명 합니다. 이 문서는 복사 작업을 사용한 데이터 이동의 일반적인 개요를 제공 하는 [데이터 이동 활동](data-factory-data-movement-activities.md) 문서를 기반으로 합니다.
...
**Azure Synapse Analytics에서** 다음 데이터 저장소로 데이터를 복사할 수 있습니다.
...
마법사를 사용하는 경우 이러한 Data Factory 엔터티(연결된 서비스, 데이터 세트 및 파이프라인)에 대한 JSON 정의가 자동으로 생성됩니다. 도구/API(.NET API 제외)를 사용하는 경우 JSON 형식을 사용하여 이러한 Data Factory 엔터티를 정의합니다. Azure Synapse Analytics 간에 데이터를 복사 하는 데 사용 되는 Data Factory 엔터티에 대 한 JSON 정의가 포함 된 샘플은이 문서의 [json 예](#json-examples-for-copying-data-to-and-from-azure-synapse-analytics) 섹션을 참조 하세요.

다음 섹션에서는 Azure Synapse Analytics에 한정 된 Data Factory 엔터티를 정의 하는 데 사용 되는 JSON 속성에 대해 자세히 설명 합니다.
...
원본 데이터가 이전 섹션에서 도입 된 조건을 충족 하지 않는 경우 중간 준비 Azure Blob Storage를 통해 데이터를 복사 하도록 설정할 수 있습니다 (Premium Storage 수 없음). 이 경우 Azure Data Factory는 PolyBase의 데이터 형식 요구 사항을 충족 하기 위해 데이터에 대 한 변환을 자동으로 수행 하 고, PolyBase를 사용 하 여 Azure Synapse Analytics로 데이터를 로드 하 고, 마지막으로 Blob 저장소에서 임시 데이터를 정리 합니다. 스테이징 Azure Blob을 통해 데이터를 복사하는 방법에 대한 자세한 내용은 [준비된 복사](data-factory-copy-activity-performance.md#staged-copy)를 참조하세요.

> [!NOTE]
> PolyBase와 스테이징을 사용 하 여 온-프레미스 데이터 저장소에서 Azure Synapse Analytics로 데이터를 복사 하는 경우 데이터 관리 Gateway 버전이 2.4 미만이 면 원본 데이터를 적절 한 형식으로 변환 하는 데 사용 되는 JRE (Java Runtime Environment)가 게이트웨이 컴퓨터에 필요 합니다. 이러한 종속성을 방지하려면 게이트웨이를 최신으로 업그레이드하는 것이 좋습니다.
>
...
```
NULL 값은 특별한 형태의 기본값입니다. 열이 null을 허용하면 해당 열에 대한 입력 데이터(Blob)는 비어 있을 수 있습니다(입력 데이터 세트에서 누락될 수 없음). PolyBase는 Azure Synapse Analytics에서 NULL을 삽입 합니다.

## <a name="auto-table-creation"></a>자동 테이블 만들기
복사 마법사를 사용 하 여 SQL Server 또는 Azure SQL Database에서 Azure Synapse Analytics로 데이터를 복사 하는 경우 원본 테이블에 해당 하는 테이블이 대상 저장소에 없으면 원본 테이블 스키마를 사용 하 여 데이터 웨어하우스의 테이블을 자동으로 만들 수 Data Factory.
...
## <a name="type-mapping-for-azure-synapse-analytics"></a>Azure Synapse Analytics에 대 한 형식 매핑
...
이 샘플은 Azure Synapse Analytics 데이터베이스의 테이블에서 1 시간 마다 blob로 시계열 (매시간, 매일 등) 데이터를 복사 합니다. 이 샘플에 사용된 JSON 속성은 샘플 다음에 나오는 섹션에서 설명합니다.

**Azure Synapse Analytics 연결 된 서비스:**
...
```
**Azure Synapse Analytics 입력 데이터 집합:**

이 샘플에서는 Azure Synapse Analytics에서 "MyTable" 테이블을 만들었으며 시계열 데이터에 대 한 "timestampcolumn" 라는 열을 포함 하 고 있다고 가정 합니다.
...
### <a name="example-copy-data-from-azure-blob-to-azure-synapse-analytics"></a>예: Azure Blob에서 Azure Synapse Analytics로 데이터 복사
샘플이 다음 데이터 팩터리 엔터티를 정의합니다.
...
이 샘플은 Azure blob에서 azure Synapse Analytics 데이터베이스의 테이블에 매시간 시계열 데이터 (매시간, 매일 등)를 복사 합니다. 이 샘플에 사용된 JSON 속성은 샘플 다음에 나오는 섹션에서 설명합니다.
...
```
**Azure Synapse Analytics 출력 데이터 집합:**

이 샘플은 Azure Synapse Analytics의 "MyTable" 이라는 테이블에 데이터를 복사 합니다. Blob CSV 파일에 포함할 것으로 간주 되는 것과 동일한 수의 열을 사용 하 여 Azure Synapse Analytics에 테이블을 만듭니다. 새 행은 매시간 테이블에 추가됩니다.
