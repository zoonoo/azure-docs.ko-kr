...
2. **sampletable**에 데이터를 삽입하는 다음 **저장 프로시저**를 만듭니다.
   ...
   > [!IMPORTANT]
   > 매개 변수(이 예에서는 DateTime)의 **이름** 및 **대/소문자**는 파이프라인/작업 JSON에서 지정된 매개 변수의 이름 및 대/소문자와 일치해야 합니다. 저장 프로시저 정의에서 **\@** 매개 변수의 접두사로 사용되는지 확인합니다.
...
3. **새 data factory** 블레이드에서 이름으로 **SProcDF**를 입력합니다. Azure 데이터 팩터리 이름은 **전역적으로 고유합니다.** 팩터리를 성공적으로 만들려면 데이터 팩터리의 이름의 접두사를 사용자의 이름으로 해야 합니다.
...
1. 클릭 **... **도구 모음에 대한 자세한 내용을 클릭하고 **새 데이터 집합을**클릭하고 **Azure SQL**을 클릭합니다. 명령 모음에서 **새 데이터 세트**를 클릭하고 **Azure SQL**을 선택합니다.
...
1. 클릭 **... **명령 모음에 대한 자세한 내용을 클릭하고 **새 파이프라인을**클릭합니다.
...
| inputs | (선택 사항) 입력 데이터 세트를 지정하는 경우 실행할 저장 프로시저 작업에 사용할 수 있어야 합니다('Ready' 상태). 저장 프로시저에서 입력 데이터 세트를 매개 변수로 사용할 수 없습니다. 저장 프로시저 작업을 시작하기 전에 종속성을 확인하는 데만 사용됩니다. |예 |
...
| storedProcedureName |출력 테이블에서 사용하는 연결된 서비스로 표시되는 Azure SQL 데이터베이스, Azure SQL Data Warehouse 또는 SQL Server 데이터베이스의 저장 프로시저 이름을 지정합니다. |yes |
...
이제 **시나리오** 매개 변수와 저장 프로시저 활동의 값을 전달합니다. 앞의 샘플의 **typeProperties** 섹션은 다음과 같은 코드 조각과 같습니다.
