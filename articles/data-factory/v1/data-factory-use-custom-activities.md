...
- [데이터 변환 작업](data-factory-data-transformation-activities.md)은 Azure HDInsight, Azure Batch, Azure Machine Learning과 같은 컴퓨팅 서비스를 사용하여 데이터를 변환하는 작업입니다.
...
3. `<accountname>`을 Azure Storage 계정 이름으로 바꾸고 `<accountkey>`를 Azure Storage 계정의 액세스 키로 바꿉니다. 저장소 액세스 키를 가져오는 방법을 알아보려면 [저장소 계정 액세스 키 관리](../../storage/common/storage-account-keys-manage.md)를 참조 하세요.
...
1. Data Factory **편집기** **에서 ...를 클릭 합니다. 추가** 를 클릭 하 고 **새 데이터 집합**을 클릭 한 다음 드롭다운 메뉴에서 **Azure Blob storage** 를 선택 합니다.
...
1. **Data Factory 편집기** **에서 ...를 클릭 합니다. 추가** 를 클릭 하 고 **새 데이터 집합**을 클릭 한 다음 **Azure Blob storage**를 선택 합니다.
2. 오른쪽 창의 JSON 스크립트를 다음 JSON 스크립트로 바꿉니다.
    ...
    ```
     ...
     출력 위치는 **adftutorial/customactivityoutput/** 이고 출력 파일 이름은 yyyy-MM-dd-HH.txt입니다. 여기서 yyyy-MM-dd-HH는 조각이 생성되는 년, 월, 일 및 시입니다. 자세한 내용은 [개발자 참조][adf-developer-reference] 를 참조 하세요.
...
1. Data Factory 편집기 **에서 ...를 클릭 합니다. 추가**를 클릭 한 다음 명령 모음에서 **새 파이프라인** 을 선택 합니다.
...
5. 출력 파일을 열면 다음과 유사한 출력이 표시됩니다.
    ...
    2 occurrences(s) of the search term "Microsoft" were found in the file inputfolder/2016-11-16-00/file.txt.
    ```
6. [Azure Portal][azure-preview-portal] 또는 Azure PowerShell cmdlet을 사용 하 여 데이터 팩터리, 파이프라인 및 데이터 집합을 모니터링 합니다. 포털을 통해서나 cmdlet을 사용하여 다운로드할 수 있는 로그(특히 user-0.log)에 있는 사용자 지정 활동의 코드에서 **ActivityLogger** 의 메시지를 확인할 수 있습니다.
