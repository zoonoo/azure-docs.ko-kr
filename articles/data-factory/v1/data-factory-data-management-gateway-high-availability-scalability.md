...
- 현재 단일 논리 게이트웨이에 최대 4개의 실제 게이트웨이 노드를 갖출 수 있습니다. 성능상의 이유로 노드가 4 개 이상 필요한 경우에 [DMGHelp@microsoft.com](mailto:DMGHelp@microsoft.com)전자 메일을 보냅니다.
...
- 모든 게이트웨이 노드에 HTTP 프록시가 필요한 경우 diahost 및 diahost에서 프록시를 설정 하 고 서버 관리자를 사용 하 여 모든 노드에 동일한 diahost .exe .config 및 diahost를 사용 하도록 합니다. 자세한 내용은 [프록시 설정 구성](data-factory-data-management-gateway.md#configure-proxy-server-settings) 섹션을 참조 하세요. 
