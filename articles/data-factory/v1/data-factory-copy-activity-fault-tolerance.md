---
title: 호환 되지 않는 행을 건너뛰어 Azure Data Factory 복사 작업에 내결함성을 추가 합니다.
...
- **원본 데이터 형식 및 싱크 원시 형식 간의 비 호환성**

    예: 3 개의 **INT** 형식 열을 포함 하는 스키마 정의를 사용 하 여 Blob 저장소의 CSV 파일에서 SQL 데이터베이스로 데이터를 복사 합니다. `123,456,789`와 같은 숫자 데이터를 포함하는 CSV 파일 행은 싱크 저장소에 성공적으로 복사됩니다. 그러나 `123,456,abc`와 같은 숫자가 아닌 값을 포함하는 행을 호환되지 않는 것으로 감지하고 건너뜁니다.

- **원본과 싱크 간의 열 수가 일치 하지 않습니다.**
