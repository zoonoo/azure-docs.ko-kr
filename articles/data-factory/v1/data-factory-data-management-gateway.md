...
예를 들어 **온-프레미스 데이터 저장소에서 Azure SQL Database 싱크 또는 Azure SQL Data Warehouse 싱크**로 복사하려면 다음 단계를 수행합니다.
...
1. 파일 탐색기에서 *Microsoft 데이터 관리 Gateway\\2.0\\Shared\\diahost\\를 사용 하 여 C:\\\\프로그램 파일* 을 안전 하 게 복사 하 여 원본 파일을 백업 합니다.
2. 관리자 권한으로 Notepad.exe 실행을 시작 하 고 텍스트 파일 *C:\\\\프로그램 파일\\Microsoft 데이터 관리 게이트웨이\\2.0\\공유\\diahost*를 엽니다. 다음 코드와 같이 system.net에 대 한 기본 태그를 찾을 수 있습니다.
