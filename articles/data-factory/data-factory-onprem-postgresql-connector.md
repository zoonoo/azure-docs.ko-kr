<properties 
	pageTitle="PostgreSQL 커넥터 - PostgreSQL에서 데이터 이동" 
	description="PostgreSQL 데이터베이스에서 데이터를 이동할 수 있는 데이터 팩터리 서비스용 PostgreSQL 커넥터에 대해 알아봅니다" 
...
# PostgreSQL 커넥터 - Azure Data Factory를 사용하여 PostgreSQL에서 데이터 이동
...
**참고:** Azure IaaS VM에서 호스팅되는 경우에도 PostgreSQL에 연결하려면 게이트웨이를 활용해야 합니다. 또한 클라우드에서 호스트되는 PostgreSQL의 인스턴스에 연결하려고 하는 경우 IaaS VM에 게이트웨이 인스턴스를 설치할 수 있습니다.
...
1.	OnPremisesPostgreSql 형식의 연결된 서비스입니다.
2.	AzureStorage 형식의 연결된 서비스입니다.
3.	RelationalTable 형식의 입력 데이터 집합입니다.
4.	BlobSink 형식의 출력 데이터 집합입니다.
...
"external": true를 설정하고 externalData 정책을 지정함으로써 데이터 팩터리에 이 테이블이 데이터 팩터리의 외부에 있으며 데이터 팩터리의 작업에 의해 생성되지 않는다는 점을 알려줍니다.
...
파이프라인은 위의 입력 및 출력 데이터 집합을 사용하도록 구성된 복사 작업을 포함하고 매시간 실행하도록 예약됩니다. 파이프라인 JSON 정의에서 **source** 형식은 **RelationalSource**로 설정되고 **sink** 형식은 **BlobSink**로 설정됩니다. **query** 속성에 지정된 SQL 쿼리는 PostgreSQL 데이터베이스의 public.usstates 테이블에서 데이터를 선택합니다.
...
속성 | 설명 | 필수
-------- | ----------- | --------
tableName | 연결된 서비스가 참조하는 PostgreSQL 데이터베이스 인스턴스에서 테이블의 이름입니다. | 예 
...
원본이 **RelationalSource** 형식인 복사 작업의 경우(PostgreSQL 포함) typeProperties 섹션에서 다음과 같은 속성을 사용할 수 있습니다.
