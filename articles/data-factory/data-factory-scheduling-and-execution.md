...
# Data Factory 예약 및 실행

이 문서에서는 Azure Data Factory 응용 프로그램 모델의 예약 및 실행에 대한 내용을 설명합니다. 이 문서는 [파이프라인 만들기](data-factory-create-pipelines.md) 및 [데이터 집합 만들기](data-factory-create-datasets.md)를 토대로 작성되었습니다. 사용자가 작업, 파이프라인, 연결된 서비스 및 데이터 집합과 같은 Data Factory 응용 프로그램 모델 개념을 이해하고 있다고 가정합니다.
...
현재 실행 중인 작업 창의 경우 작업 JSON에서 [WindowStart](data-factory-functions-variables.md#data-factory-system-variables) 및 [WindowEnd](data-factory-functions-variables.md#data-factory-system-variables) 시스템 변수를 통해 작업 창과 관련된 시간 간격에 액세스할 수 있습니다. 이러한 변수를 작업 JSON에서 다양한 용도로 사용할 수 있습니다. 예를 들어 입력에서 데이터 선택, 출력 데이터 집합에서 시계열 데이터를 표현하는 데 사용할 수 있습니다.

**스케줄러** 속성은 데이터 집합의 **가용성** 속성과 같은 하위 속성을 지원합니다. 세부 정보는 [데이터 집합 가용성](data-factory-create-datasets.md#Availability)을 참조하세요. 예: 특정 시간 오프셋에 예약, 작업 창에 대한 간격 시작 또는 끝에 처리에 맞게 모드 설정 등.

작업에 대한 스케줄러 속성을 지정할 수 있으나 선택 사항입니다. 속성을 지정할 경우 출력 데이터 집합 정의에 지정하는 빈도와 일치해야 합니다. 현재 출력 데이터 집합이 일정을 결정하므로 작업이 출력을 생성하지 않는 경우 출력 데이터 집합을 만들어야 합니다. 활동이 입력을 가져오지 않으면 입력 데이터 집합 만들기를 건너뛸 수 있습니다.
...
[SliceStart](data-factory-functions-variables.md#data-factory-system-variables) 및 [SliceEnd](data-factory-functions-variables.md#data-factory-system-variables) 변수로 데이터 집합 JSON에서 현재 생성되는 조각에 연결된 시간 간격에 액세스할 수 있습니다.
...
## 복사 작업을 통해 Azure SQL Database에서 Azure Blob 저장소로 데이터 이동
...
샘플은 availability 섹션이 매시간 빈도로 설정된 작업 일정과 데이터 집합을 보여줍니다. 이 샘플에서는 **WindowStart** 및 **WindowEnd**를 사용하여 적절한 **folderPath**의 Blob로 작업 실행 및 복사를 위한 관련 데이터를 선택하는 방법을 보여 줍니다. **folderPath**는 매시간 별도의 폴더를 포함하도록 매개 변수화됩니다.
...
-	데이터가 포함된 파일 mypath/2015/1/1/8/Data.&lt;Guid&gt;.txt
...
-	데이터가 포함된 파일 mypath/2015/1/1/9/Data.&lt;Guid&gt;.txt

			10002345,334,1,2015-01-01 09:13:00.3900000
			24379245,569,23,2015-01-01 09:25:00.3130000
			16777799,21,115,2015-01-01 09:47:34.3130000
-	데이터가 포함되지 않은 파일 mypath/2015/1/1/10/Data.&lt;Guid&gt;.txt


## 데이터 조각 만들기, 파이프라인에 대한 활성 기간 설정 및 조각을 동시 실행
...
뒷면 채우기된 데이터 조각이 병렬로 실행되도록 구성할 수 있습니다. [파이프라인 만들기](data-factory-create-pipelines.md)에 나와 있는 것처럼 작업 JSON의 정책 섹션에서 **동시성** 속성을 설정하여 이 작업을 수행할 수 있습니다.
...
### 샘플 2: 식 및 데이터 Data Factory 함수로 종속성 지정
...
모든 작업 실행에 대해 지정해야 하며 Data Factory에서 매주 입력 데이터 집합에 대해 마지막 주의 데이터 조각을 사용해야 합니다. 다음 코드 조각에 나와 있는 것처럼 Azure Data Factory 함수를 이용하여 이를 수행할 수 있습니다.
...
샘플에서 본 것처럼 종속성 기간은 생성할 데이터 조각의 기간과 같습니다. 이 경우 Data Factory는 종속성 기간에 속하는 입력 조각을 자동으로 계산합니다.
...
이러한 경우 조각의 실행이 완료된 후 출력 조각 상태가 **Validation**의 하위 상태가 있는 **Waiting**으로 변경됩니다. 조각의 유효성이 검사되었으면 조각 상태는 **Ready**로 변경됩니다.
...
## 일회성 파이프라인
파이프라인 정의 내에 지정한 시작 및 종료 시간 내에서 정기적(예: 매시간 또는 매일)으로 실행되도록 파이프라인을 만들고 일정을 예약할 수 있습니다. 자세한 내용은 [활동 예약](#scheduling-and-execution)을 참고하세요. 한 번만 실행되는 파이프라인을 만들 수도 있습니다. 이렇게 하려면, 파이프라인 정의 내에서 **pipelineMode** 속성을 **onetime**으로 설정합니다(다음 JSON 샘플 참조). 이 속성의 기본값은 **scheduled**입니다.
...
- Data Factory에서 값이 사용되지 않더라도 이 때는 입력 및 출력 데이터 집합의 **가용성**(**빈도** 및 **간격**)이 지정됩니다.
