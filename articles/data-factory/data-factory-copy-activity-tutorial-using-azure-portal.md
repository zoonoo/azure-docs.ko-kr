---
title: "자습서: Azure Portal을 사용하여 복사 작업이 있는 파이프라인 만들기 | Microsoft Docs"
...
---
# <a name="tutorial-create-a-pipeline-with-copy-activity-using-azure-portal"></a>자습서: Azure Portal을 사용하여 복사 작업이 있는 파이프라인 만들기
...
2. **새 데이터 팩터리** 블레이드에서 다음을 수행합니다.
   ...
   6. **만들기**를 클릭합니다.
      ...
      > 
      > 
3. 상태/알림 메시지를 보려면 도구 모음에서 종 아이콘을 클릭합니다. 
...
2. **편집기**의 도구 모음에서 **새 데이터 저장소** 단추를 클릭하고 드롭다운 메뉴에서 **Azure Storage**를 선택합니다. 오른쪽 창에 Azure 저장소 연결된 서비스를 만들기 위한 JSON 템플릿이 표시됩니다. 
...
### <a name="create-a-linked-service-for-the-azure-sql-database"></a>Azure SQL 데이터베이스에 대한 연결된 서비스 만들기
...
> [!NOTE]
> JSON 속성에 대한 자세한 내용은 [Azure SQL Database 간의 데이터 이동](data-factory-azure-sql-connector.md#linked-service-properties) 을 참조하세요.
> 
> 
...
> [!NOTE]
> JSON 속성에 대한 자세한 내용은 [Azure Blob 간의 데이터 이동](data-factory-azure-blob-connector.md#dataset-properties) 을 참조하세요.
> 
> 
...
1. 데이터 팩터리의 Data Factory 홈 페이지를 보려면 **X**를 클릭하여 **다이어그램** 보기를 닫습니다. 웹 브라우저를 닫은 경우 다음 단계를 수행합니다. 
   ...
   2. **시작 보드**에서 **ADFTutorialDataFactory**를 두 번 클릭하거나 왼쪽 메뉴에서 **데이트 팩터리**를 클릭하고 ADFTutorialDataFactory를 검색합니다. 
2. 이 블레이드에서 만든 테이블과 파이프라인의 개수와 이름이 표시됩니다.
...
3. 이제 **데이터 집합** 타일을 클릭합니다.
4. **데이터 집합** 블레이드에서 **InputDataset**을 클릭합니다. 이 데이터 집합은 **ADFTutorialPipeline**에 대한 입력 데이터 집합입니다.
...
5. **...(줄임표)** 를 클릭하여 데이터 조각을 모두 표시합니다.
    ...
    **최근에 업데이트된 조각** 및 **최근에 실패한 조각** 목록은 둘 다 **마지막 업데이트 시간**을 기준으로 정렬됩니다. 
...
13. (선택 사항) **ADFTutorialDataFactory**의 홈 페이지에서 **파이프라인**을 클릭하고 **파이프라인** 블레이드에서 **ADFTutorialPipeline**을 클릭한 다음 입력 테이블(**사용됨**) 또는 출력 테이블(**생성됨**)을 드릴스루합니다.
