...
![스크린샷 위쪽 막대, 그래프 및 구성 패널이 표시 된 데이터 흐름 캔버스를 표시 합니다.](media/data-flow/canvas-1.png "캔버스")
...
구성 패널에는 현재 선택한 변환과 관련 된 설정이 표시 됩니다. 선택 된 변환이 없는 경우 데이터 흐름이 표시 됩니다. 전체 데이터 흐름 구성에서는 **매개 변수** 탭을 통해 매개 변수를 추가할 수 있습니다. 자세한 내용은 [데이터 흐름 매개 변수 매핑](parameters-data-flow.md)을 참조 하세요.
