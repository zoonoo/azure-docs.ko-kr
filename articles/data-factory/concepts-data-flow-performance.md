...
**일괄 처리 크기:** 일반적으로 기본 일괄 처리 크기로 시작 하는 것으로 충분 합니다. 이 값을 추가로 조정 하려면 데이터의 대략적인 개체 크기를 계산 하 고 개체 크기 * 일괄 처리 크기가 2MB 미만인 지 확인 합니다. 이 경우 일괄 처리 크기를 늘려 처리량을 높일 수 있습니다.
...
큰 데이터베이스 쿼리와 같이 쿼리 하는 데 더 오래 걸릴 수 있는 데이터 원본으로 작업 하는 경우 조인에 대해 브로드캐스트 기능을 해제 하는 것이 좋습니다. 쿼리가 시간이 긴 원본에서는 클러스터가 계산 노드에 브로드캐스트를 시도할 때 Spark 시간 초과가 발생할 수 있습니다. 브로드캐스트 해제에는 나중에 조회 변환에 사용 하기 위해 값을 집계 하는 스트림이 있는 경우에 다른 좋은 선택이 있습니다. 이 패턴은 Spark 최적화 프로그램을 혼동 하 고 시간 초과를 일으킬 수 있습니다.
