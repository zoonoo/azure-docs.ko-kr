...
| enableServerCertificateValidation | SSL/TLS 채널을 통해 FTP를 사용 하는 경우 서버 TLS/SSL 인증서 유효성 검사를 사용 하도록 설정할지 여부를 지정 합니다.<br/>허용되는 값은 **true**(기본값), **false**입니다. | 아니요 |
| authenticationType | 인증 유형을 지정합니다.<br/>허용되는 값은 **Basic**, **Anonymous**입니다. | 예 |
| userName | FTP 서버에 액세스하는 사용자를 지정합니다. | 아니요 |
| password | 사용자(사용자 이름)의 암호를 지정합니다. 이 필드를 SecureString으로 표시하여 Data Factory에 안전하게 저장하거나, [Azure Key Vault에 저장된 비밀을 참조](store-credentials-in-key-vault.md)합니다. | 아니요 |
...
서식 기반 데이터 집합의 설정에서 `location` FTP에 대해 지원 되는 속성은 다음과 같습니다.
...
서식 기반 복사 원본에서 설정 아래의 `storeSettings` FTP에 대해 지원 되는 속성은 다음과 같습니다.

| 속성                 | Description                                                  | 필수                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 아래의 `storeSettings` type 속성은 **Ftpreadsettings**로 설정 되어야 합니다. | 예                                           |
