...
ADF의 관리 되는 id에 대 한 자세한 내용은 [Data Factory 관리 id](./data-factory-service-identity.md)를 참조 하세요.
...
1.  [AZURE AD PowerShell](/powershell/azure/active-directory/install-adv2) 모듈을 설치 합니다.

2.  을 사용 하 여 로그인 하 `Connect-AzureAD` 고, 다음 cmdlet을 실행 하 여 그룹을 만들고 변수에 저장 합니다.
...
3.  ADF의 관리 ID를 그룹에 추가합니다. [Data Factory에 대 한 관리 id](./data-factory-service-identity.md) 문서를 따라 주 관리 ID 개체 id를 가져올 수 있습니다 (예: 765ad4ab-xxxx-xxxx-51ed985819dc,이 목적을 위해 관리 되는 Id 응용 프로그램 id를 사용 하지 않음).
...
다음 단계를 사용 하 여 [SQL에서 AZURE AD 인증을 구성 하 고 관리할](../azure-sql/database/authentication-aad-configure.md) 수 있습니다.
...
3. **인증** 필드에서 **MFA를 지 원하는 Active Directory-유니버설** 을 선택 합니다. 다른 두 Active Directory 인증 유형을 사용할 수도 있습니다. SQL에서 [Azure AD 인증 구성 및 관리](../azure-sql/database/authentication-aad-configure.md)를 참조 하세요.
...
10. SQL 인증을 사용 하 여 SSISDB를 만든 경우 Azure-SSIS IR에 Azure AD 인증을 사용 하도록 전환 하려면 먼저 **master** 데이터베이스에 대 한 사용 권한을 부여 하는 단계가 성공적으로 완료 되었는지 확인 합니다. 그런 다음 **SSISDB** 데이터베이스를 마우스 오른쪽 단추로 클릭 하 고 **새 쿼리** 를 선택 합니다.
...
[SQL Managed Instance에 대 한 Azure Active Directory 관리자 프로 비전](../azure-sql/database/authentication-aad-configure.md#provision-azure-ad-admin-sql-managed-instance)의 단계를 수행 합니다.
...
6.  SQL 인증을 사용 하 여 SSISDB를 만든 경우 Azure-SSIS IR에 Azure AD 인증을 사용 하도록 전환 하려면 먼저 **master** 데이터베이스에 대 한 사용 권한을 부여 하는 단계가 성공적으로 완료 되었는지 확인 합니다. 그런 다음 **SSISDB** 데이터베이스를 마우스 오른쪽 단추로 클릭 하 고 **새 쿼리** 를 선택 합니다.
