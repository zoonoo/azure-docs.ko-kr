...
데이터베이스 워크 로드를 SQL Server 온-프레미스에서 Azure 데이터베이스 서비스 (Azure SQL Database 또는 Azure SQL Database 관리 되는 인스턴스)로 마이그레이션하는 경우 기본 부가 가치 서비스 중 하나로 SSIS (SQL Server Integration Services)의 ETL 워크 로드도 마이그레이션해야 합니다.
