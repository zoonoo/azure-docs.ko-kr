...
온프레미스 SQL Server에서 Azure 데이터베이스 서비스( 즉 Azure SQL Database 또는 Azure SQL Database 관리 인스턴스)로 데이터베이스 워크로드를 마이그레이션할 때 SSIS(SQL Server 통합 서비스)의 ETL 워크로드를 기본 부가 가치 중 하나로 사용할 수 있습니다. 서비스도 마이그레이션해야 합니다.
