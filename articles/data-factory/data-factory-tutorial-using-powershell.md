...
2. (아직 만들지 않은 경우)다음 명령을 실행하여 **ADFTutorialResourceGroup**이라는 Azure 리소스 그룹을 만듭니다.
...
16. Azure PowerShell에서 **C:\\ADFWalkthrough** 또는 파일의 압축을 푼 폴더의 **LinkedServices** 하위 폴더로 이동합니다.
17. 다음 명령을 사용하여 $df 변수를 데이터 팩터리의 이름으로 설정합니다.

		$df = “LogProcessingFactory”
17. 선호하는 편집기에서 **StorageLinkedService.json**을 열고 **계정 이름** 및 **계정 키** 값을 입력하며 파일을 저장합니다.
17. 다음과 같이 **New-AzureRmDataFactoryLinkedService** cmdlet을 사용하여 연결된 서비스를 만듭니다. 

		New-AzureRmDataFactoryLinkedService -ResourceGroupName ADF -DataFactoryName $df -File .\StorageLinkedService.json
	
18. 선호하는 편집기에서 **StorageLinkedService.json**을 열고 **계정 이름** 및 **계정 키** 값을 입력하며 파일을 저장합니다.
19. **HDInsightStorageLinkedService**을 만듭니다.

		New-AzureRmDataFactoryLinkedService -ResourceGroupName ADF -DataFactoryName $df -File .\HDInsightStorageLinkedService.json
 
19. 선호하는 편집기에서 **AzureSqlLinkedService.json**을 열고 **azure sql server** 이름, **사용자 이름** 및 **암호** 값을 입력하며 파일을 저장합니다.
19. 다음과 같이 **New-AzureRmDataFactoryLinkedService** cmdlet을 사용하여 연결된 서비스를 만듭니다. 

		New-AzureRmDataFactoryLinkedService -ResourceGroupName ADF -DataFactoryName $df -File .\AzureSqlLinkedService.json
19. 유형이 **HDInsightOnDemandLinkedService**로 설정된 좋아하는 편집기 및 알림에서 **HDInsightLinkedService.json**을 엽니다.
...
19. 다음과 같이 **New-AzureRmDataFactoryLinkedService** cmdlet을 사용하여 연결된 서비스를 만듭니다. 저장소 계정으로 시작합니다.
...
2.	다음과 같이 **New-AzureRmDataFactoryDataset** cmdlet을 사용하여 **RawGameEventsTable**.json에 대한 데이터 집합을 만듭니다.	
...
4. **Azure 포털**에서 **LogProcessingFactory**의 **DATA FACTORY** 블레이드에서 **데이터 집합**을 클릭하고 데이터 집합이 모두 표시되는지 확인합니다(테이블은 사각형 데이터 집합).
...
5.  다음과 같이 **New-AzureRmDataFactoryPipeline** cmdlet을 사용하여 **PartitionGameLogspeline**.json에 대한 파이프라인을 만듭니다.	 
...
7. **Get-AzureRmDataFactoryPipeline** cmdlet을 사용하여 파이프라인 목록을 가져옵니다.
...
파이프라인의 활성 기간을 지정하려면 Set-AzureRmDataFactoryPipelineActivePeriod cmdlet을 사용할 수 있습니다. 이 연습에서는 샘플 데이터가 5월 1일에서 5월 5일까지입니다. 2014-05-01을 StartDateTime으로 사용합니다. EndDateTime은 선택 사항입니다.
...
11. **Azure 포털**에서 **LogProcessingFactory**의 **DATA FACTORY** 블레이드에서 파이프라인 이름이 아니라 **파이프라인** 타일을 클릭하면 만든 파이프라인이 표시됩니다.
