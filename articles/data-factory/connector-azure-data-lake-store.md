...
이 문서에서는 Azure Data Lake 저장소 Gen1 간에 데이터를 복사 하는 방법 설명 (ADLS Gen1). Azure Data Factory에 대해 자세히 알아보려면 [소개 문서](introduction.md)를 참조하세요.
...
| connectVia | 데이터 저장소에 연결하는 데 사용할 [통합 런타임](concepts-integration-runtime.md)입니다. Azure Integration Runtime 또는 자체 호스팅 통합 런타임을 사용할 수 있습니다(데이터 저장소가 프라이빗 네트워크에 있는 경우). 이 속성을 지정하지 않으면 기본 Azure Integration Runtime을 사용합니다. |아닙니다. |
...
ADLS Gen1에서 데이터를 복사 하 **Parquet 또는 구분 기호로 분리 된 텍스트 형식**, 참조 [Parquet 형식](format-parquet.md) 하 고 [구분 기호로 분리 된 텍스트 형식으로](format-delimited-text.md) 형식 기반의 데이터 집합에 대 한 문서 및 지원 되는 설정입니다. 다음 속성에서 ADLS Gen1 지 `location` 형식 기반의 데이터 집합의 설정:
...
> [!NOTE]
>
> **AzureDataLakeStoreFile** 다음 섹션에 언급 된 Parquet/텍스트 형식 사용 하 여 데이터 집합 형식으로 계속 지원-데이터 흐름 매핑 작동 하지 않습니다 하지만 이전 버전과 호환성에 대 한 복사/조회/GetMetadata 작업입니다. 앞으로이 새 모델을 사용 하도록 제안 된 및 UI를 작성 하는 ADF 이러한 새 형식 생성로 전환 되었습니다.
...
ADLS Gen1에서 데이터를 복사 하 **ORC/Avro/JSON/이진 형식**, 다음 속성이 지원 됩니다.
...
ADLS Gen1에서 데이터를 복사 **Parquet 또는 구분 기호로 분리 된 텍스트 형식**를 참조 [Parquet 형식](format-parquet.md) 및 [구분 기호로 분리 된 텍스트 형식으로](format-delimited-text.md) 형식 기반의 복사 활동 원본에 대 한 문서 및 지원 되는 설정입니다. ADLS Gen1 아래에서 다음 속성이 지원 됩니다 `storeSettings` 형식 기반의 복사 원본에 설정 합니다.
...
> [!NOTE]
> Parquet/구분 기호로 분리 된 텍스트 형식에 대 한 **AzureDataLakeStoreSource** 다음 섹션에 언급 된 형식 복사 활동 원본으로 계속 지원 됩니다-이전 버전과 호환성입니다. 앞으로이 새 모델을 사용 하도록 제안 된 및 UI를 작성 하는 ADF 이러한 새 형식 생성로 전환 되었습니다.
...
ADLS Gen1에서 데이터를 복사할 **ORC/Avro/JSON/이진 파일 형식**를 복사 작업에서 다음 속성이 지원 됩니다 **원본** 섹션:
...
ADLS Gen1에서 데이터를 복사할 **Parquet 또는 구분 기호로 분리 된 텍스트 형식**를 참조 [Parquet 형식](format-parquet.md) 및 [구분 기호로 분리 된 텍스트 형식으로](format-delimited-text.md) 형식 기반의 복사 활동 sink에 대 한 문서 및 지원 되는 설정입니다. ADLS Gen1 아래에서 다음 속성이 지원 됩니다 `storeSettings` 형식 기반 복사 싱크를 설정 합니다.
...
> [!NOTE]
> Parquet/구분 기호로 분리 된 텍스트 형식에 대 한 **AzureDataLakeStoreSink** 으로 다음 섹션에 언급 된 형식 복사 활동 sink는 계속 지원-이전 버전과 호환성입니다. 앞으로이 새 모델을 사용 하도록 제안 된 및 UI를 작성 하는 ADF 이러한 새 형식 생성로 전환 되었습니다.
...
ADLS Gen1에서 데이터를 복사할 **ORC/Avro/JSON/이진 파일 형식**에서 다음 속성이 지원 됩니다 합니다 **싱크** 섹션:
