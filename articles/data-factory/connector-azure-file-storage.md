...
| host | Azure File Storage 엔드포인트를 `"host": "\\\\<storage name>.file.core.windows.net\\<file service name>"`로 지정합니다. | yes |
| userId | Azure File Storage에 `"userid": "AZURE\\<storage name>"`로 액세스할 사용자를 지정합니다. | yes |
| 암호 | 저장소 액세스 키를 지정합니다. 이 필드를 SecureString으로 표시하여 Data Factory에 안전하게 저장하거나 [Azure Key Vault에 저장되는 비밀을 참조](store-credentials-in-key-vault.md)합니다. | yes |
...
| folderPath | 파일의 경로입니다. 와일드카드 필터는 지원되지 않습니다. |yes |
