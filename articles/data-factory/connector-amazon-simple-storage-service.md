...
| 접두사                   | 원본 개체를 필터링 하는 데이터 집합에 구성 된 지정 된 버킷 S3 개체 키에 대 한 접두사입니다. 이 접두사로 시작하는 키를 가진 개체가 선택됩니다. <br>경우에만 적용 됩니다 `wildcardFolderPath` 및 `wildcardFileName` 속성을 지정 하지 않습니다. | 아닙니다.                                                          |
