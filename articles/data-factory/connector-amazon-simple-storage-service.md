...
>[!NOTE]
>작성에 Data Factory GUI를 사용할 때는 테스트 연결 및 파일 경로 찾아보기/탐색과 같은 작업을 위한 `s3:ListAllMyBuckets` 권한도 필요합니다. 이 권한을 부여하지 않으려면 연결된 서비스 만들기 페이지에서 테스트 연결을 건너뛰고 데이터 집합 설정에서 직접 경로를 지정합니다.
