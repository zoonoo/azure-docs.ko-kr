<properties 
	pageTitle="DB2 커넥터 - DB2에서 데이터 이동" 
	description="DB2 데이터베이스에서 데이터를 이동시킬 수 있는 데이터 팩터리 서비스용 DB2 커넥터를 알아봅니다" 
...
파이프라인은 위의 입력 및 출력 데이터 집합을 사용하도록 구성된 복사 작업을 포함하고 매시간 실행하도록 예약됩니다. 파이프라인 JSON 정의에서 **소스** 형식은 **RelationalSource**으로 설정되고 **싱크** 형식은 **BlobSink**으로 설정됩니다. **쿼리** 속성에 지정된 SQL 쿼리는 주문 테이블에서 데이터를 선택합니다.
...
| 속성 | 설명 | 필수 |
| -------- | ----------- | -------- | 
| type | 형식 속성은 **OnPremisesDB2**으로 설정되어야 합니다. | 예 |
...
| password | 사용자 이름에 지정한 사용자 계정에 암호를 지정합니다. | 아니요 |
...
| 속성 | 설명 | 필수 |
| -------- | ----------- | -------- | 
| tableName | 연결된 서비스가 참조하는 DB2 데이터베이스 인스턴스에서 테이블의 이름입니다. | 예 |
...
| 속성 | 설명 | 허용되는 값 | 필수 |
| -------- | ----------- | -------- | -------------- |
| 쿼리 | 사용자 지정 쿼리를 사용하여 데이터를 읽습니다. | SQL 쿼리 문자열. 예를 들어 MyTable에서 *를 선택합니다. | 아니요 |
