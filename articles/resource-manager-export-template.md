...
Azure Resource Manager 템플릿을 작성하는 방법은 이해하기 어렵다고 생각할 수 있지만 Resource Manager에서는 구독의 기존 리소스에서 템플릿을 내보낼 수 있어 해당 템플릿을 쉽게 작성할 수 있습니다. 생성된 템플릿을 사용하여 템플릿 구문에 대해 배우거나 필요에 따라 솔루션 재배포를 자동화할 수 있습니다.

이 자습서에서는 포털을 통해 저장소 계정을 만들고 해당 저장소 계정에 대한 템플릿을 내보냅니다. 그런 다음 가상 네트워크를 추가하여 리소스 그룹을 수정하고 현재 상태를 나타내는 새 템플릿을 내보냅니다. 이 항목은 주로 간단한 인프라에 대해 살펴보지만 이와 동일한 단계를 사용하여 더 복잡한 솔루션의 템플릿을 내보낼 수 있습니다.
...
2. 저장소 계정을 만들고 **storage**, 사용자의 이니셜 및 날짜로 이름을 지정합니다. 저장소 계정 이름은 Azure 전체에서 고유해야 하므로 이름이 이미 사용 중인 경우에는 변형을 사용해 봅니다. 리소스 그룹에 대해 **ExportGroup**을 사용합니다. 다른 속성에 대해서는 기본값을 사용할 수 있습니다. **만들기**를 선택합니다.
...
4. Resource Manager가 5개 파일을 자동으로 검색합니다. 아래에 이 계정과 키의 예제가 나와 있습니다.

   1. 솔루션의 인프라를 정의하는 템플릿입니다. 포털을 통해 저장소 계정을 만들 때 Resource Manager는 템플릿을 사용하여 배포하고 나중에 참조할 수 있도록 해당 템플릿을 저장했습니다.

   2. 배포하는 동안 값을 전달하는 데 사용할 수 있는 매개 변수 파일. 여기에는 첫 번째 배포 중 입력한 값이 포함되어 있지만 템플릿을 다시 배포할 때 임의 값을 변경할 수 있습니다.

   3. 템플릿 배포에 사용할 수 있는 Azure PowerShell 스크립트 파일.

   4. 템플릿 배포에 사용할 수 있는 Azure CLI 스크립트 파일.
   
   5. 템플릿 배포에 사용할 수 있는 .NET 클래스.
     ...
     여기에는 저장소 계정 이름, 유형, 위치에 대한 매개 변수와 암호화를 사용하는지 여부(기본값은 **false**)가 정의되어 있습니다. **리소스** 섹션 내에 배포할 저장소 계정에 대한 정의가 표시됩니다.
     
대괄호에는 배포 중 평가되는 식이 포함됩니다. 위에서 대괄호에 포함된 식은 배포 중 매개 변수 값을 가져오는 데 사용됩니다. 다양한 식을 사용할 수 있으며 이 항목의 뒷부분에 다른 식의 예가 나와 있습니다. 전체 목록을 보려면 [Azure Resource Manager 템플릿 함수](resource-group-template-functions.md)를 참조하세요.
...
2. 이 경우에도 솔루션을 재배포하는 데 사용할 수 있는 5개의 파일이 표시되지만 템플릿이 약간 다릅니다. 이 템플릿에는 2개의 매개 변수만 있습니다(저장소 계정 이름용, 가상 네트워크 이름용).
     ...
     Resource Manager는 배포 중 사용된 템플릿을 검색하지 않았습니다. 대신 현재 리소스 구성을 기준으로 새 템플릿을 생성했습니다. Resource Manager는 사용자가 매개 변수로 전달하려는 값을 알지 못하므로 리소스 그룹의 값을 기준으로 최근 값을 하드 코드합니다. 예를 들어 저장소 계정 위치와 복제 값은 다음으로 설정되어 있습니다.
