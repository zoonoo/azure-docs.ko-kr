...
Hive는 대규모 데이터의 요약, 쿼리 및 분석에 적용할 수 있는 SQL 같은 스크립트 언어인 *[HiveQL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DDL)*을 통해 MapReduce 작업을 실행하는 수단을 제공합니다. 이 자습서에서는 Hive를 사용하여 공항 간의 평균 지연 시간을 계산하는 방법 및 Sqoop을 사용하여 계산 결과를 SQL 데이터베이스로 내보내는 방법을 보여 줍니다.
...
이 자습서에서는 워크스테이션에 [Research and Innovative Technology Administration, Bureau of Transportation Statistics](http://www.transtats.bts.gov/DL_SelectFields.asp?Table_ID=236&DB_Short_Name=On-Time)(RITA)(영문)의 항공사 비행 데이터 운항정시성을 사용합니다. 다음을 수행합니다.
...
다음 표에는 이 자습서에서 사용되는 파일이 나와 있습니다.

<table data-morhtml="true" border="1">
<tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">파일</strong></td><td data-morhtml="true"><strong data-morhtml="true">설명</strong></td></tr>
<tr data-morhtml="true"><td data-morhtml="true">\tutotirals\flightdelays\data</td><td data-morhtml="true">Hive의 입력 비행 데이터</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">\tutorials\flightdelays\output</td><td data-morhtml="true">Hive의 출력</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">\tutorials\flightdelays\flightdelays.hql</td><td data-morhtml="true">HiveQL 스크립트 파일</td></tr>
<tr data-morhtml="true"><td data-morhtml="true">\tutorials\flightdelays\jobstatus</td><td data-morhtml="true">Hadoop 작업 상태</td></tr>
</table>
...
-   CREATE TABLE 명령은 데이터 파일을 /hive/warehouse/ 폴더로 이동합니다.
...
자세한 내용은 [HDInsight: Hive 내부 및 외부 테이블 설명][cindygross-hive-tables]을 참조하십시오.
...
1.  [Research and Innovative Technology Administration, Bureau of Transportation Statistics](http://www.transtats.bts.gov/DL_SelectFields.asp?Table_ID=236&DB_Short_Name=On-Time)(RITA)(영문)로 이동합니다.
2.  페이지에서 다음 값을 선택합니다.
 ...
 <tr data-morhtml="true"><td data-morhtml="true">Fields:</td><td data-morhtml="true">*Year*, *FlightDate*, *UniqueCarrier*, *Carrier*, *FlightNum*, *OriginAirportID*, *Origin*, *OriginCityName*, *OriginState*, *DestAirportID*, *Dest*, *DestCityName*, *DestState*, *DepDelayMinutes*, *ArrDelay*, *ArrDelayMinutes*, *CarrierDelay*, *WeatherDelay*, *NASDelay*, *SecurityDelay*, *LateAircraftDelay*(다른 모든 필드는 선택하지 않음)</td></tr>
 </table>
...
3.  다음과 같이 처음 3개의 변수를 설정한 후 명령을 실행합니다.
    ...
    <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">변수 이름</strong></td><td data-morhtml="true"><strong data-morhtml="true">설명</strong></td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$subscriptionName</td><td data-morhtml="true">Azure 구독 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$storageAccountName</td><td data-morhtml="true">비행 데이터 파일을 저장하는 데 사용되는 Azure 저장소 계정입니다. 기본 저장소 계정을 사용하는 것이 좋습니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$containerName</td><td data-morhtml="true">비행 데이터 파일을 저장하는 데 사용되는 Azure Blob 저장소 컨테이너입니다. 기본 HDInsight 클러스터 파일 시스템 Blob 컨테이너를 사용하는 것이 좋습니다.  기본적으로 컨테이너 이름은 HDInsight 클러스터 이름과 동일합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$localFolder</td><td data-morhtml="true">비행 지연 파일이 저장되는 워크스테이션의 폴더입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$destFolder</td><td data-morhtml="true">비행 지연 데이터를 업로드할 WASB 경로입니다. Hadoop(HDInsight) 경로는 대/소문자를 구분합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$month</td><td data-morhtml="true">12개의 파일을 모두 다운로드하지 못했으면 이 변수를 업데이트해야 합니다.</td></tr>
 </table>
...
3.  다음 스크립트에서 처음 6개의 변수를 설정한 후 명령을 실행합니다.
 ...
 <tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">변수 이름</strong></td><td data-morhtml="true"><strong data-morhtml="true">설명</strong></td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$subscriptionName</td><td data-morhtml="true">Azure 구독 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseServer</td><td data-morhtml="true">Sqoop에서 데이터를 내보내는 데 사용하는 SQL 데이터베이스 서버 이름입니다. 그대로 놔두면 스크립트가 이름을 하나 만듭니다. 그렇지 않은 경우 기존 SQL 데이터베이스나 SQL 서버를 지정합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseUsername</td><td data-morhtml="true">SQL 데이터베이스/SQL 서버 사용자 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabasePassword</td><td data-morhtml="true">SQL 데이터베이스/SQL 서버 사용자 암호입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseLocation</td><td data-morhtml="true">스크립트에서 SQL 데이터베이스 서버를 만들도록 할 경우에만 사용됩니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseName</td><td data-morhtml="true">Sqoop에서 데이터를 내보내는 데 사용하는 SQL 데이터베이스 이름입니다. 그대로 놔두면 스크립트가 이름을 하나 만듭니다. 그렇지 않은 경우 기존 SQL 데이터베이스나 SQL 서버를 지정합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseMaxSizeGB</td><td data-morhtml="true">스크립트에서 SQL 데이터베이스를 만들도록 할 경우에만 사용됩니다.</td></tr>
 </table>
...
Azure PowerShell을 사용하여 여러 HiveQL 문을 한 번에 하나씩 실행하거나 HiveQL 문을 스크립트 파일 패키지로 만들 수 있습니다. 이 자습서에서는 HiveQL 스크립트를 만듭니다. 스크립트 파일은 WASB에 업로드해야 합니다. 다음 섹션에서는 Azure PowerShell을 사용하여 스크립트 파일을 실행합니다.
...
HiveQL 명령의 전체 목록을 보려면 [Hive 데이터 정의 언어](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DDL)(영문)를 참조하십시오. 각 HiveQL 명령은 세미콜론으로 끝나야 합니다.
...
3.  다음과 같이 처음 두 개의 변수를 설정한 후 명령을 실행합니다.
    ...
    <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">변수 이름</strong></td><td data-morhtml="true"><strong data-morhtml="true">설명</strong></td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$storageAccountName</td><td data-morhtml="true">HiveQL 스크립트 파일을 저장하는 데 사용되는 Azure 저장소 계정입니다. 이 자습서에 제공된 PowerShell 스크립트에는 동일한 Azure 저장소 계정 및 Blob 저장소 컨테이너에 있는 스크립트 파일과 비행 데이터 파일이 둘 다 있어야 합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$containerName</td><td data-morhtml="true">HiveQL 스크립트 파일을 저장하는 데 사용되는 Azure Blob 저장소 컨테이너입니다. 이 자습서에 제공된 PowerShell 스크립트에는 동일한 Azure 저장소 계정 및 Blob 저장소 컨테이너에 있는 스크립트 파일과 비행 데이터 파일이 둘 다 있어야 합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$hqlLocalFileName</td><td data-morhtml="true">WASB로 업로드되기 전 HiveQL 스크립트의 로컬 파일 이름입니다. PowerShell 스크립트를 간소화하려면 파일을 로컬로 쓴 다음 Set-AzureStorageBlobContent cmdlet을 사용하여 스크립트 파일을 HDInsight에 업로드합니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$hqlBlobName</td><td data-morhtml="true">WASB의 경로를 가진 스크립트 파일 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$srcDataFolder</td><td data-morhtml="true">HiveQL 스크립트가 데이터를 가져오는 WASB의 폴더입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$dstDataFolder </td><td data-morhtml="true">HiveQL 스크립트가 출력을 내보내는 WASB의 폴더입니다. 자습서의 뒷부분에서 Sqoop을 사용하여 이 폴더의 데이터를 Azure SQL 데이터베이스로 내보냅니다.</td></tr>
 </table>
...
4.  다음과 같이 처음 3개의 변수를 설정한 후 명령을 실행합니다.
    ...
    <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">변수 이름</strong></td><td data-morhtml="true"><strong data-morhtml="true">설명</strong></td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$clusterName</td><td data-morhtml="true">Hive 스크립트와 Sqoop 내보내기를 실행할 HDInsight 클러스터입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$storageAccountName</td><td data-morhtml="true">HiveQL 스크립트를 저장하는 데 사용되는 Azure 저장소 계정입니다. <a data-morhtml="true" href="#createScript">Create and upload HiveQL 스크립트 만들기 및 업로드</a>를 참조하십시오.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$containerName</td><td data-morhtml="true">HiveQL 스크립트를 저장하는 데 사용되는 Azure Blob 저장소 컨테이너입니다. <a data-morhtml="true" href="#createScript">Create and upload HiveQL 스크립트 만들기 및 업로드</a>를 참조하십시오.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$hqlScriptFile</td><td data-morhtml="true">HiveQL 스크립트 파일의 URI입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$outputBlobName</td><td data-morhtml="true">HiveQL 스크립트 출력 파일입니다. 기본 이름은 *000000_0*입니다.</td></tr>
 </table>
...
Azure SQL 데이터베이스로 Hive 작업 출력 내보내기
------------------------------------------------
...
3.  다음과 같이 처음 5개의 변수를 설정한 후 명령을 실행합니다.
    ...
    <table data-morhtml="true" border="1">
 <tr data-morhtml="true"><td data-morhtml="true"><strong data-morhtml="true">변수 이름</strong></td><td data-morhtml="true"><strong data-morhtml="true">참고</strong></td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$clusterName</td><td data-morhtml="true">HDInsight 클러스터 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseServer</td><td data-morhtml="true">Sqoop에서 데이터를 내보낼 SQL 데이터베이스 서버입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseUsername</td><td data-morhtml="true">SQL 데이터베이스 사용자 이름입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabasePassword</td><td data-morhtml="true">SQL 데이터베이스 사용자 암호입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseName</td><td data-morhtml="true">Sqoop에서 데이터를 내보낼 SQL 데이터베이스입니다. 기본 이름은 *HDISqoop&quot;입니다. </td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$sqlDatabaseTableName</td><td data-morhtml="true">Sqoop에서 데이터를 내보낼 SQL 데이터베이스입니다. 기본 이름은 AvgDelays입니다. 이 테이블은 자습서의 앞부분에서 만든 테이블입니다.</td></tr>
 <tr data-morhtml="true"><td data-morhtml="true">$exportDir</td><td data-morhtml="true">Hive 출력 파일 위치입니다. Sqoop은 이 위치에 있는 파일을 SQL 데이터베이스로 내보냅니다.</td></tr>
 </table>
