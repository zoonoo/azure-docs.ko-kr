...
`api-version`은 필수입니다. 유효한 값에는 `2015-02-28` 이상 버전이 포함됩니다.
...
- **dataChangeDetectionPolicy**: 선택 사항입니다. 아래의 [데이터 변경 감지 정책](#DataChangeDetectionPolicy)을 참조하세요.

- **dataDeletionDetectionPolicy**: 선택 사항입니다. 아래의 [데이터 삭제 감지 정책](#DataDeletionDetectionPolicy)을 참조하세요.
...
대상 Azure 검색 인덱스가 아직 없으면 만듭니다. 이 작업은 [Azure 포털 UI](search-get-started.md#test-service-operations) 또는 [인덱스 만들기 API](https://msdn.microsoft.com/library/azure/dn798941.aspx)를 사용해서 수행할 수 있습니다.
...
- **예약**: 선택 사항입니다. 아래의 [인덱싱 예약](#IndexingSchedule)을 참조하세요.

###<a id="IndexingSchedule"></a>일정에 따라 인덱서 실행

인덱서는 선택적으로 일정을 지정할 수 있습니다. 일정이 제공된 경우, 인덱서가 일정에 따라 주기적으로 실행됩니다. 일정은 다음과 같은 특성을 갖습니다.

- **간격**: 필수입니다. 인덱서 실행 간격 또는 기간을 지정하는 기간 값입니다. 허용되는 가장 작은 간격은 5분이고 가장 긴 간격은 1일입니다. 형식은 XSD "dayTimeDuration" 값([ISO 8601 기간](http://www.w3.org/TR/xmlschema11-2/#dayTimeDuration) 값의 제한된 하위 집합)이어야 합니다. 패턴은 `P[nD][T[nH][nM]]`입니다. Examples: 15분마다의 경우 `PT15M`, 2시간마다의 경우 `PT2H`. 
...
축하합니다. 지금까지 DocumentDB의 인덱서를 사용해서 Azure 검색에 Azure DocumentDB를 통합하는 방법에 대해 배웠습니다.
 ...
 - Azure 검색에 대해 자세히 알아보려면 [여기](/services/search/)를 클릭하세요.
