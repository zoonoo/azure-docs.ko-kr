...
|**/bk:**<BitLockerKey\>|**선택 사항입니다.** 도구에서 원본 파일을 사용할 수 있는 암호화 된 드라이브의 잠금을 해제 하려면 BitLocker 키를 지정 합니다.|  
...
|**/PathMapFile:**<DrivePathMapFile\>|**선택 사항입니다.** 동일한 작업에서 가져온 것과 같은 이름의 파일이 여러 개 있는 경우 모호성을 해결 하는 데 사용 되는 텍스트 파일의 경로입니다. 이 도구는 처음 실행될 때 모호한 모든 이름으로 이 파일을 채울 수 있습니다. 이 도구를 나중에 실행 하면이 파일을 사용 하 여 모호성을 해결 합니다.|  
