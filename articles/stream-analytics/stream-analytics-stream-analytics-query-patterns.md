...
Azure Stream 분석의 쿼리는 [Stream Analytics 쿼리 언어 참조](https://msdn.microsoft.com/library/azure/dn834998.aspx) 가이드에 문서화된 SQL 유사 언어로 표현됩니다.  이 문서에서는 실제 시나리오에 따른 몇 가지 일반적인 쿼리 패턴에 대한 솔루션을 간략하게 설명합니다.  이 작업은 진행 중이며 새 패턴이 지속적으로 업데이트됩니다.

## <a name="query-example-data-type-conversions"></a>쿼리 예제: 데이터 형식 변환
**설명**: 입력 스트림의 속성 형식을 정의합니다.
예를 들어 자동차 무게가 입력 스트림에 문자열 형식으로 설정되면 합계를 수행하기 위해 INT로 변환해야 합니다.
...
## <a name="query-example-using-likenot-like-to-do-pattern-matching"></a>쿼리 예제: 패턴을 일치시키기 위해 Like/Not like를 사용합니다.
...
**설명**: 일부 조건에 따라 필드에 대해 다른 계산을 제공합니다.
...
첫 번째 쿼리는 ArchiveOutput이라고 지정한 출력에 받은 통과한 데이터입니다.
두 번째 쿼리는 일부 간단한 집계와 필터링을 하고 다운스트림 경고 시스템에 결과를 보냅니다.
*참고*: 여려 출력문에서 CTE(즉, WITH 문)의 결과를 재사용할 수 있습니다. 입력 원본의 판독기보다 더 적게 열 수 있는 추가적 이점을 가집니다.
예를 들면 다음과 같습니다. 
...
## <a name="query-example-counting-unique-values"></a>쿼리 예제: 고유 값 계산
...
**설명:** COUNT(DISTINCT Make)는 특정 기간 내에서 "Make" 열의 고유한 값 수를 반환합니다.
...
## <a name="query-example-find-first-event-in-a-window"></a>쿼리 예제: 창에서 첫 번째 이벤트 찾기
...
## <a name="query-example-find-last-event-in-a-window"></a>쿼리 예제: 창에서 마지막 이벤트 찾기
...
예를 들어 90초 이내에 동일한 브랜드의 차 2대가 연속해서 유료 도로로 진입했는지 확인합니다.
...
## <a name="query-example-detect-duration-between-events"></a>쿼리 예제: 이벤트 간 기간 검색
...
## <a name="query-example-detect-duration-of-a-condition"></a>쿼리 예제: 조건의 기간 감지
**설명**: 조건이 발생한 기간을 알아봅니다.
예를 들어 버그가 생겨서 모든 자동차의 중량이 정확하지 않은 경우(20.000파운드 이상) 버그의 기간을 계산할 수 있습니다.
...
**설명**:이 쿼리는 5초마다 이벤트를 생성하고 이전에 수신했던 마지막 이벤트를 출력합니다. [도약 창](https://msdn.microsoft.com/library/dn835041.aspx "도약 창 - Azure Stream Analytics") 기간은 쿼리가 마지막 이벤트를 찾는 데 얼마나 돌아가야 하는지를 결정합니다(이 예에서는&300;초).
