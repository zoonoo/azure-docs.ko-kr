...
쌍을 이루는 지역에 동일한 작업을 배포 하는 것이 좋습니다. 그런 다음 예기치 않은 상황이 발생 하면 [이러한 작업을 모니터링](./stream-analytics-set-up-alerts.md#scenarios-to-monitor) 하 여 알림 메시지를 표시 해야 합니다. Stream Analytics 서비스 업데이트 후 이러한 작업 중 하나가 [실패 한 상태로](./job-states.md) 종료 되 면 고객 지원에 문의 하 여 근본 원인을 쉽게 파악할 수 있습니다. 또한 다운스트림 소비자를 정상 작업 출력으로 장애 조치 (failover) 해야 합니다.
