<properties
    pageTitle="IoT 장치에서 데이터를 처리하도록 Azure Stream Analytics 시작 | Microsoft Azure"
...
3. **지역별 모니터링 저장소 계정** 설정에 대해 **새 저장소 계정 만들기**를 선택하고 고유한 이름을 지정합니다. Azure 스트림 분석은 이 계정을 사용하여 이후의 모든 작업에 대해 모니터링 정보를 저장합니다.
    ...
    > [AZURE.NOTE] 이 저장소 계정은 지역당 한 번만 만들어야 합니다. 이 저장소는 해당 지역에 생성되는 모든 Stream Analytics 작업에서 공유됩니다.
