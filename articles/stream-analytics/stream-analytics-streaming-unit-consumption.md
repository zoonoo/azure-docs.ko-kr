...
스트리밍 단위(SUs)는 스트림 분석 작업을 실행하기 위해 할당된 컴퓨팅 리소스를 나타냅니다. SU 수가 클수록 작업에 더 많은 CPU 및 메모리 리소스가 할당됩니다. 이러한 용량을 통해 쿼리 논리에 중점을 두고 Stream Analytics 작업을 적시에 실행하도록 하드웨어를 관리해야 할 필요성을 요약할 수 있습니다.
...
5. 작업에 할당된 S의 수는 실행 중인 경우에도 변경할 수 있습니다. 작업이 [분할되지 않은 출력을](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-parallelization#query-using-non-partitioned-output) 사용하거나 [다른 PARTITION BY 값을 가진 다중 단계 쿼리가](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-parallelization#multi-step-query-with-different-partition-by-values)있는 경우에는 불가능합니다. 작업이 실행 중일 때 SU 값 집합에서 선택하도록 제한할 수 있습니다. 
...
> [!Note]
> 특정 작업에 필요한 SU 수 선택은 입력에 대한 파티션 구성 및 작업에 정의된 쿼리에 따라 달라집니다. 작업에 대해 SU의 할당량까지 선택할 수 있습니다. 기본적으로 각 Azure 구독에는 특정 지역의 모든 분석 작업에 대해 최대 500개의 SUs가 할당됩니다. 구독의 SU를 이 할당량을 초과하여 늘리려면 [Microsoft 지원](https://support.microsoft.com)에 문의하세요. 작업당 SU에 대한 유효한 값은 1, 3, 6이며 6 단위로 증가합니다.
...
SU% 사용률은 잠시 동안 갑자기 0으로 떨어졌다가 정상 수준으로 돌아올 수도 있습니다. 이러한 현상은 일시적인 오류 또는 시스템에서 시작한 업그레이드로 인해 발생합니다. 쿼리가 [완전히 병렬상태가](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-parallelization)아닌 경우 작업에 대한 스트리밍 단위 수가 증가하면 SU% 사용률이 줄어들지 않을 수 있습니다.

기간 동안의 사용률을 비교하는 동안 [이벤트 비율 메트릭을](stream-analytics-monitoring.md)사용합니다. 입력이벤트 및 OutputEvents 메트릭은 읽고 처리한 이벤트 수를 보여 준다. 직렬화 오류와 같은 오류 이벤트 수를 나타내는 메트릭도 있습니다. 시간 단위당 이벤트 수가 증가하면 대부분의 경우 SU%가 증가합니다.
...
이전 쿼리에서 높은 카디널리티로 인한 문제를 완화하기 위해 `clusterid`에서 분할된 Event Hub로 이벤트를 보내고 아래 예제와 같이 PARTITION **BY를** 사용하여 시스템이 각 입력 파티션을 별도로 처리하도록 하여 쿼리를 확장할 수 있습니다.
...
시간 조인의 소비된 메모리(상태 크기)는 조인의 시간 흔들기 룸에 있는 이벤트 수에 비례하며, 이벤트 입력 속도에 흔들기 룸 크기를 곱한 값입니다. 즉, 조인에 의해 소비되는 메모리는 DateDiff 시간 범위에 평균 이벤트 속도를 곱한 값에 비례합니다.
...
이 작업을 해결하려면 조인 키(이 경우 ID)로 분할된 이벤트 허브로 이벤트를 보내고, 그림과 같이 **파티션 BY를** 사용하여 시스템이 각 입력 파티션을 별도로 처리하도록 하여 쿼리를 확장합니다.
...
* [Azure Stream Analytics 작업을 확장하여 처리량을 높입니다.](stream-analytics-scale-jobs.md)
