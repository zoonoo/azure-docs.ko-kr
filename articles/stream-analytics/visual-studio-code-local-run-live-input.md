...
1. Stream Analytics 프로젝트에서 **입력** 폴더를 마우스 오른쪽 단추로 클릭 합니다. 그런 다음 상황에 맞는 메뉴에서 **global.asa: 입력 추가** 를 선택 합니다.

   ![입력 폴더에서 입력 추가](./media/quick-create-vs-code/add-input-from-inputs-folder.png)
...
2. 드롭다운 목록에서 입력 원본 유형을 선택 합니다.
   ...
   ![입력 옵션으로 IoT hub를 선택 합니다.](./media/quick-create-vs-code/iot-hub.png)

3. 명령 팔레트에서 입력을 추가한 경우 입력을 사용 하는 Stream Analytics 쿼리 스크립트를 선택 합니다. **Script.asaql**에 대 한 파일 경로로 자동으로 채워집니다.

   ![에서 Stream Analytics 스크립트를 선택 Visual Studio Code](./media/quick-create-vs-code/asa-script.png)

4. 드롭다운 메뉴에서 **Azure 구독에서 선택을** 선택 합니다.

    ![구독에서 선택](./media/quick-create-vs-code/add-input-select-subscription.png)

5. 새로 생성 된 JSON 파일을 구성 합니다. CodeLens 기능을 사용 하 여 문자열을 입력 하거나, 드롭다운 목록에서 선택 하거나, 파일에서 직접 텍스트를 변경할 수 있습니다. 다음 스크린샷은 **구독에서 선택 하** 는 예를 보여 줍니다.
...
* [Npm 패키지를 사용 하 여 CI/CD 파이프라인 설정](setup-cicd-vs-code.md)
