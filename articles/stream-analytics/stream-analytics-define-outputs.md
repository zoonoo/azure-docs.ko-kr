...
다양한 응용 프로그램 패턴을 사용하기 위해 Azure 스트림 분석은 작업 출력을 저장 및 표시하는 7가지 다른 방법을 제공합니다. SQL 데이터베이스, Blob 저장소, 이벤트 허브, 서비스 버스 큐, 서비스 버스 항목, Power BI 및 테이블 저장소는 모두 지원되는 출력입니다. 작업 출력을 쉽게 볼 수 있는 편의성과 데이터 웨어하우징 및 기타 용도로 작업 출력을 사용 및 저장할 수 있는 유연성을 모두 제공합니다.
...
| 구독 | 사용자 고유의 구독에서 데이터베이스를 선택하거나 다른 구독의 데이터베이스에 연결 정보를 제공할 수 있습니다. |
...
<tr>
<td>구독</td>
<td>사용자 고유의 구독에서 Blob 저장소 계정을 선택하거나 다른 구독의 Blob 저장소 계정에 연결 정보를 제공할 수 있습니다.</td>
</tr>
<tr>
<td>저장소 계정</td>
...
<tr>
<td>구분 기호</td>
<td>스트림 분석은 CSV 데이터를 직렬화하기 위해 다양하고 일반적인 구분 기호를 지원합니다. 지원되는 값은 쉼표, 세미콜론, 공백, 탭 및 세로 막대입니다.</td>
</tr>
<tr>
<td>형식</td>
...
[이벤트 허브](https://azure.microsoft.com/services/event-hubs/)는 확장성이 뛰어난 게시-구독 이벤트 ingestor입니다. 초당 수 백만의 이벤트를 수집할 수 있습니다. 출력으로 이벤트 허브를 사용하는 한 가지 예는 스트림 분석 작업의 출력이 다른 스트리밍 작업의 입력이 되는 경우입니다.
...
| 구독 | 사용자 고유의 구독에서 이벤트 허브를 선택하거나 다른 구독의 데이터베이스에 연결 정보를 제공할 수 있습니다. |
...
| 구분 기호 | 스트림 분석은 CSV 형식에서 데이터를 직렬화하기 위해 다양하고 일반적인 구분 기호를 지원합니다. 지원되는 값은 쉼표, 세미콜론, 공백, 탭 및 세로 막대입니다. |
| 형식 | JSON 형식에만 적용됩니다. 구분된 줄은 출력이 각 JSON 개체를 새 줄로 구분된 형식이 되도록 지정합니다. 배열은 출력의 형식을 JSON 개체의 배열로 지정합니다. |
## Power BI

[Power BI](https://powerbi.microsoft.com/)을 스트림 분석 작업에 대한 출력으로 사용하여 스트림 분석 사용자에게 풍부한 시각화 환경을 제공할 수 있습니다. 작업 대시보드, 보고서 생성 및 메트릭 제어 보고에 이 기능을 이용할 수 있습니다.

### Power BI 계정 권한 부여

1.	Power BI를 출력으로 선택하는 경우 메시지가 표시되어 기존 Power BI 사용자 권한을 부여하거나 새 Power BI 계정을 만듭니다.  
...
| 구독 | 사용자 고유의 구독에서 테이블 저장소 계정을 선택하거나 저장소 계정이 다른 구독에서 오면 연결 정보를 제공할 수 있습니다. |
...
| 구독 | 사용자 고유의 구독에서 서비스 버스 네임스페이스를 선택하거나 네임스페이스가 다른 구독에서 오면 연결 정보를 제공할 수 있습니다. |
...
| 구독 | 사용자 고유의 구독에서 서비스 버스 네임스페이스를 선택하거나 네임스페이스가 다른 구독에서 오면 연결 문자열 정보를 제공할 수 있습니다. |
| 서비스 버스 네임스페이스 | 서비스 버스 네임스페이스는 메시징 엔터티 집합에 대한 컨테이너입니다. 새 이벤트 허브를 만들 때 서비스 버스 네임스페이스도 만들었습니다. |
| 항목 이름 | 항목은 이벤트 허브 및 큐와 유사한 메시징 엔터티입니다. 다양한 장치 및 서비스에서 이벤트 스트림을 수집하도록 설계됩니다. 토픽을 만들 때 특정 이름도 지정됩니다. 구독을 만들지 않으면 항목에 전송된 메시지를 사용할 수 없으므로 항목 아래에 구독이 하나 이상 있는지 확인합니다. |
| 항목 정책 이름 | 또한 항목을 만들 때 항목 구성 탭에서 공유 액세스 정책을 만들 수 있습니다. 각 공유 액세스 정책에는 이름, 사용자가 설정한 사용 권한 및 액세스 키가 있습니다. |
| 항목 정책 키 | 서비스 버스 네임스페이스에 대한 액세스를 인증하는 데 사용되는 공유 선택키 |
| 이벤트 직렬화 형식 | 출력 데이터에 대한 직렬화 형식입니다. JSON, CSV, 및 Avro를 지원합니다. |
