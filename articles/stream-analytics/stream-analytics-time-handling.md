...
description: Azure Stream Analytics에서 최적의 시작 시간을 선택하는 방법, 늦은 및 초기 이벤트를 처리하는 방법, 메트릭을 처리하는 시간 등 시간 처리가 어떻게 작동하는지 알아봅니다.
author: mamccrea
ms.author: mamccrea
...
- **워터마크**: 스트리밍 프로세서에 어떤 지점 이벤트가 유입되었는지를 나타내는 이벤트 시간 마커입니다. 워터마크를 사용하면 시스템에서 이벤트 수집에 대한 명확한 진행률을 표시할 수 있습니다. 스트림의 특성에 따라 들어오는 이벤트 데이터가 중지되지 않으므로 워터마크는 스트림의 특정 시점까지의 진행률을 나타냅니다.
...
2. **애플리케이션 시간**(이벤트 시간이라고도 함)
   ...
   애플리케이션 시간은 이벤트를 생성할 때 할당되며 이벤트 페이로드의 일부입니다. 응용 프로그램 시간별로 이벤트를 처리하려면 선택 쿼리에서 **타임스탬프 를** 사용하여 절을 사용합니다. **Timestamp by** 절이 없으면 이벤트는 도착 시간을 기준으로 처리됩니다.
...
그러면 Stream Analytics는 쿼리 사양에서 시작 시간을 파생시킵니다. 그러나 입력 이벤트 브로커는 도착 시간만으로 인덱싱되므로 시스템에서 시작 이벤트 시간을 도착 시간으로 변환해야 합니다. 시스템은 입력 이벤트 브로커의 해당 지점에서 이벤트 처리를 시작할 수 있습니다. 조기 도착 시간 제한을 사용하여 간단히 변환됩니다. 시작 이벤트 시간에서 5분 일찍 도착하는 창에서 뺀 값입니다. 이 계산은 또한 시스템이 도착 시간보다 이벤트 시간이 5분 ealier인 것으로 보이는 모든 이벤트를 삭제한다는 것을 의미합니다.
...
1. 이러한 이벤트를 통해 진행되는 워터마크의 예:
   ...
   5. 열두 번째 이벤트(장치3)가 처리되면 도착 시간(12:27)이 이벤트 시간(12:21)보다 6분 앞당겨지게 됩니다. 지연 도착 정책이 적용됩니다. 이벤트 시간이 워터마크(12:21)보다 높은 12:22로 조정되어 조정이 더 이상 적용되지 않습니다.
