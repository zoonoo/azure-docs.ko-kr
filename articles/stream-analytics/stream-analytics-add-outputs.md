...
스트림 분석 출력을 사용하여 실시간 대시보드 또는 경고를 제공하거나, 다운스트림 워크플로를 트리거하거나, 나중에 일괄 처리하기 위해 데이터를 보관할 수 있습니다. 스트림 분석은 여기에 자세히 문서화되어 있는 여러 Azure 서비스와 높은 수준으로 통합됩니다.
...
2. 출력의 형식을 지정합니다.
    ...
    ![Azure 포털 데이터 형식 선택](./media/stream-analytics-add-outputs/6-stream-analytics-add-outputs.png)
...
3. **출력 별칭** 상자에 이 출력의 이름을 입력합니다. 이 이름은 나중에 작업 쿼리에서 출력을 참조하는 데 사용할 수 있습니다.
    ...
    ![속성 추가](./media/stream-analytics-add-outputs/3-stream-analytics-add-outputs.png)
...
4. 출력 형식에 따라 데이터를 직렬화하거나 형식을 지정하는 방법을 지정해야 할 수도 있습니다. 각 출력 형식에 대한 특정 직렬화 설정은 여기에 문서화되어 있습니다.
    ...
    ![Azure 포털 추가 이벤트 허브](./media/stream-analytics-add-outputs/7-stream-analytics-add-outputs.png)
