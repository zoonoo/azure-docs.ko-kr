...
Visual Studio에서 **파일** > **새로 만들기** > **프로젝트**를 차례로 선택합니다. 왼쪽의 **템플릿** 목록으로 이동하고, **Azure Stream Analytics** > **Stream Analytics Edge** > **Azure Stream Analytics Edge 응용 프로그램**을 차례로 펼칩니다. 프로젝트에 대한 이름, 위치 및 솔루션 이름을 제공하고 **확인**을 선택합니다.

![새 Edge 프로젝트](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-edge-project.png)

프로젝트가 만들어지면 **솔루션 탐색기**로 이동하여 폴더 계층 구조를 봅니다.

![솔루션 탐색기 보기](./media/stream-analytics-tools-for-visual-studio-edge-jobs/edge-project-in-solution-explorer.png)
...
2. 원본 유형이 **데이터 스트림**, 원본이 **Edge Hub**>, 이벤트 직렬화 형식이 **Json**, 인코딩이 **UTF8**로 설정되어 있는지 확인합니다. 필요에 따라 **입력 별칭**의 이름을 바꿀 수 있으며, 이 예제에서는 그대로 두겠습니다. 입력 별칭의 이름을 바꾸는 경우 쿼리를 정의할 때 지정한 이름을 사용합니다. **저장**을 선택하여 설정을 저장합니다.  
   ![입력 구성](./media/stream-analytics-tools-for-visual-studio-edge-jobs/stream-analytics-input-configuration.png)
...
2. 싱크가 **Edge Hub**, 이벤트 직렬화 형식이 **Json**, 인코딩이 **UTF8**, 형식이 **배열**로 설정되어 있는지 확인합니다. 필요에 따라 **출력 별칭**의 이름을 바꿀 수 있으며, 이 예제에서는 그대로 두겠습니다. 출력 별칭의 이름을 바꾸는 경우 쿼리를 정의할 때 지정한 이름을 사용합니다. **저장**을 선택하여 설정을 저장합니다. 
   ![출력 구성](./media/stream-analytics-tools-for-visual-studio-edge-jobs/stream-analytics-output-configuration.png)
...
2. 팝업 창의 로컬 경로에서 샘플 데이터를 **찾아보고** **저장**을 선택합니다.
   ![로컬 입력 구성](./media/stream-analytics-tools-for-visual-studio-edge-jobs/stream-analytics-local-input-configuration.png)
...
5. 명령 프롬프트 창에 작업의 상태가 표시됩니다. 작업이 성공적으로 실행되면 프로젝트 폴더 경로("Visual Studio 2015\Projects\MyASAEdgejob\MyASAEdgejob\ASALocalRun\2018-02-23-11-31-42")에 "2018-02-23-11-31-42"와 같은 폴더가 만들어집니다. 폴더 경로로 이동하여 로컬 폴더의 결과를 봅니다.
   ...
   ![결과 폴더](./media/stream-analytics-tools-for-visual-studio-edge-jobs/result-folder.png)
...
서버 탐색기에서 작업 및 작업 다이어그램의 상태를 볼 수 있습니다. **서버 탐색기** > **Stream Analytics**에서 Edge 작업을 배포한 구독 및 리소스 그룹을 펼치면, 상태가 **만들어짐**인 MyASAEdgejob을 볼 수 있습니다. 작업 노드를 펼치고, 해당 작업 노드를 두 번 클릭하여 작업 보기를 엽니다.

![서버 탐색기 옵션](./media/stream-analytics-tools-for-visual-studio-edge-jobs/server-explorer-options.png)
 
작업 보기 창에서는 Azure Portal의 작업 새로 고침, 작업 삭제, 열기와 같은 작업을 제공합니다.

![작업 다이어그램 및 기타 옵션](./media/stream-analytics-tools-for-visual-studio-edge-jobs/job-diagram-and-other-options.png) 

## <a name="next-steps"></a>다음 단계

* [Azure Iot Edge에 대한 추가 정보](../iot-edge/how-iot-edge-works.md)
