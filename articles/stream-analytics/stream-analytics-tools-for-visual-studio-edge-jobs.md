---
title: Azure Stream Analytics tools for Visual Studio에서에서 Stream Analytics 데이터 상자 Edge 작업
description: 이 문서 작성, 디버그 및 Visual Studio 용 Stream Analytics 도구를 사용 하 여 Stream Analytics 데이터 상자 Edge 작업을 만드는 방법을 설명 합니다.
...
ms.locfileid: "66304151"
---
# <a name="develop-stream-analytics-data-box-edge-jobs-using-visual-studio-tools"></a>Visual Studio 도구를 사용 하 여 Stream Analytics 데이터 상자 Edge 작업 개발

이 자습서에서는 Visual Studio 용 Stream Analytics 도구를 사용 하는 방법을 알아봅니다. 작성, 디버그 및 Stream Analytics 데이터 상자 Edge 작업을 만드는 방법에 알아봅니다. 작업을 만들고 테스트한 후에는 Azure Portal로 이동하여 디바이스에 배포할 수 있습니다. 
...
## <a name="create-a-stream-analytics-data-box-edge-project"></a>Stream Analytics 데이터 상자 Edge 프로젝트 만들기 
...
![Visual Studio에서 새 Stream Analytics 데이터 상자 Edge 프로젝트](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-stream-analytics-edge-project.png)

프로젝트가 만들어지면 **솔루션 탐색기**로 이동하여 폴더 계층 구조를 봅니다.

![Stream Analytics 데이터 상자 Edge 작업의 솔루션 탐색기 뷰](./media/stream-analytics-tools-for-visual-studio-edge-jobs/edge-project-in-solution-explorer.png)
...
Stream Analytics 데이터 상자 Edge 환경에 배포 하는 Stream Analytics 작업을 대부분 지원 [Stream Analytics 쿼리 언어 참조](https://msdn.microsoft.com/azure/stream-analytics/reference/stream-analytics-query-language-reference?f=255&MSPPError=-2147217396)합니다. 그러나 Stream Analytics 데이터 가장자리가 상자의 작업에 대 한 다음 작업을 아직 지원 하지 않습니다. 
...
포털에서 Stream Analytics 데이터 상자 Edge 작업을 만들 때 컴파일러는 자동으로 경고 지원 되는 연산자를 사용 하지 않는 경우.
...
3. 팝업 창이 열립니다. 기존 Stream Analytics 데이터 상자 Edge 작업을 업데이트 하거나 새로 만들기를 선택 합니다. 기존 작업을 업데이트 하면이 시나리오에서는 모든 작업 구성이 대체할 경우 새 작업을 게시 해 봅니다. **새 Azure Stream Analytics 작업 만들기**를 선택하고, **MyASAEdgeJob**과 같은 작업 이름을 입력하고, 필요한 **구독**, **리소스 그룹** 및 **위치**를 선택하고, **제출**을 선택합니다.
   ...
   이제 Stream Analytics 데이터 가장자리가 상자의 작업 작성 되었습니다. 참조할 수 있습니다 합니다 [IoT Edge 자습서에서 작업을 실행할](stream-analytics-edge.md) 장치에 배포 하는 방법을 알아보려면. 

## <a name="manage-the-job"></a>작업 관리 

서버 탐색기에서 작업 및 작업 다이어그램의 상태를 볼 수 있습니다. **Stream Analytics** 에 **서버 탐색기**, 구독 및 Stream Analytics 데이터 상자 Edge 작업을 배포할 리소스 그룹을 확장 합니다. MyASAEdgejob의 상태가 **생성됨**인 것을 확인할 수 있습니다. 작업 노드를 펼치고, 해당 작업 노드를 두 번 클릭하여 작업 보기를 엽니다.
