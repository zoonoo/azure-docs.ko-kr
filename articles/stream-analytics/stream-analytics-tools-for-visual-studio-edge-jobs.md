---
title: 비주얼 스튜디오에서 Azure 스트림 분석 에지 작업
description: 이 문서에서는 Visual Studio용 스트림 분석 도구를 사용하여 IoT Edge 작업에서 스트림 분석을 작성, 디버깅 및 만드는 방법에 대해 설명합니다.
author: su-jie
ms.author: sujie
...
이 자습서에서는 Visual Studio에 스트림 분석 도구를 사용하는 방법을 알아봅니다. Stream Analytics Edge 작업을 작성, 디버깅 및 만드는 방법을 배웁니다. 작업을 만들고 테스트한 후에는 Azure Portal로 이동하여 디바이스에 배포할 수 있습니다. 

## <a name="prerequisites"></a>사전 요구 사항

이 자습서를 완료하려면 다음 필수 구성 요소가 필요합니다.

* [설치 비주얼 스튜디오 2019,](https://visualstudio.microsoft.com/downloads/)비주얼 스튜디오 [2015,](https://www.visualstudio.com/vs/older-downloads/)또는 [비주얼 스튜디오 2013 업데이트 4](https://www.microsoft.com/download/details.aspx?id=45326). Enterprise(Ultimate/Premium), Professional 및 Community Edition이 지원됩니다. Express Edition은 지원되지 않습니다.  

* 설치 [지침에](stream-analytics-tools-for-visual-studio-edge-jobs.md) 따라 Visual Studio용 스트림 분석 도구를 설치합니다.
 
## <a name="create-a-stream-analytics-edge-project"></a>Stream Analytics Edge 프로젝트 만들기 

비주얼 스튜디오에서 새**프로젝트** **파일** > 을**선택합니다.** >  왼쪽의 **템플릿** 목록으로 이동하고, **Azure Stream Analytics** > **Stream Analytics Edge** > **Azure Stream Analytics Edge 애플리케이션**을 차례로 펼칩니다. 프로젝트에 대한 이름, 위치 및 솔루션 이름을 제공하고 **확인**을 선택합니다.

![비주얼 스튜디오의 새로운 스트림 분석 에지 프로젝트](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-stream-analytics-edge-project.png)
...
2. 싱크를 선택하여 **Edge Hub를**선택하고, 이벤트 직렬화 형식을 **Json으로**설정하고, 인코딩을 **UTF8로**설정하고, 형식 **배열을**설정해야 합니다. 필요에 따라 **출력 별칭**의 이름을 바꿀 수 있으며, 이 예제에서는 그대로 두겠습니다. 출력 별칭의 이름을 바꾸는 경우 쿼리를 정의할 때 지정한 이름을 사용합니다. **저장**을 선택하여 설정을 저장합니다. 
...
스트림 분석 IoT 에지 환경에 배포된 스트림 분석 작업은 대부분의 [스트림 분석 쿼리 언어 참조를](https://msdn.microsoft.com/azure/stream-analytics/reference/stream-analytics-query-language-reference?f=255&MSPPError=-2147217396)지원합니다. 그러나 Stream Analytics Edge 작업에는 다음 작업이 아직 지원되지 않습니다. 


|**범주**  | **명령**  |
|---------|---------|
|기타 연산자 | <ul><li>PARTITION BY</li><li>TIMESTAMP BY OVER</li><li>자바 스크립트 UDF</li><li>사용자 정의 집계(UDA)</li><li>GetMetadataPropertyValue</li><li>한 단계에서 14개 이상의 집계 사용</li></ul>   |

포털에서 [분석 가장자리 스트림] 작업을 만들면 지원되는 연산자를 사용하지 않는 경우 컴파일러가 자동으로 경고합니다.
...
4. 로컬로 실행하거나 Azure에 제출할 수 있습니다. 쿼리를 테스트하려면 **로컬 로 실행을**선택합니다.  
...
3. 팝업 창이 열립니다. 기존 Stream Analytics Edge 작업을 업데이트하거나 새 작업을 만들도록 선택합니다. 기존 작업을 업데이트하면 모든 작업 구성이 대체되며 이 시나리오에서는 새 작업을 게시합니다. **새 Azure Stream Analytics 작업 만들기**를 선택하고, **MyASAEdgeJob**과 같은 작업 이름을 입력하고, 필요한 **구독**, **리소스 그룹** 및 **위치**를 선택하고, **제출**을 선택합니다.
   ...
   이제 스트림 애널리틱스 에지 작업이 만들어졌습니다. [IoT Edge의 작업 실행 자습서를](stream-analytics-edge.md) 참조하여 장치에 배포하는 방법을 알아볼 수 있습니다. 

## <a name="manage-the-job"></a>작업 관리 

서버 탐색기에서 작업 및 작업 다이어그램의 상태를 볼 수 있습니다. **서버 탐색기의** **스트림 분석에서** 구독 및 스트림 애널리틱스 에지 작업을 배포한 리소스 그룹을 확장합니다. MyASAEdgejob의 상태가 **생성됨**인 것을 확인할 수 있습니다. 작업 노드를 펼치고, 해당 작업 노드를 두 번 클릭하여 작업 보기를 엽니다.
