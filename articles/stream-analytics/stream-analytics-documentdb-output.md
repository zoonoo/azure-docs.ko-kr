...
|컨테이너 이름 | 과 `MyContainer`같은 컨테이너 이름입니다. 라는 `MyContainer` 하나의 컨테이너가 있어야 합니다.  |
|문서 ID     | 선택 사항입니다. 삽입 또는 업데이트 작업의 기준으로 사용해야 하는 고유 키로 사용되는 출력 이벤트의 열 이름입니다. 비워 두면 업데이트 옵션 없이 모든 이벤트가 삽입 됩니다.|
