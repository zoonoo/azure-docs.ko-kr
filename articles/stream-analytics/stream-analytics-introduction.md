...
## Stream Analytics를 무엇에 사용할 수 있습니까?
...
Stream Analytics 시나리오로는 다음과 같은 것들이 있습니다.
...
다음 다이어그램은 프레젠테이션 또는 작업을 위해 어떻게 데이터가 수집되고, 분석되고, 전송되는지 나타낸 Streaming Analytics 파이프라인을 보여 줍니다. 
...
스트림을 검사하기 위해 데이터 출처를 지정하는 Stream Analytics *작업*을 만듭니다. 작업은 또한 *변형*&mdash;데이터, 패턴 또는 관계를 검색하는 방법을 지정합니다. 이 작업의 경우 Stream Analytics는 일정 시간 동안 스트리밍 데이터를 필터, 정렬, 집계 및 조인할 수 있도록 SQL 방식 쿼리 언어를 지원합니다.
...
Stream Analytics는 들어오는 데이터를 초당 최대 1GB의 속도로 처리할 수 있습니다. 다시 말해서 [Azure Event Hubs](https://azure.microsoft.com/services/event-hubs/)와 [Azure IoT Hubs](https://azure.microsoft.com/services/iot-hub/)의 통합을 통해 연결된 장치, 클릭 스트림, 로그 파일 등에서 발생하는 초당 수백만 개의 이벤트를 수집할 수 있는 작업을 구현할 수 있습니다. 이벤트 허브의 파티션 기능을 사용 하면 계산을 논리 단계로 분할할 수 있고, 좀 더 분할하면 확장성을 높일 수 있습니다.
...
* 사기성 호출을 찾기 위해 전화 메타데이터를 검사하는 [종단 간 Streaming Analytics 솔루션](stream-analytics-real-time-fraud-detection.md)을 빌드합니다.
* Stream Analytics용 SQL형 쿼리 언어와 [window 함수](stream-analytics-window-functions.md)와 고유한 개념에 대해 자세히 알아보세요.
* [Stream Analytics 작업 크기 조정](stream-analytics-scale-jobs.md) 방법에 대해 알아보세요. 
* [Stream Analytics 및 Azure Machine Learning 통합](stream-analytics-machine-learning-integration-tutorial.md) 방법에 대해 알아보세요.
