...
이 문서에서는 관리 되는 Id 인증 모드를 사용 하 여 Stream Analytics 작업을 Azure SQL Database 또는 Azure Synapse Analytics SQL 풀에 연결 하는 데 필요한 단계를 보여 줍니다. 

- 먼저 Stream Analytics 작업에 대 한 시스템 할당 관리 id를 만듭니다. Azure Active Directory의 작업 id입니다.  

- SQL server 또는 Synapse 작업 영역에 Active Directory 관리자를 추가 하 여 해당 리소스에 대해 Azure AD (관리 Id) 인증을 사용 하도록 설정 합니다.

- 그런 다음 데이터베이스에서 Stream Analytics 작업의 id를 나타내는 포함 된 사용자를 만듭니다. Stream Analytics 작업이 SQL DB 또는 Synapse SQL DB 리소스와 상호 작용할 때마다이 id는 Stream Analytics 작업의 사용 권한을 확인 하기 위해 참조 하는 id입니다.

- Stream Analytics 작업에 SQL Database 또는 Synapse SQL 풀에 액세스할 수 있는 권한을 부여 합니다.

- 마지막으로, Stream Analytics 작업에서 Azure SQL Database/Azure Synapse Analytics를 출력으로 추가 합니다.
...
1. Azure SQL Database 또는 Azure Synapse Analytics SQL 풀 리소스로 이동 하 고 리소스가 아래에 있는 SQL Server 또는 Synapse 작업 영역을 선택 합니다. 리소스 개요 페이지에서 *서버 이름* 또는 *작업 영역 이름* 옆에 있는 링크를 찾을 수 있습니다.

1. **설정** 아래에서 SQL Server 및 Synapse 작업 영역에 대해 각각 **Active Directory 관리** 또는 **SQL Active Directory 관리자** 를 선택 합니다. 그런 다음, **관리자 설정** 을 선택합니다.

   ![Active Directory 관리자 페이지](./media/sql-db-output-managed-identity/active-directory-admin-page.png)

1. Active Directory 관리자 페이지에서 SQL Server의 관리자가 될 사용자 또는 그룹을 검색하고 **선택** 을 클릭합니다. 다음 섹션에서 **포함 된 데이터베이스 사용자** 를 만들 수 있는 사용자가 됩니다.

   ![Active Directory 관리자 추가](./media/sql-db-output-managed-identity/add-admin.png)

   Active Directory 관리 페이지에 해당 Active Directory에 모든 멤버와 그룹이 표시됩니다. Azure Active Directory 관리자는 지원 되지 않으므로 회색으로 표시 된 사용자 또는 그룹은 선택할 수 없습니다.  ****    [SQL Database 또는 Azure Synapse 인증에 대 한 Azure Active Directory 인증 사용](../azure-sql/database/authentication-aad-overview.md#azure-ad-features-and-limitations)의 Azure Active Directory 기능 및 제한 사항 섹션에서 지원 되는 관리자 목록을 참조 하세요.
...
자세한 내용은 Azure AD 통합의 배경: [SQL Database 및 Azure Synapse Analytics를 사용한 유니버설 인증 (MFA에 대 한 SSMS 지원)](../azure-sql/database/authentication-mfa-ssms-overview.md) 문서를 참조 하세요.
...
1. 처음으로 연결하면 다음 창이 표시될 수 있습니다.
   ...
   1. 그렇다면 Azure Portal에서 SQL Server/Synapse 작업 영역 리소스로 이동 합니다. **보안** 섹션에서 **방화벽 및 가상 네트워크/방화벽** 페이지를 엽니다. 
...
1. 연결 되 면 포함 된 데이터베이스 사용자를 만듭니다. 다음 SQL 명령은 Stream Analytics 작업과 동일한 이름을 사용하는 포함된 데이터베이스 사용자를 만듭니다. *ASA_JOB_NAME* 을 대괄호를 묶어야 합니다. 다음 T-SQL 구문을 사용하여 쿼리를 실행합니다. 
   ...
   ```
    ...
    포함 된 데이터베이스 사용자를 올바르게 추가 했는지 확인 하려면 해당 데이터베이스의 SSMS에서 다음 명령을 실행 하 고 *ASA_JOB_NAME* "이름" 열 아래에 있는지 확인 합니다.
...
1. Microsoft Azure Active Directory에서 Stream Analytics 작업이 SQL Database에 액세스했는지 확인하려면 데이터베이스와 통신할 Azure Active Directory 권한을 부여해야 합니다. 이렇게 하려면 Azure Portal의 "방화벽 및 가상 네트워크"/"방화벽" 페이지로 이동 하 고 "Azure 서비스 및 리소스에서이 서버/작업 영역에 액세스할 수 있도록 허용"을 사용 하도록 설정 합니다.
...
이전 섹션에 설명 된 것 처럼 포함 된 데이터베이스 사용자를 만들고 포털에서 Azure 서비스에 대 한 액세스 권한을 부여 하면 Stream Analytics 작업에 관리 id를 통해 Azure SQL database 리소스에 **연결할** 수 있는 권한이 관리 되는 id로 포함 됩니다. Stream Analytics 작업에 대 한 **선택** 및 **삽입** 권한을 부여 하는 것이 좋습니다 .이는 나중에 Stream Analytics 워크플로에서 필요 합니다. **SELECT** 권한을 통해 작업은 Azure SQL database의 테이블에 대 한 연결을 테스트할 수 있습니다. 입력 및 Azure SQL database 출력을 구성한 후에는 **INSERT** 권한을 사용 하 여 종단 간 Stream Analytics 쿼리를 테스트할 수 있습니다.

#### <a name="azure-synapse-analytics"></a>[Azure Synapse Analytics](#tab/azure-synapse)

이전 섹션에 설명 된 것 처럼 포함 된 데이터베이스 사용자를 만들고 포털에서 Azure 서비스에 대 한 액세스 권한을 부여 하면 Stream Analytics 작업에 관리 id를 통해 Azure Synapse 데이터베이스 리소스에 **연결할** 수 있는 권한이 관리 되는 id로 포함 됩니다. Stream Analytics 작업에 대 한 **선택**, **삽입** 및 **관리 데이터베이스 대량 작업** 을 추가 하는 것이 좋습니다 .이 작업은 나중에 Stream Analytics 워크플로에서 필요 하기 때문입니다. **SELECT** 권한을 사용 하면 작업에서 Azure Synapse 데이터베이스의 테이블에 대 한 연결을 테스트할 수 있습니다. **데이터베이스 대량 작업** **삽입** 및 관리 권한을 통해 입력 및 Azure Synapse 데이터베이스 출력을 구성한 후에는 종단 간 Stream Analytics 쿼리를 테스트할 수 있습니다.
...
1. **저장** 을 클릭 한 후에는 리소스에 대 한 연결 테스트가 자동으로 트리거됩니다. 성공적으로 완료 되 면 관리 되는 id 인증 모드를 사용 하 여 Azure SQL Database 또는 Synapse SQL Database에 연결 하도록 Stream Analytics 작업을 성공적으로 구성 했습니다. 
...
1. **저장** 을 클릭 한 후에는 리소스에 대 한 연결 테스트가 자동으로 트리거됩니다. 성공적으로 완료 되 면 이제 Stream Analytics를 사용 하 여 Azure Synapse Analytics 리소스에 대해 관리 Id 사용을 진행할 준비가 된 것입니다. 
