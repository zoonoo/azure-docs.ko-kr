<properties 
	...
	keywords="이벤트 처리, 데이터 스트림, 주요 개념, 직렬화"	
...
스트림 분석을 사용하면 다음과 같은 작업을 수행할 수 있습니다.
...
자세한 내용은 [Azure 스트림 분석 소개](stream-analytics-introduction.md)를 참조하세요.
...
각 스트림 분석 작업 정의는 작업에서 사용되고 변환될 하나 이상의 데이터 스트림 입력 소스를 포함해야 합니다. [Azure Blob 저장소](http://azure.microsoft.com/documentation/services/storage/) 및 [Azure 이벤트 허브](http://azure.microsoft.com/services/event-hubs/)는 데이터 스트림 입력된 소스로 지원됩니다. 이벤트 허브 입력 소스는 여러 다른 장치 및 서비스에서 이벤트 스트림을 수집하는 데 사용되며, Blob 저장소는 많은 양의 데이터를 수집하기 위한 입력 소스로 사용될 수 있습니다. Blob은 데이터를 스트리밍하지 않으므로 Blob의 레코드에 타임스탬프가 포함되지 않은 한, Blob을 통한 스트림 분석 작업은 본질적으로 일시적이지 않습니다.

### 참조 데이터
스트림 분석 기능은 또 다른 유형의 입력 소스인 참조 데이터도 지원하니다. 이 데이터는 상관 관계를 지정하고 조회를 수행하는 데 사용되는 보조 데이터로, 일반적으로 정적이며 자주 변경되지 않습니다. [Azure Blob 저장소](http://azure.microsoft.com/documentation/services/storage/)는 유일하게 지원되는 참조 데이터용 입력 소스입니다. 참조 데이터 소스 Blob은 크기가 50MB로 제한됩니다.
...
사용자는 제한된 시간 동안만 기다리도록 결정할 수도 있습니다. ‘최대로 허용되는 도착 지연’은 뒤에 지연되는 입력 파티션을 유지하며, ‘지연된 이벤트에 대한 동작’에 따라 이벤트에 대해 동작하고, 데이터가 나중에 도착하는 경우 해당 이벤트를 중단하거나 해당 이벤트의 타임스탬프를 조절하여 작업을 진행하도록 결정한 이후에 지연을 결정합니다. 이는 대기 시간이 중요하며 타임스탬프 이동이 허용되지만 입력이 균등하게 분배되지 않을 수 있는 경우에 유용합니다.
...
### 추가 리소스
입력 소스를 만드는 방법에 대한 자세한 내용은 [Azure 이벤트 허브 개발자 가이드](http://msdn.microsoft.com/library/azure/dn789972.aspx) 및 [Azure Blob 저장소 사용](../storage/storage-dotnet-how-to-use-blobs.md)을 참조하세요.
...
쿼리 언어에 대한 자세한 내용은 [Azure 스트림 분석 쿼리 언어 참조](http://go.microsoft.com/fwlink/?LinkID=513299)를 참조하세요.
...
- Azure 테이블 저장소 - Azure 테이블 저장소는 스키마에 제약 조건이 더 적은 구조화된 데이터 저장소입니다. 다른 스키마 및 다른 유형의 엔터티는 동일한 Azure 테이블에 저장될 수 있습니다. Azure 테이블 저장소는 지속적이고 효율적인 검색을 위해 데이터를 저장하는 데 사용할 수 있습니다. 자세한 내용은 [Azure 저장소 소개](../storage/storage-introduction.md) 및 [Azure 테이블 저장소에 대한 확장 가능한 분할 전략 설계](https://msdn.microsoft.com/library/azure/hh508997.aspx)를 참조하세요.
- Azure SQL 데이터베이스 - 이 출력 대상은 원래 관계형인 데이터나 데이터베이스에 호스트되고 있는 콘텐츠를 필요로 하는 응용 프로그램에 적합합니다.

## 스트리밍 단위
고객에 대한 보다 예측 가능한 성능 환경을 제공하기 위한 일환으로 Azure 스트림 분석은 SU(스트림 단위)를 사용하여 작업을 실행하기 위한 리소스 및 힘을 나타냅니다. SU는 CPU, 메모리의 혼합된 측정치 및 읽기/쓰기 속도를 기반으로 상대적 이벤트 처리 용량을 설명하는 방법을 제공합니다. 각 스트리밍 단위는 대략 1MB/초의 처리량에 해당합니다. 각각의 Azure 스트림 분석 작업에는 최소한 모든 작업의 기본값인 하나의 스트리밍 단위가 필요합니다. 작업에 대한 SU의 적합한 수 선택에 대해 더 알아보려면 [Azure 스트림 분석 작업 규모 지정](stream-analytics-scale-jobs.md)을 참조하세요.

## 작업 규모 지정

아래 정의된 SU % 사용률 메트릭은 Azure 스트림 분석 작업의 규모를 지정하는 데 필요한 표시기입니다. 높은 SU % 사용률의 결과는 쿼리에서 큰 창으로, 입력에서 대량의 이벤트로, 순서 비지정 허용 오차의 큰 창 또는 위의 조합으로 나타날 수 있습니다. 쿼리 분할 또는 쿼리를 더 많은 단계로 나누기 및 규모 지정 탭에서 더 많은 SU 추가는 모두 이러한 조건을 피하기 위한 전략입니다.

시스템이 일정한 양의 리소스를 사용하기 때문에 입력 이벤트 없이도 초기 리소스 사용률을 관찰할 수 있습니다. 시스템에서 사용되는 리소스의 양은 시간이 지남에 따라 변동될 수도 있습니다.

자세한 내용은 [Azure 스트림 분석 작업 규모 지정](stream-analytics-scale-jobs.md)을 참조하세요.
...
- SU % 사용률 - 하나 이상의 쿼리 단계에 대한 상대적 이벤트 처리 용량에 대한 표시입니다. 이 표시가 80% 이상에 도달하면 이벤트 처리가 지연되거나 진행을 중단할 가능성이 큽니다.
- 오류 - 스트림 분석 작업에 의해 발생한 오류 메시지의 수입니다.
- 입력 이벤트 - 이벤트 개수를 기준으로 스트림 분석 작업이 받은 데이터의 양입니다.
...
- **순서 비지정 관련 정책** - 순차적으로 스트림 분석 작업에 도착하지 않는 이벤트 처리를 위한 설정입니다. 시간 임계값을 지정하여 허용 오차 기간에 포함된 이벤트의 순서를 다시 지정하고 이 기간을 벗어나는 이벤트에 대해 **삭제** 또는 **조정** 중 수행할 조치를 결정할 수 있습니다. **삭제**는 순서 없이 수신된 모든 이벤트를 삭제하며 **조정**은 시스템을 변경합니다. 가장 최근에 수신한 순서가 지정된 이벤트의 타임스탬프 순서가 이벤트의 타임 스탬프입니다. 
...
- **로캘** - 이 설정을 사용하여 스트림 분석 작업에 대한 국제화 기본 설정을 지정할 수 있습니다. 데이터의 타임스탬프는 로캘과 관련이 없지만 여기에서 설정한 내용은 작업이 데이터를 구문 분석하고, 비교하고, 정렬하는 방식에 영향을 줍니다. 미리 보기 릴리스의 경우 **ko-KR**만 지원됩니다.

### 상태

Azure 포털에서 스트림 분석 작업의 상태를 검사할 수 있습니다. 실행 중인 작업은 **실행 중** 또는 **저하됨**의 두 가지 상태 중 하나일 수 있습니다. 이러한 각 상태에 대한 정의는 아래와 같습니다.
