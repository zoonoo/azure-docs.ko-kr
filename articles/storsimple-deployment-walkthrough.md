...
| 6 | 프록시 서버\(선택 사항\) | IP 주소/프록시 서버의 정규화 된 도메인 이름, 사용할 포트 | |
| 7 | Azure 저장소 계정 | 계정 이름 및 액세스 키와 같은 액세스 자격 증명, 저장소 계정당 | |
| 8 | 클라우드 저장소 암호화 키\(권장\) | 볼륨 컨테이너당 | |
| 9 | 호스트의 IQN | 호스트당 | |
...
다른 지역에 Azure 저장소 계정을 만들어야 하는 경우 단계별 지침은 [Azure 저장소 계정 정보](storage-create-storage-account.md)를 참조하세요.
