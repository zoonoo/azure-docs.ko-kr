...
SQL Server 인스턴스, 데이터베이스 및 하나 이상의 테이블에 대해 스트레치 데이터베이스를 사용하고 나면 콜드 데이터를 Azure로 자동으로 마이그레이션하기 시작합니다.

-   콜드 데이터를 별도 테이블에 저장하는 경우 전체 테이블을 마이그레이션할 수 있습니다.

-   테이블에 핫 및 콜드 데이터가 모두 포함된 경우 필터 조건자를 지정하여 마이그레이션할 행을 선택할 수 있습니다.
