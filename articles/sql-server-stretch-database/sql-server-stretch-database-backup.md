...
2.  스트레치 지원 데이터베이스에 대한 데이터베이스 마스터 키를 만듭니다. 자세한 내용은 [CREATE MASTER KEY(TRANSACT-SQL)](https://msdn.microsoft.com/library/ms174382.aspx)를 참조하세요.

3.  스트레치 지원 데이터베이스에 대한 데이터베이스 범위 자격 증명을 만듭니다. 자세한 내용은 [CREATE DATABASE SCOPED CREDENTIAL(TRANSACT-SQL)](https://msdn.microsoft.com/library/mt270260.aspx)을 참조하세요.

4.  저장 프로시저 [sys.sp\_rda\_reauthorize\_db(Transact-SQL)](https://msdn.microsoft.com/library/mt131016.aspx)를 실행하여 로컬 스트레치 지원 데이터베이스를 Azure에 다시 연결합니다. 이전 단계에서 만든 데이터베이스 범위 자격 증명을 sysname 또는 varchar(128) 값으로 제공합니다. varchar(max)를 사용하지 마세요.
...
![스트레치 데이터베이스 백업 다이어그램][StretchBackupImage1]
...
![백업 후 스트레치 데이터베이스][StretchBackupImage2]

저장 프로시저 **sys.sp\_rda\_reauthorize\_db**를 실행하여 로컬 데이터베이스와 해당 원격 끝점 간의 연결을 다시 설정해야 합니다. db\_owner만 이 작업을 수행할 수 있습니다. 이 저장 프로시저에는 원격 끝점의 관리자 사용자 이름 및 암호도 필요합니다. 따라서 대상 Azure 서버에 대한 관리자 로그인 및 암호를 제공해야 합니다.

![백업 후 스트레치 데이터베이스][StretchBackupImage3]
