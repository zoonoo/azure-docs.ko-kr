...
Azure 데이터 탐색기는 Azure 데이터 탐색기, [Application Insights (AI)](../app/app-insights-overview.md)및 [Log Analytics (LA)](./data-platform-logs.md)사이에서 서비스 간 쿼리를 지원 합니다. 그런 다음 Azure 데이터 탐색기 도구를 사용 하 여 Log Analytics/Application Insights 작업 영역을 쿼리하고 크로스 서비스 쿼리에서이 작업 영역을 참조할 수 있습니다. 이 문서에서는 서비스 간 쿼리를 만드는 방법 및 Azure 데이터 탐색기 웹 UI에 Log Analytics/Application Insights 작업 영역을 추가 하는 방법을 보여 줍니다.
