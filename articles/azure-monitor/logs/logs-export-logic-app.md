...
![되풀이 작업](media/logs-export-logic-app/recurrence-action.png)
...
![논리 앱 디자이너의 단계에 추가 되는 새 작업의 스크린샷 작업 선택에서 Azure Monitor 로그가 강조 표시 됩니다.](media/logs-export-logic-app/select-query-action-list.png)
...
![쿼리 실행 및 결과 시각화 라는 새 Azure Monitor 로그 동작에 대 한 설정의 스크린샷](media/logs-export-logic-app/run-query-list-action.png)
...
![JSON 작업 구문 분석 선택](media/logs-export-logic-app/select-parse-json.png)
...
![구성 작업 선택](media/logs-export-logic-app/select-compose.png)


**입력** 상자를 클릭 하 여 이전 작업의 값 목록을 표시 합니다. **JSON 구문 분석** 작업에서 **본문** 을 선택 합니다. 로그 쿼리에서 구문 분석 된 출력입니다.

[![작성 작업의 본문 선택](media/logs-export-logic-app/select-body-compose.png)](media/logs-export-logic-app/select-body-compose.png#lightbox)
...
![Blob 만들기 선택](media/logs-export-logic-app/select-create-blob.png)
