...
Log Analytics 작업 영역에서는 관리자가 [로그](../logs/data-platform-logs.md) 데이터의 흐름과 분리를 제어 하 고 특정 비즈니스 요구 사항을 해결 하는 아키텍처를 만들 수 있는 방법을 제공 합니다. [이 문서](../logs/design-logs-deployment.md) 에서는 작업 영역에 대 한 디자인, 배포 및 마이그레이션 고려 사항을 설명 하 고, [액세스 관리](../logs/manage-access.md) 문서는 로그 데이터에 대 한 사용 권한을 적용 하 고 관리 하는 방법을 설명 합니다. 서비스 공급자에게는 추가 고려 사항이 있습니다.
...
2. 중앙 위치로 Power BI: Power BI 여러 작업 영역에서 Log Analytics 작업 영역과 [Power BI](../visualize/powerbi.md)간의 통합을 사용 하 여 데이터를 내보낼 때 중앙 위치로 작동할 수 있습니다.
