...
Azure Key Vault에서 현재 키 버전을 선택하여 키 식별자 세부 정보를 가져옵니다.
...
>[!NOTE]
>키 회전은 자동 회전 또는 명시적 키 버전 업데이트의 두 가지 모드를 지원합니다. 최적의 접근 방식을 결정하려면 [키 회전](#key-rotation)을 참조하세요.
...
> - 클러스터의 `identity` `type`을 "없음"으로 설정하면 데이터에 대한 액세스 권한도 취소되지만 지원 요청을 열 수 없고 클러스터에서 `identity`를 다시 시작할 때 취소를 되돌릴 수 없으므로 이 접근 방식은 권장되지 않습니다.
...
## <a name="customer-managed-key-for-saved-queries"></a>저장된 쿼리에 대한 고객 관리형 키

Log Analytics에 사용되는 쿼리 언어는 표현적이며 쿼리에 추가하는 설명 또는 쿼리 구문에 중요한 정보를 포함할 수 있습니다. 일부 조직에서는 이러한 정보가 고객 관리형 키 정책에 따라 보호된 상태로 유지되어야 하며 암호화된 쿼리를 키로 저장해야 합니다. Azure Monitor를 사용하면 작업 영역에 연결될 때 키로 암호화된 *저장된 검색* 및 *로그 경고* 쿼리를 자체 스토리지 계정에 저장할 수 있습니다. 
...
BYOS(Bring Your Own Storage)를 작업 영역에 연결하면 서비스가 *저장된 검색* 및 *로그 경고* 쿼리를 스토리지 계정에 업로드합니다. 즉, Log Analytics 클러스터에서 데이터를 암호화하는 데 사용하는 것과 동일한 키 또는 다른 키를 사용하여 스토리지 계정과 [미사용 암호화 정책](../../storage/common/customer-managed-keys-overview.md)을 제어합니다. 그러나 해당 스토리지 계정과 관련된 비용은 사용자가 부담합니다. 
...
* 쿼리를 저장하기 위해 단일 스토리지 계정을 작업 영역에 연결할 수 있지만 *saved-searches* 및 *log-alerts* 쿼리 모두에 사용할 수 있습니다.
* 대시보드에 고정은 지원되지 않습니다.
...
*경고* 를 위해 스토리지 계정을 작업 영역에 연결합니다. *log-alerts* 쿼리는 스토리지 계정에 저장됩니다. 
...
- [이중 암호화](../../storage/common/storage-service-encryption.md#doubly-encrypt-data-with-infrastructure-encryption)는 지원되는 지역에서 2020년 10월부터 만들어진 클러스터에 자동으로 구성됩니다. 클러스터에서 GET 요청을 보내고 이중 암호화가 사용 가능한 클러스터의 `isDoubleEncryptionEnabled` 값이 `true`인지 관찰하여 클러스터가 이중 암호화로 구성되었는지 확인할 수 있습니다. 
  ...
  - 클러스터가 사용자가 할당한 관리 ID로 설정된 경우 `UserAssignedIdentities`를 `None`으로 설정하면 클러스터가 일시 중단되고 데이터에 대한 액세스가 차단되지만 지원 요청을 열지 않고는 해지를 되돌리거나 클러스터를 활성화할 수 없습니다. 이러한 제한은 시스템이 할당한 관리 ID에 적용되지 않습니다.
