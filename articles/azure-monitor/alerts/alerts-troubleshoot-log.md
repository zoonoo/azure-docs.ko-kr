...
현재 ScheduledQueryRules API를 사용 하면 예상 대로 작동 하는 [메트릭 측정](alerts-unified-log.md#calculation-of-measure-based-on-a-numeric-column-such-as-cpu-counter-value) 규칙에서 **집계** 를 설정할 수 있습니다. [현재 SCHEDULEDQUERYRULES API로 전환 하는 방법에 대해 자세히 알아보세요](../alerts/alerts-log-api-switch.md).
...
- 다음 이유로 인해 [쿼리가 실패](https://dev.loganalytics.io/documentation/Using-the-API/Errors) 합니다.
    ...
    - 데이터 흐름이 시작 되지 않았으므로 [사용자 지정 로그 테이블이](../agents/data-sources-custom-logs.md) 아직 만들어지지 않았습니다.
