...
ms.locfileid: "100620964"
---
# <a name="create-view-and-manage-log-alerts-using-azure-monitor"></a>Azure Monitor를 사용하여 로그 경고 만들기, 보기 및 관리

## <a name="overview"></a>개요

로그 경고를 통해 사용자는 [Log Analytics](../log-query/log-analytics-tutorial.md) 쿼리를 사용 하 여 설정 된 빈도 마다 리소스 로그를 평가 하 고 결과에 따라 경고를 발생 시킬 수 있습니다. 규칙은 [작업 그룹](../platform/action-groups.md)을 사용 하 여 하나 이상의 작업을 트리거할 수 있습니다. [로그 경고의 기능 및 용어에 대해 자세히 알아보세요](../platform/alerts-unified-log.md).
...
[별도의 문서](../alerts/alerts-log-create-templates.md)에서 설명 하는 Azure Resource Manager 템플릿을 사용 하 여 로그 경고 규칙을 만들 수도 있습니다.
...
> [!NOTE]
> 리소스 그룹 또는 구독 범위를 사용 하 여 여러 리소스에서 실행 되는 로그에 대해 리소스 액세스 모드를 사용 하는 경우에는 규모에 따라 경고를 만드는 것이 좋습니다. 크기에 대 한 경고는 규칙 관리 오버 헤드를 줄입니다. 리소스를 대상으로 지정할 수 있으려면 결과에 리소스 ID 열을 포함 하세요. [차원으로 경고 분할에 대해 자세히 알아보세요](../platform/alerts-unified-log.md#split-by-alert-dimensions).
...
1. [**기간**](../platform/alerts-unified-log.md#query-time-range) 옵션을 사용 하 여 지정 된 조건을 평가 하는 시간 범위를 선택 합니다.

1. 로그 경고는 다음과 같은 두 가지 유형의 [**측정값**](../platform/alerts-unified-log.md#measure)을 기반으로 할 수 있습니다.
    1. **결과 수** -쿼리에서 반환 된 레코드 수입니다.
    1. **메트릭 측정**  -  선택 된 식 및 [bin ()](/azure/kusto/query/binfunction) 선택 항목 별로 그룹화 된 요약을 사용 하 여 계산 된 *집계 값* 입니다. 다음은 그 예입니다. 
...
1. 메트릭 측정 경고 논리의 경우 **집계** 옵션을 사용 하 여 [경고를 차원과 분할](../platform/alerts-unified-log.md#split-by-alert-dimensions) 하는 방법을 선택적으로 지정할 수 있습니다. 행 그룹화 식은 고유 하 고 정렬 되어야 합니다.
...
1. 그런 다음 미리 보기 데이터를 기반으로 [ **연산자**, **임계값**](../platform/alerts-unified-log.md#threshold-and-operator)및 [**빈도**](../platform/alerts-unified-log.md#frequency)를 설정 합니다.

1. 또한 필요에 따라 **전체 또는 연속 위반** 을 사용 하 여 [경고를 트리거하기 위한 위반 수](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) 를 설정할 수 있습니다.
...
1. 경고가 발생 한 후에 규칙 작업을 표시 하지 않을 지 여부를 선택 하 고 [**경고 표시 안 함**](../platform/alerts-unified-log.md#state-and-resolving-alerts) 옵션을 사용 합니다. 규칙은 계속 실행 되 고 경고를 생성 하지만 노이즈를 방지 하기 위해 동작이 트리거되지 않습니다. 음소거 작업 값은 경고를 적용 하는 빈도 보다 커야 합니다.

    ![로그 경고에 대한 경고 표시 안 함](media/alerts-log/AlertsPreviewSuppress.png)

1. 경고 조건이 충족 될 때 경고 규칙에서 하나 이상의 [**작업 그룹**](../platform/action-groups.md#webhook) 을 트리거해야 하는지 여부를 지정 합니다.
...
1. **조건** 탭에서 시작 합니다.
    ...
    1. [**측정값**](../platform/alerts-unified-log.md#measure), [**집계 유형**](../platform/alerts-unified-log.md#aggregation-type)및 [**집계 세분성**](../platform/alerts-unified-log.md#aggregation-granularity) 이 올바른지 확인 합니다. 
        1. 기본적으로이 규칙은 지난 5 분 동안의 결과 수를 계산 합니다.
        1. 요약 된 쿼리 결과를 검색 하는 경우 규칙은 몇 초 내에 자동으로 업데이트 되어이를 캡처합니다.

    1. 필요한 경우 [차원으로 경고 분할](../platform/alerts-unified-log.md#split-by-alert-dimensions)을 선택 합니다. 
    ...
    1. 그런 다음 미리 보기 데이터를 기반으로 **경고 논리** 를 설정 합니다. [ **연산자**, **임계값**](../platform/alerts-unified-log.md#threshold-and-operator)및 [**Frequency**](../platform/alerts-unified-log.md#frequency)값이 있습니다.

        ![임계값 및 경고 논리를 사용 하 여 차트 미리 보기](media/alerts-log/chart-and-alert-logic.png)

    1. 필요에 따라 **고급 옵션** 섹션에서 [**경고를 트리거하기 위한 위반 수를**](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) 설정할 수 있습니다.
    
        ![고급 옵션](media/alerts-log/advanced-options.png)

1. **작업** 탭에서 필요한 [작업 그룹](../platform/action-groups.md)을 선택 하거나 만듭니다.

    ![작업 탭](media/alerts-log/actions-tab.png)

1. **세부 정보** 탭에서 **경고 규칙 세부 정보** 및 **프로젝트 세부 정보** 를 정의 합니다. 필요에 따라 작업 **실행을 시작** 하지 않을 지 여부를 설정 하거나 경고 규칙이 실행 된 후 기간에 대 한 [**작업을 음소거**](../platform/alerts-unified-log.md#state-and-resolving-alerts) 할 수 있습니다.
...
1. 경고 관리는 발생 한 모든 경고를 표시 합니다. [경고 관리에 대해 자세히 알아보세요](alerts-managing-alert-instances.md).
    ...
    > [!NOTE]
    > 로그 경고 규칙은 현재 [상태 비저장 이며 확인 되지](../platform/alerts-unified-log.md#state-and-resolving-alerts)않습니다.
...
> [!NOTE]
> ScheduledQueryRules PowerShell cmdlet은 현재 [예약 된 쿼리 규칙 API](/rest/api/monitor/scheduledqueryrules/)에서 만든 규칙만 관리할 수 있습니다. 레거시 [Log Analytics 경고 api](../platform/api-alerts.md) 를 사용 하 여 만든 로그 경고 규칙은 [예약 된 쿼리 규칙 api로 전환한 후에](../alerts/alerts-log-api-switch.md)만 PowerShell을 사용해 서만 관리할 수 있습니다.
...
> [!NOTE]
> Azure CLI 지원은 scheduledQueryRules API 버전 이상 에서만 사용할 수 있습니다 `2020-05-01-preview` . 이전 API 버전은 아래에 설명 된 템플릿과 함께 Azure Resource Manager CLI를 사용할 수 있습니다. 레거시 [Log Analytics 경고 API](../platform/api-alerts.md)를 사용 하는 경우 CLI를 사용 하도록 전환 해야 합니다. [전환에 대해 자세히 알아보세요](./alerts-log-api-switch.md).
...
* [로그 경고](../platform/alerts-unified-log.md)에 대해 알아봅니다.
