...
- 7 ~ 11 단계의 경우 다음 프로세스를 사용 합니다.
  ...
  1. **False** 조건에서 메트릭 경고가 논리 앱의 예상과 일치 하지 않는다는 것을 알리는 Microsoft 팀 작업을 정의 합니다. JSON 페이로드를 포함합니다. `json()` 식에서 `triggerBody` 동적 콘텐츠를 참조하는 방법을 확인합니다.
