...
1. 연결이 활성 상태로 유지 되도록 내부 프로시저를 설정 합니다. 새로 고침 토큰 수명이 정상적으로 만료 되기까지 몇 일이 지나면 다음 작업을 수행 합니다.
   ...
   1. 이전 새로 고침 토큰을 취소 합니다. 보안상의 이유로 이전 키를 유지 하지 않는 것이 좋습니다. 
      ...
      1. OAuth 이름 및 만료 날짜에 따라 목록에서 이전 토큰을 선택 합니다.
         ...
         ![OAuth에 대 한 토큰 목록을 보여 주는 스크린샷](media/itsmc-connections-servicenow/snow-system-oauth.png)
      1. **액세스**  >  **취소** 취소를 선택 합니다.
...
2. **작업 영역 데이터 원본** 에서 **itsm 연결** 을 선택 합니다.
   ...
   ![데이터 원본 선택 항목을 보여 주는 스크린샷](media/itsmc-overview/add-new-itsm-connection.png)
...
![ServiceNow 연결을 추가 하기 위한 상자 및 옵션의 스크린샷](media/itsmc-connections-servicenow/itsm-connection-servicenow-connection-latest.png)
