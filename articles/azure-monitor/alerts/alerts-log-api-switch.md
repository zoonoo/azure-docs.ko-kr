...
> [!NOTE]
> 사용자가 기본 설정을 현재 [SCHEDULEDQUERYRULES api](/rest/api/monitor/scheduledqueryrules) 로 전환 하기로 선택 하면 이전 [레거시 Log Analytics 경고 API](../platform/api-alerts.md)로 되돌릴 수 없습니다.

이전에는 사용자가 [레거시 Log Analytics 경고 API](../platform/api-alerts.md) 를 사용 하 여 로그 경고 규칙을 관리 했습니다. 현재 작업 영역에서는 [SCHEDULEDQUERYRULES API](/rest/api/monitor/scheduledqueryrules)를 사용 합니다. 이 문서에서는 레거시 API에서 현재 API로 전환 하는 과정 및 이점에 대해 설명 합니다.

## <a name="benefits"></a>이점

- 경고 규칙을 만들기 위한 단일 템플릿 (이전에는 세 개의 별도 템플릿이 필요 함).
- Log Analytics 작업 영역 또는 Application Insights 리소스 모두에 대 한 단일 API입니다.
- [PowerShell cmdlet 지원](../platform/alerts-log.md#managing-log-alerts-using-powershell).
- 다른 모든 경고 유형과 심각도 맞춤.
...
- 현재 API를 사용 하 여 모든 새 규칙을 만들고 편집 해야 합니다. [Azure 리소스 템플릿을 통한 샘플 사용](alerts-log-create-templates.md) 및 [PowerShell을 통한 샘플 사용](../platform/alerts-log.md#managing-log-alerts-using-powershell)을 참조 하세요.
...
- [Azure Monitor - 로그 경고](../platform/alerts-unified-log.md)에 대해 알아봅니다.
- [API를 사용 하 여 로그 경고를 관리](alerts-log-create-templates.md)하는 방법을 알아봅니다.
- [PowerShell을 사용 하 여 로그 경고를 관리](../platform/alerts-log.md#managing-log-alerts-using-powershell)하는 방법을 알아봅니다.
- [Azure Alerts 환경](../platform/alerts-overview.md)에 대해 자세히 알아봅니다.
