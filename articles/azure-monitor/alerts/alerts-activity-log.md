...
활동 로그에 대한 경고 규칙을 만들 수 있는 조건을 이해하는 간단한 방법은 [Azure Portal의 활동 로그](../essentials/activity-log.md#view-the-activity-log)를 통해 이벤트를 탐색하고 필터링하는 것입니다. **Azure Monitor - 활동 로그** 화면에서 필요한 이벤트를 필터링하거나 찾고 **활동 로그 경고 추가** 단추를 사용하여 경고를 만들 수 있습니다. 이후 앞에 있는 4~7단계를 수행합니다.
