...
* 경고를 사용 하지 않거나 사용 하도록 설정할 수 있습니다. 맨 위에 있는 단추를 참조 하세요.

*경고 추가 단추가 보이지 않습니다.*

* 조직 계정을 사용 중이신가요? 이 애플리케이션 리소스에 소유자 또는 참가자 액세스가 가능하면 경고를 설정할 수 있습니다. Access Control 탭을 살펴보세요. [액세스 제어에 대해 알아봅니다][roles].

> [!NOTE]
> 경고 블레이드에 [사전 진단](../../azure-monitor/app/proactive-failure-diagnostics.md) 경고 설정이 이미 표시됩니다. 자동 경고는 특정 메트릭, 요청 실패율을 모니터링합니다. 사전 경고를 사용하지 않으려는 경우가 아니면 요청 실패율에 대한 자체 경고를 설정할 필요가 없습니다.
> 
> 
...
* [브라우저 메트릭][client], 특히 브라우저 **페이지 로드 시간**은 웹 응용 프로그램에 적합 합니다. 페이지에 스크립트가 많은 경우 **브라우저 예외**를 확인해야 합니다. 이러한 메트릭 및 경고를 가져오려면 [웹 페이지 모니터링][client]을 설정 해야 합니다.
...
전자 메일 주소가 명시적으로 나열 되지 않은 경우 특정 역할의 모든 구성원에 게 자동으로 알리는 옵션을 사용 하지 않도록 설정 하는 것이 좋습니다. 대신 알림 전자 메일에서 해당 경고 규칙에 대 한 알림을 받아야 하는 모든 사용자 전자 메일을 나열 하는 것이 좋습니다. 받는 사람 필드.
