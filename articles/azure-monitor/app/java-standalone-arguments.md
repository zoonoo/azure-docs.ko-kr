...
이전 어딘가에 JVM 인수를 추가 합니다 `-javaagent:path/to/applicationinsights-agent-3.0.0.jar` `-jar` . 예를 들면 다음과 같습니다.
...
*Exec* 폼을 사용 하는 경우 매개 변수 앞의 매개 변수 목록에 매개 변수를 추가 합니다 `"-javaagent:path/to/applicationinsights-agent-3.0.0.jar"` `"-jar"` . 예를 들면 다음과 같습니다.

```
ENTRYPOINT ["java", "-javaagent:path/to/applicationinsights-agent-3.0.0.jar", "-jar", "<myapp.jar>"]
```

*Shell* 형식을 사용 하는 경우 앞에 JVM 인수를 추가 합니다 `-javaagent:path/to/applicationinsights-agent-3.0.0.jar` `-jar` . 예를 들면 다음과 같습니다.
...
파일이 이미 있으면 `<tomcat>/bin/setenv.sh` 해당 파일을 수정 하 고 `-javaagent:path/to/applicationinsights-agent-3.0.0.jar` 에 추가 `CATALINA_OPTS` 합니다.
...
파일이 이미 있는 경우 `<tomcat>/bin/setenv.bat` 해당 파일을 수정 하 고에 추가 `-javaagent:path/to/applicationinsights-agent-3.0.0.jar` 하면 `CATALINA_OPTS` 됩니다.

### <a name="running-tomcat-as-a-windows-service"></a>Windows 서비스로 Tomcat 실행

파일을 찾습니다 `<tomcat>/bin/tomcat8w.exe` .  실행 파일을 실행 하 고 `-javaagent:path/to/applicationinsights-agent-3.0.0.jar` `Java Options` 탭 아래의에를 추가 합니다 `Java` .
...
`-javaagent:path/to/applicationinsights-agent-3.0.0.jar` `JAVA_OPTS` 파일 `JBOSS_HOME/bin/standalone.conf` (Linux) 또는 (Windows)에서 기존 환경 변수에를 추가 합니다 `JBOSS_HOME/bin/standalone.conf.bat` .
...
`-javaagent:path/to/applicationinsights-agent-3.0.0.jar`에서 기존에 추가 `jvm-options` 합니다 `JBOSS_HOME/domain/configuration/host.xml` .
...
`-javaagent:path/to/applicationinsights-agent-3.0.0.jar`에서 기존에 추가 `jvm-options` 합니다 `glassfish/domains/domain1/config/domain.xml` .
