...
는 각 HTML 페이지의 헤더에 코드 조각을 수동으로 추가할 수 있지만, 코드 조각을 사이트의 모든 페이지에 삽입 하는 기본 페이지에 코드 조각을 추가 하는 것이 좋습니다. 이 문서에서 템플릿 기반 ASP.NET MVC 앱의 경우 편집 해야 하는 파일을 라고 `_Layout.cshtml` 하며 공유 된 **뷰** 아래에 있습니다  >  .
