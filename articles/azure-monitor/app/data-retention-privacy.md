...
> [!NOTE]
> 이제 2.15.0-beta3 릴리스 및 더 큰 로컬 스토리지가 Linux, Mac 및 Windows 용으로 자동 생성됩니다. Windows가 아닌 시스템의 경우 SDK는 다음 논리에 따라 로컬 스토리지 폴더를 자동으로 생성합니다.
> - `${TMPDIR}` - `${TMPDIR}` 환경 변수가 설정된 경우이 위치가 사용됩니다.
> - `/var/tmp` -이전 위치가 없는 경우 `/var/tmp`를 시도합니다.
> - `/tmp` - 둘 다 이전 위치가 없는 경우 `tmp`를 시도합니다. 
