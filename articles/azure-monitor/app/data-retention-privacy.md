...
---
# <a name="data-collection-retention-and-storage-in-application-insights"></a>애플리케이션 인사이트 내 데이터 수집, 보존 및 스토리지
...
* 추가 사용자 지정 원격 분석을 보내는 코드를 작성하여 사용 현황의 진단 및 모니터링을 도울 수 있습니다. 이 확장성은 응용 프로그램 인사이트(Application Insights)의 훌륭한 기능입니다. 개인 및 기타 중요한 데이터가 포함되도록 실수로 이 코드를 작성할 수 있습니다. 응용 프로그램이 이러한 데이터로 작동하는 경우 작성하는 모든 코드에 철저한 검토 프로세스를 적용해야 합니다.
* 앱을 개발하고 테스트하는 동안 SDK에서 보낼 항목을 검사하기 쉽습니다. IDE 및 브라우저의 디버깅 출력 창에 데이터가 나타납니다. 
* 데이터는 미국 또는 유럽의 [Microsoft Azure](https://azure.com) 서버에 저장됩니다. (하지만 앱은 어디서나 실행할 수 있습니다.) Azure는 강력한 보안 프로세스를 가지고 [있으며 광범위한 규정 준수 표준을 충족합니다.](https://azure.microsoft.com/support/trust-center/) 사용자와 지정된 팀만 데이터에 액세스할 수 있습니다. Microsoft 직원은 지식으로 제한된 특정 상황에서만 해당 데이터에 제한된 액세스 권한을 갖을 수 있습니다. 그것은 전송 및 나머지에서 암호화됩니다.
*   수집된 데이터를 검토하는 경우 일부 상황에서는 허용되지만 다른 경우에는 허용되지 않는 데이터가 포함될 수 있습니다.  이 것의 좋은 예는 장치 이름입니다. 서버의 장치 이름은 개인 정보 보호에 영향을 미치지 않으며 유용하지만 휴대폰이나 랩톱의 장치 이름은 개인 정보 보호에 영향을 미치고 덜 유용할 수 있습니다. SDK는 주로 대상 서버를 위해 개발되었으며 기본적으로 장치 이름을 수집하므로 일반 이벤트와 예외 모두에서 덮어써야 할 수 있습니다.
...
이 기능을 가져오기 위해 애플리케이션에서 해당 코드의 일부가 되는 Application Insights SDK를 설치합니다. 앱이 실행 중일 때 SDK는 작업을 모니터링하고 Application Insights 서비스에 원격 분석을 보냅니다. [Microsoft Azure](https://azure.com)에서 호스팅하는 클라우드 서비스입니다. 그러나 응용 프로그램 인사이트는 Azure에서 호스팅되는 응용 프로그램뿐만 아니라 모든 응용 프로그램에 대해 작동합니다.
...
* [개발 시](../../azure-monitor/app/asp-net.md) 또는 [런타임 시](../../azure-monitor/app/monitor-performance-live-website-now.md) 앱과 통합하는 SDK가 있습니다. 다른 애플리케이션 형식에 대한 여러 SDK가 있습니다. 페이지와 함께 최종 사용자의 브라우저에 로드되는 [웹 페이지에 대한 SDK도](../../azure-monitor/app/javascript.md)있습니다.
...
* [예외](../../azure-monitor/app/asp-net-exceptions.md) 및 충돌 - 스택 `build id` **덤프,** CPU 유형입니다. 
...
원시 데이터 요소(즉, 애널리틱스에서 쿼리하고 검색에서 검사할 수 있는 항목)는 최대 730일 동안 보관됩니다. 보존 [기간은](https://docs.microsoft.com/azure/azure-monitor/app/pricing#change-the-data-retention-period) 30일, 60일, 90일, 120일, 180일, 270일, 365일, 550일 또는 730일입니다. 데이터를 730일 이상 유지해야 하는 경우 [연속 내보내기를](../../azure-monitor/app/export-telemetry.md) 사용하여 데이터 수집 중에 저장소 계정에 데이터를 복사할 수 있습니다. 

90일 이상 보관된 데이터는 추가 요금이 부과됩니다. Azure 모니터 가격 책정 페이지에서 응용 프로그램 인사이트 가격 [책정에](https://azure.microsoft.com/pricing/details/monitor/)대해 자세히 알아봅니다.

집계 데이터(즉, 메트릭 탐색기에 표시되는 개수, 평균 및 기타 통계 데이터)는 90일 동안 1분 단위로 보존됩니다.

[디버그 스냅숏은](../../azure-monitor/app/snapshot-debugger.md) 15일 동안 저장됩니다. 이 보존 정책은 애플리케이션 단위로 설정됩니다. 이 값을 늘려야 하는 경우 Azure Portal에서 지원 사례를 열어 증가를 요청할 수 있습니다.
...
* 새 Application Insights 리소스를 만들 때 위치를 선택할 수 있습니다. 지역별 애플리케이션 인사이트 가용성에 대한 자세한 내용은 [여기를 참조하십시오.](https://azure.microsoft.com/global-infrastructure/services/?products=all)

#### <a name="does-that-mean-my-app-has-to-be-hosted-in-the-usa-europe-or-southeast-asia"></a>내 앱이 미국, 유럽 또는 동남아시아에서 호스팅되어야 한다는 의미인가요?
...
데이터는 Microsoft Azure 서버에 저장됩니다. Azure 포털의 계정의 경우 계정 제한은 [Azure 보안, 개인 정보 및 규정 준수 문서에](https://go.microsoft.com/fwlink/?linkid=392408)설명되어 있습니다.
...
모든 데이터는 미사용 데이터와 데이터 센터 간에 이동하면서 암호화됩니다.
...
예. https를 사용하여 웹 서버, 장치 및 HTTPS 웹 페이지를 포함하여 거의 모든 SDK에서 포털로 데이터를 전송합니다. 
...
로컬 저장소를 사용하는 원격 분석 채널은 APPDATA 디렉터리에서 임시 파일을 만들며, 이 파일은 응용 프로그램을 실행하는 특정 계정으로 제한됩니다. 이는 엔드포인트를 일시적으로 사용할 수 없거나 조정 제한에 도달했을 때 발생할 수 있습니다. 이 문제가 해결되면 원격 분석 채널이 모든 새 지속 데이터 및 지속된 데이터 전송을 재개합니다.

이 지속된 데이터는 로컬로 암호화되지 않습니다. 문제가 있는 경우 데이터를 검토하고 개인 데이터 수집을 제한합니다. (자세한 내용은 [개인 데이터를 내보내고 삭제하는 방법을](https://docs.microsoft.com/azure/application-insights/app-insights-customer-data#how-to-export-and-delete-private-data)참조하십시오.)

고객이 특정 보안 요구 사항으로 이 디렉터리를 구성해야 하는 경우 프레임워크에 따라 이 디렉터리를 구성할 수 있습니다. 애플리케이션을 실행하는 프로세스에 이 디렉터리에 대한 쓰기 액세스 권한이 있는지 확인하세요. 그러나 의도하지 않은 사용자가 원격 분석을 읽을 수 없도록 보호되었는지도 확인하세요.

### <a name="java"></a>Java

`C:\Users\username\AppData\Local\Temp`는 데이터를 지속하는 데 사용됩니다. 이 위치는 구성 디렉터리에서 구성할 수 없으며 이 폴더에 대한 액세스 권한은 필수 자격 증명이 있는 특정 사용자로 제한됩니다. (자세한 내용은 [구현을](https://github.com/Microsoft/ApplicationInsights-Java/blob/40809cb6857231e572309a5901e1227305c27c1a/core/src/main/java/com/microsoft/applicationinsights/internal/util/LocalFileSystemUtils.java#L48-L72)참조하십시오.)
...
기본적으로 `ServerTelemetryChannel`은 현재 사용자의 로컬 앱 데이터 폴더 `%localAppData%\Microsoft\ApplicationInsights` 또는 임시 폴더`%TMP%`를 사용합니다. [(구현은](https://github.com/Microsoft/ApplicationInsights-dotnet/blob/91e9c91fcea979b1eec4e31ba8e0fc683bf86802/src/ServerTelemetryChannel/Implementation/ApplicationFolderProvider.cs#L54-L84) 여기를 참조하십시오.) Linux 환경에서는 저장소 폴더를 지정하지 않으면 로컬 저장소가 비활성화됩니다.
...
(자세한 내용은 [AspNetCore 사용자 지정 구성을](https://github.com/Microsoft/ApplicationInsights-aspnetcore/wiki/Custom-Configuration)참조하십시오.)
...
### <a name="javascript-browser"></a>자바 스크립트 (브라우저)

[HTML5 세션 저장소는](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) 데이터를 유지하는 데 사용됩니다. 두 개의 별도 버퍼가 `AI_sent_buffer`사용됩니다. `AI_buffer` 일괄 처리되고 전송대기중의 원격 분석이 `AI_buffer`에 저장됩니다. 방금 전송된 원격 분석은 `AI_sent_buffer` 수신 서버가 성공적으로 수신되었다고 응답할 때까지 배치됩니다. 원격 분석을 성공적으로 받으면 모든 버퍼에서 제거됩니다. 일시적인 오류(예: 사용자가 네트워크 연결을 끊음)의 경우 원격 `AI_buffer` 분석이 성공적으로 수신될 때까지 유지하거나 수신 서버가 원격 분석이 유효하지 않은 것으로 응답합니다(예: 잘못된 스키마 또는 너무 오래됨).

원격 분석 버퍼를 로 [`enableSessionStorageBuffer`](https://github.com/microsoft/ApplicationInsights-JS/blob/17ef50442f73fd02a758fbd74134933d92607ecf/legacy/JavaScript/JavaScriptSDK.Interfaces/IConfig.ts#L31) 설정하여 `false`비활성화할 수 있습니다. 세션 저장소가 꺼져 있으면 로컬 배열이 영구 저장소로 대신 사용됩니다. JavaScript SDK는 클라이언트 장치에서 실행되므로 사용자는 브라우저의 개발자 도구를 통해 이 저장소 위치에 액세스할 수 있습니다.

### <a name="opencensus-python"></a>오픈인구파이썬

기본적으로 OpenCensus 파이썬 SDK는 `%username%/.opencensus/.azure/`현재 사용자 폴더를 사용합니다. 이 폴더에 대한 액세스 권한은 현재 사용자 및 관리자로 제한됩니다. [(구현은](https://github.com/census-instrumentation/opencensus-python/blob/master/contrib/opencensus-ext-azure/opencensus/ext/azure/common/storage.py) 여기를 참조하십시오.) 지속된 데이터가 있는 폴더의 이름은 원격 분석을 생성한 Python 파일의 이름을 따서 명명됩니다.

사용 중인 내보내기 생성자의 `storage_path` 매개 변수를 전달하여 저장소 파일의 위치를 변경할 수 있습니다.
...
[PCI 보안 표준 위원회는](https://www.pcisecuritystandards.org/) 이전 버전의 TLS/SSL을 비활성화하고 보다 안전한 프로토콜로 업그레이드하기 위해 [2018년 6월 30일의 기한을](https://www.pcisecuritystandards.org/pdfs/PCI_SSC_Migrating_from_SSL_and_Early_TLS_Resource_Guide.pdf) 설정했습니다. Azure가 레거시 지원을 삭제하면 애플리케이션/클라이언트가 TLS 1.2를 통해 통신할 수 없는 경우 Application Insights에 데이터를 보낼 수 없게 됩니다. 애플리케이션의 TLS 지원을 테스트하고 유효성을 검사하는 애플리케이션에서 사용하는 방법은 언어/프레임워크뿐만 아니라 운영 체제/플랫폼에 따라 달라집니다.

필요한 경우 처럼 최신 프로토콜을 자동으로 검색하고 활용할 수 있는 플랫폼 수준 보안 기능을 중단할 수 있는 경우가 아니면 응용 프로그램을 TLS 1.2만 사용하도록 명시적으로 설정하지 않는 것이 좋습니다. TLS 1.3. 특정 TLS/SSL 버전의 하드 코딩을 확인하려면 애플리케이션 코드에 대해 철저한 감사를 수행하는 것이 좋습니다.
...
|.NET | 지원됨, 구성이 버전에 따라 다릅니다. | .NET 4.7 및 이전 버전에 대한 자세한 구성 정보는 [다음 지침을](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12)참조하십시오.  |
|상태 모니터 | 지원됨, 구성이 필요합니다. | 상태 모니터는 TLS 1.2를 지원하기 위해 [OS 구성](https://docs.microsoft.com/windows-server/security/tls/tls-registry-settings) + [.NET 구성을](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12) 기반으로 합니다.
|Node.js |  지원됨, v10.5.0에서 구성이 필요할 수 있습니다. | 모든 응용 프로그램 별 구성에 대한 [공식 Node.js TLS/SSL 설명서를](https://nodejs.org/api/tls.html) 사용하십시오. |
...
예비 테스트를 실행하여 Linux 시스템이 TLS 1.2.를 통해 통신할 수 있는지 확인하려면 터미널을 열고 실행하십시오.
...
SDK는 플랫폼마다 다르며, 설치할 수 있는 여러 구성 요소가 있습니다. (응용 [프로그램 인사이트][start]참조 - 개요 .) 각 구성 요소는 서로 다른 데이터를 보냅니다.
...
| DeviceContext |`Id`, IP, 로캘, 장치 모델, 네트워크, 네트워크 유형, OEM 이름, 화면 확인, 역할 인스턴스, 역할 이름, 장치 유형 |
...
| 요청 |URL, 기간, 응답 코드 |
| 종속성 |유형(SQL, HTTP, ...), 연결 문자열 또는 URI, 동기화/비동기, 기간, 성공, SQL 문(상태 모니터 사용) |
| **예외** |유형, **메시지,** 통화 스택, 소스 파일, 줄 번호,`thread id` |
| 충돌 |`Process id`, `parent process id`, `crash thread id`; 응용 프로그램 `id`패치, , 빌드;  예외 유형, 주소, 이유; 난독 처리 된 기호 및 레지스터, 이진 시작 및 끝 주소, 이진 이름 및 경로, CPU 유형 |
| 추적 |**메시지** 및 심각도 수준 |
...
> [!NOTE]
> 클라이언트 IP는 지리적 위치를 유추하는 데 사용되지만 기본적으로 IP 데이터는 더 이상 저장되지 않으며 모든 0은 연결된 필드에 기록됩니다. 개인 데이터 처리에 대한 자세한 내용은 이 [문서](../../azure-monitor/platform/personal-data-mgmt.md#application-data)를 권장합니다. IP 주소 데이터를 저장해야 하는 경우 [IP 주소 수집 문서에서](https://docs.microsoft.com/azure/azure-monitor/app/ip-collection) 옵션을 안내합니다.

## <a name="credits"></a>크레딧
이 제품에는 MaxMind에서 만든 GeoLite2 [https://www.maxmind.com](https://www.maxmind.com)데이터가 포함되어 있습니다.
