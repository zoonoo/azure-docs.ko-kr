...
* **런타임** - App Service가 이미 라이브 상태인 경우 성능 모니터링 확장을 선택할 수 있습니다. 앱을 다시 빌드하거나 설치할 필요가 없습니다. 응답 시간, 성공률, 예외, 종속성 등을 모니터링하는 패키지의 표준 집합을 얻게 됩니다. 
...
Azure에서 App Service를 이미 실행 중인 경우 이미 일부 요청 및 오류 비율을 모니터링하고 있습니다. Application Insights를 추가하면 응답 시간, 종속성에 대한 모니터링 호출, 스마트 검색, 강력한 Kusto 쿼리 언어 등의 추가 기능을 얻을 수 있습니다. 
...
2. 사용할 리소스가 지정되면 Application Insights에서 애플리케이션에 대한 플랫폼별 데이터를 수집하는 방법을 선택할 수 있습니다(ASP.NET 앱 모니터링은 기본적으로 서로 다른 두 가지 수집 수준에서 수행됨).
    ...
    * .NET **추천 수집** 수준은 다음과 같습니다.
    ...
    .NET Core는 .NET Core 2.0 및 2.1에 대해 **추천 수집** 또는 '사용 안 함'을 제공합니다.
