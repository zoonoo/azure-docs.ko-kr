...
3. 이전에 applicationinsights .config 파일을 통해 제어할 수 있는 샘플과 같은 설정을 구성 하려면 이제 해당 접두사를 사용 하 여 응용 프로그램 설정을 통해 동일한 설정과 상호 작용할 수 있습니다. 
    ...
    * 예를 들어 초기 샘플링 비율을 변경 하려면 다음 `MicrosoftAppInsights_AdaptiveSamplingTelemetryProcessor_InitialSamplingPercentage` 의 응용 프로그램 설정 및 `100`값을 만들 수 있습니다.
...
다음은의 `AppMonitoredSite` 모든 인스턴스를 사이트 이름으로 대체 하는 샘플입니다.
...
2. 응용 프로그램이 모니터링 해야 하는 요구 사항을 충족 하는지 확인 합니다.
    * 찾아보기`https://yoursitename.scm.azurewebsites.net/ApplicationInsights`

    ![결과 페이지 https://yoursitename.scm.azurewebsites/applicationinsights 의 스크린샷](./media/azure-web-apps/app-insights-sdk-status.png)

    * 가 인지 확인 `Application Insights Extension Status` 합니다.`Pre-Installed Site Extension, version 2.8.12.1527, is running.`
    ...
    * , `AppAlreadyInstrumented` `AppContainsDiagnosticSourceAssembly`및 에대한항목이없는지확인합니다.`AppContainsAspNetTelemetryCorrelationAssembly`
        * 이러한 항목이 있는 경우 응용 프로그램 `Microsoft.ApplicationInsights`에서, `System.Diagnostics.DiagnosticSource`및 `Microsoft.AspNet.TelemetryCorrelation`패키지를 제거 합니다.
...
|`AppAlreadyInstrumented:true` | 이 값은 이전 배포에서 앱 폴더에 위의 dll이 있는 경우에도 발생할 수 있습니다. | 이러한 dll이 제거 되도록 앱 폴더를 정리 합니다.|
