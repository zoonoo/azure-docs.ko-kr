...
description: 응용 프로그램 인사이트 데이터를 데이터 보강 또는 조회 테이블, 비애플리케이션 인사이트 데이터 원본 및 사용자 지정 데이터와 같은 다른 데이터 집합과 상호 연관시다.
ms.topic: conceptual
author: eternovsky
ms.author: evternov
ms.date: 08/08/2018
...
Application Insights는 여러 다른 데이터 형식을 수집합니다(예외, 추적, 페이지 보기 및 기타). 응용 프로그램의 성능, 안정성 및 사용량을 조사하기에 충분한 경우가 많지만 응용 프로그램 인사이트(Application Insights)에 저장된 데이터를 완전히 사용자 지정 데이터 집합과 상호 연관시키는 것이 유용한 경우가 있습니다.
...
Application Insights가 강력한 Azure Monitor 로그 플랫폼에서 지원되므로 Azure Monitor의 모든 기능을 사용하여 데이터를 수집할 수 있습니다. 그런 다음 이 사용자 지정 데이터와 Azure Monitor 로그에서 사용할 수 있는 데이터와 상관 관계를 지정하는 "조인" 연산자를 사용하여 쿼리를 작성합니다. 

## <a name="ingesting-data"></a>데이터 수집

이 섹션에서는 Azure Monitor 로그로 데이터를 가져오는 방법을 검토합니다.

아직 없는 경우 "작업 영역 만들기" 단계를 통해 [다음 지침을](../learn/quick-collect-azurevm.md) 따르고 포함하여 새 Log Analytics 작업 영역을 프로비전합니다.

Azure Monitor로 로그 데이터 보내기를 시작하려면 다음과 같은 여러 옵션이 있습니다.

- 동기 메커니즘의 경우 [데이터 수집기 API를](https://docs.microsoft.com/azure/log-analytics/log-analytics-data-collector-api) 직접 호출하거나 로직 앱 커넥터를 사용하여 "Azure Log Analytics"를 찾고 "데이터 보내기" 옵션을 선택할 수 있습니다.

  ![스크린샷 선택 및 작업](./media/custom-data-correlation/01-logic-app-connector.png)  

- 비동기 옵션의 경우 데이터 수집기 API를 사용하여 처리 파이프라인을 빌드합니다. 자세한 내용은 [이 문서를](https://docs.microsoft.com/azure/log-analytics/log-analytics-create-pipeline-datacollector-api) 참조하십시오.
...
예를 들어 랩 인벤토리와 위치를 "myLA"라는 로그 분석 작업 영역에서 "LabLocations_CL"라는 테이블로 수집할 수 있습니다. 그런 다음 "myAI"라는 Application Insights 앱에서 추적된 요청을 검토하고 이전에 언급한 사용자 지정 테이블에 저장된 이러한 컴퓨터의 위치에 요청을 제공하는 컴퓨터 이름을 상호 연관시려면 다음 쿼리를 실행할 수 있습니다. 응용 프로그램 인사이트 또는 Azure 모니터 컨텍스트:
