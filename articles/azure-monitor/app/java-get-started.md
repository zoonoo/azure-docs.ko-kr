---
title: '빠른 시작: Azure 응용 프로그램 인사이트를 갖춘 Java 웹 앱 분석'
...
이 빠른 시작에서는 Application Insights를 사용하여 요청을 자동으로 계측하고, 종속성을 추적하고, 성능 카운터를 수집하고, 성능 문제 및 예외를 진단하고, 코드를 작성하여 사용자가 앱에서 수행하는 작업을 추적합니다.
...
프로젝트가 빌드에 Maven을 사용하도록 이미 설정되어 있는 경우 다음 코드를 *pom.xml* 파일에 병합합니다.
...
프로젝트가 빌드에 Gradle을 사용하도록 이미 설정되어 있는 경우 다음 코드를 *build.gradle* 파일에 병합합니다.
...
* *`-web-auto`에서 `-web` 구성 요소와 `-core` 구성 요소 간의 관계는 무엇입니까?*
  * `applicationinsights-web-auto`런타임에 응용 프로그램 인사이트 서발 필터를 자동으로 등록하여 HTTP 서브렛 요청 수와 응답 시간을 추적하는 메트릭을 제공합니다.
  * `applicationinsights-web`또한 HTTP 서버렛 요청 수와 응답 시간을 추적하는 메트릭을 제공하지만 응용 프로그램에서 응용 프로그램 인사이트 서발 필터를 수동으로 등록해야 합니다.
  * `applicationinsights-core`예를 들어 응용 프로그램이 서벌렛 기반이 아닌 경우 베어 API만 제공합니다.
  
* *SDK를 최신 버전으로 업데이트하려면 어떻게 해야 하나요?*
  * Gradle 또는 Maven을 사용하는 경우...
    * 빌드 파일을 업데이트하여 최신 버전을 지정합니다.
  * 종속성을 수동으로 관리하는 경우...
...
## <a name="add-an-applicationinsightsxml-file"></a>*애플리케이션인사이트.xml* 파일 추가
프로젝트의 리소스 폴더에 *ApplicationInsights.xml을* 추가하거나 프로젝트의 배포 클래스 경로에 추가되었는지 확인합니다. 다음 XML을 복사합니다.

계측 키를 Azure 포털에서 가져온 키로 바꿉습니다.
...
선택적으로 구성 파일은 응용 프로그램에 액세스할 수 있는 모든 위치에 있을 수 있습니다.  시스템 속성은 `-Dapplicationinsights.configurationDirectory` *ApplicationInsights.xml을*포함하는 디렉터리를 지정합니다. 예를 들어 `E:\myconfigs\appinsights\ApplicationInsights.xml`에 있는 구성 파일은 `-Dapplicationinsights.configurationDirectory="E:\myconfigs\appinsights"` 속성을 사용하여 구성됩니다.
...
* 이벤트 상관 관계는 HTTP 요청 구성 요소에 추가됩니다. 서버에서 받은 각 요청에 식별자를 할당합니다. 그런 다음 이 식별자를 모든 원격 분석 항목에 속성 'Operation.Id'으로 속성으로 추가합니다. [진단 검색][diagnostic]에서 필터를 설정하여 각 요청과 연결된 원격 분석의 상관 관계를 지정할 수 있습니다.
...
[Java 에이전트를 설치하여](java-agent.md) 나가는 HTTP 호출, JDBC 쿼리, 응용 프로그램 로깅 및 더 나은 작업 이름을 캡처합니다.
...
[측정항목에 대해 자세히 알아보세요.][metrics]

차트를 클릭하면 더 자세한 집계된 메트릭을 볼 수 있습니다.

![차트가 있는 응용 프로그램 인사이트 실패 창](./media/java-get-started/006-barcharts.png)
...
![특정 샘플 뷰로 드릴링](./media/java-get-started/007-instance.png)
...
## <a name="azure-app-service-config-spring-boot"></a>Azure 앱 서비스 구성(스프링 부팅)

Windows에서 실행되는 Spring Boot 앱은 Azure 앱 서비스에서 실행하려면 추가 구성이 필요합니다. **web.config를** 수정하고 다음 구성을 추가합니다.
...
다른 예외에 대한 데이터를 수집하려면 [코드에서 trackException()에 대한 호출을 삽입할 수 있습니다.][apiexceptions]
...
**조사**, **메트릭을**열어 다양한 성능 카운터를 확인합니다.

![프로세스 전용 바이트가 선택된 메트릭 창의 스크린샷](./media/java-get-started/011-perf-counters.png)

### <a name="customize-performance-counter-collection"></a>성능 카운터 수집 사용자 지정
표준 성능 카운터 집합의 컬렉션을 사용하지 않으려면 *ApplicationInsights.xml* 파일의 루트 노드 아래에 다음 코드를 추가합니다.
...
[가용성 웹 테스트를 설정하는 방법에 대해 자세히 알아보세요.][availability]
