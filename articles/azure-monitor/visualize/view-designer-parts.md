...
> [!NOTE]
> 보기의 파트는 Log Analytics 작업 영역의 [로그 쿼리](../log-query/log-query-overview.md) 를 기반으로 합니다. 현재는 Application Insights에서 데이터를 검색 하는 [리소스 간 쿼리](../log-query/cross-workspace-query.md) 를 지원 하지 않습니다.
...
클릭 탐색은 보기에서 머리글 또는 목록 항목을 클릭할 때 수행될 작업을 정의합니다.  [Log Analytics](../log-query/log-query-overview.md) 에서 쿼리를 열거나 다른 뷰를 시작 합니다.
...
| Color |임계값을 나타내는 색입니다. |

## <a name="next-steps"></a>다음 단계
* 시각화 부분에서 쿼리를 지 원하는 [로그 쿼리에](../log-query/log-query-overview.md) 대해 알아봅니다.
