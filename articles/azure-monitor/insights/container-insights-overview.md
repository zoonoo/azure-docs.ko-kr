...
ms.locfileid: "73477057"
---
# <a name="azure-monitor-for-containers-overview"></a>컨테이너용 Azure Monitor 개요

컨테이너에 대 한 Azure Monitor는 AKS (Azure Kubernetes Service)에서 호스트 되는 관리 되는 Kubernetes 클러스터 또는 호스트 된 자체 관리 되는 Kubernetes 클러스터 Azure Container Instances에 배포 된 컨테이너 워크 로드의 성능을 모니터링 하기 위해 설계 된 기능입니다. Azure Stack 합니다. 컨테이너 모니터링은 특히 여러 애플리케이션을 사용하여 대규모의 프로덕션 클러스터를 실행하는 경우에 중요합니다.
...
컨테이너에 대 한 Azure Monitor는 다양 한 Azure Monitor 기능을 사용 하 여 포괄적인 모니터링 환경을 제공 하므로 Kubernetes 클러스터 및 컨테이너 워크 로드의 성능 및 상태를 이해할 수 있습니다. 컨테이너에 대 한 Azure Monitor를 사용 하 여 다음을 수행할 수 있습니다.
...
* 노드 또는 컨테이너의 CPU 및 메모리 사용률이 임계값을 초과 하거나 인프라, 노드 또는 작업 상태 롤업의 클러스터에서 상태 변경이 발생 하는 경우 사전에 알리도록 경고를 구성 하거나 기록 합니다.
...
- 온-프레미스에 배포 된 Azure Stack 또는 Kubernetes에서 AKS 엔진 모니터링을 시작 하려면 [컨테이너에 대해 Azure Monitor를 사용 하 여 하이브리드 Kubernetes 클러스터 구성](container-insights-hybrid-setup.md)을 검토 하세요.  
