---
title: Azure 모니터의 Azure 네트워킹 분석 솔루션 | 마이크로 소프트 문서
description: Azure 모니터의 Azure 네트워킹 분석 솔루션을 사용하여 Azure 네트워크 보안 그룹 로그 및 Azure 응용 프로그램 게이트웨이 로그를 검토할 수 있습니다.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
ms.locfileid: "79275569"
---
# <a name="azure-networking-monitoring-solutions-in-azure-monitor"></a>Azure 모니터의 Azure 네트워킹 모니터링 솔루션
...
솔루션을 사용하려면:
1. Azure 모니터에 관리 솔루션을 추가하고
2. 진단을 사용하여 진단을 Azure 모니터의 로그 분석 작업 영역으로 안내합니다. Azure Blob Storage에 로그를 쓸 필요는 없습니다.

Application Gateway 및 네트워킹 보안 그룹 중 하나 또는 둘 다에 대해 진단 및 해당 솔루션을 사용할 수 있습니다.

특정 리소스 유형에 대한 진단 리소스 로깅을 활성화하지 않고 솔루션을 설치하면 해당 리소스의 대시보드 블레이드가 비어 있고 오류 메시지가 표시됩니다.

> [!NOTE]
> 2017년 1월에 응용 프로그램 게이트웨이 및 네트워크 보안 그룹에서 로그 분석 작업 영역으로 로그를 보내는 지원되는 방법이 변경되었습니다. **Azure Networking Analytics(사용되지 않음)** 솔루션을 표시하는 경우 수행해야 할 단계는 [이전 Networking Analytics 솔루션에서 마이그레이션](#migrating-from-the-old-networking-analytics-solution)을 참조하세요.
>
>
...
## <a name="azure-application-gateway-analytics-solution-in-azure-monitor"></a>Azure 모니터의 Azure 응용 프로그램 게이트웨이 분석 솔루션
...
1. [Azure 마켓플레이스에서](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.AzureAppGatewayAnalyticsOMS?tab=Overview) 또는 [솔루션 갤러리에서 Azure Monitor 솔루션 추가에](../../azure-monitor/insights/solutions.md)설명된 프로세스를 사용하여 Azure 응용 프로그램 게이트웨이 분석 솔루션을 활성화합니다.
...
1. Azure 포털에서 모니터링할 응용 프로그램 게이트웨이 리소스로 이동합니다.
...
## <a name="azure-network-security-group-analytics-solution-in-azure-monitor"></a>Azure 모니터의 Azure 네트워크 보안 그룹 분석 솔루션
...
> - Microsoft는 진단 설정을 사용하여 NSG 리소스 로그를 작업 영역으로 계속 전송할 수 있도록 지원합니다.
...
1. [Azure 마켓플레이스에서](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.AzureNSGAnalyticsOMS?tab=Overview) 또는 [솔루션 갤러리에서 Azure Monitor 솔루션 추가에](../../azure-monitor/insights/solutions.md)설명된 프로세스를 사용하여 Azure 네트워크 보안 그룹 분석 솔루션을 활성화합니다.
...
다음 PowerShell 스크립트는 네트워크 보안 그룹에 대한 리소스 로깅을 사용하도록 설정하는 방법의 예를 제공합니다.
```powershell
$workspaceId = "/subscriptions/d2e37fee-1234-40b2-5678-0b2199de3b50/resourcegroups/oi-default-east-us/providers/microsoft.operationalinsights/workspaces/rollingbaskets"
...
2017년 1월에 Azure 응용 프로그램 게이트웨이 및 Azure 네트워크 보안 그룹에서 로그 분석 작업 영역으로 로그를 보내는 지원되는 방법이 변경되었습니다. 이러한 변경은 다음과 같은 이점을 제공합니다.
+ 로그는 저장소 계정을 사용할 필요 없이 Azure Monitor에 직접 기록됩니다.
+ Azure Monitor에서 로그를 사용할 수 있는 로그가 생성되는 시점부터 대기 시간이 줄어듭니다.
+ 구성 단계 감소
+ 모든 유형의 Azure 진단을 위한 공통 형식

업데이트된 솔루션을 사용하려면 다음을 수행합니다.

1. [Azure 응용 프로그램 게이트웨이에서 Azure 모니터로 직접 보낼 진단 구성](#enable-azure-application-gateway-diagnostics-in-the-portal)
2. [Azure 네트워크 보안 그룹에서 Azure 모니터로 직접 보낼 진단 구성](#enable-azure-network-security-group-diagnostics-in-the-portal)
2. [솔루션 갤러리에서 Azure Monitor 솔루션 추가에](solutions.md) 설명된 프로세스를 사용하여 *Azure 응용 프로그램 게이트웨이 분석* 및 Azure 네트워크 보안 그룹 *분석* 솔루션 사용
3. 새 데이터 형식을 사용하도록 저장된 쿼리, 대시보드 또는 경고를 업데이트합니다.
   + 형식은 AzureDiagnostics로 변경합니다. ResourceType을 사용하여 Azure 네트워킹 로그로 필터링할 수 있습니다.

     | 다음 식을 사용하는 대신 | 사용: |
     | --- | --- |
     | NetworkApplicationgateways &#124; where OperationName=="ApplicationGatewayAccess" | AzureDiagnostics는 리소스 유형=="응용 프로그램 게이트웨이" 및 작업 이름=="응용 프로그램 게이트웨이 액세스" &#124; |
     | NetworkApplicationgateways &#124; where OperationName=="ApplicationGatewayPerformance" | AzureDiagnostics는 리소스 유형=="응용 프로그램 게이트웨이" 및 작업 이름="응용 프로그램 게이트웨이 성능"에 &#124;. |
...
* [Azure 모니터에서 로그 쿼리를](../log-query/log-query-overview.md) 사용하여 자세한 Azure 진단 데이터를 봅니다.
