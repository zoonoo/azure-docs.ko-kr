...
* 바이트 수신 속도 - 수신 바이트의 가장 높은 평균 상위 5 개 컴퓨터를 보여줍니다 

다섯 차트 중 하나의 오른쪽 상단 모서리에 있는 핀 아이콘을 클릭하면 선택한 차트가 마지막으로 본 Azure 대시보드에 고정됩니다.  대시보드에서 차트의 크기를 조정하고 위치를 조정할 수 있습니다. 대시보드에서 차트를 선택하면 VM용 Azure 모니터로 리디렉션되고 올바른 범위와 보기를 로드합니다.  

다섯 차트 중 하나에 있는 핀 아이콘의 왼쪽에 있는 아이콘을 클릭하면 **상위 N 목록** 보기가 열립니다.  여기서 해당 성능 메트릭의 리소스 사용률을 목록 보기의 개별 VM별로 확인하고 어떤 머신이 가장 높은지 볼 수 있습니다.  

![선택한 성능 메트릭에 대한 상위 N개 목록 보기](media/vminsights-performance/vminsights-performance-topnlist-01.png)

가상 컴퓨터를 클릭하면 **속성** 창이 오른쪽에서 확장되어 운영 체제에서 보고한 시스템 정보, Azure VM의 속성 등 선택한 항목의 속성을 표시합니다. **빠른 링크** 섹션 의 옵션 중 하나를 클릭하면 선택한 VM에서 직접 해당 기능으로 리디렉션됩니다.  
...
* 사용 가능한 메모리 - 평균, 상위 5위 및 10번째 백분위수표시 기본값 
...
목록 보기에서 개별 VM별 리소스 사용률을 확인하고 사용률이 가장 높은 추세인 컴퓨터를 보려면 **상위 N 목록** 탭을 선택합니다.  **상위 N 목록** 페이지에는 메트릭 CPU *사용률 %에*대해 95번째 백분위수로 가장 많이 사용되는 상위 20개 컴퓨터가 표시됩니다.  **추가 로드**를 선택하면 상위 500개 머신을 표시하도록 결과가 확장되므로 더 많은 머신을 볼 수 있습니다. 
...
가상 컴퓨터에서 직접 액세스하려면 다음 단계를 수행합니다.

1. Azure Portal에서 **Virtual Machines**를 선택합니다. 
2. 목록에서 VM을 선택하고 **모니터링** 섹션에서 **인사이트를**선택합니다.  
3. **성능** 탭을 선택합니다. 
...
차트 중 하나의 오른쪽 상단 모서리에 있는 핀 아이콘을 클릭하면 선택한 차트를 마지막으로 본 Azure 대시보드에 고정됩니다. 대시보드에서 차트의 크기를 조정하고 위치를 조정할 수 있습니다. 대시보드에서 차트를 선택하면 VM용 Azure 모니터로 리디렉션되고 VM에 대한 성능 세부 정보 보기가 로드됩니다.  

![VM 보기에서 직접 VM 인사이트 성능 보기](./media/vminsights-performance/vminsights-performance-directvm-01.png)

## <a name="view-performance-directly-from-an-azure-virtual-machine-scale-set"></a>Azure 가상 시스템 규모 집합에서 직접 성능 보기

Azure 가상 시스템 규모 집합에서 직접 액세스하려면 다음 단계를 수행합니다.

1. Azure 포털에서 **가상 시스템 크기 집합을**선택합니다.
2. 목록에서 VM을 선택하고 **모니터링** 섹션에서 **인사이트를** 선택하여 **성능** 탭을 봅니다.

이 페이지는 선택한 축척 집합으로 범위가 설정된 Azure Monitor 성능 보기를 로드합니다. 이를 통해 모니터링되는 메트릭 집합에서 척도 집합의 상위 N 인스턴스를 보고, 축척 집합에서 집계 성능을 보고, 스케일 집합의 개별 인스턴스에서 선택한 메트릭의 추세를 확인할 수 있습니다. 목록 보기에서 인스턴스를 선택하면 맵을 로드하거나 해당 인스턴스에 대한 자세한 성능 보기로 이동할 수 있습니다.

차트 중 하나의 오른쪽 상단 모서리에 있는 핀 아이콘을 클릭하면 선택한 차트를 마지막으로 본 Azure 대시보드에 고정됩니다. 대시보드에서 차트의 크기를 조정하고 위치를 조정할 수 있습니다. 대시보드에서 차트를 선택하면 VM용 Azure 모니터로 리디렉션되고 VM에 대한 성능 세부 정보 보기가 로드됩니다.  

![VM 인사이트 가상 머신 스케일 세트 뷰에서 직접 성능 향상](./media/vminsights-performance/vminsights-performance-directvmss-01.png)

>[!NOTE]
>스케일 집합에 대한 인스턴스 보기에서 특정 인스턴스에 대한 자세한 성능 보기에 액세스할 수도 있습니다. **설정** 섹션에서 **인스턴스로** 이동한 다음 **Insights**를 선택합니다.



## <a name="next-steps"></a>다음 단계

- VM용 Azure 모니터에 포함된 [통합 문서를](vminsights-workbooks.md) 사용하여 성능 및 네트워크 메트릭을 추가로 분석하는 방법을 알아봅니다.  
