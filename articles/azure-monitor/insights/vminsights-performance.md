...
2. 목록에서 VM을 선택 하 고 **모니터링** 섹션에서 **Insights (미리 보기)** 를 선택 하 여 **성능** 탭을 표시 합니다.
...
>[!NOTE]
>확장 집합에 대 한 인스턴스 보기에서 특정 인스턴스에 대 한 자세한 성능 보기에 액세스할 수도 있습니다. **설정** 섹션에서 **인스턴스로** 이동한 다음 **Insights (미리 보기)** 를 선택 합니다.

## <a name="alerts"></a>경고  

VM에 대한 Azure Monitor의 일부로 사용하도록 설정된 성능 메트릭에는 미리 구성된 경고 규칙이 포함되어 있지 않습니다. 높은 CPU 사용률, 사용 가능한 메모리 부족, 디스크 공간 부족 등 Azure VM에서 검색 된 성능 문제에 해당 하는 [상태 경고가](vminsights-health.md#alerts) 있습니다.  그러나 이러한 상태 경고는 VM용 Azure Monitor에 대해 사용 하도록 설정 된 모든 Vm에만 적용 됩니다. 
