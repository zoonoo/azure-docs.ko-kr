---
title: Azure 모니터의 에이전트 상태 솔루션 | 마이크로 소프트 문서
...
---
#  <a name="agent-health-solution-in-azure-monitor"></a>Azure 모니터의 에이전트 상태 솔루션
Azure의 에이전트 상태 솔루션을 사용하면 Azure Monitor의 로그 분석 작업 영역에 직접 보고하는 모든 에이전트 또는 응답하지 않는 Azure 모니터에 연결된 시스템 센터 운영 관리자 관리 그룹에 대해 운영 데이터를 제출할 수 있습니다.  또한 얼마나 많은 에이전트가 배포되었는지, 이들 에이전트가 지리적으로 어디에 분산되어 있는지 추적할 수 있으며, Azure, 기타 클라우드 환경 또는 온-프레미스에 배포된 에이전트의 분산 상태를 파악하기 위해 다른 쿼리를 수행할 수 있습니다.    
...
| System Center Operations Manager 관리 그룹 | yes | 하트비트 이벤트는 60초마다 관리 그룹에 보고하는 에이전트에서 수집된 다음 Azure Monitor로 전달됩니다. 운영 관리자 에이전트에서 Azure 모니터로 직접 연결할 필요는 없습니다. 하트비트 이벤트 데이터는 관리 그룹에서 로그 분석 작업 영역으로 전달됩니다.|
...
| 관리 그룹별 배포 | 사용자 환경에서 다른 운영 관리자 관리 그룹의 파티션입니다.|
| 에이전트의 지리적 위치 | 에이전트가 있는 여러 국가/지역의 파티션과 각 국가/지역에 설치된 에이전트 수의 총 수입니다.|
...
| `IsGatewayInstalled` | Log Analytics 게이트웨이가 설치된 경우 값은 *true이며*그렇지 않으면 값이 *false입니다.*|
| `ComputerIP` | 컴퓨터의 공용 IP 주소입니다. Azure VM에서 공용 IP를 사용할 수 있는 경우 공용 IP가 표시됩니다. 개인 IP를 사용하는 VM의 경우 개인 IP 주소가 아닌 Azure SNAT 주소가 표시됩니다. |
...
Operations Manager 관리 서버에 보고하는 각 에이전트는 두 개의 하트비트를 보내고 SCAgentChannel 속성의 값에는 구독에서 활성화한 데이터 원본 및 모니터링 솔루션에 따라 **Direct** 및 **SCManagementServer가** 모두 포함됩니다. 리콜하면 솔루션의 데이터는 Operations Manager 관리 서버에서 Azure Monitor로 직접 전송되거나 에이전트에서 수집된 데이터의 볼륨으로 인해 에이전트에서 Azure Monitor로 직접 전송됩니다. **SCManagementServer** 값이 있는 하트비트 이벤트의 경우, ComputerIP 값은 관리 서버가 데이터를 실제로 업로드하기 때문에 관리 서버의 IP 주소가 됩니다.  SCAgentChannel가 **직접**에 설정된 하트비트의 경우 이 값은 에이전트의 공용 IP 주소입니다.  
...
* 로그 쿼리에서 경고를 생성하는 방법에 대한 자세한 내용은 [Azure 모니터의 경고에](../platform/alerts-overview.md) 대해 알아봅니다. 
