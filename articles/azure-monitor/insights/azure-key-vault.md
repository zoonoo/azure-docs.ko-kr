---
title: Azure 모니터의 Azure 키 볼트 솔루션 | 마이크로 소프트 문서
description: Azure 모니터의 Azure 키 자격 증명 모음 솔루션을 사용하여 Azure 키 자격 증명 모음 로그를 검토할 수 있습니다.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
Azure 모니터의 Azure 키 자격 증명 모음 솔루션을 사용하여 Azure 키 볼트 AuditEvent 로그를 검토할 수 있습니다.
...
1. [솔루션 갤러리의 Azure Monitor 솔루션 추가에](../../azure-monitor/insights/solutions.md) 설명된 프로세스를 사용하여 로그 분석 작업 영역에 Azure 키 자격 증명 모음 솔루션을 추가합니다.
2. [포털](#enable-key-vault-diagnostics-in-the-portal) 또는 [PowerShell을](#enable-key-vault-diagnostics-using-powershell) 사용하여 키 볼트 리소스에 대한 진단 로깅을 사용하여 모니터링할 수 있습니다.
...
8. 로그 분석 작업 영역에 대한 진단 로깅을 사용하려면 *저장을* 클릭합니다.

### <a name="enable-key-vault-diagnostics-using-powershell"></a>PowerShell을 사용하여 Key Vault 진단 사용 설정
다음 PowerShell 스크립트는 키 볼트에 `Set-AzDiagnosticSetting` 대한 리소스 로깅을 활성화하는 데 사용하는 방법의 예를 제공합니다.
```
...
솔루션을 [설치한](https://azuremarketplace.microsoft.com/en-usrketplace/marketplace/apps/Microsoft.KeyVaultAnalyticsOMS?tab=Overview)후 Azure 모니터 **개요** 페이지에서 **키 볼트 분석** 타일을 클릭하여 키 볼트 데이터를 봅니다. **인사이트** 섹션에서 **기타**를 클릭하여 **Azure Monitor** 메뉴에서 이 페이지를 엽니다. 

![Azure Key Vault 타일 이미지](media/azure-key-vault/log-analytics-keyvault-tile.png)

키 볼트 **분석** 타일을 클릭한 후 로그의 요약을 보고 다음 범주에 대한 세부 정보를 드릴인할 수 있습니다.
...
| `ResourceProvider` |*Microsoft. 키볼트* |
| `ResourceType` | *VAULTS* |
...
+ 로그는 저장소 계정을 사용할 필요 없이 Log Analytics 작업 영역에 직접 기록됩니다.
...
1. [키 볼트에서 로그 분석 작업 영역으로 직접 보낼 진단 구성](#enable-key-vault-diagnostics-in-the-portal)  
2. [솔루션 갤러리에서 Azure Monitor 솔루션 추가에](../../azure-monitor/insights/solutions.md) 설명된 프로세스를 사용하여 Azure 키 자격 증명 모음 을 활성화합니다.
...
* [Azure 모니터에서 로그 쿼리를](../../azure-monitor/log-query/log-query-overview.md) 사용하여 자세한 Azure 키 자격 증명 모음 데이터를 볼 수 있습니다.
