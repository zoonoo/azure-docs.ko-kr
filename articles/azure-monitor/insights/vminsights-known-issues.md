...
- 제거하거나 삭제한 Azure VM은 3~7일 동안 VM 목록 보기에 표시됩니다. 또한 제거하거나 삭제한 VM의 상태를 클릭하면 **상태 진단** 보기가 열리고 로딩 루프가 시작됩니다. 삭제한 VM의 이름을 선택하면 VM이 삭제되었다는 내용의 메시지가 포함된 창이 열립니다.
...
- 단일 VM의 **경고 목록** 창과 Azure Monitor에는 모니터 조건이 지난 30일 동안 *발생함*으로 설정된 경고가 표시됩니다. 경고는 구성할 수 없습니다. 그러나 **경고 목록** 창이 열린 후에 요약을 클릭하면 모니터 조건 및 시간 범위의 필터 값을 변경할 수 있습니다.
- **경고 목록** 창에서 **리소스 유형**, **리소스** 및 **모니터 서비스** 필터는 수정하지 않는 것이 좋습니다. 솔루션과 관련된 값으로 구성되었습니다. 이 목록 보기는 **Azure Monitor** > **경고** 목록 보기보다 많은 필드를 표시합니다.   
- Linux VM의 **상태 진단** 보기에는 사용자 정의 VM 이름 대신 VM의 전체 도메인 이름이 표시됩니다.
- VM을 종료하면 일부 상태 조건은 *위험*으로 업데이트되고, 다른 상태 조건은 *정상*으로 업데이트됩니다. 순 VM 상태는 *위험*으로 표시됩니다.
- 상태 경고 심각도는 수정할 수 없습니다. 사용하거나 사용하지 않도록 설정할 수만 있습니다. 또한 일부 심각도는 상태 조건의 상태에 따라 업데이트됩니다.
...
- Windows의 *총 CPU 사용률* 상태 조건은 포털 및 워크로드 모니터링 API 둘 다에서 **4와 같지 않음** 임계값을 표시합니다. *총 CPU 사용률*이 95%를 초과하고 시스템 큐 길이가 15보다 큰 경우 임계값에 도달합니다. 이 릴리스에서는 이 상태 조건을 수정할 수 없습니다. 
...
- 각 상태 조건에 대해 정의된 경고 규칙은 Azure Portal에 표시되지 않습니다. [워크로드 모니터 API](https://github.com/Azure/azure-rest-api-specs/tree/master/specification/workloadmonitor/resource-manager)에서만 상태 경고 규칙을 사용하거나 사용하지 않도록 설정할 수 있습니다. 
