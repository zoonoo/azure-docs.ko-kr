---
title: Azure 모니터의 와이어 데이터 솔루션 | 마이크로 소프트 문서
...
---
# <a name="wire-data-20-preview-solution-in-azure-monitor"></a>Azure 모니터의 와이어 데이터 2.0(미리 보기) 솔루션
...
Log Analytics 에이전트 외에 Wire Data 솔루션은 IT 인프라에서 컴퓨터에 설치하는 Microsoft 종속성 에이전트를 사용합니다. 종속성 에이전트는 사용된 다양한 프로토콜 및 포트를 포함하여 [OSI 모델](https://en.wikipedia.org/wiki/OSI_model)에서 네트워크 수준 2-3에 해당하는 컴퓨터와 주고 받는 네트워크 데이터를 모니터링합니다. 그런 다음 에이전트를 사용하여 Azure Monitor로 데이터가 전송됩니다.  

>[!NOTE]
>서비스 맵을 이미 배포했거나 [VM용](../../azure-monitor/insights/vminsights-overview.md)서비스 맵 또는 Azure 모니터를 고려 중인 경우 Azure Monitor에서 수집하고 저장하는 새 연결 메트릭 데이터 집합이 있어 유선 데이터와 유사한 정보를 제공합니다.

기본적으로 Azure Monitor는 Windows 및 Linux에 내장된 카운터와 지정할 수 있는 다른 성능 카운터에서 CPU, 메모리, 디스크 및 네트워크 성능 데이터에 대한 데이터를 기록합니다. 컴퓨터에 사용되는 서브넷 및 애플리케이션 수준 프로토콜을 포함하여 네트워크 및 기타 데이터 수집이 에이전트별로 실시간으로 이루어집니다.  실시간 데이터는 TCP 전송 계층으로 내려가지 않고 애플리케이션 수준에서 네트워크 데이터를 확인합니다. 솔루션은 개별 ACK 및 SYN을 확인하지 않습니다. 핸드셰이크가 완료되면 라이브 연결로 간주되고 연결됨으로 표시됩니다. 해당 연결은 양쪽에서 소켓이 열려 있고 데이터를 앞뒤로 전달할 수 있음을 동의할 경우 실시간 상태를 유지합니다. 양쪽이 연결을 닫으면 연결이 끊어진 것으로 표시됩니다.  따라서 성공적으로 완료된 패킷의 대역폭만 계산하며 재전송된 패킷 또는 실패한 패킷 수는 보고하지 않습니다.
...
그러나 메타데이터를 보는 것이므로 자세한 문제 해결에 반드시 유용한 것은 아닙니다. Azure 모니터의 와이어 데이터는 네트워크 데이터의 전체 캡처가 아닙니다.  심도 있는 패킷 수준 문제 해결에는 적절하지 않습니다. 에이전트를 다른 컬렉션 방법에 비해 사용하면 어플라이언스를 설치하거나 네트워크 스위치를 다시 구성하거나 복잡한 구성을 수행할 필요가 없다는 이점이 있습니다. 실시간 데이터는 단순히 에이전트 기반이며 컴퓨터에 에이전트를 설치하고 자체 네트워크 트래픽을 모니터링합니다. 또 다른 장점은 사용자가 패브릭 계층을 소유하지 않는 클라우드 공급자, 호스팅 서비스 공급자 또는 Microsoft Azure에서 실행 중인 워크로드를 모니터링하는 경우입니다.

## <a name="connected-sources"></a>연결된 소스

Wire Data는 Microsoft 종속성 에이전트에서 해당 데이터를 가져옵니다. 종속성 에이전트는 Azure Monitor에 대한 연결에 대한 로그 분석 에이전트에 따라 달라집니다. 즉, 서버에 먼저 Log Analytics 에이전트를 설치하고 Dependency Agent로 구성해야 합니다. 다음 표는 Wire Data 솔루션이 지원하는 연결된 원본을 설명합니다.

| **연결된 소스** | **지원됨** | **설명** |
| --- | --- | --- |
| Windows 에이전트 | yes | Wire Data는 Windows 에이전트 컴퓨터에서 데이터를 분석하고 수집합니다. <br><br> [Windows용 로그 분석 에이전트](../platform/agent-windows.md)외에도 Windows 에이전트에는 Microsoft 종속성 에이전트가 필요합니다. 운영 체제 버전의 전체 목록은 [지원되는](vminsights-enable-overview.md#supported-operating-systems) 운영 체제를 참조하십시오. |
| Linux 에이전트 | yes | Wire Data는 Linux 에이전트 컴퓨터에서 데이터를 분석하고 수집합니다.<br><br> [Linux용 로그 분석 에이전트](../learn/quick-collect-linux-computer.md)외에도 Linux 에이전트에는 Microsoft 종속성 에이전트가 필요합니다. 운영 체제 버전의 전체 목록은 [지원되는](vminsights-enable-overview.md#supported-operating-systems) 운영 체제를 참조하십시오. |
| System Center Operations Manager 관리 그룹 | yes | Wire Data는 연결된 [System Center Operations Manager 관리 그룹](../platform/om-agents.md)의 Windows 및 Linux 에이전트에서 데이터를 분석하고 수집합니다. <br><br> 시스템 센터 운영 관리자 에이전트 컴퓨터에서 Azure 모니터로 직접 연결해야 합니다. |
| Azure Storage 계정 | 예 | Wire Data는 에이전트 컴퓨터에서 데이터를 수집하므로 Azure Storage에서 수집할 데이터는 없습니다. |

Windows에서 Microsoft 모니터링 에이전트(MMA)는 시스템 센터 운영 관리자와 Azure 모니터에서 데이터를 수집하고 전송하는 데 사용됩니다. 에이전트는 컨텍스트에 따라 System Center Operations Manager 에이전트, Log Analytics 에이전트, MMA 또는 직접 에이전트라고 합니다. 시스템 센터 운영 관리자 와 Azure 모니터는 MMA의 약간 다른 버전을 제공합니다. 이러한 버전은 각 보고서를 시스템 센터 운영 관리자, Azure Monitor 또는 둘 다에 보고할 수 있습니다.

Linux에서 Linux용 로그 분석 에이전트는 데이터를 수집하고 Azure 모니터로 보냅니다. Azure Monitor에 직접 연결된 에이전트가 있는 서버 또는 시스템 센터 운영 관리자 관리 그룹을 통해 Azure Monitor에 연결하는 서버에서 유선 데이터를 사용할 수 있습니다.

종속성 에이전트는 데이터 자체를 전송하지 않으며 방화벽이나 포트를 변경할 필요가 없습니다. 와이어 데이터의 데이터는 항상 Log Analytics 에이전트에 의해 직접 또는 로그 분석 게이트웨이를 통해 Azure Monitor로 전송됩니다.

![에이전트 다이어그램](./media/wire-data/agents.png)

Azure Monitor에 연결된 관리 그룹이 있는 시스템 센터 운영 관리자 사용자인 경우:

- 시스템 센터 운영 관리자 에이전트가 Azure Monitor에 연결하기 위해 인터넷에 액세스할 수 있는 경우 추가 구성이 필요하지 않습니다.
- 시스템 센터 운영 관리자 에이전트가 인터넷을 통해 Azure Monitor에 액세스할 수 없는 경우 시스템 센터 운영 관리자와 함께 작동하도록 Log Analytics 게이트웨이를 구성해야 합니다.

Windows 또는 Linux 컴퓨터가 서비스에 직접 연결할 수 없는 경우 로그 분석 게이트웨이를 사용하여 Azure 모니터에 연결하도록 로그 분석 에이전트를 구성해야 합니다. [Microsoft 다운로드 센터](https://www.microsoft.com/download/details.aspx?id=52666)에서 Log Analytics 게이트웨이를 다운로드할 수 있습니다.
...
- 종속성 에이전트를 설치하거나 제거하려면 관리자 권한이 필요합니다.
- 종속성 에이전트는 64비트 운영 체제가 있는 컴퓨터에 설치되어야 합니다.
...
다음 섹션에서는 Linux의 종속성 에이전트에 대해 지원되는 운영 체제를 나열합니다.  
...
1. [Azure 마켓플레이스에서](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.WireData2OMS?tab=Overview) 또는 [솔루션 갤러리의 모니터링 솔루션 추가에](../../azure-monitor/insights/solutions.md)설명된 프로세스를 사용하여 활동 로그 분석 솔루션을 활성화합니다.
2. 데이터를 얻을 수 있도록 각 컴퓨터에 종속성 에이전트를 설치합니다. 종속성 에이전트는 인접 한 이웃에 대 한 연결을 모니터링할 수 있습니다.
...
종속성 에이전트는 설치를 통해 Windows를 실행 하는 컴퓨터에 설치됩니다.DependaAgent-Windows.exe. 옵션 없이 이 실행 파일을 실행하면 마법사가 시작되고 대화형으로 설치를 수행할 수 있습니다.

다음 단계를 사용하여 Windows를 실행하는 각 컴퓨터에 종속성 에이전트를 설치합니다.

1. [사용자 환경에서 호스트되는 Windows 컴퓨터에서 데이터 수집](../../azure-monitor/platform/agent-windows.md)의 단계에 따라 Log Analytics 에이전트를 설치합니다.
2. 이전 섹션의 링크를 사용하여 Windows 종속성 에이전트를 다운로드한 다음 다음 명령을 사용하여 실행합니다.`InstallDependencyAgent-Windows.exe`
...
Windows 종속성 에이전트에 대한 파일은 기본적으로 C:\프로그램 파일\Microsoft 종속성 에이전트에 배치됩니다.

### <a name="install-the-dependency-agent-on-linux"></a>Linux에 Dependency Agent 설치

에이전트를 설치 또는 구성하려면 루트 액세스가 필요합니다.

종속성 에이전트는 설치를 통해 리눅스 컴퓨터에 설치됩니다독립에이전트-Linux64.bin, 자체 추출 바이너리가 있는 셸 스크립트. _sh_를 사용하여 파일을 실행하거나 파일 자체에 실행 권한을 추가할 수 있습니다.
...
한 번에 많은 서버에 종속성 에이전트를 쉽게 배포하려면 스크립트를 사용하는 것이 도움이 됩니다. 다음 스크립트 예제를 사용하여 Windows 또는 Linux에서 종속성 에이전트를 다운로드하고 설치할 수 있습니다.
...
원하는 상태 구성을 통해 종속성 에이전트를 배포하려면 xPSDesiredStateConfiguration 모듈과 다음과 같은 약간의 코드를 사용할 수 있습니다.
...
다음 섹션을 사용하여 종속성 에이전트를 제거할 수 있습니다.
...
#### <a name="uninstall-the-dependency-agent-on-linux"></a>Linux에서 종속성 에이전트 제거
...
Log Analytics 작업 영역에서 Wire Data가 활성화되면 해당 작업 영역의 모든 Windows 서버에 300KB 관리 팩이 전송됩니다. [연결된 관리 그룹](../platform/om-agents.md)에서 System Center Operations Manager 에이전트를 사용하는 경우 Dependency Monitor 관리 팩은 System Center Operations Manager에서 배포됩니다. 에이전트가 직접 연결된 경우 Azure Monitor는 관리 팩을 제공합니다.
...
- [솔루션 갤러리에서 모니터링](solutions.md)솔루션 추가에 설명된 프로세스를 사용하여 로그 분석 작업 영역에 와이어 데이터 솔루션을 추가합니다. 추가 구성은 필요 없습니다.
