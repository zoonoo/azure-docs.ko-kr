...
VM insights 게스트 상태는 공개 미리 보기에서 다음과 같은 제한 사항이 있습니다.

- 현재 Azure virtual machines만 지원 됩니다. 서버용 Azure Arc는 현재 지원되지 않습니다.
...
다음 Azure 리소스 공급자를 구독에 등록 하 여 VM insights 게스트 상태를 사용 하도록 설정 합니다. 
...
Azure Portal에서 가상 컴퓨터에 대한 게스트 상태를 사용하도록 설정하면 필요한 모든 구성이 자동으로 수행됩니다. 여기에는 데이터 수집 규칙을 만들고, 가상 컴퓨터에 게스트 상태 확장을 설치 하 고, 데이터 수집 규칙과의 연결을 만드는 작업이 포함 됩니다.

VM insights의 **시작** 보기에서 가상 머신에 대 한 업그레이드 메시지 옆의 링크를 클릭 한 후 **업그레이드** 단추를 클릭 합니다. 여러 가상 컴퓨터를 선택하여 함께 업그레이드할 수도 있습니다.
...
VM insights 게스트 상태의 모니터 구성은 [DCR (데이터 수집 규칙)](../agents/data-collection-rule-overview.md)에 저장 됩니다. 게스트 상태 확장이 있는 각 가상 컴퓨터에는이 규칙과의 연결이 필요 합니다.

> [!NOTE]
> [VM insights 게스트 상태 (미리 보기)의 모니터링 구성](vminsights-health-configure.md)에 설명 된 대로 추가 데이터 수집 규칙을 만들어 모니터의 기본 구성을 수정할 수 있습니다.
...
- [VM insights에서 사용 하는 모니터 사용자 지정](vminsights-health-configure.md)