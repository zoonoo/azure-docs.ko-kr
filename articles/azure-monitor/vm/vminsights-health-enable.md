...
VM용 Azure Monitor의 **시작** 보기에서 가상 머신에 대 한 업그레이드 메시지 옆의 링크를 클릭 한 후 **업그레이드** 단추를 클릭 합니다. 여러 가상 컴퓨터를 선택하여 함께 업그레이드할 수도 있습니다.
...
VM용 Azure Monitor 게스트 상태의 모니터 구성은 [데이터 수집 규칙 (DCR)](../agents/data-collection-rule-overview.md)에 저장 됩니다. 게스트 상태 확장이 있는 각 가상 컴퓨터에는이 규칙과의 연결이 필요 합니다.
