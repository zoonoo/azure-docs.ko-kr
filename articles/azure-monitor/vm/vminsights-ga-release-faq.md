...
작업 영역에서 사용 하도록 설정 된 성능 카운터를 대상으로 하는 테이블을 쿼리 하는 [로그 경고](../alerts/alerts-unified-log.md) 를 만든 경우 `Perf` 대신 테이블을 참조 하도록 이러한 규칙을 업데이트 해야 합니다 `InsightsMetrics` . 이 지침은 및를 사용 하는 모든 로그 검색 규칙에도 적용 됩니다 `ServiceMapComputer_CL` `ServiceMapProcess_CL` . 이러한 데이터 집합은 및 테이블로 이동 하기 때문 `VMComputer` `VMProcess` 입니다.
