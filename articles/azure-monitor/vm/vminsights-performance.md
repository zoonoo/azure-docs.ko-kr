...
다음은 VM insights를 사용한 성능 수집의 제한 사항입니다.
...
**상위 N개 차트** 탭에서 둘 이상의 Log Analytics 작업 영역이 경우 페이지 위쪽의 **작업 영역** 선택기에서 솔루션에서 사용할 수 있도록 설정된 작업 영역을 선택합니다. **그룹** 선택기에서 이 페이지의 차트 및 다른 페이지에 표시되는 결과를 더 자세히 필터링하는 데 사용할 수 있는 선택한 작업 영역과 관련된 컴퓨터의 구독, 리소스 그룹, [컴퓨터 그룹](../logs/computer-groups.md) 및 가상 머신 확장 집합을 반환합니다. 선택 항목은 성능 기능에만 적용되며 상태 또는 맵으로 전달되지 않습니다.  
...
5개 차트 중 하나의 오른쪽 위 모퉁이에 있는 고정 아이콘을 클릭하면 선택한 차트가 마지막으로 본 Azure 대시보드에 고정됩니다.  대시보드에서 차트의 크기를 조정하고 위치를 변경할 수 있습니다. 대시보드에서 차트를 선택 하면 VM 정보로 이동 하 여 올바른 범위와 보기를 로드 합니다.  
...
차트 중 하나의 오른쪽 위 모퉁이에 있는 고정 아이콘을 클릭하면 선택한 차트가 마지막으로 본 Azure 대시보드에 고정됩니다. 대시보드에서 차트의 크기를 조정하고 위치를 변경할 수 있습니다. 대시보드에서 차트를 선택 하면 VM insights로 리디렉션되고 VM에 대 한 성능 세부 정보 보기가 로드 됩니다.  
...
- VM insights에 포함 된 [통합 문서](vminsights-workbooks.md) 를 사용 하 여 성능 및 네트워크 메트릭을 추가로 분석 하는 방법에 대해 알아봅니다.  
