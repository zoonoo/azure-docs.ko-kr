...
1. **구성** -메트릭 경고 규칙 구성을 검토 하 여 제대로 구성 되었는지 확인 합니다.
    - **집계 유형** 및 **집계 세분성 (기간)** 이 예상 대로 구성 되었는지 확인 합니다. **집계 유형은** 메트릭 값을 집계 하는 방법 [(자세히 알아보기)을](./metrics-aggregation-explained.md#aggregation-types)결정 하 고, **집계 세분성 (기간)** 은 경고 규칙이 실행 될 때마다 평가에서 메트릭 값을 집계 하는 정도를 제어 합니다.
