...
3. 차트를 그릴 때 포함하려는 차원 값을 선택합니다(이 예제에서는 성공적인 스토리지 트랜잭션을 필터링하도록 표시함).
   ...
   ![성공적인 저장소 트랜잭션 필터링을 보여 주는 스크린샷](./media/metrics-charts/029.png)
...
2. 차트를 분할하려는 차원을 선택합니다.
   ...
   ![차트를 분할 하는 데 사용할 선택 된 차원을 보여 주는 스크린샷](./media/metrics-charts/031.png)

   이제 차트에서는 차원의 각 세그먼트에 하나씩 여러 줄을 표시합니다.

   ![차원의 각 세그먼트에 대해 하나씩 여러 줄을 보여 주는 스크린샷](./media/metrics-charts/032.png)
...
Y축 범위를 제어하려면 “...” 차트 메뉴를 선택 하 고 **차트 설정을** 선택 하 여 고급 차트 설정에 액세스 합니다.

![차트 설정 옵션을 강조 표시 하는 스크린샷](./media/metrics-charts/033.png)

 Y축 범위 섹션의 값을 수정하거나 **자동** 단추를 사용하여 기본값으로 되돌립니다.
...
차트를 구성한 후 차트의 오른쪽 위 모퉁이에서 **대시보드에 고정** 을 클릭 합니다.

![차트에 고정 하는 방법을 보여 주는 스크린샷](./media/metrics-charts/036.png)
...
![새로운 경고 규칙 단추가 빨간색으로 강조 표시됨](./media/metrics-charts/042.png)

사용자 지정 경고 규칙을 보다 쉽게 생성할 수 있도록 차트의 기본 메트릭 크기가 미리 채워진 경고 규칙 만들기 창으로 이동됩니다.

![경고 규칙 만들기](./media/metrics-charts/041.png)
