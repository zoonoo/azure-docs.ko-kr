...
description: Azure Monitor Logs Analytics 서비스에 대 한 자주 묻는 질문에 대 한 대답입니다.
...
이 Microsoft FAQ는 Azure Monitor Log Analytics 작업 영역에 대 한 자주 묻는 질문의 목록입니다. Log Analytics에 대한 추가 질문이 있으면 [토론 포럼](https://social.msdn.microsoft.com/Forums/azure/home?forum=opinsights)으로 이동하여 질문을 게시하세요. 자주 묻는 질문일 경우 빠르고 쉽게 찾을 수 있도록 이 문서에 추가하겠습니다.
...
A: 에이전트가 Log Analytics 작업 영역과 통신할 수 있도록 하려면 제어판, 보안 & 설정, **Microsoft Monitoring Agent**로 이동 합니다.
...
A: System Center Operations Manager에서 Log Analytics 관리 컴퓨터 목록에서 컴퓨터를 제거 합니다. Operations Manager는 Log Analytics에 더 이상 보고하지 않도록 에이전트의 구성을 업데이트합니다. Log Analytics에 직접 연결된 에이전트인 경우 제어판, 보안 및 설정, **Microsoft Monitoring Agent**를 통해 통신을 중지할 수 있습니다.
...
A: 작업 영역을 다른 구독 또는 리소스 그룹으로 이동하려면 먼저 작업 영역에서 Automation 계정을 연결 해제해야 합니다. Automation 계정을 연결 해제하려면 작업 영역에 설치된 업데이트 관리, 변경 내용 추적 또는 작업 시간 외 VM 시작/중지 솔루션을 제거해야 합니다. 이러한 솔루션을 제거한 후, Automation 계정 리소스의 왼쪽 창에서 **연결된 작업 영역**을 선택하고 리본에서 **작업 영역 연결 해제**를 클릭하여 Automation 계정을 연결 해제합니다.
...
A: API 본문에 ‘etag’를 추가하거나 Azure Resource Manager 템플릿 속성을 추가해야 합니다.
```
"properties": {
   "etag": "*",
...
전체 사용 현황은 [사용량](../../azure-monitor/platform/data-usage.md) 페이지에 표시됩니다.
...
Log Analytics에 대해 자세히 알아보고 몇 분 내에 시작 및 실행 하는 방법에 대 한 자세한 내용은 [Azure Monitor를 시작](../../azure-monitor/overview.md) 하세요.
