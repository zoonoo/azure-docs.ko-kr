...
1. 경고 페이로드는 보안 Webhook 작업을 통해 ITSM 도구에 전송 됩니다.
1. Itsm 응용 프로그램은 ITSM 도구를 시작할 수 있는 권한이 있는 경우 Azure AD를 사용 하 여 확인 합니다.
1. 경고에 권한이 부여 되 면 응용 프로그램은 다음을 수행 합니다.
   
   1. ITSM 도구에서 작업 항목 (예: 인시던트)을 만듭니다.
   1. CI (구성 항목)의 ID를 CMDB (고객 관리 데이터베이스)에 바인딩합니다.
...
* **[일반적인 경고 스키마](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema)**: itsmc에서 경고 페이로드의 스키마는 경고 유형에 따라 달라 집니다. 보안 내보내기에는 모든 경고 유형에 대 한 공통 스키마가 있습니다. 이 공통 스키마에는 모든 경고 유형에 대 한 CI가 포함 되어 있습니다. 모든 경고 유형은 CI를 CMDB에 바인딩할 수 있습니다.
...
1. [Microsoft id 플랫폼에 응용 프로그램 등록](https://docs.microsoft.com/azure/active-directory/develop/quickstart-register-app)의 단계를 따릅니다.
1. Azure AD에서 **응용 프로그램 노출**을 선택 합니다.
1. **응용 프로그램 ID URI**에 대해 **설정** 을 선택 합니다.
...
1. **경고**  >  **관리 작업**을 선택 합니다.
1. [작업 그룹 추가](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups#create-an-action-group-by-using-the-azure-portal)를 선택하고 필드를 입력합니다.
1. **작업 그룹 이름** 상자에 이름을 입력하고 **약식 이름** 상자에 이름을 입력합니다. 약식 이름은 이 그룹을 사용하여 알림을 보내는 경우 전체 작업 그룹 이름 대신 사용됩니다.
1. **보안 Webhook**를 선택 합니다.
1. 다음 세부 정보를 선택 합니다.
   1. 등록 한 Azure Active Directory 인스턴스의 개체 ID를 선택 합니다.
   1. URI에 대해 BMC Helix 환경에서 복사한 webhook URL에 붙여 넣습니다.
   1. **일반 경고 스키마 사용** 을 **예**로 설정 합니다. 
...
1. BMC Helix의 연결 구성의 일부로 통합 BMC 인스턴스로 이동 하 여 다음 지침을 따릅니다.
   ...
   1. **카탈로그**를 선택 합니다.
   1. **Azure alerts**를 선택 합니다.
   1. **커넥터**를 선택 합니다.
   1. **구성**을 선택 합니다.
   1. **새 연결 구성 추가** 를 선택 합니다.
   1. 구성 섹션에 대 한 정보를 입력 합니다.
      - **이름**: 직접 설정 합니다.
      - **권한 부여 유형**: **없음**
      - **설명**: 직접 작성 합니다.
      - **사이트**: **클라우드**
