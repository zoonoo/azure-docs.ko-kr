...
7. 설정 확인: `When an item is selected, export a parameter`
    1. 내보낼 필드: `Request`
...
10. 쿼리 편집기를 사용 하 여 분석을 위한 KQL 입력
    ...
    ```
11. `Run query` 하 여 결과를 확인 합니다.
12. _시각화_ 를 `Area chart` 변경
...
경우에 따라 특정 열이 아니라 선택한 행의 전체 내용을 내보내는 것이 좋습니다. 이러한 경우 위의 7.1 단계에서는 `Field to export` 속성을 설정 하지 않은 상태로 둡니다. 통합 문서는 전체 행 내용을 json으로 매개 변수에 내보냅니다. 

참조 KQL 컨트롤에서 `todynamic` 함수를 사용 하 여 json을 구문 분석 하 고 개별 열에 액세스 합니다.

 ## <a name="grid-cell-clicks"></a>표 형태의 셀 클릭
통합 문서를 사용 하면 작성자가 `link renderer`라는 특수 한 형식의 그리드 열 렌더러를 통해 대화형 작업을 추가할 수 있습니다. 링크 렌더러는 표 형태 셀을 셀의 내용에 따라 하이퍼링크로 변환 합니다. 통합 문서는 리소스 개요 블레이드, 속성 모음 뷰어, 앱 Insights 검색, 사용, 트랜잭션 추적 등을 포함 하 여 다양 한 종류의 링크 렌더러를 지원 합니다.
...
7. _열_ 섹션에서 다음을 설정 합니다.
    1. _샘플_ 열 렌더러: `Link`, 열 보기: `Cell Details`, 링크 레이블: `Sample`
...
3. 매개 변수 값을 `Yes`로 설정 합니다.
4. 영역형 차트를 사용 하는 쿼리 컨트롤에서 _고급 설정_ 아이콘 (기어 아이콘)을 클릭 합니다.
5. 설정을 확인 `Make this item conditionally visible`
    1. 매개 변수 값을 `equals` `ShowDetails` 경우이 항목이 표시 됩니다 `Yes`
6. _편집 완료_ 를 클릭 하 여 변경 내용을 커밋합니다.
7. 통합 문서 도구 모음에서 _편집 완료_ 를 클릭 하 여 읽기 모드로 전환 합니다.
8. 매개 변수 `ShowDetails`의 값을 `No`로 전환 합니다. 아래 차트가 사라집니다.

아래 이미지는 `ShowDetails` 있는 표시 되는 경우를 보여 줍니다 `Yes`

![차트가 표시 되는 조건부 표시 유형을 보여 주는 이미지](./media/workbooks-interactive/conditional-visibility.png)

다음 이미지는 `ShowDetails` 있는 숨겨진 사례를 보여 줍니다 `No`
