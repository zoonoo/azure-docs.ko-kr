...
>
최신 버전의 Azure Resource Manager 모듈을 컴퓨터에 설치하려면 [Azure PowerShell 설치 및 구성](https://docs.microsoft.com/powershell/azure/azurerm/install-azurerm-ps?view=azurermps-4.4.1)을 참조하세요.  
...
Azure 진단으로 메트릭을 수집하고 Log Analytics에 직접 보내도록 지원하는 리소스만 지원됩니다.  자세한 목록은 [Log Analytics에서 사용할 Azure 서비스 로그 및 메트릭 수집](collect-azure-metrics-logs.md)을 검토하세요. 
...
5. 구독이 둘 이상 있는 경우 올바른 값을 입력하여 목록에서 적절한 구독을 선택합니다.<br><br> ![스크립트에서 반환된 구독 선택](./media/collect-azurepass-posh/script-select-subscription.png)<br> 그렇지 않으면 사용 가능한 단일 구독이 자동으로 선택됩니다.
6. 그런 다음 스크립트는 구독에 등록된 Log Analytics 작업 영역 목록을 반환합니다.  목록에서 적절한 항목을 선택합니다.<br><br> ![스크립트에서 반환된 작업 영역 선택](./media/collect-azurepass-posh/script-select-workspace.png)<br> 
7. 컬렉션을 활성화할 Azure 리소스를 선택합니다. 예를 들어, 5를 입력하면 SQL Azure Databases에 데이터 수집을 사용하도록 설정됩니다.<br><br> ![스크립트에서 반환된 리소스 종류 선택](./media/collect-azurepass-posh/script-select-resource.png)<br>
