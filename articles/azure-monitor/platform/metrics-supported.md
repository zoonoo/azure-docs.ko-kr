...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|요청|요청|카운트|합계|지정 된 기간 동안의 총 게이트웨이 요청 수입니다. 문제를 진단 하는 데 도움이 되도록 다양 한 차원으로 분리 될 수 있습니다. |Location, BackendResponseCode, LastErrorReason, GatewayResponseCode|
|TotalRequests|총 게이트웨이 요청|카운트|합계|지정 된 기간 동안의 총 게이트웨이 요청 수입니다. 이 메트릭은 더 이상 사용 되지 않으므로 새 `Requests` 메트릭을 사용 하는 것이 좋습니다. |위치, 호스트 이름|
|SuccessfulRequests|성공적인 게이트웨이 요청|카운트|합계|지정 된 기간에 성공한 총 게이트웨이 요청 수입니다. 이 메트릭은 더 이상 사용 되지 않으므로 새 `Requests` 메트릭을 사용 하는 것이 좋습니다.|위치, 호스트 이름|
|UnauthorizedRequests|허가되지 않은 게이트웨이 요청|카운트|합계| 지정 된 기간에 권한이 없는 총 게이트웨이 요청 수입니다. 이 메트릭은 더 이상 사용 되지 않으므로 새 `Requests` 메트릭을 사용 하는 것이 좋습니다.|위치, 호스트 이름|
|FailedRequests|실패한 게이트웨이 요청|카운트|합계|지정 된 기간에 실패 한 총 게이트웨이 요청 수입니다. 이 메트릭은 더 이상 사용 되지 않으므로 새 `Requests` 메트릭을 사용 하는 것이 좋습니다.|위치, 호스트 이름|
|OtherRequests|기타 게이트웨이 요청|카운트|합계|지정 된 기간에 성공, 승인 안 함 또는 실패 한 범주에 속하지 않는 총 게이트웨이 요청 수입니다. 이 메트릭은 더 이상 사용 되지 않으므로 새 `Requests` 메트릭을 사용 하는 것이 좋습니다. |위치, 호스트 이름|
|기간|게이트웨이 요청의 전체 기간|밀리초|평균|API Management 클라이언트의 요청을 수신 하는 시간과 클라이언트에 대 한 응답을 반환 하는 시간 사이의 시간입니다.|위치, 호스트 이름|
|용량 중심|용량 중심|백분율|평균|추가 부하를 수용할 수 있도록 인스턴스 크기를 조정할지 여부를 결정 하기 위해 API Management 인스턴스에 대 한 로드 표시기입니다.|위치|
|EventHubTotalEvents|총 EventHub 이벤트|카운트|합계|지정 된 기간에 API Management에서 EventHub로 전송 된 총 이벤트 수입니다.|위치|
|EventHubSuccessfulEvents|성공한 EventHub 이벤트|카운트|합계|지정 된 기간 동안 성공한 EventHub 이벤트의 총 수입니다.|위치|
|EventHubTotalFailedEvents|실패 한 EventHub 이벤트|카운트|합계|지정 된 기간에 실패 한 EventHub 이벤트의 총 수입니다.|위치|
|EventHubRejectedEvents|거부 된 EventHub 이벤트|카운트|합계|지정 된 기간에 거부 된 EventHub 이벤트의 총 수 (잘못 된 구성 또는 권한 없음)입니다.|위치|
|EventHubThrottledEvents|제한 된 EventHub 이벤트|카운트|합계|지정 된 기간에 제한 된 EventHub 이벤트의 총 수입니다.|위치|
|EventHubTimedoutEvents|시간 초과 EventHub 이벤트|카운트|합계|지정 된 기간에 시간 초과 된 EventHub 이벤트의 총 수입니다.|위치|
|EventHubDroppedEvents|삭제 된 EventHub 이벤트|카운트|합계|지정 된 기간에 큐 크기 제한에 도달 하 여 건너뛴 총 이벤트 수입니다.|위치|
|EventHubTotalBytesSent|EventHub 이벤트 크기|바이트|합계|지정 된 기간 동안 EventHub 이벤트의 총 크기 (바이트)입니다.|위치|
...
|디스크당 OS QD|OS 디스크 QD (사용 되지 않음)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|차원 없음|
...
|OS 디스크 읽기 바이트/초|OS 디스크 읽기 바이트/초(미리 보기)|초당 개수|평균|OS 디스크의 모니터링 기간에 단일 디스크에서 읽은 바이트/초|차원 없음|
...
|OS 디스크 큐 크기|OS 디스크 큐 깊이 (미리 보기)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|차원 없음|
|인바운드 흐름|인바운드 흐름 (미리 보기)|카운트|평균|인바운드 흐름은 인바운드 방향의 현재 흐름 수 (VM으로 이동 하는 트래픽)입니다.|차원 없음|
|아웃 바운드 흐름|아웃 바운드 흐름 (미리 보기)|카운트|평균|아웃 바운드 흐름은 아웃 바운드 방향의 현재 흐름 수 (VM에서 나가는 트래픽)입니다.|차원 없음|
...
|인바운드 흐름|인바운드 흐름 (미리 보기)|카운트|평균|인바운드 흐름은 인바운드 방향의 현재 흐름 수 (VM으로 이동 하는 트래픽)입니다.|VMName|
|아웃 바운드 흐름|아웃 바운드 흐름 (미리 보기)|카운트|평균|아웃 바운드 흐름은 아웃 바운드 방향의 현재 흐름 수 (VM에서 나가는 트래픽)입니다.|VMName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|TotalPullCount|총 풀 수|카운트|평균|총 이미지 가져오기 수|차원 없음|
|SuccessfulPullCount|성공적인 끌어오기 수|카운트|평균|성공한 이미지 끌어오기 수|차원 없음|
|TotalPushCount|총 푸시 수|카운트|평균|총 이미지 푸시 수|차원 없음|
|SuccessfulPushCount|성공한 푸시 수|카운트|평균|성공한 이미지 푸시 수|차원 없음|
...
|MaxAllowedResourceCount|허용 되는 최대 엔터티 수|카운트|최대||차원 없음|
|MaxAllowedFactorySizeInGbUnits|최대 허용 팩터리 크기 (GB 단위)|카운트|최대||차원 없음|
...
|JobEndedCancelled|취소 된 작업|카운트|합계|취소 된 작업의 수입니다.|차원 없음|
...
|cpu_percent|CPU 백분율|백분율|평균|CPU 백분율|차원 없음|
|memory_percent|메모리 백분율|백분율|평균|메모리 백분율|차원 없음|
|사서함당|IOPS|카운트|평균|초당 IO 작업 수|차원 없음|
|storage_percent|스토리지 비율|백분율|평균|스토리지 비율|차원 없음|
|storage_used|사용된 스토리지|바이트|평균|사용된 스토리지|차원 없음|
|active_connections|활성 연결|카운트|평균|활성 연결|차원 없음|
|network_bytes_egress|네트워크 아웃|바이트|합계|활성 연결에서 네트워크 출력|차원 없음|
...
|d2c.endpoints.egress.builtIn.events|라우팅: 메시지/이벤트에 배달된 메시지|카운트|합계|IoT Hub 라우팅에서 기본 제공 엔드포인트(메시지/이벤트)에 메시지를 성공적으로 배달한 횟수입니다. 이 메트릭은 IoT hub에 대해 라우팅이 사용 되는 경우에만 작동 하기 시작 합니다 (https://aka.ms/iotrouting).|차원 없음|
...
|EventGridDeliveries|Event Grid 배달 (미리 보기)|카운트|합계|Event Grid에 게시 된 IoT Hub 이벤트 수입니다. 성공 및 실패 한 요청의 수에 대해 결과 차원을 사용 합니다. EventType dimension 이벤트 유형 (https://aka.ms/ioteventgrid) 을 표시 합니다.|Result, EventType|
...
|CassandraConnectionClosures|Cassandra 연결 차단|카운트|합계|1 분 세분성으로 보고 된 Cassandra 연결의 수입니다.|지역, ClosureReason|
|CassandraRequestCharges|Cassandra 요청 요금|카운트|합계|Cassandra 요청에 사용 된 RUs|DatabaseName, CollectionName, Region, OperationType, ResourceType|
|CassandraRequests|Cassandra 요청|카운트|카운트|Cassandra 요청 수|DatabaseName, CollectionName, Region, OperationType, ResourceType, ErrorCode|
...
|MetadataRequests|메타데이터 요청|카운트|카운트|메타데이터 요청 수. Cosmos DB는 각 계정에 대 한 시스템 메타 데이터 컬렉션을 유지 관리 하며,이를 통해 컬렉션, 데이터베이스 등의 기능 및 해당 구성을 무료로 열거할 수 있습니다.|DatabaseName, CollectionName, Region, StatusCode, |
...
|PublishFailCount|실패 한 이벤트 게시|카운트|합계|이 토픽에 게시하지 못한 총 이벤트|ErrorType, Error|
|UnmatchedEventCount|일치하지 않는 이벤트|카운트|합계|이 토픽에 대한 이벤트 구독과 일치하지 않는 총 이벤트|차원 없음|
|PublishSuccessLatencyInMs|게시 성공 대기 시간|카운트|합계|게시 성공 대기 시간 (밀리초)|차원 없음|
...
|INREQS|들어오는 요청 (사용 되지 않음)|카운트|합계|네임 스페이스에 대 한 총 들어오는 전송 요청 (사용 되지 않음)|차원 없음|
|SUCCREQ|성공한 요청 (사용 되지 않음)|카운트|합계|네임 스페이스에 대 한 총 성공한 요청 (사용 되지 않음)|차원 없음|
|FAILREQ|실패 한 요청 (사용 되지 않음)|카운트|합계|네임 스페이스에 대해 실패 한 총 요청 (사용 되지 않음)|차원 없음|
|SVRBSY|서버 작업 중 오류 (사용 되지 않음)|카운트|합계|네임 스페이스에 대 한 총 서버 사용 중 오류 (사용 되지 않음)|차원 없음|
|INTERR|내부 서버 오류 (사용 되지 않음)|카운트|합계|네임 스페이스에 대 한 총 내부 서버 오류 (사용 되지 않음)|차원 없음|
|MISCERR|기타 오류 (사용 되지 않음)|카운트|합계|네임 스페이스에 대해 실패 한 총 요청 (사용 되지 않음)|차원 없음|
|INMSGS|들어오는 메시지 (사용 되지 않음) (사용 되지 않음)|카운트|합계|네임스페이스에 들어오는 총 메시지 수 이 메트릭은 사용되지 않습니다. 대신 들어오는 메시지 메트릭을 사용 하세요 (사용 되지 않음).|차원 없음|
|EHINMSGS|들어오는 메시지(사용되지 않음)|카운트|합계|네임 스페이스에 대 한 총 수신 메시지 (사용 되지 않음)|차원 없음|
|OUTMSGS|보내는 메시지 (사용 되지 않음) (사용 되지 않음)|카운트|합계|네임스페이스에 보내는 총 메시지 수 이 메트릭은 사용되지 않습니다. 대신 보내는 메시지 메트릭을 사용 하세요 (사용 되지 않음).|차원 없음|
|EHOUTMSGS|보내는 메시지(사용되지 않음)|카운트|합계|네임 스페이스에 대 한 총 보내는 메시지 (사용 되지 않음)|차원 없음|
...
|EHABL|백로그 메시지 보관 (사용 되지 않음)|카운트|합계|네임 스페이스에 대 한 백로그의 이벤트 허브 보관 메시지 (사용 되지 않음)|차원 없음|
|EHAMSGS|보관 메시지 (사용 되지 않음)|카운트|합계|네임 스페이스에서 이벤트 허브 보관 된 메시지 (사용 되지 않음)|차원 없음|
...
|availabilityResults/개수|가용성 테스트|카운트|카운트|가용성 테스트 수|availabilityResult/name, availabilityResult/location, availabilityResult/success|
...
|dependencies/failed|종속성 호출 오류|카운트|카운트|외부 리소스에 대해 애플리케이션이 만든 실패한 종속성 호출 수입니다.|dependency/type, dependency/performanceBucket, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|요청/요금|서버 요청 빈도|초당 개수|평균|초당 서버 요청 수|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
| ContinuousExportNumOfRecordsExported | 연속 내보내기에서 내보낸 레코드 수 | 카운트 | 합계 | 내보내기 작업 중에 작성 된 모든 저장소 아티팩트에 대해 내보낸 레코드 수  | 없음 |
...
| ContinuousExportMaxLatenessMinutes | 연속 내보내기 최대 지연 시간 (분) | 카운트 | 최대 | 보류 중 이며 실행할 준비가 된 모든 연속 내보내기의 최대 시간 (분)입니다. | 없음 |
...
|RunsCancelled|실행 취소 됨|카운트|합계|취소 된 워크플로 실행 수입니다.|차원 없음|
...
|RunSuccessLatency|실행 성공 대기 시간|초|평균|성공한 워크플로 실행 대기 시간|차원 없음|
|RunThrottledEvents|실행 제한 이벤트|카운트|합계|워크플로 작업 또는 트리거 제한 이벤트 수|차원 없음|
|RunStartThrottledEvents|시작 제한 이벤트 실행|카운트|합계|워크플로 실행 시작 제한 이벤트의 수입니다.|차원 없음|
|RunFailurePercentage|실행 오류 비율|백분율|합계|실행 실패한 워크플로 비율.|차원 없음|
|ActionsStarted|작업 시작됨 |카운트|합계|시작된 워크플로 작업 수|차원 없음|
|ActionsCompleted|작업 완료됨 |카운트|합계|완료된 워크플로 작업 수|차원 없음|
|ActionsSucceeded|작업 성공함 |카운트|합계|성공한 워크플로 작업 수|차원 없음|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|완료 된 실행|완료 된 실행|카운트|합계|이 작업 영역에 대해 성공적으로 완료 된 실행 수|시나리오|
|실행 시작|실행 시작|카운트|합계|이 작업 영역에 대해 시작 된 실행 수|시나리오|
|실패한 실행|실패한 실행|카운트|합계|이 작업 영역에 대해 실패 한 실행 수|시나리오|
...
|DataProcessed 됨|처리된 데이터|바이트|합계|방화벽을 통과 하는 데이터 양|차원 없음|
|FirewallHealthState|방화벽 성능 상태|백분율|평균|방화벽의 상태를 나타냅니다.|상태, 이유|
|SNATPortUtilization|SNAT 포트 사용률|백분율|평균|방화벽에서 사용한 SNAT 포트의 백분율입니다.|차원 없음|
...
|CapacityUnits|현재 용량 단위|카운트|평균|소비 된 용량 단위|차원 없음|
...
|incoming.scheduled|전송된 예약된 푸시 알림|카운트|합계|예약 된 푸시 알림 취소 됨|차원 없음|
|incoming.scheduled.cancel|예약 된 푸시 알림 취소 됨|카운트|합계|예약 된 푸시 알림 취소 됨|차원 없음|
...
|outgoing.apns.badchannel|APNS 잘못된 채널 오류|카운트|합계|토큰이 잘못 되어 실패 한 푸시의 수입니다 (APNS 이진 프로토콜 상태 코드: 8). APNS HTTP 프로토콜 상태 코드: 400 "BadDeviceToken").|차원 없음|
|outgoing.apns.expiredchannel|APNS 만료된 채널 오류|카운트|합계|APNS 피드백 채널에서 무효화된 토큰의 수입니다.|차원 없음|
|outgoing.apns.invalidnotificationsize|APNS 잘못된 알림 크기 오류|카운트|합계|페이로드가 너무 커서 실패 한 푸시의 수입니다 (APNS 이진 프로토콜 상태 코드: 7).|차원 없음|
...
|DeadletteredMessages|큐/토픽에서 배달 못 한 메시지의 수입니다. (Preview)|카운트|평균|큐/토픽에서 배달 못 한 메시지의 수입니다. (Preview)|EntityName|
|ScheduledMessages|큐/토픽에 있는 예약 된 메시지 수입니다. (Preview)|카운트|평균|큐/토픽에 있는 예약 된 메시지 수입니다. (Preview)|EntityName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|AllocatedCpu|AllocatedCpu|카운트|평균|Millicores에서이 컨테이너에 할당 된 Cpu|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
...
|ActualCpu|ActualCpu|카운트|평균|Millicores의 실제 CPU 사용량|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
...
|컨테이너 상태|컨테이너 상태|카운트|평균|Service Fabric 메시 응용 프로그램의 컨테이너 상태|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName, Status|
...
|BlobCount|Blob 수|카운트|합계|스토리지 계정의 Blob service에 있는 Blob 수입니다.|BlobType|       |BlobCount|Blob 수|카운트|평균|스토리지 계정의 Blob service에 있는 Blob 수입니다.|BlobType, 계층|
...
|StorageSyncServerHeartbeat 비트|서버 온라인 상태|카운트|최대|등록 된 서버가 클라우드 끝점과 하트 비트를 성공적으로 기록할 때마다 1 값을 기록 하는 메트릭입니다.|ServerName|
...
|WarmStorageMaxProperties|웜 저장소 최대 속성|카운트|최대|S1/S2 SKU에 대해 환경에서 허용 하는 최대 속성 수 및 PAYG SKU에 대 한 웜 저장소에서 허용 하는 최대 속성 수|차원 없음|
|WarmStorageUsedProperties|웜 저장소 사용 속성 |카운트|최대|S1/S2 SKU에 대해 환경에서 사용 하는 속성 수 및 PAYG SKU에 대 한 웜 저장소에서 사용 하는 속성 수|차원 없음|
...
|DiskReadOperations|디스크 읽기 작업|카운트|합계|이전 샘플 기간의 IO 읽기 작업 수입니다. 이러한 작업에는 가변 크기를 사용할 수 있습니다.|차원 없음|
|DiskWriteOperations|디스크 쓰기 작업|카운트|합계|이전 샘플 기간의 IO 쓰기 작업 수입니다. 이러한 작업에는 가변 크기를 사용할 수 있습니다.|차원 없음|
