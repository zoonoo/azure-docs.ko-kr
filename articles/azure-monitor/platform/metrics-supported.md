...
|private_bytes_metric|프라이빗 바이트 |바이트|평균|The total amount of memory the Analysis Services engine process and Mashup container processes have allocated, not including memory shared with other processes.|ServerResourceType|
|virtual_bytes_metric|가상 바이트 |바이트|평균|The current size of the virtual address space that Analysis Services engine process and Mashup container processes are using.|ServerResourceType|
...
|mashup_engine_private_bytes_metric|M Engine Private Bytes |바이트|평균|The total amount of memory Mashup container processes have allocated, not including memory shared with other processes.|ServerResourceType|
|mashup_engine_virtual_bytes_metric|M Engine Virtual Bytes |바이트|평균|The current size of the virtual address space Mashup container processes are using.|ServerResourceType|

## <a name="microsoftapimanagementservice"></a>Microsoft.ApiManagement/service

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|요청|요청|카운트|합계|The total number of gateway requests in a given period. It can be sliced by various dimensions to help you diagnose issues. |Location, BackendResponseCode, LastErrorReason, GatewayResponseCode|
|TotalRequests|총 게이트웨이 요청|카운트|합계|The total number of gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric. |위치, 호스트 이름|
|SuccessfulRequests|성공적인 게이트웨이 요청|카운트|합계|The total number of successful gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|위치, 호스트 이름|
|UnauthorizedRequests|허가되지 않은 게이트웨이 요청|카운트|합계| The total number of unauthorized gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|위치, 호스트 이름|
|FailedRequests|실패한 게이트웨이 요청|카운트|합계|The total number of failed gateway requests in a given period. This metric has been deprecated, we recommend using the new `Requests` metric.|위치, 호스트 이름|
|OtherRequests|기타 게이트웨이 요청|카운트|합계|The total number of gateway requests in a given period that do not fall into the successful, unauthorized, or failed categories. This metric has been deprecated, we recommend using the new `Requests` metric. |위치, 호스트 이름|
|기간|게이트웨이 요청의 전체 기간|밀리초|평균|The time between when API Management receives a request from a client and when it returns a response to the client.|위치, 호스트 이름|
|용량 중심|용량 중심|백분율|평균|Indicator of load on an API Management instance for making informed decisions whether to scale the instance to accommodate more load.|위치|
|EventHubTotalEvents|Total EventHub Events|카운트|합계|The total number of events sent to EventHub from API Management in a given period.|위치|
|EventHubSuccessfulEvents|Successful EventHub Events|카운트|합계|The total number of successful EventHub events in a given period.|위치|
|EventHubTotalFailedEvents|Failed EventHub Events|카운트|합계|The total number of failed EventHub events in a given period.|위치|
|EventHubRejectedEvents|Rejected EventHub Events|카운트|합계|The total number of rejected EventHub events (wrong configuration or unauthorized) in a given period.|위치|
|EventHubThrottledEvents|Throttled EventHub Events|카운트|합계|The total number of throttled EventHub events in a given period.|위치|
|EventHubTimedoutEvents|Timed Out EventHub Events|카운트|합계|The total number of timed out EventHub events in a given period.|위치|
|EventHubDroppedEvents|Dropped EventHub Events|카운트|합계|The total number of events skipped because of queue size limit reached in a given period.|위치|
|EventHubTotalBytesSent|Size of EventHub Events|바이트|합계|The total size of EventHub events in bytes in a given period.|위치|
...
|TotalUpdateDeploymentRuns|Total Update Deployment Runs|카운트|합계|Total software update deployment runs|SoftwareUpdateConfigurationName, Status|
|TotalUpdateDeploymentMachineRuns|Total Update Deployment Machine Runs|카운트|합계|Total software update deployment machine runs in a software update deployment run|SoftwareUpdateConfigurationName, Status, TargetComputer, SoftwareUpdateConfigurationRunId|
...
|setcommands9|설정(분할 9)|카운트|합계||차원 없음|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|TotalCalls|총 호출|카운트|합계|총 호출 수.|ApiName, OperationName, Region|
|SuccessfulCalls|성공한 호출|카운트|합계|성공한 호출 수입니다.|ApiName, OperationName, Region|
|TotalErrors|총 오류|카운트|합계|오류 응답(HTTP 응답 코드 4xx 또는 5xx)이 있는 총 호출 수입니다.|ApiName, OperationName, Region|
|BlockedCalls|차단된 호출|카운트|합계|요금 또는 할당량 한도를 초과한 호출 수입니다.|ApiName, OperationName, Region|
|ServerErrors|서버 오류|카운트|합계|서비스 내부 오류(HTTP 응답 코드 5xx)가 있는 호출 수입니다.|ApiName, OperationName, Region|
|ClientErrors|클라이언트 오류|카운트|합계|클라이언트 쪽 오류(HTTP 응답 코드 4xx)가 있는 호출 수입니다.|ApiName, OperationName, Region|
|DataIn|데이터 입력|바이트|합계|들어오는 데이터 크기(바이트)입니다.|ApiName, OperationName, Region|
|DataOut|데이터 출력|바이트|합계|나가는 데이터 크기(바이트)입니다.|ApiName, OperationName, Region|
|대기 시간|대기 시간|밀리초|평균|대기 시간(밀리초)입니다.|ApiName, OperationName, Region|
|CharactersTranslated|변환된 문자|카운트|합계|들어오는 텍스트 요청에 있는 문자의 총 수입니다.|ApiName, OperationName, Region|
|CharactersTrained|Characters Trained|카운트|합계|Total number of characters trained.|ApiName, OperationName, Region|
|SpeechSessionDuration|음성 세션 기간|초|합계|음성 세션의 총 기간(초)입니다.|ApiName, OperationName, Region|
|TotalTransactions|총 트랜잭션|카운트|합계|총 트랜잭션 수|차원 없음|
|TotalTokenCalls|총 토큰 호출|카운트|합계|총 토큰 호출 수입니다.|ApiName, OperationName, Region|
...
|---|---|---|---|---|---|
|백분율 CPU|백분율 CPU|백분율|평균|현재 Virtual Machine에서 사용 중인 할당된 컴퓨팅 단위의 백분율|차원 없음|
|네트워크 인|Network In Billable|바이트|합계|The number of billable bytes received on all network interfaces by the Virtual Machine(s) (Incoming Traffic)|차원 없음|
|네트워크 아웃|Network Out Billable|바이트|합계|The number of billable bytes out on all network interfaces by the Virtual Machine(s) (Outgoing Traffic)|차원 없음|
|디스크 읽기 바이트|디스크 읽기 바이트|바이트|합계|Bytes read from disk during monitoring period|차원 없음|
|디스크 쓰기 바이트|디스크 쓰기 바이트|바이트|합계|Bytes written to disk during monitoring period|차원 없음|
|디스크 읽기 작업/초|디스크 읽기 작업/초|초당 개수|평균|디스크 읽기 IOPS|차원 없음|
|디스크 쓰기 작업/초|디스크 쓰기 작업/초|초당 개수|평균|디스크 쓰기 IOPS|차원 없음|
...
|디스크당 읽기 바이트/초|Data Disk Read Bytes/Sec (Deprecated)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period|SlotId|
|디스크당 쓰기 바이트/초|Data Disk Write Bytes/Sec (Deprecated)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period|SlotId|
|디스크당 읽기 작업/초|Data Disk Read Operations/Sec (Deprecated)|초당 개수|평균|Read IOPS from a single disk during monitoring period|SlotId|
|디스크당 쓰기 작업/초|Data Disk Write Operations/Sec (Deprecated)|초당 개수|평균|Write IOPS from a single disk during monitoring period|SlotId|
|디스크당 QD|Data Disk QD (Deprecated)|카운트|평균|데이터 디스크 큐 깊이(또는 큐 길이)|SlotId|
|디스크당 OS 읽기 바이트/초|OS Disk Read Bytes/Sec (Deprecated)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period for OS disk|차원 없음|
|디스크당 OS 쓰기 바이트/초|OS Disk Write Bytes/Sec (Deprecated)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period for OS disk|차원 없음|
|디스크당 OS 읽기 작업/초|OS Disk Read Operations/Sec (Deprecated)|초당 개수|평균|Read IOPS from a single disk during monitoring period for OS disk|차원 없음|
|디스크당 OS 쓰기 작업/초|OS Disk Write Operations/Sec (Deprecated)|초당 개수|평균|Write IOPS from a single disk during monitoring period for OS disk|차원 없음|
|디스크당 OS QD|OS Disk QD (Deprecated)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|차원 없음|
|Data Disk Read Bytes/sec|데이터 디스크 읽기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period|LUN|
|Data Disk Write Bytes/sec|데이터 디스크 쓰기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period|LUN|
|Data Disk Read Operations/Sec|데이터 디스크 읽기 작업/초(미리 보기)|초당 개수|평균|Read IOPS from a single disk during monitoring period|LUN|
|Data Disk Write Operations/Sec|데이터 디스크 쓰기 작업/초(미리 보기)|초당 개수|평균|Write IOPS from a single disk during monitoring period|LUN|
|Data Disk Queue Depth|Data Disk Queue Depth (Preview)|카운트|평균|데이터 디스크 큐 깊이(또는 큐 길이)|LUN|
|OS Disk Read Bytes/sec|OS 디스크 읽기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period for OS disk|차원 없음|
|OS Disk Write Bytes/sec|OS 디스크 쓰기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period for OS disk|차원 없음|
|OS Disk Read Operations/Sec|OS 디스크 읽기 작업/초(미리 보기)|초당 개수|평균|Read IOPS from a single disk during monitoring period for OS disk|차원 없음|
|OS Disk Write Operations/Sec|OS 디스크 쓰기 작업/초(미리 보기)|초당 개수|평균|Write IOPS from a single disk during monitoring period for OS disk|차원 없음|
|OS Disk Queue Depth|OS Disk Queue Depth (Preview)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|차원 없음|
|Inbound Flows|Inbound Flows (Preview)|카운트|평균|Inbound Flows are number of current flows in the inbound direction (traffic going into the VM)|차원 없음|
|Outbound Flows|Outbound Flows (Preview)|카운트|평균|Outbound Flows are number of current flows in the outbound direction (traffic going out of the VM)|차원 없음|
|Inbound Flows Maximum Creation Rate|Inbound Flows Maximum Creation Rate (Preview)|초당 개수|평균|The maximum creation rate of inbound flows (traffic going into the VM)|차원 없음|
|Outbound Flows Maximum Creation Rate|Outbound Flows Maximum Creation Rate (Preview)|초당 개수|평균|The maximum creation rate of outbound flows (traffic going out of the VM)|차원 없음|
|Premium Data Disk Cache Read Hit|Premium Data Disk Cache Read Hit (Preview)|백분율|평균|Premium Data Disk Cache Read Hit|LUN|
|Premium Data Disk Cache Read Miss|Premium Data Disk Cache Read Miss (Preview)|백분율|평균|Premium Data Disk Cache Read Miss|LUN|
|Premium OS Disk Cache Read Hit|Premium OS Disk Cache Read Hit (Preview)|백분율|평균|Premium OS Disk Cache Read Hit|차원 없음|
|Premium OS Disk Cache Read Miss|Premium OS Disk Cache Read Miss (Preview)|백분율|평균|Premium OS Disk Cache Read Miss|차원 없음|
|Network In Total|Network In Total|바이트|합계|Virtual Machine이 모든 네트워크 인터페이스에서 수신한(들어오는 트래픽) 바이트 수|차원 없음|
|Network Out Total|Network Out Total|바이트|합계|Virtual Machine이 모든 네트워크 인터페이스에서 내보낸(나가는 트래픽) 바이트 수|차원 없음|
...
|네트워크 인|Network In Billable|바이트|합계|The number of billable bytes received on all network interfaces by the Virtual Machine(s) (Incoming Traffic)|VMName|
|네트워크 아웃|Network Out Billable|바이트|합계|The number of billable bytes out on all network interfaces by the Virtual Machine(s) (Outgoing Traffic)|VMName|
|디스크 읽기 바이트|디스크 읽기 바이트|바이트|합계|Bytes read from disk during monitoring period|VMName|
|디스크 쓰기 바이트|디스크 쓰기 바이트|바이트|합계|Bytes written to disk during monitoring period|VMName|
...
|디스크당 OS 쓰기 작업/초|OS Disk Write Operations/Sec (Deprecated)|초당 개수|평균|Write IOPS from a single disk during monitoring period for OS disk|차원 없음|
|디스크당 OS QD|OS Disk QD (Deprecated)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|차원 없음|
|Data Disk Read Bytes/sec|데이터 디스크 읽기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period|LUN, VMName|
|Data Disk Write Bytes/sec|데이터 디스크 쓰기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period|LUN, VMName|
|Data Disk Read Operations/Sec|데이터 디스크 읽기 작업/초(미리 보기)|초당 개수|평균|Read IOPS from a single disk during monitoring period|LUN, VMName|
|Data Disk Write Operations/Sec|데이터 디스크 쓰기 작업/초(미리 보기)|초당 개수|평균|Write IOPS from a single disk during monitoring period|LUN, VMName|
|Data Disk Queue Depth|Data Disk Queue Depth (Preview)|카운트|평균|데이터 디스크 큐 깊이(또는 큐 길이)|LUN, VMName|
|OS Disk Read Bytes/sec|OS 디스크 읽기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec read from a single disk during monitoring period for OS disk|VMName|
|OS Disk Write Bytes/sec|OS 디스크 쓰기 바이트/초(미리 보기)|초당 개수|평균|Bytes/Sec written to a single disk during monitoring period for OS disk|VMName|
|OS Disk Read Operations/Sec|OS 디스크 읽기 작업/초(미리 보기)|초당 개수|평균|Read IOPS from a single disk during monitoring period for OS disk|VMName|
|OS Disk Write Operations/Sec|OS 디스크 쓰기 작업/초(미리 보기)|초당 개수|평균|Write IOPS from a single disk during monitoring period for OS disk|VMName|
|OS Disk Queue Depth|OS Disk Queue Depth (Preview)|카운트|평균|OS 디스크 큐 깊이(또는 큐 길이)|VMName|
|Inbound Flows|Inbound Flows (Preview)|카운트|평균|Inbound Flows are number of current flows in the inbound direction (traffic going into the VM)|VMName|
|Outbound Flows|Outbound Flows (Preview)|카운트|평균|Outbound Flows are number of current flows in the outbound direction (traffic going out of the VM)|VMName|
|Inbound Flows Maximum Creation Rate|Inbound Flows Maximum Creation Rate (Preview)|초당 개수|평균|The maximum creation rate of inbound flows (traffic going into the VM)|VMName|
|Outbound Flows Maximum Creation Rate|Outbound Flows Maximum Creation Rate (Preview)|초당 개수|평균|The maximum creation rate of outbound flows (traffic going out of the VM)|VMName|
|Premium Data Disk Cache Read Hit|Premium Data Disk Cache Read Hit (Preview)|백분율|평균|Premium Data Disk Cache Read Hit|LUN, VMName|
|Premium Data Disk Cache Read Miss|Premium Data Disk Cache Read Miss (Preview)|백분율|평균|Premium Data Disk Cache Read Miss|LUN, VMName|
|Premium OS Disk Cache Read Hit|Premium OS Disk Cache Read Hit (Preview)|백분율|평균|Premium OS Disk Cache Read Hit|VMName|
|Premium OS Disk Cache Read Miss|Premium OS Disk Cache Read Miss (Preview)|백분율|평균|Premium OS Disk Cache Read Miss|VMName|
|Network In Total|Network In Total|바이트|합계|Virtual Machine이 모든 네트워크 인터페이스에서 수신한(들어오는 트래픽) 바이트 수|VMName|
|Network Out Total|Network Out Total|바이트|합계|Virtual Machine이 모든 네트워크 인터페이스에서 내보낸(나가는 트래픽) 바이트 수|VMName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|TotalPullCount|Total Pull Count|카운트|평균|Number of image pulls in total|차원 없음|
|SuccessfulPullCount|Successful Pull Count|카운트|평균|Number of successful image pulls|차원 없음|
|TotalPushCount|Total Push Count|카운트|평균|Number of image pushes in total|차원 없음|
|SuccessfulPushCount|Successful Push Count|카운트|평균|Number of successful image pushes|차원 없음|
|RunDuration|Run Duration|밀리초|합계|Run Duration in milliseconds|차원 없음|
...
|kube_node_status_condition|다양한 노드 조건에 대한 상태|카운트|합계|다양한 노드 조건에 대한 상태|condition, status, status2, node|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|NICReadThroughput|Read Throughput (Network)|초당 바이트 수|평균|The read throughput of the network interface on the device in the reporting period for all volumes in the gateway.|InstanceName|
|NICWriteThroughput|Write Throughput (Network)|초당 바이트 수|평균|The write throughput of the network interface on the device in the reporting period for all volumes in the gateway.|InstanceName|
|CloudReadThroughputPerShare|Cloud Download Throughput (Share)|초당 바이트 수|평균|The download throughput to Azure from a share during the reporting period.|공유|
|CloudUploadThroughputPerShare|Cloud Upload Throughput (Share)|초당 바이트 수|평균|The upload throughput to Azure from a share during the reporting period.|공유|
|BytesUploadedToCloudPerShare|Cloud Bytes Uploaded (Share)|바이트|평균|The total number of bytes that is uploaded to Azure from a share during the reporting period.|공유|
|TotalCapacity|Total Capacity|바이트|평균|Total Capacity|차원 없음|
|AvailableCapacity|Available Capacity|바이트|평균|The available capacity in bytes during the reporting period.|차원 없음|
|CloudUploadThroughput|Cloud Upload Throughput|초당 바이트 수|평균|The cloud upload throughput  to Azure during the reporting period.|차원 없음|
|CloudReadThroughput|Cloud Download Throughput|초당 바이트 수|평균|The cloud download throughput to Azure during the reporting period.|차원 없음|
|BytesUploadedToCloud|Cloud Bytes Uploaded (Device)|바이트|평균|The total number of bytes that is uploaded to Azure from a device during the reporting period.|차원 없음|
|HyperVVirtualProcessorUtilization|Edge Compute - Percentage CPU|백분율|평균|Percent CPU Usage|InstanceName|
|HyperVMemoryUtilization|Edge Compute - Memory Usage|백분율|평균|Amount of RAM in Use|InstanceName|
...
|MaxAllowedResourceCount|Maximum allowed entities count|카운트|최대||차원 없음|
|MaxAllowedFactorySizeInGbUnits|Maximum allowed factory size (GB unit)|카운트|최대||차원 없음|
|ResourceCount|Total entities count|카운트|최대||차원 없음|
|FactorySizeInGbUnits|Total factory size (GB unit)|카운트|최대||차원 없음|
...
|JobEndedCancelled|Canceled Jobs|카운트|합계|Count of canceled jobs.|차원 없음|
|JobAUEndedSuccess|성공한 AU 시간|초|합계|성공한 작업에 대한 총 AU 시간입니다.|차원 없음|
|JobAUEndedFailure|실패한 AU 시간|초|합계|실패한 작업에 대한 총 AU 시간입니다.|차원 없음|
|JobAUEndedCancelled|Canceled AU Time|초|합계|Total AU time for canceled jobs.|차원 없음|

## <a name="microsoftdatalakestoreaccounts"></a>Microsoft.DataLakeStore/accounts

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|TotalStorage|총 스토리지|바이트|최대|계정에 저장된 총 데이터 양.|차원 없음|
...
|storage_percent|스토리지 비율|백분율|평균|스토리지 비율|차원 없음|
|storage_used|사용된 스토리지|바이트|평균|사용된 스토리지|차원 없음|
|storage_limit|스토리지 제한|바이트|평균|스토리지 제한|차원 없음|
|serverlog_storage_percent|서버 로그 스토리지 비율|백분율|평균|서버 로그 스토리지 비율|차원 없음|
...
|active_connections|활성 연결|카운트|평균|활성 연결|차원 없음|
|connections_failed|실패한 연결|카운트|합계|실패한 연결|차원 없음|
|seconds_behind_master|복제 지연 시간(초)|카운트|평균|복제 지연 시간(초)|차원 없음|
|backup_storage_used|Backup Storage used|바이트|평균|Backup Storage used|차원 없음|
...
|network_bytes_egress|네트워크 아웃|바이트|합계|활성 연결에서 네트워크 출력|차원 없음|
|network_bytes_ingress|네트워크 인|바이트|합계|활성 연결에서 네트워크 입력|차원 없음|
|pg_replica_log_delay_in_seconds|복제본 지연 시간|초|최대|Replica lag in seconds|차원 없음|
|pg_replica_log_delay_in_bytes|복제본 간 최대 지연 시간|바이트|최대|Lag in bytes of the most lagging replica|차원 없음|
...
|cpu_percent|CPU 백분율|백분율|평균|CPU 백분율|차원 없음|
|memory_percent|메모리 백분율|백분율|평균|메모리 백분율|차원 없음|
|iops|IOPS|카운트|평균|IO Operations per second|차원 없음|
|storage_percent|스토리지 비율|백분율|평균|스토리지 비율|차원 없음|
|storage_used|사용된 스토리지|바이트|평균|사용된 스토리지|차원 없음|
|active_connections|활성 연결|카운트|평균|활성 연결|차원 없음|
|network_bytes_egress|네트워크 아웃|바이트|합계|활성 연결에서 네트워크 출력|차원 없음|
...
|d2c.endpoints.egress.builtIn.events|라우팅: 메시지/이벤트에 배달된 메시지|카운트|합계|IoT Hub 라우팅에서 기본 제공 엔드포인트(메시지/이벤트)에 메시지를 성공적으로 배달한 횟수입니다. This metric only starts working when routing is enabled (https://aka.ms/iotrouting) for the IoT hub.|차원 없음|
|d2c.endpoints.latency.builtIn.events|라우팅: 메시지/이벤트에 대한 메시지 대기 시간|밀리초|평균|IoT Hub에 대한 메시지 수신과 기본 제공 엔드포인트(메시지.이벤트)에 대한 원격 분석 메시지 수신 간의 평균 대기 시간(밀리초)입니다. This metric only starts working when routing is enabled (https://aka.ms/iotrouting) for the IoT hub.|차원 없음|
|d2c.endpoints.egress.storage|라우팅: 스토리지에 배달된 메시지|카운트|합계|IoT Hub 라우팅에서 스토리지 엔드포인트에 메시지를 성공적으로 배달한 횟수입니다.|차원 없음|
...
|d2c.endpoints.egress.storage.bytes|라우팅: 스토리지에 배달된 데이터|바이트|합계|IoT Hub 라우팅에서 스토리지 엔드포인트에 배달된 데이터 양입니다(바이트).|차원 없음|
|d2c.endpoints.egress.storage.blobs|라우팅: 스토리지에 배달된 Blob|카운트|합계|IoT Hub 라우팅에서 스토리지 엔드포인트에 Blob을 배달한 횟수입니다.|차원 없음|
|EventGridDeliveries|Event Grid deliveries (preview)|카운트|합계|The number of IoT Hub events published to Event Grid. Use the Result dimension for the number of successful and failed requests. EventType dimension shows the type of event (https://aka.ms/ioteventgrid).|Result, EventType|
...
|deviceDataUsage|Total device data usage|바이트|합계|IotHub에 연결된 모든 디바이스에서 전송된 바이트|차원 없음|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|AvailableStorage|사용 가능한 스토리지|바이트|합계|Total available storage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|CassandraConnectionClosures|Cassandra 연결 차단|카운트|합계|Number of Cassandra connections that were closed, reported at a 1 minute granularity|Region, ClosureReason|
|CassandraRequestCharges|Cassandra 요청 요금|카운트|합계|RUs consumed for Cassandra requests made|DatabaseName, CollectionName, Region, OperationType, ResourceType|
|CassandraRequests|Cassandra 요청|카운트|카운트|Number of Cassandra requests made|DatabaseName, CollectionName, Region, OperationType, ResourceType, ErrorCode|
|DataUsage|데이터 사용량|바이트|합계|Total data usage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|DocumentCount|문서 수|카운트|합계|Total document count reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|DocumentQuota|문서 할당량|바이트|합계|Total storage quota reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|IndexUsage|인덱스 사용량|바이트|합계|Total index usage reported at 5 minutes granularity|CollectionName, DatabaseName, Region|
|MetadataRequests|메타데이터 요청|카운트|카운트|메타데이터 요청 수. Cosmos DB maintains system metadata collection for each account, that allows you to enumerate collections, databases, etc., and their configurations, free of charge.|DatabaseName, CollectionName, Region, StatusCode, |
...
|ReplicationLatency|P99 Replication Latency|밀리초|평균|지역 사용 계정에 대한 원본 및 대상 지역의 P99 복제 대기 시간|SourceRegion, TargetRegion|
|ServiceAvailability|서비스 가용성|백분율|평균|Account requests availability at one hour, day or month granularity|차원 없음|
...
|PublishFailCount|Publish Failed Events|카운트|합계|이 토픽에 게시하지 못한 총 이벤트|ErrorType, Error|
|UnmatchedEventCount|일치하지 않는 이벤트|카운트|합계|이 토픽에 대한 이벤트 구독과 일치하지 않는 총 이벤트|차원 없음|
|PublishSuccessLatencyInMs|Publish Success Latency|카운트|합계|Publish success latency in milliseconds|차원 없음|
...
|INREQS|Incoming Requests (Deprecated)|카운트|합계|Total incoming send requests for a namespace (Deprecated)|차원 없음|
|SUCCREQ|Successful Requests (Deprecated)|카운트|합계|Total successful requests for a namespace (Deprecated)|차원 없음|
|FAILREQ|Failed Requests (Deprecated)|카운트|합계|Total failed requests for a namespace (Deprecated)|차원 없음|
|SVRBSY|Server Busy Errors (Deprecated)|카운트|합계|Total server busy errors for a namespace (Deprecated)|차원 없음|
|INTERR|Internal Server Errors (Deprecated)|카운트|합계|Total internal server errors for a namespace (Deprecated)|차원 없음|
|MISCERR|Other Errors (Deprecated)|카운트|합계|Total failed requests for a namespace (Deprecated)|차원 없음|
|INMSGS|Incoming Messages (Deprecated) (Deprecated)|카운트|합계|네임스페이스에 들어오는 총 메시지 수 이 메트릭은 사용되지 않습니다. Please use Incoming Messages metric instead (Deprecated)|차원 없음|
|EHINMSGS|들어오는 메시지(사용되지 않음)|카운트|합계|Total incoming messages for a namespace (Deprecated)|차원 없음|
|OUTMSGS|Outgoing Messages (Deprecated) (Deprecated)|카운트|합계|네임스페이스에 보내는 총 메시지 수 이 메트릭은 사용되지 않습니다. Please use Outgoing Messages metric instead (Deprecated)|차원 없음|
|EHOUTMSGS|보내는 메시지(사용되지 않음)|카운트|합계|Total outgoing messages for a namespace (Deprecated)|차원 없음|
|EHINMBS|Incoming bytes (Deprecated) (Deprecated)|바이트|합계|네임스페이스에 들어오는 Event Hub 메시지 처리량 이 메트릭은 사용되지 않습니다. Please use Incoming bytes metric instead (Deprecated)|차원 없음|
|EHINBYTES|들어오는 바이트(사용되지 않음)|바이트|합계|Event Hub incoming message throughput for a namespace (Deprecated)|차원 없음|
|EHOUTMBS|Outgoing bytes (Deprecated) (Deprecated)|바이트|합계|네임스페이스에 보내는 Event Hub 메시지 처리량 이 메트릭은 사용되지 않습니다. Please use Outgoing bytes metric instead (Deprecated)|차원 없음|
|EHOUTBYTES|보내는 바이트(사용되지 않음)|바이트|합계|Event Hub outgoing message throughput for a namespace (Deprecated)|차원 없음|
|EHABL|Archive backlog messages (Deprecated)|카운트|합계|Event Hub archive messages in backlog for a namespace (Deprecated)|차원 없음|
|EHAMSGS|Archive messages (Deprecated)|카운트|합계|Event Hub archived messages in a namespace (Deprecated)|차원 없음|
|EHAMBS|Archive message throughput (Deprecated)|바이트|합계|Event Hub archived message throughput in a namespace (Deprecated)|차원 없음|

## <a name="microsofteventhubclusters"></a>Microsoft.EventHub/clusters

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|SuccessfulRequests|성공한 요청(미리 보기)|카운트|합계|Microsoft.EventHub에 대한 성공한 요청. (Preview)|차원 없음|
|ServerErrors|서버 오류. (Preview)|카운트|합계|Microsoft.EventHub에 대한 서버 오류. (Preview)|차원 없음|
|UserErrors|사용자 오류. (Preview)|카운트|합계|Microsoft.EventHub에 대한 사용자 오류. (Preview)|차원 없음|
|QuotaExceededErrors|할당량 초과 오류. (Preview)|카운트|합계|Microsoft.EventHub에 대한 할당량 초과 오류. (Preview)|차원 없음|
|ThrottledRequests|제한된 요청. (Preview)|카운트|합계|Microsoft.EventHub에 대한 제한된 요청. (Preview)|차원 없음|
|IncomingRequests|들어오는 요청(미리 보기)|카운트|합계|Microsoft.EventHub에 대한 들어오는 요청. (Preview)|차원 없음|
|IncomingMessages|들어오는 메시지(미리 보기)|카운트|합계|Microsoft.EventHub에 대한 들어오는 메시지. (Preview)|차원 없음|
|OutgoingMessages|보내는 메시지(미리 보기)|카운트|합계|Microsoft.EventHub에 대한 보내는 메시지. (Preview)|차원 없음|
|IncomingBytes|들어오는 바이트 (Preview)|바이트|합계|Microsoft.EventHub에 대한 들어오는 바이트. (Preview)|차원 없음|
|OutgoingBytes|보내는 바이트 (Preview)|바이트|합계|Microsoft.EventHub에 대한 보내는 바이트. (Preview)|차원 없음|
|ActiveConnections|활성 연결(미리 보기)|카운트|평균|Microsoft.EventHub에 대한 총 활성 연결. (Preview)|차원 없음|
|ConnectionsOpened|열린 연결. (Preview)|카운트|평균|Microsoft.EventHub에 대한 열린 연결. (Preview)|차원 없음|
|ConnectionsClosed|끊어진 연결. (Preview)|카운트|평균|Microsoft.EventHub에 대한 끊어진 연결. (Preview)|차원 없음|
|CaptureBacklog|캡처 백로그. (Preview)|카운트|합계|Microsoft.EventHub에 대한 캡처 백로그. (Preview)|차원 없음|
|CapturedMessages|캡처된 메시지. (Preview)|카운트|합계|Microsoft.EventHub에 대한 캡처된 메시지. (Preview)|차원 없음|
|CapturedBytes|캡처된 바이트. (Preview)|바이트|합계|Microsoft.EventHub에 대한 캡처된 바이트. (Preview)|차원 없음|
...
|NumActiveWorkers|Number of Active Workers|카운트|최대|Number of Active Workers|ClusterDnsName, MetricName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|availabilityResults/availabilityPercentage|가용성|백분율|평균|Percentage of successfully completed availability tests|availabilityResult/name, availabilityResult/location|
|availabilityResults/count|가용성 테스트|카운트|카운트|Count of availability tests|availabilityResult/name, availabilityResult/location, availabilityResult/success|
|availabilityResults/duration|Availability test duration|밀리초|평균|Availability test duration|availabilityResult/name, availabilityResult/location, availabilityResult/success|
...
|dependencies/failed|Dependency call failures|카운트|카운트|외부 리소스에 대해 애플리케이션이 만든 실패한 종속성 호출 수입니다.|dependency/type, dependency/performanceBucket, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|requests/rate|Server request rate|초당 개수|평균|Rate of server requests per second|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
|KeepAlive|활성 유지|카운트|평균|온전성 검사는 쿼리에 대한 클러스터 응답을 나타냅니다.|없음|
...
|EventProcessedForEventHubs|이벤트 처리됨(Event Hubs)|카운트|합계|이벤트 허브에서 수집하는 경우 클러스터에서 처리된 이벤트 수|없음|
|IngestionResult|수집 결과|카운트|카운트|수집 작업 수|상태|
|CPU|CPU|백분율|평균|CPU 사용률 수준|없음|
| ContinuousExportNumOfRecordsExported | Number of records exported in continuous export | 카운트 | 합계 | Number of records exported for every storage artifact written during the export operation  | 없음 |
| ExportUtilization | Export Utilization | 백분율 | 최대 | Export utilization | 없음 |
| ContinuousExportPendingCount | Continuous Export Pending Count | 카운트 | 최대 | The number of pending continuous export jobs ready for execution | 없음 |
| ContinuousExportMaxLatenessMinutes | Continuous Export Max Lateness Minutes | 카운트 | 최대 | The max time in minutes of all continuous exports which are pending and ready for execution | 없음 |

## <a name="microsoftlocationbasedservicesaccounts"></a>Microsoft.LocationBasedServices/accounts

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|사용량|사용량|카운트|카운트|API 호출 수|ApiCategory, ApiName|
...
|RunsCancelled|Runs Canceled|카운트|합계|Number of workflow runs canceled.|차원 없음|
...
|BillingUsageStorageConsumption|스토리지 소비 실행에 대한 청구 사용량|카운트|합계|청구되는 스토리지 소비 실행 수입니다.|차원 없음|
|BillingUsageNativeOperation|기본 작업 실행에 대한 청구 사용량|카운트|합계|청구되는 기본 작업 실행 수입니다.|차원 없음|
|BillingUsageStandardConnector|표준 커넥터 실행에 대한 청구 사용량|카운트|합계|청구되는 표준 커넥터 실행 수입니다.|차원 없음|
|BillingUsageStorageConsumption|스토리지 소비 실행에 대한 청구 사용량|카운트|합계|청구되는 스토리지 소비 실행 수입니다.|차원 없음|
...
|RunSuccessLatency|실행 성공 대기 시간|초|평균|성공한 워크플로 실행 대기 시간|차원 없음|
|RunThrottledEvents|실행 제한 이벤트|카운트|합계|워크플로 작업 또는 트리거 제한 이벤트 수|차원 없음|
|RunStartThrottledEvents|Run Start Throttled Events|카운트|합계|Number of workflow run start throttled events.|차원 없음|
|RunFailurePercentage|실행 오류 비율|백분율|합계|실행 실패한 워크플로 비율.|차원 없음|
|ActionsStarted|작업 시작됨 |카운트|합계|시작된 워크플로 작업 수|차원 없음|
|ActionsCompleted|작업 완료됨 |카운트|합계|완료된 워크플로 작업 수|차원 없음|
|ActionsSucceeded|작업 성공함 |카운트|합계|성공한 워크플로 작업 수|차원 없음|
...
|TriggerSuccessLatency|트리거 성공 대기 시간 |초|평균|성공한 워크플로 트리거 대기 시간|차원 없음|
|TriggerThrottledEvents|트리거 제한 이벤트|카운트|합계|워크플로 트리거 제한 이벤트 수|차원 없음|
|IntegrationServiceEnvironmentWorkflowProcessorUsage|Workflow Processor Usage for Integration Service Environment|백분율|평균|Workflow processor usage for integration service environment.|차원 없음|
|IntegrationServiceEnvironmentWorkflowMemoryUsage|Workflow Memory Usage for Integration Service Environment|백분율|평균|Workflow memory usage for integration service environment.|차원 없음|
|IntegrationServiceEnvironmentConnectorProcessorUsage|Connector Processor Usage for Integration Service Environment|백분율|평균|Connector processor usage for integration service environment.|차원 없음|
|IntegrationServiceEnvironmentConnectorMemoryUsage|Connector Memory Usage for Integration Service Environment|백분율|평균|Connector memory usage for integration service environment.|차원 없음|

## <a name="microsoftmachinelearningservicesworkspaces"></a>Microsoft.MachineLearningServices/workspaces

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|Completed Runs|Completed Runs|카운트|합계|Number of runs completed successfully for this workspace|시나리오|
|Started Runs|Started Runs|카운트|합계|Number of runs started for this workspace|시나리오|
|실패한 실행|실패한 실행|카운트|합계|Number of runs failed for this workspace|시나리오|
...
|가용성|가용성|백분율|평균|Availability of the APIs|ApiCategory, ApiName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|ApplicationRuleHit|Application rules hit count|카운트|합계|Number of times Application rules were hit|Status, Reason, Protocol|
|NetworkRuleHit|Network rules hit count|카운트|합계|Number of times Network rules were hit|Status, Reason, Protocol|
...
|CapacityUnits|Current Capacity Units|카운트|평균|Capacity Units consumed|차원 없음|
...
|AverageRoundtripMs|Avg. Round-trip Time (ms)|밀리초|평균|원본과 대상 간에 전송된 연결 모니터링 프로브의 평균 네트워크 왕복 시간(ms)|차원 없음|
...
|incoming.scheduled|전송된 예약된 푸시 알림|카운트|합계|Scheduled Push Notifications Canceled|차원 없음|
|incoming.scheduled.cancel|Scheduled Push Notifications Canceled|카운트|합계|Scheduled Push Notifications Canceled|차원 없음|
...
|outgoing.apns.badchannel|APNS 잘못된 채널 오류|카운트|합계|The count of pushes that failed because the token is invalid (APNS binary protocol status code: 8. APNS HTTP protocol status code: 400 with "BadDeviceToken").|차원 없음|
|outgoing.apns.expiredchannel|APNS 만료된 채널 오류|카운트|합계|APNS 피드백 채널에서 무효화된 토큰의 수입니다.|차원 없음|
|outgoing.apns.invalidnotificationsize|APNS 잘못된 알림 크기 오류|카운트|합계|The count of pushes that failed because the payload was too large (APNS binary protocol status code: 7).|차원 없음|
...
|Average_Disk Write Bytes/sec|디스크 쓰기 바이트/초|카운트|평균|Average_Disk Write Bytes/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Disk Writes/sec|디스크 쓰기/초|카운트|평균|Average_Disk Writes/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Avg. 디스크 초/읽기|Avg. Disk sec/Read|카운트|평균|Average_Avg. 디스크 초/읽기|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Avg. 디스크 초/전송|Avg. Disk sec/Transfer|카운트|평균|Average_Avg. 디스크 초/전송|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Avg. 디스크 초/쓰기|Avg. Disk sec/Write|카운트|평균|Average_Avg. 디스크 초/쓰기|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|ServerErrors|서버 오류. (Preview)|카운트|합계|Microsoft.ServiceBus에 대한 서버 오류. (Preview)|EntityName|
|UserErrors|사용자 오류. (Preview)|카운트|합계|Microsoft.ServiceBus에 대한 사용자 오류. (Preview)|EntityName|
|ThrottledRequests|제한된 요청. (Preview)|카운트|합계|Microsoft.ServiceBus에 대한 제한된 요청. (Preview)|EntityName|
|IncomingRequests|들어오는 요청(미리 보기)|카운트|합계|Microsoft.ServiceBus에 대한 들어오는 요청. (Preview)|EntityName|
|IncomingMessages|들어오는 메시지(미리 보기)|카운트|합계|Microsoft.ServiceBus에 대한 들어오는 메시지. (Preview)|EntityName|
|OutgoingMessages|보내는 메시지(미리 보기)|카운트|합계|Microsoft.ServiceBus에 대한 보내는 메시지. (Preview)|EntityName|
|ActiveConnections|활성 연결(미리 보기)|카운트|합계|Microsoft.ServiceBus에 대한 총 활성 연결. (Preview)|차원 없음|
|크기|크기(미리 보기)|바이트|평균|큐/토픽 크기(바이트) (Preview)|EntityName|
|메시지|큐/토픽에 있는 메시지 수 (Preview)|카운트|평균|큐/토픽에 있는 메시지 수 (Preview)|EntityName|
|ActiveMessages|큐/토픽에 있는 활성 메시지 수 (Preview)|카운트|평균|큐/토픽에 있는 활성 메시지 수 (Preview)|EntityName|
|DeadletteredMessages|Count of dead-lettered messages in a Queue/Topic. (Preview)|카운트|평균|Count of dead-lettered messages in a Queue/Topic. (Preview)|EntityName|
|ScheduledMessages|Count of scheduled messages in a Queue/Topic. (Preview)|카운트|평균|Count of scheduled messages in a Queue/Topic. (Preview)|EntityName|
|CPUXNS|네임스페이스당 CPU 사용량|백분율|최대|서비스 버스 프리미엄 네임스페이스 CPU 사용량 메트릭|차원 없음|
|WSXNS|네임스페이스당 메모리 크기 사용량|백분율|최대|서비스 버스 프리미엄 네임스페이스 메모리 사용량 메트릭|차원 없음|

## <a name="microsoftservicefabricmeshapplications"></a>Microsoft.ServiceFabricMesh/applications

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|AllocatedCpu|AllocatedCpu|카운트|평균|Cpu allocated to this container in millicores|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|AllocatedMemory|AllocatedMemory|바이트|평균|Memory allocated to this container in MB|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|ActualCpu|ActualCpu|카운트|평균|Actual CPU usage in millicores|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|ActualMemory|ActualMemory|바이트|평균|Actual memory usage in MB|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|CpuUtilization|CpuUtilization|백분율|평균|Utilization of CPU for this container as percentage of AllocatedCpu|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|MemoryUtilization|MemoryUtilization|백분율|평균|Utilization of CPU for this container as percentage of AllocatedCpu|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName|
|ApplicationStatus|ApplicationStatus|카운트|평균|Status of Service Fabric Mesh application|ApplicationName, Status|
|ServiceStatus|ServiceStatus|카운트|평균|Health Status of a service in Service Fabric Mesh application|ApplicationName, Status, ServiceName|
|ServiceReplicaStatus|ServiceReplicaStatus|카운트|평균|Health Status of a service replica in Service Fabric Mesh application|ApplicationName, Status, ServiceName, ServiceReplicaName|
|ContainerStatus|ContainerStatus|카운트|평균|Status of the container in Service Fabric Mesh application|ApplicationName, ServiceName, CodePackageName, ServiceReplicaName, Status|
|RestartCount|RestartCount|카운트|평균|Restart count of a container in Service Fabric Mesh application|ApplicationName, Status, ServiceName, ServiceReplicaName, CodePackageName|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|allocated_data_storage|Data space allocated|바이트|평균|Data space allocated. Not applicable to data warehouses.|차원 없음|
|app_cpu_billed|App CPU billed|카운트|합계|App CPU billed. Applies to serverless databases.|차원 없음|
|app_cpu_percent|App CPU percentage|백분율|평균|App CPU percentage. Applies to serverless databases.|차원 없음|
|app_memory_percent|App memory used percentage|백분율|평균|App memory used percentage. Applies to serverless databases.|차원 없음|
|blocked_by_firewall|방화벽에 의해 차단|카운트|합계|방화벽에 의해 차단|차원 없음|
|cache_hit_percent|캐시 적중 비율|백분율|최대|Cache hit percentage. Applies only to data warehouses.|차원 없음|
|cache_used_percent|캐시 사용 비율|백분율|최대|Cache used percentage. Applies only to data warehouses.|차원 없음|
...
|cpu_limit|CPU limit|카운트|평균|CPU limit. Applies to vCore-based databases.|차원 없음|
|cpu_used|CPU used|카운트|평균|CPU used. Applies to vCore-based databases.|차원 없음|
|교착 상태|교착 상태|카운트|합계|Deadlocks. Not applicable to data warehouses.|차원 없음|
|diff_backup_size_bytes|Differential backup storage size|바이트|최대|Cumulative differential backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|차원 없음|
|dtu_limit|DTU 제한|카운트|평균|DTU Limit. Applies to DTU-based databases.|차원 없음|
|dtu_consumption_percent|DTU 비율|백분율|평균|DTU percentage. Applies to DTU-based databases.|차원 없음|
|dtu_used|DTU 사용됨|카운트|평균|DTU used. Applies to DTU-based databases.|차원 없음|
...
|dwu_consumption_percent|DWU 백분율|백분율|최대|DWU percentage. Applies only to data warehouses.|차원 없음|
|dwu_limit|DWU 제한|카운트|최대|DWU limit. Applies only to data warehouses.|차원 없음|
|dwu_used|DWU 사용됨|카운트|최대|DWU used. Applies only to data warehouses.|차원 없음|
|full_backup_size_bytes|Full backup storage size|바이트|최대|Cumulative full backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|차원 없음|
|local_tempdb_usage_percent|로컬 tempdb 백분율|백분율|평균|Local tempdb percentage. Applies only to data warehouses.|차원 없음|
|log_backup_size_bytes|Log backup storage size|바이트|최대|Cumulative log backup storage size. Applies to General Purpose and Business Critical databases. Does not currently apply to Manage instance databases.|차원 없음|
|log_write_percent|로그 IO 비율|백분율|평균|Log IO percentage. Not applicable to data warehouses.|차원 없음|
|physical_data_read_percent|데이터 IO 비율|백분율|평균|데이터 IO 비율|차원 없음|
|sessions_percent|세션 백분율|백분율|평균|Sessions percentage. Not applicable to data warehouses.|차원 없음|
|sqlserver_process_core_percent|SQL Server process core percent|백분율|최대|This metric is a placeholder and not populated at this time.|차원 없음|
|sqlserver_process_memory_percent|SQL Server process memory percent|백분율|최대|This metric is a placeholder and not populated at this time.|차원 없음|
|스토리지|Data space used|바이트|최대|Total database size. Not applicable to data warehouses.|차원 없음|
|storage_percent|Data space used percent|백분율|최대|Database size percentage. Not applicable to data warehouses or hyperscale databases.|차원 없음|
|tempdb_data_size|Tempdb Data File Size Kilobytes|카운트|최대|Tempdb Data File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|차원 없음|
|tempdb_log_size|Tempdb Log File Size Kilobytes|카운트|최대|Tempdb Log File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|차원 없음|
|tempdb_log_used_percent|Tempdb Percent Log Used|백분율|최대|Tempdb Percent Log Used. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|차원 없음|
|workers_percent|작업자 백분율|백분율|평균|Workers percentage. Not applicable to data warehouses.|차원 없음|
|xtp_storage_percent|메모리 내 OLTP 스토리지 백분율|백분율|평균|In-Memory OLTP storage percent. Not applicable to data warehouses.|차원 없음|

## <a name="microsoftsqlserverselasticpools"></a>Microsoft.Sql/servers/elasticPools

|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|allocated_data_storage|Data space allocated|바이트|평균|Data space allocated|차원 없음|
|allocated_data_storage_percent|Data space allocated percent|백분율|최대|Data space allocated percent|차원 없음|
|cpu_limit|CPU limit|카운트|평균|CPU limit. Applies to vCore-based elastic pools.|차원 없음|
|cpu_percent|CPU 비율|백분율|평균|CPU 비율|차원 없음|
|cpu_used|CPU used|카운트|평균|CPU used. Applies to vCore-based elastic pools.|차원 없음|
|dtu_consumption_percent|DTU 비율|백분율|평균|DTU percentage. Applies to DTU-based elastic pools.|차원 없음|
|eDTU_limit|eDTU 제한|카운트|평균|eDTU limit. Applies to DTU-based elastic pools.|차원 없음|
|eDTU_used|eDTU 사용|카운트|평균|eDTU used. Applies to DTU-based elastic pools.|차원 없음|
|log_write_percent|로그 IO 비율|백분율|평균|로그 IO 비율|차원 없음|
|physical_data_read_percent|데이터 IO 비율|백분율|평균|데이터 IO 비율|차원 없음|
|sessions_percent|세션 백분율|백분율|평균|세션 백분율|차원 없음|
|storage_limit|Data max size|바이트|평균|스토리지 제한|차원 없음|
|storage_percent|Data space used percent||백분율|평균|스토리지 비율|차원 없음|
|storage_used|Data space used|바이트|평균|사용된 스토리지|차원 없음|
|sqlserver_process_core_percent|SQL Server process core percent|백분율|최대|This metric is a placeholder and not populated at this time.|차원 없음|
|sqlserver_process_memory_percent|SQL Server process memory percent|백분율|최대|This metric is a placeholder and not populated at this time.|차원 없음|
|tempdb_data_size|Tempdb Data File Size Kilobytes|카운트|최대|Tempdb Data File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|차원 없음|
|tempdb_log_size|Tempdb Log File Size Kilobytes|카운트|최대|Tempdb Log File Size Kilobytes. Not applicable to data warehouses. This metric will be available for databases using the vCore purchasing model or 100 DTU and higher for DTU-based purchasing models.|차원 없음|
...
|BlobCount|Blob 수|카운트|합계|스토리지 계정의 Blob service에 있는 Blob 수입니다.|BlobType|       |BlobCount|Blob 수|카운트|평균|스토리지 계정의 Blob service에 있는 Blob 수입니다.|BlobType, Tier|
|ContainerCount|Blob 컨테이너 수|카운트|평균|스토리지 계정의 Blob service에 있는 컨테이너 수입니다.|차원 없음|
|IndexCapacity|Index Capacity|바이트|평균|The amount of storage used by ADLS Gen2 (Hierarchical) Index in bytes.|차원 없음|
|트랜잭션|트랜잭션|카운트|합계|스토리지 서비스 또는 지정된 API 작업에 대해 제기된 요청 수입니다. 이 수는 성공 및 실패 요청뿐만 아니라 오류를 발생시킨 요청도 포함합니다. 다른 종류의 응답 수에 ResponseType 차원을 사용합니다.|ResponseType, GeoType, ApiName, Authentication|
|수신|수신|바이트|합계|수신 데이터 양(바이트)입니다. 이 수는 외부 클라이언트에서 Azure Storage로 수신뿐만 아니라 Azure 내의 수신도 포함합니다.|GeoType, ApiName, Authentication|
|송신|송신|바이트|합계|송신 데이터 양(바이트)입니다. 이 수는 외부 클라이언트에서 Azure Storage로 송신뿐만 아니라 Azure 내의 송신도 포함합니다. 따라서 이 수는 청구 가능한 송신을 반영하지 않습니다.|GeoType, ApiName, Authentication|
|SuccessServerLatency|성공 서버 대기 시간|밀리초|평균|Azure Storage에서 성공적인 요청을 처리하는 데 사용한 평균 대기 시간(밀리초)입니다. 이 값은 AverageE2ELatency에 지정된 네트워크 대기 시간을 포함하지 않습니다.|GeoType, ApiName, Authentication|
...
|FileCount|파일 수|카운트|평균|스토리지 계정의 파일 서비스에 있는 파일 수입니다.|차원 없음|
|FileShareCount|파일 공유 수|카운트|평균|스토리지 계정의 파일 서비스에 있는 파일 공유 수입니다.|차원 없음|
|트랜잭션|트랜잭션|카운트|합계|스토리지 서비스 또는 지정된 API 작업에 대해 제기된 요청 수입니다. 이 수는 성공 및 실패 요청뿐만 아니라 오류를 발생시킨 요청도 포함합니다. 다른 종류의 응답 수에 ResponseType 차원을 사용합니다.|ResponseType, GeoType, ApiName, Authentication|
|수신|수신|바이트|합계|수신 데이터 양(바이트)입니다. 이 수는 외부 클라이언트에서 Azure Storage로 수신뿐만 아니라 Azure 내의 수신도 포함합니다.|GeoType, ApiName, Authentication|
|송신|송신|바이트|합계|송신 데이터 양(바이트)입니다. 이 수는 외부 클라이언트에서 Azure Storage로 송신뿐만 아니라 Azure 내의 송신도 포함합니다. 따라서 이 수는 청구 가능한 송신을 반영하지 않습니다.|GeoType, ApiName, Authentication|
|SuccessServerLatency|성공 서버 대기 시간|밀리초|평균|Azure Storage에서 성공적인 요청을 처리하는 데 사용한 평균 대기 시간(밀리초)입니다. 이 값은 AverageE2ELatency에 지정된 네트워크 대기 시간을 포함하지 않습니다.|GeoType, ApiName, Authentication|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|ServerSyncSessionResult|Sync Session Result|카운트|평균|Metric that logs a value of 1 each time the Server Endpoint successfully completes a Sync Session with the Cloud Endpoint|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncSyncSessionAppliedFilesCount|Files Synced|카운트|합계|Count of Files synced|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncSyncSessionPerItemErrorsCount|동기화 상태가 아닌 파일|카운트|합계|Count of files failed to sync|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncBatchTransferredFileBytes|동기화되는 바이트 수|바이트|합계|Total file size transferred for Sync Sessions|SyncGroupName, ServerEndpointName, SyncDirection|
|StorageSyncServerHeartbeat|Server Online Status|카운트|최대|Metric that logs a value of 1 each time the registered server successfully records a heartbeat with the Cloud Endpoint|ServerName|
|StorageSyncRecallIOTotalSizeBytes|클라우드 계층화 회수|바이트|합계|Total size of data recalled by the server|ServerName|
...
|WarmStorageMaxProperties|Warm Storage Max Properties|카운트|최대|Maximum number of properties used allowed by the environment for S1/S2 SKU and maximum number of properties allowed by Warm Store for PAYG SKU|차원 없음|
|WarmStorageUsedProperties|Warm Storage Used Properties |카운트|최대|Number of properties used by the environment for S1/S2 SKU and number of properties used by Warm Store for PAYG SKU|차원 없음|
...
|메트릭|메트릭 표시 이름|단위|집계 형식|설명|차원|
|---|---|---|---|---|---|
|DiskReadBytesPerSecond|디스크 읽기 바이트/초|초당 바이트 수|평균|Average disk throughput due to read operations over the sample period.|차원 없음|
|DiskWriteBytesPerSecond|디스크 쓰기 바이트/초|초당 바이트 수|평균|Average disk throughput due to write operations over the sample period.|차원 없음|
|디스크 읽기 바이트|디스크 읽기 바이트|바이트|합계|Total disk throughput due to read operations over the sample period.|차원 없음|
|디스크 쓰기 바이트|디스크 쓰기 바이트|바이트|합계|Total disk throughput due to write operations over the sample period.|차원 없음|
|DiskReadOperations|Disk Read Operations|카운트|합계|The number of IO read operations in the previous sample period. Note that these operations may be variable sized.|차원 없음|
|DiskWriteOperations|Disk Write Operations|카운트|합계|The number of IO write operations in the previous sample period. Note that these operations may be variable sized.|차원 없음|
|디스크 읽기 작업/초|디스크 읽기 작업/초|초당 개수|평균|The average number of IO read operations in the previous sample period. Note that these operations may be variable sized.|차원 없음|
|디스크 쓰기 작업/초|디스크 쓰기 작업/초|초당 개수|평균|The average number of IO write operations in the previous sample period. Note that these operations may be variable sized.|차원 없음|
|DiskReadLatency|Disk Read Latency|밀리초|평균|Total read latency. The sum of the device and kernel read latencies.|차원 없음|
|DiskWriteLatency|Disk Write Latency|밀리초|평균|Total write latency. The sum of the device and kernel write latencies.|차원 없음|
|NetworkInBytesPerSecond|Network In Bytes/Sec|초당 바이트 수|평균|Average network throughput for received traffic.|차원 없음|
|NetworkOutBytesPerSecond|Network Out Bytes/Sec|초당 바이트 수|평균|Average network throughput for transmitted traffic.|차원 없음|
|네트워크 인|네트워크 인|바이트|합계|Total network throughput for received traffic.|차원 없음|
|네트워크 아웃|네트워크 아웃|바이트|합계|Total network throughput for transmitted traffic.|차원 없음|
|MemoryUsed|Memory Used|바이트|평균|The amount of machine memory that is in use by the VM.|차원 없음|
|MemoryGranted|Memory Granted|바이트|평균|The amount of memory that was granted to the VM by the host. Memory is not granted to the host until it is touched one time and granted memory may be swapped out or ballooned away if the VMkernel needs the memory.|차원 없음|
|MemoryActive|Memory Active|바이트|평균|The amount of memory used by the VM in the past small window of time. This is the "true" number of how much memory the VM currently has need of. Additional, unused memory may be swapped out or ballooned with no impact to the guest's performance.|차원 없음|
|백분율 CPU|백분율 CPU|백분율|평균|The CPU utilization. This value is reported with 100% representing all processor cores on the system. As an example, a 2-way VM using 50% of a four-core system is completely using two cores.|차원 없음|
|PercentageCpuReady|Percentage CPU Ready|밀리초|합계|Ready time is the time spend waiting for CPU(s) to become available in the past update interval.|차원 없음|
...
|MemoryWorkingSet|메모리 작업 집합|바이트|평균|메모리 작업 집합|인스턴스|
|AverageMemoryWorkingSet|평균 메모리 작업 집합|바이트|평균|평균 메모리 작업 집합|인스턴스|
|FunctionExecutionUnits|함수 실행 단위|MB / Milliseconds|합계|[Function Execution Units](https://github.com/Azure/Azure-Functions/wiki/Consumption-Plan-Cost-Billing-FAQ#how-can-i-view-graphs-of-execution-count-and-gb-seconds)|인스턴스|
