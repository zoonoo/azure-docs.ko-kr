...
|이벤트|이벤트|개수|평균|이벤트|소스, 이벤트 로그, 컴퓨터, 이벤트 카테고리, 이벤트 레벨, 이벤트 레벨 이름, 이벤트 ID|
...
|sqlserver_process_core_percent<sup>1</sup> |SQL Server 프로세스 코어 백분율|백분율|최대|운영 체제에서 측정한 SQL Server 프로세스의 CPU 사용률입니다.|None|
|sqlserver_process_memory_percent<sup>1</sup> |SQL Server 프로세스 메모리 백분율|백분율|최대|운영 체제에서 측정한 SQL Server 프로세스의 메모리 사용률입니다.|None|
|tempdb_data_size<sup>2</sup> |Tempdb 데이터 파일 크기 킬로바이트|개수|최대|Tempdb 데이터 파일 크기 킬로바이트.|None|
|tempdb_log_size<sup>2</sup> |Tempdb 로그 파일 크기 킬로바이트|개수|최대|Tempdb 로그 파일 크기 킬로바이트.|None|
|tempdb_log_used_percent<sup>2</sup> |Tempdb 퍼센트 로그 사용|백분율|최대|Tempdb 퍼센트 로그 사용.|None|
...
|sqlserver_process_core_percent<sup>1</sup>|SQL Server 프로세스 코어 백분율|백분율|최대|운영 체제에서 측정한 SQL Server 프로세스의 CPU 사용률입니다. 탄력적 풀에 적용됩니다. |None|
|sqlserver_process_memory_percent<sup>1</sup>|SQL Server 프로세스 메모리 백분율|백분율|최대|운영 체제에서 측정한 SQL Server 프로세스의 메모리 사용률입니다. 탄력적 풀에 적용됩니다. |None|
|tempdb_data_size<sup>2</sup>|Tempdb 데이터 파일 크기 킬로바이트|개수|최대|Tempdb 데이터 파일 크기 킬로바이트.|None|
|tempdb_log_size<sup>2</sup>|Tempdb 로그 파일 크기 킬로바이트|개수|최대|Tempdb 로그 파일 크기 킬로바이트. |None|
|tempdb_log_used_percent<sup>2</sup>|Tempdb 퍼센트 로그 사용|백분율|최대|Tempdb 퍼센트 로그 사용.|None|
...
<sup>2</sup> 이 메트릭은 vCore가 2vCore 이상또는 DTU 기반 구매 모델의 경우 200DTU 이상인 vCore 구매 모델을 사용하는 데이터베이스에 사용할 수 있습니다. 하이퍼스케일 데이터베이스는 현재 이 메트릭을 사용할 수 없습니다.
