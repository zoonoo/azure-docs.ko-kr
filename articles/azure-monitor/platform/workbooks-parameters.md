---
title: Azure Monitor 통합 문서를 사용 하 여 대화형 보고서 만들기 | 매개 변수 만들기 | Microsoft 문서
...
2. _시간 범위_ 드롭다운을 열고 아래쪽의 매개 변수 섹션에서 `Time Range` 옵션을 선택 합니다.
...
3. 이렇게 하면 쿼리 평가 시간이 매개 변수의 시간 범위 값인 `| where timestamp > ago(1d)`으로 확장 됩니다.
4. 쿼리를 실행 하 여 결과를 확인 합니다.
...
2. Markdown에 `The chosen time range is {TimeRange:label}`을 입력 합니다.
3. _편집 완료_ 를 선택 합니다.
4. 텍스트 컨트롤에 텍스트가 표시 됩니다. _선택한 시간 범위는 최근 24 시간입니다_ .

## <a name="parameter-options"></a>매개 변수 옵션
_In Text_ 섹션에는 해당 값 대신 매개 변수의 `label` 사용 되었습니다. 매개 변수는 형식에 따라 다양 한 옵션을 노출 합니다. 예를 들어 시간 범위 선택에는 value, label, query, start, end 및 그레인을 사용할 수 있습니다.

매개 변수 _편집_ 창의 `Previews` 섹션을 사용 하 여 매개 변수에 대 한 확장 옵션을 볼 수 있습니다.
