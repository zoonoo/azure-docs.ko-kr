...
리소스 진단 로그의 스키마는 `resourceId` 속성 `category` 에 정의 된 리소스 유형과 속성에 따라 달라 집니다. 다음 목록에서는 서비스 및 범주별 스키마 (사용 가능한 경우)에 대 한 링크를 사용 하 여 리소스 로그를 지 원하는 모든 Azure 서비스를 보여 줍니다.
