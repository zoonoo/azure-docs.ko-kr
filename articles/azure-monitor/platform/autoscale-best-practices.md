...
조합의 한 부분만 사용하는 경우 자동 크기 조정은 프로파일에 정의된 최대 또는 최소 인스턴스 수에 도달할 때까지 단일 방향(확장 또는 축소)에서만 작업을 수행합니다. 이는 최적의 방법이 아니며 가용성을 보장하기 위해 사용률이 높은 시간에 이상적으로 리소스 규모를 확장할 수 있습니다. 마찬가지로 사용률이 낮은 시간에 리소스 규모를 축소하여 비용 절감을 실현할 수 있습니다.
...
아래 예제와 같은 자동 크기 조정 설정은 아웃 및 조건에서 동일하거나 유사한 임계값으로 *사용하지 않는 것이 좋습니다.*

* 스레드 수 >= 600개일 때 인스턴스 를 1개 씩 늘립니다.
* 스레드 수 <= 600일 때 인스턴스 를 1개 줄입니다.
...
4. 축소하기 전에 자동 크기 조정에서 감축된 경우의 최종 상태를 예측하려고 시도합니다. 예를 들어 575x3(현재 인스턴스 수) = 1,725/2(축소된 경우의 최종 인스턴스 수) = 862.5스레드입니다. 즉, 평균 스레드 수가 동일하게 유지되거나 소량만 떨어지는 경우 자동 크기 조정이 크기 조정된 후에도 즉시 다시 확장해야 합니다. 그러나 규모가 다시 확장된 경우 전체 프로세스가 반복되므로 무한 루프가 발생합니다.
5. 이 "느슨한" 상황을 방지하기 위해 자동 크기 조정에서는 규모를 축소하지 않습니다. 대신 건너뛰고 다음에 서비스의 작업이 실행될 때 조건을 다시 평가합니다. 플래핑 상태는 평균 스레드 수가 575일 때 자동 크기 조정이 작동하지 않기 때문에 많은 사람들을 혼동시킬 수 있습니다.
...
4. 세 번째 저장소 큐 인스턴스는 확장 작업으로 인해 추가됩니다.  150/3 = 50이므로 큐의 총 메시지 수가 150개에 도달할 때까지 다음 규모 확장 동작이 발생하지 않습니다.
...
아래 그림에는 최소 인스턴스 = 2이고 최대 인스턴스 = 10인 기본 프로필의 자동 크기 조정 설정을 보여 줍니다. 이 예제에서는 큐의 메시지 수가 10보다 크고 큐의 메시지 수가 3개 미만일 때 확장하도록 규칙이 구성됩니다. 따라서 지금은 리소스의 크기를 2에서 10 사이로 조정할 수 있습니다.
...
하나의 프로필에 여러 규칙을 설정해야 할 수 있는 경우가 있습니다. 다음 자동 크기 조정 규칙은 여러 규칙이 설정된 경우 자동 크기 조정 엔진에서 사용됩니다.

*확장-* 규칙이 충족되면 자동 크기 조정이 실행됩니다.
...
* CPU가 76%이고 메모리가 50%이면 확장됩니다.
* CPU가 50%이고 메모리가 76%이면 확장됩니다.
...
기본 인스턴스 수는 메트릭을 사용할 수 없는 경우 서비스를 해당 카운트로 자동 크기 조정하기 때문에 중요합니다. 따라서 워크로드에 안전한 기본 인스턴스 수를 선택해야 합니다.

### <a name="configure-autoscale-notifications"></a>자동 크기 조정 알림 구성
자동 크기 조정은 다음 조건 중 하나가 발생할 경우 활동 로그에 게시됩니다.

* 자동 크기 조정은 배율 작업을 발행합니다.
* 자동 크기 조정 서비스는 배율 조정 작업을 성공적으로 완료합니다.
...
- [활동 로그 경고를 만들어 구독의 모든 자동 크기 조정 엔진 작업을 모니터링합니다.](https://github.com/Azure/azure-quickstart-templates/tree/master/monitor-autoscale-alert)
