---
title: Azure 자동 크기 로그 이벤트 스키마
description: 자동 크기 조정 작업을 모니터링하고 문제 해결하기 위한 로그 형식
ms.topic: conceptual
ms.date: 11/14/2019
...
---
# <a name="azure-monitor-autoscale-actions-resource-log-schema"></a>Azure 모니터 자동 크기 조정 작업 리소스 로그 스키마

다음은 예제 데이터가 포함된 자동 크기 조정 리소스 로그에 대한 일반적인 형식입니다. 아래의 모든 예제가 지정된 필드에 대해 유효할 수 있는 여러 값을 포함할 수 있기 때문에 JSON이 제대로 형성된 것은 아닙니다. 

이 유형의 이벤트를 사용하여 자동 크기 조정으로 발생할 수 있는 문제를 해결합니다. 자세한 내용은 [자동 크기 조정 문제 해결을](autoscale-troubleshoot.md)참조하십시오.


## <a name="profile-evaluation"></a>프로필 평가

자동 크기 조정이 자동 크기 조정 프로필을 처음 볼 때 기록됨
...
## <a name="profile-cooldown-evaluation"></a>프로파일 대기시간 평가

냉각 기간으로 인해 스케일을 수행하지 않아야 하는지 자동 크기 조정이 평가될 때 기록됩니다. 
...
자동 크기 조정이 처음 특정 축척 규칙 평가를 시작할 때 기록됩니다. 
...
자동 크기 조정이 배율 작업을 트리거하는 데 사용되는 메트릭을 평가할 때 기록됩니다. 
...
## <a name="instance-count-evaluation"></a>인스턴스 수 평가

자동 크기 조정이 더 시작해야 하는지, 일부를 종료할지 또는 아무 것도 하지 않는지 결정하기 위해 이미 실행 중인 인스턴스 수를 평가할 때 기록됩니다. 
...
## <a name="scale-action-evaluation"></a>스케일 작업 평가

배율 조정 작업이 수행되어야 하는 경우 자동 크기 조정이 평가를 시작할 때 기록됩니다. 
...
자동 크기 조정이 실행 중인 계산 인스턴스 수를 위 또는 아래로 업데이트할 때 기록됩니다.
...
## <a name="scale-action"></a>스케일 액션

자동 크기 조정이 스케일 작업을 시작할 때 기록됩니다. 
```json
{
  "time": "2018-09-10 18:12:00.6132593",
  "resourceId": "/SUBSCRIPTIONS/BA13C41D-C957-4774-8A37-092D62ACFC85/RESOURCEGROUPS/AUTOSCALETRACKING12042017/PROVIDERS/MICROSOFT.INSIGHTS/AUTOSCALESETTINGS/DEFAULTSETTING",
...
## <a name="scale-action-tracking"></a>스케일 작업 추적

인스턴스 배율 작업의 서로 다른 간격으로 기록됩니다.
...
자동 [크기 조정에](autoscale-overview.md) 대해 알아보기
