...
1. **확장 가능한 경고** - 동적 임계값 경고 규칙은 한 번에 수백 개의 메트릭 계열에 대한 맞춤형 임계값을 만들 수 있지만 단일 메트릭에서 경고 규칙을 쉽게 정의할 수 있습니다. 생성 및 관리에 대한 경고가 줄어듭니다. Azure 포털 또는 Azure 리소스 관리자 API를 사용하여 만들 수 있습니다. 확장 가능한 접근 방식은 메트릭 차원을 처리하거나 모든 구독 리소스와 같은 여러 리소스에 적용할 때 특히 유용합니다.  [템플릿을 사용하여 동적 임계값으로 메트릭 경고를 구성하는 방법에 자세히 알아봅니다](alerts-metric-create-templates.md).

1. **스마트 메트릭 패턴 인식** - ML 기술을 사용하여 메트릭 패턴을 자동으로 감지하고 시간에 따라 메트릭 변경에 적응할 수 있으며, 여기에는 계절성(시간별/일별/주간)이 포함될 수 있습니다. 시간에 따른 메트릭의 동작에 적응하고 패턴의 편차를 기반으로 경고를 지정하는 것은 각 메트릭에 대한 "올바른" 임계값을 아는 부담을 덜어줍니다. 동적 임계값에 사용되는 ML 알고리즘은 예상되는 패턴을 따르지 않는 노이즈(낮은 정밀도) 또는 넓은(낮은 재현율) 임계값을 방지하도록 디자인되었습니다.

1. **직관적인 구성** - 동적 임계값을 사용하면 상위 수준의 개념을 사용하여 메트릭 경고를 설정할 수 있으므로 메트릭에 대한 광범위한 도메인 지식이 필요하지 않습니다.
...
> [!NOTE]
> 계절별 패턴 감지는 시간, 일 또는 주 간격으로 설정됩니다. 즉, 이중 시간 패턴 또는 반주간 과 같은 다른 패턴이 검색되지 않을 수 있습니다.
...
- 높음 – 임계값이 좀 더 조밀하고 메트릭 시리즈 패턴에 근접합니다. 가장 작은 편차에 경고 규칙이 트리거되어 더 많은 경고가 생성됩니다.
- 중간 - 높음 민감도(기본값)보다 임계값은 덜 조밀하고 좀 더 균일하며 경고 횟수는 더 적습니다.
- 낮음 - 임계값은 메트릭 시리즈 패턴에서 느슨하게 더 멀리 떨어져 있습니다. 경고 규칙은 큰 편차에 대해서만 트리거되어 경고가 줄어듭니다.
...
## <a name="how-do-you-find-out-why-a-dynamic-thresholds-alert-was-triggered"></a>동적 임계값 경고가 트리거된 이유를 어떻게 알 수 있습니까?

전자 메일 또는 문자 메시지의 링크를 클릭하거나 브라우저에서 경고 보기에서 트리거된 경고 인스턴스를 탐색하여 Azure Portal에서 경고 보기를 볼 수 있습니다. [경고 보기에 대해 자세히 알아보세요.](alerts-overview.md#alerts-experience)

경고 보기에는 다음이 표시됩니다.

- 동적 임계값 경고가 발생되는 순간의 모든 메트릭 세부 정보입니다.
- 해당 시점에 사용된 동적 임계값을 포함하는 경고가 트리거된 기간의 차트입니다.
- 동적 임계값 경고 및 경고 보기 경험에 대한 피드백을 제공할 수 있는 기능으로 향후 검색을 개선할 수 있습니다.

## <a name="will-slow-behavior-changes-in-the-metric-trigger-an-alert"></a>메트릭의 느린 동작 변경으로 경고가 트리거됩니다.
...
경고 규칙이 처음 생성되면 차트에 나타나는 임계값은 시간 또는 일일 계절 패턴(10일)을 계산하기에 충분한 기록 데이터를 기반으로 계산됩니다. 경고 규칙이 만들어지면 동적 임계값은 사용 가능한 모든 필요한 기록 데이터를 사용하며 새 데이터를 기반으로 지속적으로 학습하고 조정하여 임계값을 보다 정확하게 만듭니다. 즉, 이 계산 후에 차트에도 주간 패턴이 표시됩니다.
...
1. [Azure 포털에서](https://portal.azure.com) **모니터를**클릭합니다. [모니터] 보기는 모든 모니터링 설정과 데이터를 하나의 보기로 통합합니다.
...
다음은 메트릭, 동적 임계값 제한 및 값이 허용된 임계값을 벗어났을 때 발생한 일부 경고를 보여 주었던 차트입니다.

![메트릭 경고를 구성하는 방법에 대해 자세히 알아보기](media/alerts-dynamic-thresholds/threshold-picture-8bit.png)

다음 정보를 사용하여 이전 차트를 해석합니다.

- **파란색 선** - 시간에 따라 실제 측정된 메트릭입니다.
- **파란색 그늘진 영역** - 메트릭에 허용되는 범위를 표시합니다. 메트릭 값이 이 범위 내에 있는 한 경고가 발생하지 않습니다.
- **파란색 점** - 차트의 일부를 클릭한 다음 파란색 선 위로 마우스를 가져가면 커서 아래에 개별 집계된 메트릭 값이 표시되는 파란색 점이 표시됩니다.
- **파란색 점으로 팝업** - 측정된 메트릭 값(파란색 점)과 허용된 범위의 상하 값을 표시합니다.  
- **검은색 원이 있는 빨간색 점** - 허용된 범위를 벗어난 첫 번째 메트릭 값을 표시합니다. 메트릭 경고를 발생시키고 활성 상태로 두는 값입니다.
- **빨간색 점**- 허용된 범위를 벗어난 추가 측정 값을 나타냅니다. 추가 메트릭 경고는 발생하지 않지만 경고는 활성 상태로 유지됩니다.
- **빨간색 영역** - 메트릭 값이 허용된 범위를 벗어난 시간을 표시합니다. 후속 측정 값이 허용 범위를 벗어난 경우 경고는 활성 상태로 유지되지만 새 경고는 발생되지 않습니다.
- **빨간색 영역의 끝** - 파란색 선이 허용된 값 안쪽으로 돌아오면 빨간색 영역이 중지되고 측정된 값 선이 파란색으로 바뀝니다. 빨간색 윤곽선이 있는 빨간색 점 시 에 발생된 메트릭 경고의 상태가 해결됨으로 설정됩니다. 
