...
description: Azure 모니터의 Linux용 로그 분석 에이전트에서 가장 일반적인 문제에 대한 증상, 원인 및 해결 에 대해 설명합니다.
ms.subservice: logs
ms.topic: conceptual
author: bwren
ms.author: bwren
...
이 문서에서는 Azure Monitor의 Linux용 로그 분석 에이전트에서 발생할 수 있는 오류를 해결하는 데 도움이 되며 이를 해결할 수 있는 해결 방법을 제안합니다.
...
| 55 | 지원되지 않는 시작 버전 또는 Azure 모니터에 연결할 수 없거나 dpkg이 잠겨 있거나 컬 프로그램이 없습니다. |
...
디버그 로깅을 사용하면 유형, 데이터 항목 수 및 전송하는 데 걸린 시간으로 구분된 Azure Monitor에 대한 일괄 업로드를 볼 수 있습니다.
...
## <a name="issue--unable-to-connect-through-proxy-to-azure-monitor"></a>문제: 프록시를 통해 Azure 모니터에 연결할 수 없음

### <a name="probable-causes"></a>가능한 원인
* 등록하는 동안 지정된 프록시가 올바르지 않습니다.
* Azure 모니터 및 Azure 자동화 서비스 끝점은 데이터 센터에 화이트백되지 않습니다. 

### <a name="resolution"></a>해결 방법
1. 옵션을 사용하도록 설정한 다음 `-v` 명령을 사용하여 Linux용 로그 분석 에이전트를 사용하여 Azure 모니터로 다시 보드합니다. 프록시를 통해 Azure Monitor에 연결하는 에이전트의 자세한 출력을 허용합니다. 
...
## <a name="issue-you-see-omiagent-using-100-cpu"></a>문제: 100% CPU를 사용하는 오미에이전트가 보입니다.

### <a name="probable-causes"></a>가능한 원인
nss-pem 패키지 [v1.0.3-5.el7의](https://centos.pkgs.org/7/centos-x86_64/nss-pem-1.0.3-7.el7.x86_64.rpm.html) 회귀는 심각한 성능 문제를 일으켰으며 Redhat /Centos 7.x 배포판에서 많이 나타났습니다. 이 문제에 대한 자세한 내용은 다음 설명서를 확인하십시오: [libcurl의 버그 1667121 성능 회귀.](https://bugzilla.redhat.com/show_bug.cgi?id=1667121)

성능 관련 버그는 항상 발생하지 않으며 재현하기가 매우 어렵습니다. 오미에이전트에 이러한 문제가 발생하면 특정 임계값을 초과할 때 오미에이전트의 스택 추적을 수집하는 스크립트 omiHighCPUDiagnostics.sh 사용해야 합니다.

1. 스크립트 다운로드 <br/>
`wget https://raw.githubusercontent.com/microsoft/OMS-Agent-for-Linux/master/tools/LogCollector/source/omiHighCPUDiagnostics.sh`

2. 30% CPU 임계값으로 24시간 동안 진단 실행 <br/>
`bash omiHighCPUDiagnostics.sh --runtime-in-min 1440 --cpu-threshold 30`

3. 호출 스택은 omiagent_trace 파일에 덤프됩니다, 당신은 많은 Curl 및 NSS 함수 호출을 통지하는 경우, 아래의 해결 단계를 따르십시오.

### <a name="resolution-step-by-step"></a>해상도(단계별)

1. nss-pem 패키지를 [v1.0.3-5.el7_6.1로](https://centos.pkgs.org/7/centos-x86_64/nss-pem-1.0.3-7.el7.x86_64.rpm.html)업그레이드합니다. <br/>
`sudo yum upgrade nss-pem`

2. nss-pem을 업그레이드할 수 없는 경우(대부분 Centos에서 발생), 컬을 7.29.0-46으로 다운그레이드합니다. 실수로 "yum 업데이트"를 실행하면 컬이 7.29.0-51로 업그레이드되고 문제가 다시 발생합니다. <br/>
...
- Azure 모니터에 온보딩실패
- Azure 모니터에 대한 연결이 차단되었습니다.
- Linux용 Log Analytics 에이전트가 백업되었습니다.

### <a name="resolution"></a>해결 방법
1. 다음 파일이 있는지 확인하여 Azure Monitor온보딩에 성공했는지 확인합니다.`/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`
...
* DSC 로그 *현재 구성이 없습니다. -Path 매개 변수를 사용하여 Start-DscConfiguration 명령을 실행하여 구성 파일을 지정하고 먼저 현재 구성을 만듭니다.* 오류를 `omsconfig.log` 로그 파일에 기록하지만, `PerformRequiredConfigurationChecks` 작업에 대한 로그 메시지가 없습니다.

### <a name="resolution"></a>해결 방법
1. auditd 패키지 같은 모든 종속성을 설치합니다.
2. 다음 파일이 있는지 확인하여 Azure Monitor에 대한 `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`온보딩이 성공했는지 확인합니다.  성공하지 못했으면 omsadmin.sh 명령줄 [지침](https://github.com/Microsoft/OMS-Agent-for-Linux/blob/master/docs/OMS-Agent-for-Linux.md#onboarding-using-the-command-line)을 사용하여 다시 등록합니다.
...
8. 경우에 따라 Linux용 로그 분석 에이전트가 Azure Monitor와 통신할 수 없는 경우 에이전트의 데이터가 전체 버퍼 크기인 50MB로 백업됩니다. `/opt/microsoft/omsagent/bin/service_control restart` 명령을 실행하여 에이전트를 다시 시작해야 합니다.
...
* Linux용 Log Analytics 에이전트 구성 에이전트가 포털 구성 서비스와 통신할 수 없어 최신 구성이 적용되지 않는 경우가 있습니다.
  ...
  2. 에이전트가 `omsconfig` 다음 명령을 `sudo su omsagent -c 'python /opt/microsoft/omsconfig/Scripts/GetDscConfiguration.py'`실행하여 Azure Monitor와 통신할 수 있는지 확인합니다. 이 명령은 Syslog 설정, Linux 성능 카운터, 사용자 지정 로그 등 에이전트가 서비스에서 수신하는 구성을 반환합니다. 이 명령이 실패하는 경우 `sudo su omsagent -c 'python /opt/microsoft/omsconfig/Scripts/PerformRequiredConfigurationChecks.py'` 명령을 실행합니다. 이 명령은 omsconfig 에이전트가 Azure Monitor와 대화하고 최신 구성을 검색하도록 강제합니다.

## <a name="issue-you-are-not-seeing-any-custom-log-data"></a>문제: 사용자 지정 로그 데이터가 보이지 않음 

### <a name="probable-causes"></a>가능한 원인
* Azure 모니터에 온보딩하지 못했습니다.
...
### <a name="resolution"></a>해결 방법
1. 다음 파일이 있는지 확인하여 Azure Monitor에 대한 `/etc/opt/microsoft/omsagent/<workspace id>/conf/omsadmin.conf`온보딩이 성공했는지 확인합니다. 실패한 경우 둘 중 하나를 수행합니다.  
