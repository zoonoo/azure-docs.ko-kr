...
* [VM 스케일 세트에 대한 리소스 관리자 템플릿을 사용하는 고급 자동 크기 조정](../../azure-monitor/platform/autoscale-virtual-machine-scale-sets.md)  
...
반대로 수직적 규모 조정은 다릅니다. 수직적 규모 조정은 동일한 수의 VM을 유지하지만 VM을 강력하게("강화") 만들거나 약하게("축소") 만듭니다. 전력은 메모리, CPU 속도, 디스크 공간 등으로 측정됩니다.  수직 크기 조정에는 더 많은 제한이 있습니다. 큰 하드웨어의 가용성에 따라 다르고 신속하게 상한값에 도달하며 지역에 따라 달라질 수 있습니다. 또한 수직적 규모 조정은 일반적으로 VM 중지 및 재시작이 필요합니다.
...
| Virtual Machines: Windows 예제 |[VM 스케일 세트에 대한 리소스 관리자 템플릿을 사용하는 고급 자동 크기 조정](../../azure-monitor/platform/autoscale-virtual-machine-scale-sets.md) |
