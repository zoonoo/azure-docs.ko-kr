...
1. Azure Portal에서 다운로드합니다.  Azure Portal에 로그인한 후:  
   ...
   1. **직접 에이전트** 블레이드에서 **Log Analytics 게이트웨이 다운로드**를 클릭합니다.<br><br> ![Log Analytics 게이트웨이 다운로드](./media/gateway/download-gateway.png)
...
1. **Welcome** 페이지에서 **다음**을 클릭합니다.<br><br> ![게이트웨이 설치 마법사](./media/gateway/gateway-wizard01.png)<br> 
...
1. **포트 및 프록시 주소** 페이지에서 다음 단계를 수행합니다.
   ...
   1. 필요에 따라 게이트웨이가 설치되어 있는 서버에서 프록시를 통해 통신해야 하는 경우 게이트웨이에서 연결해야 하는 프록시 주소를 입력합니다. 예: `http://myorgname.corp.contoso.com:80`  비워 두면 게이트웨이에서 인터넷에 직접 연결을 시도합니다.  프록시 서버에 인증이 필요한 경우 사용자 이름과 암호를 입력합니다.<br><br> ![게이트웨이 마법사 프록시 구성](./media/gateway/gateway-wizard02.png)<br>   
   1. **다음**을 클릭합니다.
...
1. Operations Manager 콘솔을 열고, **Operations Management Suite** 아래에서 **연결**을 클릭한 다음, **프록시 서버 구성**을 클릭합니다.<br><br> ![Operations Manager – 프록시 서버 구성](./media/gateway/scom01.png)<br> 
1. **프록시 서버를 사용하여 Operations Management Suite에 액세스**를 선택한 다음, Log Analytics 게이트웨이 서버의 IP 주소 또는 NLB의 가상 IP 주소를 입력합니다. `http://` 접두사로 시작해야 합니다.<br><br> ![Operations Manager – 프록시 서버 주소](./media/gateway/scom02.png)<br> 
...
1. **위치** 아래에서 해당 지역을 확인합니다.<br><br> ![Azure Portal – Automation 계정 위치](./media/gateway/location.png)  
...
게이트웨이에서 기록한 이벤트를 수집하려면 Log Analytics 에이전트도 설치해야 합니다.<br><br> ![이벤트 뷰어 – Log Analytics 게이트웨이 로그](./media/gateway/event-viewer.png)
...
![Log Analytics 게이트웨이 성능 카운터](./media/gateway/counters.png)
...
![새 지원 요청](./media/gateway/support.png)

## <a name="next-steps"></a>다음 단계
[데이터 원본을 추가](../../azure-monitor/platform/agent-data-sources.md)하여 연결된 원본에서 데이터를 수집하고 Log Analytics 작업 영역에 저장합니다.
