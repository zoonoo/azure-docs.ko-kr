---
title: Azure 모니터를 사용하여 클래식 메트릭 경고 생성 및 관리
...
Azure Monitor의 클래식 메트릭 경고를 사용하면 메트릭 중 하나가 임계값을 초과할 경우 알림을 받을 수 있습니다. 클래식 메트릭 경고는 무차원 메트릭에 대해서만 경고하는 이전 기능입니다. 클래식 메트릭 경고보다 향상된 기능을 가진, 메트릭 경고라는 최신 기능도 있습니다. [메트릭 경고 개요](../../azure-monitor/platform/alerts-metric-overview.md)에서 새로운 메트릭 경고 기능에 대해 자세히 알아볼 수 있습니다. 이 문서에서는 Azure Portal, Azure CLI 및 Powershell을 통해 클래식 메트릭 경고 규칙을 만들고, 보고, 관리하는 방법을 설명합니다.
...
7. 경고가 발생하면 추가 이메일 주소에 알림을 보내려면 **추가 관리자 이메일** 필드에 해당 이메일 주소를 추가합니다. 다음 형식으로 세미콜론으로 여러 이메일을 분리하십시오: *\@이메일\@contoso.com;email2 contoso.com*
...
1. 아직 PowerShell이 컴퓨터에서 실행되도록 설정하지 않았으면 지금 설정합니다. 자세한 내용은 [PowerShell을 설치하고 구성하는 방법을](/powershell/azure/overview)참조하십시오. [Azure Monitor(Insights) Cmdlet](https://docs.microsoft.com/powershell/module/az.applicationinsights)에서 Azure Monitor PowerShell cmdlet의 전체 목록을 살펴볼 수도 있습니다.
...
8. 클래식 경고 규칙은 PowerShell을 통해 더 이상 만들 수 없습니다. 경고 규칙을 만들려면 새로운 ['Add-AzMetricAlertRule'](/powershell/module/az.monitor/add-azmetricalertrule) 명령을 사용해야 합니다.

## <a name="next-steps"></a>다음 단계

- [Resource Manager 템플릿을 사용하여 클래식 메트릭 경고 만들기](../../azure-monitor/platform/alerts-enable-template.md)
- [웹후크를 사용하여 비 Azure 시스템에 알리도록 클래식 메트릭 경고 설정](../../azure-monitor/platform/alerts-webhooks.md)
