...
ms.locfileid: "53191804"
---
# <a name="import-azure-log-analytics-data-into-power-bi"></a>Azure Log Analytics 데이터를 Power BI로 가져오기


[Power BI](https://powerbi.microsoft.com/documentation/powerbi-service-get-started/)는 서로 다른 데이터 집합의 분석을 위해 다양한 시각화 및 보고서를 제공하는 Microsoft의 클라우드 기반 비즈니스 분석 서비스입니다.  Log Analytics 로그 검색 결과를 Power BI 데이터 집합으로 가져오면 여러 소스의 데이터를 결합하고 웹 및 모바일 디바이스에서 보고서를 공유하는 등의 기능을 활용할 수 있습니다.
...
Log Analytics에서 Power BI 데이터 세트를 채우고 싶은 데이터를 반환하는 [로그 검색](../../azure-monitor/log-query/log-query-overview.md)부터 만듭니다.  그런 다음 해당 쿼리를 [파워 쿼리(M) 언어](https://msdn.microsoft.com/library/mt807488.aspx)로 내보내면 Power BI Desktop에서 사용할 수 있습니다.
...
2. 로그 검색 포털을 사용하는 경우 **Power BI**를 클릭합니다.  Analytics 포털을 사용하는 경우 **내보내기** > **Power BI 쿼리(M)** 를 선택합니다.  두 옵션 모두 **PowerBIQuery.txt**라고 하는 텍스트 파일로 쿼리를 내보냅니다. 
    ...
    ![로그 검색 내보내기](media/powerbi/export-logsearch.png) ![로그 검색 내보내기](media/powerbi/export-analytics.png)
...
* Power BI로 내보낼 수 있는 쿼리를 작성하려면 [로그 검색](../../azure-monitor/log-query/log-query-overview.md) 에 대해 알아봅니다.
