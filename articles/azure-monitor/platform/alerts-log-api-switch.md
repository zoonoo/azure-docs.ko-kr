...
description: 레거시 savedSearch 개요는 일반적인 고객 문제를 해결 하는 세부 정보를 사용 하 여 새 ScheduledQueryRules API에 경고 규칙을 전환 하려면 Log Analytics 경고 API 및 프로세스에 기반 합니다.
author: msvijayn
...
이제 Log Analytics 경고 사용자에게 진정한 Azure 프로그래밍 대체 인터페이스인 [Azure Monitor - ScheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)를 발표합니다. 이 API는 [Azure 청구 - 로그 경고용](alerts-unified-log.md#pricing-and-billing-of-log-alerts)에도 반영됩니다. API를 사용 하 여 로그 경고를 관리 하는 방법에 자세한 내용은 참조 하세요 [Azure 리소스 템플릿을 사용 하 여 관리 로그 경고](alerts-log.md#managing-log-alerts-using-azure-resource-template) 하 고 [PowerShell을 사용 하 여 관리 로그 경고](alerts-log.md#managing-log-alerts-using-powershell)합니다.
...
- 사용 하 여 로그 경고를 관리 [Powershell cmdlet](alerts-log.md#managing-log-alerts-using-powershell)
...
사용자는 [레거시 Log Analytics 경고 API](api-alerts.md) 또는 새 [scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)를 사용할 수 있습니다. API에서 생성된 경고 규칙은 ‘동일한 API에서만 관리할 수 있고’ Azure Portal에서도 관리할 수 있습니다.  기본적으로 Azure Monitor는 계속 사용 하 여 [레거시 Log Analytics 경고 API](api-alerts.md) Log Analytics의 기존 작업 영역에 대 한 Azure portal에서 모든 새 경고 규칙 만들기에 대 한 합니다. 로 [2019 년 6 월 1 일 이후에 생성 되는 새 로그 작업 영역을 발표](https://azure.microsoft.com/updates/switch-api-preference-log-alerts/) -자동으로 사용 됩니다 [scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules) Azure portal에서 기본값을 포함 하 여 합니다.

scheduledQueryRules API로 기본 설정을 전환할 경우 미치는 영향은 다음과 같이 컴파일됩니다.

- 프로그래밍 인터페이스를 통해 로그 경고를 관리하기 위해 수행된 모든 조작은 이제 [scheduledQueryRules](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)를 사용하여 수행해야 합니다. 자세한 내용은 [예제를 사용 하 여 Azure 리소스 템플릿을 통해](alerts-log.md#managing-log-alerts-using-azure-resource-template) 및 [PowerShell 통해 샘플 사용](alerts-log.md#managing-log-alerts-using-powershell)
...
[레거시 Log Analytics 경고 API](api-alerts.md)에서 경고 규칙을 이동하는 프로세스는 어떠한 방법으로도 경고 정의, 쿼리 또는 구성의 변경을 포함하지 않습니다. 경고 규칙 및 영향을 받지 및 경고 모니터링 중지 되지 않습니다 또는 있거나 전환 된 후 중단 됩니다. 유일한 변경 내용에는 API 기본 설정에 대 한 새 API 통해 규칙 변경이 되었습니다.

> [!NOTE]
> 사용자가 새 기본 설정 전환 되 면 [scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)를 다시 선택 하거나 이전 사용으로 되돌릴 수 없습니다 [레거시 Log Analytics 경고 API](api-alerts.md)합니다.
