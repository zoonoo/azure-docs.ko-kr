...
## <a name="what-you-can-do-with-resource-logs-sent-to-an-event-hub"></a>이벤트 허브로 전송 되는 리소스 로그로 수행할 수 있는 작업
다음 기능을 제공 하기 위해 Azure의 리소스 로그를 event hubs로 스트림 합니다.
...
리소스 로그는 기본적으로 수집 되지 않습니다. Azure 리소스에 대 한 진단 설정을 만들어 이벤트 허브 및 기타 대상으로 보냅니다. 자세한 내용은 [Azure에서 로그 및 메트릭을 수집 하는 진단 설정 만들기를](diagnostic-settings.md) 참조 하세요.

## <a name="stream-data-from-compute-resources"></a>계산 리소스에서 데이터 스트리밍
이 문서의 프로세스는 [Azure 리소스 로그 개요](diagnostic-settings.md)에 설명 된 대로 비 계산 리소스에 대 한 것입니다.
Windows Azure 진단 에이전트를 사용 하 여 Azure compute 리소스에서 리소스 로그를 스트리밍합니다. 자세한 내용은 [Event Hubs를 사용 하 여 실행 부하 과다 경로에서 스트리밍 Azure 진단 데이터](diagnostics-extension-stream-event-hubs.md) 를 참조 하세요.


## <a name="consuming-log-data-from-event-hubs"></a>Event hubs에서 로그 데이터 사용
Event hubs에서 리소스 로그를 사용 하는 경우 다음 표의 요소를 포함 하는 JSON 형식입니다.
...
* [자세한 내용은 Azure 리소스 로그를 참조](resource-logs-overview.md)하세요.
