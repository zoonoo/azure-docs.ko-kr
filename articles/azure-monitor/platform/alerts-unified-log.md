...
로그 검색 경고 규칙은 구성 및 사용 된 사용자 지정 분석 쿼리에 따라 사용자가 작성한 논리에서 작동합니다. 경고 규칙을 트리거하는 정확한 조건 또는 이유를 포함 하는 모니터링 논리는 각 로그 경고 규칙 마다 다를 수 있는 분석 쿼리에서 캡슐화 됩니다. Azure 경고에는 로그 검색 경고 규칙의 임계값 조건이 충족 되거나 초과 될 때 평가 되는 특정 근본 원인 (또는) 시나리오에 대 한 정보가 부족 합니다. 따라서 로그 경고를 상태 없음 이라고 합니다. 및 로그 경고 규칙은 제공 된 사용자 지정 분석 쿼리 결과에서 경고 조건을 충족 하는 동안 계속 해 서 발생 합니다. 모니터링 실패의 정확한 근본 원인에 대 한 논리가 사용자가 제공 하는 분석 쿼리 내에서 마스킹 됨에 따라 경고가 해결 되지 않습니다. 현재 결론 내릴 추론 근본 원인을 해결 하는 것을 하는 Azure Monitor 메커니즘이 없습니다.

실제 예제를 통해 동일한 것을 볼 수 있습니다. [결과 유형 로그 경고 수에 대해 제공 된 예제의](#example-of-number-of-records-type-log-alert) 구성에 따라 *Contoso-log-alert*라는 로그 경고 규칙이 있다고 가정 합니다. 여기서 사용자 지정 경고 쿼리는 로그에서 500 결과 코드를 찾도록 설계 되었습니다.

- 1:05 PM에 Azure 경고에 의해 Contoso-로그 경고가 실행 될 때 로그 검색 결과에는 0 개의 레코드가 생성 되 고 결과 코드는 500이 있습니다. 0은 임계값 보다 낮으므로 경고가 발생 하지 않습니다.
- Azure alerts에서 Contoso-Log-Alert를 실행 했을 때 1:10 PM의 다음 반복에서 로그 검색 결과는 결과 코드가 500 인 5 개의 레코드를 제공 합니다. 5가 임계값을 초과 하 고 연결 된 작업이 트리거되면 경고가 발생 합니다.
- 1:15 PM에 Azure 경고에 의해 Contoso-로그 경고가 실행 되 면 로그 검색 결과에 500 결과 코드를 포함 하는 두 개의 레코드가 제공 됩니다. 두 개는 임계값을 초과 하 고 연결 된 작업이 트리거되면 경고가 발생 합니다.
- 이제 Azure alert에서 Contoso-Log-Alert를 실행 했을 때 1:20 PM의 다음 반복에서 500 결과 코드를 사용 하 여 로그 검색 결과에 0 레코드를 다시 제공 했습니다. 0은 임계값 보다 낮으므로 경고가 발생 하지 않습니다.

하지만 위의 나열 된 경우 오후 1:15에는 Azure alerts에서 1:10에 표시 되는 기본 문제를 확인할 수 없으며, net 새 오류가 발생 한 경우 사용자가 제공 하는 쿼리는 이전 레코드를 고려 하 고 있을 수 있습니다. Azure 경고는이를 확인할 수 있습니다. 경고에 대 한 논리는 경고 쿼리에 캡슐화 되므로 500 결과 코드를 포함 하는 두 레코드가 1:15 PM에 표시 되거나 1:10 PM에 표시 되지 않을 수 있습니다. 따라서 문제를 해결 하기 위해 Contoso-Log-경고를 1:15 PM에 실행 하면 구성 된 작업이 다시 트리거됩니다. 이제 500 결과 코드를 사용 하 여 0 개 레코드가 표시 되는 경우 오후 1:20 시에는 Azure alerts에서 500 결과 코드의 원인이 1:10 PM 및 1:15 PM으로 표시 되는 것을 알 수 없으며, 경고를 Azure Monitor 하 여 동일한 이유로 인해 오류가 발생 하지 않습니다. s. 따라서 Contoso-로그 경고는 Azure 경고 대시보드에서 해결 됨으로 변경 되지 않으며 경고의 해결을 알리는 알림 메시지를 전송 합니다. 대신 분석 쿼리에 포함 된 논리의 정확한 조건이 나 이유를 이해 하는 사용자는 필요에 따라 [경고를 닫힘으로 표시할](alerts-managing-alert-states.md) 수 있습니다.
