---
title: Azure 모니터를 사용하여 로그 경고 생성, 보기 및 관리 | 마이크로 소프트 문서
description: Azure 모니터를 사용하여 Azure에서 로그 경고 규칙을 작성, 보고 및 관리합니다.
author: yanivlavi
ms.author: yalavi
ms.topic: conceptual
ms.date: 07/29/2019
...
**로그 경고라는** 용어는 [로그 분석 작업 영역](../learn/tutorial-viewdata.md) 또는 응용 프로그램 [인사이트에서](../app/analytics.md)로그 쿼리인 경고를 설명합니다. [로그 경고 - 개요](alerts-unified-log.md)에서 기능, 용어 및 형식에 대해 자세히 알아보세요.

> [!NOTE]
> 이제 Azure 모니터의 메트릭 플랫폼에서 [로그 분석 작업 영역의](../../azure-monitor/learn/tutorial-viewdata.md) 인기 로그 데이터를 사용할 수 있습니다. 세부 정보 보기는 [로그에 대한 메트릭 경고](alerts-metric-logs.md)를 참조하세요.
...
1. [포털에서](https://portal.azure.com/) **모니터** 및 모니터 섹션 아래에서 **경고를 선택합니다.**
...
1. *로그 경고*: **리소스 종류**가 *Log Analytics* 또는 *Application Insights*와 같은 분석 원본이고 신호 유형이 **로그**인지 확인한 다음, 적절한 **리소스가** 선택되면 *완료*를 클릭합니다. 다음으로 **조건 추가** 단추를 사용하여 리소스 및 *Log Analytics* 또는 *Application Insights*와 같은 선택된 로그 모니터 서비스에 대한 신호 목록 **사용자 지정 로그 검색** 옵션에서 사용할 수 있는 신호 옵션 목록을 봅니다.
   ...
   > [!NOTE]
   > 
   > 경고 목록은 위 그림과 같이 분석 쿼리를 신호 유형 - **로그(저장된 쿼리)** 로 가져올 수 있습니다. 따라서 사용자는 애널리틱스에서 쿼리를 완벽하게 완성한 다음 나중에 경고에 사용할 수 [using log query in Azure Monitor](../log-query/log-query-overview.md) 있도록 저장할 수 [shared query in application insights analytics](../app/app-insights-overview.md)있습니다.
...
1. 세 번째이자 마지막 단계로 경고 조건이 충족될 때 경고 규칙에 대해 **작업 그룹**을 트리거해야 하는지를 지정합니다. 경고와 함께 모든 기존 작업 그룹을 선택하거나 새 작업 그룹을 만들 수 있습니다. 선택한 작업 그룹에 따라 경고가 트리거될 때 Azure는 이메일을 보내고, SMS를 보내고, 웹후크를 호출하고, Azure Runbook을 사용하여 수정하고, ITSM 도구에 푸시하는 등의 작업을 수행합니다. [작업 그룹에](action-groups.md)대해 자세히 알아봅니다.
...
또한 사용자는 [로그 분석에서](../log-query/portals.md) 분석 쿼리를 완료한 다음 푸시하여 '경고 설정' 버튼을 통해 경고를 생성한 다음 위의 자습서에서 6단계의 지침을 따르십시오.
...
1. **경고 대시보드가** 표시되며 모든 Azure 경고(로그 경고 포함)가 단일 보드에 표시됩니다. 로그 경고 규칙이 발생했을 때의 모든 인스턴스를 포함합니다. 자세한 내용은 [경고 관리](https://aka.ms/managealertinstances)를 참조하세요.
    > [!NOTE]
...
Azure 모니터 - [예약된 쿼리 규칙 API는](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/) REST API이며 Azure 리소스 관리자 REST API와 완벽하게 호환됩니다. 아래에 나열된 PowerShell cmdlet을 사용하여 [예약된 쿼리 규칙 API를](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/)활용할 수 있습니다.

1. [New-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrule) : 새 로그 경고 규칙을 만들 수 있는 powershell cmdlet입니다.
1. [설정-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/set-azscheduledqueryrule) : 기존 로그 경고 규칙을 업데이트하는 파워쉘 cmdlet입니다.
1. [New-AzScheduledQueryRuleSource](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrulesource) : 로그 경고에 대한 소스 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet입니다. [New-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrule) 및 [Set-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/set-azscheduledqueryrule) cmdlet에서 입력으로 사용됩니다.
1. [New-AzScheduledQueryRuleSchedule](https://docs.microsoft.com/powershell/module/az.monitor/New-AzScheduledQueryRuleSchedule): 로그 경고에 대한 일정 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet입니다. [New-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrule) 및 [Set-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/set-azscheduledqueryrule) cmdlet에서 입력으로 사용됩니다.
1. [New-AzScheduledQueryRuleAlertingAction](https://docs.microsoft.com/powershell/module/az.monitor/New-AzScheduledQueryRuleAlertingAction) : 로그 경고에 대한 작업 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet입니다. [New-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrule) 및 [Set-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/set-azscheduledqueryrule) cmdlet에서 입력으로 사용됩니다.
1. [New-AzScheduledQueryRuleAznsActionGroup](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryruleaznsactiongroup) : 로그 경고에 대한 작업 그룹 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet입니다. [New-AzScheduledQueryRuleAlerting작업](https://docs.microsoft.com/powershell/module/az.monitor/New-AzScheduledQueryRuleAlertingAction) cmdlet에 의해 입력으로 사용됩니다.
1. [New-AzScheduledQueryQuestionTrigger조건](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryruletriggercondition) : 로그 경고를 위한 트리거 조건 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet입니다. [New-AzScheduledQueryRuleAlerting작업](https://docs.microsoft.com/powershell/module/az.monitor/New-AzScheduledQueryRuleAlertingAction) cmdlet에 의해 입력으로 사용됩니다.
1. [New-AzScheduledQueryRuleLogMetricTrigger](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryrulelogmetrictrigger) : [메트릭 측정 유형 로그 경고에](../../azure-monitor/platform/alerts-unified-log.md#metric-measurement-alert-rules)대한 메트릭 트리거 조건 매개 변수를 지정하는 개체를 만들거나 업데이트하는 Powershell cmdlet . [New-AzScheduledQuery규칙트리거조건](https://docs.microsoft.com/powershell/module/az.monitor/new-azscheduledqueryruletriggercondition) cmdlet에 의해 입력으로 사용됩니다.
1. [Get-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/get-azscheduledqueryrule) : 기존 로그 경고 규칙 또는 특정 로그 경고 규칙을 나열하는 Powershell cmdlet
1. [업데이트-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/update-azscheduledqueryrule) : 로그 경고 규칙을 활성화 하거나 비활성화 하는 파워 쉘 cmdlet
1. [제거-AzScheduledQueryRule](https://docs.microsoft.com/powershell/module/az.monitor/remove-azscheduledqueryrule): 기존 로그 경고 규칙을 삭제하는 파워쉘 cmdlet

> [!NOTE]
> 예약쿼리규칙 PowerShell cmdlet은 cmdlet 자체또는 Azure 모니터 - [예약된 쿼리 규칙 API를](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/)사용하여 만든 규칙만 관리할 수 있습니다. 레거시 [로그 분석 경고 API](api-alerts.md) 및 [로그 애널리틱스의](../insights/solutions-resources-searches-alerts.md) 레거시 템플릿을 사용하여 생성된 로그 경고 규칙 저장된 검색 및 경고는 사용자가 [로그 분석 경고에 대한 API 기본 설정을 전환한](alerts-log-api-switch.md)후에만 ScheduledQueryRules PowerShell cmdlets를 사용하여 관리할 수 있습니다.

다음은 예약된QueryRules PowerShell cmdlet을 사용하여 샘플 로그 경고 규칙을 만드는 단계입니다.
```powershell
...
## <a name="managing-log-alerts-using-cli-or-api"></a>CLI 또는 API를 사용하여 로그 경고 관리

Azure 모니터 - [예약된 쿼리 규칙 API는](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules/) REST API이며 Azure 리소스 관리자 REST API와 완벽하게 호환됩니다. 따라서 Azure CLI에 대한 리소스 관리자 명령을 사용하여 Powershell을 통해 사용할 수 있습니다.


> [!NOTE]
> Log Analytics에 대한 로그 경고는 레거시 [Log Analytics 경고 API](api-alerts.md) 및 [Log Analytics 저장된 검색 및 경고](../insights/solutions-resources-searches-alerts.md)의 레거시 템플릿을 사용하여 관리할 수도 있습니다. 기본적으로 여기에서 설명하는 새 ScheduledQueryRules API 사용에 대한 자세한 내용은 [Log Analytics 경고에 대한 새 API로 전환](alerts-log-api-switch.md)을 참조하세요.

로그 경고에는 현재 전용 CLI 명령이 없습니다. 그러나 아래에 설명된 대로 리소스 템플릿 섹션의 이전(sampleScheduledQueryRule.json)에 대해 Azure 리소스 관리자 CLI 명령을 통해 사용할 수 있습니다.
...
* Azure [경고에서 로그 경고에](../../azure-monitor/platform/alerts-unified-log.md) 대해 알아보기
* [로그 경고에 대한 웹후크 작업](../../azure-monitor/platform/alerts-log-webhook.md) 이해
* [애플리케이션 인사이트에](../../azure-monitor/app/analytics.md) 대해 자세히 알아보기
