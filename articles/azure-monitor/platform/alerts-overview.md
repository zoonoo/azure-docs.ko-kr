...
**조건**: 대상 리소스에 적용 되는 신호 및 논리의 조합입니다. 예를 들면 다음과 같습니다. 
   - 백분율 CPU > 70%
   - 서버 응답 시간 > 4 ms 
   - 로그 쿼리의 결과 수 > 100

**경고 이름**: 사용자가 구성 하는 경고 규칙의 특정 이름입니다.
...
**작업**: 경고가 발생 하는 경우 수행 되는 특정 동작입니다. 자세한 내용은 [작업 그룹](../../azure-monitor/platform/action-groups.md)을 참조하세요.
...
모니터링 서비스 또는 신호 형식에 관계 없이 일관 된 방식으로 경고를 작성할 수 있습니다. 발생한 모든 경고 및 관련 세부 정보는 단일 페이지에 제공됩니다.
...
| 모니터 조건 | 모니터 조건을 선택 하거나 **모두를 선택 하 여** 모든 조건의 경고를 포함 합니다. |
