...
**조건**: 대상 리소스에 적용 되는 신호 및 논리의 조합입니다. 예시: 
   - 백분율 CPU > 70%
   - 서버 응답 시간 > 4 ms 
   - 로그 쿼리의 결과 수 > 100
...
**Action**: 경고가 발생 했을 때 수행 되는 특정 동작입니다. 자세한 내용은 [작업 그룹](../../azure-monitor/platform/action-groups.md)을 참조하세요.
...
경고 인스턴스를 사용 하 고 관리 하려면 사용자에 게 [모니터링 참여자](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor) 또는 [모니터링 판독기](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-reader)의 기본 제공 RBAC 역할이 있어야 합니다. 이러한 역할은 구독 수준에서 리소스 수준의 세부적인 할당까지 모든 Azure Resource Manager 범위에서 지원 됩니다. 예를 들어 사용자가 가상 컴퓨터 `ContosoVM1`에 대 한 참가자 액세스만 모니터링 하는 경우 해당 사용자는 `ContosoVM1`에 생성 된 경고만 사용 하 고 관리할 수 있습니다.
