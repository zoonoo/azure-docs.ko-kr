...
> 응용 프로그램 인사이트를 로그 애널리틱스에 통합하여 Azure Monitor에서 단일 통합 로그 저장소를 만들어 이러한 두 저장소를 함께 가져오고 있습니다. 이 변경 사항은 2020년 2분기에 계획되어 있습니다. 그때까지 애플리케이션 인사이트 데이터에 CMK를 배포할 필요가 없는 경우 통합으로 인해 이러한 배포가 중단되고 Log Analytics 작업 영역으로 마이그레이션한 후 CMK를 다시 구성해야 하므로 통합이 완료될 때까지 기다리는 것이 좋습니다. 일일 최소 1TB는 클러스터 수준에서 적용되며 2분기 동안 통합이 완료될 때까지 애플리케이션 인사이트 및 로그 분석에는 별도의 클러스터가 필요합니다.
...
또한 효율적인 쿼리 엔진 작동을 위해 지난 14일 동안 수집된 데이터는 핫 캐시(SSD 지원)에 보관됩니다. 이 데이터는 CMK 구성에 관계없이 Microsoft 키로 암호화된 상태로 유지되지만 2020년 상반기에 SSD를 CMK로 암호화하기 위해 노력하고 있습니다.
...
이 리소스는 키 자격 증명 모음과 로그 분석 작업 영역 간의 중간 ID 연결로 사용됩니다. 구독이 목록에 등록되었다는 확인을 받은 후 작업 영역이 있는 리전에서 로그 분석 *클러스터* 리소스를 만듭니다. 애플리케이션 인사이트 및 로그 분석에는 별도의 *클러스터* 리소스 유형이 필요합니다. *클러스터* 리소스의 형식은 *clusterType* 속성을 *LogAnalytics*또는 *ApplicationInsights*로 설정하여 생성 시 정의됩니다. 클러스터 리소스 유형은 이후에 변경할 수 없습니다.

애플리케이션 인사이트 CMK 구성의 경우 부록 내용을 따르십시오.

*클러스터* 리소스를 만들 때 용량 예약 수준(sku)을 지정해야 합니다. 용량 예약 수준은 하루에 1,000 ~2,000GB 범위에 있을 수 있으며 나중에 100단계로 업데이트할 수 있습니다. 하루에 용량 예약 수준이 2,000GB를 초과하는 경우 Microsoft 연락처에 문의하여 활성화하십시오. 이 속성은 현재 청구에 영향을 주지 않습니다.전용 클러스터에 대 한 가격 모델이 도입 되면 기존 CMK 배포에 대 한 청구가 적용 됩니다.
...
기능의 초기 액세스 기간 동안 ADX 클러스터는 이전 단계가 완료되면 제품 팀에서 수동으로 프로비전됩니다. 이 단계에서 Microsoft 채널을 사용하고 *클러스터* 리소스 응답을 제공합니다. 
...
이 리소스 관리자 요청은 키 식별자 세부 정보를 업데이트할 때 비동기 작업이며 용량 값을 업데이트할 때 동기식입니다.
...
200 확인 및 헤더.
Key 식별자의 전파를 완료하는 데 몇 분 정도 걸립니다. 다음 두 가지 방법으로 업데이트 상태를 확인할 수 있습니다.
1. 응답에서 Azure-AsyncOperation URL 값을 복사하 고 [비동기 작업 상태 확인을](#asynchronous-operations-and-status-check)따릅니다.
...
키 를 비활성화하거나 키 볼트에서 *클러스터* 리소스 액세스 정책을 삭제하여 데이터에 대한 액세스를 취소할 수 있습니다. Azure Monitor Storage는 일반적으로 1시간 이내에 키 사용 권한의 변경 내용을 항상 더 빨리 적용하며 저장소를 사용할 수 없게 됩니다. *클러스터* 리소스와 연결된 작업 영역에 수집된 모든 데이터가 삭제되고 쿼리가 실패합니다. *클러스터* 리소스이고 작업 영역이 삭제되지 않는 한 이전에 수집한 데이터는 Azure Monitor Storage에서 액세스할 수 없습니다. 액세스할 수 없는 데이터는 데이터 보존 정책의 적용을 받으며 보존에 도달하면 제거됩니다.
...
모든 데이터는 AEK가 새 키 암호화 키(KEK) 버전으로 암호화되는 동안 모든 데이터가 AEK(계정 암호화 키)에 의해 암호화된 상태로 유지되므로 회전 전과 그 후에 수집된 데이터를 포함하여 키 회전 작업 후에 액세스할 수 있습니다.

## <a name="limitations-and-constraints"></a>제한 및 제약 조건

- CMK 기능은 ADX 클러스터 수준에서 지원되며 하루에 1TB 이상을 전송해야 하는 전용 Azure Monitor ADX 클러스터가 필요합니다.

- 구독당 최대 *클러스터* 리소스 수는 2개로 제한됩니다.

- *ADX* 클러스터 프로비저닝이 완료되었다는 것을 확인한 후에만 작업 영역에 대한 클러스터 리소스 연결을 수행해야 합니다. 프로비저닝이 완료되기 전에 작업 영역으로 전송된 데이터는 삭제되고 복구할 수 없습니다.

- CMK 암호화는 CMK 구성 후 새로 수집된 데이터에 적용됩니다. CMK 구성 이전에 수집된 데이터는 Microsoft 키로 암호화된 상태로 유지됩니다. CMK 구성 전후에 수집된 데이터를 원활하게 쿼리할 수 있습니다.

- CMK가 특정 작업 영역에 필요하지 않다고 결정할 때 *클러스터* 리소스에서 작업 영역의 연결을 해제할 수 있습니다. 연결 해제 작업 이후에 수집된 새 데이터는 *클러스터* 리소스에 연결되기 전과 마찬가지로 공유 Log Analytics 저장소에 저장됩니다. *클러스터* 리소스가 유효한 Key Vault 키로 프로비전되고 구성된 경우 연결 해제 전후에 수집된 데이터를 원활하게 쿼리할 수 있습니다.

- Azure 키 자격 증명 모음을 복구 가능한 것으로 구성해야 합니다. 이러한 속성은 기본적으로 활성화되지 않으며 CLI 또는 PowerShell을 사용하여 구성해야 합니다.
...
- 키 볼트 가용성
    - 정상 작동 - 저장소는 짧은 시간 동안 AEK를 캐시하고 키 볼트로 돌아가주기적으로 래핑을 해제합니다.
    
    - 일시적인 연결 오류 -- 저장소는 키가 잠시 동안 캐시에 머무를 수 있도록 하여 일시적인 오류(시간 시간, 연결 오류, DNS 문제)를 처리하며 가용성이 작아도 이를 극복합니다. 쿼리 및 인비전에이션 기능은 중단 없이 계속됩니다.
...
- *클러스터* 리소스를 만들 때 충돌 오류가 발생하면 지난 14일 동안 *클러스터* 리소스를 삭제했으며 일시 삭제 기간이 있는 것일 수 있습니다. *클러스터* 리소스 이름은 소프트 삭제 기간 동안 예약된 상태로 유지되며 해당 이름으로 새 클러스터를 만들 수 없습니다. 이름은 *클러스터* 리소스가 영구적으로 삭제되는 일시 삭제 기간 이후에 해제됩니다.
...
- *클러스터* 리소스에서 *용량 예약을* 업데이트합니다- 관련 작업 영역에 대한 데이터 볼륨이 변경되고 청구 고려 사항에 대한 용량 예약 수준을 업데이트하고 [ *클러스터* 리소스 업데이트에](#update-cluster-resource-with-key-identifier-details) 따라 새 용량 값을 제공하려는 경우 용량 예약 수준은 하루에 1,000 ~2,000GB 범위, 100단계까지 가능합니다. 하루에 2,000GB를 초과하는 레벨의 경우 Microsoft 연락처에 문의하여 활성화하십시오.

- *클러스터* 리소스 삭제 - 삭제가 우발적이든 의도적이든 14일 이내에 데이터를 포함하여 *클러스터* 리소스를 복구할 수 있도록 소프트 삭제 작업이 수행됩니다. *클러스터* 리소스 이름은 소프트 삭제 기간 동안 예약된 상태로 유지되며 해당 이름으로 새 클러스터를 만들 수 없습니다. 소프트 삭제 기간이 지나면 *클러스터* 리소스 이름이 해제되고 *클러스터* 리소스 및 데이터가 영구적으로 삭제되고 복구할 수 없습니다. 연결된 모든 작업 영역은 삭제 작업시 *클러스터* 리소스에서 연결이 해제됩니다. 새 수집된 데이터는 공유 로그 분석 저장소에 저장되고 Microsoft 키로 암호화됩니다. 작업 영역의 연결 해제 작업은 비동기입니다.
...
- *클러스터* 리소스 및 데이터 복구 - 지난 14일 동안 삭제된 *클러스터* 리소스는 일시 삭제 상태이며 복구할 수 있습니다. 이 작업을 수행하면 현재 제품 그룹에서 수동으로 수행됩니다. 복구 요청에 Microsoft 채널을 사용합니다.
...
> [!NOTE]
> 통합 전에 애플리케이션 인사이트 데이터에 CMK를 배포할 필요가 없는 경우 통합으로 인해 이러한 배포가 중단되고 Log Analytics 작업 영역으로 마이그레이션한 후 CMK를 다시 구성해야 하므로 응용 프로그램 인사이트 CMK를 기다리는 것이 좋습니다. 일일 최소 1TB는 클러스터 수준에서 적용되며 2분기 동안 통합이 완료될 때까지 애플리케이션 인사이트 및 로그 분석에는 별도의 클러스터가 필요합니다.
