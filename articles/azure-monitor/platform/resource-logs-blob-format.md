---
title: Azure Monitor 리소스 로그에 대한 형식 변경 준비
description: Azure 리소스 로그는 2018년 11월 1일에 부속 Blob을 사용하도록 이동했습니다.
author: johnkemnetz
services: monitoring
ms.topic: conceptual
ms.date: 07/06/2018
...
---
# <a name="prepare-for-format-change-to-azure-monitor-platform-logs-archived-to-a-storage-account"></a>저장소 계정에 보관된 Azure Monitor 플랫폼 로그에 대한 형식 변경 준비

> [!WARNING]
> 진단 설정 또는 활동 로그를 [사용하여 저장소 계정에 Azure 리소스 로그 또는 메트릭을](resource-logs-collect-storage.md) 보내는 경우 [로그 프로필을 사용하여 저장소](resource-logs-collect-storage.md)계정의 데이터 형식이 2018년 11월 1일에 JSON Lines로 변경되었습니다. 아래 지침에서는 새 형식을 처리하도록 도구를 업데이트하는 방법 및 영향에 대해 설명합니다.
>

## <a name="what-changed"></a>변경 내용

Azure Monitor는 리소스 로그 및 활동 로그를 Azure 저장소 계정, 이벤트 허브 네임스페이스 또는 Azure 모니터의 로그 분석 작업 영역으로 보낼 수 있는 기능을 제공합니다. 시스템 성능 문제를 해결하기 위해 **2018년 11월 1일 12:00 자정 UTC에서** 로그 데이터 형식이 Blob 저장소로 전송변경되었습니다. Blob Storage에서 데이터를 읽는 도구가 있는 경우, 새 데이터 형식을 이해하도록 도구를 업데이트해야 합니다.

* 2018년 11월 1일 목요일 12:00 자정 UTC에서 Blob 형식이 [JSON 라인으로](http://jsonlines.org/)변경되었습니다. 즉, 각 레코드가 줄 바꿈으로 구분되고 외부 레코드 배열 및 JSON 레코드 사이의 쉼표가 없습니다.
* 모든 구독에서 모든 진단 설정에 대해 Blob 형식이 한 번에 변경되었습니다. 11월 1일 에 내보낸 첫 번째 PT1H.json 파일은 이 새로운 형식을 사용했습니다. Blob 및 컨테이너 이름은 동일하게 유지됩니다.
* 11월 1일 이전에 진단 설정을 설정하면 11월 1일까지 현재 형식으로 데이터를 계속 내보전했습니다.
* 이 변경은 모든 퍼블릭 클라우드 리전에서 한 번에 발생했습니다. 이 변경사항은 21Vianet, Azure 독일 또는 Azure 정부 클라우드에서 운영하는 Microsoft Azure에서 아직 발생하지 않습니다.
* 이 변경은 다음 데이터 형식에 영향을 줍니다.
  * [Azure 리소스 로그(리소스](archive-diagnostic-logs.md) [목록 참조)](diagnostic-logs-schema.md)
...
* 다음 경우에는 이 변경의 영향을 받지 않습니다.
  * 네트워크 흐름 로그
  * Azure 서비스 로그는 아직 Azure Monitor를 통해 제공되지 않습니다(예: Azure App Service 리소스 로그, 저장소 분석 로그).
  * 다른 대상으로 Azure 리소스 로그 및 활동 로그 라우팅(이벤트 허브, 로그 분석)

### <a name="how-to-see-if-you-are-impacted"></a>영향을 받는지 확인하는 방법

다음과 같은 경우에만 이 변경의 영향을 받습니다.
1. 진단 설정을 사용하여 Azure 저장소 계정으로 로그 데이터를 보내고 있으며,
2. 스토리지에 있는 이러한 로그의 JSON 구조를 사용하는 도구가 있음
 
Azure 저장소 계정으로 데이터를 보내는 진단 설정이 있는지 확인하려면 포털의 **모니터** 섹션으로 이동하여 **진단 설정을**클릭하고 **진단 상태가** **활성화됨으로**설정된 리소스를 식별할 수 있습니다.
...
* 저장소 [계정에 리소스 리소스 로그 보관에](./../../azure-monitor/platform/archive-diagnostic-logs.md) 대해 자세히 알아보기
