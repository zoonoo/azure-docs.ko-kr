...
Virtual machines 및 virtual machine scale sets에는 Azure 호스팅 환경에서 수집 되는 **가상 컴퓨터 호스트** 메트릭과 가상 컴퓨터에서 실행 중인 [모니터링 에이전트가](agents-overview.md) 수집 하는 **게스트 OS (클래식)** 메트릭에 해당 하는 두 가지 범주의 메트릭이 있습니다. [Azure 진단 확장](diagnostics-extension-overview.md)을 사용하도록 설정하여 모니터링 에이전트를 설치합니다.
