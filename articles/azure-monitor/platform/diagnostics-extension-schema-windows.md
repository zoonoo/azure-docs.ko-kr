...
|**IsEnabled**|부울입니다. 이 페이지의 다른 곳에 있는 설명을 참조하세요.|  
...
|**sinks** | 1.5에 추가되었습니다. (선택 사항) 또한 진단 데이터를 보내는 싱크 위치를 가리킵니다. 예를 들어 Azure Monitor 또는 Event Hubs입니다. 참고 Event Hubs에 업로드 된 이벤트를 리소스 ID를 갖도록 하려면 *메트릭* 요소 아래에 *resourceId* 속성을 추가 해야 합니다.|    
...
|**sinks** |**string**| 1.5에 추가되었습니다. (선택 사항) 또한 진단 데이터를 보내는 싱크 위치를 가리킵니다. Application Insights 또는 Event Hubs를 예로 들 수 있습니다. 참고 Event Hubs에 업로드 된 이벤트를 리소스 ID를 갖도록 하려면 *메트릭* 요소 아래에 *resourceId* 속성을 추가 해야 합니다.|  
 ...
 *Tree: Root - DiagnosticsConfiguration - IsEnabled*
 ...
 부울입니다. `true`를 사용하여 진단을 사용하도록 설정하거나 `false`를 사용하여 진단을 사용하지 않도록 설정합니다.
