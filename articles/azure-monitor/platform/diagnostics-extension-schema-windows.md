...
description: Azure 모니터의 WAD(Windows 진단 확장)에 대한 구성 스키마 참조입니다.
...
> [!NOTE]
> 이 문서의 스키마는 버전 1.3 및 최신 버전(Azure SDK 2.4 및 최신 버전)에 유효합니다. 최신 구성 섹션은 추가된 버전에 표시하도록 주석 처리되었습니다. 스키마의 버전 1.0 및 1.2가 보관되어 더 이상 사용할 수 없습니다. 
...
|**스토리지 계정**|데이터를 저장할 Azure Storage 계정의 이름입니다. Set-AzureServiceDiagnosticsExtension cmdlet을 실행할 때 매개 변수로 지정할 수도 있습니다.|  
...
|**DiagnosticInfrastructureLogs**|Azure Diagnostics에 의해 생성된 로그의 컬렉션을 사용하도록 설정합니다. 진단 인프라 로그는 진단 시스템 자체의 문제 해결에 유용합니다. 선택적 특성은 다음과 같습니다.<br /><br /> - **scheduledTransferLogLevelFilter** - 수집된 로그의 최소 심각도 수준을 구성합니다.<br /><br /> - **예약전송기간** - 예약된 전송에서 저장소로의 전송 사이의 간격이 가장 가까운 분까지 반올림됩니다. 값은 [XML "기간 데이터 형식"](https://www.w3schools.com/xml/schema_dtypes_date.asp)입니다. |  
...
|자식 요소|설명|  
|--------------------|-----------------|  
|**EtwEventSourceProviderConfiguration**|[EventSource 클래스](https://msdn.microsoft.com/library/system.diagnostics.tracing.eventsource\(v=vs.110\).aspx)에서 생성된 이벤트 컬렉션을 구성합니다. 필수 특성:<br /><br /> **provider** - EventSource 이벤트의 클래스 이름입니다.<br /><br /> 선택적 특성은 다음과 같습니다.<br /><br /> - **scheduledTransferLogLevelFilter** - 스토리지 계정으로 전송할 최소 심각도 수준입니다.<br /><br /> - **예약전송기간** - 예약된 전송에서 저장소로의 전송 사이의 간격이 가장 가까운 분까지 반올림됩니다. 값은 [XML "기간 데이터 형식"](https://www.w3schools.com/xml/schema_dtypes_date.asp)입니다. |  
|**EtwManifestProviderConfiguration**|필수 특성:<br /><br /> **provider** - 이벤트 공급자의 GUID<br /><br /> 선택적 특성은 다음과 같습니다.<br /><br /> - **scheduledTransferLogLevelFilter** - 스토리지 계정으로 전송할 최소 심각도 수준입니다.<br /><br /> - **예약전송기간** - 예약된 전송에서 저장소로의 전송 사이의 간격이 가장 가까운 분까지 반올림됩니다. 값은 [XML "기간 데이터 형식"](https://www.w3schools.com/xml/schema_dtypes_date.asp)입니다. |  
 ...
 **resourceId** 특성이 필요합니다.  Azure Diagnostics를 배포하는 가상 머신 또는 Virtual Machine Scale Set의 리소스 ID입니다. [Azure Portal](https://portal.azure.com)에서 **resourceID**를 가져옵니다. **리소스 그룹**  -> <** 이름을\>** **찾아선택합니다.** ->  **속성** 타일을 클릭하고 **ID** 필드에서 값을 복사합니다.  
...
|자식 요소|설명|  
|-------------------|-----------------|  
|**PerformanceCounterConfiguration**|다음과 같은 특성이 필요합니다.<br /><br /> - **counterSpecifier** - 성능 카운터의 이름입니다. `\Processor(_Total)\% Processor Time`)을 입력합니다. 호스트에서 성능 카운터의 목록을 가져오려면 명령 `typeperf`를 실행합니다.<br /><br /> - **sampleRate** - 카운터가 샘플링되는 주기입니다.<br /><br /> 선택적 특성:<br /><br /> **unit** - 카운터의 측정 단위입니다. 값은 [단위 유형 클래스에서](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.sql.models.unittype?view=azure-dotnet) 사용할 수 있습니다. |
...
## <a name="example-configuration"></a>구성 예
 다음은 JSON 및 XML모두에 표시된 Windows 진단 확장에 대한 전체 샘플 구성입니다.

 
### <a name="json"></a>JSON

*PublicConfig* 및 *PrivateConfig는* 대부분의 JSON 사용 사례에서 서로 다른 변수로 전달되기 때문에 분리됩니다. 이러한 경우는 리소스 관리자 템플릿, PowerShell 및 Visual Studio를 포함합니다.

> [!NOTE]
> 공용 구성 Azure Monitor 싱크 정의에는 *resourceId* 및 *지역두*가지 속성이 있습니다. 두 속성은 클래식 VM 및 클래식 클라우드 서비스에만 필요합니다. 이러한 속성은 다른 리소스에 사용해서는 안 됩니다.
...
> [!NOTE]
> 개인 구성 Azure Monitor 싱크 정의에는 *PrincipalId* 및 *Secret*의 두 가지 속성이 있습니다. 두 속성은 클래식 VM 및 클래식 클라우드 서비스에만 필요합니다. 이러한 속성은 다른 리소스에 사용해서는 안 됩니다.
