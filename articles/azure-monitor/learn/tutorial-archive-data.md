...
description: Azure 내의 로그 및 메트릭 데이터를 스토리지 계정에 보관합니다.
author: johnkemnetz
...
3. 표시되는 **활동 로그 내보내기** 섹션에서 **스토리지 계정으로 내보내기** 상자를 선택하고 **스토리지 계정 선택**을 클릭합니다.

   ![활동 로그 내보내기](media/tutorial-archive-data/activity-log-export.png)

4. 표시되는 섹션에서 **스토리지 계정** 드롭다운을 사용하여 이전 **스토리지 계정 만들기** 단계에서 만든 스토리지 계정 이름을 선택한 후 **확인**을 클릭합니다.

   ![스토리지 계정 선택](media/tutorial-archive-data/activity-log-storage.png)

5. **보존 기간(일)** 슬라이더를 30으로 설정합니다. 이 슬라이더는 스토리지 계정에 모니터링 데이터를 보존할 일 수를 설정합니다. Azure Monitor는 지정된 일 수보다 오래된 데이터를 자동으로 삭제합니다. 보존 기간을 0일로 설정하면 데이터를 무기한 저장합니다.

6. **저장**을 클릭하여 이 섹션을 닫습니다.

이제 구독의 모니터링 데이터가 스토리지 계정으로 이동합니다.
...
3. 표시된 섹션에서 설정에 **이름**을 지정하고 **스토리지 계정에 보관** 상자를 선택합니다.

   ![진단 설정 섹션](media/tutorial-archive-data/diagnostic-settings-home.png)

4. **스토리지 계정에 보관** 아래 **구성** 단추를 클릭하고 이전 섹션에서 만든 스토리지 계정을 선택합니다. **확인**을 클릭합니다.

   ![스토리지 계정 진단 설정](media/tutorial-archive-data/diagnostic-settings-storage.png)

5. **로그** 및 **메트릭** 아래의 상자를 모두 선택합니다. 리소스 종류에 따라 이러한 옵션 중 하나를 포함할 수 있습니다. 이 확인란에 따라 선택한 대상(이 경우, 스토리지 계정)으로 보내지는 해당 리소스 종류에서 사용할 수 있는 로그 및 메트릭 데이터의 범주가 달라집니다.

   ![진단 설정 범주](media/tutorial-archive-data/diagnostic-settings-categories.png)

6. **보존 기간(일)** 슬라이더를 30으로 설정합니다. 이 슬라이더는 스토리지 계정에 모니터링 데이터를 보존할 일 수를 설정합니다. Azure Monitor는 지정된 일 수보다 오래된 데이터를 자동으로 삭제합니다. 보존 기간을 0일로 설정하면 데이터를 무기한 저장합니다.
...
## <a name="route-virtual-machine-guest-os-data-to-the-storage-account"></a>스토리지 계정에 가상 머신(게스트 OS) 데이터 라우팅
...
4. 표시되는 섹션에서 왼쪽 탐색에서 **진단 설정**을 클릭합니다. 이 섹션에서는 가상 머신에서 Azure Monitor의 기본 모니터링 확장을 설정하고 Windows 또는 Linux에서 생성되는 데이터를 스토리지 계정으로 라우팅할 수 있습니다.
...
8. **에이전트** 탭을 클릭하고 **스토리지 계정** 아래에서 표시된 스토리지 계정 이름을 클릭합니다.

   ![스토리지 계정 업데이트](media/tutorial-archive-data/guest-storage-home.png)

9. 표시되는 섹션에서 이전 **스토리지 계정 만들기** 단계에서 만든 스토리지 계정을 선택합니다.

10. **저장**을 클릭합니다.

이제 가상 머신의 모니터링 데이터가 스토리지 계정으로 이동합니다.
...
1. 활동 로그와 같은 일부 데이터 형식의 경우, 스토리지 계정에 이벤트를 생성하는 활동이 있어야 합니다. 활동 로그에 활동을 생성하려면 [다음 지침](./../../azure-monitor/platform/quick-audit-notify-action-subscription.md)을 따르세요. 스토리지 계정에 이벤트가 표시되려면 최대 5분 동안 기다려야 할 수 있습니다.

2. 포털의 왼쪽 탐색 모음에서 찾아 **스토리지 계정** 섹션으로 이동합니다.

3. 이전 섹션에서 만든 스토리지 계정을 식별하고 클릭합니다.
...
6. 게스트 OS 모니터링 데이터는 테이블에 저장됩니다. 스토리지 계정 홈으로 돌아가 **테이블**을 클릭합니다. 메트릭, 성능 카운터 및 이벤트 로그에 대한 테이블이 있습니다.

이제 스토리지 계정에 보관할 모니터링 데이터를 성공적으로 설정했습니다.

## <a name="clean-up-resources"></a>리소스 정리

1. 이전 **스토리지 계정에 구독 로그 라우팅** 단계에서 **활동 로그 내보내기** 섹션으로 돌아가 **다시 설정**을 클릭합니다.

2. **진단 설정** 섹션으로 이동하여 이전 **스토리지 계정에 리소스 데이터 라우팅** 단계에서 진단 설정을 만든 리소스를 클릭한 후 만든 설정을 찾고 **설정 편집** 단추를 클릭하고 **삭제**를 클릭합니다.

3. 이전 **스토리지 계정에 가상 머신(게스트 OS) 데이터 라우팅** 단계에서 구성한 가상 머신에서 **진단 설정** 섹션으로 이동하고 **에이전트** 탭 아래에서 **제거**(**Azure Diagnostics 에이전트 제거** 섹션 아래)를 클릭합니다.

4. 이전 **스토리지 계정 만들기** 단계에서 만든 스토리지 계정으로 이동하여 **스토리지 계정 삭제**를 클릭합니다. 스토리지 계정의 이름을 입력한 다음 **삭제**를 클릭합니다.

5. 이전 단계에서 가상 머신 또는 논리 앱을 만든 경우 해당 항목도 삭제합니다.

## <a name="next-steps"></a>다음 단계

이 자습서에서는 스토리지 계정에 보관할 Azure 환경(구독, 리소스 및 게스트 OS)에서 모니터링 데이터를 설정하는 방법을 배웠습니다.
