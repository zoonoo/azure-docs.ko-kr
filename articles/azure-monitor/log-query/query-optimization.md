...
Azure Monitor 로그는 Azure 데이터 탐색기의 대규모 클러스터를 사용 하 여 쿼리를 실행 하 고 있으며 이러한 클러스터는 규모에 따라 달라 집니다. 시스템은 작업 영역 배치 논리 및 용량에 따라 클러스터 크기를 자동으로 조정 합니다.
