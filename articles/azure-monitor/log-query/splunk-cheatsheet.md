---
title: Splunk 및 Azure Log Analytics | Microsoft Docs
description: Splunk에 익숙한 사용자가 Log Analytics 쿼리 언어를 학습할 수 있도록 지원합니다.
...
ms.locfileid: "52885926"
---
# <a name="splunk-to-log-analytics"></a>Splunk 및 Log Analytics

이 문서는 Splunk에 익숙한 사용자가 Log Analytics 쿼리 언어를 학습하는 데 도움을 주기 위한 것입니다. 주요 차이점을 이해하기 위해 이 둘을 직접 비교하고, 기존 지식을 활용할 수 있는 유사점도 파악합니다.

## <a name="structure-and-concepts"></a>구조 및 개념

다음 표에서는 Splunk와 Log Analytics 간의 개념과 데이터 구조를 비교합니다.

 | 개념  | Splunk | Log Analytics |  주석
 | --- | --- | --- | ---
 | 배포 단위  | cluster |  cluster |  Log Analytics를 사용하면 임의의 클러스터 간 쿼리를 수행할 수 있습니다. Splunk는 그렇지 않습니다. |
 ...
 | 구조적 이벤트 메타데이터 | 해당 없음 | 테이블 |  Splunk에는 이벤트 메타데이터의 검색 언어에 대해 공개된 개념이 없습니다. Log Analytics에는 열이 있는 테이블에 대한 개념이 있습니다. 각 이벤트 인스턴스가 한 행에 매핑됩니다. |
 | 데이터 레코드 | event | 행 |  용어 변경에만 해당 |
 | 데이터 레코드 특성 | 필드 |  열 |  Log Analytics에서는 테이블 구조의 일부로 미리 정의됩니다. Splunk에서는 각 이벤트마다 자체의 필드 집합이 있습니다. |
 | 형식 | 데이터 형식 |  데이터 형식 |  Log Analytics 데이터 형식은 열에 설정되므로 더 명시적입니다. 둘 다 JSON 지원을 포함하여 데이터 형식 및 거의 동등한 데이터 형식 집합을 동적으로 사용할 수 있습니다. |
 | 쿼리 및 검색  | 검색 | 쿼리 |  개념은 기본적으로 Log Analytics와 Splunk 간에 동일합니다. |
 | 이벤트 수집 시간 | 시스템 시간 | ingestion_time() |  Splunk에서 각 이벤트는 이벤트가 인덱싱된 시간의 시스템 타임스탬프를 가져옵니다. Log Analytics에서는 ingestion_time() 함수를 통해 참조할 수 있는 시스템 열을 공개하는 ingestion_time이라는 정책을 정의할 수 있습니다. |

## <a name="functions"></a>Functions

다음 표에서는 Splunk 함수와 동일한 Log Analytics 함수를 지정합니다.

|Splunk | Log Analytics |주석
|---|---|---
|strcat | strcat()| (1) |
|split  | split() | (1) |
|if     | iff()   | (1) |
...
| substr | substring() | (1)<br>또한 Splunk는 1부터 시작하는 인덱스를 사용합니다. Log Analytics는 0부터 시작하는 인덱스를 기록합니다. |
| tolower |  tolower() | (1) |
| toupper | toupper() | (1) |
| match | matches regex |  (2)  |
| regex | matches regex | Splunk에서 `regex`는 연산자입니다. Log Analytics에서는 관계 연산자입니다. |
| searchmatch | == | Splunk에서 `searchmatch`는 일치 문자열 검색을 허용합니다.
| random | rand()<br>rand(n) | Splunk 함수는 0에서 2<sup>31</sup>-1까지의 숫자를 반환합니다. Log Analytics는 0.0에서 1.0 사이의 숫자를 반환하거나, 매개 변수가 제공되는 경우 0에서 n-1 사이의 숫자를 반환합니다.
| now | now() | (1)
| relative_time | totimespan() | (1)<br>Log Analytics에서 Splunk의 relative_time(datetimeVal, offsetVal)은 datetimeVal + totimespan(offsetVal)입니다.<br>예를 들어 <code>search &#124; eval n=relative_time(now(), "-1d@d")</code>는 <code>...  &#124; extend myTime = now() - totimespan("1d")</code>가 됩니다.

(1) Splunk에서 함수는 `eval` 연산자를 사용하여 호출됩니다. Log Analytics에서는 `extend` 또는 `project`의 일부로 사용합니다.<br>(2) Splunk에서 함수는 `eval` 연산자를 사용하여 호출됩니다. Log Analytics에서는 `where` 연산자와 함께 사용할 수 있습니다.


## <a name="operators"></a>연산자

다음 섹션에서는 Splunk와 Log Analytics 간에 서로 다른 연산자를 사용하는 예제를 보여 줍니다.

> [!NOTE]
> 다음 예제에서는 _Rule_ Splunk 필드가 Azure Log Analytics의 테이블에 매핑되고, Splunk의 기본 타임스탬프가 Log Analytics의 _ingestion_time()_ 열에 매핑됩니다.

### <a name="search"></a>검색
Splunk에서는 `search` 키워드를 생략하고 따옴표가 없는 문자열을 지정할 수 있습니다. Azure Log Analytics에서는 `find`를 사용하여 각 검색을 시작해야 하며, 따옴표가 없는 문자열은 열 이름이고, 조회 값은 따옴표가 붙은 문자열이어야 합니다. 
...
Azure Log Analytics 쿼리는 필터가 있는 테이블 형식 결과 집합에서 시작합니다. Splunk에서 필터링은 현재 인덱스에 대한 기본 작업입니다. 또한 Splunk에서 `where` 연산자도 사용할 수 있지만 권장되지 않습니다.
...
Azure Log Analytics는 `take`도 `limit`의 별칭으로 지원합니다. Splunk에서 결과가 정렬되면 `head`에서 처음 n개의 결과를 반환합니다. Azure Log Analytics에서 limit는 정렬되지 않지만 검색한 처음 n개의 행을 반환합니다.
...
아래 결과를 위해 Splunk에서 `tail`을 사용합니다. Azure Log Analytics에서는 `asc`를 사용하여 정렬 방향을 지정할 수 있습니다.
...
Splunk에는 `eval` 연산자를 사용하여 비교할 수 없는 `eval` 함수가 있습니다. Splunk의 `eval` 연산자와 Azure Log Analytics의 `extend` 연산자는 모두 스칼라 함수와 산술 연산자만 지원합니다.
...
Azure Log Analytics는 동일한 연산자를 사용하여 이름을 바꾸고 새 필드를 만듭니다. Splunk에는 별도의 두 연산자인 `eval`과 `rename`이 있습니다.
...
다양한 집계 함수는 [Log Analytics 쿼리의 집계](aggregations.md)를 참조하세요.
...
### <a name="sort"></a>정렬
Splunk에서 오름차순으로 정렬하려면 `reverse` 연산자를 사용해야 합니다. 또한 Azure Log Analytics에서는 처음부터 끝까지 null을 배치할 수 있는 위치를 정의합니다.
...
Splunk와 Log Analytics에서 모두 비슷한 연산자입니다.
...
Log Analytics 포털에서는 첫 번째 열만 공개됩니다. 모든 열은 API를 통해 사용할 수 있습니다.
