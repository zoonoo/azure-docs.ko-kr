...
조합의 한 부분만 사용 하는 경우 자동 크기 조정 기능은 프로필에 정의 된의 최대 또는 최소 인스턴스 수에 도달할 때까지 단일 방향 (규모 확장 또는에서) 으로만 동작을 수행 합니다. 이는 최적의 방법이 아니며 가용성을 보장하기 위해 사용률이 높은 시간에 이상적으로 리소스 규모를 확장할 수 있습니다. 마찬가지로 사용률이 낮은 시간에 리소스 규모를 축소하여 비용 절감을 실현할 수 있습니다.
...
4. 축소하기 전에 자동 크기 조정에서 감축된 경우의 최종 상태를 예측하려고 시도합니다. 예를 들어 575x3(현재 인스턴스 수) = 1,725/2(축소된 경우의 최종 인스턴스 수) = 862.5스레드입니다. 즉, 평균 스레드 수가 동일 하 게 유지 되거나 작은 양만큼만 떨어지면 자동 크기 조정이 확장 된 후에도 즉시 다시 확장 해야 합니다. 그러나 규모가 다시 확장된 경우 전체 프로세스가 반복되므로 무한 루프가 발생합니다.
5. 이 "느슨한" 상황을 방지하기 위해 자동 크기 조정에서는 규모를 축소하지 않습니다. 대신 건너뛰고 다음에 서비스의 작업이 실행될 때 조건을 다시 평가합니다. 플 래핑 상태는 평균 스레드 수가 575 일 때 자동 크기 조정이 작동 하지 않기 때문에 많은 사용자를 혼동할 수 있습니다.
...
> [!NOTE]
> 대상 인스턴스 수로 크기를 조정한 결과로 자동 크기 조정 엔진이 플 래핑을 검색 하는 경우에는 현재 개수와 대상 수 사이에 다른 인스턴스 수로도 확장 하려고 시도 합니다. 플 래핑이이 범위 내에서 발생 하지 않는 경우 자동 크기 조정 작업은 새 대상으로 크기 조정 작업을 계속 합니다.
...
4. 확장 작업으로 인해 세 번째 저장소 큐 인스턴스가 추가 됩니다.  150/3 = 50이므로 큐의 총 메시지 수가 150개에 도달할 때까지 다음 규모 확장 동작이 발생하지 않습니다.
...
예제를 사용 하 여 검토해 보겠습니다.

아래 그림에는 최소 인스턴스 = 2이고 최대 인스턴스 = 10인 기본 프로필의 자동 크기 조정 설정을 보여 줍니다. 이 예제에서는 큐의 메시지 수가 10 보다 큰 경우 규모를 확장 하 고, 큐의 메시지 수가 3 보다 작은 경우 규모를 축소 하도록 규칙을 구성 합니다. 따라서 지금은 리소스의 크기를 2에서 10 사이로 조정할 수 있습니다.
...
* CPU가 76%이 고 메모리가 50% 이면 규모를 확장 합니다.
* CPU가 50%이 고 메모리가 76% 이면 규모를 확장 합니다.
...
* 자동 크기 조정은 플 래핑을 감지 하 고 확장 시도를 중단 합니다. 이 경우의 로그 유형이 표시 됩니다 `Flapping` . 이 내용이 표시 되 면 임계값이 너무 좁아 있는지를 고려 하세요.
* 자동 크기 조정은 플 래핑을 검색 하지만 성공적으로 크기를 조정할 수 있습니다. 이 경우의 로그 유형이 표시 됩니다 `FlappingOccurred` . 이 문제가 표시 되는 경우 자동 크기 조정 엔진은 크기 조정 (예: 4 개 인스턴스에서 2로)을 시도 하지만이로 인해 플 래핑이 발생 함을 확인 했습니다. 대신 자동 크기 조정 엔진은 다른 인스턴스로 크기를 조정 했습니다 (예: 2 대신 3 개의 인스턴스 사용) .이로 인해 플 래핑가 더 이상 발생 하지 않으므로이 인스턴스 수로 크기가 조정 됩니다.
...
- [구독에 대 한 모든 자동 크기 조정 엔진 작업을 모니터링 하기 위한 활동 로그 경고를 만듭니다.](https://github.com/Azure/azure-quickstart-templates/tree/master/monitor-autoscale-alert)
