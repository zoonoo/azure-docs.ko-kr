...
* 이 변경은 다음 데이터 형식에 영향을 줍니다.
  ...
  * [진단 설정을 통해 내보내지는 Azure 리소스 메트릭](../essentials/diagnostic-settings.md)
...
![Azure Monitor 진단 설정 블레이드](media/resource-logs-blob-format/portal-diag-settings.png)

진단 상태가 사용으로 설정된 경우, 해당 리소스에 활성 진단 설정이 있습니다. 리소스를 클릭하여 스토리지 계정에 데이터를 보내는 진단 설정이 있는지 확인합니다.

![스토리지 계정 사용](media/resource-logs-blob-format/portal-storage-enabled.png)
