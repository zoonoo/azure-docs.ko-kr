...
2. **가상 네트워크 만들기**에서 다음 설정을 적용 합니다. 
    ...
    |속성|databricks-빠른 시작|가상 네트워크의 이름을 선택 합니다.|
    |지역|\<사용자와 가장 가까운 지역 선택\>|가상 네트워크를 호스트할 수 있는 지리적 위치를 선택 합니다. 사용자와 가장 가까운 위치를 사용합니다.|
...
3. **다음: IP 주소 >** 를 선택 하 고 다음 설정을 적용 합니다. 그런 다음 **검토 + 만들기**를 선택 합니다.
    ...
    |서브넷 이름|default|가상 네트워크에서 기본 서브넷의 이름을 선택 합니다.|
...
4. **검토 + 만들기** 탭에서 **만들기** 를 선택 하 여 가상 네트워크를 배포 합니다. 배포가 완료 되 면 가상 네트워크로 이동 하 고 **설정**아래에서 **주소 공간** 을 선택 합니다. *추가 주소 범위 추가*상자에를 삽입 `10.179.0.0/16` 하 고 **저장**을 선택 합니다.
...
2. **Azure Databricks 서비스**에서 다음 설정을 적용 합니다.
    ...
    |가격 책정 계층|표준 또는 프리미엄 중에서 선택합니다.|가격 책정 계층에 대 한 자세한 내용은 [Databricks 가격 책정 페이지](https://azure.microsoft.com/pricing/details/databricks/)를 참조 하세요.|
...
2. 클러스터 및**클러스터 만들기** **를 선택 합니다.** >  그런 다음 *databricks-cluster*와 같은 클러스터 이름을 만들고 나머지 기본 설정을 적용 합니다. **클러스터 만들기**를 선택 합니다.
