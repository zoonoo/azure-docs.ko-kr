...
이 빠른 시작에서는 가상 네트워크에서 Azure Databricks 작업 영역을 만드는 방법을 보여 줍니다. 또한 해당 작업 영역 내에서 Apache Spark 클러스터를 생성합니다.
...
2. **가상 네트워크 만들기** 아래에서, 다음 설정을 적용합니다. 
    ...
    |서브넷 이름|기본|가상 네트워크에서 기본 서브넷의 이름을 선택 합니다.|
...
2. **Azure Databricks Service** 아래에서, 다음 설정을 적용합니다.
    ...
    |Virtual Network에서 Azure Databricks 작업 영역 배포|예|이 설정을 사용 하면 가상 네트워크에 Azure Databricks 작업 영역을 배포할 수 있습니다.|
