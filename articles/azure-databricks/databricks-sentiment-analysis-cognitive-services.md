...
description: Event Hubs 및 Cognitive Services API를 통해 Azure Databricks를 사용하여 실시간 스트리밍 데이터에 대한 감정 분석을 실행하는 방법을 알아봅니다.
...
이 자습서에서는 Azure Databricks를 사용하여 실시간 데이터 스트림에서 감정 분석을 실행하는 방법에 대해 알아봅니다. Azure Event Hubs를 사용하여 실시간 데이터 수집 시스템을 설정합니다. Spark Event Hubs 커넥터를 사용하여 Event Hubs에서 Azure Databricks로 메시지를 사용합니다. 마지막으로, Microsoft Cognitive Service API를 사용하여 스트리밍된 데이터에 대한 감정 분석을 실행합니다. 
...
> * 실시간 데이터에 액세스할 수 있는 Twitter 앱 만들기
...
1. Azure Portal에서 **리소스 만들기** > **데이터 + 분석** > **Azure Databricks(미리 보기)**를 차례로 선택합니다.
...
2. **Azure Databricks(미리 보기)** 아래에서 **만들기**를 선택합니다.
...
실시간 트윗 스트림을 받으려면 Twitter에 응용 프로그램을 만들어야 합니다. 다음 단계에 따라 Twitter 응용 프로그램을 만들고 이 자습서를 완료하는 데 필요한 값을 기록합니다.
...
2. [새 라이브러리] 페이지에서 **원본**에 대해 **Maven 코디네이트**를 선택합니다. **코디네이트**에서 추가하려는 패키지에 대한 코디네이트를 입력합니다. 이 자습서에서 사용된 라이브러리에 대한 Maven 코디네이트는 다음과 같습니다.
    ...
    * Spark Event Hubs 커넥터 - `com.microsoft.azure:azure-eventhubs-spark_2.11:2.3.0`
...
이 자습서에서는 [Microsoft Cognitive Services 텍스트 분석 API](../cognitive-services/text-analytics/overview.md)를 사용하여 실시간 트윗 스트림에 대한 감정 분석을 실행합니다. API를 사용하기 전에 Azure에 Microsoft Cognitive Services 계정을 만들고 텍스트 분석 API를 사용하기 위한 액세스 키를 검색해야 합니다.
...
노트북을 실행하려면 **Shift+Enter**를 누릅니다. 다음 코드 조각과 같은 출력이 표시됩니다. 출력의 각 이벤트는 Event Hubs로 수집되는 실시간 트윗입니다. 
...
이것으로 끝입니다. Azure Databricks를 사용하여 실시간 데이터를 Azure Event Hubs에 성공적으로 스트리밍하고, Event Hubs 커넥터를 사용하여 스트림 데이터를 사용한 다음, 스트리밍 데이터에 대한 감정 분석을 실행했습니다.
