...
필요한 서브넷, 네트워크 보안 그룹 및 허용 목록 설정을 사용하여 기존 가상 네트워크를 자동으로 구성하기 위해 Azure portal에서 Azure Databricks 작업 영역 배포 인터페이스를 사용하거나 작업 영역을 배포하고 가상 네크워크를 구성하기 위해 Azure Resource Manager 템플릿를 사용할 수 있습니다.
...
Azure Databricks 작업 영역을 배포할 가상 네트워크가 있어야 합니다. 기존 가상 네트워크를 사용하거나 새 가상 네트워크를 만들 수 있습니다. 하지만 가상 네트워크는 만들려는 Azure Databricks 작업 영역과 동일한 지역에 있어야 합니다. /16- /24 사이의 CIDR 범위가 가상 네트워크에 필요합니다.
...
1. Azure portal에서 **+ 리소스 만들기 > 분석 > Azure Databricks**를 선택하여 Azure Databricks 서비스 대화 상자를 엽니다.
...
|-----------------------|-------|---------|
|East US|제어 평면 NAT </br></br>Webapp|23.101.152.95/32 </br></br>40.70.58.221/32|
...
|유럽 서부|제어 평면 NAT </br></br>Webapp|23.100.0.135/32 </br></br>52.232.19.246/32|
|유럽 북부|제어 평면 NAT </br></br>Webapp|23.100.0.135/32 </br></br>52.232.19.246/32|
...
|아시아 남동부|제어 평면 NAT </br></br>Webapp|52.187.0.85/32 </br></br>52.187.145.107/32|
|아시아 동부|제어 평면 NAT </br></br>Webapp|52.187.0.85/32 </br></br>52.187.145.107/32|
...
사용자 지정 가상 네트워크에서 작업 영역을 시작할 때 다음 오류 화면이 나타나면서 Azure Databricks 로그인이 실패합니다. **"작업 영역을 만드는 도중에 오류가 발생했습니다. 사용자 지정 네트워크 구성이 올바른지 확인하고 다시 시도 하십시오. "**

이 오류는 네트워크 구성이 요구 사항을 충족하지 않아 발생합니다. 작업 영역을 만들 때 이 항목의 지침을 수행했는지 확인합니다.

### <a name="cluster-creation-errors"></a>클러스터 생성 오류

**Instances에 연결할 수 없음: SSH를 통해 리소스에 연결할 수 없습니다.**
...
**예기치 않은 시작 오류: 클러스터를 설정 하는 동안 예기치 않은 오류가 발생 했습니다. 다시 시도하고 문제가 계속되면 Azure Databricks에 문의합니다. 내부 오류 메시지: 노드를 배치 하는 동안 시간이 초과 되었습니다.**
...
**클라우드 공급자 시작 실패: 클러스터를 설정하는 동안 클라우드 공급자 오류가 발생했습니다. 자세한 내용은 Azure Databricks 가이드를 참조 하세요. Azure 오류 코드: AuthorizationFailed/InvalidResourceReference.**

가능한 원인: 가상 네트워크 또는 서브넷이 더 이상 존재하지 않습니다. 가상 네트워크 및 서브넷이 존재하는지 확인합니다.

**클러스터가 종료됩니다. 원인: Spark 시작 실패: Spark를 시간 내에 시작할 수 없습니다. 이 문제는 Hive metastore가 제대로 작동하지 않거나, 잘못된 Spark 구성 또는 오작동 init 스크립트에 의해 발생할 수 있습니다. 이 문제를 해결하려면 Spark 드라이버 로그를 참조하고, 문제가 지속되면 Databricks에 문의합니다. 내부 오류 메시지: Spark를 시작 하지 못함: 드라이버가 제 시간에 시작하지 못했습니다.**

가능한 원인: 컨테이너가 호스팅 인스턴스 또는 DBFS 저장소 계정과 통신할 수 없습니다. 다음 홉이 인터넷인 것을 사용하여 DBFS 저장소 계정에 대한 서브넷에 사용자 지정 경로를 추가하여 수정합니다.

### <a name="notebook-command-errors"></a>노트북 명령 오류

**명령이 응답하지 않습니다.**

가능한 원인: 작업자 간 통신이 차단 됩니다. 인바운드 보안 규칙이 요구 사항을 충족하는지 확인하여 수정합니다.
