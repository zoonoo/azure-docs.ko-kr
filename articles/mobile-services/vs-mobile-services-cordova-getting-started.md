---
title: Cordova 모바일 서비스 프로젝트 시작하기(Visual Studio 연결 서비스) | Microsoft Docs
...
## 테이블에 항목 추가
데이터 테이블에 새 항목을 삽입합니다. 자동으로 ID(문자열 유형의 GUID)가 새 행의 기본 키로 만들어집니다. 반환된 **Promise** 개체에 대한 [done](https://msdn.microsoft.com/library/dn802826.aspx) 메서드를 호출하여 삽입된 개체의 복사본을 가져오고 오류를 처리합니다.
...
## 테이블 항목 업데이트
데이터 테이블의 행을 업데이트합니다. 이 코드에서는 모바일 서비스가 응답하면 항목이 목록에서 제거됩니다. 반환된 **Promise** 개체에 대한 [done](https://msdn.microsoft.com/library/dn802826.aspx) 메서드를 호출하여 삽입된 개체의 복사본을 가져오고 오류를 처리합니다.
...
## 테이블 항목 삭제
**del** 메서드를 사용하여 데이터 테이블의 행을 삭제합니다. 반환된 **Promise** 개체에 대한 [done](https://msdn.microsoft.com/library/dn802826.aspx) 메서드를 호출하여 삽입된 개체의 복사본을 가져오고 오류를 처리합니다.
