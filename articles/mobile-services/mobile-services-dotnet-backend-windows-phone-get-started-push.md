...
이 항목에서는 MPNS(Microsoft 푸시 알림 서비스)를 사용하여 .NET 백 엔드와 함께 Azure 모바일 서비스를 사용하여 Windows Phone Silverlight 8 앱에 푸시 알림을 보내는 방법을 보여 줍니다. 이 자습서에서는 빠른 시작 프로젝트에 대한 Azure 알림 허브를 사용하여 푸시 알림을 사용하도록 설정합니다. 이 작업을 완료하면 레코드가 삽입될 때마다 모바일 서비스에서 알림 허브를 사용하여 푸시 알림을 전송합니다. 모바일 서비스를 통해 무료로 만드는 알림 허브는 모바일 서비스와 별도로 관리할 수 있으며, 다른 응용 프로그램과 서비스에서 도 사용 가능합니다.

이 자습서는 모바일 서비스 TodoList 프로젝트를 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [기존 앱에 모바일 서비스 추가]를 완료하여 프로젝트를 모바일 서비스에 연결해야 합니다.

>[AZURE.NOTE]이 자습서는 Windows Phone 8.1 Silverlight 앱을 대상으로 합니다. 대신 Windows Phone 8.1 스토어 앱을 빌드하는 경우 이 자습서의 [Windows 스토어 앱](mobile-services-dotnet-backend-windows-store-dotnet-get-started-push) 버전을 참조하십시오. Windows Phone Silverlight 앱에 대한 자세한 내용과 Windows Phone 스토어 앱과의 비교 결과를 확인하려면 [Windows Phone Silverlight 8.1 앱]을 참조하십시오.
...
3. **푸시** 탭을 클릭하고 **인증되지 않은 푸시 알림을 사용하도록 설정합니다.**를 선택한 다음 **저장**을 클릭합니다.
   	...
   	![모바일 서비스 푸시 탭](./media/mobile-services-dotnet-backend-windows-phone-get-started-push/mobile-push-tab.png)
...
5. 앱의 텍스트 상자에 "hello push"라는 텍스트를 입력하고 **저장**을 클릭한 후 시작 단추 또는 뒤로 단추를 바로 클릭하여 앱을 종료합니다.
	...
	![수신된 푸시 알림](./media/mobile-services-dotnet-backend-windows-phone-get-started-push/mobile-quickstart-push5-wp8.png)
...
이 자습서에서는 Windows Phone 앱에서 모바일 서비스와 알림 허브를 사용하여 푸시 알림을 보내도록 하기 위한 기본 사항을 알아보았습니다. 이후에는 다음 자습서인 [인증된 사용자에게 푸시 알림 보내기]를 완료하는 것이 좋습니다. 이 자습서에는 태그를 사용하여 모바일 서비스에서 인증된 사용자에게만 푸시 알림을 보내는 방법을 설명합니다.

다음의 모바일 서비스 및 알림 허브 항목에 대해서도 자세히 알아보세요.

* [앱에 인증 추가] <br/>모바일 서비스를 사용하여 서로 다른 계정 유형의 앱 사용자를 인증하는 방법에 대해 알아봅니다.
...
<!-- URLs. -->
[기존 앱에 모바일 서비스 추가]: mobile-services-dotnet-backend-windows-phone-get-started-data.md
[앱에 인증 추가]: mobile-services-dotnet-backend-windows-phone-get-started-users.md

[인증된 사용자에게 푸시 알림 보내기]: mobile-services-dotnet-backend-windows-phone-push-notifications-app-users.md
