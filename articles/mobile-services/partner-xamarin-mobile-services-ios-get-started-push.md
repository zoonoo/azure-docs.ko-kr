---
title: 모바일 서비스 앱에 푸시 알림 추가(Xamarin.iOS) - 모바일 서비스
...
> [!IMPORTANT]
> APNS 요구 사항 때문에 에뮬레이터 대신 iOS 지원 장치(iPhone 또는 iPad)에서 푸시 알림을 배포 및 테스트해야 합니다.
> 
> 

APNS는 인증서를 사용하여 모바일 서비스를 인증합니다. 필요한 인증서를 만들어 모바일 서비스에 업로드하려면 해당 지침을 따르세요. 공식 APNS 기능 설명서는 [Apple Push Notification Service](영문.md)를 참조하세요.
...
3. 삽입 함수를 다음의 코드로 바꾼 후 **저장**을 클릭합니다.
   ...
   > [!NOTE]
   > 이 스크립트는 앱을 닫고 알림 메시지를 수신할 수 있는 시간을 주기 위해 알림 전송을 지연시킵니다.
   > 
   > 
...
1. **실행** 단추를 눌러 프로젝트를 빌드하고 iOS 지원 장치에서 앱을 시작한 다음, **확인**을 클릭하여 푸시 알림을 수락합니다.
   ...
   > [!NOTE]
   > 앱에서 푸시 알림을 명시적으로 수락해야 합니다. This request only occurs the first time that the app runs.
   > 
   > 
