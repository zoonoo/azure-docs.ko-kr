---
title: Azure 모바일 서비스에 iOS 클라이언트 라이브러리를 사용하는 방법
...
> [!NOTE]
> 이 가이드에서는 최신 [iOS 모바일 서비스 SDK](https://go.microsoft.com/fwLink/?LinkID=266533&clcid=0x409)를 사용합니다. 프로젝트에서 이전 버전의 SDK를 사용하는 경우 먼저 Xcode에서 프레임워크를 업그레이드합니다.
> 
> 
...
다음 예에서는 [Live Connect SDK](영문.md)를 사용하여 iOS 앱용 Single Sign-On을 사용하도록 설정합니다. 이 코드에서는 컨트롤러에 `liveClient`(이)라는 **LiveConnectClient** 인스턴스가 있으며 사용자가 로그인한 상태임을 가정합니다.
...
[모바일 서비스 퀵 스타트] 프로젝트에서 토큰을 캐시하는 방법을 살펴보겠습니다. 다른 프로젝트에도 유사한 단계를 적용할 수 있습니다. [!INCLUDE [mobile-services-ios-authenticate-app-with-token](../../includes/mobile-services-ios-authenticate-app-with-token.md)]
