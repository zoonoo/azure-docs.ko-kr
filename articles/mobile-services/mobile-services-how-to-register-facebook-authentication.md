---
title: Facebook 인증 등록 - Azure 모바일 서비스
...
5. **App Secret** 필드에서 **Show**를 클릭하고 요청될 경우 암호를 제공한 다음 **App ID** 및 **App Secret** 값을 적어 둡니다. 나중에 이 값을 사용하여 Azure에서 응용 프로그램을 구성합니다.
   ...
   > [!NOTE]
   > **보안 정보** 앱 암호는 중요한 보안 자격 증명입니다. 다른 사람과 이 암호를 공유하거나 클라이언트 응용 프로그램 내에 배포하지 마세요.
   > 
   > 
6. 왼쪽 탐색 모음에서 **Settings**를 클릭하고, **App Domains**에 모바일 서비스의 도메인을 입력하고, 선택적 **Contact Email**을 입력하고, **Add Platform**을 클릭하고, **Website**를 선택합니다.
   
       ![][3]
7. **Site URL**에 모바일 서비스의 URL을 입력하고 **Save Changes**를 클릭합니다.
8. **Show**를 클릭하고 요청될 경우 암호를 제공한 후 **App ID** 및 **App Secret** 값을 기록해 둡니다.
   ...
   > 
   > 
9. **Advanced** 탭을 클릭하고, **Valid OAuth redirect URIs**에 다음 URL 형식 중 하나를 입력한 다음 **Save Changes**를 클릭합니다.
   ...
   * **JavaScript 백 엔드**: `https://<mobile_service>.azure-mobile.net/login/facebook`
     ...
     > [!NOTE]
     > *HTTPS* 체계를 사용하여 모바일 서비스 백 엔드 유형에 맞는 URL 리디렉션 경로 형식을 사용하고 있는지 확인합니다. 형식이 올바르지 않으면 인증이 실패합니다.
     > 
     > 
