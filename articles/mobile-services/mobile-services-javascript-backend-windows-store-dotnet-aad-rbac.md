...
2. `getAADToken` 함수를 정의하는 파일의 맨 위에 다음 스크립트를 추가합니다. *tenant\_domain*, 통합 응용 프로그램 *client id* 및 응용 프로그램 *key*가 제공된 경우 이 함수는 디렉터리 정보를 읽기 위해 사용되는 그래프 액세스 토큰을 제공합니다.
    ...
    >[AZURE.NOTE]각 액세스 확인 시 새 토큰을 만드는 대신 토큰을 캐시에 저장해야 합니다. 그런 후 [그래프 API 오류 참조]에 설명된 것처럼 401 Authentication\_ExpiredToken 응답에서 토큰 결과를 사용하려고 시도할 때 캐시를 새로 고칩니다. 단순성을 위해 이에 대해서는 아래 코드에 설명되지 않지만, Active Directory에 대한 네트워크 트래픽을 추가로 줄여줍니다.
