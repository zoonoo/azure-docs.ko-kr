---
title: 재해가 발생한 경우 모바일 서비스 복구 | Microsoft Docs
...
* **모바일 서비스 스크립트 백업** [Team Foundation Service] 또는 [GitHub] 등의 소스 제어 시스템에 모바일 서비스 스크립트를 저장하는 것이 좋으며 모바일 서비스 자체에만 복사본을 가지고 있는 것은 좋지 않습니다. 모바일 서비스 [소스 제어 기능](영문.md)을 사용하거나 [Azure CLI를 사용](영문.md)하여 Azure 클래식 포털을 통해 다운로드할 수 있습니다. Azure 클래식 포털에서 "preview"라는 레이블이 있는 기능은 특히 주의하십시오. 해당 스크립트의 경우 복구가 보장되지 않으므로 고유한 소스 제어 원본에서 스크립트를 복구해야 할 수 있습니다.
...
* **Azure 클래식 포털**에서 모바일 서비스 상태가 [비정상]으로 표시됩니다.
* Azure 클래식 포털에서 모바일 서비스의 모든 탭의 위쪽에 **비정상** 배너가 나타나고 관리 작업에서 오류 메시지가 발생합니다.
...
1. Azure 클래식 포털에서 서비스의 상태가 **비정상**으로 보고되는지 확인합니다.
...
4. 이제 보조 서비스를 사용하여 기본 서비스를 복구할 수 있습니다.
   ...
   > [!IMPORTANT]
   > 마이그레이션 명령은 파일 마이그레이션 외에도 클라이언트 응용 프로그램을 업데이트할 필요가 없게 기본 서비스의 호스트 이름을 보조 서비스를 가리키도록 업데이트합니다. 그러나 호스트 이름에서 새 서비스를 확인하는 데 최대 30분이 걸립니다. 따라서 마이그레이션 명령은 재해 복구 시나리오에서만 사용하는 것이 좋습니다.
   > 
   > [!IMPORTANT]
   > 이 단계에서 명령을 실행하면 보조 서비스가 삭제되어 해당 용량을 기본 서비스 복구에 사용할 수 있게 됩니다. 스크립트 및 설정을 유지하려는 경우에는 명령을 실행하기 전에 백업하는 것이 좋습니다.
   > 
   > 
   ...
   > [!NOTE]
   > 명령이 완료된 후 Azure 클래식 포털에서 변경 내용이 표시될 때까지 몇 분 정도 걸릴 수 있습니다.
   > 
   > 
...
7. 이제 모바일 서비스가 이제 다른 물리적 위치에서 호스트됩니다. 실행 중인 사이트의 업데이트를 허용하려면 게시 및/또는 git 자격 증명을 업데이트해야 합니다.
   ...
   * **Javascript 백 엔드**를 사용하고 노드에서 서비스를 관리하는 경우에는 새 저장소를 가리키도록 git 원격을 업데이트합니다. 이렇게 하려면 git 원격에서 .git 파일 경로를 제거합니다.
     ...
     2. 최종 .git 파일 경로 없이 동일한 URL을 사용하여 원격을 업데이트합니다. git remote set-url origin https://myservice.scm.azure-mobile.net
     3. 원본에서 끌어와 올바르게 작동하는지 확인합니다.
