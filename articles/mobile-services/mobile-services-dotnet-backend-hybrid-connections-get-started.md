---
title: 하이브리드 연결을 사용하여 .NET 백 엔드 모바일 서비스에서 온-프레미스 SQL Server에 연결 | Microsoft Docs
...
5. **DbContext** 인스턴스 생성자를 변형하여 다음 코드 조각과 비슷한 기본 **DbContext** 생성자에 `OnPremisesDBConnection` 값을 전달합니다.
...
1. **저장**을 눌러 하이브리드 연결과 방금 만든 연결 문자열을 저장합니다.
2. Visual Studio를 사용하여 업데이트된 모바일 서비스 프로젝트를 Azure에 게시합니다.
   
    서비스 시작 페이지가 표시됩니다.
3. 시작 페이지에서 이전처럼 **지금 시도** 단추를 사용하거나 모바일 서비스에 연결된 클라이언트 앱을 사용하여 데이터베이스 변경 내용을 생성하는 일부 작업을 호출합니다.
   
   > [!NOTE]
   > API 도움말 페이지를 시작하기 위해 **지금 시도** 단추를 사용하는 경우 사용자의 응용 프로그램 키를 암호(빈 사용자 이름)로 제공해야 합니다.
   > 
   > 
