...
2. 탐색 창 아래에서 **+새로 만들기**를 선택한 다음 **앱 서비스**, **BizTalk Service**, **사용자 지정 만들기**를 차례로 선택합니다.

	![BizTalk 서비스 만들기][CreateBTS]

3. **BizTalk 서비스 이름**을 입력하고 **버전**을 선택합니다.

	![새로운 BizTalk 서비스 구성][ConfigureBTS]
...
1. 포털의 **모바일 서비스** 탭에서 기존 모바일 서비스를 선택하거나 새로운 서비스를 만듭니다. 
	...
	>[AZURE.NOTE].NET 백 엔드를 사용하여 생성된 서비스를 선택하거나 새 .NET 백 엔드 모바일 서비스를 만들어야 합니다. 새 .NET 백 엔드 모바일 서비스를 만드는 방법에 대한 자세한 내용은 [모바일 서비스 시작하기](mobile-services-dotnet-backend-windows-store-dotnet-get-started.md)를 참조하세요.

2. 모바일 서비스의 **구성** 탭에서 **하이브리드 연결** 섹션을 찾아 **하이브리드 연결 추가**를 선택합니다.

	![하이브리드 연결 연계][AssociateHC]

3. BizTalk 서비스 탭에서 방금 만든 하이브리드 연결을 선택하고 **확인**을 누릅니다.

	![연계된 하이브리드 연결 선택][PickHC]
...
1. **연결 문자열**의 **구성** 탭에서 `Server=onPremisesServer,1433;Database=OnPremisesDB;User ID=sa;Password={password}`와(과) 같은 값을 가지며 이름이 `OnPremisesDatabase`인 새 연결 문자열을 추가합니다.

	![온-프레미스 데이터베이스에 대한 연결 문자열][ConnectionString]

	온-프레미스 데이터베이스의 보안 암호로 `{password}`을(를) 바꿉니다.
