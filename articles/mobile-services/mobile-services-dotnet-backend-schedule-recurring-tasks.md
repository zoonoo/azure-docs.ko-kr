...
4. [Azure 클래식 포털]에서 모바일 서비스를 클릭한 후 앱을 클릭합니다.
...
4. (선택 사항) [Azure 클래식 포털]에서 모바일 서비스에 연결된 데이터베이스에 대한 관리를 클릭합니다.

    ![][6]

5. Azure 클래식 포털에서 앱에서 변경한 내용을 표시하는 쿼리를 실행합니다. 쿼리는 다음 쿼리와 비슷하지만 모바일 서비스 이름을 `todolist` 대신 스키마 이름으로 사용합니다.
