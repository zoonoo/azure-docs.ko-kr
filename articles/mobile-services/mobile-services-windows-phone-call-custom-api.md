...
[기존 앱에 모바일 서비스 추가](mobile-services-windows-phone-get-started-data.md) 자습서를 완료할 때 만든 앱에 이 기능을 추가합니다. 이 자습서는 간단한 TodoList 앱인 GetStartedWithData 샘플을 기반으로 합니다. 이 자습서를 시작하기 전에 먼저 [기존 앱에 모바일 서비스 추가](mobile-services-windows-phone-get-started-data.md)를 완료해야 합니다.
