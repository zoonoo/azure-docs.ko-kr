...
> [AZURE.NOTE]이 자습서는 Xamarin.Android 앱에서 모바일 서비스를 통해 Azure를 사용하여 데이터를 저장하고 검색할 수 있는 방법을 더욱 잘 이해할 수 있도록 돕기 위한 것입니다. 이 항목에서는 모바일 서비스 퀵 스타트에서 완료한 다수의 단계를 순서대로 안내합니다. 모바일 서비스를 처음 사용하는 경우 먼저 [모바일 서비스 시작](/develop/mobile/tutorials/get-started-xamarin-android) 자습서를 완료하는 것이 좋습니다.
...
2. **AndroidManifest.xml** 파일을 열고 다음 사용 권한 줄이 있는지 확인합니다.
  	...
  	이 경우 앱이 Azure의 모바일 서비스에 액세스할 수 있습니다.
...
5. 이전 단계가 완료되면 프로젝트에서 빌드 오류가 표시됩니다. `todoItemList` 변수가 사용되는 나머지 세 위치를 검색하여 표시된 섹션을 주석으로 처리합니다.
...
12. 파일 아래쪽에서 ProgressFilter 클래스를 찾아 주석 처리를 제거합니다. 이 클래스는 MobileServiceClient에서 네트워크 작업을 실행하는 동안 '로드' 표시기를 표시합니다.
...
[스크립트를 사용하여 데이터 유효성 검사 및 수정]: /develop/mobile/tutorials/validate-modify-and-augment-data-xamarin-android
[페이징을 사용하여 쿼리 구체화]: /develop/mobile/tutorials/add-paging-to-data-xamarin-android
