---
title: SQL 데이터베이스 대신 테이블 저장소를 사용하는 .NET 백 엔드 모바일 서비스 빌드 | Microsoft Docs
...
1. 다음과 같이 **EntityData** 대신 **StorageData**에서 파생되도록 **TodoItem** 데이터를 수정합니다.
   ...
   > [!NOTE]
   > **StorageData** 형식에는 *partitionId*,*rowValue* 형식의 문자열인 복합 키가 필요한 Id 속성이 있습니다.
   > 
   > 
...
4. 기존 **GetAllTodoItems** 메서드를 다음 코드로 바꿉니다.
...
2. 모바일 서비스에 연결된 빠른 시작 클라이언트 앱을 실행합니다.
   
    빠른 시작 자습서를 사용하여 이전에 추가한 항목이 표시되지 않습니다. 이는 테이블 저장소가 현재 비어 있기 때문입니다.
3. 새 항목을 추가하여 데이터베이스 변경 내용을 생성합니다.
