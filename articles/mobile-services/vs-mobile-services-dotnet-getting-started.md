---
title: Visual Studio .NET 모바일 서비스 프로젝트 시작하기(연결 서비스) | Microsoft Docs
...
## 테이블에 대한 참조 가져오기
다음 코드는 TodoItem에 대한 데이터를 포함하는 테이블(`todoTable`)에 대한 참조를 만듭니다. 이 참조는 데이터 테이블을 읽고 업데이트하는 후속 작업에서 사용할 수 있습니다. 모바일 서비스가 쿼리에 응답하여 보내는 JSON을 해석하려면 특성이 설정된 TodoItem 클래스가 필요합니다.
