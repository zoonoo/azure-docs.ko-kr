...
이 자습서에서는 Azure Data Factory를 사용하여 Azure SQL Edge 인스턴스의 테이블에서 Azure Blob 스토리지로 데이터를 증분 방식으로 동기화합니다.
...
7. **연결된 서비스** 에 대해 **새로 만들기** 를 선택하고, 다음 단계를 수행합니다.
    ...
    2. **서버 이름** 아래에서 SQL Edge 서버 세부 정보를 입력합니다.

    3. 목록에서 **데이터베이스 이름** 을 선택합니다.

    4. **사용자 이름** 및 **암호** 를 입력합니다.

    5. SQL Edge 인스턴스에 대한 연결을 테스트하려면 **연결 테스트** 를 선택합니다.
...
13. **새 데이터 세트** 창에서 **SQL Edge 인스턴스**, **계속** 을 차례로 선택합니다.
...
31. **SQL 계정** 탭으로 전환하고 **연결된 서비스** 아래에서 **_QLDBEdgeLinkedService_* 를 선택합니다.
...
이 자습서의 Azure Data Factory 파이프라인은 데이터를 SQL Edge 인스턴스의 테이블에서 Azure Blob 스토리지의 위치로 1시간마다 한 번씩 복사합니다. 다른 시나리오에서 Data Factory를 사용하는 방법을 알아보려면 [이 자습서](../data-factory/tutorial-copy-data-portal.md)를 참조하세요.
