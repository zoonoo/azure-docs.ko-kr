...
* [모바일 서비스용 오프라인 데이터 동기화] 기능을 사용할 때 클라이언트 SDK는 삭제된 레코드를 자동으로 쿼리하여 로컬 데이터베이스에서 제거합니다. 일시 삭제를 사용하도록 설정하지 않으면 클라이언트 SDK가 로컬 저장소에서 제거할 레코드를 확인할 수 있도록 백 엔드에서 추가 코드를 작성해야 합니다. 그렇지 않으면 이처럼 삭제된 레코드와 관련하여 클라이언트 로컬 저장소와 백 엔드가 일치하지 않게 되므로 클라이언트 메서드 `PurgeAsync()`를 호출하여 로컬 저장소를 정리해야 합니다.
* 일부 응용 프로그램에서는 업무 요구 사항에 따라 실제로 데이터를 삭제할 수 없거나 데이터를 감사한 후에만 삭제해야 합니다. 이러한 시나리오에서는 일시 삭제 기능이 유용할 수 있습니다.
* 일시 삭제를 사용하여 "삭제 취소" 기능을 구현할 수 있으므로 실수로 삭제된 데이터를 복구할 수 있습니다.
그러나 일시 삭제된 레코드는 데이터베이스의 공간을 차지하므로 일시 삭제된 레코드를 정기적으로 영구 삭제하는 예약된 작업을 만들어야 합니다. 이러한 작업의 예제는 [.NET 백 엔드에서 일시 삭제 사용] 및 [JavaScript 백 엔드에서 일시 삭제 사용].을 참조하세요. 또한 이와 같이 영구 삭제된 레코드가 장치의 로컬 데이터 저장소에 남아 있지 않도록 클라이언트 코드가 `PurgeAsync()`를 정기적으로 호출해야 합니다.



항목 개요

1. [.NET 백 엔드에 대해 일시 삭제를 사용하도록 설정]
2. [JavaScript 백 엔드에 대해 일시 삭제를 사용하도록 설정]
3. [.NET 백 엔드에서 일시 삭제 사용]
4. [JavaScript 백 엔드에서 일시 삭제 사용]
...
3. Visual Studio의 솔루션 탐색기에서 .NET 백 엔드 프로젝트 아래의 **컨트롤러** 노드를 확장하고 컨트롤러 소스를 엽니다. 예를 들면  *TodoItemController.cs*과 같습니다.
...
.NET 백 엔드 모바일 서비스의 예약된 작업에 대한 자세한 내용은 다음 항목을 참조하세요. [JavaScript 백 엔드 모바일 서비스와 되풀이 작업 예약](/ko-kr/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/) 
...
JavaScript 백 엔드 모바일 서비스의 예약된 작업에 대한 자세한 내용은 [JavaScript 백 엔드 모바일 서비스와 되풀이 작업 예약]/ko-kr/documentation/articles/mobile-services-schedule-recurring-tasks/)을 참조하세요.
