...
Azure Cloud Shell 세션을 제공하는 컴퓨터는 일시적이며 세션이 20분 동안 비활성화된 후 재순환됩니다. Azure Cloud Shell에 파일 공유를 탑재해야합니다. 따라서 Azure Cloud Shell에 액세스하도록 구독에서 저장소 리소스를 설정할 수 있어야합니다. 기타 고려 사항은 다음과 같습니다.
...
Azure Cloud Shell은 Microsoft Edge, Microsoft Internet Explorer, Google Chrome, Mozilla Firefox 및 Apple Safari의 최신 버전을 지원합니다. Safari는 개인 모드에서 지원되지 않습니다.
...
Azure Cloud Shell은 대화형 사용 사례를 위한 것입니다. 따라서 비대화형 세션을 오래 실행하면 경고 없이 종료됩니다.
...
.bashrc를 편집할 때는 Azure Cloud Shell에 예기치 않은 오류가 발생할 수 있으니 주의하세요. 

### <a name="bashhistory"></a>.bash_history
Azure Cloud Shell 세션 중단 또는 동시 세션으로 인해 bash 명령의 기록이 일관되지 않을 수 있습니다.
...
응용 프로그램(예: git, vim 및 기타)이 $Home에 기록한 데이터는 PowerShell 세션 간에 지속되지 않습니다.  해결 방법은 [여기를 참조하세요](troubleshooting.md#powershell-resolutions).
