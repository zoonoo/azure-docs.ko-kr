...
Azure Portal을 통해 Cloud Shell 아이콘에서 시작합니다.

![포털 시작](media/overview/portal-launch-icon.png)

셸 선택기 드롭다운에서 Bash 또는 PowerShell을 활용합니다.
...
Linux 사용자는 Cloud Shell의 Bash를, Windows 사용자는 Cloud Shell의 PowerShell(미리 보기)을 선택할 수 있습니다. 

### <a name="authenticated-and-configured-azure-workstation"></a>인증 및 구성된 Azure 워크스테이션
Cloud Shell은 Microsoft에서 관리되므로 널리 사용되는 명령줄 도구 및 언어 지원이 미리 설치되어 더 빠른 작업이 가능합니다. 또한 Cloud Shell은 Azure CLI 2.0 또는 Azure PowerShell cmdlet을 통해 리소스에 즉시 액세스하도록 자동으로 안전하게 인증합니다.

[Bash 환경](features.md#tools) 및 [PowerShell(미리 보기) 환경](features-powershell.md#tools)에 대한 전체 도구 목록을 확인합니다.

### <a name="multiple-access-points"></a>여러 액세스 포인트
Cloud Shell은 Azure Portal뿐 아니라 여러 방법으로 액세스할 수 있습니다.
* [Azure CLI 2.0 "실행" 문서](https://docs.microsoft.com/cli/azure/overview?view=azure-cli-latest)
...
[Azure Cloud Shell의 Bash](quickstart.md) 및 [Azure Cloud Shell의 PowerShell(미리 보기)](quickstart-powershell.md)에 대한 빠른 시작에 들어 있는 이런 예제를 시도해 봅니다.
