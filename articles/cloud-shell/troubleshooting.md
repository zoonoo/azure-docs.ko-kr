...
### <a name="error-running-azuread-cmdlets-in-powershell"></a>PowerShell에서 AzureAD cmdlet을 실행하는 오류

- **세부 정보**: 클라우드 셸과 같이 `Get-AzureADUser` AzureAD cmdlet을 `You must call the Connect-AzureAD cmdlet before calling any other cmdlets`실행하면 오류가 표시될 수 있습니다. 
- **해상도**: `Connect-AzureAD` cmdlet을 실행합니다. 이전에는 PowerShell을 시작할 때 클라우드 셸이 이 cmdlet을 자동으로 실행했습니다. 시작 시간을 단축하기 위해 cmdlet이 더 이상 자동으로 실행되지 않습니다. PowerShell의 $PROFILE 파일에 추가하여 `Connect-AzureAD` 이전 동작을 복원하도록 선택할 수 있습니다.
...
- **세부 정보**: 관리자가 사용자를 위해 Cloud Shell에 대한 액세스를 사용하지 않도록 설정할 수 있습니다. Cloud Shell은 거부될 `ux.console.azure.com` 수 있는 도메인에 대한 액세스를 활용하여 portal.azure.com, shell.azure.com, Visual Studio Code Azure 계정 확장 및 docs.microsoft.com 비롯한 Cloud Shell의 진입점에 대한 액세스를 중지합니다. 미국 정부 클라우드에서 `ux.console.azure.us`진입점은; 해당 shell.azure.us 없습니다.
- **해결 방법**: `ux.console.azure.com` `ux.console.azure.us` 네트워크 설정을 통해 환경에 대한 액세스를 제한합니다. Cloud Shell 아이콘은 Azure 포털에 계속 존재하지만 서비스에 성공적으로 연결되지는 않습니다.

### <a name="storage-dialog---error-403-requestdisallowedbypolicy"></a>스토리지 대화 상자 - 오류: 403 RequestDisallowedByPolicy

- **세부 정보**: Cloud Shell을 통해 저장소 계정을 만들 때 관리자가 배치한 Azure 정책으로 인해 저장소 계정을 만들지 못합니다. 오류 메시지에는 다음이 포함됩니다.`The resource action 'Microsoft.Storage/storageAccounts/write' is disallowed by one or more policies.`
...
### <a name="set-your-cloud-shell-connection-to-support-using-tls-12"></a>TLS 1.2를 사용하여 지원을 위한 Cloud Shell 연결 설정
 - **세부 정보**: Cloud Shell연결에 대한 TLS 버전을 정의하려면 브라우저별 설정을 설정해야 합니다.
...
Azure Cloud Shell에는 지역당 테넌트당 동시 사용자 20개로 제한됩니다. 제한보다 더 많은 동시 세션을 열려고 하면 "할당량 초과 테넌트 사용자" 오류가 표시됩니다. 이보다 더 많은 세션을 열어야 하는 합법적인 경우(예: 교육 세션) 예상 사용 전에 지원팀에 문의하여 할당량 증가를 요청하세요.

Cloud Shell은 무료 서비스로 제공되며 범용 컴퓨팅 플랫폼이 아닌 Azure 환경을 구성하는 데 사용되도록 설계되었습니다. 과도한 자동 사용은 Azure 서비스 약관을 위반하는 것으로 간주될 수 있으며 클라우드 셸 액세스가 차단될 수 있습니다.
...
PowerShell:
  ...
  ```
## <a name="azure-government-limitations"></a>Azure 정부 제한 사항
Azure 정부의 Azure 클라우드 셸은 Azure 포털을 통해서만 액세스할 수 있습니다.

>[!Note]
> 현재 교환 온라인을 위한 GCC-High 또는 정부 DoD 클라우드에 연결하는 것은 지원되지 않습니다.
