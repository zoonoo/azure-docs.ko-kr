...
이 섹션에서는 샘플에 있는 *device-connectivity*라는 프로젝트를 사용합니다. 동작, 온도 및 이산화탄소를 감지하는 센서 데이터를 시뮬레이션할 것입니다. 이 프로젝트는 센서에 대한 임의의 값을 생성하고, 디바이스 연결 문자열을 사용하여 IoT 허브로 보냅니다.

1. 별도의 명령 창에서 Azure Digital Twins 샘플로 이동한 다음, **device-connectivity** 폴더로 이동합니다.
...
1. 다음 명령을 실행하여 온도, 동작 및 이산화탄소에 대한 디바이스 이벤트 시뮬레이션을 시작합니다.
