...
장치에서 원격 분석 데이터를 Digital Twins에 보내면 개발자는 ‘유효성 검사’, ‘일치’, ‘계산’, ‘디스패치’의 네 개 단계로 데이터를 처리할 수 있습니다.
...
Azure Digital Twins의 데이터 처리는 ‘검사기’, ‘사용자 정의 함수’, ‘역할 할당’의 세 개 개체로 구성됩니다.

![Digital Twins 데이터 처리 흐름][2]
...
‘사용자 정의 함수’ 또는 _UDF_는 Azure Digital Twins의 격리된 환경 내에서 실행되는 사용자 지정 함수입니다. UDF는 수신된 원시 센서 원격 분석 메시지와 공간 그래프 및 디스패처 서비스에 모두 액세스할 수 있습니다. UDF가 그래프 내에 등록되면, UDF를 실행할 시기를 지정하려면 검사기(위에서 자세히 설명)를 만들어야 합니다. Digital Twins가 지정 센서에서 새 원격 분석을 수신하는 경우 일치하는 UDF는 마지막 몇 개의 센서 값에 대한 이동 평균을 계산할 수 있습니다.

UDF는 JavaScript로 작성할 수 있으며 개발자가 센서 원격 분석 메시지에 대해 사용자 지정 코드 조각을 실행할 수 있게 합니다. 사용자 정의 실행 환경에서 그래프와 상호 작용하는 도우미 메서드도 있습니다. UDF를 사용하면 개발자는 다음을 수행할 수 있습니다.
...
자세한 내용은 [How to Use User-Defined Functions](how-to-user-defined-functions.md)(사용자 정의 함수를 사용하는 방법)를 참조하세요.

### <a name="role-assignment"></a>역할 할당

UDF의 작업은 서비스 내의 데이터를 보호하는 Digital Twins의 역할 기반 액세스 제어에 따릅니다. 역할 할당은 지정된 UDF에 공간 그래프와 상호 작용할 수 있는 적절한 권한이 있는지 확인합니다. 예를 들어, UDF는 지정된 공간에서 그래프 데이터를 생성, 읽기, 업데이트 또는 삭제할 수 있습니다. UDF의 액세스 수준은 UDF가 데이터에 대한 그래프를 요청하거나 작업을 시도할 때 검사됩니다. 자세한 내용은 [역할 기반 액세스 제어](security-create-manage-role-assignments.md)를 참조하세요.

검사기가 역할 할당이 없는 UDF를 트리거할 수 있습니다. 이 경우 UDF는 그래프에서 데이터를 읽을 수 없습니다.

## <a name="next-steps"></a>다음 단계

이벤트 및 원격 분석 메시지를 다른 Azure 서비스로 라우팅하는 방법에 대한 자세한 내용은 [이벤트 라우팅 및 메시지](concepts-events-routing.md)를 참조하세요.
