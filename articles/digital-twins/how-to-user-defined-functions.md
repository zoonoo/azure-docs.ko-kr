...
**원격 분석** 매개 변수는 센서가 보낸 메시지에 해당 하는 **SensorId** 및 **메시지** 특성을 노출 합니다. **executionContext** 매개 변수는 다음 특성을 노출합니다.
...
1. 모든 역할에 대한 [시스템 API를 쿼리](./security-create-manage-role-assignments.md#retrieve-all-roles)하여 사용자 정의 함수에 할당할 역할 ID를 가져옵니다. 인증된 HTTP GET 요청을 수행함으로써 할 수 있습니다.
    ...
    ```
   원하는 역할 ID를 유지합니다. JSON body 특성 **roleId** (`YOUR_DESIRED_ROLE_IDENTIFIER`)로 전달 됩니다.

1. **objectId** (`YOUR_USER_DEFINED_FUNCTION_ID`)는 앞에서 만든 사용자 정의 함수 ID입니다.
1. 를 사용 `fullpath`하 여 공백을 쿼리하여`YOUR_ACCESS_CONTROL_PATH` **path** ()의 값을 찾습니다.
...
1. 인증된 HTTP POST 요청을 수행하여 반환된 `spacePaths` 값을 **경로**에 붙여넣고 사용자 정의 함수 역할 할당을 만듭니다.
    ...
    | YOUR_USER_DEFINED_FUNCTION_TYPE_ID | 사용자 정의 함수 유형 ()을 지정 하는`UserDefinedFunctionId`ID입니다. |
