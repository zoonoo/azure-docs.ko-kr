...
Azure Digital Twins는 물리적 환경 및 환경과 연결된 장치, 센서, 사람의 포괄적인 가상 표현을 가능하게 해주는 Azure IoT 서비스입니다. 도메인별 개념을 유용한 모델로 구성하여 개발을 개선합니다. 그런 다음, 모델은 공간 인텔리전스 그래프 내의 배치합니다. 이러한 개념은 사람, 공간, 장치 간의 관계와 상호 작용을 정확하게 모델링합니다.
...
공간 그래프는 공간, 장치, 센서 및 사용자를 하나로 합칩니다. 각각은 실제 세계를 모델링하는 방식으로 함께 연결됩니다. 이 샘플에서 장소 43은 4개의 층으로 구성되어 있으며 각각 다른 영역이 있습니다. 사용자는 자신의 워크스테이션과 연결되어 있고 그래프 일부에 대한 액세스 권한이 부여됩니다. 관리자는 공간 그래프를 변경하는 권한이 있는 반면 방문자는 특정 건물 데이터만 볼 수 있는 권한이 있습니다.
...
**그래프 확장성**. Digital Twins는 그래프 확장성을 보장하므로 실제 워크로드를 처리할 수 있습니다. Digital Twins를 사용하여 부동산, 인프라, 장치, 센서, 원격 분석 등의 대규모 포트폴리오를 나타낼 수 있습니다.
