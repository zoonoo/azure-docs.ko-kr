---
title: 보안 모범 사례 이해 - Azure 디지털 트윈 | 마이크로 소프트 문서
description: Azure 디지털 트윈 및 사물 인터넷에 대한 보안 모범 사례에 대해 알아봅니다.
ms.author: alinast
...
Azure Digital Twins 보안을 사용하면 IoT 그래프에서 특정 리소스 및 작업에 대한 액세스 권한을 세밀하게 지정할 수 있습니다. 역할 [기반 액세스 제어라는](./security-role-based-access-control.md)세분화된 역할 및 권한 관리를 통해 이렇게 합니다.
...
> [!TIP]
> [IoT용 Azure 보안 센터를](https://docs.microsoft.com/azure/asc-for-iot/) 사용하여 IoT 보안 위협 및 취약점을 검색할 수 있습니다.
...
> * 장치 펌웨어, 운영 체제 및 소프트웨어를 최신 상태로 유지합니다.
> * 장치, 소프트웨어, 네트워크 및 게이트웨이 보안 모범 사례를 지속적으로 개선하고 발전함에 따라 주기적으로 감사 및 검토합니다.
> * 신뢰할 수 있고 인증된 규정을 준수하는 보안 시스템, 소프트웨어 및 장치를 사용합니다. 예를 들어 Azure 클라우드에 대한 [규정 준수 제품을 검토합니다.](https://azure.microsoft.com/overview/trusted-cloud/compliance/)
...
> * 역할에 따라 액세스 및 사용 권한을 신중하게 제한합니다. 아래의 [역할 기반 액세스 제어 모범 사례](#role-based-access-control-best-practices) 섹션을 읽어보십시오.
> * 각 네트워크의 장치가 다른 네트워크와 격리되도록 분할된 네트워크 토폴로지도 고려합니다.
> * 강력한 암호화를 사용합니다. 긴 암호가 필요하고, 보안 프로토콜을 사용하고, [다단계 인증을](https://docs.microsoft.com/azure/active-directory/authentication/concept-mfa-howitworks)사용합니다.
...
> [!IMPORTANT]
> 포괄적인 [IoT 보안](../iot-fundamentals/iot-security-best-practices.md) 전략을 시작하려면 Azure IoT 보안 모범 사례를 읽어보십시오.

> [!NOTE]
> 이벤트 처리 및 모니터링에 대한 자세한 내용은 [Azure Digital Twins를 사용 하](./concepts-events-routing.md)여 경로 이벤트 및 메시지를 참조하십시오.

## <a name="azure-active-directory-best-practices"></a>Azure Active Directory 모범 사례

Azure 디지털 Twins는 [Azure Active Directory를](https://docs.microsoft.com/azure/active-directory/authentication/) 사용하여 사용자를 인증하고 응용 프로그램을 보호합니다. Azure Active Directory는 다양한 최신 아키텍처의 인증을 지원합니다. 모두 OAuth 2.0 또는 OpenID Connect와 같은 업계 표준 프로토콜을 기반으로 합니다. Azure Active Directory의 IoT 영역을 보호하는 몇 가지 주요 사례는 다음과 같습니다.
...
> * 역할 기반 액세스 제어 모범 사례에 따라 사용되지 않는 **리디렉션 URI** 및 사용 권한을 [제거합니다.](#role-based-access-control-best-practices)
