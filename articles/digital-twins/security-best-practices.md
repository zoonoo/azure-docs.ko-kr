...
Azure Digital Twins는 Azure AD(Azure Active Directory)를 비롯하여 Azure IoT에 있는 다른 보안 기능도 사용합니다. 이러한 이유로 Azure Digital Twins 앱을 구성할 때는 현재 권장되는 많은 [Azure IoT 보안 사례](https://docs.microsoft.com/azure/iot-fundamentals/iot-security-best-practices?context=azure/iot-hub/)를 사용하게 됩니다.
...
> * 역할에 따라 액세스 및 사용 권한을 신중하게 제한합니다. “역할 기반 액세스 제어 모범 사례” 섹션을 참조하세요.
> * 강력한 암호화를 사용합니다. 긴 암호가 필요하고, 보안 프로토콜 및 2단계 인증을 사용합니다.

IoT 리소스를 모니터링하여 일반적인 작업 범위를 벗어나는 이상값, 위협 또는 리소스 매개 변수를 감시합니다. Azure Analytics를 사용하여 모니터링을 관리합니다.
...
Azure Digital Twins는 Azure AD를 사용하여 사용자를 인증하고 애플리케이션을 보호합니다. Azure AD는 다양한 최신 아키텍처의 인증을 지원합니다. 모두 OAuth 2.0 또는 OpenID Connect와 같은 업계 표준 프로토콜을 기반으로 합니다. Azure AD의 IoT 영역을 보호하는 몇 가지 주요 사례는 다음과 같습니다.

> [!div class="checklist"]
> * Azure AD 앱 비밀 및 키를 [Azure Key Vault](https://azure.microsoft.com/services/key-vault/)와 같은 안전한 위치에 저장합니다.
