...
API 관리(미리 보기)에서, 새 API를 만들고 작업을 수동으로 추가하거나 API를 작업과 함께 한번에 가져올 수 있습니다.
...
이 가이드에서는 새로운 API를 만들고 그 작업과 함께 한번에 가져오는 방법을 보여 줍니다.

> API를 수동으로 만들고 작업을 추가하는 방법에 대해서는 [API를 만드는 방법][API를 만드는 방법] 및 [API에 작업을 추가하는 방법][API에 작업을 추가하는 방법]을 참조하세요.
...
새 API를 가져올 뿐만 아니라 관리 콘솔에서 API에 대한 정의를 내보낼 수 있습니다. 그렇게 하려면 **API**의 **요약 탭**에서 **API 내보내기**를 클릭합니다.

![API 내보내기][2]

API는 WADL 또는 Swagger를 사용하여 내보낼 수 있습니다. 원하는 형식을 선택하고, **저장**을 클릭하고, 파일을 저장할 위치를 선택합니다.

![API 내보내기 형식][API 내보내기 형식]
...
[관리 콘솔]: ./media/api-management-howto-import-api/api-management-management-console.png
[1]: ./media/api-management-howto-import-api/api-management-api-import-apis.png
[API 가져오기 형식]: ./media/api-management-howto-import-api/api-management-import-api-wizard.png
