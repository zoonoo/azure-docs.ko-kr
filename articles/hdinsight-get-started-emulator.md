<properties urlDisplayName="Get Started" pageTitle="HDInsight Emulator 시작" metaKeywords="hdinsight, Azure hdinsight, hdinsight azure, get started hdinsight, emulator, hdinsight emulator" description="HDInsight Emulator for Azure를 사용하는 방법에 대해 알아봅니다." umbracoNaviHide="0" disqusComments="1" editor="cgronlun" manager="paulettm" services="hdinsight" title="HDInsight Emulator 시작" author="nitinme" />
...
이 자습서에서는 Microsoft HDInsight Emulator for Azure(이전의 HDInsight Server Developer Preview)에서 Hadoop 클러스터를 시작하는 방법을 설명합니다. HDInsight Emulator에는 Azure HDInsight와 동일한 Hadoop 에코시스템의 구성 요소가 제공됩니다. 배포된 버전에 대한 정보를 포함한 자세한 내용은 [Azure HDInsight에 포함된 Hadoop 버전][Azure HDInsight에 포함된 Hadoop 버전](영문)을 참조하세요.

> [WACOM.NOTE] HDInsight Emulator는 Hadoop 클러스터만 포함하며 HBase는 포함하지 않습니다.
...
-   HDInsight Emulator에는 64비트 버전의 Windows가 필요합니다. 다음 요구 사항 중 하나는 반드시 충족되어야 합니다.
    ...
    -   Windows 8
    -   Windows Server 2012
-   Azure PowerShell을 설치하고 구성합니다. 자세한 내용은 [Azure PowerShell 설치 및 구성][Azure PowerShell 설치 및 구성]을 참조하세요.

## 자습서 내용

-   [HDInsight Emulator 설치][HDInsight Emulator 설치]
-   [단어 계산 샘플 실행][단어 계산 샘플 실행]
-   [시작 샘플 실행][시작 샘플 실행]
-   [Azure Blob 저장소에 연결][Azure Blob 저장소에 연결]
-   [HDInsight PowerShell 실행][HDInsight PowerShell 실행]
-   [다음 단계][다음 단계]
...
> [WACOM.NOTE] HDInsight Emulator는 현재 영어 OS만 지원합니다. 이전 버전의 Emulator가 설치되어 있으면 최신 버전의 Emulator를 설치하기 전에 제어판/프로그램 및 기능에서 다음 두 가지 구성 요소를 제거해야 합니다.
>
> -   Microsoft HDInsight Emulator for Microsoft Azure 또는 HDInsight Developer Preview 중 설치되어 있는 구성 요소
...
1.  Internet Explorer을 열고 [Microsoft HDInsight Emulator for Azure 설치 페이지][Microsoft HDInsight Emulator for Azure 설치 페이지]로 이동합니다.
2.  **지금 설치**를 클릭합니다.
...
7.  웹 플랫폼 설치 관리자에서 **다음 제품이 설치되었습니다.** 상태가 표시되는지 확인한 후 **마침**을 클릭합니다.
8.  **끝내기**를 클릭하여 웹 플랫폼 설치 관리자 창을 닫습니다.
...
HDInsight Emulator와 관련된 서비스는 기본적으로 시작되지 않습니다. 서비스를 시작하려면 Hadoop 명령줄을 사용하여 <system drive\>\\hdp에서 **start\_local\_hdp\_services.cmd**를 실행합니다. 컴퓨터를 다시 시작한 후 서비스를 자동으로 시작하려면**set-onebox-autostart.cmd**를 실행합니다.

HDInsight Server 설치 및 실행에 대해 알려진 문제는 [HDInsight Emulator 릴리스 정보][HDInsight Emulator 릴리스 정보]를 참조하세요. 설치 로그는 **C:\\HadoopFeaturePackSetup\\HadoopFeaturePackSetupTools\\gettingStarted.winpkg.install.log**에 있습니다.

## <a name="runwordcount"></a>단어 계산 MapReduce 작업 실행

이제 워크스테이션에 HDInsight Emulator를 구성했으므로 MapReduce 작업을 실행하여 설치를 테스트할 수 있습니다. 먼저 일부 데이터 파일을 HDFS에 업로드한 후 단어 계산 MapReduce 작업을 실행하여 해당 파일에서 특정 단어의 빈도를 계산합니다.

단어 계산 MapReduce 프로그램은 *hadoop-mapreduce-examples-2.4.0.SNAPSHOT.jar*에 패키지되어 있습니다. jar 파일은 *C:\\hdp\\hadoop-2.4.0.SNAPSHOT\\share\\hadoop\\mapreduce* 폴더에 있습니다.

단어 계산 MapReduce 작업은 다음의 두 인수를 사용합니다.

-   입력 폴더. 입력 폴더로는 *hdfs://localhost/user/HDIUser*를 사용합니다.
-   출력 폴더. 출력 디렉터리로는 *hdfs://localhost/user/HDIUser/WordCount\_Output*을 사용합니다. 출력 폴더는 기존 폴더여선 안 됩니다. 그렇지 않으면 MapReduce 작업이 실패합니다. MapReduce 작업을 두 번째 실행하려면 다른 출력 폴더를 지정하거나 기존 출력 폴더를 삭제해야 합니다.
...
Hadoop 명령에 대한 자세한 내용은 [Hadoop 명령 매뉴얼][Hadoop 명령 매뉴얼](영문)을 참조하세요.
...
샘플은 IIS W3C 로그 데이터 시나리오 처리에 대해 구성되어 있습니다. 데이터 집합을 다양한 크기로 만들고 HDFS 또는 WASB(Azure Blob 저장소)로 가져오도록 데이터 생성 도구가 제공됩니다. 자세한 내용은 [HDInsight에 Azure Blob 저장소 사용][HDInsight에 Azure Blob 저장소 사용]을 참조하세요. PowerShell 스크립트를 통해 생성된 데이터 페이지에서 MapReduce, Pig 또는 Hive 작업을 실행할 수 있습니다. Pig 및 Hive 스크립트는 MapReduce를 통한 추상화 계층이며 최종적으로는 MapReduce 프로그램으로 컴파일됩니다. 사용자는 일련의 작업을 실행하여 이처럼 서로 다른 기술을 사용하는 경우의 결과와 처리 작업 실행에 대한 데이터 크기의 영향을 파악할 수 있습니다.

### 섹션 내용

-   [IIS w3c 로그 데이터 시나리오][IIS w3c 로그 데이터 시나리오]
-   [샘플 w3c 로그 데이터 로드][샘플 w3c 로그 데이터 로드]
-   [Java MapReduce 작업 실행][Java MapReduce 작업 실행]
...
3.  다음 명령을 실행하여 HDFS로 데이터를 생성하고 가져옵니다.
    ...
    대신 WASB로 데이터를 로드하려면 [Azure Blob 저장소에 연결][Azure Blob 저장소에 연결]을 참조하세요.
...
4.  다음 명령을 실행합니다.
    ...
    <td>
    jar 파일은 C:\\hdp\\GettingStarted\\Java 폴더에 있습니다.
...
6.  다음 명령을 실행하여 **w3ctotalhitsbypage.hql** HiveQL 스크립트 파일을 실행합니다.
    ...
    <td>
    Hive 스크립트 파일을 다음 중 하나로 대체할 수 있습니다.
...
3.  다음 명령을 실행하여 Pig 작업을 제출합니다.
    ...
    <td>
    Pig Latin 스크립트 파일을 다음 중 하나로 대체할 수 있습니다.
    ...
    <td>
    매개 변수를 다음과 같은 더 큰 파일로 대체할 수 있습니다.
...
> [WACOM.NOTE] HDInsight에서 Azure Blob 저장소를 만드는 방법에 대한 자세한 내용은 [HDInsight에서 Azure Blob 저장소 사용][HDInsight에 Azure Blob 저장소 사용]을 참조하세요.

아래 지침에 따라 작업을 시작하기 전에 먼저 저장소 계정을 만들어야 합니다. 자세한 내용은 [저장소 계정을 만드는 방법][저장소 계정을 만드는 방법]을 참조하세요.
...
1.  메모장에서 **C:\\hdp\\hadoop-2.4.0.SNAPSHOT\\etc\\hadoop\\core-site.xml**을 엽니다.
2.  코드 페이지의 위쪽에 다음 <property\> 태그를 다른 <property\> 태그 옆에 추가합니다.
...
Hadoop 작업 제출에 대한 자세한 내용은 [Hadoop 작업을 프로그래밍 방식으로 제출][Hadoop 작업을 프로그래밍 방식으로 제출]을 참조하세요. HDInsight PowerShell cmdlet에 대한 자세한 내용은 [HDInsight cmdlet 참조][HDInsight cmdlet 참조]를 참조하세요.

## <a name="nextsteps"></a>다음 단계

이 자습서에서는 HDInsight Emulator를 설치했으며 일부 Hadoop 작업을 실행했습니다. 자세한 내용은 다음 문서를 참조하세요.

-   [Azure HDInsight를 사용하여 시작][Azure HDInsight를 사용하여 시작]
...
-   [HDInsight Emulator 릴리스 정보][HDInsight Emulator 릴리스 정보]
-   [HDInsight 토론을 위한 MSDN 포럼][HDInsight 토론을 위한 MSDN 포럼]
