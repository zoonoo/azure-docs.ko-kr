...
HDInsight를 사용하는 자습서는 [Azure HDInsight 사용 시작](/en-us/manage/services/hdinsight/get-started-hdinsight/)(영문)을 참조하십시오.

**필수 조건** 이 자습서를 시작하기 전에 다음이 있어야 합니다.
...
1.  Internet Explorer을 열고 [Microsoft HDInsight Emulator for Azure 설치 페이지](http://www.microsoft.com/web/gallery/install.aspx?appid=HDINSIGHT)로 이동합니다.
2.  **지금 설치**를 클릭합니다.
...
8.  웹 플랫폼 설치 관리자 4.6 창을 닫으려면 **끝내기**를 클릭합니다.
    ...
    HDInsight Server 설치 및 실행에 대해 알려진 문제는 [HDInsight Emulator 릴리스 정보](http://gettingstarted.hadooponazure.com/releaseNotes.html)를 참조하십시오. 설치 로그는 **C:\\HadoopFeaturePackSetup\\HadoopFeaturePackSetupTools\\gettingStarted.winpkg.install.log**에 있습니다.
...
일부 fs 명령도 사용할 수 있습니다. Hadoop 명령에 대한 자세한 내용은 [Hadoop 명령 매뉴얼](http://hadoop.apache.org/docs/r1.1.1/commands_manual.html)(영문)을 참조하십시오.

단어 계산 MapReduce 작업에는 두 개의 인수(입력 폴더 및 출력 폴더)가 필요합니다. 입력 폴더로는 *hdfs://localhost/user/HDIUser*를 사용하고 출력 디렉터리로는 *hdfs://localhost/user/HDIUser/WordCount\_Output*을 사용합니다. 출력 폴더는 기존 폴더여선 안 됩니다. 그렇지 않으면 MapReduce 작업이 실패합니다. MapReduce 작업을 두 번째 실행하려면 다른 출력 폴더를 지정하거나 기존 출력 폴더를 삭제해야 합니다.
...
4.  다음 명령을 실행하여 /user/HDIUser 폴더의 파일을 나열합니다.
    ...
    다음 파일이 표시되어야 합니다.
         ...
         Found 8 items
         -rw-r--r--   1 사용자 이름 supergroup      16372 2013-10-30 12:07 /user/HDIUser/CHANGES.branch-1-win.txt
         -rw-r--r--   1 사용자 이름 supergroup     463978 2013-10-30 12:07 /user/HDIUser/CHANGES.txt
         -rw-r--r--   1 사용자 이름 supergroup       6631 2013-10-30 12:07 /user/HDIUser/Jira-Analysis.txt
         -rw-r--r--   1 사용자 이름 supergroup      13610 2013-10-30 12:07 /user/HDIUser/LICENSE.txt
         -rw-r--r--   1 사용자 이름 supergroup       1663 2013-10-30 12:07 /user/HDIUser/Monarch-CHANGES.txt
         -rw-r--r--   1 사용자 이름 supergroup        103 2013-10-30 12:07 /user/HDIUser/NOTICE.txt
         -rw-r--r--   1 사용자 이름 supergroup       2295 2013-10-30 12:07 /user/HDIUser/README.Monarch.txt
         -rw-r--r--   1 사용자 이름 supergroup       1397 2013-10-30 12:07 /user/HDIUser/README.txt
...
샘플은 IIS W3C 로그 데이터 시나리오 처리에 대해 구성되어 있습니다. 데이터 집합을 다양한 크기로 만들고 HDFS 또는 WASB(Azure Blob 저장소)로 가져오도록 데이터 생성 도구가 제공됩니다. 자세한 내용은 [HDInsight에 Azure Blob 저장소 사용](/en-us/manage/services/hdinsight/howto-blob-store/)을 참조하십시오. 그다음에 MapReduce, Pig 또는 Hive 작업이 PowerShell 스크립트가 생성한 데이터 페이지에서 실행될 수 있습니다. 사용된 Pig 및 Hive 스크립트가 둘 다 MapReduce 프로그램으로 컴파일된다는 점에 유의하십시오. 사용자가 일련의 작업을 실행하여 이러한 다른 기술을 사용한 효과 및 처리 작업 실행에 대한 데이터 크기의 효과를 관찰할 수도 있습니다.
...
3.  다음 명령을 실행하여 HDFS로 데이터를 생성하고 가져옵니다.
    ...
    대신 WASB로 데이터를 로드하려면 [Azure Blob 저장소에 연결](#blobstorage)을 참조하십시오.
...
4.  다음 명령을 실행합니다.
	...
	<tr><td>w3c_scenarios.jar</td><td>jar 파일은 C:\Hadoop\GettingStarted\Java 폴더에 있습니다.</td></tr>
...
각 작업의 첫 번째 단계로서 테이블이 생성되고 이전에 만든 파일의 테이블로 데이터가 로드된다는 점에 유의하십시오. 다음 명령을 사용하여 HDFS의 /Hive 노드에서 해당 파일을 찾을 수 있습니다.
...
Azure HDInsight는 Azure Blob 저장소를 기본 파일 시스템으로 사용합니다. 자세한 내용은 [HDInsight와 함께 Azure Blob 저장소 사용](/en-us/manage/services/hdinsight/howto-blob-store/)을 참조하십시오.
...
> Hadoop Hive 서비스 중 하나도 포트 10000을 사용하기 때문입니다. 문제를 해결하려면 다음 절차를 따르십시오.
...
저장소 계정 만들기에 관한 지침은 [저장소 계정을 만드는 방법](/en-us/manage/services/storage/how-to-create-a-storage-account/)을 참조하십시오.
