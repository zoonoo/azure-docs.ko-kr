...
위의 모든 단계는 Azure 기계 학습 권장 사항 API를 통해 수행됩니다.  이러한 각 단계를 구현하는 응용 프로그램 예제를 [갤러리에서도](http://1drv.ms/1xeO2F3)
...
Microsoft Azure Marketplace에서는 기본 또는 OAuth 인증 방법을 지원합니다. 마켓플레이스의 [계정 설정](https://datamarket.azure.com/account/keys)에서 키로 이동하여 계정 키를 쉽게 찾을 수 있습니다. 
...
Azure 기계 학습 권장 사항 API에 대한 서비스 루트 URI는 [여기](https://api.datamarket.azure.com/amla/recommendations/v2/)
...
API에서 반환되는 ID는 대/소문자를 구분하며, 후속 API 호출에서 매개 변수로 전달될 때에도 마찬가지로 사용되어야 합니다. 예를 들어 모델 ID와 카탈로그 ID는 대/소문자를 구분합니다.
...
* `feed/entry/content/properties/Status` – 빌드 상태 다음 중 하나일 수 있음: Error, Building, Queued, Canceling, Canceled, Success
