...
* **Azure Storage** : 고가용성 설정을 구성 하려면 [Azure Storage 중복성](../storage/common/storage-redundancy.md)(영문)을 참조 하세요.
* **Key Vault** : Key Vault는 기본적으로 고가용성을 제공 하며 사용자 작업이 필요 하지 않습니다.  [Azure Key Vault 가용성 및 중복성을](../key-vault/general/disaster-recovery-guidance.md)참조 하세요.
* **Container Registry** : 지역에서 복제에 대 한 Premium 레지스트리 옵션을 선택 합니다. [Azure Container Registry에서 지역에서 복제를](../container-registry/container-registry-geo-replication.md)참조 하세요.
* **Application Insights** : Application Insights는 고가용성 설정을 제공 하지 않습니다. 데이터 보존 기간 및 세부 정보를 조정 하려면 [Application Insights의 데이터 수집, 보존 및 저장](../azure-monitor/app/data-retention-privacy.md#how-long-is-the-data-kept)을 참조 하세요.

## <a name="compute-resources"></a>Compute 리소스

다음 설명서를 참조 하 여 각 리소스의 고가용성 설정을 구성 해야 합니다.

* **Azure Kubernetes service** : [AKS (azure Kubernetes service)에서 비즈니스 연속성 및 재해 복구에 대 한 모범 사례](../aks/operator-best-practices-multi-region.md) 를 참조 하 고 [가용성 영역을 사용 하는 AKS (azure Kubernetes service) 클러스터를 만듭니다](../aks/availability-zones.md). Azure Machine Learning Studio, SDK 또는 CLI를 사용 하 여 AKS 클러스터를 만든 경우 지역 간 고가용성은 지원 되지 않습니다.
* **Azure Databricks** : [Azure Databricks 클러스터에 대 한 지역별 재해 복구를](/azure/databricks/scenarios/howto-regional-disaster-recovery)참조 하세요.
* **Container Instances** : orchestrator는 장애 조치 (failover)를 담당 합니다. [Azure Container Instances 및 컨테이너 orchestrator를](../container-instances/container-instances-orchestrator-relationship.md)참조 하세요.
* **Hdinsight** : [Azure hdinsight에서 지 원하는 고가용성 서비스](../hdinsight/hdinsight-high-availability-components.md)를 참조 하세요.
...
* **Data Lake Storage Gen1** : [Data Lake Storage Gen1에 대 한 고가용성 및 재해 복구 지침을](../data-lake-store/data-lake-store-disaster-recovery-guidance.md)참조 하세요.
* **SQL Database** : [Azure SQL Database 및 SQL Managed Instance의 고가용성](../azure-sql/database/high-availability-sla.md)을 참조 하세요.
* **Azure Database for PostgreSQL** : [Azure Database for PostgreSQL 단일 서버에서 고가용성 개념을](../postgresql/concepts-high-availability.md)참조 하세요.
* **Azure Database for MySQL** : [Azure Database for MySQL의 비즈니스 연속성 이해를](../mysql/concepts-business-continuity.md)참조 하세요.
* **Azure Databricks 파일 시스템** : [Azure Databricks 클러스터에 대 한 지역 재해 복구](/azure/databricks/scenarios/howto-regional-disaster-recovery)를 참조 하세요.

## <a name="azure-cosmos-db"></a>Azure Cosmos DB

사용자 고유의 고객이 관리 하는 키를 제공 하 여 Azure Machine Learning 작업 영역을 배포 하는 경우 Azure Cosmos DB도 구독 내에서 프로 비전 됩니다. 이 경우에는 고가용성 설정을 구성 해야 합니다. [Azure Cosmos DB의 고가용성](../cosmos-db/high-availability.md)을 참조하세요.
