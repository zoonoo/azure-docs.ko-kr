...
- **csdairydata.csv 데이터 집합**의 출력을 [R 스크립트][execute-r-script] 실행의 가장 왼쪽에 있는 입력(**Dataset1**)에 연결합니다.
...
실험에 로드한 데이터를 살펴보겠습니다. 실험에서 **cadairydata.csv** 데이터 집합의 출력을 두 번 클릭하고 **시각화**를 선택합니다. 그림 4와 같이 표시됩니다.

![cadairydata.csv 데이터 집합 요약][4]

*그림 4. cadairydata.csv 데이터 집합 요약*

여기에서 많은 유용한 정보를 볼 수 있습니다. **Feature Type** 행은 Azure 기계 학습 스튜디오가 데이터 집합 열에 할당하는 데이터 형식을 보여줍니다. 또한 해당 데이터 집합의 처음 몇 행도 볼 수 있습니다. 중요한 작업을 시작하기 전에 이렇게 빠르게 확인하면 온전성을 검사하는 데 좋습니다.
