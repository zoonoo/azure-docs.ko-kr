---
title: 트리거 Azure Machine Learning 파이프라인
titleSuffix: Azure Machine Learning
description: 트리거된 파이프라인을 사용 하면 데이터 처리, 학습 및 모니터링과 같은 일상적인 시간 사용 작업을 자동화할 수 있습니다.
...
## <a name="use-azure-logic-apps-for-complex-triggers"></a>복합 트리거의 경우 Azure Logic Apps 사용 
...
이 문서에서는 Python 용 Azure Machine Learning SDK를 사용 하 여 두 가지 다른 방법으로 파이프라인을 예약 했습니다. 경과 된 클록 시간에 따라 일정 하나가 되풀이 됩니다. 지정 된 `Datastore` 또는 해당 저장소의 디렉터리 내에서 파일이 수정 된 경우에는 다른 일정이 실행 됩니다. 포털을 사용 하 여 파이프라인 및 개별 실행을 검사 하는 방법을 살펴보았습니다. 파이프라인 실행이 중지 되도록 일정을 사용 하지 않도록 설정 하는 방법을 배웠습니다. 마지막으로, 파이프라인을 트리거하는 Azure 논리 앱을 만들었습니다. 
