...
이 문서에서는 Azure Machine Learning Python SDK의 interpretability 패키지를 사용 하 여 모델이 예측을 수행한 이유를 이해 하는 방법을 알아봅니다. 다음 방법을 알아봅니다.

* 로컬 및 원격 계산 리소스에 대해 학습 된 기계 학습 모델을 해석 합니다.
* Azure 실행 기록에 대 한 로컬 및 전역 설명을 저장 합니다.
* [Azure Machine Learning studio](https://ml.azure.com)에서 interpretability 시각화를 봅니다.
* 모델을 사용 하 여 점수 매기기 설명을 배포 합니다.

자세한 내용은 [Azure Machine Learning 모델 interpretability](how-to-machine-learning-interpretability.md)을 참조 하세요.

## <a name="local-interpretability"></a>로컬 interpretability

다음 예제에서는 Azure 서비스에 연결 하지 않고 interpretability 패키지를 로컬로 사용 하는 방법을 보여 줍니다.

1. 필요한 경우 `pip install azureml-interpret`를 사용 하 여 interpretability 패키지를 가져옵니다.
...
1. 설명를 로컬로 호출 합니다.
   ...
   * 설명 및 시각화에 대 한 자세한 정보를 제공 하기 위해 분류를 수행 하는 경우 기능 이름 및 출력 클래스 이름을 전달 하도록 선택할 수 있습니다.
   ...
   다음 코드 블록은 `TabularExplainer`, `MimicExplainer`및 `PFIExplainer`를 사용 하 여 로컬에서 설명 개체를 인스턴스화하는 방법을 보여 줍니다.
   * `TabularExplainer`는 아래에 있는 세 개의 SHAP explainers (`TreeExplainer`, `DeepExplainer`또는 `KernelExplainer`) 중 하나를 호출 합니다.
   * `TabularExplainer`은 사용 사례에 대해 가장 적합 한 항목을 자동으로 선택 하지만 세 가지 기본 explainers를 직접 호출할 수 있습니다.
...
전역 기능 중요도 값을 가져오는 데 도움이 되는 다음 예제를 참조 하세요.
...
개별 인스턴스 또는 인스턴스 그룹에 대 한 설명을 호출 하 여 로컬 기능 중요도 값을 가져옵니다.
> [!NOTE]
...
* 원격 실행에서 `ExplanationClient`를 사용 하 여 interpretability 컨텍스트를 업로드 합니다.
* 로컬 환경에서 나중에 컨텍스트를 다운로드 합니다.

1. 필요한 경우 `pip install azureml-contrib-interpret`를 사용 하 여 필요한 패키지를 가져옵니다.
...
엔지니어링 된 기능이 아니라 원시 레이크의 기능을 사용 하 여 설명을 볼 수 있습니다. 이 옵션의 경우 `train_explain.py`의 기능 변환 파이프라인을 설명에 전달 합니다. 그렇지 않으면 설명는 엔지니어링 된 기능 측면에서 설명을 제공 합니다.
...
`sklearn.compose.ColumnTransformer`를 사용 하거나 적합 한 변환기 튜플 목록과 함께 원시 기능에 대 한 설명을 가져옵니다. 다음 예에서는 `sklearn.compose.ColumnTransformer`를 사용 합니다.
...
|글로벌 중요도|최상위 K (구성 가능 K) 중요 기능을 전역적으로 보여 줍니다. 기본 모델의 전역 동작을 이해 하는 데 도움이 됩니다.|
|설명 탐색|기능이 모델의 예측 값 또는 예측 값의 확률 변경에 영향을 주는 방법을 보여 줍니다. 기능 상호 작용의 영향을 보여 줍니다.|
|요약 중요도|모든 데이터 요소에서 로컬 기능 중요도 값을 사용 하 여 예측 값에 대 한 각 기능 영향의 분포를 표시 합니다.|
...
플롯에서 개별 데이터 요소를 선택 하 여 모든 데이터 요소에 대 한 로컬 기능 중요도 플롯을 로드할 수 있습니다.

|그리면|Description|
|----|-----------|
|로컬 중요도|최상위 K (구성 가능 K) 중요 기능을 전역적으로 보여 줍니다. 특정 데이터 요소에 대 한 기본 모델의 로컬 동작을 보여 줍니다.|
|Perturbation 탐색|선택한 데이터 요소의 기능 값에 대 한 변경 내용을 허용 하 고 그 결과 예측 값에 대 한 변경 내용을 관찰 합니다.|
...
[원격 interpretability](#interpretability-for-remote-runs) 단계를 완료 한 경우 [Azure Machine Learning studio](https://ml.azure.com)에서 시각화 대시보드를 볼 수 있습니다. 이 대시보드는 위에서 설명한 시각화 대시보드의 더 간단한 버전입니다. 두 개의 탭만 지원 합니다.
...
전역 설명과 로컬 설명을 모두 사용할 수 있는 경우 데이터는 두 탭을 모두 채웁니다. 전역 설명만 사용할 수 있는 경우 요약 중요도 탭을 사용할 수 없습니다.
...
원본 모델과 함께 설명를 배포 하 고 유추 시간에 사용 하 여 로컬 설명 정보를 제공할 수 있습니다. 또한 유추 시 interpretability 성능을 향상 시키기 위해 가벼운 점수 매기기 explainers 제공 합니다. 가벼운 점수 매기기 설명를 배포 하는 프로세스는 모델 배포와 유사 하며 다음 단계를 포함 합니다.
