...
컴퓨팅 인스턴스용 OS 디스크는 Azure Machine Learning 스토리지 계정의 Microsoft 관리형 키로 암호화됩니다. 컴퓨팅 인스턴스의 로컬 임시 디스크는 `hbi_workspace` 매개 변수가 `TRUE`로 설정된 작업 영역의 Microsoft 관리형 키로 암호화됩니다.
