...
필요에 따라 사용자 고유의 이미지를 빌드하지 않으려는 경우 Azure ML은 미리 빌드된 큐 레이트 환경을 제공 합니다. Azure ML에는 다양 한 버전의 PyTorch에 해당 하는 PyTorch에 대 한 몇 가지 CPU 및 GPU 큐 레이트 환경이 있습니다. 자세한 내용은 [여기](resource-curated-environments.md)를 참조 하세요.
