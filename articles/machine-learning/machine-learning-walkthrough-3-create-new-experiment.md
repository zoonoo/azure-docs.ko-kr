...
실험의 다음 단계에서 두 개의 별도 데이터 집합으로 데이터 집합을 분할합니다. 하나는 모델 학습용이고 다른 하나는 테스트용으로 사용하게 됩니다.
...
이전에 언급한 대로 높은 신용 위험을 낮은 것으로 잘못 분류한 비용은 낮은 신용 위험을 높은 것으로 잘못 분류한 비용보다 5배 더 큽니다. 이를 고려하기 위해 이 비용 함수를 반영하는 새 데이터 집합을 생성합니다. 새 데이터 집합에서 위험이 높은 각 예제는 5번 복제되지만 위험이 낮은 각 예제는 복제되지 않습니다.   
...
학습 및 테스트 데이터에 같은 비용 조정이 포함되도록 [데이터 분할][split] 모듈의 각 출력에 대해 이와 같은 복제 작업을 수행해야 합니다. 방금 만든 [R 스크립트 실행][execute-r-script] 모듈을 복제하고 [데이터 분할][split] 모듈의 다른 출력 포트에 연결하여 이 작업을 수행합니다.
