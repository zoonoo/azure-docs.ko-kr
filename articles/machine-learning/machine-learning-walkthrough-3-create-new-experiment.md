...
3. 데이터 집합을 [메타데이터 편집][edit-metadata]에 연결합니다. 데이터 집합의 출력 포트(데이터 집합의 맨 아래에 있는 작은 원)를 클릭합니다. 다음으로 [메타데이터 편집][edit-metadata]의 입력 포트(모듈의 맨 위에 있는 작은 원)로 끈 다음 마우스 버튼을 놓습니다. 데이터 집합과 모듈은 캔버스에서 이동해도 연결 상태를 유지합니다.
   ...
   > [!TIP]
   > 모듈을 두 번 클릭하고 텍스트를 입력하여 모듈에 주석을 추가할 수 있습니다. 그러면 모듈이 실험에서 수행하는 내용을 한눈에 볼 수 있습니다. 이 경우 [메타데이터 편집][edit-metadata] 모듈을 두 번 클릭하고 주석 "열 머리글 추가"를 입력합니다. 캔버스의 다른 곳을 클릭하여 텍스트 상자를 닫습니다. 주석을 표시하려면 모듈에서 아래쪽 화살표를 클릭합니다.
   > 
   > 
