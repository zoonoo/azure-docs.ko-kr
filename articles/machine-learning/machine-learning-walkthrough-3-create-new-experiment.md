...
6.	**시작 문자** 아래의 행에서는 [메타데이터 편집][edit-metadata]으로 수정할 특정 열을 포함하거나 제외할 수 있습니다. *모든* 열을 수정하려고 하므로 행 오른쪽에 있는 빼기 기호("-")를 클릭하여 이 행을 삭제합니다. 대화 상자는 다음과 같습니다. ![모든 열이 선택된 열 선택기][4]
7.	**확인** 확인 표시를 클릭합니다.
