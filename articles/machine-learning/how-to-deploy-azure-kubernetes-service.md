...
끝점을 사용하여 제어된 방식으로 모델 버전을 분석하고 승격합니다. 단일 엔드포인트 뒤에 최대 6개의 버전을 배포할 수 있습니다. 끝점은 다음과 같은 기능을 제공합니다.

* 각 __끝점으로 전송된 점수 매기기 트래픽의 백분율을 구성합니다.__ 예를 들어 트래픽의 20%를 끝점 '테스트'로 라우팅하고 80%를 '프로덕션'으로 라우팅합니다.

    > [!NOTE]
    > 트래픽의 100%를 차지하지 않으면 나머지 백분율은 __기본__ 끝점 버전으로 라우팅됩니다. 예를 들어 트래픽의 10%를 얻으려면 끝점 버전 'test'를 구성하고 30%에 대해 'prod'를 구성하면 나머지 60%는 기본 끝점 버전으로 전송됩니다.
    >
    > 생성된 첫 번째 끝점 버전은 자동으로 기본값으로 구성됩니다. 끝점 버전을 만들거나 업데이트할 때 설정하여 `is_default=True` 변경할 수 있습니다.
     
* 끝점 버전을 __제어__ 또는 __처리로__태그합니다. 예를 들어 현재 프로덕션 끝점 버전이 컨트롤일 수 있지만 잠재적인 새 모델은 치료 버전으로 배포됩니다. 처리 버전의 성능을 평가한 후 현재 제어를 능가하는 경우 새 프로덕션/제어로 승격될 수 있습니다.

    > [!NOTE]
    > __하나의__ 컨트롤만 사용할 수 있습니다. 여러 가지 트리트먼트를 받으실 수 있습니다.

앱 인사이트를 활성화하여 엔드포인트 및 배포된 버전의 운영 메트릭을 볼 수 있습니다.

### <a name="create-an-endpoint"></a>엔드포인트 만들기
모델을 배포할 준비가 되면 점수 매기기 엔드포인트를 만들고 첫 번째 버전을 배포합니다. 다음 예제에서는 SDK를 사용하여 끝점을 배포하고 만드는 방법을 보여 주며 있습니다. 첫 번째 배포는 기본 버전으로 정의되며, 이는 모든 버전에서 지정되지 않은 트래픽 백분위수로 기본 버전으로 이동한다는 것을 의미합니다.  

> [!TIP]
> 다음 예제에서 구성은 트래픽의 20%를 처리하도록 초기 끝점 버전을 설정합니다. 첫 번째 끝점이므로 기본 버전이기도 합니다. 그리고 트래픽의 다른 80%에 대한 다른 버전이 없기 때문에 기본값으로도 라우팅됩니다. 트래픽의 일정 비율을 차지하는 다른 버전이 배포될 때까지 이 버전은 트래픽의 100%를 효과적으로 받습니다.
...
> [!TIP]
> 다음 코드 스니펫에 의해 만들어진 두 번째 버전은 트래픽의 10%를 허용합니다. 첫 번째 버전은 20%로 구성되므로 트래픽의 30%만 특정 버전에 대해 구성됩니다. 나머지 70%는 기본 버전이기도 하기 때문에 첫 번째 끝점 버전으로 전송됩니다.
...
기존 버전을 업데이트하거나 끝점에서 삭제합니다. 버전의 기본 유형, 제어 유형 및 트래픽 백분위수(백분율)를 변경할 수 있습니다. 다음 예제에서 두 번째 버전은 트래픽을 40%로 늘리고 이제 기본값으로 설정합니다.

> [!TIP]
> 다음 코드 조각 후 두 번째 버전이 기본값입니다. 이제 40%로 구성되고 원래 버전은 여전히 20%로 구성됩니다. 즉, 트래픽의 40%는 버전 구성에 의해 고려되지 않습니다. 이제 기본값이므로 남은 트래픽은 두 번째 버전으로 라우팅됩니다. 그것은 효과적으로 트래픽의 80 %를 받습니다.
