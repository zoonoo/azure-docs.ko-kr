...
아직 없는 경우 작업 영역 (`ws`)에 [`AmlCompute`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.compute.amlcompute%28class%29?view=azure-ml-py) 대상을 만듭니다.
...
[`TabularDataset`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py) 및 레이블로 training_data 정의 되며 [`AutoMLConfig`](https://docs.microsoft.com/python/api/azureml-train-automl-client/azureml.train.automl.automlconfig.automlconfig?view=azure-ml-py)에서 자동화 된 ML로 전달 됩니다. 기본적으로 `TabularDataset` 메서드 `from_delimited_files`는 `infer_column_types`를 true로 설정 하 여 열 형식을 자동으로 유추 합니다. 
