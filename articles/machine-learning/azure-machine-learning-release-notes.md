...
+ **버그 수정 및 향상 된 기능**
  ...
  + **azureml-core**
    ...
    + `process_count`다중 프로세스 다중 노드 PyTorch 작업을 지원 하려면 [PyTorchConfiguration](/python/api/azureml-core/azureml.core.runconfig.pytorchconfiguration) 에를 추가 합니다.
  + **azureml-pipeline-steps**
    + 이제 [Commandstep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.commandstep) 이 GA 이며 더 이상 실험적이 아닙니다.
    + [ParallelRunConfig](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.parallelrunconfig): allowed_failed_count 인수를 추가 하 고 allowed_failed_percent 하 여 미니 일괄 처리 수준에서 오류 임계값을 확인 합니다. 오류 임계값에는 이제 다음과 같은 3 가지 종류가 있습니다.
...
+ **버그 수정 및 향상 된 기능**
  + **azure-cli-ml**
    + UserAssigned를 사용 하 여 AmlCompute를 사용 하는 경우 CLI 도움말 텍스트가 수정 됨
  + **azureml-자동 dnn-비전**
    + 자동 Ml 비전 실행을 위해 배포 및 다운로드 단추가 표시 되 고 다른 AutoML 실행과 유사 하 게 모델을 배포 하거나 다운로드할 수 있습니다. 추론 및 iisnode.yml 파일을 실행 하 여 conda 환경을 다시 만드는 스크립트를 포함 하는 새 파일 scoring_file_v_1_0_0 (py 및 conda_env_v_1_0_0)이 두 개 있습니다. ' Pth ' 파일의 이름이 '. p s ' 인 확장명을 사용 하도록 변경 되었습니다.
  + **azureml-core**
    + Azure에 대 한 MSI 지원-cli-ml
    + 사용자 할당 관리 Id가 지원 됩니다.
    + 이러한 변경을 통해 고객은 미사용 암호화를 위해 고객 키 자격 증명 모음에서 키를 인출 하는 데 사용할 수 있는 사용자 할당 id를 제공할 수 있어야 합니다.
    +  매우 큰 파일의 프로필에 대 한 수정 row_count = 0-공백 채우기가 포함 된 구분 된 값에 대 한 이중 변환 오류 수정
    + 출력 데이터 집합 GA의 실험적 플래그 제거
    + 특정 모델 버전을 가져오는 방법에 대 한 설명서 업데이트
    + 개인 링크의 경우 혼합 모드 액세스를 위한 작업 영역 업데이트 허용
    + 다시 시작 기능을 위한 데이터 저장소에 대 한 추가 등록을 제거 하도록 수정
    + 기본 사용자 할당 작업 영역 id를 업데이트 하기 위한 CLI/SDK 지원이 추가 됨
  + **azureml-interpret**
    + 업데이트 된 azureml-해석으로 해석-community 0.16.0
    + azureml의 설명 클라이언트에 대 한 메모리 최적화-해석
  + **azureml-train-automl-runtime**
    + ADB 실행에 대 한 스트리밍 사용
  + **azureml-train-core**
    + 다시 시작 기능을 위한 데이터 저장소에 대 한 추가 등록을 제거 하도록 수정
  + **azureml-widgets**
    + 고객은 위젯을 사용 하 여 기존 실행 데이터 시각화에 대 한 변경 내용을 볼 수 없으며, 필요에 따라 조건부 하이퍼 매개 변수를 사용 하는 경우 지원 됩니다.
    + 이제 사용자 실행 위젯에는 실행이 대기 상태인 이유에 대 한 자세한 설명이 포함 되어 있습니다.
...
+ **새로운 기능**
   + 기본 터미널입니다. 이제 사용자는 통합 터미널을 통해 Git 작업 뿐만 아니라 통합 터미널에 액세스할 수 있습니다 [.](./how-to-access-terminal.md)
  + 중복 폴더 
  + 계산 드롭다운 비용 
  + 오프 라인 계산 Pylance 
...
+ **버그 수정 및 향상 된 기능**
  ...
  + **azureml-automl-runtime**
    + DateTime 열에 OutOfBoundsDatetime 값이 있는 경우 ConfigException을 Throw 합니다.
    + 가우스 노이즈를 사용 하 여 안쪽 여백을 허용 하 여 간단한 시계열 처리를 개선 했습니다.
    + 각 텍스트 열이 해당 텍스트 열에 있는 문자열의 길이에 따라 n-영문법 범위를 사용 하 여 문자-문법 변환을 활용할 수 있도록 합니다.
    + 사용자의 로컬 계산에서 실행 되는 AutoML 실험의 최상의 모드에 대 한 원시 기능 설명 제공
  + **azureml-core**
    + 패키지를 고정 합니다. pyjwt를 사용 하 여 향후 릴리스에서 분리 버전을 가져오지 않도록 합니다.
    + 이러한 실험이 있거나 새 실험 인 경우 실험을 만들면 동일한 지정 된 이름을 사용 하 여 활성 또는 마지막 보관 실험을 반환 합니다.
    + 이름을 기준으로 get_experiment를 호출 하면 지정 된 이름의 활성 또는 마지막 보관 된 실험이 반환 됩니다.
    + 사용자가 실험을 다시 활성화 하는 동안에는 이름을 바꿀 수 없습니다.
    + 데이터 집합이 실험에 잘못 전달 될 때 잠재적 수정 사항을 포함 하는 오류 메시지 (예: ScriptRunConfig)가 개선 되었습니다. 
  ...
  + **azureml-train-automl-runtime**
    + DateTime 열에 OutOfBoundsDatetime 값이 있는 경우 ConfigException을 Throw 합니다.
    + 사용자의 로컬 계산에서 실행 되는 AutoML 실험을 위한 최상의 모델에 대 한 원시 기능 설명을 제공 하기 위한 지원이 추가 되었습니다.
    + 가우스 노이즈를 사용 하 여 안쪽 여백을 허용 하 여 간단한 시계열 처리를 개선 했습니다.
  + **azureml-train-core**
    + 이렇게 변경 하면 빈 문자열을 script_param 값으로 사용할 수 있습니다.
  + **azureml-restclients-hyperdrive**
    + 추가 정보를 제공 하기 위해 추가 정보를 변경 했습니다.
  + **azureml-widgets**
    + 위젯의 차트/병렬-좌표 라이브러리에 문자열 지원을 추가 합니다.
...
### <a name="azure-machine-learning-studio-notebooks-experience-october-update"></a>Azure Machine Learning Studio 노트북 환경 (10 월 업데이트)
+ **새로운 기능**
...
+ **버그 수정 및 향상 된 기능**
  ...
  + **azureml-core**
    ...
    + 모델에 image_name 및 image_label 매개 변수를 추가 하 여 기본 패키지 이미지의 이름을 바꿀 수 있습니다.
    + `set_pip_requirements()`전체 pip 섹션을 한 번에 설정 하는 새 메서드입니다 [`CondaDependencies`](/python/api/azureml-core/azureml.core.conda_dependencies.condadependencies) .
  ...
  + **azureml-train-automl-runtime**
    ...
    + 데이터가 nan을 포함 하 고 기능화가 해제 되어 있는 경우 자식 실행 오류가 해결 되었습니다.
  + **azureml-train-core**
    + To 매개 변수에 전달 된 pip 요구 사항 파일에 pip 옵션 (예:--extra-url)을 지정 하기 위한 지원이 추가 되었습니다 [`Estimator`](/python/api/azureml-train-core/azureml.train.estimator.estimator) `pip_requirements_file` .
...
이제 Azure Machine Learning의 스튜디오 웹 환경 내에서 직접 machine learning 노트와 파일을 만들고, 편집 하 고, 공유할 수 있습니다. 이러한 노트북 내에서 [Azure Machine Learning PYTHON SDK](/python/api/overview/azure/ml/intro) 에서 사용할 수 있는 모든 클래스와 메서드를 사용할 수 있습니다. [여기서](./how-to-run-jupyter-notebooks.md) 시작 하세요.
...
+ **버그 수정 및 향상 된 기능**
  ...
  + **azureml-core**
    ...
    + [Environment.build_local ()](/python/api/azureml-core/azureml.core.environment.environment) 에 이미지를 AzureML 작업 영역 컨테이너 레지스트리에 푸시하는 옵션이 추가 되었습니다.
...
+ **버그 수정 및 향상 된 기능**
  ...
  + **[azureml-datadrift](/python/api/azureml-datadrift)**
    ...
    + 데이터 집합 기반 데이터 드리프트 출력 검색을 지원 합니다.
  + **azureml-explain-model**
    + MimicWrapper를 사용 하 여 직접 만들 [ScoringExplainer](/python/api/azureml-interpret/azureml.interpret.scoring.scoring_explainer.scoringexplainer) 에 대 한 지원 추가
  + **[azureml-pipeline-core](/python/api/azureml-pipeline-core)**
    + 대량 파이프라인 생성을 위한 향상 된 성능.
...
는 [`PipelineEndpoint`](/python/api/azureml-pipeline-core/azureml.pipeline.core.pipeline_endpoint.pipelineendpoint) 동일한 끝점을 유지 하면서 게시 된 파이프라인의 새 버전을 추가 하기 위해 도입 되었습니다.
...
+ **새로운 기능**
  ...
  최신 SDK 릴리스에서 지원 되는 버전 목록은 [DNN 평가기 설명서](/python/api/azureml-train-core/azureml.train.dnn)를 참조 하세요.
...
+ **새로운 기능**
  + 이제 Azure Machine Learning는 인기 있는 DNN framework 체 이너를 위한 첫 번째 클래스 지원을 제공 합니다. [`Chainer`](/python/api/azureml-train-core/azureml.train.dnn.chainer)클래스 사용자를 사용 하면 체 이너 모델을 쉽게 학습 하 고 배포할 수 있습니다.
