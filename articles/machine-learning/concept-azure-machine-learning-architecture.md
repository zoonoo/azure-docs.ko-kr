...
1. **학습**
    + 비주얼 디자이너를 사용 하 여 **Python**, **R**또는에서 기계 학습 교육 스크립트를 개발 합니다.
    + **컴퓨팅 대상**을 만들고 구성합니다.
    + 구성 된 계산 대상에 **스크립트를 제출** 하 여 해당 환경에서 실행 합니다. 학습 중에 스크립트는 데이터 **저장소**에서 읽거나 쓸 수 있습니다. 학습 중에 생성 된 로그 및 출력은 **작업 영역** 에서 **실행** 으로 저장 되 고 **실험**아래에 그룹화 됩니다.
...
* [작업 영역](#workspaces)
    ...
    * [데이터 집합 & 데이터 저장소](#datasets-and-datastores)
...
실행은 학습 스크립트를 한 번 실행 하는 것입니다. 실험은 일반적으로 여러 실행을 포함 합니다.

모든 실행을 기록 하는 Azure Machine Learning 실험에서 다음 정보를 저장 합니다.
...
* [Azure Machine Learning를 사용 하 여 대규모로 체 이너 모델을 학습 하 고 등록](how-to-train-ml-models.md)합니다.
...
### <a name="compute-instance-preview"></a><a name="compute-instance"></a>계산 인스턴스 (미리 보기)
