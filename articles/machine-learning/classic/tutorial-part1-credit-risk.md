...
원래 데이터 세트는 공백으로 구분된 형식을 사용합니다. Machine Learning Studio(클래식)는 CSV(쉼표로 구분된 값) 파일에서 더 효율적으로 작동하므로 공백을 쉼표로 바꿔서 데이터 세트를 변환합니다.  
...
데이터가 CSV 형식으로 변환되면 이를 Machine Learning Studio(클래식)에 업로드해야 합니다. 
...
실험에서의 R 스크립트 사용에 대한 자세한 내용은 [R을 사용하여 실험 확장](index.yml)을 참조하세요.
