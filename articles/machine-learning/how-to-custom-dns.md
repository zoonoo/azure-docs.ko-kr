...
ms.locfileid: "97724717"
---
# <a name="how-to-use-your-workspace-with-a-custom-dns-server"></a>사용자 지정 DNS 서버에서 작업 영역을 사용하는 방법

개인 끝점과 함께 Azure Machine Learning 작업 영역을 사용 하는 경우 [여러 가지 방법으로 DNS 이름 확인을 처리할](../private-link/private-endpoint-dns.md)수 있습니다. 기본적으로 Azure는 작업 영역 및 개인 끝점에 대 한 이름 확인을 자동으로 처리 합니다. _사용자 고유의 사용자 지정 dns 서버_ _를 대신 사용 하는 경우 dns 항목을 수동으로 만들거나 작업 영역에 대해 조건부 전달자를 사용 해야 합니다.
...
> [!IMPORTANT]
> 일부 Fqdn은 개인 끝점에 표시 되지 않지만 eastus, southcentralus 및 westus2의 작업 영역에 필요 합니다. 이러한 Fqdn은 다음 표에 나열 되어 있으며, DNS 서버 및/또는 Azure 사설 DNS 영역에도 추가 해야 합니다.
>
> * `<workspace-GUID>.workspace.<region>.cert.api.azureml.ms`
> * `<workspace-GUID>.workspace.<region>.experiments.azureml.net`
> * `<workspace-GUID>.workspace.<region>.modelmanagement.azureml.net`
...
개인 끝점을 DNS 구성에 통합 하는 방법에 대 한 자세한 내용은 [Azure 개인 끝점 DNS 구성](https://docs.microsoft.com/azure/private-link/private-endpoint-dns)을 참조 하세요.
