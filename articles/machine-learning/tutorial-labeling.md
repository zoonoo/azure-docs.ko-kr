...
### <a name="add-labelers-to-workspace"></a>작업 영역에 레이블 지정자 추가

프로젝트의 데이터에 레이블을 지정할 모든 사용자를 포함하도록 작업 영역을 설정합니다.  나중에 이러한 레이블 지정자를 특정 레이블 지정 프로젝트에 추가합니다.

1. 왼쪽에서 **데이터 레이블 지정**을 선택합니다.

1. 페이지 위쪽에서 **레이블 지정자**를 선택합니다.

1. **레이블 지정자 추가**를 선택하여 레이블 지정자의 이메일 주소를 추가합니다.

1. 완료될 때까지 레이블 지정자를 계속 추가합니다.
...
### <a name="add-labelers-to-your-project"></a>프로젝트에 레이블 지정자 추가

이 프로젝트에 레이블 지정자의 일부 또는 전체를 추가합니다.

1. 열려는 프로젝트의 이름을 선택합니다.  

1. 페이지 위쪽에서 **팀**을 선택합니다.

1. **labeling_tutorial 기본 팀** 링크를 선택합니다.

1. 이제 **레이블 지정자 할당**을 사용하여 이 프로젝트에 참여시키려는 레이블 지정자를 추가합니다. 

1. 이전에 만든 레이블 지정자의 목록에서 선택합니다.  사용하려는 레이블 지정자가 모두 선택되면 **레이블 지정자 할당**을 선택하여 기본 프로젝트 팀에 추가합니다.

## <a name="start-labeling"></a>레이블 지정 시작

Azure 리소스가 설정되고 데이터 레이블 지정 프로젝트가 구성되었습니다. 이제는 데이터에 레이블을 추가할 차례입니다.

### <a name="notify-labelers"></a>레이블 지정자에 알림

레이블을 지정할 이미지가 많은 경우 작업을 완료할 레이블 지정자도 많이 있어야 합니다.  이제 데이터에 액세스하고 레이블 지정을 시작할 수 있도록 지침을 보내려고 합니다.

1. [Machine Learning Studio](https://ml.azure.com)의 왼쪽에서 **데이터 레이블 지정**을 선택하여 프로젝트를 찾습니다.  

1. 프로젝트 이름 링크를 선택합니다.

1. 페이지 위쪽에서 **세부 정보**를 선택합니다.  프로젝트 요약이 표시됩니다.

    ![프로젝트 세부 정보](media/tutorial-labeling/project-details.png)

1. 레이블 지정자에 보낼 **레이블 지정 포털 URL** 링크를 복사합니다.

1. 이제 위쪽에서 **팀**을 선택하여 레이블 지정 팀을 찾습니다.  

1. 팀 이름 링크를 선택합니다.

1. 페이지 위쪽에서 **이메일 팀**을 선택하여 이메일을 시작합니다.  방금 복사한 레이블 지정 포털 URL을 붙여넣습니다.  

레이블 지정자가 포털 URL로 이동할 때마다 큐가 비워질 때까지 레이블을 지정할 이미지가 더 많이 표시됩니다.  

### <a name="tag-the-images"></a>이미지 태그 지정

자습서의 이 부분에서는 역할을 *프로젝트 관리자*에서 *레이블 지정자*로 전환합니다.  팀에 보낸 URL을 사용합니다.  이 URL은 프로젝트에 대한 레이블 지정 포털로 이동합니다.  지침이 추가된 경우 해당 페이지에 가보면 여기에 해당 지침이 표시됩니다.

1. 페이지 위쪽에서 **작업**을 선택하여 레이블 지정을 시작합니다.
