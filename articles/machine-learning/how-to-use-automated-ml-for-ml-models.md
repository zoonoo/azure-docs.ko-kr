...
데이터 가드레일은 자동으로 적용되어 데이터 관련 잠재적인 문제(예: 누락된 값, 클래스 불균형)를 식별하고 개선된 결과를 위해 시정 조치를 취할 수 있도록 도와줍니다. 신뢰할 수 있는 결과를 얻기 위해 사용할 수 있고 적용할 수 있는 많은 모범 사례가 있습니다. 

다음 표에서는 현재 지원되는 데이터 가드레일과 실험을 제출할 때 사용자가 접할 수 있는 관련 상태에 대해 설명합니다.

가드 레일|상태|트리거&nbsp;&nbsp;조건
---|---|---
&nbsp;누락된&nbsp;값 대평가 |**통과** <br> <br> **고정**|    입력&nbsp;열에 누락된 값 없음 <br> <br> 일부 열에 누락된 값이 있습니다.
교차 유효성 검사|**완료**|명시적 유효성 검사 집합이 제공되지 않은 경우
높은&nbsp;카디널리티&nbsp;기능&nbsp;감지|    **통과** <br> <br>**완료**|    높은 카디널리티 기능이 검색되지 않았습니다. <br><br> 높은 카디널리티 입력 열이 검색되었습니다.
클래스 밸런스 감지    |**통과** <br><br><br>**경고** |수업은 교육 데이터에서 균형을 이룹니다. 각 클래스가 샘플의 수와 비율로 측정된 데이터 집합에서 양호한 표현을 하는 경우 데이터 집합이 균형잡힌 것으로 간주됩니다. <br> <br> 교육 데이터의 클래스가 불균형
타임시리즈 데이터 일관성|**통과** <br><br><br><br> **고정** |<br> 선택한 {수평선, 지연, 롤링 창} 값을 분석했으며 메모리 부족 문제가 발견되지 않았습니다. <br> <br>선택한 {수평선, 지연, 롤링 창} 값이 분석되어 실험에 메모리가 부족할 수 있습니다. 지연 또는 롤링 창이 꺼져 있습니다.
...
1. **배포 모델** 창을 채웁니다.
    ...
    필드| 값
    ----|----
    이름| 배포에 고유한 이름을 입력합니다.
    설명| 설명을 입력하여 이 배포의 용도를 더 잘 식별할 수 있습니다.
