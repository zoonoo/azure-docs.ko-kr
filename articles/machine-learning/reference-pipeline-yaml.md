...
| YAML 키 | Description |
| ---- | ---- |
| `type` | 매개 변수의 값 형식입니다. `string`유효한 유형은, `int`, `float`, `bool`또는 `datapath`입니다. |
| `default` | 기본값입니다. |
...
각 데이터 참조는 키에 포함 되어 있습니다. 예를 들어 다음 YAML 코드 조각은 라는 `employee_data`키에 저장 된 데이터 참조를 정의 합니다.
...
| YAML 키 | Description |
| ----- | ----- |
| `script_name` | 에 상대적인 `source_directory`U-SQL 스크립트의 이름입니다. |
...
| `vm_image_urn` | 가 `create_pool` 이 `True`고 VM이를 `VirtualMachineConfiguration`사용 하는 경우 |
...
| `inputs` | 입력은 [Inputportbinding](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.graph.inputportbinding?view=azure-ml-py), [datareference](#data-reference), [portdatareference](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.portdatareference?view=azure-ml-py), [PipelineData](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.pipelinedata?view=azure-ml-py), [Dataset](https://docs.microsoft.com/python/api/azureml-core/azureml.core.dataset%28class%29?view=azure-ml-py), [datasetdefinition](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_definition.datasetdefinition?view=azure-ml-py)또는 [PipelineDataset](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.pipelinedataset?view=azure-ml-py)수 있습니다. |
| `outputs` | 출력은 [PipelineData](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.pipelinedata?view=azure-ml-py) 또는 [outputportbinding](https://docs.microsoft.com/python/api/azureml-pipeline-core/azureml.pipeline.core.graph.outputportbinding?view=azure-ml-py)중 하나일 수 있습니다. |
| `script_name` | 에 `source_directory`상대적인 Python 스크립트의 이름입니다. |
...
| YAML 키 | Description |
| ----- | ----- |
| `steps` | 하나 이상의 PipelineStep 정의 시퀀스입니다. 한 단계의 `destination` 키는 다음 단계의에 대 한 `outputs` `source` 키 `inputs` 가 됩니다.| 
...
| `path_on_datastore` | 선택 사항입니다. 수정/추가 된 blob을 모니터링할 데이터 저장소의 경로입니다. 경로는 데이터 저장소의 컨테이너 아래에 있으므로 일정 모니터링의 실제 경로는 컨테이너/`path_on_datastore`입니다. 없는 경우 데이터 저장소 컨테이너가 모니터링 됩니다. 의 `path_on_datastore` 하위 폴더에 대 한 추가/수정 내용이 모니터링 되지 않습니다. 데이터 저장소 일정에만 지원 됩니다. |
...
| `hours` | 가 `frequency` `"Day"` 또는 `"Week"`인 경우에는 0에서 23 사이의 정수를 쉼표로 구분 하 여 파이프라인을 실행 해야 하는 시간으로 지정할 수 있습니다. 또는 `time_of_day` `hours` 만 사용할 `minutes` 수 있습니다. |
| `minutes` | `frequency` 가 `"Day"` 또는 `"Week"`이면 파이프라인이 실행 되는 시간 (분)으로 0에서 59 사이의 정수 하나 이상을 쉼표로 구분 하 여 지정할 수 있습니다. 또는 `time_of_day` `hours` 만 사용할 `minutes` 수 있습니다. |
| `time_of_day` | 이 `frequency` `"Day"` 또는 `"Week"`인 경우 일정을 실행할 시간을 지정할 수 있습니다. 값의 문자열 형식은 `hh:mm`입니다. 또는 `time_of_day` `hours` 만 사용할 `minutes` 수 있습니다. |
| `week_days` | `frequency` 가 `"Week"`이면 일정을 실행 해야 할 때 하나 이상의 날짜를 쉼표로 구분 하 여 지정할 수 있습니다. 유효한 값은 `"Monday"`, `"Tuesday"`, `"Wednesday"`, `"Thursday"` `"Friday"` `"Saturday"`,, 및 `"Sunday"`입니다. |
