...
-   **피어슨 상관 관계**  
    ...
    피어슨 상관 계수는 두 변수의 공 분산을 가져와 표준 편차의 곱으로 나누는 방법으로 계산 됩니다. 두 변수의 소수 자릿수 변경은 계수에 영향을 주지 않습니다.  
...
6.  **원하는 기능 수**에 대해 결과로 반환 하려는 기능 열 수를 입력 합니다.  
    ...
    - 지정 된 수의 원하는 기능이 데이터 집합의 열 수보다 많은 경우 모든 기능이 반환 됩니다. 점수가 0 인 기능도 반환 됩니다.  

    - 기능 열 보다 더 작은 결과 열을 지정 하는 경우이 기능은 내림차순으로 순위가 지정 됩니다. 상위 기능만 반환 됩니다. 

7.  파이프라인을 실행 하거나 필터 기반 기능 선택 모듈을 선택한 다음 선택 항목 **실행**을 선택 합니다.
...
+ 분석 된 기능 열의 전체 목록 및 해당 점수를 보려면 모듈을 마우스 오른쪽 단추로 클릭 합니다. **기능**을 선택 하 고 **시각화**를 선택 합니다.  

+ 기능 선택 조건에 따라 생성 된 데이터 집합을 보려면 모듈을 마우스 오른쪽 단추로 클릭 합니다. **데이터 집합**을 선택 하 고 **시각화**를 선택 합니다. 
...
-   열에 논리 (true/false) 값이 포함 된 경우 해당 값은 `True = 1` 및 `False = 0`으로 처리 됩니다.  
