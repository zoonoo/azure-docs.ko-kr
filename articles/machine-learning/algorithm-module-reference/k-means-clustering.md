...
1.  K를 **사용** 하 여 클러스터링 모듈을 실험에 추가 합니다.  
  
2.  모델을 학습 하는 방법을 지정 하려면 **강사 모드 만들기** 옵션을 선택 합니다.  
  
    -   **단일 매개 변수**: 클러스터링 모델에서 사용할 정확한 매개 변수를 알고 있으면 특정 값 집합을 인수로 제공할 수 있습니다.  
...
4.  속성 **초기화** 는 초기 클러스터 구성을 정의 하는 데 사용 되는 알고리즘을 지정 하는 데 사용 됩니다.  
    ...
    -   **첫 번째 N**: 데이터 요소의 초기 수는 데이터 집합에서 선택 되 고 초기 수단으로 사용 됩니다. 
    
         이 메서드를 *Forgy 메서드*라고도 합니다.  
  
    -   **임의**: 이 알고리즘은 클러스터의 데이터 요소를 임의로 배치한 다음 클러스터의 임의로 할당 된 지점에 대 한 중심 초기 평균을 계산 합니다. 

         이 메서드를 *임의 파티션* 메서드 라고도 합니다.  
  
    -   **K-+ +** : 이는 클러스터를 초기화 하는 기본 방법입니다.  
...
6.  **메트릭에**대해 클러스터 벡터 간 거리를 측정 하는 데 사용할 함수를 선택 하거나 새 데이터 요소와 임의로 선택한 중심 사이에 사용할 함수를 선택 합니다. Azure Machine Learning는 다음과 같은 클러스터 거리 메트릭을 지원 합니다.  
    ...
    -   **유클리드**: 유클리드 거리는 일반적으로 K를 의미 하는 클러스터링에 대 한 클러스터 분산의 측정값으로 사용 됩니다. 이 메트릭은 지점과 중심 사이의 평균 거리를 최소화 하기 때문에 선호 됩니다.
...
8.  **레이블 할당 모드**의 경우 레이블 열 (데이터 집합에 있는 경우)을 처리 해야 하는 방법을 지정 하는 옵션을 선택 합니다.  
    ...
    -   **레이블 열 무시**: 레이블 열의 값은 무시 되며 모델을 작성 하는 데 사용 되지 않습니다.
...
저장 된 모델은 모델을 저장할 때의 학습 데이터를 나타냅니다. 나중에 실험에서 사용 되는 학습 데이터를 업데이트 하는 경우 저장 된 모델은 업데이트 되지 않습니다. 
