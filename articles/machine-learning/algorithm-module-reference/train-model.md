...
description: Azure Machine Learning 서비스의 **모델 학습** 모듈을 사용 하 여 분류 또는 회귀 모델을 학습 하는 방법에 대해 알아봅니다.
...
2. 실험에 **모델 학습** 모듈을 추가 합니다.  이 모듈은 **Machine Learning** 범주에서 찾을 수 있습니다. **학습**을 확장 하 고 **모델 학습** 모듈을 실험으로 끌어 놓습니다.
...
5.  실험을 실행합니다. 데이터가 많은 경우 다소 시간이 걸릴 수 있습니다.
...
+ 다른 실험에서 모델을 사용 하려면 모델을 마우스 오른쪽 단추로 클릭 하 고 **모델 저장**을 선택 합니다. 모델의 이름을 입력 합니다. 

    그러면 실험의 반복적인 실행으로 업데이트 되지 않는 스냅숏으로 모델이 저장 됩니다.
