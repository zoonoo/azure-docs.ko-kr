...
1. 파이프라인에 **모델 학습** 모듈을 추가 합니다.  이 모듈은 **Machine Learning** 범주에서 찾을 수 있습니다. **학습** 을 확장 하 고 **모델 학습** 모듈을 파이프라인으로 끌어 놓습니다.
  
1.  왼쪽 입력에서 학습 되지 않은 모드를 연결 합니다. 학습 **모델** 의 오른쪽 입력에 학습 데이터 집합을 연결 합니다.

    학습 데이터 집합은 레이블 열을 포함 해야 합니다. 레이블이 없는 행은 무시 됩니다.
  
1.  **레이블 열** 에 대해 모듈의 오른쪽 패널에서 **열 편집** 을 클릭 하 고 모델에서 학습에 사용할 수 있는 결과를 포함 하는 단일 열을 선택 합니다.
