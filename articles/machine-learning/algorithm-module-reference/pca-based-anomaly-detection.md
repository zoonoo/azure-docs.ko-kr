...
PCA는 여러 변수를 포함 하는 데이터를 분석 하는 방식으로 작동 합니다. 변수 간의 상관 관계를 찾고 결과의 차이를 가장 잘 캡처하는 값의 조합을 결정 합니다. 이러한 결합 된 기능 값을 사용 하 여 *주요 구성 요소*라는 보다 간결한 기능 공간을 만들 수 있습니다.
...
- [임의성을 사용 하 여 구조 찾기: 확률 알고리즘을 사용 하 여 대략적인 행렬 Decompositions](http://users.cms.caltech.edu/~jtropp/papers/HMT11-Finding-Structure-SIREV.pdf) (PDF 다운로드)를 생성 합니다. Halko, Martinsson 및 Tropp.
...
2. **PCA 기반 변칙 검색** 모듈의 **속성** 창에서 **학습 모드** 옵션을 클릭 하 고, 특정 매개 변수 집합을 사용 하 여 모델을 학습 시킬 것인지 아니면 매개 변수 비우기를 사용 하 여 가장 적합 한 매개 변수를 찾을 것인지를 나타냅니다.
...
5. **입력 기능 평균 정규화 사용**: 모든 입력 기능을 평균 0으로 정규화 하려면이 옵션을 선택 합니다. PCA의 목표는 변수 간의 차이를 최대화 하기 때문에, 정규화 또는 0으로의 크기 조정은 일반적으로 PCA에 권장 됩니다.
     ...
     이 옵션은 기본적으로 선택되어 있습니다. 다른 방법이 나 소수 자릿수를 사용 하 여 값이 이미 정규화 된 경우이 옵션의 선택을 취소 합니다.
