...
description: Azure Machine Learning 서비스에서 열 변환 모듈을 사용 하 여 지정 된 데이터 집합에서와 동일한 열 하위 집합을 선택 하는 변환을 만드는 방법을 알아봅니다.
...
ms.locfileid: "73516202"
---
# <a name="select-columns-transform"></a>열 변환 선택

이 문서에서는 Azure Machine Learning 디자이너 (미리 보기)의 **열 변형 선택** 모듈을 사용 하는 방법을 설명 합니다. **열 변환 선택** 모듈의 목적은 다운스트림 기계 학습 작업에서 예측 가능 하 고 일관성 있는 열 집합이 항상 사용 되도록 하는 것입니다.
...
이 시나리오에서는 기능 선택을 사용 하 여 모델 학습에 사용 되는 동적 열 집합을 생성 하려는 경우를 가정 합니다. 점수 매기기 프로세스에서 열 선택이 동일한 지 확인 하려면 열 **변환 선택** 모듈을 사용 하 여 열 선택 항목을 캡처하고 파이프라인의 다른 위치에 적용 합니다.

1. 디자이너에서 파이프라인에 입력 데이터 집합을 추가 합니다.

2. [필터 기반 기능 선택](filter-based-feature-selection.md)의 인스턴스를 추가 합니다.

3. 모듈을 연결 하 고 기능 선택 모듈을 구성 하 여 입력 데이터 집합에서 몇 가지 최상의 기능을 자동으로 찾을 수 있습니다.

4. 학습 [모델](train-model.md) 의 인스턴스를 추가 하 고 [필터 기반 기능 선택](filter-based-feature-selection.md) 의 출력을 학습의 입력으로 사용 합니다.

    > [!IMPORTANT]
    > 기능 중요도는 열의 값에 따라 결정 되므로 [모델 학습](train-model.md)에 사용할 수 있는 열을 미리 알 수 없습니다.  
5. 이제 **열 선택 변환** 모듈의 인스턴스를 연결 합니다. 

    이렇게 하면 다른 데이터 집합에 저장 하거나 적용할 수 있는 변환으로 열 선택이 생성 됩니다. 이 단계를 수행 하면 기능 선택으로 식별 된 열이 다른 모듈에서 다시 사용할 수 있도록 저장 됩니다.

6. [모델 점수 매기기](score-model.md) 모듈을 추가 합니다. 

    **입력 데이터 집합을 연결 하지 마십시오.**

    대신, [변환 적용](apply-transformation.md) 모듈을 추가 하 고 기능 선택 변환의 출력을 연결 합니다.
