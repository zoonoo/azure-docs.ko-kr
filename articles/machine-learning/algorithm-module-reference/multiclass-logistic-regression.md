...
3. **최적화 허용 오차**에 대 한 임계값을 지정 합니다. 반복 간의 개선 사항이 임계값 보다 낮으면 알고리즘이 중지 되 고 현재 모델이 반환 됩니다.

4. **L1 정규화 weight**, **l2 정규화 가중치**: 정규화 매개 변수 L1 및 L2에 사용할 값을 입력 합니다. 0이 아닌 값은 둘 다에 권장 됩니다.
...
학습을 완료 한 후 학습에서 배운 기능 가중치와 함께 모델의 매개 변수 요약을 확인 하 고 [모델 학습](./train-model.md) 모듈의 출력을 마우스 오른쪽 단추로 클릭 한 다음 **시각화**를 선택할 수 있습니다.
