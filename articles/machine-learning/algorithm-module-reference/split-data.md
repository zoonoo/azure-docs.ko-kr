...
이 예에서는 `Text`열에서 `Gryphon` 텍스트를 포함 하는 모든 행을 첫 번째 데이터 집합에 넣습니다. **분할 된 데이터**의 두 번째 출력에 다른 행을 배치 합니다.
...
이 예에서는 데이터 집합의 두 번째 열에 있는 임의의 위치에서 지정 된 문자열을 검색 합니다. 위치는 인덱스 값 1로 표시 됩니다. 대/소문자도 일치해야 합니다.
