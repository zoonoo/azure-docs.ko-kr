...
3.  **작업** 드롭다운 목록에서 데이터 집합을 저장 하기 전에 데이터를 정리할 것인지 여부를 지정 합니다.  
    ...
    - **ReplaceValues**:이 옵션을 사용 하 여 정확 하 게 일치 하는 값으로 바꿀 단일 값을 지정 합니다. **Replace** 메서드를 설정 하 여 누락 값 또는 사용자 지정 값을 바꿀 수 있습니다.
      ...
      - **사용자 지정**: 입력 데이터 집합에서 사용자 지정 값을 바꾸려면이 옵션을 선택 합니다. **사용자 지정 값**에 찾으려는 값을 입력 합니다. 예를 들어 데이터에 누락 된 값에 대 한 자리 표시자로 사용 `obs` 문자열이 포함 된 경우 `obs`을 입력 합니다. **새 값**에 새 값을 입력 하 여 원래 문자열을로 바꿉니다.
