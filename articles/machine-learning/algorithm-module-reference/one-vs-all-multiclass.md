...
이 모듈은 여러 출력 클래스 각각에 대해 이진 모델을 만드는 일 대 다 메서드를 구현 합니다. 모듈은 이진 분류 문제인 것 처럼 해당 보수 (모델의 다른 모든 클래스)에 대해 개별 클래스에 대해 이러한 각 이진 모델을 평가 합니다. 그런 다음 모듈은 이러한 이진 분류자를 실행 하 고 신뢰도 점수가 가장 높은 예측을 선택 하 여 예측을 수행 합니다.  
