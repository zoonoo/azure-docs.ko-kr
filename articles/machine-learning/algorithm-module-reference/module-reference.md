...
| 데이터 변환 | 데이터 표준화 또는 범주화, 차원 감소, 다양 한 파일 형식 간에 데이터 변환 등 기계 학습에 고유한 데이터에 대 한 작업입니다.| [열 추가](add-columns.md) <br/> [행 추가](add-rows.md) <br/> [누락 데이터 정리](clean-missing-data.md) <br/> [CSV로 변환](convert-to-csv.md) <br/> [데이터 집합으로 변환](convert-to-dataset.md) <br/> [메타 데이터 편집](edit-metadata.md) <br/> [데이터 조인](join-data.md) <br/> [데이터 정규화](normalize-data.md) <br/> [중복 행 제거](remove-duplicate-rows.md) <br/> [열 변환 선택](select-columns-transform.md) <br/> [데이터 집합에서 열 선택](select-columns-in-dataset.md) |
| 샘플링 | 데이터를 하나 이상의 하위 집합 하위 집합으로 분할 하 여 기계 학습 모델을 학습 하 고 테스트 합니다.  | [모델 교차 유효성 검사](cross-validate-model.md) <br/> [파티션 및 샘플](partition-and-sample.md) <br/> [SMOTE](smote.md) <br/> [데이터 분할](split-data.md) |
...
| Python 및 R | 코드를 작성 하 고이를 모듈에 포함 하 여 Python 및 R을 파이프라인과 통합 합니다. | [Python 모델 만들기](create-python-model.md) <br/> [Python 스크립트 실행](execute-python-script.md)   <br/>  [R 스크립트 실행](execute-r-script.md)
...
|  | **모델 작성 및 평가**: | |
| 교육   | 알고리즘을 통해 데이터를 실행 합니다. | [모델 학습](train-model.md)  <br/> [클러스터링 모델 학습](train-clustering-model.md) <br/>  [모델 하이퍼 매개 변수 조정](tune-model-hyperparameters.md) |
...
| 통계 함수 | 데이터 과학과 관련 된 다양 한 통계 방법을 제공 합니다. | [수학 연산 적용](apply-math-operation.md) <br/> [데이터 요약](summarize-data.md)|
