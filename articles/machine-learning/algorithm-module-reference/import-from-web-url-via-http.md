---
title: 'HTTP: Module Reference를 통해 웹 URL에서 가져오기'
titleSuffix: Azure Machine Learning service
description: Azure Machine Learning 서비스의 HTTP 모듈을 통해 웹 URL에서 가져오기를 사용 하 여 Machine Learning 파이프라인에서 사용 하기 위해 공용 웹 페이지에서 데이터를 읽는 방법을 알아봅니다.
...
이 모듈을 사용 하 여 machine learning 파이프라인에서 사용 하기 위해 공용 웹 페이지의 데이터를 읽을 수 있습니다.
...
1. 파이프라인에 **데이터 가져오기** 모듈을 추가 합니다. 인터페이스의 **데이터 입력 및 출력** 범주에서 모듈을 찾을 수 있습니다.
...
1. 파이프라인에 [데이터 가져오기](import-data.md) 모듈을 추가 합니다. 인터페이스의 **데이터 입력 및 출력** 범주에서 모듈을 찾을 수 있습니다.
...
6. 데이터를 크게 변경 하지 않을 경우 또는 파이프라인을 실행할 때마다 데이터를 다시 로드 하지 않으려는 경우에는 **캐시 된 결과 사용** 옵션을 선택 합니다.

    이 옵션을 선택 하면 모듈이 처음 실행 될 때 파이프라인이 데이터를 로드 한 다음 캐시 된 버전의 데이터 집합을 사용 합니다.

    파이프라인 데이터 집합의 각 반복에서 데이터 집합을 다시 로드 하려면 **캐시 된 결과 사용** 옵션을 선택 취소 합니다. [데이터 가져오기](import-data.md)의 매개 변수가 변경 된 경우에도 결과가 다시 로드 됩니다.
