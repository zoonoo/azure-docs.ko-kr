...
다음 단계를 수행 하 여 디자이너에서 오류 메시지를 찾을 수 있습니다.  

- 실패 한 모듈을 선택 하 고 **출력 + 로그** 탭으로 이동 하 여 **azureml 로그** 범주 아래의 **70_driver_log .txt** 파일에서 자세한 로그를 찾을 수 있습니다.

- 자세한 모듈 오류는 **module_statistics** 범주 아래의 error_info json에서 확인할 수 있습니다.

다음은 디자이너의 모듈에 대 한 오류 코드입니다.
...
|이름 또는 인덱스 "{column_names}"이 (가) 있는 열이 "{arg_name_missing_column}"에 없습니다.|
|이름 또는 인덱스 "{column_names}"의 열이 "{arg_name_missing_column}"에 없지만 "{arg_name_has_column}"에 있습니다.|
...
|매개 변수 "{arg_name}" 값 {actual_value}은 (는) {upper_boundary_meaning} 값 {upper_boundary} 보다 작거나 같아야 합니다.|
...
|열 이름은 입력 데이터 집합 {col_index} (0부터 시작) ({dataset1} 및 {dataset2})의 열 {0}과 (와) 동일 하지 않습니다.|
...
|모든 회귀 모델|[모델 학습](train-model.md) |[모델 채점](score-model.md)|
...
## <a name="error-0017"></a>오류 0017  
 ...
 예를 들어, 열 선택이 모듈에서 처리할 수 없는 데이터 형식의 열을 포함 하는 경우 (예: 수치 연산에 대 한 문자열 열 또는 범주 기능 열이 필요한 점수 열) Azure Machine Learning에서이 오류가 나타날 수 있습니다.  
...
|{dataset1}이 (가) 잘못 되었습니다 ({reason}). {troubleshoot_hint}|
...
|"{Arg_name}" 인수의 값이 "{sorting_order}" 순서로 정렬 되어 있지 않습니다.|
...
Matchbox 추천에는 항목 기능 또는 사용자 기능 중 하나를 사용 하는 경우 충족 해야 하는 특정 요구 사항이 있습니다.  이 오류는 입력으로 제공한 사용자 또는 항목에 대 한 기능 벡터가 없음을 나타냅니다. 각 사용자 또는 항목에 대 한 데이터에서 기능 벡터를 사용할 수 있는지 확인 합니다.  
 ...
 예를 들어 사용자의 나이, 위치 또는 수입 등의 기능을 사용 하 여 권장 사항 모델을 학습 했지만 학습 중에 표시 되지 않은 새 사용자에 대 한 점수를 만들려는 경우 해당 사용자에 대 한 적절 한 예측을 수행 하기 위해 새 사용자에 대해 이와 동등한 기능 집합 (예: 나이, 위치 및 수입 값)을 제공 해야 합니다. 
...
|학습 데이터 {data_name}에서 "{action_name}" 작업을 적용 하지 못했습니다. 이유: {reason}.|
...
해결 방법: 
 ...
 열 선택 영역에 레이블 열을 포함 하지만 인식 되지 않는 경우 [메타 데이터 편집](edit-metadata.md) 모듈을 사용 하 여 레이블 열로 표시 합니다.
  ...
  그런 다음 [누락 된 데이터 정리](clean-missing-data.md) 모듈을 사용 하 여 레이블 열에 누락 값이 있는 행을 제거할 수 있습니다. 
...
|예외 메시지|
|------------------------|
|데이터 프레임 열 이름은 문자열 형식 이어야 합니다. 열 이름은 문자열이 아닙니다.|
|데이터 프레임 열 이름은 문자열 형식 이어야 합니다. 열 이름 {column_names}은 (는) 문자열이 아닙니다.|
...
## <a name="error-0158"></a>오류 0158
 변환 디렉터리가 잘못 된 경우 throw 됩니다.

|예외 메시지|
|------------------------------------------------------------|
|지정 된 TransformationDirectory가 잘못 되었습니다.|
|TransformationDirectory "{arg_name}"이 (가) 잘못 되었습니다. 이유: {reason}. 변환 파일을 생성 하는 학습 실험을 다시 실행 하세요. 학습 실험을 삭제 한 경우 변환 파일을 다시 만들고 저장 하세요.|
|TransformationDirectory "{arg_name}"이 (가) 잘못 되었습니다. 이유: {reason}. {troubleshoot_hint}|


## <a name="error-0159"></a>오류 0159
 모듈 모델 디렉터리로 전달 된가 잘못 된 경우 예외가 발생 합니다. 

|예외 메시지|
|------------------------------------------------------------|
|지정 된 ModelDirectory가 잘못 되었습니다.|
|ModelDirectory "{arg_name}"이 (가) 잘못 되었습니다.|
|ModelDirectory "{arg_name}"이 (가) 잘못 되었습니다. 이유: {reason}.|
|ModelDirectory "{arg_name}"이 (가) 잘못 되었습니다. 이유: {reason}. {troubleshoot_hint}|
