...
description: 오류 코드를 사용하여 Azure Machine Learning 디자이너의 모듈 예외 문제 해결
...
## <a name="error-0159"></a>오류 0159
 모듈에 전달된 모델 디렉터리가 올바르지 않으면 예외가 발생합니다. 
...
자세한 도움말을 보려면 해당 오류와 함께 표시되는 세부 메시지를 시나리오에 대한 설명(입력으로 사용되는 데이터를 포함)과 함께 Azure Machine Learning 포럼에 게시하는 것이 좋습니다. 이 피드백은 오류의 우선 순위를 지정하고 추가 작업을 위해 가장 중요한 문제를 식별하는 데 도움이 됩니다.  
