...
|속성|Type|범위|Optional|기본값|Description|  
|----------|----------|-----------|--------------|-------------|-----------------|  
...
|일반화|부울|True 또는 False|필수|true|출력을 확률로 정규화 합니다.  변환 된 데이터 집합은 P (토픽&#124;문서)이 되 고 기능 토픽 행렬은 P (word&#124;항목)입니다.|  
|모든 옵션 표시|부울|True 또는 False|필수|False|Scikit에 특정 한 추가 매개 변수를 제공 합니다. 온라인 학습 LDA|  
|가는 매개 변수|Float|[0.00001; 1.0]|**모든 옵션 표시** 확인란이 선택 된 경우 적용 됩니다.|0.01|항목의 이전 배포 항목|  
|알파 매개 변수|Float|[0.00001; 1.0]|**모든 옵션 표시** 확인란이 선택 된 경우 적용 됩니다.|0.01|문서 항목 이전 배포|  
|예상 문서 수|정수|[1;int.MaxValue]|**모든 옵션 표시** 확인란이 선택 된 경우 적용 됩니다.|1000|예상 문서 수 (total_samples 매개 변수에 해당)|  
...
|기능 해시에 사용할 비트 수|정수|[1, 31]|**모든 옵션 표시** 확인란을 선택 *하지 않은* 경우 적용 되 고 **Ngrams의 빌드 사전이** False 인 경우 적용 됩니다.|12|기능 해시에 사용할 비트 수| 
|LDA 전에 ngrams의 사전 빌드|부울|True 또는 False|**모든 옵션 표시** 확인란이 선택 된 경우 적용 됩니다.|True|LDA 전에 ngrams 사전을 빌드합니다. 모델 검사 및 해석에 유용|  
|사전에 있는 최대 n 그램 수|정수|[1;int.MaxValue]|**모든 옵션 표시** 확인란을 선택 하 고 **Ngrams의 빌드 사전** 옵션을 True로 설정 하는 경우에 적용 됩니다.|20000|사전의 최대 크기입니다. 입력의 토큰 수가이 크기를 초과 하는 경우 충돌이 발생할 수 있습니다.|  
|해시 비트 수|정수|[1, 31]|**모든 옵션 표시** 확인란이 선택 되어 있고 **Ngrams의 빌드 사전** 옵션이 False 인 경우에 적용 됩니다.|12|기능 해시 중에 사용할 비트 수|   
