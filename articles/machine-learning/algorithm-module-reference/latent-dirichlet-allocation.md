...
|Normalize|부울|True 또는 False|필수|true|출력을 확률로 정규화 합니다.  변환 된 데이터 집합은 P (토픽&#124;문서)이 되 고 기능 토픽 행렬은 P (word&#124;토픽)가 됩니다.|  
|모든 옵션 표시|부울|True 또는 False|필수|거짓|Scikit에 고유한 추가 매개 변수를 제공 합니다 (온라인 학습 LDA).|  
...
|Ngrams의 빌드 사전|부울|True 또는 False|[ **모든 옵션 표시** ] 확인란이 선택 *되지 않은* 경우에 적용 됩니다.|참|LDA을 계산 하기 전에 ngrams 사전을 작성 합니다. 모델 검사 및 해석에 유용 합니다.|  
...
|LDA 전에 ngrams의 사전 빌드|부울|True 또는 False|[ **모든 옵션 표시** ] 확인란이 선택 된 경우 적용 됩니다.|참|LDA 전에 ngrams 사전을 빌드합니다. 모델 검사 및 해석에 유용 합니다.|  
