...
일반적으로 단어를 벡터 또는 단어 벡터화로 변환 하는 것은 NLP (자연어 처리) 프로세스입니다. 이 프로세스에서는 언어 모델 또는 기술을 사용 하 여 단어를 벡터 공간에 매핑합니다. 즉, 각 단어를 실수의 벡터로 나타냅니다. 그와 같은 의미를 가진 단어는 비슷한 표현을 가질 수 있습니다.

Word 포함는 텍스트 분류 및 감정 분석과 같은 NLP 다운스트림 작업의 초기 입력으로 사용할 수 있습니다.
...
+ Word2Vec는 단순 신경망을 사용 하 여 단어 포함을 배우는 가장 인기 있는 기술 중 하나입니다. 이 문서에서는이 문서에서 설명 합니다 .이 문서에서는 PDF 다운로드로 사용할 수 있습니다. [벡터 공간에서의 단어 표시에 대 한 효율적인 예측은 Mikolov, Tomas, et](https://arxiv.org/pdf/1301.3781.pdf)에서 설명 합니다. 이 모듈의 구현은 [Word2Vec 용 Gensim 라이브러리](https://radimrehurek.com/gensim/models/word2vec.html)를 기반으로 합니다.

+ FastText 이론은이 문서에 설명 되어 있습니다 .이 문서는 PDF 다운로드로 사용할 수 있습니다. [부분 단어까지 정보를 사용 하 여 단어 벡터 보강, Bojanowski, Pi r, et al](https://arxiv.org/pdf/1607.04606.pdf). 이 모듈의 구현은 [FastText에 대 한 Gensim 라이브러리](https://radimrehurek.com/gensim/models/fasttext.html)를 기반으로 합니다.
...
5. **Word2Vec 전략이** **gensim Word2Vec** 또는 **gensim fasttext**인 경우:
    ...
    + **Epoch 수**에 대해 모음에 대 한 epoch (반복) 수를 지정 합니다. 이 설정은 `iter` Gensim의 매개 변수에 해당 합니다.

        기본 epoch 번호는 5입니다.

6. **최대 어휘 크기**에 대해 생성 된 어휘에 있는 단어의 최대 개수를 지정 합니다.

    이 보다 고유한 단어가 더 있으면 자주 발생 하지 않는 단어를 정리 합니다.
...
데이터 집합은 위키백과에서 가져온 전체 텍스트와 함께 범주 열을 포함 합니다. 이 표에서는 몇 가지 대표적인 예제만 보여 줍니다.
...
+ 온라인 학습 및 미리 학습 된 모델의 차이점:
    ...
    글러브 미리 학습 된 모델이 단어 벡터화 전략으로 선택 된 경우 입력 데이터 집합에서 어휘를 요약 하 고 사전 학습 된 모델의 각 단어에 대해 포함 벡터를 생성 합니다. 온라인 교육이 없으면 사전 학습 된 모델을 사용 하 여 학습 시간을 절약할 수 있습니다. 특히 입력 데이터 집합 크기가 비교적 작은 경우 성능이 향상 됩니다.

+ 포함 크기:

    일반적으로 단어 포함의 길이는 좋은 성능을 얻기 위해 몇 백 (예: 100, 200, 300)로 설정 됩니다. 그 이유는 작은 포함 크기가 작은 벡터 공간을 의미 하므로 단어 포함 충돌이 발생할 수 있기 때문입니다.  

    미리 학습 된 모델의 경우 단어 포함의 길이는 고정 되어 있습니다. 이 구현에서 글러브-gigaword-100의 포함 크기는 100입니다.
