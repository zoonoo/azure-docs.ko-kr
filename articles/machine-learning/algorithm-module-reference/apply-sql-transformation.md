...
description: Azure Machine Learning 서비스의 SQL 변환 적용 모듈을 사용 하 여 입력 데이터 집합에 대해 SQLite 쿼리를 실행 하 여 데이터를 변환 하는 방법에 대해 알아봅니다.
...
-   결과에 대 한 테이블을 만들고 이식 가능한 데이터베이스에 데이터 집합을 저장 합니다.  
  
-   데이터 형식에 대 한 사용자 지정 변환을 수행 하거나 집계를 만듭니다.  
  
-   SQL 쿼리 문을 실행 하 여 데이터를 필터링 하거나 변경 하 고 쿼리 결과를 데이터 테이블로 반환 합니다.  
...
이 모듈은 SQLite 구문의 모든 표준 문을 지원 합니다. 지원 되지 않는 문의 목록은 [기술 참고 사항](#technical-notes) 섹션을 참조 하세요.
...
-   공백이 나 기타 특수 문자를 포함 하는 열 식별자의 경우에는 `SELECT` 또는 `WHERE` 절의 열을 참조할 때 항상 열 식별자를 대괄호 또는 큰따옴표로 묶어야 합니다.  
  
### <a name="unsupported-statements"></a>지원 되지 않는 문  

SQLite는 ANSI SQL 표준의 대부분을 지원 하지만 상업용 관계형 데이터베이스 시스템에서 지원 되는 많은 기능을 포함 하지 않습니다. 자세한 내용은 [SQLite의 이해](http://www.sqlite.org/lang.html)를 참조 하십시오. 또한 SQL 문을 만들 때 다음 제한 사항에 유의 해야 합니다.  
