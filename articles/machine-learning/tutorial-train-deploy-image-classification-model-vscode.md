---
title: '자습서: Azure Machine Learning Visual Studio Code 확장을 사용하여 이미지 분류 TensorFlow 모델 학습 및 배포'
...
1. 기본적으로 만든 날짜와 시간이 포함된 이름이 생성됩니다. 명령 팔레트에서 이름을 "TeamWorkspace"로 변경하고 **Enter** 키를 누릅니다.
1. 명령 팔레트에서 **새 리소스 그룹 만들기**를 선택합니다. 
1. 명령 팔레트 텍스트 상자에서 "TeamWorkspace-rg"를 입력하고 **Enter** 키를 누릅니다. 
1. 명령 팔레트에서 작업 영역의 위치를 선택합니다. 모델을 배포하려는 위치와 가장 가까운 위치를 선택하는 것이 좋습니다. 이 경우 **미국 서부 2**를 선택합니다.
1. 작업 영역 SKU를 선택하라는 메시지가 표시되면 **기본**을 선택하여 기본 작업 영역을 만듭니다. 다양한 작업 영역 제품에 대한 자세한 내용은 [Azure Machine Learning 개요](./overview-what-is-azure-ml.md#sku)를 참조하세요.
...
1. 명령 팔레트 프롬프트에서 실험 이름을 "MNIST"로 지정하고, **Enter** 키를 눌러 새 실험을 만듭니다. 
...
1. VM 크기를 선택합니다. 명령 팔레트 프롬프트에서 **Standard_F2s_v2**를 선택합니다. VM 크기는 모델을 학습시키는 데 걸리는 시간에 영향을 줍니다. VM 크기에 대한 자세한 내용은 [Azure에서 Linux 가상 머신에 대한 크기](https://docs.microsoft.com/azure/virtual-machines/linux/sizes)를 참조하세요.
1. 명령 팔레트 프롬프트에서 컴퓨팅 이름을 "TeamWkspc-com"으로 지정하고 **Enter** 키를 눌러 컴퓨팅을 만듭니다.
...
1. 명령 팔레트 프롬프트에서 실행 구성의 이름을 "MNIST-rc"로 지정하고 **Enter** 키를 눌러 컴퓨팅을 만듭니다.
...
1. 명령 팔레트 프롬프트에서 다음을 입력하여 필요한 패키지를 지정합니다.
...
1. 명령 팔레트에서 **TeamWkspc-com** 컴퓨팅 대상을 선택합니다.
...
1. 명령 팔레트에서 모델 이름을 "MNIST-TensorFlow-model"로 지정하고 **Enter** 키를 누릅니다.
1. TensorFlow 모델은 여러 개의 파일로 구성됩니다. 명령 팔레트에서 모델 경로 형식으로 **모델 폴더**를 선택합니다. 
1. `azureml_outputs/Run_1/outputs/Run_1/outputs/outputs/model` 디렉터리를 선택합니다.
...
1. 명령 팔레트에서 **Azure Container Instances**를 선택합니다.
1. 명령 팔레트에서 서비스 이름을 "mnist-tensorflow-svc"로 지정하고 **Enter** 키를 누릅니다.
1. 명령 팔레트에서 **Enter** 키를 누르고 `mnist-vscode-docs-sample` 디렉터리에서 `score.py` 파일을 찾아서 컨테이너에서 실행할 스크립트를 선택합니다.
1. 명령 팔레트에서 **Enter** 키를 누르고 `mnist-vscode-docs-sample` 디렉터리에서 `env.yml` 파일을 찾아서 스크립트를 실행하는 데 필요한 종속성을 제공합니다.
