---
title: '자습서: 디자이너를 사용 하 여 자동차 가격 예측'
...
ms.locfileid: "78396201"
---
# <a name="tutorial-predict-automobile-price-with-the-designer-preview"></a>자습서: 디자이너를 사용 하 여 자동차 가격 예측 (미리 보기)
...
>
>이를 찾으려면 작업 영역의 디자이너로 이동합니다. **새 파이프라인** 섹션에서 **Sample 1-회귀: 자동차 가격 예측 (기본)** 을 선택 합니다.
...
이제 파이프라인이 모두 설정 되었으므로 파이프라인 실행을 제출 하 여 machine learning 모델을 학습 시킬 수 있습니다. 디자이너에서 파이프라인을 빌드하는 동안 언제 든 지 파이프라인 실행을 제출할 수 있습니다. 이 작업을 수행 하 여 진행 중인 작업을 확인 하 고 예상 대로 파이프라인 함수를 확인할 수 있습니다.
...
* **MAE (절대 평균 오차)** : 절대 오류의 평균입니다. 오차는 예측 값과 실제 값 사이의 차이입니다.
* **RMSE (제곱 평균 제곱 오차)** : 테스트 데이터 집합에 대해 수행 된 예측의 평균 제곱 오차의 제곱근입니다.
...
* **결정 계수**: R 제곱 값이 라고도 하는이 통계 메트릭은 모델이 데이터에 얼마나 적합 한지를 나타냅니다.
