...
ms.locfileid: "46997322"
---
# <a name="tutorial-2-classify-iris---build-a-model"></a>자습서 2: 아이리스 분류 - 모델 구축

[!INCLUDE [workbench-deprecated](../../../includes/aml-deprecating-preview-2017.md)]

Azure Machine Learning 서비스(미리 보기)는 데이터를 준비하고, 실험을 개발하고, 클라우드 규모로 모델을 배포하는 전문 데이터 과학자를 위한 데이터 과학 및 고급 분석 통합 솔루션입니다.

이 자습서는 **3부로 구성된 시리즈 중 제2부**입니다. 이 자습서 부분에서는 Azure Machine Learning 서비스를 사용하여 다음을 수행합니다.
...
- 로컬에 설치되어 실행되는 Docker 엔진. Docker의 Community Edition 버전이면 충분합니다. Docker의 설치 방법은 https://docs.docker.com/engine/installation/을 참조하세요.
...
3. **iris_sklearn.py** Python 스크립트 파일을 선택합니다. 
   ...
   Workbench의 새 텍스트 편집기 탭에 코드가 표시됩니다. 이 스크립트를 본 자습서 파트에서 계속 사용합니다. 

   >[!NOTE]
   >이 샘플 프로젝트는 자주 업데이트되므로 사용자에게 표시되는 코드가 위의 코드와 완전히 같지 않을 수도 있습니다.
   
   ![파일 열기](media/tutorial-classifying-iris/open_iris_sklearn.png)

4. Python 스크립트 코드를 확인하여 코딩 스타일을 파악합니다. 
   ...
   * [scikit-learn](http://scikit-learn.org/stable/index.html) 기계 학습 라이브러리를 사용하여 로지스틱 회귀 모델을 구축합니다.  이 라이브러리는 Azure Machine Learning Workbench와 함께 기본 제공됩니다.
   ...
   * [matplotlib](https://matplotlib.org/) 라이브러리를 사용하여 두 개의 그래프, 즉 혼동 행렬과 다중 클래스 ROC(Receiver Operating Characteristic: 수신기 조작 특성) 곡선을 그린 다음, `outputs` 폴더에 저장합니다. 사용자 환경에 이 라이브러리가 아직 존재하지 않는 경우 라이브러리를 설치할 수 있습니다.

   * 실행 기록에 정규화 속도 및 모델 정확도를 자동으로 표시합니다. `run_logger` 개체는 정규화 속도 및 모델 정확도를 처음부터 끝까지 로그에 기록하는 데 사용됩니다. 


## <a name="run-irissklearnpy-in-your-local-environment"></a>로컬 환경에서 Iris_sklearn.py 실행
...
1. Azure Machine Learning CLI(명령줄 인터페이스)를 시작합니다.
   ...
   >[!IMPORTANT]
   >다음 단계를 계속 수행하려면 이 CLI 창을 사용해야 합니다.

1. Python 플롯팅 라이브러리, **matplotlib**이 아직 설치되지 않은 경우 CLI 창에서 이 라이브러리를 설치합니다.

   **iris_sklearn.py** 스크립트는 두 개의 Python 패키지 **scikit-learn** 및 **matplotlib**에 의존합니다.  **scikit-learn** 패키지는 사용자의 편의를 위해 Azure Machine Learning Workbench에 의해 설치되었습니다. 그러나 **matplotlib**의 경우 아직 설치되어 있지 않으면 직접 설치해야 합니다.

   **matplotlib**를 설치하지 않고 계속 진행해도 이 자습서의 코드는 성공적으로 실행됩니다. 그러나 코드는 기록 시각화에 표시된 대로 혼동 행렬 출력 및 다중 클래스 ROC 곡선 그래프를 생성할 수 없습니다.
...
1. **iris_sklearn.py**라는 탭을 찾습니다. 
   ...
   ![스크립트가 있는 탭 찾기](media/tutorial-classifying-iris/2-iris_sklearn-tab.png)

1. 해당 탭의 도구 모음에서 **로컬**을 실행 환경으로 선택하고 `iris_sklearn.py`을 실행 대상 스크립트로 선택합니다. 이 옵션들은 이미 선택되어 있을 수 있습니다.

   ![로컬 및 스크립트 선택](media/tutorial-classifying-iris/2-local-script.png)

1. 도구 모음의 오른쪽으로 이동하여 **인수** 필드에 `0.01`을 입력합니다. 
...
1. **작업** 창의 작업 상태 텍스트에서 **완료됨**을 선택합니다. 
   ...
   팝업 창이 열리고 실행에 대한 표준 출력(stdout) 텍스트가 표시됩니다. stdout(표준 출력) 텍스트를 닫으려면 팝업 창의 오른쪽 위에 있는 **닫기**(**x**) 단추를 선택합니다.

   ![표준 출력](media/tutorial-classifying-iris/2-standard-output.png)

9. **작업** 창의 동일한 작업 상태에서 **완료됨** 상태 및 시작 시간 바로 위의 파란색 **iris_sklearn.py [n]**(_n_은 실행 횟수) 텍스트를 선택합니다. **실행 속성** 창이 열리고 해당되는 특정 실행에 대한 다음 정보가 표시됩니다.
   - **실행 속성** 정보
   - **Outputs**
   - **Metrics**(메트릭)
   - **시각화**(있는 경우)
   ...
   >[!NOTE]
   >이 자습서에서는 트레이닝 세트에 이미 임의성이 적용되었으므로 실제 결과는 여기에 표시된 결과와 다를 수 있습니다.
...
1. **실행 대시보드** 탭이 열립니다. 
   ...
   여러 번 실행에 걸쳐 캡처된 통계 정보를 검토합니다. 그래프는 탭의 위쪽에서 렌더링합니다. 각 실행에는 연속 번호가 있으며 실행 세부 정보는 화면 맨 아래의 테이블에 나열됩니다.
...
1. 개별 실행을 선택하여 실행 세부 정보 보기를 표시합니다. 선택한 실행에 대한 통계는 **실행 속성** 섹션에 나열됩니다. 출력 폴더에 기록된 파일은 **출력** 섹션에 나열되고 여기서 파일을 다운로드할 수 있습니다.
...
>[!NOTE]
>원격 Azure VM 또는 Azure HDInsight Spark 클러스터의 Docker 컨테이너에서 실행할 스크립트를 디스패치하여 실험하려면, [Ubuntu 기반 Azure 데이터 과학 Virtual Machine 또는 HDInsight 클러스터를 만드는 방법](how-to-create-dsvm-hdi.md)을 확인하세요.

1. 아직 실행 환경이 구성되어 있지 않은 경우 Windows 또는 MacOS 컴퓨터에 Docker를 로컬로 설치하고 시작합니다. 자세한 내용은 https://docs.docker.com/install/에서 Docker 설치 지침을 참조하세요. Community Edition 버전이면 충분합니다.
...
2. **docker-python**, **docker-spark** 및 **local**과 같이 미리 구성된 여러 환경이 있습니다. 
   ...
   정리(clean up)하려면 열려 있는 텍스트 편집기의 탭에서 **닫기**(**x**)를 선택합니다.

3. **docker-python** 환경을 사용하여 **iris_sklearn.py** 스크립트를 실행합니다. 

   - 왼쪽 도구 모음에서 **시계** 아이콘을 선택하여 **실행** 창을 엽니다. **모든 실행**을 선택합니다. 

   - **모든 실행** 탭의 상단에서 기본 **local** 대신 **docker-python**을 대상 환경으로 선택합니다. 
...
4. 새 작업이 시작되는지 확인합니다. Workbench 창의 오른쪽에 있는 **작업** 창에서 확인할 수 있습니다.

   Docker에서 처음 실행하는 경우 이 작업을 완료하는 데 몇 분 정도 시간이 더 걸릴 수 있습니다. 
...
5. 이제 Spark를 사용해 보겠습니다. Docker 기본 이미지에는 미리 설치되고 구성된 Spark 인스턴스가 포함되어 있으며, 이 인스턴스는 PySpark 스크립트를 실행하는 데 사용할 수 있습니다. 이 기본 이미지를 사용하는 것은 Spark를 직접 설치하고 구성하는 데 시간을 소모하지 않고도 Spark 프로그램을 개발하고 테스트할 수 있는 쉬운 방법입니다. 
   ...
   `iris_spark.py` 파일을 엽니다. 이 스크립트에서는 `iris.csv` 데이터 파일을 로드하고 Spark Machine Learning 라이브러리의 로지스틱 회귀 알고리즘을 사용하여 아이리스 데이터 집합을 분류합니다. 이제 실행 환경을 **docker-spark**로 변경하고 스크립트를 **iris_spark.py**로 변경한 다음, 다시 실행합니다. 이 프로세스는 Docker 컨테이너에서 Spark 세션을 만들고 시작해야 하기 때문에 시간이 약간 더 오래 걸립니다. 또한 표준 출력(stdout)이 `iris_spark.py`의 표준 출력과 다른 것도 알 수 있습니다.

6. 몇 번 더 실행해보고 서로 다른 인수를 사용해 봅니다. 

7. `iris_spark.py` 파일을 열어 Spark Machine Learning 라이브러리를 사용하여 구축된 로지스틱 회귀 모델을 확인합니다. 

8. **작업** 패널을 확인하고, 기록 목록 보기 외에 여러 환경에서 수행된 실행에 대한 자세한 보기를 확인합니다.
...
1. Azure Machine Learning CLI(명령줄 인터페이스)를 시작합니다.
   ...
   Windows의 프로젝트 폴더 `C:\Temp\myIris\>`에서 CLI 프롬프트가 시작합니다. 이 프로젝트는 자습서 1부에서 만든 것과 같습니다.
...
1. CLI 창에서 Azure에 로그인합니다. [az 로그인에 대한 자세한 내용은 여기를 참조합니다](https://docs.microsoft.com/cli/azure/authenticate-azure-cli?view=azure-cli-latest).
   ...
   이미 로그인되어 있을 수도 있습니다. 이 경우 아래 단계를 건너뛸 수 있습니다.
   ...
   1. 명령 프롬프트에 다음을 입력합니다.
      ...
      이 명령은 https://aka.ms/devicelogin 의 브라우저에서 사용할 코드를 반환합니다.

   1. 브라우저에서 https://aka.ms/devicelogin 으로 이동합니다.

   1. 메시지가 표시되면 CLI에서 수신된 코드를 브라우저에 입력합니다.

   Workbench 앱과 CLI는 Azure 리소스에 대해 인증할 때 독립적인 자격 증명 캐시를 사용합니다. 로그인한 후 캐시된 토큰이 만료될 때까지 다시 인증하지 않아도 됩니다. 

1. 조직에 여러 Azure 구독(엔터프라이즈 환경)이 있는 경우 사용할 구독을 반드시 설정해야 합니다. 구독을 찾아 구독 ID를 사용하여 구독을 설정한 다음, 테스트합니다.

   1. 다음 명령을 사용하여 액세스할 수 있는 모든 Azure 구독을 나열합니다.
      ...
      **az account list** 명령은 로그인에 사용 가능한 구독 목록을 반환합니다. 
      반환된 구독이 2개 이상인 경우, 사용하려는 구독에 대한 구독 ID 값을 확인합니다.
...
1. CLI 창에서 **iris_sklearn.py** 스크립트를 시험 삼아 제출합니다.
   ...
   Iris_sklearn.py 실행은 로컬 컴퓨팅 컨텍스트에서 실행됩니다.
...
1. 출력을 검토합니다. Workbench를 사용하여 스크립트를 실행했을 때와 동일한 출력과 결과임을 알 수 있습니다. 

1. CLI 창에서 Docker 실행 환경(Docker가 컴퓨터에 설치된 경우)을 사용하여 Python 스크립트 **iris_sklearn.py**를 다시 실행합니다.
...
1. Workbench로 돌아가서 다음을 수행합니다.
   1. 프로젝트 파일을 나열하려면 왼쪽 창에서 폴더 아이콘을 선택합니다.
   
   1. **run.py**이라는 Python 스크립트를 엽니다. 이 스크립트는 다양한 정규화 속도에서 반복하는 데 유용합니다. 

   ![실행 목록으로 돌아가기](media/tutorial-classifying-iris/2-runpy.png)

1. 다양한 정규화 속도로 실험을 여러 번 실행합니다. 

   이 스크립트는 매우 큰 `10.0`의 정규화 속도로 `iris_sklearn.py` 작업을 시작합니다. 그런 다음, 스크립트는 다음 실행에서 속도를 반으로 줄이고 속도가 `0.005`보다 작지 않을 때까지 계속합니다. 

   스크립트에는 다음 코드가 포함되어 있습니다.

   ![실행 목록으로 돌아가기](media/tutorial-classifying-iris/2-runpy-code.png)
...
1. 다음과 같은 명령줄에서 **run.py** 스크립트를 실행합니다.
   ...
   이 명령은 서로 다른 정규화 속도로 여러 번 iris_sklearn.py를 제출합니다.
...
원격 Linux 머신의 Docker 컨테이너에서 스크립트를 실행하려면 해당 원격 머신에 대한 SSH 액세스(사용자 이름 및 암호) 권한이 있어야 합니다. 또한 머신에는 Docker 엔진이 설치되어 실행 중이어야 합니다. 이러한 Linux 머신을 얻는 가장 쉬운 방법은 Azure에서 Ubuntu 기반 DSVM(Data Science Virtual Machine)을 만드는 것입니다. [Azure ML Workbench에서 사용할 Ubuntu DSVM를 만드는 방법](how-to-create-dsvm-hdi.md#create-an-ubuntu-dsvm-in-azure-portal)을 알아봅니다.
...
1. VM이 만들어지면 `.runconfig` 및 `.compute` 파일 쌍을 생성하여 VM을 실행 환경으로 연결할 수 있습니다. 다음 명령을 사용하여 파일을 생성합니다. 
 ...
 새 컴퓨팅 대상 `myvm`의 이름을 지정합니다.
   ...
   >[!NOTE]
   >사용자 이름 및 암호(password) 인증 외에도, `--private-key-file` 및 `--private-key-passphrase`(선택 사항) 옵션을 사용하여 개인 키와 해당 전달 구(passphrase)(있는 경우)를 지정할 수 있습니다. DSVM을 만들 때 사용한 개인 키를 사용하려면 `--use-azureml-ssh-key` 옵션을 지정해야 합니다.

   다음으로, 아래 명령을 실행하여 **myvm** 컴퓨팅 대상을 준비합니다.
   
   ```azurecli
   az ml experiment prepare -c myvm
   ```
   
   위 명령은 VM에 Docker 이미지를 생성하여 스크립트 실행을 준비하게 합니다.
...
2. `aml_config` 아래에 생성된 `myvm.runconfig` 파일을 편집하고 프레임워크를 `PySpark`(기본값)에서 `Python`으로 변경합니다.
   ...
   ```
   >[!NOTE]
   >PySpark도 괜찮지만, 실제로 Python 스크립트를 실행하는 데 Spark 세션이 필요하지 않으면, Python을 사용하는 것이 더 효율적입니다.
...
1. Azure HDInsight용 Spark 클러스터에 액세스할 수 있으면 다음과 같이 HDInsight 실행 구성 명령을 생성합니다. HDInsight 클러스터 이름, HDInsight 사용자 이름 및 암호를 매개 변수로 제공합니다. 
   ...
   다음 명령을 사용하여 HDInsight 클러스터를 가리키는 컴퓨팅 대상을 만듭니다.
   ...
   HDInsight 클러스터를 준비하려면 아래 명령을 실행합니다.
   ...
   >[!NOTE]
   >`username`은 HDInsight 설정 시 정의된 클러스터 SSH 사용자 이름입니다. 기본적으로 값은 `sshuser`입니다. 값은 설치 중에 만들어져 클러스터의 관리 웹 사이트에 액세스할 수 있도록 설정되는 다른 사용자인 `admin`이 아닙니다. 

2. 다음 명령을 사용하여 HDInsight 클러스터에서 **iris_spark.py** 스크립트를 실행합니다.
...
3부로 이루어진 자습서 시리즈의 이 두 번째 부분에서는 다음 방법을 알아봤습니다.
> [!div class="checklist"]
> * Workbench에서 스크립트를 열고 코드 확인
> * 로컬 환경에서 스크립트 실행
> * 실행 기록 확인
> * 로컬 Docker 환경에서 스크립트 실행
