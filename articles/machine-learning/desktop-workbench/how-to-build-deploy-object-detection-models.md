...
이 데모에서 냉장고 내 식품 항목의 데이터 세트는 30개의 이미지 및 8개의 클래스(계란판, 요구르트, 케찹, 버섯, 머스터드, 오렌지, 호박 및 물)로 구성되어 제공됩니다. 각 jpg 이미지에는 비슷한 이름의 주석 xml 파일이 있습니다. 
...
'평가' 메서드는 모델을 평가하는 데 사용됩니다. 이 함수는 ObjectDetectionDataset 개체를 입력으로 사용해야 합니다. 학습 데이터 세트에 사용되는 것과 동일한 함수를 사용하여 평가 데이터 세트를 만들 수 있습니다. 지원되는 메트릭은 [PASCAL VOC 챌린지](http://host.robots.ox.ac.uk/pascal/VOC/pubs/everingham10.pdf)에 정의된 대로 평균 정밀도입니다.  
