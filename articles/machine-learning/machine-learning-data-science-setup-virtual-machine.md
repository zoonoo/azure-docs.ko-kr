...
다음 그림과 같이 *이 웹 사이트의 보안 인증서에 문제가 있습니다*(Internet Explorer) 또는 *Your connection is not private*(Chrome)이라는 경고가 나타납니다. **이 웹 사이트를 계속 탐색합니다(권장하지 않음)**(Internet Explorer)를 클릭하거나 **Advanced**, **Proceed to &#60;*DNS Name*> (unsafe)**(Chrome)를 차례로 클릭하여 계속합니다. 그런 다음 이전에 지정한 암호를 입력하여 IPython Notebook에 액세스합니다.
