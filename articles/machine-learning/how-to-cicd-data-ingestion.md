...
이 방법에서는 학습 데이터가 Azure blob 저장소에 저장 됩니다. Azure Data Factory 파이프라인은 입력 blob 컨테이너에서 데이터를 인출 하 고 변환한 후 출력 blob 컨테이너에 데이터를 저장 합니다. 이 컨테이너는 Azure Machine Learning 서비스에 대 한 [데이터 저장소](https://docs.microsoft.com/azure/machine-learning/concept-data#access-data-in-storage) 역할을 합니다. 데이터가 준비 되 면 Data Factory 파이프라인이 학습 Machine Learning 파이프라인을 호출 하 여 모델을 학습 합니다. 이 특정 예제에서 데이터 변환은 Azure Databricks 클러스터에서 실행 되는 Python 노트북에 의해 수행 됩니다. 
...
데이터 엔지니어가 IDE (예: [Visual Studio Code](https://code.visualstudio.com))에서 로컬로 또는 Databricks 작업 영역에서 직접 Python 노트북 소스 코드를 사용 합니다. 후자는 개발 환경에서 코드를 디버깅 하는 기능을 제공 합니다. 어떤 경우 든 코드는 분기 정책을 따라 리포지토리에 병합 됩니다. `.ipynb` Jupyter 노트북 형식이 아닌 `.py` 파일에 코드를 저장 하는 것이 좋습니다. 코드 가독성을 향상 시키고 CI 프로세스에서 자동 코드 품질 검사를 사용 하도록 설정 합니다.
...
## <a name="continuous-integration-ci"></a>CI(Continous Integration: 지속적인 통합)
