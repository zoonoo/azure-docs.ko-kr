...
워크플로를 조정하고 테스트할 때 별도의 단계를 사용하여 필요한 단계만 다시 실행할 수 있습니다. 단계는 파이프라인의 계산 단위입니다. 앞의 다이어그램에 표시된 대로 데이터 준비 태스크에는 여러 단계가 포함될 수 있습니다. 이러한 단계는 포함 되지만, 정규화, 변환, 유효성 검사 및 기능화 (featurization)로 제한 되지 않습니다. 데이터 원본 및 중간 데이터를 파이프라인을 통해 다시 사용하면 계산 시간 및 리소스가 절약됩니다. 
