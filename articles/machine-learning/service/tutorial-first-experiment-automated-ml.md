...
실험이 진행됨에 따라 **반복 차트**와 **반복 목록**이 완료 시 생성된 다른 반복(모델)으로 화면에 업데이트되며, 메트릭 점수 순서로 정렬됩니다. 기본적으로 선택한 **AUC_weighted** 메트릭에 따라 가장 높은 점수를 획득한 모델이 목록 맨 위에 표시됩니다.
