...
실행이 완료되면 **실행 세부 정보** 페이지로 다시 이동한 후 **모델** 탭을 선택합니다. **새로 고침**을 선택합니다. 
