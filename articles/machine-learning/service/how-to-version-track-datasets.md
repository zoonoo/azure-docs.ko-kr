...
다음 코드는 `create_new_version` 매개 변수를 `True`로 설정 하 여 `titanic_ds` 데이터 집합의 새 버전을 등록 합니다. 작업 영역에 등록 된 기존 `titanic_ds` 데이터 집합이 없는 경우 코드는 `titanic_ds` 이름을 사용 하 여 새 데이터 집합을 만들고 해당 버전을 1로 설정 합니다.
...
* [데이터 집합으로 학습](how-to-train-with-datasets.md)
