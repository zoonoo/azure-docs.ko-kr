...
- `output_action`: 다음 값 중 하나는 출력을 구성하는 방법을 나타냅니다.
    ...
    - `append_row`: 모든 입력 파일의 경우 출력 폴더에 줄로 구분된 모든 출력을 추가하는 하나의 파일만 생성됩니다. 이 파일의 이름은 parallel_run_step.txt입니다.
...
- `logging_level`: 로그의 자세한 정도입니다. 값의 자세한 정도는 `WARNING`, `INFO`, `DEBUG` 순서로 높아집니다. 기본값은 `INFO`입니다(선택 사항).
- `run_invocation_timeout`: `run()` 메서드 호출 시간 제한(초)입니다. 기본값은 `60`입니다.
...
- `allow_reuse`: 동일한 설정/입력으로 실행할 때 이전 결과를 단계에 다시 사용할 것인지 여부를 지정합니다. 이 매개 변수가 `False`이면 파이프라인 실행 중에 이 단계에 대해 항상 새로운 실행이 생성됩니다. (선택 사항이며 기본값은 `True`입니다.)
