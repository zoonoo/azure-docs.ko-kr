...
1. 컨테이너 레지스트리에 액세스 하는 데 필요한 자격 증명을 웹 앱에 제공 하려면 다음 명령을 사용 합니다. 사용할 `<app-name>` 이름으로 대체 합니다. `package.location` 및 `<acrinstance>` 를앞에서반환된의값으로바꿉니다.`<imagename>` 및 `<username>` 를 `<password>` 앞에서 검색 한 ACR 로그인 정보로 바꿉니다.
...
> ```
>
> 이미지가 로드 되 고 사이트가 활성화 되 면 로그에는 라는 `Container <container name> for site <app-name> initialized successfully and is ready to serve requests`메시지가 표시 됩니다.
...
이 명령은 다음 호스트 이름과 `<app-name>.azurewebsites.net`유사한 정보를 반환 합니다. 이 값은 서비스에 대 한 __기본 url__ 의 일부로 사용 합니다.
