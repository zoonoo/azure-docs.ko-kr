...
- 옵션 1: 학습 스크립트에서 직접 데이터 집합을 전달 합니다.

- 옵션 2: 데이터 집합을 사용 하 여 교육을 위해 원격 계산에 파일을 탑재 하거나 다운로드 합니다.
...
## <a name="option-1-pass-datasets-as-inputs-to-training-scripts"></a>옵션 1: 데이터 집합을 학습 스크립트에 대 한 입력으로 전달

Azure Machine Learning 데이터 집합은 [평가기](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.estimator?view=azure-ml-py) 및 [hyperdrive](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.hyperdrive?view=azure-ml-py)와 같은 Azure Machine Learning 교육 제품과 원활한 [통합을 제공](https://docs.microsoft.com/python/api/azureml-core/azureml.core.scriptrun?view=azure-ml-py)합니다. 이 예제에서는 [TabularDataset](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py) 를 만들고이를 학습을 위해 개체의 `estimator` 입력으로 사용 합니다. 
...
파일 시스템을 탑재 하는 경우 해당 파일 시스템을 디렉터리 (탑재 지점)에 연결 하 여 계산 대상에서 사용할 수 있도록 합니다. 파일은 처리 시에만 로드 되므로 탑재는 즉각적입니다. 탑재는 지원 되며 Azure Machine Learning 계산, 가상 컴퓨터 및 HDInsight를 포함 하 여 Linux 기반 계산에 권장 됩니다. Linux 기반이 아닌 계산의 경우 다운로드만 지원 됩니다.  

>[!WARNING]
> 데이터 크기가 계산 대상의 저장소 제한을 초과 하는 경우 다운로드가 실패 합니다.
...
평가기의 `inputs` 매개 변수를 통해 데이터 집합을 전달 하는 대신를 통해 `script_params` 데이터 집합을 전달 하 고 인수를 통해 학습 스크립트의 데이터 경로 (탑재 지점)를 가져올 수도 있습니다. 이러한 방식으로 학습 스크립트에서 Azure Machine Learning SDK에 대 한 종속성을 피할 수 있습니다.

Scikit [학습](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.sklearn.sklearn?view=azure-ml-py) 평가기 개체는에 대 한 실행을 제출 하는 데 사용 됩니다.
...
[샘플 노트북](https://aka.ms/dataset-tutorial) 은이 문서의 개념 (예:)을 시연 [하 고 확장](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/training-with-deep-learning/train-hyperparameter-tune-deploy-with-keras/train-hyperparameter-tune-deploy-with-keras.ipynb) 합니다.
