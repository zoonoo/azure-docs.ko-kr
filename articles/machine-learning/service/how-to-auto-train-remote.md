...
Training_data 정의 되며 AutoMLConfig에서 자동화 된 ML에 전달 되는 레이블로 `TabularDataset`및 레이블로 전달 됩니다. 기본적으로 `from_delimited_files`는 `infer_column_types`를 true로 설정 하 여 열 형식을 자동으로 유추 합니다. 
