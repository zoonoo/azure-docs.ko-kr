...
ms.locfileid: "74997261"
---
# <a name="configure-a-development-environment-for-azure-machine-learning"></a>Azure Machine Learning에 대 한 개발 환경 구성
...
| Környezet | 전문가 | Hátrányok |
| --- | --- | --- |
| [클라우드 기반 Azure Machine Learning 노트북 VM](#notebookvm) | 시작 하는 가장 쉬운 방법입니다. 전체 SDK는 작업 영역 VM에 이미 설치 되어 있으며, 노트북 자습서는 미리 복제 되어 실행할 준비가 되었습니다. | 개발 환경 및 종속성에 대 한 제어가 부족 합니다. Linux VM에 대해 발생 하는 추가 비용입니다 (요금을 방지 하기 위해 사용 하지 않을 때 VM을 중지할 수 있음). [가격 정보](https://azure.microsoft.com/pricing/details/virtual-machines/linux/)를 참조 하세요. |
...
| [Azure Databricks](#aml-databricks) | 확장 가능한 Apache Spark 플랫폼에서 대규모의 집약적 기계 학습 워크플로를 실행 하는 데 적합 합니다. | 실험적 기계 학습 또는 작은 규모의 실험 및 워크플로를 위한 과잉. Azure Databricks에 대해 발생 하는 추가 비용입니다. [가격 정보](https://azure.microsoft.com/pricing/details/databricks/)를 참조 하세요. |
...
Egy Azure Machine Learning-munkaterület. 작업 영역을 만들려면 [Azure Machine Learning 작업 영역 만들기](how-to-manage-workspace.md)를 참조 하세요. 작업 영역은 사용자 고유의 [클라우드 기반 노트북 서버](#notebookvm), [dsvm](#dsvm)또는 [Azure Databricks](#aml-databricks)를 시작 하는 데 필요 합니다.
...
- Linux 또는 macOS에서 bash 셸이 필요 합니다.

    > [!TIP]
    > Linux 또는 macOS를 사용 하 고 bash 이외의 셸 (예: zsh)을 사용 하는 경우 일부 명령을 실행할 때 오류가 발생할 수 있습니다. 이 문제를 해결 하려면 `bash` 명령을 사용 하 여 새 bash 셸을 시작 하 고 여기에서 명령을 실행 합니다.

- Windows에서 명령 프롬프트 또는 Anaconda 프롬프트가 필요 합니다 (Anaconda 및 Miniconda에서 설치).

## <a id="notebookvm"></a>사용자 고유의 클라우드 기반 노트북 VM

Azure Machine Learning 노트북 VM은 Jupyter 노트북 서버, JupyterLab 및 완전히 준비 된 ML 환경을 사용 하 여 데이터 과학자을 제공 하는 클라우드 기반의 안전한 Azure 워크스테이션입니다.
...
계산 요금이 발생 하지 않도록 하려면 [노트북 VM을 중지](tutorial-1st-experiment-sdk-train.md#clean-up-resources)합니다.

## <a id="dsvm"></a>Data Science Virtual Machine

DSVM은 사용자 지정 된 VM (가상 머신) 이미지입니다. 다음을 사용 하 여 미리 구성 된 데이터 과학 작업을 위해 설계 되었습니다.

  - TensorFlow, PyTorch, Scikit, XGBoost 및 Azure Machine Learning SDK와 같은 패키지
  - Spark 독립 실행형 및 드릴과 같은 인기 있는 데이터 과학 도구
  - Azure CLI, AzCopy, Storage 탐색기 등의 Azure 도구
  - Visual Studio Code 및 PyCharm와 같은 Ide (통합 개발 환경)
  - Jupyter Notebook 서버

Azure Machine Learning SDK는 Ubuntu 또는 Windows 버전의 DSVM에서 작동 합니다. 하지만 DSVM을 계산 대상으로 사용할 계획인 경우 Ubuntu만 지원 됩니다.
...
1. 다음 환경 중 하나에서 DSVM을 만듭니다.
    ...
    * Azure CLI:
        ...
        > [!IMPORTANT]
        > * Azure CLI 사용 하는 경우 먼저 `az login` 명령을 사용 하 여 Azure 구독에 로그인 해야 합니다.
        >
        > * 이 단계의 명령을 사용 하는 경우 리소스 그룹 이름, VM 이름, 사용자 이름 및 암호를 제공 해야 합니다.

        * Ubuntu Data Science Virtual Machine를 만들려면 다음 명령을 사용 합니다.
        ...
        * Windows Data Science Virtual Machine를 만들려면 다음 명령을 사용 합니다.
...
2. Azure Machine Learning SDK가 DSVM에 이미 설치 되어 있습니다. SDK를 포함 하는 Conda 환경을 사용 하려면 다음 명령 중 하나를 사용 합니다.
...
1. SDK에 액세스 하 여 버전을 확인할 수 있는지 확인 하려면 다음 Python 코드를 사용 합니다.
...
자세한 내용은 [데이터 과학 Virtual Machines](https://azure.microsoft.com/services/virtual-machines/data-science-virtual-machines/)를 참조 하세요.

## <a id="local"></a>로컬 컴퓨터

로컬 컴퓨터 (원격 가상 컴퓨터 일 수도 있음)를 사용 하는 경우 Anaconda 환경을 만들고 SDK를 설치 합니다. Például:
...
1. Anaconda 프롬프트를 열고 다음 명령을 사용 하 여 환경을 만듭니다.
    ...
    이 예제에서는 python 3.6.5를 사용 하 여 환경을 만들지만 특정 subversions을 선택할 수 있습니다. SDK 호환성은 특정 주 버전에서 보장 되지 않을 수 있습니다 (3.5 + 권장). 오류가 발생 하는 경우 Anaconda 환경에서 다른 버전/subversion을 사용해 보는 것이 좋습니다. 구성 요소 및 패키지를 다운로드 하는 동안 환경을 만드는 데 몇 분 정도 걸릴 수 있습니다.
...
1. 다음 명령을 사용 하 여 패키지를 설치 합니다.
   ...
   > [!NOTE]
   > * PyYAML를 제거할 수 없다는 메시지가 표시 되는 경우 다음 명령을 대신 사용 합니다.
   >
   >   `pip install --upgrade azureml-sdk[notebooks,automl] --ignore-installed PyYAML`
   >
   > * MacOS Catalina.properties부터 zsh (Z shell)는 기본 로그인 셸 및 대화형 셸입니다. Zsh에서 "\\" (백슬래시)로 괄호를 이스케이프 하는 다음 명령을 사용 합니다.
   >
...
### <a id="jupyter"></a>Jupyter 노트북

Jupyter 노트북은 [Jupyter 프로젝트](https://jupyter.org/)의 일부입니다. 또한 라이브 코드를 설명 텍스트 및 그래픽과 혼합 하는 문서를 만드는 대화형 코딩 환경을 제공 합니다. 또한 jupyter 노트북은 문서에 코드 섹션의 출력을 저장할 수 있으므로 다른 사용자와 결과를 공유 하는 좋은 방법입니다. 다양 한 플랫폼에 Jupyter 노트북을 설치할 수 있습니다.
...
1. Visual Studio Code에 대 한 Azure Machine Learning 확장을 설치 [Azure Machine Learning](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai)를 참조 하세요.
    ...
    자세한 내용은 [Visual Studio Code에 Azure Machine Learning 사용](how-to-vscode-tools.md)을 참조 하세요.
...
| Beállítás |적용 대상| Value (Díj) |
|----|---|---|
| Fürt neve |항상| yourclustername |
| A Databricks futtatókörnyezete |항상|비 ML 런타임 6.0 (scala 2.11, spark 2.4.3) |
| Python-verzió |항상| 3 |
| 들 |항상| 2 이상 |
| 작업자 노드 VM 유형 <br>(최대 동시 반복 횟수를 결정 합니다.) |Automatizált ML<br>에게만| 메모리 액세스에 최적화 된 VM 기본 설정 |
| Automatikus skálázás engedélyezése |Automatizált ML<br>에게만| Törölje a jelet a |

클러스터가 실행 될 때까지 기다렸다가 계속 진행 합니다.
...
1. 옵션을 **하나만** 선택 하십시오. 다른 SDK 설치는 지원 되지 않습니다.
   ...
   |----|---|---|
   |Databricks의 경우| Python 알 또는 PyPI 업로드 | azureml-sdk [databricks]|
   |Databricks-<br> 자동화 된 ML 기능| Python 알 또는 PyPI 업로드 | azureml-sdk [automl]|
...
1. 상태가 **연결**됨으로 변경 될 때까지 오류를 모니터링 합니다 .이는 몇 분 정도 걸릴 수 있습니다.  이 단계가 실패 하는 경우:
   ...
   1. 왼쪽 창에서 **클러스터**를 선택 합니다.
   1. 테이블에서 클러스터 이름을 선택 합니다.
   1. **라이브러리** 탭에서 **다시 시작**을 선택 합니다.
   ...
   + 또는 이전 SDK 버전이 있는 경우 클러스터의 설치 된 라이브러리에서 선택을 취소 하 고 휴지통으로 이동 합니다. 새 SDK 버전을 설치 하 고 클러스터를 다시 시작 합니다. 다시 시작한 후 문제가 발생 하면 클러스터를 분리 하 고 다시 연결 합니다.
...
작업 영역 구성 파일은 SDK에서 Azure Machine Learning 작업 영역과 통신 하는 방법을 알려 주는 JSON 파일입니다. 파일의 이름은 *config.xml*이며 다음과 같은 형식입니다.
...
이 JSON 파일은 Python 스크립트 또는 Jupyter 노트북을 포함 하는 디렉터리 구조에 있어야 합니다. 같은 디렉터리, 이름이 *azureml*인 하위 디렉터리 또는 부모 디렉터리에 있을 수 있습니다.

코드에서이 파일을 사용 하려면 `ws=Workspace.from_config()`을 사용 합니다. 이 코드는 파일에서 정보를 로드 하 고 작업 영역에 연결 합니다.

다음 세 가지 방법으로 구성 파일을 만들 수 있습니다.

* **[Write_config](https://docs.microsoft.com/python/api/overview/azure/ml/intro?view=azure-ml-py)** :를 사용 하 여 *config.xml* 파일을 작성 합니다. 파일은 작업 영역에 대 한 구성 정보를 포함 합니다. 다른 개발 환경에 *app.config* 를 다운로드 하거나 복사할 수 있습니다.

* **파일 다운로드**: [Azure Portal](https://ms.portal.azure.com)의 작업 영역 **개요** 섹션에서 **config.xml 다운로드** 를 선택 합니다.

     ![Azure Portal](./media/how-to-configure-environment/configure.png)

* **프로그래밍 방식으로 파일 만들기**: 다음 코드 조각에서 구독 ID, 리소스 그룹 및 작업 영역 이름을 제공 하 여 작업 영역에 연결 합니다. 그런 다음 파일에 작업 영역 구성을 저장 합니다.
...
- [Python 용 AZURE MACHINE LEARNING SDK](https://docs.microsoft.com/python/api/overview/azure/ml/intro?view=azure-ml-py) 참조 보기
