...
1. [설치 지침](https://azure.github.io/azureml-sdk-for-r/articles/installation.html)에 따라 다음을 수행합니다.
    + Anaconda 설치
    + `azuremlsdk` 설치
    + Python용 Azure Machine Learning SDK 설치

1. [GitHub](https://github.com/Azure/azureml-sdk-for-r/tree/master/vignettes/train-and-deploy-with-caret)에서 3개의 자습서 파일을 다운로드합니다.  **tutorials** 디렉터리에 저장합니다.

2. 아래 단계를 사용하여 Azure Machine Learning 작업 영역을 만들고 해당 구성 파일을 다운로드합니다.


### <a name="create-a-workspace"></a>작업 영역 만들기

Azure Machine Learning 작업 영역은 기계 학습 모델을 실험하고, 학습시키고, 배포하는 데 사용하는 클라우드의 기본 리소스입니다. Azure 구독 및 리소스 그룹을 SDK에서 쉽게 사용되는 개체에 연결합니다. Azure Machine Learning 작업 영역이 이미 있으면 [다음 섹션으로 건너뜁니다](#config). 그렇지 않으면 지금 작업 영역을 새로 만듭니다.

[!INCLUDE [aml-create-portal](../../../includes/aml-create-in-portal.md)]

### <a name="config"></a> config.json 파일 다운로드

1. 작업 영역 페이지 맨 위에서 **config.json 다운로드**를 선택합니다.

    ![config.json 파일 다운로드](media/tutorial-1st-r-experiment/download-config.png)  

1. **tutorials** 디렉터리에 이 파일을 추가합니다.

이제 앱을 실행할 준비가 되었습니다.

이 자습서를 실행하려면 RStudio를 사용하는 것이 좋습니다. RStudio에서 파일 > 새 프로젝트 > 기존 디렉터리를 선택하고 위에서 만든 **tutorials** 디렉터리를 선택합니다.

> [!Important]
> 이 문서의 나머지 부분에는 **train-and-deploy-to-aci.Rmd** 파일에 표시되는 것과 동일한 콘텐츠가 포함되어 있습니다.  
> RMarkdown 사용 경험이 있는 경우 해당 파일의 코드를 자유롭게 사용할 수 있습니다.  또는 해당 프로그램에서 또는 이 문서에서 R 스크립트나 명령줄로 코드 조각을 복사한 후 붙여 넣을 수 있습니다.
...
* 필요한 패키지를 설치합니다.
* 로컬 컴퓨터에서 원격 리소스와 통신할 수 있도록 작업 영역에 연결합니다.
* 실행을 추적하는 실험을 만듭니다.
