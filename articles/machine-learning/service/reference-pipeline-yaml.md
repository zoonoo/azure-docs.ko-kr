...
| `path_on_datastore` | 선택 사항입니다. 수정/추가 된 blob을 모니터링할 데이터 저장소의 경로입니다. 경로는 데이터 저장소의 컨테이너 아래에 있으므로 일정 모니터링의 실제 경로는 컨테이너/`path_on_datastore`입니다. 없는 경우 데이터 저장소 컨테이너가 모니터링 됩니다. `path_on_datastore`의 하위 폴더에 대 한 추가/수정 사항은 모니터링 되지 않습니다. 데이터 저장소 일정에만 지원 됩니다. |
