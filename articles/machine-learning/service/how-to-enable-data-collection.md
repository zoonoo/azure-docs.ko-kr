...
1. [Azure Machine Learning](https://ml.azure.com)로 이동 합니다.

1. 작업 영역을 엽니다.

1. **배포** 를 선택 하 > 서비스 > **편집**을 **선택** 합니다.

   ![서비스 편집](media/how-to-enable-data-collection/EditService.PNG)

1. **고급 설정**에서 **모델 데이터 수집 사용**을 선택 합니다.

    [데이터 컬렉션 선택 ![](media/how-to-enable-data-collection/CheckDataCollection.png)](./media/how-to-enable-data-collection/CheckDataCollection.png#lightbox)

   **AppInsights 진단 사용** 을 선택 하 여 서비스의 상태를 추적할 수도 있습니다.
...
1. **배포** 를 선택 하 > 서비스 > **편집**을 **선택** 합니다.
   ...
   [편집 옵션 ![선택 합니다.](media/how-to-enable-data-collection/EditService.PNG)](./media/how-to-enable-data-collection/EditService.PNG#lightbox)
...
1. 필터에 모델 경로를 입력 합니다. 특정 연도나 월의 파일만 확인 하려면 필터 경로를 확장 합니다. 예를 들어 3 월 데이터만 보려면 다음 필터 경로를 사용 합니다.
   ...
   /modeldata/\<subscriptionid >/\<resourcegroupname >/\<workspacename >/\<webservicename >/\<modelname >/\<modelversion >/\<지정 >/\<year >/3
...
1. Databricks 작업 영역에서 **데이터 업로드**를 선택 합니다.
    ...
    [Databricks 데이터 업로드 옵션을 선택 ![](media/how-to-enable-data-collection/dbupload.png)](./media/how-to-enable-data-collection/dbupload.png#lightbox)

1. **새 테이블 만들기** 를 선택 하 **고 다른 데이터 원본** > **Azure Blob Storage** 선택 하 > **노트북에서 테이블 만들기**를 선택 합니다.
