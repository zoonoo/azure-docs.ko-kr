...
각 작업 영역에는 즉시 사용 가능한 기본 데이터 저장소가 있으므로 저장소 계정을 직접 만들고 구성하지 않아도 됩니다.
...
기존 Azure Storage가 이미 있는 경우에는 작업 영역의 데이터 저장소로 등록할 수 있습니다. Azure Machine Learning에서는 Azure Blob 컨테이너 또는 Azure 파일 공유를 데이터 저장소로 등록하는 기능을 제공합니다. 모든 등록 메서드는 `Datastore` 클래스에 포함되어 있으며 형식은 `register_azure_*`입니다.
...
편의상 등록된 데이터 저장소 중 하나를 작업 영역의 기본 데이터 저장소로 설정하려는 경우 다음 코드를 사용할 수 있습니다.
```Python
...
### <a name="upload"></a>업로드
Python SDK를 사용하여 데이터 저장소에 디렉터리 또는 개별 파일을 업로드할 수 있습니다.
...
데이터 저장소의 `upload_files()` 메서드를 통해 개별 파일 목록을 데이터 저장소에 업로드할 수도 있습니다.

### <a name="download"></a>다운로드
업로드와 마찬가지로 데이터 저장소의 데이터를 로컬 파일 시스템에 다운로드할 수도 있습니다.
...
Python SDK를 통해 원격 컴퓨터 대상에서 교육을 실행하는 중에 학습 또는 유효성 검사 데이터를 확인하기 위해 데이터 저장소에 액세스할 수 있습니다. 

두 가지 방식을 통해 원격 컴퓨터에서 데이터 저장소를 사용할 수 있습니다.
* **탑재**  
`ds.as_mount()`: 이 탑재 모드를 지정하면 데이터 저장소가 원격 컴퓨터에 자동으로 탑재됩니다. 
* **다운로드/업로드**  
    * `ds.as_download(path_on_compute='your path on compute')`: 이 다운로드 모드를 사용하는 경우 데이터가 데이터 저장소에 `path_on_compute`로 지정된 원격 컴퓨터의 위치에 다운로드됩니다.
    * 반대로 학습 실행에서 생성된 데이터를 데이터 저장소에 업로드할 수도 있습니다. 예를 들어 학습 스크립트가 원격 컴퓨터의 현재 작업 디렉터리에 `foo.pkl` 파일을 만드는 경우 `ds.as_upload(path_on_compute='./foo.pkl')`를 사용하면 스크립트가 실행된 후에 해당 파일이 데이터 저장소에 업로드되도록 지정할 수 있습니다. 그러면 파일이 데이터 저장소의 루트에 업로드됩니다.
    
데이터 저장소의 특정 폴더나 파일을 참조하려는 경우 데이터 저장소의 **`path`** 함수를 사용할 수 있습니다. 예를 들어 데이터 저장소에 상대 경로가 `./bar`인 디렉터리가 있는데 이 폴더의 콘텐츠만 컴퓨터 대상에 다운로드하려는 경우 `ds.path('./bar').as_download()`를 사용하면 됩니다.
...
학습 중에 데이터 저장소에 액세스하려는 경우 `script_params`를 통해 학습 스크립트에 데이터 저장소를 명령줄 인수로 전달하면 됩니다.
...
데이터 저장소 목록을 Estimator 생성자의 `inputs` 매개 변수에 전달하여 데이터 저장소를 탑재하거나 데이터 저장소에서/데이터 저장소로 데이터를 복사할 수 있습니다.
