...
작업 영역의 기본 데이터 저장소를 가져오려면 다음을 수행 합니다.
...
데이터 저장소 `datastore`에 디렉터리를 업로드 하려면:
...
`target_path` 매개 변수는 업로드할 파일 공유 (또는 blob 컨테이너)의 위치를 지정 합니다. 기본적으로 `None`로 설정 되며,이 경우 데이터가 루트에 업로드 됩니다. `overwrite=True` 그렇지 않으면 `target_path`에 있는 기존 데이터를 덮어씁니다.

또는 `upload_files()` 메서드를 통해 데이터 저장소에 개별 파일 목록을 업로드 합니다.

### <a name="download"></a>Letöltés

마찬가지로 데이터 저장소에서 로컬 파일 시스템으로 데이터를 다운로드 합니다.
...
`target_path` 매개 변수는 데이터를 다운로드할 로컬 디렉터리의 위치입니다. 다운로드할 파일 공유 (또는 blob 컨테이너)의 폴더에 대 한 경로를 지정 하려면 해당 경로를 `prefix`제공 합니다. `prefix` `None`되 면 파일 공유 (또는 blob 컨테이너)의 모든 내용이 다운로드 됩니다.
...
Way|Módszer|Leírás|
----|-----|--------
Csatlakoztatás| [`as_mount()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.azure_storage_datastore.abstractazurestoragedatastore?view=azure-ml-py#as-mount--)| 를 사용 하 여 계산 대상에 데이터 저장소를 탑재 합니다. 탑재 되 면 계산 대상에서 데이터 저장소의 모든 파일에 액세스할 수 있습니다.
Letöltés|[`as_download()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.azure_storage_datastore.abstractazurestoragedatastore?view=azure-ml-py#as-download-path-on-compute-none-)|를 사용 하 여 `path_on_compute`에 지정 된 위치에 데이터 저장소의 콘텐츠를 다운로드 합니다. <br><br> 이 다운로드는 실행 전에 발생 합니다.
Feltöltés|[`as_upload()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.azure_storage_datastore.abstractazurestoragedatastore?view=azure-ml-py#as-upload-path-on-compute-none-)| `path_on_compute`에 지정 된 위치에서 데이터 저장소에 파일을 업로드 하는 데 사용 합니다. <br><br> 이 업로드는 실행 후에 수행 됩니다.
...
`script_params` 변수는 entry_script에 대 한 매개 변수를 포함 하는 사전입니다. 이를 사용 하 여 데이터 저장소를 전달 하 고 계산 대상에서 데이터를 사용할 수 있는 방법을 설명 합니다. 종단 간 [자습서](tutorial-train-models-with-aml.md)에서 자세히 알아보세요.
...
```
학습에 .Runconfig 개체를 사용 하려면 [Datareference](https://docs.microsoft.com/en-us/python/api/azureml-core/azureml.data.data_reference.datareference?view=azure-ml-py) 개체를 설정 해야 합니다. 

다음 코드에서는 예측 파이프라인에서 DataReference 개체를 사용 하는 방법을 보여 줍니다. 전체 예제는이 [노트북](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/machine-learning-pipelines/intro-to-pipelines/aml-pipelines-how-to-use-estimatorstep.ipynb)을 참조 하세요.
...
| Adatátvitel                  |[ML&nbsp;파이프라인](concept-ml-pipelines.md)                                               |–                                           |[ML&nbsp;파이프라인](concept-ml-pipelines.md)            |[ML&nbsp;파이프라인](concept-ml-pipelines.md)                                                                            |
| Databricks                     |[ML&nbsp;파이프라인](concept-ml-pipelines.md)                                              |–                                           |[ML&nbsp;파이프라인](concept-ml-pipelines.md)             |–                                                                         |
| Azure Batch                    |[ML&nbsp;파이프라인](concept-ml-pipelines.md)                                               |–                                           |–         |–                                                                         |
| Azure DataLake 분석       |–                                           |–                                           |[ML&nbsp;파이프라인](concept-ml-pipelines.md)             |–                                                                         |
