...
CLI를 사용 하 여 배포 하려면 다음 명령을 사용 합니다. `myaks`을 AKS 계산 대상의 이름으로 바꿉니다. `mymodel:1`를 등록 된 모델의 이름 및 버전으로 바꿉니다. 이 서비스를 제공할 이름으로 `myservice`을 바꿉니다.
