...
이 파이프라인은 제조업체, 모델, 마 력, 크기 등의 기술 기능에 따라 자동차 가격을 예측 하는 **의사 결정 포리스트 회귀 변수** 를 교육 합니다. "어떻게 되나요?" 라는 질문에 답변 하려고 하기 때문에 이를 회귀 문제 라고 합니다. 그러나이 예제에서 동일한 기본 단계를 적용 하 여 재발, 분류, 클러스터링 등의 모든 유형의 기계 학습 문제를 다룰 수 있습니다.
...
4. 샘플 1 파이프라인에 대 한 **열기** 단추를 선택 합니다.

    ![파이프라인 열기](media/how-to-ui-sample-regression-predict-automobile-price-basic/open-sample1.png)
