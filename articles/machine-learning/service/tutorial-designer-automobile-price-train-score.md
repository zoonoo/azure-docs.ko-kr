...
description: 끌어서 놓기 인터페이스를 사용하여 Machine Learning 모델을 학습하고 점수를 매기고 배포하는 방법에 대해 알아봅니다. 이 자습서는 선형 회귀 분석을 사용하여 자동차 가격을 예측하는 2부로 구성된 시리즈 중 제1부입니다.
author: peterclu
ms.author: peterlu
...
1부에서는 환경을 설정하고, 모듈을 대화형 캔버스에 끌어서 놓고, 서로 연결하여 Azure Machine Learning 파이프라인을 만듭니다.
...
이 자습서의 [2부](tutorial-designer-automobile-price-deploy.md)에서는 예측 모델을 실시간 추론 엔드포인트로 배포하여 보내는 기술 사양에 따라 자동차의 가격을 예측하는 방법에 대해 알아봅니다. 

> [!Note]
>이 자습서의 전체 버전은 샘플 파이프라인으로 사용할 수 있습니다.
>
>이를 찾으려면 **작업 영역의 디자이너**로 이동합니다. **새 파이프라인** 섹션에서 **샘플 1 - 회귀 분석: 자동차 가격 예측(기본)** 을 선택합니다.
...
**Enterprise 버전**에 Azure Machine Learning 작업 영역이 있으면 [다음 섹션으로 건너뜁니다](#create-the-pipeline).
...
1. 캔버스 위쪽에서 기본 파이프라인 이름인 **"Pipeline-Created-on ...** "을 선택하고, 의미 있는 이름으로 바꿉니다. 예를 들어 **"자동차 가격 예측"** 으로 바꿉니다. 이름은 고유할 필요가 없습니다.

## <a name="import-data"></a>데이터 가져오기

디자이너에는 실험에 사용할 몇 가지 샘플 데이터 세트가 포함되어 있습니다. 이 예제의 경우 **자동차 가격 데이터(원시)** 데이터 세트 샘플을 사용합니다. 
...
사용할 데이터 세트를 이해하기 위해 데이터를 시각화할 수 있습니다.
...
데이터 세트는 일반적으로 분석하기 전에 일부 전처리가 필요 합니다. 데이터 세트를 시각화할 때 일부 누락된 값이 발견될 수도 있습니다. 모델에서 데이터를 올바르게 분석할 수 있도록 이러한 누락된 값을 정리해야 합니다. 누락된 값이 많은 열을 제거하고, 누락된 값이 있는 개별 행을 제거합니다.
...
1. 앞에서 추가한 데이터 세트를 클릭하고 **데이터 세트에서 열 선택** 모듈로 끌어서 연결합니다. 데이터 세트의 출력 포트(캔버스의 데이터 세트 아래쪽에 있는 작은 원)에서 **데이터 세트에서 열 선택**의 입력 포트(모듈 위쪽에 있는 작은 원)로 끌어서 놓습니다.
...
1. **속성**에서 **매개 변수** > **주석**을 차례로 선택하고, "정규화된 손실 제외"를 입력합니다.
...
1. **누락된 데이터 정리** 모듈을 파이프라인 캔버스로 끌어 **데이터 세트에서 열 선택** 모듈에 연결합니다. 

1. 속성 창의 **정리 모드** 아래에서 **전체 열 제거**를 선택합니다.

1. [속성] 창의 **주석** 상자에서 "누락된 값 행을 제거합니다."를 입력합니다.  

    파이프라인이 이제 다음과 같이 표시됩니다.
    
    ![열 선택](./media/ui-tutorial-automobile-price-train-score/pipeline-clean.png)

## <a name="train-a-machine-learning-model"></a>기계 학습 모델 학습

이제 데이터가 전처리되었으므로 예측 모델을 생성할 수 있습니다. 데이터를 사용하여 모델을 학습합니다. 그런 다음, 모델을 테스트하여 가격을 얼마나 근접하게 예측할 수 있는지 확인합니다.
...
데이터를 두 개의 개별 데이터 세트로 분할하여 모델을 학습시키고 테스트하는 데 해당 데이터를 모두 사용합니다.
...
1. [속성] 창에서 **첫 번째 출력 데이터 세트의 행 비율**을 0.7로 설정합니다.
    ...
    그러면 데이터의 70%가 모델 학습을 위해 분할되고, 30%가 테스트를 위해 남아 있습니다.

1. [속성] 창의 **주석** 상자에서 "데이터 세트를 학습 세트(0.7)와 테스트 세트(0.3)로 분할합니다."를 입력합니다.
...
1. **Machine Learning 알고리즘**을 펼칩니다.
    ...
    기계 학습 알고리즘을 초기화하는 데 사용할 수 있는 몇 가지 범주의 모듈이 표시됩니다.

1. 이 파이프라인에 대해 **회귀** > **선형 회귀**를 차례로 선택하여 파이프라인 캔버스로 끕니다.
...
1. **레이블 열** 대화 상자에서 드롭다운 메뉴를 펼치고, **열 이름**을 선택합니다. 텍스트 상자에서 **가격**을 입력합니다. 가격은 모델에서 예측해야 하는 값입니다.
...
1. 검색 상자에서 **모델 점수 매기기**를 입력하여 **모델 점수 매기기** 모듈을 찾고, 해당 모듈을 파이프라인 캔버스로 끕니다. 

1. **모델 학습** 모듈의 출력을 **모델 점수 매기기**의 왼쪽 입력 포트에 연결합니다. **데이터 분할** 모듈의 테스트 데이터 출력(오른쪽 포트)을 **모델 점수 매기기**의 오른쪽 입력 포트에 연결합니다.

1. 검색 상자에서 **평가**를 입력하여 **모델 평가**를 찾고, 해당 모듈을 파이프라인 캔버스로 끕니다. 
...
1. **모델 점수 매기기** 모듈을 선택하여 **모델 점수 매기기** 모듈의 출력을 봅니다.

1. **속성** 창에서 **출력** > **시각화**를 차례로 선택합니다. 출력에 테스트 데이터에서 가져온 알려진 값과 함께 가격 예측 값이 표시됩니다.

    !["점수를 매긴 레이블"을 강조 표시한 출력 시각화의 스크린샷](./media/ui-tutorial-automobile-price-train-score/score-result.png)

1. **모델 점수 매기기** 모듈을 선택하여 **모델 평가** 모듈의 출력을 봅니다.

1. **속성** 창에서 **출력** > **시각화**를 차례로 선택한 다음, **시각화**를 선택합니다.
