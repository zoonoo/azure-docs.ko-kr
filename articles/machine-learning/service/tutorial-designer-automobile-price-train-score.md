...
1. 팔레트 위쪽의 검색 상자에서 **선택**을 입력하여 **데이터 세트에서 열 선택** 모듈을 찾습니다.
...
1. 검색 상자에서 **데이터 분할**을 입력하여 **데이터 분할** 모듈을 찾고, 해당 모듈을 **누락된 데이터 정리** 모듈의 왼쪽 포트에 연결합니다.
...
1. **Machine Learning 알고리즘**을 펼칩니다.
    ...
    그러면 학습 알고리즘을 초기화하는 데 사용할 수 있는 몇 가지 범주의 모듈이 표시됩니다.

1. **회귀** > **선형 회귀**를 차례로 선택하여 파이프라인 캔버스로 끕니다.
...
1. **레이블 열** 대화 상자에서 드롭다운 메뉴를 펼치고, **열 이름**을 선택합니다. 
