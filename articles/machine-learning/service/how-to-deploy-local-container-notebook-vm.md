...
ms.locfileid: "71002821"
---
# <a name="deploy-a-model-to-notebook-vms"></a>노트북 Vm에 모델 배포

Azure Machine Learning를 사용 하 여 노트북 VM에 웹 서비스로 모델을 배포 하는 방법을 알아봅니다. 다음 조건 중 하나에 해당 하는 경우 노트북 Vm을 사용 합니다.
...
1. 노트북에는 서비스가 실행 되는 URL 및 포트가 표시 됩니다. 예를 들어, `https://localhost:6789`을 입력합니다. 가 포함 `print('Local service port: {}'.format(local_service.port))` 된 셀을 실행 하 여 포트를 표시할 수도 있습니다.

    ![실행 중인 로컬 서비스 포트의 스크린샷](media/how-to-deploy-local-container-notebookvm/deploy-local-service-port.png)

1. 노트북 VM에서 서비스를 테스트 하려면 `https://localhost:<local_service.port>` URL을 사용 합니다. 원격 클라이언트에서 테스트 하려면 노트북 VM에서 실행 되는 서비스의 공용 URL을 가져옵니다. 공용 URL은 다음 수식을 사용 하 여 확인할 수 있습니다. `https://<notebookvm_name>-<local_service_port>.<azure_region_of_notebook>.notebooks.azureml.net/score`. 예를 들어, `https://mynotebookvm-6789.eastus2.notebooks.azureml.net/score`을 입력합니다.
