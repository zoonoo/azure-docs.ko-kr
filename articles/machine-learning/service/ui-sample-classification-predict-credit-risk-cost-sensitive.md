...
Machine learning을 처음 사용 하는 경우 [기본 분류자 샘플](ui-sample-classification-predict-credit-risk-basic.md) 을 먼저 살펴볼 수 있습니다.
...
4. 샘플 4 실험에 대 한 **열기** 단추를 선택 합니다.
    ...
    ![실험 열기](media/ui-sample-classification-predict-credit-risk-cost-sensitive/open-sample4.png)

## <a name="data"></a>보기

UC Irvine 리포지토리에서 독일어 신용 카드 데이터 집합을 사용 합니다. 이 데이터 집합에는 20 개의 기능이 포함 된 1000 샘플 및 1 개의 레이블이 포함 되어 있습니다. 각 샘플은 사람을 나타냅니다. 20 개의 기능에는 숫자 및 범주 기능이 포함 됩니다. 데이터 집합에 대 한 자세한 내용은 [UCI 웹 사이트](https://archive.ics.uci.edu/ml/datasets/Statlog+%28German+Credit+Data%29) 를 참조 하세요. 마지막 열은 신용 위험을 나타내며 두 개의 가능한 값, 즉 높은 신용 위험 = 2, 낮은 신용 위험 = 1만 포함 하는 레이블입니다.
...
두 방법을 모두 사용 하 여 테스트 데이터 집합을 복제와 함께 사용 하 여 결과를 cost 함수에 맞추도록 모델을 평가 합니다. 두 가지 방법을 모두 사용 하 여 두 분류자를 테스트 합니다. **2 클래스 지원 벡터 컴퓨터** 와 **2 클래스 승격 된 의사 결정 트리입니다**.
...
그런 다음 위험 예측 모델을 개발 하는 데 사용 되는 학습 및 테스트 집합을 생성 합니다. **데이터 분할** 모듈을 사용 하 여 원래 데이터 집합을 동일한 크기의 학습 및 테스트 집합으로 분할 합니다. 동일한 크기의 집합을 만들기 위해 **첫 번째 출력 데이터 집합 옵션에서 행의 소수 부분** 을 0.5으로 설정 합니다.

### <a name="generate-the-new-dataset"></a>새 데이터 집합 생성

위험을 과소 평가 하는 비용은 매우 높아서 다음과 같이 잘못 분류 하는 비용을 설정 합니다.

- 위험 수준이 높은 경우에는 위험이 잘못 분류. 5
- 위험 수준이 낮은 경우 높은 위험으로 잘못 분류: 1

이 비용 함수를 반영 하기 위해 새 데이터 집합을 생성 합니다. 새 데이터 집합에서 위험이 높은 각 예제는 5 번 복제 되지만 위험이 낮은 예제의 수는 변경 되지 않습니다. 복제 전에 데이터를 학습 및 테스트 데이터 집합으로 분할 하 여 동일한 행이 두 집합에 있는 것을 방지 합니다.

위험 수준이 높은 데이터를 복제 하기 위해 python 코드를 **Python 스크립트 실행** 모듈에 저장 합니다.
...
두 분류자, 2 **클래스 지원 벡터 컴퓨터** (SVM) 및 **2 클래스 승격 된 의사 결정 트리**를 적용 하 고 두 개의 데이터 집합을 사용 하기 때문에 총 4 개의 모델을 생성 합니다.
...
표준 실험적 워크플로를 사용 하 여 모델을 만들고 학습 하 고 테스트 합니다.
...
실험의 평가 단계에서 4 개의 각 모델에 대 한 정확도를 계산 합니다. 이 실험에서는 **모델 평가** 를 사용 하 여 동일한 오 분류 비용을 갖는 예제를 비교 합니다.

**모델 평가** 모듈은 점수가 매겨진 두 모델에 대 한 성능 메트릭을 계산할 수 있습니다. **모델 평가** 의 한 인스턴스를 사용 하 여 두 개의 SVM 모델을 평가 하 고 **모델 평가** 의 다른 인스턴스를 사용 하 여 두 개의 승격 된 의사 결정 트리 모델을 평가 합니다.
...
![출력 시각화](media/ui-sample-classification-predict-credit-risk-cost-sensitive/result.png)
