...
ms.locfileid: "104607831"
---
# <a name="azure-security-baseline-for-azure-machine-learning"></a>Azure Machine Learning에 대 한 Azure 보안 기준

이 보안 기준은 [Azure Security 벤치 마크 버전 1.0](../security/benchmarks/overview-v1.md) 의 지침을 Microsoft Azure Machine Learning 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다.
콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤** 및 Azure Machine Learning에 적용 되는 관련 지침에 따라 그룹화 됩니다. Azure Machine Learning에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

 
Azure Machine Learning 완전히 Azure 보안 벤치 마크에 매핑되는 방법을 보려면 [전체 Azure Machine Learning 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
- [Azure Machine Learning에 대 한 진단 로그를 구성 하는 방법](https://docs.microsoft.com/azure/machine-learning/monitor-azure-machine-learning#configuration)
...
- [Log Analytics 작업 영역에서 Azure Machine Learning에 대 한 쿼리를 수행 하는 방법](https://docs.microsoft.com/azure/machine-learning/monitor-azure-machine-learning#analyzing-log-data)
...
- [Azure Machine Learning 경고에 대 한 자세한 내용은](https://docs.microsoft.com/azure/machine-learning/monitor-azure-machine-learning#alerts)
...
- [Cloud services에 대 한 Microsoft 맬웨어 방지 확장을 구성 하는 방법](/powershell/module/servicemanagement/azure.service/set-azurevmmicrosoftantimalwareextension)
...
Azure Active Directory (Azure AD) Privileged Identity Management 및 Azure Resource Manager를 사용 하 여 관리 계정에 대 한 just-in-time 액세스를 사용 하도록 설정할 수도 있습니다.

- [기본 역할 Machine Learning 대 한 자세한 내용은](https://docs.microsoft.com/azure/machine-learning/how-to-assign-roles#default-roles)

- [Privileged Identity Management에 대 한 자세한 정보](/azure/active-directory/privileged-identity-management/index)
...
**지침**: Machine Learning은 Azure Active Directory (azure ad)와 통합 되며, 서비스별 개별 독립 실행형 자격 증명을 구성 하는 대신 AZURE ad SSO를 사용 합니다. Azure Security Center id 및 액세스 권장 사항을 사용 합니다.
...
**지침**: Azure Active Directory (Azure AD) 다단계 인증을 사용 하도록 설정 하 고 Azure Security Center id 및 액세스 권장 사항을 따릅니다.
...
- [휴지 상태의 데이터 암호화 Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/concept-enterprise-security#encryption-at-rest)
...
**지침**: Azure Machine Learning 다양 한 계산 리소스 및 사용자 고유의 계산 리소스에 대해 다양 한 지원을 제공 합니다. 조직에서 소유 하는 계산 리소스의 경우 Azure 역할 기반 액세스 제어 (Azure RBAC)를 사용 하 여 권한 있는 사용자만 사용자 지정 이미지에 액세스할 수 있도록 합니다. Azure 공유 이미지 갤러리를 사용 하 여 조직 내의 다른 사용자, 서비스 주체 또는 Azure Active Directory (Azure AD) 그룹에 이미지를 공유할 수 있습니다. 컨테이너 이미지를 Azure Container Registry에 저장 하 고 Azure RBAC를 사용 하 여 권한이 있는 사용자만 액세스할 수 있도록 합니다.
...
Azure Machine Learning은 다양 한 계산 리소스 및 사용자 고유의 계산 리소스에 대해 다양 한 지원을 제공 합니다. 조직에서 소유 하는 계산 리소스의 경우 클라우드 또는 온-프레미스 데이터 센터의 DSC (Desired State Configuration) 노드에 대 한 Azure Automation 상태 구성을 사용 합니다. 컴퓨터를 쉽게 등록 하 고, 선언적 구성을 할당 하 고, 지정 된 원하는 상태를 사용 하 여 각 컴퓨터의 호환성을 보여 주는 보고서를 볼 수 있습니다. 
...
- [고객 관리 키 Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/concept-enterprise-security#azure-blob-storage)
...
**지침**: Azure Machine Learning 기본 제공 역할 및 사용자 지정 역할을 만드는 기능을 모두 지원 합니다. 관리 되는 id를 사용 하 여 azure AD (Azure Active Directory)에서 자동으로 관리 되는 id를 Azure 서비스에 제공 합니다. 관리 id를 사용 하면 코드에 자격 증명 없이 Key Vault를 포함 하 여 Azure AD 인증을 지 원하는 모든 서비스에 인증할 수 있습니다.
...
- [Azure에서 Key Vault 키를 복원 하는 방법](/powershell/module/az.keyvault/restore-azkeyvaultkey)
...
**지침**: 온-프레미스 백업의 경우 Azure에 백업할 때 제공 하는 암호를 사용 하 여 미사용 암호화가 제공 됩니다. Azure RBAC (역할 기반 액세스 제어)를 사용 하 여 백업과 고객이 관리 하는 키를 보호 합니다. 
