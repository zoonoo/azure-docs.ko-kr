...
> [!Warning]
> 전용 끝점을 사용 하 여 작업 영역을 보호 하는 것은 완전 한 보안을 보장 하지 않습니다. 이 문서의 나머지 부분에 있는 단계와 VNet 시리즈를 수행 하 여 솔루션의 개별 구성 요소를 보호 해야 합니다.
...
1. Storage 서비스 계정 페이지에서 __네트워킹__ 을 선택 합니다.

   ![Azure Portal Azure Storage 페이지의 네트워킹 영역](./media/how-to-enable-virtual-network/storage-firewalls-and-virtual-networks.png)
...
1. __방화벽 및 가상 네트워크__ 탭에서 다음 작업을 수행 합니다.
    ...
    1. __신뢰할 수 있는 Microsoft 서비스가 이 스토리지 계정에 액세스하도록 허용합니다__ 확인란을 선택합니다. 이 변경 내용은 모든 Azure 서비스에 저장소 계정에 대 한 액세스 권한을 부여 하지 않습니다.
...
기본적으로 Azure Machine Learning는 SDK를 사용 하 여 데이터에 액세스 하려고 할 때 데이터 유효성 및 자격 증명 확인을 수행 합니다. 데이터가 가상 네트워크 뒤에 있는 경우 Azure Machine Learning는 이러한 검사를 완료할 수 없습니다. 이 확인을 무시 하려면 유효성 검사를 건너뛰는 데이터 저장소 및 데이터 집합을 만들어야 합니다.
...
* Azure Machine Learning 작업 영역에 [Azure Machine Learning 컴퓨팅 클러스터](how-to-create-attach-compute-cluster.md)가 포함되어야 합니다.
    ...
    > [!IMPORTANT]
    > Docker 이미지를 빌드하는 데 사용 되는 계산 클러스터는 모델을 학습 하 고 배포 하는 데 사용 되는 패키지 리포지토리에 액세스할 수 있어야 합니다. 공용 리포지토리에 대 한 액세스를 허용 하는 네트워크 보안 규칙을 추가 하거나, [사설 Python 패키지를 사용](how-to-use-private-python-packages.md)하거나, 이미 패키지를 포함 하는 [사용자 지정 Docker 이미지](how-to-train-with-custom-image.md) 를 사용 해야 할 수 있습니다.

이러한 요구 사항이 충족 되 면 다음 단계를 사용 하 여 Azure Container Registry를 사용 하도록 설정 합니다.

> [!TIP]
> 작업 영역을 만들 때 기존 Azure Container Registry를 사용 하지 않은 경우에는 존재 하지 않을 수 있습니다. 기본적으로 작업 영역은 필요한 경우에만 ACR 인스턴스를 만들지 않습니다. 하나를 강제로 만들도록 하려면이 섹션의 단계를 사용 하기 전에 작업 영역을 사용 하 여 모델을 학습 하거나 배포 합니다.
...
1. 작업 영역에 대 한 ACR을 구성 하 여 [신뢰할 수 있는 서비스의 액세스를 허용](../container-registry/allow-access-trusted-services.md)합니다.
