...
> [!WARNING]
> Microsoft는 기본 저장소에 가상 네트워크가 활성화된 경우 자동화된 ML, 데이터 집합, 데이터 레이블 지정, 디자이너 및 노트북과 같은 Azure 기계 학습 스튜디오 기능을 지원하지 않습니다.
...
1. 방화벽 및 가상 네트워크 페이지에서 다음 작업을 __수행합니다.__
    ...
    - __가상 네트워크에서__기존 __가상 네트워크 추가__ 링크를 선택합니다. 이 작업은 계산이 상주하는 가상 네트워크를 추가합니다(1단계 참조).
        ...
        > [!IMPORTANT]
        > 저장소 계정은 학습 또는 추론에 사용되는 계산 인스턴스 또는 클러스터와 동일한 가상 네트워크 및 서브넷에 있어야 합니다.
...
1. [Azure 기계 학습 스튜디오에](https://ml.azure.com/)로그인한 다음 구독 및 작업 영역을 선택합니다.

1. 왼쪽에서 __Compute__를 선택합니다.

1. 센터에서 __교육 클러스터를__ 선택한 다음 __+__ 을 선택합니다.

1. 새 __교육 클러스터__ 대화 상자에서 __고급 설정__ 섹션을 확장합니다.

1. 가상 네트워크를 사용하도록 이 계산 리소스를 구성하려면 가상 __네트워크 구성__ 섹션에서 다음 작업을 수행합니다.

    1. 리소스 __그룹__ 드롭다운 목록에서 가상 네트워크가 포함된 리소스 그룹을 선택합니다.
    1. 가상 __네트워크__ 드롭다운 목록에서 서브넷이 포함된 가상 네트워크를 선택합니다.
...
1. 중심에서 __유추 클러스터를__ 선택한 다음 __+__ 을 선택합니다.

1. 새 __추론 클러스터__ 대화 상자에서 네트워크 __구성에서__ __고급__ 을 선택합니다.
...
1. 가상 네트워크를 사용하도록 이 계산 리소스를 구성하려면 다음 작업을 수행합니다.
    ...
    1. __Kubernetes 서비스 주소 범위__ 상자에 Kubernetes 서비스 주소 범위를 입력합니다. 이 주소 범위는 클래스리스 도메인 라우팅(CIDR) 표기 형 IP 범위를 사용하여 클러스터에 사용할 수 있는 IP 주소를 정의합니다. 서브넷 IP 범위(예: 10.0.0.0/16)와 겹치지 않아야 합니다.
    1. __Kubernetes DNS 서비스 IP 주소__ 상자에 Kubernetes DNS 서비스 IP 주소를 입력합니다. 이 IP 주소는 Kubernetes DNS 서비스에 할당됩니다. Kubernetes 서비스 주소 범위(예: 10.0.0.10) 내에 있어야 합니다.
    1. Docker __브리지 주소__ 상자에 Docker 브리지 주소를 입력합니다. 이 IP 주소는 Docker 브리지에 할당됩니다. 서브넷 IP 범위 또는 Kubernetes 서비스 주소 범위(예: 172.17.0.1/16)에 없어야 합니다.
...
> [!IMPORTANT]
> Azure 컨테이너 레지스트리(ACR)는 가상 네트워크 내에 넣을 수 있지만 다음 필수 구성 조건을 충족해야 합니다.
>
> * Azure 기계 학습 작업 영역은 엔터프라이즈 버전이어야 합니다. 업그레이드에 대한 자세한 내용은 [엔터프라이즈 버전으로 업그레이드 버전을](how-to-manage-workspace.md#upgrade)참조하십시오.
> * Azure 컨테이너 레지스트리는 프리미엄 버전이어야 합니다. 업그레이드에 대한 자세한 내용은 [SCO 변경](/azure/container-registry/container-registry-skus#changing-skus)을 참조하십시오.
> * Azure 컨테이너 레지스트리는 저장소 계정과 동일한 가상 네트워크 및 서브넷에 있어야 하며 학습 또는 추론에 사용되는 대상을 계산해야 합니다.
> * Azure 기계 학습 작업 영역에 [Azure 기계 학습 계산 클러스터가](how-to-set-up-training-targets.md#amlcompute)포함되어야 합니다.
>
>     ACR이 가상 네트워크 뒤에 있는 경우 Azure 기계 학습은 이를 사용하여 Docker 이미지를 직접 빌드할 수 없습니다. 대신 계산 클러스터를 사용하여 이미지를 빌드합니다.

1. 작업 영역에 대한 Azure 컨테이너 레지스트리의 이름을 찾으려면 다음 방법 중 하나를 사용합니다.

    __Azure Portal__

    작업 영역의 개요 섹션에서 __레지스트리__ 값은 Azure 컨테이너 레지스트리에 연결됩니다.

    ![작업 영역에 대한 Azure 컨테이너 레지스트리](./media/how-to-enable-virtual-network/azure-machine-learning-container-registry.png)

    __Azure CLI__

    [Azure CLI에 대한 기계 학습 확장을 설치한](reference-azure-machine-learning-cli.md) `az ml workspace show` 경우 명령을 사용하여 작업 영역 정보를 표시할 수 있습니다.

    ```azurecli-interactive
    az ml workspace show -w yourworkspacename -g resourcegroupname --query 'containerRegistry'
    ```

    이 명령은 `"/subscriptions/{GUID}/resourceGroups/{resourcegroupname}/providers/Microsoft.ContainerRegistry/registries/{ACRname}"`와 비슷한 값을 반환합니다. 문자열의 마지막 부분은 작업 영역에 대한 Azure 컨테이너 레지스트리의 이름입니다.

1. 가상 네트워크에 대한 액세스를 제한하려면 [레지스트리에 대한 네트워크 액세스 구성의](../container-registry/container-registry-vnet.md#configure-network-access-for-registry)단계를 사용합니다. 가상 네트워크를 추가할 때 Azure 기계 학습 리소스에 대한 가상 네트워크 및 서브넷을 선택합니다.

1. Azure 기계 학습 파이썬 SDK를 사용하여 도커 이미지를 빌드하도록 계산 클러스터를 구성합니다. 다음 코드 조각에서는 이 작업을 수행하는 방법을 보여 줍니다.
