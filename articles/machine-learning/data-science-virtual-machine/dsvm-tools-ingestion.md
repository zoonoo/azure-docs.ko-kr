...
데이터 과학 또는 AI 프로젝트에서 첫 번째 기술적 단계 중 하나는 사용할 데이터 세트를 식별하고 분석 환경으로 가져오는 것입니다. 데이터 과학 Virtual Machine(DSVM)은 서로 다른 원본에서 데이터를 로컬로 DSVM이나 클라우드 또는 온-프레미스의 데이터 플랫폼에 있는 분석 데이터 저장소로 가져올 도구 및 라이브러리를 제공합니다. 
