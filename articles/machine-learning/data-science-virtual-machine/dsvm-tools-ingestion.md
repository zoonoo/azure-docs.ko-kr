...
ms.locfileid: "68557709"
---
# <a name="data-science-virtual-machine-data-ingestion-tools"></a>데이터 과학 Virtual Machine 데이터 수집 도구

데이터 과학 또는 AI 프로젝트에서 첫 번째 기술적 단계 중 하나는 사용할 데이터 세트를 식별하고 분석 환경으로 가져오는 것입니다. DSVM (Data Science Virtual Machine)은 여러 소스의 데이터를 DSVM 또는 클라우드 또는 온-프레미스의 데이터 플랫폼에서 로컬로 분석 데이터 저장소로 가져올 수 있는 도구 및 라이브러리를 제공 합니다. 
...
|    |           |
| ------------- | ------------- |
| 정의   | Azure Storage Blob에서 Azure Data Lake Store로 데이터를 복사하는 도구. 또한 두 Azure Data Lake Store 계정 간에 데이터를 복사할 수도 있습니다.      |
...
|    |           |
| ------------- | ------------- |
| 정의   | Azure용 관리 도구. Azure Storage Blob, Azure Data Lake storage와 같은 Azure 데이터 플랫폼에서 데이터를 이동하기 위한 동사형 명령을 포함합니다.     |
...
|    |           |
| ------------- | ------------- |
| 정의   | 로컬 파일, Azure Storage Blob, 파일 및 테이블 간 데이터를 복사하는 도구.      |
| 지원되는 DSVM 버전      | Windows      |
...
|    |           |
| ------------- | ------------- |
| 정의   | JSON 파일, CSV 파일, SQL, MongoDB, Azure Table Storage, Amazon DynamoDB 및 Azure Cosmos DB SQL API 컬렉션을 비롯한 다양한 원본에서 데이터를 Azure Cosmos DB로 가져오는 도구입니다.      |
| 지원되는 DSVM 버전      | Windows      |
...
|    |           |
| ------------- | ------------- |
| 정의   | SQL Server와 데이터 파일 간에 데이터를 복사하는 SQL Server 도구입니다.      |
| 지원되는 DSVM 버전      | Windows      |
...
|    |           |
| ------------- | ------------- |
| 정의   | Linux 파일 시스템에 Azure Blob 컨테이너를 마운트하는 도구      |
| 지원되는 DSVM 버전      | Linux      |
...
|    |           |
| ------------- | ------------- |
| 정의   | 온-프레미스 데이터 원본을 사용하기 위해 클라우드 서비스에 연결하는 도구.      |
| 지원되는 DSVM 버전      | Windows      |
