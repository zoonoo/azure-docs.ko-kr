...
| DSVM에 구성 및 설치 방법      |  `/dsvm/tools/drill*`에 포함 모드 전용으로만 설치됩니다.   |
| 일반적인 사용 용도      |  ETL(추출, 변환, 로드)을 요구하지 않는 원본 데이터 탐색에 사용합니다. CSV, JSON, 관계형 테이블 및 Hadoop을 비롯한 여러 다른 데이터 원본 및 형식 쿼리입니다.     |
