---
title: '참조: 우분투 데이터 과학 가상 머신'
titleSuffix: Azure Data Science Virtual Machine
description: 우분투 데이터 과학 가상 머신에 포함 된 도구에 대한 자세한 내용
author: gvashishtha
...
당신의 우분투 데이터 과학 가상 머신에서 사용할 수 있는 도구의 목록에 대 한 아래를 참조 하십시오. 
...
자세한 내용은 [GitHub및](https://github.com/Microsoft/CNTK) [CNTK 위키의 CNTK](https://github.com/Microsoft/CNTK/wiki)섹션을 참조하십시오.
...
명령줄에서 H2O를 열려면 `java -jar /dsvm/tools/h2o/current/h2o.jar`을 실행합니다. 구성해야 할 수도 있는 다양한 [명령줄 옵션](http://docs.h2o.ai/h2o/latest-stable/h2o-docs/starting-h2o.html#from-the-command-line)이 있습니다. 시작하려면 http://localhost:54321로 이동하여 Flow 웹 UI에 액세스할 수 있습니다. 샘플 노트북은 JupyterHub에서도 사용할 수 있습니다.
...
Jupyter Notebook 서버는 Python 2, Python 3 및 R 커널을 사용하여 미리 구성되어 있습니다. **Jupyter Notebook** 데스크톱 아이콘을 사용하여 브라우저를 열고 Notebook 서버에 액세스합니다. SSH 또는 X2Go 클라이언트를 통해 VM에 있는 경우 에서 [https://localhost:8000/](https://localhost:8000/)Jupyter 노트북 서버에 액세스할 수도 있습니다.
...
자세한 내용은 [SQuirrel SQL](http://squirrel-sql.sourceforge.net/index.php?page=screenshots)을 참조하십시오.
...
- **bcp**: bcp 도구 대량 은 Microsoft SQL Server 인스턴스와 사용자가 지정한 형식으로 데이터 파일 간에 데이터를 복사합니다. bcp 도구를 사용하여 많은 수의 새 행을 SQL Server 테이블로 가져오거나 데이터를 테이블에서 데이터 파일로 내보낼 수 있습니다. 데이터를 테이블로 가져오려면 해당 테이블에 대해 만든 서식 파일을 사용해야 합니다. 또는 테이블의 구조와 해당 열에 유효한 데이터 형식을 이해해야 합니다.
...
* **Azure 저장소 탐색기**: Azure 저장소 탐색기는 Azure 저장소 계정에 저장된 개체를 탐색하고 Azure Blob에서 데이터를 업로드 및 다운로드하는 데 사용할 수 있는 그래픽 도구입니다. 바탕 화면 바로 가기 아이콘을 사용하여 Storage Explorer에 액세스할 수 있습니다. 셸 프롬프트에서 **StorageExplorer**를 입력하여 열 수도 있습니다. X2Go 클라이언트에서 로그인하거나 X11 전달이 설정되어 있어야 합니다.
* **Azure 라이브러리**: 다음은 사전 설치된 라이브러리 중 일부입니다.
...
사전 설치된 Firefox 브라우저에서 [Azure 포털에](https://portal.azure.com) 액세스할 수 있습니다. Azure 포털에서 Azure 리소스를 작성, 관리 및 모니터링할 수 있습니다.
...
* **R**: R. 사전 설치된 라이브러리에는 lm, glm, randomForest 및 rpart가 포함된 다양한 기계 학습 기능 라이브러리를 사용할 수 있습니다. 다른 라이브러리는 다음 명령을 실행하여 설치할 수 있습니다.
...
1. **로그** 탭을 선택하여 이전 작업에서 생성된 R 코드를 표시합니다.
   Rattle의 현재 릴리스에서 버그로 인해 로그 텍스트에 **#** 이 **로그를 내보내기** 앞에 문자를 삽입해야 합니다.
