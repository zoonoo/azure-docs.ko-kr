...
[Azure 기계 학습을 사용한 예측 솔루션 개발](machine-learning-walkthrough-develop-predictive-solution.md) 자습서의 두 번째 단계입니다.
...
신용 위험에 대한 예측 모델을 개발하기 위해 UCI 기계 학습 리포지토리의 "UCI Statlog(독일 신용 데이터) 데이터 집합"을 사용합니다. 이 데이터 집합은 <a href="http://archive.ics.uci.edu/ml/datasets/Statlog+(German+Credit+Data)">http://archive.ics.uci.edu/ml/datasets/Statlog+(German+Credit+Data)</a>에서 찾을 수 있습니다.
...
1.	작업 영역의 소유자로 지정한 Microsoft 계정을 사용하여 기계 학습 스튜디오([https://studio.azureml.net](https://studio.azureml.net))에 로그인한 후 위쪽의 **Studio** 탭을 클릭합니다.
2.	창 아래쪽에서 **+새로 만들기**를 클릭합니다.
3.	**데이터 집합**을 선택합니다.
4.	**로컬 파일에서**를 선택합니다.
5.	**새 데이터 집합 업로드 대화 상자** 영역에서 **찾아보기**를 클릭하여 앞서 만든 **german.csv** 파일을 찾습니다.
6.	데이터 집합의 이름을 입력합니다. 이 예제에서는 데이터 집합을 "UCI 독일 신용 카드 데이터"라고 합니다.
7.	데이터 형식으로 **머리글 없는 일반 CSV 파일(.nh.csv)**을 선택합니다.
8.	원하는 경우 설명을 추가합니다.
9.	**확인**을 클릭합니다.  
