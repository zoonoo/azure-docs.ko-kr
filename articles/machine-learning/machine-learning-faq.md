...
**내 기존 웹 서비스는 어디에 나열되나요? 내 새로운 ARM 기반 웹 서비스는 어디에 나열되나요?**

기존 웹 서비스는 웹 서비스 탭의 [기계 학습 스튜디오](http://studio.azureml.net)에 나열됩니다. 새로운 ARM 기반 웹 서비스는 [Microsoft Azure 기계 학습 웹 서비스](https://services.azureml.net/) 포털에 나열됩니다. 나열된 사용 가능한 크로스는 없습니다.

## Microsoft Azure 기계 학습 웹 서비스 질문

**Azure 기계 학습 웹 서비스란?**

Azure 기계 학습 웹 서비스를 통해 외부 응용 프로그램에서 기계 학습 워크플로 점수 매기기 모델과 실시간으로 통신할 수 있습니다. 기계 학습 웹 서비스 호출은 외부 응용 프로그램에 예측 결과를 반환합니다. 기계 학습 웹 서비스를 호출하려면 웹 서비스를 배포할 때 만들어진 API 키를 전달합니다. 기계 학습 웹 서비스는 웹 프로그래밍 프로젝트에 일반적으로 사용되는 아키텍처인 REST를 기반으로 합니다.
...
REST API를 사용하고 웹 서비스에 액세스하는 여러 가지 방법이 있습니다. 예를 들어, 웹 서비스를 배포할 때 생성된 샘플 코드를 사용하여 C#, R 또는 Python에서 응용 프로그램을 작성할 수 있습니다(기계 학습 스튜디오의 웹 서비스 대시보드에서 API 도움말 페이지에 사용 가능). 또는 만든 샘플 Microsoft Excel 통합 문서를 사용할 수 있습니다(Studio의 웹 서비스 대시보드에서도 사용 가능).

**새 Azure 기계 학습 웹 서비스의 주요 업데이트는 무엇인가요?**

새 Azure 기계 학습 웹 서비스에 대한 자세한 내용은 [관련 설명서](machine-learning-whats-new.md)를 참조하세요.
...
**예측 실험을 기존 웹 서비스로 이미 배포한 경우 새 (ARM 기반) 웹 서비스로 배포할 수 있나요?**

아니요, 이전에 배포한 실험을 기존 웹 서비스로 배포할 수 없습니다. 새 예측 실험을 만들고 대신 배포해야 합니다.
...
기계 학습 스튜디오를 평가하는 동안 무료 청구 계층을 사용할 수 있습니다. 무료 계층을 사용하면 제한된 용량으로 기존 웹 서비스를 배포할 수도 있습니다.

Azure 기계 학습이 필요를 충족한다고 결정했으면 표준 계층에 등록할 수 있습니다. 등록하려면 Microsoft Azure 구독이 있어야 합니다.

표준 계층에서 기계 학습 스튜디오의 사용에 대해 사용자 요금당 매월 요금이 청구됩니다. 스튜디오에서 실험을 실행하면 실험을 실행할 때 계산 리소스가 청구됩니다. 기존 웹 서비스를 배포하는 경우 트랜잭션 및 계산 시간은 종량제(PAYG) 기준으로 청구됩니다.

새 기계 학습 웹 서비스는 비용을 예측할 수 있도록 청구 계획을 적용합니다. 계층화된 가격 책정은 할인된 요금을 제공하는 동안 용량을 많이 필요로 하는 고객을 위한 것입니다.

계획을 만들 때 API 계산 시간 및 API 트랜잭션이 포함된 수량과 함께 제공되는 고정된 비용에 커밋합니다. 포함된 수량보다 많이 필요한 경우 계획에 추가 인스턴스를 추가할 수 있습니다. 포함된 수량이 더 많이 필요한 경우 포함된 수량을 훨씬 더 많이 제공하고 할인율이 더 나은 높은 계층 계획을 선택할 수 있습니다.
...
트랜잭션은 Azure 기계 학습이 대응하는 API 호출을 나타냅니다. RRS(요청-응답 서비스) 및 BES(배치 실행 서비스) 호출에서 트랜잭션이 집계되고 청구 계획에 대한 요금을 부과합니다.

**RRS와 BES 트랜잭션 모두에 대한 계획에 포함된 트랜잭션 수량을 사용할 수 있나요?**

예, RRS와 BES에서 트랜잭션이 집계되고 청구 계획에 대한 요금을 부과합니다.

**API 계산 시간이란?**

API 계산 시간은 기계 학습 계산 리소스를 사용하여 실행하는 데 드는 API가 호출하는 시간에 대한 청구 단위입니다. 모든 호출은 청구 목적으로 집계됩니다.
...
새 Azure 기계 학습 웹 서비스는 청구 계획을 프로비전하는 데 사용할 수 있는 여러 계층을 제공합니다. 개발/테스트 계층은 비용을 발생시키지 않고 실험을 새 웹 서비스로 테스트할 수 있도록 제한된 만큼의 포함된 수량을 제공하는 계층입니다. 작동 방식을 보려면 "충분히 시험해 볼" 기회가 있습니다.

**저장소 요금은 별도입니까?**

기계 학습 무료 계층은 별도의 저장소가 필요하지 않거나 별도의 저장소를 허용하지 않습니다. 기계 학습 표준 계층을 사용하려면 Azure 저장소 계정이 있어야 합니다. Azure 저장소는 [별도로 청구됩니다](https://azure.microsoft.com/pricing/details/storage/).
...
기계 학습 서비스는 다중 테넌트 서비스이고, 백엔드에서 사용되는 실제 계산 리소스는 다양하며 성능 및 예측성을 위해 최적화됩니다.
...
계획 인스턴스는 청구 계획에 추가할 수 있는 포함된 수량의 단위입니다. 청구 계획에 대한 청구 계층을 선택하면 인스턴스 하나와 함께 제공됩니다. 포함된 수량보다 많이 필요한 경우 계획에 선택한 청구 계층의 인스턴스를 추가할 수 있습니다.

**계획 인스턴스를 얼마나 추가할 수 있나요?**

구독에는 개발/테스트 계층의 인스턴스 하나를 사용할 수 있습니다.

S1, S2 및 S3 계층에 필요한 만큼 인스턴스를 추가할 수 있습니다.

참고: 예상된 사용량에 따라 현재 계층에 인스턴스를 추가하지 않고 더 높은 포함된 수량 계층으로 업그레이드하는 편이 비용 효율적입니다.

**계획 계층을 변경(업그레이드/다운그레이드)하는 경우 어떻게 되나요?**

이전 계획은 삭제되고 현재 사용량은 비례적으로 청구됩니다. 업그레이드/다운 그레이드된 계층의 전체 포함된 수량을 사용한 새 계획은 나머지 기간 동안 생성됩니다.

참고: 포함된 수량은 기간당 할당되고 사용하지 않은 수량은 롤오버되지 않습니다.

**계획의 인스턴스를 증가시키면 어떻게 되나요?**

포함된 수량은 비례적으로 포함되고 적용되는 데 24시간이 걸릴 수 있습니다.
...
청구 계획을 만드는 두 가지 방법이 있습니다.

새 웹 서비스를 처음 배포할 때 기존 계획을 선택하거나 새 계획을 만들 수 있습니다.

이 방식으로 만든 계획은 기본 지역에 있으며 웹 서비스도 해당 지역에 배포됩니다.

서비스를 배포하기 전에 청구 계획을 정의하려고 할 수 있습니다. 예를 들어, 기본 지역 이외의 지역에 서비스를 배포하려는 경우입니다.

이 경우에 Azure 기계 학습 웹 서비스 포털에 로그인하여 계획 페이지를 탐색할 수 있습니다. 거기에서 계획을 추가하고 삭제할 수 있을 뿐만 아니라 기존 계획을 수정할 수 있습니다.

**먼저 선택해야 하는 계획은 무엇인가요?**

표준 S1 계층을 시작하고 서비스의 사용량을 모니터링하는 것이 좋습니다. 포함된 수량을 신속하게 사용하는 경우 인스턴스를 추가하거나 더 높은 계층으로 이동하여 할인된 요금을 사용할 수 있습니다. 청구 주기 동안 필요에 따라 청구 계획을 조정할 수 있습니다.

**새 계획에서 어떤 지역을 사용할 수 있나요?**

새 웹 서비스를 지원하는 다음과 같은 세 개의 프로덕션 지역에서 새 청구 계획을 사용할 수 있습니다.
...
Azure 기계 학습 웹 서비스 포털의 계획 페이지에서 모든 계획의 사용량을 볼 수 있습니다. 포털에 로그인하고 계획 메뉴 옵션을 클릭합니다.

테이블의 트랜잭션 및 계산 열에서 계획의 포함된 수량 및 사용된 백분율을 볼 수 있습니다.

**개발/테스트 계층에 포함된 수량을 사용하면 어떻게 되나요?**

개발/테스트 계층이 할당된 서비스가 다음 기간까지 중지되거나 유료 계층 중 하나로 이동됩니다.

**기존 웹 서비스 및 새 웹 서비스 초과분의 경우 RRS(요청 응답) 및 BES(배치) 워크로드에 대한 가격은 어떻게 계산되나요?**

RRS 워크로드의 경우 요청하신 모든 API 트랜잭션 호출 및 해당 요청과 연관된 계산 시간에 대한 비용이 청구됩니다. 따라서 RRS 프로덕션 API 트랜잭션 비용은 요청하신 API 호출 수에 트랜잭션 1,000개당 가격을 곱하여 계산됩니다(트랜잭션별 일할 계산). RRS API 프로덕션 API 계산 시간 비용은 각 API 호출 실행에 필요한 시간에 총 API 트랜잭션 수를 곱하고 프로덕션 API 계산 시간당 가격을 곱하여 계산됩니다. 예를 들어 표준 S1 초과분의 경우 한 번 실행하는 데 0.72초가 걸리는 API 트랜잭션이 1,000,000개 있다면 프로덕션 API 트랜잭션 비용은 $500(1,000,000 * $0.50/1K API 트랜잭션)이고 프로덕션 API 계산 시간은 $400(1,000,000 * 0.72초 * $2/시간)으로 총 $900의 금액이 산정됩니다.

BES 워크로드도 같은 방식으로 청구되지만 API 트랜잭션 비용은 제출하신 배치 작업 수를, 계산 비용은 해당 일괄 작업과 연관된 계산 시간을 나타냅니다. 따라서 BES 프로덕션 API 트랜잭션 비용은 제출하신 총 작업 수에 트랜잭션 1,000개당 가격을 곱하여 계산됩니다(트랜잭션별 일할 계산). BES API 프로덕션 API 계산 시간 비용은 작업의 각 행 실행에 필요한 시간에 작업의 총 행 수, 총 작업 수, 프로덕션 API 계산 시간당 가격을 곱하여 계산됩니다. 기계 학습 계산기를 사용할 때 트랜잭션 미터는 제출하려는 작업 수를 , 트랜잭션당 시간 필드는 각 작업의 모든 행을 실행하는 데 필요한 총 시간을 나타냅니다. 예를 들어 표준 S1 초과분을 사용하여 각 0.72초가 필요한 행 500개로 구성된 작업을 하루에 100개 제출할 경우 월간 초과 비용은 프로덕션 API 트랜잭션 비용 $1.55(하루 100개 작업 = 3,100개 작업/1개월 * $0.50/1K API 트랜잭션)이며 프로덕션 API 계산 시간은 $620(500행 * 0.72초 * 3,100개 작업 * $2/시간)으로 총 $621.55의 금액이 산정됩니다.
...
Azure 기계 학습 무료 계층은 Azure 기계 학습 스튜디오를 자세히 소개하기 위한 것입니다. Microsoft 계정을 등록하기만 하면 됩니다. 무료 계층에는 [Microsoft 계정](https://www.microsoft.com/account/default.aspx)당 하나의 Azure 기계 학습 스튜디오 작업 영역에 대한 무료 액세스 권한이 포함됩니다. 최대 10GB의 저장소를 사용하고 스테이징 API로써 모델을 운영하는 기능이 포함됩니다. 무료 계층 작업은 SLA에 포함되지 않으며 개발 및 개인 용도로만 사용할 수 있습니다. 무료 계층 작업은 온-프레미스 SQL Server에 연결하여 데이터에 액세스할 수 없습니다. 위의 테이블에는 무료 계층과 표준 계층 사이의 여러 차이점이 개요되어 있지만, 그 외 차이점이 존재할 수 있으며 언제든 변경될 수 있습니다.

**Azure 기계 학습 표준 계층 및 계획에는 무엇이 포함됩니까?**

Azure 기계 학습 표준 계층은 Azure 기계 학습 스튜디오의 유료 프로덕션 버전입니다. Azure 기계 학습 서비스 스튜디오 월별 요금은 매달 사용자를 기준으로 청구되고 1개월 중 일부는 일할 계산됩니다. Azure 기계 학습 스튜디오 실험 시간은 실제 실험의 계산 시간을 기준으로 요금이 청구됩니다. 요금은 부분 시간별로 일할 계산됩니다.

Azure 기계 학습 API 서비스는 기존 웹 서비스 또는 새로운 웹 서비스인지에 따라 요금이 청구됩니다.

구독에 대한 작업 영역당 아래 요금이 집계됩니다.

* 기계 학습 사용자 구독 - 기계 학습 사용자 구독은 기계 학습 스튜디오 작업 영역에 대한 액세스를 제공하는 월별 요금이고, 스튜디오에서 실행되며 프로덕션 API를 활용하는 실험에 필요합니다.
...
* 기존 웹 서비스의 경우:
	...
	* 프로덕션 API 트랜잭션(1000초 내) - 이 미터에는 프로덕션 웹 서비스 호출당 발생하는 요금이 포함됩니다.

새 웹 서비스의 경우 위의 요금 외에도 선택한 계획에 대한 요금이 집계됩니다.

* 표준 S1/S2/S3 API 계획(단위) - 이 미터는 새 웹 서비스에 대해 선택한 인스턴스의 유형을 나타냅니다.
* 표준 S1/S2/S3 초과 API 계산 시간 - 이 미터는 기존 인스턴스에 포함된 수량을 모두 사용한 후에 프로덕션에서 실행되는 새 웹 서비스에서 발생한 계산 요금을 포함합니다.. 추가 사용량은 S1/S2/S3 계획 계층과 관련된 초과 요금이 부과됩니다.
* 표준 S1/S2/S3 초과 API 트랜잭션(1,000초 내) - 이 미터는 기존 인스턴스에 포함된 수량을 모두 사용한 후에 새 프로덕션 웹 서비스에 대한 호출당 발생하는 요금을 포함합니다. 추가 사용량은 S1/S2/S3 계획 계층과 관련된 초과 요금이 부과됩니다.
...
Microsoft 계정만 있으면 됩니다. [Azure 기계 학습 홈](https://azure.microsoft.com/services/machine-learning/)으로 이동하고 지금 시작 단추를 클릭하세요. Microsoft 계정으로 로그인하면 무료 계층에 작업 영역이 생성됩니다. 기계 학습 실험을 바로 탐색하고 만들 수 있습니다.

**Azure 기계 학습 표준 계층에 등록하려면 어떻게 하나요?**

표준 기계 학습 작업 영역을 만들려면 먼저 Azure 구독에 대한 액세스 권한이 있어야 합니다. 30일 무료 평가판 Azure 구독에 등록하고 나중에 유료 Azure 구독으로 업그레이드하거나 유료 Azure 구독을 바로 구입할 수 있습니다. 그런 다음 구독에 대한 액세스 권한을 받은 후 Microsoft Azure 클래식 포털에서 기계 학습 작업 영역을 만들 수 있습니다. [단계별 지침](https://azure.microsoft.com/trial/get-started-machine-learning-b/)을 참조하세요.
...
예, 무료 계층의 일부로써 스테이징 API 서비스에 기계 학습 모델을 운영할 수 있습니다. 스테이징 API 서비스를 프로덕션에 넣고 운영 서비스에 대한 프로덕션 끝점을 가져오려면 표준 계층을 사용해야 합니다.

**Azure 무료 평가판과 Azure 기계 학습 무료 계층의 차이점은 무엇인가요?**

[Microsoft Azure 무료 평가판](https://azure.microsoft.com/free/)은 Azure 기계 학습 무료 계층에서 프로덕션이 아닌 작업에 대한 Azure 기계 학습 서비스에 지속적인 액세스 권한을 제공하는 중 한 달 동안 Azure 서비스에 적용할 수 있는 크레딧을 제공합니다.

**무료 계층에서 표준 계층으로 실험을 이동하려면 어떻게 하나요?**

무료 계층에서 표준 계층으로 실험을 복사하려면 아래 설명된 단계를 따르세요.
...
5.	연결된 데이터 집합, 학습한 모델 등이 모두 실험과 함께 표준 작업 영역에 복사됩니다.
6.	실험을 다시 실행하고 표준 작업 영역에 웹 서비스를 다시 게시해야 합니다.

### 스튜디오 작업 영역

**기계 학습 사용자 구독은 무엇이고 언제 필요한가요?**

기계 학습 사용자는 작업 영역을 나타냅니다. 기계 학습 스튜디오 또는 프로덕션 API 서비스에서 실험을 수행 중인 모든 사용자는 기계 학습 사용자 구독에서 다루는 것이 좋습니다.

**작업 영역마다 별도의 청구 내역을 볼 수 있습니까?**

단일 청구서에서 작업 영역 요금은 해당하는 각 미터에 대해 별도로 계산됩니다.
...
게스트 액세스는 Azure 기계 학습 스튜디오에서 인증 없이 무료로 실험을 만들어 실행할 수 있는 제한적인 체험 서비스입니다. 게스트 세션은 저장할 수 없으며 임시로 유지되고, 세션 시간은 8시간으로 제한됩니다. 또한 R 및 Python이 지원되지 않고 스테이징 API가 없으며 데이터 집합 크기와 저장소 용량도 제한됩니다. 하지만 Microsoft 계정으로 로그인할 경우 저장이 가능한 작업 영역과 보다 포괄적인 기능을 포함하여 위에 설명된 기계 학습 스튜디오의 무료 기능에 모두 액세스할 수 있습니다. [https://studio.azureml.net](https://studio.azureml.net)에서 "시작하기" 버튼을 클릭하고 게스트 액세스 또는 Microsoft 계정으로 로그인을 선택하여 무료 기계 학습 체험 방식을 선택하세요.
