<properties
	pageTitle="기계 학습 스튜디오에서 간단한 실험 | Microsoft Azure"
...
2. [데이터 집합에서 열 선택][select-columns] 모듈을 선택하고 **속성** 창에서 **열 선택기 시작**을 클릭합니다.
	...
	**데이터 집합에서 열 선택**의 속성 창은 **정규화된 손실**을 제외한 데이터 집합의 모든 열이 전달된다는 것을 나타냅니다.
    ...
    > [AZURE.TIP] 모듈을 두 번 클릭하고 텍스트를 입력하여 모듈에 주석을 추가할 수 있습니다. 그러면 모듈이 실험에서 수행하는 내용을 한눈에 볼 수 있습니다. 이 경우 [데이터 집합에서 열 선택][select-columns] 모듈을 두 번 클릭하고 "정규화된 손실 제외"라는 주석을 입력합니다.
...
4. **다음으로 시작**에서 **열 없음**을 클릭한 후 필터 행에서 **포함** 및 **열 이름**을 선택합니다. 열 이름 목록을 입력합니다. 그러면 지정한 열만 전달하는 모듈로 리디렉션됩니다.
	...
	> [AZURE.TIP] 실험을 실행하여 데이터에 대한 열 정의가 데이터 집합에서 [누락된 데이터 정리][clean-missing-data] 모듈을 통해 전달되었습니다. 즉, 연결한 다른 모듈에도 데이터 집합의 정보가 있습니다.
