...
입력 포트와 함수 매개 변수 사이의 매핑은 위치와 관련됩니다. 즉, 첫 번째로 연결된 입력 포트는 함수의 첫 번째 매개 변수에 매핑되고, 두 번째 입력(연결된 경우)은 함수의 두 번째 매개 변수에 매핑됩니다.
...
다음으로 Hello.py가 포함된 Hello.zip이라는 파일을 만들 수 있습니다.

![image7](./media/machine-learning-execute-python-scripts/figure5.png)

그림 5. 사용자 정의 Python 코드가 포함된 Zip 파일.

그런 다음 이 파일을 데이터 집합으로 Azure 기계 학습 스튜디오에 업로드합니다. 그런 다음 단순한 실험을 생성하고 실행하려면 다음 모듈을 사용합니다.
...
그림 9에서는 위에 표시된 스크립트를 사용하여 두 번째 출력 포트를 통해 그림을 반환하는 실험을 보여줍니다.
...
여러 그림을 서로 다른 이미지에 저장하여 반환하고, Azure 기계 학습 런타임 시 모든 이미지를 선택한 다음, 시각화를 위해 연결할 수 있습니다.


## 고급 예제
Azure 기계 학습에 설치된 Anaconda 환경에는 NumPy, SciPy 및 Scikits-Learn과 같은 공통 패키지가 들어 있으며, 일반 기계 학습 파이프라인의 다양한 데이터 처리 작업에 효율적으로 사용할 수 있습니다. 예를 들어, 다음 실험과 스크립트에서는 데이터 집합의 기능 중요도 점수를 계산하기 위해 Scikits-Learn의 앙상블 학습자 사용에 대해 설명합니다. 점수는 다른 기계 학습 모델에 피드하기 전에 감독된 기능 선택을 수행하는 데 사용할 수 있습니다.

중요도 점수를 계산한 다음 이 점수에 따라 기능의 순서를 지정하는 Python 함수가 아래 표시됩니다.

![image11](./media/machine-learning-execute-python-scripts/figure8.png)

그림 10. 점수별로 기능의 순위를 지정하는 기능. 다음 실험에서는 Azure 기계 학습의 “Pima Indian Diabetes” 데이터 집합에 있는 기능의 중요도 점수를 계산하여 반환합니다.
...
2.	*정교한 개발 및 디버깅 지원이 부족합니다.* 현재 Python 모듈에서는 Intellisense와 디버깅 같은 IDE 기능을 지원하지 않습니다. 또한 런타임 시 모듈이 실패하는 경우, 전체 Python 스택 추적을 사용할 수 있지만 모듈의 출력 로그에서 확인해야 합니다. 현재로서는 사용자가 IPython과 같은 환경에서 Python 스크립트를 개발하고 디버그한 다음 모듈에 코드를 가져오는 것이 좋습니다.
