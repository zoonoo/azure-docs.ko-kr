...
4. 코딩 스타일에 익숙해지기 위해 Python 스크립트 코드를 검토합니다. 스크립트는 다음 작업을 수행합니다.
   ...
   - [pickle](https://docs.python.org/3/library/pickle.html) 라이브러리를 사용하여 모델을 `outputs` 폴더의 파일에 직렬화합니다. 그런 다음 스크립트는 로드하고 메모리로 다시 역직렬화합니다.

   - 역직렬화된 모델을 사용하여 새 레코드에 대해 예측을 수행합니다. 

   - [matplotlib](https://matplotlib.org/) 라이브러리를 사용하여 두 개의 그래프, 혼동 행렬 및 다중 클래스 ROC(수신기 조작 특성) 곡선을 그린 다음, `outputs` 폴더에 저장합니다.

   - `run_logger` 개체는 정규화 속도 및 모델 정확도를 처음부터 끝까지 로그에 기록하는 데 사용됩니다. 이러한 값은 실행 기록에 자동으로 그려집니다.
...
4. **iris_sklearn.py** 탭 위쪽의 도구 모음에서 **로컬**을 실행 환경으로 선택하고, `iris_sklearn.py`를 실행할 스크립트로 선택합니다.

5. 다음으로 도구 모음의 오른쪽으로 이동하고 **인수** 필드에 `0.01`을 입력합니다. 이 값은 로지스틱 회귀 모델의 정규화 속도에 해당합니다.
...
8. **작업** 창의 작업 상태 텍스트에서 **완료됨**을 선택합니다. 팝업 창이 열리고 실행에 대한 표준 출력(stdout) 텍스트가 표시됩니다. 표준 출력 텍스트를 닫으려면 팝업 의 오른쪽 위에 있는 **닫기**(**x**) 단추를 선택합니다.
...
9. **작업** 창의 동일한 작업 상태에서 **완료됨** 상태 및 시작 시간 바로 위의 파란색 **iris_sklearn.py [n]**(_n_은 실행 번호) 텍스트를 선택합니다. **실행 속성** 창이 열리고 해당 특정 실행에 대한 다음 정보를 표시합니다.
   ...
   >[!NOTE]
   >이전에 설정한 학습에 무작위 추출(randomization)을 사용했으므로 실제 결과는 여기에 표시된 결과와 다를 수 있습니다.
...
Docker와 같은 추가 실행 환경을 쉽게 구성하고 이러한 환경에서 스크립트를 실행할 수 있습니다. 

>[!IMPORTANT]
>이 단계를 수행하려면 로컬에 설치되어 시작되는 Docker 엔진이 있어야 합니다. 자세한 내용은 [Docker 설치 지침](https://docs.docker.com/install/)을 참조하세요.
...
4. 새 작업이 시작되는지 확인합니다. Workbench 창의 오른쪽에 있는 **작업** 창에 나타납니다.
   ...
   Docker에 대해 처음 실행하는 경우 작업을 완료하는 데 몇 분 정도 더 걸립니다. 
...
5. 이제 Spark를 사용해 보겠습니다. Docker 기본 이미지에는 미리 설치되고 구성된 Spark 인스턴스가 포함되어 있으며, 이 인스턴스는 PySpark 스크립트를 실행하는 데 사용할 수 있습니다. 이는 Spark를 직접 설치하고 구성하는 데 시간을 소모하지 않고도 Spark 프로그램을 개발하고 테스트할 수 있는 쉬운 방법입니다. 
...
6. Workbench에서 왼쪽 창의 **폴더** 아이콘을 선택하여 프로젝트 파일을 나열하고 **run.py**라는 Python 스크립트를 엽니다. 
   ...
   명령줄에서 **run.py** 스크립트를 실행하려면 다음 명령을 실행합니다.
...
1. VM이 만들어지면 `.runconfig` 및 `.compute` 파일 쌍을 생성하여 VM을 실행 환경으로 연결할 수 있습니다. 다음 명령을 사용하여 파일을 생성합니다. 새 환경 이름을 `myvm`으로 지정하겠습니다.
...
3. 이번에는 대상 _myvm_을 사용하여 CLI 창에서 이전에 수행한 것과 동일한 명령을 실행합니다.
   ```azurecli
...
4. 컨테이너에서 Spark를 사용해 보겠습니다. 파일 탐색기를 엽니다. 기본 파일 조작 명령에 익숙한 경우 CLI 창에서 이 작업을 수행할 수도 있습니다. `myvm.runconfig` 파일의 복사본을 만들고 이름을 `myvm-spark.runconfig`로 지정합니다. 새 파일을 편집하여 `Framework` 설정을 `Python`에서 `PySpark`로 변경합니다.
   ```yaml
   Framework: PySpark
   ```
...
1. Azure HDInsight용 Spark 클러스터에 액세스할 수 있으면 다음과 같이 HDInsight 실행 구성 명령을 생성합니다. HDInsight 클러스터 이름, HDInsight 사용자 이름 및 암호를 매개 변수로 제공합니다. 다음 명령을 사용합니다.
   ...
   >[!NOTE]
   >`username`은 클러스터 SSH 사용자 이름입니다. HDInsight 설치 중에 변경하지 않은 경우 기본값은 `sshuser`입니다. 값은 설치 중에 만들어져 클러스터의 관리 웹 사이트에 액세스할 수 있도록 설정되는 다른 사용자인 `admin`이 아닙니다. 

2. 다음 명령을 실행하여 HDInsight 클러스터에서 **iris_spark.py** 스크립트를 실행합니다.
