...
description: "이 문서에서는 Azure ML 데이터 준비에 대한 서로 다른 런타임 및 데이터 원본의 지원되는 조합의 전체 목록을 제공합니다."
...
|     |로컬 파일  |Azure BLOB  |SQL Server 데이터베이스***  |
|---------|---------|---------|---------|---------|
...
*로컬 파일 경로를 사용하는 경우 프로젝트 내의 파일이 계산 환경으로 복사되어 읽혀집니다. 프로젝트 외부 파일은 복사되지 않고 경로가 더 이상 계산 환경에서 확인되지 않습니다. 로컬로 실행할 때 코드에서 로컬 파일을 사용한 후 다른 실행 구성을 위해 Azure Storage Blob으로 전환할 수 있도록 데이터 원본 대체를 사용하는 것이 좋습니다. 또한 데이터 원본에 대한 샘플링 지원을 사용하여 특정 실행 구성에서만 큰 데이터에 대한 실행을 관리할 수 있습니다.
