...
[Azure Machine Learning](https://ml.azure.com/) 데이터 레이블 지정에서는 레이블 지정 프로젝트를 만들고, 관리하고, 모니터링할 수 있습니다. 이를 통해 데이터, 레이블 및 팀 멤버를 조정하여 레이블 지정 작업을 효율적으로 관리할 수 있습니다. Machine Learning은 이미지 분류(다중 레이블 또는 다중 클래스) 및 경계 상자를 통한 개체 식별을 지원합니다.

데이터 레이블 지정은 진행 상황을 추적하고 완료되지 않은 레이블 지정 작업 큐를 유지 관리합니다.

프로젝트를 시작 및 중지하고 레이블 지정 진행률을 제어할 수 있습니다. 레이블이 지정된 데이터를 검토하고 레이블이 지정된 데이터를 COCO 형식 또는 Azure Machine Learning 데이터 세트로 내보낼 수 있습니다.
