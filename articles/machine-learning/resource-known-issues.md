...
* **Pip 설치: Dependecies 단일 줄 설치와 일치 하지**않을 수 있습니다 .이는 한 줄로 설치할 때 작동 하는 종속성 해결 프로그램이 없기 때문에 Pip의 알려진 제한 사항입니다. 첫 번째 고유 종속성은 보이는 유일한 종속성입니다. 예를 들어 버전 > 1.0 및 azureml (버전)가 필요한 datadrift를 설치 하는 경우 버전 < 1.2이 필요 하 고 최신 버전이 1.3 인 경우, 사용자가 아래와 같이 한 줄에 패키지를 설치 하면 azureml-학습-automl 패키지에 이전 버전이 필요 하더라도 모든 항목이 1.3로 업그레이드 됩니다. 

    * 단일 줄 설치와 일치 하지 않는 dependecies이 표시 됩니다.
    ```python
       pip install azure-ml-datadrift, azureml-train-automl
     ```
   
    * 패키지에 대 한 적절 한 버전이 설치 되었는지 확인 하려면 다음 코드와 같이 여러 줄을 사용 하 여를 설치 합니다. 순서는 중요 하지 않습니다.
