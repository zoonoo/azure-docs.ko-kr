...
하지만 먼저 이 실험을 약간 간단히 줄여야 합니다. 현재 실험에는 두 개의 다른 모델이 있지만 게시할 모델을 하나만 선택해야 합니다.
...
실험에서 파생된 웹 서비스를 게시하려면 캔버스 아래에서 **PUBLISH WEB SERVICE**(웹 서비스 게시)를 클릭합니다. 기계 학습 스튜디오에서 실험을 웹 서비스로 게시하고 서비스 대시보드로 이동합니다.

> [AZURE.TIP]웹 서비스를 게시하고 나서 업데이트할 수 있습니다. 예를 들어 모델을 변경하려면 학습 실험을 편집하고, 모델 매개 변수를 조정하고, **PUBLISH WEB SERVICE**(웹 서비스 게시)를 클릭합니다. 실험을 다시 게시하면 이 실험이 웹 서비스를 대체하고 이제 업데이트된 모델을 사용합니다.
