...
description: Azure 기계 학습 스튜디오에서 응용 프로그램 수명 주기 관리 모범 사례 적용(클래식)
...
---
# <a name="application-lifecycle-management-in-azure-machine-learning-studio-classic"></a>Azure 기계 학습 스튜디오의 응용 프로그램 수명 주기 관리(클래식)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Azure 기계 학습 스튜디오(클래식)는 Azure 클라우드 플랫폼에서 작동하는 기계 학습 실험을 개발하기 위한 도구입니다. 단일 플랫폼으로 병합된 Visual Studio IDE 및 확장 가능한 서비스와 같습니다. 다양한 자산 버전 관리에서 자동화된 실행 및 배포에 이르는 표준 ALM(응용 프로그램 수명 주기 관리) 사례를 Azure 기계 학습 스튜디오(클래식)에 통합할 수 있습니다. 이 문서는 몇 가지 옵션과 접근 방법에 대해 다룹니다.
...
Azure 기계 학습 스튜디오(클래식) 학습 실험의 실행 모델에서 실험 편집기에서 **실행을** 클릭할 때마다 실험의 변경할 수 없는 스냅숏이 작업 스케줄러에 제출됩니다. 이 스냅샷 목록을 보려면 실험 편집기 보기의 명령 모음에서 **실행 기록**을 클릭합니다.
...
Azure 기계 학습 스튜디오(클래식)의 학습된 모델은 iLearner 파일()이라고`.iLearner`하는 형식으로 직렬화되며 작업 영역과 연결된 Azure Blob 저장소 계정에 저장됩니다. 재학습 API를 통해 iLearner 파일의 사본을 얻을 수 있습니다. [이 문서](/azure/machine-learning/studio/retrain-machine-learning-model)에서는 재학습 API의 작동 방법을 설명합니다. 대략적인 단계는 다음과 같습니다.
...
Azure 기계 학습 스튜디오(클래식) 실험에서 두 가지 유형의 웹 서비스를 배포할 수 있습니다. 클래식 웹 서비스는 작업 영역뿐만 아니라 실험과도 밀접하게 연결되어 있습니다. 새 웹 서비스는 Azure Resource Manager 프레임워크를 활용하고 더 이상 원래 실험 또는 작업 영역과 연결되지 않습니다.
...
새 Azure Resource Manager 기반 웹 서비스를 만들면 엔드포인트 구문은 더 이상 사용할 수 없습니다. 대신 [내보내기-AmlWebServiceDefinitionFrom](https://github.com/hning86/azuremlps#export-amlwebservicedefinitionfromexperiment) PowerShell 명령어를 사용하거나 배포된 리소스 관리자 기반 웹 서비스에서 [*내보내기-AzMlWebservice*](https://docs.microsoft.com/powershell/module/az.machinelearning/export-azmlwebservice) PowerShell 명령어를 사용하여 예측 실험에서 JSON 형식으로 WSD(웹 서비스 정의) 파일을 생성할 수 있습니다.
...
ALM의 중요한 부분은 애플리케이션의 실행 및 배포 프로세스를 자동화할 수 있다는 것입니다. Azure 기계 학습 스튜디오(클래식)에서는 [PowerShell 모듈을](https://aka.ms/amlps)사용하여 이 작업을 수행할 수 있습니다. 다음은 [Azure 기계 학습 스튜디오(클래식) PowerShell 모듈을](https://aka.ms/amlps)사용하여 표준 ALM 자동 실행/배포 프로세스와 관련된 종단 간 단계의 예입니다. 각 단계는 해당 단계를 수행하는 데 사용할 수 있는 하나 이상의 PowerShell commandlet에 연결됩니다.

1. [데이터 집합을 업로드합니다.](https://github.com/hning86/azuremlps#upload-amldataset)
...
4. [교육 실험을 실행합니다.](https://github.com/hning86/azuremlps#start-amlexperiment)
...
* Azure [기계 학습 스튜디오(클래식) PowerShell](https://aka.ms/amlps) 모듈을 다운로드하여 ALM 작업을 자동화하기 시작합니다.
