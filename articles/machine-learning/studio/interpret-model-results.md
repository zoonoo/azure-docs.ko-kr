...
이 항목에서는 Azure 기계 학습 스튜디오(클래식)에서 예측 결과를 시각화하고 해석하는 방법을 설명합니다. 모델을 학습시키고 모델에 대한 예측을 수행("모델 점수 매기기")한 후에는 예측 결과를 이해하고 해석해야 합니다.



Azure 기계 학습 스튜디오(클래식)에는 네 가지 주요 기계 학습 모델이 있습니다.
...
이 문서에서는 이러한 각 모듈의 예측 결과를 해석하는 방법에 대해 설명합니다. 이러한 모듈에 대한 개요는 [Azure 기계 학습 스튜디오(클래식)에서 알고리즘을 최적화하는 매개 변수를 선택하는 방법을 참조하세요.](algorithm-parameters-optimize.md)

이 항목에서는 예측 해석에 대해 다루지만 모델 평가는 다루지 않습니다. 모델을 평가하는 방법에 대한 자세한 내용은 [Azure 기계 학습 Studio(클래식)에서 모델 성능을 평가하는 방법을](evaluate-model-performance.md)참조하세요.

Azure 기계 학습 스튜디오(클래식)를 새로 접하고 간단한 실험을 시작하는 데 도움이 필요한 경우 [Azure 기계 학습 Studio(클래식)의 간단한 실험 만들기를](create-experiment.md)참조하십시오.
...
Azure 기계 학습 스튜디오(클래식)에는 이러한 각 분류 유형을 처리하는 모듈이 다르지만 예측 결과를 해석하는 방법은 비슷합니다.

### <a name="two-class-classification"></a>2클래스 분류
**예제 실험**

2클래스 분류 문제의 예로 붓꽃 분류를 들 수 있습니다. 이 작업은 특징에 따라 붓꽃을 분류하는 것입니다. Azure 기계 학습 스튜디오(클래식)에서 제공하는 아이리스 데이터 집합은 두 개의 꽃 종(클래스 0 및 1)의 인스턴스만 포함하는 인기 있는 [아이리스 데이터 집합의](https://en.wikipedia.org/wiki/Iris_flower_data_set) 하위 집합입니다. 각 꽃에는 네 가지 특징이 있습니다(꽃받침 길이, 꽃받침 너비, 꽃잎 길이 및 꽃잎 너비).
...
예측 결과를 철저히 파악하고 판단한 후에는 실험을 웹 서비스로 게시할 수 있습니다. 그러면 다양한 애플리케이션에 실험을 배포하고 호출하여 모든 새 붓꽃에 대한 클래스 예측을 얻을 수 있습니다. 교육 실험을 점수 매기기 실험으로 변경하고 웹 서비스로 게시하는 방법을 알아보려면 [자습서 3: 신용 위험 모델 배포 를](tutorial-part3-credit-risk-deploy.md)참조하십시오. 이 절차에 따르면 그림 3에 표시된 대로 점수 매기기 실험이 제공됩니다.
...
왼쪽에 있는 16개의 열이 테스트 집합의 기능 값을 나타냅니다. 클래스 “XX”의 점수가 매겨진 확률이라고 이름이 지정된 열은 2클래스 사례의 점수가 매겨진 확률 열과 같습니다. 해당 항목이 특정 클래스에 속할 확률을 보여줍니다. 예를 들어 첫 번째 항목의 경우 "A", 0.000451 확률이 "B"일 확률이 0.003571입니다. 마지막 열(점수가 매겨진 레이블)은 2클래스에서의 점수가 매겨진 레이블과 같습니다. 점수가 매겨진 확률이 가장 높은 클래스를 해당 항목의 예측 클래스로 선택합니다. 예를 들어 첫 번째 항목의 경우 "F"(0.916995)가 될 확률이 가장 클 수 있으므로 점수가 매겨진 레이블은 "F"입니다.
...
웹 서비스를 게시하고 실행한 다음 입력 특징 값을 입력하고 나면 그림 10과 같은 결과가 반환됩니다. 이 손으로 쓴 편지, 그것의 추출 된 16 기능, 0.9715 확률 "T"로 예상 된다.
...
아이리스 데이터 집합을 다시 사용하여 클러스터링 실험을 빌드해 보겠습니다. 여기에서는 특징만 보유하고 클러스터링에 사용할 수 있도록 데이터 집합의 클래스 레이블을 필터링할 수 있습니다. 이 붓꽃 사용 사례에서는 학습 프로세스 중에 클러스터의 수를 2로 지정합니다. 즉, 꽃을 2클래스로 클러스터링합니다. 실험은 그림 15에 표시됩니다.

![붓꽃 클러스터링 문제 실험](./media/interpret-model-results/15.png)

그림 15. 붓꽃 클러스터링 문제 실험

클러스터링은 학습 데이터 집합자체에 근거 자료 레이블이 없다는 점에서 분류와 다릅니다. 클러스터링은 학습 데이터 집합 인스턴스를 개별 클러스터로 그룹화합니다. 학습 프로세스 중에 모델에서 해당 특징 사이의 차이점을 학습하여 항목의 레이블을 지정합니다. 그런 다음 학습된 모델을 사용하여 나중에 항목을 분류할 수 있습니다. 클러스터링 문제에서 결과의 두 부분에 관심이 있습니다. 첫 번째 부분은 학습 데이터 집합의 레이블을 지정하는 것이고, 두 번째는 학습된 모델을 사용하여 새 데이터 집합을 분류하는 것입니다.
...
Azure 기계 학습 스튜디오(클래식)에서 [성냥갑 추천자 학습][train-matchbox-recommender] 모듈을 사용하면 몇 가지 작업을 수행할 수 있습니다.
...
권장 시스템에 대한 일반적인 Azure 기계 학습 스튜디오(클래식) 실험은 그림 20과 같습니다. 추천 시스템 모듈 사용 방법에 대한 자세한 내용은 [매치박스 추천 학습][train-matchbox-recommender] 및 [매치박스 추천 점수 매기기][score-matchbox-recommender]를 참조하세요.
...
**추천 예측 유형**에서 **관련 사용자**를 선택하여 추천 시스템에서 지정된 사용자와 관련된 사용자를 찾도록 요청합니다. 관련된 사용자는 기본 설정이 비슷한 사용자입니다. 이 시나리오에서 선택할 마지막 매개 변수는 *관련 사용자 선택*입니다. 모델 **평가의 경우 해당 항목을 정격화한 사용자의** 옵션은 주로 교육 프로세스 중 모델 평가를 위한 것입니다. 이 예측 단계에서는 **모든 사용자로부터**를 선택합니다. [매치박스 추천 점수 매기기][score-matchbox-recommender] 출력의 시각화는 그림 23과 비슷합니다.
