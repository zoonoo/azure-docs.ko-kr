...
1. [Azure 클래식 포털](http://manage.windowsazure.com)에 로그인한 후 왼쪽 열에서 **Machine Learning**을 클릭합니다. 관심 있는 웹 서비스가 포함되어 있는 작업 영역을 클릭합니다.
    ...
    ![작업 영역으로 이동](./media/create-endpoint/figure-1.png)
2. **웹 서비스**를 클릭합니다.
   
    ![웹 서비스로 이동](./media/create-endpoint/figure-2.png)
3. 사용 가능한 끝점 목록을 확인할 웹 서비스를 클릭합니다.
   
    ![끝점으로 이동](./media/create-endpoint/figure-3.png)
4. 페이지 맨 아래에 있는 **끝점 추가**를 클릭합니다. 이름 및 설명을 입력하고 이 웹 서비스에 같은 이름의 다른 끝점이 없는지 확인합니다. 특별한 요구 사항이 없는 경우 제한 수준을 해당 기본값으로 그대로 둡니다. 제한에 대해 자세히 알아보려면 [API 끝점 확장](scaling-webservice.md)을 참조하세요.
   
    ![끝점 만들기](./media/create-endpoint/figure-4.png)
