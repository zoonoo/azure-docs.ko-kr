...
**Microsoft Azure Machine Learning Studio 알고리즘 치트 시트**를 사용하면 알고리즘의 Azure Machine Learning Studio 라이브러리에서 예측 분석 솔루션에 맞는 기계 학습 알고리즘을 선택할 수 있습니다.
...
이 치트 시트는 Azure Machine Learning Studio에서 시작할 알고리즘을 선택하려는 재학생 수준의 기계 학습 지식을 지닌 초급 데이터 과학자라는 매우 구체적인 대상을 염두에 두고 작성되었습니다. 따라서 일반화되고 지나치게 단순화되었을 수 있으나 안전한 방향으로 사용자를 안내합니다. 여기에 나와 있는 것 외에도 수많은 알고리즘이 있습니다. Azure Machine Learning Studio가 사용 가능한 방법을 더 포함하기 위해 확장됨에 따라 알고리즘을 더 추가할 예정입니다.
...
SVM(Support Vector Machine)은 가능한 넓은 여백으로 클래스를 구분하는 경계를 찾습니다. 두 클래스를 확실하게 구분할 수 없는 경우 알고리즘이 가능한 최적의 경계를 찾습니다. Azure Machine Learning Studio에서 작성될 때 [2클래스 SVM](https://msdn.microsoft.com/library/azure/dn905835.aspx)은 직선에서만 이 작업을 수행합니다. (SVM-speak에서 선형 커널을 사용합니다.) 이 선형 근사치를 생성하므로 매우 신속하게 실행할 수 있습니다. 텍스트 또는 유전자처럼 기능 중심의 데이터에서 특히 유용합니다. 이러한 경우 SVM은 적절한 수준의 메모리를 필요로 하면서도 대부분의 다른 알고리즘보다 과잉 맞춤을 피하면서 클래스를 보다 신속하게 구분할 수 있습니다.
...
Bayesian 메서드는 과잉 맞춤을 방지하는 매우 뛰어난 품질을 자랑합니다. 대답에 대한 가능한 분포에 대해 사전에 몇 가지 가정을 하여 이 작업을 수행합니다. 이 방법에 대한 다른 부산물은 매우 적은 매개 변수입니다. Azure Machine Learning Studio는 분류([2클래스 Bayes 지점 머신](https://msdn.microsoft.com/library/azure/dn905930.aspx)) 및 회귀([베이지언 선형 회귀](https://msdn.microsoft.com/library/azure/dn906022.aspx)) 둘 다에 대한 베이지언 알고리즘을 모두 포함합니다.
...
- 클러스터링([K-평균](https://msdn.microsoft.com/library/azure/5049a09b-bd90-4c4e-9b46-7c87e3a36810/))
...
Azure Machine Learning Studio에서는 [Vowpal Wabbit](https://msdn.microsoft.com/library/azure/8383eb49-c0a3-45db-95c8-eb56a1fef5bf)라는 강력한 기계 학습 프레임워크에도 액세스할 수 있습니다.
