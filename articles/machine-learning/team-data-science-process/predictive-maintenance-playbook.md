...
- 다음에서 비즈니스는 _예방 유지 관리_를 사용합니다. 여기서는 부품에 유용한 수명을 결정하고 오류가 발생하기 전에 해당 부품을 유지 관리하거나 교체합니다. 예방 유지 관리는 예약되지 않은 치명적인 오류를 방지합니다. 그러나 예정된 가동 중지 시간, 유용한 수명 동안 구성 요소의 사용률이 낮으며 노동력은 여전히 남아 있습니다.
...
|**에너지** |                          |
|_풍력 터빈 고장_: 풍력 터빈은 환경 적으로 책임있는 국가 / 지역의 주요 에너지 원이며 높은 자본 비용을 수반합니다. 풍력 터빈의 핵심 구성 요소는 발전기 모터로, 고장으로 인해 터빈이 비효율적입니다. 문제를 해결하는 데 비용도 많이 듭니다.|MTTF(오류 발생 시)와 같은 KPI를 예측하면 에너지 회사에서 터빈 오류를 방지하고 최소한의 가동 중지 시간을 확보할 수 있습니다. 오류 확률은 곧 실패할 수 있는 터빈을 모니터링하도록 기술 지원 담당자에게 알리고, 시간 기반 유지 관리 체계를 예약합니다. 예측 모델은 오류를 발생시키는 여러 요인에 대한 정보를 제공하며 이를 통해 기술 지원 담당자가 문제의 근본 원인을 더 잘 이해할 수 있습니다.|
...
자산의 유지 관리 기록에는 교체된 구성 요소, 수행된 수리 활동 등에 대한 세부 정보가 포함되어 있습니다. 이러한 이벤트는 성능 저하 패턴을 기록합니다. 학습 데이터에서 중요한 정보가 없는 경우 잘못된 모델 결과가 발생할 수 있습니다. 또한 오류 기록은 특수한 오류 번호 또는 부품의 주문 날짜와 같은 유지 관리 기록 내에서 찾을 수 있습니다. 오류 패턴에 영향을 주는 추가 데이터 원본은 도메인 전문가에 의해 조사되고 제공되어야 합니다.

#### <a name="machine-operating-conditions"></a>기계 작동 조건
작업에서 장비의 센서 기반(또는 기타) 스트리밍 데이터는 중요한 데이터 원본입니다. PdM의 주요 가정은 일상적인 작업 중에 시간이 지남에 따라 컴퓨터의 상태가 저하된다는 것입니다. 데이터에는 성능 저하를 일으키는 이상 상태 및 에이징 패턴을 캡처하는 시간에 따라 변화하는 기능이 포함됩니다. 시간이 지남에 따라 오류 및 오류가 아닌 패턴을 알아보기 위해 데이터의 temporal 측면이 알고리즘에 필요합니다. 이러한 데이터 요소에 따라 알고리즘은 기계가 실패하기 전에 계속 작동할 수 있는 시간 단위를 예측하는 방법을 알아냅니다.
...
| 사용 사례 | 관련 데이터의 예제 |
|:---------|---------------------------|
|_비행 지연 및 취소_ | 비행 지연 및 페이지 로그의 형식인 비행 경로 정보입니다. 비행 다리 데이터에는 출발/도착 날짜, 시간, 공항, 경유 등과 같은 라우팅 세부 정보가 포함됩니다. 페이지 로그에는 지상 유지 관리 담당자가 기록한 일련의 오류 및 유지 관리 코드가 포함됩니다.|
|_비행기 엔진 부품 오류_ | 다양한 부품의 상태에 대한 정보를 제공하는 비행기의 센서에서 수집된 데이터입니다. 유지 관리 레코드를 통해 구성 요소 오류가 발생한 시기 및 교체된 시기를 식별할 수 있습니다.|
|_ATM 오류_ | 각 트랜잭션(현금/수표 예금) 및 현금 인출에 대한 센서 판독값입니다. 노트 사이의 간격 측정에 대한 정보, 노트 두께, 노트 도착 거리, 확인 속성 등 오류 코드, 수리 정보를 제공하는 유지 보수 기록, 현금 디스펜서가 리필 된 마지막 시간.|
|_풍력 터빈 오류_ | 센서는 온도, 풍향, 발생 전력, 발전기 속도 등과 같은 터빈 조건을 모니터링합니다. 데이터는 다양한 지역에 위치한 풍력 발전 단지의 여러 풍력 터빈에서 수집됩니다. 일반적으로 각 터빈은 여러 센서가 고정된 시간 간격으로 릴레이 측정값을 읽도록 합니다.|
|_회로 차단기 오류_ | 수정, 예방 및 체계적 작업을 포함하는 유지 관리 로그입니다. 열기 및 닫기 작업 등 회로 차단기에 전송된 자동 및 수동 명령이 포함된 운영 데이터입니다. 제조 날짜, 위치, 모델 등과 같은 장치 메타데이터 전압 레벨, 지리적 위치, 주변 조건과 같은 회로 차단기 사양.|
|_엘리베이터 도어 오류_| 엘리베이터 종류, 제조 날짜, 유지 관리 빈도, 건물 종류 등 엘리베이터 메타데이터입니다. 도어 주기의 수, 평균 도어 닫기 시간 등 운영 정보입니다. 원인을 포함한 오류 기록입니다.|
|_휠 오류_ | 휠 가속, 제동 인스턴스, 주행 거리, 속도 등을 측정하는 센서 데이터 제조 업체, 제조 날짜 와 같은 바퀴에 정적 정보. 주문 날짜 및 수량을 추적하는 부품 주문 데이터베이스에서 유추된 오류 데이터입니다.|
...
기능 엔지니어링은 데이터를 모델링하기 이전의 첫 번째 단계입니다. 데이터 과학 프로세스 [의 해당 역할은 여기](https://docs.microsoft.com/azure/machine-learning/team-data-science-process/create-features)에서 설명합니다. _기능_ 온도, 압력, 진동 등 모델에 대한 예측 특성입니다. PdM의 경우 기능 엔지니어링에는 상당한 기간 동안 수집된 기록 데이터에 대해 컴퓨터의 상태를 추상화하는 작업이 포함됩니다. 이런 의미에서 원격 모니터링, 변칙 검색 및 오류 감지와 같은 다른 기능과 다릅니다. 
...
- _항공기 엔진 부품 고장_: 롤링 수단, 표준 편차 및 지난 날, 주 등 합계 이 메트릭은 비즈니스 도메인 전문가와 함께 결정해야 합니다.
...
자산의 레이블이 지정된 각 레코드에 대해 _크기<sub>W-k의</sub> _ 창이 정의되고 _k는_ _크기 W의_창 수입니다. 집계는 다음 _k_ _텀블링 창_ _W-k,<sub>W-K(k-1)</sub>, ...,<sub>W-2,</sub><sub>W-1</sub> _ 레코드의 타임스탬프 전에 마침표에 대 한 생성 됩니다. _k_는 단기 효과를 캡처할 만큼 작은 수나 장기 성능 저하 패턴을 캡처할 만큼 큰 수일 수 있습니다. (그림 2 참조).
...
> - Dong, G. Liu, H. (편집자), 기계 학습 및 데이터 분석을위한 기능 엔지니어링 (채프먼 & 홀 / CRC 데이터 마이닝 및 지식 검색 시리즈), CRC Press, 2018.
...
이진 분류는 미래의 _수평선 기간 X라고_하는 _미래 기간 내에 장비 조각이 실패할 확률을 예측하는_ 데 사용됩니다. X는 도메인 전문가와 상의하여 비즈니스 문제와 당면한 데이터에 의해 결정됩니다. 예:
...
- _항공편 지연_: X는 다음 24시간 동안지연을 예측하기 위해 하루로 선택될 수 있습니다. 그런 다음, 오류가 발생하기 전 24시간 이내의 모든 비행은 1로 레이블 지정됩니다.
...
![그림 5. 다중 클래스 분류에 대한 실패 시간 예측 레이블](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
권장된 방법은 예제를 학습 및 유효성 검사 집합으로 _시간 종속_ 방식으로 분할하는 것입니다. 이 경우에 모든 유효성 검사 예제가 모든 학습 예제보다 시간상 뒤에 있게 됩니다. 하이퍼 매개 변수 값의 각 집합에서 학습 데이터 집합에 대한 알고리즘을 학습합니다. 동일한 유효성 검사 집합을 통해 모델의 성능을 측정합니다. 최고의 성능을 보여주는 하이퍼 매개 변수 값을 선택합니다. 학습/유효성 검사 분할에 의해 선택된 하이퍼 매개 변수 값은 교차 유효성 검사에 의해 임의 선택된 값보다 향후 모델 성능이 더 우수합니다.
...
다양한 센서에서 얻은 측정값과 같은 타임스탬프가 지정된 이벤트 스트림을 가정합니다. 여러 이벤트를 포함하는 시간 프레임에 걸쳐 학습 및 테스트 예제의 기능 및 레이블을 정의합니다. 예를 들어 이진 분류의 경우 과거 이벤트를 기반으로 피쳐를 만들고 향후 "X" 시간 단위 내에서 향후 이벤트를 기반으로 레이블을 [만듭니다(피처 엔지니어링](#feature-engineering) 및 모델링 기술 섹션 참조). 따라서 예제의 레이블 지정 시간 프레임은 해당 기능의 시간 프레임보다 이후에 나타납니다.
...
- [정확도는](https://en.wikipedia.org/wiki/Accuracy_and_precision) 분류자의 성능을 설명하는 데 사용되는 가장 인기 있는 메트릭입니다. 하지만 정확도는 데이터 배포에 민감하고 불균형 데이터 집합을 포함한 시나리오에 대해 비효율적인 측정값입니다. 대신 기타 메트릭이 사용됩니다. [혼동 행렬](https://en.wikipedia.org/wiki/Confusion_matrix)과 같은 도구는 모델의 정확도를 계산하고 증명하는 데 사용됩니다.
...
| # | 제목 | 설명 |
|--:|:------|-------------|
| 2 | [Azure 예측 유지 관리 솔루션 템플릿](https://github.com/Azure/AI-PredictiveMaintenance) | Azure ML 모델링을 보여 주는 오픈 소스 솔루션 템플릿 및 IoT 원격 모니터링 컨텍스트에서 예측 유지 관리 시나리오를 지원할 수 있는 전체 Azure 인프라입니다. |
...
| 6 | [Azure AI Toolkit for IoT Edge](https://github.com/Azure/ai-toolkit-iot-edge) | 텐서플로우를 이용한 IoT 에지의 AI; 툴킷은 Azure IoT Edge 호환 Docker 컨테이너에서 딥 러닝 모델을 패키지화하고 이러한 모델을 REST API로 노출합니다.
...
| 학습 리소스  | 가용성 |
|:-------------------|--------------|
| [트리 및 임의 포리스트를 사용한 PdM에 대한 학습 경로](https://aischool.microsoft.com/learning-paths/1H5vH5wAYcAy88CoQWQcA8) | Public | 
| [심층 학습을 사용한 PdM에 대한 학습 경로](https://aischool.microsoft.com/learning-paths/FSIXxYkOGcauo0eUO8qAS) | Public |
