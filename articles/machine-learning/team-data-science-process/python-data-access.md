...
* 실험에서 중간 데이터 세트에 액세스
* Python 클라이언트 라이브러리를 사용하여 데이터 집합을 확장하고, 메타데이터에 액세스하고, 데이터 집합의 내용을 읽고, 새 데이터 집합을 만들고, 기존 데이터 집합을 업데이트합니다.
...
Azure 기계 학습 파이썬 클라이언트 라이브러리를 설치하여 이 항목에 설명된 작업을 완료합니다. 이 라이브러리는 [파이썬 패키지 인덱스에서](https://pypi.python.org/pypi/azureml)사용할 수 있습니다. Python 환경에 설치하려면 로컬 Python 환경에서 다음 명령을 실행합니다.

    pip install azureml

또는 [GitHub](https://github.com/Azure/Azure-MachineLearning-ClientLibrary-Python)의 소스에서 다운로드하여 설치할 수 있습니다.
...
## <a name="use-code-snippets-to-access-datasets"></a><a name="datasetAccess"></a>코드 조각을 사용하여 데이터 집합에 액세스
Python 클라이언트 라이브러리를 사용하면 실행된 기존 데이터 세트에 프로그래밍 방식으로 액세스할 수 있습니다.

Azure 기계 학습 스튜디오(클래식) 웹 인터페이스에서 데이터 집합을 다운로드하고 로컬 컴퓨터에서 Pandas DataFrame 개체로 다운로드하고 역직해제하는 데 필요한 모든 정보를 포함하는 코드 조각을 생성할 수 있습니다.

### <a name="security-for-data-access"></a><a name="security"></a>데이터 액세스를 위한 보안
Python 클라이언트 라이브러리와 함께 사용하기 위해 Azure 기계 학습 스튜디오(클래식)에서 제공하는 코드 조각에는 작업 영역 ID 및 권한 부여 토큰이 포함됩니다. 이러한 코드 조각은 작업 영역에 대한 전체 액세스 권한을 제공하고 암호와 같이 보호되어야 합니다.

보안상의 이유로 코드 조각은 이전에 역할이 작업 영역의 **소유자** 로 설정된 사용자만 사용할 수 있습니다. 역할은 **설정**아래의 **사용자** 페이지의 Azure 기계 학습 스튜디오(클래식)에 표시됩니다.

![보안][security]

역할이 **소유자**로 설정되지 않은 경우 소유자로 다시 초대되도록 요청하거나 작업 영역의 소유자에게 코드 조각을 제공하도록 요청할 수 있습니다.

권한 부여 토큰을 얻으려면 다음 옵션 중 하나를 선택할 수 있습니다.

* 소유자에서 토큰을 요청합니다. 소유자는 Azure 기계 학습 스튜디오(클래식)의 작업 영역설정 페이지에서 권한 부여 토큰에 액세스할 수 있습니다. 왼쪽 창에서 **설정**을 선택하고 **권한 부여 토큰**을 클릭하여 기본 및 보조 토큰을 확인할 수 있습니다. 기본 또는 보조 권한 부여 토큰을 코드 조각에서 사용할 수 있지만, 소유자가 보조 권한 부여 토크만 공유하는 것이 좋습니다.

   ![권한 부여 토큰](./media/python-data-access/ml-python-access-settings-tokens.png)

* 소유자 역할로 승격하도록 요청: 작업 영역의 현재 소유자는 먼저 작업 영역에서 사용자를 제거한 다음 소유자로 다시 초대해야 합니다.

개발자가 작업 영역 ID 및 권한 부여 토큰을 얻으면 역할에 관계없이 코드 조각을 사용하여 작업 영역에 액세스할 수 있습니다.
...
1. 기계 학습 스튜디오(클래식)에서 왼쪽탐색 모음에서 **DATASET을** 클릭합니다.
...
기계 학습 스튜디오(클래식)에서 실험을 실행한 후에는 모듈의 출력 노드에서 중간 데이터 집합에 액세스할 수 있습니다. 중간 데이터 세트는 모델 도구가 실행될 때 중간 단계를 위해 생성되고 사용된 데이터입니다.

데이터 형식이 Python 클라이언트 라이브러리와 호환되는 한 중간 데이터 세트에 액세스할 수 있습니다.

다음 형식이 지원됩니다(이러한 형식에 대한 상수는 클래스에 `azureml.DataTypeIds` 있음).
...
5. 실험을 저장하고 실행하고 작업이 완료될 때까지 기다립니다.
...
작업 영역은 Python 클라이언트 라이브러리의 진입점입니다. 클래스에 `Workspace` 작업 영역 ID 및 권한 부여 토큰을 제공하여 인스턴스를 만듭니다.
...
기계 학습 스튜디오(클래식)에서 제공하는 코드 조각은 데이터 집합을 팬더 DataFrame 개체로 자동으로 다운로드하고 역직렬화합니다. 이 작업은 `to_dataframe` 메서드를 통해 수행됩니다.
...
`name` 매개 변수의 값을 지정하여 선택적으로 새로운 이름을 설정할 수 있습니다. 지금부터 새 이름만 사용하여 데이터 세트를 검색합니다. 다음 코드는 데이터, 이름 및 설명을 업데이트합니다.
