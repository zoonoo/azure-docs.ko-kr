...
description: 항공, 유틸리티 및 운송에서 예측 유지 관리를 위한 솔루션 템플릿에 대 한 기술 가이드입니다.
...
ms.locfileid: "79087435"
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>항공에서 예측 유지 관리를 위한 솔루션 템플릿에 대 한 기술 가이드
...
솔루션 템플릿은 E2E 데모 빌드 프로세스를 가속화 하도록 디자인 되었습니다. 배포 된 템플릿은 필요한 구성 요소를 사용 하 여 구독을 프로 비전 한 다음 두 구성 요소 간의 관계를 작성 합니다. 또한 솔루션 템플릿을 배포한 후 다운로드하여 로컬 컴퓨터에 설치하는 데이터 생성기 애플리케이션의 샘플 데이터로 데이터 파이프라인을 시드합니다. 생성기의 데이터는 데이터 파이프라인을 하이드레이션하며 나중에 Power BI 대시보드에서 시각화할 수 있는 기계 학습 예측을 생성하기 시작합니다.
...
![예측 유지 관리 아키텍처](./media/predictive-maintenance-technical-guide/predictive-maintenance-architecture.png)

솔루션을 배포할 때 이벤트 허브, Stream Analytics, HDInsight, Data Factory 및 Machine Learning를 비롯 한 Azure 서비스를 활성화 합니다. 아키텍처 다이어그램에서는 항공 솔루션 템플릿에 대한 예측 유지 관리가 생성되는 방법을 보여 줍니다. 이러한 서비스는 솔루션을 배포할 때 생성된 솔루션 템플릿 다이어그램에서 해당 서비스를 클릭하여 Azure Portal에서 조사할 수 있습니다(관련 파이프라인 작업을 실행하고 나중에 삭제해야 할 때 요청 시 프로비전되는 HDInsight 제외).
...
이 템플릿의 경우 사용 되는 데이터 원본은 성공적인 배포 후 로컬로 실행 되는 다운로드 한 데스크톱 응용 프로그램에서 생성 됩니다.

이 애플리케이션 다운로드 및 설치에 대한 지침을 찾으려면 솔루션 템플릿 다이어그램에서 첫 번째 노드인 예측 유지 관리 데이터 생성기를 선택합니다. 지시 사항은 속성 표시줄에 있습니다. 이 애플리케이션은 솔루션 흐름의 나머지 부분에서 사용되는 데이터 요소 또는 이벤트로 [Azure Event Hub](#azure-event-hub) 서비스를 피드합니다. 이 데이터 원본은 [Turbofan 엔진 성능 저하 시뮬레이션 데이터 집합](https://c3.nasa.gov/dashlink/resources/139/)을 사용한 [NASA 데이터 저장소](https://ti.arc.nasa.gov/tech/dash/groups/pcoe/prognostic-data-repository/#turbofan)의 공개적으로 사용할 수 있는 데이터에서 파생되었습니다.
...
[Azure Stream Analytics](https://azure.microsoft.com/services/stream-analytics/)를 사용하여 [Azure Event Hub](#azure-event-hub) 서비스의 입력 스트림에 대한 분석을 거의 실시간으로 제공합니다. 그런 다음 결과를 [Power BI](https://powerbi.microsoft.com) 대시보드에 게시하고 모든 원시 수신 이벤트를 [Azure Data Factory](https://azure.microsoft.com/services/storage/) 서비스에 의해 나중에 처리하기 위해 [Azure Storage](https://azure.microsoft.com/documentation/services/data-factory/) 서비스에 보관합니다.

### <a name="hdinsight-custom-aggregation"></a>HDInsight 사용자 지정 집계
HDInsight를 사용 하 여 [Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx) 스크립트 (Azure Data Factory 오케스트레이션)를 실행 하 여 Azure Stream Analytics 리소스를 사용 하 여 보관 된 원시 이벤트에 대 한 집계를 제공 합니다.
...
[Azure SQL Database](https://azure.microsoft.com/services/sql-database/) 를 사용 하 여 Azure Machine Learning에서 수신 된 예측을 저장 합니다. 그런 다음 [Power BI](https://powerbi.microsoft.com) 대시보드에서 사용 됩니다.

## <a name="data-consumption"></a>데이터 사용
### <a name="power-bi"></a>Power BI
[Power BI](https://powerbi.microsoft.com)를 사용하여 [Azure Stream Analytics](https://azure.microsoft.com/services/stream-analytics/)가 제공하는 집계 및 경고뿐만 아니라 [Azure Machine Learning](https://azure.microsoft.com/services/sql-database/)을 통해 생성된 [Azure SQL Database](https://azure.microsoft.com/services/machine-learning/)에 저장된 RUL 예측을 포함하는 대시보드를 표시합니다.
...
Azure Stream Analytics 리소스를 사용 하 여 데이터 스트림에서 읽고 원하는 수의 원본으로 데이터를 출력 하 여 거의 실시간으로 분석을 제공 합니다.
...
* Azure Portal에 연결
* 솔루션이 배포될 때 생성된 Stream Analytics 작업 ![Stream Analytics 아이콘](./media/predictive-maintenance-technical-guide/icon-stream-analytics.png)(*예:* 예측 유지 관리 솔루션에 대한 **maintenancesa02asapbi** 및 **maintenancesa02asablob**) 찾기
* 선택
...
![Data Factory 데이터 세트 오류](./media/predictive-maintenance-technical-guide/data-factory-dataset-error.png)

이 섹션에서는 [Azure Data Factory](../../data-factory/concepts-pipelines-activities.md)에 포함된 필요한 [파이프라인 및 작업](https://azure.microsoft.com/documentation/services/data-factory/)을 설명합니다. 솔루션의 다이어그램 보기는 다음과 같습니다.
...
이 [파이프라인](../../data-factory/concepts-pipelines-activities.md)은 [Azure Stream Analytics](../../data-factory/transform-data-using-hadoop-hive.md) 작업 동안 [Azure Storage](https://msdn.microsoft.com/library/azure/dn893526.aspx)에 넣은 데이터를 분할하도록 [Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx) 스크립트를 실행하는 [HDInsightLinkedService](https://azure.microsoft.com/services/storage/)를 사용한 단일 작업([HDInsightHive](https://azure.microsoft.com/services/stream-analytics/) 작업)을 포함합니다.
...
* [Azure Machine Learning](../../data-factory/transform-data-using-hadoop-hive.md) 실험에 필요한 집계 및 기능 엔지니어링을 수행하도록 [Hive](https://msdn.microsoft.com/library/azure/dn893526.aspx) 스크립트를 실행하는 [HDInsightLinkedService](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx)를 사용한 [HDInsightHive](https://azure.microsoft.com/services/machine-learning/) 작업.
...
* [HDInsightHive](https://msdn.microsoft.com/library/azure/dn835035.aspx) 작업의 결과를 [AzureMLBatchScoring](../../data-factory/transform-data-using-hadoop-hive.md) 작업으로 액세스할 수 있는 단일 [Azure Storage](https://azure.microsoft.com/services/storage/) Blob으로 이동하는 [복사](https://msdn.microsoft.com/library/azure/dn894009.aspx) 작업.
* [AzureMLBatchScoring](https://msdn.microsoft.com/library/azure/dn894009.aspx) 작업은 단일 [Azure Storage](https://azure.microsoft.com/services/machine-learning/) Blob에 배치되는 결과로 [Azure Machine Learning](https://azure.microsoft.com/services/storage/) 실험을 호출합니다.

#### <a name="copyscoredresultpipeline"></a>*CopyScoredResultPipeline*
이 [파이프라인](../../data-factory/concepts-pipelines-activities.md)은 [MLScoringPipeline](https://msdn.microsoft.com/library/azure/dn835035.aspx)에서 [Azure Machine Learning](#azure-machine-learning) 실험의 결과를 솔루션 템플릿 설치의 일부로 프로비전되는 ***Azure SQL Database***에 이동하는 단일 작업([복사](https://azure.microsoft.com/services/sql-database/) 작업)을 포함합니다.
...
* 파이프라인의 마지막 단계는 SQL Database에 데이터(예: Machine Learning에서 예측)를 쓰는 것입니다. 데이터를 SQL Database에 표시하려면 최대 3시간을 기다려야 할 수도 있습니다. 얼마나 많은 데이터를 SQL Database에서 사용할 수 있는지를 모니터링하는 한 가지 방법은 [Azure Portal](https://portal.azure.com/)을 통한 방법입니다. 왼쪽 패널에서 sql DATABASE ![SQL 아이콘](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png)를 찾아 클릭 합니다. 그런 다음 데이터베이스 **pmaintenancedb**를 찾고 클릭합니다. 맨 아래의 다음 페이지에서 관리를 클릭합니다.
   
    ![관리 아이콘](./media/predictive-maintenance-technical-guide/icon-manage.png)
...
Power BI는 데이터 원본으로 Azure SQL Database에 연결 하 여 예측 결과를 저장 합니다. 

참고: 
1.    솔루션을 배포할 때 예측은 3 시간 이내에 데이터베이스에 표시 됩니다. 생성기 다운로드와 함께 제공되는 pbix 파일은 지금 바로 Power BI 대시보드를 만들 수 있도록 일부 시드 데이터를 포함합니다. 
2.    이 단계에서 필수 구성 요소는 무료 소프트웨어 [Power BI 데스크톱](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/)을 다운로드 하 고 설치 하는 것입니다.
...
1. 데이터베이스 자격 증명을 가져옵니다.
   ...
   * Azure Portal 페이지를 표시 하는 새 브라우저 탭/창이 표시 됩니다. 왼쪽 패널에서 **'리소스 그룹'** 을 클릭합니다.
   * 솔루션 배포에 사용 중인 구독을 선택한 다음 **'YourSolutionName\_ResourceGroup'** 을 선택합니다.
   * 새 팝업 패널에서 ![SQL 아이콘](./media/predictive-maintenance-technical-guide/icon-sql.png) 아이콘을 클릭하여 데이터베이스에 액세스합니다. 데이터베이스 이름(예: **'pmaintenancedb'** )은 이 아이콘 옆에 있으며, **데이터베이스 서버 이름**은 서버 이름 속성 아래에 나열되고 **YourSolutionName.database.windows.net**과 비슷합니다.
...
2. Power BI 데스크톱을 사용하여 콜드 경로 보고서 파일의 데이터 원본을 업데이트합니다.
   ...
   * 생성기 파일을 다운로드하고 압축을 푼 폴더에서 **PowerBI\\PredictiveMaintenanceAerospace.pbix** 파일을 두 번 클릭합니다. 파일을 열 때 경고 메시지가 표시되는 경우 무시합니다. 파일의 맨 위에서 **'쿼리 편집'** 을 클릭합니다.
     ...
     ![쿼리 편집](./media/predictive-maintenance-technical-guide/edit-queries.png)
   * 두 개의 테이블, **RemainingUsefulLife** 및 **PMResult**가 표시됩니다. 첫 번째 테이블을 선택하고 오른쪽 !['쿼리 설정'](./media/predictive-maintenance-technical-guide/icon-query-settings.png) 패널의 **'적용된 단계'** 아래에서 **'원본'** 옆의 **쿼리 설정 아이콘**을 클릭합니다. 표시되는 경고 메시지를 무시합니다.
   ...
   * 다음 팝 아웃 창에서 왼쪽 창에 두 가지 옵션이 표시됩니다(**Windows** 및 **데이터베이스**). **' 데이터베이스 '** 를 클릭 하 고 **' 사용자 이름 '** 및 **' 암호 '** 를 입력 합니다 (처음으로 솔루션을 배포 하 고 Azure SQL Database를 만들 때 입력 한 사용자 이름 및 암호). ***이러한 설정을 적용할 수준 선택***에서 데이터베이스 수준 옵션을 선택합니다. 그런 다음 **'연결'** 을 클릭합니다.
   * 두 번째 테이블 **PMResult**를 클릭하고 오른쪽 !['쿼리 설정'](./media/predictive-maintenance-technical-guide/icon-navigation.png) 패널의 **'적용된 단계'** 아래 **'원본'** 옆에 있는 **탐색 아이콘**을 클릭한 다음 위 단계와 마찬가지로 서버 및 데이터베이스 이름을 업데이트하고 확인을 클릭합니다.
...
3. (선택 사항) 콜드 경로 대시보드를 [Power BI 온라인](https://www.powerbi.com/)에 게시합니다. 이 단계에는 Power BI 계정 (또는 Office 365 계정)이 필요 합니다.
   ...
   * 보고서를 열면 ![PIN 아이콘](./media/predictive-maintenance-technical-guide/icon-pin.png)을 클릭하여 모든 시각화를 대시보드에 고정합니다. 자세한 지침을 찾으려면 [보고서에서 Power BI 대시보드에 타일 고정](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report)을 참조하세요.
     ...
     <br/>
     ![최종 보기](./media/predictive-maintenance-technical-guide/final-view.png)
     <br/>
   * 데이터 새로 고침을 예약 하려면 마우스를 **predictivemaintenanceaerospace.pbix** 데이터 집합 위에 놓고 ![줄임표 아이콘](./media/predictive-maintenance-technical-guide/icon-elipsis.png) 클릭 한 다음 **새로 고침 예약**을 선택 합니다.
     <br/>
     **참고:** 경고 메시지가 표시되는 경우 **자격 증명 편집**을 클릭하고 데이터베이스 자격 증명이 1단계에서 설명된 것과 동일한지 확인합니다.
     <br/>
     ![새로 고침 예약](./media/predictive-maintenance-technical-guide/schedule-refresh.png)
     <br/>
...
3. 데이터가 Power BI로 흐르게 한 후 스트리밍 데이터의 시각화를 시작할 수 있습니다. 다음은 고정된 몇 가지 실행 부하 과다 경로 시각화를 사용한 예제 대시보드입니다. 적절한 데이터 세트에 따라 다른 대시보드 타일을 만들 수 있습니다. 데이터 생성기의 실행 시간에 따라 시각화에 대한 숫자가 달라질 수 있습니다.
    ...
    ![대시보드 보기](media/predictive-maintenance-technical-guide/dashboard-view.png)

1. 위의 타일 중 하나를 만드는 몇 가지 단계는 다음과 같습니다. "센서 11과 임계값 48.26의 제 수 보기" 타일:
   ...
   * 이 꺾은선형 차트의 오른쪽 위 모서리에서 **시각적 개체 고정** 아이콘을 클릭 합니다. "대시보드에 고정" 창이 대시보드를 선택하도록 표시될 수 있습니다. "예측 유지 관리 데모"를 선택한 다음 "고정"을 클릭합니다.
   * 대시보드에서이 타일 위로 마우스를 이동 하 고 오른쪽 위 모서리에서 "편집" 아이콘을 클릭 하 여 제목을 "센서 11과 임계값 48.26"으로 변경 하 고 부제목을 "시간 경과에 따른 평균 이동"으로 변경 합니다.
