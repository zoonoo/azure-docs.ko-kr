---
title: ML 스튜디오(클래식) - 팀 데이터 과학 프로세스를 위한 데이터 준비
...
데이터 전처리 및 정리는 데이터 집합을 모델 학습에 사용하려면 먼저 수행해야 하는 중요한 작업입니다. 원시 데이터는 노이즈가 많고, 불안정하고, 값이 누락된 경우가 종종 있습니다. 이러한 데이터를 모델링에 사용하면 결과가 잘못될 수 있습니다. 이러한 작업은 TDSP(팀 데이터 과학 프로세스)의 일부이며 일반적으로 필요한 전처리를 검색하고 계획하는 데 사용되는 데이터 세트의 초기 탐색을 수행합니다. TDSP 프로세스에 대한 자세한 지침은 [팀 데이터 과학 프로세스](overview.md)에 설명된 단계를 참조하세요.

데이터 탐색 작업과 같은 전처리 및 정리 작업은 SQL 또는 Hive 또는 Azure 기계 학습 스튜디오(클래식)와 같은 다양한 환경에서 수행될 수 있으며, 데이터 위치에 따라 R 또는 Python과 같은 다양한 도구와 언어를 사용하여 수행할 수 있습니다. 저장되고 서식이 지정됩니다. TDSP는 반복 성향을 띠기 때문에, 이러한 작업은 프로세스의 워크플로 내의 다양한 단계에서 발생할 수 있습니다.

이 문서에서는 Azure 기계 학습 Studio(클래식)로 데이터를 수집하기 전이나 후에 수행할 수 있는 다양한 데이터 처리 개념 및 작업을 소개합니다.

Azure 기계 학습 스튜디오(클래식) 내에서 수행된 데이터 탐색 및 전처리의 예는 [데이터 전처리](https://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) 비디오를 참조하십시오.
...
* **일치하지 않는 데이터 레코드**. 값의 범위가 허용되는지 확인하세요. 예를 들어 데이터에 학생 GPA(성적 점수 평균)가 포함된 경우 GPA가 지정된 범위에 있는지 확인합니다(예: 0~4).

데이터 문제를 발견하면 누락된 값 정리, 데이터 정규화, 불연속화, 데이터 정렬에 영향을 줄 수 있는 포함된 문자를 제거 및/또는 바꾸기 위한 텍스트 처리, 공통 필드의 혼합 데이터 형식 등이 포함된 처리 **단계가** 필요합니다.

**Azure 기계 학습은 잘 구성된 테이블 형식 데이터를 사용합니다.**  데이터가 이미 테이블 형식인 경우 기계 학습의 Azure 기계 학습 스튜디오(클래식)를 사용하여 데이터 사전 처리를 직접 수행할 수 있습니다.  데이터가 테이블 형식이 아닌 XML 형식이라고 한다면 데이터를 테이블 형식으로 변환하려면 구분 분석이 필요할 수 있습니다.  

## <a name="what-are-some-of-the-major-tasks-in-data-pre-processing"></a>데이터 전처리의 주요 작업
* **데이터 정리**: 누락된 값을 채우고, 시끄러운 데이터와 이상값을 감지하고 제거합니다.
* **데이터 변환**: 데이터를 정규화하여 차원과 노이즈를 줄입니다.
* **데이터 감소**: 데이터 레코드 또는 특성을 샘플링하여 보다 쉽게 데이터를 처리할 수 있습니다.
* **데이터 불연속화**: 연속 특성을 범주형 특성으로 변환하여 특정 기계 학습 방법에서 사용하기 쉽도록 합니다.
* **텍스트 정리**: 예를 들어 탭 분리 된 데이터 파일에 포함된 탭, 레코드를 깨뜨릴 수있는 새 줄이 포함 될 수있는 데이터 정렬 불량을 일으킬 수있는 포함 된 문자를 제거합니다.
...
**테이블 형식 데이터의 텍스트 필드에는** 열 정렬 및/또는 레코드 경계에 영향을 주는 문자가 포함될 수 있습니다. 예를 들어 탭으로 분리된 파일에 포함된 탭은 열 정렬이 잘못되고 포함된 새 줄 문자가 레코드 줄을 끊습니다. 텍스트를 쓰거나 읽는 동안 부적절한 텍스트 인코딩 처리는 정보 손실, 읽을 수 없는 문자(예: null)의 실수로 도입되며 텍스트 구문 분석에도 영향을 줄 수 있습니다. 데이터를 올바르게 정렬하고 구조화되지 않은 데이터 또는 반 구조적 데이터에서 구조적 데이터를 추출할 수 있도록 텍스트 필드를 정리하려면 신중한 구문 분석 및 편집 작업이 필요할 수 있습니다.

**데이터 탐색**을 통해 초기에 데이터를 살펴볼 수 있습니다. 이 단계에서 다양한 데이터 문제를 파악하고 그에 맞는 적절한 방법을 적용하여 이러한 문제를 해결할 수 있습니다.  문제의 원인이 무엇인지, 문제가 어떻게 시작되었는지 등의 질문에 대한 답을 고민해 보는 것이 중요합니다. 또한 이 프로세스를 통해 이를 해결하기 위해 수행해야 하는 데이터 처리 단계를 결정할 수 있습니다. 최종 사용 사례와 페르소나 식별을 사용하여 데이터 처리 노력의 우선 순위를 지정할 수도 있습니다.
