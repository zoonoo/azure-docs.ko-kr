...
TDSP는 예측 분석 솔루션 및 지능형 응용 프로그램을 효율적으로 제공 하는 민첩 하 고 반복적인 데이터 과학 방법론입니다. 이 프로세스는 회사에서 분석 프로그램의 이점을 완전히 실현할 수 있도록 데이터 과학 이니셔티브를 성공적으로 구현 하는 데 필요한 Microsoft 및 업계의 모범 사례와 구조를 추출을 합니다. TDSP를 표준화 하는 데이터 과학 팀에 대 한 담당자 역할 및 관련 작업에 대 한 개요는 [팀 데이터 과학 프로세스 역할 및 작업](roles-tasks.md)을 참조 하세요.
