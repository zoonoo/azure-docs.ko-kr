...
NYC Taxi 여행 데이터는 약 20gb의 압축 된 CSV 파일 (48 1억7300만, 압축 되지 않음) 이며 각 여행에 대해 요금을 지불 하는 개별 트립 및 정의 요금를 구성 합니다. 각 여정 레코드는 승차 및 하차 위치, 익명 처리된 hack(기사) 면허증 번호 및 medallion(택시의 고유 ID) 번호를 포함합니다. 데이터는 2013년의 모든 여정을 포괄하며, 매월 다음 두 개의 데이터 세트로 제공됩니다.
...
* 이진 분류: 팁이 여행에 대해 지불 되었는지 여부 즉, $0 보다 큰 *팁\_금액이* 긍정적인 예이 고 *tip\_$0 amount* 가 부정적 예입니다.
...
2. VM의 데이터 디스크에 새 디렉터리를 만듭니다 (참고: VM과 함께 제공 되는 임시 디스크를 데이터 디스크로 사용 하지 않음).
...
2. Windows 인증을 사용하여 연결
    ...
    ![SSMS 연결][12]
3. SQL Server 인증 모드를 변경하여 새 SQL 로그인 사용자를 만드는 작업을 아직 수행하지 않은 경우 **Sample Scripts\_ 폴더에서** change**auth.sql**이라는 스크립트 파일을 엽니다. 기본 사용자 이름 및 암호를 변경합니다. 도구 모음에서 **실행** 을 클릭 하 여 스크립트를 실행 합니다.
...
10. 이제 데이터베이스에서 탐색, 기능 엔지니어링 및 필요에 따라 다른 작업을 수행할 준비가 완료되었습니다. 테이블은 **pickup\_datetime** 필드에 따라 분할 되기 때문에 **WHERE** 절에 **pickup\_datetime** 조건을 포함 하는 쿼리는 파티션 구성표를 활용 합니다.
...
2. 모델을 빌드하는 데 사용할 샘플링 및 엔지니어링 된 데이터를 새 데이터베이스 테이블에 유지 하 고 Azure Machine Learning의 [데이터 가져오기][import-data] 모듈에서 새 테이블을 사용 합니다.

이 섹션에서는 데이터를 추출 하 고 샘플링 하는 최종 쿼리를 저장 합니다. 두 번째 방법은 [IPython Notebook에서 데이터 탐색 및 기능 엔지니어](#ipnb) 섹션에 설명되어 있습니다.
...
1. 최종 SQL 쿼리를 저장 하 여 데이터를 추출 및 샘플링 하 고 쿼리를 복사 하 여 Azure Machine Learning의 [데이터 가져오기][import-data] 모듈에 직접 붙여 넣습니다. 이 방법은 [Azure Machine Learning에서 모델 빌드](#mlmodel) 섹션에 설명되어 있습니다.    
2. 모델을 빌드하는 데 사용할 샘플링 및 엔지니어링 된 데이터를 새 데이터베이스 테이블에 유지 한 다음 [데이터 가져오기][import-data] 모듈에서 새 테이블을 사용 합니다.
...
[Azure Machine Learning Studio](https://studio.azureml.net)에서 모델 빌드에 대 한 데이터를 준비 하는 경우 데이터 **가져오기 모듈에서 직접 사용할 SQL 쿼리** 를 결정 하거나, [데이터 가져오기][import-data] 모듈에서 간단한 **SELECT * FROM <\_새\_테이블\_이름 >** 을 사용 하 여 새 테이블에 엔지니어링 및 샘플링 된 데이터를 유지할 수 있습니다.
...
이 섹션에서는 위에서 만든 새 테이블에 유지 되는 샘플링 된 데이터 1%를 사용 하 여 데이터 분포를 탐색 합니다. 원본 테이블을 사용 하 여 유사한 탐색을 수행할 수 있습니다. 필요에 따라 **TABLESAMPLE** 을 사용 하 여 탐색 샘플을 제한 하거나 [SQL Server의 데이터 탐색 및 기능 엔지니어링](#dbexplore) 섹션에 설명 된 대로 **pickup\_datetime** 파티션을 사용 하 여 결과를 지정 된 기간으로 제한 합니다.
...
이 예에서는 연속 숫자 필드를 사전 설정 된 범주 범위 (숫자 필드를 범주 필드로 변환)로 변환 합니다.
...
이 연습에서는 이미 SQL Server에서 데이터를 탐색 및 엔지니어링하고 Azure Machine Learning에서 수집할 샘플 크기를 결정했습니다. 하나 이상의 예측 모델을 작성 하려면 다음을 결정 합니다.

1. 데이터 **입력 및 출력** 섹션에서 사용할 수 있는 데이터 [가져오기][import-data] 모듈을 사용 하 여 Azure Machine Learning 데이터를 가져옵니다. 자세한 내용은 [데이터 가져오기][import-data] 모듈 참조 페이지를 참조 하세요.
...
> 
> 불필요 한 열 및/또는 대상 누수를 제외 하려면 [데이터 집합의 열 선택][select-columns] 모듈 또는 [메타 데이터 편집][edit-metadata]을 사용할 수 있습니다. 자세한 내용은 [데이터 집합에서 열 선택][select-columns] 및 [메타 데이터][edit-metadata] 참조 페이지 편집을 참조 하세요.
> 
> 
...
점수 매기기 실험을 만들 때 필요에 따라 검토하고 조정합니다. 일반적인 조정은 서비스를 호출할 때 스키마에서 이러한 레이블을 사용할 수 없으므로 입력 데이터 집합 및/또는 쿼리를 레이블 필드가 제외 된 항목으로 바꾸는 것입니다. 입력 데이터 집합 및/또는 쿼리 크기를 몇 개의 레코드로 줄여서 입력 스키마를 나타낼 수 있는 좋은 방법 이기도 합니다. 출력 포트의 경우 일반적으로 모든 입력 필드를 제외 하 고 [데이터 집합에서 열 선택][select-columns] 모듈을 사용 하 여 **점수 매기기 레이블과** **점수가 매겨진 확률** 을 출력에 포함 합니다.
