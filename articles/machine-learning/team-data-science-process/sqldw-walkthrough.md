---
title: Azure Synapse 분석 - 팀 데이터 과학 프로세스를 사용하여 모델 빌드 및 배포
description: 공개적으로 사용 가능한 데이터 집합을 사용하여 Azure Synapse 분석을 사용하여 기계 학습 모델을 빌드하고 배포합니다.
...
1. **이진 분류**: 팁이 여행에 대해 지불되었는지 여부를 예측하기 위해, 즉 팁 *\_금액이* 0달러를 초과하는 팁 금액은 긍정적인 예이며 *팁\_금액은* $0이 부정적인 예입니다.
...
3. **회귀 작업**: 여행에 대해 지불 한 팁의 양을 예측합니다.
...
**사용자 고유의 Azure Blob 저장소 계정 만들기**

* 자체 Azure Blob 저장소를 프로비전할 때 NYC Taxi 데이터가 저장되는 **미국 중남부에**가능한 한 가깝게 Azure Blob 저장소의 지리적 위치를 선택합니다. Azure File Storage를 사용하여 파일 공유에 의존하는 레거시 응용 프로그램을 비경제적인 다시 쓰기 작업 없이 빠르게 Azure로 마이그레이션할 수 있습니다. File Storage를 사용하여 세상에 공개적으로 표시하거나 응용 프로그램 데이터를 비공개적으로 저장할 수 있습니다.
* 고유한 Azure Storage 계정을 만들려면 Azure [Storage 계정 정보에서](../../storage/common/storage-create-storage-account.md)설명하는 단계를 따릅니다. 이 연습의 뒷부분에서 필요하므로 다음 스토리지 계정 자격 증명에 대한 값을 적어두어야 합니다.

  * **저장소 계정 이름**
  * **저장소 계정 키**
  * **컨테이너 이름(Azure** Blob 저장소에 데이터를 저장하려는 이름)

**Azure 시냅스 분석 인스턴스를 프로비전합니다.**
[Azure 포털에서 Azure SQL 데이터 웨어하우스 만들기 및 쿼리의](../../synapse-analytics/sql-data-warehouse/create-data-warehouse-portal.md) 설명서를 따라 Azure Synapse 분석 인스턴스를 프로비전합니다. 이후 단계에서 사용할 다음 Azure Synapse Analytics 자격 증명에 대한 표기이 작업을 수행해야 합니다.
...
## <a name="load-the-data-into-azure-synapse-analytics"></a><a name="getdata"></a>Azure 시냅스 분석에 데이터 로드
Windows PowerShell 명령 콘솔을 엽니다. 다음 PowerShell 명령을 실행하여 GitHub에서 사용자와 공유하는 예제 SQL 스크립트 파일을 매개 변수 *-DestDir로*지정한 로컬 디렉터리로 다운로드합니다. 매개 변수 *-DestDir* 의 값을 로컬 디렉터리로 변경할 수 있습니다. *-DestDir* 이 존재하지 않는 경우 PowerShell 스크립트를 통해 생성됩니다.
...
* 다음 명령을 사용하여 개인 Blob 저장소 계정에서 **Polybase(LoadDataToSQLDW.sql을 실행하여)를 사용하여 Azure 시냅스 분석에 데이터를 로드합니다.**
  ...
  * AzCopy는 최적의 성능을 내는 간단한 명령을 사용하여 데이터를 Microsoft Azure Blob 및 File Storage에 복사하거나 이들 스토리지에서 복사하기 위한 명령줄 유틸리티입니다.
    ...
    - Azure Blob 저장소의 외부 테이블에서 Azure 시냅스 분석으로 데이터 로드
    ...
    - 샘플 데이터 테이블(NYCTaxi_Sample)을 만들고 여정 및 요금 테이블에 SQL 쿼리를 선택하여 데이터를 삽입합니다. 이 연습의 일부 단계는 이 샘플 표를 사용해야 합니다.
...
> [!TIP]
> 데이터가 실제 응용 프로그램의 개인 Azure Blob 저장소에 이미 있는 경우 PowerShell 스크립트에서 AzCopy 단계를 건너뛰고 Azure Synapse Analytics에 직접 데이터를 업로드할 수 있습니다. 데이터 형식에 맞추려면 스크립트를 추가로 편집해야 합니다.
>
>
...
## <a name="data-exploration-and-feature-engineering-in-azure-synapse-analytics"></a><a name="dbexplore"></a>Azure 시냅스 분석의 데이터 탐색 및 기능 엔지니어링
이 섹션에서는 Visual Studio 데이터 도구를 사용하여 Azure Synapse Analytics에 대해 직접 SQL 쿼리를 실행하여 데이터 탐색 및 기능 **생성을**수행합니다. 이 섹션에서 사용되는 모든 SQL 쿼리는 *SQLDW_Explorations.sql*이라는 샘플 스크립트에서 찾을 수 있습니다. 이 파일은 이미 PowerShell 스크립트에 의해 로컬 디렉터리에 다운로드되었습니다. [GitHub](https://raw.githubusercontent.com/Azure/Azure-MachineLearning-DataScience/master/Misc/SQLDW/SQLDW_Explorations.sql)에서 검색할 수도 있습니다. 그러나 GitHub의 파일에는 Azure Synapse 분석 정보가 연결되어 있지 않습니다.

Azure Synapse 애널리틱스 로그인 이름 및 암호가 있는 Visual Studio를 사용하여 Azure 시냅스 분석에 연결하고 **SQL 개체 탐색기를** 열어 데이터베이스 및 테이블을 가져왔는지 확인합니다. *SQLDW_Explorations.sql* 파일을 검색합니다.
...
이 섹션에서 수행되는 데이터 탐색 및 기능 생성 작업의 유형은 다음과 같습니다.
...
**출력:** 쿼리는 13,369개의 메달(택시)과 2013년에 완료된 운행 횟수를 지정하는 행이 있는 테이블을 반환해야 합니다. 마지막 열에는 완료된 여정 수가 포함됩니다.
...
**출력:** 이 쿼리는 승차 및 하차 위도 및 경도와 해당 직선 거리(마일)가 포함된 테이블(2,803,538개 행)을 생성합니다. 처음 세 행에 대한 결과는 다음과 같습니다.
...
다음 쿼리는 **nyctaxi\_trip** 및 **nyctaxi\_fare** 테이블을 조인하고, 이진 분류 레이블 **tipped**와 다중 클래스 분류 레이블 **tip\_class**를 생성하며, 조인된 전체 데이터 세트에서 샘플을 추출합니다. 샘플링은 승차 시간에 따라 여정의 하위 집합을 검색하여 수행됩니다.  이 쿼리는 Azure의 SQL Database 인스턴스에서 직접 데이터 수집을 위해 [Azure 기계 학습 스튜디오(클래식)](https://studio.azureml.net) [가져오기][데이터] 가져오기 모듈에 직접 붙여넣은 다음 복사할 수 있습니다. 잘못된 (0, 0) 좌표가 있는 레코드는 쿼리에서 제외됩니다.
...
1. 최종 SQL 쿼리를 저장하여 데이터를 추출 및 샘플링하고 Azure Machine Learning에서 가져오기[데이터] 모듈에 쿼리를 직접 복사하여 복사하거나
2. 새 Azure Synapse Analytics 테이블에서 모델 빌드에 사용할 샘플 및 엔지니어링 데이터를 유지하며 Azure Machine Learning의 데이터[가져오기-데이터] [가져오기]모듈의 새 테이블을 사용합니다. 이전 단계의 PowerShell 스크립트에서 이 작업을 수행했습니다. 데이터 가져오기 모듈의 이 테이블에서 직접 읽을 수 있습니다.

## <a name="data-exploration-and-feature-engineering-in-ipython-notebook"></a><a name="ipnb"></a>IPython Notebook에서 데이터 탐색 및 기능 엔지니어링
이 섹션에서는 앞에서 만든 Azure Synapse Analytics에 대해 Python 및 SQL 쿼리를 모두 사용하여 데이터 탐색 및 기능 생성을 수행합니다. **SQLDW_Explorations.ipynb**라는 샘플 IPython Notebook 및 Python 스크립트 파일 **SQLDW_Explorations_Scripts.py**는 로컬 디렉터리에 다운로드되었습니다. 또한 [GitHub](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/SQLDW)에서 사용할 수 있습니다. 이 두 파일은 Python 스크립트에서 동일합니다. IPython Notebook 서버가 없는 경우 Python 스크립트 파일이 제공됩니다. 이 두 샘플 Python 파일은 **Python 2.7**에서 디자인됩니다.

샘플 IPython 노트북및 로컬 컴퓨터에 다운로드된 파이썬 스크립트 파일에 필요한 Azure 시냅스 분석 정보가 이전에 PowerShell 스크립트에 의해 연결되었습니다. 수정 없이 실행할 수 있습니다.

Azure 기계 학습 작업 영역을 이미 설정한 경우 샘플 IPython 노트북을 AzureML IPython 노트북 서비스에 직접 업로드하고 실행을 시작할 수 있습니다. AzureML IPython 노트북 서비스에 업로드하는 단계는 다음과 같습니다.

1. Azure 기계 학습 작업 영역에 로그인하고 상단의 **Studio"를** 클릭하고 웹 페이지 왼쪽에 있는 **NOTEBOOKS를 클릭합니다.**

    ![Studio를 클릭한 다음, NOTEBOOKS 클릭][22]
2. 웹 페이지의 왼쪽 하단 모서리에서 **NEW를** 클릭하고 **파이썬 2를**선택합니다. 그런 다음 노트북에 이름을 제공하고 확인 표시를 클릭하여 새 비어 있는 IPython Notebook을 만듭니다.

    ![NEW를 클릭한 다음, Python 2 클릭][23]
3. 새로운 IPython 노트북의 왼쪽 상단 모서리에 있는 **Jupyter** 기호를 클릭합니다.
...
대규모 데이터를 사용하여 Azure 기계 학습에서 고급 분석 솔루션을 빌드할 때 권장되는 순서는 다음과 같습니다.

* 소량의 데이터 샘플을 메모리 내 데이터 프레임으로 읽습니다.
* 샘플링된 데이터를 사용하여 일부 시각화 및 탐색을 수행합니다.
* 샘플링된 데이터를 사용하여 기능 엔지니어링을 실험합니다.
* 대규모 데이터 탐색, 데이터 조작 및 기능 엔지니어링의 경우 파이썬을 사용하여 Azure Synapse 분석에 대해 직접 SQL 쿼리를 발급합니다.
...
### <a name="read-in-a-small-data-sample-from-the-azure-synapse-analytics-database"></a>Azure 시냅스 분석 데이터베이스에서 작은 데이터 샘플 읽기
    t0 = time.time()
...
이 섹션에서는 위에서 만든 새 테이블에 유지되는 샘플링된 데이터를 사용하여 데이터 분포를 살펴봅습니다. 원래 테이블을 사용하여 유사한 탐색을 수행할 수 있습니다.
...
이제 [Azure 기계 학습에서](https://studio.azureml.net)빌드 및 모델 배포를 진행할 준비가 되었습니다. 이전에 식별된 다음과 같은 예측 문제에 데이터를 사용할 준비가 되었습니다.

1. **이진 분류**: 팁이 여행에 대한 지불 되었는지 여부를 예측합니다.
2. **다중 클래스 분류**: 이전에 정의 된 클래스에 따라 지불 팁의 범위를 예측합니다.
3. **회귀 작업**: 여행에 대해 지불 한 팁의 양을 예측합니다.

모델링 연습을 시작하려면 Azure 기계 **학습(클래식)** 작업 영역에 로그인합니다. 아직 기계 학습 작업 영역을 만들지 않은 경우 [Azure 기계 학습 스튜디오(클래식) 작업 영역 만들기를](../studio/create-workspace.md)참조하십시오.

1. Azure 기계 학습을 시작하려면 [Azure 기계 학습 스튜디오(클래식)란 무엇입니까?](../studio/what-is-ml-studio.md)
2. [Azure 기계 학습 스튜디오(클래식)에](https://studio.azureml.net)로그인합니다.
3. 기계 학습 스튜디오 (클래식) 홈 페이지는 풍부한 정보, 비디오, 자습서, 모듈 참조에 대한 링크 및 기타 리소스를 제공합니다. Azure 기계 학습에 대한 자세한 내용은 [Azure 기계 학습 설명서 센터를](https://azure.microsoft.com/documentation/services/machine-learning/)참조하십시오.

일반적인 학습 실험은 다음 단계로 구성됩니다.

1. **+NEW** 실험을 만듭니다.
2. Azure 기계 학습 스튜디오(클래식)로 데이터를 가져옵니다.
...
이 연습에서는 Azure Synapse Analytics의 데이터를 이미 탐색 및 엔지니어링했으며 Azure 기계 학습 스튜디오(클래식)에서 수집할 샘플 크기를 결정했습니다. 예측 모델 중 하나 이상을 빌드하는 절차는 다음과 같습니다.

1. **데이터 입력 및 출력** 섹션에서 사용할 수 있는 데이터[가져오기-데이터] 가져오기 모듈을 사용하여 Azure 기계 학습 스튜디오(클래식)로 데이터를 가져옵니다. [Import Data] 자세한 내용은 [데이터 가져오기][import-data] 참조 페이지를 참조하세요.
...
점수 매기기 실험이 만들어지면 결과를 검토하고 필요에 따라 조정합니다. 일반적인 조정은 서비스를 호출할 때 이러한 레이블 필드가 스키마에 매핑되지 않으므로 입력 데이터 집합 또는 쿼리를 레이블 필드를 제외하는 데이터 집합 또는 쿼리로 바꾸는 것입니다. 또한 입력 스키마를 나타낼 수 있을 정도로 입력 데이터 집합 의 크기를 줄이고 몇 개의 레코드로 쿼리하는 것이 좋습니다. 출력 포트의 경우 일반적으로 [데이터 세트의 열 선택][select-columns] 모듈을 사용하여 모든 입력 필드를 제외하고 **점수가 매겨진 레이블** 및 **점수가 매겨진 확률**만 출력에 포함합니다.
