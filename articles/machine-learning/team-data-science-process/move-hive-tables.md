...
### <a name="headnode"></a>Hadoop 클러스터 헤드 노드의 Hadoop 명령줄을 통해 Hive 쿼리 제출
...
Hive 쿼리가 좀 더 복잡하고 줄이 여러 개인 경우 명령줄 또는 Hive 명령 콘솔에서 쿼리를 편집하는 방법은 실용적이지 않습니다. 대안은 Hadoop 클러스터의 헤드 노드에 있는 텍스트 편집기를 사용 하 여 헤드 노드의 로컬 디렉터리에 있는 '. hql ' 파일에 Hive 쿼리를 저장 하는 것입니다. 그러면 다음과 같이 `-f` 인수를 사용 하 여 '. hql ' 파일의 Hive 쿼리를 제출할 수 있습니다.
...
다음 예제에서 Hive 쿼리의 출력은 `hivequeryoutput.txt` 디렉터리의 `C:\apps\temp` 파일에 작성됩니다.
...
### <a name="hive-editor"></a>Hive 편집기를 사용 하 여 Hive 쿼리 제출
웹 브라우저에 *https:\//\<Hadoop 클러스터 이름 >* 의 URL을 입력 하 여 쿼리 콘솔 (Hive 편집기)을 사용할 수도 있습니다. 이 콘솔을 보려면 로그인해야 하며, Hadoop 클러스터 자격 증명이 필요합니다.

### <a name="ps"></a>Azure PowerShell 명령을 사용 하 여 Hive 쿼리 제출
...
* **\<데이터베이스 이름\>** : 만들려고 하는 데이터베이스 이름입니다. 기본 데이터베이스를 사용 하려는 경우에는 "*create database ...* " 쿼리를 생략할 수 있습니다.
...
* **blob 데이터\>에 대 한\<경로** : Hive 테이블에 업로드할 Blob 파일이 HDInsight Hadoop 클러스터의 기본 컨테이너에 있는 경우 *blob 데이터\>\<경로* 는 *이 컨테이너\</> blob 파일 이름\<'의 ' wasb://> 디렉터리*형식 이어야 합니다. blob 파일이 HDInsight Hadoop 클러스터의 추가 컨테이너에 있을 수도 있습니다. 이 경우 *blob\>데이터에 대 한\<경로* 는 *' wasb://\<container name >\<저장소 계정 이름 >. i n t/\<blob 파일 이름 > '* 형식 이어야 합니다.
...
분할 된 테이블을 쿼리 하는 경우 `where` 절의 **시작 부분** 에 파티션 조건을 추가 하 여 검색 효율성을 향상 시키는 것이 좋습니다.
...
모든 데이터가\<데이터베이스 이름\>에 삽입 된 후 다음 쿼리를 사용 하는 경우 *\<외부 텍스트 파일 테이블\>이름을* 삭제 하는 것이 안전 합니다 *.\<ORC 테이블 이름\>* :
