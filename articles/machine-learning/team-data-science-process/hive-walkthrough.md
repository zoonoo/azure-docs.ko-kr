...
더 큰 데이터 세트를 처리하는 방법을 보여주는 연습은 [팀 데이터 과학 프로세스 - 1TB 데이터 세트에서 Azure HDInsight Hadoop 클러스터 사용](hive-criteo-walkthrough.md)을 참조하세요.

IPython 노트북에서 1TB 데이터 세트를 사용하는 연습 작업을 수행할 수도 있습니다. 자세한 내용은 [Hive ODBC 연결을 사용하여 Criteo 연습](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/iPythonNotebooks/machine-Learning-data-science-process-hive-walkthrough-criteo.ipynb)을 참조하세요.
...
- trip_data CSV 파일은 승객 수, 승차 및 하차 지점, 여정 기간, 여정 거리 등 여정 세부 정보를 포함합니다. 다음은 몇 가지 샘플 레코드입니다.
...
- trip_fare CSV 파일은 지급 형식, 금액, 추가 요금 및 세금, 팁 및 통행료, 총 지불 금액 등 각 여정의 요금에 대한 세부 정보를 포함합니다. 다음은 몇 가지 샘플 레코드입니다.
...
필요한 프로세스 작업을 명확하게 하기 위해 데이터 분석을 기반으로 수행할 예측의 종류를 결정합니다. 다음은 이 연습에서 우리가 해결할 예측 문제에 대한 3가지 예제입니다. 예제의 공식은 *팁\_양* 을 기반으로 합니다.
...
여기서는 AzCopy를 사용하여 데이터가 포함된 파일을 전송하는 방법을 설명합니다. AzCopy를 다운로드하여 설치하려면 [AzCopy 명령줄 유틸리티 시작](../../storage/common/storage-use-azcopy-v10.md)의 지침을 따르세요.
...
1. 복사가 완료되면 총 24개의 압축된 파일이 선택한 데이터 폴더에 표시됩니다. 로컬 컴퓨터에서 동일한 디렉터리에 다운로드한 파일의 압축을 풉니다. 압축을 푼 파일이 있는 폴더를 적어 둡니다. 이 폴더는 다음에서 *\<path\_to\_unzipped_data\_files\>* 이라고 합니다.
...
* ***\<storage account name of Hadoop cluster>*** - HDInsight 클러스터와 연결된 스토리지 계정입니다.
...
* ***\<storage account key>*** - 클러스터에서 사용하는 스토리지 계정의 키입니다.
...
예비 데이터 분석을 위해 클러스터를 준비하려면 [GitHub](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts)에서 관련 Hive 스크립트가 포함된 '.hql' 파일을 헤드 노드의 로컬 디렉터리(C:\temp)에 다운로드합니다. 클러스터의 헤드 노드 내에서 명령 프롬프트를 열고 다음의 두 명령을 실행합니다.
...
Hive 디렉터리 프롬프트에서 Hive 데이터베이스 및 테이블을 생성하는 헤드 노드의 Hadoop 명령줄에서 다음 명령을 실행합니다.

```console
hive -f "C:\temp\sample_hive_create_db_and_tables.hql"
```

다음은 **C:\temp\sample\_hive\_create\_db\_및\_tables.hql** 파일의 컨텐츠로, Hive 데이터베이스 **nyctaxidb** 와 **trip** 및 **fare** 테이블을 만듭니다.
...
탐색 프로세스에서 사용된 여러 Hive 쿼리는 한 개나 두 개의 파티션 확인만을 포함합니다. 그러나 이러한 쿼리는 전체 데이터 세트에서 실행될 수 있습니다.
...
이전 쿼리를 약간 변경하여 편리하게 볼 수 있도록 레코드를 파일에 저장할 수 있습니다.
...
두 데이터 세트에 매월 정확히 동일한 수의 여정이 반환되어, 데이터가 올바르게 로드된 첫 번째 유효성 검사를 제공합니다.
...
두 테이블의 총 레코드 수도 동일하여 데이터가 올바르게 로드된 두 번째 유효성 검사를 제공합니다.
...
데이터 세트를 탐색할 때 값 그룹의 분포를 조사하고자 하는 경우가 많습니다. 이 섹션에서는 택시와 운전 기사에 대해 이 분석을 수행하는 방법에 대한 예제를 제공합니다.
...
이 명령에 포함된 *-S* 인수는 Hive 맵/감소 작업의 상태 화면 인쇄를 표시하지 않습니다. 이 명령이 유용한 이유는 Hive 쿼리 출력의 화면 인쇄를 좀 더 쉽게 읽을 수 있기 때문입니다.
...
다음 **sample\_hive\_tipped\_frequencies.hql** 파일은 실행할 명령을 보여줍니다.
...
쿼리는 **pickup\_datetime** 필드에서 다음 시간 매개 변수를 생성하도록 표준 Hive 함수를 직접 적용합니다.
- 하루 중 시간
- 연간 주
- 평일(‘1’은 월요일, ‘7’은 일요일을 의미함)
...
**HCatalog 서버 URI**: 클러스터 이름이 **abc123** 이면 https:\//abc123.azurehdinsight.net을 사용합니다.

**Hadoop user account name**: 클러스터에 대해 선택한 사용자 이름입니다(원격 액세스 사용자 이름이 아님).

**Hadoop 사용자 계정 암호**: 클러스터에 대해 선택한 암호입니다(원격 액세스 암호가 아님).

**출력 데이터 위치**: Azure로 선택됩니다.

**Azure Storage 계정 이름**: 클러스터와 연결된 기본 스토리지 계정의 이름입니다.

**Azure 컨테이너 이름**: 클러스터의 기본 컨테이너 이름이며, 일반적으로 클러스터 이름과 동일합니다. **abc123** 이라는 클러스터의 경우, 컨테이너 이름도 abc123입니다.
...
다운 샘플링된 데이터는 기본 컨테이너에 있으므로 Machine Learning의 Hive 쿼리 결과는 간단합니다. 간단히 **SELECT * FROM nyctaxidb.nyctaxi\_downsampled\_data** 입니다.
...
- **다중 클래스 분류**: 이전에 정의된 클래스를 사용하여 여정에 대해 지불된 팁 금액 범위를 예측합니다.
  ...
  **사용된 학습자:** 다중 클래스 로지스틱 회귀
  ...
  b. 이 실험에서는 혼동 행렬을 사용하여 다음과 같이 예측 정확도를 확인합니다.

  ![혼동 행렬](./media/hive-walkthrough/cxFmErM.png)

  많이 사용되는 클래스의 클래스 정확도는 좋지만, 드물게 사용되는 클래스에서는 모델의 "학습" 작업 성능이 좋지 않습니다.
...
이 샘플 연습 및 함께 제공되는 스크립트는 Microsoft에서 MIT 라이선스에 따라 공유하고 있습니다. 자세한 정보는 GitHub의 샘플 코드 디렉터리에 있는 **LICENSE.txt** 파일을 참조하세요.
