...
더 큰 데이터 집합을 처리하는 방법을 보여 주는 연습에서는 [1TB 데이터 집합에서 Azure HDInsight Hadoop 클러스터 사용 - 팀 데이터 과학 프로세스를](hive-criteo-walkthrough.md)참조하십시오.

IPython 노트북을 사용하여 1TB 데이터 집합을 사용하는 연습에 표시되는 작업을 수행할 수도 있습니다. 자세한 내용은 [Hive ODBC 연결을 사용하여 Criteo 연습](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/iPythonNotebooks/machine-Learning-data-science-process-hive-walkthrough-criteo.ipynb)을 참조하세요.
...
- trip_data CSV 파일에는 승객 수, 픽업 및 하차 지점, 여행 시간 및 여행 길이와 같은 여행 세부 정보가 포함되어 있습니다. 다음은 몇 가지 샘플 레코드입니다.
...
- trip_fare CSV 파일에는 결제 유형, 요금 금액, 추가 요금 및 세금, 팁 및 통행료, 지불한 총 금액 등 각 여행에 대해 지불한 요금에 대한 세부 정보가 포함되어 있습니다. 다음은 몇 가지 샘플 레코드입니다.
...
필요한 프로세스 작업을 명확히 하는 데 도움이 되도록 데이터 분석을 기반으로 수행할 예측의 종류를 결정합니다. 다음은 *팁\_양을*기준으로 이 연습에서 해결하는 예측 문제의 세 가지 예입니다.
...
1. [저장소 계정 만들기](../../storage/common/storage-account-create.md): 이 저장소 계정은 Azure Blob 저장소에 데이터를 저장하는 데 사용됩니다. HDInsight 클러스터에 사용되는 데이터도 여기에 상주합니다.
...
여기서는 AzCopy를 사용하여 데이터가 포함된 파일을 전송하는 방법을 설명합니다. AzCopy를 다운로드하여 설치하려면 [AzCopy 명령줄 유틸리티를 시작하기 의](../../storage/common/storage-use-azcopy.md)지침을 따릅니다.

1. 명령 프롬프트 창에서 다음 AzCopy 명령을 실행하여 * \<path_to_data_folder>* 원하는 대상으로 대체합니다.

        "C:\Program Files (x86)\Microsoft SDKs\Azure\AzCopy\azcopy" /Source:https://nyctaxitrips.blob.core.windows.net/data /Dest:<path_to_data_folder> /S

1. 복사가 완료되면 총 24개의 압축된 파일이 선택한 데이터 폴더에 표시됩니다. 로컬 컴퓨터에서 동일한 디렉터리에 다운로드한 파일의 압축을 풉니다. 압축을 푼 파일이 있는 폴더를 적어 둡니다. 이 폴더를 다음과 * \<같은\_\_\_파일unzipped_data\> 경로라고* 합니다.
...
* *** \<path_to_data_folder>*** 압축해제된 데이터 파일이 포함된 컴퓨터의 디렉터리(경로와 함께)입니다.  
* ***Hadoop 클러스터의 저장소 계정 이름>\<*** HDInsight 클러스터와 연결된 저장소 계정입니다.
* ***하눕 클러스터>기본 컨테이너 \<*** 클러스터에서 사용하는 기본 컨테이너입니다. 기본 컨테이너의 이름은 일반적으로 클러스터 자체와 동일한 이름입니다. 예를 들어 클러스터가 "abc123.azurehdinsight.net"인 경우 기본 컨테이너는 abc123입니다.
* 저장소 계정 키>*** \<*** 클러스터에서 사용하는 저장소 계정의 키입니다.
...
예비 데이터 분석을 위해 클러스터를 준비하려면 [GitHub의](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts) 관련 Hive 스크립트가 포함된 '.hql' 파일을 헤드 노드의 로컬 디렉터리(C:\temp)로 다운로드합니다. 클러스터의 헤드 노드 내에서 명령 프롬프트를 열고 다음 두 명령을 실행합니다.

    set script='https://raw.githubusercontent.com/Azure/Azure-MachineLearning-DataScience/master/Misc/DataScienceProcess/DataScienceScripts/Download_DataScience_Scripts.ps1'

    @powershell -NoProfile -ExecutionPolicy unrestricted -Command "iex ((new-object net.webclient).DownloadString(%script%))"

이 두 명령은 이 연습에서 필요한 모든 '.hql' 파일을 헤드 노드의 로컬 디렉터리 ***C:\temp&#92;*** 다운로드합니다.

## <a name="create-hive-database-and-tables-partitioned-by-month"></a><a name="#hive-db-tables"></a>월별로 분할된 Hive 데이터베이스 및 테이블 만들기
> [!NOTE]
> 이 작업은 일반적으로 관리자를 위한 것입니다.
> 
> 
...
Hive 디렉터리 프롬프트에서 하이브 데이터베이스 및 테이블을 만드는 헤드 노드의 Hadoop 명령줄에서 다음 명령을 실행합니다.

    hive -f "C:\temp\sample_hive_create_db_and_tables.hql"

다음은 Hive 데이터베이스 **nyctaxidb를**만드는 **C:\temp\sample 하이브\_\_생성\_db\_및\_tables.hql** 파일과 테이블 **여행** 및 **요금의**내용입니다.
...
* **여행** 테이블에는 각 라이드의 운행 세부 정보(드라이버 세부 정보, 픽업 시간, 이동 거리 및 시간)가 포함되어 있습니다.
* **운임** 표에는 운임 세부 정보(운임 금액, 팁 금액, 통행료 및 할증료)가 포함되어 있습니다.
...
**partitions.hql\_\_파일별\_샘플\_하이브\_로드 데이터에는** 다음과 같은 **LOAD** 명령이 포함되어 있습니다.

    LOAD DATA INPATH 'wasb:///nyctaxitripraw/trip_data_${hiveconf:MONTH}.csv' INTO TABLE nyctaxidb.trip PARTITION (month=${hiveconf:MONTH});
    LOAD DATA INPATH 'wasb:///nyctaxifareraw/trip_fare_${hiveconf:MONTH}.csv' INTO TABLE nyctaxidb.fare PARTITION (month=${hiveconf:MONTH});

탐색 프로세스에서 여기에 사용되는 여러 Hive 쿼리에는 하나 또는 두 개의 파티션만 보는 것이 포함됩니다. 그러나 이러한 쿼리는 전체 데이터 세트에서 실행될 수 있습니다.
...
이전 쿼리를 조금만 변경하여 편리하게 볼 수 있도록 레코드를 파일에 저장할 수 있습니다.
...
두 데이터 집합에 대해 매월 동일한 횟수의 트립이 반환되므로 데이터가 올바르게 로드되었는지 첫 번째 유효성 검사가 제공됩니다.
...
두 테이블의 총 레코드 수도 동일하므로 데이터가 올바르게 로드되었는지 두 번째 유효성 검사를 제공합니다.
...
이 예제에서는 지정된 기간 내의 여정이 100개가 넘는 medallion(택시 번호)을 식별합니다. 쿼리는 파티션 변수 **month**의 영향을 받기 때문에 테이블을 분할하면 쿼리 성능이 개선됩니다. 쿼리 결과는 헤드 노드의 로컬 파일인 **queryoutput.tsv에** `C:\temp` 기록됩니다.
...
> [!NOTE]
> 이 작업은 일반적으로 데이터 과학자를 위한 것입니다.
> 
> 

데이터 집합을 탐색할 때 값 그룹의 분포를 검사하는 경우가 자주 있습니다. 이 섹션에서는 택시 및 드라이버에 대해 이 분석을 수행하는 방법에 대한 예제를 제공합니다.
...
쿼리 결과는 로컬 **파일C:\temp\queryoutput.tsv에**기록됩니다.
...
이 명령에 포함된 *-S* 인수는 Hive 맵/감소 작업의 상태 화면 인쇄를 표시하지 않습니다. 이 명령은 Hive 쿼리 출력의 화면 인쇄를 더 읽기 쉽게 하기 때문에 유용합니다.
...
다음 **샘플\_하이브\_\_팁 주파수.hql** 파일실행 명령을 보여 주었습니다.
...
앞의 쿼리에서 R은 지구의 반경(마일)이고, pi는 라디안으로 변환됩니다. 경도 위도 점은 NYC 영역에서 멀리 떨어진 값을 제거하기 위해 필터링됩니다.
...
이 절차에는 두 단계가 있습니다. 먼저 우리는 모든 기록에 존재하는 세 가지 키에 **nyctaxidb.trip** 및 **nyctaxidb.fare** 테이블에 가입 : **메달리온,** **해킹\_라이센스,** **픽업\_날짜 시간**. 그런 다음, 이진 분류 레이블 **tipped**와 다중 클래스 분류 레이블 **tip\_class**를 생성합니다.
...
쿼리는 표준 Hive 함수를 직접 적용하여 **픽업\_날짜 시간** 필드에서 다음 시간 매개 변수를 생성합니다.
- 하루 중 시간
- 연간 주
- 평일('1'은 월요일을 의미하고, '7'은 일요일을 의미합니다).
...
**HCatalog 서버 URI**: 클러스터 이름이 **abc123인**경우\/https: /abc123.azurehdinsight.net 사용합니다.

**Hadoop 사용자 계정 이름**: 클러스터에 대해 선택한 사용자 이름(원격 액세스 사용자 이름이 아님)입니다.

**Hadoop 사용자 계정 암호**: 클러스터에 대해 선택한 암호(원격 액세스 암호가 아님)입니다.

**출력 데이터의 위치**: Azure로 선택되었습니다.

**Azure Storage 계정 이름**: 클러스터와 연결된 기본 저장소 계정의 이름입니다.

**Azure 컨테이너 이름:** 클러스터의 기본 컨테이너 이름이며 일반적으로 클러스터 이름과 동일합니다. **abc123이라는**클러스터의 경우 이름은 abc123입니다.
...
다운 샘플링된 데이터는 기본 컨테이너에 있으므로 기계 학습의 결과 Hive 쿼리는 간단합니다. 간단히 **SELECT * FROM nyctaxidb.nyctaxi\_downsampled\_data**입니다.
...
- **이진 분류**: 팁이 여행에 대한 지불 되었는지 여부를 예측합니다.

  **사용된 학습자:** 2클래스 로지스틱 회귀

  a. 이 문제의 경우 대상(또는 클래스) 레이블은 **tipped**입니다. 다운 샘플링된 원래 데이터 세트에는 이 분류 실험의 대상 누수인 몇 가지 열이 있습니다. 특히 **팁\_클래스,** **팁\_금액**및 총 **\_금액은** 테스트 시 사용할 수 없는 대상 레이블에 대한 정보를 표시합니다. [데이터 세트의 열 선택][select-columns] 모듈을 사용하여 이러한 열을 고려 대상에서 제거합니다.
...
- **다중 클래스 분류**: 이전에 정의된 클래스를 사용하여 여정에 대해 지불된 팁 금액 범위를 예측합니다.
  ...
  **사용된 학습자:** 다중 클래스 로지스틱 회귀
  ...
  a. 이 문제에 대 한 대상 (또는 클래스) 레이블은 **\_팁 클래스,** 5 개의 값 중 하나를 걸릴 수 있습니다 (0,1,2,3,4). 이진 분류와 마찬가지로 이 실험에 대한 대상 누수인 몇 개 열이 있습니다. 특히 **tipped**, **tip\_amount** 및 **total\_amount**는 테스트 시 사용할 수 없는 대상 레이블에 대한 정보를 표시합니다. [데이터 세트의 열 선택][select-columns] 모듈을 사용하여 이러한 열을 제거합니다.
  ...
  b. 이 실험에서는 혼동 행렬을 사용하여 다음과 같이 예측 정확도를 살펴봅니다.

  ![혼동 행렬](./media/hive-walkthrough/cxFmErM.png)

  널리 사용되는 클래스의 클래스 정확도는 좋지만 모델은 드문 클래스에서 "학습"을 잘하지 않습니다.

- **회귀 작업**: 여행에 대해 지불 한 팁의 양을 예측합니다.
...
이 샘플 연습 및 함께 제공되는 스크립트는 Microsoft에서 MIT 라이선스에 따라 공유하고 있습니다. 자세한 내용은 GitHub의 샘플 코드 디렉토리에 있는 **LICENSE.txt** 파일을 참조하십시오.
