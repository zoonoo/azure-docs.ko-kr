...
description: Azure의 SQL Server 가상 머신에서 Python 또는 SQL Server를 사용하여 데이터를 탐색 및 처리하고 기능을 생성합니다.
...
ms.locfileid: "93314632"
---
# <a name="process-data-in-sql-server-virtual-machine-on-azure"></a><a name="heading"></a>Azure의 SQL Server Virtual Machine에서 데이터 처리
이 문서에서는 Azure의 SQL Server VM에 저장된 데이터를 탐색하고 데이터에 대한 기능을 생성하는 방법에 대해 알아봅니다. 이렇게 하려면 SQL을 사용하여 데이터 랭글링을 수행하거나 Python과 같은 프로그래밍 언어를 사용하여 이 목표를 완료할 수 있습니다.
...
다음의 예제에서는 개수 기능을 생성하는 두 가지 방법을 보여 줍니다. 첫 번째 방법에서는 조건부 합계를 사용하고, 두 번째 방법에서는 'where' 절을 사용합니다. 이러한 결과를 원래 데이터와 함께 개수 기능을 유지하도록 원래 테이블에 조인할 수 있습니다(기본 키 열 사용).
...
다음은 위도/경도 위치 데이터에 대한 간략한 기초 정보입니다(stackoverflow [위도 및 경도의 정확도를 측정하는 방법](https://gis.stackexchange.com/questions/8650/how-to-measure-the-accuracy-of-latitude-and-longitude)에서 발췌). 위치를 하나 이상의 기능으로 포함하기 전에 이 지침을 이해하면 데 유용합니다.
...
* 단위 자리수(하나의 도 단위)는 최대 111km(60해리, 약 69마일)까지의 위치를 제공합니다. 현재 사용자가 있는 주, 국가 또는 지역을 대략적으로 알려줄 수 있습니다.
...
위치 정보는 다음과 같이 지역, 위치 및 도시 정보를 구분하여 기능화할 수 있습니다. 또한 Bing Maps API([지점별 위치 찾기](/bingmaps/rest-services/locations/find-a-location-by-point))와 같은 REST 끝점을 호출하여 지역/구역 정보를 가져올 수 있습니다.
...
데이터가 SQL Server에 있는 경우 Python을 사용하여 데이터를 탐색하고 기능을 생성하는 작업은 [데이터 과학 환경에서 Azure Blob 데이터 처리](data-blob.md)에 설명된 대로 Python을 사용하여 Azure Blob의 데이터를 처리하는 것과 유사합니다. 추가로 처리하기 위해 데이터베이스의 데이터를 Pandas 데이터 프레임으로 로드합니다. 데이터베이스에 연결하여 데이터 프레임으로 데이터를 로드하는 프로세스는 이 섹션에 설명되어 있습니다.
