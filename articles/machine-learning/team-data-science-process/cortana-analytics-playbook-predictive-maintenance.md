...
- 다음에서 비즈니스는 _예방 유지 관리_를 사용합니다. 여기서는 부품에 유용한 수명을 결정하고 오류가 발생하기 전에 해당 부품을 유지 관리하거나 교체합니다. 예방 유지 관리는 예약되지 않은 치명적인 오류를 방지합니다. 하지만 예약 된 가동 중지 시간을 최소화 하 고, 해당 수명 동안 구성 요소를 사용 하는 데 드는 비용을 증가 시킬 수 있습니다.
...
|**에너지** |                          |
|_바람 터빈 실패_: 바람 터빈는 environmentally 담당 국가/지역에 있는 주요 에너지 원본 이며, 높은 자본 비용을 포함 합니다. 바람 터빈의 핵심 구성 요소는 실패 한 터빈을 렌더링 하는 생성기 모터입니다. 문제를 해결하는 데 비용도 많이 듭니다.|MTTF(오류 발생 시)와 같은 KPI를 예측하면 에너지 회사에서 터빈 오류를 방지하고 최소한의 가동 중지 시간을 확보할 수 있습니다. 오류 확률은 곧 실패할 수 있는 터빈을 모니터링하도록 기술 지원 담당자에게 알리고, 시간 기반 유지 관리 체계를 예약합니다. 예측 모델은 오류를 발생시키는 여러 요인에 대한 정보를 제공하며 이를 통해 기술 지원 담당자가 문제의 근본 원인을 더 잘 이해할 수 있습니다.|
...
자산의 유지 관리 기록에는 교체 된 구성 요소, 수행 된 복구 작업 등에 대 한 세부 정보가 포함 됩니다. 이러한 이벤트는 성능 저하 패턴을 기록 합니다. 학습 데이터에서 중요한 정보가 없는 경우 잘못된 모델 결과가 발생할 수 있습니다. 또한 오류 기록은 특수한 오류 번호 또는 부품의 주문 날짜와 같은 유지 관리 기록 내에서 찾을 수 있습니다. 오류 패턴에 영향을 주는 추가 데이터 원본은 도메인 전문가에 의해 조사되고 제공되어야 합니다.
...
| 사용 사례 | 관련 데이터의 예제 |
|:---------|---------------------------|
|_비행 지연 및 취소_ | 비행 지연 및 페이지 로그의 형식인 비행 경로 정보입니다. 비행 레그 데이터에는 출발/도착 날짜, 시간, 공항, layovers 등의 라우팅 정보가 포함 됩니다. 페이지 로그에는 접지 유지 관리 담당자가 기록한 일련의 오류 및 유지 관리 코드가 포함 됩니다.|
|_비행기 엔진 부품 오류_ | 다양한 부품의 상태에 대한 정보를 제공하는 비행기의 센서에서 수집된 데이터입니다. 유지 관리 레코드를 통해 구성 요소 오류가 발생한 시기 및 교체된 시기를 식별할 수 있습니다.|
|_ATM 오류_ | 각 트랜잭션(현금/수표 예금) 및 현금 인출에 대한 센서 판독값입니다. 메모, 메모 두께, 메모 도착 거리, 특성 확인 등 간의 간격 측정에 대 한 정보입니다. 오류 코드, 복구 정보를 제공 하는 유지 관리 레코드, 현금 디스펜서가 마지막으로 리필 된 시간|
|_풍력 터빈 오류_ | 센서는 온도, 바람 방향, 전력 생성, 생성기 속도 등의 터빈 상태를 모니터링 합니다. 데이터는 다양 한 지역에 위치한 윈드 팜에서 여러 바람 터빈에서 수집 됩니다. 일반적으로 각 터빈은 여러 센서가 고정된 시간 간격으로 릴레이 측정값을 읽도록 합니다.|
|_회로 차단기 오류_ | 수정, 예방 및 체계적 작업을 포함하는 유지 관리 로그입니다. 열기 및 닫기 작업 등 회로 차단기에 전송된 자동 및 수동 명령이 포함된 운영 데이터입니다. 제조업체, 위치, 모델 등의 날짜와 같은 장치 메타 데이터 전압 수준, 지리적 위치, 주변 상태와 같은 회로 차단기 사양입니다.|
|_엘리베이터 도어 오류_| 엘리베이터 종류, 제조 날짜, 유지 관리 빈도, 건물 종류 등 엘리베이터 메타데이터입니다. 도어 주기의 수, 평균 도어 닫기 시간 등 운영 정보입니다. 원인을 포함한 오류 기록입니다.|
|_휠 오류_ | 휠 가속, 브레이크 인스턴스, 거리, 속도 등을 측정 하는 센서 데이터입니다. 제조업체, 제조 날짜와 같은 휠에 대 한 정적 정보입니다. 주문 날짜 및 수량을 추적하는 부품 주문 데이터베이스에서 유추된 오류 데이터입니다.|
...
자산의 레이블이 지정 된 각 레코드에 대해 크기 _w-<sub>k</sub>_  의 창이 정의 됩니다. 여기서 _k_ 는 _w_크기의 창 수입니다. 그런 다음 집계는 _k_ _연속 windows_ _w-k, w-<sub>(k-1)</sub>, ..., w-<sub>2</sub>, w-<sub>1</sub> (_ 레코드 타임 스탬프 이전의 기간)에 대해 생성 됩니다. _k_는 단기 효과를 캡처할 만큼 작은 수나 장기 성능 저하 패턴을 캡처할 만큼 큰 수일 수 있습니다. (그림 2 참조).
...
이진 분류를 사용 하 여 향후의 기간 _X_라고 _하는 향후 기간 내에 장비의 일부가 실패할 확률을 예측 합니다_ . X는 도메인 전문가에 게 문의 하 여 비즈니스 문제 및 데이터에 의해 결정 됩니다. 예:
...
- _비행 지연_: 다음 24 시간 동안의 지연을 예측 하기 위해 X를 1 일로 선택할 수 있습니다. 그런 다음, 오류가 발생하기 전 24시간 이내의 모든 비행은 1로 레이블 지정됩니다.
...
![그림 5. 다중 클래스 분류에 대 한 오류 시간 예측 레이블](./media/cortana-analytics-playbook-predictive-maintenance/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
![그림 6. 다중 클래스 분류를 위한 근본 원인 예측 레이블](./media/cortana-analytics-playbook-predictive-maintenance/labelling-for-multiclass-classification-for-root-cause-prediction.png)
...
시간 종속 분할의 경우 T_c<sub>까지 기록 데이터를 사용하여 조정된 하이퍼 매개 변수로 모델을 학습할 </sub>학습 구분 시간 T_c<sub></sub>을 선택합니다. 학습 데이터에 대한 T<sub>c</sub> 이후 향후 레이블의 누출을 방지하기 위해 학습 예제의 레이블이 T<sub>c</sub> 이전의 X 단위가 되도록 최근 시간을 선택합니다. 그림 7에 표시된 예제에서 각 칸은 위에서 설명한 대로 기능 및 레이블이 계산되는 데이터 집합의 레코드를 나타냅니다. 그림은 X=2 및 W=3에 대한 학습 및 테스트 집합으로 이동해야 하는 레코드를 보여줍니다.
...
| [Microsoft AI YouTube 웹 세미나](https://www.youtube.com/watch?v=NvrH7_KKzoM&t=4s) | 공용 |
