...
| <b>소스</b> | <b>대상: Azure VM의 SQL Server</b> |
| --- | --- |
| <b>플랫 파일</b> |1. <a href="#insert-tables-bcp">명령줄 대량 복사 유틸리티(BCP)</a><br> 2. <a href="#insert-tables-bulkquery">대량 삽입 SQL 쿼리 </a><br> 3. <a href="#sql-builtin-utilities">SQL Server의 그래픽 내장 유틸리티</a> |
| <b>온-프레미스 SQL Server</b> |1. <a href="#deploy-a-sql-server-database-to-a-microsoft-azure-vm-wizard">Microsoft Azure VM 마법사에 SQL 서버 데이터베이스 배포</a><br> 2. <a href="#export-flat-file">플랫 파일로 내보내기</a><br> 3. <a href="#sql-migration">SQL Database 마이그레이션 마법사 </a> <br> 4. <a href="#sql-backup">데이터베이스 백업 및 복원 </a><br> |

이 문서에서는 SQL 명령이 SQL Server 관리 스튜디오 또는 Visual Studio 데이터베이스 탐색기에서 실행된다고 가정합니다.
...
1. [명령줄 대량 복사 유틸리티(BCP)](#insert-tables-bcp)
...
BCP는 SQL Server와 함께 설치되는 명령줄 유틸리티로, 데이터를 이동하는 가장 빠른 방법 중 하나입니다. 세 가지 SQL Server 변형(온-프레미스 SQL Server, SQL Azure 및 Azure의 SQL Server VM)에서 모두 작동합니다.
...
1. bcp가 설치된 컴퓨터의 명령줄에서 다음 명령을 실행하여 테이블의 스키마를 설명하는 형식 파일을 생성합니다.

    `bcp dbname..tablename format nul -c -x -f exportformatfilename.xml -S servername\sqlinstance -T -t \t -r \n`
1. SQL Server가 동일한 컴퓨터에 설치될 때 명령줄에서 작동해야 하는 bcp 명령을 사용하여 데이터베이스에 데이터를 삽입합니다.
...
이동하는 데이터가 큰 경우 PowerShell 스크립트에서 여러 BCP 명령을 동시에 실행하여 작업 속도를 높일 수 있습니다.
...
SSIS(SQL Server 통합 서비스)를 사용하여 플랫 파일에서 Azure의 SQL Server VM으로 데이터를 가져올 수 있습니다.
