---
title: Spark 풀로 데이터 준비 (미리 보기)
titleSuffix: Azure Machine Learning
description: Azure Synapse 및 Azure Machine Learning를 사용 하 여 데이터 준비를 위해 Spark 풀을 연결 하는 방법을 알아봅니다.
...
ms.locfileid: "101662892"
---
# <a name="attach-synapse-spark-pools-for-data-preparation-with-azure-synapse-preview"></a>Azure Synapse (미리 보기)를 사용 하 여 데이터 준비를 위해 Synapse Spark 풀 연결

이 문서에서는 데이터 준비를 위해 [Azure Synapse](/synapse-analytics/overview-what-is.md) 에서 지 원하는 Apache Spark 풀을 연결 하 고 시작 하는 방법에 대해 알아봅니다. 

>[!IMPORTANT]
> Azure Machine Learning 및 Azure Synapse 통합은 미리 보기 상태입니다. 이 문서에 제공 된 기능에서는 언제 `azureml-synapse` 든 지 변경 될 수 있는 [실험적](/python/api/overview/azure/ml/?preserve-view=true&view=azure-ml-py#stable-vs-experimental) 미리 보기 기능을 포함 하는 패키지를 사용 합니다.

## <a name="azure-machine-learning-and-azure-synapse-integration-preview"></a>Azure Machine Learning 및 Azure Synapse 통합 (미리 보기)

Azure Synapse와 Azure Machine Learning (미리 보기)를 통합 하면 Azure Synapse에서 지원 되는 Apache Spark 풀을 연결 하 여 대화형 데이터 탐색 및 준비를 수행할 수 있습니다. 이 통합을 사용 하면 기계 학습 모델을 학습 하는 데 사용 하는 것과 동일한 Python 노트북 내에서 대규모로 데이터 준비를 위한 전용 계산을 수행할 수 있습니다.
...
## <a name="link-machine-learning-workspace-and-synapse-assets"></a>Machine learning 작업 영역 및 Synapse 자산 연결

데이터 준비를 위해 Synapse Spark 풀을 연결 하려면 먼저 Azure Machine Learning 작업 영역을 Azure Synapse 작업 영역에 연결 해야 합니다. 
...
>
> Synapse 작업 영역의 **소유자** 가 아니지만 기존 연결 된 서비스를 사용 하려는 경우 [기존 연결 된 서비스 가져오기](#get-an-existing-linked-service)를 참조 하세요.
...
1. **링크 작업 영역** 폼에서 필드를 채웁니다.
   ...
   |필드| 설명    
   |---|---
   |속성| 연결 된 서비스의 이름을 제공 합니다. 이 이름은이 특정 연결 된 서비스를 참조 하는 데 사용 됩니다.
...
기존 연결 된 서비스를 검색 하 고 사용 하려면 Synapse 작업 영역에 대 한 **사용자 또는 참가자** 권한이 필요 합니다.
...
작업 영역이 연결 되 면 데이터 준비 태스크에 대 한 전용 계산 리소스로 Synapse Spark 풀을 연결 합니다. 

다음을 통해 Synapse Spark 풀을 연결할 수 있습니다.
...
Studio를 사용 하 여 Synapse Spark 풀을 연결 하려면 다음 단계를 수행 합니다. 
...
1. **연결** 을 선택합니다. 
1. 목록에서 Synapse Spark 풀을 선택 하 고 이름을 입력 합니다.  
...
**PYTHON SDK** 를 사용 하 여 Synapse Spark 풀을 연결할 수도 있습니다. 
...
1. 다음을 사용 하 여 SynapseCompute를 구성 합니다.
   ...
   1. Synapse Spark 풀의 이름입니다. 이는 Synapse 작업 영역에 있는 기존 Apache Spark 풀과 일치 해야 합니다.
   
1. 를 전달 하 여 machine learning ComputeTarget를 만듭니다. 
   1. 사용 하려는 machine learning 작업 영역 `ws`
   1. Machine learning 작업 영역 내에서 계산을 참조 하고자 하는 이름입니다. 
   1. SynapseCompute를 구성할 때 지정한 attach_configuration입니다.
...
Synapse Spark 풀이 연결 되어 있는지 확인 합니다.
...
Synapse 세션 중에 사용할 [Azure Machine Learning 환경을](concept-environments.md) 지정할 수 있습니다. 환경에 지정 된 Conda 종속성만 적용 됩니다. Docker 이미지는 지원 되지 않습니다.

>[!WARNING]
>  환경 Conda 종속성에 지정 된 Python 종속성은 Synapse Spark 풀에서 지원 되지 않습니다. 현재는 고정 Python 버전만 지원 됩니다. 스크립트에를 포함 하 여 Python 버전을 확인  `sys.version_info` 합니다.

다음 코드에서는 `myenv` `azureml-core` 세션이 시작 되기 전에 버전 1.20.0 및 버전 1.17.0을 설치 하는 환경을 만듭니다 `numpy` . 그런 다음 Synapse session 문에이 환경을 포함할 수 있습니다 `start` .
...
Synapse Spark 풀을 사용 하 여 데이터 준비를 시작 하려면 Synapse Spark 풀 이름을 지정 하 고 구독 ID, machine learning 작업 영역 리소스 그룹, machine learning 작업 영역 이름 및 Synapse 세션 중에 사용할 환경을 제공 합니다. 

> [!IMPORTANT]
> Synapse Spark 풀을 계속 사용 하려면 `%synapse` 단일 코드 줄 및 여러 줄에 대 한 데이터 준비 작업 전체에서 사용할 계산 리소스를 지정 해야 합니다 `%%synapse` . 
...
Synapse Spark 세션이 시작 되 면 준비 하려는 데이터를 읽습니다. 데이터 로드는 Azure Blob storage 및 Azure Data Lake Storage 세대 1 및 2에 대해 지원 됩니다.
...
데이터 준비를 완료 하 고 준비 된 데이터를 저장소에 저장 한 경우 다음 명령을 사용 하 여 Synapse Spark 풀 사용을 중지 합니다.
...
Azure Synapse 및 Azure Machine Learning를 사용 하 여 단일 노트북에서 데이터 준비 및 모델 학습을 수행 하는 방법에 대 한 자세한 코드 예제는이 [종단 간 노트북](../synapse-analytics/overview-what-is.md) 을 참조 하세요.
