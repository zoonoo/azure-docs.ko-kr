...
4.	[모델 학습][train-model] 모듈을 선택합니다. **속성** 창에서 **Launch column selector**(열 선택기 시작)를 클릭하고, 첫 번째 드롭다운에서 **Include**(포함)를 선택하고, 두 번째 드롭다운에서 **column indices**(열 인덱스)를 선택하고, 텍스트 필드에 "21"을 입력합니다(**column names**(열 이름)를 선택하고 "신용 위험"을 입력할 수도 있음). 이는 모델에서 예측할 열로서 열 21, 신용 위험 값을 나타냅니다.
