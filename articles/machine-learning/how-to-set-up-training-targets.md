...
* [로컬 컴퓨터](#local)
* [Azure Machine Learning 계산](#amlcompute)
* [원격 가상 머신](#vm)
...
Azure Machine Learning 컴퓨팅은 사용자가 단일 또는 다중 노드 컴퓨팅을 손쉽게 만들 수 있는 관리형 컴퓨팅 인프라입니다. 작업 영역 지역 내에서 컴퓨팅은 작업 영역에서 다른 사용자와 공유할 수 있는 리소스로 만들어집니다. 작업이 제출될 때 컴퓨팅이 자동으로 확장되어 Azure Virtual Network에 배치될 수 있습니다. 계산은 컨테이너 화 된 환경에서 실행 되 고 [Docker 컨테이너](https://www.docker.com/why-docker)에 모델 종속성을 패키지 합니다.
...
1. **연결**: 기존 가상 컴퓨터를 계산 대상으로 연결 하려면 가상 컴퓨터에 대 한 리소스 ID, 사용자 이름 및 암호를 제공 해야 합니다. VM의 리소스 ID는 다음 문자열 형식을 사용 하 여 구독 ID, 리소스 그룹 이름 및 VM 이름을 사용 하 여 생성할 수 있습니다.`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.Compute/virtualMachines/<vm_name>`
...
1. **연결**: hdinsight 클러스터를 계산 대상으로 연결 하려면 hdinsight 클러스터에 대 한 리소스 ID, 사용자 이름 및 암호를 제공 해야 합니다. 다음 문자열 형식을 사용 하 여 구독 ID, 리소스 그룹 이름 및 HDInsight 클러스터 이름을 사용 하 여 HDInsight 클러스터의 리소스 ID를 생성할 수 있습니다.`/subscriptions/<subscription_id>/resourceGroups/<resource_group>/providers/Microsoft.HDInsight/clusters/<cluster_name>`
...
1. 더하기 기호(+)를 선택하여 컴퓨팅 대상을 추가합니다. 
1. 컴퓨팅 대상의 이름을 입력합니다. 
1. __학습__을 위해 연결할 계산 유형을 선택 하세요.
...
## <a name="submit-training-run-using-azure-machine-learning-sdk"></a><a id="submit"></a>Azure Machine Learning SDK를 사용 하 여 학습 실행 제출
