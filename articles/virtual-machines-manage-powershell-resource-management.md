<properties
   pageTitle="Azure PowerShell에서 리소스 관리자 모드를 사용하는 일반적인 VM 작업"
   description="Azure PowerShell에서 리소스 관리자 모드를 사용하는 일반적인 VM 작업에 대한 명령을 제공합니다."
...
# 리소스 관리자 모드에서 Azure PowerShell을 사용하는 일반적인 VM 작업
...
Azure 리소스 관리자 모드에서 Azure PowerShell cmdlet을 사용하면 매일 VM을 관리하기 위해 수행하는 많은 작업을 자동화할 수 있습니다. 이 문서에서는 더 간단한 작업에 대한 예제 명령과 보다 복잡한 작업에 대한 명령을 보여 주는 문서에 대한 링크를 제공합니다.
...
리소스 그룹 이름 "MyRG"와 VM 이름 "MyVM"을 사용하여 명령을 실행하는 경우 다음과 같이 표시됩니다.
