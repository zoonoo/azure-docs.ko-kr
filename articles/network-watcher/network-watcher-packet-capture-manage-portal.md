...
3. **추가**를 선택하여 패킷 캡처를 만듭니다. 다음 속성의 값을 선택할 수 있습니다.
   - **구독**: 패킷 캡처를 만들려는 가상 머신이 있는 구독입니다.
   - **리소스 그룹**: 가상 컴퓨터의 리소스 그룹입니다.
   - **대상 가상 머신**: 패킷 캡처를 만드는 데 사용할 가상 컴퓨터입니다.
   - **패킷 캡처 이름**: 패킷 캡처에 대 한 이름입니다.
   - **저장소 계정 또는 파일**: **저장소 계정**, **파일**또는 둘 다를 선택 합니다. **파일**을 선택하면 캡처가 가상 머신 내의 경로에 기록됩니다.
   - **로컬 파일 경로**: 패킷 캡처가 저장 될 가상 컴퓨터의 로컬 경로입니다 ( *파일* 을 선택한 경우에만 유효). 경로는 유효한 경로여야 합니다. Linux 가상 머신을 사용하는 경우 경로는 */var/captures*로 시작해야 합니다.
   - **Storage 계정**: *저장소 계정을*선택한 경우 기존 저장소 계정을 선택 합니다. **Storage**를 선택한 경우에만 이 옵션을 사용할 수 있습니다.
   
     > [!NOTE]
     > Premium Storage 계정에서는 패킷 캡처 저장이 현재 지원되지 않습니다.

   - **패킷 당 최대 바이트**: 캡처된 각 패킷의 바이트 수입니다. 비워 두면 모든 바이트가 캡처됩니다.
   - **세션당 최대 바이트**수: 캡처된 총 바이트 수입니다. 값에 도달하면 패킷 캡처가 중지됩니다.
   - **시간 제한 (초)** : 패킷 캡처가 중지 되기 전의 시간 제한입니다. 기본값은 18,000초입니다.
   - 필터링(선택 사항). **+ 필터 추가**를 선택합니다.
     - **프로토콜**: 패킷 캡처에 대해 필터링할 프로토콜입니다. 사용 가능한 값은 TCP, UDP 및 모두입니다.
     - **로컬 IP 주소**: 로컬 IP 주소가이 값과 일치 하는 패킷에 대 한 패킷 캡처를 필터링 합니다.
     - **로컬 포트**: 로컬 포트가이 값과 일치 하는 패킷에 대 한 패킷 캡처를 필터링 합니다.
     - **원격 IP 주소**: 원격 IP 주소가이 값과 일치 하는 패킷에 대 한 패킷 캡처를 필터링 합니다.
     - **원격 포트**: 원격 포트가이 값과 일치 하는 패킷에 대 한 패킷 캡처를 필터링 합니다.
