...
이 문서에서 단계를 수행 하려면 [Mac, Linux 및 Windows (Azure CLI)에 대 한 Azure 명령줄 인터페이스를 설치](/cli/azure/install-azure-cli)해야 합니다.
...
`az vm extension set` 명령을 실행하여 게스트 가상 컴퓨터에 패킷 캡처 에이전트를 설치합니다.
...
에이전트가 설치되었는지 확인하려면 `vm extension show` 명령을 실행하고 리소스 그룹 및 가상 시스템 이름을 전달합니다. 결과 목록을 확인하여 에이전트가 설치되어 있는지 확인합니다.
...
이 시점에서 패킷 캡처를 만들 준비가 되었습니다.  먼저 구성할 수 있는 매개 변수를 살펴보겠습니다. 필터는 패킷 캡처에 의해 저장되는 데이터를 제한하는 데 사용할 수 있는 하나의 매개 변수입니다. 다음 예제에서는 몇 가지 필터를 사용하여 패킷 캡처를 설정합니다.  처음 세 개의 필터는 로컬 IP 10.0.0.3에서 대상 포트 20, 80 및 443으로 나가는 TCP 트래픽을 수집합니다.  마지막 필터는 UDP 트래픽만을 수집합니다.

```azurecli-interactive
az network watcher packet-capture create --resource-group {resourceGroupName} --vm {vmName} --name packetCaptureName --storage-account {storageAccountName} --filters "[{\"protocol\":\"TCP\", \"remoteIPAddress\":\"1.1.1.1-255.255.255\",\"localIPAddress\":\"10.0.0.3\", \"remotePort\":\"20\"},{\"protocol\":\"TCP\", \"remoteIPAddress\":\"1.1.1.1-255.255.255\",\"localIPAddress\":\"10.0.0.3\", \"remotePort\":\"80\"},{\"protocol\":\"TCP\", \"remoteIPAddress\":\"1.1.1.1-255.255.255\",\"localIPAddress\":\"10.0.0.3\", \"remotePort\":\"443\"},{\"protocol\":\"UDP\"}]"
```

다음 예제는 명령을 실행할 때 `az network watcher packet-capture create` 예상되는 출력입니다.
...
`az network watcher packet-capture show-status` 명령을 실행하면 현재 실행 중이거나 완료된 패킷 캡처의 상태를 검색합니다.

```azurecli-interactive
az network watcher packet-capture show-status --name packetCaptureName --location {networkWatcherLocation}
```

다음 예제는 명령의 `az network watcher packet-capture show-status` 출력입니다. 다음 예제는 TimeExceeded StopReason으로 인해 캡처가 중지된 경우입니다.
...
`az network watcher packet-capture stop` 명령을 실행하면 캡처 세션이 진행 중이면 중지됩니다.
...
> [!NOTE]
> 현재 실행 중인 캡처 세션 또는 이미 중지된 기존 세션에서 실행할 때 명령은 응답을 반환하지 않습니다.
