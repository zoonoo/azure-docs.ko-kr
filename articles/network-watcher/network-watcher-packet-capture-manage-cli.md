...
에이전트가 설치 되어 있는지 확인 하려면 `vm extension show` 명령을 실행 하 여 리소스 그룹 및 가상 컴퓨터 이름으로 전달 합니다. 결과 목록을 확인하여 에이전트가 설치되어 있는지 확인합니다.
...
다음 예는 `az network watcher packet-capture create` 명령 실행의 예상 출력입니다.
