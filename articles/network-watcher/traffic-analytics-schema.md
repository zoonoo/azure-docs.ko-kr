...
전혀 표시 하는 데 도움이 됩니다. 온-프레미스에서 지난 30 일 동안의 로그를 흐름 아래 쿼리 합니다.
```
AzureNetworkAnalytics_CL
| where SubType_s == "FlowLog" and FlowStartTime_t >= ago(30d) and FlowType_s == "ExternalPublic"
| project Subnet_s  
```
위에 언급 한 쿼리를 흐름에 대 한 blob 경로 확인 하려면 아래 쿼리를 사용 합니다.
...
위의 쿼리는 blob에 직접 액세스에 대 한 URL을 생성 합니다. 자리 표시자 URL은 다음과 같습니다.
...
| SubType_s | FlowLog | 흐름 로그에 대 한 하위 형식입니다. "FlowLog"만 사용 되는 제품의 내부 작업에 대 한 SubType_s의 다른 값은 |
