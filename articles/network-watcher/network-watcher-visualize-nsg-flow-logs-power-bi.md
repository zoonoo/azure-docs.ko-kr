...
description: Azure Network Watcher에서 IP 트래픽에 대한 정보를 볼 수 있도록 Power BI를 사용하여 네트워크 보안 그룹 흐름 로그를 시각화하는 방법을 알아봅니다.
...
다음 시나리오에서는 NSG 흐름 로깅 데이터에 대한 싱크로 구성한 스토리지 계정에 Power BI 데스크톱을 연결합니다. 스토리지 계정에 연결되면 Power BI에서는 로그를 다운로드하고 구분 분석하여 네트워크 보안 그룹에서 기록한 트래픽의 시각적 표현을 제공합니다.
...
1. Power BI Desktop 애플리케이션 [Network Watcher PowerBI 흐름 로그 템플릿](https://aka.ms/networkwatcherflowlogspowerbitemplate)에서 다음 Power BI 템플릿을 다운로드하여 열기
1. 필요한 쿼리 매개 변수 입력
