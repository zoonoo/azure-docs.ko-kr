...
4. **스토리지 계정** 을 선택한 다음, 진단 정보를 기록할 스토리지 계정을 선택합니다.
5. **스토리지 계정** 목록에서 사용할 스토리지 계정을 선택합니다. 기존 스토리지 계정이 없는 경우 **+ 스토리지 계정** 을 선택하고, 필요한 정보를 입력하거나 선택하고, **만들기** 를 선택하여 스토리지 계정을 만듭니다. [필수 구성 요소](#prerequisites)의 스크립트를 사용하여 VPN 게이트웨이를 만든 경우 게이트웨이와 동일한 리소스 그룹 *TestRG1* 에 스토리지 계정을 만듭니다.
