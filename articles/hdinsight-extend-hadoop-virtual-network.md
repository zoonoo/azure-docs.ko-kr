<properties
   pageTitle="HDInsight에 Azure 가상 네트워크 확장 | VNet" metaKeywords="virtual network, vnet, azure, hdinsight" description="Azure 가상 네트워크를 사용하여 HDInsight 다른 클라우드 리소스 또는 데이터 센터에서 리소스에 연결하는 방법에 알아봅니다."
   services="hdinsight"
   documentationCenter=""
   authors="blackmist"
   manager="paulettm"
...
* 개인 네트워크(클라우드 전용)에서 클라우드 리소스 연결
	...
	가상 네트워크를 사용하여 HDInsight와 함께 Azure 서비스를 연결하는 다음 시나리오를 사용합니다.

	* Azure 웹사이트 또는 Azure 가상 컴퓨터에서 실행되는 서비스에서 **HDInsight 서비스 또는 작업을 호출**합니다.

	* HDInsight와 SQL 데이터베이스, SQL Server 또는 가상 컴퓨터에서 실행 중인 다른 데이터 저장소 솔루션 간에 **직접 데이터를 전송**합니다.
...
* VPN(가상 사설망)을 사용하여 클라우드 리소스를 로컬 데이터 센터 네트워크에 연결(사이트 간 또는 지점 및 사이트 간)
	...
	가상 네트워크를 사용하여 클라우드 및 데이터 센터를 연결하면 클라우드 전용 구성과 비슷한 시나리오가 활성화되지만 클라우드에서 리소스 사용이 제한되는 대신 데이터 센터에서 리소스로 작업할 수도 있습니다.

	* HDInsight와 데이터 센터 간에 **직접 데이터를 전송**합니다. 예를 들어 Sqoop을 사용하여 SQL Server에 또는 여기에서 전송하거나 LOB(Line of Business) 응용 프로그램에서 생성된 데이터를 읽습니다.
