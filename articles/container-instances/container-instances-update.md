...
컨테이너 인스턴스가 정상적으로 작동하는 동안 컨테이너 그룹의 컨테이너를 업데이트해야 할 수도 있습니다. 예를 들어 이미지 버전을 업데이트하거나, DNS 이름을 변경하거나, 환경 변수를 업데이트하거나, 응용 프로그램이 손상된 컨테이너의 상태를 새로 고치려는 경우가 있습니다.
...
Windows Server Core와 같이 더 큰 컨테이너 이미지를 기반으로 하는 응용 프로그램에서 삭제하여 새로 배포하는 대신 업데이트하는 경우 배포 속도가 크게 향상될 수 있습니다.
