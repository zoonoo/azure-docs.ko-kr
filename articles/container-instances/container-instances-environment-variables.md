...
컨테이너에서 환경 변수를 설정하려면 컨테이너 인스턴스를 만들 때 지정합니다. 이 문서에서는 [Azure CLI,](#azure-cli-example) [Azure PowerShell](#azure-powershell-example)및 [Azure 포털을](#azure-portal-example)통해 컨테이너를 시작할 때 환경 변수설정의 예를 보여 주며 있습니다. 
...
[aci-wordcount][aci-wordcount] 컨테이너의 기본 출력을 보려면 이 [az 컨테이너 create][az-container-create] 명령(지정된 환경 변수 없음)을 사용하여 먼저 실행합니다.
...
출력을 수정하려면 `--environment-variables` *NumWords* 및 *MinLength* 변수에 대한 값을 지정하는 인수가 추가된 두 번째 컨테이너를 시작합니다. (이 예제에서는 Bash 셸 또는 Azure Cloud Shell에서 CLI를 실행하고 있다고 가정합니다. Windows 명령 프롬프트를 사용하는 경우 `--environment-variables "NumWords"="5" "MinLength"="8"`과 같이 큰 따옴표를 사용하여 변수를 지정합니다.)
...
먼저 이 [New-AzContainerGroup][new-Azcontainergroup] 명령을 사용 하 여 기본 구성에서 [aci-wordcount][aci-wordcount] 컨테이너를 시작 합니다.
...
두 컨테이너의 상태가 모두 종료되면(Get-AzContainerInstanceLog를 사용하여 상태를 확인하려면) [Get-AzContainerInstanceLog][azure-instance-log] 명령을 사용하여 로그를 가져옵니다. *Terminated* [Get-AzContainerInstanceLog][azure-instance-log]
...
Azure 포털에서 컨테이너를 시작할 때 환경 변수를 설정하려면 컨테이너를 만들 때 **고급** 페이지에서 지정합니다.

1. **고급** 페이지에서 **다시 시작 정책을** *실패로* 설정합니다.
2. **환경 변수에서**첫 `NumWords` 번째 변수에 `5` 대한 값으로 `MinLength` 입력하고 두 `8` 번째 변수에 대한 값으로 입력합니다. 
1. **검토 + 만들기를** 선택하여 컨테이너를 확인한 다음 배포합니다.

![환경 변수 사용 단추 및 텍스트 상자를 표시하는 포털 페이지][portal-env-vars-01]

컨테이너의 로그를 보려면 **설정에서** **컨테이너를**선택한 다음 로그 를 **선택합니다.** 이전 CLI 및 PowerShell 섹션에서 표시된 출력과 유사하게 스크립트의 동작이 환경 변수에서 수정된 방법을 볼 수 있습니다. 각각이 8개 문자의 최소 길이로 5개의 단어만 표시됩니다.
...
여러 컨테이너로 큰 데이터 세트를 처리하는 일괄 처리와 같은 작업 기반 시나리오는 런타임 시 사용자 지정 환경 변수를 활용할 수 있습니다. 작업 기반 컨테이너 실행에 대한 자세한 내용은 [다시 시작 정책을 통해 컨테이너화된 작업 실행을](container-instances-restart-policy.md)참조하십시오.
