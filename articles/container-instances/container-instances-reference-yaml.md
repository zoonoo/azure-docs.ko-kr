...
|  위치 | 문자열 | No | 리소스 위치입니다. |
|  tags | 개체 | No | 리소스 태그입니다. |
|  identity | 개체 | No | 구성된 경우 컨테이너 그룹 ID입니다. - [ContainerGroupIdentity 개체](#containergroupidentity-object) |
...
|  userAssignedIdentities | 개체 | No | 컨테이너 그룹과 연결된 사용자 ID 목록입니다. 사용자 ID 사전 키 참조는 Azure Resource Manager 리소스 ID이며 형식은 ‘/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}’입니다. |
...
|  imageRegistryCredentials | array | No | 컨테이너 그룹을 만드는 데 사용할 이미지 레지스트리 자격 증명입니다. - [ImageRegistryCredential 개체](#imageregistrycredential-object) |
...
|  ipAddress | 개체 | No | 컨테이너 그룹의 IP 주소 유형입니다. - [IpAddress 개체](#ipaddress-object) |
|  osType | enum | 예 | 컨테이너 그룹의 컨테이너에 필요한 운영 체제 유형입니다. - Windows 또는 Linux |
|  volumes | array | No | 이 컨테이너 그룹의 컨테이너에서 탑재할 수 있는 볼륨의 목록입니다. - [Volume 개체](#volume-object) |
|  진단 | 개체 | No | 컨테이너 그룹에 대한 진단 정보입니다. - [ContainerGroupDiagnostics 개체](#containergroupdiagnostics-object) |
|  networkProfile | 개체 | No | 컨테이너 그룹에 대한 네트워크 프로필 정보입니다. - [ContainerGroupNetworkProfile 개체](#containergroupnetworkprofile-object) |
|  dnsConfig | 개체 | No | 컨테이너 그룹에 대한 DNS 구성 정보입니다. - [DnsConfiguration 개체](#dnsconfiguration-object) |
| sku | enum | No | 컨테이너 그룹의 SKU입니다. - 표준 또는 전용 |
| encryptionProperties | 개체 | No | 컨테이너 그룹의 암호화 속성입니다. - [EncryptionProperties 개체](#encryptionproperties-object) | 
| initContainers | array | No | 컨테이너 그룹의 init 컨테이너입니다. - [InitContainerDefinition 개체](#initcontainerdefinition-object) |
...
|  형식 | enum | 예 | IP가 공용 인터넷 또는 개인 VNET에 노출되는지 여부를 지정합니다. - Public 또는 Private |
...
|  azureFile | 개체 | No | Azure 파일 볼륨입니다. - [AzureFileVolume 개체](#azurefilevolume-object) |
|  emptyDir | 개체 | No | 빈 디렉터리 볼륨입니다. |
|  secret | 개체 | No | 비밀 볼륨입니다. |
|  gitRepo | 개체 | No | Git 리포지토리 볼륨입니다. - [GitRepoVolume 개체](#gitrepovolume-object) |
...
|  Name | Type | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  logAnalytics | 개체 | No | 컨테이너 그룹 로그 분석 정보입니다. - [LogAnalytics 개체](#loganalytics-object) |
...
|  ports | array | No | 컨테이너 인스턴스에서 노출되는 포트입니다. - [ContainerPort 개체](#containerport-object) |
|  environmentVariables | array | No | 컨테이너 인스턴스에서 설정할 환경 변수입니다. - [EnvironmentVariable 개체](#environmentvariable-object) |
|  리소스 | 개체 | 예 | 컨테이너 인스턴스의 리소스 요구 사항입니다. - [ResourceRequirements 개체](#resourcerequirements-object) |
|  volumeMounts | array | No | 컨테이너 인스턴스에 사용할 수 있는 볼륨 탑재입니다. - [VolumeMount 개체](#volumemount-object) |
|  livenessProbe | 개체 | No | 활동성 프로브입니다. - [ContainerProbe 개체](#containerprobe-object) |
|  readinessProbe | 개체 | No | 준비 상태 프로브입니다. - [ContainerProbe 개체](#containerprobe-object) |
...
|  metadata | 개체 | No | 로그 분석에 대한 메타데이터입니다. |
...
| environmentVariables | array  | No |init 컨테이너에서 설정할 환경 변수입니다. - [EnvironmentVariable 개체](#environmentvariable-object)
| volumeMounts |array   | No    | init 컨테이너에 사용할 수 있는 볼륨 탑재입니다. - [VolumeMount 개체](#volumemount-object)
...
|  ---- | ---- | ---- | ---- |
|  protocol | enum | No | 포트와 연결된 프로토콜입니다. - TCP 또는 UDP |
|  포트 | integer | 예 | 컨테이너 그룹 내에서 노출되는 포트 번호입니다. |
...
|  제한 | 개체 | No | 이 컨테이너 인스턴스의 리소스 한도입니다. - [ResourceLimits 개체](#resourcelimits-object) |
...
|  Name | Type | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  exec | 개체 | No | 검색할 실행 명령입니다. - [ContainerExec 개체](#containerexec-object) |
|  httpGet | 개체 | No | 검색할 Http Get 설정입니다. - [ContainerHttpGet 개체](#containerhttpget-object) |
...
|  gpu | 개체 | No | 이 컨테이너 인스턴스의 GPU 요청입니다. - [GpuResource 개체](#gpuresource-object) |
...
|  gpu | 개체 | No | 이 컨테이너 인스턴스의 GPU 한도입니다. - [GpuResource 개체](#gpuresource-object) |
