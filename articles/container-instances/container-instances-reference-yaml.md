---
title: Azure Container Instances YAML 참조
...
|  apiVersion | 열거형 | 예 | 2018-10-01 |
|  위치 | string | 아니요 | 리소스 위치입니다. |
|  태그 | object | 아니요 | 리소스 태그입니다. |
|  identity | object | 아니요 | 구성 된 경우 컨테이너 그룹의 id입니다. - [ContainerGroupIdentity 개체](#ContainerGroupIdentity) |
|  속성 | object | 예 | [ContainerGroupProperties 개체](#ContainerGroupProperties) |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  type | 열거형 | 아니요 | 컨테이너 그룹에 사용 되는 id 유형입니다. ' SystemAssigned 됨, UserAssigned 됨 ' 형식에는 암시적으로 생성 된 id와 사용자 할당 id 집합이 모두 포함 됩니다. ' 없음 ' 유형은 컨테이너 그룹에서 모든 id를 제거 합니다. -SystemAssigned 됨, UserAssigned 됨, SystemAssigned 됨, UserAssigned 안 함 |
...
|  NetworkProfile | object | 아니요 | 컨테이너 그룹에 대 한 네트워크 프로필 정보입니다. - [ContainerGroupNetworkProfile 개체](#ContainerGroupNetworkProfile) |
|  dnsConfig | object | 아니요 | 컨테이너 그룹에 대 한 DNS 구성 정보입니다. - [DnsConfiguration 개체](#DnsConfiguration) |
...
|  속성 | object | 예 | 컨테이너 인스턴스의 속성입니다. - [ContainerProperties 개체](#ContainerProperties) |
...
|  username | string | 예 | 개인 레지스트리의 사용자 이름입니다. |
|  password | string | 아니요 | 개인 레지스트리에 대 한 암호입니다. |
...
|  이름 | type | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  ports | 배열 | 예 | 컨테이너 그룹에 노출 된 포트 목록입니다. - [포트 개체](#Port) |
|  type | 열거형 | 예 | IP가 공용 인터넷 또는 개인 VNET에 노출 되는지 여부를 지정 합니다. -공용 또는 개인 |
|  IP | string | 아니요 | 공용 인터넷에 노출 되는 IP입니다. |
...
|  azureFile | object | 아니요 | Azure 파일 볼륨입니다. - [AzureFileVolume 개체](#AzureFileVolume) |
|  emptyDir | object | 아니요 | 빈 디렉터리 볼륨입니다. |
|  secret | object | 아니요 | 비밀 볼륨입니다. |
|  gitRepo | object | 아니요 | Git 리포지토리 볼륨입니다. - [GitRepoVolume 개체](#GitRepoVolume) |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  logAnalytics | object | 아니요 | 컨테이너 그룹 로그 분석 정보입니다. - [LogAnalytics 개체](#LogAnalytics) |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  이름 서버 | 배열 | 예 | 컨테이너 그룹에 대 한 DNS 서버입니다. -문자열 |
|  searchDomains | string | 아니요 | 컨테이너 그룹에서 호스트 이름 조회에 대 한 DNS 검색 도메인입니다. |
|  옵션 | string | 아니요 | 컨테이너 그룹에 대 한 DNS 옵션입니다. |
...
|  이름 | type | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  image | string | 예 | 컨테이너 인스턴스를 만드는 데 사용 되는 이미지의 이름입니다. |
|  command | 배열 | 아니요 | Exec 형식의 컨테이너 인스턴스 내에서 실행할 명령입니다. -문자열 |
|  ports | 배열 | 아니요 | 컨테이너 인스턴스의 노출 된 포트입니다. - [ContainerPort 개체](#ContainerPort) |
|  environmentVariables | 배열 | 아니요 | 컨테이너 인스턴스에서 설정할 환경 변수입니다. - [고 environmentvariable 개체](#EnvironmentVariable) |
|  리소스 | object | 예 | 컨테이너 인스턴스의 리소스 요구 사항입니다. - [ResourceRequirements 개체](#ResourceRequirements) |
|  volumeMounts | 배열 | 아니요 | 컨테이너 인스턴스에 사용할 수 있는 볼륨 탑재입니다. - [VolumeMount 개체](#VolumeMount) |
|  livenessProbe | object | 아니요 | 선거의 프로브입니다. - [ContainerProbe 개체](#ContainerProbe) |
|  readinessProbe | object | 아니요 | 준비 프로브입니다. - [ContainerProbe 개체](#ContainerProbe) |
...
|  이름 | type | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  protocol | 열거형 | 아니요 | 포트와 연결 된 프로토콜입니다. -TCP 또는 UDP |
|  port | integer | 예 | 포트 번호입니다. |
...
|  storageAccountKey | string | 아니요 | Azure 파일 공유에 액세스 하는 데 사용 되는 저장소 계정 액세스 키입니다. |
...
|  이름 | type | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  디렉터리 | string | 아니요 | 대상 디렉터리 이름입니다. '.. '를 포함 하거나 시작 해서는 안 됩니다.  '. '가 제공 되 면 볼륨 디렉터리는 git 리포지토리가 됩니다.  그렇지 않으면 지정 된 경우 볼륨은 지정 된 이름의 하위 디렉터리에 git 리포지토리를 포함 합니다. |
|  리포지토리(repository) | string | 예 | 리포지토리 URL |
|  revision | string | 아니요 | 지정 된 수정 버전에 대 한 해시를 커밋합니다. |
...
|  logType | 열거형 | 아니요 | 사용할 로그 유형입니다. -ContainerInsights 또는 ContainerInstanceLogs |
|  메타데이터 | object | 아니요 | Log analytics에 대 한 메타 데이터입니다. |
...
|  ---- | ---- | ---- | ---- |
|  protocol | 열거형 | 아니요 | 포트와 연결 된 프로토콜입니다. -TCP 또는 UDP |
|  port | integer | 예 | 컨테이너 그룹 내에서 노출 되는 포트 번호입니다. |
...
|  값 | string | 아니요 | 환경 변수의 값입니다. |
|  secureValue | string | 아니요 | 보안 환경 변수의 값입니다. |
...
|  이름 | type | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  요청 | object | 예 | 이 컨테이너 인스턴스의 리소스 요청입니다. - [ResourceRequests 개체](#ResourceRequests) |
|  제한 | object | 아니요 | 이 컨테이너 인스턴스의 리소스 제한입니다. - [ResourceLimits 개체](#ResourceLimits) |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  재시도 | object | 아니요 | [ContainerExec 개체](#ContainerExec) 에 대 한 실행 명령 |
|  httpGet | object | 아니요 | [ContainerHttpGet 개체](#ContainerHttpGet) 에 대 한 Http 설정 가져오기 |
...
|  GPU | object | 아니요 | 이 컨테이너 인스턴스의 GPU 요청입니다. - [GpuResource 개체](#GpuResource) |
...
|  GPU | object | 아니요 | 이 컨테이너 인스턴스의 GPU 제한입니다. - [GpuResource 개체](#GpuResource) |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  command | 배열 | 아니요 | 컨테이너 내에서 실행할 명령입니다. -문자열 |
...
|  이름 | 형식 | 필요한 공간 | 값 |
|  ---- | ---- | ---- | ---- |
|  path | string | 아니요 | 프로브 경로입니다. |
|  port | integer | 예 | 프로브 할 포트 번호입니다. |
|  체계 | 열거형 | 아니요 | 체계입니다. -http 또는 https |
...
|  sku | 열거형 | 예 | GPU 리소스의 SKU입니다. -K80, P100, V100 |
