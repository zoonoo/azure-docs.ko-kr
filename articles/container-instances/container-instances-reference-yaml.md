...
YAML 파일은 재현 가능한 배포를 위해 컨테이너 그룹을 구성 하는 편리한 방법입니다. [리소스 관리자 템플릿](/azure/templates/Microsoft.ContainerInstance/2019-12-01/containerGroups) 또는 Azure Container Instances sdk를 사용 하 여 컨테이너 그룹을 만들거나 업데이트 하는 방법에 대 한 간단한 대안입니다.

> [!NOTE]
> 이 참조는 Azure Container Instances REST API 버전에 대 한 YAML 파일에 적용 됩니다 `2019-12-01` .
...
|  Name | 유형 | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  name | 문자열 | 예 | 컨테이너 그룹의 이름입니다. |
|  apiVersion | enum | 예 | 2018-10-01 |
|  위치 | 문자열 | No | 리소스 위치입니다. |
|  tags | object | 아니요 | 리소스 태그입니다. |
|  identity | object | 아니요 | 구성 된 경우 컨테이너 그룹의 id입니다. - [ContainerGroupIdentity 개체](#containergroupidentity-object) |
...
|  ipAddress | object | 아니요 | 컨테이너 그룹의 IP 주소 유형입니다. - [IpAddress 개체](#ipaddress-object) |
...
|  진단 | object | 아니요 | 컨테이너 그룹에 대 한 진단 정보입니다. - [ContainerGroupDiagnostics 개체](#containergroupdiagnostics-object) |
|  networkProfile | object | 아니요 | 컨테이너 그룹에 대 한 네트워크 프로필 정보입니다. - [ContainerGroupNetworkProfile 개체](#containergroupnetworkprofile-object) |
|  dnsConfig | object | 아니요 | 컨테이너 그룹에 대 한 DNS 구성 정보입니다. - [DnsConfiguration 개체](#dnsconfiguration-object) |
| sku | enum | 아니요 | 컨테이너 그룹의 SKU-Standard 또는 전용 |
| 속성 속성 | object | 아니요 | 컨테이너 그룹의 암호화 속성입니다. - [속성 개체](#encryptionproperties-object) | 
| initContainers | array | 아니요 | 컨테이너 그룹에 대 한 init 컨테이너입니다. - [InitContainerDefinition 개체](#initcontainerdefinition-object) |
...
|  Name | 유형 | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  서버 | 문자열 | 예 | 프로토콜이 없는 Docker 이미지 레지스트리 서버 (예: "http" 및 "https") |
|  사용자 이름 | 문자열 | 예 | 개인 레지스트리의 사용자 이름입니다. |
...
|  azureFile | object | 아니요 | Azure 파일 볼륨입니다. - [AzureFileVolume 개체](#azurefilevolume-object) |
|  emptyDir | object | 아니요 | 빈 디렉터리 볼륨입니다. |
|  secret | object | 아니요 | 비밀 볼륨입니다. |
|  gitRepo | object | 아니요 | Git 리포지토리 볼륨입니다. - [GitRepoVolume 개체](#gitrepovolume-object) |
...
|  Name | 유형 | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  logAnalytics | object | 아니요 | 컨테이너 그룹 로그 분석 정보입니다. - [LogAnalytics 개체](#loganalytics-object) |
...
### <a name="encryptionproperties-object"></a>속성 개체
...
|  livenessProbe | object | 아니요 | 선거의 프로브입니다. - [ContainerProbe 개체](#containerprobe-object) |
|  readinessProbe | object | 아니요 | 준비 프로브입니다. - [ContainerProbe 개체](#containerprobe-object) |
...
|  Name | 유형 | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  공유 | 문자열 | 예 | 볼륨으로 탑재할 Azure 파일 공유의 이름입니다. |
|  readOnly | boolean | 아니요 | 볼륨으로 탑재 된 Azure 파일 공유를 읽기 전용인 지 여부를 나타내는 플래그입니다. |
|  storageAccountName | 문자열 | 예 | Azure 파일 공유를 포함 하는 저장소 계정의 이름입니다. |
...
|  Name | 유형 | 필수 | 값 |
|  ---- | ---- | ---- | ---- |
|  workspaceId | 문자열 | 예 | Log analytics에 대 한 작업 영역 id |
|  workspaceKey | 문자열 | 예 | Log analytics에 대 한 작업 영역 키 |
|  logType | enum | 아니요 | 사용할 로그 유형입니다. -ContainerInsights 또는 ContainerInstanceLogs |
|  metadata | object | 아니요 | Log analytics에 대 한 메타 데이터입니다. |
...
| 명령을 사용합니다.   | array | 아니요    | Exec 형식으로 init 컨테이너 내에서 실행할 명령입니다. -문자열 |
| environmentVariables | array  | 아니요 |Init 컨테이너에 설정할 환경 변수입니다. - [고 environmentvariable 개체](#environmentvariable-object)
| volumeMounts |array   | 아니요    | Init 컨테이너에 사용할 수 있는 볼륨 탑재입니다. - [VolumeMount 개체](#volumemount-object)
...
|  제한 | object | 아니요 | 이 컨테이너 인스턴스의 리소스 제한입니다. - [ResourceLimits 개체](#resourcelimits-object) |
...
|  mountPath | 문자열 | 예 | 볼륨이 탑재 되어야 하는 컨테이너 내의 경로입니다. 콜론 (:)을 포함 하지 않아야 합니다. |
...
|  gpu | object | 아니요 | 이 컨테이너 인스턴스의 GPU 요청입니다. - [GpuResource 개체](#gpuresource-object) |
...
|  gpu | object | 아니요 | 이 컨테이너 인스턴스의 GPU 제한입니다. - [GpuResource 개체](#gpuresource-object) |
