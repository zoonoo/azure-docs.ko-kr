...
description: GPU 리소스를 사용하여 계산 집약적인 컨테이너 앱을 실행하기 위해 Azure 컨테이너 인스턴스를 배포하는 방법을 알아봅니다.
ms.topic: article
ms.date: 02/19/2020
...
Azure 컨테이너 인스턴스에서 특정 계산 집약적인 워크로드를 실행하려면 *GPU 리소스를*사용하여 [컨테이너 그룹을](container-instances-container-groups.md) 배포합니다. 그룹의 컨테이너 인스턴스는 CUDA 및 딥 러닝 애플리케이션과 같은 컨테이너 워크로드를 실행하는 동안 하나 이상의 NVIDIA Tesla GPU에 액세스할 수 있습니다.

이 문서에서는 [YAML 파일](container-instances-multi-container-yaml.md) 또는 [리소스 관리자 템플릿을](container-instances-multi-container-group.md)사용하여 컨테이너 그룹을 배포할 때 GPU 리소스를 추가하는 방법을 보여 주며 있습니다. Azure 포털을 사용하여 컨테이너 인스턴스를 배포할 때 GPU 리소스를 지정할 수도 있습니다.
...
**추가 제한 사항**: [가상 네트워크에](container-instances-vnet.md)컨테이너 그룹을 배포할 때 GPU 리소스를 사용할 수 없습니다.
...
GPU 리소스를 배포할 때는 이전 표에 표시된 최대 값까지 워크로드에 적합한 CPU 및 메모리 리소스를 설정합니다. 이러한 값은 현재 GPU 리소스가 없는 컨테이너 그룹에서 사용할 수 있는 CPU 및 메모리 리소스보다 큽니다.  
...
* **가격 책정** - GPU 리소스가 없는 컨테이너 그룹과 마찬가지로, Azure는 GPU 리소스가 있는 컨테이너 그룹의 *기간* 동안 사용된 리소스 비용을 청구합니다. 이 기간은 컨테이너의 첫 번째 이미지를 끌어온 시점부터 컨테이너 그룹이 종료되는 시점 사이의 시간으로 계산됩니다. 컨테이너 그룹을 배포하는 시간은 포함되지 않습니다.
  ...
  [가격 세부 정보를](https://azure.microsoft.com/pricing/details/container-instances/)참조하십시오.

* **CUDA 드라이버** - GPU 리소스가 있는 컨테이너 인스턴스는 NVIDIA CUDA 드라이버 및 컨테이너 런타임이 미리 프로비전되므로, CUDA 워크로드용으로 개발된 컨테이너 이미지를 사용할 수 있습니다.

  이 단계에서는 CUDA 9.0을 지원합니다. 예를 들어 Docker 파일에 다음 기본 이미지를 사용할 수 있습니다.
  * [엔비디아/cuda:9.0-베이스-우분투16.04](https://hub.docker.com/r/nvidia/cuda/)
  * [텐서플로우/텐서플로우: 1.12.0 gpu-py3](https://hub.docker.com/r/tensorflow/tensorflow)
...
[az 컨테이너 create][az-container-create] 명령을 사용하여 컨테이너 그룹을 배포하여 매개 변수에 `--file` 대한 YAML 파일 이름을 지정합니다. GPU 리소스를 지원하는 *eastus*처럼 리소스 그룹의 이름과 컨테이너 그룹의 위치를 제공해야 합니다.  
...
GPU 리소스가 있는 컨테이너 그룹을 배포하는 또 다른 방법은 [Resource Manager 템플릿](container-instances-multi-container-group.md)을 사용하는 것입니다. 먼저 `gpudeploy.json` 파일을 만든 후, 다음 JSON을 이 파일에 복사합니다. 이 예제는 MNIST 데이터 집합에 대해 [TensorFlow](https://www.tensorflow.org/) 학습 작업을 실행하는 V100 GPU가 있는 컨테이너 인스턴스를 배포합니다. 리소스 요청은 워크로드를 실행하기에 충분합니다.
