...
description: Container Instances 보안 기준은 Azure 보안 벤치 마크에 지정 된 보안 권장 사항을 구현 하기 위한 절차 지침과 리소스를 제공 합니다.
author: msmbaldwin
...
이 보안 기준은 [Azure Security 벤치 마크 버전 1.0](../security/benchmarks/overview-v1.md) 의 지침을 Container Instances 적용 합니다. Azure Security Benchmark는 Azure에서 클라우드 솔루션을 보호하는 방법에 대한 권장 사항을 제공합니다.
콘텐츠는 Azure 보안 벤치 마크에 정의 된 **보안 컨트롤** 및 Container Instances에 적용 되는 관련 지침에 따라 그룹화 됩니다. Container Instances에 적용할 수 없는 **컨트롤** 은 제외 되었습니다.

 
Container Instances 완전히 Azure 보안 벤치 마크에 매핑되는 방법을 보려면 [전체 Container Instances 보안 기준 매핑 파일](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)을 참조 하세요.
...
**지침**: Azure Container Instances의 컨테이너 그룹을 Azure virtual network와 통합 합니다.  Azure virtual network를 사용 하면 여러 Azure 리소스 (예: 컨테이너 그룹)를 인터넷을 통해 라우팅할 수 없는 네트워크에 저장할 수 있습니다.
...
**지침**: Azure Container Instances에서 azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 azure container registry를 보호 하는 데 사용 되는 서브넷에 연결 된 nsg에 대 한 nsg (네트워크 보안 그룹) 흐름 로그를 사용 하도록 설정할 수 있습니다. Azure Storage 계정에 NSG 흐름 로그를 기록 하 여 흐름 레코드를 생성할 수 있습니다. 비정상적인 활동을 조사 하는 데 필요한 경우 Azure Network Watcher 패킷 캡처를 사용 하도록 설정 합니다.
...
**지침**: Azure Container Instances와 함께 Azure Container Registry를 사용 하는 경우 Azure Container Registry와 연결 된 네트워크 리소스에 대 한 표준 보안 구성을 정의 하 고 구현 하는 것이 좋습니다.

**Microsoft.containerregistry** 및 **microsoft 네트워크** 네임 스페이스의 별칭 Azure Policy 사용 하 여 컨테이너 레지스트리의 네트워크 구성을 감사 하거나 적용 하는 사용자 지정 정책을 만듭니다.

Azure 청사진을 사용 하 여 Azure Resource Manager 템플릿, Azure RBAC 컨트롤 및 정책 정의와 같은 주요 환경 아티팩트를 단일 청사진 정의로 패키징하 여 대규모 Azure 배포를 간소화할 수 있습니다. 새 구독에 청사진을 쉽게 적용 하 고 버전 관리를 통해 제어 및 관리를 세부적으로 조정 합니다.

- [Azure Policy를 사용 하 여 Azure container registry의 준수 감사](../container-registry/container-registry-azure-policy.md)
...
**지침**: Azure container Instance에 의해 생성 된 보안 데이터를 집계 하기 위해 Azure Monitor를 통해 로그를 수집 합니다. Azure Monitor 내에서 Log Analytics 작업 영역을 사용 하 여 분석을 쿼리하고 수행 하 고 장기/보관 저장소에 Azure Storage 계정을 사용 합니다.
...
**지침**: Azure Monitor 사용자 기반 이벤트에 대 한 리소스 로그 (이전의 진단 로그)를 수집 합니다. 컨테이너 인증 이벤트를 감사 하 고 컨테이너 그룹의 보안 문제를 진단할 수 있도록 끌어오기 및 푸시 이벤트와 같은 아티팩트에 대 한 전체 작업 내역을 제공 하기 위해이 데이터를 수집 하 고 사용 합니다.
...
**지침**: 비정상적인 동작에 대 한 Azure Container Instances 로그를 분석 및 모니터링 하 고 정기적으로 결과를 검토 합니다. Azure Monitor의 Log Analytics 작업 영역을 사용 하 여 로그를 검토 하 고 로그 데이터에 대 한 쿼리를 수행 합니다.
...
- [로그를 사용 하는 컨테이너 그룹 및 쿼리 로그를 만드는 방법](container-instances-log-analytics.md)
...
**지침**: Azure Container Instances에서 azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 azure Log Analytics 작업 영역을 사용 하 여 azure container registry와 관련 된 보안 로그 및 이벤트의 비정상적인 활동을 모니터링 하 고 경고 합니다.

- [진단 평가 및 감사에 대 한 Azure Container Registry 로그](../container-registry/container-registry-diagnostics-audit-logs.md)
...
**지침**: 해당 사항 없음 Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure container registry는 맬웨어 방지 관련 로그를 처리 하거나 생성 하지 않습니다.
...
**지침**: 해당 사항 없음 Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure container registry는 엔드포인트 이며 통신을 시작 하지 않으며 서비스는 DNS를 쿼리하지 않습니다.
...
Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 각 Azure container registry에 대해 기본 제공 관리자 계정이 사용 되는지 여부를 추적 합니다. 사용 하지 않을 때 계정을 사용 하지 않도록 설정 합니다.
...
- [Azure Container Registry 관리자 계정](../container-registry/container-registry-authentication.md#admin-account)
...
Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure container registry의 기본 관리자 계정을 사용 하도록 설정 하면 복잡 한 암호가 자동으로 만들어지고 회전 됩니다. 사용 하지 않을 때 계정을 사용 하지 않도록 설정 합니다.
...
Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 컨테이너 레지스트리의 기본 제공 관리자 계정을 사용 하도록 설정 하는 프로시저를 만듭니다. 사용 하지 않을 때 계정을 사용 하지 않도록 설정 합니다.
...
Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 컨테이너 레지스트리에 대 한 개별 액세스에는 Azure AD와 통합 된 개별 로그인을 사용 합니다.

- [Azure AD를 사용 하 여 SSO 이해](../active-directory/manage-apps/what-is-single-sign-on.md)

- [컨테이너 레지스트리에 대 한 개별 로그인](../container-registry/container-registry-authentication.md#individual-login-with-azure-ad)
...
Azure AD 사용자 계정에 대 한 진단 설정을 만들고 감사 로그 및 로그인 로그를 Log Analytics 작업 영역에 전송 하 여이 프로세스를 간소화할 수 있습니다. Log Analytics 작업 영역 내에서 원하는 경고를 구성할 수 있습니다.
...
**지침**: 사용할 수 없음 고객 Lockbox 현재 Azure Container Instances 지원 되지 않습니다.
...
- [컨테이너 이미지 태그 지정 및 버전 관리에 대 한 권장 사항](../container-registry/container-registry-image-tag-version.md)

- [Azure container registry에서 컨테이너 이미지 잠그기](../container-registry/container-registry-image-lock.md)
...
- [Azure virtual network 또는 방화벽 규칙을 사용 하 여 Azure container registry에 대 한 액세스 제한](../container-registry/container-registry-vnet.md)
...
Microsoft에서 관리 하는 기본 플랫폼의 경우 Microsoft는 모든 고객 데이터를 중요 한 것으로 간주 하 고 고객 데이터 손실 및 노출을 방지 하기 위해 좋은 길이로 이동 합니다. Azure 내에서 고객 데이터를 안전하게 유지하기 위해 Microsoft는 강력한 데이터 보호 제어 및 기능 모음을 구현하고 유지 관리합니다.
...
**지침**: Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 데이터 식별, 분류 및 손실 방지 기능을 아직 Azure Container Registry 사용할 수 없습니다. 규정 준수를 위해 필요한 경우 타사 솔루션을 구현합니다.
...
**지침**: Azure Container Instances Azure Container Registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 azure RBAC (역할 기반 액세스 제어)를 사용 하 여 azure Container registry의 데이터 및 리소스에 대 한 액세스를 관리 합니다.

- [Azure에서 Azure RBAC를 구성 하는 방법](../role-based-access-control/role-assignments-portal.md)

- [Azure Container Registry 역할 및 권한](../container-registry/container-registry-roles.md)
...
**지침**: 모든 Azure 리소스에서 미사용 암호화를 사용 합니다. Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 기본적으로 Azure container registry의 모든 데이터는 Microsoft 관리 키를 사용 하 여 미사용으로 암호화 됩니다.

- [Azure의 저장 데이터 암호화 이해](../security/fundamentals/encryption-atrest.md)

- [Azure Container Registry에서 고객이 관리 하는 키](../container-registry/container-registry-customer-managed-keys.md)
...
**지침**: Log Analytics 작업 영역은 Azure 리소스 뿐만 아니라 다른 클라우드의 온-프레미스 리소스와 리소스에서 로그 데이터를 저장 하 고 쿼리 하는 중앙 위치를 제공 합니다. Azure Container Instances는 Azure Monitor 로그로 로그 및 이벤트 데이터를 전송할 수 있는 기능을 기본 제공합니다.
...
**지침**: 솔루션을 활용 하 여 개인 레지스트리에서 컨테이너 이미지를 검색 하 고 잠재적 취약성을 식별 합니다. 여러 솔루션에서 제공 하는 위협 검색의 깊이를 이해 하는 것이 중요 합니다. 컨테이너 이미지에 대 한 취약성 평가를 수행 하는 Azure Security Center의 권장 사항을 따릅니다. 필요에 따라 Azure Marketplace에서 타사 솔루션을 배포 하 여 이미지 취약성 평가를 수행 합니다.
...
**지침**: 타사 솔루션을 사용 하 여 응용 프로그램 이미지를 패치할 수 있습니다. 또한 Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure Container Registry 작업을 실행 하 여 기본 이미지의 보안 패치 또는 기타 업데이트를 기반으로 컨테이너 레지스트리에서 응용 프로그램 이미지에 대 한 업데이트를 자동화할 수 있습니다.

- [ACR 작업의 기본 이미지 업데이트 정보](../container-registry/container-registry-tasks-base-images.md)
...
**지침**: Azure Container Instances에서 azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 취약점에 대해 컨테이너 이미지를 정기적으로 검색할 수 있도록 ACR (azure container registry)를 Azure Security Center와 통합 합니다. 필요에 따라 Azure Marketplace에서 타사 솔루션을 배포 하 여 정기적으로 이미지 취약성 검사를 수행 합니다.
...
**지침**: Azure Container Instances에서 Azure container registry와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure Security Center에 AZURE CONTAINER REGISTRY (ACR)를 통합 하 여 취약성에 대 한 컨테이너 이미지를 정기적으로 검색 하 고 위험을 분류 합니다. 필요에 따라 Azure Marketplace에서 타사 솔루션을 배포 하 여 정기적으로 이미지 취약성 검사 및 위험 분류를 수행 합니다.
...
**지침**: Azure Container Instances에서 AZURE CONTAINER REGISTRY (acr)와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 acr은 레지스트리의 이미지에 대 한 태그 및 매니페스트를 비롯 한 메타 데이터를 유지 관리 합니다. 아티팩트에 태그를 지정 하기 위한 권장 사례를 따릅니다.

- [레지스트리, 리포지토리 및 이미지 정보](../container-registry/container-registry-concepts.md)
...
**지침**: Azure Container Instances에서 AZURE CONTAINER REGISTRY (ACR)와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 비정상적인 동작에 대 한 Azure Container Registry 로그를 분석 및 모니터링 하 고 정기적으로 결과를 검토 합니다. Azure Monitor의 Log Analytics 작업 영역을 사용 하 여 로그를 검토 하 고 로그 데이터에 대 한 쿼리를 수행 합니다.
...
- [예를 들어 Windows 환경에서 PowerShell 스크립트 실행을 제어 하는 방법](/powershell/module/microsoft.powershell.security/set-executionpolicy?amp;preserve-view=true&view=powershell-7)
...
Azure Container Instances에서 Azure Container Registry (ACR)와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Azure Policy를 사용 하 여 Azure Container registry의 준수를 감사 합니다.
...
**지침**: 사용자 지정 Azure 정책 정의를 사용 하는 경우 Azure Repos를 사용 하 여 코드를 안전 하 게 저장 하 고 관리 합니다.
...
**지침**: 해당 사항 없음 이 컨트롤은 계산 리소스에만 적용 됩니다.
...
Azure Policy 적용 하 여 구독에 만들 수 있는 리소스 유형에 대 한 제한을 설정 합니다.
...
- [Key Vault에 인증 하는 방법](container-instances-managed-identity.md)

- [Key Vault 액세스 정책을 할당 하는 방법](../key-vault/general/assign-access-policy-portal.md)

- [Azure Container Instances에서 관리 ID를 사용하는 방법](../container-registry/container-registry-tasks-authentication-managed-identity.md)

- [Container Instances를 사용 하 여 데이터를 암호화 하는 방법](container-instances-encrypt-data.md)
...
- [Azure Container Registry에 인증하기 위해 Azure 관리 ID 사용](../container-registry/container-registry-authentication-managed-identity.md)
...
**지침**: Microsoft 맬웨어 방지 프로그램은 Azure 서비스 (예: Azure Container Instances)를 지 원하는 기본 호스트에서 사용 하도록 설정 되어 있지만 고객 데이터에서 실행 되지 않습니다.
...
**지침**: Azure Container Instances에서 AZURE CONTAINER REGISTRY (ACR)와 같은 클라우드 기반 개인 레지스트리를 사용 하는 경우 Microsoft Azure 컨테이너 레지스트리의 데이터는 항상 자동으로 복제 되어 내구성 및 고가용성을 보장 합니다. Azure Container Registry은 계획 되거나 계획 되지 않은 이벤트에서 보호 되도록 데이터를 복사 합니다.

필요에 따라 여러 Azure 지역에서 레지스트리 복제본을 유지 하기 위해 컨테이너 레지스트리를 지역에서 복제할 수 있습니다.

- [Azure Container Registry의 지리적 복제](../container-registry/container-registry-geo-replication.md)
...
- [컨테이너 이미지를 컨테이너 레지스트리로 가져오기](../container-registry/container-registry-import-images.md)

- [Azure에서 키 자격 증명 모음 키를 백업하는 방법](/powershell/module/az.keyvault/backup-azkeyvaultkey?amp;preserve-view=true&view=azps-4.8.0)

- [Container Instances를 사용 하 여 배포 데이터 암호화](container-instances-encrypt-data.md)
...
- [Azure에서 Azure Key Vault 키를 복원 하는 방법](/powershell/module/az.keyvault/restore-azkeyvaultkey?amp;preserve-view=true&view=azps-4.8.0)
...
- [NIST 게시물 - IT 계획 및 기능에 대한 테스트, 학습 및 연습 프로그램에 대한 가이드](https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-84.pdf)
