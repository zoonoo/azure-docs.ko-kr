...
이 문서에서는 응용 프로그램 [컨테이너와](container-instances-container-groups.md) TLS/SSL 공급자를 실행 하는 사이드카 컨테이너를 사용 하 여 컨테이너 그룹을 만드는 방법을 보여 주면됩니다. 별도의 TLS 끝점이 있는 컨테이너 그룹을 설정하면 응용 프로그램 코드를 변경하지 않고 응용 프로그램에 대한 TLS 연결을 사용하도록 설정합니다.
...
* TLS를 사용하도록 구성된 공용 [Nginx](https://hub.docker.com/_/nginx) 이미지를 실행하는 사이드카 컨테이너입니다. 
...
컨테이너 그룹에서 TLS를 사용하도록 설정하는 다른 방법에 대한 [다음 단계를](#next-steps) 참조하십시오.
...
Nginx를 TLS 공급자로 설정하려면 TLS/SSL 인증서가 필요합니다. 이 문서에서는 자체 서명된 TLS/SSL 인증서를 만들고 설정하는 방법을 보여 주며 있습니다. 프로덕션 시나리오의 경우 인증서 기관에서 인증서를 가져와야 합니다.

자체 서명된 TLS/SSL 인증서를 만들려면 Azure Cloud Shell 및 여러 Linux 배포판에서 사용할 수 있는 [OpenSSL](https://www.openssl.org/) 도구를 사용하거나 운영 체제에서 유사한 클라이언트 도구를 사용합니다.
...
이 섹션에서는 Nginx가 TLS를 사용할 수 있도록 구성 파일을 만듭니다. 먼저 다음 텍스트를 라는 `nginx.conf`새 파일에 복사합니다. Azure Cloud Shell에서 Visual Studio 코드를 사용하여 작업 디렉터리에서 파일을 만들 수 있습니다.
...
Base64는 Nginx 구성 파일, TLS/SSL 인증서 및 TLS 키를 인코딩합니다. 다음 섹션에서는 컨테이너 그룹을 배포하는 데 사용되는 YAML 파일에 인코딩된 내용을 입력합니다.
...
이 문서에서는 컨테이너 그룹에서 실행 중인 웹 앱에 대한 TLS 연결을 사용하도록 Nginx 컨테이너를 설정하는 방법을 보여 주어 도모했습니다. 포트 80이 아닌 다른 포트에서 수신 하는 앱에 대해 이 예제를 조정할 수 있습니다. 또한 Nginx 구성 파일을 업데이트하여 포트 80(HTTP)에서 서버 연결을 자동으로 리디렉션하여 HTTPS를 사용할 수도 있습니다.

이 문서에서는 사이드카에서 Nginx를 사용하지만 [Caddy와](https://caddyserver.com/)같은 다른 TLS 공급자를 사용할 수 있습니다.

[Azure 가상 네트워크에](container-instances-vnet.md)컨테이너 그룹을 배포하는 경우 다음을 포함하여 백 엔드 컨테이너 인스턴스에 대한 TLS 끝점을 사용하도록 설정하는 다른 옵션을 고려할 수 있습니다.
