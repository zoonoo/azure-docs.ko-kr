...
- 지정된 자산과 연관된 고유 로케이터는 한 번에 5개를 초과할 수 없습니다. 
	...
	**참고**
	로케이터는 사용자별 액세스 제어를 관리하도록 설계 되지 않았습니다. 개별 사용자에게 서로 다른 액세스 권한을 부여하려면 DRM(디지털 권한 관리)솔루션을 사용 합니다.
...
- 기본적으로 최대 5개의 라이브 채널을 미디어 서비스 계정에 추가할 수 있습니다. 
	...
	또한 동시에 최대 5개의 채널을 시작할 수 있습니다. 제한을 상향하려면 뒤에 나오는 *How to request a higher limit for updatable quotas* 섹션을 참조하세요.

- 기본적으로 단일 채널에 최대 50개의 프로그램(중지된 상태)을 추가할 수 있습니다. 

	동시에 실행 되는 프로그램은 최대 3개만 사용할 수 있습니다. 제한을 상향하려면 뒤에 나오는 *How to request a higher limit for updatable quotas* 섹션을 참조하세요.

- 기본적으로 모든 미디어 서비스 계정은 최대 2 스트리밍 끝점(원본)을 가질 수 있습니다. 

	동시에 실행 되는 스트리밍 끝점은 최대 2개만 사용할 수 있습니다.

	각 스트리밍 끝점을 최대 10개의 스트리밍 단위로 확장할 수 있습니다. 제한을 상향하려면 뒤에 나오는 *How to request a higher limit for updatable quotas* 섹션을 참조하세요.
...
1. 다음 페이지의 지침을 따르고 필요한 인코딩 또는 스트리밍 예약 단위 수에 대한 정보를 자세히 입력하세요. 
