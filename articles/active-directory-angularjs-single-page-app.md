...
9. 또한 TodoSPA 프로젝트에서 `App/Scripts/toGoListSvc.js` 파일을 엽니다.   `apiEndpoint` 변수 값을 To Go API의 위치로 바꿉니다.  기본적으로 `https://localhost:44327/`입니다.
...
1. [Azure 관리 포털]에 로그인합니다(https://manage.windowsazure.com).
...
4. 웹 사이트를 만든 후 클릭하여 관리합니다.  이 단계 집합에서는 .publishsettings 파일을 다운로드하여 저장합니다.  소스 제어와 같은 다른 배포 메커니즘을 사용할 수도 있습니다. .publishsettings 파일 사용에 대한 자세한 내용은 [방법: 구독에 연결](http://azure.microsoft.com/documentation/articles/install-configure-powershell/#Connect)(영문)을 참조하세요. 

### To Do SPA Azure 웹 사이트 만들기

1. [Azure 관리 포털]로 이동합니다(https://manage.windowsazure.com).
2. 왼쪽 탐색 창에서 **웹 사이트**를 클릭합니다.
...
2. 두 가지를 변경해야 합니다.   `App\Scripts\app.js`에서 `endpoints` 개체의 속성 이름을 To Go API의 새 위치(예: `https://togo-contoso.azurewebsites.net/`. )로 바꿉니다. `App\Scripts\toGoListSvc.js`에서 `apiEndpoint` 변수를 동일한 값으로 바꿉니다.
...
6. **연결** 탭에서 https가 되도록 대상 URL을 업데이트합니다(예: https://todo-contoso.azurewebsites.net).  **다음**을 클릭합니다.
7. **설정** 탭에서 **조직 인증 사용**이 선택되지 않았는지 확인합니다.  **게시**를 클릭합니다.
8. Visual Studio에서 프로젝트를 게시하고 브라우저가 자동으로 프로젝트의 URL로 열립니다.  프로젝트의 기본 웹 페이지가 표시되면 게시에 성공한 것입니다.
