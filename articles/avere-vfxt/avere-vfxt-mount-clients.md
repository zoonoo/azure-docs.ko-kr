...
> [!NOTE]
> Avere vFXT 클러스터를 만들 때 새 Blob 컨테이너를 만들지 않은 경우 클라이언트를 탑재하기 전에 [Storage 구성에](avere-vfxt-add-storage.md) 설명된 대로 저장소 시스템을 추가합니다.

클라이언트에서 ``mount`` 명령은 vFXT 클러스터의 가상 서버(vserver)를 로컬 파일 시스템의 경로에 매핑합니다. 형식은 ``mount <vFXT path> <local path> {options}``입니다.

마운트 명령에는 세 가지 요소가 있습니다.

* vFXT 경로 - 아래에 설명된 클러스터 9의 IP 주소와 네임스페이스 정션 경로의 조합)
* 로컬 경로 - 클라이언트의 경로
* 명령 옵션 마운트 - [마운트 명령 인수에 나열됨](#mount-command-arguments)

### <a name="junction-and-ip"></a>접합 및 IP

vserver 경로는 *IP 주소*와 *네임스페이스 접합*에 대한 경로의 조합입니다. 네임스페이스 접합은 스토리지 시스템을 추가할 때 정의된 가상 경로입니다.

Blob 저장소로 클러스터를 만든 경우 해당 컨테이너에 대한 네임스페이스 경로는`/msazure`

예: ``mount 10.0.0.12:/msazure /mnt/vfxt``

클러스터를 만든 후 저장소를 추가한 경우 네임스페이스 정션 경로는 정션을 작성할 때 **네임스페이스 경로에** 설정한 값입니다. 예를 들어 ``/avere/files``를 네임스페이스 경로로 사용한 경우 클라이언트에서 *IP_address*:/avere/files를 해당 로컬 탑재 지점에 탑재합니다.
...
클라이언트를 탑재한 후 이를 사용하여 클러스터의 새 Blob 저장소 컨테이너에 데이터를 복사할 수 있습니다. 새 저장소를 채울 필요가 없는 경우 다른 링크를 읽고 추가 설치 작업에 대해 알아봅니다.

* [클러스터 코어 파일러로 데이터 이동](avere-vfxt-data-ingest.md) - 여러 클라이언트와 스레드를 사용하여 데이터를 새 코어 파일러에 효율적으로 업로드하는 방법
