...
vFXT 클러스터가 실행된 후에도 이를 사용하기 전에 수행해야 할 몇 가지 구성 단계가 있습니다. 새 Blob 저장소 컨테이너를 만든 경우 데이터를 해당 컨테이너로 이동해야 합니다. NAS 스토리지 시스템을 사용하는 경우 클러스터를 만든 후 추가해야 합니다. 클러스터에 클라이언트를 연결하려고 합니다.
...
1. Avere vFXT 클러스터 만들기
   ...
   * 클러스터 컨트롤러 만들기
     ...
     배포 하는 동안 새 가상 네트워크 또는 서브넷을 만드는 경우 컨트롤러는 공용 IP 주소를 갖습니다. 즉, 컨트롤러는 가상 네트워크 외부에서 클러스터에 연결하기 위한 점프 호스트 역할을 할 수 있습니다.

   * 클러스터 노드 VM 만들기

   * 개별 노드에서 클러스터 만들기

   * 선택적으로 새 Blob 컨테이너를 만들고 클러스터에 대한 백 엔드 저장소로 구성합니다.

   클러스터 생성은 [vFXT 클러스터 배포에](avere-vfxt-deploy.md)대해 자세히 설명합니다.

1. 클러스터 구성

   Avere vFXT 구성 인터페이스(Avere 제어판)에 연결하여 클러스터 설정을 사용자 지정합니다. 지원 모니터링을 선택하고 하드웨어 저장소 또는 추가 Blob 컨테이너를 사용하는 경우 저장소 시스템을 추가합니다.
...
1. 데이터 추가(필요한 경우)
   ...
   Avere vFXT는 확장 가능한 다중 클라이언트 캐시이므로 데이터를 새 백 엔드 저장소 컨테이너로 이동하는 가장 좋은 방법은 다중 클라이언트, 다중 스레드 파일 복사 전략을 사용한다는 것입니다.

   작업 집합 데이터를 새 Blob 컨테이너 또는 기타 백 엔드 저장소 시스템으로 이동해야 하는 경우 [데이터를 vFXT 클러스터로 이동의](avere-vfxt-data-ingest.md)지침을 따릅니다.

## <a name="next-steps"></a>다음 단계

[Avere vFXT를 만들어](avere-vfxt-prereqs.md) 필수 구성 조건 작업을 완료할 준비를 계속합니다.
