...
VFXT 클러스터가 실행 되 고 나면 클라이언트를 연결 하는 방법 및 (선택 사항) 새 Blob 저장소 컨테이너로 데이터를 이동 하는 방법을 알고 싶을 것입니다. NAS 저장소 시스템을 사용 하는 경우 클러스터를 만든 후에 추가 해야 합니다.
...
1. Avere vFXT 클러스터 만들기
   ...
   * *클러스터 컨트롤러* 만들기  
     ...
     배포 하는 동안 새 가상 네트워크 또는 서브넷을 만드는 경우 컨트롤러는 공용 IP 주소를 갖게 됩니다. 즉, 컨트롤러는 서브넷 외부에서 클러스터에 연결 하기 위한 점프 호스트로 사용할 수 있습니다.

   * 클러스터 노드 Vm 만들기

   * 클러스터 노드 Vm을 구성 하 여 클러스터 구성
