...
새 vFXT 클러스터를 만든 후 첫 번째 작업은 Azure의 새 저장소 볼륨으로 데이터를 이동하는 것입니다. 그러나 데이터를 이동하는 일반적인 방법으로 한 클라이언트에서 단순 복사 명령을 실행하는 경우 복사 성능이 저하될 수 있습니다. 단일 스레드 복사는 Avere vFXT 클러스터의 백 엔드 저장소에 데이터를 복사하는 데 좋은 옵션이 아닙니다.

Azure 클러스터용 Avere vFXT는 확장 가능한 다중 클라이언트 캐시이므로 데이터를 복사하는 가장 빠르고 효율적인 방법은 여러 클라이언트를 사용합니다. 이 기술은 파일과 개체의 수집을 병렬로 처리합니다.
...
또한 도움이 되는 몇 가지 유틸리티도 설명합니다. 이 ``msrsync`` 유틸리티는 데이터 집합을 버킷으로 나누고 명령을 사용하는 ``rsync`` 프로세스를 부분적으로 자동화하는 데 사용할 수 있습니다. ``parallelcp`` 스크립트는 원본 디렉터리를 읽고 복사 명령을 자동으로 실행하는 또 다른 유틸리티입니다. 또한 이 ``rsync`` 도구를 두 단계로 사용하여 데이터 일관성을 제공하는 더 빠른 복사본을 제공할 수 있습니다.

다음 링크를 클릭하여 해당 섹션으로 이동합니다.

* [수동 복사 예제](#manual-copy-example) - 복사 명령 사용에 대한 완전한 설명
* [2상 rsync 예제](#use-a-two-phase-rsync-process)
...
데이터를 병렬로 복사하는 전략을 설계할 때는 파일 크기, 파일 수 및 디렉터리 깊이의 장단점을 이해해야 합니다.

* 파일이 작은 경우 관심 있는 메트릭은 초당 파일 수입니다.
* 파일이 큰 경우(10MiBi 이상) 관심 있는 메트릭은 초당 바이트 수입니다.

각 복사 프로세스에는 복사 명령의 길이를 타이밍으로 측정하고 파일 크기와 파일 수를 팩터링하여 측정할 수 있는 처리량 속도와 파일 전송 속도가 있습니다. 속도를 측정하는 방법을 설명하는 것은 이 문서의 범위를 벗어나지만, 작거나 큰 파일을 다룰 지 여부를 이해하는 것이 중요합니다.
...
단일 대상 파일 시스템 탑재 지점에 대해 충분한 병렬 스레드가 있으면 더 많은 스레드를 추가해도 처리량이 늘어나지 않습니다. 처리량은 데이터 유형에 따라 파일/초 또는 바이트/초단위로 측정됩니다. 또는, 오버 스레딩 처리량 저하를 일으킬 수 있습니다.
...
표준 ``rsync`` 유틸리티는 데이터 무결성을 보장하기 위해 많은 수의 파일 만들기 및 이름 바꾸기 작업을 생성하기 때문에 Azure 시스템의 Avere vFXT를 통해 클라우드 저장소를 채우는 데 는 잘 작동하지 않습니다. 그러나 파일 ``--inplace`` ``rsync`` 무결성을 검사하는 두 번째 실행을 따르는 경우 보다 신중한 복사 절차를 건너뛸 수 있는 옵션을 안전하게 사용할 수 있습니다.

표준 ``rsync`` 복사 작업은 임시 파일을 만들고 데이터로 채웁니다. 데이터 전송이 성공적으로 완료되면 임시 파일의 이름이 원래 파일 이름으로 바뀝니다. 이 메서드는 복사 하는 동안 파일에 액세스 하는 경우에 일관성을 보장 합니다. 그러나 이 메서드는 더 많은 쓰기 작업을 생성하여 캐시를 통한 파일 이동을 느리게 합니다.

이 ``--inplace`` 옵션은 최종 위치에 새 파일을 직접 씁니다. 파일이 전송 중에 일관되게 보장되지는 않지만 나중에 사용할 저장소 시스템을 프라이밍하는 경우에는 중요하지 않습니다.

두 ``rsync`` 번째 작업은 첫 번째 작업에 대한 일관성 검사 역할을 합니다. 파일이 이미 복사되었기 때문에 두 번째 단계는 대상의 파일이 원본의 파일과 일치하는지 확인하는 빠른 검사입니다. 일치하지 않는 파일이 있으면 다시 복사됩니다.

하나의 명령으로 두 단계를 함께 발행할 수 있습니다.

```bash
rsync -azh --inplace <source> <destination> && rsync -azh <source> <destination>
```

이 메서드는 내부 디렉터리 관리자가 처리할 수 있는 파일 수까지 데이터 집합에 대한 간단하고 시간 효율적인 방법입니다. 일반적으로 3노드 클러스터의 경우 2억 개, 6노드 클러스터의 경우 5억 개의 파일 등이 있습니다.

## <a name="use-the-msrsync-utility"></a>msrsync 유틸리티 사용

이 ``msrsync`` 도구는 Avere 클러스터의 백 엔드 코어 파일러로 데이터를 이동하는 데도 사용할 수 있습니다. 이 도구는 여러 개의 ``rsync`` 병렬 프로세스를 실행하여 대역폭 사용량을 최적화하도록 설계되었습니다. GitHub의 <https://github.com/jbd/msrsync>에서 사용할 수 있습니다.
...
명령과 ``--inplace`` 함께 ``msrsync`` 인수를 사용할 수도 있습니다. 이 옵션을 사용하는 경우 데이터 무결성을 보장하기 위해 위에서 설명한 [rsync와](#use-a-two-phase-rsync-process)마찬가지로 두 번째 명령을 실행하는 것이 좋습니다.

``msrsync``로컬 볼륨에서만 쓸 수 있습니다. 원본 및 대상은 클러스터의 가상 네트워크에서 로컬 탑재로 액세스할 수 있어야 합니다.

Azure ``msrsync`` 클라우드 볼륨을 Avere 클러스터로 채우는 데 사용하려면 다음 지침을 따르십시오.

1. 설치 ``msrsync`` 및 필수 구성 조건(rsync 및 Python 2.6 이상)
1. 복사할 파일 및 디렉터리의 총 수를 결정합니다.

   예를 들어 Avere 유틸리티를 ``prime.py`` ```prime.py --directory /path/to/some/directory``` 인수(URL <https://github.com/Azure/Avere/blob/master/src/clientapps/dataingestor/prime.py>다운로드로 사용 가능)와 함께 사용합니다.
...
1. 파일을 ``msrsync`` 복사하는 명령을 내보세요.
   ...
   을 ``--inplace``사용하는 경우 데이터가 올바르게 복사되었는지 확인하는 옵션 없이 두 번째 실행을 추가합니다.
...
스크립트는 ``parallelcp`` vFXT 클러스터의 백 엔드 저장소로 데이터를 이동하는 데에도 유용할 수 있습니다.
