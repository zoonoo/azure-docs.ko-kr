...
* **자동 호스트 레코드 관리**. Azure는 사용자 지정 DNS 레코드를 호스팅할 뿐 아니라 VM의 호스트 이름 레코드를 지정된 가상 네트워크에 자동으로 보존합니다. 이 시나리오에서는 사용자 지정 DNS 솔루션을 만들거나 응용 프로그램을 수정할 필요 없이 사용하는 도메인 이름을 최적화할 수 있습니다.
...
Azure DNS에는 다음과 같은 제한 사항이 적용됩니다.
