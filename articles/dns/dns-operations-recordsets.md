...
New-AzureRmDnsRecordSet cmdlet을 사용하여 레코드 집합을 만듭니다. 레코드 집합 이름, 영역, TTL(Time-to-Live) 및 레코드 형식을 지정해야 합니다.
...
## 레코드 집합 나열
-Name 및/또는 -RecordType 매개 변수를 생략하면 Get-AzureRmDnsRecordSet를 사용하여 레코드 집합을 나열할 수도 있습니다.
...
1.	Get-AzureRmDnsRecordSet를 사용하여 기존 레코드 집합을 가져옵니다.
...
Set-AzureRmDnsRecordSet cmdlet은 'Etag' 검사를 사용하여 동시 변경 내용을 덮어쓰지 않도록 합니다. '-Overwrite' 플래그를 사용하면 이러한 검사가 무시됩니다. 자세한 내용은 Etag 및 태그를 참조하세요.
...
Remove-AzureRmDnsRecordConfig를 사용하여 레코드 집합에서 레코드를 제거할 수 있습니다. 제거되는 레코드는 모든 매개 변수에서 기존 레코드와 정확히 일치해야 합니다. Set-AzureRmDnsRecordSet를 사용하여 변경 내용을 커밋해야 합니다.
...
## 레코드 집합 삭제
Remove-AzureRmDnsRecordSet cmdlet을 사용하여 레코드 집합을 삭제할 수 있습니다.
