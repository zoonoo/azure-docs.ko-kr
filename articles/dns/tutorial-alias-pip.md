...
2. 포털의 왼쪽 상단에서 **리소스 만들기**를 클릭하고 검색 상자에 *리소스 그룹*을 입력한 후 **RG-DNS-Alias-pip**라는 리소스 그룹을 만듭니다.
...
4. **설정**에서 **VNet-Servers** 가상 네트워크, **SN-Web** 서브넷을 선택합니다. 공용 인바운드 포트의 경우 **HTTP**, **HTTPS** 및 **RDP(3389)** 를 선택한 다음, **확인**을 클릭합니다.
5. 요약 페이지에서 **만들기**를 클릭합니다.
...
2. **레코드 집합**을 클릭합니다.
3. **이름** 텍스트 상자에 **web01**을 입력합니다.
4. **유형**을 **A** 레코드로 둡니다.
5. **별칭 레코드 집합** 확인란을 클릭합니다.
6. **Azure 서비스 선택**을 클릭하고 **Web-01-ip** 공용 IP 주소를 선택합니다.

## <a name="test-the-alias-record"></a>별칭 레코드 테스트

1. **RG-DNS-Alias-pip** 리소스 그룹에서 **Web-01** 가상 머신을 클릭합니다. 공용 IP 주소를 기록해둡니다.
1. 웹 브라우저에서 Web01-01 가상 머신의 정규화된 도메인 이름을 찾습니다. 예: **web01.contoso.com** IIS 기본 웹 페이지가 표시됩니다.
2. 웹 브라우저를 닫습니다.
3. **Web-01** 가상 머신을 중지한 다음, 다시 시작합니다.
4. 다시 시작할 때 가상 머신에 대한 새 공용 IP 주소를 기록해둡니다.
5. 새 브라우저를 열고 Web01-01 가상 머신의 정규화된 도메인 이름을 다시 찾습니다. 예: **web01.contoso.com**
6. 표준 A 레코드가 아닌 공용 IP 주소 리소스를 가리키는 별칭 레코드를 사용했으므로 여전히 성공합니다.

## <a name="clean-up-resources"></a>리소스 정리

더 이상 필요하지 않은 경우 **RG-DNS-Alias-pip** 리소스 그룹을 삭제하여 이 자습서에 대해 생성된 리소스를 모두 삭제할 수 있습니다.
