...
이 문서는 Windows, Mac 및 Linux용으로 제공되는 플랫폼 간 Azure CLI(명령줄 인터페이스)를 사용하여 DNS 영역에 대한 DNS 레코드를 관리하는 방법을 보여 줍니다. [Azure PowerShell](dns-operations-recordsets.md) 또는 [Azure Portal](dns-operations-recordsets-portal.md)을 사용하여 DNS 레코드를 관리할 수도 있습니다.
...
영역의 구로에서 레코드를 만들려면(이 경우 "contoso.com"), 따옴표를 포함한 레코드 이름 "@"를 사용합니다.
...
`azure network dns record-set create` 명령을 사용하여 레코드 집합을 만듭니다. 도움말을 보려면 `azure network dns record-set create -h`을 참조하세요.
...
다음 예제에서는 `--metadata` 매개 변수(약식 `-m`)를 사용하여 "dept=finance" 및 "environment=production"라는 두 개의 메타데이터 항목을 가진 레코드 집합을 만듭니다.
...
레코드 데이터를 지정하는 데 사용되는 매개 변수는 레코드 유형에 따라 다릅니다. 예를 들어 "A" 유형의 레코드의 경우 `-a <IPv4 address>` 매개 변수로 IPv4 주소를 지정합니다. 각 레코드 형식에 대한 매개 변수는 `azure network dns record-set add-record -h`를 사용하여 나열할 수 있습니다.
...
기존 레코드 집합을 가져오려면, `azure network dns record-set show`를 사용합니다. 도움말을 보려면 `azure network dns record-set show -h`을 참조하세요.
...
`azure network dns record-set list` 명령을 사용하여 DNS 영역에 있는 모든 레코드를 나열할 수 있습니다. 도움말을 보려면 `azure network dns record-set list -h`을 참조하세요.
...
`azure network dns record-set add-record`를 사용하여 새 레코드 집합에 레코드를 만들거나 기존 레코드 집합에 레코드를 추가할 수 있습니다.
...
기존 레코드 집합에서 DNS 레코드를 제거하려면 `azure network dns record-set delete-record`를 사용합니다. 도움말을 보려면 `azure network dns record-set delete-record -h`을 참조하세요.
...
이 명령은 확인 메시지를 표시합니다. 이 프롬프트는 `--quiet` 스위치(약식 `-q`)를 사용하여 표시되지 않게 할 수 있습니다.
...
`azure network dns record-set set-soa-record`를 사용하여 지정된 DNS 영역에 대해 SOA를 수정합니다. 도움말을 보려면 `azure network dns record-set set-soa-record -h`을 참조하세요.
...
[레코드 집합 메타데이터](dns-zones-records.md#tags-and-metadata)는 키-값 쌍의 형태로 각 레코드 집합과 응용 프로그램 특정 데이터를 연결하는 데 사용할 수 있습니다. 기존 레코드 집합의 메타데이터를 수정하려면 `azure network dns record-set set`을 사용합니다. 도움말을 보려면 `azure network dns record-set set -h`을 참조하세요.
...
삭제 작업을 확인하라는 메시지가 표시됩니다. 이 프롬프트를 표시하지 않으려면 `--quiet` 스위치(약식 `-q`)를 사용합니다.
