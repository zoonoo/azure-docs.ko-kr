...
이러한 지침은 Microsoft Azure PowerShell을 사용합니다. Azure DNS cmdlet을 사용하려면 최신 Azure PowerShell로 업데이트해야 합니다. 또한 Azure 포털 또는 플랫폼 간 CLI를 사용하여 동일한 단계를 실행할 수 있습니다.

Azure PowerShell을 사용하여 Azure DNS를 관리하려면 먼저 다음 단계를 완료해야 합니다.

Azure DNS는 ARM(Azure 리소스 관리자)을 사용합니다. Azure PowerShell 1.0.0 이상은 이 지침을 따릅니다. 자세한 내용은 [Azure Resource Manager에서 Azure PowerShell 사용](../powershell-azure-resource-manager.md)을 참조하세요.
...
첫 번째 DNS 영역을 만들었으므로 nslookup, dig 또는 [Resolve-DnsName PowerShell cmdlet](https://technet.microsoft.com/library/jj590781.aspx)과 같은 DNS 도구를 사용하여 테스트할 수 있습니다.<BR>

Azure DNS에서 새 영역을 사용하도록 도메인을 아직 위임하지 않은 경우 DNS 쿼리를 영역에 대한 이름 서버 중 하나로 직접 보내야 합니다. 위의 Get-AzureRmDnsRecordSet에 의해 나열된 것처럼 영역에 대한 이름 서버는 NS 레코드에 제공되며, 아래 명령에 영역에 대한 올바른 값을 대체해야 합니다.<BR>
