...
1. 로컬 DNS 서버는 해당 레코드를 갖고 있지 않으므로 루트 이름 서버에 요청을 보냅니다.
1. 루트 이름 서버는 해당 레코드를 갖고 있지는 않지만 `.net` 이름 서버의 주소를 알고 있으므로 DNS 서버에 해당 주소를 제공합니다.
1. DNS는 `.net` 이름 서버로 요청을 보내며, 해당 레코드를 갖고 있지는 않지만 contoso.net 이름 서버의 주소를 알고 있습니다. 이 예에서는 Azure DNS에 호스팅되는 DNS 영역입니다.
1. `contoso.net` 영역은 해당 레코드를 갖고 있지는 않지만 `partners.contoso.net`에 대한 이름 서버를 알고 있으며 그 이름 서버를 사용하여 응답합니다. 이 예에서는 Azure DNS에 호스팅되는 DNS 영역입니다.
1. DNS 서버가 `partners.contoso.net` 영역에서 `partners.contoso.net`의 IP 주소를 요청합니다. DNS 서버는 A 레코드를 포함하고 있으며 IP 주소를 사용하여 응답합니다.
1. DNS 서버가 클라이언트에 IP 주소를 제공합니다.
1. 클라이언트가 `www.partners.contoso.net` 웹 사이트에 연결합니다.
