...
다중 테넌트 응용 프로그램은 "별도의 테넌트의 사용자"가 자신의 것처럼 응용 프로그램을 볼 수 있도록 하는 공유 리소스입니다. 다중 테넌트 응용 프로그램에 자신을 빌려주는 일반적인 시나리오는 다른 테넌트의 응용 프로그램의 모든 사용자가 사용자 환경을 사용자 지정하려고 하지만 그렇지 않으면 동일한 기본 비즈니스 요구 사항이 있는 시나리오입니다. 대규모 다중 테넌트 애플리케이션의 예로는 Office 365, Outlook.com 및 visualstudio.com이 있습니다.
...
* **사용자 정의 .** 기능 추가 또는 제거, 색 및 로고 변경, 고유 코드 또는 스크립트 추가 등과 같은 다양한 방식으로 개별 테넌트에 맞게 애플리케이션을 사용자 지정하는 기능입니다.

즉, 확장성이 뛰어난 서비스를 제공하기 위해 고려해야 할 많은 고려 사항이 있지만 많은 다중 테넌트 응용 프로그램에 공통적인 많은 목표와 요구 사항도 있습니다. 일부는 특정 시나리오에서 적절하지 않을 수 있으며, 개별 목표와 요구 사항의 중요성은 시나리오별로 다릅니다. 다중 테넌트 응용 프로그램의 공급자는 테넌트의 목표 및 요구 사항, 수익성, 청구, 다중 서비스 수준, 프로비저닝, 유지 관리 모니터링 및 자동화를 충족하는 목표와 요구 사항도 갖습니다.
...
* TLS 통신 유무에 관계없이 호스트 헤더별로 웹 사이트 테넌트 분할
* 쿼리 매개 변수로 웹 사이트 테넌트 분할
* 작업자 역할의 웹 서비스
  * 일반적으로 응용 프로그램의 백 엔드에서 데이터를 처리하는 작업자 역할입니다.
  * 웹 역할. 일반적으로 애플리케이션의 프런트 엔드 역할을 합니다.

**스토리지**

많은 양의 비정형 데이터를 저장하는 서비스를 제공하는 Table 서비스 및 비디오, 오디오 및 이미지와 같은 많은 양의 비정형 텍스트 또는 이진 데이터를 저장하는 서비스를 제공하는 Blob 서비스와 같은 Azure SQL Database 또는 Azure Storage 서비스와 같은 데이터 관리입니다.

* 테넌트별 SQL Server 로그인당 SQL 데이터베이스의 다중 테넌트 데이터 보안
* 컨테이너 수준 액세스 정책을 지정하여 응용 프로그램 리소스용 Azure 테이블을 사용하면 공유 액세스 서명으로 보호되는 리소스에 대해 새 URL을 발급하지 않고도 권한을 조정할 수 있습니다.
...
* Azure AD(Azure Active Directory)는 클라우드 애플리케이션과 관련하여 ID를 관리하고 액세스를 제어할 수 있게 하는 최신 REST 기반 서비스입니다. 응용 프로그램 리소스용 Azure AD를 사용하면 인증 및 권한 부여 기능을 코드에서 제외할 수 있도록 하면서 사용자가 웹 응용 프로그램 및 서비스에 액세스할 수 있도록 인증하고 권한을 쉽게 부여할 수 있습니다.
* Azure Service Bus는 분산 및 하이브리드 애플리케이션에 대해 복잡한 방화벽 및 보안 인프라 없이도 Azure 호스트된 애플리케이션과 온-프레미스 애플리케이션 및 서비스 사이의 통신과 같은 보안 메시징 및 데이터 흐름 기능을 제공합니다. 응용 프로그램 리소스에 대한 Service Bus 릴레이를 사용하여 끝점으로 노출된 서비스에 액세스하는 것은 테넌트에 속할 수 있습니다(예: 온-프레미스와 같은 시스템 외부에서 호스팅됨) 테넌트에 맞게 특별히 프로비전된 서비스일 수 있습니다(민감한 테넌트별 데이터가 테넌트 간에 이동하기 때문에).

**리소스 프로비전**

Azure는 응용 프로그램에 대한 새 테넌을 프로비전하는 여러 가지 방법을 제공합니다. 테넌트가 많은 다중 테넌트 애플리케이션의 경우 셀프 서비스 프로비전을 사용하여 이 프로세스를 자동화해야 하는 경우가 많습니다.
...
* 테넌트용으로 SQL Database 리소스를 프로비전하는 옵션에는 다음이 포함됩니다.
  ...
  * 스크립트의 DDL 또는 어셈블리 내의 리소스로 임베디드됩니다.
