...
많은 양의 비정형 데이터를 저장하기 위한 서비스를 제공하는 Table 서비스 및 많은 양의 비정형 텍스트를 저장하는 서비스를 제공하는 Blob 서비스와 같은 Azure SQL Database 또는 Azure Storage 서비스와 같은 데이터 관리 또는 비디오, 오디오 및 이미지와 같은 이진 데이터.
...
* Azure Service Bus는 분산 및 하이브리드 애플리케이션에 대해 복잡한 방화벽 및 보안 인프라 없이도 Azure 호스트된 애플리케이션과 온-프레미스 애플리케이션 및 서비스 사이의 통신과 같은 보안 메시징 및 데이터 흐름 기능을 제공합니다. 응용 프로그램 리소스에 대한 서비스 버스 릴레이를 사용하여 끝점으로 노출된 서비스에 액세스하는 것은 테넌트에 속할 수 있습니다(예: 온-프레미스와 같은 시스템 외부에서 호스팅됨) 또는 테넌트용으로 특별히 프로비전된 서비스일 수 있습니다( 테넌트별 중요한 데이터가 해당 데이터 전체로 이동합니다.
