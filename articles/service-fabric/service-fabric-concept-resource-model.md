...
1. *.\Service-fabric-dotnet-quickstart\Voting\pkg\Debug* 디렉터리로 이동 합니다. 콘텐츠를 *응답 .zip*이라는 파일로 압축 합니다. *Applicationmanifest .xml* 파일은 zip 파일의 루트에 있어야 합니다.
...
샘플 응용 프로그램에는 응용 프로그램을 배포 하는 데 사용할 수 있는 [Azure Resource Manager 템플릿이](https://github.com/Azure-Samples/service-fabric-dotnet-quickstart/tree/master/ARM) 포함 되어 있습니다. 템플릿 파일 이름은 *Userapp. json* 및 *Userapp. Parameters입니다*.
...
| applicationTypeName    | 응용 프로그램의 형식 이름입니다.           | VotingType                                                   | ApplicationManifest과 일치 해야 합니다.                 |
| applicationTypeVersion | 응용 프로그램 유형의 버전입니다.         | 1.0.0                                                        | ApplicationManifest과 일치 해야 합니다.                 |
| serviceName            | 서비스의 이름입니다.         | 투표 ~ VotingWeb                                             | ApplicationName ~ ServiceType 형식 이어야 합니다.            |
| serviceTypeName        | 서비스의 형식 이름입니다.                | VotingWeb                                                    | Servicemanifest.xml와 일치 해야 합니다.                 |
...
* 새 서비스가 응용 프로그램에 추가 됩니다. 서비스가 응용 프로그램에 추가 되 면 서비스 정의를 *service-manifest* 및 *application-manifest* 파일에 추가 해야 합니다. 응용 프로그램의 새 버전을 반영 하려면 응용 프로그램 형식 버전을 1.0.0에서 [1.0.1의 응용](https://github.com/Azure-Samples/service-fabric-dotnet-quickstart/blob/master/ARM/UserApp.Parameters.json)프로그램 형식 버전으로 변경 해야 합니다.
...
* 기존 서비스의 새 버전이 응용 프로그램에 추가 됩니다. 응용 프로그램 코드 변경 내용 및 앱 형식 버전 및 이름에 대 한 업데이트를 예로 들 수 있습니다. 이 업그레이드의 경우 다음과 같이 UserApp. Parameters를 업데이트 합니다.
