---
title: Azure 서비스 패브릭 CLI-sfctl 응용 프로그램
description: Azure 서비스 패브릭 명령줄 인터페이스인 sfctl에 대해 알아봅니다. 응용 프로그램 관리를 위한 명령 목록을 포함합니다.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| provision | 외부 저장소의 '.sfpkg' 패키지를 사용하거나 이미지 저장소의 응용 프로그램 패키지를 사용하여 클러스터에 서비스 패브릭 응용 프로그램 유형을 프로비저닝하거나 등록합니다. |
...
외부 저장소의 '.sfpkg' 패키지를 사용하거나 이미지 저장소의 응용 프로그램 패키지를 사용하여 클러스터에 서비스 패브릭 응용 프로그램 유형을 프로비저닝하거나 등록합니다.

클러스터를 통해 Service Fabric 애플리케이션 유형을 프로비전합니다. 새 응용 프로그램을 인스턴스화하려면 앞에 프로비전이 필요합니다. 프로비저닝 작업은 relativePathInImageStore에서 지정한 응용 프로그램 패키지또는 외부 '.sfpkg'의 URI를 사용하여 수행할 수 있습니다. 외부 프로비전이 설정되지 않은 경우 이 명령은 이미지 저장소 프로비전이 필요합니다.
...
|인수|설명|
| --- | --- |
| --경로 [필수] | 로컬 애플리케이션 패키지 경로입니다. |
| --압축 | 서비스 패브릭 응용 프로그램 패키지에만 적용됩니다. 압축된 응용 프로그램 패키지가 포함된 새 폴더를 기본 위치 또는 압축 위치 매개 변수에 의해 지정된 위치로 만든 다음 새로 만든 폴더를 업로드합니다. <br><br> sfctl에 의해 생성된 압축 파일이 이미 있는 경우 이 플래그가 설정된 경우 덮어씁을 작성합니다. 디렉터리응용 프로그램 패키지가 아닌 경우 오류가 반환됩니다. 이미 압축된 응용 프로그램 패키지인 경우 폴더가 있는 것처럼 복사됩니다. 기본적으로 새로 생성된 압축 된 응용 프로그램 패키지는 성공적으로 업로드한 후 삭제됩니다. 업로드에 성공하지 못하면 필요에 따라 압축된 패키지를 수동으로 정리하십시오. 삭제는 압축된 위치 매개 변수가 존재하지 않는 디렉터리를 참조하는 경우 생성되었을 수 있는 빈 디어를 제거하지 않습니다. |
| --압축 위치 | 압축된 응용 프로그램 패키지를 넣을 위치입니다. <br><br> 위치가 제공되지 않으면 압축된 패키지는 경로 인수에 지정된 부모 디렉터리 아래에 sfctl_compressed_temp라는 새로 생성된 폴더 아래에 배치됩니다. 예를 들어 경로 인수에 C\:/FolderA/AppPkg 값이 있는 경우 압축된\:패키지가 C/FolderA/sfctl_compressed_temp/AppPkg에 추가됩니다. |
| --imagestore-string | 애플리케이션 패키지를 업로드할 대상 이미지 저장소입니다.  기본값\: fabric\:ImageStore. <br><br> 파일 위치에 업로드하려면 '파일'로\:이 매개변수를 시작합니다. 그렇지 않으면 값은 기본값과 같은 이미지 저장소 연결 문자열이어야 합니다. |
| --유지 압축 | 성공적인 업로드 완료 시 생성된 압축 패키지를 유지할지 여부입니다. <br><br> 설정하지 않으면 성공적으로 완료되면 압축된 앱 패키지가 삭제됩니다. 업로드에 성공하지 못하면 응용 프로그램 패키지는 항상 다시 업로드할 수 있는 출력 디렉토리에 보관됩니다. |
| --show-progress | 큰 패키지에 대한 파일 업로드 진행률을 표시합니다. |
| --timeout -t | 초 단위로의 총 시간 시간 입니다. 업로드 시간 시간 시간이 경과한 후 업로드가 실패하고 오류가 반환됩니다. 이 시간 제한은 전체 응용 프로그램 패키지에 적용되며 개별 파일 시간 제한은 남은 시간 제한 기간과 같습니다. 시간 시간에는 응용 프로그램 패키지를 압축하는 데 필요한 시간이 포함되지 않습니다.  기본\: 값 300. |
