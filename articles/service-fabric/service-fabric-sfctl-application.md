...
외부 저장소의 ‘.sfpkg’ 패키지를 사용하거나 Image Store의 애플리케이션 패키지를 사용하는 클러스터를 통해 Service Fabric 애플리케이션 유형을 프로비저닝하거나 등록합니다.

클러스터를 통해 Service Fabric 애플리케이션 유형을 프로비전합니다. 새 애플리케이션을 인스턴스화하려면 프로비저닝이 필요합니다. 프로비저닝 작업은 relativePathInImageStore에 지정된 애플리케이션 패키지에서 수행할 수도 있고 외부 ‘.sfpkg’의 URI를 사용하여 수행할 수도 있습니다. 외부 프로비전이 설정되지 않은 경우 이 명령은 이미지 저장소 프로비전이 필요합니다.
...
| --compress | Service Fabric 애플리케이션 패키지에만 적용됩니다. 기본 위치 또는 compressed-location 매개 변수로 지정된 위치에 압축 애플리케이션 패키지가 포함된 새 폴더를 만든 다음 새로 만든 폴더를 업로드합니다. <br><br> sfctl에서 생성된 압축 파일이 이미 있는 경우 이 플래그가 설정되면 해당 파일을 덮어씁니다. 디렉터리가 애플리케이션 패키지가 아닌 경우 오류가 반환됩니다. 이미 압축 애플리케이션 패키지인 경우 폴더가 그대로 복사됩니다. 기본적으로 업로드가 성공하면 새로 만든 압축 애플리케이션 패키지는 삭제됩니다. 업로드가 실패하면 필요에 따라 압축 패키지를 수동으로 정리하세요. compressed location 매개 변수가 존재하지 않는 디렉터리를 참조하는 경우 삭제를 수행해도 만들어져 있는 빈 디렉터리가 제거되지 않습니다. |
| --compressed-location | 압축 애플리케이션 패키지를 배치할 위치입니다. <br><br> 위치를 제공하지 않으면 path 인수에 지정된 부모 디렉터리 아래에 있는 sfctl_compressed_temp라는 새로 만든 폴더에 압축 패키지가 배치됩니다. 예를 들어 path 인수 값이 C\:/FolderA/AppPkg이면 C\:/FolderA/sfctl_compressed_temp/AppPkg에 압축 패키지가 추가됩니다. |
| --imagestore-string | 애플리케이션 패키지를 업로드할 대상 이미지 저장소입니다.  기본값\: fabric\:ImageStore. <br><br> 파일 위치에 업로드하려면 이 매개 변수를 ‘file\:’으로 시작합니다. 그렇지 않으면 기본값과 같은 Image Store 연결 문자열이 값이 됩니다. |
| --keep-compressed | 업로드가 성공적으로 완료된 후 생성된 압축 패키지를 유지할 것인지 지정합니다. <br><br> 설정하지 않으면 업로드가 성공적으로 완료된 후 압축 앱 패키지가 삭제됩니다. 업로드가 실패하면 다시 업로드할 수 있도록 항상 출력 디렉터리에 애플리케이션 패키지가 유지됩니다. |
| --show-progress | 큰 패키지에 대한 파일 업로드 진행률을 표시합니다. |
| --timeout -t | 총 시간 제한(초)입니다. 업로드 시간 제한 기간이 경과되면 업로드가 실패하고 오류를 반환합니다. 이 시간 제한은 전체 애플리케이션 패키지에 적용되며, 개별 파일의 시간 제한은 남은 시간 제한 기간과 동일합니다. 시간 제한에는 애플리케이션 패키지를 압축하는 데 필요한 시간은 포함되지 않습니다.  기본값\: 300. |
