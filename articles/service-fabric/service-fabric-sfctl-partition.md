---
title: Azure 서비스 패브릭 CLI-sfctl 파티션
description: Azure 서비스 패브릭 명령줄 인터페이스인 sfctl에 대해 알아봅니다. 서비스에 대한 파티션을 관리하기 위한 명령 목록을 포함합니다.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
- PartialDataLoss: 복제본의 쿼럼만 제거되고 OnDataLoss는 파티션에 대해 트리거되지만 실제 데이터 손실은 현재 상태 복제의 존재에 따라 달라집니다.  
- FullDataLoss: 모든 복제본이 제거되므로 모든 데이터가 손실되고 OnDataLoss가 트리거됩니다. 이 API는 대상으로 상태 저장 서비스로만 호출되어야 합니다. 시스템 서비스를 대상으로 이 API를 호출하는 것은 권장되지 않습니다.
