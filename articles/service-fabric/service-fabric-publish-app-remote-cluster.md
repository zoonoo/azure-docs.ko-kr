...
2. **연결 끝점** 섹션에서 로컬 또는 원격 서비스 패브릭 클러스터의 게시 끝점을 지정합니다. 연결 끝점을 추가하거나 변경하려면 **선택...** 단추를 선택합니다. **서비스 패브릭 클러스터 선택** 대화 상자는 Azure 구독에 따라 게시할 수 있는 사용 가능한 서비스 패브릭 클러스터 연결 끝점을 표시합니다. Visual Studio에 아직 로그인하지 않은 경우 로그인하라는 메시지가 표시됩니다.
