<properties
   ...
   description="서비스 패브릭 Reliable Actors를 사용하여 응용 프로그램을 분산 네트워크 및 그래프로 모델링하는 방법에 대한 패턴을 설계합니다."
...
# Reliable Actors 설계 패턴: 분산 네트워크 및 그래프
Azure 서비스 패브릭 Reliable Actors 프로그래밍 모델은 관계와 관련된 복잡한 솔루션을 모델링하고 이러한 관계를 개체로 모델링하는 데 가장 적합합니다.

![Azure 서비스 패브릭 Reliable Actors 모델링][1]

위의 다이어그램에 나온 것처럼 사용자를 행위자 인스턴스(네트워크의 노드)로 모델링하는 작업은 간단합니다. 예를 들어 “친구 피드”("팔로워" 문제라고도 함)를 사용하면 사용자는 Facebook 및 Twitter의 작동 방식과 비슷하게 연결되어 있는 사람들의 상태 업데이트를 볼 수 있습니다.

Reliable Actors 모델은 구체화 문제에 유연한 접근 방식을 제공합니다. 아래 그림과 같이 업데이트가 게시되는 순간 모든 친구의 친구 피드를 업데이트하여 이벤트 시간에 친구 피드를 채울 수 있습니다.

![Reliable Actors 모델 및 친구 피드 채우기][2]
...
또는 사용자가 친구 피드를 요청할 때 쿼리 타이머에서 행위자를 모델링하여 친구 피드를 팬아웃 및 컴파일할 수 있습니다. 타이머에서 친구 피드를 구체화할 수도 있습니다.(예를 들어 5분 마다) 또는 모델을 최적화하고 로그인 빈도 또는 업데이트 게시 빈도와 같은 사용자 습관에 따르는 타이머 기반 모델을 사용하여 처리하는 쿼리 시간과 이벤트 시간을 결합할 수도 있습니다.

소셜 네트워크에서 행위자를 모델링할 때 수백만의 팔로워가 있는 "슈퍼 사용자"를 고려해야 합니다. 개발자는 더 큰 요구 사항을 충족하도록 해당 사용자의 상태와 동작을 다르게 모델링해야 합니다.

비슷하게 여러 사용자 행위자를 단일 활동 행위자(허브 및 스포크)에 연결하는 활동을 모델링하려는 경우에도 마찬가지로 수행할 수 있습니다. 그룹 채팅 및 게임 호스팅은 두 가지 예제입니다. 그룹 채팅 예를 살펴보겠습니다. 일련의 참가자는 한 참가자의 메시지를 그룹에 배포할 수 있는 그룹 채팅 행위자를 만듭니다. 아래 예제에 나와 있습니다.
...
이 방법은 Reliable Actors 모델의 기능을 활용하여 행위자가 ID별로 클러스터의 다른 모든 행위자를 해결할 수 있도록 합니다. 배치, 주소 지정, 캐싱, 메시징, 직렬화 또는 라우팅에 대해 걱정할 필요 없이 통신할 수 있습니다.
