...
description: "각 노드 형식/가상 컴퓨터 크기 집합에 대한 자동 크기 조정 규칙을 설정하여 수요에 따라 Service Fabric 클러스터의 크기를 조정합니다. 서비스 패브릭 클러스터에 노드 추가 또는 제거"
...
가상 컴퓨터 크기 집합은 가상 컴퓨터의 컬렉션을 집합으로 배포하고 관리하는 데 사용할 수 있는 Azure 계산 리소스입니다. Service Fabric 클러스터에 정의된 모든 노드 형식은 별도의 가상 컴퓨터 크기 집합으로 설정됩니다. 각 노드 형식은 독립적으로 확장 또는 축소되고, 다른 포트의 집합을 열며 다른 용량 메트릭을 가질 수 있습니다. [서비스 패브릭 노드 형식](service-fabric-cluster-nodetypes.md) 문서에서 자세히 알아보세요. 클러스터에서 Service Fabric 노드 형식은 백 엔드에서 가상 컴퓨터 크기 집합으로 구성되므로 각 노드 형식/가상 컴퓨터 크기 집합에 대한 자동 크기 조정 규칙을 설정해야 합니다.
...
## <a name="choose-the-node-typevirtual-machine-scale-set-to-scale"></a>크기를 조정할 노드 형식/가상 컴퓨터 크기 집합 선택
현재는 포털을 사용하여 가상 컴퓨터 크기 집합에 대한 자동 크기 조정 규칙을 지정할 수 없으므로 Azure PowerShell(1.0+)을 사용하여 노드 형식을 나열한 후 자동 크기 조정 규칙을 추가하도록 합니다.

클러스터를 구성하는 가상 컴퓨터 크기 집합 목록을 가져오려면 다음 cmdlet을 실행합니다.
...
## <a name="set-auto-scale-rules-for-the-node-typevirtual-machine-scale-set"></a>노드 형식/가상 컴퓨터 크기 집합에 대한 자동 크기 조정 규칙 설정
클러스터에 여러 노드 형식이 있는 경우 크기를 조정(확장 또는 축소)할 각 노드 형식/가상 컴퓨터 크기 집합에 대해 이 작업을 반복합니다. 자동 크기 조정을 수행하기 전에 포함해야 할 노드 수를 고려합니다. 기본 노드 형식에 대해 포함해야 할 최소 노드 수는 선택한 안정성 수준에 따라 달라집니다. [안정성 수준](service-fabric-cluster-capacity.md)에 대해 자세히 알아보세요.
...
현재는 자동 크기 조정 기능이 응용 프로그램에서 서비스 패브릭에 보고할 수 있는 로드에 따라 결정되지 않습니다. 따라서 현재는 자동 크기 조정이 각 가상 컴퓨터 크기 집합 인스턴스에서 내보낸 성능 카운터에 의해서만 결정됩니다.  

[각 가상 컴퓨터 크기 집합에 대해 자동 크기 조정을 설정](../virtual-machine-scale-sets/virtual-machine-scale-sets-autoscale-overview.md)하는 지침을 따르세요.
...
## <a name="manually-add-vms-to-a-node-typevirtual-machine-scale-set"></a>노드 형식/가상 컴퓨터 크기 집합에 수동으로 VM 추가
...
## <a name="manually-remove-vms-from-the-primary-node-typevirtual-machine-scale-set"></a>주 노드 형식/가상 컴퓨터 크기 집합에서 수동으로 VM 제거
> [!NOTE]
...
## <a name="manually-remove-vms-from-the-non-primary-node-typevirtual-machine-scale-set"></a>주가 아닌 노드 형식/가상 컴퓨터 크기 집합에서 수동으로 VM 제거
> [!NOTE]
...
클러스터를 강화하는 경우 Service Fabric Explorer는 클러스터의 일부로 노드 수(가상 컴퓨터 크기 집합 인스턴스)를 반영합니다.  그러나 클러스터 규모를 축소할 때 적절한 노드 이름과 함께 [Remove-ServiceFabricNodeState cmd](https://msdn.microsoft.com/library/mt125993.aspx) 를 호출하지 않으면 제거된 노드/VM 인스턴스가 계속 비정상 상태로 표시됩니다.   

다음은 이 동작에 대한 설명입니다.

Service Fabric Explorer에 나열된 노드는 서비스 패브릭 시스템 서비스(특히 FM)가 클러스터에 포함된 노드 수에 대해 알고 있는 내용이 반영된 것입니다. 가상 컴퓨터 크기 집합의 규모를 축소하면 VM이 삭제되지만 FM 시스템 서비스는 해당 노드(삭제된 VM에 매핑된 노드)가 돌아올 것으로 생각합니다. 따라서 Service Fabric Explorer는 성능 상태가 오류 또는 알 수 없음이어도 해당 노드를 계속 표시합니다.

VM이 제거될 때 노드가 제거되는지 확인하기 위한 두 가지 옵션이 있습니다.

1) 클러스터에서 노드 형식에 대해 골드 또는 실버(곧 사용 가능) 내구성 수준을 선택하면 인프라 통합이 제공됩니다. 그러면 규모를 축소할 때 시스템 서비스(FM)에서 해당 노드를 자동으로 제거합니다.
