...
[빠른 시작: Service Fabric에 Linux 컨테이너 애플리케이션 배포](service-fabric-quickstart-containers-linux.md)  
[빠른 시작: Service Fabric에 Windows 컨테이너 애플리케이션 배포](service-fabric-quickstart-containers.md)  
...
Windows Server 2016 이상은 격리 수준이 다른 두 가지 유형의 컨테이너를 제공합니다. Windows Server 컨테이너와 Docker 컨테이너는 둘 다 네임스페이스와 파일 시스템 격리가 있다는 점에서 유사하지만 이들 컨테이너가 실행되는 호스트와 커널을 공유합니다. Linux에서, 이러한 격리는 일반적으로 cgroups 및 네임스페이스에 의해 제공되었으며 Windows Server 컨테이너도 유사하게 작동합니다.
...
Service Fabric은 Linux에서 Docker 컨테이너의 배포를 지원하며, Windows Server 2016 이상에서는 Hyper-V 격리 모드에 대한 지원과 함께 Windows Server 컨테이너의 배포를 지원합니다.

> [!NOTE]
> 로컬 단일 노드 Service Fabric 클러스터에서는 컨테이너를 지원하지 않습니다(OneBox의 Linux 클러스터 및 로컬 Service Fabric 설치의 Windows 클러스터 모두 지원 안 함).
