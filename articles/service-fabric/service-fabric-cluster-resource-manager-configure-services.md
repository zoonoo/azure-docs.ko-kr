...
## 배치 제약 조건
배치 제약 조건은 클러스터에서 서비스가 실제로 실행될 수 있는 노드를 제어하는 데 사용됩니다. 일반적으로 특정 명명된 서비스 인스턴스 또는 특정 노드 형식에서 실행되도록 제한된 형식의 모든 서비스가 표시되지만 배치 제약 조건은 확장될 수 있습니다. 노드 형식 단위로 속성 집합을 정의한 다음 서비스를 만들 때 제약 조건과 함께 선택할 수 있습니다. 배치 제약 조건은 서비스가 진행되는 동안 동적으로 업데이트할 수 있으므로 클러스터의 변화에 응답할 수 있습니다. 배치 제약 조건 및 구성 방법에 대한 자세한 내용은 [이 문서](service-fabric-cluster-resource-manager-cluster-description.md#placement-constraints-and-node-properties)에서 찾을 수 있습니다.
