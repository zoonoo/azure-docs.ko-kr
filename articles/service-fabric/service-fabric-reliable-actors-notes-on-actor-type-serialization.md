---
title: 액터 유형 직렬화에 대한 신뢰할 수 있는 액터 노트
...
모든 메서드의 인수, 행위자 인터페이스의 각 메서드에서 반환되는 작업의 결과 형식 및 행위자의 상태 관리자에 저장된 개체는 [데이터 계약 serializable여야](/dotnet/framework/wcf/feature-details/types-supported-by-the-data-contract-serializer)합니다. 또한 [행위자 이벤트 인터페이스](service-fabric-reliable-actors-events.md)에 정의된 메서드의 인수에도 적용됩니다. (행위자 이벤트 인터페이스 메서드는 항상 void를 반환합니다.)
...
Reliable Actor 프레임워크는 DataContract 직렬화를 사용합니다. 따라서 사용자 지정 데이터 개체와 해당 멤버는 각각 **DataContract** 및 **DataMember** 특성에 추가되어야 합니다.
