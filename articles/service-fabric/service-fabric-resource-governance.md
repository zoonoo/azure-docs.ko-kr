...
* *메모리(메트릭* `servicefabric:/_MemoryInMB`이름): 메모리는 메가바이트로 표현되며 컴퓨터에서 사용할 수 있는 실제 메모리에 매핑됩니다.
...
대부분의 고객 및 시나리오에서 CPU 및 메모리에 대한 노드 용량 자동 검색이 권장되는 구성입니다(자동 검색은 기본적으로 켜져 있음). 그러나 노드 용량의 전체 수동 설정이 필요한 경우 클러스터의 노드를 설명하는 메커니즘을 사용하여 노드 유형별 구성을 구성할 수 있습니다. 다음은 4개의 코어와 2GB의 메모리로 노드 유형을 설정하는 방법의 예입니다.
...
> [!IMPORTANT]
> Service Fabric 버전 7.0부터 사용자가 노드 리소스 용량에 대한 값을 수동으로 제공하는 경우 노드 리소스 용량이 계산되는 방법에 대한 규칙을 업데이트했습니다. 다음 시나리오를 살펴보겠습니다.
>
> * 노드에는 총 10개의 CPU 코어가 있습니다.
> * SF는 사용자 서비스(기본 설정)에 대한 총 리소스의 80%를 사용하도록 구성되며, 노드에서 실행되는 다른 서비스에 대해 20%의 버퍼를 남깁니다(서비스 패브릭 시스템 서비스 포함).
> * 사용자는 CPU 코어 메트릭의 노드 리소스 용량을 수동으로 재정의하고 5코어로 설정합니다.
>
> 서비스 패브릭 사용자 서비스에 사용할 수 있는 용량을 다음과 같은 방식으로 계산하는 방법에 대한 규칙을 변경했습니다.
>
> * Service Fabric 7.0 이전에는 사용자 서비스에 사용할 수 있는 용량이 **5코어로** 계산됩니다(용량 버퍼 20%는 무시됨).
> * Service Fabric 7.0부터 사용자 서비스에 사용할 수 있는 용량은 **4코어로** 계산됩니다(용량 버퍼 20%는 무시되지 않음).
...
이 예제에서 **ServicePackageA**라는 서비스 패키지는 자신이 배치된 노드의 코어 하나를 취합니다. 이 서비스 패키지에는 두 개의 코드 패키지(CodeA1 및 `CpuShares` **CodeA2)가**포함되어 있으며 둘 다 매개 변수를 지정합니다.**CodeA1** 비율이 CpuShares 512:256이면 코어가 코드 패키지 2개에 나뉩니다.
...
리소스 거버넌스 설정을 지정할 때 [응용 프로그램 매개 변수를](service-fabric-manage-multiple-environment-app-configuration.md) 사용하여 여러 앱 구성을 관리할 수 있습니다. 다음 예제에서는 애플리케이션 매개 변수의 사용법을 보여 줍니다.
...
Service Fabric 서비스에 리소스 거버넌스를 적용하면 리소스 관리 서비스가 리소스 할당량을 초과할 수 없지만 많은 사용자는 여전히 일부 서비스 패브릭 서비스를 비관리 모드에서 실행해야 합니다. 관리되지 않은 서비스 패브릭 서비스를 사용하는 경우 "런어웨이" 비관리 서비스가 서비스 패브릭 노드에서 사용 가능한 모든 리소스를 소비하는 상황이 발생할 수 있으며, 이로 인해 다음과 같은 심각한 문제가 발생할 수 있습니다.

* 노드에서 실행되는 다른 서비스의 리소스 부족(서비스 패브릭 시스템 서비스 포함)
* 노드가 비정상 상태로 종료됩니다.
* 응답하지 않는 서비스 패브릭 클러스터 관리 API

이러한 상황이 발생하지 않도록 Service Fabric을 사용하면 *노드에서 실행되는 모든 Service Fabric 사용자* 서비스(관리됨 및 비관리)에 대한 리소스 제한을 적용하여 사용자 서비스가 지정된 리소스 양 이상을 사용하지 않도록 보장할 수 있습니다. 이는 ClusterManifest의 배치및로드밸런싱 섹션에서 EnforceUserServiceMetricCapacities 구성에 대한 값을 true로 설정하여 달성됩니다. 이 설정은 기본적으로 꺼져 있습니다.
...
* 리소스 제한 적용은 자동 검색 메커니즘을 통해 또는 리소스 거버넌스 를 [사용하도록 설정하기](service-fabric-resource-governance.md#cluster-setup-for-enabling-resource-governance) 위한 클러스터 설정 섹션에 설명된 대로 수동으로 노드 용량을 지정하는 사용자를 통해 서비스 패브릭에서 리소스 메트릭에 대한 노드 용량을 사용할 수 있는 경우에만 작동합니다.노드 용량이 구성되지 않은 경우 Service Fabric이 사용자 서비스에 대해 예약할 리소스의 양을 알 수 없으므로 리소스 제한 적용 기능을 사용할 수 없습니다."EnforceUserServiceMetricCapacities"가 사실이지만 노드 용량이 구성되지 않은 경우 서비스 패브릭은 상태 경고를 발행합니다.
