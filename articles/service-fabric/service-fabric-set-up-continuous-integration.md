...
### 인증서를 만들어 새 Azure 주요 자격 증명 모음의 새 인스턴스에 업로드합니다.
...
3.	로컬 서비스 계정에 자동화 인증서 사용을 위한 권한을 부여합니다.
    ...
    d. **추가** 단추를 선택하고 **로컬 서비스**를 입력한 다음 **이름 확인**을 선택합니다.
...
1.	agent.zip을 다운로드합니다. 다음을 수행합니다.
    ...
    >[AZURE.NOTE] ASP.NET 5 웹 서비스를 사용할 계획이라면 배포 중 **PathTooLongExceptions** 오류가 발생하지 않게 이 폴더에 가능한 가장 짧은 이름을 선택하는 것이 좋습니다.
