...
4.	Azure PowerShell에 로그인합니다.
    ...
    1. `Login-AzureRmAccount` 명령을 실행합니다.
...
2.	다음 매개 변수로 PowerShell 스크립트 `CreateAndUpload-Certificate.ps1`를 실행합니다.
    ...
    |매개 변수|값|
    |---|---|
    |KeyVaultLocation|모든 값 클러스터를 만들려는 위치와 일치해야 합니다.|
    |CertificateSecretName|모든 값|
    |SecureCertificatePassword|모든 값. 빌드 컴퓨터에 인증서를 가져올 때 사용됩니다.|
...
3. 이름에 `PSModulePath`, 값에 `%PROGRAMFILES%\WindowsPowerShell\Modules`를 입력합니다.
	...
	1. `%PROGRAMFILES%`를 `PROGRAMFILES` 환경 변수의 값으로 대체합니다.
...
1.	빌드 컴퓨터에 인증서를 가져옵니다. 다음을 수행합니다.
    ...
    2. 관리자 PowerShell 프롬프트를 열고 앞서 `CreateAndUpload-Certificate.ps1`에 전달한 암호를 사용하여 다음 명령을 실행합니다.
...
3.	로컬 서비스 계정에 자동화 인증서 사용을 위한 권한을 부여합니다.
    ...
    4.	**추가** 단추를 선택하고 **로컬 서비스**를 입력한 다음 **이름 확인**을 클릭합니다.
...
1.	agent.zip을 다운로드합니다. 다음을 수행합니다.
    ...
    3.	제어판에서 **에이전트 풀** 탭을 선택합니다.

    4.	**에이전트 다운로드**를 선택하고 agent.zip 파일을 다운로드합니다.

    5.	앞서 만든 빌드 컴퓨터에 agent.zip을 복사합니다.

    6.	빌드 컴퓨터에서 `C:\agent`(또는 짧은 경로의 위치)에 agent.zip의 압축을 풉니다.
...
4. 빌드 에이전트가 등록된 것을 확인합니다. 다음을 수행합니다.
    ...
    2. 앞서 ConfigureAgent.ps1을 실행할 때 선택한 에이전트 풀을 선택합니다.

    3. 빌드 에이전트가 목록에 있고 녹색 상태가 강조 표시되었는지 확인합니다. 빨강으로 강조 표시된 경우 빌드 에이전트가 Team Services에 연결하는 데 문제가 있는 것입니다.
...
1.	빈 빌드 정의를 만듭니다. 다음을 수행합니다.
    ...
    4.	**비어 있음**을 선택하고 **다음** 단추를 선택합니다.

    5.  오른쪽 리포지토리 및 분기가 선택되었는지 확인합니다.

    6.  빌드 에이전트를 등록한 에이전트 대기열을 선택하고 **연속 통합** 확인란을 선택합니다.
...
**빌드 큐 대기**를 클릭하여 빌드를 시작합니다. 또한 빌드는 푸시/체크인할 때 트리거됩니다.
...
1.	Azure 관리 포털 또는 Azure PowerShell을 통해 수동으로 테스트 클러스터를 만듭니다. 참조로 `ProvisionAndDeploy-SecureCluster.ps1` 스크립트를 참조할 수 있습니다.
