...
인증서 보안을 사용하여 클라이언트와 클러스터 노드 간 통신을 보호하려면 먼저 클라이언트 인증서를 획득하고 로컬 컴퓨터의 개인(내) 저장소 또는 현재 사용자의 개인 저장소에 설치해야 합니다. 클라이언트가 클러스터를 인증할 수 있도록 하기 위해 서버 인증서의 지문도 필요합니다.

다음 PowerShell cmdlet을 실행하여 클러스터에 액세스하는 데 사용할 컴퓨터에 해당 클라이언트 인증서를 설정합니다.
