...
직렬 변환기는 일반적으로 데이터가 이전 버전이든 *최신* 버전이든 데이터를 읽고 현재 버전으로 역직렬화하는 역할을 담당합니다. 기본 직렬 변환기는 버전 관리 규칙이 잘 정의된 [데이터 계약 직렬 변환기](https://msdn.microsoft.com/library/ms733127.aspx)입니다. 신뢰할 수 있는 컬렉션을 사용하면 직렬 변환기를 재정의할 수 있지만 Reliable Actors를 사용하면 직렬 변환기를 재정의할 수 없습니다. 데이터 직렬 변환기는 롤링 업그레이드에서 중요한 역할을 합니다. 데이터 계약 직렬 변환기는 서비스 패브릭 응용 프로그램에 권장되는 직렬 변환기입니다.
...
> [!NOTE]
> 여기서 "최신 버전" 및 "이전 버전"이란 실행 중인 코드의 버전을 말합니다. "최신 직렬 변환기"는 최신 버전의 응용 프로그램에서 실행되는 직렬 변환기 코드를 말합니다. "최신 데이터"는 최신 버전의 응용 프로그램에서 직렬화된 C# 클래스를 말합니다.
> 
> 
