...
신뢰할 수 있는 환경에서 `UseServiceFabricIntegration` 메서드로 추가 된 미들웨어는 Naming Service에 게시 된 주소에 고유 식별자를 자동으로 추가 합니다. 각 요청에서 해당 식별자의 유효성을 검사 합니다. 식별자가 일치 하지 않으면 미들웨어는 즉시 HTTP 410이 아닌 응답을 반환 합니다.
...
HTTP.SYS에서 동적으로 할당 된 포트를 사용 하려면 `Endpoint` 구성에서 `Port` 속성을 생략 합니다.
...
이 예제에서는 WebHost 종속성 삽입 컨테이너에 `IReliableStateManager`의 singleton 인스턴스를 제공합니다. 이는 반드시 필요한 것은 아니지만 MVC 컨트롤러 작업 메서드에서 `IReliableStateManager` 및 신뢰할 수 있는 컬렉션을 사용할 수 있습니다.
...
Kestrel과 함께 사용 하기 위해 Servicemanifest.xml의 `Endpoint` 구성에서 정적 포트를 구성할 수 있습니다. 반드시 필요한 것은 아니지만, 다음과 같은 두 가지 잠재적인 이점을 제공 합니다.
...
| ServiceFabricIntegrationOptions | 없음 | 서비스에서 고유 식별자에 대 한 들어오는 요청의 유효성을 검사 하지 않도록 Service Fabric 통합 미들웨어를 구성할 때 `ServiceFabricIntegrationOptions.None` 옵션을 사용 합니다. 응용 프로그램의 외부 사용자는 미들웨어에서 사용 하는 고유한 식별 정보를 알지 못합니다. |
