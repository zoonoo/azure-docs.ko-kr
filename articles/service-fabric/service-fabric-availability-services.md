...
주 복제본이 중지되는 경우 Service Fabric은 활성 보조 복제본 중 하나를 새 주 복제본으로 만듭니다. 이 활성 보조 복제본은 이미 상태의 업데이트된 버전을 가지므로(*복제*를 통해) 읽기 및 쓰기 작업의 처리를 계속할 수 있습니다. 이 프로세스를 재구성이라고 하며 [재구성](service-fabric-concepts-reconfiguration.md)에서 자세히 설명합니다.

복제본이 주 또는 활성 보조가 되는 이 개념은 복제본 역할로 알려져 있습니다. [복제본 및 인스턴스](service-fabric-concepts-replica-lifecycle.md)에서 자세히 설명합니다. 
