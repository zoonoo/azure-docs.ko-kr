...
컨테이너에 대한 격리 모드입니다. 유효한 값은 default, process 또는 hyperv(Windows 컨테이너만 지원)입니다. Windows 10 호스트에서는 Hyper-V 격리 모드만 지원되므로 격리 모드 설정에 관계없이 컨테이너가 Hyper-V 격리 모드로 실행됩니다.
|특성|값|
|---|---|
|이름|격리|
|use|선택 사항|
...
서비스 매니페스트의 리소스 섹션에 선언되어야 하는 끝점의 이름입니다.  HTTPS를 사용하는 경우 동일한 노드에 배포된 다른 서비스 인스턴스(응용 프로그램과 독립적)에 동일한 포트 및 인증서를 사용하지 않습니다. 다른 응용 프로그램 인스턴스에서 동일한 포트를 사용하는 두 가지 다른 서비스를 업그레이드하면 업그레이드 오류가 발생합니다.
|특성|값|
|---|---|
|이름|EndpointRef|
...
#### <a name="servicetypename"></a>ServiceTypeName
서비스의 사용자 정의 형식입니다. 식별자입니다(예: "QueueType" 또는 "CalculatorType"). 이 값은 ApplicationManifest.xml 파일에서 서비스를 식별하는 데 사용됩니다.
|특성|값|
|---|---|
|이름|ServiceTypeName|
|use|필수|
