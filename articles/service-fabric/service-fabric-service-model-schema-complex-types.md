---
title: Azure Service Fabric 서비스 모델 XML 스키마 복합 형식
...
애플리케이션에서 오류로 처리하기 전에 비정상 상태를 유지할 수 있는 배포된 애플리케이션의 최대 허용 비율을 지정합니다. 이는 클러스터에서 애플리케이션이 현재 배포되어 있는 노드 수에 대해 배포된 비정상 애플리케이션의 수를 나누어 계산됩니다. 계산값은 적은 수의 노드에서 오류 하나를 허용할 수 있도록 반올림됩니다. 기본 비율은 0입니다.
...
Azure Storage 계정의 진단 스토리지를 설명합니다.
...
이 값은 특성 값의 보호를 정의 합니다. SecretsStoreRef/Encrypted/PlainText 일 수 있습니다.
SecretsStoreRef로 설정 된 경우 값은 클러스터의 SecureStore에 암호화 된 암호를 참조 합니다 .이 암호는 비활성화 되 고 활성화 될 때 암호가 해독 됩니다.
...
#### <a name="type"></a>형식
엔드포인트의 형식입니다. 서비스 매니페스트의 값을 재정의합니다. 입력된 엔드포인트는 외부에 포트를 노출하는 데 사용되고, 내부 엔드포인트는 애플리케이션 간 통신에 사용됩니다.
...
#### <a name="type"></a>형식
엔드포인트의 형식입니다.  입력된 엔드포인트는 외부에 포트를 노출하는 데 사용되고, 내부 엔드포인트는 애플리케이션 간 통신에 사용됩니다.
...
환경 변수의 형식입니다. 일반 텍스트/암호화/SecretsStoreRef 수 있습니다.
...
이 값은 ' Password ' 특성에 지정한 암호의 유형을 정의 합니다. SecretsStoreRef/Encrypted/PlainText를 사용할 수 있습니다.
SecretsStoreRef로 설정 하면 SecretStore에서 참조 값을 검색 합니다.
암호화 된 상태로 설정 된 경우 응용 프로그램 개발자는 인증서를 만들고 ServiceFabricEncryptSecret cmdlet을 사용 하 여 중요 한 정보를 암호화 합니다.
...
애플리케이션을 비정상으로 처리하기 전에 허용되는 최대 비정상 서비스 비율을 지정합니다. 기본 비율은 0입니다.
