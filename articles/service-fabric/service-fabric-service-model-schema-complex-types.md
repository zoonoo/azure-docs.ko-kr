...
엔드포인트의 프로토콜입니다. HTTPS 엔드포인트에는 응용 프로그램 매니페스트에 선언된 EndpointCertificate 및 EndpointBindingPolicy도 있어야 합니다. 프로토콜은 나중에 응용 프로그램 업그레이드 시 변경할 수 없습니다. 
|특성|값|
|---|---|
|이름|프로토콜|
|형식|xs:string|
...
서비스 매니페스트에 정의된 리소스 종류(엔드포인트 또는 인증서)입니다.
|특성|값|
|---|---|
|이름|ResourceType|
|use|선택 사항|
...
#### <a name="placementconstraints"></a>PlacementConstraints
서비스가 실행될 수 있는 클러스터의 노드를 제어하는 데 사용됩니다. 노드 속성 이름 및 값에 대한 서비스 요구 사항을 설명하는 키/값 쌍입니다. 개별 문을 간단한 부울 논리로 함께 그룹화하여 필요한 제약 조건을 만들 수 있습니다. 예를 들어 "(FirmwareVersion>12  && InDMZ == True)"입니다.
|특성|값|
|---|---|
|이름|PlacementConstraints|
|형식|xs:string|
