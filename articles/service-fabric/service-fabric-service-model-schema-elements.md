---
title: Azure Service Fabric 서비스 모델 XML 스키마 요소
...
이름 특성으로 이름이 지정 된 폴더와 Settings.xml 파일을 포함 하는 PackageRoot를 선언 합니다. 이 파일은 런타임에 프로세스에서 다시 읽을 수 있는 사용자 정의 키-값 쌍 설정의 섹션을 포함합니다. 업그레이드하는 동안 ConfigPackage 버전만 변경되면 실행 중인 프로세스가 다시 시작되지 않습니다. 대신, 콜백에서는 구성 설정이 변경되어 동적으로 다시 로드할 수 있음을 프로세스에 알립니다.
...
경고! 프로덕션 애플리케이션에서 콘솔 리디렉션을 사용하지 마세요. 로컬 개발 및 디버깅에만 사용하세요. 시작 스크립트의 콘솔 출력을 애플리케이션이 배포되고 실행되는 클러스터 노드의 "log"라는 애플리케이션 폴더에 있는 출력 파일로 리디렉션합니다.
...
True인 경우 이 매개 변수의 값이 암호화됩니다. 애플리케이션 개발자는 인증서를 만들고 Invoke-ServiceFabricEncryptSecret cmdlet을 사용하여 중요한 정보를 암호화하는 일을 담당합니다. 값을 암호화하는 데 사용될 인증서 정보는 인증서 섹션에 지정됩니다. 이 특성은 사용되지 않습니다. Type 특성을 사용 하십시오.
...
이 값은 ' Value ' 특성에 지정한 값의 유형을 정의 합니다. SecretsStoreRef/Encrypted/PlainText를 사용할 수 있습니다.
SecretsStoreRef로 설정 하면 SecretStore에서 참조 값을 검색 합니다.
암호화 된 상태로 설정 된 경우 응용 프로그램 개발자는 인증서를 만들고 ServiceFabricEncryptSecret cmdlet을 사용 하 여 중요 한 정보를 암호화 합니다.
...
계정 유형 (LocalUser, DomainUser, NetworkService, LocalService, ManagedServiceAccount 또는 LocalSystem)을 지정 합니다.  기본값은 LocalUser입니다. 애플리케이션이 배포된 컴퓨터에 로컬 사용자 계정이 생성됩니다. 기본적으로 이러한 계정은 여기에서 지정된 것과 이름이 다릅니다. 대신 동적으로 생성되며 임의 암호를 포함합니다. 지원되는 로컬 시스템 계정 유형은 LocalUser, NetworkService, LocalService 및 LocalSystem입니다. 도메인 계정은 Azure Active Directory를 사용할 수 있는 Windows Server 배포에서 지원됩니다.
