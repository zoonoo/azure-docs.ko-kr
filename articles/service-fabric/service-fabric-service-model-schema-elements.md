...
응용 프로그램 매니페스트 또는 클러스터 매니페스트 내에서 엔드포인트를 보호하거나 비밀을 암호화하는 데 사용되는 인증서를 선언합니다.
...
서비스에 대한 엔드포인트를 정의합니다. 특정 포트를 요청할 수 있습니다.  명시적으로 지정되지 않은 경우 포트는 예약된 응용 프로그램 포트 범위에서 할당됩니다. 다른 클러스터 노드에서 실행되는 서비스 복제본에 다른 포트 번호를 할당할 수 있으며, 동일한 노드에서 실행되는 동일한 서비스의 복제본에서 동일한 포트를 공유합니다. 이러한 포트는 복제 또는 클라이언트 요청 수신과 같은 다양한 용도로 서비스 복제본에서 사용할 수 있습니다.
...
#### <a name="name"></a>이름
로컬 그룹 계정의 이름입니다. 이 이름에는 응용 프로그램 ID가 접두사가 사용됩니다.
|특성|값|
|---|---|
|이름|이름|
|형식|xs:string|
...
응용 프로그램 인스턴스화 중에 매개 변수 값이 제공되지 않은 경우에 사용되는 매개 변수의 기본값입니다.
|특성|값|
|---|---|
|이름|DefaultValue|
|형식|xs:string|
...
서비스 매니페스트에 정의된 리소스(예: 엔드포인트)의 보안 주체에 대한 액세스 권한을 부여합니다. 일반적으로 보안 위험을 최소화하기 위해 여러 리소스에 대한 서비스 액세스를 제어하고 제한하는 것이 매우 유용합니다. 이는 다른 개발자가 개발한 마켓플레이스의 서비스 컬렉션에서 응용 프로그램을 빌드하는 경우 특히 중요합니다.
...
#### <a name="name"></a>이름
정규화된 응용 프로그램 이름 URI를 형성하는 데 사용되는 서비스 이름입니다. 서비스의 정규화된 이름 URI는 fabric:/ApplicationName/ServiceName입니다.
|특성|값|
|---|---|
|이름|이름|
|형식|xs:string|
...
#### <a name="name"></a>이름
이 응용 프로그램 이름 URI에 상대적인 이 서비스의 이름입니다. 서비스의 정규화된 이름은 응용 프로그램의 이름 URI와 이 이름의 조합입니다.
|특성|값|
|---|---|
|이름|이름|
|형식|xs:string|
