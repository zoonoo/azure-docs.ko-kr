---
title: Azure 서비스 패브릭 서비스 모델 XML 스키마 요소
...
Name 특성에 의해 명명된 폴더와 Settings.xml 파일이 포함된 PackageRoot를 선언합니다. 이 파일은 런타임에 프로세스에서 다시 읽을 수 있는 사용자 정의 키-값 쌍 설정의 섹션을 포함합니다. 업그레이드하는 동안 ConfigPackage 버전만 변경되면 실행 중인 프로세스가 다시 시작되지 않습니다. 대신, 콜백에서는 구성 설정이 변경되어 동적으로 다시 로드할 수 있음을 프로세스에 알립니다.
...
|특성|값|
|---|---|
|type|[매개 변수 유형](service-fabric-service-model-schema-complex-types.md#parametertype-complextype)|
|content|0개 요소, 0개 특성|
...
계정 의 종류를 지정합니다: LocalUser, 도메인 사용자, 네트워크 서비스, 로컬 서비스, ManagedServiceAccount, 또는 로컬 시스템.  기본값은 LocalUser입니다. 애플리케이션이 배포된 컴퓨터에 로컬 사용자 계정이 생성됩니다. 기본적으로 이러한 계정은 여기에서 지정된 것과 이름이 다릅니다. 대신 동적으로 생성되며 임의 암호를 포함합니다. 지원되는 로컬 시스템 계정 유형은 LocalUser, NetworkService, LocalService 및 LocalSystem입니다. 도메인 계정은 Azure Active Directory를 사용할 수 있는 Windows Server 배포에서 지원됩니다.
