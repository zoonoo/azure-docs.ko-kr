...
Name 특성으로 이름이 지정 된 폴더와 PackageRoot 파일이 포함 된 폴더를 선언 합니다. 이 파일은 런타임에 프로세스에서 다시 읽을 수 있는 사용자 정의 키-값 쌍 설정의 섹션을 포함합니다. 업그레이드하는 동안 ConfigPackage 버전만 변경되면 실행 중인 프로세스가 다시 시작되지 않습니다. 대신, 콜백에서는 구성 설정이 변경되어 동적으로 다시 로드할 수 있음을 프로세스에 알립니다.
...
[https://hub.docker.com](https://hub.docker.com ) 의 리포지토리 및 이미지 또는 Azure Container Registry입니다.
