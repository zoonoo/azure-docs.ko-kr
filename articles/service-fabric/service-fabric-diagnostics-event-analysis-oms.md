---
title: Azure 모니터 로그를 사용 하 고 Azure 서비스 패브릭 이벤트 분석
description: Azure Service Fabric 클러스터의 모니터링 및 진단을 위해 Azure Monitor 로그를 사용하여 이벤트를 시각화하고 분석하는 방법에 대해 알아봅니다.
author: srrengar
ms.topic: conceptual
ms.date: 02/21/2019
ms.author: srrengar
...
---
# <a name="event-analysis-and-visualization-with-azure-monitor-logs"></a>Azure 모니터 로그를 통해 이벤트 분석 및 시각화
 Azure Monitor 로그는 클라우드에서 호스팅되는 애플리케이션 및 서비스에서 원격 분석 데이터를 수집 및 분석하고, 가용성과 성능을 최대화하는 데 도움이 되는 분석 도구를 제공합니다. 이 문서에서는 Azure Monitor 로그에서 쿼리를 실행하여 클러스터에서 발생하는 문제를 해결하고 통찰력을 얻는 방법을 설명합니다. 다음과 같은 일반적인 질문을 해결합니다.
...
## <a name="overview-of-the-log-analytics-workspace"></a>로그 분석 작업 영역 개요

>[!NOTE] 
>클러스터 생성 시 진단 스토리지가 기본으로 사용하도록 설정되어 있는 경우에도 진단 스토리지에서 읽도록 Log Analytics 작업 영역을 설정해야 합니다.

Azure Monitor 로그는 Azure 저장소 테이블 또는 에이전트를 포함하여 관리되는 리소스에서 데이터를 수집하고 중앙 리포지토리에서 유지 관리합니다. 그런 다음 데이터를 분석, 경고 및 시각화하거나 내보낼 수 있습니다. Azure Monitor 로그는 이벤트, 성능 데이터 또는 기타 사용자 지정 데이터를 지원합니다. 데이터를 Azure Monitor 로그로 흐르고 있는지 확인하기 위해 [저장소의 이벤트에서 읽을 로그 Analytics 작업 영역을 만드는](service-fabric-diagnostics-oms-setup.md) [이벤트를 집계하도록 진단 확장을 구성하는](service-fabric-diagnostics-event-aggregation-wad.md) 단계를 체크 아웃합니다.

Azure Monitor 로그에 의해 데이터를 받은 후 Azure에는 여러 시나리오에 맞게 사용자 지정된 들어오는 데이터를 모니터링하기 위해 미리 패키지된 솔루션 또는 운영 대시보드인 여러 *모니터링 솔루션이* 있습니다. 여기에는 Service Fabric 클러스터를 사용할 때 진단 및 모니터링과 가장 관련된 두 개의 솔루션인 *Service Fabric 분석* 솔루션과 *컨테이너* 솔루션이 포함됩니다. 이 문서에서는 작업 영역을 사용하여 만든 Service Fabric 분석 솔루션을 사용하는 방법을 설명합니다.

## <a name="access-the-service-fabric-analytics-solution"></a>Service Fabric 분석 솔루션에 액세스

Azure [Portal에서](https://portal.azure.com)서비스 패브릭 분석 솔루션을 만든 리소스 그룹으로 이동합니다.
...
**목록**을 클릭하여 목록에서 이벤트를 봅니다. 일단 여기에 수집된 모든 시스템 이벤트가 표시됩니다. 참고로, 이들은 Azure 저장소 계정의 **WADServiceFabricSystemEventsTable에서** 온 것이며, 마찬가지로 다음에 표시되는 신뢰할 수 있는 서비스 및 행위자 이벤트는 해당 테이블에서 온 것입니다.
    
![쿼리 조작 채널](media/service-fabric-diagnostics-event-analysis-oms/oms_service_fabric_events.png)

또는 왼쪽에 있는 돋보기를 클릭하고 Kusto 쿼리 언어를 사용하여 원하는 항목을 찾을 수 있습니다. 예를 들어 클러스터의 노드에서 수행된 모든 작업을 찾으려면 다음 쿼리를 사용하면 됩니다. 아래에 사용된 이벤트 번호는 운영 [채널 이벤트 참조에서](service-fabric-diagnostics-event-generation-operational.md)찾을 수 있습니다.
...
Reliable Actors 이벤트는 비슷한 방식으로 볼 수 있습니다. Reliable Actors에 대한 자세한 이벤트를 구성하려면 진단 확장(아래 참조)에 대한 구성에서 `scheduledTransferKeywordFilter`를 변경해야 합니다. 이러한 값에 대한 자세한 내용은 [신뢰할 수 있는 행위자 이벤트 참조에](service-fabric-reliable-actors-diagnostics.md#keywords)있습니다.
...
* 온-프레미스 클러스터의 경우 Azure Monitor 로그는 Azure Monitor 로그에 데이터를 보내는 데 사용할 수 있는 게이트웨이(HTTP Forward Proxy)를 제공합니다. [Log Analytics 게이트웨이를 사용하여 Azure Monitor 로그에 인터넷에 액세스하지 않고 컴퓨터를 연결하는](../azure-monitor/platform/gateway.md)것에 대해 자세히 알아보기.
...
* Azure Monitor 로그와 제공하는 것에 대한 자세한 개요를 확인하여 [Azure Monitor 로그는 무엇입니까?](../operations-management-suite/operations-management-suite-overview.md)
