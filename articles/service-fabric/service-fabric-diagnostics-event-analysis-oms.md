...
# <a name="event-analysis-and-visualization-with-log-analytics"></a>Log Analytics를 사용하여 이벤트 분석 및 시각화
 Log Analytics는 클라우드에서 호스팅되는 서비스 및 애플리케이션에서 원격 분석 데이터를 수집 및 분석하고 가용성 및 성능을 극대화하는 데 도움이 되는 분석 도구를 제공합니다. 이 문서에서는 Log Analytics에서 쿼리를 실행하여 인사이트를 얻는 방법과 클러스터에서 발생하는 문제를 해결하는 방법에 대해 설명합니다. 다음과 같은 일반적인 질문을 해결합니다.
