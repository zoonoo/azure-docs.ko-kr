...
이 방법은 클러스터 만들기 단계에서 수행합니다. Resource Manager 템플릿을 사용하여 클러스터를 배포할 때 템플릿은 새 OMS 작업 영역을 만들고 여기에 Service Fabric 솔루션을 추가하고 적절한 저장소 테이블에서 데이터를 읽도록 구성할 수 있습니다.
...
클러스터를 배포한 후에 OMS 작업 영역을 추가하려면 Azure Marketplace로 이동하고 *"Service Fabric 분석"*을 검색합니다. 아래에 표시된 대로 "모니터링 + 관리" 범주 내에 하나의 리소스만 표시되어야 합니다.

![Marketplace의 OMS SF 분석](media/service-fabric-diagnostics-event-analysis-oms/service-fabric-analytics.png)

**만들기**를 클릭하면 OMS 작업 영역에 대한 메시지가 표시됩니다. **작업 영역 선택** 및 **새 작업 영역 만들기**를 차례로 선택합니다. 필요한 항목을 입력합니다. 단, Service Fabric 클러스터와 OMS 작업 영역에 대한 구독이 동일해야 합니다. 입력 항목의 유효성이 검사되면 OMS 작업 영역이 몇 분 안에 배포됩니다. 배포가 완료되는 동안 Service Fabric 솔루션 블레이드 만들기는 여전히 열려 있습니다. 동일한 작업 영역이 *OMS 작업 영역* 아래에 나타나는지 확인한 다음 아래쪽에서 **만들기**를 클릭하여 Service Fabric 솔루션을 작업 영역에 추가합니다.
...
* 클러스터에서 수집되는 데이터를 쉽게 구성하고 응용 프로그램이나 클러스터를 재배포하지 않고 변경합니다. 이는 OMS 작업 영역에서 에이전트 설정을 변경하여 에이전트가 자동으로 다시 설정되도록 할 수 있기 때문입니다. 특정 성능 카운터를 선택하도록 OMS 에이전트를 구성하려면 작업 영역 **홈 > 설정 > 데이터 > Windows 성능 카운터**로 이동하고 수집하려는 데이터를 선택합니다.
...
단, 에이전트이기 때문에 모든 응용 프로그램과 함께 클러스터에 배포되므로 클러스터의 응용 프로그램 성능에 최소한의 영향을 줍니다.
