...
서비스 패브릭은 Azure SQL 데이터베이스, Azure DocumentDB, Cortana, Power BI, Microsoft Intune, Azure 이벤트 허브, Azure IoT, 많은 핵심 Azure 서비스, Skype for Business를 포함하여 오늘날의 여러 Microsoft 서비스를 지원합니다.
...
마이크로 서비스 접근 방식에 대한 자세한 내용은 [응용 프로그램 구축에 마이크로 서비스 접근 방식이 필요한 이유](service-fabric-overview-microservices.md) 문서를 참조하세요.
...
간단히 모든 곳에서 상태 비저장 서비스를 사용하지 않는 이유는 무엇일까요? 두 가지 주요 이유가 있습니다.
...
- 서비스 패브릭 프로그래밍 모델을 사용하여 마이크로 서비스로 구성된 응용 프로그램을 개발하거나, 아니면 단순히 게스트 실행 파일 및 사용자가 선택한 기타 응용 프로그램 프레임워크(예: ASP.NET, Node.js 등) 호스트.
