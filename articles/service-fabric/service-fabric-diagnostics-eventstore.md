...
description: 언제든지 클러스터 또는 워크로드의 상태를 이해하고 모니터링할 수 있는 방법인 Azure Service Fabric의 EventStore에 대해 알아봅니다.
ms.topic: conceptual
ms.date: 6/6/2019
...
![NodeDown 이벤트를 포함하여 노드 창에서 여러 이벤트의 이벤트 탭을 보여 주는 스크린샷.](media/service-fabric-diagnostics-eventstore/eventstore.png)
...
API에 대해 자세히 알아보려면 [EventStore API 참조](/rest/api/servicefabric/sfclient-index-eventsstore)를 확인하세요.
...
버전 6.4를 사용하는 경우 Azure Resource Manager 템플릿을 편집하여 EventStore 서비스를 켤 수 있습니다. 이렇게 하려면 [클러스터 구성 업그레이드](service-fabric-cluster-config-upgrade-azure.md)를 수행하고 다음 코드를 추가합니다. PlacementConstraints를 사용하여 EventStore 서비스의 복제본을 특정 NodeType(예: 시스템 서비스 전용 NodeType)에 배치할 수 있습니다. `upgradeDescription` 섹션은 노드에서 다시 시작을 트리거하도록 구성 업그레이드를 구성합니다. 다른 업데이트에서 섹션을 제거할 수 있습니다.
