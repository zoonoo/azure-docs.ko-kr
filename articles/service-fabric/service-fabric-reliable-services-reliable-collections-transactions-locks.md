...
트랜잭션은 하나의 논리적 작업 단위로 수행되는 일련의 작업입니다. 데이터베이스 트랜잭션의 일반적인 [ACID](https://en.wikipedia.org/wiki/ACID)(*원자성*, *일관성*, *격리*, *내구성*) 속성을 보여 줍니다.
...
* **격리**: 동시 트랜잭션에 의한 수정은 다른 동시 트랜잭션과 격리되어야 합니다. [ITransaction](/dotnet/api/microsoft.servicefabric.data.itransaction) 내에서 작업에 사용되는 격리 수준은 작업을 수행하는 [IReliableState](/dotnet/api/microsoft.servicefabric.data.ireliablestate)에 의해 결정됩니다.
...
신뢰할 수 있는 사전 및 신뢰할 수 있는 큐는 *내 쓰기 작업 읽기* 를 지원합니다.
...
신뢰할 수 있는 컬렉션 API의 시간 초과 인수는 교착 상태 감지를 위해 사용됩니다.
예를 들어 두 트랜잭션(T1과 T2)은 K1을 읽고 업데이트하려고 합니다.
둘 다 결국 공유된 잠금을 가지게 되기 때문에 교착 상태가 될 수 있습니다.
이 경우 하나의 작업 또는 모든 작업의 시간이 초과됩니다. 이러한 상황에서 업데이트 잠금은 이와 같은 교착 상태를 방지할 수 있습니다.
