---
title: Azure 모니터 로그를 통해 성능 모니터링
...
---
# <a name="performance-monitoring-with-azure-monitor-logs"></a>Azure 모니터 로그를 통해 성능 모니터링
...
> [!NOTE]
> 이 문서는 Azure Log Analytics 작업 영역이 이미 설정되었다고 가정합니다. 그렇지 않은 경우 Azure [모니터 로그 설정으로 이동하십시오.](service-fabric-diagnostics-oms-setup.md)
...
2. 포털에서 Log Analytics 작업 영역이 있는 리소스 그룹으로 이동합니다. 로그 분석 리소스를 클릭합니다(리소스 유형은 Log Analytics 작업 영역이 됩니다). 리소스 개요 페이지에서 왼쪽 메뉴의 설정 섹션 아래에 있는 **고급 설정**을 클릭합니다.

    ![로그 분석 속성 페이지](media/service-fabric-diagnostics-oms-agent/oms-advanced-settings.png)
...
4. `vmss extension set` API를 사용하여 로그 분석 에이전트를 클러스터에 설치하려면 명령을 실행합니다.
...
* Azure Monitor 로그를 구성하여 [자동 경고를](../log-analytics/log-analytics-alerts.md) 설정하여 검색 및 진단에 도움을 주도록 합니다.
