...
**서비스 패브릭 생성 클러스터 블레이드에는** 다음 네 단계가 있습니다.

### <a name="1-basics"></a>1. 기본 사항
![새 리소스 그룹을 만드는 스크린샷입니다.][CreateRG]
...
4. 새 **리소스 그룹을**만듭니다. 클러스터 이름과 같은 이름을 사용하면 나중에 쉽게 찾을 수 있으며 특히 배포를 변경하거나 클러스터를 삭제하려고 할 때 매우 유용합니다.
...
6. **사용자 지정 끝점을 구성합니다.** 이 필드를 사용하면 Azure Load Balancer를 통해 애플리케이션에 대한 공용 인터넷에 노출하려고 하는 쉼표로 구분된 포트 목록을 입력할 수 있습니다. 예를 들어 클러스터에 웹 애플리케이션을 배포하려는 경우 여기에 "80"을 입력하여 포트 80의 트래픽이 클러스터로 이동되도록 허용합니다. 엔드포인트에 대한 자세한 내용은 [애플리케이션과 통신][service-fabric-connect-and-communicate-with-services]을 참조하세요.
...
10. 클러스터를 설정하려는 **패브릭 업그레이드 모드를** 선택합니다. 시스템이 사용 가능한 최신 버전을 자동으로 선택하고 클러스터를 최신 버전으로 업그레이드하도록 하려면 **자동**을 선택합니다. 지원되는 버전을 선택하려는 경우 모드를 **수동**으로 설정합니다. 패브릭 업그레이드 모드에 대한 자세한 내용은 [Service Fabric 클러스터 업그레이드 문서][service-fabric-cluster-upgrade]를 참조하세요.
...
![Azure 포털의 보안 구성 스크린샷입니다.][BasicSecurityConfigs]
...
알림에서 만들기 진행률을 볼 수 있습니다. (화면 오른쪽 상단에 있는 상태 표시줄 근처의 "벨" 아이콘을 클릭합니다. 클러스터를 만드는 동안 **시작 보드에 Pin을** 클릭하면 **서비스 패브릭 클러스터 배포가** **시작** 보드에 고정된 것을 볼 수 있습니다. 이 프로세스는 시간이 약간 걸립니다. 
