---
title: Azure 서비스 패브릭 CLI- sfctl
description: Azure 서비스 패브릭 명령줄 인터페이스인 sfctl에 대해 알아봅니다. 명령 및 하위 그룹 목록을 포함합니다.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
Service Fabric 클러스터 및 엔터티 관리를 위한 명령. 이 버전은 서비스 패브릭 7.0 런타임과 호환됩니다.
...
| [작성](service-fabric-sfctl-compose.md) | Docker Compose 애플리케이션을 만들고, 삭제하고, 관리합니다. |
| [컨테이너](service-fabric-sfctl-container.md) | 클러스터 노드에서 컨테이너 관련 명령을 실행합니다. |
| [이벤트](service-fabric-sfctl-events.md) | 이벤트 저장소에서 이벤트를 검색합니다(EventStore 서비스가 이미 설치된 경우). |
...
| [노드](service-fabric-sfctl-node.md) | 클러스터를 형성하는 노드를 관리합니다. |
| [파티션](service-fabric-sfctl-partition.md) | 모든 서비스에 대한 파티션을 쿼리 및 관리합니다. |
| [속성(property)](service-fabric-sfctl-property.md) | Service Fabric 이름 아래에 있는 저장소 및 쿼리 속성입니다. |
| [복제본](service-fabric-sfctl-replica.md) | 서비스 파티션에 속한 복제본을 관리합니다. |
...
| [서비스](service-fabric-sfctl-service.md) | 서비스, 서비스 유형 및 서비스 패키지를 생성, 삭제 및 관리합니다. |
| [설정](service-fabric-sfctl-settings.md) | 이 sfctl 인스턴스에 로컬인 설정을 구성합니다. |
| [스토어](service-fabric-sfctl-store.md) | 클러스터 이미지 저장소에서 기본 파일 수준 작업을 수행합니다. |
