...
장애 도메인 및 업그레이드 도메인을 사용 하 여 ![배치][Image3]
</center>
...
장애 도메인 및 업그레이드 도메인의 레이아웃을 ![][Image4]
</center>
...
### <a name="default-approach"></a>기본 방법
기본적으로 클러스터 리소스 관리자는 장애 도메인 및 업그레이드 도메인 간에 서비스 균형이 유지 됩니다. 이것이 [제약 조건](service-fabric-cluster-resource-manager-management-integration.md)으로 모델링됩니다. 장애 도메인 및 업그레이드 도메인에 대 한 제약 조건 상태: "지정 된 서비스 파티션에는 동일한의 두 도메인 간에 서비스 개체 (상태 비저장 서비스 인스턴스 또는 상태 저장 서비스 복제본) 수의 차이가 없어야 합니다. 계층의 수준입니다. "
...
### <a name="alternative-approach"></a>대체 방법
...
### <a name="adaptive-approach"></a>적응 방법
...
예를 들어 `"(NodeType == NodeType03)"`로 배치 제약 조건을 작성할 수 있습니다. **NodeType** 은 일반적으로 사용 되는 속성입니다. 이는 컴퓨터의 유형과 1:1에 해당 하므로 유용 합니다. 각 머신의 종류는 기존의 n 계층 애플리케이션에서 워크로드 유형에 해당합니다.
...
> [!NOTE]
> Azure Resource Manager 템플릿에서 노드 형식은 일반적으로 매개 변수화 됩니다. NodeType01가 아닌 `"[parameters('vmNodeType1Name')]"` 처럼 보입니다.
>
...
NodeType01의 모든 노드가 유효 하면 제약 조건 `"(NodeType == NodeType01)"`를 사용 하 여 해당 노드 형식을 선택할 수도 있습니다.
