...
이 레이아웃은 장애 도메인 제약 조건에 대 한 "최대 차이" 보증의 정의를 위반 합니다. F D 0에는 두 개의 복제본이 있고, f에는 0이 있습니다. F D 0와 f의 차이는 1의 최대값 보다 큰 총 2입니다. 제약 조건을 위반 하기 때문에 클러스터 리소스 관리자는이 정렬을 허용 하지 않습니다.

마찬가지로, N2 및 N6 (N1 및 N2 대신)를 선택한 경우 다음을 얻게 됩니다.
...
## <a name="node-buffer-and-overbooking-capacity"></a>노드 버퍼 및 과도 한 예약 용량

메트릭에 대 한 노드 용량이 지정 된 경우에는 클러스터 리소스 관리자에서 전체 부하가 지정 된 노드 용량을 초과 하는 경우 복제본을 이동 하거나 이동 하지 않습니다. 이로 인해 클러스터의 용량이 거의 충분 하 고 부하가 많은 복제본을 배치, 교체 또는 이동 해야 하는 경우에는 때때로 새 복제본을 배치 하거나 실패 한 복제본을 대체할 수 있습니다.

더 많은 유연성을 제공 하기 위해 노드 버퍼 또는 과도 한 예약 용량을 지정할 수 있습니다. 메트릭에 대해 노드 버퍼 또는 과도 한 예약 용량이 지정 된 경우 클러스터 리소스 관리자는 버퍼 또는 과도 한 예약 용량이 사용 되지 않는 상태로 유지 되는 방식으로 복제본을 이동 하거나 이동 하려고 시도 하지만, 다음과 같은 서비스 가용성을 향상 시키는 작업에 필요한 경우 버퍼 또는 과도 한 예약 용량을 사용할 수 있습니다.

* 새 복제본 배치 또는 실패 한 복제본 바꾸기
* 업그레이드 중 배치
* 소프트 및 하드 제약 조건 위반 수정
* 모음은

노드 버퍼 용량은 지정 된 노드 용량 보다 낮은 용량의 예약 된 부분을 나타내고, 과도 한 예약 용량은 지정 된 노드 용량 보다 더 많은 용량을 나타냅니다. 두 경우 모두 클러스터 리소스 관리자는이 용량을 무료로 유지 하려고 시도 합니다.

예를 들어 노드에 메트릭 *CpuUtilization* 의 지정 된 용량 (100)이 있고 해당 메트릭에 대 한 노드 버퍼 백분율이 20%로 설정 된 경우, 전체 및 버퍼링 되지 않은 용량은 각각 100 및 80이 되며, 클러스터 리소스 관리자는 정상적인 상황에서 클러스터에 80 비해 5 개 이상의 부하 단위를 노드에 배치할 수 없습니다.

![총 용량이 노드 용량과 같음 (노드 버퍼 + 버퍼링 되지 않음)](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity.png)

위에서 언급 한 서비스 가용성을 높이는 작업에만 사용 되는 노드 용량의 일부를 예약 하려는 경우 노드 버퍼를 사용 해야 합니다.

반면 노드가 과도 하 게 예약 된 비율을 사용 하 고를 20%로 설정 하면 합계 및 버퍼링 되지 않은 용량이 각각 120 및 100이 됩니다.

![총 용량은 과도 한 예약 용량과 노드 용량을 더한 값과 같습니다 (과잉 예약 + 버퍼링 되지 않음).](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity-with-overbooking.png)

전체 리소스 사용량이 용량을 초과 하는 경우에도 클러스터 리소스 관리자에서 노드에 복제본을 추가 하도록 허용 하려면 과도 한 예약 용량을 사용 해야 합니다. 이를 사용 하 여 성능 저하로 서비스에 대 한 추가 가용성을 제공할 수 있습니다. 과도 한 예약을 사용 하는 경우 사용자 응용 프로그램 논리는 필요한 것 보다 더 많은 물리적 리소스에서 작동할 수 있어야 합니다.

노드 버퍼 또는 과도 한 예약 용량이 지정 된 경우 대상 노드의 총 부하가 총 용량을 초과 하는 경우 (노드 버퍼 및 노드 용량을 포함 한 노드 용량 및 과도 하 게 예약 된 경우 노드 용량 초과 예약 용량) 클러스터 리소스 관리자에서 복제본을 이동 하거나 이동 하지 않습니다.

과도 한 예약 용량을 무제한으로 지정할 수도 있습니다. 이 경우 클러스터 리소스 관리자는 노드의 총 부하를 지정 된 노드 용량 이하로 유지 하려고 하지만, 잠재적으로 노드에 부하가 훨씬 더 많은 부하가 발생 하 여 심각한 성능 저하가 발생할 수 있습니다.

메트릭은 노드 버퍼와 과도 한 예약 용량을 동시에 지정할 수 없습니다.

*ClusterManifest.xml* 에서 노드 버퍼 또는 초과 예약 용량을 지정 하는 방법의 예는 다음과 같습니다.
...
다음은 독립 실행형 배포에 대 한 *ClusterConfig.json* 을 통해 또는 Azure에서 호스트 되는 클러스터에 대 한 *Template.js에서* 노드 버퍼를 지정 하거나 과도 하 게 예약 하는 방법의 예입니다.
