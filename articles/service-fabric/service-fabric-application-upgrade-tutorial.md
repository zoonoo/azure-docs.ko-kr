...
Azure 서비스 패브릭을 사용하면 변경된 서비스만 업그레이드하고, 업그레이드 프로세스를 통해 애플리케이션 상태를 모니터링하여 클라우드 애플리케이션 업그레이드 프로세스를 간소화할 수 있습니다. 문제가 발생할 경우 애플리케이션이 자동으로 이전 버전으로 롤백합니다. 서비스 패브릭 애플리케이션 업그레이드는 *가동 중지 시간 없이* 애플리케이션을 업그레이드합니다. 이 자습서에서는 Visual Studio를 사용하여 롤링 업그레이드를 완료하는 방법을 설명합니다.
...
변경 내용을 저장하고 이제 **애플리케이션 업그레이드** 상자를 선택합니다.
