---
title: '서비스 패브릭 클러스터 리소스 관리자: 이동 비용'
description: 서비스 패브릭 서비스의 이동 비용과 동적 구성을 포함하여 아키텍처 요구 사항에 맞게 지정하는 방법에 대해 알아봅니다.
author: masnider
ms.topic: conceptual
ms.date: 08/18/2017
ms.author: masnider
...
MoveCost는 0, 낮음, 중간, 높음 및 매우 높은 다섯 가지 수준으로 나타났습니다. 다음 규칙이 적용됩니다.

* 이동비용은 0과 VeryHigh를 제외한 서로 상대적입니다. 
* 이동 비용 0은 이동 비용이 무료임을 의미하며 솔루션 점수 계산에 포함되지 않습니다.
* 이동 비용을 높음 또는 VeryHigh로 설정한다고 해서 복제본이 *이동되지 않는다는* 보장은 *없습니다.*
* VeryHigh 이동 비용이 있는 복제본은 다른 방법으로 수정할 수 없는 클러스터에 제약 조건 위반이 있는 경우에만 이동됩니다(위반을 해결하기 위해 다른 많은 복제본을 이동해야 하는 경우에도).
...
> [!IMPORTANT]
> VeryHigh 이동 비용을 사용하면 클러스터 리소스 관리자가 클러스터에서 전역적으로 최적의 배치 솔루션을 찾는 기능이 크게 제한되기 때문에 신중하게 고려해야 합니다. VeryHigh 이동 비용이 있는 복제본은 다른 방법으로 수정할 수 없는 클러스터에 제약 조건 위반이 있는 경우에만 이동됩니다(위반을 해결하기 위해 다른 많은 복제본을 이동해야 하는 경우에도).
