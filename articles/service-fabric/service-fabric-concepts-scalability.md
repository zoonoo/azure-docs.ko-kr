...
description: Azure 서비스 패브릭의 크기 조정 및 응용 프로그램의 크기를 조정하는 데 사용되는 다양한 기술에 대해 알아봅니다.
author: masnider
ms.topic: conceptual
ms.date: 08/26/2019
ms.author: masnider
...
해결 방법은 추적하려는 워크플로의 모든 다른 인스턴스에 대해 이 서비스의 인스턴스를 만드는 것입니다. 이것은 훌륭한 패턴이며 서비스가 상태 비수상태인지 상태 비상태인지 에 관계없이 작동합니다. 이 패턴이 작동하기 위해 일반적으로 "워크로드 관리자 서비스" 역할을 하는 또 다른 서비스가 있습니다. 이 서비스의 작업은 요청을 받고 이러한 요청을 다른 서비스로 라우팅하는 것입니다. 메시지를 받은 관리자에서 워크로드 서비스의 인스턴스를 동적으로 만든 다음 해당 서비스에 요청을 전달할 수 있습니다. 관리자 서비스는 지정된 워크플로 서비스에서 작업을 완료할 때 콜백을 받을 수도 있습니다. 관리자에서 이러한 콜백을 받으면 워크플로 서비스의 해당 인스턴스를 삭제하거나 더 많은 호출이 예상되는 경우 관리자에게 남겨 둘 수 있습니다. 
...
운영 체제 간의 구현 차이로 인해 Windows 또는 Linux에서 서비스 패브릭을 사용하도록 선택하는 것은 응용 프로그램 확장의 중요한 부분이 될 수 있습니다. 한 가지 잠재적인 장벽은 스테이지로 된 로깅이 수행되는 방식입니다. Windows의 서비스 패브릭은 상태 풀 서비스 복제본 간에 공유되는 컴퓨터당 한 개의 로그에 커널 드라이버를 사용합니다. 이 로그의 무게는 약 8GB입니다. 반면 Linux는 각 복제본에 대해 256MB 스테이징 로그를 사용하므로 지정된 노드에서 실행되는 경량 서비스 복제본수를 최대화하려는 응용 프로그램에는 적합하지 않습니다. 임시 저장소 요구 사항의 이러한 차이는 서비스 패브릭 클러스터 배포에 필요한 플랫폼을 잠재적으로 알릴 수 있습니다.
