...
| FailureAction | 업그레이드가 실패한 경우 서비스 패브릭이 수행할 동작입니다. 응용 프로그램은 업데이트 이전의 마지막 버전으로 롤백되거나(Rollback), 현재 업그레이드 도메인에서 응용 프로그램 업드레이드를 중지하고 업그레이드 모드를 수동으로 변경할 수 있습니다. 허용되는 값은 Rollback 및 Manual입니다. |
...
| MaxPercentUnhealthyDeployedApplications | 기본 및 권장값은 0입니다. 응용 프로그램이 비정상이고 업그레이드에 실패했다고 간주하기 전에 비정상이 될 수 있는 배포된 응용 프로그램의 최대 수를 지정합니다([상태 섹션](service-fabric-health-introduction.md) 참조). 이는 노드에 패키지된 응용 프로그램의 상태이며, 따라서 업그레이드하는 동안의 문제 및 노드에서 배포된 응용 프로그램 패키지가 비정상(충돌 등)인 경우를 즉시 감지하는 데 유용합니다. 일반적인 경우에는 응용 프로그램의 복제본이 다른 노드에 부하를 분담하기 때문에 응용 프로그램이 정상으로 나타나고 업그레이드를 진행할 수 있습니다. 엄격한 MaxPercentUnhealthyDeployedApplications 상태를 지정하여 서비스 패브릭은 응용 프로그램 패키지의 문제를 빠르게 감지하고 빠른 업그레이드 실패 결과를 얻을 수 있습니다. |
