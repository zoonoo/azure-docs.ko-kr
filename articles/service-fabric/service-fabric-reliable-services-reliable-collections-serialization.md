---
title: 신뢰할 수 있는 컬렉션 개체 직렬화
description: Azure Service 패브릭 신뢰할 수 있는 컬렉션 개체 직렬화(기본 전략 및 사용자 지정 직렬화 를 정의하는 방법)에 대해 알아봅니다.
ms.topic: conceptual
ms.date: 5/8/2017
...
[IReliableStateManager.TryAddStateSerializer\<T>](https://docs.microsoft.com/dotnet/api/microsoft.servicefabric.data.ireliablestatemanager.tryaddstateserializer) 지정된 형식 T에 대 한 사용자 지정 직렬화기를 등록 하는 데 사용 됩니다. 이 등록은 복구가 시작되기 전에 모든 신뢰할 수 있는 컬렉션이 관련 serializer에 액세스하여 지속된 데이터를 읽을 수 있도록 StatefulServiceBase를 구성할 때 발생해야 합니다.
...
사용자 지정 직렬화자는 [IState\<Serializer T>](https://docs.microsoft.com/dotnet/api/microsoft.servicefabric.data.istateserializer-1) 인터페이스를 구현해야 합니다.

> [!NOTE]
> IStateSerializer\<T> 쓰기 및 읽기에 대 한 오버 로드를 포함 하는 추가 T 라는 기본 값에 걸립니다. 이 API는 차등 serialization에 사용됩니다. 현재 차등 serialization 기능은 노출되지 않습니다. 따라서 이러한 두 오버로드는 차등 serialization이 노출되고 사용될 때까지 호출되지 않습니다.
...
다음은 IStateSerializer\<OrderKey> 예제 구현입니다.
...
## <a name="next-steps"></a>다음 단계
  * [직렬화 및 업그레이드](service-fabric-application-upgrade-data-serialization.md)
