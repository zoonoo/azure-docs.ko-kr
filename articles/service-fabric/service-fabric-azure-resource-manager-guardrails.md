...
### <a name="overview"></a>개요
Service Fabric 클러스터에는 클러스터의 주 노드 유형에 서 실행 되는 시스템 서비스의 복제본 수를 결정 하는 데 사용 되는 [안정성 계층](./service-fabric-cluster-capacity.md#reliability-characteristics-of-the-cluster) 속성이 있습니다. 필요한 복제본 수는 클러스터의 주 노드 형식에서 유지 관리 해야 하는 최소 노드 수를 결정 합니다. 주 노드 형식의 노드 수가 안정성 계층에 필요한 최소 미만이 면 클러스터가 불안정 해지기 때문입니다.  
...
* 안정성 계층이 청동 인 경우 다음 [단계](./service-fabric-cluster-scale-in-out.md#manually-remove-vms-from-a-node-typevirtual-machine-scale-set) 에 따라 클러스터를 정상적으로 확장 하세요.
