...
4. 애플리케이션 만들기 명령에서 아래 애플리케이션을 만들기 위해 **ListenPort** 애플리케이션 매개 변수를 적어 둡니다. 이 애플리케이션 매개 변수에 대해 지정된 이 값은 Azure Files 볼륨 플러그 인이 Docker 디먼의 요청을 수신하는 포트입니다. 애플리케이션에 제공된 포트가 클러스터 또는 애플리케이션에서 사용하는 다른 포트와 충돌하지 않는지 확인하는 것은 중요합니다.
