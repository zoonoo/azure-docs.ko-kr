...
### <a name="i-did-not-specify-a-health-policy-for-application-upgrade-but-the-upgrade-still-fails-for-some-time-outs-that-i-never-specified"></a>응용 프로그램 업그레이드에 대한 상태 정책을 지정하지 않았는데도 지정한 적이 없는 시간 제한 때문에 업그레이드에 실패합니다.
