...
- **재해 복구 의 경우**: 전체 서비스 패브릭 클러스터의 영구적 손실.
- **데이터 손실 사례**: 서비스 파티션의 복제본 대부분이 영구적으로 손실됩니다.
- **데이터 손실의 경우**: 서비스의 우발적 인 삭제 또는 손상. 예를 들어 관리자가 실수로 서비스를 삭제합니다.
- **데이터 손상 사례**: 서비스의 버그로 인해 데이터가 손상됩니다. 예를 들어 신뢰할 수 있는 컬렉션에 잘못된 데이터를 작성하는 서비스 코드가 업그레이드되는 경우 데이터 손상이 발생할 수 있습니다. 그러한 경우 코드와 데이터 모두 이전 상태로 복구해야 할 수도 있습니다.
...
서비스 패브릭 탐색기에서 복원을 트리거할 수 있습니다. 서비스 패브릭 탐색기 설정에서 고급 모드가 활성화되어 있는지 확인합니다.
1. 원하는 파티션을 선택하고 작업을 클릭합니다. 
2. 트리거 파티션 복원을 선택하고 Azure에 대한 정보를 입력합니다.
...
### <a name="data-restore-for-_data-corruption__data-loss_"></a>데이터 _손상_/데이터_손실에_ 대한 데이터 복원
...
복원을 트리거하려면 목록에서 백업을 선택합니다. 현재 _데이터 손실_/_데이터 손상의_경우 다음 백업을 선택합니다.
...
1. **수락됨**: _수락된_ 복원 상태는 요청된 파티션이 올바른 요청 매개 변수로 트리거되었음을 나타냅니다.
    ```
    ...
    RestoredLsn   : 3552
    ```
2. **진행 중**: _진행 중_ 복원 상태는 요청에 언급 된 백업과 함께 파티션에서 복원이 발생했음을 나타냅니다. 파티션은 _데이터 손실_ 상태를 보고합니다.
    ```
...
3. **성공,** **실패**또는 **시간 시간:** 요청된 복원은 다음 상태 중 어느 상태에서도 완료할 수 있습니다. 각 상태에는 다음과 같은 중요도 및 응답 세부 정보가 있습니다.
    - **성공**: _성공_ 복원 상태는 회복 된 파티션 상태를 나타냅니다. 파티션은 시간(UTC)과 함께 _RestoredEpoch_ 및 _RestoredLSN_을 보고합니다.
        ...
        RestoredLsn   : 3552
        ```        
    - **실패**: _오류_ 복원 상태는 복원 요청의 실패를 나타냅니다. 오류의 원인이 보고됩니다.
        ...
        RestoredLsn   : 0
        ```
    - **시간 시간:** _시간 시간 삭제_ 복원 상태는 요청에 시간 시간이 있음을 나타냅니다. [RestoreTimeout](https://docs.microsoft.com/rest/api/servicefabric/sfclient-api-backuppartition#backuptimeout)이 더 큰 새 복원 요청을 만듭니다. 시간 제한은 기본적으로 10분입니다. 복원을 다시 요청하기 전에 파티션이 데이터 손실 상태에서 벗어났는지 확인해야 합니다.
