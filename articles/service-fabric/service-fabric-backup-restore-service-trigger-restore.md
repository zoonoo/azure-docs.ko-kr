...
- **재해 복구의 경우**: 전체 Service Fabric 클러스터가 영구적으로 손실.
- **데이터 손실의 경우**: 서비스 파티션의 복제본 대부분이 영구적으로 손실.
- **데이터 손실의 경우**: 우연한 실수로 인한 삭제 또는 서비스의 손상. 예를 들어 관리자가 실수로 서비스를 삭제합니다.
- **데이터 손상의 경우**: 서비스의 버그로 인해 데이터가 손상됨. 예를 들어 신뢰할 수 있는 컬렉션에 잘못된 데이터를 작성하는 서비스 코드가 업그레이드되는 경우 데이터 손상이 발생할 수 있습니다. 그러한 경우 코드와 데이터 모두 이전 상태로 복구해야 할 수도 있습니다.
...
- 구성 호출 생성을 위해 Microsoft.ServiceFabric.PowerShell.Http 모듈(프리뷰)을 설치합니다.
...
- Microsoft.ServiceFabric.PowerShell.Http 모듈을 사용하여 구성 요청을 생성하기 전에 `Connect-SFCluster` 명령을 사용하여 클러스터를 연결해야 합니다.
...
> [!NOTE]
> Powershell을 사용하여 파티션을 복원할 때 backuplocation에 ' $ '가 있는 경우 ' ~ '를 사용하여 이스케이프 합니다
>

### <a name="using-service-fabric-explorer"></a>Service Fabric Explorer 사용하기
Service Fabric Explorer에서 복원을 트리거할 수 있습니다. Service Fabric Explorer 설정에서 고급 모드를 사용하도록 설정했는지 확인합니다.
1. 원하는 파티션을 선택하고 작업을 클릭합니다. 
2. Azure에 대한 파티션 복원 및 정보 입력 트리거를 선택합니다.

    ![파티션 복원 트리거][2]

    또는 FileShare:

    ![파티션 복원 FileShare 트리거][3]

### <a name="data-restore-for-_data-corruption__data-loss_"></a>_데이터 손상_/_데이터 손실_ 시 데이터 복원
...
복원을 트리거하려면 목록에서 백업을 선택합니다. 현재 _데이터 손실_/_데이터 손상_ 의 경우 다음과 같은 백업을 선택합니다.
...
1. **수락됨**: _수락된_ 복원 상태는 올바른 요청 매개 변수를 사용하여 요청된 파티션이 트리거됨을 나타냅니다.
    ```
    ...
    RestoredLsn   : 3552
    ```
2. **진행중**: _진행 중인_ 복원 상태는 요청에서 언급된 백업을 사용하여 파티션에서 복원이 발생 중임을 나타냅니다. 파티션은 _데이터 손실_ 상태를 보고합니다.
    ```
...
3. **성공**, **실패**, 또는 **시간 초과**: 요청된 복원이 다음 상태로 완료될 수 있습니다. 각 상태에는 다음과 같은 중요도 및 응답 세부 정보가 있습니다.
    - **성공**: _성공_ 복원 상태는 회복된 파티션 상태를 표시합니다. 파티션은 시간(UTC)과 함께 _RestoredEpoch_ 및 _RestoredLSN_ 을 보고합니다.
    ...
    - **실패**: _실패_ 복원 상태는 복원 요청의 실패를 나타냅니다. 오류의 원인이 보고됩니다.
        ...
        RestoredLsn   : 0
        ```
    - **시간 초과**: _시간 초과_ 복원 상태는 요청 시간이 초과되었음을 나타냅니다. [RestoreTimeout](/rest/api/servicefabric/sfclient-api-backuppartition#backuptimeout)이 더 큰 새 복원 요청을 만듭니다. 시간 제한은 기본적으로 10분입니다. 복원을 다시 요청하기 전에 파티션이 데이터 손실 상태에서 벗어났는지 확인해야 합니다.
