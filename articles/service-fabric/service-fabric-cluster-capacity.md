...
클러스터에서 시작해야 하는 노드 유형의 수를 설정합니다.  각 노드 유형은 Virtual Machine 크기 조정 설정에 매핑됩니다. 각 노드 형식은 독립적으로 확장 또는 축소되고, 다른 포트의 집합을 열며 다른 용량 메트릭을 가질 수 있습니다. 따라서 노드 유형의 수를 결정할 때 기본적으로 다음 사항을 고려합니다.
...
1. 가상 컴퓨터 확장 집합 및 기타 관련 Azure 리소스에 대한 배포가 지연되거나, 시간이 초과되거나, 클러스터 또는 인프라 수준에서 발생한 문제로 인해 완전히 차단될 수 있습니다. 
...
3. 골드 또는 실버 내구성 수준이 활성화된 가상 컴퓨터 확장 집합의 노드 수는 최소 5개로 유지합니다.
4. 임의의 VM 인스턴스를 삭제하지 말고 항상 가상 컴퓨터 확장 집합 규모 축소 기능을 사용하세요. 임의의 VM 인스턴스를 삭제하면 UD와 FD 전체에서 VM 인스턴스의 불균형 생성이 확산될 가능성이 있습니다. 이러한 불균형은 서비스 인스턴스/서비스 복제본 간에 부하를 적절하게 분산하는 시스템 기능에 부정적인 영향을 줄 수 있습니다.
...
* [가상 컴퓨터 확장 집합에 대한 Nodetype의 관계](service-fabric-cluster-nodetypes.md)
