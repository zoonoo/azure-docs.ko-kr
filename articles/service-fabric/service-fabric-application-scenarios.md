...
Service Fabric의 고유한 아키텍처는 애플리케이션에서 거의 실시간으로 데이터 분석, 메모리 내 계산, 병렬 트랜잭션 및 이벤트 처리가 가능합니다. 변화 하는 리소스 요구 사항에 따라 응용 프로그램을 쉽게 확장 하거나 축소할 수 있습니다.

응용 프로그램 빌드에 대 한 디자인 지침은 [Azure Service Fabric의 마이크로 서비스 아키텍처](/azure/architecture/reference-architectures/microservices/service-fabric) 및 [Service Fabric를 사용 하는 응용 프로그램 디자인에 대 한 모범 사례](service-fabric-best-practices-applications.md)를 참조 하세요.
...
* **데이터에 대 한 계산**: Service Fabric을 사용 하면 데이터 계산을 많이 수행 하는 상태 저장 응용 프로그램을 빌드할 수 있습니다. Service Fabric를 사용 하면 응용 프로그램에서 처리 (계산) 및 데이터를 공동 배치할 수 있습니다. 
...
* **고가용성 서비스**: Service Fabric는 여러 보조 서비스 복제본을 만들어 빠른 장애 조치 (failover)를 제공 합니다. 노드, 프로세스 또는 개별 서비스가 하드웨어 또는 다른 오류로 인해 중단되면 최소한의 서비스 손실로 보조 복제본 중 하나가 주 복제본으로 승격됩니다.

* **확장 가능한 서비스**: 개별 서비스를 분할하여 클러스터 전체로 상태를 확장할 수 있습니다. 개별 서비스를 즉시 만들고 제거할 수도 있습니다. 몇몇 노드의 몇 개 인스턴스에서 여러 노드의 서비스를 수천 개의 인스턴스로 확장 한 다음 필요에 따라 다시 확장할 수 있습니다. Service Fabric를 사용 하 여 이러한 서비스를 빌드하고 전체 수명 주기를 관리할 수 있습니다.

## <a name="application-design-case-studies"></a>애플리케이션 디자인 사례 연구

응용 프로그램을 디자인 하는 데 Service Fabric를 사용 하는 방법을 보여 주는 사례 연구는 [고객 사례](https://customers.microsoft.com/search?sq=%22Azure%20Service%20Fabric%22&ff=&p=2&so=story_publish_date%20desc) 와 [Azure 사이트의 마이크로 서비스](https://azure.microsoft.com/solutions/microservice-applications/) 에 게시 됩니다.
...
마이크로 서비스로 구성 된 응용 프로그램을 작성 하는 경우 일반적으로 상태 비저장 웹 앱 (예: ASP.NET 및 Node.js)의 조합을 통해 상태 비저장 및 상태 저장 비즈니스 중간 계층 서비스를 호출 합니다. 앱과 서비스는 모두 Service Fabric 배포 명령을 통해 동일한 Service Fabric 클러스터에 배포 됩니다. 이러한 각 서비스는 규모, 안정성 및 리소스 사용량과는 독립적입니다. 이러한 독립성은 개발 및 수명 주기 관리의 민첩성과 유연성을 향상 시킵니다.
...
다음 다이어그램은 상태 비저장 응용 프로그램을 디자인 하는 것과 상태 저장 인 응용 프로그램을 디자인 하는 경우의 차이점을 보여 줍니다. [Reliable Services](service-fabric-reliable-services-introduction.md) 및 [Reliable Actors](service-fabric-reliable-actors-introduction.md) 프로그래밍 모델을 활용 하 여 상태 저장 서비스는 높은 처리량과 짧은 대기 시간을 달성 하는 동시에 응용 프로그램 복잡성을 줄입니다.

상태 비저장 서비스를 사용 하는 응용 프로그램 예제는 ![ 상태 비저장 서비스를 사용 하는 응용 프로그램입니다.][Image1]

상태 저장 서비스를 사용 하는 응용 프로그램 예제는 ![ 상태 저장 서비스를 사용 하는 응용 프로그램입니다.][Image2]
