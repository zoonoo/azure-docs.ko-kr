---
title: Azure Service Fabric 클러스터 크기 조정
description: Azure Service Fabric 클러스터의 스케일 인 또는 스케일 아웃, 규모 확장 또는 규모 축소에 대해 알아봅니다. 응용 프로그램 요구 사항이 변경 되 면 클러스터를 Service Fabric 수 있습니다.
ms.topic: conceptual
ms.date: 11/13/2018
ms.author: atsenthi
...
이 '홈 수행 '수제작' 자동 크기 조정 기능을 구현하는 한 가지 방법은 크기 조정 작업을 관리하는 새로운 상태 비저장 서비스를 Service Fabric 애플리케이션에 추가하는 것입니다. 사용자 고유의 크기 조정 서비스를 만들면 애플리케이션 동작을 최대한 원하는 대로 제어하고 사용자 지정할 수 있습니다. 이는 응용 프로그램의 크기를 조정 하거나 크기를 조정 하는 시기 또는 방법을 세밀 하 게 제어 해야 하는 시나리오에 유용할 수 있습니다. 그러나이 컨트롤은 코드 복잡성의 장단점을 제공 합니다. 이 방법을 사용하려면 특수한 크기 조정 코드가 필요합니다. 서비스의 `RunAsync` 메서드 내에서 트리거 집합은 크기 조정이 필요한지 여부(최대 클러스터 크기 및 크기 조정 휴지 시간 같은 매개 변수 확인 포함)를 확인할 수 있습니다.   

가상 머신 확장 집합 상호 작용 (현재 가상 머신 인스턴스 수를 확인 하 고 수정 하는 데 사용)에 사용 되는 API는 [흐름 Azure Management Compute 라이브러리](https://www.nuget.org/packages/Microsoft.Azure.Management.Compute.Fluent/)입니다. fluent compute 라이브러리는 가상 머신 확장 집합과 상호 작용하는 간편한 API를 제공합니다.  Service Fabric 클러스터 자체와 상호 작용하려면 [System.Fabric.FabricClient](/dotnet/api/system.fabric.fabricclient)를 사용하세요.
...
가상 머신 확장 집합은 가상 머신의 컬렉션을 집합으로 배포하고 관리하는 데 사용할 수 있는 Azure 컴퓨팅 리소스입니다. Azure 클러스터에 정의된 모든 노드 유형은 [별도의 확장 집합으로 설정](service-fabric-cluster-nodetypes.md)됩니다. 각 노드 형식을 별도로 관리할 수 있습니다.  노드 형식의 크기를 확장 하거나 축소 하려면 새 노드 형식 (업데이트 된 VM SKU 포함)과 이전 노드 형식의 제거를 추가 해야 합니다.
...
업데이트 된 VM SKU를 사용 하 여 새 주 노드 유형을 배포한 다음 시스템 서비스를 새 확장 집합으로 마이그레이션하기 위해 원래 주 노드 유형 인스턴스를 한 번에 하나씩 사용 하지 않도록 설정 합니다. 클러스터 및 새 노드가 정상 인지 확인 하 고 삭제 된 노드에 대 한 원래 크기 집합 및 노드 상태를 제거 합니다.
