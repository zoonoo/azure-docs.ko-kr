...
---
# <a name="monitoring-and-diagnostics-in-azure-service-fabric"></a>Azure Service Fabric에서 진단 모니터링

모니터링 및 진단은 프로덕션 환경에서의 개발, 테스트 및 성공적인 배포에 중요합니다. Service Fabric 솔루션은 모니터링 및 진단을 처음부터 구현할 때 가장 적합하며, 로컬 개발 환경 및 실제 프로덕션 클러스터 설정에서 서비스가 원활하게 작동하도록 하는 데 도움이 됩니다.
...
모니터링 및 진단은 모든 기능이 예상대로 작동하도록 하는 데 중요하며, 서비스 중단을 최소화하면서 상황에 대처할 수 있도록 합니다. 

모니터링 및 진단의 전체 워크플로는 다음 세 단계로 구성됩니다. 
1. **이벤트 생성**: 인프라(클러스터) 및 응용 프로그램/서비스 수준의 이벤트(로그, 추적, 사용자 지정 이벤트)가 모두 포함됩니다. 
2. **이벤트 집계**: 생성된 이벤트를 표시하려면 먼저 수집하고 집계해야 합니다. 이 작업은 Microsoft Azure 진단을 통해 설정된 저장소 테이블에 의해 또는 EventFlow 파이프라인을 만들어 수행될 수 있습니다.
3. **분석**: 필요에 따라 분석하고 표시할 수 있도록 이벤트를 특정 형식으로 시각화되고 액세스할 수 있어야 합니다. ApplicationInsights, Operations Management Suite, Kibana 등과 같은 여러 도구를 사용하도록 선택할 수 있습니다.

이러한 측면 모두를 다루는 여러 제품을 사용할 수 있지만, 많은 고객이 모니터링의 각 측면에 대해 서로 다른 기술을 선택합니다. 이에 따라 각 부분이 서로 연동되면서 응용 프로그램에 종단 간 모니터링 솔루션을 제공하는 것이 중요합니다.

필요에 따라 사용하도록 선택할 수 있는 분석 도구에 기본적으로 제공되는 경우가 많은 자동화된 경고 및 완화를 포함하도록 진단 및 모니터링을 추가로 확장할 수 있습니다. 

## <a name="event-generation"></a>이벤트 생성

인프라 계층(클러스터, 컴퓨터 또는 Service Fabric 작업) 또는 응용 프로그램 계층(앱에 추가된 인프라 및 클러스터에 배포된 서비스)에서 이벤트, 로그 및 추적을 생성할 수 있습니다.

### <a name="infrastructure-monitoring-the-cluster"></a>인프라: 클러스터 모니터링

Service Fabric을 사용하면 인프라 오류 시에도 응용 프로그램을 계속 실행할 수 있지만, 응용 프로그램이나 기본 인프라에서 오류가 발생하는지를 여전히 파악해야 합니다. 용량 계획을 위해 인프라를 모니터링해야 하므로 인프라를 추가하거나 제거할 시기를 파악할 수 있습니다. Service Fabric 배포를 구성하는 인프라와 응용 프로그램을 모두 모니터링하고 문제를 해결하는 것이 중요합니다. 고객이 응용 프로그램을 사용할 수 있는 경우에도 인프라에 잠재적인 문제 원인이 있을 수 있습니다. 클러스터가 예상대로 동작하도록 하기 위해 Service Fabric은 기본적으로 5개의 다른 로깅 채널을 설정합니다.

1. 작동 채널: Service Fabric 및 클러스터에서 수행하는 상위 수준 작업으로, 노드에 대해 발생하는 이벤트, 배포되는 새 응용 프로그램 또는 SF 업그레이드 롤백 등이 포함됩니다. 
2. 고객 정보 채널: 상태 보고서 및 부하 분산 결정
3. Reliable Services 이벤트: 프로그래밍 모델 특정 이벤트
4. Reliable Actors 이벤트: 프로그래밍 모델 특정 이벤트 및 성능 카운터
5. 지원 로그: 지원을 제공할 때만 사용되는 Service Fabric에서 생성된 시스템 로그

클러스터 생성 시 "진단"을 사용하도록 설정하는 것이 좋습니다. 이 작업은 아래에 표시된 방식으로 또는 진단 기능을 포함하는 Azure Resource Manager 템플릿을 사용하여 포털에서 수행할 수 있습니다. 

![진단이 사용되도록 설정된 Azure Portal 클러스터 만들기](./media/service-fabric-diagnostics-overview/azure-enable-diagnostics.png)

위에 표시된 것처럼 Application Insights(AppInsights) 계측 키를 추가할 선택적 필드도 있습니다. 이벤트 분석에 AppInsights를 사용하려는 경우(AppInsights는 권장 솔루션 중 하나임) 여기에 AppInsights 리소스 instrumentationKey(GUID)를 포함합니다.
...
### <a name="application-instrumenting-code-for-custom-events"></a>응용 프로그램: 사용자 지정 이벤트에 대한 계측 코드
...
## <a name="event-aggregation-and-collection"></a>이벤트 집계 및 수집
...
모니터링의 마지막 부분은 이벤트 스트림을 시각화하고, 서비스 성능을 보고하고, 문제가 감지되면 경고하는 것입니다. 이러한 모니터링 측면에 대해 서로 다른 솔루션을 사용할 수 있습니다. 이벤트 스트림을 기준으로 경고하기 위한 AppInsights 및 OMS(Operations Management Suite) Microsoft Power BI 또는 [Kibana](https://www.elastic.co/products/kibana) 또는 [Splunk](https://www.splunk.com/)와 같은 타사 솔루션을 사용하여 데이터를 시각화할 수 있습니다.

### <a name="appinsights"></a>AppInsights

AppInsights는 응용 프로그램 및 서비스를 모니터링하기 위한 권장 도구 중 하나입니다. 업데이트된 AI.SDK는 Service Fabric 이벤트에서 잘 작동하며, 적절한 데이터 시각화 및 쿼리 도구를 제공(AppInsights 분석을 통해)하는 것 외에, 응용 프로그램 또는 클러스터의 프로세스 간 종속성을 추적하는 데 도움이 될 수 있는 정확한 AppMap을 만들 수도 있습니다.

Azure Marketplace에서 "Application Insights"를 검색하여 AppInsights 리소스를 설정합니다. 하나 만든 후에 *속성*으로 이동하여 AI 계측 키(GUID 형태)를 찾습니다. 이 키는 다음에 사용됩니다.
* 진단이 사용되도록 설정되어 있다고 가정할 경우 클러스터를 만들 때 Azure Resource Manager 템플릿 또는 Azure Portal을 통해 직접 Service Fabric 클러스터에서 인프라 수준 이벤트를 받도록 AppInsights 통합
* 위의 섹션에 나와 있는 것처럼 Application Insights로 데이터를 출력하도록 EventFlow(eventFlowConfig.json) 구성

### <a name="oms"></a>OMS

OMS는 Service Fabric 클러스터의 진단 및 모니터링을 위한 또 다른 권장 도구입니다. Service Fabric 솔루션을 작업 영역에 추가할 수 있으며 대시보드에 다양한 유형의 Service Fabric 이벤트가 표시됩니다. OMS 작업 영역의 Log Analytics에는 강력한 로그 쿼리 도구도 있습니다.

OMS 작업 영역을 구성하려면 클러스터에 대해 진단이 사용되도록 설정되어 있는지 확인합니다. Azure Marketplace의 "Service Fabric 분석"을 기존 OMS 작업 영역에 추가하거나 새로 만듭니다. 클러스터에서 이벤트를 쓰는 Azure Storage 테이블에 연결하도록 작업 영역 데이터 원본을 구성합니다. 

OMS에서 사용자 지정 이벤트를 선택하도록 하려면 응용 프로그램에 추가하는 계측이 할 계측 동일한 저장소 테이블 또는 작업 영역에 대한 원본으로도 구성된 다른 저장소 테이블에도 쓰도록 해야 합니다. 

또한 OMS는 Service Fabric을 통해 조정되는 컨테이너에서 잘 작동하는 작업 영역에 컨테이너 솔루션을 추가할 수 있으므로 컨테이너에 대한 모니터링 및 진단이 필요한 경우 데이터 시각화 및 분석에 권장되는 경로이기도 합니다. 이를 설정하는 방법에 대한 간단한 가이드를 보려면 [여기](service-fabric-diagnostics-containers-windowsserver.md)로 이동하세요.
...
Watchdog은 서비스 전체에서 상태와 부하를 감시하고 상태 모델 계층 구조 상의 모든 항목에 대한 상태를 보고할 수 있는 별도의 서비스입니다. 이렇게 하면 단일 서비스 보기를 기반으로 하여 감지되지 않는 오류를 방지할 수 있습니다. 또한 Watchdog은 사용자가 개입하지 않아도 알려진 조건에 대한 수정 작업을 수행할 수 있는 코드를 호스팅하기에 좋은 출발점이기도 합니다. [여기](https://github.com/Azure-Samples/service-fabric-watchdog-service)에서 샘플 watchdog 서비스 구현을 찾을 수 있습니다.
