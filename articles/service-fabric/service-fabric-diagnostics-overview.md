...
![스크린샷에는 NodeDown 이벤트를 포함하여 노드 창의 여러 이벤트에 대한 이벤트 탭이 나와 있습니다.](media/service-fabric-diagnostics-overview/eventstore.png)

제공되는 진단은 기본적으로 포괄적인 이벤트 세트 형식으로 되어 있습니다. 이러한 [Service Fabric 이벤트](service-fabric-diagnostics-events.md)는 노드, 애플리케이션, 서비스, 파티션 등의 여러 엔터티에 대해 플랫폼에서 수행하는 작업을 보여 줍니다. 위에 나와 있는 마지막 시나리오에서 노드의 작동이 중단된 경우 플랫폼에서 `NodeDown` 이벤트를 내보내고 선택한 모니터링 도구를 통해 즉시 알림을 받을 수 있습니다. 다른 일반적인 예로 장애 조치(failover) 동안의 `ApplicationUpgradeRollbackStarted` 또는 `PartitionReconfigured`가 있습니다. **동일한 이벤트를 Windows 및 Linux 클러스터 둘 다에서 사용할 수 있습니다.**

이벤트가 Windows 및 Linux 둘 다의 표준 채널을 통해 전송되며, 이러한 채널을 지원하는 모니터링 도구에서 읽을 수 있습니다. Azure Monitor 솔루션은 Azure Monitor 로그입니다. 클러스터에 대한 사용자 지정 작업 대시보드와 경고를 만들 수 있는 샘플 쿼리가 몇 가지 포함되어 있는 [Azure Monitor 로그 통합](service-fabric-diagnostics-event-analysis-oms.md)에 대해 자세히 알아보세요. [플랫폼 수준 이벤트 및 로그 생성](service-fabric-diagnostics-event-generation-infra.md)에서 더 많은 클러스터 모니터링 개념을 확인할 수 있습니다.
...
이러한 정보를 수집하기 위한 Azure Monitor 솔루션은 플랫폼 수준 모니터링과 같은 Azure Monitor 로그입니다. 적절한 성능 카운터를 수집하고 Azure Monitor 로그에서 이를 확인하려면 [Log Analytics 에이전트](service-fabric-diagnostics-oms-agent.md)를 사용해야 합니다.
...
* [진단 에이전트](service-fabric-diagnostics-event-aggregation-wad.md) 및 [Azure Monitor 로그](service-fabric-diagnostics-oms-setup.md)를 사용하는 클러스터 모니터링
* [Azure Monitor 로그](service-fabric-diagnostics-oms-agent.md)를 사용하는 인프라 모니터링
...
* [클러스터에 대한 Azure Monitor 로그 설정](service-fabric-diagnostics-oms-setup.md)에서 Service Fabric을 사용하여 Azure Monitor 로그 통합을 구성합니다.
* 컨테이너 모니터링을 위한 Azure Monitor 로그 설정 방법 - [Azure Service Fabric에서 Windows 컨테이너에 대한 모니터링 및 진단](service-fabric-tutorial-monitoring-wincontainers.md)에 대해 알아봅니다.
