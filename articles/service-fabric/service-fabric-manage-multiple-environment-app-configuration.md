...
간단한 예로 상태 비저장 서비스에 대해 `InstanceCount`를 고려합니다. Azure에서 응용 프로그램을 실행할 때 일반적으로 이 매개 변수를 "-1"이라는 특수 값으로 설정하고자 합니다. 이 서비스가 클러스터의 모든 노드에서 실행 중인지 확인합니다. 그러나 단일 컴퓨터의 동일한 끝점에서 수신하는 여러 프로세스를 가질 수 없으므로 이 구성은 단일 컴퓨터 클러스터에 적합하지 않습니다. 대신 일반적으로 `InstanceCount`를 "1"로 설정합니다.
...
>[AZURE.NOTE] 서비스 구성 설정의 경우 키의 값을 설정할 수 있는 세 가지 위치는 다음과 같습니다. 서비스 구성 패키지, 응용 프로그램 매니페스트 및 응용 프로그램 매개 변수 파일. 서비스 패브릭은 (지정된 경우) 항상 응용 프로그램 매개 변수 파일에서 먼저 선택한 다음 응용 프로그램 매니페스트 및 마지막으로 구성 패키지에서 선택합니다.
...
기본적으로 새 응용 프로그램은 Local.xml 및 Cloud.xml라는 두 응용 프로그램 매개 변수 파일을 포함합니다.
