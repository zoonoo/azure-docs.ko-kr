...
> [!NOTE]
> 주 노드 형식에서 최소 수보다 작게 크기를 조정 하면 클러스터가 불안정 해질 수 있습니다. 이 경우 애플리케이션 및 시스템 서비스에 대한 데이터가 손실될 수 있습니다.
> 
> 
...
> [!NOTE]
> Vm을 추가 하는 데 시간이 걸리므로 추가는 즉각적이 될 것입니다. 복제본/서비스 인스턴스가 배치 될 때 VM 용량을 사용할 수 있을 때까지 10 분 넘게 걸릴 수 있도록 용량을 미리 추가 하도록 계획 합니다.
> 
...
Service fabric 시스템 서비스는 클러스터의 주 노드 형식에서 실행 됩니다. 주 노드 유형에 서 크기를 조정 하는 경우 인스턴스 수를 [안정성 계층](service-fabric-cluster-capacity.md) 에서 지 원하는 것 보다 작게 축소 하면 안 됩니다. 
...
Service Fabric Explorer에 나열된 노드는 서비스 패브릭 시스템 서비스(특히 FM)가 클러스터에 포함된 노드 수에 대해 알고 있는 내용이 반영된 것입니다. 에서 가상 머신 확장 집합의 크기를 조정 하면 VM이 삭제 되지만 FM 시스템 서비스는 삭제 된 VM에 매핑된 노드 (삭제 된 VM에 매핑된 노드)가 돌아올 것으로 간주 합니다. 따라서 Service Fabric Explorer는 성능 상태가 오류 또는 알 수 없음이어도 해당 노드를 계속 표시합니다.
