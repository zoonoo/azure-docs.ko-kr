...
Azure 서비스 패브릭은 상태 비저장 및 상태 저장 서비스 작성 및 관리를 간소화합니다. 이 항목은 다음에 대해 설명합니다.
...
*신뢰할 수 있는 서비스는* 서비스 패브릭에서 사용할 수 있는 프로그래밍 모델 중 하나입니다. 다른 하나는 신뢰할 수 있는 서비스 모델 위에 [가상 행위자](https://research.microsoft.com/en-us/projects/orleans/) 응용 프로그램 프레임 워크를 제공 하는 신뢰할 수 있는 *행위자* 프로그래밍 모델입니다. Reliable Actors에 대한 자세한 내용은 [Service Fabric Reliable Actors 소개](service-fabric-reliable-actors-introduction.md)를 참조하세요.
...
## <a name="what-are-reliable-services"></a>신뢰할 수 있는 서비스란?

Reliable Services는 단순하고 강력한 최고 수준의 프로그래밍 모델을 제공하여 애플리케이션에 중요한 내용을 나타낼 수 있도록 합니다. 신뢰할 수 있는 서비스 프로그래밍 모델은 다음을 제공합니다.

* 서비스 패브릭 API에 대한 액세스. [게스트 실행으로](service-fabric-guest-executables-introduction.md)모델링된 서비스 패브릭 서비스와 달리 신뢰할 수 있는 서비스는 서비스 패브릭 API를 직접 사용할 수 있습니다. 그러면 서비스에서 다음을 수행할 수 있습니다.
  * 시스템 쿼리
  * 클러스터의 엔터티에 대한 상태 보고
  * 구성 및 코드 변경에 대한 알림 수신
  * 다른 서비스 찾기 및 통신,
  * 신뢰할 [수 있는 컬렉션](service-fabric-reliable-services-reliable-collections.md) 사용
  * 여러 프로그래밍 언어로 된 일류 프로그래밍 모델의 다른 많은 기능에 액세스할 수 있습니다.
* 다른 익숙한 프로그래밍 모델처럼 느껴지는 사용자 고유의 코드를 실행하기 위한 간단한 모델입니다. 사용자의 코드에는 잘 정의된 진입점과 쉽게 관리되는 수명 주기가 있습니다.
...
## <a name="what-makes-reliable-services-different"></a>신뢰할 수 있는 서비스가 다른 이유

서비스 패브릭은 다음과 같은 서비스를 제공하기 때문에 이전에 작성한 서비스와 는 다릅니다.
...
* **확장성** – 서비스가 특정 하드웨어에서 분리되고, 하드웨어 또는 다른 리소스의 추가 또는 제거를 통해 필요에 따라 확장 또는 축소될 수 있습니다. 서비스가 부분 오류의 크기를 조정하고 다룰 수 있도록 손쉽게 분할됩니다(특히 상태 저장 상태에서). 코드를 통해 동적으로 서비스를 만들고 삭제할 수 있으므로 고객 요청에 대한 응답으로 필요에 따라 더 많은 인스턴스를 스펀징할 수 있습니다. 마지막으로 Service Fabric은 간단한 서비스를 권장합니다. Service Fabric을 사용하면 전체 OS 인스턴스 또는 프로세스를 서비스의 단일 인스턴스로 요구하거나 지정하기보다 단일 프로세스 내에 수천 개의 서비스가 프로비저닝될 수 있습니다.
* **일관성** - 신뢰할 수 있는 서비스에 저장된 모든 정보는 일관성을 보장할 수 있습니다. 이는 서비스 내의 여러 신뢰할 수 있는 컬렉션에서도 마찬가지입니다. 서비스 내의 컬렉션을 트랜잭션 원자 방식으로 변경할 수 있습니다.

## <a name="service-lifecycle"></a>서비스 수명 주기

서비스가 상태 저장 서비스이든, 상태 비저장 서비스이든, 신뢰할 수 있는 서비스는 신속하게 코드를 연결하고 시작할 수 있는 간단한 수명 주기를 제공합니다.  새 서비스를 시작하고 실행하려면 다음 두 가지 방법을 구현해야 합니다.
...
처음으로 Reliable Services를 알아보는 경우 계속 읽어주세요. 신뢰할 수 있는 서비스의 수명 주기에 대한 자세한 연습을 찾고 있다면 [신뢰할 수 있는 서비스 수명 주기 개요를](service-fabric-reliable-services-lifecycle.md)확인하십시오.

## <a name="example-services"></a>예제 서비스

신뢰할 수 있는 서비스 모델이 상태 비수기 및 상태 비우지 서비스 모두에서 어떻게 작동하는지 자세히 살펴보겠습니다.

### <a name="stateless-reliable-services"></a>상태 비저장 신뢰할 수 있는 서비스

*상태 비수기 서비스는* 호출 간에 서비스 내에서 유지 관리되는 상태가 없는 서비스입니다. 표시된 상태는 완전히 삭제 가능하며 동기화, 복제, 지속성 또는 고가용성이 필요하지 않습니다.
...
Service Fabric에서 상태 비저장 서비스가 사용되는 방법의 일반적인 예로 웹 애플리케이션에 대한 공용 API를 노출하는 프런트 엔드가 있습니다. 프런트 엔드 서비스는 상태 저장 서비스에 사용자의 요청을 완료하라고 말합니다. 이 경우 상태 비저장 서비스가 수신 대기 중인 알려진 포트(예: 80)에 클라이언트의 호출이 전송됩니다. 이 상태 비저장 서비스는 호출을 받고 해당 호출이 신뢰할 수 있는 대상에서 왔는지 및 어떤 서비스를 대상으로 하는지 확인합니다.  그런 다음 상태 비저장 서비스는 상태 저장 서비스의 올바른 파티션으로 호출을 전달하고 응답을 대기합니다. 상태 비저장 서비스가 응답을 받으면 원래 클라이언트에 응답합니다. 이러한 서비스의 예로는 해당 리포지토리의 다른 서비스 패브릭 샘플 중에서 *시작되는 서비스 패브릭* 샘플(C#Java)이[C#](https://github.com/Azure-Samples/service-fabric-dotnet-getting-started) / [Java](https://github.com/Azure-Samples/service-fabric-java-getting-started)있습니다.

### <a name="stateful-reliable-services"></a>상태 저장 신뢰할 수 있는 서비스

*상태 정리 서비스는* 서비스가 작동하려면 상태의 일부가 일관되고 존재해야 하는 서비스입니다. 수신하는 업데이트에 따라 일부 값의 이동 평균을 지속적으로 계산하는 서비스를 고려할 수 있습니다. 이 서비스를 수행하려면 처리해야 하는 수신 요청의 현재 집합과 현재 평균이 있어야 합니다. 정보를 검색, 처리하고 외부 저장소(예: Azure Blob 또는 현재 테이블 저장소)에 저장하는 모든 서비스는 상태 저장입니다. 외부 상태 저장소에 상태를 보관합니다.
...
이미지를 처리하는 서비스를 작성하려는 경우를 가정하겠습니다. 이를 위해 서비스는 이미지 및 일련의 변환을 사용하여 해당 이미지에서 수행합니다. 이 서비스는 `ConvertImage(Image i, IList<Conversion> conversions)`와 같은 API를 노출하는 통신 수신기(WebAPI라고 가정함)를 반환합니다. 요청을 받으면 서비스는 `IReliableQueue`을 저장하고 요청을 추적할 수 있도록 일부 ID를 클라이언트에 반환합니다.
...
이 서비스는 일반적인 .NET 서비스처럼 들리지만 사용 중인 데이터`IReliableQueue` 구조(및)는 `IReliableDictionary`Service Fabric에서 제공되며 매우 안정적이고 사용 가능하며 일관성이 있다는 점입니다.

## <a name="when-to-use-reliable-services-apis"></a>신뢰할 수 있는 서비스 API를 사용해야 하는 경우

다음과 같은 경우 신뢰할 수 있는 서비스 API를 고려하십시오.

* 서비스의 코드(및 선택사항 상태)를 가용성이 높고 안정적으로 유지하려고 합니다.
