...
1. 상태 저장 서비스의 경우 신뢰할 수 있는 서비스 프로그래밍 모델을 사용하면 C# 컬렉션을 사용해본 적이 있는 사람에게 친숙한 고가용성 컬렉션 클래스의 간단한 집합인 신뢰할 수 있는 컬렉션을 사용하여 서비스 내에 바로 상태를 일관되고 안정적으로 저장할 수 있습니다. 일반적으로 서비스는 신뢰할 수 있는 상태 관리를 위한 외부 시스템이 필요합니다. 신뢰할 수 있는 컬렉션을 사용하면 고가용성 외부 저장소에 기대하게 되는 것과 동일한 고가용성 및 안정성과 함께, 컴퓨터와 상태가 제공하는 공동 배치되는 추가 대기 시간 향상과 함께 대기 컴퓨터 옆에 상태를 저장할 수 있습니다.
...
+ <u>안정성</u> - 컴퓨터가 실패하거나 네트워크 문제가 발생한 안정적이지 않은 환경에서도 서비스가 제공됩니다.

+ <u>가용성</u> - 서비스가 연결 가능하고 응답 가능하게 됩니다(찾을 수 없거나 연결할 수 없는 서비스가 있을 수 있음).
...
+ CreateServiceReplicaListeners/CreateServiceInstanceListeners - 서비스에서 사용할 통신 스택을 정의합니다. [Web API](service-fabric-reliable-services-communication-webapi.md)와 같은 통신 스택은 서비스의 수신 대기 끝점(클라이언트가 어떻게 연결할지)을 정의하고, 표시되는 메시지가 서비스 코드의 나머지 부분과 상호 작용하는 방법을 정의합니다.
...
1. 서비스 개체(StatelessService 또는 StatefulService에서 파생되는 것)가 구성됩니다.

2. 서비스에서 선택한 하나 이상의 통신 수신기를 반환할 수 있도록 CreateServiceReplicaListeners/CreateServiceInstanceListeners 메서드가 호출됩니다.
  + 선택적이지만 대부분의 서비스는 일부 끝점을 직접 노출합니다.

3. 통신 수신기가 생성되면 열립니다.
  + 통신 수신기는 이때 호출되고 서비스의 수신 대기 주소를 반환하는 OpenAsync()라는 메서드가 있습니다. 신뢰할 수 있는 서비스가 ICommunicationListener에 내장된 것 중 하나를 사용할 경우 자동으로 처리됩니다.
...
서비스 종료되면(삭제되거나, 업그레이드되거나, 특정 위치에서 이동 하는 경우) 호출 순서는 동일합니다. 통신 수신기에서 첫 번째 CloseAsync()가 호출되고, RunAsync()에 전달된 취소 토큰이 취소됩니다.
...
이미지에서 수행되어야 하는 일련의 변환과 변환해야 하는 이미지에 대한 요청을 받은 서비스를 작성하려는 경우를 가정해 보겠습니다. 이 서비스의 경우 통신 포트를 열고 `ConvertImage(Image i, IList<Conversion> conversions)` 같은 API를 통해 제출을 허용하는 통신 수신기(WebAPI로 가정)를 반환합니다. 이 API에서 서비스는 정보를 받고 ReliableQueue에 요청을 저장한 후 요청을 추적할 수 있도록 클라이언트에 일부 토큰을 반환합니다(요청에 오랜 시간이 걸릴 수 있으므로).
...
이 서비스에 대해 알아두어야 할 한 가지는 일반 .NET 서비스처럼 보이지만 유일한 차이점은 사용 중인 데이터 구조(IReliableQueue 및 IReliableDictionary)를 서비스 패브릭에서 제공하므로 안정성, 가용성 및 일관성이 매우 높다는 점입니다.
