<properties
   pageTitle="서비스 패브릭에서 상태를 보고하고 확인하는 방법 | Microsoft Azure"
   description="이 문서는 Azure 서비스 패브릭에서 제공하는 상태 모니터링 도구를 사용하여 서비스 코드에서 상태 보고서를 보내고 서비스의 상태를 확인할 수 있는 방법에 대해 설명합니다."
...
# 서비스 상태 보고 및 확인
서비스에 문제가 발생할 때 결과 인시던트 및 중단에 응답하고 수정하는 능력은 문제를 빠르게 검색할 수 있는 능력과 밀접한 관련이 있습니다. 서비스 코드에서 서비스 패브릭 상태 관리자로 문제 및 오류를 보고하면 서비스 패브릭이 제공하는 표준 상태 모니터링 도구를 사용하여 상태를 확인할 수 있습니다. 이 문서에서는 상태 보고서를 서비스에 추가하는 예제를 안내하고 서비스 패브릭이 제공하는 도구를 사용하여 상태를 확인할 수 있는 방법을 보여 줍니다. 이 문서의 목적은 서비스 패브릭의 상태 모니터링 기능을 빠르게 소개하는 것이며 자세한 내용은 이 문서의 끝에 링크로 시작하는 상태에 대한 자세한 문서 시리즈를 읽어 보면 됩니다.

## 필수 조건
* Visual Studio 2015 * 서비스 패브릭 SDK가 설치되어 있어야 합니다.

## 응용 프로그램 배포 및 해당 상태 확인
...
4. 응용 프로그램이 실행 중인 경우 로컬 클러스터 관리자 시스템 트레이 앱을 마우스 오른쪽 단추로 클릭하고 상황에 맞는 메뉴에서 로컬 클러스터 관리를 선택하여 서비스 패브릭 탐색기를 시작합니다. ![시스템 트레이에서 서비스 패브릭 탐색기 시작]()
...
6. Powershell을 사용하여 상태를 확인할 수도 있습니다. ```Get-ServiceFabricApplicationHealth```를 사용하여 응용 프로그램의 상태를 확인하고 ```Get-ServiceFabricServiceHealth```에서 서비스 상태를 쿼리할 수 있습니다. Powershell에서 동일한 응용 프로그램에 대한 상태 보고서는 다음과 같습니다. ![Powershell에서 정상적인 응용 프로그램](./media/service-fabric-diagnostics-how-to-report-and-check-service-health/ps-healthy-app-report.png)

## 서비스 코드에 사용자 지정 상태 이벤트 추가
...
1. 위에서 만든 응용 프로그램을 Visual Studio에서 다시 열거나 VS 템플릿으로 상태 저장 서비스를 사용하는 새 응용 프로그램을 만듭니다.
...
5. 오류를 나타내는 결과 부족에 대한 상태 이벤트를 보고하려면 `myDictionary.TryGetValueAsync` 호출 이후 아래 코드를 추가합니다. 이 이벤트는 상태 저장 서비스에서 보고되므로 해당 이벤트를 `StatefulServiceReplicaHealthReport`로 보고합니다. 배포된 상태 저장 서비스는 여러 개의 파티션이 있고 각 파티션에는 여러 개의 복제본이 있을 수 있으므로 상태 모니터링 도구 중 하나에서 보고서 이벤트가 표시되면 이 보고서 이벤트에 전달된 PartitionId 및 ReplicaId를 통해 이 보고서의 원본을 식별할 수 있습니다. `HealthInformation` 매개 변수가 보고된 상태 문제에 대한 정보를 저장합니다. 이 네임스페이스를 **Stateful1.cs** 파일에 추가합니다.
...
이 보고서는 다른 보고서로 대체되거나 이 복제본이 삭제될 때까지 상태 관리자에 그대로 남아 있습니다(*HealthInformation 개체에서 이 상태 보고서에 대해 TimeToLive를 설정하지 않았으므로 만료되지 않음*). 상태를 쿼리하고 보고하려면 FabricClient가 관리자 권한이 있는 프로세스에 있어야 합니다.
