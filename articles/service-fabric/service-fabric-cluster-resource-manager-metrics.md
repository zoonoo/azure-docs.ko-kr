...
기본 메트릭은 처음부터 잘 작동하지만 지금까지만 수행됩니다. 예:  모든 파티션에서 완벽 하 게 고른 사용률에서 결과 선택 하는 파티션 구성표가 가능성 이란? 시간이 지남에 따라 지정된 서비스에 대한 로드가 일정하거나 심지어 여러 파티션에서 지금 동일하게 나타날 가능성은 얼마나 될까요?
