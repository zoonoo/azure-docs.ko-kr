...
ms.locfileid: "106279785"
---
# <a name="list-of-service-fabric-events"></a>Service Fabric 이벤트 목록 

Service Fabric은 [Service Fabric 이벤트](service-fabric-diagnostics-events.md)로 클러스터의 상태를 알리도록 클러스터 이벤트의 기본 집합을 노출합니다. 노드 및 클러스터의 Service Fabric에서 수행하는 작업 또는 클러스터 소유자/연산자에서 만드는 관리 결정을 기반으로 합니다. 이러한 이벤트는 [클러스터를 사용한 Azure Monitor 로그](service-fabric-diagnostics-oms-setup.md)를 구성하거나 [EventStore](service-fabric-diagnostics-eventstore.md)를 쿼리하는 등 여러 가지 방법으로 구성하여 액세스 할 수 있습니다. Windows 머신에서 이러한 이벤트는 EventLog에 반영되므로 이벤트 뷰어에서 Service Fabric 이벤트를 볼 수 있습니다. 

이러한 이벤트에 대한 몇 가지 특징은 다음과 같습니다.
* 각 이벤트는 클러스터의 특정 엔터티(예: 애플리케이션, 서비스, 노드, 복제본)에 연결됩니다.
* 각 이벤트에는 EventInstanceId, EventName 및 Category의 공통 필드 세트가 포함되어 있습니다.
...
| 29629 | ClusterUpgradeRollbackStarted | 업그레이드 | 클러스터 업그레이드 롤백이 시작되었습니다.  | CM | 경고 | 
| 29630 | ClusterUpgradeRollbackCompleted | 업그레이드 | 클러스터 업그레이드 롤백이 완료되었습니다. | CM | 경고 | 
| 29631 | ClusterUpgradeDomainCompleted | 업그레이드 | 업그레이드 도메인이 클러스터 업그레이드 중 업그레이드를 완료했습니다. | CM | 정보 제공 | 

**배치 이벤트**
| EventId | 속성 | Category | 설명 |원본(태스크) | Level |
| --- | --- | ---| --- | --- | --- |
| 17616 | 의사 결정 |StateTransition | 새 복제본의 배치를 결정하도록 배치 작업이 예약되었습니다. | CRM | 정보 제공 |
...
| EventId | 속성 | Category | 설명 |원본(태스크) | Level |
| --- | --- | --- | --- | --- | --- | 
| 50187 | SharedLogWriteThrottled | 의료 | 공유 로그에 대한 쓰기가 제한됩니다. | KtlLoggerNode | 정보 제공 | 
| 50188 | SharedLogWriteUnthrottled | 의료 | 공유 로그에 대한 쓰기가 제한되지 않았습니다. | KtlLoggerNode | 정보 제공 | 
...
| 29623 | ApplicationUpgradeRollbackStarted | 업그레이드 | 애플리케이션 업그레이드 롤백이 시작되었습니다. |CM | 경고 | 
| 29624 | ApplicationUpgradeRollbackCompleted | 업그레이드 | 애플리케이션 롤백이 완료되었습니다. | CM | 경고 | 
...
| 61703 | ReliableDictionaryCheckpointRecovered | LifeCycle | 신뢰할 수 있는 사전이 해당 검사점을 복구했습니다. | DistributedDictionary | 정보 제공 |
...
| 61965 | ReliableQueueCheckpointRecovered | LifeCycle | 신뢰할 수 있는 큐가 해당 검사점을 복구했습니다. | DistributedQueue | 정보 제공 |
...
| 63650 | ReliableConcurrentQueueCheckpointRecovered | LifeCycle | 신뢰할 수 있는 동시 큐가 해당 검사점을 복구했습니다. | ReliableConcurrentQueue | 정보 제공 |
| 61687 | TStoreError | 실패 | 신뢰할 수 있는 컬렉션에서 예기치 않은 오류가 발생했습니다. | TStore | 오류 |
| 63831 | PrimaryFullCopyInitiated | LifeCycle | 주 복제본에서 전체 복사를 시작했습니다. | TReplicator | 정보 제공 |
| 63832 | PrimaryPartialCopyInitiated | LifeCycle | 주 복제본에서 부분 복사를 시작했습니다. | TReplicator | 정보 제공 |
| 16831 | BuildIdleReplicaStarted | LifeCycle | 주 복제본에서 유휴 복제본 빌드를 시작했습니다. | 복제 | 정보 제공 |
| 16832 | BuildIdleReplicaCompleted | LifeCycle | 주 복제본에서 유휴 복제본 빌드를 완료했습니다. | 복제 | 정보 제공 |
| 16833 | BuildIdleReplicaFailed | LifeCycle | 주 복제본에서 유휴 복제본을 빌드하지 못했습니다. | 복제 | 경고 |
| 16834 | PrimaryReplicationQueueFull | 의료 | 주 복제본의 복제 큐가 가득 찼습니다. | 복제 | 경고 |
| 16835 | PrimaryReplicationQueueWarning | 의료 | 주 복제본의 복제 큐가 거의 가득 찼습니다. | 복제 | 경고 |
| 16836 | PrimaryReplicationQueueWarningMitigated | 의료 | 주 복제본의 복제 큐가 정상입니다. | 복제 | 정보 제공 |
| 16837 | SecondaryReplicationQueueFull | 의료 | 보조 복제본의 복제 큐가 가득 찼습니다. | 복제 | 경고 |
| 16838 | SecondaryReplicationQueueWarning | 의료 | 보조 복제본의 복제 큐가 거의 가득 찼습니다. | 복제 | 경고 |
| 16839 | SecondaryReplicationQueueWarningMitigated | 의료 | 보조 복제본의 복제 큐가 정상입니다. | 복제 | 정보 제공 |
| 16840 | PrimaryFaultedSlowSecondary | 의료 | 주 복제본에서 느린 보조 복제본에 오류가 발생했습니다. | 복제 | 경고 |
| 16841 | ReplicatorFaulted | 의료 | 복제본에 오류가 발생했습니다. | 복제 | 경고 |
