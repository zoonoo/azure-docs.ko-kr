...
1. Active Directory 도메인 관리자에 게 `New-ADServiceAccount` cmdlet을 사용 하 여 그룹 관리 서비스 계정을 만들고 `PrincipalsAllowedToRetrieveManagedPassword`에 모든 Service Fabric 클러스터 노드가 포함 되어 있는지 확인 합니다. `AccountName`, `DnsHostName` 및 `ServicePrincipalName`은 고유해야 합니다.
...
3. 사용자 보안 주체를 구성 하 고 [사용자](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-fabric-settings#runas)를 참조 하도록 `RunAsPolicy`를 구성 합니다.
