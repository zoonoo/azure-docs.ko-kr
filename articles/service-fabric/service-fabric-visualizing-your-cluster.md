...
| 노드 | 활성화 | 노드 활성화 |
|| 비활성화(일시 중지) | 현재 상태에서 노드를 일시 중지합니다. 서비스는 계속 실행하지만 가동 중단 또는 데이터 불일치를 방지하는 데 필요한 경우가 아니면 서비스 패브릭은 아무 것도 사전에 이동하지 않습니다. 이 작업은 검사하는 동안 이동하지 않도록 특정 노드에 디버깅 서비스를 사용하도록 설정하는 데 일반적으로 사용됩니다. |
