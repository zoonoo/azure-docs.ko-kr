...
<properties
   pageTitle="사물 인터넷을 위한 Azure 서비스 패브릭 행위자"
   description="Azure 서비스 패브릭 행위자는 HTTPS, MQTT 또는 AMQP 등의 다중 전송을 지원하는 메시징 시스템 프런트 엔드를 결합한 후 개별 장치를 나타내는 행위자와 통신하는 시스템에서 사용하는 주요 구성 요소(중간 계층)입니다."
