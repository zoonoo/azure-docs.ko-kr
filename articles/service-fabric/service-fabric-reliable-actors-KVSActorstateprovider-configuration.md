<properties
   pageTitle="Azure 서비스 패브릭 Reliable Actors KVSActorStateProvider 구성의 개요 | Microsoft Azure"
...
## 복제자 구성
복제자 구성은 행위자 상태 제공자의 상태를 매우 안정적으로 만드는 작업을 담당하는 복제자를 구성합니다. 기본 구성은 Visual Studio 템플릿에 의해 생성되며 충분해야 합니다. 이 섹션에서는 복제자 조정에 사용할 수 있는 추가 구성에 대해 설명합니다.
...
## 저장소 구성
저장소 구성은 복제 중인 상태를 유지하는 데 사용되는 로컬 저장소를 구성하는 데 사용됩니다. 기본 구성은 Visual Studio 템플릿에 의해 생성되며 충분해야 합니다. 이 섹션에서는 로컬 저장소 조정에 사용할 수 있는 추가 구성에 대해 설명합니다.
...
|이름|단위|기본값|설명|
|----|----|-------------|-------|
|MaxAsyncCommitDelay|밀리초|200|지속형 로컬 저장소 커밋에 대한 최대 배치 간격을 설정합니다.|
|MaxVerPages|페이지 수|8192|로컬 저장소 데이터베이스의 최대 버전 페이지 수. 처리되지 않은 트랜잭션의 최대 수를 결정합니다.|
