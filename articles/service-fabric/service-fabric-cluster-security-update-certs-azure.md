---
title: Azure 서비스 패브릭 클러스터에서 인증서 관리
...
Azure Service Fabrics SDK의 기본 인증서 로드 동작은 나중에 만료 날짜가 있는 정의된 인증서를 배포하고 사용하는 것입니다. 기본 또는 보조 구성 정의에 관계없이 클래식 동작으로 돌아가는 것은 권장되지 않는 고급 작업이며 구성 내에서 "UseSecondaryIfNewer" `Fabric.Code` 설정 매개 변수 값을 false로 설정해야 합니다.
...
보안 클러스터의 경우 하나 이상의 유효한(해지되지 않고 만기되지 않은) 인증서가 항상 필요합니다. 향후 만료 날짜에 가장 먼 상태로 배포된 인증서가 사용 중이며 이를 제거하면 클러스터작동이 중지됩니다. 만료된 인증서 또는 가장 빨리 만료되는 사용되지 않는 인증서만 제거해야 합니다.
...
> [!TIP]
> 이제 [Add-AzServiceFabricClusterCertificate](/powershell/module/az.servicefabric/add-azservicefabricclustercertificate) cmdlet을 사용하여 보조 인증서를 추가하는 더 쉽고 쉬운 방법이 있습니다. 이 섹션의 나머지 단계를 따를 필요가 없습니다.  또한 [Add-AzServiceFabricClusterCertificate](/powershell/module/az.servicefabric/add-azservicefabricclustercertificate) cmdlet을 사용할 때 클러스터를 만들고 배포하는 데 원래 사용된 템플릿이 필요하지 않습니다.
...
> [!NOTE]
> 따라 또는 시작 점으로 사용할 수 있는 샘플 템플릿 및 매개 변수를 찾고 있는 경우 이 [git-repo에서](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/Cert-Rollover-Sample)다운로드합니다. 
> 
> 

### <a name="edit-your-resource-manager-template"></a>Resource Manager 템플릿 편집

편의를 위해 5-VM-1-NodeTypes-Secure_Step2.JSON 샘플에는 앞으로 수행할 모든 편집이 포함되어 있습니다. 샘플은 [git-repo에서](https://github.com/Azure-Samples/service-fabric-cluster-templates/tree/master/Cert-Rollover-Sample)사용할 수 있습니다.
...
4. **모든** **Microsoft.Compute/virtualMachineScaleSet** 리소스 정의 변경 - Microsoft.Compute/virtualMachineScaleSet 리소스 정의를 찾습니다. "virtualMachineProfile" 아래 "publisher": "Microsoft.Azure.ServiceFabric"으로 스크롤합니다.
...
리소스 그룹에 템플릿을 배포합니다. 현재 클러스터가 배포된 동일한 리소스 그룹을 사용합니다. 새 AzResourceGroup 배포 명령을 실행합니다. 기본값은 **incremental**이므로 모드는 지정할 필요가 없습니다.
...
## <a name="deploying-client-certificates-to-the-cluster"></a>클러스터에 클라이언트 인증서를 배포합니다.
...
## <a name="adding-application-certificates-to-a-virtual-machine-scale-set"></a>가상 시스템 규모 집합에 응용 프로그램 인증서 추가

응용 프로그램에 사용하는 인증서를 클러스터에 배포하려면 [이 샘플 Powershell 스크립트를](scripts/service-fabric-powershell-add-application-certificate.md)참조하십시오.
