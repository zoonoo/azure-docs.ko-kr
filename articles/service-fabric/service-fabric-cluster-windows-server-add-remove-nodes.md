...
6. 추가할 새 노드를 설명하는 매개 변수를 사용하여 *AddNode.ps1* 스크립트를 실행합니다. 다음 예에서는 NodeType0 및 IP 주소 182.17.34.52 이라는 VM5 라는 새 노드를 UD1 및 fd:/dc1/r 0에 추가 합니다. `ExistingClusterConnectionEndPoint` 는 기존 클러스터에 이미 있는 노드에 대 한 연결 끝점으로, 클러스터에 *있는 노드의 IP* 주소일 수 있습니다. 
   ...
   스크립트 실행이 완료 되 면 [get-servicefabricnode](/powershell/module/servicefabric/get-servicefabricnode) cmdlet을 실행 하 여 새 노드가 추가 되었는지 여부를 확인할 수 있습니다.

7. 클러스터의 서로 다른 노드 간에 일관성을 보장하려면 구성 업그레이드를 시작해야 합니다. [Get-servicefabricclusterconfiguration](/powershell/module/servicefabric/get-servicefabricclusterconfiguration) 를 실행 하 여 최신 구성 파일을 가져오고 새로 추가 된 노드를 "Nodes" 섹션에 추가 합니다. 동일한 구성이 있는 클러스터를 다시 배포 해야 하는 경우 항상 최신 클러스터 구성을 사용 하는 것이 좋습니다.
...
8. [Start-ServiceFabricClusterConfigurationUpgrade](/powershell/module/servicefabric/start-servicefabricclusterconfigurationupgrade)를 실행하여 업그레이드를 시작합니다.
   ...
   Service Fabric Explorer에서 업그레이드의 진행률을 모니터링할 수 있습니다. 또는 [start-servicefabricclusterupgrade](/powershell/module/servicefabric/get-servicefabricclusterupgrade)을 실행할 수 있습니다.
