...
1. 클러스터에 대해 선택한 안정성 수준에 따라 주 노드 유형의 처음 n(3/5/7/9) 노드는 제거할 수 없습니다.
2. 개발 클러스터에서 RemoveNode 명령의 실행은 지원되지 않습니다.
2. 클러스터에서 제거하려는 VM/컴퓨터로 RDP(원격 데스크톱).
...
4. *RemoveNode.ps1* Powershell을 실행합니다. 아래 예제에서는 클러스터에서 현재 노드를 제거합니다. *ExistingClusterConnectionEndPoint*는 기존 클러스터에 이미 있는 노드에 대한 연결 끝점입니다. 이 끝점의 경우 클러스터에서 *모든* **다른 노드**의 IP 주소를 선택해야 합니다.
