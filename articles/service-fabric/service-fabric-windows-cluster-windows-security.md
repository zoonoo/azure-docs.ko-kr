...
[Microsoft.Azure.ServiceFabric.WindowsServer<version>.zip](http://go.microsoft.com/fwlink/?LinkId=730690) 독립 실행형 클러스터 패키지와 함께 다운로드된 *ClusterConfig.Windows.JSON* 샘플 구성 파일은 Windows 보안 구성을 위한 템플릿을 포함합니다. **Properties** 섹션에서 다음과 같이 Windows 보안을 구성합니다.
...
| **구성 설정** | **설명** |
| --- | --- |
| ClusterCredentialType |Windows 보안을 사용하도록 설정하려면 *Windows*로 설정합니다. |
...
[노드 간 보안](service-fabric-cluster-security.md#node-to-node-security)은 **ClusterIdentity**를 사용하여 구성됩니다. 노드 간의 신뢰 관계를 구축하기 위해 노드가 서로를 인식해야 합니다. 인식을 구축하려면 클러스터의 모든 노드를 포함하는 도메인 그룹을 만듭니다. 이 그룹 이름은 **ClusterIdentity**로 지정해야 합니다. 자세한 내용은 [Active Directory에 그룹 만들기](https://msdn.microsoft.com/library/aa545347(v=cs.70).aspx)를 참조하세요.  
