...
[Microsoft.Azure.ServiceFabric.WindowsServer.<version>.zip](http://go.microsoft.com/fwlink/?LinkId=730690) 독립 실행형 클러스터 패키지와 함께 다운로드된 샘플 *ClusterConfig.Windows.MultiMachine.JSON* 구성 파일은 Windows 보안 구성을 위한 템플릿을 포함합니다.  **Properties** 섹션에서 다음과 같이 Windows 보안을 구성합니다.
...
Active Directory 도메인 내에서 컴퓨터 그룹을 사용하려는 경우 [노드 간 보안](service-fabric-cluster-security.md#node-to-node-security)은 **ClusterIdentity** 사용을 설정하여 구성됩니다. 자세한 내용은 [Active Directory에 컴퓨터 그룹 만들기](https://msdn.microsoft.com/en-us/library/aa545347(v=cs.70).aspx)를 참조하세요.
...
> [!NOTE]
> 도메인 컨트롤러에 Service Fabric을 배포해서는 안 됩니다. 컴퓨터 그룹 또는 gMSA를 사용하는 경우 ClusterConfig.json에 도메인 컨트롤러의 IP가 포함되지 않았는지 확인합니다.
> 
> 
