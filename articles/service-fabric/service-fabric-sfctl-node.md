...
| --deactivation-intent | 노드를 비활성화하는 의도나 이유를 설명합니다. 가능한 값은 다음과 같습니다. - Pause - 노드가 일시 중지해야 함을 나타냅니다. 값은 1입니다. - Restart - 노드가 잠시 후에 다시 시작되게 하려는 것이 의도임을 나타냅니다. 값은 2입니다. -        RemoveData - 노드에서 데이터를 제거하는 것이 의도임을 나타냅니다. 값은 3입니다. .|
...
| --node-name                [필수]| 노드의 이름입니다.|
| --node-transition-type     [필수]| 수행할 전환의 유형을 나타냅니다.                       NodeTransitionType.Start는 중지된 노드를 시작합니다.                       NodeTransitionType.Stop은 작동하는 노드를 중지합니다. -                       Invalid - 예약됨.  API에 전달하지 마세요. - Start -                       중지된 노드를 작동하도록 전환. - Stop -                        작동 중인 노드를 중지하도록 전환. .|
