---
title: Azure 서비스 패브릭 CLI-sfctl 노드
description: Azure 서비스 패브릭 명령줄 인터페이스인 sfctl에 대해 알아봅니다. 클러스터 노드 관리를 위한 명령 목록을 포함합니다.
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
|명령|설명|
| --- | --- |
| 추가 구성 매개 변수 재정의 | 지정된 노드에 구성 재정의 목록을 추가합니다. |
...
| get-configuration-재정의 | 지정된 노드에서 구성 재정의 목록을 가져옵니다. |
...
| 제거 구성 재정의 | 지정된 노드에서 구성 재정의를 제거합니다. |
...
지정된 노드에 구성 재정의 목록을 추가합니다.

이 API를 사용하면 지정된 노드에 모든 기존 구성 재정의를 추가할 수 있습니다.

### <a name="arguments"></a>인수

|인수|설명|
| --- | --- |
| --구성 매개 변수 재정의 목록 [필수] | 구성 재정의 목록을 추가하기 위한 설명입니다. |
| --노드 이름 [필수] | 노드의 이름입니다. |
| --force | 지정된 노드에서 구성 재정의를 강제로 추가합니다. |
...
## <a name="sfctl-node-get-configuration-overrides"></a>sfctl 노드 get-configuration-재정의
지정된 노드에서 구성 재정의 목록을 가져옵니다.

이 API를 사용하면 지정된 노드에서 모든 기존 구성 재정의를 받을 수 있습니다.
...
## <a name="sfctl-node-remove-configuration-overrides"></a>sfctl 노드 제거 구성 재정의
지정된 노드에서 구성 재정의를 제거합니다.
...
해당 노드의 지속된 상태를 복구할 수 없다는 것을 의미합니다. 일반적으로 하드 디스크가 초기화 정리되거나 하드 디스크의 작동이 중단하는 경우에 발생합니다. 이 작업을 성공시키려면 노드는 중단되어야 합니다. 이 작업을 사용하면 Service Fabric에서 해당 노드의 복제본이 더 이상 존재하지 않으며, Service Fabric에서 해당 복제본이 다시 작동하기를 기다리는 것을 중지해야 함을 알 수 있습니다. 노드의 상태가 제거되지 않고 노드가 해당 상태 그대로 다시 작동할 수 있는 경우 이 cmdlet을 실행하지 마십시오. 서비스 패브릭 6.5에서 시작하여 시드 노드에 이 API를 사용하려면 시드 노드를 일반(비시드) 노드로 변경한 다음 이 API를 호출하여 노드 상태를 제거하십시오. 클러스터가 Azure에서 실행 중인 경우 시드 노드가 다운된 후 Service Fabric은 자동으로 시드 노드가 아닌 노드로 변경하려고 시도합니다. 이렇게 하려면 기본 노드 유형의 비시드 노드 수가 Down 시드 노드 수보다 적은 수인지 확인합니다. 필요한 경우 이를 위해 기본 노드 유형에 노드를 더 추가합니다. 독립 실행형 클러스터의 경우 Down 시드 노드가 그대로 상태로 돌아올 것으로 예상되지 않는 경우 클러스터에서 노드를 제거하고 https\://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes 참조하십시오.
