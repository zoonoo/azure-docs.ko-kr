---
title: Azure 서비스 패브릭에 대한 네트워킹 패턴
...
### <a name="allowing-the-service-fabric-resource-provider-to-query-your-cluster"></a>서비스 패브릭 리소스 공급자가 클러스터를 쿼리할 수 있도록 허용
...
배포 후에는 부하 분산 장치가 다른 리소스 그룹의 공용 고정 IP 주소에 바인딩된 것을 볼 수 있습니다. 서비스 패브릭 클라이언트 연결 끝점 및 [서비스 패브릭 탐색기](service-fabric-visualizing-your-cluster.md) 끝점은 정적 IP 주소의 DNS FQDN을 가리킵니다.

<a id="internallb"></a>
## <a name="internal-only-load-balancer"></a>내부 전용 부하 분산 장치

이 시나리오에서는 기본 Service Fabric 템플릿의 외부 부하 분산 장치를 내부 전용 부하 분산 장치로 바꿉니다. Azure 포털 및 Service Fabric 리소스 공급자에 대한 의미는 [문서의 위축된 문서를](#allowing-the-service-fabric-resource-provider-to-query-your-cluster) 참조하십시오.
...
이 시나리오는 기존 단일 노드 형식 외부 부하 분산 장치로 시작하고 동일한 노드 형식에 대한 내부 부하 분산 장치를 추가합니다. 백 엔드 주소 풀에 연결된 백 엔드 포트는 단일 부하 분산 장치에만 할당할 수 있습니다. 애플리케이션 포트를 포함할 부하 분산 장치 및 관리 엔드포인트를 포함할 부하 분산 장치를 선택합니다(포트 19000 및 19080). 관리 끝점을 내부 로드 밸로드에 넣는 경우 [이 문서의 앞부분에서](#allowing-the-service-fabric-resource-provider-to-query-your-cluster)설명한 Service Fabric 리소스 공급자 제한 사항에 유의하십시오. 사용하는 예제에서 관리 엔드포인트는 외부 부하 분산 장치에 유지됩니다. 또한 포트 80을 애플리케이션 포트에 추가한 후 내부 부하 분산 장치에 배치합니다.
...
## <a name="notes-for-production-workloads"></a>프로덕션 워크로드에 대한 참고 사항

위의 GitHub 템플릿은 SLB(기본 SLB)에 대한 기본 SKU와 함께 작동하도록 설계되었습니다. 이 SLB에는 SLA가 없으므로 프로덕션 워크로드의 경우 표준 SKU를 사용해야 합니다. 자세한 내용은 [Azure 표준 로드 밸해시 개요를](/azure/load-balancer/load-balancer-standard-overview)참조하십시오. SLB에 대한 표준 SKU를 사용하는 모든 서비스 패브릭 클러스터는 각 노드 유형에 포트 443에서 아웃바운드 트래픽을 허용하는 규칙이 있는지 확인해야 합니다. 클러스터 설정을 완료하는 데 필요하며 이러한 규칙이 없는 배포는 실패합니다. 위의 "내부 전용" 로드 밸런서의 예에서는 포트 443에 대한 아웃바운드 트래픽을 허용하는 규칙과 함께 추가 외부 로드 밸런서를 템플릿에 추가해야 합니다.
