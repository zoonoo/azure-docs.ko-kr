...
* 서비스 A – 배치 제약 조건 "NodeType = = 프런트 엔드"가 있으며 10의 부하를 보고 합니다.
* 서비스 B – 배치 제약 조건 "NodeType = = 프런트 엔드"가 있으며, 10 개의 로드를 보고 합니다.
* 서비스 C – 배치 제약 조건 "NodeType = = 백엔드" 이며 100의 부하를 보고 합니다.
* 서비스 D – 배치 제약 조건 "NodeType = = 백엔드"가 있으며 100 부하를 보고 합니다.
* 4 개의 노드가 있습니다. 둘 중 두 개는 NodeType을 "프런트 엔드"로 설정 하 고 다른 두 항목은 "백 엔드"로 설정 됩니다.

그리고 다음과 같은 배치가 있습니다.

<center>

![Subclustered 배치 예][Image1]
</center>
...
* 서비스 A: 배치 제약 조건 없음
* Service B: 배치 제약 조건 "NodeType = = 프런트 엔드"
* 서비스 C: 배치 제약 조건 "NodeType = = 백엔드"
...
리소스 관리자은 이러한 상황을 인식 하 고, 서비스 A를 프런트 엔드 노드에 배치할 수 있는 Service A1, 백엔드 노드에 배치할 수 있는 서비스 A 2 서비스로 분할 하는 상태 보고서를 생성 합니다. 이렇게 하면 최적으로 균형을 조정할 수 있는 첫 번째 범주 상황으로 다시 전환 됩니다.
