<properties
   pageTitle="응용 프로그램에 대한 프런트엔드 만들기 | Microsoft Azure"
   description="ServiceProxy를 통해 ASP.NET 5 Web API 프로젝트 및 서비스 간 통신을 사용하여 서비스 패브릭 응용 프로그램을 웹에 노출할 수 있습니다."
...
지금까지 수행한 작업의 결과를 살펴보기 위해 새 응용 프로그램을 배포하고 ASP.NET 5 Web API 템플릿에서 제공하는 기본 동작을 살펴보겠습니다.

1. Visual Studio에서 F5를 눌러 응용 프로그램을 디버깅합니다.

2. 배포가 완료되면 Visual Studio에서 http://localhost:33003처럼(포트 번호는 임의로 할당되며 컴퓨터마다 다를 수 있음) ASP.NET Web API 서비스의 루트에 브라우저를 실행합니다. ASP.NET 5 Web API 템플릿은 루트에 대한 기본 동작을 제공하지 않으므로 브라우저에서 오류가 발생할 것입니다.
...
### ServiceProxy를 사용하여 서버와 상호 작용

이제 상태 저장 서비스가 다른 서비스의 트래픽을 받을 준비가 되었으며, 그 외 서비스에는 ASP.NET 웹 서비스에서 상태 저장 서비스와 통신하기 위한 코드가 추가됩니다.
