...
기존 클러스터에 새 노드 형식 (가상 머신 확장 집합에 의해 지원 됨)을 추가 하는 것은 동일한 해당 노드의 nodetyperef를 사용 하지 않는 경우를 제외 하 고는 [주 노드 유형을 업그레이드](service-fabric-scale-up-node-type.md)하는 것과 비슷합니다. 실제로 사용 되는 가상 머신 확장 집합을 사용 하지 않도록 설정 하는 것은 아니며, 주 노드 유형을 업데이트 하지 않으면 클러스터 가용성이 손실 되지 않습니다. 
