...
기존 클러스터에 새 노드 형식(가상 머신 확장 집합에서 지원하는 노드 형식)을 추가하는 것은 [주 노드 형식을 업그레이드](service-fabric-scale-up-primary-node-type.md)하는 것과 유사하지만 동일한 NodeTypeRef를 사용하지 않으며, 현재 사용되는 가상 머신 확장 집합을 사용하지 않도록 설정하지 않고, 주 노드 형식을 업데이트하지 않으면 클러스터 가용성도 상실하지 않습니다. 
