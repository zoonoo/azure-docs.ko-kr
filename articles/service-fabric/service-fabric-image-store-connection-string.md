...
일부 문서에서 의미를 설명하지 않고 "ImageStoreConnectionString"라는 매개 변수가 있는지 간단하게 언급합니다. [PowerShell을 사용 하 여 응용 프로그램 배포 및 제거][10]와 같은 문서를 진행 한 후에는 대상 클러스터의 클러스터 매니페스트에 표시 된 것 처럼 값을 복사 하 여 붙여 넣는 것 처럼 보입니다. 따라서 클러스터 별로 설정을 구성할 수 있어야 하지만 [Azure Portal][11]를 통해 클러스터를 만들 때이 설정을 구성할 수 있는 옵션은 없으며 항상 "Fabric: ImageStore"입니다. 이 설정의 용도는 무엇인가요?
...
클러스터 자체 내의 시스템 서비스에서 이미지 스토리지를 호스팅하면 패키지 리포지토리에 대한 외부 종속성을 제거하고 스토리지의 위치를 제어할 수 있게 됩니다. 단독으로 그렇지 않으면 이미지 저장소에 대한 향후 개선은 먼저 이미지 저장소 공급자를 대상으로 할 가능성이 큽니다. 클라이언트가 대상 클러스터에 이미 연결되어 있으므로 이미지 저장소 서비스 공급자에 대한 연결 문자열에는 고유한 정보가 없습니다. 클라이언트는 시스템 서비스를 대상으로 하는 프로토콜을 사용해야 함을 알고 있어야 합니다.

개발 중에 one-box 로컬 클러스터에 이미지 저장소 서비스가 아닌 파일 시스템 공급자를 사용하여 클러스터를 약간 더 빠르게 부트스트랩합니다. 차이점은 일반적으로 작지만 개발 중인 사용자 대부분에게 유용한 최적화입니다. 다른 스토리지 공급자 유형도 one-box 로컬 클러스터를 배포할 수 있지만 개발/테스트 워크플로가 공급자에 관계 없이 동일하게 유지되기 때문에 일반적으로 작업을 수행할 필요가 없습니다. Azure Storage 공급자는 오직 Image Store Service 공급자가 도입되기 전에 배포된 기존 클러스터의 레거시 지원을 위해 존재합니다.

또한 파일 시스템 공급자 또는 Azure Storage 공급자는 여러 클러스터 간에 Image Store를 공유하는 방법으로 사용하면 안됩니다. 그렇지 않으면 각 클러스터에서 충돌하는 데이터를 Image Store에 쓸 수 있으므로 클러스터 구성 데이터가 손상됩니다. 여러 클러스터 간에 프로 비전 된 응용 프로그램 패키지를 공유 하려면 다운로드 URI를 사용 하 여 모든 외부 저장소에 업로드할 수 있는 [.sfpkg][12] 파일을 대신 사용 합니다.
