<properties
   ...
   description="이 문서에서는 독립 실행형 또는 개인 서비스 패브릭 클러스터를 구성하는 방법을 설명합니다."
...
# 독립 실행형 Windows 클라스터에 대한 구성 설정

이 문서에서는 _**ClusterConfig.JSON**_ 파일을 사용하여 독립 실행형 서비스 패브릭 클러스터를 구성하는 방법을 설명합니다. 이 파일은 [독립 실행형 서비스 패브릭 패키지를 다운로드](service-fabric-cluster-creation-for-windows-server.md#downloadpackage)할 때 작업 컴퓨터에 다운로드됩니다. ClusterConfig.JSON 파일을 사용하여 서비스 패브릭 클러스터에 대한 서비스 패브릭 노드 및 해당 IP 주소, 클러스터의 다른 노드 형식, 보안 구성에 대한 정보와 장애/업그레이드 도메인과 관련된 네트워크 토폴로지에 대한 정보를 지정할 수 있습니다.

아래에서는 이 파일의 다양한 섹션을 살펴보겠습니다.

## 일반 클러스터 구성
여기서는 아래의 JSON 코드 조각에 표시된 광범위한 클러스터 특정 구성을 살펴봅니다.

    "name": "SampleCluster",
    "clusterManifestVersion": "1.0.0",
    "apiVersion": "2015-01-01-alpha",

**name** 변수에 할당하여 서비스 패브릭 클러스터에 이름을 지정할 수 있습니다. 설정에 따라 **clusterManifestVersion**을 변경할 수 있습니다. 이 항목은 서비스 패브릭 구성을 업그레이드 하기 전에 업데이트해야 합니다. **apiVersion**은 기본값으로 남겨 두어도 됩니다.


<a id="clusternodes"></a>
## 클러스터의 노드
다음 코드 조각처럼 **nodes** 섹션을 사용하여 서비스 패브릭 클러스터에서 노드를 구성할 수 있습니다.
...
서비스 패브릭 클러스터에는 최소 3개의 노드가 필요합니다. 설정에 따라 이 섹션에 노드를 더 추가할 수 있습니다. 다음 표에서는 각 노드의 구성 설정에 대해 설명합니다.
...
|faultDomain|장애 도메인은 클러스터 관리자가 공유되는 물리적 종속성으로 인해 동시에 장애를 경험할 가능성이 있는 물리 노드를 정의할 수 있도록 합니다.|
|upgradeDomain|업그레이드 도메인은 서비스 패브릭 업그레이드를 위해 거의 같은 시간에 종료된 노드 집합을 나타냅니다. 물리적 요구 사항에 따라 제한되지 않으므로 어떤 업그레이드 도메인에 어떤 노드를 할당할지 선택할 수 있습니다.| 


## 진단 구성
다음 코드 조각과 같이 **diagnosticsFileShare** 섹션을 사용하여 진단 및 문제 해결 노드와 클러스터 오류를 사용하도록 매개 변수를 구성할 수 있습니다.
...
### **security** 
**security** 섹션은 보안 독립 실행형 서비스 패브릭 클러스터에 필요합니다. 다음 코드 조각에서는 이 섹션의 일부를 보여 줍니다.
