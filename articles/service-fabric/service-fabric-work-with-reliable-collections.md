...
description: Azure Service Fabric 애플리케이션 내에서 신뢰할 수 있는 컬렉션을 사용하기 위한 모범 사례를 알아봅니다.
ms.topic: conceptual
ms.date: 03/10/2020
...
일반적으로 위 코드에서 보여준 대로 TimeoutException을 catch하고 전체 작업을 다시 시도하여 이에 반응하는 코드를 작성합니다. 이 간단한 코드에서 매번 100밀리초를 전달하는 Task.Delay를 호출하려 합니다. 그러나 실제로는 대신 일종의 지수 백오프 지연을 사용하는 것이 좋습니다.

잠금이 설정되면 AddAsync는 ITransaction 개체와 관련된 내부 임시 사전에 키와 값 개체 참조를 추가합니다. 고유한 쓰기 읽기 의미 체계를 제공하여 수행됩니다. 즉, AddAsync를 호출한 후에 트랜잭션을 커밋하지 않더라도 (동일한 ITransaction 개체를 사용하는) TryGetValueAsync에 대한 이후의 호출은 값을 반환합니다.

> [!NOTE]
> 새 트랜잭션으로 TryGetValueAsync를 호출하면 마지막으로 커밋된 값에 대한 참조가 반환됩니다. 변경 사항을 유지하고 복제하는 메커니즘을 우회하므로 해당 참조를 직접 수정하지 않습니다. 신뢰할 수 있는 사전 API를 통해서만 키 값을 변경할 수 있도록 값을 읽기 전용으로 설정하는 것이 좋습니다.
...
예를 들어 복제된 캐시와 같은 일부 워크로드에서는 데이터 손실이 가끔 발생할 수 있습니다. 디스크에 대한 데이터의 지속성을 방지하면 신뢰할 수 있는 사전에 쓸 때 대기 시간 및 처리량을 향상할 수 있습니다. 지속성 결여의 단점은 쿼럼 손실이 발생하면 전체 데이터 손실이 발생한다는 것입니다. 쿼럼 손실은 드물게 발생하므로 이러한 워크로드에 대한 데이터 손실의 가능성이 적으므로 성능을 향상하는 것이 가치가 있습니다.

현재 휘발성 지원은 신뢰할 수 있는 사전 및 신뢰할 수 있는 큐에서만 사용할 수 있으며 ReliableConcurrentQueues에서는 사용할 수 없습니다. 휘발성 컬렉션의 사용 여부에 대한 결정을 내릴 수 있도록 [주의 사항](service-fabric-reliable-services-reliable-collections-guidelines.md#volatile-reliable-collections) 목록을 참조 하세요.

서비스에서 휘발성 지원을 사용하도록 설정 하려면 다음과 같이 서비스 형식 선언의 ```HasPersistedState``` 플래그를 ```false```로 설정합니다.
```xml
<StatefulServiceType ServiceTypeName="MyServiceType" HasPersistedState="false" />
```

>[!NOTE]
>기존의 지속형 서비스는 휘발성으로 만들 수 없으며 그 반대의 경우도 마찬가지입니다. 이렇게 하려면 기존 서비스를 삭제한 다음 업데이트된 플래그로 서비스를 배포해야 합니다. 즉, ```HasPersistedState``` 플래그를 변경하려는 경우 전체 데이터 손실이 발생할 수 있음을 감수해야 한다는 것입니다. 
...
> .NET 문자열은 키로 사용할 수 있지만 문자열 자체를 키로 사용합니다. String.GetHashCode의 결과를 키로 사용하지 않습니다.

또는 일반적으로 참조되는 요소를 2단계 업그레이드로 수행할 수 있습니다. 2단계 업그레이드로 V1에서 V2로 서비스를 업그레이드합니다. V2는 새 스키마 변경을 처리하는 방법을 알지만 이 코드가 실행하지 않는 코드를 포함합니다. V2 코드가 V1 데이터를 읽는 경우 해당 위치에서 작동하고 V1 데이터를 작성합니다. 그런 다음 모든 업그레이드 도메인에서 업그레이드가 완료되면 어떤 이유로든 실행 중인 V2 인스턴스에 업그레이드가 완료되었음을 알릴 수 있습니다. (이를 알리는 한 가지 방법은 구성 업그레이드를 롤아웃하는 것입니다. 이는 2단계 업그레이드를 수행하는 것입니다.) 이제 V2 인스턴스는 V1 데이터를 읽고, V2 데이터로 변환하고, 작동하고, V2 데이터로 쓸 수 있습니다. 다른 인스턴스가 V2 데이터를 읽을 경우 변환할 필요가 없이 해당 위치에서 작동하고 V2 데이터를 작성합니다.
