...
- 행위자의 모든 타이머를 지우는 `OnDeactivateAsync` 메서드(행위자 구현 시 재정의될 수 있음)를 호출합니다.
...
## 행위자 가비지 수집
행위자 런타임은 일정 기간 동안 사용되지 않은 행위자를 주기적으로 스캔하여 비활성화합니다. 비활성화되면 CLR에 의해 가비지 수집될 수 있습니다.
...
행위자를 사용할 때마다 유휴 시간이 0으로 재설정됩니다. 이후부터는 `IdleTimeoutInSeconds` 동안 다시 유휴 상태인 경우에만 행위자가 가비지 수집됩니다. 행위자는 행위자 미리 알림 콜백이 실행되는 행위자 인터페이스 메서드 중 하나인 경우, 사용된 것으로 간주됩니다. 타이머 콜백이 실행되는 경우 해당 행위자는 사용된 것으로 간주되지 **않습니다**.

아래 다이어그램에는 이러한 개념을 설명하는 예가 포함되어 있습니다.

![][1]

이 예에서는 활성 행위자 테이블에 활성 행위자가 하나만 있다고 가정하며 행위자 메서드 호출, 미리 알림 및 타이머가 이 행위자의 수명에 미치는 영향을 보여줍니다. 이 예제에 대해 다음과 같은 사항에 주의해야 합니다.

- 이 예제에서 ScanInterval 및 IdleTimeout은 각각 5와 10으로 설정되어 있습니다(개념을 설명하기 위한 것이므로 단위는 중요하지 않습니다).
- ScanInterval이 5로 정의되어 있으므로 가비지 수집할 행위자의 스캔은 T=0,5,10,15,20,25에서 수행됩니다.
...
- T=25에서 가비지 수집 스캔이 수행되는 동안, 결국 행위자의 유휴 시간이 10으로 설정된 IdleTimeout을 초과하고 행위자는 가비지 수집됩니다.
