---
title: Azure 서비스 패브릭은 일반적인 시나리오를 진단합니다.
description: Azure Service Fabric 응용 프로그램 내에서 일반적인 모니터링 및 진단 시나리오 문제 해결에 대해 알아봅니다.
ms.topic: article
ms.date: 02/25/2019
...
이 문서에서는 Service Fabric을 사용하여 모니터링 및 진단 영역에서 사용자에게 발생한 일반적인 시나리오에 대해 설명합니다. 여기서 제시되는 시나리오에서는 애플리케이션, 클러스터 및 인프라의 Service Fabric 3개 계층을 모두 다룹니다. 각 솔루션은 응용 프로그램 인사이트 및 Azure 모니터 로그, Azure 모니터링 도구를 사용하여 각 시나리오를 완료합니다. 각 솔루션의 단계는 사용자에게 서비스 패브릭의 컨텍스트에서 응용 프로그램 인사이트 및 Azure Monitor 로그를 사용하는 방법에 대한 소개를 제공합니다.
...
* [로그 분석 작업 영역 설정](service-fabric-diagnostics-oms-setup.md)
...
2. Service Fabric Application Insights SDK를 사용하는 경우 서로 연결된 서비스의 시각적 표현과 성공하거나 실패한 요청 수를 볼 수 있습니다. 왼쪽에서 "애플리케이션 맵"을 클릭합니다.
    ...
    ![AI 앱](media/service-fabric-diagnostics-common-scenarios/app-map-blade.png) ![지도 블레이드 AI 앱 지도](media/service-fabric-diagnostics-common-scenarios/app-map-new.png)
...
2. 블레이드 아래쪽에서 "요약"이라는 그래프를 클릭합니다.
    ...
    ![Azure 모니터 로그 솔루션](media/service-fabric-diagnostics-common-scenarios/oms-solution-azure-portal.png)
...
5. 위쪽에서 "새 경고 규칙"을 클릭하면, 이제 이 쿼리에 따라 이벤트가 도착할 때마다 선택한 통신 방법으로 경고 메시지를 받게 됩니다.
    ...
    ![Azure 모니터로 새 경고 기록](media/service-fabric-diagnostics-common-scenarios/oms-create-alert.png)
...
1. 로그 애널리틱스 에이전트를 클러스터에 추가한 후에는 추적할 특정 성능 카운터를 추가해야 합니다. 솔루션 페이지에서 작업 영역 탭이 왼쪽 메뉴에 있는 포털의 로그 분석 작업 영역 페이지로 이동합니다.
...
응용 프로그램에서 신뢰할 수 있는 서비스 또는 행위자의 성능을 추적하려면 서비스 패브릭 행위자, 행위자 메서드, 서비스 및 서비스 메서드 카운터도 수집해야 합니다. 다음은 신뢰할 수 있는 서비스 및 수집할 액터 성능 카운터의 예입니다.

>[!NOTE]
>서비스 패브릭 성능 카운터는 현재 Log Analytics 에이전트에서 수집할 수 없지만 [다른 진단 솔루션에서](service-fabric-diagnostics-partners.md) 수집할 수 있습니다.
...
* [공통 코드 패키지 활성화 오류 찾아보기](./service-fabric-diagnostics-code-package-errors.md)
...
* 검색 및 진단에 도움이 될 Azure Monitor [경고에](../log-analytics/log-analytics-alerts.md) 대해 자세히 알아봅니다.
* 온-프레미스 클러스터의 경우 Azure Monitor 로그는 Azure Monitor 로그에 데이터를 보내는 데 사용할 수 있는 게이트웨이(HTTP Forward Proxy)를 제공합니다. [로그 분석 게이트웨이를 사용하여 Azure Monitor 로그에 인터넷에 액세스하지 않고 컴퓨터를 연결하는](../azure-monitor/platform/gateway.md) 것에 대해 자세히 알아보기
* Azure Monitor 로그의 일부로 제공되는 [로그 검색 및 쿼리](../log-analytics/log-analytics-log-searches.md) 기능에 익숙해지기
* Azure Monitor 로그와 그 로그에 대한 자세한 개요를 확인하고 [Azure Monitor 로그는 무엇입니까?](../operations-management-suite/operations-management-suite-overview.md)
