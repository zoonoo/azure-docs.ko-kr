---
title: Azure 서비스 패브릭에서 Linux 클러스터 이벤트 모니터링
description: 서비스 패브릭 플랫폼 이벤트를 Syslog에 작성하여 서비스 패브릭 Linux 클러스터 이벤트를 모니터링하는 방법에 대해 알아봅니다.
author: srrengar
ms.topic: conceptual
ms.date: 10/23/2018
ms.author: srrengar
...
SyslogConsumer는 Facility `Local0`를 사용하여 모든 플랫폼 이벤트를 씁니다. 구성 구성을 변경하여 유효한 시설로 업데이트할 수 있습니다. 사용되는 ID는 `ServiceFabric`. 메시지 필드에는 다양한 도구에서 쿼리하고 사용할 수 있도록 JSON으로 직렬화된 전체 이벤트가 포함됩니다. 
...
2. 진단 섹션에서 새 소비자 인스턴스: SyslogConsumer가 추가되었습니다. 플랫폼에 또 다른 이벤트 소비자가 생긴 것입니다. 
...
2. `Data`을 클릭합니다.
3. `Syslog`을 클릭합니다.
4. Local0을 추적할 시설로 구성합니다. 패브릭에서 변경한 경우 다른 시설을 추가할 수 있습니다.
...
* [보기 디자이너를 사용하여 Azure 모니터 로그에서 사용자 지정 보기 만들기](../log-analytics/log-analytics-view-designer.md)
* Azure 모니터에 대 한 참조 [Syslog와의 통합을 기록 합니다.](../log-analytics/log-analytics-data-sources-syslog.md)
