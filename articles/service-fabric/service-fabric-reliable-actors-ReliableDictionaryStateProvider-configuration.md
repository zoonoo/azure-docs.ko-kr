<properties
   pageTitle="Azure 서비스 패브릭 신뢰 행위자 'ReliableDictionaryActorStateProvider' 구성의 개요 | Microsoft Azure"
...
|MaxPrimaryReplicationQueueSize|작업의 수|8192|기본 큐의 최대 작업 수. 작업은 주 복제자가 모든 보조 복제자로부터 승인을 받은 후 해제됩니다. 이 값은 64보다 크고 2의 제곱 값이어야 합니다.|
...
CheckpointThresholdInMB 값은 복제자가 복제자의 전용 로그 파일에 상태 정보를 저장하는 데 사용할 수 있는 디스크 공간의 크기를 제어합니다. 이 값을 기본값보다 큰 값으로 높이면 로그에서 더 자세한 작업 기록을 사용할 수 있기 때문에 새 복제본을 세트에 추가할 때 재구성 시간은 단축되지만, 충돌 후 복제본의 복구 시간이 늘어날 가능성이 있습니다.
