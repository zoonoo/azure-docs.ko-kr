---
title: 'Azure ExpressRoute: 라우팅 최적화'
...
## <a name="path-selection-on-microsoft-and-public-peerings"></a>Microsoft 및 퍼블릭 피어링의 경로 선택
Microsoft 또는 퍼블릭 피어링을 활용하는 경우 하나 이상의 ExpressRoute 회로와 IX(Internet Exchange) 또는 ISP(인터넷 서비스 공급자)를 통한 인터넷 경로가 있으면 트래픽이 적절한 경로를 통과하도록 하는 것이 중요합니다. BGP는 LPM(가장 긴 접두사 일치)을 비롯한 다양한 요인에 따라 최적 경로 선택 알고리즘을 활용합니다. Microsoft 또는 퍼블릭 피어링을 통해 Azure로 향하는 트래픽이 ExpressRoute 경로를 트래버스하도록 하려면 고객이 *로컬 기본 설정* 특성을 구현하여 ExpressRoute 경로가 항상 선호되도록 해야 합니다. 
...
![ExpressRoute 사례 1 문제 - 고객과 Microsoft 간의 최적이 아닌 라우팅이 표시된 다이어그램](./media/expressroute-optimize-routing/expressroute-localPreference.png)
