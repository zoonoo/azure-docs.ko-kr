---
title: 'Azure 익스프레스라우팅: 피어링 구성'
description: 이 문서에서는 ExpressRoute 개인 및 Microsoft 피어링을 만들고 프로비전하는 단계를 문서화합니다. 이 문서에서는 회로에 대한 피어링을 확인, 업데이트 또는 삭제하는 방법을 보여 줍니다.
...
이 문서에서는 Azure 포털을 사용하여 ARM(Azure 리소스 관리자) ExpressRoute 회로에 대한 라우팅 구성을 만들고 관리하는 데 도움이 됩니다. ExpressRoute 회로에 대한 피어링의 상태를 확인, 업데이트 또는 삭제 및 프로비전 해제를 수행할 수도 있습니다. 회로를 사용하는 다른 메서드를 사용하려는 경우 다음 목록에서 문서를 선택합니다.
...
* 활성화된 ExpressRoute 회로가 있어야 합니다. 지침을 수행하여 [ExpressRoute 회로를 만들고](expressroute-howto-circuit-portal-resource-manager.md) 진행하기 전에 연결 공급자를 통해 회로를 사용하도록 설정합니다. 피어링을 구성하려면 ExpressRoute 회로가 프로비저닝되고 활성화된 상태여야 합니다. 
* 공유 키/MD5 해시를 사용하려는 경우 터널 양쪽에서 이 해시를 사용하고 상숫자 문자 수를 최대 25개로 제한해야 합니다. 특수 문자는 지원되지 않습니다. 
...
1. ExpressRoute 회로를 구성합니다. 공급자 **상태를** 확인하여 계속하기 전에 연결 공급자가 회로를 완전히 프로비저닝되었는지 확인합니다.

   연결 공급자가 관리된 3계층 서비스를 제공하는 경우 연결 공급자를 요청하여 Microsoft 피어링을 사용하도록 할 수 있습니다. 이 경우 다음 섹션에 나열된 지침을 따를 필요가 없습니다. 그러나 연결 공급자가 회로를 만든 후 라우팅을 관리하지 않는 경우 다음 단계를 계속 진행합니다.
...
4. Microsoft 피어링을 구성합니다. 모든 매개 변수를 지정하면 구성을 **저장합니다.** 다음 이미지는 예제 구성을 보여 주며 다음과 같은 구성을 보여 주며 다음과 같은 예제 구성을 보여 주며 다음과 같은 예입니다
...
>
   ...
   회로가 '유효성 검사가 필요한' 상태가 되면 지원 티켓을 열어 접두사 소유권을 지원 팀에 표시해야 합니다. 다음 예제와 같이 포털에서 바로 지원 티켓을 열 수 있습니다.

   ![필요한 유효성 검사 - 지원 티켓](./media/expressroute-howto-routing-portal-resource-manager/ticket-portal-m.png)

5. 구성이 성공적으로 수락되면 다음 이미지와 비슷한 이미지가 표시됩니다.
...
피어링에 대한 행을 선택하여 Microsoft 피어링의 속성을 볼 수 있습니다.
...
다음 이미지와 같이 삭제 아이콘을 클릭하여 피어링 구성을 제거할 수 있습니다.
...
1. ExpressRoute 회로를 구성합니다. 계속하기 전에 연결 공급자에 의해 회로가 완전이 프로비전되었는지 확인합니다. 
   ...
   연결 공급자가 관리된 3계층 서비스를 제공하는 경우 연결 공급자를 요청하여 Azure 프라이빗 피어링을 사용하도록 할 수 있습니다. 이 경우 다음 섹션에 나열된 지침을 따를 필요가 없습니다. 그러나 연결 공급자가 회로를 만든 후 라우팅을 관리하지 않는 경우 다음 단계를 진행합니다.
...
2. 회로에 Azure 프라이빗 피어링을 구성합니다. 다음 단계를 계속 진행하기 전에 다음 항목이 있는지 확인합니다.
   ...
   * 개인 피어링을 설정할 때 BGP를 통해 온-프레미스 Edge 라우터에서 Azure로 의 경로를 보급해야 합니다.
   * **선택 사항 -** 사용하기로 선택한 경우 MD5 해시를 사용합니다.
...
4. 프라이빗 피어링을 구성합니다. 모든 매개 변수를 지정하면 구성을 **저장합니다.**
...
피어링의 행을 선택하고 피어링 속성을 수정할 수 있습니다. 업데이트 한 후 변경 내용을 저장합니다.
...
다음 단계, [VNet을 익스프레스루트 회로에 연결](expressroute-howto-linkvnet-portal-resource-manager.md)
