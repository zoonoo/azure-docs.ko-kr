...
- 네트워크 서비스 공급자를 사용하고 네트워크 서비스 공급자가 지원되는 목록에 없는 경우에도 Azure에 대한 연결을 설정할 수 있습니다.
	- 공급자가 지원되는 목록에 있는 Exchange 위치 중 하나에 있는지 확인하려면 네트워크 공급자를 확인합니다.
	- 네트워크 공급자가 선택한 Exchange 위치로 네트워크를 확장합니다.
	- Azure에 연결하려면 Exchange 공급자를 통해 Express 경로 회로를 주문합니다.
- 서비스 공급자의 인프라에 연결합니다. 다음 항목 중 하나 이상에 대한 조건을 충족해야 합니다.
