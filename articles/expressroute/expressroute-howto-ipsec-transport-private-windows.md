...
3. **IP 보안 필터 동작 마법사** 에서 **다음** 을 선택 합니다.
   ...
   [![17]][17]
4. 필터 작업에 나중에 쉽게 찾을 수 있는 이름을 지정합니다. 이 예제에서 필터 작업 이름은 **myEncryption** 입니다. 설명을 추가할 수도 있습니다. **다음** 을 선택합니다.
...
5. IP 트래픽의 원본 주소 **Ip 주소 또는 서브넷** 을 지정 하 고 **다음** 을 선택 합니다.
   ...
   [![28]][28]
6. **대상 주소:** IP 주소 또는 서브넷을 지정합니다. **다음** 을 선택합니다.

   [![29]][29]
7. **IP 프로토콜 유형** 페이지에서 **TCP** 를 선택합니다. **다음** 을 선택합니다.

   [![30]][30]
8. **IP 프로토콜 포트** 페이지에서 **모든 포트에서** 및 **이 포트로:** 를 선택합니다. 텍스트 상자에 **8080** 을 입력합니다. 이러한 설정은 대상 포트 8080의 HTTP 트래픽만 암호화되도록 지정합니다. **다음** 을 선택합니다.
...
1. OU에 연결된 **Create an IPsec policy with security ru**(Active Directory의 IPSecurity 정책)을 선택합니다. 마우스 오른쪽 단추를 클릭하고 **IP 보안 정책 만들기** 를 선택합니다.
   ...
   [![37]][37]
2. 보안 정책 이름을 지정합니다. 예: **policy-azure-onpremises**. **다음** 을 선택합니다.
