...
description: PowerShell을 사용 하 여 리소스 관리자 배포 모델에서 Express 경로 회로에 대 한 라우팅 구성을 만들고 관리 하는 방법을 알아봅니다.
...
> [!IMPORTANT]
> Microsoft는 인터넷 라우팅 레지스트리에서 지정된 '보급된 공용 접두사' 및 '피어 ASN'(또는 '고객 ASN')이 사용자에게 할당되었는지 확인합니다. 다른 엔터티에서 공용 접두사를 가져오는 경우, 라우팅 레지스트리에 할당이 기록되지 않은 경우 자동 유효성 검사가 완료되지 않으며 수동 유효성 검사가 필요합니다. 자동 유효성 검사에 실패 하는 경우 "AzExpressRouteCircuitPeeringConfig"의 출력에 ' AdvertisedPublicPrefixesState '가 ' 유효성 검사 필요 '로 표시 됩니다 (아래의 "Microsoft 피어 링 세부 정보를 가져오려면" 참조). 
> 
...
1. ExpressRoute에 대한 PowerShell 모듈을 가져옵니다.
   ...
   Az를 모두 가져옵니다. \* 알려진 의미 체계 버전 범위 내에 있는 모듈입니다.
