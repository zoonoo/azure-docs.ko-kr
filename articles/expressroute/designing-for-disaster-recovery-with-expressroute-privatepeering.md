...
그러나 Murphy의 인기 있는 말라을 사용 하는 *경우에는 문제가 발생할 수 있습니다*.이 문서에서는 단일 express 경로 회로를 사용 하 여 해결할 수 있는 오류를 벗어난 솔루션에 집중할 수 있습니다. 지역 중복 Express 경로 회로를 사용 하 여 재해 복구에 대 한 강력한 백 엔드 네트워크 연결을 빌드하기 위한 네트워크 아키텍처 고려 사항을 살펴봅니다.
...
Azure 지역 또는 온-프레미스 또는 다른 곳에서 중요 업무용 응용 프로그램을 실행 하는지 여부에 관계 없이 다른 Azure 지역을 장애 조치 사이트로 사용할 수 있습니다. 다음 문서에서는 응용 프로그램 및 프런트 엔드 액세스 관점에서의 재해 복구에 대해 다룹니다.
...
그러나 상태 저장 엔터티가 있는지 여부에 관계 없이 지역 중복 병렬 경로에서 트래픽 부하를 분산 하는 경우 네트워크 성능이 일관 되지 않을 수 있습니다. 이러한 지역 중복 병렬 경로는 위치에의 한 [공급자](expressroute-locations-providers.md#partners) 페이지에서 볼 수 있는 것과 동일한 metro 또는 다른 metro를 통과할 수 있습니다. 
...
분산 된 기업 네트워크가 많은 경우 Express 경로 회로가 여러 개 있을 가능성이 높습니다. 이 섹션에서는 다른 모든 회로를 필요로 하지 않고 활성-활성 Express 경로 회로를 사용 하 여 재해 복구를 설계 하는 방법을 알아보겠습니다. 
...
재해 복구를 설계 하는 방법은 지역 간 간 위치 간 (region1/region2 location2/location1) 트래픽이 라우팅되는 방법에 영향을 줍니다. 지역 간 위치 트래픽을 다르게 라우팅하는 서로 다른 두 가지 재해 아키텍처를 살펴보겠습니다.
