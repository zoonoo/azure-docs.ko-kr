...
ExpressRoute 및 인터넷에 대해 동일한 네트워크 주소 변환(NAT) 풀을 보급하도록 선택하면 개인 IP 주소에서 네트워크의 클라이언트와 유사한 문제가 표시됩니다. 이러한 서비스에 대한 IP 주소가 ExpressRoute를 통해 보급되지 않기 때문에 Windows 업데이트와 같은 서비스에 대한 요청은 인터넷을 통해 이동합니다. 그러나 반환 트래픽은 ExpressRoute를 통해 다시 돌아옵니다. Microsoft가 인터넷 및 ExpressRoute에서 동일한 서브넷 마스크를 가진 IP 주소를 수신하는 경우 인터넷에 대한 ExpressRoute를 선호합니다. 네트워크 에지에서 ExpressRoute에 연결된 방화벽이나 기타 상태 저장 디바이스에 흐름에 대한 이전 정보가 없는 경우 해당 흐름에 속하는 패킷을 삭제합니다.
