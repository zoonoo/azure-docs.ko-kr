...
ExpressRoute는 이제 경로 필터로 Microsoft 피어링을 사용하여 Azure PaaS 서비스(예: Azure 저장소 및 Azure SQL Database)를 지원합니다. Microsoft PaaS 및 SaaS 서비스에 액세스하려면 하나의 라우팅 도메인만 있으면 됩니다. 경로 필터를 활용하여 사용하려는 Azure 지역에 대한 PaaS 서비스 접두사를 선택적으로 보급할 수 있습니다.
...
* Azure 공용 피어링을 사용할 때 현재 Azure PaaS 서비스 끝점에 대한 네트워크 ACL(액세스 제어 목록)이 있는 경우 Microsoft 피어링으로 구성된 NAT IP 풀이 서비스 끝점에 대해 구성된 액세스 제어 목록에 포함되어 있는지 확인해야 합니다.

가동 중지 없이 Microsoft 피어링으로 이동하려면 이 문서의 단계를 제시된 순서대로 사용해야 합니다.

## <a name="1-create-microsoft-peering"></a>1. Microsoft 피어링 만들기
...
## <a name="2-validate-microsoft-peering-is-enabled"></a>2. Microsoft 피어링 사용에 대한 유효성 검사
...
## <a name="3-configure-and-attach-a-route-filter-to-the-circuit"></a>3. 회로에 경로 필터 구성 및 연결
...
경로 필터를 구성하려면 다음 문서 중 하나를 사용합니다.
...
## <a name="4-delete-the-public-peering"></a>4. 공용 피어링 삭제
