...
연결 공급자가 지원하는 전용 개인 연결을 통해 온-프레미스 네트워크를 Microsoft 클라우드로 확장하는 ExpressRoute에는 다음과 같이 고유한 세 가지 네트워크 영역이 필요합니다.
...
2.  CE: 고객 에지 라우터 
3.  PE(CE 연결): 고객 에지 라우터에 연결되는 공급자 에지 라우터/스위치
4.  PE(MSEE 연결): MSEE에 연결되는 공급자 에지 라우터/스위치
...
>[!NOTE]
>ExpressRoute 고가용성을 위해 MSEE(5)와 MSEE-PR(4) 간에 중복 쌍의 BGP 세션이 필요합니다. 또한 고객 네트워크와 MSEE-PR 간에 중복 쌍의 네트워크 경로를 사용하는 것이 좋습니다. 그러나 Any-to-Any(IPVPN) 연결 모델에서는 단일 CE 장치(2)가 하나 이상의 PE(3)에 연결될 수 있습니다.
>
>
...
연결 모델에 관계 없이 ExpressRoute 회로를 만들어야 하므로 회로 프로비전을 위해 생성된 서비스 키가 필요합니다. ExpressRoute 회로를 프로비전하면 MSEE-PR(4)와 MSEE(5) 사이에 계층 2 중복 연결이 설정됩니다. ExpressRoute 회로 만들기, 수정, 프로비전 및 확인 방법에 대한 자세한 내용은 [ExpressRoute 회로 만들기 및 수정][CreateCircuit] 문서를 참조하세요.
...
앞의 예제에서 명시한 대로 Azure 개인 피어링 라우팅 컨텍스트는 사용할 수 있지만, Azure 공용 피어링 및 Microsoft 피어링 라우팅 컨텍스트는 사용할 수 없습니다. 또한 성공적으로 사용되는 피어링 컨텍스트에는 기본 및 보조 지점 간(BGP에 필수) 서브넷이 나열되어 있습니다. /30 서브넷은 MSEE 및 MSEE-PR의 인터페이스 IP 주소로 사용됩니다. 

>[!NOTE]
>피어링을 사용하지 않는 경우 할당된 기본 및 보조 서브넷이 MSEE-PR의 구성과 일치하는지 확인합니다. 그렇지 않은 경우 MSEE 라우터의 구성을 변경하려면 [ExpressRoute 회로의 라우팅 만들기 및 수정][CreatePeering]을 참조하세요.
>
>
...
성공적으로 구성된 개인 피어링에 대한 샘플 응답은 다음과 같습니다.
 ...
 성공적으로 사용되는 피어링 컨텍스트에는 기본 및 보조 주소 접두사가 나열되어 있습니다. /30 서브넷은 MSEE 및 MSEE-PR의 인터페이스 IP 주소로 사용됩니다.
...
<p/>
>[!NOTE] 피어링을 사용하지 않는 경우 할당된 기본 및 보조 서브넷이 연결된 MSEE-PR의 구성과 일치하는지 확인합니다. 또한 t*VlandId*, *AzureASN* 및 d *PeerASN*이 MSEE에서 사용되는지와 이러한 값들이 연결된 MSEE-PR에서 사용되는 값들에 매핑되는지를 확인하세요. MD5 해싱이 선택되면 공유 키는 MSEE 및 MSEE-PR 쌍에서 동일해야 합니다. MSEE 라우터의 구성을 변경하려면 [ExpressRoute 회로의 라우팅 만들기 및 수정][CreatePeering]을 참조하세요..  
>
>
...
성공적으로 사용되는 피어링 컨텍스트에는 기본 및 보조 피어 서브넷이 나열되어 있습니다. /30 서브넷은 MSEE 및 MSEE-PR의 인터페이스 IP 주소로 사용됩니다.
...
<p/>
>[!NOTE] 피어링을 사용하지 않는 경우 할당된 기본 및 보조 피어 서브넷이 연결된 MSEE-PR의 구성과 일치하는지 확인합니다. 또한 t*VlanId*, *AzureAsn* 및 d*PeerAsn*이 MSEE에서 사용되는지와 이러한 값들이 연결된 MSEE-PR에서 사용되는 값들에 매핑되는지를 확인하세요. MSEE 라우터의 구성을 변경하려면 [ExpressRoute 회로의 라우팅 만들기 및 수정][CreatePeering]을 참조하세요..
>
>
...
>[!NOTE]
>상태가 활성 또는 유휴 상태이면 할당된 기본 및 보조 피어 서브넷이 연결된 MSEE-PR의 구성과 일치하는지 확인합니다. 또한 *VlanId*, *AzureAsn* 및 *PeerAsn*이 MSEE에서 사용되는지와 이러한 값들이 연결된 MSEE-PR에서 사용되는 값들에 매핑되는지를 확인하세요. MD5 해싱이 선택되면 공유 키는 MSEE 및 MSEE-PR 쌍에서 동일해야 합니다. MSEE 라우터의 구성을 변경하려면 [ExpressRoute 회로의 라우팅 만들기 및 수정][CreatePeering]을 참조하세요.
>
>
