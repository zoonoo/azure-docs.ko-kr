...
Express 경로를 사용하여 Microsoft 클라우드 서비스에 연결하려면 라우팅을 설치하고 관리해야 합니다. 일부 연결 공급자는 라우팅을 관리 서비스로 설치하고 관리해 줍니다. 연결 공급자를 확인하여 그런 서비스를 제공하는지를 확인합니다. 그렇지 않은 경우 아래에 설명한 요구 사항을 준수해야 합니다.

쉽게 연결할 수 있도록 설치해야 하는 라우팅 세션에 대한 설명은 [회로 라우팅 도메인](expressroute-circuit-peerings.md) 문서를 참조합니다.

**참고:** Microsoft는 고가용성 구성을 위해 라우터 중복 프로토콜을 지원하지 않습니다.(HSRP, VRRP 등) 고가용성에 대한 피어링에 대해 BGP 세션의 중복 쌍에 의존합니다.
...
192\.168.100.128/29을 선택하여 개인 피어링을 설치한 경우를 고려해 보세요. 192.168.100.128/29는 다음 중 192.168.100.128에서 192.168.100.133까지의 주소를 포함합니다.
...
라우팅 Exchange는 eBGP 프로토콜을 통합니다. EBGP 세션은 MSEE와 라우터 간에 설정됩니다. BGP 세션의 인증은 요구되지 않습니다. 필요한 경우 MD5 해시를 구성할 수 있습니다. BGP 세션 구성에 대한 정보는 라우팅 구성 워크플로를 살펴봅니다.

## 자치 시스템 번호

Microsoft는 Azure 공용, Azure 개인 및 Microsoft 피어링에 AS 12076를 사용합니다. 내부 사용을 위해 AS 65515를 예약했습니다. 16 및 32비트 AS 번호를 모두 지원합니다. Azure 개인 피어링에 개인 AS 번호를 사용할 수 있습니다. 공용 Azure 및 Microsoft 피어링에 대해 등록된 공용 AS 숫자를 사용해야 합니다.
...
Express 경로는 전송 라우터로 구성할 수 없습니다. 전송 여행 서비스의 경우 연결 공급자에 의존해야 합니다.
...
지정학적 지역 마다 하나 이상의 Express 경로 회로를 구입할 수 있습니다. 여러 연결이 있으면 지리적 중복으로 인해 고가용성에 상당한 이점을 제공합니다. Express 경로 회로가 여러 개 있는 경우 공용 피어링 및 Microsoft 경로 피어링의 Microsoft에서 보급된 동일한 접두사 집합을 받게 됩니다. 즉, 네트워크에서 Microsoft로 여러 경로가 있습니다. 잠재적으로 네트워크 내에서 최적이 아닌 라우팅이 결정될 수 있습니다. 결과적으로 다른 서비스에 최적이 아닌 연결 환경이 발생할 수도 있습니다.
...
| | 미국 중북부 | 12076:3007 |
| | 미국 중남부 | 12076:3008 |
| **남미** | 브라질 남부 | 12076:3014 |
| **유럽** | 북유럽 | 12076:3003 |
| | 서유럽 | 12076:3002 |
| **아시아 태평양** | 동아시아 | 12076:3010 |
| | 동남아시아 | 12076:3011 |
| **일본** | 일본 동부 | 12076:3012 |
| | 일본 서부 | 12076:3013 |
| **오스트레일리아** | 오스트레일리아 동부 | 12076:3015 |
| | 오스트레일리아 남동부 | 12076:3016 |
| **인도** | 인도 남부 | 12076:3019 |
| | 인도 서부 | 12076:3018 |
| | 인도 중부 | 12076:3017 |
