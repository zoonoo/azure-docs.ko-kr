---
title: '자습서: Express 경로 회로에 VNet 연결-Azure Portal'
description: 이 자습서에서는 Azure Portal를 사용 하 여 Azure Express 경로 회로에 가상 네트워크를 연결 하는 연결을 만드는 방법을 보여 줍니다.
...
ms.locfileid: "91773047"
---
# <a name="tutorial-connect-a-virtual-network-to-an-expressroute-circuit-using-the-portal"></a>자습서: 포털을 사용 하 여 Express 경로 회로에 가상 네트워크 연결
...
이 자습서를 통해 Azure Portal를 사용 하 여 Azure Express 경로 회로에 가상 네트워크를 연결 하는 연결을 만들 수 있습니다. Azure Express 경로 회로에 연결 하는 가상 네트워크는 같은 구독에 있거나 다른 구독의 일부일 수 있습니다.

이 자습서에서는 다음 작업 방법을 알아봅니다.
> [!div class="checklist"]
> - 가상 네트워킹을 동일한 구독의 회로에 연결 합니다.
> - 가상 네트워킹을 다른 구독의 회로에 연결 합니다.
> - 가상 네트워크와 Express 경로 회로 간의 링크를 삭제 합니다.
...
* 단일 VNet을 최대 4개의 ExpressRoute 회로에 연결할 수 있습니다. 다음 프로세스를 사용 하 여 연결 하려는 각 Express 경로 회로에 대 한 새 연결 개체를 만들 수 있습니다. ExpressRoute 회로는 동일한 구독, 서로 다른 구독 또는 두 가지가 혼합된 상태로 존재할 수 있습니다.
...
> [!NOTE]
> 계층 3 공급자가 피어 링을 구성한 경우 BGP 구성 정보가 나타나지 않습니다. 회로가 프로비전된 상태인 경우 연결을 만들 수 있어야 합니다.
>
...
2. 이제 연결 프로비전을 시작하여 가상 네트워크 게이트웨이를 ExpressRoute 회로에 연결할 수 있습니다. **연결**  >  **추가** 를 선택 하 여 **연결 추가** 페이지를 연 다음 값을 구성 합니다.
...
1. Express 경로 페이지에서 **권한 부여를 선택 하** 고 권한 부여에 대 한 **이름을** 입력 하 고 **저장**을 선택 합니다.
...
2. Marketplace에서 **"연결"** 을 검색 하 고, 선택 하 고, **만들기**를 선택 합니다.

    :::image type="content" source="./media/expressroute-howto-linkvnet-portal-resource-manager/search-connection.png" alt-text="ExpressRoute 회로 스크린샷":::

3. **연결 유형**이 "ExpressRoute"로 설정되어 있는지 확인합니다.
4. 세부 정보를 입력 하 고 기본 사항 페이지에서 **확인** 을 선택 합니다.
...
6. **권한 부여 키** 및 **피어 회로 URI**를 입력한 후 연결 이름을 지정합니다. **확인**을 선택합니다. **피어 회로 URI** 는 express 경로 회로의 리소스 ID입니다 .이는 Express 경로 회로의 속성 설정 창에서 찾을 수 있습니다.
...
7. **요약** 페이지에서 정보를 검토 하 고 **확인을**선택 합니다.
...
이 자습서에서는 가상 네트워크를 동일한 구독 및 다른 구독의 회로에 연결 하는 방법을 알아보았습니다. Express 경로 게이트웨이에 대 한 자세한 내용은 다음을 참조 하세요. 

> [!div class="nextstepaction"]
> [Express 경로 가상 네트워크 게이트웨이 정보](expressroute-about-virtual-network-gateways.md)
