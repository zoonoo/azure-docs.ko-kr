...
![응용 프로그램 시나리오](./media/cross-network-connectivity/premergerscenario.png)

위 그림의 중간에 있는 파선 화살표는 원하는 네트워크 상호 연결을 표시 합니다. 특히 세 가지 종류의 연결을 사용할 수 있습니다. 1) Fabrikam 및 Contoso Vnet 교차 연결, 2) 교차 지역 온-프레미스와 Vnet 교차 연결 (즉, Fabrikam 온-프레미스 네트워크를 Contoso VNet에 연결 하 고 Contoso 온-프레미스 네트워크를 Fabrikam VNet에 연결) 및 3) Fabrikam 및 Contoso 온-프레미스 네트워크 교차 연결을 연결 합니다. 

다음 표에서는 병합 전 Contoso l t d .의 Express 경로에 대 한 개인 피어 링의 경로 테이블을 보여 줍니다.

![병합 전 Contoso Express 경로 테이블](./media/cross-network-connectivity/contosoexr-rt-premerger.png)

다음 표에서는 합병 전의 Contoso 구독에 있는 VM의 유효 경로를 보여 줍니다. 테이블 마다 VNet의 VM은 VNet 주소 공간 및 Contoso 온-프레미스 네트워크를 기본 네트워크와 별도로 인식 합니다.

![병합기 앞의 Contoso VM 경로](./media/cross-network-connectivity/contosovm-routes-premerger.png)

다음 표에서는 병합 전 Fabrikam Inc.의 Express 경로에 대 한 개인 피어 링의 경로 테이블을 보여 줍니다.

![병합 전 Fabrikam Express 경로 테이블](./media/cross-network-connectivity/fabrikamexr-rt-premerger.png)

다음 표에서는 병합 전의 Fabrikam 구독에 있는 VM의 유효 경로를 보여 줍니다. 테이블 마다 VNet의 VM은 VNet 주소 공간 및 Fabrikam 온-프레미스 네트워크를 기본 파일과는 별도로 인식 합니다.

![병합 전 Fabrikam VM 경로](./media/cross-network-connectivity/fabrikamvm-routes-premerger.png)
...
![VNet 피어 링 후의 아키텍처](./media/cross-network-connectivity/vnet-peering.png )

다음 표에서는 Contoso 구독 VM에 알려진 경로를 보여 줍니다. 테이블의 마지막 항목에 주의 합니다. 이 항목은 가상 네트워크 교차 연결의 결과입니다.

![VNet 피어 링 후 Contoso VM 경로](./media/cross-network-connectivity/contosovm-routes-peering.png)

다음 표에서는 Fabrikam 구독 VM에 알려진 경로를 보여 줍니다. 테이블의 마지막 항목에 주의 합니다. 이 항목은 가상 네트워크 교차 연결의 결과입니다.

![VNet 피어 링 후 Fabrikam VM 경로](./media/cross-network-connectivity/fabrikamvm-routes-peering.png)
...
![연결할 expressroutes 교차 연결 후의 아키텍처](./media/cross-network-connectivity/exr-x-connect.png)
...
![ExR 및 Vnet를 교차 연결한 후의 Contoso Express 경로 테이블](./media/cross-network-connectivity/contosoexr-rt-xconnect.png)
...
![ExR 및 Vnet를 교차 연결한 후 Fabrikam Express 경로 테이블](./media/cross-network-connectivity/fabrikamexr-rt-xconnect.png)

다음 표에서는 Contoso 구독 VM에 알려진 경로를 보여 줍니다. 테이블의 *가상 네트워크 게이트웨이* 항목에 주의 합니다. VM은 온-프레미스 네트워크에 대 한 경로를 확인 합니다.

![ExR 및 Vnet를 교차 연결한 후의 Contoso VM 경로](./media/cross-network-connectivity/contosovm-routes-xconnect.png)

다음 표에서는 Fabrikam 구독 VM에 알려진 경로를 보여 줍니다. 테이블의 *가상 네트워크 게이트웨이* 항목에 주의 합니다. VM은 온-프레미스 네트워크에 대 한 경로를 확인 합니다.

![ExR 및 Vnet를 교차 연결한 후 Fabrikam VM 경로](./media/cross-network-connectivity/fabrikamvm-routes-xconnect.png)
...
![Global Reach 구성한 후의 아키텍처](./media/cross-network-connectivity/globalreach.png)
...
![Global Reach 후 Contoso Express 경로 테이블](./media/cross-network-connectivity/contosoexr-rt-gr.png)
...
![Global Reach 후 Fabrikam Express 경로 테이블]( ./media/cross-network-connectivity/fabrikamexr-rt-gr.png )
