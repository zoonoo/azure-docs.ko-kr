...
아닙니다. Smb 클라이언트는 SMB 서버에서 반환 되는 NIC 수와 일치 합니다.  각 저장소 볼륨은 하나의 저장소 끝점에서 액세스할 수 있습니다.  즉, 지정 된 SMB 관계에 하나의 NIC만 사용 됩니다.  
...
* 인스턴스 유형 또는 크기에 대 한 지원이 부족 하 여 가상 머신의 네트워크 인터페이스에서 고급 네트워킹을 사용 하지 않는 경우 더 큰 인스턴스 유형을 사용 하 여 사용 하지 않도록 설정 된 상태로 유지 됩니다. 이러한 경우 수동 작업을 수행 해야 합니다.
