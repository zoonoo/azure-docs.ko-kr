...
* 각 Azure Virtual Network(VNet)에서 하나의 서브넷만 Azure NetApp Files에 위임할 수 있습니다.   
   Azure를 사용하면 VNet에서 여러 개의 위임된 서브넷을 만들 수 있습니다.  그러나 두 개 이상의 위임된 서브넷을 사용하는 경우 새 볼륨을 만들려는 모든 시도가 실패합니다.
* 위임된 서브넷에서 네트워크 보안 그룹 또는 서비스 엔드포인트를 지정할 수 없습니다. 이렇게 하면 서브넷 위임이 실패합니다.
* 전역 피어가 피어있는 가상 네트워크의 볼륨에 대한 액세스는 현재 지원되지 않습니다.
* Azure NetApp 파일에 위임된 서브넷에 주소 접두사(대상)가 있는 VM 서브넷에서 [사용자 정의 사용자 지정 경로를](https://docs.microsoft.com/azure/virtual-network/virtual-networks-udr-overview#custom-routes) 만드는 것은 지원되지 않습니다. 이렇게 하면 VM 연결에 영향을 미칩니다.
...
1. 서브넷 추가 페이지에서 다음 필수 필드를 완료하여 Azure NetApp Files에 사용할 새 서브넷을 만듭니다.
    ...
    * **서브넷 위임**: **Microsoft.NetApp/볼륨을**선택합니다. 
