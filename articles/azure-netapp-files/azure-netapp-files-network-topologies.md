...
* Azure 가상 WAN에서는 Azure NetApp Files 지원 되지 않습니다.

Azure NetApp Files에는 다음과 같은 네트워크 제한이 적용 됩니다.

* Azure NetApp Files (피어 링 Vnet 포함)를 사용 하 여 VNet에서 사용 중인 Ip 수는 1000를 초과할 수 없습니다. 고객 규모 수요를 충족 하기 위해이 제한을 증가 시키기 위해 노력 하 고 있습니다. 
...
* 위의 아키텍처에서 스포크 VNET에 게이트웨이가 있는 경우 허브에서 게이트웨이를 통해 연결 하는 온-프레미스에서 ANF 볼륨에 대 한 연결이 손실 됩니다. 기본적으로, 기본 설정은 스포크 VNet의 게이트웨이에 부여 되므로 해당 게이트웨이를 통해 연결 되는 컴퓨터만 ANF 볼륨에 연결할 수 있습니다.
