...
|    볼륨 (지역 또는 전역 피어 링) 간 피어 링된 된 VNet에 연결    |    아니요    |    없음    |
|    ExpressRoute 게이트웨이 통해 볼륨에 대 한 연결    |    예    |         |
|    ExpressRoute 게이트웨이 및 VNet 게이트웨이 전송을 사용 하 여 피어 링을 통해 스포크 VNet에에서 있는 볼륨에서 온-프레미스 연결    |    아니요    |    허브 VNet (게이트웨이 사용 하 여 Azure VNet)에 위임 된 서브넷 만들기    |
...
서브넷에 Azure 리소스에서 사용할 수 있는 별도 주소 공간에 가상 네트워크를 분할 합니다.  이라는 특수 한 용도의 서브넷에 포함 되어 azure NetApp 파일 볼륨을 [서브넷을 위임](https://docs.microsoft.com/azure/virtual-network/virtual-network-manage-subnet)합니다. 
...
위의 다이어그램에서 VNet 2 및 VNet 3을 고려 합니다. VM 2 볼륨 2, 3 VM에 연결 해야 하는 경우 또는 VM 3 VM 2 또는 볼륨 1에 연결 해야 하는 경우 해야 VNet 2 및 3 VNet 간에 VNet 피어 링을 사용 하도록 설정 합니다. 
...
* VM 1 및 2 VM 온-프레미스 리소스는 사이트 간 VPN 또는 ExpressRoute 회로 통해 허브의 볼륨 1에 연결할 수 있습니다. 
* VM 1 및 2 VM 온-프레미스 리소스는 사이트 간 VPN 및 지역 Vnet 피어 링을 통해 볼륨 2 또는 3 권에 연결할 수 있습니다.
* VM 3 허브의 VNet에 연결할 수 볼륨 2 스포크 VNet 1에 볼륨 3 스포크 2 VNet에서.
