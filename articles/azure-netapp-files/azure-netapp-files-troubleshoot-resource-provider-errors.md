...
이 오류는 볼륨 `BaremetalTenantId` 을 업데이트 하거나 패치 하려고 할 때 속성에 변경 된 값이 있는 경우에 발생 합니다.
...
이 오류는 리소스에 대해 작업을 수행 하는 동안 다른 오류 처리에서 오류를 처리 하지 못한 경우에 표시 됩니다.   텍스트 ' 오류 '가 포함 됩니다. 는 `{action}` (`getting`, ,`creating` 또는)`deleting`중 하나일수있습니다`updating`.  `{resourceTypeName}` 는(예`netAppAccount`:, `capacityPool`, 등 )입니다.`volume` `resourceTypeName`
...
부분은 Active Directory 연결에 대 한 `ActiveDirectoryId` 속성에 입력 한 값입니다. `{string}`
...
`RuleIndex`이 메시지는 `AllowedClients` ,`UnixReadOnly` ,,`Nfsv3`, 및 에`Nfsv4`대 한 필드의 오류를 나타냅니다. `UnixReadWrite`

* 원인:   
`RuleIndex` ,`AllowedClients`, ,`Nfsv`, 3 및`Nfsv4`필드 중 하나 이상에 대해 입력 유효성 검사 요청이 실패 했습니다. `UnixReadWrite` `UnixReadOnly`
* 해결책:   
명령줄에서 필요한 모든 매개 변수 및 충돌 하지 않는 매개 변수를 설정 해야 합니다. 예를 들어 `UnixReadOnly` 및 `UnixReadWrite` 매개 변수를 동시에 설정할 수 없습니다.
* 해결 방법:   
위의 솔루션을 참조 하십시오.

***Vlan {1} {0} 에대한IP범위를이미사용{2} 하 고 있습니다.***
...
* 해결책:   
이 오류는 일시적일 수 있습니다. 몇 분 정도 기다린 후 다시 시도 하세요. 문제가 지속 되 면 기술 지원을 통해 문제를 조사할 티켓을 만듭니다.
* 해결 방법:   
...
***항목 수: 개체의 {value}: ExportPolicy. [RuleIndex] 규칙이 최소 최대값 범위를 벗어났습니다.***
...
* 원인:   
볼륨을 만들 때 속성을설정합니다.`OwnerId` 이후에는 속성을 변경할 수 없습니다.
* 해결책:   
...
***속성은 다음과 같습니다. 스냅숏의 위치는 볼륨과 동일 해야 합니다.***
...
이 오류는 알 수 없는 프로토콜 유형으로 볼륨을 만들 때 발생 합니다.  유효한 값은 "NFSv3" 및 "CIFS"입니다.
...
* 원인:   
볼륨을 만들 때 속성을설정합니다.`ProtocolType`  업데이트할 수 없습니다.
* 해결책:   
없음
* 해결 방법:   
...
이 오류는`NetAppAccount`리소스 ( `Volume`, `CapacityPool`, 또는 `Snapshot`)를 만들려고 하지만 할당량 제한에 도달 했을 때 발생 합니다.
...
* 해결책:   
의 값 `subnetId` 을 확인 하 여 사용 되는 서브넷에 `resourceId` 대 한가 포함 되어 있는지 확인 합니다.
* 해결 방법:   
없음 위의 솔루션을 참조 하십시오. 
...
* [REST API를 사용 하 여 Azure NetApp Files 개발](azure-netapp-files-develop-with-rest-api.md)
