...
이 오류는 볼륨을 업데이트 하거나 패치 하려고 할 때 `BaremetalTenantId` 속성 값이 변경 되는 경우에 발생 합니다.

* 원인:   
볼륨을 업데이트 하려고 하는데 `BaremetalTenantId` 속성의 값이 Azure에 저장 된 값과 다릅니다.
* 해결 방법:   
패치 및 업데이트 (put) 요청에 `BaremetalTenantId` 포함 하지 않습니다. 또는 `BaremetalTenantId` 요청에서 동일한 지 확인 합니다.
...
이 오류는 변경 된 `PoolId` 속성으로 용량 풀을 업데이트 하거나 패치 하려고 할 때 발생 합니다.

* 원인:   
용량 풀 `PoolId` 속성을 업데이트 하려고 합니다. @No__t_0 속성은 읽기 전용 속성 이므로 변경할 수 없습니다.
* 해결 방법:   
패치 및 업데이트 (put) 요청에 `PoolId` 포함 하지 않습니다.  또는 `PoolId` 요청에서 동일한 지 확인 합니다.

***CreationToken를 변경할 수 없습니다.***

이 오류는 볼륨이 만들어진 후 파일 경로 (`CreationToken`)를 변경 하려고 할 때 발생 합니다. 볼륨을 만들 때 파일 경로 (`CreationToken`)를 설정 해야 하며 나중에 변경할 수 없습니다.

* 원인:   
볼륨을 만든 후에는 지원 되는 작업이 아닌 파일 경로 (`CreationToken`)를 변경 하려고 합니다. 
* 해결 방법:   
...
이 오류는 리소스에 대해 작업을 수행 하는 동안 다른 오류 처리에서 오류를 처리 하지 못한 경우에 표시 됩니다.   텍스트 ' 오류 '가 포함 됩니다. @No__t_0 (`getting`, `creating`, `updating` 또는 `deleting`) 중 하나일 수 있습니다.  @No__t_0 `resourceTypeName` (예: `netAppAccount`, `capacityPool`, `volume` 등)입니다.
...
이 오류는 `FileSystemId`를 변경 하려고 할 때 발생 합니다.  @No__t_0를 변경 하는 작업은 지원 되지 않습니다. 

* 원인:   
볼륨을 만들 때 파일 시스템의 ID가 설정 됩니다. 이후에는 `FileSystemId`를 변경할 수 없습니다.
* 해결 방법:   
패치 및 업데이트 (put) 요청에 `FileSystemId` 포함 하지 않습니다.  또는 `FileSystemId`이 요청에서 동일한 지 확인 합니다.

***Id가 ' {string} ' 인 ActiveDirectory 없습니다.***

@No__t_0 부분은 Active Directory 연결에 대 한 `ActiveDirectoryId` 속성에 입력 한 값입니다.

* 원인:   
Active Directory 구성을 사용 하 여 계정을 만들 때 비어 있어야 하는 `ActiveDirectoryId` 값을 입력 했습니다.
* 해결 방법:   
만들기 (put) 요청에 `ActiveDirectoryId`를 포함 하지 않습니다.
...
* 원인:   
쿼리 매개 변수의 값 `api-version`에 잘못 된 값이 있습니다.
* 해결 방법:   
올바른 API 버전 값을 사용 합니다.  리소스 공급자는 많은 API 버전을 지원 합니다. 값은 yyyy-mm-dd 형식입니다.

***@No__t_1에 대해 잘못 된 값 ' {value} '이 (가) 수신 되었습니다.***

이 메시지는 `RuleIndex`, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite`, `Nfsv3`, `Nfsv4`에 대 한 필드에 오류가 있음을 나타냅니다.

* 원인:   
@No__t_0, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite`, `Nfsv`3 및 `Nfsv4` 필드 중 하나 이상에 대 한 입력 유효성 검사 요청이 실패 했습니다.
* 해결 방법:   
명령줄에서 필요한 모든 매개 변수 및 충돌 하지 않는 매개 변수를 설정 해야 합니다. 예를 들어 `UnixReadOnly`와 `UnixReadWrite` 매개 변수를 동시에 설정할 수 없습니다.
* 해결 방법:   
위의 솔루션을 참조 하십시오.

***Vlan {2} {1}에 대 한 IP 범위 {0} 이미 사용 중입니다.***
...
* 해결 방법:   
패치 및 업데이트 (put) 요청에 `MountTargets` 포함 하지 않습니다.  또는 `MountTargets` 요청에서 동일한 지 확인 합니다.
...
***' @No__t_1 '에 대 한 작업 결과 id를 찾을 수 없습니다.***
...
* 원인:   
볼륨을 만들 때 `OwnerId` 속성을 설정 합니다. 이후에는 속성을 변경할 수 없습니다.
* 해결 방법:   
패치 및 업데이트 (put) 요청에 `OwnerId` 포함 하지 않습니다. 또는 `OwnerId` 요청에서 동일한 지 확인 합니다.
...
이 오류는 리소스를 만들 때 `resourceId` name 속성 이외의 값으로 name 속성을 입력 하는 경우에 발생 합니다.

* 원인:   
리소스를 만들 때 name 속성에 잘못 된 값이 있습니다.
* 해결 방법:   
Name 속성을 비워 두거나 `resourceId`의 마지막 백슬래시 "/"와 물음표 "?" 사이에 name 속성과 동일한 값을 사용 하도록 허용 합니다.
...
* 원인:   
볼륨 `protocolType` 속성에서 잘못 된 값을 설정 하려고 합니다.
* 해결 방법:   
@No__t_0에서 유효한 문자열을 설정 합니다.
* 해결 방법:   
@No__t_0를 null로 설정 합니다.

***프로토콜 유형은 변경할 수 없습니다.***

이 오류는 볼륨에 대 한 `ProtocolType`를 업데이트 하거나 패치 하려고 할 때 발생 합니다.  ProtocolType을 변경 하는 작업은 지원 되지 않습니다.

* 원인:   
볼륨을 만들 때 `ProtocolType` 속성을 설정 합니다.  업데이트할 수 없습니다.
* 해결 방법:   
없음.
* 해결 방법:   
...
이 오류는 리소스를 만들려고 할 때 (`NetAppAccount`, `CapacityPool`, `Volume` 또는 `Snapshot`) 할당량 제한에 도달 했을 때 발생 합니다.

* 원인:   
리소스를 만들려고 하는데 할당량 한도에 도달 했습니다 (예: 구독 당 `NetAppAccounts` 또는 `NetAppAccount` 당 `CapacityPools`).
* 해결 방법:   
할당량 제한을 늘립니다.
* 해결 방법:   
...
이 오류는 볼륨을 만든 후 `subnetId`를 변경 하려고 할 때 발생 합니다.  볼륨을 만들 때 `SubnetId` 설정 해야 하며 나중에 변경할 수 없습니다.

* 원인:   
볼륨을 만든 후에 `subnetId`를 변경 하려고 합니다 .이 작업은 지원 되는 작업이 아닙니다. 
* 해결 방법:   
@No__t_0를 변경 하지 않아도 되는 경우 요청에서 매개 변수를 제거 하 여 오류 메시지를 해제 하는 것이 좋습니다.
* 해결 방법:   
@No__t_0를 변경 해야 하는 경우 새 `subnetId`를 사용 하 여 새 볼륨을 만든 다음 데이터를 새 볼륨으로 마이그레이션할 수 있습니다.

***SubnetId의 형식이 잘못 되었습니다.***

이 오류는 새 볼륨을 만들려고 하지만 `subnetId` 서브넷에 대 한 `resourceId` 되지 않은 경우에 발생 합니다.

* 원인:   
이 오류는 새 볼륨을 만들려고 하지만 `subnetId` 서브넷에 대 한 `resourceId` 되지 않은 경우에 발생 합니다. 
* 해결 방법:   
@No__t_0에 대 한 값을 확인 하 여 사용 되는 서브넷에 대 한 `resourceId` 포함 되어 있는지 확인 합니다.
* 해결 방법:   
없음. 위의 솔루션을 참조 하십시오. 

***서브넷에는 ' Microsoft NetApp/볼륨 ' 위임이 있어야 합니다.***

이 오류는 볼륨을 만들 때 선택한 서브넷이 `Microsoft.NetApp/volumes`로 위임 되지 않은 경우에 발생 합니다.

* 원인:   
볼륨을 만들려고 했지만 `Microsoft.NetApp/volumes`에 위임 되지 않은 서브넷을 선택 했습니다.
* 해결 방법:   
@No__t_0 위임 된 다른 서브넷을 선택 합니다.
* 해결 방법:   
서브넷에 올바른 위임을 추가 합니다.
