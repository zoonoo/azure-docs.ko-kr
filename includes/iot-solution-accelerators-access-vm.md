...
다음 단계에서는 Azure 클라우드 셸에서 Azure CLI를 사용합니다. 원하는 경우 개발 컴퓨터에 [Azure CLI를 설치하고](/cli/azure/install-azure-cli) 명령을 로컬로 실행할 수 있습니다.
...
1. SSH 액세스를 허용하도록 네트워크 보안 그룹을 업데이트합니다. 다음 명령에서는 네트워크 보안 그룹의 이름이 **contoso-simulation-nsg**라고 가정합니다. 이 값을 네트워크 보안 그룹의 이름으로 바꿉니다.
    ...
    테스트 및 개발하는 동안 SSH 액세스만 활성화합니다. SSH를 사용하도록 설정한 경우 [가능한 한 빨리 다시 비활성화해야 합니다.](https://docs.microsoft.com/azure/security/fundamentals/network-best-practices#disable-rdpssh-access-to-virtual-machines)
