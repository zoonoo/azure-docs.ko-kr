...
2. **AuthenticateAsync** 메서드를 다음 코드로 바꿉니다.
	...
	이 버전의 **AuthenticateAsync**에서는 앱이 **PasswordVault**에 저장된 자격 증명을 사용하여 모바일 앱에 액세스합니다. 저장된 토큰이 만료되지 않았는지 확인하기 위해 단순 쿼리가 전송됩니다. 401이 반환되면 일반 공급자 기반 로그인이 시도됩니다. 일반 로그인은 저장된 자격 증명이 없는 경우에도 수행됩니다.
