...
모듈이 서로 메시지를 전달하는 것으로 생각하는 것이 게이트웨이 작동 방식을 개념화하는 편리한 방식이지만, 이것은 벌어지는 상황을 정확하게 반영하지는 못합니다. 모듈은 메시지 버스를 사용하여 서로 통신합니다. 모듈은 버스에 메시지를 게시하고, 버스는 버스에 연결된 모든 모듈을 대상으로 메시지를 브로드캐스트합니다.

모듈은 **MessageBus\_Publish** 함수를 사용하여 메시지 버스에 메시지를 게시합니다. 메시지 버스는 콜백 함수를 호출하여 모듈에 메시지를 배달합니다. 메시지는 키/값 속성 및 메모리 블록으로 전달되는 콘텐츠의 집합으로 구성됩니다.

![][3]

메시지 버스는 연결된 모든 모듈에 브로드캐스트 메커니즘을 사용하여 각 메시지를 배달하기 때문에 각 모듈은 메시지 필터링을 담당합니다. 하나의 모듈은 그에 맞는 메시지에만 조치를 취해야 합니다. 메시지 필터링은 효과적으로 메시지 파이프라인을 만듭니다. 일반적으로 모듈은 처리해야 하는 메시지를 식별하기 위해 메시지 속성을 사용하여 수신한 메시지를 필터링합니다.
...
이전 섹션의 설명처럼, Hello World 모듈은 로거 모듈에 5초마다 메시지를 직접 전달하지 않습니다. 대신, 5초마다 메시지 버스에 메시지를 게시합니다.

로거 모듈은 메시지 버스로부터 메시지를 수신하고 필터로 속성을 검사합니다. 로거 모듈이 메시지를 처리해야 한다고 판단하면, 메시지 내용을 파일에 기록합니다.

로거 모듈은 메시지 버스의 메시지를 소비만 하고, 버스에 새 메시지를 전혀 게시하지 않습니다.

![][5]

위 그림은 Hello World 샘플의 아키텍처와 [리포지토리][lnk-gateway-sdk]에서 샘플의 다른 부분을 구현하는 소스 파일에 대한 상대 경로를 보여줍니다. 코드를 직접 알아보거나 아래 코드 조각을 참조용으로 사용합니다.
