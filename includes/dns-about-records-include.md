...
각 DNS 레코드에는 이름 및 형식이 있습니다. 레코드는 포함된 데이터에 따라 다양한 형식으로 제공됩니다. 가장 일반적인 형식은 이름을 IPv4 주소에 매핑하는 'A' 레코드입니다. 또 다른 형식은 이름을 메일 서버에 매핑하는 "MX" 레코드입니다.

Azure DNS는 A, AAAA, CNAME, MX, NS, SOA, SRV, TXT 등 모든 일반적인 DNS 레코드 형식을 지원합니다. SOA 형식의 레코드 집합은 각 영역과 함께 자동으로 생성되며 개별적으로 만들 수 없습니다. SPF 레코드는 TXT 레코드 형식을 사용하여 만들어야 합니다. 자세한 내용은 [이 페이지](http://tools.ietf.org/html/rfc7208#section-3.1)를 참조하세요.
