...
Azure DNS는 A, AAAA, CNAME, MX, NS, SOA, SRV, TXT를 포함하는 모든 일반적인 DNS 레코드 형식을 지원합니다. SOA 레코드 집합은 각 영역과 함께 자동으로 생성되며 개별적으로 만들 수 없습니다. SPF 레코드는 TXT 레코드 형식을 사용하여 만들어야 합니다. 자세한 내용은 [이 페이지](http://tools.ietf.org/html/rfc7208#section-3.1)를 참조하세요.
...
와일드카드 레코드 집합을 만들려면 레코드 집합 이름 "*"을 사용합니다. 또는 레이블 "*"이 있는 이름, 예를 들어 "*.foo"를 사용합니다.
