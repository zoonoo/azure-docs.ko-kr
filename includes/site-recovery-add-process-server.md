...
2. **시작하기 전에**추가 **프로세스 서버 추가를 선택하여 배포를 확장합니다.**

   ![프로세스 서버 추가](./media/site-recovery-add-process-server/ps-page-1.png)

3. **구성 서버 세부 정보에서**구성 서버의 IP 주소와 암호를 지정합니다.

   ![프로세스 서버 2 추가](./media/site-recovery-add-process-server/ps-page-2.png)
4. **인터넷 설정에서**구성 서버에서 실행 중인 공급자가 인터넷을 통해 Azure 사이트 복구에 연결하는 방법을 지정합니다.

   ![프로세스 서버 3 추가](./media/site-recovery-add-process-server/ps-page-3.png)

   * 컴퓨터에서 현재 설정된 프록시와 연결하려면 **기존 프록시 설정으로 연결을**선택합니다.
   * 공급자가 직접 연결하려면 프록시 **없이 직접 연결을 선택합니다.**
   * 기존 프록시에 인증이 필요하거나 공급자 연결에 대한 사용자 지정 프록시를 사용하려면 **사용자 지정 프록시 설정으로 연결을**선택합니다.

     * 사용자 지정 프록시를 사용하는 경우 주소, 포트 및 자격 증명을 지정해야 합니다.
     * 프록시를 사용하는 경우 이미 서비스 URL에 대한 액세스를 허용했어야 합니다.

5. **필수 구성 조건 검사에서**설치는 설치를 실행할 수 있는지 확인하기 위해 검사를 실행합니다. 전역 시간 동기화 **검사에**대한 경고가 나타나면 시스템 시계(날짜 및 시간 설정)의 시간이 표준**시간대와** 동일한지 확인합니다.
...
8. **네트워크 선택에서**구성 서버가 복제 데이터를 보내고 받는 리스너(네트워크 어댑터 및 SSL 포트)를 지정합니다. 9443 포트는 복제 트래픽을 보내고 받는 데 사용되는 기본 포트이지만, 환경의 요구 사항에 맞게 이 포트 번호를 수정할 수 있습니다. 9443 포트 외에도 웹 서버에서 복제 작업을 조정하기 위해 사용하는 443 포트를 엽니다. 복제 트래픽을 보내거나 받는 데 443 포트를 사용하면 안 됩니다.
