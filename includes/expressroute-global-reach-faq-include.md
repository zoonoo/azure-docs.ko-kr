...
ExpressRoute 회로를 연결하여 ExpressRoute Global Reach를 활성화합니다. 회로 연결을 끊어 기능을 해제합니다. [구성](../articles/expressroute/expressroute-howto-set-global-reach.md)을 참조 하세요.
...
Express 경로 Global Reach는 [선택 국가/지역 또는 장소](../articles/expressroute/expressroute-global-reach.md)에서 지원 됩니다. Express 경로 회로는 해당 국가/지역 또는 장소에서 피어 링 위치에 만들어야 합니다.
...
예, 회로는 지원 되는 국가/지역에 있는 한 가능 합니다. 한 번에 두 개의 ExpressRoute 회로를 연결해야 합니다. 완전한 메시형 네트워크를 만들려면 모든 회로 쌍을 열거하고 구성을 반복해야 합니다. 
...
아니요. 두 회로는 서로 다른 피어링 위치의 것이어야 합니다. 지원 되는 국가/지역의 metro에 둘 이상의 Express 경로 피어 링 위치가 있는 경우 해당 metro의 서로 다른 피어 링 위치에 생성 된 Express 경로 회로를 함께 연결할 수 있습니다. 
...
ExpressRoute Global Reach로 설정된 온-프레미스 네트워크 간의 네트워크 처리량은 두 ExpressRoute 회로 중 작은 것으로 제한됩니다. 온-프레미스와 Azure 간 트래픽 및 온-프레미스 트래픽은 동일한 회로를 공유 하며 동일한 대역폭 한도에 적용 됩니다. 
