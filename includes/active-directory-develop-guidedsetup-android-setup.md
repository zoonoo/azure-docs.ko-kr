...
이전 코드의 패키지는 획득, 캐싱, 새로 고침, 삭제를 비롯한 모든 토큰 작업을 처리하는 MSAL(Microsoft 인증 라이브러리)을 설치합니다. Microsoft ID 플랫폼으로 보호되는 API에 액세스하려면 토큰이 필요합니다.
<!--end-collapse-->
