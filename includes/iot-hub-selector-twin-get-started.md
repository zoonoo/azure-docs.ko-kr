...
* 장치 앱에서 사용된 연결 방법 등 사용 가능한 기능 및 조건과 같은 현재 상태 정보를 보고합니다.

* 펌웨어 및 구성 업데이트와 같은 장기 실행 워크플로의 상태를 장치 앱과 백 엔드 앱 간에 동기화합니다.
...
장치 쌍은 IoT 허브에 저장되며 다음 요소를 포함합니다.

* **태그**. 솔루션 백 엔드에서만 액세스할 수 있는 장치 메타데이터입니다.

* **원하는 속성**. 솔루션 백 엔드에서 수정할 수 있고 장치 앱에서 관찰할 수 있는 JSON 개체입니다.

* **reported 속성**. 장치 앱에서 수정할 수 있고 솔루션 백 엔드에서 읽을 수 있는 JSON 개체입니다.

태그 및 속성은 배열을 포함할 수 없지만, 개체는 중첩될 수 있습니다.

다음 그림에서는 장치 쌍 조직을 보여 주며,

![기능을 보여 주는 디바이스 쌍 이미지](./media/iot-hub-selector-twin-get-started/twin.png)

또한 솔루션 백 엔드는 위의 모든 데이터를 기반으로 하는 디바이스 쌍을 쿼리할 수 있습니다.
장치 쌍에 대한 자세한 내용은 [장치 쌍 이해](../articles/iot-hub/iot-hub-devguide-device-twins.md)를 참조하십시오. 쿼리에 대한 자세한 내용은 [IoT Hub 쿼리 언어를](../articles/iot-hub/iot-hub-devguide-query-language.md)참조하십시오.


이 자습서에서는 다음을 수행하는 방법에 대해 설명합니다.

* 태그를 디바이스 쌍에 추가하는 백 엔드 앱과 연결 채널을 보고된 속성으로 디바이스 쌍에 보고하는 시뮬레이션된 디바이스 앱을 만듭니다.
