...
Azure는 세 가지 유형의 Point-to-Site VPN 옵션을 지원합니다.

* SSTP(Secure Socket Tunneling Protocol) SSTP는 대부분의 방화벽이 443 SSL에서 사용하는 아웃바운드 TCP 포트를 열기 때문에 방화벽에 침투할 수 있는 Microsoft 의 독자적인 SSL 기반 솔루션입니다.

* Openvpn. OpenVPN은 대부분의 방화벽이 443 SSL에서 사용하는 아웃바운드 TCP 포트를 열기 때문에 방화벽에 침투할 수 있는 SSL 기반 솔루션입니다.

* IKEv2 VPN IKEv2 VPN은 아웃바운드 UDP 포트 500 및 4500 및 IP 프로토콜 No를 사용하는 표준 기반 IPsec VPN 솔루션입니다. 50. 방화벽이 항상 이러한 포트를 열지는 않습니다. 따라서 IKEv2 VPN이 프록시 및 방화벽을 트래버스할 수 없는 가능성이 있습니다.
...
### <a name="can-i-configure-a-point-to-site-client-to-connect-to-multiple-virtual-network-gateways-at-the-same-time"></a>동시에 여러 가상 네트워크 게이트웨이에 연결하도록 사이트 간 클라이언트를 구성할 수 있습니까?

사용된 VPN 클라이언트 소프트웨어에 따라 연결된 가상 네트워크가 연결되는 가상 네트워크 또는 클라이언트와의 네트워크 간에 충돌하는 주소 공간이 없는 경우 여러 가상 네트워크 게이트웨이에 연결할 수 있습니다.  Azure VPN 클라이언트는 많은 VPN 연결을 지원하지만 지정된 시간에 하나의 연결만 연결할 수 있습니다.
...
예. 다른 VNet과 피어인 VNet에 배포된 가상 네트워크 게이트웨이에 대한 사이트 간 연결은 피어된 다른 VNet에 액세스할 수 있습니다.  피어있는 VNet이 UseRemoteGateway / AllowGatewayTransit 기능을 사용하는 경우 지점 간 클라이언트는 피어있는 VNet에 연결할 수 있습니다.  자세한 내용은 [이](../articles/vpn-gateway/vpn-gateway-about-point-to-site-routing.md) 문서를 참조하시기 바랍니다.
...
아니요. SSTP의 경우 Windows에서 네이티브 VPN 클라이언트를 사용하고 IKEv2의 경우 Mac에서 네이티브 VPN 클라이언트를 사용할 수 있습니다. 그러나 모든 플랫폼에서 OpenVPN 클라이언트를 사용하여 OpenVPN 프로토콜을 통해 연결할 수 있습니다. 지원되는 클라이언트 운영 체제 목록을 참조하세요.

### <a name="does-azure-support-ikev2-vpn-with-windows"></a>Azure는 Windows에서 IKEv2 VPN을 지원합니까?

IKEv2는 Windows 10 및 Server 2016에서 지원됩니다. 그러나 IKEv2를 사용하려면 업데이트를 설치하고 로컬로 레지스트리 키 값을 설정해야 합니다. Windows 10 이전의 OS 버전은 지원되지 않으며 SSTP 또는 **OpenVPN® 프로토콜만**사용할 수 있습니다.
...
### <a name="how-do-i-remove-the-configuration-of-a-p2s-connection"></a><a name="removeconfig"></a>P2S 연결의 구성을 제거하려면 어떻게 해야 합니까?

다음 명령을 사용하여 Azure CLI 및 PowerShell을 사용하여 P2S 구성을 제거할 수 있습니다.
