...
| **제한 식별자** | **제한** | **설명** |
| --- | --- | --- |
| 총 용량(클라우드 포함) |가상 디바이스당 최대 64TB |전체 StorSimple 가상 배열을 다른 빈 배열에 장애 조치(Failover)할 수 있습니다. 동일한 디바이스를 복원하려는 경우 이 작업을 완료하기에 충분한 공간이 디바이스에 있는지 확인합니다. 32TB을 초과한 후에는 동일한 장치에 복원할 수 없습니다. |
