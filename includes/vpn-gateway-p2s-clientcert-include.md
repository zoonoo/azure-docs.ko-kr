...
- **엔터프라이즈 인증서:**
  ...
  - 엔터프라이즈 인증서 솔루션을 사용하는 경우 yourdomain.com 공통 이름 값 형식 *이름을\@* 사용하여 클라이언트 인증서를 생성합니다. *도메인 이름\사용자 이름* 형식 대신 이 형식을 사용합니다.
...
- **자체 서명된 루트 인증서:** 생성한 클라이언트 인증서가 P2S 연결과 호환되도록 다음 P2S 인증서 문서 중 하나의 단계를 따릅니다. 이러한 문서의 단계에서는 호환되는 클라이언트 인증서를 생성합니다. 
  ...
  * [MakeCert 지침](../articles/vpn-gateway/vpn-gateway-certificates-point-to-site-makecert.md): 인증서를 생성하기 위해 Windows 10 컴퓨터에 액세스할 수 없는 경우 MakeCert를 사용합니다. MakeCert는 더 이상 사용되지 않지만, 인증서를 생성하는 데에는 여전히 사용할 수 있습니다. 생성된 인증서는 지원되는 모든 P2S 클라이언트에 설치할 수 있습니다.
