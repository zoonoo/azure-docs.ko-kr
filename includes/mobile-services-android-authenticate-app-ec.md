...
1. **onCreate** 메서드에서 `MobileServiceClient` 개체를 인스턴스화하는 코드 다음에 다음 코드 줄을 추가합니다.
   
        authenticate();
   
    이 호출을 인증 프로세스를 시작합니다.
2. 다음과 유사하게 **onCreate** 메서드에서 `authenticate();` 뒤에 남은 코드를 새 **createTable** 메서드로 이동합니다.
...
3. **Run** 메뉴에서 **Run**을 클릭하여 앱을 시작하고 원하는 ID 공급자에 로그인합니다.
