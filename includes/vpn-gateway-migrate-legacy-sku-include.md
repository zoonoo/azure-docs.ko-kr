> [!NOTE]
> VPN Gateway 공용 IP 주소는 이전 SKU에서 새 SKU으로 마이그레이션할 때 변경됩니다.
> 

이전 SKU와 새 SKU 제품군 사이에서 직접 Azure VPN 게이트웨이의 크기를 조정할 수 없습니다. 이전 버전의 SKU를 사용하는 리소스 관리자 배포 모델의 VPN Gateway의 경우 새 SKU에 마이그레이션할 수 있습니다. 마이그레이션하려면 가상 네트워크에 대한 기존 VPN Gateway를 삭제한 다음 새 VPN Gateway를 만듭니다.
