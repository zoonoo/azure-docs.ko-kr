...
-   응용 프로그램이 Azure 클라우드 서비스에서 실행되는 경우 Azure 서비스 구성 시스템(`*.csdef`, `*.cscfg` 파일)을 사용하여 연결 문자열을 저장합니다.
...
5.  저장소 에뮬레이터(로컬 컴퓨터에서 시뮬레이트된 Windows Azure
    저장소)를 대상으로 지정할지 또는 클라우드의 저장소 계정을
    대상으로 지정할지 선택합니다. 이 가이드의 코드는 두 옵션 중
    어떤 옵션을 사용하든 작동합니다. 앞에서 Azure에 만든 저장소 계정에 큐 데이터를 저장하려면
    이 자습서의 이전 단계에서 복사한 **기본 액세스 키**
    값을 입력합니다.
...
이제 이 가이드의 방법 작업을 수행할 준비가 되었습니다.
  ...
  [Visual Studio에서 클라우드 서비스 역할에 대한 속성 선택 ]: ./media/storage-configure-connection-string/connection-string1.png
  [Visual Studio에서 클라우드 서비스 설정 추가]: ./media/storage-configure-connection-string/connection-string2.png
