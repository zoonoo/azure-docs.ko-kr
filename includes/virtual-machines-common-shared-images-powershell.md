...
[New-AzGalleryImage정의를](https://docs.microsoft.com/powershell/module/az.compute/new-azgalleryimageversion)사용하여 이미지 정의를 만듭니다. 이 예제에서 갤러리 이미지의 이름은 *myGalleryImage*입니다.
...
[New-AzGalleryImageVersion를](https://docs.microsoft.com/powershell/module/az.compute/new-azgalleryimageversion)사용하여 관리되는 이미지에서 이미지 버전을 만듭니다. 

이미지 버전에 허용되는 문자는 숫자 및 마침표입니다. 숫자는 32비트 정수 범위 내에 포함되어야 합니다. 형식 : *메이저 버전*. *마이너 버전*. *패치*.

이 예제에서 이미지 버전은 *1.0.0*이며, *미국 중서부* 및 *미국 중남부* 데이터 센터 둘 다에 복제됩니다. 복제를 위한 대상 영역을 선택할 때는 *원본* 영역을 복제 대상으로 포함해야 합니다.
...
>
> 이미지 버전을 만들 때 추가하여 `-StorageAccountType Standard_ZRS` 영역 중복 [저장소에](https://docs.microsoft.com/azure/storage/common/storage-redundancy-zrs) 이미지 버전을 저장할 수도 있습니다.
>


## <a name="share-the-gallery"></a>갤러리 공유

이미지 갤러리 수준에서 액세스를 공유하는 것이 좋습니다. 전자 메일 주소와 [Get-AzADUser](/powershell/module/az.resources/get-azaduser) cmdlet을 사용하여 사용자의 개체 ID를 받은 다음 [New-AzRoleAssignment를](/powershell/module/Az.Resources/New-AzRoleAssignment) 사용하여 갤러리에 대한 액세스 권한을 부여합니다. 이 alinne_montes@contoso.com 예제의 예제 전자 메일을 사용자 고유의 정보로 바꿉습니다.
