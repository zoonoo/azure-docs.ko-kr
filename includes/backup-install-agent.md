...
Azure 백업 자격 증명 모음을 만든 후에는 각 온-프레미스 서버(Windows Server, Windows 클라이언트 또는 Data Protection Manager 서버)에 Azure에 데이터 및 응용 프로그램을 백업할 수 있게 해주는 에이전트가 설치되어야 합니다.
...
9. **암호화 설정** 화면에서 암호를 생성하거나 암호를 제공합니다.(최소 16자) 암호를 안전한 위치에 저장해야 합니다.
