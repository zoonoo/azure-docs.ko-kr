...
---
1. 가상 네트워크 게이트웨이에 대한 페이지를 엽니다. 여러 가지 방법으로 이동할 수 있습니다. **VNet 이름 -> 개요 -> 연결된 디바이스 -> 게이트웨이 이름**으로 이동하여 게이트웨이로 이동할 수 있습니다.
2. 게이트웨이 페이지에서 **연결**을 클릭합니다. 연결 페이지의 맨 위에 있는 **+추가**를 클릭하여 **연결 추가** 페이지를 엽니다.
