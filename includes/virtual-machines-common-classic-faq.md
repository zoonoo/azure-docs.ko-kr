...
선호도 그룹은 초기 Azure 네트워크 디자인에서 VM간 네트워크 성능을 개선하고 영역에서 작은 집합의 하드웨어로 제한된 가상 네트워크(VNet)의 최초 릴리스를 지원하기 위해 원래 제공된 레거시 개념입니다. 지역 내에서 현재 Azure 네트워크는 선호도 그룹이 더 이상 필요하지 않도록 설계되었습니다. 또한 VNet은 지역 범위에 있기 때문에 VNet을 사용하는 경우 선호도 그룹이 필요하지 않습니다. 이러한 성능 향상으로 인해 일부 시나리오에서는 제한될 수 있으므로 고객에게 선호도 그룹을 사용하도록 권고하지 않습니다. 선호도 그룹을 사용하면 특정 하드웨어에 VM을 불필요하게 연결하게 되어 선택할 수 있는 VM 크기를 제한합니다. 선호도 그룹과 연결된 특정 하드웨어가 용량에 근접하는 경우 새 VM을 추가하려고 하면 용량 관련 오류가 발생할 수 있습니다.

Azure Resource Manager 배포 모델 및 포털에서 선호도 그룹 기능을 이미 지원하지 않습니다. 선호도 그룹을 만들고 클래식 포털에서 선호도 그룹에 고정된 저장소 리소스를 만들기 위한 지원을 하지 않고 있습니다. 선호도 그룹을 사용하는 기존 클라우드 서비스는 수정할 필요가 없습니다. 새 클라우드 서비스에서는 Azure 지원 전문가가 권장하지 않는 한 선호도 그룹을 사용하지 않도록 합니다.
