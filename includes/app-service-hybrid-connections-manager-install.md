...
4. **하이브리드 연결 속성** 블레이드가 열립니다. **온-프레미스 하이브리드 연결 관리자**에서 **수동으로 다운로드 및 구성**을 선택하여 다운로드된 HybridConnectionManager.msi 패키지를 저장하고 게이트웨이 연결 문자열을 복사합니다.
	
	![설치하려면 여기를 클릭](./media/app-service-hybrid-connections-manager-install/D05ClickToInstallHCM.png)
	
5. 관리자 명령 프롬프트에서 다음 명령을 입력하여 설치 관리자를 시작합니다.
