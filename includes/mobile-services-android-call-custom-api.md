...
5.  Package Explorer에서 *src* 폴더(`com.example.{프로젝트 이름}`)의 프로젝트 이름을 마우스 오른쪽 단추로 클릭하고 **New**, **Class**를 차례로 선택합니다. 대화 상자의 클래스 이름 필드에 **MarkAllResult**를 입력하고 OK를 선택한 후 결과 클래스 정의를 다음 코드로 바꿉니다.
...
6.  **ToDoActivity.java** 파일에서 **refreshItemsFromTable** 메서드를 찾아 코드의 첫 줄이 다음과 유사하게 시작하는지 확인합니다.

         mToDoTable.where().field("complete").eq(false)

    이 코드는 완료된 항목이 쿼리에서 반환되지 않도록 항목을 필터링합니다.

7.  **ToDoActivity.java** 파일에서 다음 메서드를 추가합니다.
