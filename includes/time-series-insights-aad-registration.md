...
1. [Azure portal](https://ms.portal.azure.com/)에서 **Azure Active Directory** > **앱 등록** > **새 등록**을 선택합니다.
   ...
   [![Azure 활성 디렉터리에서 새 응용 프로그램 등록](media/time-series-insights-aad-registration/active-directory-new-application-registration.png)](media/time-series-insights-aad-registration/active-directory-new-application-registration.png#lightbox)

    등록한 후에 앱이 여기 나열됩니다.

1. 애플리케이션 이름을 지정하고 **이 조직 디렉터리의 계정만**을 선택하여 API에 액세스할 수 있는 **지원되는 계정 유형**을 지정합니다. 인증된 후 사용자를 리디렉션할 유효한 URI를 선택한 다음, **등록**을 선택합니다.

   [![Azure 활성 디렉터리에서 응용 프로그램 만들기](media/time-series-insights-aad-registration/active-directory-registration.png)](media/time-series-insights-aad-registration/active-directory-registration.png#lightbox)
...
1. **인증** 블레이드에서는 중요한 인증 구성 설정을 지정합니다. 
    ...
    1. **앱이** **공용 클라이언트인지** 여부를 확인합니다. **No**

    1. 지원되는 계정 및 테넌트와 계정을 확인합니다.
...
1. 적절한 플랫폼을 선택한 후 사용자 인터페이스 오른쪽의 측면 패널에서 **리디렉션 URI** 및 **액세스 토큰**을 구성합니다.
    ...
    [![리디렉션 URIs 만들기](media/time-series-insights-aad-registration/active-directory-auth-redirect-uri.png)](media/time-series-insights-aad-registration/active-directory-auth-redirect-uri.png#lightbox)

    **구성**을 클릭한 다음, **저장**을 클릭합니다.

1. **인증서 & 비밀을** 선택한 다음 새 클라이언트 **암호를** 선택하여 클라이언트 앱에서 해당 ID를 증명하는 데 사용할 수 있는 응용 프로그램 암호를 만듭니다.

   [![새 클라이언트 보안 비밀 만들기](media/time-series-insights-aad-registration/active-directory-application-keys-save.png)](media/time-series-insights-aad-registration/active-directory-application-keys-save.png#lightbox)
...
1. Azure Active Directory 앱 Azure Time Series Insights에 연결합니다. **API 권한** > 선택**내 조직에서 사용하는**권한 > API**추가**. 

    [![API를 Azure Active Directory 앱과 연결](media/time-series-insights-aad-registration/active-directory-app-api-permission.png)](media/time-series-insights-aad-registration/active-directory-app-api-permission.png#lightbox)
...
1. 다음으로 앱에 필요한 API 사용 권한 종류를 지정합니다. 기본적으로 **위임된 사용 권한**이 강조 표시됩니다. 사용 권한 유형을 선택한 다음, **사용 권한 추가**를 선택합니다.
    ...
    [![앱에 필요한 API 권한 의 종류 지정](media/time-series-insights-aad-registration/active-directory-app-permission-grant.png)](media/time-series-insights-aad-registration/active-directory-app-permission-grant.png#lightbox)
