...
> [!NOTE]
> 웹앱을 삭제하고 다시 만들거나 앱 서비스 계획 모드가 **기본**, **공유** 또는 **표준**으로 설정된 이후 모드를 **무료**로 변경하면 웹앱의 IP 주소가 변경됩니다.
> 
> 
...
> [!NOTE]
> A 레코드는 고정 IP 주소에 매핑되므로 변경 내용을 웹앱의 IP 주소로 자동으로 확인할 수 없습니다. A 레코드에 사용할 IP 주소는 웹앱에 대한 사용자 지정 도메인 이름 설정을 구성할 때 제공됩니다. 그러나 웹앱을 삭제하고 다시 만들거나 앱 서비스 계획 모드를 다시 **무료**로 변경할 경우 이 값이 변경될 수 있습니다.
> 
> 
...
3. **사용자 지정 도메인** 블레이드에 가상 IP 주소가 표시됩니다. 이 정보는 DNS 레코드를 만들 때 사용되므로 저장합니다.
   ...
   > [!NOTE]
   > 사용자 지정 도메인 이름은 **무료** 웹앱과 함께 사용할 수 없으며, 꼭 앱 서비스 계획을 **공유**, **기본**, **표준**, **프리미엄** 계층으로 업그레이드 해야 합니다. 사용자 웹앱의 가격 책정 계층 변경을 포함한 앱 서비스 계획의 가격 책정 계층에 대한 자세한 정보는 [웹앱 확장하는 방법](../articles/app-service-web/web-sites-scale.md)을 참조하세요.
   > 
   > 
