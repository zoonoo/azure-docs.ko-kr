...
7.  **RefreshTodoItems** 메서드를 다음 코드로 한 번 더 업데이트합니다.
    ...
    > [WACOM.NOTE]이 자습서에서는 하드 코드된 페이징 값을 **Take** 및 **Skip** 메서드에 전달하여 간소화된 시나리오를 사용합니다. 실제 앱에서는 Pager 컨트롤이나 이와 비슷한 UI에서 위와 비슷한 쿼리를 사용하여 사용자가 이전 및 다음 페이지로 이동하도록 만들 수 있습니다. **IncludeTotalCount** 메서드를 호출하여 서버에서 사용 가능한 총 항목 수를 페이징 데이터와 함께 가져올 수도 있습니다.
