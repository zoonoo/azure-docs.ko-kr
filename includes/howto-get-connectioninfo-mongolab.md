MongoLab 데이터베이스를 프로비전하면 MongoLab에서 MongoDB의 표준 연결 문자열 형식으로 연결 URI를
Azure에 전송합니다. 이 값은 선택한 MongoDB 드라이버를 통해 MongoDB 연결을 시작하는 데 사용됩니다. 연결
문자열에 대한 자세한 내용은 mongodb.org의 [연결][1]을 참조하십시오.
