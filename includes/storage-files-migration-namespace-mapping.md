...
볼륨의 루트를 동기화 하는 것이 항상 가장 좋은 답은 아닙니다. 여러 위치를 동기화 하면 이점이 있습니다. 예를 들어 이렇게 하면 동기화 범위 당 항목 수를 줄일 수 있습니다. 공유 당 1억 항목 (파일 및 폴더)을 사용 하 여 Azure 파일 공유 및 Azure File Sync를 테스트 하는 동안 단일 공유에서 20 또는 3000만 이하의 수를 유지 하는 것이 가장 좋습니다. 적은 수의 항목을 사용 하 여 Azure File Sync를 설정 하는 것은 파일 동기화에는 유용 하지 않습니다. 항목 수가 적으면 다음과 같은 시나리오를 활용할 수 있습니다.

* 네임 스페이스를 사용 하 Azure File Sync 서버에 표시 하기 전에 클라우드 콘텐츠의 초기 검사가 더 빠르게 완료 될 수 있습니다.
...
> [!TIP]
> 보유 한 파일 및 폴더 수를 잘 모르겠으면 TreeSize tools in Software GmbH에서 확인할 수 있습니다.
...
동기화 범위 당 항목 수를 낮게 유지 하는 것이 좋습니다. 이는 폴더를 Azure 파일 공유에 매핑할 때 고려해 야 할 중요 한 요소입니다. 공유 당 1억 항목 (파일 및 폴더)을 사용 하 여 Azure 파일 공유 및 Azure File Sync를 테스트 하는 동안 단일 공유에서 20 또는 3000만 이하의 수를 유지 하는 것이 가장 좋습니다. 이러한 숫자를 초과 하기 시작 하는 경우 네임 스페이스를 여러 공유로 분할 합니다. 이러한 수치 미만으로 유지 한다면 여러 온-프레미스 공유를 동일한 Azure 파일 공유로 계속 그룹화 할 수 있습니다. 그러면 확장할 수 있는 공간이 제공 됩니다.
