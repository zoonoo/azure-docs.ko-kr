...
ms.locfileid: "66388757"
---
## <a name="business-disaster-recovery"></a>비즈니스 재해 복구

이 섹션에서는 앱 및 재해가 발생 한 경우에 실행 되는 서비스를 유지 하는 Azure Time Series Insights의 기능을 설명 합니다. 이 개념을 비즈니스 재해 복구 라고 합니다.

### <a name="high-availability"></a>고가용성

Azure 서비스로, Time Series Insights는 Azure 지역 수준에서 중복성을 사용 하 여 특정 고가용성 기능을 제공 합니다. 예를 들어, Microsoft Azure는 Azure의 지역 간 가용성 기능을 통해 재해 복구 기능을 지원합니다.

하 고 Azure를 통해 제공 하는 추가 고가용성 기능 또한 Time Series Insights 인스턴스에 사용할 수를 포함 합니다.
...
* **사이트 복구**: 통해 사용할 수 있는 기능 [Azure Site Recovery](https://docs.microsoft.com/azure/site-recovery/)합니다.

장치 및 사용자에 대 한 전역, 지역 간 고가용성을 제공 하려면 이러한 Azure 기능을 사용 하도록 설정 해야 합니다.

> [!NOTE]
> Azure 지역 간 가용성을 사용 하도록를 구성 된 경우 Azure Time Series Insights 내에서 필요한 추가 지역 간 가용성 구성은 하지 않습니다.

### <a name="iot-and-event-hubs"></a>IoT 및 이벤트 허브

일부 Azure IoT 서비스에는 기본 제공 비즈니스 재해 복구 기능이 포함 됩니다.

* [Azure IoT Hub 고가용성 재해 복구](https://docs.microsoft.com/azure/iot-hub/iot-hub-ha-dr), 지역 간 중복성을 포함 하는 합니다.
* [Azure Event Hubs 정책](https://docs.microsoft.com/azure/event-hubs/event-hubs-geo-dr)합니다.
* [Azure Storage 중복성](https://docs.microsoft.com/azure/storage/common/storage-redundancy)합니다.
...
Time Series Insights 데이터, 앱 및 중단 하는 경우에 실행 되는 서비스를 유지 하는 방법은 여러 가지가 있습니다. Azure Time Series 환경의 전체 백업 복사본을 필수 인지 확인할 수 있습니다.

* Time Series Insights 관련 장애 조치 인스턴스로 트래픽 및 데이터를 리디렉션하는 데 합니다.
* 감사 및 데이터 보존 목적으로 합니다.

일반적으로 Time Series Insights 환경을 복제 하는 가장 좋은 방법은 백업 Azure 지역에에서 두 번째 Time Series Insights 환경을 만드는 방법은입니다. 이벤트 주 이벤트 원본에서이 보조 환경에도 전송 됩니다. 두 번째 전용된 소비자 그룹을 사용 하 고 해당 원본 비즈니스 재해 복구, 앞서 제공 된 지침에 따라 있는지를 확인 합니다.
...
1. 해당 이벤트 원본을 새 환경에 연결합니다. 두 번째 전용된 소비자 그룹을 지정 해야 합니다.
