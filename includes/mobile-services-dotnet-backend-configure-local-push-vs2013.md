...
1. 모바일 서비스 프로젝트 폴더의 readme.html 파일을 엽니다. 

	아직 열지 않은 경우 **푸시 설정이 거의 완료되었습니다.** 페이지가 표시됩니다. **3단계: 웹 구성 수정** 섹션에 알림 허브 연결 정보가 포함되어 있습니다.

2. Visual Studio의 모바일 서비스 프로젝트에서 서비스의 Web.config 파일을 열고 **connectionStrings**에 **푸시 설정이 거의 완료되었습니다.** 페이지의 **MS\_NotificationHubConnectionString** 연결 문자열을 추가합니다.

3. **appSettings**에서 **MS\_NotificationHubName** 앱 설정의 값을 **푸시 설정이 거의 완료되었습니다.** 페이지의 알림 허브 이름으로 바꿉니다.
