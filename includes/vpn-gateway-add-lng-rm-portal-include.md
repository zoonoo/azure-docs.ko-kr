...
2. 검색 상자에 **로컬 네트워크 게이트웨이**를 입력한 후 **Enter**를 눌러서 검색합니다. 그러면 결과 목록이 반환됩니다. **로컬 네트워크 게이트웨이**를 클릭한 다음, **만들기** 단추를 클릭하여 **로컬 네트워크 게이트웨이 만들기** 페이지를 엽니다.
   ...
   ![로컬 네트워크 게이트웨이 만들기](./media/vpn-gateway-add-lng-rm-portal-include/local-network-gateway.png "로컬 네트워크 게이트웨이 만들기")
...
4. 값 지정을 완료 한 후 **만들기** 단추를 클릭 하 여 게이트웨이를 만듭니다.
