...
2. [토큰 기반 인증](../articles/notification-hubs/notification-hubs-push-notification-http2-token-authentication.md)에 사용할 수 있는 **.p8** 을 만듭니다(*보다 새로운 접근 방법*).

보다 새로운 접근 방식에는 [APNS에 대한 토큰 기반(HTTP/2) 인증](../articles/notification-hubs/notification-hubs-push-notification-http2-token-authentication.md)에 설명된 대로 인증서를 사용하는 것에 비해 여러 가지 이점이 있습니다. 그러나 두 접근 방식에 대해 단계가 필요했습니다. 
