...
관리 포털에서 가상 컴퓨터를 빨리 만들려면 **Quick Create** 방법을 사용합니다. 이 컴퓨터를 만드는 경우 하나의 대화
상자를 사용하여 구성 세부 정보를 제공합니다.

**참고**: 이 문서에서는 가상 네트워크에 연결되지 않은 가상 컴퓨터를 만듭니다. 가상 컴퓨터에서 가상 네트워크를 사용하려는
경우 **From Gallery** 방법을 대신 사용하고 가상 컴퓨터를 만들 때 가상 네트워크를 지정합니다. 가상 네트워크에 대한
자세한 내용은 [Azure 가상 네트워크 개요][1]를 참조하십시오.
