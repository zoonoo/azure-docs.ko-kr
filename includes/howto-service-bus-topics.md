...
토픽 구독은 토픽에 전송된 메시지의 복사본을 받는 가상 큐와 유사합니다. 또는 구독당 기준으로 항목에 대한 필터 규칙을 등록하여 항목 구독별로 수신할 항목 메시지를 필터링 또는 제한할 수 있습니다.

서비스 버스 항목 및 구독을 사용하면 매우 많은 수의 메시지를 여러 사용자와 응용 프로그램에 조정 및 처리할 수 있습니다.

## 네임스페이스 만들기

Azure에서 서비스 버스 항목 및 구독 사용을 시작하려면 가장 우선 *서비스 네임스페이스*를 만들어야 합니다. 네임스페이스는 응용 프로그램 내에서 서비스 버스 리소스의 주소를 지정하기 위한 범위 컨테이너를 제공합니다.
...
2.  표시된 목록에서 방금 만든 네임스페이스를 선택합니다. ![][3]
