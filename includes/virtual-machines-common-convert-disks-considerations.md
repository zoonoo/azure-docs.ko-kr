* 이전에 Azure Storage Service 암호화를 사용하여 암호화된 저장소 계정에 관리되지 않는 디스크가 있는 경우 이를 관리되는 디스크로 변환할 수 없습니다. 관리되는 디스크에서 이러한 VHD를 복사하고 사용하는 단계는 [이 문서의 뒷부분](#managed-disks-and-azure-storage-service-encryption)를 참조하세요.

* 변환은 기존 유지 관리 기간 동안 VM의 마이그레이션을 예약하도록 VM의 재시작이 필요합니다. 

* 변환하면 되돌릴 수 없습니다. 

* 변환을 테스트해야 합니다. 프로덕션에서 마이그레이션을 수행 하기 전에 테스트 가상 컴퓨터를 마이그레이션합니다.
