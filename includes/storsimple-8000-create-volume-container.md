...
3. **볼륨 컨테이너 추가** 블레이드에서:
   ...
   4. 8~32자 길이의 **클라우드 스토리지 암호화 키** 를 제공 및 확인합니다. 이 키는 암호화된 데이터에 액세스하는 디바이스가 사용합니다.
   5. **Storage 계정** 을 선택하여 이 볼륨 컨테이너와 연결합니다. 서비스를 만들 때 생성되는 기본 계정 또는 기존 스토리지 계정을 선택할 수 있습니다. **새로 추가** 옵션을 사용하여 이 서비스 구독에 연결되지 않은 스토리지 계정을 지정할 수도 있습니다.
