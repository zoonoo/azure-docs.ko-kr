...
- **응답 서버 배포**: API 요청을 처리하는 응용 프로그램 서버입니다. 가능한 값은 Bing.com(데스크톱 및 랩톱 장치에서 제공된 트래픽) 및 Bing.com-mobile(모바일 장치에서 제공된 트래픽)입니다. 서버는 요청의 사용자 에이전트 헤더 값을 사용하여 결정됩니다. 요청에 사용자 에이전트 헤더가 포함되지 않을 경우 Bing은 다른 신호에서 서버를 파생하려고 시도합니다.  
