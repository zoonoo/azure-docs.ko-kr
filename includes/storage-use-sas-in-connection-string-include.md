저장소 계정에서 리소스에 대한 액세스를 허용하는 공유 액세스 서명(SAS) URL을 보유하는 경우 연결 문자열에 SAS를 사용할 수 있습니다. URI에서 SAS가 요청을 인증하는 데 필요한 정보를 포함하므로 SAS URI는 프로토콜, 서비스 끝점 및 필요한 자격 증명을 제공하여 리소스에 액세스합니다.
