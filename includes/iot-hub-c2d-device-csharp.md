...
1. Visual Studio에서 **SimulatedDevice** 프로젝트의 **Program** 클래스에 다음 메서드를 추가합니다.
    ...
    `CompleteAsync()`에 대한 호출을 통해 메시지를 성공적으로 처리했고 장치 큐에서 안전하게 제거할 수 있다는 것을 IoT Hub에 알립니다. 장치 앱이 메시지 처리를 완료하지 못하도록 하는 경우가 발생하면 IoT Hub는 해당 메시지를 다시 배달하지 않습니다. 이때 같은 메시지를 여러 번 받는 경우 동일한 결과를 생성하도록 장치 앱의 메시지 처리 논리가 *idempotent*여야 합니다. 또한 응용 프로그램은 메시지를 일시적으로 `Abandon`할 수 있습니다. 그러면 IoT Hub가 해당 메시지를 향후에 사용할 수 있도록 큐에 보존합니다. 그렇지 않고 메시지를 `Reject`하면 해당 메시지가 큐에서 영구적으로 제거됩니다. 클라우드-장치 메시지 수명 주기에 대한 자세한 내용은 [IoT Hub 개발자 가이드][IoT Hub Developer Guide - C2D]를 참조하세요.
