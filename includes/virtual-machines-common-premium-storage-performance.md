...
시작하기 전에 Premium Storage를 처음 사용하는 경우 먼저 [Premium Storage: Azure 가상 컴퓨터 워크로드를 위한 고성능 저장소](../articles/virtual-machines/windows/premium-storage.md) 및 [Azure Storage 확장성 및 성능 목표](../articles/storage/common/storage-scalability-targets.md) 문서를 읽어 보세요.
...
Azure Premium Storage는 현재 일곱 가지 디스크 크기를 제공합니다. 각 디스크 크기는 IOPS, 대역폭 및 저장소에 대한 다른 규모 한도를 가집니다. 응용 프로그램 요구 사항 및 높은 확장성의 VM 크기에 따라 올바른 Premium Storage 디스크 크기를 선택합니다. 아래 표에서 일곱 가지 디스크 크기와 해당 기능을 보여 줍니다. P4 및 P6 크기는 현재 Managed Disks에 대해서만 지원됩니다.
...
Azure Premium Storage를 활용하는 높은 확장성의 VM에는 BlobCache 라는 다중 계층 캐싱 기술이 있습니다. BlobCache는 캐싱에 대해 가상 컴퓨터 RAM 및 로컬 SSD의 조합을 사용합니다. 이 캐시는 Premium Storage 영구 디스크 및 VM 로컬 디스크에 사용할 수 있습니다. 기본적으로 이 캐시 설정은 OS 디스크에 대해 읽기/쓰기로 Premium Storage에서 호스팅되는 데이터 디스크에 대해 읽기 전용으로 설정됩니다. Premium Storage 디스크에서 디스크 캐싱을 사용하면 높은 확장성의 VM은 기본 디스크 성능을 초과하는 매우 높은 수준의 성능을 얻을 수 있습니다.
...
2. Premium Storage는 디스크 IOPS 및 처리량으로 캐시에서 제공된 읽기를 계산하지 않습니다. 따라서 응용 프로그램은 더 높은 총 IOPS 및 처리량을 달성할 수 있습니다.
