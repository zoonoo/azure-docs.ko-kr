...
> [WACOM.NOTE] VM이 같은 클라우드 서비스에 있으면 VM의 부하를 분산하고 가용성을 관리할 수도 있습니다. 두 작업에 모두 추가적인 단계가 필요합니다. 자세한 내용은 [가상 컴퓨터 부하 분산](../../articles/load-balance-virtual-machines/) 및 [가상 컴퓨터의 가용성 관리](../../articles/manage-availability-virtual-machines/)를 참조하세요. 

먼저 새 클라우드 서비스를 사용하여 가상 컴퓨터를 만들어야 합니다. 그리고 같은 클라우드 서비스에 추가로 가상 컴퓨터를 만듭니다. 그러면 가상 컴퓨터가 '연결'됩니다. 

1. 먼저 [사용자 지정 가상 컴퓨터를 만드는 방법](../../articles/virtual-machines-create-custom/)의 단계에 따라 가상 컴퓨터를 만듭니다.
