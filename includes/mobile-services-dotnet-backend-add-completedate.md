이 섹션에서는 이름이 **CompleteDate**인 새 타임스탬프 필드를 추가하여 데이터베이스의 모델을 수정합니다. 이 필드는 todo 항목이 완료된 마지막 시간을 기록합니다. Entity Framework는 [DropCreateDatabaseIfModelChanges][DropCreateDatabaseIfModelChanges]에서 파생된 기본 데이터베이스 이니셜라이저 클래스를 사용하는 모델 변경에 따라 데이터베이스를 업데이트합니다.
