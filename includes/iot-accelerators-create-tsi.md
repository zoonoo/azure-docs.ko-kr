...
1. IoT 허브는 장치 시뮬레이션 솔루션 가속기를 배포할 때 생성되는 몇 가지 리소스 중 하나입니다. 다음 명령을 실행하고 IoT 허브의 이름을 찾습니다. 솔루션 가속기의 이름을 사용해야 합니다.
...
1. 솔루션 가속기와 동일한 리소스 그룹에 Time Series Insights 환경을 만들려면 다음 표의 값을 사용합니다.
    ...
    | 위치 | 이 예제에서는 **미국 동부**를 사용합니다. 장치 시뮬레이션 가속기와 동일한 지역에 환경을 만듭니다. |
    | SKU |**S1** |
    | 용량 | **1** |
...
1. IoT Hub를 새 이벤트 원본으로 구성하려면 다음 표에 있는 값을 사용합니다.
    ...
    | IoT Hub 이름 | **contoso-simulation7d894** 장치 시뮬레이션 솔루션 가속기에서 IoT 허브의 이름을 사용합니다. |
