...
이 문서에서는 [MongoLab][MongoLab]을 사용하여 [Azure 스토어][Azure 스토어]에서 MongoDB 데이터베이스를 만드는 방법을 보여 줍니다. [MongoLab][MongoLab]은 Azure 데이터 센터에서 MongoDB 데이터베이스를 실행 및 관리하고 모든 응용 프로그램에서 연결할 수 있게 해 주는 MasS(MongoDB-as-a-Service) 공급자입니다.

[Azure 스토어][Azure 스토어]에서 MongoDB 데이터베이스를 만들려면 다음을 수행합니다.
