...
다른 권한을 부여하려면 필요에 따라 [az ad sp create-for-rbac][/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac] 명령에서 `--role` 값을 수정하면 됩니다. 역할의 전체 목록은 [ACR 역할 및 권한](https://github.com/Azure/acr/blob/master/docs/roles-and-permissions.md)을 참조하세요.
...
다음 스크립트는 [az role assignment create][/cli/azure/role/assignment?view=azure-cli-latest#az-role-assignment-create] 명령을 사용하여 `SERVICE_PRINCIPAL_ID` 변수에 지정된 서비스 주체에게 *끌어오기* 권한을 부여합니다. 다른 수준의 액세스 권한을 부여하려면 `--role` 값을 조정합니다.
