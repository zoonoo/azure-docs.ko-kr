...
---
공유 이미지 갤러리, 이미지 정의 및 이미지 버전에 대한 작업을 수행하는 동안 문제가 발생하는 경우 실패한 명령을 디버그 모드에서 다시 실행합니다. 디버그 모드는 CLI가 있는 **-디버그** 스위치와 PowerShell이 있는 **-Debug** 스위치를 전달하여 활성화됩니다. 오류를 찾은 후에는 이 문서의 지침에 따라 오류를 해결합니다.
...
갤러리 이름에 허용되는 문자는 대문자 또는 소문자, 숫자, 점 및 마침표입니다. 갤러리 이름에는 대시가 포함될 수 없습니다. 갤러리 이름을 변경하고 다시 시도하세요. 
...
RBAC(역할 [기반 액세스 제어)를](https://docs.microsoft.com/azure/role-based-access-control/rbac-and-directory-admin-roles) 사용하여 구독 간에 공유 이미지 갤러리, 이미지 정의 및 이미지 버전 리소스공유를 사용할 수 있습니다. 
