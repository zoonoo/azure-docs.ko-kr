...
5. **패키지 설정**에서, **USERDATABASE**를 선택하고, 기존 데이터베이스를 선택하거나 새 데이터베이스를 만들 수 있습니다. 새 데이터베이스를 만들려면, 새 **데이터베이스**의 이름을 입력하고, 새 **서버**를 만들고, 해당 서버의 이름을 입력한 다음 새 SQL 데이터베이스 서버의 관리자 로그인 이름인 **로그인 이름**을 선택하고, 암호를 입력하고 확인하고 확인 단추를 클릭하여 프로세스를 완료합니다. 기존 데이터베이스를 선택하면 **서버 관리자 암호**를 제공해야 합니다.
