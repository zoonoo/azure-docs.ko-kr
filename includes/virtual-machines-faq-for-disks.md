...
API 버전 2019-07-01 이상을 사용 하 여 만든 모든 관리 디스크는 공유 디스크를 사용 하도록 설정할 수 있습니다. 이렇게 하려면 연결 된 모든 Vm에서 디스크를 분리 해야 합니다. 그런 다음 `maxShares` 디스크의 속성을 편집 합니다.
...
**관리 디스크를 만들 때 기본적으로 서버 쪽 암호화를 사용 합니까?**

예. Managed Disks는 플랫폼 관리 키를 사용 하 여 서버 쪽 암호화로 암호화 됩니다. 
...
플랫폼 관리 키는 Microsoft에서 관리 합니다. Azure Key Vault에 저장 된 고유한 키를 사용 하 고 관리할 수도 있습니다. 

**관리 디스크에 대 한 서버 쪽 암호화를 사용 하지 않도록 설정할 수 있나요?**

아니요.

**서버 쪽 암호화는 특정 지역 에서만 사용할 수 있나요?**

아니요. 플랫폼과 고객이 관리 하는 키를 모두 사용 하는 서버 쪽 암호화는 Managed Disks을 사용할 수 있는 모든 지역에서 사용할 수 있습니다. 

**온-프레미스에서 Azure로, azure에서 azure로 재해 복구 시나리오를 위해 고객이 관리 하는 키를 사용 하는 서버 쪽 암호화를 지원할 Azure Site Recovery 있나요?**

예. 

**Azure Backup 서비스를 사용 하 여 고객이 관리 하는 키를 사용 하 여 서버 쪽 암호화로 암호화 된 Managed Disks를 백업할 수 있나요?**

예.

**관리되는 스냅샷 및 이미지가 암호화되나요?**

예. 모든 관리 되는 스냅숏 및 이미지는 자동으로 암호화 됩니다. 
