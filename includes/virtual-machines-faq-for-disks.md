...
P15 이상인 프리미엄 SSD만 공유 디스크를 지원합니다.

**보유한 기존 프리미엄 SSD에서 공유 디스크를 사용할 수 있나요?**

API 버전 2019-07-01 이상으로 만든 모든 관리 디스크는 공유 디스크를 사용할 수 있습니다. 이렇게 하려면 연결된 모든 VM에서 디스크를 분리해야 합니다. 그런 다음, 디스크의 `maxShares` 속성을 편집합니다.
...
디스크 제한 외에도, VM 수준에서 적용되는 IO 제한이 있습니다. 사용하는 VM 크기가 디스크에서 구성된 수준을 지원할 수 있는지 확인하세요. VM이 적용하는 IO 제한에 대한 자세한 내용은 [Azure에서 Windows 가상 머신에 대한 크기](../articles/virtual-machines/windows/sizes.md)를 참조하세요.

**울트라 디스크에서 캐싱 수준을 사용할 수 있나요?**
아니요, 울트라 디스크는 다른 디스크 유형에서 지원되는 다른 캐싱 방법을 지원하지 않습니다. 디스크 캐싱을 [없음]으로 설정하세요.
