...
**기존 관리 디스크의 크기를 4TiB 미만의 크기에서 새로 도입된 디스크 크기인 최대 32TiB로 조정할 수 있나요?**

새로운 관리 디스크 크기인 8TiB, 16TiB 및 32TiB는 현재 미리 보기로 제공됩니다. 기존 디스크 크기를 새 디스크 크기로 조정하는 것은 아직 지원되지 않습니다.

**Azure Backup 및 Azure Site Recovery 서비스에서 지원하는 최대 디스크 크기는 어떻게 되나요?**

Azure Backup 및 Azure Site Recovery 서비스에서 지원하는 최대 디스크 크기는 4TiB입니다.

**최적화된 디스크 IOPS 및 대역폭을 달성하기 위한 표준 SSD 및 표준 HDD 디스크용 대형 디스크 크기(>4TiB)로 권장되는 VM 크기는 어떻게 되나요?**

500IOPS 및 60MiB/초 초과 표준 SSD 및 표준 HDD 대용량 디스크 크기(>4TB)의 디스크 처리량을 달성하려면 다음 VM 크기 중 하나를 사용하여 성능을 최적화해야 합니다. B-시리즈, DSv2 시리즈, Dsv3 시리즈, ESv3 시리즈, Fs 시리즈 Fsv2 시리즈, M 시리즈, GS 시리즈, NCv2 시리즈, NCv3 시리즈 또는 Ls 시리즈 VM.

**4TiB보다 큰 관리 디스크 크기가 지원되는 지역은 어디인가요?**

미리 보기 4 TiB 이외의 관리 되는 디스크 크기를 정부, 중국 및 독일을 제외한 모든 Azure 프로덕션 지역에서 지원 됩니다. 

**새로운 디스크 크기에서 호스트 캐싱 사용 설정을 지원하나요?**

4TiB 미만의 디스크 크기에서 읽기 전용 및 읽기/쓰기 호스트 캐싱을 지원합니다. 4TiB 초과 디스크 크기에 대해서는 없음 이외의 캐싱 옵션 설정은 지원하지 않습니다. VM에 캐시된 데이터를 사용하여 성능 향상을 확인할 수 있을 수 있는 더 작은 디스크 크기에 대한 캐싱을 활용하는 것이 좋습니다.
