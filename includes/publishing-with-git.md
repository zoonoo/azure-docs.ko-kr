...
> [!NOTE]
> 이 문서에 설명되어 있는 대부분의 Git 명령은 [Mac 및 Linux용 Azure 명령줄 도구](/develop/nodejs/how-to-guides/command-line-tools/)를 사용하여 웹 앱을 만드는 경우 자동으로 수행됩니다.
> 
> 
...
3. 명령줄에서 Git 리포지토리의 루트에 있는지 확인합니다. 그런 후 다음 명령을 사용하여 리포지토리에 **index.html** 파일을 추가합니다.
   ...
   > [!NOTE]
   > 명령 다음에 -help 또는 --help를 입력하면 git 명령에 대한 도움말을 찾을 수 있습니다. 예를 들어 add 명령에 대한 매개 변수 옵션의 경우 명령줄 도움말은 'git add -help'를 입력하고, 자세한 도움말은 'git add --help'를 입력합니다.
   > 
   > 
...
1. [Azure 포털]에 로그인합니다.
2. 웹앱의 블레이드에서 **설정 > 지속적인 배포**를 클릭합니다. **소스 선택**을 클릭한 다음 **로컬 Git 리포지토리**를 클릭하고 **확인**을 클릭합니다.
   
    ![로컬 Git 리포지토리](./media/publishing-with-git/azure1-local-git.png)
3. Azure에서 리포지토리를 처음 설정하는 경우 로그인 자격 증명을 만들어야 합니다. 이를 사용하여 Azure 리포지토리에 로그인하고 로컬 Git 리포지토리에서 변경 내용을 푸시합니다. 웹앱의 블레이드에서 **설정> 배포 자격 증명 설정**을 클릭한 다음 배포 사용자 이름 및 암호를 구성합니다. 완료되면 **확인**을 클릭합니다.
...
1. 웹앱의 블레이드에서 **Git URL**에 대해 **설정 > 속성**을 클릭합니다.
   
    ![](./media/publishing-with-git/azure3-repo-details.png)
   
    **Git URL**은 로컬 리포지토리로부터 배포할 원격 참조입니다. 다음 단계에서 이 URL을 사용합니다.
2. 앞서 만든 index.html 파일을 포함하는 로컬 Git 리포지토리의 루트에 있는지 명령줄을 사용하여 확인합니다.
3. `git remote`를 사용하여 1단계의 **Git URL**에 나열된 원격 참조를 추가합니다. 명령은 다음과 유사합니다.
   ...
   > 
   > 
4. 명령줄에서 다음을 사용하여 로컬 리포지토리에서 'azure' 원격으로 현재 리포지토리 내용을 푸시합니다.
   ...
   > [!NOTE]
   > 웹 앱용으로 만들어진 리포지토리는 대상인 리포지토리의 <strong>마스터</strong> 분기에 대한 푸시 요청이 필요하며, 이는 웹 앱의 콘텐츠로 사용됩니다.
   > 
   > 
5. Azure 포털에서 웹 앱의 블레이드로 돌아갑니다. **찾을 수 없는 배포**는 최신 푸시의 로그 항목을 포함한 **활성 배포**로 변경되어야 합니다.
   
    ![](./media/publishing-with-git/azure4-deployed.png)
6. 웹앱 블레이드 맨 위에 있는 **URL** 아래의 링크를 클릭하여 **index.html**이 배포되었는지 확인합니다. 'Hello Git!'가 포함된 페이지가 나타납니다.
   
    !['Hello Git!'가 포함된 웹 페이지][hello-git]
7. 텍스트 편집기를 사용하여 'Yay!'를 포함하도록 **index.html** 파일을 변경한 후 저장합니다.
8. 명령줄에서 다음 명령을 사용하여 변경 내용을 추가(**add**)하고 커밋(**commit**)한 다음 원격 리포지토리에 그 변경 내용을 푸시합니다(**push**).
...
2. 포털에 있는 웹앱의 블레이드에서 **설정 > 지속적인 업데이트**를 클릭합니다. **소스 선택**을 클릭한 다음 예를 들어 **GitHub**를 클릭합니다.
   
    ![](./media/publishing-with-git/azure6-setup-github.png)
3. **연속 배포** 블레이드에서 **권한 부여**를 클릭한 다음 **권한 부여**를 클릭합니다. Azure 포털은 권한 부여 프로세스를 완료하려면 리포지토리 사이트로 리디렉션합니다.
...
1. 이때 선택한 리포지토리에서 웹 앱으로 프로젝트가 배포됩니다. 웹앱 상태가 활성인지를 확인하려면 포털 위쪽에 있는 **URL**을 클릭합니다. 브라우저가 웹 앱으로 이동해야 합니다.
2. 사용자가 선택한 리포지토리에서 지속적으로 배포되는지 확인하려면, 리포지토리에 대한 변경을 푸시합니다. 리포지토리에 푸시한 직후 변경 내용이 반영되도록 웹 앱을 업데이트해야 합니다. 웹앱의 **배포** 블레이드에서 업데이트를 가져왔는지 확인할 수 있습니다.
...
* 리포지토리 루트에서 `.gitignore` 파일을 추가한 후 리포지토리에서 제외할 모든 파일 및 폴더(예: `Obj`, `Bin` 및 `packages` 폴더)를 지정합니다(서식 정보는 [gitignore 설명서](http://git-scm.com/docs/gitignore) 참조). 예:
  ...
  > [!NOTE]
  > GitHub를 사용하는 경우 리포지토리를 만들 때 Visual Studio 관련 .gitignore 파일을 선택적으로 생성할 수 있습니다. 여기에는 모든 공통 임시 파일, 빌드 결과 등이 포함됩니다. 그런 다음 특정 요구에 맞게 사용자 지정할 수 있습니다.
  > 
  > 
...
## 지속적 배포 사용 안 함
**배포** 블레이드에서 지속적인 배포를 사용하지 않도록 설정할 수 있습니다. 웹앱의 블레이드에서 **설정 > 지속적인 배포**를 클릭합니다. 그런 다음 **연결 끊기**를 클릭합니다.

![git-DisconnectFromGitHub](./media/publishing-with-git/azure5-disconnect.png)

확인 메시지에서 **예**를 누른 후, 다른 소스로부터 게시를 설정하려면 웹앱의 블레이드로 돌아가 **설정 > 지속적인 배포**를 클릭합니다.
