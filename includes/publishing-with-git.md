...
> [AZURE.NOTE] 이 문서에 설명되어 있는 대부분의 Git 명령은 [Mac 및 Linux용 Azure 명령줄 도구를 사용하여 웹 앱을 만드는 경우 자동으로 수행됩니다.](/develop/nodejs/how-to-guides/command-line-tools/).

<h2><a id="Step2"></a>Git 설치</h2>

Git를 설치하는 데 필요한 단계는 운영 체제마다 다릅니다. 운영 체제별 배포 및 설치 지침은 [Git 설치]를 참조하세요.
...
1. Git 리포지토리와 웹 앱 파일을 포함할 MyGit
2. 
3. 
4. 
5. 
6. sitory라는 이름의 디렉터리를 만듭니다.
...
웹 앱에서 다양한 프로그래밍 언어로 만들어진 응용 프로그램을 지원합니다. 이 예에서는 고정 .html 파일을 사용합니다. 다른 프로그래밍 언어로 Azure에 웹 앱을 게시하는 방법에 대한 내용은 [Azure 개발자 센터](영문)을 참조하세요.
...
4. 그러고 나서 다음 명령을 사용하여 리포지토리에 변경 내용을 커밋합니다.
...
<h2><a id="Step4"></a>웹 앱 리포지토리 사용</h2>
...
2. 웹 앱 만들기 프로세스가 완료될 때까지 대기하고 웹 앱 블레이드에서 웹 앱을 선택합니다.
	...
	![선택한 웹 앱을 표시하는 이미지](./media/publishing-with-git/azure1-local-git.png)
...
1. 웹 앱의 블레이드의 배포 섹션에서 **찾을 수 없는 배포**를 클릭합니다.
	...
	**Git URL**는 로컬 리포지토리로(부터) 배포할 원격 참조입니다.
...
GitHub, CodePlex 또는 BitBucket에서 파일을 배포하려면 로컬 프로젝트를 이러한 서비스 중 하나에 게시해야 합니다. 이러한 서비스에 프로젝트를 게시하는 방법에 대한 자세한 내용은 [리포지토리 만들기(GitHub)], [CodePlex에서 Git 사용], [Repo 만들기(BitBucket)], [드롭 상자를 사용하여 Git 리포지토리 공유] 또는 [빠른 시작 - Mercurial]을 참조하세요.
...
7. 이때 선택한 리포지토리에서 웹 앱으로 프로젝트가 배포됩니다. 웹 앱 상태가 활성인지를 확인하려면 포털 위쪽에 있는 **URL**을 클릭합니다. 브라우저가 웹 앱으로 이동해야 합니다.

8. 사용자가 선택한 리포지토리에서 지속적으로 배포되는지 확인하려면, 리포지토리에 대한 변경을 푸시합니다. 리포지토리에 푸시한 직후 변경 내용이 반영되도록 웹 앱을 업데이트해야 합니다. 웹 앱의 **배치** 블레이드에서 업데이트를 가져왔는지 확인할 수 있습니다.
...
Visual Studio 솔루션을 웹 앱에 푸시하는 단계는 [이전 섹션]에서와 동일합니다.(#Step7)에 나온 것과 같습니다. 단, 솔루션 및 리포지토리를 다음과 같이 구성하는 경우에 해당합니다.
...
**배포** 블레이드에서 지속적인 배포를 사용하지 않도록 설정할 수 있습니다. 웹 앱 블레이드의 **배포** 섹션에서 활성 배포를 클릭합니다. 그런 다음 **연결 끊기**를 클릭합니다.
...
**해결 방법**: Azure 포털에서 웹 앱을 시작합니다. 웹 앱이 실행되지 않으면 Git 배포가 작동하지 않습니다. 
...
[Mac 및 Linux에서 Azure 명령줄 도구를 사용하는 방법]: ../articles/xplat-cli.md
...
[CodePlex에서 Git 사용]: http://codeplex.codeplex.com/wikipage?title=Using%20Git%20with%20CodePlex&referringTitle=Source%20control%20clients&ProjectName=codeplex
