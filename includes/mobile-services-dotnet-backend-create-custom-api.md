...
2.  CompleteAllController.cs에서 네임스페이스에 다음 클래스 정의를 추가합니다. 이 클래스는 클라이언트로 보내는 응답을 래핑합니다.
...
3.  새 컨트롤러에 다음 코드를 추가합니다.
    ...
    이 코드에서는 **TodoItems** 테이블에 직접 액세스하여 모든 항목에 완료 플래그를 설정하는 데 [Database 클래스][Database 클래스]를 사용합니다. 이 메서드는 POST 요청을 지원하며, 변경된 행의 수가 정수 값으로 클라이언트에 반환됩니다.
...
다음 과정에서는 새 단추와 새 사용자 지정 API를 비동기적으로 호출하는 코드를 추가하는 퀵 스타트 앱을 수정합니다.
  ...
  [Database 클래스]: http://msdn.microsoft.com/ko-kr/library/system.data.entity.database.aspx
