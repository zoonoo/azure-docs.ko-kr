...
3. ToDoActivity.java 파일에 `cacheUserToken` 메서드에 대한 다음 정의를 추가합니다.
    ...
    이 메서드는 비공개로 표시된 기본 설정 파일에 사용자 ID와 토큰을 저장합니다. 이렇게 하면 디바이스의 다른 앱이 토큰에 액세스할 수 없도록 캐시 액세스가 보호됩니다. 기본 설정이 앱에 대해 샌드박스됩니다. 그러나 다른 사람이 디바이스에 액세스하게 되면 다른 수단을 통해 토큰 캐시에 액세스할 수도 있습니다.
