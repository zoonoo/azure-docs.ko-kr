...
* **계획되지 않은 하드웨어 유지 관리 이벤트**는 Azure 플랫폼에서 물리적 컴퓨터와 관련된 하드웨어 또는 플랫폼 구성 요소가 실패할 것으로 예측할 때 발생합니다. 플랫폼에서 오류를 예측하는 경우 계획되지 않은 하드웨어 유지 관리 이벤트를 발급하여 해당 하드웨어에서 호스팅되는 가상 머신에 미치는 영향을 줄입니다. Azure는 [라이브 마이그레이션](https://docs.microsoft.com/azure/virtual-machines/linux/maintenance-and-updates) 기술을 사용하여 실패한 하드웨어에서 정상 물리적 컴퓨터로 가상 컴퓨터를 마이그레이션합니다. 실시간 마이그레이션은 짧은 시간 동안 Virtual Machine을 일시 중지하는 VM 보존 작업입니다. 메모리, 열린 파일 및 네트워크 연결은 유지되지만 이벤트 전후에 성능이 저하될 수 있습니다. 실시간 마이그레이션을 사용할 수 없는 경우 아래에서 설명한 대로 VM에 예기치 않은 가동 중지가 발생합니다.
...
* **계획된 유지 관리 이벤트는** 가상 시스템이 실행되는 플랫폼 인프라의 전반적인 안정성, 성능 및 보안을 개선하기 위해 Microsoft에서 기본 Azure 플랫폼에 정기적으로 업데이트하는 이벤트입니다. 이러한 업데이트는 대부분 Virtual Machines 또는 Cloud Services에 영향을 주지 않고 수행됩니다([VM 보존 유지 관리](https://docs.microsoft.com/azure/virtual-machines/windows/preserving-maintenance) 참조). Azure 플랫폼에서 가능한 모든 경우에 VM 보존 유지 관리를 사용하려고 시도하는 반면, 가상 머신을 다시 부팅해야 기본 인프라에 필요한 업데이트를 적용할 수 있는 경우가 드물게 있습니다. 이 경우 적절한 기간에 VM에 대한 유지 관리를 시작하여 유지 관리 - 재배포 작업이 포함된 Azure Planned Maintenance(Azure 계획된 유지 관리)를 수행할 수 있습니다. 자세한 내용은 [Virtual Machines에 대한 계획된 유지 관리](https://docs.microsoft.com/azure/virtual-machines/windows/planned-maintenance/)를 참조하세요.


이러한 이벤트로 인한 가동 중지 시간의 영향을 줄이기 위해 가상 머신에 다음과 같은 고가용성 모범 사례를 권장합니다.

* [중복성을 위해 가용성 집합에서 여러 가상 시스템 구성]
* [가용성 집합의 VM에 Managed Disks 사용]
* [예약된 이벤트를 사용하여 VM 에 영향을 미치는 이벤트에 사전에 대응](https://docs.microsoft.com/azure/virtual-machines/virtual-machines-scheduled-events)
* [각 애플리케이션 계층을 별도의 가용성 집합으로 구성]
* [부하 밸러블러를 가용성 세트와 결합]
* [가용성 영역을 사용하여 데이터 센터 수준 오류로부터 보호]

## <a name="use-availability-zones-to-protect-from-datacenter-level-failures"></a>가용성 영역을 사용하여 데이터 센터 수준 오류로부터 사용자를 보호합니다.

[가용성 영역은](../articles/availability-zones/az-overview.md) VM에서 응용 프로그램 및 데이터의 가용성을 유지하는 데 필요한 제어 수준을 확장합니다. 가용성 영역은 Azure 지역 내의 고유한 물리적 위치입니다. 각 영역은 독립된 전원, 냉각 및 네트워킹을 갖춘 하나 이상의 데이터 센터로 구성됩니다. 복원력을 보장하기 위해 모든 활성화된 영역에 최소 3개의 별도 영역이 있습니다. 지역 내에서 가용성 영역의 물리적 구분은 애플리케이션 및 데이터를 데이터 센터 오류로부터 보호할 수 있습니다. 영역 중복 서비스는 단일 지점 오류에서 보호하기 위해 가용성 영역에서 애플리케이션 및 데이터를 복제합니다.

Azure 지역의 가용성 영역은 **오류 도메인과** **업데이트 도메인의**조합입니다. 예를 들어 Azure 지역의 3개 영역에 VM을 3개 이상 만들면 장애 도메인 3개와 업데이트 도메인 3개에 VM이 효과적으로 분산됩니다. Azure 플랫폼은 업데이트 도메인에 분산된 VM을 인식하여 다른 영역에 있는 VM이 동시에 업데이트되지 않게 합니다.

Azure는 가용성 영역을 통해 업계 최고의 99.99% VM 작동 시간 SLA를 제공합니다. 영역에서 복제된 VM을 사용하도록 솔루션을 설계하면 데이터 센터의 손실로부터 응용 프로그램과 데이터를 보호할 수 있습니다. 하나의 영역이 손상되면 다른 영역에서 복제된 앱 및 데이터를 즉시 사용할 수 있습니다.
...
> [!NOTE]
> 특정 상황에서 동일한 가용성집합의 VM 2개에서 동일한 FaultDomain을 공유할 수 있습니다. 가용성 집합으로 전환하고 **오류 도메인** 열을 확인하여 확인할 수 있습니다.
> VM을 배포하는 동안 다음 시퀀스에서 발생할 수 있습니다.
> - 첫 번째 VM 배포
> - 첫 번째 VM 중지/거래 할당
> - 이러한 상황에서 는 2번째 VM의 OS 디스크가 1vM과 동일한 장애 도메인에 생성될 수 있으므로 2번째 VM도 동일한 FaultDomain에 착륙합니다. 
> 이 문제를 방지하려면 배포 간에 VM을 중지/할당 할당 하지 않는 것이 좋습니다.
...
2. 저장소 계정에 VH를 더 추가하기 전에 **Azure Storage 계정의 관리되지 않는 디스크 수 [에](../articles/storage/blobs/scalability-targets-premium-page-blobs.md) 대한 제한을 검토합니다.**
3. **가용성 집합의 각 VM에 대해 별도의 저장소 계정을 사용합니다.** 동일한 가용성 집합에서 여러 VM을 사용하여 Storage 계정을 공유하지 않습니다. 위의 모범 사례를 따르면 여러 가용성 집합의 VM에서 스토리지 계정을 공유할 수 있습니다. ![관리되지 않는 디스크 FD](./media/virtual-machines-common-manage-availability/umd-updated.png)
...
가상 시스템이 모두 거의 동일하고 응용 프로그램에 동일한 용도로 사용되는 경우 응용 프로그램의 각 계층에 대해 가용성 영역 또는 가용성 집합을 구성하는 것이 좋습니다.  동일한 가용성 영역 또는 집합에 두 개의 서로 다른 계층을 배치하는 경우 동일한 응용 프로그램 계층의 모든 가상 컴퓨터를 한 번에 다시 부팅할 수 있습니다. 가용성 영역에서 두 개 이상의 가상 컴퓨터를 구성하거나 각 계층에 대해 설정하면 각 계층에서 하나 이상의 가상 컴퓨터를 사용할 수 있습니다.

예를 들어 IIS, 아파치 및 Nginx를 실행하는 응용 프로그램의 프런트 엔드에 모든 가상 컴퓨터를 단일 가용성 영역 또는 집합에 넣을 수 있습니다. 프런트 엔드 가상 시스템만 동일한 가용성 영역 또는 집합에 배치되어 있는지 확인합니다. 마찬가지로 복제된 SQL Server 가상 컴퓨터 또는 MySQL 가상 컴퓨터와 같이 데이터 계층 가상 컴퓨터만 자체 가용성 영역 또는 집합에 배치해야 합니다.

<!--Image reference-->
   ![애플리케이션 계층](./media/virtual-machines-common-manage-availability/application-tiers.png)

## <a name="combine-a-load-balancer-with-availability-zones-or-sets"></a>부하 분산기와 가용성 영역 또는 세트 결합
Azure [Load 밸런서를](../articles/load-balancer/load-balancer-overview.md) 가용성 영역과 결합하거나 가장 많은 응용 프로그램 복원력을 얻으려면 설정합니다. Azure 부하 분산 장치는 트래픽을 여러 가상 머신에 분산시킵니다. 표준 계층 가상 머신의 경우 Azure 부하 분산 장치가 포함되어 있습니다. 모든 가상 머신 계층에 Azure Load Balancer가 포함되어 있는 것은 아닙니다. 가상 머신 부하 분산에 대한 자세한 내용은 [가상 머신 부하 분산](../articles/virtual-machines/virtual-machines-linux-load-balance.md)을 참조하세요.
...
가용성 영역 간에 균형을 로드하는 방법에 대한 자습서는 [Azure CLI를 사용하여 모든 가용성 영역에서 VM로드 균형을](../articles/load-balancer/load-balancer-standard-public-zone-redundant-cli.md)참조하세요.


<!-- Link references -->
[중복성을 위해 가용성 집합에서 여러 가상 시스템 구성]: #configure-multiple-virtual-machines-in-an-availability-set-for-redundancy
[각 애플리케이션 계층을 별도의 가용성 집합으로 구성]: #configure-each-application-tier-into-separate-availability-zones-or-availability-sets
[부하 밸러블러를 가용성 세트와 결합]: #combine-a-load-balancer-with-availability-zones-or-sets
[Avoid single instance virtual machines in availability sets]: #avoid-single-instance-virtual-machines-in-availability-sets
[가용성 집합의 VM에 Managed Disks 사용]: #use-managed-disks-for-vms-in-an-availability-set
[가용성 영역을 사용하여 데이터 센터 수준 오류로부터 보호]: #use-availability-zones-to-protect-from-datacenter-level-failures
