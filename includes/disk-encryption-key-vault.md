...
>[!WARNING]
> 키 자격 증명 모음 및 Vm은 동일한 구독에 있어야 합니다. 또한 암호화 비밀이 지역 경계를 넘지 않도록 하려면 Azure Disk Encryption Key Vault와 Vm을 동일한 지역에 함께 배치 해야 합니다. 암호화할 Vm과 동일한 구독 및 지역에 있는 Key Vault를 만들고 사용 합니다. 
...
대신 온-프레미스 키 관리 HSM에서 KEK를 가져올 수 있습니다. 자세한 내용은 [Key Vault 설명서](/azure/key-vault/key-vault-hsm-protected-keys)를 참조 하세요.
