...
|-v|필수|Mobility Service가 설치되는 플랫폼을 지정합니다. </br> </br>- **VMware**: 실행 중인 VM에 모바일 서비스를 설치 하는 경우이 값을 사용 *VMware vSphere ESXi 호스트*하십시오 *Hyper-v 호스트*, 및 *물리적 서버*. </br> - **Azure**: Azure IaaS VM에 에이전트를 설치 하는 경우이 값을 사용 합니다.| VMware </br> Azure|
|-q|옵션|설치 관리자를 자동 모드에서 실행하도록 지정합니다.| N/A|
