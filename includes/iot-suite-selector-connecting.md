...
이 자습서를 완료하려면 활성 Azure 계정이 필요합니다. 계정이 없는 경우 몇 분 만에 무료 평가판 계정을 만들 수 있습니다. 자세한 내용은 [Azure 무료 체험][lnk-free-trial]을 참조하세요.
...
5. 장치 자격 증명(장치 ID, IoT Hub 호스트 이름, 장치 키)을 기록해 둡니다. 이 정보는 원격 모니터링 솔루션에 연결할 때 클라이언트 응용 프로그램에 필요합니다. **완료**를 클릭합니다.

    ![][4]

6. 장치가 장치 섹션에 표시되는지 확인합니다. 장치가 원격 모니터링 솔루션에 연결될 때까지 장치 상태는 **보류 중**으로 표시됩니다.
