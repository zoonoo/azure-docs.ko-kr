# <a name="securing-your-iot-deployment"></a>IoT 배포 보안 유지
...
IoT Hub는 네트워크에서 토큰이 전송되는 것을 피하기 위해 보안 토큰을 사용하여 장치 및 서비스를 인증합니다. 또한 보안 토큰은 유효 기간 및 범위가 제한됩니다. Azure IoT Hub SDK는 특별한 구성이 필요하지 않고 토큰을 자동으로 생성합니다. 그러나 일부 시나리오에서는 사용자가 보안 토큰을 직접 생성하고 사용해야 합니다. 여기에는 MQTT, AMQP 또는 HTTP 표면의 직접 사용 또는 토큰 서비스 패턴의 구현이 포함됩니다.
...
[IoT Hub는 MQTT, AMQP 및 HTTP][lnk-protocols]와 같은 프로토콜을 지원합니다. 이러한 각 프로토콜은 IoT 장치에서 IoT Hub로의 보안 토큰을 다음과 같이 다르게 사용합니다.

* AMQP: SASL PLAIN 및 AMQP 클레임 기반 보안(Hub 수준 토큰의 경우 ({policyName}@sas.root.{iothubName}, 장치 범위 토큰의 경우 {deviceId}).
...
IoT Hub 장치 ID 레지스트리를 사용하여 장치별 보안 자격 증명 및 액세스 제어를 구성할 수 있습니다. 그러나 [사용자 지정 장치 ID 레지스트리 및/또는 인증 체계][lnk-custom-auth]에 이미 상당한 투자를 한 IoT 솔루션의 경우 토큰 서비스를 만들어 IoT Hub가 있는 기존 인프라로 통합할 수 있습니다.
