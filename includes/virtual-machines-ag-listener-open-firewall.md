...
7. **이름** 페이지의 **이름** 텍스트 상자에 **AlwaysOn 수신기 프로브 포트**와 같은 규칙 이름을 지정하고 **마침**을 클릭합니다.
8. 가용성 그룹 수신기 포트(이전에 스크립트의 $EndpointPort 매개 변수에서 지정된 대로)에 대해 위의 단계를 반복하고 **AlwaysOn 수신기 포트**와 같은 적절한 규칙 이름을 지정합니다.
