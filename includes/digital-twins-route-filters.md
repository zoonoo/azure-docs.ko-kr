...
| 데이터 스키마 | DTDL 모델 ID | `dataschema = '<model-dtmi-ID>'` | **원격 분석의 경우**: 데이터 스키마는 쌍의 모델 ID 또는 원격 분석을 내보내는 구성 요소입니다. 예를 들어 `dtmi:example:com:floor4;2` <br>**알림**:의 알림 본문에서 데이터 스키마에 액세스할 수 있습니다. `$body.$metadata.$model`|
