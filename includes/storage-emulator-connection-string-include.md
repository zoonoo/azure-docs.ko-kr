...
응용 프로그램에서 저장소 에뮬레이터에 연결하는 가장 쉬운 방법은 바로 가기를 참조하는 응용 프로그램의 구성 파일내에 있는 연결 문자열을 구성하는 것입니다.`UseDevelopmentStorage=true` 예를 들어 app.config 파일의 저장소 에뮬레이터에 대한 연결 문자열은 다음과 같습니다.
...
이 값은 위에 표시된 바로 가기 `UseDevelopmentStorage=true`와(과) 동일합니다.
