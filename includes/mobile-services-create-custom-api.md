...
2. **API** 탭을 클릭한 후 **만들기**를 클릭합니다. **새 사용자 지정 API 만들기** 대화 상자가 표시됩니다.
...
5. **Script** 탭을 클릭하여 기존 코드를 다음의 코드로 바꾼 후 **Save**를 클릭합니다. 	이 코드에서는 **todoitem** 테이블에 직접 액세스하여 모든 항목에  `complete` 플래그를 설정하기 위해 [mssql 개체]를 사용합니다. **exports.post** 함수를 사용하기 때문에 작업을 수행하기 위해 클라이언트는 POST 요청을 보내고, 변경된 행의 수가 클라이언트에 정수 값으로 반환됩니다.
