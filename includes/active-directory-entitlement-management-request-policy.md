...
1. 액세스를 **요청할 수 있는 사용자** 목록에서 **디렉터리의 사용자**로를 선택 합니다.
...
1. 액세스를 **요청할 수 있는 사용자** 목록에서 **디렉터리에 없는 사용자를**선택 합니다.
...
1. 다음 옵션 중 하나를 선택합니다.
    ...
    연결 된 조직은 자주 공동 작업 하는 외부 Azure AD 디렉터리 또는 도메인입니다.

1. **연결 된 특정 조직을**선택한 경우 **디렉터리 추가** 를 클릭 하 여 관리자가 이전에 추가한 연결 된 조직의 목록에서 선택 합니다.

1. 도메인 이름을 입력 하 여 해당 도메인 이름을 사용 하는 연결 된 조직을 검색 합니다.

    ![액세스 패키지-요청-디렉터리 선택](./media/active-directory-entitlement-management-request-policy/select-directories.png)

    공동 작업 하려는 조직이 목록에 없는 경우 관리자에 게 연결 된 조직으로 추가 하도록 요청할 수 있습니다. 

1. 모든 연결 된 조직을 선택 했으면 **선택**을 클릭 합니다.

    > [!NOTE]
    > 선택한 연결 된 조직의 모든 사용자가이 액세스 패키지를 요청할 수 있습니다. 여기에는 연결 된 조직과 연결 된 모든 하위 도메인의 사용자가 포함 됩니다.
...
1. 액세스를 **요청할 수 있는 사용자** 목록에서 **없음 (관리자 직접 할당만)** 을 선택 합니다.
    ...
    액세스 패키지를 만든 후에는 액세스 패키지에 특정 내부 및 외부 사용자를 직접 할당할 수 있습니다. 외부 사용자를 지정 하는 경우 게스트 사용자 계정이 디렉터리에 생성 됩니다. 사용자를 직접 할당 하는 방법에 대 한 자세한 내용은 [액세스 패키지에 대 한 보기 및 변경 할당](../articles/active-directory/governance/entitlement-management-access-package-assignments.md)을 참조 하세요.
