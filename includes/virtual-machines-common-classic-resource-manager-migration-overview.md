...
| 컴퓨팅 |VM과 연결된 여러 서브넷 |서브넷만 참조하도록 서브넷 구성을 업데이트합니다. |
...
| 컴퓨팅 |프리미엄 저장소를 사용한 부팅 진단 |마이그레이션을 계속하기 전에 VM에 대한 부팅 진단 기능을 비활성화합니다. 마이그레이션이 완료된 후에 Resource Manager 스택에서 부팅 진단을 재활성화할 수 있습니다. 또한 스크린샷 및 직렬 로그에 대해 사용되는 blob은 그러한 blob에 대해 요금이 부과되지 않도록 삭제해야 합니다. |
...
| 컴퓨팅 | 백업 또는 스냅숏 확장이 있는 VM | 이러한 확장은 Azure Backup 서비스를 사용하여 구성된 Virtual Machine에 설치됩니다. 이러한 Virtual Machines를 마이그레이션하려면 [여기](https://docs.microsoft.com/azure/virtual-machines/windows/migration-classic-resource-manager-faq#vault)의 지침을 따르세요.  |
