...
이미지를 만드는 과정에서 파일은 다운로드 되어 `IT_<DestinationResourceGroup>_<TemplateName>` 리소스 그룹에 저장 되며,이로 인해 저장소 비용이 적게 듭니다. 이를 유지 하지 않으려면 이미지를 빌드한 후에 이미지 템플릿을 삭제 합니다.
