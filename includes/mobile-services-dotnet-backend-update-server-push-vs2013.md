이 단계를 수행하여 앱에 푸시 알림을 보내는 새 사용자 지정 [ApiController](http://go.microsoft.com/fwlink/p/?LinkId=512673)를 만듭니다. [TableController](http://msdn.microsoft.com/library/azure/dn643359.aspx) 또는 백 엔드 서비스의 다른 위치에서 같은 코드를 구현할 수 있습니다.
...
4. 컨트롤러에 대해 POST 메서드를 구현하는 다음 코드를 추가합니다.
   ...
   > [!NOTE]
   > 이 POST 메서드는 응용 프로그램 키를 소유한 모든 클라이언트가 호출할 수 있으므로 안전하지 않습니다. 끝점을 보호하려면 인증이 필요하도록 메서드나 클래스에 `[AuthorizeLevel(AuthorizationLevel.User)]` 특성을 적용합니다.
   > 
   > 
