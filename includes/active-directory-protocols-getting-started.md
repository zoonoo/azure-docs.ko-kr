...
먼저, 애플리케이션을 Azure AD(Azure Active Directory) 테넌트에 등록해야 합니다. 그러면 응용 프로그램에 대한 응용 프로그램 ID가 제공되며 토큰을 수신하는 데 사용할 수 있습니다.
...
* 프롬프트에 따라 새 응용 프로그램을 만듭니다. 이 자습서에서는 웹 애플리케이션이든지 네이티브 애플리케이션이든지 상관 없지만 웹 애플리케이션 또는 네이티브 애플리케이션에 대한 특정 예제가 필요한 경우 [빠른 시작](../articles/active-directory/develop/v1-overview.md)을 확인하세요.
