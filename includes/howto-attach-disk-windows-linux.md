Azure 가상 컴퓨터의 디스크에 대한 자세한 내용은 [Azure의 가상 컴퓨터 디스크 정보][Azure의 가상 컴퓨터 디스크 정보](영문)를 참조하세요.
...
5.  확인 표시를 클릭하여 빈 데이터 디스크를 연결합니다.
    ...
    ![빈 데이터 디스크 연결됨][빈 데이터 디스크 연결됨]

## <span id="attachexisting"></span></a>방법: 기존 디스크 연결

기존 디스크를 연결하려면 저장소 계정에 사용 가능한 .vhd가 있어야 합니다. [Add-AzureVhd][Add-AzureVhd] cmdlet을 사용하여 .vhd 파일을 저장소 계정에 업로드합니다. .vhd 파일을 만들어 업로드한 후에는 가상 컴퓨터에 연결할 수 있습니다.

1.  **가상 컴퓨터**를 클릭하고 해당 가상 컴퓨터를 선택합니다.

2.  명령 모음에서 **Attach**를 클릭한 후 **Attach Disk**를 선택합니다.

    ![데이터 디스크 연결][데이터 디스크 연결]

    **Attach Disk** 대화 상자가 나타납니다.

    ![데이터 디스크 세부 정보 입력][데이터 디스크 세부 정보 입력]
