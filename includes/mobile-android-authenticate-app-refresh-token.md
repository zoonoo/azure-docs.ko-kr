...
1. ToDoActivity.java 파일에서 ToDoActivity 클래스의 `authenticate` 메서드를 업데이트하여 토큰 및 토큰 캐시를 강제로 새로 고칠 수 있는 부울 매개 변수를 수락하도록 합니다. 또한 인증이 완료되면 차단된 모든 스레드에 알려 새 토큰을 사용할 수 있도록 해야 합니다.
...
2. ToDoActivity.java 파일에서 ToDoActivity 클래스 내의 새 `RefreshTokenCacheFilter` 클래스에 이 코드를 추가합니다.
...
2. ToDoActivity.java 파일에서 `onCreate` 메서드를 다음과 같이 업데이트합니다.
