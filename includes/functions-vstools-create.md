...
1. **새 Azure Functions 애플리케이션 만들기**에서 다음 옵션을 사용합니다.
    ...
    | **Storage 계정**  | **Storage 에뮬레이터** | HTTP 트리거는 Azure Storage 계정 연결을 사용하지 않습니다. 다른 모든 트리거 유형은 유효한 저장소 계정 연결 문자열이 필요합니다. 함수에는 스토리지 계정이 필요하기 때문에 Azure에 프로젝트를 게시할 때 할당되거나 생성됩니다. |
