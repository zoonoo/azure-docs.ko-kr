MongoLab 데이터베이스를 프로비전한 후 언제든지 해당 데이터베이스를 관리하기 위한 MongoLab 웹 UI에 액세스할 수
있습니다. Azure 관리 포털을 통해 편리한 SSO(Single-Sign-On) 액세스를 제공합니다.

MongoLab UI를 사용하여 문서 삽입 및 쿼리, 데이터베이스 통계 가져오기, 유용한 데이터베이스 명령 실행 등 대부분의
데이터베이스 작업을 수행할 수 있습니다. MongoLab UI에 대한 자세한 내용은 [지원 페이지][1](영문) 및
[블로그][2](영문)를 참조하십시오.
