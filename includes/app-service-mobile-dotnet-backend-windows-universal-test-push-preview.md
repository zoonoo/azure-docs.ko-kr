
1. Windows 스토어 프로젝트를 마우스 오른쪽 단추로 클릭하고 **시작 프로젝트로 설정**을 클릭한 다음 F5 키를 눌러 Windows 스토어 앱을 실행합니다.
	
	앱이 시작되면 장치가 푸시 알림에 대해 등록됩니다.

2. Windows 스토어 앱을 중지하고 위의 단계를 반복하여 Windows Phone 스토어 앱을 실행합니다.

	이제 두 장치가 모두 푸시 알림을 받도록 등록되었습니다.

3. Windows 스토어 앱을 다시 실행하고 **TodoItem 삽입**에 텍스트를 입력한 다음 **저장**을 클릭합니다.

   	![](./media/mobile-services-javascript-backend-windows-universal-test-push/mobile-quickstart-push1.png)

   	삽입이 완료되면 Windows 스토어 및 Windows Phone 앱이 모두 WNS에서 푸시 알림을 수신합니다.

   	![](./media/mobile-services-javascript-backend-windows-universal-test-push/mobile-quickstart-push2.png)

	앱이 실행되고 있지 않을 때도 알림이 Windows Phone에 표시됩니다.

   	![](./media/mobile-services-javascript-backend-windows-universal-test-push/mobile-quickstart-push5-wp8.png)


<!--HONumber=49-->