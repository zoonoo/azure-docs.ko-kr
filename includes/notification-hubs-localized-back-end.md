백엔드 앱에서 이제 네이티브 페이로드 대신 템플릿 알림을 보내도록 전환해야 합니다. 이렇게 하면 다른 플랫폼에 여러 페이로드를
보내지 않아도 되므로 백엔드 코드가 간단해집니다.
...
지원되는 모든 장치에 백엔드가 브로드캐스트할 수 있으므로 포함된 코드는 Windows 스토어와 iOS 장치 둘 다에
브로드캐스트합니다.
...
이 간단한 호출은 알림 허브에서 올바른 네이티브 페이로드를 생성하여 특정 태그에 가입된 모든 장치에 전달할 때 플랫폼과 상관없이
올바르게 지역화된 뉴스를 **모든** 장치에 전달합니다.
### 모바일 서비스

모바일 서비스 스케줄러에서 스크립트를 다음으로 덮어쓰십시오.
