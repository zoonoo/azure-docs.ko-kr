...
* DPS를 사용하여 디바이스를 프로비저닝합니다. 프로비저닝 정보에는 모델 ID가 포함됩니다.
...
`provision_device` 함수는 DPS를 사용하여 디바이스를 프로비저닝하고 IoT Central에 등록합니다. 이 함수는 프로비저닝 페이로드의 디바이스 모델 ID를 포함합니다.
