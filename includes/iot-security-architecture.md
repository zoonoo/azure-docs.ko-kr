...
| **구성 요소** | **위협** | **완화 방법** | **위험** | **구현** |
| --- | --- | --- | --- | --- |
| 디바이스 |S |디바이스에 ID를 할당하고 디바이스 인증 |디바이스 또는 디바이스 일부를 다른 디바이스로 대체합니다. 올바른 디바이스와 통신하고 있는지 어떻게 알 수 있을까요? |TLS(전송 계층 보안) 또는 IPSec을 사용하여 디바이스를 인증합니다. 전체 비대칭 암호화를 처리할 수 없는 PSK(미리 공유한 키)를 해당 디바이스에서 사용할 수 있도록 인프라가 지원해야 합니다. Azure AD, [OAuth](https://www.rfc-editor.org/pdfrfc/rfc6755.txt.pdf) 활용 |
...
**변조**: 장치가 제어 시스템에 알려지지 않은 상태 (알려진 보정 매개 변수 외부)에서 작동 하 여 잘못 해석 될 수 있는 데이터를 제공 하도록 다시 구성 될 수 있습니다.
...
**스푸핑/변조/부인**: 보안이 설정 되지 않으면 (소비자 원격 제어의 경우 거의 발생 하지 않음) 공격자가 장치 상태를 익명으로 조작할 수 있습니다. 모든 TV를 켜고 끌 수 있어서 사람들이 장난을 치기 위한 도구로 많이 구입하는 원격 리모콘이 대표적인 예입니다.
