...
> [AZURE.NOTE] 현재 **AvroFormat**의 데이터에 대한 압축 설정은 지원되지 않습니다.
...
위의 샘플 데이터 집합이 복사 작업의 출력으로 사용된다고 가정하면 복사 작업은 최적의 비율을 사용하여 GZIP 코덱으로 출력 데이터를 압축한 다음 압축된 데이터를 pagecounts.csv.gz라는 이름의 파일로 Azure Blob 저장소에 작성합니다.
