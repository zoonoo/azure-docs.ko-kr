...
- 유지 관리를 다시 부팅해야 하는 경우 유지 관리가 계획된 시기에 대해 알림을 받을 수 있습니다. Azure는 사용자가 편리한 시간에 직접 유지 관리를 시작할 수 있는 기간도 제공합니다. 자체 유지 관리 시간은 일반적으로 4 주 동안 유지 관리를 수행 하는 긴급 한 경우가 아니라면입니다. Azure는 Vm을 플랫폼 계획 된 유지 관리에 대 한 다시 부팅 해야 하는 경우에 사례를 줄이기 위해 기술에 투자 수도 있습니다. 
...
다시 부팅이 필요 하지 않은 가장 0이 아닌 영향 유지 관리에 대 한 목적은 10 초 미만의 VM에 대 한 일시 중지 합니다. Azure는 고객 Vm에 최소 영향을 미치므로 주의깊게 업데이트 메커니즘을 선택 합니다. 특정 한 경우 최대 30 초 동안 VM을 일시 중지 하 고 RAM의 메모리를 유지 하는 메모리 보존 유지 관리 메커니즘 사용 됩니다. VM이 다시 시작 후 및 해당 클록을 자동으로 동기화 됩니다. Azure는 점점 더 실시간 마이그레이션 기술을 사용하고 메모리 보존 유지 관리 메커니즘을 개선하여 일시 중지 기간을 줄이고 있습니다.  
