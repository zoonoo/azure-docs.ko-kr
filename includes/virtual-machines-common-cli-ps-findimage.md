...
[Azure CLI 2.0을 설치했으면](https://docs.microsoft.com/cli/azure/install-az-cli2) `az vm image list` 명령을 사용하여 인기 있는 VM 이미지의 캐시된 목록을 확인합니다. 예를 들어 다음 예제의 `az vm image list -o table` 명령은 다음과 같은 내용을 표시합니다.
...
특정 정보를 찾으려면 `az vm image list`를 [JMESPATH 쿼리 필터](https://docs.microsoft.com/cli/azure/query-az-cli2)와 함께 사용합니다. 예를 들어 다음은 **Debian**(`--all`스위치 없이 공용 이미지의 로컬 캐시를 검색만 한다는 점에 주의)에 제공되는 **sku**를 표시합니다.
