...
3. **새 계정** 블레이드에서 Azure Cosmos DB 계정에 대해 원하는 구성을 지정합니다. 
    ...
    **새 계정** 블레이드에서 다음 스크린샷의 정보를 지침으로만 사용하여 필드를 완성합니다. 사용자 고유의 값이 스크린샷의 값과 일치하지 않으므로 계정을 설정할 때 고유한 값을 선택해야 합니다. 
 
    ![Azure Cosmos DB 블레이드](./media/cosmos-db-create-dbaccount-graph/create-nosql-db-databases-json-tutorial-2.png)

    설정|제안 값|설명
    ---|---|---
    ID|*고유 값*|Azure Cosmos DB 계정을 식별하기 위해 선택하는 고유한 이름입니다. URI를 만들기 위해 제공하는 ID에 *documents.azure.com*이 추가되므로 식별할 수 있는 고유한 ID를 사용합니다. ID는 소문자, 숫자 및 하이픈(-) 문자만 포함해야 하며, 3-50자여야 합니다.
