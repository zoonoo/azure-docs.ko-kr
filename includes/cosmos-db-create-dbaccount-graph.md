...
3. **새 계정** 블레이드에서 이 Azure Cosmos DB 계정에 원하는 구성을 지정합니다. 
    ...
    설정|제안 값|설명
    ---|---|---
    ID|*고유 값*|이 Azure Cosmos DB 계정을 식별하는 고유한 이름입니다. URI를 만들기 위해 제공하는 ID에 *documents.azure.com*이 추가되므로 식별할 수 있는 고유한 ID를 사용합니다. ID는 소문자, 숫자 및 하이픈(-) 문자만 포함해야 하며, 3-50자여야 합니다.
...
5. 맨 위 도구 모음에서 **알림** 아이콘 ![알림 아이콘](./media/cosmos-db-create-dbaccount-graph/notification-icon.png)을 클릭하여 배포 프로세스를 모니터링합니다.
