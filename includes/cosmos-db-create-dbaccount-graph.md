...
3. **새 계정** 페이지에서 새 Azure Cosmos DB 계정에 대한 설정을 입력합니다. 
    ...
    API|Gremlin(그래프)|API는 만들 계정의 형식을 결정합니다. Azure Cosmos DB는 응용 프로그램의 요구 사항을 충족하기 위해 Gremlin(그래프), MongoDB, SQL(DocumentDB) 및 테이블(키-값)이라는 네 가지 API를 제공합니다. 현재 각각에는 별도의 계정이 필요합니다. <br><br>이 빠른 시작에서 Gremlin 구문을 사용하여 쿼리할 수 있는 그래프를 만들기 때문에 **Gremlin(그래프)**을 선택합니다.<br><br>[Graph API에 대한 자세한 정보](../articles/cosmos-db/graph-introduction.md)
