...
4.  데이터 디렉터리를 만듭니다. 기본적으로 MongoDB는 */data/db* 디렉터리에 데이터를 보관하지만 디렉터리는 사용자가 만들어야 합니다. 디렉터리를 만들려면 다음을 실행하십시오.
    ...
    Linux에 MongoDB를 설치하는 방법에 대한 자세한 정보는 [Quickstart Unix][1](영문)를 참조하십시오.

5.  데이터베이스를 시작하려면 다음을 실행합니다.
    
         $ mongod --dbpath /mnt/datadrive/data --logpath /mnt/datadrive/data/mongod.log
    
    MongoDB 서버가 시작되고 저널 파일이 사전 할당되면 모든 로그 메시지가 */mnt/datadrive/data/mongod.log* 파일로 보내집니다. MongoDB가 저널 파일을 사전 할당하고 연결이 수신될 때까지 몇 분이 걸릴 수 있습니다.
