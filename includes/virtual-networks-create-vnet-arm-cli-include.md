...
3. 필요한 경우 아래와 같이 **azure group create**를 실행하여 새 리소스 그룹을 만듭니다. 명령의 출력을 확인합니다. 출력 다음에 표시되는 목록은 사용되는 매개 변수를 설명합니다. 리소스 그룹에 대한 자세한 내용은 [Azure Resource Manager 개요](../articles/resource-group-overview.md#resource-groups)를 참조하세요.
   ...
   * **-n (or --name)**. 새 리소스 그룹의 이름입니다. 이 시나리오에서는 *TestRG*입니다.
...
4. 아래와 같이 **azure network vnet create** 명령을 실행하여 VNet과 서브넷을 만듭니다.
   ...
   * **-n (or --name)**. 만들 VNet의 이름입니다. 이 시나리오에서는 *TestVNet*입니다.
...
5. 아래와 같이 **azure network vnet subnet create** 명령을 실행하여 서브넷을 만듭니다. 명령의 출력을 확인합니다. 출력 다음에 표시되는 목록은 사용되는 매개 변수를 설명합니다.
   ...
   * **-n (or --name)**. 새 서브넷의 이름입니다. 이 시나리오에서는 *FrontEnd*입니다.
