## <a name="how-to-create-a-vnet-using-the-azure-cli"></a>Azure CLI를 사용하여 VNet을 만드는 방법
...
3. 필요한 경우 아래와 같이 **azure group create** 를 실행하여 새 리소스 그룹을 만듭니다. 명령의 출력을 확인합니다. 출력 다음에 표시되는 목록은 사용되는 매개 변수를 설명합니다. 리소스 그룹에 대한 자세한 내용은 [Azure 리소스 관리자 개요](../articles/azure-resource-manager/resource-group-overview.md#resource-groups)를 참조하세요.
...
4. 아래와 같이 **azure network vnet create** 명령을 실행하여 VNet과 서브넷을 만듭니다. 
   ...
   * **-n (or --name)**. 만들 VNet의 이름입니다. 이 시나리오에서는 *TestVNet*
   * **-a(또는 --address-prefixes)**. VNet 주소 공간에 사용되는 CIDR 블록의 목록입니다. 이 시나리오에서는 *192.168.0.0/16*
...
7. 아래와 같이 새 VNet의 속성을 보려면 **azure network vnet show** 명령을 실행합니다.
