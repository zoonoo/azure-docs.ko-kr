...
> [WACOM.NOTE] 이 작업의 단계를 수행하려면 공유 또는 표준 모드에 대해 웹 사이트를 구성해야 하며, 구독에 대해 청구되는 요금이 변경될 수도 있습니다. 자세한 내용은 [웹 사이트 가격 책정 세부 정보][2]를 참조하십시오.
...
> [WACOM.NOTE] 일부 도메인 등록 기관에서는 contoso.com 등의 루트 이름이 아니라 www.contoso.com 등의 CNAME 레코드를 사용할 때 하이 도메인 매핑만 허용합니다. CNAME 레코드에 대한 자세한 내용은 등록 기관에서 제공한 설명서, [CNAME 레코드에 대한 Wikipedia 항목][3] 또는 [IETF 도메인 이름 - 구현 및 사양][4] 문서를 참조하십시오.
...
Azure 웹 사이트에 대한 공유 및 표준 모드에서만 웹 사이트에 사용자 지정 도메인 이름을 설정할 수 있습니다. 웹 사이트를 무료 웹 사이트 모드에서 공유 또는 표준 웹 사이트 모드로 전환하기 전에 먼저 웹 사이트 구독에 대해 적용된 지출 한도를 제거해야 합니다. 공유 및 표준 모드 가격 책정에 대한 자세한 내용은 [가격 책정 세부 정보][5]를 참조하십시오.
...
5.  **Save**를 클릭합니다. 6.  공유 모드(또는 표준을 선택한 경우 표준 모드) 시 비용 증가에 대한 메시지가 표시될 때 동의하면 **Yes**를 클릭합니다.
...
1.  트래픽 관리자 프로필이 아직 없는 경우 [빠른 생성을 사용하여 트래픽 관리자 프로필 만들기][7]의 내용을 참조하여 만듭니다. 트래픽 관리자 프로필과 관련된 **.trafficmgr.com** 도메인 이름이 있습니다. 이 이름은 이후 단계에서 사용됩니다.
...
1.  이러한 방법 중 하나를 사용하여 웹 사이트에 할당된 **.azurewebsite.net** 도메인 이름을 찾습니다.
    * [Azure 관리 포털][6]에 로그인하고 웹 사이트를 선택한 다음 **대시보드**를 선택하고 **quick glance** 섹션에서 **Site URL** 항목을 찾습니다.
...
> [WACOM.NOTE] CNAME이 DNS 시스템을 통해 전파되는 데 약간의 시간이 걸릴 수 있습니다. CNAME이 전파될 때까지 웹 사이트에 대한 CNAME을 설정할 수 없습니다. [http://www.digwebinterface.com/][11] 등의 서비스를 사용하여 CNAME을 사용할 수 있는지 확인할 수 있습니다.
...
2.  **Web Sites** 탭에서 사이트 이름을 클릭하고 **Dashboard**를 선택한 후 페이지 맨 아래에서 **Manage Domains**를 선택합니다.
...
구성이 완료되면 사용자 지정 도메인 이름이 웹 사이트 **Configure** 페이지의 **domain names** 섹션에 표시됩니다.
...
2.  **Web Sites** 탭에서 사이트 이름을 클릭하고 **Dashboard**를 선택한 후 화면 맨 아래에서 **Manage Domains**를 선택합니다.
...
8.  별칭이 **awverify**이고 정식 도메인이 앞에서 얻은 **awverify.mysite.azurewebsites.net**인 CNAME 레코드를 만듭니다.
    ...
    > 별칭 도메인 이름인 awverify.www.customdomainname.com 또는 awverify.customdomainname.com을 요구할 수 있습니다.
...
> [WACOM.NOTE] awverify CNAME이 DNS 시스템을 통해 전파되는 데 약간의 시간이 걸릴 수 있습니다. awverify CNAME이 전파될 때가지 A 레코드에서 웹 사이트에 대해 정의한 사용자 지정 도메인 이름을 설정할 수 없습니다. [http://www.digwebinterface.com/][11] 등의 서비스를 사용하여 CNAME을 사용할 수 있는지 확인할 수 있습니다.
