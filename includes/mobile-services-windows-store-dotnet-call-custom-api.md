...
4.  **MainPage** 클래스에 다음 메서드를 추가합니다.
    ...
    이 메서드는 새 단추의 **Click** 이벤트를 처리합니다. [InvokeApiAsync][InvokeApiAsync](영문) 메서드가 클라이언트에서 호출되어 POST 요청을 새 사용자 지정 API로 보냅니다. 사용자 지정 API에서 반환하는 결과는 오류와 마찬가지로 메시지 대화 상자에 표시됩니다.
