...
* RA-GRS(읽기 액세스 지역 중복 스토리지)는 GRS를 기반으로 합니다. RA-GRS는 데이터를 보조 지역의 다른 데이터 센터에 복제하고, 보조 지역에서 읽을 수 있는 옵션도 제공합니다. RA-GRS를 사용하면 Microsoft에서 주 지역에서 보조 지역으로 장애 조치(failover)를 시작하는지 여부에 관계없이 보조 지역에서 읽을 수 있습니다.
...
주 지역 및 보조 지역에서 모두 별도의 장애 도메인에서 복제본을 관리하고, 스토리지 배율 단위 내에서 도메인을 업그레이드합니다. 스토리지 배율 단위는 데이터 센터 내의 기본 복제 단위입니다. 이 수준의 복제는 LRS에서 제공 됩니다. 자세한 내용은 LRS (로컬 [중복 저장소)를 참조 하세요. Azure Storage에 대한 저렴한 데이터 중복성](../articles/storage/common/storage-redundancy-lrs.md)을 참조하세요.

사용할 복제 옵션을 결정할 때 다음 사항에 유의하세요.

* GZRS (지역 중복 저장소) (미리 보기)는 세 개의 Azure 가용성 영역에서 데이터를 동기적으로 복제 한 다음 데이터를 보조 지역에 비동기적으로 복제 하 여 고가용성과 함께 고가용성을 제공 합니다. 보조 지역에 대 한 읽기 액세스를 사용 하도록 설정할 수도 있습니다. GZRS는 지정 된 연도 동안 최소 99.99999999999999% (16 9의) 개체 내 구성을 제공 하도록 설계 되었습니다. GZRS에 대 한 자세한 내용은 고가용성 [및 최대 내구성 (미리 보기)을 위한 지리적 영역 중복 저장소](../articles/storage/common/storage-redundancy-gzrs.md)를 참조 하세요.
* ZRS (영역 중복 저장소)는 세 개의 Azure 가용성 영역에서 동기 복제를 사용 하 여 고가용성을 제공 하며, GRS 또는 RA-GRS 보다는 일부 시나리오에서 더 나은 선택이 될 수 있습니다. ZRS에 대한 자세한 내용은 [ZRS](../articles/storage/common/storage-redundancy-zrs.md)를 참조하세요.
...
* 응용 프로그램이 보조 지역에서 읽어야 하는 경우 RA-GRS 또는 RA-GZRS (미리 보기)를 사용 하도록 설정 합니다.
