...
* 연결을 열기 전에 `client` 개체를 만들고 `dtmi:com:example:Thermostat;1` 모델 ID를 설정합니다. IoT Central은 모델 ID를 사용하여 이 디바이스에 대한 디바이스 템플릿을 식별하거나 생성합니다. 자세한 내용은 [디바이스 템플릿과 디바이스 연결](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template)을 참조하세요.
* 명령 처리기를 만듭니다.
