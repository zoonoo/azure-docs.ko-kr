...
2. Government 포털에서 **+** 를 클릭한 다음 Marketplace에서 **모두 보기**를 클릭합니다. _StorSimple 실제_를 검색합니다. **StorSimple 실제 디바이스 시리즈**를 선택하여 클릭한 후 **만들기**를 클릭합니다. 또는 Government 포털에서 **+** 를 클릭한 다음 **스토리지** 아래에서 **StorSimple 물리적 디바이스 시리즈**를 클릭합니다.
3. **StorSimple 디바이스 관리자** 블레이드에서 다음 단계를 수행합니다.
   ...
   4. 서비스의 **위치** 를 지정합니다. 위치는 디바이스를 배포하려는 지리적 영역을 가리킵니다. **미 정부 아이오와** 또는 **미 정부 버지니아**를 선택합니다.
   5. **새 스토리지 계정 만들기** 를 선택하여 서비스를 포함하는 스토리지 계정을 자동으로 만듭니다. 이 스토리지 계정에 대해 이름을 지정합니다. 다른 위치에 있는 데이터가 필요하면 확인란의 선택을 취소합니다.
   6. 대시보드에서 이 서비스에 대한 빠른 링크가 필요한 경우 **대시보드에 고정**을 확인하세요.
   7. **만들기**를 클릭하여 StorSimple 디바이스 관리자를 만듭니다. 서비스 생성은 몇 분 정도가 소요됩니다. 서비스가 성공적으로 만들어지면 알림이 표시되고 새 서비스 블레이드가 열립니다.
