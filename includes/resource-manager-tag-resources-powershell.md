...
Azure PowerShell 2.0의 2016년 8월 릴리스에서는 태그를 사용하는 방식이 크게 변경되었습니다. 계속하기 전에 AzureRm.Resources 모듈 버전을 확인합니다.
...
최신 릴리스에서 **Tags** 매개 변수 이름이 **Tag**로, 형식이 `Hashtable[]`에서 `Hashtable`로 변경되었습니다. 각 항목에 대해 더 이상 **이름** 및 **값**을 제공할 필요가 없습니다. 대신 키-값 쌍을 **키 = "값"**형식으로 제공합니다.
...
태그를 하나 이상 제거하려는 경우에는 제거하려는 태그가 없는 배열을 저장합니다.
