...
4. 수행할 동작으로 **SharePoint Online - 파일 만들기** 를 선택합니다.   **참고**: 논리 앱에 SharePoint 계정에 액세스하기 위한 권한을 아직 부여하지 않았으면 이러한 권한을 부여하라는 메시지가 표시됩니다.    
...
8. **파일 이름** 컨트롤을 선택하고 만들려는 파일의 이름을 입력합니다. 파일 이름을 입력할 때 제공된 목록에서 선택하기만 하면 이전에 만든 트리거의 모든 속성을 사용할 수 있습니다.     
