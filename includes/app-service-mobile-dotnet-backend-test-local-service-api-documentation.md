...
3. .NET 백 엔드 시작 페이지에서 **체험**을 클릭합니다.
   ...
   > [!NOTE]
   > 프로젝트를 로컬로 실행할 때는 인증하지 않아도 이 페이지에 액세스할 수 있습니다. 그러나 Azure에서 실행할 때는 이 페이지에 액세스하려면 응용 프로그램 키를 암호로 입력해야 합니다(사용자 이름은 없음).
   > 
   > 
...
5. **체험**을 클릭한 후 **보내기**를 클릭합니다.
    ...
    그러면 TodoItem 테이블의 모든 행을 반환하는 GET 요청을 로컬 모바일 앱 백 엔드로 보냅니다. 테이블은 이니셜라이저가 시드하므로 응답 메시지 본문에서는 TodoItem 개체 두 개가 반환됩니다. 이니셜라이저에 대한 자세한 내용은 [.NET 백 엔드 모바일 서비스에 대한 데이터 모델 변경 방법](../articles/mobile-services/mobile-services-dotnet-backend-how-to-use-code-first-migrations.md)을 참조하세요.
