...
5.  저장소 에뮬레이터(로컬 컴퓨터에서 시뮬레이트된 Microsoft Azure 저장소)를 대상으로 지정할지 또는 클라우드의 저장소 계정을 대상으로 지정할지 선택합니다. 이 가이드의 코드는 두 옵션 중 어떤 옵션을 사용하든 작동합니다.
	...
	클라우드에서 저장소 계정을 대상으로 하는 경우에 해당 저장소 계정에 대한 기본 액세스 키를 입력합니다. Azure 관리 포털을 통해 기본 액세스 키를 복사 하는 방법을 알아보려면 [저장소 액세스 키 보기, 복사 및 다시 생성](storage-create-storage-account.md#view-copy-and-regenerate-storage-access-keys)을 참조하세요.
