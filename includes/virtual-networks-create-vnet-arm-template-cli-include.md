...
3. 필요에 따라 아래와 같이 **`azure group create`**를 실행하여 새 리소스 그룹을 만듭니다. 명령의 출력을 확인합니다. 출력 다음에 표시되는 목록은 사용되는 매개 변수를 설명합니다. 리소스 그룹에 대한 자세한 내용은 [Azure 리소스 관리자 개요](resource-group-overview.md)를 참조하세요.
...
4. 위에서 다운로드하고 수정한 템플릿 및 매개 변수를 사용하여 새 VNet을 배포하기 위해 **`azure group deployment create`** cmdlet을 실행합니다. 출력 다음에 표시되는 목록은 사용되는 매개 변수를 설명합니다.
