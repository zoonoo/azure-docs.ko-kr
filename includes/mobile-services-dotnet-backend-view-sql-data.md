...
1. [Azure 클래식 포털](https://manage.windowsazure.com/)에서 모바일 서비스에 연결된 데이터베이스에 대한 관리를 클릭합니다.
   
    ![SQL 데이터베이스 관리를 위해 로그인](./media/mobile-services-dotnet-backend-view-sql-data/manage-sql-azure-database.png)
2. 클래식 포털에서 쿼리를 실행하여 Windows 스토어 앱에 의한 변경 사항을 확인합니다. 쿼리는 다음 쿼리와 유사하지만 <code>todolist</code> 대신 데이터베이스 이름을 사용합니다.</p>
