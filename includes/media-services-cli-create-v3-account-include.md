...
다음 명령은 Media Services 계정과 연결될 스토리지 계정을 만듭니다. 아래 스크립트에서 `storageaccountforams`를 원하는 값으로 대체할 수 있습니다. `amsResourceGroup` 이전 단계에서 리소스 그룹에 대해 제공한 값을 일치 해야 합니다. 저장소 계정 이름은 24 보다 작은 길이 있어야 합니다.
