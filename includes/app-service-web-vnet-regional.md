...
* 앱과 동일한 리전의 VNet의 리소스입니다.
* 앱이 통합된 VNet을 피어로 하는 VNet의 리소스입니다.
* 서비스 엔드포인트 보안 서비스.
...
모든 아웃바운드 트래픽을 VNet으로 라우팅하는 경우 통합 서브넷에 적용되는 NSG 및 DR의 적용을 받습니다. 모든 아웃바운드 트래픽을 VNet으로 라우팅할 때 트래픽을 다른 곳으로 보낼 경로를 제공하지 않는 한 아웃바운드 주소는 앱 속성에 나열된 아웃바운드 주소입니다.
...
* 이 기능에는 Azure 리소스 관리자 VNet에서 주소가 32개 이상인 /27인 사용하지 않은 서브넷이 필요합니다.
* 앱과 VNet은 동일한 지역에 있어야 합니다.
* 통합 된 응용 프로그램으로 VNet을 삭제할 수 없습니다. VNet을 삭제하기 전에 통합을 제거합니다.
...
다른 계획의 앱이 다른 계획의 앱에 의해 이미 연결된 VNet에 도달하도록 하려면 기존 VNet 통합에서 사용하는 것과 다른 서브넷을 선택합니다.
...
지역 별 VNet 통합을 사용하면 서비스 끝점을 사용할 수 있습니다. 앱에서 서비스 끝점을 사용하려면 지역 VNet 통합을 사용하여 선택한 VNet에 연결한 다음 통합에 사용한 서브넷의 대상 서비스로 서비스 끝점을 구성합니다. 그런 다음 서비스 끝점을 통해 서비스에 액세스하려는 경우:

1. 웹 앱으로 지역 VNet 통합 구성
1. 대상 서비스로 이동하여 통합에 사용되는 서브넷에 대해 서비스 끝점을 구성합니다.

### <a name="network-security-groups"></a>네트워크 보안 그룹

네트워크 보안 그룹을 사용하여 VNet의 리소스에 대한 인바운드 및 아웃바운드 트래픽을 차단할 수 있습니다. 지역 VNet 통합을 사용하는 앱은 [네트워크 보안 그룹을][VNETnsg] 사용하여 VNet 또는 인터넷의 리소스에 대한 아웃바운드 트래픽을 차단할 수 있습니다. 공용 주소에 대한 트래픽을 차단하려면 응용 프로그램 설정을 WEBSITE_VNET_ROUTE_ALL 설정해야 합니다. VNet 통합은 앱의 아웃바운드 트래픽에만 영향을 미치기 때문에 NSG의 인바운드 규칙은 앱에 적용되지 않습니다.
