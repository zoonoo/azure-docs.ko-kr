...
ms.locfileid: "103467997"
---
- 방화벽 및 가상 네트워크 구성을 사용 하 여 저장소 계정에 액세스 하려면 저장소 계정에서 네트워크 액세스를 제한 해야 합니다.
- Automation 계정 변수는 암호화 되어야 합니다. Automation 계정 변수는 암호화 되어야 합니다.
...
- Redis Cache에 대 한 보안 연결만 사용 하도록 설정 해야 합니다. Redis에 대 한 Azure 캐시에 대 한 보안 연결만 사용 하도록 설정 해야 합니다.
- 컨테이너 AppArmor 프로필의 재정의 또는 비활성화를 제한해야 함
- 권한 있는 컨테이너를 피해야 함
- 컨테이너를 루트 사용자로 실행하지 않아야 함
- 저장소 계정에 안전 하 게 전송 하려면 저장소 계정에 대 한 보안 전송을 사용 하도록 설정 해야 합니다.
- Service Fabric 클러스터는 ClusterProtectionLevel 속성을 EncryptAndSign로 설정 해야 Service Fabric 클러스터에서 ClusterProtectionLevel 속성을 EncryptAndSign로 설정 해야 합니다.
- Service Fabric 클러스터는 클라이언트 인증을 위해 Azure Active Directory만 사용 해야 Service Fabric 클러스터는 클라이언트 인증에 Azure Active Directory만 사용 해야 합니다.
- 서비스는 허용되는 포트에서만 수신 대기해야 함
- 스토리지 계정 공용 액세스가 허용되지 않아야 함
- 저장소 계정은 새 Azure Resource Manager 리소스에 마이그레이션해야 합니다. 리소스 저장소 계정은 새 Azure Resource Manager 리소스로 마이그레이션해야 합니다.
...
- 가상 컴퓨터를 새 Azure Resource Manager 리소스로 마이그레이션해야 합니다. 가상 컴퓨터를 새 Azure Resource Manager 리소스로 마이그레이션해야 합니다.
