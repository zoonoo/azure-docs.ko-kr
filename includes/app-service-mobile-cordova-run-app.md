...
1. [Azure 포털]을 방문합니다. **모두 찾아보기** > **Mobile Apps** > 방금 만든 백 엔드를 클릭합니다. 모바일 앱 설정에서 **빠른 시작** > **Cordova**를 클릭합니다. **클라이언트 응용 프로그램 구성**에서 **새 앱 만들기**를 선택한 다음 **다운로드**를 클릭합니다. 그러면 사전 구성된 앱을 백 엔드에 연결하기 위한 전체 Cordova 프로젝트가 다운로드됩니다.
...
5. 장치 또는 에뮬레이터에서 앱을 시작한 후, **새 텍스트 입력**에 *자습서 완료*와 같은 의미 있는 텍스트를 입력한 다음 **추가** 단추를 클릭입니다.  
   그러면 이전에 배포한 Azure 백 엔드에 POST 요청을 보냅니다. 요청의 백 엔드 데이터가 SQL 데이터베이스의 TodoItem 테이블에 삽입되고 새로 저장된 항목에 대한 정보를 모바일 앱으로 다시 반환합니다. 모바일 앱이 이 데이터를 목록에 표시합니다.
