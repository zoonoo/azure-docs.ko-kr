...
| 호스트 보류 할당 취소   | Azure에서 호스트를 다시 정상 상태로 복원할 수 없으며이 호스트에서 가상 컴퓨터를 다시 배포 하 라는 메시지가 표시 됩니다. `autoReplaceOnFailure` 사용 하도록 설정 된 경우 가상 머신은 정상 하드웨어로 *서비스를 치료* 합니다. 그렇지 않으면 가상 컴퓨터가 실패 하는 호스트에서 실행 중일 수 있습니다.|
