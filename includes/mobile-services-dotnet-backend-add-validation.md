...
1. **관리자 권한으로 실행** 옵션으로 Visual Studio를 시작하고, [시작] 또는 [데이터 시작](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md) 자습서에서 작업한 모바일 서비스 프로젝트가 포함된 솔루션을 엽니다.
...
5. 서비스 프로젝트를 마우스 오른쪽 단추로 클릭하고 **게시**를 클릭합니다. 이전에 [시작] 또는 [데이터 시작](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md) 자습서에서 사용한 게시 설정으로 Microsoft Azure 계정에 모바일 서비스를 게시합니다.
 
     >[AZURE.NOTE] 또는 IIS Express에서 서비스를 로컬로 호스트하여 테스트할 수도 있습니다. 자세한 내용은 [데이터 시작](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md) 자습서를 참조하세요.
