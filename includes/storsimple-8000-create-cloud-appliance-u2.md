...
3. **클라우드 어플라이언스 만들기** 블레이드에서 다음 정보를 지정합니다.
   ...
   2. **모델** - 클라우드 어플라이언스의 모델을 선택합니다. 8010 디바이스는 30TB의 표준 저장소를 제공하는 반면, 8020에는 64TB의 Premium Storage가 있습니다. 백업에서 항목 수준 검색 시나리오를 배포하려면 8010을 지정합니다. 성능이 높고 대기 시간이 짧은 워크로드를 배포하거나 재해 복구용 보조 디바이스로 사용하려면 8020을 선택합니다.
