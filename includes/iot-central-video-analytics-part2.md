...
배포가 완료되면 Cloud Shell을 열고 다음 명령을 실행하여 미디어 서비스 계정에 대한 **리소스 ID** 를 검색합니다.

```azurecli
az resource list --resource-group lva-rg --resource-type microsoft.media/mediaservices --output table --query "[].{ResourceID:id}"
```

:::image type="content" source="media/iot-central-video-analytics-part2/get-resource-id.png" alt-text="Cloud Shell을 사용하여 리소스 ID 가져오기":::

*scratchpad.txt* 파일에 **리소스 ID** 를 기록해 둡니다. 나중에 IoT Edge 모듈을 구성할 때 이 값을 사용합니다.
...
1. **만들기** 를 선택합니다.
    ...
    :::image type="content" source="./media/iot-central-video-analytics-part2/new-application.png" alt-text="Azure IoT Central 애플리케이션 페이지":::
...
:::image type="content" source="./media/iot-central-video-analytics-part2/administration.png" alt-text="스크린샷은 애플리케이션 URL 및 애플리케이션 ID가 강조 표시된 비디오 분석 페이지의 관리 창을 보여줍니다.":::

**API 토큰** 을 선택하고 **운영자** 역할에 대한 **LVAEdgeToken** 이라는 새 토큰을 생성합니다.

:::image type="content" source="./media/iot-central-video-analytics-part2/token.png" alt-text="토큰 생성":::
