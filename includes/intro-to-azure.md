...
Azure는 다양한 방법으로 활용할 수 있는 공용 클라우드용 Microsoft 응용 프로그램 플랫폼입니다.

-   Azure 데이터 센터에서 데이터를 실행하고 저장하는 웹 응용 프로그램을 빌드할 수 있습니다.
-   이러한 응용 프로그램으로 데이터를 저장할 수 있으며, 이 데이터를 사용하는 응용 프로그램은 온-프레미스(즉, 공용 클라우드 외부)에서 실행할 수 있습니다.
-   Azure를 사용하면 개발 및 테스트용 가상 컴퓨터를 만들거나 SharePoint 및 기타 응용 프로그램을 실행할 수 있으며,
-   아주 많은 사용자가 사용할 수 있도록 대규모로 확장 가능한 응용 프로그램을 구축할 수도 있습니다.

이 모든 기능과 더 많은 가능성은 이 플랫폼에서 제공하는 넓은 영역의 서비스에서 비롯합니다. 하지만 이러한 기능을 사용하려면 기본적인 면을 먼저 이해해야 합니다. 클라우드 컴퓨팅에 관하여 전혀 모르는 상태라고 해도 걱정하지 마세요. 이 문서를 통해 Azure의 기본 사항을 안내해 드리겠습니다. 이 문서에서는 클라우드 플랫폼을 이해하고 사용하는 데 대한 기본적인 면을 설명합니다.

**이 문서를 읽는 방법**

Azure에는 상당수의 서비스가 있고 항상 증가하고 있으므로 쉽게 과부하될 수 있습니다. 기본 서비스는 백서 앞부분에 나와 있습니다. 해당 서비스부터 먼저 살펴본 다음 추가 서비스에 대해 알아보세요. 그러나 추가 서비스만 단독으로 사용할 수 없다는 의미는 아니며 단지 기본 서비스가 Azure에서 실행되는 응용 프로그램의 핵심을 구성한다는 것입니다.

**사용자 의견 제공**

이 문서에서는 Azure의 개요를 효과적으로 제공하기 위해 작성되었습니다. 그렇지 않다고 생각되면 페이지 아래에 있는 의견 섹션에 의견을 남겨 주세요. 이 페이지에 추가하고 싶은 내용이나 개선 방법에 대해 자세히 써 주세요. Microsoft는 사용자의 의견을 소중하게 생각합니다.
...
## <span id="components"></span></a>Azure 구성 요소

Azure는 [Windows Azure 인포그래픽][]과 같은 다양한 시각적 보조 기능 및 관리 포털의 범주로 서비스를 그룹화합니다. 관리 포털은 전부는 아니지만 대부분의 Aure 서비스를 관리하는 포털입니다. 이 백서에서는 **다른 조직**을 사용하여 비슷한 기능을 기반으로 하는 서비스에 대해 설명하고 대규모 서비스를 구성하는 중요 하위 서비스에 대해서도 살펴봅니다.

![Azure 구성 요소][1]
 **그림 1: Azure는 Azure 데이터 센터에서 실행되는 인터넷 액세스가 가능한 응용 프로그램 서비스를 제공합니다.**

## <span id="portal"></span></a>관리 포털

Azure에는 관리자가 대부분의 Azure 기능(일부 기능 제외)에 액세스하여 관리할 수 있는 관리 포털이라고 하는 웹 인터페이스가 있습니다. Microsoft는 일반적으로 이전 버전을 사용 중지하기 전에 베타 버전으로 최신 UI 포털을 릴리스합니다. 최신 버전은 "미리 보기 포털"이라고 합니다. 두 포털 간에는 보통 몇 가지 겹치는 부분이 있습니다.

## <span id="compute"></span></a>계산
...
![Azure 가상 컴퓨터][]
**그림 2: Azure 가상 컴퓨터에서 클라우드의 가상 컴퓨터 인스턴스를 완벽하게 관리할 수 있습니다.**
...
VM을 작성하려면 사용할 VHD와 VM의 크기를 지정한 다음 VM을 실행한 시간만큼 결제합니다. VM이 실행되는 시간만 분 단위로 결제하지만 VHD 가용성을 유지하기 위한 최소 저장소 비용도 청구됩니다. Azure는 바로 시작할 수 있는 부팅 가능한 운영 체제를 포함한 VHD(또는 "이미지")의 갤러리를 제공합니다. 여기에는 Windows Server, Linux, SQL Server, Oracle 등 Microsoft와 파트너 옵션이 포함됩니다. 자유롭게 VHD와 이미지를 만들어 직접 업로드할 수 있습니다. 데이터만 포함하는 VHD를 업로드한 다음 실행 중인 VM에서 액세스할 수도 있습니다.
...
![Azure 웹 사이트][]
 **Azure 웹 사이트에서는 기본 웹 서버를 관리하지 않으면서 클라우드에서 웹 사이트 응용 프로그램을 실행합니다.**
...
개발자를 위해서는 관계형 저장소용 SQL 데이터베이스와 MySQL(Microsoft 파트너의 경우 ClearDB)과 함께 .NET, PHP, Node.js, Python 등을 지원합니다. 또한 WordPress, Joomla, Drupal 등의 여러 인기 있는 응용 프로그램을 기본적으로 지원합니다. Azure 웹 사이트의 목표는 경제적이며 확장 가능하고 매우 유용한 플랫폼을 제공하여 공용 클라우드에 웹 사이트와 웹 응용 프로그램을 생성할 수 있도록 하는 것입니다.
...
![Azure 클라우드 서비스][]
**Azure 클라우드 서비스는 PaaS(Platform as a Service) 환경에서 확장성이 높은 사용자 지정 코드를 실행할 수 있는 곳입니다.**
...
![Azure 저장소 SQL 데이터베이스][]
**Azure SQL 데이터베이스는 클라우드에서 관리되는 관계형 데이터베이스 서비스를 제공합니다.**
...
![Azure 저장소 테이블][]
**Azure 테이블은 데이터를 저장할 수 있는 단순한 NoSQL 방법을 제공합니다.**
...
이제 Azure의 전체적인 모습에 대해 배워 보았습니다. 다음 단계에서는 첫 Azure 응용 프로그램을 작성해 보겠습니다. 사용할 언어를 선택한 후 [적절한 SDK로][] 시작해 보세요. 클라우드 컴퓨팅은 새로운 표준입니다. 지금 시작하세요.
  ...
  [Azure 파일 서비스]: ./media/intro-to-azure/FileServiceIntroNew.png
  [가상 네트워크]: ./media/intro-to-azure/VirtualNetworkIntroNew.png
  [2]: http://msdn.microsoft.com/library/azure/jj156007.aspx
  [Express 경로]: ./media/intro-to-azure/ExpressRouteIntroNew.png
  [Express 경로 기술 개요]: http://msdn.microsoft.com/en-us/library/azure/dn606309.aspx
  ...
  [알림 허브]: ./media/intro-to-azure/NotificationHubstIntroNew.png
  [Azure 백업]: ./media/intro-to-azure/AzureBackupIntroNew.png
  [Azure 백업 필수 구성 요소를 확인]: http://technet.microsoft.com/en-us/library/dn296608.aspx
  [<link>]: http://msdn.microsoft.com/library/azure/hh767287.aspx "Azure 큐 및 서비스 버스 큐 - 비교 및 대조"
  [Azure 서비스 버스 릴레이]: ./media/intro-to-azure/ServiceBusRelayIntroNew.png
  [Azure 서비스 버스 토픽]: ./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png
  [BizTalk 서비스]: ./media/intro-to-azure/BizTalkServIntroNew2.png
  [Azure 스케줄러]: ./media/intro-to-azure/SchedulerIntroNew2.png
  [Azure 캐싱]: ./media/intro-to-azure/AzureCacheIntroNew.png
