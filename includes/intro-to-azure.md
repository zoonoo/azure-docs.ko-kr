...
Azure는 [Windows Azure 인포그래픽][Windows Azure 인포그래픽]과 같은 다양한 시각적 보조 기능 및 관리 포털의 범주로 서비스를 그룹화합니다. 관리 포털은 전부는 아니지만 대부분의 Aure 서비스를 관리하는 포털입니다.
...
Azure에는 관리자가 대부분의 Azure 기능(일부 기능 제외)에 액세스하여 관리할 수 있는 [관리 포털][2]이라고 하는 웹 인터페이스가 있습니다. Microsoft는 일반적으로 이전 버전을 사용 중지하기 전에 베타 버전으로 최신 UI 포털을 릴리스합니다. 최신 버전은 ["Azure Preview 포털"]["Azure Preview 포털"]이라고 합니다.
 ...
 **그림: Azure 웹 사이트에서는 기본 웹 서버를 관리하지 않으면서 클라우드에서 웹 사이트 응용 프로그램을 실행합니다.**
...
![Azure 저장소 SQL 데이터베이스][Azure 저장소 SQL 데이터베이스]
...
![Azure 가져오기 내보내기 서비스][Azure 가져오기 내보내기 서비스]
...
사용자에게 적합한 가상 네트워크를 계획 및 생성에 대한 자세한 내용은 [가상 네트워크][가상 네트워크]를 참조하세요.

### Express 경로

![Express 경로][Express 경로]
...
Express 경로에 대한 자세한 내용은 [Express 경로 기술 개요][Express 경로 기술 개요]를 참조하세요.
...
이러한 SDK는 Azure 응용 프로그램의 구축, 배포, 관리 등을 돕습니다. SDK는 [www.microsoftazure.com][www.microsoftazure.com] 또는 GitHub에 있으며, Visual Studio 및 Eclipse와 함께 사용할 수 있습니다. 또한 Azure는 개발자가 어느 편집기나 개발 환경에서 사용할 수 있는 명령줄 도구를 제공하며, 여기에는 Linux나 Macintosh 시스템의 응용 프로그램을 Azure로 배포하는 도구가 포함됩니다.
...
Windows Server 또는 System Center를 이미 사용하는 경우 Azure 백업은 서버 파일 시스템, 가상 컴퓨터 및 SQL Server 데이터베이스를 백업하는 데 사용할 수 있는 자연스러운 솔루션입니다. 이 솔루션은 암호화된, 스파스 및 압축 파일에 대해 작업합니다. 몇 가지 제한 사항이 있으므로 먼저 [Azure 백업 필수 구성 요소를 확인][Azure 백업 필수 구성 요소를 확인]해야 합니다.
...
![Azure 서비스 버스 릴레이][Azure 서비스 버스 릴레이]
**그림: 큐는 응용 프로그램 내의 부분 간 연결을 느슨하게 하여 크기 조정을 용이하게 해 줍니다.**

큐는 간단한 아이디어입니다. 응용 프로그램이 어떤 메시지를 큐에 보내면 다른 응용 프로그램이 그 메시지를 읽게 됩니다. 응용 프로그램에 이처럼 단순한 서비스만 필요하다면 Azure 큐가 가장 적합한 선택이 될 것입니다.

시간 경과에 따라 Azure가 증가하는 방식으로 인해 Azure 저장소 큐 및 서비스 버스 큐는 유사한 큐 서비스를 제공합니다. 서비스 간의 비교 내용은 기술 문서인 [<link>][<link>]에서 다루었습니다. 많은 시나리오에서 두 가지 서비스가 모두 작동합니다.
...
![Azure 서비스 버스 토픽][Azure 서비스 버스 토픽]
...
이제 Azure의 전체적인 모습에 대해 배워 보았습니다. 다음 단계에서는 첫 Azure 응용 프로그램을 작성해 보겠습니다. 사용할 언어를 선택한 후 [적절한 SDK로][적절한 SDK로] 시작해 보세요. 클라우드 컴퓨팅은 새로운 표준입니다. 지금 시작하세요.
  ...
  [1]: ./media/intro-to-azure/AzureComponentsIntroNew780.png
  ...
  [Express 경로 기술 개요]: http://msdn.microsoft.com/ko-kr/library/azure/dn606309.aspx
  [TrafficManager]: ./media/intro-to-azure/TrafficManagerIntroNew.png
  [www.microsoftazure.com]: http://azure.microsoft.com/ko-kr/downloads/
  ...
  [알림 허브]: ./media/intro-to-azure/NotificationHubsIntroNew.png
  [Azure 백업]: ./media/intro-to-azure/AzureBackupIntroNew.png
  [Azure 백업 필수 구성 요소를 확인]: http://technet.microsoft.com/ko-kr/library/dn296608.aspx
  [Azure 서비스 버스 릴레이]: ./media/intro-to-azure/QueuesIntroNew.png
  [<link>]: http://msdn.microsoft.com/library/azure/hh767287.aspx "Azure 큐 및 서비스 버스 큐 - 비교 및 대조"
  [3]: ./media/intro-to-azure/ServiceBusRelayIntroNew.png
  [Azure 서비스 버스 토픽]: ./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png
  [BizTalk 서비스]: ./media/intro-to-azure/BizTalkServicesIntroNew.png
  [Azure 스케줄러]: ./media/intro-to-azure/SchedulerIntroNew.png
